...
namespace UE::Dataflow
{
	namespace Private
	{
		...
		inline void NotifySimulationEnabledChanged(const TObjectPtr<UObject>& SimulationWorld, const bool bSimulationEnabled)
		{
			if (SimulationWorld)
			{
				TArray<AActor*> Actors;
				...
				for (AActor* CurrentActor : Actors)
				{
					IDataflowSimulationActor::Execute_OnDataflowSimulationEnabledChanged(CurrentActor, bSimulationEnabled);
				}
			}
		}
	}
...
void UDataflowSimulationManager::SetSimulationEnabled(const bool bSimulationEnabled)
{
	if (bIsSimulationEnabled != bSimulationEnabled)
	{
		bIsSimulationEnabled = bSimulationEnabled;
		UE::Dataflow::Private::NotifySimulationEnabledChanged(GetWorld(), bSimulationEnabled);
	}
}

void UDataflowSimulationManager::NotifyActorOfCurrentSimulationEnabled(AActor* PreviewActor) const
{
	if (PreviewActor && PreviewActor->Implements<UDataflowSimulationActor>())
	{
		IDataflowSimulationActor::Execute_OnDataflowSimulationEnabledChanged(PreviewActor, bIsSimulationEnabled);
	}
}
