...
FIOSAudioSoundBuffer::~FIOSAudioSoundBuffer(void)
{
	...
	{
		UE_LOGF(LogIOSAudio, Fatal, "Can't free resource '%ls' as it was allocated in permanent pool.", *ResourceName);
	}
...
bool FIOSAudioSoundBuffer::ReadCompressedInfo(USoundWave* InWave)
{
	...
	InWave->InitAudioResource(FName("ADPCM"));
...
FIOSAudioSoundBuffer* FIOSAudioSoundBuffer::Init(FIOSAudioDevice* IOSAudioDevice, USoundWave* InWave)
{
	...
	switch (static_cast<EDecompressionType>(InWave->DecompressionType))
	{
		...
		default:
			...
			UE_LOGF( LogIOSAudio, Warning, "Init Buffer on unsupported sound type name = %ls type = %d", *InWave->GetName(), int32(InWave->DecompressionType));
			break;
	}
