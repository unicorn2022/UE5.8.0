	...
	BusNumber(InBusNumber)
{
	...
	for (int32 Channel = 0; Channel < CHANNELS_PER_BUS; Channel++)
	{
		...
		UE_CLOGF(Status != noErr, LogIOSAudio, Error, "Failed to set kAudioUnitProperty_StreamFormat for audio mixer unit: BusNumber=%d, Channel=%d", BusNumber, Channel);
		...
		Status = AudioUnitSetParameter(IOSAudioDevice->GetMixerUnit(),
		                               ...
		                               1.0f,
		                               0);
		UE_CLOGF(Status != noErr, LogIOSAudio, Error, "Failed to set k3DMixerParam_Distance for audio mixer unit: BusNumber=%d, Channel=%d", BusNumber, Channel);

		Status = AUGraphSetNodeInputCallback(IOSAudioDevice->GetAudioUnitGraph(),
		                                     IOSAudioDevice->GetMixerNode(), 
		                                     GetAudioUnitElement(Channel),
		                                     &Input);
		UE_CLOGF(Status != noErr, LogIOSAudio, Error, "Failed to set input callback for audio mixer node: BusNumber=%d, Channel=%d", BusNumber, Channel);
	}
}
...
bool FIOSAudioSoundSource::Init(FWaveInstance* InWaveInstance)
{
	...
	{
		UE_LOGF(LogIOSAudio, Log, "Waiting for source to unlock");
	...
	for (int32 Channel = 0; Channel < IOSBuffer->NumChannels; ++Channel)
	{
		...
		Status = AudioUnitSetParameter(IOSAudioDevice->GetMixerUnit(),
		                               ...
		                               Pan,
		                               0);
		UE_CLOGF(Status != noErr, LogIOSAudio, Error, "Failed to set k3DMixerParam_Azimuth for audio mixer unit: BusNumber=%d, Channel=%d", BusNumber, Channel);
	}
...
void FIOSAudioSoundSource::Update(void)
{
	...
	if (IOSBuffer->NumChannels == 1 && WaveInstance->GetUseSpatialization())
	{
		...
		Status = AudioUnitSetParameter(IOSAudioDevice->GetMixerUnit(),
		                               ...
		                               Pan,
		                               0);
		UE_CLOGF(Status != noErr, LogIOSAudio, Error, "Failed to set k3DMixerParam_Azimuth for audio mixer unit: BusNumber=%d, Channel=%d", BusNumber, 0);
	}
	...
	for (int32 Channel = 0; Channel < IOSBuffer->NumChannels; Channel++)
	{
		Status = AudioUnitSetParameter(IOSAudioDevice->GetMixerUnit(),
		                               ...
		                               Gain,
		                               0);
		UE_CLOGF(Status != noErr, LogIOSAudio, Error, "Failed to set k3DMixerParam_Gain for audio mixer unit: BusNumber=%d, Channel=%d", BusNumber, Channel);
		...
		Status = AudioUnitSetParameter(IOSAudioDevice->GetMixerUnit(),
									   ...
									   PitchParam,
		                               0);
		UE_CLOGF(Status != noErr, LogIOSAudio, Error, "Failed to set k3DMixerParam_PlaybackRate for audio mixer unit: BusNumber=%d, Channel=%d", BusNumber, Channel);
	}
}
...
bool FIOSAudioSoundSource::AttachToAUGraph()
{
	...
	for (int32 Channel = 0; Channel < IOSBuffer->NumChannels; Channel++)
	{
		Status = AudioUnitSetParameter(IOSAudioDevice->GetMixerUnit(),
		                               ...
		                               1,
		                               0);
		UE_CLOGF(Status != noErr, LogIOSAudio, Error, "Failed to set k3DMixerParam_Enable for audio mixer unit: BusNumber=%d, Channel=%d", BusNumber, Channel);
	}
