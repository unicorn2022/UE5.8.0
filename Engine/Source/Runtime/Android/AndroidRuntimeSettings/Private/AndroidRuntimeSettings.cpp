...
UAndroidRuntimeSettings::UAndroidRuntimeSettings(const FObjectInitializer& ObjectInitializer)
	...
	, bPackageForOpenXRImmersive(false)
...
{
	const bool SupportssRGB = bPackageForMetaQuest || bPackageForOpenXRImmersive;
...
void UAndroidRuntimeSettings::HandleXRSupport()
{
	const FString SupportedDevicesTag("<meta-data android:name=\"com.oculus.supportedDevices\"");
	...
	if (bPackageForMetaQuest)
	{
		if (!bBuildForArm64)
		{
			...
			UE_LOGF(LogAndroidRuntimeSettings, Warning, "Enabled Package for Meta Quest devices. Support arm64 has been changed to true.\n");
		}
...
void UAndroidRuntimeSettings::RemoveExtraApplicationTag(FString TagToRemove)
{
	...
	if (StartIndex != INDEX_NONE)
	{
		...
		{
			UE_LOGF(LogAndroidRuntimeSettings, Warning, "RemoveExtraApplicationTag found what looks like corrupt xml, cannot remove tag.\n");
		}
		else
		{
...
void UAndroidRuntimeSettings::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyChangedEvent.Property != nullptr 
		&& (PropertyChangedEvent.Property->GetName().StartsWith(TEXT("bPackageForMetaQuest")) || PropertyChangedEvent.Property->GetName().StartsWith(TEXT("bPackageForOpenXRImmersive"))))
	{
		HandleXRSupport();
	}
...
void UAndroidRuntimeSettings::EnsureValidGPUArch()
{
	...
	{
		UE_LOGF(LogAndroidRuntimeSettings, Warning, "No GPU architecture is selected.\n");
		// Default to Vulkan for XR devices
		if (bPackageForMetaQuest || bPackageForOpenXRImmersive)
		{
			...
			UE_LOGF(LogAndroidRuntimeSettings, Warning, "Support Vulkan has been changed to true.\n");
		}
		else
		{
			bBuildForES31 = true;
			...
			UE_LOGF(LogAndroidRuntimeSettings, Warning, "Support OpenGL ES3.2 has been changed to true.\n");
		}
