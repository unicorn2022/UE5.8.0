...
{
	return Duration > FTimespan::Zero() ? (Time >= FTimespan::Zero() ? (Time % Duration) : (Duration + (Time % Duration))) : Time;
}
...
TOptional<FTimespan> FMediaPlayerFacade::GetTrackDuration(EMediaTrackType TrackType, int32 TrackIndex) const
{
	TSharedPtr<IMediaPlayer, ESPMode::ThreadSafe> CurrentPlayer(Player);
	if (!CurrentPlayer.IsValid())
	{
	...
	return CurrentPlayer->GetTracks().GetTrackDuration((EMediaTrackType)TrackType, TrackIndex);
}
...
#if WITH_EDITOR
	// Record the successful open to analytics
	if (Player->ShouldAutoRecordSourceOpenedEvent())
	{
		Player->RecordSourceOpenedEvent();
	}
#endif
...
{
	UE_LOGF(LogMediaUtils, Verbose, "PlayerFacade: Flushing sinks");
...
IMediaPlayerFactory* FMediaPlayerFacade::GetPlayerFactoryForUrl(const FString& Url, const IMediaOptions* Options) const
{
	...
	if (MediaModule == nullptr)
	{
		UE_LOGF(LogMediaUtils, Error, "Failed to load Media module");
		return nullptr;
	}
	...
	if (PlayerName != NAME_None)
	{
		...
		if (Factory == nullptr)
		{
			UE_LOGF(LogMediaUtils, Error, "Could not find desired player %ls for %ls", *PlayerName.ToString(), *Url);
		}
	...
	{
		UE_LOGF(LogMediaUtils, Error, "Cannot play %ls: no media player plug-ins are installed and enabled in this project", *Url);
		return nullptr;
	}
	struct FCandidate
	{
	...
	//
	UE_LOGF(LogMediaUtils, Error, "Cannot play %ls, because none of the enabled media player plug-ins support it:", *Url);
	for (IMediaPlayerFactory* Factory : PlayerFactories)
	{
		if (Factory->SupportsPlatform(RunningPlatformName))
		{
			UE_LOGF(LogMediaUtils, Log, "| %ls (URI scheme or file extension not supported)", *Factory->GetPlayerName().ToString());
		}
		else
		{
			UE_LOGF(LogMediaUtils, Log, "| %ls (only available on %ls, but not on %ls)", *Factory->GetPlayerName().ToString(), *FString::Join(Factory->GetSupportedPlatforms(), TEXT(", ")), *RunningPlatformName);
		}
	}
	return nullptr;
}
...
void FMediaPlayerFacade::ProcessEvent(EMediaEvent Event, bool bIsBroadcastAllowed)
{
	...
	if ((Event == EMediaEvent::MediaOpened) || (Event == EMediaEvent::MediaOpenFailed))
	{
		...
		{
			UE_LOGF(LogMediaUtils, Verbose, "PlayerFacade: Media Info: n/a");
		}
		else
		{
			UE_LOGF(LogMediaUtils, Verbose, "PlayerFacade: Media Info:\n%ls", *MediaInfo);
		}
	}
		...
		if (Player->GetPlayerFeatureFlag(IMediaPlayer::EFeatureFlag::UsePlaybackTimingV2))
		{
			...
			while(1)
			{
				...
				if (!Player.IsValid() || bEventCancelsBlock)
				{
					...
					UE_LOGF(LogMediaUtils, Warning, "Blocking media playback closed or failed. Disabling it for this playback session.");
				...
				if ((FPlatformTime::Seconds() - BlockingStart) > static_cast<double>(UE::MediaUtils::Private::GBlockOnFetchTimeout))
				{
					...
					Url = Player->GetUrl();
#endif // !UE_BUILD_SHIPPING
					UE_LOGF(LogMediaUtils, Error, "Blocking media playback timed out on range [%ls (seq:%d, loop:%d), %ls (seq:%d, loop:%d)]. Disabling it for this playback session. URL:%ls",
	...
	if (!CurrentPlayer->GetPlayerFeatureFlag(IMediaPlayer::EFeatureFlag::UsePlaybackTimingV2))
	{
		...
		while(true)
		{
			...
			if (Blocked)
			{
				...
				{
					UE_LOGF(LogMediaUtils, Verbose, "PlayerFacade: Aborted block on fetch %ls after %i seconds",
			...
			{
				UE_LOGF(LogMediaUtils, VeryVerbose, "PlayerFacade: Blocking on fetch %ls", *BlockOnRange.GetRange().GetLowerBoundValue().Time.ToString(TEXT("%h:%m:%s.%t")));
...
void FMediaPlayerFacade::ExecuteNextSeek()
{
	...
	{
		UE_LOGF(LogMediaUtils, Verbose, "PlayerFacade: Internal Seek at %ls.", *nst.GetValue().ToString(TEXT("%h:%m:%s.%t")));

		if (!Seek(nst.GetValue()))
		{
			UE_LOGF(LogMediaUtils, Error, "PlayerFacade: Failed Internal Seek at %ls.", *nst.GetValue().ToString(TEXT("%h:%m:%s.%t")));
		}
	}
}
...
bool FMediaPlayerFacade::ProcessVideoSamples(IMediaSamples& Samples, const TRange<FMediaTimeStamp>& TimeRange)
{
	...
	if (Sample.IsValid())
	{
		...
		{
			UE_LOGF(LogMediaUtils, Verbose, "PlayerFacade: No Video Sample Sinks available, Dropping Sample %ls Sequence %d Loop %d",
				*SampleTime.GetTime().ToString(TEXT("%h:%m:%s.%t")), SampleTime.GetSequenceIndex(), SampleTime.GetLoopIndex());
		}
		else
		{
			UE_LOGF(LogMediaUtils, VeryVerbose, "PlayerFacade: Enqueuing Sample %ls Sequence %d Loop %d",
				*SampleTime.GetTime().ToString(TEXT("%h:%m:%s.%t")), SampleTime.GetSequenceIndex(), SampleTime.GetLoopIndex());
		}
#endif
	...
	else
	{
		while(Samples.FetchCaption(TimeRange, Sample))
		{
			if (Sample.IsValid() && !CaptionSampleSinks.Enqueue(Sample.ToSharedRef()))
			{
				...
				UE_LOGF(LogMediaUtils, VeryVerbose, "PlayerFacade: Caption sample sink overflow");
#endif
			}
		}
	}
...
void FMediaPlayerFacade::ProcessSubtitleSamples(IMediaSamples& Samples, const TRange<FMediaTimeStamp>& TimeRange)
{
	...
	{
		const bool bReverse = CurrentRate < 0.0f;
		TRange<FMediaTimeStamp> TempRange = bReverse ? TRange<FMediaTimeStamp>::AtLeast(TimeRange.GetUpperBoundValue()) : TRange<FMediaTimeStamp>::AtMost(TimeRange.GetUpperBoundValue());
		while(Samples.FetchSubtitle(TempRange, Sample))
		{ }
		if (Sample.IsValid() &&
			((!bReverse && ((Sample->GetTime() + Sample->GetDuration()) > TimeRange.GetLowerBoundValue())) ||
				(bReverse && ((Sample->GetTime() - Sample->GetDuration()) < TimeRange.GetLowerBoundValue()))))
		{
			//UE_LOGF(LogMediaUtils, Display, "Subtitle @%.3f: %ls", Sample->GetTime().Time.GetTotalSeconds(), *Sample->GetText().ToString());
			if (Sample.IsValid() && !SubtitleSampleSinks.Enqueue(Sample.ToSharedRef()))
			{
#if MEDIAPLAYERFACADE_TRACE_SINKOVERFLOWS
				UE_LOGF(LogMediaUtils, VeryVerbose, "PlayerFacade: Subtitle sample sink overflow");
#endif
			}
		}
	}
...
void FMediaPlayerFacade::ProcessMetadataSamples(IMediaSamples& Samples, const TRange<FMediaTimeStamp>& TimeRange)
{
	...
	else
	{
		while(Samples.FetchMetadata(TimeRange, Sample))
		{
			...
			{
#if MEDIAPLAYERFACADE_TRACE_SINKOVERFLOWS
				UE_LOGF(LogMediaUtils, VeryVerbose, "PlayerFacade: Metadata sample sink overflow");
#endif
			}
		}
	}
...
void FMediaPlayerFacade::ProcessAudioSamplesV1(IMediaSamples& Samples, TRange<FTimespan> TimeRange)
{
	...
	while(Samples.FetchAudio(TimeRange, Sample))
	{
		...
		{
#if MEDIAPLAYERFACADE_TRACE_SINKOVERFLOWS
			UE_LOGF(LogMediaUtils, VeryVerbose, "PlayerFacade: Audio sample sink overflow");
#endif
		}
		else
		{
...
void FMediaPlayerFacade::ProcessVideoSamplesV1(IMediaSamples& Samples, TRange<FTimespan> TimeRange)
{
	...
	while(Samples.FetchVideo(TimeRange, Sample))
	{
		...
		UE_LOGF(LogMediaUtils, VeryVerbose, "PlayerFacade: Fetched video sample %ls", *Sample->GetTime().Time.ToString(TEXT("%h:%m:%s.%t")));
		...
		if (VideoSampleSinks.Enqueue(Sample.ToSharedRef()))
		{
			if (CurrentRate >= 0.0f)
			{
				...
				UE_LOGF(LogMediaUtils, VeryVerbose, "PlayerFacade: Next video sample time %ls", *NextVideoSampleTime.ToString(TEXT("%h:%m:%s.%t")));
			}
		}
		else
		{
#if MEDIAPLAYERFACADE_TRACE_SINKOVERFLOWS
			UE_LOGF(LogMediaUtils, VeryVerbose, "PlayerFacade: Video sample sink overflow");
#endif
		}
	}
}
...
void FMediaPlayerFacade::ProcessSubtitleSamplesV1(IMediaSamples& Samples, TRange<FTimespan> TimeRange)
{
	...
	while(Samples.FetchSubtitle(TimeRange, Sample))
	{
		if (Sample.IsValid() && !SubtitleSampleSinks.Enqueue(Sample.ToSharedRef()))
		{
			...
			//UE_LOGF(LogMediaUtils, Log, "New caption @%.3f: %ls", Sample->GetTime().Time.GetTotalSeconds(), *Caption);
...
void FMediaPlayerFacade::ReceiveMediaEvent(EMediaEvent Event)
{
	if (Event >= EMediaEvent::Internal_Start)
	{
		switch (Event)
		{
			...
			{
				UE_LOGF(LogMediaUtils, VeryVerbose, "PlayerFacade: Video samples ARE available");
				VideoSampleAvailability = 1;
				break;
			}
			case EMediaEvent::Internal_VideoSamplesUnavailable:
			{
				UE_LOGF(LogMediaUtils, VeryVerbose, "PlayerFacade: Video samples are NOT available");
				VideoSampleAvailability = 0;
				break;
			}
			case EMediaEvent::Internal_AudioSamplesAvailable:
			{
				UE_LOGF(LogMediaUtils, VeryVerbose, "PlayerFacade: Audio samples ARE available");
				AudioSampleAvailability = 1;
				break;
			}
			case EMediaEvent::Internal_AudioSamplesUnavailable:
			{
				UE_LOGF(LogMediaUtils, VeryVerbose, "PlayerFacade: Audio samples are NOT available");
				AudioSampleAvailability = 0;
				break;
			}

			case EMediaEvent::Internal_RequestVideoSampleRefresh:
			{
				// Clear the "last sample still good" cache so the next TickInput re-fetches even if player is paused.
				UE_LOGF(LogMediaUtils, VeryVerbose, "PlayerFacade [%ls]: Request video sample refresh", *PlayerGuid.ToString());
				FScopeLock Lock(&TimeValueLock);
				LastVideoSampleProcessedTimeRange = TRange<FMediaTimeStamp>::Empty();
				break;
			}
			...
			default:
			{
				UE_LOGF(LogMediaUtils, VeryVerbose, "PlayerFacade: Received media event %ls", *MediaUtils::EventToString(Event));
				break;
			}
		}
	}
