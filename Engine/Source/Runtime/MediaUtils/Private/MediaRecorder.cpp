	...
	else
	{
		...
		while (SampleQueue->Dequeue(Sample) && Sample.IsValid())
		{
			...
			if (Sample->GetFormat() != EMediaTextureSampleFormat::CharBGRA && Sample->GetFormat() != EMediaTextureSampleFormat::FloatRGBA)
			{
				...
				{
					UE_LOGF(LogMediaUtils, Warning, "Media Source type cannot be recorded by Sequence Recorder. Texture Sample Format '%ls' is not supported by Media Recorder.", MediaTextureSampleFormat::EnumToString(Sample->GetFormat()));
