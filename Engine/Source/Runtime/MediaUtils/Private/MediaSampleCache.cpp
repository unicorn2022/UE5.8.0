...
#if 1
	UE_LOGF(LogMediaUtils, Log, "---------------------------------------------------------- %d", Time.GetTicks());
	for (const TSharedPtr<IMediaAudioSample, ESPMode::ThreadSafe>& Sample : AudioSamples)
	{
		UE_LOGF(LogMediaUtils, Log, "%d - %d (%d)", Sample->GetTime().GetTicks(), (Sample->GetTime() + Sample->GetDuration()).GetTicks() - 1, Sample->GetFrames());
	}
	UE_LOGF(LogMediaUtils, Log, "----------------------------------------------------------");
#endif
