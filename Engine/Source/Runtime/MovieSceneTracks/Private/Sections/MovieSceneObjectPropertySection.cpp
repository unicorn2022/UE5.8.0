...
void UMovieSceneObjectPropertySection::PostLoad()
{
	if (UMovieSceneObjectPropertyTrack* PropertyTrack = Cast<UMovieSceneObjectPropertyTrack>(GetOuter()))
	{
		...
		{
			UE_LOGF(LogMovieScene, Warning, "Mismatch in property track's property class %ls and object channel's property class (null). Setting object channel's property class to %ls.", *PropertyTrackPropertyClass->GetName(), *PropertyTrackPropertyClass->GetName());
		...
		{
			UE_LOGF(LogMovieScene, Warning, "Mismatch in object channel's property class %ls and property track's property class (null). Setting property track's property class to %ls.", *ObjectChannelPropertyClass->GetName(), *ObjectChannelPropertyClass->GetName());
