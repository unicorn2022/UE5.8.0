...
double FMovieSceneSkeletalAnimationParams::MapTimeToAnimation(const UMovieSceneSection* InSection, FFrameTime InPosition, FFrameRate InTickResolution, UAnimSequenceBase* OverrideSequence) const
{
	...
	return MapTimeToAnimation(SectionStartTime, SectionEndTime, InPosition, InTickResolution, OverrideSequence);
}

double FMovieSceneSkeletalAnimationParams::MapTimeToAnimation(FFrameNumber InSectionStartTime, FFrameNumber InSectionEndTime, FFrameTime InPosition, FFrameRate InTickResolution, UAnimSequenceBase* OverrideSequence) const
{
	return MakeTransform(InTickResolution, TRange<FFrameNumber>(InSectionStartTime, InSectionEndTime), OverrideSequence).TransformTime(InPosition).AsDecimal();
}
...
void UMovieSceneSkeletalAnimationSection::PostLoad()
{
	...
	if (GetLinkerCustomVersion(FSequencerObjectVersion::GUID) < FSequencerObjectVersion::ConvertEnableRootMotionToForceRootLock)
	{
		...
		if (AnimSeq && AnimSeq->bEnableRootMotion && !AnimSeq->bForceRootLock)
		{
			...
			UE_LOGF(LogMovieScene, Warning, "%ls Animation has set ForceRootLock to be used in Sequencer. If this animation is used in anywhere else using root motion, that will cause conflict.", *AnimSeq->GetName());
		}
	}
...
TOptional<TRange<FFrameNumber> > UMovieSceneSkeletalAnimationSection::GetAutoSizeRange() const
{
	if (UMovieScene* MovieScene = GetTypedOuter<UMovieScene>())
	{
		...
		if (OuterStartTime.IsSet() && OuterEndTime.IsSet())
		{
			return TRange<FFrameNumber>(GetInclusiveStartFrame(), GetInclusiveStartFrame() + (OuterEndTime.GetValue() - OuterStartTime.GetValue()).FrameNumber);
		}
	}
...
double UMovieSceneSkeletalAnimationSection::MapTimeToAnimation(FFrameTime InPosition, FFrameRate InTickResolution) const
{
	return Params.MapTimeToAnimation(this, InPosition, InTickResolution);
}
...
void UMovieSceneSkeletalAnimationSection::ImportEntityImpl(UMovieSceneEntitySystemLinker* EntityLinker, const FEntityImportParams& InParams, FImportedEntity* OutImportedEntity)
{
	...
	const bool bIsAdditive = Params.Animation ? Params.Animation->IsValidAdditive() : false;
...
bool UMovieSceneSkeletalAnimationSection::GetRootMotionVelocity(FFrameTime PreviousTime, FFrameTime CurrentTime, FFrameRate InTickResolution,
	FTransform& OutVelocity, float& OutWeight) const
{
	...
	if (AnimSequence)
	{
		...
		float PreviousTimeSeconds = static_cast<float>(MapTimeToAnimation(PreviousTime, InTickResolution));
		float CurrentTimeSeconds  = static_cast<float>(MapTimeToAnimation(CurrentTime, InTickResolution));
...
void UMovieSceneSkeletalAnimationSection::MatchSectionByBoneTransform(USkeletalMeshComponent* SkelMeshComp, FFrameTime CurrentFrame, FFrameRate InTickResolution,
	const FName& BoneName)
{
	...
	if (Track)
	{
		...
		Track->MatchSectionByBoneTransform(bMatchWithPrevious,SkelMeshComp, this, CurrentFrame, InTickResolution, BoneName, DiffTransform,DiffTranslate,DiffRotate);
