...
static void LogRemovedExternalObjects(UObject* ReferencingObject, const TArray<UObject*>& InRemovedObjects)
{
	...
	UE_LOGF(LogMovieScene, Warning, "While saving %ls, the following references were cleared because they were private and external: %ls",
...
UWorld* UMovieSceneSpawnableActorBinding::GetWorldContext(TSharedRef<const UE::MovieScene::FSharedPlaybackState> SharedPlaybackState) const
{
	...
	if (DesiredLevelName != NAME_None)
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogMovieScene, Warning, "Can't find sublevel '%ls' to spawn into, defaulting to Persistent level", *DesiredLevelName.ToString());
			}
		}
	}
	return WorldContext;
