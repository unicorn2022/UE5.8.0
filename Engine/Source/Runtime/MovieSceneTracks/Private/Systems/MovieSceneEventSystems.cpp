...
void UMovieSceneEventSystem::TriggerEvents(TArrayView<const FMovieSceneEventTriggerData> Events, TSharedRef<const FSharedPlaybackState> SharedPlaybackState)
{
	...
	UE_LOGF(LogMovieScene, VeryVerbose, "%ls: Triggering %d events", *SequenceName, Events.Num());
#endif
	...
	for (const FMovieSceneEventTriggerData& Event : Events)
	{
		...
		{
#if !NO_LOGGING
			UE_LOGF(LogMovieScene, Warning, "Failed to trigger event '%ls' because no director instance was available.", *Event.Ptrs.Function->GetName());
#endif
			continue;
		}

#if !NO_LOGGING
		UE_LOGF(LogMovieScene, VeryVerbose, "%ls: - Triggering event at frame %d, subframe %f", *SequenceName, Event.RootTime.FrameNumber.Value, Event.RootTime.GetSubFrame());
#endif

#if WITH_EDITOR
		const static FName NAME_CallInEditor("CallInEditor");
		...
		{
			UE_LOGF(LogMovieScene, Verbose, "Refusing to trigger event '%ls' in editor world when 'Call in Editor' is false.", *Event.Ptrs.Function->GetName());
			continue;
		}
#endif // WITH_EDITOR


		UE_LOGF(LogMovieScene, VeryVerbose, "Triggering event '%ls'.", *Event.Ptrs.Function->GetName());
