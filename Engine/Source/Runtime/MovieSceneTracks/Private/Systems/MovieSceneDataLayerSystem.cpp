...
EDataLayerUpdateFlags FDesiredLayerStates::Apply(FPreAnimatedDataLayerStorage* PreAnimatedStorage)
{
	...
	auto IsDataLayerReady = [](const UDataLayerInstance* DataLayer, EDataLayerRuntimeState DesireState, bool bExactState)
	{
		...
		case EDataLayerRuntimeState::Unloaded:
			...
			break;
		default:
			UE_LOGF(LogMovieScene, Error, "Unkown data layer state");
			return true;
		}
				...
				if (UDataLayerManager::GetDataLayerManager(DataLayer)->SetDataLayerInstanceRuntimeState(DataLayer, DesiredStateValue))
				{
					if (StateValue.ShouldFlushStreaming(DesiredStateValue) && !IsDataLayerReady(DataLayer, DesiredStateValue, true))
					{
						...
						if (DesiredStateValue == EDataLayerRuntimeState::Activated && !IsDataLayerReady(DataLayer, EDataLayerRuntimeState::Loaded, false))
						{
							...
							UE_LOGF(LogMovieScene, Warning, "[UMovieSceneDataLayerSystem] Data layer with name '%ls' is causing a full streaming flush (%ls)",
						...
						else
						{
							...
							UE_LOGF(LogMovieScene, Log, "[UMovieSceneDataLayerSystem] Data layer with name '%ls' is causing a visibility streaming flush (%ls)",
								*DataLayer->GetDataLayerShortName(),
								*StaticEnum<EDataLayerRuntimeState>()->GetDisplayNameTextByValue((int64)DesiredStateValue).ToString());
...
void UMovieSceneDataLayerSystem::OnRun(FSystemTaskPrerequisites& InPrerequisites, FSystemSubsequentTasks& Subsequents)
{
	...
	if (World->WorldType == EWorldType::Editor)
	{
		...
		{
			UE_LOGF(LogMovieScene, Warning, "Unable to retrieve data layer editor subsystem - data layer tracks will not function correctly");
		}
		else
		{
	...
	// Outside of editor, or in PIE, we use the runtime data layer sub-system
	{
		...
		{
			UE_LOGF(LogMovieScene, Warning, "Unable to retrieve world partition subsystem - data layer tracks will not function correctly");
		}
		else
		{
			...
			UE_SUPPRESS(LogMovieScene, Warning,
			{
				if (FlushTypeString)
				{
					...
					UE_LOGF(LogMovieScene, Warning, "[UMovieSceneDataLayerSystem] %ls took %.4f seconds (%ls)", FlushTypeString, FPlatformTime::Seconds() - StartTime, *SequenceList);
				}
			});
		}
	}
