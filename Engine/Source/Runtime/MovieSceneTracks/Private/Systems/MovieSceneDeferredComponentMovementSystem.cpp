...
void UMovieSceneDeferredComponentMovementSystem::OutputDeferredMovements()
{
	...
	UE_LOGF(LogMovieScene, Log, "Outputting deferred movement updates for %d components from Sequencer:", CollisionComponents.Num() + NonCollisionComponents.Num());

	if (CollisionComponents.Num() != 0)
	{
		UE_LOGF(LogMovieScene, Log, "\t %d Components with collision enabled:", CollisionComponents.Num());

		for (USceneComponent* Component : CollisionComponents)
		{
			UE_LOGF(LogMovieScene, Log, "\t\t %ls.%ls", *Component->GetOwner()->GetName(), *Component->GetName());
		}
	}

	if (NonCollisionComponents.Num() != 0)
	{
		UE_LOGF(LogMovieScene, Log, "\t %d Components without collision:", NonCollisionComponents.Num());
