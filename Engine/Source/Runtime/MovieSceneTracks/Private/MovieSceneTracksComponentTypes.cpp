...
/* ---------------------------------------------------------------------------
 * Actor reference conversion functions
 * ---------------------------------------------------------------------------*/
void ConvertOperationalProperty(const FActorReferenceComponent& InComponentData, TObjectPtr<AActor>& Out)
{
	Out = InComponentData.WeakActor.Get();
}

void ConvertOperationalProperty(const FActorReferenceComponent& InComponentData, TSoftObjectPtr<AActor>& Out)
{
	Out = InComponentData.WeakActor.Get();
}

void ConvertOperationalProperty(TObjectPtr<AActor> const InActor, FActorReferenceComponent& Out)
{
	// This conversion is incomplete: we end up with an FActorReferenceComponent that has a resolved actor,
	// but its object binding ID is left invalid, because we don't know where that actor "came from" in terms
	// of object binding resolution. This is fine because we should only end up here for saving pre-animated
	// state, which means we only care about saving and restoring the actor pointer.
	Out.WeakActor = InActor.Get();
}

void ConvertOperationalProperty(TSoftObjectPtr<AActor> const InActor, FActorReferenceComponent& Out)
{
	// See comment above.
	Out.WeakActor = InActor.Get();
}
...
struct FActorReferenceHandler : TPropertyComponentHandler<FActorReferencePropertyTraits, FActorReferenceComponent>
{
	virtual void DispatchInitializePropertyMetaDataTasks(const FPropertyDefinition& Definition, FSystemTaskPrerequisites& InPrerequisites, FSystemSubsequentTasks& Subsequents, UMovieSceneEntitySystemLinker* Linker) override
	{
		FBuiltInComponentTypes* BuiltInComponents = FBuiltInComponentTypes::Get();
		...
		FEntityTaskBuilder()
			...
			.Read(BuiltInComponents->PropertyBinding)
			.ReadOptional(BuiltInComponents->CustomPropertyIndex)
			.Write(TrackComponents->ActorReference.MetaDataComponents.GetType<0>())
			.FilterAll({ BuiltInComponents->Tags.NeedsLink })
			.Iterate_PerEntity(&Linker->EntityManager, [TrackComponents](UObject* Object, const FMovieScenePropertyBinding& Binding, const FCustomPropertyIndex* OptionalCustomPropertyIndex, FActorReferencePropertyTraits::FActorReferenceMetaData& OutMetaData)
			{
				if (OptionalCustomPropertyIndex)
				{
					if (const auto* CustomMetaData = TrackComponents->Accessors.ActorReference.MetaData.Find(OptionalCustomPropertyIndex->Value))
					{
						OutMetaData.Type = CustomMetaData->Type;
						return;
					}
				}
				...
				FProperty* BoundProperty = Bindings.GetProperty(*Object);
				if (FObjectProperty* BoundObjectProperty = CastField<FObjectProperty>(BoundProperty))
				{
					OutMetaData.Type = EActorReferenceType::ObjectPointer;
				}
				else if (FSoftObjectProperty* BoundSoftObjectProperty = CastField<FSoftObjectProperty>(BoundProperty))
				{
					OutMetaData.Type = EActorReferenceType::SoftObjectPointer;
				}
				else
				{
					ensureAlwaysMsgf(false, TEXT("Unsupported actor reference type for property '%s' on object '%s'"), *Bindings.GetPropertyPath(), *GetNameSafe(Object));
				}
			});
	}
};
...
FMovieSceneTracksComponentTypes::FMovieSceneTracksComponentTypes()
{
	...
	ComponentRegistry->NewPropertyType(ActorReference, TEXT("Actor Reference"));
	...
	ActorReference.MetaDataComponents.Initialize(ComponentRegistry, TEXT("Actor Reference Type"));
	...
	ComponentRegistry->NewComponentType(&ActorReferenceChannel, TEXT("Actor Reference Channel"));
	ComponentRegistry->NewComponentType(&ActorReferenceResult, TEXT("Actor Reference Result"));
	...
	// --------------------------------------------------------------------------------------------
	// Set up Actor Reference properties
	BuiltInComponents->PropertyRegistry.DefineProperty(ActorReference, TEXT("Apply Actor Reference Properties"))
	.AddSoleChannel(ActorReferenceResult)
	.SetCustomAccessors(&Accessors.ActorReference)
	.Commit(FActorReferenceHandler());
	...
	// --------------------------------------------------------------------------------------------
	// Set up actor reference components
	ComponentRegistry->Factories.DuplicateChildComponent(ActorReferenceChannel);
	ComponentRegistry->Factories.DuplicateChildComponent(ActorReferenceResult);
	ComponentRegistry->Factories.DefineMutuallyInclusiveComponent(ActorReferenceChannel, ActorReferenceResult);
	ComponentRegistry->Factories.DefineMutuallyInclusiveComponent(ActorReferenceChannel, BuiltInComponents->EvalTime);
