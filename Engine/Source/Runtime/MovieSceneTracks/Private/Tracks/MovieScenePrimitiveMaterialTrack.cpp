			...
			case EComponentMaterialType::SlotOverlayMaterial:
				if (UMeshComponent* Component = Cast<UMeshComponent>(WeakObject.Get()))
				{
					return !MaterialInfo.MaterialSlotName.IsNone() ?
						FText::Format(LOCTEXT("SlotOverlayMaterialSwitcherTooltip", "Material switcher for overlay slot {0} at index {1}"), FText::FromName(MaterialInfo.MaterialSlotName), FText::AsNumber(MaterialInfo.MaterialSlotIndex)) :
						FText::Format(LOCTEXT("IndexedSlotOverlayMaterialSwitcherTooltip", "Material switcher for overlay element at index {0}"), FText::AsNumber(MaterialInfo.MaterialSlotIndex));
				}
				break;
			default:
				break;
