...
void UMovieSceneVisibilityTrack::ProcessSourceValueForKeying(const FProperty* Property, UE::MovieScene::FSourcePropertyValue& InOutValue) const
{
	// Bound bool properties (e.g. AActor::bHidden, USceneComponent::bHiddenInGame) store hidden semantics,
	// but the visibility section's bool channel stores visible semantics (see UMovieSceneVisibilitySystem).
	// Invert here so the keyed value matches the channel's semantics.
	if (Property && CastField<FBoolProperty>(Property))
	{
		if (const bool* CurrentValue = InOutValue.Cast<bool>())
		{
			InOutValue = UE::MovieScene::FSourcePropertyValue::FromValue(!*CurrentValue);
		}
	}
}
