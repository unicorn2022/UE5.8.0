...
void UMovieSceneCVarTrackInstance::OnEndUpdateInputs()
{
		...
		for (const TPair<FString, FString>& Pair : CommandsAndValues)
		{
			...
			if (CVarsNeedingUpdate.Num() == 0 || CVarsNeedingUpdate.Contains(Pair.Key))
			{
				...
				if (ExistingInfo)
				{
					...
					{
						UE_LOGF(LogMovieScene, Warning, "Multiple sections animating the same CVar '%ls', this is not supported.", *Pair.Key);
					}
				}
				else
				{
	...
	for (const TPair<FString, FCVarInfo>& Pair : CVarInfo)
	{
		...
		{
			UE_LOGF(LogMovieScene, Warning, "Attempting to animate undefined CVar '%ls' with value '%ls'.", *Pair.Key, *Pair.Value.Value);
		}
	}
}
