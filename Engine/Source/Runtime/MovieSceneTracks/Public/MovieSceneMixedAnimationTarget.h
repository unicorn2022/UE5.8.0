...
struct FMovieSceneMixedAnimationTarget
{
	...
	virtual ~FMovieSceneMixedAnimationTarget() = default;
...
#if WITH_EDITOR
	/** Get the icon to display for this target type in the Sequencer outliner. Returns nullptr for no icon. */
	virtual const FSlateBrush* GetIcon() const { return nullptr; }

	/** Get a short display name for this target (e.g., "Automatic", specific target node name, etc.) */
	virtual FText GetShortDisplayName() const { return NSLOCTEXT("MovieSceneMixedAnimationTarget", "Automatic", "Automatic"); }
#endif
