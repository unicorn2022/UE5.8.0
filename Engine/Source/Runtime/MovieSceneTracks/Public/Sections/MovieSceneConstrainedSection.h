...
public:
	...
	*/
	virtual const TArray<FConstraintAndActiveChannel>& GetConstraintsChannels() const = 0;
	...
	*/
	MOVIESCENETRACKS_API UTickableConstraint* FindConstraint(const FName InConstraintName) const;

	/*
	* Find the constraint from the channel
	*/
	MOVIESCENETRACKS_API UTickableConstraint* FindConstraint(FMovieSceneChannel* InChannel) const;

	/*
	* Removes the constraint named ConstraintName
	*/
	MOVIESCENETRACKS_API bool RemoveConstraint(UWorld* InWorld, const FName InConstraintName, const bool bDoNotCompensate) const;
	
	/*
    * Removes all the constraints stored in this section
    */
	MOVIESCENETRACKS_API bool RemoveAllConstraints(UWorld* InWorld, const bool bDoNotCompensate) const;
