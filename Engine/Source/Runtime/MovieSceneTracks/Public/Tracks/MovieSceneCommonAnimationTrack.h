...
public:
	...
	UE_API virtual void AddSectionWithAutoRow(UMovieSceneSection* Section);

	/** Computes the next available row index. For mixer tracks, considers both sections and child tracks. */
	UE_API virtual int32 ComputeNextAvailableRowIndex() const;

	/** Adds a child track of the specified class at the given row index. Only valid for mixer tracks. */
	virtual UMovieSceneTrack* AddChildTrack(FGuid ObjectBinding, TSubclassOf<UMovieSceneTrack> TrackClass, int32 RowIndex)
	{
		checkf(false, TEXT("AddChildTrack is only valid for UMovieSceneAnimationMixerTrack"));
		return nullptr;
	}

	/** Gets all child tracks. Only valid for mixer tracks. Base implementation returns empty array. */
	virtual void GetAllChildTracks(TArray<TObjectPtr<UMovieSceneTrack>>& OutChildTracks) const {}
...
public:
	...
	UE_API void MatchSectionByBoneTransform(bool bMatchWithPrevious, USkeletalMeshComponent* SkelMeshComp, UMovieSceneSkeletalAnimationSection* CurrentSection, FFrameTime CurrentFrame, FFrameRate InTickResolution,
...
public:
	...
	UPROPERTY(EditAnywhere, Category = "RootMotions")
