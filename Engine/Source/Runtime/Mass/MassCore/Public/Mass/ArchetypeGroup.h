...
namespace UE::Mass
{
	...
	struct FArchetypeGroups
	{
		...
		MASSCORE_API bool operator==(const FArchetypeGroups& OtherGroups) const;
		MASSCORE_API FArchetypeGroups& operator=(FArchetypeGroups&& InGroups);
		MASSCORE_API FArchetypeGroups& operator=(const FArchetypeGroups& InGroups);

		/**
		 * Adds or updates the given (GroupType, GroupID) combination to IDContainer
		 */
		MASSCORE_API void Add(FArchetypeGroupHandle GroupHandle);
		 ...
		 */
		[[nodiscard]] MASSCORE_API FArchetypeGroups Add(FArchetypeGroupHandle GroupHandle) const;
		 ...
		 */
		MASSCORE_API void Remove(FArchetypeGroupType GroupType);
		 ...
		 * @note using [[nodiscard]] to avoid accidental calls on const instances that would not produce any effects
		 */
		[[nodiscard]] MASSCORE_API FArchetypeGroups Remove(FArchetypeGroupType GroupType) const;

		MASSCORE_API void Shrink();
		MASSCORE_API bool IsShrunk() const;
		...
		friend MASSCORE_API uint32 GetTypeHash(const FArchetypeGroups& Instance);
