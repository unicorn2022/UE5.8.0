...
public:
		...
		, bIsSingleThreaded(!FGenericPlatformProcess::SupportsMultithreading())
	{
		...
		{
			UE_LOGF(LogCotfServerConnection, Fatal, "Failed to send handshake request to server");
			return;
		}
		...
		{
			UE_LOGF(LogCotfServerConnection, Fatal, "Failed to receive handshake response from server");
			return;
		}
		bool bIsOk;
		...
		if (!bIsOk)
		{
			UE_LOGF(LogCotfServerConnection, Fatal, "Couldn't handshake with server");
			return;
		}
		FString ZenHostName;
	...
	virtual TFuture<UE::Cook::FCookOnTheFlyResponse> SendRequest(UE::Cook::FCookOnTheFlyRequest& Request) override
	{
		...
		if (Transport)
		{
			UE_LOGF(LogCotfServerConnection, Verbose, "Sending: %ls, Size='%lld'", *RequestHeader.ToString(), Request.TotalSize());
		...
		if (!bOk)
		{
			UE_LOGF(LogCotfServerConnection, Warning, "Failed to send: %ls, Size='%lld'", *RequestHeader.ToString(), Request.TotalSize());
...
private:
	...
	bool ProcessMessagesWhile(TFunctionRef<bool()> Condition)
	{
		...
		while (Condition())
		{
			...
			{
				UE_LOGF(LogCotfServerConnection, Warning, "Failed to receive message from server");
				return false;
			}
			...
			UE_LOGF(LogCotfServerConnection, Verbose, "Received: %ls, Size='%d'", *MessageHeader.ToString(), MessagePayload.Num());
			...
			if (bIsRequest)
			{
				UE_LOGF(LogCotfServerConnection, Warning, "Received request from server, ignoring");
			}
			else if (bIsResponse)
			{
				...
				if (!PendingRequest)
				{
					UE_LOGF(LogCotfServerConnection, Warning, "Failed to match response with id '%u' to an existing request", MessageHeader.CorrelationId);
					return false;
				}
	...
	void ThreadEntry()
	{
		...
		UE_LOGF(LogCotfServerConnection, Display, "Terminating connection to server");
		Flush();
		Transport.Reset();
	}
