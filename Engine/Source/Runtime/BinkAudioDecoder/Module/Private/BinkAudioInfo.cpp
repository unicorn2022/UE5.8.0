...
namespace BinkAudioInfo
{
	static void LogOnce(BinkAudioFileHeader& Header)
	{
		...
		if (bLogHeader.exchange(false))
		{
			...
			UE_LOGF(LogBinkAudioDecoder, Log, "Header.tag = %d", Header.tag);
			UE_LOGF(LogBinkAudioDecoder, Log, "Header.version = %d", Header.version);
			UE_LOGF(LogBinkAudioDecoder, Log, "Header.channels = %d", Header.channels);
			UE_LOGF(LogBinkAudioDecoder, Log, "Header.PADDING = %d", Header.PADDING);
			UE_LOGF(LogBinkAudioDecoder, Log, "Header.rate = %d", Header.rate);
			UE_LOGF(LogBinkAudioDecoder, Log, "Header.sample_count = %d", Header.sample_count);
			UE_LOGF(LogBinkAudioDecoder, Log, "Header.max_comp_space_needed = %d", Header.max_comp_space_needed);
			UE_LOGF(LogBinkAudioDecoder, Log, "Header.flags = %d", Header.flags);
			UE_LOGF(LogBinkAudioDecoder, Log, "Header.output_file_size = %d", Header.seek_table_entry_count);
			UE_LOGF(LogBinkAudioDecoder, Log, "Header.blocks_per_seek_table_entry = %d", Header.blocks_per_seek_table_entry);
...
void FBinkAudioInfo::SeekToFrame(const uint32 InFrameNum)
{
	...
	//
	if (StreamingSoundWaveData == nullptr)
	{
	...
	else
	{
		...
		{
			const uint32 SizeOfChunk = StreamingSoundWaveData->GetSizeOfChunk(BlockIndex);
...
bool FBinkAudioInfo::ParseHeader(const uint8* InSrcBufferData, uint32 InSrcBufferDataSize, struct FSoundQualityInfo* QualityInfo)
{
	...
	{
		UE_LOGF(LogBinkAudioDecoder, Error, "FBinkAudioInfo::ParseHeader: Failed: InSrcBufferDataSize < sizeof(BinkAudioFileHeader) : %d < %llu", InSrcBufferDataSize, sizeof(BinkAudioFileHeader));
		return false;
	}
	...
	if (Header->tag != 'UEBA')
	{
		UE_LOGF(LogBinkAudioDecoder, Error, "FBinkAudioInfo::ParseHeader: Failed: Header->tag != \'UEBA\' (%d): tag = %d", (uint32)('UEBA'), Header->tag);
		BinkAudioInfo::LogOnce(*Header);
		return false;
	}
	if (Header->version != 1)
	{
		UE_LOGF(LogBinkAudioDecoder, Error, "FBinkAudioInfo::ParseHeader: Failed: Header->version != 1: version = %d", Header->version);
		BinkAudioInfo::LogOnce(*Header);
		return false;
	}
	...
	{
		UE_LOGF(LogBinkAudioDecoder, Error, "BinkAudioInfo::ParseHeader Failed: Invalid SeekTableSize: SeekTableSize = %d; sizeof(BinkAudioFileHeader) = %llu; InSrcBufferDataSize = %d", SeekTableSize, SeekTableSize + sizeof(BinkAudioFileHeader), InSrcBufferDataSize);
		BinkAudioInfo::LogOnce(*Header);
		return false;
	}
...
bool FBinkAudioInfo::CreateDecoder()
{
	...
	{
		UE_LOGF(LogBinkAudioDecoder, Error, "BinkAudioInfo::CreateDecoder Failed: BinkInterface does not exist for the current configuration");
...
FDecodeResult FBinkAudioInfo::Decode(const uint8* CompressedData, const int32 CompressedDataSize, uint8* OutPCMData, const int32 OutputPCMDataSize)
{
	...
	while (RemnOutputPCMDataSize)
	{
		...
		if (BinkAudioValidateBlock(MaxCompSpaceNeeded, CompressedData, RemnCompressedDataSize) != BINK_AUDIO_BLOCK_VALID)
		{
			...
			UE_LOGF(LogBinkAudioDecoder, Warning, "Got weird buffer, validate returned %d", BinkAudioValidateBlock(MaxCompSpaceNeeded, CompressedData, RemnCompressedDataSize));
			break;
		}
			...
			Decoder->ConsumeFrameCount == 0)
		{
			...
			if (BlockStart != BlockEnd)
			{
				...
				UE_LOGF(LogBinkAudioDecoder, Error, "BinkAudio consumed unexpected amount! BlockEnd = 0x%llx BlockStart = 0x%llx BlockBase = 0x%llx",
