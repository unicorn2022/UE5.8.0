...
static int createBVTree(const unsigned short* verts, const int /*nverts*/,
						...
						const dtPolyDetail* DMeshes, const dtTileVert* DVerts, const unsigned char* DTris, //@UE
						const dtReal cs, const dtReal ch,
						const int /*nnodes*/, dtBVNode* nodes)
{
	//@UE BEGIN
	const uint64 itemSize = sizeof(BVItem)*npolys;
	if (itemSize > INT_MAX)
	{
		return 0;
	}
	...
	// Build tree
	BVItem* items = (BVItem*)dtAlloc(itemSize, DT_ALLOC_TEMP);
	...
	for (int i = 0; i < npolys; i++)
	{
		...
		for (int k = 0; k < pd->triCount; ++k)
		{
			...
			for (int m = 0; m < 3; ++m)
			{
				if (t[m] >= vertCount)
				{
					const dtReal qY = DVerts[pd->vertBase + (t[m] - vertCount)].y / ch; //@UE
...
//@UE BEGIN: Link pool sizing
	// Compute the size of the base link pool.
	// Offmesh links and extra preallocated links are added in a dynamic array.
	int maxLinkCount;
	if (params->bMinimizeLinkPoolSize)
	{
		// Minimized link pool size.
		// Navigation meshes are primarily triangles/quads (3-4 edges per polygon).
		// Each edge may connect to one or more neighbors, requiring links for each connection.
		// The x4 multiplier provides enough capacity for typical connectivity patterns and
		// tiles exceeding this capacity fall back to secondary storage.
		maxLinkCount = dtMax(params->polyCount * 4, 16);
	}
	else
	{
		maxLinkCount = edgeCount + portalCount*2;
	}
//@UE END
	...
	const int vertsSize = dtAlign(sizeof(dtTileVert)*totVertCount); //@UE
	...
	const int detailVertsSize = dtAlign(sizeof(dtTileVert)*uniqueDetailVertCount); //@UE
	...
	dtTileVert* navVerts = (dtTileVert*)d; d += vertsSize; //@UE
	dtPoly* navPolys = (dtPoly*)d; d += polysSize;
	d += linksSize;
	dtPolyDetail* navDMeshes = (dtPolyDetail*)d; d += detailMeshesSize;
	dtTileVert* navDVerts = (dtTileVert*)d; d += detailVertsSize; //@UE
	...
	auto setHeaderShort = [](const int value, unsigned short& headerVal, const TCHAR* text, bool& bAllValuesSet)
	{
		...
		{
			UE_LOGF(LogDetour, Error, "dtCreateNavMeshData: %ls is too large to fit in an unsigned short!", text);
	...
	if (!bAllValuesSet)
	{
		UE_LOGF(LogDetour, Error, "dtCreateNavMeshData: Tile %d, %d: Layer %d: Some dtMeshHeader values are too large to fit in an unsigned short, tile not Generated! Please decrease the size of your nav mesh tiles.", params->tileX, params->tileY, params->tileLayer);
	...
	// Store vertices
	// Mesh vertices: stored as tile-local float offsets from params->bmin //@UE
	for (int i = 0; i < params->vertCount; ++i)
	{
		const unsigned short* iv = &params->verts[i*3];
		navVerts[i].x = (float)(iv[0] * params->cs); //@UE
		navVerts[i].y = (float)(iv[1] * params->ch); //@UE
		navVerts[i].z = (float)(iv[2] * params->cs); //@UE
	}
	...
	for (int i = 0; i < params->offMeshConCount; ++i)
	{
		...
		{
			navVerts[offMeshVertsBase + n*2    ].setFromWorld(params->bmin, &offMeshCon.vertsA0[0]); //@UE
			navVerts[offMeshVertsBase + n*2 + 1].setFromWorld(params->bmin, &offMeshCon.vertsB0[0]); //@UE
			n++;
		}
	}
	...
	if (params->detailMeshes)
	{
		...
		for (int i = 0; i < params->polyCount; ++i)
		{
			...
			if (ndv-nv)
			{
				for (int k = 0; k < (ndv-nv); ++k) //@UE
				{ //@UE
					navDVerts[vbase+k].setFromWorld(params->bmin, &params->detailVerts[(vb+nv+k)*3]); //@UE
				} //@UE
	...
	{
		//@UE BEGIN
		const int nodeCount = createBVTree(params->verts, params->vertCount, params->polys, params->polyCount, nvp,
					 navDMeshes, navDVerts, navDTris,
	...
	for (int i = 0; i < params->clusterCount; i++)
	{
		...
		for (int j = 0; j < params->polyCount; j++)
		{
			...
			// Sum vertices in local space
			for (int iv = 0; iv < poly->vertCount; iv++)
			{
				const dtTileVert& v = navVerts[poly->verts[iv]];
				c[0] += (dtReal)v.x;
				c[1] += (dtReal)v.y;
				c[2] += (dtReal)v.z;
			}

			// Convert centroid to world space.
			const dtReal invCount = dtReal(1.) / poly->vertCount;
			dtReal cWorld[3];
			cWorld[0] = c[0] * invCount + params->bmin[0];
			cWorld[1] = c[1] * invCount + params->bmin[1];
			cWorld[2] = c[2] * invCount + params->bmin[2];

			dtVmad(cluster.center, cluster.center, cWorld, dtReal(1.));
			break;
		}
	}
#endif // WITH_NAVMESH_CLUSTER_LINKS
...
bool dtNavMeshDataSwapEndian(unsigned char* data, const int /*dataSize*/)
{
	...
	dtTileVert* verts = (dtTileVert*)d; d += vertsSize; //@UE
	...
	dtTileVert* detailVerts = (dtTileVert*)d; d += detailVertsSize; //@UE
	...
	{
		dtSwapEndian(&verts[i].x); //@UE
		dtSwapEndian(&verts[i].y); //@UE
		dtSwapEndian(&verts[i].z); //@UE
	}
	...
	{
		dtSwapEndian(&detailVerts[i].x); //@UE
		dtSwapEndian(&detailVerts[i].y); //@UE
		dtSwapEndian(&detailVerts[i].z); //@UE
	}
...
{
	// Validate minimum size before casting to header
	...
	// Patch header pointers.
	const int headerSize = dtAlign(sizeof(dtMeshHeader));
	const int vertsSize = dtAlign(sizeof(dtTileVert)*header->vertCount);
	const int polysSize = dtAlign(sizeof(dtPoly)*header->polyCount);
	const int linksSize = dtAlign(sizeof(dtLink)*(header->maxLinkCount));
	const int detailMeshesSize = dtAlign(sizeof(dtPolyDetail)*header->detailMeshCount);
	const int detailVertsSize = dtAlign(sizeof(dtTileVert)*header->detailVertCount);
	const int detailTrisSize = dtAlign(sizeof(unsigned char)*4*header->detailTriCount);
	const int bvtreeSize = dtAlign(sizeof(dtBVNode)*header->bvNodeCount);
	const int offMeshLinksSize = dtAlign(sizeof(dtOffMeshConnection)*header->offMeshConCount);
	...
	// Validate that the computed layout fits within the provided buffer
	const int computedSize = (int)(d - data);
	if (computedSize > dataSize)
	{
		return false;
	}

	// Save old bmin before transforming the header; needed to toWorld tile-local vertex offsets
	dtReal oldBmin[3];
	dtVcopy(oldBmin, header->bmin);
	...
	// Transform tile vertices (transform from old bmin, rotate, transform into new bmin)
	for (int j = 0; j < tile.header->vertCount; ++j)
	{
		dtReal v[3], vOut[3];
		tile.verts[j].toWorld(oldBmin, v); //@UE
		dtRotate90(vOut, v, rotationCenter, rot);
		dtVadd(vOut, vOut, offset);
		tile.verts[j].setFromWorld(tile.header->bmin, vOut); //@UE
	}

	// Transform tile detail vertices (transform from old bmin, rotate, transform into new bmin)
	for (int j = 0; j < tile.header->detailVertCount; ++j)
	{
		dtReal v[3], vOut[3];
		tile.detailVerts[j].toWorld(oldBmin, v); //@UE
		dtRotate90(vOut, v, rotationCenter, rot);
		dtVadd(vOut, vOut, offset);
		tile.detailVerts[j].setFromWorld(tile.header->bmin, vOut); //@UE
	}
