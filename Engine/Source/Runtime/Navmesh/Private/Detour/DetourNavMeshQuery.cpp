...
/// Compute triangle area in tile-local space.
inline dtReal dtTriArea2DLocal(const dtTileVert& a, const dtTileVert& b, const dtTileVert& c)
{
	const float abx = b.x - a.x;
	const float abz = b.z - a.z;
	const float acx = c.x - a.x;
	const float acz = c.z - a.z;
	return (dtReal)(acx * abz - abx * acz);
}
//@UE END
										 ...
										 dtPolyRef* randomRef, dtReal* randomPt) const
{
	...
	for (int i = 0; i < tile->header->polyCount; ++i)
	{
		...
		{
			polyArea += dtTriArea2DLocal( //@UE
				tile->verts[p->verts[0]], //@UE
				tile->verts[p->verts[j-1]], //@UE
				tile->verts[p->verts[j]] //@UE
			); //@UE
		}
	...
	{
		tile->verts[poly->verts[j]].toWorld(tile->header->bmin, &verts[j*3]); //@UE
	}
													 ...
													 dtPolyRef* randomRef, dtReal* randomPt) const
{
	...
	while (!m_openList->empty())
	{
		...
		if (bestPoly->getType() == DT_POLYTYPE_GROUND)
		{
			...
			{
				polyArea += dtTriArea2DLocal( //@UE
					bestTile->verts[bestPoly->verts[0]], //@UE
					bestTile->verts[bestPoly->verts[j-1]], //@UE
					bestTile->verts[bestPoly->verts[j]] //@UE
				); //@UE
			}
			// Choose random polygon weighted by area, using reservoi sampling.
			areaSum += polyArea;
...
//@UE END
	...
	for (int iPoly = numRandomPolys - 1; iPoly >= 0 && !foundPt; iPoly--)
	{
		...
		{
			testTile->verts[testPoly->verts[j]].toWorld(testTile->header->bmin, &verts[j*3]); //@UE
		}
...
dtStatus dtNavMeshQuery::findRandomPointInPoly(dtPolyRef ref, float(*frand)(), dtReal* randomPt) const
{
	...
	{
		dtReal v0[3], v1[3]; //@UE
		tile->verts[poly->verts[0]].toWorld(tile->header->bmin, v0); //@UE
		tile->verts[poly->verts[1]].toWorld(tile->header->bmin, v1); //@UE
		const dtReal s = frand();
...
dtStatus dtNavMeshQuery::findRandomPointInCluster(dtClusterRef clusterRef, float(*frand)(), dtPolyRef* randomRef, dtReal* randomPt) const
{
	...
	for (int i = 0; i < maxGroundPolys; i++)
	{
		if (searchTile->polyClusters[i] == clusterIdx)
		{
			...
			{
				polyArea += dtTriArea2DLocal( //@UE
					searchTile->verts[testPoly->verts[0]], //@UE
					searchTile->verts[testPoly->verts[j-1]], //@UE
					searchTile->verts[testPoly->verts[j]] //@UE
				); //@UE
			}
			// Choose random polygon weighted by area, using reservoi sampling.
			areaSum += polyArea;
	...
	{
		searchTile->verts[randomPoly->verts[j]].toWorld(searchTile->header->bmin, &verts[j*3]); //@UE
	}
...
dtStatus dtNavMeshQuery::closestPointOnPolyBoundary(dtPolyRef ref, const dtReal* pos, dtReal* closest) const
{
	...
	{
		tile->verts[poly->verts[i]].toWorld(tile->header->bmin, &verts[nv*3]); //@UE
		nv++;
	}		
...
dtStatus dtNavMeshQuery::getPolyHeight(dtPolyRef ref, const dtReal* pos, dtReal* height) const
{
	...
	{
		dtReal sv0[3], sv1[3], sv2[3], sv3[3]; //@UE
		tile->verts[poly->verts[0]].toWorld(tile->header->bmin, sv0); //@UE
		tile->verts[poly->verts[1]].toWorld(tile->header->bmin, sv1); //@UE
		tile->verts[poly->verts[2]].toWorld(tile->header->bmin, sv2); //@UE
		tile->verts[poly->verts[3]].toWorld(tile->header->bmin, sv3); //@UE
		dtReal h;
		if (dtClosestHeightPointTriangle(pos, sv0, sv2, sv1, h)) //@UE
		{
			if (height) *height = h;
			return DT_SUCCESS;
		}
		else if (dtClosestHeightPointTriangle(pos, sv1, sv2, sv3, h)) //@UE
		{
			if (height) *height = h;
			return DT_SUCCESS;
		}
									   ...
									   dtPolyRef* polys, int* polyCount, const int maxPolys) const
{
	...
	for (int y = miny; y <= maxy; ++y)
	{
		for (int x = minx; x <= maxx; ++x)
		{
			...
			if (neis == nullptr)						//@UE
			{											//@UE
				return DT_FAILURE | DT_OUT_OF_MEMORY;	//@UE
			}											//@UE
	...
	*visitedCount = 0;
	...
	while (nstack)
	{
		...
		for (int i = 0; i < nverts; ++i)
			curTile->verts[curPoly->verts[i]].toWorld(curTile->header->bmin, &verts[i*3]); //@UE
...
{
	dtAssert(m_nav);
	UE_CLOGF(m_nav == nullptr, LogDebugRaycastCrash, Fatal, "dtNavMeshQuery::raycast doesn't have valid navmesh!");
...
//@UE END
			...
			dtReal left[3], right[3]; //@UE
			tile->verts[v0].toWorld(tile->header->bmin, left); //@UE
			tile->verts[v1].toWorld(tile->header->bmin, right); //@UE
	...
	while (nstack)
	{
		...
		while (i != DT_NULL_LINK)
		{
			...
			for (int neighbourPolyVertIndex = 0; neighbourPolyVertIndex < npa; ++neighbourPolyVertIndex)
				neighbourTile->verts[neighbourPoly->verts[neighbourPolyVertIndex]].toWorld(neighbourTile->header->bmin, &pa[neighbourPolyVertIndex * 3]); //@UE
			...
			for (int j = 0; j < n; ++j)
			{
				...
				for (int pastPolyVertIndex = 0; pastPolyVertIndex < npb; ++pastPolyVertIndex)
					pastTile->verts[pastPoly->verts[pastPolyVertIndex]].toWorld(pastTile->header->bmin, &pb[pastPolyVertIndex * 3]); //@UE
	...
	for (int i = 0, j = (int)poly->vertCount-1; i < (int)poly->vertCount; j = i++)
	{
		...
		else
		{
			...
			if (n < maxSegments)
			{
				...
				tile->verts[poly->verts[j]].toWorld(tile->header->bmin, seg+0); //@UE
				tile->verts[poly->verts[i]].toWorld(tile->header->bmin, seg+3); //@UE
				if (segmentRefs)
		...
		// Store segments.
		dtReal vj[3], vi[3]; //@UE
		tile->verts[poly->verts[j]].toWorld(tile->header->bmin, vj); //@UE
		tile->verts[poly->verts[i]].toWorld(tile->header->bmin, vi); //@UE
...
{
	dtReal va[3], vb[3]; //@UE
	tile->verts[poly->verts[edge]].toWorld(tile->header->bmin, va); //@UE
	tile->verts[poly->verts[(edge + 1) % poly->vertCount]].toWorld(tile->header->bmin, vb); //@UE
											...
											dtReal* hitDist, dtReal* hitPos, dtReal* hitNormal) const
{
	...
	while (!m_openList->empty())
	{
		...
		for (int i = 0, j = (int)bestPoly->vertCount-1; i < (int)bestPoly->vertCount; j = i++)
		{
						...
						if (link.ref != 0)
						{
							...
							else
							{
								...
								if (link.bmin != 0 || link.bmax != 255)
								{
									...
									const dtReal tmin = link.bmin*s;
									const dtReal tmax = link.bmax*s;
									dtReal vjTmp[3], viTmp[3]; //@UE
									bestTile->verts[bestPoly->verts[j]].toWorld(bestTile->header->bmin, vjTmp); //@UE
									bestTile->verts[bestPoly->verts[i]].toWorld(bestTile->header->bmin, viTmp); //@UE
									dtVlerp(portalLeft,  vjTmp, viTmp, tmin); //@UE
									dtVlerp(portalRight, vjTmp, viTmp, tmax); //@UE
								}
							}
						}
						break;
			...
			dtReal vjBuf[3], viBuf[3]; //@UE
			const dtReal* vj; //@UE
			const dtReal* vi; //@UE
			if (!isPortal)
			{
				bestTile->verts[bestPoly->verts[j]].toWorld(bestTile->header->bmin, vjBuf); //@UE
				bestTile->verts[bestPoly->verts[i]].toWorld(bestTile->header->bmin, viBuf); //@UE
				vj = vjBuf; //@UE
				vi = viBuf; //@UE
			}
			else
		...
		while (i != DT_NULL_LINK)
		{
			...
			// Calc distance to the edge.
			dtReal va[3], vb[3]; //@UE
			bestTile->verts[bestPoly->verts[link.edge]].toWorld(bestTile->header->bmin, va); //@UE
			CA_SUPPRESS(6385);
			bestTile->verts[bestPoly->verts[(link.edge+1) % bestPoly->vertCount]].toWorld(bestTile->header->bmin, vb); //@UE
			dtReal tseg;
			const dtReal distSqr = dtDistancePtSegSqr2D(centerPos, va, vb, tseg);
