	...
	for (int j = 0; j < pd->triCount; ++j)
	{
		...
		for (int k = 0; k < 3; ++k)
		{
			if (t[k] < poly->vertCount)
				tile->verts[poly->verts[t[k]]].toWorld(tile->header->bmin, vBuf[k]); //@UE
			else
				tile->detailVerts[pd->vertBase+(t[k]-poly->vertCount)].toWorld(tile->header->bmin, vBuf[k]); //@UE
...
{
	// allocated in linksFreeList from dtMeshTile (detour) first, then to dynamic array
	CREATE_LINK_PREALLOCATED,
	// offmesh links are added in a dynamic array (sharing the space with extra reallocated links) 
...
inline unsigned int allocLink(dtMeshTile* tile, char LinkAllocMode)
{
	...
	{
		// Preallocated links: prefer base pool, fall back to dynamic if exhausted
		...
		{
			// Base pool exhausted - fall back to secondary storage
			if (tile->dynamicFreeList == DT_NULL_LINK)
			{
				// No free secondary links, grow the array
				dtLink emptyLink;
				memset(&emptyLink, 0, sizeof(dtLink));
				emptyLink.next = DT_NULL_LINK;

				tile->dynamicFreeList = tile->dynamicLinks.size();
				tile->dynamicLinks.push(emptyLink);
			}

			newLink = tile->dynamicFreeList;
			tile->dynamicFreeList = tile->dynamicLinks[newLink].next;

			// Offset into secondary range
			newLink += tile->header->maxLinkCount;
		}
	}
	// Off-mesh links use secondary storage to preserve base pool for internal/external links
	else if (LinkAllocMode == CREATE_LINK_DYNAMIC_OFFMESH)
	{
		// Allocate from secondary storage only (never consume base pool)
		// This ensures internal/external links (PREALLOCATED) get base pool priority
		if (tile->dynamicFreeList == DT_NULL_LINK)
		{
			// No free secondary links, grow the array
			dtLink emptyLink;
...
inline void freeLink(dtMeshTile* tile, unsigned int link)
{
	...
	else if (link < firstClusterLinkIdx)
	{
		...
		tile->dynamicLinks[linkIdx].next = tile->dynamicFreeList;	//@UE
		tile->dynamicFreeList = linkIdx;							//@UE
	}
#if WITH_NAVMESH_CLUSTER_LINKS
	else
	{
...
{
	tile->dynamicLinks.~dtChunkArray();	//@UE
#if WITH_NAVMESH_CLUSTER_LINKS
	...
	const dtReal* spos, const dtReal* epos, dtReal& tmin, dtReal& tmax)
{
	...
	// Convert segment to local space instead of converting all vertices to world.
	dtReal sposLocal[3], eposLocal[3]; //@UE
	sposLocal[0] = spos[0] - tile->header->bmin[0]; //@UE
	sposLocal[1] = spos[1] - tile->header->bmin[1]; //@UE
	sposLocal[2] = spos[2] - tile->header->bmin[2]; //@UE
	eposLocal[0] = epos[0] - tile->header->bmin[0]; //@UE
	eposLocal[1] = epos[1] - tile->header->bmin[1]; //@UE
	eposLocal[2] = epos[2] - tile->header->bmin[2]; //@UE

	dtReal verts[DT_VERTS_PER_POLYGON*3];
	for (int i = 0; i < poly->vertCount; i++)
	{
		const dtTileVert& v = tile->verts[poly->verts[i]]; //@UE
		verts[i*3 + 0] = (dtReal)v.x; //@UE
		verts[i*3 + 1] = (dtReal)v.y; //@UE
		verts[i*3 + 2] = (dtReal)v.z; //@UE
	}

	int smin, smax;
	return dtIntersectSegmentPoly2D(sposLocal, eposLocal, verts, poly->vertCount, tmin, tmax, smin, smax); //@UE
}

static void addSegmentIntersections(const dtOffMeshSegmentIntersection* isec, dtOffMeshSegmentIntersection* list, int& nlist)
{
	UE_LOGF(LogSegmentLink, Log, "         %s", __FUNCTION__);
	...
	if (minIdx < 0)
	{
		UE_LOGF(LogSegmentLink, Log, "            %s minIdx=%i, add seg intersection, tile (%i,%i,%i), poly %i, %f",
	...
	{
		UE_LOGF(LogSegmentLink, Log, "            %s override", __FUNCTION__);
		list[minIdx] = isec[0];
	}
	...
	{
		UE_LOGF(LogSegmentLink, Log, "            %s maxIdx=%i, add seg intersection, tile (%i,%i,%i), poly %i, %f",
	...
	dtOffMeshSegmentTileIntersection& list)
{
	...
	UE_LOGF(LogSegmentLink, Log, "      %s for (%i,%i,%i) segment: start (%0.2f, %0.2f, %0.2f) end (%0.2f, %0.2f, %0.2f)",
	...
	for (int i = 0; i < tile->header->offMeshBase; i++)
	{
		...
		//@UE BEGIN
		dtReal vtmp[3];
		tile->verts[poly->verts[0]].toWorld(tile->header->bmin, vtmp);
		dtVcopy(bmin, vtmp);
		dtVcopy(bmax, vtmp);
		for (int j = 1; j < poly->vertCount; j++)
		{
			tile->verts[poly->verts[j]].toWorld(tile->header->bmin, vtmp);
			dtVmin(bmin, vtmp);
			dtVmax(bmax, vtmp);
		}
		//@UE END
...
static dtOffMeshSegmentData* initSegmentIntersection(const dtNavMesh& navMesh, dtMeshTile* tile)
{
	...
	UE_LOGF(LogSegmentLink, Log, "   %s for tile (%i,%i,%i)", __FUNCTION__, tile->header->x, tile->header->y, tile->header->layer);
	...
	dtOffMeshSegmentPart* parts, const int maxParts, int& nparts, unsigned short& nverts)
{
	...
	UE_LOGF(LogSegmentLink, Log, "   %s (%i,%i,%i)", __FUNCTION__, tile->header->x, tile->header->y, tile->header->layer);
	...
	UE_LOGF(LogSegmentLink, Log, "   %s %i nlinkPairs", __FUNCTION__, nlinkPairs);
	...
	for (int i = 0; i < nlinkPairs; i++)
	{
			...
			linkPair.links[1].tileA == tile || linkPair.links[1].tileB == tile)
		{
			...
			UE_LOGF(LogSegmentLink, Log, "   %s seg part %i created tA: %f, %f, tb: %f, %f, tileA (%i,%i,%i), tileB (%i,%i,%i), tile=%p, tileA=%p, tileB=%p",
				__FUNCTION__, nparts,
				part.tA0, part.tA1,
				part.tB0, part.tB1,
	...
	UE_LOGF(LogSegmentLink, Log, "   %s %i nparts created", __FUNCTION__, nparts);
}
	...
	dtOffMeshSegmentPart* parts, int nparts, unsigned short vertBase, int polyBase)
{
	...
	for (int i = 0; i < nparts; i++)
	{
		...
		dtVmad(wp, con->startA, lenA, it->tA0);
		tile->verts[vertBase+it->vA0].setFromWorld(tile->header->bmin, wp);

		dtVmad(wp, con->startA, lenA, it->tA1);
		tile->verts[vertBase+it->vA1].setFromWorld(tile->header->bmin, wp);

		dtVmad(wp, con->startB, lenB, it->tB0);
		tile->verts[vertBase+it->vB0].setFromWorld(tile->header->bmin, wp);

		dtVmad(wp, con->startB, lenB, it->tB1);
		tile->verts[vertBase+it->vB1].setFromWorld(tile->header->bmin, wp);
		//@UE END
	...
	dtChunkArray<FConnectingPolyData>& cons) const
{
	...
	for (int i = 0; i < tile->header->polyCount; ++i)
	{
		...
		for (int j = 0; j < nv; ++j)
		{
			...
			dtReal vc[3]; //@UE
			tile->verts[poly->verts[j]].toWorld(tile->header->bmin, vc); //@UE
			...
			dtReal vd[3]; //@UE
			tile->verts[poly->verts[(j + 1) % nv]].toWorld(tile->header->bmin, vd); //@UE
...
void dtNavMesh::connectExtLinks(dtMeshTile* tile, dtMeshTile* target, int side, bool updateCLinks)
{
	...
	for (int i = 0; i < tile->header->polyCount; ++i)
	{
		...
		for (int j = 0; j < nv; ++j)
		{
			...
			// Create new links
			dtReal va[3], vb[3]; //@UE
			tile->verts[poly->verts[j]].toWorld(tile->header->bmin, va); //@UE
			tile->verts[poly->verts[(j+1) % nv]].toWorld(tile->header->bmin, vb); //@UE
			...
			for (int k = 0; k < cons.size(); ++k)
			{
				...
				if (idx != DT_NULL_LINK)
				{
					dtLink& link = getLink(tile, idx);	//@UE
					link.ref = NeiData.ref;				//@UE
					link.edge = (unsigned char)j;		//@UE
					link.side = (unsigned char)dir;		//@UE

					link.next = poly->firstLink;		//@UE
					poly->firstLink = idx;
					...
					if (dir == 0 || dir == 4)
					{
						...
						if (tmin > tmax)
							dtSwap(tmin,tmax);
						link.bmin = (unsigned char)(dtClamp(tmin, 0.0f, 1.0f)*255.0f);	//@UE
						link.bmax = (unsigned char)(dtClamp(tmax, 0.0f, 1.0f)*255.0f);	//@UE
					}
					else if (dir == 2 || dir == 6)
					{
...
void dtNavMesh::connectExtOffMeshLinks(dtMeshTile* tile, dtMeshTile* target, int side, bool updateCLinks)
{
	...
	for (int i = 0; i < target->header->offMeshConCount; ++i)
	{
		...
		// For reversed links, the landing vertex is verts[0] instead of verts[1].
		const bool bReversed = targetCon->getIsReversed();
		const int landingVertIdx = bReversed ? 0 : 1;
		target->verts[targetPoly->verts[landingVertIdx]].setFromWorld(target->header->bmin, nearestPt); //@UE

		// Link off-mesh connection to landing poly.
		// For reversed links, this is the backtracker direction.
		unsigned char fwdLinkSide = oppositeSide | DT_LINK_FLAG_OFFMESH_CON | biDirFlag;
		if (tile != target)
		{
			fwdLinkSide &= ~DT_CONNECTION_INTERNAL;
		}
		if (bReversed && biDirFlag == 0)
		{
			fwdLinkSide |= DT_LINK_FLAG_OFFMESH_CON_BACKTRACKER;
		}

		// Link off-mesh connection to target poly.
		const unsigned int landPolyIdx = decodePolyIdPoly(ref);
		const unsigned char landingEdge = bReversed ? 0 : 1;

		linkOffMeshHelper(target, targetCon->poly, tile, landPolyIdx, fwdLinkSide, landingEdge);

		// Link landing poly back to off-mesh connection.
		// For non-reversed unidirectional links, this is the backtracker direction.
		unsigned char bckLinkSide = (unsigned char)(side == -1 ? DT_CONNECTION_INTERNAL : side) | DT_LINK_FLAG_OFFMESH_CON | biDirFlag;
		if (tile != target)
		{
			bckLinkSide &= ~DT_CONNECTION_INTERNAL;
		}
		if (!bReversed && biDirFlag == 0)
		{
			bckLinkSide |= DT_LINK_FLAG_OFFMESH_CON_BACKTRACKER;
		}

		linkOffMeshHelper(tile, landPolyIdx, target, targetCon->poly, bckLinkSide, 0xff);
		...
		if (updateCLinks)
		{
				...
				tile->polyClusters && target->polyClusters)
			{
				...
				const unsigned char baseFwd = bReversed ? DT_CLINK_VALID_BCK : DT_CLINK_VALID_FWD;
				const unsigned char baseBck = bReversed ? DT_CLINK_VALID_FWD : DT_CLINK_VALID_BCK;
				const unsigned char flagsFwd = baseFwd | (biDirFlag ? baseBck : 0);
				const unsigned char flagsBck = baseBck | (biDirFlag ? baseFwd : 0);
...
void dtNavMesh::connectIntLinks(dtMeshTile* tile)
{
	...
	for (int i = 0; i < tile->header->polyCount; ++i)
	{
		...
		for (int j = poly->vertCount-1; j >= 0; --j)
		{
			...
			{
				dtLink& link = getLink(tile, idx);				//@UE
				link.ref = base | (dtPolyRef)(poly->neis[j]-1); //@UE
				link.edge = (unsigned char)j;					//@UE

				link.side = DT_CONNECTION_INTERNAL;				//@UE

				link.bmin = link.bmax = 0;						//@UE
				// Add to linked list.
				link.next = poly->firstLink;					//@UE
				poly->firstLink = idx;
			}
...
void dtNavMesh::baseOffMeshLinks(dtMeshTile* tile)
{
	...
	for (int i = 0; i < tile->header->offMeshConCount; ++i)
	{
		...
		const bool bReversed = con->getIsReversed();
		const int baseVertIdx = bReversed ? 1 : 0;
		tile->verts[poly->verts[baseVertIdx]].setFromWorld(tile->header->bmin, nearestPt); //@UE
		...
		// For reversed links, swap forward/backward so that the normal traversal
		// direction goes from pos[3] to pos[0] (i.e. B0 to A0).
		const unsigned char sideToLocal = bReversed ? sideFwd : sideBck;
		const unsigned char sideFromLocal = bReversed ? sideBck : sideFwd;
		const int baseEdge = bReversed ? 1 : 0;

		// Link off-mesh connection to local ground poly.
		linkOffMeshHelper(tile, con->poly, tile, decodePolyIdPoly(ref), sideToLocal, baseEdge);

		// Link local ground poly back to off-mesh connection.
		linkOffMeshHelper(tile, decodePolyIdPoly(ref), tile, con->poly, sideFromLocal, 0xff);
	}
}
										 ...
										 const dtReal* pos, dtReal* closest) const
{
	...
	if (poly->getType() == DT_POLYTYPE_OFFMESH_POINT)
	{
		//@UE BEGIN
		dtReal v0[3], v1[3];
		tile->verts[poly->verts[0]].toWorld(tile->header->bmin, v0);
		tile->verts[poly->verts[1]].toWorld(tile->header->bmin, v1);
		const dtReal d0 = dtVdist(pos, v0);
		const dtReal d1 = dtVdist(pos, v1);

		dtReal u;
		const dtReal sum = d0 + d1;
		if (sum != 0)
			u = d0 / sum;
		else
			u = 0;
	...
	for (int i = 0; i < nv; ++i)
	{ //@UE
		tile->verts[poly->verts[i]].toWorld(tile->header->bmin, &verts[i*3]); //@UE
	} //@UE
	...
	if (poly->getType() == DT_POLYTYPE_GROUND)
	{
		...
		for (int j = 0; j < pd->triCount; ++j)
		{
			...
			for (int k = 0; k < 3; ++k)
			{
				CA_SUPPRESS(6385);
				if (t[k] < poly->vertCount)
					tile->verts[poly->verts[t[k]]].toWorld(tile->header->bmin, vBuf[k]);
				else
					tile->detailVerts[pd->vertBase+(t[k]-poly->vertCount)].toWorld(tile->header->bmin, vBuf[k]);
				v[k] = vBuf[k];
			}
			dtReal h;
									...
									dtPolyRef* polys, const int maxPolys, bool bExcludeUnwalkable) const
{
	if (tile->bvTree)
	{
		...
		UE_CLOGF(bvQuantFactor == 0.f, LogDetour, Warning, "dtNavMesh::queryPolygonsInTile bounding volume quantization factor is zero! The query might not return the right result");
	...
	else
	{
		...
		for (int i = 0; i < tile->header->polyCount; ++i)
		{
			...
			// Calc polygon bounds.
			dtReal v[3]; //@UE
			tile->verts[p->verts[0]].toWorld(tile->header->bmin, v); //@UE
			dtVcopy(bmin, v);
			dtVcopy(bmax, v);
			for (int j = 1; j < p->vertCount; ++j)
			{
				tile->verts[p->verts[j]].toWorld(tile->header->bmin, v); //@UE
				dtVmin(bmin, v);
				dtVmax(bmax, v);
			}
							...
							dtTileRef lastRef, dtTileRef* result)
{
	...
	const int vertsSize = dtAlign(sizeof(dtTileVert)*header->vertCount); //@UE
	...
	const int detailVertsSize = dtAlign(sizeof(dtTileVert)*header->detailVertCount); //@UE
	...
	tile->dynamicFreeList = DT_NULL_LINK;	//@UE
	tile->dynamicLinks.resize(0);			//@UE
...
void dtNavMesh::connectDeferredOffMeshLinks()
{
	if (!m_bDeferOffMeshLinking)
		return;

	// Called from OnAreaCostChanged after applyAreaCostOrder has set m_areaCostOrder.
	// Must not be called before area costs are applied, or bSnapToCheapestArea links
	// will snap by distance instead of cost.

	m_bDeferOffMeshLinking = false;

	ReadTilesHelper TileArray;

	// Pass 1: Initialize all tiles' off-mesh connections first.
	// baseOffMeshLinks() sets poly->firstLink on each tile's off-mesh polys.
	// connectExtOffMeshLinks() guards on firstLink != DT_NULL_LINK, so all tiles
	// must be initialized before any cross-tile connections are attempted.
	for (int i = 0; i < m_maxTiles; ++i)
	{
	...
	// Pass 2: Connect external links now that firstLink is valid on all tiles.
	for (int i = 0; i < m_maxTiles; ++i)
	{
		dtMeshTile* tile = &m_tiles[i];
		if (!tile->header) continue;
...
#if WITH_NAVMESH_CLUSTER_LINKS
		const bool bHasClusters = tile->header->clusterCount > 0;
#else
		const bool bHasClusters = false;
#endif // WITH_NAVMESH_CLUSTER_LINKS

		// Connect layers in same column — landing on this tile
		int nneis = getTileCountAt(tx, ty);
		dtMeshTile** neis = TileArray.PrepareArray(nneis);
		if (!neis) continue;
		getTilesAt(tx, ty, neis, nneis);
		for (int j = 0; j < nneis; ++j)
		{
			// neis[j]'s connections landing on tile (includes self case)
			connectExtOffMeshLinks(tile, neis[j], -1, bHasClusters);
		}

		// Connect neighbour tiles across edges — landing on this tile
		for (int side = 0; side < 8; ++side)
		{
			nneis = getNeighbourTilesCountAt(tx, ty, side);
			neis = TileArray.PrepareArray(nneis);
			if (!neis) continue;
			getNeighbourTilesAt(tx, ty, side, neis, nneis);
			for (int j = 0; j < nneis; ++j)
			{
				// neis[j]'s connections landing on tile
				connectExtOffMeshLinks(tile, neis[j], side, bHasClusters);
			}
		}
	}
}
...
void dtNavMesh::processSegmentLinksForTile(dtTileRef tileRef, unsigned int maxSkippedNeigborTiles, dtTileRef* skippedNeigborTiles, unsigned int& numSkippedNeighborTiles)
{
	...
	// Connect with layers in current tile.
	UE_LOGF(LogSegmentLink, Log, "   %s Connect with layers in current tile.", __FUNCTION__);

	int nneis = getTileCountAt(tileX, tileY);
	dtMeshTile** neis = TileArray.PrepareArray(nneis);
	if (neis == nullptr)
	{
		UE_LOGF(LogSegmentLink, Error, "   %s Out of memory.", __FUNCTION__);
	...
	// Connect with neighbour tiles.
	UE_LOGF(LogSegmentLink, Log, "   %s Connect with neighbour tiles.", __FUNCTION__);
	for (int i = 0; i < 8; ++i)
	{
...
dtStatus dtNavMesh::getOffMeshConnectionPolyEndPoints(dtPolyRef prevRef, dtPolyRef polyRef, const dtReal* currentPos, dtReal* startPos, dtReal* endPos) const
{
	...
	if (poly->getType() == DT_POLYTYPE_OFFMESH_SEGMENT)
	{
		...
		tile->verts[poly->verts[idx0]].toWorld(tile->header->bmin, start0); //@UE
		tile->verts[poly->verts[idx1]].toWorld(tile->header->bmin, start1); //@UE
		dtReal t = 0;
		...
		tile->verts[poly->verts[idx2]].toWorld(tile->header->bmin, end0); //@UE
		tile->verts[poly->verts[idx3]].toWorld(tile->header->bmin, end1); //@UE
	...
	//@UE END
	{
		tile->verts[poly->verts[idx0]].toWorld(tile->header->bmin, startPos); //@UE
		tile->verts[poly->verts[idx1]].toWorld(tile->header->bmin, endPos); //@UE
	}
...
void dtNavMesh::applyWorldOffset(const dtReal* offset)
{
	...
	for (int i = 0; i < m_maxTiles; ++i)
	{
		...
		if (tile.header != NULL)
		{
			...
			// Tile vertices are stored as local offsets from bmin; shifting bmin above is sufficient.
