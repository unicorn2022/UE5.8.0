...
int32 FLayoutUV::FChartFinder::FindCharts( const FOverlappingCorners& OverlappingCorners, TArray< FVector2f >& TexCoords, TArray< uint32 >& SortedTris, TArray< FMeshChart >& Charts )
{
	...
	UE_LOGF(LogLayoutUV, VeryVerbose, "FindCharts: %ls", *FPlatformTime::PrettyTime(End - Begin) );
...
bool FLayoutUV::FChartPacker::FindBestPacking(const TArray< FVector2f >& TexCoords, const TArray< uint32 >& SortedTris, TArray< FMeshChart >& Charts)
{
	...
	float LastEfficiency = 0.0f;
	{
		...
		while(!bAbort)
		{
			...
			if (bTrace)
			{
				UE_LOGF(LogLayoutUV, Log, "[LAYOUTUV_TRACE] Scale %f", UVScalePass);
			}
	...
	// Binary search for best fit
	{
		...
		for( int32 i = 0; i < BinarySearchSteps; i++ )
		{
			...
			if (bTrace)
			{
				UE_LOGF(LogLayoutUV, Log, "[LAYOUTUV_TRACE] Scale %f", UVScale);
			}
	...
	UE_LOGF(LogLayoutUV, Verbose, "FindBestPacking (Input Data MD5: %ls, Output Data MD5: %ls, LayoutVersion: %d, Efficiency: %0.2f %%)", *InputHash, *OutputHash, int(LayoutVersion), LastEfficiency*100);
...
{
	UE_LOGF(LogLayoutUV, Log, "FindBestPacking (Total Time: %ls, Avg Efficiency: %f)", *FPlatformTime::PrettyTime(FPlatformTime::ToSeconds64(FindBestPackingCycles.Load())), double(FindBestPackingEfficiency.Load()) / (FindBestPackingCount.Load()*1000));
}
...
bool FLayoutUV::FChartPacker::PackCharts(TArray< FMeshChart >& Charts, float UVScale, const TArray< FVector2f >& TexCoords, const TArray< uint32 >& SortedTris, float& OutEfficiency, TAtomic<bool>& bAbort, bool bTrace)
{
		...
		for( int32 BatchIndex = 0; BatchIndex < BatchSize && ChartIndex < Charts.Num() && !bAbort.Load(EMemoryOrder::Relaxed); ++ChartIndex, ++BatchIndex)
		{
			...
			for( int32 Orientation = 0; Orientation < 8; Orientation++ )
			{
				...
				else
				{
					...
					if (bTrace)
					{
						UE_LOGF(LogLayoutUV, Log, "[LAYOUTUV_TRACE] Chart %d Orientation %d Found = %d Rect = %d,%d,%d,%d\n", ChartIndex, Orientation, bFound ? 1 : 0, Rect.X, Rect.Y, Rect.W, Rect.H);
					}
			...
			else
			{
				if (bTrace)
				{
					UE_LOGF(LogLayoutUV, Log, "[LAYOUTUV_TRACE] Chart %d Found no orientation that fit\n", ChartIndex);
				}
	...
	UE_LOGF(LogLayoutUV, VeryVerbose, "PackCharts: %ls",	*FPlatformTime::PrettyTime(EndPackCharts - BeginPackCharts));
	UE_LOGF(LogLayoutUV, VeryVerbose, "  Rasterize: %llu",	RasterizeCycles);
	UE_LOGF(LogLayoutUV, VeryVerbose, "  Find: %llu",		FindCycles);
