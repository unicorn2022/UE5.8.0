	...
	: FOpenColorIOWrapperConfig()
{
	...
	OCIO_EXCEPTION_HANDLING_TRY();
		...
		if (NewConfig != nullptr)
		{
			...
			{
				UE_LOGF(LogOpenColorIOWrapper, Log, "Could not add the working color space since the config does not define \"aces_interchange\".")
			}
		}
...
TMap<FString, FString> FOpenColorIOWrapperConfig::GetBuiltInConfigNames(bool bRecommendedOnly, bool bShortenUIName) const
{
	TMap<FString, FString> Names;
	
	const OCIO_NAMESPACE::BuiltinConfigRegistry& Registry = OCIO_NAMESPACE::BuiltinConfigRegistry::Get();
	const int32 NumBuiltIns = Registry.getNumBuiltinConfigs();
	Names.Reserve(NumBuiltIns);

	for (size_t Index = 0; Index < NumBuiltIns; ++Index)
	{		
		if (bRecommendedOnly && !Registry.isBuiltinConfigRecommended(Index))
		{
			continue;
		}

		const ANSICHAR* Name = Registry.getBuiltinConfigName(Index);
		FString UIName = Registry.getBuiltinConfigUIName(Index);
		
		if (bShortenUIName)
		{
			UIName.RemoveFromStart(TEXT("Academy Color Encoding System - "));
		}

		Names.Emplace(Name, MoveTemp(UIName));
	}
...
FString FOpenColorIOWrapperConfig::ResolveConfigPath(const TCHAR* InOriginalPath) noexcept
{
	return OCIO_NAMESPACE::ResolveConfigPath(TCHAR_TO_ANSI(InOriginalPath));
}
...
FOpenColorIOWrapperProcessor FOpenColorIOWrapperProcessor::CreateTransformToWorkingColorSpace(const FOpenColorIOWrapperSourceColorSettings& InColorSettings)
{
	...
	if (BuiltInConfig.Pimpl->Config->getNamedTransform(StringCast<ANSICHAR>(*TransformName).Get()) == nullptr)
	{
		...
		{
			/**
			 * Spec-literal ST2084 / PQ: output is display-referred linear light in cd/m2 (nits).
			 * OCIO's built-in CURVE emits nits/100, so we rescale by 100 to recover full nits.
			 */
		...
		{
			/**
			 * Spec-literal BT.2100 inverse OETF via OCIO's built-in CURVE - HLG-OETF-INVERSE,
			 * which emits values in [0, 3] (i.e. 3 * BT.2100 literal). Post-multiply by 1/3
			 * to land at the BT.2100 literal [0, 1] form.
			 */
			BuiltinTransformRcPtr ChildTransform = BuiltinTransform::Create();
			ChildTransform->setStyle("CURVE - HLG-OETF-INVERSE");
			TransformToWCS->appendTransform(ChildTransform);

			MatrixTransformRcPtr MatrixTransform = MatrixTransform::Create();
			FMatrix44d ScaleMat = FMatrix44d::Identity * (1.0 / 3.0);
			ScaleMat.M[3][3] = 1.0;
			MatrixTransform->setMatrix(&ScaleMat.M[0][0]);
...
bool FOpenColorIOWrapperProcessor::TransformImage(const FImageView& InOutImage) const
{
	...
	OCIO_EXCEPTION_HANDLING_TRY();
		...
		if (IsValid())
		{
			...
			{
				UE_LOGF(LogOpenColorIOWrapper, Warning, "Unsupported texture format.");
			}
		}
...
bool FOpenColorIOWrapperProcessor::TransformImage(const FImageView& SrcImage, const FImageView& DestImage) const
{
	...
	OCIO_EXCEPTION_HANDLING_TRY();
		...
		if (IsValid())
		{
			...
			{
				UE_LOGF(LogOpenColorIOWrapper, Warning, "Unsupported texture format(s).");
			}
		}
	OCIO_EXCEPTION_HANDLING_CATCH(Error, TEXT("Failed to transform image. Error message: %s"));
