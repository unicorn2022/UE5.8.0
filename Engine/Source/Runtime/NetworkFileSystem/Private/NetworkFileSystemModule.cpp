...
public:
	...
	virtual INetworkFileServer* CreateNetworkFileServer(FNetworkFileServerOptions FileServerOptions, bool bLoadTargetPlatforms) const override
	{
		...
		case NFSP_Platform:
			...
			UE_LOGF(LogFileServer, Display, "Unreal Network File Server starting up...");
			if (CookOnTheFlyNetworkServer->Start())
			{
				...
				{
					UE_LOGF(LogFileServer, Display, "Unreal Network File Server is ready for client connections on %ls!", *ListenAddresses[0]->ToString(true));
				}
