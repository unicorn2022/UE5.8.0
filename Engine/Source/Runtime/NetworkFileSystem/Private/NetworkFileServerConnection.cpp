...
bool FNetworkFileServerClientConnection::ProcessPayload(FArchive& Ar)
{
	...
	UE_LOGF(LogFileServer, Verbose, "Processing payload with Cmd %d", Cmd);
...

	{
		...
		default:
			...
			UE_LOGF(LogFileServer, Error, "Bad incomming message tag (%d).", (int32)Msg);
		}
	}
	...
	if (Out.Num() && Result )
	{
		...
		UE_LOGF(LogFileServer, Verbose, "Returning payload with %d bytes", Out.Num());
		...
		if (bSendUnsolicitedFiles && Result )
		{
			...
			for (int32 Index = 0; Index < NumUnsolictedFiles; Index++)
			{
				...
				UE_LOGF(LogFileServer, Display, "Returning unsolicited file %ls with %d bytes", *UnsolictedFiles[Index], OutUnsolicitedFile.Num());
...
void FNetworkFileServerClientConnection::ProcessOpenFile( FArchive& In, FArchive& Out, bool bIsWriting )
{
	...
	if (!File)
	{
		UE_LOGF(LogFileServer, Display, "Open request for %ls failed for file %ls.", bIsWriting ? TEXT("Writing") : TEXT("Reading"), *Filename);
...
void FNetworkFileServerClientConnection::ProcessReportLocalFiles( FArchive& In, FArchive& Out )
{
	...
	UE_LOGF(LogFileServer, Display, "There were %d out of date files", OutOfDateFiles.Num());
}
...
bool FNetworkFileServerClientConnection::ProcessGetFileList( FArchive& In, FArchive& Out )
{
	...
	else
	{
		...
		{
			UE_LOGF(LogFileServer, Display, "    Possible Target Platform from client: %ls", *TargetPlatformNames[TPIndex]);

			// look for a matching target platform
			for (int32 ActiveTPIndex = 0; ActiveTPIndex < ActiveTargetPlatforms.Num(); ActiveTPIndex++)
			{
				UE_LOGF(LogFileServer, Display, "   Checking against: %ls", *ActiveTargetPlatforms[ActiveTPIndex]->PlatformName());
		...
		if (ConnectedPlatformName == TEXT(""))
		{
			...
			UE_LOGF(LogFileServer, Warning, "Unable to find target platform for client, terminating client connection!");

			for (int32 TPIndex = 0; TPIndex < TargetPlatformNames.Num() && ConnectedPlatformName == TEXT(""); TPIndex++)
			{
				UE_LOGF(LogFileServer, Warning, "    Target platforms from client: %ls", *TargetPlatformNames[TPIndex]);
			}
			for (int32 ActiveTPIndex = 0; ActiveTPIndex < ActiveTargetPlatforms.Num(); ActiveTPIndex++)
			{
				UE_LOGF(LogFileServer, Warning, "    Active target platforms on server: %ls", *ActiveTargetPlatforms[ActiveTPIndex]->PlatformName());
			}
			return false;
		}
	}
	...
	UE_LOGF(LogFileServer, Display, "    Connected EngineDir      = %ls", *ConnectedEngineDir);
	UE_LOGF(LogFileServer, Display, "        Local EngineDir      = %ls", *LocalEngineDir);
	UE_LOGF(LogFileServer, Display, "    Connected ProjectDir     = %ls", *ConnectedProjectDir);
	UE_LOGF(LogFileServer, Display, "        Local ProjectDir     = %ls", *LocalProjectDir);
	UE_LOGF(LogFileServer, Display, "    Connected EnginePlatformExtDir = %ls", *ConnectedEnginePlatformExtensionsDir);
	UE_LOGF(LogFileServer, Display, "        Local EnginePlatformExtDir = %ls", *LocalEnginePlatformExtensionsDir);
	UE_LOGF(LogFileServer, Display, "    Connected ProjectPlatformExtDir = %ls", *ConnectedProjectPlatformExtensionsDir);
	UE_LOGF(LogFileServer, Display, "        Local ProjectPlatformExtDir = %ls", *LocalProjectPlatformExtensionsDir);
	...
	UE_LOGF(LogFileServer, Display, "Getting files for %d directories, game = %ls, platform = %ls", RootDirectories.Num(), *GameName, *ConnectedPlatformName);
	UE_LOGF(LogFileServer, Display, "    Sandbox dir = %ls", *SandboxDirectory);

	for (int32 DumpIdx = 0; DumpIdx < RootDirectories.Num(); DumpIdx++)
	{
		UE_LOGF(LogFileServer, Display, "\t%ls", *(RootDirectories[DumpIdx]));
	}
	...
	UE_LOGF(LogFileServer, Display, "Scanning server files for timestamps...");
		...
		for (const FString& ExtensionDir : PluginExtensionDirs)
		{
			...
			FPathViews::IterateComponents(
				...
				[&bFoundPlatforms, &bDone, &bWrongPlatform, &PlatformDirectoryNameSet](FStringView CurrentPathComponent)
				{
					if (!bFoundPlatforms)
					{
						if (CurrentPathComponent == FString("Platforms"))
						{
	...
	UE_LOGF(LogFileServer, Display, "Scanned server files, found %d files in %.2f seconds", Visitor.FileTimes.Num(), FPlatformTime::Seconds() - FileScanStartTime);
	...
	{
		UE_LOGF(LogFileServer, Display, "Server list of files  %ls time %d", *FileTime.Key, *FileTime.Value.ToString() );
	}
#endif
...
bool FNetworkFileServerClientConnection::PackageFile( FString& Filename, FString& TargetFilename, FArchive& Out )
{
	...
	if (!File)
	{
		...
		UE_LOGF(LogFileServer, Warning, "Opening file %ls failed", *Filename);
		ServerTimeStamp = FDateTime::MinValue(); // if this was a directory, this will make sure it is not confused with a zero byte file
		bRetVal = false;
	}
	else
	{
		...
		if (!File->Size())
		{
			UE_LOGF(LogFileServer, Warning, "Sending empty file %ls....", *Filename);
		}
		else
		{
			...
			{
				UE_LOGF(LogFileServer, Warning, "Unable to open %ls because it is too large", *Filename);
				bRetVal = false;
			}
		}
		...
		UE_LOGF(LogFileServer, Display, "Read %ls, %d bytes", *Filename, Contents.Num());
	}
...
bool FNetworkFileServerClientConnection::ProcessSyncFile( FArchive& In, FArchive& Out )
{
	...
	UE_LOGF(LogFileServer, Verbose, "Try sync file %ls", *Filename);
