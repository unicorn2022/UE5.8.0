...
class TArrayCollectionArrayBase
{
  public:
	...
	virtual void RemoveAtSwap(const TArrayView<int32>& Indices) = 0;
	virtual uint64 SizeOfElem() const = 0;
	virtual void MoveToOtherArray(const int Idx, TArrayCollectionArrayBase& Other) = 0;
	virtual void MoveToOtherArray(const TArrayView<int32>& Indices, TArrayCollectionArrayBase& Other, const bool bRemoveElements) = 0;
