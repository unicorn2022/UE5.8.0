...
public:
	...
	/** Returns whether two host group instances are the same or not. */
	friend inline bool operator==(const FOnDemandHostGroup& Lhs, const FOnDemandHostGroup& Rhs)
	{
		return Lhs.Impl.Get() == Rhs.Impl.Get();
	}
	/** Returns the type hash of the host group. */ 
	friend inline uint32 GetTypeHash(const FOnDemandHostGroup& HostGroup)
	{
		return *reinterpret_cast<const uint32*>(HostGroup.Impl.Get());
	}
