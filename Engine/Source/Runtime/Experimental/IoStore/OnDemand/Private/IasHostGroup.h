...
public:
	...
	EReconnectionResult AttemptReconnection(bool bForce, uint32 TimeoutMs, std::atomic_bool& CancellationToken);
...
private:
	friend class FHostGroupManager;
	FIASHostGroup(FSharedImpl&& Impl);

	void SetPrimaryHost(int32 Index) const;
	void TrySetTestPath(FAnsiStringView TestPath);
...
public:
	...
	void TrySetTestPath(FName Name, FAnsiStringView TestPath);

	/** Returns true if we have tried to resolve at least one hostgroup and false if there are currently no hostgroups */
	bool ResolveInitialConnection(uint32 TimeoutMs, std::atomic_bool& CancellationToken);
