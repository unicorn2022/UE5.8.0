...
TIoStatusOr<FIASHostGroup> FIASHostGroup::Create(FName Name, TConstArrayView<FAnsiString> HostUrls)
{
	...
	HostGroup.Impl->HttpErrorHistory.Reset();
...
bool FIASHostGroup::Disconnect()
{
	// Never disconnect when the registered test URL is empty
	if (Impl->TestPath.IsEmpty())
	{
		Impl->HttpErrorHistory.Reset();
		return false;
	}
...
FIASHostGroup::EReconnectionResult FIASHostGroup::AttemptReconnection(bool bForce, uint32 TimeoutMs, std::atomic_bool& CancellationToken)
{
	if (!IsConnected() || bForce)
	{
		if (Impl->TestPath.IsEmpty())
		{
			Connect(0);
...
bool FIASHostGroup::OnFailedResponse()
{
	...
	UE_LOGF(LogIas, Verbose, "[%ls] %.2f%% the last %d HTTP requests failed", *Impl->Name.ToString(),  Average * 100.0f, GIasHttpErrorSampleCount);

	if (bAboveHighWaterMark && IsConnected())
	{
		if (Disconnect())
		{
			UE_LOGF(LogIas, Warning, "[%ls] Host group disabled due to high water mark of %.2f of the last %d requests reached",
...
void FIASHostGroup::TrySetTestPath(FAnsiStringView TestPath)
{
	if (Impl->TestPath.IsEmpty())
	{
		Impl->TestPath = TestPath;
	}
}
...
void FHostGroupManager::TrySetTestPath(FName Name, FAnsiStringView TestPath)
{
	UE::TWriteScopeLock _(Mutex);

	FIASHostGroup* Found = HostGroups.FindByPredicate([Name](const FIASHostGroup& Hg)
	{
	...
	if (Found != nullptr)
	{
		Found->TrySetTestPath(TestPath);
	}
}

bool FHostGroupManager::ResolveInitialConnection(uint32 TimeoutMs, std::atomic_bool& CancellationToken)
{
	UE::TReadScopeLock _(Mutex);
	if (!HostGroups.IsEmpty())
	{
		...
		{
			if (HostGroup.AttemptReconnection(true, TimeoutMs, CancellationToken) == FIASHostGroup::EReconnectionResult::Reconnected)
			{
...
void FHostGroupManager::Tick(uint32 TimeoutMs, std::atomic_bool& CancellationToken)
{
	...
	for (FIASHostGroup& HostGroup : HostGroups)
	{
		if (HostGroup.AttemptReconnection(false, TimeoutMs, CancellationToken) == FIASHostGroup::EReconnectionResult::Reconnected)
		{
