...
#	define IAS_HAS_WRITE_COMMIT_THRESHOLD 0
	...
	int32 GetWriteCommitThreshold(FStringView);
#else
	static int32 GetWriteCommitThreshold(FStringView)
	{
		return 0;
	}
#endif
...
{
	return MaxSize > 0 ? (GetUsed() * 100) / MaxSize : 0;
}
...
void FMemCache::Drop()
{
	UE_LOGF(LogIas, Display, "Dropping FMemCache");
...
public:
	...
	bool					DoesMagicSeedMatch(uint32 MagicSeed) const;
...
private:
	...
	static uint32			ProcessMagicSeed(uint32 MagicSeed);
...
, MaxSize(InMaxSize)
{
	...
	Magic = ProcessMagicSeed(MagicSeed);
...
void FDiskJournal::Drop()
{
	...
	if (!Ipf.DeleteFile(*JrnPath))
	{
		...
		UE_LOGF(LogIas, Error, "Failed to delete '%ls' for FDiskJournal ('%d' - '%ls'", *JrnPath, SystemError, SystemErrorMsg);
	}
...
void FDiskJournal::OpenJrnFile()
{
	...
	{
		UE_LOGF(LogIas, Error, "Failed to open '%ls' for FDiskJournal (%ls)", *JrnPath, *Result.GetError().GetMessage());
	}
}

////////////////////////////////////////////////////////////////////////////////
uint32 FDiskJournal::ProcessMagicSeed(uint32 MagicSeed)
{
...
{
	// We may have ended up exactly on the end of the journal file, immediately
	// wrap in that case.
	if (Cursor >= MaxSize)
	{
		Cursor = 0;
...
////////////////////////////////////////////////////////////////////////////////
bool FDiskJournal::DoesMagicSeedMatch(uint32 MagicSeed) const
{
	return Magic == ProcessMagicSeed(MagicSeed);
}
...
{
public:
							FDiskCache(FString&& Path, uint64 InMaxDataSize, uint32 InJournalSize, uint32 InJournalMagic);
	uint32					GetAilments() const;
	FDiskPhrase				OpenPhrase(uint32 DataSize);
	...
	static void				UpdateMagicValue(FDiskCache* Cache, uint32 MagicValue);
...
////////////////////////////////////////////////////////////////////////////////
FDiskCache::FDiskCache(FString&& Path, uint64 InMaxDataSize, uint32 InJournalSize, uint32 InJournalMagic)
: BinPath(MoveTemp(Path))
, MaxDataSize(InMaxDataSize)
, Journal(BinPath, InJournalSize, InJournalMagic)
{
	check(MaxDataSize >= HASH_CHECKSUM_SIZE);

	// Remove the space reserved for the journal and align down to the nearest MiB  keep to some assumptions
...
bool FDiskCache::Evict(uint64 Key)
{
	...
	FOnDemandIoBackendStats::Get()->OnCacheEvictedBytes(Element.Value.Size);

	DataMap.Remove(Id);

	FOnDemandIoBackendStats::Get()->OnCachePersistedBytes(MappedBytes, DataMap.Num());
...
uint64 FDiskCache::Insert(uint64 DataBase, const FDataEntry* Entries, uint32 EntryCount)
{
	...
	// DataMap is updated in the ::Insert call above
	FOnDemandIoBackendStats::Get()->OnCachePersistedBytes(MappedBytes, DataMap.Num());
...
void FDiskCache::Prune(uint64 DataBase, uint32 Size)
{
	...
	for (auto Iter = DataMap.CreateIterator(); Iter; ++Iter)
	{
		...
		const int64 CandidateSize = Candidate.Size;
		Iter.RemoveCurrent();

		MappedBytes -= CandidateSize;
		FOnDemandIoBackendStats::Get()->OnCacheEvictedBytes(CandidateSize);
...
{
	UE_LOGF(LogIas, Display, "Dropping FDiskCache");
...
void FDiskCache::Spam()
{
	...
	UE_LOGF(LogIas, VeryVerbose,
		"JournaledCache: MappedKiB=%llu Entries=%d DataCur=%llu JournalCur=%u Marker=%u)",
		(MappedBytes >> 10),
		DataMap.Num(),
		DataCursor,
		Journal.GetCursor(),
...
////////////////////////////////////////////////////////////////////////////////
void FDiskCache::UpdateMagicValue(FDiskCache* Cache, uint32 MagicValue)
{
	check(Cache != nullptr);

	// Early out if the magic value matches what the journal already has
	if (Cache->Journal.DoesMagicSeedMatch(MagicValue))
	{
		return;
	}

	UE_LOGF(LogIas, Display, "JournaledCache: A new magic value (%u) has been set, reloading the cache from disk...", MagicValue);

	// Preserve the values we will need to reload the cache
	FString Path = MoveTemp(Cache->BinPath);
	const uint64 MaxDataSize = Cache->MaxDataSize;
	const uint32 JournalMaxSize = Cache->Journal.GetMaxSize();

	// Unload the old cache
	Cache->~FDiskCache();

	// Inform the statistics system that the cache has been emptied
	FOnDemandIoBackendStats::Get()->OnCachePersistedBytes(0, 0);

	// Load the cache fresh off disk, this will take the new magic value into account
	new (Cache) FDiskCache(MoveTemp(Path), MaxDataSize, JournalMaxSize, MagicValue);
	LoadCache(*Cache);

	UE_LOGF(LogIas, Display, "JournaledCache: Cache has been reloaded with the new magic value");
}
...
static int32 LoadCache(FDiskCache& DiskCache)
{
	...
	if (auto& Handle = Journal.JrnHandle; Handle.IsValid())
	{
		...
		{
			UE_LOGF(LogIas, Error, "JournaledCache: failed reading journal");
		}
	}
	...
	UE_LOGF(LogIas, VeryVerbose, "JournaledCache: %u byte journal found", DataSize);
	...
	UE_LOGF(LogIas, VeryVerbose, "JournaledCache: %d paragraphs discovered", Paragraphs.Num());
	...
	{
		UE_LOGF(LogIas, VeryVerbose, "JournaledCache: unable to open '%ls'", *DiskCache.BinPath);
		return -1;
	}
	...
	{
		UE_LOGF(LogIas, VeryVerbose,
			"JournaledCache: Dropping - existing cache to bi; %llu/%llu",
	...
	UE_LOGF(LogIas, VeryVerbose, "JournaledCache: Mapped %u items with %llu bytes", MappedItems, MappedBytes);
	...
	{
		UE_LOGF(LogIas, VeryVerbose,
			"JournaledCache: Journal exceeds given size - dropping; %u/%u",
	...
	{
		UE_LOGF(LogIas, VeryVerbose,
			"JournaledCache: Dropping - DataCursor too big; %llu/%llu",
...
public:
	...
	void			UpdateMagicValue(uint32 MagicValue);
...
, DiskCache(MoveTemp(Config.Path), Config.DiskQuota, Config.JournalQuota, Config.JournalMagic)
{
	if (Config.DropCache)
	{
		DiskCache.Drop();
...
FCache::FGetToken FCache::Get(uint64 Key, FIoBuffer& OutData) const
{
	...
	{
		// Nothing is on disk, so lets try the memory cache
		FReadScopeLock _(MemLock);
		...
		if (CloneCachedIOBufferData > 0)
		{
			...
			else if (const FIoBuffer* Data = MemCache.Get(Key); Data != nullptr)
			{
				...
				FOnDemandIoBackendStats::Get()->OnCacheGet(OutData.GetSize(), ECacheType::Memory);
			}
		}
		else
		{
...
EIoErrorCode FCache::Materialize(FGetToken Token, FIoBuffer& OutData, uint32 Offset) const
{
	...
	{
		FOnDemandIoBackendStats::Get()->OnCacheGet(OutData.GetSize(), ECacheType::Disk);
	}
...
////////////////////////////////////////////////////////////////////////////////
void FCache::UpdateMagicValue(uint32 MagicValue)
{
	FDiskCache::UpdateMagicValue(&DiskCache, MagicValue);
}
...
public:
	...
	void					Drop();
	void					UpdateMagicValue(FCache* Cache, uint32 MagicValue);
...
////////////////////////////////////////////////////////////////////////////////
void FServiceThread::UpdateMagicValue(FCache* Cache, uint32 MagicValue)
{
	FWork Work[1] = {};

	Work[0].Cache = Cache;
	Work[0].What = FWork::Work_UpdateMagicValue;
	Work[0].Key = MagicValue;
...
void FServiceThread::ReceiveWork()
{
	...
	for (uint32 i = 0, n = InboundWork.Num(); i < n; ++i)
	{
		...
		if (Work.What == FWork::Work_UpdateMagicValue)
		{
			FCache* Cache = Work.Cache;
			Cache->UpdateMagicValue((uint32)Work.Key);
			continue;
		}
...
public:
	...
	virtual void				UpdateMagicValue(uint32 MagicValue) override;
private:
	struct FMaterialOp
	{
...
bool FJournaledCache::Initialize(const TCHAR* RootDir, const FIasCacheConfig& Config)
{
	...
	{
		UE_LOGF(LogIas, Error, "JournaledCache: Unable to create directory '%ls'", CachePath.ToString());
		return false;
	}
	...
	Cfg.CommitBufferSize	= JournaledCache::GetWriteCommitThreshold(CachePath);
	...
	{
		UE_LOGF(LogIas, Error, "JournaledCache: Error initialising inner cache '%x'", Ailments);
		return false;
	}
...
////////////////////////////////////////////////////////////////////////////////
void FJournaledCache::UpdateMagicValue(uint32 MagicValue)
{
	JournaledCache::FServiceThread::Get().UpdateMagicValue(Cache, MagicValue);
}
...
static void CacheTests(FSupport& Support)
{
	...
	// journal wrap
	{
		...
		UE_LOGF(LogIas, Display, "Journal wrap test found %u correct entries. %u entries were lost.", Found, Lost);
		check(Found > 40);
	}
	...
	// journal wrap 2
	{
		...
		UE_LOGF(LogIas, Display, "Journal wrap test 2 found %u correct entries. %u entries were lost.", Found, Lost);
		check(Found > 40);
	}
	...
	// random writes and allowances, wrap both cache and journal
	{
		...
		while (--Loops)
		{
			...
			UE_LOGF(LogIas, Display, "Random cache test found %u correct entries before reload and %u after out of %d.", FoundBefore, FoundAfter, CommittedBuffers.Num());
