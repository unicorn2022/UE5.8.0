...
public:
	...
	virtual EPackageStoreEntryStatus GetPackageStoreEntryInternal(
							FPackageId PackageId,
							FPackageStoreEntry& OutPackageStoreEntry) override;
	virtual TConstArrayView<uint32> GetSoftReferences(FPackageId PackageId, TConstArrayView<FPackageId>& OutPackageIds) override;

private:
	const FOnDemandPackageStoreEntry* FindPackageEntry(
							const FPackageId& PackageId,
							FPackageStoreEntry& OutPackageStoreEntry) const;
...
{
	const FOnDemandPackageStoreEntry* PackageEntry = FindPackageEntry(PackageId, OutPackageStoreEntry);
	if (PackageEntry == nullptr)
	{
		return EPackageStoreEntryStatus::Missing;
	}

	if (PackageEntry->IsReferenced || IsPackageStreamingEnabled())
	{
		return EPackageStoreEntryStatus::Ok;
	}

	EPackageStoreEntryStatus EntryStatus = EPackageStoreEntryStatus::NotInstalled;
#if !UE_BUILD_SHIPPING
	if (TSharedPtr<FOnDemandIoStore> Pinned = OnDemandIoStore.Pin(); Pinned.IsValid())
	{
		if (IOnDemandDevelopmentExtension* Ext = Pinned->TryGetDevelopmentExtension())
		{
			if (PackageName.IsNone())
			{
				...
				if (FStringView PackageNameView =
					TryConvertChunkIdToPackageName(CreatePackageDataChunkId(PackageId), Filename, Sb);
					PackageNameView.IsEmpty() == false)
				{
					PackageName = FName(PackageNameView);
				}
			}

			if (EPackageStoreEntryStatus Override = Ext->GetUnreferencedPackageStoreEntryStatus(
				PackageName,
				PackageId,
				OutPackageStoreEntry,
				IsDevModeEnabled()); Override == EPackageStoreEntryStatus::Missing)
			{
...
EPackageStoreEntryStatus FOnDemandPackageStoreBackend::GetPackageStoreEntryInternal(
	FPackageId PackageId,
	FPackageStoreEntry& OutPackageStoreEntry)
{
	if (const FOnDemandPackageStoreEntry* PackageEntry = FindPackageEntry(PackageId, OutPackageStoreEntry))
	{
		return EPackageStoreEntryStatus::Ok;
	}
...
const FOnDemandPackageStoreBackend::FOnDemandPackageStoreEntry* FOnDemandPackageStoreBackend::FindPackageEntry(
	const FPackageId& PackageId,
	FPackageStoreEntry& OutPackageStoreEntry) const
{
	for (const FContainer& Container : Containers)
...
void FOnDemandPackageStoreBackend::UpdateLookupTables(const TArray<FSharedOnDemandContainer>& AllContainers, const TArray<TBitArray<>>& ReferencedChunkEntryIndices)
{
	...
	check(ReferencedChunkEntryIndices.Num() ==  AllContainers.Num());
	...
	for (int32 ContainerIdx = 0; const FSharedOnDemandContainer& SharedContainer : AllContainers)
	{
		...
		check(ReferencedChunkEntryIndices[ContainerIdx].Num() == SharedContainer->GetNumChunks());
...
void FOnDemandPackageStoreBackend::UpdateReferencedPackages(const TArray<TBitArray<>>& ReferencedChunkEntryIndices)
{
	...
	{
		check(ReferencedChunkEntryIndices.Num() == Containers.Num());
		for (int32 ContainerIdx = 0; FContainer& Container : Containers)
		{
			...
			{
				ensure(Container.Container->GetNumChunks() == Refs.Num());
				for (TPair<FPackageId, FOnDemandPackageStoreEntry>& Kv : Container.EntryMap)
				{
