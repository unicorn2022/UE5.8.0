...
////////////////////////////////////////////////////////////////////////////////
static FAnsiStringView MimeTypeToString(EMimeType MimeType)
{
	switch (MimeType)
	{
	case EMimeType::Text:				return "text/html";
	case EMimeType::Binary:				return "application/octet-stream";
	case EMimeType::Json:				return "application/json";
	case EMimeType::Xml:				return "application/xml";
	case EMimeType::CbObject:			return "application/x-ue-cb";
	case EMimeType::CbPackage:			return "application/x-ue-pkg";
	case EMimeType::CompressedBuffer:	return "application/x-ue-comp";
	}
	return {};
}
...
////////////////////////////////////////////////////////////////////////////////
FRequest& FRequest::operator = (FRequest&& Rhs)
{
	Swap(Ptr, Rhs.Ptr);
	return *this;
}
...
////////////////////////////////////////////////////////////////////////////////
void FRequest::Accept(EMimeType MimeType)
{
	FAnsiStringView AsString = MimeTypeToString(MimeType);
	Accept(AsString);
}

////////////////////////////////////////////////////////////////////////////////
void FRequest::Accept(FAnsiStringView MimeType)
{
	if (!MimeType.IsEmpty())
	{
...
////////////////////////////////////////////////////////////////////////////////
void FRequest::Header(FAnsiStringView Key, FAnsiStringView Value)
{
...
////////////////////////////////////////////////////////////////////////////////
EContentStatus FResponse::GetContentStatus() const
{
	switch (GetContentLength())
	{
	case -1:	return EContentStatus::UnknownSize;
	case 0:		return EContentStatus::NoContent;
	default:	return EContentStatus::KnownSize;
	}
}
...
void FResponse::GetContentType(FAnsiStringView& Out) const
{
	...
	if (int32 SemiColon; Out.FindChar(';', SemiColon))
	{
		Out = Out.Left(SemiColon).TrimEnd();
	}
}
...
////////////////////////////////////////////////////////////////////////////////
bool FTicketStatus::IsComplete() const
{
	EId Id = GetId();
	if (Id == EId::Response)
	{
		// If the response has no body there will be no further status updates
		return GetResponse().GetContentStatus() == EContentStatus::NoContent;
	}
	...
	if (Id == EId::Content)
	{
		...
		// When the size of content is unknown then the end is signalled with
		// an empty destination buffer. Otherwise a status update is only
		// called when all the destination has been received.
		int64 ContentLength = Activity->GetTransaction()->GetContentLength();
		ContentLength = FMath::Max(ContentLength, 0ll);
		return GetContent().GetSize() == ContentLength;
	}
