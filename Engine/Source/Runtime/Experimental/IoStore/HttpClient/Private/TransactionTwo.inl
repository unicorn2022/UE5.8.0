...
public:
	...
	bool				CancelTransaction(FTlsPeer& Peer, FTransactId Id);
...
: bWithTls(Peer.IsUsingTls())
{
	...
	nghttp2_session_set_local_window_size(Session, 0, 0, RECV_WINDOW_SIZE);
}
...
FOutcome FDriverNg::Handshake(FTlsPeer& Peer)
{
	nghttp2_settings_entry Settings[] = {
		{ NGHTTP2_SETTINGS_INITIAL_WINDOW_SIZE,		RECV_WINDOW_SIZE },
		{ NGHTTP2_SETTINGS_MAX_FRAME_SIZE,			RECV_WINDOW_SIZE >> 2 },
		{ NGHTTP2_SETTINGS_ENABLE_PUSH,				0 },
	};
	nghttp2_submit_settings(Session, NGHTTP2_FLAG_NONE, Settings, UE_ARRAY_COUNT(Settings));
...
FOutcome FDriverNg::TickInner(FTlsPeer& Peer)
{
	...
	{
		if (RecvRemaining < int32(Dest.GetSize()))
		{
			return FOutcome::Error("RecvRemaining value too small");
		}
...
{
	if (RecvFrame.Type == FFrame::EType::Data)
	{
		// Sometimes streams of requests with no content can be closed with an
		// empty data frame instead of via header frame's flags. By deferring
		// RecvResponse() completion we avoid orphaning zero-sized data frames
		if ((RecvFrame.Flags & FFrame::Flag_EndStream) && RecvFrame.GetSize() == 0)
		{
			RecvFrame = {};
		}
		return FOutcome::Ok();
	...
	while (true)
	{
		...
		if (RecvFrame.Type != FFrame::EType::Headers)
		{
			return FOutcome::Error("Not a headers frame", int32(RecvFrame.Type));
		}

		int32 RecvSize = FMath::Min<int32>(sizeof(RecvBuffer), RecvRemaining);

		FMutableMemoryView Dest(RecvBuffer, RecvSize);
		FOutcome Outcome = Peer.Recv((char*)(Dest.GetData()), RecvSize);
		if (!Outcome.IsOk())
		{
			return Outcome;
		}
		...
		Dest = Dest.Left(Outcome.GetResult());
		if (!Dest.IsEmpty() && (Outcome = PumpRecv(Peer, Dest)).IsError())
		{
			return Outcome;
		}
		...
		check(RecvRemaining == 0);
		RecvRemaining -= sizeof(RecvFrame);

		int32 EndOfHeaders = (RecvFrame.Flags & FFrame::Flag_EndHeaders);

		if (RecvFrame.Flags & FFrame::Flag_EndStream)
		{
			check(EndOfHeaders);
			return FOutcome::Ok();
		}
		...
		if (Outcome = Tick(Peer); Outcome.IsError())
		{
			return Outcome;
		}

		if (Outcome.GetResult() != ThisId)
		{
		...
		if (EndOfHeaders)
		{
			// We shall end headers once the first data frame arrives, which may
			// have already happened in the Tick() call.
			return RecvResponse(Peer, ResponseHeaders);
		}
	}
}
...
FOutcome FDriverNg::RecvBody(FTlsPeer& Peer, FMutableMemoryView Dest)
{
	for (uint32 RecvSize = 0;;)
	{
		...
		if (RecvFrame.Type != FFrame::EType::Data)
		{
			return FOutcome::Error("Not a data frame", int32(RecvFrame.Type));
		}
		...
		if (View.IsEmpty())
		{
			if (RecvRemaining <= 0 && EndOfStream)
			{
				RecvFrame = {};
...
////////////////////////////////////////////////////////////////////////////////
bool FDriverNg::CancelTransaction(FTlsPeer& Peer, FTransactId Id)
{
	int32 Result = nghttp2_submit_rst_stream(Session, NGHTTP2_FLAG_NONE, Id, NGHTTP2_CANCEL);
	if (Result)
	{
		return false;
	}
...
////////////////////////////////////////////////////////////////////////////////
void FDriverNg::SkipTransaction()
{
	if (!RecvFrame.IsPertinent())
	{
		return;
...
FOutcome TickTwo(FTlsPeer& Peer, void* PeerData)
{
	...
	if (Driver == nullptr)
	{
		return FOutcome::Error("ng-driver is unexpectedly null");
	}
...
////////////////////////////////////////////////////////////////////////////////
bool CancelTwo(FTlsPeer& Peer, void* PeerData, FTransactId Id)
{
	using namespace DetailTwo;
	...
	if (Driver != nullptr)
	{
		return Driver->CancelTransaction(Peer, Id);
	}
...
////////////////////////////////////////////////////////////////////////////////
void SkipPendingTransactTwo(FTlsPeer&, void* PeerData)
{
	using namespace DetailTwo;
...
bool		CancelTwo(FTlsPeer&, void*, FTransactId){ return false; }
void		SkipPendingTransactTwo(FTlsPeer&, void*){}
