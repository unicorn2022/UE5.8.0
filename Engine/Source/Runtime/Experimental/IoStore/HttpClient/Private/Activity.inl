...
struct FActivity
{
	...
	bool				CancelTransaction(FHttpPeer& Peer);
	void				Cancel();
	void				Done();
...
////////////////////////////////////////////////////////////////////////////////
bool FActivity::CancelTransaction(FHttpPeer& Peer)
{
	if (TransactId == 0)
	{
		return true;
	...
	bool Result = Peer.Cancel(TransactId);
	return Result;
}
...
{
	check(State == EState::RecvContent || State == EState::RecvStream || State == EState::RecvDone);
	return *Dest;
}
...
FOutcome FActivity::RecvMessage(FHttpPeer& Peer)
{
	...
	{
		// Destination is too small for all the content. The user did however
		// provide a size so we'll use that to deliver content piecewise.
		bChunked = true;
	}
