...
void FSimModuleTree::Reparent(int AtIndex, int ParentIndex)
{
	...
	UE_LOGF(LogSimulationModule, Log, "Reparent %ls To %ls"
			...
			if (ISimulationModuleBase* Module = SimulationModuleTree[i].SimModule)
			{
				...
				if (Module->IsBehaviourType(eSimModuleTypeFlags::Velocity))
				{
					...
					if (Particle)
					{
						...
						FVector GroundVel = FVector::ZeroVector;
						if (FWheelBaseInterface* Wheel = Module->Cast<FWheelBaseInterface>())
						{
							GroundParticle = Wheel->GetGroundBody();
							if (GroundParticle && GroundParticle != RootParticle)
							{
								const Chaos::FVec3 GroundArm = WorldLocation - GroundParticle->GetX();
								GroundVel = GroundParticle->GetV() - Chaos::FVec3::CrossProduct(GroundArm, GroundParticle->GetW());
							}
						}

						FVector WorldVelocity = Particle->GetV() - Chaos::FVec3::CrossProduct(Arm, Particle->GetW()) - GroundVel;
