...
bool bModularVehicle_ApplyWheelForceToGroundBody = true;
FAutoConsoleVariableRef CVarModularVehicleApplyWheelForceToGroundBody(TEXT("p.ModularVehicle.ApplyWheelForceToGroundBody.Enabled"), bModularVehicle_ApplyWheelForceToGroundBody, TEXT("Enable/Disable aplication of wheel forces to the ground body."));

float bModularVehicle_WheelForceToGroundBodyScaling = 1.0f;
FAutoConsoleVariableRef CVarModularVehicleWheelForceToGroundBodyScaling(TEXT("p.ModularVehicle.WheelForceToGroundBodyScaling"), bModularVehicle_WheelForceToGroundBodyScaling, TEXT("Sacle froces applied to ground body from wheel."));
...
namespace Chaos
{
	...
	void FWheelSimModule::Simulate(float DeltaTime, const FAllInputs& Inputs, FSimModuleTree& VehicleModuleSystem)
	{
		...
		if (bTouchingGround)
		{
			...
			// This code applies torque to the body the wheel is touching.
			FVector AppliedWorldForce = -ForceFromFriction;
			...
			if (bModularVehicle_ApplyWheelForceToGroundBody && GroundBody && GroundBody->IsDynamic())
			{
				const Chaos::FVec3 WorldCOM = Chaos::FParticleUtilitiesXR::GetCoMWorldPosition(GroundBody);

				const FTransform VehicleTransform = Inputs.VehicleWorldTransform;
				FTransform WheelRelativeTransform = GetParentRelativeTransform();

				AppliedWorldForce = SteeringRotator.RotateVector(AppliedWorldForce);
				Chaos::FVec3 WorldForce = WheelRelativeTransform.TransformVector(AppliedWorldForce);
				WorldForce = VehicleTransform.TransformVector(WorldForce);
				WorldForce *= bModularVehicle_WheelForceToGroundBodyScaling;
				Chaos::FVec3 Arm = ImpactPoint - WorldCOM;
				Chaos::FVec3 WorldTorque = Chaos::FVec3::CrossProduct(Arm, WorldForce);

				GroundBody->AddForce(WorldForce);
				GroundBody->AddTorque(WorldTorque);

				//ImpactPoint.Z += 10.0f;
				//Chaos::FDebugDrawQueue::GetInstance().DrawDebugLine(ImpactPoint, ImpactPoint + WorldForce * 0.01f, FColor::Cyan, false, -1.f, 0, 2.f);
				//Chaos::FDebugDrawQueue::GetInstance().DrawDebugLine(ImpactPoint, ImpactPoint - Arm, FColor::Red, false, -1.f, 0, 2.f);
				//Chaos::FDebugDrawQueue::GetInstance().DrawDebugSphere(WorldCOM, 20, 16, FColor::Red, false, -1.f, 0, 2.f);
			}
		}
