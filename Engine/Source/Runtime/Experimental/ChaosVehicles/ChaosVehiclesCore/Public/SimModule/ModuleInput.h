...
enum class EModuleInputBufferActionType : uint8
{
	...
	// A running average of the inputs.
	Average
};
...
public:
	...
	FModuleInputValue(bool bInValue) : bHasValue(true), Value(FVector::ZeroVector), ValueInt(bInValue), ValueType(EModuleInputValueType::MBoolean)  {}
	FModuleInputValue(MInteger InValue) : bHasValue(true), Value(FVector::ZeroVector), ValueInt(InValue), ValueType(EModuleInputValueType::MInteger) {}
	FModuleInputValue(MAxis1D InValue) : bHasValue(true), Value(InValue, 0.f, 0.f), ValueInt(0), ValueType(EModuleInputValueType::MAxis1D) {}
	FModuleInputValue(MAxis2D InValue) : bHasValue(true), Value(InValue.X, InValue.Y, 0.f), ValueInt(0), ValueType(EModuleInputValueType::MAxis2D) {}
	FModuleInputValue(MAxis3D InValue) : bHasValue(true), Value(InValue), ValueInt(0), ValueType(EModuleInputValueType::MAxis3D) {}
	...
	// Build a specific type with no value
	FModuleInputValue(EModuleInputValueType InValueType)
	{
	...
	UE_API void Average(const FModuleInputValue& With);
	...
	/** During physics resimulation, if input is being decayed, inverse the decay value, this produces better results sometimes for some inputs like steering */
	void SetInverseInputDecay(const bool bInInverseInputDecay) { bInverseInputDecay = bInInverseInputDecay; }
	bool ShouldInverseInputDecay() const { return  bInverseInputDecay; }

	/** After simulation first consumes the input, clear the value so that it  doesn't persist across multiple async physics ticks */
...
public:
	...
	void AverageValueAtIndex(int Index, const FModuleInputValue& InValue, EModuleInputQuantizationType InInputQuantizationType)
	{
		InputValues[Index].Average(InValue.ReturnQuantized(InInputQuantizationType));
	}
...
public:
	...
	UE_API void AverageValue(const FName& InName, const FModuleInputValue& InValue);
	...
	void SetBool(const FName& InName, bool InBool)
	{
		...
		FModuleInputValue Value(InBool);
...
public:
	...
	/** Special case override for providing test input straight onto the physics thread */
	virtual int32 GetTestInputStartFrame() { return -1; }
