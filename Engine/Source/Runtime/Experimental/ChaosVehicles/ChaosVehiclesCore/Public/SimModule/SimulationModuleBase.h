...
namespace Chaos
{
	...
	{
		FCreatedModule() : SimType("Empty"), Guid(INDEX_NONE), TreeIndex(INDEX_NONE)
		{
		}

		FCreatedModule(const FName& InSimType, int InGuid, int InTreeIndex)
			: SimType(InSimType), Guid(InGuid), TreeIndex(InTreeIndex)
		{
		}

		FCreatedModule(const FCreatedModule& Other)
			: SimType(Other.SimType), Guid(Other.Guid), TreeIndex(Other.TreeIndex)
		{
		}
		FCreatedModule& operator=(const FCreatedModule& Other)
		{
	...
	struct FCreatedModules
	{
		...
		TArray<Chaos::FCreatedModule> ModuleEvents;
	};
...
#define DEFINE_CHAOSSIMTYPENAME(cls) \
	...
	public:
		...
		{}
		virtual ~ISimulationModuleBase() = default;
	...
	*/
	struct FModuleNetData : public FSimulationModuleTypeContainer, public TSimulationModuleTypeableBase<FModuleNetData>
	{
		...
		virtual ~FModuleNetData() = default;
	...
	struct FSimOutputData : public FSimulationModuleTypeContainer, public TSimulationModuleTypeableBase<FSimOutputData>
	{
		...
		virtual ~FSimOutputData() = default;
		...
		UE_API virtual void GetFinalAnimDataGameThread(const FTransform& NewTransform, Chaos::FSimModuleAnimationData& AnimDataOut) { AnimDataOut = AnimationData; }
	...
	public:
		...
		virtual ~IFactoryModule() = default;
