...
// RotatedRadialVector
FFieldNodeBase* URotatedRadialVector::NewEvaluationGraph(TArray<const UFieldNodeBase*>& Nodes) const
{
	if (ensureMsgf(!Nodes.Contains(this), TEXT("Cycle Dependency Error : Graph nodes may not be reused in a single chain.")))
	{
		Nodes.Add(this);
		return new FRotatedRadialVector(Magnitude, Position, Angle, Axis);
	}
	return nullptr;
};

URotatedRadialVector* URotatedRadialVector::SetRotatedRadialVector(float InMagnitude, FVector InPosition, float InAngle, FVector InAxis)
{
	this->Magnitude = InMagnitude;
	this->Position = InPosition;
	this->Angle = InAngle;
