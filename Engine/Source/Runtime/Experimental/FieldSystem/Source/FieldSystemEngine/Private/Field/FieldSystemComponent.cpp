...
UFieldSystemComponent::UFieldSystemComponent(const FObjectInitializer& ObjectInitializer)
	...
	, bIsChaosField(true)
	, SupportedSolvers()
	, bEnableInEditorWorlds(false)
	, bEnableInPreviewWorlds(false)
	, ChaosModule(nullptr)
...
{
	UE_LOGF(FSC_Log, Log, "FieldSystemComponent[%p]::UFieldSystemComponent()",this);
...
{
	UE_LOGF(FSC_Log, Log, "FieldSystemComponent[%p]::CreateSceneProxy()", this);
...
void UFieldSystemComponent::OnCreatePhysicsState()
{
	...
	if(UWorld* World = GetWorld())
	{
		const bool bValidWorld =
			World->IsGameWorld() ||
			(bEnableInEditorWorlds && World->IsEditorWorld()) ||
			(bEnableInPreviewWorlds && World->IsPreviewWorld());
