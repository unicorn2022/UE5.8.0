...
private:
	...
	void Pause(const TArray<FString>& Args)
	{
		...
		case 1:
			...
			{
				UE_LOGF(LogChaosDebug, Display, "%d", (*SolverActor)->ChaosDebugSubstepControl.bPause);
				return;
			}
			break;  // Invalid arguments
		case 2:
		...
		UE_LOGF(LogChaosDebug, Display, "Invalid arguments.");
		UE_LOGF(LogChaosDebug, Display, "Usage:");
		UE_LOGF(LogChaosDebug, Display, "  p.Chaos.Solver.Pause [SolverName] [0|1|]");
		UE_LOGF(LogChaosDebug, Display, "  SolverName  The Id name of the solver as shown by p.Chaos.Solver.List");
		UE_LOGF(LogChaosDebug, Display, "  0|1|        Use either 0 to unpause, 1 to pause, or nothing to query");
		UE_LOGF(LogChaosDebug, Display, "Example: p.Chaos.Solver.Pause ChaosSolverActor_3 1");
	}
	...
	void Step(const TArray<FString>& Args)
	{
		...
		UE_LOGF(LogChaosDebug, Display, "Invalid arguments.");
		UE_LOGF(LogChaosDebug, Display, "Usage:");
		UE_LOGF(LogChaosDebug, Display, "  p.Chaos.Solver.Step [SolverName]");
		UE_LOGF(LogChaosDebug, Display, "  SolverName  The Id name of the solver as shown by p.Chaos.Solver.List");
		UE_LOGF(LogChaosDebug, Display, "Example: p.Chaos.Solver.Step ChaosSolverActor_3");
#endif
	}
	...
	void Substep(const TArray<FString>& Args)
	{
		...
		UE_LOGF(LogChaosDebug, Display, "Invalid arguments.");
		UE_LOGF(LogChaosDebug, Display, "Usage:");
		UE_LOGF(LogChaosDebug, Display, "  p.Chaos.Solver.Substep [SolverName]");
		UE_LOGF(LogChaosDebug, Display, "  SolverName  The Id name of the solver as shown by p.Chaos.Solver.List");
		UE_LOGF(LogChaosDebug, Display, "Example: p.Chaos.Solver.Substep ChaosSolverActor_3");
#endif
	}
...
void AChaosSolverActor::ConditionalCreateSolver()
{
	if(PhysScene)
	{
		return;
...
void AChaosSolverActor::PreRegisterAllComponents()
{
	Super::PreRegisterAllComponents();

	ConditionalCreateSolver();
}
...
void AChaosSolverActor::PostUnregisterAllComponents()
{
	...
	// At this point all components/actors that connect to the solver actor via register/beginplay
	// should now no longer be resident within the physics scene so we can remove our reference
	PhysScene = nullptr;
...
void AChaosSolverActor::MakeFloor()
{
	if(bHasFloor)
	{
		...
		FloorParticle->SetShapeFilterData(0, ShapeFilterData);
		FloorParticle->SetShapeQueryCollisionEnabled(0, false);
...
{
	UE_LOGF(LogChaos, Log, "Serializing for perf test:");
	
	const FString FileName("ChaosPerf");
