...
FConstraintTickFunction& UTickableConstraint::GetTickFunction(const UWorld* InWorld)
{
	if (!ensure(InWorld && InWorld->GetCurrentLevel()))
	{
		static FConstraintTickFunction DummyFunction;
		return DummyFunction;
	}

	if (TUniquePtr<FConstraintTickFunction>* Found = ConstraintTicks.Find(InWorld->GetCurrentLevel()))
	{
		if (!ensure(Found->Get()))
		{
			(*Found) = MakeUnique<FConstraintTickFunction>();
		}
		return *Found->Get();
	}
	
	TUniquePtr<FConstraintTickFunction>& NewFunction = ConstraintTicks.Emplace(InWorld->GetCurrentLevel(), MakeUnique<FConstraintTickFunction>());
	return *NewFunction.Get();
}

const FConstraintTickFunction& UTickableConstraint::GetTickFunction(const UWorld* InWorld) const
{
	if (!ensure(InWorld && InWorld->GetCurrentLevel()))
	{
		static const FConstraintTickFunction DummyFunction;
		return DummyFunction;
	}
...
FConstraintTickFunction* UTickableConstraint::FindTickFunction(const UWorld* InWorld) const
{
	if (!ensure(InWorld && InWorld->GetCurrentLevel()))
	{
		return nullptr;
	}
	const TUniquePtr<FConstraintTickFunction>* Found = ConstraintTicks.Find(InWorld->GetCurrentLevel());
...
void UTickableConstraint::SetActive(const bool bIsActive)
{
	...
	for (auto& [WeakLevel, UniqueFunction] : ConstraintTicks)
	{
		if (WeakLevel.IsValid() && UniqueFunction.IsValid())
		{
			UniqueFunction->SetTickFunctionEnable(bIsActive);
		}
	}
}
...
{
	for (const auto& [WeakLevel, UniqueFunction] : ConstraintTicks)
	{
		if (WeakLevel.IsValid() && UniqueFunction.IsValid())
		{
			UniqueFunction->EvaluateFunctions();
		}
	}
}
...
void UTickableConstraint::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyName == GET_MEMBER_NAME_CHECKED(UTickableConstraint, Active))
	{
		for (auto& [WeakLevel, UniqueFunction] : ConstraintTicks)
		{
			if (WeakLevel.IsValid() && UniqueFunction.IsValid())
			{
				UniqueFunction->SetTickFunctionEnable(Active);
				if (Active)
				{
					UniqueFunction->EvaluateFunctions();
				}
...
void UTickableConstraint::PostEditUndo()
{
	...
	for (auto& [WeakLevel, UniqueFunction] : ConstraintTicks)
	{
		if (WeakLevel.IsValid() && UniqueFunction.IsValid())
		{
			UniqueFunction->SetTickFunctionEnable(bActiveTick);
			UniqueFunction->bCanEverTick = bActiveTick;
		}
	}
}
...
{
	UE_LOGF(LogTemp, Error, "nb consts = %d", Constraints.Num());
	for (const TObjectPtr<UTickableConstraint>& Constraint: Constraints)
	{
		if (IsValid(Constraint))
		{
			UE_LOGF(LogTemp, Warning, "\t%ls (target hash = %u)",
...
void FConstraintsManagerController::StaticConstraintCreated(UTickableConstraint* InConstraint)
{
	if (UConstraintsManager* Manager = GetManager()) //created If needed
	{
		if (Manager->Constraints.Contains(InConstraint) == false)
		{
			...
			InConstraint->Rename(nullptr, Manager, REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
...
void FConstraintsManagerController::EvaluateAllConstraints() const
{
	...
	UE::Anim::Constraints::EvaluateConstraints(Constraints);
}
