...
void UConstraintSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	{
		FCoreDelegates::GetOnPostEngineInit().AddUObject(this, &UConstraintSubsystem::InitializeInternal);
	}
...
void UConstraintSubsystem::RemoveConstraint(UWorld* InWorld, UTickableConstraint* InConstraint, bool bDoNoCompensate)
{
	...
	// store tick function so that it can be removed from prerequisites
	const FConstraintTickFunction* RemovedTickFunction = InConstraint->FindTickFunction(InWorld);
	...
	if (FConstraintsInWorld* Constraints = ConstraintsInWorldFind(InWorld))
	{
		...
		// remove useless tick prerequisites on remaining constraints
		const bool bTickFunctionRemoved = RemovedTickFunction && !InConstraint->FindTickFunction(InWorld);
		if (bTickFunctionRemoved)
		{
			static constexpr bool bEvenIfPendingKill = true;
			for (const TWeakObjectPtr<UTickableConstraint>& OtherConstraint: Constraints->Constraints)
			{
				if (OtherConstraint.IsValid() && !OtherConstraint.IsStale())
				{
					if (FConstraintTickFunction* OtherTickFunction = OtherConstraint->FindTickFunction(InWorld))
					{
						TArray<FTickPrerequisite>& Prerequisites = OtherTickFunction->GetPrerequisites();
						for (int32 PrereqIndex = 0; PrereqIndex < Prerequisites.Num(); PrereqIndex++)
						{
							...
							if (!PrereqObject)
							{
								// remove prerequisite coming from a stale object (cf. FTickFunction::QueueTickFunction)
								Prerequisites.RemoveAtSwap(PrereqIndex--);
							}
							else if (PrereqObject == this && Prerequisite.Get() == RemovedTickFunction)
							{
								// remove prerequisite from the constraint just removed (cf. UConstraintSubsystem::SetConstraintDependencies) 
								Prerequisites.RemoveAtSwap(PrereqIndex--);
							}
						}	
					}
