...
static bool	bSupportIndirectDependencies = false;
static FAutoConsoleVariableRef CVarSupportIndirectDependencies(
	TEXT("Constraints.SupportIndirectDependencies"),
	bSupportIndirectDependencies,
	TEXT("Support self indirect dependencies between handles.") );
					...
					const UTickableConstraint* InParentConstraint, const UTickableConstraint* InChildConstraint)
{
	...
	UE_LOGF(LogTemp, Warning, "%ls: '%ls' is parent of '%ls' so '%ls' must tick before '%ls'",
		*InDescription,
...
void LogIndirectDependency(const UTransformableHandle* InParentHandle, const UTransformableHandle* InChildHandle,
	const UTickableConstraint* InParentConstraint, const UTickableConstraint* InChildConstraint)
{
	if (!bDebugDependencies)
	{
	...
	UE_LOGF(LogTemp, Warning, "Self Indirect Dependency: '%ls' has a lower hierarchy depth compared to '%ls' so '%ls' must tick before '%ls'",
		*GetHandleLabel(InParentHandle), *GetHandleLabel(InChildHandle),
		*GetConstraintLabel(InParentConstraint), *GetConstraintLabel(InChildConstraint));
}
								...
								const UTickableConstraint* InAttachConstraint, const UTickableConstraint* InChildConstraint)
{
	...
	UE_LOGF(LogTemp, Warning, "%ls: '%ls' is an attach parent of '%ls' so '%ls' must tick before '%ls'",
		*InDescription,
...
void FDependencyBuilder::BuildSelfDependencies(UWorld* InWorld, UTickableTransformConstraint* InConstraint)
{
	...
	auto SetDependency = [InWorld](const FGuid& InGuidToTickBefore, const FGuid& InGuidToTickAfter)
	{
		FConstraintsManagerController& Controller = FConstraintsManagerController::Get(InWorld);
		Controller.SetConstraintsDependencies(InGuidToTickBefore, InGuidToTickAfter);
	};
	...
	bool bDirectDependencyAdded = false;
	for (const ConstraintWeakPtr& SelfConstraint: SelfConstraints)
	{
		...
		{
			SetDependency(InConstraint->ConstraintID, TransformConstraint->ConstraintID);
	...
	// look for indirect dependencies (hierarchy depth, for example)
	if (bSupportIndirectDependencies && !bDirectDependencyAdded)
	{
		for (const ConstraintWeakPtr& SelfConstraint: SelfConstraints)
		{
			const UTickableTransformConstraint* TransformConstraint = Cast<UTickableTransformConstraint>(SelfConstraint);
			
			// if the new handles indirectly depend on that constraint child then, TransformConstraint should tick before
			if (ParentHandle->HasIndirectDependencyWith(*TransformConstraint->ChildTRSHandle))
			{
				SetDependency(TransformConstraint->ConstraintID, InConstraint->ConstraintID);
				LogIndirectDependency(TransformConstraint->ChildTRSHandle, ParentHandle, TransformConstraint, InConstraint);
			}
			else if (ChildHandle->HasIndirectDependencyWith(*TransformConstraint->ChildTRSHandle))
			{
				SetDependency(TransformConstraint->ConstraintID, InConstraint->ConstraintID);
				LogIndirectDependency(TransformConstraint->ChildTRSHandle, ChildHandle, TransformConstraint, InConstraint);
			}

			// if the TransformConstraint handles indirectly depend on the new constraint child then, TransformConstraint should tick after
			if (TransformConstraint->ParentTRSHandle->HasIndirectDependencyWith(*ChildHandle))
			{
				SetDependency(InConstraint->ConstraintID, TransformConstraint->ConstraintID);
				LogIndirectDependency(ChildHandle, TransformConstraint->ParentTRSHandle, InConstraint, TransformConstraint);
			}
			else if (TransformConstraint->ChildTRSHandle->HasIndirectDependencyWith(*ChildHandle))
			{
				SetDependency(InConstraint->ConstraintID, TransformConstraint->ConstraintID);
				LogIndirectDependency(ChildHandle, TransformConstraint->ChildTRSHandle, InConstraint, TransformConstraint);
			}
		}
	}
}
				...
				for (int32 Index = ConstrainIndex-1; Index >= 0; Index--)
				{
					if (!ManagedDependencies[Index])
					{
							...
							!FConstraintCycleChecker::HasPrerequisiteDependencyWith(&ParentTickFunctionToCheck, &TickFunction, VisitedFunctions))
						{
							...
							if (bDebugDependencies)
							{
								UE_LOGF(LogTemp, Warning, "Creation Order Dependency: '%ls' must tick before '%ls' to respect creation order.",
	...
	// get all constraints having the same parent's target as the new child's target and make sure they tick after if needed 
	{
		...
		for (const ConstraintWeakPtr& Constraint: ParentTargetChildConstraints)
		{
				...
				!FConstraintCycleChecker::HasPrerequisiteDependencyWith(&ParentTickFunctionToCheck, &TickFunction, VisitedFunctions1))
			{
				...
				if (bDebugDependencies)
				{
					UE_LOGF(LogTemp, Warning, "External Dependency: '%ls' must tick before '%ls' as it acts on its parent.",
...
bool FDependencyBuilder::BuildDependencies(UWorld* InWorld, UTickableTransformConstraint* InConstraint)
{
	...
	if (bDebugDependencies)
	{
		UE_LOGF(LogTemp, Warning, "Building dependencies for '%ls' ...", *GetConstraintLabel(InConstraint));
	}
	...
	if (!ChildParentConstraints.IsEmpty())
	{
		...
		if (bDebugDependencies)
		{
			UE_LOGF(LogTemp, Warning, "Order Dependency: tick after last constraint.");
		}
	...
	{
		UE_LOGF(LogTemp, Warning, "A cycle has been formed while creating %ls.", *InConstraint->GetName());
	}
