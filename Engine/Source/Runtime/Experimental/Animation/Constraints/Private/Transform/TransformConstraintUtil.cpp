...
void NotifyError(const FText& InNotification)
{
	// output log notification
	UE_LOGF(LogAnimation, Error, "%ls", *InNotification.ToString());
...
FConstraintsInteractionCache::~FConstraintsInteractionCache()
{
	Reset();
...
UTickableTransformConstraint* CreateFromType(UWorld* InWorld, const ETransformConstraintType InType, const bool bUseDefault)
{
	if (!InWorld)
	{
		const FText Notification =
			NSLOCTEXT("TransformConstraintUtil", "CreateFromType_NoWorld", "Cannot create a new constraint in a null world.");
		Private::NotifyError(Notification);
		return nullptr;
	}
	...
	if (!ETransformConstraintTypeEnum->IsValidEnumValue(static_cast<int64>(InType)))
	{
		const FText Notification = FText::Format(
			NSLOCTEXT("TransformConstraintUtil", "CreateFromType_UnknownType",  "Cannot create a new constraint with unknown type '{0}'."),
			FText::AsNumber(static_cast<int32>(InType)));
		Private::NotifyError(Notification);
		return nullptr;
	}
...
bool AreHandlesConstrainable( UWorld* InWorld, const UTransformableHandle* InParentHandle, const UTransformableHandle* InChildHandle)
{
	if (InChildHandle->GetHash() == InParentHandle->GetHash())
	{
		const FText Notification =
			NSLOCTEXT("TransformConstraintUtil", "AreHandlesConstrainable_SameObject", "Cannot create a constraint on itself.");
	...
	if (InParentHandle->HasDirectDependencyWith(*InChildHandle))
	{
		...
		Args.Add(TEXT("Parent"), FText::FromString(InParentHandle->GetLabel()));
		Args.Add(TEXT("Child"), FText::FromString(InChildHandle->GetLabel()));
		const FText Notification = FText::Format(NSLOCTEXT("TransformConstraintUtil", "AreHandlesConstrainable_DirectDependency",
			"Cannot create a new constraint between '{Parent}' and '{Child}' as there is a direct dependency between them."), Args);
		NotifyError(Notification);
#endif
		return false;
	}
	...
	if (HasConstraintDependencyWith(InWorld, InParentHandle, InChildHandle))
	{
		...
		const FText Notification = FText::Format(NSLOCTEXT("TransformConstraintUtil", "AreHandlesConstrainable_IndirectDependency",
			"Cannot create a new constraint between '{Parent}' and '{Child}' as there is an indirect dependency between them."), Args);
		NotifyError(Notification);
#endif
		return false;
	}
	...
	const TFunction<void()>& InValidDependencyFunction)
{
	...
	{
		const FText Notification =
			NSLOCTEXT("TransformConstraintUtil", "CreateAndAddFromObjects_SanityCheck", "Cannot create a new constraint with invalid world on handles.");
		Private::NotifyError(Notification);
		return nullptr;
	}
	...
	UTickableTransformConstraint* InNewConstraint, const bool bMaintainOffset, const bool bUseDefault)
{
	...
	{
		const FText Notification =
			NSLOCTEXT("TransformConstraintUtil", "AreHandlesConstrainable_InvalidHandles", "Cannot create a new constraint between invalid handle(s).");
		Private::NotifyError(Notification);
	...
	{
		const FText Notification =
			NSLOCTEXT("TransformConstraintUtil", "AreHandlesConstrainable_InvalidConstraint", "Cannot add a null constraint.");
		Private::NotifyError(Notification);
