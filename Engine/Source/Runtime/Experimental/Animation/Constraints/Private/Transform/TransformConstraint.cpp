...
{
	if (::IsValid(ChildTRSHandle) && ::IsValid(ParentTRSHandle))
	{
...
void UTickableTransformConstraint::SetupDependencies(const UWorld* InWorld)
{
	...
	if (ParentTRSHandle)
	{
		...
		if(PTRS != this)
		{
			ParentTRSHandle->Rename(nullptr, this, REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
		}
	}
	if (ChildTRSHandle)
	{
		...
		if (CTRS != this)
		{
			ChildTRSHandle->Rename(nullptr, this, REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
		}
	}
	...
	if (UE::Private::ShouldForceParentDependency() && ParentTickFunction && (ChildTickFunction != ParentTickFunction))
	{
		...
		FConstraintTickFunction& ConstraintTick = GetTickFunction(InWorld);
...
void UTickableTransformConstraint::EnsurePrimaryDependency(const UWorld* InWorld)
{
	if (!ensure(InWorld && InWorld->GetCurrentLevel()))
	{
		return;
		...
		if (ParentPrerequisites.IsEmpty())
		{
			...
			if (FTickFunction* PotentialFunction = PrimaryPrerex.Get())
			{
				...
				if (!FConstraintCycleChecker::HasPrerequisiteDependencyWith(&ConstraintTick, PotentialFunction, VisitedFunctions))
				{
					...
					{
						UE_LOGF(LogTemp, Warning, "EnsurePrimaryDependency: '%ls' must tick before '%ls'",
...
void UTickableTransformConstraint::TeardownConstraint(UWorld* InWorld)
{
	...
	TUniquePtr<FConstraintTickFunction>* Found = Level ? ConstraintTicks.Find(Level) : nullptr;
	if (Found && Found->IsValid())
	{
		TUniquePtr<FConstraintTickFunction>& ConstraintTick = *Found;
		...
		{
			// If ChildTickFunction is null, meaning that the handle no longer references any component,
			// we must ensure that no component in this level still has it as a prerequisite.
			FConstraintTickFunction* ConstraintTickPtr = ConstraintTick.Get();
			...
			for (const TObjectPtr<AActor>& Actor: Level->Actors)
			{
				if (Actor)
				{
					for (UActorComponent* Component: Actor->GetComponents())
					{
						if (Component)
						{
							TArray<FTickPrerequisite>& Prerequisites = Component->PrimaryComponentTick.GetPrerequisites();
							for (int32 PrereqIndex = 0; PrereqIndex < Prerequisites.Num(); PrereqIndex++)
							{
								...
								if (PrereqObject == this && Prerequisite.Get() == ConstraintTickPtr)
								{
