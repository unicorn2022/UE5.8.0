...
static FAutoConsoleVariableRef CVarNewEvaluation(
	...
	FConsoleVariableDelegate::CreateLambda([](IConsoleVariable*)
	{
		UE_LOGF(LogTemp, Warning, "Constraints' new evaluation scheme %ls", bNewEvaluation ? TEXT("enabled.") : TEXT("disabled."));
...
{
	return EvaluateGlobalTransform(InSceneComponent, InSocketName);
}
	
FTransform EvaluateGlobalTransform(USceneComponent* InSceneComponent, const FName InSocketName)
{
	if (!InSceneComponent)
	{
...
FTransform GetGlobalTransform(const USceneComponent* InSceneComponent, const FName InSocketName)
{
	...
	const USkeletalMeshComponent* SkeletalMeshComponent = Cast<USkeletalMeshComponent>(InSceneComponent);	
	
	constexpr bool bValidForTransforms = true;
	FAnimationEvaluationCache& EvaluationCache = FAnimationEvaluationCache::Get();
	const FAnimationEvaluator& Evaluator = EvaluationCache.FindEvaluator(SkeletalMeshComponent);
	if (SkeletalMeshComponent && !Evaluator.IsValid(bValidForTransforms))
	{
		return InSceneComponent->GetSocketTransform(InSocketName);
	}
	
	const bool bShouldUseEvaluators = [&EvaluationCache, InSceneComponent]()
	{
		bool bUseEvaluators = false;
		
		const USceneComponent* Parent = InSceneComponent->GetAttachParent();
		FName AttachSocket = InSceneComponent->GetAttachSocketName();
		
		while (Parent)
		{
			const USkeletalMeshComponent* SkeletalMeshComponent = Cast<USkeletalMeshComponent>(Parent);
			if (SkeletalMeshComponent && AttachSocket != NAME_None)
			{
				constexpr bool bValidForTransforms = true;
				const FAnimationEvaluator& Evaluator = EvaluationCache.FindEvaluator(SkeletalMeshComponent);
				bUseEvaluators |= Evaluator.IsValid(bValidForTransforms);
				if (!bUseEvaluators)
				{
					return false;
				}
		...
		return bUseEvaluators;
	}();

	if (!bShouldUseEvaluators)
	{
		return InSceneComponent->GetSocketTransform(InSocketName);
	}

	FTransform Global = SkeletalMeshComponent && Evaluator.IsValid(bValidForTransforms) ?
		Evaluator.GetRelativeTransform(InSocketName) :
		InSceneComponent->GetSocketTransform(InSocketName, RTS_Component) * InSceneComponent->GetRelativeTransform();
	
	const USceneComponent* Parent = InSceneComponent->GetAttachParent();
	FName AttachSocket = InSceneComponent->GetAttachSocketName();
	while (Parent)
	{
		const USkeletalMeshComponent* ParentSkeletalMeshComponent = Cast<USkeletalMeshComponent>(Parent);

		bool bFromEvaluator = false;
		if (ParentSkeletalMeshComponent && AttachSocket != NAME_None)
		{
			const FAnimationEvaluator& ParentEvaluator = EvaluationCache.FindEvaluator(ParentSkeletalMeshComponent);
			if (!ParentEvaluator.IsValid(bValidForTransforms))
			{
				return InSceneComponent->GetSocketTransform(InSocketName);
			}
...
const UE::Anim::FAnimationEvaluator& EvaluateComponent(USceneComponent* InSceneComponent, const UE::Anim::FAnimationEvaluationTask& InTask)
{
	if (USkeletalMeshComponent* SkeletalMeshComponent = Cast<USkeletalMeshComponent>(InSceneComponent))
	{
