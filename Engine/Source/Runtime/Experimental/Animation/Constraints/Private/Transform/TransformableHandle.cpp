...
FTransform UTransformableComponentHandle::GetGlobalTransform() const
{
	...
	if (TransformableHandleUtils::SkipTicking())
	{
		...
		return TransformableHandleUtils::EvaluateGlobalTransform(SceneComponent, SocketName);
	}
	...
	const bool bLocal) const
{
	...
	const int32 NumTransforms = InLocalTransforms.Num();
	...
	for (int32 ChannelIndex = 0; ChannelIndex < 9; ChannelIndex++)
	{
		if (Channels.IsValidIndex(ChannelIndex))
		{
			...
			if (!Channel->GetDefault().IsSet() || (bUpdateDefault && Channel->GetNumKeys() == 0))
			{
...
FString UTransformableComponentHandle::GetLabel() const
{
	if (!Component.IsValid())
	{
		if (SocketName != NAME_None)
		{
			return FString::Printf(TEXT("(invalid-component)/%s"), *SocketName.ToString());
		}
