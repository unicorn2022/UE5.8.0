...
private:
	...
	/** Bind/Unbind this to the SkeletalMeshComponent's OnBoneTransformsFinalizedMC delegate. */
	void BindDelegates();
...
public:
	...
	/**
	 * Returns the evaluator related to InSkeletalMeshComponent if any or an invalid one otherwise.
	 * Note that calling this function won't update the evaluator so make sure to use FAnimationEvaluator::IsValid after calling that function.  
	 */
	const FAnimationEvaluator& FindEvaluator(const USkeletalMeshComponent* InSkeletalMeshComponent) const;
...
private:
	...
	TMap<USkeletalMeshComponent*, TUniquePtr<FAnimationEvaluator>> PerSkeletalMeshEvaluator;
