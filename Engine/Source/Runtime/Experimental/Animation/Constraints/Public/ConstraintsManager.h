...
public:
	UTickableConstraint() {}
	virtual ~UTickableConstraint() = default;
	...
	CONSTRAINTS_API FConstraintTickFunction& GetTickFunction(const UWorld* InWorld);
	CONSTRAINTS_API const FConstraintTickFunction& GetTickFunction(const UWorld* InWorld) const;

	/** Returns the tick function registrered in the world's current level if any (assuming both world and current leve are valid) */
	FConstraintTickFunction* FindTickFunction(const UWorld* InWorld) const;
...
protected:
	...
	mutable TMap<TWeakObjectPtr<ULevel>, TUniquePtr<FConstraintTickFunction>, TInlineSetAllocator<4>> ConstraintTicks;
};
