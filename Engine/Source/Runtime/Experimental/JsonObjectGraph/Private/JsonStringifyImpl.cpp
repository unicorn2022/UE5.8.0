...
struct FPackageReferenceFinder : public FArchiveUObject
{
		...
		, References(InReferences)
	{
		...
		const UObject* InstanceDataObject = Repository.FindInstanceDataObject(Obj);
		if (InstanceDataObject)
		{
			InstanceDataObject->GetClass()->SerializeBin(*this, (UObject*)InstanceDataObject);
		}
#endif//WITH_EDITORONLY_DATA
	}
	...
	TArray<const UObject*>& OutExports)
{
	...
	for (const UObject* Obj : DisallowList)
	{
		...
		GetObjectsWithOuter(Obj, DisallowedSubObjects, EGetObjectsFlags::IncludeNestedObjects, RF_Transient);
	...
	while (PendingRefs.Num())
	{
		...
		if (AllowedRoots.Contains(Iter) && Iter->IsA<UPackage>())
		{
			...
			GetObjectsWithOuter(Iter, PackageInners, EGetObjectsFlags::None, RF_Transient);
		...
		// The object graph is incomplete when using UE_INTERNAL_UNIVERSAL_PLACEHOLDERS
		// and so we won't be able to do full graph analysis:
#if !UE_INTERNAL_UNIVERSAL_PLACEHOLDERS
	...
	// Instead in UE_INTERNAL_UNIVERSAL_PLACEHOLDERS we treat all
	// roots as UPackage and gather all children recursively.
	// In this configuration we cannot reliably categorize objects as
	// editor only vs. not because that determination is made
	// by native code and uproject/uplugin structure - neither of 
	// which are loaded when using placeholders:
#if UE_INTERNAL_UNIVERSAL_PLACEHOLDERS
	checkf(!bFilterEditorOnly, TEXT("Cannot reliably filter editor only objects in this configuration"));
	for(const UObject* Root : OutRoots)
	{
		TArray<UObject*> ObjectInners;
		GetObjectsWithOuter(Root, ObjectInners, EGetObjectsFlags::IncludeNestedObjects, RF_Transient);
		OutExports.Append(ObjectInners);
	}
#endif
}
	...
	, RootPackage(nullptr)
{
	...
	// If the requested graph is simple we can store the root package and 
	// include said package's metadata in the result:
	if(Roots.Num() == 1 && Roots[0] && Roots[0]->IsA<UPackage>())
	{
		RootPackage = Cast<UPackage>(Roots[0]);
	}
...
#if WITH_EDITORONLY_DATA
	// attempt to write the IDO instead since it will have more info. if that fails, fallback to the regular object
	if (!TryWriteInstanceDataObjectProperties(Object))
#endif // WITH_EDITORONLY_DATA
	{ 
...
void FJsonStringifyImpl::WriteNativeObjectData()
{
	...
	// Native serializers may produce huge amounts of data that add a size and performance overhead.
	// They can be optionally disabled in scenarios where this data is less important.
	if (!(WriteOptions.Flags & EJsonStringifyFlags::DisableNativeSerializers))
	{
...
#if WITH_EDITORONLY_DATA
bool FJsonStringifyImpl::TryWriteInstanceDataObjectProperties(const UObject* ForObject)
{
#if !UE_INTERNAL_UNIVERSAL_PLACEHOLDERS
	// unless UNIVERSAL_PLACEHOLDERS is enabled, respect the impersonation flag.
	FUObjectSerializeContext* SerializeContext = FUObjectThreadContext::Get().GetSerializeContext();
	if (!SerializeContext || !SerializeContext->bImpersonateProperties)
	{
		return false;
	}
#endif
	...
	UE::FPropertyBagRepository& Repository = UE::FPropertyBagRepository::Get();
	const UObject* InstanceDataObject = Repository.FindInstanceDataObject(ForObject);
	if (!InstanceDataObject)
	{
		return false;
	}

	for (TFieldIterator<FProperty> FieldIt(InstanceDataObject->GetClass()); FieldIt; ++FieldIt)
	{
		WriteIdentifierAndValueToJson(InstanceDataObject, nullptr, *FieldIt);
	}
	return true;
}
#endif// WITH_EDITORONLY_DATA
...
void FJsonStringifyImpl::WriteIndirectlyReferencedContainedObjects(const UObject* ForObject)
{
	...
	ForEachObjectWithOuter(ForObject, 
		[this, &UnwrittenInners](UObject* ObjectToExport)
		{	
			if(ObjectsToExport.Contains(ObjectToExport) && !ObjectsExported.Contains(ObjectToExport))
			{
				UnwrittenInners.Add(ObjectToExport);
			}
		},
		EGetObjectsFlags::None, RF_Transient);
...
void FJsonStringifyImpl::WriteIdentifierAndValueToJson(const void* Container, const void* DefaultContainer, const FProperty* Property)
{
	...
	// FVCellProperty (as opposed to FVRestValueProperty) is only used by UVerseClass/UVerseStruct,
	// where it just points back to the VClass the UStruct was derived from. It can be safely ignored.
	if (const FVCellProperty* VerseProperty = CastField<FVCellProperty>(Property))
	{
		return;
	}
#endif
...
void FJsonStringifyImpl::WriteValueToJson(const void* Value, const void* DefaultValue, const FProperty* Property)
{
	...
	{
		const TObjectPtr<const UObject>* ObjectPtr = 
			static_cast<const TObjectPtr<const UObject>*>(Value);
...
{
	const FName ReferenceProperty{"ReferenceProperty"}; // this type just crashes if you try to export it
	if(Property->GetClass()->GetFName() == ReferenceProperty)
	{
		return;
	}
	...
	else if (const FByteProperty* ByteProperty = CastField<FByteProperty>(Property))
	{
		...
		{
			FString StringValue = EnumDef->GetValueOrBitfieldAsAuthoredNameString(ValueAsIntrinsic);
	...
	else if (const FEnumProperty* EnumProperty = CastField<FEnumProperty>(Property))
	{
		...
		FString StringValue = EnumDef->GetValueOrBitfieldAsAuthoredNameString(
	...
	else if( const FFloatProperty* AsFloatProperty = CastField<FFloatProperty>(Property))
	{
		float ValueAsIntrinsic = *(float*)Value;
		if(FMath::IsFinite(ValueAsIntrinsic))
		{
			Writer->WriteValueInline(ValueAsIntrinsic);
		}
		else
		{
			Writer->WriteValueInline(TEXT("inf"));
		}

	}
	else if( const FDoubleProperty* AsDoubleProperty = CastField<FDoubleProperty>(Property))
	{
		double ValueAsIntrinsic = *(double*)Value;
		if(FMath::IsFinite(ValueAsIntrinsic))
		{
			Writer->WriteValueInline(ValueAsIntrinsic);
		}
...
void FJsonStringifyImpl::WriteMapToJson(const void* MapInstance, const FMapProperty* MapProperty)
{
	...
	for (int32 Index = 0; MapNum; ++Index)
	{
		...
		{
			FPendingScope PropertyIdentifier(this, [Impl = this]()
				{
					Impl->Writer->WriteIdentifierPrefix(UE_JSON_TMAP_KEY_KEY_TCHAR);
				});
		...
		}
		{
			FPendingScope PropertyIdentifier(this, [Impl = this]()
				{
					Impl->Writer->WriteIdentifierPrefix(UE_JSON_TMAP_VALUE_KEY_TCHAR);
				});
...
void FJsonStringifyImpl::WriteOptionalToJson(const void* OptionalInstnace, const FOptionalProperty* OptionalProperty)
{
	...
	if (const void* ValueAddress = static_cast<const void*>(OptionalProperty->GetValuePointerForReadOrReplaceIfSet(OptionalInstnace)))
	{
		FPendingScope PropertyIdentifier(this, [Impl = this]()
			{
				Impl->Writer->WriteIdentifierPrefix(UE_JSON_OPTIONAL_VALUE_KEY_TCHAR);
			});
...
{
	FPendingScope PackageSummary(this, 
		[this]()
		{
...
void FJsonStringifyImpl::WriteCustomVersions()
{
#if WITH_TEXT_ARCHIVE_SUPPORT
	if (Versions.Num() == 0)
	{
...
void FJsonStringifyImpl::WritePackageMetadata()
{
#if WITH_METADATA
	if(!RootPackage)
	{
	...
	const FPackageFileVersion& Version = RootPackage->GetLinkerPackageVersion();
	if(!Version.IsCompatible( 
		FPackageFileVersion(VER_LATEST_ENGINE_UE4, EUnrealEngineObjectUE5Version::METADATA_SERIALIZATION_OFFSET)) )
	{
		return;
	}
	const FMetaData& MetaData = const_cast<UPackage*>(RootPackage)->GetMetaData();
	const bool bHasObjectMetaData = MetaData.ObjectMetaDataMap.Num() > 0;
	const bool bHasRootMetaData = MetaData.RootMetaDataMap.Num() > 0;
	const bool bHasAnyMetadata = bHasObjectMetaData || bHasRootMetaData;
	if(!bHasAnyMetadata)
	{
		return;
	}
	
	const auto WriteNameStringMap = [this](const TMap<FName, FString>& NameStringMap)
	{
		Writer->WriteArrayStartInline();
		for(const TPair<FName, FString>& Values : NameStringMap)
		{
			Writer->WriteObjectStart();
			Writer->WriteValue(TEXT("Key"), Values.Key.ToString());
			Writer->WriteValue(TEXT("Value"), Values.Value);
			Writer->WriteObjectEnd();
		}
		Writer->WriteArrayEnd();
	};
	...
	Writer->WriteIdentifierPrefix(TEXT("PackageMetadata"));
	Writer->WriteObjectStartInline();

	if(bHasObjectMetaData)
	{
		Writer->WriteIdentifierPrefix(TEXT("ObjectMetadata"));
		Writer->WriteArrayStartInline();
		for(const TPair<FSoftObjectPath, TMap<FName, FString>>& ObjectMetaData : MetaData.ObjectMetaDataMap)
		{
			Writer->WriteObjectStart();

			Writer->WriteValue(TEXT("Key"), ObjectMetaData.Key.ToString());
			
			Writer->WriteIdentifierPrefix(TEXT("Values"));
			WriteNameStringMap(ObjectMetaData.Value);
	...
	if(bHasRootMetaData)
	{
		Writer->WriteIdentifierPrefix(TEXT("RootMetadata"));
		WriteNameStringMap(MetaData.RootMetaDataMap);
	}
...
#if UE_INTERNAL_UNIVERSAL_PLACEHOLDERS
void FJsonStringifyImpl::WriteLinkerTables()
{
	if(!RootPackage)
	{
		return;
	...
	FLinkerLoad* Linker = FLinkerLoad::FindExistingLinkerForPackage(RootPackage);
	if(!Linker)
	{
		return;
	}
	...
	// The ordered list of FName strings baked into the package binary. In
	// placeholder mode, names that only appear in stream-serialised data are invisible 
	// in the exported object graph; the raw table makes them auditable.
	if(Linker->NameMap.Num() > 0)
	{
		Writer->WriteIdentifierPrefix(UE_JSON_LINKER_NAME_TABLE_KEY_TCHAR);
		Writer->WriteArrayStartInline();
		Writer->WriteLineTerminator();
		for(FNameEntryId EntryId : Linker->NameMap)
		{
			// FName::CreateFromDisplayId is the same conversion FLinkerLoad::operator<<(FName&)
			// uses when deserialising a name reference: pair the entry id with number 0 (the
			// per-reference number is stored separately in the binary, not in the table itself).
			Writer->WriteValueInline(FName::CreateFromDisplayId(EntryId, NAME_NO_NUMBER_INTERNAL).ToString());
		}
		Writer->WriteArrayEnd();
	}

	// Every external object this package depends on, encoded as "uobject:" path
	// strings using the same convention as live object references in the rest of
	// the JSON. In placeholder mode the object graph is incomplete, so we encode
	// all imports in the import table here.
	//
	// Path resolution: top-level imports (OuterIndex == null) store the full
	// package path as ObjectName (e.g. "/Script/Engine").  Sub-imports walk the
	// outer chain and join with '.' between a package and its direct children,
	// then ':' (SUBOBJECT_DELIMITER_CHAR) between a direct child and its immediate
	// subobjects, then '.' for all deeper levels — matching the convention of
	// UObjectBaseUtility::GetPathName
	// (e.g. "/Script/Engine.StaticMesh:SubObject.DeepSubObject").
	const TArray<FObjectImport>& ImportMap = Linker->ImportMap;
	if(ImportMap.Num() > 0)
	{
		// Returns the plain path (no prefix) for the import at Idx.
		// Mirrors UObjectBaseUtility::GetPathName: recurse into the outer first,
		// then append the separator and this object's name.
		const auto BuildPath = [&ImportMap](int32 Idx, auto& Self, FUtf8StringBuilderBase& Result) -> void
		{
			const FObjectImport& Import = ImportMap[Idx];
			if (!Import.OuterIndex.IsImport())
			{
				Import.ObjectName.AppendString(Result);
				return;
			}
			const int32 OuterIdx = Import.OuterIndex.ToImport();
			const FObjectImport& Outer = ImportMap[OuterIdx];
			Self(OuterIdx, Self, Result);
			// Use ':' when the outer is not a package but the outer's outer is,
			// matching the SUBOBJECT_DELIMITER_CHAR convention in GetPathName.
			const UTF8CHAR Sep = (Outer.OuterIndex.IsImport() && ImportMap[Outer.OuterIndex.ToImport()].OuterIndex.IsNull()) ? UTF8CHAR(':') : UTF8CHAR('.');
			Result << Sep;
			Import.ObjectName.AppendString(Result);
		};

		Writer->WriteIdentifierPrefix(UE_JSON_LINKER_IMPORT_TABLE_KEY_TCHAR);
		Writer->WriteArrayStartInline();
		Writer->WriteLineTerminator();
		for(int32 Idx = 0; Idx < ImportMap.Num(); ++Idx)
		{
			TUtf8StringBuilder<256> Ref;
			Ref << UE_JSON_OBJECT_REF_PREFIX;
			BuildPath(Idx, BuildPath, Ref);
			Writer->WriteValueInline(Ref.ToView());
		}
		Writer->WriteArrayEnd();
	}
}
