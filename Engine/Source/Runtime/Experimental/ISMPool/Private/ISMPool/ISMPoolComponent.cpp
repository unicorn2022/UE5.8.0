...
static void ISMPoolComponent_EnableTicksForPoolManagedComponents(IConsoleVariable*)
{
	//exclude CDOs & garbage
	constexpr EObjectFlags ExclusionFlags = RF_ClassDefaultObject;
	constexpr EInternalObjectFlags InternalExclusionFlags = EInternalObjectFlags::Garbage;
	
	constexpr bool bIncludeDerivedClasses = true;

	for (TObjectIterator<UISMPoolComponent> It(ExclusionFlags, bIncludeDerivedClasses, InternalExclusionFlags); It; ++It)
	{
		//non-pool managed components don't need their tick re-enabling
		It->EnableComponentTickIfPoolManagementEnabled();
	}
}
...
FAutoConsoleVariableRef CVarISMPoolComponentKeepAlive(
	...
	FConsoleVariableDelegate::CreateStatic(ISMPoolComponent_EnableTicksForPoolManagedComponents));
...
bool FISMPoolMeshGroup::BatchUpdateInstancesTransforms(FISMPool& ISMPool, FMeshId MeshId, int32 StartInstanceIndex, TArrayView<const FTransform> NewInstancesTransforms, bool bWorldSpace, bool bMarkRenderStateDirty, bool bTeleport)
{
	...
	UE_LOGF(LogChaos, Warning, "UISMPoolComponent : Invalid mesh Id (%d) for this mesh group", MeshId);
	return false;
}
...
void FISMPoolISM::InitISM(const FISMPoolStaticMeshInstance& InMeshInstance, bool bKeepAlive, bool bOverrideTransformUpdates)
{
	...
	ISMComponent->SetRenderCustomDepth(MeshInstance.Desc.Flags & FISMPoolComponentDescription::UsesCustomDepthStencil);
	ISMComponent->SetCustomDepthStencilValue(MeshInstance.Desc.CustomDepthStencilValue);
	...
	// Make sure this is called at the end so the full state is set, since SetStaticMesh implicitly calls PrecachePSO()
...
bool FISMPool::BatchUpdateInstancesTransforms(FISMPoolMeshInfo& MeshInfo, int32 StartInstanceIndex, TArrayView<const FTransform> NewInstancesTransforms, bool bWorldSpace, bool bMarkRenderStateDirty, bool bTeleport, bool bAllowPerInstanceRemoval)
{
	...
	{
		UE_LOGF(LogChaos, Warning, "UISMPoolComponent : Invalid ISM Id (%d) when updating the transform ", MeshInfo.ISMIndex);
		return false;
	}
...
bool FISMPool::RequiresTick() const
{
	return !PrellocationQueue.IsEmpty() || (FreeListISM.Num() > GComponentFreeListTargetSize);
}
...
void UISMPoolComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	Pool.Tick(this);
	SetComponentTickEnabled(Pool.RequiresTick());
}
...
UISMPoolComponent::FMeshId UISMPoolComponent::AddMeshToGroup(FMeshGroupId MeshGroupId, const FISMPoolStaticMeshInstance& MeshInstance, int32 InstanceCount, TArrayView<const float> CustomDataFloats)
{
	...
	UE_LOGF(LogChaos, Warning, "UISMPoolComponent : Trying to add a mesh to a mesh group (%d) that does not exists", MeshGroupId);
	return INDEX_NONE;
}
...
bool UISMPoolComponent::BatchUpdateInstancesTransforms(FMeshGroupId MeshGroupId, FMeshId MeshId, int32 StartInstanceIndex, TArrayView<const FTransform> NewInstancesTransforms, bool bWorldSpace, bool bMarkRenderStateDirty, bool bTeleport)
{
	...
	UE_LOGF(LogChaos, Warning, "%ls's ISMPoolComponent is trying to update instance with mesh group (%d) that not exists", GetOwner() ? *GetOwner()->GetName() : TEXT(""), MeshGroupId) ;
	return false;
}
...
{
	bIsPoolManagementEnabled = bEnablePoolManagement;
...
void UISMPoolComponent::EnableComponentTickIfPoolManagementEnabled()
{
	if (bIsPoolManagementEnabled)
	{
