...
void CreateNaniteData(FGeometryCollectionBuiltMeshData&& InMeshData, FGeometryCollectionRenderData::FCreateDesc const& InDesc, FGeometryCollectionRenderData& OutRenderData)
{
	...
	if (!NaniteBuilderModule.Build(
		*OutRenderData.NaniteResourcesPtr.Get(),
		...
		NaniteSettings))
	{
		UE_LOGF(LogStaticMesh, Error, "Failed to build Nanite for geometry collection. See previous line(s) for details.");
	}
}
...
#if RHI_RAYTRACING
		if (IsRayTracingAllowed() && Owner.bSupportRayTracing && MeshDescription.NumVertices > 0)
		{
			FName OwnerName = Owner.GetFName();

			ENQUEUE_RENDER_COMMAND(InitGeometryCollectiomRayTracingGeometry)(
				[this, OwnerName](FRHICommandListImmediate& RHICmdList)
				{
					// Create a single ray tracing geometry group to be used by parts

					check(RayTracingResource.GroupHandle.IsNull());
					RayTracingResource.GroupHandle = GRayTracingGeometryManager->RegisterRayTracingGeometryGroup(/*NumLODs*/ 0, /*CurrentFirstLODIdx*/ 0);

					// TODO: Combine sections using the same transform into a single BLAS

					RayTracingResource.PartGeometries.SetNum(MeshDescription.SubSections.Num());

					for (int32 SectionIndex = 0; SectionIndex < MeshDescription.SubSections.Num(); ++SectionIndex)
					{
						FRayTracingGeometryInitializer Initializer;
						Initializer.DebugName = FDebugName(OwnerName, SectionIndex);
						...
						{
							const FGeometryCollectionMeshElement& Section = MeshDescription.SubSections[SectionIndex];
							...
							Segment.MaxVertices = MeshResource.PositionVertexBuffer.GetNumVertices(); // TODO: investigate why Section.VertexEnd doesn't work here when using SubSections
						...
						RayTracingResource.PartGeometries[SectionIndex].GroupHandle = RayTracingResource.GroupHandle;
						RayTracingResource.PartGeometries[SectionIndex].LODIndex = FRayTracingGeometry::AdditionalGeometryLODIndex;
						RayTracingResource.PartGeometries[SectionIndex].SetInitializer(MoveTemp(Initializer));
						RayTracingResource.PartGeometries[SectionIndex].InitResource(RHICmdList);
					}
				}
				);
		}
...
void FGeometryCollectionRenderData::ReleaseResources()
{
	...
	if (bHasMeshData)
	{
		...
		ENQUEUE_RENDER_COMMAND(CmdReleaseRayTracingGeometryGroup)(
			...
			{
				for (FRayTracingGeometry& PartRayTracingGeometry : RayTracingResource.PartGeometries)
				{
					PartRayTracingGeometry.ReleaseResource();
				}
				RayTracingResource.PartGeometries.Empty();

				if (!RayTracingResource.GroupHandle.IsNull())
				{
					GRayTracingGeometryManager->ReleaseRayTracingGeometryGroup(RayTracingResource.GroupHandle);
					RayTracingResource.GroupHandle = {};
				}
			});
#endif
