...
{
	//UE_LOGF(UGeometryCollectionConversionLogging, Log, "FGeometryCollectionEngineConversion::AppendSkeletalMesh()");
#if WITH_EDITOR
	int LODIndex = 0;
...
void FGeometryCollectionEngineConversion::ConvertActorToGeometryCollection(const AActor* Actor, FManagedArrayCollection& OutCollection, TArray<TObjectPtr<UMaterialInterface>>& OutMaterialInstances, TArray<FGeometryCollectionAutoInstanceMesh>& OutInstancedMeshes, const FSkeletalMeshToCollectionConversionParameters& ConversionParameters, bool bSplitComponents)
{
	...
	if (UGeometryCollection* NewGeometryCollection = NewObject<UGeometryCollection>())
	{
		...
		{
			UE_LOGF(UGeometryCollectionConversionLogging, Log, "FGeometryCollectionEngineConversion::ConvertActorToGeometryCollection() - All bones were parented under new root.");
...
void FGeometryCollectionEngineConversion::ConvertCollectionToSkeleton(const FManagedArrayCollection& InCollection, USkeleton* OutSkeleton, TArray<int32>& OutIndexRemap, bool bCenterBonesOnGeometry)
{
	...
	{
		UE_LOGF(UGeometryCollectionConversionLogging, Error, "FGeometryCollectionEngineConversion::ConvertCollectionToSkeleton() - Bounds Facade not valid - check the schema of the collection");
		return;
	}
	...
	if (TransformFacade.IsValid() && TransformFacade.HasBoneNameAttribute() && OutSkeleton)
	{
		...
		TransformFacade.EnforceSingleRoot("root");
		UE_LOGF(UGeometryCollectionConversionLogging, Log, "FGeometryCollectionEngineConversion::ConvertCollectionToSkeleton(NumTransforms:%d)", OutSkeleton->GetReferenceSkeleton().GetRawBoneNum());
	}
}
...
void FGeometryCollectionEngineConversion::ConvertCollectionToSkeletalMesh(const FManagedArrayCollection& InCollection, const TArray<TObjectPtr<UMaterialInterface>> InMaterials, USkeletalMesh& OutSkeletalMesh, USkeleton& OutSkeleton)
{
	...
	{
		UE_LOGF(UGeometryCollectionConversionLogging, Error, "FGeometryCollectionEngineConversion::ConvertCollectionToSkeletalMesh() - Transform Facade not valid - check the schema of the collection");
		return;
	}
	...
	{
		UE_LOGF(UGeometryCollectionConversionLogging, Error, "FGeometryCollectionEngineConversion::ConvertCollectionToSkeletalMesh() - Mesh Facade not valid - check the schema of the collection");
		return;
	}
	...
	if (!BoundsFacade.IsValid())
	{
		UE_LOGF(UGeometryCollectionConversionLogging, Error, "FGeometryCollectionEngineConversion::ConvertCollectionToSkeletalMesh() - Bounds Facade not valid - check the schema of the collection");
		return;
	}
...
#else
	UE_LOGF(UGeometryCollectionConversionLogging, Error, "FGeometryCollectionEngineConversion::ConvertCollectionToSkeletalMesh() - conversion only supported in editor.");
#endif // WITH_EDITOR
}
