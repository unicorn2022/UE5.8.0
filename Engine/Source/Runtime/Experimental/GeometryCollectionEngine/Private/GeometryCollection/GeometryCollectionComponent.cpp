...
bool bChaos_GC_DestroyProxyOnSetSimulatePhysicsFalse = false;
FAutoConsoleVariableRef CVarChaosGCDestroyProxyOnSetSimulatePhysicsFalse(TEXT("p.Chaos.GC.DestroyProxyOnSetSimulatePhysicsFalse"), bChaos_GC_DestroyProxyOnSetSimulatePhysicsFalse, TEXT("When true, calling SetSimulatePhysics(false) on a Geometry Collection component destroys its physics proxy. Default false preserves legacy behavior of keeping the proxy alive."));
...
bool bUpdateProxyWorldTransformWhenSkippingPhysicsUpdate = true;
FAutoConsoleVariableRef CVarUpdateProxyWorldTransformWhenSkippingPhysicsUpdate(
	TEXT("p.Chaos.GC.UpdateProxyWorldTransformWhenSkippingPhysicsUpdate"), 
	bUpdateProxyWorldTransformWhenSkippingPhysicsUpdate, 
	TEXT("When true, bSkipPhysicsUpdate is true in OnUpdateTransform, the physics proxy will be requested to update it's world transform data internally - this fix issues when bUpdateComponentTransformToRootBone is enabled"));
...
FString RoleToString(ENetRole InRole)
{
	...
	case ROLE_SimulatedProxy:
		return FString("SimProxy");
	case ROLE_AutonomousProxy:
		return FString("AutoProxy");
	case ROLE_Authority:
		return FString("Auth");
	default:
		break;
...
FPrimitiveSceneProxy* UGeometryCollectionComponent::CreateSceneProxy()
{
	...
	{		
		...
		{
			UE_LOGF(LogGeometryCollectionComponent, Verbose, "Skipping CreateSceneProxy for UGeometryCollectionComponent %ls (UGeometryCollectionComponent PSOs are still compiling)", *GetFullName());
			return nullptr;
		}
...
void UGeometryCollectionComponent::SetNotifyTrailings(bool bNewNotifyTrailings)
{
	if (bNotifyTrailing != bNewNotifyTrailings)
	{
		if (PhysicsProxy)
		{
			PhysicsProxy->SetNotifyTrailings_External(bNewNotifyTrailings);
		}
		bNotifyTrailing = bNewNotifyTrailings;
		RegisterForEvents();
	}
}
...
void UGeometryCollectionComponent::SetNotifyGlobalTrailings(bool bNewNotifyGlobalTrailings)
{
	if (bNotifyGlobalTrailings != bNewNotifyGlobalTrailings)
	{
		if (PhysicsProxy)
		{
			PhysicsProxy->SetNotifyGlobalTrailings_External(bNewNotifyGlobalTrailings);
		}
		bNotifyGlobalTrailings = bNewNotifyGlobalTrailings;
	}
}
...
void UGeometryCollectionComponent::SetSimulatePhysics(bool bEnabled)
{
	...
	else if (!bEnabled && PhysicsProxy && bChaos_GC_DestroyProxyOnSetSimulatePhysicsFalse)
	{
		DestroyPhysicsState();
	}
}
...
bool UGeometryCollectionComponent::DoCustomNavigableGeometryExport(FNavigableGeometryExport& GeomExport) const
{
	...
	{
		UE_LOGF(LogGeometryCollectionComponent, Log,
			"Collection data is set to be stripped on cook so there is nothing left to export. Make sure to use bUseRootProxyForNavigation with bStripOnCook for %ls.", *GetFullNameSafe(GetOwner()));
	}
...
void UGeometryCollectionComponent::ProcessRepDataOnPT()
{
	...
	if (Chaos::FPhysicsSolver* CurrSolver = GetSolver(*this))
	{
		...
		// Capture a weak component pointer to make sure we can check for validity when executing the lambda 
		// This is needed to access VersionProcessed, LastHardsnapTimeInMs, OneOffActivatedProcessed in Read/Write
		// Note : those properties are only used in that function and in the reset function and both are executed on the Physics Thread

		TWeakObjectPtr<UGeometryCollectionComponent> WeakThis(this);
		CurrSolver->EnqueueCommandImmediate([WeakThis, PhysicsProxy = PhysicsProxy, RepDataCopy = RepData, bReplicateMovement](Chaos::FReal DeltaTime, Chaos::FReal SimTime)
		{
			if (TStrongObjectPtr<UGeometryCollectionComponent> StrongThis = WeakThis.Pin())
			{
				::ProcessRepDataCommon(DeltaTime, SimTime, RepDataCopy, PhysicsProxy, StrongThis->VersionProcessed, StrongThis->OneOffActivatedProcessed, StrongThis->LastHardsnapTimeInMs, bReplicateMovement);
			}
		});
	}
}
...
void UGeometryCollectionComponent::ProcessRepStateDataOnPT()
{
	...
	if (Chaos::FPhysicsSolver* CurrSolver = GetSolver(*this))
	{
		...
		// Capture a weak component pointer to make sure we can check for validity when executing the lambda 
		// This is needed to access VersionProcessed in Read/Write
		// Note : this property is only used in that function and in the reset function and both are executed on the Physics Thread

		TWeakObjectPtr<UGeometryCollectionComponent> WeakThis(this);
		CurrSolver->EnqueueCommandImmediate(
			[WeakThis, PhysicsProxy = PhysicsProxy, RepStateDataCopy = RepStateData]
			(Chaos::FReal /*DeltaTime*/, Chaos::FReal /*SimTime*/)
			{
				if (TStrongObjectPtr<UGeometryCollectionComponent> StrongThis = WeakThis.Pin())
				{
					::ProcessRepStateDataCommon(PhysicsProxy, RepStateDataCopy, StrongThis->VersionProcessed);
				}
			});
	}
}
...
void UGeometryCollectionComponent::ProcessRepDynamicDataOnPT()
{
	...
	if (Chaos::FPhysicsSolver* CurrSolver = GetSolver(*this))
	{
		...
		TWeakObjectPtr<UGeometryCollectionComponent> WeakThis(this);
		CurrSolver->EnqueueCommandImmediate(
			[WeakThis, PhysicsProxy = PhysicsProxy, RepDynamicDataCopy = RepDynamicData, bReplicateMovement]
			(Chaos::FReal DeltaTime, Chaos::FReal SimTime)
			{
				if (TStrongObjectPtr<UGeometryCollectionComponent> StrongThis = WeakThis.Pin())
				{
					::ProcessRepDynamicDataCommon(PhysicsProxy, RepDynamicDataCopy, StrongThis->DynamicRepDataVersionProcessed, bReplicateMovement, SimTime, DeltaTime, StrongThis->LastHardsnapTimeInMs);
				}
			});
	}
}
...
#if WITH_EDITOR
	// Make sure that custom renderer is updated _after_ any move has been applied to the full component hierarchy.
...
void UGeometryCollectionComponent::OnUpdateTransform(EUpdateTransformFlags UpdateTransformFlags, ETeleportType Teleport)
{
	...
	if (PhysicsProxy)
	{
		// Even when skipping the physics push, keep the proxy's cached world transforms in sync
		// with the actual component transform - the rebase that turns particle world XR into
		// component-local transforms depends on them being accurate.
		if (bSkipPhysicsUpdate)
		{
			if (bUpdateProxyWorldTransformWhenSkippingPhysicsUpdate)
			{
				PhysicsProxy->UpdateWorldTransformCaches_External(GetComponentTransform());
			}
		}
		else
		{
...
{
	//UE_LOGF(UGCC_LOG, Log, "GeometryCollectionComponent[%p]::TickComponent()", this);
...
{
	//UE_LOGF(UGCC_LOG, Log, "GeometryCollectionComponent[%p]::OnRegister()[%p]", this,RestCollection );
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
		...
		SimulationParameters.bGenerateGlobalTrailingData = bNotifyGlobalTrailings;
		...
		SimulationParameters.bGyroscopicTorqueEnabled = BodyInstance.bGyroscopicTorqueEnabled;
...
#endif
	PhysicsProxy = new FGeometryCollectionPhysicsProxy(this, DynamicCollection.ToSharedRef(), SimulationParameters, InitialShapeFilterData, InitialFilterInstanceData, CollectorGuid);
...
void UGeometryCollectionComponent::MoveComponentToRootTransform()
{
	if (RestCollection && PhysicsProxy)
	{
		...
		if (AssetCollection && DynamicCollection && DynamicCollection->IsDirty())
		{
			...
			// bUpdateComponentTransformToRootBone only supports a dynamically simulated root that is
			// not a cluster union member. Other configurations are out of scope for this feature.
			if (!DynamicStateFacade.IsDynamicOrSleeping(RootIndex) || DynamicStateFacade.HasClusterUnionParent(RootIndex))
			{
				return;
			}
...
void UGeometryCollectionComponent::LoadCollisionProfiles()
{
	...
	auto CreateOrGetCollisionProfileData = [this, &CachedData](const FName& ProfileName) -> const FCollisionProfileDataCache*
	{
		...
		{
			Cache.ShapeFilterData = InitialShapeFilterData;
			Cache.FilterInstanceData = InitialFilterInstanceData;
		...
		else if (UCollisionProfile::Get()->GetProfileTemplate(ProfileName, Template))
		{
			...
			FPhysicsFilterBuilder FilterBuilder;
			FilterBuilder.SetOwnerID(ActorID);
			FilterBuilder.SetComponentID(CompID);
			FilterBuilder.SetCollisionChannelIndex(Template.ObjectType);
			FilterBuilder.SetMaskFilter(BodyInstance.GetMaskFilter());
			FilterBuilder.SetResponses(Template.ResponseToChannels);
			FilterBuilder.SetFlags(Chaos::EFilterFlags::SimpleCollision | Chaos::EFilterFlags::ComplexCollision, true);
			FilterBuilder.SetFlags(Chaos::EFilterFlags::CCD, BodyInstance.bUseCCD);
			FilterBuilder.SetFlags(Chaos::EFilterFlags::ContactNotify, bNotifyCollisions || bNotifyGlobalCollisions);

			Cache.FilterInstanceData = FilterBuilder.BuildInstanceData();
			Cache.ShapeFilterData = FilterBuilder.BuildShapeFilterData();
	...
	for (int32 ParticleIndex = 0; ParticleIndex < PhysicsObjects.Num(); ++ParticleIndex)
	{
		if (PhysicsObjects[ParticleIndex] != nullptr)
		{
			...
			else // last resort use the body instance set collision settings
			{
				Data = CreateOrGetCollisionProfileData(DefaultCollisionProfileName);
			}
			...
			if (Data)
			{
				...
				Interface->UpdateCombinedShapeFilterData(ParticleView, Chaos::Filter::FCombinedShapeFilterData(ParticleData.ShapeFilterData, ParticleData.FilterInstanceData));
			}
		}
	}
...
{
	//UE_LOGF(UGCC_LOG, Log, "GeometryCollectionComponent[%p]::SendRenderDynamicData_Concurrent()", this);
...
void UGeometryCollectionComponent::SetCollisionObjectType(ECollisionChannel Channel)
{
	...
	if (PhysicsProxy)
	{
		PhysicsProxy->UpdateFilterData_External(Chaos::Filter::FCombinedShapeFilterData(InitialShapeFilterData, InitialFilterInstanceData));
	}
}
...
void UGeometryCollectionComponent::OnActorEnableCollisionChanged()
{
	...
	// Update Shape Filter and Instance data
	BuildInitialFilterData();
...
void UGeometryCollectionComponent::BuildInitialFilterData()
{
	...
	FPhysicsFilterBuilder Builder(FilterData.SimpleShapeFilterData, FilterData.FilterInstanceData);
	// Enable for complex and simple (no dual representation currently like other meshes)
	Builder.SetFlags(Chaos::EFilterFlags::SimpleCollision | Chaos::EFilterFlags::ComplexCollision, true);	
	// Should this actually clear the flag if the body is true but the geometry component is false?
	if (bNotifyCollisions || bNotifyGlobalCollisions)
	{
		Builder.SetFlags(Chaos::EFilterFlags::ContactNotify, true);
	}

	InitialFilterInstanceData = Builder.BuildInstanceData();
	InitialShapeFilterData = Builder.BuildShapeFilterData();
}
...
void UGeometryCollectionComponent::SetRestCollection(const UGeometryCollection* RestCollectionIn, bool bApplyAssetDefaults)
{
	...
	//UE_LOGF(UGCC_LOG, Log, "GeometryCollectionComponent[%p]::SetRestCollection()", this);
	if (RestCollectionIn)
	{
...
FPhysScene_Chaos* UGeometryCollectionComponent::GetInnerChaosScene() const
{
	...
	else
	{
		...
		UE_LOGF(LogPhysics, Error, "Failed to find valid UWorld in UGeometryCollectionComponent::GetInnerChaosScene");
		return nullptr;
	}
}
...
TArray<FTransform> UGeometryCollectionComponent::GetCurrentTransforms()
{
	TArray<FTransform> ReturnedTransforms;

	const TArray<FTransform3f>& AllTransforms = ComponentSpaceTransforms.RequestAllTransforms();
	ReturnedTransforms.Reserve(AllTransforms.Num());

	const TManagedArray<FTransform>* MassToLocal = nullptr;
	if (RestCollection && RestCollection->GetGeometryCollection())
	{
		MassToLocal = RestCollection->GetGeometryCollection()->FindAttribute<FTransform>("MassToLocal", FGeometryCollection::TransformGroup);
	}

	for (int32 TransformIndex = 0; TransformIndex < AllTransforms.Num(); ++TransformIndex)
	{
		const FTransform MassToLocalTransform = (MassToLocal && MassToLocal->IsValidIndex(TransformIndex)) ? (*MassToLocal)[TransformIndex] : FTransform::Identity;
		ReturnedTransforms.Emplace(MassToLocalTransform * FTransform(AllTransforms[TransformIndex]));
	}

	return ReturnedTransforms;
}

TArray<FTransform> UGeometryCollectionComponent::GetCurrentTransformsFromIndices(const TArray<int32>& TransformIndices)
{
	TArray<FTransform> ReturnedTransforms;
	ReturnedTransforms.Reserve(TransformIndices.Num());
	...
	{
		const FTransform MassToLocalTransform = (MassToLocal && MassToLocal->IsValidIndex(TransformIndex)) ? (*MassToLocal)[TransformIndex] : FTransform::Identity;
		const FTransform Transform = (AllTransforms.IsValidIndex(TransformIndex)) ? FTransform(AllTransforms[TransformIndex]) : FTransform::Identity;
		ReturnedTransforms.Emplace(MassToLocalTransform * Transform);
...
void UGeometryCollectionComponent::SetLinearDamping(float InDamping)
{
	Super::SetLinearDamping(InDamping);
	if (PhysicsProxy)
	{
		PhysicsProxy->SetLinearDamping_External(InDamping);
	}
}

void UGeometryCollectionComponent::SetLinearDampingPerTransform(const TArray<int32>& TransformIndices, float InDamping)
{
	if (PhysicsProxy)
	{
		PhysicsProxy->SetLinearDampingPerTransform_External(TransformIndices, InDamping);
	}
}

void UGeometryCollectionComponent::SetAngularDamping(float InDamping)
{
	Super::SetAngularDamping(InDamping);
	if (PhysicsProxy)
	{
		PhysicsProxy->SetAngularDamping_External(InDamping);
	}
}

void UGeometryCollectionComponent::SetAngularDampingPerTransform(const TArray<int32>& TransformIndices, float InDamping)
{
	if (PhysicsProxy)
	{
		PhysicsProxy->SetAngularDampingPerTransform_External(TransformIndices, InDamping);
	}
}
