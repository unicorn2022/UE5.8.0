...
void GeometryCollectionEngineUtility::PrintDetailedStatistics(const FGeometryCollection* GeometryCollection, const UGeometryCollectionCache* InCache)
{
	...
	GeometryCollectionAlgo::ComputeCoincidentVertices(GeometryCollection, 1e-2f, CoincidentVerticesMap, VertexToDeleteSet);
	int32 NumCoincidentVertices = VertexToDeleteSet.Num();

	TSet<int32> FaceToDeleteSet;
	GeometryCollectionAlgo::ComputeZeroAreaFaces(GeometryCollection, 1e-4f, FaceToDeleteSet);
	...
	UE_LOGF(LogGeoemtryCollectionClean, Log, "%ls", *Buffer);
}
...
#else
	UE_LOGF(LogGeoemtryCollectionClean, Fatal, "GeometryCollectionEngineUtility::ComputeNormals not supported in non-editor builds");
#endif
}
...
#else
	UE_LOGF(LogGeoemtryCollectionClean, Fatal, "GeometryCollectionEngineUtility::ComputeTangents not supported in non-editor builds");
#endif
}
