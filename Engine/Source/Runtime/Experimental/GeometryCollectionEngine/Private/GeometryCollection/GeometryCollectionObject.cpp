...
static constexpr float DefaultMaxSizeValue = 99999.9f;
...
void UGeometryCollection::GetSharedSimulationParams(FSharedSimulationParameters& OutParams) const
{
	...
	if (SizeSpecificDefault.CollisionShapes.Num())
	{
		...
		{
			UE_LOGF(LogGeometryCollectionInternal, Verbose, "LevelSet geometry selected but non-particle collisions selected. Forcing particle-implicit collisions for %ls", *GetPathName());
...
void UGeometryCollection::Serialize(FArchive& Ar)
{
	...
	if (Ar.CustomVer(FDestructionObjectVersion::GUID) >= FDestructionObjectVersion::GeometryCollectionInDDCAndAsset)
	{
		...
		if (Ar.IsLoading() || (Ar.IsCountingMemory() && !Ar.IsFilterEditorOnly()))
		{
			...
			// Legacy clean-up: previous versions persisted FVertexBoneWeightsFacade's
			// SkeletalMesh attribute (TManagedArray<TObjectPtr<UObject>>). It is Dataflow
			// editor-time scratch and the DDC simulation cooker's FMemoryWriter cannot
			// serialize FObjectPtr. Drop it before EnsureSimulationDataIsCooked runs below.
			if (Ar.IsLoading() && GeometryCollection)
			{
				const FName SkelMeshAttr = GeometryCollection::Facades::FVertexBoneWeightsFacade::SkeletalMeshAttributeName;
				if (GeometryCollection->HasAttribute(SkelMeshAttr, FGeometryCollection::GeometryGroup))
				{
					GeometryCollection->RemoveAttribute(SkelMeshAttr, FGeometryCollection::GeometryGroup);
				}
			}
		}
		else
...
void UGeometryCollection::FillAutoInstanceMeshesInstancesIfNeeded()
{
	...
	if (GeometryCollection && AutoInstanceMeshes.Num() > 0 && AutoInstanceMeshes[0].NumInstances == 0)
	{
		...
		{
			UE_LOGF(LogGeometryCollectionInternal, Warning, "[%ls] Could not find AutoInstanceMeshIndex attribute but the asset as instanced meshes assigned, you may need to regenerate this asset", *GetPathName());
		}
	}
}
...
TSharedPtr<FGeometryCollection, ESPMode::ThreadSafe> UGeometryCollection::GenerateMinimalGeometryCollection() const
{
	...
	static const FName AnchoredAttributeName("Anchored");
	...
	static const FName AutoInstanceMeshIndexAttributeName("AutoInstanceMeshIndex");
	...
	static const FName SizeAttributeName("Size");
...
#if RHI_RAYTRACING
	for (const FRayTracingGeometry& PartRayTracingGeometry : RenderData->RayTracingResource.PartGeometries)
	{
		// TODO: Have a dedicated size for ray tracing?
		InfoOut.MeshResourcesSize += PartRayTracingGeometry.RawData.GetAllocatedSize();
	}
#endif
...
void UGeometryCollection::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyChangedEvent.Property)
	{
		...
		else if (PropertyChangedEvent.Property->GetFName() == GET_MEMBER_NAME_CHECKED(UGeometryCollection, bSupportRayTracing))
		{
			bDoInvalidateCollection = true;
			bRebuildRenderData = true;
		}
...
void UGeometryCollection::EnsureDataIsCooked(bool bInitResources, bool bIsTransacting, bool bIsPersistent, bool bAllowCopyFromDDC)
{
	EnsureSimulationDataIsCooked(bIsTransacting, bAllowCopyFromDDC);
...
void UGeometryCollection::PreSave(FObjectPreSaveContext SaveContext)
{
	...
	constexpr bool bIsPersistent = false; // note that this has no effect on the call below  
	constexpr bool bAllowCopyFromDDC = false;
	EnsureDataIsCooked(bInitResources, bIsTransacting, bIsPersistent, bAllowCopyFromDDC);
#endif
...
#if WITH_EDITOR
		RenderDataGuid == ValidRenderDataGuid &&
#endif
		RenderData->bHasMeshData;
}
...
void UGeometryCollection::GetAssetRegistryTags(FAssetRegistryTagsContext Context) const
{
	// Dataflow specific tags
	UE::Dataflow::InstanceUtils::GetAssetRegistryTags(this, Context);

	// Geometry collection specific tags
	const int32 NumTransforms = GeometryCollection? GeometryCollection->Transform.Num(): 0;
	Context.AddTag(FAssetRegistryTag(TEXT("NumTransforms"), FString::FromInt(NumTransforms), FAssetRegistryTag::TT_Numerical));
