...
bool FGeometryCollectionMeshGroup::BatchUpdateInstancesTransforms(FGeometryCollectionISMPool& ISMPool, FMeshId MeshId, int32 StartInstanceIndex, TArrayView<const FTransform> NewInstancesTransforms, bool bWorldSpace, bool bMarkRenderStateDirty, bool bTeleport)
{
	...
	UE_LOGF(LogChaos, Warning, "UGeometryCollectionISMPoolComponent : Invalid mesh Id (%d) for this mesh group", MeshId);
	return false;
}
...
bool FGeometryCollectionISMPool::BatchUpdateInstancesTransforms(FGeometryCollectionMeshInfo& MeshInfo, int32 StartInstanceIndex, TArrayView<const FTransform> NewInstancesTransforms, bool bWorldSpace, bool bMarkRenderStateDirty, bool bTeleport, bool bAllowPerInstanceRemoval)
{
	...
	{
		UE_LOGF(LogChaos, Warning, "UGeometryCollectionISMPoolComponent : Invalid ISM Id (%d) when updating the transform ", MeshInfo.ISMIndex);
		return false;
	}
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
{
	...
	UE_LOGF(LogChaos, Warning, "UGeometryCollectionISMPoolComponent : Trying to add a mesh to a mesh group (%d) that does not exists", MeshGroupId);
	return INDEX_NONE;
}
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
{
	...
	UE_LOGF(LogChaos, Warning, "UGeometryCollectionISMPoolComponent : Trying to update instance with mesh group (%d) that not exists", MeshGroupId);
	return false;
}
