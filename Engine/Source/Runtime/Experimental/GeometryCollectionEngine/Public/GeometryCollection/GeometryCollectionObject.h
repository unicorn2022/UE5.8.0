...
struct FGeometryCollectionEmbeddedExemplar
{
	...
	FGeometryCollectionEmbeddedExemplar() 
		: StaticMeshExemplar(FString("None"))
...
public:
	...
	GEOMETRYCOLLECTIONENGINE_API void EnsureDataIsCooked(bool bInitResources, bool bIsTransacting, bool bIsPersistent, bool bAllowCopyFromDDC = true);
#endif
	...
	*/
	...
	/**
	 * If true, a ray tracing acceleration structure will be built for this geometry collection and it may be used in ray tracing effects
	 */
	UPROPERTY(EditAnywhere, Category = "Rendering")
...
#if WITH_EDITORONLY_DATA
	/** 
	 * Serialized version of RenderDataGuid. 
	 * This is used to determine when the stored RenderData mismatches the current GC object state.
	 * This can happen temporarily in editor during a undo/redo transaction when a GC component processing the PostUndo() before the GC object.
	 * todo: Remove this fix and move all relevent state to the RenderData so that it is the only source of truth for render scene proxies.
	 */
	UPROPERTY()
	FGuid ValidRenderDataGuid;
#endif
