...
USTRUCT()
struct UE_DEPRECATED(5.8, "This struct is no longer used.") FChaosVDCollisionFilterData
{
	GENERATED_BODY()
...
struct FChaosVDCollisionFilterDataV2
{
	bool operator==(const FChaosVDCollisionFilterDataV2& Other) const = default;

	CHAOSVDRUNTIME_API bool Serialize(FArchive& Ar);

	UE_INTERNAL CHAOSVDRUNTIME_API uint8 GetMaskFilter() const;
	UE_INTERNAL CHAOSVDRUNTIME_API uint32 GetFlags() const;
	UE_INTERNAL CHAOSVDRUNTIME_API void SetMaskFilterAndFlags(uint32 InMaskFilter, uint32 InFlags);

	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	UE_INTERNAL CHAOSVDRUNTIME_API void LoadLegacy(const FChaosVDCollisionFilterData& QueryData, const FChaosVDCollisionFilterData& SimData);
	PRAGMA_ENABLE_DEPRECATION_WARNINGS

	UE_INTERNAL CHAOSVDRUNTIME_API static void ExtractWord3(const uint32 Word3, uint8& OutMaskFilter, uint8& OutChannelIndex, uint32& OutFlags);

	UE_INTERNAL uint32 OwnerId = 0;
	UE_INTERNAL uint32 ComponentId = 0;
	UE_INTERNAL uint64 OverlapMask = 0;
	UE_INTERNAL uint64 BlockMask = 0;
	UE_INTERNAL uint64 CollisionChannelMask = 0;
	UE_INTERNAL uint32 MaskFilterAndFlags = 0;

	UE_INTERNAL bool bLoadedFromLegacy = false;
	UE_INTERNAL uint32 LegacySimBlockMask = 0;
	UE_INTERNAL uint32 LegacySimWord3 = 0;

private:
	UE_INTERNAL static constexpr uint32 NumFlagsBits = 21;
	UE_INTERNAL static constexpr uint32 NumCollisionChannelIndexBits = 5;
	UE_INTERNAL static constexpr uint32 NumMaskFilterBits = 6;

	PRAGMA_DISABLE_INTERNAL_WARNINGS
	UE_INTERNAL static constexpr uint32 CollisionChannelIndexOffset = NumFlagsBits;
	UE_INTERNAL static constexpr uint32 MaskFilterOffset = CollisionChannelIndexOffset + NumCollisionChannelIndexBits;

	UE_INTERNAL static constexpr uint32 FlagsMask = ((1 << NumFlagsBits) - 1);
	UE_INTERNAL static constexpr uint32 CollisionChannelIndexMask = ((1 << NumCollisionChannelIndexBits) - 1) << CollisionChannelIndexOffset;
	UE_INTERNAL static constexpr uint32 MaskFilterMask = ((1 << NumMaskFilterBits) - 1) << MaskFilterOffset;
	PRAGMA_ENABLE_INTERNAL_WARNINGS
};

CVD_IMPLEMENT_SERIALIZER(FChaosVDCollisionFilterDataV2)
...
struct FChaosVDShapeCollisionData
{
	...
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	FChaosVDShapeCollisionData() = default;
	FChaosVDShapeCollisionData(const FChaosVDShapeCollisionData&) = default;
	FChaosVDShapeCollisionData(FChaosVDShapeCollisionData&&) = default;
	FChaosVDShapeCollisionData& operator=(const FChaosVDShapeCollisionData&) = default;
	FChaosVDShapeCollisionData& operator=(FChaosVDShapeCollisionData&&) = default;
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	UE_DEPRECATED(5.8, "QueryData has been deprecated. Use FilterData instead.")
	UPROPERTY(meta = (DeprecatedProperty, DeprecationMessage = "QueryData has been deprecated"))
	FChaosVDCollisionFilterData QueryData_DEPRECATED;

	UE_DEPRECATED(5.8, "SimData has been deprecated. Use FilterData instead.")
	UPROPERTY(meta = (DeprecatedProperty, DeprecationMessage = "SimData has been deprecated"))
	FChaosVDCollisionFilterData SimData_DEPRECATED;
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	#endif // WITH_EDITORONLY_DATA

	FChaosVDCollisionFilterDataV2 FilterData;
...
struct FChaosVDCollisionChannelsInfoContainer
{
	...
	UE_INTERNAL static constexpr int32 MaxSupportedChannels = 64;

	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	FChaosVDCollisionChannelsInfoContainer() = default;
	FChaosVDCollisionChannelsInfoContainer(const FChaosVDCollisionChannelsInfoContainer&) = default;
	FChaosVDCollisionChannelsInfoContainer(FChaosVDCollisionChannelsInfoContainer&&) = default;
	FChaosVDCollisionChannelsInfoContainer& operator=(const FChaosVDCollisionChannelsInfoContainer&) = default;
	FChaosVDCollisionChannelsInfoContainer& operator=(FChaosVDCollisionChannelsInfoContainer&&) = default;
	PRAGMA_ENABLE_DEPRECATION_WARNINGS

	CHAOSVDRUNTIME_API static FStringView WrapperTypeName;

	#if WITH_EDITORONLY_DATA
	UE_DEPRECATED(5.8, "CustomChannelsNames has been deprecated")
	UPROPERTY(meta = (DeprecatedProperty, DeprecationMessage = "CustomChannelsNames has been deprecated"))
	FChaosVDCollisionChannelInfo CustomChannelsNames_DEPRECATED[32] = {};
	#endif // WITH_EDITORONLY_DATA

	UE_INTERNAL TArray<FChaosVDCollisionChannelInfo> CollisionChannelInfos;
