...
struct FChaosVDContext
{
	...
	FORCEINLINE void SetDataChannel(const TWeakPtr<Chaos::VisualDebugger::FChaosVDOptionalDataChannel>& NewDataChannel)
	{
		CurrentDataChannel = NewDataChannel;
	}

	FORCEINLINE bool IsDataChannelEnabled() const
	{
		if (TSharedPtr<Chaos::VisualDebugger::FChaosVDOptionalDataChannel> LockedCurrentDataChannel = CurrentDataChannel.Pin())
		{
			return LockedCurrentDataChannel->IsChannelEnabled();
		}
		else
		{
			return false;
...
private:
	TWeakPtr<Chaos::VisualDebugger::FChaosVDOptionalDataChannel> CurrentDataChannel = CVDDC_Default;
};
...
public:
	...
	CHAOS_API virtual ~FChaosVDThreadContext() override;
...
protected:
	...
	CHAOS_API TArray<uint8>& GetTLSDataBufferRef();
...
{
	FChaosCVDScopedDataChannelOverride(const TWeakPtr<Chaos::VisualDebugger::FChaosVDOptionalDataChannel>& NewDataChannel)
	{
