...
namespace Chaos::VisualDebugger
{
	...
	struct FChaosVDOptionalDataChannel : TSharedFromThis<FChaosVDOptionalDataChannel>
	{
		...
		virtual ~FChaosVDOptionalDataChannel();
	...
	public:
		...
		UE_DEPRECATED(5.9, "please use EnumerateDataChannels instead")
		void EnumerateChannels(TCallback Callback)
		{
			for (const TPair<FName, TWeakPtr<FChaosVDOptionalDataChannel>>& ChannelWithName : AvailableChannels)
			{
				if (TSharedPtr<FChaosVDOptionalDataChannel> LockedChannel = ChannelWithName.Value.Pin())
				{
					if (!Callback(LockedChannel))
					{
						return;
					}
				}
		 ...
		 * If the callback returns false, the iteration will be stopped.
		 */
		void EnumerateDataChannels(TFunction<bool(const TWeakPtr<FChaosVDOptionalDataChannel>&)> Callback)
		{
			for (const TPair<FName, TWeakPtr<FChaosVDOptionalDataChannel>>& ChannelWithName : AvailableChannels)
			{
				if (!Callback(ChannelWithName.Value))
				{
					return;
				}
			}
		...
		TWeakPtr<FChaosVDOptionalDataChannel> GetChannelById(FName ChannelId)
		{
			if (TWeakPtr<FChaosVDOptionalDataChannel>* ChannelFound = AvailableChannels.Find(ChannelId))
			{
				return *ChannelFound;
			}
	...
	private:
		...
		void RegisterChannel(const TWeakPtr<FChaosVDOptionalDataChannel>& NewChannel)
		{
			if (TSharedPtr<FChaosVDOptionalDataChannel> LockedNewChannel = NewChannel.Pin())
			{
				AvailableChannels.Add(LockedNewChannel->GetId(), NewChannel);
			}
		}
		
		/** Remove a CVD Data channel instance - Do not call directly */
		void UnregisterChannel(FName ChannelId)
		{
			AvailableChannels.Remove(ChannelId);
		}

		TMap<FName, TWeakPtr<FChaosVDOptionalDataChannel>> AvailableChannels;
	...
	UE_DEPRECATED(5.9, "please use CreateDataChannelShared instead to keep the lifetime of the channel explicit");
	...
	/** Creates a CVD Data Channel Instance - Only to be used by the CVD Macros */
	CHAOS_API TSharedPtr<FChaosVDOptionalDataChannel> CreateDataChannelShared(FName InChannelID, const TSharedRef<FText>& InDisplayName, EChaosVDDataChannelInitializationFlags InitializationFlags);
}
...
/** Declares an Optional CVD Data channel to be available globally. The data channel can be accessed by using CVDDC_TheNameOfTheChannelUsedWithThisMacro */
	...
	#define CVD_DECLARE_OPTIONAL_DATA_CHANNEL(DataChannelName) \
			extern CHAOS_API TSharedPtr<Chaos::VisualDebugger::FChaosVDOptionalDataChannel> CVDDC_##DataChannelName;
#endif
...
/** Declares an Optional CVD Data channel outside the Chaos Module to be available globally. The data channel can be accessed by using CVDDC_TheNameOfTheChannelUsedWithThisMacro */
	...
	#define CVD_DECLARE_OPTIONAL_DATA_CHANNEL_EXTERN(DataChannelName, ModuleAPI) \
		extern ModuleAPI TSharedPtr<Chaos::VisualDebugger::FChaosVDOptionalDataChannel> CVDDC_##DataChannelName;
#endif
...
/** Defines and initializes an Optional CVD Data Channel */
	...
	#define CVD_DEFINE_OPTIONAL_DATA_CHANNEL(DataChannelName, InitializationFlags) \
			TSharedPtr<Chaos::VisualDebugger::FChaosVDOptionalDataChannel> CVDDC_##DataChannelName = Chaos::VisualDebugger::CreateDataChannel(#DataChannelName, MakeShared<FText>(NSLOCTEXT(CVD_STRINGIZE(ChaosVisualDebugger), CVD_STRINGIZE(CVD_CONCAT(DataChannelName,_ChannelName)), #DataChannelName)), InitializationFlags);
#endif
...
CVD_DECLARE_OPTIONAL_DATA_CHANNEL(CameraPaths);

// Channel for logging more detailed steps related to resim
CVD_DECLARE_OPTIONAL_DATA_CHANNEL(ResimDetails)
