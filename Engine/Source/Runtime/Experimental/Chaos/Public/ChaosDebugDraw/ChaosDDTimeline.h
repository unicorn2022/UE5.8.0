...
namespace ChaosDD::Private
{
	...
	public:
		...
		void BeginFrame(double InTime, double InDt)
		{
			...
			//UE_LOGF(LogChaosDD, VeryVerbose, "BeginFrame %ls %lld", *GetName(), ActiveFrame->GetFrameIndex());
		}
		...
		void EndFrame()
		{
			...
			//UE_LOGF(LogChaosDD, VeryVerbose, "EndFrame %ls %lld (%d Commands)", *GetName(), ActiveFrame->GetFrameIndex(), ActiveFrame->GetNumCommands());
