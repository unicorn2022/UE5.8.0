		...
		return Read([](auto* Particle)
			{
				...
				return EResimType::FrozenDuringResim;
			});
	}
...
public:
	...
	void SetShapeSimData(int32 InShapeIndex, const FCollisionFilterData& SimData)
	{
		...
		VerifyContext();
		GetParticle_LowLevel()->SetShapeSimData(InShapeIndex, SimData);
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
	}
	CHAOS_API void SetShapeFilterData(int32 InShapeIndex, const Filter::FShapeFilterData& FilterData);
	CHAOS_API void SetShapeFilterInstanceData(int32 InShapeIndex, const Filter::FInstanceData& InstanceData);
	...
	void MarkNonFrequentDataDirty()
	{
		VerifyContext();

		FGeometryParticle* GTParticle = GetParticle_LowLevel();
		if (GTParticle)
		{
			// Mark NonFrequentData as dirty.
			GTParticle->ForceDirty(EChaosPropertyFlags::NonFrequentData);

			// Manually add proxy to solver's dirty list (replicating what MarkDirty does)
			if (IPhysicsProxyBase* Proxy = GetProxy())
			{
				if (FPhysicsSolverBase* SolverBase = Proxy->GetSolverBase())
				{
					SolverBase->AddDirtyProxy(Proxy);
				}
			}
		}
	}
