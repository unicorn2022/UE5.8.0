...
public:
	...
	struct FConvexDecompositionSettings
	{
		...
		/** Directly-specified empty space to use *in addition* to any that would be added via bProtectNegativeSpace. If provided, this covering will be used even if bProtectNegativeSpace is false. */
		const UE::Geometry::FSphereCovering* CustomEmptySpace = nullptr;

		/** Transform to apply to the custom empty space sphere covering, relative to the top-level/world space of the geometry collection. */
		FTransform CustomEmptySpaceWorldTransform = FTransform::Identity;
	};
