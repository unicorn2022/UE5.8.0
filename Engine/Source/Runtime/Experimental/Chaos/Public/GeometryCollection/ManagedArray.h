...
public:
	...
	virtual ~FManagedArrayBase() = default;
...
public:
	...
	*
	*/
	virtual ~TManagedArrayBase() = default;
...
void InitHelper(TArray<T>& Array, const TManagedArrayBase<T>& NewTypedArray, int32 Size)
{
	check(Size <= Array.Num() && Size <= NewTypedArray.Num());
	CopyAssignItems(Array.GetData(), NewTypedArray.GetData(), Size);
}
...
void CopyRangeHelper(TArray<T>& Target, const TManagedArrayBase<T>& Source, int32 Start, int32 Stop, int32 Offset)
{
	const int32 NumCopy = FMath::Min(FMath::Min(Source.Num() - Start, Stop - Start), Target.Num() - Start - Offset);
	if (NumCopy > 0)
	{
		check(Start >= 0 && Start + Offset >= 0);
		CopyAssignItems(Target.GetData() + Start + Offset, Source.GetData() + Start, NumCopy);
	}
}
...
public:
	...
	virtual ~FManagedBitArrayBase() = default;
...
template<> inline void InitHelper(TArray<TYPE>& Array, const TManagedArrayBase<TYPE>& NewTypedArray, int32 Size) { \
	UE_LOGF(LogChaos,Warning, "Cannot make a copy of unique array of type (%ls) within the managed array collection. Regenerate unique pointer attributes if needed.", NAME); }\
template<> inline void CopyRangeHelper(TArray<TYPE>& Target, const TManagedArrayBase<TYPE>& Source, int32 Start, int32 Stop, int32 Offset) {\
	UE_LOGF(LogChaos, Warning, "Cannot make a range copy of unique array of type (%ls) within the managed array collection. Regenerate unique pointer attributes if needed.", NAME); \
}
...
public:
	...
	{
		UE_LOGF(LogChaos, VeryVerbose, "TManagedArray<int32>[%p]::Reindex()",this);
...
public:
	...
	virtual void Reindex(const TArray<int32> & Offsets, const int32 & FinalSize, const TArray<int32> & SortedDeletionList, const TSet<int32>& DeletionSet) override
	{
		UE_LOGF(LogChaos, VeryVerbose, "TManagedArray<TArray<int32>>[%p]::Reindex()", this);
...
public:
	...
	virtual void Reindex(const TArray<int32> & Offsets, const int32 & FinalSize, const TArray<int32> & SortedDeletionList, const TSet<int32>& DeletionSet) override
	{
		UE_LOGF(LogChaos, VeryVerbose, "TManagedArray<FIntVector>[%p]::Reindex()", this);
		int32 ArraySize = Num(), MaskSize = Offsets.Num();
		for (int32 Index = 0; Index < ArraySize; Index++)
		{
