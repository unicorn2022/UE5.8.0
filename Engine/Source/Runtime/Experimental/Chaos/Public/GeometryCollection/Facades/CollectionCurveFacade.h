...
namespace GeometryCollection::Facades
{
	...
	public:
		...
		/** Get the point positions of a specific curve */
		CHAOS_API void GetCurvePointPositions(const int32 CurveIndex, TArray<FVector3f>& OutPoints) const;

		/** 
		* Get the resampled positions of a specific curve 
		* the curve will be sampled at regular interval along the curve, to match the NumSamples parameters
		*/
		CHAOS_API void GetCurveResampledPositions(const int32 CurveIndex, int32 NumSamples, TArray<FVector3f>& OutPoints) const;
	...
	class FCollectionCurveAttributesFacade
	{
	public:

		/** Collection attribute names */
		static CHAOS_API const FName CurveCardGroupsAttribute;

		CHAOS_API FCollectionCurveAttributesFacade(FManagedArrayCollection& InCollection);
		CHAOS_API FCollectionCurveAttributesFacade(const FManagedArrayCollection& InCollection);
		...
		/** Get the number of curves */
		int32 GetNumCurves() const { return CurveCardGroups.Num(); }
		
		/** Get the curve card groups */
		const TArray<FName>& GetCurveCardGroups() const { return CurveCardGroups.Get().GetConstArray(); }

		/** Set the curve card groups */
		void SetCurveCardGroups(const TArray<FName>& InCurveCardGroups) { CurveCardGroups.Modify() = InCurveCardGroups; }
	...
	protected :
		...
		/** Curve card groups */
		TManagedArrayAccessor<FName> CurveCardGroups;
	};
}
