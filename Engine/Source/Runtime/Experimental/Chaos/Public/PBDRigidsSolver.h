...
DECLARE_MULTICAST_DELEGATE_TwoParams(FSolverParticleUnregisteredGT, Chaos::FSingleParticlePhysicsProxy* /*Proxy*/, Chaos::FUniqueIdx /*UniqueIdx*/);
...
namespace Chaos::Private
{
	...
	UE_INTERNAL CHAOS_API bool IsUsingSingleThreadPullData();
}
/**
*
*/
namespace Chaos
{
	...
	*/
	...
	public:
		...
		/** Fired on the game thread when a single particle physics proxy is unregistered.
		    Called during UnregisterObject before the proxy is enqueued for destruction. */
		UE_INTERNAL CHAOS_API FDelegateHandle AddParticleUnregisteredGTCallback(FSolverParticleUnregisteredGT::FDelegate InDelegate);
		UE_INTERNAL CHAOS_API bool            RemoveParticleUnregisteredGTCallback(FDelegateHandle InHandle);
		...
		*/
		...
		CHAOS_API void SetSuspensionTarget(Chaos::FSuspensionConstraint* GTConstraint, const FVector& TargetPos, const FVector& Normal, bool Enabled, const IPhysicsProxyBase* GroundProxy = nullptr);
	...
	private:
		...
		FSolverParticleUnregisteredGT EventParticleUnregisteredGT;
		...
		struct FPendingDestroyInfo
		{
			...
			const bool operator==(const FPendingDestroyInfo Other) const
			{
		...
		struct FPendingProxyDestroyInfo
		{
			...
			const bool operator==(const FPendingProxyDestroyInfo Other) const
			{
				return Proxy == Other.Proxy;
			}
		};
		...
		/** Process callback lifecycle (add/remove) and SimCommands from a skipped frame's PushData.
		 *  Skips dirty proxy processing and input assignment. Used during adaptive resim coalescing.
		 *  Caller must set MTime to the correct value for the skipped frame before calling. */
		void ProcessSkippedFramePushData_Internal(FPushPhysicsData& PushData, int32 SkippedFrame);
