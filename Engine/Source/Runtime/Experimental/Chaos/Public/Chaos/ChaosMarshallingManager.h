...
public:
	...
	/** Debug and print the history buffer of the latest NumFrames. */
	CHAOS_API void DebugHistory_Internal(int32 NumFrames);
...
private:
	...
	TPhysicsSpscQueue<FPullPhysicsData*> PullDataQueue;	//the results the simulation has written to. Consumed by external thread
	TPhysicsSpscQueue<FPullPhysicsData*> PullDataPool;	//the pull data pool to avoid reallocs. Pushed by external thread, popped by internal
