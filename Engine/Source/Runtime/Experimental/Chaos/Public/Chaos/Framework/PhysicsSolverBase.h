...
namespace Chaos
{
	...
	/**
	 * Task responsible for rewinding the solver if required by networked physics
	 */
	class FPhysicsSolverRewindTask
	{
	public:

		FPhysicsSolverRewindTask(FPhysicsSolverBase& InSolver, FPushPhysicsData* InPushData) : Solver(InSolver), PushData(InPushData) {}

		TStatId GetStatId() const { RETURN_QUICK_DECLARE_CYCLE_STAT(FPhysicsSolverRewindTask, STATGROUP_TaskGraphTasks); }
		static CHAOS_API ENamedThreads::Type GetDesiredThread();
		static ESubsequentsMode::Type GetSubsequentsMode() { return ESubsequentsMode::TrackSubsequents; }
		void DoTask(ENamedThreads::Type CurrentThread, const FGraphEventRef& MyCompletionGraphEvent) { RewindSolver(); }
		void RewindSolver();
	...
	public:
		...
		FPhysicsSolverRewindTask RewindTask;
	...
	public:
		...
		FSolverTasksPTOnly(FPhysicsSolverBase& InSolver, FPushPhysicsData* InPushData)
			...
			, RewindTask(InSolver, InPushData)
			, Solver(InSolver)
		{}
	...
	public:
		...
		inline TSimCallbackObjectType* CreateAndRegisterSimCallbackObject_External(ConstructorArgTypes&&... ConstructorArgs)
		{
			...
			NewCallbackObject->PostInitialize_External();
		...
		{
			SimCallbackObject->PreShutdown_External();
			MarshallingManager.UnregisterSimCallbackObject_External(SimCallbackObject);

			// Unregister external callback
			for (int32 Idx = InjectInputsWatchers_External.Num() - 1; Idx >= 0; --Idx)
			{
				if (InjectInputsWatchers_External[Idx]->bPendingDelete_External)
				{
					InjectInputsWatchers_External.RemoveAtSwap(Idx, EAllowShrinking::No);
				}
			}
		}
		...
		* The command will be reapplied during physics resimulation on the same step */
		...
		const FRewindData* GetRewindData() const
		{
			return MRewindData.Get();
		}
		...
		const IRewindCallback* GetRewindCallback() const
		{
			return MRewindCallback.Get();
		}
		...
		* and particles that are in sync doesn't need collision data since we just step through their states in history." */
		...
		const IPhysicsReplicationAsync* GetPhysicsReplication_Internal() const
		{
			return PhysicsReplicationAsync;
		}
		...
		void SetAsyncDeltaTime(FReal InDt) { AsyncDt = InDt; }
		
		CHAOS_API void DisableAsyncMode();
		
		virtual void AddResimulationRequest_Internal(FPushPhysicsData& PushData);
		...
		*/
		...
		CHAOS_API static bool CanDebugNetworkPhysicsPrediction();
		...
		CHAOS_API static float NetworkPhysicsInterpolationLerp();
	...
	protected:
		...
		TArray<ISimCallbackObject*> InjectInputsWatchers_External;
		TArray<ISimCallbackObject*> ProcessInputsWatchers;
		...
		IPhysicsReplicationAsync* PhysicsReplicationAsync = nullptr;
		IPhysicsReplicationLODAsync* PhysicsReplicationLODAsync = nullptr;
	...
	private:
		...
		void RegisterSimCallbackObject_External(ISimCallbackObject* SimCallbackObject)
		{
			...
			if (SimCallbackObject->HasOption(ESimCallbackOptions::InjectInputsExternal))
			{
				InjectInputsWatchers_External.Add(SimCallbackObject);
			}
		...
		friend FPhysicsSolverRewindTask;
