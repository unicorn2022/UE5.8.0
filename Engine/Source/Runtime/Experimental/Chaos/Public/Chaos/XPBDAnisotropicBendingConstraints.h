...
public:
	...
	virtual ~FXPBDAnisotropicBendingConstraints() override = default;
	...
	CHAOS_API void AddAnisotropicBendingResidualAndHessian(const FSolverParticlesRange& Particles, const int32 ConstraintIndex, const int32 ConstraintIndexLocal, const FSolverReal Dt, TVec3<FSolverReal>& ParticleResidual, Chaos::PMatrix<FSolverReal, 3, 3>& ParticleHessian);
	...
	TArray<TArray<int32>> GetConstraintsArray() const
	{
		...
		{
			ConstraintsArray[ConstraintIdx].SetNum(4);
			for (int32 LocalIdx = 0; LocalIdx < 4; LocalIdx++)
			{
				ConstraintsArray[ConstraintIdx][LocalIdx] = Constraints[ConstraintIdx][LocalIdx];
			}
		}
