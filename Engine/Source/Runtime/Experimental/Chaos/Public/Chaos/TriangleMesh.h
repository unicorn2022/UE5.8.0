...
namespace Chaos
{
	...
	public:
		...
		*/
		...
		template<typename T>
		CHAOS_API bool PointClosestTriangleQuery(const TSpatialHashType<T>& SpatialHash, TFunctionRef<const TVec3<T>&(int32 PointIndex)> GetPosition, const int32 PointIndex, const TVec3<T>& PointPosition, const T PointThickness, const T ThisThickness,
			TFunctionRef<bool(const int32 PointIndex, const int32 TriangleIndex)> BroadphaseTest, TArray<TTriangleCollisionPoint<T>>& Result) const;
