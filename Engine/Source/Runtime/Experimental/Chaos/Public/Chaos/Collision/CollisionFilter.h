...
namespace Chaos
{
	...
	*/
	UE_DEPRECATED(5.8, "Use FShapeFilterData.IsValid instead")
	...
	*/
	...
	{
		return (!Shape || (Shape->GetSimEnabled() && Shape->GetShapeFilterData().IsSimValid()));
	}
	...
	*/
	inline bool DoCollide(EImplicitObjectType Implicit0Type, const FPerShapeData* Shape0, EImplicitObjectType Implicit1Type, const FPerShapeData* Shape1)
	{
		...
		if (Shape0 && Shape1)
		{
			const Filter::FShapeFilterData& ShapeFilter0 = Shape0->GetShapeFilterData();
			const Filter::FShapeFilterData& ShapeFilter1 = Shape1->GetShapeFilterData();
			const Filter::ENarrowFilterResult HitType = ShapeFilter0.NarrowFilter(ShapeFilter1);
