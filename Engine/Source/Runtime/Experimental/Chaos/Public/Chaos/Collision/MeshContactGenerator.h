...
namespace Chaos::Private
{
	...
	*/
	...
	private:
		...
		// Info about a vertex contact that allows us to prevent adding duplicates from all the triangles that 
		// share the vertex. Contains a contact index combined with a flag to indicate if the normal is roughly 
		// along the triangle face
		struct FVertexContactData
		{
			FVertexContactData()
			{
			}
			
			FVertexContactData(const FContactVertexID& InID, const int32 InContactIndex, const bool bInIsFaceContact)
				: ID(InID)
		...
		// Info about an edge contact that allows us to prevent adding duplicate edge contacts from the two
		...
		struct FEdgeContactData
		{
			...
			FEdgeContactData(const FContactEdgeID& InID, const int32 InLocalTriangleIndex)
			{
				Init(InID, InLocalTriangleIndex);
			}

			void Init(const FContactEdgeID& InID, const int32 InLocalTriangleIndex)
			{
				ID = InID;
				LocalTriangleIndex = InLocalTriangleIndex;
				ContactIndices[0] = INDEX_NONE;
				ContactIndices[1] = INDEX_NONE;
				ContactNormalDotTriangleNormal = 0.0f;
				bIsFaceContact = false;
			}

			void AddContact(int32 InContactIndex, const FRealSingle InContactNormalDotTriangleNormal, const bool bInIsFaceContact)
			{
				if (ContactIndices[0] == INDEX_NONE)
				{
					ContactIndices[0] = InContactIndex;
				}
				else
				{
					ContactIndices[1] = InContactIndex;
				}
				ContactNormalDotTriangleNormal = FMath::Max(ContactNormalDotTriangleNormal, InContactNormalDotTriangleNormal);
				bIsFaceContact = bIsFaceContact || bInIsFaceContact;
			}

			FContactEdgeID ID;
			int32 LocalTriangleIndex;
			int32 ContactIndices[2];
		...
		{
			if (const FVertexContactData* ContactData = VertexContactDataMap.Find(VertexID))
			{
				return ContactData->bIsFaceContact;
			}
			return false;
		}
	...
	private:
		...
		// A map of VertexID to contact data for that vertex - we only ever keep one contact per vertex
		struct FVertexContactIndicesMapTraits
		{
			static uint32 GetIDHash(const FContactVertexID& VertexID) { return MurmurFinalize32(VertexID); }
			static FContactVertexID GetElementID(const FVertexContactData& VertexContactData) { return VertexContactData.ID; }
		};
		THashMappedArray<FContactVertexID, FVertexContactData, FVertexContactIndicesMapTraits> VertexContactDataMap;

		// A map of EdgeID to contact data for that edge - we only ever keep contacts from one triangle sharing the edge
		struct FEdgeContactIndicesMapTraits
		{
			static uint32 GetIDHash(const FContactEdgeID& EdgeID) { return uint32(MurmurFinalize64(EdgeID.EdgeID)); }
			static FContactEdgeID GetElementID(const FEdgeContactData& EdgeContactData) { return EdgeContactData.ID; }
		};
		THashMappedArray<FContactEdgeID, FEdgeContactData, FEdgeContactIndicesMapTraits> EdgeContactDataMap;
	};
