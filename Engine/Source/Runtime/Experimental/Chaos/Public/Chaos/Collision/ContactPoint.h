	...
	public:
		union FFlags
		{
			...
			struct
			{
				...
				uint8 bDisablePositionSolve : 1;			// Whether we should disable position solve on this manifold point
				uint8 bDisableVelocitySolve : 1;			// Whether we should disable velocity solve on this manifold point
				uint8 bDisableFriction : 1;					// Whether we should disable friction solve on this manifold point
			};
			uint8 Bits;
		};

		// Verify that all FFlags data can be initialized to zero (even unused bits)
		static_assert(sizeof(FFlags) == sizeof(FFlags::Bits), "FManifoldPoint::FFlags has exceeded capacity. Change to uint16 or uint32.");
