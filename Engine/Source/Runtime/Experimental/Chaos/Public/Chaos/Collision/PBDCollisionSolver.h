	...
	namespace Private
	{
		...
		*/
		...
		public:
			...
			union FFlags
			{
				struct 
				{
					uint8 bDisablePositionSolve : 1;	// Disable the position solver step
					uint8 bDisableVelocitySolve : 1;	// Disable the velocity solver step
					uint8 bDisableFriction : 1;		// Disable friction
				};
				uint8 Bits;
			};
			...
			// Verify that all FFlags data can be initialized to zero (even unused bits)
			static_assert(sizeof(FFlags) == sizeof(FFlags::Bits), "FPBDCollisionSolverManifoldPoint::FFlags has exceeded capacity. Change to uint16 or uint32.");
		};
		...
		*/
		...
		public:
			...
			*/
			void InitManifoldPoint(
				...
				const FSolverReal InSplitImpulseAlpha = 0.0f,
				const bool bDisablePositionSolve = false,
				const bool bDisableVelocitySolve = false,
				const bool bDisableFriction = false);
			...
			FPBDCollisionSolverManifoldPoint& ManifoldPoint)
		{
			...
			if (IsDynamic(0))
			{
				...
				// NOTE: No split impulse mode for friction
			...
			FPBDCollisionSolverManifoldPoint& ManifoldPoint)
		{
			...
			// How much of this position correction should be momentumless?
			// In SplitImpulse mode, only residual overlap corrections apply momentum.
			FSolverReal SplitAlpha = ManifoldPoint.SplitImpulseAlpha;
			FSolverReal SplitAlphaInv = FSolverReal(1) - ManifoldPoint.SplitImpulseAlpha;
			...
			ManifoldPoint.NetPushOutNormal += SplitAlphaInv * PushOutNormal;
			...
			if (IsDynamic(0))
			{
				...
				if (SplitAlpha > 0)
				{
					Body0.ApplyPositionCorrectionDelta(SplitAlpha * DX0);
					Body0.ApplyRotationCorrectionDelta(SplitAlpha * DR0);
				}

				if (SplitAlphaInv > 0)
				{
					Body0.ApplyPositionDelta(SplitAlphaInv * DX0);
					Body0.ApplyRotationDelta(SplitAlphaInv * DR0);
				}
			}
			if (IsDynamic(1))
			{
				...
				if (SplitAlpha > 0)
				{
					Body1.ApplyPositionCorrectionDelta(SplitAlpha * DX1);
					Body1.ApplyRotationCorrectionDelta(SplitAlpha * DR1);
				}

				if (SplitAlphaInv > 0)
				{
					Body1.ApplyPositionDelta(SplitAlphaInv * DX1);
					Body1.ApplyRotationDelta(SplitAlphaInv * DR1);
				}
			}
		}
			...
			FPBDCollisionSolverManifoldPoint& ManifoldPoint)
		{
			...
			if ((DynamicFriction > 0) && (Dt > 0))
			{
				...
				if (NetImpulseTangentSq > (MaxImpulseTangentSq + UE_SMALL_NUMBER))
				{
					...
					ManifoldPoint.StaticFrictionRatio = ImpulseTangentScale;
				}
		...
		FORCEINLINE_DEBUGGABLE void FPBDCollisionSolver::InitManifoldPoint(
			...
			const FSolverReal InWorldContactDeltaTangentV,
			const FSolverReal InWorldContactVelocityTargetNormal,
			const FSolverReal InSplitImpulseAlpha,
			const bool bDisablePositionSolve,
			const bool bDisableVelocitySolve,
			const bool bDisableFriction
			)
		{
			check(State.ManifoldPoints != nullptr);
			check(PointIndex >= 0);
			check(PointIndex < State.NumManifoldPoints);
			check(InSplitImpulseAlpha >= 0.0f);
			check(InSplitImpulseAlpha <= 1.0f);
			...
			ManifoldPoint.SplitImpulseAlpha = InSplitImpulseAlpha;
			...
			ManifoldPoint.NetSoftPushOutNormal = FSolverReal(0);
			ManifoldPoint.StaticFrictionRatio = FSolverReal(0);

			ManifoldPoint.Flags.Bits = 0;
			ManifoldPoint.Flags.bDisablePositionSolve = bDisablePositionSolve;
			ManifoldPoint.Flags.bDisableVelocitySolve = bDisableVelocitySolve;
			ManifoldPoint.Flags.bDisableFriction = bDisableFriction;
		}
			...
			const FSolverReal MaxPushOut)
		{
			...
			if (FrictionStiffness > 0)
			{
				for (int32 PointIndex = 0; PointIndex < NumManifoldPoints(); ++PointIndex)
				{
					...
					const bool bFrictionSolveDisabled = !!SolverManifoldPoint.Flags.bDisableFriction || !!SolverManifoldPoint.Flags.bDisablePositionSolve;
					if (bFrictionSolveDisabled)
					{
						continue;
					}
			...
			const FSolverReal MaxPushOut)
		{
			...
			{
				FPBDCollisionSolverManifoldPoint& SolverManifoldPoint = State.ManifoldPoints[PointIndex];
				if (!!SolverManifoldPoint.Flags.bDisablePositionSolve)
				{
					continue;
				}
		...
		*/
			...
			const bool bApplyDynamicFriction)
		{
			...
			{
				FPBDCollisionSolverManifoldPoint& SolverManifoldPoint = State.ManifoldPoints[PointIndex];
				if (!!SolverManifoldPoint.Flags.bDisableVelocitySolve)
				{
					continue;
				}

				// Solver for normal velocity when...
				// - we applied a positional impulse
				// - we skipped the position solve, but not the velocity solve
				// - we are in split impulse mode and have an overlap
				const bool bShouldSolveNormalVelocity = 
					(SolverManifoldPoint.NetPushOutNormal > FSolverReal(0)) 
					|| !!SolverManifoldPoint.Flags.bDisablePositionSolve
					|| ((SolverManifoldPoint.SplitImpulseAlpha > 0) && (SolverManifoldPoint.ContactDeltaNormal < 0.0f));

				// Solver for tangential velocity if we have non-zero friction and a position correction
				// (unless the position solve step is disabled, in which case we assume we want to run the velocity solve)
				const bool bShouldSolveTangentVelocity = 
					(DynamicFriction > 0)
					&& (
						bShouldSolveNormalVelocity 
						|| (SolverManifoldPoint.ContactDeltaNormal < State.SoftPhi) 
						|| (SolverManifoldPoint.NetSoftPushOutNormal > FSolverReal(0)) 
					) 
					&& !SolverManifoldPoint.Flags.bDisableFriction;
				...
				if (bShouldSolveNormalVelocity || bShouldSolveTangentVelocity)
				{
					...
					if (bShouldSolveTangentVelocity)
					{
						...
						// 1D friction: zero out the V component. TangentU is already velocity-aligned
						if (CVars::bChaos_PBDCollisionSolver_Use1DFriction)
						{
							ContactVelocityDeltaTangentV = FSolverReal(0);
						}
