...
namespace Chaos
{
	...
	public:
		...
		virtual ~FIndexedConstraintHandle() = default;
	...
	*/
	...
	public:
		...
		virtual void ApplyPositionConstraints(const FReal Dt, const int32 It, const int32 NumIts, const int32 BeginIndex, const int32 EndIndex) override final
		{
			...
			{
				TArrayView<int32> RangeIndices = MakeArrayView(&ConstraintIndices[BeginIndex], RangeSize);
				ConstraintContainer.ApplyPositionConstraints(RangeIndices, Dt, It, NumIts);
			}
		}
		...
		virtual void ApplyVelocityConstraints(const FReal Dt, const int32 It, const int32 NumIts, const int32 BeginIndex, const int32 EndIndex) override final
		{
			...
			{
				TArrayView<int32> RangeIndices = MakeArrayView(&ConstraintIndices[BeginIndex], RangeSize);
				ConstraintContainer.ApplyVelocityConstraints(RangeIndices, Dt, It, NumIts);
			}
		}
		...
		virtual void ApplyProjectionConstraints(const FReal Dt, const int32 It, const int32 NumIts, const int32 BeginIndex, const int32 EndIndex) override final
		{
			...
			{
				TArrayView<int32> RangeIndices = MakeArrayView(&ConstraintIndices[BeginIndex], RangeSize);
				ConstraintContainer.ApplyProjectionConstraints(RangeIndices, Dt, It, NumIts);
			}
		}
	...
	private:
		...
		{
			Private::SortArrayByPartitionIndex(ConstraintIndices, PartitionIndices, PrefixSum);
			return true;
		}
		...
		virtual void GetAllConstrainedParticles(TArray<const FGeometryParticleHandle*>& ParticleHandles) const override final
		{
			...
			ParticleHandles.Reserve(ConstraintIndices.Num() * 2);

			for (int32 Index : ConstraintIndices)
			{
				const FConstraintHandle* Constraint = ConstraintContainer.GetConstraintHandles()[Index];
				const FParticlePair& Pair = Constraint->GetConstrainedParticles();
				ParticleHandles.Add(Pair[0]);
				ParticleHandles.Add(Pair[1]);
			}
		}
