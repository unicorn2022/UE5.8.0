...
struct TAABBTreeLeafArray : public TBoundsWrapperHelper<TPayloadType, T, bComputeBounds>
{
	...
	void RemoveElement(TPayloadType Payload)
	{
		for (int32 Idx = 0; Idx < Elems.Num(); ++Idx)
		{
			...
			{
				UE_LOGF(LogChaos, Warning, "AABBTree: Element not removed");
			}
		}
		bDirtyLeaf = true;
	}
	...
	void PrintLeaf() const
	{
		...
		for (const auto& Elem : Elems)
		{
			UE_LOGF(LogChaos, Log, "Elem[%d] with bounds = %f %f %f | %f %f %f", ElemIndex, 
...
public:
	...
	int32 AllocateInternalNode()
	{
		...
		{
			UE_LOGF(LogChaos, Warning, "AABBTree: Allocated internal node is a leaf");
		}
	...
	NodeAndLeafIndices AllocateLeafNodeAndLeaf(const TPayloadType& Payload, const TAABB<T, 3>& NewBounds)
	{
		...
		{
			UE_LOGF(LogChaos, Warning, "AABBTree: Allocated leaf node is not a leaf");
		}
	...
	void DeAllocateInternalNode(int32 NodeIdx)
	{
		...
		{
			UE_LOGF(LogChaos, Warning, "AABBTree: Deallocated Internal node is a leaf");
		}
	}
	...
	void  DeAllocateLeafNode(int32 NodeIdx)
	{
		...
		{
			UE_LOGF(LogChaos, Warning, "AABBTree: Deallocated Leaf node is not a leaf");
		}
	}
	...
	int32 WhichChildAmI(int32 NodeIdx)
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogChaos, Warning, "AABBTree: Child node not found");
			}
			return 1;
		}
	}
...
public:
	...
	void RotateNode(uint32 NodeIdx, bool debugAssert = false)
	{
		...
		if (BestGrandChildToSwap != INDEX_NONE)
		{
			...
			{
				UE_LOGF(LogChaos, Warning, "AABBTree: 1: Rotate Node loop detected");
				return;
			}
			...
			if (UNLIKELY(!ensure(BestAuntToSwap != MotherOfBestGrandChild)))
			{
				...
				UE_LOGF(LogChaos, Warning, "AABBTree: 2: Rotate Node loop detected");
				return;
			}
	...
	NodeAndLeafIndices InsertLeaf(const TPayloadType& Payload, const TAABB<T, 3>& NewBounds)
	{
		...
		{
			UE_LOGF(LogChaos, Warning, "AABBTree: 1: Insert leaf loop detected");
		}
		...
		{
			UE_LOGF(LogChaos, Warning, "AABBTree: 2: Insert leaf loop detected");
		}
		Nodes[BestSibling].ParentNode = NewParent;
		if (UNLIKELY(!ensure(NewLeafIndices.NodeIdx != NewParent)))
		{
			UE_LOGF(LogChaos, Warning, "AABBTree: 3: Insert leaf loop detected");
		}
	...
	void  UpdateAncestorBounds(int32 NodeIdx, bool bDoRotation = false)
	{
		...
		while (ParentNodeIdx != INDEX_NONE)
		{
			...
			{
				UE_LOGF(LogChaos, Warning, "AABBTree: 1: UpdateAncestorBounds loop detected");
	...
	void RemoveLeafNode(int32 LeafNodeIdx, const TPayloadType& Payload)
	{
		...
		{
			UE_LOGF(LogChaos, Warning, "AABBTree: RemoveLeafNode on node that is not a leaf");
		}
		...
		if (ParentNodeIdx != INDEX_NONE)
		{
			...
			{
				UE_LOGF(LogChaos, Warning, "AABBTree: RemoveLeafNode loop detected");
			}
	...
	virtual bool RemoveElement(const TPayloadType& Payload) override
	{
		...
		{
			UE_LOGF(LogChaos, Warning, "AABBTree: RemoveElement unsafe updated from multiple threads detected");
		}
	...
	virtual bool UpdateElement(const TPayloadType& Payload, const TAABB<T, 3>& NewBounds, bool bInHasBounds) override
	{
		if (UNLIKELY(!ensure(bModifyingTreeMultiThreadingFastCheck == false)))
		{
			UE_LOGF(LogChaos, Warning, "AABBTree: UpdateElement unsafe updated from multiple threads detected");
		}
		bModifyingTreeMultiThreadingFastCheck = true;
#if !WITH_EDITOR
...
#endif
			UE_LOGF(LogChaos, Warning, "AABBTree encountered invalid bounds. Min: %ls Max: %ls Name: %ls"
		...
		{
			UE_LOGF(LogChaos, Verbose, "Bounding volume exceeded maximum dirty elements (%d dirty of max %d) and is forcing a tree rebuild.", DirtyElements.Num(), MaxDirtyElements);
			ReoptimizeTree();
		}
		bModifyingTreeMultiThreadingFastCheck = false;
		return bElementExisted;
	...
	{
		UE_LOGF(LogChaos, Log, "Num Leaves = %d", Leaves.Num());
		for(int32 LeafIndex = 0, NumLeaves = Leaves.Num(); LeafIndex < NumLeaves; ++LeafIndex)
		{
			auto& Leaf = Leaves[LeafIndex];
			UE_LOGF(LogChaos, Log, "Overlapping Count[%d] = %d with bounds = %f %f %f | %f %f %f", LeafIndex,
			...
			for(int32 OverlappingIndex = OverlappingOffsets[LeafIndex]; OverlappingIndex < OverlappingOffsets[LeafIndex+1]; ++OverlappingIndex)
			{
				UE_LOGF(LogChaos, Log, "Overlapping Leaf[%d] = %d", LeafIndex, OverlappingLeaves[OverlappingIndex]);
			}
		}
	}
...
//#endif
	...
	TArray<TPayloadType> FindAllIntersectionsImp(const FAABB3& Intersection) const
	{
		struct FSimpleVisitor
		{
			...
			bool PrePreFilter(const TPayloadType&) const { return false; }
	...
	virtual void DebugDraw(ISpacialDebugDrawInterface<T>* InInterface) const override
	{
		if (InInterface)
		{
			...
			if (DirtyElementTree)
			{
				DirtyElementTree->DebugDraw(InInterface);
			}
		}
	}
#endif
