...
enum class ESpatialAccelerationCollectionBucketInnerIndex
{
...
// Bucket inner indices define for specific use cases		
enum UE_DEPRECATED(5.8, "Use ESpatialAccelerationCollectionBucketInnerIndex instead")
ESpatialAccelerationCollectionBucketInnerIdx
{
	Default_Renamed_To_Avoid_Compile_Errors = 0, // If acceleration structures are not split up by body type, they end up in this bucket, otherwise static bodies will go here
	Dynamic = 1, // For dynamic bodies if they use a separate acceleration structure
	DefaultQueryOnly = 2, // Query only bodies if they are separated
	DynamicQueryOnly = 3 // Dynamic Query only bodies if they are separated
};
...
public:
	...
	{		
		const BucketType& Bucket = Buckets[0];
		{
			...
			{
				constexpr uint16 DefaultQueryOnlyAsUInt16 = static_cast<std::underlying_type_t<ESpatialAccelerationCollectionBucketInnerIndex>>(ESpatialAccelerationCollectionBucketInnerIndex::DefaultQueryOnly);
				...
				if (BucketInnerIdx != DefaultQueryOnlyAsUInt16 && BucketInnerIdx != DynamicQueryOnlyAsUInt16)
				{
