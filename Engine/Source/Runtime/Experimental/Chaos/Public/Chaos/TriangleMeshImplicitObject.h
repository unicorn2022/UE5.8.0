...
namespace Chaos
{
	...
	struct FTrimeshBVH
	{
		...
		struct alignas(16) FChildData
		{
			...
			FORCEINLINE const FAABBVectorized& GetBounds(int ChildIndex) const { return Bounds[ChildIndex]; }
		...
		// original visit faces method, using original/full-float face bounds
		...
		FORCEINLINE_DEBUGGABLE void VisitTree(BoundsFilterType& BoundsFilter, FaceVisitorType& FaceVisitor) const
		{
			...
			{
				VisitTreeImpl<BoundsFilterType, FaceVisitorType, false>(BoundsFilter, FaceVisitor);
			}
			else
			{
				VisitTreeImpl<BoundsFilterType, FaceVisitorType, true>(BoundsFilter, FaceVisitor);
			}
		}
private:
		template <typename BoundsFilterType, typename FaceVisitorType, bool bUseQuantizedBounds>
		FORCEINLINE_DEBUGGABLE void VisitTreeImpl(BoundsFilterType& BoundsFilter, FaceVisitorType& FaceVisitor) const
		{
				...
				for (int32 ChildIndex = 0; ChildIndex < 2; ++ChildIndex)
				{
					...
					if (FaceIndex != INDEX_NONE)
					{
						if (BoundsFilter(ChildData.GetBounds(ChildIndex)) == EFilterResult::Keep)
						{
							...
							if (FaceCount > 0)
							{
								if constexpr (!bUseQuantizedBounds)
								{
								...
								{
									if (EVisitorResult::Stop == VisitFacesQuantized(ChildData.GetBounds(ChildIndex), FaceIndex, FaceCount, BoundsFilter, FaceVisitor))
									{
										return;
									}
								}
...
public:
		...
		/// Quantized bounds data/methods:
		/// Optionally stores the FaceBounds as uint8 fractions of their parent node bounds
		/// to save storage costs, instead of using the FaceBounds array

private:
		// Array of quantized face bound data; with 6 elements per triangle + one buffer element at the end to allow safe use of vectorized loads
		TArray<uint8> QuantizedFaceBoundData;

		// @return true if the BVH should use quantized bounds -- i.e., if it is not empty and has no FaceBounds
		bool NeedsQuantizedBounds();
		
		// Set quantized bounds from mesh data
		void InitializeQuantizedBounds(const TParticles<FRealSingle, 3>& Particles, const FTrimeshIndexBuffer& IndexBuffer);
		
		// Reset face bounds data
		void ResetFaceBounds(int32 NumTris, bool bUseQuantized);
		
		// Add a new quantized bound
		void AddQuantizedFaceBound(const TAABB<FRealSingle, 3>& NodeBounds, const TAABB<FRealSingle, 3>& TriBounds);	

		// alternate visit faces method, using quantized face bounds
		template <typename BoundsFilterType, typename FaceVisitorType>
		FORCEINLINE_DEBUGGABLE EVisitorResult VisitFacesQuantized(const FAABBVectorized& NodeBounds, int32 StartIndex, int32 IndexCount, BoundsFilterType& BoundsFilter, FaceVisitorType& FaceVisitor) const
		{
			// compute the scale factor to convert quantized uint8 values to relative positions within the node
			constexpr VectorRegister4Float DeQuantFracScale = MakeVectorRegisterFloatConstant(1.f / 255.f, 1.f / 255.f, 1.f / 255.f, 1.f);
			const VectorRegister4Float DeQuantScale = VectorMultiply(VectorSubtract(NodeBounds.GetMax(), NodeBounds.GetMin()), DeQuantFracScale);

			const int32 EndIndex = (StartIndex + IndexCount);
			int32 FaceIndex = StartIndex;
			// Note EndIndex*6 is 1 element *past* the actual quantized bounds data, but should still be valid because we store an extra buffer byte to allow VectorLoadByte4 to be safely used
			checkSlow(IndexCount > 0 && (QuantizedFaceBoundData.IsValidIndex(FaceIndex * 6) && QuantizedFaceBoundData.IsValidIndex(EndIndex * 6)));
			for (const uint8* QBoundPtr = QuantizedFaceBoundData.GetData() + StartIndex * 6; FaceIndex < EndIndex; ++FaceIndex, QBoundPtr += 6)
			{
				if (BoundsFilter(FAABBVectorized(
					VectorMultiplyAdd(DeQuantScale, VectorLoadByte4(&QBoundPtr[0]), NodeBounds.GetMin()),
					VectorMultiplyAdd(DeQuantScale, VectorLoadByte4(&QBoundPtr[3]), NodeBounds.GetMin()))) == EFilterResult::Keep)
				{
					if (FaceVisitor(FaceIndex) == EVisitorResult::Stop)
					{
						return EVisitorResult::Stop;
		...
		// Private helper to set quantized bound data
		void SetQuantizedBounds(uint8* QuantizedData, const TAABB<FRealSingle, 3>& NodeBounds, const TAABB<FRealSingle, 3>& TriBounds);
		
		// FTriangleMeshImplicitObject has friend access to build the quantized bounds data
		friend class FTriangleMeshImplicitObject;
	};
	...
	public:
		...
		{
			// Whether to store "quantized" bounds per triangle, instead of full vectorized bounds
			// This can save roughly 40% memory for the in-RAM representation of the triangle mesh, for some additional access cost (depending on the platform)
			FTrimeshBVH::EPerFaceBoundsRepresentation PerFaceBounds = FTrimeshBVH::EPerFaceBoundsRepresentation::VectorizedFloat;
			
			FBuildSettings() {}
		};

		// Bitflag representation to pack triangle mesh settings
		enum class ETriangleMeshSettingsFlags : uint8
		{
			CullsBackFaceRaycast = 1 << 0,
			QuantizePerFaceBounds = 1 << 1
		};
		FRIEND_ENUM_CLASS_FLAGS(ETriangleMeshSettingsFlags);

		template <typename IdxType>
		FTriangleMeshImplicitObject(ParticlesType&& Particles, TArray<TVec3<IdxType>>&& Elements, TArray<uint16>&& InMaterialIndices,
			...
			const FBuildSettings InBuildSettings = FBuildSettings())
		...
		, MeshSettingsFlags(PackSettingsFlags(bInCullsBackFaceRaycast, InBuildSettings))
		{
		...
		CHAOS_API bool SweepGeom(const FSphere& QueryGeom, const FRigidTransform3& StartTM, const FVec3& Dir, const FReal Length, FReal& OutTime, FVec3& OutPosition, FVec3& OutNormal, int32& OutFaceIndex, FVec3& OutFaceNormal, const FReal Thickness = 0, const bool bComputeMTD = false, FVec3 TriMeshScale = FVec3(1.0f), bool bOutputIncludesScale = false) const;
		CHAOS_API bool SweepGeom(const TBox<FReal, 3>& QueryGeom, const FRigidTransform3& StartTM, const FVec3& Dir, const FReal Length, FReal& OutTime, FVec3& OutPosition, FVec3& OutNormal, int32& OutFaceIndex, FVec3& OutFaceNormal, const FReal Thickness = 0, const bool bComputeMTD = false, FVec3 TriMeshScale = FVec3(1.0f), bool bOutputIncludesScale = false) const;
		CHAOS_API bool SweepGeom(const FCapsule& QueryGeom, const FRigidTransform3& StartTM, const FVec3& Dir, const FReal Length, FReal& OutTime, FVec3& OutPosition, FVec3& OutNormal, int32& OutFaceIndex, FVec3& OutFaceNormal, const FReal Thickness = 0, const bool bComputeMTD = false, FVec3 TriMeshScale = FVec3(1.0f), bool bOutputIncludesScale = false) const;
		CHAOS_API bool SweepGeom(const FConvex& QueryGeom, const FRigidTransform3& StartTM, const FVec3& Dir, const FReal Length, FReal& OutTime, FVec3& OutPosition, FVec3& OutNormal, int32& OutFaceIndex, FVec3& OutFaceNormal, const FReal Thickness = 0, const bool bComputeMTD = false, FVec3 TriMeshScale = FVec3(1.0f), bool bOutputIncludesScale = false) const;
		...
		CHAOS_API static ETriangleMeshSettingsFlags PackSettingsFlags(bool bInCullsBackFaceRaycast, FBuildSettings BuildSettings);
		...
		void SerializeImp(FChaosArchive& Ar)
		{
			...
			// Quantized bounds are not serialized; we recompute them on load
			if (Ar.IsLoading() && FastBVH.NeedsQuantizedBounds())
			{
				MeshSettingsFlags |= ETriangleMeshSettingsFlags::QuantizePerFaceBounds;
				FastBVH.InitializeQuantizedBounds(MParticles, MElements);
			}
		}
	...
	private:
		...
		ETriangleMeshSettingsFlags MeshSettingsFlags;

		// Initialising constructor privately declared for use in CopySlow to copy the underlying BVH
		template <typename IdxType>
		FTriangleMeshImplicitObject(ParticlesType&& Particles, TArray<TVec3<IdxType>>&& Elements, TArray<uint16>&& InMaterialIndices, const BVHType& InBvhToCopy,
			TUniquePtr<TArray<int32>>&& InExternalFaceIndexMap = nullptr, TUniquePtr<TArray<int32>>&& InExternalVertexIndexMap = nullptr, 
			const bool bInCullsBackFaceRaycast = false,
			const FBuildSettings InBuildSettings = FBuildSettings())
			: FImplicitObject(EImplicitObject::HasBoundingBox | EImplicitObject::DisableCollisions, ImplicitObjectType::TriangleMesh)
			, MParticles(MoveTemp(Particles))
		...
		friend class UE::Physics::FTriangleMeshGeometry;
		...
		template<typename QueryGeomType>
		bool SweepGeomImp(const QueryGeomType& QueryGeom, const FRigidTransform3& StartTM, const FVec3& Dir, const FReal Length, FReal& OutTime, FVec3& OutPosition, FVec3& OutNormal, int32& OutFaceIndex, FVec3& OutFaceNormal, const FReal Thickness, const bool bComputeMTD, FVec3 TriMeshScale = FVec3(1.0f), bool bOutputIncludesScale = false) const;
	...
	ENUM_CLASS_FLAGS(FTriangleMeshImplicitObject::ETriangleMeshSettingsFlags);
