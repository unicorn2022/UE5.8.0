...
// Returns the closest distance between two edges (X0,X1) and (Y0,Y1) and their barycentric coordinates of closest point pair
template<typename T, int d>
T FindClosestPointAndBaryBetweenEdges(const TVector<T, d>& X0, const TVector<T, d>& X1, const TVector<T, d>& Y0, const TVector<T, d>& Y1, 
	TVector<T, 2>& BaryX, TVector<T, 2>& BaryY, T Eps = UE_SMALL_NUMBER)
{
	// Edge directions
	const TVector<T, d> U = X1 - X0;
	const TVector<T, d> V = Y1 - Y0;
	const TVector<T, d> W = X0 - Y0;

	const T a = U.SizeSquared();
	const T b = U.Dot(V);
	const T c = V.SizeSquared();
	const T dval = U.Dot(W);
	const T eval = V.Dot(W);
	...
	// Barycentric weights
	const T s = (FMath::Abs(sN) < Eps) ? (T)0 : sN / sD;
	const T t = (FMath::Abs(tN) < Eps) ? (T)0 : tN / tD;
	BaryX[0] = (T)1 - s;
	BaryX[1] = s;
	BaryY[0] = (T)1 - t;
	BaryY[1] = t;
	...
	// Closest points
	const TVector<T, d> ClosestX = X0 + U * s;
	const TVector<T, d> ClosestY = Y0 + V * t;

	return (ClosestX - ClosestY).Size();
}
