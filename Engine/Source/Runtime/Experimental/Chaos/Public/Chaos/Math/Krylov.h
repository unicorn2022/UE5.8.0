	...
	bool check_residual = false) 
{
	...
	if (beta <= res) 
	{
		UE_LOGF(LogChaos, Warning, "Lanczos residual = %f. Lanczos converged in 0 iterations.", beta)
		scale(x, T(0));
		return;
	}
	set(q, b);
	...
	for (int it = 1; it < max_it; it++) 
	{
		...
		if (check_residual == true)
		{
			...
			if (residual < res) 
			{
				UE_LOGF(LogChaos, VeryVerbose, "Ax - b residual = %g. Lanczos converged in %d iterations.", residual, it);
				return;
			}
		}
		else
		{
			if (beta < res) 
			{
				UE_LOGF(LogChaos, VeryVerbose, "Lanczos residual = %g. Lanczos converged in %d iterations.", beta, it);
				return;
			}
		}
		T mu = beta / d;
	...
	if (check_residual)
	{
		UE_LOGF(LogChaos, Verbose, "Lanczos used max iterations (%d). Residual = %g.", max_it, residual);
	}
	else
	{
		UE_LOGF(LogChaos, Verbose, "Lanczos used max iterations (%d).", max_it);
	}
}
