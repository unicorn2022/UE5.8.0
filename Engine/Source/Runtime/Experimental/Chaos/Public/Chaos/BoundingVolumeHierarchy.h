...
class TBoundingVolumeHierarchy final : public ISpatialAcceleration<int32, T,d>
{
  ...
  private:
	...
	{
		UE_LOGF(LogChaos, Verbose, "%lsNode has Box: (%f, %f, %f) to (%f, %f, %f) with %d Children", *Prefix, MyNode->MMin[0], MyNode->MMin[1], MyNode->MMin[2], MyNode->MMax[0], MyNode->MMax[1], MyNode->MMax[2], MyNode->MChildren.Num());
