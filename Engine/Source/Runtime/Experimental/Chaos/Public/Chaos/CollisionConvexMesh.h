...
namespace Chaos
{
	...
	public:
		...
		static void BuildConvexHull(const TArray<FVec3Type>& InVertices, TArray<TVec3<int32>>& OutIndices, const Params& InParams = Params())
		{
			...
			FString InitialFacesString("Generated Initial Hull: ");
			...
			UE_LOGF(LogChaos, VeryVerbose, "%ls", *InitialFacesString);
#endif
			...
			while(ConflictV)
			{
				...
				UE_LOGF(LogChaos, VeryVerbose, "# GENERATED HULL before adding Vtx %d (%f %f %f)", ConflictV->Vertex, ConflictPos.X, ConflictPos.Y, ConflictPos.Z);
				UE_LOGF(LogChaos, VeryVerbose, "# ------------------------------------------------------");
				...
				while (Face)
				{
					...
					UE_LOGF(LogChaos, VeryVerbose, "v %f %f %f", P1.X, P1.Y, P1.Z);
					UE_LOGF(LogChaos, VeryVerbose, "v %f %f %f", P2.X, P2.Y, P2.Z);
					UE_LOGF(LogChaos, VeryVerbose, "v %f %f %f", P3.X, P3.Y, P3.Z);
					UE_LOGF(LogChaos, VeryVerbose, "f -3 -2 -1");
					Face = Face->Next;
				}
#endif
#endif
	...
	private:
		...
		{
			UE_CLOGF(DEBUG_HULL_GENERATION, LogChaos, VeryVerbose, "Finding conflict vertex");

			for(FConvexFace* CurFace = FaceList; CurFace; CurFace = CurFace->Next)
			{
				UE_CLOGF(DEBUG_HULL_GENERATION, LogChaos, VeryVerbose, "\tTesting Face (%d %d %d)", CurFace->FirstEdge->Vertex, CurFace->FirstEdge->Next->Vertex, CurFace->FirstEdge->Prev->Vertex);
				...
				UE_CLOGF((DEBUG_HULL_GENERATION && !MaxV), LogChaos, VeryVerbose, "\t\tNo Conflict List");
				UE_CLOGF((DEBUG_HULL_GENERATION && MaxV), LogChaos, VeryVerbose, "\t\tFound %d at distance %f", MaxV->Vertex, MaxD);
		...
		{
#if DEBUG_HULL_GENERATION
			UE_LOGF(LogChaos, VeryVerbose, "Generate horizon - START");
#endif
			...
			while(Queue.Num())
			{
...
#if DEBUG_HULL_GENERATION
				UE_LOGF(LogChaos, VeryVerbose, "\tPop edge [%d - %d] from queue", Edge->Vertex, Edge->Next->Vertex);
#endif 

				if(Processed.Contains(NextFace))
				{
#if DEBUG_HULL_GENERATION
					UE_LOGF(LogChaos, VeryVerbose, "\tTwin Face [%d] already processed - skip", NextFace);
#endif
					continue;
				}
				...
				{
#if DEBUG_HULL_GENERATION
					UE_LOGF(LogChaos, VeryVerbose, "\tDistance [%f] > Epsilon [%f] - add to queue", Distance, Epsilon);
#endif 
				...
				{
#if DEBUG_HULL_GENERATION
					UE_LOGF(LogChaos, VeryVerbose, "\tAdd [%d - %d] to horizon ", Edge->Vertex, Edge->Next->Vertex);
#endif 
					...
					{
#if DEBUG_HULL_GENERATION
						UE_LOGF(LogChaos, VeryVerbose, "\tNON VALID EDGE LOOP - internal horizon edge detected [%d - %d] - skipping ", Edge->Vertex, Edge->Next->Vertex);
#endif 
					}
				}
			}
			...
			UE_LOGF(LogChaos, VeryVerbose, "# ======================================================");
			const FVec3Type ConflictPos = InVertices[ConflictV->Vertex];
			UE_LOGF(LogChaos, VeryVerbose, "# BUILD_HORIZON - Conflict Vertex = %d (%f %f %f)", ConflictV->Vertex, ConflictPos.X, ConflictPos.Y, ConflictPos.Z);
			UE_LOGF(LogChaos, VeryVerbose, "# ------------------------------------------------------");
			...
			FString HorizonString("> Final Horizon: (");
			...
			HorizonString += TEXT(")");
			UE_LOGF(LogChaos, VeryVerbose, "%ls", *HorizonString);

			UE_LOGF(LogChaos, VeryVerbose, "Generate horizon - END");
#endif
		...
		{
			UE_CLOGF(DEBUG_HULL_GENERATION, LogChaos, VeryVerbose, "Adding Vertex %d", ConflictV->Vertex);
...
#if DEBUG_HULL_GENERATION
			FString NewFaceString("\tNew Faces: ");
			...
			UE_LOGF(LogChaos, VeryVerbose, "%ls", *NewFaceString);

			FString DeleteFaceString("\tDelete Faces: ");
			...
			UE_LOGF(LogChaos, VeryVerbose, "%ls", *DeleteFaceString);
#endif
