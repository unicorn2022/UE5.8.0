...
class FPerParticlePBDUpdateFromDeltaPosition : public FPerParticleRule
{
  public:
	...
	virtual ~FPerParticlePBDUpdateFromDeltaPosition() = default;
	...
	inline void Apply(FPBDParticles& InParticles, const FReal Dt, const int32 Index) const override
	{
		InParticles.V(Index) = (InParticles.GetP(Index) - InParticles.GetX(Index)) / Dt;
		InParticles.SetX(Index,  InParticles.GetP(Index));
	}

	inline void Apply(TPBDRigidParticles<FReal, 3>& InParticles, const FReal Dt, const int32 Index) const override
	{
	...
	inline void Apply(FPBDRigidParticleHandle* Particle, const FReal Dt) const override
	{
