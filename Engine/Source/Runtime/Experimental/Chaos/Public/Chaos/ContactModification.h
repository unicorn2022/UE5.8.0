...
namespace Chaos
{
	...
	public:
		...
		*/
		...
		CHAOS_API void ModifyEnablePositionSolve(bool bEnable, int32 ContactPointIdx);
		CHAOS_API void ModifyEnableVelocitySolve(bool bEnable, int32 ContactPointIdx);
		CHAOS_API void ModifyEnableFriction(bool bEnable, int32 ContactPointIdx);
		...
		*/
		...
		/*
		* Add a new contact point. WorldPos0 and WorldPos1 are the world-space contact positions on each particle.
		* WorldNormal is the contact normal. The contact separation is implicitly (WorldPos0 - WorldPos1).WorldNormal.
		* If WorldPos0 and WorldPos1 are not aligned along WorldNormal, then friction will be applied to attempt to 
		* correct the tangential separation.
		*/
		CHAOS_API int32 AddContact(const FVec3& WorldPos0, const FVec3& WorldPos1, const FVec3& WorldNormal) const;
