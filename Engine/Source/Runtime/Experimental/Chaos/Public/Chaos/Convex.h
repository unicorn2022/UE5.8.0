...
namespace Chaos
{
	...
	public:
		...
		virtual Pair<FVec3, bool> FindClosestIntersectionImp(const FVec3& StartPoint, const FVec3& EndPoint, const FReal Thickness) const override
		{
			...
			for (int32 Idx = 0; Idx < NumPlanes; ++Idx)
			{ 
				// Offset plane with thickness in the normal direction and do a 0 thickness intersection query, so you only find intersections on the +Thickness*Normal plane, not the -Thickness*Normal plane.
				const Pair<FVec3, bool> PlaneIntersection = Thickness > 0 ? 
					FPlaneType(Planes[Idx].X() + Thickness * Planes[Idx].Normal(), Planes[Idx].Normal()).FindClosestIntersection(StartPoint, EndPoint, 0) :
					Planes[Idx].FindClosestIntersection(StartPoint, EndPoint, 0);
			...
			for (const Pair<FReal, FVec3>& Elem : Intersections)
			{
