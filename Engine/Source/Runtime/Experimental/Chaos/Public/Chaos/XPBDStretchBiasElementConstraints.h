...
public:
	...
	virtual ~FXPBDStretchBiasElementConstraints() = default;
	...
	TArray<TArray<int32>> GetConstraintsArray() const //TODO: avoid converting
	{
		...
		{
			ConstraintsArray[ConstraintIdx].SetNum(3);
			for (int32 TriLocalIdx = 0; TriLocalIdx < 3; TriLocalIdx++)
			{
				ConstraintsArray[ConstraintIdx][TriLocalIdx] = Constraints[ConstraintIdx][TriLocalIdx];
			}
		}
	...
	CHAOS_API void AddStretchBiasElementResidualAndHessian(const FSolverParticlesRange& Particles, const int32 ConstraintIndex, const int32 ConstraintIndexLocal, const FSolverReal Dt, TVec3<FSolverReal>& ParticleResidual, Chaos::PMatrix<FSolverReal, 3, 3>& ParticleHessian);
	...
	CHAOS_API void InitializeDmInvAndMeasures(const FSolverParticlesRange& Particles);

	CHAOS_API void InitializeDmInvAndMeasures(const FSolverParticles& Particles);

	CHAOS_API void AddInternalForceDifferential(const FSolverParticlesRange& InParticles, const TArray<TVector<FSolverReal, 3>>& DeltaParticles, TArray<TVector<FSolverReal, 3>>& Ndf);
