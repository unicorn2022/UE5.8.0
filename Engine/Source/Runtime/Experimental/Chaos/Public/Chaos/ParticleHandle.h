...
void GeometryParticleDefaultConstruct(FConcrete& Concrete, const FGeometryParticleParameters& Params)
{
	...
	Concrete.SetResimType(EResimType::FrozenDuringResim);
...
void PBDRigidParticleDefaultConstruct(FConcrete& Concrete, const FPBDRigidParticleParameters& Params)
{
	...
	Concrete.SetMaxLinearSpeedSq(TNumericLimits<float>::Max());
	Concrete.SetMaxAngularSpeedSq(TNumericLimits<float>::Max());
...
UE_DEPRECATED(5.8, "Use FShapeUnionFilterData::BroadFilter(const FQueryFilterData&) instead")
...
UE_DEPRECATED(5.8, "Use the member functions on FShapeFilterData instead")
...
//Returns true when there is no way a later PreFilter will succeed. Avoid virtuals etc..
UE_DEPRECATED(5.8, "Use FShapeUnionFilterData::BroadFilter(const FShapeFilterData&) instead")
...
public:
	...
	bool PrePreQueryFilter(const void* QueryData) const
	{
		if(bCanPrePreFilter)
		{
			if (const FCollisionFilterData* QueryFilterData = static_cast<const FCollisionFilterData*>(QueryData))
			{
				...
				const Chaos::Filter::FQueryFilterData QuerierFilter = Chaos::Filter::FQueryFilterBuilder::BuildFromLegacyQueryFilter(*QueryFilterData);
				PRAGMA_ENABLE_INTERNAL_WARNINGS
				return UnionFilter.BroadFilter(QuerierFilter) == Chaos::Filter::EBroadFilterResult::Reject;
			}
		}
	...
	bool PrePreSimFilter(const void* SimData) const
	{
		if (bCanPrePreFilter)
		{
			...
			{
				PRAGMA_DISABLE_INTERNAL_WARNINGS
				const Chaos::Filter::FShapeFilterData ShapeFilter = Chaos::Filter::FShapeFilterBuilder::BuildFromLegacySimFilter(*SimFilterData);
				PRAGMA_ENABLE_INTERNAL_WARNINGS
				Chaos::Filter::FShapeUnionFilterData SimUnionFilter;
				SimUnionFilter.Combine(ShapeFilter);
				return UnionFilter.BroadFilter(SimUnionFilter) == Chaos::Filter::EBroadFilterResult::Reject;
			}
		}
	...
	bool PrePreFilter(const Chaos::Filter::FQueryFilterData& QueryFilterData) const
	{
		if (bCanPrePreFilter)
		{
			return UnionFilter.BroadFilter(QueryFilterData) == Chaos::Filter::EBroadFilterResult::Reject;
		}

		return false;
	}

	bool PrePreFilter(const Chaos::Filter::FShapeUnionFilterData& QuerierFilterData) const
	{
		if (bCanPrePreFilter)
		{
			return UnionFilter.BroadFilter(QuerierFilterData) == Chaos::Filter::EBroadFilterResult::Reject;
		}

		return false;
	}

	bool PrePreFilter(const FAccelerationStructureHandle& OtherHandle) const
	{
		return PrePreFilter(OtherHandle.UnionFilter);
	}

	void UpdateFrom(const FAccelerationStructureHandle& InOther)
	{
		UnionFilter = InOther.UnionFilter;
	}
...
public:
	...
	**/
	...
	UE_DEPRECATED(5.8, "Use ComputeFilterDataFromShapes instead")
	static void ComputeParticleQueryFilterDataFromShapes(const TParticle& Particle, FCollisionFilterData& OutQueryFilterData);

	// Computes union filter data from the associated shapes.
	template <typename TParticle>
	static void ComputeFilterDataFromShapes(const TParticle& Particle, Chaos::Filter::FShapeUnionFilterData& OutFilter);
...
private:
	...
	Chaos::Filter::FShapeUnionFilterData UnionFilter;
...
public:
	...
	*/
	...
	/**
	 * Only contains persistent constraints (joints, etc.), not transient constraints (collisions)
	*/
...
public:
	...
	static constexpr EParticleType StaticType() { return EParticleType::Clustered; }
...
public:
	...
	UE_DEPRECATED(5.8, "Use SetShapeFilterData instead")
	...
	void SetShapeFilterData(int32 InShapeIndex, const Chaos::Filter::FShapeFilterData& FilterData)
	{
		const Chaos::Filter::FShapeFilterData& Current = MShapesArray[InShapeIndex]->GetShapeFilterData();
		if (Current != FilterData)
		{
			MShapesArray[InShapeIndex]->SetShapeFilterData(FilterData);
		}
	}

	void SetShapeFilterInstanceData(int32 InShapeIndex, const Chaos::Filter::FInstanceData& InstanceData)
	{
		const Chaos::Filter::FInstanceData& Current = MShapesArray[InShapeIndex]->GetFilterInstanceData();
		if (Current != InstanceData)
		{
			MShapesArray[InShapeIndex]->SetFilterInstanceData(InstanceData);
		}
	}
	...
	if (!ShapesParticles.IsEmpty() && (ShapesParticles.Num() <= (ShapesArray.Num())))
	{
		for (int32 ParticleIndex = 0; ParticleIndex < ShapesParticles.Num(); ++ParticleIndex)
		{
			...
			if (ShapeData && TemplateShape)
			{
				...
				{
					const Chaos::Filter::FInstanceData InstanceData(ActorId, ComponentID);
					ShapeData->SetFilterInstanceData(InstanceData);
					ShapeData->SetShapeFilterData(TemplateShape->GetShapeFilterData());
				}
...
template <typename TParticle>
/* static */ void FAccelerationStructureHandle::ComputeFilterDataFromShapes(const TParticle& Particle, Chaos::Filter::FShapeUnionFilterData& OutFilter)
{
	const FShapesArray& Shapes = Particle.ShapesArray();
	OutFilter = Chaos::Filter::FShapeUnionFilterData();
	for (const auto& Shape : Shapes)
	{
		const bool bSimEnabled = Shape->GetSimEnabled() || Shape->GetIsProbe();
		const bool bQueryEnabled = Shape->GetQueryEnabled();
		OutFilter.Combine(Shape->GetShapeFilterData(), bQueryEnabled, bSimEnabled);
	}
}
...
{
	ComputeFilterDataFromShapes(Particle, UnionFilter);
