...
namespace Chaos::Softs
{
	...
	public:
		...
		static bool AreValuesEqual(const FCollectionPropertyConstFacade& Facade, int32 KeyIndex, const FCollectionPropertyConstFacade& Other, int32 OtherKeyIndex)
		{
			return
				Facade.GetLowValue<FVector3f>(KeyIndex) == Other.GetLowValue<FVector3f>(OtherKeyIndex) &&
				Facade.GetHighValue<FVector3f>(KeyIndex) == Other.GetHighValue<FVector3f>(OtherKeyIndex) &&
				Facade.GetStringValue(KeyIndex) == Other.GetStringValue(OtherKeyIndex) &&
				EnumOnlyContainsFlags(Facade.GetFlags(KeyIndex) ^ Other.GetFlags(OtherKeyIndex), ECollectionPropertyFlags::Dirty | ECollectionPropertyFlags::StringDirty);
		}
		...
		static bool AreValuesEqual(const FCollectionPropertyConstFacade& Facade, const FCollectionPropertyConstFacade& Other, int32 KeyIndex)
		{
			return AreValuesEqual(Facade, KeyIndex, Other, KeyIndex);
		}
	...
	public:
		...
		template<typename T UE_REQUIRES(TIsWeightedType<T>::Value)>
		void SetLowValue(int32 KeyIndex, const T& Value) { SetValue(KeyIndex, ModifyLowValueArray(), FVector3f(Value)); }

		template<typename T UE_REQUIRES(TIsWeightedType<T>::Value)>
		void SetHighValue(int32 KeyIndex, const T& Value) { SetValue(KeyIndex, ModifyHighValueArray(), FVector3f(Value)); }
		...
		void SetStringValue(int32 KeyIndex, const FString& Value) { if (StringValueArray[KeyIndex] != Value) { ModifyStringValueArray()[KeyIndex] = Value; SetStringDirty(KeyIndex); } }
		...
		/** Clear all individual properties' dirty flags and this collection facade dirty flag. */
		CHAOS_API void ClearDirtyFlags();

		/** Make this collection facade dirty (does not change individual property flags). */
		CHAOS_API void MakeDirty();
	...
	protected:
		...
		// Mutable array accessors via ModifyAttribute (marks collection dirty)
		CHAOS_API TArrayView<FName> ModifyKeyNameArray();
		CHAOS_API TArrayView<FVector3f> ModifyLowValueArray();
		CHAOS_API TArrayView<FVector3f> ModifyHighValueArray();
		CHAOS_API TArrayView<FString> ModifyStringValueArray();
		CHAOS_API TArrayView<ECollectionPropertyFlags> ModifyFlagsArray();

		UE_DEPRECATED(5.8, "Use ModifyKeyNameArray instead.")
		const TArrayView<FName>& GetKeyNameArray() { return reinterpret_cast<TArrayView<FName>&>(KeyNameArray); }
		UE_DEPRECATED(5.8, "Use ModifyLowValueArray instead.")
		const TArrayView<FVector3f>& GetLowValueArray() { return reinterpret_cast<TArrayView<FVector3f>&>(LowValueArray); }
		UE_DEPRECATED(5.8, "Use ModifyHighValueArray instead.")
		const TArrayView<FVector3f>& GetHighValueArray() { return reinterpret_cast<TArrayView<FVector3f>&>(HighValueArray); }
		UE_DEPRECATED(5.8, "Use ModifyStringValueArray instead.")
		const TArrayView<FString>& GetStringValueArray() { return reinterpret_cast<TArrayView<FString>&>(StringValueArray); }
		UE_DEPRECATED(5.8, "Use ModifyFlagsArray instead.")
