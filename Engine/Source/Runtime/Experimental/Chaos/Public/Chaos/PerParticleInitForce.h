...
class FPerParticleInitForce : public FPerParticleRule
{
  public:
	...
	virtual ~FPerParticleInitForce() = default;

	inline void Apply(FDynamicParticles& InParticles, const FReal Dt, const int Index) const override
	{
		InParticles.Acceleration(Index) = FVec3(0);
	}

	inline void Apply(TRigidParticles<FReal, 3>& InParticles, const FReal Dt, const int Index) const override
	{
		InParticles.Acceleration(Index) = FVec3(0);
