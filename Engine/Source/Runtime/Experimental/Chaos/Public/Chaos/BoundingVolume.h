...
private:
	...
	void GenerateTree(const ParticleView& Particles, const bool bUseVelocity, const T Dt, const int32 MaxCells)
	{
		...
		{
			UE_LOGF(LogChaos, Error, "BoundingVolume computed invalid GlobalBox from bounds: GlobalBox.Min(): (%f, %f, %f), GlobalBox.Max(): (%f, %f, %f)",
		...
		UE_LOGF(LogChaos, Verbose, "Generated Tree with (%d, %d, %d) Nodes and %f Per Cell", MGrid.Counts()[0], MGrid.Counts()[1], MGrid.Counts()[2], NumObjectsInCells / NumObjectsWithBounds);
	}
