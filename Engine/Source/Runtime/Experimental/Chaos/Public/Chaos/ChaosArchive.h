...
public:
	...
	CHAOS_API void PushSection(const FName& SectionName, const int64 MemoryLocation, const bool bAbsorbChildren);

	CHAOS_API void PopSection(const int64 MemoryLocation);

	CHAOS_API void BeginSerialize(const int64 MemoryLocation);

	CHAOS_API void EndSerialize(const int64 MemoryLocation);
...
private:
	...
	public:
		...
		virtual ~FSharedPtrHolder() = default;
	};
	...
	public:
		...
		virtual ~FRefCountPtrHolder() = default;
	};
...
public:
	FChaosArchive(FArchive& ArIn)
		: FArchiveProxy(ArIn)
		, Context(MakeShared<FChaosArchiveContext>())
	{
	}

protected:
	FChaosArchive(FArchive& ArIn, const TSharedRef<FChaosArchiveContext>& InExternalSharedContext)
	: FArchiveProxy(ArIn)
	, Context(InExternalSharedContext)
	{
	}
...
public:
	...
	{
		Context->BeginSerialize(Tell());
		FArchiveProxy::Serialize(V, Length);
		Context->EndSerialize(Tell());
	}
#endif

	UE_DEPRECATED(5.8, "This method is no longer supported and there is no replacement")
	...
	{
		return MakeUnique<FChaosArchiveContext>();
	}
...
private:
	...
	TSharedRef<FChaosArchiveContext> Context;
...
public:
	...
	: Ar(ArIn)
	{
		Ar.Context->PushSection(SectionName, Ar.Tell(), bAbsorbChildren);
	}

	~FChaosArchiveScopedMemory()
	{
		Ar.Context->PopSection(Ar.Tell());
	}
private:
	FChaosArchive& Ar;
#else
