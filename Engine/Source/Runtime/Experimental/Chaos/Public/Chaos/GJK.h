...
namespace Chaos
{
	...
	*/
	...
	{
		const T EpsilonScale = FMath::Max<T>(A.TGeometryA::BoundingBox().Extents().Max(), B.TGeometryB::BoundingBox().Extents().Max());

		const bool bUseDouble = EpsilonScale > 1.e5;
		if (bUseDouble)
		{
			return GJKIntersectionImpl<T, FReal>(A, B, EpsilonScale, BToATM, InThicknessA, InitialDir);
		}
		return GJKIntersectionImpl<T, FRealSingle>(A, B, EpsilonScale, BToATM, InThicknessA, InitialDir);
	}
	...
	template <typename T>
	inline bool GJKIntersectionSimd(const FGeomGJKHelperSIMD& RESTRICT A, const FGeomGJKHelperSIMD& RESTRICT B, FRealSingle EpsilonScale, const T& Translation, const T& Rotation, FRealSingle InThicknessA, const T& InitialDir)
	{
		T V = VectorNegate(InitialDir);
		V = VectorNormalizeSafe(V, MakeVectorRegisterFloatConstant(-1.f, 0.f, 0.f, 0.f));

		const T AToBRotation = VectorQuaternionInverse(Rotation);
		bool bTerminate;
		bool bNearZero = false;
		int NumIterations = 0;
		T PrevDist2Simd = MakeVectorRegisterFloatConstant(FLT_MAX, FLT_MAX, FLT_MAX, FLT_MAX);

		T SimplexSimd[4] = { VectorZero(), VectorZero(), VectorZero(), VectorZero() };
		T BarycentricSimd;
		int32 NumVerts = 0;
		...
		const T Inflation = VectorSet1(InflationReal);
		const T Inflation2 = VectorMultiply(Inflation, Inflation);
		...
		do
		{
			...
			const T NegVSimd = VectorNegate(V);
			const T SupportASimd = A.SupportFunction(TMakeVectorRegisterFloatFromDouble<T>(NegVSimd), ThicknessA);
			const T VInBSimd = VectorQuaternionRotateVector(AToBRotation, V);
			const T SupportBLocalSimd = B.SupportFunction(TMakeVectorRegisterFloatFromDouble<T>(VInBSimd), ThicknessB);
			const T SupportBSimd = VectorAdd(VectorQuaternionRotateVector(Rotation, SupportBLocalSimd), Translation);
			const T WSimd = VectorSubtract(SupportASimd, SupportBSimd);
			...
			V = VectorSimplexFindClosestToOrigin<T, false>(SimplexSimd, NumVerts, BarycentricSimd, nullptr, nullptr);

			const T NewDist2Simd = VectorDot3(V, V);
	...
	template <typename PrevT, typename NewT>
	bool GJKIntersectionImpl(const FGeomGJKHelperSIMD& RESTRICT A, const FGeomGJKHelperSIMD& RESTRICT B, PrevT EpsilonScale, const TRigidTransform<PrevT, 3>& BToATM, const PrevT InThicknessA = 0, const TVector<PrevT, 3>& InitialDir = TVector<PrevT, 3>(1, 0, 0))
	{
		const UE::Math::TQuat<PrevT>& Rotation = BToATM.GetRotation();
		TVectorRegisterType<NewT> RotationSimd = MakeVectorRegister(NewT(Rotation.X), NewT(Rotation.Y), NewT(Rotation.Z), NewT(Rotation.W));

		const UE::Math::TVector<PrevT>& TranslationDouble = BToATM.GetTranslation();
		const TVectorRegisterType<NewT> TranslationSimd = MakeVectorRegister(NewT(TranslationDouble.X), NewT(TranslationDouble.Y), NewT(TranslationDouble.Z), NewT(0.0));
		RotationSimd = VectorNormalizeSafe(RotationSimd, GlobalVectorConstants::Float0001);

		const TVectorRegisterType<NewT> InitialDirSimd = MakeVectorRegister(NewT(InitialDir[0]), NewT(InitialDir[1]), NewT(InitialDir[2]), NewT(0.0));

		return GJKIntersectionSimd<TVectorRegisterType<NewT>>(A, B, FRealSingle(EpsilonScale), TranslationSimd, RotationSimd, FRealSingle(InThicknessA), InitialDirSimd);
	}
	...
	bool GJKPenetrationWarmStartable(const TGeometryA& A, const TGeometryB& B, const TRigidTransform<T, 3>& BToATM, T& OutPenetration, TVec3<T>& OutClosestA, TVec3<T>& OutClosestB, TVec3<T>& OutNormalA, TVec3<T>& OutNormalB, int32& OutVertexA, int32& OutVertexB, TGJKSimplexData<T>& InOutSimplexData, T& OutMaxSupportDelta, const T Epsilon = T(1.e-3), const T EPAEpsilon = T(1.e-2))
	{
		...
		if (bIsContact)
		{
			...
			case EEPAResult::BadInitialSimplex:
				...
				//UE_LOGF(LogChaos, Warning, "EPA Touching Case");
				break;
			case EEPAResult::Degenerate:
				...
				//UE_LOGF(LogChaos, Warning, "EPA Degenerate Case");
				break;
			}
		}
