...
public:
	...
	FPBDSoftBodyCollisionConstraintBase(
		...
		const FSolverVec2 InSoftBodyCollisionThickness = FSolverVec2::ZeroVector,
		...
		, InnerCollisionThickness(FSolverVec2::ZeroVector)
		, CollisionThickness(InCollisionThickness)
		, SoftBodyCollisionThicknessWeighted(InSoftBodyCollisionThickness, TConstArrayView<FRealSingle>(), 0)
	...
	UE_DEPRECATED(5.8, "Use the overload taking FSolverVec2 for InSoftBodyCollisionThickness")
	FPBDSoftBodyCollisionConstraintBase(
		const TArray<FSolverRigidTransform3>& InLastCollisionTransforms,
		FSolverReal InCollisionThickness,
		FSolverReal InFrictionCoefficient,
		...
		const FSolverReal InSoftBodyCollisionThickness,
		bool bInEnableSimpleColliders = true,
		bool bInEnableComplexColliders = true,
		bool bInUsePlanarConstraintForSimpleColliders = bDefaultUsePlanarConstraintForSimpleColliders,
		bool bInUsePlanarConstraintForComplexColliders = bDefaultUsePlanarConstraintForComplexColliders,
		...
		: FPBDSoftBodyCollisionConstraintBase(InLastCollisionTransforms, InCollisionThickness,
			InFrictionCoefficient, bInUseCCD, InProximityStiffness,
			InCollisionParticleCollided, InContacts, InNormals, InPhis,
			FSolverVec2(InSoftBodyCollisionThickness, InSoftBodyCollisionThickness),
			bInEnableSimpleColliders, bInEnableComplexColliders,
			bInUsePlanarConstraintForSimpleColliders, bInUsePlanarConstraintForComplexColliders,
			InComplexBoneData)
	{}
	...
	FSolverReal GetParticleClothThickness(int32 ParticleIndex) const
	{
		return SoftBodyCollisionThicknessWeighted.GetValue(ParticleIndex);
	}

	FSolverReal GetParticleInnerThickness(int32 ParticleIndex) const
	{
		return InnerCollisionThickness.GetValue(ParticleIndex);
	}
...
protected:
	...
	FPBDWeightMap InnerCollisionThickness;
	FSolverReal CollisionThickness;
	FPBDFlatWeightMap SoftBodyCollisionThicknessWeighted;
	UE_DEPRECATED(5.8, "Use SoftBodyCollisionThicknessWeighted instead")
	FSolverReal SoftBodyCollisionThickness = 0.f;
...
public:
	...
	CHAOS_API FPBDSoftBodyCollisionConstraint(
		const TArray<FSolverRigidTransform3>& InLastCollisionTransforms,
		...
		TArray<FSolverReal>* InPhis = nullptr,
		const TMap<FParticleRangeIndex, FPBDComplexColliderBoneData>& InComplexBoneData = TMap<FParticleRangeIndex, FPBDComplexColliderBoneData>(),
		const TMap<FString, TConstArrayView<float>>& WeightMaps = TMap<FString, TConstArrayView<float>>());
	...
	UE_DEPRECATED(5.8, "Use the overload that accepts WeightMaps")
	void SetProperties(const FCollectionPropertyConstFacade& PropertyCollection)
	{
		SetProperties(PropertyCollection, TMap<FString, TConstArrayView<float>>());
	}
...
private:
	...
	UE_CHAOS_DECLARE_INDEXLESS_PROPERTYCOLLECTION_NAME(InnerCollisionThickness, float);
};
