		...
		*/
		...
		public:
			...
			*/
			...
			/**
			 * Returns the max number of serial constraint batches for the given ContainerId across all active island groups.
			 * Returns 0 if partitioning is disabled or no group has batches for the given container.
			*/
			UE_INTERNAL int32 GetMaxSerialConstraintBatches(const int32 ContainerId) const;
		...
		private:
			...
			CHAOS_API void SolveGroupConstraintsSerial(const int32 GroupIndex, const FReal Dt);
			CHAOS_API void SolveGroupConstraintsParallelCalledFromTask(const int32 GroupIndex, const FReal Dt);
			CHAOS_API void SolveGroupConstraintsParallelNotCalledFromTask(const int32 GroupIndex, const FReal Dt);

			CHAOS_API void SolveGroupConstraintsCalledFromTask(const int32 GroupIndex, const FReal Dt);
			CHAOS_API void SolveGroupConstraintsNotCalledFromTask(const int32 GroupIndex, const FReal Dt);

			template<typename Lambda>
			TArray<UE::Tasks::FTask> DispatchBatchedTasks(
				Lambda BatchedTask,
