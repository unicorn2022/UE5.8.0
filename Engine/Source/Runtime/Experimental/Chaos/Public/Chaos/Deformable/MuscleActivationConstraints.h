...
namespace Chaos::Softs
{
	...
	// Stores data for muscle activation and apply rest volume changes to tetrahedral constraints
	class FMuscleActivationTetrahedralConstraints
	{
	public:
		FMuscleActivationTetrahedralConstraints(){}

		virtual ~FMuscleActivationTetrahedralConstraints() = default;

		void AddMuscles(const FSolverParticles& RestParticles, const Facade& FMuscleActivation, int32 VertexOffset = 0, int32 ElementOffset = 0)
		{
			for (int MuscleIdx = 0; MuscleIdx < FMuscleActivation.NumMuscles(); MuscleIdx++)
			{
				...
				if (FMuscleActivation.IsValidGeometryIndex(MuscleActivationData.GeometryGroupIndex))
				{
							...
							TEXT("FiberLengthRatioAtMaxActivation %f of muscle indexed %d is <= 0, please check your setup."), MuscleActivationData.InflationVolumeScale, MuscleIdx))
					{
						...
						{
							UE_LOGF(LogMuscleActivationConstraint, Error, "Muscle Idx[%d] has invalid origin[%d] or insertion[%d].",
						...
						MuscleActivation.Add(FSolverReal(0));
				...
				{
					UE_LOGF(LogMuscleActivationConstraint, Error, "Muscle Idx[%d] has invalid geometry index[%d].", 
						MuscleIdx, MuscleActivationData.GeometryGroupIndex);
				}
			}
		}

		void UpdateLengthBasedMuscleActivation(const FSolverParticles& InParticles)
		{
			for (int32 MuscleIdx = 0; MuscleIdx < MuscleActivationElement.Num(); MuscleIdx++)
			{
					...
					TEXT("MuscleLengthRatioThresholdForMaxActivation %f of muscle indexed %d is out of range (0,1), please check your setup."), MuscleLengthRatioThresholdForMaxActivation[MuscleIdx], MuscleIdx))
				{
					...
					const FSolverReal MuscleLengthRatio = (InParticles.P(OriginInsertionPair[MuscleIdx][0]) - InParticles.P(OriginInsertionPair[MuscleIdx][1])).Size() / MuscleRestLength[MuscleIdx];
					if (MuscleLengthRatio >= FSolverReal(1))
					{
						// not active
						MuscleActivation[MuscleIdx] = FSolverReal(0);
					}
					else
					{
						FSolverReal NormalizedMuscleLengthLevel = FMath::Clamp((FSolverReal(1) - MuscleLengthRatio) / (FSolverReal(1) - MuscleLengthRatioThresholdForMaxActivation[MuscleIdx]), FSolverReal(0), FSolverReal(1));
		...
		void ApplyMuscleActivation(FXPBDCorotatedTetrahedralConstraints& Constraints) const
		{
			for (int32 MuscleIdx = 0; MuscleIdx < MuscleActivationElement.Num(); MuscleIdx++)
			{
				...
				{
					const FSolverReal FiberLengthRatio = FSolverReal(1) - MuscleActivation[MuscleIdx] * (FSolverReal(1) - FiberLengthRatioAtMaxActivation[MuscleIdx]);
		...
		void ApplyInflationVolumeScale(FXPBDCorotatedTetrahedralConstraints& Constraints) const
		{
			for (int32 MuscleIdx = 0; MuscleIdx < MuscleActivationElement.Num(); MuscleIdx++)
			{
				if (ensureMsgf(MuscleActivationElement[MuscleIdx].Num() == FiberDirectionMatrix[MuscleIdx].Num(),
		...
		FSolverReal GetMuscleActivation(int32 MuscleIndex) const { 
		...
		bool SetMuscleActivation(int32 MuscleIndex, FSolverReal InMuscleActivation)
		{
			if (MuscleActivation.IsValidIndex(MuscleIndex))
			{
				MuscleActivation[MuscleIndex] = FMath::Clamp(InMuscleActivation, FSolverReal(0), FSolverReal(1));
				return true;
			}
			return false;
		};
	...
	private:	
		...
		TArray<FSolverReal> MuscleRestLength;
		TArray<FSolverReal> MuscleActivation;
		TArray<TArray<Chaos::PMatrix33d>> FiberDirectionMatrix;
		TArray<TArray<FSolverReal>> ContractionVolumeScale;
		TArray<FSolverReal> FiberLengthRatioAtMaxActivation;
		TArray<FSolverReal> MuscleLengthRatioThresholdForMaxActivation;
		TArray<FSolverReal> InflationVolumeScale;
	...
	using FMuscleActivationConstraints UE_DEPRECATED(5.8, "Deprecated. this class is to be deleted, use FMuscleActivationTetrahedralConstraints instead") = FMuscleActivationTetrahedralConstraints;
