...
{
	using Chaos::TVec3;
	struct FGaussSeidelDynamicSpringConstraints: public FGaussSeidelSpringConstraints
	{

		typedef FGaussSeidelSpringConstraints Base;
		using Base::ConstraintsData;
		using Base::NodalWeights;
		using Base::WCIncidentElements;
		using Base::WCIncidentElementsLocal;
		typedef typename FGaussSeidelSpringConstraints::FGaussSeidelConstraintHandle GSConstraintHandle;

		FGaussSeidelDynamicSpringConstraints(const FDeformableXPBDWeightedSpringConstraintParams& InParams)
			: Base({}, {}, {}, {}, {}, {}, {}, InParams)
		{
			ConstraintsData.AddArray(&Handles);
		}

		virtual ~FGaussSeidelDynamicSpringConstraints()
		{
		...
		const GSConstraintHandle* AddSingleParticleTetrahedraConstraint(const GeometryCollection::Facades::FTetrahedralFacade& InTargetGeom, const FSolverParticles& AllParticles, const GeometryCollection::Facades::TetrahedralParticleEmbedding& InIntersection, const int32 InConstraintIndex, const Chaos::FRange& SourceRange, const Chaos::FRange& TargetRange, const FSolverReal PositionTargetStiffness)
		{
			...
			SingleConstraintData.SingleWeights = {(FSolverReal)1.};
			...
			SingleConstraintData.SingleNormal = TVec3<FSolverReal>((FSolverReal)0.);
			...
			SingleConstraintData.SingleStiffness = (FSolverReal)ConstraintStiffness;
		...
		void RemoveSingleConstraint(const GSConstraintHandle* DeletedSingleConstraint)
		{
			...
			const FGaussSeidelSpringConstraintSingleData& SingleData = ConstraintsData.GetSingleConstraintData(OldConstraintIndex);
		...
		TArray<const GSConstraintHandle*> AddParticleTetrahedraConstraints(const GeometryCollection::Facades::FTetrahedralFacade& InTargetGeom, const FSolverParticles& AllParticles, const TArray<GeometryCollection::Facades::TetrahedralParticleEmbedding>& InIntersections, const Chaos::FRange& SourceRange, const Chaos::FRange& TargetRange, const FSolverReal ConstraintStiffness)
		{
		...
		void ReComputeNodalWeights(const TSet<int32>& DirtyVerts)
		{
			for (const int32 Vert : DirtyVerts)
			{
				...
				NodalWeights[Vert].Init(FSolverReal(0), 6);
		...
		void AdjustStiffness(const TArray<const GSConstraintHandle*> ConstraintHandles, const TArray<FSolverReal>& StiffnessPerConstraint, const FSolverParticles& Particles)
		{
			TSet<int32> DirtyVertices;
			for (int32 i = 0; i < ConstraintHandles.Num(); i++)
			{
				const GSConstraintHandle* SingleHandle = ConstraintHandles[i];
				const FSolverReal NewStiffness = StiffnessPerConstraint[i];
				const int32 ConstraintIndex = SingleHandle->ConstraintIndex;
				const FGaussSeidelSpringConstraintSingleData& SingleConstraintData = ConstraintsData.GetSingleConstraintData(ConstraintIndex);
				FSolverReal NewConstraintStiffness = (FSolverReal)0.;
	...
	using FGaussSeidelDynamicWeakConstraints UE_DEPRECATED(5.8, "Deprecated. this struct is to be deleted, use FGaussSeidelDynamicSpringConstraints instead") = FGaussSeidelDynamicSpringConstraints;
