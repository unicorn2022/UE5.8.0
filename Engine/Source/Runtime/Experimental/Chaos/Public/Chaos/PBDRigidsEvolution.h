...
public:
	...
	*/
	...
	void DestroyParticle(FGeometryParticleHandle* Particle)
	{
		...
		RemoveParticleFromAccelerationStructure(*Particle);
		DisconnectPersistentConstraints(TSet<FGeometryParticleHandle*>({ Particle }));
		DestroyTransientConstraints(Particle);
		IslandManager.RemoveParticle(Particle);
	...
	/** Change the object state for a list of particles */
	CHAOS_API void SetParticlesObjectState(TArrayView<FPBDRigidParticleHandle*> Particles, const EObjectStateType ObjectState, const bool bUpdateViews);
	...
	*/
	UE_DEPRECATED(5.8, "Use DisconnectPersistentConstraints") void DisconnectConstraints(const TSet<FGeometryParticleHandle*>& RemovedParticles) { DisconnectPersistentConstraints(RemovedParticles); }
	void DisconnectPersistentConstraints(const TSet<FGeometryParticleHandle*>& RemovedParticles)
	{
	...
	/** 
	* Disable constraints (all types except collisions) from a particle to be removed (or destroyed)
