...
public:
	...
	template <typename TOther>
	bool IsNearlyEqual(const TOther& Other, FReal Leniency = UE_KINDA_SMALL_NUMBER) const
	{
		return FVec3::IsNearlyEqual(MX, Other.X(), Leniency) && FRotation3f::IsNearlyEqual(MR, FRotation3f(Other.R()), static_cast<FRealSingle>(Leniency));
	}
...
public:
	...
	bool IsNearlyEqual(const TOther& Other, FReal Leniency = UE_KINDA_SMALL_NUMBER) const
	{
		return FVec3::IsNearlyEqual(MV, Other.GetV(), Leniency) && FVec3::IsNearlyEqual(MW, Other.GetW(), Leniency);
	}
...
public:
	...
	bool IsNearlyEqual(const TOther& Other, FReal Leniency = UE_KINDA_SMALL_NUMBER) const
	{
		return FVec3::IsNearlyEqual(Acceleration(), Other.Acceleration(), Leniency)
			&& FVec3::IsNearlyEqual(AngularAcceleration(), Other.AngularAcceleration(), Leniency)
			&& FVec3::IsNearlyEqual(LinearImpulseVelocity(), Other.LinearImpulseVelocity(), Leniency)
			&& FVec3::IsNearlyEqual(AngularImpulseVelocity(), Other.AngularImpulseVelocity(), Leniency);
	}
...
public:
	...
	bool IsNearlyEqual(const TOther& Other, FReal Leniency = UE_KINDA_SMALL_NUMBER) const
	{
		return FVec3::IsNearlyEqual(CenterOfMass(), Other.CenterOfMass(), Leniency)
			&& FRotation3::IsNearlyEqual(RotationOfMass(), Other.RotationOfMass(), static_cast<FRealSingle>(Leniency))
			&& FVec3::IsNearlyEqual(I(), Other.I(), Leniency)
			&& FVec3::IsNearlyEqual(InvI(), Other.InvI(), Leniency)
			&& FMath::Abs(M() - Other.M()) <= Leniency
			&& FMath::Abs(InvM() - Other.InvM()) <= Leniency;
	}
...
public:
	...
	{
		return ObjectState() == Other.ObjectState()
			&& FMath::Abs(MLinearEtherDrag - Other.MLinearEtherDrag) <= Leniency
			&& FMath::Abs(MAngularEtherDrag - Other.MAngularEtherDrag) <= Leniency
			&& FMath::Abs(MMaxLinearSpeedSq - Other.MMaxLinearSpeedSq) <= Leniency
			&& FMath::Abs(MMaxAngularSpeedSq - Other.MMaxAngularSpeedSq) <= Leniency
			&& FMath::Abs(InitialOverlapDepenetrationVelocity() - Other.InitialOverlapDepenetrationVelocity()) <= Leniency
			&& FMath::Abs(SleepThresholdMultiplier() - Other.SleepThresholdMultiplier()) <= Leniency
			&& CollisionGroup() == Other.CollisionGroup()
			&& SleepType() == Other.SleepType()
			&& CollisionConstraintFlags() == Other.CollisionConstraintFlags()
			&& ControlFlags() == Other.ControlFlags()
...
public:
	...
	bool IsNearlyEqual(const TOther& Other, FReal Leniency = UE_KINDA_SMALL_NUMBER) const
	{
		// No properties that can be nearly compared
		return IsEqual(Other);
	}
...
struct FCollisionData
{
	...
	UE_INTERNAL CHAOS_API FCollisionFilterData GetQueryData() const;
	UE_INTERNAL CHAOS_API void SetQueryData(const FCollisionFilterData& InQueryData);
	UE_INTERNAL CHAOS_API FCollisionFilterData GetSimData() const;
	...
	void Serialize(FChaosArchive& Ar)
	{
		...
		{
			FCollisionFilterData OldQueryData, OldSimData;
			if(Ar.IsLoading())
			{
				Ar << OldQueryData;
				Ar << OldSimData;
				const Chaos::Filter::FCombinedShapeFilterData CombinedFilter = Chaos::Filter::FShapeFilterBuilder::LoadFromLegacySerialization(OldQueryData, OldSimData);
				SetCombinedShapeFilterData(CombinedFilter);
			}
			else
			{
				Chaos::Filter::FCombinedShapeFilterData CombinedFilter = GetCombinedShapeFilterData();
				Chaos::Filter::FShapeFilterBuilder::GetLegacyShapeFilter(CombinedFilter, OldQueryData, OldSimData);
				Ar << OldQueryData;
				Ar << OldSimData;
			}
		}
		else
		{
			...
			if (Ar.IsLoading())
			{
				Ar << InstanceData;
				Ar << FilterData;
				SetFilterInstanceData(InstanceData);
				SetShapeFilterData(FilterData);
			}
			else
			{
				InstanceData = GetFilterInstanceData();
				FilterData = GetShapeFilterData();
				Ar << InstanceData;
				Ar << FilterData;
			}
		}
...
#undef SHAPE_PROPERTY
	...
	public:
		...
		void SetBits(const int32 BitsIn)
		{
			Bits = BitsIn;
		}
