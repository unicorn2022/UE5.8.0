...
{
	// Tetrahedral constraint using fixed corotated + muscle fiber constitutive model
	// Fixed corotated: [Stomakhin et al., 2012]
	// Muscle fiber: [Blemker et al., 2005]
	class FXPBDCorotatedFiberTetrahedralConstraints : public FXPBDCorotatedTetrahedralConstraints
	{
	...
	public:
		...
		FXPBDCorotatedFiberTetrahedralConstraints(
			...
			const TVector<FSolverReal, 3> InFiberDir = TVector<FSolverReal, 3>((FSolverReal)1., (FSolverReal)0., (FSolverReal)0.),
		...
		virtual ~FXPBDCorotatedFiberTetrahedralConstraints() = default;
		
		TVector<FSolverReal, 3> GetFiberDir() { return FiberDir; }

		void SetActivation(const FSolverReal AlphaIn) { AlphaActivation = AlphaIn; }
		...
		{
			TVec4<TVector<FSolverReal, 3>> PolarDelta = Base::GetPolarDelta(Particles, Dt, ElementIndex);
			...
			TVec4<TVector<FSolverReal, 3>> DetDelta = Base::GetDeterminantDelta(Particles, Dt, ElementIndex);
			...
			TVec4<TVector<FSolverReal, 3>> FiberDelta = GetFiberDelta(Particles, Dt, ElementIndex);
		...
		TVec4<TVector<FSolverReal, 3>> GetFiberGradient(const FSolverReal dFpdL, const FSolverReal dFadL,const FSolverReal C3, const TVec4<TVector<FSolverReal, 3>>& dLdX) const 
		{
			...
			TVec4<TVector<FSolverReal, 3>> dC3(TVector<FSolverReal, 3>((FSolverReal)0.));
			for (int32 i = 0; i < 4; i++)
			{
				for (int32 j = 0; j < 3; j++)
				{
	...
	private:
		...
		TVec4<TVector<FSolverReal, 3>> GetFiberDelta(const FSolverParticlesRange& Particles, const FSolverReal Dt, const int32 ElementIndex, const FSolverReal Tol = FSolverReal(1e-3)) const
		{
			...
			TVector<FSolverReal, 3> FeV = Fe.GetTransposed() * FiberDir;
			TVector<FSolverReal, 3> DmInverseV = Base::ElementDmInv(ElementIndex).GetTransposed() * FiberDir;
			FSolverReal L = FeV.Size();
			TVec4<TVector<FSolverReal, 3>> dLdX(TVector<FSolverReal, 3>((FSolverReal)0.));
			...
			TVec4<TVector<FSolverReal, 3>> dC3 = GetFiberGradient(dFpdL,dFadL, C3, dLdX);
	...
	private:
		...
		TVector<FSolverReal, 3> FiberDir;

	};

	template <typename T, typename ParticleType>
	using FXPBDCorotatedFiberConstraints UE_DEPRECATED(5.8, "Deprecated. this class is to be deleted, use FXPBDCorotatedFiberTetrahedralConstraints instead") = FXPBDCorotatedFiberTetrahedralConstraints;
