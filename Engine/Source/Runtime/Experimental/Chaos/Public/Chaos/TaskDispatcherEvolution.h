...
public:
	...
	void PruneInternalPendingData(Lambda HasToBeUpdated)
	{
		...
		{
			UE_LOGF(LogChaos, Verbose, "No pruning happened in the Spatial Acceleration structure, a slight perfromance hit could occured. IntegrationPendingTasks.Num(): %d !=  NumBatchDyn:  %d", IntegrationPendingTasks.Num(), NumBatchDyn);
		...
		else if (NumBatchKin > 0 && InternalAccelerationQueue.KinematicBatchStartIndex > 0)
		{
			...
			UE_LOGF(LogChaos, Verbose, "No pruning happened in the Spatial Acceleration structure, a slight perfromance hit could occured. KinematicPendingTasks.Num(): %d !=  NumBatchKin:  %d", KinematicPendingTasks.Num(), NumBatchKin);
	...
	void FlushAccelerationQueue(Lambda FlushAccelerationQueueLambda)
	{
		...
		PendingTasks.Append(KinematicPendingTasks);
		PendingTasks.Append(IntegrationPendingTasks);
