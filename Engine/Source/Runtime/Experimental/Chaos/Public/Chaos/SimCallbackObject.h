...
enum class ESimCallbackOptions : uint32
{
	None					= 0,
	Presimulate				= 1 << 0,
	...
	PreSolve				= 1 << 13,
	PostSolve				= 1 << 14,
	InjectInputsExternal	= 1 << 15,
	ProcessInputsExternal	= 1 << 16,
	ProcessInputsInternal	= 1 << 17,
};
...
public:
	...
	{
		OnInjectInputs_External(PhysicsStep, NumSteps);
	}

	virtual void ProcessInputs_External(int32 PhysicsStep)
	{
		OnProcessInputs_External(PhysicsStep);
	}
	...
	void PostInitialize_External()
	{
		OnPostInitialize_External();
	}

	void PreShutdown_External()
	{
		OnPreShutdown_External();
	}

	virtual void ProcessInputs_Internal(int32 PhysicsStep)
	{
		OnProcessInputs_Internal(PhysicsStep);
	}
...
private:
	...
	/** Called before physics task is dispatched from the game thread, last call to produce async inputs in
	* Register with ESimCallbackOptions::InjectInputsExternal to get callback */
	virtual void OnInjectInputs_External(int32 PhysicsStep, int32 NumSteps)
	{
		check(false);
	}

	/** Called before async inputs are marshaled over to the physics thread, last call to alter already produced async inputs in
	* Register with ESimCallbackOptions::ProcessInputsExternal to get callback */
	virtual void OnProcessInputs_External(int32 PhysicsStep)
	{
		// check(false); // Enable when FNetworkPhysicsCallback no longer calls this
	}

	/**
	* Called once when callback object is registered and after input data have been marshaled
	*/
	virtual void OnPostInitialize_Internal() { }

	/**
	* Called on the external thread after the callback object is registered with the solver.
	* The solver is guaranteed to be available via GetSolver() at this point.
	* Use this for GT-side setup such as registering with solver delegates.
	*/
	virtual void OnPostInitialize_External() { }

	/**
	* Called on the external thread before the callback object is unregistered from the solver.
	* Symmetric to OnPostInitialize_External — use this to clean up any solver-level registrations
	* (e.g. delegates) that were set up during initialization.
	*/
	virtual void OnPreShutdown_External() { }

	/** Called before any sim callbacks are triggered but after async inputs has marshaled over
	* Register with ESimCallbackOptions::ProcessInputsInternal to get callback */
	virtual void OnProcessInputs_Internal(int32 PhysicsStep)
	{
		// check(false); // Enable when FNetworkPhysicsCallback no longer calls this
	}
