...
FRecordedTransformTrack FRecordedTransformTrack::ProcessRawRecordedData(const FRecordedTransformTrack& InCache)
{
	...
	if (CVarLogCacheReduction.GetValueOnAnyThread() != 0)
	{
		...
		UE_LOGF(LogGeometryCollectionCache, Log, "Geometry Collection - Processed Raw Capture");
		UE_LOGF(LogGeometryCollectionCache, Log, "    Begin       : %d Particles", NumTotalParticles);
		UE_LOGF(LogGeometryCollectionCache, Log, "    End         : %d Particles", NumTotalParticles - NumRemovedParticles);
		UE_LOGF(LogGeometryCollectionCache, Log, "    Removed     : %d Particles", NumRemovedParticles);
		UE_LOGF(LogGeometryCollectionCache, Log, "    Size Before : %ls KB", *FText::AsNumber(ArchiveBeforeSize / 1024, &Opts).ToString());
		UE_LOGF(LogGeometryCollectionCache, Log, "    Size After  : %ls KB", *FText::AsNumber(ArchiveAfterSize / 1024, &Opts).ToString());
		UE_LOGF(LogGeometryCollectionCache, Log, "    Saved       : %ls KB", *FText::AsNumber((ArchiveBeforeSize - ArchiveAfterSize) / 1024, &Opts).ToString());
		UE_LOGF(LogGeometryCollectionCache, Log, "    On average %.3f removed per tick.", (float)NumRemovedParticles / (float)NumRecords);
		UE_LOGF(LogGeometryCollectionCache, Log, "-------------------------------------------");
	}
