...
namespace GeometryCollection::Facades
{
	...
	void FVertexBoneWeightsFacade::DefineSchema()
	{
		...
		// SkeletalMesh is Dataflow editor-time scratch and must not persist: it holds
		// TObjectPtr<UObject>, which a base FArchive (e.g. the DDC cooker's FMemoryWriter)
		// cannot serialize. Drop any legacy persistent entry first so the re-Add lands
		// with bSaved=false.
		if (SkeletalMeshAttribute.IsPersistent())
		{
			SkeletalMeshAttribute.Remove();
		}
		SkeletalMeshAttribute.Add(ManageArrayAccessor::EPersistencePolicy::KeepExistingPersistence);

		GeometryLODAttribute.Add();
		
		if (!BoneIndicesAttribute.IsValid())
		{
			UE_LOGF(LogDataflowVertexBoneWeightsFacade, Warning, "FVertexBoneWeightsFacade failed to initialize because '%ls' attribute from '%ls' group expected type TArray<int32>.", *FVertexBoneWeightsFacade::BoneIndicesAttributeName.ToString(), *FGeometryCollection::VerticesGroup.ToString());
		}
		if (!BoneWeightsAttribute.IsValid())
		{
			UE_LOGF(LogDataflowVertexBoneWeightsFacade, Warning, "FVertexBoneWeightsFacade failed to initialize because '%ls' attribute from '%ls' group expected type TArray<float>.", *FVertexBoneWeightsFacade::BoneWeightsAttributeName.ToString(), *FGeometryCollection::VerticesGroup.ToString());
		}
		if (!KinematicWeightAttribute.IsValid())
		{
			UE_LOGF(LogDataflowVertexBoneWeightsFacade, Warning, "FVertexBoneWeightsFacade failed to initialize because '%ls' attribute from '%ls' group expected type <float>.", *FVertexBoneWeightsFacade::KinematicWeightAttributeName.ToString(), *FGeometryCollection::VerticesGroup.ToString());
		}
		if (!SkeletalMeshAttribute.IsValid())
		{
			UE_LOGF(LogDataflowVertexBoneWeightsFacade, Warning, "FVertexBoneWeightsFacade failed to initialize because '%ls' attribute from '%ls' group expected type <TObjectPtr<UObject>>.", *FVertexBoneWeightsFacade::SkeletalMeshAttributeName.ToString(), *FGeometryCollection::GeometryGroup.ToString());
		}
		if (!GeometryLODAttribute.IsValid())
		{
			UE_LOGF(LogDataflowVertexBoneWeightsFacade, Warning, "FVertexBoneWeightsFacade failed to initialize because '%ls' attribute from '%ls' group expected type <int32>.", *FVertexBoneWeightsFacade::GeometryLODAttributeName.ToString(), *FGeometryCollection::GeometryGroup.ToString());
		}
	}
	...
	void FVertexBoneWeightsFacade::ModifyBoneWeight(int32 VertexIndex, const TArray<int32>& VertexBoneIndices, const TArray<float>& VertexBoneWeights)
	{
		...
		{ 
			...
			if (IndicesArray.IsValidIndex(VertexIndex) && WeightsArray.IsValidIndex(VertexIndex))
			{
				...
				{
					UE_LOGF(LogChaos, Warning, "FVertexBoneWeightsFacade::ModifyBoneWeight: Bone weight sum %f is not 1 on vertex %d", TotalWeight, VertexIndex);
				}
			}
		}
	}
				...
				if (0 <= BoneIndex && BoneIndex < NumBones())
				{
					for (int32 LocalVertex = 0; LocalVertex < VertexIndices.Num(); ++LocalVertex)
					{
						...
						if (0 <= VertexIndex && VertexIndex < NumVertices() && !IndicesArray[VertexIndex].Contains(BoneIndex))
						{
							...
							{
								UE_LOGF(LogChaos, Warning, "Bone weight sum %f exceeds 1 on vertex %d", TotalWeights[VertexIndex] + VertexWeight, VertexIndex);
							}
						}
					}
				}
