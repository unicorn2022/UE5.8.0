...
namespace GeometryCollection::Facades
{
	...
	TArray<int32> FCollectionTransformSelectionFacade::SelectRandom(bool bDeterministic, int32 RandomSeed, float RandomThresholdVal) const
	{
		TArray<int32> OutSelection;
	...
	TArray<int32> FCollectionTransformSelectionFacade::SelectRandom(const int32 NumElems, const bool bDeterministic, const int32 RandomSeed, const float RandomThresholdVal) const
	{
		TArray<int32> OutSelection;
	...
	static void RandomShuffleArray(TArray<int32>& InArray, bool Deterministic, int32 RandomSeed)
	{
		FRandomStream Stream(RandomSeed);
	...
	void FCollectionTransformSelectionFacade::SelectByPercentage(TArray<int32>& InOutSelection, int32 InPercentage, bool Deterministic, int32 RandomSeed)
	{
		RandomShuffleArray(InOutSelection, Deterministic, RandomSeed);

		if (!FMath::IsWithinInclusive<int32>(InPercentage, 0, 100))
		{
			InPercentage = 100;
		}
	...
	void FCollectionTransformSelectionFacade::SelectVerticesInTransformedBox(const FBox& InBox, const FTransform& InBoxTransform, TArray<bool>& OutVertexSelection) const
	{
		const FCollectionMeshFacade MeshFacade(ConstCollection);
		const FCollectionTransformFacade TransformFacade(ConstCollection);

		OutVertexSelection.Reset();
		if (!MeshFacade.IsValid() || !TransformFacade.IsValid())
		{
			return;
		}
		const int32 NumVertices = MeshFacade.VertexAttribute.Num();
		OutVertexSelection.Reserve(NumVertices);

		const TArray<FTransform> CollectionSpaceTransforms = TransformFacade.ComputeCollectionSpaceTransforms();

		const FVector BoxCenter = InBox.GetCenter();
		const FVector BoxExtentsScaled = InBox.GetExtent() * InBoxTransform.GetScale3D();
		const FBox ScaledBoxCenterAtZero(-BoxExtentsScaled, BoxExtentsScaled);

		for (int32 VertexIdx = 0; VertexIdx < NumVertices; ++VertexIdx)
		{
			const int32 TransformIdx = MeshFacade.BoneMapAttribute.IsValidIndex(VertexIdx) ? MeshFacade.BoneMapAttribute[VertexIdx] : INDEX_NONE;

			bool bIsSelected = false;
			if (CollectionSpaceTransforms.IsValidIndex(TransformIdx))
			{
				const FVector VertexInBoneSpace = FVector(MeshFacade.VertexAttribute[VertexIdx]);
				const FVector VertexInCollectionSpace = CollectionSpaceTransforms[TransformIdx].TransformPosition(VertexInBoneSpace);
				const FVector VertexInBoxSpace = InBoxTransform.InverseTransformPositionNoScale(VertexInCollectionSpace - BoxCenter);

				bIsSelected = ScaledBoxCenterAtZero.IsInside(VertexInBoxSpace);
			}
			OutVertexSelection.Add(bIsSelected);
		}
	}
	
	/* --------------------------------------------------------------------------------------------------------------------------- */

	void FCollectionTransformSelectionFacade::SelectVerticesInTransformedBox(const FBox& InBox, const FTransform& InBoxTransform, TBitArray<>& OutVertexSelection) const
	{
		const FCollectionMeshFacade MeshFacade(ConstCollection);
		const FCollectionTransformFacade TransformFacade(ConstCollection);
		...
		const int32 NumVertices = MeshFacade.VertexAttribute.Num();
		OutVertexSelection.Reserve(NumVertices);
		OutVertexSelection.SetNum(NumVertices, false);
		...
		for (int32 VertexIdx = 0; VertexIdx < NumVertices; ++VertexIdx)
		{
			...
			OutVertexSelection[VertexIdx] = bIsSelected;
		}
	}

	/* --------------------------------------------------------------------------------------------------------------------------- */

	void FCollectionTransformSelectionFacade::SelectVerticesInSphere(const FSphere& InSphere, TArray<bool>& OutVertexSelection) const
	{
		const FCollectionMeshFacade MeshFacade(ConstCollection);
		const FCollectionTransformFacade TransformFacade(ConstCollection);
		...
		for (int32 VertexIdx = 0; VertexIdx < NumVertices; ++VertexIdx)
		{
			...
			if (CollectionSpaceTransforms.IsValidIndex(TransformIdx))
			{
				...
				bIsSelected = InSphere.IsInside(VertexInCollectionSpace);
			}
			OutVertexSelection.Add(bIsSelected);
		}
	}
	...
	void FCollectionTransformSelectionFacade::SelectVerticesInSphere(const FSphere& InSphere, TBitArray<>& OutVertexSelection) const
	{
		const FCollectionMeshFacade MeshFacade(ConstCollection);
		const FCollectionTransformFacade TransformFacade(ConstCollection);
	...
	void FCollectionTransformSelectionFacade::SelectVerticesOnPlaneSide(const FPlane& InPlane, bool bUsePositiveSide, TArray<bool>& OutVertexSelection) const
	{
		const FCollectionMeshFacade MeshFacade(ConstCollection);
		const FCollectionTransformFacade TransformFacade(ConstCollection);
		...
		for (int32 VertexIdx = 0; VertexIdx < NumVertices; ++VertexIdx)
		{
			...
			if (CollectionSpaceTransforms.IsValidIndex(TransformIdx))
			{
				...
				const double DistanceFromPlane = InPlane.PlaneDot(VertexInCollectionSpace);
				bIsSelected = bUsePositiveSide
					? (DistanceFromPlane >= 0)
					: (DistanceFromPlane <= 0);
			}
			OutVertexSelection.Add(bIsSelected);
	...
	void FCollectionTransformSelectionFacade::SelectVerticesOnPlaneSide(const FPlane& InPlane, bool bUsePositiveSide, TBitArray<>& OutVertexSelection) const
	{
		const FCollectionMeshFacade MeshFacade(ConstCollection);
		const FCollectionTransformFacade TransformFacade(ConstCollection);
	...
	void FCollectionTransformSelectionFacade::GetVertexToFaceIndicesArray(TArray<TArray<int32>>& VertexToFaceIndices) const
	{
		if (IndicesAttribute.IsValid())
		{
			...
			for (int32 FaceIdx = 0; FaceIdx < NumFaces; ++FaceIdx)
			{
				if (Indices.IsValidIndex(FaceIdx))
				{
					VertexToFaceIndices[Indices[FaceIdx].X].AddUnique(FaceIdx);
					VertexToFaceIndices[Indices[FaceIdx].Y].AddUnique(FaceIdx);
					VertexToFaceIndices[Indices[FaceIdx].Z].AddUnique(FaceIdx);
	...
	{
		TArray<int32> OutSelectedCurves;
		...
		if (CurveFacade.IsValid())
		{
			...
			TBitArray<> SelectedCurves;
			SelectedCurves.Init(false, NumCurves);
			const TArray<int32>& CurveIndicesByPoint = CurveFacade.GetPointCurveIndices();

			for (const int32& VertexIndex : InVertexSelection)
			{
				// there's two vertices for each point
				const int32 PointIndex = (VertexIndex >> 1);
				if (CurveIndicesByPoint.IsValidIndex(PointIndex))
				{
					const int32 CurveIndex = CurveIndicesByPoint[PointIndex];
					if (SelectedCurves.IsValidIndex(CurveIndex) && !SelectedCurves[CurveIndex])
					{
						OutSelectedCurves.Add(CurveIndex);
						SelectedCurves[CurveIndex] = true;
					}
				}
			}

			// let's filter the selected curves further with a more expensive check if needed
			if (bAllElementsMustBeSelected)
			{
				// Convert vertex index array into an O(1) addressable storage
				TBitArray<> SelectedVertices;
				SelectedVertices.Init(false, CurveFacade.GetPointRestOrientations().Num() * 2);
				for (const int32& VertexIndex : InVertexSelection)
				{
					if (SelectedVertices.IsValidIndex(VertexIndex))
					{
						SelectedVertices[VertexIndex] = true;
					}
				}

				TArray<int32> PotentiallySelectedCurves = MoveTemp(OutSelectedCurves);
				OutSelectedCurves.Reserve(PotentiallySelectedCurves.Num());
				for (int32 CurveIdx : PotentiallySelectedCurves)
				{
					const int32 PointIndexStart = CurveIdx == 0 ? 0 : CurveFacade.GetCurvePointOffsets()[CurveIdx - 1];
					const int32 PointIndexEnd = CurveFacade.GetCurvePointOffsets()[CurveIdx];
					...
					{
						if (SelectedVertices.IsValidIndex(VertexIdx))
						{
							if (!SelectedVertices[VertexIdx])
							{
								bAllIncluded = false;
								break;
							}
					...
					if (bAllIncluded)
					{
						OutSelectedCurves.Add(CurveIdx);
					}
				}
			}
		}
		return OutSelectedCurves;
	}

	void FCollectionTransformSelectionFacade::ConvertVertexSelectionToCurveSelection(const TBitArray<>& InVertexSelection, bool bAllElementsMustBeSelected, TBitArray<>& OutCurveSelection) const
	{
		GeometryCollection::Facades::FCollectionCurveGeometryFacade CurveFacade(ConstCollection);
		if (CurveFacade.IsValid())
		{
			const int32 NumCurves = CurveFacade.GetNumCurves();
			OutCurveSelection.Init(false, NumCurves);

			const TArray<int32>& CurveIndicesByPoint = CurveFacade.GetPointCurveIndices();

			const int32 NumVertices = InVertexSelection.Num();
			for (int32 VertexIndex = 0; VertexIndex < NumVertices; ++VertexIndex)
			{
				if (InVertexSelection[VertexIndex])
				{
					// there's two vertices for each point
					const int32 PointIndex = (VertexIndex >> 1);
					if (CurveIndicesByPoint.IsValidIndex(PointIndex))
					{
						const int32 CurveIndex = CurveIndicesByPoint[PointIndex];
						if (OutCurveSelection.IsValidIndex(CurveIndex) && !OutCurveSelection[CurveIndex])
						{
							OutCurveSelection[CurveIndex] = true;
						}
					}
				}
			}
			...
			if (bAllElementsMustBeSelected)
			{
				for (int32 CurveIdx = 0; CurveIdx < NumCurves; ++CurveIdx)
				{
					...
					for (int32 VertexIdx = VertexIndexStart; VertexIdx < VertexIndexEnd; ++VertexIdx)
					{
						if (InVertexSelection.IsValidIndex(VertexIdx))
						{
							if (!InVertexSelection[VertexIdx])
							{
								OutCurveSelection[CurveIdx] = false;
								break;
							}
						}
					}
