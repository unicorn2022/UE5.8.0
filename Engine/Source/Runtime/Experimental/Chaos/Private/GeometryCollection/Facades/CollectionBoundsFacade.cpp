...
namespace GeometryCollection::Facades
{
	...
	TArray<FVector> FBoundsFacade::GetTransformCentroidsInCollectionSpace() const
	{
		TArray<FVector> Centroids;
		if (IsValid())
		{
			const int32 NumTransforms = TransformToGeometryIndexAttribute.Num();
			Centroids.Init(FVector::ZeroVector, NumTransforms);
			...
			if (TransformFacade.IsValid())
			{
				...
				for (int32 TransformIndex = 0; TransformIndex < NumTransforms; ++TransformIndex)
				{
					...
					if (BoundingBoxAttribute.IsValidIndex(GeoIndex))
					{
						...
						Centroids[TransformIndex] = CollectionSpaceTransform.TransformPosition(GeoBoundingBox.GetCenter());
					}
				}
			}
		}
