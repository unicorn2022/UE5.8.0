...
namespace GeometryCollection::Facades
{
	...
	// Curves attributes facade 
	const FName FCollectionCurveAttributesFacade::CurveCardGroupsAttribute("CardGroups");
	...
	void FCollectionCurveGeometryFacade::GetCurvePointPositions(const int32 CurveIndex, TArray<FVector3f>& OutPoints) const
	{
		OutPoints.Reset();
		...
		const int32 StartPointIndex = (CurveIndex == 0) ? 0 : CurvePointOffsets[CurveIndex-1];
		const int32 EndPointIndex = CurvePointOffsets[CurveIndex];
		const int32 NumPoints = (EndPointIndex - StartPointIndex);
		OutPoints.Reserve(NumPoints);
		for (int32 Index = StartPointIndex; Index < EndPointIndex; ++Index)
		{
			OutPoints.Add(PointRestPositions[Index]);
		}
	}

	void FCollectionCurveGeometryFacade::GetCurveResampledPositions(const int32 CurveIndex, int32 NumSamples, TArray<FVector3f>& OutPoints) const
	{
		OutPoints.Reset();
		...
		GetCurvePointPositions(CurveIndex, Positions);
		if (Positions.Num() < 2)
		{
			return;
		}
		...
		// compute lengths
		TArray<double> DistancesAt;
		DistancesAt.Init(0, Positions.Num());

		for (int32 PointIndex = 1; PointIndex < Positions.Num(); ++PointIndex)
		{
			const double SegmentSize = (Positions[PointIndex] - Positions[PointIndex - 1]).Size();
			DistancesAt[PointIndex] = SegmentSize + DistancesAt[PointIndex - 1];
		}

		const double TotalCurveLength = DistancesAt.Last();
		...
		const double SamplingLength = TotalCurveLength / (double)(NumSamples - 1);

		OutPoints.Add(Positions[0]);

		double SampledDistance = SamplingLength;;
		int32 PointIndex = 0;
		while (PointIndex < DistancesAt.Num())
		{
			// find which segment this belong 
			int NextIndex = (PointIndex + 1);
			while (NextIndex < DistancesAt.Num())
			{
				if (SampledDistance < DistancesAt[NextIndex])
				{
					const double RelativeDistance = SampledDistance - DistancesAt[NextIndex - 1];
					const double SegmentLength = DistancesAt[NextIndex] - DistancesAt[NextIndex - 1];
					const double Alpha = (SegmentLength > 0) ? (RelativeDistance / SegmentLength) : 0;
					const FVector3f InterpolatedPos = FMath::Lerp(Positions[NextIndex - 1], Positions[NextIndex], Alpha);
					OutPoints.Add(InterpolatedPos);

					SampledDistance += SamplingLength;
					break;
				}
				++NextIndex;
			}
			...
			// we may have reach the end or gone too far
			if (SampledDistance >= DistancesAt.Last()/* && NumSamples > OutPoints.Num()*/)
			{
				if (OutPoints.Num() < NumSamples)
				{
					OutPoints.Add(Positions.Last());
				}
				break;
			}
		}
	...
	FCollectionCurveAttributesFacade::FCollectionCurveAttributesFacade(FManagedArrayCollection& InCollection) :
		ConstCollection(InCollection), Collection(&InCollection),
		CurveCardGroups(InCollection, CurveCardGroupsAttribute, FCollectionCurveGeometryFacade::CurvesGroup)
	{
		DefineSchema();
	}
	
	FCollectionCurveAttributesFacade::FCollectionCurveAttributesFacade(const FManagedArrayCollection& InCollection) :
		ConstCollection(InCollection), Collection(nullptr),
		CurveCardGroups(InCollection, CurveCardGroupsAttribute, FCollectionCurveGeometryFacade::CurvesGroup)
	{
	}
	
	bool FCollectionCurveAttributesFacade::IsValid() const
	{
		return CurveCardGroups.IsValid();
	}
	
	void FCollectionCurveAttributesFacade::DefineSchema()
	{
		check(!IsConst());
		CurveCardGroups.Add();
	}
