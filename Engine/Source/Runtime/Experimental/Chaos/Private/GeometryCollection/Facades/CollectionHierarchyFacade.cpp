...
namespace Chaos::Facades
{
	...
	int32 FCollectionHierarchyFacade::ComputeLevelAttributeMax() const
	{
		if (LevelAttribute.IsValid())
		{
			const TManagedArray<int32>& Level = LevelAttribute.Get();

			int32 LevelMax = -1;
			for (int Index = 0; Index < Level.Num(); Index++)
			{
				if (Level[Index] > LevelMax)
				{
