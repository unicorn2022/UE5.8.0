...
namespace GeometryCollection::Facades
{
	...
	bool FMuscleActivationFacade::SetUpMuscleActivation(const TArray<int32>& InOrigin, const TArray<int32>& Insertion, float InContractionVolumeScale)
	{
		...
		if (InOrigin.Num() == 0)
		{
			UE_LOGF(LogDataflowMuscleActivationFacade, Error, "Muscle activation setup failed: No origins given");
			return false;
		}
		if (Insertion.Num() == 0)
		{
			UE_LOGF(LogDataflowMuscleActivationFacade, Error, "Muscle activation setup failed: No insertions given");
			return false;
		}
		if (!ConstCollection.FindAttribute<FVector3f>("Vertex", FGeometryCollection::VerticesGroup))
		{
			UE_LOGF(LogDataflowMuscleActivationFacade, Error, "Muscle activation setup failed: Collection has no Vertex attribute");
			return false;
		}
		if (!ConstCollection.FindAttribute<FIntVector4>("Tetrahedron", "Tetrahedral"))
		{
			UE_LOGF(LogDataflowMuscleActivationFacade, Error, "Muscle activation setup failed: Collection has no Tetrahedron attribute");
			return false;
		}
		if (!ConstCollection.FindAttribute<FVector3f>("FiberDirection", "Tetrahedral"))
		{
			UE_LOGF(LogDataflowMuscleActivationFacade, Error, "Muscle activation setup failed: Collection has no FiberDirection attribute");
			return false;
		}
		...
		if (OverlapCount)
		{
			UE_LOGF(LogDataflowMuscleActivationFacade, Error, "Muscle activation setup failed: origins and insertions have %d common indices out of total %d, please check if they are from different sources.", OverlapCount, InOrigin.Num());
			if (TransformIndex && BoneName)
			{
				for (const int32 GeomIdx : OverlapGeometries)
				{
					...
					{
						UE_LOGF(LogDataflowMuscleActivationFacade, Error, "Overlapped origins and insertions are from transform indexed %d, bone name %ls.", (*TransformIndex)[GeomIdx], *(*BoneName)[(*TransformIndex)[GeomIdx]]);
					}
				}
			}
			return false;
		...
		{
			UE_LOGF(LogDataflowMuscleActivationFacade, Warning, "Geometry %d has only insertions but no origins.", ComponentIdx);
		}
