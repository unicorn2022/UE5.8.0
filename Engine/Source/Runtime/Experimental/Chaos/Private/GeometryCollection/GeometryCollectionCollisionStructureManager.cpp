	...
	const int32 CollisionParticlesMaxInput)
{
	...
	if (Implicit || Vertices.Size())
	{
		...
		UE_LOGF(LogChaos, Log, "NewSimplicial: InitialSize: %d, ImplicitExterior: %d, FullCopy: %d, FinalSize: %d", IndicesArray.Num(), LSVCounter, (int32)bFullCopy, NumParticles);
		return Simplicial;
	}
	UE_LOGF(LogChaos, Log, "NewSimplicial::Empty");
	return Simplicial;
}
