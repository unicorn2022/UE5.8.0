...
namespace GeometryCollectionAlgo
{
	...
	)
	{
		...
		UE_LOGF(GeometryCollectionAlgoLog, Log, "%ls", *Buffer);
	...
	void TriangulateBoundaries(FGeometryCollection* GeometryCollection, const TArray<TArray<TArray<int32>>> &BoundaryVertexIndices, bool bWoundClockwise, float MinTriangleAreaSq)
	{
		...
		for (int32 GeometryIdx = 0; GeometryIdx < NumGeometries; GeometryIdx++)
		{
			...
			for (const TArray<int32>& Boundary : GeomBoundaries)
			{
				...
				// Note: technically this is 4x the squared area, but matches how Chaos generally filters degenerate triangles
				auto GetAreaSq = [GeometryCollection](int32 A, int32 B, int32 C) -> double
				{
					...
					return Cross.SizeSquared();
				};

				// Try to find a start index that isn't a locally degenerate/collinear chain (if we have a positive degenerate tri threshold)
				int32 UseStartIdx = 0;
				if (MinTriangleAreaSq > 0 && Boundary.Num() > 3)
				{
					int32 NumLocalNonCollinear = 0;
					for (int32 Prev2 = Boundary.Num() - 2, Prev = Boundary.Num() - 1, Idx = 0; Idx < Boundary.Num(); Prev2 = Prev, Prev = Idx++)
					{
						if (GetAreaSq(Boundary[Prev2], Boundary[Prev], Boundary[Idx]) < MinTriangleAreaSq)
						{
							NumLocalNonCollinear = 0;
						}
						else
						{
							++NumLocalNonCollinear;
						}
						if (NumLocalNonCollinear == 3)
						{
							UseStartIdx = Prev2;
							break;
						}
				...
				int32 First = Boundary[UseStartIdx];
				
				for (int32 BoundaryIdx = 1; BoundaryIdx + 1 < Boundary.Num(); BoundaryIdx++)
				{
					int32 A = First, B = Boundary[(UseStartIdx + BoundaryIdx) % Boundary.Num()], C = Boundary[(UseStartIdx + BoundaryIdx + 1) % Boundary.Num()];

					if (MinTriangleAreaSq > 0)
					{
						if (GetAreaSq(A, B, C) < MinTriangleAreaSq)
						{
							continue;
						}
					}
	...
	void PrintStatistics(const FGeometryCollection* GeometryCollection)
	{
		...
		Buffer += FString::Printf(TEXT("------------------------------------------------------------\n\n"));
		UE_LOGF(LogGeometryCollectionClean, Log, "%ls", *Buffer);
	}
