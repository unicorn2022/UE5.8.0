...
)
{
	...
	ParallelFor(NumBones, [&](int32 Idx)
	{
		...
		else if (SimulationType[Idx] == RigidType && GeomIdx != INDEX_NONE)
		{
			...
			{
				bool bAttemptDecomposition = OptionalDecompositionSettings->MaxHullsPerGeometry > 1 || OptionalDecompositionSettings->ErrorTolerance > 0.0 || OptionalDecompositionSettings->bProtectNegativeSpace || OptionalDecompositionSettings->CustomEmptySpace;
				...
				if (bAttemptDecomposition)
				{
					...
					if (OptionalDecompositionSettings->bProtectNegativeSpace || OptionalDecompositionSettings->CustomEmptySpace)
					{
						...
						if (OptionalDecompositionSettings->CustomEmptySpace)
						{
							// Get the matrix transform from CustomEmptySpace to bone-local space
							FMatrix ToBoneLocal = OptionalDecompositionSettings->CustomEmptySpaceWorldTransform.ToMatrixWithScale() * GlobalTransformArray[Idx].ToInverseMatrixWithScale();
							// conservatively scale radii in the non-uniform case, since we don't want the spheres to overlap the input geometry
							double ToBoneLocalScale = ToBoneLocal.GetMinimumAxisScale();
							Decomposition.AddCoveringToInitialNegativeSpace(*OptionalDecompositionSettings->CustomEmptySpace, ToBoneLocal, ToBoneLocalScale);
						}
...
void FGeometryCollectionConvexUtility::GenerateClusterConvexHullsFromLeafOrChildrenHullsInternal(FGeometryCollection& Collection, const FClusterConvexHullSettings& Settings, bool bOnlySubset, bool bUseDirectChildren, const TArrayView<const int32> TransformSubset)
{
	...
	for (int32 TransformIndex : TransformsToProcess)
	{
		...
		{
			UE_LOGF(LogChaos, Warning, "Transform Index %d out of bounds [0,%d)", TransformIndex, SimulationTypeAttribute.Num());
			continue;
		}
	...
	const FManagedArrayCollection& SourceCollection, const TArray<int32>& SourceTransformIndices, bool bSkipIfEmpty)
{
	...
	if (TransformIndices.Num() != SourceTransformIndices.Num())
	{
		UE_LOGF(LogChaos, Warning,
			"Attempted to copy convex hulls failed because the source and target transform indices array did not have the same length (%d vs %d)", 
	...
	{
		UE_LOGF(LogChaos, Warning,
			"Failed to copy convex hulls: Hull attributes not found on source collection");
		return false;
	}
	...
	for (int32 ToTransferIdx = 0; ToTransferIdx < TransformIndices.Num(); ++ToTransferIdx)
	{
		...
		{
			UE_LOGF(LogChaos, Warning,
				"Invalid source transform index: %d",
				FromTransformIdx);
			continue;
		}
...
void FGeometryCollectionConvexUtility::SetVolumeAttributes(FManagedArrayCollection* Collection)
{
	...
	ParallelFor(GeoVolumes.Num(), [&](int32 GeoIdx)
	{
		...
		{
			float ComputedVolume = (float)ComputeGeometryVolume(Collection, GeoIdx, Transforms[Bone], 1.0);
			// if a mesh has inverted orientation the volume will be negative, but we take the abs() here so it's more usable as a size metric
			GeoVolumes[GeoIdx] = FMath::Abs(ComputedVolume);
		}
	});
