	...
	void FCCDManager::ProjectCCD(const FReal Dt, TArrayView<FPBDCollisionConstraint* const> SweptConstraints)
	{
		...
		while (!bSolved && (NumIterations < MaxIterations))
		{
			...
			for (FPBDCollisionConstraint* Constraint : Constraints)
			{
				...
				FReal TargetSeparation = 0;
				if (const FManifoldPoint* ManifoldPoint = Constraint->GetClosestManifoldPoint())
				{
					TargetSeparation = ManifoldPoint->TargetPhi;
				}
				const FReal MinPhi = TargetSeparation - PhiTolerance;
