...
namespace Chaos
{
	namespace
	{
		...
		// Aim is to prevent re-ordering of operatioins when performing a remove and add in the same frame
		bool bFixRemoveDuringSort = true;
		FAutoConsoleVariableRef CVarChaosFixRemoveDuringSort(
			TEXT("p.Chaos.ClusterUnion.FixRemoveDuringSort"),
			bFixRemoveDuringSort,
			TEXT("Fix remove to top during sort.")
		);
		...
		if (!PendingClusterIndexOperations.IsEmpty())
		{
			for (TPair<FClusterUnionIndex, FClusterOpMap>& OpMap : PendingClusterIndexOperations)
			{
				...
				OpMap.Value.KeyStableSort(
					...
					{
						// remove must be bubbled up to the top (for the case when remove and add are used at the same time)
						if (bFixRemoveDuringSort)
						{
							if (A == EClusterUnionOperation::Remove)
							{
								return true;
							}
							else if (B == EClusterUnionOperation::Remove)
							{
								return false;
							}
						}
		...
		for (FPBDRigidParticleHandle* Handle : ParticleSet)
		{
			...
			if (FPBDRigidClusteredParticleHandle* ClusterHandle = Handle->CastToClustered())
			{
				...
				if (bChaosClusterUnionDoNotAddEmptyClusters)
				{
					...
					{
						UE_LOGF(LogChaos, Verbose, "FClusterUnionManager::HandleAddOperation rejecting particle because it is 'broken' %ls", *ClusterHandle->GetDebugName());
						continue;
					}
				}
