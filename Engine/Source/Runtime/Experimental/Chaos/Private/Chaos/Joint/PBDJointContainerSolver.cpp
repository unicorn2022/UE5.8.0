...
namespace Chaos
{
	namespace Private
	{
		...
		template <typename JointSolverType>
		void TPBDJointContainerSolver<JointSolverType>::ResetImpl(const int32 InMaxConstraints)
		{
		...
		template <typename JointSolverType>
		bool TPBDJointContainerSolver<JointSolverType>::SortConstraintsByPartitionIndex(const TArray<int32>& PartitionIndices, const TArray<int32>& PrefixSum)
		{
			Private::SortArrayByPartitionIndex(ContainerIndices, PartitionIndices, PrefixSum);
			return true;
		}

		template <typename JointSolverType>
		void TPBDJointContainerSolver<JointSolverType>::GetAllConstrainedParticles(TArray<const FGeometryParticleHandle*>& ParticleHandles) const
		{
			ParticleHandles.Reset();
			ParticleHandles.Reserve(ContainerIndices.Num() * 2);

			for (int32 Index : ContainerIndices)
			{
				const FParticlePair& ParticlePair = ConstraintContainer.GetConstrainedParticles(Index);
				ParticleHandles.Add(ParticlePair[0]);
				ParticleHandles.Add(ParticlePair[1]);
			}
		}
		...
		template<typename SolverType>
		void ApplyPositionConstraintsImpl(const TPBDJointContainerSolver<SolverType>& Container, TArray<SolverType>& Solvers, const TArray<int32>& SolverGlobalIndices, 
			const FReal Dt, const int32 It, const int32 NumIts, const int32 BeginIndex, const int32 EndIndex)
		{
			...
			for (int32 SolverConstraintIndex = FMath::Max(0, BeginIndex); SolverConstraintIndex < FMath::Min(EndIndex, Solvers.Num()); ++SolverConstraintIndex)
			{
				SolverType& Solver = Solvers[SolverConstraintIndex];
		...
		{
			// ResizeSolverArrays() called in AddBodies() after adding all constraints and solvers.
			check(ConstraintSolvers.Num() == ContainerIndices.Num());
			ApplyPositionConstraints(Dt, It, NumIts, 0, GetNumConstraints());
		}

		template<typename SolverType>
		void TPBDJointContainerSolver<SolverType>::ApplyPositionConstraints(const FReal Dt, const int32 It, const int32 NumIts, const int32 BeginIndex, const int32 EndIndex)
		{
			// ResizeSolverArrays() called in AddBodies() after adding all constraints and solvers.
			check(ConstraintSolvers.Num() == ContainerIndices.Num());
 			ApplyPositionConstraintsImpl(*this, ConstraintSolvers, ContainerIndices, Dt, It, NumIts, BeginIndex, EndIndex);
		}
		...
		void ApplyVelocityConstraintsImpl(const TPBDJointContainerSolver<SolverType>& Container, TArray<SolverType>& Solvers, const FReal Dt, const int32 It, const int32 NumIts,
			const int32 BeginIndex, const int32 EndIndex)
		{
			const FPBDJointSolverSettings& Settings = Container.GetSettings();
			const FReal IterationStiffness = GetJointIterationStiffness(Settings, It, NumIts);
		...
		{
			ApplyVelocityConstraints(Dt, It, NumIts, 0, ConstraintSolvers.Num());
		}

		template<typename SolverType>
		void TPBDJointContainerSolver<SolverType>::ApplyVelocityConstraints(const FReal Dt, const int32 It, const int32 NumIts, const int32 BeginIndex, const int32 EndIndex)
		{
			ApplyVelocityConstraintsImpl(*this, ConstraintSolvers, Dt, It, NumIts, BeginIndex, EndIndex);
		}

		template<typename SolverType>
		void TPBDJointContainerSolver<SolverType>::PreApplyProjectionConstraints(const FReal Dt)
		{
			PreApplyProjectionConstraints(Dt, 0, ConstraintSolvers.Num());
		}

		template<typename SolverType>
		void TPBDJointContainerSolver<SolverType>::PreApplyProjectionConstraints(const FReal Dt, const int32 BeginIndex, const int32 EndIndex)
		{
			if (UseLinearSolver())
			{
				PreApplyLinearProjectionConstraints(Dt, BeginIndex, EndIndex);
			}
			else
			{
				PreApplyNonLinearProjectionConstraints(Dt, BeginIndex, EndIndex);
			}
		}
		...
		{
			ApplyProjectionConstraints(Dt, It, NumIts, 0, ConstraintSolvers.Num());
		}

		template<typename SolverType>
		void TPBDJointContainerSolver<SolverType>::ApplyProjectionConstraints(const FReal Dt, const int32 It, const int32 NumIts, const int32 BeginIndex, const int32 EndIndex)
		{
			if (UseLinearSolver())
			{
				ApplyLinearProjectionConstraints(Dt, It, NumIts, BeginIndex, EndIndex);
			}
			else
			{
				ApplyNonLinearProjectionConstraints(Dt, It, NumIts, BeginIndex, EndIndex);
			}
		}
		...
		template<>
		void TPBDJointContainerSolver<FPBDJointCachedSolver>::PreApplyLinearProjectionConstraints(const FReal Dt, const int32 BeginIndex, const int32 EndIndex)
		{
			...
			for (int32 SolverConstraintIndex = FMath::Max(0, BeginIndex); SolverConstraintIndex < FMath::Min(EndIndex, ConstraintSolvers.Num()); ++SolverConstraintIndex)
			{
		...
		template<>
		void TPBDJointContainerSolver<FPBDJointSolver>::PreApplyLinearProjectionConstraints(const FReal Dt, const int32 BeginIndex, const int32 EndIndex)
		{
		}

		template<>
		void TPBDJointContainerSolver<FPBDJointCachedSolver>::PreApplyNonLinearProjectionConstraints(const FReal Dt, const int32 BeginIndex, const int32 EndIndex)
		{
		}

		template<>
		void TPBDJointContainerSolver<FPBDJointSolver>::PreApplyNonLinearProjectionConstraints(const FReal Dt, const int32 BeginIndex, const int32 EndIndex)
		{
		}


		template<>
		void TPBDJointContainerSolver<FPBDJointCachedSolver>::ApplyLinearProjectionConstraints(const FReal Dt, const int32 It, const int32 NumIts, const int32 BeginIndex, const int32 EndIndex)
		{
			constexpr bool bUseSolveLinear = true;
			const FPBDJointSolverSettings& Settings = GetSettings();
		...
		template<>
		void TPBDJointContainerSolver<FPBDJointSolver>::ApplyLinearProjectionConstraints(const FReal Dt, const int32 It, const int32 NumIts, const int32 BeginIndex, const int32 EndIndex)
		{
		}

		template<>
		void TPBDJointContainerSolver<FPBDJointCachedSolver>::ApplyNonLinearProjectionConstraints(const FReal Dt, const int32 It, const int32 NumIts, const int32 BeginIndex, const int32 EndIndex)
		{
		}

		template<>
		void TPBDJointContainerSolver<FPBDJointSolver>::ApplyNonLinearProjectionConstraints(const FReal Dt, const int32 It, const int32 NumIts, const int32 BeginIndex, const int32 EndIndex)
		{
			const FPBDJointSolverSettings& Settings = GetSettings();
