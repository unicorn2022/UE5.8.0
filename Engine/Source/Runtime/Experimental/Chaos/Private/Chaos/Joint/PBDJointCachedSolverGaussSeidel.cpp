...
{
	// SIMD is only enabled when all 3 axes are limited and soft (see InitRotationConstraints).
	// Soft constraints should not apply velocity corrections. They are handled through
	// the damping term in the position solve phase instead. So call to ApplyAngularVelocityConstraintSimd is removed. 
	...
	const FReal Dtd)
{
	...
	// Check and apply MaxLambda 
	RotationDrives.ApplyMaxLambdaSIMD(DeltaLambda, RotationDrives.Simd.ConstraintLambda);
	...
	const FReal Dt)
{
	...
	if ((InMaxForce > 0) && (InMaxForce < UE_MAX_FLT))
	{
		...
		{
			FReal ConstraintIM = Data.ConstraintHardIM[ConstraintIndex];
			if (bUseSimd)
			{
				ConstraintIM = FReal(VectorGetComponentDynamic(Simd.ConstraintHardIM, ConstraintIndex));
			}
			if (ConstraintIM > UE_SMALL_NUMBER)
			{
				MaxLambda /= ConstraintIM;
			}
		}
...
void FAxisConstraintDatas::ApplyMaxLambdaSIMD(
	VectorRegister4Float& DeltaLambda,
	VectorRegister4Float& Lambda)
{
	// 1. Load MaxLambdas
	VectorRegister4Float MaxLambda = MakeVectorRegisterFloatFromDouble(MakeVectorRegister(ConstraintMaxLambda[0], ConstraintMaxLambda[1], ConstraintMaxLambda[2], 0.0f));

	// 2. Prepare -MaxLambda
	VectorRegister4Float NegMaxLambda = VectorNegate(MaxLambda);

	// 3. Create a mask where MaxLambda is greater than zero
	VectorRegister4Float Zero = VectorZero();
	VectorRegister4Float IsActiveMask = VectorCompareGT(MaxLambda, Zero);

	// 4. If (Lambda > MaxLambda) -> Clamp Top
	VectorRegister4Float TooHighMask = VectorCompareGT(Lambda, MaxLambda);

	// 5. else if (Lambda < -MaxLambda) -> Clamp Bottom
	VectorRegister4Float TooLowMask = VectorCompareLT(Lambda, NegMaxLambda);

	// 6. Calculate clamped Lambda values
	// FinalLambda = (Lambda > Max) ? Max : ((Lambda < -Max) ? -Max : Lambda)
	VectorRegister4Float ClampedLambda = VectorSelect(TooHighMask, MaxLambda, Lambda);
	ClampedLambda = VectorSelect(TooLowMask, NegMaxLambda, ClampedLambda);

	// 7. Apply check (ConstraintMaxLambda > 0)
	VectorRegister4Float OriginalLambda = Lambda;
	Lambda = VectorSelect(IsActiveMask, ClampedLambda, Lambda);

	// 8. Adjust DeltaLambda
	DeltaLambda = VectorAdd(DeltaLambda, VectorSubtract(Lambda, OriginalLambda));
}
