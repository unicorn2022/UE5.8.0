...
namespace Chaos
{
		...
		const FSolverBodyContainer& SolverBodyContainerB)
	{
		for (int32 ConstraintIndex = 0; ConstraintIndex < SolverA.GetNumConstraints(); ++ConstraintIndex)
		{
			...
			{
				UE_LOGF(LogChaos, Warning, "ManifoldPoint count mismatch");
				return;
			}
			...
			for (int32 ManifoldPointIndex = 0; ManifoldPointIndex < ConstraintSolverA.NumManifoldPoints(); ++ManifoldPointIndex)
			{
				...
				if (bIsError)
				{
					UE_LOGF(LogChaos, Warning, "SimdCollisionSolver mismatch");
				}
			}
		}
		...
		{
			UE_LOGF(LogChaos, Warning, "Body count mismatch");
			return;
		}
		...
		for (int BodyIndex = 0; BodyIndex < SolverBodyContainerA.Num(); ++BodyIndex)
		{
			...
			if (BodyA.P() != BodyB.P())
			{
				UE_LOGF(LogChaos, Warning, "Position mismatch");
				bIsError = true;
			}

			if (BodyA.Q() != BodyB.Q())
			{
				UE_LOGF(LogChaos, Warning, "Rotation mismatch");
				bIsError = true;
			}

			if (BodyA.DP() != BodyB.DP())
			{
				UE_LOGF(LogChaos, Warning, "PositionDelta mismatch");
				bIsError = true;
			}

			if (BodyA.DQ() != BodyB.DQ())
			{
				UE_LOGF(LogChaos, Warning, "RotationDelta mismatch");
				bIsError = true;
			}

			if (BodyA.V() != BodyB.V())
			{
				UE_LOGF(LogChaos, Warning, "Velocity mismatch");
				bIsError = true;
			}

			if (BodyA.W() != BodyB.W())
			{
				UE_LOGF(LogChaos, Warning, "AngVel mismatch");
				bIsError = true;
			}

			if (BodyA.InvM() != BodyB.InvM())
			{
				UE_LOGF(LogChaos, Warning, "InvM mismatch");
				bIsError = true;
			}

			if (BodyA.InvI() != BodyB.InvI())
			{
				UE_LOGF(LogChaos, Warning, "InvI mismatch");
				bIsError = true;
			}
	...
	{
		// Does nothing for collision constraints
	}
