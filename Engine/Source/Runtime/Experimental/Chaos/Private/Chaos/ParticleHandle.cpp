...
namespace Chaos
{
	...
	{
		// We always have a union at the root. Add one if these are the first objects to be added
		if (MNonFrequentData.Read().GetGeometry() == nullptr)
		{
			Chaos::FImplicitObjectPtr Union = MakeImplicitObjectPtr<Chaos::FImplicitObjectUnion>(MoveTemp(Objects));
			SetGeometry(Union);
			return;
		}

		// we only support FImplicitObjectUnion
		check(MNonFrequentData.Read().GetGeometry());
		if (ensure(MNonFrequentData.Read().GetGeometry()->GetType() == FImplicitObjectUnion::StaticType()))
		{
