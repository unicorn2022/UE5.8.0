...
FSolverReal FXPBDBendingConstraints::ComputeTotalEnergy(const FSolverParticles& InParticles, const FSolverReal ExplicitStiffness)
{
	...
	for (int32 ConstraintIndex = 0; ConstraintIndex < Constraints.Num(); ConstraintIndex++)
	{
		...
		const FSolverVec3 Z0Normalized = Z0.GetSafeNormal();
		const FSolverVec3 Z1Normalized = Z1.GetSafeNormal();

		const FSolverVec3 Tb = FSolverVec3::CrossProduct(Z0, Z1).GetSafeNormal();

		const FSolverReal S = FSolverVec3::DotProduct(Tb, T1);
...
void FXPBDBendingConstraints::AddBendingResidualAndHessian(const FSolverParticlesRange& Particles, const int32 ConstraintIndex, const int32 ConstraintIndexLocal, const FSolverReal Dt, TVec3<FSolverReal>& ParticleResidual, Chaos::PMatrix<FSolverReal, 3, 3>& ParticleHessian)
{
	const FSolverReal ExpStiffnessValue = (FSolverReal)XPBDStiffness;
	const FSolverReal ExpBucklingValue = (FSolverReal)XPBDBucklingStiffness;
...
{
	AddBendingResidualAndHessian(FSolverParticlesRange(Particles), ConstraintIndex, ConstraintIndexLocal, Dt, ParticleResidual, ParticleHessian);
}


void FXPBDBendingConstraints::AddInternalForceDifferential(const FSolverParticlesRange& InParticles, const TArray<TVector<FSolverReal, 3>>& DeltaParticles, TArray<TVector<FSolverReal, 3>>& Ndf)
{
	const FSolverReal ExpStiffnessValue = (FSolverReal)XPBDStiffness;
	const FSolverReal ExpBucklingValue = (FSolverReal)XPBDBucklingStiffness;
	...
	for (int32 ConstraintIndex = 0; ConstraintIndex < Constraints.Num(); ConstraintIndex++)
	{
		...
		const TArray<int32> LocalIndexMap = { 2, 1, 0, 3 };
		for (int32 Index = 0; Index < 4; Index++)
		{
			const int32 ActualConstraintIndexLocal = LocalIndexMap[Index];
			ComputeGradTheta(P3, P2, P1, P4, ActualConstraintIndexLocal, Alldthetadx[Index], Theta);
		}
		...
		for (int32 Index = 0; Index < 4; Index++)
		{
			for (int32 Alpha = 0; Alpha < 3; Alpha++)
			{
				DeltaC += Alldthetadx[Index][Alpha] * DeltaParticles[Constraint[Index]][Alpha];
			}
		}

		for (int32 Index = 0; Index < 4; Index++)
		{
			Ndf[Constraint[Index]] += BiphasicStiffnessValue * DeltaC * Alldthetadx[Index];
		}
