...
namespace Chaos
{
	void FSerializedMultiPhysicsState::ReadElementDataIntoBuffer(FSerializedDataBuffer& TargetBuffer)
	{
		...
		UE_LOGF(LogChaos, Verbose, "[MultiStateSerialization] Attempted to read an empty data entry. This might means we failed to obtain data to serialize a specific body or constraint.");
		...
		{
			UE_LOGF(LogChaos, Error, "[MultiStateSerialization] Attempted to read data out of bounds! | Buffer size [%d] | Start Pos [%d]| SizeToCopy [%d]", AsByteArray.Num(), DataTag.DataOffset, DataTag.DataSize);
		}
