	...
	void FConvexBuilder::Build(const TArray<FVec3Type>& InVertices, TArray <FPlaneType>& OutPlanes, TArray<TArray<int32>>& OutFaceIndices, TArray<FVec3Type>& OutVertices, FAABB3Type& OutLocalBounds, EBuildMethod BuildMethod)
	{
		...
		if (NumVerticesIn == 3)
		{
			...
			if (ensureMsgf(bIsValidTriangle, TEXT("FConvexBuilder::Build(): Generated invalid triangle!")))
			{
				...
				UE_LOGF(LogChaos, Verbose, "Encountered a triangle in convex hull generation. Will prepare a prism of thickness %.5f in place of a triangle.", TriQuadPrismInflation());
			}
			else
			{
				return;
		...
		else if (IsPlanarShape(InVertices, PlanarNormal))
		{
			Inflate(InVertices, ModifiedVertices, PlanarNormal, TriQuadPrismInflation());
			VerticesToUse = &ModifiedVertices;
			UE_LOGF(LogChaos, Verbose, "Encountered a planar shape in convex hull generation. Will prepare a prism of thickness %.5f in place of a triangle.", TriQuadPrismInflation());
		}
		...
		UE_CLOGF(OutVertices.Num() == 0, LogChaos, Warning, "Convex hull generation produced zero convex particles, collision will fail for this primitive.");
	}
