...
void FXPBDAnisotropicBendingConstraints::AddAnisotropicBendingResidualAndHessian(const FSolverParticlesRange& Particles, const int32 ConstraintIndex, const int32 ConstraintIndexLocal, const FSolverReal Dt, TVec3<FSolverReal>& ParticleResidual, Chaos::PMatrix<FSolverReal, 3, 3>& ParticleHessian)
{
	const FSolverVec3 ExpStiffnessValue((FSolverReal)StiffnessWeft, (FSolverReal)StiffnessWarp, (FSolverReal)StiffnessBias);
	const FSolverVec3 ExpBucklingValue((FSolverReal)BucklingStiffnessWeft, (FSolverReal)BucklingStiffnessWarp, (FSolverReal)BucklingStiffnessBias);
	const FSolverReal DampingRatioValue = (FSolverReal)DampingRatio;
...
{
	AddAnisotropicBendingResidualAndHessian(FSolverParticlesRange(Particles), ConstraintIndex, ConstraintIndexLocal, Dt, ParticleResidual, ParticleHessian);
}


void FXPBDAnisotropicBendingConstraints::AddInternalForceDifferential(const FSolverParticlesRange& InParticles, const TArray<TVector<FSolverReal, 3>>& DeltaParticles, TArray<TVector<FSolverReal, 3>>& Ndf)
{
	const FSolverVec3 ExpStiffnessValue((FSolverReal)StiffnessWeft, (FSolverReal)StiffnessWarp, (FSolverReal)StiffnessBias);
	const FSolverVec3 ExpBucklingValue((FSolverReal)BucklingStiffnessWeft, (FSolverReal)BucklingStiffnessWarp, (FSolverReal)BucklingStiffnessBias);
		...
		PhysicsParallelFor(ColorSize, [&](const int32 Index)
			{
				...
				{
					const int32 ActualConstraintIndexLocal = LocalIndexMap[LocalIdx];
					ComputeGradTheta(P3, P2, P1, P4, ActualConstraintIndexLocal, AllDThetaDx[LocalIdx], Theta);
				}
				...
				for (int32 LocalIdx = 0; LocalIdx < 4; LocalIdx++)
				{
					for (int32 Alpha = 0; Alpha < 3; Alpha++)
					{
						DeltaC += AllDThetaDx[LocalIdx][Alpha] * DeltaParticles[Constraint[LocalIdx]][Alpha];
					}
				}

				for (int32 LocalIdx = 0; LocalIdx < 4; LocalIdx++)
				{
					Ndf[Constraint[LocalIdx]] += BiphasicStiffnessValue * DeltaC * AllDThetaDx[LocalIdx];
				}
