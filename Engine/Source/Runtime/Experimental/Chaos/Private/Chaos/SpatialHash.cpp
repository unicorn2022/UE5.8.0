...
namespace Chaos
{
	...
	void TSpatialHash<T>::Init(const T Radius)
	{
		...
		Timer.Stop();
		UE_LOGF(LogChaosSpatialHash, Log, "TSpatialHash<T>::Init() Time is %f", Time);
	}					
	...
	TArray<int32> TSpatialHash<T>::GetClosestPoints(const TVector<T, 3>& Particle, const T MaxRadius)
	{
		...
		Timer.Stop();
		UE_LOGF(LogChaosSpatialHash, Log, "TSpatialHash<T>::GetClosestPoints() Time is %f", Time);
	...
	int32 TSpatialHash<T>::GetClosestPoint(const TVector<T, 3>& Particle)
	{
		...
		Timer.Stop();
		UE_LOGF(LogChaosSpatialHash, Log, "TSpatialHash<T>::GetClosestPoint() Time is %f", Time);
