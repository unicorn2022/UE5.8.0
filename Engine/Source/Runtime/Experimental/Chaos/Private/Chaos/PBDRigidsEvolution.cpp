...
namespace Chaos
{
	...
	FAutoConsoleVariableRef CVarAccelerationStructureSplitStaticAndDynamic(TEXT("p.Chaos.AccelerationStructureSplitStaticDynamic"), AccelerationStructureSplitStaticAndDynamic, TEXT("Set to 1: Sort Dynamic and Static bodies into separate acceleration structures, any other value will disable the feature"));
	...
	bool bPruningInSingleWorkerPhysics = false;
	FAutoConsoleVariableRef CVarPruningInSingleWorkerPhysics(TEXT("p.Chaos.PruningInSingleWorkerPhysics"), bPruningInSingleWorkerPhysics, TEXT("Enable Pruning to update acceleration structure in Single Worker mode.\
																Pruning the acceleration structure is an optimization technique that speed up the spatial acceleration update which is a normally a single threaded process by using several threads to prune data to avoid some unecessary update. \
																In single worker physics it would be better to disable it."));
	...
	struct FDefaultCollectionFactory : public ISpatialAccelerationCollectionFactory
	{
		...
		{
			constexpr uint16 DynamicAsUInt16 = static_cast<std::underlying_type_t<ESpatialAccelerationCollectionBucketInnerIndex>>(ESpatialAccelerationCollectionBucketInnerIndex::Dynamic);
			constexpr uint16 DynamicQueryOnlyAsUInt16 = static_cast<std::underlying_type_t<ESpatialAccelerationCollectionBucketInnerIndex>>(ESpatialAccelerationCollectionBucketInnerIndex::DynamicQueryOnly);			
			return AccelerationStructureUseDynamicTree && (SpatialAccelerationIdx.InnerIdx == DynamicAsUInt16 || SpatialAccelerationIdx.InnerIdx == DynamicQueryOnlyAsUInt16);
		}
		...
		TUniquePtr<ISpatialAccelerationCollection<FAccelerationStructureHandle, FReal, 3>> CreateEmptyCollection() override
		{
			...
			{
				// For static bodies
				constexpr uint16 DefaultAsUInt16 = static_cast<std::underlying_type_t<ESpatialAccelerationCollectionBucketInnerIndex>>(ESpatialAccelerationCollectionBucketInnerIndex::Default);
				Collection->AddSubstructure(CreateAccelerationPerBucket_Threaded(Empty, BucketIdx, true, false, false), BucketIdx, DefaultAsUInt16);
				...
				if (AccelerationStructureSplitStaticAndDynamic == 1 || AccelerationStructureIsolateQueryOnlyObjects == 1) 
				{
					constexpr uint16 DynamicAsUInt16 = static_cast<std::underlying_type_t<ESpatialAccelerationCollectionBucketInnerIndex>>(ESpatialAccelerationCollectionBucketInnerIndex::Dynamic);
					// Non static bodies
					Collection->AddSubstructure(CreateAccelerationPerBucket_Threaded(Empty, BucketIdx, true, IsDynamicTree(FSpatialAccelerationIdx{BucketIdx, DynamicAsUInt16 }), true), BucketIdx, DynamicAsUInt16);
				}
			}

			if (AccelerationStructureIsolateQueryOnlyObjects && NumBuckets == 1)
			{
				const auto DefaultQueryOnlyAsUInt16 = static_cast<std::underlying_type_t<ESpatialAccelerationCollectionBucketInnerIndex>>(ESpatialAccelerationCollectionBucketInnerIndex::DefaultQueryOnly);
				constexpr uint16 BucketIdx = 0;
				Collection->AddSubstructure(CreateAccelerationPerBucket_Threaded(Empty, BucketIdx, true, false, false), BucketIdx, DefaultQueryOnlyAsUInt16);
				if (AccelerationStructureSplitStaticAndDynamic == 1)
				{
					constexpr uint16 DynamicQueryOnlyAsUInt16 = static_cast<std::underlying_type_t<ESpatialAccelerationCollectionBucketInnerIndex>>(ESpatialAccelerationCollectionBucketInnerIndex::DynamicQueryOnly);

					// Non static bodies
					Collection->AddSubstructure(CreateAccelerationPerBucket_Threaded(Empty, BucketIdx, true, IsDynamicTree(FSpatialAccelerationIdx{ BucketIdx,  DynamicQueryOnlyAsUInt16 }), false), BucketIdx, DynamicQueryOnlyAsUInt16);
				}
			}
	...
	void FPBDRigidsEvolutionBase::PruneInternalPendingData(FAccelerationStructure& AccelerationStructure)
	{
		...
		if (Chaos::ShouldExecuteTasks(Particles.GetActiveDynamicMovingKinematicParticlesView().Num()))
		{
		...
		else
		{
			...
			if (bPruningInSingleWorkerPhysics)
			{
	...
	void FPBDRigidsEvolutionBase::FlushAsyncAccelerationQueue()
	{
		...
		if (!Chaos::ShouldExecuteTasks(Particles.GetActiveDynamicMovingKinematicParticlesView().Num()))
		{
			FlushAsyncAccelerationQueueLambda();
		}
		else
		{
	...
	void FPBDRigidsEvolutionBase::SetParticlesObjectState(TArrayView<FPBDRigidParticleHandle*> RigidParticles, 
		const EObjectStateType ObjectState, const bool bUpdateViews)
	{
		...
		// Sorting algorithm are modifying the array view in place and is O(N-ParticleOffset), N being the view size
		int32 ParticleOffset = 0;
		
		// The particles in [0-ParticleOffset) will be the valid ones the [ParticleOffset,NumParticles) will be the others
		Private::ReorderParticleView(RigidParticles, [](FPBDRigidParticleHandle* RigidParticle) -> bool 
			{ return (RigidParticle != nullptr);},ParticleOffset);
		
		const int32 NumValids = ParticleOffset;
		if (NumValids > 0)
		{
			TArrayView<FPBDRigidParticleHandle*> ValidParticles(&RigidParticles[0], NumValids);
			ParticleOffset = 0;

			// The particles in [0-ParticleOffset) will be the clustered one whereas the [ParticleOffset,NumParticles) will be the others
			Private::ReorderParticleView(ValidParticles, [](FPBDRigidParticleHandle* RigidParticle) -> bool
				{ return (RigidParticle->CastToClustered() != nullptr);}, ParticleOffset);
		
			// Move particles to clustered SOA
			const int32 NumClustered = ParticleOffset;
			if(NumClustered > 0)
			{
				TArrayView<FPBDRigidParticleHandle*> ClusteredParticles(&ValidParticles[0], NumClustered);
				Particles.SetClusteredParticlesSOA(ClusteredParticles, ObjectState, false);
			}
		
			// Move particles to dynamics SOA
			const int32 NumDynamics = ValidParticles.Num()-NumClustered;
			if (NumDynamics > 0)
			{
				TArrayView<FPBDRigidParticleHandle*> DynamicParticles(&ValidParticles[NumClustered], NumDynamics);
				Particles.SetDynamicParticlesSOA(DynamicParticles, ObjectState, false);
			}
		}
		...
		Private::ReorderParticleView(RigidParticles, [&ObjectState](FPBDRigidParticleHandle* RigidParticle) -> bool 
			{ return RigidParticle && (RigidParticle->ObjectState() != ObjectState && !RigidParticle->Disabled());}, ParticleOffset);	
		...
		if (NumTransients > 0)
		{
			...
			{
				Particles.MarkTransientDirtyParticles(TransientParticles, false);
			}
		...
		// Modify particle object state
		int32 ParticleIndex = 0;
		for (FPBDRigidParticleHandle* Particle : RigidParticles)
		{
			if (Particle)
			{
				const bool bWasDynamic = (Particle->ObjectState() == EObjectStateType::Dynamic) || (Particle->ObjectState() == EObjectStateType::Sleeping);
				...
				// Change the low level object state 
	...
	void FPBDRigidsEvolutionBase::SetParticleObjectState(FPBDRigidParticleHandle* Particle, EObjectStateType ObjectState)
	{
		...
		SetParticlesObjectState(RigidParticles, ObjectState, true);
	}
	...
	void FPBDRigidsEvolutionBase::SetParticleSleepType(FPBDRigidParticleHandle* Particle, ESleepType InSleepType)
	{
		...
		// Refresh cached sleep thresholds on the island node so that the island manager respects the new sleep type
