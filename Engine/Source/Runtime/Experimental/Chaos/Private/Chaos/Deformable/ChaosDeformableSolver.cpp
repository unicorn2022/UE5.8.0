...
namespace Chaos::Softs
{
	...
	FDeformableXPBDWeightedSpringConstraintParams GDeformableXPBDWeakConstraintParams;
	...
	void FDeformableSolver::Reset(const FDeformableSolverProperties& InProps)
	{
		...
		if (Property.bUseGaussSeidelConstraints)
		{
			...
			GSWeakConstraints.Reset(new FGaussSeidelSpringConstraints({}, {}, {}, {}, {}, {}, {}, GDeformableXPBDWeakConstraintParams));
			GSDynamicWeakConstraints.Reset(new FGaussSeidelDynamicSpringConstraints(GDeformableXPBDWeakConstraintParams));
			GSSphereRepulsionConstraints.Reset(new FGaussSeidelParticleSphereRepulsionConstraints(Property.SphereRepulsionRadius, Property.SphereRepulsionStiffness, Evolution->Particles(), GDeformableXPBDWeakConstraintParams));
			GSVolumeConstraints.Reset(new FGaussSeidelUnilateralVolumeConstraints(Evolution->Particles(), {}, {}));
			MuscleIndexOffset.Empty();
			MuscleActivationConstraints.Reset(new FMuscleActivationTetrahedralConstraints());
		}
					...
					for (int32 vdx : BoundVerts)
					{
						if (vdx >= 0 && !VertexBoneWeightsFacade.IsKinematicVertex(vdx))
						{
							if (!bHavePrintedLog)
							{
								...
								UE_LOGF(LogChaosDeformableSolver, Warning, "Detected deprecated kinematic initialization, reevaluate input asset");
							}
	...
	void FDeformableSolver::InitializeWeakConstraint(FFleshThreadingProxy& Proxy)
	{
		...
		if (Property.bEnablePositionTargets)
		{
			...
			else
			{
				...
				FXPBDWeightedSpringConstraints* WeakConstraint =
					new FXPBDWeightedSpringConstraints(Evolution->Particles(),
				...
				WeakConstraints.Add(TUniquePtr<FXPBDWeightedSpringConstraints>(WeakConstraint));
			}
		}
		...
		GSVolumeConstraints.Reset(new FGaussSeidelUnilateralVolumeConstraints(Evolution->Particles(), MoveTemp(InConstraints), MoveTemp(InStiffnessArray)));
		if (GSVolumeConstraints->NumConstraints() && !Property.bUseGaussSeidelConstraints)
		{
			UE_LOGF(LogChaosDeformableSolver, Error, "Error: must check [Use Gauss Seidel constraints] for volume constraints.");
		}
	}
			...
			if (this->CurrentInputPackage->ObjectMap[Owner] != nullptr)
			{
				...
				if (ConstraintsInputBuffer)
				{
					for (auto& AddConstraints : ConstraintsInputBuffer->Added)
					{
						...
						UE_LOGF(LogChaosDeformableSolver, Log, "Process Constraint : %ls", *Proxy.GetOwner()->GetName());
						...
						if (Proxies.Contains(AddConstraints.Get<0>()) && Proxies.Contains(AddConstraints.Get<1>()))
						{
							...
							if (SourceProxy && TargetProxy)
							{
								...
								{
									UE_LOGF(LogChaosDeformableSolver, Log, "... Intersections : %d", Intersections.Num());
									if (GSDynamicWeakConstraints && Property.bEnableDynamicSprings)
									{
										float Stiffness = AddConstraints.Parameters.Stiffness;
										TArray<const FGaussSeidelSpringConstraints::FGaussSeidelConstraintHandle*> ConstraintHandles = GSDynamicWeakConstraints -> AddParticleTetrahedraConstraints(Geom, Evolution->Particles(), Intersections, SampleRange, TargetRange, Stiffness);
			...
			if (uint32 NumElements = Tetrahedron->Num())
			{
				...
				if (Rest.HasAttributes({ FManagedArrayCollection::TManagedType<FSolverReal>("Stiffness", FGeometryCollection::VerticesGroup) }))
				{
					...
					if (Property.bEnableCorotatedConstraints)
					{
						...
						{
							FBlendedXPBDCorotatedTetrahedralConstraints* BlendedCorotatedConstraint =
								new FBlendedXPBDCorotatedTetrahedralConstraints(
							...
							BlendedCorotatedConstraints.Add(TUniquePtr<FBlendedXPBDCorotatedTetrahedralConstraints>(BlendedCorotatedConstraint));

						}
						else
						{
							FXPBDCorotatedTetrahedralConstraints* CorotatedConstraint =
								new FXPBDCorotatedTetrahedralConstraints(
							...
							CorotatedConstraints.Add(TUniquePtr<FXPBDCorotatedTetrahedralConstraints>(CorotatedConstraint));
						}
					}
							...
							if (Rest.FindAttributeTyped<FVector3f>("Vertex", FGeometryCollection::VerticesGroup))
							{
								...
								if (WeightsFacade.HasValidBoneIndicesAndWeights())
								{
									...
									if (ensure(0 <= ObjectVertexIndex && ObjectVertexIndex < NumObjectVertices))
									{
										if (FleshInputBuffer)
										{
											const TArray<int32>& BoneIndices = WeightsFacade.GetBoneIndices()[ObjectVertexIndex];
											const TArray<float>& BoneWeights = WeightsFacade.GetBoneWeights()[ObjectVertexIndex];
	...
	void FDeformableSolver::InitializeGridBasedConstraintVariables()
	{
		...
		GridBasedCorotatedConstraint.Reset(new Chaos::Softs::FXPBDGridBasedCorotatedTetrahedralConstraints(
	...
	void FDeformableSolver::InitializeGaussSeidelConstraintVariables()
	{
		...
		GSMainConstraint.Reset(new Chaos::Softs::FGaussSeidelMainConstraints(Evolution->Particles(), Property.bDoQuasistatics, Property.bUseSOR, Property.OmegaSOR, GSParallelMax, MaxDxRatio));

		if (AllUnconstrainedSurfaceElementsCorotatedCod->Num() > 0) 
		{
			GSCorotatedCodConstraints.Reset(new Chaos::Softs::FGaussSeidelCorotatedTriangleConstraints(
			...
			GSMainConstraint->StaticConstraintResidualAndHessian()[StaticIndex] = [this](const FSolverParticlesRange& Particles, const int32 ElementIndex, const int32 ElementIndexLocal, const FSolverReal Dt, TVec3<FSolverReal>& ParticleResidual, Chaos::PMatrix<FSolverReal, 3, 3>& ParticleHessian)
			{
		...
		{
			GSLinearCodConstraints.Reset(new Chaos::Softs::FGaussSeidelLinearTriangleConstraints(
		...
		{
			GSNeohookeanConstraints.Reset(new Chaos::Softs::FGaussSeidelNeohookeanTetrahedralConstraints(
		...
		{
			GSCorotatedConstraints.Reset(new Chaos::Softs::FGaussSeidelCorotatedTetrahedralConstraints(
				Evolution->Particles(), *AllElements, *AllTetEMeshArray, *AllTetNuMeshArray, MoveTemp(*AllTetAlphaJArray), MoveTemp(*AllIncidentElements), MoveTemp(*AllIncidentElementsLocal), 0, Evolution->Particles().Size(), Property.bDoQuasistatics, Property.bUseSOR, Property.OmegaSOR, GDeformableXPBDCorotatedParams));
			Evolution->ResetConstraintRules();
		...
		if (Property.bEnablePositionTargets)
		{
			...
			const TArray<TArray<int32>>* StaticIncidentElements = nullptr;
			const TArray<TArray<int32>>* StaticIncidentElementsLocal = nullptr;
			...
			GSMainConstraint->StaticConstraintResidualAndHessian()[StaticIndex1] = [this](const FSolverParticlesRange& Particles, const int32 ConstraintIndex, const int32 ConstraintIndexLocal, const FSolverReal Dt, TVec3<FSolverReal>& ParticleResidual, Chaos::PMatrix<FSolverReal, 3, 3>& ParticleHessian)
				{
		...
		if (Property.bEnableDynamicSprings)
		{
			...
			GSMainConstraint->DynamicConstraintResidualAndHessian()[DynamicIndex] = [this](const FSolverParticlesRange& Particles, const int32 ConstraintIndex, const int32 ConstraintIndexLocal, const FSolverReal Dt, TVec3<FSolverReal>& ParticleResidual, Chaos::PMatrix<FSolverReal, 3, 3>& ParticleHessian)
			{
			...
			Evolution->ConstraintInits()[InitIndex1] =
				[this](FSolverParticles& InParticles, const FSolverReal Dt)
				{
					...
					{
						const TArray<TArray<int32>>* WCDynamicIncidentElements = nullptr;
						const TArray<TArray<int32>>* WCDynamicIncidentElementsLocal = nullptr;
		...
		if (Property.bDoSpringCollision)
		{
			...
			GSMainConstraint->TransientConstraintResidualAndHessian()[TransientIndex] = [this](const FSolverParticlesRange& Particles, const int32 ConstraintIndex, const int32 ConstraintIndexLocal, const FSolverReal Dt, TVec3<FSolverReal>& ParticleResidual, Chaos::PMatrix<FSolverReal, 3, 3>& ParticleHessian)
				{
					this->GSWeakConstraints->AddWCResidual(Particles, ConstraintIndex + this->GSWeakConstraints->GetInitialWCSize(), ConstraintIndexLocal, Dt, ParticleResidual, ParticleHessian);
				};
		...
		if (Property.bDoSphereRepulsion)
		{
			...
			GSMainConstraint->TransientConstraintResidualAndHessian()[TransientIndex] =
				[this](const FSolverParticlesRange& Particles, const int32 ConstraintIndex, const int32 ConstraintIndexLocal, const FSolverReal Dt, TVec3<FSolverReal>& ParticleResidual, Chaos::PMatrix<FSolverReal, 3, 3>& ParticleHessian)
				{
	...
	void FDeformableSolver::DebugDrawSimulationData()
	{
		...
		if (Property.bUseGaussSeidelConstraints && GDeformableXPBDCorotatedParams.NumLogExtremeParticle > 0)
		{
			...
			{
				UE_LOGF(LogChaosDeformableSolver, Warning, "Particle index %d has average deformation ratio %f", DistIndices[i], DistRatio[DistIndices[i]]);
			}
		}
#endif
	}
	...
	void FDeformableSolver::WriteFrame(FThreadingProxy& InProxy, const FSolverReal DeltaTime)
	{
		...
		if (FFleshThreadingProxy* Proxy = InProxy.As<FFleshThreadingProxy>())
		{
			...
			if (SurfaceElements)
			{
				...
				if (Frame == 0)
				{
					FFileHelper::SaveStringToFile(FString("DeltaTime\r\n"), *file);
				}
				FFileHelper::SaveStringToFile((FString::SanitizeFloat(DeltaTime) + FString("\r\n")), *file, FFileHelper::EEncodingOptions::AutoDetect, &IFileManager::Get(), EFileWrite::FILEWRITE_Append);
			}
		}
	}
	...
	void FDeformableSolver::WriteTrisGEO(const FSolverParticles& Particles, const TArray<TVec3<int32>>& Mesh)
	{
		...
		FString HeaderInfo = FString("NPoints ") + FString::FromInt(Np) + FString(" NPrims ") + FString::FromInt(NPrims) + FString("\r\n");
		...
		for (int32 i = 0; i < Np; i++) {
			...
			FString ParticleInfo = FString::SanitizeFloat(Particles.GetX(i)[0]) + FString(" ") + FString::SanitizeFloat(Particles.GetX(i)[1]) + FString(" ") + FString::SanitizeFloat(Particles.GetX(i)[2]) + FString(" ") + FString::FromInt(1) + FString("\r\n");
		...
		for (int32 i = 0; i < Mesh.Num(); i++) {
			//outstream << "Poly 3 < ";
			FString ElementToWrite = FString("Poly 3 < ") + FString::FromInt(Mesh[i][0]) + FString(" ") + FString::FromInt(Mesh[i][1]) + FString(" ") + FString::FromInt(Mesh[i][2]) + FString("\r\n");
