...
namespace Chaos
{
	...
	int32 FMidPhaseModifierAccessor::GetNumMidPhases() const
	{
		return Evolution.GetCollisionConstraints().GetConstraintAllocator().GetNumMidPhases();
	}

	int32 FMidPhaseModifierAccessor::GetNumMidPhases(const FGeometryParticleHandle* Particle) const
	{
		return Particle->ParticleCollisions().Num();
	}
