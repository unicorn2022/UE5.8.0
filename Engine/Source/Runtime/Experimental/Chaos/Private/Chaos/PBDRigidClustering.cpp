...
namespace Chaos
{
		...
		FPBDCollisionConstraints& CollisionRule)
	{
		...
		UE_LOGF(LogChaos, Verbose, "START FRAME with Dt %f", Dt);
		...
		if(GDumpClusterAndReleaseStats == 1)
		{
			...
			{
				UE_LOGF(LogChaos, Display, "Clustering | Frame %.5u | Clusters: %.3u, Released: %.4u (TotalClusters: %.3u, TotalReleased: %.4u)",
					   AdvanceCount,
		...
		Timer.Stop();
		UE_LOGF(LogChaos, Verbose, "Cluster Break Update Time is %f", Time);
	}
	...
	bool FRigidClustering::BreakClustersByProxy(const IPhysicsProxyBase* Proxy)
	{
		...
		for (FPBDRigidClusteredParticleHandle* ClusteredHandle : GetTopLevelClusterParents())
		{
			...
			if (bIsInputProxy || bIsInPhysicsProxiesSet)
			{
				...
				if (bIsInPhysicsProxiesSet && !bIsInputProxy)
				{
