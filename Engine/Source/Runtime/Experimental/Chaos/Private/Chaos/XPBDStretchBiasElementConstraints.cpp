...
void FXPBDStretchBiasElementConstraints::ApplyHelper(SolverParticlesOrRange& Particles, const FSolverReal Dt, const int32 ConstraintIndex, const FSolverVec3& ExpStiffnessValue, const FSolverReal DampingRatioValue, const FSolverReal WarpScaleValue, const FSolverReal WeftScaleValue) const
{
	...
	const int32 Idx0 = Constraint[0];
	const int32 Idx1 = Constraint[1];
	const int32 Idx2 = Constraint[2];
	if (Particles.InvM(Idx0) == (FSolverReal)0. && Particles.InvM(Idx1) == (FSolverReal)0. && Particles.InvM(Idx2) == (FSolverReal)0.)
	{
		return;
	}

	const FSolverReal CombinedInvMass = Particles.InvM(Idx0) + Particles.InvM(Idx1) + Particles.InvM(Idx2);

	FSolverVec3 DXDu, DXDv;
	CalculateUVStretch(ConstraintIndex, Particles.P(Idx0), Particles.P(Idx1), Particles.P(Idx2), DXDu, DXDv);
	...
	const FSolverVec3 V0TimesDt = Particles.GetP(Idx0) - Particles.GetX(Idx0);
	const FSolverVec3 V1TimesDt = Particles.GetP(Idx1) - Particles.GetX(Idx1);
	const FSolverVec3 V2TimesDt = Particles.GetP(Idx2) - Particles.GetX(Idx2);
	...
	FSolverVec3 DLambda;
	DLambda[0] = CalcDLambda(FinalStiffnesses[0], Dt, Lambda[0], Damping[0], Particles.InvM(Idx0), Particles.InvM(Idx1), Particles.InvM(Idx2),
		V0TimesDt, V1TimesDt, V2TimesDt, Cu, GradCu0, GradCu1, GradCu2);
	DLambda[1] = CalcDLambda(FinalStiffnesses[1], Dt, Lambda[1], Damping[1], Particles.InvM(Idx0), Particles.InvM(Idx1), Particles.InvM(Idx2),
		V0TimesDt, V1TimesDt, V2TimesDt, Cv, GradCv0, GradCv1, GradCv2);
	DLambda[2] = CalcDLambda(FinalStiffnesses[2], Dt, Lambda[2], Damping[2], Particles.InvM(Idx0), Particles.InvM(Idx1), Particles.InvM(Idx2),
		V0TimesDt, V1TimesDt, V2TimesDt, Cs, GradCs0, GradCs1, GradCs2);

	Particles.P(Idx0) -= Particles.InvM(Idx0) * (DLambda[0] * GradCu0 + DLambda[1] * GradCv0 + DLambda[2] * GradCs0);
	Particles.P(Idx1) -= Particles.InvM(Idx1) * (DLambda[0] * GradCu1 + DLambda[1] * GradCv1 + DLambda[2] * GradCs1);
	Particles.P(Idx2) -= Particles.InvM(Idx2) * (DLambda[0] * GradCu2 + DLambda[1] * GradCv2 + DLambda[2] * GradCs2);

	Lambda += DLambda;
}


void FXPBDStretchBiasElementConstraints::InitializeDmInvAndMeasures(const FSolverParticlesRange& Particles)
{
	...
	for (int32 ElemIdx = 0; ElemIdx < Constraints.Num(); ElemIdx++)
	{
		const TVec3<FSolverReal> X1X0 = Particles.GetX(Constraints[ElemIdx][1]) - Particles.GetX(Constraints[ElemIdx][0]);
		const TVec3<FSolverReal> X2X0 = Particles.GetX(Constraints[ElemIdx][2]) - Particles.GetX(Constraints[ElemIdx][0]);
		...
		Measure[ElemIdx] = Chaos::TVector<FSolverReal, 3>::CrossProduct(X1X0, X2X0).Size() / 2.f;
		ensureMsgf(Measure[ElemIdx] > 0.f, TEXT("Degenerate triangle detected"));

		const PMatrix<FSolverReal, 2, 2> DmInv = Dm.Inverse();
		DmInverse[ElemIdx] = DmInv;
		DmArray[ElemIdx] = Dm;
		RestDmArray[ElemIdx] = Chaos::PMatrix<FSolverReal, 3, 2>(X1X0, X2X0);
	}
...
{
	InitializeDmInvAndMeasures(FSolverParticlesRange(Particles));
}

void FXPBDStretchBiasElementConstraints::AddStretchBiasElementResidualAndHessian(const FSolverParticlesRange& Particles, const int32 ConstraintIndex, const int32 ConstraintIndexLocal, const FSolverReal Dt, TVec3<FSolverReal>& ParticleResidual, Chaos::PMatrix<FSolverReal, 3, 3>& ParticleHessian)
{
	const bool StiffnessWarpHasWeightMap = StiffnessWarp.HasWeightMap();
	const bool StiffnessWeftHasWeightMap = StiffnessWeft.HasWeightMap();
	const bool StiffnessBiasHasWeightMap = StiffnessBias.HasWeightMap();
	...
	else
	{
		for (int32 Alpha = 0; Alpha < 3; Alpha++)
		{
			...
			{
				ResContribution[Alpha] -= Coeffu * DXDu[Alpha] * DeltaUVInv.M[Col];
				ResContribution[Alpha] -= Coeffv * DXDv[Alpha] * DeltaUVInv.M[Col + 2];
			}
		}
	}
	...
	if (ConstraintIndexLocal > 0)
	{
		...
		for (int32 Col = 0; Col < 2; Col++)
		{
			DmInvSum += DmInvMat.GetAt(ConstraintIndexLocal - 1, Col) * DmInvMat.GetAt(ConstraintIndexLocal - 1, Col);
		}
		for (int32 Alpha = 0; Alpha < 3; Alpha++)
		{
	...
	else
	{
		for (int32 Alpha = 0; Alpha < 3; Alpha++)
		{
			for (int32 Beta = 0; Beta < 3; Beta++)
			{
				for (int32 Col = 0; Col < 2; Col++)
				{
					...
					{
						//constraint u (definitely could be simplified by adding u[Col] first):
						HessianContribution.SetAt(Alpha, Beta, HessianContribution.GetAt(Alpha, Beta) + RankOneuCoeff * DeltaUVInv.M[Col] * DeltaUVInv.M[Row] * DXDu[Alpha] * DXDu[Beta]);

						//constraint v (definitely could be simplified by adding v[Col] first):
						HessianContribution.SetAt(Alpha, Beta, HessianContribution.GetAt(Alpha, Beta) + RankOnevCoeff * DeltaUVInv.M[Col+2] * DeltaUVInv.M[Row+2] * DXDv[Alpha] * DXDv[Beta]);
					}
				}
			}
			HessianContribution.SetRow(Alpha, HessianContribution.GetRow(Alpha) + HessianCoeffs * GradCs[Alpha] * GradCs);
		...
		for (int32 Row = 0; Row < 2; Row++)
		{
			...
			for (int32 Col = 0; Col < 2; Col++)
			{
				LocalDmSum += DmInvMat.M[Row * 2 + Col];
			}
...
{
	AddStretchBiasElementResidualAndHessian(FSolverParticlesRange(Particles), ConstraintIndex, ConstraintIndexLocal, Dt, ParticleResidual, ParticleHessian);
}


void FXPBDStretchBiasElementConstraints::AddInternalForceDifferential(const FSolverParticlesRange& InParticles, const TArray<TVector<FSolverReal, 3>>& DeltaParticles, TArray<TVector<FSolverReal, 3>>& Ndf)
{
		...
		PhysicsParallelFor(ColorSize, [&](const int32 Index)
			{
				...
				if (InParticles.InvM(Idx0) == (FSolverReal)0. && InParticles.InvM(Idx1) == (FSolverReal)0. && InParticles.InvM(Idx2) == (FSolverReal)0.)
				{
					return;
				}

				FSolverVec3 DXDu, DXDv;
				CalculateUVStretch(ConstraintIndex, InParticles.P(Idx0), InParticles.P(Idx1), InParticles.P(Idx2), DXDu, DXDv);
				...
				const FSolverVec3 DeltaP01 = DeltaParticles[Idx1 - ParticleStart] - DeltaParticles[Idx0 - ParticleStart];
				const FSolverVec3 DeltaP02 = DeltaParticles[Idx2 - ParticleStart] - DeltaParticles[Idx0 - ParticleStart];
				for (int32 Alpha = 0; Alpha < 3; Alpha++)
				{
					DeltaCu += DXDuNormalized[Alpha] * DeltaP01[Alpha] * DeltaUVInv.M[0];
					DeltaCu += DXDuNormalized[Alpha] * DeltaP02[Alpha] * DeltaUVInv.M[1];
					DeltaCv += DXDvNormalized[Alpha] * DeltaP01[Alpha] * DeltaUVInv.M[2];
					DeltaCv += DXDvNormalized[Alpha] * DeltaP02[Alpha] * DeltaUVInv.M[3];
					DeltaCs += GradCs1[Alpha] * DeltaP01[Alpha];
					DeltaCs += GradCs2[Alpha] * DeltaP02[Alpha];
				}

				const FSolverVec3 FinalStiffnesses = StiffnessScales[ConstraintIndex] * ExpStiffnessValue;

				FSolverVec3 DeltaDXDu, DeltaDXDv;
				CalculateUVStretch(ConstraintIndex, DeltaParticles[Idx0 - ParticleStart], DeltaParticles[Idx1 - ParticleStart], DeltaParticles[Idx2 - ParticleStart], DeltaDXDu, DeltaDXDv);
				...
				Ndf[Idx0] += FinalStiffnesses[0] * DeltaCu * GradCu0 + FinalStiffnesses[0] * Cu * DeltaGradCu0;
				Ndf[Idx0] += FinalStiffnesses[1] * DeltaCv * GradCv0 + FinalStiffnesses[0] * Cu * DeltaGradCu1;
				Ndf[Idx0] += FinalStiffnesses[2] * DeltaCs * GradCs0 + FinalStiffnesses[0] * Cu * DeltaGradCu2;
				Ndf[Idx1] += FinalStiffnesses[0] * DeltaCu * GradCu1 + FinalStiffnesses[1] * Cv * DeltaGradCv0;
				Ndf[Idx1] += FinalStiffnesses[1] * DeltaCv * GradCv1 + FinalStiffnesses[1] * Cv * DeltaGradCv1;
				Ndf[Idx1] += FinalStiffnesses[2] * DeltaCs * GradCs1 + FinalStiffnesses[1] * Cv * DeltaGradCv2;
				Ndf[Idx2] += FinalStiffnesses[0] * DeltaCu * GradCu2;
				Ndf[Idx2] += FinalStiffnesses[1] * DeltaCv * GradCv2;
				Ndf[Idx2] += FinalStiffnesses[2] * DeltaCs * GradCs2;
			});
	}
}
...
{
	AddInternalForceDifferential(FSolverParticlesRange(InParticles), DeltaParticles, Ndf);
}
