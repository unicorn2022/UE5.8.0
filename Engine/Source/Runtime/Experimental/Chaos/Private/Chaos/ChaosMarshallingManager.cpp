...
void FChaosMarshallingManager::DebugHistory_Internal(int32 NumFrames)
{
	const int32 UseNumFrames = FMath::Min(NumFrames, HistoryQueue_Internal.Num());
	for (int32 Idx = 0; Idx < UseNumFrames; ++Idx)
	{
		UE_LOGF(LogChaos, Log, "					History internal[%d] at frame %d", Idx, HistoryQueue_Internal[Idx]->InternalStep);
	}
}
