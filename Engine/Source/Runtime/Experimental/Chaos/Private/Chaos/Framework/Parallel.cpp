...
namespace Chaos
{
	// If GSingleThreadedPhysics is 1 physics simulation runs on the Game Thread in single thread 
	CHAOS_API int32 GSingleThreadedPhysics = 0;
	// If bSingleWorkerPhysics is true physics simulation run only on one thread on the Physics Thread
	CHAOS_API bool bSingleWorkerPhysics = false;
	CHAOS_API int32 InnerParallelForBatchSize = 0;
	CHAOS_API int32 MinRangeBatchSize = 0;
	CHAOS_API int32 MaxNumWorkers = 100;
	// Batch size for collision 
	// can be smaller in general because Narrow Phase between two rigid bodies can be a set of many shapes with height field or mesh. 
	CHAOS_API int32 CollisionSmallBatchSize = 10;
	// Batch size  for multi threading, use not to launch tasks if not enough instance
	CHAOS_API int32 GlobalSmallBatchSize = 40;
	...
	FAutoConsoleVariableRef CVarSmallBatchCollisionSize(TEXT("p.Chaos.CollisionSmallBatchSize"), CollisionSmallBatchSize, TEXT("Small batch size used for Collision Detection"));
	FAutoConsoleVariableRef CVarGlobalSmallBatchSize(TEXT("p.Chaos.GlobalSmallBatchSize"), GlobalSmallBatchSize, TEXT("Minimum batch size needed to launch multi-threaded tasks"));
	FAutoConsoleVariableRef CVarLargeBatchSize(TEXT("p.Chaos.LargeBatchSize"), LargeBatchSize, TEXT("Large batch size for chaos tasks"));
	FAutoConsoleVariableRef CVarSingleWorkerPhysics(TEXT("p.Chaos.SingleWorkerPhysics"), bSingleWorkerPhysics, TEXT("Run Chaos on a single thread"));
}
...
void Chaos::PhysicsParallelFor(int32 InNum, TFunctionRef<void(int32)> InCallable, bool bForceSingleThreaded)
{
	...
	const bool bSingleThreaded = bSingleWorkerPhysics || bDisablePhysicsParallelFor || bForceSingleThreaded;
...
void Chaos::PhysicsParallelForRange(int32 InNum, TFunctionRef<void(int32, int32)> InCallable, const int32 InMinBatchSize, bool bForceSingleThreaded)
{
	...
	::ParallelFor(RangeIndex.Num() - 1, PassThrough, bSingleWorkerPhysics || bDisablePhysicsParallelFor || bForceSingleThreaded);
}
...
bool Chaos::ShouldExecuteTasks(int32 InInstanceNum)
{
	...
	else if (InInstanceNum >= GlobalSmallBatchSize)
	{
