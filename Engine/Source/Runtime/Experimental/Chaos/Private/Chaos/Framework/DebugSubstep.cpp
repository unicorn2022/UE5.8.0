...
namespace Chaos
{
	...
	void FDebugSubstep::Enable(bool bEnable)
	{
		...
		UE_LOGF(LogChaosThread, Verbose, "[Game Thread] Enable=%ls", bEnable ? TEXT("True"): TEXT("False"));
	}

	void FDebugSubstep::ProgressToSubstep()
	{
		CommandQueue.Enqueue(ECommand::ProgressToSubstep);
		UE_LOGF(LogChaosThread, Verbose, "[Game Thread] Progress");
	}
	...
	bool FDebugSubstep::SyncAdvance(bool bIsSolverEnabled)
	{
		...
		while (CommandQueue.Dequeue(Command))
		{
			...
			default:
				UE_LOGF(LogChaosThread, Fatal, "Unknown debug step command.");
				break;
			}
		}
	...
	void FDebugSubstep::Add(bool bInStep, const TCHAR* Label) const
	{
		...
		if (bIsEnabled)
		{
			...
			UE_LOGF(LogChaosThread, Log, "Reached %ls '%ls'", bInStep ? TEXT("step"): TEXT("substep"), Label ? Label: TEXT(""));
			UE_LOGF(LogChaosThread, Verbose, "[Debug Thread] Triggering substep event");
			SubstepEvent->Trigger();
			UE_LOGF(LogChaosThread, Verbose, "[Debug Thread] Waiting for progress event");
			ProgressEvent->Wait();
			UE_LOGF(LogChaosThread, Verbose, "[Debug Thread] Progress event received, wait ended");
		}
		else if (bInStep)  // Trigger one last event at the step boundary when disabled
		{
			UE_LOGF(LogChaosThread, Log, "Reached step '%ls'", Label ? Label: TEXT(""));
			UE_LOGF(LogChaosThread, Verbose, "[Debug Thread] Triggering substep event");
			SubstepEvent->Trigger();
		}
	}
	...
	void FDebugSubstep::Start()
	{
		...
		bIsEnabled = true;
		UE_LOGF(LogChaosThread, Verbose, "[%ls Thread] bIsEnabled changed (false->true)", IsInGameThread() ? TEXT("Game") : TEXT("Physics"));
		UE_LOGF(LogChaosThread, Log, "Chaos' debug substep mode is now engaged. Pausing solver thread at next step.");
	...
	void FDebugSubstep::Stop()
	{
		...
		bIsEnabled = false;
		UE_LOGF(LogChaosThread, Verbose, "[%ls Thread] bIsEnabled changed (true->false)", IsInGameThread() ? TEXT("Game") : TEXT("Physics"));
		...
		UE_LOGF(LogChaosThread, Verbose, "[%ls Thread] Waiting for last step event", IsInGameThread() ? TEXT("Game") : TEXT("Physics"));
		SubstepEvent->Wait();  // Last wait event will be triggered on debug thread exit

		UE_LOGF(LogChaosThread, Verbose, "[%ls Thread] Substep event received, wait ended", IsInGameThread() ? TEXT("Game") : TEXT("Physics"));
		UE_LOGF(LogChaosThread, Log, "Chaos' debug substep mode is now disengaged. Resuming solver thread at next step.");
	...
	void FDebugSubstep::Substep(bool bShouldStep)
	{
		...
		UE_LOGF(LogChaosThread, Verbose, "[%ls Thread] Triggering progress event", IsInGameThread() ? TEXT("Game") : TEXT("Physics"));
		...
		do
		{
			...
			UE_LOGF(LogChaosThread, Verbose, "[%ls Thread] Waiting for substep event", IsInGameThread() ? TEXT("Game") : TEXT("Physics"));
			SubstepEvent->Wait();
			UE_LOGF(LogChaosThread, Verbose, "[%ls Thread] Substep event received, wait ended", IsInGameThread() ? TEXT("Game") : TEXT("Physics"));
		} while (bWaitForStep);
	}
