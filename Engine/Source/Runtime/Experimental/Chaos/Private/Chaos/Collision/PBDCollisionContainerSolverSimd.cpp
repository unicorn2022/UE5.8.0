...
namespace Chaos
{
	...
	namespace Private
	{
		...
		void FPBDCollisionContainerSolverSimd::ResetImpl(const int32 MaxCollisions)
		{
			NumConstraints = 0;
			...
			AppliedShockPropagation.Init(1, MaxCollisions);
		...
		void FPBDCollisionContainerSolverSimd::AddConstraints(const TArrayView<Private::FPBDIslandConstraint*>& IslandConstraints)
		{
			...
			// @todo(chaos): If there are many sleeping constraints, we will be overallocating here.
		...
		bool FPBDCollisionContainerSolverSimd::SortConstraintsByPartitionIndex(const TArray<int32>& PartitionIndices, const TArray<int32>& PrefixSum)
		{
			//@todo(Chaos): Implement constraint color sorting for the SIMD solver
			// This requires sorting constraints before the SIMD lanes are allocated 
			// so that only constraints with the same partition index are sharing SIMD data.
			return false;
		}

		void FPBDCollisionContainerSolverSimd::GetAllConstrainedParticles(TArray<const FGeometryParticleHandle*>& ParticleHandles) const
		{
			//@todo(Chaos): Implement constraint color sorting for the SIMD solver
			// This requires sorting constraints before the SIMD lanes are allocated 
			// so that only constraints with the same partition index are sharing SIMD data.
		...
		{
			ApplyPositionConstraints(InDt, It, NumIts, 0, SimdData.SimdSolvers.Num());
		}

		void FPBDCollisionContainerSolverSimd::ApplyPositionConstraints(const FReal InDt, const int32 It, const int32 NumIts, const int32 BeginIndex, const int32 EndIndex)
		{
			...
			UpdatePositionShockPropagation(It, NumIts, BeginIndex, EndIndex, SolverSettings);
			...
			if (bApplyStaticFriction)
			{
				Private::FPBDCollisionSolverHelperSimd::SolvePositionWithFriction(
					MakeArrayView(&SimdData.SimdSolvers[BeginIndex], EndIndex - BeginIndex),
					MakeArrayView(&SimdData.SimdManifoldPoints[BeginIndex], EndIndex - BeginIndex),
					MakeArrayView(&SimdData.SimdSolverBodies[BeginIndex], EndIndex - BeginIndex),
					Dt,
					MaxPushOut);
			}
			else
		...
		{
			ApplyVelocityConstraints(InDt, It, NumIts, 0, SimdData.SimdSolvers.Num());
		}

		void FPBDCollisionContainerSolverSimd::ApplyVelocityConstraints(const FReal InDt, const int32 It, const int32 NumIts, const int32 BeginIndex, const int32 EndIndex)
		{
			...
			UpdateVelocityShockPropagation(It, NumIts, BeginIndex, EndIndex, SolverSettings);
		...
		{
			ApplyProjectionConstraints(Dt, It, NumIts, 0, SimdData.SimdSolvers.Num());
		}

		void FPBDCollisionContainerSolverSimd::ApplyProjectionConstraints(const FReal Dt, const int32 It, const int32 NumIts, const int32 BeginIndex, const int32 EndIndex)
		{
			// Not supported for collisions
		}

		void FPBDCollisionContainerSolverSimd::ApplyShockPropagation(const FSolverReal ShockPropagation, const int32 BeginIndex, const int32 EndIndex)
		{
		...
		void FPBDCollisionContainerSolverSimd::UpdatePositionShockPropagation(const int32 It, const int32 NumIts, const int32 BeginIndex, const int32 EndIndex, const FPBDCollisionSolverSettings& SolverSettings)
		{
		...
		void FPBDCollisionContainerSolverSimd::UpdateVelocityShockPropagation(const int32 It, const int32 NumIts, const int32 BeginIndex, const int32 EndIndex, const FPBDCollisionSolverSettings& SolverSettings)
		{
		...
		void FPBDCollisionContainerSolverSimd::UpdateCollisions(const FSolverReal InDt, const int32 BeginIndex, const int32 EndIndex)
		{
			//const FSolverReal Dt = FSolverReal(InDt);
