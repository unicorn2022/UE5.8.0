...
namespace Chaos
{
	...
	namespace Private
	{
		...
		void FCollisionConstraintAllocator::ProcessNewNonCcdConstraints(const TArray<FPBDCollisionConstraint*>& NewConstraints, TArray<FPBDCollisionConstraint*>& AllCCDConstraints)
		{
			...
			if (Chaos::ShouldExecuteTasks(NumConstraints))
			{
				const int32 NumTasks = FMath::Max(FMath::Min(FTaskGraphInterface::Get().GetNumWorkerThreads(), Chaos::MaxNumWorkers), 1);
				const int32 ConstraintByTask = FMath::Max(FMath::DivideAndRoundUp(NumConstraints, NumTasks), CollisionSmallBatchSize);
				...
				for (int32 BatchIndex = 0; BatchIndex < NumBatches; BatchIndex++)
				{
					...
					EndIndex = FMath::Min(NumConstraints, EndIndex);
		...
		void FCollisionConstraintAllocator::ProcessNewItems()
		{
			...
			const int32 NumConstraints = ContextAllocators.IsEmpty() ? 0 : ContextAllocators[0]->NewActiveConstraints.Num();
