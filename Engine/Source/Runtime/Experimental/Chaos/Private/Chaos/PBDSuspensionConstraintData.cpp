...
namespace Chaos
{
	...
	void FSuspensionConstraint::SetGroundBody(FPhysicsObjectHandle& InBody)
	{
		check(InBody);

		GroundBody.Modify(/*bInvalidate=*/true, DirtyFlags, Proxy, [InBody](FPhysicsObjectProperty& Data)
			{
				Data.PhysicsBody = InBody;
			});
	}
