...
namespace Chaos::CVars
{
	...
	/** Cvar to enable/disable level-based waking for all islands */
	bool bChaosSolverLevelBasedWake = true;
	FAutoConsoleVariableRef CVarChaosSolverLevelBasedWake(TEXT("p.Chaos.Solver.Sleep.PartialIslandSleep.LevelBasedWake"), bChaosSolverLevelBasedWake, TEXT("[EXPERIMENTAL] Safeguard only: Wake events will always propagate to higher levels, i.e. away from the closest kinematic.[def:true]"));
	...
	* 0: never prevent partial sleeping for non-collision constraints, i.e. all particles are allowed to sleep independent of their neighbors' sleep state and constraint types. 
	* 1: propagate partial wake events through non-collision constraints, AND post-solver re-waking of sleeping particles connected to an awake particle by non-collision constraint
	* -1: propagate partial wake events through non-collision constraints, NO post-solver re-waking of sleeping particles connected to an awake particle by non-collision constraint */
	int32 ChaosSolverPartialSleepCollisionConstraintsOnly = 1;
	FAutoConsoleVariableRef CVarChaosSolverPartialSleepCollisionConstraintsOnly(TEXT("p.Chaos.Solver.Sleep.PartialIslandSleep.CollisionConstraintsOnly"), ChaosSolverPartialSleepCollisionConstraintsOnly, TEXT("[EXPERIMENTAL] Safeguard only: Prevent partial island sleep between non-collision constraints (0: off, 1: re-wake sleeping particles connected to awake particles by non-collision constraints, -1: propagate wake events through non-collision constraints).[def:1]"));
...
namespace Chaos::Private
{
	...
	{
		// Print more information about remaining nodes to track down UE-296297
		checkf(Nodes.IsEmpty(), 
			TEXT("Number of nodes: %d, Nodes[0]: %s, Number of edges: %d"), 
			Nodes.Num(), 
			*(Nodes[0]->GetParticle()->GetDebugName()), 
			Nodes[0]->Edges.Num());
		check(NumEdges == 0);
		check(ArrayIndex == INDEX_NONE);
	...
	bool FPBDIslandManager::ShouldAssignLevels(const FPBDIsland* Island) const
	{
		// Partial island sleeping with level-based wake always needs levels
		return bAssignLevels || (Island->IsPartialSleepEnabled() && CVars::bChaosSolverLevelBasedWake);
	}
	...
	void FPBDIslandManager::UpdateParticles()
	{
		...
		// The above loop may leave orphaned nodes in the Nodes array when dynamics are made
		...
		for (int32 NodeIndex = Nodes.Num() - 1; NodeIndex >= 0; --NodeIndex)
		{
			...
			{
				// Every dynamic node needs to be in an island, non-dynamic nodes cannot be in an island.
				ensureMsgf((Node->Flags.bIsDynamic && (Node->Island != nullptr)) || (!Node->Flags.bIsDynamic && (Node->Island == nullptr)),
					TEXT("Node->Flags.bIsDynamic: %d, Node->Island: %d, Node->Edges.IsEmpty(): %d, DebugName: %s"), 
					Node->Flags.bIsDynamic, Node->Island != nullptr, Node->Edges.IsEmpty(), *(Node->GetParticle()->GetDebugName()));
			}
		}
	}
	...
	void FPBDIslandManager::RemoveParticleConstraints(FGeometryParticleHandle* Particle)
	{
		if (FPBDIslandParticle* Node = GetGraphNode(Particle))
		{
			...
			{
				// Print more information about remaining nodes to track down UE-297690
				checkf(EdgeIndex < Node->Edges.Num(),
					TEXT("Array index out of bounds: %d into an array of size %d, Node->GetDebugName() %s."),
					EdgeIndex, Node->Edges.Num(), 
					*(Node->GetParticle()->GetDebugName()));
	...
	FPBDIslandParticle* FPBDIslandManager::CreateGraphNode(FGeometryParticleHandle* Particle)
	{
		if (Particle != nullptr)
		{
			...
			UE_MT_SCOPED_WRITE_ACCESS(GlobalNodesAccessDetector);
	...
	void FPBDIslandManager::DestroyGraphNode(FPBDIslandParticle* Node)
	{
		if (Node != nullptr)
		{
			...
			check(Node->IslandArrayIndex == INDEX_NONE);
	...
	FPBDIslandConstraint* FPBDIslandManager::CreateGraphEdge(const int32 ContainerId, FConstraintHandle* Constraint, const TVec2<FGeometryParticleHandle*>& ConstrainedParticles)
	{
		...
		if (IsParticleDynamic(ConstrainedParticles[0]) || IsParticleDynamic(ConstrainedParticles[1]))
		{
			...
			UE_MT_SCOPED_WRITE_ACCESS(GlobalEdgesAccessDetector);
	...
	void FPBDIslandManager::DestroyGraphEdge(FPBDIslandConstraint* Edge)
	{
		if (Edge != nullptr)
		{
			...
			check(Edge->IslandArrayIndex == INDEX_NONE);
	...
	void FPBDIslandManager::BindEdgeToNodes(FPBDIslandConstraint* Edge, FPBDIslandParticle* Node0, FPBDIslandParticle* Node1)
	{
		...
		check(Edge->NodeArrayIndices[0] == INDEX_NONE);
		check(Edge->NodeArrayIndices[1] == INDEX_NONE);
		check(Edge->Island == nullptr);

		UE_MT_SCOPED_WRITE_ACCESS(LocalEdgeAccessDetector);
		UE_MT_SCOPED_WRITE_ACCESS(LocalNodeAccessDetector);
	...
	void FPBDIslandManager::UnbindEdgeFromNodes(FPBDIslandConstraint* Edge)
	{
		...
		for (int32 NodeIndex = 0; NodeIndex < 2; ++NodeIndex)
		{
			if (FPBDIslandParticle* Node = Edge->Nodes[NodeIndex])
			{
				...
				// These checks are supposed to help print out more information about the crashes in UE-215521, UE-296295, UE-314497
				// If the array index is out of bounds, we print out information about "Edge", i.e. the edge we intended to remove. 
				checkf(ArrayIndex < Node->Edges.Num(), 
					TEXT("Array index out of bounds: %d into an array of size %d, Node: %s, Edge->Type: %s, Edge->Node[0]: %s, Edge->Node[1]: %s."),
					ArrayIndex, Node->Edges.Num(),
					*(Node->GetParticle()->GetDebugName()), 
					*(Edge->GetConstraint()->GetType().ToString()), 
					Edge->Nodes[0] ? *(Edge->Nodes[0]->GetParticle()->GetDebugName()) : *(FString()),
					Edge->Nodes[1] ? *(Edge->Nodes[1]->GetParticle()->GetDebugName()) : *(FString()));
				// If the array index is within the bounds but does not correspond to the constraint we intended to remove, 
				// we print out information about "Edge", i.e. the edge we intended to remove, and the edge we currently reference "Node->Edges[ArrayIndex]".
				checkf(Node->Edges[ArrayIndex] == Edge, 
					TEXT("Selected edge does not correspond to ArrayIndex. Node: %s, Edge->Type: %s, Edge->Node[0]: %s, Edge->Node[1]: %s, Node->Edges[ArrayIndex]->Type: %s, Edges[ArrayIndex]->Node[0]: %s, Edges[ArrayIndex]->Node[1]: %s."), 
					*(Node->GetParticle()->GetDebugName()), 
					*(Edge->GetConstraint()->GetType().ToString()), 
					Edge->Nodes[0] ? *(Edge->Nodes[0]->GetParticle()->GetDebugName()) : *(FString()),
					Edge->Nodes[1] ? *(Edge->Nodes[1]->GetParticle()->GetDebugName()) : *(FString()),
					*(Node->Edges[ArrayIndex]->GetConstraint()->GetType().ToString()), 
					Node->Edges[ArrayIndex]->Nodes[0] ? *(Node->Edges[ArrayIndex]->Nodes[0]->GetParticle()->GetDebugName()) : *(FString()),
					Node->Edges[ArrayIndex]->Nodes[1] ? *(Node->Edges[ArrayIndex]->Nodes[1]->GetParticle()->GetDebugName()) : *(FString()));
				...
				if (ArrayIndex < Node->Edges.Num())
				{
					...
					{
						checkf(Node->Edges[ArrayIndex]->NodeArrayIndices[0] == Node->Edges.Num(), 
							TEXT("The node we swapped in was not at the end of the array. Node->Edges[ArrayIndex]->NodeArrayIndices[0]: %d, Node->Edges.Num(): %d, Node->DebugName: %s, Node->Edges[ArrayIndex]->Type: %s, Edges[ArrayIndex]->Node[0]: %s, Edges[ArrayIndex]->Node[1]: %s."),
							Node->Edges[ArrayIndex]->NodeArrayIndices[0], Node->Edges.Num(), 
							*(Node->GetParticle()->GetDebugName()),
							*(Node->Edges[ArrayIndex]->GetConstraint()->GetType().ToString()),
							Node->Edges[ArrayIndex]->Nodes[0] ? *(Node->Edges[ArrayIndex]->Nodes[0]->GetParticle()->GetDebugName()) : *(FString()),
							Node->Edges[ArrayIndex]->Nodes[1] ? *(Node->Edges[ArrayIndex]->Nodes[1]->GetParticle()->GetDebugName()) : *(FString()));
					...
					{
						checkf(Node->Edges[ArrayIndex]->NodeArrayIndices[1] == Node->Edges.Num(),
							TEXT("The node we swapped in was not at the end of the array. Node->Edges[ArrayIndex]->NodeArrayIndices[1]: %d, Node->Edges.Num(): %d, Node: %s, Node->Edges[ArrayIndex]->Type: %s, Edges[ArrayIndex]->Node[0]: %s, Edges[ArrayIndex]->Node[1]: %s."),
							Node->Edges[ArrayIndex]->NodeArrayIndices[1], Node->Edges.Num(), 
							*(Node->GetParticle()->GetDebugName()),
							*(Node->Edges[ArrayIndex]->GetConstraint()->GetType().ToString()),
							Node->Edges[ArrayIndex]->Nodes[0] ? *(Node->Edges[ArrayIndex]->Nodes[0]->GetParticle()->GetDebugName()) : *(FString()),
							Node->Edges[ArrayIndex]->Nodes[1] ? *(Node->Edges[ArrayIndex]->Nodes[1]->GetParticle()->GetDebugName()) : *(FString()));
					...
					{
						// If we get here, the list Node->Edges must be corrupted.
						checkf(false, 
							TEXT("Node %s was not found between Node->Edges[ArrayIndex]->Nodes[0] %s and Node->Edges[ArrayIndex]->Nodes[1] %s."),
							*(Node->GetParticle()->GetDebugName()), 
							*(Node->Edges[ArrayIndex]->Nodes[0]->GetParticle()->GetDebugName()),
							*(Node->Edges[ArrayIndex]->Nodes[1]->GetParticle()->GetDebugName()));
					}
				}
			}
		}
	...
	void FPBDIslandManager::AddNodeToIsland(FPBDIslandParticle* Node, FPBDIsland* Island)
	{
		...
		if (Node != nullptr)
		{
			...
			{
				UE_MT_SCOPED_WRITE_ACCESS(LocalNodeAccessDetector);
				UE_MT_SCOPED_WRITE_ACCESS(LocalIslandAccessDetector);
				Node->Island = Island;
	...
	void FPBDIslandManager::ProcessIslandSplits(FPBDIsland* Island)
	{
		...
		for (FPBDIslandParticle* RootNode : IslandNodes) //-V1078 7.39 Swap Bug
		{
	...
	void FPBDIslandManager::ProcessIslandSleep(FPBDIsland* Island, const FRealSingle Dt)
	{
		...
		for (FPBDIslandParticle* Node : Island->GetParticles())
		{
			...
			if ((Node->SleepLinearThresholdSq <= 0) && (Node->SleepAngularThresholdSq <= 0))
			{
				...
				// NOTE: We will not sleep if there is a particle with non-positive thresholds.
				// But we want to keep iterating to update MotionlessCount.
				continue;
			}
			...
			// The particle SleepType may have changed since the node cached thresholds
			// This is needed because we have API on the particle to SetSleepType which doesn't end up updating thresholds which is done when setting SleepType via FPBDRigidsEvolutionBase::SetParticleSleepType
			if (Rigid->SleepType() == ESleepType::NeverSleep)
			{
				bWithinSleepThreshold = false;
				// NOTE: We will not sleep if there is a never-sleeping particle.
				// But we want to keep iterating to update MotionlessCount.
				continue;
			}
	...
	void FPBDIslandManager::ProcessPartialIslandSleep(FPBDIsland* Island, const FRealSingle Dt)
	{
		...
		if (CVars::ChaosSolverPartialSleepCollisionConstraintsOnly >= 1)
		{
	...
	void FPBDIslandManager::ProcessPartialIslandWake(FPBDIsland* Island, const bool bProcessEdgeBasedWake)
	{
		...
		for (FPBDIslandParticle* Node : Island->Nodes)
		{
			// Partial wake events need to be performed for bAreSleepThresholdsNonPositive and bIsParticleNeverSleep.
			// Otherwise, we might end up with sleeping particles above a never-sleeping or zero-sleep-threshold particle.
			const bool bAreSleepThresholdsNonPositive = (Node->SleepLinearThresholdSq <= 0) && (Node->SleepAngularThresholdSq <= 0);
			bool bIsParticleNeverSleep = false;
			if (FPBDRigidParticleHandle* Rigid = Node->Particle->CastToRigidParticle())
			{
				bIsParticleNeverSleep = Rigid->SleepType() == ESleepType::NeverSleep;
			}

			if ((Node->Flags.bWasUpdated || bAreSleepThresholdsNonPositive || bIsParticleNeverSleep) && !Node->Flags.bWasProcessed)
			{
			...
			for (FPBDIslandConstraint* Edge : Node->Edges)
			{
				if (!Edge->Flags.bWasProcessed)
				{
					...
					if ((OtherNode != nullptr) && OtherNode->Flags.bIsDynamic)
					{
						...
						const bool bIsUpwardPropagation = ShouldAssignLevels(Edge->Island) ? OtherNode->Level >= Edge->Level : false;	// Propagation upward through the levels, i.e. away from closest kinematic
						const bool bEdgePropagatesPartialWake = (CVars::ChaosSolverPartialSleepCollisionConstraintsOnly != 0) && Edge->GetConstraint() && !Edge->GetConstraint()->SupportsPartialIslandSleeping();
