...
namespace Chaos
{
	...
	FPBDSuspensionConstraints::FConstraintContainerHandle* FPBDSuspensionConstraints::AddConstraint(TGeometryParticleHandle<FReal, 3>* Particle, const FVec3& InSuspensionLocalOffset, const FPBDSuspensionSettings& InConstraintSettings)
	{
		...
		CollisionSolvers[CollisionSolvers.Num() - 1].MakeInitialized();
	...
	void FPBDSuspensionConstraints::RemoveConstraint(int ConstraintIndex)
	{
		...
		GroundParticles.RemoveAtSwap(ConstraintIndex);
	...
	void FPBDSuspensionConstraints::DisconnectConstraints(const TSet<TGeometryParticleHandle<FReal, 3>*>& RemovedParticles)
	{
				...
				if (FPBDSuspensionConstraintHandle* SuspensionHandle = ConstraintHandle->As<FPBDSuspensionConstraintHandle>())
				{
					...
					if (ConstraintIndex != INDEX_NONE)
					{
						if (ConstrainedParticles[ConstraintIndex] == RemovedParticle)
						{
							...
							GroundParticles[ConstraintIndex] = nullptr;
						}
					}
				}
			}
	...
	void FPBDSuspensionConstraints::SetConstraintEnabled(int32 ConstraintIndex, bool bEnabled)
	{
		if (ConstraintIndex < ConstrainedParticles.Num())
		{
	...
	void FPBDSuspensionConstraints::SetGroundParticle(int32 ConstraintIndex, Chaos::FGeometryParticleHandle* InGroundParticle)
	{
		if (GroundParticles.IsValidIndex(ConstraintIndex))
		{
			if (GroundParticles[ConstraintIndex] != InGroundParticle)
			{
				if (FConstraintContainerHandle* Constraint = GetConstraintHandle(ConstraintIndex))
				{
					Constraint->SetGroundParticleChanged();
				}
			}

			GroundParticles[ConstraintIndex] = InGroundParticle;
		}
	}

	void FPBDSuspensionConstraints::AddConstraintsToGraph(Private::FPBDIslandManager& IslandManager)
	{
		for (FPBDSuspensionConstraintHandle* Constraint : GetConstraints())
		{
			const bool bIsInGraph = Constraint->IsInConstraintGraph();
			const bool bShouldBeInGraph = Constraint->IsEnabled();
			const bool bShouldUpdateGraph = Constraint->HasGroundParticleChanged();
			...
			else if (bIsInGraph && bShouldUpdateGraph)
			{
				IslandManager.RemoveConstraint(Constraint);
				IslandManager.AddConstraint(ContainerId, Constraint, Constraint->GetConstrainedParticles());
				Constraint->ClearGroundParticleChanged();
			}
		}
	}
	...
	{
		// FindOrAdd expects a valid pointer
		if (ConstrainedParticles[ConstraintIndex])
		{
			if (GroundParticles[ConstraintIndex])
			{
				ConstraintSolverBodies[ConstraintIndex] =
				{
					SolverBodyContainer.FindOrAdd(ConstrainedParticles[ConstraintIndex]),
					SolverBodyContainer.FindOrAdd(GroundParticles[ConstraintIndex])
				};
			}
			else
			{
	...
	void FPBDSuspensionConstraints::GatherInput(const int32 ConstraintIndex, const FReal Dt)
	{
		...
		// Hard-stop Collision Manifold Generation
		{
			...
			// this is the case if the raycast has not found a body to hit - can we disable constraint in that instance rather than running
			if (ConstraintSolverBodies[ConstraintIndex][1] == nullptr)
			{
				return;
			}
			...
			FSolverBody* Body0 = ConstraintSolverBodies[ConstraintIndex][0];	// vehicle chassis			
			FSolverBody* Body1 = ConstraintSolverBodies[ConstraintIndex][1];	// body that the wheel is touching

			bool bHitSelf = (Body0 == Body1);

			const FPBDSuspensionSettings& Setting = ConstraintSettings[ConstraintIndex];

			if (Body0->IsDynamic() && !bHitSelf && Setting.Enabled)
			{
				...
				const FVec3 WorldArm0 = BodyQ.RotateVector(SuspensionCoMOffset);
				const FVec3 WorldArm1 = WorldSpaceX - Body1->CorrectedP();
				...
				Solver->InitManifoldPoint(
					...
					FSolverVec3(WorldArm0),					// RelativeContactPosition0,
					FSolverVec3(WorldArm1),					// RelativeContactPosition1,
	...
	void FPBDSuspensionConstraints::ApplySingle(int32 ConstraintIndex, const FReal Dt)
	{
		...
		bool bHitSelf = (ConstraintSolverBodies[ConstraintIndex][0] == ConstraintSolverBodies[ConstraintIndex][1]);
		...
		if (Body0.IsDynamic() && !bHitSelf && Setting.Enabled)
		{
			...
			const FVec3 BodyP = Body0.CorrectedP();
			const FVec3 WorldSpaceX = BodyQ.RotateVector(SuspensionCoMOffset) + BodyP;
			const FRotation3 BodyQ2 = Body1.CorrectedQ();
			const FVec3 BodyP2 = Body1.CorrectedP();
			...
			{
				if (Body0.V().SquaredLength() < (SpeedThreshold * SpeedThreshold))
				{
					SurfaceNormal = FVec3(0.f, 0.f, 1.f);
				}
				else
				{
					const FReal Speed = FMath::Abs(Body0.V().Length());
			...
			const FVec3 V = FVec3::CalculateVelocity(Body0.X(), BodyP, Dt);
			const FVec3 W = FRotation3::CalculateAngularVelocity(Body0.R(), BodyQ, Dt);
...

			{
				...
				FReal DLambda = 0.f;
				{
					...
					const FReal SpringMassScale = (bAccelerationMode) ? FReal(1) / Body0.InvM() : FReal(1);
			...
			const FVec3 DP = Body0.InvM() * DX;
			const FVec3 DR = Body0.InvI() * FVec3::CrossProduct(Arm, DX);
			Body0.ApplyTransformDelta(DP, DR);
			Body0.UpdateRotationDependentState();

			if (Body1.IsDynamic())
			{
				const FVec3 Arm2 = WorldSpaceX - BodyP2;
				const FVec3 DP2 = Body1.InvM() * -DX;
				const FVec3 DR2 = Body1.InvI() * FVec3::CrossProduct(Arm2, -DX);
				Body1.ApplyTransformDelta(DP2, DR2);
