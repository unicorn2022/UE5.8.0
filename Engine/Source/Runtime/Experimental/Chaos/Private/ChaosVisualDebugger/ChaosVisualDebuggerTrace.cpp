...
static_assert(TRACE_BASED_DEBUGGERS_WITHOUT_TRACE || UE_TRACE_MINIMAL_ENABLED || UE_TRACE_ENABLED,
	"Chaos Visual Debugger support requires Trace or Trace minimal support."
	" Please enable trace support with the Unreal Build Tool flag bEnableTrace, disable cvd support with bCompileChaosVisualDebuggerSupport"
	" or allows CVD support without trace by defining TRACE_BASED_DEBUGGERS_WITHOUT_TRACE to 1");
...
UE_TRACE_MINIMAL_CHANNEL_DEFINE(ChaosVDChannel, "Chaos physics solver frames, simulation steps, particles, constraints (collision, joints, character ground), scene queries, implicit geometry, and debug shapes. Required for the Chaos Visual Debugger (CVD) tool.");
...
UE_TRACE_MINIMAL_EVENT_DEFINE(ChaosVDLogger, ChaosVDMiscHeaderData)
...
namespace Chaos::VisualDebugger::Cvars
{
	...
	CompressionMode,
	TEXT("Oodle compression mode to use, 4 is by default which equals to ECompressionLevel::VeryFast"));

	static bool bUsesSharedGeometryArchiveContext = true;
	FAutoConsoleVariableRef CVarChaosVDUsesSharedGeometryArchiveContext(
	TEXT("p.Chaos.VD.UseSharedGeometryArchiveContext"),
	bUsesSharedGeometryArchiveContext,
	TEXT("If set to true, CVD will serialize collision geometry using a shared archive context to de-duplicate geometry during serialization instead of during deserialization"));
}
...
namespace Chaos::VD
{
	...
	bIsTracing(false), ArchiveContext(MakeShared<FChaosArchiveContext>())
	{
	}
...
void FChaosVisualDebuggerTrace::TraceParticle(Chaos::FGeometryParticleHandle* ParticleHandle, const FChaosVDContext& ContextData)
{
	...
	if (!ParticleHandle)
	{
		UE_LOGF(LogChaos, Warning, "Tried to Trace a null particle %s", __FUNCTION__);
		return;
	}
...
void FChaosVisualDebuggerTrace::TraceCamera(AActor* Object, UWorld* World, FName CameraName, const FVector& Position, const FQuat& Rotation, float FOV)
{
	using namespace Chaos::VisualDebugger::Utils;
	if (!Object || !IsTracing() || !CVDDC_CameraPaths->IsChannelEnabled())
	{
		return;
	}

	FChaosVDCameraDataWrapper DataWrapper;

	DataWrapper.Camera.CameraName = CameraName;
	DataWrapper.Camera.ActorName = Object->GetFName();
	DataWrapper.Camera.ActorAssetPath = FSoftObjectPath(Object->GetClass()).GetAssetPath();
	DataWrapper.Camera.MapAssetPath = FTopLevelAssetPath(World);
	DataWrapper.Position = Position;
	DataWrapper.Rotation = Rotation;
	DataWrapper.FOV = FOV;

	FChaosVDScopedTLSBufferAccessor TLSDataBuffer;
	Chaos::VisualDebugger::WriteDataToBuffer(TLSDataBuffer.BufferRef, DataWrapper);

	TraceBinaryData(TLSDataBuffer.BufferRef, FChaosVDCameraDataWrapper::WrapperTypeName);
}
...
uint32 FChaosVisualDebuggerTrace::TraceImplicitObject(Chaos::FImplicitObject* GeometryPtr)
{
	...
	if (SessionState.UsingSharedGeometryArchiveContext)
	{
		// When we use the shared archive, geometry serialization happens essentially in serial
		// And we need to make sure we unlock the context only after the geometry data was written to the trace stream, otherwise
		// during deserialization we might try to deserialize it before it is loaded. The reason is that if we read an object tag in the context but we don't have the object
		// Chaos Archive context assumes the data that follows is the serialized object.

		// The perf impact of doing this serially is mitigated by serializing way less data (as everything that is instanced is serialized only once)
		// and by the fact whe only attempt to serialize a ptr once. Every subsequent attempt earlies out as this geometry hash will be already cached in TracedGeometryHashCache
		// and that cache can be accessed in parallel

		UE::TScopeLock ScopeLock(SessionState.ArchiveContextLock);
		Chaos::VisualDebugger::WriteDataToBuffer<FChaosVDImplicitObjectWrapper>(TLSDataBuffer.BufferRef, GeometryDataWrapper, SessionState.ArchiveContext);
...
void FChaosVisualDebuggerTrace::InvalidateGeometryFromCache(const Chaos::FImplicitObject* CachedGeometryToInvalidate)
{
	...
	{
		UE::TScopeLock ContextScopeLock(SessionState.ArchiveContextLock);

		// TODO: Should we walk the object hierarchy and remove all inner objects?
		SessionState.ArchiveContext->ObjToTag.Remove(CachedGeometryToInvalidate);
	}
...
{
	{
		...
		{
			SessionState.RecordingStartedDelegateHandle = FChaosVDRuntimeModule::RegisterRecordingStartedCallback(FRecordingStateChangedDelegate::FDelegate::CreateStatic(&FChaosVisualDebuggerTrace::HandleRecordingStart));
		}

		if (!SessionState.RecordingStoppedDelegateHandle.IsValid())
		{
			SessionState.RecordingStoppedDelegateHandle = FChaosVDRuntimeModule::RegisterRecordingStopCallback(FRecordingStateChangedDelegate::FDelegate::CreateStatic(&FChaosVisualDebuggerTrace::HandleRecordingStop));
		}
...
void FChaosVisualDebuggerTrace::Reset()
{
	...
	SessionState.ArchiveContext = MakeShared<Chaos::FChaosArchiveContext>();
	SessionState.UsingSharedGeometryArchiveContext = Chaos::VisualDebugger::Cvars::bUsesSharedGeometryArchiveContext;
}
...
{
	UE_LOGF(LogChaos, Log, "[%ls] Channel list override provided - Enabling [%d] Requested channels...", ANSI_TO_TCHAR(__FUNCTION__), EnabledDataChannelsOverrideList.Num());

	using namespace Chaos::VisualDebugger;
	FChaosVDDataChannelsManager::Get().EnumerateDataChannels([&EnabledDataChannelsOverrideList](const TWeakPtr<FChaosVDOptionalDataChannel>& Channel)
	{
		if (TSharedPtr<FChaosVDOptionalDataChannel> LockedChannel = Channel.Pin())
		{
			if (LockedChannel->CanChangeEnabledState())
			{
				...
				const FString ChannelIdAsString = LockedChannel->GetId().ToString();
				const bool bChannelShouldBeEnabled = EnabledDataChannelsOverrideList.Contains(ChannelIdAsString);
				LockedChannel->SetChannelEnabled(bChannelShouldBeEnabled);

				UE_LOGF(LogChaos, Log, "[%ls] Setting enabled state for channel [%ls] to [%ls]...", ANSI_TO_TCHAR(__FUNCTION__), *ChannelIdAsString, bChannelShouldBeEnabled ? TEXT("True") : TEXT("False"));
			}
		}
		return true;
	});
...
void FChaosVisualDebuggerTrace::RegisterExternalParticleDebugNameGenerator(const Chaos::VD::FRecordingSessionState::FParticleMetaDataGeneratorDelegate& InCallback)
{
	if (SessionState.ExternalParticleMetadataGenerator.IsBound())
	{
		UE_LOGF(LogChaos, Error, "An external debug name generator for particles is already registered. Ignoring new register attempt");
...
void FChaosVisualDebuggerTrace::HandleRecordingStart()
{
	...
	UE_TRACE_MINIMAL_LOG(ChaosVDLogger, ChaosVDMiscHeaderData, ChaosVDChannel)
		<< ChaosVDMiscHeaderData.bUsesSharedGeometryContextArchive(SessionState.UsingSharedGeometryArchiveContext);
