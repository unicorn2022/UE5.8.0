...
FChaosVDJointConstraint FChaosVDDataWrapperUtils::BuildJointDataWrapper(const Chaos::FPBDJointConstraintHandle* ConstaintHanlde)
{
	...
	if (ConstaintHanlde)
	{
		...
		if (Chaos::FGeometryParticleHandle* Particle0 = ConstaintHanlde->GetConstrainedParticles()[0])
		{
			WrappedJointData.ParticleParIndexes[0] = Particle0->UniqueIdx().Idx;
		}
		if (Chaos::FGeometryParticleHandle* Particle1 = ConstaintHanlde->GetConstrainedParticles()[1])
		{
			WrappedJointData.ParticleParIndexes[1] = Particle1->UniqueIdx().Idx;
		}
...
void FChaosVDDataWrapperUtils::CopyShapeDataToWrapper(const Chaos::FShapeInstancePtr& ShapeDataPtr, FChaosVDShapeCollisionData& OutCopyTo)
{
	...
	const Chaos::Filter::FInstanceData InstanceData = CollisionData.GetFilterInstanceData();
	const Chaos::Filter::FShapeFilterData ShapeFilter = CollisionData.GetShapeFilterData();

	OutCopyTo.FilterData.OwnerId = InstanceData.GetOwnerId();
	OutCopyTo.FilterData.ComponentId = InstanceData.GetComponentId();
	OutCopyTo.FilterData.OverlapMask = ShapeFilter.GetOverlapChannels();
	OutCopyTo.FilterData.BlockMask = ShapeFilter.GetBlockChannels();
	OutCopyTo.FilterData.CollisionChannelMask = ShapeFilter.GetCollisionChannelMask();
	OutCopyTo.FilterData.SetMaskFilterAndFlags(ShapeFilter.GetMaskFilter(), (uint32)ShapeFilter.GetFlags());
}
...
void FChaosVDDataWrapperUtils::BuildDataWrapperFromAABBStructure(const Chaos::ISpatialAccelerationCollection<Chaos::FAccelerationStructureHandle, Chaos::FReal, 3>* SceneAccelerationStructures, int32 OwnerSolverID, TArray<FChaosVDAABBTreeDataWrapper>& OutAABBTrees)
{
	...
	for (const FSpatialAccelerationIdx SpatialIndex : SpatialIndices)
	{
		...
		if (const TAABBTree<FAccelerationStructureHandle, TAABBTreeLeafArray<FAccelerationStructureHandle>>* AABBTree = AccelerationStructure->template As<TAABBTree<FAccelerationStructureHandle, TAABBTreeLeafArray<FAccelerationStructureHandle>>>())
		{
			...
			if (AABBTree->DirtyElementTree)
			{
				FChaosVDAABBTreeDataWrapper DirtyTreeWrapper;
				DirtyTreeWrapper.SolverId = OwnerSolverID;
				BuildDataWrapperFromAABBStructure(*AABBTree->DirtyElementTree, DirtyTreeWrapper);
				OutAABBTrees.Emplace(DirtyTreeWrapper);
			}
		}
		else if (const TAABBTree<FAccelerationStructureHandle, TBoundingVolume<FAccelerationStructureHandle>>* AABBTreeBV = AccelerationStructure->template As<TAABBTree<FAccelerationStructureHandle, TBoundingVolume<FAccelerationStructureHandle>>>())
		{
			...
			if (AABBTreeBV->DirtyElementTree)
			{
				FChaosVDAABBTreeDataWrapper DirtyTreeWrapper;
				DirtyTreeWrapper.SolverId = OwnerSolverID;
				BuildDataWrapperFromAABBStructure(*AABBTreeBV->DirtyElementTree, DirtyTreeWrapper);
				OutAABBTrees.Emplace(DirtyTreeWrapper);
			}
		}
	}
