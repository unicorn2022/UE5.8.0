						...
						for (int32 Index = StartRangeIndex; Index < EndRangeIndex; ++Index)
						{
							...
							if (ensureMsgf(!Constraint.AccumulatedImpulse.ContainsNaN(), TEXT("Constraint contains a NaN within the accumulated impulse.")))
							{
								...
								if (ensureMsgf(FMath::IsFinite(Constraint.GetPhi()), TEXT("Constraint Phi is not finite.")))
								{
									...
									const bool bFilter0Notify = Shape0 ? Shape0->GetShapeFilterData().HasFlag(EFilterFlags::ContactNotify) : true;
									const bool bFilter1Notify = Shape1 ? Shape1->GetShapeFilterData().HasFlag(EFilterFlags::ContactNotify) : true;
						...
						ActiveParticle->GetGeometry()->HasBoundingBox())
					{
							...
							FMath::IsFinite(ActiveParticle->M())))
						{
							...
							if (TrailingData.Proxy->GetType() == EPhysicsProxyType::GeometryCollectionType)
							{
								...
								TrailingData.SetEmitterFlag(ConcreteProxy->GetSimParameters().bGenerateTrailingData, ConcreteProxy->GetSimParameters().bGenerateGlobalTrailingData);
							}
							else
							{
							...
							{
								const int32 NewIdx = AllTrailingsDataArray.Emplace(TrailingData);
								...
								TrailingEventData.TrailingData.bHasGlobalEvent |= (TrailingData.EmitterFlag & EventEmitterFlag::GlobalDispatcher) != 0;
							}
						}
					}
				}
