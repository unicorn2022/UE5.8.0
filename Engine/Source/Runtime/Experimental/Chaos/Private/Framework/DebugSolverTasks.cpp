...
namespace Chaos
{
	...
	void FDebugSolverTasks::DebugStep(FPhysicsSolverBase* Solver, TFunction<void()> StepFunction)
	{
		...
		if (bIsDebugThreadRunning != bNeedsDebugThreadRunning)
		{
			...
			{
				UE_LOGF(LogChaosThread, Verbose, "[Physics Thread] Spawning new debug thread");
			...
			{
				// Delete AsyncTask
				UE_LOGF(LogChaosThread, Verbose, "[Physics Thread] Removing debug thread");
				(*DebugSolverTask)->EnsureCompletion(false);
				delete *DebugSolverTask;
				*DebugSolverTask = nullptr;
				UE_LOGF(LogChaosThread, Verbose, "[Physics Thread] Completed and deleted debug thread");
			}
		}
	...
	void FDebugSolverTasks::Add(FPhysicsSolverBase* Solver)
	{
		...
		UE_LOGF(LogChaosThread, Verbose, "[Physics Thread] Initializing debug thread");
	...
	void FDebugSolverTasks::Remove(FPhysicsSolverBase* Solver)
	{
		...
		if (bFound && DebugSolverTask)
		{
			...
			UE_LOGF(LogChaosThread, Verbose, "[Physics Thread] Shutting down debug thread");
			...
			UE_LOGF(LogChaosThread, Verbose, "[Physics Thread] Deleting debug async task");
			DebugSolverTask->EnsureCompletion(false);
			delete DebugSolverTask;
		}
#endif
	...
	void FDebugSolverTasks::Shutdown()
	{
		...
		for (auto& SolverTask: SolverToTaskMap)
		{
			...
			UE_LOGF(LogChaosThread, Verbose, "[Physics Thread] Shutting debug thread");
