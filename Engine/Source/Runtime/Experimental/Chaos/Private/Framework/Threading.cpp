...
namespace Chaos
{
	...
	namespace ThreadingPrivate
	{
		...
		void CheckLockReadAssumption(const TCHAR* Context)
		{
			...
			{
				UE_LOGF(LogChaos, Error, "Thread %u called %ls without taking a read lock. Wrap the function call in a scene read lock to safely read data.", FPlatformTLS::GetCurrentThreadId(), Context);
				CHAOS_ENSURE(false);
			}
		}
		...
		void CheckLockWriteAssumption(const TCHAR* Context)
		{
			...
			{
				UE_LOGF(LogChaos, Error, "Thread %u called %ls without taking a write lock. Wrap the function call in a scene write lock to safely write data.", FPlatformTLS::GetCurrentThreadId(), Context);
				CHAOS_ENSURE(false);
			}
		}
#endif
