...
void PushToPhysicsStateImp(const Chaos::FDirtyPropertiesManager& Manager, Chaos::FGeometryParticleHandle* Handle, int32 DataIdx, const Chaos::FDirtyProxy& Dirty, Chaos::FShapeDirtyData* ShapesData, Chaos::FPBDRigidsSolver& Solver, bool bResimInitialized, Chaos::FReal ExternalDt)
{
...
#endif
	{
		...
		// move the copied game thread data into the handle
		{
			...
			bool bSkipDirtyParticle = false;
			if (NewKinematicTargetGT)
			{
				bSkipDirtyParticle = Evolution.SetParticleKinematicTarget(KinematicHandle, *NewKinematicTargetGT);
			}
			...
			if(NewXR || NewNonFrequentData || NewVelocities || NewKinematicTargetGT)
			{
				...
				// Kinematic target added the particle in the active list or in the moving kinematic list and so it will be updated
				// No need to added into the dirty list 
				if (!bSkipDirtyParticle)
				{
	...
	if (Rigid)
	{
		...
		if (NextPullData)
		{
			...
			if (bUpdatePositionFromSimulation)
			{
				...
				{
					FVec3 Target = FMath::Lerp(*Prev, NextPullData->X, LerpAlpha);
					if (bIsReplicationErrorSmoothing)
					{
						...
						Target += InterpData->GetErrorX(LerpAlpha);
						...
						if (InterpData->IsErrorVelocitySmoothing())
						{
							...
							if (RenderInterpolationCVars::bRenderInterpDebugDraw)
							{
								...
								Chaos::FDebugDrawQueue::GetInstance().DrawDebugDirectionalArrow(ZOffset + (Target - InterpData->GetErrorX(LerpAlpha)), ZOffset + Target, 1, FColor::Blue, false, 5.0f, 0, 0.5f);
								Chaos::FDebugDrawQueue::GetInstance().DrawDebugDirectionalArrow(ZOffset + InterpData->GetErrorVelocitySmoothingX(LerpAlpha), ZOffset + Target, 1, FColor::Blue, false, 5.0f, 0, 0.5f);
							...
							const int32 RenderInterpErrorVelocitySmoothingDurationTicks = FMath::FloorToInt32(RenderInterpolationCVars::RenderInterpErrorVelocitySmoothingDuration / AsyncFixedTimeStep); // Convert duration from seconds to simulation ticks
							Target = FMath::Lerp(Target, InterpData->GetErrorVelocitySmoothingX(LerpAlpha), InterpData->GetErrorVelocitySmoothingAlpha(RenderInterpErrorVelocitySmoothingDurationTicks));
						}
					}
				...
				{
					FQuat Target = FMath::Lerp(*Prev, NextPullData->R, LerpAlpha);
					if (bIsReplicationErrorSmoothing)
					{
						Target = Target * InterpData->GetErrorR(LerpAlpha); // Add rotational error offset in local space
					}
				...
				if (RenderInterpolationCVars::bRenderInterpDebugDraw)
				{
					...
					if (bIsReplicationErrorSmoothing)
					{
						...
						if (InterpData->IsErrorVelocitySmoothing())
						{
							Chaos::FDebugDrawQueue::GetInstance().DrawDebugBox(ZOffset + InterpData->GetErrorVelocitySmoothingX(LerpAlpha), FVector(2, 2, 2), Rigid->R(), FColor::Purple, false, 5.f, 0, 0.5f);
						}
						else
						{
							Chaos::FDebugDrawQueue::GetInstance().DrawDebugDirectionalArrow(ZOffset + (Rigid->X() - InterpData->GetErrorX(LerpAlpha)), ZOffset + Rigid->X(), 1, FColor::Blue, false, 5.0f, 0, 0.5f);
						}
					}
				}
#endif // CHAOS_DEBUG_DRAW
			...
			{
				FVec3 Target = FMath::Lerp(*Prev, NextPullData->V, LerpAlpha);
				Rigid->SetV(Target, false);
			}

			if (const FVec3* Prev = LerpHelper(PullData.W, ProxyTimestamp->OverWriteW))
			{
				FVec3 Target = FMath::Lerp(*Prev, NextPullData->W, LerpAlpha);
			...
			else if(ProxyTimestamp->ObjectStateTimestamp == SolverSyncTimestamp && LerpAlpha == 1.f)
			{
...
void FRigidBodyHandle_External::SetShapeFilterData(int32 InShapeIndex, const Filter::FShapeFilterData& FilterData)
{
	VerifyContext();
	GetParticle_LowLevel()->SetShapeFilterData(InShapeIndex, FilterData);
}

void FRigidBodyHandle_External::SetShapeFilterInstanceData(int32 InShapeIndex, const Filter::FInstanceData& InstanceData)
{
	VerifyContext();
	GetParticle_LowLevel()->SetShapeFilterInstanceData(InShapeIndex, InstanceData);
}
