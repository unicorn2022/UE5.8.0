...
void FSuspensionConstraintPhysicsProxy::UpdateTargetOnPhysicsThread(FPBDRigidsSolver* RBDSolver, const FVector& TargetPos, const IPhysicsProxyBase* GroundProxy, const FVector& Normal, bool Enabled)
{
	if (Constraint_PT && Constraint_PT->IsValid())
	{
		auto& SuspensionConstraints = RBDSolver->GetSuspensionConstraints();
		...
		if (GroundProxy)
		{
			if (GroundProxy->GetType() == EPhysicsProxyType::SingleParticleProxy)
			{
				GroundParticle = ((FSingleParticlePhysicsProxy*)GroundProxy)->GetHandle_LowLevel();
			}
		}

		SuspensionConstraints.SetGroundParticle(Constraint_PT->GetConstraintIndex(), GroundParticle);
	}
}
