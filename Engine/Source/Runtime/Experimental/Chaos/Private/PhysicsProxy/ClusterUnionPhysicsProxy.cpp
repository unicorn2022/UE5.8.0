...
namespace Chaos
{
	...
	static int32 GFixLockedChildXROnClusterUnionPush = 0;
	FAutoConsoleVariableRef CVar_FixLockedChildXROnClusterUnionPush(
		TEXT("p.Chaos.ClusterUnion.FixLockedChildXROnPush"),
		GFixLockedChildXROnClusterUnionPush,
		TEXT("If 1, after a cluster union receives a parent XR push, re-align locked children's cached X/R to ChildToParent * NewParentTM. Fixes a wrong-CoM bug when a child Remove lands on the same tick as a parent XR replication."));
	...
	namespace
	{
		...
		void BufferPhysicsResultsImp(FClusterUnionPhysicsProxy* Proxy, TParticle* Particle, FDirtyClusterUnionData& BufferData)
		{
			...
			BufferData.ShapeFilterData.Empty(ShapeArray.Num());
			BufferData.ShapeInstanceData.Empty(ShapeArray.Num());

			for (const TUniquePtr<Chaos::FPerShapeData>& ShapeData : ShapeArray)
			{
				BufferData.CollisionData.Add(ShapeData->GetCollisionData());
				BufferData.ShapeFilterData.Add(ShapeData->GetShapeFilterData());
				BufferData.ShapeInstanceData.Add(ShapeData->GetFilterInstanceData());
			}
	...
	ESleepType FClusterUnionPhysicsProxy::GetSleepType_External() const
	{
		if (!ensure(Particle_External))
		{
		...
		return Particle_External->SleepType();
	}

	void FClusterUnionPhysicsProxy::SetSleepType_External(const ESleepType SleepType)
	{
		if (!Solver || !ensure(Particle_External))
		{
			return;
		...
		Solver->EnqueueCommandImmediate(
			[this, SleepType]() mutable
			{
				if (Particle_Internal)
				{
					if (Particle_Internal->SleepType() != SleepType)
					{
						if (FPBDRigidsEvolutionGBF* Evolution = GetEvolution(this))
						{
							Evolution->SetParticleSleepType(Particle_Internal, SleepType);
						}
					}
				}
			}
	...
	bool FClusterUnionPhysicsProxy::GetDisabled_External() const
	{
		if (!ensure(Particle_External))
		{
			return false;
		...
		return Particle_External->Disabled();
	}

	void FClusterUnionPhysicsProxy::SetDisabled_External(const bool Disabled)
	{
		if (!Solver || !ensure(Particle_External))
		{
			return;
		...
		Solver->EnqueueCommandImmediate(
			[this, Disabled]() mutable
			{
				if (Particle_Internal)
				{
					if (Particle_Internal->Disabled() != Disabled)
					{
						if (FPBDRigidsEvolutionGBF* Evolution = GetEvolution(this))
						{
							if (Disabled)
							{
								Evolution->DisableParticle(Particle_Internal);
							}
							else
							{
								Evolution->EnableParticle(Particle_Internal);
							}
						}
					}
				}
	...
	bool FClusterUnionPhysicsProxy::GetGravityEnabled_External() const
	{
		if (!ensure(Particle_External))
		{
			return false;
		...
		return Particle_External->GravityEnabled();
	}

	void FClusterUnionPhysicsProxy::SetGravityEnabled_External(const bool Enabled)
	{
		if (!Solver || !ensure(Particle_External))
		{
			return;
		...
		Solver->EnqueueCommandImmediate(
			[this, Enabled]() mutable
			{
				if (Particle_Internal)
				{
					Particle_Internal->SetGravityEnabled(Enabled);
				}
			}
		);
	}
	...
	void FClusterUnionPhysicsProxy::PushToPhysicsState(const FDirtyPropertiesManager& Manager, int32 DataIdx, const FDirtyProxy& Dirty)
	{
		...
		if (const FParticlePositionRotation* NewXR = ParticleData.FindClusterXR(Manager, DataIdx))
		{
			...
			// Re-align LOCKED children's cached X/R to the new parent transform.
			// Their ChildToParent is authoritative; their cached world X/R must follow the
			// parent so that UpdateClusterMassProperties (which derives ChildToParent from
			// current X/R) doesn't read a stale value relative to the just-updated parent
			// and bake a wrong CoM offset on a same-tick Remove.
			if (GFixLockedChildXROnClusterUnionPush)
			{
				if (FClusterUnion* LockedUnion = ClusterUnionManager.FindClusterUnion(ClusterUnionIndex); LockedUnion && !LockedUnion->bNeedsXRInitialization)
				{
					for (FPBDRigidParticleHandle* Particle : LockedUnion->ChildParticles)
					{
						FPBDRigidClusteredParticleHandle* ClusterParticle = Particle ? Particle->CastToClustered() : nullptr;
						if (!ClusterParticle || !ClusterParticle->IsChildToParentLocked())
						{
							continue;
						}

						const FRigidTransform3 NewChildWorldTM = ClusterParticle->ChildToParent() * NewTransform;
						ClusterParticle->SetX(NewChildWorldTM.GetTranslation());
						ClusterParticle->SetP(NewChildWorldTM.GetTranslation());
						ClusterParticle->SetR(NewChildWorldTM.GetRotation());
						ClusterParticle->SetQ(NewChildWorldTM.GetRotation());

						Evolution.GetParticles().MarkTransientDirtyParticle(ClusterParticle);
					}
				}
			}
	...
	bool FClusterUnionPhysicsProxy::PullFromPhysicsState(const FDirtyClusterUnionData& PullData, int32 SolverSyncTimestamp, const FDirtyClusterUnionData* NextPullData, const FRealSingle* Alpha, const FDirtyRigidParticleReplicationErrorData* Error, const Chaos::FReal AsyncFixedTimeStep)
	{
		...
		for (int32 ShapeIndex = 0; ShapeIndex < ShapeArray.Num(); ++ShapeIndex)
		{
			if (const TUniquePtr<Chaos::FPerShapeData>& ShapeData = ShapeArray[ShapeIndex])
			{
				...
				if (CurrentPullData.ShapeFilterData.IsValidIndex(ShapeIndex))
				{
					ShapeData->SetShapeFilterData(CurrentPullData.ShapeFilterData[ShapeIndex]);
				}

				if (CurrentPullData.ShapeInstanceData.IsValidIndex(ShapeIndex))
				{
					ShapeData->SetFilterInstanceData(CurrentPullData.ShapeInstanceData[ShapeIndex]);
				}
			}
		}
		...
		if (NextPullData)
		{
			...
			if (const FVec3* Prev = LerpHelper(PullData.X, ProxyTimestamp->OverWriteX))
			{
				...
				const FVec3 NewX = bIsReplicationErrorSmoothing ?
					FMath::Lerp(*Prev, NextPullData->X, LerpAlpha) + InterpData->GetErrorX(LerpAlpha) :
					FMath::Lerp(*Prev, NextPullData->X, LerpAlpha);
			...
			if (const FQuat* Prev = LerpHelper(PullData.R, ProxyTimestamp->OverWriteR))
			{
				const FQuat NewR = bIsReplicationErrorSmoothing ? 
					FMath::Lerp(*Prev, NextPullData->R, LerpAlpha) * InterpData->GetErrorR(LerpAlpha) :
					FMath::Lerp(*Prev, NextPullData->R, LerpAlpha); // Add rotational error offset in local space
			...
			{
				const FVec3 NewV = FMath::Lerp(*Prev, NextPullData->V, LerpAlpha);
				Particle_External->SetV(NewV, false);
			}

			if (const FVec3* Prev = LerpHelper(PullData.W, ProxyTimestamp->OverWriteW))
			{
				const FVec3 NewW = FMath::Lerp(*Prev, NextPullData->W, LerpAlpha);
			...
			if (RenderInterpolationCVars::bRenderInterpDebugDraw)
			{
				...
				if (bIsReplicationErrorSmoothing)
				{
					...
					Chaos::FDebugDrawQueue::GetInstance().DrawDebugDirectionalArrow(ZOffset + (Particle_External->GetX() - InterpData->GetErrorX(LerpAlpha)), ZOffset + Particle_External->GetX(), 1, FColor::Blue, false, 5.0f, 0, 0.5f);
				}
			}
