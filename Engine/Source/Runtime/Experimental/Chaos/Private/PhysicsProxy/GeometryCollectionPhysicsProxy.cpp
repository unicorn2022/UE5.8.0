...
float GeometryCollectionLinearVelocityUpdateTolerance = UE_KINDA_SMALL_NUMBER;
FAutoConsoleVariableRef CVarGeometryCollectionLinearVelocityUpdateTolerance(
	TEXT("p.GeometryCollection.LinearVelocityUpdateTolerance"),
	GeometryCollectionLinearVelocityUpdateTolerance,
	TEXT("Tolerance to detect if particle linear velocity has changed has changed when syncing PT to GT"));

float GeometryCollectionAngularVelocityUpdateTolerance = UE_KINDA_SMALL_NUMBER;
FAutoConsoleVariableRef CVarGeometryCollectionAngularVelocityUpdateTolerance(
	TEXT("p.GeometryCollection.AngularVelocityUpdateTolerance"),
	GeometryCollectionAngularVelocityUpdateTolerance,
	TEXT("Tolerance to detect if particle angular velocity has changed has changed when syncing PT to GT"));
...
bool bUpdateGTParticlesOnWorldTransformUpdate = false;
FAutoConsoleVariableRef CVarUpdateGTParticlesOnWorldTransformUpdate(
	TEXT("p.GeometryCollection.UpdateGTParticlesOnWorldTransformUpdate"),
	bUpdateGTParticlesOnWorldTransformUpdate,
	TEXT("If enabled, SetWorldTransform_External will immediately update the GT particle to reflect the new transform instead of waiting for a full GT -> PT -> GT sync to do so"));
...
void SetImplicitToPTParticles(Chaos::TPBDRigidParticleHandle<Chaos::FReal, 3>* Handle,
	...
	const Chaos::Filter::FShapeFilterData& ShapeFilterDataIn,
	const Chaos::Filter::FInstanceData& ShapeInstanceDataIn,
	...
	float CollisionParticlesPerObjectFraction)
{
	...
	if (GeometryCollectionCollideAll) // cvar
	{
		...
		const Chaos::Filter::FShapeFilterData ShapeFilterData = Chaos::Filter::FShapeFilterBuilder::BuildBlockAll(Chaos::EFilterFlags::All);
	...
	else
	{
		for (const TUniquePtr<Chaos::FPerShapeData>& Shape : Handle->ShapesArray())
		{
			Shape->SetShapeFilterData(ShapeFilterDataIn);
			Shape->SetFilterInstanceData(ShapeInstanceDataIn);
		}
	}
}
	...
	float CollisionParticlesPerObjectFraction)
{
	...
	SetImplicitToPTParticles(Handle, SharedParams, Simplicial, Implicit, ShapeFilterDataIn, ShapeInstanceDataIn, WorldTransform, CollisionParticlesPerObjectFraction);
...
#endif
{
	Chaos::Filter::FCombinedShapeFilterData CombinedShapeFilter = Chaos::Filter::FShapeFilterBuilder::BuildFromLegacyShapeFilter(InQueryFilter, InSimFilter);
	ShapeFilterData = CombinedShapeFilter.GetShapeFilterData();
	ShapeInstanceData = CombinedShapeFilter.GetInstanceData();
...
FGeometryCollectionPhysicsProxy::FGeometryCollectionPhysicsProxy(
	...
	TSharedRef<FGeometryDynamicCollection> GameThreadCollectionSharedRefIn,
	const FSimulationParameters& SimulationParameters,
	const Chaos::Filter::FShapeFilterData& InShapeFilterData,
	const Chaos::Filter::FInstanceData& InShapeInstanceData,
	FGuid InCollectorGuid,
	const Chaos::EMultiBufferMode BufferMode)
	: Base(InOwner)
	, Parameters(SimulationParameters)
	...
	, IsObjectDeleting(false)
	, bSkipChildToParentUpdateWhenInClusterUnion(false)
	, ShapeFilterData(InShapeFilterData)
	, ShapeInstanceData(InShapeInstanceData)
	, PhysicsThreadCollection(Parameters.RestCollectionShared)
	, GameThreadCollection(GameThreadCollectionSharedRefIn.Get())
	, GameThreadCollectionSharedPtr(GameThreadCollectionSharedRefIn.ToSharedPtr())
#if WITH_EDITORONLY_DATA
...
void FGeometryCollectionPhysicsProxy::Initialize(Chaos::FPBDRigidsEvolutionBase *Evolution)
{
	...
	ParticlesToDirty.Init(false, NumEffectiveParticles);
		...
		for (const FSharedSimulationSizeSpecificData& Data : Parameters.Shared.SizeSpecificData)
		{
			if (ensure(Data.CollisionShapesData.Num()))
			{
				if (Data.CollisionShapesData[0].CollisionParticleData.CollisionParticlesFraction >= ReportHighParticleFraction)
				{
					...
					UE_LOGF(LogChaos, Warning, "Collection with small particle fraction(%f):%ls", Data.CollisionShapesData[0].CollisionParticleData.CollisionParticlesFraction, *Parameters.Name);
				}
			}
		}
	}
	...
	for (int32 Index = 0; Index < NumEffectiveParticles; ++Index)
	{
		...
		if (P != nullptr)
		{
			...
			for (int32 ShapeIndex = 0; ShapeIndex < NumShapes; ++ShapeIndex)
			{
				...
				Shape->SetShapeFilterData(ShapeFilterData);
				Shape->SetFilterInstanceData(ShapeInstanceData);
				Shape->SetProxy(this);
...
void FGeometryCollectionPhysicsProxy::InitializeBodiesPT(Chaos::FPBDRigidsSolver* RigidsSolver, typename Chaos::FPBDRigidsSolver::FParticlesType& Particles)
{
	...
	if (Parameters.Simulating && RestCollection)
	{
		...
		if (!ensure(Implicits))
		{
			UE_LOGF(LogChaos, Error, "Geometry collection %ls - InitializeBodiesPT called with collision missing in the collection ", Owner ? *Owner->GetFullName(): TEXT("Unknown"));
		}
		...
		if (Parameters.EnableClustering)// && Parameters.CacheType != EGeometryCollectionCacheType::Play)
		{
			...
			for (const int32 TransformGroupIndex : RecursiveOrder)
			{
				...
				if (ParticleIndex != INDEX_NONE && SolverParticleHandles[ParticleIndex] == nullptr)
				{
					...
					{
						UE_LOGF(LogChaos, Warning, "Too many children (%d) in a single cluster:%ls", 
		...
		for (int32 ParticleIndex = 0; ParticleIndex < SolverParticleHandles.Num(); ++ParticleIndex)
		{
			if (Chaos::FPBDRigidParticleHandle* Handle = SolverParticleHandles[ParticleIndex])
			{
				...
				Handle->SetGyroscopicTorqueEnabled(Parameters.bGyroscopicTorqueEnabled);
			}
			...
			for (int32 ParticleIndex = 0; ParticleIndex < NumEffectiveParticles; ParticleIndex++)
			{
				...
				if (TransformIndex != Parameters.InitialRootIndex)
				{
					...
					if (ensure(Handle) && Handle->GetGeometry() == nullptr)
					{
						...
						SetImplicitToPTParticles(Handle, Parameters.Shared, Simplicial, Implicits? (*Implicits)[TransformIndex]: nullptr, ShapeFilterData, ShapeInstanceData, WorldTransform, CollisionParticlesPerObjectFraction);
	...
	const Chaos::FUniqueIdx* ExistingIndex)
{
	...
	if (ReportNoLevelsetCluster && Parent->GetGeometry())
	{
		...
		UE_LOGF(LogChaos, Warning, "Union object generated for cluster:%ls", *Parameters.Name);
	}
	...
	const EFieldObjectType ObjectType)
{
	...
	if ((ObjectType == EFieldObjectType::Field_Object_All) || (ObjectType == EFieldObjectType::Field_Object_Destruction) || (ObjectType == EFieldObjectType::Field_Object_Max))
	{
		...
		else if (FilterType == EFieldFilterType::Field_Filter_DynamicAndSleeping)
		{
			for (FClusterHandle* ClusterHandle : SolverParticleHandles)
			{
				if (ClusterHandle && (ClusterHandle->ObjectState() == Chaos::EObjectStateType::Dynamic || ClusterHandle->ObjectState() == Chaos::EObjectStateType::Sleeping))
				{
					Handles.Add(ClusterHandle);
				}
			}
		...
		RBDSolver->EnqueueCommandImmediate([this, Force, WorldLocation, RBDSolver]()
			{
				...
				if (ClosestHandle)
				{
					if (ClosestHandle->IsSleeping())
					{
						RBDSolver->GetEvolution()->WakeParticle(ClosestHandle);
					}
...
void FGeometryCollectionPhysicsProxy::SetNotifyTrailings_External(bool bNotify)
{
	ExecuteOnPhysicsThread(*this,
		[this, bNotify]()
		{
			Parameters.bGenerateTrailingData = bNotify;
		});
}
...
void FGeometryCollectionPhysicsProxy::SetNotifyGlobalTrailings_External(bool bNotify)
{
	ExecuteOnPhysicsThread(*this,
		[this, bNotify]()
		{
			Parameters.bGenerateGlobalTrailingData = bNotify;
			if (bNotify)
			{
				if (Chaos::FPBDRigidsSolver* Solver = GetSolver<Chaos::FPBDRigidsSolver>())
				{
...
int32 FGeometryCollectionPhysicsProxy::CalculateEffectiveParticles(const FGeometryDynamicCollection& DynamicCollection, int32 NumTransform, int32 InMaxSimulatedLevel, bool bEnableClustering, const UObject* Owner, TBitArray<>& EffectiveParticles)
{
	...
	for (int32 TransformIndex = 0; TransformIndex < NumTransform; ++TransformIndex)
	{
		...
		if (Level <= MaxSimulatedLevel || !bEnableClustering)
		{
			...
			else
			{
				...
				UE_LOGF(LogChaos, Verbose, "GeometryCollection Transform %d has no geometry", TransformIndex);
				NumMissingGeometry++;
			}
		}
	}
	...
	if (NumMissingGeometry > 0)
	{
		if (Owner != nullptr)
		{
			UE_LOGF(LogChaos, Verbose, "Geometry collection %ls tried to create %d particles with no geometry", *Owner->GetFullName(), NumMissingGeometry);
		}
		else
		{
			UE_LOGF(LogChaos, Verbose, "Geometry collection tried to create %d particles with no geometry", NumMissingGeometry);
		}
	}
...
void FGeometryCollectionPhysicsProxy::UpdateWorldTransformCaches_External(const FTransform& WorldTransform)
{
	if (WorldTransform.Equals(WorldTransform_External))
	{
		return;
	}

	bIsGameThreadWorldTransformDirty = true;
	PreviousWorldTransform_External = WorldTransform_External;
	WorldTransform_External = WorldTransform;

	// Mirror to the PT-side cache only; do not run the kinematic / cluster-union / rescale
	// side effects that SetWorldTransform_Internal performs. PrevWorldTransform stays under
	// end-of-tick snapshot ownership (see BufferPhysicsResults_Internal).
	ExecuteOnPhysicsThread(*this,
		[this, WorldTransform]()
		{
			Parameters.WorldTransform = WorldTransform;
		});
}
...
void FGeometryCollectionPhysicsProxy::SetWorldTransform_External(const FTransform& WorldTransform)
{
	...
	if (bHasTransformChanged)
	{
		...
		// update GT particle if needed - experimental
		if (bUpdateGTParticlesOnWorldTransformUpdate)
		{
			const TManagedArray<bool>* AnimationsActive = GameThreadCollection.GetAnimateTransformAttribute();

			const int32 NumParticles = GTParticles.Num();
			for (int32 ParticleIndex = 0; ParticleIndex < NumParticles; ParticleIndex++)
			{
				if (const TUniquePtr<FParticle>& GTParticlePtr = GTParticles[ParticleIndex])
				{
					const int32 TransformIndex = FromParticleToTransformIndex[ParticleIndex];
					...
					{
						const FTransform PreviousParticleTransform(GTParticlePtr->R(), GTParticlePtr->X());
						const FTransform RelativeParticleTransform = PreviousParticleTransform.GetRelativeTransform(PreviousWorldTransform_External);
						const FTransform NewParticleTransform = RelativeParticleTransform * WorldTransform_External;
						GTParticlePtr->SetX(NewParticleTransform.GetTranslation());
						GTParticlePtr->SetR(NewParticleTransform.GetRotation());
					}
				}
			}
		}
...
void FGeometryCollectionPhysicsProxy::SetLinearDamping_External(float InDamping)
{
	// update physics thread
	ExecuteOnPhysicsThread(*this, [this, InDamping]()
		{
			SetLinearDamping_Internal(InDamping);
		});
}

void FGeometryCollectionPhysicsProxy::SetLinearDamping_Internal(float InDamping)
{
	Parameters.LinearDamping = InDamping;
	...
	for (Chaos::FPBDRigidClusteredParticleHandle* Handle : SolverParticleHandles)
	{
		...
		{
			Handle->SetLinearEtherDrag(Parameters.LinearDamping);
			// make sure we update internal clusters parent
			if (Chaos::FPBDRigidClusteredParticleHandle* ParentHandle = Handle->Parent())
			{
				if (ParentHandle->InternalCluster() && ParentHandle->PhysicsProxy() == Handle->PhysicsProxy())
				{
					ParentHandle->SetLinearEtherDrag(Parameters.LinearDamping);
				}
			}
		}
	}
...
void FGeometryCollectionPhysicsProxy::SetLinearDampingPerTransform_External(const TArray<int32>& TransformIndices, float InDamping)
{
	// update physics thread
	ExecuteOnPhysicsThread(*this, [this, InDamping, TransformIndices]()
		{
			SetLinearDampingPerTransform_Internal(TransformIndices, InDamping);
		});
}

void FGeometryCollectionPhysicsProxy::SetLinearDampingPerTransform_Internal(const TArray<int32>& TransformIndices, float InDamping)
{
	for (const int32 TransformIndex: TransformIndices)
	{
		...
		{
			const int32 ParticleIndex = FromTransformToParticleIndex[TransformIndex];
			if (Chaos::FPBDRigidClusteredParticleHandle* Handle = SolverParticleHandles.IsValidIndex(ParticleIndex) ? SolverParticleHandles[ParticleIndex] : nullptr)
			{
				Handle->SetLinearEtherDrag(InDamping);
			}
		}
	}
}

void FGeometryCollectionPhysicsProxy::SetAngularDamping_External(float InDamping)
{
	// update physics thread
	ExecuteOnPhysicsThread(*this, [this, InDamping]()
		{
			SetAngularDamping_Internal(InDamping);
		});
}

void FGeometryCollectionPhysicsProxy::SetAngularDamping_Internal(float InDamping)
{
	Parameters.AngularDamping = InDamping;
	...
	for (Chaos::FPBDRigidClusteredParticleHandle* Handle : SolverParticleHandles)
	{
		if (Handle)
		{
			...
			if (Chaos::FPBDRigidClusteredParticleHandle* ParentHandle = Handle->Parent())
			{
				if (ParentHandle->InternalCluster() && ParentHandle->PhysicsProxy() == Handle->PhysicsProxy())
				{
					ParentHandle->SetAngularEtherDrag(Parameters.AngularDamping);
				}
			}
		}
	}
...
void FGeometryCollectionPhysicsProxy::SetAngularDampingPerTransform_External(const TArray<int32>& TransformIndices, float InDamping)
{
	// update physics thread
	ExecuteOnPhysicsThread(*this, [this, InDamping, TransformIndices]()
		{
			SetAngularDampingPerTransform_Internal(TransformIndices, InDamping);
		});
}

void FGeometryCollectionPhysicsProxy::SetAngularDampingPerTransform_Internal(const TArray<int32>& TransformIndices, float InDamping)
{
	for (const int32 TransformIndex : TransformIndices)
	{
		if (FromTransformToParticleIndex.IsValidIndex(TransformIndex))
		{
			...
			if (Chaos::FPBDRigidClusteredParticleHandle* Handle = SolverParticleHandles.IsValidIndex(ParticleIndex) ? SolverParticleHandles[ParticleIndex] : nullptr)
			{
				Handle->SetAngularEtherDrag(InDamping);
			}
		}
	}
}
...
void FGeometryCollectionPhysicsProxy::FlushDirtyParticles(Chaos::FPBDRigidsSolver& CurrentSolver)
{
	for (TConstSetBitIterator<> It(ParticlesToDirty); It; ++It)
	{
		if (FClusterHandle* Handle = SolverParticleHandles[It.GetIndex()])
		{
			CurrentSolver.GetEvolution()->DirtyParticle(*Handle);
		}
	}
	ParticlesToDirty.Init(false, ParticlesToDirty.Num());
}

static void UpdateParticleHandleTransform(Chaos::FPBDRigidsSolver& CurrentSolver, Chaos::FPBDRigidClusteredParticleHandle& Handle, const Chaos::FRigidTransform3& NewTransform, TBitArray<>& ParticlesToDirty, int32 ParticleIndex)
{
	...
	ParticlesToDirty[ParticleIndex] = true;
}

static void UpdateParticleHandleTransformIfNeeded(Chaos::FPBDRigidsSolver& CurrentSolver, Chaos::FPBDRigidClusteredParticleHandle& Handle, const Chaos::FRigidTransform3& NewTransform, TBitArray<>& ParticlesToDirty, int32 ParticleIndex)
{
	...
	{
		UpdateParticleHandleTransform(CurrentSolver, Handle, NewTransform, ParticlesToDirty, ParticleIndex);
	}
}
	...
	{ 
		...
		for (int32 ParticleIndex = 0; ParticleIndex < NumEffectiveParticles; ++ParticleIndex)
		{
			...
			if (ClusterParent && ClusterParent->InternalCluster())
			{
				...
				UpdateParticleHandleTransform(*CurrentSolver, *Handle, ParticleToWorld, ParticlesToDirty, ParticleIndex);
			}
			...
			if (bGeometryCollectionEnabledNestedChildTransformUpdates)
			{
				if (ClusterParent && !ClusterParent->Disabled())
				{
					...
					UpdateParticleHandleTransformIfNeeded(*CurrentSolver, *Handle, ChildToWorld, ParticlesToDirty, ParticleIndex);
...
// Update game thread particle V and W properties and retun true if they changed from previous values
static inline bool UpdateGTParticleVW(Chaos::FPBDRigidParticle& GTParticle, const Chaos::FVec3& NewV, const Chaos::FVec3& NewW)
{
	GC_PHYSICSPROXY_CHECK_FOR_NAN(NewX);

	const Chaos::FVec3 OldV = GTParticle.GetV();
	const bool bNeedUpdateV = (!NewV.Equals(OldV, GeometryCollectionLinearVelocityUpdateTolerance));
	if (bNeedUpdateV)
	{
		GTParticle.SetV(NewV, false);
	}

	const Chaos::FVec3 OldW = GTParticle.GetW();
	const bool bNeedUpdateW = (!NewW.Equals(OldW, GeometryCollectionAngularVelocityUpdateTolerance));
	if (bNeedUpdateW)
	{
		GTParticle.SetW(NewW, false);
	}
	return bNeedUpdateV || bNeedUpdateW;
}
	...
	for (int32 EntryIndex = 0; EntryIndex < CurrentResults.GetNumEntries(); EntryIndex++)
	{
		...
		if (bForcePullXRVW)
		{
			...
			if (bIsActive || bAnimatingWhileDisabled)
			{
				...
				const bool VWModified = UpdateGTParticleVW(GTParticle, VelocityData.ParticleV, VelocityData.ParticleW);
				bIsCollectionDirty |= VWModified;
...
bool FGeometryCollectionPhysicsProxy::PullFromPhysicsState(const Chaos::FDirtyGeometryCollectionData& PullData, const int32 SolverSyncTimestamp, const Chaos::FDirtyGeometryCollectionData* NextPullData, const Chaos::FRealSingle* Alpha, const Chaos::FDirtyRigidParticleReplicationErrorData* Error, const Chaos::FReal AsyncFixedTimeStep)
{
	...
	{
		// At this point the component may be gone and we have no way to get the name of the asset/component
		UE_LOGF(LogChaos, Error, 
			"Attempt to update physics state on thegamethread where the number of transform (%d) does not match anymore the rest collection (%d) - skipping physics update",
			NumTransforms, 
	...
	if (Error)
	{
		...
		// Accumulate error if within interpolation limits
		if (Error->ErrorX.SizeSquared() < (MaxErrorCorrection * MaxErrorCorrection))
		{
			const int32 RenderInterpErrorCorrectionDurationTicks = FMath::FloorToInt32(ErrorCorrectionDuration / AsyncFixedTimeStep); // Convert duration from seconds to simulation ticks
			InterpData->AccumulateErrorXR(Error->ErrorX, Error->ErrorR, SolverSyncTimestamp, RenderInterpErrorCorrectionDurationTicks);
		}
	}
	...
	if (NumTransforms > 0)
	{
		...
		{
			const Chaos::FRealSingle LerpAlpha = RenderInterpolationCVars::bSnapToLatest ? 1.0f : *Alpha;
			...
			for (int32 ParticleIndex = 0; ParticleIndex < NumEffectiveParticles; ++ParticleIndex)
			{
				...
				FParticle& GTParticle = *GTParticles[ParticleIndex];
				const FResultInterpolator ResultInterpolator(TransformGroupIndex, PrevResults, NextResults, GTParticle, LerpAlpha);
				...
				if (bActive || bAnimatingWhileDisabled)
				{
					...
					{
						NewX = NewX + InterpData->GetErrorX(LerpAlpha);
						NewR = NewR * InterpData->GetErrorR(LerpAlpha); // Add rotational error offset in local space
					}
					...
					if (RenderInterpolationCVars::bRenderInterpDebugDrawGC)
					{
						...
						if (EntryIndex != INDEX_NONE && NextEntryIndex != INDEX_NONE)
						{
							...
							if (bIsReplicationErrorSmoothing)
							{
								...
								Chaos::FDebugDrawQueue::GetInstance().DrawDebugDirectionalArrow(ZOffset + (NewX - InterpData->GetErrorX(LerpAlpha)), ZOffset + NewX, 1, FColor::Blue, false, 3.0f, 0, 0.5f);
							}
						}
					}
					...
					const bool VWModified = UpdateGTParticleVW(GTParticle, NewV, NewW);
					bIsCollectionDirty |= VWModified;
...
void FGeometryCollectionPhysicsProxy::SetFilterData_Internal(const Chaos::Filter::FCombinedShapeFilterData& NewShapeFilter)
{
	...
	Parameters.ShapeFilterData = NewShapeFilter.GetShapeFilterData();
	Parameters.ShapeInstanceData = NewShapeFilter.GetInstanceData();
		...
		for (const FParticleCollisionFilterData& Data : PerParticleData)
		{
			if (Data.bIsValid && Data.ParticleIndex != INDEX_NONE)
			{
				...
				if (ParticleIndex != INDEX_NONE)
				{
					...
					Interface.UpdateCombinedShapeFilterData(ParticleView, Chaos::Filter::FCombinedShapeFilterData(Data.ShapeFilterData, Data.FilterInstanceData));
	...
	const FSharedSimulationParameters& SharedParams)
{
	...
	if (!bUseRelativeSize)
	{
		UE_LOGF(LogChaos, Display, "Relative Size not found on Rest Collection. Using bounds volume for SizeSpecificData indexing instead.");
	}
	...
	{
		UE_LOGF(LogChaos, Warning, "Some geometry is too small to be simulated and has been skipped.");
	}
				...
				if (Chaos::BuildFieldSamplePoints(this, RigidSolver, FieldCommand, FieldData.ExecutionDatas, PrevResolutionType, PrevFilterType, PrevObjectType, PrevPositionType))
				{
							...
							SCOPE_CYCLE_COUNTER(STAT_ParamUpdateField_DynamicState);
							{
								...
								for (const FFieldContextIndex& Index : EvaluatedSamples)
								{
									...
									if (RigidHandle)
									{
										...
										const int32 TransformIndex = GetTransformGroupIndexFromHandle(RigidHandle);
					...
					else if (FieldCommand.RootNode->Type() == FFieldNodeBase::EFieldType::EField_FVector)
					{
						...
						if (FieldCommand.PhysicsType == EFieldPhysicsType::Field_InitialLinearVelocity)
						{
							if (Parameters.InitialVelocityType == EInitialVelocityTypeEnum::Chaos_Initial_Velocity_User_Defined)
							{
								SCOPE_CYCLE_COUNTER(STAT_ParamUpdateField_LinearVelocity);
								{
									...
									{
										UE_LOGF(LogChaos, Warning, "Geometry collection : trying to apply a InitialLinearVelocity field but InitialVelocityType is not set to None")
									}
								}
							}
							else
							{
								UE_LOGF(LogChaos, Error, "Field based evaluation of the simulations 'InitialLinearVelocity' requires the geometry collection be set to User Defined Initial Velocity");
							}
						}
						else if (FieldCommand.PhysicsType == EFieldPhysicsType::Field_InitialAngularVelocity)
						{
							...
							{
								UE_LOGF(LogChaos, Error, "Field based evaluation of the simulations 'InitialAngularVelocity' requires the geometry collection be set to User Defined Initial Velocity");
							}
						}
						else
						{
