...
void FChaosSolversModule::DestroySolver(Chaos::FPhysicsSolverBase* InSolver)
{
	...
	{
		UE_LOGF(LogChaosGeneral, Warning, "Passed valid solver state to DestroySolverState but it wasn't in the solver storage list! Make sure it was created using the Chaos module.");
	}
...
void FChaosSolversModule::DumpHierarchyStats(int32* OutOptMaxCellElements)
{
	...
	for(int32 SolverIndex = 0; SolverIndex < NumSolvers; ++SolverIndex)
	{
		...
		if(Boxes.Num() > 0)
		{
			...
			UE_LOGF(LogChaos, Warning, "%ls", *OutputString);
		}
#endif
...
{
	bPauseSolvers = true;
	UE_LOGF(LogChaosDebug, Verbose, "Pausing solvers.");
...
{
	bPauseSolvers = false;
	UE_LOGF(LogChaosDebug, Verbose, "Resuming solvers.");
}
...
void FChaosSolversModule::SingleStepSolvers()
{
	...
	UE_LOGF(LogChaosDebug, Verbose, "Single-stepping solvers.");
	// Sync physics to allow last minute updates
	if(IsPersistentTaskRunning())
	{
		SyncTask(true);
