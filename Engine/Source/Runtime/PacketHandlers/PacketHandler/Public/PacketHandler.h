...
namespace UE
{
	namespace Net
	{
		...
		// bShouldDiscard=true will prevent the log warning from firing when calling NET_LIGHTWEIGHT_TIME_GUARD_END
		// If called in a parallel thread, this will only apply to the current thread of execution
		PACKETHANDLER_API void SetDiscardTimeguardMeasurement(const bool bShouldDiscard);
		PACKETHANDLER_API bool GetDiscardTimeguardMeasurement();
#endif
	}
}
...
public:
	...
	*/
	...
	/**
	 * Whether or not all the HandlerComponents can support parallel Outgoing Connections
	 */
	PACKETHANDLER_API bool CanRunHandlerComponentsOnParallelOutgoingConnections();

	/**
	 * Output names of HandlerComponents to PacketHandlerLog
	 */
	PACKETHANDLER_API void LogHandlerComponentList();
...
public:
	...
	*/
	...
	/**
	 * Whether or not this HandlerComponent supports running its Outgoing logic on multiple separate connections simultaneously.
	 * Any accesses on a per-connection basis are safe, but writing to Global/NetDriver properties are not
	 *
	 * @return	Whether or not the above is supported
	 */
