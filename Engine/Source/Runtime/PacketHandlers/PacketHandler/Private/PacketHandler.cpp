...
static FTransactionallySafeRWLock PacketHandlerTimeguardLock;
static int32 GPacketHandlerTimeguardLimit = 20;
static float GPacketHandlerTimeguardThresholdMS = 0.0f;
static UE_AUTORTFM_DECLARE_THREAD_LOCAL_VAR(bool, GPacketHandlerDiscardTimeguardMeasurement);
...
namespace UE
{
	namespace Net
	{
		// GPacketHandlerDiscardTimeguardMeasurement is unique per-thread, but accessible from other
		// compilation units, so it's exposed here.
		void SetDiscardTimeguardMeasurement(const bool bShouldDiscard)
		{
			GPacketHandlerDiscardTimeguardMeasurement = bShouldDiscard;
		}

		bool GetDiscardTimeguardMeasurement()
		{
			return GPacketHandlerDiscardTimeguardMeasurement;
		}

		// GPacketHandlerTimeguardLimit can be accessed across threads to limit the total number of time guard warnings
		// In cases where multiple threads go above the threshold simultaneously, it's possible to go above GPacketHandlerTimeguardLimit
		// but after that frame no more warnings will be sent.
		int32 GetTimeguardLimit()
		{
			UE::TReadScopeLock Lock(PacketHandlerTimeguardLock);
			return GPacketHandlerTimeguardLimit;
		}

		void DecrementTimeguardLimit()
		{
			UE::TWriteScopeLock Lock(PacketHandlerTimeguardLock);
			GPacketHandlerTimeguardLimit--;
		}
	}
}
...
#define NET_LIGHTWEIGHT_TIME_GUARD_BEGIN( Name, ThresholdMS ) \
	double UE_JOIN(__TimeGuard_ThresholdMS_, Name) = ThresholdMS; \
	uint64 UE_JOIN(__TimeGuard_StartCycles_, Name) = ( ThresholdMS > 0.0f && UE::Net::GetTimeguardLimit() > 0 ) ? FPlatformTime::Cycles64() : 0; \
	UE::Net::SetDiscardTimeguardMeasurement(false);

#define NET_LIGHTWEIGHT_TIME_GUARD_END( Name, NameStringCode ) \
	if ( UE_JOIN(__TimeGuard_ThresholdMS_, Name) > 0.0f && UE::Net::GetTimeguardLimit() > 0 && !UE::Net::GetDiscardTimeguardMeasurement() ) \
	{\
		double UE_JOIN(__TimeGuard_MSElapsed_,Name) = FPlatformTime::ToMilliseconds64( FPlatformTime::Cycles64() - UE_JOIN(__TimeGuard_StartCycles_,Name) ); \
		if ( UE_JOIN(__TimeGuard_MSElapsed_,Name) > UE_JOIN(__TimeGuard_ThresholdMS_, Name) ) \
		{ \
			FString ReportName = NameStringCode; \
			UE_LOGF(PacketHandlerLog, Warning, "PacketHandler: %ls - %ls took %.2fms!", TEXT(#Name), *ReportName, UE_JOIN(__TimeGuard_MSElapsed_,Name)); \
			UE::Net::DecrementTimeguardLimit(); \
		} \
	}
...
OutType IntCastLog(InType In)
{
	...
	UE_CLOGF(!bFitsIn, PacketHandlerLog, Warning, "PacketHandler: Loss of data caused by truncated cast");
								...
								TSharedPtr<IAnalyticsProvider> InProvider/*=nullptr*/, FDDoSDetection* InDDoS/*=nullptr*/, FName InDriverProfile/*=NAME_None*/)
{
	...
	if (GConfig->GetString(TEXT("PacketHandlerComponents"), TEXT("EncryptionComponent"), EncryptionComponentName, GEngineIni) && !EncryptionComponentName.IsEmpty())
	{
		...
		{
			UE_LOGF(PacketHandlerLog, Warning, "PacketHandler encryption component is configured as %ls, but it won't be used because the cvar net.AllowEncryption is false.", *EncryptionComponentName);
		}
	}
	...
	{
		UE_LOGF(PacketHandlerLog, Warning, "bEnableReliability for PacketHandlerComponents is deprecated. For fully-reliable data, use reliable RPCs or a separate connection with a reliable protocol.");
...
void PacketHandler::AddHandler(TSharedPtr<HandlerComponent>& NewHandler, bool bDeferInitialize/*=false*/)
{
	...
	{
		UE_LOGF(PacketHandlerLog, Warning, "Packet handler already contains a component with name %ls.", *NewHandler->GetName().ToString());
		return;
	}
	...
	if (!ComponentStr.IsEmpty())
	{
		...
		if (ComponentName != TEXT("ReliabilityHandlerComponent"))
		{
			...
			if (FactoryComponentDelim != INDEX_NONE)
			{
				...
				{
					UE_LOGF(PacketHandlerLog, Warning, "Unable to load HandlerComponent factory: %ls", *ComponentName);
				}
			}
			...
			else
			{
				...
				{
					UE_LOGF(PacketHandlerLog, Warning, "Unable to Load Module: %ls", *ComponentName);
				}
			}
			...
			if (ReturnVal.IsValid())
			{
				UE_LOGF(PacketHandlerLog, Log, "Loaded PacketHandler component: %ls (%ls)", *ComponentName,
						*ComponentOptions);
		...
		{
			UE_LOGF(PacketHandlerLog, Warning, "PacketHandlerComponent 'ReliabilityHandlerComponent' is internal-only.");
		}
	}
...
EIncomingResult PacketHandler::Incoming_Internal(FReceivedPacketView& PacketView)
{
...
#if !UE_BUILD_SHIPPING
			UE_CLOGF((DDoS == nullptr || !DDoS->CheckLogRestrictions()), PacketHandlerLog, Error,
	...
	if (UNLIKELY(!!GPacketHandlerCRCDump))
	{
		...
		UE_LOGF(PacketHandlerLog, Log, "PacketHandler::Incoming: CRC Dump: NetConnection: %ls, Component: %ls, Socket: %08X",
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	if (UNLIKELY(!!GPacketHandlerCRCDump))
	{
		...
		UE_LOGF(PacketHandlerLog, Log, "PacketHandler::Outgoing: CRC Dump: NetConnection: %08X, Component: %ls, Socket: %ls",
...
bool PacketHandler::CanRunHandlerComponentsOnParallelOutgoingConnections()
{
	bool bCanRunInParallel = true;
	for (int32 i = 0; i < HandlerComponents.Num(); ++i)
	{
		if (HandlerComponents[i])
		{
			HandlerComponent& CurComponent = *HandlerComponents[i];
			bCanRunInParallel &= CurComponent.SupportsParallelConnectionOutgoing();
		}
	}

	return bCanRunInParallel;
}

void PacketHandler::LogHandlerComponentList()
{
	for (int32 i = 0; i < HandlerComponents.Num(); ++i)
	{
		if (HandlerComponents[i])
		{
			...
			UE_LOGF(PacketHandlerLog, Log, "Handler: %ls SupportsParallel: %ls", *CurComponent.GetName().ToString(), CurComponent.SupportsParallelConnectionOutgoing() ? TEXT("TRUE") : TEXT("FALSE"));
		}
	}
}
...
#if UE_BUILD_SHIPPING
				UE_LOGF(PacketHandlerLog, Fatal, "Some HandlerComponents require bEnableReliability!!!");
#else
				UE_LOGF(PacketHandlerLog, Warning, "Some HandlerComponents require bEnableReliability!!!");
#endif
