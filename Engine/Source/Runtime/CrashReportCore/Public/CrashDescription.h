...
public:
	...
	CRASHREPORTCORE_API const FString& AsString() const;

	/** Getter for bool. */
	CRASHREPORTCORE_API bool AsBool() const;

	/** Getter for int64. */
	CRASHREPORTCORE_API int64 AsInt64() const;
...
protected:
	/** Default constructor. */
	CRASHREPORTCORE_API FPrimaryCrashProperties();
...
public:
	...
	*/
	...
	const FString& GetXmlFilepath() { return XmlFilepath; }
...
protected:
	...
	/** Sets the TimeOfCrash and calls UpdateIDs if the XMLFile is valid */
	void UpdateDefaults();

	/** Sets the CrashGUID based on the report's path. */
	...
	CRASHREPORTCORE_API void MakeCrashEventAttributes(TArray<FAnalyticsEventAttribute>& OutCrashAttributes) const;
...
struct FCrashContext : public FPrimaryCrashProperties
{
	...
	explicit FCrashContext( const FString& CrashContextFilepath, bool bIsReadOnly = false )
		: FCrashContext(CrashContextFilepath, nullptr, bIsReadOnly)
	{
	}

	/** Initializes an instance based on a in memory buffer. */
	CRASHREPORTCORE_API FCrashContext(const FString& CrashContextFilepath, const TCHAR* Buffer, bool bIsReadOnly = false);

	FCrashContext(const FCrashContext&) = delete;
	FCrashContext& operator=(const FCrashContext&) = delete;
...
struct FCrashWERContext : public FPrimaryCrashProperties
{
	...
	CRASHREPORTCORE_API explicit FCrashWERContext( const FString& WERXMLFilepath, bool bIsReadOnly = false );

	FCrashWERContext(const FCrashWERContext&) = delete;
	FCrashWERContext& operator=(const FCrashWERContext&) = delete;
