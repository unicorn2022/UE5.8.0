...
void FCrashReportCoreConfig::ApplyProjectOverrides(const FString& ConfigFilePath)
{
	...
	{
		UE_LOGF(CrashReportCoreLog, Display, "Applying project settings from '%ls'.", *ConfigFilePath)
...
{
	UE_LOGF(CrashReportCoreLog, Log, "CrashReportClientVersion=%ls", *CrashReportClientVersion);
	if (CrashReportReceiverIP.IsEmpty())
	{
		UE_LOGF(CrashReportCoreLog, Log, "CrashReportReceiver disabled");
	}
	else
	{
		UE_LOGF(CrashReportCoreLog, Log, "CrashReportReceiverIP: %ls", *CrashReportReceiverIP);
	}
	if (DataRouterUrl.IsEmpty())
	{
		UE_LOGF(CrashReportCoreLog, Log, "DataRouter disabled");
	}
	else
	{
		UE_LOGF(CrashReportCoreLog, Log, "DataRouterUrl: %ls", *DataRouterUrl);
	}
	for (auto Location : FullCrashDumpConfigurations)
	{
		UE_LOGF( CrashReportCoreLog, Log, "FullCrashDump: %ls, NetworkLocation: %ls, bExactMatch:%i", *Location.BranchName, *Location.Location, Location.bExactMatch);
	}
}
