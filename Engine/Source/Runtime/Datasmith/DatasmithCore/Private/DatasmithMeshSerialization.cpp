...
bool CompressInline(TArray<uint8>& UncompressedData, ECompressionMethod Method)
{
	...
	UE_LOGF(LogDatasmith, Warning, "Compression failed");
	return false;
}
...
bool DecompressInline(TArray<uint8>& CompressedData)
{
	...
	UE_LOGF(LogDatasmith, Warning, "Decompression failed");
	return false;
}
...
FDatasmithPackedMeshes GetDatasmithMeshFromFile(const FString& MeshPath)
{
	...
	if ( !Archive.IsValid() )
	{
		UE_LOGF(LogDatasmith, Warning, "Cannot read file %ls", *MeshPath);
		return Result;
	}
	...
	*Archive << LeagacyNumMeshesCount;
	...
	else
	{
		...
		if (Archive->IsError())
		{
			...
			UE_LOGF(LogDatasmith, Warning, "Failed to read meshes from %ls", *MeshPath);
		}
	}
...
FDatasmithPackedCloths GetDatasmithClothFromFile(const FString& Path)  // UE_DEPRECATED(5.5, "The experimental Cloth importer is no longer supported.")
{
	...
	if ( !Archive.IsValid() )
	{
		UE_LOGF(LogDatasmith, Warning, "Cannot read file %ls", *Path);
		return {};
	}
	...
	if (Archive->IsError())
	{
		UE_LOGF(LogDatasmith, Warning, "Failed to read cloth from %ls", *Path);
		return {};
	}
