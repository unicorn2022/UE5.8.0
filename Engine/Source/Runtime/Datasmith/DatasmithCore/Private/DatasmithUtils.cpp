	...
	void CleanUpEnvironments( TSharedPtr<IDatasmithScene> Scene )
	{
		...
		for (int32 Index = Scene->GetActorsCount() - 1; Index >= 0; --Index)
		{
			if ( Scene->GetActor(Index)->IsA( EDatasmithElementType::EnvironmentLight ) )
			{
				...
				{
					UE_LOGF(LogDatasmith, Warning, "Environment %ls removed because it is not supported yet", EnvironmentElement->GetName());
		...
		for ( int32 Index = Scene->GetActorsCount() - 1; Index >= 0; --Index )
		{
			if ( Scene->GetActor(Index)->IsA( EDatasmithElementType::EnvironmentLight ) )
			{
				...
				if ( bIsADuplicate )
				{
					UE_LOGF(LogDatasmith, Warning, "Environment %ls removed because only one environment of its type is supported", EnvironmentElement->GetName());
					Scene->RemoveActor( EnvironmentElement, EDatasmithActorRemovalRule::RemoveChildren );
				}
			}
		}
	...
	struct FDatasmithSceneCleaner
	{
		...
		void Clean()
		{
			...
			for (int32 Index = Scene->GetMeshesCount() - 1; Index >= 0; --Index)
			{
				...
				{
					UE_LOGF(LogDatasmith, Warning, "Mesh element %ls removed because it is unused", MeshElement->GetName());
			...
			for (int32 Index = Scene->GetMaterialsCount() - 1; Index >= 0; --Index)
			{
				...
				{
					UE_LOGF(LogDatasmith, Warning, "Material element %ls removed because it is unused", MaterialElement->GetName());
			...
			for (int32 Index = Scene->GetTexturesCount() - 1; Index >= 0; --Index)
			{
				...
				{
					UE_LOGF(LogDatasmith, Warning, "Texture element %ls removed because it is unused", TextureElement->GetName());
			...
			for (int32 Index = Scene->GetLevelVariantSetsCount() - 1; Index >= 0; --Index)
			{
				...
				{
					UE_LOGF(LogDatasmith, Warning, "VariantSets element %ls removed because it references no actor part of the scene", VariantSetsElement->GetName());
		...
		void CleanUpLevelSequences()
		{
			...
			for (int32 Index = LevelSequencesLastIndex; Index >= 0; --Index)
			{
				...
				if (!bValidSequence)
				{
					UE_LOGF(LogDatasmith, Warning, "LevelSequence element %ls removed because it references no actor part of the scene", SequenceElement->GetName());
...
FString FDatasmithUniqueNameProviderBase::GenerateUniqueName(const FString& InBaseName, int32 CharBudget)
{
	...
	UE_LOGF(LogDatasmith, Warning, "Cannot generate a unique name from '%ls'.", *InBaseName);
	return {};
}
