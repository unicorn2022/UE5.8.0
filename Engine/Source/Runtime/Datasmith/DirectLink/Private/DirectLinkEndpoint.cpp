	...
	, Internal(*InternalPtr)
{
	...
	{
		UE_LOGF(LogDirectLinkNet, Error, "Endpoint '%ls': Unable to start endpoint: support for threads is required for DirectLink.", *SharedState.NiceName);
		return;
	}
	...
	{
		UE_LOGF(LogDirectLinkNet, Error, "Endpoint '%ls': Unable to start communication (error code:%d):", *SharedState.NiceName, int(ComStatus));
		return;
	}

	if (Internal.Init())
	{
		UE_LOGF(LogDirectLinkNet, Log, "Endpoint '%ls' Start internal thread", *SharedState.NiceName);
...
FEndpoint::~FEndpoint()
{
	UE_LOGF(LogDirectLinkNet, Log, "Endpoint '%ls' closing...", *SharedState.NiceName);
	...
	UE_LOGF(LogDirectLinkNet, Log, "Endpoint '%ls' closed", *SharedState.NiceName);
}
...
FSourceHandle FEndpoint::AddSource(const FString& Name, EVisibility Visibility)
{
	...
	UE_CLOGF(SharedState.bDebugLog, LogDirectLinkNet, Log, "Endpoint '%ls': Source added '%ls'", *SharedState.NiceName, *Name);
	return Id;
}
...
FEndpoint::EOpenStreamResult FEndpoint::OpenStream(const FSourceHandle& SourceId, const FDestinationHandle& DestinationId)
{
	...
	if (!bRequestFromSource && !bRequestFromDestination)
	{
		...
		UE_LOGF(LogDirectLinkNet, Log, "Endpoint '%ls': Cannot open stream: no source or destination point found.", *SharedState.NiceName);
		return EOpenStreamResult::SourceAndDestinationNotFound;
	}

	if (bRequestFromSource && bRequestFromDestination)
	{
		UE_LOGF(LogDirectLinkNet, Log, "Endpoint '%ls': Cannot open stream: have source and destination.", *SharedState.NiceName);
	...
	FMessageAddress RemoteAddress;
	{
		...
		if (FRawInfo::FDataPointInfo* DataPointInfo = SharedState.RawInfo.DataPointsInfo.Find(RemoteDataPointId))
		{
			...
			{
				UE_LOGF(LogDirectLinkNet, Warning, "Endpoint '%ls': Cannot open stream: Remote connection Point is private.", *SharedState.NiceName);
	...
	if (RemoteAddress.IsValid())
	{
		...
		UE_CLOGF(SharedState.bDebugLog, LogDirectLinkNet, Log, "Endpoint '%ls': Send FDirectLinkMsg_OpenStreamRequest", *SharedState.NiceName);
	...
	{
		UE_LOGF(LogDirectLinkNet, Error, "Connection Request failed: no recipent found");
...
{
	{
		...
		{
			UE_LOGF(LogDirectLinkNet, Warning, "Endpoint '%ls': Dropped delta message (no stream at port %d)", *SharedState.NiceName, Message.DestinationStreamPort);
			return;
		}
		...
		{
			UE_LOGF(LogDirectLinkNet, Warning, "Endpoint '%ls': Dropped delta message (inactive stream used on port %d)", *SharedState.NiceName, Message.DestinationStreamPort);
			return;
		}
...
{
	{
		...
		if (StreamPtr == nullptr)
		{
			UE_LOGF(LogDirectLinkNet, Warning, "Endpoint '%ls': Dropped havelist message (no stream at port %d)", *SharedState.NiceName, Message.SourceStreamPort);
			return;
		}
		...
		{
			UE_LOGF(LogDirectLinkNet, Warning, "Endpoint '%ls': Dropped havelist message (inactive stream used on port %d)", *SharedState.NiceName, Message.SourceStreamPort);
			return;
		}
...
void FInternalThreadState::Handle_EndpointLifecycle(const FDirectLinkMsg_EndpointLifecycle& Message, const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& Context)
{
	...
	UE_CLOGF(SharedState.bDebugLog, LogDirectLinkNet, Verbose, "Endpoint '%ls': Handle_EndpointLifecycle", *SharedState.NiceName);
	...
	switch (Message.LifecycleState)
	{
		...
		case FDirectLinkMsg_EndpointLifecycle::ELifecycle::Heartbeat:
		{
			...
			if (!bIsUpToDate)
			{
				UE_CLOGF(SharedState.bDebugLog, LogDirectLinkNet, Log, "Endpoint '%ls': Send FDirectLinkMsg_QueryEndpointState", *SharedState.NiceName);
...
void FInternalThreadState::Handle_EndpointState(const FDirectLinkMsg_EndpointState& Message, const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& Context)
{
		...
		|| GetCurrentProtocolVersion() < Message.MinProtocolVersion)
	{
		...
		UE_CLOGF(!bAlreadyIn, LogDirectLinkNet, Warning, "Endpoint '%ls': Remote Endpoint %ls ignored, incompatible protocol versions. Supported: [%d..%d], Remote [%d..%d]"
	...
	UE_CLOGF(SharedState.bDebugLog, LogDirectLinkNet, Log, "Endpoint '%ls' Handle_EndpointState", *SharedState.NiceName);
	UE_CLOGF(SharedState.bDebugLog, LogDirectLinkNet, Log, "%ls", *ToString_dbg());
}
...
void FInternalThreadState::Handle_OpenStreamRequest(const FDirectLinkMsg_OpenStreamRequest& Message, const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& Context)
{
	...
	auto DenyConnection = [&](const FString& Reason)
	{
		...
		UE_LOGF(LogDirectLinkNet, Log, "Endpoint '%ls': Refused connection: %ls", *SharedState.NiceName, *Reason);
	...
	else
	{
		...
		UE_LOGF(LogDirectLinkNet, Log, "Endpoint '%ls': Accepted connection", *SharedState.NiceName);
		MessageEndpoint->Send(Answer, RemoteEndpointAddress);
	}

	UE_CLOGF(SharedState.bDebugLog, LogDirectLinkNet, Verbose, "Endpoint '%ls': Handle_OpenStreamRequest", *SharedState.NiceName);
	UE_CLOGF(SharedState.bDebugLog, LogDirectLinkNet, Verbose, "%ls", *ToString_dbg());
}
...
{
	UE_CLOGF(SharedState.bDebugLog, LogDirectLinkNet, Verbose, "Endpoint '%ls': Handle_OpenStreamAnswer", *SharedState.NiceName);
	const FMessageAddress& RemoteEndpointAddress = Context->GetSender();

	{
		...
		if (FStreamDescription* StreamPtr = SharedState.GetStreamByLocalPort(Message.RecipientStreamPort, _))
		{
			...
			if (Stream.Status == EStreamConnectionState::RequestSent)
			{
				...
				{
					Stream.Status = EStreamConnectionState::Closed;
					UE_LOGF(LogDirectLinkNet, Warning, "stream connection refused. %ls", *Message.Error);
				}
			}
		}
		else
		{
			UE_LOGF(LogDirectLinkNet, Warning, "error: no such stream (%d)", Message.RecipientStreamPort);
		}
	}
...
void FInternalThreadState::Handle_CloseStreamRequest(const FDirectLinkMsg_CloseStreamRequest& Message, const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& Context)
{
	...
	UE_LOGF(LogDirectLinkNet, Verbose, "Endpoint '%ls': Handle_CloseStreamRequest", *SharedState.NiceName);
	UE_CLOGF(SharedState.bDebugLog, LogDirectLinkNet, Verbose, "%ls", *ToString_dbg());
}
...
void FInternalThreadState::ReplicateState(const FMessageAddress& RemoteEndpointAddress) const
{
	if (MessageEndpoint.IsValid())
	{
		...
		*EndpointStateMessage = ThisDescription;
		...
		{
			UE_LOGF(LogDirectLinkNet, Verbose, "Endpoint '%ls': Send FDirectLinkMsg_EndpointState", *SharedState.NiceName);
			MessageEndpoint->Send(EndpointStateMessage, RemoteEndpointAddress);
		}
		else
		{
			UE_LOGF(LogDirectLinkNet, Verbose, "Endpoint '%ls': Publish FDirectLinkMsg_EndpointState", *SharedState.NiceName);
...
{
	{
		...
		for (const TSharedPtr<FStreamDestination>& Dest : SharedState.Destinations)
		{
			if (Dest->GetId() == DestinationGuid)
			{
				...
				UE_LOGF(LogDirectLinkNet, Log, "Endpoint '%ls': Handle_OpenStreamRequest: new connection refused by provider", *SharedState.NiceName);
				break;
			}
		}
	}
...
void FInternalThreadState::RemoveEndpoint(const FMessageAddress& RemoteEndpointAddress)
{
	...
	{
		UE_CLOGF(SharedState.bDebugLog, LogDirectLinkNet, Display, "Endpoint '%ls' removes remote Endpoint '%ls'", *SharedState.NiceName, *RemoteState->NiceName);
	}
	...
	// close remaining associated streams
	{
		...
		for (auto& Stream : SharedState.Streams)
		{
			if (Stream.RemoteAddress == RemoteEndpointAddress
				&& Stream.Status != EStreamConnectionState::Closed)
			{
				UE_CLOGF(SharedState.bDebugLog, LogDirectLinkNet, Log, "Endpoint '%ls': Closed connection  (reason: remote endpoint removed)", *SharedState.NiceName);
	...
	for (const auto& KV : RemoteEndpointDescriptions)
	{
		if (double* LastSeen = RemoteLastSeenTime.Find(KV.Key))
		{
			if (Now_s - *LastSeen > gConfig.ThresholdEndpointCleanup_s)
			{
				...
				UE_LOGF(LogDirectLinkNet, Log, "Endpoint '%ls': Removed Endpoint %ls (timeout)", *SharedState.NiceName, *KV.Value.NiceName);
			}
		}
	}
...
void FInternalThreadState::Run()
{
	...
	if (gUdpMessagingInitializationTime > 0.)
	{
		...
		if (WaitTime > 0.)
		{
			UE_LOGF(LogDirectLinkNet, Verbose, "Endpoint '%ls': wait after UDP init. (In order to avoid that temporisation, Load 'UdpMessaging' module sooner in the game thread).", *SharedState.NiceName);
			FPlatformProcess::Sleep(WaitTime);
		}
	}

	UE_CLOGF(SharedState.bDebugLog, LogDirectLinkNet, Verbose, "Endpoint '%ls': Publishing FDirectLinkMsg_EndpointLifecycle Start", *SharedState.NiceName);
	...
	while (SharedState.bInnerThreadShouldRun)
	{
		...
		{
			UE_CLOGF(SharedState.bDebugLog, LogDirectLinkNet, Verbose, "Endpoint '%ls': Publishing FDirectLinkMsg_EndpointLifecycle Heartbeat %f", *SharedState.NiceName, Now_s);
	...
	UE_CLOGF(SharedState.bDebugLog, LogDirectLinkNet, Verbose, "Endpoint '%ls': Publishing FDirectLinkMsg_EndpointLifecycle Stop", *SharedState.NiceName);
...
void FSharedState::CloseStreamInternal(FStreamDescription& Stream, const FRWScopeLock& _, bool bNotifyRemote)
{
	...
	{
		UE_CLOGF(bDebugLog, LogDirectLinkNet, Log, "Endpoint '%ls': Stream removed", *NiceName);
		...
		UE_CLOGF(bDebugLog, LogDirectLinkNet, Log, "Endpoint '%ls': Send FDirectLinkMsg_CloseStreamRequest", *NiceName);
