...
protected:
	 ...
	 * Method to call to generate the curve sampling
	 */
	virtual void SamplingInitializing()
	{
...
public:
	...
	void CheckResult()
	{
		...
		UE_LOGF(LogCADKernelBase, Verbose, "Chord Desired Med Sd Min Max %f %f %f %f %f", MaxError, ChordMed, ChordStandartDeviation, MinError[0], MaxError[0]);
		UE_LOGF(LogCADKernelBase, Verbose, "Param Desired Med Sd Min Max %f %f %f %f %f", MaxError, ParamMed, ParamStandartDeviation, MinError[1], MaxError[1]);
	}
#endif
};
