...
int32 FShell::Orient()
{
	...
	TFunction<void()> FixOpenShellOrientation = [&]()
	{
		...
		{
			UE_LOGF(LogCADKernelBase, Verbose, "the open shell %ls (Id %d) is badly oriented. Its orientation is swapped", GetName(), GetId());
	...
	TFunction<void()> CheckAndFixSubshellMainOrientation = [&]()
	{
		...
		{
#ifdef CADERNEL_DEV
			UE_LOGF(LogCADKernelBase, Verbose, "The closed shell %ls (Id %d) is badly oriented. Its orientation is swapped", *GetName(), GetId());
#endif
			SwapSubshellOrientation();
			ShellSwappedFaceCount += (SubshellFaceCount - SubshellSwappedFaceCount);
		}
