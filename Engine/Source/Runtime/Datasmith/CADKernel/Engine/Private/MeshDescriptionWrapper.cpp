...
namespace UE::CADKernel::MeshUtilities
{
	...
	bool FMeshDescriptionWrapper::GetVertexInstances(const FArray3i& Vertices, FVertexInstanceID3& VertexInstances)
	{
		...
		{
			UE_LOGF(LogCADKernelEngine, Verbose, "FMeshDescriptionWrapper::GetVertexInstances: Detected a non-manifold edge");
	...
	bool FMeshDescriptionWrapper::ReserveNewTriangles(int32 TriangleCount)
	{
		...
		{
			UE_LOGF(LogCADKernelEngine, Warning, "The main UE5 rendering systems do not support more than %d materials per mesh. Only the first %d materials are kept. The others are replaced by the last one", NANITE_MAX_CLUSTER_MATERIALS, NANITE_MAX_CLUSTER_MATERIALS);
		}
	...
	bool FMeshDescriptionWrapper::AddTriangle(int32 GroupID, uint32 MaterialID, const FArray3i& VertexIndices, const TArrayView<FVector3f>& InNormals, const TArrayView<FVector2f>& InTexCoords)
	{
		...
		VertexInstanceUVs.Set(VertexInstanceIDs[0], 0/*UVChannel*/, FVector2f(InTexCoords[Orientation[0]] * static_cast<float>(ScaleUV)));
		VertexInstanceUVs.Set(VertexInstanceIDs[1], 0/*UVChannel*/, FVector2f(InTexCoords[Orientation[1]] * static_cast<float>(ScaleUV)));
		VertexInstanceUVs.Set(VertexInstanceIDs[2], 0/*UVChannel*/, FVector2f(InTexCoords[Orientation[2]] * static_cast<float>(ScaleUV)));
	...
	bool FMeshDescriptionWrapper::AddFaceTriangles(const TArray<FFaceTriangle>& FaceTriangles)
	{
		...
		for (const FFaceTriangle& FaceTriangle : FaceTriangles)
		{
			...
			{
				VertexInstanceUVs.Set(VertexInstanceIDs[0], 0/*UVChannel*/, FVector2f(TexCoords[FaceTriangle.TexCoords[Orientation[0]]] * static_cast<float>(ScaleUV)));
				VertexInstanceUVs.Set(VertexInstanceIDs[1], 0/*UVChannel*/, FVector2f(TexCoords[FaceTriangle.TexCoords[Orientation[1]]] * static_cast<float>(ScaleUV)));
				VertexInstanceUVs.Set(VertexInstanceIDs[2], 0/*UVChannel*/, FVector2f(TexCoords[FaceTriangle.TexCoords[Orientation[2]]] * static_cast<float>(ScaleUV)));
			}
