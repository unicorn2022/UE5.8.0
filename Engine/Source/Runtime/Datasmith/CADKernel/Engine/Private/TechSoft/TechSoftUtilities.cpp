...
#error Platform not supported
	...
	bool TechSoftUtilities::Tessellate(A3DRiRepresentationItem* Representation, const FTessellationContext& Context, MeshUtilities::FMeshWrapperAbstract& MeshWrapper, bool bEmptyMesh)
	{
		...
		static FCriticalSection TechSoftCriticalSection;

		// #cad_import: Track when TechSoft tessellation is thread safe
		FScopeLock Lock(&TechSoftCriticalSection);
	...
	FVector2d TechSoftUtilities::GetUVScale(const A3DTopoFace* TopoFace, double TextureUnit)
	{
		...
		constexpr int32 IsoCurveCount = 7;
		...
		if (DeltaU < UE_DOUBLE_SMALL_NUMBER || DeltaV < UE_DOUBLE_SMALL_NUMBER)
		{
			return FVector2d::UnitVector;
		}
		...
		// Texture unit is meter, Coord unit from TechSoft is mm, so TextureScale = 0.01 to convert cm into m
