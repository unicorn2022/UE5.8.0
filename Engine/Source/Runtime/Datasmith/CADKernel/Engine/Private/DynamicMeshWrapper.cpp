...
namespace UE::CADKernel::MeshUtilities
{
	...
	void FDynamicMeshWrapper::AddTriangles()
	{
		...
		for (int TriID : MeshOut.TriangleIndicesItr())
		{
			...
			{
				TriVertexIDs[0] = UVOverlay->AppendElement(FVector2f(TexCoords[TriangleDataIter->TexCoordIndices[0]] * static_cast<float>(ScaleUV)));
				TriVertexIDs[1] = UVOverlay->AppendElement(FVector2f(TexCoords[TriangleDataIter->TexCoordIndices[1]] * static_cast<float>(ScaleUV)));
				TriVertexIDs[2] = UVOverlay->AppendElement(FVector2f(TexCoords[TriangleDataIter->TexCoordIndices[2]] * static_cast<float>(ScaleUV)));
