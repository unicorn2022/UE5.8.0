...
UENUM(BlueprintType)
enum ECADKernelStitchingTechnique : uint8
{
	StitchingNone = 0,
	StitchingHeal,
	StitchingSew,
...
UENUM(BlueprintType)
enum ECADKernelTessellator : uint8
{
	CADKernel = 0,
	TechSoft,
};
...
struct CADKERNELENGINE_API FCADKernelTessellationSettings
{
	...
	FCADKernelTessellationSettings(
			...
			ECADKernelTessellator InTessellator = ECADKernelTessellator::CADKernel
		)
		...
		, Tessellator(InTessellator)
		, GeometricTolerance(InGeometricTolerance)
		, StitchingTolerance(InStichingTolerance)
	{
		Tessellator = bWithTechSoft ? InTessellator : ECADKernelTessellator::CADKernel;
	}
...
public:
	...
	TEnumAsByte<ECADKernelStitchingTechnique> StitchingTechnique;

	/**
	 * Indicates which tessellator to use: CADKernel or TechSoft. By default it is CADKernel.
	 */
	UPROPERTY(config, EditAnywhere, BlueprintReadWrite, AdvancedDisplay, Category = "Geometry & Tessellation Options", meta = (Tooltip = "Select tessellator to use. Default to CADKernel. Disabled if TechSoft is not available.", editCondition = "bWithTechSoft == true"))
	TEnumAsByte<ECADKernelTessellator> Tessellator = ECADKernelTessellator::CADKernel;
	
	/**
	 * T-Junctions are edge configurations where a border vertex lies on an opposing border edge.
	 * If true, the mesh is analyzed and all T-Junctions found on the mesh will be resolved.
	 * Only applicable with the TechSoft tessellator. Default is false
	 */
	UPROPERTY(config, EditAnywhere, BlueprintReadWrite, AdvancedDisplay, Category = "Geometry & Tessellation Options", meta = (EditCondition = "Tessellator == ECADKernelTessellator::TechSoft", EditConditionHides, Tooltip = "If true, enables the removal of T-Junctions from the resulting mesh. Default is false."))
...
public:
	...
	{
		uint32 Hash = HashCombine(GetTypeHash(StitchingTechnique), GetTypeHash(Tessellator));
	...
	bool UseCADKernel() const { return Tessellator == ECADKernelTessellator::CADKernel; }
};
