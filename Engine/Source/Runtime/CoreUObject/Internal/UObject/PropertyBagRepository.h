...
public:
	...
	enum class EPlaceholderObjectFeature : uint8
	{
		...
		/** Always replace missing type imports with a placeholder type on load, regardless of the underlying UClass derivative type */
		DoNotRestrictMissingTypeImports,
		/** Disable validation check that stops placeholders from being saved */
		AllowSavingPlaceholderExports,
	};
...
public:
	...
	/** Replace an existing Ido with a new one who's class is generated from the cached unknown property tree */
	UObject* ReinstanceWithCachedUnknownPropertyTree(UObject* Owner);
	...
	/**
	 * Return true if the object has an unknown property at the provided path
	 * @param Object - object to test
	 * @param Path - the specific property path to look up and check for an unknown property
	 */
	COREUOBJECT_API bool RequiresFixup(const UObject* Object, const FPropertyPathName& Path) const;
	...
	COREUOBJECT_API UObject* FindFirstInstanceThatNeedsFixup(const UClass* Class);
...
#else
	// Stub out accessors when the system is not available in cooked builds

	// query for whether or not the given struct/class is a placeholder type
	static inline bool IsPropertyBagPlaceholderType(const UStruct* Type) { return false; }
	// query for whether or not the given object was created as a placeholder type
	static inline bool IsPropertyBagPlaceholderObject(const UObject* Object) { return false; }
	// query for whether or not creating property bag placeholder objects should be allowed
	static inline bool IsPropertyBagPlaceholderObjectSupportEnabled() { return false; }
	// query for whether or not a specific property bag placeholder object feature is enabled
	static inline bool IsPropertyBagPlaceholderObjectFeatureEnabled(EPlaceholderObjectFeature Feature) { return false; }
