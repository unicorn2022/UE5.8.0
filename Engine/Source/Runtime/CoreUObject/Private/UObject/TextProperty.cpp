...
FTextProperty::FTextProperty(FFieldVariant InOwner, const FName& InName)
...
bool FTextProperty::Identical(TNotNull<const void*> A, const void* B, uint32 PortFlags) const
{
...
void FTextProperty::SerializeItem(FStructuredArchive::FSlot Slot, TNotNull<void*> Value, void const* Defaults) const
{
	TCppType* TextPtr = GetPropertyValuePtr(Value);
	Slot << *TextPtr;
}

void FTextProperty::ExportText_Internal(FString& ValueStr, TNotNull<const void*> PropertyValueOrContainer, EPropertyPointerType PropertyPointerType, const void* DefaultValue, UObject* Parent, int32 PortFlags, UObject* ExportRootScope) const
{
	// get current value:
	FText TextValue;
...
const TCHAR* FTextProperty::ImportText_Internal( const TCHAR* Buffer, TNotNull<void*> ContainerOrPropertyPtr, EPropertyPointerType PropertyPointerType, UObject* Parent, int32 PortFlags, FOutputDevice* ErrorText ) const
{
	FText ImportedText;
