...
{
	UE_LOGF(LogMetaData, Log, "METADATA %ls", *Package->GetName());
	...
	for (TMap< FSoftObjectPath, TMap<FName, FString> >::TIterator It(PackageMetaData.ObjectMetaDataMap); It; ++It)
	{
		...
		for (TMap<FName, FString>::TIterator MetaDataIt(MetaDataValues); MetaDataIt; ++MetaDataIt)
		{
			...
			if (Key != FName("ToolTip"))
			{
				UE_LOGF(LogMetaData, Log, "%ls: %ls=%ls", *It.Key().ToString(), *MetaDataIt.Key().ToString(), *MetaDataIt.Value());
			}
		}
	}
	...
	for (TMap<FName, FString>::TIterator MetaDataIt(PackageMetaData.RootMetaDataMap); MetaDataIt; ++MetaDataIt)
	{
		...
		if (Key != FName("ToolTip"))
		{
			UE_LOGF(LogMetaData, Log, "Root: %ls=%ls", *MetaDataIt.Key().ToString(), *MetaDataIt.Value());
		}
	}
}
	...
	if (GIsEditor)
	{
		...
		if (NewObjectPath != OldObjectPath)
		{
			...
			if (bShouldSearchChildren)
			{
				...
				GetObjectsWithOuter(OldObject, Children, EGetObjectsFlags::IncludeNestedObjects);
...
UE::FMutex UDEPRECATED_MetaData::KeyRedirectMapLock;
	...
	{
		{
			...
			{
				UE_LOGF(LogMetaData, Verbose, "Metadata was already loaded by linker. %ls", *GetFullName());
			}
			else
			{
				if (bLoadFromLinker && ObjectMetaDataMap.Num())
				{
					UE_LOGF(LogMetaData, Verbose, "Metadata: Some values, filled while serialization, may be lost. %ls", *GetFullName());
				}
		...
		if (Ar.CustomVer(FEditorObjectVersion::GUID) >= FEditorObjectVersion::RootMetaDataSupport)
		{
			...
			if (bLoadFromLinker && HasAnyFlags(RF_LoadCompleted))
			{
				UE_LOGF(LogMetaData, Verbose, "Root metadata was already loaded by linker. %ls", *GetFullName());
			}
			else
			{
				if (bLoadFromLinker && RootMetaDataMap.Num())
				{
					UE_LOGF(LogMetaData, Verbose, "Metadata: Some root values, filled while serialization, may be lost. %ls", *GetFullName());
				}
		...
		for (TMap< FWeakObjectPtr, TMap<FName, FString> >::TIterator ObjectIt(ObjectMetaDataMap); ObjectIt; ++ObjectIt)
		{
			...
			for (TMap<FName, FString>::TIterator PairIt(CurrentMap); PairIt; ++PairIt)
			{
				...
				if (NewKey != NAME_None)
				{
					...
					UE_LOGF(LogMetaData, Verbose, "Remapping old metadata key '%ls' to new key '%ls' on object '%ls'.", *OldKey.ToString(), *NewKey.ToString(), *ObjectIt.Key().Get()->GetPathName());
				}
			}
		...
		for (TMap<FName, FString>::TIterator PairIt(RootMetaDataMap); PairIt; ++PairIt)
		{
			...
			if (NewKey != NAME_None)
			{
				...
				UE_LOGF(LogMetaData, Verbose, "Remapping old metadata key '%ls' to new key '%ls' on root.", *OldKey.ToString(), *NewKey.ToString());
			}
		}
...
{
	static std::atomic<bool> bKeyRedirectMapInitialized = false;

	if (!bKeyRedirectMapInitialized.load(std::memory_order_acquire))
	{
		if (GConfig)
		{
			UE::TScopeLock _(KeyRedirectMapLock);
			if (bKeyRedirectMapInitialized.load(std::memory_order_relaxed))
			{
				return;
			}

			const FName MetadataRedirectsName("MetadataRedirects");
			...
			bKeyRedirectMapInitialized.store(true, std::memory_order_release);
		}
	}
}
...
UE::FMutex FMetaData::KeyRedirectMapLock;
...
void FMetaData::RemoveMetaDataOutsidePackage(UPackage* MetaDataPackage)
{
	...
	for(int32 i=0; i<ObjectsToRemove.Num(); i++)
	{
		...
		{
			UE_LOGF(LogMetaData, Log, "Removing '%ls' ref from Metadata '%ls'", *ObjectToRemove->GetPathName(), *MetaDataPackage->GetPathName());
		}
