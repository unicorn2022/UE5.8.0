...
static int32 GUnreachableObjectIndex = 0;
...
FGCCSyncObject::FGCCSyncObject()
{
	...
	UE::Private::SetIsGCLockedFunction([] { return FGCCSyncObject::Get().IsGCLocked(); });
}
FGCCSyncObject::~FGCCSyncObject()
{
...
FGCScopeGuard::FGCScopeGuard()
{
	...
	if (FPlatformProperties::RequiresCookedData() && ElapsedTime > 0.001)
	{
		...
		UE_LOGF(LogGarbage, Warning, "%f ms for acquiring ASYNC lock", ElapsedTime * 1000);
	}
#endif
}
...
namespace UE::GC
{
	...
	void FDetailedClassStats::LogClassCountInfo( const TCHAR* LogText, TMap<const FName, uint64>& ClassToCountMap, int32 NumItemsToLog, uint64 TotalCount )
	{
		...
		for (const TPair<FName, uint64>& Pair : ClassToCountMap)
		{
			...
			const FString PercentString = (TotalCount > 0) ? FString::Printf(TEXT("%6.2f%%"), Percent) : FString("  N/A  ");
	...
	void FDetailedClassStats::LogDetailedStatsSummary()
	{
		...
		UE_LOGF(LogGarbage, Log, "%d objects traversed", NumObjectsTraversed);
		UE_LOGF(LogGarbage, Log, "%d clusters traversed", NumClustersTraversed);
		UE_LOGF(LogGarbage, Log, "%d cluster to cluster refs followed", NumClusterToClusterRefs);
		UE_LOGF(LogGarbage, Log, "%d cluster to object refs followed", NumClusterToObjectRefs);
		UE_LOGF(LogGarbage, Log, "%d cluster to clustered mutable object refs followed", NumClusterToLateClusterRefs);
	...
	TSAN_ATOMIC(bool) GIsIncrementalReachabilityPending = false;
} // namespace UE::GC
...
namespace UE::GC::Private
{
	...
	// When using constinit UObjects with merged modules, it becomes tricky to unload Native objects.
	// We need to know during GC that objects are Native in order to not destroy them because they have a static lifetime.
	// Native is part of EInternalObjectFlags_GarbageCollectionKeepFlags so we need a way to let GC know these objects are no longer relevant while keeping the flag.
	void RemoveFromRootForUnload(TSet<const UObject*> ObjectsToRemove)
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(RemoveFromRootForUnload);

		GRootsMutex.Lock();

		for (const UObject* Object : ObjectsToRemove)
		{
			const int32 Index = GUObjectArray.ObjectToIndex(Object);
			if (GUObjectArray.IndexToObject(Index)->GetRefCount() > 0)
			{
				UE_LOGF(LogGarbage, Warning, "Cannot remove refcounted object %ls from roots!", *Object->GetName());
			}
			else
			{
				GRoots.Remove(Index);
...
AUTORTFM_DISABLE static bool GatherUnreachableObjects(UE::GC::EGatherOptions Options, double TimeLimit = 0.0);
...
AUTORTFM_DISABLE void OnDisregardForGCSetDisabled(int32 NumObjects)
{
	using namespace UE::GC;
	using namespace UE::GC::Private;
...
public:
	...
	FORCENOINLINE void VerifyAllObjectsDestroyed() const
	{
		...
		{
			UE_CLOGF(UnreachableObject.Object, LogGarbage, Fatal, "Object 0x%016llx has not been destroyed during purge", (int64)(PTRINT)UnreachableObject.Object);
		}
	}
} GUObjectPurge;
...
class AUTORTFM_DISABLE FContextPool
{
...
class AUTORTFM_DISABLE FContextPoolScope
{
...
AUTORTFM_DISABLE static void MarkReferencedClustersAsReachable(int32 ClusterIndex, ContainerType& ObjectsToSerialize);
...
struct AUTORTFM_DISABLE TAddAdapter
{
...
template<EGCOptions Options>
AUTORTFM_DISABLE FORCEINLINE static void MarkReferencedClustersAsReachableThunk(int32 ClusterIndex, FWorkBlockifier& ObjectsToSerialize)
{
...
template<EGCOptions Options, class ContainerType>
AUTORTFM_DISABLE static bool MarkClusterMutableObjectsAsReachable(FUObjectCluster& Cluster, ContainerType& ObjectsToSerialize)
{
	using namespace UE::GC::Private;
...
#if WITH_VERSE_VM || defined(__INTELLISENSE__)
AUTORTFM_DISABLE static void MarkClusterMutableCellsAsReachable(FUObjectCluster& Cluster)
{
...
#else
AUTORTFM_DISABLE FORCEINLINE static void MarkClusterMutableCellsAsReachable(FUObjectCluster& Cluster)
{
}
#endif
...
template<EGCOptions Options, class ContainerType>
AUTORTFM_DISABLE static FORCENOINLINE void MarkReferencedClustersAsReachable(int32 ClusterIndex, ContainerType& ObjectsToSerialize)
{
	using namespace UE::GC::Private;
	...
	for (int32& ReferncedClusterIndex : Cluster.ReferencedClusters)
	{
		if (ReferncedClusterIndex >= 0) // Garbag Elimination support
		{
			...
			if (!ReferencedClusterRootObjectItem->HasAnyFlags(EInternalObjectFlags::Garbage))
			{
				...
				{
					// Mark everything in the referenced cluster along with its mutable objects as reachable
...
struct FRemoteReference { UObject** Object; };
struct FResolvedRemoteReference : FImmutableReference
{
	UObject** Mutable;
};
...
FORCEINLINE_DEBUGGABLE static bool ValidateReference(UObject* Object, FPermanentObjectPoolExtents PermanentPool, FReferenceToken Referencer, FMemberId MemberId)
{
	...
	if (bOk)
	{
		...
		{
			UE_LOGF(LogGarbage, Fatal, "Invalid object in GC: 0x%016llx, Referencer: %ls, MemberId %ls (%d)",
				(int64)(PTRINT)Object,
...
struct AUTORTFM_DISABLE FReferenceMetadata
{
...
protected:
	...
	struct FValidatedBitmask
	{
		...
		FORCEINLINE_DEBUGGABLE uint32 CountBits()
		{
			...
			{
				Out += FPlatformMath::CountBits64(Word);
			}
			return Out;
		}
...
FORCEINLINE UObject*						GetObject(FRemoteReference In) { return *In.Object; }
...
FORCEINLINE void							PrefetchObjectPointer(FRemoteReference In) { FPlatformMisc::Prefetch(In.Object); }
FORCEINLINE FImmutableReference				ToImmutableReference(FMutableReference In) { return {*In.Object}; }
FORCEINLINE FImmutableReference				ToImmutableReference(FRemoteReference In) { return { *In.Object }; }
...
FORCEINLINE FResolvedRemoteReference		ToResolvedReference(FRemoteReference In) { return { ToImmutableReference(In), In.Object }; }
...
template<> FORCEINLINE FRemoteReference		MakeReference(UObject*& Object) { return { &Object }; }
...
private:
	...
	FORCEINLINE_DEBUGGABLE void DrainValidated(const uint32 Num)
	{
		...
		if (Num > PrefetchAhead)
		{
			FMemory::Memzero(Metadatas + Num, PrefetchAhead * sizeof(FReferenceMetadata));
...
///////////////////////// TDummyReferenceBatcher ///////////////////////

/**
 * Dummy reference batcher used to avoid allocating memory for batchers that will not be used in the current build config.
 * Note that the dummy batcher is only to satisfy the compiler and the template function definitions should not be needed because the linker should never require them.
 */
class FDummyReferenceBatcher
{
public:
	FDummyReferenceBatcher(FWorkerContext& InContext)
	{
	}
	template<class ArrayType>
	void PushArray(ArrayType& Array);
	template<class ViewType>
	void PushReferenceView(ViewType View);
	template<class UnvalidatedReferenceType>
...
FORCEINLINE_DEBUGGABLE void MakeRemoteHandleReference(UObject*& Object) 
{ 
#if UE_WITH_REMOTE_OBJECT_HANDLE
	Object = reinterpret_cast<UObject*>(FObjectHandle::ConvertToRemoteHandle(Object).Payload);
#endif
}
	...
	*/
	...
	static FORCEINLINE_DEBUGGABLE void ProcessReferenceDirectly(FWorkerContext& Context, FPermanentObjectPoolExtents PermanentPool, const UObject* ReferencingObject, UObject*& Object, FMemberId MemberId)
	{
		...
		if (ValidateReference(Object, PermanentPool, FReferenceToken(ReferencingObject), MemberId))
		{
			...
			if (FRemoteObjectId::RemoteObjectSupportCompiledIn && Metadata.Has(EInternalObjectFlags::Remote))
			{
				MakeRemoteHandleReference(Object);
				return;
			}
			else if (Metadata.Has(KillFlag) & bKillable)
			{
	...
	FORCEINLINE static void HandleBatchedReference(FWorkerContext& Context, FResolvedMutableReference Reference, FReferenceMetadata Metadata)
	{
		...
		if (FRemoteObjectId::RemoteObjectSupportCompiledIn && Metadata.Has(EInternalObjectFlags::Remote))
		{
			MakeRemoteHandleReference(*Reference.Mutable);
		}
		else if (Metadata.Has(KillFlag))
		{
			checkSlow(Metadata.ObjectItem->GetOwnerIndex() <= 0);
	...
	FORCEINLINE static void HandleBatchedReference(FWorkerContext& Context, FResolvedRemoteReference Reference, FReferenceMetadata Metadata)
	{
		UE::GC::GDetailedStats.IncreaseObjectRefStats(GetObject(Reference));		
		if (FRemoteObjectId::RemoteObjectSupportCompiledIn && Metadata.Has(EInternalObjectFlags::Remote))
		{
...
struct AUTORTFM_DISABLE TBatchDispatcher
{
...
#if UE_WITH_REMOTE_OBJECT_HANDLE
	TReferenceBatcher<FRemoteReference, FResolvedRemoteReference, ProcessorType> RemoteHandleBatcher;
#else
	FDummyReferenceBatcher RemoteHandleBatcher;
#endif
	...
	FORCENOINLINE void FlushQueuedReferences()
	{
		...
		RemoteHandleBatcher.FlushQueues();
	}
	...
	FORCEINLINE_DEBUGGABLE void QueueReference(const UObject* ReferencingObject,  UObject*& Object, FMemberId MemberId, EKillable Killable)
	{
		...
		else if constexpr (FRemoteObjectId::RemoteObjectSupportCompiledIn)
		{
			// Sanity check so that we never compile this path with the dummy batcher
			static_assert(!std::is_same_v<decltype(RemoteHandleBatcher), FDummyReferenceBatcher>);

			FPlatformMisc::Prefetch(&Object);
			RemoteHandleBatcher.PushReference(FRemoteReference{ &Object });
		}
		else
		{
			ImmutableBatcher.PushReference(FImmutableReference{Object});
	...
	FORCEINLINE_DEBUGGABLE void QueueReferences(const UObject* ReferencingObject, TArrayView<UObject*> References, FMemberId MemberId, EKillable Killable)
	{
		...
		{
			RemoteHandleBatcher.PushReferenceView(References);
		}
		else
		{
	...
	FORCEINLINE_DEBUGGABLE void QueueArray(const UObject* ReferencingObject, ArrayType&& Array, FMemberId MemberId, EKillable Killable)
	{
		...
		{
			RemoteHandleBatcher.PushArray((ArrayType&&)Array);
		}
		else
		{
	...
	{
		if (FRemoteObjectId::RemoteObjectSupportCompiledIn || Killable == EKillable::Yes)
		{
			for (UObject*& Object : Objects)
			{
				QueueReference(ReferencingObject, Object, MemberId, Killable);
			}
		}
		else
		{
...
FORCEINLINE static FName GetReferencerName(const UObject* Referencer, FMemberId MemberId)
{
	...
	else if (FGCObject::GGCObjectReferencer == Referencer)
	{
		...
		{
			static const FName NAME_Unknown = FName("Unknown");
			return NAME_Unknown;
		}
	}
	return NAME_None;
...
template <EGCOptions Options>
struct AUTORTFM_DISABLE TVerseDebugReachabilityVisitor : public Verse::FAbstractVisitor
{
...
template <EGCOptions Options>
class AUTORTFM_DISABLE TReachabilityCollectorBase : public FReferenceCollector
{
protected:
...
template <EGCOptions Options>
class AUTORTFM_DISABLE TReachabilityCollector final : public TReachabilityCollectorBase<Options>
{
...
template <EGCOptions InOptions>
class AUTORTFM_DISABLE TDebugReachabilityProcessor
{
public:
	FORCEINLINE void BeginTimingObject(UObject* CurrentObject)
	{
	...
	FORCENOINLINE void HandleTokenStreamObjectReference(FWorkerContext& Context, const UObject* ReferencingObject, UObject*& Object, FMemberId MemberId, EOrigin Origin, bool bAllowReferenceElimination)
	{
		...
		if (ValidateReference(Object, PermanentPool, FReferenceToken(ReferencingObject), MemberId))
		{
			...
			else if (bAllowReferenceElimination & Metadata.Has(TReachabilityProcessor<Options>::KillFlag)) //-V792
			{
...
template <EGCOptions Options>
class AUTORTFM_DISABLE TDebugReachabilityCollector final : public TReachabilityCollectorBase<Options>
{
	using Super = TReachabilityCollectorBase<Options>;
...
public:
	...
	virtual void HandleObjectReference(UObject*& Object, const UObject* ReferencingObject, const FProperty* ReferencingProperty) override
	{
		...
		{
			UE_LOGF(LogGarbage, Fatal, "Invalid object in GC: 0x%016llx, ReferencingObject: %ls, ReferencingProperty: %ls", 
				(int64)(PTRINT)Object, 
	...
	if constexpr (DispatcherType::bBatching)
	{
		...
		do
		{
			...
			do
			{
				...
				do
				{
					Private::VisitMembers(Dispatcher, AoS.Schema, StructIt, 0); 
					StructIt += AoS.Stride;
				}
...
AUTORTFM_DISABLE static FORCEINLINE Verse::FIOContext RelinquishVerseHeapAccess(Verse::FRunningContext RunningContext)
{
...
AUTORTFM_DISABLE static FORCEINLINE Verse::FRunningContext AcquireVerseHeapAccess(Verse::FIOContext Context)
{
	Context.SetIsInManuallyEmptyStack(false);
	return Context.AcquireAccessForManualStackScanning();
}

AUTORTFM_DISABLE static bool UpdateFrankenGCMode()
{
...
bool IsFrankenGCEnabled()
{
	return GEnableFrankenGC;
}
...
AUTORTFM_DISABLE static FORCEINLINE void StartVerseGC()
{
...
AUTORTFM_DISABLE static FORCEINLINE void StopVerseGC()
{
...
class AUTORTFM_DISABLE FRealtimeGC : public FGarbageCollectionTracer
{
...

		{
			...
			ParallelFor(TEXT("GC.MarkRootObjectsAsReachable"), MarkRootsState.NumWorkerThreads(), 1, [&ThreadIterators, &RootsArray](int32 ThreadIndex)
			{
				...
				while (ThreadState.Index <= ThreadState.LastIndex)
				{
					...
					checkCode(if (RootItem->HasAllFlags(EInternalObjectFlags::Garbage | EInternalObjectFlags::RootSet)) { UE_LOGF(LogGarbage, Fatal, "Object %ls is part of root set though has been marked as Garbage!", *Object->GetFullName()); });
#endif
...
private:
	...
	void ConditionallyAddBarrierReferencesToHistory(FWorkerContext& Context)
	{
		...
		{
			static const FName NAME_Barrier = FName("Barrier");
	...
	void PerformReachabilityAnalysisPass(const EGCOptions Options)
	{
		...
		if (!Private::GReachableObjects.IsEmpty())
		{
			...
			UE_LOGF(LogGarbage, Verbose, "Adding %d object(s) marker by GC barrier to the list of objects to process", InitialObjects.Num());
		...
		{
			// Add VNativeStructs marked by the Verse GC or the GC barrier
...
void AcquireGCLock()
{
	...
	if (FPlatformProperties::RequiresCookedData() && ElapsedTime > 0.001)
	{
		UE_LOGF(LogGarbage, Warning, "%f ms for acquiring GC lock", ElapsedTime * 1000);
	}
}
...
void IncrementalPurgeGarbage(bool bUseTimeLimit, double TimeLimit)
{
	...
	if (AutoRTFM::IsTransactional())
	{
		...
		UE_LOGF(LogGarbage, Log, "IncrementalPurgeGarbage: skipping garbage collection because an AutoRTFM transaction is active.");
		return;
	}
...
#else
// ASAN builds can be extremely slow on some platforms. During world trimming, GC may wait too long while the render thread 
// is still flushing commands, which can trigger a timeout crash even though it’s a false positive.
static bool GEnableTimeoutOnPendingDestroyedObjectGC = false;
#endif
#if UE_BUILD_SHIPPING
...
bool IncrementalDestroyGarbage(bool bUseTimeLimit, double TimeLimit)
{
	...
	{
		check(GUnreachableObjectIndex >= GUnreachableObjects.Num());
...

		{
			...
			while (GObjCurrentPurgeObjectIndex < GUnreachableObjects.Num())
			{
				...
				if (ObjectItem->IsUnreachable())
				{
					...
					checkf( Object->HasAnyFlags( RF_BeginDestroyed ) && !Object->HasAnyFlags( RF_FinishDestroyed ), TEXT("Invalid UObject destroy flags: 0x%08X%hs%hs"),
						Object->GetFlags(),
						Object->HasAnyFlags( RF_BeginDestroyed ) ? "" : ", RF_BeginDestroyed is not set",
						Object->HasAnyFlags( RF_FinishDestroyed ) ? ", RF_FinishDestroyed is already set" : "");
		...
		if (GObjCurrentPurgeObjectIndex >= GUnreachableObjects.Num())
		{
			...
			while( GGCObjectsPendingDestructionCount > 0 )
			{
				...
				{
					UE_LOGF(LogGarbage, Warning, "Not destroying garbage because GAllowSkipDestroyGarbage==1 && GSkipDestroyGarbage==1 (outer loop)");
					break;
				}
				...
				while (CurPendingObjIndex < GGCObjectsPendingDestructionCount)
				{
					...
					{
						UE_LOGF(LogGarbage, Warning, "Not destroying garbage because GAllowSkipDestroyGarbage==1 && GSkipDestroyGarbage==1 (per obj loop)");
						break;
					}
				...
				else if( GGCObjectsPendingDestructionCount > 0 )
				{
					if (FPlatformProperties::RequiresCookedData())
					{
							...
							((FPlatformTime::Seconds() - LastTimeoutWarningTime) > MaxTimeForFinishDestroy))
						{
							...
							{
								UE_LOGF(LogGarbage, Warning, "Spent more than %.2fs on routing FinishDestroy to objects (objects in queue: %d)", MaxTimeForFinishDestroy, GGCObjectsPendingDestructionCount);
								...
								for (int32 ObjectIndex = 0; ObjectIndex < GGCObjectsPendingDestructionCount; ++ObjectIndex)
								{
									...
									if (!bReady)
									{
										...
										UE_LOGF(LogGarbage, Warning, "  [%d]: %ls", ObjectIndex, *GetFullNameSafe(Obj));
										++NonReadyObjectIndex;
									}
								}
								...
								if (LastObjectNotReadyForFinishDestroy != nullptr)
								{
									...
									UE_LOGF(LogGarbage, Warning, "Spent too much time waiting for FinishDestroy for %d object(s) (last object: %ls), check log for details",
										GGCObjectsPendingDestructionCount,
									...
									if (LastObjectNotReadyForFinishDestroy)
									{
										if (GAllowSkipDestroyGarbage && GSkipDestroyGarbage.load(std::memory_order_relaxed))
										{
											UE_LOGF(LogGarbage, Warning, "Not crashing because GAllowSkipDestroyGarbage==1 and GSkipDestroyGarbage==1");
											break;
										}
							...
							{
								UE_LOGF(LogGarbage, Warning, "Spent more than %.2fs on routing FinishDestroy to objects (objects in queue: %d) - Skipping FATAL - GC Time out is disabled", MaxTimeForFinishDestroy, GGCObjectsPendingDestructionCount);
			...
			if( GGCObjectsPendingDestructionCount == 0 )
			{
				if (bFinishDestroyTimeExtended)
				{
					...
					UE_LOGF(LogGarbage, Warning, "%ls", *Msg );
	...
	{
		UE_LOGF(LogGarbage, Verbose, "%.3f ms for incrementally purging unreachable objects (FinishDestroyed: %d, Destroyed: %d / %d)",
			ElapsedTime * 1000,
			GObjCurrentPurgeObjectIndex,
	...
	{		
		...
		UE_LOGF(LogGarbage, Log, "GC purged %i objects (%i -> %i) in %.3fms (%d iteration(s))", PurgedObjectCountSinceLastMarkPhase,
...
AUTORTFM_DISABLE static void DissolveUnreachableClusters(UE::GC::EGatherOptions Options)
{
	...
	for (int32 ClusterIndex : ClustersToDestroy)
	{
		...
		UE_LOGF(LogGarbage, Log, "Destroying cluster (%d) %ls", ClusterRootItem->GetClusterIndex(), *static_cast<UObject*>(ClusterRootItem->Object)->GetFullName());
#endif
	...
	UE_LOGF(LogGarbage, Log, "%f ms for Dissolve Unreachable Clusters (%d/%d clusters dissolved containing %d cluster objects)",
...
bool GatherUnreachableObjects(UE::GC::EGatherOptions Options, double TimeLimit /*= 0.0*/)
{
	...
	UE_LOGF(LogGarbage, Log, "%f ms for Gather Unreachable Objects (%d objects collected / %d scanned with %d thread(s))",
	...
	if (bCompleted)
	{
		...
		{			
			TArrayView<UE::GC::FUnreachableObject> UnreachableObjectsView(GUnreachableObjects);
			...
			const TArrayView<FUObjectItem*>& UnreachableObjectItemsView = *reinterpret_cast<TArrayView<FUObjectItem*>*>(&UnreachableObjectsView);
			NotifyUnreachableObjects(UnreachableObjectItemsView);

#if UE_WITH_REMOTE_OBJECT_HANDLE
			if (!GExitPurge)
			{
				UE::RemoteObject::Transfer::Private::StoreUnreachableRemoteObjectsToDatabase(UnreachableObjectItemsView);
			}
...
class AUTORTFM_DISABLE FWeakReferenceEliminator final : public TReachabilityCollectorBase<EGCOptions::None>
{
...
{	
	...
	if (GGarbageReferenceTrackingEnabled == 1 || GGarbageReferenceTrackingEnabled == 3)
	{
		...
		for (const TUniquePtr<FWorkerContext>& Context : Contexts)
		{
			for (FGarbageReferenceInfo& GarbageReference : Context->GarbageReferences)
			{
				...
				UE_LOGF(LogGarbage, Warning, "Reachable garbage object: %ls", *GarbageReference.GarbageObject->GetFullName());
				UE_LOGF(LogGarbage, Warning, "Referenced by:            %ls", *GarbageReference.GetReferencingObjectInfo());
				UE_LOGF(LogGarbage, Warning, "");
				NumGarbageReferences++;
			}
			Context->GarbageReferences.Reset();
		}
		
		UE_CLOGF(GGarbageReferenceTrackingEnabled == 3 && NumGarbageReferences != 0, LogGarbage, Fatal, "Reported %d garbage references", NumGarbageReferences);
		UE_LOGF(LogGarbage, Log, "Reported %d garbage references", NumGarbageReferences);
	}
	...
	else if (GGarbageReferenceTrackingEnabled == 2)
	{
		...
		UE_LOGF(LogGarbage, Log, "Reported %d/%d garbage references", ReportedGarbageReferences, TotalGarbageReferences);
	}
#endif // !UE_BUILD_SHIPPING
}
...
AUTORTFM_DISABLE FORCENOINLINE void CollectGarbageImpl(EObjectFlags KeepFlags);
...
AUTORTFM_DISABLE void PreCollectGarbageImpl(EObjectFlags KeepFlags)
{
	...
	if (!GIsIncrementalReachabilityPending)
	{
		...
		{
			UE_LOGF(LogGarbage, Log, "CollectGarbageInternal() is flushing async loading");
			FlushAsyncLoading();
		...
		if (GFlushStreamingOnGC && IsAsyncLoading())
		{
			UE_LOGF(LogGarbage, Log, "CollectGarbageInternal() is flushing async loading because PreGarbageCollect triggered new loads");
			FlushAsyncLoading();
			GGCStats.bFlushedAsyncLoading = true;
		}
...

	{
		...
		UE_LOGF(LogGarbage, Log, "%ls%lsCollecting garbage%ls",
		...
		if (!GIsIncrementalReachabilityPending)
		{
			...
			if (GShouldVerifyGCAssumptions || (bPerformFullPurge && GShouldVerifyGCAssumptionsOnFullPurge) || bShouldRandomlyVerifyGCAssumptions)
			{
				...
				UE_LOGF(LogGarbage, Log, "%.2f ms for Verify GC Assumptions", GGCStats.VerifyTime * 1000);
			}
#endif
		}
	}
...
template<bool bPerformFullPurge>
AUTORTFM_DISABLE void PostCollectGarbageImpl(EObjectFlags KeepFlags)
{
	...
	if (!GIsIncrementalReachabilityPending)
	{
		...
		if (GUObjectClusters.ClustersNeedDissolving())
		{
			...
			UE_LOGF(LogGarbage, Log, "%f ms for dissolving GC clusters", (FPlatformTime::Seconds() - StartTime) * 1000);
		}
...

	{
		...
		if (UE_LOG_ACTIVE(LogGarbage, Log))
		{
			if (GIsIncrementalReachabilityPending)
			{
				...
				UE_LOGF(LogGarbage, Log, "GC Reachability iteration time: %.2f ms (%.2f ms on reference traversal, latency: %.3f ms)", ElapsedTime * 1000, ReferenceProcessingElapsedTime * 1000, SuspendLatency * 1000.0);
			}
			else
			{
				...
				UE_LOGF(LogGarbage, Log, "GC Reachability Analysis total time: %.2f ms (%.2f ms on reference traversal)", IncrementalMarkPhaseTotalTimeMs, ReferenceProcessingTotalTimeMs);
				FString ExtraDetail;
#if WITH_VERSE_VM
				ExtraDetail = FString::Printf(TEXT("and %d Verse cells"), Stats.NumVerseCells);
#endif
				UE_LOGF(LogGarbage, Log, "%.2f ms for %lsGC - %d refs/ms while processing %d references from %d objects %ls with %d clusters",
...
void PerformIncrementalReachabilityAnalysis(double TimeLimit)
{
	if (AutoRTFM::IsTransactional())
	{
		// The garbage collector is not usable from within a transaction.
		UE_LOGF(LogGarbage, Log, "PerformIncrementalReachabilityAnalysis: skipping garbage collection because an AutoRTFM transaction is active.");
		return;
	}
...
{
	return GUnreachableObjectIndex < GUnreachableObjects.Num() || UE::GC::Private::GGatherUnreachableObjectsState.IsPending();
}
...
bool UnhashUnreachableObjects(bool bUseTimeLimit, double TimeLimit)
{
	...
	{
		// Memory cannot be freed within a transaction as this would prevent us from rolling back to the initial state.
		UE_LOGF(LogGarbage, Log, "UnhashUnreachableObjects: skipping garbage collection because an AutoRTFM transaction is active.");
	...
	if (!bUseTimeLimit)
	{
		UE_LOGF(LogGarbage, Log, "%f ms for %lsunhashing unreachable objects (%d objects unhashed)",
			ElapsedTime * 1000,
	...
	else if (!bTimeLimitReached)
	{
		...
		UE_LOGF(LogGarbage, Log, "Finished unhashing unreachable objects (%d objects unhashed).", GUnreachableObjects.Num());
	}
	else if (bFirstIteration)
	{
		// When doing incremental unhashing log only the first and last iteration (this was the first one)
		UE_LOGF(LogGarbage, Log, "Starting unhashing unreachable objects (%d objects to unhash).", GUnreachableObjects.Num());
	}
...
bool TryCollectGarbage(EObjectFlags KeepFlags, bool bPerformFullPurge)
{
	...
	{
		// During initial load classes may not yet have their GC token streams assembled
		UE_LOGF(LogGarbage, Log, "Skipping TryCollectGarbage() call during initial load. It's not safe.");
		return false;
	}
...
void UObject::CallAddReferencedObjects(FReferenceCollector& Collector)
{
	...
	if (!bCalledSuperARO)
	{
		UE_LOGF(LogGarbage, Warning, "Class %ls or a super class did not call Super::AddReferencedObjects", *GetClass()->GetName());
	}
#else
	GetClass()->CallAddReferencedObjects(this, Collector);
#endif
...
bool FStructProperty::ContainsObjectReference(TArray<const FStructProperty*>& EncounteredStructProps, EPropertyObjectReferenceType InReferenceType /*= EPropertyObjectReferenceType::Strong*/) const
{
	...
	if (!Struct)
	{
		UE_LOGF(LogGarbage, Warning, "Broken FStructProperty does not have a UStruct: %ls", *GetFullName() );
		return false;
	}
...
void FArrayProperty::EmitReferenceInfo(UE::GC::FSchemaBuilder& Schema, int32 BaseOffset, TArray<const FStructProperty*>& EncounteredStructProps, UE::GC::FPropertyStack& DebugPath)
{
	...
	if (Inner->ContainsObjectReference(EncounteredStructProps, EPropertyObjectReferenceType::Strong))
	{
		...
		else
		{
			...
			// If inner property wrote nothing to the inner schema, the outer schema will assert. Assert here while we have more info.
			checkf(InnerSchema.NumMembers() > 0, TEXT("%s: Inner GC schema for property %s has no members after returning true from ContainsObjectReference."), 
				*DebugPath.GetPropertyPath(), *Inner->GetName());
		}
...
void UClass::AssembleReferenceTokenStream(bool bForce)
{
	...
	UE_CLOGF(!IsInGameThread() && !IsGarbageCollectionLocked(), LogGarbage, Fatal, "AssembleReferenceTokenStream for %ls called on a non-game thread while GC is not locked.", *GetFullName());
...
AUTORTFM_OPEN static void DumpMemoryStats(FOutputDevice& OutputDevice)
{
	FContextPoolScope Pool;
...
AUTORTFM_DISABLE static TArrayView<FWorkerContext*> InitializeAsyncProcessingContexts(FWorkerContext& InContext)
{
...
AUTORTFM_DISABLE void ReleaseAsyncProcessingContexts(FWorkerContext& InContext, TArrayView<FWorkerContext*> Contexts)
{
	FContextPoolScope ContextPool;
