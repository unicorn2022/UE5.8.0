...
FName FLinkerTables::GetExportClassName(int32 ExportIndex) const
{
...
{
	return FString("UnexpectedCallToBaseClassGetDebugName");
}
...
static void LogGetPackageLinkerError(FUObjectSerializeContext* LoadContext, const FPackagePath& PackagePath, const FText& InErrorMessage, UObject* InOuter, uint32 LoadFlags)
{
	...
	FString LoadingFile = !PackagePath.IsEmpty() ? PackagePath.GetDebugName() : FString("NULL");
	...
	if( GIsEditor && !IsRunningCommandlet() )
	{
		...
		if (LoadFlags & (LOAD_NoWarn | LOAD_Quiet))
		{
			...
			UE_LOGF(LogLinker, Log, "%ls", *FullErrorMessage.ToString());
			CLEAR_WARN_COLOR();
		}
		else
		{
			SET_WARN_COLOR(COLOR_RED);
			UE_LOGF(LogLinker, Warning, "%ls", *FullErrorMessage.ToString());
			CLEAR_WARN_COLOR();
...
static FPackagePath GetPackagePath(UPackage* InOuter, const TCHAR* InPackageNameOrFileName)
{
	if (InPackageNameOrFileName)
	{
		...
		{
			UE_LOGF(LogLinker, Warning, "GetPackagePath: Path \"%ls\" is not in a mounted path; returning empty PackagePath. LoadPackage or GetPackageLinker will fail.", InPackageNameOrFileName);
			return FPackagePath();
		}
	}
	else if (InOuter)
	...
	{
		UE_LOGF(LogLinker, Warning, "GetPackagePath was passed an empty PackageName.");
		return FPackagePath();
	}
}
...
void ConditionalFlushAsyncLoadingForLinkers(TConstArrayView<FLinkerLoad*> InLinkers)
{
	...
	if (InLinkers.Num() > 0 && IsAsyncLoading())
	{
		...
		{
			if (Linker->AsyncRoot != nullptr)
			{
				int32 Request = INDEX_NONE;
				if (!Linker->GetInstancingContext().IsInstanced())
				{
					Request = LoadPackageAsync(Linker->GetPackagePath(), FLoadPackageAsyncOptionalParams{ .PackagePriority = MAX_int32 });
				}
				else
				{
					FName RemappedPackageName = Linker->GetInstancingContext().RemapPackage(Linker->GetPackagePath().GetPackageFName());
					Request = LoadPackageAsync(Linker->GetPackagePath(), FLoadPackageAsyncOptionalParams{ .CustomPackageName = RemappedPackageName, .PackagePriority = MAX_int32, .InstancingContext = &Linker->GetInstancingContext() });
				}
		...
		if (RequestIds.Num())
		{
			UE_LOGF(LogLinker, Log, "Conditionally flushing loading for linker(s) (%ls)", *GetPathNameSafe(InLinkers[0]->LinkerRoot));
