...
int32 FPropertyTypeNameTable::StoreByIndex(const FPropertyTypeNameNode* Nodes, int32 Count)
{
	...
	UE_CLOGF(Count > GPropertyTypeNameBlockOffsetCount, LogCore, Fatal,
		"Invalid property type name with %d nodes. This can happen when serializing from a corrupt or invalid archive.", Count);
	...
	UE_CLOGF(Index + Count > GPropertyTypeNameBlockCount * GPropertyTypeNameBlockOffsetCount, LogCore, Fatal,
		"Exceeded property type name capacity of %d nodes when storing %d nodes.",
