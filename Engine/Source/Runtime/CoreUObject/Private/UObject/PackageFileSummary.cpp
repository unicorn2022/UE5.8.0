...
void operator<<(FStructuredArchive::FSlot Slot, FPackageFileSummary& Sum)
{
	...
	if (BaseArchive.IsLoading())
	{
		...
		UE_CLOGF(!bCanStartSerializing, LogLinker, Warning,
			"Failed to read package file summary, the file \"%ls\" is too small (%lld bytes, expected at least %lld bytes)",
	...
	if (Sum.Tag == PACKAGE_FILE_TAG || Sum.Tag == PACKAGE_FILE_TAG_SWAPPED)
	{
		...
		*/
			...
			if (LegacyFileVersion < 0) // means we have modern version numbers
			{
				...
				if (Sum.bUnversioned)
				{
					...
					if (!GAllowUnversionedContentInEditor)
					{
						...
						UE_LOGF(LogLinker, Warning, "Failed to read package file summary, the file \"%ls\" is unversioned and we cannot safely load unversioned files in the editor.", *BaseArchive.GetArchiveName());
						return;
					}
#endif
				...
				if (Sum.FileVersionUE < VER_UE4_OLDEST_LOADABLE_PACKAGE 
					|| (IsEnforcePackageCompatibleVersionCheck() && Sum.IsFileVersionTooNew()))
				{
		...
		{
			UE_LOGF(LogLinker, Warning, "Failed to read package file summary, the file \"%ls\" has invalid compression flags (%d).", *BaseArchive.GetArchiveName(), Sum.CompressionFlags);
		...
		if (CompressedChunks.Num())
		{
			...
			UE_LOGF(LogLinker, Warning, "Failed to read package file summary, the file \"%ls\" is has package level compression (and is probably cooked). These old files cannot be loaded in the editor.", *BaseArchive.GetArchiveName());
			Sum.InvalidateFileVersion();
