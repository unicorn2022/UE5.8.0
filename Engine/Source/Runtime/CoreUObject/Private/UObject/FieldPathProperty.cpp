...
FFieldPathProperty::FFieldPathProperty(FFieldVariant InOwner, const FName& InName)
...
bool FFieldPathProperty::Identical(TNotNull<const void*> A, const void* B, uint32 PortFlags) const
{
	if (!B)
	{
		return false;
...
void FFieldPathProperty::SerializeItem(FStructuredArchive::FSlot Slot, TNotNull<void*> Value, void const* Defaults) const
{
	FFieldPath* FieldPtr = GetPropertyValuePtr(Value);
	Slot << *FieldPtr;
}

void FFieldPathProperty::ExportText_Internal( FString& ValueStr, TNotNull<const void*> PropertyValueOrContainer, EPropertyPointerType PropertyPointerType, const void* DefaultValue, UObject* Parent, int32 PortFlags, UObject* ExportRootScope ) const
{
	FFieldPath Value;
...
const TCHAR* FFieldPathProperty::ImportText_Internal(const TCHAR* Buffer, TNotNull<void*> ContainerOrPropertyPtr, EPropertyPointerType PropertyPointerType, UObject* Parent, int32 PortFlags, FOutputDevice* ErrorText) const
{
	check(Buffer);
	...
	if (InPathName.Split(TEXT("."), &OldPackageName, &Tmp) && Tmp.Split(TEXT(":"), &OldClassName, &OldFieldName))
	{
		// Check for full property path redirect
		{
			...
			if (FCoreRedirects::RedirectNameAndValues(RedirectFlags, OldRedirectName, NewObjectName, &FoundValueRedirect))
			{
				...
				UE_LOGF(LogCoreRedirects, Verbose, "FFieldPathProperty: Redirected '%ls' -> '%ls'", *InPathName, *NewPathName);
				return NewPathName;
			}
		}
