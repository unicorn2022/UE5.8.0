...
public:
	...
	static void HandleEnsureFailed(const ANSICHAR* /*Expr*/, const ANSICHAR* /*File*/, int32 /*Line*/, const TCHAR* /*Msg*/, const TCHAR* /*CombinedErrorMsg*/)
	{
		VerseHangDetection::EndOverheadScope();
	}
...
#if WITH_EDITORONLY_DATA
		FCoreDelegates::GetOnPostEngineInit().AddStatic(&RegisterCustomLocalizationDataGathering);
#endif

#if UE_TRACK_ENSURE_OVERHEAD_DURING_SCRIPTS
		// Register ensure handler for time tracking during script execution.
		FCoreDelegates::OnEnsureFailed.AddStatic(&FCoreUObjectModule::HandleEnsureFailed);
#endif // UE_TRACK_ENSURE_OVERHEAD_DURING_SCRIPTS
...
UClass* Z_Construct_UClass_UObject(ETypeConstructPhase)
{
...
static TAutoConsoleVariable<bool> CVarEnableLinkerPatchingForReinstancedExports(
	TEXT("s.EnableLinkerPatchingForReinstancedExports"),
	true,
	TEXT("Enable/disable linker patching of existing exports that get moved aside during deferred subobject instancing on load.")
);
					...
					else
					{
						...
						if ( (GIsClient && bShouldLoadForClient) || (GIsServer && bShouldLoadForServer) || bShouldLoadForEditor || bShouldLoadForStandalone )
						{
							...
							if ( SubobjectOuter == nullptr )
							{
								...
								// Constructing the outer may have also instanced this subobject - avoid reconstructing it again with the same archetype.
								InstancedSubobject = GetDestinationObject(SourceSubobject);
								if (InstancedSubobject != nullptr)
								{
							...
							if (IsLoadingObject()
								...
								&& (FOverridableManager::Get().IsEnabled(SubobjectOuter)
									|| SubobjectOuter->GetClass()->ShouldUseDynamicSubobjectInstancing()))
							{
								...
								ExistingObject = StaticFindObjectFast(nullptr, SubobjectOuter, SubobjectName);
								if (ExistingObject)
								{
									if (!ExistingObject->HasAnyFlags(RF_NeedLoad | RF_NeedPostLoad)
									...
									else
									{
										...
										// Also, don't reset the linker as this object may still be pending load/PostLoad; this is handled by linker patching below.
										FName NewName = MakeUniqueObjectName(SubobjectOuter, ExistingObject->GetClass(), SubobjectName);
										ExistingObject->Rename(*NewName.ToString(), nullptr, REN_DoNotDirty | REN_NonTransactional | REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
									}
								}
							}
							...
							if (!InstancedSubobject)
							{
								...
								// Copy transient flags from template to avoid saving subobjects that shouldn't be saved
								EObjectFlags SourceFlagsToCopy = SourceSubobject->GetFlags() & (EObjectFlags)(RF_Transient | RF_TextExportTransient);
								Params.SetFlags = SubobjectOuter->GetMaskedFlags(RF_PropagateToSubObjects) | SourceFlagsToCopy;
							...
							// Patch the subobject into the export table when replacing an existing object while loading
							if (IsLoadingObject() && ExistingObject
								&& ExistingObject != InstancedSubobject
								&& CVarEnableLinkerPatchingForReinstancedExports.GetValueOnAnyThread())
							{
								if (FLinkerLoad* Linker = ExistingObject->GetLinker())
								{
									// This should only be happening on the game thread or for outer objects that are still in the construction loading phase.
									check(IsInGameThread() || (IsInAsyncLoadingThread() && SubobjectOuter->HasAnyInternalFlags(EInternalObjectFlags::AsyncLoadingPhase1)));

									// Acquire the linker's mutex first in case this is an async load
									UE::TUniqueLock LinkerLock(Linker->Mutex);
									Linker->PRIVATE_PatchNewObjectIntoExport(ExistingObject->GetLinkerIndex(), InstancedSubobject);
								}
							}
						}
					}
...
UObject* FObjectInstancingGraph::InstancePropertyValue(UObject* SubObjectTemplate, TNotNull<UObject*> CurrentValue, TNotNull<UObject*> Owner, EInstancePropertyValueFlags Flags)
{
	...
	else if (!bCausesInstancing && !bAllowSelfReference && Owner->GetClass()->ShouldUseDynamicSubobjectInstancing())
	{
		...
		// As an optimization, we only need to do this even if the Owner is the same as destination root during reinstantiation.
		if (Owner != DestinationRoot || GIsReinstancing)
		{
		...
		else if (CurrentValue->HasAllFlags(RF_ArchetypeObject)
				 && SubObjectTemplate && CurrentValue->GetOuter() == SubObjectTemplate->GetOuter()->GetClass()->GetDefaultObject())
		{
			// In this case, the source may have been instanced from class default data (e.g. a transient reference).
			// It is similar to the case above in that if the instancing graph does not yet contain a mapping for this
			// archetype, we'll allow instancing to proceed and construct a new instance under the current owner. But
			// in this scenario, the current value is referencing an archetype that's based outside of its source graph.
			bCausesInstancing = true;
		}
	}
...
{
	checkf(HasDestinationRoot(), TEXT("Expecting the destination root to be set before calling this method"));
...
void FObjectInstancingGraph::PrepopulateWithInstance(TNotNull<UObject*> ObjectInstance, TNotNull<UObject*> InArchetype)
{
	checkf(!HasDestinationRoot(), TEXT("Expecting destination root to NOT be set before calling method"));

	SourceToDestinationMap.Add(InArchetype, ObjectInstance);
	DestinationToSourceMap.Add(ObjectInstance, InArchetype);
}

void FObjectInstancingGraph::RemoveInstanceFromPrepopulate(TNotNull<UObject*> ObjectInstance)
{
	checkf(!HasDestinationRoot(), TEXT("Expecting destination root to NOT be set before calling method"));

	UObject** FoundArchetype = DestinationToSourceMap.Find(ObjectInstance);

	if (FoundArchetype)
	{
		check(*FoundArchetype);
		SourceToDestinationMap.Remove(*FoundArchetype);
		DestinationToSourceMap.Remove(ObjectInstance);
	}
}
