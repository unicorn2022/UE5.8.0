	...
	FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& Args)
		{
			...
			UE_LOGF(LogGarbage, Display, "Memory allocated for GC history: %lldb", TotalSize);
		})
);
	...
	FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& Args)
		{
			if (Args.Num())
			{
				...
				UE_LOGF(LogGarbage, Display, "GC history size changed to: %d", HistorySize);
			}
			else
			{
				UE_LOGF(LogGarbage, Display, "GC history size currently set to: %d", FGCHistory::Get().GetHistorySize());
			}			
		})
);
