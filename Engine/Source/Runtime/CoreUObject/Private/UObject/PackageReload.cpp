...
void DumpExternalReferences(UObject* InObject, UPackage* InPackage)
{
	...
	{
		UE_LOGF(LogUObjectGlobals, Display, "ReloadPackage external references for '%ls'.", *InObject->GetPathName());
		for (const FString& ExternalRefDump : ExternalRefDumps)
		{
			UE_LOGF(LogUObjectGlobals, Display, "    %ls", *ExternalRefDump);
		}
	}
}
...
UPackage* ValidateAndPreparePackageForReload(UPackage* InExistingPackage)
{
	...
	{
		UE_LOGF(LogUObjectGlobals, Warning, "ReloadPackage cannot reload '%ls' as it is marked PKG_InMemoryOnly.", *InExistingPackage->GetName());
		return nullptr;
	}
...
UPackage* LoadReplacementPackage(UPackage* InExistingPackage, const uint32 InLoadFlags)
{
	...
	const ERenameFlags PkgRenameFlags = REN_DoNotDirty | REN_DontCreateRedirectors | REN_NonTransactional | REN_SkipGeneratedClasses | REN_AllowPackageLinkerMismatch;
	...
	if (!NewPackage)
	{
		UE_LOGF(LogUObjectGlobals, Warning, "ReloadPackage cannot reload '%ls' as the new package failed to load. The old package will be restored.", *ExistingPackageName);
...
TSharedPtr<FPackageReloadedEvent> GeneratePackageReloadEvent(UPackage* InExistingPackage, UPackage* InNewPackage)
{
	...
	if (InExistingPackage && InNewPackage)
	{
		...
		for (const auto& ObjectMappingPair : RedirectedObjectsMap)
		{
			...
			{
				UE_LOGF(LogUObjectGlobals, Display, "ReloadPackage failed to find a replacement object for '%ls' (transient) in the new package '%ls'. Any existing references to this object will be nulled out.", *ExistingObject->GetPathName(InExistingPackage), *InNewPackage->GetName());
			}
			else
			{
				UE_LOGF(LogUObjectGlobals, Warning, "ReloadPackage failed to find a replacement object for '%ls' in the new package '%ls'. Any existing references to this object will be nulled out.", *ExistingObject->GetPathName(InExistingPackage), *InNewPackage->GetName());
			}
		}
...
void ReloadPackages(const TArrayView<FReloadPackageData>& InPackagesToReload, TArray<UPackage*>& OutReloadedPackages, int32 InNumPackagesPerBatch)
{
	...
	NewPackages.Refs.Reserve(ExistingPackages.Refs.Num());
	{
		...
		while (PackageIndex < ExistingPackages.Refs.Num())
		{
			...
			for (; PackageIndex < ExistingPackages.Refs.Num(); ++PackageIndex)
			{
				...
				const bool bEndBatch = true; //(PackageIndex == (BatchStartIndex + InNumPackagesPerBatch - 1)) || (ExistingPackage && ExistingPackage->ContainsMap());
				if (bEndBatch)
				{
			...
			if (PackageIndex > 0 && (PackageIndex % InNumPackagesPerBatch) == 0)
			{
				CollectGarbage(GARBAGE_COLLECTION_KEEPFLAGS);
			}
	...
	for (int32 PackageIndex = 0; PackageIndex < ExistingPackages.Refs.Num(); ++PackageIndex)
	{
		...
		if (ExistingPackage && NewPackage)
		{
			UE_LOGF(LogUObjectGlobals, Warning, "ReloadPackage failed to purge the old package '%ls'. This is unexpected, and likely means that it was still externally referenced.", *ExistingPackage->GetName());
