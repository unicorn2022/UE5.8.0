...
bool GValidateVirtualExportsFileOffset = true;
static FAutoConsoleVariableRef CVarValidateVirtualExportsFileOffset(
	TEXT("save.ValidateVirtualExportsFileOffset"),
	GValidateVirtualExportsFileOffset,
	TEXT("When true additional validation will be run when saving a package file."));
...
ESavePackageResult ValidatePackage(FSaveContext& SaveContext)
{
	...
	if (UE::IsSavingPackage() && !SaveContext.IsConcurrent())
	{
	...
	if (SaveContext.IsCooking())
	{
		...
		if (SaveContext.IsStripEditorOnly())
		{
			...
			{
				UE_CLOGF(SaveContext.IsGenerateSaveError(), LogSavePackage, Verbose, "Package marked as editor-only: %ls. Package will not be saved.", *SaveContext.GetPackage()->GetName());
	...
	if (Asset)
	{
		...
		if (!SaveContext.IsCooking() && TopLevelFlags != RF_NoFlags && !Asset->HasAnyFlags(TopLevelFlags))
		{
			...
			{
				UE_LOGF(LogSavePackage, Warning, "The Asset %ls being saved is missing the RF_Standalone flag; adding it.", *Asset->GetPathName());
			...
			else
			{
				...
				{
					UE_LOGF(LogSavePackage, Warning, "%ls", *ErrorText.ToString());
				}
	...
	if (SaveContext.IsWarningLongFilename())
	{
		...
		if (CleanBaseFilename.Len() > MaxFilenameLength)
		{
			...
			{
				UE_LOGF(LogSavePackage, Warning, "%ls", *FString::Printf(TEXT("Filename is too long (%d characters); this may interfere with cooking for consoles. Unreal filenames should be no longer than %d characters. Filename value: %s"), BaseFilename.Len(), MaxFilenameLength, *BaseFilename));
			}
		}
	}
...
void EnsurePackageLocalization(UPackage* InPackage, const FSaveContext& InSaveContext)
{
#if USE_STABLE_LOCALIZATION_KEYS
	if (GIsEditor)
	{
		bool bAlwaysResetPackageLocIdOnSave = false;
		GConfig->GetBool(TEXT("Internationalization"), TEXT("bAlwaysResetPackageLocIdOnSave"), bAlwaysResetPackageLocIdOnSave, GEditorIni);
		if (bAlwaysResetPackageLocIdOnSave && !InSaveContext.IsCooking())
		{
			// Package localization IDs are meant to be stable by default. But to automatically fix conflicts, having them reset on save can be useful in some contexts.
			// The downside being that text keys might get regenerated (they might feel unstable). Having translation memory with external tools can mitigate that downside though.
			TextNamespaceUtil::ResetPackageNamespace(InPackage);
		}
		else
		{
			// In any case, we need to ensure that we have a package localization namespace as the package loading will need it
			// We need to do this before entering the UE::IsSavingPackage() block as it may change the package meta-data
...
{
#if WITH_EDITOR
	// If the in memory package filename is different the filename we are saving it to and if the save context allows for it,
	// regenerate a new persistent id for the package.
	UPackage* Package = SaveContext.GetPackage();
	if (!SaveContext.ShouldKeepPersistentGUID() && !Package->GetLoadedPath().IsEmpty() && Package->GetLoadedPath() != SaveContext.GetTargetPackagePath())
	{
...
AUTORTFM_DISABLE ESavePackageResult HarvestPackage(FSaveContext& SaveContext)
{
		...
		ForEachObjectWithPackage(SaveContext.GetPackage(), [&TryHarvestRootObject, TopLevelFlags](UObject* InObject)
			{
				...
				return true;
			}, EGetObjectsFlags::IncludeNestedObjects, RF_Transient);
	}
#if WITH_VERSE_VM || defined(__INTELLISENSE__)
...
ESavePackageResult ValidateRealms(FSaveContext& SaveContext)
{
	...
	if (SaveContext.GetIllegalReferences().Num())
	{
		for (const FIllegalReference& Reference : SaveContext.GetIllegalReferences())
		{
			...
			{
				UE_LOGF(LogSavePackage, Error, "%ls", *ErrorMessage);
			}
...
ESavePackageResult ValidateExports(FSaveContext& SaveContext)
{
	...
	{
		UE_CLOGF(SaveContext.IsGenerateSaveError(), LogSavePackage, Verbose, "No exports found (or all exports are editor-only) for %ls. Package will not be saved.", SaveContext.GetFilename());
	...
	if (SaveContext.GetPackage()->ContainsMap())
	{
		...
		if (!bContainsMap)
		{
			...
			UE_LOGF(LogSavePackage, Error, "Attempting to save a map package '%ls' that does not contain a map object.", *SaveContext.GetPackage()->GetName());
	...
	{
		if (!UE::FPropertyBagRepository::IsPropertyBagPlaceholderObjectFeatureEnabled(UE::FPropertyBagRepository::EPlaceholderObjectFeature::AllowSavingPlaceholderExports))
		{
	...
	if (SaveContext.HasExternalExportValidations())
	{
		...
		{
			SaveContext.Result = ValidateExport({ SaveContext.GetPackage(), Exports, SaveContext.GetObjectSaveContext().SaveOverrides, Flags, SaveContext.GetTargetPlatform(), OutputDevice });
		...
		if (SaveContext.NameExists(NAME_UniqueObjectNameForCooking))
		{
			...
			for (const FTaggedExport& Export : SaveContext.GetExports())
			{
				...
				if (NameInUse.GetComparisonIndex() == UniqueObjectNameForCookingComparisonId)
				{
					...
					UE_LOGF(LogSavePackage, Warning, "Saving object into cooked package %ls which was created at cook time, Object Name %ls, Full Path %ls, Class %ls, Outer %ls, Outer class %ls", SaveContext.GetFilename(), *NameInUse.ToString(), *Export.Obj->GetFullName(), *Export.Obj->GetClass()->GetName(), Outer ? *Outer->GetName() : TEXT("None"), Outer ? *Outer->GetClass()->GetName() : TEXT("None"));
				}
			}
		}
	}
	...
	ESaveRealm HarvestedRealm)
{
	...
	if (ObjectsInOtherMaps.Num() > 0)
	{
		...
		ErrorEpilogue = FString(" (private object belongs to an external map).");
	}
	else if (PrivateObjects.Num() > 0)
	{
		ObjectsToCheck = &PrivateObjects;
		ErrorPrologue = FString::Printf(TEXT("Can't save '%s': Illegal reference to private object: "), SaveContext.GetFilename());
		ErrorEpilogue = FString(".");
	}
	else if (PrivateContentObjects.Num() > 0)
	{
		...
		ErrorEpilogue = FString(", which is in a different mount point and its package is marked as PKG_NotExternallyReferenceable.");
	}
	...
	if (ObjectsToCheck)
	{
		...
		UE_LOGF(LogSavePackage, Warning, "%ls", *CulpritDiagnostics);
		if (SaveContext.IsGenerateSaveError())
		{
			SaveContext.GetError()->Logf(ELogVerbosity::Warning, TEXT("%s"), *ErrorMessage);
		}
...
ESavePackageResult ValidateImports(FSaveContext& SaveContext)
{
	...
	for (const TObjectPtr<UObject>& Import : Imports)
	{
		...
		if (SaveContext.IsPrestreamPackage(ImportPackage))
		{
			...
			UE_LOGF(LogSavePackage, Display, "Prestreaming package %ls ", *ImportPackage.GetPathName());
			continue;
		}
		...
		if (bWrongImport)
		{
			...
			{
				UE_LOGF(LogSavePackage, Warning, "Bad Object=%ls", *Import->GetFullName());
			}
			else
			{
...
ESavePackageResult CreateLinker(FSaveContext& SaveContext)
{
	...
	else
	{
		...
		else if (!SaveContext.IsTextFormat())
		{
			...
			if (!LinkerSave->TryAssignFileSaver(TempFilename, bByteSwapping, bSaveUnversioned))
			{
				...
				FText ErrorText = FText::Format(
					...
					Arguments);
				UE_LOGF(LogSavePackage, Error, "%ls", *ErrorText.ToString());
				if (SaveContext.IsGenerateSaveError())
				{
		...
		else // IsTextFormat
		{
			...
			UE_LOGF(LogSavePackage, Error, "Could not save package to text format. Text Archive Support is not compiled into this version of the editor executable.");
			return ESavePackageResult::Error;
#else
			...
			if (!LinkerSave->TryAssignFileSaver(TempFilename, bByteSwapping, bSaveUnversioned))
			{
				UE_LOGF(LogSavePackage, Error, "Could not create temporary save filename %ls.", *TempFilename);
				return ESavePackageResult::Error;
			}
	...
	// Map Export Indices
	{
		...
		for (FObjectExport& Export : Linker->ExportMap)
		{
			...
			if (SaveContext.IsCooking())
			{
				...
				UE_CLOGF(Export.TemplateIndex.IsNull(), LogSavePackage, Fatal, "%ls was an archetype of %ls but returned a null index mapping the object.", *Archetype->GetFullName(), *Export.Object->GetFullName());
		...
		if (!SaveContext.IsCooking())
		{
			...
			for (FObjectExport& Export : Linker->ExportMap)
			{
				if (!Export.ClassIndex.IsNull())
				{
					...
					{
						FObjectExport& ClassExport = Linker->ExportMap[Export.ClassIndex.ToExport()];
						ClassObject = ClassExport.Object;
					}
					else
					{
						FObjectImport& ClassImport = Linker->ImportMap[Export.ClassIndex.ToImport()];
						ClassObject = ClassImport.XObject;
					}
					
					if (ClassObject)
					{
						UStruct* Struct = Cast<UStruct>(ClassObject);
						if (Struct)
						{
							const FPackageIndex PackageIndex = Linker->MapObject(ClassObject);
							// This should always be in the linker tables already.
							if (ensureMsgf(!PackageIndex.IsNull(), TEXT("%s: Struct object: '%s' is not mapped"), *Linker->LinkerRoot->GetFullName(), *Struct->GetFullName()))
							{
								...
								if (!Linker->ImportTypeHierarchies.Contains(PackageIndex))
								{
									...
									// that support PropertyBag placeholders, or when we will have dynamic imports
									const bool bExportHasDynamicImports = ((Export.ObjectFlags & RF_HasDynamicImports) != 0);
									if (bExportHasDynamicImports || UE::CanCreatePropertyBagPlaceholderForType(Struct->GetClass(), Struct))
									{
...
void GetCellsWithOuter(Verse::FAllocationContext Context, FSaveContext& SaveContext, UObject* Outer, TArray<Verse::VCell*>& Stack, TSet<Verse::VCell*>& Results)
{
	struct FSubCellVisitor : UE::SavePackageUtilities::Private::FArchiveSavePackageCollector
	{
		...
		{
			if (InCell && InCell->GetCppClassInfo()->InstancedCell)
			{
				// VObjects representing class instances are not currently instanced from CDOs,
				// so there is no need to traverse into them here. See VValueObject::InstancedCell.
				if (Verse::VObject* Object = InCell->DynamicCast<Verse::VObject>(); Object && !Object->IsStruct())
				{
					return *this;
				}
...
AUTORTFM_DISABLE void SavePreloadDependencies(FStructuredArchive::FRecord& StructuredArchiveRoot, FSaveContext& SaveContext)
{
#if WITH_VERSE_VM || defined(__INTELLISENSE__)
	Verse::FAllocationContext Context = Verse::FAllocationContextPromise{};
#endif
	...
	auto IncludeObjectAsDependency = [Linker, &SaveContext](int32 CallSite, TSet<FPackageIndex>& AddTo, TObjectPtr<UObject> ToTest, UObject* ForObj, bool bMandatory, bool bOnlyIfInLinkerTable)
	{
		...
		if (ToTest)
		{
			...
			{
				UE_LOGF(LogSavePackage, Warning, "A dependency '%ls' of '%ls' is in the linker table, but is transient. We will keep the dependency anyway (%d).", *ToTest.GetFullName(), *ForObj->GetFullName(), CallSite);
			}
			if (!Index.IsNull() && !IsValid(ToTest))
			{
				UE_LOGF(LogSavePackage, Warning, "A dependency '%ls' of '%ls' is in the linker table, but is pending kill or garbage. We will keep the dependency anyway (%d).", *ToTest.GetFullName(), *ForObj->GetFullName(), CallSite);
			}
			bool bIncludedInHarvest = SaveContext.IsIncluded(ToTest);
			if (bMandatory && !bIncludedInHarvest)
			{
				UE_LOGF(LogSavePackage, Warning, "A dependency '%ls' of '%ls' was filtered, but is mandatory. This indicates a problem with editor only stripping. We will keep the dependency anyway (%d).", *ToTest.GetFullName(), *ForObj->GetFullName(), CallSite);
			...
			if (bIncludedInHarvest)
			{
				...
				{
					UE_CLOGF(Outermost->HasAnyPackageFlags(PKG_CompiledIn), LogSavePackage, Verbose, "A compiled in dependency '%ls' of '%ls' was not actually in the linker tables and so will be ignored (%d).", *ToTest->GetFullName(), *ForObj->GetFullName(), CallSite);
					UE_CLOGF(!Outermost->HasAnyPackageFlags(PKG_CompiledIn), LogSavePackage, Fatal, "A dependency '%ls' of '%ls' was not actually in the linker tables and so will be ignored (%d).", *ToTest->GetFullName(), *ForObj->GetFullName(), CallSite);
				}
			}
			check(!bMandatory);
		}
	...
	if (SaveContext.IsCooking())
	{
		...
		UE_LOGF(LogSavePackage, VeryVerbose, "Saved %d dependencies for %d exports.", Linker->Summary.PreloadDependencyCount, Linker->ExportMap.Num());
		...
		for (int32 CellExportIndex = 0; CellExportIndex < Linker->CellExportMap.Num(); ++CellExportIndex)
		{
...
#if WITH_VERSE_VM || defined(__INTELLISENSE__)
			{
				...
				{
					// Map keys are hashed and compared during insertion.
					// TODO: Recursively add the elements of composite keys (structs, tuples, arrays, maps, options, rationals).
				...
				if (Verse::VClass* Class = Export.Cell->DynamicCast<Verse::VClass>())
				{
					...
					for (int32 Index = 0; Index < Archetype.NumEntries; ++Index)
					{
						...
						if (Value.IsCellOfType<Verse::VFunction>())
						{
...
AUTORTFM_DISABLE ESavePackageResult WritePackageHeader(FStructuredArchive::FRecord& StructuredArchiveRoot, FSaveContext& SaveContext)
{
	FLinkerSave* Linker = SaveContext.GetLinker();
#if WITH_EDITOR
...
[[nodiscard]] ESavePackageResult WriteBulkData(FSaveContext& SaveContext, int64& VirtualExportsFileOffset)
{
	...
	// Saving non-inline bulk data to separate file(s) (Cooking/EditorDomain)
	...
	if (Linker.IsCooking() == false)
	{
		...
		if (bSaveBulkDataByReference)
		{
			...
			{
				UE_LOGF(LogSavePackage, Error, "Save bulk data '%ls' FAILED, reason '%ls'",
	...
	auto GetFilePath = [&SaveContext, bIsOptionalRealm](EPackageExtension Ext, FBulkDataCookedIndex CookedIndex = FBulkDataCookedIndex::Default) -> FString
	{
		...
		{
			FString FileExt = bIsOptionalRealm ? FString(".o") + LexToString(Ext) : LexToString(Ext);
...
ESavePackageResult WriteAdditionalExportFiles(FSaveContext& SaveContext)
{
	...
	if (SaveContext.IsCooking() && SaveContext.AdditionalFilesFromExports.Num() > 0)
	{
		...
		for (FLargeMemoryWriter& Writer : SaveContext.AdditionalFilesFromExports)
		{
			...
			UE_LOGF(LogSavePackage, Verbose, "SavePackage writing additional file '%ls' due to package '%ls'.", *FileInfo.Filename, *FileInfo.PackageName.ToString());
	...
	// Write Real Summary
	{
		...
		// @todo: custom versions: when can this be checked?
		{
			...
			for (const FCustomVersion& LinkerCustomVer : Linker->GetCustomVersions().GetAllVersions())
			{
				...
				{
					UE_LOGF(LogSavePackage, Error,
						"Unexpected custom version \"%ls\" found when saving %ls. This usually happens when export tagging and final serialization paths differ. Package will not be saved.",
	...
	FPackageFileSummary& Summary, bool bByteSwapping, FStringView PackageDebugName, FIoHash& OutSavedHash)
{
	...
	{
		UE_LOGF(LogSavePackage, Error,
			"Error writing PackageSaveHash for '%.*ls': Could not hash file %ls.",
	...
	{
		UE_LOGF(LogSavePackage, Error,
			"Error writing PackageSaveHash for '%.*ls': Could not open for writing file %ls.",
			PackageDebugName.Len(), PackageDebugName.GetData(), *Filename);
		return false;
	}
	...
	*ReopenedFile << OutSavedHash;
	...
	{
		UE_LOGF(LogSavePackage, Error,
			"Error writing PackageSaveHash for '%.*ls': Write failed to file %ls.",
			PackageDebugName.Len(), PackageDebugName.GetData(), *Filename);
		return false;
	}
...
ESavePackageResult FinalizeFile(FStructuredArchive::FRecord& StructuredArchiveRoot, FSaveContext& SaveContext)
{
	...
	else
	{
		...
		else if (!SaveContext.IsTextFormat())
		{
			...
			{
				UE_LOGF(LogSavePackage, Error, "Error writing temp file '%ls' for '%ls'",
			...
			if (FinalizeResult != ESavePackageResult::Success)
			{
				if (SaveContext.IsGenerateSaveError())
				{
					UE_LOGF(LogSavePackage, Error, "%ls",
				...
				{
					UE_LOGF(LogSavePackage, Warning, "Error saving '%ls'", SaveContext.GetFilename());
				}
		...
		else
		{
			...
			if (FinalizeResult != ESavePackageResult::Success)
			{
				if (SaveContext.IsGenerateSaveError())
				{
					UE_LOGF(LogSavePackage, Error, "%ls", *FString::Printf(TEXT("Error saving '%s'"), SaveContext.GetFilename()));
				...
				{
					UE_LOGF(LogSavePackage, Warning, "%ls", *FString::Printf(TEXT("Error saving '%s'"), SaveContext.GetFilename()));
				}
				return FinalizeResult;
			}
		}
...
AUTORTFM_DISABLE ESavePackageResult SaveHarvestedRealms(FSaveContext& SaveContext, ESaveRealm HarvestingContextToSave)
{
	...
	{
		ensureMsgf(!GValidateVirtualExportsFileOffset || SaveContext.GetLinker()->Tell() == VirtualExportsFileOffset,
			TEXT("VirtualExportsFileOffset is no longer in sync (%lld vs %lld) when writing out '%s'"),
			VirtualExportsFileOffset, SaveContext.GetLinker()->Tell(), * SaveContext.GetPackage()->GetName());
	}
 ...
 */
AUTORTFM_DISABLE ESavePackageResult InnerSaveInternal(FSaveContext& SaveContext)
{
	// Create slow task dialog if needed
	...
	// @todo: Need to adjust UE::IsSavingPackage() to properly prevent generating reference once, package harvesting is done
	// UE::IsSavingPackage() is too harsh however, since it should be scope only to the current package.
	//
	// Note: ValidateIllegalReferences will temporarily override FScopedSavingFlag's setting of UE::IsSavingPackage()
...
void BeginSave(FSaveContext& SaveContext, FScopedSlowTask* SlowTask = nullptr)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UPackage::BeginSave);
	...
	}
	{
		if (!SaveContext.IsConcurrent())
		{
			...
			EnsurePackageLocalization(Package, SaveContext);
...
AUTORTFM_DISABLE void InnerSave(FSaveContext& SaveContext, FScopedSlowTask* SlowTask = nullptr)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UPackage::InnerSave);

	SaveContext.Result = InnerSaveInternal(SaveContext);

	// in case of failure or cancellation, do not exit here, still run cleanup (e.g. PostSaveRoot)
}

void EndSave(FSaveContext& SaveContext, FScopedSlowTask* SlowTask = nullptr)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UPackage::EndSave);
...
#define COOK_SAVEPACKAGE_SCOPE(Package) \
	UE_LOG_CONTEXT("Package", FSoftObjectPath(Package)); \
	COOK_STAT(FScopedDurationTimer FuncSaveTimer(FSavePackageStats::SavePackageTimeSec)); \
	UE_SCOPED_COOK_STAT(Package->GetFName(), EPackageEventStatType::SavePackage); \
	UE_TRACK_REFERENCING_PACKAGE_SCOPED(Package, PackageAccessTrackingOps::NAME_Save);

UE::Tasks::TTask<FSavePackageResultStruct> UPackage::SaveAsync(UPackage* InPackage, UObject* InAsset, const TCHAR* InFilename,
	const FSavePackageArgs& SaveArgs)
{
	AutoRTFM::UnreachableIfClosed("TODO: Mark this function as AUTORTFM_DISABLED and fixup callers");

	COOK_STAT(FSavePackageStats::NumPackagesSaved++);

	auto SaveContext = MakeShared<FSaveContext>(InPackage, InAsset, InFilename, SaveArgs);

	// It's important to know if the request was aborted after BeginSave. We cannot
	// just look for errors in the context because if BeginSave didn't error out,
	// we need to execute both InnerSave and EndSave even if InnerSave fails.
	auto AbortFlag = MakeShared<bool>(false);
	auto BeginTask =
		UE::Tasks::Launch(
			TEXT("BeginSave"),
			[SaveContext, AbortFlag]()
			{
				AutoRTFM::UnreachableIfClosed("This should never get called under autortfm");

				COOK_SAVEPACKAGE_SCOPE(SaveContext->GetPackage());

				BeginSave(*SaveContext);

				*AbortFlag = (SaveContext->Result != ESavePackageResult::Success);
			},
...
FSavePackageResultStruct UPackage::Save(UPackage* InPackage, UObject* InAsset, const TCHAR* InFilename,
	const FSavePackageArgs& SaveArgs)
{
	AutoRTFM::UnreachableIfClosed("TODO: Mark this function as AUTORTFM_DISABLED and fixup callers");
	...
	COOK_SAVEPACKAGE_SCOPE(InPackage);
	COOK_STAT(FSavePackageStats::NumPackagesSaved++);
	...
	BeginSave(SaveContext, &SlowTask);
	if (SaveContext.Result != ESavePackageResult::Success)
	{
		return SaveContext.Result;
	}
	...
	SlowTask.EnterProgressFrame();
	InnerSave(SaveContext, &SlowTask);
	EndSave(SaveContext, &SlowTask);

	return SaveContext.GetFinalResult();
}

#undef COOK_SAVEPACKAGE_SCOPE
...
ESavePackageResult UPackage::SaveConcurrent(TArrayView<FPackageSaveInfo> InPackages, const FSavePackageArgs& SaveArgs, TArray<FSavePackageResultStruct>& OutResults)
{
	...
	TArray<FSaveContext> PackageSaveContexts;
	{
		...
		for (FPackageSaveInfo& PackageSaveInfo : InPackages)
		{
			...
			// Ensures
			EnsurePackageLocalization(SaveContext.GetPackage(), SaveContext);
	...
	SlowTask.EnterProgressFrame();
	{
		...
		// @todo: Once UE::IsSavingPackage() is reworked we should reinstore the saving flag here for the gc lock
		// Passing in false here so that UE::IsSavingPackage() is set to true on top of locking the GC
		//FScopedSavingFlag IsSavingFlag(false);

		// Concurrent Part
		ParallelFor(PackageSaveContexts.Num(), [&PackageSaveContexts] AUTORTFM_DISABLE (int32 PackageIdx)
			{
				InnerSaveInternal(PackageSaveContexts[PackageIdx]);
			});
	}
