...
FLazyObjectProperty::FLazyObjectProperty(FFieldVariant InOwner, const FName& InName)
	: TFObjectPropertyBase(InOwner, InName)
{
}
...
void FLazyObjectProperty::SerializeItem(FStructuredArchive::FSlot Slot, TNotNull<void*> Value, void const* Defaults) const
{
	...
	if( !UnderlyingArchive.IsObjectReferenceCollector() || UnderlyingArchive.IsModifyingWeakAndStrongReferences() )
	{
		...
		Slot << *(FLazyObjectPtr*)NotNullGet(Value);
...
bool FLazyObjectProperty::Identical(TNotNull<const void*> A, const void* B, uint32 PortFlags) const
{
	FLazyObjectPtr ObjectA = *(FLazyObjectPtr*)NotNullGet(A);
