...
FSoftObjectProperty::FSoftObjectProperty(FFieldVariant InOwner, const FName& InName)
...
bool FSoftObjectProperty::Identical(TNotNull<const void*> A, const void* B, uint32 PortFlags) const
{
	FSoftObjectPtr ObjectA = *(FSoftObjectPtr*)NotNullGet(A);
...
void FSoftObjectProperty::SerializeItem(FStructuredArchive::FSlot Slot, TNotNull<void*> Value, void const* Defaults) const
{
	...
	{
		Slot << *(FSoftObjectPtr*)NotNullGet(Value);
	}
	else
	{
...
bool FSoftObjectProperty::NetSerializeItem(FArchive& Ar, UPackageMap* Map, TNotNull<void*> Data, TArray<uint8>* MetaData) const
{
	// Serialize directly, will use FBitWriter/Reader
	Ar << *(FSoftObjectPtr*)NotNullGet(Data);

	return true;
}

void FSoftObjectProperty::ExportText_Internal( FString& ValueStr, TNotNull<const void*> PropertyValueOrContainer, EPropertyPointerType PropertyPointerType, const void* DefaultValue, UObject* Parent, int32 PortFlags, UObject* ExportRootScope ) const
{
...
const TCHAR* FSoftObjectProperty::ImportText_Internal(const TCHAR* InBuffer, TNotNull<void*> ContainerOrPropertyPtr, EPropertyPointerType PropertyPointerType, UObject* Parent, int32 PortFlags, FOutputDevice* ErrorText) const
{
	FSoftObjectPath SoftObjectPath;
