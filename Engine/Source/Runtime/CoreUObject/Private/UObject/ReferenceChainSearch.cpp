...
namespace UE::ReferenceChainSearch
{
	...
	struct FPolicyUObjectHeap
	{
		...
		FVertex ObjectToVertex(ConstObjectType Object) const
		{
			...
			UE_CLOGF(Vertex < 0 || Vertex >= GetNumVertices(), LogReferenceChain, Fatal, "Invalid object index in reference chain search %d", Vertex);
			return Vertex;
		}
	...
	struct FPolicyGCHistory
	{
		...
		FVertex ObjectToVertex(ObjectType Object) const
		{
			...
			UE_CLOGF(!Vertex || *Vertex < 0 || *Vertex >= GetNumVertices(), LogReferenceChain, Fatal, "Invalid object index in reference chain search %d", Vertex ? *Vertex : -1);
			return *Vertex;
		}
		...
		UE::Graph::FGraph& OutGraph)
	{
		...
		if (bMinimal)
		{
			...
			UE_LOGF(LogReferenceChain, Display, "InitialGather memory usage: %.2f", static_cast<double>(OutGraph.GetAllocatedSize() + Search.GetAllocatedSize()) / 1024.0 / 1024.0);
		}
		else
		{
	...
	void RemoveDuplicatedChains(TArray<FGraphPath>& InOutPaths, FVertex GCObjectReferencerVertex)
	{
		...
		TSparseMap<FChainDedupKey, FGraphPath*> UniqueChains;
...
FString FReferenceChainSearch::GetObjectFlags(const FGCObjectInfo& InObject)
{
	...
	{
		Flags += FString::Printf(TEXT("(MaybeUnreachable<%d>) "), uint32{EnumToUnderlyingType(FGCFlags::GetMaybeUnreachableFlagValue_ForGC())});
	}
	...
	, VerbosityForPrint(InVerbosityForPrint)
{
	...
	UE_LOGF(LogReferenceChain, Display, "Post-search memory usage: %.2f", static_cast<double>(ReferenceGraph.GetAllocatedSize() + Paths.GetAllocatedSize() + GetAllocatedSize()) / 1024.0 / 1024.0);
...
void FReferenceChainSearch::PerformSearchFromGCSnapshot(TConstArrayView<UObject*> InObjectsToFindReferencesTo, FGCSnapshot& InSnapshot)
{
	...
	for (const UObject* Obj : InObjectsToFindReferencesTo)
	{
		...
		{
			UE_LOGF(LogReferenceChain, Warning, "Target object %ls was not present in GC snapshot.", *Obj->GetPathName());
		}
	}
	...
	for (TPair<FReferenceToken, TArray<FGCDirectReference>*>& Pair : InSnapshot.DirectReferences)
	{
		for (const FGCDirectReference& RefInfo : *Pair.Value)
		{
			...
			{
				UE_LOGF(LogReferenceChain,
					Display,
					"Direct ref in GC history from %ls to %ls",
...
TArray<FString> FReferenceChainSearch::FindAndPrintStaleReferencesToObjects(TConstArrayView<UObject*> ObjectsToFindReferencesTo, EPrintStaleReferencesOptions Options)
{
	...
	UE_LOGF(LogLoad, Log, "Beginning reference chain search...");
	for (UObject* Obj : ObjectsToFindReferencesTo)
	{
		UE_LOGF(LogLoad, Log, " - %ls", *Obj->GetFullName());
	}
	...
	for (UObject* ObjectToFindReferencesTo : ObjectsToFindReferencesTo)
	{
		...
		else
		{
			...
			if (LastGCSnapshot)
			{
				UE_LOGF(LogLoad, Log, "Looking for references to %ls in the last GC run history...", *ObjectToFindReferencesTo->GetFullName());
	...
	if ((Options & EPrintStaleReferencesOptions::Fatal) == EPrintStaleReferencesOptions::Fatal)
	{
		...
		// Emit an error and then a fatal error since Gauntlet can truncate Fatal Error messages.
		...
		UE_LOGF(LogLoad, Fatal, "Crashing due to uobject leaks (world leaks). See log above for details.");
	}
