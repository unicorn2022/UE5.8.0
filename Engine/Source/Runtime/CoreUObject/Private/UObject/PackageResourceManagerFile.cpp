...
bool FPackageResourceManagerFile::TryMatchCaseOnDisk(const FPackagePath& PackagePath, FPackagePath* OutPackagePath)
{
	...
	if (!FilenameOnDisk.IsEmpty())
	{
		if (OutPackagePath)
		{
			...
			{
				UE_LOGF(LogPackageName, Verbose,
					"TryMatchCaseOnDisk: Unexpected non-matching LocalPath \"%ls\" found when searching for PackagePath \"%ls\". Case will not be normalized.",
	...
	FStringView PackageMount, FStringView FileMount, FStringView RootRelPath, FStringView BasenameWildcard)
{
	...
	for (const FString& Filename : FoundFilenames)
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogPackageResourceManager, Warning,
					"FindPackagesRecursive: Filename \"%ls\" returned from FindFilesRecursive does not start with RootPath \"%ls\". Ignoring it.",
	...
	bool bIsDirectory, FPackagePath& OutPackagePath, EPackageSegment& OutPackageSegment)
{
	...
	if (!Filename.StartsWith(FileMount, ESearchCase::IgnoreCase))
	{
		...
		{
			UE_LOGF(LogPackageResourceManager, Warning,
				"FDirectoryVisitorBaseMounted: FileManager IterateDirectoryRecursively(\"%ls\") returned file \"%ls\" that is not a subpath of the root \"%ls\".",
