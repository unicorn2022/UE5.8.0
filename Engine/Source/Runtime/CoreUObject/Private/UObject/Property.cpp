...
const TCHAR* ReadPropertyToken(const TCHAR* Buffer, TStringType& Out, bool bDottedNames)
{
	if (*Buffer == TEXT('"'))
	{
		...
		{
			UE_LOGF(LogProperty, Warning, "ReadToken: Bad quoted string: %ls", Buffer);
			return nullptr;
		}
		Buffer += NumCharsRead;
...
const TCHAR* FPropertyHelpers::ReadToken( const TCHAR* Buffer, FString& Out, bool bDottedNames)
{
	return ReadPropertyToken(Buffer, Out, bDottedNames);
}
...
//
// Parse a terminated token.
//
template <typename TStringType>
const TCHAR* ReadTerminatedPropertyToken(const TCHAR* Buffer, TStringType& Out, const TCHAR Terminator, const int32* MaxTokenLen)
{
	TStringBuilder<256> Temp;
	while (*Buffer && *Buffer != Terminator && (!MaxTokenLen || Temp.Len() < *MaxTokenLen)) // Walk to the Terminator, with an optional limit to guard against bad data
	{
		if (*Buffer == TEXT('"'))
		{
			int32 NumCharsRead = 0;
			...
			{
				UE_LOGF(LogProperty, Warning, "ReadTerminatedToken: Bad quoted string: %ls", Buffer);
				return nullptr;
			}
			Buffer += NumCharsRead;
		}
	...
	if (*Buffer != Terminator) // Verify we found the Terminator
	{
		return nullptr;
	}
	Out += Temp.ToView();
...
const TCHAR* FPropertyHelpers::ReadTerminatedToken(const TCHAR* Buffer, FString& Out, const TCHAR Terminator, const int32* MaxTokenLen)
{
	return ReadTerminatedPropertyToken(Buffer, Out, Terminator, MaxTokenLen);
}

const TCHAR* FPropertyHelpers::ReadTerminatedToken(const TCHAR* Buffer, FStringBuilderBase& Out, const TCHAR Terminator, const int32* MaxTokenLen)
{
	return ReadTerminatedPropertyToken(Buffer, Out, Terminator, MaxTokenLen);
}
...
// Constructors.
//
FProperty::FProperty(FFieldVariant InOwner, const FName& InName)
...
FProperty::FProperty(FFieldVariant InOwner, const UECodeGen_Private::FPropertyParamsBaseWithOffset& Prop, EPropertyFlags AdditionalPropertyFlags /*= CPF_None*/)
	: Super(InOwner, UTF8_TO_TCHAR(Prop.NameUTF8))
	, ArrayDim(1)
	, ElementSize(0)
...
bool FProperty::ValidateImportFlags( uint32 PortFlags, FOutputDevice* ErrorHandler ) const
{
	...
	if ((PortFlags & PPF_RestrictImportTypes) && (PropertyFlags & CPF_Config))
	{
		...
		{
			UE_LOGF(LogProperty, Warning, "%ls", *ErrorMsg);
		}
...
bool FProperty::ShouldSerializeValue(FArchive& Ar) const
{
	...
	if (IsEditorOnlyProperty() && Ar.IsFilterEditorOnly())
	{
		...
		if (bAllowed)
		{
			UE_LOGF(LogInit, Display, "allowing serialize of EO property, even when filtering EO data in an Optional object - %ls", *GetFullName());
		}
#else
		bool bAllowed = false;
#endif
...
// Net serialization.
//
bool FProperty::NetSerializeItem( FArchive& Ar, UPackageMap* Map, TNotNull<void*> Data, TArray<uint8> * MetaData ) const
{
...
namespace UE::CoreUObject::Private
{
	...
	{
		UE_LOGF(LogProperty, Fatal, "Invalid property size %u when linking property %ls of size %d", InvalidPropertySize, *Prop->GetFullName(), Prop->GetSize());
		for (;;);
	}
}
											...
											FOutputDevice* Warn, TArray<FDefinedProperty>& DefinedProperties )
{
	...
	if (*Str && !PropertyName.IsNone())
	{
		...
		if(*Str == '.')
		{
			...
			// Special case for struct properties
			else if (FStructProperty* const StructProperty = ExactCastField<FStructProperty>(Property))
			{
				TArray<FDefinedProperty> StructDefinedProperties;
				return ImportSingleProperty(Str, Property->ContainerPtrToValuePtr<void>(DestData), StructProperty->Struct, SubobjectOuter, PortFlags, Warn, StructDefinedProperties);
			}
		}
			...
			if (Operation.IsSet() && Operation.GetValue() != EOverriddenPropertyOperation::Replace)
			{
				if (ArrayProperty)
				{
					...
					switch(Operation.GetValue())
					{
						...
						{
							checkf(ArrayProperty->Inner->HasAnyPropertyFlags(CPF_PersistentInstance), TEXT("Only instanced sub object is supporting externalized operation"));
							const FObjectPropertyBase* InnerObjectProperty = CastFieldChecked<FObjectPropertyBase>(ArrayProperty->Inner);
