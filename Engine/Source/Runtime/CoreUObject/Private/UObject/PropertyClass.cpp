...
FClassProperty::FClassProperty(FFieldVariant InOwner, const FName& InName)
	...
	: Super(InOwner, (const UECodeGen_Private::FObjectPropertyParams&)Prop)
{
	...
	MetaClass = Prop.MetaClassFunc ? Prop.MetaClassFunc(ETypeConstructPhase::Inner) : nullptr;
}
...
const TCHAR* FClassProperty::ImportText_Internal(const TCHAR* Buffer, TNotNull<void*> ContainerOrPropertyPtr, EPropertyPointerType PropertyPointerType, UObject* Parent, int32 PortFlags, FOutputDevice* ErrorText) const
{
...
bool FClassProperty::Identical(TNotNull<const void*> A, const void* B, uint32 PortFlags) const
{
	TObjectPtr<UObject> ObjectA = GetObjectPtrPropertyValue(A);
...
#endif
	{
		// We need to see A and B in the same check.
		checkf(
			(ObjectA == nullptr || ObjectA.IsA<UClass>()) && (ObjectB == nullptr || ObjectB.IsA<UClass>()),
			TEXT("FClassProperty::Identical - ObjectA: (Name: %s Path: %s Class: %s) ObjectB: (Name: %s Path: %s Class: %s)"),
			ObjectA ? *ObjectA->GetName() : TEXT("nullptr"),
			ObjectA ? *ObjectA->GetPathName() : TEXT("nullptr"),
			ObjectA ? *ObjectA->GetClass()->GetName() : TEXT("nullptr"),
			ObjectB ? *ObjectB->GetName() : TEXT("nullptr"),
			ObjectB ? *ObjectB->GetPathName() : TEXT("nullptr"),
			ObjectB ? *ObjectB->GetClass()->GetName() : TEXT("nullptr")
		);
	}
