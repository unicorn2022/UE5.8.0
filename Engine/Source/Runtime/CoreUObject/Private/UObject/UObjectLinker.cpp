...
void UObject::SetLinker( FLinkerLoad* LinkerLoad, int32 LinkerIndex, bool bShouldDetachExisting, bool bSilenceLogs)
{
	...
	if( Existing.Linker && bShouldDetachExisting )
	{
		...
		{
			UE_CLOGF(!bSilenceLogs, LogUObjectLinker, Error,
				"Detaching from existing linker %ls while object %ls needs loading (%ls). Setting linker to %ls. See log for more information.",
	...
	{
		bShouldDetachExisting = false; // no previous linker, we don't need to detach anything
	}
	...
	if (bShouldDetachExisting)
	{
		...
		PostLinkerChange();
#else
		UE_CLOGF(Existing.Linker && LinkerLoad, LogUObjectLinker, Fatal,
			"It is only legal to change linkers in the editor. Trying to change linker on %ls from %ls (Existing->LinkerRoot=%ls) to %ls (LinkerLoad->LinkerRoot=%ls)",
			*GetFullName(),
