...
FCoreUObjectDelegates::FOnHotfixableAssetLoaded FCoreUObjectDelegates::OnHotfixableAssetLoaded;
FCoreUObjectDelegates::FOnPackageLoadCompleted FCoreUObjectDelegates::OnPackageLoadCompleted;
...
FCoreUObjectDelegates::FOnObjectRenamed FCoreUObjectDelegates::OnLoadedObjectRenamed;
...
FCoreUObjectDelegates::FOnObjectsReinstanced FCoreUObjectDelegates::OnObjectsReinstanced;
...
// This map holds raw UClass pointers. It is the responsibility of the callers
// of Register/UnregisterHotfixableAssetClass to keep these pointers alive.
// The class pointer is reference counted in the map to allow for multiple
// Register/Unregister calls from different sources.
// Guarded by HotfixableAssetClassesLock: IsHotfixableAssetClass may be called
// from the async loading thread concurrently with Register/Unregister on the game thread.
static FRWLock HotfixableAssetClassesLock;
static TMap<UClass*, uint32> HotfixableAssetClasses;
// Number of distinct registered classes. Enables a lock-free fast path in IsHotfixableAssetClass.
static std::atomic<int32> HotfixableAssetClassCount{0};

void FCoreUObjectDelegates::RegisterHotfixableAssetClass(UClass* Class)
{
	FWriteScopeLock Lock(HotfixableAssetClassesLock);
	uint32& RefCount = HotfixableAssetClasses.FindOrAdd(Class);
	if (RefCount == 0)
	{
		HotfixableAssetClassCount.fetch_add(1, std::memory_order_release);
	}
	++RefCount;
}

void FCoreUObjectDelegates::UnregisterHotfixableAssetClass(UClass* Class)
{
	FWriteScopeLock Lock(HotfixableAssetClassesLock);
	if (uint32* RefPtr = HotfixableAssetClasses.Find(Class))
	{
		if (*RefPtr <= 1)
		{
			HotfixableAssetClasses.Remove(Class);
			HotfixableAssetClassCount.fetch_sub(1, std::memory_order_release);
		}
		else
		{
			--*RefPtr;
...
bool FCoreUObjectDelegates::IsHotfixableAssetClass(UClass* Class)
{
	if (HotfixableAssetClassCount.load(std::memory_order_acquire) == 0)
	{
		return false;
	}

	FReadScopeLock Lock(HotfixableAssetClassesLock);
	return HotfixableAssetClasses.Contains(Class);
}
...
void TrashObject(TNotNull<UObject* const> Object)
{
	// We should *never* trash an object that is in the process of being loaded.
	// TODO: This should probably degrade to a `check` longer term, but out of
	// an abundance of caution we'll start it as an `ensure`.

	const bool bIsBeingLoaded = Object->HasAnyFlags(EObjectFlags::RF_WillBeLoaded) ||
		Object->HasAnyInternalFlags(EInternalObjectFlags_AsyncLoading);

	if (ensure(!bIsBeingLoaded))
	{
		const bool bIsAPackage = Object->IsA<UPackage>();

		const FName BaseName = bIsAPackage ? NAME_TrashedPackage : NAME_None;

		const FName NewName = MakeUniqueObjectName(Object->GetOuter(), Object->GetClass(), BaseName);
		Object->Rename(*NewName.ToString(), nullptr, REN_DontCreateRedirectors | REN_NonTransactional | REN_DoNotDirty);
...
{
	UE_CLOGF(UE::IsSavingPackage(nullptr), LogUObjectGlobals, Fatal, "Illegal call to StaticFindObjectFast() while serializing object data!");
	UE_CLOGF(IsGarbageCollectingAndLockingUObjectHashTables(), LogUObjectGlobals, Fatal, "Illegal call to StaticFindObjectFast() while garbage collecting!");
...
UObject* StaticFindObjectChecked( UClass* ObjectClass, UObject* ObjectParent, FStringView InName, EFindObjectFlags Flags )
{
	...
	if( !Result )
	{
		UE_LOGF(LogUObjectGlobals, Fatal, "Failed to find object '%ls %ls.%.*ls'", *ObjectClass->GetName(), ObjectParent ? *ObjectParent->GetName() : TEXT("None"), InName.Len(), InName.GetData());
	}
#endif
	return Result;
}
...
{
	UE_CLOGF(UE::IsSavingPackage(nullptr) || IsGarbageCollectingAndLockingUObjectHashTables(), LogUObjectGlobals, Fatal, "Illegal call to StaticFindAllObjectsFast() while serializing object data or garbage collecting!");
...
bool StaticFindAllObjects(TArray<UObject*>& OutFoundObjects, UClass* ObjectClass, const TCHAR* OrigInName, EFindObjectFlags Flags)
{
	...
	UE_CLOGF(UE::IsSavingPackage(nullptr), LogUObjectGlobals, Fatal, "Illegal call to StaticFindAllObjects() while serializing object data!");
	UE_CLOGF(IsGarbageCollectingAndLockingUObjectHashTables(), LogUObjectGlobals, Fatal, "Illegal call to StaticFindAllObjects() while collecting garbage!");
		...
		if (StaticFindAllObjectsFast(FoundObjects, Class, ObjectName, FindObjectFlags))
		{
			if (FoundObjects.Num() > 1)
			{
				...
				if (AmbiguousMessageVerbosity != ELogVerbosity::NoLogging || !!(Options & EFindFirstObjectOptions::EnsureIfAmbiguous))
				{
					...
					{
						UE_LOGF(LogUObjectGlobals, Fatal, "%ls", Message.ToString());
					}
...
void StaticTick( float DeltaTime, bool bUseFullTimeLimit, float AsyncLoadingTime )
{
	...
	if( GNativeDuplicate )
	{
		UE_LOGF(LogUObjectGlobals, Fatal, "Duplicate native registered: %i", GNativeDuplicate );
	}
...
bool SafeLoadError( UObject* Outer, uint32 LoadFlags, const TCHAR* ErrorMessage)
{
	...
	{
		UE_LOGF(LogUObjectGlobals, Error, "%ls", ErrorMessage);
		bRetVal = true;
	}
	else
	{
		...
		{ 
			UE_LOGF(LogUObjectGlobals, Warning, "%ls", ErrorMessage);
			bRetVal = true;
		}
	}
...
UPackage* FindPackage( UObject* InOuter, const TCHAR* PackageName )
{
	...
	{
		UE_LOGF(LogUObjectGlobals, Fatal, "Attempted to find a package named 'None' - InName: %ls", PackageName);
	}
	return Result;
}
...
UPackage* CreatePackage(const TCHAR* PackageName )
{
	...
	if( PackageName )
	{
		...
		if( InName.ToView().EndsWith( TEXT( "." ) ) )
		{
			...
			UE_LOGF(LogUObjectGlobals, Log,  "Invalid Package Name entered - '%ls' renamed to '%ls'", *InName, InName2.GetData() );
			InName = InName2;
		}
	}
	...
	if ( InName.Len() == 0 )
	{
		UE_LOGF(LogUObjectGlobals, Fatal, "%ls", TEXT("Attempted to create a package with an empty package name.") );
	}
	...
	if ( InName.ToView() != TEXT("None") )
	{
		...
		if( Result == NULL )
		{
			...
			{
				UE_LOGF(LogUObjectGlobals, Warning, "Attempted to create a package with a short package name: %ls Outer: %ls", PackageName, Outer ? *Outer->GetFullName() : TEXT("NullOuter"));
			}
			else
			{
	...
	{
		UE_LOGF(LogUObjectGlobals, Fatal, "%ls", TEXT("Attempted to create a package named 'None'") );
	}
...
FString ResolveIniObjectsReference(const FString& ObjectReference, const FString* IniFilename, bool bThrow)
{
	...
	if (!GConfig->GetString(*Section, *Key, Output, *IniFilename))
	{
		if (bThrow == true)
		{
			UE_LOGF(LogUObjectGlobals, Error, " %ls %ls ", *FString::Printf(TEXT("Can't find '%s' in configuration file section=%s key=%s"), *ObjectReference, *Section, *Key), **IniFilename);
		}
	}
...
UObject* StaticLoadObjectInternal(UClass* ObjectClass, UObject* InOuter, FStringView InName, FStringView Filename, uint32 LoadFlags, UPackageMap* Sandbox, bool bAllowObjectReconciliation, const FLinkerInstancingContext* InstancingContext)
{
	...
	uint32 ResolveFlags = LoadFlags;
	ResolveFlags &= LOAD_EditorOnly | LOAD_NoVerify | LOAD_DeferDependencyLoads;
	ResolveFlags |= LOAD_Quiet | LOAD_NoWarn;
	// break up the name into packages, returning the innermost name and its outer
	ResolveName2(InOuter, StrName, true, true, ResolveFlags, InstancingContext);
...
{
	UObject* Result = StaticLoadObjectInternal(ObjectClass, InOuter, InName, Filename, LoadFlags | LOAD_NoWarn, Sandbox, bAllowObjectReconciliation, InstancingContext);
	if (!Result)
	{
		...
		bool bWarnIfMissing = (LoadFlags & (LOAD_NoWarn | LOAD_Quiet)) == 0;
		auto IsKnownMissing = [](UObject* Obj) { return Obj && FLinkerLoad::IsKnownMissingPackage(FName(*Obj->GetPathName())); };
		if (bWarnIfMissing && !IsKnownMissing(InOuter))
		{
			// we haven't created or found the object, error
			TStringBuilder<128> ObjectPath;
			if (ObjectClass)
			{
				ObjectPath << ObjectClass->GetFName() << TEXT(" ");
			}
			if (InOuter)
			{
				InOuter->GetPathName(nullptr, ObjectPath);
				ObjectPath << TEXT(".");
			}
			ObjectPath << ObjectName;

			FFormatNamedArguments Arguments;
			Arguments.Add(TEXT("ObjectPath"), FText::FromStringView(ObjectPath.ToView()));
			const FString Error = FText::Format(NSLOCTEXT("Core", "ObjectNotFound", "Failed to find object '{ObjectPath}'"), Arguments).ToString();
	...
	virtual void Serialize(void* V, int64 Length) override
	{
		...
		if (DiffArchive && !bDisable)
		{
			...
			if (FMemory::Memcmp((const void*)Data.GetData(), V, Length) != 0)
			{
				...
				UE_LOGF(LogUObjectGlobals, Warning, "Diff cooked package archive recognized a difference %lld Filename %ls", Pos, *InnerArchive.GetArchiveName());

				UE_LOGF(LogUObjectGlobals, Warning, "debug stack %ls", *DebugStackString);
	...
	const FLinkerInstancingContext* InstancingContext, const FPackagePath* DiffPackagePath)
{
	...
	{
		UE_LOGF(LogUObjectGlobals, Warning, "Attempted to LoadPackage from empty PackagePath.");
		return nullptr;
	}
	...
	{
		UE_LOGF(LogUObjectGlobals, Warning, "Attempted to LoadPackage from non-mounted path %ls. This is not supported.", *PackagePath.GetDebugName());
		return nullptr;
	}
...
UPackage* LoadPackage(UPackage* InOuter, const TCHAR* InLongPackageNameOrFilename, uint32 LoadFlags, FArchive* InReaderOverride, const FLinkerInstancingContext* InstancingContext)
{
	...
	if (InLongPackageNameOrFilename && InLongPackageNameOrFilename[0] != '\0')
	{
		...
		if (FPackageName::IsShortPackageName(FStringView(InLongPackageNameOrFilename)))
		{
			...
			if (ScriptPackageName)
			{
				UE_LOGF(LogUObjectGlobals, Warning, "LoadPackage: %ls is a short script package name.", InLongPackageNameOrFilename);
			...
			{
				UE_CLOGF(bWarnIfMissing, LogUObjectGlobals, Warning, "LoadPackage can't find package %ls.", InLongPackageNameOrFilename);
				return nullptr;
			}
		}
	...
	{
		UE_LOGF(LogUObjectGlobals, Warning, "Empty name passed to LoadPackage.");
		return nullptr;
	}
...
UPackage* LoadPackage(UPackage* InOuter, const FPackagePath& PackagePath, uint32 LoadFlags, FArchive* InReaderOverride, const FLinkerInstancingContext* InstancingContext, const FPackagePath* DiffPackagePath)
{
...
#if	STATS && 0
	static FString Package = "Package";
	...
	UE_TRACE_METADATA_SCOPE_ASSET_FNAME(NAME_None, UPackage::StaticClass()->GetFName(), PackagePath.GetPackageFName());
...
void BeginLoad(FUObjectSerializeContext* LoadContext, const TCHAR* DebugContext)
{
	...
	if (!LoadContext->HasStartedLoading() && !IsInAsyncLoadingThread())
	{
		...
		{
			UE_LOGF(LogUObjectGlobals, Log, "BeginLoad(%ls) is flushing async loading", DebugContext);
		}
...
void EndLoad(FUObjectSerializeContext* LoadContext, TArray<UPackage*>* OutLoadedPackages)
{
...
#endif
	TSet<UObject*> HotfixableAssetsLoaded;
	...
	while (LoadContext->DecrementBeginLoadCount() == 0 && (LoadContext->HasLoadedObjects() || LoadContext->HasPendingImportsOrForcedExports()))
	{
		...
		while (LoadContext->HasLoadedObjects())
		{
			...
			for( int32 CurObjIndex=0; CurObjIndex<ObjLoaded.Num(); CurObjIndex++ )
			{
				...
				if (FCoreUObjectDelegates::IsHotfixableAssetClass(Obj->GetClass()))
				{
					// If this is a sync load during async loading. Async loading will handle this.
					if (!FUObjectThreadContext::Get().AsyncPackageLoader)
					{
						HotfixableAssetsLoaded.Add(Obj);
					}
				}
			}
	...
	for (UObject* HotfixableAsset : HotfixableAssetsLoaded)
	{
		check(HotfixableAsset);
		FCoreUObjectDelegates::OnHotfixableAssetLoaded.Broadcast(HotfixableAsset);
	}
		...
		void Store(UObject* Parent, FNameEntryId BaseId, FName UsedName)
		{
			...
			if (!Entry)
			{
				...
				else
				{
					...
					UE_LOGF(LogUniqueObjectName, Log, "EVICT: %ls", *FName(Entry->BaseName, Entry->BaseName, NAME_NO_NUMBER_INTERNAL).ToString());
					Entry->Reset(BaseId);
				}

				UE_LOGF(LogUniqueObjectName, Log, "STORE: %ls", *FName(BaseId, BaseId, NAME_NO_NUMBER_INTERNAL).ToString());
			}
			...
			FName AllocateName(UObject* Parent, FNameEntryId BaseId, FName BaseName)
			{
				...
				if (Iterator > NextNewNumber)
				{
					...
					for (int32 i = Start; i > NextNewNumber && i > End; --i)
					{
						...
						{
							UE_LOGF(LogUniqueObjectName, Log, "HIT: %ls %ls", *Parent->GetPathName(), *TestName.ToString());
							Iterator = i - 1;
							return TestName;
						}
					}
				...
				for (int32 i = NextNewNumber; i > 0; --i)
				{
					...
					if (CanUseNameInOuter(Parent, TestName))
					{
						...
						UE_LOGF(LogUniqueObjectName, Log, "MISS: %ls %ls", *Parent->GetPathName(), *TestName.ToString());
						return TestName;
					}
				}
...
namespace UE::ObjectDuplication
{
	...
	void GatherDefaultSubobjectsForDuplication(UObject* SrcObject, UObject* DstObject, FDuplicatedObjectAnnotation& DuplicatedObjectAnnotation, FDuplicateDataWriter& Writer)
	{
...
{
	using namespace UE::ObjectDuplication;
	...
	UE_CLOGF(FPlatformProperties::RequiresCookedData() && Parameters.SourceObject->HasAnyInternalFlags(EInternalObjectFlags_AsyncLoading), LogUObjectGlobals, Warning, "Duplicating object '%ls' that's still being async loaded", *Parameters.SourceObject->GetFullName());
	...
	Writer.SetDuplicatedCells(DuplicatedCells);
	...
	TArray<FSerializedObject> SerializedObjects;
	...
	GatherDefaultSubobjectsForDuplication(Parameters.SourceObject, DupRootObject, DuplicatedObjectAnnotation, Writer);
	...
	while(Writer.UnserializedObjects.Num())
	{
		...
		int64 ObjectOffset = Writer.Tell();
		Object->Serialize(Writer);
		SerializedObjects.Emplace(Object, ObjectOffset);

		// After serialization we may have created default sub-objects (DSO) that we don't want to duplicate on subsequent calls.
		// e.g. Duplicating an ActorComponent that contains a DSO will have appended the ActorComponent to the UnserializedObjects but not accounted for the 
		// DSO in the annotation map. We do that here now.
		FDuplicatedObject DupObj = DuplicatedObjectAnnotation.GetAnnotation(Object);
		if (!DupObj.IsDefault())
		{
			GatherDefaultSubobjectsForDuplication(Object, DupObj.DuplicatedObject.GetEvenIfUnreachable(), DuplicatedObjectAnnotation, Writer);
		}

#if WITH_VERSE_VM || defined(__INTELLISENSE__)
		DuplicatedCells.WriteUnserializedCells(Context, Writer);
#endif
	}
	...
	Reader.SetDuplicatedCells(DuplicatedCells);
...
#endif
	for (const FSerializedObject& Entry : SerializedObjects)
	{
		UObject* SerializedObject = Entry.Object;
	...
	for (const FSerializedObject& Entry : SerializedObjects)
	{
		UObject* OrigObject = Entry.Object;
...
bool StaticAllocateObjectErrorTests( const UClass* InClass, UObject* InOuter, FName InName, EObjectFlags InFlags)
{
	...
	if( !InClass )
	{
		UE_LOGF(LogUObjectGlobals, Fatal, "Empty class for object %ls", *InName.ToString() );
		return true;
	}
	...
	if (FScopedAllowAbstractClassAllocation::IsDisallowedAbstractClass(InClass, InFlags))
	{
		...
		UE_LOGF(LogUObjectGlobals, Warning, "%ls", *ErrorMsg);
	...
	if( InOuter == NULL )
	{
		...
		{
			UE_LOGF(LogUObjectGlobals, Fatal, "%ls", *FString::Printf( TEXT("Object is not packaged: %s %s"), *InClass->GetName(), *InName.ToString()) );
			return true;
		}
...
extern const FName NAME_UniqueObjectNameForCooking("UniqueObjectNameForCooking");
static thread_local bool GOutputCookingWarnings = false;

COREUOBJECT_API bool GetOutputCookingWarnings()
{
	return GOutputCookingWarnings;
}

COREUOBJECT_API void SetOutputCookingWarnings(bool bValue)
{
	GOutputCookingWarnings = bValue;
}
	...
	FGCReconstructionGuard* GCGuard)
{
	...
	FName LLMScope_Name;
	FString LLMClassNameString;
	if (FLowLevelMemTracker::Get().IsTagSetActive(ELLMTagSet::UObjectClasses))
	{
		LLMClassNameString = FString::Printf(TEXT("UObject/%s"), *InClass->GetName());
		LLMScope_Name = *LLMClassNameString;
	}
	LLM_SCOPE_DYNAMIC(LLMScope_Name, ELLMTracker::Default, ELLMTagSet::UObjectClasses, FLLMDynamicTagConstructorStatString(LLMClassNameString));
	...
	if(InName == NAME_None)
	{
		...
		{
#if WITH_EDITOR
			if ( GetOutputCookingWarnings() && GetTransientPackage() != InOuter->GetOutermost() )
			{
	...
	else
	{
		...
		if (Obj && !Obj->GetClass()->IsChildOf(InClass))
		{
			...
			UE_LOGF(LogUObjectGlobals, Fatal,
				"%lsCannot replace existing object of a different class.\n"
				     "\tNew Object: %ls %ls.%ls\n"
				     "\tExisting Object: %ls %ls(0x%08x 0x%08x)",
	...
	int32 OffsetToObject = -InClass->GetPropertiesStartOffset(); // Start offset for properties can be negative so we want to move object forward in allocation
	int32 OldIndex = -1;
	...
	{	
		...
		Obj = (UObject *)GUObjectAllocator.AllocateUObject(TotalSize, Alignment, GIsInitialLoad, OffsetToObject);
	}
	else
	{
...
#if UE_WITH_REMOTE_OBJECT_HANDLE
			|| (UE::RemoteObject::Handle::GetResidence(Obj) == EResidence::Remote)
#endif // UE_WITH_REMOTE_OBJECT_HANDLE
			))
		{
			...
			if (IsInGameThread())
			{
				...
				if (GCGuard && Obj->HasAnyInternalFlags(EInternalObjectFlags_AsyncLoading))
				{
					GCGuard->Lock();
				}
			}
			else if (IsInAsyncLoadingThread() && Obj->HasAnyInternalFlags(EInternalObjectFlags::AsyncLoadingPhase1))
			{
				...
				// No need for the GCGuard here since async loading always has one when its active.
			}
			else
			{
				ensureMsgf(false, TEXT("GC lock can only be acquired on the game thread. If you hit this ensure then an object is being reconstructed on a worker thread which is not thread-safe"));
			}
			...
			if(!Obj->HasAnyFlags(RF_FinishDestroyed))
			{
				...
				{
					// We disable the AutoRTFM sanitizer on this write because this spans the entire
					// object's memory including fields that may be accessed only in the open (e.g.
					// object flags which are set atomically), and mixing closed writes with open
					// writes will trigger the AutoRTFM sanitizer.
					...
					char* InternalIndexEnd = InternalIndexStart + sizeof(std::remove_reference_t<decltype(Obj->GetObjectHashInternalIndex())>);
					char* ObjectEnd = ObjectStart + TotalSize;
					AutoRTFM::RecordOpenWrite(ObjectStart, InternalIndexStart - ObjectStart);
					AutoRTFM::RecordOpenWrite(InternalIndexEnd, ObjectEnd - InternalIndexEnd);
#else
					AutoRTFM::RecordOpenWrite(Obj, TotalSize);
#endif
				};
				...
				if (bPrinted)
				{
					...
					UE_LOGF(LogUObjectGlobals, Warning, "Gamethread hitch waiting for resource cleanup on a UObject (%ls) overwrite took %6.2fms. Fix the higher level code so that this does not happen.", *OldName, ThisTime * 1000.0);
				}
	...
	const bool bIsAsyncLoading = (IsInAsyncLoadingThread() || IsInParallelLoadingThread());
	
	if (!bSubObject)
	{
		// This needs to be set at construction to prevent the object from having a short period where it can be found
		// between its creation and the call to NotifyConstructedDuringAsyncLoading.
		if (bIsAsyncLoading)
		{
			InternalSetFlags |= EInternalObjectFlags::AsyncLoadingPhase1;
		}
		...
		{
			FMemory::Memzero((uint8 *)Obj - OffsetToObject, TotalSize);
...
void UObject::PostInitProperties()
{
	...
	{ 
		UObject* Popped = FUObjectThreadContext::Get().PostInitPropertiesCheck.Pop();
		check(this == Popped);
	});
#endif
...
// Construct the UObject in the open with AutoRTFM sanitizer disabled as UObjects can be 
// reconstructed, and the re-initialization of the object can upset the AutoRTFM sanitizer.
UE_AUTORTFM_ALWAYS_OPEN_NO_SANITIZER
UObject::UObject()
{
#if UE_WITH_REMOTE_OBJECT_HANDLE
	...
	UE_CLOGF(!ObjectInitializerPtr, LogUObjectGlobals, Fatal, "%ls is not being constructed with NewObject.", *GetName());
	FObjectInitializer& ObjectInitializer = *ObjectInitializerPtr;
	UE_CLOGF(ObjectInitializer.Obj != nullptr && ObjectInitializer.Obj != this, LogUObjectGlobals, Fatal, "UObject() constructor called but it's not the object that's currently being constructed with NewObject. Maybe you are trying to construct it on the stack, which is not supported.");
...
UObject::UObject(const FObjectInitializer& ObjectInitializer)
{
	...
	UE_CLOGF(ObjectInitializer.Obj != nullptr && ObjectInitializer.Obj != this, LogUObjectGlobals, Fatal, "UObject(const FObjectInitializer&) constructor called but it's not the object that's currently being constructed with NewObject. Maybe you are trying to construct it on the stack, which is not supported.");
	const_cast<FObjectInitializer&>(ObjectInitializer).Obj = this;
	const_cast<FObjectInitializer&>(ObjectInitializer).FinalizeSubobjectClassInitialization();
}
...
void FObjectInitializer::Construct_Internal()
{
	...
	if (Obj)
	{
		bool bSetupObjectInitializer = GetAllowNativeComponentClassOverrides();
		if (!bSetupObjectInitializer)
		{
			// The Class of the Class may not be available during very early initialization of CoreUObject.
			UClass* ClassClass = Obj->GetClass()->GetClass();
			bSetupObjectInitializer = ClassClass && ClassClass->IsChildOf<UVerseClass>();
		}
		if (bSetupObjectInitializer)
		{
...
FObjectInitializer::~FObjectInitializer()
{
	...
	if (OnGCObjectCreatedHandle.IsValid())
	{
		...
		for (FGCObject* CreatedObj : CreatedGCObjects)
		{
			...
			UE_LOGF(LogUObjectGlobals, Warning, "Class %ls contains an FGCObject (%ls) member or is derived from it", *Obj->GetClass()->GetPathName(), *CreatedObj->GetReferencerName());
		}
	}
#endif // WITH_EDITORONLY_DATA
...
void FObjectInitializer::PostConstructInit()
{
	...
	UE::CoreUObject::Private::ConstructPartials(*Class, Obj);
	...
	UE_AUTORTFM_OPEN
	{
		if (ThreadContext.PostInitPropertiesCheck.IsEmpty() || ThreadContext.PostInitPropertiesCheck.Pop() != Obj)
		{
			UE_LOGF(LogUObjectGlobals, Fatal, "%ls failed to route PostInitProperties. Call Super::PostInitProperties() in %ls::PostInitProperties().", *Obj->GetClass()->GetName(), *Obj->GetClass()->GetName());
		}
	};
...
void FObjectInitializer::FOverrides::Add(FName InComponentName, const UClass* InComponentClass, const TArrayView<const FName>* InFullComponentPath)
{
	...
	else if (InComponentClass)
	{
		if (Overrides[Index].ComponentClass)
		{
			...
			{
				UE_LOGF(LogUObjectGlobals, Error, "%ls is not a legal override for component %ls because it does not derive from %ls. Will use %ls when constructing component.",
	...
	else
	{
		...
		if (Overrides[Index].ComponentClass)
		{
			UE_LOGF(LogUObjectGlobals, Error, "%ls is not a legal override for component %ls because a parent class is marking it do not create.",
				*Overrides[Index].ComponentClass->GetFullName(), *GetSubobjectPath());
		}
		if (Overrides[Index].SubOverrides)
		{
			UE_LOGF(LogUObjectGlobals, Error, "Component %ls has recorded nested subobject overrides, but won't be created because a parent class is marking it do not create.",
				*GetSubobjectPath());
		}
...
FObjectInitializer::FOverrides::FOverrideDetails FObjectInitializer::FOverrides::Get(FName InComponentName, const UClass* ReturnType, const UClass* ClassToConstructByDefault, bool bOptional) const
{
	...
	else if (Overrides[Index].ComponentClass)
	{
		if (IsLegalOverride(Overrides[Index].ComponentClass, ReturnType)) // if THE base class is asking for a T, the existing override (which we are going to use) had better be derived
		{
			...
			{
				UE_LOGF(LogUObjectGlobals, Error, "Ignored DoNotCreateDefaultSubobject for %ls as it's marked as required. Creating %ls.", *InComponentName.ToString(), *Result.Class->GetName());
			}
		}
		else
		{
			if (Overrides[Index].bDoNotCreate)
			{
				UE_LOGF(LogUObjectGlobals, Error, "Ignored DoNotCreateDefaultSubobject for %ls as it's marked as required. Creating %ls.", *InComponentName.ToString(), *ClassToConstructByDefault->GetName());
			}
			UE_LOGF(LogUObjectGlobals, Error, "%ls is not a legal override for component %ls because it does not derive from %ls. Using %ls to construct component.",
...
{
	UE_CLOGF(!bSubobjectClassInitializationAllowed, LogUObjectGlobals, Fatal,
		"%ls.%ls: Subobject class setup is only allowed in base class constructor call (in the initialization list)", Obj ? *Obj->GetFullName() : TEXT("NULL"), *SubobjectName.GetPlainNameString());
}

void FObjectInitializer::AssertIfSubobjectSetupIsNotAllowed(const FStringView SubobjectName) const
{
	UE_CLOGF(!bSubobjectClassInitializationAllowed, LogUObjectGlobals, Fatal,
		"%ls.%.*ls: Subobject class setup is only allowed in base class constructor call (in the initialization list)", Obj ? *Obj->GetFullName() : TEXT("NULL"), SubobjectName.Len(), SubobjectName.GetData());
}
...
void FObjectInitializer::AssertIfSubobjectSetupIsNotAllowed(TArrayView<const FName> SubobjectNames) const
{
	...
	UE_CLOGF(!bSubobjectClassInitializationAllowed, LogUObjectGlobals, Fatal,
		"%ls.%ls: Subobject class setup is only allowed in base class constructor call (in the initialization list)", Obj ? *Obj->GetFullName() : TEXT("NULL"), *MakeSubobjectPath());
}
...
{
	FUObjectThreadContext& ThreadContext = FUObjectThreadContext::Get();
	UE_CLOGF(ThreadContext.IsInConstructor && Outer == ThreadContext.ConstructedObject, LogUObjectGlobals, Fatal, "%ls", ErrorMessage);
}

FObjectInitializer& FObjectInitializer::Get()
{
	FUObjectThreadContext& ThreadContext = FUObjectThreadContext::Get();
	UE_CLOGF(!ThreadContext.IsInConstructor, LogUObjectGlobals, Fatal, "FObjectInitializer::Get() can only be used inside of UObject-derived class constructor.");
...
void ConstructorHelpers::CheckIfIsInConstructor(const TCHAR* ObjectToFind)
{
	...
	UE_CLOGF(!ThreadContext.IsInConstructor, LogUObjectGlobals, Fatal, "FObjectFinders can't be used outside of constructors to find %ls", ObjectToFind);
}
...
{	
	...
	if( NameStartIndex != INDEX_NONE )
	{
		...
		{
			UE_CLOGF( bAssertOnBadPath, LogUObjectGlobals, Fatal, "Bad path name: %ls, missing \' or an incorrect format", *PathName );
		}
	}
}
...
// Process FObjectProperty reference
template<EPropertyCollectFlags CollectFlags>
...
#if WITH_VERSE_VM || defined(__INTELLISENSE__)
		else if (EnumHasAnyFlags(CastFlags, CASTCLASS_FVValueProperty))
		{
			Collector.AddReferencedVerseValue(*static_cast<Verse::TWriteBarrier<Verse::VValue>*>(Value), Referencer, &Property);
		}
		else if (EnumHasAnyFlags(CastFlags, CASTCLASS_FVRestValueProperty))
		{
			Collector.AddReferencedVerseValue(*static_cast<Verse::VRestValue*>(Value), Referencer, &Property);
		}
		else if (EnumHasAnyFlags(CastFlags, CASTCLASS_FVCellProperty))
		{
			Collector.AddReferencedVerseValue(*static_cast<Verse::TWriteBarrier<Verse::VCell>*>(Value), Referencer, &Property);
		}
#endif
		else
		{
	...
	void PerformReachabilityAnalysis( EObjectFlags KeepFlags, EInternalObjectFlags InternalKeepFlags, EObjectFlags SearchFlags = RF_NoFlags, FReferencerInformationList* FoundReferences = NULL)
	{
		...
		for( FThreadSafeObjectIterator It; It; ++It )
		{
			...
			if( Object->IsRooted() )
			{
				...
				checkCode( if( !IsValidChecked(Object) ) { UE_LOGF(LogUObjectGlobals, Fatal, "Object %ls is part of root set though is invalid!", *Object->GetFullName() ); } );
...
private:
	...
	void AddToObjectList( const UObject* ReferencingObject, const FProperty* ReferencingProperty, UObject* Object )
	{
		...
		if ( GIsEditor && !GIsPlayInEditorWorld && !CurrentObject->HasAnyFlags(RF_Transient) && Object->RootPackageHasAnyFlags(PKG_PlayInEditor) )
		{
			...
			{
				UE_LOGF(LogGarbage, Warning, "GC detected illegal reference to PIE object from content [possibly via %ls]:", ReferencingProperty != nullptr ? *ReferencingProperty->GetFullName() : *FString());
				UE_LOGF(LogGarbage, Warning, "      PIE object: %ls", *Object->GetFullName());
				UE_LOGF(LogGarbage, Warning, "  NON-PIE object: %ls", *CurrentObject->GetFullName());
			}
		}
...
{
	UE_CLOGF(!FUObjectThreadContext::Get().IsInConstructor, LogUObjectGlobals, Fatal, "Subobjects cannot be created outside of UObject constructors. UObject constructing subobjects cannot be created using new or placement new operator.");
	if (SubobjectFName == NAME_None)
	{
		UE_LOGF(LogUObjectGlobals, Fatal, "Illegal default subobject name: %ls", *SubobjectFName.ToString());
	}
	...
	if (OverrideClass)
	{
		...
		if (OverrideClass->HasAnyClassFlags(CLASS_Abstract))
		{
			...
			{
				UE_LOGF(LogUObjectGlobals, Warning, "Required default subobject %ls not created as requested class %ls is abstract. Returning null.", *SubobjectFName.ToString(), *OverrideClass->GetName());
			}
		}
		else
		{
			...
			{
				UE_LOGF(LogUObjectGlobals, Fatal, "Default subobject %ls %ls already exists for %ls.", *OverrideClass->GetName(), *SubobjectFName.ToString(), *Outer->GetFullName());
			}
			else
			{
	...
	const int32 Line)
{
	...
	FLogRecord Record;
	Record.SetUtf8Format(UTF8TEXT("[AssetLog] {Asset}: {Message}"));
...
namespace UECodeGen_Private
{
	...
	FProperty* ConstructFProperty(FFieldVariant Outer, const FPropertyParamsBase* const*& PropertyArray, int32& NumProperties)
	{
		...
		switch (PropBase->Flags & PropertyTypeMask)
		{
			...
			{
				NewProp = NewFProperty<FVCellProperty, FVerseCellValuePropertyParams>(Outer, *PropBase);
			}
			break;
#endif
			...
			{
				NewProp = NewFProperty<FVRestValueProperty, FVerseValuePropertyParams>(Outer, *PropBase);
				ReadMore = 1; // Next property is the legacy value
			}
			break;
#endif
	...
	// It is UB to call `SingletonFunc` below because it can return a `UClass*` not `UObject*` (even though
	// there is a parent-child relation there). So we suppress UBSan for this function.
	UE_DISABLE_UBSAN
	void ConstructUPackage(UPackage*& OutPackage, const FPackageParams& Params)
	{
		...
		if (!FoundPackage)
		{
			UE_LOGF(LogUObjectGlobals, Log, "Creating package on the fly %ls", UTF8_TO_TCHAR(Params.NameUTF8));
		...
		FIoHash SavedHash;
		constexpr size_t SavedHashSize = sizeof(SavedHash.GetBytes()); // -V568
		FMemory::Memcpy(&SavedHash.GetBytes(), &DeterministicGuid,
			FMath::Min(SavedHashSize, sizeof(DeterministicGuid)));
...
#endif
		for (FTypeConstructFunc*const* SingletonFunc = Params.SingletonFuncArray, *const* SingletonFuncEnd = SingletonFunc + Params.NumSingletons; SingletonFunc != SingletonFuncEnd; ++SingletonFunc)
		{
			UObject* Object = (*SingletonFunc)(ETypeConstructPhase::Outer);
#if WITH_RELOAD
