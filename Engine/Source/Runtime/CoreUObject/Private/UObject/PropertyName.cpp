...
FNameProperty::FNameProperty(FFieldVariant InOwner, const FName& InName)
...
void FNameProperty::ExportText_Internal(FString& ValueStr, TNotNull<const void*> PropertyValueOrContainer, EPropertyPointerType PropertyPointerType, const void* DefaultValue, UObject* Parent, int32 PortFlags, UObject* ExportRootScope) const
{
	FName Temp;
...
const TCHAR* FNameProperty::ImportText_Internal(const TCHAR* Buffer, TNotNull<void*> ContainerOrPropertyPtr, EPropertyPointerType PropertyPointerType, UObject* Parent, int32 PortFlags, FOutputDevice* ErrorText) const
{
	FName ImportedName;
	if (!(PortFlags & PPF_Delimited))
	{
