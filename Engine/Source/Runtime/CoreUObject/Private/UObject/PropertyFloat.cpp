...
FFloatProperty::FFloatProperty(FFieldVariant InOwner, const FName& InName)
...
bool FFloatProperty::Identical(TNotNull<const void*> A, const void* B, uint32 PortFlags) const
{
	return UE::PreciseFPEqual(*static_cast<const float*>(NotNullGet(A)), B ? *static_cast<const float*>(B) : 0.0f);
}
