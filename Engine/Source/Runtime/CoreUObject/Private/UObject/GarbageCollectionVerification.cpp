...
public:
	...
	FORCEINLINE_DEBUGGABLE void HandleTokenStreamObjectReference(FWorkerContext& ObjectsToSerializeStruct, UObject* ReferencingObject, UObject*& Object, FMemberId MemberId, EOrigin Origin, bool bAllowReferenceElimination)
	{
		if (Object)
		{
				...
				!Object->IsValidLowLevelFast())
			{
				...
				UE_LOGF(LogGarbage, Fatal, "Invalid object while verifying Disregard for GC assumptions: 0x%016llx, ReferencingObject: %ls, %ls, MemberId: %d",
					(int64)(PTRINT)Object,
			...
			{
				UE_LOGF(LogGarbage, Warning, "Disregard for GC object %ls referencing %ls which is not part of root set. Property: %ls",
					*GetFullNameSafe(ReferencingObject),
					*Object->GetFullName(),
					ObjectsToSerializeStruct.SchemaStack ? *ObjectsToSerializeStruct.SchemaStack->ToString() : TEXT("unknown"));
				NumErrors.Increment();
			}
		}
	}
...
{	
	...
	UE_CLOGF(Processor.GetErrorCount() > 0, LogGarbage, Fatal, "Encountered %d object(s) breaking Disregard for GC assumptions. Please check log for details.", Processor.GetErrorCount());
}
	...
	FORCEINLINE_DEBUGGABLE void HandleTokenStreamObjectReference(FGCArrayStruct& ObjectsToSerializeStruct, UObject* ReferencingObject, UObject*& Object, FMemberId MemberId, EOrigin Origin, bool bAllowReferenceElimination)
	{
		if (Object)
		{
				...
				!Object->IsValidLowLevelFast())
			{
				...
				UE_LOGF(LogGarbage, Fatal, "Invalid object while verifying cluster assumptions: 0x%016llx, ReferencingObject: %ls, %ls, MemberId: %d",
					(int64)(PTRINT)Object,
					ReferencingObject ? *ReferencingObject->GetFullName() : TEXT("NULL"),
					*DebugInfo, MemberId.AsPrintableIndex());
			}
			...
			if (ObjectItem->GetOwnerIndex() <= 0)
			{
				...
				if (ObjectItem->HasAnyFlags(EInternalObjectFlags::ClusterRoot))
				{
					...
					&&  !Cluster->MutableObjects.Contains(OtherClusterRootIndex),  // reference to an external object which later became a cluster root is legal 
						LogGarbage, Warning,
						"Object %ls from source cluster %ls (%d) is referencing cluster root object %ls (0x%016llx) (%d) which is not referenced by the source cluster.",
						*GetFullNameSafe(ReferencingObject),
				...
				) 
				{
					...
					UE_LOGF(LogGarbage, Warning, "Object %ls (0x%016llx) from cluster %ls (0x%016llx / 0x%016llx) is referencing 0x%016llx %ls which is not part of root set or cluster.",
			...
			else if (ObjectItem->GetOwnerIndex() != Cluster->RootIndex)
			{
				...
				UE_CLOGF(
					...
					LogGarbage, Warning,
					"Object %ls from source cluster %ls (%d) is referencing object %ls (0x%016llx) from cluster %ls (%d) which is not referenced by the source cluster.",
					*GetFullNameSafe(ReferencingObject),
					*ClusterRootObject->GetFullName(),
					ClusterRootObjectItem->GetClusterIndex(),
					*Object->GetFullName(),
	...
	FORCEINLINE void HandleTokenStreamVerseCellReference(FWorkerContext& Context, UObject* ReferencingObject, Verse::VCell* Cell, FMemberId MemberId, EOrigin Origin)
	{
		if (Cell)
		{
			...
			UE_CLOGF(
				...
				LogGarbage, Warning,
				"Object %ls from source cluster %ls (%d) is referencing cell (0x%016llx) which is not part of cluster.",
				*GetFullNameSafe(ReferencingObject),
				*ClusterRootObject->GetFullName(),
				ClusterRootObjectItem->GetClusterIndex(),
				(int64)(PTRINT)Cell);
...
void VerifyClustersAssumptions()
{
	...
	UE_CLOGF(NumErrors.GetValue() > 0, LogGarbage, Fatal, "Encountered %d object(s) breaking GC Clusters assumptions. Please check log for details.", NumErrors.GetValue());
}
...
void VerifyObjectFlags()
{
	...
	ParallelFor( TEXT("GC.VerifyObjectFlags"),NumThreads,1, [&NumErrors, NumberOfObjectsPerThread, NumThreads, MaxNumberOfObjects](int32 ThreadIndex)
	{
		...
		for (int32 ObjectIndex = 0; ObjectIndex < NumObjects && (FirstObjectIndex + ObjectIndex) < MaxNumberOfObjects; ++ObjectIndex)
		{
			...
			if (ObjectItem.GetObject())
			{
				...
				{
					UE_LOGF(LogGarbage, Warning, "RF_Garbage (%d) and EInternalObjectFlags::Garbage (%d) flag mismatch on %ls%ls",
				...
				{
					UE_LOGF(LogGarbage, Warning, "Object %ls%ls is NOT marked as Reachable at the beginning of GC",
						*FReferenceChainSearch::GetObjectFlags(FGCObjectInfo(Object)),
						*Object->GetFullName());
				...
				{
					UE_LOGF(LogGarbage, Warning, "Object %ls%ls is marked with at least one of the unreachable flags at the beginning of GC",
						*FReferenceChainSearch::GetObjectFlags(FGCObjectInfo(Object)),
						*Object->GetFullName());
	...
	UE_CLOGF(NumErrors > 0, LogGarbage, Fatal, "Encountered %d object(s) breaking Object and Internal flag assumptions. Please check log for details.", (uint32)NumErrors);
}
	...
	FORCEINLINE_DEBUGGABLE void HandleTokenStreamObjectReference(FGCArrayStruct& ObjectsToSerializeStruct, UObject* ReferencingObject, UObject*& Object, FMemberId MemberId, EOrigin Origin, bool bAllowReferenceElimination)
	{
		...
		if (Object)
		{
			...
			if (ObjectItem->HasAnyFlags(FGCFlags::GetMaybeUnreachableFlagValue_ForGC() | EInternalObjectFlags::Unreachable))
			{
				...
				UE_LOGF(LogGarbage, Warning, "Object %ls%ls is being referenced by reachable object %ls through %ls",
					*FReferenceChainSearch::GetObjectFlags(FGCObjectInfo(Object)),
					*Object->GetFullName(),
...
void VerifyNoUnreachableObjects(int32 NumUnreachable)
{
		...
		for (int32 ObjectIndex = 0; ObjectIndex < NumObjects && (FirstObjectIndex + ObjectIndex) < GUObjectArray.GetObjectArrayNum(); ++ObjectIndex)
		{
			...
			if (ObjectItem.GetObject())
			{
				...
				if (!ObjectItem.HasAnyFlags(EInternalObjectFlags::Unreachable))
				{
					...
					{
						UE_LOGF(LogGarbage, Warning, "Object [%d] %ls%ls is still marked as MaybeUnreachable after Reachability Analysis is complete.",
					...
					if (!FGCFlags::IsReachable_ForGC(&ObjectItem) && !GUObjectArray.IsDisregardForGC(Object))
					{
						UE_LOGF(LogGarbage, Warning, "Object [%d] %ls%ls is NOT marked as Unreachable and NOT marked as Reachable.",
							ObjectIndexInGlobalArray,
							*FReferenceChainSearch::GetObjectFlags(FGCObjectInfo(Object)),
							*Object->GetFullName());
						NumErrors++;
				...
				else
				{
					...
					{
						UE_LOGF(LogGarbage, Warning, "Object [%d] %ls%ls is still marked as Reachable.",
							ObjectIndexInGlobalArray,
							*FReferenceChainSearch::GetObjectFlags(FGCObjectInfo(Object)),
							*Object->GetFullName());
						NumErrors++;
	...
	{
		NumErrors++;
		UE_LOGF(LogGarbage, Warning, "The actual number of objects with the Unreachable flag (%d) does not match the number of gathered unreachable objects (%d).", VerifiedNumUnreachable.load(), NumUnreachable);
	}

	UE_CLOGF(NumErrors > 0, LogGarbage, Fatal, "Detected %d case(s) of breaking reachability assumptions.", NumErrors.load());

	UE_LOGF(LogGarbage, Log, "%f ms for VerifyNoUnreachableObjects", (FPlatformTime::Seconds() - StartTime) * 1000);
}
...
void FScopedCBDProfile::DumpProfile()
{
	...
	{
		UE_LOGF(LogGarbage, Log, "    %6d cnt %6.2fus per   %6.2fms total  %ls", Item.Value.Items, 1000.0f * 1000.0f * Item.Value.TotalTime / float(Item.Value.Items), 1000.0f * Item.Value.TotalTime, *Item.Key.ToString());
