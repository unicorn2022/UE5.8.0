...
namespace UE::CoreRedirects::Private
{
	...
	bool RunAssetRedirectTests()
	{
		...
		{
			bSuccess = false;
			UE_LOGF(LogCoreRedirects, Error, "Failed asset redirect validation.");
		}
		...
		for (auto& Test : Tests)
		{
			...
			{
				bSuccess = false;
				UE_LOGF(LogCoreRedirects, Error, "Failed %ls. Source = %ls was unexpectedly redirected to %ls",
			...
			if (Test.ExpectTrue)
			{
				...
				{
					bSuccess = false;
					UE_LOGF(LogCoreRedirects, Error, "Failed to FindPreviousNames for %ls", Test.Destination);
				}
				else
				{
					...
					{
						bSuccess = false;
						UE_LOGF(LogCoreRedirects, Error, "Failed to find expected previous name for %ls", Test.Destination);
					}
				}
			}
		}
		...
		for (auto& Test : Tests)
		{
			...
			{
				bSuccess = false;
				UE_LOGF(LogCoreRedirects, Error, "Found unexpected redirect from %ls to %ls",
		...
		// Ensure validation failure if chains exist
		{
			...
			{
				bSuccess = false;
				UE_LOGF(LogCoreRedirects, Error, "Failed to detect erroneous chained redirect in ValidateAssetRedirects()");
			}
			FCoreRedirects::RemoveAllAssetRedirects();
		}
...
bool FCoreRedirectObjectName::HasValidCharacters(ECoreRedirectFlags Type) const
{
	...
	if (FPackageName::IsVersePackage(WriteToString<NAME_SIZE>(PackageName)))
	{
		...
		static FString InvalidVerseCharacters = "\n\r\t";
	...
	static FString InvalidRedirectNameCharacters = "\"',|&!~\n\r\t@#(){}[]=;^%$`";
...
bool FCoreRedirectObjectName::ExpandNames(const FStringView InStringView, FName& OutName, FName& OutOuter, FName &OutPackage)
{
	...
	{
		// We cannot make FNames from strings of this size
		if (FullStringView.Len() >= NAME_SIZE)
		{
			return false;
		}
	...
	else
	{
		...
		{
			const int32 StartPos = FirstPeriodIndex + 1;
			const int32 Count = LastPeriodIndex - FirstPeriodIndex - 1;
			// We cannot make FNames from strings of this size
			if (Count >= NAME_SIZE)
			{
				return false;
			...
			OutOuter = FName(FullStringView.Mid(StartPos, Count));
		}
	}

	const int32 StartPos = LastPeriodIndex + 1;
	// We cannot make FNames from strings of this size
	if ((FullStringView.Len() - StartPos) >= NAME_SIZE)
	{
		return false;
	}
	OutName = FName(FullStringView.Mid(StartPos));
...
int32 FCoreRedirect::GetTypeHash() const
{
	uint32 Hashes[] = {
		GetTypeHashHelper(RedirectFlags),
		GetTypeHashHelper(OldName),
		GetTypeHashHelper(NewName),
		GetTypeHashHelper(OverrideClassName),
	};

	uint32 SmooshedHash = 0;

	for (const uint32 Hash : Hashes)
	{
		SmooshedHash = HashCombineFast(SmooshedHash, Hash);
	}
	...
	for (const TPair<FString, FString>& Pair : ValueArray)
	{
		SmooshedHash = HashCombineFast(SmooshedHash, GetTypeHashHelper(Pair.Key));
		SmooshedHash = HashCombineFast(SmooshedHash, GetTypeHashHelper(Pair.Value));
	}
...
{
	// Iteratively apply redirects until a fixed point or a non-iterative redirect is reached.
	FCoreRedirectObjectName ObjectName = OldObjectName;
	for (int NumIterations = 0;;++NumIterations)
	{
		bool bShouldIterate = false;
		auto ProcessRedirect = [&FoundValueRedirect, OldObjectName, &bShouldIterate](const FCoreRedirect* Redirect, const FCoreRedirectObjectName& NewObjectName)
		{
			if (FoundValueRedirect && (Redirect->HasValueChanges() || Redirect->OverrideClassName.IsValid()))
			{
				if (*FoundValueRedirect)
				{
					...
					{
						UE_LOGF(LogCoreRedirects, Error, "RedirectNameAndValues(%ls) found multiple conflicting value redirects, %ls and %ls!",
			...
			if (!!(Redirect->RedirectFlags & ECoreRedirectFlags::Option_Iterate))
			{
				bShouldIterate = true;
			}
		...
		if (GetMatchingRedirectsUnderReadLock(Type, ObjectName, FoundRedirects, MatchFlags, LockedContext))
		{
			if (FoundRedirects.Num() > 1)
			{
				// Sort them based on match
				FoundRedirects.Sort([&ObjectName, MatchFlags](const FCoreRedirect& A, const FCoreRedirect& B)
					{ return A.OldName.MatchScore(ObjectName, A.RedirectFlags, MatchFlags) > B.OldName.MatchScore(ObjectName, B.RedirectFlags, MatchFlags); });
		...
		{
			UE_CLOGF(LockedContext.Get().IsInDebugMode(), LogCoreRedirects, Verbose,
				"RedirectNameAndValues(%ls) replaced by %ls", *ObjectName.ToString(), *NewObjectName.ToString());
		...
		// Limit the maximum number of iterations to ensure we don't get stuck in an infinite loop here if somebody accidentally created a redirect cycle.
		constexpr int MaxIterations = 10;
		if (!ensureMsgf(NumIterations < MaxIterations, TEXT("RedirectNameAndValues stopping after maximum number of iterations: %s -> %s"), *OldObjectName.ToString(), *NewObjectName.ToString()))
		{
			bShouldIterate = false;
		}
		...
		for (const TPair<FName, TArray<FCoreRedirect>>& Pair : RedirectNameMap->RedirectMap)
		{
			...
			for (const FCoreRedirect& Redirect : Pair.Value)
			{
				...
				if (!(Redirect.RedirectFlags & ECoreRedirectFlags::Option_Iterate))
				{
					...
					{
						UE_LOGF(LogCoreRedirects, Warning, "Found redirect from existing redirect. Chained redirects will not be followed. %ls --> %ls --> %ls",
	...
	const FCoreRedirectObjectName& OldObjectName, ECoreRedirectMatchFlags MatchFlags)
{
	...
	{
		UE_CLOGF(LockedContext.Get().IsInDebugMode(), LogCoreRedirects, VeryVerbose, "GetValueRedirects found %d matches for %ls",
...
bool FCoreRedirects::FindPreviousNames(ECoreRedirectFlags SearchFlags, const FCoreRedirectObjectName& NewObjectName, TArray<FCoreRedirectObjectName>& PreviousNames)
{
	...
	UE_CLOGF(bFound && LockedContext.Get().IsInDebugMode(), LogCoreRedirects, VeryVerbose, "FindPreviousNames found %d previous names for %ls", PreviousNames.Num(), *NewObjectName.ToString());
...
void FCoreRedirects::AppendHashOfGlobalRedirects(FBlake3& Hasher)
{
	...
	if (!!(Context.GetFlags() & FCoreRedirectsContext::EFlags::UseRedirectionSummary))
	{
		FRedirectionSummary& RedirectionSummary = Context.GetRedirectionSummary();
		RedirectionSummary.AppendHashGlobal(Hasher);

		static bool bDebugPrintGlobalCoreRedirects = FParse::Param(FCommandLine::Get(), TEXT("DebugPrintGlobalCoreRedirects"));
		if (bDebugPrintGlobalCoreRedirects)
		{
			RedirectionSummary.DebugPrintGlobalCoreRedirects();
		}
	}
}

void FCoreRedirects::GetObjectRedirectHashAffectingPackage(const FName& PackageName, FBlake3& Hasher)
{
	FCoreRedirectsContext& Context = FCoreRedirectsContext::GetThreadContext();
	if (!!(Context.GetFlags() & FCoreRedirectsContext::EFlags::UseRedirectionSummary))
	{
		FRedirectionSummary& RedirectionSummary = Context.GetRedirectionSummary();
		RedirectionSummary.GetObjectRedirectHashAffectingPackage(PackageName, Hasher);
	}
}
...
void FCoreRedirects::RecordAddedObjectRedirector(const FSoftObjectPath& Source, const FSoftObjectPath& Dest)
{
	...
	if (!!(Context.GetFlags() & FCoreRedirectsContext::EFlags::UseRedirectionSummary))
	{
		...
		Context.GetRedirectionSummary().AddObjectRedirector(ConvertedToCoreRedirect);
	}
}
...
void FCoreRedirects::RecordRemovedObjectRedirector(const FSoftObjectPath& Source, const FSoftObjectPath& Dest)
{
	...
	if (!!(Context.GetFlags() & FCoreRedirectsContext::EFlags::UseRedirectionSummary))
	{
		FCoreRedirect ConvertedToCoreRedirect(ECoreRedirectFlags::Type_Object,
			FCoreRedirectObjectName(Source), FCoreRedirectObjectName(Dest));
		Context.GetRedirectionSummary().RemoveObjectRedirector(ConvertedToCoreRedirect);
	}
}

FCoreRedirects::FGlobalCoreRedirectAddedEvent* FCoreRedirects::GetOnGlobalCoreRedirectAdded()
{
	FCoreRedirectsContext& Context = FCoreRedirectsContext::GetThreadContext();
	if (!!(Context.GetFlags() & FCoreRedirectsContext::EFlags::UseRedirectionSummary))
	{
		return &Context.GetRedirectionSummary().GetOnGlobalCoreRedirectAdded();
	}
	return nullptr;
}
...
bool FCoreRedirects::RunTests()
{
	...
	UE_LOGF(LogCoreRedirects, Log, "Running FCoreRedirect Tests");
	...
	NewRedirects.Emplace(ECoreRedirectFlags::Type_Package | ECoreRedirectFlags::Option_MatchPrefix | ECoreRedirectFlags::Option_Iterate, TEXT("/oldoldprefix"), TEXT("/oldprefix"));
	NewRedirects.Emplace(ECoreRedirectFlags::Type_Package | ECoreRedirectFlags::Option_MatchPrefix | ECoreRedirectFlags::Option_Iterate, TEXT("/oldoldoldprefix"), TEXT("/oldoldprefix"));
	NewRedirects.Emplace(ECoreRedirectFlags::Type_Package | ECoreRedirectFlags::Option_MatchPrefix, TEXT("/noiterate_oldprefix"), TEXT("/oldprefix"));

	// Test ordering of package redirects and other redirects to ensure the package redirect is applied last so we can still redirect non-package names that might be using the old package name
	NewRedirects.Emplace(ECoreRedirectFlags::Type_Package,	TEXT("/Script/PackageRedirectOrderingTest_PackageRedirectFirst"),			TEXT("/Script/PackageRedirectOrderingTest_PackageRedirectFirst_New"));
	NewRedirects.Emplace(ECoreRedirectFlags::Type_Struct,	TEXT("/Script/PackageRedirectOrderingTest_PackageRedirectFirst.OldStruct1"),	TEXT("/Script/PackageRedirectOrderingTest_PackageRedirectFirst_New.NewStruct1"));
	NewRedirects.Emplace(ECoreRedirectFlags::Type_Struct, TEXT("/Script/PackageRedirectOrderingTest_PackageRedirectLast.OldStruct1"), TEXT("/Script/PackageRedirectOrderingTest_PackageRedirectLast_New.NewStruct1"));
	NewRedirects.Emplace(ECoreRedirectFlags::Type_Package, TEXT("/Script/PackageRedirectOrderingTest_PackageRedirectLast"), TEXT("/Script/PackageRedirectOrderingTest_PackageRedirectLast_New"));

	FCoreRedirect EnumRedirectWithValues(ECoreRedirectFlags::Type_Enum, TEXT("MaskTarget_PhysMesh"), TEXT("/Script/ClothingSystemRuntimeCommon.EWeightMapTargetCommon"));
	EnumRedirectWithValues.ValueChanges.Add(TEXT("MaskTarget_PhysMesh::MaxDistance"), TEXT("EWeightMapTargetCommon::MaxDistance"));
	EnumRedirectWithValues.ValueChanges.Add(TEXT("MaskTarget_PhysMesh::BackstopDistance"), TEXT("EWeightMapTargetCommon::BackstopDistance"));
	EnumRedirectWithValues.ValueChanges.Add(TEXT("MaskTarget_PhysMesh::BackstopRadius"), TEXT("EWeightMapTargetCommon::BackstopRadius"));
	EnumRedirectWithValues.ValueChanges.Add(TEXT("MaskTarget_PhysMesh::AnimDriveMultiplier"), TEXT("EWeightMapTargetCommon::AnimDriveStiffness"));
	NewRedirects.Add(EnumRedirectWithValues);

	// This value redirect extends the values from the EnumRedirectWithValues redirect above
	FCoreRedirect EnumValueRedirect(ECoreRedirectFlags::Type_Enum, TEXT("/Script/ClothingSystemRuntimeCommon.EWeightMapTargetCommon"), TEXT(""));
	EnumValueRedirect.ValueChanges.Add(TEXT("EWeightMapTargetCommon::AnimDriveMultiplier"), TEXT("EWeightMapTargetCommon::AnimDriveStiffness"));
	NewRedirects.Add(EnumValueRedirect);
	...
	// Iteration tests
	Tests.Emplace(TEXT("/oldoldoldprefix_SomeGame/Package.DefaultClass"), TEXT("/newprefix_SomeGame/Package.DefaultClass"), ECoreRedirectFlags::Type_Package);
	Tests.Emplace(TEXT("/noiterate_oldprefix_SomeGame/Package.DefaultClass"), TEXT("/oldprefix_SomeGame/Package.DefaultClass"), ECoreRedirectFlags::Type_Package);
	...
	Tests.Emplace(TEXT("/Script/PackageRedirectOrderingTest_PackageRedirectFirst"), TEXT("/Script/PackageRedirectOrderingTest_PackageRedirectFirst_New"), ECoreRedirectFlags::Type_Package);
	Tests.Emplace(TEXT("/Script/PackageRedirectOrderingTest_PackageRedirectFirst.OldStruct1"), TEXT("/Script/PackageRedirectOrderingTest_PackageRedirectFirst_New.NewStruct1"), ECoreRedirectFlags::Type_Struct);
	Tests.Emplace(TEXT("/Script/PackageRedirectOrderingTest_PackageRedirectLast.OldStruct1"), TEXT("/Script/PackageRedirectOrderingTest_PackageRedirectLast_New.NewStruct1"), ECoreRedirectFlags::Type_Struct);
	Tests.Emplace(TEXT("/Script/PackageRedirectOrderingTest_PackageRedirectLast"), TEXT("/Script/PackageRedirectOrderingTest_PackageRedirectLast_New"), ECoreRedirectFlags::Type_Package);

	// Enum value tests
	FRedirectTest EnumValueTest(TEXT("/Script/ClothingSystemRuntimeCommon.MaskTarget_PhysMesh"), TEXT("/Script/ClothingSystemRuntimeCommon.EWeightMapTargetCommon"), ECoreRedirectFlags::Type_Enum);
	EnumValueTest.Expected.ValueChanges.Add(TEXT("MaskTarget_PhysMesh::MaxDistance"), TEXT("EWeightMapTargetCommon::MaxDistance"));
	EnumValueTest.Expected.ValueChanges.Add(TEXT("MaskTarget_PhysMesh::BackstopDistance"), TEXT("EWeightMapTargetCommon::BackstopDistance"));
	EnumValueTest.Expected.ValueChanges.Add(TEXT("MaskTarget_PhysMesh::BackstopRadius"), TEXT("EWeightMapTargetCommon::BackstopRadius"));
	EnumValueTest.Expected.ValueChanges.Add(TEXT("MaskTarget_PhysMesh::AnimDriveMultiplier"), TEXT("EWeightMapTargetCommon::AnimDriveStiffness"));
	Tests.Add(EnumValueTest);

	// Confirms we inherit all the mappings from the MaskTarget_PhysMesh -> EWeightMapTargetCommon mapping
	// plus the explicit addition to EWeightMapTargetCommon added via a separate value redirect
	FRedirectTest EnumInheritedAndAppendedValueTest(TEXT("/Script/ClothingSystemRuntimeCommon.EWeightMapTargetCommon"), TEXT("/Script/ClothingSystemRuntimeCommon.EWeightMapTargetCommon"), ECoreRedirectFlags::Type_Enum);
	EnumInheritedAndAppendedValueTest.Expected.ValueChanges.Add(TEXT("MaskTarget_PhysMesh::MaxDistance"), TEXT("EWeightMapTargetCommon::MaxDistance"));
	EnumInheritedAndAppendedValueTest.Expected.ValueChanges.Add(TEXT("MaskTarget_PhysMesh::BackstopDistance"), TEXT("EWeightMapTargetCommon::BackstopDistance"));
	EnumInheritedAndAppendedValueTest.Expected.ValueChanges.Add(TEXT("MaskTarget_PhysMesh::BackstopRadius"), TEXT("EWeightMapTargetCommon::BackstopRadius"));
	EnumInheritedAndAppendedValueTest.Expected.ValueChanges.Add(TEXT("MaskTarget_PhysMesh::AnimDriveMultiplier"), TEXT("EWeightMapTargetCommon::AnimDriveStiffness"));
	EnumInheritedAndAppendedValueTest.Expected.ValueChanges.Add(TEXT("EWeightMapTargetCommon::AnimDriveMultiplier"), TEXT("EWeightMapTargetCommon::AnimDriveStiffness"));
	Tests.Add(EnumInheritedAndAppendedValueTest);

	for (FRedirectTest& Test : Tests)
	{
		FCoreRedirectObjectName ActualNewName;
		const FCoreRedirect* ValueRedirects = nullptr;
		RedirectNameAndValues(Test.Expected.RedirectFlags, Test.Expected.OldName, ActualNewName, &ValueRedirects);
		bool bTestSuccess = ActualNewName == Test.Expected.NewName;
		if (bTestSuccess && !Test.Expected.ValueChanges.IsEmpty())
		{
			bTestSuccess &= ValueRedirects != nullptr;
			if (ValueRedirects)
			{
				for (const TPair<FString, FString>& Pair : Test.Expected.ValueChanges)
				{
					const FString* ActualChangeValue = ValueRedirects->ValueChanges.Find(Pair.Key);
					bTestSuccess &= ActualChangeValue != nullptr;
					if (ActualChangeValue)
					{
						bTestSuccess &= Pair.Value == *ActualChangeValue;
					}
					if (!bTestSuccess)
					{
						break;
		...
		{
			bSuccess = false;
			TStringBuilder<256> ExpectedBuilder;
			TStringBuilder<256> ActualBuilder;
			auto PrintRedirect = [](FCoreRedirect& Redirect, TStringBuilderBase<TCHAR>& Builder)
				{
					Builder.Appendf(TEXT("%s -> %s"), *Redirect.OldName.ToString(), *Redirect.NewName.ToString());
					for (TPair<FString, FString>& Pair : Redirect.ValueChanges)
					{
						Builder.Appendf(TEXT("\n\t[%s -> %s]"), *Pair.Key, *Pair.Value);
					}
				};
			PrintRedirect(Test.Expected, ExpectedBuilder);
			FCoreRedirect ActualRedirect = ValueRedirects ? *ValueRedirects : FCoreRedirect(Test.Expected.RedirectFlags, Test.Expected.OldName, ActualNewName);
			PrintRedirect(ActualRedirect, ActualBuilder);
			UE_LOGF(LogCoreRedirects, Error, "FCoreRedirect Test Failed:\nExpected:\t%ls,\nActual:\t\t%ls", ExpectedBuilder.ToString(), ActualBuilder.ToString());
		}
	}
	...
	{
		bSuccess = false;
		UE_LOGF(LogCoreRedirects, Error, "FCoreRedirect Test Failed: ReverseLookup (direct matching)!");
	}
	OldNames.Empty();
	...
	{
		bSuccess = false;
		UE_LOGF(LogCoreRedirects, Error, "FCoreRedirect Test Failed: ReverseLookup (substring matching)!");
	}
	OldNames.Empty();
	...
	{
		bSuccess = false;
		UE_LOGF(LogCoreRedirects, Error, "FCoreRedirect Test Failed: ReverseLookup (prefix matching)!");
	}
	OldNames.Empty();
	...
	{
		bSuccess = false;
		UE_LOGF(LogCoreRedirects, Error, "FCoreRedirect Test Failed: ReverseLookup (suffix matching)!");
	}
	OldNames.Empty();
	...
	{
		bSuccess = false;
		UE_LOGF(LogCoreRedirects, Error, "FCoreRedirect Test Failed: HasValidCharacters should report Object redirects with \\n in the ObjectName as invalid");
	}
	...
	{
		bSuccess = false;
		UE_LOGF(LogCoreRedirects, Error, "FCoreRedirect Test Failed: HasValidCharacters should report verse Object redirects with \\n in the ObjectName as invalid");
	}
	...
	{
		bSuccess = false;
		UE_LOGF(LogCoreRedirects, Error, "FCoreRedirect Test Failed: HasValidCharacters should report Object redirects with $ in the ObjectName as valid");
	}
	if (FCoreRedirectObjectName(TEXT("/Foo/Foo.$Foo:New")).HasValidCharacters(ECoreRedirectFlags::Type_Object))
	{
		bSuccess = false;
		UE_LOGF(LogCoreRedirects, Error, "FCoreRedirect Test Failed: HasValidCharacters should report Object redirects with $ in the OuterName as invalid");
	}
	if (FCoreRedirectObjectName(TEXT("/Foo/$Foo.Foo:New")).HasValidCharacters(ECoreRedirectFlags::Type_Object))
	{
		bSuccess = false;
		UE_LOGF(LogCoreRedirects, Error, "FCoreRedirect Test Failed: HasValidCharacters should report Object redirects with $ in the PackageName as invalid");
	}
	...
	{
		bSuccess = false;
		UE_LOGF(LogCoreRedirects, Error, "FCoreRedirect Test Failed: HasValidCharacters should report verse Object redirects with $ in the ObjectName as valid");
	}
	if (!FCoreRedirectObjectName(TEXT("/Foo/_Verse/Foo.$Foo:New")).HasValidCharacters(ECoreRedirectFlags::Type_Object))
	{
		bSuccess = false;
		UE_LOGF(LogCoreRedirects, Error, "FCoreRedirect Test Failed: HasValidCharacters should report verse Object redirects with $ in the OuterName as valid");
	}
	if (!FCoreRedirectObjectName(TEXT("/Foo/_Verse/$Foo.Foo:New")).HasValidCharacters(ECoreRedirectFlags::Type_Object))
	{
		bSuccess = false;
		UE_LOGF(LogCoreRedirects, Error, "FCoreRedirect Test Failed: HasValidCharacters should report verse Object redirects with $ in the PackageName as valid");
	}
	...
	{
		bSuccess = false;
		UE_LOGF(LogCoreRedirects, Error, "FCoreRedirect Test Failed: /Game/RemovedPackage should be removed!");
	}

	if (!IsKnownMissing(ECoreRedirectFlags::Type_Package, FCoreRedirectObjectName(TEXT("/Game/MissingLoadPackage"))))
	{
		bSuccess = false;
		UE_LOGF(LogCoreRedirects, Error, "FCoreRedirect Test Failed: /Game/MissingLoadPackage should be removed!");
	}

	if (IsKnownMissing(ECoreRedirectFlags::Type_Package, FCoreRedirectObjectName(TEXT("/Game/NotRemovedPackage"))))
	{
		bSuccess = false;
		UE_LOGF(LogCoreRedirects, Error, "FCoreRedirect Test Failed: /Game/NotRemovedPackage should be removed!");
	}
	...
	{
		bSuccess = false;
		UE_LOGF(LogCoreRedirects, Error, "FCoreRedirect Test Failed: /Game/NotRemovedMissingLoad should be removed now!");
	}
	...
	{
		bSuccess = false;
		UE_LOGF(LogCoreRedirects, Error, "FCoreRedirect Test Failed: RemoveKnownMissing of /Game/NotRemovedMissingLoad but with bIsMissingLoad=false should not have removed the redirect!");
	}
	...
	{
		bSuccess = false;
		UE_LOGF(LogCoreRedirects, Error, "FCoreRedirect Test Failed: /Game/NotRemovedMissingLoad should no longer be removed!");
	}
	...
	{
		bSuccess = false;
		UE_LOGF(LogCoreRedirects, Error, "FCoreRedirect Test Failed: /Game/NotRemovedPackage should be removed now!");
	}
	...
	{
		bSuccess = false;
		UE_LOGF(LogCoreRedirects, Error, "FCoreRedirect Test Failed: RemoveKnownMissing of /Game/NotRemovedPackage but with bIsMissingLoad=true should not have removed the redirect!");
	}
	...
	{
		bSuccess = false;
		UE_LOGF(LogCoreRedirects, Error, "FCoreRedirect Test Failed: /Game/NotRemovedPackage should no longer be removed!");
	}

	UE_LOGF(LogCoreRedirects, Log, "FCoreRedirect Test %ls!", (bSuccess ? TEXT("Passed") : TEXT("Failed")));
	return bSuccess;
}
...
bool FCoreRedirectObjectNameTest::RunTest(const FString& Parameters)
{
	...
	FName NamePackage("/Root/Package");
	FName NameA("A");
	FName NameB("B");
	FName NameC("C");
	FName NameD("D");
	FName NameADotB("A.B");
	FName NameADotBDotC("A.B.C");
...
void FCoreRedirects::AddAssetRedirects(const TMap<FSoftObjectPath, FSoftObjectPath>& InRedirects)
{
	...
	for (const TPair<FSoftObjectPath, FSoftObjectPath>& Pair : InRedirects)
	{
		...
		{
			UE_LOGF(LogCoreRedirects, Warning, "Attempted to register asset redirector that was missing a package or object name. Redirector was from %ls to %ls",
	...
	UE_LOGF(LogCoreRedirects, Display, 
		"Object redirects provided to FCoreRedirects: %d. Redirects add: %d. Redirects skipped: %d",
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE_STR("FCoreRedirects::ReadRedirectsFromIni");
	FScopeCoreRedirectsWriteLockedContext LockedContext(FCoreRedirectsContext::GetThreadContext());
	ensureMsgf(IsInitialized(), TEXT("FCoreRedirects must be initialized on the game thread before use."));
	...
	if (GConfig)
	{
		...
		if (RedirectSection)
		{
			...
			for (FConfigSection::TConstIterator It(*RedirectSection); It; ++It)
			{
				...
				FParse::Bool(*ValueString, TEXT("SuppressIterateWarning="), bSuppressIterateWarning);
				...
				if (FlagPtr)
				{
					...
					if (bIterate)
					{
						NewFlags |= ECoreRedirectFlags::Option_Iterate;

						if (!bSuppressIterateWarning)
						{
							UE_LOGF(LogCoreRedirects, Warning,
								"ReadRedirectsFromIni(%ls) Redirect %ls is iterative. Iterative redirects are expensive, and should be used with caution.",
								*IniName, *ValueString);
						}
					}
					...
					{
						UE_CLOGF(bMatchSubstring, LogCoreRedirects, Warning, "ReadRedirectsFromIni(%ls) 'MatchSubstring=' is deprecated. "
							"Please prefer `MatchWildcard=' instead for redirect %ls.\n\t"
						...
						{
							UE_LOGF(LogCoreRedirects, Error, "ReadRedirectsFromIni(%ls) failed to parse OldName for wildcard redirect %ls! "
					...
					if (ValueChangesIndex != INDEX_NONE)
					{
						...
						if (!Buffer)
						{
							UE_LOGF(LogCoreRedirects, Error, "ReadRedirectsFromIni(%ls) failed to parse ValueChanges for redirect %ls!", *IniName, *ValueString);
				...
				{
					UE_LOGF(LogCoreRedirects, Error, "ReadRedirectsFromIni(%ls) failed to parse type for redirect %ls!", *IniName, *ValueString);
				}
			}
		...
		{
			UE_LOGF(LogCoreRedirects, Verbose, "ReadRedirectsFromIni(%ls) did not find any redirects", *IniName);
		}
	}
	else
	{
		UE_LOGF(LogCoreRedirects, Warning, " **** CORE REDIRECTS UNABLE TO INITIALIZE! **** ");
	}
	return false;
}
...
{
	UE_LOGF(LogCoreRedirects, Verbose, "AddRedirect(%ls) adding %d redirects", *SourceString, Redirects.Num());
	...
	for (const FCoreRedirect& NewRedirect : Redirects)
	{
		...
		{
			UE_LOGF(LogCoreRedirects, Error, "AddRedirect(%ls) failed to add redirect from %ls to %ls with empty name!", *SourceString, *NewRedirect.OldName.ToString(), *NewRedirect.NewName.ToString());
			continue;
		}
			...
			&& !EnumHasAnyFlags(NewRedirect.RedirectFlags, ECoreRedirectFlags::Type_Asset))
		{
			...
			{
				UE_LOGF(LogCoreRedirects, Error, "AddRedirect(%ls) failed to add redirect from %ls to %ls with invalid characters!", *SourceString, *NewRedirect.OldName.ToString(), *NewRedirect.NewName.ToString());
				continue;
			}
		}
		...
		{
			UE_LOGF(LogCoreRedirects, Verbose, "AddRedirect(%ls) has wildcard redirect %ls, these are very slow and should be resolved as soon as possible! Please refer to the documentation in Engine/Config/BaseEngine.ini.", *SourceString, *NewRedirect.OldName.ToString());
		}
	...
	FScopeCoreRedirectsWriteLockedContext& LockedContext)
{
	...
	static bool bDebugNullCook = FParse::Value(FCommandLine::Get(), TEXT("-DebugNullCook="), ClassName);
	bool bPrintLogsDebugNullCook = bDebugNullCook && NewRedirect.OldName.ObjectName.ToString() == ClassName;

	if (bPrintLogsDebugNullCook)
	{
		UE_LOGF(LogCoreRedirects, Display, "[DebugNullCook] Adding redirector %ls->%ls from %ls", *NewRedirect.OldName.ToString(),
			*NewRedirect.NewName.ToString(), *SourceString);
	}
#endif // #if WITH_EDITOR
		...
		if (ExistingRedirect.IdenticalMatchRules(NewRedirect))
		{
			...
			if (bIsSameNewName)
			{
				...
				if (bBothHaveValueChanges)
				{
					...
					UE_LOGF(LogCoreRedirects, Verbose, "AddRedirect(%ls) merging value redirects for %ls", *SourceString, *ExistingRedirect.NewName.ToString());
				}
				else if (bOneIsPartialOtherIsFull)
				{
					UE_LOGF(LogCoreRedirects, Warning, "AddRedirect(%ls) found duplicate redirects for %ls to %ls, one a FullPath and the other ObjectName-only." " This used to be required for StructRedirects but now you should remove the ObjectName-only redirect and keep the FullPath.",
						*SourceString, *ExistingRedirect.OldName.ToString(), *ExistingRedirect.NewName.ToString());
				}
				else
				{
					UE_LOGF(LogCoreRedirects, Verbose, "AddRedirect(%ls) ignoring duplicate redirects for %ls to %ls", *SourceString, *ExistingRedirect.OldName.ToString(), *ExistingRedirect.NewName.ToString());
				}
			}
			else
			{
				UE_LOGF(LogCoreRedirects, Error, "AddRedirect(%ls) found conflicting redirects for %ls! Old: %ls, New: %ls", *SourceString, *ExistingRedirect.OldName.ToString(), *ExistingRedirect.NewName.ToString(), *NewRedirect.NewName.ToString());
			}
			break;
		}
	}
...
{
	FScopeCoreRedirectsWriteLockedContext LockedContext(FCoreRedirectsContext::GetThreadContext());
	UE_LOGF(LogCoreRedirects, Verbose, "RemoveRedirect(%ls) Removing %d redirects", *SourceString, Redirects.Num());
	...
	for (const FCoreRedirect& RedirectToRemove : Redirects)
	{
		...
		{
			UE_LOGF(LogCoreRedirects, Error, "RemoveRedirect(%ls) failed to remove redirect from %ls to %ls with empty name!", *SourceString, *RedirectToRemove.OldName.ToString(), *RedirectToRemove.NewName.ToString());
			continue;
		}

		if (RedirectToRemove.HasValueChanges())
		{
			UE_LOGF(LogCoreRedirects, Error, "RemoveRedirect(%ls) failed to remove redirect from %ls to %ls as it contains value changes!", *SourceString, *RedirectToRemove.OldName.ToString(), *RedirectToRemove.NewName.ToString());
			continue;
		}
		...
		{
			UE_LOGF(LogCoreRedirects, Error, "RemoveRedirect(%ls) failed to remove redirect from %ls to %ls with invalid characters!", *SourceString, *RedirectToRemove.OldName.ToString(), *RedirectToRemove.NewName.ToString());
			continue;
		}
		...
		{
			UE_LOGF(LogCoreRedirects, Error, "RemoveRedirect(%ls) failed to remove redirect, it's not valid to modify package from %ls to %ls while specifying outer!", *SourceString, *RedirectToRemove.OldName.ToString(), *RedirectToRemove.NewName.ToString());
			continue;
		}

		if (RedirectToRemove.IsWildcardMatch())
		{
			UE_LOGF(LogCoreRedirects, Verbose, "RemoveRedirect(%ls) has wildcard redirect %ls, these are very slow and should be resolved as soon as possible! Please refer to the documentation in Engine/Config/BaseEngine.ini.", *SourceString, *RedirectToRemove.OldName.ToString());
		}
	...
	for (const FCoreRedirect& Redirect : Redirects)
	{
		if (Redirect.NewName.IsValid())
		{
			...
			if (CheckRedirectFlagsMatch(Redirect.RedirectFlags, ECoreRedirectFlags::Type_Class))
			{
				...
				{
					UE_LOGF(LogCoreRedirects, Warning, "ValidateRedirect(%ls) has missing package for Class redirect from %ls to %ls!", *SourceString, *OldPath, *NewPath);
				}
				else
				{
					...
					if (!FindClass)
					{
						if (NewPackage)
						{
							UE_LOGF(LogCoreRedirects, Error, "ValidateRedirect(%ls) failed to find destination Class for redirect from %ls to %ls with loaded package!", *SourceString, *OldPath, *NewPath);
						}
						else
						{
							UE_LOGF(LogCoreRedirects, Log, "ValidateRedirect(%ls) can't validate destination Class for redirect from %ls to %ls with unloaded package", *SourceString, *OldPath, *NewPath);
						}
					}
				}
			}
			...
			if (CheckRedirectFlagsMatch(Redirect.RedirectFlags, ECoreRedirectFlags::Type_Struct))
			{
				if (Redirect.NewName.PackageName.IsNone())
				{
					UE_LOGF(LogCoreRedirects, Warning, "ValidateRedirect(%ls) has missing package for Struct redirect from %ls to %ls!", *SourceString, *OldPath, *NewPath);
				}
				else
				{
					...
					if (!FindStruct)
					{
						if (NewPackage)
						{
							UE_LOGF(LogCoreRedirects, Error, "ValidateRedirect(%ls) failed to find destination Struct for redirect from %ls to %ls with loaded package!", *SourceString, *OldPath, *NewPath);
						}
						else
						{
							UE_LOGF(LogCoreRedirects, Log, "ValidateRedirect(%ls) can't validate destination Struct for redirect from %ls to %ls with unloaded package", *SourceString, *OldPath, *NewPath);
						}
					}
				}
			}
			...
			if (CheckRedirectFlagsMatch(Redirect.RedirectFlags, ECoreRedirectFlags::Type_Enum))
			{
				if (Redirect.NewName.PackageName.IsNone())
				{
					...
					{
						UE_LOGF(LogCoreRedirects, Warning, "ValidateRedirect(%ls) has missing package for Enum redirect from %ls to %ls!", *SourceString, *OldPath, *NewPath);
					}
				}
				else
				{
					...
					if (!FindEnum)
					{
						if (NewPackage)
						{
							UE_LOGF(LogCoreRedirects, Error, "ValidateRedirect(%ls) failed to find destination Enum for redirect from %ls to %ls with loaded package!", *SourceString, *OldPath, *NewPath);
						}
						else
						{
							UE_LOGF(LogCoreRedirects, Log, "ValidateRedirect(%ls) can't validate destination Enum for redirect from %ls to %ls with unloaded package", *SourceString, *OldPath, *NewPath);
						}
					}
				}
			}
