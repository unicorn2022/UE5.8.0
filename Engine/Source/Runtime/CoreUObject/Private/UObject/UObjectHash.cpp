...
alignas(PLATFORM_CACHE_LINE_SIZE) FUObjectArray GUObjectArray;

static EInternalObjectFlags GetMandatoryInternalExclusionFlags()
{
	// Common EInternalObjectFlags used in this file to filter out objects.
	static constexpr EInternalObjectFlags MandatoryInternalExclusionFlags =
		EInternalObjectFlags::Unreachable | EInternalObjectFlags::AutoRTFMConstructionAborted;

	return MandatoryInternalExclusionFlags | UE::GetAsyncLoadingInternalFlagsExclusion();
}

static FORCENOINLINE void OnHashFailure(UObjectBaseUtility* Object, const TCHAR* HashName, const TCHAR* FailureKind)
{
	UE_LOGF(LogUObjectHash, Error, "UObject %ls consistency failure (%ls). Checking for memory corruption", HashName, FailureKind);
	UE_CLOGF(!Object->IsValidLowLevelFast(false), LogUObjectHash, Fatal, "IsValidLowLevelFast failure");
	UE_CLOGF(!Object->IsValidLowLevel(), LogUObjectHash, Fatal, "IsValid failure");
	UObjectBaseUtility* Outer = Object->GetOuter();
	UE_CLOGF(Outer && !Outer->IsValidLowLevelFast(false), LogUObjectHash, Fatal, "Outer IsValidLowLevelFast failure");
	UObjectBaseUtility* Class = Object->GetClass();
	UE_CLOGF(Class && !Class->IsValidLowLevelFast(false), LogUObjectHash, Fatal, "Class IsvalidLowLevelFast failure");
	
	UE_LOGF(LogUObjectHash, Fatal, "Unidentified failure for object %ls, hash itself may be corrupted or buggy.", *Object->GetFullName());
}
...
{
	typedef TCompactSet<UObjectBase*>::TIterator TIterator;

	/** This always empty set is used to get an iterator if the bucket doesn't use a TCompactSet (has only 1 element) */
	static TCompactSet<UObjectBase*> EmptyBucket;

	/*
	* Acts like an alias for either 1-2 elements or a pointer to a TCompactSet
	* If these are both null, this bucket is empty
	* If the first one is null, but the second one is non-null, then the second one is a TCompactSet pointer
	* If the first one is not null, then it is a uobject ptr, and the second ptr is either null or a second element
	*/
	void* Elements[2];

	FORCEINLINE TCompactSet<UObjectBase*>* GetItems()
	{
		if (Elements[1] && !Elements[0])
		{
			return (TCompactSet<UObjectBase*>*)Elements[1];
		}
		return nullptr;
	}

	FORCEINLINE const TCompactSet<UObjectBase*>* GetItems() const
	{
		if (Elements[1] && !Elements[0])
		{
			return (TCompactSet<UObjectBase*>*)Elements[1];
	...
	{
		TCompactSet<UObjectBase*>* Items = GetItems();
		if (Items)
		{
			Items->Add(Object);
		}
		...
		{
			Items = new TCompactSet<UObjectBase*>();
	...
	FORCEINLINE int32 Remove(UObjectBase* Object)
	{
		...
		if (Items)
		{
			...
			if (Items->Num() <= 2)
			{
				auto It = TCompactSet<UObjectBase*>::TIterator(*Items);
				Elements[0] = *It;
				checkSlow((bool)It);
				++It;
				Elements[1] = *It;
	...
	{
		const TCompactSet<UObjectBase*>* Items = GetItems();
		if (Items)
		{
	...
	/** Returns the amount of memory allocated for and by Items TCompactSet */
	...
	/** Gets an iterator for the TCompactSet in this bucket or for the EmptyBucker if Items is null */
	FORCEINLINE TCompactSet<UObjectBase*>::TIterator GetIterator()
	{
		TCompactSet<UObjectBase*>* Items = GetItems();
...
TCompactSet<UObjectBase*> FSetHashBucket::EmptyBucket;
...
public:
	...
	{
#if !UE_BUILD_SHIPPING
		UE_CLOGF(ReadOnlyLock != 0, LogObj, Fatal, "Trying to modify UObject map (Compact) that is currently being iterated. Please make sure you're not creating new UObjects or Garbage Collecting while iterating UObject hash tables.");
#endif
		Super::Compact();
	}
	...
	{
#if !UE_BUILD_SHIPPING
		UE_CLOGF(ReadOnlyLock != 0, LogObj, Fatal, "Trying to modify UObject map (Add) that is currently being iterated. Please make sure you're not creating new UObjects or Garbage Collecting while iterating UObject hash tables.");
#endif
		Super::Add(Key);
	}
	...
	{
#if !UE_BUILD_SHIPPING
		UE_CLOGF(ReadOnlyLock != 0, LogObj, Fatal, "Trying to modify UObject map (Remove) that is currently being iterated. Please make sure you're not creating new UObjects or Garbage Collecting while iterating UObject hash tables.");
#endif
		Super::Remove(Key);
	}
	...
	{
#if !UE_BUILD_SHIPPING
		UE_CLOGF(ReadOnlyLock != 0, LogObj, Fatal, "Trying to modify UObject map (FindOrAdd) that is currently being iterated. Please make sure you're not creating new UObjects or Garbage Collecting while iterating UObject hash tables.");
#endif
...
public:
	...
	void ShrinkMaps()
	{
		...
		UE_LOGF(LogUObjectHash, Log, "Compacting FUObjectHashTables data took %6.2fms", 1000.0f * float(FPlatformTime::Seconds() - StartTime));
	}
...
{
	const EInternalObjectFlags ExclusiveInternalFlags = GetMandatoryInternalExclusionFlags();
...
{
	ExclusiveInternalFlags |= GetMandatoryInternalExclusionFlags();
...
UObject* StaticFindObjectFastInternalThreadSafe(FUObjectHashTables& ThreadHash, const UClass* ObjectClass, const UObject* ObjectOuter, FName ObjectName, EFindObjectFlags Flags, EObjectFlags ExcludeFlags, EInternalObjectFlags ExclusiveInternalFlags)
{
	ExclusiveInternalFlags |= GetMandatoryInternalExclusionFlags();
	...
	{
		int32 Hash = GetObjectOuterHash(ObjectName, (PTRINT)ObjectOuter);
		FHashTableLock HashLock(ThreadHash);
		bool bExactClass = !!(Flags & EFindObjectFlags::ExactClass);
		for (TMultiMap<int32, uint32>::TConstKeyIterator HashIt(ThreadHash.HashOuter, Hash); HashIt; ++HashIt)
		{
			FUObjectItem* ObjectEntry = GUObjectArray.IndexToObject(HashIt.Value());
			UObject* Object = static_cast<UObject*>(ObjectEntry->GetObject());
			if
				/* check that the name matches the name we're searching for */
				((Object->GetFName() == ObjectName)
				...
				&& Object->GetOuter() == ObjectOuter

				/** If a class was specified, check that the object is of the correct class */
				&& (ObjectClass == nullptr || (bExactClass ? Object->GetClass() == ObjectClass : Object->IsA(ObjectClass)))
				
				/** Include (or not) pending kill objects */
				&& !ObjectEntry->HasAnyFlags(ExclusiveInternalFlags))
			{
				checkf(!ObjectEntry->IsUnreachable(), TEXT("%s"), *Object->GetFullName());
				if (Result)
				{
					UE_LOGF(LogUObjectHash, Warning, "Ambiguous search, could be %ls or %ls", *GetFullNameSafe(Result), *GetFullNameSafe(Object));
				}
				else
				{
					Result = Object;
...
#if WITH_EDITOR
		// if the search fail and the ObjectOuter is a UPackage, lookup potential external package
		if (Result == nullptr)
		{
			if (const UPackage* ObjectPackage = Cast<UPackage>(ObjectOuter))
			{
				Result = StaticFindObjectInPackageInternal(ThreadHash, ObjectClass, ObjectPackage, ObjectName, Flags, ExcludeFlags, ExclusiveInternalFlags);
			}
		}
#endif
	}
		...
		if (Bucket)
		{
			...
			for (FHashBucketIterator It(*Bucket); It; ++It)
			{
					...
					&& SearchPath.MatchOuterNames(Object->GetOuter()))
				{
					...
					if (Result)
					{
						UE_LOGF(LogUObjectHash, Warning, "Ambiguous path search, could be %ls or %ls", *GetFullNameSafe(Result), *GetFullNameSafe(Object));
					}
					else
					{
						Result = Object;
...
{
#if UE_WITH_REMOTE_OBJECT_HANDLE
	const EInternalObjectFlags ExclusiveInternalFlags = GetMandatoryInternalExclusionFlags() | InExlusiveInternalFlags;
...
#else
	UE_LOGF(LogUObjectHash, Fatal, "StaticFindObjectFastInternal override that takes FRemoteObjectId can only be used with remote object handles enabled");
...
{
	FRemoteObjectId RemoteId(Object);
	FHashBucket& Bucket = ThreadHash.HashId.FindOrAdd(GetTypeHash(RemoteId));
#if DO_CHECK
	for (FHashBucketIterator It(Bucket); It; ++It)
	{
		UObject* ExistingObject = (UObject*)*It;
		checkf(ExistingObject != Object, TEXT("%s (%s) is being added to the remote id map for the second time"), *GetPathNameSafe(ExistingObject), *RemoteId.ToString());
		FRemoteObjectId ExistingId(ExistingObject);
		checkf(ExistingId != RemoteId || ExistingObject->IsUnreachable(), TEXT("Trying to add %s to the remote id map but it has the same remote id (%s) as %s"), *GetPathNameSafe((UObject*)Object), *RemoteId.ToString(), *GetPathNameSafe(ExistingObject));
	}
#endif
	Bucket.Add(Object);
}
...
bool GAllowGetObjectsReturnUnreachableObjects = false;

static inline void AddMandatoryInternalExclusionFlags(EInternalObjectFlags& InOutExclusionFlags)
{
	InOutExclusionFlags |= GetMandatoryInternalExclusionFlags();
}
static inline void ConditionallyRemoveUnreachableFlag(EInternalObjectFlags& InOutExclusionFlags, EGetObjectsFlags Flags)
{
#if UE_WITH_REMOTE_OBJECT_HANDLE
	if (EnumHasAnyFlags(Flags, EGetObjectsFlags::EvenIfUnreachable))
	{
		checkf(GAllowGetObjectsReturnUnreachableObjects, TEXT("EGetObjectsFlags::EvenIfUnreachable specified but it's not allowed in the current context"));
		InOutExclusionFlags &= ~(EInternalObjectFlags::Unreachable);
	}
#endif
}

void GetObjectsWithOuter(const class UObjectBase* Outer, TArray<UObject *>& Results, EGetObjectsFlags Flags, EObjectFlags ExclusionFlags, EInternalObjectFlags ExclusionInternalFlags)
{
...
#if WITH_EDITOR
	if (const UPackage* OuterPackage = Cast<UPackage>(Outer))
	{
		// uncomment ensure to more easily find place where GetObjectsWithOuter should be replaced with GetObjectsWithPackage
		// ensure(!OuterPackage);

		GetObjectsWithPackage(OuterPackage, Results, Flags, ExclusionFlags, ExclusionInternalFlags);
		return;
	}
#endif
	...
	AddMandatoryInternalExclusionFlags(ExclusionInternalFlags);
	ConditionallyRemoveUnreachableFlag(ExclusionInternalFlags, Flags);
	const bool bIncludeNestedObjects = EnumHasAnyFlags(Flags, EGetObjectsFlags::IncludeNestedObjects);
...
void ForEachObjectWithOuterBreakable(const class UObjectBase* Outer, TFunctionRef<bool(UObject*)> Operation, EGetObjectsFlags Flags, EObjectFlags ExclusionFlags, EInternalObjectFlags ExclusionInternalFlags)
{
	checkf(Outer != nullptr, TEXT("Getting objects with a null outer is no longer supported. If you want to get all packages you might consider using GetObjectsOfClass instead."));
	...
	if (const UPackage* OuterPackage = Cast<UPackage>(Outer))
	{
		...
		ForEachObjectWithPackage(OuterPackage, [Operation](UObject* InObject) { Operation(InObject); return true; }, Flags, ExclusionFlags, ExclusionInternalFlags);
		return;
	}
#endif
...
UObjectBase* FindObjectWithOuter(const class UObjectBase* Outer, const class UClass* ClassToLookFor, FName NameToLookFor)
{
	...
	if(!NameToLookFor.IsNone())
	{
		// Don't calculate mandatory exclusion flags here as they are handled inside
		Result = StaticFindObjectFastInternal(ClassToLookFor, static_cast<const UObject*>(Outer), NameToLookFor);
	}
	else
	{
		// We don't want to return any objects that are currently being background loaded unless we're using the object iterator during async loading.
		EInternalObjectFlags ExclusionInternalFlags = GetMandatoryInternalExclusionFlags();
...
void GetObjectsWithPackage(const class UPackage* Package, TArray<UObject *>& Results, EGetObjectsFlags Flags, EObjectFlags ExclusionFlags, EInternalObjectFlags ExclusionInternalFlags)
{
	ForEachObjectWithPackage(Package, [&Results](UObject* Object)
	{
		Results.Add(Object);
		return true;
	}, Flags, ExclusionFlags, ExclusionInternalFlags);
}

void ForEachObjectWithPackage(const class UPackage* Package, TFunctionRef<bool(UObject*)> Operation, EGetObjectsFlags Flags, EObjectFlags ExclusionFlags, EInternalObjectFlags ExclusionInternalFlags)
{
	check(Package != nullptr);
...
static void RecursivelyPopulateDerivedClasses(FUObjectHashTables& ThreadHash, const UClass* ParentClass, TArray<ClassType, ArrayAllocator>& OutAllDerivedClass, const EInternalObjectFlags ExclusionFlags)
{
...
FORCEINLINE void ForEachObjectOfClasses_Implementation(FUObjectHashTables& ThreadHash, TArrayView<const UClass* const> ClassesToLookFor, TFunctionRef<void(UObject*)> Operation, EObjectFlags ExcludeFlags /*= RF_ClassDefaultObject*/, EInternalObjectFlags ExclusionInternalFlags /*= EInternalObjectFlags::None*/)
{
	...
	// We don't want to return any objects that are currently being background loaded unless we're using the object iterator during async loading.
	ExclusionInternalFlags |= GetMandatoryInternalExclusionFlags();
...
void ForEachObjectOfClass(const UClass* ClassToLookFor, TFunctionRef<void(UObject*)> Operation, bool bIncludeDerivedClasses, EObjectFlags ExclusionFlags, EInternalObjectFlags ExclusionInternalFlags)
{
	...
	{
		// RecursivelyPopulate expects the full exclusion flags, while the implementation function adds them
		RecursivelyPopulateDerivedClasses(ThreadHash, ClassToLookFor, ClassesToSearch, ExclusionInternalFlags | GetMandatoryInternalExclusionFlags());
	}
...
void GetDerivedClasses(const UClass* ClassToLookFor, TArray<UClass*>& Results, bool bRecursive)
{
	...
	const EInternalObjectFlags ExclusionFlags = GetMandatoryInternalExclusionFlags();
	if (bRecursive)
	{
		RecursivelyPopulateDerivedClasses(ThreadHash, ClassToLookFor, Results, ExclusionFlags);
	}
	else
	{
		...
		if (DerivedClasses)
		{
			...
			{
				if (!DerivedClass->HasAnyInternalFlags(ExclusionFlags))
				{
...
TMap<UClass*, TSet<UClass*>> GetAllDerivedClasses()
{
	...
	TMap<UClass*, TSet<UClass*>> Classes;
	bool bIsAsyncLoading = false;
	{
		FUObjectHashTables& ThreadHash = FUObjectHashTables::Get();
		FHashTableLock HashLock(ThreadHash);
		Classes = ThreadHash.ClassToChildListMap;
		bIsAsyncLoading = IsAsyncLoading();
	}

	// We don't need any filtering if the loader is currently idle
	if (bIsAsyncLoading)
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(GetAllDerivedClasses_AsyncLoadingFilter);

		const EInternalObjectFlags ExclusionFlags = GetMandatoryInternalExclusionFlags();
		for (auto It = Classes.CreateIterator(); It; ++It)
		{
			if (It->Key->HasAnyInternalFlags(ExclusionFlags))
			{
				It.RemoveCurrent();
			}
			else
			{
				...
				{
					UClass* Class = *InnerIt;
					if (Class->HasAnyInternalFlags(ExclusionFlags))
					{
...
bool ClassHasInstancesAsyncLoading(const UClass* ClassToLookFor)
{
	...
	// No exclusion flags since the goal of this function is to test those flags presence
	RecursivelyPopulateDerivedClasses(ThreadHash, ClassToLookFor, ClassesToSearch, EInternalObjectFlags::None);
...
UPackage* GetObjectExternalPackageThreadSafe(const UObjectBase* Object)
{
	...
	UE_CLOGF(!ExternalPackage && ((Object->GetFlags() & RF_HasExternalPackage) != 0), LogUObjectHash, Warning, "Object %ls ExternalPackage is invalid: RF_HasExternalPackage is set, but ExternalPackage is nullptr.", *static_cast<const UObjectBaseUtility*>(Object)->GetPathName());
