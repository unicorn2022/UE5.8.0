...
{
	UE_LOGF(LogObj, Log, "Finding Edges");
	for( FThreadSafeObjectIterator It; It; ++It )
	{
		UObject* Object = *It;
		{
			...
			for (int32 Index = 0; Index < ArFind.References.Num(); Index++)
			{
				...
				{
					UE_LOGF(LogObj, Log, "Finding Edges %d", AllEdges.Num());
				}
			}
		}
	}
	UE_LOGF(LogObj, Log, "Finding Edges Done %d", AllEdges.Num());

	UE_LOGF(LogObj, Log, "Finding permanent objects");
