...
FNumericProperty::FNumericProperty(FFieldVariant InOwner, const FName& InName)
	: FProperty(InOwner, InName)
{
}
...
int64 FNumericProperty::ReadEnumAsInt64(FStructuredArchive::FSlot Slot, const UStruct* DefaultsStruct, const FPropertyTag& Tag)
{
	...
	if (!Enum)
	{
		UE_LOGF(LogClass, Warning, "Failed to find enum '%ls' when converting property '%ls' during property loading - setting to 0", *EnumTypeName.ToString(), *Tag.Name.ToString());
		return 0;
	}
...
const TCHAR* FNumericProperty::ImportText_Internal(const TCHAR* Buffer, TNotNull<void*> ContainerOrPropertyPtr, EPropertyPointerType PropertyPointerType, UObject* Parent, int32 PortFlags, FOutputDevice* ErrorText) const
{
	if ( Buffer != NULL )
	{
...
void FNumericProperty::ExportText_Internal(FString& ValueStr, TNotNull<const void*> PropertyValueOrContainer, EPropertyPointerType PropertyPointerType, const void* DefaultValue, UObject* Parent, int32 PortFlags, UObject* ExportRootScope) const
{
