...
FSetProperty::FSetProperty(FFieldVariant InOwner, const FName& InName)
...
bool FSetProperty::Identical(TNotNull<const void*> A, const void* B, uint32 PortFlags) const
{
...
void FSetProperty::SerializeItem(FStructuredArchive::FSlot Slot, TNotNull<void*> Value, const void* Defaults) const
{
	...
	if (UnderlyingArchive.IsLoading())
	{
		...
		if(ElementProp->HasAnyPropertyFlags(CPF_HasGetValueTypeHash)) 
#endif
		{
			SetHelper.Rehash();
		}
...
bool FSetProperty::NetSerializeItem(FArchive& Ar, UPackageMap* Map, TNotNull<void*> Data, TArray<uint8>* MetaData) const
{
	UE_LOGF( LogProperty, Error, "Replicated TSets are not supported." );
	return 1;
}
...
void FSetProperty::ExportText_Internal(FString& ValueStr, TNotNull<const void*> ContainerOrPropertyPtr, EPropertyPointerType PropertyPointerType, const void* DefaultValue, UObject* Parent, int32 PortFlags, UObject* ExportRootScope) const
{
	checkSlow(ElementProp);
...
const TCHAR* FSetProperty::ImportText_Internal(const TCHAR* Buffer, TNotNull<void*> ContainerOrPropertyPtr, EPropertyPointerType PropertyPointerType, UObject* Parent, int32 PortFlags, FOutputDevice* ErrorText) const
{
	checkSlow(ElementProp);
...
void FSetProperty::CopyValuesInternal(void* Dest, void const* Src, int32 Count) const
{
	...
	checkf(SrcSetHelper.GetMaxIndex() >= Num, TEXT("Corrupt source set: Num = %d, MaxIndex = %d"), Num, SrcSetHelper.GetMaxIndex());
...
EConvertFromTypeResult FSetProperty::ConvertFromType(const FPropertyTag& Tag, FStructuredArchive::FSlot Slot, uint8* Data, const UStruct* DefaultsStruct, const uint8* Defaults)
{
	...
	if (!bConversionSucceeded)
	{
		UE_LOGF(LogClass, Warning, "Set Element Type mismatch in %ls - Previous (%ls) Current (%ls) for package: %ls",
...
bool FSetProperty::LoadTypeName(UE::FPropertyTypeName Type, const FPropertyTag* Tag)
{
	...
	FField* Field = FField::TryConstruct(ElementType.GetName(), this, GetFName());
