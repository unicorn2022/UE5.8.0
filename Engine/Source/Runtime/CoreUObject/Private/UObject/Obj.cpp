...
{
	UE_CLOGF(GIsRetrievingVTablePtr, LogCore, Fatal, "We are currently retrieving VTable ptr. Please use FVTableHelper constructor instead.");
}
...
UObject* UObject::CreateDefaultSubobject(FName SubobjectFName, UClass* ReturnType, UClass* ClassToCreateByDefault, bool bIsRequired, bool bIsTransient)
{
	...
	UE_CLOGF(!CurrentInitializer, LogObj, Fatal, "No object initializer found during construction.");
	UE_CLOGF(CurrentInitializer->Obj != this, LogObj, Fatal, "Using incorrect object initializer.");
...
bool UObject::Rename(const TCHAR* InName, UObject* NewOuter, ERenameFlags Flags)
{
...
#endif // WITH_METADATA
	const bool bIsTest = Flags & REN_Test;
	if (NewOuter && NewOuter != GetOuter())
	{
		...
		if (!HasAnyFlags(RF_ClassDefaultObject | RF_ArchetypeObject) && NewOuter->GetPackage() != GetTransientPackage())
		{
			...
			if (!NewOuter->IsA(GetClass()->ClassWithin))
			{
				...
				UE_LOGF(LogObj, Fatal, "Cannot rename %ls into Outer %ls as it is not of type %ls",
					*GetFullName(),
		...
		if (((Flags & REN_AllowPackageLinkerMismatch) == 0) && Linker != NewOuterLinker)
		{
			...
			if (NewOuterLinker || GetPackage() != NewOuter->GetPackage())
			{
				if (Linker && HasAnyFlags(RF_NeedLoad | RF_NeedPostLoad))
				{
					...
					UE_LOGF(LogObj, Error,
						"Rename is moving object '%ls' to a new package (%ls) or a new linker while the object is still being loaded (it hasn't been %ls yet). " "%ls " "Renaming a partially loaded object without first calling FLinkerLoad::InvalidateExport(ObjectToRename) " "will result in a partially loaded package likely leading to crashes and/or corrupt data.",
						*GetFullName(),
...
#if WITH_EDITOR // until we validate the runtime should also detach (renaming is uncommon)
	// If we are renaming a loaded object while keeping its outer and won't be creating a redirector, we need to detach from the linker if the user
	// hasn't specifically requested to allow for the linker mismatch. This way, we can reload the renamed object if the package is rerequested
	// Users who don't want such reload behaviour for the export should be calling InvalidateExport() on the object before renaming.
	else if (!bIsTest && ((Flags & (REN_AllowPackageLinkerMismatch | REN_DontCreateRedirectors)) == REN_DontCreateRedirectors) && HasAnyFlags(RF_WasLoaded))
	{
		if (UPackage* Package = Cast<UPackage>(this))
		{
			// When renaming a package, we must ensure that the LinkerAnnotations for each of its objects are detached to avoid "Detaching from existing linker" errors on 
			// objects loaded later on from the same package, because there is a small window of possible Object Index reuse until the next GC cycle.
			if (FLinkerLoad* Linker = Package->GetLinker()) // Note: invoking UPackage::GetLinker, not UObjectBaseUtility::GetLinker
			{
				Linker->DetachExports();
			}
		}
		else if (FLinkerLoad* Linker = GetLinker())
		{
			const int32 LinkerIndex = GetLinkerIndex();
			if (LinkerIndex != INDEX_NONE)
			{
				Linker->DetachExport(LinkerIndex);
			}
		}
	}
#endif
	...
	if (InName)
	{
		...
		else if (ExistingObject)
		{
			...
			{
				UE_LOGF(LogObj, Fatal,"Renaming an object (%ls) on top of an existing object (%ls) is not allowed", *GetFullName(), *ExistingObject->GetFullName());
			}
		}
	}
...

	{
		...
		//UE_LOGF(LogObj, Log,  "Renaming %ls to %ls", *OldName.ToString(), *NewName.ToString() );
...
#if WITH_EDITOR
	UE::FStructPostLoadContext::Get().OnPostLoad(this);
#endif
}
...
void UObject::PostRename(UObject* OldOuter, const FName OldName)
{
	...
	{
		FCoreUObjectDelegates::OnLoadedObjectRenamed.Broadcast(this, OldOuter, OldName);
	}
...
struct FClassExclusionData
{
	...
	bool IsExcluded(UClass* InClass)
	{
		...
		while (InClass != nullptr)
		{
			...
			{
				UE_LOGF(LogObj, Display, "Class %ls is excluded because its module is excluded in the current platform", *OriginalClassName.ToString());
...
void UObject::BeginDestroy()
{
	...
	{
		UE_LOGF(LogObj, Fatal,
			"Trying to call UObject::BeginDestroy from outside of UObject::ConditionalBeginDestroy on object %ls. Please fix up the calling code.",
			*GetName()
			);
	}

	UE::CoreUObject::Private::BeginDestroyPartials(*GetClass(), this);

	// Remove from linker's export table.
	SetLinker( NULL, INDEX_NONE );

	// Remove from the internal hash tables
	// The object was hashed in the open (see UObjectBase constructor calling UObjectBase::AddObject) so we need to unhash it in the open too.
	// There's two places that can call this function:
	// - GC which always runs in the open
	// - Constructing an object on top of an existing object (may be run in the closed) where the object is first destroyed (here) and then re-constucted (this is where UObjectBase::AddObject is being called again)
	// Aborting the reconstuction will always leave the object in a hashed state so no need for an OnAbort handler
	UE_AUTORTFM_OPEN
	{
#if STATS || ENABLE_STATNAMEDEVENTS_UOBJECT
		((UObject*)this)->ResetStatID(); // reset the stat id since this thing is about to be destroyed
#endif
		UnhashObject(this);
	};
...
void UObject::FinishDestroy()
{
	...
	{
		UE_LOGF(LogObj, Fatal,
			"Trying to call UObject::FinishDestroy from outside of UObject::ConditionalFinishDestroy on object %ls. Please fix up the calling code.",
			*GetName()
			);
	}
	...
	if (Class->GetPropertiesStartOffset())
	{
		UE::CoreUObject::Private::DestructPartials(*Class, this);
	}
...
bool UObject::ConditionalBeginDestroy()
{
	...
	if( !HasAnyFlags(RF_BeginDestroyed) )
	{
		...
		if ((++TotalCnt) % 1000 == 0)
		{
			UE_LOGF(LogObj, Log, "ConditionalBeginDestroy %d cnt %fus", TotalCnt, 1000.0f * 1000.0f * TotalTime / float(TotalCnt));
			...
			{
				UE_LOGF(LogObj, Log, "    %6d cnt %6.2fus per   %6.2fms total  %ls", Item.Value.Count, 1000.0f * 1000.0f * Item.Value.TotalTime / float(Item.Value.Count), 1000.0f * Item.Value.TotalTime, *Item.Key.ToString());
				if (NumPrint++ > 30)
				{
					break;
				}
		...
		if( DebugBeginDestroyed.Contains(this) )
		{
			...
			UE_LOGF(LogObj, Fatal, "%ls failed to route BeginDestroy", *GetFullName() );
		}
#endif
		return true;
	}
...
bool UObject::ConditionalFinishDestroy()
{
	...
	if( !HasAnyFlags(RF_FinishDestroyed) )
	{
		...
		{
			UE_LOGF(LogObj, Fatal, "%ls failed to route FinishDestroy", *GetFullName() );
		}
#endif
		return true;
	}
...
void UObject::ConditionalPostLoad()
{
	...
	ensureAlwaysMsgf((GetLoaderType() != ELoaderType::ZenLoader) || !HasAnyFlags(RF_NeedLoad), TEXT("Object '%s' still has RF_NeedLoad and is not ready for postload!"), *GetFullName());

	if (HasAnyFlags(RF_NeedPostLoad))
	{
		if (!ensureAlwaysMsgf(IsInGameThread() || HasAnyFlags(RF_ClassDefaultObject|RF_ArchetypeObject) || IsPostLoadThreadSafe() || IsA(UClass::StaticClass()), 
			TEXT("Illegal call to ConditionalPostLoad() on '%s'"), *GetFullName()))
		{
			return;
		}
		...
		{
			UE_LOGF(LogObj, Fatal, "%ls failed to route PostLoad.  Please call Super::PostLoad() in your <className>::PostLoad() function.", *GetFullName());
		}
#endif
	}
}
...
void UObject::PostLoadSubobjects( FObjectInstancingGraph* OuterInstanceGraph/*=NULL*/ )
{
	...
	if( GetClass()->HasAnyClassFlags(CLASS_HasInstancedReference) )
	{
		...
		// Override serialization and class types with non-serialized references may require instancing in cooked build configurations.
		auto NeedsPostLoadSubobjectInstancing = [](TNotNull<const UObject*> InObj)
		{
			// The instance is modified, but the reference itself is not overridden/serialized
			if (FOverridableManager::Get().NeedSubObjectTemplateInstantiation(InObj))
			{
				return true;
			}

			// The class type has transient default subobjects that require instancing on load
			if (InObj->GetClass()->RequiresPostLoadSubobjectInstancing())
			{
				return true;
			}
		...
		if (!FPlatformProperties::RequiresCookedData() || NeedsPostLoadSubobjectInstancing(this))
		{
			// In a cooked build, for references explicitly marked as always instanced, optimize to only those cases that did not get serialized.
			EObjectInstancingGraphOptions Options = EObjectInstancingGraphOptions::None;
			if (FPlatformProperties::RequiresCookedData())
			{
				Options = EObjectInstancingGraphOptions::InstanceTemplatesOnly;
			}

			FObjectInstancingGraph CurrentInstanceGraph(Options);
...
void UObject::PreSave(FObjectPreSaveContext SaveContext)
{
	...
	if (ObjClass->ClassWithin != nullptr)
	{
		if (!HasAnyFlags(RF_ClassDefaultObject | RF_ArchetypeObject) && GetOuter() != nullptr && !GetOuter()->IsA(ObjClass->ClassWithin))
		{
			...
			UE_LOGF(LogUObjectGlobals, Error, "%ls", *ErrorMsg);
...
void UObject::Serialize(FStructuredArchive::FRecord Record)
{
...

	{
		...
		// This item is very special from a serialization standpoint. It is never actually serialized.
		UClass *ObjClass = GetClass();
		...
		{
			// This item is very special from a serialization standpoint. It is never actually serialized.
			// Only query the outer and FName when necessary to avoid race conditions with UObject rename when serializing for reference collection
		...
		else if (UnderlyingArchive.IsTransacting())
		{
			if (!UnderlyingArchive.IsIgnoringOuterRef())
			{
				...
				// These items are very special from a serialization standpoint. They are never actually serialized.
				// Only query the outer and FName when necessary to avoid race conditions with UObject rename when serializing for reference collection
				FName LoadName = GetFName();
				UObject* LoadOuter = GetOuter();
...
void UObject::InjectDynamicImportsFor(const class FLinkerLoad& Linker, const struct FObjectExport& Export, TArray<struct FDynamicImport>& OutDynamicImports)
{
}
#endif
...
#define CompCheck(Pred) \
	...
	{ \
		Result = false; \
		UE_DEBUG_BREAK(); \
		UE_LOGF(LogObj, Error, "CompCheck %ls failed.", TEXT(#Pred)); \
	}
...
bool UObject::CheckDefaultSubobjectsInternal() const
{
	...
	if (ObjClass != UFunction::StaticClass())
	{
		...
		for (int32 Index = 0; Index < OtherReferencedSubobjects.Num(); ++Index)
		{
			...
			UE_LOGF(LogObj, Error, "%ls has a reference to default subobject (%ls) of %ls.", *GetFullName(), *TestObject->GetFullName(), *TestObject->GetOuter()->GetFullName());
		}
...
void UObject::GetResourceSizeEx(FResourceSizeEx& CumulativeResourceSize)
{
	if (CumulativeResourceSize.GetResourceSizeMode() == EResourceSizeMode::EstimatedTotal)
	{
		...
		TArray<UObject*> SubObjects;
		GetObjectsWithOuter(this, SubObjects, EGetObjectsFlags::None);
	...
	if (Sec == nullptr)
	{
		...
		if (Bucket.Sections.ContainsByHash(SectionNameHash, SectionName) == false)
		{
			...
			if (ShortSectionName != SectionName)
			{
				...
				if (Sec != nullptr)
				{
					UE_LOGF(LogObj, Fatal, "Short class section names (%ls) are not supported, please use long name: %ls", *ShortSectionName, *SectionName);
				}
			}
...
#if WITH_EDITOR
static FName GConsoleVariableFName("ConsoleVariable");
#endif
...
{
	FLoadConfigParams Context;
	Context.ConfigClass = ConfigClass;
	Context.Filename = InFilename;
	Context.PropagationFlags = PropagationFlags;
	Context.PropertyToLoad = PropertyToLoad;
	Context.AccessedValues = OutAccessedValues;

	LoadConfig(Context);
}

class FPropertyLoadConfigContext
{
public:
	FPropertyLoadConfigContext(const UObject::FLoadConfigParams& InLoadParams, UObject* InObject)
		: LoadParams(InLoadParams)
		, ConfigSystem(LoadParams.ConfigSystem ? LoadParams.ConfigSystem : GConfig)
		, ActiveObject(InObject)
		, bTrackingAccessOnly(LoadParams.AccessedValues != nullptr) // if we are tracking, we don't acutally LoadConfig onto properties
	{
	}
	
	bool InitContext()
	{
		// all must pass for the Init to succeed, and for LoadConfig to continue
		if (PrepareObject() && Propagate() && PrepareConfig())
		{
			if (LoadParams.PropertyToLoad == nullptr)
			{
				UE_LOGF(LogConfig, VeryVerbose, "(%ls) '%ls' loading configuration from %ls",
					*ConfigClass->GetName(), *ActiveObject->GetName(), *Filename);
			}
			else
			{
				UE_LOGF(LogConfig, VeryVerbose, "(%ls) '%ls' loading configuration for property %ls from %ls",
					*ConfigClass->GetName(), *ActiveObject->GetName(), *LoadParams.PropertyToLoad->GetName(), *Filename);
			}
	...
	bool InitForProperty(const FProperty* Property)
	{
		...
		if (LoadParams.PropertyToLoad != nullptr && LoadParams.PropertyToLoad != Property)
		{
			return false;
		}
		
		const bool bGlobalConfig = EnumHasAnyFlags(Property->PropertyFlags, CPF_GlobalConfig);
		FString ModifiedConfigFilename;
		
		// if the property is GlobalConfig, then the class to load the property from is the class that owns the property
		// so we may need to look in a different section
		if (bGlobalConfig && Property->GetOwnerClass() != ConfigClass)
		{
			const UClass* OwnerClass = Property->GetOwnerClass();
			
			if (!bPerObject)
			{
				ClassSection = OwnerClass->GetPathName();
			}
			// if we forced a filename, then we will use the ConfigFile already found for the class
			if (LoadParams.Filename.IsEmpty())
			{
				// look up in the owning class' branch, which in rare cases is different than this class's branch
				// however, to see if it's different is a slowish function call and a string compare, so just always look up
				ModifiedConfigFilename = OwnerClass->GetConfigName();
				ConfigFile = ConfigSystem->Find(ModifiedConfigFilename);				
			}
			else
			{
...
//			
		...
		{
			// @todo precalc this in the PrepConfig above?
			if (!bPerObject)
			{
				ClassSection = ConfigClass->GetPathName();
			}
			ConfigFile = ClassConfigFile;
		}
		...
		if (!bPerObject)
		{
			...
			ActiveObject->OverrideConfigSection(ClassSection);
		}

		Key = Property->GetName();
		PortFlags = EPropertyPortFlags::PPF_SerializedAsImportText;
		...
		if (bTrackingAccessOnly)
		{
			const FString& PropFileName = ModifiedConfigFilename.IsEmpty() ? Filename : ModifiedConfigFilename;
			LoadParams.AccessedValues->Add(UE::ConfigAccessTracking::FConfigAccessData(
		...
		ConfigSection = ConfigFile->FindSection(ClassSection);
		// look in alternate section name for POC
		if (ConfigSection == nullptr && bPerObject && ClassPathSection.Len())
		{
			ConfigSection = ConfigFile->FindSection(ClassPathSection);
		}

#if !UE_BUILD_SHIPPING
		if (ConfigSection == nullptr && !FPlatformProperties::RequiresCookedData())
		{
			const FString& PropFileName = ModifiedConfigFilename.IsEmpty() ? Filename : ModifiedConfigFilename;
		...
		UE_LOGF(LogConfig, VeryVerbose, "   Loading value for %ls from [%ls]", *Key, *ClassSection);

		// if we didn't find a section, no need to process this property - 
		// @todo: can we actually return false? some code may want to empty the property then skip if no Section was found
//		return ConfigSection == nullptr;
		return true;
	}

	bool ProcessArrayProperty(const FArrayProperty* ArrayProperty)
	{
		FScriptArrayHelper_InContainer ArrayHelper(ArrayProperty, ActiveObject);
		...
		if (ConfigSection)
		{
			...
			ConfigSection->MultiFind(KeyName, List);

			const int32 Size = ArrayProperty->Inner->GetElementSize();
			...
			if (!List.IsEmpty() || ConfigSection->EmptyInitializedKeys.Contains(KeyName))
			{
				ArrayHelper.EmptyAndAddValues(List.Num());
				for (int32 i = List.Num() - 1, c = 0; i >= 0; i--, c++)
				{
					ArrayProperty->Inner->ImportText_Direct(*List[i].GetValue(), ArrayHelper.GetRawPtr(c), ActiveObject, PortFlags);
				}
			}
			else
			{
				...
				do
				{
					...
					ElementValue = ConfigSection->Find(IndexedName);
					...
					if (ElementValue != nullptr)
					{
						...
						ArrayProperty->Inner->ImportText_Direct(*ElementValue->GetValue(), ArrayHelper.GetRawPtr(Index), ActiveObject, PortFlags);
					}
	...
	bool ProcessSetProperty(const FSetProperty* SetProperty)
	{
		bool bProcessedProperty = true;

		FScriptSetHelper_InContainer SetHelper(SetProperty, ActiveObject);
			...
			if (!bSingleSetEntry && !List.IsEmpty())
			{
				...
				for (const FConfigValue& ListValue : List)
				{
					...
					const TCHAR* SetImportResult =
						SetProperty->ImportText_Direct(*ListString, TempSet, ActiveObject, PortFlags);
					...
					else
					{
						...
						SetProperty->ElementProp->ImportText_Direct(*ListString, TempElement, ActiveObject, PortFlags);
		...
		// if we get here, we didn't process the property as a Set, so return false, and we will try again 
		// with ProcessStandardProperty
		return false;
	}
	
	void ProcessStandardProperty(FProperty* Property)
	{
		for (int32 i = 0; i < Property->ArrayDim; i++)
		{
			...
			{
				FString Value;
				if (ConfigSection->GetString(*Key, Value))
				{
					if (Property->ImportText_Direct(*Value, Property->ContainerPtrToValuePtr<uint8>(ActiveObject, i), ActiveObject, PortFlags) == nullptr)
					{
						...
						UE_LOGF(LogObj, Error, "LoadConfig (%ls): import failed for %ls in: %ls", *ActiveObject->GetPathName(), *Property->GetName(), *Value);
					}
				}
			}
		}		
...
private:	
	...
	bool PrepareObject()
	{
		ConfigClass = LoadParams.ConfigClass ? LoadParams.ConfigClass : ActiveObject->GetClass();
		if (!ConfigClass->HasAnyClassFlags(CLASS_Config))
		{
			return false;
		}

		bPerObject = UsesPerObjectConfig(ActiveObject);
		bIsPerPlatformConfig = ActiveObject->GetClass()->HasAnyClassFlags(CLASS_PerPlatformConfig);

		// handle some special cases and validations
		if (bPerObject)
		{
			HandlePerObject();
		}
		...
		// we only support tracking values in with the editor
#if !WITH_EDITOR
		if (bTrackingAccessOnly)
		{
			return false;
	...
	bool PrepareConfig()
	{
		if (LoadParams.OverrideFile != nullptr)
		{
			checkf(LoadParams.Filename.IsEmpty(), TEXT("Cannot have FLoadConfigParams::OverrideFile and FLoadConfigParams::Filename both set, it indicates a logic issue."));
			ClassConfigFile = LoadParams.OverrideFile;
			return true;
		}
		
		// if a filename was specified, load all properties from that file no matter which class they come from
		Filename = LoadParams.Filename.IsEmpty() ? GetConfigFilename(ActiveObject) : LoadParams.Filename;
		...
		const TCHAR* ConfigOverridePlatform = ActiveObject->GetConfigOverridePlatform();

		if (LoadParams.Filename.IsEmpty() && ConfigOverridePlatform != nullptr &&
			FCString::Stricmp(ConfigOverridePlatform, ANSI_TO_TCHAR(FPlatformProperties::IniPlatformName())) != 0)
		{
			...
			if (!bTrackingAccessOnly)
			{
				ClassConfigFile = FConfigCacheIni::FindOrLoadPlatformConfig(PlatformOverrideConfig,
					*ActiveObject->GetClass()->ClassConfigName.ToString(), ConfigOverridePlatform);
//				bUseConfigOverride = true;
			}
		}
#if WITH_EDITOR
		else if (bIsPerPlatformConfig && UObject::OnGetPreviewPlatform.IsBound())
		{
			...
			if (UObject::OnGetPreviewPlatform.Execute(PreviewPlatform))
			{
				...
				{
					ClassConfigFile = FConfigCacheIni::FindOrLoadPlatformConfig(PlatformOverrideConfig,
						*ActiveObject->GetClass()->ClassConfigName.ToString(), *PreviewPlatformStr);
//					bUseConfigOverride = true;
				}
			}
		}
		...
		// if we didn't have one specified already, we will use the ConfigSystem's branch for this class 
		if (ClassConfigFile == nullptr)
		{
			ClassConfigFile = ConfigSystem->Find(Filename);
		}
		
		// if we can't find the branch, nothing to do here (unless tracking, then let it continue tracking what would have been read)
		return ClassConfigFile != nullptr || bTrackingAccessOnly;
	}
	...
	bool Propagate()
	{
		...
		if (ParentClass != nullptr)
		{
			...
			{
				if ((LoadParams.PropagationFlags & UE::LCPF_ReadParentSections) != 0)
				{
					// call LoadConfig on the parent class
					UObject::FLoadConfigParams ParentContext = LoadParams;
					ParentContext.ConfigClass = ParentClass;
					ParentContext.Filename = TEXT("");
					ActiveObject->LoadConfig(ParentContext);
					...
					if ((LoadParams.PropagationFlags & (UE::LCPF_PropagateToChildDefaultObjects | UE::LCPF_PropagateToInstances)) != 0)
					{
						return false;
					}
				}
				else if ((LoadParams.PropagationFlags & UE::LCPF_PropagateToChildDefaultObjects) != 0)
				{
					// not propagating the call upwards, but we are propagating the call to all child classes

					UObject::FLoadConfigParams ChildContext = LoadParams;
					// mask out the PropgateToParent and PropagateToChildren values
					ChildContext.PropagationFlags = (LoadParams.PropagationFlags & (UE::LCPF_PersistentFlags | UE::LCPF_PropagateToInstances));
					ChildContext.Filename = TEXT("");
					for (TObjectIterator<UClass> It; It; ++It)
					{
						if (It->IsChildOf(ConfigClass))
						{
							ChildContext.ConfigClass = *It;
							It->GetDefaultObject()->LoadConfig(ChildContext);
						}
					}
				...
				else if ((LoadParams.PropagationFlags & UE::LCPF_PropagateToInstances) != 0)
				{
					...
					UObject::FLoadConfigParams InstanceContext = LoadParams;
					// mask out the PropgateToParent and PropagateToChildren values
					InstanceContext.PropagationFlags = (LoadParams.PropagationFlags & (UE::LCPF_PersistentFlags));
					InstanceContext.Filename = TEXT("");
					for (TObjectIterator<UObject> It; It; ++It)
					{
						if (It->IsA(ConfigClass))
						{
							// null would also work
							InstanceContext.ConfigClass = It->GetClass();
							if (!GIsEditor || bTrackingAccessOnly)
							{
								It->LoadConfig(InstanceContext);
							}
	#if WITH_EDITOR
							else
							{
	...
	void HandlePerObject()
	{
		FString PathNameString;
		UObject* Outermost = ActiveObject->GetOutermost();

		if (Outermost == GetTransientPackage())
		{
			PathNameString = ActiveObject->GetName();
		}
		else
		{
			ActiveObject->GetPathName(Outermost, PathNameString);
		}

		// this will be set per-property for non-per-object
		ClassSection = PathNameString + TEXT(" ") + ActiveObject->GetClass()->GetName();

		FString OverrideClassSection;
		ActiveObject->OverridePerObjectConfigSection(OverrideClassSection);
		...
		else
		{
			...
			ClassPathSection = PathNameString + TEXT(" ") + ActiveObject->GetClass()->GetPathName();
		}
	}

	void CheckEarlyAccess()
	{
#if DO_CHECK
		...
		{
			FString ErrorMessage = FString::Printf(TEXT("Attempting to load config data for %s before the Class has been constructed/registered/linked (likely during module loading or early startup). This will result in the load silently failing and should be fixed."), *ActiveObject->GetName());
			if (GPrematureConfigLoadErrorLevel == 0)
			{
				UE_LOGF(LogObj, Display, "%ls", *ErrorMessage);
			}
			else if (GPrematureConfigLoadErrorLevel == 1)
			{
				UE_LOGF(LogObj, Warning, "%ls", *ErrorMessage);
			}
...
public:
	// the final class to walk over properties in LoadConfig
	const UClass* ConfigClass = nullptr;

private:

	///// settings for the entire class

	const UObject::FLoadConfigParams& LoadParams;
	FConfigCacheIni* ConfigSystem = nullptr;
	UObject* ActiveObject = nullptr;
	bool bPerObject = false;
	bool bIsPerPlatformConfig = false;
	// the default section for the class, but each property can potentially override it when setting ConfigSection
	FString ClassSection;
	FString ClassPathSection;
	// the filename to look up in ConfigSystem,
	FString Filename;
	// storage for a local file when loading another platform's config
	FConfigFile PlatformOverrideConfig;
	// the config file for the class, but it can change per property (rarely)
	const FConfigFile* ClassConfigFile = nullptr;

	///// per-property settings

	// flags to control the text import
	int32 PortFlags = 0;
	// the final file to load from
	const FConfigFile* ConfigFile = nullptr;
	// the final Section to read Key from
	const FConfigSection* ConfigSection = nullptr;
	// the key to use to look in ConfigSection
	FString Key;

	
	// the ConfigPlatform is only used for tracking
	bool bTrackingAccessOnly = false;
#if WITH_EDITOR
	FName ConfigPlatform;
#endif
};

void UObject::LoadConfig(const FLoadConfigParams& Params)
{
	SCOPE_CYCLE_COUNTER(STAT_LoadConfig);

	// make the local context that manages the complexity
	FPropertyLoadConfigContext PropertyContext(Params, this);
	
	// there are various reasons why we won't need to continue, so early out
	if (PropertyContext.InitContext() == false)
	{
		return;
	}
		
	for (FProperty* Property = PropertyContext.ConfigClass->PropertyLink; Property; Property = Property->PropertyLinkNext)
	{
#if WITH_EDITOR
		...
		if (!PropertyContext.InitForProperty(Property))
		{
			continue;
		}
		...
		if (ArrayProperty)
		{
			bProcessedProperty = PropertyContext.ProcessArrayProperty(ArrayProperty);
		}
		else if (SetProperty)
		{
			bProcessedProperty = PropertyContext.ProcessSetProperty(SetProperty);
		}
		
		if (!bProcessedProperty)
		{
			PropertyContext.ProcessStandardProperty(Property);			
		}
	}

	// if we are reloading config data after the initial class load, fire the callback now
	if (Params.AccessedValues == nullptr && (Params.PropagationFlags & UE::LCPF_ReloadingConfigData) != 0)
	{
		PostReloadConfig(Params.PropertyToLoad);
	}
}
...
#if WITH_METADATA
			static const FName NAME_DeprecatedProperty = "DeprecatedProperty";
...
bool UObject::TryUpdateDefaultConfigFile(const FString& SpecificFileLocation, bool bWarnIfFail)
{
	...
	if (bWarnIfFail)
	{
		UE_LOGF(LogObj, Warning, "Ini File '%ls' is read-only and cannot be written to", *ConfigFile);
	}
...
void UObject::UpdateSinglePropertyInConfigFile(const FProperty* InProperty, const FString& InConfigIniName)
{
	...
	if (!InProperty->IsA(FArrayProperty::StaticClass()))
	{
		...
		if (!ensureMsgf(!Keys.IsEmpty(), TEXT("Unable to find section for property '%s' in '%s'. Make sure the property has the 'config' specifier.")
			, *PropertyKey
	...
	{
		UE_LOGF(LogObj, Warning, "UObject::UpdateSinglePropertyInConfigFile does not support this property type.");
		return;
	}
}
...
static void StaticShutdownAfterError()
{
	if( UObjectInitialized() )
	{
		...
		bShutdown = true;
		UE_LOGF(LogExit, Log, "Executing StaticShutdownAfterError" );
	...
	else if( FParse::Command(&Str,TEXT("OBJ")) )
	{
		...
		else if( FParse::Command(&Str,TEXT("TRANSACTIONAL")) )
		{
			...
			for( FThreadSafeObjectIterator It; It; ++It )
			{
				...
				UE_LOGF(LogObj, Log, "%1d %ls",(int32)It->HasAnyFlags(RF_Transactional),*It->GetFullName());
			}
			UE_LOGF(LogObj, Log, "%d/%d",NumTransactional,Num);
			return true;
		}
		else if( FParse::Command(&Str,TEXT("MARK")) )
		{
			UE_LOGF(LogObj, Log,  "Marking objects" );
			for( FThreadSafeObjectIterator It; It; ++It )
			{
		...
		{
			UE_LOGF(LogObj, Log,  "Unmarked (new) objects:" );
			for( FThreadSafeObjectIterator It; It; ++It )
			{
				if(!DebugMarkAnnotation.Get(*It))
				{
					UE_LOGF(LogObj, Log,  "%ls", *It->GetFullName() );
				}
			}
			return true;
		}
		...
		{
			UE_LOGF(LogObj, Log,  "InvMarking existing objects" );
		...
		{
			UE_LOGF(LogObj, Log,  "Objects that were deleted:" );
			for (int32 Old = 0; Old < DebugInvMarkNames.Num(); Old++)
			{
				...
				else
				{
					check(DebugInvMarkWeakPtrs[Old].IsStale());
					check(!DebugInvMarkWeakPtrs[Old].IsValid());
					UE_LOGF(LogObj, Log,  "%ls", *DebugInvMarkNames[Old]);
				}
			}
			return true;
		}
		...
		{
			UE_LOGF(LogObj, Log,  "Spikemarking objects" );
		...
		{
			UE_LOGF(LogObj, Log,  "Spikemarked (created and then destroyed) objects:" );
			for( const FString& Name : DebugSpikeMarkNames )
			{
				UE_LOGF(LogObj, Log,  "  %ls", *Name );
			}
			return true;
		}
		else if( FParse::Command(&Str,TEXT("REFS")) )
		{
			...
			if (ParseObject(Str,TEXT("NAME="),Object,nullptr))
			{
				...
				while(FParse::Token(Str, Tok, false))
				{
					if (FCString::Stricmp(*Tok, TEXT("shortest")) == 0)
					{
						...
						{
							UE_LOGF(LogObj, Log, "Specifing 'shortest' AND 'longest' is invalid. Ignoring this occurence of 'shortest'.");
						}
					...
					else if (FCString::Stricmp(*Tok, TEXT("longest")) == 0)
					{
						...
						{
							UE_LOGF(LogObj, Log, "Specifing 'shortest' AND 'longest' is invalid. Ignoring this occurence of 'longest'.");
						}
					...
					else if (FParse::Value(Str, TEXT("history="), HistoryLevel))
					{
						if (FGCHistory::Get().IsActive())
						{
							...
							{
								UE_LOGF(LogObj, Log, "GC History level %d will be clamped to the current max %d", HistoryLevel, MaxHistoryLevel);
								HistoryLevel = MaxHistoryLevel;
							}
						}
						else if (HistoryLevel != 0)
						{
							UE_LOGF(LogObj, Log, "GC History is not currently enabled. Use 'gc.HistorySize 1' console command to enable it.");
							HistoryLevel = 0;
						}
					}
				...
				else
				{
					...
					{
						UE_LOGF(LogObj, Log, "There's not been that many (%d) GC runs to be able to search for object refs in GC history.", (HistoryLevel + 1));
					}
				}
#endif // ENABLE_GC_HISTORY
			}
			...
			{
				UE_LOGF(LogObj, Log, "Couldn't find object.");
			}
			return true;
		}
		...
		else if( FParse::Command(&Str,TEXT("DEPENDENCIES")) )
		{
			...
			if( ParseObject<UPackage>(Str,TEXT("PACKAGE="),Pkg,NULL) )
			{
				...
				UE_LOGF(LogObj, Log, "Package wasn't found.");
			return true;
		}
		...
		else if( FParse::Command(&Str,TEXT("LISTCONTENTREFS")) )
		{
			...
			if( Class )
			{
				...
				if( bShouldListAsCSV )
				{
					UE_LOGF(LogObj, Log, ",Class,Object");
				}
				else
				{
					UE_LOGF(LogObj, Log, "Dumping references for %ls",*Class->GetFullName());
				}
				...
				for( TSet<UObject*>::TConstIterator It(ListRefsAr.ReferencedObjects); It; ++It ) 
				{
					...
					&&	ObjectReference->GetOutermost() != GetTransientPackage() )
					{
						if( bShouldListAsCSV )
						{
							UE_LOGF(LogObj, Log, ",%ls,%ls",*ObjectReference->GetClass()->GetPathName(),*ObjectReference->GetPathName());
						}
						else
						{
							UE_LOGF(LogObj, Log, "   %ls",*ObjectReference->GetFullName());
						}
					}
				}
			}
...
void InitUObject();
AUTORTFM_DISABLE void StaticExit();

void InitUObject()
{
	TRACE_CPUPROFILER_EVENT_SCOPE(InitUObject);
...
#if (!UE_BUILD_SHIPPING || defined(UE_ENABLE_ASSET_READ_LOGGER))
	// Initialize asset read logger if enabled via command line
	FAssetReadLogger::Get().Initialize();
#endif
...
void StaticUObjectInit()
{
	...
	UE_LOGF(LogInit, Log, "Object subsystem initialized" );
}

// Internal cleanup functions
AUTORTFM_DISABLE void ShutdownGarbageCollection();
void CleanupLinkerAnnotations();
AUTORTFM_DISABLE void PurgeAllUObjectsOnExit();
...
void StaticExit()
{
...
#if (!UE_BUILD_SHIPPING || defined(UE_ENABLE_ASSET_READ_LOGGER))
	// Shutdown asset read logger and write output file
	FAssetReadLogger::Get().Shutdown();
#endif
	...
	FObjectArchetypeHelper::CleanupCachedArchetypes();
	UE::RemoteObject::Private::ShutdownRemoteObjects();

	UE_LOGF(LogExit, Log, "Object subsystem successfully closed." );
}
...
void UObject::AbortInsideMemberFunction() const
{
	...
	UE_LOGF(LogObj, Warning, "UObject::AbortInsideMemberFunction called on object %ls.", *GetFullName());
