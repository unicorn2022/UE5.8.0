	...
	void ConvertIntValueToEnumProperty(OldIntType OldValue, FEnumProperty* EnumProp, FNumericProperty* UnderlyingProp, UEnum* Enum, void* Obj)
	{
		...
		if (!UnderlyingProp->CanHoldValue(NewValue) || !Enum->IsValidEnumValueOrBitfield(NewValue))
		{
			...
			UE_LOGF(
				LogClass,
				Warning,
				"Failed to find valid enum value '%ls' for enum type '%ls' when converting property '%ls' during property loading - setting to '%ls'",
...
FEnumProperty::FEnumProperty(FFieldVariant InOwner, const FName& InName)
...
{
	Enum = Prop.EnumFunc ? Prop.EnumFunc(ETypeConstructPhase::Inner) : nullptr;
...
void FEnumProperty::SerializeItem(FStructuredArchive::FSlot Slot, TNotNull<void*> Value, void const* Defaults) const
{
...
bool FEnumProperty::NetSerializeItem(FArchive& Ar, UPackageMap* Map, TNotNull<void*> Data, TArray<uint8>* MetaData) const
{
...
void FEnumProperty::ExportText_Internal(FString& ValueStr, TNotNull<const void*> PropertyValueOrContainer, EPropertyPointerType PropertyPointerType, const void* DefaultValue, UObject* Parent, int32 PortFlags, UObject* ExportRootScope) const
{
	if (Enum == nullptr)
	{
		UE_LOGF(
			LogClass,
			Warning,
			"Member 'Enum' of %ls is nullptr, export operation would fail. This can occur when the enum class has been moved or deleted.",
			*GetFullName()
		);
		return;
	}
...
const TCHAR* FEnumProperty::ImportText_Internal(const TCHAR* Buffer, TNotNull<void*> ContainerOrPropertyPtr, EPropertyPointerType PropertyPointerType, UObject* Parent, int32 PortFlags, FOutputDevice* ErrorText) const
{
	check(Enum);
	check(UnderlyingProp);
...
bool FEnumProperty::Identical(TNotNull<const void*> A, const void* B, uint32 PortFlags) const
{
...
void FEnumProperty::AppendSchemaHash(FBlake3& Builder, bool bSkipEditorOnly) const
{
	...
	if (Enum)
	{
		...
		// All redirectors that should trigger a recook are handled either by schema hash, package digest, package redirector dependencies or 
		// global core redirect dependencies. But there is one case not handled : native package redirectors mapping an enum value to another. 
		// To detect when an enum value is redirected to another value, I add the FCoreRedirect to the schema hash. This will trigger
		// a recook when an enum property value might be changed because of a redirector.
		// It might trigger useless recooks as well since the redirection could be about a value not used by the current object but it's
		// better than not detecting those cases at all.
		// Adding enum value redirectors to the schema hash will also change the package digest of any package containing an object with this enum. 
		// This is also a case I want to detect. For example : package A depends on package B and package B has an object with an enum property. 
		// If the enum property value is redirected, then it could matter for package A. The change of package digest for B will trigger a recook of A.
		FCoreRedirectObjectName OldName(*NameBuilder);
		FCoreRedirectObjectName NewName;
		const FCoreRedirect* Redirect = nullptr;
		FCoreRedirects::RedirectNameAndValues(ECoreRedirectFlags::Type_Enum, OldName, NewName, &Redirect);

		if (Redirect)
		{
			Redirect->AppendHash(Builder);
		}
	}
}
#endif
...
bool FEnumProperty::LoadTypeName(UE::FPropertyTypeName Type, const FPropertyTag* Tag)
{
	...
	FField* Field = FField::TryConstruct(UnderlyingType.GetName(), this, GetFName());
	if (FNumericProperty* Property = CastField<FNumericProperty>(Field); Property && Property->LoadTypeName(UnderlyingType, Tag))
	{
		Enum = LocalEnum;
		UE_CLOGF(!Property->CanHoldValue(Enum->GetMaxEnumValue()), LogClass, Warning,
			"Enum '%ls' does not fit in a %ls loading property '%ls'.",
