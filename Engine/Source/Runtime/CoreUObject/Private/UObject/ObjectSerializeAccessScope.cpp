			...
			: bEnabled(bEnabled)
		{
			if (bEnabled)
			{
				FObjectHandleReadFunc Func = [this](const TArrayView<const UObject* const>& ReadObject)
				{
					...
					if (ReadObject.Num() == 1)
					{
						UE_LOGF(LogLinker, Warning, "%ls trying to access referenced object %ls during serialize\n%ls", *CurrentObject.GetClass()->GetFullName(), (ReadObject[0] ? *ReadObject[0]->GetFullName() : TEXT("None")), ANSI_TO_TCHAR(StackTrace));
					}
					else
					{
						UE_LOGF(LogLinker, Warning, "%ls trying to access an array of referenced objects during serialize\n%ls", *CurrentObject.GetClass()->GetFullName(), ANSI_TO_TCHAR(StackTrace));
					}
				};
