...
FField* FVerseStringProperty::GetInnerFieldByName(const FName& InName)
{
...
void FVerseStringProperty::GetInnerFields(TArray<FField*>& OutFields)
{
	if (Inner)
	{
		OutFields.Add(Inner);
...
void FVerseStringProperty::ExportText_Internal(FString& ValueStr, TNotNull<const void*> PropertyValueOrContainer, EPropertyPointerType PropertyPointerType, const void* DefaultValue, UObject* Parent, int32 PortFlags, UObject* ExportRootScope) const
{
...
const TCHAR* FVerseStringProperty::ImportText_Internal(const TCHAR* Buffer, TNotNull<void*> ContainerOrPropertyPtr, EPropertyPointerType PropertyPointerType, UObject* Parent, int32 PortFlags, FOutputDevice* ErrorText) const
{
...
void FVerseStringProperty::PostDuplicate(const FField& InField)
{
	const FVerseStringProperty* Source = CastFieldChecked<FVerseStringProperty>(&InField);
...
bool FVerseStringProperty::LoadTypeName(UE::FPropertyTypeName Type, const FPropertyTag* Tag)
{
	...
	Inner = new FByteProperty(this, GetFName());
	return true;
}
