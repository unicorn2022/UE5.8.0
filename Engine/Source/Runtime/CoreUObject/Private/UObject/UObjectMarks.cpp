...
struct FObjectMark
{
	...
	bool ClearMarks(FObjectMark InMarksToClear)
	{
		Marks = static_cast<EObjectMark>(Marks & ~InMarksToClear.Marks);
		return IsDefault();
	}
...
public:
	...
	void RemoveFromAnnotations(const TAnnotation& Annotation)
	{
		if (!AnnotationMap.IsEmpty())
		{
			AnnotationCacheKey = nullptr;
			AnnotationCacheValue = TAnnotation();
			for (typename TMap<const UObjectBase*, TAnnotation>::TIterator It(AnnotationMap); It; ++It)
			{
				if (It.Value().ClearMarks(Annotation))
				{
					It.RemoveCurrent();
				}
			}
...
void UnMarkAllObjects(EObjectMark Marks)
{
	...
	{
		ThreadMarkAnnotation.RemoveFromAnnotations(Marks);
	}
}
