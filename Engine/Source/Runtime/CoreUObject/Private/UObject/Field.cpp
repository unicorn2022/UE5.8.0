...
{
#if !IS_MONOLITHIC
	InFieldClass->SuperClass = InFieldClass->GetSuperStaticClass();
#endif
...
FFieldClass::FFieldClass(const TCHAR* InCPPName, uint64 InId, EClassCastFlags InCastFlags, FFieldClass* InSuperClass, FConstructFunction InConstructFn)
...
{
	FField* NewDefault = Construct(UClass::StaticClass()->GetOutermost(), *FString::Printf(TEXT("Default__%s"), *GetName()));
	return NewDefault;
}
...
FField* FField::Construct(const FFieldVariant& InOwner, const FName& InName)
{
...
#if UE_WITH_CONSTINIT_UOBJECT
constinit TNoDestroy<TFieldClass<FField>> FField::ConstInitClass{NoDestroyConstEval};
#endif // UE_WITH_CONSTINIT_UOBJECT
...
FField::FField(FFieldVariant InOwner, const FName& InName)
	: Owner(InOwner)
	, Next(nullptr)
	, NamePrivate(InName)
#if WITH_METADATA
...
void FField::Serialize(FArchive& Ar)
{
	...
	{
		uint32 FlagsPrivateDummy = 0;
		Ar << FlagsPrivateDummy;
	}
...
{
	UE_LOGF(LogClass, Fatal, "FField::AddCppProperty");
}
...
const FText FField::GetMetaDataText(const TCHAR* MetaDataKey, const FTextKey LocalizationNamespace, const FTextKey LocalizationKey) const
{
	...
	{
		DefaultMetaData = FName::NameToDisplayString(GetAuthoredName(), IsA(FBoolProperty::StaticClass()));
	}
...
FField* FField::Duplicate(const FField* InField, FFieldVariant DestOwner, const FName DestName)
{
	check(InField);
	FField* NewField = InField->GetClass()->Construct(DestOwner, (DestName == NAME_None) ? InField->GetFName() : DestName);
	NewField->PostDuplicate(*InField);
	return NewField;
}

FField* FField::Construct(const FName& FieldTypeName, const FFieldVariant& InOwner, const FName& InName)
{
	...
	FField* Instance = (*FieldClassPtr)->Construct(InOwner, InName);
	return Instance;
}

FField* FField::TryConstruct(const FName& FieldTypeName, const FFieldVariant& InOwner, const FName& InName)
{
	if (FFieldClass* FieldClassPtr = FFieldClass::GetNameToFieldClassMap().FindRef(FieldTypeName))
	{
		return FieldClassPtr->Construct(InOwner, InName);
	}
	return nullptr;
}
