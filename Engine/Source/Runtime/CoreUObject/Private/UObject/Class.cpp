...
{
	UE_LOGF(LogClass, Fatal,"UField::AddCppProperty");
}
...
FText UField::GetToolTipText(bool bShortTooltip) const
{
	...
	static const FName NAME_ShortTooltip("ShortTooltip");
...
void UField::FormatNativeToolTip(FString& ToolTipString, bool bRemoveExtraSections)
{
	...
	static const FString DoxygenSee("@see");
	static const FString TooltipSee("See:");
...
FText UField::GetMetaDataText(const TCHAR* MetaDataKey, const FTextKey LocalizationNamespace, const FTextKey LocalizationKey) const
{
	...
	{
		DefaultMetaData = FName::NameToDisplayString(GetAuthoredName(), false);
	}
...
void UStruct::Link(FArchive& Ar, bool bRelinkExistingProperties)
{
	...
	{
		Wrapper->Rename(nullptr, GetTransientPackage(), REN_DoNotDirty | REN_DontCreateRedirectors | REN_NonTransactional| REN_AllowPackageLinkerMismatch);
...
//
void UStruct::SerializeBin(FStructuredArchive::FSlot Slot, TNotNull<void*> Data) const
{
#if WITH_EDITORONLY_DATA
...
void UStruct::SerializeBinEx( FStructuredArchive::FSlot Slot, TNotNull<void*> Data, void const* DefaultData, UStruct* DefaultStruct ) const
{
...
void UStruct::LoadTaggedPropertiesFromText(FStructuredArchive::FSlot Slot, TNotNull<uint8*> Data, const UStruct* DefaultsStruct, const uint8* Defaults, const UObject* BreakRecursionIfFullyLoad) const
{
	FArchive& UnderlyingArchive = Slot.GetUnderlyingArchive();
	FUObjectSerializeContext* SerializeContext = FUObjectThreadContext::Get().GetSerializeContext();
...
void UStruct::SerializeTaggedProperties(FStructuredArchive::FSlot Slot, TNotNull<uint8*> Data, const UStruct* DefaultsStruct, const uint8* Defaults, const UObject* BreakRecursionIfFullyLoad) const
{
#if WITH_EDITORONLY_DATA
	FUObjectSerializeContext* SerializeContext = FUObjectThreadContext::Get().GetSerializeContext();
	const bool bSaveSerializedPropertyPath = IsA<UClass>() && SerializeContext && !SerializeContext->SerializedPropertyPath.IsEmpty();
...
void UStruct::SerializeVersionedTaggedProperties(FStructuredArchive::FSlot Slot, TNotNull<uint8*> Data, const UStruct* DefaultsStruct, const uint8* Defaults, const UObject* BreakRecursionIfFullyLoad) const
{
	using namespace UE;
	...
	if (bIsUClass && UnderlyingArchive.UEVer() >= EUnrealEngineObjectUE5Version::PROPERTY_TAG_EXTENSION_AND_OVERRIDABLE_SERIALIZATION)
	{
		...
		if (EnumHasAnyFlags(SerializationControl, EClassSerializationControlExtension::OverridableSerializationInformation))
		{
			...
			if (UnderlyingArchive.IsLoading())
			{
				...
				{
					ControlContext.OverriddenProperties = FOverridableManager::Get().RestoreOverrideOperation((UObject*)NotNullGet(Data), Operation, /*bNeedsSubobjectTemplateInstantiation*/true, UnderlyingArchive.ArMergeOverrides);
				}
			}
		}
	}
...
#endif // WITH_TEXT_ARCHIVE_SUPPORT
		{
			...
			while (!UnderlyingArchive.IsCriticalError())
			{
				...
				if (Property)
				{
					...
					{
						UE_LOGF(LogClass, Warning, "Array bound exceeded in %ls of %ls - %d exceeds [0-%d] in package: %ls",
					...
					{
						UE_CLOGF((UnderlyingArchive.IsPersistent() && FPlatformProperties::RequiresCookedData()), LogClass, Warning, "Skipping saved property %ls of %ls since it is no longer serializable for asset:  %ls. (Maybe resave asset?)", *Tag.Name.ToString(), *GetName(), *UnderlyingArchive.GetArchiveName());
					}
#if WITH_EDITORONLY_DATA
					// Handle the sub object shadowing serialization,
					// do not skip this property if loading into a unknown property or into a placeholder
					else if (Tag.OverrideOperation == EOverriddenPropertyOperation::SubObjectsShadowing && 
						(!SerializeContext->bImpersonateProperties ))
					{
				...
				if (UNLIKELY(bTryStoreUnknownPropertyPath))
				{
					...
					if (UnknownPropertyTree)
					{
						...
						{
							TUniquePtr<FField> TempField(FField::TryConstruct(Tag.Type, {}, Tag.Name));
							if (FProperty* TempProperty = CastField<FProperty>(TempField.Get()); TempProperty && TempProperty->LoadTypeName(Tag.GetType(), &Tag))
							{
								...
								if (bSerializeValue)
								{
									TGuardValue<bool> ScopedLoadingUnknownPropertyTree(SerializeContext->bIsLoadingToUnknownPropertyTree, true);
				...
				else if (Tag.Size != Loaded)
				{
					...
					if(!UnderlyingArchive.IsCriticalError())
...
#if WITH_EDITOR
						static const FName NAME_PropertySerialize = FName("PropertySerialize");
...
void UStruct::SerializeProperties(FArchive& Ar)
{
	...
	if (Ar.IsLoading())
	{
		...
		for (int32 PropertyIndex = 0; PropertyIndex < PropertyCount; ++PropertyIndex)
		{
			...
			Ar << PropertyTypeName;
			FField* Prop = FField::Construct(PropertyTypeName, this, NAME_None);
			check(Prop);
			Prop->Serialize(Ar);
...
void UStruct::Serialize(FArchive& Ar)
{
	...
	else
	{
		...
		if (!GIsDuplicatingClassForReinstancing)
		{
			...
			{
				UE_LOGF(LogClass, Fatal, "Script serialization mismatch: Got %i, expected %i", iCode, ScriptBytecodeSize);
			}
...
void UStruct::PostLoad()
{
	...
	if (UnresolvedScriptProperties)
	{
		for (TPair<TFieldPath<FField>, int32>& MissingProperty : *UnresolvedScriptProperties)
		{
			...
			{
				UE_LOGF(LogClass, Warning, "Failed to resolve bytecode referenced field from path: %ls when loading %ls", *MissingProperty.Key.ToString(), *GetFullName());
			}
		}
		DeleteUnresolvedScriptProperties();
	}
...
#include "UObject/ScriptSerialization.inl"
	return Expr;
...
{
	const FObjectInitializer* ObjInitializer = FUObjectThreadContext::Get().TopInitializer();
	...
	for (FProperty* Property = RefLink; Property != nullptr; Property = Property->NextRef)
	{
		...
		const bool bPropertyInExclusionList =
			...
			(ObjInitializer && ObjInitializer->IsPropertyInSubobjectExclusionList(Property))
		;
...
bool FindConstructorUninitialized(UStruct* BaseClass,uint8* Data,uint8* Defaults)
{
	...
	for(FProperty* P=BaseClass->PropertyLink; P; P=P->PropertyLinkNext )
	{
		...
		else
		{
			...
			{
//				UE_LOGF(LogClass, Warning,"Mismatch %d %d",(int32)*(Data + P->Offset), (int32)*(Defaults + P->Offset));
				bProblem = true;
			}
		}
		if (bProblem)
		{
			...
			if (!PrintedWarnings.Contains(Issue))
			{
				...
				if (PS)
				{
					UE_LOGF(LogClass, Warning,"%ls",*Issue);
//					OutputDebugStringW(*FString::Printf(TEXT("%s\n"),*Issue));
				}
				else
				{
					UE_LOGF(LogClass, Warning,"Native constructor does not initialize all properties %ls (may need to recompile excutable with new headers)",*Issue);
...
{
	UE_LOGF(LogClass, Verbose, "RemoveAllDeferredCppStructOps: %ls", *PackageName.ToString());
...
void UScriptStruct::PrepareCppStructOps()
{
	...
	if (!CppStructOps)
	{
		...
		if (!CppStructOps)
		{
			...
			{
				UE_LOGF(LogClass, Fatal, "Couldn't bind to native struct %ls. Headers need to be rebuilt, or a noexport class is missing a IMPLEMENT_STRUCT.",*GetName());
			}
	...
	else
	{
		...
		{
			UE_LOGF(LogClass, Verbose, "Native struct %ls has a native copy.",*GetName());
			StructFlags = EStructFlags(StructFlags | STRUCT_CopyNative);
		}
		if (CppStructOps->HasMoveAssign())
		{
			UE_LOGF(LogClass, Verbose, "Native struct %ls has a native move assign.",*GetName());
			StructFlags = EStructFlags(StructFlags | STRUCT_MoveAssignNative);

			// Move-only types are not yet supported. MoveAssignScriptStruct falls back to
			// CopyScriptStruct for code paths that haven't been updated, so WithCopy must
			// also be set to ensure correct behavior in those fallback paths.
			checkf(CppStructOps->HasCopy(), TEXT("Native struct %s has WithMoveAssign but not WithCopy. "
				"Move-only types are not yet supported; WithCopy must also be set."), *GetName());
		}
		if (!CppStructOps->HasDestructor())
		{
			UE_LOGF(LogClass, Verbose, "Native struct %ls has no destructor.",*GetName());
			StructFlags = EStructFlags(StructFlags | STRUCT_NoDestructor);
		}
	}
	if (CppStructOps->HasZeroConstructor())
	{
		UE_LOGF(LogClass, Verbose, "Native struct %ls has zero construction.",*GetName());
	...
	if (CppStructOps->IsPlainOldData() && !CppStructOps->HasZeroConstructor())
	{
		...
		if (IsZeroConstruct)
		{
			UE_LOGF(LogClass, Verbose, "Native struct %ls has DISCOVERED zero construction. Size = %d",*GetName(), Size);
			StructFlags = EStructFlags(StructFlags | STRUCT_ZeroConstructor);
		}
	}
	if (CppStructOps->HasIdentical())
	{
		UE_LOGF(LogClass, Verbose, "Native struct %ls has native identical.",*GetName());
		StructFlags = EStructFlags(StructFlags | STRUCT_IdenticalNative);
	}
	if (CppStructOps->HasAddStructReferencedObjects())
	{
		UE_LOGF(LogClass, Verbose, "Native struct %ls has native AddStructReferencedObjects.",*GetName());
		StructFlags = EStructFlags(StructFlags | STRUCT_AddStructReferencedObjects);
	}
	if (CppStructOps->HasExportTextItem())
	{
		UE_LOGF(LogClass, Verbose, "Native struct %ls has native ExportTextItem.",*GetName());
		StructFlags = EStructFlags(StructFlags | STRUCT_ExportTextItemNative);
	}
	if (CppStructOps->HasImportTextItem())
	{
		UE_LOGF(LogClass, Verbose, "Native struct %ls has native ImportTextItem.",*GetName());
	...
	{
		UE_LOGF(LogClass, Verbose, "Native struct %ls has native SerializeFromMismatchedTag.",*GetName());
		StructFlags = EStructFlags(StructFlags | STRUCT_SerializeFromMismatchedTag);
	}
#if WITH_EDITOR
	if (CppStructOps->HasCanEditChange())
	{
		UE_LOGF(LogClass, Verbose, "Native struct %ls has native CanEditChange.", *GetName());
		StructFlags = EStructFlags(StructFlags | STRUCT_CanEditChange);
	}
	if (CppStructOps->HasPostLoad())
	{
		UE_LOGF(LogClass, Verbose, "Native struct %ls has native PostLoad.", *GetName());
		StructFlags = EStructFlags(StructFlags | STRUCT_PostLoad);
	}
#endif

	if (CppStructOps->HasVisitor())
	{
		UE_LOGF(LogClass, Verbose, "Native struct %ls has native property Visit.", *GetName());
...
void UScriptStruct::SerializeItem(FArchive& Ar, TNotNull<void*> Value, void const* Defaults)
{
	SerializeItem(FStructuredArchiveFromArchive(Ar).GetSlot(), Value, Defaults);
}

void UScriptStruct::SerializeItem(FStructuredArchive::FSlot Slot, TNotNull<void*> Value, void const* Defaults)
{
	FArchive& UnderlyingArchive = Slot.GetUnderlyingArchive();
	...
	if (!bItemSerialized)
	{
		...
		{
			SerializeTaggedProperties(Slot, (uint8*)NotNullGet(Value), this, (uint8*)Defaults);
		}
	}
...
const TCHAR* UScriptStruct::ImportText(const TCHAR* InBuffer, TNotNull<void*> Value, UObject* OwnerObject, int32 PortFlags, FOutputDevice* ErrorText, const FString& StructName, bool bAllowNativeOverride) const
{
	return ImportText(InBuffer, Value, OwnerObject, PortFlags, ErrorText, [&StructName](){return StructName;}, bAllowNativeOverride);
}

const TCHAR* UScriptStruct::ImportText(const TCHAR* InBuffer, TNotNull<void*> Value, UObject* OwnerObject, int32 PortFlags, FOutputDevice* ErrorText, const TFunctionRef<FString()>& StructNameGetter, bool bAllowNativeOverride) const
{
...
void UScriptStruct::ExportText(FString& ValueStr, TNotNull<const void*> Value, const void* Defaults, UObject* OwnerObject, int32 PortFlags, UObject* ExportRootScope, bool bAllowNativeOverride) const
{
...
void UScriptStruct::Link(FArchive& Ar, bool bRelinkExistingProperties)
{
	...
	if (!HasDefaults()) // if you have CppStructOps, then that is authoritative, otherwise we look at the properties
	{
		...
		{
			UE_LOGF(LogClass, Verbose, "Non-Native struct %ls is plain old data.",*GetName());
		}
		if (StructFlags & STRUCT_NoDestructor)
		{
			UE_LOGF(LogClass, Verbose, "Non-Native struct %ls has no destructor.",*GetName());
		}
		if (StructFlags & STRUCT_ZeroConstructor)
		{
			UE_LOGF(LogClass, Verbose, "Non-Native struct %ls has zero construction.",*GetName());
		}
	}
}
...
void UScriptStruct::MoveAssignScriptStruct(void* InDest, void* InSrc, int32 ArrayDim) const
{
	uint8* Dest = (uint8*)InDest;
	check(Dest);
	...
	if (StructFlags & STRUCT_MoveAssignNative)
	{
		check(!(StructFlags & STRUCT_IsPlainOldData)); // should not have both
		UScriptStruct::ICppStructOps* TheCppStructOps = GetCppStructOps();
		check(TheCppStructOps);
		check(Stride == TheCppStructOps->GetSize() && PropertiesSize == Stride);
		if (TheCppStructOps->MoveAssign(Dest, Src, ArrayDim))
		{
			return;
		}
	}

	// Fallback: use copy. For POD this is memcpy; for non-POD this is copy assignment.
	// Source is not modified — caller's destructor handles cleanup as before.
	CopyScriptStruct(Dest, Src, ArrayDim);
}
...
void UScriptStruct::DestroyStruct(void* Dest, int32 ArrayDim) const
{
	...
	if (TheCppStructOps)
	{
		...
		ClearedSize = TheCppStructOps->GetSize();
		// here we want to make sure C++ and the property system agree on the size
		checkf(Stride == ClearedSize && PropertiesSize <= ClearedSize, TEXT("C++ and the property system struct size mismatch for %s (C++ Size: %d, Aligned Property Size: %d, Unaligned Property Size: %d)"),
			*GetPathName(), ClearedSize, Stride, PropertiesSize);
	}
		...
		if (ScriptStruct->IsNative())
		{
			...
			else
			{
				...
				if (StructOps)
				{
					...
					{
						UE_LOGF(LogClass, Warning, "Struct %ls%ls has Cpp alloc size = %d > ScriptStruct->GetStructureSize() = %d, this could result in allocations that are too small to fit the structure.",
...
static void FindUninitializedScriptStructMembers(UScriptStruct* ScriptStruct, EScriptStructTestCtorSyntax ConstructorSyntax, TSet<const FProperty*>& OutUninitializedProperties)
{
	...
	{
#if	WITH_EDITORONLY_DATA
		static const FName NAME_IgnoreForMemberInitializationTest("IgnoreForMemberInitializationTest");
		...
		else
		{
			...
			if (bShouldInspect)
			{
				...
				// UE_LOGF(LogClass, Log, "Testing %ls%ls::%ls for proper initialization", ScriptStruct->GetPrefixCPP(), *ScriptStruct->GetName(), *Property->GetNameCPP());
...
#if WITH_EDITORONLY_DATA
	static const FName NAME_ModuleRelativePath("ModuleRelativePath");
...
int32 FStructUtils::AttemptToFindUninitializedScriptStructMembers()
{
	...
	FExecuteIfExceedsTimeLimit TotalTimeLimit(2.0, [](double Time) {
		UE_LOGF(LogClass, Display, "AttemptToFindUninitializedScriptStructMembers took more than 2s to complete. Time: %.2f", Time);
	});
	...
	struct FScriptStructSettings
	{
		...
		FScriptStructSettings()
		{
			FExecuteIfExceedsTimeLimit SettingsTimeLimit(0.05, [](double Time) {
				UE_LOGF(LogClass, Display, "AttemptToFindUninitializedScriptStructMembers took more than 50ms to construct the FScriptStructSettings. Time: %.1f", Time * 1000);
				});
			{
...

	{
		...
		if (*RawValue != BadPointer)
		{
			UE_LOGF(LogClass, Error, "ObjectProperty %ls%ls::%ls seems to be initialized properly but it shouldn't be. Verify that AttemptToFindUninitializedScriptStructMembers() is working properly",
		...
		if (*RawValue != nullptr)
		{
			UE_LOGF(LogClass, Error, "ObjectProperty %ls%ls::%ls seems to be not initialized properly but it should be. Verify that AttemptToFindUninitializedScriptStructMembers() is working properly",
	...
	{
		UE_LOGF(LogClass, Display, "AttemptToFindUninitializedScriptStructMembers took more than 50ms before starting to check ScriptStructs. Time(ms):%.1f", PreScriptStructTime * 1000);
	}
	...
	for (TObjectIterator<UScriptStruct> ScriptIt; ScriptIt; ++ScriptIt)
	{
		...
		FExecuteIfExceedsTimeLimit StructTimeLimit(0.001, [ScriptStruct](double Time) {
			UE_LOGF(LogClass, Display, "AttemptToFindUninitializedScriptStructMembers took more than 1ms to process ScriptStruct %ls. Time(ms): %.1f", ScriptStruct ? *ScriptStruct->GetPathName() : TEXT("None"), Time * 1000);
			});
	...
	{
		UE_LOGF(LogClass, Display, "%i Uninitialized script struct members found including %d object properties", UninitializedScriptStructMemberCount, UninitializedObjectPropertyCount);
	}
...
static bool CheckIfMetaDataValueIsShortTypeName(FName MetaDataKey, const FString& MetaDataValue, FString& OutParsedMetaDataValue)
{
	...
	static const FName NAME_RequiredAssetDataTags("RequiredAssetDataTags");
...
static FString GetSuggestedPathNameForTypeShortName(const FString& ShortName)
{
	...
	{
		return FString("No valid type found in memory.");
	}
}
...
int32 FStructUtils::AttemptToFindShortTypeNamesInMetaData()
{
	...
	{
		UE_LOGF(LogClass, Display, "%i short type names in reflected types' MetaData", ShortTypeNamesCount);
	}
...
static bool FindUndeclaredObjectReferencesInStructSerializers(FAutomationTestBase& Test, bool bExactCheck)
{
	...
	for (TObjectIterator<UScriptStruct> ScriptIt; ScriptIt; ++ScriptIt)
	{
		...
		if(bExactCheck && EnumHasAllFlags(DeclaredReferences, EPropertyObjectReferenceType::Conservative))
		{
			...
			for (EPropertyObjectReferenceType TypedFlag : MakeFlagsRange(Ar.References))
			{
...
void UClass::Bind()
{
	...
	{
		UE_LOGF(LogClass, Fatal, "Can't bind to native class %ls", *GetPathName() );
	}
	...
	if( !ClassConstructor )
	{
		UE_LOGF(LogClass, Fatal, "Can't find ClassConstructor for class %ls", *GetPathName() );
	}
}
...
namespace UE::CoreUObject::Private::AssetRegistryTags
{
#if WITH_EDITOR
	static const FName ParentClassName        = "ParentClass";
	static const FName ModuleNameName         = "ModuleName";
	static const FName ModuleRelativePathName = "ModuleRelativePath";
#endif
}
...
void UClass::GetAssetRegistryTags(FAssetRegistryTagsContext Context) const
{
	...
	Context.AddTag( FAssetRegistryTag(AssetRegistryTags::ParentClassName, ((ParentClass) ? ParentClass->GetPathName() : FString()), FAssetRegistryTag::TT_Alphabetical) );

	const UPackage* const ClassPackage = GetOuterUPackage();
	Context.AddTag( FAssetRegistryTag(AssetRegistryTags::ModuleNameName, ((ClassPackage) ? FPackageName::GetShortFName(ClassPackage->GetFName()) : NAME_None).ToString(), FAssetRegistryTag::TT_Alphabetical) );

	const FString& ClassModuleRelativeIncludePath = GetMetaData(AssetRegistryTags::ModuleRelativePathName);
	Context.AddTag( FAssetRegistryTag(AssetRegistryTags::ModuleRelativePathName, ClassModuleRelativeIncludePath, FAssetRegistryTag::TT_Alphabetical) );
#endif
}

#if WITH_EDITOR

void UClass::GetAssetRegistryTagMetadata(TMap<FName, FAssetRegistryTagMetadata>& OutMetadata) const
{
	using namespace UE::CoreUObject::Private;
	
	Super::GetAssetRegistryTagMetadata(OutMetadata);

	OutMetadata.Emplace(AssetRegistryTags::ParentClassName, FAssetRegistryTagMetadata()
		.SetDisplayName(NSLOCTEXT("UClass", "ParentClass", "Parent Class"))
		.SetTooltip(NSLOCTEXT("UClass", "ParentClassTooltip", "The parent class of this type."))
	);

	OutMetadata.Emplace(AssetRegistryTags::ModuleNameName, FAssetRegistryTagMetadata()
		.SetDisplayName(NSLOCTEXT("UClass", "ModuleName", "Module Name"))
		.SetTooltip(NSLOCTEXT("UClass", "ModuleNameTooltip", "The module name of this type."))
	);

	OutMetadata.Emplace(AssetRegistryTags::ModuleRelativePathName, FAssetRegistryTagMetadata()
		.SetDisplayName(NSLOCTEXT("UClass", "ModuleRelativePath", "Relative Module Path"))
		.SetTooltip(NSLOCTEXT("UClass", "ModuleRelativePathTooltip", "The relative module path of this type."))
	);
}
...
EStructPropertyLinkFlags UClass::GetPropertyLinkFlags(UStruct* ContainerStruct, FProperty* Property) const
{
	...
	if (!ContainerStruct->Script.IsEmpty() || !HasAnyClassFlags(CLASS_Native | CLASS_Intrinsic) || Property->HasAnyPropertyFlags(CPF_ForcePostConstructLink))
	{
			...
			(auto CheckStructRecursive, const FProperty* InProp, ECheckStructType CheckType)
				-> void
				{
					auto BadStruct = [&BaseProp](const FStructProperty* StructProp)
						{
							UE_LOGF(LogClass, Warning, "Property %ls contained nested NetDeltaSerialize struct '%ls', "
								"when this is not supported. Only use NetDeltaSerialize structs at class level. "
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	...
	if (InClassDefaultObject != ClassDefaultObject && ImmutableDefaultObject)
	{
		...
		ImmutableDefaultObject->Rename(*MakeUniqueObjectName(GetTransientPackage(), this).ToString(), GetTransientPackage(), REN_DoNotDirty | REN_NonTransactional | REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
...
void UClass::Serialize( FArchive& Ar )
{
	...
	if( Ar.IsLoading() )
	{
		...
		{
			UE_LOGF(LogClass, Log, "CDO was changed while class serialization.\n\tOld: '%ls'\n\tSerialized: '%ls'\n\tActual: '%ls'"
...
#endif	// USE_CIRCULAR_DEPENDENCY_LOAD_DEFERRING
				{
					UE_LOGF(LogClass, Error, "CDO for class %ls did not load!", *GetPathName());
...
void* UClass::CreateSparseClassData()
{
	...
	if (SparseClassData)
	{
		...
		if (SparseArchetypeData)
		{
			...
			{
				UE_LOGF(LogClass, Warning, "SparseClassData %ls for class %ls archetype is of unrelated type %ls",
	...
	)
{
	...
	*/
	...
	UE_LOGF(LogClass, Verbose, "Attempting to change VTable for class %ls.",*GetName());
	...
	{
		UE_LOGF(LogClass, Warning, "Reload:  Was not expecting temporary object '%ls' for class '%ls' to become rooted during construction.  This object cannot be marked pending kill.", *TempObjectForVTable->GetFName().ToString(), *this->GetName() );
	}
	...
	if (NewVTable != OldVTable)
	{
		...
		UE_LOGF(LogClass, Verbose, "Updated the vtable for %d live objects and %d blueprint classes.  %016llx -> %016llx", Count, CountClass, PTRINT(OldVTable), PTRINT(NewVTable));
	}
	else
	{
		UE_LOGF(LogClass, Error, "VTable for class %ls did not change?",*GetName());
	}
...
void UClass::AddNativeFunction(const ANSICHAR* InName, FNativeFuncPtr InPointer)
{
	...
	if (IsReloadActive())
	{
		...
		else
		{
			...
			UE_LOGF(LogClass, Log, "Function %ls is new or belongs to a modified class.", *InFName.ToString());
		}
	}
#endif
...
void UClass::CreateLinkAndAddChildFunctionsToMap(const FClassFunctionLinkInfo* Functions, uint32 NumFunctions)
{
	for (; NumFunctions; --NumFunctions, ++Functions)
	{
		...
		UFunction*  Func         = Functions->CreateFuncPtr(ETypeConstructPhase::Outer);
...
const FString UClass::GetConfigName() const
{
	...
	{
		UE_LOGF(LogClass, Fatal,"UObject::GetConfigName() called on class with config name 'None'. Class flags = 0x%08X", (uint32)ClassFlags );
		return TEXT("");
	}
	else
	{
...
UField* UClass::TryFindTypeSlow(UClass* TypeClass, FStringView InPathNameOrShortName, EFindFirstObjectOptions InOptions)
{
	...
	if (!InPathNameOrShortName.IsEmpty() && InPathNameOrShortName != TEXT("None"))
	{
		...
		else
		{
			...
			UE_LOGF(LogClass, Warning, "Short type name \"%.*ls\" provided for TryFindType. Please convert it to a path name (suggested: \"%ls\"). Callstack:\r\n\r\n%ls", InPathNameOrShortName.Len(), InPathNameOrShortName.GetData(), *GetPathNameSafe(FoundType), Callstack.ToString());
		}
	}
	return FoundType;
}
...
{
	static const FName NAME_HideFunctions("HideFunctions");
...
{
	static const FName NAME_AutoExpandCategories("AutoExpandCategories");
...
{
	static const FName NAME_AutoCollapseCategories("AutoCollapseCategories");
...
{
	static const FName NAME_ClassGroupNames("ClassGroupNames");
...
#if WITH_EDITORONLY_DATA
void UClass::CallAppendToClassSchema(FAppendToClassSchemaContext& Context) const
{
	// Call the C++ Append function for the class and its superclasses
	check(CppClassStaticFunctions.GetAppendToClassSchema());
	CppClassStaticFunctions.GetAppendToClassSchema()(Context);

	// Add on the ini append values for the class and its superclasses.
	FString AppendText;
	const UClass* SuperClassIterator = this;
	check(GConfig);
	while (SuperClassIterator != nullptr)
	{
		if (GConfig->GetString(TEXT("AppendToClassSchema"), *SuperClassIterator->GetPathName(),
			AppendText, GEditorIni))
		{
			AppendText.TrimStartAndEndInline();
			if (!AppendText.IsEmpty())
			{
				Context.Update(*AppendText, sizeof(**AppendText) * AppendText.Len());
			}
		}
...
{
#if WITH_EDITOR
	const static FName NAME_UFunction("UFunction");
...
void UFunction::Bind()
{
	...
	else
	{
		...
		{
			UE_LOGF(LogClass, Warning,"Failed to bind native function %ls.%ls",*OwnerClass->GetName(),*GetName());
		}
#endif
#if UE_WITH_CONSTINIT_UOBJECT
...
static UScriptStruct* StaticGetBaseStructureInternal(FName Name)
{
	...
	if (!Result)
	{
		UE_LOGF(LogClass, Fatal, "Failed to find native struct '%ls.%ls'", *CoreUObjectPkg->GetName(), *Name.ToString());
	}
#endif
	return Result;
}
...
UScriptStruct* TBaseStructure<FDoubleInterval>::Get()
{
	static UScriptStruct* ScriptStruct = StaticGetBaseStructureInternal(TEXT("DoubleInterval"));
	return ScriptStruct;
}
...
UScriptStruct* TBaseStructure<FPackedRemoteObjectHeader>::Get()
{
	static UScriptStruct* ScriptStruct = StaticGetBaseStructureInternal(TEXT("PackedRemoteObjectHeader"));
	return ScriptStruct;
}
...
{
	static UScriptStruct* ScriptStruct = StaticGetBaseStructureInternal(TEXT("RemoteObjectReference"));
	return ScriptStruct;
}
