...
FByteProperty::FByteProperty(FFieldVariant InOwner, const FName& InName)
...
{
	this->Enum = Prop.EnumFunc ? Prop.EnumFunc(ETypeConstructPhase::Outer) : nullptr;
}
...
void FByteProperty::SerializeItem(FStructuredArchive::FSlot Slot, TNotNull<void*> Value, void const* Defaults) const
{
	FArchive& UnderlyingArchive = Slot.GetUnderlyingArchive();

	uint8* ByteValuePtr = (uint8*)NotNullGet(Value);

	if(Enum && UnderlyingArchive.UseToResolveEnumerators())
	{
		Slot.EnterStream();
		const int64 ResolvedIndex = Enum->ResolveEnumerator(UnderlyingArchive, *ByteValuePtr);
		*ByteValuePtr = static_cast<uint8>(ResolvedIndex);
		return;
	}
	...
	{
		Super::SerializeItem(Slot, ByteValuePtr, Defaults);
	}
	// Loading
	else if (UnderlyingArchive.IsLoading())
	{
		...
		*ByteValuePtr = IntCastChecked<uint8>(EnumValue);
	}
	// Saving
	else
	{
		FName EnumValueName;
		uint8 ByteValue = *ByteValuePtr;
...
bool FByteProperty::NetSerializeItem(FArchive& Ar, UPackageMap* Map, TNotNull<void*> Data, TArray<uint8>* MetaData) const
{
...
void FByteProperty::ExportText_Internal(FString& ValueStr, TNotNull<const void*> PropertyValueOrContainer, EPropertyPointerType PropertyPointerType, const void* DefaultValue, UObject* Parent, int32 PortFlags, UObject* ExportRootScope) const
{
...
const TCHAR* FByteProperty::ImportText_Internal(const TCHAR* InBuffer, TNotNull<void*> ContainerOrPropertyPtr, EPropertyPointerType PropertyPointerType, UObject* Parent, int32 PortFlags, FOutputDevice* ErrorText) const
{
	...
	if (Temp == TEXT("True") || Temp == *(CoreTexts.True.ToString()))
	{
		...
		if (Enum)
		{
			UE_LOGF(LogClass, Warning, "ConsoleVariable-Backed Enum Property of type '%ls' was set from a string. Please update the cvar in your ini files.", *Enum->GetPathName());
		}
