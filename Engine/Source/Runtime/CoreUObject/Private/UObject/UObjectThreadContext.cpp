...
FObjectInitializer& FUObjectThreadContext::ReportNull()
{
	...
	UE_CLOGF(!ObjectInitializerPtr, LogUObjectThreadContext, Fatal, "Tried to get the current ObjectInitializer, but none is set. Please use NewObject to construct new UObject-derived classes.");
...
FUObjectSerializeContext::FUObjectSerializeContext()
	...
	, bIsLoadingToUnknownPropertyTree(false)
#endif
{
}
...
#endif
{
	...
	SavedSerializedPropertyPath = SerializeContext->SerializedPropertyPath;
...
FScopedObjectSerializeContext::~FScopedObjectSerializeContext()
{
	...
	SerializeContext->SerializedPropertyPath = SavedSerializedPropertyPath;
#endif
}

FScopedObjectSerializeContext::FScopedObjectSerializeContext(UObject*const InObject, FArchive& InArchive, const FPropertyPathName& PathBeingSerialized)
	: FScopedObjectSerializeContext(InObject, InArchive)
{
#if WITH_EDITORONLY_DATA
	const bool bIsLoading = Archive.IsLoading();
	FUObjectSerializeContext* SerializeContext = FUObjectThreadContext::Get().GetSerializeContext();
	SerializeContext->bTrackSerializedPropertyPath = bIsLoading;
	SerializeContext->bTrackUnknownProperties = bIsLoading;
	SerializeContext->bTrackUnknownEnumNames = bIsLoading;
	SerializeContext->SerializedPropertyPath = PathBeingSerialized;
#endif // WITH_EDITORONLY_DATA
}
