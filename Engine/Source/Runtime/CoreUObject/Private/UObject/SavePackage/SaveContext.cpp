	...
	bool bEmitWarning)
{
	...
	if (bEmitWarning && ObjectStatus.SaveableStatus != ESaveableStatus::Success)
	{
			...
			&& InObject.GetPackage() == GetPackage())
		{
			...
			UE_LOGF(LogSavePackage, Warning, "%ls has unsaveable outer %ls (outer is %ls), so it will not be saved.",
	...
	static bool bAssetListGenerated = [](TArray<TSoftClassPtr<UObject>>& OutAssetList)
		{
			...
			for (const FString& AssetType : AssetList)
			{
				...
				{
					UE_LOGF(LogSavePackage, Warning, "The asset type '%ls' found while building the allowlist for automatic optional data inclusion list is not a valid class path.", *AssetType);
				}
			}
			return true;
