				...
				for (UObject* ObjTemplate : ObjectTemplates)
				{
					...
					else
					{
						...
						UE_LOGF(LogSavePackage, Warning,
							"Invalid subobject on a CDO; we will skip importing it. Found when saving package %ls which imported the CDO containing subobject %ls.",
...
{
	// Searchable names are only ever saved into the editor scope, so don't harvest the type object for game saves
	FHarvestScope EditorOnlyScope = EnterConditionalEditorOnlyScope(true);
...
void FPackageHarvester::HarvestImportTypeHierarchyNames(const UObject* Import)
{
	...
	if (Struct)
	{
		// Harvest names for types that can be used to create placeholders with, or types from an export with RF_HasDynamicImports
		// See comment in SavePackage2.cpp.
		bool bRequiresNames = UE::CanCreatePropertyBagPlaceholderForType(Struct->GetClass(), Struct) ||
			(CurrentExportDependencies.CurrentExport && CurrentExportDependencies.CurrentExport->HasAnyFlags(RF_HasDynamicImports));
		if (bRequiresNames)
		{
