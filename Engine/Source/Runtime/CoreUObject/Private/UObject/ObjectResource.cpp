...
void operator<<(FStructuredArchive::FSlot Slot, FObjectImport& I)
{
	...
	if (Slot.GetUnderlyingArchive().UEVer() >= VER_UE4_NON_OUTER_PACKAGE_IMPORT)
	{
		if (Slot.GetUnderlyingArchive().IsSaving())
		{
			FName SavePackageName = I.PackageName;
			if (Slot.GetUnderlyingArchive().IsFilterEditorOnly() && SavePackageName.IsNone())
			{
				// Hack to avoid collecting the "None" name in cooked packages that does not use the external package feature.
				// This minimizes the patch size impact, else the PackageName addition would impact the majority of cooked packages.
				// Serialize an existing name table entry instead, and reset it on load for the legacy cooked loading path.
				// Note that the PackageStoreOptimizer strips this data anyway, so it will never be loaded in the iostore loading path.
				ensureMsgf(I.ObjectName.IsNone() || I.ObjectName != I.PackageName,
					TEXT("Unexpected import names in archive %s: ObjectName=%s, PackageName=%s"),
					*Slot.GetUnderlyingArchive().GetArchiveName(), *I.ObjectName.ToString(), *I.PackageName.ToString());
				SavePackageName = I.ObjectName;
			}
			Record << SA_VALUE(TEXT("PackageName"), SavePackageName);
		}
		else
		{
	...
	if (Slot.GetUnderlyingArchive().IsLoading())
	{
		...
		I.XObject = NULL;
		if (Slot.GetUnderlyingArchive().IsFilterEditorOnly() && I.PackageName == I.ObjectName)
		{
			// Reset the data from the above save hack to avoid collecting the "None" name in cooked packages that
			// does not use the external package feature.
			I.PackageName = FName{};
		}
	}
}
