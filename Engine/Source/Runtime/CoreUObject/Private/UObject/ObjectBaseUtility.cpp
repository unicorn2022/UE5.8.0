...
#define UE_GARBAGE_ELIMINATION_ALLOWED !UE_USE_COMPACT_SET_AS_DEFAULT || UE_ENABLE_COMPACTHASHTABLE_KEYSTORAGE
...
int32 GGarbageEliminationEnabled = UE_GARBAGE_ELIMINATION_ALLOWED;
...
{
	check(GConfig);
	bool bGarbageEliminationEnabled = UObjectBaseUtility::IsGarbageEliminationEnabled();
	if (GConfig->GetBool(TEXT("/Script/Engine.GarbageCollectionSettings"), TEXT("gc.PendingKillEnabled"), bGarbageEliminationEnabled, GEngineIni))
	{
		...
		UE_LOGF(LogObj, Warning, "Deprecated ini setting [/Script/Engine.GarbageCollectionSettings] gc.PendingKillEnabled=%ls found. Please rename it to gc.GarbageEliminationEnabled",
	...
	UObjectBaseUtility::SetGarbageEliminationEnabled(bGarbageEliminationEnabled);
}

void UObjectBaseUtility::SetGarbageEliminationEnabled(bool bEnabled)
{
#if UE_GARBAGE_ELIMINATION_ALLOWED
	// Try to sync even though we're not gonna use the console var
	UObjectBaseUtility::bGarbageEliminationEnabled = bEnabled;
	GGarbageEliminationEnabled = bEnabled;
#else
	if (bEnabled)
	{
		// Garbage elimination is not compatible when compact sets are default since it can modify key data which invalidates
		// the lookup table used to manage the set which can lead to crashes or infinite loops on subsequent operations.
		UE_LOGF(LogObj, Warning, "Request made to enable Garbage Elimination, however this feature is not currently compatible with compact sets. Leaving it disabled.");
	}

	UObjectBaseUtility::bGarbageEliminationEnabled = false;
	GGarbageEliminationEnabled = 0;
#endif
}