...
FORCENOINLINE UObject* LoadRemotePathName(const PathType& InPathName, const FRemoteObjectTables& InTables, int32 InNameIndex)
{
	...
	if (!Object)
	{
		if (InNameIndex == 0)
		{
			UE_LOGF(LogRemoteObject, Warning, "Failed to load asset object %ls (%ls)", *LoadPathName, *InPathName.GetSegmentId(InNameIndex, InTables).ToString());
		}
		else
		{
			UE_LOGF(LogRemoteObject, Warning, "Failed to load asset object %ls which is an outer of remote object %ls (%ls)",
				*LoadPathName,
