...
UObject* UObjectBaseUtility::GetOutermostObject() const
{
	...
	for (;;)
	{
		...
		if (Cast<UPackage>(CurrentOuter) != nullptr)
		{
			return Top;
		}
		Top = CurrentOuter;
...
bool UObjectBaseUtility::MarkPackageDirty() const
{
	...
	if( Package )
	{
		...
		// we explicitly disable the ability to dirty a package or map during load/undo/redo.  Non-cook commandlets can still
...
{
	if (const UPackage* OuterPackage = Cast<UPackage>(SomeOuter))
	{
		return IsInPackage(OuterPackage);
	}
...
{	
	...
	static const FString NativeClassPrefix("/Script/");
	static const FString BPClassPrefix("/Game/");
	...
	for (const TPair<FName, TSet<FString>>& Pair : ChangeTracker->ModifiedSectionsPerBranch)
	{
		...
		{
			UE_LOGF(LogConfig, Verbose, "Found modified section: %ls", *SectionName);
			...
			if (PerObjConfigDelimIdx != INDEX_NONE)
			{
				...
				UClass* ObjClass = (UClass*)StaticFindFirstObject(UClass::StaticClass(), *ClassName, EFindFirstObjectOptions::EnsureIfAmbiguous | EFindFirstObjectOptions::NativeFirst, ELogVerbosity::Error, TEXT("UObjectBaseUtility::ReloadObjectsFromModifiedConfigSections"));
				...
				{
					UE_LOGF(LogCore, Warning, "[%ls]: Couldn't find PerObjectConfig class %ls for %ls, config changes won't be reloaded.", *Pair.Key.ToString(), *ClassName, *ObjectName);
				}
			}
			...
			else
			{
				...
				// if we are unloading, neverf call LoadObject as it can cause problems to load as we unload - likely we never need to call LoadObject, but minimizing side effects
				if (SectionName.StartsWith(NativeClassPrefix) || (ChangeTracker->bIsUnloadingPlugin && SectionName.StartsWith(BPClassPrefix)))
				{
	...
	{
		UE_LOGF(LogConfig, Verbose, "Reloading %ls", *ReloadObject->GetPathName());
...
//		ReloadObject->ReloadConfig();
	...
	UE_LOGF(LogCore, Log, "Updating config from took %f seconds and reloaded %d objects",
...
void FScopeCycleCounterUObject::ReportHitch()
{
	...
	if (CurrentThreadId == GGameThreadId)
	{
		...
		{
			StackString = FString("[UObject was invalid]");
		}
	}
	else
	{
		StackString = FString("[Not grabbing UObject name from other threads]");
	}
	UE_LOGF(LogCore, Error, "Leaving UObject scope on hitch (+%8.2fms) [%ls] %ls", Delta, *ThreadString, *StackString);
}
#endif
