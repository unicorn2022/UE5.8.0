...
FStructProperty::FStructProperty(FFieldVariant InOwner, const FName& InName)
...
static EPropertyFlags GetStructComputedPropertyFlags(const UECodeGen_Private::FStructPropertyParams& Prop)
{
	...
	UScriptStruct* Struct = Prop.ScriptStructFunc ? Prop.ScriptStructFunc(ETypeConstructPhase::Outer) : nullptr;
...
{
	Struct = Prop.ScriptStructFunc ? Prop.ScriptStructFunc(ETypeConstructPhase::Outer) : nullptr;
}
...
void FStructProperty::LinkInternal(FArchive& Ar)
{
	...
	{
		UE_LOGF(LogProperty, Error, "Struct type unknown for property '%ls'; perhaps the USTRUCT() was renamed or deleted?", *GetFullName());
...
bool FStructProperty::Identical(TNotNull<const void*> A, const void* B, uint32 PortFlags) const
{
...
bool FStructProperty::NetSerializeItem(FArchive& Ar, UPackageMap* Map, TNotNull<void*> Data, TArray<uint8> * MetaData) const
{
	//------------------------------------------------
	...
	if (Struct->StructFlags & STRUCT_NetSerializeNative)
	{
		...
		if (!bSuccess)
		{
			UE_LOGF(LogProperty, Warning, "Native NetSerialize %ls (%ls) failed.", *GetFullName(), *Struct->GetFullName() );
		}
		return bMapped;
	}
...
void FStructProperty::Serialize( FArchive& Ar )
{
	...
	if (Ar.IsPersistent() && Ar.GetLinker())
	{
		...
		{
			UE_LOGF(LogProperty, Error, "FStructProperty::Serialize Loading: Property '%ls'. Unknown structure.", *GetFullName());
			Struct = FallbackStruct;
		}
		else if ((FallbackStruct == Struct) && Ar.IsSaving())
		{
			UE_LOGF(LogProperty, Error, "FStructProperty::Serialize Saving: Property '%ls'. FallbackStruct structure.", *GetFullName());
		}
	}
#endif // WITH_EDITOR
	if (Struct)
...
void FStructProperty::ExportText_Internal(FString& ValueStr, TNotNull<const void*> PropertyValueOrContainer, EPropertyPointerType PropertyPointerType, const void* DefaultValue, UObject* Parent, int32 PortFlags, UObject* ExportRootScope) const
{
...
const TCHAR* FStructProperty::ImportText_Internal(const TCHAR* InBuffer, TNotNull<void*> ContainerOrPropertyPtr, EPropertyPointerType PropertyPointerType, UObject* Parent, int32 PortFlags, FOutputDevice* ErrorText) const
{
	...
	if (Struct)
	{
		...
		if ((Struct->StructFlags & STRUCT_SerializeFromMismatchedTag) && !bCanSerialize)
		{
			...
			else
			{
				...
				{
					UE_LOGF(LogClass, Warning, "SerializeFromMismatchedTag failed: Type mismatch in %ls - Previous (%ls) Current(%ls) in package: %ls",
		...
		{
			UE_LOGF(LogClass, Warning, "Struct Property %ls has a struct type mismatch (tag %ls != prop %ls) in package: %ls. If that struct got renamed, add an entry to ActiveStructRedirects.",
