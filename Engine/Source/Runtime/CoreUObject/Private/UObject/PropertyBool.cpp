...
FBoolProperty::FBoolProperty(FFieldVariant InOwner, const FName& InName)
...
int32 FBoolProperty::GetMinAlignment() const
{
	...
	default:
		UE_LOGF(LogProperty, Fatal, "Unsupported FBoolProperty %ls size %d.", *GetName(), (int32)GetElementSize());
	}
	return Alignment;
}
void FBoolProperty::LinkInternal(FArchive& Ar)
{
	...
	// Can't call FField::IsNative here because sometimes bool props don't have an owner when linked in the python script plugin
	if (UObject* Obj = GetOwnerUObject(); Obj && Obj->IsNative())
	{
...
FString FBoolProperty::GetCPPType( FString* ExtendedTypeText/*=NULL*/, uint32 CPPExportFlags/*=0*/ ) const
{
	...
	else
	{
		...
		case sizeof(uint8):
			return TEXT("uint8");
		default:
			UE_LOGF(LogProperty, Fatal, "Unsupported FBoolProperty %ls size %d.", *GetName(), GetElementSize());
			break;
		}
	}
	return TEXT("uint32");
...
void FBoolProperty::ExportText_Internal(FString& ValueStr, TNotNull<const void*> ContainerOrPropertyPtr, EPropertyPointerType PropertyPointerType, const void* DefaultValue, UObject* Parent, int32 PortFlags, UObject* ExportRootScope) const
{
	check(FieldSize != 0);
...
const TCHAR* FBoolProperty::ImportText_Internal(const TCHAR* Buffer, TNotNull<void*> ContainerOrPropertyPtr, EPropertyPointerType PropertyPointerType, UObject* Parent, int32 PortFlags, FOutputDevice* ErrorText) const
{
	FString Temp; 
	...
	{
		//UE_LOGF(LogProperty, Log,  "Import: Failed to get bool" );
		return NULL;
	}
...
bool FBoolProperty::Identical(TNotNull<const void*> A, const void* B, uint32 PortFlags) const
{
	check(FieldSize != 0);
	const uint8* ByteValueA = (const uint8*)NotNullGet(A) + ByteOffset;
	const uint8* ByteValueB = (const uint8*)B + ByteOffset;
	return ((*ByteValueA ^ (B ? *ByteValueB : 0)) & FieldMask) == 0;
}

void FBoolProperty::SerializeItem(FStructuredArchive::FSlot Slot, TNotNull<void*> Value, void const* Defaults) const
{
	check(FieldSize != 0);
	uint8* ByteValue = (uint8*)NotNullGet(Value) + ByteOffset;
	uint8 B = (*ByteValue & FieldMask) ? 1 : 0;
	Slot << B;
	*ByteValue = ((*ByteValue) & ~FieldMask) | (B ? ByteMask : 0);
}

bool FBoolProperty::NetSerializeItem(FArchive& Ar, UPackageMap* Map, TNotNull<void*> Data, TArray<uint8> * MetaData) const
{
	check(FieldSize != 0);
	uint8* ByteValue = (uint8*)NotNullGet(Data) + ByteOffset;
