...
static TArray<TMap<FGuid, FGuid>> PIEGuidMap;
static FTransactionallySafeMutex PIEGuidMapLock;
...
static FGuid RemapGuid(const FUniqueObjectGuid& Guid, int32 PIEInstanceID)
{
	...
	UE::TScopeLock Lock(PIEGuidMapLock);

	if (PIEInstanceID >= PIEGuidMap.Num())
	{
		PIEGuidMap.AddDefaulted(PIEInstanceID + 1 - PIEGuidMap.Num());
	}
					...
					if (bDuplicate || bReassigning)
					{
						...
						else
						{
							if (!bReassigning)
							{
								...
								UE_CLOGF(!bIsMapOrMapData || (!bInGame && !Package->HasAnyPackageFlags(PKG_EditorOnly)), LogUObjectGlobals, Warning,
									"Guid referenced by %ls is already used by %ls, which should never happen in the editor but could happen at runtime with duplicate level loading or PIE",
									*Object->GetFullName(), *OtherObject->GetFullName());
							}
							else
							{
								UE_LOGF(LogUObjectGlobals, Warning, "Assigning new Guid to %ls", *Object->GetFullName());
							}
...
void FLazyObjectPtr::ResetPIEFixups()
{
	...
	UE::TScopeLock Lock(PIEGuidMapLock);
	if (PIEGuidMap.IsValidIndex(PlayInEditorID))
	{
