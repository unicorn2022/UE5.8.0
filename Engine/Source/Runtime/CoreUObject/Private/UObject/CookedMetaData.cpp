...
void PrepareCookedMetaDataForPurge(UObject* CookedMetaDataPtr)
{
	...
	if (!CookedMetaDataPtr->GetPackage()->HasAnyPackageFlags(PKG_Cooked))
	{
		...
		CookedMetaDataPtr->Rename(FNameBuilder(MakeUniqueObjectName(CookedMetaDataPtr->GetOuter(), CookedMetaDataPtr->GetClass(), FName(BaseMetaDataName))).ToString(), nullptr, REN_DoNotDirty | REN_DontCreateRedirectors | REN_NonTransactional | REN_AllowPackageLinkerMismatch);
	}
...
void PreloadCookedMetaData(CookedMetaDataType* CookedMetaDataPtr)
{
#if WITH_METADATA
...
void UEnumCookedMetaData::Serialize(FArchive& Ar)
{
	Super::Serialize(Ar);

	if (Ar.IsLoading())
	{
		CookedMetaDataUtil::Internal::PreloadCookedMetaData<UEnum, UEnumCookedMetaData>(this);
	}
}
...
void UStructCookedMetaData::Serialize(FArchive& Ar)
{
	Super::Serialize(Ar);

	if (Ar.IsLoading())
	{
		CookedMetaDataUtil::Internal::PreloadCookedMetaData<UScriptStruct, UStructCookedMetaData>(this);
	}
}
...
void UClassCookedMetaData::Serialize(FArchive& Ar)
{
	Super::Serialize(Ar);

	if (Ar.IsLoading())
	{
		CookedMetaDataUtil::Internal::PreloadCookedMetaData<UClass, UClassCookedMetaData>(this);
	}
}
