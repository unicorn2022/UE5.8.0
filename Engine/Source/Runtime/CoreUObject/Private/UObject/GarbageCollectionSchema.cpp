...
{
	checkf(IsIn<ExtraWordTypes>(Member.Type) == IsSet(Member.ExtraWord), TEXT("FSchemaBuilder::Add while IsIn(Type) doesn't match IsSet(ExtraWord). DebugName:%s"), *Member.DebugName.ToString());
	checkf(IsIn<~GeneratedTypes>(Member.Type), TEXT("Member type %d is generated by Build() and shouldn't be declared"), int32{EnumToUnderlyingType(Member.Type)});
...
static void CopyDebugNames(const FName* In, TArrayView<FMemberDeclaration> Out)
{
	for (FMemberDeclaration& Member : Out)
	{
		...
		Member.DebugName = *In++;
#else
		Member.DebugName = ToName(Member.Type);
#endif
	}
}

void FSchemaBuilder::Append(FSchemaView SuperSchema, int32 OffsetForProperties)
{
	...
	while (true)
	{
		...
		for (Private::FMemberUnpacked Member : Word.Members)
		{
			...
			{
				Members.Add({ FName(), OffsetForProperties + (JumpedWords + Member.WordOffset) * 8, Member.Type, ExtraWord});
