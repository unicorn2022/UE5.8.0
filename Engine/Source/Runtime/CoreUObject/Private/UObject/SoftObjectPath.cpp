...
static FTransactionallySafeRWLock PIEPackageNamesLock;
	...
	else
	{
		...
		if (*Buffer == TCHAR('\''))
		{
			...
			ImportedPath.Reset();
			{
				...
				NewBuffer = FPropertyHelpers::ReadTerminatedToken(++Buffer, /*Out*/ImportedPath, TEXT('\''), &MaxTokenLength);
			}
			if (!NewBuffer)
			{
				return false;
...
UObject* FSoftObjectPath::TryLoad(FUObjectSerializeContext* InLoadContext, ELoadFlags LoadFlags /* = LOAD_None */) const
{
	...
	if (!IsNull())
	{
		...
		else
		{
			...
			LoadedObject = StaticLoadObject(UObject::StaticClass(), nullptr, *PathString, nullptr, LoadFlags, nullptr, true);
			...
			if (!LoadedObject && SoftObjectPath::bResolveCoreRedirects)
			{
				...
				{
					LoadedObject = LoadObject<UObject>(nullptr, *FixupObjectPath.ToString(), {}, LoadFlags);
				}
			}
...
{
	UE::TWriteScopeLock WriteLock(PIEPackageNamesLock);
...
bool FSoftObjectPath::FixupForPIE(int32 InPIEInstance, TFunctionRef<void(int32, FSoftObjectPath&)> InPreFixupForPIECustomFunction)
{
	...
	if (InPIEInstance != INDEX_NONE && !IsNull())
	{
		...
		if (!ShortPackageOuterAndName.StartsWith(PLAYWORLD_PACKAGE_PREFIX))
		{
			...
			bool bIsPIEPackage;
			{
				UE::TReadScopeLock ReadLock(PIEPackageNamesLock);
				bIsPIEPackage = PIEPackageNames.Contains(PIEPackage);
			}
			if (bIsChildOfLevel || bIsPIEPackage)
			{
