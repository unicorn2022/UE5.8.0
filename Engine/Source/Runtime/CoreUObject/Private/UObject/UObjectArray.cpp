...
void FUObjectArray::OpenDisregardForGC()
{
	...
	OpenForDisregardForGC = true;
	UE_LOGF(LogUObjectArray, Log, "OpenDisregardForGC: %d/%d objects in disregard for GC pool", ObjLastNonGCIndex + 1, MaxObjectsNotConsideredByGC);
}
...
void FUObjectArray::CloseDisregardForGC()
{
	...
	if (!IsEngineExitRequested())
	{
		...
		if (GIsInitialLoad)
		{
			...
			UE_LOGF(LogUObjectArray, Log, "%i objects as part of root set at end of initial load.", NumAlwaysLoadedObjects);
			if (GUObjectArray.DisregardForGCEnabled())
			{
				UE_LOGF(LogUObjectArray, Log, "%i objects are not in the root set, but can never be destroyed because they are in the DisregardForGC set.", NumAlwaysLoadedObjects - NumRootObjects);
			}
		}
	}
	...
	UE_LOGF(LogUObjectArray, Log, "CloseDisregardForGC: %d/%d objects in disregard for GC pool", ObjLastNonGCIndex + 1, MaxObjectsNotConsideredByGC);	
...
void FUObjectArray::DisableDisregardForGC()
{
	...
	if (!GExitPurge && NumDisregardForGCObjects > 0)
	{
		...
		checkf(!AutoRTFM::IsTransactional(), TEXT("Disregard for GC can't be changed while inside of a transaction."));
...
{ 
	...
	if( Index == INDEX_NONE )
	{
		UE_LOGF(LogUObjectArray, Warning, "Object is not in global object array" );
		return false;
	}
	if( !ObjObjects.IsValidIndex(Index))
	{
		UE_LOGF(LogUObjectArray, Warning, "Invalid object index %i", Index );
		return false;
	}
	const FUObjectItem& Slot = ObjObjects[Index];
	if( Slot.GetObject() == NULL )
	{
		UE_LOGF(LogUObjectArray, Warning, "Empty slot" );
		return false;
	}
	if( Slot.GetObject() != Object )
	{
		UE_LOGF(LogUObjectArray, Warning, "Other object in slot" );
		return false;
	}
	return true;
}
...
{
	{
		...
		UE_CLOGF(UObjectDeleteListeners.Num(), LogUObjectArray, Fatal, "All UObject delete listeners should be unregistered when shutting down the UObject array");
	}
	{
#if THREADSAFE_UOBJECTS
		UE::TScopeLock UObjectCreateListenersLock(UObjectCreateListenersCritical);
		...
		UE_CLOGF(UObjectCreateListeners.Num(), LogUObjectArray, Fatal, "All UObject delete listeners should be unregistered when shutting down the UObject array");
	}
}

void FUObjectArray::DumpUObjectCountsToLog() const
{
	UE_LOGF(LogUObjectArray, Display, "Dumping allocated UObject counts to log:");
	struct FClassEntry
	{
	...
	{		
		...
		{
			UE_LOGF(LogUObjectArray, Display, "%8d instances of %ls", ClassEntry.NumInstances, *ClassEntry.Class->GetPathName());
	...
	if (NumInstancesSkipped > 0)
	{
		...
		{
			UE_LOGF(LogUObjectArray, Display, "%8d instances in the remaining %.3f%% of instances of %d classes", NumInstancesSkipped, (1.0f - PercentOfInstancesPrinted) * 100.0f, NumClassesSkipped);
		}
		else
		{
			UE_LOGF(LogUObjectArray, Display, "%8d instances of %d classes with less than %d instances per class", NumInstancesSkipped, NumClassesSkipped, MinInstanceNum);
		}
	}
	UE_LOGF(LogUObjectArray, Display, "%d total UObjects (%d classes)", NumUObjects, NumClasses);
}
...
void UE::UObjectArrayPrivate::FailMaxUObjectCountExceeded(const int32 MaxUObjects, const int32 NewUObjectCount)
{
	...
	UE_LOGF(LogUObjectArray, Fatal, "Maximum number of UObjects (%d) exceeded when trying to add %d object(s), make sure you update MaxObjectsInGame/MaxObjectsInEditor/MaxObjectsInProgram in project settings.", MaxUObjects, NewUObjectCount);
}
