...
FText AssetAccessSpecifierAsText(EAssetAccessSpecifier InAccess)
{
#define LOCTEXT_NAMESPACE "AssetAccessSpecifier"
	switch (InAccess)
	{
	...
	{
		return LOCTEXT("EpicInternal", "Epic Internal");
	}
	...
	default:
		ensureMsgf(false, TEXT("Unknwon EAssetAccessSpecifier value %d"), EnumToUnderlyingType(InAccess));
		...
		if (MaxRunCount != 0)
		{
			PackageDirtyStateChangedEventHandle = UPackage::PackageDirtyStateChangedEvent.AddLambda([MaxRunCount](const UPackage* InPackage)
			{
				...
				if (InPackage && InPackage->IsDirty())
				{
					...
					UE_LOGF(LogObj, Warning, "***** Package %ls Marked Dirty ******\n%ls", *InPackage->GetPathName(), ANSI_TO_TCHAR(StackTrace));
					RunsSoFar++;
				}
			});
		}
...
{
	TSet<UPackage*> Result;
	ForEachObjectWithPackage(const_cast<UPackage*>(this), 
		[this, &Result](UObject* Object)
		{
			ForEachObjectWithOuter(Object, 
				[this, &Result](UObject* InObject)
				{
					UPackage* ObjectPackage = InObject->GetExternalPackage();
					if (ObjectPackage && ObjectPackage != this)
					{
