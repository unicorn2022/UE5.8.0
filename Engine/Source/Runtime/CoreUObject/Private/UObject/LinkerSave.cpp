...
bool FLinkerSave::TryAssignFileSaver(FStringView InFilename, bool bForceByteSwapping, bool bInSaveUnversioned)
{
	...
	if (!LocalSaver)
	{
		...
		UE_LOGF(LogLinker, Error, "Error opening file '%.*ls': %ls",
...
bool FLinkerSave::IsUnsaveable(TObjectPtr<UObject> Object) const
{
	if (SaveContext)
	{
		return SaveContext->IsUnsaveable(Object, SaveContext->GetCachedObjectStatus(Object), /*bEmitWarning*/false);
	}
	return false;
}
...
FPackageIndex FLinkerSave::MapObject(TObjectPtr<const UObject> Object, bool bValidateExcluded) const
{
	if (Object)
	{
		...
		if (Found)
		{
				...
				*Found != CurrentlySavingExport) // would be weird, but I can't be a dependency on myself
			{
				...
				if (!bFoundDep)
				{
					...
					if (!IsNativeDep)
					{
						UE_LOGF(LogLinker, Fatal, "Attempt to map an object during save that was not listed as a dependency. Saving Export %d %ls in %ls. Missing Dep on %ls %ls.",
		...
		{
			UE_CLOGF(SaveContext->FindCachedObjectStatus(Object) == nullptr, LogLinker, Error,
					"Invalid serialization during save of Export '%ls' in package '%ls'. Object '%ls' encountered during serialization without being previously harvested. This will cause the reference to be null at runtime. ",
...
void FLinkerSave::UsingCustomVersion(const struct FGuid& Guid)
{
	...
	if (Summary.GetCustomVersionContainer().GetVersion(Guid) == nullptr)
	{
		...
		UE_LOGF(LogLinker, Warning, "%ls", *CustomVersionWarning);
	}
}
...
{
	UE_LOGF(LogLinker, Warning, "Data will not be able to load because derived data is not saved yet.");
...
bool FLinkerSave::SerializeBulkData(FBulkData& BulkData, const FBulkDataSerializationParams& Params) 
{
	...
	else
	{
		...
		else if (bSaveBulkDataToSeparateFiles && FBulkData::HasFlags(SerializedMeta.Flags, BULKDATA_MemoryMappedPayload) && bSupportsMemoryMapping)
		{
			...
			UE_CLOGF(!Params.CookedIndex.IsDefault(), LogLinker, Warning, "%ls: Cooked Index is not supported for MemoryMappedPayloads, value will be ignored", *LinkerRoot->GetName());
		...
		else
		{
			if (bSaveBulkDataToSeparateFiles && FBulkData::HasFlags(SerializedMeta.Flags, BULKDATA_DuplicateNonOptionalPayload))
			{
				...
				UE_CLOGF(!Params.CookedIndex.IsDefault(), LogLinker, Warning, "%ls: Cooked Index is not supported for DuplicateNonOptionalPayloads, value will be ignored", *LinkerRoot->GetName());
