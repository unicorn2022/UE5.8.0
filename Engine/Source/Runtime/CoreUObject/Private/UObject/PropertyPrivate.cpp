...
void UStructProperty::Serialize(FArchive& Ar)
{
	...
	if (Ar.IsPersistent() && Ar.GetLinker())
	{
		...
		{
			UE_LOGF(LogProperty, Error, "UStructProperty::Serialize Loading: Property '%ls'. Unknown structure.", *GetFullName());
			Struct = FallbackStruct;
		}
		else if ((FallbackStruct == Struct) && Ar.IsSaving())
		{
			UE_LOGF(LogProperty, Error, "UStructProperty::Serialize Saving: Property '%ls'. FallbackStruct structure.", *GetFullName());
		}
	}
#endif // WITH_EDITOR
	if (Struct)
