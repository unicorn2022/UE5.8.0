...
FORCENOINLINE UObject* FindArchetypeFromRequiredInfoImpl(const UClass* Class, const UObject* Outer, FName Name, EObjectFlags ObjectFlags, bool bUseUpToDateClass, const FObjectArchetypeHelper::IObjectArchetypePolicy* Policy)
{
	...
	{
		if (Outer && Cast<UPackage>(Outer) == nullptr) // packages cannot have subobjects
		{
			...
			if (!Result)
			{
				// No archetype found yet - but the ICH mangles the DefaultSceneRoot name *and* 
				// tries to cull unused templates so do a relaxed compare to find the real default 
				// scene root - see UInheritableComponentHandler::SCSDefaultSceneRootOverrideNamePrefix:
				...
				if (Name == DefaultSceneRootSCS || Name == ICHDefaultSceneRoot)
				{
					if (const UClass* OuterClass = Cast<UClass>(Outer))
					{
						if (const UClass* SuperClass = OuterClass->GetSuperClass())
						{
							Result = SuperClass->FindArchetype(Class, Name);
						}
					}
				}
			}
...
void FObjectArchetypeHelper::CacheArchetypeForObject(UObject* Object, UObject* Archetype)
{
#if UE_CACHE_ARCHETYPE
#if UE_VERIFY_CACHED_ARCHETYPE
...
UObject* GetArchetypeImpl(const UObject* InObject, const FObjectArchetypeHelper::IObjectArchetypePolicy* Policy)
{
	...
	else
	{
		...
		{
			UE_LOGF(LogClass, Fatal, "Cached archetype mismatch, expected: %ls, cached: %ls", *GetFullNameSafe(ExpectedArchetype), *GetFullNameSafe(Archetype));
		}
#endif // UE_VERIFY_CACHED_ARCHETYPE
	}
...
void FObjectArchetypeHelper::CleanupCachedArchetypes()
{
#if UE_CACHE_ARCHETYPE
...
void FObjectArchetypeHelper::InvalidateArchetypeCache(const UObject* Object)
{
#if UE_CACHE_ARCHETYPE
	ArchetypeAnnotation.RemoveAnnotation(Object);
#endif
}
