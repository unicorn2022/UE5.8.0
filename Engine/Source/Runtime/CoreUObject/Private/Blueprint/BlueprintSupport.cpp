...
void FBlueprintSupport::RepairDeferredDependenciesInObject(UObject* Object)
{
	...
	for (TPropertyValueIterator<const FObjectProperty> It(Object->GetClass(), Object); It; ++It)
	{
		...
		if (Placeholder)
		{
			UE_LOGF(LogBlueprintSupport, Warning, "Object %ls still has a %ls '%ls' in property %ls. This indicates a failure to resolve every deferred/circular dependency in blueprints.", *Object->GetName(), *PropertyValueAsObj->GetClass()->GetName(), *PropertyValueAsObj->GetName(), *Property->GetName());
				...
				if (const UPackage* PlaceholderPackage = PropertyValueAsObj->GetPackage())
				{
					if (FLinkerLoad* PlaceholderLinker = PlaceholderPackage->GetLinker())
					{
						...
						if ((Import.XObject != nullptr) && (Import.XObject != PropertyValueAsObj))
						{
							...
							UE_LOGF(LogBlueprintSupport, Display, "Repaired deferred dependency on object %ls: replaced '%ls' with '%ls'", *Object->GetName(), *PropertyValueAsObj->GetName(), *Import.XObject->GetName());
						}
					}
				}
			}
			...
			{
				UE_LOGF(LogBlueprintSupport, Error, "Failed to repair deferred dependency on object %ls (%ls). This may lead to blueprint execution problems.", *Object->GetName(), *PropertyValueAsObj->GetName());
			}
		}
	}
...
#if WITH_EDITOR
thread_local UClass* FScopedClassDependencyGather::BatchAuthorityClass = nullptr;
thread_local TArray<UClass*> FScopedClassDependencyGather::BatchClassDependencies;
thread_local FScopedClassDependencyGather* FScopedClassDependencyGather::AuthoritativeScope = nullptr;
...
FScopedClassDependencyGather::~FScopedClassDependencyGather()
{
	...
	if (AuthoritativeScope == this)
	{
		AuthoritativeScope = nullptr;

		OnScopeExitDelegate.Broadcast();
	}
}

FScopedClassDependencyGather* FScopedClassDependencyGather::GetAuthoritativeScope()
{
...
bool ShouldSkipResolveDeferredDependencies(UStruct* LoadStruct)
{
	if (IsInActualLoadingThread() || IsInParallelLoadingThread())
	{
		if (!LoadStruct->HasAnyInternalFlags(EInternalObjectFlags::AsyncLoadingPhase1))
		{
			// If we're in the loader, it would be a race hazard to try to recurse into
			// packages and linkers that have finished loading, so we skip unless the
			// package we target is also owned by the loader.
			// We know that a struct can't contain deferred dependencies anymore once it
			// leaves its phase 1 and is ready for postload.
			return true;
		}
	}
...
// 				}
		...
		for (int32 ImportIndex = StartingImportIndex; ImportIndex < ImportMap.Num() && HasUnresolvedDependencies(); ++ImportIndex)
		{
			...
			else if (UScriptStruct* StructObj = Cast<UScriptStruct>(Import.XObject))
			{
				...
				if (SourceLinker != nullptr && !ShouldSkipResolveDeferredDependencies(StructObj))
				{
		...
		{
			if (!ShouldSkipResolveDeferredDependencies(SuperStruct))
			{
	...
	if (FBlueprintSupport::UseDeferredDependencyLoading() && (LoadFlags & LOAD_DeferDependencyLoads))
	{
		...
		else if (ObjectType->IsChildOf<UClass>())
		{
			...
			if (!FPackageName::IsScriptPackage(ObjectPathStr) && ObjectType->HasAnyClassFlags(CLASS_NeedsDeferredDependencyLoading))
			{
				...
				{
					UE_LOGF(LogBlueprintSupport, Error, "'%ls' was not found in the import table for package '%ls'.", ObjectPathStr.GetData(), *LinkerRoot->GetFullName());
				}
			}
		}
	}
