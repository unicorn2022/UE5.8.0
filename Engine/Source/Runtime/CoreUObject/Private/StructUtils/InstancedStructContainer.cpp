...
void FInstancedStructContainer::AddStructReferencedObjects(FReferenceCollector& Collector)
{
	...
	if (UE::StructUtils::FStructReinstancer* Reinstancer = UE::StructUtils::FStructReinstancer::GetInstance())
	{
		for (int32 Index = 0; Index < NumItems; ++Index)
		{
			TObjectPtr<const UScriptStruct>& ScriptStruct = GetItem(Index).ScriptStruct;
...
bool FInstancedStructContainer::Identical(const FInstancedStructContainer* Other, uint32 PortFlags) const
{
	...
	{ 
		...
		if (Item.ScriptStruct != nullptr && OtherItem.ScriptStruct != nullptr)
		{
			...
			if (Reinstancer
				&& Reinstancer->IsSerializingForReinstantiation()
				&& Reinstancer->IsReinstanting(Item.ScriptStruct))
			{
				bResult = false;
				break;
			}
...
bool FInstancedStructContainer::Serialize(FArchive& Ar, UStruct* DefaultsStruct, const void* Defaults)
{
	const bool bValidDefaults = DefaultsStruct && DefaultsStruct->IsChildOf(FInstancedStructContainer::StaticStruct());
	const FInstancedStructContainer* DefaultInstancedStructContainer = bValidDefaults ? static_cast<const FInstancedStructContainer*>(Defaults) : nullptr;
	...
	{
		UE_LOGF(LogCore, Error, "Invalid Version: %hhu", int(Version));
		Ar.SetError();
		return false;
	}
	...
	auto SerializeWithDefaults = [Self = this, &Ar, DefaultInstancedStructContainer](const FItem& Item, int32 Index)
		{
			...
			if (bUseDefaults)
			{
				const bool bSameStruct = DefaultInstancedStructContainer
					&& DefaultInstancedStructContainer->IsValidIndex(Index)
					&& DefaultInstancedStructContainer->GetItem(Index).ScriptStruct == Item.ScriptStruct;
				if (bSameStruct)
				{
					// Use the parent Defaults
					const void* DefaultMemory = DefaultInstancedStructContainer->Memory + DefaultInstancedStructContainer->GetItem(Index).Offset;
					
					if (Ar.IsLoading())
					{
						ConstCast(Item.ScriptStruct)->CopyScriptStruct(Self->Memory + Item.Offset, DefaultMemory);
					}
					
					ConstCast(Item.ScriptStruct)->SerializeItem(Ar, Self->Memory + Item.Offset, DefaultMemory);
				}
				else
				{
					// Create a temporary struct on the stack. The struct has default values.
					const int32 MinAlignment = Item.ScriptStruct->GetMinAlignment();
					const int32 RequiredSize = Item.ScriptStruct->GetStructureSize();
					...
					ConstCast(Item.ScriptStruct)->InitializeStruct(TempMemory);
					ConstCast(Item.ScriptStruct)->SerializeItem(Ar, Self->Memory + Item.Offset, TempMemory);
					ConstCast(Item.ScriptStruct)->DestroyStruct(TempMemory);
				}
			}
			else
			{
				// Serialize without defaults
				ConstCast(Item.ScriptStruct)->SerializeItem(Ar, Self->Memory + Item.Offset, nullptr);
			}
		};
	...
	if (NumItemsSerialized > 0)
	{
		if (Ar.IsCountingMemory() || Ar.IsModifyingWeakAndStrongReferences() || Ar.IsObjectReferenceCollector())
		{
			...
			for (int32 Index = 0; Index < NumItemsSerialized; Index++)
			{
				...
				if (Item.ScriptStruct != nullptr)
				{
					SerializeWithDefaults(Item, Index);
				}
			}
		}
		else if (Ar.IsLoading())
		{
			...
			for (int32 Index = 0; Index < NumItemsSerialized; Index++)
			{
				...
				if (SerializedScriptStruct)
				{
					...
					if (Reinstancer && Reinstancer->IsSerializingForReinstantiation())
					{
			...
			for (int32 Index = 0; Index < NumItemsSerialized; Index++)
			{
				...
				if (Item.ScriptStruct == nullptr && SerialSize > 0)
				{
					...
					UE_LOGF(LogCore, Warning, "Unable to find serialized UScriptStruct -> Advance %u bytes in the archive and reset to empty FInstancedStructContainer. LinkerRoot:%ls", SerialSize, Ar.GetLinker() ? *GetPathNameSafe(Ar.GetLinker()->LinkerRoot) : TEXT("NoLinker"));
		...
		else
		{
			...
			// or the CityHash64-based archive used by GetStructHash64). The serial-size backpatch is gated
			// on the archive being seekable - non-seekable archives (FArchive::Tell defaults to INDEX_NONE)
			// don't need it. Without this branch the contained items would not contribute to the hash.
