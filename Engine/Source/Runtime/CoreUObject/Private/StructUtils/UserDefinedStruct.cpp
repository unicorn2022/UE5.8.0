...
FName UUserDefinedStruct::FindPropertyNameFromGuid(const FGuid& PropertyID) const
{
	for (const UE::StructUtils::FUserDefinedPropertyID& Property : PropertyIDs)
	{
...
FGuid UUserDefinedStruct::FindPropertyGuidFromName(const FName PropertyName) const
{
	for (const UE::StructUtils::FUserDefinedPropertyID& Property : PropertyIDs)
	{
...
void UUserDefinedStruct::Serialize(FStructuredArchive::FRecord Record)
{
	...
	if (UnderlyingArchive.IsLoading())
	{
		...
		if (EUserDefinedStructureStatus::UDSS_UpToDate == Status)
		{
			...
			// While duplicating, the struct is not linked and is therefore invalid.
			if (!(UnderlyingArchive.GetPortFlags() & PPF_Duplicate))
			{
				...
				if (FUserDefinedStructEditorUtils::EStructureError::Ok != Result)
				{
					...
					UE_LOGF(LogClass, Log, "UUserDefinedStruct.Serialize '%ls' validation: %ls", *GetName(), *ErrorMessage);
				}
			}
		}
	}
...
void UUserDefinedStruct::SerializeTaggedProperties(FStructuredArchive::FSlot Slot, TNotNull<uint8*> Data, const UStruct* DefaultsStruct, const uint8* Defaults, const UObject* BreakRecursionIfFullyLoad) const 
{
...
{
	if (ensureMsgf(DefaultStructInstance.IsValid(), TEXT("DefaultStructInstance is invalid for '%s'."), *GetPathName()))
	{
		const UStruct* DefaultStruct = DefaultStructInstance.GetStruct();
		ensureMsgf(DefaultStruct == this, TEXT("DefaultStructInstance struct mismatch: '%s' vs. '%s'"), *GetPathNameSafe(DefaultStruct),
			*GetPathName());
	}
