...
int32 FStructTracker::FindOrAddStructTypeIndex(TNotNull<const UStruct*> InStructType)
{
	// Get existing index...
...
int32 FStructTracker::RegisterImplementation(TNotNull<const UStruct*> InStructType, const bool bCheckExisting)
{
	...
	OnTypeRegistered.Broadcast(InStructType, NewIndex);
...
int32 FStructTracker::FindStructTypeIndex(TNotNull<const UStruct*> InStructType) const
{
	const uint32 Hash = PointerHash(InStructType);
	const FSetElementId ElementId = StructTypeToIndexSet.FindIdByHash(Hash, Hash);
...
{	
	...
	else if (Ar.IsLoading())
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogStructUtils, Error, "Trying to load mismatching BitSet data. Current base class: %ls, read base class: %ls"
