...
bool FInstancedStruct::Serialize(FArchive& Ar, UStruct* DefaultsStruct, const void* Defaults)
{
	...
	const bool bValidDefaults = DefaultsStruct && DefaultsStruct->IsChildOf(FInstancedStruct::StaticStruct());
	const FInstancedStruct* DefaultInstancedStruct = bValidDefaults ? static_cast<const FInstancedStruct*>(Defaults) : nullptr;

	auto SerializeWithDefaults = [Self = this, &Ar, DefaultInstancedStruct](UScriptStruct* ScriptStruct)
		{
			const bool bDuplicate = (0 != (Ar.GetPortFlags() & PPF_Duplicate));
			const bool bSavingLoading = (Ar.IsSaving() || Ar.IsLoading());
			const bool bUseDefaults = !bDuplicate
				&& bSavingLoading
				&& !Ar.IsCooking()
			...
			if (bUseDefaults)
			{
				const bool bSameStruct = DefaultInstancedStruct && DefaultInstancedStruct->GetScriptStruct() == ScriptStruct;
				if (bSameStruct)
				{
					// Use the parent Defaults
					ScriptStruct->SerializeItem(Ar, Self->GetMutableMemory(), DefaultInstancedStruct->GetMemory());
				}
				else
				{
					// Create a temporary struct on the stack. The struct has default values.
					const int32 MinAlignment = ScriptStruct->GetMinAlignment();
					const int32 RequiredSize = ScriptStruct->GetStructureSize();
					TArray<uint8, TInlineAllocator<64>> TmpMemoryBuffer;
					TmpMemoryBuffer.SetNumUninitialized(FMath::Max(1, RequiredSize) + FMath::Max(0, MinAlignment - (int32)MIN_ALIGNMENT));
					void* TempMemory = Align(TmpMemoryBuffer.GetData(), MinAlignment);
					ScriptStruct->InitializeStruct(TempMemory);
					ScriptStruct->SerializeItem(Ar, Self->GetMutableMemory(), TempMemory);
					ScriptStruct->DestroyStruct(TempMemory);
				}
			}
			else
			{
				// Serialize without defaults
				ScriptStruct->SerializeItem(Ar, Self->GetMutableMemory(), nullptr);
			}
		};
	...
	if (Ar.IsCountingMemory() || Ar.IsModifyingWeakAndStrongReferences() || Ar.IsObjectReferenceCollector())
	{
		// Report type
		Ar << ScriptStruct;
		...
		{
			SerializeWithDefaults(ConstCast(ScriptStruct));
		}
	}
	else if (Ar.IsLoading())
	{
		// UScriptStruct type
		TObjectPtr<UScriptStruct> SerializedScriptStruct = nullptr;
		Ar << SerializedScriptStruct;
		if (SerializedScriptStruct)
		{
#if WITH_EDITOR
			// During reinstantiation, the serialized struct can be the the duplicated struct.
			//Replace it with the new version.
			if (const UE::StructUtils::FStructReinstancer* Reinstancer = UE::StructUtils::FStructReinstancer::GetInstance();
				Reinstancer && Reinstancer->IsSerializingForReinstantiation())
			{
				if (const UScriptStruct* CompiledStruct = Reinstancer->GetCompiledReinstantingStruct(SerializedScriptStruct))
				{
					SerializedScriptStruct = const_cast<UScriptStruct*>(CompiledStruct);
				}
			}
#endif
		...
		// Create a new one using the struct default values.
		if (DefaultInstancedStruct && SerializedScriptStruct && SerializedScriptStruct->IsChildOf(DefaultInstancedStruct->GetScriptStruct()))
		{
			InitializeAs(SerializedScriptStruct, DefaultInstancedStruct->GetMemory());
		}
		else
		{
		...
		if (ScriptStruct == nullptr && SerialSize > 0)
		{
			...
			UE_LOGF(LogCore, Warning, "Unable to find serialized UScriptStruct -> Advance %u bytes in the archive and reset to empty FInstancedStruct. SerializedProperty:%ls LinkerRoot:%ls",
				SerialSize,
	...
	{
		// Saving + inspector archives (e.g. FArchiveCrc32 used by UE::StructUtils::GetStructInstanceCrc32,
		// or the CityHash64-based archive used by GetStructHash64). The serial-size backpatch is gated on
		// the archive being seekable - non-seekable archives (FArchive::Tell defaults to INDEX_NONE) don't
		// need it. Without this branch the FInstancedStruct payload would not contribute to the hash, and
		// shared fragments differing only in their FInstancedStruct member would be wrongly aliased.
		Ar << ScriptStruct;
			...
			if (!UnderlyingArchive.IsTextFormat())
			{
				...
				if (StructInstanceEndOffset != ExpectedStructInstanceEndOffset)
				{
					if (StructTypePtr)
					{
						...
						UE_LOGF(LogCore, Error, "FStructVariant expected to read %lld bytes for struct %ls but read %lld bytes!", StructInstanceSerializedSize, *StructTypePtr->GetName(), StructInstanceEndOffset - StructInstanceStartOffset);
					}
...
bool FInstancedStruct::Identical(const FInstancedStruct* Other, uint32 PortFlags) const
{
	...
	if (StructTypePtr)
	{
		...
		if (const UE::StructUtils::FStructReinstancer* Reinstancer = UE::StructUtils::FStructReinstancer::GetInstance())
		{
			if (Reinstancer->IsSerializingForReinstantiation()
				&& Reinstancer->IsReinstanting(StructTypePtr))
			{
				return false;
			}
		}
...
void FInstancedStruct::AddStructReferencedObjects(FReferenceCollector& Collector)
{
	...
	{
		if (ScriptStruct && Reinstancer->IsReinstanting(ScriptStruct))
		{
...
bool FInstancedStruct::NetSerialize(FArchive& Ar, UPackageMap* Map, bool& bOutSuccess)
{
	...
	if (bValidData)
	{
		if (Ar.IsLoading())
		{
			...
			if (!IsValid())
			{
				UE_LOGF(LogCore, Error, "FInstancedStruct::NetSerialize: Bad script struct serialized, cannot recover.");
				Ar.SetError();
				bOutSuccess = false;
			}
		}
...
bool FInstancedStruct::FindInnerPropertyInstance(FName PropertyName, const FProperty*& OutProp, const void*& OutData) const
{
	...
	{
		if( Prop->GetAuthoredName() == PropertyName )
		{
			OutProp = Prop;
