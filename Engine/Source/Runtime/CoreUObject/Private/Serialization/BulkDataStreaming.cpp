	...
	uint8* UserSuppliedMemory)
{
	...
	UE_CLOGF(bIsOutsideBulkDataRange, LogSerialization, Warning,
		"Reading outside of bulk data range, RequestRange='%lld, %lld', BulkDataRange='%llu, %llu', ChunkId='%ls'",
	...
	FBulkDataBatchReadRequest* OutRequest)
{
	...
	checkf(Dst.GetSize() == 0 || Dst.GetSize() == ReadSize, TEXT("Buffer size (%llu) does not match the read size (%llu)"), Dst.GetSize(), Size);
