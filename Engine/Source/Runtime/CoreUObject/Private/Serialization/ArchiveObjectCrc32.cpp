...
uint32 FArchiveObjectCrc32::Crc32(UObject* Object, UObject* Root, uint32 CRC)
{
	...
	UE_LOGF(LogArchiveObjectCrc32, Log, "### Calculating CRC for object: %ls with outer: %ls", *Object->GetName(), Object->GetOuter() ? *Object->GetOuter()->GetName() : TEXT("NULL"));
#endif
	RootObject = Root;
	if (Object)
	{
		...
		while (ObjectsToSerialize.Dequeue(Object))
		{
			...
			{
#ifdef DEBUG_ARCHIVE_OBJECT_CRC32
				UE_LOGF(LogArchiveObjectCrc32, Log, "- Serializing object: %ls with outer: %ls", *Object->GetName(), Object->GetOuter() ? *Object->GetOuter()->GetName() : TEXT("NULL"));
#endif
				// Serialize it
...
#ifdef DEBUG_ARCHIVE_OBJECT_CRC32
				UE_LOGF(LogArchiveObjectCrc32, Log, "=> object: '%ls', total size: %d bytes, checksum: 0x%08x", *GetPathNameSafe(Object), SerializedObjectData.Num(), CRC);
#endif
				// Cleanup
...
#ifdef DEBUG_ARCHIVE_OBJECT_CRC32
	UE_LOGF(LogArchiveObjectCrc32, Log, "### Finished (%.02f ms), final checksum: 0x%08x", (FPlatformTime::Seconds() - StartTime) * 1000.0f, CRC);
#endif
	return CRC;
}
