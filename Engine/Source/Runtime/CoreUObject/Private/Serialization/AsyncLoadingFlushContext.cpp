...
void FAsyncLoadingFlushContext::Flush(const FOnAsyncLoadingFlushComplete& OnFlushComplete)
{
	...
	UE_LOGF(AsyncLoadingFlush, Verbose, "Context[%ls:%d] Flushing async loading.", *Context, Id);
}
...
bool FAsyncLoadingFlushContext::OnAsyncLoadingCheck(float DeltaTime)
{
	...
	UE_LOGF(AsyncLoadingFlush, Verbose, "Context[%ls:%d] Flushed in %f seconds.", *Context, Id, CurrentTimeSeconds - StartTime);
...
{
	UE_LOGF(AsyncLoadingFlush, Warning, "Context[%ls:%d] Waiting on async loading to complete. Total wait time: %f seconds.", *Context, Id, FPlatformTime::Seconds() - StartTime);
	return true;
}
