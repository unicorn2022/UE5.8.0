...
EIoChunkType GetIoChunkTypeFromFlags(EBulkDataFlags Flags)
{
	if (!FBulkData::HasFlags(Flags, BULKDATA_PayloadAtEndOfFile))
	{
		return EIoChunkType::ExportBundleData;
	}

	if (FBulkData::HasFlags(Flags, BULKDATA_OptionalPayload))
	{
		return EIoChunkType::OptionalBulkData;
	}

	if (FBulkData::HasFlags(Flags, BULKDATA_MemoryMappedPayload))
	{
...
bool FBulkMetaData::FromSerialized(FArchive& Ar, int64 ElementSize, FBulkMetaData& OutMetaData, int64& OutDuplicateOffset)
{
	...
	if (Ar.IsError())
	{
		...
		UE_LOGF(LogSerialization, Fatal, "Bulkdata error when serializing '%ls', could not serialize FBulkMetaResource correctly", *GetDebugNameFromArchive(Ar));
		return false;
	}
...
void FBulkData::FAllocatedPtr::Free(FBulkData* Owner)
{
	...
	{
		FMemory::Free(Allocation.RawData);
...
void FBulkData::DumpBulkDataUsage( FOutputDevice& Log )
{
	...
	// Log information.
	UE_LOGF(LogSerialization, Log, "");
	UE_LOGF(LogSerialization, Log, "Per class summary of bulk data use:");
	for( int32 PerClassIndex=0; PerClassIndex<PerClassSizeArray.Num(); PerClassIndex++ )
	{
	...
	UE_LOGF(LogSerialization, Log, "");
	UE_LOGF(LogSerialization, Log, "Detailed per object stats of bulk data use:");
	...
	UE_LOGF(LogSerialization, Log, "");
#else
	UE_LOGF(LogSerialization, Log, "Please recompiled with TRACK_BULKDATA_USE set to 1 in UnBulkData.cpp.");
#endif
}
	...
	if( *Dest )
	{
		...
		if( IsBulkDataLoaded() )
		{
			...
			if( bDiscardInternalCopy && CanDiscardInternalData() )
			{
				...
				UE_LOGF(LogSerialization, Warning, "FBulkData::GetCopy both copied and discarded it's data, passing in an empty pointer would avoid an extra allocate and memcpy!");
#endif
				FreeData();
			}
		}
 ...
 */
void* FBulkData::Lock(uint32 LockFlags, UE::FSourceLocation Location)
{
	check(IsUnlocked());
	...
	LockSourceLocation.Emplace(Location);
#endif
	...
	{
		UE_LOGF(LogSerialization, Fatal, "Unknown lock flag '%u'", LockFlags);
		return nullptr;
	}
}

const void* FBulkData::LockReadOnly(UE::FSourceLocation Location) const
{
	check(IsUnlocked());
...
#if WITH_EDITOR && DO_CHECK
	mutable_this->LockSourceLocation.Emplace(Location);
#endif
...
#if WITH_EDITOR && DO_CHECK
	mutable_this->LockSourceLocation.Reset();
#endif
...
void FBulkData::DetachFromArchive( FArchive* Ar, bool bEnsureBulkDataIsLoaded )
{
	...
	if (!IsUnlocked())
	{
		FLinker* FromLinker = Ar->GetLinker();
#if WITH_EDITOR
		UE::FSourceLocation* Location = LockSourceLocation.GetPtrOrNull();
#else
		UE::FSourceLocation* Location = nullptr;
#endif // WITH_EDITOR
		checkf(false, TEXT("Bulk data cannot be detached while still locked. BulkData: %s, Linker: %s, Archive: %s. LockLocation: %s %s(%d)"), 
			*GetDebugName(), FromLinker ? *FromLinker->GetDebugName() : TEXT("nullptr"), *Ar->GetArchiveName(),
			Location ? StringCast<TCHAR>(Location->GetFunctionName()).Get() : TEXT("null"),
			Location ? StringCast<TCHAR>(Location->GetFileName()).Get() : TEXT("null"),
			Location ? Location->GetLine() : 0
		);
	}
#endif // DO_CHECK
...
bool FBulkData::TryLoadDataIntoMemory(FIoBuffer Dest)
{
		...
		{
			{
				...
				FArchiveBoundsScope Bounds(Ar->SetBounds(Dest.GetSize()));
	...
	{
		UE_LOGF(LogSerialization, Error, "Attempting to load a BulkData object that cannot be loaded from disk");
		return false;
	}
