...
FArchive& FArchiveUObject::SerializeWeakObjectPtr(FArchive& Ar, FWeakObjectPtr& Value)
{
	...
	if (!Ar.IsObjectReferenceCollector() || Ar.IsModifyingWeakAndStrongReferences())
	{
		...
		// Only commit the value using the same checks that we have in map/set to align with rehashing, otherwise those containers can become inoperable and lead to crashes
...
FArchive& FObjectAndNameAsStringProxyArchive::operator<<(UObject*& Obj)
{
	if (IsLoading())
	{
		...
		if (!Obj && bResolveCoreRedirects)
		{
			const FCoreRedirectObjectName OldRedirectObjectName(LoadedString);

			// We're matching the logic in FSoftObjectPath::FixupCoreRedirects
			const bool bIsNative = FPackageName::IsScriptPackage(LoadedString);
			const FCoreRedirectObjectName NewRedirectObjectName = FCoreRedirects::GetRedirectedName(
				bIsNative ? ECoreRedirectFlags::Type_AllMask : ECoreRedirectFlags::Type_Object,
				OldRedirectObjectName
			);

			if (NewRedirectObjectName != OldRedirectObjectName)
			{
				// look up the redirected object by fully qualified pathname
				LoadedString = NewRedirectObjectName.ToString();
				Obj = FindObject<UObject>(nullptr, *LoadedString, EFindObjectFlags::None);
...
void FArchiveReplaceObjectRefBase::SerializeObject(UObject* ObjectToSerialize)
{
	...
	for (int32 CellIter = 0; CellIter < PendingSerializationCells.Num(); CellIter++)
	{
		AutoRTFM::UnreachableIfClosed("VerseVM does not support serialization in the closed");
		PendingSerializationCells[CellIter]->Serialize(Context, *this);
	}
	PendingSerializationCells.Reset();
#endif
}
