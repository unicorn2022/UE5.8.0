	...
	FLargeMemoryWriter& ExportsArchive, const TArray<FFileRegion>& FileRegions)
{
	...
	{
		UE_LOGF(LogPackageWriter, Error,
			"PackageWriter->WritePackageData called more than once for package (Package: %ls). Ignoring second value.",
	...
	const FIoBuffer& FileData)
{
	...
	{
		UE_LOGF(LogPackageWriter, Warning, "PackageWriter->WriteAdditionalFile called with an unexpected chunkid: Should match CreateExternalFileChunkId (Package: %ls Filename: %ls)", *Info.PackageName.ToString(), *AdditionalFile.Info.Filename);
	}
	else
	{
...
void FPackageWriterRecords::WritePackageTrailer(const IPackageWriter::FPackageTrailerInfo& Info, const FIoBuffer& Data)
{
	...
	{
		UE_LOGF(LogPackageWriter, Error,
			"PackageWriter->WritePackageTrailer called more than once for package (Package: %ls). Ignoring second value.",
			*Info.PackageName.ToString());
		return;
	}
