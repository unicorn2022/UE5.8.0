	...
	, AllObjectsInPackage()
{
	...
	ForEachObjectWithPackage(Package, [this](UObject* Object)
	{
		...
		return true;
	}, EGetObjectsFlags::IncludeNestedObjects, RF_Transient, EInternalObjectFlags::Garbage);
	...
	// Note: This calls GetObjectsWithPackage rather than test the AllObjectsInPackage array, as external actors don't pass a "Object->GetOuter() == Package" test when trying to query for root objects
	{
		...
		GetObjectsWithPackage(Package, RootObjectsInPackage, EGetObjectsFlags::None, RF_Transient, EInternalObjectFlags::Garbage);
...
void FPropertyLocalizationDataGatherer::GatherLocalizationDataFromObject(const UObject* Object, const EPropertyLocalizationGathererTextFlags GatherTextFlags)
{
	...
	if (!(GatherTextFlags & EPropertyLocalizationGathererTextFlags::SkipSubObjects))
	{
		...
		GetObjectsWithOuter(Object, InnerObjects, EGetObjectsFlags::None, RF_Transient, EInternalObjectFlags::Garbage);
...
{
	auto AddGatheredText = [this, &Description](const FString& InNamespace, const FString& InKey, const FTextSourceData& InSourceData, const bool InIsEditorOnly, const FString& InDevNotes)
	{
		...
		// We might attempt to add the same text multiple times if we process the same object with slightly different flags - only add this source site once though.
		{
			...
			if (!bFoundSourceSiteContext)
			{
				...
				SourceSiteContext.DevNotes = InDevNotes;
#endif
			}
		}
	};
...
#if WITH_EDITORONLY_DATA
	AddGatheredText(CleanNamespace, Key, SourceData, bIsEditorOnly, Text.GetDevNotes());
#else
	static const FString EmptyString = TEXT("");
	AddGatheredText(CleanNamespace, Key, SourceData, bIsEditorOnly, EmptyString);
#endif
		...
		case EBlueprintTextLiteralType::LocalizedTextWithNotes:
			{
				...
				{
					SerializeExpr(iCode, Ar);
					DevNotes = MoveTemp(LastParsedString);
				}

				bIsParsingText = false;

#if WITH_EDITORONLY_DATA
				const FText TextInstance = FText::AsLocalizable_Advanced(TextNamespace, TextKey, MoveTemp(SourceString), DevNotes);
#else
