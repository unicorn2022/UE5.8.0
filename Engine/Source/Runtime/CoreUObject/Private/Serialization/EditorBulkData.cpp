...
static void LogPackageOpenFailureMessage(const FPackagePath& PackagePath, EPackageSegment PackageSegment)
{
	...
	if (SystemError != 0)
	{
		...
		UE_LOGF(LogSerialization, Error, "Could not open the file '%ls' for reading due to system error: '%ls' (%d))", *PackagePath.GetLocalFullPath(PackageSegment), SystemErrorMsg, SystemError);
	}
	else
	{
		UE_LOGF(LogSerialization, Error, "Could not open (%ls) to read FEditorBulkData with an unknown error", *PackagePath.GetLocalFullPath(PackageSegment));
	}
}
...
bool GCVarLegacyLinkerDiscovery = false;
static FAutoConsoleVariableRef CVar_LegacyLinkerDiscovery(
	TEXT("Serialization.LegacyLinkerDiscovery"),
	GCVarLegacyLinkerDiscovery,
	TEXT("When enabled we revert to the legacy behavior of trying find the FLinkerLoad from the owning UObject rather than the FArchive")
);

/** Utility for finding the FPackageTrailer associated from either a UObject or FArchive */
static const FPackageTrailer* FindPackageTrailer(UObject* Owner, FArchive& Ar)
{
	const FLinkerLoad* Linker = GCVarLegacyLinkerDiscovery ? GetLinkerLoadFromOwner(Owner) : Cast<FLinkerLoad>(Ar.GetLinker());
...
/** Utility for finding the package path associated from either a UObject or FArchive */
static FPackagePath FindPackagePath(UObject* Owner, FArchive& Ar)
{
	const FLinkerLoad* Linker = GCVarLegacyLinkerDiscovery ? GetLinkerLoadFromOwner(Owner) : Cast<FLinkerLoad>(Ar.GetLinker());
...
FArchive& operator<<(FArchive& Ar, FSharedBuffer& Buffer)
{
	...
	if (Ar.IsLoading())
	{
		...
		if (BufferLength >= 0)
		{
			...
			{
				Ar.SetError();
				UE_LOGF(LogSerialization, Error, "Not enough data remaining in archive (%ls) to load the payload. Bytes required: %lld vs bytes remaining %lld", *Ar.GetArchiveName(), BufferLength, SpaceRemaining);
			}
		}
	}
	else if (Ar.IsSaving())
...
static FGuid CreateUniqueGuid(const FGuid& NonUniqueGuid, const UObject* Owner, const TCHAR* DebugName)
{
	...
	{
		UE_LOGF(LogSerialization, Warning,
			"CreateFromBulkData received an invalid FGuid. A temporary one will be generated until the package is next re-saved! Package: '%ls'",
			DebugName);
	...
	const FGuid& FailedBulkDataId, const TCHAR* CallerName, bool bHandledByCreateUniqueGuid) const
{
	...
	if (Value == UE::BulkDataRegistry::ERegisterResult::AlreadyExists)
	{
		...
		if (bHandledByCreateUniqueGuid)
		{
			...
			{
				static FName BulkDataDuplicatesSystemName("BulkDataDuplicates");
				UE::SavePackageUtilities::OnAddResaveOnDemandPackage.Execute(BulkDataDuplicatesSystemName, OwnerPackage->GetFName());
				UE_LOGF(LogBulkDataRegistry, Display, "AddPackageToResave %ls: BulkData %ls collided with an ID in package %ls.",
			...
			else
			{
				...
				if (bSuppressWarning)
				{
					UE_LOGF(LogBulkDataRegistry, Verbose, "%ls", *LogMessage);
				}
				else
				{
					UE_LOGF(LogBulkDataRegistry, Warning, "%ls", *LogMessage);
				}
			}
			bMessageLogged = true;
		}
		...
		{
			UE_LOGF(LogBulkDataRegistry, Verbose,
				"%ls failed to register BulkData %ls from %ls in the BulkDataRegistry: it already exists with owner %ls.\n" "            Payload: %ls\n" "    ExistingPayload: %ls",
	...
	if (!bMessageLogged)
	{
		UE_LOGF(LogBulkDataRegistry, Warning,
			"%ls failed to register BulkData %ls from %ls in the BulkDataRegistry: %ls.",
...
void FEditorBulkData::Serialize(FArchive& Ar, UObject* Owner, bool bAllowRegister)
{
	...
	else if (Ar.IsPersistent() && !Ar.IsObjectReferenceCollector() && !Ar.ShouldSkipBulkData())
	{
		...
		else if (Ar.IsLoading())
		{
			...
			const FPackageTrailer* Trailer = FindPackageTrailer(Owner, Ar);
			...
			if (IsStoredInPackageTrailer())
			{
				...
				{
					UE_LOGF(LogSerialization, Warning, "Payload was saved with an invalid flag and required fixing. Please re-save the package! [%ls]", *GetDebugNameFromOwner(Owner));
			...
			else
			{
				...
				// TODO: Consider removing in 5.2+
				UE_CLOGF(IsDataVirtualized() && Ar.GetLinker() != nullptr, LogSerialization, Warning, "Payload is virtualized in an older format and should be re-saved! [%ls]", *GetDebugNameFromOwner(Owner));
			...
			if (!IsDataVirtualized())
			{
				...
				{
					PackagePath = FindPackagePath(Owner, Ar);
				}
				else
				{
					PackagePath.Empty();
...
FCompressedBuffer FEditorBulkData::LoadFromDisk() const
{
	...
	if (PackagePath.IsEmpty())
	{
		...
		UE_LOGF(LogSerialization, Error, "Cannot load a payload as the package path is empty!");
	...
	else if (!CanLoadDataFromDisk())
	{
		...
		{
			UE_LOGF(LogSerialization, Error, 
				"Cannot attempt to load the payload '%ls' from '%ls' as the package is no longer attached to the file on disk and the payload is in an old style bulkdata structure", 
		...
		{
			UE_LOGF(LogSerialization, Error, 
				"Cannot attempt to load the payload '%ls' from '%ls' as the package is no longer attached to the file on disk and the payload is not stored in a package trailer",
				*LexToString(PayloadContentId),
				*PackagePath.GetLocalFullPath(EPackageSegment::Header));
		...
		{
			UE_LOGF(LogSerialization, Error,
				"Cannot attempt to load the payload '%ls' from '%ls' as the package is no longer attached to the file on disk and the payload was not found in the package trailer",
				*LexToString(PayloadContentId),
				*PackagePath.GetLocalFullPath(EPackageSegment::Header));
...
FCompressedBuffer FEditorBulkData::LoadFromPackageFile() const
{
	...
	UE_LOGF(LogSerialization, Verbose, "Attempting to load payload from the package file '%ls'", *PackagePath.GetLocalFullPath(EPackageSegment::Header));
...
FCompressedBuffer FEditorBulkData::LoadFromPackageTrailer() const
{
	...
	UE_LOGF(LogSerialization, Verbose, "Attempting to load a payload via the package trailer from file '%ls'", *PackagePath.GetLocalFullPath(EPackageSegment::Header));
	...
	if (Trailer.TryLoadBackwards(*BulkArchive))
	{
		...
		UE_CLOGF(CompressedPayload.IsNull(), LogSerialization, Error, "Could not find the payload '%ls' in the package trailer of file '%ls'",
			*LexToString(PayloadContentId ), 
			*PackagePath.GetLocalFullPath(EPackageSegment::Header));
	...
	{
		UE_LOGF(LogSerialization, Error, "Could not read the package trailer from file '%ls'", *PackagePath.GetLocalFullPath(EPackageSegment::Header));
		return FCompressedBuffer();
	}
}
...
FCompressedBuffer FEditorBulkData::LoadFromSidecarFileInternal(ErrorVerbosity Verbosity) const
{
	...
	if (Result.Archive.IsValid() && Result.Format == EPackageFormat::Binary)
	{
		...
		{
			UE_CLOGF(Verbosity > ErrorVerbosity::None, LogSerialization, Error, "Unable to parse FPackageTrailer in '%ls'", *PackagePath.GetLocalFullPath(EPackageSegment::PayloadSidecar));
			return FCompressedBuffer();
		}

		FCompressedBuffer CompressedPayload = Trailer.LoadLocalPayload(PayloadContentId, *Result.Archive);

		UE_CLOGF(CompressedPayload.IsNull(), LogSerialization, Error, "Could not find the payload '%ls' in '%ls'",
			*LexToString(PayloadContentId),
...
FCompressedBuffer FEditorBulkData::LoadFromSidecarFile() const
{
	...
	UE_LOGF(LogSerialization, Verbose, "Attempting to load payload from the sidecar file: '%ls'",
		*PackagePath.GetLocalFullPath(EPackageSegment::PayloadSidecar));
	...
	if (CVarShouldAllowSidecarSyncing.GetValueOnAnyThread())
	{
		...
		{
			UE_LOGF(LogSerialization, Verbose, "Initial load from sidecar failed, attempting to sync the file: '%ls'",
				*PackagePath.GetLocalFullPath(EPackageSegment::PayloadSidecar));
			...
			{
				UE_LOGF(LogSerialization, Error, "Failed to find IVirtualizationSourceControlUtilities, unable to try and sync: '%ls'",
					*PackagePath.GetLocalFullPath(EPackageSegment::PayloadSidecar));
			}
		}
...
bool FEditorBulkData::SerializeData(FArchive& Ar, FCompressedBuffer& InPayload, const EFlags PayloadFlags) const
{
	...
	else if (Ar.IsLoading()) 
	{
		...
		{
			Ar.SetError();
			UE_LOGF(LogSerialization, Error, "Not enough data remaining in archive (%ls) to load the payload. Bytes required: %lld vs bytes remaining %lld", *Ar.GetArchiveName(), Size, SpaceRemaining);
...
FCompressedBuffer FEditorBulkData::PullData() const
{
	...
	if (System.IsEnabled())
	{
		...
		UE_CLOGF(PulledPayload.IsNull(), LogSerialization, Error, "Failed to pull payload '%ls'%ls",
			*LexToString(PayloadContentId),
			PostFixStatus(Flags));
	...
	{
		UE_LOGF(LogSerialization, Error, "Failed to pull payload '%ls' as the virtualization system is disabled%ls",
			*LexToString(PayloadContentId), 
			PostFixStatus(Flags));
...
FCompressedBuffer FEditorBulkData::GetDataInternal() const
{
	...
	if (IsDataVirtualized())
	{
		...
		UE_CLOGF(!IsDataValid(*this, CompressedPayload), LogSerialization, UE_CORRUPTED_DATA_SEVERITY, "Virtualized payload '%ls' is corrupt! Check the backend storage.", *LexToString(PayloadContentId));
	...
	else
	{
		...
		UE_CLOGF(CompressedPayload.IsNull(), LogSerialization, Error, "Failed to load payload '%ls", *LexToString(PayloadContentId));
		UE_CLOGF(!IsDataValid(*this, CompressedPayload), LogSerialization, UE_CORRUPTED_DATA_SEVERITY, "Payload '%ls' loaded from package '%ls' is corrupt! Check the package file on disk.",
			*LexToString(PayloadContentId),
...
bool FEditorBulkData::IsPayloadLoaded() const
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FEditorBulkData::IsPayloadLoaded);
...
bool FEditorBulkData::TryPayloadValidationForSaving(const FCompressedBuffer& PayloadForSaving, FLinkerSave* LinkerSave) const
{
	if (!IsDataValid(*this, PayloadForSaving) || (GetPayloadSize() > 0 && PayloadForSaving.IsNull()))
	{
		...
		{
			UE_LOGF(LogSerialization, Error, "%ls", *ErrorMessage);
		}
