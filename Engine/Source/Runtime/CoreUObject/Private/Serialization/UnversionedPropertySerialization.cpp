...
struct FDefaultStruct
{
	...
	FDefaultStruct(const uint8* InData, const UStruct* InStruct)
		: Data(InData)
	{
		StructStart = 0;
		StructEnd = 0;
		if (InData)
		{
			if (const UClass* StructAsClass = Cast<const UClass>(InStruct))
			{
				StructStart = StructAsClass->GetPropertiesStartOffset();
				StructEnd = InStruct->GetPropertiesSize() + StructStart;
			}
			else
			{
				StructEnd = InStruct->GetPropertiesSize();
			}
		}
	}
#else
...
public:
	...
	{
#if CACHE_UNVERSIONED_PROPERTY_SCHEMA
		return Offset >= Defaults.StructStart && Offset < Defaults.StructEnd ? Defaults.Data + Offset : nullptr;
#else
...
protected:
	...
	void SaveZeroMaskData(FStructuredArchive::FStream Stream, uint32 NumBits, const uint32* Data) const
	{
		...
		uint32 LastWordMask = ~0u >> ((32u - NumBits) % 32u); //-V1064
		if (NumBits <= 8)
		{
