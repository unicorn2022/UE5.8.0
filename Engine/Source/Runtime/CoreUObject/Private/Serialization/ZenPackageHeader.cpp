...
FZenPackageHeader FZenPackageHeader::MakeView(FMemoryView Memory)
{
	...
	if (!Error.IsEmpty())
	{
		UE_LOGF(LogStreaming, Fatal, "%ls", *Error);
	}
	return Result;
}
...
FZenPackageHeader FZenPackageHeader::MakeView(FMemoryView Memory, FString& OutError)
{
	...
	checkf(PackageSummary->HeaderSize <= Memory.GetSize(), TEXT("Corrupted package: HeaderSize=%u exceeds MemorySize=%llu"), PackageSummary->HeaderSize, Memory.GetSize());
