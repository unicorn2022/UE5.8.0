...
FPackagePath FPackagePath::FromPackageNameChecked(FStringView InPackageName)
{
	...
	{
		UE_LOGF(LogPackageName, Error, "FromPackageNameChecked: Invalid LongPackageName \"%.*ls\"",
...
bool FPackagePath::TryFromMountedName(FStringView InPackageNameOrFilePath, FPackagePath& OutPackagePath)
{
		...
		RelPath, ObjectName, Extension, CustomExtension))
	{
		if (ObjectName.Len() > 0)
		{
			UE_LOGF(LogPackageName, Warning,
				"FPackagePath::TryFromMountedName was passed an ObjectPath (%.*ls) rather than a PackageName or FilePath;" " it will be converted to the PackageName." " Accepting ObjectPaths is deprecated behavior and will be removed in a future release;" " TryFromMountedName will fail on ObjectPaths.",
...
void FPackagePath::AppendLocalFullPath(FStringBuilderBase& Builder, EPackageSegment PackageSegment) const
{
	...
	else
	{
		if (Extension == EPackageExtension::Unspecified)
		{
			...
			{
				UE_LOGF(LogPackageName, Warning,
					"GetLocalFullPath called on FPackagePath %ls which has an unspecified header extension," " and the path does not exist on disk. Assuming EPackageExtension::Asset.",
...
FPackagePath FPackagePath::FromLocalPath(FStringView InFilename, EPackageSegment& OutPackageSegment)
{
	...
	{
		UE_LOGF(LogPackageName, Error, "FromLocalPath: Failed converting filename \"%ls\" to package name", *FileNameString);
	}
	FPackagePath PackagePath;
	...
	EPackageExtension InExtension, FStringView InCustomExtension)
{
	...
	{
		UE_LOGF(LogPackageName, Error, "FromMountedComponents: Invalid FileName \"%ls\"", *FileNameString);
	}
	FPackagePath PackagePath;
	PackagePath.PackageName = FName(PackageNameString);
	PackagePath.HeaderExtension = InExtension;
...
FString FPackagePath::GetLocalFullPath(EPackageSegment PackageSegment) const
{
	if (!PackageName.IsNone())
	{
		...
		{
			UE_LOGF(LogPackageName, Error, "GetLocalFullPath: Failed converting package name \"%ls\" to file name", *PackageNameString);
		}
	}
	return TEXT("");
}
...
FString FPackagePath::GetLocalBaseFilenameWithPath() const
{
	if (!PackageName.IsNone())
	{
		...
		{
			UE_LOGF(LogPackageName, Error, "GetLocalBaseFilenameWithPath: Failed converting package name \"%ls\" to file name", *PackageNameString);
		}
	}
	return TEXT("");
}
