...
{
	// Resolve the leaf-most object that is UseOuter() == false, without any UseOuter() == true objects above it
	// This is needed as there may be a break in the stable reference chain, eg, Actor -> Component -> InstancedObject, 
	// may have a UseOuter() == true Component despite the InstancedObject itself being UseOuter() == false, in which 
	// case Actor needs to be the root object rather than InstancedObject
	RootObject = nullptr;
	{
		auto UseOuter = [](const UObject* Obj)
		{
			...
			const bool bIsClassCDO = Obj->HasAllFlags(RF_ClassDefaultObject) && Obj->GetClass()->GetDefaultObject(false) == Obj; // Guard against a trashed CDO
			const bool bIsDSO = Obj->HasAnyFlags(RF_DefaultSubObject);
			if (bIsClassCDO || bIsDSO)
			{
				return true;
			}

			for (OnUseOuterDelegate Delegate : GetOnUseOuterDelegates())
			{
				if (Delegate(Obj))
				{
					return true;
		...
		// First pass to find the correct root object
		for (UObject* CurObject = InObject; CurObject; CurObject = CurObject->GetOuter())
		{
			if (CurObject->IsCapturingAsRootObjectForTransaction())
			{
				// Explicitly a root object; stop walking
				if (!RootObject)
				{
					RootObject = CurObject;
				}
				break;
			}
			if (UseOuter(CurObject))
			{
				// This reference is unstable, so reset any current root object and keep going
				RootObject = nullptr;
			}
			else if (!RootObject)
			{
		...
		// Second pass to fill SubObjectHierarchyIDs with the objects under the root object
		for (UObject* CurObject = InObject; CurObject && CurObject != RootObject; CurObject = CurObject->GetOuter())
		{
			SubObjectHierarchyIDs.Add(CurObject->GetFName());
		}
	}
	check(RootObject);
...
TArray<FPersistentObjectRef::OnUseOuterDelegate>& FPersistentObjectRef::GetOnUseOuterDelegates()
{
	static TArray<FPersistentObjectRef::OnUseOuterDelegate> Delegates;
	return Delegates;
}
