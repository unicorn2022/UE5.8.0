...
const FChunkDependencyTreeNode* UChunkDependencyInfo::BuildChunkDependencyGraph(int32 HighestChunk)
{
	...
	for (int32 DepIndex = DependencyArray.Num() - 1; DepIndex >= 0; DepIndex --)
	{
		...
		if (Dep.ChunkID == Dep.ParentChunkID ||
			!ensureMsgf(Dep.ChunkID != 0, TEXT("Primary chunk 0 must not have a parent chunk, but ParentChunkID=%d was specified. This creates a cyclic dependency."), Dep.ParentChunkID))
		{
			// Remove cycles
...
void UChunkDependencyInfo::PostInitProperties()
{
	Super::PostInitProperties();

	// Handle deprecated INI section since this class has moved from UnrealEd into CoreUObject module
	{
		TArray<FString> DeprecatedDependencyStrings;
		if (GConfig->GetArray(TEXT("/Script/UnrealEd.ChunkDependencyInfo"), TEXT("DependencyArray"), DeprecatedDependencyStrings, GEngineIni))
		{
			UE_LOGF(LogChunkDependencyInfo, Warning, "Configuration [/Script/UnrealEd.ChunkDependencyInfo]:DependencyArray is deprecated since UE 5.8; Use [/Script/CoreUObject.ChunkDependencyInfo]:DependencyArray instead!");
			if (DependencyArray.IsEmpty())
			{
				UScriptStruct* StructType = FChunkDependency::StaticStruct();
				for (const FString& Entry : DeprecatedDependencyStrings)
				{
					StructType->ImportText(*Entry, &DependencyArray.AddDefaulted_GetRef(), nullptr, PPF_None, GLog, StructType->GetName());
				}
			}
			else
			{
				UE_LOGF(LogChunkDependencyInfo, Error,
					"Conflict between deprecated config [/Script/UnrealEd.ChunkDependencyInfo]:DependencyArray (Size=%d) and new config [/Script/CoreUObject.ChunkDependencyInfo]:DependencyArray (Size=%d)",
					DeprecatedDependencyStrings.Num(), DependencyArray.Num());
			}
		}
	}
}
...
int32 UChunkDependencyInfo::FindHighestSharedChunk(const TArray<int32>& ChunkIDs) const
{
	...
	{
		UE_LOGF(LogChunkDependencyInfo, Error, "Unable to find parent.");
		return 0;
	}
