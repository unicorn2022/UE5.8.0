...
void FPackageLocalizationManager::PerformLazyInitialization()
{
	...
	{
		UE_LOGF(LogPackageLocalizationManager, Display, "Localization packages disabled.");
		return;
	}
	...
	if (!bIsInitialized.load(std::memory_order_acquire) && LazyInitFunc)
	{
		...
		if (!ActiveCache.IsValid())
		{
			UE_LOGF(LogPackageLocalizationManager, Warning, "InitializeFromLazyCallback was bound to a callback that didn't initialize the active cache.");
		}
	}
}
...
FName FPackageLocalizationManager::FindLocalizedPackageName(const FName InSourcePackageName)
{
	...
	{
		UE_LOGF(LogPackageLocalizationManager, Warning, "Localized package requested for '%ls' before the package localization manager cache was ready. Falling back to a non-cached look-up...", *InSourcePackageName.ToString());
	...
	UE_CLOGF(!LocalizedPackageName.IsNone(), LogPackageLocalizationManager, Verbose, "Resolved localized package '%ls' for source package '%ls'", *LocalizedPackageName.ToString(), *InSourcePackageName.ToString());
