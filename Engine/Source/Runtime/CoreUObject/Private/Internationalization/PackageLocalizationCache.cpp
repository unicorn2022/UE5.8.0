...
void FPackageLocalizationCultureCache::ConditionalUpdateCache_NoLock()
{
	...
	for (const FString& SourceRootPath : PendingSourceRootPathsToSearch)
	{
		...
		for (const FString& PrioritizedCultureName : PrioritizedCultureNamesAndVerseIdentifiers)
		{
			...
			UE_LOGF(LogPackageLocalizationCache, Verbose, "Processing localized package path '%ls'...", *LocalizedRootPath);
		}
	}
	OwnerCache->FindLocalizedPackages(NewSourceToLocalizedPaths, SourcePackagesToLocalizedPackages);

	UE_LOGF(LogPackageLocalizationCache, Log, "Processed %d localized package path(s) for %d prioritized culture(s) in %0.6f seconds", PendingSourceRootPathsToSearch.Num(), PrioritizedCultureNames.Num(), FPlatformTime::Seconds() - CacheStartTime);
...
void FPackageLocalizationCultureCache::RemoveRootSourcePath(const FString& InRootPath)
{
	...
	// Make sure the StartsWith ends with a slash to avoid false positives (eg, "/Foo" also removing "/FooBar")
	const FString RootPathWithSlash = InRootPath / FString();

	// Remove all paths under this root
	for (auto It = SourcePathsToLocalizedPaths.CreateIterator(); It; ++It)
	{
		if (It->Key == InRootPath || It->Key.StartsWith(RootPathWithSlash))
		{
			It.RemoveCurrent();
			continue;
		}
	...
	for (auto It = SourcePackagesToLocalizedPackages.CreateIterator(); It; ++It)
	{
		...
		if (SourcePackageName.ToView().StartsWith(RootPathWithSlash))
		{
			It.RemoveCurrent();
			continue;
		}
...
FName FPackageLocalizationCultureCache::FindLocalizedPackageName(const FName InSourcePackageName)
{
	...
	UE_CLOGF(!LocalizedPackageName.IsNone(), LogPackageLocalizationCache, Verbose, "Found localized package '%ls' for source package '%ls'", *LocalizedPackageName.ToString(), *InSourcePackageName.ToString());
		...
		auto ReadAssetGroupClassSettings = [this](const TCHAR* InConfigLogName, const FString& InConfigFilename)
		{
			...
			if (const FConfigSection* AssetGroupClassesSection = GConfig->GetSection(TEXT("Internationalization.AssetGroupClasses"), false, InConfigFilename))
			{
				for (const auto& SectionEntryPair : *AssetGroupClassesSection)
				{
					...
					{
						UE_CLOGF(AssetClassGroupPair->Value != ClassName.GetAssetName(), LogPackageLocalizationCache, Warning, "Class '%ls' was already assigned to asset group '%ls', ignoring request to assign it to '%ls' from the %ls configuration.", *ClassName.ToString(), *AssetClassGroupPair->Value.ToString(), *GroupName.ToString(), InConfigLogName);
					}
					else
					{
						AssetClassesToAssetGroups.Add(MakeTuple(ClassName, GroupName));
						UE_LOGF(LogPackageLocalizationCache, Log, "Assigning class '%ls' to asset group '%ls' from the %ls configuration.", *ClassName.ToString(), *GroupName.ToString(), InConfigLogName);
					}
				}
			}
		};
