...
void VPackage::SerializeImpl(FAllocationContext Context, FStructuredArchiveVisitor& Visitor)
{
	...
	// We need to serialize the imports and register used types so that the interpreter can look up
	// the shapes of blueprint-generated classes at runtime.
	Visitor.Visit(UsedImports, TEXT("UsedImports"));

	if (Visitor.IsLoading())
	{
		GetOrCreateUPackage(Context);
		GlobalProgram->AddPackage(Context, *Name, *this);
	}
}
