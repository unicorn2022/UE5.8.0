...
void VCell::SetEmergentType(FAccessContext Context, uint32 Offset)
{
	Context.RunWriteBarrierNonNull(FHeap::EmergentTypeOffsetToPtr(Offset));
	EmergentTypeOffset = Offset;
}
...
FOpResult VCell::Freeze(FAllocationContext Context, VTask* Task)
{
	return GetEmergentType()->CppClassInfo->Freeze(Context, this, Task);
}
...
void VCell::SerializeLayout(FAllocationContext Context, const VCppClassInfo* CppClassInfo, VCell*& This, FArchive& Ar)
{
	...
	CppClassInfo->SerializeLayout(Context, This, Visitor);
}
...
FOpResult VCell::FreezeImpl(FAllocationContext Context, VTask* Task)
{
