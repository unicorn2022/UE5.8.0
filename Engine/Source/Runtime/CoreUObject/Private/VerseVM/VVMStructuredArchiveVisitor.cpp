...
void FStructuredArchiveVisitor::WriteValueBody(FStructuredArchiveRecord Record, VValue& InValue, bool bAllowBatch)
{
	...
	{
		// Serialized as VCell, but with TransparentRef type.  This must be
		// before `ExtractCell`, as `ExtractCell` also handles
		// `VTransparentRef`.
