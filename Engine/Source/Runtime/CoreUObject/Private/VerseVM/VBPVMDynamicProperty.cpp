...
FVerseDynamicProperty::FVerseDynamicProperty(FFieldVariant InOwner, const FName& InName)
	: TProperty(InOwner, InName)
{
}
...
bool FVerseDynamicProperty::Identical(TNotNull<const void*> A, const void* B, uint32 PortFlags) const
{
	return B && UE::Verse::FRuntimeTypeDynamic::Get().AreIdentical(A, B);
}

void FVerseDynamicProperty::SerializeItem(FStructuredArchive::FSlot Slot, TNotNull<void*> InValue, void const* Defaults) const
{
...
void FVerseDynamicProperty::ExportText_Internal(FString& ValueStr, TNotNull<const void*> ContainerOrPropertyPtr, EPropertyPointerType PropertyPointerType, const void* DefaultValue, UObject* Parent, int32 PortFlags, UObject* ExportRootScope) const
{
...
const TCHAR* FVerseDynamicProperty::ImportText_Internal(const TCHAR* InputString, TNotNull<void*> ContainerOrPropertyPtr, EPropertyPointerType PropertyPointerType, UObject* OwnerObject, int32 PortFlags, FOutputDevice* ErrorText) const
{
