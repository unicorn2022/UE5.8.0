...
void FRuntimeTypeDynamic::InitializeValueFrom(void* DestData, const void* SourceData, const FRuntimeType& SourceType) const
{
...
void FRuntimeTypeDynamic::ExportValueToText(FString& OutputString, const void* Data, const void* DefaultData, UObject* Parent, UObject* ExportRootScope) const
{
	...
	::Verse::IEngineEnvironment* Engine = ::Verse::VerseVM::GetEngineEnvironment();
	check(Engine);
	const bool bValueIsUninitialized = &Value.GetType() == &FDynamicallyTypedValue::NullType();
	if (!bValueIsUninitialized)
	{
		Engine->ExportRuntimeTypeToText(OutputString, static_cast<FRuntimeType&>(Value.GetType()));
		OutputString += TEXT('(');
		const void* const InnerDefaultValue =
			(DefaultValue && &DefaultValue->GetType() == &Value.GetType())
				? DefaultValue->GetDataPointer()
				: nullptr;
		static_cast<FRuntimeType&>(Value.GetType()).ExportValueToText(OutputString, Value.GetDataPointer(), InnerDefaultValue, Parent, ExportRootScope);
		OutputString += TEXT(')');
	}
}
...
void FRuntimeTypeDynamic::ForEachSubobject(const void* Data, TFunctionRef<void(const UObject*)> ObjectFunc) const
{
	const FDynamicallyTypedValue& Value = *static_cast<const FDynamicallyTypedValue*>(Data);
	Value.ForEachSubobject(ObjectFunc);
}
