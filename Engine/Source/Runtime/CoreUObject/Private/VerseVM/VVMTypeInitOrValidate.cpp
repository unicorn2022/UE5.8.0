...
void FInitOrValidateUEnum::SetEnums(TArray<TPair<FName, int64>>& InNames, UEnum::ECppForm InCppForm, UEnum::EUnderlyingType InUnderlyingType) const
{
	if (bIsValidating)
	{
		UEnum* Enum = GetUEnum();
		CheckValueMismatch(Enum->GetCppForm(), InCppForm, TEXT("CppForm"));
		CheckValueMismatch(Enum->GetUnderlyingType(), InUnderlyingType, TEXT("UnderlyingType"));
	...
	{
		GetUEnum()->SetEnums(InNames, InCppForm, InUnderlyingType, EEnumFlags::None, UEnum::EAddMaxKeyIfMissing::Yes);
	}
}
