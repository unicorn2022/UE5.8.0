...
ECompares VValueObject::EqualImpl(FAllocationContext Context, VCell* Other, const TFunction<void(VValue, VValue)>& HandlePlaceholder)
{
	if (!IsStruct())
	{
		// Consider X = X to decidably succeed.  This matches `VValue::Equal`.
		if (this == Other)
		{
			return ECompares::Eq;
		}
		VValueObject* OtherValueObject = Other->DynamicCast<VValueObject>();
		if (!OtherValueObject)
		{
			return ECompares::Neq;
		}
		if (OtherValueObject->IsStruct())
		{
			return ECompares::Neq;
		}
		VClass& Class = GetClass();
		if (&Class != &OtherValueObject->GetClass())
		{
			return ECompares::Neq;
		}
		if (Class.IsUnique())
	...
	for (VShape::FieldsMap::TConstIterator It = EmergentType->Shape->Fields.CreateConstIterator(); It; ++It)
	{
...
VValue VValueObject::MeltImpl(FAllocationContext Context)
{
	...
	else
	{
		for (auto It = EmergentType.Shape->CreateFieldsIterator(); It; ++It)
		{
			...
			FOpResult Result = NewObject.SetField<EWriteMode::NonTransactional>(Context, *It->Key.Get(), MeltResult);
...
FOpResult VValueObject::FreezeImpl(FAllocationContext Context, VTask* Task)
{
	V_DIE_UNLESS(IsStruct());
	...
	for (uint64 I = 0; I != NumIndexedFields; ++I)
	{
		VValue FieldValue = UnwrapTransparentRef(
			...
			[&](VValue FieldValue) {
				...
				Data[I].Set(Context, FieldValue);
			});
		FOpResult Result = VValue::Freeze(Context, FieldValue, Task);
...
void VValueObject::SerializeImpl(FAllocationContext Context, FStructuredArchiveVisitor& Visitor)
{
	...
	if (Visitor.IsLoading())
	{
		...
		for (TPair<TWriteBarrier<VUniqueString>, TWriteBarrier<VValue>>& Field : Fields)
		{
			...
			Verse::FOpResult Result = SetField<EWriteMode::NonTransactional>(Context, *Field.Key, Field.Value.Get());
			V_DIE_UNLESS(Result.IsReturn());
		}
