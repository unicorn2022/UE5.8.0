...
FDelegateHandle FHeap::OnLLMTickHandle;
...
static void HandlePASAssertionFailure(const char* Filename, int Line, const char* Function, const char* Expression, const uint64_t* MaybeExtra)
{
	...
	{
		UE_LOGF(LogVerseGC, Fatal, "Assertion failed: %s [File:%s] [Line: %i] \nlibpas assert in %s failed\n", Expression, Filename, Line, Function);
	}
#endif
}
...
static void HandlePASPanic(const char* Format, va_list ArgList)
{
	...
	UE_LOGF(LogVerseGC, Fatal, "libpas panic: %s", *Builder);
}
...
void FHeap::Initialize()
{
	if (!bIsInitialized)
	{
		...
		OnLLMTickHandle = FCoreDelegates::OnLowLevelMemTrackerTick.AddStatic(&FHeap::OnLowLevelMemTrackerTick);
...
void FHeap::Deinitialize()
{
	if (bIsInitialized)
	{
		...
		FCoreDelegates::OnLowLevelMemTrackerTick.Remove(OnLLMTickHandle);
	}
}

void FHeap::OnLowLevelMemTrackerTick()
{
#if ENABLE_LOW_LEVEL_MEM_TRACKER
	pas_heap_summary HeapSummary = pas_all_heaps_lock_and_compute_total_non_utility_summary(pas_lock_is_not_held);

	FLowLevelMemTracker::Get().SetTagAmountForTracker(ELLMTracker::Platform, LLM_TAG_NAME(Libpas), ELLMTagSet::None, HeapSummary.committed, /*bAddToTotal*/ false);
	FLowLevelMemTracker::Get().SetTagAmountForTracker(ELLMTracker::Default, LLM_TAG_NAME(Libpas), ELLMTagSet::None, HeapSummary.allocated, /*bAddToTotal*/ false);
#endif
}
...
void FHeap::CollectorThreadBody(FIOContext Context)
{
	...
	UE_LOGF(LogVerseGC, Verbose, "GC thread starting!");
	for (;;)
	{
...
{
	UE_LOGF(LogVerseGC, Verbose, "GC marking terminated!");
...
void FHeap::BeginCollection(FIOContext Context)
{
	...
	UE_LOGF(LogVerseGC, Verbose, "GC received trigger for GC %llu", CompletedCycleVersion + 1);
	...
	UE_LOGF(LogVerseGC, Verbose, "Starting with live cell bytes %zu, native bytes %zu, total bytes %zu", LiveCellBytesAtStart, CurrentLiveNativeBytes, LiveCellBytesAtStart + CurrentLiveNativeBytes);
}
...
void FHeap::Terminate()
{
	...
	// TODO(SOL-8993): Concurrent GC incorrectly garbage collects memory that is
	// in the AutoRTFM write log. Until this is fixed, we depend on the
	// "FrankenGC" being enabled, which guarantees that GC only happens when
	// there are no transactions in-flight.
	AUTORTFM_SANITIZER_ONLY(V_DIE_IF(AutoRTFM::Sanitizer::AnyTransactionsInFlight()));
...
void FHeap::EndCollection(FIOContext Context)
{
	...
	if (CompletedCycleVersion == RequestedCycleVersion)
	{
		...
		if (LiveBytes >= NewTrigger)
		{
			...
			UE_LOGF(
				LogVerseGC, Verbose,
				"Swept bytes %zu, kept native %zu, survived %zu, new live %zu, beginning another GC immediately (due to new trigger %zu)",
		...
		else
		{
			...
			UE_LOGF(
				LogVerseGC, Verbose,
				"Swept bytes %zu, kept native %zu, survived %zu, new live %zu, new trigger %zu",
	...
	else
	{
		UE_LOGF(
			LogVerseGC, Verbose,
			"Swept bytes %zu, kept native %zu, survived %zu, new live %zu, beginning another GC immediately (per pending request)",
...
FCollectionCycleRequest FHeap::RequestCollectionCycle(uint64 DesiredRequestCompleteDelta)
{
	...
	CheckCycleTriggerInvariants();
	UE_LOGF(LogVerseGC, Verbose, "Requested cycle %llu", RequestedCycleVersion);
	return FCollectionCycleRequest(RequestedCycleVersion);
}

void FHeap::LiveBytesTriggerCallback()
{
	UE_LOGF(LogVerseGC, Verbose, "Trigger callback called with live bytes %zu, trigger %zu",
...
void FHeap::EnableExternalControl(FIOContext Context)
{
	...
	NormalizeWithoutThreadingAtCollectionStart();
	while (!bIsTerminated || RequestedCycleVersion > CompletedCycleVersion)
	{
		ConditionVariable.Wait(Mutex);
	}
...
void FHeap::ExternallySynchronouslyStartGC(FIOContext Context)
{
...

	{
		...
		{
			UE_LOGF(LogVerseGC, Verbose, "RequestedCycleVersion = %llu, CompletedMarkingCycleVersion = %llu, CompletedCycleVersion = %llu, incrementing RequestedCycleVersion", RequestedCycleVersion, CompletedMarkingCycleVersion, CompletedCycleVersion);
			RequestedCycleVersion++;
			verse_heap_live_bytes_trigger_threshold = SIZE_MAX;
		}
		else
		{
			UE_LOGF(LogVerseGC, Verbose, "RequestedCycleVersion = %llu, CompletedMarkingCycleVersion = %llu, CompletedCycleVersion = %llu, not incrementing anything", RequestedCycleVersion, CompletedMarkingCycleVersion, CompletedCycleVersion);
		}
		CheckCycleTriggerInvariants();
