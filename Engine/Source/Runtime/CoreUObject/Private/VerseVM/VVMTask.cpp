...
VTask::VTask(FAllocationContext Context, VEmergentType& TaskEmergentType, FOp* YieldPC, VFrame* YieldFrame, VTask* YieldTask, VTask* Parent, VFrame* InRootFrame)
	: VValueObject(Context, TaskEmergentType)
	, TIntrusiveTree(Context, Parent, EWriteMode::Transactional)
	, ResumeSlot(Context, nullptr)
	, YieldPC(YieldPC)
	, YieldFrame(Context, YieldFrame)
	, YieldTask(Context, YieldTask)
	, RootFrame(Context, InRootFrame)
{
}
...
bool VTask::RemoveFromTaskGroup(FAllocationContext Context)
{
	...
	if (TaskGroup)
	{
		...
		TaskGroup.ResetTransactionally();
	}
	return bDidRemove;
}
...
void VTask::BindStruct(FAllocationContext Context, VClass& TaskClass)
{
	...
	Verse::VNativeProcedure::SetThunk(VerseNativePackage, VerseScopePath, "(/Verse.org/Concurrency/task:)Active", &VTask::ActiveImpl);
	Verse::VNativeProcedure::SetThunk(VerseNativePackage, VerseScopePath, "(/Verse.org/Concurrency/task:)Completed", &VTask::CompletedImpl);
	Verse::VNativeProcedure::SetThunk(VerseNativePackage, VerseScopePath, "(/Verse.org/Concurrency/task:)Canceling", &VTask::CancelingImpl);
	Verse::VNativeProcedure::SetThunk(VerseNativePackage, VerseScopePath, "(/Verse.org/Concurrency/task:)Canceled", &VTask::CanceledImpl);
	Verse::VNativeProcedure::SetThunk(VerseNativePackage, VerseScopePath, "(/Verse.org/Concurrency/task:)Unsettled", &VTask::UnsettledImpl);
	Verse::VNativeProcedure::SetThunk(VerseNativePackage, VerseScopePath, "(/Verse.org/Concurrency/task:)Settled", &VTask::SettledImpl);
	Verse::VNativeProcedure::SetThunk(VerseNativePackage, VerseScopePath, "(/Verse.org/Concurrency/task:)Uninterrupted", &VTask::UninterruptedImpl);
	Verse::VNativeProcedure::SetThunk(VerseNativePackage, VerseScopePath, "(/Verse.org/Concurrency/task:)Interrupted", &VTask::InterruptedImpl);

	Verse::VNativeProcedure::SetThunk(VerseNativePackage, VerseScopePath, "Await", &VTask::AwaitImpl);
	Verse::VNativeProcedure::SetThunk(VerseNativePackage, VerseScopePath, "Cancel", &VTask::CancelImpl);
...
FOpResult VTask::ActiveImpl(FRunningContext Context, VValue Scope, VNativeProcedure::Args Arguments)
{
...
FOpResult VTask::CompletedImpl(FRunningContext Context, VValue Scope, VNativeProcedure::Args Arguments)
{
	V_DIE_UNLESS(Scope.IsCellOfType<VTask>());
	VTask* Self = &Scope.StaticCast<VTask>();
...
FOpResult VTask::CancelingImpl(FRunningContext Context, VValue Scope, VNativeProcedure::Args Arguments)
{
	V_DIE_UNLESS(Scope.IsCellOfType<VTask>());
	VTask* Self = &Scope.StaticCast<VTask>();
...
FOpResult VTask::CanceledImpl(FRunningContext Context, VValue Scope, VNativeProcedure::Args Arguments)
{
	V_DIE_UNLESS(Scope.IsCellOfType<VTask>());
	VTask* Self = &Scope.StaticCast<VTask>();
...
FOpResult VTask::UnsettledImpl(FRunningContext Context, VValue Scope, VNativeProcedure::Args Arguments)
{
	V_DIE_UNLESS(Scope.IsCellOfType<VTask>());
	VTask* Self = &Scope.StaticCast<VTask>();
...
FOpResult VTask::SettledImpl(FRunningContext Context, VValue Scope, VNativeProcedure::Args Arguments)
{
	V_DIE_UNLESS(Scope.IsCellOfType<VTask>());
	VTask* Self = &Scope.StaticCast<VTask>();
...
FOpResult VTask::UninterruptedImpl(FRunningContext Context, VValue Scope, VNativeProcedure::Args Arguments)
{
	V_DIE_UNLESS(Scope.IsCellOfType<VTask>());
	VTask* Self = &Scope.StaticCast<VTask>();
...
FOpResult VTask::InterruptedImpl(FRunningContext Context, VValue Scope, VNativeProcedure::Args Arguments)
{
	V_DIE_UNLESS(Scope.IsCellOfType<VTask>());
	VTask* Self = &Scope.StaticCast<VTask>();
...
FOpResult VTask::AwaitImpl(FRunningContext Context, VValue Scope, VNativeProcedure::Args Arguments)
{
	V_DIE_UNLESS(Scope.IsCellOfType<VTask>());
	VTask* Self = &Scope.StaticCast<VTask>();
...
FOpResult VTask::CancelImpl(FRunningContext Context, VValue Scope, VNativeProcedure::Args Arguments)
{
	V_DIE_UNLESS(Scope.IsCellOfType<VTask>());
...
bool VTask::CancelChildren(FRunningContext Context)
{
	...
	AutoRTFM::TScopedAssign GuardRunning(bRunning, true);
...
void VTask::TerminateRecursively(FAllocationContext Context)
{
	...
	// These loops are structured similarly to ExecNativeDefer and ExecNativeAwaits, but the hooks are invoked non-transactionally.
	while (NativeDefers)
	{
		VTaskNativeHook& Hook = *NativeDefers;
		NativeDefers.Set(Context, Hook.Next.Get());
...
void VTask::ExecNativeDefer(FAllocationContext Context)
{
	...
	{
		VTaskNativeHook& Hook = *NativeDefers;
		NativeDefers.SetTransactionally(Context, Hook.Next.Get());
		Hook.InvokeTransactionally(Context, this);
	}
}
...
void VTask::ExecNativeAwaits(FAllocationContext Context)
{
	...
	NativeAwaitsTail.ResetTransactionally();
}
...
void VTask::InitializeGlobals(FAllocationContext Context)
{
	...
	Emitter.EndTask(EmptyLocation(), Emitter.NoRegister(), Emitter.NoRegister(), EndTaskHelper.ResultRegister, FValueOperand(), FValueOperand());
...
void VTask::VisitReferencesImpl(TVisitor& Visitor)
{
	...
	Visitor.Visit(NativeDefers, TEXT("NativeDefers"));
	...
	Visitor.Visit(RootFrame, TEXT("RootFrame"));
