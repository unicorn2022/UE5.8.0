...
namespace UE
{
	...
	struct FDynamicallyTypedValueType
	{
		...
		virtual void InitializeValueFrom(void* DestData, const void* SourceData, const Verse::FRuntimeType& SourceType) const = 0;
		...
		virtual void ForEachSubobject(const void* Data, TFunctionRef<void(const UObject*)> ObjectFunc) const = 0;
	...
	// A value stored in some uninterpreted memory and a pointer to a type that contains methods to interpret it.
	struct FDynamicallyTypedValue
	{
		...
		// Traverses the data based on the type of value that is stored and invokes ObjectFunc for each UObject that is found.
		void ForEachSubobject(TFunctionRef<void(const UObject*)> ObjectFunc) const
		{
			Type->ForEachSubobject(GetDataPointer(), ObjectFunc);
		}
