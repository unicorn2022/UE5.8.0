...
enum class ECoreRedirectFlags : uint32
{
	...
	Option_Iterate =			0x08000000, // Indicates that the result of applying this redirect should be iteratively redirected.
...
struct FCoreRedirect
{
	...
	COREUOBJECT_API int32 GetTypeHash() const;

	friend int32 GetTypeHash(const FCoreRedirect& CoreRedirect)
	{
		return CoreRedirect.GetTypeHash();
	}

	bool operator==(const FCoreRedirect& Other) const
	{
...
struct FCoreRedirects
{
	 ...
	 * redirecting from or to the package. It doesn't include the GlobalRedirects and doesn't include the object redirects.
	...
	/** Calculate the hash of the object redirectors affecting a package, if any. */
	static COREUOBJECT_API void GetObjectRedirectHashAffectingPackage(const FName& PackageName, FBlake3& Hasher);

	/** Add the given Source->Path redirector to the summary used for GetObjectRedirectHashAffectingPackage. */
	static COREUOBJECT_API void RecordAddedObjectRedirector(const FSoftObjectPath& Source, const FSoftObjectPath& Dest);

	/** Remove the given Source->Path redirector to the summary used for GetObjectRedirectHashAffectingPackage. */
	static COREUOBJECT_API void RecordRemovedObjectRedirector(const FSoftObjectPath& Source, const FSoftObjectPath& Dest);

	/** Thread-safe delegate broadcasted when a global core redirect is added to FRedirectionSummary. It can be called from any thread. */
	DECLARE_TS_MULTICAST_DELEGATE_OneParam(FGlobalCoreRedirectAddedEvent, const FCoreRedirect& CoreRedirect);
	
	/** 
	 * Return the delegate called when a global core redirect is added to FRedirectionSummary.
	 * Can return nullptr if the current thread context does not use a FRedirectionSummary.
	 */
	static COREUOBJECT_API FGlobalCoreRedirectAddedEvent* GetOnGlobalCoreRedirectAdded();
