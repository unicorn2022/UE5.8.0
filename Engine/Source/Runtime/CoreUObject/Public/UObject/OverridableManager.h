...
public:
	 ...
	 * @param bSkipAddedState Set to True if the actual state should be determined instead of just returning "Added", if this object is added.
	 * @param ConsiderSubObjectForOverriddenState Predicate to decide whether a given SubObject should be included in the state determination.
	...
	COREUOBJECT_API EOverriddenState GetOverriddenState(TNotNull<UObject*> Object, 
		...
		bool bSkipAddedState = false, const TFunction<bool(TNotNull<const UObject*>)>& ConsiderSubObjectForOverriddenState = {});
