...
COREUOBJECT_API FText AssetAccessSpecifierAsText(EAssetAccessSpecifier);
...
public:
	...
	*/
	...
	{
		const FName LoadedPathName = LoadedPath.GetPackageFName();
		return !LoadedPathName.IsNone() && (LoadedPathName != GetFName());
	}
...
public:
	...
	*
	* @param	InPackage		the package to save the asset into
	* @param	InAsset			the asset that should be saved into the package
	...
	*
	* @return	A task containing a FSavePackageResultStruct enum value with the result of saving a package as well as extra data
	*
	* @note         Some tasks might need to execute on the gamethread. If the engine isn't ticking, it might be required
        *               to pump the gamethread task queue manually. (i.e. FTaskGraphInterface::Get().ProcessThreadUntilIdle(ENamedThreads::GameThread) or similar).
	*/
	COREUOBJECT_API static UE::Tasks::TTask<FSavePackageResultStruct> SaveAsync(UPackage* InPackage, UObject* InAsset, const TCHAR* InFilename,
	...
	*/
	...
	/**
	 * Save a list of packages concurrently.
