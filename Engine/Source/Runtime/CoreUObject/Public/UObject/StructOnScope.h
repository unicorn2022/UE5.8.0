...
public:
	...
	void InitializeFromChecked(const FStructOnScope& InOther)
	{
		...
		{
			UE_LOGF(LogClass, Fatal, "Initialize of %ls to %ls failed", *InOther.GetStruct()->GetName(), *TBaseStructure<T>::Get()->GetName());
		}
	}
	...
	void InitializeFromChecked(FStructOnScope&& InOther)
	{
		...
		{
			UE_LOGF(LogClass, Fatal, "Initialize of %ls failed", *TBaseStructure<T>::Get()->GetName());
		}
	}
	...
	U* CastChecked()
	{
		...
		if (!IsValid())
		{
			UE_LOGF(LogClass, Fatal, "Cast of nullptr to %ls failed", *TBaseStructure<U>::Get()->GetName());
			return Result;
		}
		...
		if (!Result)
		{
			UE_LOGF(LogClass, Fatal, "Cast of %ls to %ls failed", *TBaseStructure<T>::Get()->GetName(), *TBaseStructure<U>::Get()->GetName());
		}
		return Result;
	}
