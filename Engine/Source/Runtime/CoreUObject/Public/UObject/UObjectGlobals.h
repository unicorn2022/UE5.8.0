 ...
 * @see IsSavingPackage()
 */
UE_DEPRECATED(5.8, "Use the IsSavingPackage() function instead.")
...
{
	/**
	 * Return if the current thread is in a saving scope.
	 * @returns true the current thread is inside a saving scope.
	 */
	COREUOBJECT_API bool IsSavingPackage();

	/**
	 * Return if the passed in package is currently saving,
	 * or if if the current thread is in a saving scope if InOuter == nullptr.
	 * @param InOuter The object which we want to check if its package is saving
	 * @returns true if the package is saving, or if the current thread is in a saving scope if InOuter == nullptr.
	 */
	COREUOBJECT_API bool IsSavingPackage(UObject* InOuter);

	/**
	 * Used to toggle the value which is needed is some rare circumstance.
	 *
	 * @param bIsSavingPackage The value we want to set for the current thread.
	 * 
	 * @note Should be used with TGuardValueAccessors for RAII style restore of the old flag.
	 */
	COREUOBJECT_API void SetIsSavingPackage(bool bIsSavingPackage);
 ...
 * It will not assert on UE::IsSavingPackage() or IsGarbageCollectingAndLockingUObjectHashTables(). If called from within package saving code or GC, will return nullptr
 * This will only find top level packages or subobjects nested directly within a passed in outer.
 *
 * @param	Class			The to be found object's class
 * @param	InOuter			Outer object to look inside, if null this will only look for top level packages
...
/** Internal version of StaticFindObject that will not assert on UE::IsSavingPackage() or IsGarbageCollectingAndLockingUObjectHashTables() */
...
/**
 * Tries to find an object in memory. This version uses FTopLevelAssetPath to find the object.
 * Version of StaticFindObject that will not assert on UE::IsSavingPackage() or IsGarbageCollectingAndLockingUObjectHashTables()
 *
 * @param	Class			The to be found object's class
 * @param	ObjectPath		FName pair representing the outer package object and the inner top level object (asset)
 * @param	Flags			Flags which control the search
 ...
 * Fast version of StaticFindAllObjects that relies on the passed in FName being the object name without any group/package qualifiers.
 * This will find all objects matching the specified name and class.
 * This version of StaticFindAllObjectsFast will not assert on UE::IsSavingPackage() or IsGarbageCollecting()
 * 
 * @param	OutFoundObjects	Array of objects matching the search parameters
 * @param	ObjectClass		The to be found object's class
 * @param	ObjectName		Object name to look for relative to InOuter
...
/**
 * Tries to find all objects matching the search parameters in memory. This will handle fully qualified paths of the form /path/packagename.object:subobject and resolve references for you.
 * This version of StaticFindAllObjects will not assert on UE::IsSavingPackage() or IsGarbageCollecting()
 * 
 * @param	OutFoundObjects	Array of objects matching the search parameters
 * @param	Class			The to be found object's class
 * @param	Name			The object path to search for an object, relative to InOuter
 ...
 * This version of StaticFindFirstObject will not assert on UE::IsSavingPackage() or IsGarbageCollecting()
 * If multiple objects share the same name the returned object is random and not based on its time of creation unless otherwise specified in Options (see EFindFirstObjectOptions::NativeFirst)
 * This function is slow and should not be used in performance critical situations.
 * 
 * @param	Class						The to be found object's class
...
public:
	...
	FLoadPackageAsyncProgressDelegate(DelegateType&& InDelegate, uint32 InMask = MAX_uint32)
		: Delegate(Forward<DelegateType>(InDelegate))
		, Mask(InMask)
	{}
...
*/
UE_DEPRECATED(5.8, "This function is deprecated and will be removed. Do not use.")
COREUOBJECT_API void CancelAsyncLoading();

/**
* Returns true if the event driven loader is enabled in the current build
*/
UE_DEPRECATED(5.8, "This function is deprecated and will be removed. Use FPlatformProperties::RequiresCookedData() directly if appropriate.")
 ...
 */
AUTORTFM_DISABLE COREUOBJECT_API void GatherUnreachableObjects(bool bForceSingleThreaded);
 ...
 */
struct AUTORTFM_OPEN FUniversallyUniqueObjectNameGenerator
{
...
public:
	...
	*/
	 ...
	 */
	UE_INTERNAL void AddPropertyToSubobjectExclusionList(const FProperty* Property)
	{
		SubobjectInstantiationExclusionList.Add(Property);
	}

	/**
	 * Checks if a member variable property should not be instanced for this object. Not intended for use outside of engine code.
	 */
	UE_INTERNAL bool IsPropertyInSubobjectExclusionList(const FProperty* Property) const
	{
...
/**
 * Find an object without asserting on UE::IsSavingPackage() or IsGarbageCollectingAndLockingUObjectHashTables()
...
/**
 * Find an optional object with proper handling of potential ambiguity without asserting on UE::IsSavingPackage() or IsGarbageCollecting()
 * @see StaticFindFirstObject()
 */
template< class T >
...
class TObjectArrayIterator
{
	...
	{
		UE_LOGF(LogUObjectGlobals, Log, "Item %ls",*It->GetFullName());
	}
	*/
public:
	/**
...
struct FCoreUObjectDelegates
{
	...
	/** Called when an object that was loaded is renamed. This gets called in both the editor and standalone game editor builds, for every renamed loaded object */
	static COREUOBJECT_API FOnObjectRenamed OnLoadedObjectRenamed;
	...
	DECLARE_TS_MULTICAST_DELEGATE_OneParam(FOnObjectsReplaced, const FReplacementObjectMap&);
	...
	/** Callback when a package finishes loading via the async loader. Available in all builds (including shipping).
	 *  This fires for each package that completes loading, providing the loaded UPackage.
	 *  Note: This is called from the game thread after the package is fully loaded and post-loaded. */
	DECLARE_MULTICAST_DELEGATE_OneParam(FOnPackageLoadCompleted, UPackage*);
	static COREUOBJECT_API FOnPackageLoadCompleted OnPackageLoadCompleted;

	DECLARE_MULTICAST_DELEGATE_OneParam(FOnHotfixableAssetLoaded, UObject*);
	/** Fired after an asset of a registered hotfixable class finishes loading. */
	static COREUOBJECT_API FOnHotfixableAssetLoaded OnHotfixableAssetLoaded;
	/**
	 * Registers a class whose instances should trigger OnHotfixableAssetLoaded when loaded.
	 * Only the exact class is matched; derived and parent classes are not included.
	 * Register each concrete class separately. Reference-counted to support multiple callers.
	 */
	static COREUOBJECT_API void RegisterHotfixableAssetClass(UClass* Class);
	/**
	 * Unregisters a previously registered hotfixable class. Only affects the exact class passed.
	 */
	static COREUOBJECT_API void UnregisterHotfixableAssetClass(UClass* Class);
	/** Returns whether the exact class (not parents or subclasses) is registered as hotfixable. */
	static COREUOBJECT_API bool IsHotfixableAssetClass(UClass* Class);

	/** Called when new sparse class data has been created (and the base data initialized) for the given class */
	DECLARE_TS_MULTICAST_DELEGATE_ThreeParams(FOnPostInitSparseClassData, UClass*, UScriptStruct*, void*);
...
#define ASSET_LOG_FORMAT_STRING TEXT(ASSET_LOG_FORMAT_STRING_ANSI)
	...
	{ \
		...
		{ \
			UE_LOG_EXPAND_IS_FATAL(Verbosity, UE_EMPTY, if (!CategoryName.IsSuppressed(ELogVerbosity::Verbosity))) \
			{ \
...
namespace UECodeGen_Private
{
	enum class EPropertyGenFlags : uint8
	{
		...
		VerseCell         = 0x26,
		VerseType         = 0x27,
		VerseLegacyValue  = 0x28, // FVRestValueProperty with a legacy property
...
// Macros with the standard field layouts
#define UE_PROPERTY_PARAMS_BASE_FIELDS \
	const char*					NameUTF8; \
...
#define UE_PROPERTY_PARAMS_BASE_WITHOUT_OFFSET_FIELDS \
	UE_PROPERTY_PARAMS_BASE_FIELDS

#define UE_PROPERTY_PARAMS_BASE_WITH_OFFSET_FIELDS \
	UE_PROPERTY_PARAMS_BASE_FIELDS \
	uint16						Offset;

#if WITH_METADATA
#define UE_PROPERTY_PARAMS_METADATA_FIELDS \
	uint16						NumMetaData; \
...
#define UE_PROPERTY_PARAMS_METADATA_FIELDS
	...
	{
		UE_PROPERTY_PARAMS_BASE_WITH_OFFSET_FIELDS;
		UClass*					(*ClassFunc)(ETypeConstructPhase);
		UE_PROPERTY_PARAMS_METADATA_FIELDS;
	};
	...
	struct FClassPropertyParams // : FObjectPropertyParams
	{
		UE_PROPERTY_PARAMS_BASE_WITH_OFFSET_FIELDS;
		UClass*					(*ClassFunc)(ETypeConstructPhase);
		UClass*					(*MetaClassFunc)(ETypeConstructPhase);		
		UE_PROPERTY_PARAMS_METADATA_FIELDS;
	};
	...
	{
		UE_PROPERTY_PARAMS_BASE_WITH_OFFSET_FIELDS;
		UClass*					(*InterfaceClassFunc)(ETypeConstructPhase);
		UE_PROPERTY_PARAMS_METADATA_FIELDS;
	};
	...
	{
		UE_PROPERTY_PARAMS_BASE_WITH_OFFSET_FIELDS;
		UScriptStruct*			(*ScriptStructFunc)(ETypeConstructPhase);
		UE_PROPERTY_PARAMS_METADATA_FIELDS;
	};
	...
	{
		UE_PROPERTY_PARAMS_BASE_WITH_OFFSET_FIELDS;
		UFunction*				(*SignatureFunctionFunc)(ETypeConstructPhase);
		UE_PROPERTY_PARAMS_METADATA_FIELDS;
	};
	...
	typedef FGenericPropertyParams FObjectPropertyParamsWithoutClass;
	...
	typedef FGenericPropertyParams FVerseCellValuePropertyParams;
	typedef FGenericPropertyParams FVerseStringPropertyParams;

#undef UE_PROPERTY_PARAMS_BASE_FIELDS
#undef UE_PROPERTY_PARAMS_BASE_WITHOUT_OFFSET_FIELDS
#undef UE_PROPERTY_PARAMS_BASE_WITH_OFFSET_FIELDS
#undef UE_PROPERTY_PARAMS_METADATA_FIELDS

	struct FFunctionBaseParams
	{
		FTypeConstructFunc*					OuterFunc;
		UFunction*                        (*SuperFunc)(ETypeConstructPhase);
		const char*                         NameUTF8;
	...
	struct FEnumParams
	{
		...
		uint8                       UnderlyingType; // this is of type UEnum::EUnderlyingType
#if WITH_METADATA
		uint16                      NumMetaData;
		const FMetaDataPairParam*   MetaDataArray;
#endif
	...
	struct FVerseEnumParams : FEnumParams
	{
		...
		const FVniTypeDesc*			NativeTypeDesc;
	};

	struct FStructParams
	{
		FTypeConstructFunc*					OuterFunc;
		UScriptStruct*                    (*SuperFunc)(ETypeConstructPhase);
	...
	{
		const char*                        NameUTF8;
		FTypeConstructFunc* const*		   SingletonFuncArray;
		int32                              NumSingletons;
...
#if WITH_VERSE_VM
#define IF_WITH_VERSE_VM(x, y) UE_REMOVE_OPTIONAL_PARENS(x)
#else
#define IF_WITH_VERSE_VM(x, y) UE_REMOVE_OPTIONAL_PARENS(y)
#endif
...
{
	// Special tags for constructing classes such as UClass that are self-referential
	// i.e. the class of class is class
	enum class EClass { Class };
	enum class EObject { Object };

#if WITH_METADATA
	struct FMetaData
	{
		// NOTE: These should be changed to UTF8CHAR when UHT is modified to emit \u and \U
		// for unicode characters rather than \x
		const char* NameUTF8;
		const char* ValueUTF8;
	};
#endif
	...
	// FFieldOwner cannot be used directly because we can't use the tagged pointer/union code in constexpr
	...
	// Parameter wrapper for either an object pointer or an encoded reference to an object in another module to be linked
	// TCompiledInObjectPtr cannot be used directly because we can't use the tagged pointer/union code in constexpr
	template<typename T>
	struct TObjectRefParam
	{
		T* ObjectPtr = nullptr;
#if !IS_MONOLITHIC
		...
		constexpr TObjectRefParam() = default;
		constexpr TObjectRefParam(const TObjectRefParam&) = default;
		constexpr TObjectRefParam& operator=(const TObjectRefParam&) = default;

		// These constructors are not explicit to simplify UHT generated code 
		constexpr TObjectRefParam(T* InPtr)
			: ObjectPtr(InPtr)
		{
		}

#if IS_MONOLITHIC
		constexpr bool IsUnset() const
		{
			return ObjectPtr == nullptr;
		}
#else // IS_MONOLITHIC
		constexpr TObjectRefParam(uint32 InModuleId, uint32 InObjectId)
			: CompiledInPtr(FCompiledInObjectReference{InModuleId, InObjectId})
		{
		}

		constexpr TObjectRefParam(FCompiledInObjectReference InRef)
			: CompiledInPtr(InRef)
		{
		}
		constexpr bool IsUnset() const
		{
			return ObjectPtr == nullptr && CompiledInPtr.IsUnset();
		}
#endif // IS_MONOLITHIC
		...
		constexpr bool operator==(TYPE_OF_NULLPTR)
		{
			return IsUnset();
		}
	...
	using FObjectRefParam = TObjectRefParam<UObject>;
	...
	struct FObjectParams	
	{
		...
		FObjectRefParam Class;
		const UTF8CHAR* NameUTF8 = nullptr;
		FObjectRefParam Outer;

		consteval FObjectParams& SetClass(UObject* InClass)
		{ 
			Class.ObjectPtr = InClass;
			return *this;
		}
	};
	...
	{
		TObjectRefParam<UField> Next;
	};

	struct FStructParams
	{
		TObjectRefParam<UStruct> Super;
		UField* FirstChild; // No cross-module linking necessary 
		TArrayView<TCompiledInObjectPtr<const FStructBaseChain>> BaseChain;
	...
	{
		TCompiledInObjectPtr<UClass> Class;
		int32 PointerOffset;
		bool bImplementedByK2;
	...
	struct FClassParams
	{
		...
		TObjectRefParam<UClass> ClassWithin;
		TArrayView<FClassImplementedInterface> Interfaces;
		TObjectRefParam<UScriptStruct> SparseClassDataStruct;

		consteval FClassParams& SetWithin(UClass* WithinClass)
		{ 
			ClassWithin.ObjectPtr = WithinClass;
			return *this;
		}
	};
