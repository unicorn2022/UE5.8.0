...
private:
	...
	std::atomic<bool> bHasFoundExistingExports;
	/** Whether we are already fully initialized.																			*/
	std::atomic<bool> bHasFinishedInitialization;
	...
	/** Whether we already added dynamic imports.																			*/
	bool					bHasAddedDynamicImports : 1;
	...
	/** Index of the 1st dynamic import that was added (all dynamic imports are consecutive at the end of the import table  */
	int32					DynamicImportsIndex;
...
public:
	 ...
	 */
	UE_INTERNAL COREUOBJECT_API UObject* ResolveResource(FPackageIndex ResourceIndex);
	 ...
	 */
	void COREUOBJECT_API BuildPathName( FString& OutPathName, FPackageIndex ExportIndex ) const;
...
private:
	...
	// Universal placeholders force the loader to make placeholders for all objects
	// inferring what it can about the objects within the package and creating
	// placeholder imports to serve only the interests of the objects within the
	// package. To understand these functions simply read CreateUniversalPlaceholders
	void CreateUniversalPlaceholders();
	void TryCreateImportedPackage(FObjectImport& Import);
	void CreatePlaceholderImport(FObjectImport& Import);
	void TryCreatePlaceholderClassExport(const FObjectExport& Export);
	void TryCreateImportedClassForExport(const FObjectExport& Export);
	void TryCreateImportedClassesForImports();
	UClass* ForceCreatePlaceholderClassImport(FObjectImport& Import, UClass* SuperClassOverride = nullptr);
#endif // UE_INTERNAL_UNIVERSAL_PLACEHOLDERS
#if UE_INTERNAL_LINKER_BOUNDS
	// FArchive bounds implementation - needed for high placeholder environments but
	// likely useful everywhere that can afford them:
	COREUOBJECT_API virtual void SetBoundsImpl(FArchiveBounds NewBounds) override;
	COREUOBJECT_API virtual FArchiveBounds GetBoundsImpl() const override;
	FArchiveBounds CurrentBounds;
...
#if UE_INTERNAL_LINKER_BOUNDS
		// UE_INTERNAL_UNIVERSAL_PLACEHOLDERS has more bounds available:
		const bool bOutOfBounds = CurrentBounds == FArchiveBounds() ? 
			false :
			Tell() + Length > CurrentBounds.BoundsBasis + CurrentBounds.BoundsSize;
#endif // UE_INTERNAL_LINKER_BOUNDS
		if (UNLIKELY(IsLoadingToPropertyBagObject() && !bIsSerializingScriptProperties))
		{
			Loader->Seek(Tell() + Length);
		}
#if UE_INTERNAL_LINKER_BOUNDS
		else if (UNLIKELY(bOutOfBounds))
		{
			Loader->Seek(Tell() + Length);
			if(bOutOfBounds)
			{
				SetCriticalError(); // report misread to FPropertyTag so we can skip bad data
			}
		}
#endif // UE_INTERNAL_LINKER_BOUNDS
		else
...
private:
	...
	/**
	 * Handle class callbacks for each export.
	 */
	ELinkerStatus AddDynamicImports();
#endif
...
private:
	...
	*/
	...
	/** @return True when the loader is serializing to a property bag placeholder object. */
	bool IsLoadingToPropertyBagObject() const;
#endif // WITH_EDITOR
