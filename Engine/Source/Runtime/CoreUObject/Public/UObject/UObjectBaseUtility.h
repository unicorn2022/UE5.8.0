...
public:
	 ...
	 *
	 * @param FlagsToCheck	Object flags to check for.
	 * @param MemoryOrder	Memory ordering to apply when reading the flags from memory
	 * @return true if any of the passed in flags are set, false otherwise  (including no flags passed in).
	 */
	inline bool HasAnyFlags( EObjectFlags FlagsToCheck, std::memory_order InMemoryOrder) const
	{
		checkSlow(!(FlagsToCheck & (RF_MarkAsNative | RF_MarkAsRootSet)) || FlagsToCheck == RF_AllFlags); // These flags can't be used outside of constructors / internal code
		return (GetFlags(InMemoryOrder) & FlagsToCheck) != 0;
	}
	 ...
	 * @return true if all of the passed in flags are set (including no flags passed in), false otherwise
	 */
	inline bool HasAllFlags( EObjectFlags FlagsToCheck, std::memory_order InMemoryOrder) const
	{
		checkSlow(!(FlagsToCheck & (RF_MarkAsNative | RF_MarkAsRootSet)) || FlagsToCheck == RF_AllFlags); // These flags can't be used outside of constructors / internal code
		return ((GetFlags(InMemoryOrder) & FlagsToCheck) == FlagsToCheck);
	}
	...
	/** Helper function to try to set the private bGarbageEliminationEnabled variable. */
	static COREUOBJECT_API void SetGarbageEliminationEnabled(bool bEnabled);

	/** Helper function to set the private bGarbageEliminationEnabled variable for testing purposes only as garbage elimination may not be available for general use based on compile settings. */
	static inline void ForceSetGarbageEliminationEnabledForTest(bool bEnabled)
	{
...
/**
 * Returns the name of this object (with no path information). This version can avoid migrations for remote objects.
 * @param Object object to retrieve the name for; null gives "None".
 * @return Name of the object.
 */
template<class T UE_REQUIRES(TIsDerivedFrom<T, UObjectBase>::Value)>
inline FString GetNameSafe(const TObjectPtr<T>& ObjectPtr)
{
	if (ObjectPtr)
	{
