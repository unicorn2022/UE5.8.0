...
public:
	...
	template<typename U>
	[[nodiscard]] explicit consteval FObjectPtr(UE::CodeGen::ConstInit::TObjectRefParam<U> Ptr)
#if IS_MONOLITHIC
		: Handle(UE::CoreUObject::Private::MakeObjectHandle(Ptr.ObjectPtr))
#else
		: Handle(UE::CoreUObject::Private::MakeCompiledInObjectHandle(Ptr.ObjectPtr, Ptr.CompiledInPtr))
#endif // IS_MONOLITHIC
	{
	}
	...
	*/	
	[[nodiscard]] FORCEINLINE EResidence GetResidence() const { return GetObjectHandleResidence(Handle); }

	UE_DEPRECATED(5.8, "Use GetResidence() instead.")
	[[nodiscard]] FORCEINLINE bool IsRemote() const { return GetResidence() == EResidence::Remote; }
	...
	[[nodiscard]] FORCEINLINE FString GetName() const
	{
		...
		// We enforce GetFName() does not cause migrations when run in the Open and instead will return NAME_None if non-local
		...
		// We know the handle was non-null, so we are remote
		return FString::Printf(TEXT("%s (%s)"), *GetRemoteId().ToString(), EnumToString(GetResidence()));
#else
...
public:
...
#if UE_WITH_CONSTINIT_UOBJECT
	[[nodiscard]] explicit consteval TObjectPtr(UE::CodeGen::ConstInit::TObjectRefParam<T> Ptr)
		: ObjectPtr(Ptr)
	{
	}
#endif
	...
	[[nodiscard]] FORCEINLINE EResidence GetResidence() const
	{
		return ObjectPtr.GetResidence();
	}
...
private:
	...
	[[nodiscard]] UE_REWRITE FObjectHandle& GetHandleRef() const 
	{
		return ObjectPtr.GetHandleRef();
	}
...
namespace ObjectPtr_Private
{
	...
	struct Friend
	{
		...
		FORCEINLINE static FArchive& Serialize(FArchive& Ar, TObjectPtr<T>& InObjectPtr)
		{
			...
			if( Ar.IsLoading() )
			{
				// Prevent incompatibly mocked objects from leaking to callers.. this can happen
				// because of CoreUObject types (e.g. UScriptStruct) being extended in modules
				// which are unavailable in UE_INTERNAL_UNIVERSAL_PLACEHOLDERS environments. In this case
				// the type that CoreUObject wants to use is erased above so the linker
				// can't create a placeholder that necessarily conforms to the native code's
				// requirements.
				const UObject* Resolved = InObjectPtr;
				if(Resolved && !(InObjectPtr.template IsA<T>()))
				{
					InObjectPtr = nullptr;
				}
			}
		...
		template<typename T>
		UE_REWRITE static FObjectHandle& GetHandleRef(FObjectPtr& Ptr)
		{
			return Ptr.GetHandleRef();
		}

		template<typename T>
		UE_REWRITE static FObjectHandle& GetHandleRef(TObjectPtr<T>& Ptr)
		{
			return Ptr.GetHandleRef();
		}
		
		template<typename T>
		static FObjectHandle& GetHandleRef(TNonAccessTrackedObjectPtr<T>& Ptr);
	};
	...
	public:
...
#if UE_WITH_CONSTINIT_UOBJECT
		template<typename U>
		explicit consteval TNonAccessTrackedObjectPtr(UE::CodeGen::ConstInit::TObjectRefParam<U> Ptr)
			: ObjectPtr{FObjectPtr(Ptr)}
		{
		}
#endif // UE_WITH_CONSTINIT_UOBJECT
		...
		[[nodiscard]] UE_REWRITE FObjectHandle GetHandle() const 
		{
			return ObjectPtr.GetHandle();
		}
	...
	template<typename T>
	inline FObjectHandle& Friend::GetHandleRef(TNonAccessTrackedObjectPtr<T>& Ptr)
	{
		return Ptr.ObjectPtr.GetHandleRef();
	}
}
...
struct TContainerElementTypeCompatibility<TObjectPtr<T>>
{
	...
	{
#if UE_WITH_OBJECT_HANDLE_LATE_RESOLVE || UE_WITH_OBJECT_HANDLE_TRACKING || UE_WITH_REMOTE_OBJECT_HANDLE
		while (Iter != IterEnd)
		{
			(void)Operator(Iter).Get();
			check(UE_WITH_REMOTE_OBJECT_HANDLE || Operator(Iter).IsResolved());
			++Iter;
		}
#endif
	}
	...
	static void ReinterpretRangeContiguous(IterBeginType Iter, IterEndType IterEnd, SizeType Size, OperatorType Operator = TContainerElementTypeCompatibility_DefaultOperatorFunc)
	{
		...
		while (Iter != IterEnd)
		{
			...
			check(UE_WITH_REMOTE_OBJECT_HANDLE || ObjPtr.IsResolved());
			++Iter;
		}
