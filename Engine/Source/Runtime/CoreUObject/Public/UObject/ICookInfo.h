...
public:
	 ...
	 */
	virtual FInstigator GetInstigator(FName PackageName) const = 0;
	/**
	 ...
	 */
	virtual TArray<const ITargetPlatform*> GetSessionPlatforms() const = 0;
	/**
	 ...
	 */
	virtual FString GetCookOutputFolder(const ITargetPlatform* TargetPlatform) const = 0;
	/**
	 * Returns the path within the GetCookOutputFolder where the Metadata folder for the cook is written. For basegame
	 * cooks this is GetCookOutputFolder()/<ProjectName>/Metadata, but for DLC cooks it is in the relative path to the
	 * DLC Folder: GetCookOutputFolder()/<RelPathToDLC>/Metadata.
	 */
	virtual FString GetCookMetadataOutputFolder(const ITargetPlatform* TargetPlatform) const = 0;
	 ...
	 */
	virtual const TSet<IPlugin*>* GetEnabledPlugins(const ITargetPlatform* TargetPlatform) const = 0;
...
public:
	/**
	 * Called as early as possible after parameters for the current cook session are loaded to allow modifying cook
	 * startup behavior, such as registering extra artifacts or collectors.
	 */
	static COREUOBJECT_API FCookInfoEvent ConfigureCookSession;
...
struct FCookLoadScope
{
	...
	UE_FORCEINLINE_HINT ~FCookLoadScope() = default;
};
