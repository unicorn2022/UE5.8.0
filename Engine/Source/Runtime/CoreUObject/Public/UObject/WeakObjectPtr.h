...
public:
	...
	/** Operators for direct access to Get() */
	UE_REWRITE UObject& operator*() const
	{
		return *Get();
	}
	UE_REWRITE UObject* operator->() const
	...
	*/
	...
	{
		return UE::RemoteObject::Handle::GetResidence(ObjectRemoteId);
	}
...
private:
	...
	COREUOBJECT_API bool CanBeResolved(UE::RemoteObject::Handle::ECanResolveObjectBehavior CanResolveObjectBehavior) const;
#endif
	...
	*/
	inline bool Internal_IsValid(bool bEvenIfGarbage, bool bThreadsafeTest) const
	{
		...
		if (!ObjectItem || !SerialNumbersMatch(ObjectItem))
		{
			if (bEvenIfGarbage)
			{
				return CanBeResolved(UE::RemoteObject::Handle::ECanResolveObjectBehavior::Default);
			}
			else
			{
				return CanBeResolved(UE::RemoteObject::Handle::ECanResolveObjectBehavior::CannotResolveGarbageObjects);
			}
		}
