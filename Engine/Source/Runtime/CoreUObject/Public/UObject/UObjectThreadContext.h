...
public:
	...
	*/
	...
	TArray<UObject*, TInlineAllocator<16, FNonshrinkingAllocator>> PostInitPropertiesCheck;
...
public:
	...
	/** True when serializing to a property bag placeholder object while loading. */
	UE_INTERNAL bool bIsLoadingToPropertyBagObject;
	/** True when serializing to an unknown property tree for determining property type info. The loaded value doesn't need to be correct so we can skip
		loading values in certain cases for the sake of bypassing issues with incomplete info or for the sake of optimization */
	UE_INTERNAL bool bIsLoadingToUnknownPropertyTree;
#endif
