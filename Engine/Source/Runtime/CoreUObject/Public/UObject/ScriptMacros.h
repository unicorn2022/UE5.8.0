...
#define PARAM_PASSED_BY_VAL(ParamName, PropertyType, ParamType)																							\
	UE_REMOVE_OPTIONAL_PARENS(ParamType) ParamName;																										\
	Stack.StepCompiledIn<PropertyType>(&ParamName);

#define PARAM_PASSED_BY_VAL_ZEROED(ParamName, PropertyType, ParamType)																					\
	UE_REMOVE_OPTIONAL_PARENS(ParamType) ParamName = (UE_REMOVE_OPTIONAL_PARENS(ParamType))0;															\
	Stack.StepCompiledIn<PropertyType>(&ParamName);

#define PARAM_PASSED_BY_VAL_INITED(ParamName, PropertyType, ParamType, ...)																				\
	UE_REMOVE_OPTIONAL_PARENS(ParamType) ParamName{__VA_ARGS__};																						\
	Stack.StepCompiledIn<PropertyType>(&ParamName);

#define PARAM_PASSED_BY_REF_PASSTHRU(ParamName, PropertyType, ParamType, PassThruType)																	\
	UE_REMOVE_OPTIONAL_PARENS(PassThruType) ParamName##Temp;																							\
	UE_REMOVE_OPTIONAL_PARENS(ParamType)& ParamName = Stack.StepCompiledInRef<PropertyType, UE_REMOVE_OPTIONAL_PARENS(ParamType)>(&ParamName##Temp);

#define PARAM_PASSED_BY_REF(ParamName, PropertyType, ParamType)																							\
	UE_REMOVE_OPTIONAL_PARENS(ParamType) ParamName##Temp;																								\
	UE_REMOVE_OPTIONAL_PARENS(ParamType)& ParamName = Stack.StepCompiledInRef<PropertyType, UE_REMOVE_OPTIONAL_PARENS(ParamType)>(&ParamName##Temp);

#define PARAM_PASSED_BY_REF_ZEROED(ParamName, PropertyType, ParamType)																					\
	UE_REMOVE_OPTIONAL_PARENS(ParamType) ParamName##Temp = (UE_REMOVE_OPTIONAL_PARENS(ParamType))0;														\
	UE_REMOVE_OPTIONAL_PARENS(ParamType)& ParamName = Stack.StepCompiledInRef<PropertyType, UE_REMOVE_OPTIONAL_PARENS(ParamType)>(&ParamName##Temp);
...
#define P_GET_TMAP(KeyType,ValueType,ParamName)			PARAM_PASSED_BY_VAL(ParamName, FMapProperty, (TMap<KeyType, ValueType>))
#define P_GET_TMAP_REF(KeyType,ValueType,ParamName)		PARAM_PASSED_BY_REF(ParamName, FMapProperty, (TMap<KeyType, ValueType>))
...
#define P_GET_WEAKOBJECT(ObjectType,ParamName)		PARAM_PASSED_BY_VAL(ParamName, FWeakObjectProperty, typename UE_REMOVE_OPTIONAL_PARENS(ObjectType)::ElementType*)
#define P_GET_WEAKOBJECT_REF(ObjectType,ParamName)	PARAM_PASSED_BY_REF_PASSTHRU(ParamName, FWeakObjectProperty, ObjectType, typename UE_REMOVE_OPTIONAL_PARENS(ObjectType)::ElementType*)

#define P_GET_WEAKOBJECT_NO_PTR(ObjectType,ParamName)		PARAM_PASSED_BY_VAL(ParamName, FWeakObjectProperty, typename UE_REMOVE_OPTIONAL_PARENS(ObjectType)::ElementType*)
#define P_GET_WEAKOBJECT_REF_NO_PTR(ObjectType,ParamName)	PARAM_PASSED_BY_REF_PASSTHRU(ParamName, FWeakObjectProperty, ObjectType, typename UE_REMOVE_OPTIONAL_PARENS(ObjectType)::ElementType*)
...
#define P_GET_VERSECONCRETETYPE(ParamName)			PARAM_PASSED_BY_VAL_INITED(ParamName, FObjectPropertyBase, verse::concrete_type, Verse::EDefaultConstructNativeType::UnsafeDoNotUse)
#define P_GET_VERSECONCRETETYPE_REF(ParamName)		PARAM_PASSED_BY_REF(ParamName, FObjectPropertyBase, verse::concrete_type)
...
#define P_GET_VERSECASTABLECONCRETETYPE(ParamName)			PARAM_PASSED_BY_VAL_INITED(ParamName, FObjectPropertyBase, verse::castable_concrete_type, Verse::EDefaultConstructNativeType::UnsafeDoNotUse)
#define P_GET_VERSECASTABLECONCRETETYPE_REF(ParamName)		PARAM_PASSED_BY_REF(ParamName, FObjectPropertyBase, verse::castable_concrete_type)

#define P_GET_VERSECASTABLECONCRETESUBTYPE(ElementType, ParamName)		PARAM_PASSED_BY_VAL_INITED(ParamName, FObjectPropertyBase, verse::castable_concrete_subtype<ElementType>, Verse::EDefaultConstructNativeType::UnsafeDoNotUse)
#define P_GET_VERSECASTABLECONCRETESUBTYPE_REF(ElementType, ParamName)	PARAM_PASSED_BY_REF(ParamName, FObjectPropertyBase, verse::castable_concrete_subtype<ElementType>)


#define P_GET_INTERFACEINSTANCE(ElementType, ParamName)		PARAM_PASSED_BY_VAL_INITED(ParamName, FObjectPropertyBase, TInterfaceInstance<ElementType>, EDefaultConstructNonNullPtr::Uninitialized)
