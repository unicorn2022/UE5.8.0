...
public:
	 ...
	 * @param ExportsArchive		The linker associated with the package being written to disk.
	 * @param DataArchive			The archive where the package data has been written to. This is where the FPackageTrailer will be written to
	 * @param InOutArchiveOffset	The offset at which the trailer is written in the archive. In this function version, DataArchive might
	 *								be an archive for the entire archive, or it might be a separate archive solely for the package trailer.
	 *								The caller must provide the initial value with the accumulated offset, and the function will increment this
	 *								value by how many bytes are written to DataArchive.
	 * @return True if the builder was created and appended successfully and false if any error was encountered
	 */
	[[nodiscard]] COREUOBJECT_API bool BuildAndAppendTrailer(FLinkerSave* Linker, FArchive& DataArchive, int64& InOutArchiveOffset);
...
public:
	...
	/** Returns the offset (in bytes) of the package trailer in the archive that it was loaded from or written too. */
	[[nodiscard]] int64 GetPackageTrailerOffset() const
	{
		return PackageTrailerOffset;
	}
...
private:
	...
	/** Offset into the archive where the FPackageTrailer is located */
	int64 PackageTrailerOffset = INDEX_NONE;
