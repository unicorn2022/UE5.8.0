...
public:
	...
	{
		if (const FName* RemappedName = InstancedPackageMapping.Find(FPackageId::FromName(PackageName)))
		{
			if (bWasFoundPtr)
			{
				*bWasFoundPtr = true;
	...
	FPackageId RemapPackageId(const FPackageId& PackageId, bool* bWasFoundPtr = nullptr) const
	{
		if (const FName* RemappedName = InstancedPackageMapping.Find(PackageId))
		{
			if (bWasFoundPtr)
			{
				*bWasFoundPtr = true;
			}
			return FPackageId::FromName(*RemappedName);
		}
		if (bWasFoundPtr)
		{
			*bWasFoundPtr = false;
	...
	bool ValidateAgainst(FLinkerInstancedPackageMap& OtherPackageMap) const;
...
private:
	 ...
	 */
	TMap<FPackageId, FName> InstancedPackageMapping;
...
{
public:
	enum class EContextSharingMode
	{
		Copy,
		Link,
		Share
	...
	COREUOBJECT_API FLinkerInstancingContext(const FLinkerInstancingContext&);
	...
	* 
	*	Link: * If the Source context has no shared immutable data it'll construct this context by using the Source context shared data as the 
	*		     immutable data for this context and create a new share data object to use for mutations specific to this context. 
	* 
	*		   * If the Source context has shared immutable data we copy the ptr to share it as our shared immutable data , and we copy the 
	*			 the shared data ptr to share it. This means the context has been linked more than 1 time. 
	* 
	*	Share: We copy both the shared data and immutable share data ptrs. 
	* 
	*	The Link context is intended to be used internally in the loader/linker to ensure that mappings that occur during loading have no side
	*	effects on further loads done with the same FLinkerInstancingContext thus allowing them to be resued. 
	* 
	*/	
	COREUOBJECT_API FLinkerInstancingContext(const FLinkerInstancingContext& Source, EContextSharingMode Mode);
	...
	COREUOBJECT_API FPackageId RemapPackageId(const FPackageId& PackageId) const;
...
private:
	...
	TSharedPtr<FSharedLinkerInstancingContextData> ImmutableData;
};
