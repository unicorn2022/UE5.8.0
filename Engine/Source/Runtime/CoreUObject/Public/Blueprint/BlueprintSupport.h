...
public:
	...
	/**
	 * Returns the current authoratitative scope on the stack if one exist.
	 */
	static COREUOBJECT_API FScopedClassDependencyGather* GetAuthoritativeScope();

	DECLARE_TS_MULTICAST_DELEGATE(FOnScopeExit);

	/**
	 * Delegate called when the scope exits;
	 */
	FOnScopeExit OnScopeExitDelegate;
private:
	...
	static thread_local UClass* BatchAuthorityClass;

	/** List of dependencies (i.e. UClasses that have been newly instantiated) in the scope of this dependency gather */
	static thread_local TArray<UClass*> BatchClassDependencies;

	/** Contains the authoritative scope on the current thread */
	static thread_local FScopedClassDependencyGather* AuthoritativeScope;
...
{
public:
	virtual ~FDeferredInitializationTrackerBase() = default;
