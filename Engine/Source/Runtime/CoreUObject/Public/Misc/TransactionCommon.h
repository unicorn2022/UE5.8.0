...
public:
	...
	UE_API explicit FPersistentObjectRef(UObject* InObject);
	...
	using OnUseOuterDelegate = bool(*)(const UObject*);
	UE_API static TArray<OnUseOuterDelegate>& GetOnUseOuterDelegates();
...
{
public:
	static UE_API FSerializedTaggedData FromOffsetAndSize(const int64 InOffset, const int64 InSize);
	static UE_API FSerializedTaggedData FromStartAndEnd(const int64 InStart, const int64 InEnd);

	UE_API void AppendSerializedData(const int64 InOffset, const int64 InSize);
	UE_API void AppendSerializedData(const FSerializedTaggedData& InData);

	UE_API bool HasSerializedData() const;
...
public:
	...
	UE_API void Read(void* Dest, int64 Offset, int64 Num) const;
	UE_API void Write(const void* Src, int64 Offset, int64 Num);
...
UE_API extern const FName TaggedDataKey_UnknownData;
UE_API extern const FName TaggedDataKey_ScriptData;

/** Core archive to read a transaction object from the buffer. */
class FSerializedObjectDataReader : public FArchiveUObject
{
public:
	UE_API FSerializedObjectDataReader(const FSerializedObject& InSerializedObject);
...
protected:
	UE_API virtual void Serialize(void* SerData, int64 Num) override;

	using FArchiveUObject::operator<<;
	UE_API virtual FArchive& operator<<(class FName& N) override;
	UE_API virtual FArchive& operator<<(class UObject*& Res) override;
...
{
public:
	UE_API FSerializedObjectDataWriterCommon(FSerializedObjectData& InSerializedData);
...
{
public:
	UE_API FSerializedObjectDataWriter(FSerializedObject& InSerializedObject);
...
{
public:
	UE_API FDiffableObjectDataWriter(FDiffableObject& InDiffableObject, TArrayView<const FProperty*> InPropertiesToSerialize = TArrayView<const FProperty*>());

protected:
	UE_API FName GetTaggedDataKey() const;

	UE_API bool DoesObjectMatchDiffableObject(const UObject* Obj) const;

	UE_API virtual bool ShouldSkipProperty(const FProperty* InProperty) const override;

	UE_API virtual void MarkScriptSerializationStart(const UObject* Obj) override;
	UE_API virtual void MarkScriptSerializationEnd(const UObject* Obj) override;

	UE_API virtual void OnDataSerialized(int64 InOffset, int64 InNum) override;
...
namespace DiffUtil
{
UE_API bool IsGeneratingDiffableObject(const FArchive& Ar);
 ...
 */
UE_API FDiffableObject GetDiffableObject(const UObject* Object, const FGetDiffableObjectOptions& Options = FGetDiffableObjectOptions());
 ...
 */
UE_API FTransactionObjectDeltaChange GenerateObjectDiff(const FDiffableObject& OldDiffableObject, const FDiffableObject& NewDiffableObject, const FGenerateObjectDiffOptions& DiffOptions = FGenerateObjectDiffOptions(), FDiffableObjectArchetypeCache* ArchetypeCache = nullptr);
UE_API void GenerateObjectDiff(const FDiffableObject& OldDiffableObject, const FDiffableObject& NewDiffableObject, FTransactionObjectDeltaChange& OutDeltaChange, const FGenerateObjectDiffOptions& DiffOptions = FGenerateObjectDiffOptions(), FDiffableObjectArchetypeCache* ArchetypeCache = nullptr);
