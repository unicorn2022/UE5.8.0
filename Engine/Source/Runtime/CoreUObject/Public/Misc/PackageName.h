...
public:
	...
	/**
	 * This will mount a mount point if not already mounted, and will update its order to the front of the override
	 * chain (so it can overlap an existing mount point and win).
	 * If you register a mount point (even if you do so only in certain circumstances) consider also adding a handler
	 * for GetExplanationForUnavailablePackage to help debug cases where a package can't be found.
	 * This function can fail only if the input arguments are not valid text for RootPath or ContentPath.
	 *
	 * @param RootPath LongPackageName location for the root of the MountPoint,
	 *        Examples: /Engine/, /PluginName/
	 * @param ContentPath LocalPath location for the root of the MountPoint,
	 *        Examples: ../../../Engine/Content, d:\FullPathToUnreal\Engine\Plugins\PluginName
	 * 
	 * @return RefCountPtr to the mounted IMountPoint, or nullptr if failed.
	 */
	static COREUOBJECT_API TRefCountPtr<UE::PackageName::IMountPoint>
		RegisterMountPoint(const FString& RootPath, const FString& ContentPath);
	/**
	 * Mounts a mount point if not already mounted, and updates its order to front of the override chain.
	 * If you register a mount point (even if you do so only in certain circumstances) consider also adding a handler
	 * for GetExplanationForUnavailablePackage to help debug cases where a package can't be found.
	 */
	static COREUOBJECT_API void RegisterMountPoint(const TRefCountPtr<UE::PackageName::IMountPoint>& MountPoint);
	...
	static COREUOBJECT_API void UnRegisterMountPoint(const TRefCountPtr<UE::PackageName::IMountPoint>& MountPoint);


	/**
	 * Returns whether the given RootPath is the LongPackageName of a MountPoint that has been mounted, by default or
	 * by an earlier call to RegisterMountPoint. 
	 */
	...
	/**
	 * Looks for the first allocated MountPoint with the given RootLongPackageName. If multiple are allocated, only
	 * returns the one with the highest override priority (the one that was added last). A discovered MountPoint might
	 * or might not be mounted, call MountPoint->IsMounted() to check for mounted.
	 * 
	 * @param RootLongPackageName LongPackageName location for the root of the MountPoint,
	 *        Examples: /Engine/, /PluginName/
	 * @return RefCountPtr to the IMountPoint, or nullptr if not yet allocated.
	 */
	static COREUOBJECT_API TRefCountPtr<UE::PackageName::IMountPoint>
		FindMountPointByRootPackageName(FStringView RootLongPackageName);
	/**
	 * Looks for the first MountPoint with the given RootLocalPath. If multiple are allocated, only returns the
	 * one with the highest override priority (the one that was added last). A discovered MountPoint might or might
	 * not be mounted, call MountPoint->IsMounted() to check for mounted.
	 *
	 * @param RootLocalPath LocalPath location for the root of the MountPoint,
	 *        Examples: ../../../Engine/Content, d:\FullPathToUnreal\Engine\Plugins\PluginName
	 * @return RefCountPtr to the IMountPoint, or nullptr if not yet allocated.
	 */
	static COREUOBJECT_API TRefCountPtr<UE::PackageName::IMountPoint>
		FindMountPointByRootLocalPath(FStringView RootLocalPath);
	/**
	 * Looks for an existing allocated MountPoint with the given RootLongPackageName and RootLocalPath. These fields
	 * uniquely identify a MountPoint so at most one can exist.
	 * If the MountPoint exists, it might or might not be mounted, call MountPoint->IsMounted() to check for mounted.
	 *
	 * @param RootLongPackageName LongPackageName location for the root of the MountPoint,
	 *        Examples: /Engine/, /PluginName/
	 * @param RootLocalPath LocalPath location for the root of the MountPoint,
	 ...
	 */
	static COREUOBJECT_API TRefCountPtr<UE::PackageName::IMountPoint>
		FindMountPoint(FStringView RootLongPackageName, FStringView RootLocalPath);
	/**
	 * Looks for an existing allocated MountPoint with the given RootLongPackageName and RootLocalPath, adds it
	 * if it does not already exist. These fields uniquely identify a MountPoint so at most one can exist.
	 * When the MountPoint is returned, it might or might not be mounted, call MountPoint->IsMounted() to check for
	 * mounted.
	 * This function can only return null if the input arguments are not valid text for RootLongPackageName or
	 * RootLocalPath.
	 *
	 * @param RootLongPackageName LongPackageName location for the root of the MountPoint,
	 *        Examples: /Engine/, /PluginName/
	 * @param RootLocalPath LocalPath location for the root of the MountPoint,
	 *        Examples: ../../../Engine/Content, d:\FullPathToUnreal\Engine\Plugins\PluginName
	 * @return RefCountPtr to the IMountPoint, or nullptr if failed.
	 */
	static COREUOBJECT_API TRefCountPtr<UE::PackageName::IMountPoint>
		FindOrAddMountPoint(FStringView RootLongPackageName, FStringView RootLocalPath);

	/**
	 * Looks for the first MountPoint that is equal to or is a parent path of the given ChildLongPackageName.
	 * If multiple are allocated, only returns the one with the highest override priority (the one that was added
	 * last). A discovered MountPoint might or might not be mounted, call MountPoint->IsMounted() to check for mounted.
	 *
	 * @param ChildLongPackageName LongPackageName for a path under the MountPoint,
	 *        Examples: /Engine/, /Engine/Asset, /PluginName/, /PluginName/Asset
	 * @return RefCountPtr to the IMountPoint, or nullptr if not yet allocated.
	 */
	static COREUOBJECT_API TRefCountPtr<UE::PackageName::IMountPoint>
		FindMountPointByChildLongPackageName(FStringView ChildLongPackageName);
	...
	*/
	...
	/** Event triggered when a MountPoint is mounted. */
	DECLARE_MULTICAST_DELEGATE_OneParam(FMountPointEvent, const TRefCountPtr<UE::PackageName::IMountPoint>& MountPoint)
	static FMountPointEvent& OnMountPointMounted()
	{
		return OnMountPointMountedEvent;
	}
	/** Legacy event triggered when a MountPoint is mounted; passes strings instead of IMountPoint. */
	...
	/** Event triggered when a MountPoint is dismounted. */
	static FMountPointEvent& OnMountPointDismounted()
	{
		return OnMountPointDismountedEvent;
	}
	/** Legacy event triggered when a MountPoint is dismounted; passes strings instead of IMountPoint. */
...
private:
	...
	static COREUOBJECT_API FMountPointEvent OnMountPointMountedEvent;
	...
	static COREUOBJECT_API FMountPointEvent OnMountPointDismountedEvent;
