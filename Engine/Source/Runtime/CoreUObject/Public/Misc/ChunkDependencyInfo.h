...
class UChunkDependencyInfo : public UObject
{
	...
	COREUOBJECT_API const FChunkDependencyTreeNode* GetOrBuildChunkDependencyGraph(int32 HighestChunk = 0, bool bForceRebuild = false);
	...
	COREUOBJECT_API const FChunkDependencyTreeNode* BuildChunkDependencyGraph(int32 HighestChunk);

	/** Removes redundant chunks from a chunk list */
	COREUOBJECT_API void RemoveRedundantChunks(TArray<int32>& ChunkIDs) const;
	...
	COREUOBJECT_API int32 FindHighestSharedChunk(const TArray<int32>& ChunkIDs) const;

	/**
	 * Returns the chunks that the in chunk depends upon through the parent rules.
	 */
	COREUOBJECT_API void GetChunkDependencies(const int32 InChunk, TSet<int32>& OutChunkDependencies) const;
...
private:
	/** Transfers data from the deprecated location [/Script/UnrealEd.ChunkDependencyInfo] and emits a warning in that case */
