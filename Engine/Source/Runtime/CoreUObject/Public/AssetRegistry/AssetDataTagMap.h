...
{
	// Base struct for FixedTagPrivate::FStore (defined in AssetDataTagMapSerializationDetails.h), containing members needed
	// by debug visualizer for FixedTagPrivate::FMapHandle.
	struct FStoreBase
	{
		// Pairs for all unsorted maps that use this store
		TArrayView<FixedTagPrivate::FNumberedPair> Pairs;
		TArrayView<FixedTagPrivate::FNumberlessPair> NumberlessPairs;

		// Values for visualizable maps in this store
...
} // end namespace UE::CoreUObject::Private
...
public:
	...
	bool										IsFixed() const { return Fixed.IsFixed() != 0; }
