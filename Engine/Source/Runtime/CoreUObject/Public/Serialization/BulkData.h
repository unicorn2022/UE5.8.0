...
/** Returns the IoChunk type that we expect to be produced from a given EBulkDataFlags */
COREUOBJECT_API EIoChunkType GetIoChunkTypeFromFlags(EBulkDataFlags Flags);
...
public:
	...
	*/
	 ...
	 */
	COREUOBJECT_API void* Lock(uint32 LockFlags, UE::FSourceLocation Location = UE::FSourceLocation::Current());

	/**
	 * Locks the bulk data and returns a read-only pointer to it.
	 * This variant can be called on a const bulkdata
	 */
	COREUOBJECT_API const void* LockReadOnly(UE::FSourceLocation Location = UE::FSourceLocation::Current()) const;
...
protected:
	...
	FBulkDataCookedIndex	CookedIndex;
#if DO_CHECK
	TOptional<UE::FSourceLocation> LockSourceLocation;
#endif
#endif // WITH_EDITOR
	   //
#if !USE_RUNTIME_BULKDATA
