...
protected:
	...
	*/
	...
	/** List of VCells that have already been serialized */
	TSet<Verse::VCell*> SerializedCells;

	/** VCells that will be serialized */
	TArray<Verse::VCell*> PendingSerializationCells;
#endif
...
public:
	...
	virtual FArchive& operator<<(Verse::VCell*& Cell) override
	{
		if (Cell && Cell->GetCppClassInfo()->InstancedCell)
		{
			...
			SerializedCells.Add(Cell, &bIsAlreadyInSet);
			if (!bIsAlreadyInSet)
			{
				PendingSerializationCells.Add(Cell);
			}
		}
		return *this;
	}
