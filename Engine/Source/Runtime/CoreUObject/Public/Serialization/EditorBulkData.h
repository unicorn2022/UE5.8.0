...
public:
	...
	/** Returns true if the bulkdata object specifies a valid payload greater than zero bytes in size. */
	...
	/**
	 * Returns whether the payload is loaded in memory and we have a pointer to it.
	 * If true, calling GetPayload will return immediately with the value (unless modified inbetween).
	 */
	COREUOBJECT_API bool IsPayloadLoaded() const;

	/** Returns if the payload would require loading in order to be accessed. Returns false if IsPayloadLoaded or GetPayloadSize() == 0. */
...
private:
	...
	/** Returns true if we should use legacy serialization instead of the FPackageTrailer system. */
