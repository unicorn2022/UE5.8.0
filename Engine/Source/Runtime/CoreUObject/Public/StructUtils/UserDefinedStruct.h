...
private:
	UPROPERTY()
	TArray<UE::StructUtils::FUserDefinedPropertyID> PropertyIDs;
...
public:
	...
	friend FUserDefinedStructureCompilerInner;
#endif	// WITH_EDITOR

	//~ UObject interface.
	COREUOBJECT_API virtual void Serialize(FStructuredArchive::FRecord Record) override;
	COREUOBJECT_API virtual void SerializeTaggedProperties(FStructuredArchive::FSlot Slot, TNotNull<uint8*> Data, const UStruct* DefaultsStruct, const uint8* Defaults, const UObject* BreakRecursionIfFullyLoad = nullptr) const override;
	...
	{
		return PropertyIDs.Num() > 0;
	}
	COREUOBJECT_API virtual FName FindPropertyNameFromGuid(const FGuid& PropertyID) const override;
	COREUOBJECT_API virtual FGuid FindPropertyGuidFromName(const FName PropertyName) const override;
