...
enum class EPropertyBagPropertyType : uint8
{
	...
	Class,
	SoftClass,
	Int8,	// Type not fully supported at UI, will work with restrictions to type editing
	Int16,	// Type not fully supported at UI, will work with restrictions to type editing
	UInt16,	// Type not fully supported at UI, will work with restrictions to type editing
...
enum class EPropertyBagAlterationResult : uint8
{
	...
	PropertyKeyTypeNotHashable,          // The property has Map container type and the given key type is not hashable.
};
...
struct FPropertyBagPropertyDesc
{
	...
	FPropertyBagPropertyDesc(const FName InName, const EPropertyBagContainerType InContainerType, const EPropertyBagPropertyType InValueType, const UObject* InValueTypeObject = nullptr, EPropertyFlags InPropertyFlags = CPF_Edit, const EPropertyBagPropertyType InKeyType = EPropertyBagPropertyType::None, const UObject* InKeyTypeObject = nullptr)
		: ValueTypeObject(InValueTypeObject)
		, Name(InName)
		, ValueType(InValueType)
		...
		, KeyType(InKeyType)
		, KeyTypeObject(InKeyTypeObject)
	{
	}

	FPropertyBagPropertyDesc(const FName InName, const FPropertyBagContainerTypes& InNestedContainers, const EPropertyBagPropertyType InValueType, const UObject* InValueTypeObject = nullptr, EPropertyFlags InPropertyFlags = CPF_Edit, const EPropertyBagPropertyType InKeyType = EPropertyBagPropertyType::None, const UObject* InKeyTypeObject = nullptr)
		: ValueTypeObject(InValueTypeObject)
		, Name(InName)
		, ValueType(InValueType)
	...
	/** @return true if the property type is numeric (bool, (u)int8, (u)int16, (u)int32, (u)int64, float, double, enum) */
	UE_API bool IsNumericType() const;

	/** @return true if the property type is unsigned (uint8, uint16, uint32, uint64) */
	UE_API bool IsUnsignedNumericType() const;

	/** @return true if the property type is integral ((u)int8, (u)int16, (u)int32, (u)int64) */
	UE_API bool IsNumericIntegralType() const;
	...
	/** Type of this property's key (for map containers only) */
	UPROPERTY(EditAnywhere, Category = "Default")
	EPropertyBagPropertyType KeyType = EPropertyBagPropertyType::None;

	/** Pointer to object that defines the Enum, Struct, or Class of this property's key (for map containers only) */
	UPROPERTY(EditAnywhere, Category = "Default")
 ...
 *
 *		static const FName TemperatureName("Temperature");
 *		static const FName IsHotName("bIsHot");
 *
 *		FInstancedPropertyBag Bag;
 *
...
struct FInstancedPropertyBag
{
	 ...
	 * @param OptionalPropertyIdsSubset Optional array of property ids subset to copy from Other
	 */
	 ...
	 * @param InValueTypeObject Type object (for struct, class, enum) of the new property
	 * @param bOverwrite Overwrite the property if it already exists.
	 * @param InKeyType Type of the new property's key
	 * @param InKeyTypeObject Type object (for struct, class, enum) of the new property's key
	 * @return The result of the alteration.
	 */
	UE_API EPropertyBagAlterationResult AddContainerProperty(const FName InName, const EPropertyBagContainerType InContainerType, const EPropertyBagPropertyType InValueType, const UObject* InValueTypeObject = nullptr, bool bOverwrite = true, const EPropertyBagPropertyType InKeyType = EPropertyBagPropertyType::None, const UObject* InKeyTypeObject = nullptr);
	 ...
	 * @return The result of the alteration.
	 */
	UE_API EPropertyBagAlterationResult AddContainerProperty(const FName InName, const FPropertyBagContainerTypes InContainerTypes, const EPropertyBagPropertyType InValueType, const UObject* InValueTypeObject, bool bOverwrite = true, const EPropertyBagPropertyType InKeyType = EPropertyBagPropertyType::None, const UObject* InKeyTypeObject = nullptr);
	 ...
	 * @param InKeyTypeObject Type object (for struct, class, enum) of the new property's key
	 * @param InValueTypeObject Type object (for struct, class, enum) of the new property
	 * @param InNestedContainerType Type of (optional) nested container to create
	 * @param bOverwrite Overwrite the property if it already exists.
	 * @return The result of the alteration.
	 */
	UE_API EPropertyBagAlterationResult AddMapProperty(const FName InName, const EPropertyBagPropertyType InKeyType, const EPropertyBagPropertyType InValueType, const UObject* InKeyTypeObject = nullptr, const UObject* InValueTypeObject = nullptr, const EPropertyBagContainerType InNestedContainerType = EPropertyBagContainerType::None, bool bOverwrite = true);
	...
	/**
	 * Getters
	 * Numeric types (bool, (u)int8, (u)int16, (u)int32, (u)int64, float, double) support type conversion.
	 */
	UE_API TValueOrError<bool, EPropertyBagResult> GetValueBool(const FName Name) const;
	UE_API TValueOrError<int8, EPropertyBagResult> GetValueInt8(const FName Name) const;
	UE_API TValueOrError<uint8, EPropertyBagResult> GetValueByte(const FName Name) const;
	UE_API TValueOrError<int16, EPropertyBagResult> GetValueInt16(const FName Name) const;
	UE_API TValueOrError<uint16, EPropertyBagResult> GetValueUInt16(const FName Name) const;
	...
	UE_API TValueOrError<int64, EPropertyBagResult> GetValueEnumInt64(const FName Name, const UEnum* RequestedEnum) const;
	...
	TValueOrError<T, EPropertyBagResult> GetValueEnum(const FName Name) const
	{
		...
		TValueOrError<int64, EPropertyBagResult> Result = GetValueEnumInt64(Name, StaticEnum<T>());
		if (!Result.IsValid())
		{
...
private:
	template<typename T>
	TValueOrError<T, EPropertyBagResult> GetNumericalValue(const FName Desc) const;

public:
	UE_API TValueOrError<bool, EPropertyBagResult> GetValueBool(const FPropertyBagPropertyDesc& Desc) const;
	UE_API TValueOrError<int8, EPropertyBagResult> GetValueInt8(const FPropertyBagPropertyDesc& Desc) const;
	UE_API TValueOrError<uint8, EPropertyBagResult> GetValueByte(const FPropertyBagPropertyDesc& Desc) const;
	UE_API TValueOrError<int16, EPropertyBagResult> GetValueInt16(const FPropertyBagPropertyDesc& Desc) const;
	UE_API TValueOrError<uint16, EPropertyBagResult> GetValueUInt16(const FPropertyBagPropertyDesc& Desc) const;
	...
	UE_API TValueOrError<int64, EPropertyBagResult> GetValueEnumInt64(const FPropertyBagPropertyDesc& Desc, const UEnum* RequestedEnum) const;
	...
	UE_API TValueOrError<FPropertyBagMapRef, EPropertyBagResult> GetMutableMapRef(const FPropertyBagPropertyDesc& Desc);
	UE_API TValueOrError<const FPropertyBagMapRef, EPropertyBagResult> GetMapRef(const FPropertyBagPropertyDesc& Desc) const;
	...
	TValueOrError<T, EPropertyBagResult> GetValueEnum(const FPropertyBagPropertyDesc& Desc) const
	{
		...
		TValueOrError<int64, EPropertyBagResult> Result = GetValueEnumInt64(Desc, StaticEnum<T>());
		if (!Result.IsValid())
		{
			return MakeError(Result.GetError());
		}
...
private:
	template<typename T>
	TValueOrError<T, EPropertyBagResult> GetNumericalValue(const FPropertyBagPropertyDesc& Desc) const;
...
public:
	...
	UE_API EPropertyBagResult SetValueInt8(const FName Name, const int8 InValue);
	UE_API EPropertyBagResult SetValueByte(const FName Name, const uint8 InValue);
	UE_API EPropertyBagResult SetValueInt16(const FName Name, const int16 InValue);
	UE_API EPropertyBagResult SetValueUInt16(const FName Name, const uint16 InValue);
	...
	UE_API EPropertyBagResult SetValueEnum(const FName Name, const int64 InValue, const UEnum* Enum);
	...
	{
		static_assert(TIsEnum<T>::Value, "Should only call this with enum types");
		return SetValueEnum(Name, static_cast<int64>(InValue), StaticEnum<T>());
	}
	...
	UE_API EPropertyBagResult SetValueInt8(const FPropertyBagPropertyDesc& Desc, const int8 InValue);
	UE_API EPropertyBagResult SetValueByte(const FPropertyBagPropertyDesc& Desc, const uint8 InValue);
	UE_API EPropertyBagResult SetValueInt16(const FPropertyBagPropertyDesc& Desc, const int16 InValue);
	UE_API EPropertyBagResult SetValueUInt16(const FPropertyBagPropertyDesc& Desc, const uint16 InValue);
	...
	UE_API EPropertyBagResult SetValueEnum(const FPropertyBagPropertyDesc& Desc, const int64 InValue, const UEnum* Enum);
	...
	{
		static_assert(TIsEnum<T>::Value, "Should only call this with enum types");
		return SetValueEnum(Desc, static_cast<int64>(InValue), StaticEnum<T>());
	}
	...
	*/
	...
	/**
	 * Returns helper class to modify and access a map property.
	 * Note: The map reference is not valid after the layout of the referenced property bag has changed!
	 * @returns helper class to modify and access maps
	*/
	UE_API TValueOrError<FPropertyBagMapRef, EPropertyBagResult> GetMutableMapRef(const FName Name);

	/**
	 * Returns helper class to access a map property.
	 * Note: The map reference is not valid after the layout of the referenced property bag has changed!
	 * @returns helper class to modify and access maps
	*/
	UE_API TValueOrError<const FPropertyBagMapRef, EPropertyBagResult> GetMapRef(const FName Name) const;
...
public:	
	...
	UE_API TValueOrError<int8, EPropertyBagResult> GetValueInt8(const int32 Index) const;
	UE_API TValueOrError<uint8, EPropertyBagResult> GetValueByte(const int32 Index) const;
	UE_API TValueOrError<int16, EPropertyBagResult> GetValueInt16(const int32 Index) const;
	UE_API TValueOrError<uint16, EPropertyBagResult> GetValueUInt16(const int32 Index) const;
	...
	UE_API TValueOrError<int64, EPropertyBagResult> GetValueEnumInt64(const int32 Index, const UEnum* RequestedEnum) const;
	...
	TValueOrError<T, EPropertyBagResult> GetValueEnum(const int32 Index) const
	{
		...
		TValueOrError<int64, EPropertyBagResult> Result = GetValueEnumInt64(Index, StaticEnum<T>());
		if (!Result.IsValid())
		{
			return MakeError(Result.GetError());
		}
...
private:
	template<typename T>
	TValueOrError<T, EPropertyBagResult> GetNumericalValue(const int32 Index) const;
...
public:
    ...
    */
	...
	UE_API EPropertyBagResult SetValueInt8(const int32 Index, const int8 InValue);
	UE_API EPropertyBagResult SetValueByte(const int32 Index, const uint8 InValue);
	UE_API EPropertyBagResult SetValueInt16(const int32 Index, const int16 InValue);
	UE_API EPropertyBagResult SetValueUInt16(const int32 Index, const uint16 InValue);
	...
	UE_API EPropertyBagResult SetValueEnum(const int32 Index, const int64 InValue, const UEnum* Enum);
	...
	{
		static_assert(TIsEnum<T>::Value, "Should only call this with enum types");
		return SetValueEnum(Index, static_cast<int64>(InValue), StaticEnum<T>());
	}
...
public:	
	...
	UE_API EPropertyBagResult AddValueInt8(const int8 InValue);
	UE_API EPropertyBagResult AddValueByte(const uint8 InValue);
	UE_API EPropertyBagResult AddValueInt16(const int16 InValue);
	UE_API EPropertyBagResult AddValueUInt16(const uint16 InValue);
	...
	{
		static_assert(TIsEnum<T>::Value, "Should only call this with enum types");
		return AddValueEnum(static_cast<int64>(InValue), StaticEnum<T>());
	}
...
public:
	...
	/**
	 * Fix property descriptions that are not properly formed.
	 * Remove duplicated property descriptions.
	 * Remove unsupported types.
	 */
	static UE_API TArray<FPropertyBagPropertyDesc> SanitizePropertyDescs(const TArrayView<const FPropertyBagPropertyDesc> PropertyDescs);
...
protected:
	...
	{
		return PropertyDescs.Num() > 0;
	}

	UE_API virtual FName FindPropertyNameFromGuid(const FGuid& PropertyID) const override;
	UE_API virtual FGuid FindPropertyGuidFromName(const FName PropertyName) const override;
