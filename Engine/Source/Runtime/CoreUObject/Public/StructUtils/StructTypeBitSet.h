 ...
 */
struct FConstBitSetContainer : TBitArray<TInlineAllocator<8>>
{
	using FBitArrayBase = TBitArray;
	 ...
	 */
	bool HasAll(const FBitArrayBase& Other) const
	{
	 ...
	 */
	bool HasAny(const FBitArrayBase& Other) const
	{
		FConstWordIterator ThisIterator(*this);
		FConstWordIterator OtherIterator(Other);
	...
	FConstBitSetContainer operator&(const FBitArrayBase& Other) const
	{
		const int32 MinNumBits = FMath::Min(Num(), Other.Num());
		if (MinNumBits > 0)
		{
			FBitArrayBase Result(false, MinNumBits);

			FConstWordIterator IteratorA(*this);
			FConstWordIterator IteratorB(Other);

			FWordIterator IteratorResult(Result);

			for ( ; IteratorResult; ++IteratorResult, ++IteratorA, ++IteratorB)
			{
				const uint32 NewValue = IteratorA.GetWord() & IteratorB.GetWord();
				IteratorResult.SetWord(NewValue);
			}

			return FConstBitSetContainer(MoveTemp(Result));
		}
	...
	FConstBitSetContainer operator|(const FBitArrayBase& Other) const
	{
		const int32 MaxNumBits = FMath::Max(Num(), Other.Num());
		if (MaxNumBits > 0)
		{
			FBitArrayBase Result(false, MaxNumBits);
			...
			for (; IteratorA && IteratorB; ++IteratorResult, ++IteratorA, ++IteratorB)
			{
				const uint32 NewValue = IteratorA.GetWord() | IteratorB.GetWord();
				IteratorResult.SetWord(NewValue);
			}
			for (; IteratorA; ++IteratorResult, ++IteratorA)
			{
				const uint32 NewValue = IteratorA.GetWord();
				IteratorResult.SetWord(NewValue);
			}
			for (; IteratorB; ++IteratorResult, ++IteratorB)
			{
				const uint32 NewValue = IteratorB.GetWord();
				IteratorResult.SetWord(NewValue);
			}
	...
	FConstBitSetContainer AndNot(const FBitArrayBase& Other) const
	{
		const int32 NumResultBits = Num();
		if (NumResultBits > 0)
		{
			FBitArrayBase Result(false, NumResultBits);
			...
			for ( ; IteratorResult; ++IteratorResult, ++IteratorA, ++IteratorB)
			{
				const uint32 NewValue = IteratorB
					? IteratorA.GetWord() & ~IteratorB.GetWord()
					: IteratorA.GetWord();
				IteratorResult.SetWord(NewValue);
			}
	...
	{
		return (IsEmpty() && Other.IsEmpty()) || FBitArrayBase::operator==(Other);
	}
...
protected:
	 ...
	 */
	explicit FConstBitSetContainer(const FBitArrayBase& Source)
		: FBitArrayBase(Source)
	{
	}
	 ...
	 */
	explicit FConstBitSetContainer(FBitArrayBase&& Source)
		: FBitArrayBase(Forward<FBitArrayBase>(Source))
	{
	}
};
...
struct FBitSetContainer : FConstBitSetContainer
{
	...
	explicit FBitSetContainer(const FBitArrayBase& Source)
		: FConstBitSetContainer(Source)
	{
	}
	explicit FBitSetContainer(FBitArrayBase&& Source)
		: FConstBitSetContainer(Forward<FBitArrayBase>(Source))
	{
	}
	FBitSetContainer(FConstBitSetContainer&& Source)
		: FConstBitSetContainer(Forward<FConstBitSetContainer>(Source))
	{
	}

	FBitSetContainer& operator=(const FBitArrayBase& Other)
	{
		static_assert(sizeof(FBitSetContainer) == sizeof(FBitArrayBase)
			, "This operator requires FBitSetContainer and FBitArrayBase sizes to match (i.e. FBitSetContainer cannot have member variables nor virtual funtions)");
		*((FBitArrayBase*)this) = Other;
		return *this;
	}
	...
	FBitSetContainer& operator+=(const FBitArrayBase& Other)
	{
		// make sure we have enough space for all the contents of Other
		// not that it's fine to pass in a negative number, TBitArray::Add handles that gracefully. 
		Add(false, Other.Num() - Num());
...
struct FStructTracker
{
	...
	DECLARE_MULTICAST_DELEGATE_TwoParams(FOnTypeRegistered, TNotNull<const UStruct*> /*Type*/, int32 /*Index*/);
	 ...
	 */
	UE_API int32 FindStructTypeIndex(TNotNull<const UStruct*> InStructType) const;

	UE_DEPRECATED(5.8, "Use overload taking TNotNull")
	int32 FindStructTypeIndex(const UStruct& InStructType) const
	{
		return FindStructTypeIndex(&InStructType);
	}
	 ...
	 */
	UE_API int32 FindOrAddStructTypeIndex(TNotNull<const UStruct*> InStructType);

	UE_DEPRECATED(5.8, "Use overload taking TNotNull")
	int32 FindOrAddStructTypeIndex(const UStruct& InStructType)
	{
		return FindOrAddStructTypeIndex(&InStructType);
	}
	 ...
	 */
	int32 Register(TNotNull<const UStruct*> InStructType)
	{
	...
	FOnTypeRegistered OnTypeRegistered;
...
protected:
	UE_API int32 RegisterImplementation(TNotNull<const UStruct*> InStructType, const bool bCheckPrevious);

	UE_DEPRECATED(5.8, "Use overload taking TNotNull")
	int32 RegisterImplementation(const UStruct& InStructType, const bool bCheckPrevious)
	{
		return RegisterImplementation(&InStructType, bCheckPrevious);
	}
 ...
 */
template<typename TImplementation, typename TBaseStruct, typename TUStructType, typename TBitSetContainer, bool bTestInheritanceAtRuntime=WITH_STRUCTUTILS_DEBUG>
struct TTypeBitSetBase
{
	using FBaseStruct = TBaseStruct;
	using FContainer = TBitSetContainer;
	using FBitArrayBase = FStructTypeBitSet::FConstBitSetContainer::FBitArrayBase;
	...
	private:
		FBitArrayBase::FConstIterator It;
	 ...
	 */
	void Add(TNotNull<const TUStructType*> InStructType)
	{
		if constexpr (bTestInheritanceAtRuntime)
		{
	 ...
	 */
	void Remove(TNotNull<const TUStructType*> InStructType)
	{
		if constexpr (bTestInheritanceAtRuntime)
		{
			if (UNLIKELY(!ensureMsgf(InStructType->IsChildOf(GetBaseUStruct()),
	 ...
	 */
	bool Contains(TNotNull<const TUStructType*> InStructType) const
	{
		if constexpr (bTestInheritanceAtRuntime)
		{
			if (UNLIKELY(!ensureMsgf(InStructType->IsChildOf(GetBaseUStruct()),
	...
	UE_DEPRECATED(5.8, "Use overload taking TNotNull")
	void Contains(const TUStructType& InStructType)
	{
		Contains(&InStructType);
	}
	...
	{
		return TImplementation(StructTypesBitArray & Other.StructTypesBitArray);
	}
	...
	/**
	 * Performs a bitwise AND operation with the negation of the Other bitset.
	 * @return A new bitset representing the result of the operation.
	 */
	UE_FORCEINLINE_HINT TImplementation AndNot(const TImplementation& Other) const
	{
		return TImplementation(StructTypesBitArray.AndNot(Other.StructTypesBitArray));
	}
	...
	bool IsEquivalentFiltered(const TImplementation& Other, const TImplementation& Filter) const
	{
		constexpr uint32 MissingBitsFill = 0;

		FBitArrayBase::FConstWordIterator ThisIterator(StructTypesBitArray);
		FBitArrayBase::FConstWordIterator OtherIterator(Other.StructTypesBitArray);
		FBitArrayBase::FConstWordIterator FilterIterator(Filter.StructTypesBitArray);

		ThisIterator.FillMissingBits(MissingBitsFill);
		OtherIterator.FillMissingBits(MissingBitsFill);
		FilterIterator.FillMissingBits(MissingBitsFill);

		while (FilterIterator && (ThisIterator || OtherIterator))
		{
			const uint32 A = ThisIterator  ? (ThisIterator.GetWord() & FilterIterator.GetWord()) : MissingBitsFill;
			const uint32 B = OtherIterator ? (OtherIterator.GetWord() & FilterIterator.GetWord()) : MissingBitsFill;
			if (A != B)
			{
				return false;
			}
	 ...
	 */
	TTypeBitSetBase& operator+=(const TImplementation& Other)
	{
	 ...
	 */
	UE_FORCEINLINE_HINT TTypeBitSetBase& operator-=(const TImplementation& Other)
	{
		StructTypesBitArray -= Other.StructTypesBitArray;
		return *this;
	}
	 ...
	 */
	TImplementation operator+(TNotNull<const TUStructType*> NewElement) const
	{
	...
	UE_DEPRECATED(5.8, "Use overload taking TNotNull")
	TImplementation operator+(const TUStructType& NewElement) const
	{
		return operator+(&NewElement);
	}
	...
	{
		FBitArrayBase::FConstIterator It(StructTypesBitArray);
		while (It)
		{
			if (It.GetValue())
			{
...
protected:
	 ...
	 */
	explicit TTypeBitSetBase(const FContainer& InContainer)
		: StructTypesBitArray(InContainer)
	{
	}

	explicit TTypeBitSetBase(FBitArrayBase&& InContainer)
		: StructTypesBitArray(MoveTemp(InContainer))
	{
	}

#if WITH_STRUCTUTILS_DEBUG
	// For unit testing purposes only
	const FBitArrayBase& DebugGetStructTypesBitArray() const { return StructTypesBitArray; }
	FBitArrayBase& DebugGetMutableStructTypesBitArray() { return StructTypesBitArray; }
#endif // WITH_STRUCTUTILS_DEBUG
...
struct TStructTypeBitSet : TTypeBitSetBase<TStructTypeBitSet<TBaseStruct, TStructTrackerWrapper, TUStructType>, TBaseStruct, TUStructType, FStructTypeBitSet::FBitSetContainer>
{
	...
	using FBaseStruct = TBaseStruct;
	using FBitSetContainer = FStructTypeBitSet::FBitSetContainer;
	using FBitArrayBase = FStructTypeBitSet::FConstBitSetContainer::FBitArrayBase;
	...
	TStructTypeBitSet(const TStructTypeBitSet&) = default;
	TStructTypeBitSet(TStructTypeBitSet&&) = default;
	TStructTypeBitSet& operator=(const TStructTypeBitSet&) = default;
	TStructTypeBitSet& operator=(TStructTypeBitSet&&) = default;

	/**
	 * Constructor that initializes the bitset with a single struct type.
	 * @param StructType - The struct type to add.
	 */
	explicit TStructTypeBitSet(TNotNull<const TUStructType*> StructType)
	{
		Add(StructType);
	}
...
private:
	 ...
	 */
	TStructTypeBitSet(FBitArrayBase&& Source)
		: Super(MoveTemp(Source))
	{
	}

	TStructTypeBitSet(const FBitArrayBase& Source)
	{
		StructTypesBitArray = Source;
	}

public:
	explicit TStructTypeBitSet(const int32 BitToSet)
	{
	 ...
	 */
	static int32 GetTypeIndex(TNotNull<const TUStructType*> InStructType)
	...
	{
		return GetTypeIndex(&InStructType);
	}
	...
	{
		FBitArrayBase::FConstIterator It(Super::StructTypesBitArray);
...
#define _DECLARE_TYPEBITSET_IMPL(EXPORTED_API, ContainerTypeName, BaseType, BaseUStructType) struct ContainerTypeName##StructTrackerWrapper \
	...
	static_assert(std::is_move_constructible_v<ContainerTypeName> && std::is_move_assignable_v<ContainerTypeName>, "TStructTypeBitSet as configured is expected to be movable")
