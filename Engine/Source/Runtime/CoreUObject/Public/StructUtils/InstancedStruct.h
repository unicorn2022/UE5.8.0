...
public:
	...
	{
		checkf(InScriptStruct, TEXT("ScriptStruct cannot be nullptr"));
	...
	T& InitializeAs(TArgs&&... InArgs)
	{
		return InstancedStruct.InitializeAs<T>(Forward<TArgs>(InArgs)...);
	}
	...
	/**
	 * Cast directly to FInstancedStruct.
	 * Intended for compatibility with functions that require FInstancedStruct.
	 * Be sure you know what you're doing, as this will discard some type-checking safeguards!
	 */
	explicit operator const FInstancedStruct&() const &
	{
		return InstancedStruct;
	}

	/**
	 * Deleted overload to prevent accidental dangling refs to temps, such as
	 * const FInstancedStruct& BadRef = static_cast<const FInstancedStruct&>(TInstancedStruct<T>::Make());
	 */
	explicit operator const FInstancedStruct&() && = delete;
	...
	[[nodiscard]] UE_REWRITE BaseStructT& operator*() requires (!std::is_const_v<BaseStructT>)
	{
		return GetMutablePtr();
	}

	/** Returns an immutable reference. */
	[[nodiscard]] UE_REWRITE const std::decay_t<BaseStructT>& operator*() const
	{
		return GetPtr();
	}
