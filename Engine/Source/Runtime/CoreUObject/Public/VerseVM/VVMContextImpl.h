...
struct AUTORTFM_DISABLE FNativeFrame
{
	VFailureContext* FailureContext = nullptr;
	VTask* Task = nullptr;
	VBatchedRefs* BatchedRefs = nullptr;

	VValue EffectToken;

	const VNativeProcedure* Callee = nullptr;
	...
	COREUOBJECT_API bool IsCurrent(FAccessContext Context) const;
	...
	// Walks frames in this order:
	// - 'this' native frame if its caller is 'Frame' to handle native call's being the starting point
	// - Loops over VFrame->CallerFrame till we hit a null CallerFrame:
	//   - if we hit the 'root frame' of the current task during this loop, swaps to Yield Task/Frame and continues
	// - If 'PreviousNativeFrame', recurses at end with CallerFrame and Task
	//
	// FrameCallback receives (const FOp* PC, VFrame*) where PC is the bytecode op
	// at which that VFrame is paused: InPC for the topmost frame, the previous
	// frame's CallerPC walking down the chain, the parent task's YieldPC across
	// a task-spawn pivot, or the native frame's CallerPC across a native boundary.
	template <typename VFrameCallback, typename NativeFrameCallback>
	AUTORTFM_INFER void WalkCallstackFrames(const FOp* InPC, VFrame* InFrame, VTask* InTask, VFrameCallback FrameCallback, NativeFrameCallback NativeCallback) const;
...
struct FContextImpl
{
	...
	COREUOBJECT_API void EnterConservativeStack(AUTORTFM_IMPLICIT_DISABLE TFunctionRef<void()>);
	COREUOBJECT_API void ExitConservativeStack(AUTORTFM_IMPLICIT_DISABLE TFunctionRef<void()>);
	...
	{
		return State.load(std::memory_order_relaxed) & HasDebuggerBit;
	}
	...
	void AddOverheadTime(double Duration);
	void PauseComputationWatchdog() { bVerseIsRunning.store(false, std::memory_order_seq_cst); }
	void ResumeComputationWatchdog();
	void StopComputationWatchdog();
	COREUOBJECT_API static void AttachedDebugger();
	COREUOBJECT_API static void DetachedDebugger();

	AUTORTFM_DISABLE COREUOBJECT_API VPackage* SetCurrentPackage(VPackage* Package);
	...
	{
#if WITH_EDITORONLY_DATA
		V_DIE_UNLESS(ImportQueue.IsNull() && !Path.IsNull());
		ImportQueue = Path;
#endif
	}
	...
	{
#if WITH_EDITORONLY_DATA
		V_DIE_UNLESS(ImportQueue.IsValid());
		FTopLevelAssetPath Result = ImportQueue;
		ImportQueue.Reset();
		return Result;
#else
		VERSE_UNREACHABLE();
...
private:
	...
	// Incremented by native writes. Used to detect when a nested VNativeRef may have been invalidated.
	uint64 CurrentWriteEpoch = 0;
	...
	std::atomic<double> TotalOverheadTime = 0.0;
	std::atomic<bool> bStopRequested = false;
	std::atomic<bool> bVerseIsRunning = true;
	UE::FEventCount WaitForVerseEvent;
	UE::FEventCount StopWatchdogEvent;

#if WITH_EDITORONLY_DATA
	VWeakCellMap* CellToPackage = nullptr;
	VPackage* CurrentPackage = nullptr;

	// Single-element "queue" of @import_as paths requested by the compiler.
	FTopLevelAssetPath ImportQueue;
#endif
};
