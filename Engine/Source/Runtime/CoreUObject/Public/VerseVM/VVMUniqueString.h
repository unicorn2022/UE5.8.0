...
private:
	...
	AUTORTFM_DISABLE COREUOBJECT_API VUniqueString& Intern(FAllocationContext Context, FUtf8StringView String);
	...
	TWeakBarrierSet<VUniqueString, FUniqueStringSetKeyFuncs<TWeakBarrier<VUniqueString>>> UniqueStrings;
...
struct VUniqueString final : VArray
{
	 ...
	 */
	AUTORTFM_DISABLE static VUniqueString& New(FAllocationContext Context, FUtf8StringView String)
	{
	...
	AUTORTFM_DISABLE static void SerializeLayout(FAllocationContext Context, VUniqueString*& This, FStructuredArchiveVisitor& Visitor);
	AUTORTFM_DISABLE void SerializeImpl(FAllocationContext Context, FStructuredArchiveVisitor& Visitor);

private:
	AUTORTFM_DISABLE static VUniqueString& Make(FAllocationContext Context, FUtf8StringView String)
	{
...
private:
	...
	AUTORTFM_DISABLE static VUniqueStringSet& Make(FAllocationContext Context, const TSet<VUniqueString*>& InSet);
...
private:
	...
	AUTORTFM_DISABLE COREUOBJECT_API VUniqueStringSet& Intern(FAllocationContext Context, const TSet<VUniqueString*>& InSet);

	/// This gives the pool the ability to conduct census on its own to clear references to the sets.
	COREUOBJECT_API virtual void ConductCensus() override;

	TWeakBarrierSet<VUniqueStringSet, FHashableUniqueStringSetKeyFuncs> Sets;
