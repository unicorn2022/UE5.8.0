...
struct VValue
{
	// VValue can be returned from AutoRTFM::Open() via a copy-construction in the closed.
	...
	AUTORTFM_DISABLE static ECompares Equal(FAllocationContext Context, VValue Left, VValue Right, HandlePlaceholderFunction HandlePlaceholder);

	// This may return a placeholder which is suspended upon as a FOpResult::Block result would be.
	AUTORTFM_DISABLE static VValue Melt(FAllocationContext Context, VValue Value);
	AUTORTFM_DISABLE static FOpResult Freeze(FAllocationContext Context, VValue Value, VTask* Task);
	...
	VCell* ExtractCell() const
	{
		...
		{
			return reinterpret_cast<VCell*>(Ref);
		}
		return nullptr;
	}
