...
struct FInitOrValidateUEnum : FInitOrValidateUField
{
	...
	COREUOBJECT_API void SetEnums(TArray<TPair<FName, int64>>& InNames, UEnum::ECppForm InCppForm, UEnum::EUnderlyingType InUnderlyingType) const;
...
struct FVerseVMInitOrValidate : FInitOrValidatorSelector<UEType>::Validator
{
	...
	~FVerseVMInitOrValidate()
	{
		if (bGotError)
		{
			UE_LOGF(LogVerseValidation, Fatal, "Type \"%ls\" validation terminated due to mismatches with UHT type.", *Super::GetField()->GetName());
		}
	}

	virtual void LogError(const FString& text) const override
	{
		UE_LOGF(LogVerseValidation, Error, "Type \"%ls\" %ls", *Super::GetField()->GetName(), *text);
		bGotError = true;
	}
