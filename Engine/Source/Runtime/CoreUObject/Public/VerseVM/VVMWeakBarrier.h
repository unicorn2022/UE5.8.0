...
// Define a TWeakBarrierSet that uses TSparseSet to avoid TCompactSet's need to compute hashes on removal.
// This could also alternatively use TSet/TCompactSet, but wrap the element type in a TStoredHash element type.
template <typename InElementType, typename KeyFuncs = DefaultKeyFuncs<TWeakBarrier<InElementType>>, typename Allocator = FDefaultSparseSetAllocator>
using TWeakBarrierSet = TSparseSet<TWeakBarrier<InElementType>, KeyFuncs, Allocator>;
