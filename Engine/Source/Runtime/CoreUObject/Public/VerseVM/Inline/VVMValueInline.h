...
{
	return GetTypeHash(RestValue.Value.Follow());
}
...
inline FOpResult VValue::Freeze(FAllocationContext Context, VValue Value, VTask* Task)
{
	...
	{
		return Value.AsCell().Freeze(Context, Task);
	}
	V_RETURN(Value);
}
