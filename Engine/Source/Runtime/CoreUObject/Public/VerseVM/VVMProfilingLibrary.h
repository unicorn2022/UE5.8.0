...
/** Source range of a profile-time block */
USTRUCT()
struct FProfileLocus
{
	GENERATED_BODY()
public:
	uint64_t BeginRow;    // 1-based, inclusive
	uint64_t BeginColumn; // 1-based, inclusive (UTF-8 byte offset)
	uint64_t EndRow;      // 1-based
	uint64_t EndColumn;   // 1-based (UTF-8 byte offset)
