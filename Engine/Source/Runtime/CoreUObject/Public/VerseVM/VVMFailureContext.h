...
struct AUTORTFM_DISABLE VFailureContext : VCell
	, TIntrusiveTree<VFailureContext>
{
	...
	static VFailureContext& New(
		FAllocationContext Context,
		VTask* Task,
		VFailureContext* Parent,
		VFrame& Frame,
		VValue IncomingEffectToken,
		VBatchedRefs* BatchedRefs,
		FOp* FailurePC)
	{
		return *new (Context.AllocateFastCell(sizeof(VFailureContext))) VFailureContext(
			Context,
			Task,
			Parent,
			Frame,
	...
	template <FTransaction::EAbortMode AbortMode = FTransaction::EAbortMode::Default>
	void Fail(FAccessContext Context)
	{
		...
		uint32 NumActiveTransactions = 0;
		TArray<FTrail*, TInlineAllocator<4>> PreOrder;
		ForEachInPreOrder([&](VFailureContext& FailureContext) {
			PreOrder.Push(&FailureContext.Trail);
		...
		for (uint32 I = 0; I < NumActiveTransactions; ++I)
		{
			Context.CurrentTransaction()->Abort<AbortMode>(Context);
		}

		// Backtrack for all children before their parent.
		for (FTrail* T : ReverseIterate(PreOrder))
		{
			T->Log.Backtrack(Context);
			T->Log.Empty();
		}
	}
	...
	VFailureContext(
		...
		, BatchedRefs(Context, BatchedRefs)
		, FailurePC(FailurePC)
		, Trail(Parent ? &Parent->Trail : nullptr)
	{
	}
};
