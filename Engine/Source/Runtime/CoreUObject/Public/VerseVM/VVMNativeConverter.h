...
struct AUTORTFM_DISABLE FNativeConverter
{
	...
	static FOpResult FromVValue(FAllocationContext Context, const VValue Value, TFromVValue<EVerseTrue>& OutNative)
	{
		...
		OutNative.Value = EVerseTrue::Value;
	...
	static FOpResult FromVValue(FAllocationContext Context, const VValue Value, TFromVValue<FNativeString>& OutNative)
	{
		...
		if (TOptional<FUtf8StringView> View = Array.AsOptionalStringView())
		{
			OutNative.Value = *View;
			return {FOpResult::Return};
		}
		if (TOptional<FUtf8String> String = Array.AsOptionalUtf8String())
		{
			OutNative.Value = MoveTemp(*String);
			return {FOpResult::Return};
		}
		V_DIE("Unable to convert array to string");
	}
...
template <typename NativeType>
NativeType FVRestValueProperty::GetVersePropertyValue_InContainer(const void* Container) const
{
	return AutoRTFM::Open([this, Container] {
		Verse::FRunningContext Context = Verse::FRunningContextPromise{};
		Verse::VRestValue* RestValue = const_cast<Verse::VRestValue*>(GetPropertyValuePtr_InContainer(Container));
		Verse::TFromVValue<NativeType> NativeValue;
		Verse::FOpResult Result = Verse::FNativeConverter::FromVValue(Context, RestValue->Get(Context), NativeValue);
