...
struct FContext
{
	...
	static FNativeFrame* NativeFrame()
	{
		FContextImpl* Impl = FContextImpl::GetCurrentImpl();
		return (Impl && Impl->IsLive()) ? Impl->NativeFrame() : nullptr;
	}
...
protected:
	...
	friend struct FSamplingProfiler;
...
struct FIOContext : FContext
{
	...
	static void Create(AUTORTFM_IMPLICIT_DISABLE const TFunc& Func, EContextHeapRole HeapRole = EContextHeapRole::Mutator)
	{
	...
	template <typename TFunc>
	void AcquireAccess(AUTORTFM_IMPLICIT_DISABLE const TFunc& Func) const;
	...
	COREUOBJECT_API void PairHandshake(FContext Context, AUTORTFM_IMPLICIT_DISABLE TFunctionRef<void(FHandshakeContext)> HandshakeAction) const;
	...
	COREUOBJECT_API void SoftHandshake(AUTORTFM_IMPLICIT_DISABLE TFunctionRef<void(FHandshakeContext)> HandshakeAction) const;
	...
	COREUOBJECT_API void HardHandshake(AUTORTFM_IMPLICIT_DISABLE TFunctionRef<void(FHardHandshakeContext)> HandshakeAction) const;
...
public:
	FPackageScope(FInitializer Initializer)
		: Impl(Initializer.Impl)
		, OldPackage(Impl->SetCurrentPackage(Initializer.NewPackage))
	{
	}
	...
	FPackageScope& operator=(FPackageScope&& Other) = delete;
...
struct FAccessContext : FContext
{
	...
	// Called when a native call to C++, via AutoRTFM::Close() returns a result
	// that is not Executing. This function will handle the failure based on the
	// failure kind.
	AUTORTFM_DISABLE COREUOBJECT_API FOpResult HandleAutoRTFMFailure(AutoRTFM::ETransactionStatus TransactionResult);

	// Calls Functor in the closed.
	// If the transaction fails, then the failure status is translated to an
	// FOpResult and is returned.
	// Functor must either return void or FOpResult.
	// If Functor returns FOpResult then this will be returned by Close() (on
	// transaction success).
	template <typename FunctorType, typename FunctorReturnType = decltype(std::declval<FunctorType>()())>
		requires(std::is_same_v<FunctorReturnType, void> || std::is_same_v<FunctorReturnType, FOpResult>)
	[[nodiscard]] AUTORTFM_DISABLE FOpResult Close(AUTORTFM_IMPLICIT_ENABLE FunctorType&& Functor)
	{
		FOpResult Result{FOpResult::Return};
		AutoRTFM::ETransactionStatus Status = AutoRTFM::Close([&] {
			if constexpr (std::is_same_v<FunctorReturnType, FOpResult>)
			{
				Result = Functor();
			}
			else
		...
		{
			return {HandleAutoRTFMFailure(Status)};
		}
		return Result;
	}

	// If bCondition is true, then Functor is passed to FAccessContext::Close()
	// and its result is returned, otherwise the function is called immediately
	// in the open and FOpResult::Return is returned.
	template <typename FunctorType>
	[[nodiscard]] AUTORTFM_DISABLE FOpResult CloseIf(bool bCondition, AUTORTFM_IMPLICIT_ENABLE FunctorType&& Functor)
	{
		if (bCondition)
		{
			return Close(Functor);
		}
	...
	uint64 GetWriteEpoch()
	{
		return GetImpl()->CurrentWriteEpoch;
	}

	void BumpWriteEpoch()
	{
		GetImpl()->CurrentWriteEpoch++;
	}

	[[nodiscard]] FPackageScope::FInitializer SetCurrentPackage(VPackage* Package)
	{
		return FPackageScope::FInitializer(GetImpl(), Package);
	}
	...
	void PushImport(FTopLevelAssetPath Path)
	{
		return GetImpl()->PushImport(Path);
	}
	FTopLevelAssetPath PopImport()
	{
		return GetImpl()->PopImport();
	}
...
struct FRunningContext : FAccessContext
{
	...
	template <typename TFunctor>
	void EnterVM(AUTORTFM_IMPLICIT_DISABLE TFunctor F)
	{
		AutoRTFM::UnreachableIfClosed("#jira SOL-8415");
	...
	template <typename TFunctor>
	void TransactThenEnterVM(AUTORTFM_IMPLICIT_DISABLE TFunctor F)
	{
...
struct FAllocationContext : FAccessContext
{
	...
	AUTORTFM_DISABLE std::byte* AllocateFastCell(size_t NumBytes) const
	{
		CheckInvariants();
		return GetImpl()->AllocateFastCell(NumBytes);
	}

	AUTORTFM_DISABLE std::byte* TryAllocateFastCell(size_t NumBytes) const
	{
		CheckInvariants();
		return GetImpl()->TryAllocateFastCell(NumBytes);
	}

	AUTORTFM_DISABLE std::byte* AllocateAuxCell(size_t NumBytes) const
	{
		CheckInvariants();
		return GetImpl()->AllocateAuxCell(NumBytes);
	}

	AUTORTFM_DISABLE std::byte* TryAllocateAuxCell(size_t NumBytes) const
	{
		CheckInvariants();
	...
	AUTORTFM_DISABLE std::byte* AllocateEmergentType(size_t NumBytes) const
	{
		CheckInvariants();
		return FHeap::EmergentSpace->Allocate(NumBytes);
	}

	AUTORTFM_DISABLE std::byte* TryAllocate(FSubspace* Subspace, size_t NumBytes) const
	{
		CheckInvariants();
		return Subspace->TryAllocate(NumBytes);
	}
	AUTORTFM_DISABLE std::byte* TryAllocate(FSubspace* Subspace, size_t NumBytes, size_t Alignment) const
	{
		CheckInvariants();
		return Subspace->TryAllocate(NumBytes, Alignment);
	}
	AUTORTFM_DISABLE std::byte* Allocate(FSubspace* Subspace, size_t NumBytes) const
	{
		CheckInvariants();
		return Subspace->Allocate(NumBytes);
	}
	AUTORTFM_DISABLE std::byte* Allocate(FSubspace* Subspace, size_t NumBytes, size_t Alignment) const
	{
		CheckInvariants();
