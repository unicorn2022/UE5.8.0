...
struct FOpEmitter
{
	...
	VUniqueString& GetProcedureName()
	{
	...
	void EnterUnwindRegion(FLabel OnUnwind)
	{
		...
		// Leave the region on the top of the stack. It will be re-entered when we leave the new region.
	...
	void NoteUnwind()
	{
		...
		{
			FUnwindRegion& Region = UnwindStack.Top();
			Region.bHandlesUnwind = true;
			Region.bLabelInvoked = true;
		}
	}

	// Returns true if any unwinds were recorded at this level of the stack.
	bool LeaveUnwindRegion()
	{
		int32 Offset = OpBytes.Num();
		...
		// Re-enter the next region on the stack, but preserve its bLabelInvoked.
		FUnwindRegion& Outer = UnwindStack.Top();
		Outer.Begin = Offset;
		Outer.bHandlesUnwind = false;

		return Inner.bLabelInvoked;
	}

	AUTORTFM_DISABLE COREUOBJECT_API VProcedure& MakeProcedure(FAllocationContext Context);
...
private:
	...
	template <typename OpType>
	void MaybeNoteUnwind(OpType& Op)
	{
	...
	void MaybeNoteUnwind(FOpYield& Op)
	{
		// Yield's NoteUnwind must be called at its resume label.
	}

	void MaybeNoteUnwind(FOpCall& Op)
	{
		if (Op.bCalleeYields)
		{
			NoteUnwind();
	...
	void MaybeNoteUnwind(FOpCallWithSelf& Op)
	{
		if (Op.bCalleeYields)
		{
			NoteUnwind();
