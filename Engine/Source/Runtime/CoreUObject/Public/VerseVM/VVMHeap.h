...
LLM_DECLARE_TAG_API(Libpas, COREUOBJECT_API);
...
public:
	...
	AUTORTFM_DISABLE COREUOBJECT_API static void Initialize();
	...
	// It doesn't mean it's actually a valid cell, but it does mean that libpas owns this address.
	// This doesn't alter the contents of the heap, so it's safe to call from the closed, and can be useful
	// if you need to do pointer sanity-checking.
	AUTORTFM_OPEN COREUOBJECT_API static bool OwnsAddress(void*);
...
private:
	...
	static void OnLowLevelMemTrackerTick();
	...
	static FDelegateHandle OnLLMTickHandle;
