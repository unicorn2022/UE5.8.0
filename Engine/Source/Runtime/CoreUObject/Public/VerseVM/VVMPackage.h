...
struct VPackage : VCell
{
	...
	AUTORTFM_DISABLE COREUOBJECT_API void NotifyUsedTupleType(FAllocationContext Context, VTupleType* TupleType);
	template <typename FunctorType> // FunctorType is (VTupleType*) -> void
	void ForEachUsedTupleType(FunctorType&& Functor);

	AUTORTFM_DISABLE COREUOBJECT_API void NotifyUsedImport(FAllocationContext Context, VNamedType* TypeWithImport);
	...
	AUTORTFM_DISABLE static VPackage& New(FAllocationContext Context, VUniqueString& Name, VUniqueString& RootPath, uint32 Capacity)
	{
	...
	AUTORTFM_DISABLE static void SerializeLayout(FAllocationContext Context, VPackage*& This, FStructuredArchiveVisitor& Visitor);
...
private:
	...
	AUTORTFM_DISABLE COREUOBJECT_API VPackage(FAllocationContext Context, VUniqueString* InName, VUniqueString* InRootPath, uint32 Capacity);
	~VPackage();
