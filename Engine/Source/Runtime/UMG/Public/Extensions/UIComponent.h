...
#if WITH_EDITOR
	/**
	 * Called during asset validation to validate this component in the context of its owner widget.
	 * Override to perform owner-aware validation. Calls IsDataValid by default.
	 * @param OwnerWidget	The widget this component is attached to, or nullptr if it could not be resolved.
	 * @param Context		The validation context to add errors/warnings to.
	 */
	UMG_API virtual EDataValidationResult IsDataValidForOwner(const UWidget* OwnerWidget, FDataValidationContext& Context) const;
#endif // WITH_EDITOR
