...
protected:
	...
	/** When enabled, overrides the render target pixel format instead of using the default Slate format (RGBA8). */
	UPROPERTY(EditAnywhere, Category=Rendering, AdvancedDisplay)
	bool bOverrideRenderTargetFormat;

	/** The render target format to use when bOverrideRenderTargetFormat is true. */
	UPROPERTY(EditAnywhere, Category=Rendering, AdvancedDisplay, meta=(EditCondition="bOverrideRenderTargetFormat"))
	TEnumAsByte<ETextureRenderTargetFormat> RenderTargetFormatOverride;
