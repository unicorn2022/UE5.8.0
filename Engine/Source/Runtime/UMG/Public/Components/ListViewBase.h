...
public:
	...
	DECLARE_MULTICAST_DELEGATE(FOnTouchStart);
	virtual FOnTouchStart& OnTouchStart() const = 0;

	DECLARE_MULTICAST_DELEGATE(FOnTouchMove);
	virtual FOnTouchMove& OnTouchMove() const = 0;

	DECLARE_MULTICAST_DELEGATE(FOnTouchEnd);
	virtual FOnTouchEnd& OnTouchEnd() const = 0;
...
protected:
		...
		const FListViewConstructArgs& Args = FListViewConstructArgs())
	{
		...
		TSharedRef<ListViewT<ItemType>> ListView = SNew(ListViewT<ItemType>)
			...
			.EnableProximateEntryNavigation(Args.bEnableProximateEntryNavigation)
			...
			.OnTouchStart_UObject(Implementer, &UListViewBaseT::HandleTouchStart)
			.OnTouchMove_UObject(Implementer, &UListViewBaseT::HandleTouchMove)
			.OnTouchEnd_UObject(Implementer, &UListViewBaseT::HandleTouchEnd)
...
protected:
	...
	virtual void OnListViewTouchStartInternal() {}
	virtual void OnListViewTouchMoveInternal() {}
	virtual void OnListViewTouchEndInternal() {}
...
private:
	...
	{
		OnListViewTouchStartInternal();
		OnTouchStart().Broadcast();
	}

	void HandleTouchMove()
	{
		OnListViewTouchMoveInternal();
		OnTouchMove().Broadcast();
	}

	void HandleTouchEnd()
	{
		OnListViewTouchEndInternal();
...
public:
	...
	UFUNCTION(BlueprintCallable, Category = ListViewBase)
	float GetOverscroll() const { return MyTableViewBase->GetOverscroll(); }
	...
	/** Enables/Disable touch scrolling **/
	UFUNCTION(BlueprintCallable, Category = ListViewBase)
	UMG_API void SetIsTouchScrollingEnabled(bool bInIsTouchScrollingEnabled);
...
private:	\
	...
	mutable FOnTouchStart OnTouchStartEvent;	\
	mutable FOnTouchMove OnTouchMoveEvent;	\
	mutable FOnTouchEnd OnTouchEndEvent;	\
...
public:	\
	...
	virtual FOnTouchStart& OnTouchStart() const override { return OnTouchStartEvent; }	\
	virtual FOnTouchMove& OnTouchMove() const override { return OnTouchMoveEvent; }	\
	virtual FOnTouchEnd& OnTouchEnd() const override { return OnTouchEndEvent; }	\
