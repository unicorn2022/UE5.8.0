...
public:
	...
	DECLARE_MULTICAST_DELEGATE_TwoParams(FOnEditableTextBoxCursorMovedWithSelectionEvent, const FTextLocation&, const FTextSelection&);
	DECLARE_DYNAMIC_MULTICAST_SPARSE_DELEGATE(FOnAllFontFacesFinishLoading_EditableTextBox, UEditableTextBox, OnAllFontFacesFinishLoading);
public:
...
public:
	...
	FOnEditableTextBoxCursorMovedWithSelectionEvent OnCursorMovedWithSelectionEvent;
...
public:
	...
	*/
	...
	/** Whether any text is selected in the text block */
	UMG_API bool IsAnyTextSelected() const;
	...
	UMG_API void SetFontFacesLoadingPaintPolicy(EFontFacesLoadingPaintPolicy InFontFacesLoadingPaintPolicy);
...
protected:
	...
	UMG_API virtual void HandleOnCursorMovedWithSelection(const FTextLocation& InLocation, const FTextSelection& Selection);

#if WITH_ACCESSIBILITY
	UMG_API virtual TSharedPtr<SWidget> GetAccessibleWidget() const override;
#endif

	void SlateHandleAllFontFacesFinishLoading()
	{
		OnAllFontFacesFinishLoading.Broadcast();
	}
protected:
...
protected:
	/** Controls how painting the text in the widget should behave when font faces for the text is still loading. */
	UPROPERTY(EditAnywhere, Setter, Category = Font)
	EFontFacesLoadingPaintPolicy FontFacesLoadingPaintPolicy;

	/** Called when all font faces for the text has finished async loading. Is not called if font faces are synchronously loaded. */
	UPROPERTY(BlueprintAssignable, Category = "Widget Event", meta = (DisplayName = "OnAllFontFacesFinishLoading(Editable Text)"))
	FOnAllFontFacesFinishLoading_EditableTextBox OnAllFontFacesFinishLoading;
};
