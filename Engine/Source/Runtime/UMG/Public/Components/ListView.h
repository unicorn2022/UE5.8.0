...
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnListViewTouchStartDynamic);
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnListViewTouchMoveDynamic);
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnListViewTouchEndDynamic);
...
public:
	...
	/** Adds an the item to the end of the list */
	UFUNCTION(BlueprintCallable, Category = ListView)
	UMG_API void AddItem(UObject* Item);

	/** Adds an the item to the list at the given index */
	UFUNCTION(BlueprintCallable, Category = ListView)
	UMG_API void AddItemAt(UObject* Item, int32 Index);

	/** Adds multiple items to the end of the list */
	UFUNCTION(BlueprintCallable, Category = ListView)
	UMG_API void AddItems(const TArray<UObject*>& Items);

	/** Adds multiple items to the list starting at the given index */
	UFUNCTION(BlueprintCallable, Category = ListView)
	UMG_API void AddItemsAt(const TArray<UObject*>& Items, int32 Index);
	...
	UFUNCTION(BlueprintCallable, Category = ListView)
	UMG_API void RemoveItems(const TArray<UObject*>& Items);
	...
	/** Sets whether spatially proximate focus routing is enabled for this list view at runtime. */
	UFUNCTION(BlueprintCallable, Category = ListView)
	UMG_API void SetEnableProximateEntryNavigation(bool bNewValue);
...
protected:
	...
	UMG_API virtual void OnListViewTouchStartInternal() override;
	UMG_API virtual void OnListViewTouchMoveInternal() override;
	UMG_API virtual void OnListViewTouchEndInternal() override;
	...
	TSharedRef<ListViewT<UObject*>> ConstructListView()
	{
		...
		Args.bEnableProximateEntryNavigation = bEnableProximateEntryNavigation;
...
protected:
	...
	/**
	 * EXPERIMENTAL - Enables the ability to naturally navigate items within this listview, always
	 * preferring the closest focusable target. This also works with nested focus targets.
	 */
	UPROPERTY(Experimental, EditAnywhere, BlueprintReadOnly, Category = ListView)
...
public:
	...
	/** Set the return Focus to selection flag. */
	UFUNCTION(BlueprintCallable, Category = ListView)
...
private:
	...
	UPROPERTY(BlueprintAssignable, Category = Events, meta = (DisplayName = "On List View Touch Start"))
	FOnListViewTouchStartDynamic BP_OnListViewTouchStart;

	UPROPERTY(BlueprintAssignable, Category = Events, meta = (DisplayName = "On List View Touch Move"))
	FOnListViewTouchMoveDynamic BP_OnListViewTouchMove;

	UPROPERTY(BlueprintAssignable, Category = Events, meta = (DisplayName = "On List View Touch End"))
	FOnListViewTouchEndDynamic BP_OnListViewTouchEnd;
