...
public:
	...
	*/
	...
	/** Find an existing extension of the request type, or create it if not found */
	template<typename ExtensionType>
	ExtensionType& GetOrAddExtension()
	{
		return *CastChecked<ExtensionType>(GetOrAddExtension(ExtensionType::StaticClass()));
	}

	/** Find an existing extension of the request type, or create it if not found */
	UFUNCTION(BlueprintCallable, Category = "User Interface|Extension", Meta = (DeterminesOutputType = "InExtensionType"))
	UMG_API UUserWidgetExtension* GetOrAddExtension(TSubclassOf<UUserWidgetExtension> InExtensionType);
