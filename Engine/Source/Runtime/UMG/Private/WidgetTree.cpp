...
#if WITH_EDITOR
bool UWidgetTree::IsCapturingAsRootObjectForTransaction() const 
{ 
	// Track changes for child UWidgets with this WidgetTree as the outer
	return true; 
}
#endif //WITH_EDITOR
...
UWidget* UWidgetTree::FindWidget(const FName& Name) const
{
	...
	ForEachWidgetUntil([&FoundWidget, &Name] (UWidget* Widget) {
...
UWidget* UWidgetTree::FindWidget(TSharedRef<SWidget> InWidget) const
{
	...
	ForEachWidgetUntil([&FoundWidget, &InWidget] (UWidget* Widget) {
...
bool UWidgetTree::TryMoveWidgetToNewTree(UWidget* Widget, UWidgetTree* DestinationTree)
{
	...
	{
		bWidgetMoved = Widget->Rename(*Widget->GetName(), DestinationTree, REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
	}
...
void UWidgetTree::PreSave(FObjectPreSaveContext ObjectSaveContext)
{
	...
	ForEachWidgetAndDescendants([this](UWidget* InChildWidget) {
		...
		{
			UE_LOGF(LogUMG, Warning, "WidgetTree(%ls) Contains Foreign Child (%ls)", *GetPathName(), *InChildWidget->GetPathName());
		}
	});
#endif
...
void UWidgetTree::FixupOrphanedWidgetNames()
{
	// Build set of widgets actually referenced by panel slots
    TSet<UWidget*> Widgets;
    TSet<UWidget*> ReferencedWidgets;
	ForEachWidget([&](UWidget* Widget)
    {
        ...
        if (UPanelWidget* Panel = Cast<UPanelWidget>(Widget))
        {
            for (int32 i = 0; i < Panel->GetChildrenCount(); ++i)
            {
                ...
                {
                    ReferencedWidgets.Add(Child);
                }
            }
        }
    });

    for (const auto& NamedSlotEntry : NamedSlotBindings)
    {
        if (UWidget* NamedSlotContent = NamedSlotEntry.Value)
        {
            ReferencedWidgets.Add(NamedSlotContent);
        }
    }

    TMap<FName, UWidget*> NameToWidgetMap;
    for(UWidget* Widget : Widgets)
    {
        FName Name = Widget->GetFName();
        if (UWidget** ExistingWidget = NameToWidgetMap.Find(Name))
        {
			// Rename whichever widget is not already referenced 
            UWidget* WidgetToRename = ReferencedWidgets.Contains(Widget) ? *ExistingWidget : Widget;
            const FName NewName = MakeUniqueObjectName(WidgetToRename->GetOuter(), WidgetToRename->GetClass(), Name);

			UE_LOGF(LogUMG, Warning, "WidgetTree(%ls) Contain duplicate FName (%ls). Referencing both '%ls' and '%ls'", *GetPathName(), *Name.ToString(), *((*ExistingWidget)->GetPathName()), *Widget->GetPathName());
			UE_LOGF(LogUMG, Warning, "Renaming '%ls' to '%ls'", *WidgetToRename->GetName(), *NewName.ToString());

			WidgetToRename->Rename(*NewName.ToString(), nullptr, REN_DoNotDirty);
			NameToWidgetMap[NewName] = Widget;
        }
        else
        {
            NameToWidgetMap.Add(Name, Widget);
        }
    }
}
