...
FWidgetDesignTimeInfo UWidgetLayoutLibrary::GetDesignTimeInfo(const UWidget* Widget)
{
	FWidgetDesignTimeInfo Info;

	// Validate that the caller is in a design-time preview context.
	if (!Widget || !Widget->IsDesignTime())
	{
		UE_LOGF(LogUMG, Error, "GetDesignTimeInfo() called outside of a design-time preview. This function should only be called from PreConstruct in the UMG Designer.");
		return Info;
	}
	...
	const FString ConfigSection = TEXT("UMGEditor.Designer");

	GConfig->GetInt(*ConfigSection, TEXT("PreviewWidth"), PreviewWidth, GEditorPerProjectIni);
	GConfig->GetInt(*ConfigSection, TEXT("PreviewHeight"), PreviewHeight, GEditorPerProjectIni);

	Info.PreviewSize = FVector2D(static_cast<double>(PreviewWidth), static_cast<double>(PreviewHeight));
	Info.bIsPortrait = PreviewHeight > PreviewWidth;
#endif
