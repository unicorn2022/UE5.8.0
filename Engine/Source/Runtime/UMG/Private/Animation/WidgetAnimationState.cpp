...
void FWidgetAnimationState::Initialize(UWidgetAnimation* InAnimation, UUserWidget* InUserWidget)
{
	...
	{
		UE_LOGF(LogUMG, Log, "Animation: Initializing '%ls' playing on '%ls' (%p), instance [%d]%ls", 
...
void FWidgetAnimationState::Tick(float InDeltaSeconds)
{
	...
	if (!UserWidget || !SharedPlaybackState)
	{
		...
		if (UE::UMG::GVarAnimationDetailedLogging)
		{
			UE_LOGF(LogUMG, Log, "Animation: Ticking '%ls' on '%ls' (%p) aborted, invalid state.",
	...
	if (!EntitySystemRunner)
	{
		...
		if (UE::UMG::GVarAnimationDetailedLogging)
		{
			UE_LOGF(LogUMG, Log, "Animation: Ticking '%ls' on '%ls' (%p) aborted, shared playback state has no runner.",
					*GetNameSafe(Animation), *GetNameSafe(UserWidget), UserWidget);
		}
#endif
		return;
	...
	if (PreviousPlaybackStatus == EMovieScenePlayerStatus::Playing)
	{
		...
		for (int32 Index = 0; Index < TickContexts.Num(); ++Index)
		{
			...
			if (UE::UMG::GVarAnimationDetailedLogging)
			{
				UE_LOGF(LogUMG, Log, "Animation: Ticking '%ls' on '%ls' (%p), Time=%ls, Status=%ls, Direction=%ls (update %d)",
						*GetNameSafe(Animation), *GetNameSafe(UserWidget), UserWidget,
		...
		if (UE::UMG::GVarAnimationDetailedLogging)
		{
			UE_LOGF(LogUMG, Log, "Animation: Ticking done '%ls' on '%ls' (%p), NumLoopsCompleted=%d, PlaybackSpeed=%f",
					*GetNameSafe(Animation), *GetNameSafe(UserWidget), UserWidget,
	...
	else
	{
		...
		if (UE::UMG::GVarAnimationDetailedLogging)
		{
			UE_LOGF(LogUMG, Log, "Animation: Ticking '%ls' on '%ls' (%p) skipped, state not playing.",
					*GetNameSafe(Animation), *GetNameSafe(UserWidget), UserWidget);
		}
#endif
	}
...
void FWidgetAnimationState::OnBegunPlay()
{
	...
	{
		UUserWidget* UserWidget = WeakUserWidget.Get();
		UE_LOGF(LogUMG, Log, "Animation: OnBegunPlay '%ls' on '%ls' (%p)",
				*GetNameSafe(Animation), *GetNameSafe(UserWidget), UserWidget);
	}
#endif
}
...
void FWidgetAnimationState::OnStopped()
{
	...
	if (UE::UMG::GVarAnimationDetailedLogging)
	{
		...
		UUserWidget* UserWidget = WeakUserWidget.Get();
		UE_LOGF(LogUMG, Log, "Animation: OnStopped '%ls' on '%ls' (%p) %ls%ls",
				*GetNameSafe(Animation), *GetNameSafe(UserWidget), UserWidget,
...
void FWidgetAnimationState::Play(const FWidgetAnimationStatePlayParams& PlayParams)
{
	...
	if (UE::UMG::GVarAnimationDetailedLogging)
	{
		UE_LOGF(LogUMG, Log, "Animation: Play '%ls' on '%ls' (%p), StartTime=%ls, NumLoopsToPlay=%d, PlaybackSpeed=%f, PlayMode=%ls%ls",
				*GetNameSafe(Animation), *GetNameSafe(UserWidget), UserWidget,
...
void FWidgetAnimationState::Stop()
{
	...
	{
		UUserWidget* UserWidget = WeakUserWidget.Get();
		UE_LOGF(LogUMG, Log, "Animation: Stop '%ls' on '%ls' (%p)",
				*GetNameSafe(Animation), *GetNameSafe(UserWidget), UserWidget);
	}
#endif
...
void FWidgetAnimationState::Pause()
{
	...
	{
		UUserWidget* UserWidget = WeakUserWidget.Get();
		UE_LOGF(LogUMG, Log, "Animation: Pause '%ls' on '%ls' (%p), PauseTime=%ls",
				*GetNameSafe(Animation), *GetNameSafe(UserWidget), UserWidget,
				*LexToString(PlaybackManager.GetCurrentTime()));
	}
#endif
...
void FWidgetAnimationState::RemoveEvaluationData()
{
	...
	{
		UUserWidget* UserWidget = WeakUserWidget.Get();
		UE_LOGF(LogUMG, Log, "Animation: RemoveEvaluationData '%ls' on '%ls' (%p)",
				*GetNameSafe(Animation), *GetNameSafe(UserWidget), UserWidget);
	}
#endif
}
...
void FWidgetAnimationState::TearDown()
{
	...
	{
		UUserWidget* UserWidget = WeakUserWidget.Get();
		UE_LOGF(LogUMG, Log, "Animation: TearDown '%ls' on '%ls' (%p)",
				*GetNameSafe(Animation), *GetNameSafe(UserWidget), UserWidget);
	}
#endif
}
