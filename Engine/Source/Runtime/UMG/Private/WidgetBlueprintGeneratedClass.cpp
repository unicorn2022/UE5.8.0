...
FAutoConsoleCommand GDumpTemplateSizesCommand(
	...
	FConsoleCommandDelegate::CreateStatic([]()
	{
		...
		UE_LOGF(LogUMG, Display, "%-60ls %-15ls", TEXT("Template Class"), TEXT("Size (bytes)"));
		for (const FClassAndSize& Entry : TemplateSizes)
		{
			TotalSizeBytes += Entry.TemplateSize;
			if (Entry.TemplateSize > 0)
			{
				UE_LOGF(LogUMG, Display, "%-60ls %-15d", *Entry.ClassName, Entry.TemplateSize);
			}
			else
			{
				UE_LOGF(LogUMG, Display, "%-60ls %-15ls", *Entry.ClassName, TEXT("0 - (No Template)"));
			}
		}

		UE_LOGF(LogUMG, Display, "Total size of templates %.3f MB", TotalSizeBytes/(1024.f*1024.f));
	}), ECVF_Cheat);
...
void UWidgetBlueprintGeneratedClass::PostLoad()
{
	...
	{
		static const FName Visiblity("Visiblity");
		static const FName Visibility("Visibility");
