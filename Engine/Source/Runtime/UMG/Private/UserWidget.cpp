...
{
	// If the widget is invalid or not contained within our known widget tree, return false
	if (!Widget)
	{
		return false;
	}
	...
	// Otherwise, we should still remember the name to allow to resolve later against a valid tree.
	return SetDesiredFocusWidget(Widget->GetFName());
}
...
UWidget* UUserWidget::GetDesiredFocusWidget() const
{
	if (WidgetTree)
	{
		// Ensure that we are always resolving against the widget tree
		return const_cast<FWidgetChild&>(DesiredFocusWidget).Resolve(WidgetTree);
	}
	return nullptr;
}
...
void UUserWidget::CreateInputComponent()
{
	...
	// Just an editor-time sanity check for this behavior.
	// bAutomaticallyRegisterInputOnConstruction should be true if this widget or any of its
	// BP parent classes have input nodes in their graphs which we want to set up
	// automatic bindings for.
#if WITH_EDITOR
	if (!HasAnyFlags(RF_ClassDefaultObject))
	{
		const UUserWidget* DefaultWidget = CastChecked<UUserWidget>(GetClass()->GetDefaultObject());
		ensure(bAutomaticallyRegisterInputOnConstruction == DefaultWidget->bAutomaticallyRegisterInputOnConstruction);
	}
...
FReply UUserWidget::NativeOnFocusReceived( const FGeometry& InGeometry, const FFocusEvent& InFocusEvent )
{
	...
	if (UWidget* WidgetToFocus = GetDesiredFocusWidget())
	{
		if (TSharedPtr<SWidget> SlateWidget = WidgetToFocus->GetCachedWidget())
		{
			FWidgetPath Path;
			// Ensure that the widget is actually hosted/ the Widget tree is not null
			if (FSlateApplication::Get().GeneratePathToWidgetUnchecked(SlateWidget.ToSharedRef(), Path) && Path.IsValid() && Path.ContainsWidget(GetCachedWidget().Get()))
			{
				// Execute full slate focus tunneling using the path
				FSlateApplication::Get().SetUserFocus(InFocusEvent.GetUser(), Path, InFocusEvent.GetCause());
...
UUserWidget* UUserWidget::CreateInstanceInternal(UObject* Outer, TSubclassOf<UUserWidget> UserWidgetClass, FName InstanceName, UWorld* World, ULocalPlayer* LocalPlayer)
{
	...
	if (!UserWidgetClass)
	{
		UE_LOGF(LogUMG, Error, "CreateWidget called with a null class.");
		return nullptr;
	}
#endif
...
UUserWidgetExtension* UUserWidget::GetOrAddExtension(TSubclassOf<UUserWidgetExtension> InExtensionType)
{
	if (UUserWidgetExtension* ExistingExtension = GetExtension(InExtensionType))
	{
		return ExistingExtension;
	}
	return AddExtension(InExtensionType);
}
