...
static void StaticMeshToSlateRenderData(const UStaticMesh& DataSource, TArray<FSlateMeshVertex>& OutSlateVerts, TArray<uint32>& OutIndexes, FVector2D& OutExtentMin, FVector2D& OutExtentMax )
{
	...
	if (NumSections > 1)
	{
		UE_LOGF(LogUMG, Warning, "StaticMesh %ls has %d sections. SMeshWidget expects a static mesh with 1 section.", *DataSource.GetName(), NumSections);
	}
	else
	{
		// Populate Vertex Data
		{
			...
			{
				UE_LOGF(LogStaticMesh, Warning, "[%ls] has %d UV sets; slate vertex data supports at most %d", *DataSource.GetName(), TexCoordsPerVertex, MAX_SUPPORTED_UV_SETS);
			}
