...
bool GSlateEnableRetainedRenderingHittestRemap = true;
FAutoConsoleVariableRef CVarGlateEnableRetainedRenderingHittestRemap(
	TEXT("Slate.EnableRetainedRenderingHittestRemap"),
	GSlateEnableRetainedRenderingHittestRemap,
	TEXT("If enables, the rotation delta of the SRetainer widget will be applied to the hittest grid. Used to resolve an issue with layout and paint space deviations in retained widgets")
);
...
SRetainerWidget::EPaintRetainedContentResult SRetainerWidget::PaintRetainedContentImpl(const FSlateInvalidationContext& Context, const FGeometry& AllottedGeometry, int32 LayerId)
{
	...
	if (bRenderRequested)
	{
		// SRetainerWidget defines a relationship between a UWorld and a Slate hierarchy (VirtualWindow and its children).  Therefore it is
		// responsible for pushing its world's scene onto the stack, so that any child widgets that use Material Parameter Collection will resolve to
		// the MPC instance associated with that scene.
		UE::Slate::FSceneRegistrationScope SceneScope;
		UWorld* TickWorld = OuterWorld.Get();
		if (TickWorld && TickWorld->Scene && IsInGameThread())
		{
			SceneScope = FSlateApplication::Get().GetRenderer()->RegisterCurrentScene(TickWorld->Scene);
		}
		...
		if (bTextureIsTooLarge || bTextureSizeZero)
		{
			...
			if (!bInvalidSizeLogged)
			{
				...
				if (bTextureIsTooLarge)
				{
					...
					{
						UE_LOGF(LogUMG, Warning, "The requested size for SRetainerWidget is too large. W:%i H:%i", RenderTargetWidth, RenderTargetHeight);
					}
				}
				else
				{
					if (bEnableWarnOnInvalidSize)
					{
						UE_LOGF(LogUMG, Warning, "The requested size for SRetainerWidget is 0. W:%i H:%i", RenderTargetWidth, RenderTargetHeight);
					}
				}
			}
...
int32 SRetainerWidget::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	...
	if (bShouldRetainRendering && IsAnythingVisibleToRender())
	{
		...
		{
			// Nothing was drawn, pass the caller's layer through unchanged.
			return LayerId;
		}
		else
		{
...
int32 SRetainerWidget::PaintSlowPath(const FSlateInvalidationContext& Context)
{
	...
	if (GSlateEnableRetainedRenderingHittestRemap)
	{
		// Get the delta from simplified render transform to the original (should contain just the missing rotation about retainer pivot)
		const FSlateRenderTransform NewToOriginalPaintSpace =
			Concatenate(
				Inverse(NewPaintSpaceGeometry.GetAccumulatedRenderTransform()),
				OriginalPaintSpaceGeometry.GetAccumulatedRenderTransform());

		HittestGrid->SetPaintToHitTestTransform(NewToOriginalPaintSpace);
	}
	else
	{
		HittestGrid->SetPaintToHitTestTransform(FSlateRenderTransform());
	}
