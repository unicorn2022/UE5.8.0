	...
	if (RenderRuns.Num() > 0)
	{
		...
		for (int RunIndex = 0; RunIndex < RenderRuns.Num(); ++RunIndex)
		{
			...
			else
			{
				if( !GUsingNullRHI )
				{
					UE_LOGF(LogUMG, Warning, "SMeshWidget did not render a run because of one of these Brush: %ls, InstanceBuffer: %ls, NumVertexes: %d, NumIndexes: %d",
	...
	else
	{
		...
		for (int i = 0; i < RenderData.Num(); ++i)
		{
			...
			else
			{
				if( !GUsingNullRHI )
				{
					UE_LOGF(LogUMG, Warning, "SMeshWidget did not render a run because of one of these Brush: %ls, NumVertexes: %d, NumIndexes: %d",
						RunRenderData.RenderingResourceHandle.IsValid() ? TEXT("valid") : TEXT("nullptr"),
						RunRenderData.VertexData.Num(),
						RunRenderData.IndexData.Num());
				}
