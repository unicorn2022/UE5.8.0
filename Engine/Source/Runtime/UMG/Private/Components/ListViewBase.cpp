...
ESlateVisibility UListViewBase::GetScrollbarVisibility() const
{
	if (MyTableViewBase)
	{
		return UWidget::ConvertRuntimeToSerializedVisibility(MyTableViewBase->GetScrollbarVisibility());
	}
	// Returning collapsed if TableView is not available, as the scrollbar does not occupy space in the ui layout  
...
UMG_API void UListViewBase::SetIsTouchScrollingEnabled(bool bInIsTouchScrollingEnabled)
{
	bEnableTouchScrolling = bInIsTouchScrollingEnabled;
	if (MyTableViewBase)
	{
		MyTableViewBase->SetIsTouchScrollingEnabled(bInIsTouchScrollingEnabled);
	}
}
