...
UMG_API void UScrollBox::SlateHandleFocusChanging(const FWeakWidgetPath& PreviousFocusPath, const FWidgetPath& NewWidgetPath)
{
	if (const SWidget* SlateWidget = this->GetCachedWidget().Get())
	{
		const bool bNewFocusIsInScrollBox = NewWidgetPath.ContainsWidget(SlateWidget);
		const bool bOldFocusIsInScrollBox = PreviousFocusPath.ContainsWidget(SlateWidget);
		if (!bNewFocusIsInScrollBox || !bOldFocusIsInScrollBox)
		{
			OnFocusUpdated.Broadcast(this, !bOldFocusIsInScrollBox);
		}
	}
}
