...
TSharedRef<SWidget> UEditableTextBox::RebuildWidget()
{
	MyEditableTextBlock = SNew(SEditableTextBox)
		...
		.OnCursorMovedWithSelection(BIND_UOBJECT_DELEGATE(FOnCursorMovedWithSelection, HandleOnCursorMovedWithSelection))
		...
		.OnAllFontFacesFinishLoading(OnAllFontFacesFinishLoading.IsBound() ? BIND_UOBJECT_DELEGATE(FSimpleDelegate, SlateHandleAllFontFacesFinishLoading) : FSimpleDelegate());
...
void UEditableTextBox::SynchronizeProperties()
{
	...
	MyEditableTextBlock->SetFontFacesLoadingPaintPolicy(FontFacesLoadingPaintPolicy);
...
bool UEditableTextBox::IsAnyTextSelected() const
{
	if ( MyEditableTextBlock.IsValid() )
	{
		return MyEditableTextBlock->AnyTextSelected();
	}
...
void UEditableTextBox::HandleOnCursorMovedWithSelection(const FTextLocation& InLocation, const FTextSelection& Selection)
{
	OnCursorMovedWithSelectionEvent.Broadcast(InLocation, Selection);
}
...
void UEditableTextBox::SetFontFacesLoadingPaintPolicy(EFontFacesLoadingPaintPolicy InFontFacesLoadingPaintPolicy)
{
	FontFacesLoadingPaintPolicy = InFontFacesLoadingPaintPolicy;

	if (MyEditableTextBlock)
	{
		MyEditableTextBlock->SetFontFacesLoadingPaintPolicy(InFontFacesLoadingPaintPolicy);
	}
}
/////////////////////////////////////////////////////
