...
void FUMGViewportClient::Draw(FViewport* InViewport, FCanvas* Canvas)
{
	...
	CachedViewProjectionMatrix = View->ViewMatrices.GetWorldToClip();
...
class SAutoRefreshViewport : public SViewport
{
	SLATE_BEGIN_ARGS( SAutoRefreshViewport )
		: _bIsEditorPreview(false)
	{
	}
	SLATE_ARGUMENT( bool, bIsEditorPreview )
	SLATE_END_ARGS()
	
	SAutoRefreshViewport() = default;
	...
	void Construct(const FArguments& InArgs)
	{
		...
		bIsEditorPreview = InArgs._bIsEditorPreview;
		
		PreviewScene = MakeUnique<FPreviewScene>(FPreviewScene::ConstructionValues().SetEditor(bIsEditorPreview));
		

		ViewportClient = MakeShareable(new FUMGViewportClient(PreviewScene.Get()));
...
TSharedRef<SWidget> UViewport::RebuildWidget()
{
	...
	else
	{
		ViewportWidget = SNew(SAutoRefreshViewport)
			.bIsEditorPreview(bIsEditorPreview);
...
UWorld* UViewport::GetViewportWorld() const
{
	if ( ViewportWidget.IsValid() )
	{
		return ViewportWidget->PreviewScene.Get()->GetWorld();
	}
...
{
	ViewportWidget->PreviewScene.Get()->SetLightBrightness(InLightIntensity);
}

void UViewport::SetSkyIntensity(float InLightIntensity)
{
	ViewportWidget->PreviewScene.Get()->SetSkyBrightness(InLightIntensity);
}
