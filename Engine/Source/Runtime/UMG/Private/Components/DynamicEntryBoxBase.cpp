...
void UDynamicEntryBoxBase::SetRadialSettings(const FRadialBoxSettings& InSettings)
{
	...
	if (MyPanelWidget.IsValid() && EntryBoxType == EDynamicBoxType::Radial)
	{
		...
		StaticCastSharedPtr<SRadialBox>(MyPanelWidget)->SetClockwiseOrder(InSettings.bClockwiseOrder);
		...
		StaticCastSharedPtr<SRadialBox>(MyPanelWidget)->SetMarginSize(InSettings.MarginSize);
	}
}
...
TSharedRef<SWidget> UDynamicEntryBoxBase::RebuildWidget()
{
	...
	case EDynamicBoxType::Radial:
		EntryBoxWidget = SAssignNew(MyPanelWidget, SRadialBox)
			...
			.MarginSize(RadialBoxSettings.MarginSize)
...
UUserWidget* UDynamicEntryBoxBase::CreateEntryInternal(TSubclassOf<UUserWidget> InEntryClass)
{
	...
	{
		UE_LOGF(LogSlate, Error, "'%ls' cannot be added to DynamicEntry '%ls' because it is already a child and it would create a recurssion."
		...
		{
			UE_LOGF(LogSlate, Log, "%ls", *RecursiveItem->GetName());
		}
#endif
		return nullptr;
	}
