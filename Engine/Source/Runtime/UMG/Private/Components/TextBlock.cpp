...
void UTextBlock::SetFontSize(float DisplayFontSize)
{
	// Convert from your project FontResolution setting DPI to the native default render DPI
	const UUserInterfaceSettings* UISettings = GetDefault<UUserInterfaceSettings>();
	if (UISettings)
	{

		Font.Size = UISettings->ConvertFontSizeFromDisplayToNative(DisplayFontSize);
		SetFont(Font);
	}
}

UMG_API float UTextBlock::GetFontSize() const
{
	const UUserInterfaceSettings* UISettings = GetDefault<UUserInterfaceSettings>();
	if (UISettings)
	{
		return UISettings->ConvertFontSizeFromNativeToDisplay(Font.Size);
	}
	return 0.0f;
}
...
void UTextBlock::OnBindingChanged(const FName& Property)
{
	...
	if ( MyTextBlock.IsValid() )
	{
		static const FName TextProperty("TextDelegate");
		static const FName ColorAndOpacityProperty("ColorAndOpacityDelegate");
		static const FName ShadowColorAndOpacityProperty("ShadowColorAndOpacityDelegate");
