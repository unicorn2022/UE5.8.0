...
void UListView::AddItemAt(UObject* Item, int32 Index)
{
	if (Item == nullptr)
	{
		FFrame::KismetExecutionMessage(TEXT("Cannot add null item into ListView."), ELogVerbosity::Warning, "NullListViewItem");
	...
	// Safety clamp the provided index to a valid range
	Index = FMath::Clamp(Index, 0, ListItems.Num());
...
void UListView::AddItems(const TArray<UObject*>& Items)
{
	ListItems.Reserve(ListItems.Num() + Items.Num());
...
void UListView::AddItemsAt(const TArray<UObject*>& Items, int32 Index)
{
	ListItems.Reserve(ListItems.Num() + Items.Num());
	...
	for (UObject* Item : Items)
	{
		if (Item != nullptr)
		{
			ListItems.Insert(Item, Index + Offset);
			++Offset;
		}
	}
...
void UListView::RemoveItems(const TArray<UObject*>& Items)
{
	for (UObject* Item : Items)
	{
		if (Item)
...
void UListView::SetEnableProximateEntryNavigation(bool bNewValue)
{
	bEnableProximateEntryNavigation = bNewValue;
	if (MyListView.IsValid())
	{
		MyListView->SetEnableProximateEntryNavigation(bNewValue);
	}
}
...
void UListView::OnItemsChanged(const TArray<UObject*>& AddedItems, const TArray<UObject*>& RemovedItems)
{
	...
	for (UObject* AddedItem : AddedItems)
	{
		...
		else if (AActor* AddedItemOuterActor = AddedItem ? AddedItem->GetTypedOuter<AActor>() : nullptr)
		{
...
void UListView::OnListViewTouchStartInternal()
{
	ITypedUMGListView::OnListViewTouchStartInternal();
	BP_OnListViewTouchStart.Broadcast();
}

void UListView::OnListViewTouchMoveInternal()
{
	ITypedUMGListView::OnListViewTouchMoveInternal();
	BP_OnListViewTouchMove.Broadcast();
}

void UListView::OnListViewTouchEndInternal()
{
	ITypedUMGListView::OnListViewTouchEndInternal();
	BP_OnListViewTouchEnd.Broadcast();
}
...
void UListView::SetReturnFocusToSelection(bool bEnabled)
{
	bReturnFocusToSelection = bEnabled;
	if (MyListView.IsValid())
	{
		MyListView->SetReturnFocusToSelection(bEnabled);
	}
}
