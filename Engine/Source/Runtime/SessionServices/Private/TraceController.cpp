...
void FTraceController::OnDiscoveryResponse(const FTraceControlDiscovery& Message, const TSharedRef<IMessageContext>& Context)
{
	...
	if (!Instance)
	{
		...
		Instance = &Instances.Emplace(Context->GetSender(), {Bus.ToSharedRef(), Context->GetSender(), this});
	...
	Instance->LastMessageTimestamp = FDateTime::Now();
...
void FTraceController::OnSettings(const FTraceControlSettings& Message, const TSharedRef<IMessageContext>& Context)
{
	...
	if (const auto Instance = Instances.Find(Context->GetSender()))
	{
		...
		Algo::Transform(Message.Capabilities, Settings.Capabilities, [](const FTraceCapability& In) { 
				return FTraceStatus::FCapability { In.Name }; 
		});
...
void FTraceController::OnSecureResponse(const FTraceControlSecureResponse& Message, const TSharedRef<IMessageContext>& Context)
{
	FWriteScopeLock _(InstancesLock);
	
	if (const auto Instance = Instances.Find(Context->GetSender()))
	{
		FSecureConnection Connection;
		Connection.Host = Message.Host;
		Connection.Port = Message.Port;
		Connection.AuthorizationToken = Message.AuthorizationToken;
		if (Message.Mode == ETraceSecureMode::SelfSignedCertAndAuthToken)
		{
			Connection.CertificateAuthority = Message.CertificateAuthority;
		}
		else if (Message.Mode == ETraceSecureMode::AuthToken)
		{
			Connection.Identity = Message.Identity;
		}
		SecureResponse.Broadcast(Instance->Status, Connection);
	}
}
...
bool FTraceController::IsActive(FGuid InstanceId, double TimeoutSec) const
{
	if (const auto& InstanceAddress = InstanceToAddress.Find(InstanceId))
	{
		if (auto* Instance = Instances.Find(*InstanceAddress))
		{
			return (FDateTime::Now() - Instance->LastMessageTimestamp).GetTotalSeconds() <= TimeoutSec;
		}
	}
...
bool FTraceController::HasCapability(FGuid InstanceId, FStringView Name) 
{
	FReadScopeLock _(InstancesLock);
	if (const auto& InstanceAddress = InstanceToAddress.Find(InstanceId))
	{
		if (auto* Instance = Instances.Find(*InstanceAddress))
		{
			for (const auto& Capability : Instance->Status.Settings.Capabilities)
			{
				if (Name.Equals(Capability.Name, ESearchCase::IgnoreCase))
				{
					return true;
				}
			}
...
FTraceController::FTracingInstance::FTracingInstance(const TSharedRef<IMessageBus>& InMessageBus, FMessageAddress InService, FTraceController* Controller)
	: Status()
	, Commands(InMessageBus, InService, Controller)
{
}
