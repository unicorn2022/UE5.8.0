...
struct FRpcConfig
{
	...
	// it is valid to have mappings, but no active ports, in the case of a client with multiple endpoint hosts
	TArray<int> ActiveHostPorts;
};

/**
 * Context struct used to measure execution time of RPCs
 * Execution time can be returned in the response to measure actual runtime of the RPC minus the network traffic
*/
USTRUCT()
...
public:
	...
	double StopTiming()
	{
		ExecutionTimeMs = (FDateTime::Now() - StartTime).GetTotalMilliseconds();
...
protected:
	...
	TMap<FName, FName> MultiPortRouteMapping;
	TArray<int> PortsToUse;
...
public:
	...
	int GetPortToUse()
	{
		return PortsToUse.Num() ? PortsToUse[0] : 0;
	}
