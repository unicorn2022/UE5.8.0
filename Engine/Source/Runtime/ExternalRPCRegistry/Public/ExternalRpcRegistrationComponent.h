...
public:
	...
	UE_API UExternalRpcRegistrationComponent();

	/**
	 * Deserializes the incoming request body into a JSON object
	 * 
	 * @param  InRequestBody  Incoming array of bytes from the request
	 * 
	 * @return Valid JSON object on a successful deserialization; otherwise returns a nullptr
	 */
	UE_API TSharedPtr<FJsonObject> GetJsonObjectFromRequestBody(const TArray<uint8>& InRequestBody);
...
protected:
	...
	/**
	 * Appends timing context information to a response 
	 * 
	 * @param  InResponse		The response to append execution time to
	 * @param  InTimingContext  Handle to the timing context that was used to measure the execution time
	 */
	UE_API void AppendTimingContextToResponse(TUniquePtr<FHttpServerResponse>& InResponse, FRpcTimingContext& InTimingContext);
};
