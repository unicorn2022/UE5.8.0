...
static const FString SaveReplayExt(".sav_rep");
...
namespace SaveGameReplay
{
	...
	public:
		...
		void StartTask(const FStreamer& OwningStreamer, const FString& Description, typename TAsyncTypes<TResult>::TAsyncFunc InAsyncWork, typename TAsyncTypes<TResult>::TPostAsyncFunc InPostAsyncWork)
		{
			if (ensureMsgf(IsInGameThread(), TEXT("SaveGameReplay::FAsyncTaskManager::StartTask - Called from outside the GameThread.")))
			{
				...
				{
					UE_LOGF(LogSaveGameReplay, Warning, "SaveGameReplay::FAsyncTaskManager::StartTask - New task attempted while processing pending task (NewTask = %ls PendingTask = %ls)", *Description, *(OutstandingTask->GetDescription()));
				...
				else
				{
					...
					{
						UE_LOGF(LogSaveGameReplay, Warning, "SaveGameReplay::FAsyncTaskManager::StartTask - Async work function was invalid (NewTask = %ls)", *Description);
	...
	private:
		...
		static void RunCommand(const TArray<FString>& Params, const TFunction<void(const TSharedPtr<INetworkReplayStreamer>&, const FString&, const int32)> CommandToRun)
		{
			...
			{
				UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameReplayMoveFileHelper commands are client only.");
				return;
			}
			...
			{
				UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameReplayMoveFileHelper commands take either a Streamer Override or no arguments.");
				return;
			}
		...
		static void SanitizeNames(const FString& DemoPath)
		{
			...
			{
				UE_LOGF(LogSaveGameReplay, Log, "FSaveGameReplayMoveFileHelper::SanitizeNames - Handling %ls", *CurrentName);
				...
				{
					UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameReplayMoveFileHelper::SanitizeNames - Failed to sanitize %ls", *CurrentName);
				}
			}
		}
		...
		static void CopyFile(const TSharedPtr<FMoveContext>& Context, const FString& BaseFileName)
		{
			...
			if (0 != Result)
			{
				UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameReplayMoveFileHelper::CopyFile: Failed - from '%ls' to '%ls' error = %lu", *SourceFileName, *DestinationFileName, Result);
			}

			UE_LOGF(LogSaveGameReplay, Log, "FSaveGameReplayMoveFileHelper::CopyFile - from '%ls' to '%ls': Result = %d", *SourceFileName, *DestinationFileName, Result);
		}
		...
		static void SaveFile(const TSharedPtr<FMoveContext>& Context, const FString& SaveGameName, ISaveGameSystem* SaveGameSystem)
		{
			...
			{
				UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameReplayMoveFileHelper::SaveFile: Failed to load save game %ls", *SaveGameName)
					return;
			}
			...
			if (FileAR->IsError())
			{
				UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameReplayMoveFileHelper::SaveFile: Failed to save game %ls to %ls", *SaveGameName, *DestinationFileName);
			}
		}
		...
		static void OnEnumerateStreamsComplete(const FEnumerateStreamsResult& Result, const TSharedPtr<FMoveContext> Context)
		{
			...
			UE_LOGF(LogSaveGameReplay, Log, "FSaveGameReplayMoveFileHelper::OnEnumerateStreamsComplete: Success=%ls NumFiles=%d", *(Result.WasSuccessful() ? CoreTexts.True : CoreTexts.False).ToString(), Result.FoundStreams.Num());
			...
			if (Result.WasSuccessful())
			{
				if (Result.FoundStreams.Num() > 0)
				{
					if (IsSaveGameFileName(Result.FoundStreams[0].Name))
					{
						...
						{
							UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameReplayMoveFileHelper::OnEnumerateRecentStreamsComplete: Unable to get SaveGameSystem");
						}
					}
					else
					{
						Async(EAsyncExecution::TaskGraph,
							...
							[Context]()
							{
								UE_LOGF(LogSaveGameReplay, Log, "FSaveGameReplayMoveFileHelper::OnEnumerateStreamsComplete: Async copy complete.");
			...
			{
				UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameReplayMoveFileHelper::OnEnumerateRecentStreamsComplete: Enumerate failed");
			}
		...
		static void OnEnumerateRecentStreamsComplete(const FEnumerateStreamsResult& Result, const TSharedPtr<FMoveContext> Context)
		{
			if (Result.WasSuccessful())
			{
				Async(EAsyncExecution::TaskGraph, 
					...
					[]()
					{
						UE_LOGF(LogSaveGameReplay, Log, "FSaveGameReplayMoveFileHelper::OnEnumerateRecentStreamsComplete: Async copy complete.")
					});
			}
			else
			{
		...
		static void MoveFilesLocalInternal(const TSharedPtr<FMoveContext>& MoveContext)
		{
			...
			for (FString& ReplayFileName : ReplayFiles)
			{
				...
				{
					UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameReplayMoveFileHelper::MoveFilesLocalInternal - Replay file %ls is not lowercase, import will fail.", *ReplayFileName);
				}
#endif
	...
	public:
		...
		{
			UE_LOGF(LogSaveGameReplay, Log, "FSaveGameReplayMoveFileHelper::MoveFiles: Moving files from %ls to %ls", *FPaths::ConvertRelativePathToFull(SourceDirectory), *FPaths::ConvertRelativePathToFull(DestinationDirectory));
		...
		static void MoveFilesFromTemp(const FString& DestinationDirectory, const int32 UserIndex)
		{
			...
			Async(EAsyncExecution::TaskGraph,
				...
				[]()
				{
					UE_LOGF(LogSaveGameReplay, Log, "FSaveGameReplayMoveFileHelper::MoveFilesFromTemp: Async copy complete.")
				});
		}
...
void FSaveGameNetworkReplayStreamer::StartStreaming(const FStartStreamingParameters& Params, const FStartStreamingCallback& Delegate)
{
	...
	{
		UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::StartStreaming - Invalid UserString");
...
void FSaveGameNetworkReplayStreamer::StartStreaming_Internal(const FStartStreamingParameters& Params, FStartStreamingResult& Result)
{
	...
	if (Params.bRecord)
	{
		UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::StartStreaming: Cannot record directly to a save game, use KeepReplay() instead.");
		return;
	}
	...
	if (SaveExistsResult != ESaveExistsResult::OK)
	{
		...
		UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::StartStreaming: Replay does not exist or is invalid.");
		return;
	}
	...
	{
		UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::StartStreaming: Failed to load replay data.");
		return;
	}
	...
	{
		UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::StartStreaming: Failed to copy replay to local file.");
		return;
	}
...
void FSaveGameNetworkReplayStreamer::DeleteFinishedStream(const FString& ReplayName, const FDeleteFinishedStreamCallback& Delegate)
{
	...
	{
		UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::DeleteFinishedStream - Invalid UserIndex.");
...
void FSaveGameNetworkReplayStreamer::DeleteFinishedStream_Internal(const FString& InReplayName, const int32 UserIndex, FDeleteFinishedStreamResult& Result) const
{
	...
	{
		UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::DeleteFinishedStream: Invalid Replay name %ls", *InReplayName);
	...
	{
		UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::DeleteFinishedStream: Unable to delete replay or metadata %ls (meta deleted = %d, replay deleted = %d)", *InReplayName, bDeletedMeta, bDeletedReplay);
		return;
	}
...
{
	UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::KeepReplay - Invalid UserIndex.");
...
void FSaveGameNetworkReplayStreamer::KeepReplay_Internal(const FString& ReplayName, const bool bKeep, const int32 UserIndex, FKeepReplayResult& Result)
{
	...
	if (IsSaveGameFileName(ReplayName))
	{
		...
		UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::KeepReplay: Requested to keep an already kept replay %ls", *ReplayName);
		return;
	}
	...
	{
		UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::KeepReplay: Requested replay does not exist %ls", *ReplayName);
		Result.Result = EStreamingOperationResult::ReplayNotFound;
		return;
	}
	...
	for (; SaveSlot < MaxNumReplays; ++SaveSlot)
	{
		...
		{
			UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::KeepReplay: Mismatched save file statuses Index=%d ReplayStatus=%d MetaStatus=%d", SaveSlot, static_cast<int32>(ReplaySaveFileStatus), static_cast<int32>(MetaSaveFileStatus));
			continue;
		}
	...
	{
		UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::KeepReplay: No available save slots remain %ls", *ReplayName);
			...
			if (ReadMetaDataFromLocalStream(MetaDataReader, MetaData))
			{
				...
				if (SerializeMetaData(MetaDataWriter, MetaData))
				{
					...
					if (SaveGameSystem->SaveGame(/*bAttemptToUseUI=*/false, *ReplaySaveFileName, UserIndex, ReplayData))
					{
						...
						{
							UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::KeepReplay: Failed to save replay meta data to slot %ls", *ReplayName);
						}
					}
					else
					{
						UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::KeepReplay: Failed to save replay to slot %ls", *ReplayName);
					}
				}
				else
				{
					UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::KeepReplay: Unable to generate meta data %ls", *ReplayName);
				}
			}
			else
			{
				UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::KeepReplay: Unable to grab meta data from replay %ls", *ReplayName);
			}
		}
		else
		{
			UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::KeepReplay: Unable to read StreamInfo");
		}
	}
}
...
void FSaveGameNetworkReplayStreamer::RenameReplayFriendlyName(const FString& ReplayName, const FString& NewFriendlyName, const FRenameReplayCallback& Delegate)
{
	...
	{
		UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::RenameReplayFriendlyName - Invalid UserIndex.");
...
void FSaveGameNetworkReplayStreamer::RenameReplayFriendlyName_Internal(const FString& ReplayName, const FString& NewFriendlyName, const int32 UserIndex, FRenameReplayResult& Result) const
{
	...
	if (!SerializeMetaData(MetaDataWriter, MetaData))
	{
		UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::RenameReplayFriendlyNameSaved: Failed to write meta data %ls", *ReplayName);
		return;
	}
	...
	{
		UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::RenameReplayFriendlyNameSaved: Failed to save meta data %lss", *ReplayName);
		return;
	}
...
{
	UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::RenameReplay: Is currently unsupported");
...
void FSaveGameNetworkReplayStreamer::EnumerateStreams(const FNetworkReplayVersion& ReplayVersion, const int32 UserIndex, const FString& MetaString, const TArray< FString >& ExtraParms, const FEnumerateStreamsCallback& Delegate)
{
	...
	if (UserIndex == INDEX_NONE)
	{
		UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::EnumerateStreams - Invalid UserIndex.");
...
void FSaveGameNetworkReplayStreamer::EnumerateStreams_Internal(const FNetworkReplayVersion& ReplayVersion, const int32 UserIndex, const FString& MetaString, const TArray< FString >& ExtraParms, FEnumerateStreamsResult& Result)
{
	...
	if (ensureMsgf(SaveGameSystem, TEXT("FSaveGameNetworkReplayStreamer::EnumerateStreams: Unable to retrieve save game systems")))
	{
		...
		for (int32 i = 0; i < MaxNumReplays; ++i)
		{
			...
			{
				UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::EnumerateStreams: Found corrupted stream Index=%d", i);
			...
			{
				UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::EnumerateStreams: Mismatched result Index=%d ReplayResult=%d MetaResult=%d", i, static_cast<int32>(SaveFileStatus.Result), static_cast<int32>(MetaFileStatus.Result));
				continue;
			}
			...
			{
				UE_LOGF(LogSaveGameReplay, Log, "FSaveGameNetworkReplayStreamer::EnumerateStreams: Error reading save files Index=%d Status=%d", i, static_cast<int32>(SaveFileStatus.Result));
				continue;
			}
...
void FSaveGameNetworkReplayStreamer::EnumerateEvents(const FString& ReplayName, const FString& Group, const FEnumerateEventsCallback& Delegate)
{
	...
	{
		UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::EnumerateEvents - Invalid UserIndex.");
...
void FSaveGameNetworkReplayStreamer::EnumerateEvents_Internal(const FString& ReplayName, const FString& Group, const int32 UserIndex, FEnumerateEventsResult& Result) const
{
	...
	{
		UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::EnumerateEvents: Passed in group conflicts with configured value. CVar=%ls | Group=%ls", *ConfigFilterGroup, *Group);
	}
...
void FSaveGameNetworkReplayStreamer::RequestEventData(const FString& EventID, const FRequestEventDataCallback& Delegate)
{
	...
	UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::RequestEventData: No replay name available, defaulting to FLocalFileStreamFArchive::RequestEventData");
...
void FSaveGameNetworkReplayStreamer::RequestEventData(const FString& ReplayName, const FString& EventID, const FRequestEventDataCallback& Delegate)
{
	...
	{
		UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::RequestEventData - Invalid UserIndex.");
...
{
	UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::RequestEventGroupData: No replay name available, defaulting to FLocalFileStreamFArchive::RequestEventData");
...
void FSaveGameNetworkReplayStreamer::RequestEventGroupData(const FString& ReplayName, const FString& Group, const FRequestEventGroupDataCallback& Delegate)
{
	...
	{
		UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::RequestEventGroupData - Invalid UserIndex.");
...
bool FSaveGameNetworkReplayStreamer::ReadMetaDataFromLocalStream(FArchive& StreamArchive, FSaveGameMetaData& OutMetaData) const
{
	...
	for (int32 i = 0; i < FileReplayInfo.Events.Num() && !StreamArchive.IsError(); ++i)
	{
		...
		if (LocalEvent.SizeInBytes > 0)
		{
			...
			{
				UE_LOGF(LogSaveGameReplay, Error, "ReadMetaDataFromLocalStream: Event SizeInBytes > MaxEventSize (%d / %d)", LocalEvent.SizeInBytes, SaveGameReplay::MaxEventSize);
				return false;
			}
			...
			if (FileReplayInfo.bEncrypted)
			{
				if (SupportsEncryption())
				{
					...
					{
						UE_LOGF(LogSaveGameReplay, Error, "FSaveGameNetworkReplayStreamer::ReadMetaDataFromLocalStream. DecryptBuffer failed.");
						return false;
					}
				...
				{
					UE_LOGF(LogSaveGameReplay, Error, "FSaveGameNetworkReplayStreamer::ReadMetaDataFromLocalStream. Encrypted event but streamer does not support encryption.");
					return false;
				}
			}
		}
...
bool FSaveGameNetworkReplayStreamer::ReadMetaDataFromSaveGame(class ISaveGameSystem& SaveGameSystem, const FSaveGameSanitizedNames& SanitizedNames, const int32 UserIndex, FSaveGameMetaData& OutMetaData, FStreamingResultBase& OutResult) const
{
	...
	if (ESaveExistsResult::OK != ReplayMetaSaveFileStatus)
	{
		...
		UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::ReadMetaDataFromSaveGame: Replay does not exist or is invalid.");
		return false;
	}
	...
	{
		UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::ReadMetaDataFromSaveGame: Failed to load replay.");
		return false;
	}
	...
	{
		UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::EnumerateEvents: Failed to read meta data.");
		return false;
	}
...
bool FSaveGameNetworkReplayStreamer::SerializeVersionedMetaData(FArchive& Archive, FSaveGameMetaData& MetaData) const
{
	...
	if (MetaData.VersionedInfo.FileVersion >= SaveGameReplay::HISTORY_EVENTS)
	{
		...
		{
			UE_LOGF(LogSaveGameReplay, Error, "SerializeVersionedMetaData: NumEvents > MaxEvents (%d / %d)", NumEvents, SaveGameReplay::MaxEvents);
			Archive.SetError();
			return false;
		}
		...
		for (int32 i = 0; i < NumEvents && !Archive.IsError(); ++i)
		{
			...
			if (EventDataSize > 0)
			{
				...
				{
					UE_LOGF(LogSaveGameReplay, Error, "SerializeVersionedMetaData: EventDataSize > MaxEventSize (%d / %d)", EventDataSize, SaveGameReplay::MaxEventSize);
					Archive.SetError();
					return false;
				}
...
bool FSaveGameNetworkReplayStreamer::StreamNameToSanitizedNames(const FString& StreamName, FSaveGameSanitizedNames& OutSanitizedNames) const
{
	...
	{
		UE_LOGF(LogSaveGameReplay, Warning, "FSaveGameNetworkReplayStreamer::StreamNameToSanitizedNames: Failed to parse replay index from name %ls", *StreamName);
		return false;
	}
...
const FString& FSaveGameNetworkReplayStreamer::GetTempDemoRelativeSavePath()
{
	...
	static const FString TempDemoSavePath("Temp/");
	return TempDemoSavePath;
}

const FString& FSaveGameNetworkReplayStreamer::GetDefaultPlaybackName()
{
	static const FString DefaultPlaybackName("Playback");
...
TSharedPtr<INetworkReplayStreamer> FSaveGameNetworkReplayStreamingFactory::CreateReplayStreamer()
{
	...
	{
		UE_LOGF(LogSaveGameReplay, Error, "FSaveGameNetworkReplayStreamingFactory: Unable to get SaveGameSystem.");
		return nullptr;
	}
