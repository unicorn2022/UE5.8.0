...
namespace UE::Net::LocalFileReplay
{
	...
	TAutoConsoleVariable<bool> CVarbUsePlayerAccountIdPostfix(
		TEXT("localReplay.UsePlayerAccountIdPostfix"),
#if UE_BUILD_SHIPPING
		false,
#else
		true,
#endif
		TEXT("If true, the local replay filenames have the local player's accountId as a postfix. Useful to spectate multiple clients in the same device."),
		ECVF_Cheat
	);
};
...
bool FLocalFileNetworkReplayStreamer::ReadReplayInfo(const FString& StreamName, FLocalFileReplayInfo& Info, EReadReplayInfoFlags Flags) const
{
	...
	TSharedPtr<FArchive> LocalFileReader = CreateLocalFileReader(GetDemoFullFilename(StreamName));
	if (LocalFileReader.IsValid())
	{
		return ReadReplayInfo(*LocalFileReader.Get(), Info, Flags);
	}
...
bool FLocalFileNetworkReplayStreamer::ReadReplayInfo(FArchive& Archive, FLocalFileReplayInfo& Info, FLocalFileSerializationInfo& SerializationInfo, EReadReplayInfoFlags Flags) const
{
	...
	if (Archive.TotalSize() != 0)
	{
		...
		if (MagicNumber == FLocalFileNetworkReplayStreamer::FileMagic)
		{
			...
			if (DoNotUse_FileVersion >= FLocalFileReplayCustomVersion::CustomVersions)
			{
				...
				for (const FCustomVersionDifference& Diff : VersionDiffs)
				{
					...
					{
						UE_LOGF(LogLocalFileReplay, Error, "Replay was saved with a custom version that is not present. Tag %ls Version %d", *Diff.Version->Key.ToString(), Diff.Version->Version);
						Archive.SetError();
						return false;
					}
					else if (Diff.Type == ECustomVersionDifference::Invalid)
					{
						UE_LOGF(LogLocalFileReplay, Error, "Replay was saved with an invalid custom version. Tag %ls Version %d", *Diff.Version->Key.ToString(), Diff.Version->Version);
						Archive.SetError();
						return false;
					}
					else if (Diff.Type == ECustomVersionDifference::Newer)
					{
						...
						UE_LOGF(LogLocalFileReplay, Error, "Replay was saved with a newer custom version than the current. Tag %ls Name '%ls' ReplayVersion %d  MaxExpected %d",
			...
			if (Archive.IsError())
			{
				UE_LOGF(LogLocalFileReplay, Error, "ReadReplayInfo: Failed to serialize replay friendly name.");
				return false;
			}
			...
			else
			{
				...
				UE_LOGF(LogLocalFileReplay, Warning, "ReadReplayInfo - Loading an old replay, friendly name length **must not** be changed.");
			}
			...
			if (SerializationInfo.GetLocalFileReplayVersion() >= FLocalFileReplayCustomVersion::EncryptionSupport)
			{
				...
				{
					UE_LOGF(LogLocalFileReplay, Error, "ReadReplayInfo: Serialized an invalid encryption key size: %d", KeySize);
					Archive.SetError();
					return false;
				}
			}
			...
			{
				UE_LOGF(LogLocalFileReplay, Error, "ReadReplayInfo: Completed replay is marked encrypted but has no key!");
				Archive.SetError();
				return false;
			}
			...
			while (!Archive.AtEnd())
			{
				...
				{
					UE_LOGF(LogLocalFileReplay, Error, "ReadReplayInfo: Invalid chunk size: %d", Chunk.SizeInBytes);
					Archive.SetError();
					return false;
				}
				...
				case ELocalFileChunkType::Header:
				{
					...
					{
						UE_LOGF(LogLocalFileReplay, Error, "ReadReplayInfo: Found multiple header chunks");
						Archive.SetError();
						return false;
					}
				}
				...
				case ELocalFileChunkType::Checkpoint:
				{
					...
					{
						UE_LOGF(LogLocalFileReplay, Error, "ReadReplayInfo: Invalid checkpoint disk size: %d", Checkpoint.SizeInBytes);
						Archive.SetError();
						return false;
					}
				}
				...
				case ELocalFileChunkType::ReplayData:
				{
					...
					{
						UE_LOGF(LogLocalFileReplay, Error, "ReadReplayInfo: Invalid stream chunk disk size: %d", DataChunk.SizeInBytes);
						Archive.SetError();
						return false;
					}
					...
					{
						UE_LOGF(LogLocalFileReplay, Error, "ReadReplayInfo: Invalid stream chunk memory size: %d", DataChunk.MemorySizeInBytes);
						Archive.SetError();
						return false;
					}
				...
				case ELocalFileChunkType::Event:
				{
					...
					{
						UE_LOGF(LogLocalFileReplay, Error, "ReadReplayInfo: Invalid event disk size: %d", Event.SizeInBytes);
						Archive.SetError();
						return false;
					}
				}
				...
				case ELocalFileChunkType::Unknown:
					UE_LOGF(LogLocalFileReplay, Verbose, "ReadReplayInfo: Skipping unknown (cleared) chunk");
					break;
				default:
					UE_LOGF(LogLocalFileReplay, Warning, "ReadReplayInfo: Unhandled file chunk type: %d", (uint32)ChunkType);
					break;
				}

				if (Archive.IsError())
				{
					UE_LOGF(LogLocalFileReplay, Error, "ReadReplayInfo: Archive error after parsing chunk");
					return false;
				}
		...
		for(const FLocalFileReplayDataInfo& DataInfo : Info.DataChunks)
		{
			...
			for(const FLocalFileReplayDataInfo& DataInfoCompare : Info.DataChunks)
			{
				if (DataInfo.ChunkIndex != DataInfoCompare.ChunkIndex)
				{
					...
					{
						UE_LOGF(LogLocalFileReplay, Error, "ReadReplayInfo: Found overlapping data chunks");
						Archive.SetError();
						return false;
					}
				}
		...
		for (const FLocalFileEventInfo& Checkpoint : Info.Checkpoints)
		{
			...
			{
				UE_LOGF(LogLocalFileReplay, Error, "ReadReplayInfo: Found duplicate checkpoint id: %ls", *Checkpoint.Id);
				Archive.SetError();
				return false;
			}
		...
		UE_LOGF(LogLocalFileReplay, Verbose, "ReadReplayInfo: IsValid: %ls MemSize: %lld bytes", *LexToString(Info.bIsValid), InfoSize);
...
bool FLocalFileNetworkReplayStreamer::AllowEncryptedWrite() const
{
	...
	UE_LOGF(LogLocalFileReplay, VeryVerbose, "FLocalFileNetworkReplayStreamer::AllowEncryptedWrite: %ls", *LexToString(bAllowWrite));
#endif
...
bool FLocalFileNetworkReplayStreamer::WriteReplayInfo(FArchive& Archive, const FLocalFileReplayInfo& InReplayInfo, FLocalFileSerializationInfo& SerializationInfo)
{
	if (SerializationInfo.GetLocalFileReplayVersion() < FLocalFileReplayCustomVersion::FixedSizeFriendlyName)
	{
		UE_LOGF(LogLocalFileReplay, Warning, "FLocalFileNetworkRepalyStreamer::WriteReplayInfo: Unable to safely rewrite old replay info");
		return false;
	}
	...
	if (SerializationInfo.GetLocalFileReplayVersion() < FLocalFileReplayCustomVersion::FriendlyNameCharEncoding)
	{
		...
		{
			UE_LOGF(LogLocalFileReplay, Warning, "FLocalFileNetworkRepalyStreamer::WriteReplayInfo: Forcing friendly name to ANSI to avoid corrupting file");
...
void FLocalFileNetworkReplayStreamer::StartStreaming(const FStartStreamingParameters& Params, const FStartStreamingCallback& Delegate)
{
	...
	if (IsStreaming())
	{
		UE_LOGF( LogLocalFileReplay, Warning, "FLocalFileNetworkReplayStreamer::StartStreaming. IsStreaming == true." );
	...
	{
		UE_LOGF( LogLocalFileReplay, Warning, "FLocalFileNetworkReplayStreamer::StartStreaming. IsFileRequestInProgress == true." );
		Delegate.ExecuteIfBound(Result);
		return;
	}
			...
			[this, Delegate](TLocalFileRequestCommonData<FStartStreamingResult>& RequestData)
			{
				...
				else
				{
					...
					else
					{
						...
						AddDelegateFileRequestToQueue<FStartStreamingCallback, FStartStreamingResult>(EQueuedLocalFileRequestType::StartPlayback, FStartStreamingCallback(),
							[this, Delegate](TLocalFileRequestCommonData<FStartStreamingResult>& PlaybackRequestData)
							{
								PostHeaderDownloaded([this, Delegate]()
									{
										if (StreamerState == EReplayStreamerState::Idle || bStopStreamingCalled)
										{
											// we were cancelled during operation.
											return;
										}
	...
	else
	{
		...
		{
			UE_LOGF(LogLocalFileReplay, Log, "Writing replay to '%ls' with %.2fMB free", *GetDemoPath(), (double)TotalDiskFreeSpace / 1024 / 1024);
		}
		...
		// LocalFileAr should be null if IsStreaming() returned false earlier */
		check(LocalFileAr == nullptr);
		...
		AddDelegateFileRequestToQueue<FStartStreamingResult>(EQueuedLocalFileRequestType::StartRecording,
			[this, Params, EncryptionKey=CurrentReplayInfo.EncryptionKey](TLocalFileRequestCommonData<FStartStreamingResult>& RequestData)
			{
				...
				UE_LOGF(LogLocalFileReplay, Verbose, "FLocalFileNetworkReplayStreamer::StartStreaming. Filename: %ls.", *FinalDemoName);
				...
				{
					UE_LOGF(LogLocalFileReplay, Warning, "FLocalFileNetworkReplayStreamer::StartStreaming. Invalid replay file name for recording: %ls.", *FullDemoFilename);
					RequestData.AsyncError = ELocalFileReplayResult::InvalidName;
					return;
				}

				// Delete any existing demo with this name if it exists
				if (IFileManager::Get().FileExists(*FullDemoFilename))
				{
					...
					if (ReadReplayInfo(FinalDemoName, ExistingInfo))
					{
						...
						{
							UE_LOGF(LogLocalFileReplay, Warning, "StartStreaming is overwriting an existing live replay file.");
						}
					}
					IFileManager::Get().Delete(*FullDemoFilename);
				}

				LocalFileAr = CreateLocalFileWriter(FullDemoFilename);
				if (!LocalFileAr.IsValid())
				{
					UE_LOGF(LogLocalFileReplay, Warning, "FLocalFileNetworkReplayStreamer::StartStreaming. Invalid LocalFileAr. You may need to enable localReplay.UsePlayerAccountIdPostfix.");
					RequestData.AsyncError = ELocalFileReplayResult::FileWriter;
					TaskReplayInfo.bIsValid = false;
					return;
				}
				...
				TaskReplayInfo.bIsValid = WriteReplayInfo(*LocalFileAr.Get(), TaskReplayInfo);

				if (!TaskReplayInfo.bIsValid)
				{
					UE_LOGF(LogLocalFileReplay, Warning, "StartStreaming was unable to write to the replay file: %ls", *FullDemoFilename);
					RequestData.AsyncError = ELocalFileReplayResult::FileWriter;
				}
				else
				{
			...
			[this, Delegate](TLocalFileRequestCommonData<FStartStreamingResult>& RequestData)
			{
				...
				{
					UE_LOGF(LogLocalFileReplay, Verbose, "FLocalFileNetworkReplayStreamer::StartStreaming. AsyncError: %d.", static_cast<uint32>(RequestData.AsyncError));
				}
...
void FLocalFileNetworkReplayStreamer::StopStreaming()
{
	...
	{
		UE_LOGF( LogLocalFileReplay, Warning, "FLocalFileNetworkReplayStreamer::StopStreaming. Called while existing StartStreaming request wasn't finished" );
		CancelStreamingRequests();
		check( !IsStreaming() );
		return;
	}
	...
	if (!IsStreaming())
	{
		UE_LOGF(LogLocalFileReplay, Log, "FLocalFileNetworkReplayStreamer::StopStreaming. Not currently streaming.");
	...
	{
		UE_LOGF(LogLocalFileReplay, Log, "FLocalFileNetworkReplayStreamer::StopStreaming. Already called");
		return;
	}
		...
		AddGenericRequestToQueue<ELocalFileReplayResult>(EQueuedLocalFileRequestType::StopRecording,
			[this, TotalLengthInMS](ELocalFileReplayResult& ReplayResult)
			{
				if (LocalFileAr.IsValid())
				{
					...
					WriteReplayInfo(*LocalFileAr.Get(), TaskReplayInfo);
				}
				LocalFileAr = nullptr;
			},
	...
	AddSimpleRequestToQueue(EQueuedLocalFileRequestType::StopStreaming,
		[]()
		{
			UE_LOGF(LogLocalFileReplay, Verbose, "FLocalFileNetworkReplayStreamer::StopStreaming");
		},
		[this]()
		{
			bStopStreamingCalled = false;
...
void FLocalFileNetworkReplayStreamer::DeleteFinishedStream_Internal(const FString& StreamName, const int32 UserIndex, const FDeleteFinishedStreamCallback& Delegate)
{
	AddDelegateFileRequestToQueue<FDeleteFinishedStreamCallback, FDeleteFinishedStreamResult>(EQueuedLocalFileRequestType::DeletingFinishedStream, Delegate,
		[this, StreamName](TLocalFileRequestCommonData<FDeleteFinishedStreamResult>& RequestData)
		{
			...
			{
				UE_CLOGF(bIsLive, LogLocalFileReplay, Warning, "Deleting network replay stream %ls that is currently live!", *StreamName);
			...
			{
				UE_LOGF(LogLocalFileReplay, Warning, "Can't delete network replay stream %ls because it is live!", *StreamName);
			}
		});
}
...
{
	UE_LOGF(LogLocalFileReplay, Log, "FLocalFileNetworkReplayStreamer::EnumerateRecentStreamsEnumerateRecentStreams is currently unsupported.");
...
{
	UE_LOGF(LogLocalFileReplay, Log, "FLocalFileNetworkReplayStreamer::AddUserToReplay is currently unsupported.");
}
...
void FLocalFileNetworkReplayStreamer::AddEvent(const uint32 TimeInMS, const FString& Group, const FString& Meta, const TArray<uint8>& Data)
{
	if (StreamerState != EReplayStreamerState::Recording)
	{
		UE_LOGF(LogLocalFileReplay, Warning, "FLocalFileNetworkReplayStreamer::AddEvent. Not recording.");
		return;
	}
...
void FLocalFileNetworkReplayStreamer::AddOrUpdateEvent(const FString& Name, const uint32 TimeInMS, const FString& Group, const FString& Meta, const TArray<uint8>& Data)
{
	if (StreamerState != EReplayStreamerState::Recording)
	{
		UE_LOGF(LogLocalFileReplay, Warning, "FLocalFileNetworkReplayStreamer::AddOrUpdateEvent. Not recording.");
		return;
	}

	UE_LOGF(LogLocalFileReplay, Verbose, "FLocalFileNetworkReplayStreamer::AddOrUpdateEvent. Size: %i", Data.Num());
		...
		[this, Name, Group, TimeInMS, Meta, Data](ELocalFileReplayResult& ReplayResult)
		{
			...
			if (LocalFileAr.IsValid())
			{
				...
				if (AllowEncryptedWrite())
				{
					...
					{
						UE_LOGF(LogLocalFileReplay, Warning, "FLocalFileNetworkReplayStreamer::AddOrUpdateEvent - EncryptBuffer failed");
	...
	AddDelegateFileRequestToQueue<FRequestEventDataCallback, FRequestEventDataResult>(EQueuedLocalFileRequestType::RequestingEvent, RequestEventDataComplete,
		[this, ReplayName, EventID](TLocalFileRequestCommonData<FRequestEventDataResult>& RequestData)
		{
			...
			else
			{
				...
				if (ReadReplayInfo(FileName, StoredReplayInfo))
				{
					TSharedPtr<FArchive> LocalFileReader = CreateLocalFileReader(FullDemoFilename);
					if (LocalFileReader.IsValid())
					{
						...
						for (const FLocalFileEventInfo& EventInfo : StoredReplayInfo.Events)
						{
							if (EventInfo.Id == EventID)
							{
								...
								LocalFileReader->Seek(EventInfo.EventDataOffset);

								TArray<uint8> EventData;

								EventData.AddUninitialized(EventInfo.SizeInBytes);
								LocalFileReader->Serialize(EventData.GetData(), EventData.Num());
								...
								if (StoredReplayInfo.bEncrypted)
								{
									if (SupportsEncryption())
									{
										...
										{
											UE_LOGF(LogLocalFileReplay, Error, "FLocalFileNetworkReplayStreamer::RequestEventData_Internal. DecryptBuffer failed.");
									...
									{
										UE_LOGF(LogLocalFileReplay, Error, "FLocalFileNetworkReplayStreamer::RequestEventData_Internal. Encrypted event but streamer does not support encryption.");
					...
					{
						UE_LOGF(LogLocalFileReplay, Error, "FLocalFileNetworkReplayStreamer::RequestEventData_Internal. Unable to read replay file: %ls", *FullDemoFilename);
						RequestData.DelegateResult.Result = EStreamingOperationResult::Unspecified;
					}
				}
				else
				{
					UE_LOGF(LogLocalFileReplay, Error, "FLocalFileNetworkReplayStreamer::RequestEventData_Internal. Failed to read the replay info: %ls", *FileName);
					RequestData.DelegateResult.Result = EStreamingOperationResult::Unspecified;
				}
			}
		});
		...
		[this, ReplayName, Group](TLocalFileRequestCommonData<FRequestEventGroupDataResult>& RequestData)
		{
			...
			else
			{
				...
				if (ReadReplayInfo(FileName, StoredReplayInfo))
				{
					...
					{
						UE_LOGF(LogLocalFileReplay, Error, "FLocalFileNetworkReplayStreamer::RequestEventData_Internal. Failed to read the replay file: %ls", *FullDemoFilename);
						RequestData.DelegateResult.Result = EStreamingOperationResult::Unspecified;
					}
				}
				else
...
{
	UE_LOGF(LogLocalFileReplay, Log, "FLocalFileNetworkReplayStreamer::SearchEvents is currently unsupported.");
...
void FLocalFileNetworkReplayStreamer::RenameReplayFriendlyName_Internal(const FString& ReplayName, const FString& NewFriendlyName, const int32 UserIndex, const FRenameReplayCallback& Delegate)
{
	AddDelegateFileRequestToQueue<FRenameReplayCallback, FRenameReplayResult>(EQueuedLocalFileRequestType::RenameReplayFriendlyName, Delegate,
		[this, ReplayName, NewFriendlyName](TLocalFileRequestCommonData<FRenameReplayResult>& RequestData)
		{
			...
			{
				UE_LOGF(LogLocalFileReplay, Warning, "FLocalFileNetworkReplayStreamer::RenameReplayFriendlyName: Replay does not exist %ls", *ReplayName);
				RequestData.DelegateResult.Result = EStreamingOperationResult::ReplayNotFound;
				return;
			}
			...
			{
				TSharedPtr<FArchive> LocalFileReader = CreateLocalFileReader(FullReplayName);
				if (!LocalFileReader.IsValid() || LocalFileReader->TotalSize() <= 0 || !ReadReplayInfo(*LocalFileReader.Get(), TempReplayInfo, SerializationInfo, EReadReplayInfoFlags::None))
				{
					UE_LOGF(LogLocalFileReplay, Warning, "FLocalFileNetworkReplayStreamer::RenameReplayFriendlyName: Failed to read replay info %ls", *ReplayName);
					return;
				}

				if (SerializationInfo.GetLocalFileReplayVersion() < FLocalFileReplayCustomVersion::FixedSizeFriendlyName)
				{
					UE_LOGF(LogLocalFileReplay, Warning, "FLocalFileNetworkReplayStreamer::RenameReplayFriendlyName: Replay too old to rename safely %ls", *ReplayName);
					return;
				}
			}
			...
			// Do this inside a scope, to make sure the file archive is closed before continuing.
			{
				// ensure existing replay is flushed and closed
				LocalFileAr = nullptr;

				TSharedPtr<FArchive> LocalFileWriter = CreateLocalFileWriter(FullReplayName);
				if (!LocalFileWriter.IsValid() || LocalFileWriter->TotalSize() <= 0 || !WriteReplayInfo(*LocalFileWriter.Get(), TempReplayInfo, SerializationInfo))
				{
					UE_LOGF(LogLocalFileReplay, Warning, "FLocalFileNetworkReplayStreamer::RenameReplayFriendlyName: Failed to write replay info %ls", *ReplayName);
					return;
				}
			}
...
void FLocalFileNetworkReplayStreamer::RenameReplay_Internal(const FString& ReplayName, const FString& NewName, const int32 UserIndex, const FRenameReplayCallback& Delegate)
{
	AddDelegateFileRequestToQueue<FRenameReplayCallback, FRenameReplayResult>(EQueuedLocalFileRequestType::RenameReplay, Delegate,
		[this, ReplayName, NewName](TLocalFileRequestCommonData<FRenameReplayResult>& RequestData)
		{
			...
			if (!FPaths::FileExists(FullReplayName))
			{
				UE_LOGF(LogLocalFileReplay, Warning, "FLocalFileNetworkReplayStreamer::RenameReplay: Replay does not exist (old %ls new %ls)", *ReplayName, *NewName);
				RequestData.DelegateResult.Result = EStreamingOperationResult::ReplayNotFound;
				return;
			}
			...
			{
				UE_LOGF(LogLocalFileReplay, Warning, "FLocalFileNetworkReplayStreamer::RenameReplay: Path separator characters present in replay (old %ls new %ls)", *ReplayName, *NewName);
				return;
			}

			if (!IFileManager::Get().Move(*NewReplayName, *FullReplayName, /* bReplace= */ false))
			{
				UE_LOGF(LogLocalFileReplay, Warning, "FLocalFileNetworkReplayStreamer::RenameReplay: Failed to rename replay (old %ls new %ls)", *ReplayName, *NewName);
				return;
			}
...
void FLocalFileNetworkReplayStreamer::FlushStream(const uint32 TimeInMS)
{
	...
	if (IsFileRequestPendingOrInProgress(EQueuedLocalFileRequestType::WriteHeader))
	{
		...
		UE_LOGF( LogLocalFileReplay, Warning, "FLocalFileNetworkReplayStreamer::FlushStream. Waiting on header upload." );
		return;
	}
	...
	AddGenericRequestToQueue<ELocalFileReplayResult>(EQueuedLocalFileRequestType::WritingStream,
		[this, StreamChunkStartMS, StreamChunkEndMS, TotalLengthInMS, StreamData = MoveTemp(StreamAr.Buffer)](ELocalFileReplayResult& ReplayResult) mutable
		{
			...
			if (LocalFileAr.IsValid())
			{
				...
				if (SupportsCompression())
				{
					...
					{
						UE_LOGF(LogLocalFileReplay, Warning, "FLocalFileNetworkReplayStreamer::FlushStream - CompressBuffer failed");
				...
				if (AllowEncryptedWrite())
				{
					...
					{
						UE_LOGF(LogLocalFileReplay, Warning, "FLocalFileNetworkReplayStreamer::FlushStream - EncryptBuffer failed");
						ReplayResult = ELocalFileReplayResult::EncryptBuffer;
						return;
					}
				}
		...
		[this](ELocalFileReplayResult& ReplayResult)
		{
			...
			{
				UE_LOGF(LogLocalFileReplay, Error, "FLocalFileNetworkReplayStreamer::FlushStream failed.");
				SetLastError(ReplayResult);
			}
		});
...
void FLocalFileNetworkReplayStreamer::FlushCheckpoint(const uint32 TimeInMS)
{
	...
	{
		UE_LOGF( LogLocalFileReplay, Warning, "FLocalFileNetworkReplayStreamer::FlushCheckpoint. Checkpoint is empty." );
		return;
	}
...
void FLocalFileNetworkReplayStreamer::FlushCheckpointInternal(const uint32 TimeInMS)
{
	...
	AddGenericRequestToQueue<ELocalFileReplayResult>(EQueuedLocalFileRequestType::WritingCheckpoint, 
		...
		[this](ELocalFileReplayResult& ReplayResult)
		{
			...
			{
				UE_LOGF(LogLocalFileReplay, Error, "FLocalFileNetworkReplayStreamer::FlushCheckpointInternal failed.");
				SetLastError(ReplayResult);
			}
		});
...
void FLocalFileNetworkReplayStreamer::GotoCheckpointIndex(const int32 CheckpointIndex, const FGotoCallback& Delegate, EReplayCheckpointType CheckpointType)
{
	if (IsFileRequestPendingOrInProgress(EQueuedLocalFileRequestType::ReadingCheckpoint))
	{
		...
		UE_LOGF(LogLocalFileReplay, Warning, "FLocalFileNetworkReplayStreamer::GotoCheckpointIndex. Busy processing another checkpoint.");
	...
	if (CheckpointIndex == INDEX_NONE)
	{
		AddSimpleRequestToQueue(EQueuedLocalFileRequestType::ReadingCheckpoint, 
			[]()
			{
				UE_LOGF(LogLocalFileReplay, Verbose, "FLocalFileNetworkReplayStreamer::GotoCheckpointIndex");
			},
			[this, Delegate]()
			{
	...
	{
		UE_LOGF(LogLocalFileReplay, Warning, "FLocalFileNetworkReplayStreamer::GotoCheckpointIndex. Invalid checkpoint index.");
		Delegate.ExecuteIfBound(FGotoResult());
		return;
	}
	...
	if (CheckpointType == EReplayCheckpointType::Delta)
	{
			...
			[this, CheckpointIndex](TLocalFileRequestCommonData<FGotoResult>& RequestData)
		{
			...
			if (LocalFileReader.IsValid())
			{
					...
					for (int32 i = 0; i <= CheckpointIndex; ++i)
					{
						...
						{
							LocalFileReader->Seek(TaskReplayInfo.Checkpoints[i].EventDataOffset);

							CheckpointData.Reset();
							CheckpointData.AddUninitialized(TaskReplayInfo.Checkpoints[i].SizeInBytes);

							LocalFileReader->Serialize(CheckpointData.GetData(), CheckpointData.Num());
							...
							if (TaskReplayInfo.bEncrypted)
							{
								if (SupportsEncryption())
								{
									...
									{
										UE_LOGF(LogLocalFileReplay, Error, "FLocalFileNetworkReplayStreamer::GotoCheckpointIndexDelta. DecryptBuffer FAILED.");
										RequestData.DataBuffer.Empty();
										return;
									}
								}
								...
								{
									UE_LOGF(LogLocalFileReplay, Error, "FLocalFileNetworkReplayStreamer::GotoCheckpointIndexDelta. Encrypted checkpoint but streamer does not support encryption.");
									RequestData.DataBuffer.Empty();
									return;
								}
							}
							...
							if (TaskReplayInfo.bCompressed)
							{
								if (SupportsCompression())
								{
									...
									{
										UE_LOGF(LogLocalFileReplay, Error, "FLocalFileNetworkReplayStreamer::GotoCheckpointIndexDelta. DecompressBuffer FAILED.");
										RequestData.DataBuffer.Empty();
										return;
									}
								}
								...
								{
									UE_LOGF(LogLocalFileReplay, Error, "FLocalFileNetworkReplayStreamer::GotoCheckpointIndexDelta. Compressed checkpoint but streamer does not support compression.");
									RequestData.DataBuffer.Empty();
									return;
								}
							}
			...
			[this, CheckpointIndex, Delegate](TLocalFileRequestCommonData<FGotoResult>& RequestData)
		{
			...
			{
				UE_LOGF(LogLocalFileReplay, Warning, "FLocalFileNetworkReplayStreamer::GotoCheckpointIndexDelta. Checkpoint empty.");
				Delegate.ExecuteIfBound(RequestData.DelegateResult);
				LastGotoTimeInMS = -1;
				return;
			}
			...
			{
				UE_LOGF(LogLocalFileReplay, Error, "FLocalFileNetworkReplayStreamer::GotoCheckpointIndexDelta. Checkpoint data chunk index invalid: %d", DataChunkIndex);
				Delegate.ExecuteIfBound(RequestData.DelegateResult);
				LastGotoTimeInMS = -1;
				return;
			}
			...
			UE_LOGF(LogLocalFileReplay, Verbose, "FLocalFileNetworkReplayStreamer::GotoCheckpointIndexDelta. SUCCESS. StreamChunkIndex: %i", StreamChunkIndex);
	...
	else
	{
			...
			[this, CheckpointIndex](TLocalFileRequestCommonData<FGotoResult>& RequestData)
		{
			...
			if (LocalFileReader.IsValid())
			{
				LocalFileReader->Seek(TaskReplayInfo.Checkpoints[CheckpointIndex].EventDataOffset);

				RequestData.DataBuffer.AddUninitialized(TaskReplayInfo.Checkpoints[CheckpointIndex].SizeInBytes);

				LocalFileReader->Serialize(RequestData.DataBuffer.GetData(), RequestData.DataBuffer.Num());
				...
				if (TaskReplayInfo.bEncrypted)
					{
						...
						if (!DecryptBuffer(RequestData.DataBuffer, DecryptedData, TaskReplayInfo.EncryptionKey))
							{
								UE_LOGF(LogLocalFileReplay, Error, "FLocalFileNetworkReplayStreamer::GotoCheckpointIndex. DecryptBuffer FAILED.");
								RequestData.DataBuffer.Empty();
								return;
							}
						...
						{
							UE_LOGF(LogLocalFileReplay, Error, "FLocalFileNetworkReplayStreamer::GotoCheckpointIndex. Encrypted checkpoint but streamer does not support encryption.");
							RequestData.DataBuffer.Empty();
							return;
						}
					}
				...
				if (TaskReplayInfo.bCompressed)
					{
						if (SupportsCompression())
						{
							...
							{
								UE_LOGF(LogLocalFileReplay, Error, "FLocalFileNetworkReplayStreamer::GotoCheckpointIndex. DecompressBuffer FAILED.");
								RequestData.DataBuffer.Empty();
								return;
							}
						...
						{
							UE_LOGF(LogLocalFileReplay, Error, "FLocalFileNetworkReplayStreamer::GotoCheckpointIndex. Compressed checkpoint but streamer does not support compression.");
							RequestData.DataBuffer.Empty();
							return;
						}
					}
		...
		[this, CheckpointIndex, Delegate](TLocalFileRequestCommonData<FGotoResult>& RequestData)
		{
			...
			if (RequestData.DataBuffer.Num() == 0)
			{
				UE_LOGF(LogLocalFileReplay, Warning, "FLocalFileNetworkReplayStreamer::GotoCheckpointIndex. Checkpoint empty.");
				Delegate.ExecuteIfBound(RequestData.DelegateResult);
				LastGotoTimeInMS = -1;
				return;
			}
			...
			UE_LOGF(LogLocalFileReplay, Verbose, "FLocalFileNetworkReplayStreamer::GotoCheckpointIndex. SUCCESS. StreamChunkIndex: %i", StreamChunkIndex);
...
void FLocalFileNetworkReplayStreamer::GotoTimeInMS(const uint32 TimeInMS, const FGotoCallback& Delegate, EReplayCheckpointType CheckpointType)
{
	if (IsFileRequestPendingOrInProgress(EQueuedLocalFileRequestType::ReadingCheckpoint) || LastGotoTimeInMS != -1)
	{
		...
		UE_LOGF(LogLocalFileReplay, Log, "FLocalFileNetworkReplayStreamer::GotoTimeInMS. Busy processing pending requests.");
		Delegate.ExecuteIfBound( FGotoResult() );
		return;
	}

	UE_LOGF(LogLocalFileReplay, Verbose, "FLocalFileNetworkReplayStreamer::GotoTimeInMS. TimeInMS: %i", (int)TimeInMS );
...
bool FLocalFileNetworkReplayStreamer::ProcessNextFileRequest()
{
	...
	if (QueuedRequests.Num() > 0)
	{
		...
		UE_LOGF(LogLocalFileReplay, Verbose, "FLocalFileNetworkReplayStreamer::ProcessNextFileRequest. Dequeue Type: %ls", EQueuedLocalFileRequestType::ToString(QueuedRequest->GetRequestType()));
...
bool FLocalFileNetworkReplayStreamer::GetDemoFreeStorageSpace(uint64& DiskFreeSpace, const FString& DemoPath)
{
	...
	{
		UE_LOGF(LogLocalFileReplay, Log, "Failed to get persistent storage useage for %ls from the FPeristentStorageManager, falling back to global total disk size", *DemoPath);
	}
#endif
	...
	if (!bActualStorageQueryResult)
	{
		...
		if (!bActualStorageQueryResult)
		{
			UE_LOGF(LogLocalFileReplay, Log, "FLocalFileNetworkReplayStreamer::GetDemoFreeStorageSpace. Unable to determine free space in %ls.", *DemoPath);
			return false;
		}
	}
...
bool FLocalFileNetworkReplayStreamer::CleanUpOldReplays(const FString& DemoPath, TArrayView<const FString> AdditionalRelativeDemoPaths)
{
	...
	if (!bUnlimitedDemos)
	{
		...
		{
			const FString WildCardPath = GetDemoFullFilename(DemoPath, AutoPrefix + FString("*"));
			...
			((TotalDiskFreeSpace < MinFreeSpace) || (SortedAutoReplays.Num() >= MaxDemos)))
		{
			...
			{
				UE_LOGF(LogLocalFileReplay, Warning, "FLocalFileNetworkReplayStreamer::CleanUpOldReplays. Unable to delete old replay %ls.", *OldestReplay.Path);
				return false;
			}

			if (!GetDemoFreeStorageSpace(TotalDiskFreeSpace, DemoPath))
			{
				UE_LOGF(LogLocalFileReplay, Warning, "FLocalFileNetworkReplayStreamer::CleanUpOldReplays. Unable to refresh free space in %ls.", *DemoPath);
				return false;
			}
		}
...
FString FLocalFileNetworkReplayStreamer::GetAutomaticDemoName() const
{
	...
	if (bUseDatePostfix)
	{
		...
		const FString FinalDemoName =
			UE::Net::LocalFileReplay::CVarbUsePlayerAccountIdPostfix->GetBool()
			? AutoPrefix + GetLocalPlayerUniqueNetId() + "-" + FDateTime::Now().ToString()
			: AutoPrefix + FDateTime::Now().ToString();

		UE_LOGF(LogLocalFileReplay, Verbose, "FLocalFileNetworkReplayStreamer::CleanUpOldReplays. FinalDemoName: %ls.", *FinalDemoName);
...
FString FLocalFileNetworkReplayStreamer::GetLocalPlayerUniqueNetId() const
{
	if (GEngine)
	{
		...
		AddDelegateFileRequestToQueue<FDownloadHeaderResult>(EQueuedLocalFileRequestType::ReadingHeader,
			[this, HeaderSize, HeaderOffset](TLocalFileRequestCommonData<FDownloadHeaderResult>& RequestData)
			{
				...
				if (LocalFileReader.IsValid())
				{
					...
					LocalFileReader->Seek(HeaderOffset);
					LocalFileReader->Serialize((void*)HeaderData.GetData(), HeaderData.Num());
...
void FLocalFileNetworkReplayStreamer::WriteHeader()
{
	...
	{
		// IF there is no active session, or we are not recording, we don't need to flush
		UE_LOGF(LogLocalFileReplay, Warning, "FLocalFileNetworkReplayStreamer::WriteHeader. No session name!");
		return;
	}
	...
	if (HeaderAr.TotalSize() == 0)
	{
		...
		UE_LOGF(LogLocalFileReplay, Warning, "FLocalFileNetworkReplayStreamer::WriteHeader. No header to upload");
		return;
	}

	if (!IsStreaming())
	{
		UE_LOGF(LogLocalFileReplay, Warning, "FLocalFileNetworkReplayStreamer::WriteHeader. Not currently streaming");
		return;
	}
	...
	AddGenericRequestToQueue<ELocalFileReplayResult>(EQueuedLocalFileRequestType::WritingHeader,
		[this, HeaderData=MoveTemp(HeaderAr.Buffer)](ELocalFileReplayResult& ReplayResult)
		{
			...
			if (LocalFileAr.IsValid())
			{
				if (TaskReplayInfo.HeaderChunkIndex == INDEX_NONE)
				{
					...
					{
						UE_LOGF(LogLocalFileReplay, Error, "FLocalFileNetworkReplayStreamer::WriteHeader - Current file already has unexpected header");
		...
		[this](ELocalFileReplayResult& ReplayResult)
		{
			...
			{
				UE_LOGF(LogLocalFileReplay, Error, "FLocalFileNetworkReplayStreamer::WriteHeader failed.");
				SetLastError(ReplayResult);
			}
		});
...
void FLocalFileNetworkReplayStreamer::RefreshHeader()
{
	AddSimpleRequestToQueue(EQueuedLocalFileRequestType::WriteHeader, 
		[]()
		{
			UE_LOGF(LogLocalFileReplay, Verbose, "FLocalFileNetworkReplayStreamer::RefreshHeader");
		},
		[this]()
		{
			WriteHeader();
...
void FLocalFileNetworkReplayStreamer::OnFileRequestComplete(const TSharedPtr<FQueuedLocalFileRequest, ESPMode::ThreadSafe>& Request)
{
	...
	{
		UE_LOGF(LogLocalFileReplay, Verbose, "FLocalFileNetworkReplayStreamer::OnFileRequestComplete. Type: %ls", EQueuedLocalFileRequestType::ToString(Request->GetRequestType()));
	...
	if (!bReallyNeedToLoadChunk)
	{
		...
		if ((StreamTimeRange.Max > StreamTimeRange.Min) && (StreamAr.TotalSize() > 0))
		{
			...
			if (TimeLeft > MaxBufferedTimeSeconds)
			{
				...
				UE_LOGF(LogLocalFileReplay, VeryVerbose, "ConditionallyLoadNextChunk. Cancelling due buffer being large enough. TotalStreamTime: %2.2f, PercentIn: %2.2f, TimeLeft: %2.2f", TotalStreamTimeSeconds, PercentIn, TimeLeft);
				return;
			}
		}
	}

	UE_LOGF(LogLocalFileReplay, Log, "FLocalFileNetworkReplayStreamer::ConditionallyLoadNextChunk. Index: %d", StreamChunkIndex);
	...
	AddCachedFileRequestToQueue<FStreamingResultBase>(EQueuedLocalFileRequestType::ReadingStream, CurrentReplayInfo.DataChunks[StreamChunkIndex].ChunkIndex,
		[this, RequestedStreamChunkIndex](TLocalFileRequestCommonData<FStreamingResultBase>& RequestData)
		{
			...
			check(TaskReplayInfo.DataChunks.IsValidIndex(RequestedStreamChunkIndex));
				...
				if (LocalFileReader.IsValid())
				{
					LocalFileReader->Seek(TaskReplayInfo.DataChunks[RequestedStreamChunkIndex].ReplayDataOffset);
				...
				if (TaskReplayInfo.bEncrypted)
					{
						...
						if (DecryptBuffer(RequestData.DataBuffer, DecryptedData, TaskReplayInfo.EncryptionKey))
							...
							{
								UE_LOGF(LogLocalFileReplay, Error, "ConditionallyLoadNextChunk failed to decrypt data.");
								RequestData.DataBuffer.Empty();
						...
						{
							UE_LOGF(LogLocalFileReplay, Error, "ConditionallyLoadNextChunk: Replay is marked encrypted but streamer does not support it.");
							RequestData.DataBuffer.Empty();
				...
				if (TaskReplayInfo.bCompressed)
					{
						if (SupportsCompression())
						{
							...
							{
								UE_LOGF(LogLocalFileReplay, Error, "ConditionallyLoadNextChunk failed to uncompresss data.");
								RequestData.DataBuffer.Empty();
						...
						{
							UE_LOGF(LogLocalFileReplay, Error, "ConditionallyLoadNextChunk: Replay is marked compressed but streamer does not support it.");
							RequestData.DataBuffer.Empty();
		...
		[this, RequestedStreamChunkIndex](TLocalFileRequestCommonData<FStreamingResultBase>& RequestData)
		{
			...
			{
				UE_LOGF(LogLocalFileReplay, Warning, "FLocalFileNetworkReplayStreamer::ConditionallyLoadNextChunk failed with requested chunk index mismatch.");
...
void FLocalFileNetworkReplayStreamingFactory::Tick( float DeltaTime )
{
	for (int i = LocalFileStreamers.Num() - 1; i >= 0; i--)
	{
		...
		if (LocalFileStreamers[i].IsUnique() && !LocalFileStreamers[i]->HasPendingFileRequests())
		{
			...
			{
				UE_LOGF(LogLocalFileReplay, Warning, "FLocalFileNetworkReplayStreamingFactory::Tick. Stream was stopped early.");
			}
...
void FLocalFileNetworkReplayStreamingFactory::Flush()
{
	...
	if (bFlushStreamersOnShutdown)
	{
		...
		while (HasAnyPendingRequests())
		{
			...
			{
				UE_LOGF(LogLocalFileReplay, Display, "Abandoning streamer flush after waiting %0.2f seconds", TotalWait);
				break;
			}
			...
			if (HasAnyPendingRequests())
			{
				...
				{
					UE_LOGF(LogLocalFileReplay, Display, "Sleeping 0.1s to wait for outstanding requests.");
