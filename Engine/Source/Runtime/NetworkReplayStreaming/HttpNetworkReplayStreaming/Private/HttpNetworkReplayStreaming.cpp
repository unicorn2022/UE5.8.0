...
void FHttpNetworkReplayStreamer::StartStreaming(const FStartStreamingParameters& Params, const FStartStreamingCallback& Delegate)
{
	...
	{
		UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::StartStreaming. SessionName already set." );
	...
	if ( IsStreaming() )
	{
		UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::StartStreaming. IsStreaming == true." );
		Delegate.ExecuteIfBound(StreamingResult);
		return;
	}
	...
	{
		UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::StartStreaming. ServerURL is empty, aborting." );
		Delegate.ExecuteIfBound(StreamingResult);
		return;
	}
	...
	{
		UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::StartStreaming. IsHttpRequestInFlight == true." );
		Delegate.ExecuteIfBound(StreamingResult);
		return;
	}
	...
	if ( !Params.bRecord )
	{
		...
		UE_LOGF( LogHttpReplay, Log, "FHttpNetworkReplayStreamer::StartStreaming. URL: %ls", *URL );
...
{
	UE_LOGF( LogHttpReplay, Verbose, "FHttpNetworkReplayStreamer::AddRequestToQueue. Type: %ls", EQueuedHttpRequestType::ToString( Type ) );
...
{
	UE_LOGF( LogHttpReplay, Verbose, "FHttpNetworkReplayStreamer::AddCustomRequestToQueue. Type: %ls", EQueuedHttpRequestType::ToString( Request->Type ) );
...
bool FHttpNetworkReplayStreamer::RetryRequest( TSharedPtr< FQueuedHttpRequest > Request, FHttpResponsePtr HttpResponse, const bool bIgnoreResponseCode )
{
	...
	UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::RetryRequest. Retrying: %i, %ls", Request->RetryProgress, *BuildRequestErrorString( Request->Request, HttpResponse ) );
...
void FHttpNetworkReplayStreamer::StopStreaming()
{
	...
	{
		UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::StopStreaming. Called while existing StartStreaming request wasn't finished" );
	...
	if ( !IsStreaming() )
	{
		UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::StopStreaming. Not currently streaming." );
	...
	{
		UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::StopStreaming. Already called" );
		return;
	}
...
void FHttpNetworkReplayStreamer::UploadHeader()
{
	...
	if ( SessionName.IsEmpty() )
	{
		...
		UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::UploadHeader. No session name!" );
		return;
	}
	...
	if ( HeaderArchive.Buffer.Num() == 0 )
	{
		...
		UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::UploadHeader. No header to upload" );
		return;
	}

	if ( !IsStreaming() )
	{
		UE_LOGF(LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::UploadHeader. Not currently streaming");
		return;
	}

	// First upload the header
	UE_LOGF( LogHttpReplay, Log, "FHttpNetworkReplayStreamer::UploadHeader. Header. StreamChunkIndex: %i, Size: %i", StreamChunkIndex, HeaderArchive.Buffer.Num() );
...
bool FHttpNetworkReplayStreamer::CompressRequest(FHttpRequestPtr HttpRequest, const TArray<uint8>& RequestBuffer) const
{
	if ( SupportsCompression() )
	{
		...
		UE_LOGF( LogHttpReplay, VeryVerbose, "Compressed request. Original: %i, Compressed: %i, Time: %2.2f MS", StreamArchive.Buffer.Num(), Compressed.Buffer.Num(), ( EndTime - StartTime ) * 1000.0f );
...
void FHttpNetworkReplayStreamer::FlushStream()
{
	...
	if ( SessionName.IsEmpty() )
	{
		...
		UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::FlushStream. Waiting on header upload." );
		return;
	}
	...
	UE_LOGF( LogHttpReplay, Verbose, "FHttpNetworkReplayStreamer::FlushStream. StreamChunkIndex: %i, Size: %i", StreamChunkIndex, StreamArchive.Buffer.Num() );
...
void FHttpNetworkReplayStreamer::FlushCheckpoint( const uint32 TimeInMS )
{
	...
	{
		UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::FlushCheckpoint. Checkpoint is empty." );
		return;
	}
...
void FHttpNetworkReplayStreamer::GotoCheckpointIndex( const int32 CheckpointIndex, const FGotoCallback& Delegate, EReplayCheckpointType CheckpointType )
{
	...
	{
		UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::GotoCheckpointIndex. Unsupported checkpoint type." );
	...
	if ( GotoCheckpointDelegate.IsBound() || DownloadCheckpointIndex != -1 || DeltaDownloadCheckpointIndex != -1 )
	{
		...
		UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::GotoCheckpointIndex. Busy processing another checkpoint." );
		Delegate.ExecuteIfBound(FGotoResult());
		return;
	}
	...
	if ( ServerURL.IsEmpty() )
	{
		UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::GotoCheckpointIndex. ServerURL is empty, aborting." );
		Delegate.ExecuteIfBound(FGotoResult());
		return;
	}
	...
	{
		UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::GotoCheckpointIndex. Invalid checkpoint index." );
		Delegate.ExecuteIfBound(FGotoResult());
		return;
	}
...
void FHttpNetworkReplayStreamer::SearchEvents(const FString& EventGroup, const FSearchEventsCallback& Delegate)
{
	if ( ServerURL.IsEmpty() )
	{
		UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::SearchEvents. ServerURL is empty, aborting." );
...
{
	UE_LOGF(LogHttpReplay, Verbose, "FHttpNetworkReplayStreamer::RequestEventData: Ignoring explicit ReplayName.");
...
void FHttpNetworkReplayStreamer::RequestEventData(const FString& EventID, const FRequestEventDataCallback& Delegate)
{
	if ( ServerURL.IsEmpty() )
	{
		UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::RequestEventData. ServerURL is empty, aborting." );
...
void FHttpNetworkReplayStreamer::HttpRequestEventDataFinished(FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool bSucceeded, FRequestEventDataCallback RequestEventDataCompleteDelegate)
{
	...
	{
		UE_LOGF(LogHttpReplay, Verbose, "FHttpNetworkReplayStreamer::HttpRequestEventDataFinished.");
	...
	{
		// FAIL
		UE_LOGF(LogHttpReplay, Error, "FHttpNetworkReplayStreamer::HttpRequestEventDataFinished. FAILED, Response code: %d", HttpResponse.IsValid() ? HttpResponse->GetResponseCode() : 0);
	}
...
{
	UE_LOGF(LogHttpReplay, Log, "FHttpNetworkReplayStreamer::RequestEventGroupData is currently unsupported.");
...
void FHttpNetworkReplayStreamer::GotoTimeInMS( const uint32 TimeInMS, const FGotoCallback& Delegate, EReplayCheckpointType CheckpointType )
{
	if ( !IsCheckpointTypeSupported(CheckpointType) )
	{
		UE_LOGF( LogHttpReplay, Log, "FHttpNetworkReplayStreamer::GotoTimeInMS. Unsupported checkpoint type." );
		Delegate.ExecuteIfBound( FGotoResult() );
		return;
	}
	...
	if ( LastGotoTimeInMS != -1 || DownloadCheckpointIndex != -1 )
	{
		...
		UE_LOGF( LogHttpReplay, Log, "FHttpNetworkReplayStreamer::GotoTimeInMS. Busy processing pending requests." );
		Delegate.ExecuteIfBound( FGotoResult() );
		return;
	}
	...
	{
		// If we're currently going to a checkpoint now, ignore this request
		UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::GotoTimeInMS. Busy processing another checkpoint." );
		Delegate.ExecuteIfBound( FGotoResult() );
		return;
	}

	UE_LOGF( LogHttpReplay, Verbose, "FHttpNetworkReplayStreamer::GotoTimeInMS. TimeInMS: %i", (int)TimeInMS );
...
void FHttpNetworkReplayStreamer::FlushCheckpointInternal( uint32 TimeInMS )
{
	...
	// Upload any new streamed data to the http server
	UE_LOGF(LogHttpReplay, Log, "FHttpNetworkReplayStreamer::FlushCheckpointInternal. Size: %i, StreamChunkIndex: %i, Time: %2.2f MS", 
...
bool FQueuedHttpRequestAddEvent::PreProcess( FHttpNetworkReplayStreamer* Streamer, const FString& ServerURL, const FString& SessionName )
{
	if ( SessionName.IsEmpty() )
	{
		UE_LOGF( LogHttpReplay, Warning, "FQueuedHttpRequestAddEvent::PreProcess. SessionName is empty." );
		return false;
	}
...
void FHttpNetworkReplayStreamer::AddOrUpdateEvent( const FString& Name, const uint32 TimeInMS, const FString& Group, const FString& Meta, const TArray<uint8>& Data )
{
	...
	{
		UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::AddOrUpdateEvent. Not streaming." );
		return;
	}

	// Upload a custom event to replay server
	UE_LOGF( LogHttpReplay, Verbose, "FHttpNetworkReplayStreamer::AddEvent. Size: %i, StreamChunkIndex: %i", Data.Num(), StreamChunkIndex );
...
void FHttpNetworkReplayStreamer::AddEvent( const uint32 TimeInMS, const FString& Group, const FString& Meta, const TArray<uint8>& Data )
{
	if (StreamerState != EReplayStreamerState::Recording && StreamerState != EReplayStreamerState::Playback)
	{
		UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::AddEvent. Not streaming." );
		return;
	}
...
void FHttpNetworkReplayStreamer::ConditionallyDownloadNextChunk()
{
	...
	if ( !bReallyNeedToDownloadChunk )
	{
		...
		if ( DownloadElapsedTime < MAX_WAIT_FOR_NEXT_CHUNK_IN_SECONDS && StreamTimeRangeEnd > StreamTimeRangeStart && StreamArchive.Buffer.Num() > 0 )
		{
			...
			if ( TimeLeft > MAX_BUFFERED_TIME )
			{
				...
				UE_LOGF( LogHttpReplay, VeryVerbose, "FHttpNetworkReplayStreamer::ConditionallyDownloadNextChunk. Cancelling due buffer being large enough. TotalStreamTime: %2.2f, PercentIn: %2.2f, TimeLeft: %2.2f", TotalStreamTime, PercentIn, TimeLeft );
				return;
			}
		}
	}
	...
	UE_LOGF( LogHttpReplay, Verbose, "FHttpNetworkReplayStreamer::ConditionallyDownloadNextChunk. URL: %ls", *URL );
...
void FHttpNetworkReplayStreamer::KeepReplay( const FString& ReplayName, const bool bKeep, const FKeepReplayCallback& Delegate )
{
	if ( ServerURL.IsEmpty() )
	{
		UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::KeepReplay. ServerURL is empty, aborting." );
...
{
	UE_LOGF(LogHttpReplay, Log, "FHttpNetworkReplayStreamer::RenameReplayFriendlyName is currently unsupported.");
...
{
	UE_LOGF(LogHttpReplay, Log, "FHttpNetworkReplayStreamer::RenameReplay is currently unsupported.");
	FRenameReplayResult Result;
	Result.Result = EStreamingOperationResult::Unsupported;
	Delegate.Execute(Result);
}
...
void FHttpNetworkReplayStreamer::KeepReplayFinished( FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool bSucceeded, FKeepReplayCallback Delegate, FString ReplayName )
{
	...
	{
		UE_LOGF( LogHttpReplay, Error, "FHttpNetworkReplayStreamer::KeepReplayFinished. FAILED, Response code: %d", HttpResponse.IsValid() ? HttpResponse->GetResponseCode() : 0 );
	}
	else
	{
		Result.Result = EStreamingOperationResult::Success;
...
void FHttpNetworkReplayStreamer::RefreshViewer( const bool bFinal )
{
	if ( ServerURL.IsEmpty() )
	{
		UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::RefreshViewer. ServerURL is empty, aborting." );
		return;
	}
...
{
	UE_LOGF(LogHttpReplay, Log, "FHttpNetworkReplayStreamer::DeleteFinishedStream is currently unsupported.");
...
void FHttpNetworkReplayStreamer::EnumerateStreams( const FNetworkReplayVersion& InReplayVersion, const int32 UserIndex, const FString& MetaString, const TArray< FString >& ExtraParms, const FEnumerateStreamsCallback& Delegate )
{
	if ( ServerURL.IsEmpty() )
	{
		UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::EnumerateStreams. ServerURL is empty, aborting." );
...
void FHttpNetworkReplayStreamer::EnumerateRecentStreams( const FNetworkReplayVersion& InReplayVersion, const int32 UserIndex, const FEnumerateStreamsCallback& Delegate )
{
	if ( ServerURL.IsEmpty() )
	{
		UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::EnumerateRecentStreams. ServerURL is empty, aborting." );
		Delegate.ExecuteIfBound( FEnumerateStreamsResult() );
		return;
	}
...
bool FQueuedHttpRequestAddUser::PreProcess( FHttpNetworkReplayStreamer* Streamer, const FString& ServerURL, const FString& SessionName )
{
	if ( SessionName.IsEmpty() )
	{
		UE_LOGF( LogHttpReplay, Warning, "FQueuedHttpRequestAddUser::PreProcess. SessionName is empty." );
		return false;
	}
...
void FHttpNetworkReplayStreamer::AddUserToReplay(const FString& UserString)
{
	...
	if ( UserString.IsEmpty() )
	{
		UE_LOGF(LogHttpReplay, Log, "FHttpNetworkReplayStreamer::AddUserToReplay: can't add a user with an empty UserString.");
		return;
	}
...
void FHttpNetworkReplayStreamer::HttpStartUploadingFinished( FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool bSucceeded )
{
	...
	if ( bSucceeded && HttpResponse->GetResponseCode() == EHttpResponseCodes::Ok )
	{
		...
		{
			UE_LOGF(LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::HttpStartUploadingFinished. FromJson FAILED");
		...
		UE_LOGF( LogHttpReplay, Log, "FHttpNetworkReplayStreamer::HttpStartUploadingFinished. SessionName: %ls", *SessionName );
	...
	else
	{
		...
		UE_LOGF( LogHttpReplay, Error, "FHttpNetworkReplayStreamer::HttpStartUploadingFinished. FAILED, %ls", *BuildRequestErrorString( HttpRequest, HttpResponse ) );
...
void FHttpNetworkReplayStreamer::HttpStopUploadingFinished( FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool bSucceeded )
{
	...
	if ( bSucceeded && HttpResponse->GetResponseCode() == EHttpResponseCodes::NoContent )
	{
		UE_LOGF( LogHttpReplay, Log, "FHttpNetworkReplayStreamer::HttpStopUploadingFinished. SessionName: %ls", *SessionName );
	}
	else
	{
		if ( RetryRequest( SavedFlightHttpRequest, HttpResponse ) )
		...
		UE_LOGF( LogHttpReplay, Error, "FHttpNetworkReplayStreamer::HttpStopUploadingFinished. FAILED, %ls", *BuildRequestErrorString( HttpRequest, HttpResponse ) );
		SetLastError(EHttpReplayResult::InvalidHttpResponse);
	}
...
void FHttpNetworkReplayStreamer::HttpHeaderUploadFinished( FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool bSucceeded )
{
	...
	if ( bSucceeded && HttpResponse->GetResponseCode() == EHttpResponseCodes::NoContent )
	{
		...
		UE_LOGF( LogHttpReplay, Verbose, "FHttpNetworkReplayStreamer::HttpHeaderUploadFinished. TotalUploadBytes: %llu", TotalUploadBytes );
		Result.Result = EStreamingOperationResult::Success;
	}
	else
	{
		...
		UE_LOGF( LogHttpReplay, Error, "FHttpNetworkReplayStreamer::HttpHeaderUploadFinished. FAILED, %ls", *BuildRequestErrorString( HttpRequest, HttpResponse ) );
		SetLastError(EHttpReplayResult::InvalidHttpResponse);
	}
...
void FHttpNetworkReplayStreamer::HttpUploadStreamFinished( FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool bSucceeded )
{
	...
	if ( bSucceeded && HttpResponse->GetResponseCode() == EHttpResponseCodes::NoContent )
	{
		UE_LOGF( LogHttpReplay, Verbose, "FHttpNetworkReplayStreamer::HttpUploadStreamFinished." );
	}
	else
	{
		if ( RetryRequest( SavedFlightHttpRequest, HttpResponse ) )
		...
		UE_LOGF( LogHttpReplay, Error, "FHttpNetworkReplayStreamer::HttpUploadStreamFinished. FAILED, %ls", *BuildRequestErrorString( HttpRequest, HttpResponse ) );
		SetLastError(EHttpReplayResult::InvalidHttpResponse);
	}
}
...
void FHttpNetworkReplayStreamer::HttpUploadCheckpointFinished( FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool bSucceeded )
{
	...
	if ( bSucceeded && ( HttpResponse->GetResponseCode() == EHttpResponseCodes::Ok || HttpResponse->GetResponseCode() == EHttpResponseCodes::NoContent ) )
	{
		...
		UE_LOGF( LogHttpReplay, Verbose, "FHttpNetworkReplayStreamer::HttpUploadCheckpointFinished. TotalUploadBytes: %llu", TotalUploadBytes );
	}
	else
	{
		if ( RetryRequest( SavedFlightHttpRequest, HttpResponse ) )
		...
		UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::HttpUploadCheckpointFinished. FAILED, %ls", *BuildRequestErrorString( HttpRequest, HttpResponse ) );
		SetLastError(EHttpReplayResult::InvalidHttpResponse);
	}
}
...
void FHttpNetworkReplayStreamer::HttpUploadCustomEventFinished(FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool bSucceeded)
{
	...
	if ( bSucceeded && ( HttpResponse->GetResponseCode() == EHttpResponseCodes::Ok || HttpResponse->GetResponseCode() == EHttpResponseCodes::NoContent ) )
	{
		...
		UE_LOGF( LogHttpReplay, Verbose, "FHttpNetworkReplayStreamer::HttpUploadCustomEventFinished. TotalUploadBytes: %llu", TotalUploadBytes );
	}
	else
	{
		UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::HttpUploadCustomEventFinished. FAILED. Extra info: %ls", *BuildRequestErrorString( HttpRequest, HttpResponse ) );
	}
}
...
void FHttpNetworkReplayStreamer::HttpStartDownloadingFinished( FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool bSucceeded )
{
	...
	if ( bSucceeded && HttpResponse->GetResponseCode() == EHttpResponseCodes::Ok )
	{
		...
		{
			UE_LOGF(LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::HttpStartDownloadingFinished. FromJson FAILED");
			// FIXME: Is there more I should do here?
			return;
		}
		...
		UE_LOGF( LogHttpReplay, Log, "FHttpNetworkReplayStreamer::HttpStartDownloadingFinished. Viewer: %ls, State: %ls, NumChunks: %i, DemoTime: %2.2f", *ViewerName, *State, NumTotalStreamChunks, (float)TotalDemoTimeInMS / 1000 );

		if ( NumTotalStreamChunks == 0 )
		{			
			UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::HttpStartDownloadingFinished. NO CHUNKS" );
	...
	{
		UE_LOGF( LogHttpReplay, Error, "FHttpNetworkReplayStreamer::HttpStartDownloadingFinished. FAILED, %ls", *BuildRequestErrorString( HttpRequest, HttpResponse ) );
...
void FHttpNetworkReplayStreamer::HttpDownloadHeaderFinished( FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool bSucceeded, FDownloadHeaderCallback Delegate)
{
	...
	if ( bSucceeded && HttpResponse->GetResponseCode() == EHttpResponseCodes::Ok )
	{
		...
		UE_LOGF( LogHttpReplay, Log, "FHttpNetworkReplayStreamer::HttpDownloadHeaderFinished. Size: %i", HeaderArchive.Buffer.Num() );
	}
	else
	{
		// FAIL
		UE_LOGF( LogHttpReplay, Error, "FHttpNetworkReplayStreamer::HttpDownloadHeaderFinished. FAILED, %ls", *BuildRequestErrorString( HttpRequest, HttpResponse ) );
...
bool FHttpNetworkReplayStreamer::DecompressResponse(FHttpResponsePtr HttpResponse, TArray<uint8>& ResultBuffer) const
{
	if ( SupportsCompression() )
	{
		...
		{
			ResultBuffer.Empty();
			UE_LOGF( LogHttpReplay, Error, "FHttpNetworkReplayStreamer::DecompressResponse. DecompressBuffer FAILED." );
			return false;
		}
...
void FHttpNetworkReplayStreamer::HttpDownloadFinished( FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool bSucceeded, int32 RequestedStreamChunkIndex, bool bStreamWasLive )
{
	...
	if ( bSucceeded && HttpResponse->GetResponseCode() == EHttpResponseCodes::Ok )
	{
		...
		{
			UE_LOGF( LogHttpReplay, Error, "FHttpNetworkReplayStreamer::HttpDownloadFinished. StreamChunkIndex changed while request was in flight" );
			StreamArchive.Buffer.Empty();
		...
		if ( HttpResponse->GetHeader( TEXT( "NumChunks" ) ) == TEXT( "" ) )
		{
			...
			UE_LOGF( LogHttpReplay, Log, "FHttpNetworkReplayStreamer::HttpDownloadFinished. NO HEADER FIELDS. Live: %i, Progress: %i / %i, Start: %i, End: %i, DemoTime: %2.2f", ( int )bStreamIsLive, StreamChunkIndex, NumTotalStreamChunks, ( int )StreamTimeRangeStart, ( int )StreamTimeRangeEnd, ( float )TotalDemoTimeInMS / 1000 );
			return;
		}
		...
		if ( HttpResponse->GetContent().Num() > 0 || bStreamIsLive )
		{
			...
			else if ( HighPriorityEndTime != 0 )
			{
				...
				UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::HttpDownloadFinished. HighPriorityEndTime != 0. Live: %i, Progress: %i / %i, Start: %i, End: %i, DemoTime: %2.2f", ( int )bStreamIsLive, StreamChunkIndex, NumTotalStreamChunks, ( int )StreamTimeRangeStart, ( int )StreamTimeRangeEnd, ( float )TotalDemoTimeInMS / 1000 );
				HighPriorityEndTime = 0;
			}

			UE_LOGF( LogHttpReplay, Verbose, "FHttpNetworkReplayStreamer::HttpDownloadFinished. Live: %i, Progress: %i / %i, Start: %i, End: %i, DemoTime: %2.2f", (int)bStreamIsLive, StreamChunkIndex, NumTotalStreamChunks, (int)StreamTimeRangeStart, (int)StreamTimeRangeEnd, (float)TotalDemoTimeInMS / 1000 );
		}
		else
		{
	...
	else
	{
		if ( bStreamIsLive )
		{
			...
			UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::HttpDownloadFinished. Failed live, turning off live flag. Response code: %d, Live: %i, Progress: %i / %i, Start: %i, End: %i, DemoTime: %2.2f", HttpResponse.IsValid() ? HttpResponse->GetResponseCode() : 0, ( int )bStreamIsLive, StreamChunkIndex, NumTotalStreamChunks, ( int )StreamTimeRangeStart, ( int )StreamTimeRangeEnd, ( float )TotalDemoTimeInMS / 1000 );
		}
		else
		{
			UE_LOGF( LogHttpReplay, Error, "FHttpNetworkReplayStreamer::HttpDownloadFinished. FAILED, %ls", *BuildRequestErrorString( HttpRequest, HttpResponse ) );
			StreamArchive.Buffer.Empty();
			SetLastError(EHttpReplayResult::InvalidHttpResponse);
		}
	}
...
{
	UE_LOGF( LogHttpReplay, Verbose, "FHttpNetworkReplayStreamer::HttpDownloadCheckpointFinished." );
	...
	if ( bSucceeded && HttpResponse->GetResponseCode() == EHttpResponseCodes::Ok )
	{
		if ( HttpResponse->GetContent().Num() == 0 )
		{
			UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::HttpDownloadCheckpointFinished. Checkpoint empty." );
			GotoCheckpointDelegate.ExecuteIfBound(FGotoResult());
			GotoCheckpointDelegate = FGotoCallback();
			return;
		}
		...
		UE_LOGF( LogHttpReplay, Verbose, "FHttpNetworkReplayStreamer::HttpDownloadCheckpointFinished. SUCCESS. StreamChunkIndex: %i", StreamChunkIndex );
	}
	else
	{
		// Oops, something went wrong, notify game code of failure
		UE_LOGF( LogHttpReplay, Error, "FHttpNetworkReplayStreamer::HttpDownloadCheckpointFinished. FAILED, %ls", *BuildRequestErrorString( HttpRequest, HttpResponse ) );
		GotoCheckpointDelegate.ExecuteIfBound(FGotoResult());
	}
...
void FHttpNetworkReplayStreamer::HttpRefreshViewerFinished( FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool bSucceeded )
{
	...
	if ( !bSucceeded || HttpResponse->GetResponseCode() != EHttpResponseCodes::NoContent )
	{
		...
		{
			UE_LOGF( LogHttpReplay, Error, "FHttpNetworkReplayStreamer::HttpRefreshViewerFinished. FAILED, %ls", *BuildRequestErrorString( HttpRequest, HttpResponse ) );
			SetLastError(EHttpReplayResult::InvalidHttpResponse);
		}
		else
		{
			const int32 RetriesLeft = MaxViewerRefreshFails - RefreshViewerFails;
			UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::HttpRefreshViewerFinished. Failed. Retries left: %i, %ls", RetriesLeft, *BuildRequestErrorString( HttpRequest, HttpResponse ) );
		}
	}
	else
	{
...
void FHttpNetworkReplayStreamer::HttpEnumerateSessionsFinished( FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool bSucceeded, FEnumerateStreamsCallback Delegate )
{
	...
	if ( bSucceeded && HttpResponse->GetResponseCode() == EHttpResponseCodes::Ok )
	{		
		UE_LOGF( LogHttpReplay, Verbose, "FHttpNetworkReplayStreamer::HttpEnumerateSessionsFinished." );
		...
		{
			UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::HttpEnumerateSessionsFinished. FromJson FAILED" );
		}
		else
		{
			Result.Result = EStreamingOperationResult::Success;
	...
	{
		UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::HttpEnumerateSessionsFinished. FAILED, %ls", *BuildRequestErrorString( HttpRequest, HttpResponse ) );
	}
...
void FHttpNetworkReplayStreamer::HttpEnumerateCheckpointsFinished( FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool bSucceeded )
{
	...
	if ( bSucceeded && HttpResponse->GetResponseCode() == EHttpResponseCodes::Ok )
	{		
		UE_LOGF( LogHttpReplay, Verbose, "FHttpNetworkReplayStreamer::HttpEnumerateCheckpointsFinished." );
		...
		{
			UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::HttpEnumerateCheckpointsFinished. FromJson FAILED" );
	...
	{
		UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::HttpEnumerateCheckpointsFinished. FAILED, %ls", *BuildRequestErrorString( HttpRequest, HttpResponse ) );
		SetLastError(EHttpReplayResult::InvalidHttpResponse);
	}
...
void FHttpNetworkReplayStreamer::HttpEnumerateEventsFinished(FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool bSucceeded, FEnumerateEventsCallback Delegate)
{
	...
	if (bSucceeded && HttpResponse->GetResponseCode() == EHttpResponseCodes::Ok)
	{
		...
		{
			UE_LOGF(LogHttpReplay, Verbose, "FHttpNetworkReplayStreamer::HttpEnumerateEventsFinished. %ls", *JsonString);
			Result.Result = EStreamingOperationResult::Success;
		}
		else
		{
			UE_LOGF(LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::HttpEnumerateEventsFinished. FromJson FAILED");
			SetLastError(EHttpReplayResult::FailedJsonParse);
		}
	}
	else
	{
		UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::HttpEnumerateEventsFinished. FAILED, %ls", *BuildRequestErrorString( HttpRequest, HttpResponse ) );
		SetLastError(EHttpReplayResult::InvalidHttpResponse);
	}
...
void FHttpNetworkReplayStreamer::HttpAddUserFinished(FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool bSucceeded)
{
	...
	if ( bSucceeded && HttpResponse->GetResponseCode() == EHttpResponseCodes::NoContent )
	{		
		UE_LOGF( LogHttpReplay, Verbose, "FHttpNetworkReplayStreamer::HttpAddUserFinished." );
	}
	else
	{
		// Don't consider this a fatal error
		UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::HttpAddUserFinished. FAILED, %ls", *BuildRequestErrorString( HttpRequest, HttpResponse ) );
	}
}
...
bool FHttpNetworkReplayStreamer::ProcessNextHttpRequest()
{
	...
	if ( QueuedHttpRequests.Num() > 0 )
	{
		...
		UE_LOGF( LogHttpReplay, Verbose, "FHttpNetworkReplayStreamer::ProcessNextHttpRequest. Dequeue Type: %ls", EQueuedHttpRequestType::ToString( QueuedRequest->Type ) );
...
{
	UE_LOGF( LogHttpReplay, Verbose, "FHttpNetworkReplayStreamer::HttpDownloadCheckpointDeltaFinished." );
	...
	if ( bSucceeded && HttpResponse->GetResponseCode() == EHttpResponseCodes::Ok )
	{
		if ( HttpResponse->GetContent().Num() == 0 )
		{
			UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamer::HttpDownloadCheckpointDeltaFinished. Checkpoint empty." );
			GotoCheckpointDelegate.ExecuteIfBound(FGotoResult());
			GotoCheckpointDelegate = FGotoCallback();
			DownloadCheckpointIndex = -1;
			DeltaDownloadCheckpointIndex = -1;
		...
		else
		{
			...
			UE_LOGF( LogHttpReplay, Verbose, "FHttpNetworkReplayStreamer::HttpDownloadCheckpointDeltaFinished. SUCCESS. StreamChunkIndex: %i", StreamChunkIndex );
		}
	}
	else
	{
		// Oops, something went wrong, notify game code of failure
		UE_LOGF( LogHttpReplay, Error, "FHttpNetworkReplayStreamer::HttpDownloadCheckpointDeltaFinished. FAILED, %ls", *BuildRequestErrorString( HttpRequest, HttpResponse ) );
		GotoCheckpointDelegate.ExecuteIfBound(FGotoResult());
	}
...
void FHttpNetworkReplayStreamingFactory::Tick( float DeltaTime )
{
	for ( int i = HttpStreamers.Num() - 1; i >= 0; i-- )
	{
		...
		if ( HttpStreamers[i].IsUnique() && !HttpStreamers[i]->HasPendingHttpRequests() )
		{
			...
			{
				UE_LOGF( LogHttpReplay, Warning, "FHttpNetworkReplayStreamingFactory::Tick. Stream was stopped early." );
			}
...
void FHttpNetworkReplayStreamingFactory::Flush()
{
	...
	if (bFlushStreamersOnShutdown)
	{
		...
		while (HasPendingHttpRequests())
		{
			...
			{
				UE_LOGF(LogHttpReplay, Display, "Abandoning streamer flush after waiting %0.2f seconds", TotalWait);
				break;
			}
			...
			if (HasPendingHttpRequests())
			{
				...
				{
					UE_LOGF(LogHttpReplay, Display, "Sleeping to wait for outstanding requests.");
