...
template <>
inline TEnableIf<!TIsBulkSerializable<FName>::Value, FArchive>::Type& operator<<( FArchive& Ar, TMeshAttributeArrayBase<FName>& Array )
{
	if (Ar.CustomVer(FFortniteMainBranchObjectVersion::GUID) < FFortniteMainBranchObjectVersion::DynamicMeshAttributesTriangleLabels)
	{		
		// see non-specialized implementation
		if (Ar.IsLoading() &&
		   Ar.CustomVer(FReleaseObjectVersion::GUID) != FReleaseObjectVersion::MeshDescriptionNewFormat &&
		   Ar.CustomVer(FUE5MainStreamObjectVersion::GUID) < FUE5MainStreamObjectVersion::MeshDescriptionNewFormat)
		{
	...
	TArray<FName>& Names = Array.Container;
	...
	if (Ar.IsLoading())
	{
		...
		if (NumUniqueNames == 1)
		{
			Names.Init(UniqueNames[0], NumNames);
			return Ar;
		}
		
		// all names are unique 
		if (NumUniqueNames == NumNames)
		{
			Names = MoveTemp(UniqueNames);
			return Ar;
		}
		
		Names.Init(NAME_None, NumNames);
		...
		for (uint32 Index = 0; Index < NumNames; ++Index)
		{
			...
			{
				Names[Index] = UniqueNames[NameIndex];
			}
		}
	}
	else
