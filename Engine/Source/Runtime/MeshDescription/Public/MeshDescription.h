...
public:
	...
	MESHDESCRIPTION_API static TConstArrayView<TPair<FGuid, int32>> GetSerializationFormatVersions();
...
private:
	...
	friend struct FMeshDescriptionBulkData; 
};
...
{
public:
	FMeshDescriptionBulkData() = default;
	...
	/** Return unique ID string for this bulk data that does not include versioning info. 
	 *  Use this for constructing IDs that do not rely on the serialization format of 
	 *  FMeshDescription but is only dependent on its contents, such as storing intermediate
	 *  representations build from the FMeshDescription data in the DDC.
	 */
	FString GetUnversionedIdString() const
	{
		return GetIdStringFromVersionList(GetFrozenVersions());
	}

	/** Return unique ID string for this bulk data that also includes current versioning info for
	 *  FMeshDescription serialization. 
	 *  Use this for constructing IDs that are required for serializing the FMeshDescription
	 *  bulk data to disk, such as storing FMeshDescription archives in a shared DDC storage.
	 */
	FString GetVersionedIdString() const
	{
		return GetIdStringFromVersionList(FMeshDescription::GetSerializationFormatVersions());
	}

	UE_DEPRECATED(5.8, "Use GetVersionedIdString or GetUnversionedIdString instead, depending on requirements")
	FString GetIdString() const
	{
		return GetUnversionedIdString();
	}
	
	/** Return an ID string that represents the serialization version for FMeshDescription based on the
	 *  live custom versions. This differs from GetVersionedIdString() in that it does not include
	 *  the content hash.
	 */
	static MESHDESCRIPTION_API FString GetSerializationVersionIdString();
	...
	/** Returns the CustomVersions GUIDs that are used when serializing an FMeshDescription. */
	static MESHDESCRIPTION_API TConstArrayView<FGuid> GetMeshDescriptionCustomVersions();
	
private:
	void UpdateMeshDescriptionFormat();
	
	MESHDESCRIPTION_API FString GetIdStringFromVersionList(TConstArrayView<TPair<FGuid, int32>> InVersionList) const;

	static MESHDESCRIPTION_API TConstArrayView<TPair<FGuid, int32>> GetFrozenVersions();
	...
	int32 LicenseeUEVersion = INDEX_NONE;
