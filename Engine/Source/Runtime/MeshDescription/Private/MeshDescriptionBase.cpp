...
void UMeshDescriptionBase::CreateVertexWithID(FVertexID VertexID)
{
	...
	{
		UE_LOGF(LogMeshDescription, Warning, "CreateVertexWithID: VertexID %d already exists.", VertexID.GetValue());
	}
}
...
void UMeshDescriptionBase::DeleteVertex(FVertexID VertexID)
{
	...
	{
		UE_LOGF(LogMeshDescription, Warning, "DeleteVertex: VertexID %d doesn't exist.", VertexID.GetValue());
	}
}
...
FVertexInstanceID UMeshDescriptionBase::CreateVertexInstance(FVertexID VertexID)
{
	...
	{
		UE_LOGF(LogMeshDescription, Warning, "CreateVertexInstance: VertexID %d doesn't exist.", VertexID.GetValue());
		return INDEX_NONE;
	}
}
...
void UMeshDescriptionBase::CreateVertexInstanceWithID(FVertexInstanceID VertexInstanceID, FVertexID VertexID)
{
	if (GetMeshDescription().IsVertexValid(VertexID))
	{
		...
		{
			UE_LOGF(LogMeshDescription, Warning, "CreateVertexInstanceWithID: VertexInstanceID %d already exists.", VertexInstanceID.GetValue());
		}
	}
	else
	{
		UE_LOGF(LogMeshDescription, Warning, "CreateVertexInstanceWithID: VertexID %d doesn't exist.", VertexID.GetValue());
	}
}
...
void UMeshDescriptionBase::DeleteVertexInstance(FVertexInstanceID VertexInstanceID, TArray<FVertexID>& OrphanedVertices)
{
	...
	{
		UE_LOGF(LogMeshDescription, Warning, "DeleteVertexInstance: VertexInstanceID %d doesn't exist.", VertexInstanceID.GetValue());
	}
}
...
FEdgeID UMeshDescriptionBase::CreateEdge(FVertexID VertexID0, FVertexID VertexID1)
{
	...
	{
		UE_LOGF(LogMeshDescription, Warning, "CreateEdge: VertexID %d doesn't exist.", VertexID0.GetValue());
		return INDEX_NONE;
	}

	if (!GetMeshDescription().IsVertexValid(VertexID1))
	{
		UE_LOGF(LogMeshDescription, Warning, "CreateEdge: VertexID %d doesn't exist.", VertexID1.GetValue());
		return INDEX_NONE;
	}
...
void UMeshDescriptionBase::CreateEdgeWithID(FEdgeID EdgeID, FVertexID VertexID0, FVertexID VertexID1)
{
	...
	{
		UE_LOGF(LogMeshDescription, Warning, "CreateEdgeWithID: EdgeID %d already exists.", EdgeID.GetValue());
		return;
	}

	if (!GetMeshDescription().IsVertexValid(VertexID0))
	{
		UE_LOGF(LogMeshDescription, Warning, "CreateEdgeWithID: VertexID %d doesn't exist.", VertexID0.GetValue());
		return;
	}

	if (!GetMeshDescription().IsVertexValid(VertexID1))
	{
		UE_LOGF(LogMeshDescription, Warning, "CreateEdgeWithID: VertexID %d doesn't exist.", VertexID1.GetValue());
		return;
	}
...
void UMeshDescriptionBase::DeleteEdge(FEdgeID EdgeID, TArray<FVertexID>& OrphanedVertices)
{
	...
	{
		UE_LOGF(LogMeshDescription, Warning, "DeleteEdge: EdgeID %d doesn't exist.", EdgeID.GetValue());
	}
}
...
FTriangleID UMeshDescriptionBase::CreateTriangle(FPolygonGroupID PolygonGroupID, const TArray<FVertexInstanceID>& VertexInstanceIDs, TArray<FEdgeID>& NewEdgeIDs)
{
	...
	{
		UE_LOGF(LogMeshDescription, Warning, "CreateTriangle: PolygonGroupID %d doesn't exist.", PolygonGroupID.GetValue());
		return INDEX_NONE;
	}
...
void UMeshDescriptionBase::CreateTriangleWithID(FTriangleID TriangleID, FPolygonGroupID PolygonGroupID, const TArray<FVertexInstanceID>& VertexInstanceIDs, TArray<FEdgeID>& NewEdgeIDs)
{
	...
	{
		UE_LOGF(LogMeshDescription, Warning, "CreateTriangleWithID: TriangleID %d already exists.", TriangleID.GetValue());
		return;
	}

	if (!GetMeshDescription().IsPolygonGroupValid(PolygonGroupID))
	{
		UE_LOGF(LogMeshDescription, Warning, "CreateTriangleWithID: PolygonGroupID %d doesn't exist.", PolygonGroupID.GetValue());
		return;
	}
...
void UMeshDescriptionBase::DeleteTriangle(FTriangleID TriangleID, TArray<FEdgeID>& OrphanedEdges, TArray<FVertexInstanceID>& OrphanedVertexInstances, TArray<FPolygonGroupID>& OrphanedPolygonGroups)
{
	if (!GetMeshDescription().IsTriangleValid(TriangleID))
	{
		UE_LOGF(LogMeshDescription, Warning, "DeleteTriangle: TriangleID %d doesn't exist.", TriangleID.GetValue());
		return;
	}
...
FPolygonID UMeshDescriptionBase::CreatePolygon(FPolygonGroupID PolygonGroupID, TArray<FVertexInstanceID>& VertexInstanceIDs, TArray<FEdgeID>& NewEdgeIDs)
{
	if (!GetMeshDescription().IsPolygonGroupValid(PolygonGroupID))
	{
		UE_LOGF(LogMeshDescription, Warning, "CreatePolygon: PolygonGroupID %d doesn't exist.", PolygonGroupID.GetValue());
		return INDEX_NONE;
	}
...
void UMeshDescriptionBase::CreatePolygonWithID(FPolygonID PolygonID, FPolygonGroupID PolygonGroupID, TArray<FVertexInstanceID>& VertexInstanceIDs, TArray<FEdgeID>& NewEdgeIDs)
{
	...
	{
		UE_LOGF(LogMeshDescription, Warning, "CreatePolygonWithID: PolygonID %d already exists.", PolygonID.GetValue());
		return;
	}

	if (!GetMeshDescription().IsPolygonGroupValid(PolygonGroupID))
	{
		UE_LOGF(LogMeshDescription, Warning, "CreatePolygonWithID: PolygonGroupID %d doesn't exist.", PolygonGroupID.GetValue());
		return;
	}
...
void UMeshDescriptionBase::DeletePolygon(FPolygonID PolygonID, TArray<FEdgeID>& OrphanedEdges, TArray<FVertexInstanceID>& OrphanedVertexInstances, TArray<FPolygonGroupID>& OrphanedPolygonGroups)
{
	if (!GetMeshDescription().IsPolygonValid(PolygonID))
	{
		UE_LOGF(LogMeshDescription, Warning, "DeletePolygon: PolygonID %d doesn't exist.", PolygonID.GetValue());
		return;
	}
...
void UMeshDescriptionBase::CreatePolygonGroupWithID(FPolygonGroupID PolygonGroupID)
{
	if (GetMeshDescription().IsPolygonGroupValid(PolygonGroupID))
	{
		UE_LOGF(LogMeshDescription, Warning, "CreatePolygonGroupWithID: PolygonGroupID %d already exists.", PolygonGroupID.GetValue());
		return;
	}
...
void UMeshDescriptionBase::DeletePolygonGroup(FPolygonGroupID PolygonGroupID)
{
	if (!GetMeshDescription().IsPolygonGroupValid(PolygonGroupID))
	{
		UE_LOGF(LogMeshDescription, Warning, "DeletePolygonGroup: FPolygonGroupID %d doesn't exist.", PolygonGroupID.GetValue());
		return;
	}
...
bool UMeshDescriptionBase::IsVertexOrphaned(FVertexID VertexID) const
{
	if (!GetMeshDescription().IsVertexValid(VertexID))
	{
		UE_LOGF(LogMeshDescription, Warning, "IsVertexOrphaned: VertexID %d doesn't exist.", VertexID.GetValue());
		return false;
	}
...
FEdgeID UMeshDescriptionBase::GetVertexPairEdge(FVertexID VertexID0, FVertexID VertexID1) const
{
	if (!GetMeshDescription().IsVertexValid(VertexID0))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetVertexPairEdge: VertexID %d doesn't exist.", VertexID0.GetValue());
		return INDEX_NONE;
	}

	if (!GetMeshDescription().IsVertexValid(VertexID1))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetVertexPairEdge: VertexID %d doesn't exist.", VertexID1.GetValue());
		return INDEX_NONE;
	}
...
void UMeshDescriptionBase::GetVertexConnectedEdges(FVertexID VertexID, TArray<FEdgeID>& OutEdgeIDs) const
{
	if (!GetMeshDescription().IsVertexValid(VertexID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetVertexConnectedEdges: VertexID %d doesn't exist.", VertexID.GetValue());
		return;
	}
...
int32 UMeshDescriptionBase::GetNumVertexConnectedEdges(FVertexID VertexID) const
{
	if (!GetMeshDescription().IsVertexValid(VertexID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetNumVertexConnectedEdges: VertexID %d doesn't exist.", VertexID.GetValue());
		return 0;
	}
...
void UMeshDescriptionBase::GetVertexVertexInstances(FVertexID VertexID, TArray<FVertexInstanceID>& OutVertexInstanceIDs) const
{
	if (!GetMeshDescription().IsVertexValid(VertexID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetVertexVertexInstances: VertexID %d doesn't exist.", VertexID.GetValue());
		return;
	}
...
int32 UMeshDescriptionBase::GetNumVertexVertexInstances(FVertexID VertexID) const
{
	if (!GetMeshDescription().IsVertexValid(VertexID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetNumVertexVertexInstances: VertexID %d doesn't exist.", VertexID.GetValue());
		return 0;
	}
...
void UMeshDescriptionBase::GetVertexConnectedTriangles(FVertexID VertexID, TArray<FTriangleID>& OutConnectedTriangleIDs) const
{
	if (!GetMeshDescription().IsVertexValid(VertexID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetVertexConnectedTriangles: VertexID %d doesn't exist.", VertexID.GetValue());
		return;
	}
...
int32 UMeshDescriptionBase::GetNumVertexConnectedTriangles(FVertexID VertexID) const
{
	if (!GetMeshDescription().IsVertexValid(VertexID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetNumVertexConnectedTriangles: VertexID %d doesn't exist.", VertexID.GetValue());
		return 0;
	}
...
void UMeshDescriptionBase::GetVertexConnectedPolygons(FVertexID VertexID, TArray<FPolygonID>& OutConnectedPolygonIDs) const
{
	if (!GetMeshDescription().IsVertexValid(VertexID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetVertexConnectedPolygons: VertexID %d doesn't exist.", VertexID.GetValue());
		return;
	}
...
int32 UMeshDescriptionBase::GetNumVertexConnectedPolygons(FVertexID VertexID) const
{
	if (!GetMeshDescription().IsVertexValid(VertexID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetNumVertexConnectedPolygons: VertexID %d doesn't exist.", VertexID.GetValue());
		return 0;
	}
...
void UMeshDescriptionBase::GetVertexAdjacentVertices(FVertexID VertexID, TArray<FVertexID>& OutAdjacentVertexIDs) const
{
	if (!GetMeshDescription().IsVertexValid(VertexID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetVertexAdjacentVertices: VertexID %d doesn't exist.", VertexID.GetValue());
		return;
	}
...
FVector UMeshDescriptionBase::GetVertexPosition(FVertexID VertexID) const
{
	if (!GetMeshDescription().IsVertexValid(VertexID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetVertexAttribute: VertexID %d doesn't exist.", VertexID.GetValue());
		return FVector::ZeroVector;
	}

	if (!GetMeshDescription().VertexAttributes().HasAttribute(MeshAttribute::Vertex::Position))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetVertexAttribute: VertexAttribute Position doesn't exist.");
		return FVector::ZeroVector;
	}
...
void UMeshDescriptionBase::SetVertexPosition(FVertexID VertexID, const FVector& Position)
{
	if (!GetMeshDescription().IsVertexValid(VertexID))
	{
		UE_LOGF(LogMeshDescription, Warning, "SetVertexAttribute: VertexID %d doesn't exist.", VertexID.GetValue());
		return;
	}

	if (!GetMeshDescription().VertexAttributes().HasAttribute(MeshAttribute::Vertex::Position))
	{
		UE_LOGF(LogMeshDescription, Warning, "SetVertexAttribute: VertexAttribute Position doesn't exist.");
		return;
	}
...
FVertexID UMeshDescriptionBase::GetVertexInstanceVertex(FVertexInstanceID VertexInstanceID) const
{
	if (!GetMeshDescription().IsVertexInstanceValid(VertexInstanceID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetVertexInstanceVertex: VertexInstanceID %d doesn't exist.", VertexInstanceID.GetValue());
		return INDEX_NONE;
	}
...
FEdgeID UMeshDescriptionBase::GetVertexInstancePairEdge(FVertexInstanceID VertexInstanceID0, FVertexInstanceID VertexInstanceID1) const
{
	...
	{
		UE_LOGF(LogMeshDescription, Warning, "GetVertexInstancePairEdge: VertexInstanceID %d doesn't exist.", VertexInstanceID0.GetValue());
		return INDEX_NONE;
	}

	if (!GetMeshDescription().IsVertexInstanceValid(VertexInstanceID1))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetVertexInstancePairEdge: VertexInstanceID %d doesn't exist.", VertexInstanceID1.GetValue());
		return INDEX_NONE;
	}
...
void UMeshDescriptionBase::GetVertexInstanceConnectedTriangles(FVertexInstanceID VertexInstanceID, TArray<FTriangleID>& OutConnectedTriangleIDs) const
{
	if (!GetMeshDescription().IsVertexInstanceValid(VertexInstanceID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetVertexInstanceConnectedTriangles: VertexInstanceID %d doesn't exist.", VertexInstanceID.GetValue());
		return;
	}
...
int32 UMeshDescriptionBase::GetNumVertexInstanceConnectedTriangles(FVertexInstanceID VertexInstanceID) const
{
	if (!GetMeshDescription().IsVertexInstanceValid(VertexInstanceID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetNumVertexInstanceConnectedTriangles: VertexInstanceID %d doesn't exist.", VertexInstanceID.GetValue());
		return 0;
	}
...
void UMeshDescriptionBase::GetVertexInstanceConnectedPolygons(FVertexInstanceID VertexInstanceID, TArray<FPolygonID>& OutConnectedPolygonIDs) const
{
	if (!GetMeshDescription().IsVertexInstanceValid(VertexInstanceID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetVertexInstanceConnectedPolygons: VertexInstanceID %d doesn't exist.", VertexInstanceID.GetValue());
		return;
	}
...
int32 UMeshDescriptionBase::GetNumVertexInstanceConnectedPolygons(FVertexInstanceID VertexInstanceID) const
{
	if (!GetMeshDescription().IsVertexInstanceValid(VertexInstanceID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetNumVertexInstanceConnectedPolygons: VertexInstanceID %d doesn't exist.", VertexInstanceID.GetValue());
		return 0;
	}
...
bool UMeshDescriptionBase::IsEdgeInternal(FEdgeID EdgeID) const
{
	if (!GetMeshDescription().IsEdgeValid(EdgeID))
	{
		UE_LOGF(LogMeshDescription, Warning, "IsEdgeInternal: EdgeID %d doesn't exist.", EdgeID.GetValue());
		return false;
	}
...
bool UMeshDescriptionBase::IsEdgeInternalToPolygon(FEdgeID EdgeID, FPolygonID PolygonID) const
{
	if (!GetMeshDescription().IsEdgeValid(EdgeID))
	{
		UE_LOGF(LogMeshDescription, Warning, "IsEdgeInternalToPolygon: EdgeID %d doesn't exist.", EdgeID.GetValue());
		return false;
	}

	if (!GetMeshDescription().IsPolygonValid(PolygonID))
	{
		UE_LOGF(LogMeshDescription, Warning, "IsEdgeInternalToPolygon: PolygonID %d doesn't exist.", PolygonID.GetValue());
		return false;
	}
...
void UMeshDescriptionBase::GetEdgeConnectedTriangles(FEdgeID EdgeID, TArray<FTriangleID>& OutConnectedTriangleIDs) const
{
	if (!GetMeshDescription().IsEdgeValid(EdgeID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetEdgeConnectedTriangles: EdgeID %d doesn't exist.", EdgeID.GetValue());
		return;
	}
...
int32 UMeshDescriptionBase::GetNumEdgeConnectedTriangles(FEdgeID EdgeID) const
{
	if (!GetMeshDescription().IsEdgeValid(EdgeID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetNumEdgeConnectedTriangles: EdgeID %d doesn't exist.", EdgeID.GetValue());
		return 0;
	}
...
void UMeshDescriptionBase::GetEdgeConnectedPolygons(FEdgeID EdgeID, TArray<FPolygonID>& OutConnectedPolygonIDs) const
{
	if (!GetMeshDescription().IsEdgeValid(EdgeID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetEdgeConnectedPolygons: EdgeID %d doesn't exist.", EdgeID.GetValue());
		return;
	}
...
int32 UMeshDescriptionBase::GetNumEdgeConnectedPolygons(FEdgeID EdgeID) const
{
	if (!GetMeshDescription().IsEdgeValid(EdgeID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetNumEdgeConnectedPolygons: EdgeID %d doesn't exist.", EdgeID.GetValue());
		return 0;
	}
...
FVertexID UMeshDescriptionBase::GetEdgeVertex(FEdgeID EdgeID, int32 VertexNumber) const
{
	if (!GetMeshDescription().IsEdgeValid(EdgeID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetEdgeVertex: EdgeID %d doesn't exist.", EdgeID.GetValue());
		return INDEX_NONE;
	}

	if (VertexNumber != 0 && VertexNumber != 1)
	{
		UE_LOGF(LogMeshDescription, Warning, "GetEdgeVertex: invalid vertex number %d.", VertexNumber);
		return INDEX_NONE;
	}
...
void UMeshDescriptionBase::GetEdgeVertices(const FEdgeID EdgeID, TArray<FVertexID>& OutVertexIDs) const
{
	if (!GetMeshDescription().IsEdgeValid(EdgeID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetEdgeVertices: EdgeID %d doesn't exist.", EdgeID.GetValue());
		return;
	}
...
FPolygonID UMeshDescriptionBase::GetTrianglePolygon(FTriangleID TriangleID) const
{
	if (!GetMeshDescription().IsTriangleValid(TriangleID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetTrianglePolygon: TriangleID %d doesn't exist.", TriangleID.GetValue());
		return INDEX_NONE;
	}
...
FPolygonGroupID UMeshDescriptionBase::GetTrianglePolygonGroup(FTriangleID TriangleID) const
{
	if (!GetMeshDescription().IsTriangleValid(TriangleID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetTrianglePolygonGroup: TriangleID %d doesn't exist.", TriangleID.GetValue());
		return INDEX_NONE;
	}
...
bool UMeshDescriptionBase::IsTrianglePartOfNgon(FTriangleID TriangleID) const
{
	if (!GetMeshDescription().IsTriangleValid(TriangleID))
	{
		UE_LOGF(LogMeshDescription, Warning, "IsTrianglePartOfNgon: TriangleID %d doesn't exist.", TriangleID.GetValue());
		return false;
	}
...
void UMeshDescriptionBase::GetTriangleVertexInstances(FTriangleID TriangleID, TArray<FVertexInstanceID>& OutVertexInstanceIDs) const
{
	if (!GetMeshDescription().IsTriangleValid(TriangleID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetTriangleVertexInstances: TriangleID %d doesn't exist.", TriangleID.GetValue());
		return;
	}
...
FVertexInstanceID UMeshDescriptionBase::GetTriangleVertexInstance(FTriangleID TriangleID, int32 Index) const
{
	if (!GetMeshDescription().IsTriangleValid(TriangleID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetTriangleVertexInstance: TriangleID %d doesn't exist.", TriangleID.GetValue());
		return INDEX_NONE;
	}

	if (Index < 0 || Index > 2)
	{
		UE_LOGF(LogMeshDescription, Warning, "GetTriangleVertexInstance: invalid vertex index %d.", Index);
		return INDEX_NONE;
	}
...
void UMeshDescriptionBase::GetTriangleVertices(FTriangleID TriangleID, TArray<FVertexID>& OutVertexIDs) const
{
	if (!GetMeshDescription().IsTriangleValid(TriangleID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetTriangleVertices: TriangleID %d doesn't exist.", TriangleID.GetValue());
		return;
	}
...
void UMeshDescriptionBase::GetTriangleEdges(FTriangleID TriangleID, TArray<FEdgeID>& OutEdgeIDs) const
{
	if (!GetMeshDescription().IsTriangleValid(TriangleID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetTriangleEdges: TriangleID %d doesn't exist.", TriangleID.GetValue());
		return;
	}
...
void UMeshDescriptionBase::GetTriangleAdjacentTriangles(FTriangleID TriangleID, TArray<FTriangleID>& OutTriangleIDs) const
{
	if (!GetMeshDescription().IsTriangleValid(TriangleID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetTriangleAdjacentTriangles: TriangleID %d doesn't exist.", TriangleID.GetValue());
		return;
	}
...
FVertexInstanceID UMeshDescriptionBase::GetVertexInstanceForTriangleVertex(FTriangleID TriangleID, FVertexID VertexID) const
{
	if (!GetMeshDescription().IsTriangleValid(TriangleID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetVertexInstanceForTriangleVertex: TriangleID %d doesn't exist.", TriangleID.GetValue());
		return INDEX_NONE;
	}
...
void UMeshDescriptionBase::GetPolygonTriangles(FPolygonID PolygonID, TArray<FTriangleID>& OutTriangleIDs) const
{
	if (!GetMeshDescription().IsPolygonValid(PolygonID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetPolygonTriangles: PolygonID %d doesn't exist.", PolygonID.GetValue());
		return;
	}
...
int32 UMeshDescriptionBase::GetNumPolygonTriangles(FPolygonID PolygonID) const
{
	if (!GetMeshDescription().IsPolygonValid(PolygonID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetNumPolygonTriangles: PolygonID %d doesn't exist.", PolygonID.GetValue());
		return 0;
	}
...
void UMeshDescriptionBase::GetPolygonVertexInstances(FPolygonID PolygonID, TArray<FVertexInstanceID>& OutVertexInstanceIDs) const
{
	if (!GetMeshDescription().IsPolygonValid(PolygonID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetPolygonVertexInstances: PolygonID %d doesn't exist.", PolygonID.GetValue());
		return;
	}
...
int32 UMeshDescriptionBase::GetNumPolygonVertices(FPolygonID PolygonID) const
{
	if (!GetMeshDescription().IsPolygonValid(PolygonID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetNumPolygonVertices: PolygonID %d doesn't exist.", PolygonID.GetValue());
		return 0;
	}
...
void UMeshDescriptionBase::GetPolygonVertices(FPolygonID PolygonID, TArray<FVertexID>& OutVertexIDs) const
{
	if (!GetMeshDescription().IsPolygonValid(PolygonID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetPolygonVertices: PolygonID %d doesn't exist.", PolygonID.GetValue());
		return;
	}
...
void UMeshDescriptionBase::GetPolygonPerimeterEdges(FPolygonID PolygonID, TArray<FEdgeID>& OutEdgeIDs) const
{
	if (!GetMeshDescription().IsPolygonValid(PolygonID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetPolygonPerimeterEdges: PolygonID %d doesn't exist.", PolygonID.GetValue());
		return;
	}
...
void UMeshDescriptionBase::GetPolygonInternalEdges(FPolygonID PolygonID, TArray<FEdgeID>& OutEdgeIDs) const
{
	if (!GetMeshDescription().IsPolygonValid(PolygonID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetPolygonInternalEdges: PolygonID %d doesn't exist.", PolygonID.GetValue());
		return;
	}
...
int32 UMeshDescriptionBase::GetNumPolygonInternalEdges(FPolygonID PolygonID) const
{
	if (!GetMeshDescription().IsPolygonValid(PolygonID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetNumPolygonInternalEdges: PolygonID %d doesn't exist.", PolygonID.GetValue());
		return 0;
	}
...
void UMeshDescriptionBase::GetPolygonAdjacentPolygons(FPolygonID PolygonID, TArray<FPolygonID>& OutPolygonIDs) const
{
	if (!GetMeshDescription().IsPolygonValid(PolygonID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetPolygonAdjacentPolygons: PolygonID %d doesn't exist.", PolygonID.GetValue());
		return;
	}
...
FPolygonGroupID UMeshDescriptionBase::GetPolygonPolygonGroup(FPolygonID PolygonID) const
{
	if (!GetMeshDescription().IsPolygonValid(PolygonID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetPolygonPolygonGroup: PolygonID %d doesn't exist.", PolygonID.GetValue());
		return INDEX_NONE;
	}
...
FVertexInstanceID UMeshDescriptionBase::GetVertexInstanceForPolygonVertex(FPolygonID PolygonID, FVertexID VertexID) const
{
	if (!GetMeshDescription().IsPolygonValid(PolygonID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetVertexInstanceForPolygonVertex: PolygonID %d doesn't exist.", PolygonID.GetValue());
		return INDEX_NONE;
	}
...
void UMeshDescriptionBase::SetPolygonVertexInstances(FPolygonID PolygonID, const TArray<FVertexInstanceID>& VertexInstanceIDs)
{
	if (!GetMeshDescription().IsPolygonValid(PolygonID))
	{
		UE_LOGF(LogMeshDescription, Warning, "SetPolygonVertexInstances: PolygonID %d doesn't exist.", PolygonID.GetValue());
		return;
	}
	...
	for (FVertexInstanceID VertexInstanceID : VertexInstanceIDs)
	{
		if (!GetMeshDescription().IsVertexInstanceValid(VertexInstanceID))
		{
			UE_LOGF(LogMeshDescription, Warning, "SetPolygonVertexInstances: VertexInstanceID %d doesn't exist.", VertexInstanceID.GetValue());
			return;
		}
	}
...
void UMeshDescriptionBase::SetPolygonPolygonGroup(FPolygonID PolygonID, FPolygonGroupID PolygonGroupID)
{
	if (!GetMeshDescription().IsPolygonValid(PolygonID))
	{
		UE_LOGF(LogMeshDescription, Warning, "SetPolygonPolygonGroup: PolygonID %d doesn't exist.", PolygonID.GetValue());
		return;
	}

	if (!GetMeshDescription().IsPolygonGroupValid(PolygonGroupID))
	{
		UE_LOGF(LogMeshDescription, Warning, "SetPolygonPolygonGroup: PolygonGroupID %d doesn't exist.", PolygonGroupID.GetValue());
		return;
	}
...
void UMeshDescriptionBase::ReversePolygonFacing(FPolygonID PolygonID)
{
	if (!GetMeshDescription().IsPolygonValid(PolygonID))
	{
		UE_LOGF(LogMeshDescription, Warning, "ReversePolygonFacing: PolygonID %d doesn't exist.", PolygonID.GetValue());
		return;
	}
...
void UMeshDescriptionBase::ComputePolygonTriangulation(FPolygonID PolygonID)
{
	if (!GetMeshDescription().IsPolygonValid(PolygonID))
	{
		UE_LOGF(LogMeshDescription, Warning, "ComputePolygonTriangulation: PolygonID %d doesn't exist.", PolygonID.GetValue());
		return;
	}
...
void UMeshDescriptionBase::GetPolygonGroupPolygons(FPolygonGroupID PolygonGroupID, TArray<FPolygonID>& OutPolygonIDs) const
{
	if (!GetMeshDescription().IsPolygonGroupValid(PolygonGroupID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetPolygonGroupPolygons: PolygonGroupID %d doesn't exist.", PolygonGroupID.GetValue());
		return;
	}
...
int32 UMeshDescriptionBase::GetNumPolygonGroupPolygons(FPolygonGroupID PolygonGroupID) const
{
	if (!GetMeshDescription().IsPolygonGroupValid(PolygonGroupID))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetNumPolygonGroupPolygons: PolygonGroupID %d doesn't exist.", PolygonGroupID.GetValue());
		return 0;
	}
