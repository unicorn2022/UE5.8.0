...
FArchive& operator<<( FArchive& Ar, FAttributesSetBase& AttributesSet )
{
	...
	if( Ar.IsSaving() && !Ar.IsTransacting() )
	{
		...
		// Remove the items outside the for loop above, since compact sets disallow removal via an iterator during iteration.
		for (const TTuple<FName, FAttributesSetEntry>& Item: TransientArrays)
		{
			AttributesSet.Map.Remove(Item.Key);
		}
	}
	...
	if( Ar.IsSaving() && !Ar.IsTransacting() )
	{
		for( TTuple<FName, FAttributesSetEntry>& TransientArray : TransientArrays )
		{
...
void FAttributesSetBase::AppendAttributesFrom(const FAttributesSetBase& OtherAttributesSet)
{
	...
	for (const auto& MapPair : OtherAttributesSet.Map)
	{
		...
		{
			UE_LOGF(LogMeshDescription, Log, "Appending attribute '%ls' which already exists.", *MapPair.Key.ToString());
		}
		else
		{
