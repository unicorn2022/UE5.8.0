...
private:
	...
	void UpdateBodyScaling(const int32 BodyIndex, float Scale) const;
	...
	// When true, physics simulation is enabled.
	uint8 bEnabled : 1;

	// When true, input bone transform scale has changed since the last physics evaluation. Will trigger an update of physics body scales in the next physics evaluation.
	uint8 bShouldUpdateBodiesOnScaleChange : 1;
