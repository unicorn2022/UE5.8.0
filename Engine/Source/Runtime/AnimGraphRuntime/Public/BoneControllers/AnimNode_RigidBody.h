...
public:
	...
	*/
	...
	/**
		Allow / Prevent bone transform scales denfined in the input pose from being passed to the output pose. This setting is true by default 
		but will only have an effect if bone scale propagation is enabled project wide with the p.RigidBodyNode.EnablePoseScalePropagation 
		console variable. The details pannel checkbox will be hidden if the feature is not enabled.
	*/
	UPROPERTY(EditAnywhere, Category = Settings)
	uint8 bAllowPoseScalePropagation : 1;
...
private:
	...
	struct FWorldObject
	{
		...
		FWorldObject(
			...
			: Component(InComponent), ActorHandle(InActorHandle), LastSeenTick(InLastSeenTick)
			, bExpired(false), bNew(true) {
		}
...
inline FTransform ConvertCSTransformToSimSpace(
	ESimulationSpace SimSpace, const FTransform& InCSTransform, const FTransform& ComponentToWorld, const FTransform& BaseBoneTM)
{
	switch (SimSpace)
	{
...
inline FVector ConvertCSVectorToSimSpace(
	ESimulationSpace SimSpace, const FVector& InCSVector, const FTransform& ComponentToWorld, const FTransform& BaseBoneTM)
{
	switch (SimSpace)
	{
	case ESimulationSpace::ComponentSpace: return InCSVector;
	case ESimulationSpace::WorldSpace:  return ComponentToWorld.TransformVectorNoScale(InCSVector);
	case ESimulationSpace::BaseBoneSpace: return BaseBoneTM.InverseTransformVectorNoScale(InCSVector); break;
	default: ensureMsgf(false, TEXT("Unsupported Simulation Space")); return InCSVector;
	}
}

inline FVector ConvertCSPositionToSimSpace(
	ESimulationSpace SimSpace, const FVector& InCSPosition, const FTransform& ComponentToWorld, const FTransform& BaseBoneTM)
{
	switch (SimSpace)
	{
	case ESimulationSpace::ComponentSpace: return InCSPosition;
	case ESimulationSpace::WorldSpace:  return ComponentToWorld.TransformPosition(InCSPosition);
	case ESimulationSpace::BaseBoneSpace: return BaseBoneTM.InverseTransformPosition(InCSPosition); break;
	default: ensureMsgf(false, TEXT("Unsupported Simulation Space")); return InCSPosition;
	}
}

ANIMGRAPHRUNTIME_API bool RigidBodyNodeIsPoseScalePropagationEnabledGlobally();
