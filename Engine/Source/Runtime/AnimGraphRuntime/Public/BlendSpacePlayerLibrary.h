...
public:
	...
	UFUNCTION(BlueprintCallable, Category = "Blend Space Player", meta = (BlueprintThreadSafe))
	static ANIMGRAPHRUNTIME_API FBlendSpacePlayerReference SetAccumulatedTime(const FBlendSpacePlayerReference& BlendSpacePlayer, float Time);

	/** Gets the current accumulated time of the blend space player. For blend spaces, this is normalized (0 - 1) */
	UFUNCTION(BlueprintPure, Category = "Blend Space Player", meta = (BlueprintThreadSafe))
	static ANIMGRAPHRUNTIME_API float GetAccumulatedTime(const FBlendSpacePlayerReference& BlendSpacePlayer);
};
