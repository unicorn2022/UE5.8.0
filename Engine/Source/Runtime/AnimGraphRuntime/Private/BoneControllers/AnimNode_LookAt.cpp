...
void FAnimNode_LookAt::Initialize_AnyThread(const FAnimationInitializeContext& Context)
{
	...
	{
		UE_LOGF(LogAnimation, Warning, "Zero-length look-up axis specified in LookAt node. Reverting to default. Instance:%ls", *GetFullNameSafe(Context.GetAnimInstanceObject()));
		LookUp_Axis.Axis = DefaultLookUpAxis;
	}
	LookAt_Axis.Initialize();
	if (LookAt_Axis.Axis.IsZero())
	{
		UE_LOGF(LogAnimation, Warning, "Zero-length look-at axis specified in LookAt node. Reverting to default. Instance:%ls", *GetFullNameSafe(Context.GetAnimInstanceObject()));
