...
void FAnimNode_Constraint::GatherDebugData(FNodeDebugData& DebugData)
{
	...
	for (int32 ConstraintIndex = 0; ConstraintIndex < ConstraintNum; ++ConstraintIndex)
	{
		...
		const float Weight = ConstraintWeights.IsValidIndex(ConstraintIndex) ? ConstraintWeights[ConstraintIndex] : 0.0f;
...
void FAnimNode_Constraint::EvaluateSkeletalControl_AnyThread(FComponentSpacePoseContext& Output, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	for (int32 ConstraintIndex = 0; ConstraintIndex < ConstraintNum; ++ConstraintIndex)
	{
		...
		if (Constraint.IsValidToEvaluate(BoneContainer) && Constraint.ConstraintDataIndex != INDEX_NONE)
		{
	...
	for (int32 ConstraintIndex = 0; ConstraintIndex < ConstraintNum; ++ConstraintIndex)
	{
		TRACE_ANIM_NODE_VALUE(Output, *ConstraintSetup[ConstraintIndex].TargetBone.BoneName.ToString(), ConstraintWeights.IsValidIndex(ConstraintIndex) ? ConstraintWeights[ConstraintIndex] : 0.0f);
	}
#endif
}
...
bool FAnimNode_Constraint::IsValidToEvaluate(const USkeleton* Skeleton, const FBoneContainer& RequiredBones) 
{
	...
	for (int32 ConstraintIndex = 0; ConstraintIndex<ConstraintSetup.Num() ; ++ConstraintIndex)
	{
		...
		if (ConstraintWeights.IsValidIndex(ConstraintIndex) && ConstraintWeights[ConstraintIndex] > ZERO_ANIMWEIGHT_THRESH)
		{
