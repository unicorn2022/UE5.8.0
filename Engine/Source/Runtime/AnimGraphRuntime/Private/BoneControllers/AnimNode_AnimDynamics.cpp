...
TAutoConsoleVariable<bool>  CVarScaleInitialBodyMassByComponentSpace(TEXT("p.AnimDynamics.ScaleInitialBodyMassByComponentSpace"), true, TEXT("Scale Initial Body Mass By Component Space Scale when using World Space."));
TAutoConsoleVariable<bool>  CVarTeleportBodiesOnScaleChange(TEXT("p.AnimDynamics.TeleportBodiesOnScaleChange"), true, TEXT("Teleport Bodies On Scale Change when using World Space."));
TAutoConsoleVariable<bool>  CVarScaleWorldSpaceSprings(TEXT("p.AnimDynamics.ScaleWorldSpaceSprings"), true, TEXT("Scale spring constants to match body scale changes when in world space."));
TAutoConsoleVariable<bool>  CVarPropagateSimulationScaleToOutputPose(TEXT("p.AnimDynamics.EnableUniformPoseScalePropagation"), true, TEXT("Copies a the scale applied to the simulated bodies into the output bone transforms. This scale is the minimum value of the x,y and z scales that are set in the input bone transforms."));
TAutoConsoleVariable<bool>  CVarUpdateBodiesOnScaleChange(TEXT("p.AnimDynamics.EnableUpdateBodiesOnScaleChange"), true, TEXT("When true, input bone transform scale is compared to the scale applied to simulated bodies in the last physics evaluation. Physics body scales will be updated in the next physics evaluation if they are different."));
...
void FindChainBones(const FName BeginBoneName, const FName EndBoneName, TFunctionRef< FName (const FName) > GetParentBoneName, TArray<FName>& OutChainBoneNames)
{
	...
	if (!EndBoneName.IsNone())
	{
		...
		{
			UE_LOGF(LogAnimation, Error, "AnimDynamics: Attempted to find bone chain starting at %ls and ending at %ls but failed.", *BeginBoneName.ToString(), *EndBoneName.ToString());
...
void FAnimNode_AnimDynamics::DrawBodies(FComponentSpacePoseContext& InContext, const TArray<FAnimPhysRigidBody*>& InBodies)
{
	...
	for(int32 BodyIndex = 0 ; BodyIndex < NumBodies ; ++BodyIndex)
	{
		...
		FTransform Transform(Body.Pose.Orientation, Body.Pose.Position, SimulationSpace != AnimPhysSimSpaceType::World || CVarDisableWorldSpace.GetValueOnAnyThread() ? FVector(Body.Pose.Scale) : FVector(1));
		Transform = GetComponentSpaceTransformFromSimSpace(SimulationSpace, InContext, Transform);

		Proxy->AnimDrawDebugCoordinateSystem(ToWorldV(InContext, Transform.GetTranslation()), Transform.Rotator(), 2.0f * Body.Pose.Scale, false, -1.0f, 0.15f);
...
#endif
, bEnabled(true)
, bShouldUpdateBodiesOnScaleChange(false)
#if ENABLE_ANIM_DRAW_DEBUG
...
void FAnimNode_AnimDynamics::UpdateBodyScaling(const int32 BodyIndex, float Scale) const
{
	...
	const FAnimPhysBodyDefinition& PhysicsBodyDef = PhysicsBodyDefinitions[BodyIndex];
	FAnimPhysRigidBody* RigidBody = BaseBodyPtrs[BodyIndex];
	...
	if (RigidBody->Pose.Scale != Scale)
	{
		...
		FAnimPhys::RigidBodyShapeScaleUpdate(RigidBody, Scale / RigidBody->Pose.Scale);
	}
}
...
void FAnimNode_AnimDynamics::EvaluateSkeletalControl_AnyThread(FComponentSpacePoseContext& Output, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	if (IsAnimDynamicsSystemEnabledFor(Output.AnimInstanceProxy->GetLODLevel()) && bEnabled)
	{
		...
		const FTransform SimWorldTransform = GetSimToWorldSpaceTransform(SimulationSpace != AnimPhysSimSpaceType::World ? SimulationSpace : AnimPhysSimSpaceType::Component, Output);

		if (bScalingEnabled)
		{
			if (bShouldUpdateBodiesOnScaleChange || !SimWorldTransform.GetScale3D().Equals(PreviousSimSpaceTM.GetScale3D(), UE_KINDA_SMALL_NUMBER))
			{
				const bool bWorldSpace = SimulationSpace == AnimPhysSimSpaceType::World && !CVarDisableWorldSpace.GetValueOnAnyThread();
				const bool bTeleportEnabled = bWorldSpace && CVarTeleportBodiesOnScaleChange.GetValueOnAnyThread();
				...
				for (int32 ActiveIndex : ActiveBoneIndices)
				{
					...
					float Scale = BoundBoneTransform.GetScale3D().GetAbsMin();
					Scale = Scale < UE_KINDA_SMALL_NUMBER ? UE_KINDA_SMALL_NUMBER : Scale;

					if (RigidBody.Pose.Scale != Scale)
					{
						if (bWorldSpace)
						{
							UpdateBodyScaling(ActiveIndex, Scale);
						}
						RigidBody.Pose.Scale = Scale;

						if (bTeleportEnabled)
						{
							FTransform BodyTransform(RigidBody.Pose.Orientation, RigidBody.Pose.Position - RigidBody.Pose.Orientation.RotateVector(PhysicsBodyDefinitions[ActiveIndex].LocalJointOffset));
							...
							RigidBody.Pose.Orientation = BodyTransform.GetRotation();
							RigidBody.PreviousOrientation = RigidBody.Pose.Orientation;
							RigidBody.NextOrientation = RigidBody.Pose.Orientation;

							RigidBody.Pose.Position = BodyTransform.GetTranslation() + RigidBody.Pose.Orientation.RotateVector(PhysicsBodyDefinitions[ActiveIndex].LocalJointOffset);
						}
					}
				}
		...
		while(BodiesToReset.Num() > 0)
		{
			...
			if(BodyToReset && BodyToReset->RigidBody.BoundBone.IsValidToEvaluate(RequiredBones))
			{
				...
				float Scale = BoneTransform.GetScale3D().GetAbsMin();
				Scale = Scale < UE_KINDA_SMALL_NUMBER ? UE_KINDA_SMALL_NUMBER : Scale;
				PhysBody.Pose.Scale = Scale;
		...
		// Disable simulation below minimum scale in world space mode.
		if (bScalingEnabled && (SimulationSpace == AnimPhysSimSpaceType::World) && !CVarDisableWorldSpace.GetValueOnAnyThread() && (SimWorldTransform.GetScale3D().SizeSquared() < WorldSpaceMinimumScale * WorldSpaceMinimumScale))
		{
			return;
		}
		...
		if (ShouldDoPhysicsUpdate() && NextTimeStep > AnimPhysicsMinDeltaTime)
		{
			...
			CalculateSimulationSpaceMotion(SimulationSpace, UseSimSpaceSettings, Output, NextTimeStep, SimFrameAngularVelocity, SimFrameAngularAcc);
		...
		if (bDoEval)
		{
			...
			const bool bPropagateSimulationScaleToOutputPose = CVarPropagateSimulationScaleToOutputPose.GetValueOnAnyThread();
			...
			for (int32 Idx = 0; Idx < Bodies.Num(); ++Idx)
			{
				...
				if (bScalingEnabled && SimulationSpace == AnimPhysSimSpaceType::World && !CVarDisableWorldSpace.GetValueOnAnyThread())
				{
				...
				if (bPropagateSimulationScaleToOutputPose)
				{
					// Override output bone transform scale with a uniform scale vector with the same value on each axis as the scale value applied in the simulation.
					NewBoneTransform.SetScale3D(FVector(CurrentBody.Pose.Scale));
				}
		...
		PreviousSimSpaceTM = SimWorldTransform;
...
{
	const bool bScalingSupportEnabled = CVarEnableScaling.GetValueOnAnyThread() && (SimulationSpace == AnimPhysSimSpaceType::World) && !CVarDisableWorldSpace.GetValueOnAnyThread();
	...
	case ETeleportType::ResetPhysics:
	{
		...
		const FTransform InitialComponentTransform = GetSimToWorldSpaceTransform(AnimPhysSimSpaceType::Component, Output);
		float InitialComponentScale = InitialComponentTransform.GetScale3D().GetAbsMin();
		InitialComponentScale = InitialComponentScale < UE_KINDA_SMALL_NUMBER ? UE_KINDA_SMALL_NUMBER : InitialComponentScale;
		...
		for (FAnimPhysBodyDefinition& PhysicsBodyDef : PhysicsBodyDefinitions)
		{
			...
			float BodyScale = BodyTransform.GetScale3D().GetAbsMin();
			BodyScale = BodyScale < UE_KINDA_SMALL_NUMBER ? UE_KINDA_SMALL_NUMBER : BodyScale;
			
			if (bScalingSupportEnabled)
			{
				BodyShapes.Add(FAnimPhysShape::MakeBox(PhysicsBodyDef.BoxExtents * BodyScale));
			...
			PhysicsBody.Pose.Scale = BodyScale;
			...
			//Masses originally were always 1 regardless of Scale or BodyShape Bounds. Allow the user to enable mass scaling so masses are adjusted based on initial scale.
			if (bScalingSupportEnabled && !CVarDisableWorldSpace.GetValueOnAnyThread()  && CVarScaleInitialBodyMassByComponentSpace.GetValueOnAnyThread())
			{
				FAnimPhys::ScaleRigidBodyMass(&PhysicsBody, InitialComponentScale);
			}

			switch (PhysicsBody.CollisionType)
			{
			case AnimPhysCollisionType::CustomSphere:
				PhysicsBody.SphereCollisionRadius = static_cast<float>(bScalingSupportEnabled ? PhysicsBodyDef.SphereCollisionRadius * BodyScale : PhysicsBodyDef.SphereCollisionRadius);
				break;
			case AnimPhysCollisionType::InnerSphere:
				PhysicsBody.SphereCollisionRadius = static_cast<float>(bScalingSupportEnabled ? (PhysicsBodyDef.BoxExtents.GetAbsMin() * BodyScale / 2.0) : PhysicsBodyDef.BoxExtents.GetAbsMin() / 2.0);
				break;
			case AnimPhysCollisionType::OuterSphere:
				PhysicsBody.SphereCollisionRadius = static_cast<float>(bScalingSupportEnabled ? (PhysicsBodyDef.BoxExtents.GetAbsMax() * BodyScale / 2.0) : PhysicsBodyDef.BoxExtents.GetAbsMax() / 2.0);
				break;
			default:
				break;
			}
			...
			if (Bodies.Num() > 0)
			{
				...
				// Calculate constraint offset positions in the space of each body. For world space we scale the limit distances in UpdateLimits to account for dynamic scale. Store the unscaled value in ConstraintOffsets.
				const FVector ConstaintLocationSimSpace = (BodyTransform.GetTranslation() - PreviousBodyTransformSimSpace.GetTranslation()) * 0.5f / (bScalingSupportEnabled ? PhysicsBody.Pose.Scale : 1);
	...
	PreviousSimSpaceTM = GetSimToWorldSpaceTransform(SimulationSpace != AnimPhysSimSpaceType::World ? SimulationSpace : AnimPhysSimSpaceType::Component, Output);
}
...
void FAnimNode_AnimDynamics::UpdateLimits(FComponentSpacePoseContext& Output)
{
	...
	const bool bShouldTestForBoneTransformScaleChange = CVarUpdateBodiesOnScaleChange.GetValueOnAnyThread();
	...
	const bool bShouldScale = SimulationSpace == AnimPhysSimSpaceType::World && !CVarDisableWorldSpace.GetValueOnAnyThread() && CVarEnableScaling.GetValueOnAnyThread();
	const bool bScaleSprings = bShouldScale && CVarScaleWorldSpaceSprings.GetValueOnAnyThread();
	...
	for(int32 ActiveIndex : ActiveBoneIndices)
	{
		...
		FTransform BoundBoneComponentSpaceTransform = Output.Pose.GetComponentSpaceTransform(BoneIndex);
		FTransform BoundBoneTransform = GetSimSpaceTransformFromComponentSpace(SimulationSpace, Output, BoundBoneComponentSpaceTransform);

		if(bShouldTestForBoneTransformScaleChange && !bShouldUpdateBodiesOnScaleChange)
		{
			// Check if input bone transform scale has changed since the last physics init.
			bShouldUpdateBodiesOnScaleChange = RigidBody.Pose.Scale != BoundBoneComponentSpaceTransform.GetScale3D().GetAbsMin();
		}
		...
		if (PrevBody)
		{
			ShapeTransform = FTransform(FQuat::Identity, ConstraintOffsets[ActiveIndex].Body0Offset * (bShouldScale ? PrevBody->Pose.Scale : 1));
		}

		// Local offset to joint for Body1
		const FVector Body1JointOffset = ConstraintOffsets[ActiveIndex].Body1Offset * (bShouldScale ? RigidBody.Pose.Scale : 1);
		...
		if (PhysicsBodyDef.ConstraintSetup.AngularConstraintType == AnimPhysAngularConstraintType::Angular)
		{
			...
			{
				UE_LOGF(LogAnimation, Warning, "AnimDynamics: Min/Max angular limits for bone %ls incorrect, at least one min axis value is greater than the corresponding max.", *BoundBone.BoneName.ToString());
			}
#endif
		...
		if(SphericalLimits.Num() > 0 && bUseSphericalLimits)
		{
			for(FAnimPhysSphericalLimit& SphericalLimit : SphericalLimits)
			{
				...
				case ESphericalLimitType::Inner:
					FAnimPhys::ConstrainSphericalInner(NextTimeStep, LinearLimits, &RigidBody, SphereTransform, SphericalLimit.LimitRadius * (bShouldScale ? RigidBody.Pose.Scale : 1));
					break;
				case ESphericalLimitType::Outer:
					FAnimPhys::ConstrainSphericalOuter(NextTimeStep, LinearLimits, &RigidBody, SphereTransform, SphericalLimit.LimitRadius * (bShouldScale ? RigidBody.Pose.Scale : 1));
					break;
				default:
					break;
				}
		...
		if (bAngularSpring || bLinearSpring)
		{
			...
			NewSpring.SpringConstantLinear = LinearSpringConstant * (bScaleSprings ? RigidBody.Pose.Scale : 1);
			NewSpring.SpringConstantAngular = AngularSpringConstant * (bScaleSprings ? RigidBody.Pose.Scale * RigidBody.Pose.Scale * RigidBody.Pose.Scale : 1);
...
void FAnimNode_AnimDynamics::PreUpdate(const UAnimInstance* InAnimInstance)
{
	...
	bEnabled = SkelComp->GetAllowAnimDynamicsAnimNode();
...
void FAnimNode_AnimDynamics::ConvertSimulationSpace(FComponentSpacePoseContext& Output, AnimPhysSimSpaceType From, AnimPhysSimSpaceType To) const
{
	...
	const bool bWorldDisabled = CVarDisableWorldSpace.GetValueOnAnyThread();
	...
	for(int Idx = 0; Idx < BaseBodyPtrs.Num(); Idx++)
	{
		...
		// Get transform
		FTransform BodyTransform(Body->Pose.Orientation, Body->Pose.Position, FVector(Body->Pose.Scale));
		...
		float NewScale = BodyTransform.GetScale3D().GetAbsMin();
		NewScale = NewScale < UE_KINDA_SMALL_NUMBER ? UE_KINDA_SMALL_NUMBER : NewScale;

		if (bScalingSupportEnabled)
		{
			if (From == AnimPhysSimSpaceType::World && !bWorldDisabled && To != AnimPhysSimSpaceType::World)
			{
				UpdateBodyScaling(Idx, 1);
			}
			else if (From != AnimPhysSimSpaceType::World && To == AnimPhysSimSpaceType::World && !bWorldDisabled)
			{
				UpdateBodyScaling(Idx, NewScale * Body->Pose.Scale);
			}
		}

		Body->Pose.Scale = NewScale;
	}
}
