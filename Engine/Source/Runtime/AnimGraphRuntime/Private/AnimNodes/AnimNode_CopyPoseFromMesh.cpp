...
namespace
{
	static bool bTestLeaderPoseEarly = true;
	static FAutoConsoleVariableRef CVarTestLeaderPoseEarly(
		TEXT("a.CopyPose.TestLeaderPoseEarly"),
		bTestLeaderPoseEarly,
		TEXT("In CopyPoseFromMesh, test if we have a Leader Pose early to avoid potential mesh corruption"));
}
...
void FAnimNode_CopyPoseFromMesh::RefreshMeshComponent(USkeletalMeshComponent* TargetMeshComponent)
{
	auto ResetMeshComponent = [this](USkeletalMeshComponent* InMeshComponent, USkeletalMeshComponent* InTargetMeshComponent)
	{
		...
		if (bTestLeaderPoseEarly && InMeshComponent)
		{
			if (USkeletalMeshComponent* LeaderPoseComponent = Cast<USkeletalMeshComponent>(InMeshComponent->LeaderPoseComponent.Get()))
			{
				InMeshComponent = LeaderPoseComponent;
			}
		}
...
void FAnimNode_CopyPoseFromMesh::PreUpdate(const UAnimInstance* InAnimInstance)
{
	...
	if (CurrentMeshComponent && CurrentMeshComponent->GetSkeletalMeshAsset() && CurrentMeshComponent->IsRegistered())
	{
		...
		if(CurrentMeshComponent->GetSkeletalMeshAsset() && CurrentMeshComponent->IsRegistered())
		{
			...
			if (UE::Anim::bUseSafeMeshPoseIndices)
			{
				SourceMeshValidMeshPoseTransforms = CurrentMeshComponent->GetValidMeshPoseTransforms();
			}
	...
	if(SourceMeshTransformArray.Num() > 0 && CurrentMesh)
	{
		...
		if (bUseMeshPose)
		{
			...
			for (FCompactPoseBoneIndex PoseBoneIndex : OutPose.ForEachBoneIndex())
			{
				...
				{
					if ((!UE::Anim::bUseSafeMeshPoseIndices || (SourceMeshValidMeshPoseTransforms.IsValidIndex(*Value) && SourceMeshValidMeshPoseTransforms[*Value])))
					{
