...
void FAnimNode_LayeredBoneBlend::Update_AnyThread(const FAnimationUpdateContext& Context)
{
	...
	TArray<float, TInlineAllocator<64>> ChildRootMotionWeight;
	if (IsLODEnabled(Context.AnimInstanceProxy))
	{
		GetEvaluateGraphExposedInputs().Execute(Context);

		if (bUpdateBasePoseFirst)
		{
			ChildRootMotionWeight.SetNumZeroed(BlendPoses.Num());
		}
		...
		for (int32 ChildIndex = 0; ChildIndex < BlendPoses.Num(); ++ChildIndex)
		{
			...
			if (FAnimWeight::IsRelevant(ChildWeight))
			{
				...
				if (bUpdateBasePoseFirst)
				{
					ChildRootMotionWeight[ChildIndex] = ThisPoseRootMotionWeight;
				}
				else
				{
	...
	if (!ChildRootMotionWeight.IsEmpty())
	{
		check(bUpdateBasePoseFirst);
		check(ChildRootMotionWeight.Num() == BlendPoses.Num());
		...
		for (int32 ChildIndex = 0; ChildIndex < BlendPoses.Num(); ++ChildIndex)
		{
			...
			if (FAnimWeight::IsRelevant(ChildWeight))
			{
				const float ThisPoseRootMotionWeight = ChildRootMotionWeight[ChildIndex];
				BlendPoses[ChildIndex].Update(Context.FractionalWeightAndRootMotion(ChildWeight, ThisPoseRootMotionWeight));
			}
		}
	}
	...
	TRACE_ANIM_NODE_VALUE(Context, TEXT("Has Relevant Poses"), bHasRelevantPoses);
	for (int32 LayerIndex = 0; LayerIndex < BlendWeights.Num(); ++LayerIndex)
	{
		TRACE_ANIM_NODE_VALUE(Context, *FString::Printf(TEXT("Layer %d Weight"), LayerIndex), BlendWeights[LayerIndex]);
	}
#endif
}
...
void FAnimNode_LayeredBoneBlend::GatherDebugData(FNodeDebugData& DebugData)
{
	...
	DebugLine += FString::Printf(TEXT("(Num Poses: %i, Has Relevant Poses: %s)"), NumPoses, bHasRelevantPoses ? TEXT("true") : TEXT("false"));
