...
static FAutoConsoleCommand AuditLoadedAnimGraphs(
	...
	{
		UE_LOGF(LogAnimation, Display, "--- BEGIN audit all loaded anim graphs ---");
		UE_LOGF(LogAnimation, Display, "NodeType, Size (B), Count, TotalSize (B)");
		...
		for(const TPair<UScriptStruct*, uint64> NodeTypeCountPair : SortedPerNodeTypeCounts)
		{
			...
			UE_LOGF(LogAnimation, Display, "%ls, %llu, %llu, %llu", *NodeStruct->GetName(), Size, Count, Size * Count);
		}

		UE_LOGF(LogAnimation, Display, "Total anim graph mem, %llu Bytes, (%llu KB)", TotalAnimGraphMemory, TotalAnimGraphMemory / 1024ull);
		UE_LOGF(LogAnimation, Display, "UAnimInstance count, %d", NumAnimInstances);
		UE_LOGF(LogAnimation, Display, "Unique UAnimInstance classes, %d", Classes.Num());	

		UE_LOGF(LogAnimation, Display, "--- END audit all loaded anim graphs ---");
	}),
	ECVF_Cheat);
