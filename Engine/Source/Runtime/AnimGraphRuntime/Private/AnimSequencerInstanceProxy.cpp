...
void FAnimSequencerInstanceProxy::InitAnimTrack(UAnimSequenceBase* InAnimSequence, uint32 SequenceId)
{
	if (InAnimSequence != nullptr)
	{
		...
		if (PlayerState == nullptr)
		{
			...
			{
				// this doesn't work
				UE_LOGF(LogAnimation, Warning, "ERROR: Animation [%ls] in Sequencer has Mesh Space additive animation.  No support on mesh space additive animation. ", *GetNameSafe(InAnimSequence));
			}
