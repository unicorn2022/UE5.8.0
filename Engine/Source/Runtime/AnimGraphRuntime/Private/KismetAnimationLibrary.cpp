...
float UKismetAnimationLibrary::K2_CalculateVelocityFromPositionHistory(
	...
	{
		UE_LOGF(LogAnimation, Warning, "Position History has a different number of position and velocity samples.");
		return 0.f;
	}
...
float UKismetAnimationLibrary::K2_EndProfilingTimer(bool bLog, const FString& LogPrefix)
{
	...
	{
		UE_LOGF(LogAnimation, Warning, "Unbalanced use of Start & End Profiling Timer nodes.");
		return 0.f;
	}
	...
	if (bLog)
	{
		...
		{
			UE_LOGF(LogAnimation, Warning, "%.03f ms", Delta);
		}
		else
		{
			UE_LOGF(LogAnimation, Warning, "[%ls] %.03f ms", *LogPrefix, Delta);
		}
	}
