...
private:
	...
	struct FPairType
	{
		...
		FORCEINLINE friend FArchive& operator<<(FArchive& Ar, FPairType& Pair)
		{
			...
			{
				Ar << StringLength;
				checkf(StringLength < FName::StringBufferSize, TEXT("Trying to deserialize FName with length %d; buffer size is %d"), StringLength, FName::StringBufferSize);
			...
			else
			{
				...
				checkf(StringLength < FName::StringBufferSize, TEXT("FName %s length (%d) exceeds buffer size (%d)"), *Pair.Key.ToString(), StringLength, FName::StringBufferSize);
