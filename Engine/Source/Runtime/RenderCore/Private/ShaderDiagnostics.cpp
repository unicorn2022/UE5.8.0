...
// Returns true if the shader compile job or pipeline compile job requested to always log its diagnostic messages.
static bool AreDiagnosticMessagesForced(const FShaderCommonCompileJob& InJob)
{
	bool bOutAreDiagnosticMessagesForced = false;
	InJob.ForEachSingleShaderJob([&bOutAreDiagnosticMessagesForced](const FShaderCompileJob& SingleJob) {
		if (SingleJob.Output.bForceLogDiagnosticMessages)
		{
			bOutAreDiagnosticMessagesForced = true;
		}
	});
	return bOutAreDiagnosticMessagesForced;
}
...
FShaderDiagnosticInfo::FShaderDiagnosticInfo(const TArray<FShaderCommonCompileJobPtr>& Jobs)
{
	...
	for (const FShaderCommonCompileJobPtr& Job : Jobs)
	{
		...
		else if (ShouldShowShaderWarnings() || AreDiagnosticMessagesForced(*Job))
		{
	...
	for (EShaderPlatform ErrorPlatform : ErrorPlatforms)
	{
		...
		{
			TargetShaderPlatformString += FString(", ") + FDataDrivenShaderPlatformInfo::GetName(ErrorPlatform).ToString();
		}
	}
}
