...
extern std::atomic<bool> GRDGAllowRHIAccess;
extern std::atomic<bool> GRDGAllowRHIAccessAsync;
...
public:
	...
	{
		check(!GRDGAllowRHIAccess.load(std::memory_order_relaxed));
		GRDGAllowRHIAccess.store(true, std::memory_order_relaxed);
	}

	~FRDGAllowRHIAccessScope()
	{
		check(GRDGAllowRHIAccess.load(std::memory_order_relaxed));
		GRDGAllowRHIAccess.store(false, std::memory_order_relaxed);
	}
};
...
extern std::atomic_int32_t GRDGStatTransitionBatchCount;
...
FORCEINLINE bool IsAsyncComputeSupported(EShaderPlatform ShaderPlatform)
{
	...
	return GRDGAsyncCompute > 0 && !IsImmediateMode() && !IsRenderPassMergeEnabled(ShaderPlatform) && GSupportsEfficientAsyncCompute && GRHISupportsSeparateDepthStencilCopyAccess;
}
