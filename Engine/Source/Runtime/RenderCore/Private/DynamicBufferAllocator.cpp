...
FGlobalDynamicReadBuffer::FAllocation FGlobalDynamicReadBuffer::AllocateInternal(FDynamicReadBufferPool* BufferPool, uint32 Num)
{
	...
	if (Buffer == nullptr || ByteOffset + SizeInBytes > Buffer->NumBytes)
	{
		...
		if (Buffer == nullptr)
		{
			...
			{
				UE_LOGF(LogRendererCore, Warning, "FGlobalDynamicReadBuffer::AllocateInternal, trying to allocate a Buffer of size (in elements) %d that is greater than the platform max of %llu", NewBufferSize, MaxViewDimensionForTypedBuffer);
			}
