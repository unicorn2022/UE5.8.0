...
private:
			...
			{				
					...
					FParse::Value(*DefineString, TEXT("Define="), DefineName))
				{
					...
					if (ShaderType == nullptr || ShaderType->GetGlobalShaderType() == nullptr)
					{
						...
						UE_LOGF(LogShaders, Warning, "Global shader definition '%ls' found in engine config for global shader '%ls', which does not exist", *DefineName, *ShaderName);
						continue;
					}
	...
	static void ErrorCheckPlatformsForShaderFormat(const FPlatformInfo& PlatformA, const FPlatformInfo& PlatformB, FName ShaderFormat)
	{
		...
		if (ShadersWithError.Num() > 0)
		{
			...
			UE_LOGF(LogShaders, Fatal, "It has been detected that one or more global shaders are configured differently between platform '%ls' and '%ls' for "
				"shader format '%ls'. This is unsupported, and could result in data mismatches in the Derived Data Cache. Please check that all entries "
				"of '[GlobalShaderDefines]` in the various engine config .ini files are the same for these shaders on all platforms that use this shader "
...
bool FGlobalShaderType::ForEachCompiledPermutation(EShaderPlatform Platform, EShaderPermutationFlags PermutationFlags, TFunction<bool(int32)> Function)
{
	bool bSuccess = true;
	if (FDataDrivenShaderPlatformInfo::GetSupportsSpecializationConstants(Platform) && (GetSpecializationCount() > 0))
	{
		// If the shader type includes specialization, collapse the permutations into valid IDs
		TSet<int32> UnspecializedIDs;
		UnspecializedIDs.Reserve(GetPermutationCount());
		for (int32 PermutationId = 0; PermutationId < GetPermutationCount(); PermutationId++)
		{
			if (ShouldCompilePermutation(Platform, PermutationId, PermutationFlags))
			{
				UnspecializedIDs.Add(GetUnspecializedId(PermutationId));
			}
		}
		for (int32 PermutationId : UnspecializedIDs)
		{
			if (!Function(PermutationId))
			{
				bSuccess = false;
				break;
			}
...
void PrimeGlobalShaderSourceHashCache(EShaderPlatform Platform, const ITargetPlatform* TargetPlatform)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(PrimeGlobalShaderSourceHashCache);
	...
	TArray<FGlobalShaderType*> ShaderTypesToHash;
	for (TLinkedList<FShaderType*>::TIterator ShaderTypeIt(FShaderType::GetTypeList()); ShaderTypeIt; ShaderTypeIt.Next())
	{
		FGlobalShaderType* GlobalShaderType = ShaderTypeIt->GetGlobalShaderType();
		if (!GlobalShaderType)
		...
		for (int32 PermutationId = 0; PermutationId < GlobalShaderType->GetPermutationCount(); PermutationId++)
		{
			if (GlobalShaderType->ShouldCompilePermutation(Platform, PermutationId, PermutationFlags))
			{
				ShaderTypesToHash.Add(GlobalShaderType);
				break;
			}
		}
	}

	TArray<FString> ShaderFilenames;
	ShaderFilenames.Reserve(ShaderTypesToHash.Num());
	for (FGlobalShaderType* GlobalShaderType : ShaderTypesToHash)
	{
		ShaderFilenames.Add(GlobalShaderType->GetShaderFilename());
	}
	
	PrimeShaderFileHashCache(ShaderFilenames, Platform);
#endif
}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FGlobalShaderMapSection::CreateFromCache);
...
TShaderRef<FShader> FGlobalShaderMapSection::GetShader(EShaderPlatform ShaderPlatform, FShaderType* ShaderType, int32 PermutationId) const
{
	const bool bSupportsSpecConsts = FDataDrivenShaderPlatformInfo::GetSupportsSpecializationConstants(ShaderPlatform);
	FShader* Shader = GetContent()->GetShader(ShaderType, bSupportsSpecConsts ? ShaderType->GetUnspecializedId(PermutationId) : PermutationId);
	return Shader ? TShaderRef<FShader>(Shader, *this, bSupportsSpecConsts ? ShaderType->GetSpecializationValues(PermutationId) : FSpecializationContainerType()) : TShaderRef<FShader>();
}
...
TShaderRef<FShader> FGlobalShaderMap::GetShader(FShaderType* ShaderType, int32 PermutationId) const
{
	...
	return Section ? (*Section)->GetShader(Platform, ShaderType, PermutationId) : TShaderRef<FShader>();
}
...
bool FGlobalShaderMap::IsComplete(const ITargetPlatform* TargetPlatform) const
{
	...
	for (TLinkedList<FShaderType*>::TIterator ShaderTypeIt(FShaderType::GetTypeList()); ShaderTypeIt; ShaderTypeIt.Next())
	{
		...
		const bool bSuccess = GlobalShaderType->ForEachCompiledPermutation(Platform, PermutationFlags, [&](int32 PermutationId)
		{
			return HasShader(GlobalShaderType, PermutationId);
		});
		if (!bSuccess)
		{
			return false;
...
void FGlobalShaderMap::LoadFromGlobalArchive(FArchive& Ar)
{
	...
	for (int32 i = 0; i < NumSections; ++i)
	{
		...
		{
			UE_LOGF(LogShaders, Fatal, "Could not load section %d (of %d) of the global shadermap.", i, NumSections);
		}
	}
}
