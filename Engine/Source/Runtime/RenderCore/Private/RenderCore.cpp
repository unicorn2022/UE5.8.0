...
static TAutoConsoleVariable<bool> CVarHDRDisplayOverrideOSMaxLuminance(
	TEXT("r.HDR.Display.OverrideOSMaxLuminance"),
	false,
	TEXT("Whether to override the maximum display luminance provided by the OS."),
	ECVF_RenderThreadSafe);
...
TAutoConsoleVariable<float> CVarHDRPaperWhite(
	TEXT("r.HDR.PaperWhite"),
	203.f,
	TEXT("Luminance in nits to redefine HDR Reference White as specified in BT.2408."),
	ECVF_RenderThreadSafe
);

TAutoConsoleVariable<int32> CVarHDRPaperWhiteMode(
	TEXT("r.HDR.PaperWhite.Mode"),
	0,
	TEXT("Specifies how scene content incorporates paper white values.\n")
	TEXT("0: Use the platform's paper white if available, otherwise r.HDR.PaperWhite (default)\n")
	TEXT("1: Use r.HDR.PaperWhite"),
	ECVF_RenderThreadSafe
);
...
{
	const bool bHDRSupported = IsHDREnabled() && GRHISupportsHDROutput;
	return
	{
		.DisplayOutputFormat = HDRGetDefaultDisplayOutputFormat(),
		.DisplayColorGamut = HDRGetDefaultDisplayColorGamut(),
		.MinimumLuminanceInNits = bHDRSupported ?
			FMath::Pow(10.f, static_cast<float>(CVarHDRDisplayMinLuminanceLog10.GetValueOnAnyThread())) :
			0.f,
		.MaximumLuminanceInNits = bHDRSupported ?
			static_cast<float>(CVarHDRDisplayMaxLuminance.GetValueOnAnyThread()) :
			100.f,
		.MaximumFullFrameLuminanceInNits = bHDRSupported ?
			static_cast<float>(CVarHDRDisplayMaxLuminance.GetValueOnAnyThread()) :
			100.f,
		.HDRPaperWhiteInNits = 203.f,
		.LimitingColorSpace = bHDRSupported ? UE::Color::FColorSpace::GetP3D65() : UE::Color::FColorSpace::GetSRGB(),
		.bHDRSupported = bHDRSupported,
	};
}
...
RENDERCORE_API void HDRAddCustomMetaData(void* OSWindow, EDisplayOutputFormat DisplayOutputFormat, EDisplayColorGamut DisplayColorGamut, bool bHDREnabled)
{
	...
	FHDRMetaData& HDRMetaData = GWindowsWithDefaultParams.Emplace(OSWindow);
	HDRMetaData.DisplayOutputFormat = DisplayOutputFormat;
	HDRMetaData.DisplayColorGamut = DisplayColorGamut;
	// TODO: Plumb color space.
	HDRMetaData.LimitingColorSpace = bHDREnabled ?
		UE::Color::FColorSpace::GetP3D65() :
		UE::Color::FColorSpace::GetSRGB();
	HDRMetaData.MinimumLuminanceInNits = bHDREnabled ?
		FMath::Pow(10.f, static_cast<float>(CVarHDRDisplayMinLuminanceLog10.GetValueOnAnyThread())) :
		0.f,
	HDRMetaData.MaximumLuminanceInNits = bHDREnabled ?
		CVarHDRDisplayMaxLuminance.GetValueOnAnyThread() :
		100.f;
	HDRMetaData.MaximumFullFrameLuminanceInNits = bHDREnabled ?
		CVarHDRDisplayMaxLuminance.GetValueOnAnyThread() :
		100.f;
	HDRMetaData.HDRPaperWhiteInNits = 203.f;
								   ...
								   const FVector2D& WindowTopLeft, const FVector2D& WindowBottomRight, void* OSWindow)
{
	...
	HDRGetMetaDataForWindow(OutHDRMetaData, WindowTopLeft, WindowBottomRight, OSWindow);
	OutDisplayOutputFormat = OutHDRMetaData.DisplayOutputFormat;
	OutDisplayColorGamut = OutHDRMetaData.DisplayColorGamut;
	OutbHDRSupported = OutHDRMetaData.bHDRSupported;
}

RENDERCORE_API void HDRGetMetaDataForWindow(FHDRMetaData& OutHDRMetaData, const FVector2D& WindowTopLeft, const FVector2D& WindowBottomRight, void* OSWindow)
{
#if WITH_EDITOR
	// this has priority over IsHDREnabled because MovieSceneCapture might request custom parameters
	if (HdrHasWindowParamsFromCVars(OSWindow, OutHDRMetaData))
	{
		return;
	}
#endif
	
	OutHDRMetaData = HDRGetDefaultMetaData();
	...
	OutHDRMetaData.bHDRSupported = DisplayList[BestDisplayForWindow].bHDRSupported;
	OutHDRMetaData.DisplayOutputFormat = EDisplayOutputFormat::SDR_sRGB;
	OutHDRMetaData.DisplayColorGamut = EDisplayColorGamut::sRGB_D65;
	OutHDRMetaData.MinimumLuminanceInNits = OutHDRMetaData.bHDRSupported ? DisplayList[BestDisplayForWindow].MinimumLuminanceInNits : 0.f;
	OutHDRMetaData.MaximumLuminanceInNits = OutHDRMetaData.bHDRSupported ? DisplayList[BestDisplayForWindow].MaximumLuminanceInNits : 100.f;
	OutHDRMetaData.MaximumFullFrameLuminanceInNits = OutHDRMetaData.bHDRSupported ? DisplayList[BestDisplayForWindow].MaximumFullFrameLuminanceInNits : 100.f;
#if PLATFORM_PROVIDES_HDR_PAPER_WHITE
	if (CVarHDRPaperWhiteMode.GetValueOnAnyThread() == 0)
	{
		OutHDRMetaData.HDRPaperWhiteInNits = DisplayList[BestDisplayForWindow].HDRPaperWhiteInNits;
	}
	else
#endif
	{
		OutHDRMetaData.HDRPaperWhiteInNits = CVarHDRPaperWhite.GetValueOnAnyThread();
	}
	OutHDRMetaData.LimitingColorSpace = OutHDRMetaData.bHDRSupported ? DisplayList[BestDisplayForWindow].LimitingColorSpace : UE::Color::FColorSpace::GetSRGB();

	if (OutHDRMetaData.bHDRSupported)
	{
		FPlatformMisc::ChooseHDRDeviceAndColorGamut(GRHIVendorId, CVarHDRDisplayMaxLuminance.GetValueOnAnyThread(), OutHDRMetaData.DisplayOutputFormat, OutHDRMetaData.DisplayColorGamut);
	}
}
...
RENDERCORE_API FMatrix44f GamutToXYZMatrix(EDisplayColorGamut ColorGamut)
{
	static const FMatrix44f sRGB_2_XYZ_MAT(
		FVector3f(0.4124564f, 0.3575761f, 0.1804375f),
		FVector3f(0.2126729f, 0.7151522f, 0.0721750f),
		FVector3f(0.0193339f, 0.1191920f, 0.9503041f),
		FVector3f(0, 0, 0)
	);

	static const FMatrix44f Rec2020_2_XYZ_MAT(
		FVector3f(0.6369736f, 0.1446172f, 0.1688585f),
		FVector3f(0.2627066f, 0.6779996f, 0.0592938f),
		FVector3f(0.0000000f, 0.0280728f, 1.0608437f),
		FVector3f(0, 0, 0)
	);

	static const FMatrix44f P3D65_2_XYZ_MAT(
		FVector3f(0.4865906f, 0.2656683f, 0.1981905f),
		FVector3f(0.2289838f, 0.6917402f, 0.0792762f),
		FVector3f(0.0000000f, 0.0451135f, 1.0438031f),
		FVector3f(0, 0, 0)
	);
	switch (ColorGamut)
	{
...
RENDERCORE_API FMatrix44f XYZToGamutMatrix(EDisplayColorGamut ColorGamut)
{
	static const FMatrix44f XYZ_2_sRGB_MAT(
		FVector3f(3.2409699419f, -1.5373831776f, -0.4986107603f),
		FVector3f(-0.9692436363f, 1.8759675015f, 0.0415550574f),
		FVector3f(0.0556300797f, -0.2039769589f, 1.0569715142f),
		FVector3f(0, 0, 0)
	);

	static const FMatrix44f XYZ_2_Rec2020_MAT(
		FVector3f(1.7166084f, -0.3556621f, -0.2533601f),
		FVector3f(-0.6666829f, 1.6164776f, 0.0157685f),
		FVector3f(0.0176422f, -0.0427763f, 0.94222867f),
		FVector3f(0, 0, 0)
	);

	static const FMatrix44f XYZ_2_P3D65_MAT(
		FVector3f(2.4933963f, -0.9313459f, -0.4026945f),
		FVector3f(-0.8294868f, 1.7626597f, 0.0236246f),
		FVector3f(0.0358507f, -0.0761827f, 0.9570140f),
		FVector3f(0, 0, 0)
	);
...
static const float MIN_STOP_SDR = -6.5f;
static const float MAX_STOP_SDR = 6.5f;

static const float MIN_STOP_RRT = -15.f;
static const float MAX_STOP_RRT = 18.f;

static const float MIN_LUM_SDR = 0.02f;
static const float MAX_LUM_SDR = 48.0f;

static const float MIN_LUM_RRT = 0.0001f;
static const float MAX_LUM_RRT = 10000.0f;
...
)
{
	...
	FVector2f bendsLow[2] = { FVector2f(MIN_STOP_RRT, 0.18f),
						   FVector2f(MIN_STOP_SDR, 0.35f) };
...
)
{
	...
	// Middle coefficient (which defines the "sharpness of the bend") is linearly interpolated
	FVector2f bendsHigh[2] = { FVector2f(MAX_STOP_SDR, 0.89f) ,
							FVector2f(MAX_STOP_RRT, 0.90f) };
