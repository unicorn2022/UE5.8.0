...
FRenderResource::~FRenderResource()
{
	...
	if (IsInitialized() && !GIsCriticalError)
	{
		...
		UE_LOGF(LogRendererCore, Fatal, "A FRenderResource was deleted without being released first! [Resource=%ls, Owner=%ls, Scope=%ls]", *GetResourceName().ToString(), *GetOwnerName().ToString(), *GRenderResourceScopeName.ToString());
	}
}
...
double FTextureReference::GetLastRenderTime() const
{
	...
	return FRHITexture::EmptyLastRenderTime;
}
