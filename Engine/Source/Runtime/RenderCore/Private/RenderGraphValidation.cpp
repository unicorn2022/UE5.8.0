...
void FRDGResource::ValidateRHIAccess() const
{
	...
	checkf(bAllowRHIAccess || GRDGAllowRHIAccess.load(std::memory_order_relaxed) || GRDGAllowRHIAccessAsync.load(std::memory_order_relaxed),
...
void FRDGBarrierValidation::ValidateBarrierBatchBegin(const FRDGPass* Pass, const FRDGBarrierBatchBegin& Batch)
{
	...
	const auto LogHeader = [&]()
	{
		...
		{
			bFoundFirst = true;
			UE_LOGF(LogRDG, Display, "[%ls(Index: %d, Pipeline: %ls): Batch: (%p) %ls] (Begin):", Pass->GetName(), Pass->GetHandle().GetIndex(), *GetRHIPipelineName(Pass->GetPipeline()), &Batch, Batch.DebugName);
		}
	};
	...
	for (const auto& KeyValue : ResourceMap->Aliases)
	{
		...
		if (Info.IsAcquire())
		{
			...
			{
				LogHeader();
				UE_LOGF(LogRDG, Display, "\tRDG(%p) RHI(%p) %ls - Acquire", Resource, Resource->GetRHIUnchecked(), Resource->Name);
			}
		}
	}
	...
	for (const auto& Pair : ResourceMap->Textures)
	{
		...
		{
			LogHeader();
			UE_LOGF(LogRDG, Display, "\tRDG(%p) RHI(%p) %ls:", Texture, Texture->GetRHIUnchecked(), Texture->Name);
		}
		...
		{
			UE_LOGF(LogRDG, Display, "\t\tMip(%d), Array(%d), Slice(%d): [%ls, %ls] -> [%ls, %ls]",
	...
	for (const auto& Pair : ResourceMap->Buffers)
	{
		...
		UE_LOGF(LogRDG, Display, "\tRDG(%p) RHI(%p) %ls: [%ls, %ls] -> [%ls, %ls]",
			Buffer,
...
void FRDGBarrierValidation::ValidateBarrierBatchEnd(const FRDGPass* Pass, const FRDGBarrierBatchEnd& Batch)
{
	...
	for (const FRDGBarrierBatchBegin* Dependent : Batch.Dependencies)
	{
		...
		const auto LogHeader = [&]()
		{
			...
			{
				bFoundFirst = true;
				UE_LOGF(LogRDG, Display, "[%ls(Index: %d, Pipeline: %ls) Batch: (%p) %ls] (End):", Pass->GetName(), Pass->GetHandle().GetIndex(), *GetRHIPipelineName(Pass->GetPipeline()), Dependent, Dependent->DebugName);
			}
		};
		...
		for (FRDGTextureRef Texture : Textures)
		{
			...
			{
				LogHeader();
				UE_LOGF(LogRDG, Display, "\tRDG(%p) RHI(%p) %ls - End:", Texture, Texture->GetRHIUnchecked(), Texture->Name);
			}
		}
		...
		for (FRDGBufferRef Buffer : Buffers)
		{
			...
			{
				LogHeader();
				UE_LOGF(LogRDG, Display, "\tRDG(%p) RHI(%p) %ls - End", Buffer, Buffer->GetRHIUnchecked(), Buffer->Name);
			}
		}
