...
public:
	...
	virtual void InitRHI(FRHICommandListBase& RHICmdList) override
	{
		...
		const FRHITextureCreateDesc Desc =
			FRHITextureCreateDesc::Create2D(GetTextureName(), 1, 1, PF_R8G8B8A8)
	...
	virtual const TCHAR* GetTextureName() const { return TEXT("ColoredTexture"); }
};
...
class FWhiteTexture : public FColoredTexture<255, 255, 255, 255>
{
	virtual const TCHAR* GetTextureName() const override { return TEXT("FWhiteTexture"); }
};

class FBlackTexture : public FColoredTexture<0, 0, 0, 0>
{
	virtual const TCHAR* GetTextureName() const override { return TEXT("FBlackTexture"); }
};
...
public:
	...
	virtual const TCHAR* GetTextureName() const override { return TEXT("FBlackTextureWithSRV"); }
};

FTextureWithSRV* GWhiteTextureWithSRV = new TGlobalResource<FWhiteTexture, FRenderResource::EInitPhase::Pre>;
FTextureWithSRV* GBlackTextureWithSRV = new TGlobalResource<FBlackTextureWithSRV, FRenderResource::EInitPhase::Pre>();
FTextureWithSRV* GTransparentBlackTextureWithSRV = new TGlobalResource<FBlackTexture, FRenderResource::EInitPhase::Pre>;
...
struct TDynamicBufferPool : public FRenderResource
{
	...
	DynamicBufferType* Acquire(FRHICommandListBase& RHICmdList, uint32 SizeInBytes, uint32 Stride)
	{
		...
		{
			UE_LOGF(LogRendererCore, Warning, "FGlobalDynamicVertexBuffer::Allocate(%u), will have allocated %u total this frame", SizeInBytes, TotalAllocatedMemory.load());
		}
