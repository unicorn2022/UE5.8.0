...
#pragma pack(pop)
	...
	/**
	 * Implements a proxy archive that serializes FName and FShaderHash as verbatim data or an index (if the same value is repeated).
	 */
	struct FIndexedHashAndFNameProxyArchive : public FNameAsStringIndexProxyArchive
	{
		using Super = FNameAsStringIndexProxyArchive;

		/** When a hash is first encountered, it is added to the table and saved verbatim, otherwise, its index is written. Indices can be looked up from this TSet since it is not compacted. */
		TSet<FShaderHash> HashesSeenOnSave;

		/** Table of hashes that is populated as the archive is being loaded. */
		TArray<FShaderHash> HashesLoaded;
		 ...
		 */
		FIndexedHashAndFNameProxyArchive(FArchive& InInnerArchive)
	...
	void SavePermutation(FArchive& Ar, const FPermsPerPSO& PermDescriptor, const UE::PipelineCacheUtilities::FPermutation& Perm, int64 TotalNumberOfShaderKeys)
	{
		...
		for (int32 Idx = 0; Idx < UE_ARRAY_COUNT(Perm.Slots); ++Idx)
		{
			...
			{
				checkf(Perm.Slots[Idx] < TotalNumberOfShaderKeys, TEXT("Slot %d contains impossible stable shader key index %d (more than %lld we have)"),
...
bool UE::PipelineCacheUtilities::LoadStableKeysFile(const FStringView& Filename, TArray<FStableShaderKeyAndValue>& InOutArray)
{
	...
	*Archive << NumHashes;
	...
	InOutArray.Reserve(Header.NumEntries);
	for (int64 Idx = 0; Idx < Header.NumEntries; ++Idx)
	{
		...
		*Archive << Item.PermutationId;
		...
		auto ReadHash = [&Hashes, &Item](FArchive& Archive, FShaderHash& OutHash) -> bool
			{
				...
				OutHash = Hashes[static_cast<int32>(HashIdx)];
				return true;
			};

		bool bOk = ReadHash(*Archive, Item.PipelineHash) && ReadHash(*Archive, Item.OutputHash);
		if (!bOk)
		{
			return false;
		}
...
uint32 UE::PipelineCacheUtilities::LoadMultipleStableKeysFiles(TArrayView<const FStringView> FileNames, FStableShaderSet& StableMap)
{
	TArray<TArray<FStableShaderKeyAndValue>> StableArrays;
	StableArrays.AddDefaulted(FileNames.Num());
	ParallelFor(FileNames.Num(), [&StableArrays, &FileNames](int32 Index) { LoadStableKeysFile(FileNames[Index], StableArrays[Index]); });
	...
	if (StableArrays.Num() > 0)
	{
		...
		{
			// If this file had any data, increase merged file counter
			MergedCount += StableArray.IsEmpty() ? 0 : 1;
...
bool UE::PipelineCacheUtilities::SaveStablePipelineCacheFile(const FString& OutputFilename, const TArray<FPermsPerPSO>& StableResults, const TArray<FStableShaderKeyAndValue>& StableShaderKeyIndexTable)
{
	...
	// the rest of the file is compressed
	{
		...
		UE::PipelineCacheUtilities::Private::FIndexedHashAndFNameProxyArchive MemWriter(PlainMemWriter);
		...
		for (const FPermsPerPSO& Item : StableResults)
		{
			...
			NewPSO.ComputeDesc.ComputeShader = FShaderHash();
			NewPSO.GraphicsDesc.VertexShader = FShaderHash();
			NewPSO.GraphicsDesc.FragmentShader = FShaderHash();
			NewPSO.GraphicsDesc.GeometryShader = FShaderHash();
			NewPSO.GraphicsDesc.MeshShader = FShaderHash();
			NewPSO.GraphicsDesc.AmplificationShader = FShaderHash();
			NewPSO.RayTracingDesc.ShaderHash = FShaderHash();
...
bool UE::PipelineCacheUtilities::LoadStablePipelineCacheFile(const FString& Filename, const FStableShaderSet& StableMap, TSet<FPipelineCacheFileFormatPSO>& OutPSOs, FName& OutTargetPlatform, int32& OutPSOsRejected, int32& OutPSOsMerged)
{
	...
	*Archive << Header;
	...
	if (Header.Magic != SupportedHeader.Magic)
	{
		UE_LOGF(LogPipelineCacheUtilities, Warning, "Rejecting %ls, wrong magic (0x%x vs expected 0x%x).", *Filename, int(Header.Magic), int(SupportedHeader.Magic));
		return false;
	}
	...
	if (Header.Version != SupportedHeader.Version)
	{
		UE_LOGF(LogPipelineCacheUtilities, Warning, "Rejecting %ls, version %d is not supported (current supported version is %d).", *Filename, int(Header.Version), int(SupportedHeader.Version));
		return false;
	}

	if (Header.Sizeof_FPipelineCacheFileFormatPSO != SupportedHeader.Sizeof_FPipelineCacheFileFormatPSO)
	{
		UE_LOGF(LogPipelineCacheUtilities, Warning, "Rejecting %ls, different size of FPipelineCacheFileFormatPSO, serialization issues possible (%u vs expected %u).", *Filename, Header.Sizeof_FPipelineCacheFileFormatPSO, SupportedHeader.Sizeof_FPipelineCacheFileFormatPSO);
		return false;
	}
	...
	UE::PipelineCacheUtilities::Private::FIndexedHashAndFNameProxyArchive MemReader(PlainMemReader);
	...
	TArray<FShaderHash> HashesForStableKeys;
...
#define UE_PCU_GET_SHADER_HASH_FOR_SLOT(ShaderFreq) \
				(PermGroup.ActivePerSlot[ShaderFreq] ? HashesForStableKeys[Perm.Slots[ShaderFreq]] : FShaderHash())
...
bool UE::PipelineCacheUtilities::LoadChunkInfo(const FString& Filename, const FString& InShaderFormat, int32& OutChunkId, FString& OutChunkedCacheFilename, TSet<FName>& OutPackages)
{
	...
	// check version
	{
		...
		{
			UE_LOGF(LogPipelineCacheUtilities, Warning, "Rejecting asset info file %ls: missing CacheChunkInfoVersion (damaged file?)",
				*Filename);
			return false;
		}
		...
		{
			UE_LOGF(LogPipelineCacheUtilities, Warning, "Rejecting cache chunk info file %ls: expected version %d, got unsupported version %d.",
	...
	// get ChunkID
	{
		...
		{
			UE_LOGF(LogPipelineCacheUtilities, Warning, "Rejecting asset info file %ls: missing ChunkId (damaged file?)",
				*Filename);
			return false;
		}
	...
	// find what SF-specific output files we committed to producing
	{
		...
		{
			UE_LOGF(LogPipelineCacheUtilities, Warning, "Rejecting cache chunk info info file %ls: missing OutputFilesPerFormat array (damaged file?)",
				*Filename);
			return false;
		}
		...
		for (int32 IdxPair = 0, NumPairs = OutputFilesPerFormatArray.Num(); IdxPair < NumPairs; ++IdxPair)
		{
			...
			{
				UE_LOGF(LogPipelineCacheUtilities, Warning, "Rejecting cache chunk info file %ls: OutputFilesPerFormat array contains unreadable mapping format #%d (damaged file?)",
					*Filename,
					IdxPair
				);
				return false;
			...
			{
				UE_LOGF(LogPipelineCacheUtilities, Warning, "Rejecting cache chunk info file %ls: OutputFilesPerFormat array contains unreadable ShaderFormat %d (damaged file?)",
					*Filename,
					IdxPair
				);
				return false;
			...
			if (ShaderFormat == InShaderFormat)
			{
				...
				{
					UE_LOGF(LogPipelineCacheUtilities, Warning, "Rejecting cache chunk info file %ls: OutputFilesPerFormat array contains unreadable OutputFile %d (damaged file?)",
						*Filename,
						IdxPair
					);
					return false;
		...
		{
			UE_LOGF(LogPipelineCacheUtilities, Warning, "Rejecting cache chunk info file %ls: unable to determine output file format for shader format %ls",
				*Filename,
				*InShaderFormat
			);
			return false;
	...
	// read package (asset) names
	{
		...
		{
			UE_LOGF(LogPipelineCacheUtilities, Warning, "Rejecting cache chunk info info file %ls: missing Packages array (damaged file?)",
				*Filename);
			return false;
		}
