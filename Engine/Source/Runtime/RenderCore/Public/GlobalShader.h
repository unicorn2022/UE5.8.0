...
public:
	...
	RENDERCORE_API bool ForEachCompiledPermutation(EShaderPlatform Platform, EShaderPermutationFlags PermutationFlags, TFunction<bool(int32)> Function);
};
...
private:
	...
	UE_DEPRECATED(5.8, "Specifying the shader platform is now required when getting a shader from the FGlobalShaderMapSection.")
		inline TShaderRef<FShader> GetShader(FShaderType* ShaderType, int32 PermutationId = 0) const
	{
		return GetShader(GetShaderPlatform(), ShaderType, PermutationId);
	}

	TShaderRef<FShader> GetShader(EShaderPlatform ShaderPlatform, FShaderType* ShaderType, int32 PermutationId = 0) const;
...
public:
	...
	TShaderRef<ShaderType> GetShader(int32 PermutationId) const
	{
		...
		checkf(Shader.IsValid(), TEXT("Failed to find permutation %d of shader type %s in Platform %s"), PermutationId, ShaderType::GetStaticType().GetName(), *LegacyShaderPlatformToShaderFormat(Platform).ToString());
...
RENDERCORE_API void PrimeGlobalShaderSourceHashCache(EShaderPlatform Platform, const ITargetPlatform* TargetPlatform);

inline void PrimeGlobalShaderSourceHashCache()
{
	EShaderPlatform Platform = GShaderPlatformForFeatureLevel[GMaxRHIFeatureLevel];
	PrimeGlobalShaderSourceHashCache(Platform, nullptr);
}
