...
{
	FIntRect DesktopCoordinates = FIntRect(0, 0, 0, 0);
	bool bHDRSupported = false;
	float MinimumLuminanceInNits = 0.f;
	float MaximumLuminanceInNits = 100.f;
	float MaximumFullFrameLuminanceInNits = 100.f;
	float HDRPaperWhiteInNits = 203.f;
	UE::Color::FColorSpace LimitingColorSpace = UE::Color::FColorSpace::GetSRGB();
};
