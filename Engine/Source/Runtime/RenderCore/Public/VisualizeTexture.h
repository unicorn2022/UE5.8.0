...
public:
	...
	RENDERCORE_API void BeginViewRenderThread(ERHIFeatureLevel::Type InFeatureLevel, int32 UniqueId, const TCHAR* Description, bool bIsSceneCapture, bool bIsReflectionCapture = false);
	...
	// Returns whether the given actor Name (from GetActorNameOrLabel()) is currently being visualized.
	RENDERCORE_API bool IsNamedView(const TCHAR* Name) const;

	// bUseConfig specifies that the active configuration settings should be used to visualize the texture.  This includes checking to
	// make sure the texture is active in the config, and pulling settings (such as mip level) from the config.
...
#define VISUALIZE_TEXTURE_BEGIN_VIEW(FeatureLevel, UniqueId, Description, bIsSceneCapture, bIsReflectionCapture) GVisualizeTexture.BeginViewRenderThread(FeatureLevel, UniqueId, Description, bIsSceneCapture, bIsReflectionCapture)
#define VISUALIZE_TEXTURE_END_VIEW() GVisualizeTexture.EndViewRenderThread()

#else
#define VISUALIZE_TEXTURE_BEGIN_VIEW(FeatureLevel, UniqueId, Description, bIsSceneCapture, bIsReflectionCapture) (void)0
