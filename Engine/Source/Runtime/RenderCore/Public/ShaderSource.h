...
public:
	...
	/* Returns the array which contains the compressed data for external population.
	 * Note that this is only intended to be used in a very specific situation (loading UBA-virtualized files
	 * in SCW) and will only work properly if DecompressedCharCount is correctly set.
	 */
	inline TArray<uint8>& EditCompressed() { return SourceCompressed; }
	...
	inline FMemoryView GetCompressed() const
	{
		checkf(IsCompressed(), TEXT("Requesting compressed shader source but source has not been compressed"));
		return FMemoryView(SourceCompressed.GetData(), (uint64)SourceCompressed.Num());
	}
