...
namespace PipelineCacheUtilities
{
	...
	* 
	* @param FileNames array of filenames (with path if needed)
	* @param Values destination for merged file values
	* @return Count of files read in and merged
	*/
	RENDERCORE_API uint32 LoadMultipleStableKeysFiles(TArrayView<const FStringView> FileNames, FStableShaderSet& Values);
