...
namespace UE::ShaderPermutationUtils
{
	...
	void FormatPermutationParameter(const typename TDimension::Type& E, FString& OutString, bool bFullNames, const TCHAR* Prefix)
	{
		...
		else if constexpr (std::is_base_of_v<FShaderPermutationBool<SHADER_SPECIALIZATION>, TDimension>)
		{
			if (bFullNames)
			{
				OutString.Appendf(TEXT("%s (%s) *spec*"), TDimension::DefineName, !FShaderPermutationBool<SHADER_SPECIALIZATION>::ToDefineValue(E) ? TEXT("false") : TEXT("true"));
			}
			else
			{
				OutString.Appendf(TEXT("%d"), static_cast<int32>(FShaderPermutationBool<SHADER_SPECIALIZATION>::ToDefineValue(E)));
			}
		}
		else if constexpr (std::is_base_of_v<FShaderPermutationBool<SHADER_PERMUTATION>, TDimension>)
		{
			if (bFullNames)
			{
				OutString.Appendf(TEXT("%s (%s)"), TDimension::DefineName, !FShaderPermutationBool<SHADER_PERMUTATION>::ToDefineValue(E) ? TEXT("false") : TEXT("true"));
			}
			else
			{
				OutString.Appendf(TEXT("%d"), static_cast<int32>(FShaderPermutationBool<SHADER_PERMUTATION>::ToDefineValue(E)));
			}
		}
		else
		{
			if (bFullNames)
			{
				OutString.Appendf(TEXT("%s (%d)%s"), TDimension::DefineName, static_cast<int32>(TDimension::ToDefineValue(E)), TDimension::IsSpecialization ? TEXT(" *spec*") : TEXT(""));
			}
			else
			{
	...
	int32 ConvertToUnspecializedId(const EShaderPlatform Platform, const DomainType& PermutationVector)
	{
		if (FDataDrivenShaderPlatformInfo::GetSupportsSpecializationConstants(Platform))
		{
			return DomainType::ToDimensionUnspecializedId(PermutationVector);
		}
		else
		{
			return DomainType::ToDimensionValueId(PermutationVector);
		}
	}
}
