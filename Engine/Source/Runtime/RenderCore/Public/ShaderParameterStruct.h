...
/** Generate a list of excluded resources from the specialization constants. */
using FResourceExclusionArray = TArray<uint16>;
using FResourceExclusionArrayView = TArrayView<uint16>;
extern RENDERCORE_API FResourceExclusionArray GetSpecializationResourceExclusions(const TShaderRef<FShader>& Shader);

/** Validates that all resource parameters of a shader are set. */
#if DO_CHECK
extern RENDERCORE_API void ValidateShaderParameters(const TShaderRef<FShader>& Shader, const FShaderParametersMetadata* ParametersMetadata, const void* ParametersData, const FResourceExclusionArrayView ExclusionList);
#else
inline void ValidateShaderParameters(const TShaderRef<FShader>& Shader, const FShaderParametersMetadata* ParametersMetadata, const void* ParametersData, const FResourceExclusionArrayView ExclusionList) {}
#endif
...
{
	ValidateShaderParameters(Shader, ParametersMetadata, ParametersData, GetSpecializationResourceExclusions(Shader));
}
...
{
	ValidateShaderParameters(Shader, TShaderClass::FParameters::FTypeInfo::GetStructMetadata(), &Parameters);
}
...
inline void SetShaderParameters(
	...
	const void* ParametersData)
{
	const FResourceExclusionArrayView NoResourceExclusions;
	SetShaderParameters(BatchedParameters, Bindings, ParametersMetadata, ParametersData, NoResourceExclusions);
}
...
//UE_DEPRECATED(5.8, "Calls to SetShaderParameters now require ResourceExclusions to be specified.")
inline void SetShaderParameters(FRHIComputeCommandList& RHICmdList, FRHIComputeShader* ShaderRHI, const FShaderParameterBindings& Bindings, const FShaderParametersMetadata* ParametersMetadata, const void* ParametersData)
{
	checkf(!ShaderRHI->HasSpecializationConstants(), TEXT("SetShaderParameters called with a shader using specialization constants, a ResourceExclusion array needs to be specified."));
	SetShaderParameters(RHICmdList, ShaderRHI, Bindings, ParametersMetadata, ParametersData, FResourceExclusionArrayView());
}
...
//UE_DEPRECATED(5.8, "Calls to SetShaderParameters now require ResourceExclusions to be specified.")
inline void SetShaderParameters(FRHICommandList& RHICmdList, FRHIGraphicsShader* ShaderRHI, const FShaderParameterBindings& Bindings, const FShaderParametersMetadata* ParametersMetadata, const void* ParametersData)
{
	checkf(!ShaderRHI->HasSpecializationConstants(), TEXT("SetShaderParameters called with a shader using specialization constants, a ResourceExclusion array needs to be specified."));
	SetShaderParameters(RHICmdList, ShaderRHI, Bindings, ParametersMetadata, ParametersData, FResourceExclusionArrayView());
}
...
//UE_DEPRECATED(5.8, "Calls to SetShaderParameters now require ResourceExclusions to be specified.")
inline void SetShaderParameters(FRHICommandList& RHICmdList, FRHIComputeShader* ShaderRHI, const FShaderParameterBindings& Bindings, const FShaderParametersMetadata* ParametersMetadata, const void* ParametersData)
{
	checkf(!ShaderRHI->HasSpecializationConstants(), TEXT("SetShaderParameters called with a shader using specialization constants, a ResourceExclusion array needs to be specified."));
	SetShaderParameters(RHICmdList, ShaderRHI, Bindings, ParametersMetadata, ParametersData, FResourceExclusionArrayView());
}
...
inline void SetShaderParameters(
	...
	const typename TShaderClass::FParameters& Parameters)
{
	FResourceExclusionArray ResourceExclusions = GetSpecializationResourceExclusions(Shader);

	ValidateShaderParameters(Shader, ParametersMetadata, &Parameters, ResourceExclusions);
	...
	SetShaderParameters(RHICmdList, ShaderRHI, Shader->Bindings, ParametersMetadata, &Parameters, ResourceExclusions);
}
