...
#define VIRTUALTEXTURE_INVALID_FORCE_SPACEID 0xFF
...
struct FAllocatedVTDescription
{
	...
	uint8 ForceSpaceID = VIRTUALTEXTURE_INVALID_FORCE_SPACEID;
	uint8 AdaptiveLevelBias = 0u;
	uint8 AllocatedAdaptiveBias = 0u;
	...
	friend inline bool operator==(const FAllocatedVTDescription& Lhs, const FAllocatedVTDescription& Rhs)
	{
		if (Lhs.TileSize != Rhs.TileSize ||
			...
			Lhs.AllocatedAdaptiveBias != Rhs.AllocatedAdaptiveBias ||
	...
	friend inline uint32 GetTypeHash(const FAllocatedVTDescription& Description)
	{
		...
		Hash = HashCombine(Hash, GetTypeHash(Description.AllocatedAdaptiveBias));
...
{
	// Disable bNotifyCompleted deprecation warnings here.
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	RENDERCORE_API FVTProducerDescription();
	FVTProducerDescription(const FVTProducerDescription&) = default;
	FVTProducerDescription& operator=(const FVTProducerDescription&) = default;
	...
	bool bTrackSamplingHistory = false;
	
	UE_DEPRECATED(5.8, "OnRequestsCompleted() callbacks are deprecated for future removal. They are currently disabled by default with r.VT.EnableNotifyCompleted.")
...
public:
	...
	*/
	...
	/** Called on every virtual texture system update once all requests are completed, if bNotifyCompleted is enabled and r.VT.EnableNotifyCompleted=1. */
	UE_DEPRECATED(5.8, "OnRequestsCompleted() callbacks are deprecated for future removal.")
...
public:
	...
	inline uint32 GetAllocatedAdaptiveBias() const { return Description.AllocatedAdaptiveBias; }
...
protected:
	...
	virtual ~IAllocatedVirtualTexture() = default;
...
protected:
	friend class FVirtualTextureSystem;
	virtual ~IAdaptiveVirtualTexture() = default;
