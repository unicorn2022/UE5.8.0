 ...
 */
	...
	#define RDG_EVENT_SCOPE_IMPL(GraphBuilder, Condition, ScopeFlags, GPUStatArgs, StaticName, FormatString, ...)	\
		TOptional<TRDGEventScopeGuard<FRDGScope_RHI>> UE_JOIN(__RDG_ScopeRef_, __LINE__);							\
		RDG_EVENT_SCOPE_CONSTRUCT(																					\
			  UE_JOIN(__RDG_ScopeRef_, __LINE__)																	\
			, GraphBuilder																							\
			, Condition																								\
			, ScopeFlags																							\
			, GPUStatArgs																							\
...
#define RDG_GPU_STAT_SCOPE(GraphBuilder, StatName)                          UE_DEPRECATED_MACRO(5.8, "The legacy GPU profiler has been removed from the engine. RDG_GPU_STAT_SCOPE"      " has been deprecated, and now does nothing. GPU stats are handled via the _STAT versions of RDG_EVENT_SCOPE macros, e.g. RDG_EVENT_SCOPE_STAT.")
#define RDG_GPU_STAT_SCOPE_VERBOSE(GraphBuilder, StatName, Description)     UE_DEPRECATED_MACRO(5.8, "The legacy GPU profiler has been removed from the engine. RDG_GPU_STAT_SCOPE_VERBOSE has been deprecated, and now does nothing. GPU stats are handled via the _STAT versions of RDG_EVENT_SCOPE macros, e.g. RDG_EVENT_SCOPE_STAT.")

#if CSV_PROFILER_STATS
	#define RDG_CSV_STAT_EXCLUSIVE_SCOPE(GraphBuilder, StatName) TRDGEventScopeGuard<FRDGScope_CSVExclusive> UE_JOIN(__RDG_CSVStat_##StatName,__LINE__) ((GraphBuilder), ERDGScopeFlags::AlwaysEnable, #StatName);
#else
...
#define RDG_RHI_EVENT_SCOPE_CONDITIONAL_STAT(GraphBuilder, Stat, Name, Condition) RDG_EVENT_SCOPE_CONDITIONAL_STAT(GraphBuilder, Condition, Stat, #Name); RHI_BREADCRUMB_EVENT_CONDITIONAL_STAT(GraphBuilder.RHICmdList, Stat, Condition, #Name)

#define RDG_RHI_GPU_STAT_SCOPE(  GraphBuilder, StatName)   UE_DEPRECATED_MACRO(5.8, "The legacy GPU profiler has been removed from the engine. RDG_RHI_GPU_STAT_SCOPE has been deprecated, and now does nothing. Use RDG_RHI_EVENT_SCOPE_STAT instead.")
