...
{
public:
	virtual ~TRDGUniformBuffer() = default;
...
protected:
	...
	// Use explicit booleans due to access from the render thread while the async setup queue task is running.

	/** Whether any sub-resource has been used for write by a pass. Separate from the bitfield because it is assigned by the async setup queue. */
...
class FRDGPooledBuffer final
	: public UE::Private::FQueryableRefCountedObject
{
public:
...
public:
	...
	inline uint64 GetCommittedSize() const
	{
		return PooledBuffer ? PooledBuffer->GetCommittedSize() : 0;
	}
