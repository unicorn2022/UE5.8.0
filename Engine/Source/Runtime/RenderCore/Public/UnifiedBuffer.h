...
public:
	...
	template<typename ResourceType>
	void ResourceUploadTo(FRHICommandList& RHICmdList, const ResourceType& DstBuffer);

	template<typename ResourceType>
	UE_DEPRECATED(5.8, "ResourceUploadTo with the optional bFlush argument is deprecated. If submission of GPU work is required immediately afterwards, the caller should do this via the immediate command list.")
	void ResourceUploadTo(FRHICommandList& RHICmdList, const ResourceType& DstBuffer, bool /*unused bFlush*/)
	{
		ResourceUploadTo(RHICmdList, DstBuffer);
	}
...
public:
	...
	{
		check(NumScatters + ElementScatterOffsets.Num() <= MaxScatters);
	...
	void* Add_GetRef(uint32 Index, uint32 Num = 1)
	{
		check(NumScatters + Num <= MaxScatters);
		check(ScatterData != nullptr);
		check(UploadData != nullptr);
	...
	void* Set_GetRef(uint32 ElementIndex, uint32 ElementScatterOffset, uint32 Num = 1)
	{
		check(ElementIndex + Num <= MaxScatters);
		check(ScatterData != nullptr);
		check(UploadData != nullptr);
...
{
public:
	RENDERCORE_API void Lock(FRHICommandListBase& RHICmdList, uint32 ActualMaxScatters);

	void Lock(FRHICommandListBase& RHICmdList)
	{
		Lock(RHICmdList, MaxScatters);
	}
...
public:
	...
	using FCommandListFunction = TFunction<void(FRHICommandList&)>;
	...
	/** AddPass overload with deferred scatter count. GetMaxScattersFunction runs after prerequisite tasks
	  * but before Lock, and returns the actual maximum number of scatters. Must be <= MaxNumElements. */
	void AddPass(
		FRDGBuilder& GraphBuilder,
		FRDGAsyncScatterUploadBuffer& UploadBuffer,
		FRDGViewableResource* DstResource,
		...
		FPassFunction&& Function)
	{
		Passes.Emplace(UploadBuffer, UploadBuffer.Begin(GraphBuilder, DstResource, MaxNumElements, NumBytesPerElement, Name), MoveTemp(Function));
		Passes.Last().GetMaxScattersFunction = MoveTemp(GetMaxScattersFunction);
		MaxBytes += MaxNumElements * NumBytesPerElement;
	}

	void AddFunction(FCommandListFunction&& InFunction)
	{
		CommandListFunctions.Emplace(MoveTemp(InFunction));
	}

	void AddPrerequisiteTask(const UE::Tasks::FTask& Task)
	{
		PrerequisiteTasks.Emplace(Task);
	}
...
private:
	...
	TArray<FCommandListFunction, SceneRenderingAllocator> CommandListFunctions;
	TArray<UE::Tasks::FTask, SceneRenderingAllocator> PrerequisiteTasks;
	uint32 MaxBytes = 0;
