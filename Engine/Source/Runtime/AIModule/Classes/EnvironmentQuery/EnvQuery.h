...
public:
	/** Set to true to not cook this query into client builds. Useful for server-only AI queries. */
	UPROPERTY(EditAnywhere, Category=Query)
	uint8 bStripFromClientBuilds : 1 = false;
	...
	{
		return !bStripFromClientBuilds && Super::NeedsLoadForClient();
	}
	virtual bool NeedsLoadForEditorGame() const override
	{
		return !bStripFromClientBuilds && Super::NeedsLoadForEditorGame();
	}
