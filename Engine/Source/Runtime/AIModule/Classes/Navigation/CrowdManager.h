...
struct FCrowdAvoidanceConfig
{
	...
	/** Threshold for side selection, larger values widen the dead zone for head-on encounters */
	UPROPERTY(EditAnywhere, Category=Crowd, meta=(ClampMin="0.001", ClampMax="1.0"))
	float SideBiasThreshold;

	/** Weight for preferring right-side passing, reduces oscillation (0 = disabled) */
	UPROPERTY(EditAnywhere, Category=Crowd, meta=(ClampMin="0.0"))
	float RightSideBias;
	...
	FCrowdAvoidanceConfig() :
		...
		SideBiasThreshold(0.01f), RightSideBias(0.0f),
