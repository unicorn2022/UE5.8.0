...
public:
	...
	/** If true, the RootComponent's transform will not be updated when SetControlRotation() is called. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = AI)
	uint32 bSkipRootComponentUpdate:1;
...
public:
	...
	/** Set the control rotation. The RootComponent's rotation will also be updated to match
	  * if bSkipRootComponentUpdate is false and RootComponent->bAbsoluteRotation is true. */
	AIMODULE_API virtual void SetControlRotation(const FRotator& NewRotation) override;
