...
namespace FEQSHelpers
{
	const ANavigationData* FindNavigationDataForQuery(FEnvQueryInstance& QueryInstance, TSubclassOf<UEnvQueryContext> NavDataOverrideContext)
	{
			...
			if (QueryInstance.PrepareContext(NavDataOverrideContext, NavDataCandidateActors))
			{
				for (const AActor* NavDataCandidateActor : NavDataCandidateActors)
				{
					if (const ANavigationData* NavDataCandidate = Cast<ANavigationData>(NavDataCandidateActor))
					{
						...
						return NavDataCandidate;
					}
					if (NavSys)
					{
						// Check if this actor use a NavData that could be used
						if (const INavAgentInterface* NavAgent = Cast<INavAgentInterface>(QueryInstance.Owner.Get()))
						{
							const FNavAgentProperties& NavAgentProps = NavAgent->GetNavAgentPropertiesRef();
							if (NavAgentProps.IsValid() || NavAgentProps.PreferredNavData.IsValid())	// test that to avoid GetNavDataForProps warning
							{
								if (const ANavigationData* AgentNavData = NavSys->GetNavDataForProps(NavAgent->GetNavAgentPropertiesRef()))
								{
									return AgentNavData;
								}
							}
