	...
	if (VerNum < EnvQueryTestVersion::ReferenceValueFix && bDefineReferenceValue)
	{
		...
		if (World == nullptr)
		{
			...
			else if (ScoringEquation == EEnvTestScoreEquation::Linear)
			{
				...
				UE_LOGF(LogEQS, Verbose, "%ls using a AIDataProvider for ScoringFactor. Due to a bug-fix the effect will be flipped so ScoringEquation has been automatically flipped from Linear to InverseLinear for this EQS Test instance."
					, *GetPathName());
			}
			else if (ScoringEquation == EEnvTestScoreEquation::InverseLinear)
			{
				// we can still address this by flipping the ScoringEquation to 
				ScoringEquation = EEnvTestScoreEquation::Linear;
				UE_LOGF(LogEQS, Verbose, "%ls using a AIDataProvider for ScoringFactor. Due to a bug-fix the effect will be flipped so ScoringEquation has been automatically flipped from InverseLinear to Linear for this EQS Test instance."
					, *GetPathName());
			}
			else
			{
				UE_LOGF(LogEQS, Warning, "%ls using a AIDataProvider for ScoringFactor. Due to a bug-fix the effect will be flipped and an automated change could not be performed. Please address manually."
					, *GetPathName());
			}

			if (MarkPackageDirty() == false)
			{
				UE_LOGF(LogEQS, Warning, "%ls unable to mark package dirty - please resave manually or use a commandlet."
					, *GetPathName());
			}
		}
	}
