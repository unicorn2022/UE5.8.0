...
namespace FEQSHelpers
{
	...
	struct FBatchTrace
	{
		...
		{
			UE_LOGF(LogEQS, Error, "FBatchTrace::DoSingleSourceMultiDestinations called with unhandled trace type: %d", int32(TraceType));
		}
		...
		{
			UE_LOGF(LogEQS, Error, "FBatchTrace::DoMultiSourceMultiDestinations2D called with unhandled trace type: %d", int32(TraceType));
		}
