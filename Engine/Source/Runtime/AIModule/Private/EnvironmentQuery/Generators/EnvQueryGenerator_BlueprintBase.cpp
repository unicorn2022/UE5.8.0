	...
	: Super(ObjectInitializer)
{
	...
	{
		UE_LOGF(LogEQS, Error, "Blueprint Generator class %ls doesn't override DoItemGeneration or DoItemGenerationFromActors and can't generate anything.", *GetNameSafe(GetClass()));
	}
}
...
void UEnvQueryGenerator_BlueprintBase::GenerateItems(FEnvQueryInstance& QueryInstance) const
{
	...
	{
		UE_LOGF(LogEQS, Warning, "Blueprint generator %ls with class %ls doesn't override DoItemGeneration or DoItemGenerationFromActors and can't generate anything.", *GetName(), *GetNameSafe(GetClass()));
	}
...
void UEnvQueryGenerator_BlueprintBase::AddGeneratedVector(FVector Vector) const
{
	...
	{
		UE_LOGF(LogEQS, Error, "Trying to generate a Vector item while generator %ls is configured to produce Actor items"
			, *GetName());
	}
}
...
void UEnvQueryGenerator_BlueprintBase::AddGeneratedActor(AActor* Actor) const
{
	...
	{
		UE_LOGF(LogEQS, Error, "Trying to generate an Actor item while generator %ls is configured to produce Vector items. Will use Actor\'s location, but please update your BP code."
			, *GetName());
		if (Actor)
		{
