...
UEnvQueryGenerator_Donut::UEnvQueryGenerator_Donut(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	...
	PointOnRingSpacingMethod = EEnvQueryPointSpacingMethod::ByNumberOfPoints;
	SpaceBetweenPoints.DefaultValue = 50.f;
...
bool UEnvQueryGenerator_Donut::GenerateDonutNavPoints(const TArray<FVector>& CenterPoints, TArray<FNavLocation>& Points, FEnvQueryInstance& QueryInstance) const
{
	...
	SpaceBetweenPoints.BindData(BindOwner, QueryInstance.QueryID);
	ArcAngle.BindData(BindOwner, QueryInstance.QueryID);

	const FVector::FReal ArcAngleValue = ArcAngle.GetValue();
	const FVector::FReal InnerRadiusValue = InnerRadius.GetValue();
	const FVector::FReal OuterRadiusValue = OuterRadius.GetValue();
	const int32 NumRings = NumberOfRings.GetValue();

	if ((InnerRadiusValue < 0.) || (OuterRadiusValue <= 0.) ||
		(InnerRadiusValue > OuterRadiusValue) || (NumRings < 1))
	{
		return false;
	}
	...
	const FVector::FReal RadiusDelta = NumRings > 1 ? (OuterRadiusValue - InnerRadiusValue) / (NumRings - 1) : 0.;

	// Handle the two spacing methods differently
	if (PointOnRingSpacingMethod == EEnvQueryPointSpacingMethod::ByNumberOfPoints)
	{
		// ByNumberOfPoints: All rings have the same point count
		const int32 NumPoints = PointsPerRing.GetValue();
		if (NumPoints < 1)
		{
			return false;
		}
	...
	{
		// BySpaceBetween: Each ring has different point count based on its circumference
		const FVector::FReal SpaceBetweenPointsValue = SpaceBetweenPoints.GetValue();
		if (SpaceBetweenPointsValue <= 0.0)
		{
			return false;
		}
		...
		// Reserve approximate space (use outer ring's count as upper bound)
		const FVector::FReal OuterArcLength = 2.0 * UE_DOUBLE_PI * OuterRadiusValue * ArcAngleDeg / 360.0;
		const int32 ApproxMaxPoints = FMath::CeilToInt(OuterArcLength / SpaceBetweenPointsValue) + 1;
		Points.Reserve(ApproxMaxPoints * NumRings * CenterPoints.Num());

		FVector::FReal SectionAngle = FMath::DegreesToRadians(ArcBisectDeg);
		// Generate points for each ring independently
		for (int32 RingIdx = 0; RingIdx < NumRings; RingIdx++, RingRadius += RadiusDelta)
		{
			// Calculate point count for this specific ring
			const FVector::FReal RingLength = 2.0 * UE_DOUBLE_PI * RingRadius;
			const int32 RingPointCount = FMath::CeilToInt(RingLength / SpaceBetweenPointsValue) + 1;

			// Generate points evenly distributed around this ring
			const FVector::FReal RingAngleDelta = 2.0 * UE_DOUBLE_PI / RingPointCount;

			for (int32 PointIdx = 0; PointIdx < RingPointCount; PointIdx++, SectionAngle += RingAngleDelta)
			{
				if (IsAngleAllowed(SectionAngle, ArcBisectDeg, ArcAngleDeg, bDefineArc))
				{
					const FVector::FReal SinValue = FMath::Sin(SectionAngle);
			...
			// for spiral pattern we keep the section angle from one ring to the other. Else we just reset it
			SectionAngle = bUseSpiralPattern ? (SectionAngle + RingAngleDelta) : FMath::DegreesToRadians(ArcBisectDeg);
		}
	}
...
FText UEnvQueryGenerator_Donut::GetDescriptionDetails() const
{
	...
	if (PointOnRingSpacingMethod == EEnvQueryPointSpacingMethod::ByNumberOfPoints)
	{
		...
		Desc = FText::Format(LOCTEXT("DonutDescriptionByCount", "radius: {InnerRadius} to {OuterRadius}\nrings: {NumRings}, points per ring: {NumPerRing}"), Args);
	}
	else
	{
		Args.Add(TEXT("Spacing"), FText::FromString(SpaceBetweenPoints.ToString()));
		Desc = FText::Format(LOCTEXT("DonutDescriptionBySpacing", "radius: {InnerRadius} to {OuterRadius}\nrings: {NumRings}, spacing: {Spacing} (points vary per ring)"), Args);
	}
...
void UEnvQueryGenerator_Donut::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	if (PropertyChangedEvent.Property != NULL)
	{
		...
		else if (PropName == GET_MEMBER_NAME_CHECKED(UEnvQueryGenerator_Donut, SpaceBetweenPoints))
		{
			SpaceBetweenPoints.DefaultValue = FMath::Max(1.0f, SpaceBetweenPoints.DefaultValue);
		}
	}
