...
void UAIPerceptionSystem::OnListenerConfigUpdated(FAISenseID SenseID, const UAIPerceptionComponent& Listener)
{
	...
	{
		UE_LOGF(LogAIPerception, Warning, "Sense must exist to update its sense config");
		return;
	}
	...
	{
		UE_LOGF(LogAIPerception, Warning, "Listener must have a valid id to update its sense config");
		return;
	}
...
{
	UE_CLOGF(NoiseMaker == nullptr && NoiseInstigator == nullptr, LogAIPerception, Warning
		, "UAIPerceptionSystem::MakeNoiseImpl called with both NoiseMaker and NoiseInstigator being null. Unable to resolve UWorld context!");
