...
float UAISense_Sight::Update()
{
	...
	for (int32 QueryIndex = 0; QueryIndex < SightQueriesInRange.Num() + SightQueriesOutOfRange.Num(); ++QueryIndex)
	{
		...
		if (TargetActor && ListenerPtr)
		{
			...
			{
				UE_CLOGF(VisibilityResult != EVisibilityResult::Visible && VisibilityResult != EVisibilityResult::NotVisible, LogAIPerception, Error, "UAISense_Sight::Update received invalid Visibility result [%d] for query between Listener %ls and Target %ls. We'll consider it as NotVisible", int(VisibilityResult), *GetNameSafe(ListenerBodyActor), *GetNameSafe(TargetActor));
