...
// 	UE_LOGF(LogPath, Warning, "DistanceToOtherSquared = %f, SightRadiusSquared: %f", SelfToOtherDistSquared, FMath::Square(SightRadius));

	// check field of view
	FVector const SelfToOtherDir = SelfToOther.GetSafeNormal();
	FVector const MyFacingDir = GetSensorRotation().Vector();

// 	UE_LOGF(LogPath, Warning, "DotProductFacing: %f, PeripheralVisionCosine: %f", SelfToOtherDir | MyFacingDir, PeripheralVisionCosine);
