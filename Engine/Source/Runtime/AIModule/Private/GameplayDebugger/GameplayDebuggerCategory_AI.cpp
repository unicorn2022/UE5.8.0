...
void FGameplayDebuggerCategory_AI::CollectData(APlayerController* OwnerPC, AActor* DebugActor)
{
	...
	DataPack.PawnName = MyPawn ? MyPawn->GetHumanReadableName() : FString("{red}No selected pawn.");
	...
	if (MyPawn && IsValidChecked(MyPawn))
	{
		...
		if (CharMovementComp)
		{
			const FMovementBaseInterfaceData* MovementBaseData = MyPawn->GetMovementBaseInterfaceData();
			const UObject* FloorObject = MovementBaseData && MovementBaseData->IsValid() ? MovementBaseData->GetMovementBaseObjectOwner() : nullptr;
			DataPack.MovementBaseInfo = MovementBaseData && MovementBaseData->GetMovementBaseObject() ? FString::Printf(TEXT("%s.%s"), *GetNameSafe(FloorObject), *MovementBaseData->GetMovementBaseObject()->GetName()) : FString("None");
...
void FGameplayDebuggerCategory_AI::DrawData(APlayerController* OwnerPC, FGameplayDebuggerCanvasContext& CanvasContext)
{
	...
	if (DataPack.bIsUsingCharacter)
	{
		...
		if (IsCategoryLocal() && !IsCategoryAuth())
		{
			...
			{
				const FMovementBaseInterfaceData* MovementBaseData = MyChar->GetMovementBaseInterfaceData();
				const UObject* FloorObject = MovementBaseData && MovementBaseData->IsValid() ? MovementBaseData->GetMovementBaseObjectOwner() : nullptr;
				const FString ClientMovementBaseInfo = MovementBaseData && MovementBaseData->GetMovementBaseObject() ? FString::Printf(TEXT("%s.%s"), *GetNameSafe(FloorObject), *MovementBaseData->GetMovementBaseObject()->GetName()) : FString("None");
...
{
	FString FailsafeIcon = "/Engine/EngineResources/AICON-Green.AICON-Green";
