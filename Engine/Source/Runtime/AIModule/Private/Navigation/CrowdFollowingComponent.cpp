...
void UCrowdFollowingComponent::UpdateCachedDirections(const FVector& NewVelocity, const FVector& NextPathCorner, bool bTraversingLink)
{
	...
	// Cache bTraversingLink
	bIsTraversingLink = bTraversingLink;
...
bool UCrowdFollowingComponent::IsFollowingNavLink() const
{
	if (IsCrowdSimulationEnabled())
	{
		return bIsTraversingLink;
	}
	else
	{
		return Super::IsFollowingNavLink();
	}
}
