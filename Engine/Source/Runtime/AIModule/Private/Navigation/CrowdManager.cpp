...
void UCrowdManager::RegisterAgent(ICrowdAgentInterface* Agent)
{
	if (Agent == nullptr)
	{
		UE_LOGF(LogCrowdFollowing, Warning, "%s, Trying to add an invalid agent interface pointer.", __FUNCTION__);
		return;
	}
...
void UCrowdManager::CreateCrowdManager()
{
	if (MyNavData == nullptr)
	{
		...
		UE_CLOGF(MyNavData == nullptr, LogCrowdFollowing, Warning, "Unable to find RecastNavMesh instance while trying to create UCrowdManager instance");
		return;
	}
...
void UCrowdManager::UpdateAvoidanceConfig()
{
	...
	for (int32 Idx = 0; Idx < AvoidanceConfig.Num(); Idx++)
	{
		...
		params.sideBiasThreshold = ConfigInfo.SideBiasThreshold;
		params.weightRightBias = ConfigInfo.RightSideBias;
