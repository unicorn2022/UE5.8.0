...
#if WITH_EDITOR
	// During loading, those will be registered to in postload instead for thread-safety
	if (GIsEditor && !HasAnyFlags(RF_ClassDefaultObject) && !IsInAsyncLoadingThread())
	{
...
void ANavLinkProxy::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent) 
{
	...
	static const FName NAME_AreaClass = "AreaClass";
