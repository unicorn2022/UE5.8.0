...
{
	UE_CLOGF(BehaviorAsset == nullptr, LogBehaviorTree, Warning, "BTTask_RunBehaviorDynamic node (\"%ls\") executed with no BehaviorAsset"
		, *GetNodeName());
	UE_CLOGF(BehaviorAsset != nullptr && BehaviorAsset->RootDecorators.Num() > 0, LogBehaviorTree
		, Error, "BTTask_RunBehaviorDynamic node (\"%ls\") executed with a BehaviorAsset (\"%ls\") containing root level decorators. These decorators will be ignored by design."
...
const UObject* UBTTask_RunBehaviorDynamic::GetAssociatedAsset(TOptional<FBehaviorTreeNodeDebugContext> DebugContext) const
{
	...
	{
		static const FRegexPattern RegexPatern(FString("[\\n\\r].*subtree path:\\s*([^\\n\\r]*)"));
