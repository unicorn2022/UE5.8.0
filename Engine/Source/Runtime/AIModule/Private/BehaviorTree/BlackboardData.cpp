...
bool UBlackboardData::IsValid() const
{
	if (Parent)
	{
		for (int32 KeyIndex = 0; KeyIndex < Keys.Num(); KeyIndex++)
		{
			...
			if (KeyID != FBlackboard::InvalidKey)
			{
				UE_LOGF(LogBehaviorTree, Error, "Blackboard asset (%ls) has duplicated key (%ls) in parent chain!",
...
void UBlackboardData::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyChangedEvent.Property)
	{
		if (PropertyChangedEvent.Property->GetFName() == NAME_Parent)
		{
			...
			{
				UE_LOGF(LogBehaviorTree, Warning, "Blackboard asset (%ls) has (%ls) in parent chain! Clearing value to avoid cycle.",
