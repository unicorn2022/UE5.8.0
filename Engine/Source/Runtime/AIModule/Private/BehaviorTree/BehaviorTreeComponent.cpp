							 ...
							 const UBTCompositeNode*& CommonParentNode, uint16& CommonInstanceIdx)
{
	...
	if (!NodeA || !NodeB)
	{
		...
		UE_LOGF(LogBehaviorTree, Fatal, "Fatal error in FindCommonParent() call.\nInNodeA: %ls, InstanceIdxA: %d (%ls), NodeA: %ls\nInNodeB: %ls, InstanceIdxB: %d (%ls), NodeB: %ls\nCommonInstanceIdx: %d (%ls), ActiveNode: %ls%ls",
...
{
	uint8* NodeMemory = InstanceStack.IsValidIndex(InstanceIdx) ? (uint8*)Node->GetNodeMemory<uint8>(InstanceStack[InstanceIdx]) : nullptr;
	return NodeMemory ? NodeMemory : GetNodeMemoryAux(Node, InstanceIdx);
}

uint8* UBehaviorTreeComponent::GetNodeMemoryAux(const UBTNode* Node, int32 InstanceIdx) const
{
	return nullptr;
}
