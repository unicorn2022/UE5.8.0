...
AActor* UBTNode::GetGameplayTaskOwner(const UGameplayTask* Task) const
{
	if (Task == nullptr)
	{
		...
		{
			UE_LOGF(LogBehaviorTree, Warning, "%ls: Unable to determine default GameplayTaskOwner!", *GetName());
			return nullptr;
		}
	}
...
AActor* UBTNode::GetGameplayTaskAvatar(const UGameplayTask* Task) const
{
	if (Task == nullptr)
	{
		...
		{
			UE_LOGF(LogBehaviorTree, Warning, "%ls: Unable to determine default GameplayTaskAvatar!", *GetName());
			return nullptr;
		}
	}
...
void UBTNode::OnGameplayTaskInitialized(UGameplayTask& Task)
{
	...
	if (AITask && (AITask->GetAIController() == nullptr))
	{
		...
		UE_LOGF(LogBehaviorTree, Error, "Missing AIController in AITask %ls", *AITask->GetName());
	}
}
