...
void FBlackboardKeySelector::ResolveSelectedKey(const UBlackboardData& BlackboardAsset)
{
	if (SelectedKeyName.IsNone() == false || !bNoneIsAllowedValue)
	{
		...
		UE_CLOGF(IsSet() == false, LogBehaviorTree, Warning
			, "%ls> Failed to find key \'%ls\' in BB asset %ls. BB Key Selector will be set to \'Invalid\'"
	...
	if(!SelectedKeyName.IsNone())
	{
		...
		if (Id != FBlackboard::InvalidKey)
		{
			if (const FBlackboardEntry* BlackboardEntry = Blackboard.GetKey(Id))
			{
				...
				UE_LOGF(LogBehaviorTree, Warning, "%ls in node %ls in bt %ls is bound to key %ls in blackboard %ls but the key type isn't allowed. %ls", *PropertyName.ToString(), *GetNameSafe(Outer), Outer ? *GetNameSafe(Outer->GetTypedOuter<UBehaviorTree>()) : TEXT("None"), *SelectedKeyName.ToString(), *Blackboard.GetFullName(), ResetValueMessage);
			}
			else
			{
				UE_LOGF(LogBehaviorTree, Warning, "%ls in node %ls in bt %ls is bound to key %ls in blackboard %ls but the key doesn't exist. %ls", *PropertyName.ToString(), *GetNameSafe(Outer), Outer ? *GetNameSafe(Outer->GetTypedOuter<UBehaviorTree>()) : TEXT("None"), *SelectedKeyName.ToString(), *Blackboard.GetFullName(), ResetValueMessage);
			}
		}
		else
		{
