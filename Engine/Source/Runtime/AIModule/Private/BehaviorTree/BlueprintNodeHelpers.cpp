...
#define GET_STRUCT_NAME_CHECKED(StructName) \
	...
	{
		UE_LOGF(LogBehaviorTree, Verbose, "Looking for runtime properties of class: %ls", *GetNameSafe(Ob->GetClass()));
		...
		for (FProperty* TestProperty = Ob->GetClass()->PropertyLink; TestProperty; TestProperty = TestProperty->PropertyLinkNext)
		{
			...
			{
				UE_LOGF(LogBehaviorTree, Verbose, "> name: '%ls'", *GetNameSafe(TestProperty));
