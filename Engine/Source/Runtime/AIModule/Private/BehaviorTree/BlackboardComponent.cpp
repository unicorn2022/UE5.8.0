...
bool UBlackboardComponent::InitializeBlackboard(UBlackboardData& NewAsset)
{
	...
	// in resetting, since we'd lose all the accumulated knowledge
	...
	ValueMemory.Reset();
	ValueOffsets.Reset();
	bBlackboardMemoryInitialized = false;
	bSynchronizedKeyPopulated = false;
	...
	if (BlackboardAsset->IsValid())
	{
		...
		bBlackboardMemoryInitialized = true;
	}
	else
	{
		UE_LOGF(LogBlackboard, Error, "Blackboard asset (%ls) has errors and can't be used!", *GetNameSafe(BlackboardAsset));
	}

	return bBlackboardMemoryInitialized;
}

void UBlackboardComponent::DestroyValues()
{
	if (!bBlackboardMemoryInitialized || 
		!ensureMsgf(ValueMemory.Num() > 0 && ValueOffsets.Num() > 0, TEXT("The Blackboard is initialized but no memory was allocated to store values for Blackboard asset: %s"), *GetNameSafe(BlackboardAsset)))
	{
		return;
	}
	...
	for (UBlackboardData* It = BlackboardAsset; It; It = It->Parent)
	{
		for (int32 KeyIndex = 0; KeyIndex < It->Keys.Num(); KeyIndex++)
		{
			if (UBlackboardKeyType* KeyType = It->Keys[KeyIndex].KeyType)
			{
				const int32 OffsetKey = KeyIndex + static_cast<int32>(It->GetFirstKeyID());
				check(FBlackboard::FKey(OffsetKey) != FBlackboard::InvalidKey);
				...
				{
					UE_LOGF(LogBlackboard, Warning, 
						   "DestroyValues skipping key '%ls' - no memory allocated. Blackboard asset (%ls) may have failed initialization.", 
						   *It->Keys[KeyIndex].EntryName.ToString(), *GetNameSafe(BlackboardAsset));
				}
			}
		}
	}
