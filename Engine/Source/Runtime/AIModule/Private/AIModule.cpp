...
{ 
	...
	if (GIsEditor)
	{
		...
		AIAssetCategoryBit = AssetTools.RegisterAdvancedAssetCategory(FName("Artificial Intelligence"), LOCTEXT("AIAssetCategory", "Artificial Intelligence"));
	}
#endif // WITH_EDITOR 
...
{
	UE_LOGF(LogAIModule, Verbose, "Creating AISystem for world %ls", *GetNameSafe(World));
	...
	if (AISystemClassName.IsValid())
	{
		...
		{
			UE_LOGF(LogAIModule, Error, "Failed to create AISystem instance of class %ls!", *AISystemClassName.ToString());
		}
	}
