...
UAIAsyncTaskBlueprintProxy* UAIBlueprintHelperLibrary::CreateMoveToProxyObject(UObject* WorldContextObject, APawn* Pawn, FVector Destination, AActor* TargetActor, float AcceptanceRadius, bool bStopOnOverlap)
{
	...
	if (WorldContextObject == nullptr)
	{
		...
		{
			UE_LOGF(LogAIBlueprint, Warning, "Empty (None) world context as well as Pawn passed in while trying to create a MoveTo proxy");
			return nullptr;
		}
	}
	...
	// Make sure the controller is still a valid Actor that can be stored in a weak pointer.
	// This early validation is a tentative fix for low occurence crashes where the AAIController
	// pointer is considered valid by IsValid() (which validates for non-null ptr and without Garbage flag)
	// but then TWeakObjectPtr<>::Get would return null (potentially unreachable) when trying to access
	// the ReceiveMoveCompleted delegate below.
	// We don't want to perform the MoveTo at all in that case.
	const TWeakObjectPtr WeakAIController = Cast<AAIController>(Pawn->GetController());
	const TStrongObjectPtr<AAIController> AIController = WeakAIController.Pin();
	if (AIController != nullptr)
	{
		...
		case EPathFollowingRequestResult::RequestSuccessful:
			MyObj->AIController = WeakAIController;
...
void UAIBlueprintHelperLibrary::SimpleMoveToActor(AController* Controller, const AActor* Goal)
{
	...
	{
		UE_LOGF(LogNavigation, Warning, "UNavigationSystemV1::SimpleMoveToActor called for NavSys:%ls Controller:%ls controlling Pawn:%ls with goal actor %ls (if any of these is None then there's your problem",
...
void UAIBlueprintHelperLibrary::SimpleMoveToLocation(AController* Controller, const FVector& GoalLocation)
{
	...
	{
		UE_LOGF(LogNavigation, Warning, "UNavigationSystemV1::SimpleMoveToActor called for NavSys:%ls Controller:%ls controlling Pawn:%ls (if any of these is None then there's your problem",
