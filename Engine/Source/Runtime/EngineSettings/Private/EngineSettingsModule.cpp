...
FString UGameMapsSettings::GetGameModeForName(const FString& GameModeName)
{
	...
	if (GameModeSection)
	{
		...
		if (ConfigLines.Num())
		{
			UE_LOGF(LogLoad, Warning, "GameMode::GameModeClassAliases are deprecated, move to GameMapsSettings");
...
FString UGameMapsSettings::GetGameModeForMapName(const FString& MapName)
{
	...
	if (GameModeSection)
	{
		...
		if (ConfigLines.Num())
		{
			UE_LOGF(LogLoad, Warning, "GameMode::DefaultMapPrefixes are deprecated, move to GameMapsSettings::GameModeMapPrefixes");
