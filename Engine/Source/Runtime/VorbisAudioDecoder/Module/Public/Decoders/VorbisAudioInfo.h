...
public:
	...
	virtual int32 GetCurrentChunkOffset() const override { return (CurrentStreamingChunksSize > 0)? BufferOffset % CurrentStreamingChunksSize : -1; }
	virtual bool HasError() const override;
	// End of ICompressedAudioInfo Interface

protected:
	VORBISAUDIODECODER_API virtual bool StreamCompressedInfoInternal(const TSharedRef<const FSoundWaveData>& InWaveData, struct FSoundQualityInfo* QualityInfo) override;

	friend class FAudioFormatOgg;
	VORBISAUDIODECODER_API int32 GetAudioDataStartOffset() const;
private:
	using Super = ICompressedAudioInfo; 
	const uint8* GetLoadedChunk(const TSharedRef<const FSoundWaveData>& InSoundWaveData, uint32 ChunkIndex, uint32& OutChunkSize);
