...
size_t FVorbisAudioInfo::ReadStreaming(void *Ptr, uint32 Size )
{
	...
	while (NumBytesRead < Size)
	{
		if (!CurrentStreamingChunkData || CurrentStreamingChunkIndex != NextStreamingChunkIndex)
		{
			...
			if (static_cast<uint32>(CurrentStreamingChunkIndex) >= StreamingSoundWaveData->GetNumChunks())
			{
				CurrentStreamingChunkData = nullptr;
				CurrentStreamingChunksSize = 0;
			}
			...
			{
				CurrentStreamingChunkData = GetLoadedChunk(StreamingSoundWaveData.ToSharedRef(), CurrentStreamingChunkIndex, CurrentStreamingChunksSize);
			}

			if (CurrentStreamingChunkData)
			{
				check(CurrentStreamingChunkIndex < (int32)StreamingSoundWaveData->GetNumChunks());
				CurrentBufferChunkOffset = 0;
			}
		}
...
static size_t OggReadStreaming( void *ptr, size_t size, size_t nmemb, void *datasource )
{
	...
	UE_LOGF(LogAudio, Error, "OggReadStreaming had null audio info datasource.");
	return 0;
}
...
bool FVorbisAudioInfo::GetCompressedInfoCommon(void* Callbacks, FSoundQualityInfo* QualityInfo)
{
	if (!bDllLoaded)
	{
		UE_LOGF(LogAudio, Error, "FVorbisAudioInfo::GetCompressedInfoCommon failed due to vorbis DLL not being loaded.");
		bHasError = true;
		return false;
	}
	...
	if (Result < 0)
	{
		UE_LOGF(LogAudio, Warning, "FVorbisAudioInfo::ReadCompressedInfo, ov_open_callbacks error code: %d : %ls", Result,GetVorbisError(Result));
		bHasError = true;
...
bool FVorbisAudioInfo::ReadCompressedInfo( const uint8* InSrcBufferData, uint32 InSrcBufferDataSize, FSoundQualityInfo* QualityInfo )
{
	if (!bDllLoaded)
	{
		UE_LOGF(LogAudio, Error, "FVorbisAudioInfo::ReadCompressedInfo failed due to vorbis DLL not being loaded.");
		return false;
	}
	
	if (bHeaderParsed)
	{
		UE_LOGF(LogAudio, Error, "FVorbisAudioInfo::ReadCompressedInfo failed due to the header being parsed already.");
		return false;
	}

	if (!VFWrapper)
	{
		UE_LOGF(LogAudio, Error, "FVorbisAudioInfo::ReadCompressedInfo failed due to not having vorbis wrapper.");
		return false;
	}
	...
	if (!bHeaderParsed)
	{	
		UE_LOGF(LogAudio, Error, "Failed to parse header for compressed vorbis file.");
	}
...
void FVorbisAudioInfo::ExpandFile( uint8* DstBuffer, FSoundQualityInfo* QualityInfo )
{
	if (!bDllLoaded)
	{
		UE_LOGF(LogAudio, Error, "FVorbisAudioInfo::ExpandFile failed due to vorbis DLL not being loaded.");
		return;
	}
	...
	if (!bHeaderParsed)
	{
		UE_LOGF(LogAudio, Error, "Failed to expand vorbis file to not parsing header first.");
		return;
	}
...
bool FVorbisAudioInfo::ReadCompressedData( uint8* InDestination, bool bLooping, uint32 BufferSize )
{
	if (!bDllLoaded)
	{
		UE_LOGF(LogAudio, Error, "FVorbisAudioInfo::ReadCompressedData failed due to vorbis DLL not being loaded.");
		return true;
	}
	...
	if (!bHeaderParsed)
	{
		UE_LOGF(LogAudio, Error, "FVorbisAudioInfo::ReadCompressedData failed due to not parsing header first.");
		return true;
	}
...
void FVorbisAudioInfo::SeekToTime( const float SeekTime )
{
	if (!bDllLoaded)
	{
		UE_LOGF(LogAudio, Error, "FVorbisAudioInfo::SeekToTime failed due to vorbis DLL not being loaded.");
		return;
	}
	...
	if (!bHeaderParsed)
	{
		UE_LOGF(LogAudio, Error, "FVorbisAudioInfo::SeekToTime failed due to not parsing header first.");
		return;
	}
	
	if (!VFWrapper->vf.seekable)
	{
		UE_LOGF(LogAudio, Error, "FVorbisAudioInfo::SeekToTime failed due to vorbis file not being seekable");
		return;
	}
...
void FVorbisAudioInfo::EnableHalfRate( bool HalfRate )
{
	if (!bDllLoaded)
	{
		UE_LOGF(LogAudio, Error, "FVorbisAudioInfo::EnableHalfRate failed due to vorbis DLL not being loaded.");
		return;
	}
	...
	if (!bHeaderParsed)
	{
		UE_LOGF(LogAudio, Error, "FVorbisAudioInfo::EnableHalfRate failed due to not parsing header first.");
		return;
	}
...
bool FVorbisAudioInfo::StreamCompressedInfoInternal(const TSharedRef<const FSoundWaveData>& InWaveData, struct FSoundQualityInfo* QualityInfo)
{
	if (!bDllLoaded)
	{
		UE_LOGF(LogAudio, Error, "FVorbisAudioInfo::StreamCompressedInfoInternal failed to parse header due to vorbis DLL not being loaded for sound '%ls'.", *InWaveData->GetFName().ToString());
		return false;
	}
	...
	if (!VFWrapper)
	{
		UE_LOGF(LogAudio, Error, "FVorbisAudioInfo::StreamCompressedInfoInternal failed due to no vorbis wrapper for sound '%ls'.", *InWaveData->GetFName().ToString());
		return false;
	}
	...
	if (!bHeaderParsed)
	{
		UE_LOGF(LogAudio, Error, "FVorbisAudioInfo::StreamCompressedInfoInternal failed to parse header for '%ls'. LastError=%ls",
			*InWaveData->GetFName().ToString(), GetVorbisError(LastErrorCode));
	}
...
const uint8* FVorbisAudioInfo::GetLoadedChunk(const TSharedRef<const FSoundWaveData>& InSoundWaveData, uint32 ChunkIndex, uint32& OutChunkSize)
{
	...
	{
		CurCompressedChunkHandle = IStreamingManager::Get().GetAudioStreamingManager().GetLoadedChunk(InSoundWaveData, ChunkIndex, /* bBlockForLoad */ true, true);
...
bool FVorbisAudioInfo::StreamCompressedData(uint8* InDestination, bool bLooping, uint32 BufferSize, int32& OutNumBytesStreamed)
{
	...
	if (!bDllLoaded)
	{
		UE_LOGF(LogAudio, Error, "FVorbisAudioInfo::StreamCompressedData failed due to vorbis DLL not being loaded.");
		return true;
	}
	...
	if (!bHeaderParsed)
	{
		UE_LOGF(LogAudio, Error, "FVorbisAudioInfo::StreamCompressedData failed due to not parsing header first.");
		return true;
	}
	...
	while( BufferSize > 0 )
	{
		...
		if( BytesActuallyRead <= 0 )
		{
			...
			check(StreamingSoundWaveData->GetNumChunks() < ((uint32)TNumericLimits<int32>::Max()));

			if (NextStreamingChunkIndex < static_cast<int32>(StreamingSoundWaveData->GetNumChunks()))
			{
			...
			UE_CLOGF(bDidTimeoutOnDecompressionFailiures, LogAudio, Error, "Timed out trying to decompress a looping sound after %d attempts; aborting.", VorbisReadFailiureTimeoutCVar);
			...
			if (Result < 0)
			{
				LastErrorCode = Result;
				bHasError = true;
				UE_LOGF(LogAudio, Error, "FVorbisAudioInfo::StreamCompressedData, ov_open_callbacks error code: %d : %ls", Result, GetVorbisError(Result));
				break;
			}
...
void LoadVorbisLibraries()
{
	static const bool bDone = []() -> bool
	{
		...
		// Load the Ogg dlls
#  if _MSC_VER >= 1900
		FString VSVersion = "VS2015/";
#  else
#    error "Unsupported Visual Studio version."
#  endif
		FString PlatformString = "Win64";
		FString DLLNameStub = "_64.dll";
		...
		if (!bDllLoaded)
		{
			UE_LOGF(LogAudio, Error, "Failed to load lib vorbis libraries.");
		}
		else
		{
			UE_LOGF(LogAudioDebug, Display, "Lib vorbis DLL was dynamically loaded.");
		}
#elif WITH_OGGVORBIS
		bDllLoaded = true;
