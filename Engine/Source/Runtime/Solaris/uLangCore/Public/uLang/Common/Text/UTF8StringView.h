...
public:
    ...
    // Accessors
    const UTF8Char* Data() const        { return _Begin; }
    int32_t ByteLen() const             { return int32_t(_End - _Begin); }
    bool IsEmpty() const                { return _Begin >= _End; }
    bool IsFilled() const               { return _Begin < _End; }
    ...
    UTF8Char NextByte(const CUTF8StringView& Enclosing) const
    {
    ...
    UTF8Char NextNextByte(const CUTF8StringView& Enclosing) const
    {
        return ((_End + 1) < Enclosing._End) ? *(_End + 1) : 0u;
    }

    /// @return the first code point in this string (decodes UTF-8)
    SUniCodePointLength FirstCodePoint() const
    {
        return CUnicode::DecodeUTF8(_Begin, _End - _Begin);
    }

    bool StartsWith(const CUTF8StringView& Text) const
    {
    ...
    bool EndsWith(const CUTF8StringView& Text) const
    {
    ...
    int32_t Find(UTF8Char Char) const
    {
    ...
    int32_t Find(const CUTF8StringView& Text) const
    {
        if (_Begin == _End)
        {
            return IndexNone;
    ...
    bool Contains(UTF8Char Char) const
    {
        return Find(Char) != IndexNone;
    }

    bool Contains(const CUTF8StringView& Text) const
    {
        return Find(Text) != IndexNone;
    }

    bool ContainsCaseIndependent(const CUTF8StringView& Text) const
    {
        const UTF8Char* EndChar = _End - Text.ByteLen(); // No need to check further as Text wouldn't fit
        for (const UTF8Char* ThisChar = _Begin; ThisChar <= EndChar; ++ThisChar)
        {
    ...
    SUniCodePointLength PopFirstCodePoint()
    {
    ...
    CUTF8StringView SubViewBegin(int32_t ByteCount) const
    {
    ...
    CUTF8StringView SubViewEnd(int32_t ByteCount) const
    {
    ...
    CUTF8StringView SubViewTrimBegin(int32_t ByteIndex) const
    {
    ...
    CUTF8StringView SubViewTrimEnd(int32_t ByteCount) const
    {
        return { _Begin, CMath::Clamp(_End - ByteCount, _Begin, _End) };
    }

    /// @return the substring from ByteIndex position for ByteCount bytes
    CUTF8StringView SubView(int32_t ByteIndex, int32_t ByteCount = INT32_MAX) const
    {
    ...
    CUTF8StringView SubView(const SIdxRange& Range) const
    {
    ...
    CUTF8StringView SubViewEmpty(int32_t ByteIndex) const
    {
    ...
    {
    public:
        UnicodeConstIterator(const UTF8Char* CurrentByte, size_t CurrentByteLen)
        ...
        UnicodeConstIterator operator++(int)
        {
        ...
        UniCodePoint operator* () const { return _CurrentValue._CodePoint; }

        bool operator==(const UnicodeConstIterator& Other) const { return _CurrentByte == Other._CurrentByte; }
        bool operator!=(const UnicodeConstIterator& Other) const { return _CurrentByte != Other._CurrentByte; }
		const UTF8Char* CurrentByte() const { return this->_CurrentByte; }
    ...
    UnicodeConstIterator begin() const { return UnicodeConstIterator(_Begin, _End - _Begin); }
    UnicodeConstIterator end() const { return UnicodeConstIterator(_End, 0); }
...
protected:
     ...
     */
    int32_t InputByteIdxToDirectIdx(int32_t InIdx) const
    {
        if (InIdx < 0)
        {
     ...
     */
    bool InputByteIdxSpan(int32_t& InOutIdx, int32_t& InOutSpan) const
    {
        int32_t ByteLen = int32_t(_End - _Begin);
...
inline uint32_t GetTypeHash(const CUTF8StringView& String) { return CCRC32::Generate(String._Begin, String._End); }
