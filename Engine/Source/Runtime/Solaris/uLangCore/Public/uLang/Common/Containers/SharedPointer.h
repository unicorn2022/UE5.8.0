...
class AUTORTFM_INFER TSPtrG
{
public:

    // Construction

    TSPtrG(NullPtrType NullPtr = nullptr) : _Object(nullptr), _Allocator(DefaultInit), _ReleaseFunc(nullptr) { static_assert(AllowNull, "Cannot default construct a shared reference, as it is not allowed to be null."); }
    TSPtrG(const TSPtrG & Other) : _Object(Other._Object), _Allocator(Other._Allocator) { EnableRelease(); if (Other._Object) { Other._Object->Reference(); } }
    ...
    TSPtrG(const TSPtrG<OtherObjectType, OtherAllowNull, AllocatorType, AllocatorArgsType...> & Other) : _Object(Other._Object), _Allocator(Other._Allocator) { EnableRelease(); if (Other._Object) { Other._Object->Reference(); } }
    TSPtrG(TSPtrG && Other) : _Object(Other._Object), _Allocator(Other._Allocator) { EnableRelease(); Other._Object = nullptr; } // Note: We null here even when OtherAllowNull==false since 99% likely will be destructed immediately after anyway
    template<class OtherObjectType, bool OtherAllowNull>
    requires (std::is_convertible_v<OtherObjectType*, ObjectType*> && (AllowNull || !OtherAllowNull))
    TSPtrG(TSPtrG<OtherObjectType, OtherAllowNull, AllocatorType, AllocatorArgsType...> && Other) : _Object(Other._Object), _Allocator(Other._Allocator) { EnableRelease(); Other._Object = nullptr; } // Note: We null here even when OtherAllowNull==false since 99% likely will be destructed immediately after anyway 
    ~TSPtrG() { if (_Object) Release(); }

    template<typename... CtorArgsType>
    TSPtrG& SetNew(CtorArgsType&&... CtorArgs) { EnableRelease(); if (_Object) { Release(); } ObjectType * Object = new(_Allocator) ObjectType(uLang::ForwardArg<CtorArgsType>(CtorArgs)...); ULANG_ASSERT(Object); Object->Reference(); _Object = Object; return *this; }
    template<typename... CtorArgsType>
    static TSPtrG New(AllocatorArgsType&&... AllocatorArgs, CtorArgsType&&... CtorArgs) { AllocatorType Allocator(AllocatorArgs...); ObjectType* Object = new(Allocator) ObjectType(uLang::ForwardArg<CtorArgsType>(CtorArgs)...); ULANG_ASSERT(Object); return TSPtrG(Object, uLang::Move(Allocator)); }

    // Assignment

    TSPtrG & operator=(NullPtrType) { static_assert(AllowNull, "Cannot assign null to shared reference, as it is not allowed to be null."); if (_Object) { Release(); } _Object = nullptr; return *this; }
    TSPtrG & operator=(const TSPtrG & Other) { return AssignCopy(Other); }
    template<class OtherObjectType, bool OtherAllowNull>
    requires (std::is_convertible_v<OtherObjectType*, ObjectType*> && (AllowNull || !OtherAllowNull))
    TSPtrG & operator=(const TSPtrG<OtherObjectType, OtherAllowNull, AllocatorType, AllocatorArgsType...> & Other) { return AssignCopy(Other); }
    TSPtrG & operator=(TSPtrG && Other) { return AssignMove(ForwardArg<TSPtrG>(Other)); }
    template<class OtherObjectType, bool OtherAllowNull>
    requires (std::is_convertible_v<OtherObjectType*, ObjectType*>)
    TSPtrG & operator=(TSPtrG<OtherObjectType, OtherAllowNull, AllocatorType, AllocatorArgsType...> && Other) { return AssignMove(uLang::ForwardArg<TSPtrG<OtherObjectType, OtherAllowNull, AllocatorType, AllocatorArgsType...>>(Other)); }

    // Conversion methods

    operator ObjectType*() const { return _Object; }
    ObjectType &          operator*() const { ULANG_ASSERT(_Object); return *_Object; }
    ObjectType *          operator->() const { ULANG_ASSERT(AllowNull || _Object); return _Object; }
    ObjectType *          Get() const { ULANG_ASSERT(AllowNull || _Object); return _Object; }
    const AllocatorType & GetAllocator() const { return _Allocator; }
    void                  Reset() { if (_Object) { Release(); _Object = nullptr; } }

    TSPtrG<ObjectType, false, AllocatorType, AllocatorArgsType...>& AsRef()&
    {
    ...
    const TSPtrG<ObjectType, false, AllocatorType, AllocatorArgsType...>& AsRef() const&
    {
        static_assert(AllowNull, "Unnecessary conversion!");
        ULANG_ASSERTF(_Object, "Converting null pointer to reference!");
    ...
    TSPtrG<OtherObjectType, AllowNull, AllocatorType, AllocatorArgsType...>& As() { return *reinterpret_cast<TSPtrG<OtherObjectType, AllowNull, AllocatorType, AllocatorArgsType...> *>(this); }
    template<class OtherObjectType>
    requires (std::is_convertible_v<OtherObjectType*, ObjectType*> || std::is_convertible_v<ObjectType*, OtherObjectType*>)
    const TSPtrG<OtherObjectType, AllowNull, AllocatorType, AllocatorArgsType...>& As() const { return *reinterpret_cast<const TSPtrG<OtherObjectType, AllowNull, AllocatorType, AllocatorArgsType...> *>(this); }

    // Comparison operators

    bool operator==(NullPtrType)  const { return _Object == nullptr; }
    bool operator!=(NullPtrType)  const { return _Object != nullptr; }
    bool operator==(const TSPtrG & Other)  const { return _Object == Other._Object; }
    template<class OtherObjectType, bool OtherAllowNull>
    bool operator==(const TSPtrG<OtherObjectType, OtherAllowNull, AllocatorType, AllocatorArgsType...> & Other)  const { return _Object == Other._Object; }
    template<class OtherObjectType, bool OtherAllowNull>
    ...
    friend void Swap(TSPtrG& A, TSPtrG& B) { uLang::Swap(A, B); }

    // Composability methods
    template <typename FuncType, typename... ArgTypes>
    TSPtrG Map(FuncType&& Func, ArgTypes&&... Args) &&
    {
        TSPtrG Result{*this};
    ...
    TSPtrG(ObjectType& Object, const AllocatorType& Allocator)
        : _Object(&Object)
...
protected:
    ...
    template<class OtherObjectType, bool OtherAllowNull>
    TSPtrG & AssignCopy(const TSPtrG<OtherObjectType, OtherAllowNull, AllocatorType, AllocatorArgsType...> & Other)
    {
    ...
    template<class OtherObjectType, bool OtherAllowNull>
    TSPtrG & AssignMove(TSPtrG<OtherObjectType, OtherAllowNull, AllocatorType, AllocatorArgsType...> && Other)
    {
        EnableRelease();
...
template<class ObjectType>
decltype(auto) SharedThis(ObjectType&& This)
{
    return TSPtrG<std::remove_reference_t<ObjectType>, false, CHeapRawAllocator>(This, {});
}
 ...
 */
inline bool CSharedMix::Dereference() const
{
