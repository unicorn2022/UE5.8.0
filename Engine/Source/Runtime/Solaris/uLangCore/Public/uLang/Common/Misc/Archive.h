...
public:
    ...
    bool IsLoading() const { return _bIsLoading; }
    ...
    friend CArchive& operator<<(CArchive& Ar, T& Value) { Ar.Serialize(&Value, sizeof(Value)); return Ar; }

    // Convenience operator for serializing a string
    friend CArchive& operator<<(CArchive& Ar, CUTF8String& Value)
    {
    ...
    template<class T>
    friend CArchive& operator<<(CArchive& Ar, TOptional<T>& Value)
    {
    ...
    friend CArchive& operator<<(CArchive& Ar, TArray<ElementType>& Value)
    {
