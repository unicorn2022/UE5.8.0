...
public:
    ...
    public:
        ...
        std::enable_if_t<!_bConst, reference> operator*()
        {
            return _CurrentEntry->_KeyValue;
        }

        template <bool _bConst = bConst>
        std::enable_if_t<_bConst, reference> operator*() const
        {
            return _CurrentEntry->_KeyValue;
        }

        template <bool _bConst = bConst>
        std::enable_if_t<!_bConst, pointer> operator->()
        {
            return _CurrentEntry->_KeyValue;
        }

        template <bool _bConst = bConst>
        std::enable_if_t<_bConst, pointer> operator->() const
        {
            return _CurrentEntry->_KeyValue;
        }
...
protected:
    ...
    static uint32_t ComputeNonZeroHash(const KeyType& Key)
    {
        ...
        for (;;)
        {
            ...
            if (Entry._Hash == Hash)
            {
                ...
                if (Entry._KeyValue == Key)    // Check for equality of key regardless of value
                {
                    ...
                    if (!std::is_same_v<KeyType, KeyValueType>)    // Equality could be stored in the template as constexpr bool bIsSet
                    {
    ...
    uint32_t Lookup(const KeyType& Key) const
    {
        if (!_NumEntries)
        {
