...
#include "uLang/Common/Common.h"
    ...
    namespace uLang
    {
        inline uint32_t InterlockedCompareExchange(volatile uint32_t* Value, uint32_t ReplaceWithThis, uint32_t IfEqualToThis)
        {
            return __sync_val_compare_and_swap(Value, IfEqualToThis, ReplaceWithThis);
        }

        inline void* InterlockedCompareExchange(void* volatile* Value, void* ReplaceWithThis, void* IfEqualToThis)
        {
            return __sync_val_compare_and_swap(Value, IfEqualToThis, ReplaceWithThis);
        }
    }
