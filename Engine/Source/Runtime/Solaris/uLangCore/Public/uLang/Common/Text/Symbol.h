...
private:
    ...
    struct SEntry
    {
        ...
        CUTF8StringView AsStringView() const
        {
...
{
public:
    CSymbol() : _Entry(&CSymbolTable::_EntryNull) {}

    SymbolId GetId() const   { return _Entry->_Id; }
    bool IsNull() const      { return _Entry->_Id == SymbolId_Null; }
    bool IsGenerated() const { return _Entry->_bIsGenerated; }

    CUTF8String AsString() const          { return _Entry->AsStringView(); }
    CUTF8StringView AsStringView() const  { return _Entry->AsStringView(); }
    const char* AsCString() const         { return _Entry->AsCString(); }
    UTF8Char FirstByte() const            { return _Entry->_Data[0]; }

    EEquate Compare(const CSymbol& Other) const   { return (_Entry == Other._Entry) ? EEquate::Equal : ((_Entry < Other._Entry) ? EEquate::Less : EEquate::Greater); }
    bool operator == (const CSymbol& Other) const { return _Entry == Other._Entry; }
    bool operator != (const CSymbol& Other) const { return _Entry != Other._Entry; }
    bool operator <= (const CSymbol& Other) const { return _Entry <= Other._Entry; }
    bool operator >= (const CSymbol& Other) const { return _Entry >= Other._Entry; }
    bool operator <  (const CSymbol& Other) const { return _Entry <  Other._Entry; }
    ...
    friend uint32_t GetTypeHash(CSymbol Symbol)
    {
...
private:
    ...
    CSymbol(const CSymbolTable::SEntry* Entry) : _Entry(Entry) {}
...
inline CSymbol CSymbolTable::Get(SymbolId Id) const
{
