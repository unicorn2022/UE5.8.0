...
public:
    ...
    static bool IsWhitespaceASCII(const UTF8Char Ch) { return _ASCIITable_Whitespace[Ch]; }
    static bool IsDigitASCII(const UTF8Char Ch)      { return (unsigned(Ch) - unsigned('0')) < 10u; }
    static bool IsUpperASCII(const UTF8Char Ch)      { return (unsigned(Ch) - unsigned('A')) < 26u; }
    static bool IsLowerASCII(const UTF8Char Ch)      { return (unsigned(Ch) - unsigned('a')) < 26u; }
    static bool IsAlphaASCII(const UTF8Char Ch)      { return ((unsigned(Ch) - unsigned('a')) < 26u) || ((unsigned(Ch) - unsigned('A')) < 26u); }
    static UTF8Char ToLower_ASCII(const UTF8Char Ch) { return IsUpperASCII(Ch) ? Ch + ('a' - 'A') : Ch; }
    static UTF8Char ToUpper_ASCII(const UTF8Char Ch) { return IsLowerASCII(Ch) ? Ch - ('a' - 'A') : Ch; }
...
inline SUniCodePointLength CUnicode::DecodeUTF8(const UTF8Char * Text, size_t TextByteLength)
{
...
// This inline function optimizes for the most common case that the code point is ASCII
// Only for non-ASCII code points an actual function call is made
inline bool CUnicode::IsIdentifierStart(UniCodePoint CodePoint)
{
    // If ASCII, deal with it right here
...
// This inline function optimizes for the most common case that the code point is ASCII
// Only for non-ASCII code points an actual function call is made
inline bool CUnicode::IsIdentifierTail(UniCodePoint CodePoint)
{
    // If ASCII, deal with it right here
    if (CodePoint < ASCII_RANGE)
    {
