...
inline uint32_t RotateCRC32(uint32_t CRC)
{
...
public:
    ...
    void * Allocate(size_t NumBytes) const { return (*_Allocate)(this, NumBytes); }
    void * Reallocate(void * Memory, size_t NumBytes) const { return (*_Reallocate)(this, Memory, NumBytes); }
    void   Deallocate(void * Memory) const { (*_Deallocate)(this, Memory); }

    EObserverId GenerateObserverId() { return EObserverId(_ObserverIdGenerator = RotateCRC32(_ObserverIdGenerator)); }
...
public:
    CHeapRawAllocator() {}
    CHeapRawAllocator(EDefaultInit) {}

    static void * Allocate(size_t NumBytes)                  { return GetSystemParams()._HeapMalloc(NumBytes); }
    static void * Reallocate(void * Memory, size_t NumBytes) { return GetSystemParams()._HeapRealloc(Memory, NumBytes); }
    static void   Deallocate(void * Memory)                  { GetSystemParams()._HeapFree(Memory); }

    static EObserverId GenerateObserverId()
    {
...
{
public:
    CInstancedRawAllocator(CAllocatorInstance * AllocatorInstance) : _AllocatorInstance(AllocatorInstance) {}
    CInstancedRawAllocator(EDefaultInit) : _AllocatorInstance(nullptr) {} // Requires explicit argument to prevent default initialization by simply passing no arguments

    void * Allocate(size_t NumBytes) const { return _AllocatorInstance->Allocate(NumBytes); }
    void * Reallocate(void * Memory, size_t NumBytes) { return _AllocatorInstance->Reallocate(Memory, NumBytes); }
    void   Deallocate(void * Memory) const { _AllocatorInstance->Deallocate(Memory); }

    EObserverId GenerateObserverId() const { return _AllocatorInstance->GenerateObserverId(); }
...
inline void * operator new(size_t NumBytes, const ::uLang::CHeapRawAllocator & Allocator) { return Allocator.Allocate(NumBytes); }
// Unfortunately, C++ does not have a clean syntax to invoke custom delete operators so this is a bit useless...
inline void operator delete(void * Memory, const ::uLang::CHeapRawAllocator & Allocator) { Allocator.Deallocate(Memory); }

// Deliberately NOT implementing new[] and delete[] here
inline void * operator new(size_t NumBytes, const ::uLang::CInstancedRawAllocator & Allocator) { return Allocator.Allocate(NumBytes); }
// Unfortunately, C++ does not have a clean syntax to invoke custom delete operators so this is a bit useless...
inline void operator delete(void * Memory, const ::uLang::CInstancedRawAllocator & Allocator) { Allocator.Deallocate(Memory); }
...
*/
inline size_t DefaultQuantizeSize(size_t Count, uint32_t Alignment)
{
    return Count;
}
...
public:
    ...
    public:
        /** Default constructor. */
        ForAnyElementType(EDefaultInit) : _Data(nullptr), _RawAllocator(DefaultInit) {}

        /** Constructor with given allocator */
        ForAnyElementType(const RawAllocatorType & Allocator) : _Data(nullptr), _RawAllocator(Allocator) {}
        ForAnyElementType(AllocatorArgsType... AllocatorArgs) : _Data(nullptr), _RawAllocator(AllocatorArgs...) {}
        ...
        SScriptContainerElement* GetAllocation() const
        {
            return _Data;
        }

        /** Accesses the container's raw allocator. */
        const RawAllocatorType & GetRawAllocator() const
        {
         ...
         */
        void ResizeAllocation(int32_t PreviousNumElements, int32_t NumElements, size_t NumBytesPerElement)
        {
         ...
         * @param NumBytesPerElement - The number of bytes/element.
         */
        int32_t CalculateSlackReserve(int32_t NumElements, int32_t NumBytesPerElement) const
        {
        ...
        * @param NumBytesPerElement - The number of bytes/element.
        */
        int32_t CalculateSlackShrink(int32_t NumElements, int32_t NumAllocatedElements, int32_t NumBytesPerElement) const
        {
        ...
        * @param NumBytesPerElement - The number of bytes/element.
        */
        int32_t CalculateSlackGrow(int32_t NumElements, int32_t NumAllocatedElements, int32_t NumBytesPerElement) const
        {
    ...
    public:
        ...
        ForElementType(EDefaultInit) : ForAnyElementType(DefaultInit) {}
        ForElementType(const RawAllocatorType & Allocator) : ForAnyElementType(Allocator) {}
        ForElementType(AllocatorArgsType&&... AllocatorArgs) : ForAnyElementType(uLang::ForwardArg<AllocatorArgsType>(AllocatorArgs)...) {}
...
public:
    ...
    public:
        ...
        /** Constructor with given allocator */
        ForElementType(const RawAllocatorType& Allocator) : _SecondaryData(Allocator) {} //-V730
        ForElementType(AllocatorArgsType... AllocatorArgs) : _SecondaryData(AllocatorArgs...) {} //-V730
