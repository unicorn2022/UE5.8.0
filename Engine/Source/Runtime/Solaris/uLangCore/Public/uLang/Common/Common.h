...
struct SSystemParams
{
    ...
    int      _APIVersion = 0;         ///< Set this to ULANG_API_VERSION

    FMalloc  _HeapMalloc = nullptr;   ///< Allocate system heap memory
    FRealloc _HeapRealloc = nullptr;  ///< Reallocate system heap memory
    FFree    _HeapFree = nullptr;     ///< Free system heap memory

    FAssert  _AssertFailed = nullptr; ///< Called when an assert fails
    FLog     _LogMessage = nullptr;   ///< Print a message
...
extern ULANGCORE_API SSystemParams GSystemParams;
inline SSystemParams& GetSystemParams() { return GSystemParams; }
