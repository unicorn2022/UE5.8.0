...
namespace Private
{
    ...
    struct TCanMoveTArrayPointersBetweenArrayTypes
    {
        ...
        static constexpr bool Value =
            std::is_same_v<FromAllocatorType, ToAllocatorType> &&       // Allocators must be equal
    ...
    {
        static constexpr bool Value = std::is_convertible_v<decltype(std::declval<const Type&>() < std::declval<const Type&>()), bool>;
    };
}
...
public:
     ...
     */
    TArrayG(RawAllocatorArgsType&&... RawAllocatorArgs)
     ...
     */
    TArrayG(const typename ElementAllocatorType::RawAllocatorType & RawAllocator)
     ...
     * @see Append
     */
    TArrayG(const ElementType* Ptr, int32_t Count, RawAllocatorArgsType&&... RawAllocatorArgs)
        : _ElementStorage(ForwardArg<RawAllocatorArgsType>(RawAllocatorArgs)...)
    {
    ...
    TArrayG(bool, const ElementType& = ElementType(), RawAllocatorArgsType&&...) = delete;

    TArrayG(int32_t Count, const ElementType& Value = ElementType(), RawAllocatorArgsType&&... RawAllocatorArgs)
        : _ElementStorage(ForwardArg<RawAllocatorArgsType>(RawAllocatorArgs)...)
    {
        _ArrayNum = Count;
     ...
     */
    template <typename OtherElementType, typename OtherElementAllocatorType, typename... OtherRawAllocatorArgsType>
    explicit TArrayG(const TArrayG<OtherElementType, OtherElementAllocatorType, OtherRawAllocatorArgsType...>& Other, RawAllocatorArgsType&&... RawAllocatorArgs)
        : _ElementStorage(ForwardArg<RawAllocatorArgsType>(RawAllocatorArgs)...)
    {
     ...
     */
    TArrayG(const TArrayG& Other, int32_t ExtraSlack)
        : _ElementStorage(Other._ElementStorage.GetRawAllocator())
    {
...
private:
     ...
     */
    template <typename FromArrayType, typename ToArrayType>
    requires (Private::TCanMoveTArrayPointersBetweenArrayTypes<FromArrayType, ToArrayType>::Value)
    static void MoveOrCopy(ToArrayType& ToArray, FromArrayType& FromArray, int32_t PrevMax)
    {
    ...
    template <typename FromArrayType, typename ToArrayType>
    requires (Private::TCanMoveTArrayPointersBetweenArrayTypes<FromArrayType, ToArrayType>::Value)
    static void MoveOrCopyWithSlack(ToArrayType& ToArray, FromArrayType& FromArray, int32_t PrevMax, int32_t ExtraSlack)
    {
...
public:
    ...
    explicit TArrayG(TArrayG<ElementType, OtherElementAllocatorType, OtherRawAllocatorArgsType...>&& Other)
    {
        MoveOrCopy(*this, Other, 0);
    }
     ...
     */
    void RangeCheck(int32_t Index) const
    {
        CheckInvariants();
    ...
    const typename ElementAllocatorType::RawAllocatorType & GetRawAllocator() const
    {
     ...
     */
    ElementType& Last(int32_t IndexFromTheEnd = 0)
    {
     ...
     * @returns Reference to n-th last element from the array.
     */
    const ElementType& Last(int32_t IndexFromTheEnd = 0) const
    {
        RangeCheck(_ArrayNum - IndexFromTheEnd - 1);
        return GetData()[_ArrayNum - IndexFromTheEnd - 1];
    }
     ...
     */
    bool FindLast(const ElementType& Item, int32_t& Index) const
    {
     ...
     *
     * @param OtherArray Array to compare.
     * @returns True if this array is less than OtherArray. False otherwise.
     */
    bool operator<(const TArrayG& OtherArray) const
        requires (Private::TLessThanComparable<InElementType>::Value)
    {
        const int32_t MyNum = (*this).Num();
        const int32_t OtherNum = OtherArray.Num();
        ...
        {
            for (int32_t Index = 0; Index < MyNum; Index++)
            {
                const InElementType& MyElement = (*this)[Index];
                const InElementType& OtherElement = OtherArray[Index];

                if (MyElement < OtherElement)
                {
                    return true;
                }
                else if (OtherElement < MyElement)
                {
                    return false;
                }
            }
     ...
     * @see Add, Remove
     */
    void CheckAddress(void const* Addr) const
    {
...
public:
    ...
    {
        static_assert(!std::is_same_v<CountType, bool>, "TArrayG::RemoveAt: unexpected bool passed as the Count argument");
...
public:
    ...
    {
        static_assert(!std::is_same_v<CountType, bool>, "TArrayG::RemoveAtSwap: unexpected bool passed as the Count argument");
     ...
     */
    template <typename... ArgsType>
    void EmplaceAt(int32_t Index, ArgsType&&... Args)
    {
        InsertUninitialized(Index, 1);
        new(GetData() + Index) ElementType(ForwardArg<ArgsType>(Args)...);
    }
     ...
     */
    template <typename... ArgsType>
    ElementType& EmplaceAt_GetRef(int32_t Index, ArgsType&&... Args)
    {
        InsertUninitialized(Index, 1);
        ElementType* Ptr = GetData() + Index;
        new(Ptr) ElementType(ForwardArg<ArgsType>(Args)...);
     ...
     * @see AddDefaulted, AddUnique, AddZeroed, Append, Insert
     */
    int32_t Add(const ElementType& Item)
    {
        CheckAddress(&Item);
        return Emplace(Item);
    }
...
public:
     ...
     */
    int32_t AddUnique(ElementType&& Item) { return AddUniqueImpl(uLang::MoveIfPossible(Item)); }
     ...
     * @see Add, AddDefaulted, AddZeroed, Append, Insert
     */
    int32_t AddUnique(const ElementType& Item) { return AddUniqueImpl(Item); }
...
public:
     ...
     */
    ElementType *       begin()       { return GetData(); }
    const ElementType * begin() const { return GetData(); }
    ElementType *       end()         { return GetData() + Num(); }
    const ElementType * end() const   { return GetData() + Num(); }
...
protected:
    ...
    using ElementStorageType = typename std::conditional_t<
...
template <typename T>
uint32_t GetTypeHash(const TArray<T> Array)
{
    uint32_t Result = 0;
