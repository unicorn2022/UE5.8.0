...
public:
     ...
     * Constructor
     */
    TSPtrSetG(RawAllocatorArgsType&&... RawAllocatorArgs)
     ...
     */
    TSPtrSetG(const TSPtrSetG & Other)
        : Super(Other)
    {
    }
     ...
     */
    TSPtrSetG(const TSPtrSetG & Other, int32_t ExtraSlack)
     ...
     * @see Find
     */
    TOptional<PointerType> Find(const KeyType& Key) const
    {
        int32_t UpperBound = Helper::GetUpperBound(_PointerStorage, Key);
        if (UpperBound < Num())
        {
...
{
    static_assert(TAllocatorTraits<ElementAllocatorType>::SupportsMove, "TSPtrSet no longer supports move-unaware allocators");
