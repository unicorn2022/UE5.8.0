...
uint32_t CSymbolTable::GetBucketIndex(const CUTF8StringView & Text) const
{
    return CCRC16::Generate(Text._Begin, Text._End) & (_HashBuckets.Num() - 1);
}

const CSymbolTable::SEntry * CSymbolTable::AddInternal(const CUTF8StringView & Text, uint32_t BucketIndex, bool bIsGenerated)
{
...
const CSymbolTable::SEntry * CSymbolTable::FindOrAddInternal(const CUTF8StringView & Text, bool bIsGenerated)
{
