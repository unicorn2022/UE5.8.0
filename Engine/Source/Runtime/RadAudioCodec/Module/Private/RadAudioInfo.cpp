...
void FRadAudioInfo::SeekToFrame(uint32 InSeekTimeFrames)
{
	...
	{
		UE_LOGF(LogRadAudioDecoder, Error, "Seek block destination passed frame count caps but offset exceeds int32 limits (got: %zu", OffsetToBlockS);
		return;
	}
	...
	else
	{
		...
		{
			UE_LOGF(LogRadAudioDecoder, Error, "Unable to find seek chunk for offset %u: ChunkCount: %d Total Chunk Size: %llu", OffsetToBlock, TotalStreamingChunks, TotalChunkSizesExamined);
		}
	}
}
...
bool FRadAudioInfo::CreateDecoder()
{
	...
	{
		UE_LOGF(LogRadAudioDecoder, Error, "Invalid/insufficient data in FRadAudioInfo::CreateDecoder - bad buffer passed / bad cook? Size = %d", SrcBufferDataSize);
		if (SrcBufferDataSize > 8)
		{
			UE_LOGF(LogRadAudioDecoder, Error, "First 8 bytes: 0x%llx", *(uint64*)SrcBufferData);
		}
	...
	{
		UE_LOGF(LogRadAudioDecoder, Error, "Failed to open decoder, likely corrupted data.");
		RawMemory.Empty();
		return false;
	}
...
FDecodeResult FRadAudioInfo::Decode(const uint8* CompressedData, const int32 CompressedDataSize, uint8* OutPCMData, const int32 OutputPCMDataSize)
{
	...
	while (RemnOutputFrames)
	{
		...
		if (BlockResult != RadAExamineBlockResult::Valid)
		{
			...
			UE_LOGF(LogRadAudioDecoder, Warning, "Invalid block in FRadAudioInfo::Decode: Result = %d, RemnSize = %d", EnumToUnderlyingType(BlockResult), RemnCompressedDataSize);
			if (RemnCompressedDataSize >= 8)
			{
				UE_LOGF(LogRadAudioDecoder, Warning, "First 8 bytes of buffer: 0x%02x 0x%02x 0x%02x 0x%02x:0x%02x 0x%02x 0x%02x 0x%02x", 
		...
		{
			UE_LOGF(LogRadAudioDecoder, Error, "Failed to decode block that passed validation checks, corrupted buffer?");
			bErrorStateLatch = true;
