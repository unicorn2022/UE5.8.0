...
struct FTraceControlSettings
{
	...
	UPROPERTY(EditAnywhere, Category = "Message")
	TArray<FTraceCapability> Capabilities;
};
...
/**
 * Start tracing to a host using a secure connection (if supported on the target)
 */
USTRUCT()
struct FTraceControlSecure : public FTraceControlStartCommon
{
	GENERATED_USTRUCT_BODY()
};
 ...
 * See TraceAuxiliary for details.
 */
UENUM()
enum class ETraceSecureMode : uint8
{
...
/**
 * Response to a secure start request. Specifies port to connect
 * to, optional authorization token, and optional certificate if
 * using self-signed certificates.
 */
USTRUCT()
struct FTraceControlSecureResponse
{
	GENERATED_USTRUCT_BODY()
	...
	UPROPERTY(EditAnywhere, Category="Message")
	ETraceSecureMode Mode = ETraceSecureMode::SelfSignedCertAndAuthToken;
