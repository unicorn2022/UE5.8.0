...
private:
	...
	uint8 GetToken()
	{
		...
		UE_LOGF(LogGameplayTags, Warning, "Error parsing FGameplayTagQuery!");
		bReadError = true; 
		return 0;
	}
};
...
bool FGameplayTagContainer::Serialize(FStructuredArchive::FSlot Slot)
{
	...
	if (bOldTagVer)
	{
		...
		UE_LOGF(LogGameplayTags, Error, "Failed to load old GameplayTag container, too old to migrate correctly");
	}
	else
	{
		Slot << GameplayTags;
...
#if WITH_EDITOR
		FObjectSavePackageSerializeContext* SaveContext = nullptr;
		bool bCreateDependencies = false;
		if (UnderlyingArchive.IsCooking())
		{
			FArchiveSavePackageData* SavePackageData = const_cast<FArchive&>(UnderlyingArchive).GetSavePackageData();
			check(SavePackageData);
			SaveContext = &SavePackageData->SavePackageContext;

			if (SaveContext->IsHarvestingCookDependencies())
			{
				bCreateDependencies = true;
			}
		}
...
#if WITH_EDITOR
			if (bCreateDependencies)
			{
				SaveContext->AddCookLoadDependency(UGameplayTagsManager::Get().CreateCookDependency(Tag));
			}
#endif //#if WITH_EDITOR
...
#undef LOCTEXT_NAMESPACE
	...
	switch (MatchType)
	{
		...
		default:
			UE_LOGF(LogGameplayTags, Warning, "Invalid value for TagsToMatch (EGameplayContainerMatchType) %d.  Should only be Any or All.", static_cast<int32>(MatchType));
			break;
	}
...
bool FGameplayTag::NetSerialize_ForReplayUsingFastReplication(FArchive& Ar, UPackageMapClient& PackageMapClient)
{
	...
	if (Ar.IsLoading())
	{
		...
		if (NetIndex != INVALID_TAGNETINDEX && ensure(NetFieldExportGroup.IsValid()) && ensure(NetIndex < NetFieldExportGroup->NetFieldExports.Num()))
		{
			...
			{ 
				UE_LOGF(LogGameplayTags, Warning, "Gameplay tag not found (marking incompatible): %ls", *TagName.ToString());
...
namespace UE::GameplayTags::GameplayTagDynamicSerialization
{
		...
		if (Ar.IsSaving())
		{
			...
			if (TagToken.IsValid())
			{
				...
				if (!NetFieldExport.bExported)
				{
					...
					UE_LOGF(LogGameplayTags, Verbose, "Replay> Exported Tag %ls as NetFieldIndex %u", *NetFieldExport.ExportName.ToString(), TagTokenIndex);
				}
	...
	bool NetSerialize(FGameplayTag& GameplayTag, FArchive& Ar, UPackageMap* Map)
	{
		...
		if (Ar.IsSaving())
		{
			...
			{
				UE_LOGF(LogGameplayTags, Error, "FGameplayTag::NetSerialize::Could not find required FGameplayTagTokenStore");
				ensure(false);
			}
		}
		else if (Ar.IsLoading())
...
void FGameplayTag::PostSerialize(const FArchive& Ar)
{
	...
	if (Ar.IsSaving())
	{
		...
		if (Ar.IsCooking())
		{
			FArchiveSavePackageData* SavePackageData = const_cast<FArchive&>(Ar).GetSavePackageData();
			check(SavePackageData);
			FObjectSavePackageSerializeContext& SaveContext = SavePackageData->SavePackageContext;

			if (SaveContext.IsHarvestingCookDependencies())
			{
				SaveContext.AddCookLoadDependency(UGameplayTagsManager::Get().CreateCookDependency(*this));
			}
		}
#endif //#if WITH_EDITOR
	}
...
{
	AddTagsHandle = UGameplayTagsManager::CallOrRegister_OnAddNativeTagsDelegate(
		FSimpleMulticastDelegate::FDelegate::CreateRaw(this, &FGameplayTagNativeAdder::AddTags));
}

FGameplayTagNativeAdder::~FGameplayTagNativeAdder()
{
	UGameplayTagsManager::UnregisterNativeTagDelegate(AddTagsHandle);
}
...
bool FGameplayTagQueryExpression::ConvertToJsonObject(TSharedRef<FJsonObject>& OutObject) const
{
	...
	for (const FGameplayTag& GameplayTag : TagSet)
	{
		...
		{
			UE_LOGF(LogGameplayTags, Error, "Failed to convert GameplayTag (%ls) to json", *GameplayTag.ToString());
			return false;
		}
	}
	for (const FGameplayTagQueryExpression& TagQueryExpression : ExprSet)
	{
		...
		{
			UE_LOGF(LogGameplayTags, Error, "Failed to convert tag query expression to json");
			return false;
		}
	}
...
bool FGameplayTagQueryExpression::MakeFromJsonObject(const TSharedRef<FJsonObject>& InObject, FGameplayTagQueryExpression& OutQueryExpression)
{
	...
	{
		UE_LOGF(LogGameplayTags, Error, "Failed to get ExprType from json object. Unable to make GameplayTagQueryExpression");
		return false;
	}

	{
		...
		if (InObject->TryGetArrayField(TEXT("TagSet"), TagSetArray))
		{
			for (const TSharedPtr<FJsonValue>& GameplayTagObject : *TagSetArray)
			{
				...
				{
					UE_LOGF(LogGameplayTags, Error, "Failed to make GameplayTag from Json");
					return false;
				}
			}
		}
	}
	{
		...
		if (InObject->TryGetArrayField(TEXT("ExprSet"), ExprSetArray))
		{
			for (const TSharedPtr<FJsonValue>& ExprJsonObject : *ExprSetArray)
			{
				...
				{
					UE_LOGF(LogGameplayTags, Error, "Failed to make GameplayTagQueryExpression from Json");
					return false;
				}
			}
		}
...
static void TagPackingTest()
{
	...
	UE_LOGF(LogGameplayTags, Warning, "TagPackingTest completed!");
