...
static TAutoConsoleVariable<int32> CVarGameplayTagsLruCacheSize(
	TEXT("GameplayTags.LruCacheSize"),
	3,
	TEXT("Accelerate consequent RequestGameplayTagChildren searches at the cost of the memory usage.")
);
...
namespace GameplayTagUtil
{
	...
	static void DumpGameplayTagStrings(const FGameplayTagNode& RootNode, const FString& Filename)
	{
		...
		{
			UE_LOGF(LogGameplayTags, Display, "Wrote Tag Dump: %ls", *TagDumpFilename);
		}
	}	
	...
	static void DumpRegisteredSearchPaths(const TMap<FString, FGameplayTagSearchPathInfo>& RegisteredSearchPaths, const FString& Filename)
	{
		...
		{
			UE_LOGF(LogGameplayTags, Display, "Wrote RegisteredSearchPaths Dump: %ls", *DumpFilename);
		}
	}
	...
	static void DumpRestrictedGameplayTagSourceNames(const TSet<FName>& RestrictedGameplayTagSourceNames, const FString& Filename)
	{
		...
		if (!FFileHelper::SaveStringArrayToFile(Lines, *DumpFilename, FFileHelper::EEncodingOptions::ForceUTF8WithoutBOM))
		{
			UE_LOGF(LogGameplayTags, Display, "Wrote RestrictedGameplayTagSourceNames Dump: %ls", *DumpFilename);
		}
	}
#endif

#if WITH_EDITOR
	struct FGameplayTagCookDependency
	{
	public:
		FGameplayTagCookDependency()
			: TagName()
			, SourceTagNames()
			, bTagExists(false)
		{ }
		...
		void Save(FCbWriter& Writer)
		{
			...
			SourceTagNames.Sort(FNameLexicalLess());
			Writer << SourceTagNames;
			
			Writer << bTagExists;
		}

		bool Load(FCbFieldViewIterator& Args)
		{
			FUtf8StringView TagNameAsString = Args.AsString();
			TagName = FName(TagNameAsString);

			bool bSuccess = LoadFromCompactBinary((++Args), SourceTagNames);
			if (!bSuccess)
			{
				return false;
			}

			bTagExists = (++Args).AsBool();

			return true;
		}

		void Hash(FBlake3& OutHasher)
		{
			FString TagNameString = TagName.ToString();
			TagNameString.ToLowerInline();
			OutHasher.Update(TagNameString.GetCharArray().GetData(), TagNameString.NumBytesWithoutNull());

			SourceTagNames.Sort(FNameLexicalLess());
			for (const FName& SourceTagName : SourceTagNames)
			{
				FString SourceTagNameString = SourceTagName.ToString();
				SourceTagNameString.ToLowerInline();
				OutHasher.Update(SourceTagNameString.GetCharArray().GetData(), SourceTagNameString.NumBytesWithoutNull());
			}

			OutHasher.Update(&bTagExists, sizeof(bTagExists));
		}

		FName TagName;
		TArray<FName> SourceTagNames; //if the tag was redirected, contains all the source tag names.
		bool bTagExists;
	};
...
static const FName NAME_Native = FName("Native");
...
void UGameplayTagsManager::AddTagsFromAdditionalLooseIniFiles(const TArray<FString>& IniFileList)
{
	...
	for (const FString& IniFilePath : IniFileList)
	{
		...
		UE_CLOGF(GAMEPLAYTAGS_VERBOSE, LogGameplayTags, Display, "Loading Tag File: %ls", *IniFilePath);
	...
	if (!GameplayRootTag.IsValid())
	{
...
#endif
		{
			...
			for (FName TagName : Default->CommonlyReplicatedTags)
			{
				...
				{
					UE_LOGF(LogGameplayTags, Warning, "%ls was found in the CommonlyReplicatedTags list but doesn't appear to be a valid tag!", *TagName.ToString());
				}
			}
...
void UGameplayTagsManager::ConstructNetIndex()
{
	...
	UE_CLOGF(PrintNetIndiceAssignment, LogGameplayTags, Display, "Assigning NetIndices to %d tags.", NetworkGameplayTagNodeIndex.Num() );
	...
	for (FGameplayTagNetIndex i = 0; i < NetworkGameplayTagNodeIndex.Num(); i++)
	{
		if (NetworkGameplayTagNodeIndex[i].IsValid())
		{
			...
			UE_CLOGF(PrintNetIndiceAssignment, LogGameplayTags, Display, "Assigning NetIndex (%d) to Tag (%ls)", i, *NetworkGameplayTagNodeIndex[i]->GetCompleteTag().ToString());
		}
		else
		{
			UE_LOGF(LogGameplayTags, Warning, "TagNode Indice %d is invalid!", i);
		}
	}

	UE_LOGF(LogGameplayTags, Log, "NetworkGameplayTagNodeIndexHash is %x", NetworkGameplayTagNodeIndexHash);
}
...
{
	++DeferBroadcastOnGameplayTagTreeChanged;
}

void UGameplayTagsManager::PopDeferOnGameplayTagTreeChangedBroadcast()
{
	--DeferBroadcastOnGameplayTagTreeChanged;
	ensure(DeferBroadcastOnGameplayTagTreeChanged >= 0);
	if (DeferBroadcastOnGameplayTagTreeChanged == 0 && bBroadcastOnGameplayTagTreeChangedWasDeferred)
	{
		bBroadcastOnGameplayTagTreeChangedWasDeferred = false;
		BroadcastOnGameplayTagTreeChanged();
	}
}

void UGameplayTagsManager::PushDeferGameplayTagTreeRebuilds()
{
	++DeferGameplayTagTreeRebuilds;
}

void UGameplayTagsManager::PopDeferGameplayTagTreeRebuilds()
{
	--DeferGameplayTagTreeRebuilds;
	ensure(DeferGameplayTagTreeRebuilds >= 0);
	if (DeferGameplayTagTreeRebuilds == 0 && bGameplayTagTreeRebuildWasDeferred)
	{
		bGameplayTagTreeRebuildWasDeferred = false;
		HandleGameplayTagTreeChanged(true);
	}
}
...
bool UGameplayTagsManager::ShouldUnloadTags() const
{
	...
	if (bShouldAllowUnloadingTags && GIsEditor && GEngine)
	{
		...
		{
			UE_LOGF(LogGameplayTags, Warning, "Ignoring request to unload tags during Play In Editor because AllowGameTagUnloading=false");
			return false;
		}
	}
#endif
...
{
	bool bWasDeferred = ShouldDeferGameplayTagTreeRebuilds.IsSet() && ShouldDeferGameplayTagTreeRebuilds.GetValue();

	ShouldDeferGameplayTagTreeRebuilds = bShouldDeferRebuilds;

	if (!bWasDeferred && bShouldDeferRebuilds)
	{
		PushDeferGameplayTagTreeRebuilds();
	}
	else if (bWasDeferred && !bShouldDeferRebuilds)
	{
		PopDeferGameplayTagTreeRebuilds();
	}
}
...
void UGameplayTagsManager::ClearShouldDeferGameplayTagTreeRebuilds(bool bRebuildTree)
{
	...
	if (bWasDeferred)
	{
		// This will do a rebuild if one was queued and there were no other deferrals
		PopDeferGameplayTagTreeRebuilds();
	} 
}
...
{
	// This should never be called during shutdown, use GetIfAllocated
	...
	SingletonManager->ChildNodesCache.Empty(CVarGameplayTagsLruCacheSize.GetValueOnAnyThread());
	...
	FCoreDelegates::GetOnPostEngineInit().AddUObject(SingletonManager, &UGameplayTagsManager::DoneAddingNativeTags);
}
...
void UGameplayTagsManager::PopulateTreeFromDataTable(class UDataTable* InTable)
{
	...
	static const FString ContextString("UGameplayTagsManager::PopulateTreeFromDataTable");
...
void UGameplayTagsManager::AddTagTableRow(const FGameplayTagTableRow& TagRow, FName SourceName, bool bIsRestrictedTag)
{
...
#if WITH_EDITOR
	{
		...
		if (!IsValidGameplayTagString(FullTagString, &ErrorText, &FixedString))
		{
			...
			{
				// No way to fix it
				UE_LOGF(LogGameplayTags, Error, "Invalid tag %ls from source %ls: %ls!", *FullTagString, *SourceName.ToString(), *ErrorText.ToString());
				return;
			}
			else
			{
				UE_LOGF(LogGameplayTags, Error, "Invalid tag %ls from source %ls: %ls! Replacing with %ls, you may need to modify InvalidTagCharacters", *FullTagString, *SourceName.ToString(), *ErrorText.ToString(), *FixedString);
				FullTagString.Reset();
...
{
	if (DeferBroadcastOnGameplayTagTreeChanged > 0)
	{
		bBroadcastOnGameplayTagTreeChangedWasDeferred = true;
	}
	else
	{
...
void UGameplayTagsManager::HandleGameplayTagTreeChanged(bool bRecreateTree)
{
	...
	{
		if (bRecreateTree && DeferGameplayTagTreeRebuilds > 0)
		{
			// Defer the full rebuild but do the simple one
			bGameplayTagTreeRebuildWasDeferred = true;
			bRecreateTree = false;
		}
...
void UGameplayTagsManager::DestroyGameplayTagTree()
{
	...
	ChildNodesCache.Empty(ChildNodesCache.Max());
}
...
int32 UGameplayTagsManager::InsertTagIntoNodeArray(FName Tag, FName FullTag, TSharedPtr<FGameplayTagNode> ParentNode, TArray< TSharedPtr<FGameplayTagNode> >& NodeArray, FName SourceName, const FString& DevComment, bool bIsExplicitTag, bool bIsRestrictedTag, bool bAllowNonRestrictedChildren)
{
	...
	if (FoundNodeIdx == INDEX_NONE)
	{
...

		{
			...
			// Invalidate the cache behind the lock
			ChildNodesCache.Empty(ChildNodesCache.Max());
		}
	}
...
void UGameplayTagsManager::PrintReplicationIndices()
{
	...
	UE_LOGF(LogGameplayTags, Display, "::PrintReplicationIndices (TOTAL %d)", GameplayTagNodeMap.Num());
	...
	for (auto It : GameplayTagNodeMap)
	{
		...
		UE_LOGF(LogGameplayTags, Display, "Tag %ls NetIndex: %d", *Tag.ToString(), Node->GetNetIndex());
	}
...
void UGameplayTagsManager::PrintReplicationFrequencyReport()
{
	...
	UE_LOGF(LogGameplayTags, Warning, "=================================");
	UE_LOGF(LogGameplayTags, Warning, "Gameplay Tags Replication Report");

	UE_LOGF(LogGameplayTags, Warning, "\nTags replicated solo:");
	ReplicationCountMap_SingleTags.ValueSort(TGreater<int32>());
	for (auto& It : ReplicationCountMap_SingleTags)
	{
		UE_LOGF(LogGameplayTags, Warning, "%ls - %d", *It.Key.ToString(), It.Value);
	}
	...
	for (int32 Bits=1; Bits < NetIndexTrueBitNum; ++Bits)
	{
		...
		for (auto& It : ReplicationCountMap)
		{
			...
			//UE_LOGF(LogGameplayTags, Warning, "[Bits: %d] Tag %ls would save %d bits", Bits, *It.Key.ToString(), Savings);
			ExpectedNetIndex++;
	...
	for (auto& It : SavingsMap)
	{
		...
		UE_LOGF(LogGameplayTags, Warning, "%d bits would save %d (%.2f)", It.Key, It.Value, (float)It.Value / (float)BaselineCost);
	}
	...
	for (auto& It : ReplicationCountMap)
	{
		UE_LOGF(LogGameplayTags, Warning, "+CommonlyReplicatedTags=%ls", *It.Key.ToString());
	...
	UE_LOGF(LogGameplayTags, Warning, "NetIndexFirstBitSegment=%d", BestBits);
...
{
	if (DeferGameplayTagTreeRebuilds > 0)
	{
		// This will call HandleGameplayTagTreeChanged(true) which comes back to here
		bGameplayTagTreeRebuildWasDeferred = true;
...
{
public:
	static void Validate(FCbFieldViewIterator Args, UE::Cook::FCookDependencyContext& Context)
	{
		UGameplayTagsManager& Manager = UGameplayTagsManager::Get();
		Manager.ValidateGameplayTagCookDependencies(Args, Context);
	}
};

UE_COOK_DEPENDENCY_FUNCTION(GameplayTagDependency, UGameplayTagsManagerIncrementalCookFunctions::Validate);

UE::Cook::FCookDependency UGameplayTagsManager::CreateCookDependency(const FGameplayTag& Tag) const
{
	GameplayTagUtil::FGameplayTagCookDependency Dependencies = CreateGameplayTagCookDependency(Tag);

	FCbWriter Writer;
	Dependencies.Save(Writer);
	FCbFieldIterator Fields = Writer.Save();

	return UE::Cook::FCookDependency::Function(UE_COOK_DEPENDENCY_FUNCTION_CALL(GameplayTagDependency), MoveTemp(Fields));
}

void UGameplayTagsManager::AppendDependencyHash(const FName& TagName, FBlake3& OutHasher) const
{
	FGameplayTag Tag(TagName);
	GameplayTagUtil::FGameplayTagCookDependency Dependencies = CreateGameplayTagCookDependency(Tag);
	Dependencies.Hash(OutHasher);
}

GameplayTagUtil::FGameplayTagCookDependency UGameplayTagsManager::CreateGameplayTagCookDependency(const FGameplayTag& Tag) const
{
	GameplayTagUtil::FGameplayTagCookDependency Dependencies;
	{
		UE::TScopeLock Lock(GameplayTagMapCritical);

		// Redirectors
		bool bRedirectorFound = FGameplayTagRedirectors::Get().FindRedirectorsFromTargetTagName(Tag.GetTagName(), Dependencies.SourceTagNames);

		// Does the tag exist
		const TSharedPtr<FGameplayTagNode>* Node = GameplayTagNodeMap.Find(Tag);
		Dependencies.bTagExists = (Node && Node->IsValid()) ? true : false;

		Dependencies.TagName = Tag.GetTagName();
	}

	return Dependencies;
}

void UGameplayTagsManager::ValidateGameplayTagCookDependencies(const FCbFieldViewIterator& Args, UE::Cook::FCookDependencyContext& Context) const
{
	UE::TScopeLock Lock(GameplayTagMapCritical);

	FCbFieldViewIterator ArgsField(Args);
	GameplayTagUtil::FGameplayTagCookDependency Dependency;
	bool bSuccess = Dependency.Load(ArgsField);
	if (!bSuccess)
	{
		Context.LogInvalidated(FString::Printf(TEXT("Failed to load the parameters.")));
		return;
	}

	const FGameplayTag Tag(Dependency.TagName);
	const TSharedPtr<FGameplayTagNode>* Node = GameplayTagNodeMap.Find(Tag);
	bool bTagExists = (Node && Node->IsValid()) ? true : false;
	if (bTagExists != Dependency.bTagExists)
	{
		if (bTagExists)
		{
			Context.LogInvalidated(FString::Printf(TEXT("Tag %s was missing but does exist now."), *Dependency.TagName.ToString()));
		}
		else
		{
			Context.LogInvalidated(FString::Printf(TEXT("Tag %s used to exist but is now missing."), *Dependency.TagName.ToString()));
		}

		return;
	}

	if (Dependency.SourceTagNames.IsEmpty())
	{
		// TagName was not redirected in the past. Let's check if it has a redirection now
		FGameplayTag TargetTag;
		bool bRedirectorFound = FGameplayTagRedirectors::Get().RedirectTag(Dependency.TagName, TargetTag);
		if (bRedirectorFound)
		{
			Context.LogInvalidated(FString::Printf(TEXT("New redirection for tag %s. Target tag is %s."),
				*Dependency.TagName.ToString(), *TargetTag.GetTagName().ToString()));
	...
	{
		// TagName was the target of redirectors. SourceTagNames used to be the sources. Let's check if the sources have changed.
		// I don't use FGameplayTagRedirectors::FindRedirectorsFromTargetTagName here because RedirectTag is faster and because I don't care
		// about any new redirectors.
		for (const FName& SourceTagName : Dependency.SourceTagNames)
		{
			FGameplayTag TargetTag;
			bool bRedirectorFound = FGameplayTagRedirectors::Get().RedirectTag(SourceTagName, TargetTag);
			if (!bRedirectorFound)
			{
				Context.LogInvalidated(FString::Printf(TEXT("Tag %s used to be redirected to %s but the redirection can't be found anymore."),
					*SourceTagName.ToString(), *Dependency.TagName.ToString()));
				return;
			}

			FName TargetTagName = TargetTag.GetTagName();
			if (TargetTagName != Dependency.TagName)
			{
				Context.LogInvalidated(FString::Printf(TEXT("Tag %s redirection was modified. It used to be redirected to %s and is now redirected to %s."),
					*SourceTagName.ToString(), *Dependency.TagName.ToString(), *TargetTagName.ToString()));
...
bool IsValidGameplayTagString(const FStringView& TagString, FText* OutError, FixedStringType* OutFixedString, const FString& InvalidTagCharacters)
{
	...
	/*
	FStringView SubString = TagString;
	while (SubString.Len() > 0)
	{
		const FStringView Trimmed = FAsciiSet::TrimPrefixWithout(SubString, Period);
		if (const FStringView SubTrim = FAsciiSet::TrimPrefixWith(Trimmed.RightChop(1), Period); SubTrim.Len() != Trimmed.Len() - 1 && SubTrim.Len() > 0)
		{
			if (OutError)
			{
				Errors.Add(LOCTEXT("DoubleDelimiter", "Tag may not have double delimiters ('..')"));
			}
			bIsValid = false;
			TArray<FStringView> FixedSubstrings = {FixedString.LeftChop(Trimmed.Len()), SubTrim};
			TStringBuilder<2048> Rebuilder(InPlace, UE::String::Join(FixedSubstrings, '.'));
			FixedString = Rebuilder.ToString();
		}

		SubString = Trimmed.RightChop(1);
	}
	*/
...
FGameplayTag UGameplayTagsManager::AddNativeGameplayTag(FName TagName, const FString& TagDevComment)
{
	...
	// Add to temporary tree and call nondestructive notify, which won't do anything if we're still in initial startup
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS // Remove when OnLastChanceToAddNativeTags is made private

FDelegateHandle UGameplayTagsManager::CallOrRegister_OnAddNativeTagsDelegate(const FSimpleMulticastDelegate::FDelegate& Delegate)
{
	UGameplayTagsManager* Manager = UGameplayTagsManager::GetIfAllocated();
	if (Manager && Manager->bDoneAddingNativeTags)
	{
		if (!Manager->HandleModuleObjectsLoadedHandle.IsValid())
		{
			// Need to schedule a module loaded callback as this is called from the FGameplayTagNativeAdder constructor which can't safely call the virtual functions at this scope
			Manager->HandleModuleObjectsLoadedHandle = FModuleManager::Get().OnProcessLoadedObjectsCallback().AddUObject(Manager, &UGameplayTagsManager::HandleModuleObjectsLoaded);
		}
	}

	return OnLastChanceToAddNativeTags().Add(Delegate);
}

FDelegateHandle UGameplayTagsManager::CallOrRegister_OnDoneAddingNativeTagsDelegate(const FSimpleMulticastDelegate::FDelegate& Delegate)
{
	UGameplayTagsManager* Manager = UGameplayTagsManager::GetIfAllocated();
	if (Manager && Manager->bDoneAddingNativeTags)
	{
...
void UGameplayTagsManager::UnregisterNativeTagDelegate(FDelegateHandle DelegateHandle)
{
	if (DelegateHandle.IsValid() && !IsEngineExitRequested())
	{
		if (!OnLastChanceToAddNativeTags().Remove(DelegateHandle))
		{
			OnDoneAddingNativeTagsDelegate().Remove(DelegateHandle);
		}
	}
}
...
void UGameplayTagsManager::DoneAddingNativeTags()
{
	...
	// CallOrRegister_OnAddNativeTags will execute immediately after the first successful call
	if (GEngine && !bDoneAddingNativeTags)
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(UGameplayTagsManager::DoneAddingNativeTags);
		
		FSimpleMulticastDelegate& AddNativeTagsDelegate = OnLastChanceToAddNativeTags();

		UE_CLOGF(GAMEPLAYTAGS_VERBOSE, LogGameplayTags, Display, "UGameplayTagsManager::DoneAddingNativeTags. DelegateIsBound: %d", (int32)AddNativeTagsDelegate.IsBound());
		AddNativeTagsDelegate.Broadcast();
		AddNativeTagsDelegate.Clear();
...
void UGameplayTagsManager::HandleModuleObjectsLoaded(FName InModuleName, bool bCanProcessNewlyLoadedObjects)
{
	// Remove the loaded callback
	FModuleManager::Get().OnProcessLoadedObjectsCallback().Remove(HandleModuleObjectsLoadedHandle);
	HandleModuleObjectsLoadedHandle.Reset();

	// Handle any pending native tag callbacks from the last load
	FSimpleMulticastDelegate& AddNativeTagsDelegate = OnLastChanceToAddNativeTags();
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UGameplayTagsManager_RequestGameplayTagChildren);
	UE::TScopeLock Lock(GameplayTagMapCritical);
	...
	if (GameplayTagNode.IsValid())
	{
		if (FGameplayTagContainer* Cached = ChildNodesCache.FindAndTouch(GameplayTag))
		{
			return *Cached;
		}
		...
		ChildNodesCache.SetMaxNumElements(CVarGameplayTagsLruCacheSize.GetValueOnAnyThread());
		if (ChildNodesCache.Max() > 0)
		{
			ChildNodesCache.Add(GameplayTag, TagContainer);
		}
	}
	return TagContainer;
}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UGameplayTagsManager_AddChildrenTags);

	TArray<FGameplayTag> TagsToAdd;
	TArray<TSharedPtr<FGameplayTagNode>> Queue;
	Queue.Push(GameplayTagNode);
	bool bFirstOne = true;
	while (!Queue.IsEmpty())
	{
		TSharedPtr<FGameplayTagNode> CurrentNode = Queue.Pop(EAllowShrinking::No);
		if (CurrentNode.IsValid())
		{
			bool bShouldInclude = !bFirstOne;

#if WITH_EDITORONLY_DATA
			if (OnlyIncludeDictionaryTags && !CurrentNode->IsExplicitTag())
			{
			...
			if (bShouldInclude)
			{
				TagsToAdd.Add(CurrentNode->GetCompleteTag());
			}

			if (bFirstOne || RecurseAll)
			{
				Queue.Append(CurrentNode->GetChildTagNodes());
			}

			bFirstOne = false;
		}
	}
	TagContainer.AppendTags(FGameplayTagContainer::CreateFromArray(TagsToAdd));
}
