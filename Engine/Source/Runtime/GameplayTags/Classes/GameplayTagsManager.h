...
#if WITH_EDITOR
namespace GameplayTagUtil { struct FGameplayTagCookDependency; }
...
ENUM_RANGE_BY_COUNT(EGameplayTagSourceType, EGameplayTagSourceType::Invalid);
...
class UGameplayTagsManager : public UObject
{
	...
	*/
	 ...
	 * This can be called after initial engine startup but can invalidate FastReplication
	 *
...
public:
	...
	UE_DEPRECATED(5.8, "Replace with CallOrRegister_OnAddNativeTagsDelegate or call AddNativeGameplayTag directly, tags can be safely added after startup with default settings")
	static GAMEPLAYTAGS_API FSimpleMulticastDelegate& OnLastChanceToAddNativeTags();

	/**
	 * Register a callback for initialization to add additional native tags right before fully initializing gameplay tags.
	 * Or, if the startup native tags have already been added, execute Delegate before the end of the current module load.
	 */
	static GAMEPLAYTAGS_API FDelegateHandle CallOrRegister_OnAddNativeTagsDelegate(const FSimpleMulticastDelegate::FDelegate& Delegate);
	 ...
	 */
	static GAMEPLAYTAGS_API FDelegateHandle CallOrRegister_OnDoneAddingNativeTagsDelegate(const FSimpleMulticastDelegate::FDelegate& Delegate);

	/** 
	 * Unregisters a previously registered callback from either OnAddNativeTagsDelegate or OnDoneAddingNativeTagsDelegate.
	 * If the delegate has already activated or the handle is empty this will do nothing.
	 */ 
	static GAMEPLAYTAGS_API void UnregisterNativeTagDelegate(FDelegateHandle DelegateHandle);
	...
	/** Temporarily disables IGameplayTagsModule::OnGameplayTagTreeChanged callback for changes to the gameplay tag tree. This does not stop it from actually rebuilding the tree */
	GAMEPLAYTAGS_API void PushDeferOnGameplayTagTreeChangedBroadcast();

	/** Re-enables OnGameplayTagTreeChanged callbacks, there must be one call to this function for every Push. This will execute callbacks immediately if they were deferred */
...
public:
	...
	/** 
	 * This is called after EditorRefreshGameplayTagTree. Useful if you need to do anything editor related when the gameplay tag tree is destroyed and recreated.
	 * IGameplayTagsModule::OnGameplayTagTreeChanged is called first and will happen when any tags are added or removed, not just when the settings are fully refreshed.
	 */
	...
	*/
	...
	/**
	 * Used for incremental cooking. Create an FCookDependency for a gameplay tag.
	 ...
	 * tags change.
	 */
	GAMEPLAYTAGS_API UE::Cook::FCookDependency CreateCookDependency(const FGameplayTag& Tag) const;

	GAMEPLAYTAGS_API void AppendDependencyHash(const FName& TagName, FBlake3& OutHasher) const;
...
private:
	...
	void VerifyNetworkIndex() const
	{
		...
		{
			UE_LOGF(LogGameplayTags, Warning, "%s called when not using FastReplication (not rebuilding the fast replication cache)", __func__);
		}
	...
	/** Handles pending tag changes */
	void HandleModuleObjectsLoaded(FName InModuleName, bool bCanProcessNewlyLoadedObjects);
	
	/** Temporarily disables any game or editor rebuilds of the gameplay tag tree, this should only be used when making multiple tag changes at once */
	void PushDeferGameplayTagTreeRebuilds();

	/** Re-enables game and editor rebuilds of the gameplay tag tree, there must be one call to this function for every Push. This will rebuild the tree immediately if it was deferred */
	void PopDeferGameplayTagTreeRebuilds();

#if WITH_EDITOR

	GameplayTagUtil::FGameplayTagCookDependency CreateGameplayTagCookDependency(const FGameplayTag& Tag) const;
	
	void ValidateGameplayTagCookDependencies(const FCbFieldViewIterator& Args, UE::Cook::FCookDependencyContext& Context) const;
	...
	/** Used for HandleModuleObjectsLoaded */
	FDelegateHandle HandleModuleObjectsLoadedHandle;

	/** True if native tags have all been added and flushed */
	bool bDoneAddingNativeTags;

	/** Used to suppress calls to BroadcastOnGameplayTagTreeChanged*/
	int32 DeferBroadcastOnGameplayTagTreeChanged = 0;
	bool bBroadcastOnGameplayTagTreeChangedWasDeferred = false;

	/** Used to suppress calls to HandleGameplayTagTreeChanged that would result in a complete rebuild of the GameplayTag tree */
	int32 DeferGameplayTagTreeRebuilds = 0;
	bool bGameplayTagTreeRebuildWasDeferred = false;
	...
	mutable TLruCache<FGameplayTag, FGameplayTagContainer> ChildNodesCache;
};
