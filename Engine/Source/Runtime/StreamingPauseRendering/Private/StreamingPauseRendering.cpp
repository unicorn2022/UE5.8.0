...
void FStreamingPauseRenderingModule::BeginStreamingPause( FViewport* GameViewport )
{
	...
	if (IsMoviePlayerEnabled() && (GetMoviePlayer()->IsInitialized() && !GetMoviePlayer()->IsMovieCurrentlyPlaying()))
	{
		...
		{
			const TStrongPtrVariant<FViewportClient>& ViewportClientVariant = GameViewport->GetClientPtr();
			FViewportClient* const ViewportClient = ViewportClientVariant.Get();
			...
			SceneViewport->EnqueueEndRenderFrame(false, false);
