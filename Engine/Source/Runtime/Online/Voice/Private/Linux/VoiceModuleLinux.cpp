...
void FVoiceCaptureSDL::ReadData(Uint8* Stream, int Length)
{
	...
	{
		UE_LOGF(LogVoiceCapture, Warning, "Discarding %i voice bytes\n", AudioBufferAvailableData + Length - AudioBuffer.Num());
	}
#endif
...
static void SDLCALL VoiceCallbackWrapper(void *UserData, SDL_AudioStream *AudioStream, int AdditionalAmount, int TotalAmount)
{
	if (AdditionalAmount > 0)
	{
		...
		if(Data)
		{
			int BytesRead = SDL_GetAudioStreamData(AudioStream, Data, AdditionalAmount);
			if (BytesRead > 0)
			{
				FVoiceCaptureSDL::AudioCallback(UserData, Data, BytesRead);
			}
			SDL_stack_free(Data);
		}
	}
...
bool FVoiceCaptureSDL::Init(const FString& DeviceName, int32 SampleRate, int32 NumChannels)
{
	...
	{
		UE_LOGF(LogVoiceCapture, Error, "Unable to open Audio Stream: %ls", ANSI_TO_TCHAR(SDL_GetError()));
		return false;
	}
...
bool FVoiceCaptureSDL::Start()
{
	...
	if (SDL_AudioStreamDevicePaused(AudioStream) == false)
	{
	...
	{
		UE_LOGF(LogVoiceCapture, Warning, "Failed to start capture");
		return false;
	}
}
...
{
	/** stubbed */
	UE_LOGF(LogVoiceCapture, Display, "FVoiceCaptureSDL::DumpState() is not implemented");
}
...
bool InitVoiceCapture()
{
	...
	{
		GVoiceCaptureNeedsToShutdownSDLAudio = SDL_InitSubSystem(SDL_INIT_AUDIO);
