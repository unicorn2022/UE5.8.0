...
bool FVoiceCaptureWindows::Init(const FString& DeviceName, int32 SampleRate, int32 NumChannels)
{
	...
	if (!VoiceDev)
	{
		UE_LOGF(LogVoiceCapture, Warning, "No voice capture interface.");
		return false;
	}
...
bool FVoiceCaptureWindows::CreateCaptureBuffer(const FString& DeviceName, int32 SampleRate, int32 NumChannels)
{
	...
	{
		UE_LOGF(LogVoiceCapture, Warning, "Voice capture doesn't support %d hz", SampleRate);
		return false;
	}

	if (NumChannels < 0 || NumChannels > 2)
	{
		UE_LOGF(LogVoiceCapture, Warning, "Voice capture only supports 1 or 2 channels");
		return false;
	}
	...
	if (DeviceInfo)
	{
		UE_LOGF(LogVoiceCapture, Display, "Creating capture %ls [%d:%d]", *DeviceInfo->DeviceName, SampleRate, NumChannels);
		...
		if (FAILED(hr))
		{
			...
			UE_LOGF(LogVoiceCapture, Warning, "Failed to create capture device 0x%08x", hr);
			return false;
		}
		...
		if (FAILED(hr))
		{
			UE_LOGF(LogVoiceCapture, Warning, "Failed to get mic device caps 0x%08x", hr);
			return false;
		}
		...
		if (FAILED(hr))
		{
			UE_LOGF(LogVoiceCapture, Warning, "Failed to create voice capture buffer 0x%08x", hr);
			return false;
		}
		...
		if (FAILED(hr))
		{
			UE_LOGF(LogVoiceCapture, Warning, "Failed to get voice buffer caps 0x%08x", hr);
			return false;
		}
		...
		{
			UE_LOGF(LogVoiceCapture, Warning, "Failed to create voice buffer notifications");
			return false;
		}
	}
	else
	{
		UE_LOGF(LogVoiceCapture, Warning, "No voice capture device %ls found.", *DeviceName);
		return false;
	}
...
bool FVoiceCaptureWindows::Start()
{
	...
	{
		UE_LOGF(LogVoiceCapture, Warning, "CV->VoiceCaptureBuffer8 == nullptr");
		return false;
	}
	...
	if (FAILED(hr))
	{
		UE_LOGF(LogVoiceCapture, Warning, "Failed to start capture 0x%08x", hr);
		return false;
	}
...
bool FVoiceCaptureWindows::ChangeDevice(const FString& DeviceName, int32 SampleRate, int32 NumChannels)
{
	...
	{
		UE_LOGF(LogVoiceCapture, Warning, "Unable to change device, not initialized");
		return false;
	}
}
...
{		
	if (CV->VoiceCaptureBuffer8)
	{
		...
		if (FAILED(hr))
		{
			UE_LOGF(LogVoiceCapture, Warning, "Failed to get voice buffer status 0x%08x", hr);
		}
...
void FVoiceCaptureWindows::ProcessData()
{
	...
	if (FAILED(hr))
	{
		UE_LOGF(LogVoiceCapture, Warning, "Failed to get voice buffer cursor position 0x%08x", hr);
		VoiceCaptureState = EVoiceCaptureState::Error;
		return;
	}
	...
	if(LockSize != 0) 
	{ 
		//UE_LOGF( LogVoiceCapture, Log, "LockSize: %i, CurrentCapturePos: %i, CurrentReadPos: %i, NextCaptureOffset: %i", LockSize, CurrentCapturePos, CurrentReadPos, CV->NextCaptureOffset );
		...
		if (SUCCEEDED(hr))
		{
			...
			{ 
				UE_LOGF(LogVoiceCapture, Warning, "Resetting UncompressedAudioBuffer.");
			...
			UE_LOGF(LogVoiceCapture, VeryVerbose, "LastCapture: %f %ls", (NewTime - LastCaptureTime) * 1000.0, EVoiceCaptureState::ToString(VoiceCaptureState));
			LastCaptureTime = NewTime;
#endif
		}
		else
		{
			UE_LOGF(LogVoiceCapture, Warning, "Failed to lock voice buffer 0x%08x", hr);
			VoiceCaptureState = EVoiceCaptureState::Error;
		}
	}
}
...
EVoiceCaptureState::Type FVoiceCaptureWindows::GetVoiceData(uint8* OutVoiceBuffer, const uint32 InVoiceBufferSize, uint32& OutBytesWritten, uint64& OutSampleClockCounter)
{
	...
	if (DisplayAmplitudeCvar && GEngine)
	{
		...
		if (FPlatformTime::Seconds() - TimeLastPrinted > AmplitudeStringDisplayRate)
		{
			...
			GEngine->AddOnScreenDebugMessage(30, static_cast<float>(AmplitudeStringDisplayRate), TextColor, PrintString, false);
...
bool FVoiceCaptureWindows::CreateNotifications(uint32 BufferSize)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		{
			UE_LOGF(LogVoiceCapture, Warning, "Error creating stop event");
		}
		else
		{
			...
			{
				UE_LOGF(LogVoiceCapture, Warning, "Failed to set stop notifications 0x%08x", hr);
	...
	{
		UE_LOGF(LogVoiceCapture, Warning, "Failed to create voice notification interface 0x%08x", hr);
	}
...
bool FVoiceCaptureWindows::Tick(float DeltaTime)
{
		...
		VoiceCaptureState != EVoiceCaptureState::NotCapturing)
	{
		...
		{
			UE_LOGF(LogVoiceCapture, Verbose, "Voice capture stopped");
...
void FVoiceCaptureWindows::DumpState() const
{
	...
	if (CV)
	{
		...
		UE_LOGF(LogVoiceCapture, Display, "Device %ls", *PrintMSGUID(&CV->VoiceCaptureDeviceGuid));
		...
		UE_LOGF(LogVoiceCapture, Display, "Capture Format");
		UE_LOGF(LogVoiceCapture, Display, "- Tag: %d", CV->WavFormat.wFormatTag);
		UE_LOGF(LogVoiceCapture, Display, "- Channels: %d", CV->WavFormat.nChannels);
		UE_LOGF(LogVoiceCapture, Display, "- BitsPerSample: %d", CV->WavFormat.wBitsPerSample);
		UE_LOGF(LogVoiceCapture, Display, "- SamplesPerSec: %d", CV->WavFormat.nSamplesPerSec);
		UE_LOGF(LogVoiceCapture, Display, "- BlockAlign: %d", CV->WavFormat.nBlockAlign);
		UE_LOGF(LogVoiceCapture, Display, "- AvgBytesPerSec: %d", CV->WavFormat.nAvgBytesPerSec);

		UE_LOGF(LogVoiceCapture, Display, "Capture Buffer");
		UE_LOGF(LogVoiceCapture, Display, "- Flags: 0x%08x", CV->VoiceCaptureBufferDesc.dwFlags);
		UE_LOGF(LogVoiceCapture, Display, "- BufferBytes: %d", CV->VoiceCaptureBufferDesc.dwBufferBytes);
		UE_LOG(LogVoiceCapture, Display, TEXT("- Format: 0x%08" UPTRINT_x_FMT), CV->VoiceCaptureBufferDesc.lpwfxFormat);

		UE_LOGF(LogVoiceCapture, Display, "Device Caps");
		UE_LOGF(LogVoiceCapture, Display, "- Size: %d", CV->VoiceCaptureDevCaps.dwSize);
		UE_LOGF(LogVoiceCapture, Display, "- Flags: 0x%08x", CV->VoiceCaptureDevCaps.dwFlags);
		UE_LOGF(LogVoiceCapture, Display, "- Formats: %d", CV->VoiceCaptureDevCaps.dwFormats);
		UE_LOGF(LogVoiceCapture, Display, "- Channels: %d", CV->VoiceCaptureDevCaps.dwChannels);

		UE_LOGF(LogVoiceCapture, Display, "D3D8 Caps");
		UE_LOGF(LogVoiceCapture, Display, "- Size: %d", CV->VoiceCaptureBufferCaps8.dwSize);
		UE_LOGF(LogVoiceCapture, Display, "- Flags: 0x%08x", CV->VoiceCaptureBufferCaps8.dwFlags);
		UE_LOGF(LogVoiceCapture, Display, "- BufferBytes: %d", CV->VoiceCaptureBufferCaps8.dwBufferBytes);
	}
	else
	{
		UE_LOGF(LogVoiceCapture, Display, "No capture device to dump state");
	}
#endif // !NO_LOGGING
}
