	...
	)
{
	...
	if (lpGuid != nullptr)
	{
		...
		{
			UE_LOGF(LogVoice, Display, "VoiceCapture device overridden by HMD to use '%s' %ls", lpcstrDescription, *PrintMSGUID(lpGuid));
			VCPtr->DefaultVoiceCaptureDevice = DeviceDesc;
...
public:
	...
	static HRESULT EnableDuckingOptOut(IMMDevice* pEndpoint, bool bDuckingOptOutChecked)
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			if (FAILED(hr))
			{
				UE_LOGF(LogVoiceCapture, Display, "Failed to duck audio endpoint. Error: 0x%08x", hr);
			}
	...
	static bool UpdateAudioDucking(bool bDuckingOptOutChecked)
	{
		...
		{
			{
				...
				if (SUCCEEDED(hr))
				{
					...
					UE_LOGF(LogVoiceCapture, Display, "%ls ducking on audio device. Desc: %ls", bDuckingOptOutChecked ? TEXT("Disabling") : TEXT("Enabling"), Desc);
					CoTaskMemFree(Desc);
						...
						if (SUCCEEDED(hr) && pCollEndpoint)
						{
							...
							if (SUCCEEDED(hr))
							{
								...
								if (SUCCEEDED(hr))
								{
									...
									UE_LOGF(LogVoiceCapture, Display, "%ls ducking on audio device [%d]: \"%ls\" (%ls)",
		...
		if (FAILED(hr))
		{
			UE_LOGF(LogVoiceCapture, Warning, "Failed to duck audio endpoint. Error: 0x%08x", hr);
		}
...
bool FVoiceCaptureDeviceWindows::Init()
{
	...
	if (FAILED(hr))
	{
		UE_LOGF(LogVoiceCapture, Warning, "Failed to init DirectSound %d", hr);
		return false;
	}
	...
	if (FAILED(hr))
	{
		UE_LOGF(LogVoiceCapture, Warning, "Failed to enumerate capture devices %d", hr);
		return false;
	}
