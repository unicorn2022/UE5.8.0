...
#define OPUS_CHECK_CTL(Category, CTL) \
	if (ErrCode != OPUS_OK) \
	{ \
		UE_LOGF(Category, Warning, "Failure to get CTL %s", #CTL); \
	}
...
void DebugEncoderInfo(OpusEncoder* Encoder)
{
	...
	UE_LOGF(LogVoiceEncode, Display, "Opus Encoder Details");
	UE_LOGF(LogVoiceEncode, Display, "- Application: %d", Application);
	UE_LOGF(LogVoiceEncode, Display, "- Signal: %d", Signal);
	UE_LOGF(LogVoiceEncode, Display, "- BitRate: %d", BitRate);
	UE_LOGF(LogVoiceEncode, Display, "- SampleRate: %d", SampleRate);
	UE_LOGF(LogVoiceEncode, Display, "- Vbr: %d", Vbr);
	UE_LOGF(LogVoiceEncode, Display, "- Complexity: %d", Complexity);
}
...
void DebugDecoderInfo(OpusDecoder* Decoder)
{
	...
	UE_LOGF(LogVoiceDecode, Display, "Opus Decoder Details");
	UE_LOGF(LogVoiceDecode, Display, "- Gain: %d", Gain);
	UE_LOGF(LogVoiceDecode, Display, "- Pitch: %d", Pitch);
}
...
void DebugFrameInfoInternal(const uint8* PacketData, uint32 PacketLength, uint32 SampleRate, bool bEncode)
{
	...
	{
		UE_LOGF(LogVoice, Warning, "opus_packet_get_nb_frames: Invalid voice packet data!");
	}
	...
	{
		UE_LOGF(LogVoice, Warning, "opus_packet_get_nb_samples: Invalid voice packet data!");
	}
	...
	if (bEncode)
	{
		UE_LOGF(LogVoiceEncode, Verbose, "PacketLength: %d NumFrames: %d NumSamples: %d Bandwidth: %ls Encoding: %d Stereo: %d FrameDesc: %d",
			PacketLength, NumFrames, NumSamples, BandwidthStr, TOCEncoding, TOCStereo, TOCMode);
	}
	else
	{
		UE_LOGF(LogVoiceDecode, Verbose, "PacketLength: %d NumFrames: %d NumSamples: %d Bandwidth: %ls Encoding: %d Stereo: %d FrameDesc: %d",
			PacketLength, NumFrames, NumSamples, BandwidthStr, TOCEncoding, TOCStereo, TOCMode);
	}
}
...
{
	UE_LOGF(LogVoiceEncode, Display, "EncoderVersion: %ls", ANSI_TO_TCHAR(opus_get_version_string()));
	...
	if (InSampleRate != 8000 &&
		...
		InSampleRate != 48000)
	{
		UE_LOGF(LogVoiceEncode, Warning, "Voice encoder doesn't support %d hz", InSampleRate);
		return false;
	}

	if (InNumChannels < 1 || InNumChannels > 2)
	{
		UE_LOGF(LogVoiceEncode, Warning, "Voice encoder only supports 1 or 2 channels");
		return false;
	}
	...
	{
		UE_LOGF(LogVoiceEncode, Warning, "Failed to init Opus Encoder: %ls", ANSI_TO_TCHAR(opus_strerror(EncError)));
		Destroy();
	}
...
int32 FVoiceEncoderOpus::Encode(const uint8* RawPCMData, uint32 RawDataSize, uint8* OutCompressedData, uint32& OutCompressedDataSize)
{
	...
	for (int32 i = 0; i < NumFramesToEncode; i++)
	{
		...
		if (CompressedLength < 0)
		{
			...
			UE_LOGF(LogVoiceEncode, Warning, "Failed to encode: [%d] %ls", CompressedLength, ANSI_TO_TCHAR(ErrorStr));
		...
		else if (CompressedLength != 1)
		{
			...
			UE_LOGF(LogVoiceEncode, VeryVerbose, "Entropy[%d]=%d", i, Entropy[LastEntropyIdx]);
		...
		{
			UE_LOGF(LogVoiceEncode, Warning, "Nothing to encode!");
	...
	UE_LOGF(LogVoiceEncode, VeryVerbose, "OpusEncode[%d]: RawSize: %d HeaderSize: %d CompressedSize: %d NumFramesEncoded: %d Remains: %d", Generation, RawDataSize, HeaderSize, OutCompressedDataSize, NumFramesToEncode, DataRemainder);
...
void FVoiceEncoderOpus::Reset()
{
	...
	if (Encoder)
	{
		...
		if (EncError != OPUS_OK)
		{
			UE_LOGF(LogVoiceEncode, Warning, "Failure to reset Opus encoder");
		}
...
void FVoiceEncoderOpus::DumpState() const
{
	...
	{
		UE_LOGF(LogVoiceEncode, Display, "No encoder to dump state");
	}
}
...
{
	UE_LOGF(LogVoiceDecode, Display, "DecoderVersion: %ls", ANSI_TO_TCHAR(opus_get_version_string()));
	...
	if (InSampleRate != 8000 &&
		...
		InSampleRate != 48000)
	{
		UE_LOGF(LogVoiceDecode, Warning, "Voice decoder doesn't support %d hz", InSampleRate);
		return false;
	}

	if (InNumChannels < 1 || InNumChannels > 2)
	{
		UE_LOGF(LogVoiceDecode, Warning, "Voice decoder only supports 1 or 2 channels");
		return false;
	}
	...
	{
		UE_LOGF(LogVoiceDecode, Warning, "Failed to init Opus Decoder: %ls", ANSI_TO_TCHAR(opus_strerror(DecError)));
		Destroy();
	}
...
void FVoiceDecoderOpus::Reset()
{
	...
	if (Decoder)
	{
		...
		if (DecError != OPUS_OK)
		{
			UE_LOGF(LogVoiceDecode, Warning, "Failure to reset Opus decoder");
		}
...
void FVoiceDecoderOpus::Decode(const uint8* InCompressedData, uint32 CompressedDataSize, uint8* OutRawPCMData, uint32& OutRawDataSize)
{
	...
	{
		UE_LOGF(LogVoiceDecode, Verbose, "Packet generation skipped from %d to %d", LastGeneration, PacketGeneration);
	}
	...
	if ((NumFramesToDecode > 0) && (NumFramesToDecode <= MaxFramesEncoded))
	{
				...
				if (UncompressedBufferAvail >= (MAX_OPUS_FRAMES * BytesPerFrame))
				{
					if (CompressedOffsets[i] > 0)
					{
						...
						if (NumDecompressedSamples < 0)
						{
							...
							UE_LOGF(LogVoiceDecode, Warning, "Failed to decode: [%d] %ls", NumDecompressedSamples, ANSI_TO_TCHAR(ErrorStr));
						}
						else
						{
							if (NumDecompressedSamples != FrameSize)
							{
								UE_LOGF(LogVoiceDecode, Warning, "Unexpected decode result NumSamplesDecoded %d != FrameSize %d", NumDecompressedSamples, FrameSize);
							}
							...
							{
								UE_LOGF(LogVoiceDecode, Verbose, "Decoder Entropy[%d/%d] = %d expected %d", i, NumFramesToDecode - 1, Entropy[LastEntropyIdx], EntropyOffsets[i]);
							}
					...
					{
						UE_LOGF(LogVoiceDecode, Verbose, "Decompression buffer skipped a frame");
						// Nothing to advance within the compressed input stream
					}
				}
				else
				{
					UE_LOGF(LogVoiceDecode, Warning, "Decompression buffer too small to decode voice");
					break;
				}
			}
		...
		{
			UE_LOGF(LogVoiceDecode, Warning, "Failed to decode: header corrupted");
			OutRawDataSize = 0;
		}
	}
	else
	{
		UE_LOGF(LogVoiceDecode, Warning, "Failed to decode: buffer corrupted");
		OutRawDataSize = 0;
	}

	UE_LOGF(LogVoiceDecode, VeryVerbose, "OpusDecode[%d]: RawSize: %d HeaderSize: %d CompressedSize: %d NumFramesDecoded: %d ", PacketGeneration, OutRawDataSize, HeaderSize, CompressedDataSize, NumFramesToDecode);
...
void FVoiceDecoderOpus::DumpState() const
{
	...
	{
		UE_LOGF(LogVoiceDecode, Display, "No decoder to dump state");
	}
}
