...
void FIOManagerBSDSocketSelectImpl::PollInternal(FTimespan WaitTime)
{
	...
	for (FInternalHandle RemovedHandle : RemovedHandles)
	{
		...
		{
			UE_LOGF(LogEventLoop, VeryVerbose, "[FIOManagerBSDSocketSelectImpl::PollInternal] Request removed: Socket: %p, Handle: %ls", reinterpret_cast<void*>(RemovedSocket), *RemovedHandle.ToString());
	...
	for (FInternalHandle AddedHandle : AddedHandles)
	{
		...
		if (FIORequestBSDSocket* NewRequest = IORequestStorage.Find(AddedHandle))
		{
			...
			{
				UE_LOGF(LogEventLoop, Verbose, "[FIOManagerBSDSocketSelectImpl::PollInternal] Request failed: Socket: %p, Reason: %ls", reinterpret_cast<void*>(NewRequest->Socket), *LexToString(ESocketIoRequestStatus::NoResources));
			...
			{
				UE_LOGF(LogEventLoop, Verbose, "[FIOManagerBSDSocketSelectImpl::PollInternal] Request failed: Socket: %p, Reason: %ls", reinterpret_cast<void*>(NewRequest->Socket), *LexToString(ESocketIoRequestStatus::Invalid));
				NewRequest->Callback(NewRequest->Socket, ESocketIoRequestStatus::Invalid, EIOFlags::None);
				IORequestStorage.Remove(AddedHandle);
				continue;
			}

			UE_LOGF(LogEventLoop, VeryVerbose, "[FIOManagerBSDSocketSelectImpl::PollInternal] Request added: Socket: %p, Handle: %ls, Flags: 0x%08X", reinterpret_cast<void*>(NewRequest->Socket), *AddedHandle.ToString(), uint32{EnumToUnderlyingType(NewRequest->Flags)});
	...
	if (SelectStatus > 0)
	{
		...
		for (TPair<FInternalHandle, FIORequestBSDSocket&>& IORequestEntry : IORequestStorage)
		{
			...
			{
				// Signal event status.
				UE_LOGF(LogEventLoop, VeryVerbose, "[FIOManagerBSDSocketSelectImpl::PollInternal] Request signaled: Socket: %p, Handle: %ls, Flags: 0x%08X", reinterpret_cast<void*>(IORequest.Socket), *IORequestEntry.Key.ToString(), uint32{EnumToUnderlyingType(SignaledFlags)});
	...
	{
		UE_LOGF(LogEventLoop, Fatal, "[FIOManagerBSDSocketSelectImpl::PollInternal] Select error: %ls", *GetLastErrorString());
	}
}
