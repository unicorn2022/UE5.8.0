...
public:
	...
	{
		const bool bWasValid = InHandle.IsValid();
	...
	virtual bool RunOnce(FTimespan WaitTime) override
	{
		...
		// waiting to be rescheduled. Ticking with zero delta reschedules them into the heap
		// without advancing internal time, which prevents short-interval timers from
		// immediately re-firing in the same iteration.
	...
	{
		Traits::CheckIsEventLoopThread(EventLoopThreadId);
		return LoopTime;
	}
...
private:
	...
	void TryNotify()
	{
		QUICK_SCOPE_CYCLE_COUNTER(STAT_TEventLoop_TryNotify);
