...
void FWinHttpWebSocket::Connect()
{
		...
		State == EWebSocketConnectionState::Connected)
	{
		...
		UE_LOGF(LogWebSockets, Verbose, "WinHttp WebSocket[%p]: Attempted to connect while in the %ls state.", this, LexToString(State));
		return;
	}
	...
	if (!bDisableDomainAllowlist)
	{
		...
		{
			UE_LOGF(LogWebSockets, Warning, "WinHttp WebSocket[%p]: %ls is not in the allowed list, refusing to connect.", this, *Url);
			HandleCloseComplete(EWebSocketConnectionState::FailedToConnect, UE_WEBSOCKET_CLOSE_APP_FAILURE, FString("Invalid Domain"));
			return;
		}
	}
	else
	{
		UE_LOGF(LogWebSockets, Log, "WinHttp WebSocket[%p]: Domain allowed list has been disabled by config.", this);
	}

	FWinHttpHttpManager* Manager = FWinHttpHttpManager::GetManager();
	if (!Manager)
	{
		UE_LOGF(LogWebSockets, Warning, "WinHttp WebSocket[%p]: WinHttp Manager shutdown", this);

		HandleCloseComplete(EWebSocketConnectionState::FailedToConnect, UE_WEBSOCKET_CLOSE_APP_FAILURE, FString("WinHttp Manager shutdown"));
		return;
	}
...
void FWinHttpWebSocket::Close(const int32 Code, const FString& Reason)
{
	switch (State)
	{
		...
		case EWebSocketConnectionState::Closed:
		{
			...
			UE_LOGF(LogWebSockets, Verbose, "WinHttp WebSocket[%p]: Close socket while in %ls state, ignoring", this, LexToString(State));
			return;
		}
		case EWebSocketConnectionState::Connecting:
		case EWebSocketConnectionState::Connected:
		{
			...
			{
				// Already closing
				UE_LOGF(LogWebSockets, Verbose, "WinHttp WebSocket[%p]: Call to close while another close was in progress.", this);
				return;
			}
...
void FWinHttpWebSocket::Send(const FString& Data)
{
	if (!IsConnected())
	{
		UE_LOGF(LogWebSockets, Warning, "WinHttp WebSocket[%p]: Failed to send message, we are not connected", this);
		return;
	}
...
{
	UE_LOGF(LogWebSockets, Verbose, "SetTextMessageMemoryLimit not implemented for WinHttpWebSocket.");
}
...
void FWinHttpWebSocket::HandleSessionCreated(FWinHttpSession* SessionPtr)
{
	...
	if (bCloseRequested)
	{
		...
		UE_LOGF(LogWebSockets, Warning, "WinHttp WebSocket[%p]: connection closed before it could start.", this);
	...
	if (!SessionPtr)
	{
		...
		UE_LOGF(LogWebSockets, Warning, "WinHttp WebSocket[%p]: Unable to create WinHttp Session, failing request", this);
		HandleCloseComplete(EWebSocketConnectionState::FailedToConnect, UE_WEBSOCKET_CLOSE_APP_FAILURE, FString("Unable to create WinHttp Session"));
		return;
	}
	...
	{
		UE_LOGF(LogWebSockets, Warning, "WinHttp WebSocket[%p]: Failed to create connection", this);
		HandleCloseComplete(EWebSocketConnectionState::FailedToConnect, UE_WEBSOCKET_CLOSE_APP_FAILURE, FString("Failed to create connection"));
		return;
	}
	...
	{
		UE_LOGF(LogWebSockets, Warning, "WinHttp WebSocket[%p]: Unable to start Connection", this);
		HandleCloseComplete(EWebSocketConnectionState::FailedToConnect, UE_WEBSOCKET_CLOSE_APP_FAILURE, FString("Failed to create connection"));
		return;
	}
...
void FWinHttpWebSocket::HandleCloseComplete(const EWebSocketConnectionState NewState, const uint16 Code, const FString& Reason)
{
	...
	{
		UE_LOGF(LogWebSockets, Warning, "WinHttp WebSocket[%p]: connection closed before while session creation in progress. Code=[%u] Reason=[%ls]", this, Code, *Reason);
		return;
	}
	...
	switch (PreviousState)
	{
		...
		case EWebSocketConnectionState::Closed:
		{
			...
			UE_LOGF(LogWebSockets, Verbose, "WinHttp WebSocket[%p]: Connection close occurred while in %ls state, ignoring. Code=[%u] Reason=[%ls]", this, LexToString(PreviousState), Code, *Reason);
			return;
		}
		case EWebSocketConnectionState::Connecting:
		{
			UE_LOGF(LogWebSockets, Log, "WinHttp WebSocket[%p]: Connection error occurred while connecting. Code=[%u] Reason=[%ls]", this, Code, *Reason);
		...
		case EWebSocketConnectionState::Connected:
		{
			UE_LOGF(LogWebSockets, Log, "WinHttp WebSocket[%p]: Connection closed. Code=[%u] Reason=[%ls]", this, Code, *Reason);
...
{
	UE_LOGF(LogWebSockets, Verbose, "WinHttp WebSocket[%p]: Received connection close event. Code=[%u] Reason=[%ls] bWasGraceful=[%d]", this, Code, *Reason, bGracefulDisconnect);
