...
void CALLBACK UE_WinHttpWebSocketStatusCallback(HINTERNET hInternet, DWORD_PTR dwContext, DWORD dwInternetStatus, LPVOID lpvStatusInformation, DWORD dwStatusInformationLength)
{
	...
	{
		UE_LOGF(LogWinHttp, Warning, "WebSocket[%p]: Received unknown WinHttp Status %lu", reinterpret_cast<void*>(dwContext), dwInternetStatus);
		return;
	}
	...
	const TMap<FString, FString>& InUpgradeHeaders)
{
	...
	{
		UE_LOGF(LogWinHttp, Warning, "Attempted to create a WinHttp WebSocket without an active session");
		return nullptr;
	}
	if (InUrl.IsEmpty())
	{
		UE_LOGF(LogWinHttp, Warning, "Attempted to create a WinHttp WebSocket with an empty url");
		return nullptr;
	}
	...
	{
		UE_LOGF(LogWinHttp, Warning, "Attempted to create an insecure WinHttp WebSocket which is disabled on this platform");
		return nullptr;
	}
	...
	if (Domain.IsEmpty())
	{
		UE_LOGF(LogWinHttp, Warning, "Attempted to create a WinHttp WebSocket with an unset domain");
		return nullptr;
	}
	const FString PathAndQuery = FGenericPlatformHttp::GetUrlPath(InUrl, true, false);
	if (PathAndQuery.IsEmpty())
	{
		UE_LOGF(LogWinHttp, Warning, "Attempted to create a WinHttp WebSocket with an unset path");
		return nullptr;
	}
...
{
	UE_LOGF(LogWinHttp, VeryVerbose, "WinHttp WebSocket[%p]: SendMessage MessageType=[%ls] MessageSize=[%d]", this, LexToString(MessageType), Message.Num());
...
{
	UE_LOGF(LogWinHttp, Verbose, "WinHttp WebSocket[%p]: sending Close message to server. Code=[%d] Reason=[%ls]", this, static_cast<int32>(Code), *Reason);

	FScopeLock StateLock(&SyncObject);

	if (!WebSocketHandle.IsValid())
	{
		UE_LOGF(LogWinHttp, Warning, "WinHttp WebSocket[%p]: could not send close message to server, our WebSocket has already shutdown. Code=[%d] Reason=[%ls]", this, static_cast<int32>(Code), *Reason);
		return false;
	}
	...
	if (SentCloseInfo.IsSet())
	{
		UE_LOGF(LogWinHttp, Warning, "WinHttp WebSocket[%p]: could not send close message to server, we have already sent a close message. Code=[%d] Reason=[%ls]", this, static_cast<int32>(Code), *Reason);
		return false;
	}
...
FWinHttpConnectionWebSocket::FWinHttpConnectionWebSocket(
	...
	const TArray<FString>& InProtocols,
	const TMap<FString, FString>& InUpgradeHeaders)
	: FWinHttpConnectionHttp(InSession, FString("GET"), InRequestUrl, bInIsSecure, InDomain, InPort, InPathAndQuery, InUpgradeHeaders, nullptr)
{
...
{
	UE_LOGF(LogWinHttp, VeryVerbose, "WinHttp WebSocket[%p]: Received Callback Status=[HEADERS_AVAILABLE]", this);
	...
	{
		UE_LOGF(LogWinHttp, Warning, "WinHttp WebSocket[%p]: Did not receive expected Switch Protocol response code, instead received %d", this, WebSocketUpgradeResponseCode);
		FinishRequest(EHttpRequestStatus::Failed);
		return;
	}
	...
	if (!WebSocketHandle.IsValid())
	{
		...
		UE_LOGF(LogWinHttp, Warning, "WinHttp WebSocket[%p]: Failed to complete WebSocket upgrade", this);
		FinishRequest(EHttpRequestStatus::Failed);
		return;
	}
	...
	if (WinHttpSetStatusCallback(WebSocketHandle.Get(), UE_WinHttpWebSocketStatusCallback, WebSocketNotifications, 0) == WINHTTP_INVALID_STATUS_CALLBACK)
	{
		...
		UE_LOGF(LogWinHttp, Warning, "WinHttp WebSocket[%p]: Failed to to set WebSocket callback", this);
		FinishRequest(EHttpRequestStatus::Failed);
		return;
	}
...
{
	UE_LOGF(LogWinHttp, VeryVerbose, "WinHttp WebSocket[%p]: Received Callback Status=[HANDLE_CLOSING]", this);
...
{
	UE_LOGF(LogWinHttp, VeryVerbose, "WinHttp WebSocket[%p]: Received Callback Status=[READ_COMPLETE] BytesRead=[%u] MessageType=[%ls] bIsFragment=[%d]", this, BytesRead, LexToString(MessageType), bIsFragment);
	...
	if (ReceiveFragmentMessageType.IsSet())
	{
		...
		{
			UE_LOGF(LogWinHttp, Warning, "WinHttp WebSocket[%p]: Received message with mismatched data type. ExpectedMessageType=[%ls] ActualMessageType=[%ls]", this, LexToString(ReceiveFragmentMessageType.GetValue()), LexToString(MessageType));
	...
	else
	{
		...
		UE_LOGF(LogWinHttp, VeryVerbose, "WinHttp WebSocket[%p]: Received complete message MessageType=[%ls]", this, LexToString(MessageType));
...
{
	UE_LOGF(LogWinHttp, VeryVerbose, "WinHttp WebSocket[%p]: Received Callback Status=[WRITE_COMPLETE] BytesWritten=[%u] MessageType=[%ls]", this, BytesWritten, LexToString(MessageType));
...
{
	UE_LOGF(LogWinHttp, VeryVerbose, "WinHttp WebSocket[%p]: Received Callback Status=[CLOSE_COMPLETE]", this);
	...
	UE_LOGF(LogWinHttp, Verbose, "WinHttp WebSocket[%p]: Received Close message from the server. Code=[%d] Reason=[%ls]", this, static_cast<int32>(CloseCode), *CloseReason);
...
{
	UE_LOGF(LogWinHttp, VeryVerbose, "WinHttp WebSocket[%p]: Received Callback Status=[REQUEST_ERROR] OperationId=[%d] ErrorCode=[0x%0.8X]", this, WebSocketOperationId, ErrorCode);
	...
	case WINHTTP_WEB_SOCKET_SHUTDOWN_OPERATION:
		...
		break;
	default:
		UE_LOGF(LogWinHttp, Error, "Unknown WebSocket API (%d) failed with error code 0x%0.8X", WebSocketOperationId, ErrorCode);
		break;
	}
...
void FWinHttpConnectionWebSocket::HandleWebSocketStatusCallback(HINTERNET ResourceHandle, EWinHttpCallbackStatus Status, void* StatusInformation, uint32 StatusInformationLength)
{
	...
	switch (Status)
	{
		...
		case EWinHttpCallbackStatus::WriteComplete:
		{
			...
			if (SocketStatus->eBufferType == WINHTTP_WEB_SOCKET_CLOSE_BUFFER_TYPE)
			{
				UE_LOGF(LogWinHttp, Error, "WinHttp WebSocket sent unexpected Close message");
				return;
			}
