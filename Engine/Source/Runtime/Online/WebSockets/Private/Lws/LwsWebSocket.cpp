...
{
	UE_LOGF(LogWebSockets, VeryVerbose, "FLwsWebSocket[%d]: Constructed url=%ls protocols=%ls", Identifier, *InUrl, *FString::Join(Protocols, TEXT(",")));
}

FLwsWebSocket::~FLwsWebSocket()
{
	UE_LOGF(LogWebSockets, VeryVerbose, "FLwsWebSocket[%d]: Destroyed", Identifier);
...
void FLwsWebSocket::Connect()
{
	...
	{
		UE_LOGF(LogWebSockets, Warning, "FLwsWebSocket[%d]::Connect: State is not None (%ls), unable to start connecting!", Identifier, ToString(LastGameThreadState));
		return;
	}

	if (!FLwsWebSocketsManager::Get().bDisableDomainAllowlist)
	{
		UE::Core::FURLRequestFilter DomainFilter(TEXT("Online.HttpManager"), GEngineIni);
		if (!DomainFilter.IsRequestAllowed(Url))
		{
			State = EState::Error;
			UE_LOGF(LogWebSockets, Warning, "FLwsWebSocket[%d]::Connect: %ls is not allowlisted. Refusing to connect.", Identifier, *Url);
	...
	{
		UE_LOGF(LogWebSockets, Log, "FLwsWebSocket[%d]::Connect: Domain allowlisting has been disabled by config.", Identifier);
	}
	...
	UE_LOGF(LogWebSockets, Verbose, "FLwsWebSocket[%d]::Connect: setting State=%ls url=%ls bWantsMessageEvents=%d bWantsRawMessageEvents=%d", Identifier, ToString(State), *Url, (int32)bWantsMessageEvents, (int32)bWantsRawMessageEvents);
...
void FLwsWebSocket::Close(int32 Code, const FString& Reason)
{
	...
	{
		UE_LOGF(LogWebSockets, Warning, "FLwsWebSocket[%d]::Close: Already closing, ignoring subsequent attempt", Identifier);
		return;
	}
	...
	UE_LOGF(LogWebSockets, Verbose, "FLwsWebSocket[%d]::Close: Close queued with code=%d reason=%ls", Identifier, Code, *Reason);
...
void FLwsWebSocket::SendFromQueue()
{
	...
	if (SendQueue.Peek(CurrentBuffer))
	{
		...
		if (!bWriteSuccessful)
		{
			...
			UE_LOGF(LogWebSockets, Warning, "FLwsWebSocket[%d]::SendFromQueue: Error writing buffer Size=%d BytesWritten=%d bIsBinary=%d", 
		...
		{
			UE_LOGF(LogWebSockets, VeryVerbose, "FLwsWebSocket[%d]::SendFromQueue: Wrote %d bytes, %d bytes remaining in this packet", 
...
int FLwsWebSocket::LwsCallback(lws* Instance, lws_callback_reasons Reason, void* Data, size_t Length)
{
	...
	case LWS_CALLBACK_CLIENT_ESTABLISHED:
	{
		...
		UE_LOGF(LogWebSockets, Verbose, "FLwsWebSocket[%d]::LwsCallback: Received LWS_CALLBACK_CLIENT_ESTABLISHED, setting State=%ls PreviousState=%ls", Identifier, ToString(State), ToString(PreviousState));
	...
	case LWS_CALLBACK_CLIENT_RECEIVE:
	{
		...
		if (bWantsMessageEvents && !bIsBinary)
		{
			...
			if (ReceiveBuffer.Len() + Length > MaxTextMessageBufferSize)
			{
				...
				UE_LOGF(LogWebSockets, Verbose, "Received text message too large - use SetTextMessageMemoryLimit() to increase buffer size. Current Size=%llu", MaxTextMessageBufferSize);
			}
			else
			{
	...
	case LWS_CALLBACK_WS_PEER_INITIATED_CLOSE:
	{
			...
			State == EState::ClosingByRequest)
		{
			...
			UE_LOGF(LogWebSockets, Verbose, "FLwsWebSocket[%d]::LwsCallback: Received LWS_CALLBACK_WS_PEER_INITIATED_CLOSE, setting State=%ls CloseStatus=%d Reason=%ls bPeerSpecifiedReason=%d PreviousState=%ls",
		...
		{
			UE_LOGF(LogWebSockets, Verbose, "FLwsWebSocket[%d]::LwsCallback: Received LWS_CALLBACK_WS_PEER_INITIATED_CLOSE, but ignoring because our State=%ls CloseStatus=%d Reason=%ls",
	...
	case LWS_CALLBACK_WSI_DESTROY:
		...
		{
			FString CloseReasonString("Connection timed out");
			UE_LOGF(LogWebSockets, Verbose, "FLwsWebSocket[%d]::LwsCallback: Received LWS_CALLBACK_WSI_DESTROY, setting State=%ls PreviousState=%ls", Identifier, ToString(EState::Error), ToString(State));
		...
		else
		{
			...
			UE_LOGF(LogWebSockets, Verbose, "FLwsWebSocket[%d]::LwsCallback: Received LWS_CALLBACK_WSI_DESTROY, State=%ls", Identifier, ToString(State));
		}
		LwsConnection = nullptr;
		break;
	case LWS_CALLBACK_CLOSED:
	{
			...
			State == EState::ClosingByRequest)
		{
			...
			static const FString LocalInitiatedReason = "Successfully closed connection to our peer";
			static const FString PeerInitiatedReason = "Connection closed by peer";
			FString CloseReasonString = bClosingByRequest ? LocalInitiatedReason : PeerInitiatedReason;
			UE_LOGF(LogWebSockets, Verbose, "FLwsWebSocket[%d]::LwsCallback: Received LWS_CALLBACK_CLOSED, setting State=%ls CloseReason=%ls PreviousState=%ls",
		...
		{
			UE_LOGF(LogWebSockets, Verbose, "FLwsWebSocket[%d]::LwsCallback: Received LWS_CALLBACK_CLOSED, ignoring and waiting for LWS_CALLBACK_WSI_DESTROY, State=%ls",
	...
	case LWS_CALLBACK_CLIENT_CONNECTION_ERROR:
	{
		...
		bool bWakeGameThread = false;
		UE_LOGF(LogWebSockets, Verbose, "FLwsWebSocket[%d]::LwsCallback: Received LWS_CALLBACK_CLIENT_CONNECTION_ERROR, setting State=%ls CloseReason=%ls PreviousState=%ls",
...
void FLwsWebSocket::GameThreadFinalize()
{
	...
	UE_LOGF(LogWebSockets, Verbose, "FLwsWebSocket[%d]::GameThreadFinalize: setting State=%ls PreviousState=%ls",
...
void FLwsWebSocket::LwsThreadTick()
{
	if (State == EState::Connected)
	{
		...
		if (State == EState::ClosingByRequest)
		{
			UE_LOGF(LogWebSockets, Verbose, "FLwsWebSocket[%d]::LwsThreadTick: Close requested while connected, setting State=%ls PreviousState=%ls",
...
void FLwsWebSocket::ConnectInternal(struct lws_context &LwsContext)
{
	...
	UE_LOGF(LogWebSockets, Verbose, "FLwsWebSocket[%d]::ConnectInternal: setting State=%ls PreviousState=%ls",
	...
	{
		FString Reason("Bad URL");

		UE_LOGF(LogWebSockets, Verbose, "FLwsWebSocket[%d]::ConnectInternal: setting State=%ls PreviousState=%ls Reason=%ls",
	...
	{
		UE_LOGF(LogWebSockets, Verbose, "FLwsWebSocket[%d]::ConnectInternal: lws_client_connect_via_info succeeded", Identifier);
	}
	else
	{
		FString Reason("Could not initialize connection");
