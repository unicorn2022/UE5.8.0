...
void FUnixPlatformSslCertificateManager::BuildRootCertificateArray()
{
	...
	for (const TCHAR* CurrentBundle : KnownBundlePaths)
	{
		...
		UE_LOGF(LogSsl, Log, "Checking if '%ls' exists", *FileName);

		if (FPaths::FileExists(FileName))
		{
			UE_LOGF(LogSsl, Log, "Loading certificates from %ls", *FileName);
	...
	{
		UE_LOGF(LogSsl, Warning, "Unable to find a cert bundle in any of known locations. Platform provided certificates will not be used");
	}
	else
	{
		UE_LOGF(LogSsl, Warning, "Unable to find a cert bundle in any of known locations. TLS may not work.");
	}
}
