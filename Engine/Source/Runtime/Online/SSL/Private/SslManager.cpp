...
bool FSslManager::InitializeSsl()
{
	...
	if (InitCount == 1)
	{
		UE_LOGF(LogSsl, Log, "Initializing SSL");
...
void FSslManager::ShutdownSsl()
{
	...
	if (InitCount == 0)
	{
		UE_LOGF(LogSsl, Log, "Shutting down SSL");
		EVP_cleanup();
...
#undef RESTRICT_SSL_TLS_PROTOCOL
			...
			{
				UE_LOGF(LogSsl, Warning, "FSslManager::CreateSslContext: Failed to create the SSL context: %ls", *GetSslErrorString());
			}
		}
		else
		{
			UE_LOGF(LogSsl, Warning, "FSslManager::CreateSslContext: Failed to create method SSLv23_client_method: %ls", *GetSslErrorString());
		}
	}
#endif // IS_MONOLITHIC || UE_MERGED_MODULES
	return SslContext;
