...
void FWindowsPlatformSslCertificateManager::BuildRootCertificateArray()
{
		...
		while (nullptr != (CertContext = CertEnumCertificatesInStore(SystemRootStore, CertContext)))
		{
			if ((CertContext->dwCertEncodingType & X509_ASN_ENCODING) != 0)
			{
				...
				else
				{
					...
					UE_LOGF(LogSsl, Log, "Unable to convert certificate: name:%ls error:%ls", Name, *GetSslErrorString());
				}
			}
			else
			{
				TCHAR Name[128];
				CertGetNameString(CertContext, CERT_NAME_SIMPLE_DISPLAY_TYPE, 0, nullptr, Name, UE_ARRAY_COUNT(Name));
				UE_LOGF(LogSsl, Log, "Unhandled certificate encoding: name:%ls encodingType:0x%08x", Name, CertContext->dwCertEncodingType);
			}
		}
	...
	{
		UE_LOGF(LogSsl, Warning, "Unable to open ROOT certificate store. Platform provided certificates will not be used");
	}
	else
	{
		UE_LOGF(LogSsl, Warning, "Unable to open ROOT certificate store. TLS may not work.");
	}
}
