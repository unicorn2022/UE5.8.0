...
public:
	...
	virtual BuildPatchServices::IDirectoryChunkerRef CreateDirectoryChunker(BuildPatchServices::FDirectoryChunkerConfiguration Configuration) const override;
	...
	virtual BuildPatchServices::IChunkDeltaOptimiserRef CreateChunkDeltaOptimiser(BuildPatchServices::FChunkDeltaOptimiserConfiguration Configuration) const override;
	virtual BuildPatchServices::IChunkHarvesterRef CreateChunkHarvester(BuildPatchServices::FChunkHarvesterConfiguration Configuration) const override;
	...
	virtual void RegisterEncryptionSecret(const FGuid& EncryptionSecretId, TArray<uint8> EncryptionSecretKey) override;
	virtual bool UnregisterEncryptionSecret(const FGuid& EncryptionSecretId) override;
	virtual bool CreateRandomBytes(const TArrayView<uint8> OutData) override;
	...
	virtual IBuildManifestPtr MergeManifests(const IBuildManifestRef& ManifestA, const IBuildManifestRef& ManifestB, const FString& NewVersionString, const TSet<FString>& FilesFromA, const TSet<FString>& FilesFromB) override;
...
private:
	...
	// Get the encryption secret by the specified secret ID.
	TOptional<TArray<uint8>> FindEncryptionSecret(const FGuid& EncryptionSecretId) const;

	// Get all registered encryption secrets.
	TMap<FGuid, TArray<uint8>> GetEncryptionSecrets() const;

	// Decrypt manifest when the manifest's encryption secret is available
	void DecryptManifestIfNecessary(FBuildPatchAppManifest& Manifest) const;
...
private:
	...
	// Critical section for multi-threaded access to AvailableEncryptionSecrets
	mutable FCriticalSection AvailableEncryptionSecretsCS;

	// Holds available encryption secrets used for decrypting installation data
	TMap<FGuid, TArray<uint8>> AvailableEncryptionSecrets;
