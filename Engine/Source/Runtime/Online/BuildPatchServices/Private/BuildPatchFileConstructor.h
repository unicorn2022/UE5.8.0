...
namespace BuildPatchServices
{
	...
	struct FFileConstructorConfig
	{
		...
		// Whether we are needing to use SHA1 hash string value as filename for temp staging files.
		bool bUseSHA1StageFilenames;
		...
		const TMap<FString, FString>* PerFileSubdirectories = nullptr;

		IBuildInstallerSharedContext* SharedContext = nullptr;

		// See comments in installer config (FBuildInstallerConfiguration)
	...
	public:
		...
		{
			// This should _never_ be called. I'm not sure how you can Reset() a constructor..
			check(false);
		}
		...
		/**
		 * Gets flag indicating that at least one file is corrupt due to invalid hash sum.
		 * @return	true if at least one file is corrupt.
		 */
		bool HasCorruptFiles() const;
		 ...
		 */
		DECLARE_EVENT_OneParam(FBuildPatchFileConstructor, FOnBeforeDeleteFile, const FString& /*FullFilePath*/);
		FOnBeforeDeleteFile& OnBeforeDeleteFile();

		/**
		 * Broadcasts with the build relative path for a file that has finished being staged (including by a previous incomplete installation).
		 * @return	Reference to the event object.
		 */
		DECLARE_EVENT_OneParam(FBuildPatchFileConstructor, FOnFileIsStaged, const FString& /*BuildFilename*/);
		FOnFileIsStaged& OnFileIsStaged();
	...
	private:
		 ...
		 * @param InProgressFileSize		The remaining size required for the file currently being constructed.
		 * @return Number of bytes needed to complete the installation, or zero if space will be freed by removing chunks.
		 */
		uint64 CalculateDiskSpaceRequirementsWithDeleteDuringInstall(int64 InProgressFileSize);

		/**
		 * Sequentially build the files required, potentially skipping already created files or resuming partial
		 * files.
		 */
		void ConstructFiles(FResumeData& ResumeData);
	...
	private:
		...
		struct FFileToConstruct
		{
			...
			// Name of the staging file. It may be obfuscated if bUseSHA1StageFilenames is true.
			FString StagingFilename;
		};
		...
		// Event executed when a file has now been staged or was previously staged.
		FOnFileIsStaged FileIsStagedEvent;
		...
		bool HarvestChunksForCompletedFile(const FString& InstallationDirectory, const FString& FilenameInManifest);
		// Starts an asynchronous truncation task in the chunk reading thread, which guarantees single-threaded access to chunkdb.
		void StartTruncateChunkDbForCompletedBatch(const FBatchState& Batch);

		void CompleteConstructedFile(FFileConstructionState& CurrentFile, FResumeData& ResumeData);

		// Determine and return the file stage based on its existence and file size.
		EFileStage GetFileStage(const FFileConstructionState& CurrentFile, const FResumeData& ResumeData) const;

		// Initialize and validate.
		void InitFile(FFileConstructionState& CurrentFile, const FResumeData& ResumeData);
		
		// Removes staged files that are no longer required for the current operation.
		void RemoveUnnecessaryStagedFiles(const FString& StagingDirectory) const;

		// Set of files considered corrupt due to an invalid hash sum.
		TSet<FString> CorruptFiles;
	...
	{
	public:
		virtual ~IFileConstructorStat() = default;
