...
{
	// Debug sanity checks
	static_assert(UE_ARRAY_COUNT(InstallErrorPrefixes::ErrorTypeStrings) == (uint64)EBuildPatchInstallError::InvalidOrMax, "Please add support for the extra values to the InstallErrorPrefixes::ErrorTypeStrings.");
#if UE_BUILD_DEBUG
...
IBuildInstallerRef FBuildPatchServicesModule::CreateBuildInstaller(BuildPatchServices::FBuildInstallerConfiguration Configuration, FBuildPatchInstallerDelegate CompleteDelegate) const
{
	...
	TMap<FGuid, TArray<uint8>> InstallerSecrets = GetEncryptionSecrets();
	InstallerSecrets.Append(MoveTemp(Configuration.EncryptionSecrets));
	Configuration.EncryptionSecrets = MoveTemp(InstallerSecrets);
...
IDirectoryChunkerRef FBuildPatchServicesModule::CreateDirectoryChunker(BuildPatchServices::FDirectoryChunkerConfiguration Configuration) const
{
	using namespace BuildPatchServices;
	return MakeShareable(FDirectoryChunkerFactory::Create(MoveTemp(Configuration)));
}
...
IChunkDeltaOptimiserRef FBuildPatchServicesModule::CreateChunkDeltaOptimiser(BuildPatchServices::FChunkDeltaOptimiserConfiguration Configuration) const
{
	using namespace BuildPatchServices;
	TMap<FGuid, TArray<uint8>> AllSecrets = GetEncryptionSecrets();
	AllSecrets.Append(MoveTemp(Configuration.EncryptionSecrets));
	Configuration.EncryptionSecrets = MoveTemp(AllSecrets);
	return MakeShareable(FChunkDeltaOptimiserFactory::Create(MoveTemp(Configuration)));
}

IChunkHarvesterRef FBuildPatchServicesModule::CreateChunkHarvester(BuildPatchServices::FChunkHarvesterConfiguration Configuration) const
{
	using namespace BuildPatchServices;
	return MakeShareable(FChunkHarvesterFactory::Create(MoveTemp(Configuration)));
}

void FBuildPatchServicesModule::DecryptManifestIfNecessary(FBuildPatchAppManifest& Manifest) const
{
	bool bIsManifestEncrypted = false;
	const FGuid EncryptionSecretId = Manifest.GetEncryptionSecretId(&bIsManifestEncrypted);
	if (!bIsManifestEncrypted)
	{
		return;
	}

	const TOptional<TArray<uint8>> EncryptionSecret = FindEncryptionSecret(EncryptionSecretId);
	if (!EncryptionSecret.IsSet())
	{
		return;
	}

	const TMap<FGuid, TArray<uint8>> EncryptionSecrets = { {EncryptionSecretId, *EncryptionSecret} };
	Manifest.DecryptData(EncryptionSecrets);
}
...
IBuildManifestPtr FBuildPatchServicesModule::LoadManifestFromFile( const FString& Filename )
{
	...
	{
		DecryptManifestIfNecessary(*Manifest);
		return Manifest;
	}
	else
	{
		GLog->Log(FString::Printf(TEXT("BuildPatchServicesModule: '%s' file is absent or can't be loaded."), *Filename));
		return nullptr;
	}
}
...
{
	return StaticCastSharedRef<FBuildPatchAppManifest>(Manifest)->SaveToFile(Filename, Manifest->GetFeatureLevel());
}
...
{
	return FBuildMergeManifests::MergeManifests(ManifestFilePathA, ManifestFilePathB, ManifestFilePathC, NewVersionString, SelectionDetailFilePath, GetEncryptionSecrets());
}

IBuildManifestPtr FBuildPatchServicesModule::MergeManifests(const IBuildManifestRef& ManifestA, const IBuildManifestRef& ManifestB, const FString& NewVersionString, const TSet<FString>& FilesFromA, const TSet<FString>& FilesFromB)
{
	return FBuildMergeManifests::MergeManifests(ManifestA, ManifestB, NewVersionString, FilesFromA, FilesFromB);
}
...
bool FBuildPatchServicesModule::UnregisterAppInstallation(FString AppInstallDirectory)
{
	FPaths::NormalizeDirectoryName(AppInstallDirectory);
	FPaths::CollapseRelativeDirectories(AppInstallDirectory);
	if (AvailableInstallations.Remove(AppInstallDirectory) > 0)
	{
		return true;
	}
	return false;
...
void FBuildPatchServicesModule::RegisterEncryptionSecret(const FGuid& EncryptionSecretId, TArray<uint8> EncryptionSecretKey)
{
	FScopeLock ScopeLock(&AvailableEncryptionSecretsCS);
	AvailableEncryptionSecrets.Add(EncryptionSecretId, MoveTemp(EncryptionSecretKey));
}

bool FBuildPatchServicesModule::UnregisterEncryptionSecret(const FGuid& EncryptionSecretId)
{
	FScopeLock ScopeLock(&AvailableEncryptionSecretsCS);
	if (AvailableEncryptionSecrets.Remove(EncryptionSecretId) > 0)
	{
		return true;
	}
	return false;
...
TOptional<TArray<uint8>> FBuildPatchServicesModule::FindEncryptionSecret(const FGuid& EncryptionSecretId) const
{
	FScopeLock ScopeLock(&AvailableEncryptionSecretsCS);
	const TArray<uint8>* EncryptionSecret = AvailableEncryptionSecrets.Find(EncryptionSecretId);
	return EncryptionSecret ? TOptional<TArray<uint8>>{*EncryptionSecret} : TOptional<TArray<uint8>>{};
}

TMap<FGuid, TArray<uint8>> FBuildPatchServicesModule::GetEncryptionSecrets() const
{
	FScopeLock ScopeLock(&AvailableEncryptionSecretsCS);
	return AvailableEncryptionSecrets;
}


bool FBuildPatchServicesModule::CreateRandomBytes(const TArrayView<uint8> OutData)
{
	TUniquePtr<ICrypto> Crypto{ FCryptoFactory::Create() };

	return Crypto->CreateRandomBytes(OutData);
}
