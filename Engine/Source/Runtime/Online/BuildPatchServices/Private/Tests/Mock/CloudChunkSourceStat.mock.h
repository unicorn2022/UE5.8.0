...
namespace BuildPatchServices
{
	...
	public:
		...
		typedef TTuple<double, int32> FActiveRequestCountAverageUpdated;
		typedef TTuple<double, FString, uint64> FCloudDirectoryDataReceived;
		typedef TTuple<double, FString> FCloudDirectoryError;
	...
	public:
		...
		{
			RxActiveRequestCountAverageUpdated.Emplace(FStatsCollector::GetSeconds(), AverageRequestCount);
		}
		...
		{
			RxCloudDirectoryDataReceived.Emplace(FStatsCollector::GetSeconds(), CloudDirectory, ReceivedData);
		}

		virtual void OnCloudDirectoryError(const FString& CloudDirectory) override
		{
			RxCloudDirectoryError.Emplace(FStatsCollector::GetSeconds(), CloudDirectory);
		}
	...
	public:
		...
		TArray<FActiveRequestCountUpdated> RxActiveRequestCountAverageUpdated;
		TArray<FCloudDirectoryDataReceived> RxCloudDirectoryDataReceived;
		TArray<FCloudDirectoryError> RxCloudDirectoryError;
	};
}
