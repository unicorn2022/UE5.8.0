...
void FFileAttributionSpec::MakeUnit()
{
	...
	for (const FString& File : AllFiles)
	{
		...
		FSHA1::HashBuffer(*FGuid::NewGuid().ToString(), sizeof(TCHAR) * 32, FileManifest.SHA1Hash.Hash);
		FileManifest.ChunkParts.Add(ChunkPart);
		FileManifest.FileMetaFlags |= ReadOnlyFiles.Contains(File) ? EFileMetaFlags::ReadOnly : EFileMetaFlags::None;
		FileManifest.FileMetaFlags |= CompressedFiles.Contains(File) ? EFileMetaFlags::Compressed : EFileMetaFlags::None;
		FileManifest.FileMetaFlags |= ExeFiles.Contains(File) ? EFileMetaFlags::UnixExecutable : EFileMetaFlags::None;
