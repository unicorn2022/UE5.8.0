	...
	xDescribe("CloudChunkSource", [this]()
	{
		...
		Describe("SetPaused", [this]()
		{
			BeforeEach([this]()
			{
				...
				FakeDownloadService->RequestFileFunc = [this](const FString&, const FDownloadCompleteDelegate&, const FDownloadProgressDelegate&, int32* OutRequestId) -> bool
				{
					if (++CallCount == 2)
					{
						PauseFor(PauseTime);
					}
					const int32 NewRequestId = ++FakeDownloadService->Count;
					if (OutRequestId)
					{
