...
namespace BuildPatchServices
{
	...
	inline bool FFakeHttpRequest::ProcessRequest()
	{
		++RxProcessRequest;
		return FakeHttpModule->OnProcessRequest(this);
	}

	inline int32 FFakeHttpResponse::GetResponseCode() const
	{
		return static_cast<int32>(Code);
	}

	inline const TArray<uint8>& FFakeHttpResponse::GetContent() const
	{
		return Data;
	}

	inline FString FFakeHttpResponse::GetContentAsString() const
	{
	...
	inline uint64 FFakeHttpResponse::GetContentLength() const
	{
		return Data.Num();
	}

	inline FFakeHttpManager::FFakeHttpManager(FTSTicker& Ticker)
	{
		Ticker.AddTicker(FTickerDelegate::CreateRaw(this, &FFakeHttpManager::Tick));
	}

	inline TSharedRef<IHttpRequest, ESPMode::ThreadSafe> FFakeHttpManager::CreateRequest()
	{
		++RxCreateRequest;
	...
	inline bool FFakeHttpManager::Tick(float Delta)
	{
		// Complete progressed.
	...
	inline bool FFakeHttpManager::OnProcessRequest(FFakeHttpRequest* FakeHttpRequest)
	{
