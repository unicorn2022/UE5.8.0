...
namespace BuildPatchServices
{
	...
	public:
		...
		bool IsVerifyOnly() const { return InstallerAction.IsVerifyOnly(); }
		...
		FString GetUniqueBuildId() const { return InstallManifest.IsValid() ? InstallManifest->GetUniqueBuildId() : FString(); }
	...
	private:
		// Hide tagged file accessors, they are only expected to be used by the manifest set, to prevent other systems from confusing
		// flat manifest filenames, and filenames including the action subdirectory.
		friend class FBuildPatchManifestSet;
	...
	{
	public:
		virtual ~IBuildManifestSet() = default;
		...
		virtual void GetReferencedChunks(TArray<FGuid>& DataGuids) const = 0;
		...
		virtual FGuid GetDataSecretId(const FGuid& DataGuid) const = 0;
		virtual FString GetChunkCloudSubdirectory(const FGuid& DataGuid) const = 0;
		virtual FString GetChunkFilename(const FGuid& DataGuid) const = 0;
		virtual FString GetChunkFilenameWithCloudSubdirectory(const FGuid& DataGuid) const = 0;
		virtual FString GetChunkUniqueBuildId(const FGuid& DataGuid) const = 0;
		virtual bool GetChunkFromDeltaStatus(const FGuid& DataGuid) const = 0;
		...
		// Gets files from the current manifest that are missing from disk.
		virtual void GetMissedFiles(const FString& InstallDirectory, TSet<FString>& OutMissedFiles) const = 0;
		...
		virtual bool IsExclusivelyRepairing() const = 0;
		virtual bool HasFileAttributes() const = 0;
		virtual bool IsExclusivelyVerifying() const = 0;

		// Custom install support
		virtual void GetPreReqInfo(TArray<FPreReqInfo>& PreReqInfo) const = 0;
		virtual void GetUninstallActionInfo(TArray<FUninstallActionInfo>& UninstallActionsInfo) const = 0;
	};
