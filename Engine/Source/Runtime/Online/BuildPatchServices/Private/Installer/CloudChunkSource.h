...
namespace BuildPatchServices
{
	...
	{
	public:
		virtual ~IConstructorCloudChunkSource() = default;
		...
		virtual void SetDownloadBytesPerSecondLimit(uint64 BytesPerSecondLimit) = 0;
	...
	{
	public:
		virtual ~ICloudChunkSource() = default;

		/**
		 * Update the download rate limit.
		 * @param InDownloadRateLimitBps    The new maximum download rate in bytes per second. Infinite can be specified as 0.
		 */
	...
	struct FCloudSourceConfig
	{
		...
		// Infinite can be specified as < 0.
		int32 MaxRetryCount = 6;
		// Number of retries to enable logging of http warnings.
		int32 RetriesToEnableVerbose;
		...
		FCloudSourceConfig(const TArray<FString>& InCloudRoots)
			: CloudRoots(InCloudRoots)
			, RetriesToEnableVerbose(MaxRetryCount - 1)
		{
			const float RetryFloats[] = {0.5f, 1.0f, 1.0f, 3.0f, 3.0f, 10.0f, 10.0f, 20.0f, 20.0f, 30.0f};
			RetryDelayTimes.Empty(UE_ARRAY_COUNT(RetryFloats));
			RetryDelayTimes.Append(RetryFloats, UE_ARRAY_COUNT(RetryFloats));
	...
	{
	public:
		virtual ~ICloudChunkSourceStat() = default;
		...
		/**
		 * Called once per second whenever there has been an active request.
		 * @param AverageRequestCount      The average number of active requests that there has currently been.
		 */
		virtual void OnActiveRequestCountAvgUpdated(int32 AverageRequestCount) = 0;
		...
		/**
		 * Called when a chunk is decrypted.
		 * @param DecryptionCycles  The amount of cycles spent on decryption a chunk.
		 */
		virtual void OnChunkDecrypted(uint64 DecryptionCycles) = 0;

		/**
		 * Called when new data has been received from the given cloud directory.
		 * @param CloudDirectory  The identifier for this cloud directory, e.g. base URL.
		 * @param ReceivedData    The number of bytes received from this cloud directory.
		 */
		virtual void OnCloudDirectoryDataReceived(const FString& CloudDirectory, uint64 ReceivedData) = 0;

		/**
		 * Called when a download from the given cloud directory has failed.
		 * @param CloudDirectory  The identifier for this cloud directory, e.g. base URL.
		 */
		virtual void OnCloudDirectoryError(const FString& CloudDirectory) = 0;
	};
}
