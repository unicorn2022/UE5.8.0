...
namespace BuildPatchServices
{
	...
	public:
		 ...
		 * @param OutRequestId          The request id for this download, which will match that provided to the delegates.
		 * @return if the request was queued successfully.
		 */
		virtual bool RequestFile(const FString& FileUri, const FDownloadCompleteDelegate& OnCompleteDelegate, const FDownloadProgressDelegate& OnProgressDelegate, int32* OutRequestId = nullptr) = 0;

		/** As per RequestFile, except adds the given headers to the request as a <Name, Value> pair. */
		virtual bool RequestFileWithHeaders(const FString& FileUri, const TMap<FString, FString>& RequestHeaders, const FDownloadCompleteDelegate& OnCompleteDelegate, const FDownloadProgressDelegate& OnProgressDelegate, int32* OutRequestId = nullptr) = 0;
	...
	public:
		 ...
		 */
		static IDownloadService* Create(TSharedPtr<IHttpManager> HttpManager, TSharedPtr<IFileSystem> FileSystem, TSharedPtr<IDownloadServiceStat> DownloadServiceStat, TSharedPtr<IInstallerAnalytics> InstallerAnalytics);
	};
	...
	{
	public:
		virtual ~IDownloadServiceStat() = default;
		...
		struct FDownloadRecord
		{
			...
			// If failed, the reason why the request failed
