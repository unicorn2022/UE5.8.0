...
namespace BuildPatchServices
{
	...
	public:
		...
		/**
		 * @return the avg number of download requests.
		 */
		virtual uint32 GetAvgRequestCount() const = 0;

		/**
		 *  @return the A/B Experiment if applicable
		 */
		virtual const TCHAR* GetExperimentVariant() const = 0;

		/**
		 * @return the total time in seconds spent on chunks decryption.
		 */
		virtual double GetTotalDecryptTime() const = 0;

		/**
		 * @return the stats collected for each cloud directory.
		 */
		virtual const TArray<FCloudDirectoryStats>& GetCloudDirectoryStats() const = 0;
	};
