...
namespace BuildPatchServices
{
	...
	{
	public:
		FInstallChunkSourceStatistics(ISpeedRecorder* SpeedRecorder, IInstallerAnalytics* InstallerAnalytics, IInstallActionStatistics* InInstallerActionStatistics, IFileOperationTracker* FileOperationTracker);
	...
	private:
		...
		IInstallActionStatistics* InstallerActionStatistics;
	...
	FInstallChunkSourceStatistics::FInstallChunkSourceStatistics(ISpeedRecorder* InSpeedRecorder, IInstallerAnalytics* InInstallerAnalytics, IInstallActionStatistics* InInstallerActionStatistics, IFileOperationTracker* InFileOperationTracker)
		: SpeedRecorder(InSpeedRecorder)
		, InstallerAnalytics(InInstallerAnalytics)
		, InstallerActionStatistics(InInstallerActionStatistics)
	...
	void FInstallChunkSourceStatistics::OnLoadComplete(const FGuid& ChunkId, const ELoadResult& Result, const ISpeedRecorder::FRecord& Record)
	{
		...
		else
		{
			...
			InstallerActionStatistics->OnChunkRecycleFailure(ChunkId);
	...
	IInstallChunkSourceStatistics* FInstallChunkSourceStatisticsFactory::Create(ISpeedRecorder* SpeedRecorder, IInstallerAnalytics* InstallerAnalytics, IInstallActionStatistics* InInstallerActionStatistics, IFileOperationTracker* FileOperationTracker)
	{
		check(SpeedRecorder != nullptr);
		check(InstallerAnalytics != nullptr);
		check(FileOperationTracker != nullptr);
		return new FInstallChunkSourceStatistics(SpeedRecorder, InstallerAnalytics, InInstallerActionStatistics, FileOperationTracker);
	}
};