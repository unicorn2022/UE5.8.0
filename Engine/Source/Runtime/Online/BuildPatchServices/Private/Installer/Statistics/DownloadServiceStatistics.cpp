...
namespace BuildPatchServices
{
	...
	{
	public:
		FDownloadServiceStatistics(const TSharedPtr<ISpeedRecorder>& SpeedRecorder, const TSharedPtr<IDataSizeProvider>& DataSizeProvider, const TSharedPtr<IInstallerAnalytics>& InstallerAnalytics);
	...
	private:
		TWeakPtr<ISpeedRecorder> SpeedRecorder; // AddRecord is safe, but SpeedRecorder is ticked
		TWeakPtr<IDataSizeProvider> DataSizeProvider;
		TWeakPtr<IInstallerAnalytics> InstallerAnalytics;
	...
	FDownloadServiceStatistics::FDownloadServiceStatistics(const TSharedPtr<ISpeedRecorder>& InSpeedRecorder, const TSharedPtr<IDataSizeProvider>& InDataSizeProvider, const TSharedPtr<IInstallerAnalytics>& InInstallerAnalytics)
	...
	void FDownloadServiceStatistics::OnDownloadComplete(const FDownloadRecord& DownloadRecord)
	{
		...
		if (DownloadRecord.bSuccess && EHttpResponseCodes::IsOk(DownloadRecord.ResponseCode))
		{
			...
			if (const TSharedPtr<ISpeedRecorder> SpeedRecorderPtr = SpeedRecorder.Pin())
			{
				SpeedRecorderPtr->AddRecord(DownloadRecord.SpeedRecord);
			}
		...
		else if (DownloadRecord.FailureReason != EHttpFailureReason::Cancelled)
		{
			// Ignore deltas as they might not exist. OptimisedDelta enforces the DeltaPolicy and handles errors if deltas are required
			FString UriFileExtension = FPaths::GetExtension(DownloadRecord.Uri);
			if (UriFileExtension.Equals(TEXT("delta"), ESearchCase::IgnoreCase) && (DownloadRecord.ResponseCode == EHttpResponseCodes::Forbidden || DownloadRecord.ResponseCode == EHttpResponseCodes::NotFound))
			{
				// Parse installed/installing version strings for analytics
				FString PatchVersion = FPaths::GetBaseFilename(DownloadRecord.Uri);
				FString BaseVersion = FPaths::GetCleanFilename(FPaths::GetPath(DownloadRecord.Uri));

				if (const TSharedPtr<IInstallerAnalytics> InstallerAnalyticsPtr = InstallerAnalytics.Pin())
				{
					InstallerAnalyticsPtr->RecordMissingDelta(DownloadRecord.Uri, BaseVersion, PatchVersion);
				}
				return;
			}
			...
			if (const TSharedPtr<IInstallerAnalytics> InstallerAnalyticsPtr = InstallerAnalytics.Pin())
			{
				InstallerAnalyticsPtr->RecordChunkDownloadError(DownloadRecord.Uri, DownloadRecord.ResponseCode, TEXT("DownloadFail"));
			}
		}
	}
	...
	TArray<FDownload> FDownloadServiceStatistics::GetCurrentDownloads() const
	{
		...
		if (const TSharedPtr<IDataSizeProvider> DataSizeProviderPtr = DataSizeProvider.Pin())
		{
			DataSizeProviderPtr->GetDownloadSize(DownloadData, DownloadSizes);
	...
	IDownloadServiceStatistics* FDownloadServiceStatisticsFactory::Create(const TSharedPtr<ISpeedRecorder>& SpeedRecorder, const TSharedPtr<IDataSizeProvider>& DataSizeProvider, const TSharedPtr<IInstallerAnalytics>& InstallerAnalytics)
	{
