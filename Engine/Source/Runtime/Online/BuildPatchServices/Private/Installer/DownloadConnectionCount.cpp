...
namespace BuildPatchServices
{
	...
	public:
		...
		virtual ~FDownloadConnectionCount();
		virtual uint32 GetAdjustedCount(uint32 NumProcessing, EBuildPatchDownloadHealth CurrentHealth);
		void SetConnectionScaling(bool bEnableScaling) override;
	...
	private:
		...
		TAtomic<bool> bConnectionScalingEnabled;
		bool bPreviousConnectionScalingEnabledStatus;
	};
	...
	FDownloadConnectionCount::FDownloadConnectionCount(FDownloadConnectionCountConfig InConfiguration, IDownloadServiceStatistics* InDownloadStatistics)
		...
		, HighBandwidthLevel(0.0)
		, LowBandwidthLevel(0.0)
		, AveragesSeen(0U)
		, bConnectionScalingEnabled(!Configuration.bDisableConnectionScaling)
		, bPreviousConnectionScalingEnabledStatus(!Configuration.bDisableConnectionScaling)
	{
	}

	void FDownloadConnectionCount::Reset()
	{
		ConsecutiveDecreases = 0U;
		RequestAllowance = Configuration.FallbackCount;
		ConsecutiveIncreases = 0U;
		...
		// Calling with 0 resets the internal values.
		DownloadStatistics->GetImmediateAverageSpeedPerRequest(0);
	}
	...
	{
		if (bConnectionScalingEnabled == false)
		{
			bPreviousConnectionScalingEnabledStatus = false;
			return Configuration.FallbackCount;
		}
		else if (!bPreviousConnectionScalingEnabledStatus)
		{
			Reset();
		}
		bPreviousConnectionScalingEnabledStatus = true;
		...
		if (bLog)
		{
			UE_LOGF(LogDownloadConnectionCount, Verbose, "New Request Count = %d", GetAllowance());
		}

		return GetAllowance();
	}

	void FDownloadConnectionCount::SetConnectionScaling(bool bEnableScaling)
	{
		bConnectionScalingEnabled = bEnableScaling;
	}
