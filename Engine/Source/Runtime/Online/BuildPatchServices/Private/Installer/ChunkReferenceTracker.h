...
namespace BuildPatchServices
{
	...
	public:
		virtual ~IChunkReferenceTracker() = default;
		 ...
		 * Note that this does NOT return chunks in usage order anymore! If you were tacitly relying
		 * on the enumeration order of an unordered TSet, this likely broke! Use CopyOutOrderedUseList.
	...
	namespace CustomChunkReferencesHelpers
	{
		...
		FORCEINLINE TArray<FGuid> OrderedUniquePatchReferencesTagged(const FBuildPatchAppManifestRef& InstallManifest, const TSet<FString>& InstallTagSet, const FBuildPatchAppManifestRef& CurrentManifest, const TSet<FString>& CurrentTagSet)
		{
			...
			{
				const FFileManifest* const OldFileManifest = CurrentManifest->GetFileManifest(TaggedFile);
				const FFileManifest* const NewFileManifest = InstallManifest->GetFileManifest(TaggedFile);
				if (!OldFileManifest || OldFileManifest->SHA1Hash != NewFileManifest->SHA1Hash || !FilesInstalled.Contains(TaggedFile))
				{
					for (const FChunkPart& ChunkPart : NewFileManifest->ChunkParts)
					{
