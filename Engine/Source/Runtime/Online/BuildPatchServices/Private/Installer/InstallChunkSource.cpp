...
DECLARE_LOG_CATEGORY_EXTERN(LogInstallChunkSource, Log, All);
...
namespace BuildPatchServices
{
	...
	public:
		FInstallChunkSource(IFileSystem* FileSystem, IInstallChunkSourceStat* InInstallChunkSourceStat, const TMultiMap<FString, FBuildPatchAppManifestRef>& InInstallationSources, 
			const TSet<FGuid>& ChunksThatWillBeNeeded, bool bInAllowLoadingAnyChunk, const TMap<FString, FString>& PerFileSubdirectories);
		~FInstallChunkSource();
			...
			UE::TUniqueLock _(OpenedFileHandlesLock);
			{
				...
				if (File)
				{
					if ((*File)->FileHandleLock.TryLock() == false)
					{
						...
						UE_LOGF(LogInstallChunkSource, Error, "Critical error: trying to delete %ls while read lock is held.", *FilePath);
		...
		virtual void GetChunksForFile(const FString& InstallationDirectory, const FString& FilenameInManifest, TSet<FGuid>& OutChunks) const override;
	...
	private:
		...
		// This is keyed off the resolved file name (including subdirectory mapping)
		...
		// When we try and recreate chunks from an existing installation, there are chunks that don't get enumerated
		// as producible. Additionally, the chunks might not fill the full window. Setting this allows any request for
		// a chunk where we have any parts to succeed and be zero filled for any missing parts.
		bool bAllowLoadingAnyChunk = false;

		// Contains files that have a custom subdirectory to install into. This is accessed from multiple threads
		// but should be static across execution and so requires no lock.
	...
	FInstallChunkSource::FInstallChunkSource(IFileSystem* InFileSystem, IInstallChunkSourceStat* InInstallChunkSourceStat, 
		...
		bool bInAllowLoadingAnyChunk, const TMap<FString, FString>& InPerFileSubdirectories)
		: FileSystem(InFileSystem)
		, InstallChunkSourceStat(InInstallChunkSourceStat)
		, bAllowLoadingAnyChunk(bInAllowLoadingAnyChunk)
		, PerFileSubdirectories(InPerFileSubdirectories)
	{
		...
		for (const TPair<FString, FBuildPatchAppManifestRef>& InstallationSourcePair : InInstallationSources)
		{
			...
			for (const FString& InstalledFileName : InstalledFiles)
			{
				...
				{
					// Should never happen?
					UE_LOGF(LogInstallChunkSource, Error, "Missing file manifest from file provided from manifest? %ls", *InstalledFileName);
					continue;
				}

				bool bExistenceCheckPassed = false;

				int32 ChunksForFile = 0;
				...
				for (int32 ChunkPartIndex = 0; ChunkPartIndex < InstalledFileManifest->ChunkParts.Num(); ChunkPartIndex++, FileOffset += ChunkPartSize)
				{
					...
					{
						const FString DiskFileName = FBuildPatchUtils::ResolveInstallationFileName(InstallationDirectory, PerFileSubdirectories, InstalledFileName);

						int64 SizeOnDisk = 0;
						bool bGotFileSizeSuccessfully = InFileSystem->GetFileSize(*DiskFileName, SizeOnDisk);
					...
					if (ChunkAssemblyInfo.SourceDetails.Num() == 0)
					{
						...
						if (!ChunkInfoPtr)
						{
							UE_LOGF(LogInstallChunkSource, Display, "Missing chunk info for guid: %ls", *LexToString(ChunkPart.Guid));
							continue;
						}

						if (ChunkPart.Offset == 0 &&
							ChunkInfoPtr->DataSizeUncompressed == ChunkPart.Size)
						{
					...
					if (PartialStructure.GetHead() == PartialStructure.GetTail() &&
						PartialStructure.GetHead()->GetSize() == ChunkInfoPtr->DataSizeUncompressed)
					{
				...
				UE_CLOGF(ChunksForFile, LogInstallChunkSource, Verbose, "%ls produced %d chunks", *InstalledFileName, ChunksForFile);
			}
			...
			for (TPair<FGuid, FChunkAssemblyInfo>& ChunkAssemblyPair : ChunkAssemblyInfos)
			{
				...
				{
					IncompleteChunks++;
					// Skip incomplete chunks unless we allow loading incomplete chunks
					if (!bAllowLoadingAnyChunk)
					{
						continue;
					}
				}
			...
			UE_LOGF(LogInstallChunkSource, Verbose, "Got %u complete chunks, %u incomplete chunks, %u repeat chunks", CompleteChunks, IncompleteChunks, ChunksInAnotherSource);
			if (CompleteChunks)
			{
		...
		UE_LOGF(LogInstallChunkSource, Log, "Useful Sources:%d/%d. Available Chunks:%d/%d.", InstallationSources.Num(), InInstallationSources.Num(), AvailableInBuilds.Num(), ChunksThatWillBeNeeded.Num());
	}
	...
	void FInstallChunkSource::GetChunksForFile(const FString& InstallationDirectory, const FString& FileInManifest, TSet<FGuid>& OutChunks) const
	{
		const FFileManifest* FileManifest = nullptr;
		FString NormalisedInstallDirectory = InstallationDirectory;
		FPaths::NormalizeDirectoryName(NormalisedInstallDirectory);
		for (const TPair<FString, FBuildPatchAppManifestRef>& Pair : InstallationSources)
		{
			FString NormaliasedInstallSourcePath = Pair.Key;
			FPaths::NormalizeDirectoryName(NormaliasedInstallSourcePath);
			if (FPaths::IsSamePath(NormalisedInstallDirectory, NormaliasedInstallSourcePath))
			{
				FileManifest = Pair.Value->GetFileManifest(FileInManifest);
				break;
			}
		}
	...
	IConstructorChunkSource::FRequestProcessFn FInstallChunkSource::CreateRequest(const FGuid& DataId, FMutableMemoryView DestinationBuffer, void* UserPtr, FChunkRequestCompleteDelegate CompleteFn)
	{
		...
		return [this, FoundInstallManifest, FoundInstallDirectory = *FoundInstallDirectory, DataId, DestinationBuffer, UserPtr, CompleteFn](bool bIsAborted)
		{
			...
			if (!ChunkInfoPtr || !ChunkSource)
			{
				if (!ChunkSource)
				{
					UE_LOGF(LogInstallChunkSource, Error, "Chunk %ls not found in sources.", *DataId.ToString());
				}
				else
				{
					UE_LOGF(LogInstallChunkSource, Error, "Chunk info for %ls not found in install manifest.", *DataId.ToString());
				}
			...
			for (int32 FileChunkPartsIdx = 0; FileChunkPartsIdx < ChunkSource->Num(); ++FileChunkPartsIdx)
			{
				if (bLoadedWholeChunk)
				{
					...
					//UE_LOGF(LogInstallChunkSource, Display, "Chunk %ls had more chunk sources than necessary to re-assemble", *WriteToString<40>(DataId));
					break;
				}
				...
				if (ChunkEndLocation > DestinationBuffer.GetSize())
				{
					...
					UE_LOGF(LogInstallChunkSource, Error, "Chunk %ls assembled larger than the actual chunk size (chunk wanted end %u vs buffer size %llu", *WriteToString<40>(DataId), ChunkEndLocation, DestinationBuffer.GetSize());
				...
				{
					const FString DiskFileName = FBuildPatchUtils::ResolveInstallationFileName(FoundInstallDirectory, PerFileSubdirectories, FileChunkPart.InstalledFileManifest->Filename);
					OpenedFileHandlesLock.Lock();
					TUniquePtr<FOpenedFile>* OpenedFilePtr = OpenedFileHandles.Find(DiskFileName);
					if (OpenedFilePtr == nullptr)
					{
						TRACE_CPUPROFILER_EVENT_SCOPE(Install_OpenSource);
						TUniquePtr<FArchive> NewReader = FileSystem->CreateFileReader(*DiskFileName);
						if (NewReader.IsValid())
						{
							TUniquePtr<FOpenedFile> NewOpenedFile = MakeUnique<FOpenedFile>(MoveTemp(NewReader));

							OpenedFileHandles.Add(DiskFileName, MoveTemp(NewOpenedFile));
							OpenedFilePtr = OpenedFileHandles.Find(DiskFileName);
						}
					}
					if (OpenedFilePtr)
					{
				...
				{
					UE_LOGF(LogInstallChunkSource, Error, "Failed to open: %ls on chunk %ls", *FileChunkPart.InstalledFileManifest->Filename, *LexToString(DataId));
...

				{
					...
					if (NewChunk.Intersect(ChunkBlocks).GetHead() != nullptr)
					{
						...
						UE_LOGF(LogInstallChunkSource, Error, "Chunk %ls had overlapping chunk parts", *WriteToString<40>(DataId));
						Result = IInstallChunkSourceStat::ELoadResult::InvalidChunkParts;
						break;
					}
				...
				bLoadedWholeChunk = ChunkBlocks.GetHead() && ChunkBlocks.GetHead() == ChunkBlocks.GetTail() && ChunkBlocks.GetHead()->GetSize() == ChunkInfoPtr->DataSizeUncompressed;
			}

			// If we didn't have enough parts but we're allowing loading partial chunks, succeed the chunk
			// and zero fill what we didn't have.
			if (!bLoadedWholeChunk && 
				Result == IInstallChunkSourceStat::ELoadResult::Success &&
				bAllowLoadingAnyChunk)
			{
				FBlockStructure Remnants;
				Remnants.Add(0, ChunkInfoPtr->DataSizeUncompressed);
				Remnants.Remove(ChunkBlocks);

				const FBlockEntry* Entry = Remnants.GetHead();
				while (Entry)
				{
					FMemory::Memset((uint8*)DestinationBuffer.GetData() + Entry->GetOffset(), 0, Entry->GetSize());
					Entry = Entry->GetNext();
				}
				bLoadedWholeChunk = true;
			}
	...
	IConstructorInstallChunkSource* IConstructorInstallChunkSource::CreateInstallSource(IFileSystem* FileSystem, IInstallChunkSourceStat* InstallChunkSourceStat, 
		const TMultiMap<FString, FBuildPatchAppManifestRef>& InstallationSources, const TSet<FGuid>& ChunksThatWillBeNeeded, bool bAllowLoadingAnyChunk, const TMap<FString, FString>& PerFileSubdirectories)
	{
		return new FInstallChunkSource(FileSystem, InstallChunkSourceStat, InstallationSources, ChunksThatWillBeNeeded, bAllowLoadingAnyChunk, PerFileSubdirectories);
	}
