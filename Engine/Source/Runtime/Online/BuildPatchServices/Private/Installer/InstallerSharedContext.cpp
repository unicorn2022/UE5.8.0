...
namespace BuildPatchServices
{
	...
	IBuildInstallerThread* FBuildInstallerSharedContext::CreateThreadInternal()
	{
		...
		UE_LOGF(LogBuildPatchServices, Display, "Creating thread %ls #%d", DebugName, ThreadCount);
	...
	FBuildInstallerSharedContext::~FBuildInstallerSharedContext()
	{
		...
		{
			UE_LOGF(LogBuildPatchServices, Error, "Threads still allocated: Expected %d, Actual %u", ThreadFreeList.Num(), ThreadCount);
	...
	IBuildInstallerThread* FBuildInstallerSharedContext::CreateThread()
	{
		...
		if (!Thread)
		{
			UE_CLOGF(bWarnOnCreateThread, LogBuildPatchServices, Warning, "Allocating installer thread, free list exhausted, check PreallocateResources()");
			FScopeLock Lock(&ThreadFreeListCS);
