...
namespace BuildPatchServices
{
	...
	/**
	 * A static helper function to parallelise retrieving file sizes for performance on large sets of files.
	 * @param FilesToCheck    The set of files to lookup the filesize for
	 * @param RootDirectory   The root directory of the files. Empty string can be passed if FilesToCheck has full file paths.
	 * @return a map of string from FilesToCheck to size on disk. INDEX_NONE if the file does not exist.
	 */
	...
	public:
		...
		/**
		 * Delete a directory path including all containing files and folders.
		 * @param DirectoryPath     The directory path to delete.
		 * @return true if successful.
		 */
		virtual bool DeleteDirectoryRecursively(const TCHAR* DirectoryPath) const = 0;
		...
		/**
		 * Sets the size of a file, can increase or decrease, and will also create a file that does not exist..
		 * @param Filename          The filename for the request.
		 * @param FileSize          The desired size in bytes.
		 * @return true if successful.
		 */
		virtual bool SetFileSize(const TCHAR* Filename, int64 FileSize) const = 0;
		 ...
		 * @param Filename          The filename for the request.
		 * @param ReadFlags         The file open flags.
		 * @param OutHandle			Pointer to the file handler used by the archive. The returned FArchive owns the instance of OutHandle and controls its lifetime.
		 * @return unique pointer to the created archive, invalid if failed to open the file.
		 */
		virtual TUniquePtr<FArchive> CreateFileReaderEx(const TCHAR* Filename, EReadFlags ReadFlags, IFileHandle** OutHandle) const = 0;
		...
		/**
		 * Load a file on disk into a byte array.
		 * @param Filename          The filename for the request.
		 * @param Contents          The contents of the file if successful.
		 * @return true if successful.
		 */
		virtual bool LoadFileToArray(const TCHAR* Filename, TArray<uint8>& Contents) const = 0;
		 ...
		 * @return true if successful.
		 */
		virtual bool SaveArrayToFile(const TCHAR* Filename, const TArray<uint8>& Contents) const = 0;
		 ...
		 * @param bDoNotRetryOrError Do not try to repeat failed move operation. 
		 * @return true if the file was moved/renamed successfully.
		 */
		virtual bool MoveFile(const TCHAR* FileDest, const TCHAR* FileSource, bool bDoNotRetryOrError = true) const = 0;
		 ...
		 * @param FileExtension     The extension to filter by, in the form of TEXT(".ext"). If null or empty string, all files are found.
		 * @param bFollowSymlinks   Whether symlinks to directories should be followed and enumerated, or added as a file.
		 */
		virtual void FindFilesRecursively(TArray<FString>& FoundFiles, const TCHAR* Directory, const TCHAR* FileExtension = nullptr, bool bFollowSymlinks = true) const = 0;
		 ...
		 * @param bFollowSymlinks   Whether symlinks to directories should be followed and enumerated, or added as a file.
		 */
		virtual void ParallelFindFilesRecursively(TArray<FString>& FoundFiles, const TCHAR* Directory, const TCHAR* FileExtension = nullptr, EAsyncExecution AsyncExecution = EAsyncExecution::ThreadPool, bool bFollowSymlinks = true) const = 0;
		 ...
		 * @param Filename          The filename or directory for the request.
		 * @return resulted stat data of the requested file or dir.
		 */
		virtual FFileStatData GetStatData(const TCHAR* FilenameOrDirectory) const = 0;
