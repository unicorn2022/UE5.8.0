...
namespace BuildPatchServices
{
	...
	public:
		...
		virtual double GetAverageSpeed(float Seconds, bool bIncludeInactiveTime) const;
	...
	double FSpeedRecorder::GetAverageSpeed(float Seconds, bool bIncludeInactiveTime) const
	{
		checkSlow(IsInGameThread());
		...
		double SecondsDivisor = bIncludeInactiveTime ? Seconds : FStatsCollector::CyclesToSeconds(TotalCycles);
		return TotalCycles > 0 ? TotalSize / SecondsDivisor : 0.0;
	}
