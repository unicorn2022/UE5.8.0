...
{
	TMap<FString, int64> ParallelGetFileSizes(const TSet<FString>& FilesToCheck, const FString& RootDirectory)
	{
		TArray<TTuple<FString, int64>> TargetFiles;
		TargetFiles.Reserve(FilesToCheck.Num());
		for (const FString& File : FilesToCheck)
		{
			TargetFiles.Add(TTuple<FString, int64>(File, INDEX_NONE));
		}
		auto GetFileSizesTasks = CreateTaskSchedulerLimitedByCoresNumber<int64>(EAsyncExecution::TaskGraph);
		for (TTuple<FString, int64>& FileInfo : TargetFiles)
		{
			GetFileSizesTasks.ScheduleTask(
				[&RootDirectory, &FileInfo]() { return IFileManager::Get().FileSize(*(RootDirectory / FileInfo.Get<0>())); },
				[&FileInfo](int64 FileSize) { FileInfo.Get<1>() = FileSize; }
			);
		}
		GetFileSizesTasks.WaitAll();
		TMap<FString, int64> Results;
		for (TTuple<FString, int64>& FileInfo : TargetFiles)
		{
			Results.Add(FileInfo.Get<0>(), FileInfo.Get<1>());
		}
		return Results;
	}


	class FDirectoryEnumeratorBase : public IPlatformFile::FDirectoryVisitor
	{
	protected:
		FDirectoryEnumeratorBase(const TCHAR* InFileExtension, bool bInFollowSymlinks)
			: bFollowSymlinks(bInFollowSymlinks)
		...
		bool IsSymlink(const TCHAR* Filename)
		{
			...
			if (!bFollowSymlinks)
			{
				...
				if (lstat(TCHAR_TO_UTF8(Filename), &StatBuf) != -1)
				{
					bIsSymlink = S_ISLNK(StatBuf.st_mode);
				}
#endif
			}
			return bIsSymlink;
	...
	class FDirectoryEnumerator : public FDirectoryEnumeratorBase
	{
	public:
		FDirectoryEnumerator(IPlatformFile& InPlatformFile, const TCHAR* InFileExtension, bool bInFollowSymlinks)
			: FDirectoryEnumeratorBase(InFileExtension, bInFollowSymlinks)
			, PlatformFile(InPlatformFile)
		{  }

		virtual bool Visit(const TCHAR* FilenameOrDirectory, bool bIsDirectory) override
		{
			const bool bIsSymlink = IsSymlink(FilenameOrDirectory);
			if (bIsDirectory && !bIsSymlink)
			{
				FString Directory = FilenameOrDirectory;
				PlatformFile.IterateDirectory(*Directory, *this);
			}
	...
	class FParallelDirectoryEnumerator : public FDirectoryEnumeratorBase
	{
	public:
		FParallelDirectoryEnumerator(IPlatformFile& InPlatformFile, const TCHAR* InFileExtension, EAsyncExecution InAsyncExecution, bool bInFollowSymlinks)
			: FDirectoryEnumeratorBase(InFileExtension, bInFollowSymlinks)
			, PlatformFile(InPlatformFile)
		...
		void GetFiles(TArray<FString>& Results)
		{
			...
			Results.Pop(EAllowShrinking::Yes);
			Results.Sort();
		}
	...
	public:
		...
		virtual bool SetFileSize(const TCHAR* Filename, int64 FileSize) const;
		...
		virtual void FindFilesRecursively(TArray<FString>& FoundFiles, const TCHAR* Directory, const TCHAR* FileExtension = nullptr, bool bFollowSymlinks = true) const override;
		...
		virtual FFileStatData GetStatData(const TCHAR* Filename) const override;
	...
	bool FFileSystem::DeleteDirectoryRecursively(const TCHAR* DirectoryPath) const
	{
	...
	bool FFileSystem::SetFileSize(const TCHAR* Filename, int64 FileSize) const
	{
		TUniquePtr<IFileHandle> Handle{ PlatformFile.OpenWrite(Filename, true, true) };
		if (Handle.IsValid())
		{
			return Handle->Truncate(FileSize);
		}
		return false;
	}
	...
	TUniquePtr<FArchive> FFileSystem::CreateFileReaderEx(const TCHAR* Filename, EReadFlags ReadFlags, IFileHandle** OutHandle) const
	{
		IFileHandle* Handle = FPlatformFileManager::Get().GetPlatformFile().OpenWrite(Filename, true, true);
		if (OutHandle)
		{
			*OutHandle = Handle;
		}
		...
		if (!Handle)
		{
			if (static_cast<uint32>(ReadFlags) & FILEREAD_NoFail)
			{
				GLog->Logf(ELogVerbosity::Type::Fatal, TEXT("CreateFileReaderEx: Failed to read file : %s"), Filename);
			}
			return nullptr;
		}
		Handle->Seek(0); // Manually move the position to the beginning because Append moved it to EOF.
		return MakeUnique<FArchiveFileReaderGeneric>(Handle, Filename, Handle->Size(), PLATFORM_FILE_READER_BUFFER_SIZE, static_cast<uint32>(ReadFlags));
	}
	...
	bool FFileSystem::LoadFileToArray(const TCHAR* Filename, TArray<uint8>& Contents) const
	{
		return FFileHelper::LoadFileToArray(Contents, Filename);
	}

	bool FFileSystem::SaveArrayToFile(const TCHAR* Filename, const TArray<uint8>& Contents) const
	{
		return FFileHelper::SaveArrayToFile(Contents, Filename);
	}
	...
	bool FFileSystem::MoveFile(const TCHAR* FileDest, const TCHAR* FileSource, bool bDoNotRetryOrError) const
	{
		return FileManager.Move(FileDest, FileSource, true, true, true, bDoNotRetryOrError);
	}
	...
	void FFileSystem::FindFilesRecursively(TArray<FString>& FoundFiles, const TCHAR* Directory, const TCHAR* FileExtension, bool bFollowSymlinks) const
	{
		FDirectoryEnumerator DirectoryEnumerator(PlatformFile, FileExtension, bFollowSymlinks);
		PlatformFile.IterateDirectory(Directory, DirectoryEnumerator);
		DirectoryEnumerator.GetFiles(FoundFiles);
	}

	void FFileSystem::ParallelFindFilesRecursively(TArray<FString>& FoundFiles, const TCHAR* Directory, const TCHAR* FileExtension, EAsyncExecution AsyncExecution, bool bFollowSymlinks) const
	{
		FParallelDirectoryEnumerator DirectoryEnumerator(PlatformFile, FileExtension, AsyncExecution, bFollowSymlinks);
		PlatformFile.IterateDirectory(Directory, DirectoryEnumerator);
		DirectoryEnumerator.GetFiles(FoundFiles);
	}

	FFileStatData FFileSystem::GetStatData(const TCHAR* FilenameOrDirectory) const
	{
