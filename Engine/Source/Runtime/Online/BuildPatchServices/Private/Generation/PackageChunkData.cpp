...
namespace BuildPatchServices
{
	namespace ChunkDbHelpers
	{
		...
		uint32 GetPerChunkEntryHeaderSize()
		{
			static const uint32 PerEntryHeaderSize = []()
			{
				...
				HeaderWriter << ChunkDbHeader;
				return HeaderData.Num() - GetChunkDbHeaderSize();
			}();
			return PerEntryHeaderSize;
		}
	}
	...
	private:
		...
		bool TagsAreValid(const FBuildPatchAppManifestPtr& ManifestPtr, const TArray<TSet<FString>>& ProvidedTagSetArray, const FString& BuildType);
		bool TagsAreValid(const FBuildPatchAppManifestPtr& ManifestPtr, const TSet<FString>& ProvidedTagSetArray, const FString& BuildType);
	...
	private:
		...
		TSharedPtr<IOptimisedDelta> OptimisedDelta;
		...
		IBuildInstallerSharedContextPtr CloudSourceSharedContext;
	...
	bool FPackageChunks::Run()
	{
		...
		bShouldRun = bShouldRun && DownloadService->RequestFile(Configuration.ManifestFilePath, DownloadCompleteDelegate, DownloadProgressDelegate, &RequestIdManifestFile);
		bShouldRun = bShouldRun && DownloadService->RequestFile(Configuration.PrevManifestFilePath, DownloadCompleteDelegate, DownloadProgressDelegate, &RequestIdPrevManifestFile);
		if (!bShouldRun)
		{
			bSuccess = false;
		}
	...
	void FPackageChunks::HandleManifestComplete()
	{
		...
		if (bBothManifestsReady && !bManifestsProcessed)
		{
			...
			{
				UE_LOGF(LogPackageChunkData, Error, "Could not download ManifestFilePath from %ls.", *Configuration.ManifestFilePath);
				bSuccess = false;
				bShouldRun = false;
			}
			...
			{
				UE_LOGF(LogPackageChunkData, Error, "Could not download PrevManifestFilePath from %ls.", *Configuration.PrevManifestFilePath);
				bSuccess = false;
				bShouldRun = false;
			}

			if ((Manifest.IsValid() && !Configuration.bIgnoreManifestFileInvalidTags && !TagsAreValid(Manifest, Configuration.TagSetArray, TEXT("build")))
				|| (PrevManifest.IsValid() && !Configuration.bIgnorePrevManifestFileInvalidTags && !TagsAreValid(PrevManifest, Configuration.PrevTagSet, TEXT("previous build"))))
			{
				bSuccess = false;
				bShouldRun = false;
			}
			...
			if (bSuccess)
			{
				...
				TSharedRef<IUriProvider, ESPMode::ThreadSafe> UriProvider = FUriProviderFactory::Create(MessagePump.Get(), { FPaths::GetPath(Configuration.ManifestFilePath) });
				...
				OptimisedDelta = FOptimisedDeltaFactory::Create(OptimisedDeltaConfiguration, MoveTemp(OptimisedDeltaDependencies));
			}
		}
	}

	bool FPackageChunks::TagsAreValid(const FBuildPatchAppManifestPtr& ManifestPtr, const TArray<TSet<FString>>& ProvidedTagSetArray, const FString& BuildType)
	{
		TSet<FString> ProvidedTags;
		for (const TSet<FString>& ProvidedTagSet : ProvidedTagSetArray)
		{
			ProvidedTags.Append(ProvidedTagSet);
		}
		return TagsAreValid(ManifestPtr, ProvidedTags, BuildType);
	}

	bool FPackageChunks::TagsAreValid(const FBuildPatchAppManifestPtr& ManifestPtr, const TSet<FString>& ProvidedTagSet, const FString& BuildType)
	{
		bool bTagsAreValid = true;
		TArray<FString> UnknownManifestTags;

		TSet<FString> ValidTagSet;
		ManifestPtr->GetFileTagList(ValidTagSet);
		for (const FString& ProvidedTag : ProvidedTagSet)
		{
			if (!ValidTagSet.Contains(ProvidedTag))
			{
				UnknownManifestTags.Add(ProvidedTag);
				bTagsAreValid = false;
			}
		}
		...
		if (!bTagsAreValid)
		{
			UE_LOGF(LogPackageChunkData, Error, "The provided tags were invalid for the %ls: %ls", *BuildType, *FString::Join(UnknownManifestTags, TEXT(",")));
			UE_LOGF(LogPackageChunkData, Error, "Valid tags for the %ls are: %ls", *BuildType, *FString::Join(ValidTagSet.Array(), TEXT(",")));
		}

		return bTagsAreValid;
	}
	...
	void FPackageChunks::BeginPackageProcess()
	{
		...
		TArray<FGuid> FullDataSetArray;
		ChunkReferenceTracker->CopyOutOrderedUseList(FullDataSetArray);
		TSet<FGuid> FullDataSet(MoveTemp(FullDataSetArray));
		if (FullDataSet.Num() > 0)
		{
			...
			CloudSourceSharedContext = FBuildInstallerSharedContextFactory::Create(TEXT("PackageChunks"));
			...
			CloudChunkSourceStatistics.Reset(FCloudChunkSourceStatisticsFactory::Create(
				...
				&BuildProgress,
				FileOperationTracker.Get()));
			FileConstructorStatistics.Reset(FFileConstructorStatisticsFactory::Create(
				DiskReadSpeedRecorder.Get(),
				DiskWriteSpeedRecorder.Get(),
				&BuildProgress,
				FileOperationTracker.Get()));
			ChunkDataSerialization.Reset(FChunkDataSerializationFactory::Create(
				FileSystem.Get(), Crypto.Get(), ChunkDataSerializationConfig));
		...
		{
			UE_LOGF(LogPackageChunkData, Log, "No chunks were necessary for the provided manifest(s) and tagset(s). No chunkdb files were created.");
	...
	void FPackageChunks::OnPackageComplete(bool bInSuccess)
	{
		...
		{
			UE_LOGF(LogPackageChunkData, Error, "%ls: %ls", *InstallerError->GetErrorCode(), *InstallerError->GetErrorText().BuildSourceString());
			bSuccess = false;
		}
		else
		{
			UE_LOGF(LogPackageChunkData, Log, "Downloaded %ls at %ls/sec.", *FText::AsMemory(DownloadServiceStatistics->GetBytesDownloaded(), EMemoryUnitStandard::IEC).ToString(), *FText::AsMemory(DownloadSpeedRecorder->GetAverageSpeed(TNumericLimits<float>::Max()), EMemoryUnitStandard::IEC).ToString());
		}
		...
		if (bSuccess && Configuration.ResultDataFilePath.IsEmpty() == false)
		{
			...
			if (!bSuccess)
			{
				UE_LOGF(LogPackageChunkData, Error, "Could not save output to %ls", *Configuration.ResultDataFilePath);
			}
		}
