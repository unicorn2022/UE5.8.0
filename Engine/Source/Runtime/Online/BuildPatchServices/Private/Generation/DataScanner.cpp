...
DEFINE_LOG_CATEGORY(LogDataScanner);

namespace BuildPatchServices
{
	bool ReadForceSkipMatches()
	{
		FString ForceSkipMatches;
		if (!FParse::Value(FCommandLine::Get(), TEXT("ForceSkipMatches="), ForceSkipMatches))
		{
			GConfig->GetString(TEXT("Portal.BuildPatch"), TEXT("ForceSkipMatches"), ForceSkipMatches, GEngineIni);
		}
		return ForceSkipMatches == TEXT("true");
	}

	bool GetForceSkipMatches()
	{
		static bool bForceSkipMatches = ReadForceSkipMatches();
		return bForceSkipMatches;
	}

	uint32 ReadMaxCollisionCountPerMB()
	{
		uint32 CommandlineValue = TNumericLimits<uint32>::Max();
		if (!FParse::Value(FCommandLine::Get(), TEXT("MaxCollisionCountPerMB="), CommandlineValue))
		{
			int32 ConfigValue = 0;
			if (GConfig->GetInt(TEXT("Portal.BuildPatch"), TEXT("MaxCollisionCountPerMB"), ConfigValue, GEngineIni))
			{
				return static_cast<uint32>(ConfigValue);
			}
		}
		return CommandlineValue;
	}

	uint32 GetCollisionCountLimit(uint64 DataSize)
	{
		static uint64 MaxCollisionCountPerMB = ReadMaxCollisionCountPerMB();
		const uint64 NumMBsInData = DataSize / 1000000;
		const uint64 CollisionCountLimit = NumMBsInData * MaxCollisionCountPerMB;
		return FMath::Min<uint64>(TNumericLimits<uint32>::Max(), CollisionCountLimit);
	}
	...
	{
	public:
		FDataScanner(const TArray<uint32>& ChunkWindowSizes, const TArray<uint8>& Data, const ICloudEnumeration* CloudEnumeration, FStatsCollector* StatsCollector, uint64 DataOffset);
	...
	private:
		...
		const uint32 CollisionCountLimit;
		const bool bForceSkipMatches;
	...
	FDataScanner::FDataScanner(const TArray<uint32>& InChunkWindowSizes, const TArray<uint8>& InData, const ICloudEnumeration* InCloudEnumeration, FStatsCollector* InStatsCollector, uint64 InDataOffset)
		...
		, CollisionCountLimit(GetCollisionCountLimit(Data.Num()))
		, bForceSkipMatches(GetForceSkipMatches())
		, bIsComplete(false)
		, bShouldAbort(false)
		, CollisionCount(0)
	...
	bool FDataScanner::FindChunkDataMatch(const FRollingHash& RollingHash, const uint32& DataStart, FGuid& OutChunkMatch, FSHAHash& OutChunkSha)
	{
		...
		if (bMightHaveMatch)
		{
			...
			{
				UE_LOGF(LogDataScanner, VeryVerbose, "@%llu: Collision at %d with chunkhash %llX SHA %ls", DataOffset, DataStart, RollingHash.GetWindowHash(), *OutChunkSha.ToString());
				++CollisionCount;
				FStatsCollector::Accumulate(StatHashCollisions, 1);
				// Check for aborting based on high collisions. This is an emergency feature for rare problems
				// with unusually high processing times.
				if (CollisionCount > CollisionCountLimit)
				{
					UE_LOGF(LogDataScanner, Log, "@%llu: Aborting scanner with %d collisions!", DataOffset, CollisionCount);
					bShouldAbort = true;
				}
			}
		}
	...
	TArray<FChunkMatch> FDataScanner::ScanData()
	{
		...
		for (const uint32 WindowSize : ChunkWindowSizes)
		{
			...
			bool bScanningData = true;
			{
				...
				while (bScanningData && !bShouldAbort)
				{
					...
					const bool bCanSkipData = bFoundChunkMatch && (!bChunkOverlap || bForceSkipMatches);
					if (bCanSkipData)
					{
						RollingHash.Clear();
		...
		UE_LOGF(LogDataScanner, Log, "@%llu: Scanner completed in %ls with %d collisions.", DataOffset, *FPlatformTime::PrettyTime(ScanTimer.GetSeconds()), CollisionCount);
	...
	IDataScanner* FDataScannerFactory::Create(const TArray<uint32>& ChunkWindowSizes, const TArray<uint8>& Data, const ICloudEnumeration* CloudEnumeration, FStatsCollector* StatsCollector, uint64 DataOffset)
	{
		return new FDataScanner(ChunkWindowSizes, Data, CloudEnumeration, StatsCollector, DataOffset);
	}
}
