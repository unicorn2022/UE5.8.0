...
namespace BuildPatchServices
{
	...
	public:
		...
		// This is the match as reported by the hash comparisions. It's offset and size should always exactly match the Range below.
		const FChunkMatch Match;

		// This is the range in the new version data where we have the match. 
		const FBlockRange Range;

		// Whether the match came from the same source file by name.
		const bool bSameFile;

		// This is the distance to the match in the source stream. It's used for selecting closer matches in the event of multiple valid matches.
	...
	public:
		...
		bool AddMatch(const FChunkMatchMeta& NewMatchMeta, bool& bForceSkip)
		{
			...
			{
				// First advance idx if no overlap in the source data. If the new match is distinct, then
				// we end up at Matches.Num(), which falls through to accepting the match.
	...
	void FDeltaScanner::FindChunkDataMatches(const FRollingHash& RollingHash, const TSet<FDeltaChunkId>*& ChunkMatches, FShaId& ChunkShaId)
	{
		...
		if (bMightHaveMatch)
		{
			...
			{
				// These are the actual chunks we got from the base build. We might have duplicates. We don't get to know where they came from.
	...
	bool FDeltaScanner::ProcessMatch(const FDeltaChunkId& ChunkMatch, const FBlockRange& ScanLocation, bool& bSameFile, uint64& LocationOffset)
	{
		...
		while (!FilesListNode->GetValue().FileScannerRange.Overlaps(ScanLocation))
		{
		...
		check(ScanLocation.GetFirst() >= ScanFileInfo.FileScannerRange.GetFirst());
		const uint64 ScanFileLocation = ScanFileInfo.ScannerFileLocation + (ScanLocation.GetFirst() - ScanFileInfo.FileScannerRange.GetFirst());
		bSameFile = false;

		// We only accept matches from the same tagset. Note that it matches _any_ tag in the set, so having multiple tags
		// can run into issues where if the tag sets are not installed, this might think they are and expect an on-disk match
		// that doesn't exist.
		if (MatchBuildReference.FileReferences[0].Tagset.Num() == 0 || MatchBuildReference.FileReferences[0].Tagset.Includes(ScanFileInfo.ScanTagSet))
		{
			bSameFile = MatchBuildReference.FileReferences[0].Filename == ScanFileInfo.ScanFilename;
			if (bSameFile)
			{
				LocationOffset = FMath::Max<uint64>(MatchBuildReference.FileReferences[0].Location, ScanFileLocation) - FMath::Min<uint64>(MatchBuildReference.FileReferences[0].Location, ScanFileLocation);
			}
			else
			{
	...
	TArray<FChunkMatch> FDeltaScanner::ScanData()
	{
		...
		bool bScanningData = true;
		{
			...
			while (bScanningData && !bShouldAbort)
			{
				...
				// Check for a chunk match at this offset. All matches are identical copies - we could have it in many locations
				...
				if (bFoundChunkMatch)
				{
					...
					// FindChunkDataMatches returns a bunch of matches that are the same data, and we need to pick one. Any of them will provide
					// the data we want.
