...
DEFINE_LOG_CATEGORY(LogManifestBuildStreamer);
...
namespace BuildPatchServices
{
			...
			while (DataStack.Num() && GrabbedBytes < ReqSize)
			{
				...
				if (ChunkDataAccess != nullptr)
				{
					...
					if (TopStackOffset >= NextData.Size)
					{
						...
						{
							UE_LOGF(LogManifestBuildStreamer, Fatal, "Ordering failure, lost track of references.");
						}
						TopStackOffset = 0;
						DataStack.Pop();
					}
				...
				{
					UE_LOGF(LogManifestBuildStreamer, Error, "Could not get chunk data (%ls).", *NextData.Guid.ToString());
					break;
				}
			}
		}
	...
	void FManifestBuildStreamer::Initialise()
	{
		...
		for (const FString& BuildFile : BuildFiles)
		{
			...
			{
				UE_LOGF(LogManifestBuildStreamer, Fatal, "Could not get file manifest (%ls).", *BuildFile);
			}
