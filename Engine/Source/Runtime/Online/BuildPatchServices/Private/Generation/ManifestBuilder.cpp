...
namespace BuildPatchServices
{
	...
	public:
		...
		virtual bool SaveToFile(const FString& Filename, FSHAHash* OutSHA1Hash, FMD5Hash* OutMD5Hash) override;
		virtual FBuildPatchAppManifest& GetManifest() const override;
	...
	private:
		...
		FGuid ManifestEncryptionSecretId;
		TArray<uint8> ManifestEncryptionSecretKey;
		FFileAttributesMap FileAttributesMap;
		...
		, bFinalizationSucceeded(false)
	{
		...
		Manifest->ManifestMeta.UninstallActionPath = InDetails.UninstallActionPath;
		Manifest->ManifestMeta.UninstallActionArgs = InDetails.UninstallActionArgs;
		ManifestEncryptionSecretId = InDetails.EncryptionSecretId;
		ManifestEncryptionSecretKey = InDetails.EncryptionSecretKey;
	...
	void FManifestBuilder::AddChunkMatch(const FGuid& ChunkGuid, const FBlockStructure& Structure)
	{
		...
		UE_LOGF(LogManifestBuilder, Verbose, "Match added for chunk %ls.", *ChunkGuid.ToString());
	}
	...
	bool FManifestBuilder::FinalizeData(const TArray<FFileSpan>& FileSpans, TArray<FChunkInfo> ChunkInfo)
	{
		...
		{
			FFileAttributes FileAttributes = FileAttributesMap.Find(FileSpan.Filename);
			...
			FileManifest.MD5Hash = FileSpan.MD5Hash;
			FileManifest.SHA1Hash = FileSpan.SHA1Hash;
			FileManifest.SHA256Hash = FileSpan.SHA256Hash;
			FileManifest.MIMEType = FileSpan.MIMEType;
		...
		UE_LOGF(LogManifestBuilder, Verbose, "Manifest references %d chunks.", ReferencedChunks.Num());
		...
		UE_LOGF(LogManifestBuilder, Verbose, "Chunk info list trimmed from %d to %d.", TotalChunkListNum, Manifest->ChunkDataList.ChunkList.Num());
		...
		for (const FGuid& ReferencedChunk : ReferencedChunks)
		{
			...
			{
				UE_LOGF(LogManifestBuilder, Error, "Generated manifest is missing ChunkInfo for chunk %ls.", *ReferencedChunk.ToString());
				bHasAllInfo = false;
			}
		}
		...
		{
			UE_LOGF(LogManifestBuilder, Log, "Setting PrereqIds to be the SHA hash of the PrereqPath.");
			FSHAHash PrereqHash;
		...
		{
			UE_LOGF(LogManifestBuilder, Error, "Build structure is empty and was not explicitly allowed.");
			return false;
		}
		if (!bBuildStructureIsWhole)
		{
			UE_LOGF(LogManifestBuilder, Error, "Build structure added was not whole or complete.");
			return false;
		}
		if (BuildStructureSize != Manifest->GetBuildSize())
		{
			UE_LOGF(LogManifestBuilder, Error, "Generated manifest build size did not equal build structure added.");
			return false;
		}

		if (Manifest->ManifestMeta.FeatureLevel <= EFeatureLevel::UsesRuntimeGeneratedBuildId)
		{
			Manifest->ManifestMeta.BuildId = FBuildPatchUtils::GetBackwardsCompatibleBuildId(Manifest->ManifestMeta);
		}

		if (ManifestEncryptionSecretId.IsValid())
		{
			if (!Manifest->EncryptData(ManifestEncryptionSecretId, ManifestEncryptionSecretKey))
			{
				UE_LOGF(LogManifestBuilder, Error, "Generated manifest failed to encrypt properly.");
				return false;
			}
		}
	...
	bool FManifestBuilder::SaveToFile(const FString& Filename, FSHAHash* OutSHA1Hash, FMD5Hash* OutMD5Hash)
	{
		...
		if (bFinalizationSucceeded)
		{
			...
			bSuccess = Manifest->SaveToFile(TmpFilename, Manifest->ManifestMeta.FeatureLevel, OutSHA1Hash, OutMD5Hash);
			if (!bSuccess)
			{
				UE_LOGF(LogManifestBuilder, Error, "Could not export manifest to %ls.", *TmpFilename);
				return bSuccess;
			}
			bSuccess = FileSystem->MoveFile(*Filename, *TmpFilename);
			if (!bSuccess)
			{
				UE_LOGF(LogManifestBuilder, Error, "Could not move manifest from %ls to %ls.", *TmpFilename, *Filename);
			}
		}
		else
		{
			UE_LOGF(LogManifestBuilder, Error, "Cannot save manifest when build integrity check did not succeed.");
		}
		return bSuccess;
	}

	FBuildPatchAppManifest& FManifestBuilder::GetManifest() const
	{
		check(bFinalizationSucceeded);
		return *Manifest;
	}
