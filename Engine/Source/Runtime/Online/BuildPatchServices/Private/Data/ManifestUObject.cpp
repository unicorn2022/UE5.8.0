...
{
	UBuildPatchManifest* CreateUBuildPatchManifestFromAsync()
	{
	...
	BuildPatchServices::FFileManifest FromFileManifestData(const FFileManifestData& Input)
	{
		...
		FMemory::Memcpy(Output.SHA1Hash.Hash, Input.FileHash.Hash, FSHA1::DigestSize);
		...
		Output.FileMetaFlags |= Input.bIsReadOnly ? EFileMetaFlags::ReadOnly : EFileMetaFlags::None;
		Output.FileMetaFlags |= Input.bIsCompressed ? EFileMetaFlags::Compressed : EFileMetaFlags::None;
		Output.FileMetaFlags |= Input.bIsUnixExecutable ? EFileMetaFlags::UnixExecutable : EFileMetaFlags::None;
		return Output;
	}
	...
	FFileManifestData ToFileManifestData(const BuildPatchServices::FFileManifest& Input)
	{
		...
		Output.Filename = Input.Filename;
		FMemory::Memcpy(Output.FileHash.Hash, Input.SHA1Hash.Hash, FSHA1::DigestSize);
		...
		Output.InstallTags = Input.InstallTags;
		Output.bIsUnixExecutable = EnumHasAllFlags(Input.FileMetaFlags, EFileMetaFlags::UnixExecutable);
		Output.SymlinkTarget = Input.SymlinkTarget;
		Output.bIsReadOnly = EnumHasAllFlags(Input.FileMetaFlags, EFileMetaFlags::ReadOnly);
		Output.bIsCompressed = EnumHasAllFlags(Input.FileMetaFlags, EFileMetaFlags::Compressed);
		return Output;
	}
...
bool FManifestUObject::LoadFromMemory(const TArray<uint8>& DataInput, FBuildPatchAppManifest& AppManifest)
{
	...
	UE_LOGF(LogManifestUObject, Error, "FManifestUObject::LoadFromMemory called but UObjects are disabled for BuildPatchServices");
	return false;
...
bool FManifestUObject::SaveToArchive(FArchive& Ar, const FBuildPatchAppManifest& AppManifest)
{
	...
	if (Ar.IsSaving())
	{
		...
		if (bSaveOk && !ManifestData.IsError())
		{
			...
			Header.StoredAs = bDataIsCompressed ? EManifestStorageFlags::Compressed : EManifestStorageFlags::RawData;
	...
	UE_LOGF(LogManifestUObject, Error, "FManifestUObject::SaveToArchive called but UObjects are disabled for BuildPatchServices");
	Ar.SetError();
	return false;
...
bool FManifestUObject::LoadInternal(FArchive& Ar, FBuildPatchAppManifest& AppManifest)
{
	...
	UBuildPatchManifest* Data = ManifestUObjectHelpers::CreateUBuildPatchManifestFromAsync();
	...
	UE_LOGF(LogManifestUObject, Error, "FManifestUObject::SerializeInternal called but UObjects are disabled for BuildPatchServices");
	return false;
