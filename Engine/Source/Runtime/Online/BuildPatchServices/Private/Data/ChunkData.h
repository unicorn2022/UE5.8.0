...
namespace BuildPatchServices
{
	...
	enum class EChunkHashFlags : uint8
	{
		...
		// Flag for AES AuthTag being present in header header.
		AesAuthTag = 0x02,
	};
	...
	enum class EChunkLoadResult : uint8
	{
		...
		// If the decryption key necessary has not been provided, this result is given.
		// It is up to the calling code to tell whether this is acceptable (expected preloading for example).
		MissingSecretKey,

		// The data was saved encrypted, and we had the secret key, but decryption failed.
		DecryptionFailure,
	...
	enum class EChunkSaveResult : uint8
	{
		...
		// If we were asked to encrypted saved out chunks but did not get the key, we cannot save at all.
		MissingEncryptionKey,

		// If the crypto interface returns an error.
		EncryptionFail,
	...
	struct FChunkHeader
	{
		...
		FGuid Guid;
		// An ID that identifies the encryption secret required to decrypt this chunk. This is blank if the chunk is not encrypted.
	...
	struct FChunkPart
	{
		...
		uint32 Size;
		// Determine whether the chunk is from delta file or manifest (default).
	...
	struct FChunkInfo
	{
		...
		uint32 DataSizeUncompressed = 1048576;
	...
	{
	public:
		virtual ~IChunkDataAccess() = default;
	...
	{
	public:
		virtual ~IChunkDataSerialization() = default;
		 ...
		 * @param Filename          The full file path to the file.
		 * @param ChunkDataAccess   Ptr to the chunk data to save. The header will be updated with relevant changes.
		 * @return the result, EChunkSaveResult::Success if successful.
		 */
		virtual EChunkSaveResult SaveToFile(const FString& Filename, IChunkDataAccess* ChunkDataAccess) const = 0;
		 ...
		 * @return the result, EChunkSaveResult::Success if successful.
		 */
		virtual EChunkSaveResult SaveToMemory(TArray<uint8>& Memory, IChunkDataAccess* ChunkDataAccess) const = 0;
		 ...
		 * @return the result, EChunkSaveResult::Success if successful.
		 */
		virtual EChunkSaveResult SaveToArchive(FArchive& Archive, IChunkDataAccess* ChunkDataAccess) const = 0;

		// In performance sensitive situations, can request the serializer to save uncompressed if implemented
		virtual EChunkSaveResult SaveToArchiveUncompressed(FArchive& Archive, IChunkDataAccess* ChunkDataAccess) const
		{
		...
		virtual bool DecompressAndDecryptValidatedRead(const FChunkHeader& InHeader, FMutableMemoryView InDestionationBuffer, const FUniqueBuffer& InCompressedBuffer) const = 0;
		 ...
		 * @return true if ShaHashData was successfully injected into memory, false otherwise.
		 */
		virtual bool InjectShaToChunkData(TArray<uint8>& Memory, const FSHAHash& ShaHashData) const = 0;
	};

	struct FChunkDataSerializationConfig
	{
		EFeatureLevel FeatureLevel = EFeatureLevel::Latest;
		TMap<FGuid,TArray<uint8>> EncryptionSecrets;
		FGuid WriteEncryptionSecretId;
	...
	{
	public:
		static IChunkDataSerialization* Create(IFileSystem* FileSystem, ICrypto* Crypto, FChunkDataSerializationConfig Config);
	};
}