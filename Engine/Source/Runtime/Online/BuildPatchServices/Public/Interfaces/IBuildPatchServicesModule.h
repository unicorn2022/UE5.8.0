...
DECLARE_MULTICAST_DELEGATE_TwoParams(FSingleChunkWrittenDelegate, FString /* Filename */, FString /* MD5Hash */);
DECLARE_MULTICAST_DELEGATE_ThreeParams(FManifestCreatedDelegate, FString /* Filename */, FString /* SHA1Hash */, FString /* MD5Hash */);
...
public:
	 ...
	 * Virtual destructor.
	 */
	virtual ~IBuildPatchServicesModule() = default;
	...
	/**
	 * Factory providing construction of a directory chunker class.
	 * @param Configuration         Specifies the settings for the operation.  See BuildPatchServices::FDirectoryChunkerConfiguration comments.
	 * @return an instance of an IDirectoryChunker implementation.
	 */
	virtual BuildPatchServices::IDirectoryChunkerRef CreateDirectoryChunker(BuildPatchServices::FDirectoryChunkerConfiguration Configuration) const = 0;
	...
	/**
	 * Factory providing construction of a chunk delta optimiser class.
	 ...
	 * @return an instance of an IChunkDeltaOptimiser implementation.
	 */
	virtual BuildPatchServices::IChunkDeltaOptimiserRef CreateChunkDeltaOptimiser(BuildPatchServices::FChunkDeltaOptimiserConfiguration Configuration) const = 0;

	/**
	 * Factory providing construction of a chunk harvester class.
	 * @param Configuration         Specifies the settings for the operation.  See BuildPatchServices::FChunkHarvesterConfiguration comments.
	 * @return an instance of an IChunkHarvester implementation.
	 */
	virtual BuildPatchServices::IChunkHarvesterRef CreateChunkHarvester(BuildPatchServices::FChunkHarvesterConfiguration Configuration) const = 0;
	 ...
	 * @param EncryptionSecretId    The ID for the secret key.
	 * @param EncryptionSecretKey   The secret key.
	 */
	virtual void RegisterEncryptionSecret(const FGuid& EncryptionSecretId, TArray<uint8> EncryptionSecretKey) = 0;

	/**
	 * Unregisters an encryption secret.
	 * @param EncryptionSecretId    The ID for the secret key.
	 * @return returns true if the secret existed and was removed.
	 */
	virtual bool UnregisterEncryptionSecret(const FGuid& EncryptionSecretId) = 0;

	/**
	 * Generates random data via OpenSSL.
	 * @param OutData    The object to fill with random data.
	 * @return returns true if OpenSSL was able to generate random data.
	*/
	virtual bool CreateRandomBytes(const TArrayView<uint8> OutData) = 0;
	 ...
	 * @param ManifestFilePathB         A full file path for the merge manifest to be loaded, by default files in B will stomp over A.
	 * @param NewVersionString          The new version string for the build, all other meta will be copied from B.
	 * @param FilesFromA                A set containing the files that should be kept from ManifestA.
	 * @param FilesFromB                A set containing the files that should be kept from ManifestB.
	 * @return resulting manifest if successful.
	 */
	virtual IBuildManifestPtr MergeManifests(const IBuildManifestRef& ManifestA, const IBuildManifestRef& ManifestB, const FString& NewVersionString, const TSet<FString>& FilesFromA, const TSet<FString>& FilesFromB) = 0;
	...
	UE_DEPRECATED(4.16, "EnumerateManifestData(const FString& ManifestFilePath, const FString& OutputFile, bool bIncludeSizes) has been deprecated.  Please use CreatePatchDataEnumeration(BuildPatchServices::FPatchDataEnumerationConfiguration Configuration) instead.")
	virtual bool EnumerateManifestData(const FString& ManifestFilePath, const FString& OutputFile, bool bIncludeSizes)
	{
		...
		BuildPatchServices::IPatchDataEnumerationRef PatchDataEnumeration = CreatePatchDataEnumeration(MoveTemp(Configuration));
		return PatchDataEnumeration->Run();
	}

	UE_DEPRECATED(4.23, "ChunkBuildDirectory(const BuildPatchServices::FChunkBuildConfiguration& Configuration) has been deprecated.  Please use CreateDirectoryChunker(BuildPatchServices::FDirectoryChunkerConfiguration Configuration) instead.")
	virtual bool ChunkBuildDirectory(const BuildPatchServices::FChunkBuildConfiguration& Configuration)
	{
		BuildPatchServices::IDirectoryChunkerRef DirectoryChunker = CreateDirectoryChunker(Configuration);
		return DirectoryChunker->Run();
	}

	UE_DEPRECATED(4.23, "EnumeratePatchData(const BuildPatchServices::FPatchDataEnumerationConfiguration& Configuration) has been deprecated.  Please use CreatePatchDataEnumeration(BuildPatchServices::FPatchDataEnumerationConfiguration Configuration) instead.")
	virtual bool EnumeratePatchData(const BuildPatchServices::FPatchDataEnumerationConfiguration& Configuration)
	{
		BuildPatchServices::IPatchDataEnumerationRef PatchDataEnumeration = CreatePatchDataEnumeration(Configuration);
		return PatchDataEnumeration->Run();
	}

	UE_DEPRECATED(4.23, "OptimiseChunkDelta(const BuildPatchServices::FChunkDeltaOptimiserConfiguration& Configuration) has been deprecated.  Please use CreateChunkDeltaOptimiser(BuildPatchServices::FChunkDeltaOptimiserConfiguration Configuration) instead.")
	virtual bool OptimiseChunkDelta(const BuildPatchServices::FChunkDeltaOptimiserConfiguration& Configuration)
	{
		BuildPatchServices::IChunkDeltaOptimiserRef ChunkDeltaOptimiser = CreateChunkDeltaOptimiser(Configuration);
		return ChunkDeltaOptimiser->Run();
	}

	UE_DEPRECATED(4.21, "GenerateChunksManifestFromDirectory(const BuildPatchServices::FGenerationConfiguration& Configuration) has been deprecated.  Please use CreateDirectoryChunker(BuildPatchServices::FDirectoryChunkerConfiguration Configuration) instead.")
