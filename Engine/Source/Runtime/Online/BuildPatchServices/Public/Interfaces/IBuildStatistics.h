...
namespace BuildPatchServices
{
	...
	public:
		 ...
		 * Virtual destructor.
		 */
		virtual ~IBuildStatistics() = default;
		 ...
		 */
		virtual int64 GetDownloadedBytes() const = 0;

		/**
		 * @return the number of bytes read so far.
		 */
		virtual int64 GetDiskReadBytes() const = 0;

		/**
		 * @return the number of bytes written so far.
		 */
		virtual int64 GetDiskWrittenBytes() const = 0;
...
inline const TCHAR* LexToString(BuildPatchServices::EFileOperationState Value)
{
	using namespace BuildPatchServices;
	static_assert(int32(EFileOperationState::NUM_States) == int32(EFileOperationState::VerifiedSuccess) + 1, "Please add support for the extra values below.");
#define CASE_ENUM_TO_STR(_X_) case EFileOperationState::_X_: return TEXT(#_X_)
	switch (Value)
	{
		CASE_ENUM_TO_STR(Unknown);
		CASE_ENUM_TO_STR(PendingLocalChunkDbData);
		CASE_ENUM_TO_STR(RetrievingLocalChunkDbData);
		CASE_ENUM_TO_STR(PendingLocalInstallData);
		CASE_ENUM_TO_STR(RetrievingLocalInstallData);
		CASE_ENUM_TO_STR(PendingRemoteCloudData);
		CASE_ENUM_TO_STR(RetrievingRemoteCloudData);
		CASE_ENUM_TO_STR(PendingLocalDataStore);
		CASE_ENUM_TO_STR(RetrievingLocalDataStore);
		CASE_ENUM_TO_STR(DataInMemoryStore);
		CASE_ENUM_TO_STR(Staged);
		CASE_ENUM_TO_STR(Installed);
		CASE_ENUM_TO_STR(Verifying);
		CASE_ENUM_TO_STR(VerifiedFail);
		CASE_ENUM_TO_STR(VerifiedSuccess);
		CASE_ENUM_TO_STR(NUM_States);
		default: return TEXT("InvalidOrMax");
	}
#undef CASE_ENUM_TO_STR
}

inline void LexFromString(BuildPatchServices::EFileOperationState& Value, const TCHAR* Buffer)
{
	using namespace BuildPatchServices;
	static_assert(int32(EFileOperationState::NUM_States) == int32(EFileOperationState::VerifiedSuccess) + 1, "Please add support for the extra values below.");
#define RETURN_IF_EQUAL(_X_) if (FCString::Stricmp(Buffer, TEXT(#_X_)) == 0) { Value = EFileOperationState::_X_; return; }
	const TCHAR* const Prefix = TEXT("EFileOperationState::");
	const SIZE_T PrefixLen = FCString::Strlen(Prefix);
	if (FCString::Strnicmp(Buffer, Prefix, PrefixLen) == 0)
	{
		Buffer += PrefixLen;
	}
	RETURN_IF_EQUAL(Unknown);
	RETURN_IF_EQUAL(PendingLocalChunkDbData);
	RETURN_IF_EQUAL(RetrievingLocalChunkDbData);
	RETURN_IF_EQUAL(PendingLocalInstallData);
	RETURN_IF_EQUAL(RetrievingLocalInstallData);
	RETURN_IF_EQUAL(PendingRemoteCloudData);
	RETURN_IF_EQUAL(RetrievingRemoteCloudData);
	RETURN_IF_EQUAL(PendingLocalDataStore);
	RETURN_IF_EQUAL(RetrievingLocalDataStore);
	RETURN_IF_EQUAL(DataInMemoryStore);
	RETURN_IF_EQUAL(Staged);
	RETURN_IF_EQUAL(Installed);
	RETURN_IF_EQUAL(Verifying);
	RETURN_IF_EQUAL(VerifiedFail);
	RETURN_IF_EQUAL(VerifiedSuccess);
	RETURN_IF_EQUAL(NUM_States);
	// Did not match
	Value = EFileOperationState::Unknown;
