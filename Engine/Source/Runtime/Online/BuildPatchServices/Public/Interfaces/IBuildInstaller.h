...
enum class EBuildPatchInstallError
{
	...
	// The custom uninstall action failed
	CustomUninstallActionError = 12,

	// Used to help verify logic
	InvalidOrMax,
	// Deprecated value. Use InvalidOrMax instad of NumInstallErrors
	NumInstallErrors = InvalidOrMax,
};
...
namespace InstallErrorPrefixes
{
	static const TCHAR* ErrorTypeStrings[] =
	{
		...
		TEXT("UA"), // UninstallCustomActionError
	};
};
...
{
	// The identifier for this cloud directory, e.g. base URL.
	FString CloudDirectory;

	// Total number of bytes downloaded from this cloud directory.
	uint64 TotalReceivedData = 0;

	// Total number of errors encountered from this cloud directory.
	uint32 ErrorCount = 0;
};
...
{
public:
	bool UsedOptimizedDelta = false;
	uint64 OptimizedDeltaHTTPResponseCode = 0;
	uint64 NumLocalChunksExpected = 0;
	uint64 NumLocalChunksAvailable = 0;
	uint64 NumChunksRecycleFailures = 0;
};
...
struct FBuildInstallStats
{
	...
	// The average number of simultaneous download requests 
	uint32 ActiveRequestCountAvg;
	// The Experiment name if applicable for A/B Testing
	FString ExperimentName;
	// The Variant of the Experiment if applicable for A/B Testing
	FString ExerimentVariantName;
	...
	float ConstructTime;
	// The time spent on chunks decryption.
	uint64 TotalDecryptTime;
	// The time spent during the custom uninstall actiion run.
	float UninstallActionTime;
	...
	// Stats tracked per cloud directory.
	TArray<FCloudDirectoryStats> CloudDirectoryStats;
	// Stats tracked per action. Array order matches actions order in configuration.
	TArray<FInstallActionStats> ActionsStats;
};
...
public:
	 ...
	 * Virtual destructor.
	 */
	virtual ~IBuildInstaller() = default;
	...
	/**
	 * Get whether the install is performing a repair of an existing install.
	 * @return	true if installation is a repair
	 */
	virtual bool IsRepair() const = 0;

	/**
	 * Get whether the install is performing a verify only of an existing install, without repair.
	 * @return	true if installation is a verify only.
	 */
	virtual bool IsVerifyOnly() const = 0;

	/**
	 * Get whether the install is actually uninstalling instead.
	 * @return	true if installation is actually an uninstallation
	 */
	virtual bool IsUninstall() const = 0;
	 ...
	 * @param InDownloadRateLimitBps    throttle value in bytes per second. 
	 */
	virtual void SetDownloadBytesPerSecondLimit(uint64 InDownloadRateLimitBps) = 0;

	/**
	 * Gets the download rate limit in Bytes per second.
	 * @return the current download rate limit.
	 */
	virtual uint64 GetDownloadBytesPerSecondLimit() const = 0;
	...
	/**
	* [Experimental]
	* Pass the installation folder on a per file basis. This path will be appended to the installation directory passed
	* in the FBuildInstallerConfiguration::InstallDirectory member. The old version is used for patch source data, the new
	* for everything else.
	*/
	virtual void SetPerFileSubdirectories(TMap<FString, FString>&& InOldPerFileSubdirectories, TMap<FString, FString>&& InNewPerFileSubdirectories) = 0;
};

static_assert((uint32)EBuildPatchInstallError::InvalidOrMax == 13, "Please add support for the extra values to the Lex functions below.");
...
#define CASE_ENUM_TO_STR(Value) case EBuildPatchInstallError::Value: return TEXT(#Value)
	switch (Error)
	{
		...
		CASE_ENUM_TO_STR(CustomUninstallActionError);
...
#define RETURN_IF_EQUAL(Value) if (FCString::Stricmp(Buffer, TEXT(#Value)) == 0) { Error = EBuildPatchInstallError::Value; return; }
	...
	RETURN_IF_EQUAL(CustomUninstallActionError);
	// Did not match
	Error = EBuildPatchInstallError::InvalidOrMax;
	return;
