...
namespace BuildPatchServices
{
	...
	enum class EBuildPatchState : uint32
	{
		...
		// The patch process is running custom uninstall action
		RunningCustomUninstallAction,
...
static_assert((uint32)BuildPatchServices::EBuildPatchState::NUM_PROGRESS_STATES == 13, "Please add support for the extra values to the Lex functions below.");
...
#define CASE_ENUM_TO_STR(Value) case BuildPatchServices::EBuildPatchState::Value: return TEXT(#Value)
	switch (State)
	{
		...
		CASE_ENUM_TO_STR(RunningCustomUninstallAction);
...
#define RETURN_IF_EQUAL(Value) if (FCString::Stricmp(Buffer, TEXT(#Value)) == 0) { Error = BuildPatchServices::EBuildPatchState::Value; return; }
	...
	RETURN_IF_EQUAL(RunningCustomUninstallAction);
