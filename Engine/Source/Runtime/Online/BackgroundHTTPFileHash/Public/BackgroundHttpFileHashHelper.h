...
public:
	...
	// Looks for a temp filename mapping for the given URL. If one isn't found returns false and does NOT generate one.
	BACKGROUNDHTTPFILEHASH_API bool FindTempFilenameMappingForURL(const FString& URL, const FString& RequestID, FString& OutTempFilename) const;
	UE_DEPRECATED(5.8, "Please use FindTempFilenameMappingForURL with RequestID and reference out arguments instead")
	...
	BACKGROUNDHTTPFILEHASH_API const FString& FindOrAddTempFilenameMappingForURL(const FString& URL, const FString& RequestID);

	// Removes URL mapping
	BACKGROUNDHTTPFILEHASH_API void RemoveURLMapping(const FString& URL, const FString& RequestID);
	UE_DEPRECATED(5.8, "Please use RemoveURLMapping with RequestID argument instead")
	void RemoveURLMapping(const FString& URL) { RemoveURLMapping(URL, {}); }
    ...
    static BACKGROUNDHTTPFILEHASH_API const FString& GetTemporaryDownloadPath();
	...
	// Helper function that returns a full path representation of the given TempFilename
...
private:
	...
	BACKGROUNDHTTPFILEHASH_API const FURLTempFileMapping* FindMappingForURLNoLock(const FString& URL, const FString& RequestID) const;

	// Finds the hash that should be used to refer to this URL+RequestID. Either finds the hash used in the map to reference the
	// URLFileMappings entry for this URL+RequestID or returns the hash we should use (the next valid hash entry) without inserting it into the map.
	BACKGROUNDHTTPFILEHASH_API FString FindValidFilenameHashForURLNoLock(const FString& URL, const FString& RequestID) const;

	// Creates a hashed Filename with a modifier for the given collision count.
	static BACKGROUNDHTTPFILEHASH_API FString GenerateHashedFilenameForURLNoLock(const FString& URL, const FString& RequestID, uint32 CollisionCount);
