...
{
	static FName ModularFeatureName = FName("BackgroundHttpModularFeature");
...
void FModularFeaturePlatformBackgroundHttp::CacheModularFeature()
{
	if (!bHasCheckedForModularFeature)
	{
		...
		if (ModuleName.IsEmpty())
		{
			...
			UE_LOGF(LogBackgroundHttpModularFeature, Display, "BackgroundHttpModularFeature module not set, falling back to generic implementation.");
			return;
		}
		...
		if (bDesiredModuleExists)
		{
			...
			if (IModularFeatures::Get().IsModularFeatureAvailable(GetModularFeatureName()))
			{
				...
				UE_LOGF(LogBackgroundHttpModularFeature, Display, "Using BackgroundHTTPModularFeature module: %ls for ModularFeatureName: %ls", *CachedModularFeature->GetDebugModuleName(), *(GetModularFeatureName().ToString()));
			}
			else
			{
				UE_LOGF(LogBackgroundHttpModularFeature, Display, "Module %ls exists but is not available/registered for ModularFeatureName: %ls", *ModuleName, *(GetModularFeatureName().ToString()));
			}
		}
		
		if (nullptr == CachedModularFeature)
		{
			UE_LOGF(LogBackgroundHttpModularFeature, Display, "Using Generic Implementation as ModularFeature: %ls is not registered or available", *(GetModularFeatureName().ToString()));
		}
	}
}