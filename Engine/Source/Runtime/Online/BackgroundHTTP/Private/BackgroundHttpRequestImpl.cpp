...
{
	UE_LOGF(LogBackgroundHttpRequest, Verbose, "Processing Request - RequestID:%ls", *GetRequestID());
...
{
	UE_LOGF(LogBackgroundHttpRequest, Display, "Cancelling Request - RequestID:%ls", *GetRequestID());
	FBackgroundHttpModule::Get().GetBackgroundHttpManager()->RemoveRequest(SharedThis(this));
}

void FBackgroundHttpRequestImpl::RequeueRequest()
{
	UE_LOGF(LogBackgroundHttpRequest, Display, "RequeueRequest - RequestID:%ls", *GetRequestID());
	FBackgroundHttpModule::Get().GetBackgroundHttpManager()->RequeueRequest(SharedThis(this));
}

void FBackgroundHttpRequestImpl::PauseRequest()
{
    //for now a pause is just wrapping a cancel in the general case
    UE_LOGF(LogBackgroundHttpRequest, Display, "Pausing Request (through cancel) - RequestID:%ls", *GetRequestID());
    CancelRequest();
}
...
void FBackgroundHttpRequestImpl::ResumeRequest()
{
    ...
    UE_LOGF(LogBackgroundHttpRequest, Display, "Resuming Request (through restart) - RequestID:%ls", *GetRequestID());
    ProcessRequest();
}
...
void FBackgroundHttpRequestImpl::OnBackgroundDownloadComplete()
{
	...
	UE_LOGF(LogBackgroundHttpRequest, Display, "Download Complete - RequestID:%ls | bWasSuccess:%d | ResponseTempLocation:%ls", *GetRequestID(), (int)(bWasSuccess), *ResponseTempLocation);
	...
	NotifyNotificationObjectOfComplete(bWasSuccess ? EBackgroundHttpRequestCompletionResult::Success : EBackgroundHttpRequestCompletionResult::GenericError);
	...
	if (OptionalMetricsInfo.IsSet())
	{
		...
		{
			UE_LOGF(LogBackgroundHttpRequest, Warning, "OptionalMetricsInfo neither OnRequestMetrics() or OnRequestMetricsExtended() are bound - RequestID:%ls", *GetRequestID());
		}
	...
	{
		UE_LOGF(LogBackgroundHttpRequest, Warning, "OptionalMetricsInfo missing, possibly iOS not yet didFinishCollectingMetrics: RequestID:%ls", *GetRequestID());
	}
}

void FBackgroundHttpRequestImpl::NotifyNotificationObjectOfComplete(bool bSuccess)
{
	NotifyNotificationObjectOfComplete(bSuccess ? EBackgroundHttpRequestCompletionResult::Success : EBackgroundHttpRequestCompletionResult::GenericError);
}

void FBackgroundHttpRequestImpl::NotifyNotificationObjectOfComplete(EBackgroundHttpRequestCompletionResult Result)
{
	...
	// If Result indicates out of diskspace issues, set our flag now
	if (Result == EBackgroundHttpRequestCompletionResult::OutOfDiskSpaceError)
	{
		bHasSeenOutOfDiskSpaceError = true;
	}
	
	if (LocalDownloadCompleteNotificationObject.IsValid())
	{
		LocalDownloadCompleteNotificationObject->NotifyOfDownloadResult(Result == EBackgroundHttpRequestCompletionResult::Success);
	}
}
...
void FBackgroundHttpRequestImpl::CompleteWithExistingResponseData(FBackgroundHttpResponsePtr BackgroundResponse)
{
	...
	UE_LOGF(LogBackgroundHttpRequest, Verbose, "Completing Download With Existing Response Data - RequestID:%ls | bHasValidResponse:%d | ResponseTempDownloadLocation:%ls", *GetRequestID(), (int)(bHasValidResponse), *ResponseTempLocation);
