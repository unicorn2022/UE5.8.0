...
public:
	...
	*/
	...
	/**
	* Sets whether or not the background downloader's temporary files are to be
	* wiped clean at startup. Can only be set before the first call to
	* GetBackgroundHttpManager().
	*/
	BACKGROUNDHTTP_API static void ClearPreExistingRequestsAtStartup(bool bEnable);
...
private:
	...
	/** Setting representing whether or not all pre-existing requests are to be removed
	* at startup. False by default. Use ClearPreExistingRequestsAtStartup(bool) to change.
	*/
	static bool bClearPreExistingRequestsAtStartup;
};
