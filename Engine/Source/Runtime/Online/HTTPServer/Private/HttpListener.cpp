...
bool FHttpListener::StartListening()
{
	...
	{
		UE_LOGF(LogHttpListener, Error, 
			"HttpListener - SocketSubsystem Initialization Failed");
		return false;
	}
	...
	{
		UE_LOGF(LogHttpListener, Error, 
			"HttpListener - Unable to allocate stream socket");
		return false;
	}
	...
	else
	{
		...
		{
			UE_LOGF(LogHttpListener, Error,
				"HttpListener detected invalid bind address (%ls:%u)",
	...
	{
		UE_LOGF(LogHttpListener, Error,
			"HttpListener detected invalid port %u",
			ListenPort);
		return false;
	}
	...
	{
		UE_LOGF(LogHttpListener, Error, 
		"HttpListener unable to bind to %ls",
	...
	{
		UE_LOGF(LogHttpListener, Log, 
			"HttpListener unable to set desired buffer size (%d): Limited to %d",
	...
	{
		UE_LOGF(LogHttpListener, Error, 
			"HttpListener unable to listen on socket");
		return false;
	}
	...
	UE_LOGF(LogHttpListener, Log, 
		"Created new HttpListener on %ls", 
		*BindAddress->ToString(true));
	return true;
}
...
void FHttpListener::StopListening()
{
	...
	{
		UE_LOGF(LogHttpListener, Log,
			"HttpListener stopping listening on Port %u", ListenPort);
...
void FHttpListener::AcceptConnections()
{
	...
	for (int32 i = 0; i < Config.MaxConnectionsAcceptPerFrame; ++i)
	{
		...
		{
			UE_LOGF(LogHttpListener, 
				Error, "ListenSocket failed to query pending connection");
			return;
		}
		...
		if (bHasPendingConnection)
		{
			...
			if (nullptr == IncomingConnection)
			{
				...
				FString ErrorStr = "SocketSubsystem Unavialble";
				...
				UE_LOGF(LogHttpListener, Error,
					"Error accepting expected connection [%d] %ls", (int32)ErrorCode, *ErrorStr);
				return;
			}
