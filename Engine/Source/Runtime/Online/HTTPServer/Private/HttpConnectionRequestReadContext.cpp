...
EHttpConnectionContextState FHttpConnectionRequestReadContext::ReadStream(float DeltaTime)
{
	...
	{
		UE_LOGF(LogHttpConnectionRequestReadContext, Verbose,
...
bool FHttpConnectionRequestReadContext::ParseBody(uint8* ByteBuffer, int32 BufferLen)
{
	...
	{
		// Some HTTP clients (e.g. xCurl with CURL_HTTP_VERSION_2TLS falling back to HTTP/1.1) send
		// "Transfer-Encoding: chunked" even for bodyless requests (GET, HEAD, etc.) and append the
		// chunked empty-body terminator "0\r\n\r\n" (5 bytes) after the request headers. Since the
		// server does not implement chunked request-body decoding, accept this specific case:
		// when no body was expected (IncomingRequestBodyBytesToRead == 0) and the extra bytes are
		// exactly the chunked terminator, treat the body as empty and complete.
		static const uint8 ChunkedTerminator[] = { '0', '\r', '\n', '\r', '\n' };
		static constexpr int32 ChunkedTerminatorLen = UE_ARRAY_COUNT(ChunkedTerminator);
		if (IncomingRequestBodyBytesToRead == 0 &&
			BufferLen >= ChunkedTerminatorLen &&
			FMemory::Memcmp(ByteBuffer, ChunkedTerminator, ChunkedTerminatorLen) == 0)
		{
			// Empty chunked body — consume the terminator and complete body parsing
			bParseBodyComplete = true;
			return true;
		}
...
TSharedPtr<FHttpServerRequest> FHttpConnectionRequestReadContext::BuildRequest(const FString& RequestHeader)
{
	...
	else if (0 == RequestVerb.Compare(TEXT("HEAD"),
		ESearchCase::IgnoreCase))
	{
		Request->Verb = EHttpServerRequestVerbs::VERB_HEAD;
	}
	else if (0 == RequestVerb.Compare(TEXT("TRACE"),
		ESearchCase::IgnoreCase))
	{
		Request->Verb = EHttpServerRequestVerbs::VERB_TRACE;
	}
	else if (0 == RequestVerb.Compare(TEXT("CONNECT"),
		ESearchCase::IgnoreCase))
	{
		Request->Verb = EHttpServerRequestVerbs::VERB_CONNECT;
	}
	else
	{
		// Unknown Verb
	...
	Request->FullPath.SetPath(RequestHttpPath);
