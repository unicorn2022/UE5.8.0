...
void FHttpServerModule::StartAllListeners()
{
	...
	UE_LOGF(LogHttpServerModule, Log,
...
TSharedPtr<IHttpRouter> FHttpServerModule::GetHttpRouter(uint32 Port, bool bFailOnBindFailure /* = false */)
{
	...
	if (ExistingListener)
	{
		if (Impl->bHttpListenersEnabled)
		{
			...
			{
				UE_LOGF(LogHttpServerModule, Verbose, "[%ls] found an existing, active listener for port %d", ANSI_TO_TCHAR(__FUNCTION__), Port);
				return ExistingListener->Get()->GetRouter();
			}
			else
			{
				// get rid of it and create a new one for now
				UE_LOGF(LogHttpServerModule, Error, "[%ls] the existing listener for port %d is not listening/bound and listeners are still enabled",
		...
		{
			UE_LOGF(LogHttpServerModule, Verbose, "[%ls] found an existing listener for port %d but listeners are currently disabled", ANSI_TO_TCHAR(__FUNCTION__), Port);
			return ExistingListener->Get()->GetRouter();
		}
	}

	// Otherwise create a new one
	UE_LOGF(LogHttpServerModule, VeryVerbose, "[%ls] creating a new listener for port %d", ANSI_TO_TCHAR(__FUNCTION__), Port);
	...
	if (Impl->bHttpListenersEnabled)
	{
		...
		{
			UE_LOGF(LogHttpServerModule, Warning, "[%ls] failed to start listening on port %d! (bFailOnBindFailure? %ls)", ANSI_TO_TCHAR(__FUNCTION__), Port, *LexToString(bFailOnBindFailure));
