...
private:
	...
	/**
	 * Single-slot pending response captured by `OnProcessingComplete` while the connection is still draining
	 * a previous write (state == `Writing`). `CompleteWrite` drains it once the in-flight response finishes;
	 * a `checkf(!PendingResponse)` at the queue site enforces the two-deep `FHttpResultCallback` contract.
	 *
	 * Silently dropped if the connection transitions to teardown before the drain runs: `bGracefulDestroyRequested`,
	 * `!bKeepAlive`, or `HandleWriteError`. A `Verbose` log is emitted on the drop path so the disappearance is
	 * traceable in production logs.
	 */
	TUniquePtr<FHttpServerResponse> PendingResponse;
