...
public:
	...
	/**
	 * Determine if the connection should expect multiple writes as part of this response before the next 
	 * read/close by re-using an existing FHttpResultCallback.  Streams should issue the first response callback 
	 * with an empty body to write headers.
	 *
	 * @return true if there are multiple writes expected
	 */
	bool IsMultipleWriteStream() const { return Response.IsValid() && EnumHasAnyFlags(Response->Flags, EHttpServerResponseFlags::MultipleWriteStream); }

	/**
	 * Determine if the context's current state is expecting additional body writes
	 *
	 * @return true if there are more writes
	 */
	bool HasAdditionalWrites() const { return Response.IsValid() && EnumHasAllFlags(Response->Flags, EHttpServerResponseFlags::MultipleWriteStream | EHttpServerResponseFlags::HasAdditionalWrites); }
...
private:
	...
	/** For streaming responses, defer sending headers until first chunk arrives */
	bool bStreamingHeadersPending = false;
};