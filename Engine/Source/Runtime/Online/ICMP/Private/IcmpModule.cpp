...
bool FIcmpModule::Exec_Dev(UWorld* InWorld, const TCHAR* Cmd, FOutputDevice& Ar)
{
	...
	if (FParse::Command(&Cmd, TEXT("Icmp")))
	{
		if (FParse::Command(&Cmd, TEXT("Ping")))
		{
			...
			if (Url.IsEmpty())
			{
				UE_LOGF(LogConsoleResponse, Warning, "Url is missing, console command is Icmp Ping URL, defaulting to 127.0.0.1");
				Url = TEXT("127.0.0.1");
			}
			FIcmp::IcmpEcho(Url, 1.0f, [](FIcmpEchoResult Result)
				{
					if (Result.Status == EIcmpResponseStatus::Success)
					{
						...
						UE_LOGF(LogConsoleResponse, Display, "Ping success: Reply from %ls time=%.02f ms", *Result.ReplyFrom, CurPing);
					}
					else
					{
						UE_LOGF(LogConsoleResponse, Display, "Ping failure");
					}
				});
			return true;
		}
