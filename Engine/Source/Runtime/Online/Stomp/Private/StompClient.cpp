...
{
	static const FName LoginHeader("login");
	static const FName PasscodeHeader("passcode");
	static const FName HeartbeatHeader("heart-beat");
...
void FStompClient::HandleDisconnectCompleted(bool bIsSuccess, const FString& Error)
{
	if (bIsSuccess)
	{
		UE_LOGF(LogStomp, Verbose, "Successfully disconnected from server");
	}
	else
	{
		UE_LOGF(LogStomp, Warning, "Error when disconnecting from Stomp server: %ls", *Error);
...
{
	static const FName ReceiptHeader("receipt-id");
	static const FName HeartBeatHeader("heart-beat");
	static const FName MessageHeader("message");
	static const FName VersionHeader("version");
	static const FName SessionHeader("session");
	static const FName ServerHeader("server");
	...
	else if (Command == MessageCommand)
	{
		...
		{
			UE_LOGF(LogStomp, Warning, "Received a message from %ls with an unknown or unhandled subscription id %ls", *Message.GetDestination(), *Id);
		}
		else
		{
	...
	else if (Command == ReceiptCommand)
	{
		...
		{
			UE_LOGF(LogStomp, Warning, "Got a receipt with an unknown or unhandled recept id %ls", *ReceiptId);
		}
		else
		{
	...
	{
		UE_LOGF(LogStomp, Error, "Got an unknown command %ls", *Command.ToString());
...
bool FStompClient::Tick(float DeltaTime)
{
	...
	if (IsConnected())
	{
			...
			Now - LastReceivedFrame >= PongInterval * (float)MissedServerPongsBeforeError)
		{
			...
			UE_LOGF(LogStomp, Error, "No Stomp heartbeat for %.1f seconds", (Now - LastReceivedFrame).GetTotalSeconds());
			if (ReceiveBuffer.Num() != 0)
			{
				UE_LOGF(LogStomp, Log, "Stomp: %d bytes pending, received %.1f seconds ago", ReceiveBuffer.Num(), (Now - LastReceivedPacket).GetTotalSeconds());
