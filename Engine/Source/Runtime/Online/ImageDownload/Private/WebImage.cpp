...
FWebImage::FWebImage()
	...
	, DownloadedBrushDrawType(ESlateBrushDrawType::Image)
...
void FWebImage::SetDownloadedBrushDrawType(ESlateBrushDrawType::Type DrawType)
{
	DownloadedBrushDrawType = DrawType;
	if (DownloadedBrush)
	{
		DownloadedBrush->DrawAs = DownloadedBrushDrawType;
	}
}

void FWebImage::SetDownloadedBrushOutlineSettings(FSlateBrushOutlineSettings OutlineSettings)
{
	DownloadedBrushOutlineSettings = MoveTemp(OutlineSettings);
	if (DownloadedBrush)
	{
		DownloadedBrush->OutlineSettings = DownloadedBrushOutlineSettings;
	}
}
...
bool FWebImage::ProcessHttpResponse(const FString& RequestUrl, FHttpResponsePtr HttpResponse)
{
	...
	{
		UE_LOGF(LogImageDownload, Error, "Image Download: Connection Failed. url=%ls", *RequestUrl);
		return false;
	}
	...
	if (StatusCode / 100 != 2)
	{
		...
		UE_LOGF(LogImageDownload, Error, "Image Download: HTTP response %d. url=%ls", StatusCode, *RequestUrl);
		return false;
	}
	...
	if (!ImageWrapperModule.DecompressImage(Content.GetData(), Content.Num(), DownloadedImage))
	{
		...
		UE_LOGF(LogImageDownload, Error, "Image Download: Could not recognize file type of image downloaded from url %ls, server-reported content type: %ls", *RequestUrl, *ContentType);
		return false;
	}
