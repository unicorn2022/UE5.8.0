...
public:
	...
	/**
	 * Get the proxy address from the highest-priority available source:
	 * command-line (-httpproxy=), INI ([HTTP] HttpProxyAddress), or OS proxy settings.
	 * It's to ensure FAndroidPlatformHttp::GetOperatingSystemProxyAddress() is called.
	 */
