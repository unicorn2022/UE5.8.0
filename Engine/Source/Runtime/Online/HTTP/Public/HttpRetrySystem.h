...
namespace FHttpRetrySystem
{
	...
	struct FRetryDomains
	{
		...
		FRetryDomains(TArray<FString>&& InDomains, const int32 InActiveIndex, const FString& InConfigSection, const FString& InConfigKeyName)
			: Domains(MoveTemp(InDomains))
			, ActiveIndex(InActiveIndex)
			, ConfigSection(InConfigSection)
			, ConfigKeyName(InConfigKeyName)
		{}
		...
		/** Config section name that can be used for storing active domain for reusing during next sessions */
		const FString ConfigSection;

		/** Config key name that can be used for storing active domain for reusing during next sessions */
		const FString ConfigKeyName;
	};
	...
	*/
	...
	using FRetryExponentialBackoffCurvePtr = TSharedPtr<FExponentialBackoffCurve, ESPMode::ThreadSafe>;
...
namespace FHttpRetrySystem
{
	...
	protected:
		...
		EHttpFailureReason LastFailureReason = EHttpFailureReason::None;
