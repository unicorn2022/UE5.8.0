...
void FEventLoopHttpThread::StopThread()
{
	...
	if (EventLoop)
	{
		...
		{
			UE_LOGF(LogHttp, Warning, "Still waiting for event loop shutdown. Elapsed time: %f seconds", FPlatformTime::Seconds() - ShutdownStartTime);
		},
		ShutdownWarnTime,
		true /* repeat */);
...
void FEventLoopHttpThread::AddRequest(FHttpRequestCommon* Request)
{
	...
	QUICK_SCOPE_CYCLE_COUNTER(STAT_FEventLoopHttpThread_AddRequest);
...
bool FEventLoopHttpThread::Init()
{
	...
	{
		UE_LOGF(LogHttp, Error, "Failed to initialize the event loop.");
		return false;
	}
