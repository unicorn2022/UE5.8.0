	...
	: bForceSecureConnections(bInForceSecureConnections)
{
	...
	{
		UE_LOGF(LogWinHttp, Verbose, "WinHttp local machine has support for HTTP/2");
	}
	else
	{
		UE_LOGF(LogWinHttp, Verbose, "WinHttp local machine does not support HTTP/2, HTTP/1.1 will be used");
	}
#else // ^^^ UE_HAS_HTTP2_SUPPORT ^^^ // vvv !UE_HAS_HTTP2_SUPPORT vvv
	UE_LOGF(LogWinHttp, Verbose, "UE WinHttp compiled without HTTP/2 support");
	...
	{
		UE_LOGF(LogWinHttp, Verbose, "WinHttp local machine has support for compression");
	}
	else
	{
		UE_LOGF(LogWinHttp, Verbose, "WinHttp local machine does not support for compression");
	}

	float HttpConnectionTimeout = 30.0f;
	float HttpActivityTimeout = 30.0f;
	...
	const FTimespan ConnectionTimeout = FTimespan::FromSeconds(HttpConnectionTimeout > 0 ? HttpConnectionTimeout : 0);
	const FTimespan ResolveTimeout = ConnectionTimeout;
	const FTimespan ReceiveTimeout = FTimespan::FromSeconds(HttpActivityTimeout > 0 ? HttpActivityTimeout : 0);
