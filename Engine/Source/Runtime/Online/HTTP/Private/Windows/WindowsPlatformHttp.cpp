...
void FWindowsPlatformHttp::Init()
{
	...
	{
		UE_LOGF(LogHttp, Warning, "-HTTP=WinInet is no longer valid");
	}
...
{
	FString MimeType = "application/unknown";
...
FString FWindowsPlatformHttp::GetConfiguredProxyAddress()
{
	if (TOptional<FString> ConfigProxy = GetProxyAddressFromConfig())
	{
		return ConfigProxy.GetValue();
	}
	if (TOptional<FString> OsProxy = FWindowsPlatformHttp::GetOperatingSystemProxyAddress())
	{
