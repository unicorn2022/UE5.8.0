...
FString FHttpResponseCommon::GetHeader(const FString& HeaderName) const
{
	...
	if (!bIsReady)
	{
		UE_LOGF(LogHttp, Warning, "Can't get cached header [%ls]. Response still processing. %ls", *HeaderName, *GetURL());
	}
	else
	{
...
const TArray<uint8>& FHttpResponseCommon::GetContent() const
{
	if (!bIsReady)
	{
		UE_LOGF(LogHttp, Warning, "Payload is incomplete. Response still processing for %ls", *GetURL());
	}
	return Payload;
}

TArray<uint8> FHttpResponseCommon::TakeContent()
{
	if (!bIsReady)
	{
		UE_LOGF(LogHttp, Warning, "Payload is incomplete. Response still processing for %ls", *GetURL());
