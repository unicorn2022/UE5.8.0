...
TAutoConsoleVariable<bool> CVarHttpLogJsonContentOnly(
	TEXT("http.LogJsonContentOnly"),
	true,
	TEXT("When log request/response payload, log json content only"),
...
void FHttpRequestCommon::SetHeader(const FString& HeaderName, const FString& HeaderValue)
{
	if (CompletionStatus == EHttpRequestStatus::Processing)
	{
		UE_LOGF(LogHttp, Warning, "FHttpRequestCommon::SetHeader() - attempted to set header on a request that is inflight");
		return;
	}
...
FString FHttpRequestCommon::GetHeader(const FString& HeaderName) const
{
	FString Result;
...
bool FHttpRequestCommon::PreCheck() const
{
	...
	{
		UE_LOGF(LogHttp, Verbose, "Http disabled. Skipping request. url=%ls", *GetURL());
		return false;
	}
	...
	if (CompletionStatus == EHttpRequestStatus::Processing)
	{
		UE_LOGF(LogHttp, Warning, "ProcessRequest failed. Still processing last request.");
		return false;
	}
	...
	if (GetURL().IsEmpty())
	{
		UE_LOGF(LogHttp, Warning, "ProcessRequest failed. No URL was specified.");
		return false;
	}

	if (GetVerb().IsEmpty())
	{
		UE_LOGF(LogHttp, Warning, "ProcessRequest failed. No Verb was specified.");
		return false;
	}

	if (!FHttpModule::Get().GetHttpManager().IsDomainAllowed(GetURL()))
	{
		UE_LOGF(LogHttp, Warning, "ProcessRequest failed. URL '%ls' is not using an allowed domain.", *GetURL());
		return false;
	}

	if (bTimedOut)
	{
		UE_LOGF(LogHttp, Warning, "ProcessRequest failed. Request with URL '%ls' already timed out.", *GetURL());
		return false;
	}
...
bool FHttpRequestCommon::PreProcess()
{
	...
	UE_LOGF(LogHttp, Verbose, "%p: Verb='%ls' URL='%ls'", this, *GetVerb(), *GetURL());
...
void FHttpRequestCommon::HandleRequestSucceed()
{
	...
	FHttpModule::Get().GetHttpManager().RecordStatRequestSucceeded();
}
...
void FHttpRequestCommon::HandleRequestFailed()
{
	...
	FHttpModule::Get().GetHttpManager().RecordStatRequestFailed(FailureReason);
...
{
	UE_CLOGF(FailureReason != EHttpFailureReason::None, LogHttp, Warning, "FailureReason had been set to %ls, now setting to %ls", LexToString(FailureReason), LexToString(InFailureReason));
...
void FHttpRequestCommon::CancelRequest()
{
	...
	UE_LOGF(LogHttp, Verbose, "HTTP request canceled. URL=%ls", *GetURL());
...
void FHttpRequestCommon::StartActivityTimeoutTimerBy(double DelayToTrigger)
{
	...
	{
		UE_LOGF(LogHttp, Warning, "Request %p already started activity timeout timer", this);
		return;
	}
...
void FHttpRequestCommon::OnActivityTimeoutTimerTaskTrigger()
{
	...
	{
		UE_LOGF(LogHttp, Warning, "Request %p had finished when activity timeout timer trigger at [%ls]", this, *FDateTime::Now().ToString(TEXT("%H:%M:%S:%s")));
		return;
	}
	...
	{
		// Check back later
		UE_LOGF(LogHttp, VeryVerbose, "Request %p check response timeout at [%ls], will check again in %.5f seconds", this, *FDateTime::Now().ToString(TEXT("%H:%M:%S:%s")), ActivityTimeoutAt - FPlatformTime::Seconds());
	...
	AbortRequest();
	UE_LOGF(LogHttp, Log, "Request [%ls] timed out at [%ls] because of no responding for %0.2f seconds", *GetURL(), *FDateTime::Now().ToString(TEXT("%H:%M:%S:%s")), GetActivityTimeoutOrDefault());
}
...
void FHttpRequestCommon::ResetActivityTimeoutTimer(FStringView Reason)
{
	...
	UE_LOGF(LogHttp, VeryVerbose, "Request [%p] reset response timeout timer at %ls: %ls", this, *FDateTime::Now().ToString(TEXT("%H:%M:%S:%s")), Reason.GetData());
}
...
void FHttpRequestCommon::OnTotalTimeoutTimerTaskTrigger()
{
	...
	QUICK_SCOPE_CYCLE_COUNTER(STAT_FHttpRequestCommon_AbortRequest);
	UE_LOGF(LogHttp, Warning, "HTTP request timed out after %0.2f seconds URL=%ls", GetTimeoutOrDefault(), *GetURL());
...
bool FHttpRequestCommon::StartThreadedRequest()
{
	LogRequest();
	return true;
}
...
double FHttpRequestCommon::GetTimeStartedWaitingInQueue() const
{
	check(TimeStartedWaitingInQueue != 0.0);
...
void FHttpRequestCommon::SetURL(const FString& InURL)
{
	if (CompletionStatus == EHttpRequestStatus::Processing)
	{
		UE_LOGF(LogHttp, Warning, "FHttpRequestCommon::SetURL() - attempted to set url on a request that is inflight");
		return;
	}
...
{
	UE_LOGF(LogHttp, Verbose, "FHttpRequestCommon::SetContentAsStreamedFileDefaultImpl() - %ls", *Filename);

	if (CompletionStatus == EHttpRequestStatus::Processing)
	{
		UE_LOGF(LogHttp, Warning, "FHttpRequestCommon::SetContentAsStreamedFileDefaultImpl() - attempted to set content on a request that is inflight");
		return false;
	}
...
bool FHttpRequestCommon::OpenRequestPayloadDefaultImpl()
{
	...
	{
		UE_LOGF(LogHttp, Warning, "An HTTP Get request cannot contain a payload.");
		return false;
	}
...
void FHttpRequestCommon::LogRequest() const
{
	bool bShouldLogDetails = FHttpModule::Get().GetHttpManager().ShouldInspect(GetURL());
	UE_HTTP_LOG_AS_WARNING_IF(bShouldLogDetails, TEXT("%p %s %s started from http thread"), this, *GetVerb(), *GetURL());

	LogHeaders(RequestHeaders, bShouldLogDetails, false/*bIsResponse*/);
	LogContent(bShouldLogDetails, *this, false/*bIsResponse*/);
}

void FHttpRequestCommon::LogResponse(const TSharedPtr<FHttpResponseCommon>& InResponse) const
{
	bool bShouldLogDetails = FHttpModule::Get().GetHttpManager().ShouldInspect(GetURL());
	UE_HTTP_LOG_AS_WARNING_IF(bShouldLogDetails, TEXT("%p %s %s completed with code %d after %.2fs"), this, *GetVerb(), *GetURL(), InResponse->GetResponseCode(), ElapsedTime);

	LogHeaders(InResponse->Headers, bShouldLogDetails, true/*bIsResponse*/);
	LogContent(bShouldLogDetails, *InResponse, true/*bIsResponse*/);
}

void FHttpRequestCommon::LogHeaders(const TMap<FString, FString>& Headers, bool bShouldLogAsWarning, bool bIsResponse) const
{
	for (const TPair<FString, FString>& HeaderPair : Headers)
	{
		bool bIsSensitiveData = 
			HeaderPair.Key.Contains(FHttpConstants::HEADER_AUTHORIZATION, ESearchCase::IgnoreCase) || 
			HeaderPair.Key.Contains(TEXT("Set-Cookie"), ESearchCase::IgnoreCase);

		UE_HTTP_LOG_AS_WARNING_IF(bShouldLogAsWarning, TEXT("%p %s Header %s:%s"), 
			this,
			bIsResponse ? TEXT("Response") : TEXT("Request"),
			*HeaderPair.Key,
			bIsSensitiveData ? TEXT("[REDACTED]") : *HeaderPair.Value);
	}
}

void FHttpRequestCommon::LogContent(bool bShouldLogDetails, const IHttpBase& HttpBase, bool bIsResponse) const
{
	...
	if (CVarHttpLogJsonContentOnly.GetValueOnAnyThread())
	{
		// Use Contains instead of Compare, for request/response header like "Content-Type: application/json;charset=utf-8"
		bool bIsContentTypeJson = HttpBase.GetHeader(FHttpConstants::HEADER_CONTENT_TYPE).Contains(FHttpConstants::MEDIATYPE_JSON, ESearchCase::IgnoreCase);
		if (!bIsContentTypeJson)
			return;
	}

	const TArray<uint8>& Content = HttpBase.GetContent();
	if (Content.IsEmpty())
	{
		return;
	}

	UE_LOGFMT(LogHttp, Warning, "{0} Content({1}):", bIsResponse ? TEXT("Response") : TEXT("Request"), FText::AsMemory(HttpBase.GetContentLength(), EMemoryUnitStandard::IEC).ToString());
	...
	while (StartPos < ResponseStringView.Len())
	{
		...
		{
			EndPos = ResponseStringView.Len();
		}

		int32 LengthToLog = EndPos - StartPos;
		while (LengthToLog > 0 && ResponseStringView[StartPos + LengthToLog - 1] == '\r')
		{
			LengthToLog = LengthToLog - 1;
		}

		if (LengthToLog > 0)
		{
			FUtf8StringView Line(&ResponseStringView[StartPos], LengthToLog);
