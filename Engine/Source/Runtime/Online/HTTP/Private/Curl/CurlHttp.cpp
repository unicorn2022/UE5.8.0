...
TAutoConsoleVariable<bool> CVarHttpCurlResetRequestCompletedOnRetryEnabled(
	TEXT("http.CurlResetRequestCompletedOnRetryEnabled"),
	true,
	TEXT("When enabled, bCurlRequestCompleted is reset to false in ClearInCaseOfRetry so the request can be reused for retrying."),
	ECVF_SaveForNextBoot
);

TAutoConsoleVariable<bool> CVarHttpCurlDeferEasyHandleLifetime(
	TEXT("http.CurlDeferEasyHandleLifetime"),
	true,
	TEXT("When enabled, the curl easy handle is created in SetupRequest and destroyed in CleanupRequest rather than in the constructor/destructor. This prevents the handle from outliving the curl library during shutdown."),
	ECVF_SaveForNextBoot
);
...
bool FCurlHttpRequest::InitEasyHandle()
{
	...
	if (!EasyHandle)
	{
		UE_LOGF(LogHttp, Error, "%p: Failed to initialize curl easy handle", this);
		return false;
	}
	...
	if (!CVarHttpReuseConnectionEnabled.GetValueOnAnyThread())
	{
	...
	static const char* const CertBundlePath = []() -> const char* {
		...
		for (const char* BundlePath : KnownBundlePaths)
		{
			...
			UE_LOGF(LogHttp, Log, " Libcurl: checking if '%ls' exists", *FileName);
...
void FCurlHttpRequest::DestroyEasyHandle()
{
	...
	{
		// Curl library has already been shut down (e.g. user code holds a reference to the request shared ptr).
		// Curl handles cannot be safely cleaned up; memory is leaked. Flush all HTTP requests before shutting
		// down the HTTP module to avoid this.
		UE_LOGF(LogHttp, Warning, "Curl library has already been shut down. Curl handles will be leaked. Flush all HTTP requests before shutting down the HTTP module to avoid this.");
	}
}
...
void FCurlHttpRequest::SetVerb(const FString& InVerb)
{
	...
	{
		UE_LOGF(LogHttp, Warning, "FCurlHttpRequest::SetVerb() - attempted to set verb on a request that is inflight");
		return;
	}
...
void FCurlHttpRequest::SetOption(const FName Option, const FString& OptionValue)
{
	if (CompletionStatus == EHttpRequestStatus::Processing)
	{
		UE_LOGF(LogHttp, Warning, "FCurlHttpRequest::SetOption() - attempted to set option on a request that is inflight");
		return;
	}
...
void FCurlHttpRequest::SetContent(TArray<uint8>&& ContentPayload)
{
	if (CompletionStatus == EHttpRequestStatus::Processing)
	{
		UE_LOGF(LogHttp, Warning, "FCurlHttpRequest::SetContent() - attempted to set content on a request that is inflight");
		return;
	}
...
void FCurlHttpRequest::SetContentAsString(const FString& ContentString)
{
	if (CompletionStatus == EHttpRequestStatus::Processing)
	{
		UE_LOGF(LogHttp, Warning, "FCurlHttpRequest::SetContentAsString() - attempted to set content on a request that is inflight");
		return;
	}
...
{
	UE_LOGF(LogHttp, Verbose, "FCurlHttpRequest::SetContentFromStream() - %ls", *Stream->GetArchiveName());

	if (CompletionStatus == EHttpRequestStatus::Processing)
	{
		UE_LOGF(LogHttp, Warning, "FCurlHttpRequest::SetContentFromStream() - attempted to set content on a request that is inflight");
		return false;
	}
...
void FCurlHttpRequest::AppendToHeader(const FString& HeaderName, const FString& AdditionalHeaderValue)
{
	if (CompletionStatus == EHttpRequestStatus::Processing)
	{
		UE_LOGF(LogHttp, Warning, "FCurlHttpRequest::AppendToHeader() - attempted to append to header on a request that is inflight");
		return;
	}
...
size_t FCurlHttpRequest::ReceiveResponseHeaderCallback(void* Ptr, size_t SizeInBlocks, size_t BlockSizeInBytes)
{
	...
	{
		UE_LOGF(LogHttp, Warning, "%p: Could not process response header for request - header size (%d) is invalid.", this, HeaderSize);
	}
...
size_t FCurlHttpRequest::ReceiveResponseBodyCallback(void* Ptr, size_t SizeInBlocks, size_t BlockSizeInBytes)
{
	...
	const size_t SizeToDownload = SizeInBlocks * BlockSizeInBytes;
	const int64 TotalRead = TotalBytesRead.load();
	const size_t CurrentBytesRead = TotalRead > 0 ? TotalRead : 0;
	TSharedPtr<FCurlHttpResponse> Response = StaticCastSharedPtr<FCurlHttpResponse>(ResponseCommon);
	UE_LOGF(LogHttp, Verbose, "%p: ReceiveResponseBodyCallback: %llu bytes out of %llu received. (SizeInBlocks=%llu, BlockSizeInBytes=%llu, TotalBytesRead=%llu, Response->GetContentLength()=%llu, SizeToDownload=%llu (<-this will get returned from the callback))",
		this, CurrentBytesRead + SizeToDownload, Response->GetContentLength(),
		SizeInBlocks, BlockSizeInBytes, CurrentBytesRead, Response->GetContentLength(), SizeToDownload);
	...
	{
		const size_t NewTotalSize = CurrentBytesRead + SizeToDownload;
		const uint64 ContentLength = Response->GetContentLength();
		if (NewTotalSize > TNumericLimits<int32>::Max())
		{
			UE_LOGF(LogHttp, Warning, "%p: ReceiveResponseBodyCallback: Payload size exceeds int32 limits: %llu bytes, consider using SetResponseBodyReceiveStream instead", this, NewTotalSize);
			SetFailureReason(EHttpFailureReason::ResponseTooLarge);
			return 0;
		}

		// First time chunk added, reserve the capacity, if we know the content length, avoiding relocations
		if (Response->Payload.IsEmpty() && ContentLength > 0 && ContentLength <= TNumericLimits<int32>::Max())
		{
			Response->Payload.Reserve(static_cast<int32>(ContentLength));
		}

		// Grow the array if needed
		if (NewTotalSize > static_cast<size_t>(Response->Payload.Num()))
		{
			const size_t PayloadMax = Response->Payload.Max();
			if (ContentLength == 0 && NewTotalSize > PayloadMax)
			{
				constexpr size_t SizeMultiplier = 2;
				const size_t NewReserve = FMath::Max<size_t>(NewTotalSize, PayloadMax * SizeMultiplier);
				if (NewReserve > TNumericLimits<int32>::Max())
				{
					UE_LOGF(LogHttp, Warning, "%p: ReceiveResponseBodyCallback: Payload size exceeds int32 limits: TotalSize = %llu bytes, NewTotalSize = %llu, consider using SetResponseBodyReceiveStream instead", this, NewTotalSize, NewReserve);
					SetFailureReason(EHttpFailureReason::ResponseTooLarge);
					return 0;
				}
				Response->Payload.Reserve(NewReserve);
			}

			Response->Payload.SetNumUninitialized(static_cast<int32>(NewTotalSize));
		}

		// Finally, copy the data
		uint8* Dest = Response->Payload.GetData() + CurrentBytesRead;
		FMemory::Memcpy(Dest, Ptr, SizeToDownload);
...
size_t FCurlHttpRequest::UploadCallback(void* Ptr, size_t SizeInBlocks, size_t BlockSizeInBytes)
{
	...
	UE_LOGF(LogHttp, Verbose, "%p: UploadCallback: %llu bytes out of %llu sent (%llu bytes total sent). (SizeInBlocks=%llu, BlockSizeInBytes=%llu, SizeToSendThisTime=%llu (<-this will get returned from the callback))",
...
int FCurlHttpRequest::SeekCallback(curl_off_t Offset, int Origin)
{
	...
	{
		UE_LOGF(LogHttp, Log, "%p: SeekCallback: Resetting to the beginning. We had uploaded %llu bytes", this, BytesSent.load());
		BytesSent.store(0);
		bIsRequestPayloadSeekable = false; // Do not attempt to re-seek
		return CURL_SEEKFUNC_OK;
	}
	UE_LOGF(LogHttp, Warning, "%p: SeekCallback: Failed to seek to Offset=%lld, Origin=%d %ls", 
		this,
		(int64)(Offset),
		Origin, 
		...
		case CURLINFO_TEXT:
			{
				...
				UE_LOGF(LogHttp, VeryVerbose, "%p: '%ls'", this, *DebugText);
		...
		case CURLINFO_HEADER_OUT:
			{
				if (UE_LOG_ACTIVE(LogHttp, VeryVerbose))
				{
					...
					if (bIsPrintable)
					{
						...
						DebugText.ReplaceInline(TEXT("\n"), TEXT(""), ESearchCase::CaseSensitive);
						DebugText.ReplaceInline(TEXT("\r"), TEXT(""), ESearchCase::CaseSensitive);
						UE_LOGF(LogHttp, VeryVerbose, "%p: Sent header (%d bytes) - %ls", this, RecalculatedSize, *DebugText);
					}
					else
					{
						UE_LOGF(LogHttp, VeryVerbose, "%p: Sent header (%d bytes) - contains binary data", this, RecalculatedSize);
					}
					}
			}
			break;
...
bool FCurlHttpRequest::SetupRequest()
{
	...
	UE_LOGF(LogHttp, Verbose, "%p: Custom headers are %ls", this, RequestHeaders.Num() ? TEXT("present") : TEXT("NOT present"));
	UE_LOGF(LogHttp, Verbose, "%p: Payload size=%llu", this, RequestPayload->GetContentLength());
...
{
	QUICK_SCOPE_CYCLE_COUNTER(STAT_FCurlHttpRequest_StartThreadedRequest);
	{
		QUICK_SCOPE_CYCLE_COUNTER(STAT_FCurlHttpRequest_StartThreadedRequest_SLIST_FREE_HEADERS);
		curl_slist_free_all(HeaderList);
		HeaderList = nullptr;
	}

	{
		QUICK_SCOPE_CYCLE_COUNTER(STAT_FCurlHttpRequest_StartThreadedRequest_EASY_SETOPT);

		curl_easy_setopt(EasyHandle, CURLOPT_URL, TCHAR_TO_ANSI(*GetURL()));

		SetupOptionUnixSocketPath();
		SetupOptionHttpVersion();
		SetupOptionEnableCookies();
		...
		{
			QUICK_SCOPE_CYCLE_COUNTER(STAT_FCurlHttpRequest_StartThreadedRequest_SLIST_APPEND_HEADERS);

			for (const TPair<FString, FString>& HeaderPair : RequestHeaders)
			{
				FString FullHeader = FString::Format(TEXT("{0}: {1}"), { HeaderPair.Key, HeaderPair.Value });
				curl_slist* NewHeaderList = curl_slist_append(HeaderList, TCHAR_TO_UTF8(ToCStr(FullHeader)));
				if (!NewHeaderList)
				{
					UE_LOGF(LogHttp, Warning, "Failed to append header '%ls'", *HeaderPair.Key);
					continue;
				}
...
#if !WITH_CURL_XCURL
	{
		...
		QUICK_SCOPE_CYCLE_COUNTER(STAT_FCurlHttpRequest_StartThreadedRequest_EASY_CURLOPT_SHARE);
	...
	return FHttpRequestCommon::StartThreadedRequest();
}
...
void FCurlHttpRequest::SetupOptionHttpVersion()
{
	...
	if (!HttpVersion.IsEmpty())
	{
		...
		{
			UE_LOGF(LogHttp, Warning, "Unsupported http version %ls", *HttpVersion);
		}
	}
}

void FCurlHttpRequest::SetupOptionEnableCookies()
{
	const FString EnableCookies = GetOption(HttpRequestOptions::EnableCookies);
	if (EnableCookies.ToBool())
	{
...
bool FCurlHttpRequest::ProcessRequest()
{
	...
	UE_LOGF(LogHttp, Verbose, "%p: request (easy handle:%p) has been added to threaded queue for processing", this, EasyHandle);
	return true;
}
...
void FCurlHttpRequest::ClearInCaseOfRetry()
{
	...
	TotalBytesRead = 0;
	bAnyHttpActivity = false;
	if (CVarHttpCurlResetRequestCompletedOnRetryEnabled.GetValueOnAnyThread())
	{
		bCurlRequestCompleted = false;
	}
...
void FCurlHttpRequest::FinishRequest()
{
	...
	if (bCurlRequestCompleted)
	{
		if (Response.IsValid())
		{
			...
			{
				UE_LOGF(LogHttp, Warning, "%p: invalid HTTP response code received. URL: %ls, HTTP code: %d, content length: %llu, actual payload size: %llu",
...
void FCurlHttpRequest::LogCurlErrors()
{
	if (CurlAddToMultiResult != CURLM_OK)
	{
		UE_LOGF(LogHttp, Warning, "%p: request failed, libcurl multi error: %d (%ls)", this, (int32)CurlAddToMultiResult, ANSI_TO_TCHAR(curl_multi_strerror(CurlAddToMultiResult)));
	}
	else if (CurlCompletionResult != CURLE_OK)
	{
		UE_LOGF(LogHttp, Warning, "%p: request failed, libcurl error: %d (%ls)", this, (int32)CurlCompletionResult, ANSI_TO_TCHAR(curl_easy_strerror(CurlCompletionResult)));
	}
	...
	if (!bAborted)
	{
		...
		{
			int RelativeIndex = (LeastRecentlyCachedInfoMessageIndex + i) % InfoMessageCache.Num();
			if (!InfoMessageCache[RelativeIndex].IsEmpty())
			{
				UE_LOGF(LogHttp, Warning, "%p: libcurl info message cache %d (%ls)", this, RelativeIndex, *(InfoMessageCache[RelativeIndex]));
			}
		}
	}
}
...
TArray<FString> FCurlHttpResponse::GetAllHeaders() const
{
	...
	if (!bIsReady)
	{
		UE_LOGF(LogHttp, Warning, "Can't get cached headers. Response still processing. %ls", *GetURL());
	}
	else
	{
