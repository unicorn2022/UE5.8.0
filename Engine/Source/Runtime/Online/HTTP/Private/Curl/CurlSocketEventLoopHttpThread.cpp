...
bool FCurlSocketEventLoopHttpThread::StartThreadedRequest(FHttpRequestCommon* Request)
{
	...
	if (!FEventLoopHttpThread::StartThreadedRequest(Request))
	{
		UE_LOGF(LogHttp, Warning, "Could not set libcurl options for easy handle, processing HTTP request failed. Increase verbosity for additional information.");
		return false;
	}
	...
	{
		UE_LOGF(LogHttp, Warning, "Failed to add easy handle %p to multi handle with code %d", EasyHandle, (int)AddResult);
		return false;
	}
...
int FCurlSocketEventLoopHttpThread::HandleCurlSocketCallback(CURL* CurlE, curl_socket_t Socket, int EventFlags, void* SocketData)
{
	...
	UE_LOGF(LogHttp, VeryVerbose, "[FCurlSocketEventLoopHttpThread::HandleCurlSocketCallback] Socket: %p, EventFlags: 0x%08X, SocketData: %p", reinterpret_cast<void*>(Socket), EventFlags, SocketData);
...
int FCurlSocketEventLoopHttpThread::HandleCurlTimerCallback(CURLM* CurlM, long TimeoutMS)
{
	...
	UE_LOGF(LogHttp, VeryVerbose, "[FCurlSocketEventLoopHttpThread::HandleCurlTimerCallback] Timeout: %ld", TimeoutMS);
	...
	if (TimeoutMS >= 0)
	{
		UE_LOGF(LogHttp, VeryVerbose, "[FCurlSocketEventLoopHttpThread::HandleCurlTimerCallback] Set timer for: %ld ms", TimeoutMS);
...
void FCurlSocketEventLoopHttpThread::ProcessCurlSocketActions(curl_socket_t Socket, int EventFlags)
{
	...
	UE_LOGF(LogHttp, VeryVerbose, "[FCurlSocketEventLoopHttpThread::ProcessCurlSocketActions] Socket: %p CurlEvents: 0x%08X", reinterpret_cast<void*>(Socket), EventFlags);
...
void FCurlSocketEventLoopHttpThread::ProcessCurlSocketEvent(curl_socket_t Socket, UE::EventLoop::ESocketIoRequestStatus Status, UE::EventLoop::EIOFlags Flags)
{
	...
	{
		UE_LOGF(LogHttp, Warning, "Socket event request failed. Socket %p, Status: %ls", reinterpret_cast<void*>(Socket), *LexToString(Status));
		ProcessCurlSocketActions(CURL_SOCKET_TIMEOUT, 0);
	}
	else
	{
	...
	for (;;)
	{
		...
		if (Message->msg == CURLMSG_DONE)
		{
			...
			if (Request)
			{
				...
				UE_LOGF(LogHttp, Verbose, "Request %p (easy handle:%p) has completed (code:%d) and has been marked as such", CurlRequest, CompletedHandle, (int32)Message->data.result);
			...
			{
				UE_LOGF(LogHttp, Warning, "Could not find mapping for completed request (easy handle: %p)", CompletedHandle);
			}
		}
	}
