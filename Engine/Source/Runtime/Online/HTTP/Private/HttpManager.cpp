...
TAutoConsoleVariable<FString> CVarHttpUrlPatternsToInspect(
	TEXT("http.UrlPatternsToInspect"),
	TEXT(""),
...
TAutoConsoleVariable<bool> CVarHttpSkipShutdownFlush(
	TEXT("http.SkipShutdownFlush"),
	false,
	TEXT("When enabled, shutdown does not call flush to avoid waiting past allowed shutdown timing")
);

TAutoConsoleVariable<bool> CVarHttpFlushTriggerFatalLogDuringCompleteDelegateEnabled(
	TEXT("http.FlushTriggerFatalLogDuringCompleteDelegateEnabled"),
	true,
	TEXT("When enabled, trigger fatal log if flush during http complete delegate")
);
...
void FHttpManager::Initialize()
{
	if (!Thread)
	{
		...
		if (CVarHttpEventLoopEnableChance.GetValueOnGameThread() < 100 || !bUseEventLoop)
		{
			UE_LOGF(LogHttp, Error, "Legacy http thread has been deprecated, please always use event loop thread instead.");
		}
	...
	UpdateUrlPatternsToInspect(CVarHttpUrlPatternsToInspect.AsVariable());
	CVarHttpUrlPatternsToInspect.AsVariable()->OnChangedDelegate().AddRaw(this, &FHttpManager::UpdateUrlPatternsToInspect);
...
{
	CVarHttpUrlPatternsToInspect.AsVariable()->OnChangedDelegate().Clear();
...

	{
		...
		UE_CLOGF(ShouldOutputHttpWarnings() && Requests.Num(), LogHttp, Warning, "[FHttpManager::Shutdown] Unbinding delegates for %d outstanding Http Requests:", Requests.Num());
		...
		for (TArray<FHttpRequestRef>::TIterator It(Requests); It; ++It)
		{
			...
			// Don't emit these tracking logs in commandlet runs. Build system traps warnings during cook, and these are not truly fatal, but useful for tracking down shutdown issues.
			UE_CLOGF(ShouldOutputHttpWarnings(), LogHttp, Warning, "	verb=[%ls] url=[%ls] refs=[%d] status=%ls", *Request->GetVerb(), *Request->GetURL(), Request.GetSharedReferenceCount(), EHttpRequestStatus::ToString(Request->GetStatus()));
		}
	}
	...
	// Flush all requests
	if (!CVarHttpSkipShutdownFlush.GetValueOnAnyThread())
	{
...
void FHttpManager::ReloadFlushTimeLimits()
{
	...
	for (EHttpFlushReason Reason : TEnumRange<EHttpFlushReason>())
	{
		...
		case EHttpFlushReason::Shutdown:
			...
			{
				UE_CLOGF(!IsRunningCommandlet(), LogHttp, Warning, "Soft limit[%.02f] is higher than the hard limit set[%.02f] in file [%ls]. Please change the soft limit to a value lower than the hard limit for Flush to work correctly. - 1 is unlimited and therefore the highest possible value.", static_cast<float>(SoftLimitSeconds), static_cast<float>(HardLimitSeconds), *GEngineIni);
...
static void CheckNoDuplicatedProtocol(FStringView UrlStringView)
{
	...
	if (FoundIndex != INDEX_NONE)
	{
		...
		UE_CLOGF(Domain.Contains(Delimiter), LogHttp, Warning, "Duplicated delimiter '://' found in url %.*ls, is that intended?", UrlStringView.Len(), UrlStringView.GetData());
	}
}
...
void FHttpManager::Flush(EHttpFlushReason FlushReason)
{
	...
	if (IsCurrentThreadCompletingRequest())
	{
		UE_CLOGFMT(CVarHttpFlushTriggerFatalLogDuringCompleteDelegateEnabled.GetValueOnAnyThread(), LogHttp, Fatal, "It's not supported to call Flush during http request complete delegate!");
		return;
	}
	...
	UE_CLOGF(!IsRunningCommandlet(), LogHttp, Verbose, "[FHttpManager::Flush] FlushReason [%ls] FlushTimeSoftLimitSeconds [%.3fs] FlushTimeHardLimitSeconds [%.3fs] SecondsToSleepForOutstandingThreadedRequests [%.3fs]", LexToString(FlushReason), FlushTimeSoftLimitSeconds, FlushTimeHardLimitSeconds, SecondsToSleepForOutstandingThreadedRequests);
	...
	UE_CLOGF(!IsRunningCommandlet() && RequestsNum, LogHttp, Verbose, "[FHttpManager::Flush] Cleanup starts for %d outstanding Http Requests.", RequestsNum);
	...
	while (RequestsNum > 0 && (FlushTimeHardLimitSeconds < 0 || (AppTime - BeginWaitTime < FlushTimeHardLimitSeconds)))
	{
		...
		{
			// Don't emit these tracking logs in commandlet runs. Build system traps warnings during cook, and these are not truly fatal, but useful for tracking down shutdown issues.
			UE_CLOGF(ShouldOutputHttpWarnings(), LogHttp, Warning, "[FHttpManager::Flush] FlushTimeSoftLimitSeconds [%.3fs] exceeded. Cancelling %d outstanding HTTP requests:", FlushTimeSoftLimitSeconds, RequestsNum);
		...
		if (RequestsNum > 0)
		{
			if (Thread)
			{
				if (Thread->NeedsSingleThreadTick())
				{
					...
					{
						// Don't emit these tracking logs in commandlet runs. Build system traps warnings during cook, and these are not truly fatal, but useful for tracking down shutdown issues.
						UE_CLOGF(ShouldOutputHttpWarnings(), LogHttp, Warning, "	Ticking HTTPThread for %d outstanding Http requests.", RequestsNum);
				...
				{
					// Don't emit these tracking logs in commandlet runs. Build system traps warnings during cook, and these are not truly fatal, but useful for tracking down shutdown issues.
					UE_CLOGF(ShouldOutputHttpWarnings(), LogHttp, Warning, "	Sleeping %.3fs to wait for %d outstanding Http Requests.", SecondsToSleepForOutstandingThreadedRequests, RequestsNum);
	...
	UE_CLOGF(!IsRunningCommandlet(), LogHttp, Verbose, "[FHttpManager::Flush] Cleanup ended after %.3fs. %d outstanding Http Requests.", AppTime - BeginWaitTime, RequestsNum);

	// Don't emit these tracking logs in commandlet runs. Build system traps warnings during cook, and these are not truly fatal, but useful for tracking down shutdown issues.
	if (RequestsNum > 0 && (FlushTimeHardLimitSeconds > 0 && (AppTime - BeginWaitTime > FlushTimeHardLimitSeconds)) && ShouldOutputHttpWarnings())
	{
		UE_LOGF(LogHttp, Warning, "[FHttpManager::Flush] FlushTimeHardLimitSeconds [%.3fs] exceeded. The following requests are being abandoned without being flushed:", FlushTimeHardLimitSeconds);
		...
		for (TArray<FHttpRequestRef>::TIterator It(Requests); It; ++It)
		{
			...
			UE_LOGF(LogHttp, Warning, "	verb=[%ls] url=[%ls] refs=[%d] status=%ls", *Request->GetVerb(), *Request->GetURL(), Request.GetSharedReferenceCount(), EHttpRequestStatus::ToString(Request->GetStatus()));
		}
	}
...
{
	{
		QUICK_SCOPE_CYCLE_COUNTER(STAT_FHttpManager_AddRequest);
		FScopeLock ScopeLock(&RequestLock);
		UE_CLOGF(bFlushing, LogHttp, Warning, "Adding request %ls to http manager while flushing", *Request->GetURL());
...
void FHttpManager::BroadcastHttpRequestCompleted(const FHttpRequestRef& Request)
{
	...
	if (Response.IsValid())
	{
		...
		const int64 TotalDuration = FMath::Max<int64>(0, HttpStatsHistory.TotalDuration);
		const int64 DurationDivisor = TotalDuration + 1;
		HttpStats.BandwidthMbps = (HttpStatsHistory.TotalDownloadedBytes * 8) / DurationDivisor / 1000;
		if (FHttpStatsHistory::HttpHistoryCount > 0)
		{
			HttpStats.HttpDurationMsAvg = TotalDuration / FHttpStatsHistory::HttpHistoryCount;
			// Increment index
			HttpStatsHistory.HistoryIndex = (HttpStatsHistory.HistoryIndex + 1) % FHttpStatsHistory::HttpHistoryCount;
		}
		else
		{
			HttpStats.HttpDurationMsAvg = 0;
		}
	}
...
void FHttpManager::RecordStatRequestSucceeded()
{
	++HttpStats.RequestResults[static_cast<uint8>(EHttpFailureReason::None)];
}

void FHttpManager::RecordStatRequestFailed(EHttpFailureReason Reason)
{
	const uint8 Index = static_cast<uint8>(Reason);
	if (Index < static_cast<uint8>(EHttpFailureReason::Count))
	{
		++HttpStats.RequestResults[Index];
	}
}
...
void FHttpManager::UpdateUrlPatternsToInspect(IConsoleVariable* CVar)
{
	const FScopeLock CacheLock(&UrlPatternsToInspectCriticalSection);
	const FString UrlPatternsToInspectStr = CVar->AsVariable()->GetString();
	UrlPatternsToInspectStr.ParseIntoArray(UrlPatternsToInspect, TEXT(" "));
}

bool FHttpManager::ShouldInspect(FStringView Url) const
{
	const FScopeLock CacheLock(&UrlPatternsToInspectCriticalSection);
	for (const FString& UrlPatternToInspect : UrlPatternsToInspect)
	{
		if (Url.Contains(UrlPatternToInspect))
		{
			return true;
		}
	}
