...
static bool ValidateCertificatePublicKeyPinningApple(NSURLAuthenticationChallenge *challenge)
{
	...
	{
		UE_LOGF(LogHttp, Error, "failed certificate pinning validation: could not parse parameters during certificate pinning evaluation");
		return false;
	}

	if (!SecTrustEvaluateWithError(RemoteTrust, nil))
	{
		UE_LOGF(LogHttp, Error, "failed certificate pinning validation: default certificate trust evaluation failed for domain '%ls'", *RemoteHost);
		return false;
	}
	...
	if (Certificates == nil)
	{
		UE_LOGF(LogHttp, Error, "No certificate could be copied in the certificate chain used to evaluate trust.");
		return false;
	}
	...
	for (int i = 0; i < CertificateCount; ++i)
	{
		...
		TCFRef<CFStringRef> Summary = SecCertificateCopySubjectSummary(Cert);
		UE_LOGF(LogHttp, VeryVerbose, "inspecting certificate. Summary: %ls ", *AppleGetCertificateSummary(Cert));
		...
		if (!CertPubKeyData)
		{
			UE_LOGF(LogHttp, Warning, "could not extract public key from certificate %i for domain '%ls'; skipping!", i, *RemoteHost);
			continue;
		}
		...
		if ([CertPubKeyType isEqualToString: (NSString *)kSecAttrKeyTypeRSA])
		{
			...
			{
				case 2048:
					UE_LOGF(LogHttp, VeryVerbose, "found 2048 bit RSA pubkey");
					CertPubKeyASN1Header = (char *)rsa2048Asn1Header;
					CertPubKeyASN1HeaderSize = sizeof(rsa2048Asn1Header);
					break;
				case 4096:
					UE_LOGF(LogHttp, VeryVerbose, "found 4096 bit RSA pubkey");
					CertPubKeyASN1Header = (char *)rsa4096Asn1Header;
					CertPubKeyASN1HeaderSize = sizeof(rsa4096Asn1Header);
					break;
				default:
					UE_LOGF(LogHttp, Log, "unsupported RSA key length %i for certificate %i for domain '%ls'; skipping!", [CertPubKeySize integerValue], i, *RemoteHost);
					continue;
			}
		}
		else if ([CertPubKeyType isEqualToString: (NSString *)kSecAttrKeyTypeECSECPrimeRandom])
		{
			...
			{
				case 256:
					UE_LOGF(LogHttp, VeryVerbose, "found 256 bit ECDSA pubkey");
					CertPubKeyASN1Header = (char *)ecdsaSecp256r1Asn1Header;
					CertPubKeyASN1HeaderSize = sizeof(ecdsaSecp256r1Asn1Header);
					break;
				case 384:
					UE_LOGF(LogHttp, VeryVerbose, "found 384 bit ECDSA pubkey");
					CertPubKeyASN1Header = (char *)ecdsaSecp384r1Asn1Header;
					CertPubKeyASN1HeaderSize = sizeof(ecdsaSecp384r1Asn1Header);
					break;
				default:
					UE_LOGF(LogHttp, Log, "unsupported ECDSA key length %i for certificate %i for domain '%ls'; skipping!", [CertPubKeySize integerValue], i, *RemoteHost);
					continue;
			}
		}
		else {
			UE_LOGF(LogHttp, Log, "unsupported key type (not RSA or ECDSA) for certificate %i for domain '%ls'; skipping!", i, *RemoteHost);
			continue;
		}
		
		UE_LOGF(LogHttp, VeryVerbose, "constructed key header: [%d] %ls", CertPubKeyASN1HeaderSize, *FString::FromHexBlob((const uint8*)CertPubKeyASN1Header, CertPubKeyASN1HeaderSize));
		UE_LOGF(LogHttp, VeryVerbose, "current pubkey: [%d] %ls", [(NSData*)CertPubKeyData length], *FString::FromHexBlob((const uint8*)[(NSData*)CertPubKeyData bytes], [(NSData*)CertPubKeyData length]));
		...
		UE_LOGF(LogHttp, VeryVerbose, "reconstructed key: [%d] %ls", [ReconstructedPubKey length], *FString::FromHexBlob((const uint8*)[ReconstructedPubKey bytes], [ReconstructedPubKey length]));
		...
		{
			UE_LOGF(LogHttp, Warning, "could not calculate SHA256 digest of public key %d for domain '%ls'; skipping!", i, *RemoteHost);
		}
		else
		{
			CertDigests.Add(CertCalcDigest);
			UE_LOGF(LogHttp, Verbose, "added SHA256 digest to list for evaluation: domain: '%ls' Base64 digest: %ls", *RemoteHost, *FBase64::Encode(CertCalcDigest.GetData(), CertCalcDigest.Num()));
			UE_LOGF(LogHttp, VeryVerbose, "Certificate digest binary content: [%d] '%ls'", CertCalcDigest.Num(), *FString::FromHexBlob(CertCalcDigest.GetData(), CertCalcDigest.Num()));
		}
	}
	...
	if (CertDigests.IsEmpty() || !FSslModule::Get().GetCertificateManager().VerifySslCertificates(CertDigests, RemoteHost))
	{
		...
		UE_LOGF(LogHttp, Error, "failed certificate pinning validation: no SPKI hashes in request matched pinned hashes for domain '%ls' (was provided %d certificates in request)", *RemoteHost, CertDigests.Num());
		return false;
	}
	else
	{
		UE_LOGF(LogHttp, Verbose, "certificate public key pinning either succeeded or is disabled; continuing with auth");
		return true;
	}
}
...
- (void)URLSession:(NSURLSession *)session didReceiveChallenge:(NSURLAuthenticationChallenge *)challenge completionHandler:(void (^)(NSURLSessionAuthChallengeDisposition disposition, NSURLCredential *credential))completionHandler
{
	...
	{
		UE_LOGF(LogHttp, Error, "failed certificate pinning validation: SslCertificateManager is using non-SHA256 SPKI hashes [expected %d bytes, got %d bytes]", CC_SHA256_DIGEST_LENGTH, ISslCertificateManager::PUBLIC_KEY_DIGEST_SIZE);
	...
	{
		UE_LOGF(LogHttp, Verbose, "challenge was not a server trust; continuing with auth");
