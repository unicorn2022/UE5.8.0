...
{
	UE_LOGF(LogHttp, Warning, "FNullHttpRequest::SetContentAsStreamedFile is not implemented");
	return false;
}
...
bool FNullHttpRequest::SetContentFromStream(TSharedRef<FArchive, ESPMode::ThreadSafe> Stream)
{
	...
	UE_LOGF(LogHttp, Warning, "FNullHttpRequest::SetContentFromStream is not implemented");
	return false;
}

bool FNullHttpRequest::SetResponseBodyReceiveStream(TSharedRef<FArchive> Stream)
{
	UE_LOGF(LogHttp, Warning, "FNullHttpRequest::SetResponseBodyReceiveStream is not implemented");
	return false;
}
...
bool FNullHttpRequest::ProcessRequest()
{
	...
	UE_LOGF(LogHttp, Log, "Start request. %p %ls url=%ls", this, *GetVerb(), *GetURL());
	return true;
}
...
void FNullHttpRequest::Tick(float DeltaSeconds)
{
	if (CompletionStatus == EHttpRequestStatus::Processing)
	{
		...
		{
			UE_LOGF(LogHttp, Warning, "Timeout processing Http request. %p",
				this);
...
void FNullHttpRequest::FinishedRequest()
{
	...
	UE_LOGF(LogHttp, Log, "Finished request %p. no response %ls url=%ls elapsed=%.3f",
