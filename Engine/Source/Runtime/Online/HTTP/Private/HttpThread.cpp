...
CSV_DEFINE_CATEGORY(HttpRequest, true);
...
TAutoConsoleVariable<bool> CVarHttpCsvRegions(
	TEXT("http.csvRegions"),
	false,
	TEXT("Add CSV regions for requests"),
	ECVF_Default
);

static void CsvRegion(const FHttpRequestCommon* Request, bool bBegin)
{
#if CSV_PROFILER_STATS && !UE_BUILD_SHIPPING
	if (CVarHttpCsvRegions.GetValueOnAnyThread(true) && FCsvProfiler::Get()->IsCategoryEnabled(CSV_CATEGORY_INDEX(HttpRequest)))
	{
		// Trim the https:// and anything passed via URL params
		FString Name = Request->GetURL();
		if (Name.StartsWith(TEXT("https://")))
		{
			Name.RightChopInline(8);
		}
		int32 Index;
		if (Name.FindChar(TCHAR('?'), Index))
		{
			Name.MidInline(0, Index);
		}
		Name = FString::Printf(TEXT("%s (%s)"), *Name, *Request->GetVerb());
		if (bBegin)
		{
			CSV_REGION_BEGIN(HttpRequest, *Name);
		}
		else
		{
			CSV_REGION_END(HttpRequest, *Name);
		}
	}
#endif
}
...
{
	UE_LOGF(LogInit, Log, "Creating http thread with maximum %d concurrent requests", MaxConcurrentRequests);
}
...
void FHttpThreadBase::UpdateThreadPriorityIfNeeded()
{
	if ( !bIsSingleThread && ensure(!IsInGameThread()))
	{
		...
		{
			UE_LOGF(LogHttp, Display, "Updating HTTP thread priority to %ls", GHttpThreadPriortyNames[ThreadPriorityIndex]);
...
{
	QUICK_SCOPE_CYCLE_COUNTER(STAT_FHttpThreadBase_AddRequest);
...
void FHttpThreadBase::ConsumeCanceledRequestsAndNewRequests(TArray<FHttpRequestCommon*>& RequestsToCancel, TArray<FHttpRequestCommon*>& RequestsToComplete)
{
	...
	for (FHttpRequestCommon* Request : RequestsToCancel)
	{
		...
		else
		{
			...
			UE_LOGF(LogHttp, Log, "Unable to find request (%p) in HttpThread", Request);
		}
	}
}
...
void FHttpThreadBase::StartRequestsWaitingInQueue(TArray<FHttpRequestCommon*>& RequestsToComplete)
{
	...
	if (RunningThreadedRequestsCounter < MaxConcurrentRequests)
	{
		while(RunningThreadedRequestsCounter < MaxConcurrentRequests && !RateLimitedThreadedRequests.IsEmpty())
		{
			...
			const double DurationInQueue = FPlatformTime::Seconds() - ReadyThreadedRequest->GetTimeStartedWaitingInQueue();
			UE_CLOGF(!FApp::IsUnattended() && DurationInQueue > CVarHttpDurationInQueueToWarnInSeconds.GetValueOnAnyThread(), LogHttp, Warning, "Request (%p) %ls %ls waited in queue for %.2fs before starting", ReadyThreadedRequest, *ReadyThreadedRequest->GetVerb(), *ReadyThreadedRequest->GetURL(), DurationInQueue);
			constexpr float StartImmediately = 0.01f;
			if (DurationInQueue > StartImmediately)
			{
				HttpManager.RecordMaxTimeToWaitInQueue(DurationInQueue);
			}

			CsvRegion(ReadyThreadedRequest, true);
			if (StartThreadedRequest(ReadyThreadedRequest))
			{
				...
				UE_LOGF(LogHttp, Verbose, "Started http request in thread (%p). Waited in queue for (%.2fs) Running threaded requests (%d) Rate limited threaded requests (%d)", 
...
void FHttpThreadBase::MoveCompletingRequestsToCompletedRequests(TArray<FHttpRequestCommon*>& RequestsToComplete)
{
	...
	for (int32 Index = 0; Index < RunningThreadedRequests.Num(); ++Index)
	{
		...
		if (Request->IsThreadedRequestComplete())
		{
			...
			--Index;
			UE_LOGF(LogHttp, Verbose, "Threaded request (%p) completed. Running threaded requests (%d)", Request, RunningThreadedRequests.Num());
		}
	}
}
...
FLegacyHttpThread::FLegacyHttpThread()
{
	...
	UE_LOGF(LogHttp, Log, "HTTP thread active frame time %.1f ms. Minimum active sleep time is %.1f ms. HTTP thread idle frame time %.1f ms. Minimum idle sleep time is %.1f ms.", HttpThreadActiveFrameTimeInSeconds * 1000.0, HttpThreadActiveMinimumSleepTimeInSeconds * 1000.0, HttpThreadIdleFrameTimeInSeconds * 1000.0, HttpThreadIdleMinimumSleepTimeInSeconds * 1000.0);
}
