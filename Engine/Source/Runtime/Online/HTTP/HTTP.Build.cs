...
public class HTTP : ModuleRules
{
	...
	protected virtual bool bPlatformSupportsOSProxyAddress
	{
		get
		{
	...
	protected virtual int CurlDefaultMaxTotalConnections { get { return 256; } }
	protected virtual int CurlDefaultMaxIdleConnections { get { return 5; } } // Default value of CURLMOPT_MAXCONNECTS is 5

	protected virtual int DefaultMaxConcurrentRequests
	{
		get
		{
			if (bPlatformSupportsLibCurl)
			{
				if (CurlDefaultMaxTotalConnections <= CurlDefaultMaxIdleConnections)
				{
					throw new BuildException($"CurlDefaultMaxTotalConnections {CurlDefaultMaxTotalConnections} must be greater than CurlDefaultMaxIdleConnections {CurlDefaultMaxIdleConnections}");
				}

				return CurlDefaultMaxTotalConnections - CurlDefaultMaxIdleConnections; // Means max active connections
			}
	...
	public HTTP(ReadOnlyTargetRules Target) : base(Target)
	{
		...
		PrivateDefinitions.Add("UE_HTTP_PLATFORM_SUPPORTS_OS_PROXY_ADDRESS=" + (bPlatformSupportsOSProxyAddress ? "1" : "0"));
		...
		PrivateDefinitions.Add("UE_HTTP_CURL_DEFAULT_MAX_TOTAL_CONNECTIONS=" + CurlDefaultMaxTotalConnections);
		PrivateDefinitions.Add("UE_HTTP_CURL_DEFAULT_MAX_IDLE_CONNECTIONS=" + CurlDefaultMaxIdleConnections);
		...
		PublicDefinitions.Add("UE_HTTP_SUPPORT_OPTION_REUSE_CONNECTION=" + (bPlatformSupportsLibCurl ? "1" : "0"));
	}
}
