...
void FXmppConnectionStrophe::Login(const FString& UserId, const FString& Auth)
{
	...
	{
		UE_LOGF(LogXmpp, Log, "Starting Login on connection");
		UE_LOGF(LogXmpp, Log, "  Server = %ls:%d", *ServerConfiguration.ServerAddr, ServerConfiguration.ServerPort);
		UE_LOGF(LogXmpp, Log, "  User = %ls", *NewJid.ToDebugString());
		...
		{
			UE_LOGF(LogXmpp, Log, "Reusing existing connection for Jid=%ls", *GetUserJid().ToDebugString());
	...
	if (!ErrorStr.IsEmpty())
	{
		UE_LOGF(LogXmpp, Warning, "Login failed. %ls", *ErrorStr);
...
{
	UE_LOGF(LogXmpp, Log, "Starting Strophe XMPP thread");
...
{
	UE_LOGF(LogXmpp, Log, "Stopping Strophe XMPP thread");
...
void FXmppConnectionStrophe::ReceiveConnectionStateChange(EStropheConnectionEvent StateChange, bool bQueue)
{
	...
	if (bQueue)
	{
		UE_LOGF(LogXmpp, Log, "Strophe XMPP thread received state change Was: %ls Now: %ls", EXmppLoginStatus::ToString(LoginStatus), EXmppLoginStatus::ToString(NewLoginStatus));
...
{
	UE_LOGF(LogXmpp, Error, "Received Strophe XMPP Stanza %ls with error %ls", *Error.GetStanza().GetName(), *Error.GetErrorString());
}

void FXmppConnectionStrophe::ReceiveStanza(const FStropheStanza& Stanza)
{
	UE_LOGF(LogXmpp, Verbose, "Received Strophe XMPP Stanza %ls", *Stanza.GetName());
	...
	if (MessagesStrophe.IsValid())
	{
		...
		{
			UE_LOGF(LogXmpp, VeryVerbose, "%ls Stanza handled by Messages", *Stanza.GetName());
			return;
		}
	}
	if (MultiUserChatStrophe.IsValid())
	{
		...
		{
			UE_LOGF(LogXmpp, VeryVerbose, "%ls Stanza handled by MultiUserChat", *Stanza.GetName());
			return;
		}
	}
	if (PingStrophe.IsValid())
	{
		...
		{
			UE_LOGF(LogXmpp, VeryVerbose, "%ls Stanza handled by Ping", *Stanza.GetName());
			return;
		}
	}
	if (PresenceStrophe.IsValid())
	{
		...
		{
			UE_LOGF(LogXmpp, VeryVerbose, "%ls Stanza handled by Presence", *Stanza.GetName());
			return;
		}
	}
	if (PrivateChatStrophe.IsValid())
	{
		...
		{
			UE_LOGF(LogXmpp, VeryVerbose, "%ls Stanza handled by PrivateChat", *Stanza.GetName());
			return;
		}
	}
	if (PubSubStrophe.IsValid())
	{
		...
		{
			UE_LOGF(LogXmpp, VeryVerbose, "%ls Stanza handled by PubSub", *Stanza.GetName());
			return;
		}
	}

	UE_LOGF(LogXmpp, Warning, "%ls Stanza left unhandled", *Stanza.GetName());
}
...
void FXmppConnectionStrophe::ProcessLoginStatusChange(EXmppLoginStatus::Type NewLoginStatus)
{
	...
	{
		UE_LOGF(LogXmpp, Log, "Strophe processing LoginStatus change Was: %ls Now: %ls", EXmppLoginStatus::ToString(OldLoginStatus), EXmppLoginStatus::ToString(NewLoginStatus));
		...
		{
			UE_LOGF(LogXmpp, Log, "Logged IN JID=%ls", *GetUserJid().ToDebugString());
		...
		{
			UE_LOGF(LogXmpp, Log, "Logged OUT JID=%ls", *GetUserJid().ToDebugString());
			if (OldLoginStatus == EXmppLoginStatus::ProcessingLogin)
			{
...
{
	UE_LOGF(LogXmpp, Warning, "JID=%ls Status=%ls", *GetUserJid().ToDebugString(), EXmppLoginStatus::ToString(GetLoginStatus()));
}
