...
bool FXmppMessagesStrophe::HandleMessageStanza(const FStropheStanza& IncomingStanza)
{
	...
	{
		UE_LOGF(LogXmpp, Warning, "Message: Failed to Deserialize JsonBody %ls", *IncomingStanza.GetBodyText().GetValue());
	}
...
bool FXmppMessagesStrophe::HandleMessageErrorStanza(const FStropheStanza& ErrorStanza)
{
	...
	if (ErrorList.Num() > 0)
	{
		for (const FStropheStanza& ErrorItem : ErrorList)
		{
			...
			UE_LOGF(LogXmpp, Error, "Message: Received error %ls", *OutError);
		}
	}
	else
	{
		UE_LOGF(LogXmpp, Warning, "Received unknown message stanza error");
	}
	return true;
}
...
bool FXmppMessagesStrophe::SendMessage(const FXmppUserJid& RecipientId, const FString& Type, const FString& Payload, bool bPayloadIsSerializedJson)
{
	...
	{
		UE_LOGF(LogXmpp, Warning, "Unable to send message. Invalid jid: %ls", *RecipientId.ToDebugString());
		return false;
	}
