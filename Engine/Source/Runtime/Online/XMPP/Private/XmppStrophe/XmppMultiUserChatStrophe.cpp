...
bool FXmppMultiUserChatStrophe::HandlePresenceErrorStanza(const FStropheStanza& IncomingStanza)
{
	...
	UE_LOGF(LogXmpp, Warning, "MUC: Received error %ls", *OutError.ErrorMessage);
...
bool FXmppMultiUserChatStrophe::HandleGroupChatErrorStanza(const FStropheStanza& IncomingStanza)
{
	...
	UE_LOGF(LogXmpp, Warning, "MUC: Received GroupChat error %ls", *ErrorMessage);
...
{
	UE_LOGF(LogXmpp, Verbose, "MUC: CreateRoom=%ls Nickname=%ls", *RoomId, *Nickname);
	...
	if (!bSuccess)
	{
		UE_LOGF(LogXmpp, Warning, "MUC: CreateRoom failed. %ls", *ErrorStr);
...
{
	UE_LOGF(LogXmpp, Verbose, "MUC: ConfigureRoom RoomId=%ls", *RoomId);
	...
	if (!bSuccess)
	{
		UE_LOGF(LogXmpp, Warning, "MUC: Failed to configure Room=%ls Error=%ls", *RoomId, *ErrorStr);
...
{
	UE_LOGF(LogXmpp, Verbose, "MUC: RefreshRoomInfo RoomId=%ls", *RoomId);
...
{
	UE_LOGF(LogXmpp, Verbose, "MUC: JoinPublicRoom RoomId=%ls Nickname=%ls", *RoomId, *Nickname);
	...
	if (!bSuccess)
	{
		UE_LOGF(LogXmpp, Warning, "MUC: JoinPublicRoom failed. %ls", *ErrorStr);
...
{
	UE_LOGF(LogXmpp, Verbose, "MUC: JoinPrivateRoom RoomId=%ls Nickname=%ls", *RoomId, *Nickname);
	...
	if (!bSuccess)
	{
		UE_LOGF(LogXmpp, Warning, "MUC: JoinPrivateRoom failed. %ls", *ErrorStr);
...
{
	UE_LOGF(LogXmpp, Verbose, "MUC: RegisterMember RoomId=%ls Nickname=%ls", *RoomId, *Nickname);
...
{
	UE_LOGF(LogXmpp, Verbose, "MUC: UnregisterMember RoomId=%ls Nickname=%ls", *RoomId, *Nickname);
	// No-op currently in libjingle version
	return false;
}
...
{
	UE_LOGF(LogXmpp, Verbose, "MUC: ExitRoom RoomId=%ls", *RoomId);
	...
	{
		UE_LOGF(LogXmpp, Warning, "MUC: Cannot leave room %ls: %ls", *RoomId, *ErrorStr);
...
{
	UE_LOGF(LogXmpp, Verbose, "MUC: SendChat RoomId=%ls", *RoomId);
...
void FXmppMultiUserChatStrophe::DumpMultiUserChatState() const
{
	...
	for (const TMap<FXmppRoomId, FXmppRoomStrophe>::ElementType& Room : Chatrooms)
	{
		...
		UE_LOGF(LogXmpp, Display, "RoomId: %ls", *RoomId);
		UE_LOGF(LogXmpp, Display, " Owner: %ls Subj: %ls Priv: %d", *XmppRoom.Info.OwnerId, *XmppRoom.Info.Subject, XmppRoom.Info.bIsPrivate);
		UE_LOGF(LogXmpp, Display, " Status: %d", (int32)XmppRoom.Status);

		UE_LOGF(LogXmpp, Display, " Members: %d", XmppRoom.Members.Num());
		for (const FXmppChatMemberRef& Member : XmppRoom.Members)
		{
			UE_LOGF(LogXmpp, Display, "  %ls", *Member->ToDebugString());
		}
	}
}
...
{
	UE_LOGF(LogXmpp, VeryVerbose, "MUC: OnReceiveMucPresence: jid=%ls nick=%ls roomid=%ls role=%ls affiliation=%ls", *MemberPresence.UserJid.ToDebugString(), *MemberPresence.GetNickName(), *MemberPresence.GetRoomId(), *MemberPresence.Role, *MemberPresence.Affiliation);
	...
	{
		UE_LOGF(LogXmpp, VeryVerbose, "MUC: OnReceiveMucPresence Ignored presence from room we haven't joined: Room=%ls Connjid=%ls", *MemberPresence.GetRoomId(), *ConnectionManager.GetUserJid().Id);
	}
}
...
void FXmppMultiUserChatStrophe::OnReceiveMucPresenceError(FXmppStropheErrorPair&& PresenceError)
{
	...
	{
		UE_LOGF(LogXmpp, Warning, "MUC: OnReceiveMucPresenceError Received Error from room we haven't joined: Room=%ls Connjid=%ls Error=%ls", *PresenceError.RoomId, *ConnectionManager.GetUserJid().Id, *PresenceError.ErrorMessage);
	}
}
...
void FXmppMultiUserChatStrophe::OnReceiveGroupChatMessage(TUniquePtr<FXmppChatMessage>&& GroupChatMessage)
{
	...
	{
		UE_LOGF(LogXmpp, Log, "MUC: OnReceiveGroupChatMessage Ignored GroupChat from room we haven't joined: Room=%ls Connjid=%ls", *GroupChatMessage->FromJid.Id, *ConnectionManager.GetUserJid().Id);
	}
}
...
void FXmppMultiUserChatStrophe::OnReceiveGroupChatSubject(FXmppStropheSubjectUpdate&& SubjectUpdate)
{
	...
	{
		UE_LOGF(LogXmpp, Log, "MUC: OnReceiveGroupChatMessage Ignored Updated Room Subject from room we haven't joined: Room=%ls Connjid=%ls NewSubject=%ls", *SubjectUpdate.RoomId, *ConnectionManager.GetUserJid().Id, *SubjectUpdate.NewSubject);
	}}
...
void FXmppMultiUserChatStrophe::OnReceiveRoomConfigError(FXmppStropheErrorPair&& RoomConfigError)
{
	...
	if (RoomPtr != nullptr)
	{
		...
		if (ensure(CallbackTypePtr != nullptr))
		{
			...
			UE_LOGF(LogXmpp, Warning, "MUC: Failed to Configure Room Room=%ls Error=%ls", *RoomConfigError.RoomId, *RoomConfigError.ErrorMessage);
		}
		else
		{
			UE_LOGF(LogXmpp, Warning, "MUC: OnReceiveRoomConfigError Received Error from room we have no callback for: Room=%ls Connjid=%ls Error=%ls", *RoomConfigError.RoomId, *ConnectionManager.GetUserJid().Id, *RoomConfigError.ErrorMessage);
		}
	}
	else
	{
		UE_LOGF(LogXmpp, Warning, "MUC: OnReceiveRoomConfigError Received Error from room we haven't joined: Room=%ls Connjid=%ls Error=%ls", *RoomConfigError.RoomId, *ConnectionManager.GetUserJid().Id, *RoomConfigError.ErrorMessage);
	}
}
	...
	if (RoomPtr != nullptr)
	{
		...
		if (ensure(CallbackTypePtr != nullptr))
		{
			...
			case EConfigureRoomTypeStrophe::UseCreateCallback:
				...
				{
					UE_LOGF(LogXmpp, Warning, "OnReceiveRoomConfigSuccess: Room %ls is not in the CreatePending state (%ls)", *RoomPtr->GetRoomId(), LexToString(RoomPtr->Status));
				}
				OnXmppRoomCreateCompleteDelegate.Broadcast(ConnectionManager.AsShared(), true, RoomPtr->GetRoomId(), FString());
				break;
			}

			UE_LOGF(LogXmpp, Verbose, "MUC: OnReceiveRoomConfigSuccess Received success for room %ls", *RoomId);
		}
		else
		{
			UE_LOGF(LogXmpp, Warning, "MUC: OnReceiveRoomConfigSuccess Received success from room with no callback Room=%ls", *RoomId);
		}
	}
	else
	{
		UE_LOGF(LogXmpp, Warning, "MUC: OnReceiveRoomConfigSuccess Received RoomConfig from room we haven't joined: Room=%ls Connjid=%ls", *RoomId, *ConnectionManager.GetUserJid().Id);
	}
}
...
void FXmppMultiUserChatStrophe::OnReceieveRoomInfoUpdate(FXmppRoomId&& RoomId)
{
	...
	{
		UE_LOGF(LogXmpp, Warning, "MUC: OnReceieveRoomInfoUpdate Received RoomInfoUpdate for room we haven't joined: Room=%ls Connjid=%ls", *RoomId, *ConnectionManager.GetUserJid().Id);
	}
}
...
{
	UE_LOGF(LogXmpp, Verbose, "MUC: HandleCreateRoomComplete: Room: %ls User: %ls", *Room.GetRoomId(), *MemberPresence.GetNickName());
...
{
	UE_LOGF(LogXmpp, Verbose, "MUC: HandleJoinPrivateRoomComplete: Room: %ls User: %ls", *Room.GetRoomId(), *MemberPresence.GetNickName());
...
{
	UE_LOGF(LogXmpp, Verbose, "MUC: HandleJoinPublicRoomComplete: Room: %ls User: %ls", *Room.GetRoomId(), *MemberPresence.GetNickName());
...
{
	UE_LOGF(LogXmpp, Verbose, "MUC: HandleExitRoomComplete: Room: %ls User: %ls", *Room.GetRoomId(), *MemberPresence.GetNickName());
	...
	else
	{
		...
		{
			UE_LOGF(LogXmpp, Verbose, "MUC: Server initiated room exit complete; in state %ls", LexToString(Room.Status));
		}
...
{
	UE_LOGF(LogXmpp, Verbose, "MUC: HandleRoomMemberJoined: Room: %ls User: %ls", *Room.GetRoomId(), *MemberPresence.GetNickName());
...
{
	UE_LOGF(LogXmpp, Verbose, "MUC: HandleRoomMemberChanged: Room: %ls User: %ls", *Room.GetRoomId(), *MemberPresence.GetNickName());
...
{
	UE_LOGF(LogXmpp, Verbose, "MUC: HandleRoomMemberLeft: Room: %ls User: %ls", *Room.GetRoomId(), *MemberPresence.GetNickName());
