	...
	case EJson::Object:
		{
			...
			if (LhsObject.IsValid())
			{
				...
				{
					const FJsonObject::FStringType& Key = It.Key;
					const TSharedPtr<FJsonValue> RhsValue = RhsObject->GetFieldUntyped(Key);
					if (!RhsValue.IsValid())
					{
					...
					if (LhsValue.IsValid())
					{
						if (!CompareEqual(*LhsValue, *RhsValue))
						{
							return false;
						}
					}
...
void FJsonValue::ErrorMessage(const FString& InType) const
{
	if (IsNull())
	{
		UE_LOGF(LogJson, Warning, "Json Value of type '%ls' used as a '%ls'.", *GetType(), *InType);
	}
	else
	{
		UE_LOGF(LogJson, Error, "Json Value of type '%ls' used as a '%ls'.", *GetType(), *InType);
	}
}
