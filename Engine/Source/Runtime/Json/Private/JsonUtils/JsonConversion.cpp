...
namespace Private {
	...
	static TOptional<FValue> ConvertToRapidJsonValueInternal(const FJsonValue& SrcValue, FDocument::AllocatorType& Allocator)
	{
		...
		case EJson::Object:
			{
				...
				if (SrcValue.TryGetObject(SrcObject))
				{
					...
					for (const TPair<FJsonObject::FStringType, TSharedPtr<FJsonValue>>& Member : (*SrcObject)->Values)
					{
		...
		UE_LOGF(LogJsonConversion, Error, "UE::Json::Private::ConvertToRapidJsonValueInternal Unable to convert unknown default Json type %d", int32{EnumToUnderlyingType(SrcValue.Type)});
		return {};
	}
}
...
TSharedPtr<FJsonValue> ConvertRapidJsonToSharedJsonValue(const FValue& Value)
{
	...
	UE_LOGF(LogJsonConversion, Warning, "UE::Json::ConvertRapidJsonToSharedJsonValue Unhandled value type %d", int32(Value.GetType()));
	return {};
}
...
TSharedPtr<FJsonObject> ConvertRapidJsonToSharedJsonObject(FConstObject Object)
{
	...
	{
		UE_LOGF(LogJsonConversion, Error, "UE::Json::ConvertRapidJsonToSharedJsonObject unable to convert JSON object");
		return {};
	}
...
TOptional<FDocument> ConvertSharedJsonToRapidJsonDocument(const FJsonObject& SrcObject)
{
	...
	for (const TPair<FJsonObject::FStringType, TSharedPtr<FJsonValue>>& SrcField : SrcObject.Values)
	{
