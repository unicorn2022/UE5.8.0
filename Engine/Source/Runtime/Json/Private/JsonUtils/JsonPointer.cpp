...
namespace UE::Json
{
	...
	static TSharedPtr<FJsonValue> FindPartInJsonValue(const TSharedPtr<FJsonValue>& Value, const FString& PathPart)
	{
		TSharedPtr<FJsonValue> NextValue = Value;
		...
		if (NextValue->TryGetArray(ArrayPtr))
		{
			if(!PathPart.IsNumeric())
			{
				...
				UE_LOGF(LogJson, Warning, "FJsonPointer::TryGet: JsonValue was an array and expected a numeric part but was a string.");
				return nullptr;
			}
			...
			{
				// index out of range
				UE_LOGF(LogJson, Warning, "FJsonPointer::TryGet: Array index %d for array is out of range.", PartAsIndex);
				return nullptr;
			}
		...
		else
		{
			...
			{
				NextValue = (*ObjectPtr)->GetFieldUntyped(PathPart);
				if (!NextValue.IsValid())
				{
					// part of path didn't exist
					UE_LOGF(LogJson, Warning, "FJsonPointer::TryGet: Field \"%ls\" not found.", *PathPart);
					return {};
				}
			}
		}
	...
	bool FJsonPointer::TryGet(const TSharedPtr<FJsonValue>& RootValue, TSharedPtr<FJsonValue>& OutValue) const
	{
		...
		TSharedPtr<FJsonValue> CurrentValue = RootValue;
	...
	bool FJsonPointer::TryGet(const TArray<TSharedPtr<FJsonValue>>& RootValues, TSharedPtr<FJsonValue>& OutValue) const
	{
		...
		{
			UE_LOGF(LogJson, Warning, "FJsonPointer::TryGet: RootObject was an array, but JsonPointer path requested a field \"%ls\".", *FirstPart);
			return false;
		}
		...
		{
			// index out of range
			UE_LOGF(LogJson, Warning, "FJsonPointer::TryGet: Array index %d for array %ls is out of range.", PartAsIndex, *FirstPart);
			return false;
		}
	...
	bool FJsonPointer::ParsePath(const FString& InPath)
	{
		...
		for (int32 Idx = 0; Idx < PathStrings.Num(); ++Idx)
		{
			...
			{
				PathParts.Empty();
				UE_LOGF(LogJson, Warning, "Path part was empty in JSON pointer \"%ls\"", *InPath);
				return false;
			}
