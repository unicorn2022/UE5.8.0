...
/** Check to make inputs aren't required to be null terminated **/
TEST_CASE_NAMED(FRapidJsonUtilsParseWithoutNullTerminator, "System::Engine::FileSystem::RapidJSONUtils::NoNullTerminator", "[ApplicationContextMask][SmokeFilter]")
{
	using namespace UE::Json;

	FString UnterminatedJsonText(TEXT(R"json(
		{ "key" : "value" };trailing
		)json"));

	int32 Index = 0;
	REQUIRE(UnterminatedJsonText.FindLastChar('}', Index));
	TArrayView<TCHAR> View = MakeArrayView(GetData(UnterminatedJsonText), Index+1);

	TValueOrError<FDocument, FParseError> Result = Parse(FStringView(View));
	REQUIRE(!Result.HasError());

	TValueOrError<FDocument, FParseError> InPlaceResult = ParseInPlace(MakeArrayView(View));
	REQUIRE(!InPlaceResult.HasError());
}
