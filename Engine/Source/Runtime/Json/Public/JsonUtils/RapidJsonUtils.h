...
namespace Private
{
	...
	/**
	 * Reads a string from the specified value.
	 * @note The given value must be a string.
	 */
	inline FStringView ValueAsStringView(const FValue& Value)
	{
	...
	/**
	 * Attempts to read a string from the specified value.
	 *
	 * Returns the value, if it matches the type explicitly
	 */
	JSON_API TOptional<FStringView> GetStringValue(const FValue& Value);
