...
struct FJsonSerializerPolicy_JsonObject
{
	...
	struct FGlobalState
	{
		...
		TSharedPtr<UE::JSON::Private::FJsonStringSet> StringSet = MakeShared<UE::JSON::Private::FJsonStringSet>();
#endif
	};
	...
	{
		FStackState() = default;
		explicit FStackState(const FGlobalState& GlobalState)
#if !UE_JSONOBJECT_LEGACY_STRING_KEYS
			: StringSet(GlobalState.StringSet)
#endif
		{
		}
	...
	JSON_API static bool GetValueFromState(const FStackState& State, FValue& OutValue);
	JSON_API static bool GetValueFromState(const FStackState& State, FArrayOfValues& OutArray);
	JSON_API static bool GetValueFromState(const FStackState& State, FMapOfValues& OutMap);
	JSON_API static void ResetValue(FValue& OutValue);
	JSON_API static void ReadObjectStart(FStackState& State);
	JSON_API static void ReadObjectEnd(FStackState& State, FValue& OutValue);
	JSON_API static void ReadArrayStart(FStackState& State);
	JSON_API static void ReadArrayEnd(FStackState& State, FValue& OutValue);
	...
	JSON_API static void AddValueToObject(FStackState& State, const FString& Identifier, FValue& NewValue);
	JSON_API static void AddValueToArray(FStackState& State, FValue& NewValue);
...
public:
	...
	{
		typename Policy::FStackState State;
...
private:
	template <class CharType>
	static bool Deserialize(TJsonReader<CharType>& Reader, typename Policy::FStackState& OutStackState, EFlags InOptions)
	{
		typename Policy::FGlobalState GlobalState;
		TArray<TSharedRef<typename Policy::FStackState>> ScopeStack; 
		TSharedPtr<typename Policy::FStackState> CurrentState;
			...
			case EJsonNotation::ObjectStart:
				{
					...
					CurrentState = MakeShared<typename Policy::FStackState>(GlobalState);
	...
	static bool Serialize(const TSharedRef<typename Policy::FElement>& StartingElement, TJsonWriter<CharType, PrintPolicy>& Writer, bool bCloseWriter)
	{
		...
		while (ElementStack.Num() > 0)
		{
			...
			UE_LOGF(LogJson, Fatal, "Could not print Json Value, unrecognized type.");
		}
