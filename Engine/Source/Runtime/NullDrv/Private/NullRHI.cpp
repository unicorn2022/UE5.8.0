...
void* FNullDynamicRHI::GetStaticBuffer(size_t Size)
{
	...
	{
		UE_LOGF(LogRHI, Log, "NullRHI preferably does not allocate memory on the server. Try to change the caller to avoid doing allocs in when FApp::ShouldUseNullRHI() is true.");
		bLogOnce = true;
	}
#endif
...
void FNullDynamicRHI::RHIEndFrame(const FRHIEndFrameArgs& Args)
{
	...
	EventStream.Emplace<UE::RHI::GPUProfiler::FEvent::FFrameBoundary>(ERHIPipeline::Graphics, Args, FPlatformTime::Cycles64());
