...
void AARSharedWorldPlayerController::Tick(float DeltaSeconds)
{
	...
	if (!HasAuthority() && !bIsReadyToReceive && IsGameStateReady())
	{
		...
		UE_LOGF(LogAR, Verbose, "Notifying server ready to receive via ServerMarkReadyForReceiving()");
	}
}
...
{
	bIsReadyToReceive = true;
	UE_LOGF(LogAR, Verbose, "Client is ready to receive");
}
...
{
	UE_LOGF(LogAR, Verbose, "Client received ClientInitSharedWorld(%d, %d)", PreviewImageSize, ARWorldDataSize);
...
{
	UE_LOGF(LogAR, Verbose, "Client received ClientUpdatePreviewImageData(%d, %d)", Offset, Buffer.Num());
...
{
	UE_LOGF(LogAR, Verbose, "Client received ClientUpdateARWorldData(%d, %d)", Offset, Buffer.Num());
