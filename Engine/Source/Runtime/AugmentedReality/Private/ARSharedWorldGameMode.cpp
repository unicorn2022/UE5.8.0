...
void AARSharedWorldGameMode::SetARWorldSharingIsReady()
{
	...
	if (!bShouldSendSharedWorldData)
	{
		...
		UE_LOGF(LogAR, Log, "Ready to share AR data with clients. AR world size is (%d) and preview image size is (%d)", SharedGameState->ARWorldData.Num(), SharedGameState->PreviewImageData.Num());
	}
}
			...
			if (PC != nullptr && !PC->IsLocalController() && PC->IsReadyToReceive())
			{
				if (PlayerToReplicationStateMap.Contains(PC))
				{
					...
					if (CurrentOffset < SharedGameState->PreviewImageData.Num())
					{
						...
						UE_LOGF(LogAR, Verbose, "Sent ClientUpdatePreviewImageData(%d, %d) to PC (%ls)", CurrentOffset, BytesToSend, *PC->GetName());
					}
					...
					if (CurrentOffset < SharedGameState->ARWorldData.Num())
					{
						...
						UE_LOGF(LogAR, Verbose, "Sent ClientUpdateARWorldData(%d, %d) to PC (%ls)", CurrentOffset, BytesToSend, *PC->GetName());
					}
				}
				else
				{
					...
					UE_LOGF(LogAR, Verbose, "Sent InitSharedWorld(%d, %d) to PC (%ls)", SharedGameState->PreviewImageData.Num(), SharedGameState->ARWorldData.Num(), *PC->GetName());
				}
			}
		}
	}
...
void AARSharedWorldGameMode::Logout(AController* Exiting)
{
	...
	{
		UE_LOGF(LogAR, Verbose, "Removing PC (%ls)", *PC->GetName());
