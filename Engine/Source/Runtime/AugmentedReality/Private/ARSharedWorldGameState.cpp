...
void AARSharedWorldGameState::InitSharedWorld(int32 PreviewImageSize, int32 ARWorldDataSize)
{
	...
	if (GIsServer)
	{
		UE_LOGF(LogAR, Warning, "AARSharedWorldGameState::InitSharedWorld() was called on the server. This is client only");
		return;
	}
	...
	{
		UE_LOGF(LogAR, Warning, "AARSharedWorldGameState::InitSharedWorld() was called with invalid sizes (%d, %d)", PreviewImageSize, ARWorldDataSize);
	}
}
...
void AARSharedWorldGameState::UpdatePreviewImageData(int32 Offset, const uint8* Buffer, int32 Size)
{
	...
	if (GIsServer)
	{
		UE_LOGF(LogAR, Warning, "AARSharedWorldGameState::UpdatePreviewImageData() was called on the server. This is client only");
		return;
	}
	...
	{
		UE_LOGF(LogAR, Warning, "AARSharedWorldGameState::UpdatePreviewImageData() was called with bad offset (%d) or size (%d)", Offset, Size);
	}
}
...
void AARSharedWorldGameState::UpdateARWorldData(int32 Offset, const uint8* Buffer, int32 Size)
{
	...
	if (GIsServer)
	{
		UE_LOGF(LogAR, Warning, "AARSharedWorldGameState::UpdateARWorldData() was called on the server. This is client only");
		return;
	}
	...
	{
		UE_LOGF(LogAR, Warning, "AARSharedWorldGameState::UpdateARWorldData() was called with bad offset (%d) or size (%d)", Offset, Size);
	}
}
...
void AARSharedWorldGameState::TriggerCompletionIfDone()
{
	...
	{
		UE_LOGF(LogAR, Log, "Notifying client AR world data is ready");
