...
namespace Audio
{
	...
	*/
	...
	void FUniformPartitionConvolution::FImpulseResponse::SetImpulseResponse(const float* InSamples, int32 InNum)
	{
		...
		{
			UE_LOGF(LogSignalProcessing, Warning, "Truncating impulse response of %d samples to maximum length of %d samples", InNum, MaxNumSamples);
			InNum = MaxNumSamples;
		}
	...
	{
		static const FName FactoryName("UniformPartitionConvolutionFactory");
		return FactoryName;
	}
	...
	TUniquePtr<IConvolutionAlgorithm> FUniformPartitionConvolutionFactory::NewConvolutionAlgorithm(const FConvolutionSettings& InSettings)
	{
		...
		{
			UE_LOGF(LogSignalProcessing, Error, "Failed to create uniform partition convolution algorithm due to failed FFT creation");
