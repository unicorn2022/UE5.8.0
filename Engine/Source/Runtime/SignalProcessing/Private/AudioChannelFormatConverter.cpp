...
namespace Audio
{
	...
	void FBaseChannelFormatConverter::SetMixGain(int32 InInputChannelIndex, int32 InOutputChannelIndex, float InGain, bool bFadeToGain)
	{
		...
		{
			UE_LOGF(LogSignalProcessing, Warning, "Skipping mix entry. Input channel (%d) does not exist for input format with %d channels.", InInputChannelIndex, InputFormat.NumChannels);
			return;
		}

		if ((InOutputChannelIndex >= OutputFormat.NumChannels) || (InOutputChannelIndex < 0))
		{
			UE_LOGF(LogSignalProcessing, Warning, "Skipping mix entry. Output channel (%d) does not exist for output format with %d channels.", InOutputChannelIndex, OutputFormat.NumChannels);
			return;
		}
	...
	void FBaseChannelFormatConverter::ProcessAudio(const TArray<FAlignedFloatBuffer>& InInputBuffers, TArray<FAlignedFloatBuffer>& OutOutputBuffers)
	{
		...
		for (FMixElement& Element : ChannelMixStates)
		{
			...
			if (MixState.InputChannelIndex != CurrentInputChannelIndex)
			{
				...
				{
					UE_LOGF(LogSignalProcessing, Warning, "Input buffer frame count (%d) does not match expected frame count (%d)", InputBuffer.Num(), NumFramesPerCall);
	...
	TUniquePtr<FBaseChannelFormatConverter> FBaseChannelFormatConverter::CreateBaseFormatConverter(const FInputFormat& InInputFormat, const FOutputFormat& InOutputFormat, TArrayView<const FChannelMixEntry> InMixEntries, int32 InNumFramesPerCall)
	{
		...
		{
			UE_LOGF(LogSignalProcessing, Error, "Invalid input format channel count (%d). Must be greater than zero", InInputFormat.NumChannels);
			return TUniquePtr<FBaseChannelFormatConverter>(nullptr);
		}

		if (InOutputFormat.NumChannels < 1) 
		{
			UE_LOGF(LogSignalProcessing, Error, "Invalid output format channel count (%d). Must be greater than zero", InOutputFormat.NumChannels);
			return TUniquePtr<FBaseChannelFormatConverter>(nullptr);
		}

		if (InNumFramesPerCall < 1)
		{
			UE_LOGF(LogSignalProcessing, Error, "Invalid num frames per call (%d). Must be greater than zero", InNumFramesPerCall);
			return TUniquePtr<FBaseChannelFormatConverter>(nullptr);
		}
