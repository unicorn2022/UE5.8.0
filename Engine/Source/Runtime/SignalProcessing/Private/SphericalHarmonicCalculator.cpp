...
bool FSphericalHarmonicCalculator::IndexToOrderAndDegree(const int32 InChannelIndex, int32& OutOrder, int32& OutDegree)
{
	if (InChannelIndex >= 0)
	{
		OutOrder = FMath::FloorToInt(FMath::Sqrt(static_cast<float>(InChannelIndex)));
		for (; OutOrder * (OutOrder + 1) > InChannelIndex; --OutOrder)
		{
		}
		OutDegree = InChannelIndex - OutOrder * (OutOrder + 1) - OutOrder;
		return true;
	}
	return false;
}
