...
namespace Audio
{
		...
		,	ReadPos(0)
	{
		...
		{
			UE_LOGF(LogSignalProcessing, Warning, "MaxNumInspectSamples [%d] reduced to maximum storage [%d]", MaxNumInspectSamples, MaxStorage);
			MaxNumInspectSamples = MaxStorage;
		}

		if (SampleCapacity > MaxStorage)
		{
			UE_LOGF(LogSignalProcessing, Warning, "SampleCapacity [%d] reduced to maximum storage [%d]", SampleCapacity, MaxStorage);
			SampleCapacity = MaxStorage;
		}

		if (MaxNumInspectSamples > SampleCapacity)
		{
			UE_LOGF(LogSignalProcessing, Warning, "SampleCapacity [%d] increased to Maxmimum Inspection [%d]", SampleCapacity, MaxNumInspectSamples);
