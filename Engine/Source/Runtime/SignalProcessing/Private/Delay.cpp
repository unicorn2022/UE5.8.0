...
namespace Audio
{
	...
	void FDelay::WriteToBuffer(const float InInput)
	{
		InputAttenuation = FMath::Min(InputAttenuation + InputFadeGainStep, 1.f); // Ceil to 1.0
		AudioBuffer[WriteIndex] = InInput * FMath::Max(InputAttenuation, 0.f);   // Floor to 0.0
	}

	void FDelay::AdvanceIndices()
	{
	...
	void FDelay::ProcessAudioBuffer(const float* InAudio, int32 InNumSamples, float* OutAudio)
	{
		...
		{
			Update();
			WriteToBuffer(InAudio[SampleIndex]);
			OutAudio[SampleIndex] = OutputAttenuation * Read();
			AdvanceIndices();
		}
	}
