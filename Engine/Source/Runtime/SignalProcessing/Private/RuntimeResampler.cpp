...
namespace Audio
{
	...
	int32 FRuntimeResampler::GetNumInputFramesNeededToProduceOutputFrames(const int32 InNumOutputFrames) const
	{
		...
		if (InNumOutputFrames > 0)
		{
			...
			// We need the frame after InputFrameFP, so we take the floor and add 1. Because we're returning
			//  the number of frames needed rather than the index of the last frame, we add another 1. We add these
			//  frames before dividing to make sure the numerator is positive and the division gives us the floor.
			return int32((InputFrameFP + 2 * FPScale) / FPScale);
		}
		return 0;
	}
