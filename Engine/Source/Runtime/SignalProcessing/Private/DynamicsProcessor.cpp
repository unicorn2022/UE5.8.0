...
namespace Audio
{
	...
	void FDynamicsProcessor::SetNumChannels(const int32 InNumChannels)
	{
		...
		if (InNumChannels != LookaheadDelay.Num())
		{
			...
			LookaheadDelay.Reset(InNumChannels);
	...
	{
		// There are rare cases where the number of output channels is mutated but the key count is
		// not (such as during device swap, adding & removing endpoints such as controllers, etc.).
		// For now, ensure key number of channels matches that of the output number of channels.
		const int32 NumChannels = GetNumChannels();
	...
	void FDynamicsProcessor::ProcessAudioDeinterleaveInternal(const float* const* const InBuffers, const int32 InNumFrames, float* const* OutBuffers, const float* const* const InKeyBuffers, float* const* OutEnvelopes)
	{
		...
		for (int32 Channel = 0; Channel < KeyNumChannels; ++Channel)
		{
			...
			// EnvelopeFollow key
			TRACE_CPUPROFILER_EVENT_SCOPE(FDynamicsProcessor::ProcessAudioDeinterleaveInternal_EnvFollower);
