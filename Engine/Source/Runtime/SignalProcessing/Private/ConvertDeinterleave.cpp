...
namespace Audio
{
	namespace ConvertDeinterleavePrivate
	{
		...
		struct TConvertDeinterleave : IConvertDeinterleave
		{
			...
			virtual void ProcessAudio(TArrayView<const float> InSamples, const TArray<TArrayView<float>>& OutMultiChannelAudio) const override
			{
				checkf((InSamples.Num() % NumInputChannels) == 0, TEXT("Input sample buffer contains partial audio frame."));
				checkf((InSamples.Num() / NumInputChannels) == GetMultichannelBufferNumFrames(OutMultiChannelAudio), TEXT("Audio buffer frame count mismatch."));
				checkf(NumOutputChannels == OutMultiChannelAudio.Num(), TEXT("Output audio buffer not initialized to expected channel count."));

				const int32 NumFrames = GetMultichannelBufferNumFrames(OutMultiChannelAudio);
				...
				{
					OutputChannelDataPtrs[OutChannelIndex] = OutMultiChannelAudio[OutChannelIndex].GetData();
				}
		...
		struct TConvertDeinterleave<1, NumOutputChannels> : IConvertDeinterleave
		{
			...
			virtual void ProcessAudio(TArrayView<const float> InSamples, const TArray<TArrayView<float>>& OutMultiChannelAudio) const override
			{
				checkf(InSamples.Num() == GetMultichannelBufferNumFrames(OutMultiChannelAudio), TEXT("Audio buffer frame count mismatch."));
				checkf(NumOutputChannels == OutMultiChannelAudio.Num(), TEXT("Output audio buffer not initialized to expected channel count."));
				...
				for (int32 OutChannelIndex = 0; OutChannelIndex < NumOutputChannels; OutChannelIndex++)
				{
					...
					if (Gain == 1.f)
					{
						// Simply copy the audio to the output channels when gain is 1
						FMemory::Memcpy(OutMultiChannelAudio[OutChannelIndex].GetData(), InSamples.GetData(), NumFrames * sizeof(float));
					}
					else if (Gain != 0.f)
					{
						// Only need to multiply if gain is non-zero
						Audio::ArrayMultiplyByConstant(InSamples, ChannelGains[OutChannelIndex], OutMultiChannelAudio[OutChannelIndex]);
					}
					else
					{
						// If gain is zero, we can set the output channel to zero.
						FMemory::Memset(OutMultiChannelAudio[OutChannelIndex].GetData(), 0, NumFrames * sizeof(float));
					}
				}
			}
		...
		struct TConvertDeinterleave<1, 1> : IConvertDeinterleave
		{
			...
			{
				checkf(InSamples.Num() == GetMultichannelBufferNumFrames(OutMultiChannelAudio), TEXT("Audio buffer frame count mismatch."));
				checkf(1 == OutMultiChannelAudio.Num(), TEXT("Output audio buffer not initialized to expected channel count."));

				const int32 NumFrames = GetMultichannelBufferNumFrames(OutMultiChannelAudio);
				const float* InSampleData = InSamples.GetData();
				FMemory::Memcpy(OutMultiChannelAudio[0].GetData(), InSampleData, NumFrames * sizeof(float));
			}
		};
