...
namespace Audio
{
	...
	{
		static const FName ModularFeatureName = FName("AudioConvolutionAlgorithmFactory");
		return ModularFeatureName;
	}

	const FName FConvolutionFactory::AnyAlgorithmFactory = FName("AnyAlgorithmFactory");
	...
	TUniquePtr<IConvolutionAlgorithm> FConvolutionFactory::NewConvolutionAlgorithm(const FConvolutionSettings& InSettings, const FName& InAlgorithmFactoryName)
	{
		...
		if (0 == Factories.Num())
		{
			UE_LOGF(LogSignalProcessing, Warning, "No registered IConvolutionAlgorithmFactories exist.");
		...
		if (AnyAlgorithmFactory != InAlgorithmFactoryName)
		{
			...
			if (0 == Factories.Num())
			{
				UE_LOGF(LogSignalProcessing, Warning, "No IConvolutionAlgorithmFactories named \"%ls\" found.", *InAlgorithmFactoryName.ToString());
				return TUniquePtr<IConvolutionAlgorithm>();
			}
		}
		...
		for (IConvolutionAlgorithmFactory* Factory : Factories)
		{
			if (Factory->AreConvolutionSettingsSupported(InSettings))
			{
				...
				{
					UE_LOGF(LogSignalProcessing, Warning, "IConvolutionAlgorithmFactory failed to create IConvolutionAlgorithm despite supporting ConvolutionReverb Settings.");
				}
				else
				{
		...
		UE_LOGF(LogSignalProcessing, Warning, "No IConvolutionAlgorithmFactories can create IConvolutionAlgorithm for given settings.");
		return TUniquePtr<IConvolutionAlgorithm>();
	}
}
