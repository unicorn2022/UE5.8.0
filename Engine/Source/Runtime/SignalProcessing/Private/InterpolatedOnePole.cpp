...
namespace Audio
{
	...
	{
		checkf(InNumChannels > 0, TEXT("FInterpolatedLPF::Init called with InNumChannels <= 0"));
	...
	void FInterpolatedLPF::StartFrequencyInterpolation(const float InTargetFrequency, const int32 InInterpLength)
	{
		...
		if (!FMath::IsNearlyEqual(InTargetFrequency, CutoffFrequency))
		{
			...
			B1Delta = (CurrInterpLength > 1) ? (B1Target - B1Curr) / static_cast<float>(CurrInterpLength) : 0.0f;
		}
	...
	void FInterpolatedLPF::ProcessAudioBuffer(const float *RESTRICT InputBuffer, float *RESTRICT OutputBuffer, const int32 NumSamples)
	{
		...
		if (NumChannels == 1)
		{
			...
			for (int32 SampleIndex = 0; SampleIndex < NumSamples; ++SampleIndex)
			{
				...
				float Yn = InputSample + B1 * (Z1D - InputSample); // LPF
				Z1D = Yn;
	...
	{
		checkf(InNumChannels > 0, TEXT("FInterpolatedHPF::Init called with InNumChannels <= 0"));
		SampleRate = InSampleRate;
	...
	void FInterpolatedHPF::StartFrequencyInterpolation(const float InTargetFrequency, const int32 InterpLength)
	{
		...
		if (!FMath::IsNearlyEqual(InTargetFrequency, CutoffFrequency))
		{
			...
			A0Target = G / (1.0f + G);
			A0Delta = (CurrInterpLength > 1) ? (A0Target - A0Curr) / static_cast<float>(CurrInterpLength) : 0.0f;
		}
	...
	void FInterpolatedHPF::ProcessBufferInPlace(float* InOutBuffer, const int32 NumSamples)
	{
		float* Z1Data = Z1.GetData();
		...
		if (NumChannels == 1)
		{
			...
			for (int32 SampleIndex = 0; SampleIndex < NumSamples; ++SampleIndex)
			{
				...
				InOutBuffer[SampleIndex] = InputSample - LPF;
			}
		...
		if (NumChannels == 2)
		{
			...
			if (NumToSimd)
			{
				A0CurrVec = VectorAdd(A0CurrVec, MakeVectorRegister(A0Delta, A0Delta, A0Delta + A0Delta, A0Delta + A0Delta));
				}
			...
			for (; SampleIndex < NumToSimd; SampleIndex += AUDIO_NUM_FLOATS_PER_VECTOR_REGISTER)
			{
				...
				VectorRegister4Float VnLPFSum = VectorAdd(Vn, LPF);
				Z1DataVec = VectorShuffle(Z1DataVec, VnLPFSum, 0, 1, 0, 1);
				Vn = VectorMultiply(VectorSubtract(InputSample, Z1DataVec), A0CurrVec);
				LPF = VectorAdd(Vn, Z1DataVec);

				VnLPFSum = VectorAdd(Vn, LPF);
				Z1DataVec = VectorSwizzle(VnLPFSum, 2, 3, 2, 3);

				VectorStore(VectorSubtract(InputSample, LPF), &InOutBuffer[SampleIndex]);
