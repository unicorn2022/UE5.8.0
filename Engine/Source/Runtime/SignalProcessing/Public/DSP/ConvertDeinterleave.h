...
namespace Audio
{
	...
	struct IConvertDeinterleave
	{
		 ...
		 */
		void ProcessAudio(TArrayView<const float> InSamples, FMultichannelBuffer& OutSamples) const
		{
			TArray<TArrayView<float>> OutMultiChannelAudio = MakeMultichannelBufferView(OutSamples);
			ProcessAudio(InSamples,OutMultiChannelAudio);
		}
		 ...
		 *                    samples must be evenly divisible by the number of 
		 *                    input channels. 
		 * @param OutMultiChannelAudio - A multichannel buffer of samples generated by deinterleaving
		 *                     the input samples and possibly upmixing or downmixing
		 *                     them to the target channel count. 
		 */
		virtual void ProcessAudio(TArrayView<const float> InSamples, const TArray<TArrayView<float>>& OutMultiChannelAudio) const = 0;
