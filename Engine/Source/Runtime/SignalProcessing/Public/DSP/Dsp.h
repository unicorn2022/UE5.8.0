...
namespace Audio
{
	...
	{	
		...
		{
			UE_LOGF(LogSignalProcessing, Log, "SampleValue Was %.2f", InSample);
		}
	}
	...
	*/
	...
	inline bool IsAudioBufferSilent(const float* AudioBuffer, const int32 NumSamples)
	{
		bool bIsSilent = true;
		...
		for (; Index < NumSamples; ++Index)
		{
			...
			if (FMath::Abs(AudioBuffer[Index]) > UE_SMALL_NUMBER)
			{
				bIsSilent = false;
				break;
			}
	...
	// Using passed in tuning frequency (standard is 440), compute midi from frequency in hz
	inline float GetMidiFromFrequency(const float InFrequency, const float TuningFrequency)
	{
		if (TuningFrequency <= 0.f)
		{
			return 0.f;
		}
		return 69.0f + 12.0f * FMath::LogX(2.0f, InFrequency / TuningFrequency);
	}
