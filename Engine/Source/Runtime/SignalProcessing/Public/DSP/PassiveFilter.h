...
namespace Audio
{
	...
	*/
	static void Filter(TArrayView<float>& Signal, const FPassiveFilterParams& InParams)
	{
		...
		{
			UE_LOGF(LogAudio, Error, "Error in Filter: if using TArrayView<float>, Signal's length should be power of 2.");
			return;
		}
		...
		if (!FFT.IsValid())
		{
			UE_LOGF(LogAudio, Error, "Failed to create an FFT Algorithm with log2size %d", FFTSettings.Log2Size);
			return;
		}
