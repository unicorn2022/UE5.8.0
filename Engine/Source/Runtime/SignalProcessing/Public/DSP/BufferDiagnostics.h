...
#define AUDIO_CHECK_BUFFER(BUFFER)\
	static FCheckedBufferState UE_JOIN(BufferCheck, __LINE__)(__LINE__,TEXT(__FILE__),TEXT(UE_STRINGIZE(BUFFER)));\
	if (!UE_JOIN(BufferCheck, __LINE__).DoCheck(BUFFER) )\
	{\
		UE_JOIN(BufferCheck, __LINE__).FailedBufferCheckImpl(TEXT(""));\
	}

#define AUDIO_CHECK_BUFFER_NAMED(BUFFER,NAME)\
	static FCheckedBufferState UE_JOIN(BufferCheck, __LINE__)(__LINE__,TEXT(__FILE__),NAME);\
	if (!UE_JOIN(BufferCheck, __LINE__).DoCheck(BUFFER) )\
	{\
		UE_JOIN(BufferCheck, __LINE__).FailedBufferCheckImpl(TEXT(""));\
	}
...
#define AUDIO_CHECK_BUFFER_NAMED_MSG(BUFFER,NAME,MSG,...)\
	...
	if (!UE_JOIN(BufferCheck, __LINE__).DoCheck(BUFFER))\
	{\
		UE_JOIN(BufferCheck, __LINE__).FailedBufferCheckImpl(MSG,__VA_ARGS__);\
	}
