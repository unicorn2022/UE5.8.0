...
public:
	...
	~FMovieSceneSerializerContext() = default;
	...
	bool CheckHeader(bool bInFileExisted, TArray<FMovieSceneSessionInfo>& SessionInfos, HeaderData& InOutHeader, int64& ReadHeaderPos,  FText& OutFailReason)
	{
		...
		if (bInFileExisted)
		{
			...
			if (ArchiveVer > FTempCustomVersion::LatestVersion)
			{
				...
				UE_LOGF(MovieSceneSerialization, Error, "Cannot open Movie Scene Serialization cache %ls. File version: %d. Max supported version: %d", *Context.Filename, ArchiveVer, FTempCustomVersion::LatestVersion);
				Context.Close();
				return false;
			}
...
bool TMovieSceneSerializer<HeaderData, FrameData>::OpenForWrite(const FString& InFilename, HeaderData& Header, FText& OutFailReason)
{
	if (MovieSceneSerializationNamespace::bAutoSerialize)
	{
		...
		if (Handle != nullptr)
		{
			UE_LOGF(MovieSceneSerialization, Display, "Opened Data cache: %ls", *AbsoluteFilePath);
			...
			if (Runnable->CheckHeader(Guid, false, SessionInfos, Header, ReadHeaderPos, OutFailReason))
			{
				...
				if (!Thread)
				{
					...
					UE_LOGF(MovieSceneSerialization, Display, "Created Thread For Type: %ls", *ThreadName);
				}
		...
		else
		{
			...
			UE_LOGF(MovieSceneSerialization, Error, "Cannot open Movie Scene Serialization cache %ls. Failed to create archive.", *InFilename);
			return false;
		}
	}
	else
...
bool TMovieSceneSerializer<HeaderData, FrameData>::OpenForRead(const FString& AbsoluteFilePath, HeaderData& OutHeader, FText& OutFailReason)
{
	...
	if (Handle != nullptr)
	{
		UE_LOGF(MovieSceneSerialization, Display, "Opened Movie Scene Serialization cache: %ls", *AbsoluteFilePath);
	...
	else
	{
		...
		UE_LOGF(MovieSceneSerialization, Error, "Cannot open Movie Scene Serialization cache %ls. Failed to create archive.", *AbsoluteFilePath);
		return false;
	}
}
template<typename HeaderData, typename FrameData>
