...
{
	static FName DefaultEndpointName = FName("Default Endpoint");
	return DefaultEndpointName;
}

FName IAudioEndpointFactory::GetModularFeatureName()
{
	static FName ModularFeatureName = "External Audio Endpoint";
...
IAudioEndpointFactory* IAudioEndpointFactory::Get(const FName& InName)
{
	...
	UE_LOGF(LogAudioEndpoints, Display, "No endpoint implementation for %ls found for this platform. Endpoint Submixes set to this type will not do anything.", *InName.ToString());
