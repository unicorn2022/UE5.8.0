...
TSharedPtr<Audio::IProxyData> IAudioProxyDataFactory::CreateProxyData(const Audio::FProxyDataInitParams& InitParams) 
{
	...
	if (Proxy.IsValid())
	{
		...
		if (!LoggedProxyWarnings.Contains(ProxyTypeName))
		{
			...
			UE_LOGF(LogAudioExtensions, Warning, "Use of deprecated 'TUniquePtr<Audio::IProxyData> CreateNewProxyData(...)' for proxy type \"%ls\". Please override 'TSharedPtr<Audio::IProxyData> CreateProxyData(...)'", *ProxyTypeName.ToString());
		}
	}
