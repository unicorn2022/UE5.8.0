...
namespace Audio
{
	...
	struct FWaveformTransformationWaveInfo
	{
		...
		int32 StartSampleOffset = 0;
		int32 NumEditedSamples = 0;
	};
	...
	public:
		...
		virtual ~IWaveTransformation() = default;
	};
...
struct FSoundWaveCuePoint
{
	...
	const bool operator==(const FSoundWaveCuePoint& Other) const
	{ 
		return CuePointID == Other.CuePointID &&
			FrameLength == Other.FrameLength &&
			FramePosition == Other.FramePosition &&
			Label == Other.Label &&
			IsLoopRegion() == Other.IsLoopRegion();
	}

	const bool operator!=(const FSoundWaveCuePoint& Other) const { return !(*this == Other); }
...
public:
	void SetOriginalNumFrames(const int64 InOriginalNumFrames) { OriginalNumFrames = InOriginalNumFrames; }
	const int64 GetOriginalNumFrames() const { return OriginalNumFrames; }

	int64 GetStartFrameOffset() const { return FMath::RoundToInt64(StartTime * SampleRate); }
	AUDIOEXTENSIONS_API int64 GetEndFrameOffset() const;
};

#if WITH_EDITORONLY_DATA
// Information to be retrieved from each transformation (Add members as needed)
...
public:
	...
	{
		Algo::StableSort(InOutTransformations, [](const TObjectPtr<UWaveformTransformationBase>& TransformationA, const TObjectPtr<UWaveformTransformationBase>& TransformationB)
			{
	...
	virtual FString GetTransformationHash() const PURE_VIRTUAL(UWaveformTransformationBase::GetTransformationHash, return FString(););
