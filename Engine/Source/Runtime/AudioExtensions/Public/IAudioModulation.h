...
namespace Audio
{
	...
	struct FModulationParameter
	{
		...
		FModulationParameter(const FModulationParameter& InParam) = default;
		FModulationParameter(FModulationParameter&& InParam) = default;

		FModulationParameter& operator=(FModulationParameter&& InParam) = default;
		FModulationParameter& operator=(const FModulationParameter& InParam) = default;
		...
		// Whether or not to clamp normalized values to [0,1] when using this parameter. 
		bool bClampToNormalizedRange = true;
	...
	UE_INTERNAL
	AUDIOEXTENSIONS_API void SetModulationParameterClampOverride(FName InName, bool bClamp);
	
	UE_INTERNAL
	AUDIOEXTENSIONS_API void ClearModulationParameterClampOverride(FName InName);
	
	UE_INTERNAL
	AUDIOEXTENSIONS_API void ClearAllModulationParameterClampOverrides();

	// Returns false if parameter is not registered
	UE_INTERNAL
	AUDIOEXTENSIONS_API bool GetModulationParameterClampValue(const FName& ParameterName, bool& bClampOut);
	...
	private:
		...
		FModulatorHandleId HandleId = (FModulatorHandleId)INDEX_NONE;
		FModulatorTypeId ModulatorTypeId = (FModulatorTypeId)INDEX_NONE;
		FModulatorId ModulatorId = (FModulatorId)INDEX_NONE;
...
public:
	...
	virtual void GetControlBusAssetPath(TArray<FTopLevelAssetPath>& OutClassNames) = 0;
