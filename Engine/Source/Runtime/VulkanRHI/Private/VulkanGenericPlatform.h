...
public:
	...
	/** The status quo is true, Platforms might opt out if GPU timing queries are not supported. */
	static bool SupportsDynamicResolution() { return true; }
	...
	// Check if a native window handle is still valid for presentation.
	// Platforms where surfaces can be invalidated externally (e.g. Wayland) should override this.
	static bool IsWindowValid(void* WindowHandle) { return WindowHandle != nullptr; }

	// Test-and-clear a platform-side swapchain-recreate request.
	static bool TryConsumeSwapchainRecreateRequest(void* WindowHandle) { return false; }
	...
	static void OnCreatePipelineCrash(struct FCrashContextExtendedWriter& Writer, TConstArrayView<FVulkanShader*> Shaders, TStringView<TCHAR> ExtraInfo, bool bIsPrecache);
#endif
};
