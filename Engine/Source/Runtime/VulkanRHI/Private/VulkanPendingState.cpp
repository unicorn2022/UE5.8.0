...
extern int32 GVulkanNumFramesToWaitForResourceDelete;
...
void FVulkanDescriptorPoolsManager::GC()
{
	...
	for (int32 Index = PoolSets.Num() - 1; Index >= 0; Index--)
	{
		...
		if (PoolSet->IsUnused() && ((int32)(GFrameNumberRenderThread - PoolSet->GetLastFrameUsed()) > GVulkanNumFramesToWaitForResourceDelete))
		{
...
void FVulkanPendingComputeState::PrepareForDispatch(FVulkanCommandListContext& Context)
{
	...
	if (CurrentPipeline->UsesBindless())
	{
		...
		CurrentPipeline->Bind(Context.GetCommandBuffer());
	}
	else
	{
...
void FVulkanPendingGfxState::PrepareForDraw(FVulkanCommandListContext& Context)
{
	...
	else
	{
		...
		{
			UpdateInputAttachments(Context);
		}
			...
			if (CurrStream.Stream == VK_NULL_HANDLE)
			{
				...
				for (uint32 AttributeIndex = 0; AttributeIndex < VertexInputStateInfo.AttributesNum; AttributeIndex++)
				{
					if (VertexInputStateInfo.Attributes[AttributeIndex].binding == CurrBinding.binding)
					{
						...
						UE_LOGF(LogVulkanRHI, Warning, "Missing input assembly binding on location %d in Vertex shader '%ls'",
							CurrBinding.binding,
...
{
	const bool bHasEXTExtendedDynamicState1 = Device.GetOptionalExtensions().HasEXTExtendedDynamicState1 || Device.SupportsShaderObjects();
	const bool bHasEXTExtendedDynamicState2 = Device.GetOptionalExtensions().HasEXTExtendedDynamicState2 || Device.SupportsShaderObjects();
	const bool bHasEXTExtendedDynamicState3 = Device.GetOptionalExtensions().HasEXTExtendedDynamicState3 || Device.SupportsShaderObjects();
	const bool bHasEXTVertexInputDynamicState = Device.GetOptionalExtensions().HasEXTVertexInputDynamicState || Device.SupportsShaderObjects();
	...
	// Set the unused graphic states at least once when we use a command buffer for graphics the first time
	if (bForceUpdate)
	{
		CommandBuffer.SetUnusedGraphicsDynamicStates();
	}
...
#define VULKAN_DYNAMIC_STATE_BLOCK_END()      }
	...
	if (bHasEXTExtendedDynamicState2)
	{
		...
		VULKAN_DYNAMIC_STATE_BLOCK_START(Rasterizer.DepthBiasConstantFactor, Rasterizer.DepthBiasSlopeScale);
		...
		VulkanRHI::vkCmdSetDepthBias(CommandBuffer.GetHandle(), NewDynamicState.Rasterizer.DepthBiasConstantFactor, depthBiasClamp, NewDynamicState.Rasterizer.DepthBiasSlopeScale);
		VULKAN_DYNAMIC_STATE_BLOCK_END();
	}
	...
	{
		VULKAN_DYNAMIC_STATE_BLOCK_START(DepthStencil.FrontCompareMask);
		VulkanRHI::vkCmdSetStencilCompareMask(CommandBuffer.GetHandle(), VK_STENCIL_FACE_FRONT_BIT, NewDynamicState.DepthStencil.FrontCompareMask);
		VULKAN_DYNAMIC_STATE_BLOCK_END();

		VULKAN_DYNAMIC_STATE_BLOCK_START(DepthStencil.BackCompareMask);
		VulkanRHI::vkCmdSetStencilCompareMask(CommandBuffer.GetHandle(), VK_STENCIL_FACE_BACK_BIT, NewDynamicState.DepthStencil.BackCompareMask);
		VULKAN_DYNAMIC_STATE_BLOCK_END();

		VULKAN_DYNAMIC_STATE_BLOCK_START(DepthStencil.FrontWriteMask);
		VulkanRHI::vkCmdSetStencilWriteMask(CommandBuffer.GetHandle(), VK_STENCIL_FACE_FRONT_BIT, NewDynamicState.DepthStencil.FrontWriteMask);
		VULKAN_DYNAMIC_STATE_BLOCK_END();

		VULKAN_DYNAMIC_STATE_BLOCK_START(DepthStencil.BackWriteMask);
		VulkanRHI::vkCmdSetStencilWriteMask(CommandBuffer.GetHandle(), VK_STENCIL_FACE_BACK_BIT, NewDynamicState.DepthStencil.BackWriteMask);
		VULKAN_DYNAMIC_STATE_BLOCK_END();

		VULKAN_DYNAMIC_STATE_BLOCK_START(ShadingRate, RenderTargets.bHasFragmentDensityAttachment, Combiner);
		{
			if (GRHISupportsPipelineVariableRateShading && (GRHIVariableRateShadingImageDataType == VRSImage_Palette))
			{
				const bool bNewUsingVariableRateShading = (NewDynamicState.ShadingRate != EVRSShadingRate::VRSSR_1x1) ||
					(NewDynamicState.RenderTargets.bHasFragmentDensityAttachment && NewDynamicState.Combiner != EVRSRateCombiner::VRSRB_Passthrough);

				if (bNewUsingVariableRateShading)
				{
					extern VkFragmentShadingRateCombinerOpKHR GetFragmentCombinerOp(EVRSRateCombiner RateCombiner);

					const VkExtent2D FragmentSize = Device.GetBestMatchedFragmentSize(NewDynamicState.ShadingRate);
					VkFragmentShadingRateCombinerOpKHR CombinerOps[2] =
					{
						GetFragmentCombinerOp(NewDynamicState.Combiner),
						VK_FRAGMENT_SHADING_RATE_COMBINER_OP_MAX_KHR // @todo: This needs to be specified too.
					};
					VulkanRHI::vkCmdSetFragmentShadingRateKHR(CommandBuffer.GetHandle(), &FragmentSize, CombinerOps);
				}
				else
				{
			...
			else if (bForceUpdate && Device.GetOptionalExtensionProperties().FragmentShadingRateFeatures.pipelineFragmentShadingRate)
			{
				const VkExtent2D FragmentSize = { 1, 1 };
				const VkFragmentShadingRateCombinerOpKHR CombinerOps[] = { VK_FRAGMENT_SHADING_RATE_COMBINER_OP_KEEP_KHR , VK_FRAGMENT_SHADING_RATE_COMBINER_OP_KEEP_KHR };
				VulkanRHI::vkCmdSetFragmentShadingRateKHR(CommandBuffer.GetHandle(), &FragmentSize, CombinerOps);
...
void FVulkanPendingGfxState::UpdateInputAttachments(FVulkanCommandListContext& Context)
{
	const FVulkanShader* PixelShader = CurrentPipeline->GetVulkanShader(SF_Pixel);
	if (PixelShader)
	{
		const bool bHasDynamicRendering = Device.GetOptionalExtensions().HasKHRDynamicRendering;
		...
		auto GetDynamicRenderingInfo = [&Context]() {

			FVulkanDynamicRenderingInfo* DynamicRenderingInfo = Context.CurrentDynamicRenderingInfo;
			if (!DynamicRenderingInfo && Context.GetParallelRenderPassInfo())
			{
				DynamicRenderingInfo = Context.GetParallelRenderPassInfo()->DynamicRenderingInfo;
			}
			checkf(DynamicRenderingInfo, TEXT("Missing DynamicRenderingInfo fo input attachments!"));
			return DynamicRenderingInfo;
		};
		...
		for (int32 Index = 0; Index < InputAttachmentData.Num(); ++Index)
		{
			...
			case FVulkanShaderHeader::EAttachmentType::Color7:
			{
				...
				const FVulkanView* ColorView = nullptr;
				if (bHasDynamicRendering)
				{
					ColorView = GetDynamicRenderingInfo()->ColorAttachmentViews[ColorIndex];
				}
				else
				{
					check((ColorIndex >= 0) && (ColorIndex < (int32)Context.GetCurrentFramebuffer()->GetNumColorAttachments()));
					ColorView = Context.GetCurrentFramebuffer()->AttachmentTextureViews[ColorIndex];
				}

				checkf(ColorView, TEXT("Missing view pointer for color attachment %d."), ColorIndex);
				CurrentState->SetInputAttachment(ShaderStage::Pixel, AttachmentData.BindingIndex, ColorView->GetTextureView(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
			}
			break;
			case FVulkanShaderHeader::EAttachmentType::Depth:
			{
				const FVulkanView::FTextureView* PartialDepthView = nullptr;
				if (bHasDynamicRendering)
				{
					FVulkanDynamicRenderingInfo* DynamicRenderingInfo = GetDynamicRenderingInfo();
					checkf(DynamicRenderingInfo->PartialDepthAttachmentView, TEXT("Missing PartialDepthAttachmentView for input attachments!"))
					PartialDepthView = &DynamicRenderingInfo->PartialDepthAttachmentView->GetTextureView();
				}
				else
				{
					PartialDepthView = &Context.GetCurrentFramebuffer()->GetPartialDepthTextureView();
				}

				CurrentState->SetInputAttachment(ShaderStage::Pixel, AttachmentData.BindingIndex, *PartialDepthView, VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL);
			}
			break;
			default:
				checkf(0, TEXT("Unknown attachment type %d."), (int32)AttachmentData.Type);
			}
		}
	}
}
...
void FVulkanDescriptorSetCache::AddCachedPool()
{
	...
	if (FreePool)
	{
		...
		UE_LOGF(LogVulkanRHI, Display, "FVulkanDescriptorSetCache::AddCachedPool() MaxDescriptorSets Delta/Err: %f. Tolerance: [%f..%f].",	Error, MinErrorTolerance, MaxErrorTolerance);
		FreePool.Reset();
	}
...
void FVulkanDescriptorSetCache::GC()
{
	...
	while ((CachedPools.Num() > GDSetCacheMaxPoolLookups) && CachedPools.Last()->CanGC())
	{
		...
		if (FreePool)
		{
			UE_LOGF(LogVulkanRHI, Warning, "FVulkanDescriptorSetCache::GC() Free Pool is not empty! Too small r.Vulkan.DSetCacheTargetSetsPerPool?");
		}
...
{
	return ((int32)(GFrameNumberRenderThread - RecentFrame) > GVulkanNumFramesToWaitForResourceDelete);
}
...
float FVulkanDescriptorSetCache::FCachedPool::CalcAllocRatio() const
{
	...
	{
		UE_LOGF(LogVulkanRHI, Warning, "FVulkanDescriptorSetCache::FCachedPool::CalcAllocRatio() Pool Allocation Ratio is too low: %f. Using: %f.", AllocRatio, MinAllocRatio);
