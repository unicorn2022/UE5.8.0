...
extern VkPipelineStageFlags GVulkanDeviceGraphicsPipelineStageBits;
...
static void GetVkStageAndAccessFlags(ERHIAccess RHIAccess, FRHITransitionInfo::EType ResourceType, uint32 UsageFlags, bool bIsDepthStencil, bool bSupportsReadOnlyOptimal, VkPipelineStageFlags& StageFlags, VkAccessFlags& AccessFlags, VkImageLayout& Layout, bool bIsSourceState)
{
	...
	if (EnumHasAnyFlags(RHIAccess, ERHIAccess::SRVGraphicsNonPixel))
	{
		StageFlags |= (GVulkanDeviceGraphicsPipelineStageBits & ~VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT);
		...
		ProcessedRHIFlags |= (uint32)ERHIAccess::SRVGraphicsNonPixel;
	}

	if (EnumHasAnyFlags(RHIAccess, ERHIAccess::SRVGraphicsPixel))
	{
		...
		ProcessedRHIFlags |= (uint32)ERHIAccess::SRVGraphicsPixel;
	}
	...
	{
		StageFlags |= GVulkanDeviceGraphicsPipelineStageBits;
	...
	ensureMsgf(RemainingFlags == 0, TEXT("Some access flags were not processed. RHIAccess=%x, ProcessedRHIFlags=%x, RemainingFlags=%x"), EnumToUnderlyingType(RHIAccess), ProcessedRHIFlags, RemainingFlags);
}
...
static void GetDepthOrStencilStageAndAccessFlags(ERHIAccess Access, VkPipelineStageFlags& StageFlags, VkAccessFlags& AccessFlags)
{
	...
	ensureMsgf(RemainingFlags == 0, TEXT("Some access flags were not processed. Access=%x, ProcessedRHIFlags=%x, RemainingFlags=%x"), EnumToUnderlyingType(Access), ProcessedRHIFlags, RemainingFlags);
}
...
void FVulkanDynamicRHI::RHICreateTransition(FRHITransition* Transition, const FRHITransitionCreateInfo& CreateInfo)
{
	...
	// If queue ownership transfers are necessary, always use fences on cross queue to ensure they are submitted in order
	if ((SrcPipelines != DstPipelines) && (Device->NeedsQueueFamilyOwnershipTransfers() || !EnumHasAnyFlags(CreateInfo.Flags, ERHITransitionCreateFlags::NoFence)))
	{
...
void ConvertTransitionToBarriers(FVulkanCommandListContext& Context, const FVulkanTransitionData& Data, BarrierArrayType& OutBarriers)
{
	...
	for (const FRHITransitionInfo& Info : Data.TransitionInfos)
	{
		...
		else
		{
			...
			// Keep this hack around until we know we can always be running with UseDedicatedNonPixelAccess
		...
		else if (Texture != nullptr)
		{
			...
			// For 3D textures created with VK_IMAGE_CREATE_2D_ARRAY_COMPATIBLE_BIT when Maintenance9 is enabled,
			// a layerCount==1 refers to a single slice (used to refer to the rest of the texture).  Maintain the same behavior
			// with/without Maintenance9 (without altering values for drivers without).
			const FRHITextureDesc& Desc = Texture->GetDesc();
			const bool bUseAllSlices =
				(Desc.Dimension == ETextureDimension::Texture3D) &&
				Context.Device.GetOptionalExtensions().HasKHRMaintenance9 &&
				!EnumHasAnyFlags(Desc.Flags, TexCreate_ReservedResource);

			// Fill the ImageBarrier.subresourceRange from the FRHITransitionInfo
			ImageBarrier.subresourceRange = {
				.aspectMask = AspectFlags,
				.baseMipLevel = Info.IsAllMips() ? 0u : Info.MipIndex,
				.levelCount = Info.IsAllMips() ? VK_REMAINING_MIP_LEVELS : 1u,
				.baseArrayLayer = Info.IsAllArraySlices() ? 0u : Info.ArraySlice,
				.layerCount = (Info.IsAllArraySlices() || bUseAllSlices) ? VK_REMAINING_ARRAY_LAYERS : 1u
			};
...
static void MaskSupportedAsyncFlags(FVulkanDevice& Device, TArray<BarrierType>& InOutBarriers, bool bMaskSrc, bool bMaskDst, bool bRemoveEmpty)
{
	...
	for (int32 Index = InOutBarriers.Num() - 1; Index >= 0; --Index)
	{
		BarrierType& Barrier = InOutBarriers[Index];
		if (bMaskSrc)
		{
		...
		if (bRemoveEmpty && ((Barrier.srcStageMask == 0) || (Barrier.dstStageMask == 0)))
		{
			InOutBarriers.RemoveAtSwap(Index);
		}
	}
}
...
static void PatchCrossPipeTransitions(TArray<BarrierType>& Barriers, FVulkanCommandListContext& Context, ERHIPipeline SrcPipelines, ERHIPipeline DstPipelines, bool bIsBeginTransition)
{
	...
	else // Src/Dst is ERHIPipeline::All, add a single queue transfer for now :todo-jn:
	{
		...
		MaskSupportedAsyncFlags(Context.Device, Barriers, !bSrcIsGraphics, bSrcIsGraphics, false);
...
void ProcessTransitionSync2(FVulkanCommandListContext& Context, TArrayView<const FRHITransition*>& Transitions, bool bIsBeginTransition)
{
	...
	const bool bUseOwnershipTransfers = Context.Device.HasAsyncComputeQueue() && !GVulkanAllowConcurrentImage && Context.Device.NeedsQueueFamilyOwnershipTransfers();
	for (const FRHITransition* Transition : Transitions)
	{
		const FVulkanTransitionData* Data = Transition->GetPrivateData<FVulkanTransitionData>();
		...
		if (IsSingleRHIPipeline(Data->SrcPipelines) && IsSingleRHIPipeline(Data->DstPipelines))
		{
			...
			if (Data->SrcPipelines != Data->DstPipelines)
			{
				if (bUseOwnershipTransfers)
				{
					// For buffer transitions, just remove unsupported flags for the compute queue
					if (ExecutingPipeline == ERHIPipeline::AsyncCompute)
					{
						MaskSupportedAsyncFlags(Context.Device, BarrierArrays.BufferBarriers, true, true, true);
					}
				...
				{
					// When we don't have ownership transfers, only submit the barrier once on the graphics queue (ensures all stages are supported)
					check((TargetPipeline == ERHIPipeline::Graphics) || (OtherPipeline == ERHIPipeline::Graphics));
					if (ExecutingPipeline != ERHIPipeline::Graphics)
					{
						continue;
					}
				}
		...
		// For images without concurrent sharing mode, make copies of the array so we can:
		// - wipe the layout transitions from the barriers
		// - patch in the actual queue family ownership transfer
		// NOTE: Maintenance9 being a requirement, ownership transfers aren't required for buffers
		if (bUseOwnershipTransfers && (bNeedsRelease || bNeedsAcquire) && !GVulkanAllowConcurrentImage)
		{
		...
		if (bNeedsPreLayoutChange)
		{
			...
			if (ExecutingPipeline == ERHIPipeline::AsyncCompute)
			{
				MaskSupportedAsyncFlags(Context.Device, BarrierArrays.BufferBarriers, false, true, true);
				MaskSupportedAsyncFlags(Context.Device, BarrierArrays.ImageBarriers, false, true, false);
			}
		...
		if (TempImageBarriers.Num())
		{
			SubmitBarriers(MakeArrayView(BarrierArrays.MemoryBarriers), {/* no ownership transfers for buffers */}, MakeArrayView(TempImageBarriers));
		...
		if (bNeedsPostLayoutChange)
		{
			...
			if (ExecutingPipeline == ERHIPipeline::AsyncCompute)
			{
				MaskSupportedAsyncFlags(Context.Device, BarrierArrays.BufferBarriers, true, false, true);
				MaskSupportedAsyncFlags(Context.Device, BarrierArrays.ImageBarriers, true, false, false);
			}
