	...
	if (Payloads.Num() > 0)
	{
		...
		if (Payload->CommandBuffers.Num() > 0)
		{
			...
			if (!CommandBuffer->HasEnded())
			{
				...
				const bool bIsParallelCommandBuffer = (CommandBuffer->GetCommandBufferType() == EVulkanCommandBufferType::Parallel);

				// Only record begin/end timestamps on primary command buffers
				FVulkanQueryPool* OptionalTimestampQueryPool = GRHIGlobals.SupportsTimestampRenderQueries && (bIsPrimaryCommandBuffer || bIsParallelCommandBuffer)
					? GetCurrentTimestampQueryPool(*Payload)
					: nullptr;

				if (!CommandBuffer->IsOutsideRenderPass() && (bIsPrimaryCommandBuffer || bIsParallelCommandBuffer))
				{
					if (Device.GetOptionalExtensions().HasKHRDynamicRendering)
					{
						UE_LOGF(LogVulkanRHI, Warning, "Forcing EndDynamicRendering() for submission");
						CommandBuffer->EndDynamicRendering(bIsParallelCommandBuffer ? OptionalTimestampQueryPool : nullptr);
					}
					else
					{
						UE_LOGF(LogVulkanRHI, Warning, "Forcing EndRenderPass() for submission");
						CommandBuffer->EndRenderPass();
					}
				}

				CommandBuffer->End(bIsPrimaryCommandBuffer ? OptionalTimestampQueryPool : nullptr);
			}
		}
	}
}

void FVulkanContextCommon::AppendParallelRenderPayload(FVulkanPayload* ParallelRenderingPayload, VkRenderingInfo* NeedsResumeInfo)
{
	...
	// Close the last suspended render pass with an empty call, do not use any timestamps or work tracking
	if (NeedsResumeInfo)
	{
		PrepareNewCommandBuffer(GetPayload(EPhase::Execute), false);
		GetCommandBuffer().BeginDynamicRendering(*NeedsResumeInfo, nullptr);
		GetCommandBuffer().EndDynamicRendering(nullptr);
		GetCommandBuffer().End(nullptr);
	}

	PrepareNewCommandBuffer(GetPayload(EPhase::Execute));
...
void FVulkanContextCommon::PrepareNewCommandBuffer(FVulkanPayload& Payload, bool bAllowTimestamp)
{
	...
	FVulkanQueryPool* OptionalTimestampQueryPool = bAllowTimestamp && GRHIGlobals.SupportsTimestampRenderQueries && bIsPrimaryCommandBuffer
		? GetCurrentTimestampQueryPool(Payload)
		: nullptr;

	VkRenderPass RenderPassHandle = GetParallelRenderPassInfo()
		? GetParallelRenderPassInfo()->RenderPassHandle
		: VK_NULL_HANDLE;
...
FVulkanCommandListContext::FVulkanCommandListContext(FVulkanDevice& InDevice, ERHIPipeline InPipeline, bool bInImmediate)
	...
	, bImmediate(bInImmediate)
{
	...
	if (InDevice.GetOptionalExtensions().HasKHRDynamicRendering)
	{
		CurrentDynamicRenderingInfo = new FVulkanDynamicRenderingInfo;
	}
}

FVulkanCommandListContext::FVulkanCommandListContext(FVulkanDevice& InDevice, FVulkanParallelRenderPassInfo* InParallelRenderPassInfo)
	: FVulkanContextCommon(InDevice, *InDevice.GetQueue(ERHIPipeline::Graphics), InDevice.GetOptionalExtensions().HasKHRDynamicRendering ? EVulkanCommandBufferType::Parallel : EVulkanCommandBufferType::Secondary)
...
void FVulkanCommandListContext::ReleasePendingState()
{
	...
	if (CurrentDynamicRenderingInfo)
	{
		delete CurrentDynamicRenderingInfo;
		CurrentDynamicRenderingInfo = nullptr;
	}
}
...
void FVulkanCommandListContext::FillDynamicRenderingInfo(const FRHIRenderPassInfo& InRenderPassInfo, FVulkanDynamicRenderingInfo& OutRenderingInfo)
{
	...
	FMemory::Memzero(OutRenderingInfo.ColorAttachmentViews);
	OutRenderingInfo.PartialDepthAttachmentView = nullptr;
	...
	if (NumColorRenderTargets)
	{
		...
		for (int32 Index = 0; Index < NumColorRenderTargets; ++Index)
		{
			...
			OutRenderingInfo.ColorAttachmentViews[Index] = View;
		}
	}
	...
	if (InRenderPassInfo.DepthStencilRenderTarget.DepthStencilTarget)
	{
		...
		OutRenderingInfo.PartialDepthAttachmentView = Texture->PartialView;
		...
		if (InRenderPassInfo.DepthStencilRenderTarget.ExclusiveDepthStencil.IsUsingDepth())
		{
			...
			DepthView = FVulkanFramebuffer::GetDepthStencilTargetViewDesc(Texture, NumColorRenderTargets, MipIndex, InRenderPassInfo.MultiViewCount, NumLayers);
...
void FVulkanCommandListContext::RHIEndParallelRenderPass()
{
	if (!CurrentParallelRenderPassInfo->SecondaryPayloads.IsEmpty())
	{
		...
		FVulkanPayload* MergedPayload = CurrentParallelRenderPassInfo->SecondaryPayloads[0];
		for (int32 Index = 1; Index < CurrentParallelRenderPassInfo->SecondaryPayloads.Num(); ++Index)
		{
			FVulkanPayload* OtherPayload = CurrentParallelRenderPassInfo->SecondaryPayloads[Index];
		...
		if (Device.GetOptionalExtensions().HasKHRDynamicRendering)
		{
			// Make sure the first command buffer is the one that only suspends (should rarely happen)
			if (VKHasAllFlags(MergedPayload->CommandBuffers[0]->LastDynamicRenderingFlags, VK_RENDERING_RESUMING_BIT))
			{
				for (int32 Index = 1; Index < MergedPayload->CommandBuffers.Num(); ++Index)
				{
					if (!VKHasAllFlags(MergedPayload->CommandBuffers[Index]->LastDynamicRenderingFlags, VK_RENDERING_RESUMING_BIT))
					{
						MergedPayload->CommandBuffers.Swap(0, Index);
						break;
					}
				}
			}
			...
			VkRenderingInfo* NeedsResumeInfo = bNeedsResume ? &CurrentParallelRenderPassInfo->DynamicRenderingInfo->RenderingInfo : nullptr;

			// Insert all the parallel payloads
			AppendParallelRenderPayload(MergedPayload, NeedsResumeInfo);
		}
		else
		{
...
void FVulkanCommandListContext::ApplyShaderBindingLayout()
{
	if (ShaderBindingLayout && ShaderBindingLayout->GetNumUniformBufferEntries())
	{
		...
		TArray<uint32> UBBindlessIndices;
		UBBindlessIndices.SetNum(ShaderBindingLayout->GetNumUniformBufferEntries());
		for (uint32 Index = 0; Index < ShaderBindingLayout->GetNumUniformBufferEntries(); ++Index)
		{
			checkf(StaticUniformBuffers[Index], TEXT("Missing static uniform buffer for shader binding layout (%s)."),
				*ShaderBindingLayout->GetUniformBufferEntry(Index).LayoutName);

			FVulkanUniformBuffer* StaticUniformBuffer = ResourceCast(StaticUniformBuffers[Index]);
		...
		GetCommandBuffer().SetStaticUniformBufferPushConstants(UBBindlessIndices);
	}
}

// Inserts a Vulkan-specific end frame marker, which can be used by tools like RenderDoc to
// identify when a frame is done when there is no native Present (e.g. with a standalone XR
// device rendering to an offscreen buffer
void FVulkanCommandListContext::InsertEndFrameDebugLabel()
{
#if VULKAN_ENABLE_DRAW_MARKERS
	if (auto CmdDbgInsertLabel = Device.GetCmdInsertDebugLabel())
	{
		FTCHARToUTF8 Converter(TEXT("vr-marker,frame_end,type,application"));
		VkDebugUtilsLabelEXT Label;
		ZeroVulkanStruct(Label, VK_STRUCTURE_TYPE_DEBUG_UTILS_LABEL_EXT);
		Label.pLabelName = Converter.Get();
		const FLinearColor LColor = FLinearColor::Blue;
		Label.color[0] = LColor.R;
		Label.color[1] = LColor.G;
		Label.color[2] = LColor.B;
		Label.color[3] = LColor.A;
		CmdDbgInsertLabel(GetCommandBuffer().GetHandle(), &Label);
	}
