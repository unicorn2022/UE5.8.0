...
#else
#define VULKAN_VALIDATE_DESCRIPTORS_WRITTEN RHI_INCLUDE_SHADER_DEBUG_DATA
#endif
...
public:
	...
	void FinalizeBindings(const FVulkanDevice& Device, const FUniformBufferGatherInfo& UBGatherInfo, bool bUsesBindless);

	void GenerateHash(VkPipelineBindPoint InBindPoint);
...
public:
	...
	bool WriteBindlessUniformBuffer(FVulkanDevice& Device, uint32 DescriptorIndex, const FVulkanUniformBuffer* UniformBuffer);
...
protected:
	...
	// UB handles when running with bindless descriptor heaps
	TArray<uint32> BindlessUBIndices;
	...
	TArray<uint32, TInlineAllocator<2> > ExcludedMask;
#endif
	...
	void SetExcludedResources(const TArrayView<uint32> ExcludedResourceCount);
};
