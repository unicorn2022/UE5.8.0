...
public:
	...
	uint8 bHasBoundDescriptorHeap		: 1 = 0;
	...
	void BeginDynamicRendering(const VkRenderingInfo& RenderingInfo, FVulkanQueryPool* OptionalQueryPool);
	void EndDynamicRendering(FVulkanQueryPool* OptionalQueryPool);
	...
	void SetUnusedGraphicsDynamicStates();

	void BindPipeline(VkPipelineBindPoint BindPoint, VkPipeline PipelineHandle, bool bBindless);
	void BindGraphicsShaderObjects(TArrayView<FVulkanShader*> VulkanShaders);
	void BindComputeShaderObject(FVulkanShader* VulkanShader);

	void SetStaticUniformBufferPushConstants(TConstArrayView<uint32> StaticUniformBuffers);
	void SetBindingPushConstants(EShaderFrequency Frequency, uint64 GlobalsAddressOrFirstBinding, TConstArrayView<uint32> Bindings);
...
private:
	...
	VkShaderEXT CurrentComputeShaderObject = VK_NULL_HANDLE;
	VkShaderEXT CurrentGraphicsShaderObjects[ShaderStage::NumGraphicsStages] = { VK_NULL_HANDLE };
	TArray<uint32> CurrentPushConstants;

	void ResetCurrentState();
	void ResetBoundShaderObject();
