...
public:
	...
	bool ReadAllCacheChunks(FPSOArchiveReader& ArchiveReader, TUniqueFunction<void(FPSOArchiveReader& ArchiveReader)> OnFoundCacheChunk)
	{
		...
		{
			UE_LOGF(LogVulkanRHI, Log, "FVulkanChunkedPipelineCacheManager: incorrect Cache file version (%d, expected %d)", Version, CacheFileVersion);
			return false;
		}
		...
		{
			UE_LOGF(LogVulkanRHI, Log, "FVulkanChunkedPipelineCacheManager: mismatched hash version (%d, expected %d)", PrecacheHashVersion, FVulkanDynamicRHI::GetPrecachePSOHashVersion());
			return false;
		}
		...
		{
			UE_LOGF(LogVulkanRHI, Log, "FVulkanChunkedPipelineCacheManager: Cache file params have changed (%d, %x=%x)", LastValidOffset, ParamHash, GetCacheBuildingParamHash());
			return false;
		}
	...
	bool LoadAllCacheChunks(TUniqueFunction<void(FPSOArchiveReader& ArchiveReader)> OnFoundCacheChunk)
	{
		...
		if (Stats.FileSize < (UE::Vulkan::GMaxTotalCacheSizeMb * 1024 * 1024))
		{
			...
			{
				UE_LOGF(LogVulkanRHI, Log, "FVulkanChunkedPipelineCacheManager: loading %ls", *FullCachePath);
				bReadSuccess = ReadAllCacheChunks(ArchiveReader, MoveTemp(OnFoundCacheChunk));
			}
			else
			{
				UE_LOGF(LogVulkanRHI, Log, "FVulkanChunkedPipelineCacheManager: Cache file could not open %ls for reading", *FullCachePath);
			}
		}
		else
		{
	...
	bool OpenWriteHandle(bool bAppend)
	{
		...
		UE_CLOGF(PSOFileWriter, LogRHI, Log, "Opened binary cache for write (%ls)", *FullCachePath);
		UE_CLOGF(PSOFileWriter == nullptr, LogRHI, Warning, "Failed to open Vulkan binary cache output file. (%ls)", *FullCachePath);
		UE_CLOGF(PSOFileWriter && (PSOFileWriter->IsError() || PSOFileWriter->IsCriticalError()), LogRHI, Error, "Vulkan binary cache output archive error (%ls, %d,%d)", *FullCachePath, PSOFileWriter->IsError(), PSOFileWriter->IsCriticalError());
...
public:
	...
	void InitNewCache()
	{
		...
		check(PipelineCacheObj == VK_NULL_HANDLE);
...
#if LOGCACHEINFO
					UE_LOGF(LogVulkanRHI, Log, "FVulkanChunkedPipelineCacheManager: create pso - Chunk Capacity reached %ls, Finalizing as No pending jobs remain..", *BinaryCacheFileInfo.Filename);
#endif
	...
	void ReservePendingPSO()
	{
		...
		if ((TotalNumUniquePSOs + PendingAddToCompiles) >= UE::Vulkan::GMaxPSOsPerChunk)
		{
			...
			if ((TotalNumUniquePSOs + PendingAddToCompiles) >= UE::Vulkan::GMaxPSOsPerChunk)
			{
				// become closed..
#if LOGCACHEINFO
				UE_LOGF(LogVulkanRHI, Log, "FVulkanChunkedPipelineCacheManager: Chunk Capacity reached , cachestate %d, CacheSize %d, TotalNumPSOs %d, TotalNumUniquePSOs %d, PendingAddToCompiles %d, %ls, pending finalize..", CacheState, CacheSize, TotalNumPSOs.Load(EMemoryOrder::Relaxed), TotalNumUniquePSOs.Load(EMemoryOrder::Relaxed), PendingAddToCompiles.load(), *BinaryCacheFileInfo.Filename);
#endif
	...
	void LogStats(FString&& LogInfo)
	{
		FRWScopeLock PipelineLock(PipelineCacheObjLock, SLT_ReadOnly);
		FRWScopeLock Lock(CacheStateLock, SLT_ReadOnly); // take the cache readlock, 
		UE_LOGF(LogVulkanRHI, Log, "FVulkanChunkedPipelineCacheManager: %ls Cache name %ls num PSOs %d (%d unique), cache size %d, last used %llu, resident %d, state %d", *LogInfo, *BinaryCacheFileInfo.Filename, TotalNumPSOs.Load(EMemoryOrder::Relaxed), TotalNumUniquePSOs.Load(EMemoryOrder::Relaxed), CacheSize, LastUsedFrame.load(), CacheState != ECacheState::FinalizedEvicted, (int)CacheState);
	}
...
#if LOGCACHEINFO
		UE_LOGF(LogVulkanRHI, Log, "FVulkanChunkedPipelineCacheManager: Evicting cache %ls, %d bytes", *GetCombinedFilePath(), CacheSize);
#endif
...
// 		TArray<uint64> PSOsToBeFlushed;
		...
		{
			IPlatformFile& PlatformFile = FPlatformFileManager::Get().GetPlatformFile();
			return FString("VulkanPSO_") + FGuid::NewGuid().ToString();
		}
...
public:
	...
	void Tick()
	{
		...
		{ // Logging
			...
			if (bLogMe)
			{
				UE_LOGF(LogVulkanRHI, Log, "FVulkanChunkedPipelineCacheManager: Total precache PSOs stored %d, Num Chunks %d", PrecachePSOToCacheChunkMap.Num(), CacheChunksMap.Num());
				for (auto& ChunkPair : CacheChunksMap)
				{
					ChunkPair.Value.Get()->LogStats(FString::Printf(TEXT("Chunk id %d"), ChunkPair.Key));
				}
				UE_LOGF(LogVulkanRHI, Log, "FVulkanChunkedPipelineCacheManager: PSOs created from cache %d ", FoundPSOs.load());
...
private:
	...
	bool LoadAllCaches()
	{
		...
		const bool bDeleteAll = FParse::Param(FCommandLine::Get(), TEXT("ClearVulkanBinaryProgramCache")) || FParse::Param(FCommandLine::Get(), TEXT("clearPSODriverCache"));
		...
		for (FString& FoundFile : FoundFiles)
		{
			...
			if (FoundFile != CacheSubDir || !bIsDir || bDeleteAll)
			{
				bool bSuccess;
				if (bIsDir)
				{
				...
				UE_LOGF(LogRHI, Verbose, "FVulkanChunkedPipelineCacheManagerImpl: Deleting %ls %ls", bIsDir ? TEXT("dir") : TEXT("file"), *FullPath);
				UE_CLOGF(!bSuccess, LogRHI, Warning, "FVulkanChunkedPipelineCacheManagerImpl: Failed to delete %ls", *FullPath);
			}
		}
		...
		for (FString& FoundFile : FoundFiles)
		{
			if (!FoundFile.Equals(FVulkanCombinedChunkCacheFile::Get().GetFilename()))
			{
				...
				UE_LOGF(LogVulkanRHI, Log, "FVulkanChunkedPipelineCacheManager: deleting unexpected Cache %ls.", *FullPath);
		...
		if (!bSuccess)
		{
			UE_LOGF(LogVulkanRHI, Log, "FVulkanChunkedPipelineCacheManager: failed to open chunked PSO cache file, PSO caching is disabled.");
	...
	FVulkanPipelineCacheChunk* GetChunk(FVulkanChunkedPipelineCacheManager::FPSOCreateCallbackFunc<TPipelineState>& PSOCreateFunc, const TInitializer& Initializer, FVulkanPipelineCacheChunk::EPSOCacheFindResult& FindResult, FCacheChunkKey& ChunkKeyOUT, FRWScopeLock& Lock, const bool bTryAdd = false)
	{
		...
		if (!ReturnChunk)
		{
			...
			auto FindChunk = [this,&FindResult,&Initializer,&PSOCreateFunc]
			{
				...
				{
#if LOGCACHEINFO
					UE_LOGF(LogVulkanRHI, Log, "FVulkanChunkedPipelineCacheManager: redundant PSO ..");
#endif
