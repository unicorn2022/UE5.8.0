...
int32 GVulkanNumFramesToWaitForResourceDelete = 2;
...
static int32 GVulkanEvictionAllowBuffers = 0;
static FAutoConsoleVariableRef CVarVulkanEvictionAllowBuffers(
	TEXT("r.Vulkan.EvictionAllowBuffers"),
	GVulkanEvictionAllowBuffers,
	TEXT("Allow for buffers to be evicted.  Used to debug memory peaks. (default: 0)"),
	ECVF_RenderThreadSafe
);

static int32 GVulkanEvictionAllowRenderTargets = 0;
static FAutoConsoleVariableRef CVarVulkanEvictionAllowRenderTargets(
	TEXT("r.Vulkan.EvictionAllowRenderTargets"),
	GVulkanEvictionAllowRenderTargets,
	TEXT("Allow for render targets to be evicted.  Used to debug memory peaks. (default: 0)"),
	ECVF_RenderThreadSafe
);
#endif
...
namespace VulkanRHI
{
	...
	{
		const bool bHasMemoryBudget = Device->GetOptionalExtensions().HasMemoryBudget;
		...
		// Save Memory stats in FRHIMemoryStats format
		MemoryStats = FRHIMemoryStats{};
		uint64 HeapBudget, HeapUsed;

		for (uint32 HeapIndex = 0; HeapIndex < MemoryProperties.memoryHeapCount; ++HeapIndex)
		{
			const bool bLocalHeap = (MemoryProperties.memoryHeaps[HeapIndex].flags & VK_MEMORY_HEAP_DEVICE_LOCAL_BIT) != 0;

			if (bHasMemoryBudget)
			{
				HeapBudget = MemoryBudget.heapBudget[HeapIndex];
				HeapUsed = MemoryBudget.heapUsage[HeapIndex];
			}
			else
			{
				HeapBudget = MemoryProperties.memoryHeaps[HeapIndex].size;
				
				if (HeapIndex < (uint32)HeapInfos.Num())
				{
					HeapUsed = HeapInfos[HeapIndex].UsedSize;
				}
				else
				{
					HeapUsed = 0;
			...
			if (bLocalHeap)
			{
				MemoryStats.BudgetLocal += HeapBudget;
				MemoryStats.UsedLocal += HeapUsed;
			}
			else
			{
				MemoryStats.BudgetSystem += HeapBudget;
				MemoryStats.UsedSystem += HeapUsed;
			}
		}

		MemoryStats.AvailableLocal = (MemoryStats.UsedLocal < MemoryStats.BudgetLocal) ? (MemoryStats.BudgetLocal - MemoryStats.UsedLocal) : 0;
		MemoryStats.AvailableSystem = (MemoryStats.UsedSystem < MemoryStats.BudgetSystem) ? (MemoryStats.BudgetSystem - MemoryStats.UsedSystem) : 0;

		MemoryStats.DemotedLocal = (MemoryStats.UsedLocal > MemoryStats.BudgetLocal) ? (MemoryStats.UsedLocal - MemoryStats.BudgetLocal) : 0;
		MemoryStats.DemotedSystem = (MemoryStats.UsedSystem > MemoryStats.BudgetSystem) ? (MemoryStats.UsedSystem - MemoryStats.BudgetSystem) : 0;
	}
...
#if (UE_BUILD_DEBUG || UE_BUILD_DEVELOPMENT)
			case EVulkanAllocationMetaMultiBuffer: return (GVulkanEvictionAllowBuffers != 0);
			case EVulkanAllocationMetaImageRenderTarget: return (GVulkanEvictionAllowRenderTargets != 0);
#endif
	...
	void FDeviceMemoryManager::Deinit()
	{
		...
		for (int32 Index = 0; Index < HeapInfos.Num(); ++Index)
		{
			...
			{
				UE_LOGF(LogVulkanRHI, Warning, "Found %d unfreed allocations!", HeapInfos[Index].Allocations.Num());
				DumpMemory();
			}
		}
		NumAllocations = 0;
	...
	FDeviceMemoryAllocation* FDeviceMemoryManager::Alloc(bool bCanFail, VkDeviceSize AllocationSize, uint32 MemoryTypeIndex, void* DedicatedAllocateInfo, float Priority, bool bExternal, const char* File, uint32 Line)
	{
		...
		if (Result == VK_ERROR_OUT_OF_DEVICE_MEMORY || Result == VK_ERROR_OUT_OF_HOST_MEMORY)
		{
			if (bCanFail)
			{
				UE_LOGF(LogVulkanRHI, Warning, "Failed to allocate Device Memory, Requested=%.2fKb MemTypeIndex=%d", (float)Info.allocationSize / 1024.0f, Info.memoryTypeIndex);
				return nullptr;
			}
			...
			UE_LOGF(LogVulkanRHI, Fatal, "Out of %ls Memory, Requested%.2fKB MemTypeIndex=%d\n", MemoryType, AllocationSize / 1024.f, MemoryTypeIndex);
		}
		else
		{
		...
		{
			UE_LOGF(LogVulkanRHI, Warning, "Hit Maximum # of allocations (%d) reported by device!", NumAllocations);
		}
	...
	bool FVulkanResourceHeap::AllocateResource(FVulkanAllocation& OutAllocation, FVulkanEvictable* AllocationOwner, EType Type, uint32 Size, uint32 Alignment, bool bMapAllocation, bool bForceSeparateAllocation, EVulkanAllocationMetaType MetaType, bool bExternal, const char* File, uint32 Line)
	{
		...
		if (!DeviceMemoryAllocation)
		{
			UE_LOGF(LogVulkanRHI, Warning, "Out of memory on Vulkan; MemoryTypeIndex=%d, AllocSize=%0.3fMB", MemoryTypeIndex, (float)AllocationSize / 1048576.0f);
			return false;
		}
		if (bMapAllocation)
		{
		...
		// Download heap. Optional type per the spec.
		{
			uint32 TypeIndex = 0;
			{
				...
				else
				{
					...
					UE_LOGF(LogVulkanRHI, Fatal, "No Memory Type found supporting VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT!");
				}
			}
			uint64 HeapSize = MemoryProperties.memoryHeaps[MemoryProperties.memoryTypes[TypeIndex].heapIndex].size;
			ResourceTypeHeaps[TypeIndex] = new FVulkanResourceHeap(this, TypeIndex, STAGING_HEAP_PAGE_SIZE);
	...
	void FMemoryManager::DestroyResourceAllocations()
	{
		...
		for (auto& UsedAllocations : UsedBufferAllocations)
		{
			for (int32 Index = UsedAllocations.Num() - 1; Index >= 0; --Index)
			{
				...
				{
					UE_LOGF(LogVulkanRHI, Warning, "Suballocation(s) for Buffer %p were not released.%ls", (void*)BufferAllocation->Buffer, *VULKAN_TRACK_STRING(BufferAllocation->Track));
				}
	...
	void FMemoryManager::ReleaseFreedResources(bool bImmediately)
	{
		TArray<FVulkanSubresourceAllocator*> BufferAllocationsToRelease;
		{
			...
			for (auto& FreeAllocations : FreeBufferAllocations)
			{
				...
				{
					FVulkanSubresourceAllocator* BufferAllocation = FreeAllocations[Index];
					if (bImmediately || BufferAllocation->FrameFreed + GVulkanNumFramesToWaitForResourceDelete < GFrameNumberRenderThread)
					{
	...
	FVulkanSubresourceAllocator::~FVulkanSubresourceAllocator()
	{
		if (!JoinFreeBlocks())
		{
			UE_LOGF(LogVulkanRHI, Warning, "FVulkanSubresourceAllocator %p has unfreed %ls resources %ls", (void*)this, VulkanAllocationTypeToString(Type), *VULKAN_TRACK_STRING(Track));
			uint32 LeakCount = 0;
			for (FVulkanAllocationInternal& Data : InternalData)
			{
				if (Data.State == FVulkanAllocationInternal::EALLOCATED)
				{
					UE_LOGF(LogVulkanRHI, Warning, " ** LEAK %03d [%08x-%08x] %u  %ls \n%ls", LeakCount++, Data.AllocationOffset, Data.AllocationSize,  Data.Size, VulkanAllocationMetaTypeToString(Data.MetaType), *VULKAN_TRACK_STRING(Data.Track));
				}
			}
		}
	...
	static VkMemoryPropertyFlags GetMemoryPropertyFlags(EVulkanAllocationFlags AllocFlags, bool bHasUnifiedMemory)
	{
		...
		else
		{
			...
			if (EnumHasAnyFlags(AllocFlags, EVulkanAllocationFlags::Protected))
			{
				MemFlags |= VK_MEMORY_PROPERTY_PROTECTED_BIT;
			}
			else if (bHasUnifiedMemory)
			{
				MemFlags |= (VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
			}
		}
	...
	bool FMemoryManager::AllocateImageMemory(FVulkanAllocation& OutAllocation, FVulkanEvictable* AllocationOwner, const VkMemoryRequirements& MemoryReqs, VkMemoryPropertyFlags MemoryPropertyFlags, EVulkanAllocationMetaType MetaType, bool bExternal, const char* File, uint32 Line)
	{
		...
		if (DeviceMemoryManager->GetMemoryTypeFromProperties(MemoryReqs.memoryTypeBits, MemoryPropertyFlags, &TypeIndex) != VK_SUCCESS)
		{
			...
			{
				UE_LOGF(LogVulkanRHI, Fatal, "Cannot find memory type for MemSize %d, MemPropTypeBits %u, MemPropertyFlags %u, %ls(%d)", (uint32)MemoryReqs.size, (uint32)MemoryReqs.memoryTypeBits, (uint32)MemoryPropertyFlags, ANSI_TO_TCHAR(File), Line);
			}
		}
		if (!ResourceTypeHeaps[TypeIndex])
		{
			UE_LOGF(LogVulkanRHI, Fatal, "Missing memory type index %d, MemSize %d, MemPropTypeBits %u, MemPropertyFlags %u, %ls(%d)", TypeIndex, (uint32)MemoryReqs.size, (uint32)MemoryReqs.memoryTypeBits, (uint32)MemoryPropertyFlags, ANSI_TO_TCHAR(File), Line);
		}
		...
		if (!ResourceTypeHeaps[TypeIndex]->AllocateResource(OutAllocation, AllocationOwner, EType::Image, MemoryReqs.size, MemoryReqs.alignment, bMapped, bForceSeparateAllocation, MetaType, bExternal, File, Line))
		{
			...
			if (!ResourceTypeHeaps[TypeIndex]->AllocateResource(OutAllocation, AllocationOwner, EType::Image, MemoryReqs.size, MemoryReqs.alignment, bMapped, bForceSeparateAllocation, MetaType, bExternal, File, Line))
			{
				...
				UE_LOGF(LogVulkanRHI, Fatal, "Out Of Memory, trying to allocate %lld bytes\n", MemoryReqs.size);
				return false;
			}
		}
		return true;
	...
	bool FMemoryManager::AllocateBufferMemory(FVulkanAllocation& OutAllocation, FVulkanEvictable* AllocationOwner, const VkMemoryRequirements& MemoryReqs, VkMemoryPropertyFlags MemoryPropertyFlags, EVulkanAllocationMetaType MetaType, bool bExternal, bool bForceSeparateAllocation, const char* File, uint32 Line)
	{
		...
		if ((Result != VK_SUCCESS) || !ResourceTypeHeaps[TypeIndex])
		{
			...
			{
				UE_LOGF(LogVulkanRHI, Fatal, "Unable to find alternate type for index %d, MemSize %d, MemPropTypeBits %u, MemPropertyFlags %u, %ls(%d)",
			...
			{
				DumpMemory();
				UE_LOGF(LogVulkanRHI, Fatal, "Missing memory type index %d (originally requested %d), MemSize %d, MemPropTypeBits %u, MemPropertyFlags %u, %ls(%d)", TypeIndex, OriginalTypeIndex, (uint32)MemoryReqs.size, (uint32)MemoryReqs.memoryTypeBits, (uint32)MemoryPropertyFlags, ANSI_TO_TCHAR(File), Line);
			}
		}
	...
	bool FMemoryManager::AllocateDedicatedImageMemory(FVulkanAllocation& OutAllocation, FVulkanEvictable* AllocationOwner, VkImage Image, VkMemoryPropertyFlags MemoryPropertyFlags, EVulkanAllocationMetaType MetaType, bool bExternal, bool bForceDedicatedAllocation, const char* File, uint32 Line)
	{
		...
		const VkMemoryRequirements& MemoryReqs = MemoryReqs2.memoryRequirements;

		const bool bUseDedicated = bForceDedicatedAllocation || (DedMemoryReqs.prefersDedicatedAllocation != VK_FALSE || DedMemoryReqs.requiresDedicatedAllocation != VK_FALSE);
		if (bUseDedicated)
		{
			uint32 TypeIndex = 0;
			VERIFYVULKANRESULT(DeviceMemoryManager->GetMemoryTypeFromProperties(MemoryReqs.memoryTypeBits, MemoryPropertyFlags, &TypeIndex));
	...
	void FMemoryManager::DumpMemory(bool bFullDump)
	{
		...
		for (int32 PoolSizeIndex = 0; PoolSizeIndex < UE_ARRAY_COUNT(UsedBufferAllocations); PoolSizeIndex++)
		{
			...
			if (UsedAllocations.Num() > 0)
			{
				...
				for (int32 Index = 0; Index < UsedAllocations.Num(); ++Index)
				{
					...
					VULKAN_LOGMEMORY(TEXT("%6d 0x%016llx 0x%016llx 0x%06x 0x%08x %6d   %6d        %llu/%u"), Index, (void*)BA->Buffer, (void*)BA->MemoryAllocation->GetHandle(), BA->MemoryPropertyFlags, BA->BufferUsageFlags, BA->NumSubAllocations, BA->FreeList.Num(), BA->UsedSize, BA->MaxSize);
		...
		auto WriteLogLine = [](const FString& Name, FResourceHeapStats& Stat)
		{
			...
			VULKAN_LOGMEMORY(TEXT("\t\t%-33s  |%8.2fmb / %8.2fmb / %11.2fmb / %11.2fmb | %10lld %10lld | %6lld %6lld %6lld | %05x | %s"),
				*Name,
...
#undef VULKAN_LOGMEMORY_PAD2
	...
	void FMemoryManager::AllocUniformBuffer(FVulkanAllocation& OutAllocation, uint32 Size)
	{
		...
		if (!AllocateBufferPooled(OutAllocation, nullptr, Size, UBAlignment, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, EVulkanAllocationMetaUniformBuffer, __FILE__, __LINE__))
		{
			HandleOOM(false);
			checkNoEntry();
		}
	...
	{
		if (InAllocation.bTempBlockAllocator)
		{
			Device.GetDeferredDeletionQueue().EnqueueFunc([InAllocation](FVulkanDevice& Device)
			{
				Device.GetTempBlockAllocator().FreeReference(InAllocation);
			});
		}
		else if (InAllocation.HasAllocation())
		{
	...
	void FMemoryManager::ProcessPendingUBFreesNoLock(bool bForce)
	{
		...
		else
		{
			...
			int32 OldestFrameToKeep = GFrameNumberRenderThread - GVulkanNumFramesToWaitForResourceDelete;
			int32 NumAlloc = UBAllocations.PendingFree.Num();
			int32 Index = 0;
			for (; Index < NumAlloc; ++Index)
			{
				...
				else
				{
					...
					GFrameNumberRenderThread_WhenWeCanDelete = Alloc.Frame + GVulkanNumFramesToWaitForResourceDelete + 1;
					break;
				}
			}
	...
	FVulkanAllocation::FVulkanAllocation()
		: bHasOwnership(0), bTransient(false), bTempBlockAllocator(false)
	{
	...
	VkMemoryPropertyFlags FVulkanAllocation::GetMemoryPropertyFlags(FVulkanDevice* Device) const
	{
		const VkPhysicalDeviceMemoryProperties& MemoryProperties = Device->GetDeviceMemoryManager().GetMemoryProperties();
		const uint32 MemoryTypeIndex = GetSubresourceAllocator(Device)->GetMemoryTypeIndex();
		check(MemoryTypeIndex < MemoryProperties.memoryTypeCount);
	...
	void FStagingManager::Deinit()
	{
		...
		{
			UE_LOGF(LogVulkanRHI, Warning,
				"Some resources in the FStagingManager were not freed!  (UsedBuffer=%d, PendingFree=%d, FreeBuffer=%d)",
	...
	void FStagingManager::ProcessPendingFreeNoLock(bool bImmediately, bool bFreeToOS)
	{
		...
		if (bFreeToOS)
		{
			...
			{
				FFreeEntry& Entry = FreeStagingBuffers[Index];
				if (bImmediately || Entry.FrameNumber + GVulkanNumFramesToWaitForResourceDelete < GFrameNumberRenderThread)
				{
	...
	void FDeferredDeletionQueue2::EnqueueFunc(TUniqueFunction<void(FVulkanDevice&)>&& Func)
	{
		FEntry Entry;
		Entry.StructureType = EType::Func;
		Entry.FrameNumber = GVulkanRHIDeletionFrameNumber;
		Entry.Func = new TUniqueFunction<void(FVulkanDevice&)>(MoveTemp(Func));
		{
			FScopeLock ScopeLock(&CS);
			Entries.Add(Entry);
		}
...
#undef VKSWITCH
	...
	FTempBlockAllocator::FInternalAlloc FTempBlockAllocator::InternalAlloc(uint32 InSize, FVulkanContextCommon* Context)
	{
		...
		// Parallel scope: allocate in existing block from existing command buffer
		{
			...
			if (!Context || CurrentBlock->SyncPoints.Contains(Context->GetContextSyncPoint()))
			{
	...
	uint8* FTempBlockAllocator::Alloc(uint32 InSize, uint32 InAlignment, FVulkanContextCommon* Context, FVulkanAllocation& OutAllocation, VkDescriptorAddressInfoEXT* OutDescriptorAddressInfo)
	{
		...
		OutAllocation.bTempBlockAllocator = (Context == nullptr);
	...
	void FTempBlockAllocator::FreeReference(const FVulkanAllocation& InAllocation)
	{
		FReadScopeLock ScopeLock(RWLock);

		if ((InAllocation.VulkanHandle == (uint64)CurrentBlock->Buffer) &&
			(InAllocation.AllocatorIndex == CurrentBlock->Allocation.AllocatorIndex) &&
			(InAllocation.AllocationIndex == CurrentBlock->Allocation.AllocationIndex))
		{
			CurrentBlock->References--;
			return;
		}

		for (int32 Index = 0; Index < BusyBlocks.Num(); ++Index)
		{
			FTempMemoryBlock* Block = BusyBlocks[Index];
			FVulkanAllocation& BlockAllocation = Block->Allocation;
			if ((InAllocation.VulkanHandle == (uint64)Block->Buffer) &&
				(InAllocation.AllocatorIndex == BlockAllocation.AllocatorIndex) &&
				(InAllocation.AllocationIndex == BlockAllocation.AllocationIndex))
			{
				check(Block->References > 0);
				Block->References--;
				return;
			}
		}

		checkf(false, TEXT("Freed allocation's block was not found in the temp allocator!"));
	}
	...
	void FTempBlockAllocator::UpdateBlocks()
	{
		...
		for (int32 Index = BusyBlocks.Num() - 1; Index >= 0; --Index)
		{
			...
			bool BlockReady = (Block->References == 0);
...
// Patch external allocations into the engine's allocators
class FVulkanMemoryAllocator : public IVulkanMemoryAllocator
{
public:
	FVulkanMemoryAllocator(FVulkanDevice& InDevice) : Device(InDevice), MemoryManager(InDevice.GetMemoryManager()) {}
	virtual ~FVulkanMemoryAllocator() {}

	virtual VkResult AllocateImageMemory(VkImage Image, VkMemoryPropertyFlags Flags, bool bForceDedicatedAllocation, AllocHandle* OutAllocHandle) override final
	{
		const VulkanRHI::EVulkanAllocationMetaType MetaType = VulkanRHI::EVulkanAllocationMetaImageOther;

		// AllocateDedicatedImageMemory will let the driver decide if allocation should be dedicated
		VulkanRHI::FVulkanAllocation* Allocation = new VulkanRHI::FVulkanAllocation();
		if (Device.GetMemoryManager().AllocateDedicatedImageMemory(*Allocation, nullptr, Image, Flags, MetaType, false, bForceDedicatedAllocation, __FILE__, __LINE__))
		{
			(*OutAllocHandle) = Allocation;
			return VK_SUCCESS;
		}
		else
		{
			delete Allocation;
			(*OutAllocHandle) = nullptr;

			const bool bIsHostMemory = VKHasAnyFlags(Flags, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT);
			return bIsHostMemory ? VK_ERROR_OUT_OF_HOST_MEMORY : VK_ERROR_OUT_OF_DEVICE_MEMORY;
		}
	}

	virtual VkResult AllocateBufferMemory(VkBuffer Buffer, VkMemoryPropertyFlags Flags, bool bForceDedicatedAllocation, AllocHandle* OutAllocHandle) override final
	{
		VulkanRHI::EVulkanAllocationFlags UEAllocFlags = VulkanRHI::EVulkanAllocationFlags::None;

		const bool bIsHostMemory = VKHasAnyFlags(Flags, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT);
		if (bIsHostMemory)
		{
			UEAllocFlags |= VulkanRHI::EVulkanAllocationFlags::HostVisible;
		}
		if (VKHasAnyFlags(Flags, VK_MEMORY_PROPERTY_HOST_CACHED_BIT))
		{
			UEAllocFlags |= VulkanRHI::EVulkanAllocationFlags::HostCached;
		}
		if (VKHasAnyFlags(Flags, VK_MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT))
		{
			UEAllocFlags |= VulkanRHI::EVulkanAllocationFlags::Memoryless;
		}
		if (VKHasAnyFlags(Flags, VK_MEMORY_PROPERTY_PROTECTED_BIT))
		{
			UEAllocFlags |= VulkanRHI::EVulkanAllocationFlags::Protected;
		}
		if (bForceDedicatedAllocation)
		{
			UEAllocFlags |= VulkanRHI::EVulkanAllocationFlags::Dedicated;
		}

		VulkanRHI::FVulkanAllocation* Allocation = new VulkanRHI::FVulkanAllocation();
		if (MemoryManager.AllocateBufferMemory(*Allocation, Buffer, UEAllocFlags, TEXT("External")))
		{
			(*OutAllocHandle) = Allocation;
			return VK_SUCCESS;
		}
	...
	virtual void FreeMemory(const AllocHandle& AllocHandle) override final
	{
		VulkanRHI::FVulkanAllocation* Allocation = reinterpret_cast<VulkanRHI::FVulkanAllocation*>(AllocHandle);
		Allocation->Free(Device);
		delete Allocation;
	}

	virtual FVulkanRHIAllocationInfo GetAllocInfo(const AllocHandle& AllocHandle) override final
	{
		VulkanRHI::FVulkanAllocation* Allocation = reinterpret_cast<VulkanRHI::FVulkanAllocation*>(AllocHandle);
		FVulkanRHIAllocationInfo Info;
		Info.Handle = Allocation->GetDeviceMemoryHandle(&Device);
		Info.Offset = Allocation->Offset;
		Info.Size = Allocation->Size;
		Info.Flags = Allocation->GetMemoryPropertyFlags(&Device);
		return Info;
	}

	virtual void* MapMemory(const AllocHandle& AllocHandle) override final
	{
		VulkanRHI::FVulkanAllocation* Allocation = reinterpret_cast<VulkanRHI::FVulkanAllocation*>(AllocHandle);
		return Allocation->GetMappedPointer(&Device);
	}

	virtual void UnmapMemory(const AllocHandle& AllocHandle) override final
	{
		// Not needed for current allocator since all host visible blocks are always mapped
		// Keep the hooks around in case future allocators don't behave the same way
	}

	virtual void FlushMemory(const AllocHandle& AllocHandle, VkDeviceSize Offset, VkDeviceSize Size) override final
	{
		VulkanRHI::FVulkanAllocation* Allocation = reinterpret_cast<VulkanRHI::FVulkanAllocation*>(AllocHandle);
		if (Size == VK_WHOLE_SIZE)
		{
			check(Offset < Allocation->Size);
			Size = Allocation->Size - Offset;
		}
		else
		{
			checkf(Offset + Size <= Allocation->Size, TEXT("Flushing memory beyond the allocation size."));
		}
		VulkanRHI::FVulkanSubresourceAllocator* Allocator = Allocation->GetSubresourceAllocator(&Device);
		Offset += Allocation->Offset;
		Allocator->Flush(Offset, Size);
	}

	virtual void InvalidateMemory(const AllocHandle& AllocHandle, VkDeviceSize Offset, VkDeviceSize Size) override final
	{
		VulkanRHI::FVulkanAllocation* Allocation = reinterpret_cast<VulkanRHI::FVulkanAllocation*>(AllocHandle);
		if (Size == VK_WHOLE_SIZE)
		{
		...
		{
			checkf(Offset + Size <= Allocation->Size, TEXT("Invalidating memory beyond the allocation size."));
		}
		VulkanRHI::FVulkanSubresourceAllocator* Allocator = Allocation->GetSubresourceAllocator(&Device);
		Offset += Allocation->Offset;
		Allocator->Invalidate(Offset, Size);
	...
	virtual FRHIMemoryStats GetMemoryStats() override final
	{
		return Device.GetDeviceMemoryManager().GetRHIMemoryStats();
	}
...
IVulkanMemoryAllocator* FVulkanDynamicRHI::RHIGetVulkanMemoryAllocator()
{
	return new FVulkanMemoryAllocator(*Device);
}
