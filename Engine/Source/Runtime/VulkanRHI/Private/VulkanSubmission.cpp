...
FDynamicRHI::FRHIFinalizeContextsResult FVulkanDynamicRHI::RHIFinalizeContexts(FRHIFinalizeContextsArgs&& Args)
{
	...
	if (Args.UploadContext)
	{
		...
		Result.UploadCommandList = PlatformCmdList;
	...
	{
		if (FVulkanCommandListContext* Context = static_cast<FVulkanCommandListContext*>(Args.Contexts[Pipeline]))
		{
			if (FVulkanParallelRenderPassInfo* ParallelRPInfo = Context->GetParallelRenderPassInfo())
			{
				// Handle finalizing of parallel render pass contexts.
				if (Device->GetOptionalExtensions().HasKHRDynamicRendering)
				{
					FVulkanQueryPool* OptionalTimestampQueryPool = GRHIGlobals.SupportsTimestampRenderQueries 
						? Context->GetCurrentTimestampQueryPool()
						: nullptr;

					Context->GetCommandBuffer().EndDynamicRendering(OptionalTimestampQueryPool);
				}

				FScopeLock Lock(&GSecondaryPayloadsCS);
				Context->Finalize(ParallelRPInfo->SecondaryPayloads);
			}
			else
			{
				FVulkanPlatformCommandList* PlatformCmdList = new FVulkanPlatformCommandList;
				Result.CommandLists[Pipeline] = PlatformCmdList;

				Context->Finalize(*PlatformCmdList);
			}
...
IRHICommandContext* FVulkanDynamicRHI::RHIGetParallelCommandContext(FRHIParallelRenderPassInfo const& ParallelRenderPass)
{
	FVulkanParallelRenderPassInfo* VulkanParallelRenderPassInfo = static_cast<FVulkanParallelRenderPassInfo*>(ParallelRenderPass.RHIPlatformData);
	...
	FVulkanCommandListContext* ParallelCommandContext = new FVulkanCommandListContext(*Device, VulkanParallelRenderPassInfo);
	...
	if (Device->GetOptionalExtensions().HasKHRDynamicRendering)
	{
		...
		FVulkanQueryPool* OptionalTimestampQueryPool = GRHIGlobals.SupportsTimestampRenderQueries
			? ParallelCommandContext->GetCurrentTimestampQueryPool()
			: nullptr;

		const int32 ContextIndex = VulkanParallelRenderPassInfo->NumParallelContexts++;
		if (ContextIndex == 0)
		{
			// The first context has nothing to resume, it only needs the suspend flag
			// NOTE: a dummy pass will be created to resume without suspending at the end
			...
			ParallelCommandContext->GetCommandBuffer().BeginDynamicRendering(RenderingInfo, OptionalTimestampQueryPool);
		}
		else
		{
			ParallelCommandContext->GetCommandBuffer().BeginDynamicRendering(VulkanParallelRenderPassInfo->DynamicRenderingInfo->RenderingInfo, OptionalTimestampQueryPool);
		}
	}
...
bool FVulkanDynamicRHI::WaitAndProcessInterruptQueue()
{
	...
	// Wait for progress
	{
		...
		Device->ForEachQueue([&NumSyncs, &NextPayloads, &bProgress](FVulkanQueue& Queue)
		{
			...
			if (FVulkanPayload* Payload = Queue.PendingInterrupt.Peek())
			{
	...
	for (TArray<FVulkanQueryPool*>& QueryPoolArray : Payload->QueryPools)
	{
		for (FVulkanQueryPool* QueryPool : QueryPoolArray)
		{
			...
			while (Result >= VK_SUCCESS)
			{
				...
				else if (GGPUCrashDetected.load())
				{
			...
			for (uint32 QueryIndex = 0; QueryIndex < QueryPool->CurrentQueryCount; ++QueryIndex)
			{
				...
				if (!ResultPtr)
					...
					for (uint32 ViewIndex = 0; ViewIndex < QueryPool->QueryStride; ++ViewIndex)
					{
						...
						{
							UE_LOGF(LogVulkanRHI, Warning, "Could not read back query: [PoolType=%d, Index=%d, View=%d]", (int32)QueryPool->GetPoolType(), QueryIndex, ViewIndex);
						}
					}
								...
								if (!bBypassTiming)
								{
									...
									if (!bWarnOnce)
									{
										...
										UE_LOGF(LogVulkanRHI, Warning,
											"GPU timestamp can't be moved to CPU timeline.  " "Calibration data is invalid (Curr=%lld  Cal=%lld)!  " "CPU and GPU profiling data will not line up!",
										QueryResult, GPUCalibrationTimestamp);
									}

									const uint64 CurrentTimestamp = FPlatformTime::Cycles64();

							ReferenceCPUTimestamp = ((CPUCalibrationTimestamp != 0) && (CPUCalibrationTimestamp < CurrentTimestamp))
