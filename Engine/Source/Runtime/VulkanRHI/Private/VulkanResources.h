...
class FVulkanShaderModule : public FRefCountedObject
{
...
public:
	...
	public:
		...
		int32 GetCodeSizeInBytes() {return CodeView.Num() * sizeof(uint32);}
	};
	...
	VkShaderEXT GetShaderObject() const
	{
		return ShaderObject;
	}
...
protected:
	...
	FGraphEventRef					AsyncCompileTask;
...
protected:
	...
	friend class FVulkanShaderObjectManager;
	friend class FVulkanGenericPlatform;
};
...
public:
	...
	virtual FReturnedRefCountValue Release() const override final
	{
		return FReturnedRefCountValue{ FRHIResource::Release() };
	}
	virtual FReturnedRefCountValue GetRefCount() const override final
	{
		return FReturnedRefCountValue{ FRHIResource::GetRefCount() };
	}
};
...
public:
	...
	FVulkanView(FVulkanDevice& InDevice, ERHIDescriptorType InDescriptorType);
...
{
protected:
	FVulkanLinkedView(FVulkanDevice& Device, ERHIDescriptorType DescriptorType)
...
public:
	...
	*/
	...
	static void InternalLockWrite(FVulkanContextCommon& Context, FVulkanTexture* Surface, const VkBufferImageCopy& Region, VulkanRHI::FStagingBuffer* StagingBuffer, bool bNeedsTransitions);
...
public:
	...
	void UpdateResourceTable(TArrayView<FRHIResource*> Resources);
	...
	FRHIDescriptorHandle GetBindlessHandle() const override
	{
		return BindlessHandle;
	}
	
	void UpdateBindlessHandle(const FVulkanContextArray& Contexts, bool bImmediate);
	VkDeviceAddress GetDeviceAddress() const;

	void SetupUniformBufferView();

	void AllocateMemory(const void* InitialData);

public:
	const EUniformBufferUsage Usage;
