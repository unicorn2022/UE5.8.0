...
static int32 GQueryUseControlPrecisionBit = 1;
FAutoConsoleVariableRef CVarQueryUseControlPrecisionBit(
	TEXT("r.Vulkan.QueryUseControlPrecisionBit"),
	GQueryUseControlPrecisionBit,
	TEXT("Whether to use VK_QUERY_CONTROL_PRECISE_BIT when doing a render query. (Default : 1 (on), 0 (off))\n"),
	ECVF_ReadOnly
);
	...
	, QueryType(InQueryType)
{
	...
	VkQueryPoolCreateInfo PoolCreateInfo = {
		...
		.pNext = nullptr,
		.flags = 0,
		.queryType = GetVkQueryType(InQueryType),
		.queryCount = MaxQueries,
		.pipelineStatistics = 0
	};

	// With maintenance9, always set an initial state for the queries
	if (Device.GetOptionalExtensions().HasKHRMaintenance9)
	{
		QueryRefs.AddDefaulted(MaxQueries / QueryStride);
		QueryResults.SetNumZeroed(MaxQueries / QueryStride);
		PoolCreateInfo.flags |= VK_QUERY_POOL_CREATE_RESET_BIT_KHR;
		bIsDirty = false;
	}
...
void FVulkanQueryPool::Reset(FVulkanCommandBuffer* InCmdBuffer, uint32 InQueryStride)
{
	...
	{
		checkf(Device.GetOptionalExtensions().HasEXTHostQueryReset, TEXT("VK_EXT_host_query_reset must be supported to reset queries without a command buffer."));
...
{
	const uint32 QueryStride = MultiViewCount == 0 ? 1 : MultiViewCount;
	...
	if (NewOcclusionQueryPool->IsDirty() || (NewOcclusionQueryPool->GetQueryStride() != QueryStride))
	{
		FVulkanCommandBuffer* CommandBuffer = Device.GetOptionalExtensions().HasEXTHostQueryReset ? nullptr : &GetCommandBuffer();
		NewOcclusionQueryPool->Reset(CommandBuffer, QueryStride);
	}
...
void FVulkanCommandListContext::RHIBeginRenderQuery(FRHIRenderQuery* QueryRHI)
{
	...
	if (Query->QueryType == RQT_Occlusion)
	{
		...
		if (GQueryUseControlPrecisionBit > 0)
		{
			Flags |= VK_QUERY_CONTROL_PRECISE_BIT;
		}
		VulkanRHI::vkCmdBeginQuery(GetCommandBuffer().GetHandle(), CurrentOcclusionQueryPool->GetHandle(), Query->IndexInPool, Flags);
	}
	else if (Query->QueryType == RQT_AbsoluteTime)
	{
...
FVulkanQueryPool* FVulkanContextCommon::GetCurrentTimestampQueryPool(FVulkanPayload& Payload)
{
	...
	if ((TimestampPoolArray.Num() == 0) || TimestampPoolArray.Last()->IsFull() || TimestampPoolArray.Last()->GetQueryStride() != DesiredQueryStride)
	{
		...
		if (NewPool->IsDirty() || (NewPool->GetQueryStride() != DesiredQueryStride))
		{
			...
			if (!Device.GetOptionalExtensions().HasEXTHostQueryReset)
			{
				...
				CommandBuffer = Payload.CommandBuffers.Last();
			}
			NewPool->Reset(CommandBuffer, DesiredQueryStride);
		}
