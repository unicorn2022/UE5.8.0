...
void FVulkanSemaphore::GetTimelineSemaphoreValue(uint64& OutValue)
{
	...
	VERIFYVULKANRESULT(VulkanRHI::vkGetSemaphoreCounterValueKHR(Device.GetHandle(), SemaphoreHandle, &TempValue));
	if (TempValue != MAX_uint64)
	{
		OutValue = TempValue;
	}
	else
	{
		// Limit how many times the message is printed (but leave a few so it is visible)
		static int32 MessageCounter = 0;
		if (MessageCounter++ < 10)
		{
			UE_LOGF(LogVulkanRHI, Warning, "Timeline semaphore value is MAX_uint64 (this can happen when the device is lost).");
		}
	}
}
