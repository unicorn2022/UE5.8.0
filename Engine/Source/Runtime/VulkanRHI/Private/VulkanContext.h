...
struct FVulkanDynamicRenderingInfo
{
	...
	// View tracking used for input attachments
	TStaticArray<FVulkanView*, MaxSimultaneousRenderTargets> ColorAttachmentViews;
	FVulkanView* PartialDepthAttachmentView;
};
...
protected:
	...
	void AppendParallelRenderPayload(FVulkanPayload* ParallelRenderingPayload, VkRenderingInfo* NeedsResumeInfo);

private:

	FVulkanQueryPool* GetCurrentTimestampQueryPool(FVulkanPayload& Payload);
	void PrepareNewCommandBuffer(FVulkanPayload& Payload, bool bAllowTimestamp=true);
	void NewPayload();
	void EndPayload();
...
{
public:
	FVulkanCommandListContext(FVulkanDevice& InDevice, ERHIPipeline InPipeline, bool bInImmediate);
	virtual ~FVulkanCommandListContext();

	// Constructor for parallel render contexts that use secondary command buffers
	FVulkanCommandListContext(FVulkanDevice& InDevice, FVulkanParallelRenderPassInfo* InParallelRenderPassInfo);
	...
	FVulkanDynamicRenderingInfo* CurrentDynamicRenderingInfo = nullptr;
