...
int32 GVulkanAllowConcurrentImage = 0;
static TAutoConsoleVariable<int32> GCVarAllowConcurrentImage(
	...
	TEXT(" 0 to use queue family ownership transfers with images (default)\n")
...
static VkImageLayout GetInitialLayoutFromRHIAccess(ERHIAccess RHIAccess, bool bIsDepthStencilTarget, bool bSupportReadOnlyOptimal)
{
	...
	case ERHIAccess::CPURead:	return VK_IMAGE_LAYOUT_UNDEFINED;
...
void FVulkanTexture::InternalLockWrite(FVulkanContextCommon& Context, FVulkanTexture* Surface, const VkBufferImageCopy& Region, VulkanRHI::FStagingBuffer* StagingBuffer, bool bNeedsTransitions)
{
	...
	// We might skip transitions in passes where copies are batched
	...
	if (bNeedsTransitions)
	{
		const bool bIsTargetable = EnumHasAnyFlags(Surface->GetDesc().Flags, TexCreate_RenderTargetable | TexCreate_DepthStencilTargetable);
		const ERHIAccess DefaultAccess = RHIGetDefaultResourceState(Surface->GetDesc().Flags, false);
		const VkImageLayout DestLayout = GetInitialLayoutFromRHIAccess(DefaultAccess, bIsTargetable && Surface->IsDepthOrStencilAspect(), Surface->SupportsSampling());

		FVulkanPipelineBarrier Barrier;
		Barrier.AddImageLayoutTransition(Surface->Image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, DestLayout, SubresourceRange);
		Barrier.Execute(CmdBuffer);
	}
...
{
	UE_LOGF(LogVulkanRHI, Error, "Invalid ViewType %ls", VK_TYPE_TO_STRING(VkImageViewType, GetViewType()));
}
	...
	bool bForceLinearTexture)
{
	...
	if (InDevice.GetOptionalExtensions().HasEXTFragmentDensityMapOffset && GRHISupportsVariableRateShadingImageOffsets)
	{
		ImageCreateInfo.flags |= VK_IMAGE_CREATE_FRAGMENT_DENSITY_MAP_OFFSET_BIT_EXT;
	}
	...
	{
		UE_LOGF(LogVulkanRHI, Warning, "Not allowed to create Linear textures with %d samples, reverting to 1 sample", NumSamples);
		NumSamples = 1;
	}
...
FVulkanDynamicRHI::FCreateTextureResult FVulkanDynamicRHI::BeginCreateTextureInternal(const FRHITextureCreateDesc& CreateDesc, const FRHITransientHeapAllocation* InTransientHeapAllocation)
{
	...
	const bool bIsTargetable = EnumHasAnyFlags(CreateDesc.Flags, TexCreate_RenderTargetable | TexCreate_DepthStencilTargetable);
	const bool bDoInitialClear = VKHasAnyFlags(Texture->ImageUsageFlags, VK_IMAGE_USAGE_SAMPLED_BIT) && bIsTargetable && !bIsTransientResource;

	FCreateTextureResult Result{};
	Result.Texture = Texture;
	Result.DefaultLayout = GetInitialLayoutFromRHIAccess(CreateDesc.InitialState, bIsTargetable && Texture->IsDepthOrStencilAspect(), Texture->SupportsSampling());
...
void FVulkanTexture::UploadInitialData(FRHICommandListBase& RHICmdList, VulkanRHI::FStagingBuffer* UploadBuffer)
{
	RHICmdList.EnqueueLambda(TEXT("FVulkanTexture::UploadInitialData"),
		[this, UploadBuffer](FRHICommandListBase& ExecutingCmdList) mutable
		{
			...
			// The layers refer to slices in 3D textures created with VK_IMAGE_CREATE_2D_ARRAY_COMPATIBLE_BIT if Maintenance9 is enabled 
			const bool bUseSlices =
				(Desc.Dimension == ETextureDimension::Texture3D) &&
				Device->GetOptionalExtensions().HasKHRMaintenance9 &&
				!EnumHasAnyFlags(Desc.Flags, TexCreate_ReservedResource);

			const VkImageSubresourceRange SubresourceRange = FVulkanPipelineBarrier::MakeSubresourceRange(FullAspectMask, 0, Desc.NumMips, 0, bUseSlices ? VK_REMAINING_ARRAY_LAYERS : LayerCount);
...
{
	UE_LOGF(LogVulkan, Fatal, "RHIAsyncCreateTexture2D is not supported");
	VULKAN_SIGNAL_UNIMPLEMENTED();
...
void FVulkanDynamicRHI::RHIUnlockTexture(FRHICommandListImmediate& RHICmdList, const FRHILockTextureArgs& Arguments)
{
	...
	const bool bNeedsTransition = RHICmdList.NeedsExtraTransitions();
	RHICmdList.EnqueueLambda(TEXT("FVulkanTexture::InternalLockWrite"),
		[Texture, Region, StagingBuffer, bNeedsTransition](FRHICommandListBase& ExecutingCmdList)
		{
			FVulkanTexture::InternalLockWrite(FVulkanCommandListContext::Get(ExecutingCmdList), Texture, Region, StagingBuffer, bNeedsTransition);
		});
}
...
void FVulkanDynamicRHI::InternalUpdateTexture3D(FRHICommandListBase& RHICmdList, FRHITexture* TextureRHI, uint32 MipIndex, const FUpdateTextureRegion3D& UpdateRegion, uint32 SourceRowPitch, uint32 SourceDepthPitch, const uint8* SourceData)
{
	...
	// The layers refer to slices in 3D textures created with VK_IMAGE_CREATE_2D_ARRAY_COMPATIBLE_BIT if Maintenance9 is enabled 
	// (Maintenance5 allows the use of VK_REMAINING_ARRAY_LAYERS for copies)
	const bool bUseSlices = Device->GetOptionalExtensions().HasKHRMaintenance5 && Device->GetOptionalExtensions().HasKHRMaintenance9 && !EnumHasAnyFlags(Texture->GetDesc().Flags, TexCreate_ReservedResource);

	const VkBufferImageCopy Region = {
		.bufferOffset = 0,
		.bufferRowLength = 0,   // zero assumes tightly packed buffer
		.bufferImageHeight = 0, // zero assumes tightly packed buffer
		.imageSubresource = {
			.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT,
			.mipLevel = MipIndex,
			.baseArrayLayer = 0,
			.layerCount = bUseSlices ? VK_REMAINING_ARRAY_LAYERS : 1
		},
		.imageOffset = { 
			.x = (int32)UpdateRegion.DestX, 
			.y = (int32)UpdateRegion.DestY,
			.z = (int32)UpdateRegion.DestZ
		},
		.imageExtent = {
			.width = (uint32)FMath::Min((int32)(TextureSizeX - UpdateRegion.DestX), (int32)UpdateRegion.Width),
			.height = (uint32)FMath::Min((int32)(TextureSizeY - UpdateRegion.DestY), (int32)UpdateRegion.Height),
			.depth = (uint32)FMath::Min((int32)(TextureSizeZ - UpdateRegion.DestZ), (int32)UpdateRegion.Depth)
		}
	};
	...
	, ImageOwnerType(EImageOwnerType::LocalOwner)
{
	...
	if (EnumHasAnyFlags(InCreateDesc.Flags, TexCreate_ReservedResource))
	{
		...
		checkf(SparseBlockSize % ReservedResourceData->MemoryRequirements.alignment == 0,
			TEXT("The value of r.Vulkan.SparseImageAllocSizeMB (%dMB) is not compatible with this resource's tile size requirements (%lld)."),
		...
		checkf(ReservedResourceData->LastBlockSize % ReservedResourceData->MemoryRequirements.alignment == 0,
			TEXT("The value of GRHIGlobals.ReservedResources.TileSizeInBytes (%d) is not compatible with this resource's tile size requirements (%lld)."),
	...
	else
	{
		...
		else
		{
			...
			{
				constexpr bool bForceDedicated = false;  // it should be considered for dedicated, but don't force it and let driver decide
				if (!InDevice.GetMemoryManager().AllocateDedicatedImageMemory(Allocation, this, Image, MemoryFlags, MetaType, bExternal, bForceDedicated, __FILE__, __LINE__))
				{
					checkNoEntry();
				}
			}
...
static void CleanupVulkanAndroidTextureResources(void* UserData)
{
	...
	if (VulkanResources->Device)
	{
		VulkanResources->Device->GetDeferredDeletionQueue().EnqueueFunc([VulkanResources](FVulkanDevice& Device)
		{
			if (VulkanResources->SamplerYcbcrConversion != VK_NULL_HANDLE)
			{
				VulkanRHI::vkDestroySamplerYcbcrConversion(Device.GetHandle(), VulkanResources->SamplerYcbcrConversion, VULKAN_CPU_ALLOCATOR);
			}

			if (VulkanResources->DeviceMemory != VK_NULL_HANDLE)
			{
				VulkanRHI::vkFreeMemory(Device.GetHandle(), VulkanResources->DeviceMemory, VULKAN_CPU_ALLOCATOR);
			}

			if (VulkanResources->Image != VK_NULL_HANDLE)
			{
				VulkanRHI::vkDestroyImage(Device.GetHandle(), VulkanResources->Image, VULKAN_CPU_ALLOCATOR);
			}
	...
	, ImageOwnerType(EImageOwnerType::ExternalOwner)
{
	...
	VkImage VulkanImage;
	VERIFYVULKANRESULT(VulkanRHI::vkCreateImage(VulkanDevice, &ImageCreateInfo, VULKAN_CPU_ALLOCATOR, &VulkanImage));
	...
	VERIFYVULKANRESULT(VulkanRHI::vkAllocateMemory(VulkanDevice, &MemoryAllocateInfo, VULKAN_CPU_ALLOCATOR, &VulkanDeviceMemory));
	...
	VkSamplerYcbcrConversion SamplerYcbcrConversion;
	VERIFYVULKANRESULT(VulkanRHI::vkCreateSamplerYcbcrConversion(VulkanDevice, &SamplerYcbcrConversionCreateInfo, VULKAN_CPU_ALLOCATOR, &SamplerYcbcrConversion));
...
void FVulkanTexture::InitViews(VkImageUsageFlags ViewUsageFlags, VkSamplerYcbcrConversion SamplerYcbcrConversion)
{
	if (Image != VK_NULL_HANDLE && GetViewType() != VK_IMAGE_VIEW_TYPE_MAX_ENUM)
	{
		...
		{
			const ERHIDescriptorType DescriptorType =
				(FullAspectMask != PartialAspectMask) ? ERHIDescriptorType::Invalid :
				SupportsSampling() ? ERHIDescriptorType::TextureSRV : ERHIDescriptorType::TextureUAV;
...
void FVulkanTexture::UpdateViews(const FVulkanContextArray& Contexts, VkImageUsageFlags ViewUsageFlags, VkSamplerYcbcrConversion SamplerYcbcrConversion)
{
	...
	{
		const FVulkanTextureViewDesc ViewDesc = CreateTextureViewDesc(this, ViewUsageFlags, SamplerYcbcrConversion);
		const ERHIDescriptorType DescriptorType = SupportsSampling() ? ERHIDescriptorType::TextureSRV : ERHIDescriptorType::TextureUAV;
...
FVulkanView* FVulkanTexture::FindOrAddInternalView(const FVulkanTextureViewDesc& ViewDesc)
{
	...
	if (!VulkanView)
	{
		// When creating a view for a framebuffer, only VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT will be set
		const ERHIDescriptorType DescriptorType = VKHasAnyFlags(ViewDesc.ImageUsageFlags, VK_IMAGE_USAGE_SAMPLED_BIT) ? ERHIDescriptorType::TextureSRV : ERHIDescriptorType::Invalid;
		VulkanView = new FVulkanView(*Device, DescriptorType);
...
void FVulkanCommandListContext::RHICopyTexture(FRHITexture* SourceTexture, FRHITexture* DestTexture, const FRHICopyTextureInfo& CopyInfo)
{
	...
	if (EnumHasAllFlags(Dest->GetDesc().Flags, TexCreate_CPUReadback))
	{
		...
		for (uint32 Index = 0; Index < CopyInfo.NumMips; ++Index)
		{
			...
			CopyRegion[Index].imageSubresource.mipLevel = SourceMipIndex + Index;
