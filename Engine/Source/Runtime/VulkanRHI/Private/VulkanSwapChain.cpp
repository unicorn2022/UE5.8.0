...
#define VERIFYVULKANSURFACERESULT(VkFunction) \
	{\
		...
		/* Bail gracefully on the recoverable surface-state errors: \
		 *  - SURFACE_LOST_KHR: the surface backing has gone away. \
		 *  - INITIALIZATION_FAILED: on Linux X11/XWayland a destroyed underlying window \
		 *    surfaces here (driver-translated X11 BadWindow). \
		 *  - NATIVE_WINDOW_IN_USE_KHR: another swapchain still holds the surface, or \
		 *    another API holds the native window. Common during rapid popup churn on \
		 *    XWayland when an old viewport's swapchain is still being torn down. \
		 * Returning null lets the caller retry next frame; fatal handling kills the \
		 * editor mid-Slate-popup. */ \
		if (ScopedResult == VK_ERROR_SURFACE_LOST_KHR || \
		    ScopedResult == VK_ERROR_INITIALIZATION_FAILED || \
		    ScopedResult == VK_ERROR_NATIVE_WINDOW_IN_USE_KHR) \
		{ \
			UE_LOGF(LogVulkanRHI, Warning, "Swapchain surface bail (%ls) at %ls; returning null for caller retry.", \
				VK_TYPE_TO_STRING(VkResult, ScopedResult), TEXT(#VkFunction)); \
			DestroyRecreateInfo(); \
	...
	if (NumFormats == 0)
	{
		UE_LOGF(LogVulkanRHI, Warning, "vkGetPhysicalDeviceSurfaceFormatsKHR returned 0 formats! Handling as surface lost.");
		VERIFYVULKANSURFACERESULT(VK_ERROR_SURFACE_LOST_KHR);
	}
	...
	for (int32 Index = 1; Index < Formats.Num(); ++Index)
	{
		if (Formats[Index].colorSpace != RequestedColorSpace)
		{
			...
			case EDisplayOutputFormat::HDR_ACES_2000nit_ST2084:
				...
				break;
			default:
				UE_LOGF(LogVulkanRHI, Warning, "Requested color format %d not supported in Vulkan, falling back to sRGB. Please check the value of r.HDR.Display.OutputDevice.", int(OutputDevice));
				RequestedColorSpace = VK_COLOR_SPACE_SRGB_NONLINEAR_KHR;
				break;
			}
	...
	{
		check(InOutPixelFormat != PF_Unknown);

		{
			...
			if (GPixelFormats[InOutPixelFormat].Supported)
			{
				...
				if (!bFound)
				{
					if (!GPixelFormatNotSupportedWarning.Contains(InOutPixelFormat))
					{
						...
						UE_LOGF(LogVulkanRHI, Display, "Requested PixelFormat %d not supported by this swapchain! Falling back to supported swapchain format...", (uint32)InOutPixelFormat);
					}
			...
			{
				UE_LOGF(LogVulkanRHI, Warning, "Requested PixelFormat %d not supported by this Vulkan implementation!", (uint32)InOutPixelFormat);
				InOutPixelFormat = PF_Unknown;
			}
		}
			...
			for (int32 Index = 0; Index < Formats.Num(); ++Index)
			{
				...
				for (int32 PFIndex = 0; PFIndex < PF_MAX; ++PFIndex)
				{
					if (Formats[Index].format == GPixelFormats[PFIndex].PlatformFormat && Formats[Index].colorSpace == RequestedColorSpace)
					{
						...
						CurrFormat = Formats[Index];
						UE_LOGF(LogVulkanRHI, Verbose, "No swapchain format requested, picking up VulkanFormat %ls", VK_TYPE_TO_STRING(VkFormat, CurrFormat.format));
						break;
					}
				}
		...
		if (InOutPixelFormat == PF_Unknown)
		{
			UE_LOGF(LogVulkanRHI, Warning, "Can't find a proper pixel format for the swapchain, trying to pick up the first available");
		...
		if (InOutPixelFormat == PF_Unknown)
		{
			...
			UE_LOGF(LogVulkanRHI, Fatal, "Unable to find a pixel format for the swapchain; swapchain returned %d Vulkan formats %ls", Formats.Num(), *Msg);
		}
	}

	if (CurrFormat.colorSpace != RequestedColorSpace)
	{
		UE_LOGF(LogVulkanRHI, Warning, "Requested color format %d not supported by this Vulkan implementation, falling back to %d. Please check the value of r.HDR.Display.OutputDevice.", (int32)RequestedColorSpace, (int32)CurrFormat.colorSpace);
	}
	...
	if (FVulkanPlatform::SupportsQuerySurfaceProperties())
	{
		...
		UE_CLOGF(bFirstTimeLog, LogVulkanRHI, Display, "Found %d Surface present modes:", NumFoundPresentModes);
		...
		for (size_t i = 0; i < NumFoundPresentModes; i++)
		{
			...
			case VK_PRESENT_MODE_MAILBOX_KHR:
				...
				UE_CLOGF(bFirstTimeLog, LogVulkanRHI, Display, "- VK_PRESENT_MODE_MAILBOX_KHR (%d)", (int32)VK_PRESENT_MODE_MAILBOX_KHR);
				break;
			case VK_PRESENT_MODE_IMMEDIATE_KHR:
				bFoundPresentModeImmediate = true;
				UE_CLOGF(bFirstTimeLog, LogVulkanRHI, Display, "- VK_PRESENT_MODE_IMMEDIATE_KHR (%d)", (int32)VK_PRESENT_MODE_IMMEDIATE_KHR);
				break;
			case VK_PRESENT_MODE_FIFO_KHR:
				bFoundPresentModeFIFO = true;
				UE_CLOGF(bFirstTimeLog, LogVulkanRHI, Display, "- VK_PRESENT_MODE_FIFO_KHR (%d)", (int32)VK_PRESENT_MODE_FIFO_KHR);
				break;
			case VK_PRESENT_MODE_FIFO_RELAXED_KHR:
				UE_CLOGF(bFirstTimeLog, LogVulkanRHI, Display, "- VK_PRESENT_MODE_FIFO_RELAXED_KHR (%d)", (int32)VK_PRESENT_MODE_FIFO_RELAXED_KHR);
				break;
			default:
				UE_CLOGF(bFirstTimeLog, LogVulkanRHI, Display, "- VkPresentModeKHR %d", (int32)FoundPresentModes[i]);
				break;
			}
		}
		...
		if (FParse::Value(FCommandLine::Get(), TEXT("vulkanpresentmode="), RequestedPresentMode))
		{
			...
			{
				UE_CLOGF(bFirstTimeLog, LogVulkanRHI, Warning, "Requested PresentMode (%d) is not handled or available, ignoring...", RequestedPresentMode);
		...
		if (RequestedPresentMode == -1)
		{
			...
			{
				UE_LOGF(LogVulkanRHI, Warning, "Couldn't find desired PresentMode! Using %ls", VK_TYPE_TO_STRING(VkPresentModeKHR, FoundPresentModes[0]));
				PresentMode = FoundPresentModes[0];
			}
		}

		UE_CLOGF(bFirstTimeLog, LogVulkanRHI, Display, "Selected VkPresentModeKHR mode %ls", VK_TYPE_TO_STRING(VkPresentModeKHR, PresentMode));
		bFirstTimeLog = false;
	}
	...
	if (((*InOutDesiredNumBackBuffers) < SurfProperties.minImageCount) || 
		((SurfProperties.maxImageCount > 0) && ((*InOutDesiredNumBackBuffers) > SurfProperties.maxImageCount)))
	{
		UE_LOGF(LogVulkanRHI, Warning, "VkSurfaceCapabilitiesKHR returned [%u,%u] image count, requested %u", 
			SurfProperties.minImageCount, SurfProperties.maxImageCount, *InOutDesiredNumBackBuffers);
	}
	...
	{
		UE_LOGF(LogVulkanRHI, Log, "Creating new VK swapchain with %ls, %ls, %ls, num images %d",
	...
	{
		// Unlink fullscreen
		UE_LOGF(LogVulkanRHI, Warning, "Create swapchain failed with Initialization error; removing FullScreen extension...");
...
int32 FVulkanSwapChain::AcquireImageIndex(TRefCountPtr<FVulkanSemaphore>* OutSemaphore)
{
	...
	if (Result == VK_ERROR_VALIDATION_FAILED_EXT)
	{
		...
		{
			UE_LOGF(LogVulkanRHI, Fatal, "vkAcquireNextImageKHR failed with Validation error. Try running with r.Vulkan.EnableValidation=1 to get information from the driver");
		}
	}
	else
#endif
	...
	if (!bPlatformHandlesFramePacing)
	{
		...
		if (GVulkanCPURHIFramePacer && FramePace > 0)
		{
			...
			if (TimeToSleep > 0.0)
			{
				...
				{
					UE_LOGF(LogVulkanRHI, Log, "CurrentID: %i, CPU TimeToSleep: %f, TargetWEps: %f", PresentID, TimeToSleep * 1000.0, TargetIntervalWithEpsilon * 1000.0);
				}
			}
			else
			{
				if (GPrintVulkanVsyncDebug)
				{
					UE_LOGF(LogVulkanRHI, Log, "CurrentID: %i, CPU TimeToSleep: %f", PresentID, TimeToSleep * 1000.0);
				}
			}
...
void FVulkanDevice::SetupPresentQueue(VkSurfaceKHR Surface)
{
	if (!PresentQueue)
	{
		const auto SupportsPresent = [Surface](VkPhysicalDevice PhysicalDevice, FVulkanQueue* Queue)
		{
			...
			if (bSupportsPresent)
			{
				UE_LOGF(LogVulkanRHI, Display, "Queue Family %d: Supports Present", FamilyIndex);
			}
