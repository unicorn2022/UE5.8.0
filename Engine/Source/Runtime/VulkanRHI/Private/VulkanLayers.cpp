	...
	TArray<FLayerWithExtensions> EnumerateLayerProperties() const
	{
		...
		if (Count > 0)
		{
			...
			for (uint32 i=0; i < Count; ++i)
			{
				...
				UE_LOGF(LogVulkanRHI, Display, "Intance Layer %ls provides %d extensions:", ANSI_TO_TCHAR(TempLayerProperties[i].layerName), (int)OutLayerProperties[i].ExtensionProperties.Num());
				for (VkExtensionProperties ep : OutLayerProperties[i].ExtensionProperties)
				{
					UE_LOGF(LogVulkanRHI, Display, "  %ls", ANSI_TO_TCHAR(ep.extensionName));
				}
			}
		}
	...
	TArray<FLayerWithExtensions> EnumerateLayerProperties() const
	{
		...
		if (Count > 0)
		{
			...
			for (uint32 i = 0; i < Count; ++i)
			{
				...
				UE_LOGF(LogVulkanRHI, Display, "Device Layer %ls provides %d extensions:", ANSI_TO_TCHAR(TempLayerProperties[i].layerName), (int)OutLayerProperties[i].ExtensionProperties.Num());
				for (VkExtensionProperties ep : OutLayerProperties[i].ExtensionProperties)
				{
					UE_LOGF(LogVulkanRHI, Display, "  %ls", ANSI_TO_TCHAR(ep.extensionName));
				}
...
static TArray<const ANSICHAR*> SetupLayers(SetupHelperType& VulkanSetupHelper, TArray<TUniquePtr<ExtensionType>>& UEExtensions)
{
	...
	UE_LOGF(LogVulkanRHI, Display, "Found %d available %ls layers %ls", LayerProperties.Num(), VulkanSetupHelper.HelperTypeName, LayerProperties.Num() ? TEXT(":") : TEXT("!"));
	for (const FLayerWithExtensions& Prop : LayerProperties)
	{
		UE_LOGF(LogVulkanRHI, Display, "  * %ls", ANSI_TO_TCHAR(Prop.LayerProperties.layerName));
	}
	...
	for (const ANSICHAR* VulkanBridgeLayer : VulkanSetupHelper.ExternalLayers)
	{
		...
		{
			UE_LOGF(LogVulkanRHI, Warning, "Unable to find VulkanExternalExtensions %ls layer '%ls'", VulkanSetupHelper.HelperTypeName, ANSI_TO_TCHAR(VulkanBridgeLayer));
		}
	}
	...
	for (const ANSICHAR* PlatformLayer : PlatformLayers)
	{
		...
		{
			UE_LOGF(LogVulkanRHI, Warning, "Unable to find platform %ls layer '%ls'", VulkanSetupHelper.HelperTypeName, ANSI_TO_TCHAR(PlatformLayer));
		}
	}
...
void IVulkanDynamicRHI::AddEnabledDeviceExtensionsAndLayers(TArrayView<TSharedRef<FVulkanRHIExternalDeviceExtensionBase>> InDeviceExtensions, TArrayView<const ANSICHAR* const> InDeviceLayers)
{
...
void FVulkanIntanceSetupHelper::AddDebugLayers(const TArray<FLayerWithExtensions>& LayerProperties, FVulkanInstanceExtensionArray& UEExtensions, TArray<const ANSICHAR*>& OutLayers)
{
	...
	if (FParse::Param(FCommandLine::Get(), TEXT("vulkanapidump")))
	{
		...
		{
			UE_LOGF(LogVulkanRHI, Warning, "Can't enable api_dump when GfxReconstruct/VkTrace is enabled.");
		}
		else
		{
			...
			{
				UE_LOGF(LogVulkanRHI, Warning, "Unable to find Vulkan instance layer %ls", ANSI_TO_TCHAR(VkApiDumpName));
			}
		}
	}
	...
	if (!bGfxReconstructOrVkTrace && bUseVulkanValidation)
	{
		if (!AddRequestedLayer(KHRONOS_STANDARD_VALIDATION_LAYER_NAME, LayerProperties, UEExtensions, OutLayers))
		{
			...
			UE_LOGF(LogVulkanRHI, Warning, "Unable to find Vulkan instance validation layer %ls;  Do you have the Vulkan SDK Installed?", TEXT(KHRONOS_STANDARD_VALIDATION_LAYER_NAME));
#else
			UE_LOGF(LogVulkanRHI, Warning, "Unable to find Vulkan instance validation layer %ls", TEXT(KHRONOS_STANDARD_VALIDATION_LAYER_NAME));
#endif
		}
	}
