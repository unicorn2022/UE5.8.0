...
static VkPipeline CreatePreRasterShaderLibrary(FVulkanDevice* Device, FVulkanShader* Shader, VkShaderStageFlagBits StageFlag)
{
	...
	FVulkanShader::FSpirvCode SpirvCode = Shader->GetSpirvCode();
	...
	const VkPipelineShaderStageCreateInfo ShaderStageCreateInfo = {
		...
		.flags = 0,
		.stage = StageFlag,
		.module = VK_NULL_HANDLE, // specified in pNext
		.pName = EntryPoint,
		.pSpecializationInfo = nullptr // :todo: create every specialization
	};
	...
	const VkPipelineDynamicStateCreateInfo DynamicStateCreateInfo = {
		...
		.pNext = nullptr,
		.flags = 0,
		.dynamicStateCount = UE_ARRAY_COUNT(PreRasterDynamicStates),
	...
	const VkPipelineCreateFlags2CreateInfo Flags2CreateInfo = {
		...
		.flags = VK_PIPELINE_CREATE_2_LIBRARY_BIT_KHR | 
				 ...
				 (GVulkanGraphicPipelineLibraryLinkingMode == 1 ? VK_PIPELINE_CREATE_2_RETAIN_LINK_TIME_OPTIMIZATION_INFO_BIT_EXT : 0)
	};
...
static VkPipeline CreatePixelShaderLibrary(FVulkanDevice* Device, FVulkanShader* PixelShader)
{
	...
	const VkPipelineRenderingCreateInfo RenderingCreateInfo = {
		...
		.pNext = nullptr,
		.viewMask = 0,  // :todo-jn: fill viewMask for multiview
		.colorAttachmentCount = 0, // not used
		.pColorAttachmentFormats = nullptr, // not used
		.depthAttachmentFormat = VK_FORMAT_UNDEFINED, // not used
		.stencilAttachmentFormat = VK_FORMAT_UNDEFINED // not used
	};
	...
	FVulkanShader::FSpirvCode SpirvCode = PixelShader->GetSpirvCode();
	const VkShaderModuleCreateInfo ModuleCreateInfo = {
		.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO,
		.pNext = nullptr,
		.flags = 0,
	...
	const VkPipelineShaderStageCreateInfo ShaderStageCreateInfo = {
		...
		.module = VK_NULL_HANDLE,  // specified in pNext chain
		.pName = EntryPoint,
		.pSpecializationInfo = nullptr // :todo: create every specialization
	};
	...
	const VkPipelineDynamicStateCreateInfo DynamicStateCreateInfo = {
		...
		.pNext = nullptr,
		.flags = 0,
		.dynamicStateCount = UE_ARRAY_COUNT(FragmentShaderDynamicStates),
		.pDynamicStates = FragmentShaderDynamicStates
	};
	...
	if (RetShader == nullptr)
	{
...

		{
			...
			else
			{
				...
				if (FDataDrivenShaderPlatformInfo::GetSupportsSpecializationConstants(GMaxRHIShaderPlatform))
				{
					RetShader->SetSpecializationConstants(RetShader->GetCodeHeader().SpecializationConstantData.Values.Num() > 0);
					RetShader->SetSpecializationExclusions(RetShader->GetCodeHeader().SpecializationConstantData.GetExclusionMap());
				}

				if (Device->SupportsShaderObjects())
				{
					Device->GetShaderObjectManager()->CreateShaderObject(*RetShader);
				}
				else if (Device->SupportsGraphicPipelineLibraries())
				{
					if constexpr (std::is_same_v<ShaderType, FVulkanVertexShader>)
					{
						RetShader->PipelineLibrary = CreatePreRasterShaderLibrary(Device, RetShader, VK_SHADER_STAGE_VERTEX_BIT);
					}
					if constexpr (std::is_same_v<ShaderType, FVulkanMeshShader>)
					{
						RetShader->PipelineLibrary = CreatePreRasterShaderLibrary(Device, RetShader, VK_SHADER_STAGE_MESH_BIT_EXT);
					}
					if constexpr (std::is_same_v<ShaderType, FVulkanPixelShader>)
					{
...
FVulkanShader::~FVulkanShader()
{
	...
	if (AsyncCompileTask.IsValid() && !AsyncCompileTask->IsComplete())
	{
		AsyncCompileTask->Wait();
	}

	if (ShaderObject)
	{
		Device->GetDeferredDeletionQueue().EnqueueFunc([Handle=ShaderObject](FVulkanDevice& Device) 
		{
			VulkanRHI::vkDestroyShaderEXT(Device.GetHandle(), Handle, VULKAN_CPU_ALLOCATOR);
		});
...
FVertexShaderRHIRef FVulkanDynamicRHI::RHICreateVertexShader(const FRHICreateShaderDesc& CreateShaderDesc)
{
	return Device->GetShaderFactory().CreateShader<FVulkanVertexShader>(CreateShaderDesc.Code, Device);
}

FPixelShaderRHIRef FVulkanDynamicRHI::RHICreatePixelShader(const FRHICreateShaderDesc& CreateShaderDesc)
{
	return Device->GetShaderFactory().CreateShader<FVulkanPixelShader>(CreateShaderDesc.Code, Device);
}

FMeshShaderRHIRef FVulkanDynamicRHI::RHICreateMeshShader(const FRHICreateShaderDesc& CreateShaderDesc)
{
	return Device->GetShaderFactory().CreateShader<FVulkanMeshShader>(CreateShaderDesc.Code, Device);
}

FAmplificationShaderRHIRef FVulkanDynamicRHI::RHICreateAmplificationShader(const FRHICreateShaderDesc& CreateShaderDesc)
{
	return Device->GetShaderFactory().CreateShader<FVulkanTaskShader>(CreateShaderDesc.Code, Device);
}

FGeometryShaderRHIRef FVulkanDynamicRHI::RHICreateGeometryShader(const FRHICreateShaderDesc& CreateShaderDesc)
{ 
	return Device->GetShaderFactory().CreateShader<FVulkanGeometryShader>(CreateShaderDesc.Code, Device);
}

FComputeShaderRHIRef FVulkanDynamicRHI::RHICreateComputeShader(const FRHICreateShaderDesc& CreateShaderDesc)
{ 
	return Device->GetShaderFactory().CreateShader<FVulkanComputeShader>(CreateShaderDesc.Code, Device);
}

FRayTracingShaderRHIRef FVulkanDynamicRHI::RHICreateRayTracingShader(const FRHICreateShaderDesc& CreateShaderDesc, EShaderFrequency ShaderFrequency)
{
	switch (ShaderFrequency)
	{
	case EShaderFrequency::SF_RayGen:
		 return Device->GetShaderFactory().CreateRayTracingShader<SF_RayGen>(CreateShaderDesc.Code, Device);

	case EShaderFrequency::SF_RayMiss:
		return Device->GetShaderFactory().CreateRayTracingShader<SF_RayMiss>(CreateShaderDesc.Code, Device);

	case EShaderFrequency::SF_RayCallable:
		return Device->GetShaderFactory().CreateRayTracingShader<SF_RayCallable>(CreateShaderDesc.Code, Device);

	case EShaderFrequency::SF_RayHitGroup:
		return Device->GetShaderFactory().CreateRayTracingShader<SF_RayHitGroup>(CreateShaderDesc.Code, Device);
