...
void FGDTimingFramePacer::PollPastFrameInfo()
{
	for (;;)
	{
		...
		{
			UE_LOGF(LogVulkanRHI, Warning, "PastPresentationTiming actualPresentTime is less than desiredPresentTime! Resetting LastScheduledPresentTime...");
...
void FVulkanAndroidPlatform::NotifyFoundDeviceLayersAndExtensions(VkPhysicalDevice PhysicalDevice, const TArray<const ANSICHAR*>& Layers, const TArray<const ANSICHAR*>& Extensions)
{
	...
	if (FVulkanAndroidPlatform::bSwappyEnabledAtRHIInit)
	{
		...
		if (PropertyCount > 0)
		{
			...
			{
				UE_LOGF(LogVulkanRHI, Log, "Swappy requires %d extensions:", SwappyRequiredExtensionCount);
			...
			{
				UE_LOGF(LogVulkanRHI, Log, "Swappy didn't ask for any extensions");
			}
		}
		else
		{
			UE_LOGF(LogVulkanRHI, Log, "No extensions available for Swappy");
		}
	}
#endif
	...
	UE_LOGF(LogVulkanRHI, Log, "bHasGoogleDisplayTiming = %d", FVulkanAndroidPlatform::bHasGoogleDisplayTiming);
#endif
}
...
namespace AndroidVulkan
{
	...
	static std::atomic<bool> bWaitingForWindowCreated{false};
#endif
	...
	static void OnLostWindow(TOptional<FAndroidWindow::FNativeAccessor> WindowContainer)
	{
		...
		UE_LOGF(LogVulkanRHI, Log, "OnLostWindow: Event thread");
		...
		{
				UE_LOGF(LogVulkanRHI, Log, "OnLostWindow: RT");
				RHICmdList.EnqueueLambda([&WindowContext](FRHICommandListImmediate& ExecutingCmdList) mutable
					{
						...
						UE_LOGF(LogVulkanRHI, Log, "OnLostWindow: RHI");
						...
						for (FVulkanViewport* Viewport : Viewports)
						{
							...
							{
								UE_LOGF(LogVulkanRHI, Log, "OnLostWindow: Destroying viewport swapchain (window %p -> null)", WindowContext.GetANativeWindow() );
								Viewport->DestroySwapchain(nullptr);
								UE_LOGF(LogVulkanRHI, Log, "OnLostWindow: Destroying viewport swapchain done.");
								bFound = true;
								break;
							}
						}
						check(bFound);
						UE_LOGF(LogVulkanRHI, Log, "OnLostWindow: RHI done");
					});
				...
				UE_LOGF(LogVulkanRHI, Log, "OnLostWindow:RT done");
				OnComplete->DispatchSubsequents();
			});

		UE_LOGF(LogVulkanRHI, Log, "OnLostWindow: wait RT");
		OnComplete->Wait();
		UE_LOGF(LogVulkanRHI, Log, "OnLostWindow: done");
	}
	...
	static void OnFoundWindow(TOptional<FAndroidWindow::FNativeAccessor> WindowContainer)
	{
		...
		UE_LOGF(LogVulkanRHI, Log, "OnFoundWindow:");

#if USE_ANDROID_STANDALONE
		bWaitingForWindowCreated.store(true);
#endif
		FGraphEventRef OnComplete = FGraphEvent::CreateGraphEvent();
		ENQUEUE_RENDER_COMMAND(OnAndroidFoundWindow)([OnComplete, &WindowContext](FRHICommandListImmediate& RHICmdList) mutable
			{
				UE_LOGF(LogVulkanRHI, Log, "OnFoundWindow: RT");
				RHICmdList.EnqueueLambda([&WindowContext](FRHICommandListImmediate& ExecutingCmdList) mutable
					{
						check(IsInRHIThread() || IsRunningRHIInSeparateThread());
						UE_LOGF(LogVulkanRHI, Log, "OnFoundWindow: RHI");
						...
						for (FVulkanViewport* Viewport : Viewports)
						{
							if (Viewport->GetWindowHandle() == WindowContext.GetWindowHandle())
							{
								UE_LOGF(LogVulkanRHI, Log, "OnFoundWindow: RHI RecreateSwapchain (window %p)", WindowContext.GetANativeWindow());
								Viewport->RecreateSwapchain(Context, WindowContext);
								UE_LOGF(LogVulkanRHI, Log, "OnFoundWindow: RHI RecreateSwapchain done (window %p)", WindowContext.GetANativeWindow());
								bFound = true;
								break;
							}
						}
						check(bFound);
						UE_LOGF(LogVulkanRHI, Log, "OnFoundWindow: RHI done");
					});
				// Swapchain creation pushes some commands - flush the command buffers now to begin with a fresh state
				RHICmdList.RHIThreadFence(true);
				RHICmdList.SubmitAndBlockUntilGPUIdle();
				UE_LOGF(LogVulkanRHI, Log, "OnFoundWindow: RT done");
				OnComplete->DispatchSubsequents();
			});
		UE_LOGF(LogVulkanRHI, Log, "OnFoundWindow: wait RT");
		OnComplete->Wait();
		UE_LOGF(LogVulkanRHI, Log, "OnFoundWindow: done");
	}
}

#if USE_ANDROID_STANDALONE
bool FVulkanAndroidPlatform::IsWaitingForWindowCreated()
{
	return AndroidVulkan::bWaitingForWindowCreated.load();
}
#endif
...
void FVulkanAndroidPlatform::RecreateSwapChain(void* NewNativeWindow)
{
	...
	if (NewNativeWindow)
	{
		...
		{
			UE_LOGF(LogVulkanRHI, Log, "Recreate swapchain ... ");
...
void FVulkanAndroidPlatform::DestroySwapChain()
{
	...
	ENQUEUE_RENDER_COMMAND(VulkanDestroySwapChain)(
		...
		{
			UE_LOGF(LogVulkanRHI, Log, "Destroy swapchain ... ");
...
void FVulkanAndroidPlatform::OverridePlatformHandlers(bool bInit)
{
...
#if USE_ANDROID_STANDALONE
			UE::FAndroidPlatformDynamicRHI::SetRHIOnWindowCreatedCallback(
				[]()
				{
					// Enqueue on the render thread so this executes AFTER the viewport resize
					// render command (triggered by OnWindowSizeChanged) has completed.
					// Unblocking immediately here would race with the resize, allowing a frame
					// to present with the old swapchain size.
					ENQUEUE_RENDER_COMMAND(OnVulkanWindowCreated)([](FRHICommandListImmediate& RHICmdList)
					{
						RHICmdList.EnqueueLambda([](FRHICommandListImmediate&)
						{
							AndroidVulkan::bWaitingForWindowCreated.store(false);
							UE_LOGF(LogVulkanRHI, Display, "OnWindowCreatedProcessed: rendering unblocked");
						});
					});
				});
#endif
	...
	else
	{
		...
		UE::FAndroidPlatformDynamicRHI::SetRHIOnWindowCreatedCallback(nullptr);
...
void FVulkanAndroidPlatform::SetupRequiresDepthStencilFullWriteWorkaround(const FVulkanDevice& Device)
{
	...
	if (Device.GetVendorId() == EGpuVendorId::Arm)
	{
		...
		if (DriverVersion >= VK_MAKE_API_VERSION(0, 16, 0, 0) && DriverVersion < VK_MAKE_API_VERSION(0, 24, 0, 0))
		{
			...
			UE_LOGF(LogRHI, Display, "Enabling workaround that changes partial depth/stencil attachment stores into full depth/stencil stores.");
		}
	}
}
...
void FVulkanAndroidPlatform::SetupRequiresMergedDepthStencilLayoutWorkaround(const FVulkanDevice& Device)
{
	...
	if (Device.GetVendorId() == EGpuVendorId::Arm)
	{
		...
		if (DriverVersion < VK_MAKE_API_VERSION(0, 54, 1, 0))
		{
			...
			UE_LOGF(LogRHI, Display, "Enabling workaround that avoids usage of dedicated stencil layout via VkAttachmentDescriptionStencilLayout.");
		}
	}
}
	...
	if (Result == VK_SUCCESS)
	{
		...
		if (FVulkanAndroidPlatform::bSwappyEnabledAtRHIInit)
		{
			...
			if (ensure(Env))
			{
				...
				UE_LOGF(LogVulkanRHI, Log, "SwappyVk_initAndGetRefreshCycleDuration: %llu", RefreshDuration);
			}
...
void FVulkanAndroidPlatform::DestroySwapchainKHR(VkDevice Device, VkSwapchainKHR Swapchain, const VkAllocationCallbacks* Allocator)
{
	...
	if (FVulkanAndroidPlatform::bSwappyEnabledAtRHIInit)
	{
		...
		UE_LOGF(LogVulkanRHI, Log, "SwappyVk_destroySwapchain");
	}
#endif
...
bool FAndroidVulkanFramePacer::SupportsFramePaceInternal(int32 QueryFramePace, int32& OutRefreshRate, int32& OutSyncInterval)
{
	...
	FString DebugString = "FAndroidVulkanFramePacer -> Supported Refresh Rates:";
	for (int32 RefreshRate : RefreshRates)
	{
		DebugString += FString::Printf(TEXT(" %d"), RefreshRate);
	}
	UE_LOGF(LogRHI, Log, "%ls", *DebugString);

	for (int32 Rate : RefreshRates)
	{
		if ((Rate % QueryFramePace) == 0)
		{
			UE_LOGF(LogRHI, Log, "Supports %d using refresh rate %d and sync interval %d", QueryFramePace, Rate, Rate / QueryFramePace);
			OutRefreshRate = Rate;
	...
	if (FAndroidPlatformRHIFramePacer::CVarSupportNonVSyncMultipleFrameRates.GetValueOnAnyThread() == 1)
	{
		for (int32 Rate : RefreshRates)
		{
			...
			{
				UE_LOGF(LogRHI, Log, "Supports %d using refresh rate %d with CPU frame pacing", QueryFramePace, Rate);
				OutRefreshRate = Rate;
				OutSyncInterval = 0;
				return true;
			}
...
void GetVKStructsFromPNext(const void* InNext, TMap<VkStructureType, const void*>& VkStructs, const TArray<VkStructureType>& ValidTypes)
{
	...
	while (Next != nullptr)
	{
		...
		{
			UE_LOGF(LogRHI, Warning, "GetVKStructsFromPNext: Unexpected type found when reading pNext->sType %d, Valid Types: ", (uint32_t)Next->sType);
		}
...
void HandleAttachmentDescriptionPNext(const VkAttachmentDescription2& Description, TArray<char>& MemoryStream)
{
	TMap<VkStructureType, const void*> VkStructs;
	GetVKStructsFromPNext(Description.pNext, VkStructs, { VK_STRUCTURE_TYPE_ATTACHMENT_DESCRIPTION_STENCIL_LAYOUT });

	int32_t HandledCount = 0;

	const void** Struct = VkStructs.Find(VK_STRUCTURE_TYPE_ATTACHMENT_DESCRIPTION_STENCIL_LAYOUT);
	const bool bHasStencilLayout = Struct != nullptr;
	COPY_TO_BUFFER(MemoryStream, &bHasStencilLayout, sizeof(bool));

	if (bHasStencilLayout)
	{
		VkAttachmentDescriptionStencilLayout* StencilLayout = (VkAttachmentDescriptionStencilLayout*)*Struct;
		check(StencilLayout->pNext == nullptr);
		COPY_TO_BUFFER(MemoryStream, StencilLayout, sizeof(VkAttachmentDescriptionStencilLayout));
		HandledCount++;
	}
...
void PipelineToBinary(FVulkanDevice* Device, const VkGraphicsPipelineCreateInfo* PipelineInfo, const FGfxPipelineDesc* GfxEntry, const FVulkanRenderTargetLayout* RTLayout, TArray<char>& MemoryStream)
{
	...
	for (int32_t Idx = 0; Idx < PipelineInfo->stageCount; ++Idx)
	{
		...
		const bool bHasSpecializationInfo = PipelineInfo->pStages[Idx].pSpecializationInfo != nullptr;
		COPY_TO_BUFFER(MemoryStream, &bHasSpecializationInfo, sizeof(bHasSpecializationInfo));
		if (bHasSpecializationInfo)
		{
			const VkSpecializationInfo SpecializationInfo = {
				.mapEntryCount = PipelineInfo->pStages[Idx].pSpecializationInfo->mapEntryCount,
				.pMapEntries = nullptr,
				.dataSize = PipelineInfo->pStages[Idx].pSpecializationInfo->dataSize,
				.pData = nullptr
			};
			COPY_TO_BUFFER(MemoryStream, &SpecializationInfo, sizeof(VkSpecializationInfo));
			COPY_TO_BUFFER(MemoryStream, PipelineInfo->pStages[Idx].pSpecializationInfo->pMapEntries, SpecializationInfo.mapEntryCount * sizeof(VkSpecializationMapEntry));
			COPY_TO_BUFFER(MemoryStream, PipelineInfo->pStages[Idx].pSpecializationInfo->pData, SpecializationInfo.dataSize);
		}
	}
	...
	if (bUseRenderPass2)
	{
		...
		if(RenderPassCreateInfo.attachmentCount > 0)
		{
			...
			for (uint32_t AttachmentIndex = 0; AttachmentIndex < RenderPassCreateInfo.attachmentCount; ++AttachmentIndex)
			{
				HandleAttachmentDescriptionPNext(CreateInfo.pAttachments[AttachmentIndex], MemoryStream);
			}
		}
...
struct FVKRemoteProgramCompileJNI
{
	...
	jmethodID SetProgramInitDataRetainLimit = 0;
	jmethodID GetRetainedProgramInitDataPath = 0;
	...
	void Init(JNIEnv* Env)
	{
		...
		if (PSOServiceAccessor)
		{
			...
			CHECK_JNI_EXCEPTIONS(Env);
			SetProgramInitDataRetainLimit = FJavaWrapper::FindStaticMethod(Env, PSOServiceAccessor, "AndroidThunkJava_SetProgramInitDataRetainLimit", "(I)V", false);
			CHECK_JNI_EXCEPTIONS(Env);
			GetRetainedProgramInitDataPath = FJavaWrapper::FindStaticMethod(Env, PSOServiceAccessor, "AndroidThunkJava_GetRetainedProgramInitDataPath", "()Ljava/lang/String;", false);
			CHECK_JNI_EXCEPTIONS(Env);
		...
		bAllFound = PSOServiceAccessor && DispatchPSOCompile && DispatchPSOCompileShm && StartRemoteProgramLink && HaveServicesFailed && AreProgramServicesReady && StopRemoteProgramLink && SetProgramInitDataRetainLimit && GetRetainedProgramInitDataPath
			&& ProgramResponseClass && ProgramResponse_SuccessField && ProgramResponse_CompiledBinaryField && ProgramResponse_ErrorField && ProgramResponse_SHMOutputHandleField && ProgramResponse_CompilationDurationField;
		UE_CLOGF(!bAllFound, LogRHI, Fatal, "Failed to find JNI Vulkan remote program compiler.");
	}
...
static bool AreAndroidVulkanRemoteCompileServicesAvailable()
{
	...
	if (RemoteCompileService == -1)
	{
		...
		UE_LOGF(LogRHI, Log, "External PSO compilers = %ls", RemoteCompileService == 0 ? TEXT("disabled") : TEXT("enabled"));
	}
...
namespace AndroidVulkan
{
	namespace PSOCapture
	{
#if WITH_ADDITIONAL_CRASH_CONTEXTS
		int32 GPSOInitHistoryLimit = 0;
		static FAutoConsoleVariableRef CVarVulkanExtensionFramePacer(
			TEXT("android.Vulkan.RetainLimitForPSOCreateCrashReport"),
			GPSOInitHistoryLimit,
			TEXT("Retain the PSO creation source data from the external compiler services when > 0. If a crash occurs the data is added to the crash report.\n")
			TEXT(" 0 to disable (default)\n")
			TEXT(" >0 == amount of history to retain per service."),
			ECVF_RenderThreadSafe
		);

		static FString PSOCrashHistoryDir;

		static void OnCrashDumpPSOHistoryInfo(FCrashContextExtendedWriter& Writer)
		{
			if (GPSOInitHistoryLimit > 0 && FVulkanAndroidPlatform::AreRemoteCompileServicesActive())
			{
				FPlatformMisc::LowLevelOutputDebugStringf(TEXT("OnCrashDumpPSOHistoryInfo"));
				IPlatformFile::GetPlatformPhysical().IterateDirectory(*PSOCrashHistoryDir, [&Writer](const TCHAR* Pathname, bool bIsDirectory)
					{
						if(!bIsDirectory)
						{
							FPlatformMisc::LowLevelOutputDebugStringf(TEXT("OnCrashDumpPSOHistoryInfo: %s -> %s"), Pathname, *FPaths::GetCleanFilename(Pathname));
							Writer.MoveFile(Pathname);
						}
						return true;
					});
			}
...
#endif
		static void InitCrashDataCapture()
		{
#if WITH_ADDITIONAL_CRASH_CONTEXTS
			UE_LOGF(LogRHI, Log, "Retaining remote compile services history, limit = %d", AndroidVulkan::PSOCapture::GPSOInitHistoryLimit);

			if (AndroidVulkan::PSOCapture::GPSOInitHistoryLimit > 0)
			{
				check(IsInRenderingThread());
				check(PSOCrashHistoryDir.IsEmpty());
				JNIEnv* Env = FAndroidApplication::GetJavaEnv();
				Env->CallStaticVoidMethod(VKRemoteProgramCompileJNI.PSOServiceAccessor, VKRemoteProgramCompileJNI.SetProgramInitDataRetainLimit, (jint)AndroidVulkan::PSOCapture::GPSOInitHistoryLimit);
				PSOCrashHistoryDir = FJavaHelper::FStringFromLocalRef(Env, (jstring)Env->CallStaticObjectMethod(VKRemoteProgramCompileJNI.PSOServiceAccessor, VKRemoteProgramCompileJNI.GetRetainedProgramInitDataPath));
				CHECK_JNI_EXCEPTIONS(Env);
			}
#endif
		}
...
#if WITH_ADDITIONAL_CRASH_CONTEXTS
void FVulkanAndroidPlatform::OnCreatePipelineCrash(FCrashContextExtendedWriter& Writer, TConstArrayView<FVulkanShader*> Shaders, TStringView<TCHAR> ExtraInfo, bool bIsPrecache)
{
	if(bIsPrecache)
	{
		// we've crashed during PSO compile of a precache PSO, dump all the retained history.
		// just do this once.
		static std::atomic_bool bDoneOnce = false;
		if (bDoneOnce.exchange(true) == false)
		{
			AndroidVulkan::PSOCapture::OnCrashDumpPSOHistoryInfo(Writer);
		}
	}

	// Call generic version to write out shader hashes
	FVulkanGenericPlatform::OnCreatePipelineCrash(Writer, Shaders, ExtraInfo, bIsPrecache);
}
...
bool FVulkanAndroidPlatform::StartRemoteCompileServices(int NumServices)
{
	...
	if (Env && AreAndroidVulkanRemoteCompileServicesAvailable() && !AndroidVulkanService::GRemoteCompileServicesStarted)
	{
		...
		AndroidVulkan::PSOCapture::InitCrashDataCapture();
	}
...
void FVulkanAndroidPlatform::StopRemoteCompileServices()
{
	...
	{
		UE_LOGF(LogVulkanRHI, Log, "Stopping Remote Compile Services");
...
VkPipelineCache FVulkanAndroidPlatform::PrecompilePSO(
		...
		const TArrayView<const uint8> ContextData,
		const TArrayView<const uint8> OptionalPSOCacheData, 
		...
		TArrayView<const uint32_t> VS, 
		TArrayView<const uint32_t> PS, 
		size_t& AfterSize,
...
// 	FScopedDurationTimeLogger Timer(TEXT("FVulkanAndroidPlatform::PrecompilePSO"));
	...
	if (Env && ensure(VKRemoteProgramCompileJNI.bAllFound))
	{
		...
		auto ContextJavaArray = NewScopedJavaObject(Env, Env->NewByteArray(ContextData.Num()));
		Env->SetByteArrayRegion(*ContextJavaArray, 0, ContextData.Num(), reinterpret_cast<const jbyte*>(ContextData.GetData()));
		CHECK_JNI_EXCEPTIONS(Env);
		...
		FScopedJavaObject<_jobject*> ProgramResponseObj;
		{
			...
			if (ensure(SharedMemFD > -1))
			{
				...
				if (ensure(SharedBuffer))
				{
					...
					bool bEnableTimeOuts = !FPlatformMisc::IsDebuggerPresent();
					{
						...
						ProgramResponseObj = NewScopedJavaObject(Env, Env->CallStaticObjectMethod(VKRemoteProgramCompileJNI.PSOServiceAccessor, VKRemoteProgramCompileJNI.DispatchPSOCompileShm, *ContextJavaArray, PriorityInfo.GetPriorityInfo(), SharedMemFD, VSSize, PSSize, PSOParamsSize, PreSuppliedCacheSize, bEnableTimeOuts));
					}
					CHECK_JNI_EXCEPTIONS(Env);
				...
				{
					UE_LOGF(LogRHI, Error, "Failed to alloc %lld bytes for external PSO compile: %d",
						TotalSHMSizeAligned,
						errno
						);
				}
			...
			{
				UE_LOGF(LogRHI, Error, "Failed to alloc %lld bytes for external PSO compile: %d (%ls)",
					TotalSHMSizeAligned,
					errno,
...
void FAndroidVulkanFramePacer::Init()
{
	...
	{
		UE_LOGF(LogRHI, Warning, "In Vulkan mode, changing a.UseSwappyForFramePacing after the RHI is initialized has no effect. Swappy is %ls.", FVulkanAndroidPlatform::bSwappyEnabledAtRHIInit ? TEXT("Enabledd") : TEXT("Disabled"));
	}));
#endif
#endif
}
...
void FVulkanAndroidPlatform::PostInitGPU(const FVulkanDevice& InDevice)
{
	...
	// The Vulkan specification does not provide concrete guidance on anisotropic filtering for 3D textures, 
	// and on some Android GPUs this results in undefined behavior
	GRHIGlobals.SupportsTexture3DAnisotropicFiltering = false;
...
void FVulkanAndroidPlatform::SetupImageMemoryRequirementWorkaround(const FVulkanDevice& InDevice)
{
	...
	// AFBC workarounds
	{
		...
		{
			UE_LOGF(LogRHI, Display, "Enabling workaround to reduce memory requirement for BGRA textures (%ls flag). 128x128 - 8 Mips BGRA texture: %llu KiB -> %llu KiB",
	...
	if ((formatProperties.linearTilingFeatures & (VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_LINEAR_BIT | VK_FORMAT_FEATURE_TRANSFER_DST_BIT)) == (VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_LINEAR_BIT | VK_FORMAT_FEATURE_TRANSFER_DST_BIT))
	{
		...
		if (ImageLinearMem_ASTC.size * MEM_SIZE_THRESHOLD <= ImageOptimal_SIZE)
		{
			...
			UE_LOGF(LogRHI, Display, "Enabling workaround to reduce memory requirement for ASTC textures (VK_IMAGE_TILING_LINEAR). 128x128 - 8 Mips ASTC_8x8 texture: %llu KiB -> %llu KiB",
...
void FVulkanAndroidPlatform::OnGPUCrash(const TCHAR* Message)
{
	...
	GLog->Panic();
	UE_LOGF(LogVulkanRHI, Fatal, "%ls", Message);
}
