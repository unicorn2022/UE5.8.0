...
int32 GVulkanDiagnosticBuffer = 1;
static FAutoConsoleVariableRef CVarVulkanDiagnosticBuffer(
	TEXT("r.Vulkan.DiagnosticBuffer"),
	GVulkanDiagnosticBuffer,
...
int32 FVulkanQueue::SubmitQueuedPayloads(TMap<VkSemaphore, FBinarySemaphoreSignalInfo>& SignaledSemas)
{
	...
	{
		const TArray<TRefCountPtr<FVulkanSemaphore>>& WaitSemas = Payload->WaitSemaphores;
		for (const TRefCountPtr<FVulkanSemaphore>& WaitSema : WaitSemas)
		{
	...
	while (FVulkanPayload * Payload = PendingSubmission.Peek())
	{
...
int32 FVulkanQueue::ProcessInterruptQueue(uint64 Timeout)
{
	...
	if (bUseTimelineSemaphores)
	{
		...
		TimelineSempahore->GetTimelineSemaphoreValue(CompletedTimelineSemaphoreValue);
	}
	...
	{
		bSuccess = false;
		if (FVulkanPayload* Payload = PendingInterrupt.Peek())
		{
...
static void ManuallyWriteMarker(const VkCommandBuffer CommandBufferHandle, const VkBuffer BufferHandle, const uint64 WriteOffset, const uint32 BreadcrumbID)
{
	...
	const bool bUsePreciseBarriers = (GVulkanDiagnosticBuffer > 1);
