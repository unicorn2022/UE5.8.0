...
void FVulkanBackBuffer::AcquireBackBufferImage(FVulkanCommandListContext& Context)
{
	...
	{
		if (Viewport->TryAcquireImageIndex(Context))
		{
			const int32 AcquiredImageIndex = Viewport->AcquiredImageIndex;
			check(AcquiredImageIndex >= 0 && AcquiredImageIndex < Viewport->BackBufferData.Num());

			FVulkanView* ImageView = Viewport->BackBufferData[AcquiredImageIndex].TextureView.Get();
...
FVulkanViewport::~FVulkanViewport()
{
	...
	if (SupportsStandardSwapchain())
	{
		for (int32 Index = 0; Index < BackBufferData.Num(); ++Index)
		{
			FBackbufferImageData& Data = BackBufferData[Index];
			...
			{
				Device.NotifyDeletedImage(Data.Texture->Image, true);
			}
		}
		BackBufferData.Empty();
...
bool FVulkanViewport::DoCheckedSwapChainJob(FVulkanCommandListContext& Context)
{
	...
	{
		// The swapchain may have been destroyed if the window was closed.
		if (!Viewport->SwapChain)
		{
			return (int32)FVulkanSwapChain::EStatus::Healthy;
		}

		// May happen if swapchain was recreated in DoCheckedSwapChainJob()
		...
		return (int32)Viewport->SwapChain->Present(PresentQueue, Viewport->BackBufferData[Viewport->AcquiredImageIndex].RenderingDoneSemaphores);
	};
	...
	{
		// Don't attempt to recreate if the window has been destroyed.
		...
		{
			UE_LOGF(LogVulkanRHI, Verbose, "Swapchain is out of date! Trying to recreate the swapchain.");
		}
		else if (Status == (int32)FVulkanSwapChain::EStatus::SurfaceLost)
		{
			UE_LOGF(LogVulkanRHI, Warning, "Swapchain surface lost! Trying to recreate the swapchain.");
		}
		else
		{
			check(0);
...
bool FVulkanViewport::TryAcquireImageIndex(FVulkanCommandListContext& Context)
{
	if (SwapChain && FVulkanPlatform::IsWindowValid(WindowHandle))
	{
		TRefCountPtr<FVulkanSemaphore> AcquiredSemaphore;
		const int32 Result = SwapChain->AcquireImageIndex(&AcquiredSemaphore);
		if (Result >= 0)
		{
			AcquiredImageIndex = Result;

			// Wait for semaphore signal before writing to backbuffer image
			Context.AddWaitSemaphore(VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT | VK_PIPELINE_STAGE_TRANSFER_BIT, AcquiredSemaphore);

			if (BackBufferData[AcquiredImageIndex].bFirstUse)
			{
				FBackbufferImageData& Data = BackBufferData[AcquiredImageIndex];
				Data.bFirstUse = false;

				VkClearColorValue ClearColor;
				FMemory::Memzero(ClearColor);

				const VkImage BackbufferImage = Data.Texture->Image;
				...
				VulkanSetImageLayout(&CommandBuffer, BackbufferImage, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, Range);
				VulkanRHI::vkCmdClearColorImage(CommandBuffer.GetHandle(), BackbufferImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &ClearColor, 1, &Range);
				VulkanSetImageLayout(&CommandBuffer, BackbufferImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR, Range);
			}
...
void FVulkanViewport::AdvanceBackBufferFrame(FRHICommandListImmediate& RHICmdList, bool bPresent)
{
	check(IsInRenderingThread());

	if (SupportsStandardSwapchain() && GVulkanDelayAcquireImage != EDelayAcquireImageType::DelayAcquire && bPresent)
	{
		check(RHIBackBuffer);
...
void FVulkanDynamicRHI::RHITick(float DeltaTime)
{
	...
	{
		Viewport->CheckSwapChain();
	}
}

void FVulkanViewport::CheckSwapChain()
{
	check(IsInGameThread());

	// Honor platform-side recreate requests (Linux uses this after a Hide/Show cycle
	// Vulkan WSI's OUT_OF_DATE detection across xdg role destroy/recreate is unreliable).
	// Must NOT gate on SwapChain FVulkanSwapChain::Create can leave it null after a
	// graceful surface-lost bail, and we still need to recreate when the platform asks.
	const bool bPlatformRequestsRecreate =
		FVulkanPlatform::TryConsumeSwapchainRecreateRequest(WindowHandle);

	const bool bVsyncChanged =
		SwapChain && FPlatformAtomics::AtomicRead(&LockToVsync) != SwapChain->DoesLockToVsync();

	if (bPlatformRequestsRecreate || bVsyncChanged)
	{
		FVulkanPlatformWindowContext WindowContext(WindowHandle);
...
void FVulkanViewport::RecreateSwapchainFromRT(FRHICommandListImmediate& RHICmdList, EPixelFormat PreferredPixelFormat, FVulkanPlatformWindowContext& WindowContext)
{
	...
	{
		// Skip swapchain recreation if the window has been destroyed or is no longer valid.
		if (!FVulkanPlatform::IsWindowValid(WindowHandle))
		{
			return;
		}
...
void FVulkanViewport::InitImages(TConstArrayView<VkImage> Images)
{
	...
	BackBufferData.SetNum(Images.Num());
	...
	for (int32 Index = 0; Index < Images.Num(); ++Index)
	{
		...
		Data.bFirstUse = true;
		if (!Data.RenderingDoneSemaphores.IsValid())
		{
			Data.RenderingDoneSemaphores = new FVulkanSemaphore(Device);
		}

		const VkSurfaceTransformFlagBitsKHR CachedSurfaceTransform = SwapChain->GetCachedSurfaceTransform();
		const bool bSwapImageDimensions = (CachedSurfaceTransform == VK_SURFACE_TRANSFORM_ROTATE_90_BIT_KHR || CachedSurfaceTransform == VK_SURFACE_TRANSFORM_ROTATE_270_BIT_KHR);
		const uint32 ImageSizeX = bSwapImageDimensions ? SizeY : SizeX;
		const uint32 ImageSizeY = bSwapImageDimensions ? SizeX : SizeY;
		Data.Texture = (FVulkanTexture*)FVulkanDynamicRHI::Get().RHICreateTexture2DFromResource(PixelFormat, ImageSizeX, ImageSizeY, 1, 1, Images[Index], ETextureCreateFlags::RenderTargetable | ETextureCreateFlags::Presentable).GetReference();

		Data.TextureView = MakeUnique<FVulkanView>(Device, ERHIDescriptorType::TextureSRV);
		Data.TextureView->CreateTextureView(Images[Index], ViewDesc);

#if VULKAN_ENABLE_DRAW_MARKERS
		if (Device.GetSetDebugName())
		{
			VulkanRHI::SetDebugName(Device.GetSetDebugName(), Device.GetHandle(), Data.Texture->Image, "VulkanBackBuffer");
		}
#endif
	}
}
		...
		if (WindowContext.IsValid())
		{
			...
			if (SwapChain)
			{
				ensureMsgf(Images.Num() >= NumRequestedSwapchainImages,
					...
					NumRequestedSwapchainImages, Images.Num(), BackBufferData.Num());
...
bool FVulkanViewport::Present(FVulkanCommandListContext& Context, const FRHIPresentArgs& InPresentArgs)
{
	check(Context.IsImmediate());

#if PLATFORM_ANDROID && USE_ANDROID_STANDALONE
	if (FVulkanPlatform::IsWaitingForWindowCreated())
	{
		UE_LOGF(LogVulkanRHI, Log, "Present: skipping frame, waiting for WINDOW_CREATED");
		return true;
	}
#endif
	...
	FPlatformAtomics::AtomicStore(&LockToVsync, InPresentArgs.bLockToVsync ? 1 : 0);
	...
	if (SupportsStandardSwapchain())
	{
		...
		if (GVulkanDelayAcquireImage == EDelayAcquireImageType::DelayAcquire && RenderingBackBuffer)
		{
			...
			if (LIKELY(TryAcquireImageIndex(Context)))
			{
				const uint32 WindowSizeX = FMath::Min(SizeX, SwapChain->InternalWidth);
				const uint32 WindowSizeY = FMath::Min(SizeY, SwapChain->InternalHeight);

				FBackbufferImageData& Data = BackBufferData[AcquiredImageIndex];
				CopyImageToBackBuffer(Context, *RenderingBackBuffer.GetReference(), *Data.Texture.GetReference(), SizeX, SizeY, WindowSizeX, WindowSizeY, SwapChain->GetCachedSurfaceTransform());
			}
			else
			{
		...
		else
		{
			if (AcquiredImageIndex != -1)
			{
				...
				check(RHIBackBuffer != nullptr && RHIBackBuffer->Image == Data.Texture->Image);
				VulkanSetImageLayout(&CommandBuffer, Data.Texture->Image, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR, FVulkanPipelineBarrier::MakeSubresourceRange(VK_IMAGE_ASPECT_COLOR_BIT));
			}
			else
			{
		...
		if (LIKELY(!bFailedToDelayAcquireBackbuffer))
		{
			...
			{
				Context.AddSignalSemaphore(BackBufferData[AcquiredImageIndex].RenderingDoneSemaphores);
			}
		}
		else
		{
			if (FVulkanPlatformWindowContext::CanCreateSwapchainOnDemand()) // on android we dont want to attempt to recreate the window when we dont have the window lock...
			{
				...
				// Don't attempt to recreate the swapchain if the window has been destroyed.
				if (!FVulkanPlatform::IsWindowValid(WindowHandle))
				{
					return true;
				}

				UE_LOGF(LogVulkanRHI, Log, "AcquireNextImage() failed due to the outdated swapchain, not even attempting to present.");
	...
	{
		// If we don't do the actual Present call (e.g. XR device rendering to an offscreen
		// compositor buffer), we need to manually set the end of the frame marker
		// for tools like RenderDoc to be able to judge when the frame has ended before we submit
		// the active command buffer below
		Context.InsertEndFrameDebugLabel();
	}
	...
	if (bHasCustomPresent)
	{
		...
		bNeedNativePresent = CustomPresent->Present(this, Context, SyncInterval);
	}
	...
	{
		// Skip present if the native window has been destroyed. On Wayland, presenting
		// to a destroyed surface crashes in the driver's wl_proxy_marshal.
		if (!FVulkanPlatform::IsWindowValid(WindowHandle))
		{
			AcquiredImageIndex = -1;
			if (SwapChain)
		...
		{
			UE_LOGF(LogVulkanRHI, Error, "Swapchain present failed!");
			bResult = false;
		}
		else
		{
...
{
	if (FVulkanDynamicRHI::Get().bIsStandaloneStereoDevice)
	{
		return PF_R8G8B8A8;
	}
	else
 ...
 *=============================================================================*/
FViewportRHIRef FVulkanDynamicRHI::RHICreateViewport(void* WindowHandle, uint32 SizeX, uint32 SizeY, bool bIsFullscreen, EPixelFormat PixelFormat)
{
	check(IsInGameThread());
	return new FVulkanViewport(*Device, WindowHandle, SizeX, SizeY, bIsFullscreen, PixelFormat);
}

void FVulkanDynamicRHI::RHIResizeViewport(FRHIViewport* ViewportRHI, uint32 SizeX, uint32 SizeY, bool bIsFullscreen, EPixelFormat PixelFormat)
{
	check(IsInGameThread());
	FVulkanViewport* Viewport = ResourceCast(ViewportRHI);

	if (Viewport->GetSizeXY() != FIntPoint(SizeX, SizeY) || Viewport->IsFullscreen() != bIsFullscreen || Viewport->PixelFormat != PixelFormat)
	{
		FVulkanPlatformWindowContext WindowContext(Viewport->WindowHandle);

		ENQUEUE_RENDER_COMMAND(ResizeViewport)(
			[Viewport, SizeX, SizeY, bIsFullscreen, PixelFormat, &WindowContext](FRHICommandListImmediate& RHICmdList)
			{
				Viewport->Resize(RHICmdList, SizeX, SizeY, bIsFullscreen, PixelFormat, WindowContext);
			});
		FlushRenderingCommands();
	}
}
