...
public:
	...
	virtual ~FVulkanCommonPipelineDescriptorState() = default;
	...
	inline void SetUniformBuffer(uint8 DescriptorSet, uint32 BindingIndex, const FVulkanUniformBuffer* UniformBuffer, bool bDynamic)
	{
		...
		{
			check(!bDynamic);
			MarkDirty(DSWriter[DescriptorSet].WriteBindlessUniformBuffer(Device, BindingIndex, UniformBuffer));
		}
		else if (bDynamic)
		{
	...
	virtual TArray<uint32> GetExcludedResources(uint32 SetIndex) const = 0;
...
struct FVulkanPackedUniformBufferState
{
	FPackedUniformBuffers UniformBuffers;
	uint32 Mask = 0;
	uint32 Dirty = 0;

	void Init(const FVulkanShaderHeader& CodeHeader)
	{
		UniformBuffers.Init(CodeHeader, Mask);
	}
	...
	void SetParameter(uint32 ByteOffset, uint32 NumBytes, const void* NewValue)
	{
		UniformBuffers.SetPackedGlobalParameter(ByteOffset, NumBytes, NewValue, Dirty);
	}
};
...
public:
	...
	void Reset()
	{
		...
		PackedUniformBufferState.Reset();
	}

	FVulkanPackedUniformBufferState& GetUniformBuffers(uint8 Stage)
	{
		return PackedUniformBufferState;
	}

	void SetParameter(uint32 ByteOffset, uint32 NumBytes, const void* NewValue)
	{
		PackedUniformBufferState.SetParameter(ByteOffset, NumBytes, NewValue);
	}
	...
	TArray<uint32> GetExcludedResources(uint32 SetIndex) const override final;

protected:
	FVulkanPackedUniformBufferState PackedUniformBufferState;
...
public:
	...
	FVulkanPackedUniformBufferState& GetUniformBuffers(uint8 Stage)
	{
		return PackedUniformBufferStates[Stage];
	}

	void SetParameter(uint8 Stage, uint32 ByteOffset, uint32 NumBytes, const void* NewValue)
	{
		PackedUniformBufferStates[Stage].SetParameter(ByteOffset, NumBytes, NewValue);
	}
	...
	void Reset()
	{
		for (FVulkanPackedUniformBufferState& State : PackedUniformBufferStates)
		{
			State.Reset();
		}
...
protected:
	TStaticArray<FVulkanPackedUniformBufferState, ShaderStage::NumGraphicsStages> PackedUniformBufferStates;
