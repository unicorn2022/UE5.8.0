...
VkBufferUsageFlags FVulkanBuffer::UEToVKBufferUsageFlags(FVulkanDevice& InDevice, EBufferUsageFlags InUEUsage, bool bZeroSize)
{
	...
	if (!bZeroSize)
	{
		...
		if (InDevice.GetOptionalExtensions().HasRaytracingExtensions())
		{
			...
			// Add storage bit for when we bind hit groups in FVulkanRayTracingGeometry::SetupHitGroupSystemParameters
			if (EnumHasAnyFlags(InUEUsage, BUF_VertexBuffer | BUF_IndexBuffer) && !EnumHasAnyFlags(InUEUsage, BUF_AccelerationStructure))
			{
	...
	, Device(InDevice)
{
	...
	if (!bZeroSize)
	{
		...
		else if (bIsSparseResource)
		{
			...
			{
				CurrentBufferAlloc.Alloc.HandleId = ++GVulkanBufferViewHandleIdCounter;
			}
			...
			checkf(SparseBlockSize % ReservedResourceData->MemoryRequirements.alignment == 0, 
				TEXT("The value of r.Vulkan.SparseBufferAllocSizeMB (%dMB) is not compatible with this resource's tile size requirements (%lld)."),
...
void FVulkanBuffer::Unlock(FRHICommandListBase& RHICmdList)
{
	...
	else
	{
		...
		const bool bNeedsTransitions = RHICmdList.NeedsExtraTransitions();
		RHICmdList.EnqueueLambda(TEXT("FVulkanBuffer::Unlock"), [Buffer=this, PendingLock, bNeedsTransitions](FRHICommandListBase& ExecutingCmdList)
		{
			...
			else if (PendingLock.LockMode == RLM_WriteOnly)
			{
				...
				{
					CommonContext = &FVulkanUploadContext::Get(ExecutingCmdList);
				}
				else
				{
					CommonContext = &FVulkanCommandListContext::Get(ExecutingCmdList);
				}
				...
				// High level code expects the data in Buffer to be ready to read, unless the transitions are batched in a pass with multiple copies
