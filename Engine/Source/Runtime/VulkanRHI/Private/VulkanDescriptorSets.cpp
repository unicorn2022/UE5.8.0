...
void FVulkanDescriptorSetsLayoutInfo::FinalizeBindings(const FVulkanDevice& Device, const FUniformBufferGatherInfo& UBGatherInfo, bool bUsesBindless)
{
	...
	CompileTypesUsageID();
	GenerateHash(bIsCompute ? VK_PIPELINE_BIND_POINT_COMPUTE : VK_PIPELINE_BIND_POINT_GRAPHICS);
}

template void FVulkanDescriptorSetsLayoutInfo::FinalizeBindings<true>(const FVulkanDevice& Device, const FUniformBufferGatherInfo& UBGatherInfo, bool bUsesBindless);
template void FVulkanDescriptorSetsLayoutInfo::FinalizeBindings<false>(const FVulkanDevice& Device, const FUniformBufferGatherInfo& UBGatherInfo, bool bUsesBindless);
...
void FVulkanDescriptorSetsLayoutInfo::GenerateHash(VkPipelineBindPoint InBindPoint)
{
...
#undef VKSWITCHCASE
	...
	auto GetWrittenMask = [WrittenMask=WrittenMask, ExcludedMask=ExcludedMask](int32 Index)
	{
			return WrittenMask[Index] | ExcludedMask[Index];
	};

	bool bFail = false;
	if (Writes <= 32) //early out for the most common case.
	{
		bFail = GetWrittenMask(0) != ((1llu << Writes) - 1);
	}
	else
	{
		...
		{
			const uint64 Mask = GetWrittenMask(i);
		...
		bFail = bFail || TailMask != GetWrittenMask(Last);
	}
	...
	if (bFail)
	{
		...
		UE_LOGF(LogVulkanRHI, Warning, "Not All descriptors where filled out. this can/will cause a driver crash\n%ls\n", *Descriptors);
...
void FVulkanDescriptorSetWriter::InitWrittenMasks(uint32 NumDescriptorWrites)
{
	...
	ExcludedMask.Empty(Size);
	ExcludedMask.SetNumZeroed(Size);
#endif
}

void FVulkanDescriptorSetWriter::SetExcludedResources(const TArrayView<uint32> ExcludedResourceCount)
{
#if VULKAN_VALIDATE_DESCRIPTORS_WRITTEN	
	for (uint32 DescriptorIndex : ExcludedResourceCount)
	{
		const uint32 Index = DescriptorIndex / 32;
		const uint32 Mask = DescriptorIndex % 32;
		ExcludedMask[Index] |= (1 << Mask);
	}
#endif
}

bool FVulkanDescriptorSetWriter::WriteBindlessUniformBuffer(FVulkanDevice& Device, uint32 DescriptorIndex, const FVulkanUniformBuffer* UniformBuffer)
{
	if (Device.GetBindlessDescriptorManager()->UseDescriptorHeaps())
	{
		BindlessUBIndices[DescriptorIndex] = UniformBuffer->GetBindlessHandle().GetIndex();
		return true;
	}
	else
	{
		const VulkanRHI::FVulkanAllocation& Allocation = UniformBuffer->GetAllocation();
		const VkDeviceSize Range = UniformBuffer->IsUniformView() ? PLATFORM_MAX_UNIFORM_BUFFER_RANGE : UniformBuffer->GetSize();
		return WriteUniformBuffer(DescriptorIndex, Allocation.GetBufferHandle(), Allocation.HandleId, UniformBuffer->GetOffset(), Range);
	}
}
