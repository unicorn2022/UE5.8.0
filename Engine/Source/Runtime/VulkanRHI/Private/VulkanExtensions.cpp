...
TAutoConsoleVariable<int32> GVulkanAllowDynamicRenderingCVar(
	...
	TEXT("0: Do not enable support for dynamic rendering\n")
	TEXT("1: Enable dynamic rendering (default)"),
	ECVF_ReadOnly
);
...
TAutoConsoleVariable<int32> GVulkanAllowDynamicStatesCVar(
	...
	0,
	TEXT("Enables the use of VK_EXT_extended_dynamic_state1, 2, 3 and \n")
	TEXT("VK_EXT_vertex_input_dynamic_state to reduce amount of unique pipelines. \n")
	TEXT("0: Do not enable support for a dynamic state extensions (default)\n")
	TEXT("1: Enable only VK_EXT_extended_dynamic_state\n")
	TEXT("2: Enable only VK_EXT_extended_dynamic_state2\n")
	TEXT("4: Enable only VK_EXT_extended_dynamic_state3\n")
	TEXT("8: Enable only VK_EXT_vertex_input_dynamic_state\n")
	TEXT("7: Enable VK_EXT_extended_dynamic_state1,2,3.\n")
	TEXT("15: Enable all dynamic state extensions.\n"),
	ECVF_ReadOnly
);
...
TAutoConsoleVariable<int32> GVulkanAllowRobustness2CVar(
	TEXT("r.Vulkan.AllowRobustness2"),
	0,
	TEXT("Enables the use of VK_EXT_Robustness2. \n")
	TEXT("0: Do not enable support for Robustness2 (default)\n")
	TEXT("1: Enable Robustness2 if available"),
	ECVF_ReadOnly
);
...
TAutoConsoleVariable<int32> GVulkanAllowFDMOffsetCVar(
	TEXT("r.Vulkan.AllowFDMOffset"),
	0,
	TEXT("Enables the use of VK_EXT_fragment_density_map_offset\n")
	TEXT("0: Do not enable support for fragment_density_map_offset (default)\n")
	TEXT("1: Enable fragment_density_map_offset if available"),
	ECVF_ReadOnly
);

TAutoConsoleVariable<int32> GVulkanAllowImageRobustness(
	TEXT("r.Vulkan.AllowImageRobustness"),
	1,
	TEXT("Whether to allow VK_EXT_image_robustness\n")
	TEXT("0 to disable\n")
	TEXT("1 to enable (default)"),
	ECVF_ReadOnly
);

TAutoConsoleVariable<int32> GVulkanAllowAddressBindingReport(
	TEXT("r.Vulkan.AllowAddressBindingReport"),
	0,
	TEXT("Whether to allow VK_EXT_device_address_binding_report\n")
...
TArray<TSharedRef<FVulkanRHIExternalDeviceExtensionBase>> FVulkanDeviceExtension::ExternalExtensions;
...
// ***** VK_KHR_maintenance6
class FVulkanKHRMaintenance6Extension : public FVulkanDeviceExtension
{
public:

	FVulkanKHRMaintenance6Extension(FVulkanDevice* InDevice)
		: FVulkanDeviceExtension(InDevice, VK_KHR_MAINTENANCE_6_EXTENSION_NAME, VULKAN_EXTENSION_ENABLED, VK_API_VERSION_1_4)
	{
	}

	virtual void PrePhysicalDeviceFeatures(VkPhysicalDeviceFeatures2KHR& PhysicalDeviceFeatures2) override final
	{
		ZeroVulkanStruct(Maintenance6Features, VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_MAINTENANCE_6_FEATURES_KHR);
		AddToPNext(PhysicalDeviceFeatures2, Maintenance6Features);
	}

	virtual void PostPhysicalDeviceFeatures(FOptionalVulkanDeviceExtensions& ExtensionFlags) override final
	{
		bRequirementsPassed = (Maintenance6Features.maintenance6 == VK_TRUE);
		if (bRequirementsPassed)
		{
			ExtensionFlags.HasKHRMaintenance6 = true;
		}
	}
	...
	virtual void PreCreateDevice(VkDeviceCreateInfo& DeviceCreateInfo) override final
	{
		if (bRequirementsPassed)
		{
			AddToPNext(DeviceCreateInfo, Maintenance6Features);
		}
	}
...
// ***** VK_KHR_maintenance9
class FVulkanKHRMaintenance9Extension : public FVulkanDeviceExtension
{
public:

	FVulkanKHRMaintenance9Extension(FVulkanDevice* InDevice)
		: FVulkanDeviceExtension(InDevice, VK_KHR_MAINTENANCE_9_EXTENSION_NAME, VULKAN_EXTENSION_ENABLED)
	{
	}

	virtual void PrePhysicalDeviceFeatures(VkPhysicalDeviceFeatures2KHR& PhysicalDeviceFeatures2) override final
	{
		ZeroVulkanStruct(Maintenance9Features, VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_MAINTENANCE_9_FEATURES_KHR);
		AddToPNext(PhysicalDeviceFeatures2, Maintenance9Features);
	}

	virtual void PostPhysicalDeviceFeatures(FOptionalVulkanDeviceExtensions& ExtensionFlags) override final
	{
		bRequirementsPassed = (Maintenance9Features.maintenance9 == VK_TRUE);
		if (bRequirementsPassed)
		{
			ExtensionFlags.HasKHRMaintenance9 = true;
		}
	}
	...
	virtual void PreCreateDevice(VkDeviceCreateInfo& DeviceCreateInfo) override final
	{
		if (bRequirementsPassed)
		{
			AddToPNext(DeviceCreateInfo, Maintenance9Features);
		}
	}
...
public:
	...
	virtual void PostPhysicalDeviceProperties(FOptionalVulkanDeviceExtensions& ExtensionFlags) override final
	{
		...
		UE_LOGF(LogVulkanRHI, Display, "Vulkan Physical Device Driver Properties:");
		UE_LOGF(LogVulkanRHI, Display, "- driverName: %s", PhysicalDeviceDriverProperties.driverName);
		UE_LOGF(LogVulkanRHI, Display, "- driverInfo: %s", PhysicalDeviceDriverProperties.driverInfo);
	}
};
...
public:
		...
		: FVulkanDeviceExtension(InDevice, VK_KHR_FRAGMENT_SHADING_RATE_EXTENSION_NAME, VULKAN_EXTENSION_ENABLED)
	{
		...
		bEnabledInCode &= (VRSFormatPreference <= (uint8) EVulkanVariableRateShadingPreference::PreferFDM);
	}
	...
	virtual void PostPhysicalDeviceProperties(FOptionalVulkanDeviceExtensions& ExtensionFlags) override final
	{
		...
		if (FragmentShadingRateFeatures.attachmentFragmentShadingRate == VK_TRUE)
		{
			...
			UE_LOGF(LogVulkanRHI, Verbose, "Image-based Variable Rate Shading supported via KHRFragmentShadingRate extension. Selected VRS tile size %u by %u pixels per VRS image texel.", GRHIVariableRateShadingImageTileMinWidth, GRHIVariableRateShadingImageTileMinHeight);
		}
	}
...
public:
	...
	virtual void PostPhysicalDeviceProperties(FOptionalVulkanDeviceExtensions& ExtensionFlags) override final
	{
		...
		if ((!GRHISupportsAttachmentVariableRateShading || (GVulkanVariableRateShadingFormatCVar->GetInt() >= (uint8) EVulkanVariableRateShadingPreference::PreferFDM)) && bRequirementsPassed)
		{
			...
			UE_LOGF(LogVulkanRHI, Display, "Image-based Variable Rate Shading supported via EXTFragmentDensityMap extension. Selected VRS tile size %u by %u pixels per VRS image texel.", GRHIVariableRateShadingImageTileMinWidth, GRHIVariableRateShadingImageTileMinHeight);
		}
	}
...
class FVulkanEXTFragmentDensityMapOffsetExtension : public FVulkanDeviceExtension
{
public:

	FVulkanEXTFragmentDensityMapOffsetExtension(FVulkanDevice* InDevice)
		: FVulkanDeviceExtension(InDevice, VK_EXT_FRAGMENT_DENSITY_MAP_OFFSET_EXTENSION_NAME, VULKAN_EXTENSION_ENABLED)
	{
		bEnabledInCode = bEnabledInCode && 
			HardwareVariableRateShadingSupportedByPlatform(GMaxRHIShaderPlatform) && GVulkanAllowFDMOffsetCVar.GetValueOnAnyThread();
	}

	virtual void PrePhysicalDeviceFeatures(VkPhysicalDeviceFeatures2KHR& PhysicalDeviceFeatures2) override final
	{
		ZeroVulkanStruct(FragmentDensityMapOffsetFeatures, VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_FRAGMENT_DENSITY_MAP_OFFSET_FEATURES_EXT);
		AddToPNext(PhysicalDeviceFeatures2, FragmentDensityMapOffsetFeatures);
	}

	virtual void PostPhysicalDeviceFeatures(FOptionalVulkanDeviceExtensions& ExtensionFlags) override final
	{
		bRequirementsPassed = (FragmentDensityMapOffsetFeatures.fragmentDensityMapOffset == VK_TRUE) && (ExtensionFlags.HasEXTFragmentDensityMap || ExtensionFlags.HasEXTFragmentDensityMap2);
		ExtensionFlags.HasEXTFragmentDensityMapOffset = bRequirementsPassed;
	}

	virtual void PrePhysicalDeviceProperties(VkPhysicalDeviceProperties2KHR& PhysicalDeviceProperties2) override final
	{
		ZeroVulkanStruct(FragmentDensityMapOffsetProperties, VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_FRAGMENT_DENSITY_MAP_OFFSET_PROPERTIES_EXT);
	...
	virtual void PostPhysicalDeviceProperties(FOptionalVulkanDeviceExtensions& ExtensionFlags) override final
	{
		if (bRequirementsPassed)
		{
			GRHISupportsVariableRateShadingImageOffsets = true;
...
private:
	VkPhysicalDeviceFragmentDensityMapOffsetFeaturesEXT FragmentDensityMapOffsetFeatures;
	VkPhysicalDeviceFragmentDensityMapOffsetPropertiesEXT FragmentDensityMapOffsetProperties;
};
...
public:
	...
	{
		VkPhysicalDeviceFragmentShaderBarycentricPropertiesKHR& FragmentShaderBarycentricProps = GetDeviceExtensionProperties().FragmentShaderBarycentricProps;
		//UE_LOGF(LogVulkanRHI, Verbose, "triStripVertexOrderIndependentOfProvokingVertex is %ls", FragmentShaderBarycentricProps.triStripVertexOrderIndependentOfProvokingVertex ? TEXT("true") : TEXT("false"));
	}
...
// ***** VK_NV_ray_tracing_linear_swept_spheres (vendor)
class FVulkanNVRayTracingLinearSweptSphereExtension : public FVulkanDeviceExtension
{
public:

	FVulkanNVRayTracingLinearSweptSphereExtension(FVulkanDevice* InDevice)
		: FVulkanDeviceExtension(InDevice, VK_NV_RAY_TRACING_LINEAR_SWEPT_SPHERES_EXTENSION_NAME, VULKAN_EXTENSION_ENABLED)
	{
		bEnabledInCode = bEnabledInCode && UE::RHICore::AllowVendorDevice();
	}

	virtual void PrePhysicalDeviceFeatures(VkPhysicalDeviceFeatures2KHR& PhysicalDeviceFeatures2) override final
	{
		ZeroVulkanStruct(LinearSweptSphereFeaturesNV, VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_RAY_TRACING_LINEAR_SWEPT_SPHERES_FEATURES_NV);
		AddToPNext(PhysicalDeviceFeatures2, LinearSweptSphereFeaturesNV);
	}

	virtual void PostPhysicalDeviceFeatures(FOptionalVulkanDeviceExtensions& ExtensionFlags) override final
	{
		bRequirementsPassed = (LinearSweptSphereFeaturesNV.linearSweptSpheres == VK_TRUE);
	}
	...
	virtual void PreCreateDevice(VkDeviceCreateInfo& DeviceCreateInfo) override final
	{
		...
		if (bRequirementsPassed)
		{
			AddToPNext(DeviceCreateInfo, LinearSweptSphereFeaturesNV);

			GRHIGlobals.RayTracing.SupportsLinearSweptSpheres = (LinearSweptSphereFeaturesNV.linearSweptSpheres == VK_TRUE);
		}
	}

private:
	VkPhysicalDeviceRayTracingLinearSweptSpheresFeaturesNV LinearSweptSphereFeaturesNV;
};
...
class FVulkanEXTDeviceAddressBindingReportExtension : public FVulkanDeviceExtension
{
public:

	FVulkanEXTDeviceAddressBindingReportExtension(FVulkanDevice* InDevice)
		: FVulkanDeviceExtension(InDevice, VK_EXT_DEVICE_ADDRESS_BINDING_REPORT_EXTENSION_NAME, VULKAN_EXTENSION_ENABLED)
	{
		bEnabledInCode = bEnabledInCode && UE::RHI::UseGPUCrashDebugging() && GVulkanAllowAddressBindingReport.GetValueOnAnyThread();
	}

	virtual void PrePhysicalDeviceFeatures(VkPhysicalDeviceFeatures2KHR& PhysicalDeviceFeatures2) override final
	{
		ZeroVulkanStruct(DeviceAddressBindingReportFeatures, VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_ADDRESS_BINDING_REPORT_FEATURES_EXT);
		AddToPNext(PhysicalDeviceFeatures2, DeviceAddressBindingReportFeatures);
	}

	virtual void PostPhysicalDeviceFeatures(FOptionalVulkanDeviceExtensions& ExtensionFlags) override final
	{
		bRequirementsPassed = (DeviceAddressBindingReportFeatures.reportAddressBinding == VK_TRUE);
		ExtensionFlags.HasEXTDeviceAddressBindingReport = bRequirementsPassed;
	}
	...
	virtual void PreCreateDevice(VkDeviceCreateInfo& DeviceCreateInfo) override final
	{
		if (bRequirementsPassed)
		{
			AddToPNext(DeviceCreateInfo, DeviceAddressBindingReportFeatures);
		}
	}
...
public:
	...
	virtual void PostPhysicalDeviceProperties(FOptionalVulkanDeviceExtensions& ExtensionFlags) override final
	{
		if (bRequirementsPassed)
		{
			...
			if (DescriptorBufferProperties.bufferlessPushDescriptors == VK_FALSE)
			{
				UE_LOGF(LogVulkanRHI, Display, "Disabling Vulkan Descriptor Buffers because bufferless isn't supported.");

				ExtensionFlags.HasEXTDescriptorBuffer = false;
			}
		}
	}
	...
	virtual void PreCreateDevice(VkDeviceCreateInfo& DeviceCreateInfo) override final
	{
		if (bRequirementsPassed)
		{
			...
			UE_LOGF(LogVulkanRHI, Display, "Enabling Vulkan Descriptor Buffers with: "
				"allowSamplerImageViewPostSubmitCreation=%u, maxDescriptorBufferBindings=%u, "
				"maxSamplerDescriptorBufferBindings=%u, maxResourceDescriptorBufferBindings=%u, "
				"samplerDescriptorBufferAddressSpaceSize=%llu, resourceDescriptorBufferAddressSpaceSize=%llu, "
				"maxSamplerDescriptorBufferRange=%llu, maxResourceDescriptorBufferRange=%llu, "
				"descriptorBufferAddressSpaceSize=%llu, descriptorBufferOffsetAlignment=%llu, "
				"samplerDescriptorSize=%llu",
...
class FVulkanKHRShaderUntypedPointersExtension : public FVulkanDeviceExtension
{
public:

	FVulkanKHRShaderUntypedPointersExtension(FVulkanDevice* InDevice)
		: FVulkanDeviceExtension(InDevice, VK_KHR_SHADER_UNTYPED_POINTERS_EXTENSION_NAME, VULKAN_EXTENSION_ENABLED)
	{
	}

	virtual void PrePhysicalDeviceFeatures(VkPhysicalDeviceFeatures2KHR& PhysicalDeviceFeatures2) override final
	{
		ZeroVulkanStruct(ShaderUntypedPointersFeatures, VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_SHADER_UNTYPED_POINTERS_FEATURES_KHR);
		AddToPNext(PhysicalDeviceFeatures2, ShaderUntypedPointersFeatures);
	}

	virtual void PostPhysicalDeviceFeatures(FOptionalVulkanDeviceExtensions& ExtensionFlags) override final
	{
		bRequirementsPassed = ShaderUntypedPointersFeatures.shaderUntypedPointers == VK_TRUE;
	}
	...
	virtual void PreCreateDevice(VkDeviceCreateInfo& DeviceCreateInfo) override final
	{
		if (bRequirementsPassed)
		{
			AddToPNext(DeviceCreateInfo, ShaderUntypedPointersFeatures);
		}
	}
...
class FVulkanEXTDescriptorHeapExtension : public FVulkanDeviceExtension
{
public:

	FVulkanEXTDescriptorHeapExtension(FVulkanDevice* InDevice)
		: FVulkanDeviceExtension(InDevice, VK_EXT_DESCRIPTOR_HEAP_EXTENSION_NAME, VULKAN_EXTENSION_ENABLED)
	{
	}

	virtual void PrePhysicalDeviceFeatures(VkPhysicalDeviceFeatures2KHR& PhysicalDeviceFeatures2) override final
	{
		ZeroVulkanStruct(DescriptorHeapFeatures, VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_DESCRIPTOR_HEAP_FEATURES_EXT);
		AddToPNext(PhysicalDeviceFeatures2, DescriptorHeapFeatures);
	}

	virtual void PostPhysicalDeviceFeatures(FOptionalVulkanDeviceExtensions& ExtensionFlags) override final
	{
		bRequirementsPassed = DescriptorHeapFeatures.descriptorHeap == VK_TRUE;
		ExtensionFlags.HasEXTDescriptorHeap = bRequirementsPassed;
	}
	...
	virtual void PrePhysicalDeviceProperties(VkPhysicalDeviceProperties2KHR& PhysicalDeviceProperties2) override final
	{
		if (bRequirementsPassed)
		{
			VkPhysicalDeviceDescriptorHeapPropertiesEXT& DescriptorHeapProperties = GetDeviceExtensionProperties().DescriptorHeapProps;
			ZeroVulkanStruct(DescriptorHeapProperties, VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_DESCRIPTOR_HEAP_PROPERTIES_EXT);
			AddToPNext(PhysicalDeviceProperties2, DescriptorHeapProperties);
		}
	}
	...
	virtual void PostPhysicalDeviceProperties(FOptionalVulkanDeviceExtensions& ExtensionFlags) override final
	{
		if (bRequirementsPassed)
		{
			const VkPhysicalDeviceDescriptorHeapPropertiesEXT& DescriptorBufferProperties = GetDeviceExtensionProperties().DescriptorHeapProps;
			UE_LOGF(LogVulkanRHI, Display, "Descriptor Heap is supported with following properties: "
				"maxSamplerHeapSize=%llu, maxResourceHeapSize=%llu, maxPushDataSize=%llu, "
				"samplerDescriptorAlignment=%llu, imageDescriptorAlignment=%llu, bufferDescriptorAlignment=%llu, "
				"samplerDescriptorSize=%llu, imageDescriptorSize=%llu, bufferDescriptorSize=%llu, ",
				DescriptorBufferProperties.maxSamplerHeapSize, DescriptorBufferProperties.maxResourceHeapSize, DescriptorBufferProperties.maxPushDataSize,
				DescriptorBufferProperties.samplerDescriptorAlignment, DescriptorBufferProperties.imageDescriptorAlignment, DescriptorBufferProperties.bufferDescriptorAlignment,
				DescriptorBufferProperties.samplerDescriptorSize, DescriptorBufferProperties.imageDescriptorSize, DescriptorBufferProperties.bufferDescriptorSize
				);
		}
	}
	...
	virtual void PreCreateDevice(VkDeviceCreateInfo& DeviceCreateInfo) override final
	{
		if (bRequirementsPassed)
		{
			AddToPNext(DeviceCreateInfo, DescriptorHeapFeatures);
		}
	}
...
class FVulkanEXTImageRobustnessExtension : public FVulkanDeviceExtension
{
public:

	FVulkanEXTImageRobustnessExtension(FVulkanDevice* InDevice)
		: FVulkanDeviceExtension(InDevice, VK_EXT_IMAGE_ROBUSTNESS_EXTENSION_NAME, VULKAN_EXTENSION_ENABLED)
	{
		bEnabledInCode = bEnabledInCode && (GVulkanAllowImageRobustness.GetValueOnAnyThread() != 0);
	}

	virtual void PrePhysicalDeviceFeatures(VkPhysicalDeviceFeatures2KHR& PhysicalDeviceFeatures2) override final
	{
		ZeroVulkanStruct(ImageRobustnessFeature, VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_IMAGE_ROBUSTNESS_FEATURES_EXT);
		AddToPNext(PhysicalDeviceFeatures2, ImageRobustnessFeature);
	}

	virtual void PostPhysicalDeviceFeatures(FOptionalVulkanDeviceExtensions& ExtensionFlags) override final
	{
		bRequirementsPassed = (ImageRobustnessFeature.robustImageAccess == VK_TRUE);
		ExtensionFlags.HasEXTImageRobustness = bRequirementsPassed;
	}
	...
	virtual void PreCreateDevice(VkDeviceCreateInfo& DeviceCreateInfo) override final
	{
		if (bRequirementsPassed)
		{
			AddToPNext(DeviceCreateInfo, ImageRobustnessFeature);
		}
	}

	VkPhysicalDeviceImageRobustnessFeaturesEXT ImageRobustnessFeature;
};
...
public:
	...
	virtual void PostPhysicalDeviceFeatures(FOptionalVulkanDeviceExtensions& ExtensionFlags) override final
	{
		...
		ExtensionFlags.HasEXTMutableDescriptorType = bRequirementsPassed;
	}
...
public:
	...
	{
		bEnabledInCode = bEnabledInCode && ((GVulkanAllowDynamicStatesCVar.GetValueOnAnyThread() & 1) != 0);
	}
...
public:
	...
	{
		bEnabledInCode = bEnabledInCode && ((GVulkanAllowDynamicStatesCVar.GetValueOnAnyThread() & 2) != 0);
	}
...
public:
	...
	{
		bEnabledInCode = bEnabledInCode && ((GVulkanAllowDynamicStatesCVar.GetValueOnAnyThread() & 4) != 0);
	}
...
public:
	...
	{
		bEnabledInCode = bEnabledInCode && ((GVulkanAllowDynamicStatesCVar.GetValueOnAnyThread() & 8) != 0);
	}
...
public:
	...
	{
		bEnabledInCode = bEnabledInCode && (GVulkanAllowShaderObjectCVar.GetValueOnAnyThread() != 0);
	}
...
public:
	...
	virtual void PostPhysicalDeviceProperties(FOptionalVulkanDeviceExtensions& ExtensionFlags) override final
	{
		...
		if (bRequirementsPassed)
		{
			UE_LOGF(LogVulkanRHI, Display, "Host Image Copy Enabled (SrcCount=%d, DstCount=%d)", 
...
class FVulkanEXTDeviceGeneratedCommandsExtension : public FVulkanDeviceExtension
{
public:

	FVulkanEXTDeviceGeneratedCommandsExtension(FVulkanDevice* InDevice)
		: FVulkanDeviceExtension(InDevice, VK_EXT_DEVICE_GENERATED_COMMANDS_EXTENSION_NAME, VULKAN_EXTENSION_ENABLED, VK_API_VERSION_1_4)
		, DeviceGeneratedCommandsProperties(GetDeviceExtensionProperties().DeviceGeneratedCommandsProperties)
	{
		bEnabledInCode = bEnabledInCode && FDataDrivenShaderPlatformInfo::GetSupportsShaderBundleDispatch(GMaxRHIShaderPlatform);
	}

	virtual void PrePhysicalDeviceFeatures(VkPhysicalDeviceFeatures2KHR& PhysicalDeviceFeatures2) override final
	{
		ZeroVulkanStruct(DeviceGeneratedCommandsFeatures, VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_DEVICE_GENERATED_COMMANDS_FEATURES_EXT);
		AddToPNext(PhysicalDeviceFeatures2, DeviceGeneratedCommandsFeatures);
	}

	virtual void PostPhysicalDeviceFeatures(FOptionalVulkanDeviceExtensions& ExtensionFlags) override final
	{
		bRequirementsPassed = (DeviceGeneratedCommandsFeatures.deviceGeneratedCommands == VK_TRUE);
	}
	...
	virtual void PrePhysicalDeviceProperties(VkPhysicalDeviceProperties2KHR& PhysicalDeviceProperties2) override final
	{
		if (bRequirementsPassed)
		{
			ZeroVulkanStruct(DeviceGeneratedCommandsProperties, VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_DEVICE_GENERATED_COMMANDS_PROPERTIES_EXT);
			AddToPNext(PhysicalDeviceProperties2, DeviceGeneratedCommandsProperties);
		}
	}

	virtual void PostPhysicalDeviceProperties(FOptionalVulkanDeviceExtensions& ExtensionFlags) override final
	{
		bRequirementsPassed = bRequirementsPassed && (DeviceGeneratedCommandsProperties.maxIndirectCommandsTokenCount >= 32);
		ExtensionFlags.HasEXTDeviceGeneratedCommands = bRequirementsPassed;
		if (bRequirementsPassed)
		{
			UE_LOGF(LogVulkanRHI, Display, "Device Generated Commands Enabled (PipelineCount=%d, SequenceCount=%d)",
				DeviceGeneratedCommandsProperties.maxIndirectPipelineCount, DeviceGeneratedCommandsProperties.maxIndirectSequenceCount);

			GRHISupportsShaderBundleDispatch = true;
			GRHISupportsShaderBundleParallel = false;
		}
	}
	...
	virtual void PreCreateDevice(VkDeviceCreateInfo& DeviceCreateInfo) override final
	{
		if (bRequirementsPassed)
		{
			AddToPNext(DeviceCreateInfo, DeviceGeneratedCommandsFeatures);
		}
	}

	VkPhysicalDeviceDeviceGeneratedCommandsFeaturesEXT DeviceGeneratedCommandsFeatures;
	VkPhysicalDeviceDeviceGeneratedCommandsPropertiesEXT& DeviceGeneratedCommandsProperties;
};
...
public:
	...
	FVulkanEXTRobustness2Extension(FVulkanDevice* InDevice)
		: FVulkanDeviceExtension(InDevice, VK_EXT_ROBUSTNESS_2_EXTENSION_NAME, VULKAN_EXTENSION_ENABLED)
	{
		bEnabledInCode = bEnabledInCode && (GVulkanAllowRobustness2CVar.GetValueOnAnyThread() != 0);
	}

	virtual void PrePhysicalDeviceFeatures(VkPhysicalDeviceFeatures2KHR& PhysicalDeviceFeatures2) override final
	{
		ZeroVulkanStruct(Robustness2Features, VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_ROBUSTNESS_2_FEATURES_EXT);
		AddToPNext(PhysicalDeviceFeatures2, Robustness2Features);
	}

	virtual void PostPhysicalDeviceFeatures(FOptionalVulkanDeviceExtensions& ExtensionFlags) override final
	{
		bRequirementsPassed = Robustness2Features.robustBufferAccess2 || Robustness2Features.robustImageAccess2 || Robustness2Features.nullDescriptor;
	}
	...
	virtual void PrePhysicalDeviceProperties(VkPhysicalDeviceProperties2KHR& PhysicalDeviceProperties2) override final
	{
		if (bRequirementsPassed)
		{
			ZeroVulkanStruct(Robustness2Properties, VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_ROBUSTNESS_2_PROPERTIES_EXT);
			AddToPNext(PhysicalDeviceProperties2, Robustness2Properties);
		}
	}
	...
	virtual void PostPhysicalDeviceProperties(FOptionalVulkanDeviceExtensions& ExtensionFlags) override final
	{
		if (bRequirementsPassed)
		{
			UE_LOGF(LogVulkanRHI, Display, "Robustness2 Enabled: Buffer=%d (Align SB/UB=%u/%u), Image=%d, NullDescriptor=%d",
				Robustness2Features.robustBufferAccess2, 
				(uint32)Robustness2Properties.robustStorageBufferAccessSizeAlignment, (uint32)Robustness2Properties.robustUniformBufferAccessSizeAlignment,
				Robustness2Features.robustImageAccess2, Robustness2Features.nullDescriptor);
		}
	}
	...
	virtual void PreCreateDevice(VkDeviceCreateInfo& DeviceCreateInfo) override final
	{
		if (bRequirementsPassed)
		{
			AddToPNext(DeviceCreateInfo, Robustness2Features);
		}
	}
...
static void FlagExtensionSupport(const TArray<VkExtensionProperties>& ExtensionProperties, TArray<TUniquePtr<ExtensionType>>& UEExtensions, uint32 ApiVersion, const ANSICHAR* ExtensionTypeName)
{
	// Flag the extension support
	UE_LOGF(LogVulkanRHI, Display, "Found %d available %s extensions :", ExtensionProperties.Num(), ExtensionTypeName);
	for (const VkExtensionProperties& Extension : ExtensionProperties)
	{
		...
		UE_LOGF(LogVulkanRHI, Display, "  %s %s", bIsCore ? "*" : bFound ? "+" : "-", Extension.extensionName);
	}
}


class FVulkanExternalDeviceExtensionWrapper : public FVulkanDeviceExtension
{
public: 
	FVulkanExternalDeviceExtensionWrapper(FVulkanDevice* InDevice, TSharedRef<FVulkanRHIExternalDeviceExtensionBase> InExternalDeviceExtensionBase)
		: FVulkanDeviceExtension(InDevice, *InExternalDeviceExtensionBase->ExtensionName, VULKAN_EXTENSION_ENABLED)
		, ExternalDeviceExtensionBase(InExternalDeviceExtensionBase)
	{
	}

	virtual void PrePhysicalDeviceProperties(VkPhysicalDeviceProperties2KHR& PhysicalDeviceProperties2) override final
	{
		VkBaseOutStructure* PropertiesStruct = bRequirementsPassed ? ExternalDeviceExtensionBase->GetPropertiesStruct() : nullptr;
		if (PropertiesStruct)
		{
			PropertiesStruct->pNext = (VkBaseOutStructure*)PhysicalDeviceProperties2.pNext;
			PhysicalDeviceProperties2.pNext = PropertiesStruct;
		}
	}
	...
	virtual void PostPhysicalDeviceProperties(FOptionalVulkanDeviceExtensions& ExtensionFlags) override final
	{
		if (bRequirementsPassed)
		{
			bRequirementsPassed = ExternalDeviceExtensionBase->PropertiesCallback();
		}
	}

	virtual void PrePhysicalDeviceFeatures(VkPhysicalDeviceFeatures2KHR& PhysicalDeviceFeatures2) override final
	{
		VkBaseOutStructure* FeaturesStruct = bRequirementsPassed ? ExternalDeviceExtensionBase->GetFeaturesStruct() : nullptr;
		if (FeaturesStruct)
		{
			FeaturesStruct->pNext = (VkBaseOutStructure*)PhysicalDeviceFeatures2.pNext;
			PhysicalDeviceFeatures2.pNext = FeaturesStruct;
		}
	}
	...
	virtual void PostPhysicalDeviceFeatures(FOptionalVulkanDeviceExtensions& ExtensionFlags) override final
	{
		if (bRequirementsPassed)
		{
			bRequirementsPassed = ExternalDeviceExtensionBase->FeaturesCallback();
		}
	}
	...
	virtual void PreCreateDevice(VkDeviceCreateInfo& DeviceCreateInfo)  override final
	{
		...
		if (FeaturesStruct)
		{
			FeaturesStruct->pNext = (VkBaseOutStructure*)DeviceCreateInfo.pNext;
			DeviceCreateInfo.pNext = FeaturesStruct;
		}
	}

private:
	TSharedRef<FVulkanRHIExternalDeviceExtensionBase> ExternalDeviceExtensionBase;
};
...
FVulkanDeviceExtensionArray FVulkanDeviceExtension::GetUESupportedDeviceExtensions(FVulkanDevice* InDevice, uint32 ApiVersion)
{
	...
	ADD_SIMPLE_EXTENSION(VK_KHR_PUSH_DESCRIPTOR_EXTENSION_NAME,          VULKAN_EXTENSION_ENABLED,             VK_API_VERSION_1_4,            DEVICE_EXT_FLAG_SETTER(HasKHRPushDescriptors));
	...
	ADD_CUSTOM_EXTENSION(FVulkanKHRMaintenance6Extension);
	ADD_CUSTOM_EXTENSION(FVulkanKHRMaintenance7Extension);
	ADD_CUSTOM_EXTENSION(FVulkanKHRMaintenance9Extension);
	...
	ADD_CUSTOM_EXTENSION(FVulkanEXTFragmentDensityMapOffsetExtension)
	...
	ADD_CUSTOM_EXTENSION(FVulkanEXTDeviceAddressBindingReportExtension);
	...
	ADD_CUSTOM_EXTENSION(FVulkanEXTImageRobustnessExtension);
	...
	ADD_CUSTOM_EXTENSION(FVulkanEXTDeviceGeneratedCommandsExtension);
	ADD_CUSTOM_EXTENSION(FVulkanEXTRobustness2Extension);
	ADD_CUSTOM_EXTENSION(FVulkanKHRShaderUntypedPointersExtension);
	ADD_CUSTOM_EXTENSION(FVulkanEXTDescriptorHeapExtension);
	...
	ADD_CUSTOM_EXTENSION(FVulkanNVRayTracingLinearSweptSphereExtension);

	// Add in platform specific extensions
	FVulkanPlatform::GetDeviceExtensions(InDevice, OutUEDeviceExtensions);

	// Merge extensions from outside the RHI into the main array using the wrapper
	for (TSharedRef<FVulkanRHIExternalDeviceExtensionBase>& ExtensionBase : ExternalExtensions)
	{
		const int32 ExtensionIndex = FindExtension(OutUEDeviceExtensions, *ExtensionBase->ExtensionName);
		if (ExtensionIndex == INDEX_NONE)
		{
			OutUEDeviceExtensions.Add(MakeUnique<FVulkanExternalDeviceExtensionWrapper>(InDevice, ExtensionBase));
			UE_LOGF(LogVulkanRHI, Warning, "Adding externally requested extension: %s", *ExtensionBase->ExtensionName);
		}
		else
		{
			OutUEDeviceExtensions[ExtensionIndex]->SetActivated();
			ExtensionBase->DuplicateCallback();
		}
	}

	// Release the external extensions array, the device extensions are holding the references now
	FVulkanDeviceExtension::ExternalExtensions.Empty();
	...
	// Add HMD requested extensions
	{
		...
		if (FVulkanDynamicRHI::HMDVulkanExtensions.IsValid())
		{
			...
			for (const ANSICHAR* ExtensionName : HMDExtensions)
			{
				const int32 ExtensionIndex = FindExtension(OutUEDeviceExtensions, ExtensionName);
				if (ExtensionIndex == INDEX_NONE)
				{
					OutUEDeviceExtensions.Add(MakeUnique<FVulkanExternalDeviceExtensionWrapper>(InDevice, MakeShared<FVulkanRHIExternalDeviceExtensionBase>(ExtensionName)));
					UE_LOGF(LogVulkanRHI, Warning, "Adding HMD requested extension: %s", ExtensionName);
				}
			}
		}
	}
	...
	FlagExtensionSupport(GetDriverSupportedDeviceExtensions(InDevice->GetPhysicalHandle()), OutUEDeviceExtensions, ApiVersion, "device");
...
FVulkanInstanceExtensionArray FVulkanInstanceExtension::GetUESupportedInstanceExtensions(uint32 ApiVersion)
{
	...
	auto ActivateExternalExtensions = [&] (TArray<const ANSICHAR*> Extensions, const ANSICHAR* Requester) {
		for (const ANSICHAR* ExtensionName : Extensions)
		{
			...
			if (ExtensionIndex == INDEX_NONE)
			{
				...
				UE_LOGF(LogVulkanRHI, Warning, "%s requested instance extension [%s] isn't part of the engine's core extension list. Adding it on-the-fly...", Requester, ExtensionName);
			}
			else
			{
	...
	// Add HMD requested extensions
	{
		...
		if (FVulkanDynamicRHI::HMDVulkanExtensions.IsValid())
		{
			...
			ActivateExternalExtensions(HMDExtensions, "HMD");
		}
	}

	// Add extensions added outside the RHI (eg plugins)
	ActivateExternalExtensions(ExternalExtensions, "Externally");

	// Now that all the extensions are listed, update their support flags
	FlagExtensionSupport(GetDriverSupportedInstanceExtensions(), OutUEInstanceExtensions, ApiVersion, "instance");
