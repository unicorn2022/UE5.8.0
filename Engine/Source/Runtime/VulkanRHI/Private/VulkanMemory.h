...
namespace VulkanRHI
{
	...
	public:
		...
		FRHIMemoryStats GetRHIMemoryStats() const { return MemoryStats; }
	...
	public:
		...
		uint8 bTransient : 1;
		uint8 bTempBlockAllocator : 1;

		static_assert(EVulkanAllocationSize < 32, "Not enough bits to hold EVulkanAllocationType");
		...
		VkMemoryPropertyFlags GetMemoryPropertyFlags(FVulkanDevice* Device) const;
	...
	public:
		...
		uint8 GetSubresourceAllocatorFlags() const
		{
		...
		bool GetIsDefragging() const
		{
			return bIsDefragging;
		}
	...
	enum class EVulkanAllocationFlags : uint16
	{
		...
		Protected    = 0x0040,	  // Will use PROTECTED

		NoError      = 0x0080,    // OOM is not fatal, return an invalid allocation
		AutoBind     = 0x0100,	  // Will automatically bind the allocation to the supplied VkBuffer/VkImage, avoids an extra lock to bind separately
	};
	...
	public:
		...
		bool AllocateDedicatedImageMemory(FVulkanAllocation& Allocation, FVulkanEvictable* AllocationOwner, VkImage Image, VkMemoryPropertyFlags MemoryPropertyFlags, EVulkanAllocationMetaType MetaType, bool bExternal, bool bForceDedicatedAllocation, const char* File, uint32 Line);
	private:
...
// 			64 * 1024,
	...
	class FStagingBuffer : public FRefCountedObject
	{
	public:
	...
	public:
		...
		uint8* Alloc(uint32 InSize, uint32 InAlignment, FVulkanContextCommon& Context, FVulkanAllocation& OutAllocation, VkDescriptorAddressInfoEXT* OutDescriptorAddressInfo = nullptr)
		{
			return Alloc(InSize, InAlignment, &Context, OutAllocation, OutDescriptorAddressInfo);
		}

		void FreeReference(const FVulkanAllocation& InAllocation);
