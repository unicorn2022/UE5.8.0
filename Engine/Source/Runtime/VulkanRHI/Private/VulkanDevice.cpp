...
static TAutoConsoleVariable<int32> GRHIAllowAsyncComputeCvar(
	...
	1,
	TEXT("0 to disable async compute queue (default)\n")
	TEXT("1 to allow async compute queue if available"),
	ECVF_ReadOnly
);
...
static TAutoConsoleVariable<int32> GRHIAllowTransferQueueCvar(
	...
	0,
	TEXT("0 to disable transfer queue (default)\n")
	TEXT("1 to allow transfer queue if available"),
	ECVF_ReadOnly
);
...
static void LoadValidationCache(VkDevice Device, VkValidationCacheEXT& OutValidationCache)
{
	...
	UE_LOGF(LogVulkanRHI, Display, "Trying validation cache file %ls", *CacheFilename);
	if (FFileHelper::LoadFileToArray(InData, *CacheFilename, FILEREAD_Silent) && InData.Num() > 0)
	{
		...
		if (*DataPtr > 0)
		{
			...
			{
				UE_LOGF(LogVulkanRHI, Warning, "Bad validation cache file %ls, version=%d, expected %d", *CacheFilename, Version, VK_PIPELINE_CACHE_HEADER_VERSION_ONE);
				InData.Reset(0);
			}
		}
		else
		{
			UE_LOGF(LogVulkanRHI, Warning, "Bad validation cache file %ls, header size=%d", *CacheFilename, *DataPtr);
			InData.Reset(0);
		}
	}
	...
	if (vkCreateValidationCache)
	{
		...
		{
			UE_LOGF(LogVulkanRHI, Warning, "Failed to create Vulkan validation cache, VkResult=%d", Result);
		}
	}
}
#endif
...
void FVulkanPhysicalDeviceFeatures::Query(VkPhysicalDevice PhysicalDevice, uint32 APIVersion)
{
	...
	if (APIVersion >= VK_API_VERSION_1_4)
	{
		...
		Core_1_4.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_VULKAN_1_4_FEATURES;
	}
	...
	, PipelineStateCache(nullptr)
{
	...
	UE_LOGF(LogVulkanRHI, Display, "- DeviceName: %ls", ANSI_TO_TCHAR(GpuProps.deviceName));
	UE_LOGF(LogVulkanRHI, Display, "- API=%d.%d.%d (0x%x) Driver=0x%x VendorId=0x%x", VK_VERSION_MAJOR(GpuProps.apiVersion), VK_VERSION_MINOR(GpuProps.apiVersion), VK_VERSION_PATCH(GpuProps.apiVersion), GpuProps.apiVersion, GpuProps.driverVersion, GpuProps.vendorID);
	UE_LOGF(LogVulkanRHI, Display, "- DeviceID=0x%x Type=%ls", GpuProps.deviceID, VK_TYPE_TO_STRING(VkPhysicalDeviceType, GpuProps.deviceType));
	UE_LOGF(LogVulkanRHI, Display, "- Max Descriptor Sets Bound %d", GpuProps.limits.maxBoundDescriptorSets);
	UE_LOGF(LogVulkanRHI, Display, "- Timestamps:  ComputeAndGraphics=%d  Domain=%ls  Period=%f", 
...
void FVulkanDevice::CreateDevice(TArray<const ANSICHAR*>& DeviceLayers, FVulkanDeviceExtensionArray& UEExtensions)
{
	...
	UE_LOGF(LogVulkanRHI, Display, "Found %d Queue Families", QueueFamilyProps.Num());
	...
	for (int32 FamilyIndex = 0; FamilyIndex < QueueFamilyProps.Num(); ++FamilyIndex)
	{
		...
		if (VKHasAllFlags(CurrProps.queueFlags, VK_QUEUE_COMPUTE_BIT))
		{
			...
			if ((ComputeQueueFamilyIndex == -1) &&
				...
				!FParse::Param(FCommandLine::Get(), TEXT("DisableAsyncCompute")) &&
				(GfxQueueFamilyIndex != FamilyIndex) && 
				SupportsParallelRendering() &&
				OptionalDeviceExtensions.HasKHRMaintenance9)
			{
		...
		{
			// Allocate a transfer queue if:
			// - a transfer queue hasn't been found already
			// - there is a dedicated non graphics/compute queue
			// - cvars allow for a dedicated transfer queue
			// - a new family index is available
			// - Sync2 is available
			// - maintenance9 is available
			if ((TransferQueueFamilyIndex == -1) &&
				...
				(GfxQueueFamilyIndex != FamilyIndex) && (ComputeQueueFamilyIndex != FamilyIndex) &&
				SupportsParallelRendering() &&
				OptionalDeviceExtensions.HasKHRMaintenance9)
			{
		...
		if (!bIsValidQueue)
		{
			UE_LOGF(LogVulkanRHI, Display, "Skipping unnecessary Queue Family %d: %d queues%ls", FamilyIndex, CurrProps.queueCount, *GetQueueInfoString(CurrProps));
			continue;
		}
		...
		UE_LOGF(LogVulkanRHI, Display, "Initializing Queue Family %d: %d queues%ls", FamilyIndex, CurrProps.queueCount, *GetQueueInfoString(CurrProps));
	}
	...
	UE_LOGF(LogVulkanRHI, Display, "TimestampValidBits=%u", NumBits);
	...
	if (HasAsyncComputeQueue() && OptionalDeviceExtensions.HasKHRMaintenance9)
	{
		TArray<VkQueueFamilyProperties2> QueueFamilyProperties2;
		QueueFamilyProperties2.SetNumZeroed(QueueFamilyProps.Num());
		TArray<VkQueueFamilyOwnershipTransferPropertiesKHR> OwnershipTransferProperties;
		OwnershipTransferProperties.SetNumUninitialized(QueueFamilyProperties2.Num());

		for (int32 QueueIndex = 0; QueueIndex < QueueFamilyProperties2.Num(); ++QueueIndex)
		{
			OwnershipTransferProperties[QueueIndex] = {
				.sType = VK_STRUCTURE_TYPE_QUEUE_FAMILY_OWNERSHIP_TRANSFER_PROPERTIES_KHR,
				.pNext = nullptr,
				.optimalImageTransferToQueueFamilies = 0
			};

			QueueFamilyProperties2[QueueIndex] = {
				.sType = VK_STRUCTURE_TYPE_QUEUE_FAMILY_PROPERTIES_2,
				.pNext = &OwnershipTransferProperties[QueueIndex],
			};
		}

		uint32 QueueCount = QueueFamilyProperties2.Num();
		VulkanRHI::vkGetPhysicalDeviceQueueFamilyProperties2(Gpu, &QueueCount, QueueFamilyProperties2.GetData());

		const bool bGraphicsOwnership = OwnershipTransferProperties[GfxQueueFamilyIndex].optimalImageTransferToQueueFamilies & (1u << ComputeQueueFamilyIndex);
		const bool bAsyncOwnership = OwnershipTransferProperties[ComputeQueueFamilyIndex].optimalImageTransferToQueueFamilies & (1u << GfxQueueFamilyIndex);
		const bool bTransferOwnership = (TransferQueueFamilyIndex == -1) || (
			(OwnershipTransferProperties[TransferQueueFamilyIndex].optimalImageTransferToQueueFamilies & (1u << GfxQueueFamilyIndex)) &&
			(OwnershipTransferProperties[TransferQueueFamilyIndex].optimalImageTransferToQueueFamilies & (1u << ComputeQueueFamilyIndex)));
		if (bGraphicsOwnership && bAsyncOwnership && bTransferOwnership)
		{
			// Queue ownership transfers aren't necessary between the used queue families
			bNeedsQueueFamilyOwnershipTransfers = false;
		}
		else
		{
			// Do not merge pipelines together when queue family ownership transfers are necessary
			GRHIMultiPipelineMergeableAccessMask = ERHIAccess::Unknown;
		}
	}
	...
	UE_LOGF(LogVulkanRHI, Display, "Using %d device layers%ls", DeviceLayers.Num(), DeviceLayers.Num() ? TEXT(":") : TEXT("."));
	for (const ANSICHAR* Layer : DeviceLayers)
	{
		UE_LOGF(LogVulkanRHI, Display, "* %ls", ANSI_TO_TCHAR(Layer));
	}

	UE_LOGF(LogVulkanRHI, Display, "Using %d device extensions:", DeviceExtensions.Num());
	for (const ANSICHAR* Extension : DeviceExtensions)
	{
		UE_LOGF(LogVulkanRHI, Display, "* %ls", ANSI_TO_TCHAR(Extension));
	}
...
bool FVulkanDevice::SupportsDeviceLocalHostVisible() const
{
	if (HasUnifiedMemory())
	{
		return true;
	...
	if (DeviceMemoryManager.SupportsMemoryType(VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT | VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT))
	{
		return (VendorId == EGpuVendorId::Amd) || (GpuProps.deviceType == VK_PHYSICAL_DEVICE_TYPE_INTEGRATED_GPU);
	}
...
void FVulkanDevice::SetupDrawMarkers()
{
	...
	if (RHI->SupportsDebugUtilsExt() || bUseLegacyDebugMarkerExt)
	{
		...
		if (OptionalDeviceExtensions.HasEXTToolingInfo)
		{
			...
			for (VkPhysicalDeviceToolPropertiesEXT const& Tool : ToolProperties)
			{
				...
				UE_LOGF(LogVulkanRHI, Display, "Tool \"%ls\" version %ls PurposeFlags=0x%x", ANSI_TO_TCHAR(Tool.name), ANSI_TO_TCHAR(Tool.version), Tool.purposes);
			}
		}
		...
		else
		{
			if (bUseLabel)
			{
				...
				DebugMarkers.CmdInsertLabel = (PFN_vkCmdInsertDebugUtilsLabelEXT)(void*)VulkanRHI::vkGetInstanceProcAddr(RHI->GetInstance(), "vkCmdInsertDebugUtilsLabelEXT");
			}
			if (bUseName)
			{
		...
		UE_LOGF(LogVulkanRHI, Display, "Vulkan debug markers support: resource names %ls, begin/end labels %ls.", bUseName ? TEXT("enabled") : TEXT("disabled"), bUseLabel ? TEXT("enabled") : TEXT("disabled"));
	}
	...
	{
		UE_LOGF(LogVulkanRHI, Warning, "Vulkan API trace tool detected but not running in ideal GPU capture mode.");
	}
#endif // VULKAN_HAS_DEBUGGING_ENABLED
#endif // VULKAN_ENABLE_DRAW_MARKERS
...
void FVulkanDevice::SetupFormats()
{
	...
	if (GMaxRHIFeatureLevel >= ERHIFeatureLevel::SM5)
	{
		...
		{
			UE_LOGF(LogVulkanRHI, Warning, "64bit image atomics were enabled, but the R64 format does not have UAV capabilities.  Disabling support.");
	...
	// Verify available Vertex Formats
	{
		...
		for (int32 VETIndex = (int32)VET_None + 1; VETIndex < VET_MAX; ++VETIndex)
		{
			...
			{
				UE_LOGF(LogVulkanRHI, Warning, "EVertexElementType(%d) is not supported with VkFormat %d", (int32)UEType, (int32)VulkanFormat);
			}
		}
	}
...
#undef VULKAN_CHECK_FORMAT_CAPABILITY
		...
		UE_LOGF(LogVulkanRHI, Warning, "Pixel Format Mappings for Vulkan:");
		UE_LOGF(LogVulkanRHI, Warning, "%24ls | %24ls | BlockBytes | Components | ComponentMapping | BufferFormat | Capabilities | SRGBFormat", 
		...
		for (int32 PixelFormatIndex = 0; PixelFormatIndex < PF_MAX; ++PixelFormatIndex)
		{
			if (GPixelFormats[PixelFormatIndex].Supported)
			{
				...
				UE_LOGF(LogVulkanRHI, Warning, "%24ls | %24ls | %10d | %10d | %10d,%d,%d,%d | %12d |  0x%08X  | %ls",
		...
		UE_LOGF(LogVulkanRHI, Warning, "Pixel Format Capabilities for Vulkan:");
		for (int32 PixelFormatIndex = 0; PixelFormatIndex < PF_MAX; ++PixelFormatIndex)
		{
			if (GPixelFormats[PixelFormatIndex].Supported)
			{
				const FString CapabilitiesString = GetFormatCapabilities(GPixelFormats[PixelFormatIndex].Capabilities);
				UE_LOGF(LogVulkanRHI, Warning, "%24ls : %ls", GPixelFormats[PixelFormatIndex].Name, *CapabilitiesString);
			}
		}
	}
...
void FVulkanDevice::MapImageFormatSupport(FPixelFormatInfo& PixelFormatInfo, const TArrayView<const VkFormat>& PrioritizedFormats, EPixelFormatCapabilities RequiredCapabilities)
{
	...
	for (int32 FormatIndex = 0; FormatIndex < PrioritizedFormats.Num(); ++FormatIndex)
	{
		...
		if (EnumHasAllFlags(Capabilities, RequiredCapabilities))
		{
			...
			if (FormatIndex > 0)
			{
				UE_LOGF(LogVulkanRHI, Display, "MapImageFormatSupport: %ls is not supported with VkFormat %d, falling back to VkFormat %d", PixelFormatInfo.Name, (int32)PrioritizedFormats[0], (int32)PrioritizedFormats[FormatIndex]);
			}
...
void FVulkanDevice::MapFormatSupport(EPixelFormat UEFormat, std::initializer_list<VkFormat> InPrioritizedFormats, const VkComponentMapping& ComponentMapping, EPixelFormatCapabilities RequiredCapabilities, int32 BlockBytes)
{
	...
	{
		UE_LOGF(LogVulkanRHI, Error, "MapFormatSupport: %ls is not supported with VkFormat %d", PixelFormatInfo.Name, (int32)PrioritizedFormats[0]);
	}
}
...
void FVulkanDevice::ChooseVariableRateShadingMethod()
{
	...
	{
		return ExtensionFlags.HasEXTFragmentDensityMap == 1;
	};
	...
	UE_LOGF(LogVulkanRHI, Display, "Vulkan Variable Rate Shading choice: %d.", VRSFormatPreference);

	const bool bFSRAvailable = IsFragmentShadingRateAvailable(OptionalDeviceExtensionProperties.FragmentShadingRateFeatures);
	const bool bFDMAvailable = IsFragmentDensityMapAvailable(OptionalDeviceExtensions);

	// Whether we prefer Fragment Shading Rate, or Fragment Density Map.
	const bool bPreferFSR = (VRSFormatPreference <= (uint8)EVulkanVariableRateShadingPreference::RequireFSR);
	
	// Whether we allow falling back to the other extension if our preferred extension is unavailable.
	const bool bAllowFallback = (VRSFormatPreference == (uint8)EVulkanVariableRateShadingPreference::PreferFSR || VRSFormatPreference == (uint8)EVulkanVariableRateShadingPreference::PreferFDM);

	// Whether we should print an error if our preferred extension is unavailable.
	const bool bErrorIfUnavailable = !bAllowFallback && (VRSFormatPreference == (uint8)EVulkanVariableRateShadingPreference::RequireFSR || VRSFormatPreference == (uint8)EVulkanVariableRateShadingPreference::RequireFDM);

	const bool bPreferredAvailable = bPreferFSR ? bFSRAvailable : bFDMAvailable;
	const bool bFallbackAvailable = bAllowFallback && (bPreferFSR ? bFDMAvailable : bFSRAvailable);

	if (bPreferredAvailable)
	{
		UE_LOGF(LogVulkanRHI, Display, "Fragment %ls was %ls and is available.", bPreferFSR ? TEXT("Shading Rate") : TEXT("Density Map"), bAllowFallback ? TEXT("requested") : TEXT("required"));

		// Turn off our non-preferred method in case both are available in order to prevent Vulkan validation layers warnings.
		if (bPreferFSR)
		{
	...
	{
		if (bErrorIfUnavailable)
		{
			UE_LOGF(LogVulkanRHI, Error, "Fragment %ls was required but is not available.", bPreferFSR ? TEXT("Shading Rate") : TEXT("Density Map"));
		}
		else
		{
			UE_LOGF(LogVulkanRHI, Display, "Fragment %ls was %ls but is not available.", bPreferFSR ? TEXT("Shading Rate") : TEXT("Density Map"), bAllowFallback ? TEXT("requested") : TEXT("required"));
		}
		...
		if (bAllowFallback)
		{
			...
			{
				UE_LOGF(LogVulkanRHI, Display, "Falling back to Fragment %ls.", bPreferFSR ? TEXT("Density Map") : TEXT("Shading Rate"));
			}
			else
			{
				UE_LOGF(LogVulkanRHI, Display, "Fragment %ls is not available as a fallback.", bPreferFSR ? TEXT("Density Map") : TEXT("Shading Rate"));
			}
		}
	}
}
...
void FVulkanDevice::InitGPU()
{
	...
	UE_LOGF(LogVulkanRHI, Display, "Device properties: Geometry %d BufferAtomic64 %lld ImageAtomic64 %lld", 
	...
	ShaderObjectManager = new FVulkanShaderObjectManager(*this);
	...
	ImmediateContext = new FVulkanCommandListContext(*this, ERHIPipeline::Graphics, true);
...
void FVulkanDevice::Destroy()
{
	...
	if (ShaderObjectManager)
	{
		delete ShaderObjectManager;
		ShaderObjectManager = nullptr;
	}
