...
static int32 GVulkanRayTracingMaxShaderGroupStride = 512;
static FAutoConsoleVariableRef GCVarVulkanRayTracingMaxShaderGroupStride(
	...
	TEXT("The default size to allocate for each record (default: 512)."),
	ECVF_ReadOnly
);
	...
	FVkRtBLASBuildData& BuildData)
{
	...
	// Pre allocate LSS Segments to avoid memory reallocation
	if (GeometryType == RTGT_LinearSweptSpheres)
	{
		BuildData.LSSSegments.Reserve(Segments.Num());
	}
	...
	for (int32 SegmentIndex = 0; SegmentIndex < Segments.Num(); ++SegmentIndex)
	{
		...
		VkAccelerationStructureGeometryLinearSweptSpheresDataNV LSSSegmentGeometry;
		ZeroVulkanStruct(LSSSegmentGeometry, VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_GEOMETRY_LINEAR_SWEPT_SPHERES_DATA_NV);
		...
		switch (GeometryType)
		{
			...
			case RTGT_LinearSweptSpheres:
				// no other application rather than hair strands for now.
				check(IndexStrideInBytes == 4);
				check(IndexBufferRHI != nullptr); // must have an index buffer in this case

				// there are 2 indices per primitive
				PrimitiveOffset = Segment.FirstPrimitive * 2 * IndexStrideInBytes;

				LSSSegmentGeometry.indexType = VK_INDEX_TYPE_UINT32;
				LSSSegmentGeometry.indexData = IndexBufferDeviceAddress;
				LSSSegmentGeometry.indexStride = sizeof(uint32);
				LSSSegmentGeometry.indexingMode = VkRayTracingLssIndexingModeNV::VK_RAY_TRACING_LSS_INDEXING_MODE_LIST_NV;
				
				LSSSegmentGeometry.vertexData = VertexBufferDeviceAddress;
				LSSSegmentGeometry.vertexFormat = VkFormat::VK_FORMAT_R32G32B32_SFLOAT;
				LSSSegmentGeometry.vertexStride = 4 * sizeof(float);
				
				LSSSegmentGeometry.radiusData = {.deviceAddress = VertexBufferDeviceAddress.deviceAddress + 3 * sizeof(float)};
				LSSSegmentGeometry.radiusFormat = VkFormat::VK_FORMAT_R32_SFLOAT;
				LSSSegmentGeometry.radiusStride = 4 * sizeof(float);

				LSSSegmentGeometry.endCapsMode = VkRayTracingLssPrimitiveEndCapsModeNV::VK_RAY_TRACING_LSS_PRIMITIVE_END_CAPS_MODE_NONE_NV;
				
				BuildData.LSSSegments.Add(LSSSegmentGeometry);

				SegmentGeometry.pNext = &BuildData.LSSSegments[SegmentIndex];
				SegmentGeometry.geometryType = VkGeometryTypeKHR::VK_GEOMETRY_TYPE_LINEAR_SWEPT_SPHERES_NV;
	...
	: FRHIRayTracingGeometry(InInitializer), Device(InDevice)
{
	...
	DebugName = !Initializer.DebugName.IsNone() ? Initializer.DebugName : FDebugName(FName("BLAS"));
...
void FVulkanRayTracingGeometry::SetupHitGroupSystemParameters()
{
	...
	auto GetBindlessHandle = [BindlessDescriptorManager](FVulkanBuffer* Buffer, uint32 ExtraOffset)
	{
		if (Buffer)
		{
			FRHIDescriptorHandle BindlessHandle = BindlessDescriptorManager->AllocateDescriptor(ERHIDescriptorType::BufferUAV);
			check(ExtraOffset < Buffer->GetCurrentSize());
			BindlessDescriptorManager->UpdateDescriptor({}, BindlessHandle, Buffer->GetHandle(), Buffer->GetOffset() + ExtraOffset, Buffer->GetCurrentSize() - ExtraOffset, true);
...
void FVulkanRayTracingGeometry::ReleaseBindlessHandles()
{
	...
	{
		Device->GetDeferredDeletionQueue().EnqueueBindlessHandle(BindlesHandle);
	}
	HitGroupSystemVertexViews.Reset(Initializer.Segments.Num());

	if (HitGroupSystemIndexView.IsValid())
	{
		Device->GetDeferredDeletionQueue().EnqueueBindlessHandle(HitGroupSystemIndexView);
...
void FVulkanRayTracingScene::BindBuffer(FRHIBuffer* InBuffer, uint32 InBufferOffset)
{
...

	{
		...
		View = MakeUnique<FVulkanView>(Device, ERHIDescriptorType::AccelerationStructure);
		...
		if (const VkAccelerationStructureKHR NativeAccelerationStructureHandle = View->GetAccelerationStructureView().Handle)
		{
			FString DebugNameString = Initializer.DebugName.ToString();
	...
	if (EnumHasAnyFlags(ShaderBindingMode, ERayTracingShaderBindingMode::RTPSO))
	{
		auto InitAlloc = [HandleSizeAligned = HandleSizeAligned](FVulkanShaderTableAllocation& Alloc, uint32 InHandleCount, bool InUseLocalRecord)
		{
			...
			if (Alloc.HandleCount > 0)
			{
				...
				// Host buffer
				Alloc.HostBuffer.SetNumZeroed(Alloc.Region.size);
			}
		};
		...
		auto CreatePipelineLibraryTask = [this, PipelineLibrary, Shader, Key, bPartial=Initializer.bPartial]()
			{
				...
				ShaderStages[ShaderCount].stage = UEFrequencyToVKStageBit(Frequency);  // will default to closestHitShader for SF_RayHitGroup
				...
				const VkRayTracingPipelineInterfaceCreateInfoKHR RayTracingPipelineInterfaceCreateInfo = {
					.sType = VK_STRUCTURE_TYPE_RAY_TRACING_PIPELINE_INTERFACE_CREATE_INFO_KHR,
					.pNext = nullptr,
					.maxPipelineRayPayloadSize = Key.MaxPayloadSizeInBytes,
					.maxPipelineRayHitAttributeSize = Key.MaxAttributeSizeInBytes
				};
				...
				const VkPipelineCreateFlags2CreateInfo Flags2CreateInfo = {
					...
					.flags = VK_PIPELINE_CREATE_2_LIBRARY_BIT_KHR |
							 ...
							 (GRHIGlobals.RayTracing.SupportsLinearSweptSpheres ? VK_PIPELINE_CREATE_2_RAY_TRACING_ALLOW_SPHERES_AND_LINEAR_SWEPT_SPHERES_BIT_NV : 0ull)
				};

				TStaticArray<VkShaderDescriptorSetAndBindingMappingInfoEXT, MaxHitGroupShaderCount> BindingMappingInfos;
				if (Device.GetBindlessDescriptorManager()->UseDescriptorHeaps())
				{
					TConstArrayView<VkDescriptorSetAndBindingMappingEXT> BindingMappings = Device.GetBindlessDescriptorManager()->GetBindingMappings(Frequency, false);
					for (uint32 StageIndex=0; StageIndex < ShaderCount; ++StageIndex)
					{
						BindingMappingInfos[StageIndex] = {
						...
						VkPipelineShaderStageCreateInfo& StageCreateInfo = ShaderStages[StageIndex];
						AddToPNext(StageCreateInfo, BindingMappingInfos[StageIndex]);
					}
				}
				...
				RayTracingPipelineCreateInfo.flags = 0; // using VkPipelineCreateFlags2CreateInfoKHR
				RayTracingPipelineCreateInfo.pNext = &Flags2CreateInfo;
...
void FVulkanDevice::InitializeRayTracing()
{
	if (GRHISupportsRayTracingShaders)
	{
		...
		{
			UE_LOGF(LogRHI, Warning, "Specified value for r.Vulkan.RayTracing.MaxShaderGroupStride is too large for this device! It will be capped.");
		}
...
static FVulkanUniformBuffer* GetDummyUB(FVulkanDevice& Device, uint32 UBLayoutHash)
{
	...
	if (DummyMetadata && DummyMetadata->GetLayoutPtr())
	{
		...
		UE_LOGF(LogRHI, Warning, "Vulkan ray tracing using DummyUB for %ls.", LayoutName.IsEmpty() ? TEXT("<unknown>") : *LayoutName);
		return DummyUB;
	}
	return nullptr;
}
	...
	, bIsPartialPipeline(Initializer.bPartial)
{
	...
	const VkRayTracingPipelineInterfaceCreateInfoKHR RayTracingPipelineInterfaceCreateInfo = {
		.sType = VK_STRUCTURE_TYPE_RAY_TRACING_PIPELINE_INTERFACE_CREATE_INFO_KHR,
		.pNext = nullptr,
		.maxPipelineRayPayloadSize = Initializer.MaxPayloadSizeInBytes,
		.maxPipelineRayHitAttributeSize = Initializer.MaxAttributeSizeInBytes
	};

	const VkPipelineLibraryCreateInfoKHR PipelineLibraryCreateInfo = {
		.sType = VK_STRUCTURE_TYPE_PIPELINE_LIBRARY_CREATE_INFO_KHR,
		.pNext = nullptr,
		.libraryCount = (uint32)Pipelines.Num(),
		.pLibraries = Pipelines.GetData()
	};
	...
	RayTracingPipelineCreateInfo.flags = 0; // using VkPipelineCreateFlags2CreateInfo
	RayTracingPipelineCreateInfo.pNext = &Flags2CreateInfo;
...
{
	const FShaderHash Hash = Shader->GetHash();
...
void FVulkanRayTracingCompactedSizeQueryPool::Reset()
{
	VulkanRHI::vkResetQueryPoolEXT(Device.GetHandle(), QueryPool, 0, MaxQueries);
	SyncPoint = nullptr;
...
static void SetRayGenResources(FVulkanDevice& Device, FVulkanCommandListContext& Context, const FRayTracingShaderBindings& InGlobalResourceBindings)
{
	// Static uniform buffers
	Context.ApplyShaderBindingLayout();
	...
	if (Device.GetBindlessDescriptorManager()->UseDescriptorHeaps())
	{
		TArray<uint32> PushData;
		PushData.Reserve(MaxUniformBuffers);

		uint64 GlobalAddressOrFirstBinding = MAX_uint64;
		for (uint32 UBIndex = 0; UBIndex < MaxUniformBuffers; ++UBIndex)
		{
			...
			if (UniformBuffer)
			{
				check(UniformBuffer->GetBindlessHandle().IsValid());
				if (UBIndex == 0)
				{
					GlobalAddressOrFirstBinding = UniformBuffer->GetBindlessHandle().GetIndex();
				}
				else
				{
					PushData.SetNumZeroed(UBIndex);
					PushData[UBIndex-1] = UniformBuffer->GetBindlessHandle().GetIndex();
				}
			}
		}

		Context.GetCommandBuffer().SetBindingPushConstants(SF_RayGen, GlobalAddressOrFirstBinding, PushData);
	}
	else 
	{
		TArray<VkWriteDescriptorSet, TInlineAllocator<VulkanBindless::MaxUniformBuffersPerStage>> UBWrites;
		TArray<VkDescriptorBufferInfo, TInlineAllocator<VulkanBindless::MaxUniformBuffersPerStage>> UBBufferInfos;
		...
		for (uint32 UBIndex = 0; UBIndex < MaxUniformBuffers; ++UBIndex)
		{
			...
			if (UniformBuffer)
			{
				VkDescriptorBufferInfo& BufferInfo = UBBufferInfos.AddZeroed_GetRef();
				BufferInfo.buffer = UniformBuffer->GetBufferHandle();
				BufferInfo.offset = UniformBuffer->GetOffset();
				BufferInfo.range = UniformBuffer->GetSize();

				VkWriteDescriptorSet& Write = UBWrites.AddZeroed_GetRef();
				Write.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
				Write.dstBinding = UBIndex;
		...
		Device.GetBindlessDescriptorManager()->RegisterUniformBuffers(Context, VK_SHADER_STAGE_RAYGEN_BIT_KHR, UBWrites);
	}
}
	...
	uint32 InWidth, uint32 InHeight)
{
	...
	SetRayGenResources(Device, *this, InGlobalResourceBindings);
