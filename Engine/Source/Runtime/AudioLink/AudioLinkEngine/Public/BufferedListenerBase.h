...
public:
	...
	{
		int32 CanPush = FMath::Min(Remainder(), InNumSamplesOfSilence);
		FScopeLock Lock(&CS);
		for (int32 i = 0; i < CanPush; ++i)
		{
			Enqueue(0.f);
	...
	// Reset read/write pointers, discarding all buffered data without reallocating.
	void Flush()
	{
		FScopeLock Lock(&CS);
		Read = 0;
	...
	// Grow the buffer to at least InMinCapacity if current capacity is insufficient. Never shrinks.
	// Flushes read/write pointers (does not preserve contents).
	void EnsureCapacity(int32 InMinCapacity)
	{
		check(InMinCapacity > 1);

		if (!FMath::IsPowerOfTwo(InMinCapacity))
		{
			InMinCapacity = FMath::RoundUpToPowerOfTwo(InMinCapacity);
		}

		FScopeLock Lock(&CS);
		if (InMinCapacity > Buffer.Num())
		{
			Buffer.SetNumZeroed(InMinCapacity, EAllowShrinking::No);
			Mask = InMinCapacity - 1;
		}
		Read = 0;
		Write = 0;
	}
...
protected:
	...
	//* Return the number of samples able to be popped off the queue */
	AUDIOLINKENGINE_API int32 GetNumBufferedSamples();

	//* Flush the circular buffer and reset the known format so OnFormatKnown re-fires on the next buffer */
	AUDIOLINKENGINE_API void FlushAndResetFormat();

	//* Flush the circular buffer, grow capacity if needed (never shrinks), and reset format */
	AUDIOLINKENGINE_API void FlushAndEnsureCapacity(int32 InMinCapacity);
