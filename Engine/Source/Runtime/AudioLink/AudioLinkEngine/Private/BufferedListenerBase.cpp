...
void FBufferedListenerBase::OnBufferReceived(const FBufferFormat& InFormat, TArrayView<const float> InBuffer)
{
	...
	// Do we know the format yet? (do this under a read-only lock, unless we need to change it).
	{
		...
		else
		{
			...
			// Note, in this case it is okay to be off a couple samples if we are doing sample rate conversion	
			ensure(InFormat.NumChannels == KnownFormat->NumChannels &&
				FMath::Abs(InFormat.NumSamplesPerBlock - KnownFormat->NumSamplesPerBlock) <= InFormat.NumChannels &&
				InFormat.NumSamplesPerSec == KnownFormat->NumSamplesPerSec);
		}
	}
	...
	int32 SamplesPushed = 0;
	{
		...
		UE_LOGF(LogAudioLink, VeryVerbose,
			"FBufferedListenerBase::OnBufferReceived()(post-push), SamplesPushed=%d, InBuffer.Num()=%d, LocklessCircularBuffer.Num()=%d, LocklessCircularBuffer.Remainder()=%d, LocklessCircularBuffer.Capacity()=%d, This=0x%p",
	...
	if (SamplesPushed < InBuffer.Num())
	{
		...
		UE_CLOGF(LogPacifier++ % NumLogMessagesToSkip == 0, LogAudioLink,
			Verbose, "FBufferedListenerBase: Overflow by '%d' Samples in Buffer Listener", InBuffer.Num() - SamplesPushed);
	}
}
...
int32 FBufferedListenerBase::GetNumBufferedSamples()
{
	// Lock for safety
	FScopeLock Lock(&CircularBuffer.GetCriticialSection());
	return CircularBuffer.Num();
}

void FBufferedListenerBase::FlushAndResetFormat()
{
	CircularBuffer.Flush();
	ResetFormat();
}

void FBufferedListenerBase::FlushAndEnsureCapacity(int32 InMinCapacity)
{
	CircularBuffer.EnsureCapacity(InMinCapacity);
	ResetFormat();
}
