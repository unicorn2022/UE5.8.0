...
namespace Audio
{
	...
	FAudioPlatformSettings FAudioMixerPlatformAudioLink::GetPlatformSettings() const
	{
		...
		if (const IAudioLinkSynchronizer* Synchronizer = GetOrCreateSynchronizer())
		{
			...
			{
				UE_LOGF(LogAudioLink, Warning, "%s - OpenStreamParams has not been set, all settings will be from the Default Engine settings, not the host.", __func__);
			}
		}
		else
		{
			UE_LOGF(LogAudioLink, Warning, "%s - Synchronizer does not exist yet.", __func__);
		}
		return Settings;;
#else
		return {};
	...
	{
		UE_LOGF(LogAudioLink, VeryVerbose, "FAudioMixerPlatformAudioLink::OnLinkRenderEnd, TickID=%llu, FramesMade=%d, LastBufferTickID=%d, FrameRemainder=%d, AudioMixer.NumFrames=%d, AudioMixer.NumBuffers=%dm This=0x%p", 
