...
public:
	...
	IPerfCounters* CreatePerformanceCounters(const FString& UniqueInstanceId) override
	{
		...
		{
			UE_LOGF(LogPerfCounters, Display, "CreatePerformanceCounters: instance already exists, new instance not created.");
			return PerfCountersSingleton.Get();
		}
		...
		{
			UE_LOGF(LogPerfCounters, Warning, "CreatePerformanceCounters: could not create perfcounters");
			return nullptr;
		}
...
int32 IPerfCountersModule::GetHTTPStatsPort()
{
	...
	if (StatsPort > 0 && FParse::Param(FCommandLine::Get(), TEXT("forkAutoStatsPort")))
	{
		...
		UE_LOGF(LogPerfCounters, Display, "ForkAutoStatsPort assigned port %d to child process %d", StatsPort, ChildProcessIdx);
	}
