...
bool FPerfCounters::Initialize()
{
	...
	if (StatsPort < 0)
	{
		UE_LOGF(LogPerfCounters, Log, "FPerfCounters JSON socket disabled.");
		return true;
	}
	...
	if (!HttpRouter)
	{
		UE_LOGF(LogPerfCounters, Error, 
			"FPerfCounters unable to bind to specified statsPort [%d]", StatsPort);
		return false;
	}
	...
	{
		UE_LOGF(LogPerfCounters, Error,
			"FPerfCounters unable bind route: /stats");
		return false;
	}
	...
	{
		UE_LOGF(LogPerfCounters, Error,
			"FPerfCounters unable bind route: /exec");
		return false;
	}
...
void FPerfCounters::TickZeroLoad(float DeltaTime)
{
	...
	if (LIKELY(ZeroLoadThread->GetHitchMessages(LogMessages)))
	{
		...
		{
			UE_LOGF(LogPerfCounters, Warning, "%ls", *LogMessage);
		}
	}
}
...
double FPerfCounters::GetNumber(const FString& Name, double DefaultValue)
{
	...
	{
		UE_LOGF(LogPerfCounters, Warning, "Attempting to get PerfCounter '%ls' as number, but it is not (Json format=%d). Default value %f will be returned", 
...
bool FPerfCounters::StartMachineLoadTracking(double TickRate, const TArray<double>& FrameTimeHistogramBucketsMs)
{
	...
	{
		UE_LOGF(LogPerfCounters, Warning, "Machine load tracking has already been started.");
		return false;
	}
	...
	{
		UE_LOGF(LogPerfCounters, Warning, "Failed to create zero load thread.");
...
bool FPerfCounters::StopMachineLoadTracking()
{
	if (UNLIKELY(ZeroLoadRunnable == nullptr || ZeroLoadThread == nullptr))
	{
		UE_LOGF(LogPerfCounters, Warning, "Machine load tracking has already been stopped.");
		return false;
	}
	...
	{
		UE_LOGF(LogPerfCounters, Warning, "Could not kill zero-load thread, crash imminent.");
	}
