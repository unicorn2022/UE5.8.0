	...
	: Super(ObjectInitializer)
{
	...
	NanitePixelProgrammableDistance = 0.f;
...
void FFoliageStaticMesh::UpdateComponentSettings(const UFoliageType_InstancedStaticMesh* InSettings)
{
	if (Component)
	{
		...
		if (Component->bVisibleInReflections != FoliageType->bVisibleInReflections)
		{
			Component->bVisibleInReflections = FoliageType->bVisibleInReflections;
			bNeedsMarkRenderStateDirty = true;
		}
		...
		if(Component->NanitePixelProgrammableDistance != FoliageType->NanitePixelProgrammableDistance)
		{
			Component->NanitePixelProgrammableDistance = FoliageType->NanitePixelProgrammableDistance;
			bNeedsMarkRenderStateDirty = true;
		}
...
void FFoliageInfo::CheckValid()
{
	...
	if (MismatchCount != 0)
	{
		UE_LOGF(LogInstancedFoliage, Log, "transform mismatch: %d", MismatchCount);
	}
#endif
...
void AInstancedFoliageActor::PostLoad()
{
	...
	if (OwningLevel && !OwningLevel->bIsPartitioned && !OwningLevel->IsWorldPartitionRuntimeCell() && !APartitionActor::IsPartitioned(this))
	{
...
bool AInstancedFoliageActor::CheckCollisionWithWorld(const UWorld* InWorld, const UFoliageType* Settings, const FFoliageInstance& Inst, const FVector& HitNormal, const FVector& HitLocation, UPrimitiveComponent* HitComponent)
{
	...
	static FName NAME_FoliageCollisionWithWorld = FName("FoliageCollisionWithWorld");
