		...
		else if (Ar.IsLoading()) // Regenerate inverse map whenever we load cache
		{
			...
			for (const auto& Pair : InstanceBaseCache.InstanceBaseMap)
			{
				...
				if (InstanceBaseCache.InstanceBaseInvMap.Contains(BaseInfo.BasePtr))
				{
					...
					UE_LOGF(LogInstancedFoliage, Warning, "Instance base cache - integrity verification(3): Counter: %d Size: %d, InvSize: %d (Key: %ls)",
...
FFoliageInstanceBaseId FFoliageInstanceBaseCache::AddInstanceBaseId(UActorComponent* InComponent)
{
	...
	bool bIgnoreComponentClass = false; 
#if WITH_EDITOR
	if (IFoliageEditModuleBase* FoliageModule = IFoliageEditModuleBase::Get(); ensure(FoliageModule))
	{
		bIgnoreComponentClass = FoliageModule->ShouldIgnoreComponentForBaseID(InComponent);
	}
#endif // WITH_EDITOR

	if (InComponent && !InComponent->IsCreatedByConstructionScript() && !bIgnoreComponentClass)
	{
		...
		if (BaseId == FFoliageInstanceBaseCache::InvalidBaseId)
		{
			...
			if (InstanceBaseInvMap.Contains(BaseInfo.BasePtr))
			{
				...
				UE_LOGF(LogInstancedFoliage, Error, "Instance base cache - integrity verification(2): Counter: %d Size: %d, InvSize: %d, BaseUID: %ls, BaseName: %ls", 
