...
void FRuntimeAssetCacheAsyncWorker::DoWork()
{
	...
	{
		UE_LOGF(RuntimeAssetCache, Warning, "Caching asset %ls to unknown bucket %ls. Asset won't be cached.", *CacheBuilder->GetAssetUniqueName(), Bucket);
		Data = nullptr;
		return;
	}
	...
	checkf(DataSize < CurrentBucket->GetSize(), TEXT("Cached asset is bigger than cache size. Increase cache size (%d) or reduce asset size(%lld). Asset cache key: %s"), CurrentBucket->GetSize(), DataSize, *CacheKey);
