...
public:
	inline FPrimitiveUniformShaderParametersBuilder& Defaults()
	{
		...
		bEnableObjectWorldPositionFix				= false;
		...
		Parameters.PackedNaniteFlags					= 0;
...
#define PRIMITIVE_UNIFORM_BUILDER_FLAG_METHOD(INPUT_TYPE, VARIABLE_NAME) \
	...
	PRIMITIVE_UNIFORM_BUILDER_FLAG_METHOD(bool,			EnableObjectWorldPositionFix);
	...
	PRIMITIVE_UNIFORM_BUILDER_METHOD(FVector2f,			InstancingFadeOutParams);
...
#undef PRIMITIVE_UNIFORM_BUILDER_METHOD
	...
	{
		Parameters.PackedNaniteFlags = FilterFlags;
		return *this;
	}
	...
	inline const FPrimitiveUniformShaderParameters& Build()
	{
...

		{
			...
			else
			{
				...
				Parameters.WorldToPreviousWorld = FMatrix44f::Identity;
			}
		}

		if (GPrimitiveHasTileOffsetData)
		{
		...
		// TODO: Rename the flag later - not done now to avoid affecting more shaders than needed
		Parameters.Flags |= bEnableObjectWorldPositionFix ? PRIMITIVE_SCENE_DATA_FLAG_UNUSED_2 : 0u;
...
private:
	...
	uint32 bEnableObjectWorldPositionFix : 1;
};
