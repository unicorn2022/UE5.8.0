...
/**
 * This class provides an easy way to register gamethread-only tickable objects. It is an
 * abstract base class requiring you to implement the Tick() and GetStatId() methods.
 * It is intended to be used for simple objects that cannot easily use other methods for ticking.
 * It can optionally also be ticked in the Editor, allowing for an object that both ticks
 * during edit time and at runtime.
 * 
 * NOTE: Do not use this class with UObjects unless you add explicit code to start and stop ticking.
 * To do this, construct with ETickableTickType::Never and call SetTickableTickType to enable ticking later.
 * Ticking cannot be safely started until after object construction finishes, and should be set back to Never in BeginDestroy or earlier.
 * UObjects are often created and destroyed on background threads so it is not safe to control ticking from the constructor/destructor.
 */
...
public:
	 ...
	 * NOTE: This is not threadsafe with the default tick type! 
	 * For UObjects and other classes that can be constructed on worker threads, you must construct with ETickableTickType::Never and enable ticking later.
	 * 
