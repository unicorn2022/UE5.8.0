...
struct FSkinWeightVertexBufferInfo
{
	...
	GPUSkinBoneInfluenceType BoneInfluenceType = DefaultBoneInfluence;
	uint8 bUse16BitBoneIndex  : 1 = 0;
	uint8 bUse16BitBoneWeight : 1 = 0;
};
...
enum class EBoneTransformAllocationMode : uint8
{
...
// Returns the globally configured bone transform allocation mode.
EBoneTransformAllocationMode GetBoneTransformAllocationMode();

enum class EBoneTransformStorageMode : uint8
{
...
// If true, the skinning scene extension supports GPU skin skeletal mesh objects.
ENGINE_API extern bool IsGPUSkinSceneExtensionEnabled();
...
public:
	...
	{
		TConstArrayView<FMatrix44f> CurrentTransforms;
		TConstArrayView<FMatrix44f> PreviousTransforms;
	...
	virtual FBoneTransforms GetBoneTransforms() const
	{
		return FBoneTransforms{};
	}

	struct FBoneBufferSRV
	{
	...
	virtual FBoneBufferSRV GetSectionBoneBuffer(int32 InLODIndex, int32 InSectionIndex, bool bPreviousFrame) const
	{
		return FBoneBufferSRV{};
	}

	/** Called when creating a new scene extension proxy object. */
	virtual FSkinningSceneExtensionProxy* CreateSceneExtensionProxy(const USkinnedAsset* InSkinnedAsset, bool bAllowScaling) const { return nullptr; }

	/** Called when the scene extension updates the skinning header for the current mesh. */
	virtual void UpdateSceneExtensionHeader(FRHICommandListBase& RHICmdList, const FSkinningHeader& SkinningHeader, FRHIShaderResourceView* TransformDataBufferSRV) const {}
	...
	*/
	...
	/** Returns the mode for storing transforms on the GPU. */
	...
	/** The minimum allowed LOD level for this mesh. */
	const int32 MinLODLevel;
	...
	UE_DEPRECATED(5.8, "Use GetSkinWeightVertexBuffer_RenderThread instead")
	virtual const FSkinWeightVertexBuffer* GetSkinWeightVertexBuffer(int32 LODIndex) const { return nullptr; }

	/** Get the weight buffer for specific LOD index. Only needed for GPU skin cache */
	virtual FSkinWeightVertexBufferInfo GetSkinWeightVertexBuffer_RenderThread(int32 LODIndex) const
	{
		return FSkinWeightVertexBufferInfo();
	}
	...
	/** Private data for the skinning scene extension. */
	mutable class FSkinningSceneExtensionPrivateData
	{
	private:
		friend class FSkinningSceneExtension;
		int32 CachedPrimitiveIndex = INDEX_NONE;

	} SkinningSceneExtensionPrivateData;

	TConstArrayView<FBoneReference> GetCachedBonesToRemove(const USkinnedAsset* SkinnedAsset, int32 InLODIndex)
	{
		if (SkinnedAsset != BonesToRemoveCache.SkinnedAsset || InLODIndex != BonesToRemoveCache.LODIndex)
		{
			BonesToRemoveCache.SkinnedAsset = SkinnedAsset;
			BonesToRemoveCache.LODIndex = InLODIndex;
			BonesToRemoveCache.BonesToRemove = SkinnedAsset->GetLODInfo(InLODIndex)->BonesToRemove;
		}
...
protected:
	...
	/** Cache for bones removed at a given LOD level, used by the forward kinematic fixup. GameThread only. */
	struct
	{
