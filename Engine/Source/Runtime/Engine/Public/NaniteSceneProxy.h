...
struct FMaterialAudit
{
	...
	inline bool HasTranslucency(int32 MaterialIndex) const
	{
		if (Entries.IsValidIndex(MaterialIndex))
		{
			return Entries[MaterialIndex].bHasTranslucency;
		}
...
struct FResourcePrimitiveInfo
{
	...
	ENGINE_API explicit FResourcePrimitiveInfo(const FResources& Resources);
};
...
public:
	...
	struct FMaterialSection
	{
		...
		uint8 bHasCurves					: 1	= false;
		uint8 bHidden						: 1 = false;
		uint8 bTranslucent					: 1 = false;
...
public:
	...
	ENGINE_API virtual ERayTracingPrimitiveFlags GetRayTracingPrimitiveFlags() override;
	ENGINE_API virtual void GetStaticRayTracingInstances(FStaticRayTracingInstances& OutRayTracingInstances) override;
...
protected:
	...
	/** Whether CardRepresentationData is generated at runtime */
	uint32 bHasRuntimeCardRepresentationData : 1;
	...
	TArray<FRayTracingGeometry> DynamicRayTracingGeometries;
...
{
	ENGINE_API static TMulticastDelegate<void(const FSkinnedSceneProxy*)> OnCreateRenderThreadResources;
	ENGINE_API static TMulticastDelegate<void(const FSkinnedSceneProxy*)> OnDestroyRenderThreadResources;
};
