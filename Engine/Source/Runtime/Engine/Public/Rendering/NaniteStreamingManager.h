...
public:
	...
	void GetAndClearModifiedPages(TSet<FPageInfo>& OutInstalledPages, TSet<FPageInfo>& OutUninstalledPages)
	{
		OutInstalledPages = MoveTemp(InstalledPages);
		OutUninstalledPages = MoveTemp(UninstalledPages);
	}

	ENGINE_API void	PrefetchResource(const FResources* Resource, uint32 NumFramesUntilRender);
	ENGINE_API void RequestNanitePages(TArray<uint32>&& InRequestStream);

#if WITH_EDITOR
	ENGINE_API uint64		GetRequestRecordBuffer(TMap<uint64, uint32>& OutRequestData);
	ENGINE_API void			SetRequestRecordBuffer(uint64 Handle);
	ENGINE_API static void	PackRequestsToStream(TMap<uint64, uint32> const& InPageIdAndPriorities, TArray<uint32>& OutRequestStream);
#endif
...
private:
	...
	struct FPendingPage
	{
		...
		std::atomic<bool>		DCCRequestCompleted{false};
		enum class EState
		{
			None,
			DDC_Pending,
		...
		#if WITH_EDITOR
			SharedBuffer.Reset();
			DCCRequestCompleted.store(false, std::memory_order_relaxed);
			State = EState::None;
		#endif
			RequestBuffer = FIoBuffer();
			Request.Reset();

			GPUPageIndex = INDEX_NONE;
			InstallKey = FPageKey();
			RingBufferAllocationSize = 0;
			BytesLeftToStream = 0;
			RetryCount = 0;
		}
	};
	...
	TSet<FPageInfo>						InstalledPages;
	TSet<FPageInfo>						UninstalledPages;
