...
struct FSkelMeshRenderSection
{
	...
	/** If true, this section contains a unified bone map, meaning all other sections contain the same map with the union of all bones. */
	...
	UE_DEPRECATED(5.8, "Use ClothMappingDataLODs instead.")
	...
	inline bool HasUnifiedBoneMap() const
	{
		return bUnifiedBoneMap;
	}
...
class FSkeletalMeshLODRenderData : public UE::Private::FQueryableRefCountedObject
{
public:
	UE_DEPRECATED(5.8, "Temporary fix. Do not use in production code")
	mutable FTransactionallySafeCriticalSection RenderDataMutex;
	...
	/** When populated, all sections share the same unified bone map across this LOD. */
	TConstArrayView<FBoneIndexType> UnifiedBoneMap;
	...
	*/
	...
	inline bool HasUnifiedBoneMap() const
	{
		return !UnifiedBoneMap.IsEmpty();
	}

	inline TConstArrayView<FBoneIndexType> GetUnifiedBoneMap() const
	{
	...
	FSkinWeightVertexBuffer* GetSkinWeightProfileVertexBuffer(const FSkinWeightProfileStack& InProfileStack);
	...
	*/
	...
	/** 
	* @return Returns true if data is not being streamed/serialized in and is ready for use. If data is being serialized i.e. in midst 
	* of Serialize(...) where data is being loaded in, especially from a different thread, this will return false. This is an atomic
	* load and may not be suitable for perf-critical paths. 
	*/
	bool IsDataReady() const { return bIsDataReady.Load(); }
...
private:
	...
	inline void SetIsDataReady(bool bReady) { bIsDataReady.Store(bReady); }

	/** Atomic bool to indicate if required data has been streamed/serialized in. */
	TAtomic<bool> bIsDataReady;
