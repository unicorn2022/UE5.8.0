...
struct FPackedHierarchyNode
{
	...
	struct
	{
		...
		uint32		NumBones_BoneIndicesOffset;	// BoneIndicesOffset (in dwords): NANITE_HIERARCHY_BONE_INDICES_OFFSET_BITS, NumBones: NANITE_HIERARCHY_NUM_BONES_BITS
...
inline uint32 ToFloat10(float In)
{
	const uint16 Raw = FFloat16(In).Encoded;
	return (Raw >> 5) & 0x000003FF;
}
...
struct FPackedCluster
{
	...
	uint32		NumCurves_NumPoints_CurveRadiusScale_CRBits_HasCR;		// NumCurves: 8, NumPointPerCurve:8, CurveRadius Scale:10, CurveRadius Bits: 4, HasCurveRadius:1 
	...
	void		SetNumCurves(uint32 NumCurves)			{ SetBits(NumCurves_NumPoints_CurveRadiusScale_CRBits_HasCR, NumCurves, 8, 0); }
	void		SetNumPointsPerCurve(uint32 NumPoints)	{ SetBits(NumCurves_NumPoints_CurveRadiusScale_CRBits_HasCR, NumPoints, 8, 8); }
	void		SetCurveRadiusScale(float InScale)		{ SetBits(NumCurves_NumPoints_CurveRadiusScale_CRBits_HasCR, ToFloat10(InScale / 255.f), 10, 16); } // Embedded 8bits to normalized scaling [0..255] -> [0..1]
	void		SetCurveRadiusBits(uint32 NumBits)		{ SetBits(NumCurves_NumPoints_CurveRadiusScale_CRBits_HasCR, NumBits, 4, 26); }
	void		SetHasCurveRadius(bool bHasCurveRadius)	{ SetBits(NumCurves_NumPoints_CurveRadiusScale_CRBits_HasCR, bHasCurveRadius, 1, 30); }
	void		SetParentLODError(float In)				{ SetBits(ParentLODError_ParentSphereBoundRadius, FFloat16(In).Encoded, 16, 0); }
	void		SetParentSphereBoundRadius(float In)	{ SetBits(ParentLODError_ParentSphereBoundRadius, FFloat16(In).Encoded, 16, 16); }
	void		SetCurveFadingIndex(uint32 In)			{ SetBits(BrickDataOffset_Num, In, 10, 22); } // Alias BrickDataNum, as curve and voxel are mutually exclusive
...
struct FPageInfo
{
	...
	{
		return GPUPageIndex == Other.GPUPageIndex
			&& NumClusters == Other.NumClusters
			&& RuntimeResourceID == Other.RuntimeResourceID;
	}

	/** Hash function. */
	friend uint32 GetTypeHash(const FPageInfo& In)
	{
		return HashCombine(HashCombine(In.GPUPageIndex, In.NumClusters), In.RuntimeResourceID);
	}
};
...
struct FResources
{
	...
	uint32	NumAssemblyTransforms	= 0;
