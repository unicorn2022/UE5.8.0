...
struct TLocation
{
	...
	friend inline FArchive& operator<<(FArchive& Ar, TLocation& Item)
	{
		Ar << Item.Coord;
		Ar << Item.Level;
		return Ar;
...
inline FVector3d CalcWorldCellCenter(const FLocation64& Loc)
{
	double LevelCellSize = GetCellSize(Loc.Level);
	FVector Center = FVector3d(Loc.Coord) * LevelCellSize + LevelCellSize * 0.5;
	return Center;
}
