...
struct FEngineShowFlags
{
	...
	static bool FindShowFlagDisplayName(const FString& InName, FText& OutText)
	{
		...
		if (LocNames.Num() == 0)
		{
			#define SHOWFLAG_ALWAYS_ACCESSIBLE(a,b,c) LocNames.Add( TEXT(UE_STRINGIZE(a)), c);
			#define SHOWFLAG_FIXED_IN_SHIPPING(v,a,b,c) LocNames.Add( TEXT(UE_STRINGIZE(a)), c);
			#include "ShowFlagsValues.inl"
	 ...
	 * @return empty string if not found, otherwise name of the flag e.g. FString("PostProcessing")
	 */
	...
	{
		#define SHOWFLAG_ALWAYS_ACCESSIBLE(a,...) if( !Sink.OnEngineShowFlag((uint32)SF_##a, TEXT(UE_STRINGIZE(a))) ) { return; }
		#include "ShowFlagsValues.inl"
...
private:
	...
	void Init(EShowFlagInitMode InitMode)
	{
		...
		SetVisualizeColorGrading(false);
		...
		SetStreamingTextureDeficit(false);
		SetStreamingTextureResidency(false);
		SetStreamingMeshLODDeficit(false);
		SetStreamingMeshLODResidency(false);
