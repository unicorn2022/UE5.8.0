...
public:
	...
	inline static bool BoxInFrustum(const FBox& Box, const FConvexVolume& ViewFrustum)
	{
		FVector Center, Extents;
		Box.GetCenterAndExtents(Center, Extents);
		return BoxInFrustum(Center, Extents, ViewFrustum);
	}
	...
	struct FDebugLine
	{
		FDebugLine(const FVector &InStart, const FVector &InEnd, const FColor &InColor, const float InThickness = 0.f, ESceneDepthPriorityGroup InPriorityGroup = ESceneDepthPriorityGroup::SDPG_World)
			: Start(InStart)
			, End(InEnd)
			, Color(InColor)
		...
		FColor Color;
		float Thickness;
		ESceneDepthPriorityGroup PriorityGroup;
