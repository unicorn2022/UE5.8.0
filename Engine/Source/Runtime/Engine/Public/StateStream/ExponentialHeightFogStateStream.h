...
// private: // Should be private but complicates some of the code that needs support both this path and UComponent path
	...
	UPROPERTY()
	float SkyLightCaptureAffectsHeightFogRoughness = 0.0f;
	UPROPERTY()
	float SkyLightCaptureAffectsHeightFogStrength = 0.0f;
	UPROPERTY()
	...
	UPROPERTY()
	bool bEnableFSSS = false;
	UPROPERTY()
	float FSSSSceneColorScatteringAmountScale = 1.0f;
	UPROPERTY()
	float FSSSSceneColorScatteringAmountPower = 1.0f;
	UPROPERTY()
	float FSSSSpreadScale = 0.1f;
	UPROPERTY()
	float FSSSBlurControl = 0.5f;
	UPROPERTY()
...
public:
	...
	virtual FExponentialHeightFogHandle Game_CreateInstance(const FExponentialHeightFogStaticState& Ss, const FExponentialHeightFogDynamicState& Ds, uint32 LaneId = DefaultLaneId) = 0;
};
