...
struct FAnimNotifyEventReference
{
	...
	// NotifyInstanceID is generated when we dispatch the NotifyBegin/Notify call.
	// In the case of a non-unique/existing Notify, we re-use/propagate the ID from the previous notify. 
	int32 GetNotifyInstanceID() const { return NotifyInstanceID; }
	void SetNotifyInstanceID(int32 InNewInstanceID) { NotifyInstanceID = InNewInstanceID; }
...
private:
	...
	// Unique per-notify instance ID. This gets populated before firing the notifies and should only be used after when notifies are actually fired.
	UPROPERTY(transient, Category=Notifies, BlueprintReadOnly, meta=(AllowPrivateAccess="true"))
