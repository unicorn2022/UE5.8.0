...
public: 
	...
	/** Track an asset for which TryRegisterStreamOutProtectedAsset succeeded.
	 *  Safe to call from the async task */
	void ENGINE_API RegisterStreamOutProtection_AsyncSafe(USkinnedAsset* SkinnedAsset);
protected:
	...
	/** Walk ProtectedMeshes and unregister any whose readback is past the danger window. */
	void TickReleaseStreamOutProtection_GameThread();
protected:
	...
	/** Skinned assets for which we hold a stream-out protection registration on the streaming manager. */
	TSet<TWeakObjectPtr<USkinnedAsset>> ProtectedSkinnedAssets;
	FCriticalSection ProtectedSkinnedAssetsCS;
public:
