...
public:
	...
	ENGINE_API bool FetchData(const ITargetPlatform* InPlatform);
	...
	static int32 GUseHighFidelityAdditiveCurveCompression;
...
struct ICompressedAnimData
{
	...
	virtual ~ICompressedAnimData() = default;
...
public:
...
#if WITH_EDITOR
	// Whether compressed data has been populated via DDC fetch or local compression.
	bool bHasValidCompressedData = false;
#endif
	...
	UE_DEPRECATED(5.8, "ClearCompressedBoneData is not intended for external use and will become private. Use Reset() to clear all compressed data.")
	ENGINE_API void ClearCompressedBoneData();
	UE_DEPRECATED(5.8, "ClearCompressedCurveData is not intended for external use and will become private. Use Reset() to clear all compressed data.")
