...
struct FNamedValueArrayUtils
{
	...
	static void Union(TNamedValueArray<AllocatorTypeResult, ElementTypeResult>& InOutValueArray0, const TNamedValueArray<AllocatorTypeParam, ElementTypeParam>& InValueArray1, PredicateType InPredicate)
	{
		...
		// Nothing to merge, just run the predicate on every elements.
		if (NumElements1 == 0)
		{
			InOutValueArray0.SortElementsIfRequired();
			ElementTypeParam DefaultElement;
			for (auto& Element : InOutValueArray0.Elements)
			{
				DefaultElement.Name = Element.Name;
				InPredicate(Element, DefaultElement, UE::Anim::ENamedValueUnionFlags::ValidArg0);
			}
			return;
		}
		...
		// Tape merge is done, process anything that might be remaining
		if (ElementPtr0 < ElementEndPtr0)
		{
			// Reached end of ValueArray1 with remaining elements in ValueArray0, call predicate on each
	...
	/**
	 * Removes elements in InOutValueArray0 that match InValueArray1 if predicate returns true
	 **/
