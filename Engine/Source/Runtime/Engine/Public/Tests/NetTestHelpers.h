...
/**
 * Temporarily overrides network emulation settings for a specific NetDriver while in scope.
 * Only works when DO_ENABLE_NET_TEST is true.
 */
class [[nodiscard]] FScopedNetEmulationOverride
{
public:
	ENGINE_API FScopedNetEmulationOverride(UNetDriver* InNetDriver, const FPacketSimulationSettings& InSettings);
	ENGINE_API ~FScopedNetEmulationOverride();

private:
	TWeakObjectPtr<UNetDriver> WeakNetDriver;
	FPacketSimulationSettings SavedSettings;
};
...
#if WITH_EDITOR
	FScopedCVarOverrideInt ClientsShareAudioDevice;
#endif
...
/** 
 * Scope used when a test is expected to trigger an ensure. 
 * Prevents the ensure from triggering a DEBUG_BREAK when running with a debugger attached.
 */
...
public:
	...
	explicit FTestWorldInstance(bool bDelayedInit)
	{
		// GameInstance will be created manually later
	}
	...
	ENGINE_API int32 GetPort() const;
	...
	/** Returns true if the world was correctly loaded and ready to play. */
	bool IsLoaded() const
	{
		return bIsLoaded;
	}
...
struct FTestWorlds
{
	...
	ENGINE_API void SetDescription(const FString& Description);
	...
	/** Returns the ClientId associated with the passed world or INDEX_NONE if it's not a known client world. */
	ENGINE_API int32 FindClientIdFromWorld(const UWorld* ClientWorld) const;

	/** Returns the world context data for an existing client. */
	ENGINE_API FTestWorldInstance::FContext GetClientContext(int32 ClientId) const;
	...
	/**
	 * Ticks all server & client worlds until Predicate returns true, or MaxTicks is reached using the default Delta
	 * Returns true if Predicate did, false if it didn't within MaxTicks.
	 */
	template<class PredicateT>
	bool TickUntil(const PredicateT& Predicate, int32 MaxTicks = 60);
	...
	/** Move a client's PlayerController to a location on the server */
	ENGINE_API void MovePlayerToLocation(uint32 ClientIndex, const FVector& Location);
	 ...
	 * @param ClientIndex The client you want a remote instance of. If not set the function will use the first client available.
	 * @return Return the remote (client) instance of the same object if it exists.
	 */
	ENGINE_API UObject* FindReplicatedObjectOnClient(UObject* ServerObject, uint32 ClientIndex = UINT32_MAX) const;

	template<typename T>
	T* FindReplicatedObjectOnClient(T* ServerObject, uint32 ClientIndex = UINT32_MAX) const
	{
		return Cast<T>(FindReplicatedObjectOnClient(Cast<UObject>(ServerObject), ClientIndex));
	}
	 ...
	 * @param ClientIndex The client you want to test on. If not set the function will use the first client available.
	 */
	ENGINE_API bool DoesReplicatedObjectExistOnClient(UObject* ServerObject, uint32 ClientIndex = UINT32_MAX) const;
...
template<class PredicateT>
inline bool FTestWorlds::TickUntil(const PredicateT& Predicate, int32 MaxTicks)
{
	PreTickAllDelegate.Broadcast();
