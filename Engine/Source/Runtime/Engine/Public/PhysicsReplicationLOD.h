...
class FPhysicsReplicationLODAsync : public IPhysicsReplicationLODAsync
	...
	Chaos::ESimCallbackOptions::Presimulate | Chaos::ESimCallbackOptions::Rewind | Chaos::ESimCallbackOptions::PhysicsObjectUnregister | Chaos::ESimCallbackOptions::PostSolve>
{
...
public:
	...
	virtual ~FPhysicsReplicationLODAsync() = default;
...
private:
	...
	// Solver frame on which ConsumeAsyncInput was last performed, ensures we only process it once per frame.
	int32 LastInputProcessStep = INDEX_NONE;

	// Solver frame on which CacheParticlesInFocalIslands was last performed, ensures we only process it once per frame.
	int32 LastFocalCacheStep = INDEX_NONE;
