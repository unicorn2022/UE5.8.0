...
public:
	...
	UE_DEPRECATED(5.8, "HandleDataMovementInAtlas now requires a command list.")
	void HandleDataMovementInAtlas(int32 OldOffset, int32 NewOffset)
	{
		HandleDataMovementInAtlas(FRHICommandListImmediate::Get(), OldOffset, NewOffset);
	}

	ENGINE_API void HandleDataMovementInAtlas(FRHIComputeCommandList& RHICmdList, int32 OldOffset, int32 NewOffset);
	
	ENGINE_API void AddToSceneData(FRHICommandListImmediate& RHICmdList, FPrecomputedVolumetricLightmapData* SceneData);
	
	UE_DEPRECATED(5.8, "AddToSceneData now requires a command list.")
	void AddToSceneData(FPrecomputedVolumetricLightmapData* SceneData)
	{
		AddToSceneData(FRHICommandListImmediate::Get(), SceneData);
	}

	ENGINE_API void RemoveFromSceneData(FRHICommandListImmediate& RHICmdList, FPrecomputedVolumetricLightmapData* SceneData, int32 PersistentLevelBrickDataBaseOffset);

	UE_DEPRECATED(5.8, "RemoveFromSceneData now requires a command list.")
	void RemoveFromSceneData(FPrecomputedVolumetricLightmapData* SceneData, int32 PersistentLevelBrickDataBaseOffset)
	{
		RemoveFromSceneData(FRHICommandListImmediate::Get(), SceneData, PersistentLevelBrickDataBaseOffset);
	}
...
public:
	...
	ENGINE_API void Insert(FRHICommandListImmediate& RHICmdList, int32 Index, FPrecomputedVolumetricLightmapData* Data);
	ENGINE_API void Remove(FRHICommandListImmediate& RHICmdList, FPrecomputedVolumetricLightmapData* Data);

	UE_DEPRECATED(5.8, "Insert now requires a command list.")
	void Insert(int32 Index, FPrecomputedVolumetricLightmapData* Data)
	{
		Insert(FRHICommandListImmediate::Get(), Index, Data);
	}

	UE_DEPRECATED(5.8, "Remove now requires a command list.")
	void Remove(FPrecomputedVolumetricLightmapData* Data)
	{
		Remove(FRHICommandListImmediate::Get(), Data);
	}
