...
struct FShaderCompileMemoryUsage
{
	...
	*/
	 ...
	 */
	uint64 MaxWorkerMemory;

	/**
	 * The number of active shader compiler workers
	 */
	int32 ActiveWorkerCount;
};
...
class FShaderCompileThreadRunnableBase : public FRunnable
{
	...
	/** Priority index range is RW lock protected so that we can change it for all threads when the distributed compiling thread is shutting down and transfers its jobs to local workers. */
	mutable FRWLock PriorityRangeLock;
	TInterval<int32> PriorityIndexRange;
...
private:
	...
	mutable FCriticalSection PerWorkerMemUsageLock;

	/** Tracks the last time that this thread checked if the workers were still active. */
	double LastCheckForWorkersTime = 0.0;

	/** Tracks the last time the per worker memory usage was checked. This is expensive to query, so if recent enough not queried again. */
	double LastCheckForWorkersMemoryUsageTime = 0.0;
	...
	/** Is dynamic SCW budget enabled? When enabled, the SCW budget is adjusted based on current memory needs */
	bool bDynamicSCWBudgetEnabled;
	/** Memory to reserve in MiB when running the dynamic budget */
	int32 DynamicSCWBudgetReserveInMiB;
	/** Granularity of SCW budget. This is to reduce the frequency of changes, limiting it to significant changes */
	int32 DynamicSCWBudgetGranularityInMiB;

	struct FMemoryMonitoringState
	{
		double LastTimeOfMemoryLimitPoll = 0.0;
		double LastTimeOfSuspendingOrResumingWorkers = 0.0;
		bool bHasFailedToSuspendWorkers = false;
	}
	MemoryMonitoringState;

	/** Most recent capture of per-worker memory usage. Note: Not updated every tick. */
	TArray<FWorkerMemoryUsage> PerWorkerMemoryUsage;
...
public:
	...
	/** Returns most recent capture of per-worker memory usage */
	TArray<FWorkerMemoryUsage> GetRecentPerWorkerMemoryUsage() const
	{
		FScopeLock PerWorkerMemScopeLock(&PerWorkerMemUsageLock);
		return PerWorkerMemoryUsage;
	};
...
private:
	...
	/** Caches memory usage of each individual worker into PerWorkerMemoryUsage if stale */
	void CachePerWorkerMemoryUsage();

	/** Calculates median virtual memory usage from worker memory list */
	uint64 GetPerWorkerVirtualMemoryMedian(const TArray<FWorkerMemoryUsage>& InWorkerMemoryUsage) const;

	/** Writes per-worker summary to insights and log */
	void TracePerWorkerSummary(const TArray<FWorkerMemoryUsage>& InPerWorkerMemoryUsage, const bool bInHasMemoryViolation) const;
	...
	/** Calculates memory totals from per-worker memory usage array */
	FShaderCompileMemoryUsage CalcPerWorkerMemoryUsageTotal(const TArray<FWorkerMemoryUsage>& InPerWorkerMemoryUsage) const;
	...
	/** Return an estimate for how much memory is required for a single worker. Used for both suspension and resumption */
	int64 GetEstimatedMemoryRequiredPerWorker(const FShaderCompileMemoryUsage& LimitInfo);

	/** Queries the status if the worker processes have violated the memory limitation. */
	bool QueryMemoryLimitViolationStatus(const FShaderCompileMemoryUsage& InSCMemoryUsage, FShaderCompilerJobMemoryLimitInfo& OutInfo);

	/** Dynamically adapts the SCW memory budget to fit within current memory constraints. */
	void UpdateDynamicSCWMemoryBudget(const FShaderCompileMemoryUsage& InSCMemoryUsage);
};
...
class FShaderCompileDistributedThreadRunnable_Interface : public FShaderCompileThreadRunnableBase
{
	...
	TSparseArray<TUniquePtr<class FDistributedShaderCompilerTask>> DispatchedTasks;
...
struct FShaderCompilerCounters
{
	...
	/** Largest violation of shader compiler memory. */
	UPROPERTY()
	int64 LargestViolationMemorySizeInBytes = 0;

	/** Largest violation of shader compiler memory - worker count */
	UPROPERTY()
	int32 LargestViolationWorkerCount = 0;
	...
	FShaderCompilerCounters& operator+=(const FShaderCompilerCounters& Other)
	{
		...
		if (Other.LargestViolationMemorySizeInBytes > LargestViolationMemorySizeInBytes)
		{
			LargestViolationMemorySizeInBytes = Other.LargestViolationMemorySizeInBytes;
			LargestViolationWorkerCount = Other.LargestViolationWorkerCount;
		}
...
public:
	...
	/** Informs statistics about shader memory violation */
	void RegisterMemoryViolation(const int64 InMemorySizeInBytes, const int32 InWorkerCount);
...
extern ENGINE_API FShaderCompilerMemoryLimits GetShaderCompilerCurrentMemoryLimits();
extern ENGINE_API void SetShaderCompilerTargetMemoryLimitInMiB(int32 NewLimit);
extern ENGINE_API void SetShaderCompilerAssetMemoryLoanInMiB(int32 NewLoan);
...
private:
	...
	/** When the shader compiling manager is shutting down, don't respond to notifications from the compiling threads anymore. */
	bool bIsManagerShuttingDown = false;
	...
	/**
	 * Notifies the manager that one of the shader compiling thread has started.
	 * This updates the job priority range for all threads depending on how many threads the manager handles.
	 */
	void OnShaderCompilingThreadStarted(FShaderCompileThreadRunnableBase* Thread);

	/**
	 * Notifies the manager that one of the shader compiling thread has stopped and must be removed from the list.
	 * This updates the job priority range for all threads depending on how many threads the manager handles.
	 */
	void OnShaderCompilingThreadStopped(FShaderCompileThreadRunnableBase* Thread);

	/** Assigns the job priorities each shader compiling thread is responsible for depending on how many threads there are. */
	void AssignShaderCompilingThreadJobPriorities();
...
public:
	...
	bool AllowForIncompleteShaderMaps() const
	{
		return bAllowForIncompleteShaderMaps;
	}
	 ...
	 */
	ENGINE_API void TransferJobsToLocalWorker(TArray<FShaderCommonCompileJobPtr>& JobsToTransfer);
