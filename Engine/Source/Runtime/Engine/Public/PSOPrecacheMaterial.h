...
/**
 * Boost priority to highest when the PSOs are required for drawing. 
 */
extern ENGINE_API void BoostPSOPriorityOnDraw(const TArray<FMaterialPSOPrecacheRequestID>& MaterialPSORequestIDs);
...
/**
 * Check if all precached PSOs for active materials need to be kept in memory
 */
extern ENGINE_API bool KeepPrecachedPSOsInMemoryForActiveMaterials();
...
/**
 * Dump material PSO precache data to CSV file
 */
extern ENGINE_API void DumpPSOPrecacheMaterialStats();

/**
 * Get the total compile time for all material based PSOs for given pass and/or vertex factory name (if nullptr then collected for all passes and/or vertex factory types)
 */
extern ENGINE_API uint64 GetPSOPrecacheMaterialCompileTime(const TCHAR* InPassName, const TCHAR* InVertexFactoryName);
