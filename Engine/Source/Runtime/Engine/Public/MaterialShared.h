...
namespace MIR { struct FDebugDumpIRState; }
#if WITH_EDITOR
...
using EShaderFrequencyMask = uint16;
...
public:
	...
	friend bool operator==(const FMaterialVirtualTextureStack& Lhs, const FMaterialVirtualTextureStack& Rhs)
	{
		...
		// Note: ShaderFrequencyMask is intentionally ignored — it only affects the editor-side decision to include the parameter in the VS compact uniform buffer.
		return true;
	}
	...
	LAYOUT_FIELD_EDITORONLY(EShaderFrequencyMask, ShaderFrequencyMask);
};
...
/**
* Represents the evaluation of a material parameter directly to a word offset in the preshader uniform buffer.
* A UniformParameterEvaluation avoids the overhead of executing an entire preshader when all it does is 
* evaluate a parameter with a single instruction, and is also used to efficiently preload parameters.
* 
* Note that during material translation, BufferOffset may contain a unique "register" index, which is later
* fixed up to the buffer offset by a call to FixupNumericParameterEvaluations.
*/
...
public:
	...
	LAYOUT_FIELD(uint16, BufferOffset); 	/** Offset in words where result will be written. */
	LAYOUT_FIELD(uint16, ComponentCount); 	/** Number of components to write */
};
...
public:
	friend inline bool operator==(const FMaterialUniformPreshaderHeader& Lhs, const FMaterialUniformPreshaderHeader& Rhs)
	{
		return Lhs.Type == Rhs.Type &&
			...
			Lhs.OpcodeOffset == Rhs.OpcodeOffset &&
	...
	LAYOUT_FIELD(uint16, BufferOffset); /** Offset in the uniform buffer where result will be written */
...
/** Metadata allowing to create a dense uniform buffer from the generic one*/
class FCompactionMetaDataSegment
{
	DECLARE_TYPE_LAYOUT(FCompactionMetaDataSegment, NonVirtual);
public:
	LAYOUT_FIELD(uint16, SrcOffset);
	LAYOUT_FIELD(uint16, DestOffset);
	LAYOUT_FIELD(uint16, SizeToCopy);
};


/** Stores information per frequency about the values stored in FUniformExpressionSet*/
class FCompactUniformExpressionSet
{
	DECLARE_TYPE_LAYOUT(FCompactUniformExpressionSet, NonVirtual);
public:
	FCompactUniformExpressionSet() : UniformPreshaderBufferSize(0u) {}

	LAYOUT_FIELD(FRHIUniformBufferLayoutInitializer, UniformBufferLayoutInitializer);
	LAYOUT_FIELD(TMemoryImageArray<FCompactionMetaDataSegment>, CompactionSegments);
...
public:
...
#if WITH_EDITOR
	TArray<FShaderParametersMetadata*> CreateBufferStructs(EShaderPrecisionModifier::Type ParameterPrecision);
#endif
	...
	static void CompactUniformBuffer(uint8* DestBuffer, int32 DestBufferSize, const uint8* SrcBuffer, int32 SrcBufferSize, const FCompactUniformExpressionSet& CompactUniforms);
	...
	void AddTextureShaderFrequencyMask(EMaterialTextureParameterType Type, int32 Index, EShaderFrequencyMask Mask)
	{
		UniformTextureParameters[(int32)Type][Index].ShaderFrequencyMask |= Mask;
	}
#endif
	...
	void AddNumericParameterEvaluation(uint32 ParameterIndex, uint32 BufferOffset, uint32 ComponentCount);
	
	// Fixes up numeric parameter evaluation indices to offsets in the output buffer, using the given remapping table.
	// Also, optionally fixes up component counts.
	void FixupNumericParameterEvaluations(TConstArrayView<uint16> Remap, TConstArrayView<uint8> UniformComponentCount);
	...
	// Returns size of uniform preshader buffer in float4s.
	uint32 GetUniformBufferSize() const { return UniformPreshaderBufferSize; }

	// Returns the uniform preshader data.
	UE::Shader::FPreshaderData& GetPreshaderData() { return UniformPreshaderData; }
	const UE::Shader::FPreshaderData& GetPreshaderData() const { return UniformPreshaderData; }

	// Returns size of temporary preshader buffer in float4s.  Temporary space should be added to the CPU side uniform buffer.
	uint32 GetTemporaryBufferSize() const { return UniformPreshaderData.Preshader2TemporarySize; }
	...
	/// @param BufferOffset Specifies the location in the preshader buffer where the result of the preshader instruction will be written to. See FMaterialUniformPreshaderHeader::BufferOffset.
	/// @param EntryType Specifies the value type of the preshader instruction output. See FMaterialUniformPreshaderHeader::Type.
	...
	const FCompactUniformExpressionSet* GetCompactUniformsVS() const 
	{ 
		check(CompactUniformsVSOptional.Num() <= 1); 
		return CompactUniformsVSOptional.IsEmpty() ? nullptr : &CompactUniformsVSOptional[0]; 
	}

	FCompactUniformExpressionSet* GetCompactUniformsVS() 
	{ 
		check(CompactUniformsVSOptional.Num() <= 1); 
		return CompactUniformsVSOptional.IsEmpty() ? nullptr : &CompactUniformsVSOptional[0]; 
	}
	...
	FCompactUniformExpressionSet* CreateCompactUniformsVS()
	{
		...
		CompactUniformsVSOptional.Add(FCompactUniformExpressionSet());
		return &CompactUniformsVSOptional[0];
	}
...
protected:
	...
	friend struct MIR::FDebugDumpIRState;
	...
	// Using TMemoryImageArray here because CompactUniformsVS is optional for now, and we don't want to bloat FUniformExpressionSet when disabled
	// FUniformExpressionSet are getting copied with shadermaps, so things are usually copied by value, so having a pointer makes things trickier
	LAYOUT_FIELD(TMemoryImageArray<FCompactUniformExpressionSet>, CompactUniformsVSOptional);
...
public:
	FMaterialCompilationOutput() :
		...
		bUsesOpacityMask(false),
		bUsesCustomHLSLPixelDiscard(false),
		bUsesOpacity(false)
	{
#if WITH_EDITOR
	...
	/** True if the opacity mask isn't constant 1.0 (after constant folding). */
	LAYOUT_BITFIELD(uint8, bUsesOpacityMask, 1);

	/** True if any UMaterialExpressionCustom compiled into this material uses clip() or discard to conditionally discard pixels, making it behave like an opacity mask. */
	LAYOUT_BITFIELD(uint8, bUsesCustomHLSLPixelDiscard, 1);

	/** True if the opacity isn't constant 1.0 (after constant folding). */
	LAYOUT_BITFIELD(uint8, bUsesOpacity, 1);
};
...
struct FMaterialShaderParametersBase
{
	...
	union
	{
		...
		struct
		{
			...
			uint16 bSamplesMaterialCache : 1;
			uint16 bHasMaterialCacheOutput : 1;
			uint16 bIsUsedWithMeshDeformer : 1;
			uint16 bIsUsedWithInstancedSkinnedMesh : 1;
		};
	};
...
struct FMaterialShaderParameters : public FMaterialShaderParametersBase
{
	...
	UE_DEPRECATED(5.8, "This will be moved under protected. Use FCompilationShaderParameters or FConservativeShaderParameters to create Shader Parameters.")
	ENGINE_API explicit FMaterialShaderParameters(const FMaterial* InMaterial = nullptr);

protected:
	void InitializeShaderLayoutParamsFromMaterial(const FMaterial* InMaterial);
	void InitializeShaderLayoutParamsFromCompOutput(const FMaterial* InMaterial, const FMaterialCompilationOutput& InCompilationOutput);
};

// For Shader Compilation. Built based on Translation output.
struct FCompilationShaderParameters : public FMaterialShaderParameters
{
	ENGINE_API explicit FCompilationShaderParameters(const FMaterial* InMaterial, const FMaterialCompilationOutput& InCompilationOutput);
};

// For DDC Key/Iterative cooking. Built based on asset data.
struct FConservativeShaderParameters : public FMaterialShaderParameters
{
	ENGINE_API FConservativeShaderParameters();
	ENGINE_API explicit FConservativeShaderParameters(const FMaterial* InMaterial);
...
{
public:
	uint64 CookedShaderMapIdHash = 0u;
...
public:
	 ...
	 */
	uint64 TextureReferencesHash = 0u;

	/** A hash of the content of files (dynamically) included by material expressions. */
	uint64 ExpressionIncludesHash = 0u;
	
	/** A hash of the base property overrides for this material instance. */
	uint64 BasePropertyOverridesHash = 0u;

	/** A hash of all external code declarations referenced in the shader map. */
	uint64 ExternalCodeReferencesHash = 0u;

	/** A hash of the base material's full path */
	uint64 BaseMaterialPathHash = 0u;
	...
	{
#if WITH_EDITOR
		return !IsCookedId() ? BaseMaterialId.IsValid() : (CookedShaderMapIdHash != 0u);
#else
		return (CookedShaderMapIdHash != 0u);
#endif
	}
	...
	{
#if WITH_EDITOR
		return !Ref.IsCookedId() ? Ref.BaseMaterialId.A : (uint32)(Ref.CookedShaderMapIdHash);
#else
		// Using the a truncated form of the cooked shadermap hash value directly, this is fine assuming a good quality hash
		return (uint32)Ref.CookedShaderMapIdHash;
#endif
	}
	...
	UE_DEPRECATED(5.8, "Use version returning uint64 (SHA1 hashes no longer used)")
	inline void GetMaterialHash(FSHAHash& OutHash, bool bWithStaticParameters) const 
	{ 
		uint64 NewHash = GetMaterialHash(bWithStaticParameters);
		OutHash = FSHA1::HashBuffer(&NewHash, sizeof(NewHash));
	}

	ENGINE_API uint64 GetMaterialHash(bool bWithStaticParameters) const;

	uint64 GetMaterialHash() const
	{
		return GetMaterialHash(true);
	}
#endif
...
private:
	...
	LAYOUT_FIELD(FShaderHash, ShaderContentHash);
...
public:
	...
	{
		virtual ~FAsyncLoadContext() = default;
	...
	*/
	...
	void SaveToDerivedDataCache(const FConservativeShaderParameters& ShaderParameters);
#endif
	...
	const FShaderHash& GetShaderContentHash() const { return GetContent()->ShaderContentHash; }
...
#endif
	const FMaterialCompilationOutput& GetCompilationOutput() const { return GetContent()->MaterialCompilationOutput; }
	...
	const FRHIUniformBufferLayout* GetUniformBufferLayoutVS() const { return UniformBufferLayoutVS; }
...
#if WITH_EDITOR
	void InitializeForODSC(EShaderPlatform TargetShaderPlatform, const FMaterialCompilationOutput& NewCompilationOutput);
...
private:
	...
	FUniformBufferLayoutRHIRef UniformBufferLayoutVS;
...
#if WITH_EDITOR
	FShader* ProcessCompilationResultsForSingleJob(class FShaderCompileJob* SingleJob, const FShaderPipelineType* ShaderPipeline, uint64 MaterialShaderMapHash);
#endif
...
public:	
		...
		EMaterialKeyInclude IncludeFlags = EMaterialKeyInclude::All) const
	{
		BuildShaderMapIdOverride({
			.OutId = &OutId, .Platform = GetShaderPlatform(), .TargetPlatform = TargetPlatform, .IncludeFlags = IncludeFlags, .BaseMaterialPath = GetBaseMaterialPathName()}
			);
	}
...
public:
	...
	virtual bool IsUsedWithCurves() const { return false; }
	...
	virtual bool IsUsedWithMeshDeformer() const { return false; }
	virtual bool IsUsedWithInstancedSkinnedMesh() const { return false; }
	virtual bool IsUsedWithStaticMesh() const { return true; }
	virtual bool HasAnyUsage() const { return true; }
	...
	*/
	...
	virtual TObjectPtr<UObject> GetMatDefaultTextureAtIdx(const int32 inIdx) const = 0;

#if WITH_EDITOR
	virtual int32 GetMatDefaultTextureIdx(TObjectPtr<UObject> InTexture) const = 0;
	virtual const TArray<TObjectPtr<UObject>>& GetMatDefaultTextures() const = 0;
#endif
	...
	*/
	...
	ENGINE_API bool WritesEveryPixel(bool bShadowPass, bool bVFTypeSupportsNullPixelShader, bool bOpacityMaskIsOpaque) const;
	...
	*
	*/
	UE_DEPRECATED(5.7, "Call GetShaderTypes without EShaderPlatform and with specialized ShaderParameters")
	...
	UE_DEPRECATED(5.8, "Call GetShaderTypes with specialized ShaderParameters")
	/* Returns shader types based on conservative Shader Parameters. */
	...
	/* Returns shader types based on given Shader Parameters. */
	ENGINE_API void GetShaderTypes(const FPlatformTypeLayoutParameters& LayoutParams, TArray<FDebugShaderTypeInfo>& OutShaderInfo, const FMaterialShaderParameters& InParameters) const;
...
protected:
	...
	*/
	...
	/** Retrieves the full path of the base UMaterial associated with this FMaterial */
	virtual FString GetBaseMaterialPathName() const { return TEXT(""); }
	/** Get a debug name useful for debugging, just the base material path by default */
	...
	/** Produces arrays of any shader and vertex factory type that this material is dependent on, using conservative parameters. */
...
#if WITH_EDITOR
	uint64 GetReferencedTexturesHash() const;

	uint64 GetExpressionIncludesHash() const;

	uint64 GetExternalCodeReferencesHash() const;
#endif // WITH_EDITOR
...
public:
	...
	ENGINE_API virtual bool IsUsedWithMeshDeformer() const override;
	ENGINE_API virtual bool IsUsedWithInstancedSkinnedMesh() const override;
	ENGINE_API virtual bool IsUsedWithNanite() const override;
	ENGINE_API virtual bool IsUsedWithVoxels() const override;
	ENGINE_API virtual bool IsUsedWithCurves() const override;
	...
	ENGINE_API virtual bool HasAnyUsage() const override;
	...
	/** @return The index at which the given texture sits in the default textures array for this material. */
	ENGINE_API virtual int32 GetMatDefaultTextureIdx(TObjectPtr<UObject> InTexture) const override;

	/** @return All default textures present in this material. */
	ENGINE_API virtual const TArray<TObjectPtr<UObject>>& GetMatDefaultTextures() const override;
#endif // WITH_EDITOR
	...
	/**
	 * Returns the relevant texture for the given Idx.
	 * In editor, it looks through all default textures. 
	 * In non-editor, it has access to default textures based on cook output, if ShouldStripUnusedDefaultTextures returns true.
	 * @return The texture present at the given Idx or nullptr if not available.
	 */
	ENGINE_API virtual TObjectPtr<UObject> GetMatDefaultTextureAtIdx(const int32 InIdx) const override;

	UE_DEPRECATED(5.8, "Please use the GetMatDefaultTextures for editor or GetMatDefaultTextureAtIdx for non-editor.")
...
class FMaterialUpdateContext
{
	...
	/** True if the SkipRecacheStaticPermutations option was specified. */
	bool bSkipRecacheStaticPermutations;
	/** True if the SkipUpdateStaticDrawLists option was specified. */
	bool bSkipUpdateStaticDrawLists;
...
public:
	...
	struct EOptions
	{
		enum Type
		{
			 ...
			 */
			SyncWithRenderingThread = 0x02,
			/* Recreates only the render state for all components (mutually exclusive with ReregisterComponents) */
			RecreateRenderStates = 0x04,
			/** Skip recache of material instance permutations. Only safe to use if you know that no shader code or static switches are changing. */
			SkipRecacheStaticPermutations = 0x08,
			/**
			 * Skip updating static draw lists.
			 * Has no meaning if ReregisterComponents or RecreateRenderStates is set as it is already skipped.
			 * Only safe to use this if you know that no static draw lists could be using the material.
			*/
			SkipUpdateStaticDrawLists = 0x10,
...
class FEditorMaterialResourceBase : public FMaterial
{
	ENGINE_API virtual const TArray<TObjectPtr<UObject>>& GetMatDefaultTextures() const override;
	ENGINE_API virtual int32 GetMatDefaultTextureIdx(TObjectPtr<UObject> InTexture) const override;
	ENGINE_API virtual TObjectPtr<UObject> GetMatDefaultTextureAtIdx(const int32 InIdx) const override;
...
/**
 * Utility method for getting the shaders for the current platform, using FCompilationShaderParameters.
 * In case the ShaderMap is available, it uses the Compilation output within to generate the shaders.
 * In case it is not, it handles creating that output efficiently, without triggering shader compilation.
 */
ENGINE_API TArray<FDebugShaderTypeInfo> GetShadersUsingCompilationParameters_GameThread(UMaterialInterface* InMaterial, const EShaderPlatform InShaderPlatform = GMaxRHIShaderPlatform);
#endif
