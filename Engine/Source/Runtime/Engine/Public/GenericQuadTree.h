...
void TQuadTree<ElementType, NodeCapacity>::Insert(const ElementType& Element, const FBox2D& Box, const TCHAR* DebugContext)
{
	...
	if (!Box.Intersect(TreeBox))
	{
		...
		UE_LOGF(LogQuadTree, Warning, "[%ls] Adding element (%ls) that is outside the bounds of the quadtree root (%ls). Consider resizing.", DebugContext ? DebugContext : TEXT("Unknown Source"), *Box.ToString(), *TreeBox.ToString());
	}
...
void TQuadTree<ElementType, NodeCapacity>::InsertElementRecursive(const ElementType& Element, const FBox2D& Box, const TCHAR* DebugContext)
{
	...
	if (NumQuads == 0)
	{
		...
		if (!bCanSplitTree || Nodes.Num() < NodeCapacity)
		{
			...
			if (!bCanSplitTree)
			{
				UE_LOGF(LogQuadTree, Verbose, "[%ls] Minimum size %f reached for quadtree at %ls. Filling beyond capacity %d to %d", DebugContext ? DebugContext : TEXT("Unknown Source"), MinimumQuadSize, *Position.ToString(), NodeCapacity, Nodes.Num());
			}
		}
		else
		{
