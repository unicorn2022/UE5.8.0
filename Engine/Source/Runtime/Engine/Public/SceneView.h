...
struct FSceneViewInitOptions : public FSceneViewProjectionData
{
	...
	FSceneViewInitOptions()
		...
		, SkylightScale(FLinearColor::White)
...
struct FViewMatrices
{
	...
	{
		FMatrix ViewRotationMatrix = FMatrix::Identity;
		FMatrix ProjectionMatrix = FMatrix::Identity; // ViewToClip
		FVector ViewOrigin = FVector::ZeroVector;
		FIntRect ConstrainedViewRect = FIntRect(0, 0, 0, 0);
		FVector CameraToViewTarget = FVector::ZeroVector;
	...
	FViewMatrices()
	{
		...
		ViewToClipNoAA.SetIdentity();
		ClipToView.SetIdentity();
		WorldToView.SetIdentity();
		ViewToWorld.SetIdentity();
		WorldToClip.SetIdentity();
		ClipToWorld.SetIdentity();
		...
		TranslatedWorldToClip.SetIdentity();
		ClipToTranslatedWorld.SetIdentity();
		ScreenToClipMatrix.SetIdentity();
		WorldToFirstPerson.SetIdentity();
		FirstPersonToWorld.SetIdentity();
...
private:
	...
	/** ProjectionMatrix : UE projection matrix projects such that clip space Z=1 is the near plane, and Z=0 is the infinite far plane. */
	FMatrix		ViewToClip;
	/** ProjectionNoAAMatrix : UE projection matrix projects such that clip space Z=1 is the near plane, and Z=0 is the infinite far plane. Don't apply any AA jitter */
	FMatrix		ViewToClipNoAA;
	/** InvProjectionMatrix : UE projection matrix projects such that clip space Z=1 is the near plane, and Z=0 is the infinite far plane. */
	FMatrix		ClipToView;
	// ViewMatrix..
	FMatrix		WorldToView;
	// InvViewMatrix..
	FMatrix		ViewToWorld;
	// ViewProjectionMatrix : UE projection matrix projects such that clip space Z=1 is the near plane, and Z=0 is the infinite far plane. */
	FMatrix		WorldToClip;
	// InvViewProjectionMatrix : UE projection matrix projects such that clip space Z=1 is the near plane, and Z=0 is the infinite far plane. */
	FMatrix		ClipToWorld;
	...
	/** TranslatedViewProjectionMatrix: The view-projection transform, starting from world-space points translated by -ViewOrigin. */
	FMatrix		TranslatedWorldToClip;
	/** InvTranslatedViewProjectionMatrix: The inverse view-projection transform, ending with world-space points translated by -ViewOrigin. */
	...
	/** Scale and shear to avoid first person primitives to clip with the scene and to achieve a first person specific FOV. Transforms from world space to first person "space". */
	FMatrix		WorldToFirstPerson;
	/** Inverse FirstPersonTransform. Transforms from first person "space" to world space. */
	FMatrix		FirstPersonToWorld;
...
public:
	...
	const FMatrix& GetViewToClip() const
	{
		return ViewToClip;
	}

	const FMatrix& GetViewToClipNoAA() const
	{
		return ViewToClipNoAA;
	}

	const FMatrix& GetClipToView() const
	{
		return ClipToView;
	}

	const FMatrix& GetWorldToView() const
	{
		return WorldToView;
	}

	const FMatrix& GetViewToWorld() const
	{
		return ViewToWorld;
	}

	const FMatrix& GetWorldToClip() const
	{
		return WorldToClip;
	}

	const FMatrix& GetClipToWorld() const
	{
		return ClipToWorld;
	}
	...
	const FMatrix& GetTranslatedWorldToClip() const
	{
		return TranslatedWorldToClip;
	}

	const FMatrix& GetClipToTranslatedWorld() const
	{
		return ClipToTranslatedWorld;
	}
	...
	const FMatrix& GetWorldToFirstPerson() const
	{
		return WorldToFirstPerson;
	}

	const FMatrix& GetFirstPersonToWorld() const
	{
		return FirstPersonToWorld;
	}

	UE_DEPRECATED(5.8, "Use GetViewToClip() instead")
	inline const FMatrix& GetProjectionMatrix()						const { return GetViewToClip(); }	
	UE_DEPRECATED(5.8, "Use GetViewToClipNoAA() instead")
	inline const FMatrix& GetProjectionNoAAMatrix() 				const { return GetViewToClipNoAA(); }	
	UE_DEPRECATED(5.8, "Use GetClipToView() instead")
	inline const FMatrix& GetInvProjectionMatrix() 					const { return GetClipToView(); }	
	UE_DEPRECATED(5.8, "Use GetWorldToView() instead")
	inline const FMatrix& GetViewMatrix() 							const { return GetWorldToView(); }	
	UE_DEPRECATED(5.8, "Use GetViewToWorld() instead")
	inline const FMatrix& GetInvViewMatrix() 						const { return GetViewToWorld(); }
	UE_DEPRECATED(5.8, "Use GetWorldToClip() instead")
	inline const FMatrix& GetViewProjectionMatrix() 				const { return GetWorldToClip(); }
	UE_DEPRECATED(5.8, "Use GetClipToWorld() instead")
	inline const FMatrix& GetInvViewProjectionMatrix() 				const { return GetClipToWorld(); }
	UE_DEPRECATED(5.8, "Use GetTranslatedWorldToClip() instead")
	inline const FMatrix& GetTranslatedViewProjectionMatrix() 		const { return GetTranslatedWorldToClip(); }
	UE_DEPRECATED(5.8, "Use GetClipToTranslatedWorld() instead")
	inline const FMatrix& GetInvTranslatedViewProjectionMatrix()	const { return GetClipToTranslatedWorld(); }
	UE_DEPRECATED(5.8, "Use GetWorldToFirstPerson() instead")
	inline const FMatrix& GetFirstPersonTransform() 				const { return GetWorldToFirstPerson(); }
	UE_DEPRECATED(5.8, "Use GetFirstPersonToWorld() instead")
	inline const FMatrix& GetInvFirstPersonTransform() 				const { return GetFirstPersonToWorld(); }
	...
	inline FVector2f GetInvTanHalfFov() const
	{
		...
		{
			return FVector2f(static_cast<float>(ViewToClip.M[0][0]), static_cast<float>(ViewToClip.M[1][1]));
		}
	...
	inline FVector2f GetTanHalfFov() const
	{
		...
		if (IsPerspectiveProjection())
		{
			return FVector2f(static_cast<float>(ClipToView.M[0][0]), static_cast<float>(ClipToView.M[1][1]));
		}
		return  FVector2f(1.0f,1.0f);
	}
	...
	inline FVector4f GetTanAndInvTanHalfFOV() const
	{
		...
		if (IsPerspectiveProjection())
		{
			return FVector4f(static_cast<float>(ClipToView.M[0][0]), //ClipToView[0][0] - X axis
							 static_cast<float>(ClipToView.M[1][1]), //ClipToView[1][1] - Y axis
							 static_cast<float>(ViewToClip.M[0][0]), //ViewToClip[0][0] - 1/X axis
							 static_cast<float>(ViewToClip.M[1][1])); //ViewToClip[1][1] - 1/Y axis
		}
	...
	void HackAddTemporalAAProjectionJitter(const FVector2D& InTemporalAAProjectionJitter)
	{
		...
		if (IsPerspectiveProjection())
		{
			ViewToClip.M[2][0] += TemporalAAProjectionJitter.X;
			ViewToClip.M[2][1] += TemporalAAProjectionJitter.Y;
		}
		else
		{
			ViewToClip.M[3][0] += TemporalAAProjectionJitter.X;
			ViewToClip.M[3][1] += TemporalAAProjectionJitter.Y;
		}
		ClipToView = InvertProjectionMatrix(ViewToClip);
	...
	{
		FMatrix ProjNoAAMatrix = ViewToClip;
	...
	{
		return WorldToView.RemoveTranslation() * ViewToClip;
	}
	...
	const FVector2D ComputeHalfFieldOfViewPerAxis() const
	{
		if(IsPerspectiveProjection())
		{
			const FMatrix ClipToViewNoAA = ComputeInvProjectionNoAAMatrix();

			FVector VCenter = FVector(ClipToViewNoAA.TransformPosition(FVector(0.0, 0.0, 0.0)));
			FVector VUp = FVector(ClipToViewNoAA.TransformPosition(FVector(0.0, 1.0, 0.0)));
			FVector VRight = FVector(ClipToViewNoAA.TransformPosition(FVector(1.0, 0.0, 0.0)));
	...
	{
		return ( ViewToClip.M[3][3] - ViewToClip.M[3][2] ) / ( ViewToClip.M[2][2] - ViewToClip.M[2][3] );
	}
	...
	FMatrix::FReal ComputeOrthoFarPlane() const
	{
		if(!IsPerspectiveProjection())
		{
			return ComputeNearPlane() - ClipToView.M[2][2];
		}
		return 0.0;
	}
	...
	void ApplyWorldOffset(const FVector& InOffset)
	{
		...
		WorldToView.SetOrigin(WorldToView.GetOrigin() + WorldToView.TransformVector(-InOffset));
		ViewToWorld.SetOrigin(ViewOrigin);
		RecomputeDerivedMatrices();
	}
	...
	FVector2f GetOrthoDimensions() const
	{
		if (!IsPerspectiveProjection())
		{
			return  FVector2f(static_cast<float>(ClipToView.M[0][0]) * 2.0f, static_cast<float>(ClipToView.M[1][1]) * 2.0f);
		}
...
private:
	inline void RecomputeDerivedMatrices()
	{
		...
		WorldToClip = GetWorldToView() * GetViewToClip();
		ClipToWorld = GetClipToView() * GetViewToWorld();

		// Compute a transform from view origin centered world-space to clip space.
		TranslatedWorldToClip = GetTranslatedViewMatrix() * GetViewToClip();
		ClipToTranslatedWorld = GetClipToView() * GetInvTranslatedViewMatrix();
	}
...
BEGIN_GLOBAL_SHADER_PARAMETER_STRUCT_WITH_CONSTRUCTOR(FMobileDirectionalLightShaderParameters, ENGINE_API)
	...
	SHADER_PARAMETER_EX(FVector4f, DirectionalLightShadowDepthRange, EShaderPrecisionModifier::Half)
...
#define VIEW_UNIFORM_BUFFER_MEMBER_TABLE \
	...
	VIEW_UNIFORM_BUFFER_MEMBER_PER_VIEW(FMatrix44f, InvFirstPersonTransform) \
	...
	VIEW_UNIFORM_BUFFER_MEMBER(FVector3f, VolumetricFogGridSize) \
	...
	VIEW_UNIFORM_BUFFER_MEMBER(FVector4f, ToonProfileTextureSizeAndInvSize) \
	VIEW_UNIFORM_BUFFER_MEMBER(FVector2f, ToonProfileDitherPatternSizeAndInvSize) \
	...
	VIEW_UNIFORM_BUFFER_MEMBER_ARRAY(FIntVector4, PhysicsFieldTargetMappings, MAX_PHYSICS_FIELD_TARGETS) \
...
BEGIN_GLOBAL_SHADER_PARAMETER_STRUCT_WITH_CONSTRUCTOR(FViewUniformShaderParameters, ENGINE_API)
	...
	SHADER_PARAMETER_TEXTURE(Texture2D<float4>,  ShadingEnergyGGXMaxSpecTexture)
	...
	// Toon Profile texture
	SHADER_PARAMETER_TEXTURE(Texture2DArray, ToonProfileTexture)
	SHADER_PARAMETER_SAMPLER(SamplerState, ToonProfileSampler)
	// Water
	...
	SHADER_PARAMETER_SRV(Buffer<float>, PhysicsFieldNodesParams)
	SHADER_PARAMETER_SRV(Buffer<int>, PhysicsFieldNodesOffsets)
	SHADER_PARAMETER_SRV(Buffer<int>, PhysicsFieldTargetsOffsets)
...
{
	inline void CopyIntoInstancedViewParameters(FInstancedViewUniformShaderParameters& InstancedViewParameters, const FViewUniformShaderParameters& ViewParameters, uint32 CopyViewId)
	{
		VIEW_UNIFORM_BUFFER_MEMBER_TABLE
	}
}
...
namespace ViewParametersUtils
{
	...
	inline void CopyViewParametersOnly(FViewUniformShaderParameters& DstView, const FViewUniformShaderParameters& SrcView)
	{
		VIEW_UNIFORM_BUFFER_MEMBER_TABLE
	}
}
...
public:
	...
	/** Color scale multiplier applied to sky light color */
	FLinearColor SkylightScale;
	...
	/** Current MegaLights visualization mode */
	FName CurrentMegaLightsVisualizationMode;
	...
	*/
	...
	/** DEPRECATED use IsInstancedStereo() instead. 
	 * True if instanced stereo is enabled. */
	bool bIsInstancedStereoEnabled;

	/** DEPRECATED all instanced stereo is now what used to be called multi viewport.
	 * use IsInstancedStereo() instead.
	 * True if mobile multi-viewport is enabled. */
	UE_DEPRECATED(5.9, "all instanced stereo is now what used to be called multi viewport. IsInstancedStereo() should be used")
	bool bIsMultiViewportEnabled;
	
	/* True if multi-view instanced stereo rendering is enabled. */
	bool bIsMobileMultiViewEnabled;

public:
	/** Number of views when using mobile multi view. */
	[[nodiscard]] ENGINE_API int32 GetMultiViewCount() const;
	
	/** True if the view should render as an instanced stereo pass */
	[[nodiscard]] ENGINE_API bool IsInstancedStereo() const;
	
	/** Whether the shaders are compiled with mobile multi view == true and thus expect texture arrays instead of a single texture */
	[[nodiscard]] ENGINE_API bool UsesMobileMultiViewShaders() const;
	
	/** Whether the shaders are compiled with instanced stereo == true and thus expect a single texture and an instanced view */
	[[nodiscard]] ENGINE_API bool UsesInstancedStereoShaders() const;
	
	/** Creates a texture or a texture array of the right size based on setup (texture array for mobile multi view shaders) */
	[[nodiscard]] ENGINE_API FRDGTextureDesc CreateStereoTextureDesc(FIntPoint Size, EPixelFormat Format, FClearValueBinding Clear, ETextureCreateFlags Flags) const;
	
private:
	/** Number of views for multiview scenarios. */
	uint32 MultiViewCount = 0;
...
public:
	...
	*/
	...
	struct FPostProcessCompositePrimitiveParams
	{
		...
		float OccludedDitheringPatternSize = 2.0f;
		/** The strength of the dithering applied to occluded primitives. */
		float OccludedDithering = 0.8f;
		/** The brightness of occluded primitives. */
		float OccludedBrightness = 0.7f;
	} CompositePrimitiveParams;
	...
	FName PostProcessDebugPropertyName;
...
public:
	...
	ENGINE_API FVector4 CursorToScreen(float X, float Y, float Z, bool bClamped = true) const;
	...
	inline FVector GetViewRight() const { return ViewMatrices.GetWorldToView().GetColumn(0); }
	inline FVector GetViewUp() const { return ViewMatrices.GetWorldToView().GetColumn(1); }
	inline FVector GetViewDirection() const { return ViewMatrices.GetWorldToView().GetColumn(2); }
	 ...
	 */
	ENGINE_API void OverridePostProcessSettings(const FPostProcessSettings& Src, float Weight, bool bAllowBlendables = true);
	...
	/** True if the view should render as an instanced stereo pass */
	UE_DEPRECATED(5.9, "IsInstancedStereoPass() is deprecated, use IsInstancedStereo() instead")
	...
	inline bool IsRuntimeReflectionCapture() const
	{
		return bIsSceneCaptureCube && bIsReflectionCapture;
	}

	/** Buffer to the shader binding data used for inline raytracing - only valid when inline raytracing is enabled. */
	virtual FRDGBufferRef GetInlineRayTracingBindingDataBuffer() const { return nullptr; }
...
protected:
	 ...
	 */
	virtual ~ISceneViewFamilyExtention() = default;
...
public:
	...
	*/
	...
	{
		/** The color channel to mask to, whose value will be output as a grayscale color */
		EColorChannelMask ColorChannelMask = EColorChannelMask::All;

		/** When true, the alpha channel is inverted when being visualized; does not effect the actual alpha value, just its visualization */
		bool bInvertAlphaChannelMask = false;

		/** Indicates that a checkerboard should be alpha blended underneath the viewport to help visualize the view's alpha */
	...
	/** Scale between the current rendering resolution and the final output resolution.
	 *  Used e.g. by high-res tiled screenshot capture where individual tiles are rendered
	 *  at a smaller resolution and composited into a larger final image */
	float FinalResolutionScale = 1.0f;
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	inline void SetMegaFiltersInterface(UE::Renderer::Private::IMegaFilters* InMegaFiltersInterface)
	{
		check(InMegaFiltersInterface);
		checkf(MegaFiltersInterface == nullptr, TEXT("View family already had mega filters assigned."));
		MegaFiltersInterface = InMegaFiltersInterface;
	}

	inline const UE::Renderer::Private::IMegaFilters* GetMegaFiltersInterface() const
	{
		return MegaFiltersInterface;
	}
...
private:
	...
	UE::Renderer::Private::IMegaFilters* MegaFiltersInterface;
...
ENGINE_API FVector4f CreateInvDeviceZToWorldZTransform(const FMatrix& ProjMatrix);