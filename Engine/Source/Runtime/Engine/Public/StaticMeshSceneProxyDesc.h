...
struct FStaticMeshSceneProxyDesc : public FPrimitiveSceneProxyDesc
{
	...
	// Optional pre-resolved resource pointer for MeshPaintTexture. Populated GT-side by callers
	// that build the desc on a thread where UTexture::GetResource() is not safe to call (e.g.
	// FastGeo's cooked async render-state worker). When non-null, GetMeshPaintTextureResource()
	// returns this directly and skips the live GetResource() call. Stays null for callers that
	// don't pre-resolve, preserving the original behavior.
	FTextureResource* CachedMeshPaintTextureResource = nullptr;
