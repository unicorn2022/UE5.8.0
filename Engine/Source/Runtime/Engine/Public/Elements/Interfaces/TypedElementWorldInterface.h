...
struct FWorldElementPasteImporter
{
	...
	struct FContext
	{
		...
		UWorld* World;
		FTypedElementHandle PasteUnder;
		FStringView Text;
	};
...
struct TTypedElement<ITypedElementWorldInterface> : public TTypedElementBase<ITypedElementWorldInterface>
{
	...
	UE_DEPRECATED(5.8, "The element handle is already included in TTypedElement access. Use the version of this function that does not take the FTypedElementHandle parameter.")
	bool AddIgnoredElementToCollisionQueryParams(const FTypedElementHandle& InElementHandle, FCollisionQueryParams& InOutParams, bool bAlsoIgnoreSubElements = true) const { return AddIgnoredElementToCollisionQueryParams(InOutParams, bAlsoIgnoreSubElements); }
	bool AddIgnoredElementToCollisionQueryParams(FCollisionQueryParams& InOutParams, bool bAlsoIgnoreSubElements = true) const { return InterfacePtr->AddIgnoredElementToCollisionQueryParams(*this, InOutParams, bAlsoIgnoreSubElements); }
