...
extern ENGINE_API void ConditionalBreakOnPSOPrecacheShader(const FComputePipelineStateInitializer& ComputeInitializer);
...
inline void ConditionalBreakOnPSOPrecacheShader(FRHIComputeShader* ComputeShader)
{
	FComputePipelineStateInitializer Initializer(ComputeShader);
	ConditionalBreakOnPSOPrecacheShader(Initializer);
}
...
namespace PSOCollectorStats
{
	...
	/**
	* Gets the state PSOs hash only from the grpahics PSOs - should be valid
	*/
	inline uint64 GetPSOPrecacheStateHash(const FGraphicsPipelineStateInitializer& GraphicsPSOInitializer)
	{
		check(GraphicsPSOInitializer.StatePrecachePSOHash != 0);
	 ...
	 */
	extern ENGINE_API uint64 GetPSOPrecacheHash(const FComputePipelineStateInitializer& ComputeInitializer);

	//UE_DEPRECATED(5.8, "Identifying compute pipelines specifying only a shader is deprecated, use a FComputePipelineStateInitializer.")
	inline uint64 GetPSOPrecacheHash(FRHIComputeShader& ComputeShader)
	{
		FComputePipelineStateInitializer ComputeInitializer(&ComputeShader);
		return GetPSOPrecacheHash(ComputeInitializer);
	}

	/**
	 * Create a shaders-only initializer where all state except for shaders is removed from the given initializer.
	 */
	extern ENGINE_API FGraphicsPipelineStateInitializer GetShadersOnlyInitializer(const FGraphicsPipelineStateInitializer& Initializer);
	...
	/**
	 * Check if the global compute PSO is precached and if not output information on what state is missing
	 */
	extern ENGINE_API void CheckGlobalComputePipelineStateInCache(const FComputePipelineStateInitializer& ComputeInitializer);

	//UE_DEPRECATED(5.8, "Identifying compute pipelines specifying only a shader is deprecated, use a FComputePipelineStateInitializer.")
	inline void CheckComputePipelineStateInCache(
	...
	{
		FComputePipelineStateInitializer ComputeInitializer(&ComputeShader);
		CheckComputePipelineStateInCache(ComputeInitializer, PSOPrecacheResult, Material, PSOCollectorIndex);
	}
	...
	/*
	* Update the global tracking stats
	*/
	extern ENGINE_API void UpdateGlobalStats(EPSOPrecacheType PSOPrecacheType, const FPSOPrecacheData& PrecacheData);
	...
	struct FPrecacheUsageData
	{
		...
		uint64 GetTotalMeshPassCount();
		uint64 GetTotalGlobalCount();
	...
	public:
		...
		FPrecacheStats& GetStats() { return Stats; }
...
#if PSO_PRECACHING_TRACKING
					Value->MaterialName = Material ? Material->GetAssetName() : FString("Unknown");
		...
		ENGINE_API bool IsUsed(uint64 PrecacheStateHash);
#if PSO_PRECACHING_TRACKING
	...
	extern ENGINE_API void DumpPSOPrecacheValidationStats();
}
