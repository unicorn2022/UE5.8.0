...
public:
	...
	*/
	...
	virtual float GetMinimumLuminanceInNits() const { return 0.f; }
	virtual float GetMaximumLuminanceInNits() const { return 100.f; }
	virtual float GetMaximumFullFrameLuminanceInNits() const { return 100.f; }
	virtual float GetHDRPaperWhiteInNits() const { return 203.f; }
	virtual const UE::Color::FColorSpace& GetLimitingColorSpace() const { return UE::Color::FColorSpace::GetSRGB(); }
...
using FViewportFrame UE_DEPRECATED(5.8, "FViewportFrame is deprecated. Use FSceneViewport instead.") = class FSceneViewport;
...
DECLARE_MULTICAST_DELEGATE_ThreeParams(FOnHDRScreenshotCaptured, int32 /*Width*/, int32 /*Height*/, const TArray<FLinearColor>& /*Colors*/);
...
struct FScreenshotRequest
{
	 ...
	 * @param bInRestrictToGameViewport If true then the capture will be restricted to the game viewport, so will not incldue the entire editor in PIE for example
	 */
	static ENGINE_API void RequestScreenshot(bool bInShowUI, const bool bInRestrictToGameViewport = false);
	 ...
	 * @param bInRestrictToGameViewport If true then the capture will be restricted to the game viewport, so will not incldue the entire editor in PIE for example
	 */
	static ENGINE_API void RequestScreenshot(const FString& InFilename, bool bInShowUI, bool bAddFilenameSuffix, bool bHdrScreenshot = false, const FIntRect& InViewRect = FIntRect(), const bool bInRestrictToGameViewport = false);
	...
	/**
	 * @return True if capture should be restricted to the game viewport
	 */
	static bool ShouldRestrictToGameViewport() { return bRestrictToGameViewport; }
	...
	static FOnHDRScreenshotCaptured& OnHDRScreenshotCaptured()
	{
		return HDRScreenshotCapturedDelegate;
	}
...
private:
	...
	static ENGINE_API FOnHDRScreenshotCaptured HDRScreenshotCapturedDelegate;
	...
	static ENGINE_API bool bRestrictToGameViewport;
...
struct FStatThermalsData
{
	FStatThermalsData() = default;
...
public:
	...
	ENGINE_API FViewport(FViewportClient* const InClient);
	...
	virtual void GetUnboundedMousePos(FIntPoint& OutUnboundedMousePosition, const bool bLocalPosition = true) {}
	...
	virtual int32 GetCurrentMonitorIndex() const { return -1; }
	virtual bool IsExclusiveFullscreenSupported() const { return true; }
	...
	/**
	 * Indicate that this frame is a camera cut.
	*/
	virtual void SetCameraCut(bool bInCameraCut) {}
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	UE_DEPRECATED(5.8, "FViewport::GetViewportFrame() and FViewportFrame are deprecated. Use FSceneViewport instead.")
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	*/
	...
	/**
	 *	Sets the client this viewport is associated with.  AddAssociation/RemoveAssociation will be called on the new/old clients.  nullptr to clear.
	 */
	ENGINE_API virtual void SetViewportClient(const TStrongPtrVariant<FViewportClient>& InViewportClient);

	UE_REWRITE void SetViewportClient(TYPE_OF_NULLPTR InViewportClient)
	{
		SetViewportClient(TStrongPtrVariant<FViewportClient>(nullptr));
	}

	template<UE::CPointerConvertibleTo<FViewportClient> ClientType>
	requires (!UE::CUObject<ClientType>)
	UE_DEPRECATED(5.8, "Use SetViewportClient(const TStrongPtrVariant<FViewportClient>&) instead")
	void SetViewportClient(ClientType* InViewportClient)
	{
		Client.Reset(); // No way to set this safely. If caller is using deprecated setter, safe to assume they're still only using raw pointer getter
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
	...
	const TStrongPtrVariant<FViewportClient>& GetClientPtr() const
	{
		return Client;
	}

	ENGINE_API FViewportClient& GetClientChecked() const;

	/** Returns a raw pointer to the client.
	 * Use when you know the Viewport will stay alive throughout the scope in which you access the returned pointer copy.
	 * Do not Viewport->GetClient()->... — either test the result, or use GetClientChecked() instead.
	 */
	 ...
	 */
	virtual void SetInitialSize( FIntPoint InitialSizeXY ) = 0;
	...
	/** Casts this instance to an FSceneViewport, or returns nullptr if this is not a scene viewport. */
	virtual class FSceneViewport* AsSceneViewport() { return nullptr; }

	TUniquePtr<FViewport> Snapshot();
...
protected:
	...
	UE_DEPRECATED(5.8, "Use Client instead")
	...
	public:
		...
		void Init(FRHICommandListBase& RHICmdList, uint32 NewSizeX, uint32 NewSizeY);

		UE_DEPRECATED(5.8, "RHI command list is now a required argument")
	...
	uint32 InitialPositionX = 0;

	/** The initial position of the viewport. */
	uint32 InitialPositionY = 0;
...
/** Informs a world (and the streaming manager) that a view has been rendered within it during the current frame.
 * Must be called for asset streaming, and any other features that need to know "where is the scene being drawn from?"
 */
ENGINE_API void AddStreamingViewInfo(UWorld& World, const FSceneView& View);
