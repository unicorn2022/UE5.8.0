...
#pragma region GetInternals
	...
	/** 
	 * Resolves and validates a FSkeletalMeshObject from a scene + component ID.
	 * Returns nullptr if the primitive is not found, not a skinned mesh, not yet updated, or CPU-skinned. 
	 */
	ENGINE_API static FSkeletalMeshObject* GetSkeletalMeshObject(FSceneInterface* InSceneInterface, FPrimitiveComponentId InComponentId);
...
#pragma region Deprecated
	...
	UE_DEPRECATED(5.8, "GetMorphTargetBufferForReading no longer requires section index and frame number.")
	/** Get direct access to morph target buffer SRV. */
	ENGINE_API static FRHIShaderResourceView* GetMorphTargetBufferForReading(
		FSkeletalMeshObject const* InMeshObject,
		int32 InLodIndex,
