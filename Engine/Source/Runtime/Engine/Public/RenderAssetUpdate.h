...
public:
	...
	*/
	...
	void AddRef() const final override 
	{ 
		NumRefs.Increment();
	}

	UE_API FReturnedRefCountValue Release() const final override;

	FReturnedRefCountValue GetRefCount() const final override
	{
...
private:
	...
	/** Iterate calling internal tick as long as there is progress. */
	ETaskState IterateInternalTick(EThreadType InCurrentThread, bool bWasAlreadyLocked);

#if !UE_BUILD_SHIPPING
	/** Iterate calling internal tick as long as there is progress with added profiling. */
	ETaskState IterateInternalTickWithProfiling(EThreadType InCurrentThread, bool bWasAlreadyLocked);
#endif
...
ENGINE_API void SuspendRenderAssetStreaming();
ENGINE_API void ResumeRenderAssetStreaming();
