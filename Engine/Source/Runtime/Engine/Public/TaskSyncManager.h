...
private:
	...
	static constexpr uint32 MaxWorkIndex = (1 << 24) - 1;
...
private:
	...
	struct FBatchData
	{
		...
		/** List of sync points that were reset during the frame, will be deleted at end */
		TArray<TSharedPtr<FActiveSyncPoint>> SyncPointsToDelete;
	...
	void InitializeForWorld(FWorldContextId WorldContext);
	void OnPostWorldInitialization(UWorld* World, const UWorld::InitializationValues IVS);
	void InitializeBatchForNewWorld(FBatchData& BatchData);
#if WITH_EDITOR
	void EndPIE(bool bStartSimulate);
#endif

	FActiveSyncPoint* GetOrCreateSyncPoint(FBatchData& BatchData, FRegisteredSyncPointData& SyncData);
	bool DestroySyncPointIfValid(FBatchData& BatchData, FSyncPointId::FInternalId SyncId);
