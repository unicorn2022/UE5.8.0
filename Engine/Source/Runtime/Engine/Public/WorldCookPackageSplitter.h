...
public:
	...
	struct ISubSplitter
	{
		virtual ~ISubSplitter()	= default;
		virtual TArray<ICookPackageSplitter::FGeneratedPackage> GetGenerateList(const UPackage* OwnerPackage) = 0;
		virtual TOptional<bool> RequiresEmptyPackageBeforePopulate(const UPackage* Package) = 0;
		...
		/**
		 * This can return true to disable the save of the generator package, for fine-tuned output control. Return unset by default.
		 * All splitters should return the same value, or leave unset (as only one sub-splitter should be responsible for each package), otherwise the resulting mismatch will log an error.
		 */
		virtual TOptional<bool> SkipSaveGeneratorPackage(const UPackage* OwnerPackage) = 0;
