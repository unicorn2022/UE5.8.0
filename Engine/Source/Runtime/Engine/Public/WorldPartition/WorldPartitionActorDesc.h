...
{
	virtual ~FWorldPartitionAssetDataPatcher() = default;
...
public:
	virtual ~FWorldPartitionComponentDesc() = default;
...
public:
	...
	virtual ~FWorldPartitionActorDesc() = default;
	...
	inline const TArray<FName>& GetNonExternalDataLayers() const { return DataLayers; }
	...
	inline const FTransform& GetActorTransformRelative() const { return ActorTransformRelative; }
	...
	UE_DEPRECATED(5.8, "Actors no longer contain data layers without assets")
	inline bool IsUsingDataLayerAsset() const { return true; }
...
public:
	...
	ENGINE_API virtual void UpdateActorToWorld(FTransform* InParentTransform = nullptr);
...
protected:
	...
	bool							bIsUsingDataLayerAsset; // No longer used, but serialized manually and needed if we ever load old data. Treat as a UPROPERTY in terms of deprecation.
...
class FWorldPartitionActorDescStringBuilder : public TStringBuilder<1024>
{
public:
	FWorldPartitionActorDescStringBuilder(FWorldPartitionActorDesc::EToStringMode InMode);
	void AppendFromString(const TCHAR* Name, const FString& Value, bool bUseLineStart = true);
	void AppendFromBool(const TCHAR* Name, bool bValue, bool bUseLineStart = true);

	template <typename Type>
	void AppendToString(const TCHAR* Name, const Type& Value, bool bUseLineStart = true)
	{
		if constexpr (std::is_same_v<Type, FBox>)
		{
			if (!Value.IsValid)
			{
				AppendPair(Name, TEXT("IsValid=false"), bUseLineStart);
				return;
			}
		}
		...
		AppendFromString(Name, Value.ToString(), bUseLineStart);
	}

private:
	void AppendPair(const TCHAR* Name, const TCHAR* Value, bool bUseLineStart);

	FWorldPartitionActorDesc::EToStringMode Mode;
