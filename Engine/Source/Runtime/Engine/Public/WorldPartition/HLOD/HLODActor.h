...
DECLARE_MULTICAST_DELEGATE_OneParam(FWorldPartitionHLODActorEditorRegisteredDelegate, AWorldPartitionHLOD*)
DECLARE_MULTICAST_DELEGATE_OneParam(FWorldPartitionHLODActorEditorUnregisteredDelegate, AWorldPartitionHLOD*)
#endif
...
public:
	...
	FName GetHLODResourcesPackagePath() const { return HLODResourcesPackagePath; }
	void SetHLODResourcesPackagePath(FName InHLODResourcesPackagePath) { HLODResourcesPackagePath = InHLODResourcesPackagePath; }

	ENGINE_API UPackage* GetHLODResourcesPackage() const;

	ENGINE_API bool BuildHLOD(bool bForceBuild = false);
	ENGINE_API bool ShouldBuildHLOD() const;
	...
	ENGINE_API static FWorldPartitionHLODActorEditorRegisteredDelegate& GetHLODActorEditorRegisteredDelegate();
	ENGINE_API static FWorldPartitionHLODActorEditorUnregisteredDelegate& GetHLODActorEditorUnregisteredDelegate();
	...
	ENGINE_API void UpdateHLODBuildReportContent();

	ENGINE_API const FHLODRebuildPolicyDataSet& GetHLODRebuildPolicyDataSet() const;
	ENGINE_API void SetHLODRebuildPolicyDataSet(const FHLODRebuildPolicyDataSet& InHLODRebuildPolicyDataSet);

	UE_DEPRECATED(5.8, "Hash data is now stored in a UHLODRebuildPolicyData object.")
	uint32 GetHLODHash() const { return HLODHash_DEPRECATED; }
	UE_DEPRECATED(5.8, "Hash data is now stored in a UHLODRebuildPolicyData object.")
	uint32 ComputeHLODHash() const { return 0; }
	UE_DEPRECATED(5.8, "Hash data is now stored in a UHLODRebuildPolicyData object.")
	void SetHLODHash(uint32 InHLODHash, const FString& InHLODBuildReportContent) {}
	UE_DEPRECATED(5.8, "Use override with no parameter.")
...
private:
	...
	void OnForceDisableShadowsCVarChanged(IConsoleVariable* CVar);
	void ApplyCastShadowToComponents(bool bInShouldCastShadows);
	...
	TArray<TObjectPtr<UHLODRebuildPolicyData>> HLODRebuildPolicyDataSet;

	UPROPERTY()
	FName HLODResourcesPackagePath;
#endif
	...
	UPROPERTY()
	uint32 HLODHash_DEPRECATED;
#endif

#if WITH_EDITOR
	static FWorldPartitionHLODBuildEventDelegate HLODBuildEventDelegate;
	static FWorldPartitionHLODActorEditorRegisteredDelegate HLODActorEditorRegisteredDelegate;
	static FWorldPartitionHLODActorEditorUnregisteredDelegate HLODActorEditorUnregisteredDelegate;
#endif
};
