...
{
	UE_DEPRECATED(5.8, "Replaced by TargetWorld to disambiguate vs BuildWorld")
	UWorld*	World;

	/** World for which HLODs are being built */
	UWorld* TargetWorld;

	/** Temporary world used to perform the HLOD build */
	UWorld* BuildWorld;
	...
	/** Flags to set for generated assets */
	EObjectFlags AssetsObjectFlags;
};
...
protected:
	...
	static inline TArray<TComponentClass*> FilterComponents(const TArray<UActorComponent*>& InSourceComponents)
	{
		...
		{
			UE_LOGF(LogHLODBuilder, Warning, "Excluding %d components from the HLOD build.", InSourceComponents.Num() - FilteredComponents.Num());
		}
