...
public:
	...
	virtual ~FWorldPartitionActorDescInstance() = default;
	
	DECLARE_MULTICAST_DELEGATE_OneParam(FActorDescUpdatedEvent, FWorldPartitionActorDescInstance*);
	UE_EXPERIMENTAL(5.8, "This new static delegate is experimental and the API is subject to change.")
	static ENGINE_API FActorDescUpdatedEvent OnActorDescUpdated;
	...
	virtual bool IsUsingDataLayerAsset() const { return true; }
...
protected:
	virtual FBox GetLocalEditorBounds(bool bInForceComputeOnChildContainerInstance = false) const;
...
private:
	/** Called by UActorDescContainerInstance::OnActorDescPreUpdate */
	void OnPreUpdate();
...
protected:
	...
	mutable TOptional<FBox>						ChildContainerLocalEditorBoundsCache;
#endif // WITH_EDITOR
};
