...
public:
	...
	virtual ~FStreamingGenerationActorDescView() = default;
	...
	ENGINE_API void SetIsSpatiallyLoaded(bool bValue);
	...
	ENGINE_API void SetIsHLODRelevant(bool bValue);
...
private:
	ENGINE_API FString ToString(EToStringMode ActorDescViewMode, FWorldPartitionActorDesc::EToStringMode ActorDescMode, TFunctionRef<bool(const TArray<FName>&, TArray<const UDataLayerInstance*>&)> GetRuntimeDataLayerInstancesFromNamesFunc) const;
	...
	TOptional<bool> bIsHLODRelevant;
...
struct FActorDescViewMutator
{
	...
	TOptional<bool> bIsHLODRelevant;
	TOptional<FSoftObjectPath> HLODLayer;
};
