...
public:
	...
	private:
		// Should return true if the context caches objects that can be released outside its control
		virtual bool IsDeferringRawObjects() { return false; };
		...
		virtual void CancelOperation(FWorldPartitionActorDescInstance* InActorDescInstance) = 0;
	};
	...
	private:
		...
		virtual void CancelOperation(FWorldPartitionActorDescInstance* InActorDescInstance) override {} // Immediate context, nothing to cancel
	};
	...
	private:
		virtual bool IsDeferringRawObjects() override { return true; };
		ENGINE_API virtual void RegisterActor(FWorldPartitionActorDescInstance* InActorDescInstance) override;
		ENGINE_API virtual void UnregisterActor(FWorldPartitionActorDescInstance* InActorDescInstance) override;
		ENGINE_API virtual void CancelOperation(FWorldPartitionActorDescInstance* InActorDescInstance) override;
		...
		TSet<FWorldPartitionActorDescInstance*> ContainerInstanceCancellations;
	...
	private:
		...
		virtual void CancelOperation(FWorldPartitionActorDescInstance* InActorDescInstance) override {}
	};

	// Returns true if the current context is vulnerable to deleted objects
	static bool IsDeferringRawObjects();
	// If the current context is operating on the actor desc, cancel it
	static void CancelOperationIfAny(FWorldPartitionActorDescInstance* InActorDescInstance);
