...
struct FRuntimePartitionDesc
{
    ...
    FRuntimePartitionDesc() = default;
    ~FRuntimePartitionDesc() = default;
    FRuntimePartitionDesc(const FRuntimePartitionDesc&) = default;
    FRuntimePartitionDesc(FRuntimePartitionDesc&&) = default;
    FRuntimePartitionDesc& operator=(const FRuntimePartitionDesc&) = default;
    FRuntimePartitionDesc& operator=(FRuntimePartitionDesc&&) = default;
	...
	/** Partition class */
	UE_DEPRECATED(5.8, "This property is replaced by MainLayer")
	UPROPERTY()
...
struct FRuntimePartitionStreamingData
{
	...
	bool HasSpatialIndex() const { return SpatialIndex.IsValid() || SpatialIndexForce2D.IsValid() || SpatialIndex2D.IsValid(); }
...
public:
	...
	ENGINE_API virtual TOptional<FFixedGridInfo> GetFixedGridInfo(FName InGridName) const override;
	ENGINE_API virtual void DumpStateLog(FHierarchicalLogArchive& Ar) const override;
	ENGINE_API virtual void ForEachHLODLayer(TFunctionRef<bool(FName, FName, int32)> Func) const override; // RuntimePartitionName, HLODSetupName, HLODSetupIndex
	ENGINE_API virtual int32 ComputeHLODHierarchyDepth() const override;
	ENGINE_API virtual FName ResolveRuntimeGridNameForHLODLayer(FName RuntimeGrid, const FSoftObjectPath& HLODLayerPath) const override;
	ENGINE_API virtual bool CopyGridSetupFrom(const UWorldPartitionRuntimeHash* Other) override;
#endif
