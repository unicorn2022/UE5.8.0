...
{
	class FDataLayerInstanceNamesTest;
}
...
#if WITH_EDITOR
	void Append(const FDataLayerInstanceNames& Other)
	{
		check(!HasExternalDataLayer() || !Other.HasExternalDataLayer() || (GetExternalDataLayer() == Other.GetExternalDataLayer()));

		if (!Other.HasExternalDataLayer())
		{
			DataLayers.Append(Other.GetNonExternalDataLayers());
		}
		else
		{
			TArrayView<const FName> ThisNonDataLayersView(GetNonExternalDataLayers());
			TArray<FName> ThisNonDataLayers(ThisNonDataLayersView.GetData(), ThisNonDataLayersView.Num());
			DataLayers = Other.ToArray();
			bIsFirstDataLayerExternal = Other.bIsFirstDataLayerExternal;
			DataLayers.Append(ThisNonDataLayers);
		}

		if (bIsForcedEmptyNonExternalDataLayers)
		{
			ForceEmptyNonExternalDataLayers();
		}

		// Remove potential duplicates from sorted non-external data layers
		GetNonExternalDataLayers().Sort(FNameFastLess());
		DataLayers.SetNum(Algo::Unique(DataLayers));
	}
#endif
...
private:
	...
	friend class WorldPartitionTests::FDataLayerInstanceNamesTest;
};