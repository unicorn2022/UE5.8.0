...
public:
	...
	template<class T>
	UE_DEPRECATED(5.8, "Use SetDataLayerRuntimeState instead.")
	...
	static FName GetWorldPartitionWorldDataLayersName() { return FName("WorldDataLayers"); } // reserved for ULevel::WorldDataLayers
...
private:
	...
	UE_DEPRECATED(5.8, "DeprecatedDataLayerNameToDataLayerInstance Property is no longer used.")
	UPROPERTY()
	TMap<FName, TWeakObjectPtr<UDataLayerInstance>> DeprecatedDataLayerNameToDataLayerInstance;

	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	UE_DEPRECATED(all, "Convert Data layers to DataLayerInstances and DataLayerAssets using DataLayerToAsset CommandLet and use DataLayerInstances instead.")
	UPROPERTY()
	TSet<TObjectPtr<class UDEPRECATED_DataLayer>> WorldDataLayers_DEPRECATED;
...
void AWorldDataLayers::OverwriteDataLayerRuntimeStates(const TArray<IdentifierType>* InActiveDataLayers, const TArray<IdentifierType>* InLoadedDataLayers)
{
	if (GetLocalRole() == ROLE_Authority)
	{
		...
		auto IdentifierToInstance = [this](const IdentifierType& Identifier) -> const UDataLayerInstance*
		{
			if constexpr (std::is_same_v<IdentifierType, TSoftObjectPtr<UDataLayerAsset>>)
			{
				return GetDataLayerInstance(Identifier.Get());
			}
			else
			{
				return GetDataLayerInstance(Identifier);
			}
		};
		...
		if (InActiveDataLayers)
		{
			...
			{
				const UDataLayerInstance* DataLayerInstance = IdentifierToInstance(DataLayerIdentitier);
