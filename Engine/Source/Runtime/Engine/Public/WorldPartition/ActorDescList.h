...
public:
	...
	virtual ~TActorDescList() = default;
...
void TActorDescList<DescType>::AddActorDescriptor(DescType* ActorDesc)
{
	...
	{
		UE_LOGF(LogWorldPartition, Error, "Duplicated actor descriptor detected:\n\tExisting: %ls\n\t     New: %ls", 
...
void TActorDescList<DescType>::RemoveActorDescriptor(DescType* ActorDesc)
{
	...
	if (!ActorsByGuid.Remove(ActorDesc->GetGuid()))
	{
		UE_LOGF(LogWorldPartition, Error, "Removing unknown actor descriptor: %ls", *ActorDesc->ToString(FWorldPartitionActorDesc::EToStringMode::Full));
	}
}
