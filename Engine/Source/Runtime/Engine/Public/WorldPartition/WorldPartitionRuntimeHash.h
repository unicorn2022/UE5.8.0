...
public:
	...
	ENGINE_API EDataLayerRuntimeState GetDataLayerEffectiveRuntimeState(FName DataLayerName) const;
#if WITH_EDITOR
	const FDynamicLoadingRangeScales& GetDynamicLoadingRangeScales() const { return DynamicLoadingRangeScales; }
#endif // WITH_EDITOR
...
#if WITH_EDITOR
	FDynamicLoadingRangeScales DynamicLoadingRangeScales;
#endif // WITH_EDITOR
...
class UWorldPartitionRuntimeHash : public UObject
{
	...
	virtual TOptional<FFixedGridInfo> GetFixedGridInfo(FName InGridName) const { return {}; }
	virtual void ForEachHLODLayer(TFunctionRef<bool(FName, FName, int32)> Func) const {}; // RuntimePartitionName, HLODSetupName, HLODSetupIndex
	virtual int32 ComputeHLODHierarchyDepth() const { return 0; }
	virtual FName ResolveRuntimeGridNameForHLODLayer(FName RuntimeGrid, const FSoftObjectPath& HLODLayerPath) const { return NAME_None; }
	virtual bool CopyGridSetupFrom(const UWorldPartitionRuntimeHash* Other) { return false; }
...
protected:
	...
	// Variable set in scoped guard to skip non spatials cells in repeated queries
	mutable bool bQueryNonSpatialCells = true;
