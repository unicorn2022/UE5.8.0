...
struct FWorldPartitionPerWorldSettings
{
	...
	void Reset()
	{
		...
		bForceDisableDynamicLoadingRangeScaling = false;
	}
#endif
	...
	/** Lets users disable the map's DynamicLoadingRangeScaling enable setting locally without having to checkout the map or risk submitting the setting */
	UPROPERTY()
	bool bForceDisableDynamicLoadingRangeScaling = false;
#endif
};
...
public:
	...
	UE_DEPRECATED(5.8, "Use GetWorldDataLayerAssetsNotLoadedInEditor instead.")
	TArray<const UDataLayerInstance*> GetWorldDataLayersNotLoadedInEditor(UWorld* InWorld) const { return TArray<const UDataLayerInstance*>(); }
	UE_DEPRECATED(5.8, "Use GetWorldDataLayerAssetsLoadedInEditor instead.")
	TArray<const UDataLayerInstance*> GetWorldDataLayersLoadedInEditor(UWorld* InWorld) const { return TArray<const UDataLayerInstance*>(); }
	UE_DEPRECATED(5.8, "Use GetWorldDataLayerAssetsNotLoadedInEditor instead.")
	TArray<UDataLayerInstance*> GetWorldDataLayersNotLoadedInEditor(UWorld* InWorld) { return TArray<UDataLayerInstance*>(); }
	UE_DEPRECATED(5.8, "Use GetWorldDataLayerAssetsLoadedInEditor instead.")
	TArray<UDataLayerInstance*> GetWorldDataLayersLoadedInEditor(UWorld* InWorld) { return TArray<UDataLayerInstance*>(); }

	ENGINE_API TArray<const UDataLayerAsset*> GetWorldDataLayerAssetsLoadedInEditor(UWorld* InWorld) const;
	ENGINE_API TArray<UDataLayerAsset*> GetWorldDataLayerAssetsLoadedInEditor(UWorld* InWorld);
	ENGINE_API TArray<const UDataLayerAsset*> GetWorldDataLayerAssetsNotLoadedInEditor(UWorld* InWorld) const;
	ENGINE_API TArray<UDataLayerAsset*> GetWorldDataLayerAssetsNotLoadedInEditor(UWorld* InWorld);
...
#if WITH_EDITORONLY_DATA
public:
	ENGINE_API bool GetForceDisableDynamicLoadingRangeScaling(UWorld* InWorld) const;
	ENGINE_API void SetForceDisableDynamicLoadingRangeScaling(UWorld* InWorld, bool bOverride);
