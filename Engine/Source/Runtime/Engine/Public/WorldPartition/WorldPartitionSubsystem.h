...
// Container for the different domains of loading range scales
struct FDynamicLoadingRangeScales
{
	// Increases linearly with time
	float LinearScale = 1.0f;
	// Square root of LinearScale
	float Scale2D = 1.0f;
	// Cubic root of LinearScale
	float Scale3D = 1.0f;
};
...
public:
	...
	// When enabled, returns a linear loading range scaling ramp to boost initial PIE availability, at the cost of popping. When disabled, the values are 1.f
	const FDynamicLoadingRangeScales& GetDynamicLoadingRangeScales() const { return LoadingRangeScales; }
...
private:
	...
	ENGINE_API void UpdateStreamingSourcesVelocity(const UWorldPartition* InWorldPartition, TArray<FWorldPartitionStreamingSource>& InOutStreamingSources) const;
...
#if WITH_EDITOR
	// If enabled, compute a linear loading range scaling ramp to boost initial PIE availability, at the cost of popping.
	void InitDynamicLoadingRangeScale();
	void UpdateDynamicLoadingRangeScale();

	float DynamicLoadingRangeScaleStartTime = -1.0f;
	FDynamicLoadingRangeScales LoadingRangeScales{1.f};
#endif // WITH_EDITOR
	...
	int32 LevelStreamingContinuouslyIncrementalGCWhileActorsPendingPurge;
