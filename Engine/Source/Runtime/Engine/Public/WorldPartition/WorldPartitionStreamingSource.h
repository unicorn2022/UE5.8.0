...
struct FWorldPartitionStreamingQuerySource
{
	...
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Query", meta = (EditCondition = "!bIncludeAnyDataLayer"))
	...
	/* If True, Include cells that have DataLayers without caring for a match in the DataLayers array. */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Query")
	bool bIncludeAnyDataLayer;
	

	/* If False, Location/Radius will not be used to find spatial cells. Non-spatial cells are always returned by the query, but can be further filtered by bDataLayersOnly. */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Query")
	bool bSpatialQuery;
