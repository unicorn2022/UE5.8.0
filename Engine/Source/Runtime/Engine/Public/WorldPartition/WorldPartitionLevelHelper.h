...
public:
	...
	struct FPackageReferencer
	{
		...
		ENGINE_API void AddReference(UPackage* InPackage);
		ENGINE_API void RemoveReferences();
	};
	...
	ENGINE_API static void MoveExternalActorsToLevel(const TArray<FWorldPartitionRuntimeCellObjectMapping>& InChildPackages, ULevel* InLevel, TArray<UPackage*>& OutModifiedPackages);
	...
	ENGINE_API static bool LoadActors(const FLoadActorsParams& InParams);
	ENGINE_API static bool LoadActors(FLoadActorsParams&& InParams);
