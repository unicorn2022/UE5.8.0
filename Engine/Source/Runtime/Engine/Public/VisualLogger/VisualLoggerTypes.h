...
#if (UE_DEBUG_RECORDING_ENABLED) || (UE_DEBUG_VISUALIZER_TOOL_ENABLED)
...
public:
	...
	virtual ~FVisualLogDevice() = default;
	...
	virtual void GetRecordedLogs(TArray<FVisualLogEntryItem>& OutLogs) const { /* Empty */ }
...
#if UE_DEBUG_VISUALIZER_TOOL_ENABLED || UE_DEBUG_RECORDING_USING_VLOG
namespace FVisualLoggerHelpers
{
	extern ENGINE_API FArchive& Serialize(FArchive& Ar, TArray<FVisualLogDevice::FVisualLogEntryItem>& RecordedLogs);
};
#endif
...
namespace FVisualLoggerHelpers
{
	extern ENGINE_API void GetCategories(const FVisualLogEntry& RecordedLogs, TArray<FVisualLoggerCategoryVerbosityPair>& OutCategories);
	extern ENGINE_API void GetHistogramCategories(const FVisualLogEntry& RecordedLogs, TMap<FString, TArray<FString>>& OutCategories);
};
...
{
public:
	virtual ~FVisualLogExtensionInterface() = default;
	...
	virtual void OnLogLineSelectionChanged(IVisualLoggerEditorInterface* EdInterface, TSharedPtr<FLogEntryItem> SelectedItem, int64 UserData) {};
...
namespace FVisualLoggerHelpers
{
	extern ENGINE_API FArchive& Serialize(FArchive& Ar, FName& Name);

#if UE_DEBUG_RECORDING_USING_VLOG
	extern ENGINE_API FString GenerateTemporaryFilename(const FString& FileExt);
	extern ENGINE_API FString GenerateFilename(const FString& TempFileName, const FString& Prefix, double StartRecordingTime, double EndTimeStamp);
...
#endif // UE_DEBUG_RECORDING_ENABLED || UE_DEBUG_VISUALIZER_TOOL_ENABLED