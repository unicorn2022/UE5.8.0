...
public:
	...
	uint64 AddOrTouchChunk(const FChunkKey& InKey, const TSharedRef<const FSoundWaveData>& InSoundWavePtr, TFunction<void(EAudioChunkLoadResult) > OnLoadCompleted, ENamedThreads::Type CallbackThread, bool bNeededForPlayback);
	...
	TArrayView<uint8> GetChunk(const FChunkKey& InKey, const TSharedRef<const FSoundWaveData>& InSoundWavePtr, bool bBlockForLoadCompletion, bool bNeededForPlayback, uint64& InOutCacheLookupID);
	...
	void AddForceInlineSoundWave(const TSharedRef<const FSoundWaveData>& InSoundWaveData); 
	
	UE_DEPRECATED(5.8, "Please use the TSharedRef<const FSoundWaveData>& overload instead")
	void AddForceInlineSoundWave(const FSoundWaveProxyPtr& InSoundWave)
	{
		if (ensure(InSoundWave.IsValid()))
		{
			AddForceInlineSoundWave(InSoundWave->GetSoundWaveDataRef());	
		}
	}

	void RemoveForceInlineSoundWave(const TSharedRef<const FSoundWaveData>& InSoundWaveData);
	
	UE_DEPRECATED(5.8, "Please use the TSharedRef<const FSoundWaveData>& overload instead")
	void RemoveForceInlineSoundWave(const FSoundWaveProxyPtr& InSoundWave)
	{
		if (ensure (InSoundWave.IsValid()))
		{
			RemoveForceInlineSoundWave(InSoundWave->GetSoundWaveDataRef());
		}
	}
	...
	void HandleStarvation(const TSharedRef<const FSoundWaveData>& InSoundWaveData, const uint32 ChunkIndex, const uint32 InBytesProduced, const uint32 InBytesExpected);
...
private:
	...
	struct FCacheElement
	{
		...
		TWeakPtr<const FSoundWaveData, ESPMode::ThreadSafe> SoundWaveWeakPtr;
		...
		~FCacheElement()
		{
			...
			if(NumConsumers.GetValue() != 0)
			{
				...
				UE_LOGF(LogAudioStreamCaching, Error, "Tried to destroy streaming cache while the cached data was in use! (Sound: %ls - Loading Behavior: %ls - Chunk Index: %i)"
	...
	TMap<const FSoundWaveData*, FSoundWaveMemoryTracker> SoundWaveTracker;
	...
	FCacheElement* InsertChunk(const FChunkKey& InKey, const TSharedRef<const FSoundWaveData>& InSoundWaveWeakPtr);
...
public:
	...
	virtual void AddForceInlineSoundWave(const TSharedRef<const FSoundWaveData>& SoundWaveData) override;
	virtual void RemoveForceInlineSoundWave(const TSharedRef<const FSoundWaveData>& SoundWaveData) override;
	virtual bool RequestChunk(const TSharedRef<const FSoundWaveData>& SoundWaveData, uint32 ChunkIndex, TFunction<void(EAudioChunkLoadResult)> OnLoadCompleted, ENamedThreads::Type ThreadToCallOnLoadCompletedOn, bool bForImmediatePlayback = false) override;
	virtual FAudioChunkHandle GetLoadedChunk(const TSharedRef<const FSoundWaveData>& SoundWaveData, uint32 ChunkIndex, bool bBlockForLoad = false, bool bForImmediatePlayback = false) const override;
	 ...
	 * @param InSoundWaveData Wave data
	 * @param InCurrentChunkIndex Current index to move forwards of
	 * @return Chunk Index or INDEX_NONE if there is only one chunk 
	 */
	static int32 GetNextChunkIndex(const TSharedRef<const FSoundWaveData>& InSoundWaveData, const uint32 InCurrentChunkIndex);
	...
	static int32 GetNextChunkIndex(const FSoundWaveProxyPtr& InSoundWave, const uint32 InCurrentChunkIndex)
	{
		if (ensure(InSoundWave.IsValid()))
		{
			return GetNextChunkIndex(InSoundWave->GetSoundWaveDataRef(), InCurrentChunkIndex);
		}
...
protected:
	...
	virtual void HandleStarvation(const TSharedRef<const FSoundWaveData>& SoundWaveData, const uint32 ChunkIndex, const uint32 InBytesProduced, const uint32 InBytesExpected) override;
	 ...
	 */
	FAudioChunkCache* GetCacheForWave(const TSharedRef<const FSoundWaveData>& InSoundWave) const;
	...
	FAudioChunkCache* GetCacheForWave(const FSoundWaveProxyPtr& InSoundWave) const
	{
		if (ensure(InSoundWave.IsValid()))
		{
			return GetCacheForWave(InSoundWave->GetSoundWaveDataRef());
		}
