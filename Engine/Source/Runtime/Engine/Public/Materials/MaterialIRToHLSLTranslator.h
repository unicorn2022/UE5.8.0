...
struct FMaterialIRToHLSLTranslation
{
	...
	// Executes the translation process, filling the material shader template parameters in OutShaderParameters and configuring OutEnvironment for shader compilation.
	// After this call, interpolate the MaterialTemplate shader with the list of parameters to get the final HLSL shader.
	void Run(TMap<FString, FString>& OutShaderParameters, FShaderCompilerEnvironment& OutEnvironment);
};
