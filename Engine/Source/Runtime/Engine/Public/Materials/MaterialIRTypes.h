...
enum class ETypeKind : uint8
{
	...
	Texture2D,				// A 2D texture object.
	TextureCube,			// A cube texture object.
	Texture2DArray,			// A 2D texture array object.
	TextureCubeArray,		// A cube texture array object.
	Texture3D,				// A 3D (volume) texture object.
	TextureExternal,		// An external texture
	VirtualTexture,			// A virtual texture object (UTexture - MCT_TextureVirtual).
	RuntimeVirtualTexture,	// A runtime virtual texture object (URuntimeVirtualTexture).
...
// Returns the larger scalar kind between A and B.
static inline constexpr EScalarKind GetCommonScalarKind(EScalarKind A, EScalarKind B)
{
...
struct FPrimitive
{
	...
	bool IsBoolean() const { return ScalarKind == EScalarKind::Boolean; }
	...
	bool IsInteger() const { return ScalarKind == EScalarKind::Integer; }
...
struct FType
{
	...
	static FType MakeBoolScalar() { return MakeScalar(EScalarKind::Boolean); }

	// Returns a scalar type representing a integer value.
	static FType MakeIntScalar() { return MakeScalar(EScalarKind::Integer); }
	...
	static FType MakeBoolVector(int NumColumns) { return MakeVector(EScalarKind::Boolean, NumColumns); }

	// Returns a row-major vector type of integer values with the specified number of columns.
	static FType MakeIntVector(int NumColumns) { return MakeVector(EScalarKind::Integer, NumColumns); }
	...
	static FType MakeInt(int NumRows, int NumColumns) { return MakePrimitive(EScalarKind::Integer, NumRows, NumColumns); }
	...
	// Returns the Texture2D object type.
	static FType MakeTexture2D();

	// Returns the TextureCube object type.
	static FType MakeTextureCube();

	// Returns the Texture2DArray object type.
	static FType MakeTexture2DArray();

	// Returns the TextureCubeArray object type.
	static FType MakeTextureCubeArray();

	// Returns the Texture3D object type.
	static FType MakeTexture3D();

	// Returns the texture external object type.
	static FType MakeTextureExternal();

	// Returns the VirtualTexture object type (for MCT_TextureVirtual, backed by UTexture).
	static FType MakeVirtualTexture();
	...
	bool IsVector(int32 NumComponents) const { return Is(ETypeKind::Primitive) && Primitive.IsRowVector() && Primitive.NumComponents() == NumComponents; }
	...
	// Returns whether this type is any texture object stored in FTextureUniform (includes virtual textures, excludes RuntimeVirtualTexture).
	bool IsTexture() const { return Kind == ETypeKind::Texture2D || Kind == ETypeKind::TextureCube || Kind == ETypeKind::Texture2DArray || Kind == ETypeKind::TextureCubeArray || Kind == ETypeKind::Texture3D || Kind == ETypeKind::VirtualTexture || Kind == ETypeKind::TextureExternal; }

	// Returns whether this type is a VirtualTexture object (MCT_TextureVirtual, backed by UTexture).
	bool IsVirtualTexture() const { return Kind == ETypeKind::VirtualTexture; }
	...
	friend uint32 GetTypeHash(const FType& T);
