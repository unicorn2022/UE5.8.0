...
public:
	...
	static ENGINE_API void AddCustomAttribute(const FGuid& AttributeID, UClass* ExpressionClass,
		const FString& AttributeName, const FString& FunctionName, int32 OutputIndex, EMaterialValueType ValueType,
		const FVector4& DefaultValue, EShaderFrequency ShaderFrequency, bool bNeedsPreviousFrameEvaluation = false,
	 ...
	 * This function is not threadsafe, do not use. Use GetCustomAttributeCopy instead.
	 * TODO: Deprecate this function.
	 */
	static ENGINE_API const FMaterialCustomOutputAttributeDefintion* GetCustomAttribute(const FString& AttributeName);
	/** Returns the first custom attribute definition that has the specificed attribute name. */
	static ENGINE_API TOptional<FMaterialCustomOutputAttributeDefintion> GetCustomAttributeCopy(const FString& AttributeName);
	...
	/** Returns the first custom attribute definition that has the specificed attribute AttributeID. */
	static ENGINE_API TOptional<FMaterialCustomOutputAttributeDefintion> GetCustomAttributeCopy(const FGuid& AttributeID);
	...
	/**
	 * This function is not threadsafe, do not use. Use GetReadLockedOrderedVisibleAttributeList instead.
	 * TODO: Deprecate this function.
	 */
	static ENGINE_API const TArray<FGuid>& GetOrderedVisibleAttributeList();
	
	struct FReadLockedOrderVisibleAttributeList : public FNoncopyable
	{
	public:
		ENGINE_API ~FReadLockedOrderVisibleAttributeList();
		const TArray<FGuid>& List;
	private:
		FReadLockedOrderVisibleAttributeList();
		FReadLockedOrderVisibleAttributeList(FReadLockedOrderVisibleAttributeList&&) = delete;
		FReadLockedOrderVisibleAttributeList& operator=(FReadLockedOrderVisibleAttributeList&&) = delete;
		friend FMaterialAttributeDefinitionMap;
	};
	static ENGINE_API FReadLockedOrderVisibleAttributeList GetReadLockedOrderedVisibleAttributeList();
...
private:
	...
	void AddCustomAttributeInner(const FGuid& AttributeID, UClass* ExpressionClass, const FString& AttributeName,
		const FString& FunctionName, int32 OutputIndex, EMaterialValueType ValueType, const FVector4& DefaultValue, 
		EShaderFrequency ShaderFrequency, bool bNeedsPreviousFrameEvaluation = false,
		MaterialAttributeBlendFunction BlendFunction = nullptr);
	...
	static FText GetAttributeOverrideForMaterialInLock(const FGuid& AttributeID, UMaterial* Material);
	static FString GetPinNameFromShadingModelFieldInLock(FMaterialShadingModelField InShadingModels,
	...
	static uint64 GetConnectedMaterialAttributesBitmask_Internal(const ExpressionsArrayType& Expressions);
#endif

	// Functions that we call internally from other functions on the class, the caller guarantees that the readlock
	// is already held.
	static int32 GetIndexInLock(EMaterialProperty Property);
	static int32 GetIndexInLock(const FGuid& AttributeID);
	static uint64 GetBitmaskInLock(EMaterialProperty Property);
	static uint64 GetBitmaskInLock(const FGuid& AttributeID);
