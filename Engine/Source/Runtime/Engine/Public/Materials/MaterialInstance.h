...
class UMaterialInstance : public UMaterialInterface
{
	...
	/** Defines ToonProfile override used by this instance. If not set, it uses the parent ones. */
	UPROPERTY()
	TObjectPtr<class UToonProfile> ToonProfileOverride;
	...
	/** Defines if ToonProfile from this instance is used or it uses the parent one. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = MaterialInstance)
	uint8 bOverrideToonProfile:1;
	...
	/** Defines if PhysMaterial from this instance is used or if the parent one is used. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = MaterialInstance)
	uint8 bOverridePhysMaterial : 1;
...
public:
	...
	UE_DEPRECATED(5.8, "Exposed public Instance Render Proxy derivative has been moved to protected. Please use either GetRenderProxy or GetInstanceRenderProxy.")
	class FMaterialInstanceRenderProxy* Resource;
	...
	ENGINE_API virtual bool CalculateTexturesSamplingInfo(UTexture* Texture, FTextureSamplingInfo& OutTextureSamplingInfo) const override;
...
protected:
	 ...
	 */
	class FMaterialInstanceRenderProxy* RenderProxy;
...
public:
	...
	virtual ENGINE_API uint32 GetUsageFlags() const override;
	virtual ENGINE_API bool GetUsageByFlag(const EMaterialUsage Usage) const override;
	virtual ENGINE_API bool SetMaterialUsage(EMaterialUsage Usage) override;
	...
	inline FMaterialInstanceRenderProxy* GetInstanceRenderProxy() const { return RenderProxy; }
	...
	ENGINE_API virtual bool IsLightFunctionMaterial() const override;
	...
	ENGINE_API virtual UToonProfile* GetToonProfileOverride_Internal() const override;
	...
	/** Updates all default textures of this material instance, based on permutations and Parent Material. */
	ENGINE_API void UpdateReferencedDefaultTextures();

	/**
	* Set just the permutation parameters, without triggering any shader compilation.
	* This is useful for operations which require the Material Instance to have the right data
	* but it will be invalid for rendering until UpdateStaticPermutation() is called.
	*/
	ENGINE_API void SetPermutationParameters(const FStaticParameterSet& InParameters, FMaterialInstanceBasePropertyOverrides& InBasePropertyOverrides);
	...
	*/
	...
	ENGINE_API void SetStaticComponentMaskParameterValueEditorOnly(const FMaterialParameterInfo& ParameterInfo, FStaticComponentMaskValue Value);
#endif // WITH_EDITORONLY_DATA
	...
	UE_DEPRECATED(5.8, "Use parameterless version returning a uint64; SHA1 hashes are no longer used")
	inline void GetBasePropertyOverridesHash(FSHAHash& OutHash) const
	{
		uint64 NewHash = GetBasePropertyOverridesHash();
		OutHash = FSHA1::HashBuffer(&NewHash, sizeof(NewHash));
	}
	ENGINE_API uint64 GetBasePropertyOverridesHash() const;
	...
	/**
	 * Looks through this Instance and all its parents for the relevant texture.
...
protected:
	...
	*/
	...
	/** Directly set the usage flags. Should only be called from within SetMaterialUsage(). */
	void SetUsageByFlag(EMaterialUsage Usage, bool NewValue);

	ENGINE_API bool SetParentInternal(class UMaterialInterface* NewParent, bool RecacheShaders);

	/** Returns the textures which are used by the Uniform Expressions of the given Material Resource, along with any existing transient overrides. */
	ENGINE_API virtual void GetTextureExpressionValues(const FMaterialResource* MaterialResource, TArray<UTexture*>& OutTextures, TArray<TArray<int32>>* OutIndices = nullptr, bool bIncludeTextureCollections = false, bool bAllowOverrides = false) const override;
	...
	*/
	...
	friend class FMaterialInstanceRenderProxy;
