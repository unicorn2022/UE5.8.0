...
enum EMaterialSubSurfaceType : int
{
	...
	MSS_TransmittanceShadow	= SSS_TYPE_TRANSMITTANCE_SHADOW	UMETA(Hidden),	// Only used from shaders as of today and might be refactor so not added here explicitely in UI.
	MSS_MAX
};
...
UE_DEPRECATED(5.8, "CompileSubstrateBlendFunction is deprecated and will be removed. Material attributes are no longer allowed to flow SubstrateData.")
...
class UMaterialExpressionSubstrateShadingModels : public UMaterialExpressionSubstrateBSDF
{
	...
	* The single scattering Albedo defining the overall color of the Material (type = float3, unit = unitless, default = 0)
	...
	UE_DEPRECATED(5.8, "Please, SubstrateGenerateMaterialTopologyTreeCommon taking the FSubstrateTranslatorData parameter.")
	...
	static MIR::FValueRef BuildCommon(MIR::FEmitter& Em, UMaterial* Material,
		FExpressionInput& BaseColor, FExpressionInput& Specular, FExpressionInput& Metallic, FExpressionInput& Roughness, FExpressionInput& EmissiveColor,
		FExpressionInput& Opacity, FExpressionInput& SubSurfaceColor, FExpressionInput& ClearCoat, FExpressionInput& ClearCoatRoughness,
		FExpressionInput& ShadingModel, TEnumAsByte<enum EMaterialShadingModel> ShadingModelOverride,
		FExpressionInput& TransmittanceColor, FExpressionInput& ThinTranslucentSurfaceCoverage,
...
UCLASS(MinimalAPI, collapsecategories, hidecategories = Object, DisplayName = "Substrate Toon BSDF", meta = (DevelopmentStatus = "Experimental"))
class UMaterialExpressionSubstrateToonBSDF : public UMaterialExpressionSubstrateBSDF
{
	GENERATED_UCLASS_BODY()

	/**
	 * Surface base color. (type = float3, unit = unitless, defaults to 0.18 grey)
	 */
	UPROPERTY()
	FExpressionInput BaseColor;

	/**
	 * Surface metallic value to transition from a dielectric to a conductor. (type = float, unit = unitless, defaults to 0.0)
	 */
	UPROPERTY()
	FExpressionInput Metallic;

	/**
	 * Surface specular value. (type = float, unit = unitless, defaults to 0.5)
	 */
	UPROPERTY()
	FExpressionInput Specular;
	...
	/**
	 * Emissive color on top of the surface (type = float3, unit = luminance, defaults to black)
	 */
	UPROPERTY()
	FExpressionInput EmissiveColor;

	/**
	 * UVs used to render the diffuse, specular or any other artistic pattern. It can only store data in [0,1].
	 */
	UPROPERTY()
	FExpressionInput PatternUVs;
	...
	*/
	...
	/** Toon profile used to control the appearance of this BSDF. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = Material, meta = (DisplayName = "Toon Profile"))
	TObjectPtr<class UToonProfile> ToonProfile;
...
class UMaterialExpressionSubstrateSelect : public UMaterialExpressionSubstrateBSDF
{
	...
	/**
	 * If <= Threshold, A is selected, otherwise B is selected. (Default: 0))
	 */
	UPROPERTY()
