...
public:
	...
	virtual ~ULevelInstancePropertyOverrideAsset() = default;
		
#if WITH_EDITOR
	const TSoftObjectPtr<UWorld>& GetWorldAsset() const { return WorldAsset; }

	// Remove the container path from overrides.
	void RemoveContainerOverride(const FActorContainerPath& InContainerPath) { PropertyOverridesPerContainer.Remove(InContainerPath); }

	// Remove ActorOverrides under InContainerPath whose actor GUIDs are not present in InActorDescs.
	// If pruning leaves no remaining ActorOverrides at the path, the container entry itself is removed.
	void PruneStaleActorOverrides(const FActorContainerPath& InContainerPath, const TMap<FGuid, TSharedPtr<class FWorldPartitionActorDesc>>& InActorDescs);
...
private:
	...
	static bool SerializeActorPropertyOverrides(ULevelStreamingLevelInstanceEditorPropertyOverride* InLevelStreaming, AActor* InActor, bool bForReset, FActorPropertyOverride& OutActorPropertyOverrides, const TMap<UObject*, UObject*>* InInstanceToArchetypeMap = nullptr, const TMap<UObject*, UObject*>* InArchetypeToInstanceMap = nullptr);
