...
{
public:
	virtual ~ILevelInstanceEditorModule() = default;
	...
	/** Broadcasts when the state of a LI being edited changes */
	enum class ELevelInstanceEditMode : uint8
	{
		None,     /** The level instance is not being edited */
		Original, /** The level instance's original is being edited */
		Override  /** A level instance override is being edited */
	};
	DECLARE_MULTICAST_DELEGATE_TwoParams(FOnLevelInstanceEditModeChanged, ILevelInstanceInterface* /*LevelInstance*/, ELevelInstanceEditMode /*NewMode*/);
	virtual FOnLevelInstanceEditModeChanged& OnLevelInstanceEditModeChanged() = 0;
	virtual void BroadcastOnLevelInstanceEditModeChanged(ILevelInstanceInterface* LevelInstance, ELevelInstanceEditMode NewMode) = 0;
