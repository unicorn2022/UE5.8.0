...
USTRUCT()
struct FParameterAssetPathsEntry
{
	GENERATED_USTRUCT_BODY()

	UPROPERTY()
	TMap<FString, int32> AssetPaths;
...
struct FMaterialCachedExpressionEditorOnlyData
{
	...
	UPROPERTY()
	TMap<FName, FParameterAssetPathsEntry> AssetPathsMap;
	...
	/** All the default textures present in the nodes of the material this represents. */
	UPROPERTY()
	TArray<TObjectPtr<UObject>> EditorReferencedDefaultTextures;
};
...
struct FMaterialCachedExpressionData
{
	...
	void UpdatePropertyConnections(UMaterial& Material);
	...
	void AnalyzeMaterial(UMaterial& Material, const FMaterialLayersFunctions* LayersOverrides = nullptr);
	...
	void AddParameter(const FMaterialParameterInfo& ParameterInfo, const FMaterialParameterMetadata& ParameterMeta, const FString& InAssetPath ,UObject*& OutReferencedTexture, UTextureCollection*& OutReferencedTextureCollection, UMaterialExpression* InExpression);

	void AddParameter(const FMaterialParameterInfo& ParameterInfo, const FMaterialParameterMetadata& ParameterMeta, const FString& InAssetPath, UObject*& OutReferencedTexture, UMaterialExpression* InExpression)
	{
		UTextureCollection* UnusedReferencedTextureCollection = nullptr;
		return AddParameter(ParameterInfo, ParameterMeta, InAssetPath, OutReferencedTexture, UnusedReferencedTextureCollection, InExpression);
	}
#endif // WITH_EDITOR
...
#if WITH_EDITORONLY_DATA
	/** @return The index at which the given texture sits in the default textures array or INDEX_NONE if not present. */
	int32 GetDefaultTextureIdx(UObject* InTexture) const;
#endif
...
private:
#if WITH_EDITOR
	void DoPostAnalyzeChecks(const UMaterial& BaseMat);
#endif
	bool CheckDuplicateParameterValuesEqual(const int32 InExistingParamIdx, const FMaterialParameterMetadata& DuplicateParameterMeta);
	FString GetParameterValueAsText(const FMaterialParameterMetadata& ParameterMeta);
	void InsertParameterValue(const FMaterialParameterMetadata& ParameterMeta, const int32 Index, UObject*& OutReferencedTexture, UTextureCollection*& OutReferencedTextureCollection);
...
public:
...
#if WITH_EDITOR
	struct FParamExpressionsValues
	{
		TArray<TPair<TObjectPtr<class UMaterialExpression>, FString>> ExpressionsValues;
		bool bHasDuplicatesWithDifferentValues = false;
	};
	/** Stores Parameters to expresions and string values. Can be used for error logging duplicates.*/
	TMap<FName, FParamExpressionsValues> ParameterExpressionsAndValues;
	...
	struct FParsedExpressionData
	{
		...
		int32 ParameterIdx = INDEX_NONE;
		FName ParameterName;
		
		// This could theoretically be gathered from Expression 
		// but being in a Material Function can override values
		FMaterialParameterMetadata ParameterMeta;
	};

	// Used for any post analyze checks
	// Should be cleared after AnalyzeMaterial
	TArray<FParsedExpressionData> ParsedExpressions;
#endif
};
