...
public:
	...
	/**
	 * Allows subsystems to modify SoundParseParameters just before Sound.Parse checks them to populate WaveInstances.
	 * 
	 * Modifying Transform is not recommended. It is used for calculations before this function is called,
	 * which will not be able to observe any modifications done here.
	 * 
	 * Modifying SoundClass is allowed but not compatible with use of FAudioDevice::SetSoundClassDistanceScale.
	 * The scale value will always be taken from the old class, never the new one.
	 * 
	 * NOTE! Called on the AudioThread during each audio frame Update
	 *
	 * @param ActiveSound	The active sound affected
	 * @param ParseParams	The parameters to apply to the wave instances
	 */
	virtual void SubsystemsAdjustParseParams(const FActiveSound& ActiveSound, FSoundParseParameters& ParseParams) {}
