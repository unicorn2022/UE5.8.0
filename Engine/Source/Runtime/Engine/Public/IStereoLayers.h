...
public:
	...
	struct FLayerDesc
	{
		...
		bool HasValidTexture() const { return (TextureObj.IsValid() && TextureObj->GetResource() != nullptr); }
		...
		template <typename T> 
		[[nodiscard]] bool HasShape() const { return ensure(Shape.IsValid()) && Shape.Wrapped->GetShapeName() == T::ShapeName; }
