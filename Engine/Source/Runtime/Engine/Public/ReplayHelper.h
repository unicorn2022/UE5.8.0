	...
	struct FCheckpointStepHelper
	{
		...
		~FCheckpointStepHelper()
		{
			...
			case ECheckpointSaveState::SerializeGuidCache:
				...
				break;
			default:
				ensureMsgf(false, TEXT("FCheckpointStepHelper: Unsupported checkpoint state: %d"), EnumToUnderlyingType(CheckpointState));
				break;
			}

			UE_LOGF(LogDemo, Verbose, "Checkpoint. %ls: %i/%i, took %.2fms (Total this frame: %.2fms)", StateStr, *CurrentIndex, TotalCount, StepTimeInMS, TotalTimeInMS);
		}
