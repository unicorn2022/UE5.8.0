...
public:
	...
	*/
	 ...
	 * @param bRenderThreadStopped if true, skips waiting on the render thread event and pumps commands inline immediately.
	 */
	virtual void FlushAudioRenderingCommands(bool bPumpSynchronously = false, bool bRenderThreadStopped = false) {}
	 ...
	 */
	ENGINE_API virtual void Precache(USoundWave* SoundWave, bool bSynchronous = false, bool bTrackMemory = false, bool bForceFullDecompression = false);
	...
	*/
	...
	/**
	 * Allows subsystems to modify ParseParams before Sound.Parse begins
	 */
	ENGINE_API void SubsystemsAdjustParseParams(FActiveSound& ActiveSound, FSoundParseParameters& ParseParams) const;
...
public:
	...
	*/
	...
	{
		UE_LOGF(LogAudio, Error, "Submixes are only supported in audio mixer.");
	}
	...
	{
		UE_LOGF(LogAudio, Error, "Submix recording only works with the audio mixer. Please run using -audiomixer to or set INI file use submix recording.");
	}
	...
	{
		UE_LOGF(LogAudio, Error, "Submixes are only supported in audio mixer & 'UpdateSubmixModulationSettings' function taking single modulators no longer supported.");
	}
...
protected:
	...
	static constexpr float SoundIsAlivePingWaitTimeSec = 1.0f;
	float SoundIsAlivePingTimeSec = 0.0f;
