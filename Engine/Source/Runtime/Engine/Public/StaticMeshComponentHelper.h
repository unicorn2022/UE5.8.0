...
template<typename T>
concept HasOverridesMaterialsMember = requires(T t)
{
	t.OverrideMaterials;
};
...
public:
	...
	template<class T>
	static TConstArrayView<TObjectPtr<UMaterialInterface>> GetOverrideMaterials(const T& Component)
	{
		if constexpr (HasOverridesMaterialsMember<std::remove_reference_t<T>>)
		{
			return Component.OverrideMaterials;
		}
		else
		{
			return Component.GetOverrideMaterials();
		}
	}
};
...
void FStaticMeshComponentHelper::GetUsedRayTracingOnlyMaterials(const T& Component, TArray<UMaterialInterface*>& OutMaterials)
{
	...
	const auto& OverrideMaterials = GetOverrideMaterials(Component);

	if (UStaticMesh* StaticMesh = Component.GetStaticMesh())
	{
		StaticMesh->GetUsedMaterials(RayTracingMaterials, [&Component, &OverrideMaterials](int32 MaterialIndex)
		{
...
void FStaticMeshComponentHelper::CollectPSOPrecacheDataImpl(const T& Component, const FVertexFactoryType* VFType, const FPSOPrecacheParams& BasePrecachePSOParams, FStaticMeshComponentHelper::GetPSOVertexElementsFn GetVertexElements, FMaterialInterfacePSOPrecacheParamsList& OutParams)
{
	...
	for (int32 LODIndex = MeshMinLOD; LODIndex < LODResources.Num(); ++LODIndex)
	{
		...
		for (FStaticMeshSection& RenderSection : LODRenderData.Sections)
		{
			...
			int16 MaterialIndex = (int16)RenderSection.MaterialIndex;
			...
			if (VFsPerMaterial == nullptr)
			{
				...
				VFsPerMaterial->MaterialIndex = (int16)RenderSection.MaterialIndex;
			}
	...
	PrecachePSOParams.bHasWPODistance = Component.GetWorldPositionOffsetDisableDistance() > 0;
	PrecachePSOParams.bHasPixelDistance = Component.GetNanitePixelProgrammableDistance() > 0;
	PrecachePSOParams.bRequireRTDynamicGeometryUpdate = Component.EvaluateWorldPositionOffsetInRayTracing();
...
void FStaticMeshComponentHelper::CollectPSOPrecacheData(const T& Component, const FPSOPrecacheParams& BasePrecachePSOParams, FMaterialInterfacePSOPrecacheParamsList& OutParams)
{
	...
	if (Nanite::FNaniteResourcesHelper::ShouldCreateNaniteProxy(Component, &NaniteMaterials))
	{
		...
		// If bRequireRTDynamicGeometryUpdate is set then add another entry with local vertex factory because nanite can't be used for dynamic RT geometry updates
		FMaterialInterfacePSOPrecacheParamsList RTDynamicGeometryUpdateParams;
		for (FMaterialInterfacePSOPrecacheParams& Params : OutParams)
		{
			if (Params.PSOPrecacheParams.bRequireRTDynamicGeometryUpdate)
			{
				// Add new entry with local vertex factory and disabling rendering to all passes to reduce number of PSOs to compile
				FMaterialInterfacePSOPrecacheParams NewEntry = Params;
				NewEntry.VertexFactoryDataList.Empty(1);
				NewEntry.VertexFactoryDataList.Add(FPSOPrecacheVertexFactoryData(&FLocalVertexFactory::StaticType));
				NewEntry.PSOPrecacheParams.bRenderInMainPass = false;
				NewEntry.PSOPrecacheParams.bRenderInDepthPass = false;
				NewEntry.PSOPrecacheParams.bCastShadow = false;
				NewEntry.PSOPrecacheParams.bRenderCustomDepth = false;
				RTDynamicGeometryUpdateParams.Add(NewEntry);

				// Nanite VF entry doesn't require RT dynamic geometry update anymore
				Params.PSOPrecacheParams.bRequireRTDynamicGeometryUpdate = false;
			}
		}
		OutParams.Append(RTDynamicGeometryUpdateParams);
	}
	else
	{
	...
	if (!Scale3D.IsZero())
	{
		if (const UStaticMesh* Mesh = Component.GetStaticMesh())
		{
...
#endif // WITH_EDITOR
			{
				...
				{
					// Call GetNavigationModifier for obstacles or when surface area class is used.
					if (Component.ShouldExportAsObstacle(*NavCollision) || NavCollision->HasSurfaceAreaClass())
					{
...
FPrimitiveSceneProxy* FStaticMeshComponentHelper::CreateSceneProxy(T& Component, FStaticMeshComponentHelper::ESceneProxyCreationError* OutError)
{
	...
	if constexpr (!bAssumeRenderDataIsReady)
	{
		...
		{
			UE_LOGF(LogStaticMesh, Verbose, "Skipping CreateSceneProxy for StaticMeshComponent %ls (StaticMesh is null)", *UObjectHelper::GetFullNameIfAvailable(Component));
		...
		{
			UE_LOGF(LogStaticMesh, Verbose, "Skipping CreateSceneProxy for StaticMeshComponent %ls (StaticMesh is not ready)", *UObjectHelper::GetFullNameIfAvailable(Component));
		...
		{
			UE_LOGF(LogStaticMesh, Verbose, "Skipping CreateSceneProxy for StaticMeshComponent %ls (RenderData is null)", *UObjectHelper::GetFullNameIfAvailable(Component));
			SetError(ESceneProxyCreationError::InvalidMesh);
			return nullptr;
		}
		...
		{
			UE_LOGF(LogStaticMesh, Verbose, "Skipping CreateSceneProxy for StaticMeshComponent %ls (RenderData is not initialized)", *UObjectHelper::GetFullNameIfAvailable(Component));
			SetError(ESceneProxyCreationError::InvalidMesh);
			return nullptr;
		}
	}
	...
	{
		UE_LOGF(LogStaticMesh, Verbose, "Skipping CreateSceneProxy for StaticMeshComponent %ls (Static mesh component PSOs are still compiling)", *UObjectHelper::GetFullNameIfAvailable(Component));
	...
	if (Component.HasValidNaniteData())
	{
		...
		if (!bFallbackGenerated)
		{
			...
			UE_LOGF(LogStaticMesh, Warning, "Unable to create a proxy for StaticMeshComponent [%ls] because it doesn't have a fallback mesh.", *UObjectHelper::GetFullNameIfAvailable(Component));
			SetError(ESceneProxyCreationError::InvalidMesh);
			return nullptr;
		}
	}
	...
	{
		UE_LOGF(LogStaticMesh, Verbose, "Skipping CreateSceneProxy for StaticMeshComponent %ls (LOD problems)", *UObjectHelper::GetFullNameIfAvailable(Component));
		SetError(ESceneProxyCreationError::InvalidMesh);
		return nullptr;
	}
