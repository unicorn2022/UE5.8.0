...
enum class EPrimitiveDirtyRayTracingState
{
	None = 0,
...
ENUM_CLASS_FLAGS(EPrimitiveDirtyRayTracingState);
...
public:
	...
	*/
	...
	/**
	 * Adds new lights to the scene
	 *
	 * @param InLights - lights to add
	 */
	virtual void BatchAddLights(TArrayView<FLightSceneDesc> InLights) = 0;

	/**
	 * Removes lights from the scene
	 *
	 * @param InLights - lights to remove
	 */
	virtual void BatchRemoveLights(TArrayView<FLightSceneDesc> InLights) = 0;

	/**
	 * Adds new invisible lights to the scene
	 *
	 * @param InLights - lights to add
	 */
	virtual void BatchAddInvisibleLights(TArrayView<FLightSceneDesc> InLights) = 0;
	...
	*/
	 ...
	 * NOTE: prefer BuildPrimitiveUniformShaderParameters_RenderThread since it is more efficient and general.
	 */
	virtual void GetPrimitiveUniformShaderParameters_RenderThread(const FPrimitiveSceneInfo* PrimitiveSceneInfo, bool& bHasPrecomputedVolumetricLightmap, FMatrix& PreviousLocalToWorld, int32& SingleCaptureIndex, bool& OutputVelocity) const = 0;

	/**
	 * Sets up primitive uniform shader parameters that are internal to the renderer.
	 */
	virtual void BuildPrimitiveUniformShaderParameters_RenderThread(const FPrimitiveSceneInfo* PrimitiveSceneInfo, FPrimitiveUniformShaderParametersBuilder& Builder) const = 0;
	...
	UE_DEPRECATED(5.8, "No longer implemented. For similar use-cases refer to IRendererModule::UpdateStaticDrawListsForMaterials.")
	...
	virtual IAnimBankTransformProvider* GetAnimBankTransformProvider() = 0;
	virtual IAnimRuntimeTransformProvider* GetAnimRuntimeTransformProvider() = 0;
	virtual IAnimSequenceTransformProvider* GetAnimSequenceTransformProvider() = 0;
	...
	UE_DEPRECATED(5.8, "Use UpdatePrimitivesIsDrawn_RenderThread instead.")
	virtual void UpdatePrimitivesDrawnInGame_RenderThread(TArrayView<FPrimitiveSceneProxy*> PrimitiveSceneProxies, bool bDrawnInGame) final {}

	virtual void UpdatePrimitivesIsDrawn_RenderThread(TArrayView<FPrimitiveSceneProxy*> PrimitiveSceneProxies, bool bIsDrawn) = 0;
	 ...
	 * @param	Primitive				Primitive to retrieve interacting lights for
	 */
	virtual int32 GetRelevantLightCount(UPrimitiveComponent* Primitive) const = 0;
	/**
	 * Indicates if hit proxies should be processed by this scene
	 *
	 * @return true if hit proxies should be rendered in this scene.
	 */
	UE_DEPRECATED(5.8, "No longer exposed, only used internally.")
	...
	*/
	...
	virtual void UpdateCachedRayTracingState(class FPrimitiveSceneProxy* SceneProxy, EPrimitiveDirtyRayTracingState PrimitiveDirtyState) {}

	void UpdateCachedRayTracingState(class FPrimitiveSceneProxy* SceneProxy)
	{
		UpdateCachedRayTracingState(SceneProxy, EPrimitiveDirtyRayTracingState::Default);
	}
...
protected:
	...
	/** The shader platform this scene was created with */
