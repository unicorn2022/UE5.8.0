...
public:
	...
	void FitToNewBounds(const FBox& NewBounds);
...
public:
	...
	/** Fetches completed tasks for a specific static mesh and applies them to the scene */
	ENGINE_API void ProcessAsyncTasksForMesh(const UStaticMesh* InStaticMesh, bool bLimitExecutionTime = false);
	...
	/** Returns the number of outstanding referenced tasks for a specific static mesh. */
	ENGINE_API int32 GetNumOutstandingTasks(const UStaticMesh* InStaticMesh) const;
...
private:
	...
	void ProcessPendingTasksForMesh(const UStaticMesh* InStaticMesh);

	TUniquePtr<FQueuedThreadPool> ThreadPool;

	/** Builds a single task. Called from the worker thread. */
	void Build(FAsyncCardRepresentationTask* Task);
...
#if WITH_EDITOR
	/** Finishes a singular async task to completion and deletes its owner task */
	void FinalizeAndDeleteTask(FAsyncCardRepresentationTask* InTask, FObjectCacheContextScope& InObjectCacheScope);
#endif
