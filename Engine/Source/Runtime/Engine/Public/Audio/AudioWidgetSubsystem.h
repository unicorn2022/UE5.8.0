...
public:
	...
	ENGINE_API TArray<UUserWidget*> CreateUserWidgets(
		UWorld& InWorld,
		TSubclassOf<UInterface> InWidgetInterface,
		TFunction<bool(UUserWidget*)> FilterFunction = TFunction<bool(UUserWidget*)>(),
		TFunction<bool(UBlueprint*)> BlueprintFilterFunction = TFunction<bool(UBlueprint*)>()) const;
};
