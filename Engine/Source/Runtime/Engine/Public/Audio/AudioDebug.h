...
namespace Audio
{
	...
	public:
		...
		ENGINE_API bool IsSubmixMuteOrSoloActive() const;
		...
		ENGINE_API void QuerySoloMuteSubmix(const FString& Name, bool& bOutIsSoloed, bool& bOutIsMuted, FString& OutReason) const;
		...
		bool IsSoloSubmix(FName InName) const { return DebugNames.SoloSubmix.Contains(InName); }
		...
		bool IsMuteSubmix(FName InName) const { return DebugNames.MuteSubmix.Contains(InName); }
		...
		// Submix chain solo/mute. (game thread)
		// Recomputes the solo chain by walking the USoundSubmix hierarchy from explicitly soloed submixes.
		// Submixes linked to explicitly soloed submixes in the USoundSubmix hierarchy are implicitly soloed.
		// Explicitly-muted submixes are excluded from the implicit-solo chain.
		ENGINE_API void ToggleSoloSubmixChain(const FName InSubmixName);
		ENGINE_API void SetSoloSubmixChain(const FName InSubmixName, const bool bInSolo);

		ENGINE_API void ToggleMuteSubmix(const FName InSubmixName);
		ENGINE_API void SetMuteSubmix(const FName InSubmixName, const bool bInMute);

		ENGINE_API void ClearSubmixMuteSoloState();
		ENGINE_API bool IsSubmixExplicitlySoloed(const FName InSubmixName) const;
		ENGINE_API bool IsSubmixExplicitlyMuted(const FName InSubmixName) const;
	...
	private:
		...
		void RefreshComputedSubmixMuteSoloChain();
		void ProcessExplicitlySoloedSubmixChains(const TObjectPtr<USoundSubmix>& InMainSubmix, const TObjectPtr<USoundSubmix>& InBaseDefaultSubmix, TSet<FName>& OutComputedSoloNames);
		void ProcessExplicitlySoloedMainSubmixChains(const TObjectPtr<USoundSubmix>& InMainSubmix, const TObjectPtr<USoundSubmix>& InBaseDefaultSubmix, TSet<FName>& OutComputedSoloNames);
		void AddImplicitSolosFromHierarchy(const TObjectPtr<USoundSubmix>& InSubmix, const TObjectPtr<USoundSubmix>& InMainSubmix, const TObjectPtr<USoundSubmix>& InBaseDefaultSubmix, TSet<FName>& OutComputedSoloNames);
		void CollectSubmixHierarchyNames(const TObjectPtr<USoundSubmix>& InSubmix, const TObjectPtr<USoundSubmix>& InMainSubmix, const TObjectPtr<USoundSubmix>& InBaseDefaultSubmix, TSet<FName>& OutNames) const;
		...
		/** Explicit = submixes directly soloed/muted by the user. Other submixes may be soloed implicitly. */
		TSet<FName> ExplicitlySoloedSubmixes;
		TSet<FName> ExplicitlyMutedSubmixes;
