...
struct FVertexBufferAndSRV
{
	...
	FVertexBufferAndSRV(const FVertexBufferAndSRV& RHS)
	{
		*this = RHS;
	}

	FVertexBufferAndSRV& operator=(const FVertexBufferAndSRV& RHS)
	{
		VertexBufferRHI = RHS.VertexBufferRHI;
		VertexBufferSRV = RHS.VertexBufferSRV;
		Size = RHS.Size;
		return *this;
	}
...
public:
	...
	uint32 GetBufferSize(uint32 NumBones);
...
{
public:
	FUnifiedBoneBuffer(TConstArrayView<FBoneIndexType> BoneMap);
	~FUnifiedBoneBuffer();

	void Allocate(FRHICommandList& RHICmdList, bool bPrevious);

	void Update(FRHICommandList& RHICmdList, const FName& AssetPathName, TConstArrayView<FMatrix44f> ReferenceToLocalMatrices, bool bPrevious);
	...
	FVertexBufferAndSRV& GetBuffer(bool bPrevious)
	{
		return Buffers[CurrentIndex ^ (uint32)bPrevious];
	}

	const FVertexBufferAndSRV& GetBuffer(bool bPrevious) const
	{
		return Buffers[CurrentIndex ^ (uint32)bPrevious];
	}

	TConstArrayView<FBoneIndexType> GetBoneMap() const
	{
		return BoneMap;
	}
...
private:
	...
	TStaticArray<FVertexBufferAndSRV, 2> Buffers;
...
public:
	...
	struct FShaderDataType
	{
		FShaderDataType()
		{
			...
			FMemory::Memset(&SkinningHeader, 0, sizeof(FSkinningHeader));
		}

		void AllocateBoneBuffer(FRHICommandList& RHICmdList, uint32 BufferSize, FVertexBufferAndSRV& OutBoneBuffer);
		...
		// References a bone map buffer to use as an indirection on the GPU.
		FShaderResourceViewRHIRef SectionBoneMapBuffer;

		// The current skinning header assigned by the scene extension.
		FSkinningHeader SkinningHeader;

		// Frame number of the bone data that is last updated
		uint32 UpdatedFrameNumber = 0;

		// Whether bone buffers are shared between all sections.
		bool bUnifiedBoneBuffer = false;
	...
	virtual ~FGPUBaseSkinVertexFactory() = default;
	...
	void UpdateUniformBuffer(FRHICommandListBase& RHICmdList, const FSkinningHeader* SkinningHeader, FRHIShaderResourceView* TransformDataBufferSRV);
