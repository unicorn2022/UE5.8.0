...
struct FTemporalLODState
{
	...
	ENGINE_API float GetTemporalLODTransition(float LastRenderTime) const;
...
{
public:
	virtual ~FLightCacheInterface() = default;
...
ENGINE_API bool ShouldCompileDistanceFieldAOShaders(EShaderPlatform ShaderPlatform);
...
{
public:
	virtual ~FStaticPrimitiveDrawInterface() = default;
...
public:
	...
	virtual ~FOneFrameResource() = default;
};

/** One-frame resource that owns a TArray<FMeshBatch> for use as shared backing storage for mesh batch views. */
struct FOneFrameMeshBatchArray : public FOneFrameResource
{
...
{
	UE_DEPRECATED(5.8, "Use MeshBatchesView instead. Allocate mesh batches from FRayTracingInstanceCollector::AllocateMeshBatchArray() for shared ownership.")
	TArray<FMeshBatch> MeshBatches;

	/** Non-owning view of mesh batches. Preferred over MeshBatches TArray to avoid redundant copies.
	 *  When set, GetMeshBatches() returns this view instead of the deprecated MeshBatches array. */
	TConstArrayView<FMeshBatch> MeshBatchesView;

	/** Returns the mesh batches, preferring the view if set, falling back to the deprecated owned array. */
	TConstArrayView<FMeshBatch> GetMeshBatches() const
	{
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		if (MeshBatchesView.Num() > 0) return MeshBatchesView;
		return TConstArrayView<FMeshBatch>(MeshBatches);
	...
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	FRayTracingDynamicGeometryUpdateParams() = default;
	FRayTracingDynamicGeometryUpdateParams(const FRayTracingDynamicGeometryUpdateParams&) = default;
	FRayTracingDynamicGeometryUpdateParams& operator=(const FRayTracingDynamicGeometryUpdateParams&) = default;
	FRayTracingDynamicGeometryUpdateParams(FRayTracingDynamicGeometryUpdateParams&&) = default;
	FRayTracingDynamicGeometryUpdateParams& operator=(FRayTracingDynamicGeometryUpdateParams&&) = default;
	~FRayTracingDynamicGeometryUpdateParams() = default;
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
};
...
public:
	...
	ENGINE_API void AddReferencedGeometry(RayTracing::FGeometryHandle Geometry);
	...
	/** Allocate a frame-lifetime mesh batch array. Both FRayTracingInstance::MaterialsView and
	 *  FRayTracingDynamicGeometryUpdateParams::MeshBatchesView can reference the returned array
	 *  to avoid redundant copies and ensure both paths see the same mesh batch data. */
	TArray<FMeshBatch>& AllocateMeshBatchArray()
	{
		return AllocateOneFrameResource<FOneFrameMeshBatchArray>().MeshBatches;
	}
...
private:
	...
	TChunkedArray<FRayTracingInstanceAndViewIndex, 16384, SceneRenderingAllocator> RayTracingInstances;
	TChunkedArray<FRayTracingDynamicGeometryUpdateRequest, 16384, SceneRenderingAllocator> RayTracingGeometriesToUpdate;
