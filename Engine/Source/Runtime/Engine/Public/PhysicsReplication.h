...
struct FPhysicsReplicationAsyncInput : public Chaos::FSimCallbackInput
{
	...
	bool NetworkPhysicsTickOffsetAssigned;
...
struct FReplicatedPhysicsTargetAsync
{
	FReplicatedPhysicsTargetAsync()
		...
		, SimDecayTimeScale(1.0f)
		, LastLODFrame(INDEX_NONE)
		, PrevTickCountCached(0)
		, bPrevAllowTargetAlteringCached(false)
		, bFrameOffsetCorrectedCached(false)
	{ }
	...
	/** Per-frame time-scale used by SimulationDecay during resim integration steps. 1.0 is full integration, 0.0 is integration fully reverted */
	float SimDecayTimeScale;

	/** Solver frame at which ApplyPhysicsReplicationLOD last ran for this target */
	int32 LastLODFrame;

	/** Per-frame transient helper properties */
	int32 PrevTickCountCached;
	bool bPrevAllowTargetAlteringCached;
	bool bFrameOffsetCorrectedCached;
...
public:
	...
	/** Remove the replicated target*/
	ENGINE_API void RemoveReplicatedTarget(Chaos::FConstPhysicsObjectHandle PhysicsObject);
...
private:
	...
	TArray<TWeakObjectPtr<UPrimitiveComponent>> PendingDeleteFromComponentsToTargets;
