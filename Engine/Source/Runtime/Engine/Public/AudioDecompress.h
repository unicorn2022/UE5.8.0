...
public:
	...
	*/
	...
	ENGINE_API bool StreamCompressedInfo(const TSharedRef<const FSoundWaveData>& Wave, struct FSoundQualityInfo* QualityInfo);
...
protected:
	...
	ENGINE_API virtual bool StreamCompressedInfoInternal(const TSharedRef<const FSoundWaveData>& InWaveData, struct FSoundQualityInfo* QualityInfo);
	
	UE_DEPRECATED(5.8, "Please use the const TSharedRef<const FSoundWaveData>& overload instead")
	virtual bool StreamCompressedInfoInternal(const TSharedPtr<const FSoundWaveProxy>& InWaveProxy, struct FSoundQualityInfo* QualityInfo) { return false; }
...
public:
	...
	*/
	...
	virtual const TSharedPtr<const FSoundWaveData>& GetStreamingSoundWaveData() const { return StreamingSoundWaveData; }
	
	UE_DEPRECATED(5.8, "Use the GetStreamingSoundWaveData() instead, return value points to nullptr")
	virtual const FSoundWaveProxyPtr& GetStreamingSoundWave() const { static const FSoundWaveProxyPtr EmptyProxy; return EmptyProxy; }

	/** Return a conformed decode size that best matches all codecs. */
	static int32 ConformDecodeSize(int32 InMaxDesiredDecodeSizeInFrames);
...
protected:
	...
	UE_DEPRECATED(5.8, "Use StreamingSoundWaveData instead")
	FSoundWaveProxyPtr StreamingSoundWave;
	
	TSharedPtr<const FSoundWaveData> StreamingSoundWaveData;
};
...
public:
	...
	ENGINE_API virtual bool StreamCompressedInfoInternal(const TSharedRef<const FSoundWaveData>& InWaveData, FSoundQualityInfo* QualityInfo) override;
...
protected:
	...
	*/
	 ...
	 */
	ENGINE_API const uint8* GetLoadedChunk(const TSharedRef<const FSoundWaveData>& InSoundWaveData, uint32 ChunkIndex, uint32& OutChunkSize);
	
	UE_DEPRECATED(5.8, "Please use the TSharedRef<const FSoundWaveData>& overload instead")
	ENGINE_API const uint8* GetLoadedChunk(const FSoundWaveProxyPtr& InSoundWave, uint32 ChunkIndex, uint32& OutChunkSize);

	void RetainNextChunkInSequence(const TSharedRef<const FSoundWaveData>& InSoundWaveData, const uint32 InChunkIndex);
	...
	/** 
		...
		This and StreamSeekBlockOffset are expected to be set by the codec's SeekToTime() & SeekToFrame() functions.
	*/
...
struct IAudioInfoFactoryRegistry
{
	...
	virtual ICompressedAudioInfo* Create(FName InName, FName InAssetName = NAME_None) const = 0;

	UE_EXPERIMENTAL(5.8, "Decoder overriding is experimental and the API may be changed or removed")
	virtual void RegisterOverride(IAudioDecoderOverride* Override) = 0;
	UE_EXPERIMENTAL(5.8, "Decoder overriding is experimental and the API may be changed or removed")
	virtual void UnregisterOverride(IAudioDecoderOverride* Override) = 0;
};
