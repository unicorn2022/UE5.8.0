...
public:
	...
	{
		virtual ~FPerBoneInterpolationData() = default;
	};
...
/**
 * Intermediary Interface class to be implemented to access FAnimationRuntime::RetargetBoneTransform.
 * This is specialized in FBoneContainerRetargetBoneTransformContext, for example.
 */
...
public:
	...
	virtual const USkeleton* GetSkeleton(bool bEvenIfUnreachable = false) const = 0;

	/**
	 * Returns the skeleton bone index for a context bone index.
	 * Context bone index in this case is index within the wrapped context - for example the pose.
	 */
	virtual int32 GetSkeletonBoneIndex(const int32& InContextBoneIndex) const = 0;

	/**
	 * Returns the number of bones within this context - for example the number of bones within a pose
	 */
	virtual int32 GetNumBonesInContext() const = 0;
	
	/**
	 * Returns true if retargeting for this context is disabled.
	 */
	virtual bool IsRetargetingDisabled() const = 0;

	/**
	 * Returns the reference pose transform for a given context bone index. The context bone index refers 
	 * to the index of the bone within the context - for example a wrapped pose.
	 */
	virtual const FTransform& GetRefPoseTransform(const int32& InContextBoneIndex) const = 0;

	/**
	 * Returns the cached source data for retargeting within this context.
	 * This is current implemented only against the FBoneContainer.
	 */
	virtual const FRetargetSourceCachedData& GetRetargetSourceCachedData(const FName& InSourceName, const FSkeletonRemapping& InRemapping, const TArray<FTransform>& InRetargetTransforms) const = 0;
};

/**
 * Specialization of the FAnimationRuntimeRetargetBoneTransformContext to map against a FBoneContainer.
 * This allows to Retarget Bone Transforms for all elements in an FBoneContainer.
 */ 
class FBoneContainerRetargetBoneTransformContext : public FAnimationRuntimeRetargetBoneTransformContext
{
public:

	FBoneContainerRetargetBoneTransformContext(const FBoneContainer& InBoneContainer)
	...
	{
		return BoneContainer.GetSkeletonAsset(bEvenIfUnreachable);
	}
	
	virtual int32 GetSkeletonBoneIndex(const int32& InContextBoneIndex) const override
	{
		return BoneContainer.GetSkeletonIndex(FCompactPoseBoneIndex(InContextBoneIndex));
	}
	
	virtual int32 GetNumBonesInContext() const override
	{
		return BoneContainer.GetCompactPoseNumBones();
	}
	
	virtual bool IsRetargetingDisabled() const override
	{
		return BoneContainer.GetDisableRetargeting();
	}
	
	virtual const FTransform& GetRefPoseTransform(const int32& InContextBoneIndex) const override
	{
		return BoneContainer.GetRefPoseTransform(FCompactPoseBoneIndex(InContextBoneIndex));
	}
	...
	{
		return BoneContainer.GetRetargetSourceCachedData(InSourceName, InRemapping, InRetargetTransforms);
	}
...
public:
	...
	*/
	...
	/**
	 * Blends two poses together where the first pose is already in the output buffer (in-place).
	 * The first pose is scaled by WeightOfPoseOne, then the second pose is accumulated with (1 - WeightOfPoseOne).
	 * This avoids one pose allocation compared to BlendTwoPosesTogether.
	 */
	static ENGINE_API void BlendTwoPosesTogetherInPlace(
...
public:
	...
	*/
	...
	static ENGINE_API void RetargetBoneTransform(const USkeleton* SourceSkeleton, const FName& SourceName, const TArray<FTransform>& RetargetTransforms, FTransform& BoneTransform, const int32 SkeletonBoneIndex, const FCompactPoseBoneIndex& BoneIndex, const FBoneContainer& RequiredBones, const bool bIsBakedAdditive);
	static ENGINE_API void RetargetBoneTransform(const USkeleton* SourceSkeleton, const FName& SourceName, const TArray<FTransform>& RetargetTransforms, FTransform& BoneTransform, const int32 SkeletonBoneIndex, const FCompactPoseBoneIndex& BoneIndex, const FAnimationRuntimeRetargetBoneTransformContext* InContext, const bool bIsBakedAdditive);
