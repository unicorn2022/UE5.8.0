...
inline FProperty* GetReplicatedProperty(const UClass* CallingClass, const UClass* PropClass, const FName& PropName)
{
	...
	{
		UE_LOGF(LogNet, Fatal,"Attempt to replicate property '%ls' that was not tagged to replicate! Please use 'Replicated' or 'ReplicatedUsing' keyword in the UPROPERTY() declaration.", *TheProperty->GetFullName());
	}
#endif
	return TheProperty;
}
...
#define RPC_VALIDATE( expression )			\
	...
	{										\
		...
		TEXT( UE_STRINGIZE( expression ) )	\
		TEXT(" File: ")						\
		TEXT( UE_STRINGIZE( __FILE__ ) )	\
		TEXT(" Line: ")						\
		TEXT( UE_STRINGIZE( __LINE__ ) ) );	\
		return false;						\
	}
