...
enum class ENetPingControlMessage : uint8
{
	...
	SetAlternatePingAddress,	// Sent from the server to the client, to set an alternate ICMP/UDP ping address (not immediately activated)
...
/**
 * Which ping endpoint are we setting here? 
 */
enum class ENetPingTargetEndpoint : uint8
{
	DefaultEndpoint,				// Used when setting the primary ping endpoint
	AlternateEndpoint,				// Used when setting an alternate ping endpoint, typically used to handle accelerated routing.

	Max = AlternateEndpoint
};
...
public:
	...
	/**
	 * Activates a previously received alternate ping address for the specified ping type,
	 * replacing the current active ping address override.
	 *
	 * @param PingType	The type of ping to activate the alternate address for.
	 */
	ENGINE_API void UseAlternatePingAddress(EPingType PingType);

	/**
	 * Returns whether an alternate ping address has been received for the specified ping type.
	 *
	 * @param PingType	The type of ping to check for an alternate address.
	 * @return			True if an alternate address is available.
	 */
	ENGINE_API bool HasAlternatePingAddress(EPingType PingType) const;

	/** Delegate broadcast when the client receives an alternate ping address from the server. */
	using FOnAlternatePingAddressReceivedSignature = TMulticastDelegate<void(UNetConnection* /*Connection*/, EPingType /*PingType*/, const FString& /*Address*/)>;
	static FOnAlternatePingAddressReceivedSignature::RegistrationType& GetOnAlternatePingAddressReceivedDelegate() { return OnAlternatePingAddressReceivedDelegate; }
...
private:
	 ...
	 * @param TargetEndpoint		The target endpoint that the ping address is being set for
	 */
	void ServerSetPingAddress(EPingType PingType, FString PingAddress, ENetPingTargetEndpoint TargetEndpoint = ENetPingTargetEndpoint::DefaultEndpoint);
...
private:
	...
	/** Alternate address for UDPQoS pings, stored but not active until UseAlternatePingAddress is called */
	FString AlternatePingUDPQoSAddress;

	/** Alternate port for UDPQoS pings, stored but not active until UseAlternatePingAddress is called */
	int32 AlternatePingUDPQoSPort = INDEX_NONE;

	/** Delegate fired when an alternate Ping Address is received*/
	ENGINE_API static FOnAlternatePingAddressReceivedSignature OnAlternatePingAddressReceivedDelegate;
};
