...
struct FReplicationSystemUtil
{
	...
	/** Update group memberships used by COND_NetGroup sub object filtering for the specified SubObject */
	ENGINE_API static void UpdateSubObjectGroupMemberships(const UObject* SubObject, const UWorld* World);

	/** Update replication status for all NetGroups used by COND_NetGroup sub object filtering for the provided PlayerController */
...
/** Expose internal deferred replication system calls to the rest of the engine. */
class FDeferredReplicationSystemCalls
{
private:

	/** 
	 * Deferred version of the following functions:
	 *	FReplicationSystemUtil::StartReplicatingActor(AActor*)
	 *	FReplicationSystemUtil::StopReplicatingActor(AActor*, FStopReplicatingActorParams*)
	 * 	FReplicationSystemUtil::StartReplicatingActorComponent(const AActor*, UActorComponent*)
	 *	FReplicationSystemUtil::StopReplicatingActorComponent(UActorComponent*)
	 *
	 * The functions below are wrappers for equivalent functions in UEngineReplicationBridge.
	 * 
	 * All deferred functions will be processed when FlushDeferred() is called.
	 */
	static void StartReplicatingActorDefer(AActor* Actor);
	static void StopReplicatingActorDefer(AActor* Actor, const FStopReplicatingActorParams& StopReplicatingActorParams);
	static void StartReplicatingActorComponentDefer(const AActor* Actor, UActorComponent* ActorComponent);
	static void StopReplicatingActorComponentDefer(UActorComponent* ActorComponent, EEndReplicationFlags EndReplicationFlags);
	static void FlushDeferred(UEngineReplicationBridge* Bridge);
