...
{
public:
	virtual ~IStereoRendering() = default;
	...
	*/
	...
	/**
	* True if the device manages the ViewState for the specified view index.
	*/
	virtual bool HasExternalViewState(const int32 ViewIndex) const
	{
		return false;
	}

	/**
	* Gets a ViewState for the device, given the specified view index.
	* Note: The device may return nullptr, but it is expected to configure
	*       rendering properly (disable TemporalAA, Postprocessing, etc.).
	*/
	virtual FSceneViewStateInterface* GetExternalViewState(const int32 ViewIndex)
	{
		return nullptr;
	}
