...
{
	Initializer = RayTracingGeometry.GetInitializer();
...
void FSkeletalMeshStreamOut::ConditionalMarkComponentsDirty(const FContext& Context)
{
	...
	if (!IsCancelled() && Mesh && RenderData)
	{
		...
		TArray<const IPrimitiveComponent*> Comps;
		IStreamingManager::Get().GetRenderAssetStreamingManager().GetAssetComponents(Mesh, Comps);
		for (const IPrimitiveComponent* Component : Comps)
		{
			const_cast<IPrimitiveComponent*>(Component)->OnRenderAssetFirstLodChange(Mesh, RenderData->PendingFirstLODIdx);
		}
	}
	else
	{
...
void FSkeletalMeshStreamOut::WaitForReferences(const FContext& Context)
{
	...
	if (Mesh && RenderData)
	{
		...
		if (NumExternalReferences > PreviousNumberOfExternalReferences && NumReferenceChecks > 0)
		{
			...
			UE_LOGF(LogSkeletalMesh, Warning, "[%ls] Streamed out LODResources got referenced while in pending stream out.", *Mesh->GetName());
		}
	}
	...
	else
	{
		...
		{
			UE_LOGF(LogSkeletalMesh, Warning, "[%ls] Streamed out LODResources references are not getting released.", *Mesh->GetName());
		}
...
void FSkeletalMeshStreamIn_IO::ReportIOError(const FContext& Context)
{
	...
	if (bFailedOnIOError && Context.Mesh)
	{
		...
		UE_LOGF(LogContentStreaming, Warning, "[%ls] SkeletalMesh stream in request failed due to IO error (LOD %d-%d).", *Context.Mesh->GetName(), PendingFirstLODIdx, CurrentFirstLODIdx - 1);
	}
}
...
void FSkeletalMeshStreamIn_IO::SerializeLODData(const FContext& Context)
{
	...
	if (!IsCancelled() && Mesh && RenderData)
	{
		...
		for (int32 LODIndex = PendingFirstLODIdx; LODIndex < CurrentFirstLODIdx; ++LODIndex)
		{
			...
			if (Ar.IsError())
			{
				UE_LOGF(LogContentStreaming, Error,
					"[%ls] SkeletalMesh stream in failed due to possibly corrupted data. LOD %d %d-%d. BulkData %#x offset %lld size %lld flags %#x. bForceKeepCPUResources %d. bNeedsCPUAccess %d.",
					*Mesh->GetPathName(),
					LODIndex,
					PendingFirstLODIdx,
				...
				GLog->Flush();
				UE_LOGF(LogContentStreaming, Fatal, "Possibly corrupted skeletal mesh LOD data detected.");
#endif
			}
		}
