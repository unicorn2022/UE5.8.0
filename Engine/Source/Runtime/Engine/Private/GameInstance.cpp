...
FGameInstancePIEResult UGameInstance::InitializeForPlayInEditor(int32 PIEInstanceIndex, const FGameInstancePIEParameters& Params)
{
	...
	else
	{
		if (Params.OverrideMapURL.Len() > 0)
		{
			...
			if (WorldToDuplicate && EditorWorld->Scene != nullptr)
			{
				WorldToDuplicate->ShaderPlatformChanged(EditorWorld->Scene->GetShaderPlatform(), false);
...
void UGameInstance::OnWorldChanged(UWorld* OldWorld, UWorld* NewWorld)
{
	if (NewWorld)
	{
		...
		{
			UE_LOGF(LogLoad, Warning, "GameInstance %ls changed from world %ls to world %ls with a different world context!", *GetName(), *GetPathNameSafe(OldWorld), *GetPathNameSafe(NewWorld));
			WorldContext = NewWorldContext;
		}
	}
	else
...
void UGameInstance::StartGameInstance()
{
	...
	// Cache a stable identifier to the WorldContext to enable WorldContext to be restored if the initial call to Browse fails.
	// This may be browsing to a user provided map which may not exist. On failure, this enable the user to decide to load to 
	// the default map.
	FName InitialWorldContextHandle = WorldContext->ContextHandle;
	...
	{
		UE_LOGF(LogLoad, Error, "%ls", *FString::Printf(TEXT("Failed to enter %s: %s. Please check the log for errors."), *URL.Map, *Error));
		...
		{
			// Restore the WorldContext used in the initial call to Browse or create a new WorldContext if the WorldContext was destroyed. 
			WorldContext = &Engine->GetWorldContextFromHandleChecked(InitialWorldContextHandle);
	...
	if (BrowseRet == EBrowseReturnVal::Failure)
	{
		UE_LOGF(LogLoad, Error, "%ls", *FString::Printf(TEXT("Failed to enter %s: %s. Please check the log for errors."), *DefaultMap, *Error));
...
ULocalPlayer* UGameInstance::CreateLocalPlayer(FPlatformUserId UserId, FString& OutError, bool bSpawnPlayerController)
{
	...
	else if (LocalPlayers.Num() < MaxSplitscreenPlayers)
	{
		...
		{
			UE_LOGF(LogPlayerManagement, Warning, "Controller ID (%d) is unlikely to map to any physical device, so this player will not receive input", UserId.GetInternalId());
		}
	...
	if (OutError != TEXT(""))
	{
		UE_LOGF(LogPlayerManagement, Log, "UPlayer* creation failed with error: %ls", *OutError);
	}
...
int32 UGameInstance::AddLocalPlayer(ULocalPlayer* NewLocalPlayer, FPlatformUserId UserId)
{
	...
	UE_LOGF(LogPlayerManagement, Log, "UGameInstance::AddLocalPlayer: Added player %ls with PlatformUserId %d at index %d (%d remaining players)", *NewLocalPlayer->GetName(), NewLocalPlayer->GetPlatformUserId().GetInternalId(), InsertIndex, LocalPlayers.Num());
...
bool UGameInstance::RemoveLocalPlayer(ULocalPlayer* ExistingPlayer)
{
	...
	if (ExistingPlayer->PlayerController != nullptr)
	{
		...
		if (CurrentWorld && CurrentWorld->IsPlayingReplay())
		{
			if (UDemoNetDriver* DemoNetDriver = CurrentWorld->GetDemoNetDriver())
			{
				...
				{
					UE_LOGF(LogPlayerManagement, Warning, "UGameInstance::RemovePlayer: Did not remove player %ls with ControllerId %i as it was unable to be removed from the demo",
	...
	UE_LOGF(LogPlayerManagement, Log, "UGameInstance::RemovePlayer: Removed player %ls with ControllerId %i at index %i (%i remaining players)", *ExistingPlayer->GetName(), ExistingPlayer->GetControllerId(), OldIndex, LocalPlayers.Num());
...
void UGameInstance::DebugCreatePlayer(int32 ControllerId)
{
	...
	if (Error.Len() > 0 || !LP)
	{
		UE_LOGF(LogPlayerManagement, Error, "Failed to DebugCreatePlayer: %ls", *Error);
	}
	else
	{
...
bool UGameInstance::EnableListenServer(bool bEnable, int32 PortOverride /*= 0*/)
{
	...
	else
	{
		...
		{
			UE_LOGF(LogGameSession, Warning, "EnableListenServer: Dedicated servers always listen for connections");
			return false;
		}
		...
		if (ExistingMode == NM_ListenServer)
		{
			...
			UE_LOGF(LogGameSession, Warning, "EnableListenServer: Disabling a listen server with active connections does not disconnect existing players by default");
		}
...
bool UGameInstance::ClientTravelToSession(int32 ControllerId, FName InSessionName)
{
	...
	if (UOnlineEngineInterface::Get()->GetResolvedConnectString(World, InSessionName, URL))
	{
		...
		{
			UE_LOGF(LogGameSession, Warning, "Failed to find local player for controller id %d", ControllerId);
		}
	}
	else
	{
		UE_LOGF(LogGameSession, Warning, "Failed to resolve session connect string for %ls", *InSessionName.ToString());
	}
...
AGameModeBase* UGameInstance::CreateGameModeForURL(FURL InURL, UWorld* InWorld)
{
	...
	if (!GameParam.IsEmpty())
	{
		...
		{
			UE_LOGF(LogLoad, Warning, "Failed to load game mode '%ls' specified by URL options.", *GameClassName);
		}
	}
	...
	if (!GameClass)
	{
		...
		if (!GameClassName.IsEmpty())
		{
			...
			{
				UE_LOGF(LogLoad, Warning, "Failed to load game mode '%ls' specified by prefixed map name %ls.", *GameClassName, *MapNameNoPath);
			}
		}
	}
	...
	// Spawn the GameMode.
	UE_LOGF(LogLoad, Log, "Game class is '%ls'", *GameClass->GetName());
