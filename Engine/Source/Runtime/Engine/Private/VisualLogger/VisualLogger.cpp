...
bool FVisualLogger::bUseVerbosityFilterWhenRecording = false;

namespace UE::VisualLogger
{
FAutoConsoleCommandWithOutputDevice CmdActivateVerbosityFilterWhenRecording(
	TEXT("vislog.ActivateVerbosityFilterWhenRecording"),
	TEXT("Active log category verbosity will be used to filter out entries to record.\n"),
	FConsoleCommandWithOutputDeviceDelegate::CreateLambda([](FOutputDevice& OutputDevice)
		{
			GetMutableDefault<UVisualLoggerSettings>()->SetUseVerbosityFilterWhenRecording(true);
			check(FVisualLogger::bUseVerbosityFilterWhenRecording);

			OutputDevice.Log(ELogVerbosity::Log, TEXT("LogCategory verbosity based filtering is now 'ACTIVE'"));
		}),
	ECVF_Default);

FAutoConsoleCommandWithOutputDevice CmdDeactivateVerbosityFilterWhenRecording(
	TEXT("vislog.DeactivateVerbosityFilterWhenRecording"),
	TEXT("Log category verbosity is deactivated and everything is recorded.\n"),
	FConsoleCommandWithOutputDeviceDelegate::CreateLambda([](FOutputDevice& OutputDevice)
		{
			GetMutableDefault<UVisualLoggerSettings>()->SetUseVerbosityFilterWhenRecording(false);
			check(!FVisualLogger::bUseVerbosityFilterWhenRecording);

			OutputDevice.Log(ELogVerbosity::Log, TEXT("LogCategory verbosity based filtering is now 'INACTIVE'"));
		}),
	ECVF_Default);
}
...
static void CollapsedLogfV(const SerializeFuncType& SerializeFunc, const UObject* Object, const FName& CategoryName, ELogVerbosity::Type Verbosity, const TCHAR* Fmt, va_list Args)
{
	...
	FVisualLogEntry NewEntry;
	FVisualLogger::FObjectNames Names;
	if (FVisualLogger::CreateEntryInternal(Object, NewEntry, Names))
	{
		SerializeFunc(&NewEntry, TStringBuilder<512>().AppendV(Fmt, Args).ToString());

		FVisualLoggerTraceDevice::Get().Serialize(Object, Names.Name, Names.DisplayName, Names.ClassName, NewEntry);
	}
}
...
bool FVisualLogger::CheckVisualLogInputInternal(const UObject* Object, const FName& CategoryName, ELogVerbosity::Type, UWorld** OutWorld, FVisualLogEntry** OutCurrentEntry)
{
	...
	FVisualLogger& VisualLogger = Get();
...
#else // ^^^ UE_DEBUG_RECORDING_USING_VLOG ^^^ / vvv TRACE ONLY vvv

bool FVisualLogger::CreateEntryInternal(const UObject* Object, FVisualLogEntry& OutEntry, FVisualLogger::FObjectNames& OutNames)
{
	if (IsRecording() == false || !Object || !GEngine || GEngine->bDisableAILogging || Object->HasAnyFlags(RF_ClassDefaultObject))
	{
		return false;
	...
	FVisualLogEntry NewEntry;
	NewEntry.InitializeEntry(VisualLogger.GetTimeStampForObject(Object));

	FObjectNames* Names;
	{
		UE::TReadScopeLock Lock(VisualLogger.RedirectRWLock);
		Names = VisualLogger.ObjectToNamesMap.Find(Object);
	}

	if (Names == nullptr)
	{
		UE::TWriteScopeLock Lock(VisualLogger.RedirectRWLock);
		Names = VisualLogger.ObjectToNamesMap.Find(Object);
		if (Names == nullptr)
		{
			Names = &VisualLogger.ObjectToNamesMap.Add(Object, VisualLogger.CreateObjectNames(Object));
		}
	}

	OutEntry = MoveTemp(NewEntry);
	OutNames = *Names;
...
FVisualLogEntry* FVisualLogger::GetEntryToWriteInternal(const UObject* Object, const double TimeStamp, const ECreateIfNeeded ShouldCreate)
{
	...
	else if (ShouldCreate == ECreateIfNeeded::Create)
	{
		...
		ObjectToNamesMap.Add(LogOwner, CreateObjectNames(LogOwner));
		ObjectToWorldMap.Add(LogOwner, GetWorldForVisualLogger(LogOwner));
...
void FVisualLogger::FlushEntry(FVisualLogEntry& Entry, const FObjectKey& ObjectKey)
{
	...
	{
		const FObjectNames& Names = ObjectToNamesMap[ObjectKey];
		Device->Serialize(OwnerObject, Names.Name, Names.DisplayName, Names.ClassName, Entry);
	}
	Entry.Reset();
}
...
FVisualLogger::FVisualLogger()
{
	...
	if (FParse::Param(FCommandLine::Get(), TEXT("EnableVisualLoggerTrace")))
	{
		SetIsRecordingToTrace(true);
	}
	else
	{
...
void FVisualLogger::SetIsRecording(const bool InIsRecording)
{
	...
	{
		NavigationDataDump(GetWorldForVisualLogger(nullptr), LogNavigation, ELogVerbosity::Log, FBox());
	}
...
{
	TWeakObjectPtr TargetWeakPtr(Object);
...
FVisualLogger::FObjectNames FVisualLogger::CreateObjectNames(TNotNull<const UObject*> LogOwner)
{
	if (bForceUniqueLogNames && UE::VisLog::Private::UniqueLogPrefix.IsEmpty())
	{
		UE::VisLog::Private::UniqueLogPrefix = FString::Printf(TEXT("[%d] "), UE::VisLog::Private::UniqueRunNumber);
	}
	...
	const FName LogName(*FString::Printf(TEXT("%s%s%s"),
		*UE::VisLog::Private::UniqueLogPrefix,
	...
	return {.Name = LogName, .DisplayName = LogDisplayName, .ClassName = LogOwner->GetClass()->GetFName()};
}
...
void FVisualLogger::Cleanup(UWorld* OldWorld, const bool bReleaseMemory)
{
	...
	Flush();
	for (FVisualLogDevice* Device : Get().OutputDevices)
	{
...
FVisualLogger::FOwnerToChildrenRedirectionMap& FVisualLogger::GetRedirectionMap(const UObject* InObject)
{
	...
	if (const TWeakObjectPtr<const UWorld>* Entry = Get().ObjectToWorldMap.Find(InObject))
	{
		World = Entry->Get();
	}
...
protected:
	...
	virtual bool Exec_Dev(UWorld* InWorld, const TCHAR* Cmd, FOutputDevice& Ar) override
	{
		if (FParse::Command(&Cmd, TEXT("VISLOG")))
		{
			...
			{
				FGlobalTabmanager::Get()->TryInvokeTab(FName("VisualLogger"));
				return true;
			}
		}
