...
bool FVisualLogTest::RunTest(const FString& Parameters)
{
	...
	// Make sure our log is recorded by setting the active verbosity.
	// This is required if verbosity filtering when recording is enabled.
	LOG_SCOPE_VERBOSITY_OVERRIDE(LogVisual, ELogVerbosity::Log);
	...
	{
		const FString TextToLog = "Simple text line to test if UE_VLOG_UELOG works fine";
...
bool FVisualLogEventsTest::RunTest(const FString& Parameters)
{
	...
	const FName EventTag1 = "ATLAS_C_0";
	const FName EventTag2 = "ATLAS_C_1";
	const FName EventTag3 = "ATLAS_C_2";
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FVisualLogRedirectionsWithinHierarchyTest, "System.Engine.VisualLogger.Redirections.WithinHierarchy", EAutomationTestFlags::ClientContext | EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
bool FVisualLogRedirectionsWithinHierarchyTest::RunTest(const FString& Parameters)
{
	UWorld* World = GetSimpleEngineAutomationTestWorld(GetTestFlags());
	CHECK_NOT_NULL(World);
