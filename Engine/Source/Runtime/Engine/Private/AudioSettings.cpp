...
#include "Sound/AudioPlatformSettings.h"
	...
	: Super(ObjectInitializer)
{
	...
	PlatformSettings.Initialize(UAudioPlatformSettings::StaticClass());
}
...
void UAudioSettings::PostEditChangeChainProperty(FPropertyChangedChainEvent& PropertyChangedEvent)
{
	if (PropertyChangedEvent.Property)
	{
		...
		if (PropertyName == GET_MEMBER_NAME_CHECKED(UAudioSettings, MasterSubmix))
		{
			...
			|| PropertyName == GET_MEMBER_NAME_CHECKED(UAudioSettings, PlatformSettings))
		{
			bPromptRestartRequired = true;
		}
...
{
	UE_LOGF(LogAudio, Display, "Loading Default Audio Settings Objects...");

	// TODO: Move all soft object paths to load here (SoundMixes, Submixes, etc.)
	static const FString EngineSoundsDir = "/Engine/EngineSounds";
	...
	if (!DefaultSoundClass)
	{
		UE_LOGF(LogAudio, Warning, "Failed to load Default SoundClassObject from path '%ls'.  Attempting to fall back to engine default.", *DefaultSoundClassName.GetAssetPathString());
	...
	if (!DefaultSoundClass)
	{
		UE_LOGF(LogAudio, Error, "Failed to load Default SoundClassObject from path '%ls'.", *DefaultSoundClassName.GetAssetPathString());
	}
	...
	{
		UE_LOGF(LogAudio, Display, "No default MediaSoundClassObject specified (or failed to load).");
	}
	...
	{
		UE_LOGF(LogAudio, Display, "No default SoundConcurrencyObject specified (or failed to load).");
	}
}
...
void UAudioSettings::RegisterParameterInterfaces()
{
	...
	{
		UE_LOGF(LogAudio, Display, "Registering Engine Module Parameter Interfaces...");
		...
		InterfaceRegistry.RegisterInterface(Audio::SourceLocationInterface::GetInterface());
		InterfaceRegistry.RegisterInterface(Audio::ListenerLocationInterface::GetInterface());
		InterfaceRegistry.RegisterInterface(Audio::SourceOcclusionInterface::GetInterface());
		InterfaceRegistry.RegisterInterface(Audio::WorldTimeInterface::GetInterface());
	}
}
...
ENGINE_API FDecodeCacheSettings UAudioSettings::GetDecodeCacheSettings() const
{
	if (UAudioPlatformSettings* Settings = UPlatformSettingsManager::Get().GetSettingsForPlatform<UAudioPlatformSettings>())
	{
		return Settings->DecodeCacheSettings;
	}
	return FDecodeCacheSettings();
}
