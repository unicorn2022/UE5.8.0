...
IMPLEMENT_GLOBAL_SHADER_PARAMETER_STRUCT(FSplineMeshVFPrimitiveShaderParameters, "SplineMeshVFPrimitiveShaderParameters");
...
bool FSplineMeshSceneProxyCommon::OnMeshCardsTransformChanged(const FVector& LocalToWorldScale)
{
	const FVector3f NewLocalToWorldScale(LocalToWorldScale);
	const bool bWasUniform = CachedLocalToWorldScale.IsUniform();
	const bool bIsUniform = NewLocalToWorldScale.IsUniform();

	// Uniformity changed or non-uniform scale changed
	const bool bRefitCardRepresentation = bWasUniform != bIsUniform || !(bIsUniform || (NewLocalToWorldScale - CachedLocalToWorldScale).IsNearlyZero());

	CachedLocalToWorldScale = NewLocalToWorldScale;
	return bRefitCardRepresentation;
}

FMatrix FSplineMeshSceneProxyCommon::GetMeshCardsToWorld(const FMatrix& LocalToWorld) const
{
	return FTransform(FQuat(MeshCardsToLocalRotation), FVector(MeshCardsToLocalTranslation)).ToMatrixNoScale()
		* (bCardsInWorldScale ? LocalToWorld.GetMatrixWithoutScale() : LocalToWorld);
}

void FSplineMeshSceneProxyCommon::SetSplineTextureCoord_RenderThread(uint32 SplineIndex, FUintVector2 TexCoord)
{
	...
	RepackSplineMeshParamsInternal();
}
	
void FSplineMeshSceneProxyCommon::UpdateSplineMeshParams_RenderThread(const FSplineMeshShaderParams& Params)
{
...
void FSplineMeshSceneProxyCommon::RepackSplineMeshParamsInternal()
{
	auto& SceneProxy = GetProxy();
	...
	if (SplineMeshInstanceData.Update(SplineParams))
	{
		...
		SceneProxy.GetScene().RequestGPUSceneUpdate(*SceneProxy.GetPrimitiveSceneInfo(), EPrimitiveDirtyState::ChangedOther);
	}

	if (SplineMeshVFPrimitiveShaderParametersUB)
	{
		FSplineMeshVFPrimitiveShaderParameters SplineMeshVFPrimitiveShaderParameters;
		PackSplineMeshParams(SplineParams, SplineMeshVFPrimitiveShaderParameters.SplineParams);
		SplineMeshVFPrimitiveShaderParametersUB.UpdateUniformBufferImmediate(FRHICommandListImmediate::Get(), SplineMeshVFPrimitiveShaderParameters);
	}
	...
	FStaticMeshSceneProxy(InMeshDesc, false)
{
	...
	if (FeatureLevel > ERHIFeatureLevel::ES3_1 && UseGPUScene(GetScene().GetShaderPlatform()))
	{
	...
	else
	{
		FSplineMeshVFPrimitiveShaderParameters SplineMeshVFPrimitiveShaderParameters;
		PackSplineMeshParams(SplineParams, SplineMeshVFPrimitiveShaderParameters.SplineParams);
		SplineMeshVFPrimitiveShaderParametersUB = CreateUniformBufferImmediate(SplineMeshVFPrimitiveShaderParameters, EUniformBufferUsage::UniformBuffer_MultiFrame);
	}
...
void FSplineMeshSceneProxy::SetupMeshBatchForSpline(int32 InLODIndex, FMeshBatch& OutMeshBatch) const
{
	...
	OutMeshBatch.Elements[0].LooseParametersUniformBuffer = SplineMeshVFPrimitiveShaderParametersUB;
...
void FSplineMeshSceneProxy::GetDynamicRayTracingInstances(FRayTracingInstanceCollector& Collector)
{
		...
		for (int32 BatchIndex = 0; BatchIndex < NumBatches; BatchIndex++)
		{
			for (int32 SectionIndex = 0; SectionIndex < RayTracingLOD.Sections->Num(); SectionIndex++)
			{
				...
				if (bResult)
				{
					...
					CachedRayTracingMaterials.Add(MoveTemp(MeshBatch));
				}
			}
		}
	...
	if (bNeedsUpdate && (*RenderData->RayTracingProxy->LODVertexFactories)[LODIndex].SplineVertexFactory->GetType()->SupportsRayTracingDynamicGeometry())
	{
		...
		UpdateParams.NumTriangles = Geometry.GetInitializer().TotalPrimitiveCount;
		UpdateParams.Geometry = &Geometry;
	...
	checkf(RayTracingInstance.Geometry->GetInitializer().Segments.Num() >= CachedRayTracingMaterials.Num(), TEXT("Segments/Materials mismatch. Number of segments: %d. Number of Materials: %d. LOD Index: %d"),
		RayTracingInstance.Geometry->GetInitializer().Segments.Num(),
...
void FNaniteSplineMeshSceneProxy::OnTransformChanged(FRHICommandListBase& RHICmdList)
{
	...
	if (IsVisibleInLumenScene() && CardRepresentationData)
	{
		const FVector LocalToWorldScale = GetLocalToWorld().GetScaleVector();
		const bool bRefitCardRepresentation = OnMeshCardsTransformChanged(LocalToWorldScale);

		if (bRefitCardRepresentation || !bHasRuntimeCardRepresentationData)
		{
			...
			FSplineMeshSceneProxyDesc SplineDesc;
			SplineDesc.InitializeFrom(GetSplineMeshParams(), StaticMesh, RenderData);

			FBox MeshCardsBounds;
			FTransform MeshCardsToLocal;
			bCardsInWorldScale = SplineDesc.ComputeDistortedBoundsForLumenCardCapture(LocalToWorldScale, MeshCardsBounds, MeshCardsToLocal);

			FVector LocalBoundsExtent = GetLocalBounds().BoxExtent;
			if (bCardsInWorldScale)
			{
				LocalBoundsExtent *= LocalToWorldScale;
			}
			
			auto GetBoxSurfaceArea = [](const FVector& Extent)
			{
				return Extent.X * Extent.Y + Extent.Y * Extent.Z + Extent.Z * Extent.X;
			};

			FCardRepresentationData* NewCardRepresentationData;
			if (GetBoxSurfaceArea(LocalBoundsExtent) < GetBoxSurfaceArea(MeshCardsBounds.GetExtent()))
			{
				MeshCardsToLocalRotation = FQuat4f::Identity;
				MeshCardsToLocalTranslation = FVector3f::ZeroVector;
				bCardsInWorldScale = false;

				NewCardRepresentationData = new FCardRepresentationData;
				NewCardRepresentationData->MeshCardsBuildData.Bounds = GetLocalBounds().GetBox();
				NewCardRepresentationData->MeshCardsBuildData.bMostlyTwoSided = CardRepresentationData->MeshCardsBuildData.bMostlyTwoSided;

				MeshCardRepresentation::SetCardsFromBounds(NewCardRepresentationData->MeshCardsBuildData);
			}
			else
			{
				MeshCardsToLocalRotation = FQuat4f(MeshCardsToLocal.GetRotation());
				MeshCardsToLocalTranslation = FVector3f(MeshCardsToLocal.GetTranslation());

				// Mesh cards bounds is the SDF bounds or the render data bounds (see FMeshUtilities::GenerateCardRepresentationData)
				// while the new bounds is computed based on SourceMeshBounds which is the LOD0 bounds.
				// Try to match by expanding the new bounds by the same ratio.
				FVector ExpandedBoxExtent;
				if (DistanceFieldData && DistanceFieldData->LocalSpaceMeshBounds.IsValid)
				{
					ExpandedBoxExtent = FVector(DistanceFieldData->LocalSpaceMeshBounds.GetExtent());
				}
				else
				{
					ExpandedBoxExtent = RenderData->Bounds.BoxExtent;
				}

				const FVector ExpansionScale = FVector::Max(ExpandedBoxExtent / FVector::Max(SplineDesc.SourceMeshBounds.BoxExtent, FVector::OneVector), FVector::OneVector);
				const FVector MeshCardsCenter = MeshCardsBounds.GetCenter();
				const FVector MeshCardsExtent = MeshCardsBounds.GetExtent() * ExpansionScale;
				MeshCardsBounds = FBox(MeshCardsCenter - MeshCardsExtent, MeshCardsCenter + MeshCardsExtent);
			
				NewCardRepresentationData = new FCardRepresentationData;
				NewCardRepresentationData->MeshCardsBuildData = CardRepresentationData->MeshCardsBuildData;
				NewCardRepresentationData->MeshCardsBuildData.FitToNewBounds(MeshCardsBounds);
			}

			CardRepresentationData = NewCardRepresentationData;
			bHasRuntimeCardRepresentationData = true;
		}
	}
}

FMatrix FNaniteSplineMeshSceneProxy::GetMeshCardsToWorld(int32 InstanceIndex) const
{
	return FSplineMeshSceneProxyCommon::GetMeshCardsToWorld(GetLocalToWorld());
}
