...
// 	UE_LOGF(LogPlayerCameraManager, Log, "%f AssignViewTarget OldTarget: %ls, NewTarget: %ls, BlendTime: %f", GetWorld()->TimeSeconds, VT.Target ? *VT.Target->GetFName().ToString() : TEXT("NULL"),
...
void APlayerCameraManager::UpdateViewTarget(FTViewTarget& OutVT, float DeltaTime)
{
	...
	else
	{
		...
		static const FName NAME_Fixed = FName("Fixed");
		static const FName NAME_ThirdPerson = FName("ThirdPerson");
		static const FName NAME_FreeCam = FName("FreeCam");
		static const FName NAME_FreeCam_Default = FName("FreeCam_Default");
		static const FName NAME_FirstPerson = FName("FirstPerson");
...
#if UE_WITH_REMOTE_OBJECT_HANDLE
FRemoteServerId APlayerCameraManager::GetPreferredRemoteServer() const
{
	if (!AutoRTFM::IsClosed() && PCOwner.GetResidence() == EResidence::Remote)
	{
		// We are outside of a transaction (probably push migration step) and the
		// PCOwner is already on another server - that IS its preferred server
		FRemoteServerId PCServerId = UE::RemoteObject::Handle::GetOwnerServerId(PCOwner.GetRemoteId());
		if (PCServerId.IsValid())
		{
			return PCServerId;
		}
	}
	else if (APlayerController* LocalPC = PCOwner.Get())
	{
		return LocalPC->GetPreferredRemoteServer();
	}
	
	// No PCOwner — stay where we are (don't use spatial delegate, our location is not meaningful)
...
void APlayerCameraManager::UpdateCamera(float DeltaTime)
{
	...
	if ((PCOwner->Player && PCOwner->IsLocalPlayerController()) || !bUseClientSideCameraUpdates || bDebugClientSideCamera)
	{
		...
		if (bShouldSendClientSideCameraUpdate && IsNetMode(NM_Client))
		{
			...
			FVector LastCachedCameraLocation;
			FRotator LastCachedCameraRotation;
			GetLastFrameCameraCacheLocationAndRotationForServer(LastCachedCameraLocation, LastCachedCameraRotation);
			FVector CachedCameraLocation;
			FRotator CachedCameraRotation;
			GetCameraCacheLocationAndRotationForServer(CachedCameraLocation, CachedCameraRotation);

			FVector ClientCameraPosition = FRepMovement::RebaseOntoZeroOrigin(CachedCameraLocation, this);
			FVector PrevClientCameraPosition = FRepMovement::RebaseOntoZeroOrigin(LastCachedCameraLocation, this);
			...
			if (bPositionThreshold || (TimeSinceLastServerUpdateCamera > ClientNetCamUpdateDeltaTime))
			{
				...
				int32 const ShortYaw = FRotator::CompressAxisToShort(CachedCameraRotation.Yaw);
				int32 const ShortPitch = FRotator::CompressAxisToShort(CachedCameraRotation.Pitch);
				int32 const CompressedRotation = (ShortYaw << 16) | ShortPitch;

				int32 const PrevShortYaw = FRotator::CompressAxisToShort(LastCachedCameraRotation.Yaw);
				int32 const PrevShortPitch = FRotator::CompressAxisToShort(LastCachedCameraRotation.Pitch);
	...
	{
		const FMinimalViewInfo& NewInfo = GetCameraCacheView();
		CVD_TRACE_CAMERA(this, GetWorld(), GetFName(), NewInfo.Location, FQuat(NewInfo.Rotation), NewInfo.FOV);
	}
#endif
}
...
void APlayerCameraManager::GetCameraCacheLocationAndRotationForServer(FVector& OutLocation, FRotator& OutRotation) const
{
	const FMinimalViewInfo& ViewInfo = GetCameraCacheView();
	OutLocation = ViewInfo.Location;
	OutRotation = ViewInfo.Rotation;
}

void APlayerCameraManager::GetLastFrameCameraCacheLocationAndRotationForServer(FVector& OutLocation, FRotator& OutRotation) const
{
	const FMinimalViewInfo& ViewInfo = GetLastFrameCameraCacheView();
	OutLocation = ViewInfo.Location;
	OutRotation = ViewInfo.Rotation;
}
