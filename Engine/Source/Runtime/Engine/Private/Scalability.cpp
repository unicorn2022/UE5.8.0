...
int32 ComputeOptionFromPerfIndex(const FString& GroupName, float CPUPerfIndex, float GPUPerfIndex)
{
	...
	if (GConfig)
	{
		const FString ArrayKey = FString("PerfIndexThresholds_") + GroupName;
...
void ApplyScalabilityGroupFromPlatformIni(const TCHAR* InSectionName, const TCHAR* InIniFilename)
{
	...
	TFunction<void(IConsoleVariable*, const FString&, const FString&)> Func = [&](IConsoleVariable* CVar, const FString& KeyString, const FString& ValueString)
	{
		...
		if (!PlatformScalabilityCVarDenyList.Contains(CVar) && (PlatformScalabilityCVarAllowList.Num() == 0 || PlatformScalabilityCVarAllowList.Contains(CVar)))
		{
			...
			UE_LOGF(LogConfig, Verbose, "Setting scalability CVar [[%ls:%ls]]", *KeyString, *ValueString);
...
{
//	UE_LOGF(LogConsoleResponse, Display, "  %ls %d", InGroupName, InQualityLevel);
...
//	UE_LOGF(LogConsoleResponse, Display, "  ResolutionQuality %.2f", "", InResolutionQualityLevel);
...
static float GetRenderScaleLevelFromQualityLevel(int32 InQualityLevel, EQualityLevelBehavior Behavior = EQualityLevelBehavior::EAbsolute)
{
	...
	{
		UE_LOGF(LogConsoleResponse, Display, "Failed to find resolution value strings in scalability ini. Falling back to default.");
		return 100.0f;
	}
...
static void PrintGroupInfo(const TCHAR* InGroupName, bool bInInfoMode)
{
	...
	UE_LOGF(LogConsoleResponse, Display, "  %ls (0..3): %ls", InGroupName, *GroupQualityLevelDisplayName);
	...
	if (bInInfoMode)
	{
		if (QualityLevel != -1)
		{
			...
			{
				UE_LOGF(LogConsoleResponse, Display, "    %ls", *CVars[CVarDataIndex]);
			}
		}
	}
}
...
void ProcessCommand(const TCHAR* Cmd, FOutputDevice& Ar)
{
	...
	if (FParse::Token(Cmd, Token, true))
	{
		...
		{
			UE_LOGF(LogConsoleResponse, Error, "Scalability unknown parameter");
	...
	if (bPrintUsage)
	{
		UE_LOGF(LogConsoleResponse, Display, "Scalability Usage:");
		UE_LOGF(LogConsoleResponse, Display, "  \"Scalability\" (Print scalability usage and information)");
		UE_LOGF(LogConsoleResponse, Display, "  \"Scalability [0..3]\" (Set all scalability groups to the specified quality level and save state)");
		UE_LOGF(LogConsoleResponse, Display, "  \"Scalability reapply\" (apply the state of the scalability group (starting with 'sg.') console variables)");
		UE_LOGF(LogConsoleResponse, Display, "  \"Scalability auto\" (Run synth benchmark and adjust the scalability levels for your system and save state)");
	}

	if (bPrintCurrentSettings)
	{
		UE_LOGF(LogConsoleResponse, Display, "Current Scalability Settings:");
		...
		{
			UE_LOGF(LogConsoleResponse, Display, "CPU benchmark value: %f", CPUBenchmarkValue);
		}
		if (GPUBenchmarkValue >= 0.0f)
		{
			UE_LOGF(LogConsoleResponse, Display, "GPU benchmark value: %f", GPUBenchmarkValue);
		}
	}
}
...
#if WITH_EDITOR
	else if (GEditor && GEditor->IsPreviewPlatformActive())
	{
		EConsoleVariableFlags MaxPriority = ECVF_SetByPreview;
		EConsoleVariableFlags MinPriority = ECVF_SetByScalability;
...
int32 FQualityLevels::GetSingleQualityLevel() const
{
	...
	if ((Target == AntiAliasingQuality) && (Target == ShadowQuality) && (Target == GlobalIlluminationQuality) && (Target == ReflectionQuality) && (Target == PostProcessQuality) && (Target == TextureQuality) && (Target == EffectsQuality) && (Target == FoliageQuality) && (Target == ShadingQuality) && (Target == LandscapeQuality))
	{
