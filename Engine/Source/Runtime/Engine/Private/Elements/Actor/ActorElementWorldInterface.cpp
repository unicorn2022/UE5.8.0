	...
	FCollisionQueryParams& InOutParams, FTransform& OutSuitableTransform)
{
	...
	if (ensure(Registry))
	{
		...
		{
			if (TTypedElement<ITypedElementWorldInterface> WorldInterface = Registry->GetElement<ITypedElementWorldInterface>(ElementToIgnore))
			{
				WorldInterface.AddIgnoredElementToCollisionQueryParams(InOutParams);
			}
		}
	}
...
void UActorElementWorldInterface::AddIgnoredCollisionQueryElement(const FTypedElementHandle& InElementHandle, FCollisionQueryParams& InOutParams)
{
	...
	if (TTypedElement<ITypedElementWorldInterface> WorldInterface = Registry->GetElement<ITypedElementWorldInterface>(InElementHandle))
	{
		// Preserving legacy behavior, where we didn't add child elements:
		const bool bAlsoIgnoreSubElements = false;

		WorldInterface.AddIgnoredElementToCollisionQueryParams(InOutParams, bAlsoIgnoreSubElements);
	}
}
