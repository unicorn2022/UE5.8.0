...
#define LOCTEXT_NAMESPACE "SkeletalMesh"
...
void FScopedSkeletalMeshPostEditChange::SetSkeletalMesh(USkeletalMesh* InSkeletalMesh)
{
	...
	if (SkeletalMesh->StackPostEditChange() == 1)
	{
		...
		if (bCallPostEditChange)
		{
			SkeletalMesh->GetOnPreMeshChange().Broadcast();
...
void USkeletalMesh::UpdateSkeletalMaterialsInfoCache(const TArray<FSkeletalMaterial>& InMaterials)
{
	FScopeLock Lock(&MaterialsInfoCriticalSection);
	SkeletalMaterialsInfoCache.Reset();
	for (int32 Index = 0; Index < InMaterials.Num(); Index++)
	{
		...
		SkeletalMaterialsInfoCache.Add(FSkeletalMaterialMinimalInfo(MaterialSlotName, MaterialObjectPath));
	}
}
...
const FString USkeletalMesh::ExportSkeletalMaterialsInfo() const
{
	FScopeLock Lock(&MaterialsInfoCriticalSection);
	FString OutText;
	for (const FSkeletalMaterialMinimalInfo& Slot : SkeletalMaterialsInfoCache)
	{
...
const TArray<FSkeletalMaterialMinimalInfo> USkeletalMesh::ImportSkeletalMaterialsInfo(const FString& InData)
{
	TArray<FSkeletalMaterialMinimalInfo> OutArray;
	if (!InData.IsEmpty())
	{
		TArray<FString> Slot;
		...
		for (int32 Index = 0; Index < NumPairs; Index++)
		{
			...
			if (NumEntries == 2)
			{
				...
				OutArray.Add(FSkeletalMaterialMinimalInfo(FName(Pair[0]), Pair[1]));
			}
		}
	}
	return OutArray;
...
void USkeletalMesh::SetMaterials(const TArray<FSkeletalMaterial>& InMaterials)
{
	...
	UpdateSkeletalMaterialsInfoCache(Materials);
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
}
	...
	for (const FNaniteAssemblyPart& PartData : NaniteSettings.NaniteAssemblyData.Parts)
	{
		...
		if (Ref == nullptr)
		{
			UE_LOGF(
				...
				"Failed to load skeletal mesh %ls when precaching Nanite Assembly references for %ls.",
...
void USkeletalMesh::CacheClothDependencies()
{
	ClothDependencyCache.Reset();
	for (UClothingAssetBase* const ClothingAsset : GetMeshClothingAssets())
	{
		if (USkinnedAsset* const Dependency = ClothingAsset ? ClothingAsset->GetSkinnedAssetDependency() : nullptr)
		{
			ClothDependencyCache.AddUnique(Dependency);
		}
	}
}
...
{
	return IsNaniteAssembly() || ClothDependencyCache.Num();
}
...
bool USkeletalMesh::HasAnyDependenciesCompiling() const
{
	...
	for (const TWeakObjectPtr<USkinnedAsset>& Dependency : ClothDependencyCache)
	{
		if (USkinnedAsset* const ResolvedDependency = Dependency.Get())
		{
			if (ResolvedDependency && ResolvedDependency->IsCompiling())
			{
				return true;
			}
		}
...
TArray<USkinnedAsset*> USkeletalMesh::GetSkinnedAssetDependencies() const
{
	...
	Deps.Reserve(AssemblyReferenceCache.Num() + ClothDependencyCache.Num());
...
{
	check(IsInGameThread());
	if (UClothingAssetBase* const Asset = GetSectionClothingAsset(InLodIndex, InSectionIndex))
	{
		Asset->UnbindFromSkeletalMesh(this, InLodIndex, InSectionIndex);
		TArray<UClothingAssetBase*> ClothingAssetsInUse;
		GetClothingAssetsInUse(ClothingAssetsInUse);
		if (!ClothingAssetsInUse.Contains(Asset))
		{
	...
	if (SkelMeshRenderData)
	{
		...
		if (GetImportedModel())
		{
			for (int32 LODIndex = 0; LODIndex < GetLODNum(); ++LODIndex)
			{
				...
				checkf(ImportLODModel.Sections.Num() == RenderLODModel.RenderSections.Num(), TEXT("Mismatch between import and render LOD model section count for %s LOD %d. %d vs %d"),
					*GetPathName(), LODIndex, ImportLODModel.Sections.Num(), RenderLODModel.RenderSections.Num());
...
{
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	static const FName bSupportRayTracingPropName = GET_MEMBER_NAME_CHECKED(USkeletalMesh, bSupportRayTracing);
	static const FName RayTracingMinLODPropName = GET_MEMBER_NAME_CHECKED(USkeletalMesh, RayTracingMinLOD);
	static const FName ClothLODBiasModePropName = GET_MEMBER_NAME_CHECKED(USkeletalMesh, ClothLODBiasMode);
	static const FName PhysicsAssetPropName = GET_MEMBER_NAME_CHECKED(USkeletalMesh, PhysicsAsset);
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	if (GIsEditor &&
		PropertyThatChanged &&
		(PropertyThatChanged->GetFName() == bSupportRayTracingPropName ||
		 PropertyThatChanged->GetFName() == RayTracingMinLODPropName ||
		 PropertyThatChanged->GetFName() == ClothLODBiasModePropName))
	{
	...
	if( GIsEditor &&
		PropertyThatChanged &&
		PropertyThatChanged->GetFName() == PhysicsAssetPropName )
	{
	...
	{
		FScopedSkeletalMeshReregisterContexts ReregisterContext(this);
	}
	...
	for (UAssetUserData* Datum : GetAssetUserDataEditorOnly())
	{
		if (Datum != nullptr)
		{
			Datum->PostEditChangeOwner(PropertyChangedEvent);
	...
	UpdateSkeletalMaterialsInfoCache(GetMaterials());
...
void USkeletalMesh::PreEditUndo()
{
	...
	// Remove any cached linkup in case the reference skeleton changes
	if (USkeleton* Skel = GetSkeleton())
	{
		Skel->RemoveLinkup(this);
	}
...
EDataValidationResult USkeletalMesh::IsDataValid(FDataValidationContext& Context) const
{
	...
	if (GetPackage()->HasAnyPackageFlags(PKG_Cooked) == false)
	{
		...
		else
		{
			...
			{
				Context.AddError(FText::Format(LOCTEXT("SkeletalMeshValidation_IncompatibleSkeleton", "This skeletal mesh asset has an incompatible Skeleton '{0}'. Assign a compatible skeleton to this skeletal mesh asset."), FText::FromString(GetPathNameSafe(GetSkeleton()))));
				ValidationResult = EDataValidationResult::Invalid;
			}
		}
...
void USkeletalMesh::Build()
{
	...
	if (FSkinnedAssetCompilingManager::Get().IsAsyncCompilationAllowed(this))
	{
		...
		int64 RequiredMemory = 1024 * 1024 * static_cast<int64>(CVarMemoryForSkeletalMeshAssetCompile.GetValueOnAnyThread());
		AsyncTask = MakeUnique<FSkinnedAssetAsyncBuildTask>(this, MoveTemp(Context));
		if (!HasAnyDependenciesCompiling())
		{
			AsyncTask->StartBackgroundTask(SkeletalMeshThreadPool, BasePriority, EQueuedWorkFlags::DoNotRunInsideBusyWait, RequiredMemory, TEXT("SkeletalMesh"));
		}
...
void USkeletalMesh::BeginBuildInternal(FSkinnedAssetBuildContext& Context)
{
	...
	CacheClothDependencies();

	// Make sure the InlineReductionCacheDatas structure has enough room for all available LODs. Since the reduction can happen on multiple threads
	// this has to happen before we trigger the mesh build.
	GetImportedModel()->InlineReductionCacheDatas.SetNum(GetNumSourceModels());
...
FEvent* USkeletalMesh::LockPropertiesUntil()
{
	...
	int64 RequiredMemory = 1024ll * 1024ll * static_cast<int64>(CVarMemoryForSkeletalMeshAssetCompile.GetValueOnAnyThread());
	...
	if(GetRefBasesInvMatrix().Num() != NumRealBones)
	{
		...
		for( int32 b=0; b<NumRealBones; b++)
		{
				...
				ZAxis.IsNearlyZero(UE_SMALL_NUMBER))
			{
				...
				UE_LOGF(LogSkeletalMesh, Warning, "Reference Pose for asset %ls for joint (%ls) includes NIL matrix. Zero scale isn't allowed on ref pose. ", *GetPathName(), *GetRefSkeleton().GetBoneName(b).ToString());
			}
	...
	)
{
			...
			for (const FName& MorphTargetName: ValidMorphTargetNames)
			{
				if (!ExistingMorphTargetNames.Contains(MorphTargetName))
				{
					...
					if (Obj)
					{
						...
						{
							UE_LOGF(LogSkeletalMesh, Error, "Skeletal Mesh (%ls) : Commit mesh description, cannot create a morph target name %ls because a subobject of class %ls already exist", *GetPathName(), *(MorphTargetName.ToString()), *(Obj->GetClass()->GetName()));
						}
					}
					else
					{
	...
	if (ensure(GetImportedModel()->LODModels.IsValidIndex(InLODIndex)))
	{
		...
		LODModel.RawSkeletalMeshBulkDataID = BulkData.GetUnversionedIdString();
...
void USkeletalMesh::Clear()
{
	if (!GetMorphTargets().IsEmpty())
	{
		UnregisterAllMorphTarget();
	}

#if WITH_EDITORONLY_DATA
	if (FSkeletalMeshModel* ImportedResource = GetImportedModel())
	{
	...
	GetMaterials().Empty();
	GetRefSkeleton().Empty();
	SetSkeleton(nullptr);
	SetPhysicsAsset(nullptr);
	GetRefBasesInvMatrix().Empty();
	GetCachedComposedRefPoseMatrices().Empty();
	GetMeshOnlySocketList().Empty();
	GetNodeMappingData().Empty();
	GetMeshClothingAssets().Empty();
	SetHasActiveClothingAssets(false);
	GetSkinWeightProfiles().Empty();
...
bool USkeletalMesh::HasHalfEdgeBuffer(int32 LODIndex) const
{
	...
	if (GetTargetMeshDeformers())
	{
		for (const TSoftObjectPtr<UMeshDeformer>& MeshDeformer : GetTargetMeshDeformers()->GetMeshDeformers())
		{
...
void USkeletalMesh::BeginPostLoadInternal(FSkinnedAssetPostLoadContext& Context)
{
	...
	// Update the clothing asset dependencies, this is used to access the cloth skinned asset dependencies during the async build
	CacheClothDependencies();

	// Ensure inner skinned asset dependencies are PostLoaded
	// Notes: This is so they register with the compiling manager before this mesh enters the dependency queue.
	//        Unlike the ConditionalPostLoad above, these are standalone assets whose RF_NeedsPostLoad flag is reliably managed by the linker.
	for (const TWeakObjectPtr<USkinnedAsset>& Dependency : ClothDependencyCache)
	{
		if (USkinnedAsset* const ResolvedDependency = Dependency.Get())
		{
			ResolvedDependency->ConditionalPostLoad();
		}
	}
...
void USkeletalMesh::PostLoad()
{
	...
	// Rebuild the ref skeleton to match the skeleton
	if (GetSkeleton())
	{
		GetSkeleton()->ConditionalPostLoad();
...
#if WITH_EDITOR
void USkeletalMesh::PrepareDerivedDataForActiveTargetPlatforms()
{
	TRACE_CPUPROFILER_EVENT_SCOPE(USkeletalMesh::PrepareDerivedDataForActiveTargetPlatforms);
	...
	for (int32 PlatformIndex = 0; PlatformIndex < TargetPlatforms.Num(); ++PlatformIndex)
	{
		...
		if (Platform != RunningPlatform)
		{
			...
			constexpr bool bIsSerializeSaving = false;
			::GetPlatformSkeletalMeshRenderData(this, Platform, bIsSerializeSaving);
		}
	}
}
#endif
	...
	if (GetMeshClothingAssets().Num() > 0)
	{
		...
		for (UClothingAssetBase* MeshClothingAsset : GetMeshClothingAssets())
		{
			...
			if (!bFound)
			{
				...
				MeshClothingAsset->UnbindFromSkeletalMesh(this, INDEX_NONE, INDEX_NONE);
			}
		}
	}
#endif // WITH_EDITOR
...
void USkeletalMesh::GetAssetRegistryTags(FAssetRegistryTagsContext Context) const
{
	...
	Context.AddTag(FAssetRegistryTag(GetMaterialsMemberName(), ExportSkeletalMaterialsInfo(), FAssetRegistryTag::TT_Hidden));
	...
	if (GetImportedModel())
	{
		...
		{
			// Note: the max bone influences value may be affected by project settings such as r.GPUSkin.UnlimitedBoneInfluences.
...
void USkeletalMesh::GetAssetRegistryTagMetadata(TMap<FName, FAssetRegistryTagMetadata>& OutMetadata) const
{
	...
	OutMetadata.Add("NaniteEnabled",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("USkeletalMesh", "NaniteEnabled_Label", "Nanite Enabled"))
		);

	OutMetadata.Add("NaniteTriangles",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("USkeletalMesh", "NaniteTriangles_Label", "Nanite Triangles"))
		);

	OutMetadata.Add("NaniteVertices",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("USkeletalMesh", "NaniteVertices_Label", "Nanite Vertices"))
		);

	OutMetadata.Add("Vertices",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("USkeletalMesh", "Vertices_Label", "Vertices"))
		);

	OutMetadata.Add("Triangles",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("USkeletalMesh", "Triangles_Label", "Triangles"))
		);

	OutMetadata.Add("LODs",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("USkeletalMesh", "LODs_Label", "LODs"))
		);

	OutMetadata.Add("Bones",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("USkeletalMesh", "Bones_Label", "Bones"))
		);

	OutMetadata.Add("MorphTargets",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("USkeletalMesh", "MorphTargets_Label", "Morph Targets"))
		);

	OutMetadata.Add("SkinWeightProfiles",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("USkeletalMesh", "SkinWeightProfiles_Label", "Skin Weight Profiles"))
		);

	OutMetadata.Add("EstTotalCompressedSize",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("USkeletalMesh", "EstTotalCompressedSize_Label", "Estimated Total Compressed Size"))
		);

	OutMetadata.Add("EstNaniteCompressedSize",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("USkeletalMesh", "EstNaniteCompressedSize_Label", "Estimated Nanite Compressed Size"))
		);

	OutMetadata.Add("MaxBoneInfluences",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("USkeletalMesh", "MaxBoneInfluences_Label", "Max Bone Influences"))
		);

	OutMetadata.Add("PhysicsSize",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("USkeletalMesh", "PhysicsSize_Label", "Physics Size"))
		);

	OutMetadata.Add("PhysicsAsset",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("USkeletalMesh", "PhysicsAsset_Label", "Physics Asset"))
...
void USkeletalMesh::DebugVerifySkeletalMeshLOD()
{
	...
	if (NumLODs > 1)
	{
		for(int32 LODIndex = 1; LODIndex < NumLODs; LODIndex++)
		{
			...
			{
				// too small
				UE_LOGF(LogSkeletalMesh, Warning, "SkelMeshLOD (%ls) : ScreenSize for LOD %d may be too small (%0.5f)", *GetPathName(), LODIndex, DefaultScreenSize);
			}
		}
	}
	else
	{
		// no LODInfo
		UE_LOGF(LogSkeletalMesh, Warning, "SkelMeshLOD (%ls) : LOD does not exist", *GetPathName());
	}
}
...
bool USkeletalMesh::RegisterMorphTarget(UMorphTarget* MorphTarget, bool bInvalidateRenderData)
{
	if ( MorphTarget )
	{
		...
		for ( int32 Index = 0; Index < RegisteredMorphTargets.Num(); ++Index )
		{
			...
			{
				UE_LOGF( LogSkeletalMesh, Verbose, "RegisterMorphTarget: %ls already exists, replacing", *MorphTarget->GetName() );
...
void USkeletalMesh::UnregisterMorphTarget(UMorphTarget* MorphTarget, bool bInvalidateRenderData)
{
	if ( MorphTarget )
	{
		...
		UE_LOGF( LogSkeletalMesh, Log, "UnregisterMorphTarget: %ls not found.", *MorphTarget->GetName() );
	}
}
...
#if WITH_EDITOR
bool USkeletalMesh::RemoveMorphTargets(TConstArrayView<FName> InMorphTargetNames, bool bInRebuildRenderMesh)
{
	...
	for(FName MorphTargetName : InMorphTargetNames)
	{
		...
		if(MorphTarget)
		{
			...
			bool bMorphTargetRemovedFromMesh = false;
			for (int32 LODIndex = 0; LODIndex < GetNumSourceModels(); LODIndex++)
			{
				if (HasMeshDescription(LODIndex))
				{
					...
					if (MeshAttributes.GetMorphTargetNames().Contains(MorphTargetName))
					{
						...
						bMorphTargetRemovedFromMesh = true;
					}
				}
			}
			
			if (!bMorphTargetRemovedFromMesh)
			{
	...
	if (bRemoved && bInRebuildRenderMesh)
	{
		PostEditChange();
...
bool USkeletalMesh::RenameMorphTarget(FName InOldName, FName InNewName)
{
	...
	MorphTarget->Rename(*InNewName.ToString(), nullptr, REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
...
void USkeletalMesh::AddSocket(USkeletalMeshSocket* InSocket, bool bAddToSkeleton /*= false*/)
{
	...
	{
		UE_LOGF(LogSkeletalMesh, Error, "Failed to add socket as the socket its outer should be %ls but is %ls.", *GetFullName(), *InSocket->GetOuter()->GetFullName());
		return;
	}
	...
	else
	{
		...
		}))
		{
			...
			UE_LOGF(LogSkeletalMesh, Error, "Failed to add socket as a socket with name %ls already exist.", *InSocket->BoneName.ToString());
			return;
		}
	}
	...
	{
		UE_LOGF(LogSkeletalMesh, Error, "Failed to add socket as the provided bone name %ls does not exist.", *InSocket->BoneName.ToString());
		return;
	}
...
void USkeletalMesh::ValidateBoneWeights(const ITargetPlatform* TargetPlatform)
{
	if (TargetPlatform->SupportsFeature(ETargetPlatformFeatures::MobileRendering))
	{
		...
		for (int32 LODIndex = 0; LODIndex < GetLODNum(); ++LODIndex)
		{
			...
			{
				UE_LOGF(LogSkeletalMesh, Warning, "Mesh: %ls has more than 12 max bone influences, it has: %d", *GetFullName(), MaxBoneInfluences);
			}
		}
	}
}
...
bool USkeletalMesh::IsCachedCookedPlatformDataLoaded(const ITargetPlatform* TargetPlatform)
{
	if (IsCompiling())
	{
		return false;
...
FString USkeletalMesh::BuildDerivedDataKey(const ITargetPlatform* TargetPlatform)
{
	...
	//Add the max gpu bone per section (deprecated, kept for DDC key backward compatibility)
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	...
	if (IsNaniteEnabled())
	{
		...
		KeySuffix += TEXT("_NSK_WIP_2");

		static FString CachedNaniteVersion = *Nanite::IBuilderModule::Get().GetVersionString();
	...
	// Invalidate the DDC for meshes that use the "Optimize For Instancing" (unified bone map)
	// path so they rebuild with the unified-bone-map-aware RequiredBones/reparent-target fixes.
	// Only append when the setting is enabled on any LOD so existing meshes that don't use
	// unified bone maps keep their current DDC key and aren't forced to rebuild.
	{
		bool bAnyLODOptimizeForInstancing = false;

		for (int32 LODIndex = 0; LODIndex < GetLODNum(); ++LODIndex)
		{
			if (const FSkeletalMeshLODInfo* LODInfo = GetLODInfo(LODIndex))
			{
				if (LODInfo->BuildSettings.bOptimizeForInstancing)
				{
					bAnyLODOptimizeForInstancing = true;
					break;
				}
			}
		}

		if (bAnyLODOptimizeForInstancing)
		{
			KeySuffix += TEXT("_UBM_REPARENT_00");
		}
	}
...
void USkeletalMesh::RemoveMeshSection(int32 InLodIndex, int32 InSectionIndex)
{
	...
	{
		UE_LOGF(LogSkeletalMesh, Warning, "Failed to remove skeletal mesh section, ImportedResource is invalid.");
		return;
	}
	...
	{
		UE_LOGF(LogSkeletalMesh, Warning, "Failed to remove skeletal mesh section, LOD%d does not exist in the mesh", InLodIndex);
		return;
	}
	...
	{
		UE_LOGF(LogSkeletalMesh, Warning, "Failed to remove skeletal mesh section, Section %d does not exist in LOD%d.", InSectionIndex, InLodIndex);
		return;
	}
	...
	if(UserSectionToDisableData.HasClothingData())
	{
		...
		UE_LOGF(LogSkeletalMesh, Warning, "Failed to remove skeletal mesh section, clothing is currently bound to Lod%d Section %d, unbind clothing before removal.", InLodIndex, InSectionIndex);
		return;
	}
...
bool USkeletalMesh::RemoveSkinWeightProfiles(TConstArrayView<FName> InProfileNames, bool bInRebuildRenderMesh)
{
	if (Algo::NoneOf(GetSkinWeightProfiles(), [InProfileNames](const FSkinWeightProfileInfo& Profile) { return InProfileNames.Contains(Profile.Name); }))
	{
		return false;
	}
	
	FScopedTransaction Transaction(LOCTEXT("RemoveSkinWeightProfiles", "Remove Skin Weight Profiles"));
	
	Modify();

	// Only update skin weight profile info.
	FCommitMeshDescriptionParams CommitParams;
	CommitParams.bUpdateMorphTargets = false;
	CommitParams.bUpdateVertexAttributes = false;
	CommitParams.bUpdateVertexColors = false;
	
	bool bSkinWeightProfileRemovedFromMesh = false;
	for (int32 LODIndex = 0; LODIndex < GetNumSourceModels(); LODIndex++)
	{
		if (HasMeshDescription(LODIndex))
		{
			// Remove the skin weight profile from the editable mesh. The render mesh will be generated from it.
			FMeshDescription* MeshDescription = GetMeshDescription(LODIndex);
			FSkeletalMeshAttributes MeshAttributes(*MeshDescription);
			...
			{
				if (MeshAttributes.GetSkinWeightProfileNames().Contains(ProfileName))
				{
					if (!bNeedsCommit)
					{
						ModifyMeshDescription(LODIndex);
					}
					MeshAttributes.UnregisterSkinWeightAttribute(ProfileName);
					bNeedsCommit = true;
				}
			}
			...
			{
				CommitMeshDescription(LODIndex, CommitParams);
				bSkinWeightProfileRemovedFromMesh = true;
			}
		}
	}
	
	if (bSkinWeightProfileRemovedFromMesh && bInRebuildRenderMesh)
	{
		PostEditChange();
		FSkinnedAssetCompilingManager::Get().FinishCompilation({ this });		
	}
	
	return bSkinWeightProfileRemovedFromMesh;
}
...
{
	// Mark every component using this mesh dirty so its render state is
	// recreated, dropping cached pointers into the profile buffers
	// (FSkeletalMeshObjectLOD::MeshObjectWeightBuffer, FVertexFactory::Streams,
	// cached FMeshDrawCommands) before we release them.
	{
		FObjectCacheContextScope ObjectCacheScope;
		for (ISkinnedMeshComponent* Component :
			 ObjectCacheScope.GetContext().GetSkinnedMeshComponents(this))
		{
...
void USkeletalMesh::SetRefSkeleton(const FReferenceSkeleton& InRefSkeleton)
{
	...
	// Remove any cached linkup we might have as it might no longer be valid
	if (USkeleton* Skel = GetSkeleton())
	{
		Skel->RemoveLinkup(this);
	}
...
void USkeletalMeshSocket::SetSocketParent(USkeletalMesh* InSkeletalMesh, FName InBoneName)
{
	if (!InSkeletalMesh)
	{
		UE_LOGF(LogSkeletalMesh, Error, "SetSocketParent: No skeletal mesh asset given.");
		return;
	}
	...
	if (GetOuter() == InSkeletalMesh)
	{
		...
		{
			UE_LOGF(LogSkeletalMesh, Error, "SetSocketParent: The owning skeletal asset (%ls) does not contain any bone named '%ls'.",
	...
	else if (GetOuter() == InSkeletalMesh->GetSkeleton())
	{
		...
		{
			UE_LOGF(LogSkeletalMesh, Error, "SetSocketParent: The owning skeleton (%ls) does not contain any bone named '%ls'.",
				*InSkeletalMesh->GetName(), *BoneName.ToString());
			return;
		}
	}
	...
	{
		UE_LOGF(LogSkeletalMesh, Error, "SetSocketParent: Neither the skeletal asset (%ls) nor its skeleton are the owners of this socket (%ls).",
