...
{
	constexpr const TCHAR* InstancedStructTypeFieldName = TEXT("_structType");
...
{
	// FInstancedStruct: emit a runtime type marker and recurse into the inner struct. 
	if (InStruct == TBaseStructure<FInstancedStruct>::Get())
	{
		const FInstancedStruct* Instanced = static_cast<const FInstancedStruct*>(InStructData);
		if (const UScriptStruct* RuntimeStruct = Instanced->GetScriptStruct())
		{
			if (const uint8* RuntimeMemory = Instanced->GetMemory())
			{
				JsonWriter->WriteValue(UE::DataTableJSON::InstancedStructTypeFieldName, RuntimeStruct->GetPathName());
				return WriteStructUsingFieldIterator(RuntimeStruct, RuntimeMemory, FieldToSkip);
			}
		}
	...
	, const FString* FieldToSkip)
{
	...
	// We always push override for structs (when UseJsonObjectsForStructs is set in the export flags) and arrays 
	// so an empty override list or an unset value on top of the stack
	...
	auto PushFunc = [this, &IdentifierOverrides](const FPropertyVisitorInfo& InVisitorInfo)
		{
			...
			else if (Property->IsA<FStructProperty>())
			{
				if (EnumHasAnyFlags(DTExportFlags, EDataTableExportFlags::UseJsonObjectsForStructs))
				{
					...
					// Push a "no override" entry when entering a struct to export it as json object. If UseJsonObjectsForStructs is not used the struct is saved as string value.
	...
	auto PopFunc = [this, &IdentifierOverrides](const FPropertyVisitorInfo& InVisitorInfo)
		{
			...
			else if (Property->IsA<FStructProperty>())
			{
				if (EnumHasAnyFlags(DTExportFlags, EDataTableExportFlags::UseJsonObjectsForStructs))
				{
					// Only close object and pop override if saving structs as json objects. If not they are saved as string values so no need to close it
					JsonWriter->WriteObjectEnd();
					IdentifierOverrides.Pop();
				}
			}
	...
	InStruct->Visit(Context,
		[FieldToSkip, this, &IdentifierOverrides](const FPropertyVisitorContext& InContext)-> EPropertyVisitorControlFlow
		{
			...
			// FInstancedStruct: emit a "_structType" marker at the current level and recurse into the struct
			if (const FStructProperty* StructProp = CastField<const FStructProperty>(Property))
			{
				if (StructProp->Struct == TBaseStructure<FInstancedStruct>::Get()
					&& EnumHasAnyFlags(DTExportFlags, EDataTableExportFlags::UseJsonObjectsForStructs))
				{
					const FInstancedStruct* Instanced = static_cast<const FInstancedStruct*>(InContext.Data.PropertyData);
					if (const UScriptStruct* RuntimeStruct = Instanced->GetScriptStruct())
					{
						JsonWriter->WriteValue(InstancedStructTypeFieldName, RuntimeStruct->GetPathName());
						if (Instanced->GetMemory())
						{
							WriteStructUsingVisitor(RuntimeStruct, Instanced->GetMemory(), /*FieldToSkip=*/nullptr);
						}
					}
					return EPropertyVisitorControlFlow::StepOver;
				}
...
bool FDataTableImporterJSON::ReadRow(const TSharedRef<FJsonObject>& InParsedTableRowObject, const int32 InRowIdx, bool bRemoveDuplicate, FName* OutRowName)
{
	...
	if (!DataTable->bIgnoreExtraFields)
	{
		...
		for (const TPair<FJsonObject::FStringType, TSharedPtr<FJsonValue>>& ParsedPropertyKeyValuePair : InParsedTableRowObject->Values)
		{
			if (FStringView(ParsedPropertyKeyValuePair.Key) == RowKey)
			{
				// Skip the row name, as that doesn't match a property
				continue;
			}

			FName PropName = DataTableUtils::MakeValidName(FString(ParsedPropertyKeyValuePair.Key));
			...
			for (TFieldIterator<FProperty> It(DataTable->RowStruct); It && !ColumnProp; ++It)
			{
				...
				ColumnProp = TempPropertyImportNames.Contains(FString(ParsedPropertyKeyValuePair.Key)) ? *It : nullptr;
			}
...
{
	// FInstancedStruct: resolve the struct type from the "_structType" marker
	// emitted by the exporter, then recurse into the inner struct.
	if (InStruct == TBaseStructure<FInstancedStruct>::Get())
	{
		FInstancedStruct* Instanced = static_cast<FInstancedStruct*>(InStructData);
		FString StructPathName;
		if (!InParsedObject->TryGetStringField(UE::DataTableJSON::InstancedStructTypeFieldName, StructPathName)
			|| StructPathName.IsEmpty())
		{
			// No marker - treat as empty InstancedStruct
			Instanced->Reset();
			return true;
		}

		// Honor core redirects
		FCoreRedirectObjectName OldName(StructPathName);
		...
		if (OldName != NewName)
		{
			StructPathName = NewName.ToString();
		}

		UScriptStruct* StructTypePtr = LoadObject<UScriptStruct>(nullptr, *StructPathName);
		if (!StructTypePtr)
		{
			ImportProblems.Add(FString::Printf(TEXT("Row '%s' InstancedStruct type '%s' could not be resolved."),
				*InRowName.ToString(), *StructPathName));
			Instanced->Reset();
			return false;
		}
		Instanced->InitializeAs(StructTypePtr);
		return ReadStruct(InParsedObject, StructTypePtr, InRowName, Instanced->GetMutableMemory());
	}
	...
	for (TFieldIterator<FProperty> It(InStruct); It; ++It)
	{
		...
		if (!ParsedPropertyValue.IsValid())
		{
			...
			static const FName DataTableImportOptionalMetadataKey("DataTableImportOptional");
