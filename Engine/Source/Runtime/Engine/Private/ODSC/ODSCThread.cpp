...
{
	UE_LOGF(LogODSC, Display, "Received response in %lf seconds.", FPlatformTime::Seconds() - RequestStartTime);
...
FODSCThread::FODSCThread(const FString& HostIP)
	...
	, ODSCHostIP(HostIP)
{
	UE_LOGF(LogODSC, Log, "ODSC Thread active.");
...
bool FODSCThread::ConnectToODSCHost()
{
	...
	{
		UE_LOGF(LogODSC, Warning, "Failed to connect to cook on the fly server.");
		return false;
	}
...
bool FODSCThread::CheckODSCConnection()
{
	...
	if ((CookOnTheFlyServerConnection == nullptr) || (!CookOnTheFlyServerConnection->IsConnected()))
	{
		...
		UE_LOGF(LogODSC, Display, "Detected that CookOnTheFlyServerConnection has been lost, trying again");
...
bool FODSCThread::SendMessageToServer(IPlatformFile::IFileServerMessageHandler* Handler)
{
	...
	{
		UE_LOGF(LogODSC, Display, "Received error response from CookOnTheFlyServerConnection; disconnecting");
