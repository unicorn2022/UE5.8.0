...
int64 UChannel::Close(EChannelCloseReason Reason)
{
	...
	if ( !Closing && ( Connection->GetConnectionState() == USOCK_Open || Connection->GetConnectionState() == USOCK_Pending || (Connection->GetConnectionState() == USOCK_Closing && ChIndex == 0)) && !SentClosingBunch)
	{
		if ( ChIndex == 0 )
		{
			UE_LOGF(LogNet, Log, "UChannel::Close: Sending CloseBunch. ChIndex == 0. Name: %ls", *Describe());
		}

		UE_LOGF(LogNetDormancy, Verbose, "UChannel::Close: Sending CloseBunch. Reason: %ls, %ls", LexToString(Reason), *Describe());
...
bool UChannel::CleanUp(const bool bForDestroy, EChannelCloseReason CloseReason)
{
	...
	{
		UE_LOGF(LogNet, Log, "UChannel::CleanUp: ChIndex == 0. Closing connection. %ls", *Describe());
	...
	if (OutRec != NULL && !Connection->IsInternalAck())
	{
		...
		//UE_LOGF(LogNetTraffic, Log, "%i save pending out bunch %i",ChIndex,Connection->PendingOutRec[ChIndex]);
	}
...
bool UChannel::ReceivedAcks(EChannelCloseReason& OutCloseReason)
{
	...
	*/
	...
	while( OutRec && OutRec->ReceivedAck )
	{
		if (OutRec->bOpen)
		{
			...
			if (OutRec->bPartial)
			{
				...
				while (OpenBunch)
				{
					UE_LOGF(LogNet, VeryVerbose, "   Channel %i open partials %d ackd %d final %d ", ChIndex, OpenBunch->PacketId, OpenBunch->ReceivedAck, OpenBunch->bPartialFinal);
			...
			if (OpenFinished)
			{
				UE_LOGF(LogNet, VeryVerbose, "Channel %i is fully acked. PacketID: %d", ChIndex, OutRec->PacketId );
				OpenAcked = 1;
			}
			else
			{
	...
	if (OpenTemporary && OpenAcked)
	{
		...
		UE_LOGF(LogNetDormancy, Verbose, "ReceivedAcks: Cleaning up after close acked. CloseReason: %ls %ls", LexToString(CloseReason), *Describe());
	...
	else if (bCleanup)
	{
		...
		UE_LOGF(LogNetDormancy, Verbose, "ReceivedAcks: Channel queued for cleaning up after close acked. CloseReason: %ls %ls", LexToString(CloseReason), *Describe());
...
bool UChannel::ReceivedSequencedBunch( FInBunch& Bunch )
{
	...
	if( Bunch.bClose )
	{
		...
		if ( ChIndex == 0 )
		{
			UE_LOGF(LogNet, Log, "UChannel::ReceivedSequencedBunch: Bunch.bClose == true. ChIndex == 0. Calling ConditionalCleanUp." );
		}

		UE_LOGF(LogNetTraffic, Log, "UChannel::ReceivedSequencedBunch: Bunch.bClose == true. Calling ConditionalCleanUp. ChIndex: %i Reason: %ls", ChIndex, LexToString(Bunch.CloseReason));
...
void UChannel::ReceivedRawBunch( FInBunch & Bunch, bool & bOutSkipAck )
{
	...
	if ( Bunch.bHasPackageMapExports && !Connection->IsInternalAck() )
	{
		...
		if ( Bunch.IsError() )
		{
			...
			UE_LOGF( LogNetTraffic, Error, "UChannel::ReceivedRawBunch: Bunch.IsError() after ReceiveNetGUIDBunch. ChIndex: %i", ChIndex );
	...
	if ( Connection->IsInternalAck() && Broken )
	{
		...
		{
			UE_LOGF(LogNetTraffic, Log, "Replay handling a close bunch for a broken channel: %ls", *Bunch.ToString());
	...
	if ( Bunch.bReliable && Bunch.ChSequence != Connection->InReliable[ChIndex] + 1 )
	{
		...
		{
			UE_LOGF(LogNetPartialBunch, Warning, "New Pessimistic Nak Performed on %ls", *Bunch.ToString());
			bOutSkipAck = true;
		}

		// Find the place for this item, sorted in sequence.
		UE_LOGF(LogNetTraffic, Log, "      Queuing bunch with unreceived dependency: %d / %d", Bunch.ChSequence, Connection->InReliable[ChIndex]+1 );
		FInBunch** InPtr;
		...
		*InPtr        = New;
		...
		if ( NumInRec >= RELIABLE_BUFFER )
		{
			...
			UE_LOGF( LogNetTraffic, Error, "UChannel::ReceivedRawBunch: Too many reliable messages queued up" );
	...
	else
	{
		...
		if ( Bunch.IsError() )
		{
			UE_LOGF( LogNetTraffic, Error, "UChannel::ReceivedRawBunch: Bunch.IsError() after ReceivedNextBunch 1" );
		...
		while( InRec )
		{
			...
			if( InRec->ChSequence!=Connection->InReliable[ChIndex]+1 )
				break;
			UE_LOGF(LogNetTraffic, Log, "      Channel %d Unleashing queued bunch", ChIndex );
			...
			if ( bLocalSkipAck )
			{
				UE_LOGF( LogNetTraffic, Warning, "UChannel::ReceivedRawBunch: bLocalSkipAck == true for already acked packet" );
			}

			if ( Bunch.IsError() )
			{
				UE_LOGF( LogNetTraffic, Error, "UChannel::ReceivedRawBunch: Bunch.IsError() after ReceivedNextBunch 2" );
	...
	if (Bunch.bPartial)
	{
			...
			if (InPartialBunch != NULL)
			{
				if (!InPartialBunch->bPartialFinal)
				{
					if ( InPartialBunch->bReliable )
					{
						if ( Bunch.bReliable )
						{
							UE_LOGF(LogNetPartialBunch, Warning, "Reliable partial trying to destroy reliable partial 1. %ls", *Describe());
						...
						UE_LOGF(LogNetPartialBunch, Log, "Unreliable partial trying to destroy reliable partial 1" );
						bOutSkipAck = true;
						return false;
					}

					// We didn't complete the last partial bunch - this isn't fatal since they can be unreliable, but may want to log it.
					UE_LOGF(LogNetPartialBunch, Verbose, "Incomplete partial bunch. Channel: %d ChSequence: %d", InPartialBunch->ChIndex, InPartialBunch->ChSequence);
				}
		...
		else
		{
			...
			if ( InPartialBunch && !InPartialBunch->bPartialFinal && bSequenceMatches && InPartialBunch->bReliable == Bunch.bReliable )
			{
				...
				UE_LOGF(LogNetPartialBunch, VeryVerbose, " %ls", *Bunch.ToString() );
				...
				if (Bunch.bPartialFinal)
				{
					...
					if ( Bunch.bHasPackageMapExports )
					{
						...
						UE_LOGF(LogNetPartialBunch, Warning, "Corrupt partial bunch. Final partial bunch has package map exports. %ls", *Describe());
			...
			else
			{
				...
				if ( InPartialBunch && InPartialBunch->bReliable )
				{
					if ( Bunch.bReliable )
					{
						UE_LOGF(LogNetPartialBunch, Warning, "Reliable partial trying to destroy reliable partial 2. %ls", *Describe());
					...
					UE_LOGF( LogNetPartialBunch, Log, "Unreliable partial trying to destroy reliable partial 2" );
					return false;
				}
				...
				if (UE_LOG_ACTIVE(LogNetPartialBunch,Verbose)) // Don't want to call appMemcrc unless we need to
				{
					...
					{
						UE_LOGF(LogNetPartialBunch, Verbose, "Received Partial Bunch Out of Sequence when InPartialBunch was NULL!");
					}
				}
		...
		{
			UE_LOGF(LogNetPartialBunch, Verbose, "Partial CustomExports Bunch (ChSequence %d PacketId %d) is being processed.", Bunch.ChSequence, Bunch.PacketId);
			...
			if (InPartialBunch && !Connection->IsInternalAck())
			{
				...
				if (InPartialBunch->IsError())
				{
					...
					UE_LOGF(LogNetTraffic, Error, "UChannel::ReceivedRawBunch: Bunch.IsError() after ReceiveCustomExportsBunch. ChIndex: %i", ChIndex);
					return false;
				}
	...
	if ( HandleBunch != NULL )
	{
		...
		if ( HandleBunch->bOpen )
		{
			if ( !bBothSidesCanOpen )	// Voice channels can open from both side simultaneously, so ignore this logic until we resolve this
			{
				...
				{
					UE_LOGF( LogNetTraffic, Error, "Received channel open command for channel that was already opened locally. %ls", *Describe() );
			...
			UE_LOGF( LogNetTraffic, Verbose, "ReceivedNextBunch: Channel now fully open. ChIndex: %i, OpenPacketId.First: %i, OpenPacketId.Last: %i", ChIndex, OpenPacketId.First, OpenPacketId.Last );
		}
		...
		if ( !bBothSidesCanOpen )	// Voice channels can open from both side simultaneously, so ignore this logic until we resolve this
		{
			...
			if ( !OpenedLocally && !OpenAcked )
			{
				...
				{
					UE_LOGF( LogNetTraffic, Error, "ReceivedNextBunch: Reliable bunch before channel was fully open. ChSequence: %i, OpenPacketId.First: %i, OpenPacketId.Last: %i, bPartial: %i, %ls", Bunch.ChSequence, OpenPacketId.First, OpenPacketId.Last, ( int32 )HandleBunch->bPartial, *Describe() );
				...
				UE_LOGF( LogNetTraffic, Verbose, "ReceivedNextBunch: Skipping bunch since channel isn't fully open. ChIndex: %i", ChIndex );
				return false;
			}
...
FPacketIdRange UChannel::SendBunch( FOutBunch* Bunch, bool Merge )
{
	...
	if ((GCVarNetPartialBunchReliableThreshold > 0) && (OutgoingBunches.Num() >= GCVarNetPartialBunchReliableThreshold) && !Connection->IsInternalAck())
	{
		...
		{
			UE_LOGF(LogNetPartialBunch, Log, "	OutgoingBunches.Num (%d) exceeds reliable threashold (%d). Making bunches reliable. Property replication will be paused on this channel until these are ACK'd.", OutgoingBunches.Num(), GCVarNetPartialBunchReliableThreshold);
		...
		else
		{
			...
			UE_LOGF(LogNetPartialBunch, Warning, "	OutgoingBunches.Num (%d) exceeds reliable threashold (%d) but this would overflow the reliable buffer! Consider sending less stuff. Channel: %ls", OutgoingBunches.Num(), GCVarNetPartialBunchReliableThreshold, *Describe());
		}
	}

	if (Bunch->bReliable && bOverflowsReliable)
	{
		UE_LOGF(LogNetPartialBunch, Warning, "SendBunch: Reliable partial bunch overflows reliable buffer! %ls", *Describe() );
		UE_LOGF(LogNetPartialBunch, Warning, "   Num OutgoingBunches: %d. NumOutRec: %d", OutgoingBunches.Num(), NumOutRec );
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
	...
	UE_CLOGF((OutgoingBunches.Num() > 1), LogNetPartialBunch, Log, "Sending %d Bunches. Channel: %d %ls", OutgoingBunches.Num(), Bunch->ChIndex, *Describe());
	for( int32 PartialNum = 0; PartialNum < OutgoingBunches.Num(); ++PartialNum)
	{
		...
		if (UE_LOG_ACTIVE(LogNetPartialBunch,Verbose) && (OutgoingBunches.Num() > 1)) // Don't want to call appMemcrc unless we need to
		{
			...
			UE_LOGF(LogNetPartialBunch, VeryVerbose, "	 %ls", *ThisOutBunch->GetDebugString());
		}
...
FOutBunch* UChannel::PrepBunch(FOutBunch* Bunch, FOutBunch* OutBunch, bool Merge)
{
	...
	if( Bunch->bReliable )
	{
		...
		if( OutBunch==NULL )
		{
			...
			{
				UE_LOGF(LogNetTraffic, Warning, "PrepBunch: Reliable buffer overflow! %ls", *Describe());
				PRAGMA_DISABLE_DEPRECATION_WARNINGS
				PrintReliableBunchBuffer();
				PRAGMA_ENABLE_DEPRECATION_WARNINGS
			}
		...
		{
			UE_LOGF(LogNetTraffic, Warning, "%ls. Reliable: %ls", *Describe(), *Bunch->DebugString);
		}
...
void UChannel::PrintReliableBunchBuffer()
{
	...
	for( FOutBunch* Out=OutRec; Out; Out=Out->Next )
	{
		UE_LOGF(LogNetTraffic, Warning, "Out: %ls", *Out->DebugString );
	}
	UE_LOGF(LogNetTraffic, Warning, "-------------------------\n");
#endif
}
...
IMPLEMENT_CONTROL_CHANNEL_MESSAGE(IrisNetRefHandleErrorWithDiagnosticMessage);
IMPLEMENT_CONTROL_CHANNEL_MESSAGE(IrisProtocolMismatchWithCDOState);
...
bool UControlChannel::CheckEndianess(FInBunch& Bunch)
{
	...
	if (Bunch.GetNumBytes() >= 2)
	{
		if (HelloMessage[0] == NMT_Hello)
		{
			...
			UE_LOGF(LogNet, Log, "Remote platform little endian=%d", int32(OtherPlatformIsLittle));
			UE_LOGF(LogNet, Log, "This platform little endian=%d", int32(IsLittleEndian));
...
void UControlChannel::ReceivedBunch( FInBunch& Bunch )
{
	...
	while (!Bunch.AtEnd() && bStopReadingBunch == false && Connection != nullptr && Connection->GetConnectionState() != USOCK_Closed) // if the connection got closed, we don't care about the rest
	{
			...
			if (Connection->Driver->ServerConnection == NULL)
			{
				...
				if (FNetControlMessage<NMT_ActorChannelFailure>::Receive(Bunch, ChannelIndex))
				{
					...
					if (ChannelIndex >= 0 && ChannelIndex < Connection->Channels.Num())
					{
						...
						UE_CLOGF(ActorChan != nullptr, LogNet, Log, "Actor channel failed: %ls", *ActorChan->Describe());
						...
						if (ActorChan != nullptr && ActorChan->Actor != nullptr)
						{
							...
							{
								UE_LOGF(LogNet, Warning, "UControlChannel::ReceivedBunch: NetConnection::Close() [%ls] [%ls] [%ls] from failed to initialize the PlayerController channel. Closing connection.",
									Connection->Driver ? *Connection->Driver->NetDriverName.ToString() : TEXT("NULL"),
									Connection->PlayerController ? *Connection->PlayerController->GetName() : TEXT("NoPC"),
									Connection->OwningActor ? *Connection->OwningActor->GetName() : TEXT("No Owner"));
							...
							{
								//UE_LOGF(LogNet, Warning, "UControlChannel::ReceivedBunch: PlayerController doesn't exist for the client, but the client is reporting an actor channel failure that isn't the PlayerController.");
							}
						}
					}
					...
					{
						UE_LOGF(LogNet, Warning, "UControlChannel::ReceivedBunch: The client is sending an actor channel failure message with an invalid actor channel index.");
					}
				}
			}
		}
		...
		else if (MessageType == NMT_SecurityViolation)
		{
			...
			{
				UE_LOGF(LogSecurity, Warning, "%ls: Closed: %ls", *Connection->RemoteAddressToString(), *DebugMessage);
		...
		else if (MessageType == NMT_DestructionInfo)
		{
			...
			{
				UE_LOGF(LogNet, Warning, "UControlChannel::ReceivedBunch: Server received a NMT_DestructionInfo which is not supported. Closing connection.");
		...
		else if (MessageType == NMT_IrisProtocolMismatchWithCDOState)
		{
			uint64 NetRefHandleId = 0;
			TArray<uint8> ClientCDOStateBytes;
			if (FNetControlMessage<NMT_IrisProtocolMismatchWithCDOState>::Receive(Bunch, NetRefHandleId, ClientCDOStateBytes))
			{
				if (UReplicationSystem* IrisRepSystem = Connection->Driver->GetReplicationSystem())
				{
					IrisRepSystem->ReportProtocolMismatch(NetRefHandleId, Connection->GetConnectionHandle().GetParentConnectionId(), ClientCDOStateBytes);
				}
			}
		}
		else if (MessageType == NMT_IrisProtocolMismatch)
		{
			...
			if (FNetControlMessage<NMT_IrisProtocolMismatch>::Receive(Bunch, NetRefHandleId))
			{
				if (UReplicationSystem* IrisRepSystem = Connection->Driver->GetReplicationSystem())
				{
					IrisRepSystem->ReportProtocolMismatch(NetRefHandleId, Connection->GetConnectionHandle().GetParentConnectionId(), TArray<uint8>());
				}
			}
		}
		...
		else if (MessageType == NMT_IrisNetRefHandleErrorWithDiagnosticMessage)
		{
			UE::Net::ENetRefHandleError NetRefHandleError = UE::Net::ENetRefHandleError::None;
			uint64 NetRefHandleId = 0;
			TArray<uint64> ExtraNetRefs;
			FString DiagnosticMessage;
			if (FNetControlMessage<NMT_IrisNetRefHandleErrorWithDiagnosticMessage>::Receive(Bunch, NetRefHandleError, NetRefHandleId, ExtraNetRefs, DiagnosticMessage))
			{
				if (UReplicationSystem* IrisRepSystem = Connection->Driver->GetReplicationSystem())
				{
					IrisRepSystem->ReportErrorWithNetRefHandle(NetRefHandleError, NetRefHandleId, Connection->GetConnectionHandle().GetParentConnectionId(), ExtraNetRefs, DiagnosticMessage);
				}
			}
		}
		...
		if (Pos == Bunch.GetPosBits() && !Bunch.IsError())
		{
			switch (MessageType)
			{
				...
				case NMT_CloseChildConnection:
					...
					break;
				case NMT_IrisNetRefHandleErrorWithDiagnosticMessage:
					FNetControlMessage<NMT_IrisNetRefHandleErrorWithDiagnosticMessage>::Discard(Bunch);
					break;
				case NMT_IrisProtocolMismatchWithCDOState:
					FNetControlMessage<NMT_IrisProtocolMismatchWithCDOState>::Discard(Bunch);
					break;
				default:
					...
					UE_LOGF(LogNet, Log, "Received unknown control channel message %i. Closing connection.", int32(MessageType));
...
void UControlChannel::QueueMessage(const FOutBunch* Bunch)
{
	...
	if (QueuedMessages.Num() >= MAX_QUEUED_CONTROL_MESSAGES)
	{
		...
		UE_LOGF(LogNet, Log, "Overflowed control channel message queue, disconnecting client");
...
FPacketIdRange UControlChannel::SendBunch(FOutBunch* Bunch, bool Merge)
{
	...
	else
	{
		...
		else
		{
			...
			UE_LOGF(LogNet, Error, "Control channel bunch overflowed");
	...
	if( !OpenAcked )
	{
		...
		for( FOutBunch* Out=OutRec; Out; Out=Out->Next )
		{
			if( !Out->ReceivedAck )
			{
				...
				if (Wait > 1.0)
				{
					UE_LOGF(LogNetTraffic, Log, "Channel %i ack timeout); resending %i...", ChIndex, Out->ChSequence );
					check(Out->bReliable);
					Connection->SendRawBunch( *Out, 0 );
				}
			}
		...
		while (QueuedMessages.Num() > 0 && !Closing)
		{
			...
			else
			{
				...
				else
				{
					...
					UE_LOGF(LogNet, Error, "Control channel queued bunch overflowed");
...
void UActorChannel::Init( UNetConnection* InConnection, int32 InChannelIndex, EChannelCreateFlags CreateFlags )
{
	...
	UE_LOGF(LogNetLifecycle, Log, "UActorChannel::Init: ChIndex: %d Actor: %ls", ChIndex, *GetFullNameSafe(Actor));
}
...
{
	UE_LOGF(LogNetLifecycle, Log, "UActorChannel::Close: ChIndex: %d Actor: %ls", ChIndex, *GetFullNameSafe(Actor));

	FScopedRepContext RepContext(Connection, Actor);

	UE_LOGF(LogNetTraffic, Log, "UActorChannel::Close: ChIndex: %d, Actor: %ls, Reason: %ls", ChIndex, *GetFullNameSafe(Actor), LexToString(Reason));
...
{
	UE_LOGF(LogNetLifecycle, Log, "UActorChannel::CleanUp: ChIndex: %d Actor: %ls", ChIndex, *GetFullNameSafe(Actor));
	...
	UE_LOGF( LogNetTraffic, Log, "UActorChannel::CleanUp (ChIndex: %i): %ls | NetGuid: %ls | Reason: %ls", ChIndex, *GetFullNameSafe(Actor), *ActorNetGUID.ToString(), LexToString(CloseReason));
...
#endif // #if DO_CHECK
			{
				UE_LOGF(LogNet, VeryVerbose, "UActorChannel::CleanUp: Adding to KeepProcessingActorChannelBunchesMap. Channel: %i, Num: %i", ChIndex, Connection->KeepProcessingActorChannelBunchesMap.Num());
	...
	if (!bIsServer)
	{
		...
		if (IReplicatedObjectInterface* ReplicatedObject = Cast<IReplicatedObjectInterface>(Actor))
		{
			if (CloseReason == EChannelCloseReason::Relevancy)
			{
				...
				{
					UE_LOGF(LogNetTraffic, VeryVerbose, "UActorChannel::CleanUp: Closed for ReplicatedObject Relevancy. %ls - [%ls] FullNetGUIDPath: %ls", *Describe(), *ActorNetGUID.ToString(), *Connection->Driver->GetNetGuidCache()->FullNetGUIDPath(ActorNetGUID));
					bHandled = true;
				}
			}
		}
		...
		if (!bHandled && Actor != nullptr)
		{
			...
			{
				UE_LOGF(LogNetTraffic, Verbose, "UActorChannel::CleanUp: Closed for TearOff but actor flag not set, last property update may have been dropped: %ls", *Describe());
			...
			{
				UE_LOGF(LogNetTraffic, Verbose, "UActorChannel::CleanUp: Skipped Destroying NetStartupActor %ls due to EChannelCloseReason::LevelUnloaded", *Describe());
			}
			...
			{
				UE_LOGF(LogNetDormancy, Verbose, "UActorChannel::CleanUp: Destroying Actor (ChIndex: %i): %ls", ChIndex, *GetFullNameSafe(Actor));
...
void UActorChannel::ReceivedNak( int32 NakPacketId )
{
	...
	if (Info)
	{
		...
		{
			UE_LOGF(LogNetTraffic, Verbose, "ActorChannel[%d]: Reseting object keys due to Nak: %d", ChIndex, NakPacketId);
			for (auto It = Info->ObjKeys.CreateIterator(); It; ++It)
			{
				SubobjectRepKeyMap.FindOrAdd(*It) = INDEX_NONE;
				UE_LOGF(LogNetTraffic, Verbose, "    %d", *It);
			}
		}
	}
...
void UActorChannel::SetChannelActor(AActor* InActor, ESetChannelActorFlags Flags)
{
	...
	if (World && InActor)
	{
		...
		{
			UE_LOGF(LogNet, Verbose, "UActorChannel::SetChannelActor: actor %ls is not in the same level collection as the net driver (%ls)!", *GetFullNameSafe(InActor), *GetFullNameSafe(Connection->Driver));
		}
	}
	...
	UE_LOGF(LogNetTraffic, VeryVerbose, "SetChannelActor (ChIndex: %i): Actor: %ls | NetGUID: %ls", ChIndex, *GetFullNameSafe(Actor), *ActorNetGUID.ToString() );
	...
	if (Connection->PendingOutRec.IsValidIndex(ChIndex) && Connection->PendingOutRec[ChIndex] > 0)
	{
		...
		// UE_LOGF(LogNetTraffic, Log, "%i Actor %ls WILL BE sending %i vs first %i", ChIndex, *Actor->GetName(), Connection->PendingOutRec[ChIndex],Connection->OutReliable[ChIndex]);
		...
		{
			// UE_LOGF(LogNetTraffic, Log, "%i SYNCHRONIZING by sending %i", ChIndex, Connection->PendingOutRec[ChIndex]);
			...
			else
			{
				...
				UE_LOGF(LogNetTraffic, Fatal, "SetChannelActor failed. Overflow while sending reliable bunch synchronization.");
				break;
			}
		}
...
bool UActorChannel::ShouldProcessAllQueuedBunches(float CurrentTimeSeconds)
{
	...
	if (bExceededFailsafeTime)
	{
		...
		UE_LOGF(LogNet, Verbose, "UActorChannel::ProcessQueuedBunches hit frame time failsafe after %.3f seconds. Processing entire queue of %d bunches for channel: %ls", SecondsWithQueuedBunches, QueuedBunches.Num(), *Describe());
	}
...
bool UActorChannel::ProcessQueuedBunches()
{
	...
	for (auto It = PendingGuidResolves.CreateIterator(); It; ++It)
	{
		...
		else if (GuidCache->IsGUIDBroken(*It, true))
		{
			...
			UE_LOGF(LogNet, Warning, "UActorChannel::ProcessQueuedBunches: Guid is broken. NetGUID: %ls, ChIndex: %i, Actor: %ls", *It->ToString(), ChIndex, *GetPathNameSafe(Actor));
			It.RemoveCurrent();
		}
	}
	...
	if (bHasTimeToProcess)
	{
			...
			&& !Connection->Driver->ShouldQueueBunchesForActorGUID(ActorNetGUID))
		{
			...
			UE_LOGF(LogNet, VeryVerbose, "UActorChannel::ProcessQueuedBunches: Flushing %i of %i queued bunches. ChIndex: %i, Actor: %ls", BunchIndex, QueuedBunches.Num(), ChIndex, Actor != NULL ? *Actor->GetPathName() : TEXT("NULL"));
		...
		else
		{
			if ((FPlatformTime::Seconds() - QueuedBunchStartTime) > UE::Net::QueuedBunchTimeoutSeconds)
			{
				...
				{
					UE_LOGF(LogNet, Warning, "UActorChannel::ProcessQueuedBunches: Queued bunches for longer than normal. ChIndex: %i, Actor: %ls, Queued: %i, PendingGuidResolves: %i", ChIndex, *GetPathNameSafe(Actor), QueuedBunches.Num(), PendingGuidResolves.Num());

					if (UE_LOG_ACTIVE(LogNet, Log))
					{
						for (const FNetworkGUID& Guid : PendingGuidResolves)
						{
							UE_LOGF(LogNet, Log, "  PendingGuidResolve [%ls] %ls", *Guid.ToString(), *GuidCache->Describe(Guid));

							const FNetworkGUID OuterGuid = GuidCache->GetOuterNetGUID(Guid);
							UE_LOGF(LogNet, Log, "      OuterGuid [%ls] %ls", *OuterGuid.ToString(), *GuidCache->Describe(OuterGuid));
						}
					}
				}
...
void UActorChannel::ReceivedBunch( FInBunch & Bunch )
{
	...
	if (Connection->Driver->IsServer())
	{
		if (Bunch.bHasMustBeMappedGUIDs)
		{
			UE_LOGF(LogNetTraffic, Error, "UActorChannel::ReceivedBunch: Client attempted to set bHasMustBeMappedGUIDs. Actor: %ls", *GetNameSafe(Actor));
	...
	else
	{
		if (Bunch.bHasMustBeMappedGUIDs)
		{
			...
			//UE_LOGF( LogNetTraffic, Warning, "Read must be mapped GUID's. NumMustBeMappedGUIDs: %i", NumMustBeMappedGUIDs );
			...
			for (int32 i = 0; i < NumMustBeMappedGUIDs; i++)
			{
				...
				else
				{
					...
					UE_LOGF(LogNet, Warning, "UActorChannel::ReceivedBunch: Received a MustBeMappedGUID that is not registered. ChIndex: %i NetGUID: %ls Channel: %ls Bunch: %ls", ChIndex, *NetGUID.ToString(), *Describe(), *Bunch.ToString());
			...
			Connection->Driver->HasExceededIncomingBunchFrameProcessingTime())
		{
			...
			{
				UE_LOGF(LogNet, Verbose, "UActorChannel::ReceivedBunch: Queuing bunch because another channel (that closed) is processing bunches for this guid still. ActorNetGUID: %ls", *ActorNetGUID.ToString());
			}
...
void UActorChannel::ProcessBunchInternal(FInBunch & Bunch)
{
	...
	if( Actor == NULL )
	{
		if( !Bunch.bOpen )
		{
			...
			UE_LOGF(LogNetTraffic, Error, "UActorChannel::ProcessBunch: New actor channel received non-open packet. bOpen: %i, bClose: %i, bReliable: %i, bPartial: %i, bPartialInitial: %i, bPartialFinal: %i, ChName: %ls, ChIndex: %i, Closing: %i, OpenedLocally: %i, OpenAcked: %i, NetGUID: %ls", (int)Bunch.bOpen, (int)Bunch.bClose, (int)Bunch.bReliable, (int)Bunch.bPartial, (int)Bunch.bPartialInitial, (int)Bunch.bPartialFinal, *ChName.ToString(), ChIndex, (int)Closing, (int)OpenedLocally, (int)OpenAcked, *ActorNetGUID.ToString() );
			return;
		}
		...
		if (!IsValid(NewChannelActor))
		{
			...
			if (!bSpawnedNewActor && Bunch.bReliable && Bunch.bClose && Bunch.AtEnd())
			{
				...
				{
					UE_LOGF(LogNet, Verbose, "UActorChannel::ProcessBunch: SerializeNewActor received close bunch for destroyed actor. Actor: %ls, Channel: %i", *GetFullNameSafe(NewChannelActor), ChIndex);
				}
			...
			UE_LOGF(LogNet, Warning, "UActorChannel::ProcessBunch: SerializeNewActor failed to find/spawn actor. Actor: %ls, Channel: %i, FullNetGUIDPath: %ls", *GetFullNameSafe(NewChannelActor), ChIndex, *Connection->Driver->GetNetGuidCache()->FullNetGUIDPath(ActorNetGUID));
			Broken = 1;
		...
		{
			UE_LOGF(LogNetLifecycle, Log, "UActorChannel::ProcessBunch: Creating ChIndex: %d Actor: %ls", ChIndex, *NewChannelActor->GetName());

			if (UE::Net::bDiscardTornOffActorRPCs && NewChannelActor->GetTearOff())
			{
				UE_LOGF(LogNet, Warning, "UActorChannel::ProcessBunch: SerializeNewActor received an open bunch for a torn off actor. Actor: %ls, Channel: %i", *GetFullNameSafe(NewChannelActor), ChIndex);
				Broken = 1;
		...
		UE_LOGF(LogNetTraffic, Log, "      Channel Actor %ls:", *NewChannelActor->GetFullName());
	...
	{
		UE_LOGF(LogNetTraffic, Log, "      Actor %ls:", *Actor->GetFullName() );
	}
	...
	while ( !Bunch.AtEnd() && Connection != NULL && Connection->GetConnectionState() != USOCK_Closed )
	{
		...
		if ( Bunch.IsError() )
		{
			if ( Connection->IsInternalAck() )
			{
				UE_LOGF( LogNet, Warning, "UActorChannel::ReceivedBunch: ReadContentBlockPayload FAILED. Bunch.IsError() == TRUE. (IsInternalAck) Breaking actor. RepObj: %ls, Channel: %i", RepObj ? *RepObj->GetFullName() : TEXT( "NULL" ), ChIndex );
				Broken = 1;
				break;
			}

			UE_LOGF( LogNet, Error, "UActorChannel::ReceivedBunch: ReadContentBlockPayload FAILED. Bunch.IsError() == TRUE. Closing connection. RepObj: %ls, Channel: %i", RepObj ? *RepObj->GetFullName() : TEXT( "NULL" ), ChIndex );
		...
		if ( !IsValid(RepObj) )
		{
			if ( !IsValid(Actor) )
			{
				...
				UE_LOGF( LogNet, Warning, "UActorChannel::ProcessBunch: ReadContentBlockPayload failed to find/create ACTOR. RepObj: %ls, Channel: %i", RepObj ? *RepObj->GetFullName() : TEXT( "NULL" ), ChIndex );
				Broken = 1;
			}
			else
			{
				UE_LOGF( LogNet, Warning, "UActorChannel::ProcessBunch: ReadContentBlockPayload failed to find/create object. RepObj: %ls, Channel: %i", RepObj ? *RepObj->GetFullName() : TEXT( "NULL" ), ChIndex );
			}
		...
		if ( !IsValid(Actor) )
		{
			UE_LOGF( LogNet, VeryVerbose, "UActorChannel::ProcessBunch: Actor was destroyed during Replicator.ReceivedBunch processing" );
	...
	{
		CSV_SCOPED_TIMING_STAT_EXCLUSIVE(PostNetInit);
		DECLARE_SCOPE_CYCLE_COUNTER(TEXT("ActorChan_PostNetInit"), Stat_PostNetInit, STATGROUP_Net);
...
void UActorChannel::ProcessBunch(FInBunch& Bunch)
{
	...
	{
		static FName TransactionalWorkName("UActorChannel::ProcessBunch");
		...
		if (!bIsNewlyReplicationPaused)
		{
			...
			if (PendingObjKeys.Num() > 0)
			{
				...
				for (int32 PacketId = PacketRange.First; PacketId <= PacketRange.Last; ++PacketId)
				{
					...
					{
						UE_LOGF(LogNetTraffic, Verbose, "ActorChannel[%d]: Clearing out PacketRepKeyInfo for new packet: %d", ChIndex, PacketId);
					...
					if (UE_LOG_ACTIVE(LogNetTraffic, Verbose))
					{
						...
						UE_LOGF(LogNetTraffic, Verbose, "ActorChannel[%d]: Sending ObjKeys: %ls", ChIndex, *VerboseString);
					}
				}
			}
#endif // NET_ENABLE_SUBOBJECT_REPKEYS
...
bool UActorChannel::UpdateDeletedSubObjects(FOutBunch& Bunch)
{
	...
	auto DeleteSubObject = [this, &bWroteSomethingImportant, &Bunch](FNetworkGUID ObjectNetGUID, const TWeakObjectPtr<UObject>& ObjectPtrToRemove, ESubObjectDeleteFlag DeleteFlag)
	{
		if (ObjectNetGUID.IsValid())
		{
			...
			UE_LOGF(LogNetSubObject, Verbose, "NetSubObject: Sending request to %ls %ls::%ls (0x%p) NetGUID %ls", ToString(DeleteFlag), *Actor->GetName(), *GetNameSafe(ObjectPtrToRemove.GetEvenIfUnreachable()), ObjectPtrToRemove.GetEvenIfUnreachable(), *ObjectNetGUID.ToString());
		...
		{
			UE_LOGF(LogNetTraffic, Error, "Unable to write subobject delete for %ls::%ls (0x%p), object replicator has invalid NetGUID", *GetPathNameSafe(Actor), *GetNameSafe(ObjectPtrToRemove.GetEvenIfUnreachable()), ObjectPtrToRemove.GetEvenIfUnreachable());
		}
	};
...
bool UActorChannel::ValidateReplicatedSubObjects()
{
	...
	for (int32 i=DataChannelInternal::LegacySubObjectsCollected.Num()-1; i >= 0; i--)
	{
		...
		UE_CLOGF(!bWasFound && bLogAllErrors, LogNetSubObject, Warning, "%ls was not replicated by the subobject list.", *Info.Describe(Actor));
	...
	for (int32 i=DataChannelInternal::ReplicatedSubObjectsTracker.Num()-1; i >= 0; i--)
	{
		...
		ensureMsgf(false, TEXT("%s was replicated only in the registered subobject list in %s. Not in the legacy path."),
			*Info.Describe(Actor), *Connection->GetDriver()->NetDriverName.ToString());
		UE_CLOGF(bLogAllErrors, LogNet, Warning, "%ls was replicated only in the registered subobject list. Not in the legacy path.", *Info.Describe(Actor));
	}
...
{
	UE_LOGF(LogNetDormancy, Verbose, "BecomeDormant: %ls", *Describe() );
	bPendingDormancy = false;
	bIsInDormancyHysteresis = false;
	Dormant = true;
...
void UActorChannel::StartBecomingDormant()
{
	...
	UE_LOGF(LogNetDormancy, Verbose, "StartBecomingDormant: %ls", *Describe() );
...
UObject* UActorChannel::ReadContentBlockHeader(FInBunch& Bunch, bool& bObjectDeleted, bool& bOutHasRepLayout)
{
	...
	if (Bunch.IsError())
	{
		UE_LOGF(LogNetTraffic, Error, "UActorChannel::ReadContentBlockHeader: Bunch.IsError() == true after bOutHasRepLayout. Actor: %ls", *Actor->GetName());
	...
	if (Bunch.IsError())
	{
		UE_LOGF(LogNetTraffic, Error, "UActorChannel::ReadContentBlockHeader: Bunch.IsError() == true after reading actor bit. Actor: %ls", *Actor->GetName());
	...
	if (Bunch.IsError())
	{
		UE_LOGF(LogNetTraffic, Error, "UActorChannel::ReadContentBlockHeader: Bunch.IsError() == true after SerializeObject. SubObj: %ls, Actor: %ls", SubObj ? *SubObj->GetName() : TEXT("Null"), *Actor->GetName());
	...
	if (Bunch.AtEnd())
	{
		UE_LOGF(LogNetTraffic, Error, "UActorChannel::ReadContentBlockHeader: Bunch.AtEnd() == true after SerializeObject. SubObj: %ls, Actor: %ls", SubObj ? *SubObj->GetName() : TEXT("Null"), *Actor->GetName());
	...
	if (SubObj)
	{
		...
		{
			UE_LOGF(LogNetTraffic, Error, "UActorChannel::ReadContentBlockHeader: Sub-object not allowed to be actor type. SubObj: %ls, Actor: %ls", *SubObj->GetName(), *Actor->GetName());
		...
		{
			UE_LOGF(LogNetTraffic, Error, "UActorChannel::ReadContentBlockHeader: Actor component not in parent actor. SubObj: %ls, Actor: %ls", *SubObj->GetFullName(), *Actor->GetFullName());
	...
	if (IsServer)
	{
		...
		if (!SubObj)
		{
			UE_LOGF(LogNetTraffic, Error, "ReadContentBlockHeader: Client attempted to create sub-object. Actor: %ls", *Actor->GetName());
	...
	if (Bunch.IsError())
	{
		UE_LOGF(LogNetTraffic, Error, "UActorChannel::ReadContentBlockHeader: Bunch.IsError() == true after reading stably named bit. Actor: %ls", *Actor->GetName());
	...
	if (bStablyNamed)
	{
		...
		if (!SubObj)
		{
			...
			if (!Connection->IsInternalAck())
			{
				UE_LOGF(LogNetTraffic, Log, "ReadContentBlockHeader: Stably named sub-object not found. Its level may have streamed out. Component: %ls, Actor: %ls", *Connection->Driver->GetNetGuidCache()->FullNetGUIDPath(NetGUID), *Actor->GetName());
			}
	...
	if (bDeleteSubObject)
	{
		if (SubObj)
		{
			...
			UE_LOGF(LogNetSubObject, Verbose, "NetSubObject: Client received request to %ls %ls::%ls (0x%p) NetGuid %ls", ToString(DeleteFlag), *Actor->GetName(), *GetNameSafe(SubObj), SubObj, *NetGUID.ToString());
	...
	if (!SubObjClass)
	{
		UE_LOGF(LogNetTraffic, Warning, "UActorChannel::ReadContentBlockHeader: Unable to read sub-object class. Actor: %ls", *Actor->GetName());
		...
		if (!SubObj)
		{
			...
			if (!Connection->IsInternalAck())
			{
				UE_LOGF(LogNetTraffic, Error, "UActorChannel::ReadContentBlockHeader: Unable to read sub-object class (SubObj == NULL). Actor: %ls", *Actor->GetName());
	...
	else
	{
		...
		{
			UE_LOGF(LogNetTraffic, Error, "UActorChannel::ReadContentBlockHeader: SubObjClass == UObject::StaticClass(). Actor: %ls", *Actor->GetName());
		...
		{
			UE_LOGF(LogNetTraffic, Error, "UActorChannel::ReadContentBlockHeader: Sub-object cannot be actor class. Actor: %ls", *Actor->GetName());
	...
	if (Bunch.EngineNetVer() >= FEngineNetworkCustomVersion::SubObjectOuterChain)
	{
		...
		if (Bunch.IsError())
		{
			UE_LOGF(LogNetTraffic, Error, "UActorChannel::ReadContentBlockHeader: Bunch.IsError() == true after reading actor is outer bit. Actor: %ls", *Actor->GetName());
			return nullptr;
		}
		...
		if (!bActorIsOuter)
		{
			...
			if (Bunch.IsError())
			{
				UE_LOGF(LogNetTraffic, Error, "UActorChannel::ReadContentBlockHeader: Bunch.IsError() == true after serializing the subobject's outer. Actor: %ls", *Actor->GetName());
				return nullptr;
			}
			...
			if (ObjOuter == nullptr && SubObj == nullptr)
			{
				...
				UE_LOGF(LogNetTraffic, Log, "UActorChannel::ReadContentBlockHeader: Unable to serialize subobject's outer, using owning actor as outer instead. Class: %ls, Actor: %ls", *GetNameSafe(SubObjClass), *GetNameSafe(Actor));
	...
	if (!SubObj)
	{
		...
		UE_LOGF( LogNetTraffic, Log, "UActorChannel::ReadContentBlockHeader: Instantiating sub-object. Class: %ls, Actor: %ls, Outer: %ls", *GetNameSafe(SubObjClass), *Actor->GetName(), *ObjOuter->GetName() );
...
UObject* UActorChannel::ReadContentBlockPayload( FInBunch &Bunch, FNetBitReader& OutPayload, bool& bOutHasRepLayout )
{
	...
	if ( Bunch.IsError() )
	{
		UE_LOGF( LogNet, Error, "UActorChannel::ReadContentBlockPayload: ReadContentBlockHeader FAILED. Bunch.IsError() == TRUE. Closing connection. RepObj: %ls, Channel: %i", RepObj ? *RepObj->GetFullName() : TEXT( "NULL" ), ChIndex );
	...
	if ( Bunch.IsError() )
	{
		UE_LOGF( LogNet, Error, "UActorChannel::ReceivedBunch: Read NumPayloadBits FAILED. Bunch.IsError() == TRUE. Closing connection. RepObj: %ls, Channel: %i", RepObj ? *RepObj->GetFullName() : TEXT( "NULL" ), ChIndex );
...
bool UActorChannel::ReadFieldHeaderAndPayload( UObject* Object, const FClassNetCache* ClassCache, FNetFieldExportGroup* NetFieldExportGroup, FNetBitReader& Bunch, const FFieldNetCache** OutField, FNetBitReader& OutPayload ) const
{
	*OutField = nullptr;
	...
	if ( Connection->IsInternalAck() )
	{
		...
		{
			UE_LOGF( LogNet, Warning, "ReadFieldHeaderAndPayload: NetFieldExportGroup was null. Object: %ls", *Object->GetFullName() );
			Bunch.SetError();
			return false;
		}
		...
		if ( Bunch.IsError() )
		{
			UE_LOGF( LogNet, Error, "ReadFieldHeaderAndPayload: Error reading NetFieldExportHandle. Object: %ls", *Object->GetFullName() );
			return false;
		}

		if ( !ensure( NetFieldExportHandle < NetFieldExportGroup->NetFieldExports.Num() ) )
		{
			UE_LOGF( LogRep, Error, "ReadFieldHeaderAndPayload: NetFieldExportHandle too large. Object: %ls, NetFieldExportHandle: %i", *Object->GetFullName(), NetFieldExportHandle );
			Bunch.SetError();
			return false;
		}
		...
		{
			UE_LOGF( LogNet, Error, "ReadFieldHeaderAndPayload: NetFieldExport.CompatibleChecksum was 0. Object: %ls, Property: %ls, NetFieldExportHandle: %i", *Object->GetFullName(), *NetFieldExport.ExportName.ToString(), NetFieldExportHandle );
			Bunch.SetError();
			return false;
		}
		...
		*OutField = ClassCache->GetFromChecksum( NetFieldExport.CompatibleChecksum );
		...
		if ( *OutField == NULL )
		{
			...
			{
				UE_LOGF( LogNet, Warning, "ReadFieldHeaderAndPayload: GetFromChecksum failed (NetBackwardsCompatibility). Object: %ls, Property: %ls", *Object->GetFullName(), *NetFieldExport.ExportName.ToString() );
	...
	else
	{
		...
		if ( Bunch.IsError() )
		{
			UE_LOGF( LogRep, Error, "ReadFieldHeaderAndPayload: Error reading RepIndex. Object: %ls", *Object->GetFullName() );
		...
		{
			UE_LOGF( LogRep, Error, "ReadFieldHeaderAndPayload: RepIndex too large. Object: %ls", *Object->GetFullName() );
		...
		*OutField = ClassCache->GetFromIndex( RepIndex );
		...
		if ( *OutField == NULL )
		{
			UE_LOGF( LogNet, Warning, "ReadFieldHeaderAndPayload: GetFromIndex failed. Object: %ls", *Object->GetFullName() );
		}
	}
	...
	if ( Bunch.IsError() )
	{
		UE_LOGF( LogNet, Error, "ReadFieldHeaderAndPayload: Error reading numbits. Object: %ls, OutField: %ls", *Object->GetFullName(), ( *OutField && (*OutField)->Field ) ? *(*OutField)->Field.GetName() : TEXT( "NULL" ) );
	...
	if ( Bunch.IsError() )
	{
		UE_LOGF( LogNet, Error, "ReadFieldHeaderAndPayload: Error reading payload. Object: %ls, OutField: %ls", *Object->GetFullName(), ( *OutField && (*OutField)->Field ) ? *(*OutField)->Field.GetName() : TEXT( "NULL" ) );
...
TSharedRef<FObjectReplicator>& UActorChannel::CreateReplicator(UObject* Obj, bool bCheckDormantReplicators)
{
	...
	if(NewReplicator.IsValid())
	{
		UE_LOGF(LogNetTraffic, Log, "CreateReplicator - Found existing replicator for (0x%p) %ls - Replicator (0x%p)", Obj , *Obj->GetName(), NewReplicator.Get());
	}
	else
	{
		...
		UE_LOGF(LogNetTraffic, Log, "CreateReplicator - creating new replicator for (0x%p) %ls - Replicator (0x%p)", Obj, *Obj->GetName(), NewReplicator.Get());
...
static void	ListOpenActorChannels( UWorld* InWorld )
{
	...
	for (auto It = Connection->ActorChannelConstIterator(); It; ++It)
	{
		...
		UE_LOGF(LogNet, Warning, "Chan[%d] %ls ", Chan->ChIndex, *Chan->Actor->GetFullName() );
	...
	UE_LOGF(LogNet, Warning, "-----------------------------" );

	for (auto It = ClassMap.CreateIterator(); It; ++It)
	{
		UE_LOGF(LogNet, Warning, "%4d - %ls", It.Value(), *It.Key()->GetName() );
	}
}
...
static void	DeleteDormantActor( UWorld* InWorld )
{
	...
	for (auto It = Connection->Driver->GetNetworkObjectList().GetAllObjects().CreateConstIterator(); It; ++It)
	{
		...
		UE_LOGF(LogNet, Warning, "Deleting actor %ls", *ThisActor->GetName());
...
static void	FindNetGUID( const TArray<FString>& Args, UWorld* InWorld )
{
	for (FThreadSafeObjectIterator ObjIt(UNetDriver::StaticClass()); ObjIt; ++ObjIt)
	{
		...
		{
			UE_LOGF(LogNet, Warning, "FindNetGUID - GuidCacheHistory is not enabled");
			return;
		}
		...
		if (Args.Num() <= 0)
		{
			...
			for (auto It = Driver->GetNetGuidCache()->History.CreateIterator(); It; ++It)
			{
				...
				UE_LOGF(LogNet, Warning, "<%ls> - %ls", *NetGUID.ToString(), *Str);
			}
		}
		else
		{
			...
			if (!Str.IsEmpty())
			{
				UE_LOGF(LogNet, Warning, "Found: %ls", *Str);
			}
			else
			{
				UE_LOGF(LogNet, Warning, "No matches" );
			}
		}
	}
}
...
static void	TestObjectRefSerialize( const TArray<FString>& Args, UWorld* InWorld )
{
	...
	if (!Object)
	{
		UE_LOGF(LogNet, Warning, "Couldn't find object: %ls", *Args[0]);
		return;
	}

	UE_LOGF(LogNet, Warning, "Repping reference to: %ls", *Object->GetName());
