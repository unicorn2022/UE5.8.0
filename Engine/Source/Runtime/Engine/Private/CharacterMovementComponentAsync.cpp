...
/*	if (!HasValidData())
	...
	if (bRotationChanged || (OldBaseLocation != NewBaseLocation))
	{
		...
		// move attached actor
		#if (0)//bFastAttachedMove) // TODO bFastAttachedMove
		{
...
void FCharacterMovementComponentAsyncInput::StartNewPhysics(float deltaTime, int32 Iterations, FCharacterMovementComponentAsyncOutput& Output) const
{
	...
	case MOVE_Custom:
		...
		*/
	default:
		//UE_LOGF(LogCharacterMovement, Warning, "Async Character Movement called with unsupported movement mode %d", int32(Output.MovementMode));
...
void FCharacterMovementComponentAsyncInput::PhysWalking(float deltaTime, int32 Iterations, FCharacterMovementComponentAsyncOutput& Output) const
{
	...
	#if (0)//!CharacterOwner || (!CharacterOwner->Controller && !bRunPhysicsWithNoController && !Output.RootMotionParams.bHasRootMotion && !Output.CurrentRootMotion.HasOverrideVelocity() && (LocalRole != ROLE_SimulatedProxy)))
	{
...
//	SCOPE_CYCLE_COUNTER(STAT_CharPhysFalling);
	...
	while ((remainingTime >= MIN_TICK_TIME) && (Iterations < MaxSimulationIterations))
	{
		...
		// TODO Rootmotion
		//UE_LOGF(LogCharacterMovement, Log, "dt=(%.6f) OldLocation=(%ls) OldVelocity=(%ls) NewVelocity=(%ls)", timeTick, *(UpdatedComponent->GetComponentLocation()).ToString(), *OldVelocity.ToString(), *Velocity.ToString());
...
//	UE_LOGF(LogCharacterMovement, VeryVerbose, "[Role:%d] FindFloor: %ls at location %ls", (int32)CharacterOwner->GetLocalRole(), *GetNameSafe(CharacterOwner), *CapsuleLocation.ToString());
	...
	if (FloorLineTraceDist > 0.f || FloorSweepTraceDist > 0.f)
	{
		...
		else
		{
			...
			#if (0)//!bForceNextFloorCheck && !IsActorBasePendingKill && MovementBase)
			{
...
bool FCharacterMovementComponentAsyncInput::ResolvePenetration(const FVector& ProposedAdjustment, const FHitResult& Hit, const FQuat& NewRotation, FCharacterMovementComponentAsyncOutput& Output) const
{
	...
	//	QUICK_SCOPE_CYCLE_COUNTER(STAT_MovementComponent_ResolvePenetration);
		...
		/*UE_LOGF(LogMovement, Verbose, "ResolvePenetration: %ls.%ls at location %ls inside %ls.%ls at location %ls by %.3f (netmode: %d)",
			*ActorOwner->GetName(),
		...
		if (!bEncroached)
		{
			...
			//UE_LOGF(LogMovement, Verbose, "ResolvePenetration:   teleport by %ls", *Adjustment.ToString());
		...
		else
		{
			...
			//UE_LOGF(LogMovement, Verbose, "ResolvePenetration:   sweep by %ls (success = %d)", *Adjustment.ToString(), bMoved);
			...
			if (!bMoved && SweepOutHit.bStartPenetrating)
			{
				...
				if (SecondMTD != Adjustment && !CombinedMTD.IsZero())
				{
					...
					//UE_LOGF(LogMovement, Verbose, "ResolvePenetration:   sweep by %ls (MTD combo success = %d)", *CombinedMTD.ToString(), bMoved);
				}
			}
			...
			if (!bMoved)
			{
				...
				if (!MoveDelta.IsZero())
				{
					...
					//UE_LOGF(LogMovement, Verbose, "ResolvePenetration:   sweep by %ls (adjusted attempt success = %d)", *(Adjustment + MoveDelta).ToString(), bMoved);
					...
					if (!bMoved && FVector::DotProduct(MoveDelta, Adjustment) > 0.f)
					{
						...
						//UE_LOGF(LogMovement, Verbose, "ResolvePenetration:   sweep by %ls (Original move, attempt success = %d)", *(MoveDelta).ToString(), bMoved);
					}
				}
			}
