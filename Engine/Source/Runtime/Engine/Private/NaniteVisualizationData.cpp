...
void FNaniteVisualizationData::Initialize()
{
	if (!bIsInitialized)
	{
		...
		AddVisualizationMode(TEXT("Curves"), LOCTEXT("Curves", "Curves"), FModeType::Advanced, NANITE_VISUALIZE_CURVES, false);
		...
		if (ICVarOverview)
		{
			...
			if (IsDifferentToCurrentOverviewModeList(OverviewModeList))
			{
				...
				while (OverviewModeList.Len())
				{
					...
					{
						UE_LOGF(LogNaniteVisualization, Warning, "Unknown Nanite visualization mode '%ls'", *Left);
					}
					else
					{
			...
			if (ICVarVisualize)
			{
				...
				if (!ConsoleVisualizationMode.IsEmpty() && !bDisable)
				{
					...
					if (ModeID == INDEX_NONE)
					{
						...
						{
							UE_LOGF(LogNaniteVisualization, Warning, "Unknown Nanite visualization mode '%ls'", *ConsoleVisualizationMode);
						}
					}
					else
					{
