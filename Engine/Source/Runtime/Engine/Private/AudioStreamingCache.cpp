...
static FAutoConsoleCommand GFlushAudioCacheCommand(
		...
		[]()
	{
		...
		UE_LOGF(LogAudioStreamCaching, Display, "Audio Cache Flushed! %f megabytes free.", NumBytesFreed / (1024.0 * 1024.0));
	})
);
		...
		[](const TArray< FString >& Args)
{
	...
	UE_LOGF(LogAudioStreamCaching, Display, "Audio Cache Shrunk! Now set to be %f MB.", InMB);
})
);
		...
		[]()
{
	...
	UE_LOGF(LogAudioStreamCaching, Display, "Enabled profiling mode on the audio stream cache.");
})
);
		...
		[]()
{
	...
	UE_LOGF(LogAudioStreamCaching, Display, "Disabled profiling mode on the audio stream cache.");
})
);

FAudioChunkCache::FChunkKey::FChunkKey(const TSharedRef<const FSoundWaveData>& InSoundWave, uint32 InChunkIndex
#if WITH_EDITOR
...
void FAudioChunkCache::FCacheElement::WaitForAsyncLoadCompletion(bool bCancel)
{
	...
	if (Read.IsPending())
	{
		...
		else
		{
			...
			{
				UE_CLOGF(Attempt % 5 == 0, LogAudioStreamCaching, Warning,
					"Still Waiting for Sound: %ls:%i",*Key.SoundWaveName.ToString(), Key.ChunkIndex);
			}
		}
		Read = {};
	}
...
{
	TSharedPtr<const FSoundWaveData> SoundWaveDataPtr = SoundWaveWeakPtr.Pin();
...
void FAudioChunkCache::FCacheElement::ReleaseRetainedAudioOnSoundWave()
{
	if (TSharedPtr<const FSoundWaveData> SoundWaveDataPtr = SoundWaveWeakPtr.Pin())
	{
		UE_CLOGF(WantsFilteredLogging(), LogAudioTiming, Verbose, "[%ls] - Release Retained Audio, chunk %d, ObjectKey=%ls", *WriteToString<128>(Key.SoundWaveName), Key.ChunkIndex, *WriteToString<40>(Key.ObjectKey));
...
void FCachedAudioStreamingManager::AddForceInlineSoundWave(const TSharedRef<const FSoundWaveData>& SoundWave)
{
...
void FCachedAudioStreamingManager::RemoveForceInlineSoundWave(const TSharedRef<const FSoundWaveData>& SoundWave)
{
...
void FCachedAudioStreamingManager::HandleStarvation(const TSharedRef<const FSoundWaveData>& SoundWaveData, const uint32 ChunkIndex, const uint32 InBytesProduced, const uint32 InBytesExpected) 
{
	// If this sound wave is managed by a cache, use that to get the chunk:
	if (FAudioChunkCache* Cache = GetCacheForWave(SoundWaveData))
	{
		Cache->HandleStarvation(SoundWaveData, ChunkIndex, InBytesProduced, InBytesExpected);
	}
}

FAudioChunkHandle FCachedAudioStreamingManager::GetLoadedChunk(const TSharedRef<const FSoundWaveData>& SoundWaveData, uint32 ChunkIndex, bool bBlockForLoad, bool bForImmediatePlayback) const
{
	LLM_SCOPE(ELLMTag::AudioStreamCache);
	bBlockForLoad |= (ForceBlockForLoadCVar != 0);

	// If this sound wave is managed by a cache, use that to get the chunk:
	FAudioChunkCache* Cache = GetCacheForWave(SoundWaveData);
	if (Cache)
	{
		...
		FAudioChunkCache::FChunkKey ChunkKey(
			...
			, ChunkIndex
#if WITH_EDITOR
			, (uint32)SoundWaveData->GetCurrentChunkRevision()
#endif
		);

		if (!FAudioChunkCache::DoesKeyContainValidChunkIndex(ChunkKey, *SoundWaveData))
		{
			UE_LOGF(LogAudioStreamCaching, Warning, "Invalid Chunk Index %d Requested for Wave %ls!", ChunkIndex, *SoundWaveData->GetFName().ToString());
		...
		UE_CLOGF(bBlockForLoad && !LoadedChunk.GetData(), LogAudioStreamCaching, Display, "Synchronous load of chunk index %d for SoundWave %ls failed to return any data. Likely because the cache was blown.", ChunkIndex, *SoundWaveData->GetFName().ToString());

		// Set the updated cache offset for this chunk index.
		Cache->SetCacheLookupIDForChunk(ChunkKey, LookupIDForChunk);

		UE_CLOGF(!bBlockForLoad && !LoadedChunk.GetData(), LogAudioStreamCaching, Verbose, "GetLoadedChunk called for chunk index %d of SoundWave %ls when audio was not loaded yet. This will result in latency.", ChunkIndex, *SoundWaveData->GetFName().ToString());

		// Finally, if there's a chunk after this in the sound, request that it is in the cache.
		const int32 NextChunk = GetNextChunkIndex(SoundWaveData, ChunkIndex);
		...
		if (NextChunk != INDEX_NONE)
		{
			...
			{
				UE_LOG(LogAudioStreamCaching, Warning, TEXT("Failed to add follow-up chunk for %s. This should not happen with our current TrimMemory() logic"), *SoundWaveData->GetFName().ToString());
			}
			else
			{
		...
		return BuildChunkHandle(LoadedChunk.GetData(), LoadedChunk.Num(), SoundWaveData, SoundWaveData->GetFName(), ChunkIndex, LookupIDForChunk);
	}
	else
	{
		ensureMsgf(false, TEXT("Failed to find cache for wave %s. Are you sure this is a streaming wave?"), *SoundWaveData->GetFName().ToString());
		return FAudioChunkHandle();
	}
}

FAudioChunkCache* FCachedAudioStreamingManager::GetCacheForWave(const TSharedRef<const FSoundWaveData>& InSoundWave) const
{
	LLM_SCOPE(ELLMTag::AudioStreamCache);
...
int32 FCachedAudioStreamingManager::GetNextChunkIndex(const TSharedRef<const FSoundWaveData>& InSoundWaveData, uint32 InCurrentChunkIndex)
{
	LLM_SCOPE(ELLMTag::AudioStreamCache);
	...
	const int32 NumChunksTotal = InSoundWaveData->GetNumChunks();
	if (NumChunksTotal <= 2)
	{
...
bool FCachedAudioStreamingManager::RequestChunk(const TSharedRef<const FSoundWaveData>& SoundWaveData, uint32 ChunkIndex, TFunction<void(EAudioChunkLoadResult)> OnLoadCompleted, ENamedThreads::Type ThreadToCallOnLoadCompletedOn, bool bForImmediatePlayback)
{
	LLM_SCOPE(ELLMTag::AudioStreamCache);
	FAudioChunkCache* Cache = GetCacheForWave(SoundWaveData);
	...
	if (Cache)
	{
		...
		uint64 LookupIDForChunk = Cache->AddOrTouchChunk(ChunkKey, SoundWaveData, OnLoadCompleted, ThreadToCallOnLoadCompletedOn, bForImmediatePlayback);
	...
	else
	{
		...
		// Also legitimately reached during USoundCue::PostLoad when FSoundWaveProxy::PublishedSoundWaveData has not yet been
		// Publish'd in sync with USoundWave::WorkingSoundWaveData — the gate at the call site reads working data, while the
		// data ref we get from the proxy is still the default-initialized published copy. Silently return false in that case.
		return false;
	}
}
	...
	, bLogCacheMisses(false)
{
	...
	{
		UE_LOGF(LogAudioStreamCaching, Display, "Audio stream cache size is 0 bytes. Audio will not play. To resolve this issue, set \'CacheSizeKB\' to a non-zero value.");
	}
}
...
FAudioChunkCache::~FAudioChunkCache()
{
	...
	// Ensure all in-flight loads are cancelled and waited on before the pool is destroyed.
	CachePool.Reset();
...
uint64 FAudioChunkCache::AddOrTouchChunk(const FChunkKey& InKey, const TSharedPtr<FSoundWaveData>& InSoundWaveData, TFunction<void(EAudioChunkLoadResult)> OnLoadCompleted, ENamedThreads::Type CallbackThread, bool bNeededForPlayback)
{
	...
	TSharedRef<const FSoundWaveData> SoundWaveDataRef = InSoundWaveData.ToSharedRef();

	return AddOrTouchChunk(InKey, SoundWaveDataRef, OnLoadCompleted, CallbackThread, bNeededForPlayback);
}

uint64 FAudioChunkCache::AddOrTouchChunk(const FChunkKey& InKey, const TSharedRef<const FSoundWaveData>& InSoundWaveData, TFunction<void(EAudioChunkLoadResult)> OnLoadCompleted, ENamedThreads::Type CallbackThread, bool bNeededForPlayback)
{
	...
	if (FoundElement)
	{
		...
		if (FoundElement->Read.IsPending())
		{
			...
			{
				UE_CLOGF(FoundElement->WantsFilteredLogging(), LogAudioStreamCaching, Verbose,
					"[%ls] - Bumping priority of existing read, now we're in playback), chunk %d, ObjectKey=%ls",
	...
	UE_CLOGF(FoundElement->WantsFilteredLogging(), LogAudioTiming, Verbose, "[%ls] - AddOrTouchChunk(found), chunk %d, ObjectKey=%ls", *WriteToString<128>(FoundElement->Key.SoundWaveName), FoundElement->Key.ChunkIndex, *WriteToString<40>(FoundElement->Key.ObjectKey));
...
{
	if (ensure(InSoundWavePtr.IsValid()))
	{
		TSharedRef<const FSoundWaveData> SoundWaveDataRef = InSoundWavePtr.ToSharedRef();
			return GetChunk(InKey, SoundWaveDataRef, bBlockForLoadCompletion, bNeededForPlayback, OutCacheOffset);
	}

	return {};
}

TArrayView<uint8> FAudioChunkCache::GetChunk(const FChunkKey& InKey, const TSharedRef<const FSoundWaveData>& InSoundWavePtr, bool bBlockForLoadCompletion, bool bNeededForPlayback, uint64& OutCacheOffset)
{
	FScopeLock ScopeLock(&CacheMutationCriticalSection.Get());
	FCacheElement* FoundElement = FindElementForKey(InKey);
	if (FoundElement)
	{
		UE_CLOGF(FoundElement->WantsFilteredLogging(), LogAudioTiming, Verbose, "[%ls] - GetChunk(found), chunk %d, ObjectKey=%ls", *WriteToString<128>(FoundElement->Key.SoundWaveName), FoundElement->Key.ChunkIndex, *WriteToString<40>(FoundElement->Key.ObjectKey));
	...
	else
	{
		...
		if (!FoundElement)
		{
			...
			UE_LOGF(LogAudioStreamCaching, Display, "GetChunk failed to find an available chunk slot in the cache, likely because the cache is blown.");
			return TArrayView<uint8>();
		}
...
// cleared out
		...
		if (bBlockForLoadCompletion)
		{
			...
			{
				UE_CLOGF(FoundElement->WantsFilteredLogging(), LogAudioTiming, Verbose, "[%ls] - GetChunk(blocking, hit), chunk %d, ObjectKey=%ls", *WriteToString<128>(FoundElement->Key.SoundWaveName), FoundElement->Key.ChunkIndex, *WriteToString<40>(FoundElement->Key.ObjectKey));
				...
				*MemoryCounterBytes -= FoundElement->ChunkDataSize;

				{
					...
					if (!Result)
					{
						UE_LOGF(LogAudioStreamCaching, Warning, "Failed to retrieve chunk data from Bulk Data for soundwave: %ls", *InKey.SoundWaveName.ToString());
						return TArrayView<uint8>();
					}
				}
			...
			{
				UE_LOGF(LogAudioStreamCaching, Error, "Failed to get chunk %d from soundwave: %ls", InKey.ChunkIndex, *InKey.SoundWaveName.ToString());
			}
		}
		else
		{
...
void FAudioChunkCache::ClearCache()
{
	...
	UE_LOGF(LogAudioStreamCaching, Verbose, "Clearing Cache");
...
void FAudioChunkCache::AddForceInlineSoundWave(const TSharedRef<const FSoundWaveData>& InSoundWaveData)
{
	ensureMsgf(InSoundWaveData->GetLoadingBehavior() == ESoundWaveLoadingBehavior::ForceInline, 
		TEXT("AudioStreamingCache::AddForceInlineSoundWave: Attempted to add SoundWave not set to ForceInline: %s"), *InSoundWaveData->GetFName().ToString());

	if (InSoundWaveData->GetLoadingBehavior() != ESoundWaveLoadingBehavior::ForceInline)
	{
		return;
	}
	
	FName Format = InSoundWaveData->GetRuntimeFormat();
	FByteBulkData* Data = InSoundWaveData->GetCompressedData(Format);
	int64 MemoryCount = Data ? Data->GetBulkDataSize() : 0;
	int32 RefCount = 0;
	{
		FScopeLock Lock(&SoundWaveMemoryTrackerCritSec.Get());
		FSoundWaveMemoryTracker& Tracker = SoundWaveTracker.FindOrAdd(&InSoundWaveData.Get());
		checkf(Tracker.RefCount >= 0, TEXT("AudioStreamCache::AddForceInlineSoundWave: ref count for Added sound wave is negative!: %s"), *InSoundWaveData->GetFName().ToString());
...
void FAudioChunkCache::RemoveForceInlineSoundWave(const TSharedRef<const FSoundWaveData>& InSoundWaveData)
{
	ensureMsgf(InSoundWaveData->GetLoadingBehavior() == ESoundWaveLoadingBehavior::ForceInline,
		TEXT("AudioStreamingCache::RemoveForceInlineSoundWave: Attempted to remove SoundWave not set to ForceInline: %s"), *InSoundWaveData->GetFName().ToString());
	...
	// scope lock
	{
		...
		FSoundWaveMemoryTracker* Tracker = SoundWaveTracker.Find(&InSoundWaveData.Get());
		checkf(Tracker != nullptr, TEXT("AudioStreamCache::RemoveForceInlineSoundWave: Attempted to remove SoundWave that was never added, or has already been removed: %s"), *InSoundWaveData->GetFName().ToString());
		checkf(Tracker->RefCount > 0, TEXT("AudioStreamCache::RemoveForceInlineSoundWve: Attempted to remove SoundWave that has a ref count of zero or less. Something has gone horribly wrong: %s"), *InSoundWaveData->GetFName().ToString());
		MemoryCount = Tracker->MemoryCount;
		RefCount = --Tracker->RefCount;
		// use the memory count we cached from the last sound wave add
		if (RefCount == 0)
		{
			SoundWaveTracker.Remove(&InSoundWaveData.Get());
		}
	}
...
{
	UE_LOGF(LogAudioStreamCaching, Log, "Adding Memory Counted Feature (%ls) Memory Usage: %d bytes", *Feature.GetFeatureName().ToString(), (int32)Feature.GetMemoryUseInBytes());
	const uint64 OldMemoryCount = FeatureMemoryCounterBytes->AddExchange(Feature.GetMemoryUseInBytes());
	UE_LOGF(LogAudioStreamCaching, Log, "Total Memory Usage for all features: %d -> %d bytes", (int32)OldMemoryCount, (int32)FeatureMemoryCounterBytes->Load());
	const uint64 MemoryUsageBytes = GetCurrentMemoryUsageBytes();
	if (TrimCacheWhenOverBudgetCVar != 0 && MemoryUsageBytes > MemoryLimitBytes)
	{
		uint64 MemoryToTrim = 0;
...
{
	UE_LOGF(LogAudioStreamCaching, Log, "Removing Memory Counted Feature (%ls) Memory Usage: %d", *Feature.GetFeatureName().ToString(), (int32)Feature.GetMemoryUseInBytes());
	checkf(FeatureMemoryCounterBytes->Load() >= Feature.GetMemoryUseInBytes(), TEXT("Count (%llu) < Remove (%llu)"), FeatureMemoryCounterBytes->Load(), Feature.GetMemoryUseInBytes());
	const uint32 OldMemoryCount = FeatureMemoryCounterBytes->SubExchange(Feature.GetMemoryUseInBytes());
	UE_LOGF(LogAudioStreamCaching, Log, "Total Memory Usage for all features: %d -> %d", (int32)OldMemoryCount, (int32)FeatureMemoryCounterBytes->Load());
}
...
uint64 FAudioChunkCache::TrimMemory(uint64 BytesToFree, bool bInAllowRetainedChunkTrimming)
{
	...
	while (CurrentElement != ElementToStopAt && BytesFreed < BytesToFree)
	{
		...
		{
			UE_CLOGF(CurrentElement->WantsFilteredLogging(), LogAudioTiming, Verbose, "[%ls] - Evicting for trim, chunk %d, ObjectKey=%ls", *WriteToString<128>(CurrentElement->Key.SoundWaveName), CurrentElement->Key.ChunkIndex, *WriteToString<40>(CurrentElement->Key.ObjectKey));
...
void FAudioChunkCache::BlockForAllPendingLoads() const
{
	...
	do
	{
		...
		if (bLoadInProgress)
		{
			...
			UE_LOGF(LogAudioStreamCaching, Log, "Waited %f seconds for async audio chunk loads.", TimeSinceStarted);
...
FString FAudioChunkCache::FlushCacheMissLog()
{
	...
	FString TopChunkMissesLog = "Most Missed Chunks:\n";
...
FAudioChunkCache::FCacheElement* FAudioChunkCache::LinearSearchCacheForElement(const FChunkKey& InKey)
{
	...
	while (CurrentElement != nullptr)
	{
...
#endif
			UE_LOGF(LogAudioStreamCaching, Display, "Found element in cache using linear search (LRU)");
		...
		else
		{
			...
			{
				UE_LOGF(LogAudioStreamCaching, Warning, "Possible cycle in our LRU cache list. Please check to ensure any place FCacheElement::MoreRecentElement or FCacheElement::LessRecentElement is changed is locked by CacheMutationCriticalSection.");
				return nullptr;
			}
		}
	}
...
FAudioChunkCache::FCacheElement* FAudioChunkCache::LinearSearchChunkArrayForElement(const FChunkKey& InKey)
{
	for (int i = 0; i < ChunksInUse; ++i)
	{
		...
		{
			UE_LOGF(LogAudioStreamCaching, Display, "Found element in cache using linear search (Chunk Array)");
			return &CachePool[i];
		}
	}
...
FAudioChunkCache::FCacheElement* FAudioChunkCache::InsertChunk(const FChunkKey& InKey, const TSharedRef<const FSoundWaveData>& InSoundWavePtr)
{
...

	{
		...
		else
		{
			...
			{
				UE_LOGF(LogAudioStreamCaching, Display, "Audio Stream Cache: Using %d of %d chunks..", ChunksInUse, CachePool.Num());
			...
			{
				UE_LOGF(LogAudioStreamCaching, Warning, "Failed to find an available chunk slot in the audio streaming manager. Finding a load in flight for an unreferenced chunk and cancelling it.");
				CacheElement = EvictLeastRecentChunk(true);
			}

			if (!CacheElement)
			{
				UE_LOGF(LogAudioStreamCaching, Display, "Failed to find an available chunk slot in the audio streaming manager, likely because the cache was blown.");
				return nullptr;
			}
		}

		UE_CLOGF(CacheElement->WantsFilteredLogging(), LogAudioTiming, Verbose, "[%ls] - Inserting chunk index %d, ObjectKey=%ls",
...
FAudioChunkCache::FCacheElement* FAudioChunkCache::EvictLeastRecentChunk(bool bBlockForPendingLoads /* = false */)
{
	...
	{
		UE_CLOGF(CacheElement->WantsFilteredLogging(), LogAudioTiming, Verbose, "[%ls] - Evicting ununsed LRU chunk index %d, ObjectKey=%ls", *WriteToString<128>(CacheElement->Key.SoundWaveName), CacheElement->Key.ChunkIndex, *WriteToString<40>(CacheElement->Key.ObjectKey));
	...
	{
		UE_CLOGF(CacheElement->WantsFilteredLogging(), LogAudioTiming, Verbose, "[%ls] - Evicting ununsed loading chunk index %d, ObjectKey=%ls", *WriteToString<128>(CacheElement->Key.SoundWaveName), CacheElement->Key.ChunkIndex, *WriteToString<40>(CacheElement->Key.ObjectKey));
	...
	else
	{
		...
		{
			UE_LOGF(LogAudioStreamCaching, Warning, "Cache blown! Please increase the cache size (currently %llu bytes) or load less audio.", ReportCacheSize());
			return nullptr;
		}

#if !UE_BUILD_SHIPPING
		UE_CLOGF(CacheElement->bWantsExtraLogging, LogAudioStreamCaching, Display, "[%ls] - Evicting unused chunk index %d, ObjectKey=%ls", *WriteToString<128>(CacheElement->Key.SoundWaveName), CacheElement->Key.ChunkIndex, *WriteToString<40>(CacheElement->Key.ObjectKey));
#endif
	}
...
void FAudioChunkCache::KickOffAsyncLoad(FCacheElement* CacheElement, const FChunkKey& InKey, TFunction<void(EAudioChunkLoadResult)> OnLoadCompleted, ENamedThreads::Type CallbackThread, bool bNeededForPlayback)
{
	...
	if (nullptr == Chunk)
	{
		UE_LOGF(LogAudioStreamCaching, Error, "Failed to kick off async load of chunk %d in soundwave \"%ls\". Could not find chunk.", InKey.ChunkIndex, *InKey.SoundWaveName.ToString());
		return;
	}
	...
	UE_CLOGF(CacheElement->WantsFilteredLogging(), LogAudioTiming, Verbose, "[%ls] - KickOffAsyncLoad pri %d outstanding %d chunk index %d, ObjectKey=%ls", *WriteToString<128>(CacheElement->Key.SoundWaveName), (uint32)AsyncIOPriority, NumberOfLoadsInFlight.GetValue(), CacheElement->Key.ChunkIndex, *WriteToString<40>(CacheElement->Key.ObjectKey));
	...
	*MemoryCounterBytes += CacheElement->ChunkDataSize;
	...
	if (Chunk->DerivedDataKey.IsEmpty() == false)
	{
		...
		{
			UE_CLOGF(!CacheElement->DDCTask->IsDone(), LogAudioStreamCaching, Display, "DDC work was not finished for a requested audio streaming chunk slot berfore reuse; This may cause a hitch.");
		...
		TFunction<void(bool)> OnLoadComplete = [OnLoadCompleted, CallbackThread, CacheElement, InKey, ChunkDataSize](bool bRequestFailed)
		{
			...
			if (bRequestFailed)
			{
				...
				UE_LOGF(LogAudio, Warning, "FAudioChunkCache::KickOffAsyncLoad -> DDCTask.OnLoadComplete: Request Failed. ChunkIdx: %d; SoundWave: %ls", InKey.ChunkIndex, *InKey.SoundWaveName.ToString());
			}
...
#endif // #if WITH_EDITORONLY_DATA
	{
		if (CacheElement->IsLoadInProgress())
		{
			...
			UE_CLOGF(CacheElement->WantsFilteredLogging(), LogAudioTiming, Verbose, "[%ls] - Waiting for async load while kicking async load, chunk %d, ObjectKey=%ls", *WriteToString<128>(CacheElement->Key.SoundWaveName), CacheElement->Key.ChunkIndex, *WriteToString<40>(CacheElement->Key.ObjectKey));
			CacheElement->WaitForAsyncLoadCompletion(true);
		}
		...
		FBulkDataRequest::FCompletionCallback AsyncFileCallBack = [this, OnLoadCompleted, CacheElement, InKey, ChunkDataSize, CallbackThread, AsyncIOPriority](FBulkDataRequest::EStatus Status) 
		{
			...
			UE_CLOGF(CacheElement->WantsFilteredLogging(), LogAudioTiming, Verbose, "[%ls] - AsyncReadComplete index %d, ObjectKey=%ls", *WriteToString<128>(CacheElement->Key.SoundWaveName), CacheElement->Key.ChunkIndex, *WriteToString<40>(CacheElement->Key.ObjectKey));
			UE_CLOGF(CacheElement->WantsFilteredLogging(), LogAudioTiming, Verbose, "[%ls] - Cancelled=%d, Size=%u, InKey %ls", *WriteToString<128>(CacheElement->Key.SoundWaveName), bWasCancelled, ChunkDataSize, bMatched ? TEXT("matched") : TEXT("DID NOT MATCH!"));
			UE_CLOGF(CacheElement->WantsFilteredLogging() && !bMatched, LogAudioTiming, Verbose, "[%ls] - InKey %ls / %d / %ls", *WriteToString<128>(CacheElement->Key.SoundWaveName), *WriteToString<128>(InKey.SoundWaveName), InKey.ChunkIndex, *WriteToString<40>(InKey.ObjectKey));
			...
			{
				FMemory::Memzero(CacheElement->ChunkData, CacheElement->ChunkDataSize);
				UE_LOGF(LogAudio, Warning, "FAudioChunkCache::KickOffAsyncLoad -> AsyncFileCallBack: Request Cancelled. ChunkIdx: %d; SoundWave: %ls", InKey.ChunkIndex, *InKey.SoundWaveName.ToString());
			}
		...
		{
			UE_CLOGF(CacheElement->WantsFilteredLogging(), LogAudioTiming, Verbose, "[%ls] - Chunk %d has bulk data loaded, , ObjectKey=%ls", *WriteToString<128>(CacheElement->Key.SoundWaveName), CacheElement->Key.ChunkIndex, *WriteToString<40>(CacheElement->Key.ObjectKey));
...
#if DEBUG_STREAM_CACHE
			UE_LOGF(LogAudioStreamCaching, Verbose, "Loading Inlined Chunk: %ls, %d, TimeToLoad=%2.2fms", *InKey.SoundWaveName.ToString(),
			...
			UE_LOGF(LogAudioStreamCaching, VeryVerbose, "COPY+DISCARD %ls - Bulk=0x%p", *InKey.SoundWaveName.ToString(), &Chunk->BulkData);
#endif //DEBUG_STREAM_CACHE
		...
		{
			UE_LOGF(LogAudioStreamCaching, VeryVerbose, "DISK %ls - Bulk=0x%p", *InKey.SoundWaveName.ToString(), &Chunk->BulkData)
			...
			UE_CLOGF(CacheElement->DebugInfo.bWasInlinedButUnloaded,LogAudioStreamCaching, Log, "IO LOAD FOR INLINE %ls - Bulk=0x%p", *InKey.SoundWaveName.ToString(), &Chunk->BulkData);
#endif //DEBUG_STREAM_CACHE
			
			UE_LOGF(LogAudioStreamCaching, Verbose, "Loading Chunk: %ls, %d", *InKey.SoundWaveName.ToString(), InKey.ChunkIndex);

#if !UE_BUILD_SHIPPING
			UE_CLOGF(CacheElement->bWantsExtraLogging, LogAudioStreamCaching, Display, "[%ls] - CreateStreamingRequest: chunk %d, ObjectKey=%ls", *WriteToString<128>(CacheElement->Key.SoundWaveName), CacheElement->Key.ChunkIndex, *WriteToString<40>(CacheElement->Key.ObjectKey));
#endif
...
void FAudioChunkCache::HandleStarvation(
	const TSharedRef<const FSoundWaveData>& InSoundWaveData, const uint32 ChunkIndex, const uint32 InBytesProduced, const uint32 InBytesExpected)
{
	const FChunkKey Key(
		InSoundWaveData
		, ChunkIndex
#if WITH_EDITOR
		, (uint32)InSoundWaveData->GetCurrentChunkRevision()
#endif
	);
	if (FCacheElement* FoundElement = FindElementForKey(Key))
	{
		const float PercentProduced = InBytesExpected > 0.f ? InBytesProduced / (float)InBytesExpected : 0.f;
		...
		if (FoundElement->Read.IsPending() && FoundElement->Priority != StarvingPriority)
		{
			...
			UE_CLOGF(FoundElement->WantsFilteredLogging(), LogAudioTiming, Verbose, "FAudioChunkCache::HandleStarvation Wave:%ls, Chunk:%d, LoadRequested=-%.2f (secs), OldPriority=%ls, NewPriority=%ls, Starvometer=%2.f",
				*InSoundWaveData->GetFName().ToString(), ChunkIndex, TimeStartedSecs, LexToString(FoundElement->Priority), LexToString(StarvingPriority), Starvometer);
#endif //DEBUG_STREAM_CACHE
	...
	UE_LOG(LogAudioTiming, Verbose, TEXT("Decoder Starving waiting for Wave:%s, Chunk:%d (not in cache!)"), *InSoundWaveData->GetFName().ToString(), ChunkIndex)
}
...
uint64 FCachedAudioStreamingManager::TrimMemory(uint64 NumBytesToFree)
{
	...
	UE_LOGF(LogAudioStreamCaching, Display, "Call to IAudioStreamingManager::TrimMemory successfully freed %llu of the requested %llu bytes.", TotalBytesFreed, NumBytesToFree);
