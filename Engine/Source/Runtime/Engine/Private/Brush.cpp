...
void ABrush::PostLoad()
{
	...
	if ( Brush && Brush->Polys )
	{
		...
		if (BrushComponent)
		{
			...
			{
				UE_LOGF(LogBrush, Warning, "Component inside %ls had missing brush, but actor had it. "
					"This should not happen. Resave package '%ls' to remove this warning.",
		...
		for(FPoly& Poly : Brush->Polys->Element)
		{
			...
			if(!Poly.CalcNormal())
			{
				...
				{
					UE_LOGF(LogBrush, Log, "%ls had invalid poly normals which have been fixed. Resave the level '%ls' to remove this warning.", *Brush->GetName(), *GetLevel()->GetOuter()->GetName());
					if(IsStaticBrush())
					{
						UE_LOGF(LogBrush, Log, "%ls had invalid poly normals which have been fixed. Resave the level '%ls' to remove this warning.", *Brush->GetName(), *GetLevel()->GetOuter()->GetName());
		...
		{
			// Complete loading of the polys before renaming
			Brush->Polys->ConditionalPostLoad();
	...
	{
		UE_LOGF(LogPhysics, Log, "%ls does not have BrushBodySetup. No collision.", *GetName());
	}
}
