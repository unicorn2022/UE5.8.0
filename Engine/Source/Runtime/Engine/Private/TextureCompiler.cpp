...
bool FTextureCompilingManager::PostCompilation(UTexture* Texture)
{
	TGuardValue PostCompilationGuard(bIsRoutingPostCompilation, true);
	TGuardValue PostCompilationTextureGuard(PostCompilationTexture, Texture);
	bIsRoutingPostCompilationError = false;

	check(IsInGameThread());
	TRACE_CPUPROFILER_EVENT_SCOPE(FTextureCompilingManager::PostCompilation);

	UE_LOGF(LogTexture, Verbose, "Refreshing texture %ls because it is ready", *Texture->GetName());
	...
	// Most likely a recursion happened during UpdateResource to add the texture again
	// so we must return an error and take action on it.
	if (bIsRoutingPostCompilationError)
	{
		return false;
	}
...
void FTextureCompilingManager::AddTextures(TArrayView<UTexture* const> InTextures)
{
	...
	if (bIsRoutingPostCompilation)
	{
		...
		StringBuilder.Appendf(
			TEXT("Registering a texture to the compile manager from inside a texture postcompilation usually\n")
		...
		StringBuilder.Appendf(TEXT("  PostCompilation Texture: %s\n"), PostCompilationTexture ? *PostCompilationTexture->GetPathName() : TEXT("<nullptr>"));

		// Empty keys most likely means we are on ddc2.
		StringBuilder.Appendf(TEXT("  PostCompilation DDCKey: %s\n"), *CurrentPostCompilationDDCKey);
		StringBuilder.Appendf(TEXT("  AddTextures Count: %d\n"), InTextures.Num());

		for (const UTexture* ConstTexture : InTextures)
		{
			if (ConstTexture)
			{
				if (ConstTexture == PostCompilationTexture)
				{
					bIsRoutingPostCompilationError = true;
				}
				...
				{
					StringBuilder.Appendf(TEXT("      -> No RunningPlatformData!\n"));
				}
				else
				{
					FString* Key = RunningPlatformData[0]->FetchFirstDerivedDataKey.TryGet<FString>();
					StringBuilder.Appendf(TEXT("       FetchFirstKey: %s\n"), Key ? **Key : TEXT("<empty, likely new texture build flow?>"));
					Key = RunningPlatformData[0]->FetchOrBuildDerivedDataKey.TryGet<FString>();
					StringBuilder.Appendf(TEXT("       FetchOrBuildKey: %s\n"), Key ? **Key : TEXT("<empty, likely new texture build flow?>"));
				}
			}
		}

		if (bIsRoutingPostCompilationError && InTextures.Num() == 1)
		{
			// Just log as error, will be catched and returned by PostCompilation, and acted on accordingly.
			UE_LOGF(LogTexture, Error, "%ls", StringBuilder.ToString());
			return;
		}

		// Treat any other case that we didn't plan on as fatal...
		UE_LOGF(LogTexture, Fatal, "%ls", StringBuilder.ToString());
	}
		...
		if (Textures.Num())
		{
			...
			auto CancelOrMarkCanceled = [&Textures](TSet<TWeakObjectPtr<UTexture>>& Set)
			{
				for (auto Iterator = Set.CreateIterator(); Iterator; ++Iterator)
				{
					...
					{
						UE_LOGF(LogTexture, Verbose, "Canceling texture %ls async compilation as requested", *Texture->GetName());
...
void FTextureCompilingManager::FinishCompilation(TArrayView<UTexture* const> InTextures)
{
	...
	if (bIsRoutingPostCompilation)
	{
		...
		UE_LOGF(LogTexture, Fatal, "Calling FinishCompilation is not allowed during PostCompilation. NumTextures = %d, Texture[0] = %ls", InTextures.Num(), *InTextures[0]->GetPathName());
	}
	...
	TArray<UTexture*> InitialTextures(PendingTextures.Array());
	while (PendingTextures.Num())
	{
		...
		AsyncCompilationHelpers::FinishCompilation(
			...
			LogTexture,
			[this, &PendingTextures](ICompilable* Object)
			{
				UTexture* Texture = static_cast<FCompilableTexture*>(Object)->Texture.Get();
				// If the compilation doesn't succeed, it means we need to do another pass on it
				// to wait and postcompile again, so leave it in the PendingTextures.
				if (PostCompilation(Texture))
				{
	...
	// Do the group post compilation only once after all textures has successfully passed individual PostCompilation.
	PostCompilation(InitialTextures);
}
				...
				if (OldPriority != InPriority)
				{
					if (AsyncTask->SetPriority(InPriority))
					{
						UE_LOGF(
							LogTexture,
							Verbose,
							"Changing priority of %ls (%ls) from %ls to %ls",
							*InTexture->GetName(),
				...
				if (TexturesToProcess.Num())
				{
					...
					for (TWeakObjectPtr<UTexture>& Texture : TexturesToProcess)
					{
						if (Texture.IsValid())
						{
							...
							if ((bIsHighestPrio || bHasTimeLeft) && Texture->IsAsyncCacheComplete() && PostCompilation(Texture.Get()))
							{
...
void FTextureCompilingManager::ProcessAsyncTasks(const AssetCompilation::FProcessAsyncTaskParams& Params)
{
	if (bIsRoutingPostCompilation)
	{
		...
		// Workers that need to interact with textures should do that work in response to
		// a game tick via e.g. ExecuteOnGameThread
		UE_LOGF(LogTexture, Fatal, "Calling ProcessAsyncTasks is not allowed during PostCompilation.");
	}
