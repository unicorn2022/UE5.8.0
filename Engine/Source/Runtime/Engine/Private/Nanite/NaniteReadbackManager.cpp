...
void FReadbackManager::QueueReadback(FRDGBuilder& GraphBuilder)
{
	...
	if (ReadbackBuffer.Buffer.IsValid())
	{
		// Reset ready state so it will not appear ready until the RDG pass below gets executed.
		check(ReadbackBuffer.Buffer->IsReady());
		ReadbackBuffer.Buffer->ResetReady();
	}
	else
	{
