...
float UUserInterfaceSettings::ConvertFontSizeFromNativeToDisplay(float NativeFontSize) const
{
	const float FontDisplayDPI = static_cast<float>(GetFontDisplayDPI());
	const float DisplayedSize = NativeFontSize * static_cast<float>(FontConstants::RenderDPI) / FontDisplayDPI;
	return DisplayedSize;
}

float UUserInterfaceSettings::ConvertFontSizeFromDisplayToNative(float DisplayFontSize) const
{
	const float FontDisplayDPI = static_cast<float>(GetFontDisplayDPI());
...
void UUserInterfaceSettings::ForceLoadResources(bool bForceLoadEverything) const
{
	...
	if (bShouldLoadCursors || bForceLoadEverything)
	{
		...
		for (int32 i = 0; i < LoadedClasses.Num(); ++i)
		{
			...
			{
				UE_LOGF(LogLoad, Warning, "UUserInterfaceSettings::ForceLoadResources: Failed to load cursor resource %d.", i);
			}
		}
