...
static TAutoConsoleVariable<bool> CVarEnqueueTransactionalRpcs(
	TEXT("net.EnqueueTransactionalRpcs"),
	true,
	TEXT("When set to true (and net.QueueRpcsForTransactions to 1), each individual RPC will be enqueued rather than immediately executed (allows further unblocking of transactions)"));
...
public:
	...
	virtual void NetSerializeStruct(FNetDeltaSerializeInfo& Params) override final
	{
		...
		if (EnumHasAnyFlags(CachedRequestState.Struct->StructFlags, STRUCT_NetSerializeNative))
		{
			...
			if (!bSuccess)
			{
				UE_LOGF(LogRep, Warning, "NetSerializeStruct: Native NetSerialize %ls failed.", *Params.Struct->GetFullName());
			}
		}
		else
		{
...
bool FObjectReplicator::ValidateAgainstState( const UObject* ObjectState )
{
	...
	{
		UE_LOGF(LogRep, Warning, "ValidateAgainstState: RepLayout.IsValid() == false");
		return false;
	}

	if (!RepState.IsValid())
	{
		UE_LOGF(LogRep, Warning, "ValidateAgainstState: RepState.IsValid() == false");
		return false;
	}

	if (!ChangelistMgr.IsValid())
	{
		UE_LOGF(LogRep, Warning, "ValidateAgainstState: ChangelistMgr.IsValid() == false");
		return false;
	}
	...
	{
		UE_LOGF(LogRep, Warning, "ValidateAgainstState: ChangelistState == nullptr");
		return false;
	}
	...
	{
		UE_LOGF(LogRep, Warning, "ValidateAgainstState: Properties changed for %ls", *ObjectState->GetName());
		return false;
	}
...
void FObjectReplicator::InitWithObject(UObject* InObject, UNetConnection* InConnection, bool bUseDefaultState)
{
	...
	if (!WeakObjectPtr.IsValid())
	{
		...
		UE_LOGF(LogRep, Error, "InitWithObject: Invalid Object pointer %p with name `%ls`", InObject, ToCStr(GetNameSafe(InObject)));
		return;
	}
	...
	if ((Source == nullptr) && bUseDefaultState)
	{
		...
		{
			UE_LOGF(LogRep, Error, "FObjectReplicator::InitWithObject: Invalid object archetype, initializing shadow state to class default state: %ls", *GetFullNameSafe(InObject));
			Source = (uint8*)ObjectClass->GetDefaultObject();
		}
		else
		{
			UE_LOGF(LogRep, Error, "FObjectReplicator::InitWithObject: Invalid object archetype and class, initializing shadow state to current object state: %ls", *GetFullNameSafe(InObject));
...
void FObjectReplicator::StartReplicating(class UActorChannel * InActorChannel)
{
	...
	if (Object == nullptr)
	{
		UE_LOGF(LogRep, Error, "StartReplicating: Object == nullptr");
		return;
	}
	...
	if (UNLIKELY(!bIsValidToReplicate))
	{
		...
		UE_LOGF(LogRep, Error, "StartReplicating: Invalid Net GUID. Object may fail to replicate properties or handle RPCs. Object %ls", *Object->GetPathName());
		return;
	}
		...
		if (!Connection->IsInternalAck())
		{
			if (FSendingRepState * SendingRepState = RepState.IsValid() ? RepState->GetSendingRepState() : nullptr)
			{
				...
				if (OwningChannel->SpawnAcked)
				{
					...
					UE_LOGF(LogRep, Verbose, "FObjectReplicator::InitRecentProperties -  OwningChannel->SpawnAcked is true, also setting SendingRepState->bOpenAckedCalled to true. Object = %ls. Channel actor = %ls.",
...
void FObjectReplicator::ReceivedNak( int32 NakPacketId )
{
	...
	if (Object == nullptr)
	{
		UE_LOGF(LogNet, Verbose, "FObjectReplicator::ReceivedNak: Object == nullptr");
		return;
	}
	else if (ObjectClass == nullptr)
	{
		UE_LOGF(LogNet, Verbose, "FObjectReplicator::ReceivedNak: ObjectClass == nullptr");
	}
		...
		if (FSendingRepState* SendingRepState = RepState.IsValid() ? RepState->GetSendingRepState() : nullptr)
		{
			...
			for (int32 i = SendingRepState->Retirement.Num() - 1; i >= 0; i--)
			{
				...
				while (Rec != nullptr)
				{
					...
					{
						UE_LOGF(LogNet, VeryVerbose, "Restoring Previous Base State of dynamic property. Channel: %ls, NakId: %d, First: %d, Last: %d, Address: %ls)", *OwningChannel->Describe(), NakPacketId, Rec->OutPacketIdRange.First, Rec->OutPacketIdRange.Last, *Connection->LowLevelGetRemoteAddress(true));
...
bool FObjectReplicator::ReceivedBunch(FNetBitReader& Bunch, const FReplicationFlags& RepFlags, const bool bHasRepLayout, bool& bOutHasUnmapped)
{
	...
	if (!Object)
	{
		UE_LOGF(LogNet, Verbose, "ReceivedBunch: Object == nullptr");
		return false;
	}
	...
	if (!ClassCache)
	{
		UE_LOGF(LogNet, Error, "ReceivedBunch: ClassCache == nullptr: %ls", *Object->GetFullName());
		return false;
	}
	...
	if (bHasRepLayout)
	{
		...
		if (bIsServer)
		{
			UE_LOGF(LogNet, Error, "Server received RepLayout properties: %ls", *Object->GetFullName());
			return false;
		}
		...
		{
			UE_LOGF(LogRep, Error, "RepLayout->ReceiveProperties FAILED: %ls", *Object->GetFullName());
			return false;
		}
	...
	while (true)
	{
		...
		{
			UE_LOGF(LogNet, Error, "ReceivedBunch: Error reading field: %ls", *Object->GetFullName());
			return false;
		}

		else if (UNLIKELY(FieldCache == nullptr))
		{
			UE_LOGF(LogNet, Warning, "ReceivedBunch: FieldCache == nullptr: %ls", *Object->GetFullName());
			continue;
		}
		...
		else if (UNLIKELY(FieldCache->bIncompatible))
		{
			...
			UE_LOGF(LogNet, Verbose, "ReceivedBunch: FieldCache->bIncompatible == true. Object: %ls, Field: %ls", *Object->GetFullName(), *FieldCache->Field.GetFName().ToString());
			continue;
		}
		...
		if (FStructProperty* ReplicatedProp = CastField<FStructProperty>(FieldCache->Field.ToField()))
		{
			...
			if (bIsServer)
			{
				UE_LOGF(LogNet, Error, "Server received unwanted property value %ls in %ls", *ReplicatedProp->GetName(), *Object->GetFullName());
				return false;
			}
...
#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
			{
				...
				{
					UE_LOGF(LogRep, Log, "Replicating Property[%d] %ls on %ls", ReplicatedProp->RepIndex, *ReplicatedProp->GetName(), *Object->GetName());
				}
			}
#endif
			...
			UE_LOGF(LogRepTraffic, Log, " %ls - %ls", *Object->GetName(), *Parms.DebugName);
		}
		...
		{
			UE_LOGF( LogRep, Error, "ReceivedBunch: Invalid replicated field %i in %ls", FieldCache->FieldNetIndex, *Object->GetFullName() );
			return false;
		}
	}
...
bool FObjectReplicator::ReceivedRPC(FNetBitReader& Reader, TSet<FNetworkGUID>& UnmappedGuids, bool& bOutSkippedRpcExec, const FReplicationFlags& RepFlags, const FFieldNetCache* FieldCache, ESkipRpcBehavior SkipRpcBehavior)
{
	...
	if (Function == nullptr)
	{
		UE_LOGF(LogNet, Error, "ReceivedRPC: Function not found. Object: %ls, Function: %ls", *Object->GetFullName(), *FunctionName.ToString());
		HANDLE_INCOMPATIBLE_RPC
	}

	if ((Function->FunctionFlags & FUNC_Net) == 0)
	{
		UE_LOGF(LogRep, Error, "Rejected non RPC function. Object: %ls, Function: %ls", *Object->GetFullName(), *FunctionName.ToString());
		HANDLE_INCOMPATIBLE_RPC
	}
	...
	{
		UE_LOGF(LogRep, Error, "Rejected server RPC function due to access rights. Object: %ls, Function: %ls", *Object->GetFullName(), *FunctionName.ToString());
		HANDLE_INCOMPATIBLE_RPC
	}

	if ((Function->FunctionFlags & (FUNC_NetClient | FUNC_NetMulticast)) && bIsServer)
	{
		UE_LOGF(LogRep, Error, "Rejected client RPC function due to access rights. Object: %ls, Function: %ls", *Object->GetFullName(), *FunctionName.ToString());
		HANDLE_INCOMPATIBLE_RPC
	}
	...
	if (RPCDoS != nullptr && bIsServer && RPCDoS->IsRPCDoSDetectionEnabled() && !Connection->IsReplay())
	{
		...
		{
			ERPCNotifyResult Result = RPCDoS->NotifyReceivedRPC(Object, Function, FunctionName);

			if (UNLIKELY(Result == ERPCNotifyResult::BlockRPC))
			{
				RPCDoS->TrackBlockedRPC(FunctionName);
	...
	UE_LOGF(LogRepTraffic, Log, "      Received RPC: %ls", *FunctionName.ToString());
	...
	if (bCanExecute)
	{
		...
		{
			UE_LOGF(LogRep, Error, "ReceivedRPC: GetFunctionRepLayout returned an invalid layout.");
			return false;
		}
		...
		if (Reader.IsError())
		{
			UE_LOGF(LogRep, Error, "ReceivedRPC: ReceivePropertiesForRPC - Reader.IsError() == true: Function: %ls, Object: %ls", *FunctionName.ToString(), *Object->GetFullName());
			HANDLE_INCOMPATIBLE_RPC
		}

		if (Reader.GetBitsLeft() != 0)
		{
			UE_LOGF(LogNet, Error, "ReceivedRPC: ReceivePropertiesForRPC - Mismatch read. Function: %ls, Object: %ls", *FunctionName.ToString(), *Object->GetFullName());
			HANDLE_INCOMPATIBLE_RPC
		}
		...
		{
			UE_LOGF(LogRep, Error, "ReceivedRPC: RPC_GetLastFailedReason: %ls", RPC_GetLastFailedReason());
			return false;
		}
	}
	else
	{
		UE_LOGF(LogRep, Verbose, "Rejected unwanted function %ls in %ls", *FunctionName.ToString(), *Object->GetFullName());
	}
	...
	const UObject* Object)
{
	...
	while (*Rec != nullptr)
	{
		...
		{
			UE_LOGF(LogRepTraffic, Verbose, "Deleting Property Record (%d >= %d)", OutAckPacketId, (*Rec)->OutPacketIdRange.Last);
...
bool FObjectReplicator::ReplicateProperties_r(FOutBunch& Bunch, FReplicationFlags RepFlags, FNetBitWriter& Writer)
{
	...
	if (Object == nullptr)
	{
		UE_LOGF(LogRep, Verbose, "ReplicateProperties: Object == nullptr");
		return false;
	}
	...
	if ( RemoteFunctions != nullptr && RemoteFunctions->GetNumBits() > 0 )
	{
		...
		{
			UE_LOGF( LogRepTraffic, Warning,	"      Sending queued RPCs: %ls. Channel[%d] [%.1f bytes]", *Object->GetName(), OwningChannel->ChIndex, RemoteFunctions->GetNumBits() / 8.f );
		}
...
void FObjectReplicator::ForceRefreshUnreliableProperties()
{
	if (GetObject() == nullptr)
	{
		UE_LOGF(LogRep, Verbose, "ForceRefreshUnreliableProperties: Object == nullptr");
		return;
	}
...
void FObjectReplicator::PostSendBunch( FPacketIdRange & PacketRange, uint8 bReliable )
{
	...
	if ( Object == nullptr )
	{
		UE_LOGF(LogNet, Verbose, "PostSendBunch: Object == nullptr");
		return;
	}
...
bool FObjectReplicator::ReadyForDormancy(bool bSuppressLogs)
{
	if (GetObject() == nullptr)
	{
		UE_LOGF(LogRep, Verbose, "ReadyForDormancy: Object == nullptr");
	...
	if (!bLastUpdateEmpty)
	{
		UE_CLOGF(!bSuppressLogs, LogRepTraffic, Verbose, "    [%d] Not ready for dormancy. bLastUpdateEmpty = false", OwningChannel->ChIndex);
	...
	if (FSendingRepState* SendingRepState = RepState.IsValid() ? RepState->GetSendingRepState() : nullptr)
	{
		...
		for (FPropertyRetirement& Retirement : SendingRepState->Retirement)
		{
			...
			{
				UE_CLOGF(!bSuppressLogs, LogRepTraffic, Verbose, "    [%d] Not ready for dormancy. OutAckPacketId: %d First: %d Last: %d ", OwningChannel->ChIndex, OwningChannel->Connection->OutAckPacketId, Retirement.OutPacketIdRange.First, Retirement.OutPacketIdRange.Last);
				return false;
			}
		}
	}
...
void FObjectReplicator::StartBecomingDormant()
{
	if ( GetObject() == nullptr )
	{
		UE_LOGF( LogRep, Verbose, "StartBecomingDormant: Object == nullptr" );
		return;
	}
...
void FObjectReplicator::UpdateUnmappedObjects(bool & bOutHasMoreUnmapped)
{
	...
	{
		UE_LOGF(LogNet, Verbose, "FObjectReplicator::UpdateUnmappedObjects: Connection->State == USOCK_Closed");
		return;
	}
	...
	if (PackageMapClient && OwningChannel)
	{
		...
		for (int32 RPCIndex = 0; RPCIndex < PendingLocalRPCs.Num(); RPCIndex++)
		{
			...
			bool bSuccess = true;
			FString FunctionName = "(Unknown)";
			
			if (FieldCache == nullptr)
			{
				UE_LOGF(LogNet, Warning, "FObjectReplicator::UpdateUnmappedObjects: FieldCache not found. Object: %ls", *Object->GetFullName());
				bSuccess = false;
			}
			else
			{
			...
			if (!bSuccess)
			{
				if (bIsServer && !Connection->IsInternalAck())
				{
					...
					UE_LOGF(LogNet, Error, "FObjectReplicator::UpdateUnmappedObjects: Failed executing delayed RPC %ls on Object %ls, closing connection!", *FunctionName, *Object->GetFullName());
				...
				{
					UE_LOGF(LogNet, Warning, "FObjectReplicator::UpdateUnmappedObjects: Failed executing delayed RPC %ls on Object %ls, skipping RPC!", *FunctionName, *Object->GetFullName());
			...
			else
			{
				...
				if (DelayedFrames > 0)
				{
					UE_LOGF(LogNet, Verbose, "FObjectReplicator::UpdateUnmappedObjects: RPC %ls on Object %ls was finally executed after being delayed for %u frames (~%f ms)",
...
void FObjectReplicator::EnqueuePendingTransactionalRPCs()
{
	...
	TArray<TUniqueFunction<void(void)>> TransactionsToEnqueue;
	TransactionsToEnqueue.Reserve(ServiceTheseRPCs.Num());

	for (const FRPCPendingLocalCall& Pending : ServiceTheseRPCs)
	{
		TransactionsToEnqueue.Emplace([this, Pending, ObjectFullName]()
			{
				const FClassNetCache* ClassCache = Connection->Driver->NetCache->GetClassNetCache(ObjectClass);
				...
				if (FieldCache == nullptr)
				{
					UE_LOGF(LogNet, Warning, "%s: FieldCache not found. Object: %ls", __func__, *ObjectFullName);
					bSuccess = false;
				}
				else
				{
					...
					{
						UE::RemoteExecutor::FSpeculationExecutionScope OptimizationScope;
						FNetBitReader Reader(Connection->PackageMap, Pending.Buffer.GetData(), Pending.NumBits);
						Connection->SetNetVersionsOnArchive(Reader);
				...
				if (!bSuccess)
				{
					...
					{
						// Close our connection and abort rpcs as things are invalid
						UE_LOGF(LogNet, Error, "%s: Failed executing delayed RPC %ls on Object %ls, closing connection!", __func__, *FunctionName, *ObjectFullName);
					...
					{
						UE_LOGF(LogNet, Warning, "%s: Failed executing delayed RPC %ls on Object %ls, skipping RPC!", __func__, *FunctionName, *ObjectFullName);
					}
				}
			});
	}

	if (CVarEnqueueTransactionalRpcs.GetValueOnAnyThread(true))
	{
		UE::RemoteExecutor::EnqueueWorkList(Object->GetFName(), true, MoveTemp(TransactionsToEnqueue));
	}
	else
	{
		for (const TUniqueFunction<void(void)>& RpcFunc : TransactionsToEnqueue)
		{
			RpcFunc();
		}
	}
		...
		*GetPathNameSafe(Object), *Property->GetName()))
	{
		...
		if (RepNotifyFunc->NumParms > 0)
		{
			...
			else if (MetaData.Num() > 0)
			{
				...
				//UE_LOGF(LogRepTraffic, Verbose, "Property %ls had MetaData: ", *Property->GetName() );
				//for (auto MetaIt = MetaData.CreateIterator(); MetaIt; ++MetaIt)
				//	UE_LOGF(LogRepTraffic, Verbose, "   %d", *MetaIt );
