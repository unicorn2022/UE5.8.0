...
// 		else UE_LOGF(LogPolygon, Warning, "FPoly::Fix: Collapsed a point" );
	}
	if(j < 3)
	{
		Vertices.Empty();
...
) const
{
	...
	else
	{
		...
		*BackPoly = *this; // Copy all info.
		...
		if( FrontPoly->Fix()<3 )
		{
// 			UE_LOGF(LogPolygon, Warning, "FPoly::SplitWithPlane: Ignored front sliver" );
			return SP_Back;
		}
		else if( BackPoly->Fix()<3 )
	    {
// 			UE_LOGF(LogPolygon, Warning, "FPoly::SplitWithPlane: Ignored back sliver" );
			return SP_Front;
		}
...
int32 FPoly::CalcNormal( bool bSilent )
{
	...
	{
// 		if( !bSilent )
// 			UE_LOGF(LogPolygon, Warning, "FPoly::CalcNormal: Zero-area polygon" );
		return 1;
	}
	Normal.Normalize();
	return 0;
...
// 		UE_LOGF(LogPolygon, Warning, "FPoly::Finalize: Not enough vertices (%i)", Vertices.Num() );
		if( NoError )
			return -1;
		else
		{
// 			UE_LOGF(LogPolygon, Log,  "FPoly::Finalize: Not enough vertices (%i) : polygon removed from brush", Vertices.Num() );
			return -2;
		}
	}
	...
	if( CalcNormal() )
	{
// 		UE_LOGF(LogPolygon, Warning, "FPoly::Finalize: Normalization failed, verts=%i, size=%f", Vertices.Num(), Normal.Size() );
		if( NoError )
		{
			return -1;
		}
		...
		{
			UE_LOGF(LogPolygon, Error, "FPoly::Finalize: Normalization failed, verts=%d, size=%f", Vertices.Num(), Normal.Size());
			for (int32 VertIdx = 0; VertIdx < Vertices.Num(); ++VertIdx)
			{
				UE_LOGF(LogPolygon, Error, "Vertex %d: <%.6f, %.6f, %.6f>", VertIdx, Vertices[VertIdx].X, Vertices[VertIdx].Y, Vertices[VertIdx].Z);
			}
			ensure(false);
		}
	}
