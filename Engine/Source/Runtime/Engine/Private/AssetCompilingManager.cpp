...
#include "Animation/AnimSequenceTransformProviderCompiler.h"
...
public:
	...
	virtual int64 GetEstimatedMemory() const override
	{
		return TotalEstimatedMemory.load(std::memory_order_relaxed);
	}
	
	static int64 GetHardMemoryLimit()
	{
		return (int64)CVarAsyncAssetCompilationMaxMemoryUsage.GetValueOnAnyThread() * 1024ll * 1024ll * 1024ll;
	}
	...
	{
		return FMath::Max(0ll, (int64)CVarAsyncAssetCompilationMemoryPerCore.GetValueOnAnyThread(false) * 1024ll * 1024ll * 1024ll);
	}
	...
	int64 GetMemoryAvailableForAssetCompilation() const
	{
		...
		UE_LOGF(LogAsyncCompilation, Verbose, "GetMemoryAvailableForAssetCompilation UsedPhysical = %.3f MB (Peak %.3f MB) UsedVirtual = %.3f MB (Peak %.3f MB) AvailPhysical = %.3f MB AvailVirtual = %.3f MB ",
		...
		if (SystemAvailableMemory < MemoryAvailableAgainstHardLimit) 
		{
			...
			if (CurrentDefecit > LargestDefecit) 
			{
				...
				UE_LOGF(LogAsyncCompilation, VeryVerbose, "Memory available for asset compilation less than exptected: (Hard Limit %.3f MB - SystemAvailableMemory %.3f MB) < MemoryAvailableAgainstHardLimit = %.3f MB",
					   HardMemoryLimit / (1024.f * 1024.f),
					   SystemAvailableMemory / (1024.f * 1024.f),
					   MemoryAvailableAgainstHardLimit / (1024.f * 1024.f));
			}
		}
	...
	public:
		int32 MemoryInMiBToBorrowFromSCW;	//	Memory in MiB to borrow from shader compilation when required for asset compilation.
		bool IsMemorySharingEnabled;

		MemorySharingInfo(TFunction<int64()> InGetAvailableMemory)
			: GetAvailableMemory(InGetAvailableMemory)
		{
			MemoryInMiBToBorrowFromSCW = GConfig->GetIntOrDefault(TEXT("AssetCompilation"), TEXT("MemoryToBorrowFromSCW"), 2000, GEngineIni);	
			...
			UE_LOGF(LogAsyncCompilation, Display, "MemorySharingInfo: UsedMemory=%lldMiB, MaxMemoryAvailable=%lldMiB", MemoryStats.UsedVirtual / 1024ull / 1024ull,	MemoryConstants.TotalVirtual / 1024ull / 1024ull);
		}	
		...
		void LogMemoryInfo() const
		{
			const FPlatformMemoryStats MemoryStats = FPlatformMemory::GetStats();
			const FPlatformMemoryConstants& MemoryConstants = FPlatformMemory::GetConstants();
			UE_LOGF(LogAsyncCompilation, Display, "MemorySharingInfo: UsedMemory=%lldMiB, MaxMemoryAvailable=%lldMiB Currently Available=%lldMiB", MemoryStats.UsedVirtual / 1024 / 1024,	MemoryConstants.TotalVirtual / 1024 / 1024, GetAvailableMemory() / 1024 / 1024);
		}

		bool IsSCWMemoryWithinLimit() const 
		{ 			
			FShaderCompileMemoryUsage SCWMemoryUsage = GShaderCompilingManager->GetExternalMemoryUsage();
			FShaderCompilerMemoryLimits SCWMemoryLimits = GetShaderCompilerCurrentMemoryLimits();		

			return SCWMemoryUsage.VirtualMemory <= static_cast<uint64>(SCWMemoryLimits.CurrentLimit * 1024ull * 1024ull);
		}
	};
		...
		{						
			if (MemoryInfo.IsMemorySharingEnabled && FMemoryBoundQueuedThreadPoolWrapper::IsMemoryConstrained())
			{
				auto WaitForSCWMemory = [this]() 
				{
					...
					if (Counter > 0)
					{
						UE_LOGF(LogAsyncCompilation, Display, "Waited for SCW memory: %d iterations for a total of %f seconds",
							   Counter,
						...
						{
							UE_LOGF(LogAsyncCompilation, Display, "Waited for the maximum allowed time for SCW to reduce memory usage, proceeding with work anyway");
						}
					}
				...
				{
					SetShaderCompilerAssetMemoryLoanInMiB(MemoryInfo.MemoryInMiBToBorrowFromSCW);
					WaitForSCWMemory();
					FQueuedThreadPoolWrapper::FScheduledWork::DoThreadedWork();
					SetShaderCompilerAssetMemoryLoanInMiB(0);
				}
				else
				{
					FQueuedThreadPoolWrapper::FScheduledWork::DoThreadedWork();
	...
	void OnScheduled(const IQueuedWork* InWork) override
	{
		...
		Work->TraceRegionId = TRACE_BEGIN_REGION_WITH_ID(*FString::Printf(TEXT("%s (%.3f MiB)"), DebugName, Work->RequiredMemory/(1024*1024.f)), TEXT("AsyncCompilation"));

		UE_LOGF(LogAsyncCompilation, Verbose, "OnScheduled [%ls] Work->RequiredMemory = %.3f MB TotalEstimatedMemory = %.3f MB (TraceId %lld)",
				DebugName,
				Work->RequiredMemory/(1024.f*1024.f),
				TotalEstimatedMemory/(1024.f*1024.f),
				Work->TraceRegionId
			);
	...
	void OnUnscheduled(const IQueuedWork* InWork) override
	{
		...
		if (Work->TraceRegionId)
		{
			TRACE_END_REGION_WITH_ID(Work->TraceRegionId);
		}
		
		// don't call Work->GetDebugName , it's gone now
		UE_LOGF(LogAsyncCompilation, Verbose, "OnUnscheduled Work->RequiredMemory = %.3f MB TotalEstimatedMemory = %.3f MB (TraceId %lld)",
				Work->RequiredMemory/(1024.f*1024.f),
				TotalEstimatedMemory/(1024.f*1024.f),
				Work->TraceRegionId
			);
	...
	int32 GetMaxConcurrency() const override
	{
		...
		if ( NewRequiredMemory > 0 && NewRequiredMemory >= AvailableAssetCompilationMemory )
		{
				...
				if (bHasMemoryEstimate)
				{
					...
					if (NewRequiredMemory > HardMemoryLimit && HardMemoryLimit > 0)
					{
						...
						UE_LOGFMT_NSLOC(LogAsyncCompilation, Warning, "AsyncAssetCompilation", "HardMemoryLimitExceeded",
							...
							("TaskName", DebugName),
							("RequiredMemory", FString::SanitizeFloat(NewRequiredMemory / (1024.f * 1024.f), 3)),
							("TotalEstimatedMemory", FString::SanitizeFloat(TotalEstimatedMemory / (1024.f * 1024.f), 3)),
							("AvailableAssetCompilationMemory", FString::SanitizeFloat(AvailableAssetCompilationMemory / (1024.f * 1024.f), 3)),
							("HardMemoryLimit", FString::SanitizeFloat(HardMemoryLimit / (1024.f * 1024.f), 3))
						);
					}
					else
					{
			...
			{
				UE_LOGF(LogAsyncCompilation, Verbose, "CONCURRENCY LIMITED to %d/%d; RequiredMemory = %.3f MB + %.3f MB AvailableAssetCompilationMemory = %.3f MB ",
	...
	void AddQueuedWork(IQueuedWork* InQueuedWork, EQueuedWorkPriority InPriority) override
	{
		...
		if ( RequiredMemory <= 0 )
		{
			...
			UE_LOGF(LogAsyncCompilation, Display, "AddQueuedWork [%ls] did not supply a RequiredMemory estimate. This is a bug, all asset types should provide an estimate. When no estimate is provided asset compilation is slowed as the queue has to operate very conservatively.",DebugName);
		}
		else
		{
			UE_LOGF(LogAsyncCompilation, Verbose, "AddQueuedWork [%ls] RequiredMemory = %lld",DebugName,RequiredMemory);
		}
...
FAssetCompilingManager::FAssetCompilingManager()
{
	...
	RegisterManager(&FAnimSequenceTransformProviderCompilingManager::Get());
...
bool FAssetCompilingManager::RegisterManager(IAssetCompilingManager* InAssetCompilingManager)
{
	...
	if (!Algo::TopologicalSort(AssetCompilingManagers, GetVertexDependencies))
	{
		...
		UE_LOGF(LogAsyncCompilation, Error, "AssetCompilingManager %ls introduced a circular dependency, it's dependencies will be ignored", *InAssetCompilingManager->GetAssetTypeName().ToString());
	}
...
#if WITH_EDITOR
	// Hold a Slate responsive-mode request while there is pending work, so the editor ticks at full rate
	// and the per-frame processing budget is consumed as fast as possible. Without this, Slate can
	// throttle itself during load (no user input), causing asset compilation to take far longer than necessary.
	if (NumRemainingAssets > 0)
	{
		if (!ResponsiveModeHandle.IsValid() && FSlateApplication::IsInitialized())
		{
			ResponsiveModeHandle = FSlateThrottleManager::Get().EnterResponsiveMode();
		}
	}
	else if (ResponsiveModeHandle.IsValid())
	{
		FSlateThrottleManager::Get().LeaveResponsiveMode(ResponsiveModeHandle);
	}
#endif
}
