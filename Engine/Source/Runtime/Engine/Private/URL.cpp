	...
	if (Valid == 1 && URLStr.Len() > 0)
	{
		...
		if (URLStr[0] != '/')
		{
			...
			if (!bFoundMap)
			{
				...
				UE_LOGF(LogLongPackageNames, Warning, "Can't Find URL: %ls: %ls. Invalidating and reverting to Default URL.", *URLStr, *MapNameError.ToString());
				*this = FURL();
				Valid = 0;
			}
		}
	...
	if (FParse::Command(&Cmd, TEXT("URLSERIALIZATION")))
	{
		...
		for (const auto& TestCase : TestCases)
		{
			for (int32 TestType=0; TestType<2; TestType++)
			{
				...
				{
					UE_LOGF(LogCore, Log, "Test %ls (%ls) passed!", *TestCase.QueryString, (bConversionTest ? *CurQueryString : TEXT("")));
				}
				else
				{
					bAllCasesPassed = false;
					UE_LOGF(LogCore, Warning, "Test %ls (%ls) failed! Matching flags: Host[%d] Port[%d] Protocol[%d]", *TestCase.QueryString,
							(bConversionTest ? *CurQueryString : TEXT("")), bHostMatched, bPortMatched, bProtocolMatched);
					if (!bHostMatched)
					{
						UE_LOGF(LogCore, Warning, "URL had host %ls, expected %ls", *TestURL.Host, *TestCase.Host);
					}

					if (!bPortMatched)
					{
						UE_LOGF(LogCore, Warning, "URL had port %d, expected %d", TestURL.Port, TestCase.Port);
					}

					if (!bProtocolMatched)
					{
						UE_LOGF(LogCore, Warning, "URL had protocol %ls, expected %ls", *TestURL.Protocol, *TestCase.Protocol);
					}
				}
			}
		}
		...
		if (bAllCasesPassed)
		{
			UE_LOGF(LogCore, Log, "All URL serialization cases passed.");
		}
		else
		{
			UE_LOGF(LogCore, Warning, "An URL serialization case failed!");
		}
