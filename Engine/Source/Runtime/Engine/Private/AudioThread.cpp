...
struct FAudioThreadInteractor
{
	static void UseAudioThreadCVarSinkFunction()
	{
		...
		if (bLastSuspendAudioThread != bSuspendAudioThread)
		{
			...
			{
				UE_LOGF(LogAudio, Warning, "Audio threading is disabled in the editor.");
			}
			else if (!FAudioThread::IsUsingThreadedAudio())
			{
				UE_LOGF(LogAudio, Warning, "Cannot manipulate audio thread when disabled by platform or ini.");
			}
		}
	}
};
...
{
	UE_LOGF(LogConsoleResponse, Display, "AudioTaskPriority was %ls.", LowLevelTasks::ToString(GAudioTaskPriority));

	if (Args.Num() > 1)
	{
		UE_LOGF(LogConsoleResponse, Display, "WARNING: This command requires a single argument while %d were provided, all extra arguments will be ignored.", Args.Num());
	}
	else if (Args.IsEmpty())
	{
		UE_LOGF(LogConsoleResponse, Display, "ERROR: Please provide a new priority value.");
		return;
	}

	if (!LowLevelTasks::ToTaskPriority(*Args[0], GAudioTaskPriority))
	{
		UE_LOGF(LogConsoleResponse, Display, "ERROR: Invalid priority: %ls.", *Args[0]);
	}

	UE_LOGF(LogConsoleResponse, Display, "Audio Task Priority was set to %ls.", LowLevelTasks::ToString(GAudioTaskPriority));
}
...
void FAudioThread::SetUseThreadedAudio(const bool bInUseThreadedAudio)
{
	...
	{
		UE_LOGF(LogAudio, Error, "You cannot disable using threaded audio once the thread has already begun running.");
	}
	else
	{
...
#else
	OutLongestTask = FString("NoStats");
#endif
