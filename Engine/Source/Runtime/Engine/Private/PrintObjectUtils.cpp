...
void PrintObjectsInOuter(UObject* Object, EPrintObjectFlag Flags, FOutputDevice* InOutputDevice)
{
	...
	if (OutputDevice)
	{
		if (Object)
		{
			...
			GetObjectsWithOuter(Object, ChildObjects, EGetObjectsFlags::IncludeNestedObjects);
...
void PrintObjectOverrides(UObject* Object, EPrintObjectFlag Flags, FOutputDevice* InOutputDevice)
{
	FOutputDevice* OutputDevice = InOutputDevice ? InOutputDevice : GLog;
	if (!OutputDevice)
	{
	...
	if (Object)
	{
		...
		LogObjectState(Object, OutputDevice, bVerbose, TEXT("Printing overrides for object "));

		FOverridableManager& OverridableManager = FOverridableManager::Get();

		const FOverriddenPropertySet* Overrides = OverridableManager.GetOverriddenProperties(Object);

		if (Overrides)
		{
			FString OverrideStr = Overrides->ToDebugString();
			OutputDevice->Logf(TEXT("%s"), *OverrideStr);
		}
		else
		{
			OutputDevice->Logf(TEXT("No overrides found for object: %s"), *Object->GetPathName());
		}
	}
	else
	{
		OutputDevice->Logf(TEXT("PrintObjectOverrides: NULL object"));
	}
}
...
FAutoConsoleCommandWithArgsAndOutputDevice CVarCommandPrintObjectOverrides(
	TEXT("Obj.DumpOverrides"),
	TEXT("Lists the overridable serialization overrides of an object (the object must be specified as a name or path, e.g., /MyLevel/MyLevel.MyLevel:PersistentLevel.MyActor)."),
	FConsoleCommandWithArgsAndOutputDeviceDelegate::CreateLambda([](const TArray<FString>& Args, FOutputDevice& OutputDevice)
	{
		if (Args.Num() > 0)
		{
			...
			if (Object)
			{
				PrintObjectOverrides(Object, Flags, &OutputDevice);
			}
			else
			{
				OutputDevice.Logf(TEXT("PrintObjectOverrides: failed to find any objects for path: '%s'"), *ObjectPath);
			}
		}
		else
		{
			OutputDevice.Logf(TEXT("PrintObjectOverrides: no object path specified (example usage: Obj.DumpOverrides /MyLevel/MyLevel.MyLevel:PersistentLevel.MyActor)"));
			LogHelp(OutputDevice);
		}
	}));
