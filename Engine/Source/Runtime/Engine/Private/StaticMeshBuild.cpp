...
bool UStaticMesh::CanBuild() const
{
	...
	{
		UE_LOGF(LogStaticMesh, Warning, "Static mesh has no source models: %ls", *GetPathName());
		return false;
	}

	if (GetNumSourceModels() > MAX_STATIC_MESH_LODS)
	{
		UE_LOGF(LogStaticMesh, Warning, "Cannot build LOD %d.  The maximum allowed is %d.  Skipping.", GetNumSourceModels(), MAX_STATIC_MESH_LODS);
		return false;
	}
...
#else
	UE_LOGF(LogStaticMesh, Fatal, "UStaticMesh::Build should not be called on non-editor builds.");
#endif
}
	...
	if (bHasRenderDataChanged)
	{
		...
		if (HasBadNTB(this, bZeroNormals, bZeroTangents, bZeroBinormals))
		{
			...
			if (bZeroTangents || bZeroBinormals)
			{
				...
				{
					UE_LOGF(LogStaticMesh, Display, "%ls", *WarningMsg.ToString());
				}
				else
				{
					UE_LOGF(LogStaticMesh, Warning, "%ls", *WarningMsg.ToString());
				}
