...
static void GenerateConvenientWindowedResolutions(int32 WorkAreaWidth, int32 WorkAreaHeight, TArray<FIntPoint>& OutResolutions)
{
	bool bInPortraitMode = WorkAreaWidth < WorkAreaHeight;
	...
	for (uint32 ScaleIndex = 0; ScaleIndex < NumScales; ++ScaleIndex)
	{
		for (uint32 AspectIndex = 0; AspectIndex < NumRatios; ++AspectIndex)
		{
			...
			if (bInPortraitMode)
			{
				TargetHeight = FMath::RoundToFloat(WorkAreaHeight * Scales[ScaleIndex]);
				TargetWidth = TargetHeight * Aspect;
			}
			else
			{
				TargetWidth = FMath::RoundToFloat(WorkAreaWidth * Scales[ScaleIndex]);
				TargetHeight = TargetWidth * Aspect;
			}

			if (TargetWidth < WorkAreaWidth && TargetHeight < WorkAreaHeight && TargetWidth >= MinWidth && TargetHeight >= MinHeight)
			{
	...
	{
		if (WorkAreaHeight > MinHeight && WorkAreaWidth > MinWidth)
		{
		...
		else
		{
			...
			float TargetWidth = FMath::RoundToFloat((float)WorkAreaWidth) * Scales[NumScales - 1];
			float TargetHeight = FMath::RoundToFloat((float)WorkAreaHeight) * Scales[NumScales - 1];
			OutResolutions.Add(FIntPoint(TargetWidth, TargetHeight));
		}
	}
}
...
{
	GenerateConvenientWindowedResolutions(InDisplayMetrics.PrimaryDisplayWidth, InDisplayMetrics.PrimaryDisplayHeight, OutResolutions);
}
...
TSharedRef<SViewport> UGameEngine::CreateGameViewportWidget()
{
...
{
	GameViewportWidget = CreateGameViewportWidget();
}
...
void UGameEngine::CreateGameViewport( UGameViewportClient* GameViewportClient )
{
	...
	SceneViewport = GameViewportClient->CreateViewport(GameViewportWidget);
	...
	if (FParse::Value(FCommandLine::Get(), TEXT("SAVEWINPOS="), SaveWinPos) && SaveWinPos > 0 )
	{
		...
		{
			UE_LOGF(LogEngine, Warning, "SaveWinPos index %d exceeds the maximum of %d. See cvar GameUserSettings.WindowPositionsMaxSize. Window position won't be saved.", SaveWinPosIndex, MaxSlots - 1);
		}
	}

	RegisterViewport(SceneViewport.ToSharedRef());
...
void UGameEngine::ConditionallyOverrideSettings(int32& ResolutionX, int32& ResolutionY, EWindowMode::Type& WindowMode, int32 MonitorIndex)
{
	...
	DetermineGameWindowResolution(ResolutionX, ResolutionY, WindowMode, false, MonitorIndex);
}

void UGameEngine::DetermineGameWindowResolution( int32& ResolutionX, int32& ResolutionY, EWindowMode::Type& WindowMode, bool bUseWorkAreaForWindowed, int32 MonitorIndex )
{
	...
	if (DisplayMetrics.MonitorInfo.IsValidIndex(MonitorIndex))
	{
		...
		const FMonitorInfo& Info = DisplayMetrics.MonitorInfo[MonitorIndex];
		int32 MonitorWidth = Info.MaxResolution.X;
		int32 MonitorHeight = Info.MaxResolution.Y;
		if (MonitorWidth == 0 || MonitorHeight == 0)
		{
			MonitorWidth = Info.NativeWidth;
			MonitorHeight = Info.NativeHeight;
		}

		DisplayAspect = (float)MonitorWidth / (float)MonitorHeight;

		if (bUseWorkAreaForWindowed && WindowMode == EWindowMode::Windowed)
		{
			MaxResolutionX = Info.WorkArea.Right - Info.WorkArea.Left;
			MaxResolutionY = Info.WorkArea.Bottom - Info.WorkArea.Top;
		}
		else
		{
			MaxResolutionX = MonitorWidth;
			MaxResolutionY = MonitorHeight;
		}
	}
	else if (WindowMode == EWindowMode::Fullscreen && !DisplayMetrics.MonitorInfo.IsEmpty())
	{
	...
	if (!bForceRes && !IsRunningDedicatedServer() && ((ResolutionX <= 0 || ResolutionX > MaxResolutionX) || (ResolutionY <= 0 || ResolutionY > MaxResolutionY)))
	{
		...
		if (WindowMode == EWindowMode::Windowed)
		{
			...
			GenerateConvenientWindowedResolutions(MaxResolutionX, MaxResolutionY, WindowedResolutions);
...
TSharedRef<SWindow> UGameEngine::CreateGameWindow()
{
	...
	const int32 MonitorIndex = GSystemResolution.MonitorIndex;
	{
		ConditionallyOverrideSettings(ResX, ResY, WindowMode, MonitorIndex);
	...
	FText RHIName = FText::FromName(LegacyShaderPlatformToShaderFormat( GMaxRHIShaderPlatform ));
#if WITH_EDITOR
	FString ShaderPlatformName;
	// If we are overriding the ShaderPlatform, update the RHIName
	if (FParse::Value(FCommandLine::Get(), TEXT("OverrideSP="), ShaderPlatformName) && !ShaderPlatformName.IsEmpty())
	{
		...
		if (OverrideShaderPlatform != SP_NumPlatforms)
		{
			if (FDataDrivenShaderPlatformInfo::GetIsPreviewPlatform(OverrideShaderPlatform))
			{
				...
				FString FriendlyDeviceProfilePreviewName = FDataDrivenShaderPlatformInfo::GetFriendlyName(OverrideShaderPlatform).ToString();
				// If we are overriding the DeviceProfile, append the DP name to the RHIName.
				if(FParse::Value(FCommandLine::Get(), TEXT("dp.Override="), DeviceProfileName))
				{
					FriendlyDeviceProfilePreviewName += TEXT(" DeviceProfile: ") + DeviceProfileName;
				}
				RHIName = FText::FromName(FName(FriendlyDeviceProfilePreviewName));
			}
		}
	}
#endif
	Args.Add( TEXT("RHIName"), RHIName);
	...
	if (FParse::Value(FCommandLine::Get(), TEXT("SAVEWINPOS="), SaveWinPos) && SaveWinPos > 0)
	{
		...
		FString ScriptEngineCategory = "/Script/Engine.Engine";
		FString GameUserSettingsCategory = "/Script/Engine.GameUserSettings";
		...
		else
		{
			...
			if (PositionStrings.IsValidIndex(Index))
			{
				...
				{
					UE_LOGF(LogEngine, Warning, "Saved position for SaveWinPos=%d invalid, ignoring.", SaveWinPos);
				}
			}
		}
	}
	...
	if ((MonitorIndex != -1) && (AutoCenterType == EAutoCenter::PrimaryWorkArea))
	{
		FDisplayMetrics DisplayMetrics;
		if (FSlateApplication::IsInitialized())
		{
		...
		{
			AutoCenterType = EAutoCenter::None;
			WinX = DisplayMetrics.MonitorInfo[MonitorIndex].WorkArea.Left;
			WinY = DisplayMetrics.MonitorInfo[MonitorIndex].WorkArea.Top;

			if (WindowMode == EWindowMode::Windowed)
			{
				const FPlatformRect& WorkArea = DisplayMetrics.MonitorInfo[MonitorIndex].WorkArea;
				const int32 WorkWidth = WorkArea.Right - WorkArea.Left;
				const int32 WorkHeight = WorkArea.Bottom - WorkArea.Top;

				if (ResX < WorkWidth)
				{
					WinX += (WorkWidth - ResX) / 2;
				}

				if (ResY < WorkHeight)
				{
					WinY += (WorkHeight - ResY) / 2;
				}
			}
		}
	}
	...
	.Style(bUseBorderlessWindow ? &FWindowStyle::GetBorderless() : &FCoreStyle::Get().GetWidgetStyle<FWindowStyle>("Window"))
...
void UGameEngine::SwitchGameWindowToUseGameViewport()
{
	if (TSharedPtr<SWindow> GameViewportWindowPtr = GameViewportWindow.Pin())
	{
		if (GameViewportWindowPtr->GetContent() != GameViewportWidget)
		{
			...
			{
				UE_LOGF(LogEngine, Error, "The Game Viewport Widget is invalid.");
			}
...
void UGameEngine::OnGameWindowClosed(const TSharedRef<SWindow>& WindowBeingClosed)
{
	...
	// Hide crash for users but not developers.
	if (ensure(SceneViewport.IsValid()))
#endif
	{
		// This will shutdown the game
		GameViewport->CloseRequested(SceneViewport.Get());
		UnregisterViewport(SceneViewport.ToSharedRef());
		SceneViewport.Reset();
	}
}
...
void UGameEngine::OnGPUCrash()
{
	...
	if (!Pixels.IsEmpty())
	{
		...
		{
			UE_LOGF(LogEngine, Warning, "Error saving crash screenshot to file: %ls", *ScreenshotPath);
		}
	}
#endif
}
...
//	#endif
	...
	virtual bool Exec(UWorld* Inworld, const TCHAR* Cmd, FOutputDevice& Ar) override
	{
		...
		{
			UE_LOGF(LogEngine, Display, "Embedded state: %ls", *FEmbeddedCommunication::GetDebugInfo());
			return true;
		}
				...
				if (FJsonSerializer::Deserialize(Reader, CommandObject))
				{
					...
					if (CommandObject->TryGetObjectField(TEXT("Args"), Args))
					{
						for (auto It : (*Args)->Values)
						{
							...
							Helper.Parameters.Add(FString(It.Key), ValueString);
						}
					}
...
void UGameEngine::Init(IEngineLoop* InEngineLoop)
{
	...
	// Create game instance.  For GameEngine, this should be the only GameInstance that ever gets created.
	{
		...
		{
			UE_LOGF(LogEngine, Error, "Unable to load GameInstance Class '%ls'. Falling back to generic UGameInstance.", *GameInstanceClassName.ToString());
...
//  	FWorldContext& InitialWorldContext = CreateNewWorldContext(EWorldType::Game);
	...
	if(ViewportClient)
	{
		...
		{
			UE_LOGF(LogEngine, Log, "GameWindow did not exist.  Was created");
		...
		{
			UE_LOGF(LogEngine, Fatal,"%ls",*Error);
		}
	...
	UE_LOGF(LogInit, Display, "Game Engine Initialized." );
...
void UGameEngine::Start()
{
	...
	UE_LOGF(LogInit, Display, "Starting Game.");
...
{
	UE_LOGF(LogInit, Display, "PreExit Game.");
	...
	for (int32 WorldIndex = 0; WorldIndex < WorldList.Num(); ++WorldIndex)
	{
		...
		{
			// A world may not be bound to the context in an early shutdown case such as the world failing to load. 
			// Preserve the call to Shutdown to avoid introducing a different destruction order in an exceptional error 
			// case.
			CancelPending(WorldList[WorldIndex]);
			
			if (WorldList[WorldIndex].OwningGameInstance != nullptr)
			{
				WorldList[WorldIndex].OwningGameInstance->Shutdown();
			}
		}
	}
...
void UGameEngine::FinishDestroy()
{
	...
	{
		// Game exit.
		UE_LOGF(LogExit, Log, "Game engine shut down" );
	}
...
bool UGameEngine::Exec( UWorld* InWorld, const TCHAR* Cmd, FOutputDevice& Ar )
{
	...
	{
		UE_LOGF(LogConsoleResponse, Warning, "Deprectated command! Please use 'Reattach.Components' instead.");
		return true;
	}
...
void UGameEngine::Tick( float DeltaSeconds, bool bIdleMode )
{
	...
	if( DeltaSeconds < 0.0f )
	{
		...
		UE_LOGF(LogEngine, Fatal,"Negative delta time!");
#else
	...
	{
		UE_LOGF(LogEngine, Log, "Slow GT frame detected (GT frame %llu, delta time %f s)", GFrameCounter - 1, DeltaSeconds);
	}
	...
	{
		UE_LOGF(LogEngine, Log,  "All Windows Closed" );
	...
	for (int32 WorldIdx = 0; WorldIdx < WorldList.Num(); ++WorldIdx)
	{
		...
		if (!IsRunningDedicatedServer() && !IsRunningCommandlet())
		{
			...
			// Checking AreAlwaysLoadedLevelsLoaded() is quite expensive. Check that we have captures to update first.
			if (USkyLightComponent::HasSkyCapturesToUpdate() || UReflectionCaptureComponent::HasReflectionCapturesToUpdate())
			{
		...
		if( Context.World()->bWorldWasLoadedThisTick )
		{
			...
			{
				UE_LOGF(LogEngine, Log, "Issuing initial cause event passed from URL: %ls", InitialExec);
	...
	if (bIsRenderingScene)
	{
		...
		if (!GUseUnifiedTimeBudgetForStreaming && !GIsServer && WorldList.Num() > 0)
		{
			SCOPE_CYCLE_COUNTER(STAT_UpdateLevelStreaming);
			for (FWorldContext& Context : WorldList)
			{
	...
	ENQUEUE_RENDER_COMMAND(TickPoolElements, LoopTick)([](FRHICommandListImmediate& RHICmdList)
	{			
