...
void CacheAudioCookOverrides(FPlatformAudioCookOverrides& OutOverrides, const TCHAR* InPlatformName=nullptr)
{
	...
	if (PlatformFile->GetInt(*CategoryName, TEXT("CacheSizeKB"), RetrievedCacheSize))
	{
		...
		{
			UE_LOGF(LogConfig, Display, "Audio Stream Cache \"Max Cache Size KB\" set to 0 by config: \"%ls%ls.ini\". Default value of %d KB will be used. You can update Project Settings here: Project Settings->Platforms->%ls->Audio->Cook Overrides->Stream Caching->Max Cache Size (KB)",
