	...
	: Super(ObjectInitializer)
{
	...
	static FName CollisionProfileName("OverlapAll");
...
void AVolume::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	static FName NAME_BrushBuilder("BrushBuilder");
...
FBoxSphereBounds AVolume::GetBounds() const
{
	...
	{
		UE_LOGF(LogVolume, Error, "AVolume::GetBounds : No BrushComponent");
...
bool AVolume::EncompassesPoint(FVector Point, float SphereRadius/*=0.f*/, float* OutDistanceToPoint) const
{
	...
	{
		UE_LOGF(LogVolume, Log, "AVolume::EncompassesPoint : No BrushComponent on %ls", *GetActorNameOrLabel());
		return false;
	}
}
