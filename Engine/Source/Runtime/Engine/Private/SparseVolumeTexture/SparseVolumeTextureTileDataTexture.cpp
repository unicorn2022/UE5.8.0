	...
	FallbackValueB(InFallbackValueB)
{
		...
		|| (ResolutionInTiles.Z * SPARSE_VOLUME_TILE_RES_PADDED) > SVT::MaxVolumeTextureDim)
	{
		...
		UE_LOGF(LogSparseVolumeTextureStreamingManager, Warning, "Requested SparseVolumeTexture tile data texture resolution (in tiles) (%i, %i, %i) exceeds the resource size limit. Using the maximum value of (%i, %i. %i) instead.",
