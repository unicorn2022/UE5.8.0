...
void FStreamingManager::BeginAsyncUpdate(FRDGBuilder& GraphBuilder, bool bUseAsyncThread)
{
	...
	{
		UE_LOGF(LogSparseVolumeTextureStreamingManager, Display, "SVT Streaming Update %i", UpdateIndex);
	}
	...
	if (GSVTStreamingPrintMemoryStats != 0)
	{
		for (TUniquePtr<FStreamingInfo>& SVTInfo : StreamingInfo)
		{
			...
			const int32 NumFrames = SVTInfo->PerFrameInfo.Num();
			UE_LOGF(LogSparseVolumeTextureStreamingManager, Display, "Memory stats for SVT '%ls':", *SVTInfo->SVTName.ToString());
			...
			for (int32 FrameIdx = 0; FrameIdx < NumFrames; ++FrameIdx)
			{
				...
				UE_LOGF(LogSparseVolumeTextureStreamingManager, Display, "SVT Frame %3u: Size: %5.4f MiB Num Tiles: %u", FrameIdx, Size / 1024.0f / 1024.0f, NumTiles);
			}

			UE_LOGF(LogSparseVolumeTextureStreamingManager, Display, "SVT Frame Stats: Min: %3.2f MiB, Max: %3.2f MiB, Avg: %3.2f, Total All: %3.2f MiB", (float)MinFrameMiB, (float)MaxFrameMiB, (float)(SumFrameMiB / NumFrames), SumFrameMiB);
		}
...
void FStreamingManager::ComputeBandwidthLimit()
{
	...
	if (GSVTStreamingBandwidthLimit > 0 && (TotalRequestedBandwidth > AvailableBandwidth))
	{
		...
		if (GSVTStreamingLogVerbosity > 2)
		{
			UE_LOGF(LogSparseVolumeTextureStreamingManager, Display, "Total Requested Bandwidth: %f MiB/s, Total Available Bandwidth: %f MiB/s, Percentage: %f %%", 
		...
		for (FStreamingInstance* StreamingInstance : ActiveStreamingInstances)
		{
			...
			if (GSVTStreamingLogVerbosity > 2)
			{
				UE_LOGF(LogSparseVolumeTextureStreamingManager, Display, "Key: %u, Requested: %f MiB, Allocated: %f MiB, FPS: %f, InBudgetMipLevel: %f",
	...
	for (FTileRange& TileRange : TileRangesToStream)
	{
		...
		for (uint32 i = 0; i < TileRange.TileCount; ++i)
		{
			...
			if (TileCoord == INDEX_NONE)
			{
				...
				{
					UE_LOGF(LogSparseVolumeTextureStreamingManager, Warning, "Streaming pool of SVT '%ls' is oversubscribed! Tried to allocate %u tiles for frame %u, but only %u were available.",
			...
			if (PreviousAllocation.bIsAllocated)
			{
				...
				if (uint32* PendingRequestIndexPtr = FrameInfoTmp.TileIndexToPendingRequestIndex.Find((uint32)PreviousAllocation.TileIndexInFrame))
				{
					...
					if (FrameInfoTmp.StreamingTiles.CountSetBits(PendingRequest.TileOffset, PendingRequest.TileOffset + PendingRequest.TileCount) == 0)
					{
						...
						if (GSVTStreamingLogVerbosity > 2)
						{
							UE_LOGF(LogSparseVolumeTextureStreamingManager, Warning, "SVT '%ls' frame %i: Cancelled IO request for frame %i, tile index %i.",
								*SVTInfo->SVTName.ToString(), TileRange.FrameIndex, PreviousAllocation.FrameIndex, PreviousAllocation.TileIndexInFrame);
						}
					}
		...
		// Create request
		{
			auto HandleRequestAllocationFailure = [&](uint32 FirstTileIndex, uint32 LastTileIndex)
			{
				...
				UE_LOGF(LogSparseVolumeTextureStreamingManager, Error, "Ran out of SparseVolumeTexture IO request slots (%i)! r.SparseVolumeTexture.Streaming.MaxPendingRequests must be increased to fix this issue.", MaxPendingRequests);
			};
...
int32 FStreamingManager::DetermineReadyRequests()
{
	...
	for (int32 i = 0; i < NumPendingRequests; ++i)
	{
		...
		if (PendingRequest.State == FPendingRequest::EState::DDC_Ready)
		{
			...
			{
				check(SVTInfo);
				UE_LOGF(LogSparseVolumeTextureStreamingManager, Display, "SVT DDC retry succeeded for '%ls' (frame %i, tile offset %i, tile count %i) on %i attempt.",
		...
		else if (PendingRequest.State == FPendingRequest::EState::DDC_Failed)
		{
			...
			{
				UE_LOGF(LogSparseVolumeTextureStreamingManager, Warning, "SVT DDC request failed for '%ls' (frame %i, tile offset %i, tile count %i). Retrying...",
...
#endif // WITH_EDITORONLY_DATA
		{
			...
			if (PendingRequest.Request.IsCompleted())
			{
				if (!PendingRequest.Request.IsOk())
				{
					...
					{
						UE_LOGF(LogSparseVolumeTextureStreamingManager, Warning, "SVT IO request failed for %ls (frame %i, tile offset %i, tile count %i, offset %i, size %i). Retrying...",
