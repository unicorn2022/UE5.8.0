...
{
	static FString CachedVersionString = "C9CD1136-8E72-4E0B-A846-15B339414160";	// Bump this if you want to ignore all cached data so far.
...
bool FHeader::Validate(bool bPrintToLog)
{
	for (int32 i = 0; i < 2; ++i)
	{
		if (!SVT::IsSupportedFormat(AttributesFormats[i]))
		{
			if (bPrintToLog)
			{
				UE_LOGF(LogSparseVolumeTexture, Warning, "'%ls' is not a supported SparseVolumeTexture format!", GPixelFormats[AttributesFormats[i]].Name);
			}
			return false;
		}
	}
	...
	if (PageTableVolumeResolution.X > SVT::MaxVolumeTextureDim || PageTableVolumeResolution.Y > SVT::MaxVolumeTextureDim || PageTableVolumeResolution.Z > SVT::MaxVolumeTextureDim)
	{
		if (bPrintToLog)
		{
			UE_LOGF(LogSparseVolumeTexture, Warning, "SparseVolumeTexture page table texture dimensions exceed limit (%ix%ix%i): %ix%ix%i",
	...
	if (PageTableSizeBytes > SVT::MaxResourceSize)
	{
		if (bPrintToLog)
		{
			UE_LOGF(LogSparseVolumeTexture, Warning, "SparseVolumeTexture page table texture memory size (%lld) exceeds the 2048MB GPU resource limit!", (long long)PageTableSizeBytes);
		}
		return false;
	}
...
void USparseVolumeTexture::NotifyMaterials(const ENotifyMaterialsEffectOnShaders EffectOnShaders)
{
	// Create a material update context to safely update materials.
	{
		...
		for (TObjectIterator<UMaterialInterface> It; It; ++It)
		{
			...
			if (!FPlatformProperties::IsServerOnly() && (MaterialInterface->ContainsDefaultTexture(this)))
			{
...
bool USparseVolumeTextureFrame::IsCachedCookedPlatformDataLoaded(const ITargetPlatform* TargetPlatform)
{
	...
	if (bFailed)
	{
		UE_LOGF(LogSparseVolumeTexture, Log, "Failed to recover SparseVolumeTexture streaming from DDC for '%ls' Frame %i. Rebuilding and retrying.", *Owner->GetPathName(), FrameIndex);
...
bool UStreamableSparseVolumeTexture::BeginInitialize(int32 NumExpectedFrames)
{
	...
	{
		UE_LOGF(LogSparseVolumeTexture, Error, "Tried to call UStreamableSparseVolumeTexture::BeginInitialize() while not in the Uninitialized init state.");
		return false;
	}
...
bool UStreamableSparseVolumeTexture::AppendFrame(UE::SVT::FTextureData& UncookedFrame, const FTransform& FrameTransform)
{
	...
	{
		UE_LOGF(LogSparseVolumeTexture, Error, "Tried to call UStreamableSparseVolumeTexture::AppendFrame() while not in the Pending init state.");
		return false;
	}
	...
	{
		UE_LOGF(LogSparseVolumeTexture, Error, "Tried to add a frame to a SparseVolumeTexture with a VirtualVolumeAABBMin < 0 (%i, %i, %i)",
	...
	if (Frames.IsEmpty())
	{
		...
		{
			UE_LOGF(LogSparseVolumeTexture, Error, "Tried to add a frame with unsupported formats to a SparseVolumeTexture! Formats: (%i %i)",
	...
	else
	{
		...
		{
			UE_LOGF(LogSparseVolumeTexture, Error, "Tried to add a frame to a SparseVolumeTexture without matching formats! Expected: (%i, %i), Actual: (%i %i)",
		...
		{
			UE_LOGF(LogSparseVolumeTexture, Error, "Tried to add a frame to a SparseVolumeTexture without matching fallback/null tile values!");
			return false;
		}
	}
...
bool UStreamableSparseVolumeTexture::EndInitialize()
{
	...
	if (InitState != EInitState_Pending)
	{
		UE_LOGF(LogSparseVolumeTexture, Error, "Tried to call UStreamableSparseVolumeTexture::EndInitialize() while not in the Pending init state.");
		return false;
	}
	...
	if (Frames.IsEmpty())
	{
		UE_LOGF(LogSparseVolumeTexture, Warning, "SVT has zero frames! Adding a dummy frame. SVT: %ls", *GetName());
	...
	{
		UE_LOGF(LogSparseVolumeTexture, Warning, "Initialized a SparseVolumeTexture with a VirtualVolumeAABBMin > 0 (%i, %i, %i). This wastes memory",
			VolumeBoundsMin.X, VolumeBoundsMin.Y, VolumeBoundsMin.Z);
	}
...
bool UStreamableSparseVolumeTexture::Initialize(const TArrayView<UE::SVT::FTextureData>& InUncookedData, const TArrayView<FTransform>& InFrameTransforms)
{
	...
	{
		UE_LOGF(LogSparseVolumeTexture, Error, "Tried to initialize a SparseVolumeTexture with no frames");
		return false;
	}
...
void UStreamableSparseVolumeTexture::RecacheFrames()
{
	...
	{
		UE_LOGF(LogSparseVolumeTexture, Warning, "Tried to cache derived data of an uninitialized SVT: %ls", *GetName());
		return;
	}
	...
	for (USparseVolumeTextureFrame* Frame : Frames)
	{
		...
		{
			bCanceled = true;
			UE_LOGF(LogSparseVolumeTexture, Warning, "Canceled '%ls' SparseVolumeTexture caching. All frames not yet cached will be empty!", *GetName());
		}
		if (!bCanceled)
		{
			RecacheTask.EnterProgressFrame(1.0f);
...
bool UStaticSparseVolumeTexture::AppendFrame(UE::SVT::FTextureData& UncookedFrame, const FTransform& InFrameTransform)
{
	if (!Frames.IsEmpty())
	{
		UE_LOGF(LogSparseVolumeTexture, Error, "Tried to initialize a UStaticSparseVolumeTexture with more than 1 frame");
		return false;
	}
