...
static TAutoConsoleVariable<bool> CVarSkinnedMeshSkinWeightProfilesOnRenderThread(
	TEXT("r.SkinnedMesh.SkinWeightProfilesOnRenderThread"),
	false,
	TEXT("Do all skin weight profile management on the render thread (experimental / disabled by default - causing busted vertices)"),
	ECVF_Default);
...
,	MinLODLevel(InMeshDesc.GetMinLODLevel())
...
void FSkeletalMeshObject::UpdateMinDesiredLODLevel(const FSceneView* View, const FBoxSphereBounds& Bounds)
{
	...
	const bool bMultiView = View->Family->AllViews.Num() > 1;
	if (bMultiView)
	{
...
FSkinWeightVertexBuffer* FSkeletalMeshObject::GetSkinWeightVertexBuffer(FSkeletalMeshLODRenderData& LODData, const FSkelMeshComponentLODInfo* CompLODInfo)
{
	if (CompLODInfo)
	{
		if (CompLODInfo->OverrideSkinWeights && CompLODInfo->OverrideSkinWeights->GetNumVertices() == LODData.StaticVertexBuffers.PositionVertexBuffer.GetNumVertices())
		{
			return CompLODInfo->OverrideSkinWeights;
		}
		
		FSkinWeightProfileStack ActiveProfileStack;
		const bool bRenderSideProfiles = IsSkinnedMeshSkinWeightProfilesOnRenderThread();
		
		if (bRenderSideProfiles 
			&& CompLODInfo->GetActiveSkinWeightProfileStack(ActiveProfileStack))
		{
			// If buffer is null, it means it has not been created yet and probably
			// GPU Readback has been enqueued. The default override buffer will be 
			// returned. Once the buffer is ready, skinned mesh component will update 
			// its skin weight buffer accordingly. 
			FSkinWeightVertexBuffer* OverrideProfileSkinWeights = LODData.GetSkinWeightProfileVertexBuffer(ActiveProfileStack);
			if (OverrideProfileSkinWeights)
			{
				check(OverrideProfileSkinWeights->GetNumVertices() == LODData.StaticVertexBuffers.PositionVertexBuffer.GetNumVertices());
				check(LODData.SkinWeightVertexBuffer.GetMaxBoneInfluences() == OverrideProfileSkinWeights->GetMaxBoneInfluences());
				return OverrideProfileSkinWeights;
			}
		}
		
		if (!bRenderSideProfiles && CompLODInfo->OverrideProfileSkinWeights &&
	...
	TArray<FTransform>* LeaderBoneMappedComponentSpaceTransform)
{
	...
	const bool bCheckForValidMeshPoseOnLeader = UE::Anim::bUseSafeMeshPoseIndices && bIsLeaderCompValid && !DynamicData.GetLeaderValidMeshPoseTransforms()->IsEmpty();
	...
	for (int32 RequiredBoneSetIndex = 0; RequiredBoneSets[RequiredBoneSetIndex] != NULL; RequiredBoneSetIndex++)
	{
		...
		for (int32 BoneIndex = 0; BoneIndex < NumRequiredBoneIndices; BoneIndex++)
		{
			...
			if( bIsLeaderCompValid )
			{
				...
				const TBitArray<>& LeaderValidMeshPoseTransforms = *(DynamicData.GetLeaderValidMeshPoseTransforms());

				if (ComponentTransform.IsValidIndex(LeaderBoneIndex) && (!bCheckForValidMeshPoseOnLeader || LeaderValidMeshPoseTransforms[LeaderBoneIndex]))
				{
...
void UpdateBonesRemovedByLOD(
	...
	const USkinnedAsset* SkinnedAsset,
	int32 LODIndex,
	bool bUsePreviousTransforms)
{
	// When the animation system removes bones at higher LODs, the pose in USkinnedMeshComponent::GetComponentSpaceTransforms()
	// will leave the LOD'd bone transforms at their last updated position/rotation. This is not a problem for standard GPU skinning
	...
	//
	// But Nanite skinning uses LOD-0 weights (dynamically interpolating for higher-LOD clusters), and the ray tracing LOD may differ
	// from the main rendering LOD. In either case, the "frozen" bone transforms affect the skin and must be corrected.
	//
	// This function performs an FK update of the frozen branch of transforms to put them back in their ref pose relative position.
	...
	TArrayView<const FTransform> ComponentSpacePose = bUsePreviousTransforms
		? InDynamicData.GetPreviousComponentTransformsArray()
		: InDynamicData.GetComponentSpaceTransforms();

	if (ComponentSpacePose.IsEmpty())
	{
		return;
	}
	...
	if (PoseBuffer.Num() < NumBones || RefBasesInvMatrix->Num() < NumBones)
	{
		return;
	}
	...
	for (const FBoneReference& RemovedBone : BonesToRemove)
	{
		...
		const int32 BoneIndex = RefSkeleton.FindRawBoneIndex(RemovedBone.BoneName);
		...
		// First pass: generate component space transforms from ref pose
		for (int32 ChildIndex = 0; ChildIndex < AllChildrenBones.Num(); ++ChildIndex)
		{
			const int32 ChildBoneIndex = AllChildrenBones[ChildIndex];
			const int32 ParentIndex = RefSkeleton.GetParentIndex(ChildBoneIndex);

			FMatrix44f ParentComponentTransform;
			if (ParentIndex == INDEX_NONE)
			{
				ParentComponentTransform = FMatrix44f::Identity;
			}
			else if (ChildIndex == 0)
			{
				ParentComponentTransform = ComponentSpacePose.IsValidIndex(ParentIndex)
					? static_cast<FMatrix44f>(ComponentSpacePose[ParentIndex].ToMatrixWithScale())
					: FMatrix44f::Identity;
			}
			else
			{
		...
		// Second pass: make relative to ref pose
		for (const int32 ChildBoneIndex : AllChildrenBones)
		{
...
bool IsSkinnedMeshSkinWeightProfilesOnRenderThread()
{
	return CVarSkinnedMeshSkinWeightProfilesOnRenderThread.GetValueOnAnyThread();
}
