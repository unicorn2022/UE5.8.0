...
#if WITH_EDITOR
	// Tiles have been deserialized from original, so we should be considered up-to-date
	bRescanned = true;
#endif // WITH_EDITOR
...
void UWorldComposition::Rescan(bool bForce)
{
...
void UWorldComposition::CaclulateTilesAbsolutePositions()
{
	for (FWorldCompositionTile& Tile : Tiles)
	{
		...
		do 
		{
			...
			{
				UE_LOGF(LogWorldComposition, Warning, "World composition tile (%ls) has a cycled parent (%ls)", *Tile.PackageName.ToString(), *NextParentTileName.ToString());
...
{
	// UWorld::InitWorld has not been invoked yet, but we need the tiles so they get packaged. Scan now.
	Rescan();
