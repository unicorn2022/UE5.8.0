...
namespace Audio
{
	...
	void FSoundWavePCMLoader::LoadSoundWave(USoundWave* InSoundWave, TFunction<void(const USoundWave* SoundWave, const Audio::FSampleBuffer& OutSampleBuffer)> OnLoaded, bool bSynchronous /** = false */)
	{
		...
		if (bRequestPrecache)
		{
			...
			AudioDevice->Precache(InSoundWave, bSynchronous, false, true);
		}
		else
		{
	...
	bool FSoundWavePCMWriter::BeginGeneratingSoundWaveFromBuffer(const TSampleBuffer<>& InSampleBuffer, USoundWave* SoundWaveToSaveTo, TFunction<void(const USoundWave*)> OnSuccess)
	{
		if (!IsDone())
		{
			UE_LOGF(LogAudio, Error, "This instance of FSoundWavePCMWriter is already processing another write operation.");
			return false;
		}
	...
	bool FSoundWavePCMWriter::BeginWriteToSoundWave(const FString& FileName, const TSampleBuffer<>& InSampleBuffer, FString InPath, TFunction<void(const USoundWave*)> OnSuccess, TObjectPtr<USoundWave> InSoundWaveTransformationTarget)
	{
		...
		if (!GIsEditor)
		{
			UE_LOGF(LogAudio, Error, "Writing to a SoundWave is only available in the editor.");
			return false;
		}
		...
		AbsoluteFilePath = TEXT("/Game/") + InPath + FString("/") + FileName;
		...
		{
			UE_LOGF(LogAudio, Warning, "File path given was not valid (%ls): the recording output will be saved to a USoundWave at the root of the Content folder.", *InvalidPathReason.ToString());
			AbsoluteFilePath = TEXT("/Game/") + FString("/") + FileName;
			AbsoluteFilePath = AbsoluteFilePath.Replace(TEXT("//"), TEXT("/"), ESearchCase::CaseSensitive);
		}
		...
		CurrentOperation.Reset(new FAsyncSoundWavePCMWriterTask(this, ESoundWavePCMWriteTaskType::GenerateAndWriteSoundWave, [this, OnSuccess, InSoundWaveTransformationTarget](const USoundWave* InSoundWave)
		{
			...
			{
				UE_LOGF(LogAudio, Error, "CurrentSoundWave is not valid in GenerateAndWriteSoundWave.");
				return;
			}
	...
	bool FSoundWavePCMWriter::PrepWavFileOutput(const TSampleBuffer<>& InSampleBuffer, const FString& FileName, const FString& FilePath)
	{
		...
		{
			UE_LOGF(LogAudio, Error, "Write to Wav File failed: Invalid directory path %ls", *AbsoluteFilePath);
	...
	USoundWave* FSoundWavePCMWriter::SynchronouslyWriteSoundWave(const TSampleBuffer<>& InSampleBuffer, const FString* FileName, const FString* FilePath, TObjectPtr<USoundWave> InSoundWaveTransformationTarget)
	{
		if (!IsDone())
		{
			UE_LOGF(LogAudio, Error, "This instance of FSoundWavePCMWriter is already processing another write operation.");
		...
		if (GIsEditor && FileName != nullptr)
		{
			if (FilePath != nullptr)
			{
				AbsoluteFilePath = TEXT("/Game/") + *FilePath + FString("/") + *FileName;
			}
			else
			{
		...
		// Invalidate compressed data on new soundwave so that it gets refreshed or the soundwave is not seekable
		// and causes this error: FMixerBuffer::CreateStreamingBuffer failed to StreamCompressedInfo on SoundWave
		CurrentSoundWave->InvalidateCompressedData(true);
	...
	USoundWave* FSoundWavePCMWriter::GetFinishedSoundWave()
	{
		if (!IsDone())
		{
			UE_LOGF(LogAudio, Warning, "Failed to get finished soundwave: write operation currently still in progress.");
			return nullptr;
		}
		else if (CurrentState != ESoundWavePCMWriterState::Suceeded)
		{
			UE_LOGF(LogAudio, Warning, "Failed to get finished soundwave: write operation failed.");
			return nullptr;
		}
	...
	void FSoundWavePCMWriter::SaveFinishedSoundWaveToPath(const FString& FileName, FString InPath)
	{
		...
		if (!GIsEditor)
		{
			UE_LOGF(LogAudio, Warning, "SoundWave assets can only be saved with the editor.");
			return;
		}
		else if (!IsDone())
		{
			UE_LOGF(LogAudio, Warning, "Failed to kick off save: write operation still in progress.");
			return;
		}
		else if (CurrentState != ESoundWavePCMWriterState::Suceeded)
		{
			UE_LOGF(LogAudio, Warning, "Failed to kick off save: write operation failed.");
			return;
		}

		if (!CreateDirectoryIfNeeded(InPath))
		{
			UE_LOGF(LogAudio, Warning, "Failed to kick off save: invalid directory %ls", *InPath);
			return;
		}

		AbsoluteFilePath = InPath + FString("/") + FileName;
		AbsoluteFilePath = AbsoluteFilePath.Replace(TEXT("//"), TEXT("/"), ESearchCase::CaseSensitive);
	...
	void FSoundWavePCMWriter::SerializeSoundWaveToAsset()
	{
		...
		{
			UE_LOGF(LogAudio, Error, "Writing out wav file failed- There was no audio data to write.");
			CurrentState = ESoundWavePCMWriterState::Failed;
			return;
		}
		...
		UE_LOGF(LogAudio, Verbose, "Serializing %d sample file (%d bytes) to sound asset at %ls", CurrentBuffer.GetNumSamples(), SerializedWavData.Num(), *AbsoluteFilePath);
	...
	void FSoundWavePCMWriter::SerializeBufferToWavFile()
	{
		...
		SerializeWaveFile(SerializedWavData, (const uint8*) CurrentBuffer.GetData(), CurrentBuffer.GetNumSamples() * sizeof(int16), CurrentBuffer.GetNumChannels(), CurrentBuffer.GetSampleRate());
		UE_LOGF(LogAudio, Verbose, "Serializing %d sample file (%d bytes) to %ls", CurrentBuffer.GetNumSamples(), SerializedWavData.Num(), *AbsoluteFilePath);
		if (SerializedWavData.Num() == 0)
		{
			UE_LOGF(LogAudio, Error, "Wave serialize operation failed: failiure in SerializeWaveFile");
			CurrentState = ESoundWavePCMWriterState::Failed;
			return;
		}
		else
		{
			...
			if (FileHandle)
			{
				...
				UE_LOGF(LogAudio, Verbose, "Writing wav file in %d chunks...", NumChunks);
				for (int32 ChunkIndex = 0; ChunkIndex < NumChunks; ChunkIndex++)
				{
					...
					{
						UE_LOGF(LogAudio, Error, "Wave serialize operation failed: failiure in SerializeWaveFile. Chunk: %d Index: %d ChunkSize: %d", ChunkIndex, BufferIndex, CurrentChunkSize);
						SerializedWavData.Reset();
						delete FileHandle;
						CurrentState = ESoundWavePCMWriterState::Failed;
						return;
		...
		UE_LOGF(LogAudio, Verbose, "Succeeded in writing wav file.");
		CurrentState = ESoundWavePCMWriterState::Suceeded;
	}
