...
void UBlueprintPlatformLibrary::ClearAllLocalNotifications()
{
	if(platformService == nullptr)
	{
		UE_LOGF(LogBlueprintUserMessages, Warning, "ClearAllLocalNotifications(): No local notification service");
		return;
	}
...
int32 UBlueprintPlatformLibrary::ScheduleLocalNotificationAtTime(const FDateTime& FireDateTime, bool inLocalTime, const FText& Title, const FText& Body, const FText& Action, const FString& ActivationEvent)
{
	if(platformService == nullptr)
	{
		UE_LOGF(LogBlueprintUserMessages, Warning, "ScheduleLocalNotificationAtTime(): No local notification service");
		return -1;
	}

	UE_LOGF(LogBlueprintUserMessages, Log, "Scheduling notification %ls at %d/%d/%d %d:%d:%d %ls", *(Title.ToString()), FireDateTime.GetMonth(), FireDateTime.GetDay(), FireDateTime.GetYear(), FireDateTime.GetHour(), FireDateTime.GetMinute(), FireDateTime.GetSecond(), inLocalTime ? TEXT("Local") : TEXT("UTC"));
...
int32 UBlueprintPlatformLibrary::ScheduleLocalNotificationBadgeAtTime(const FDateTime& FireDateTime, bool inLocalTime, const FString& ActivationEvent)
{
	if (platformService == nullptr)
	{
		UE_LOGF(LogBlueprintUserMessages, Warning, "ScheduleLocalNotificationBadgeAtTime(): No local notification service");
		return -1;
	}

	UE_LOGF(LogBlueprintUserMessages, Log, "Scheduling notification badge %ls at %d/%d/%d %d:%d:%d %ls", *ActivationEvent, FireDateTime.GetMonth(), FireDateTime.GetDay(), FireDateTime.GetYear(), FireDateTime.GetHour(), FireDateTime.GetMinute(), FireDateTime.GetSecond(), inLocalTime ? TEXT("Local") : TEXT("UTC"));
...
void UBlueprintPlatformLibrary::CancelLocalNotification(const FString& ActivationEvent)
{
	if(platformService == nullptr)
	{
		UE_LOGF(LogBlueprintUserMessages, Warning, "CancelLocalNotification(): No local notification service");
		return;
	}

	UE_LOGF(LogBlueprintUserMessages, Log, "Canceling notification %ls", *ActivationEvent);
...
void UBlueprintPlatformLibrary::CancelLocalNotificationById(int32 NotificationId)
{
	...
	UE_LOGF(LogBlueprintUserMessages, Log, "Canceling notification %d", NotificationId);
...
void UBlueprintPlatformLibrary::GetLaunchNotification(bool& NotificationLaunchedApp, FString& ActivationEvent, int32& FireDate)
{
	if(platformService == nullptr)
	{
		UE_LOGF(LogBlueprintUserMessages, Warning, "GetLaunchNotification(): No local notification service");
		return;
	}
