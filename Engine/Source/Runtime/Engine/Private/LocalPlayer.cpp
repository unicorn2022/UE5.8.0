...
UScriptViewportClient* ULocalPlayer::GetViewportClient() const
{
	return ViewportClientOverride
		? ViewportClientOverride.Get()
		: ViewportClient.Get();
}

void ULocalPlayer::OverrideViewportClient(UScriptViewportClient* InViewportClient)
{
	ViewportClientOverride = InViewportClient;
}
...
void ULocalPlayer::SendSplitJoin(TArray<FString>& Options, UNetDriver* NetDriver, UE::Net::EJoinFlags Flags)
{
	...
	{
		UE_LOGF(LogPlayerManagement, Warning, "SendSplitJoin(): Not connected to a server");
	}
	...
	void LockView(ULocalPlayer* Player, const TArray<FString>& Args)
	{
		...
		if (bPrintHelp)
		{
			UE_LOGF(LogConsoleResponse,Display,
				"Locks the player view and rendering time.\n" "r.LockView ?\n" "   Displays this message.\n" "r.LockView\n" "   Toggles whether the view is currently locked.\n" "r.LockView <name>\n" "   Locks the view at the named location. If there is no stored view with that name the current view is stored with that name.\n" "r.LockView x y z pitch yaw roll\n" "   Locks the view at the specified location and rotation.\n" "r.LockView <name> x y z pitch yaw roll\n" "   Locks the view at the specified location and rotation and stores it with the specified name.\n"
				);
		}
	}
...
private:
	...
	static void CopyLockedViews()
	{
		...
		UE_LOGF(LogConsoleResponse,Display,"%ls",*LockedViewsStr);
	}
...
void ULocalPlayer::GetViewPoint(FMinimalViewInfo& OutViewInfo) const
{
	...
	if (ViewportClient != nullptr && ViewportClientOverride == nullptr)
	{
	...
	int32 StereoViewIndex)
{
	...
	if ((PlayerController == nullptr) || (Size.X <= 0.f) || (Size.Y <= 0.f) || (Viewport == nullptr))
	{
		return false;
	}
	...
	// True when this view should use a device-managed ViewState.
	const bool bUseExternalViewState = GEngine->IsStereoscopic3D(Viewport)
		&& GEngine->StereoRenderingDevice->HasExternalViewState(StereoViewIndex);

	if (!bUseExternalViewState)
	{
		...
		// Use ViewState from ULocalPlayer
		ViewInitOptions.SceneViewStateInterface = ViewStates[ViewIndex].GetReference();
	}
	else
	{
		// Use ViewState from IStereoRendering
		ViewInitOptions.SceneViewStateInterface = GEngine->StereoRenderingDevice->GetExternalViewState(StereoViewIndex);
	}
...
bool ULocalPlayer::HandleDNCommand( const TCHAR* Cmd, FOutputDevice& Ar )
{
	...
	if( PlayerController )
	{
		...
		UE_LOGF(LogPlayerManagement, Log, "Note Dropped: (%3.2f,%3.2f,%3.2f) - '%ls'", NewNote.Location.X, NewNote.Location.Y, NewNote.Location.Z, *NewNote.Comment);
	}
	return true;
}
...
bool ULocalPlayer::HandleListSkelMeshesCommand( const TCHAR* Cmd, FOutputDevice& Ar )
{
	...
	for( TObjectIterator<USkeletalMesh> It; It; ++It )
	{
		...
		if( SkeletalMesh && SkeletalMeshComponents.Num() )
		{
			...
			UE_LOGF(LogPlayerManagement, Log, "%5i Vertices for LOD 0 of %ls", SkelMeshRenderData->LODRenderData[0].GetNumVertices(),*SkeletalMesh->GetFullName());
			...
			for( int32 InstanceIndex=0; InstanceIndex<SkeletalMeshComponents.Num(); InstanceIndex++ )
			{
				...
				UE_LOGF(LogPlayerManagement, Log, "%ls%2i  Component    : %ls",
					(TimeSinceLastRender > 0.5) ? TEXT(" ") : TEXT("*"),
					InstanceIndex,
					*SkeletalMeshComponent->GetFullName() );
				if( SkeletalMeshComponent->GetOwner() )
				{
					UE_LOGF(LogPlayerManagement, Log, "     Owner        : %ls",*SkeletalMeshComponent->GetOwner()->GetFullName());
				}
				UE_LOGF(LogPlayerManagement, Log, "     LastRender   : %f", TimeSinceLastRender);
				UE_LOGF(LogPlayerManagement, Log, "     CullDistance : %f   Distance: %f   Location: (%7.1f,%7.1f,%7.1f)",
...
bool ULocalPlayer::HandleListPawnComponentsCommand( const TCHAR* Cmd, FOutputDevice& Ar )
{
	...
	{
		APawn *Pawn = *It;
		UE_LOGF(LogPlayerManagement, Log, "Components for pawn: %ls (collision component: %ls)",*Pawn->GetName(),*Pawn->GetRootComponent()->GetName());
		...
		for (UActorComponent* Comp : Pawn->GetComponents())
		{
			...
			{
				UE_LOGF(LogPlayerManagement, Log, "  %d: %ls",CompIdx,*Comp->GetName());
			}
			++CompIdx;
		}
	}
...
//Added ability to toggle this on during a test build if needed
	...
	if( GetEmitDrawEvents() )
	{
		...
		UE_LOGF(LogEngine, Warning, "Draw events are now DISABLED");
	}
	else
	{
		SetEmitDrawEvents(true);
		UE_LOGF(LogEngine, Warning, "Draw events are now ENABLED");
	}
#endif
	return true;
}
...
void ULocalPlayer::ExecMacro( const TCHAR* Filename, FOutputDevice& Ar )
{
	...
	{
		FixedFilename = FString("../../Binaries/") + Filename;
	...
	{
		UE_LOGF(LogPlayerManagement, Log, "Execing %ls", Filename);
...
void ULocalPlayer::SetControllerId( int32 NewControllerId )
{
	...
	{
		UE_LOGF(LogPlayerManagement, Log, "%ls changing ControllerId from %i to %i", *GetFName().ToString(), ControllerId, NewControllerId);
