...
static int32 GShowTimeStampsInStatUnit = 0;
static FAutoConsoleVariableRef CVarShowTimeStampsInStatUnit(
	TEXT("stats.UnitTimestamp"),
	GShowTimeStampsInStatUnit,
	TEXT("Enables UTC time and framecounter in stat unit."));
...
float FRenderTarget::GetEngineDisplayGamma() // static
{
	...
	else
	{
		...
		{
			UE_LOGF(LogClient, Error, "Invalid DisplayGamma! Resetting to the default of 2.2");
...
void FScreenshotRequest::RequestScreenshot(bool bInShowUI, const bool bInRestrictToGameViewport)
{
	// empty string means we'll later pick the name
	RequestScreenshot(TEXT(""), bInShowUI, /*bAddUniqueSuffix*/ true, /*bHdrScreenshot*/ false, FIntRect(), bInRestrictToGameViewport);
}

void FScreenshotRequest::RequestScreenshot(const FString& InFilename, bool bInShowUI, bool bAddUniqueSuffix, bool bHdrScreenshot, const FIntRect& InViewRect, const bool bInRestrictToGameViewport)
{
	...
	{
		bShowUI = bInShowUI;
		bRestrictToGameViewport = bInRestrictToGameViewport;
		ViewRect = InViewRect;
...
FOnHDRScreenshotCaptured FScreenshotRequest::HDRScreenshotCapturedDelegate;
...
bool FScreenshotRequest::bRestrictToGameViewport = false;
...
int32 FStatUnitData::DrawStat(FViewport* InViewport, FCanvas* InCanvas, int32 InX, int32 InY)
{
	...
	const bool bShowUnitCriticalPathTimes = Client && Client->IsStatEnabled(TEXT("UnitCriticalPath"));
	const bool bShowUnitMaxTimes = Client && Client->IsStatEnabled(TEXT("UnitMax"));
	const bool bShowTSRStatistics = Client && Client->IsStatEnabled(TEXT("TSR"));
#if !UE_BUILD_SHIPPING
	const bool bShowRawUnitTimes = Client && Client->IsStatEnabled(TEXT("Raw"));
	...
	const bool bShowUnitTimeGraph = Client && Client->IsStatEnabled(TEXT("UnitGraph"));
	...
	// Draw unit.
	{
...
#if PLATFORM_DESKTOP
		{
			FRHIMemoryStats RHIMemStats;
			RHIGetMemoryStats(RHIMemStats);

			if (RHIMemStats.IsValid())
			{
				const bool bOverBudget = RHIMemStats.IsOverBudget();
				const FColor Color = bOverBudget ? StatRed : StatGreen;
				DrawTitleString(TEXT("VRam"), NoUnitGraphColor);
				// Show "Used / Budget" format
				FString VRAMText = FString::Printf(TEXT("%s / %s"),
					*GetMemoryString(RHIMemStats.UsedLocal),
					*GetMemoryString(RHIMemStats.BudgetLocal));
				DrawDefaultAvgCell(VRAMText, Color);
		...
		if (DynamicResolutionStateInfos.Status == EDynamicResolutionStatus::Enabled || DynamicResolutionStateInfos.Status == EDynamicResolutionStatus::DebugForceEnabled)
		{
			...
			if (DynamicResolutionStateInfos.Status == EDynamicResolutionStatus::Enabled && DynamicRenderScaling::IsSupported())
			{
				for (TLinkedList<DynamicRenderScaling::FBudget*>::TIterator BudgetIt(DynamicRenderScaling::FBudget::GetGlobalList()); BudgetIt; BudgetIt.Next())
				{
					...
					ResolutionFraction = DynamicResolutionStateInfos.ResolutionFractionApproximations[Budget];
					ScreenPercentage = ResolutionFraction * 100.0f;
		...
		// Timestamp / Frame count if enabled
		if (GShowTimeStampsInStatUnit)
		{
			DrawTitleString(TEXT("UTC Time"), NoUnitGraphColor);
			DrawDefaultAvgCell(FDateTime::UtcNow().ToString(TEXT("[%H.%M.%S:%s]")), StatGreen);
			PushRows(/* RowsCount = */ 1);

			DrawTitleString(TEXT("FrameNum"), NoUnitGraphColor);
			DrawDefaultAvgCell(FString::Printf(TEXT("%llu"), GFrameCounter), StatGreen);
			PushRows(/* RowsCount = */ 1);
		}
	...
	if (bShowUnitTimeGraph)
	{
		...
		const bool bShowFrameTimeInUnitGraph = Client && Client->IsStatEnabled(TEXT("UnitTime"));
		for (int32 StatIndex = 0; StatIndex < EGS_Count; ++StatIndex)
		{
			...
			else if (StatIndex == EGS_DynRes) //-V547
			{
				...
				RelativeAlertValueThreshold = 0.05f;
...
int32 FStatThermalsData::DrawStat(FViewport* InViewport, FCanvas* InCanvas, int32 InX, int32 InY)
{
	const EDeviceThermalState CurrentThermalState = FPlatformMisc::GetDeviceThermalState();

	FLinearColor StatColor = FColor::White;
	FString SeverityStr;
	switch (CurrentThermalState)
	{
	case EDeviceThermalState::Unsupported:
		SeverityStr = TEXT("Unsupported");
		StatColor = FColor::Blue;
		break;
	case EDeviceThermalState::None:
		SeverityStr = TEXT("None");
		StatColor = FColor::Green;
		break;
	case EDeviceThermalState::Light:
		SeverityStr = TEXT("Light");
		StatColor = FColor::Yellow;
		break;
	case EDeviceThermalState::Moderate:
		SeverityStr = TEXT("Moderate");
		StatColor = FColor::Orange;
		break;
	case  EDeviceThermalState::Severe:
		SeverityStr = TEXT("Severe");
		StatColor = FColor::Red;
		break;
	...
	const FString LabelText = TEXT("Thermal throttle state:   ");
	const int32 TitleSize = SmallFont->GetStringSize(*LabelText);

	const int32 LabelCoordX = InX - TitleSize;
	InCanvas->DrawShadowedString(
		LabelCoordX,
		InY,
		*LabelText,
...
int32 FStatHitchesData::DrawStat(FViewport* InViewport, FCanvas* InCanvas, int32 InX, int32 InY)
{
	...
	if (LastTime > 0)
	{
		...
		if (DeltaSeconds > HitchThresholdSecs)
		{
			...
			UE_LOGF(LogClient, Warning, "HITCH %d              running cnt = %5d", int32(DeltaSeconds * 1000), Count++);
		}
...
FViewport::FViewport(FViewportClient* const InClient)
	: FViewport()
{
PRAGMA_DISABLE_DEPRECATION_WARNINGS
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	if (InClient)
	{
		...
		bRequiresHitProxyStorage = InClient->RequiresHitProxyStorage();
#if !WITH_EDITOR && !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
		if (bRequiresHitProxyStorage)
		{
			UE_LOGF(LogClient, Warning, "Consoles don't need hitproxy storage - wasting memory!?");
		}
#endif
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	, bRequiresHitProxyStorage(false)
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	{
		FViewport::SetViewportClient(TStrongPtrVariant<FViewportClient>(nullptr));
	}
}
...
bool FViewport::TakeHighResScreenShot()
{
	...
	if (GScreenshotResolutionX > MaxTextureDimension || GScreenshotResolutionY > MaxTextureDimension)
	{
		...
		UE_LOGF(LogClient, Warning, "The specified multiplier for high resolution screenshot is too large for your system (requested size %ux%u, max size %ux%u)! Please try again with a smaller value.", GScreenshotResolutionX, GScreenshotResolutionY, MaxTextureDimension, MaxTextureDimension);
...
{
	GFrameCounter++;
	ENQUEUE_RENDER_COMMAND(BeginFrameCommand, LoopTick)(
...
{
	ENQUEUE_RENDER_COMMAND(EndFrameCommand, LoopTick)(
...
{
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	FViewportClient* const ViewportClientPtr = ViewportClient;
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	FEngineShowFlags* const EngineShowFlags = ViewportClientPtr->GetEngineShowFlags();
	if (!EngineShowFlags)
	{
		return;
	}
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	TUniquePtr<FDummyViewport> DummyViewport = FDummyViewport::Create(ViewportClientPtr);
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	DummyViewport->SetInitialSize(FIntPoint(
		(GScreenshotResolutionX > 0) ? GScreenshotResolutionX : SizeX,
		(GScreenshotResolutionY > 0) ? GScreenshotResolutionY : SizeY
	));

	DummyViewport->SetupHDR(GetDisplayColorGamut(), GetDisplayOutputFormat(), GetSceneHDREnabled());

	BeginInitResource(DummyViewport.Get());

	bool MaskShowFlagBackup = EngineShowFlags->HighResScreenshotMask;
	const auto MotionBlurShowFlagBackup = EngineShowFlags->MotionBlur;

	EngineShowFlags->SetHighResScreenshotMask(GetHighResScreenshotConfig().bMaskEnabled);
	...
	while (FrameDelay)
	{
		HighResScreenshotBeginFrame(DummyViewport.Get());
		if (FrameDelay == 1)
		{
			GIsHighResScreenshot = true;
		}
		UWorld* const World = ViewportClientPtr->GetWorld();
		FCanvas Canvas(DummyViewport.Get(), nullptr, World, World->GetFeatureLevel());
		{
			ViewportClientPtr->Draw(DummyViewport.Get(), &Canvas);
		}
		...
		HighResScreenshotEndFrame(DummyViewport.Get());
	...
	EngineShowFlags->SetHighResScreenshotMask(MaskShowFlagBackup);
	EngineShowFlags->MotionBlur = MotionBlurShowFlagBackup;
	bool bIsScreenshotSaved = ViewportClientPtr->ProcessScreenShots(DummyViewport.Get());
	...
	BeginReleaseResource(DummyViewport.Get());
	FlushRenderingCommands();
	DummyViewport.Reset();
	...
	if ((GIsEditor || !IsFullscreen()) && !GIsAutomationTesting && bIsScreenshotSaved)
	{
		...
		FSlateNotificationManager::Get().AddNotification(Info);
		UE_LOGF(LogClient, Log, "%ls %ls", *Message.ToString(), *HyperLinkText);
	}
...
{
	// @todo - MGPU AFR is no longer supported. RHIGetViewportNextPresentGPUIndex should be removed.
	// We cannot call that function here as we no longer have the FRHIViewport instance.
	return FRHIGPUMask::GPU0();//RHIGetViewportNextPresentGPUIndex(GetViewportRHI()));
}

void AddStreamingViewInfo(UWorld& World, const FSceneView& View)
{
	const float StreamingScale = 1.f / FMath::Clamp(View.LODDistanceFactor, 0.2f, 1.f);
	IStreamingManager::Get().AddViewInformation(
		View.ViewMatrices.GetViewOrigin(),
		View.UnscaledViewRect.Width(),
		View.UnscaledViewRect.Width() * View.ViewMatrices.GetViewToClip().M[0][0],
		StreamingScale, false, 0, nullptr, &World);

	World.ViewLocationsRenderedLastFrame.Add(View.ViewMatrices.GetViewOrigin());

	FWorldCachedViewInfo& Info = World.CachedViewInfoRenderedLastFrame.AddDefaulted_GetRef();
	Info.ViewMatrix = View.ViewMatrices.GetWorldToView();
	Info.ProjectionMatrix = View.ViewMatrices.GetViewToClip();
	Info.ViewProjectionMatrix = View.ViewMatrices.GetWorldToClip();
	Info.ViewToWorld = View.ViewMatrices.GetViewToWorld();
}
...
void FViewport::Draw( bool bShouldPresent /*= true */)
{
	...
	if (!(World && World->IsGameWorld() && !bIsGameRenderingEnabled))
	{
		...
		if( SizeX > 0 && SizeY > 0 )
		{
			...
			{
				bLockToVsync |= (Player->PlayerController->bCinematicMode);
			}
			...
			FCanvas Canvas(this, nullptr, World, World ? World->GetFeatureLevel() : GMaxRHIFeatureLevel, FCanvas::CDM_DeferDrawing, ViewportClientRef.ShouldDPIScaleSceneCanvas() ? ViewportClientRef.GetDPIScale() : 1.0f);
			Canvas.SetRenderTargetRect(FIntRect(0, 0, SizeX, SizeY));
			{
				ViewportClientRef.Draw(this, &Canvas);
			}
			Canvas.Flush_GameThread();
				
			UGameViewportClient::OnViewportRendered().Broadcast(this);

			ViewportClientRef.ProcessScreenShots(this);
...
{
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	FViewportClient* ViewportClientPtr = ViewportClient;
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	FScopedConditionalWorldSwitcher WorldSwitcher(ViewportClientPtr);
	...
	else if (!bHitProxiesCached)
	{
		...
		FCanvas Canvas(&HitProxyMap, &HitProxyMap, ViewportClientPtr->GetWorld(), GetFeatureLevel(), FCanvas::CDM_DeferDrawing,
			ViewportClientPtr->ShouldDPIScaleSceneCanvas() ? ViewportClientPtr->GetDPIScale() : 1.0f);
		{
			ViewportClientPtr->Draw(this, &Canvas);
		}
		Canvas.Flush_GameThread();
		...
		EnqueueEndRenderFrame(false, false);
...
{
	const FIntPoint VPSize = GetSizeXY();
	// if we haven't been initialized, we can't really hit anything.
	if (VPSize == FIntPoint::ZeroValue)
	{
		return nullptr;
	}
	...
	if(TestSizeX > 0 && TestSizeY > 0)
	{
		...
		bool bIsOrtho = GetClientChecked().IsOrtho();
...
void FViewport::SetViewportClient(const TStrongPtrVariant<FViewportClient>& InViewportClient)
{
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	FViewportClient* const OldClient = ViewportClient;
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	FViewportClient* const NewClient = InViewportClient.Get();
	...
	if (OldClient)
	{
		OldClient->RemoveAssociation(*this);
	}

	Client = InViewportClient;
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	ViewportClient = NewClient;
PRAGMA_ENABLE_DEPRECATION_WARNINGS

	if (NewClient)
	{
		bRequiresHitProxyStorage = NewClient->RequiresHitProxyStorage();
#if !WITH_EDITOR && !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
		if (bRequiresHitProxyStorage)
		{
			UE_LOGF(LogClient, Warning, "Consoles don't need hitproxy storage - wasting memory!?");
...
void FViewport::FHitProxyMap::Init(FRHICommandListBase& RHICmdList, uint32 NewSizeX,uint32 NewSizeY)
{
	SizeX = NewSizeX;
	SizeY = NewSizeY;
	...
	}
	{
		...
		HitProxyCPUTexture = RHICmdList.CreateTexture(Desc);
	}
}

void FViewport::FHitProxyMap::Init(uint32 NewSizeX, uint32 NewSizeY)
{
	Init(FRHICommandListImmediate::Get(), NewSizeX, NewSizeY);
}
...
FViewportClient& FViewport::GetClientChecked() const
{
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	FViewportClient* Result = ViewportClient;
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	check(Result);
	return *Result;
}
...
ENGINE_API bool GetHighResScreenShotInput(const TCHAR* Cmd, FOutputDevice& Ar, uint32& OutXRes, uint32& OutYRes, float& OutResMult, FIntRect& OutCaptureRegion, bool& OutShouldEnableMask, bool& OutDumpBufferVisualizationTargets, bool& OutCaptureHDR, FString& OutFilenameOverride, bool& OutUseDateTimeAsFileName)
{
	...
	const FString FilenameSearchString = "filename=";
...
TUniquePtr<FDummyViewport> FDummyViewport::Create(const TStrongPtrVariant<FViewportClient>& InClient)
{
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	TUniquePtr<FDummyViewport> Self = Create(InClient.Get());
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	Self->Client = InClient;
	return Self;
}

TUniquePtr<FDummyViewport> FDummyViewport::Create(FViewportClient* InClient)
{
	TUniquePtr<FDummyViewport> Self(new FDummyViewport());

	UWorld* World = InClient ? InClient->GetWorld() : nullptr;
	Self->DebugCanvas = new FCanvas(Self.Get(), nullptr, World, World ? World->GetFeatureLevel() : GMaxRHIFeatureLevel);
	Self->DebugCanvas->SetAllowedModes(0);

	// Skip invocation of Add/Remove Association for screenshots
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	Self->ViewportClient = InClient;
PRAGMA_ENABLE_DEPRECATION_WARNINGS
