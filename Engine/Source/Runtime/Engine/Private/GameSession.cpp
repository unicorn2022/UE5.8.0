...
void AGameSession::HandleMatchHasStarted()
{
	...
	if (STATS && !UE_BUILD_SHIPPING)
	{
		...
		{
			UE_LOGF(LogGameSession, Log, "Match has started - begin automatic stat capture");
...
{
	UE_LOGF(LogGameSession, Verbose, "OnStartSessionComplete %ls bSuccess: %d", *InSessionName.ToString(), bWasSuccessful);
}
...
void AGameSession::HandleMatchHasEnded()
{
	if (STATS && !UE_BUILD_SHIPPING)
	{
		if (FParse::Param(FCommandLine::Get(), TEXT("MatchAutoStatCapture")))
		{
			UE_LOGF(LogGameSession, Log, "Match has ended - end automatic stat capture");
...
{
	UE_LOGF(LogGameSession, Verbose, "OnEndSessionComplete %ls bSuccess: %d", *InSessionName.ToString(), bWasSuccessful);
}
...
void AGameSession::InitOptions( const FString& Options )
{
	...
	if (GameMode)
	{
		...
		{
			UE_LOGF(LogGameSession, Error, "Player State class is invalid for game mode: %ls!", *GameMode->GetName());
		}
	}
}
...
{
	UE_LOGF(LogGameSession, Warning, "Autologin attempt failed, unable to register server!");
}
...
FString AGameSession::ApproveLogin(const FString& Options)
{
	...
	{
		UE_LOGF(LogGameSession, Warning, "ApproveLogin: A maximum of %i splitscreen players are allowed", MaxSplitscreensPerConnection);
...
int32 AGameSession::GetNextPlayerID()
{
	...
	{
		UE_LOGF(LogGameSession, Warning, "AGameSession::GetNextPlayerID had to wrap the Player ID, this probably shouldn't have happened. PlayerID collisions may occur! Is this function being called incorrectly in a loop?");
...
{
	UE_LOGF(LogGameSession, Log, "  MaxPlayers: %i", MaxPlayers);
	UE_LOGF(LogGameSession, Log, "  MaxSpectators: %i", MaxSpectators);
