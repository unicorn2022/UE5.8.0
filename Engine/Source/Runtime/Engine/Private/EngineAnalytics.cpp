...
FString FEngineAnalytics::CreateAnalyticsUserId(const FString& EpicAccountId)
{
...
void FEngineAnalytics::AppendMachineStats(TArray<FAnalyticsEventAttribute>& EventAttributes)
{
	...
	EventAttributes.Emplace(TEXT("Is64BitOS"), true);
	...
	EventAttributes.Emplace(TEXT("UEBuildArch"), FString("AppleSilicon"));
#else
	EventAttributes.Emplace(TEXT("UEBuildArch"), FString("Intel(Mac)"));
#endif
#endif
}
...
void FEngineAnalytics::SendMachineInfoForAccount(const FString& EpicAccountId)
{
	...
	if (Analytics && !SessionEpicAccountIds.Contains(EpicAccountId))
	{
		...
		static const FString SZEventName = "SessionMachineStats";
