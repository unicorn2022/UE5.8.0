		...
		if (StaticMeshes.Num())
		{
			auto CancelOrMarkCanceled = [&StaticMeshes](TSet<TWeakObjectPtr<UStaticMesh>>& Set)
			{
				for (auto Iterator = Set.CreateIterator(); Iterator; ++Iterator)
				{
					...
					{
						UE_LOGF(LogStaticMesh, Verbose, "Canceling static mesh %ls async compilation as requested", *StaticMesh->GetName());
	...
	if (GetNumRemainingMeshes())
	{
		...
		auto CancelOrCollectUnreachable = [&PendingStaticMeshes](TSet<TWeakObjectPtr<UStaticMesh>>& Set)
		{
			for (auto Iterator = Set.CreateIterator(); Iterator; ++Iterator)
			{
				...
				{
					UE_LOGF(LogStaticMesh, Verbose, "Cancelling static mesh %ls async compilation because it's being garbage collected", *StaticMesh->GetName());
...
void FStaticMeshCompilingManager::PostCompilation(UStaticMesh* StaticMesh)
{
	...
	if (StaticMesh->AsyncTask && !IsEngineExitRequested())
	{
		...
		// The scope is important here to destroy the FStaticMeshAsyncBuildScope before broadcasting events
		{
			...
			UE_LOGF(LogStaticMesh, Verbose, "Refreshing static mesh %ls because it is ready", *StaticMesh->GetName());
							...
							(PlayInEditorMode == 0 || bHasRelevantCollision))
						{
										...
										if (FMath::SphereAABBIntersection(ActorBounds.Origin, ActorBounds.SphereRadius * ActorBounds.SphereRadius, ComponentBounds.GetBox()))
										{
											...
											if (!bIsAlreadyInSet)
											{
												UE_LOGF(
													LogStaticMesh,
													Display,
													"Waiting on static mesh %ls being ready because it affects collision/navigation and is near a player/bot",
							...
							else 
							{
								...
								if (!bIsAlreadyInSet)
								{
									...
									{
										UE_LOGF(LogStaticMesh, Display, "Waiting on static mesh %ls being ready before playing", *StaticMesh->GetFullName());
									}
									else
									{
										UE_LOGF(LogStaticMesh, Display, "Waiting on static mesh %ls being ready because it affects collision/navigation", *StaticMesh->GetFullName());
									}
								}
