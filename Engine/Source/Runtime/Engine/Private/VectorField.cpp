...
{
	UE_LOGF(LogVectorField, Fatal,"%ls must override InitInstance.", *(GetClass()->GetName()));
}
...
void UVectorFieldStatic::UpdateCPUData(bool bDiscardData)
{
	if (bAllowCPUAccess)
	{
		...
		if (!ensure(Ptr))
		{
			UE_LOGF(LogVectorField, Error, "Vector field data is not loaded.");
			return;
		}
		...
		if (bDiscardData && SourceData.IsBulkDataLoaded())
		{
			...
			UE_LOGF(LogVectorField, Warning, "SourceData.GetCopy() is supposed to unload the data after copying, but it is still loaded.");
		}
