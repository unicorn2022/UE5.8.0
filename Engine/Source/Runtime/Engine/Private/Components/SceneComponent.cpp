...
{
	static const FName DefaultSceneRootVariableName("DefaultSceneRoot");
	static const FName MobilityName("Mobility");
	static const FText MobilityWarnText = LOCTEXT("InvalidMove", "move");
	static const FName PhysicsVolumeTraceName("PhysicsVolumeTrace");
}
...
#if DO_ENSURE
	bool bCheckGarbageAttachments = false;
	static FAutoConsoleVariableRef CVarCheckGarbageAttachments(
		TEXT("s.CheckGarbageAttachments"),
		bCheckGarbageAttachments,
		TEXT("Whether or not to have ensures/assertions regarding attaching child components that are marked garbage to parents that aren't and vice versa"),
		ECVF_Default
	);
#endif
...
EDataValidationResult USceneComponent::IsDataValid(FDataValidationContext& Context) const
{
	...
	if (GValidateSceneComponentAttachmentEditorOnlySettings && IsEditorOnly())
	{
		...
		{
			if (	ChildSceneComponent 
				&& !ChildSceneComponent->IsEditorOnly() 
				&& !(ChildSceneComponent->GetOwner() && ChildSceneComponent->GetOwner()->IsEditorOnly()))
			{
...
void USceneComponent::AttachToParent()
{
	...
	{
#if DO_ENSURE
		ensureMsgf(!SceneComponentCVars::bCheckGarbageAttachments || IsValid(GetAttachParent()), TEXT("Trying to attach a scene component to a parent which is garbage"));
#endif // DO_ENSURE
			...
			while (ChildCount > 0)
			{
				...
				if (Child && Child->GetOwner() != MyOwner)
				{
					if (Child->GetAttachParent())
					{
						...
						else
						{
...
#endif
							{
								...
								UE_LOGF(LogSceneComponent, Error, "Component '%ls' has '%ls' in its AttachChildren array, however, '%ls' believes it is attached to '%ls'", *GetFullName(), *Child->GetFullName(), *Child->GetFullName(), *Child->GetAttachParent()->GetFullName());
							}
					...
					else 
					{
						...
						{
							UE_LOGF(LogSceneComponent, Error, "Component '%ls' has '%ls' in its AttachChildren array, however, '%ls' believes it is not attached to anything", *GetFullName(), *Child->GetFullName(), *Child->GetFullName());
						}
						AttachChildren.Pop(EAllowShrinking::No);
					}
...
#if WITH_EDITORONLY_DATA
							// If we are in the middle of a transaction it isn't entirely unexpected that an AttachParent/AttachChildren pairing is wrong
							if (!ensureAlwaysMsgf(GIsTransacting || !bRegistered, TEXT("Component '%s' has '%s' in its AttachChildren array, however, '%s' believes it is attached to '%s'"), *GetFullName(), *Child->GetFullName(), *Child->GetFullName(), *Child->GetAttachParent()->GetFullName()))
#endif
							{
								// We've gotten in to a bad state where the Child's AttachParent doesn't jive with the AttachChildren array
								// so instead of crashing, output an error and gracefully handle
...
void USceneComponent::SetRelativeLocationAndRotation(FVector NewLocation, const FQuat& NewRotation, bool bSweep, FHitResult* OutSweepHitResult, ETeleportType Teleport)
{
	...
	if (GEnsureOnNANDiagnostic)
	{
		...
		if (!bIsNormalized)
		{
			UE_LOGF(LogSceneComponent, Warning, "USceneComponent::SetRelativeLocationAndRotation has unnormalized NewRotation (%ls). %ls", *NewRotation.ToString(), *GetNameSafe(GetOwner()));
		}
	}
#else
...
void USceneComponent::SetRelativeScale3D(FVector NewScale3D)
{
	if (NewScale3D != GetRelativeScale3D())
	{
		...
		{
			UE_LOGF(LogBlueprint, Warning, "SetRelativeScale3D : Invalid Scale (%ls) set for '%ls'. Resetting to 1.f.", *NewScale3D.ToString(), *GetFullName());
...
USceneComponent* USceneComponent::GetChildComponent(int32 ChildIndex) const
{
	if (ChildIndex < 0)
	{
		UE_LOGF(LogBlueprint, Log, "SceneComponent::GetChild called with a negative ChildIndex: %d", ChildIndex);
		return nullptr;
	}
	...
	{
		UE_LOGF(LogBlueprint, Log, "SceneComponent::GetChild called with an out of range ChildIndex: %d; Number of children is %d.", ChildIndex, AttachedChildren.Num());
		return nullptr;
	}
...
bool USceneComponent::AttachToComponent(USceneComponent* Parent, const FAttachmentTransformRules& AttachmentRules, FName SocketName)
{
	...
	if(Parent != nullptr)
	{
...
#if DO_ENSURE
		if (SceneComponentCVars::bCheckGarbageAttachments)
		{
			bool ThisValid = IsValid(this);
			bool ParentValid = IsValid(Parent);
			// Allow attaching two garbage things to each other in case that's spammy with e.g. undo buffer stuff?
			ensureMsgf(ThisValid == ParentValid, TEXT("Attempting to create attachment between garbage and non-garbage objects. Parent: %s Attachment: %s ParentValid: %d, AttachmentValid: %d"),
				*Parent->GetPathName(), *GetPathName(), ParentValid, ThisValid);
		}
#endif // DO_ENSURE
		...
		{
			UE_LOGF(LogSceneComponent, Warning, "AttachTo: '%ls' will not allow '%ls' to be attached as a child.", *Parent->GetPathName(), *GetPathName());
			return false;
		}
		...
		if (bSameAttachParentAndSocket && !IsRegistered() && AttachmentRules.LocationRule == EAttachmentRule::KeepRelative && AttachmentRules.RotationRule == EAttachmentRule::KeepRelative && AttachmentRules.ScaleRule == EAttachmentRule::KeepRelative && LastAttachIndex == INDEX_NONE)
		{
			...
			//UE_LOGF(LogSceneComponent, Verbose, "[%ls] skipping DetachFromParent() for same pending parent [%ls] during registration.",
...
void USceneComponent::Serialize(FArchive& Ar)
{
	...
	if (Ar.IsMigratingRemoteObjects() && Ar.IsSaving() && !SceneComponentCVars::bAllowRemoteAttachedChildren)
	{
		for (int Index = AttachChildren.Num() - 1 ; Index >= 0 ; --Index)
		{
			...
			if (ensureMsgf(Child.GetResidence() != EResidence::Remote, TEXT("About to save a Remote Attached Child in %s"), *GetFullNameSafe(this)))
			{
				if (Child->GetOwner() != GetOwner())
				{
					UE_LOGF(LogSceneComponent, Warning, "Saving a Child %ls which is not within the same hierarchy as %ls (would become a RemoteReference - not sending it)", *Child->GetName(), *GetName());
	...
	if (Ar.IsMigratingRemoteObjects())
	{
		...
		if (Ar.IsLoading() && !SceneComponentCVars::bAllowRemoteAttachedChildren)
		{
			...
			{
				ensureMsgf(Child.GetResidence() != EResidence::Remote, TEXT("Loaded a Remote Attached Child in %s"), *GetFullNameSafe(this));
			}
		}
	}
#endif
...
#if UE_WITH_REMOTE_OBJECT_HANDLE
void USceneComponent::PostMigrate(const FUObjectMigrationContext& MigrationContext)
{
	Super::PostMigrate(MigrationContext);

	if (MigrationContext.MigrationSide == EObjectMigrationSide::Send)
	{
		// Clear references to attachments so they don't get cleared on the other objects when this instance is destroyed.
