...
void UProjectileMovementComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction *ThisTickFunction)
{
	...
	while (bSimulationEnabled && RemainingTime >= MIN_TICK_TIME && (Iterations < MaxSimulationIterations) && IsValid(ActorOwner) && !HasStoppedSimulation())
	{
		...
		// Logging
		UE_LOGF(LogProjectileMovement, Verbose, "Projectile %ls: (Role: %d, Iteration %d, step %.3f, [%.3f / %.3f] cur/total) sim (Pos %ls, Vel %ls)",
		...
		if( !Hit.bBlockingHit )
		{
			...
			// Logging
			UE_LOGF(LogProjectileMovement, VeryVerbose, "Projectile %ls: (Role: %d, Iteration %d, step %.3f) no hit (Pos %ls, Vel %ls)",
		...
		else
		{
			...
			// Logging
			UE_CLOGF(UpdatedComponent != nullptr, LogProjectileMovement, VeryVerbose, "Projectile %ls: (Role: %d, Iteration %d, step %.3f) new hit at t=%.3f: (Pos %ls, Vel %ls)",
			...
			// Logging
			UE_CLOGF(UpdatedComponent != nullptr, LogProjectileMovement, VeryVerbose, "Projectile %ls: (Role: %d, Iteration %d, step %.3f) deflect at t=%.3f: (Pos %ls, Vel %ls)",
			...
			if (SubTickTimeRemaining >= MIN_TICK_TIME)
			{
				...
				if (NumImpacts <= BounceAdditionalIterations)
				{
					...
					// Logging
					UE_LOGF(LogProjectileMovement, Verbose, "Projectile %ls: (Role: %d, Iteration %d, step %.3f) allowing extra iteration after bounce %u (t=%.3f, adding %.3f secs)",
...
UProjectileMovementComponent::EHandleBlockingHitResult UProjectileMovementComponent::HandleBlockingHit(const FHitResult& Hit, float TimeTick, const FVector& MoveDelta, float& SubTickTimeRemaining)
{
	...
	{
		UE_LOGF(LogProjectileMovement, Verbose, "Projectile %ls is stuck inside %ls.%ls with velocity %ls!", *GetNameSafe(ActorOwner), *Hit.HitObjectHandle.GetName(), *GetNameSafe(Hit.GetComponent()), *Velocity.ToString());
		return EHandleBlockingHitResult::Abort;
	}
...
bool UProjectileMovementComponent::CheckStillInWorld()
{
	...
	else if( UpdatedComponent && UpdatedComponent->IsRegistered() )
	{
		...
		{
			UE_LOGF(LogProjectileMovement, Warning, "%ls is outside the world bounds!", *ActorOwner->GetName());
		...
		else
		{
			...
			if (const UWorld* const World = GetWorld())
			{
				...
				if (World->DeltaTimeSeconds < 0.20f)
				{
					...
					{
						UE_LOGF(LogProjectileMovement, Warning, "GetSimulationTimeStep() - Max iterations %d hit while remaining time %.6f > MaxSimulationTimeStep (%.3f) for '%ls'", MaxSimulationIterations, RemainingTime, MaxSimulationTimeStep, *GetPathNameSafe(UpdatedComponent));
					}
				}
			}
#endif
	...
	if (!bInterpolationComplete)
	{
		if (bInterpMovement)
		{
			...
			if (USceneComponent* InterpComponent = GetInterpolatedComponent())
			{
				...
				{
					UE_LOGF(LogProjectileMovementInterpolation, Verbose, "--- Skip  Interpolation (%d frames : %ls)", ThrottleInterpolationFramesSinceInterp, *GetPathNameSafe(InterpComponent));
				...
				{
					ThrottleInterpolationFramesSinceInterp = 0;
					UE_LOGF(LogProjectileMovementInterpolation, Verbose, "+++ Apply Interpolation (%d frames : %ls)", ThrottleInterpolationFramesSinceInterp, *GetPathNameSafe(InterpComponent));
