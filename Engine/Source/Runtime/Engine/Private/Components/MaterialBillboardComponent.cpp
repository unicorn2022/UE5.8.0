		...
		if (WorstCaseVertexBufferSize > 0)
		{
			...
			for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
			{
				if (VisibilityMap & (1 << ViewIndex))
				{
					...
					const FVector CameraUp = -View->ViewMatrices.GetClipToWorld().TransformVector(FVector(1.0f, 0.0f, 0.0f));
					const FVector CameraRight = -View->ViewMatrices.GetClipToWorld().TransformVector(FVector(0.0f, 1.0f, 0.0f));
					const FMatrix WorldToLocal = GetLocalToWorld().InverseFast();
					const FMatrix ViewToLocal = View->ViewMatrices.GetViewToWorld() * WorldToLocal;
					...
					for (int32 ElementIndex = 0; ElementIndex < Elements.Num(); ++ElementIndex)
					{
						...
						if (Element.Material)
						{
							...
							const float W = View->ViewMatrices.GetWorldToClip().TransformPosition(SourcePosition).W;
