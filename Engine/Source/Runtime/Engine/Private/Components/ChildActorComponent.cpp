...
void UChildActorComponent::Serialize(FArchive& Ar)
{
	...
	// Since we sometimes serialize properties instead of using duplication and we can end up pointing at the wrong template
...
void UChildActorComponent::DeleteCachedInstanceData()
{
...
void UChildActorComponent::ApplyComponentInstanceData(FChildActorComponentInstanceData* ChildActorInstanceData, const ECacheApplyPhase CacheApplyPhase)
{
	...
	if (ChildActor)
	{
		   ...
		   ChildActor->GetFName() != ChildActorName)
		{
			...
			if (ChildActor->Rename(*ChildActorNameString, nullptr, REN_Test | REN_AllowPackageLinkerMismatch))
			{
				ChildActor->Rename(*ChildActorNameString, nullptr, REN_DoNotDirty | REN_AllowPackageLinkerMismatch);
#if WITH_EDITOR
...
void UChildActorComponent::SetChildActorClass(TSubclassOf<AActor> Class, AActor* ActorTemplate)
{
	...
	if (IsTemplate())
	{
		if (ChildActorClass)
		{
			if (ChildActorTemplate == nullptr || ActorTemplate || (ChildActorTemplate->GetClass() != ChildActorClass))
			{
				...
				ChildActorTemplate->Rename(*TemplateName, this, REN_DoNotDirty | REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
			}
		}
	...
	else
	{
		...
		{
			UE_LOGF(LogChildActorComponent, Warning, "Call to SetChildActorClass on '%ls' supplied ActorTemplate '%ls', but it will not be used due to the component not being registered.", *GetPathName(), *ActorTemplate->GetPathName());
		}
	}
}
	...
	if (Actor && (Actor->HasAuthority() || !IsChildActorReplicated()) && !IsLevelBeingRemoved())
	{
		...
		if (World != nullptr)
		{
			...
			if (!IsGarbageCollecting())
			{
				...
				Actor->Rename(*MakeUniqueObjectName(Actor->GetOuter(), ChildClass, *ObjectBaseName).ToString(), nullptr, REN_DoNotDirty | REN_AllowPackageLinkerMismatch);
			}
			else
			{
				ChildActorName = NAME_None;
...
void UChildActorComponent::CreateChildActor(TFunction<void(AActor*)> CustomizerFunc)
{
	...
	if (ChildActorClass != nullptr)
	{
		...
		if (World != nullptr)
		{
			...
			{
				UE_LOGF(LogChildActorComponent, Display, "Not creating child actor due to replay! Comp:%ls ChildActor:%ls", *GetPathName(), *GetPathNameSafe(ChildActor));
				return;
			}
			...
			while (Actor && bSpawn)
			{
				...
				{
					bSpawn = false;
					UE_LOGF(LogChildActorComponent, Error, "Found cycle in child actor component '%ls'.  Not spawning Actor of class '%ls' to break.", *GetPathName(), *ChildActorClass->GetName());
				}
			...
			if (bSpawn)
			{
				...
				// If necessary, the actor will eventually be saved to the transaction buffer,
				// but only after we've finished initializing it manually.
				...
				if (ChildActor != nullptr)
				{
					...
					// Since child actors are expected to be attached to parent actors, they must have a root component.
					if (!ChildRootComponent)
					{
						ChildRootComponent = NewObject<USceneComponent>(ChildActor, USceneComponent::GetDefaultSceneRootVariableName());
						ChildActor->SetRootComponent(ChildRootComponent);
						ChildRootComponent->RegisterComponent();
					}

					{
						TGuardValue<TEnumAsByte<EComponentMobility::Type>> MobilityGuard(ChildRootComponent->Mobility, Mobility);
						ChildRootComponent->AttachToComponent(this, FAttachmentTransformRules::SnapToTargetNotIncludingScale);
					}
					...
					// Must be called last: ensures all relevant state is saved to the transaction bufffer
					const bool bIsTransactional = HasAllFlags(RF_Transactional);
					if (bIsTransactional)
					{
						ChildActor->SetFlags(RF_Transactional);
						ChildRootComponent->SetFlags(RF_Transactional);
						ChildActor->Modify();
						ChildRootComponent->Modify();
					}
				}
			}
		}
...
void UChildActorComponent::DestroyChildActor()
{
	...
	// If we're in a replay scrub and this is a startup actor and the class exists, assume the reference will be restored later
	if (LocalWorld && LocalWorld->IsPlayingReplay() && LocalWorld->GetDemoNetDriver()->IsRestoringStartupActors() && MyOwner && MyOwner->IsNetStartupActor() && ChildActorClass)
	{
		UE_LOGF(LogChildActorComponent, Display, "Not creating destroying actor due to replay! Comp:%ls ChildActor:%ls", *GetPathName(), *GetPathNameSafe(ChildActor));
		return;
	}
	...
	if (ChildActor && (ChildActor->HasAuthority() || !IsChildActorReplicated()) && !IsLevelBeingRemoved())
	{
		...
		if (!GExitPurge)
		{
			...
			if (!bIsChildActorPendingKillOrUnreachable)
			{
				...
				if (!HasAnyFlags(RF_BeginDestroyed) && !IsUnreachable() && !IsBeingDestroyed() && !(MyOwner && MyOwner->IsActorBeingDestroyed()))
				{
					CachedInstanceData = new FChildActorComponentInstanceData(this);
				}
			}
