...
/** Static delegate broadcasted when the first active FGlobalComponentReregisterContext is created. */
FGlobalComponentReregisterContext::FOnGlobalComponentReregisterContextCreated FGlobalComponentReregisterContext::GlobalComponentReregisterContextCreatedEvent;

/** Static delegate broadcasted when the last active FGlobalComponentReregisterContext is destroyed. */
FGlobalComponentReregisterContext::FOnGlobalComponentReregisterContextDestroyed FGlobalComponentReregisterContext::GlobalComponentReregisterContextDestroyedEvent;
...
FGlobalComponentReregisterContext::FGlobalComponentReregisterContext()
{
	...
	// Only broadcast if this is the first active context
	if (ActiveGlobalReregisterContextCount == 1)
	{
		GlobalComponentReregisterContextCreatedEvent.Broadcast();
	}
...
FGlobalComponentReregisterContext::~FGlobalComponentReregisterContext()
{
	...
	if (--ActiveGlobalReregisterContextCount == 0)
	{
		...
		GlobalComponentReregisterContextDestroyedEvent.Broadcast();
	...
	if (FApp::CanEverRender())
	{
		...
		//if (--ActiveGlobalRecreateRenderStateContextCount == 0)
		{
			...
			if (GPrecachePSOsOnComponentRecreateRenderContext)
			{
				...
				UMaterial* PSOPrecacheFallbackMaterial = UPSOPrecacheSettingsManager::GetFallbackMaterial();
				if (PSOPrecacheFallbackMaterial)
				{
					UMaterialInterface::PrecacheFallbackMaterialPSOs(PSOPrecacheFallbackMaterial);
				}
			}
...
void UActorComponent::PostMigrate(const struct FUObjectMigrationContext& MigrationContext)
{
	...
	{
		const EResidence Residence = UE::RemoteObject::Handle::GetResidence(this);
		const EObjectMigrationSendType SendType = MigrationContext.GetObjectMigrationSendType(this);
		UE_LOGF(LogActorComponent, VeryVerbose, "%s: %ls %ls (Residence: %ls)", __func__, ToString(SendType), *GetName(), EnumToString(Residence));

		if (Residence != EResidence::Local)
		{
			ensureMsgf(Residence == EResidence::LocalNotReady, TEXT("Trying to send a non-local object"));
			return;
		}
		...
		// On the next receive, force components to re-register their replicated subobjects when the owning actor calls ResetOwnedComponents.
		...
		if (SendType == EObjectMigrationSendType::ReassignOwnership)
		{
			// It's not necessary to explicitly defer a call to StopReplicatingActorComponent(). It will automatically be
			// deferred by the deferred call to StopReplicatingActor() for the parent actor.
			//
			// NOTE: This should be re-enabled if components ever migrate separately to actors
			//
			//UE::Net::FDeferredReplicationSystemCalls::StopReplicatingActorComponentDefer(this, EEndReplicationFlags::Migrated);
		}

		// Prevent calling StopReplicatingActorComponent() in EndPlay(). 
		bCallStopReplicationInEndPlay = false;
	}
	else if (ensureMsgf(MigrationContext.MigrationSide == EObjectMigrationSide::Receive, TEXT("Invalid MigrationContext: Side was not Send or Receive")))
	{
		...
		UE_LOGF(LogActorComponent, VeryVerbose, "%s: %ls %ls", __func__, ToString(RecvType), *GetName());

		// It's not necessary to explicitly defer a call to StartReplicatingActorComponent(). It will automatically be
		// deferred by the deferred call to StartReplicatingActor() for the parent actor.
		//
		// NOTE: This should be re-enabled if components ever migrate separately to actors
		// 
		//if (RecvType == EObjectMigrationRecvType::AssignedOwnership)
		//{
		//	UE::Net::FDeferredReplicationSystemCalls::StartReplicatingActorComponentDefer(GetOwner(), this);
		//}
		...
		if (HasBegunPlay() && IsSupportedForNetworking() && (RecvType != EObjectMigrationRecvType::ReturnedLoan))
		{
		...
		{
			const FLoanedState LoanedState = RemoteObjects_LoanedStates.GetAndRemoveAnnotation(this);

			const AActor* Owner = GetOwner();
			if (ensure(Owner) && Owner->IsActorBeingDestroyed())
			{
				// If the Actor is being destroyed, then this Component would have been destroyed through
				// the Actor's Destroy call, so let's ignore the state changes
			}
			else if (NewState != LoanedState)
			{
				const bool bNewlyDestroyed = IsBeingDestroyed() && !LoanedState.bWasDestroyed;
				UE_CLOGF(bNewlyDestroyed, LogActorComponent, Warning, "%s: Destroying %ls back on owning server (it was destroyed while loaned-out)", __func__, *GetName());
		...
		if (RecvType == EObjectMigrationRecvType::AssignedOwnership)
		{
			...
			// We now own this object, so if we EndPlay, then actually stop replication.
			bCallStopReplicationInEndPlay = true;
		}
	}
}
#endif
...
bool UActorComponent::Rename( const TCHAR* InName, UObject* NewOuter, ERenameFlags Flags )
{
	...
	{
		UE_LOGF(LogActorComponent, Fatal, "%ls failed to route PostRename.  Please call Super::PostRename() in your <className>::PostRename() function. ", *GetFullName() );
	}
...
void UActorComponent::PostRename(UObject* OldOuter, const FName OldName)
{
	...
	if (OldOuter != GetOuter())
	{
		...
		if (OwnerPrivate != OldOwner)
		{
			...
			GetObjectsWithOuter(this, Children, EGetObjectsFlags::None);
			...
			for (int32 Index = 0; Index < Children.Num(); ++Index)
			{
				...
				if (!Child->IsA<AActor>())
				{
					...
					GetObjectsWithOuter(Child, Children, EGetObjectsFlags::None);
				}
			}
		}
	}
...
FPhysScene* UActorComponent::GetPhysicsScene() const
{
	return WorldPrivate ? WorldPrivate->GetPhysicsScene() : nullptr;
}
...
{
	return bPhysicsStateCreated && WorldPrivate && GetPhysicsScene() && AllowsAsyncPhysicsStateDestruction();
}
...
{
#if	UE_WITH_REMOTE_OBJECT_HANDLE
	return (Target.GetResidence() != EResidence::Remote ? Target->GetFullName() : Target.GetRemoteId().ToString()) + TEXT("[TickComponent]");
#else
...
{
	if (FPhysScene_Chaos* Scene = static_cast<FPhysScene_Chaos*>(GetPhysicsScene()))
	{
		if (bRegister)
		{
...
void UActorComponent::PreRegisterComponentWithWorld(UWorld* InWorld)
{
	...
	if (!IsValidChecked(this))
	{
		UE_LOGF(LogActorComponent, Log, "Trying to pre-register component with IsValid() == false. Aborting.");
		return;
	}

	if (IsPreRegistering())
	{
		UE_LOGF(LogActorComponent, Log, "Component already pre-registering. Aborting.");
		return;
	}
	...
	if (IsPreRegistered())
	{
		UE_LOGF(LogActorComponent, Log, "Component already pre-registered. Aborting.");
		return;
	}
	...
	{
		UE_LOGF(LogActorComponent, Log, "Component owner belongs to a DEADCLASS");
		return;
	}
	...
	if (MyOwner)
	{
		...
		UE_CLOGF(InWorld != MyOwner->GetWorld(), LogActorComponent, Log, "Specifying a world, but an Owner Actor found, and InWorld is not GetOwner()->GetWorld()");
	}
...
void UActorComponent::RegisterComponentWithWorld(UWorld* InWorld, FRegisterComponentContext* Context)
{
	...
	{
		UE_LOGF(LogActorComponent, Error, "RegisterComponentWithWorld: (%ls) Trying to register component in invalid or cleaned up world. Aborting.", *GetPathName());
		return;
	}
	...
	if (MyOwner && MyOwner->GetClass()->HasAnyClassFlags(CLASS_NewerVersionExists))
	{
		UE_LOGF(LogActorComponent, Log, "RegisterComponentWithWorld: Owner belongs to a DEADCLASS %ls", *GetPathNameSafe(MyOwner->GetClass()));
		return;
	}
	...
	if(MyOwner)
	{
		...
		{
			// The only time you should specify a scene that is not Owner->GetWorld() is when you don't have an Actor
			UE_LOGF(LogActorComponent, Log, "RegisterComponentWithWorld: (%ls) Specifying a world, but an Owner Actor found, and InWorld is not GetOwner()->GetWorld()", *GetPathName());
		}
	}
#endif // !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
...
#if UE_WITH_REMOTE_OBJECT_HANDLE
	if (UE::RemoteObject::Handle::GetResidence(this) == EResidence::LocalNotReady)
	{
		UE_LOGF(LogActorComponent, Log, "UnregisterComponent: (%ls) has LocalNotReady residence. Skipping unregistration.", *GetPathName());
		return;
	}
#endif
...
void UActorComponent::K2_DestroyComponent(UObject* Object)
{
	...
	else
	{
		...
		UE_LOGF(LogActorComponent, Error, "May not destroy component %ls owned by %ls.", *GetFullName(), *MyOwner->GetFullName());
	}
}
...
#if LOG_RENDER_STATE
	UE_LOGF(LogActorComponent, Log, "SendRenderTransform_Concurrent: %ls", *GetPathName());
#endif
}
...
#if LOG_RENDER_STATE
	UE_LOGF(LogActorComponent, Log, "SendRenderDynamicData_Concurrent: %ls", *GetPathName());
#endif
}
...
#if LOG_RENDER_STATE
	UE_LOGF(LogActorComponent, Log, "SendRenderInstanceData_Concurrent: %ls", *GetPathName());
#endif
}
...
#if LOG_RENDER_STATE
	UE_LOGF(LogActorComponent, Log, "DestroyRenderState_Concurrent: %ls", *GetPathName());
#endif
...
void UActorComponent::OnAsyncDestroyPhysicsStateBegin_GameThread(TSet<UObject*>& OutRootedObjects)
{
...
void UActorComponent::DestroyPhysicsState()
{
	...
	if (IsAsyncCreatePhysicsStateRunning())
	{
		...
		// RemoveAsyncPhysicsStateJobs invokes OnPostExecute_GameThread on the cancelled Create job,
		// which routes to OnAsyncCreatePhysicsStateEnd_GameThread and performs the full state cleanup:
		// it clears the PhysicsStateAsyncCreation* members, transitions the component to PreRegistered
		// via OnPreRegistered (which removes it from ULevel::PreRegisteringActorComponents), and sets
		// bCallAsyncPhysicsStateCreatedDelegate to request a deferred Create delegate broadcast.
		PhysicsStateAsyncCreationScene->RemoveAsyncPhysicsStateJobs(this);

		// Suppress the deferred Create broadcast: the bPhysicsStateCreated branch below is about
		// to tear the body down, so listeners would just see a paired Create/Destroy with no
		// observable lifetime in between. Clearing the flag also prevents a later sync
		// CreatePhysicsState from firing a spurious broadcast via its bCallAsyncPhysicsStateCreatedDelegate branch.
		bCallAsyncPhysicsStateCreatedDelegate = false;
		// Reset other members used by asynchronous physics state creation to be sure we are in the right state
		PhysicsStateAsyncCreationState = EPhysicsStateAsyncCreationState::None;
		PhysicsStateAsyncCreationScene = nullptr;
	}
...
void UActorComponent::ReregisterComponent()
{
	if (AllowReregistration())
	{
		if (!IsRegistered())
		{
			UE_LOGF(LogActorComponent, Log, "ReregisterComponent: (%ls) Not currently registered. Aborting.", *GetPathName());
			return;
		}
...
void UActorComponent::DoDeferredRenderUpdates_Concurrent()
{
	...
	if(!IsRegistered())
	{
		UE_LOGF(LogActorComponent, Log, "UpdateComponent: (%ls) Not registered, Aborting.", *GetPathName());
		return;
	}
...
void UActorComponent::SetAutoActivate(bool bNewAutoActivate)
{
	...
	{
		UE_LOGF(LogActorComponent, Warning, "SetAutoActivate called on component %ls after construction!", *GetFullName());
	}
}
...
void UActorComponent::SetIsReplicated(bool bShouldReplicate)
{
	if (GetIsReplicated() != bShouldReplicate)
	{
		...
		{
			UE_LOGF(LogActorComponent, Error, "Calling SetIsReplicated on component of Class '%ls' which cannot replicate.", *GetClass()->GetName());
		}
	}
}
...
void UActorComponent::RegisterReplicationFragments(UE::Net::FFragmentRegistrationContext& Context, UE::Net::EFragmentRegistrationFlags RegistrationFlags)
{
	if (CreationMethod == EComponentCreationMethod::UserConstructionScript)
	{
		if (!IsNameStableForNetworking() && GetArchetype() != GetClass()->GetDefaultObject())
		{
			...
			UE_LOGF(LogIris, Warning, "The default state of replicated dynamic component %ls::%ls will be built using the class CDO instead of the archetype. The non-replicated properties of the component on clients may be initialized wrong.", *GetNameSafe(GetOwner()), *GetName());
		}
	}
...
void UActorComponent::DetermineUCSModifiedProperties()
{
	if (CreationMethod == EComponentCreationMethod::SimpleConstructionScript)
	{
		...
		public:
			...
			{
				static const FName MD_SkipUCSModifiedProperties("SkipUCSModifiedProperties");
...
void UActorComponent::Serialize(FArchive& Ar)
{
	...
	if (Ar.IsMigratingRemoteObjects())
	{
		...
		if (Ar.IsSaving())
		{
			...
			ComponentFlags |= (bPhysicsStateCreated & 1) 		<< 4;
			ComponentFlags |= (IsComponentTickEnabled() & 1)	<< 5;
			// bIsReadyForReplication is intentionally not serialized here because it's reset in UActorComponent::PostMigrate when receiving.
		}
		...
		if (Ar.IsLoading())
		{
			...
			bPhysicsStateCreated 		= (ComponentFlags >> 4) & 1;
			bComponentTickEnabled 		= (ComponentFlags >> 5) & 1;
		}
	}
