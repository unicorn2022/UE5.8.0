...
// The project setting for the sky light component to affect the height fog in scattering (disable runtime and shader code)
static TAutoConsoleVariable<int32> CVarSupportsSkyLightCaptureAffectsHeightFogInScattering(
	TEXT("r.SupportsSkyLightCaptureAffectsHeightFogInScattering"),
	0,
	TEXT("Enables the inscattering of the sky light capture into the distant height fog."),
...
void USkyLightComponent::SanitizeCubemapSize()
{
	...
	const int32 MaxCubemapResolution = GSkylightCubemapMaxResolution > 0 ? FMath::Clamp(uint32(GSkylightCubemapMaxResolution), uint32(MinCubemapResolution), GetMaxCubeTextureDimension()) : GetMaxCubeTextureDimension();
...
bool USkyLightComponent::HasSkyCapturesToUpdate()
{
	if (GUpdateSkylightsEveryFrame || !SkyCapturesToUpdateBlendDestinations.IsEmpty())
	{
		return true;
	}
	UE::TScopeLock Lock(SkyCapturesToUpdateLock);
	return !SkyCapturesToUpdate.IsEmpty();
}
