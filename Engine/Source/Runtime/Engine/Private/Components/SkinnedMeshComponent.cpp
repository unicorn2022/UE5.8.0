...
namespace UE::Anim
{
	bool bUseSafeMeshPoseIndices = false;
	FAutoConsoleVariableRef CVarUseSafeMeshPoseIndices(TEXT("a.UseSafeMeshPoseIndices"), bUseSafeMeshPoseIndices
		, TEXT("Whether to compute and use valid mesh pose indices on skeletal mesh components. This allows us to safely reset missing bones in leader/follower setups. This has a small additional cost.")
		, ECVF_Default);
}
	...
	, AnimUpdateRateParams(nullptr)
{
	...
	bValidMeshPoseTransformsUpToDate = false;
...
FPrimitiveSceneProxy* USkinnedMeshComponent::CreateSceneProxy()
{
	...
	{
		UE_LOGF(LogSkinnedMeshComp, Verbose, "Skipping CreateSceneProxy for USkinnedMeshComponent %ls (USkinnedMeshComponent PSOs are still compiling)", *GetFullName());
		return nullptr;
	}
	...
	if (SkelMeshRenderData &&
		...
		MeshObject)
	{
		if (MeshObject->IsNaniteMesh() && ShouldNaniteSkin())
		{
...
void USkinnedMeshComponent::Serialize(FArchive& Ar)
{
	...
	{
		// TODO: Optimize by re-initializing anims (AllocateTransformData + InitAnim in SkeletalMeshComponent::PostMigrate) instead of serializing.
		// Blocked by UE-363679 (server hang in PostMigrate).
		Ar << ComponentSpaceTransformsArray[0];
		Ar << ComponentSpaceTransformsArray[1];
	}
...
void USkinnedMeshComponent::PostApplyToComponent()
{
	NotifyIfSkinnedAssetChanged();
...
void USkinnedMeshComponent::PostReinitProperties()
{
	NotifyIfSkinnedAssetChanged();
...
void USkinnedMeshComponent::PostInitProperties()
{
	NotifyIfSkinnedAssetChanged();
...
#if UE_WITH_REMOTE_OBJECT_HANDLE
void USkinnedMeshComponent::PostMigrate(const struct FUObjectMigrationContext& MigrationContext)
{
	Super::PostMigrate(MigrationContext);

	if (MigrationContext.MigrationSide == EObjectMigrationSide::Send)
	{
		// When migrating-out, remove us from the local non-serialized cache
	...
	else if (ensure(MigrationContext.MigrationSide == EObjectMigrationSide::Receive))
	{
		// When migrating-in, reconstruct this and add it to the local non-serialized cache
...
void USkinnedMeshComponent::CollectPSOPrecacheData(const FPSOPrecacheParams& BasePrecachePSOParams, FMaterialInterfacePSOPrecacheParamsList& OutParams)
{
	...
	FSkeletalMeshRenderData* RenderData = GetSkinnedAsset()->GetResourceForRendering();

	// Ensure LOD render data has been serialized/streamed in by now. If data is not ready, it is possible
	// the data is being loaded in from a different thread causing potential data races. 
	if (!RenderData->IsLODDataReady())
	{
		return;
	}
	...
	bool bAnySectionCastsShadows = RenderData->AnyRenderSectionCastsShadows(MinLODIndex);
	...
	PrecachePSOParams.bRequireRTDynamicGeometryUpdate = PrecachePSOParams.bAnyMaterialHasWorldPositionOffset;
	...
	bool bHaveNullEntry = true;
	FMaterialOverlayHelper::AppendAllOverlayMaterial(AssetAndComponentMaterialSlotsOverlayMaterial, AllOverlayMaterials, bHaveNullEntry);

	// always add default overlay material if available
	if (GetOverlayMaterial())
	{
...
void USkinnedMeshComponent::InitializeBeforeRegister()
{
	NotifyIfSkinnedAssetChanged();
...
void USkinnedMeshComponent::InitializeOnRegister()
{
...
void USkinnedMeshComponent::OnPreRegister()
{
	// For more information about the logic in this method, see USkinnedMeshComponent::OnRegister comments.
	TRACE_CPUPROFILER_EVENT_SCOPE(USkinnedMeshComponent::OnPreRegister);

	// Super schedules an async physics job and TransformData must be allocated before that job starts running
	InitializeBeforeRegister();
	
	Super::OnPreRegister();

	InitializeOnRegister();
}
...
void USkinnedMeshComponent::CreateRenderState_Concurrent(FRegisterComponentContext* Context)
{
	...
	if( GetSkinnedAsset() )
	{
		...
		UE_TRACE_METADATA_SCOPE_ASSET_FNAME(NAME_None, UWorld::StaticClass()->GetFName(), GetSkinnedAsset()->GetPackage()->GetFName());
		...
		if ( FApp::CanEverRender() && ShouldComponentAddToScene() )
		{
			...
			if( FSkeletalMeshRenderData* SkelMeshRenderData = GetSkinnedAsset()->GetResourceForRendering() )
			{
				...
				for (int LODIndex = MinLODIndex; LODIndex < SkelMeshRenderData->LODRenderData.Num(); LODIndex++)
				{
					...
					{
						UE_LOGF(LogSkinnedMeshComp, Warning, "Invalid Lod %i for Rendering Asset: %ls", LODIndex, *GetSkinnedAsset()->GetFullName());
					}
				}
			#endif
	...
	if (GetSkinnedAsset())
	{
		...
		if(MeshObject)
		{
			...
			UseLOD = FMath::Clamp(GetPredictedLODLevel(), MinLodIndex, MaxLODIndex);
...
void USkinnedMeshComponent::UpdateSkinWeightProfileReadbackIfNeeded(int32 LODIndex)
{
	QUICK_SCOPE_CYCLE_COUNTER(STAT_USkinnedMeshComponent_UpdateSkinWeightProfileReadbackIfNeeded);

	if (bSkinWeightProfilePending && IsSkinnedMeshSkinWeightProfilesOnRenderThread())
	{
		FSkeletalMeshRenderData* SkelMeshRenderData = GetSkeletalMeshRenderData();
		if (!SkelMeshRenderData)
		{
		...
		if (!LODRenderData.IsDataReady())
		{
			// Data is actively being serialized in a different thread,
			// skip this time.
			return;
		}

		USkinnedAsset* Asset = GetSkinnedAsset();
		if (Asset != nullptr && Asset->HasPendingInitOrStreaming())
		{
			return;
		}

		FSkinWeightProfilesData& SkinWeightProfilesData = LODRenderData.SkinWeightProfilesData;
		const FSkinWeightProfileStack ProfileStack{ CurrentSkinWeightProfileLayers };

		// If a real GPU readback is going to fire, register stream-out protection so the
		// underlying VB resource isn't replaced out from under the captured BaseBuffer*
		// before the RT lambda runs EnqueueCopy.
		if (IStreamingManager::Get_Concurrent() && IStreamingManager::Get().IsRenderAssetStreamingEnabled(EStreamableRenderAssetType::SkeletalMesh) && SkinWeightProfilesData.ShouldEnqueueGPUReadback())
		{
			IRenderAssetStreamingManager& RAMgr = IStreamingManager::Get().GetRenderAssetStreamingManager();
			if (!RAMgr.TryRegisterStreamOutProtectedAsset(Asset))
			{
				// Pending streaming op in flight; defer to next call.
				// bSkinWeightProfilePending stays true so we retry next frame.
				return;
			}

			// Hand release responsibility to FSkinWeightProfileManager's polling.
			if (FSkinWeightProfileManager* SWPMgr = FSkinWeightProfileManager::Get(GetWorld()))
			{
				SWPMgr->RegisterStreamOutProtection_AsyncSafe(Asset);
			}
		}

		if(SkinWeightProfilesData.DoReadbackIfNeeded(ProfileStack))
		{
			bSkinWeightProfilePending = false;
			bSkinWeightProfileBufferOverridePending = true;
		}
	}
}
...
void USkinnedMeshComponent::SendRenderDynamicData_Concurrent()
{
	...
	if( MeshObject && GetSkinnedAsset() && (bForceMeshObjectUpdate || (bRecentlyRendered || VisibilityBasedAnimTickOption == EVisibilityBasedAnimTickOption::AlwaysTickPoseAndRefreshBones || GIsEditor || MeshObject->bHasBeenUpdatedAtLeastOnce == false)) )
	{
		...
		UpdateSkinWeightProfileReadbackIfNeeded(UseLOD);
...
void USkinnedMeshComponent::PostEditUndo()
{
	NotifyIfSkinnedAssetChanged();

	Super::PostEditUndo();

	// Post undo, make sure that any socket attachments are up to date.
	UpdateCachedSocketAttachmentFlag();
}

void USkinnedMeshComponent::PostDuplicate(bool bDuplicateForPIE)
{
	NotifyIfSkinnedAssetChanged();
...
void USkinnedMeshComponent::PostEditImport()
{
	NotifyIfSkinnedAssetChanged();
...
void USkinnedMeshComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction *ThisTickFunction)
{
	...
	if(!PrimaryComponentTick.bRunOnAnyThread)
	{
		...
		if (IsSkinnedMeshSkinWeightProfilesOnRenderThread() && bSkinWeightProfileBufferOverridePending)
		{
			// Update the skin weight override buffer once GPU Readback of weight data is done. 
			// As it cannot be updated during send dynamic render data, we override the buffer
			// here. 
			UpdateSkinWeightOverrideBuffer();
			bSkinWeightProfileBufferOverridePending = false;
...
void USkinnedMeshComponent::SetCPUSkinningEnabled(bool bEnable, bool bRecreateRenderStateImmediately)
{
	...
	{
		UE_LOGF(LogSkinnedMeshComp, Warning, "It is expensive to enable CPU skinning with LOD streaming on.");
...
void USkinnedMeshComponent::OnRenderAssetFirstLodChange(const UStreamableRenderAsset* RenderAsset, int32 FirstLodIndex)
{
	if (!IsComponentTickEnabled())
	{
		check(RenderAsset == GetSkinnedAsset());

		if (GetPredictedLODLevel() < FirstLodIndex)
		{
			SetPredictedLODLevel(FirstLodIndex);
			bForceMeshObjectUpdate = true;
			MarkRenderDynamicDataDirty();
		}
	}
...
void USkinnedMeshComponent::RebuildVisibilityArray()
{
	...
	if ( ShouldUpdateBoneVisibility())
	{
		...
		{
			UE_LOGF(LogSkinnedMeshComp, Warning, "RebuildVisibilityArray() failed because EditableBoneVisibilityStates size: %d not equal to RefSkeleton bone count: %d.", EditableBoneVisibilityStates.Num(), GetSkinnedAsset()->GetRefSkeleton().GetNum());
			return;
		}
...
{
	// UpdateBounds is currently the closest place we can get from the SkinnedAsset property
...
{
#if WITH_EDITOR
	// We cannot read the asset's bounds while it is compiling. Reaching into USkinnedAsset::GetBounds()
	// during a LockPropertiesUntil() can self-deadlock the GT against its own parked async-build worker
	// (e.g. UGroomComponent::CalcBounds running from SendAllEndOfFrameUpdates while a reimport holds the lock).
	// Match the pattern already enforced by USkinnedMeshComponent::UpdateBounds and SendRenderDynamicData_Concurrent.
	if (IsCompiling())
	{
	...
	{
#if WITH_EDITOR
		if (LeaderPoseComponentInst->IsCompiling())
		{
			OutBounds = FBoxSphereBounds(ForceInitToZero);
			return;
		}
...
bool USkinnedMeshComponent::AreBoneTransformsValid() const
{
	return IsRegistered();
}
...
FMatrix USkinnedMeshComponent::GetBoneMatrix(int32 BoneIdx) const
{
	...
	{
		if(BoneIdx < GetLeaderBoneMap().Num())
		{
			int32 ParentBoneIndex = GetLeaderBoneMap()[BoneIdx];
			...
			{
				UE_LOGF(LogSkinnedMeshComp, Verbose, "GetBoneMatrix : ParentBoneIndex(%d:%ls) out of range of LeaderPoseComponent->SpaceBases for %ls(%ls)", BoneIdx, *GetNameSafe(LeaderPoseComponentInst->GetSkinnedAsset()), *GetNameSafe(GetSkinnedAsset()), *GetPathName());
				return FMatrix::Identity;
			}
		}
		else
		{
			UE_LOGF(LogSkinnedMeshComp, Warning, "GetBoneMatrix : BoneIndex(%d) out of range of LeaderBoneMap for %ls (%ls)", BoneIdx, *this->GetFName().ToString(), GetSkinnedAsset() ? *GetSkinnedAsset()->GetFName().ToString() : TEXT("NULL") );
			return FMatrix::Identity;
		}
	}
	else
	{
		...
		{
			UE_LOGF(LogSkinnedMeshComp, Warning, "GetBoneMatrix : BoneIndex(%d) out of range of SpaceBases for %ls (%ls)", BoneIdx, *GetPathName(), GetSkinnedAsset() ? *GetSkinnedAsset()->GetFullName() : TEXT("NULL") );
			return FMatrix::Identity;
		}
	}
}
...
FTransform USkinnedMeshComponent::GetBoneTransform(int32 BoneIdx, const FTransform& LocalToWorld) const
{
	...
	if(LeaderPoseComponentInst)
	{
		if (!LeaderPoseComponentInst->AreBoneTransformsValid())
		{
			// We aren't going to get anything valid from the leader pose if it
			// isn't valid so for now return identity
			return FTransform::Identity;
		}
		if(BoneIdx >= 0 && BoneIdx < GetLeaderBoneMap().Num())
		{
			const int32 LeaderBoneIndex = GetLeaderBoneMap()[BoneIdx];
			...
			else
			{
				...
				UE_LOGF(LogSkinnedMeshComp, Verbose, "GetBoneTransform : ParentBoneIndex(%d) out of range of LeaderPoseComponent->SpaceBases for %ls", BoneIdx, *this->GetFName().ToString() );
				return FTransform::Identity;
			}
		}
		else
		{
			UE_LOGF(LogSkinnedMeshComp, Warning, "GetBoneTransform : BoneIndex(%d) out of range of LeaderBoneMap for %ls", BoneIdx, *this->GetFName().ToString() );
			return FTransform::Identity;
		}
	}
	else
	{
		...
		{
			UE_LOGF(LogSkinnedMeshComp, Verbose, "GetBoneTransform : BoneIndex(%d) out of range of SpaceBases for %ls (%ls)", BoneIdx, *GetPathName(), GetSkinnedAsset() ? *GetSkinnedAsset()->GetFullName() : TEXT("NULL") );
			return FTransform::Identity;
		}
	}
}
...
{
	ERHIFeatureLevel::Type FeatureLevel = GetScene() ? GetScene()->GetFeatureLevel() : GMaxRHIFeatureLevel;
	return IsSkinCacheAllowed(LodIdx, FeatureLevel);
}

bool USkinnedMeshComponent::IsSkinCacheAllowed(int32 LodIdx, ERHIFeatureLevel::Type FeatureLevel) const
{
	if (!FGPUSkinCache::IsGPUSkinCacheEnabled(FeatureLevel))
	{
		return false;
	}
	...
	if (FSkeletalMeshRenderData* SkelMeshRenderData = GetSkinnedAsset()->GetResourceForRendering())
	{
		if (SkelMeshRenderData->bSupportRayTracing && FGPUSkinCache::IsGPUSkinCacheRayTracingSupported(FeatureLevel) && IsRayTracingEnabled())
		{
			return true;
		}
	}
...
bool USkinnedMeshComponent::BoneIsChildOf(FName BoneName, FName ParentBoneName) const
{
	...
	if(GetSkinnedAsset())
	{
		...
		if(BoneIndex == INDEX_NONE)
		{
			UE_LOGF(LogSkinnedMeshComp, Log, "execBoneIsChildOf: BoneName '%ls' not found in SkeletalMesh '%ls'", *BoneName.ToString(), *GetSkinnedAsset()->GetName());
			return bResult;
		}
		...
		if(ParentBoneIndex == INDEX_NONE)
		{
			UE_LOGF(LogSkinnedMeshComp, Log, "execBoneIsChildOf: ParentBoneName '%ls' not found in SkeletalMesh '%ls'", *ParentBoneName.ToString(), *GetSkinnedAsset()->GetName());
			return bResult;
		}
...
void USkinnedMeshComponent::SetSkinnedAssetAndUpdate(USkinnedAsset* InSkinnedAsset, bool /*bReinitPose*/)
{
...

	{
		...
		if (IsRegistered() || IsPreRegistering())
		{
			AllocateTransformData();
			UpdateLeaderBoneMap();
			InvalidateCachedBounds();
...
{
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	USkinnedAsset* CurrentSkinnedAsset = !SkeletalMesh ? SkinnedAsset : SkeletalMesh;	// Return the SkeletalMesh pointer for backward compatibility until it is retired
#if WITH_EDITOR
	...
	if (KnownSkinnedAsset != CurrentSkinnedAsset)
	{
		OutdatedKnownSkinnedAssetDetected();
	}
#endif
	return CurrentSkinnedAsset;
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
}
...
FMeshDeformerSet USkinnedMeshComponent::GetActiveMeshDeformers() const
{
	...
	if (!bIsDeformerRequestedByUser)
	{
		...
		else if (bIsDeformerRequiredForUBI)
		{
			...
			{
				if (LODRenderData.IsDataReady() && 
					LODRenderData.SkinWeightVertexBuffer.GetBoneInfluenceType() == UnlimitedBoneInfluence)
				{
					bMeshUsesUBI = true;
					break;
				}
		...
		if (bShouldStillUseMeshDeformer)
		{
			...
			if (MeshDeformerProvider)
			{
				...
				if (!ActiveDeformer)
				{
					UE_LOGF(LogSkinnedMeshComp, Error, "Failed to fetch default mesh deformer for a mesh that requires it: %ls. "
						"Ensure DefaultDeformer is set to a valid Deformer Graph asset in the project settings", *GetSkinnedAsset()->GetPathName());
				}
			}
		}
	}
...
UMeshDeformerInstanceSettings* USkinnedMeshComponent::GetMeshDeformerInstanceSettings() const
{
	return MeshDeformerInstanceSettings;
}
...
void USkinnedMeshComponent::SetLeaderPoseComponent(class USkinnedMeshComponent* NewLeaderBoneComponent, bool bForceUpdate, bool bInFollowerShouldTickPose)
{
	...
	if (ValidNewLeaderPose)
	{
		...
		if (ValidNewLeaderPose)
		{
			...
			UE_CLOGF(ValidNewLeaderPose == NewLeaderBoneComponent, LogSkinnedMeshComp, Verbose,
				"LeaderPoseComponent chain is detected (%ls). We re-route to top-most LeaderPoseComponent (%ls)",
	...
	if (ValidNewLeaderPose)
	{
		...
		{
			UE_LOGF(LogSkinnedMeshComp, Verbose,
				"LeaderPoseComponent chain is detected (%ls). We re-route all children to new LeaderPoseComponent (%ls)",
...
class USkeletalMeshSocket const* USkinnedMeshComponent::GetSocketInfoByName(FName InSocketName, FTransform& OutTransform, int32& OutBoneIndex) const
{
	...
	else
	{
		if (OverrideSocket)
		{
			UE_LOGF(LogSkinnedMeshComp, Warning, "GetSocketInfoByName(%ls -> override To %ls): No SkeletalMesh for Component(%ls) Actor(%ls)",
				*InSocketName.ToString(), *OverrideSocketName.ToString(), *GetName(), *GetFullNameSafe(GetOuter()));
		}
		else
		{
			UE_LOGF(LogSkinnedMeshComp, Warning, "GetSocketInfoByName(%ls): No SkeletalMesh for Component(%ls) Actor(%ls)",
...
class USkeletalMeshSocket const* USkinnedMeshComponent::GetSocketByName(FName InSocketName) const
{
	...
	else
	{
		if (OverrideSocket)
		{
			UE_LOGF(LogSkinnedMeshComp, Warning, "GetSocketByName(%ls -> override To %ls): No SkeletalMesh for Component(%ls) Actor(%ls)",
				*InSocketName.ToString(), *OverrideSocketName.ToString(), *GetName(), *GetFullNameSafe(GetOuter()));
		}
		else
		{
			UE_LOGF(LogSkinnedMeshComp, Warning, "GetSocketByName(%ls): No SkeletalMesh for Component(%ls) Actor(%ls)",
				*OverrideSocketName.ToString(), *GetName(), *GetFullNameSafe(GetOuter()));
		}
	}
...
void USkinnedMeshComponent::AddSocketOverride(FName SourceSocketName, FName OverrideSocketName, bool bWarnHasOverrided)
{
	if (FName* FoundName = SocketOverrideLookup.Find(SourceSocketName))
	{
		if (*FoundName != OverrideSocketName)
		{
			...
			{
				UE_LOGF(LogSkinnedMeshComp, Warning, "AddSocketOverride(%ls, %ls): Component(%ls) Actor(%ls) has already defined an override for socket(%ls), replacing %ls as override",
...
FQuat USkinnedMeshComponent::GetBoneQuaternion(FName BoneName, EBoneSpaces::Type Space) const
{
	...
	if( BoneIndex == INDEX_NONE )
	{
		UE_LOGF(LogSkinnedMeshComp, Warning, "USkinnedMeshComponent::execGetBoneQuaternion : Could not find bone: %ls", *BoneName.ToString());
	...
	if( Space == EBoneSpaces::ComponentSpace )
	{
		...
		if(LeaderPoseComponentInst)
		{
			if(BoneIndex < GetLeaderBoneMap().Num())
			{
				int32 ParentBoneIndex = GetLeaderBoneMap()[BoneIndex];
				// If ParentBoneIndex is valid, grab matrix from LeaderPoseComponent.
				if(	ParentBoneIndex != INDEX_NONE && 
					ParentBoneIndex < LeaderPoseComponentInst->GetNumComponentSpaceTransforms())
				{
...
FVector USkinnedMeshComponent::GetBoneLocation(FName BoneName, EBoneSpaces::Type Space) const
{
	...
	if( BoneIndex == INDEX_NONE )
	{
		UE_LOGF(LogAnimation, Log, "USkinnedMeshComponent::GetBoneLocation (%ls %ls): Could not find bone: %ls", *GetFullName(), *GetDetailedInfo(), *BoneName.ToString() );
		return FVector::ZeroVector;
	}
...
FVector USkinnedMeshComponent::GetBoneAxis( FName BoneName, EAxis::Type Axis ) const
{
	...
	if (BoneIndex == INDEX_NONE)
	{
		UE_LOGF(LogSkinnedMeshComp, Warning, "USkinnedMeshComponent::execGetBoneAxis : Could not find bone: %ls", *BoneName.ToString());
		return FVector::ZeroVector;
	}
	else if (Axis == EAxis::None)
	{
		UE_LOGF(LogSkinnedMeshComp, Warning, "USkinnedMeshComponent::execGetBoneAxis: Invalid axis specified");
		return FVector::ZeroVector;
	}
	else
	{
...
void USkinnedMeshComponent::SetRefPoseOverride(const TArray<FTransform>& NewRefPoseTransforms)
{
	if (!GetSkinnedAsset())
	{
		UE_LOGF(LogSkeletalMesh, Warning, "SetRefPoseOverride (%ls) : Not valid without SkeletalMesh assigned.", *GetName());
		return;
	}
	...
	{
		UE_LOGF(LogSkeletalMesh, Warning, "SetRefPoseOverride (%ls) : Expected %d transforms, got %d.", *GetSkinnedAsset()->GetName(), NumRealBones, NewRefPoseTransforms.Num());
		return;
	}
	...
	for (int32 BoneIndex = 0; BoneIndex < NumRealBones; BoneIndex++)
	{
			...
			ZAxis.IsNearlyZero(UE_SMALL_NUMBER))
		{
			...
			UE_LOGF(LogSkeletalMesh, Warning, "Reference Pose for asset %ls for joint (%ls) includes NIL matrix. Zero scale isn't allowed on ref pose. ", *GetSkinnedAsset()->GetPathName(), *GetSkinnedAsset()->GetRefSkeleton().GetBoneName(BoneIndex).ToString());
		}
...
void USkinnedMeshComponent::SetEvaluateWorldPositionOffset(bool bNewValue)
{
	if (bEvaluateWorldPositionOffset != bNewValue)
	{
		bEvaluateWorldPositionOffset = bNewValue;
		if (GetSkinnedAsset() && GetScene() && SceneProxy)
		{
			SceneProxy->SetEvaluateWorldPositionOffset_GameThread(bNewValue);
			MarkRenderTransformDirty();
		}
	}
}

void USkinnedMeshComponent::SetWorldPositionOffsetDisableDistance(int32 NewValue)
{
...
void USkinnedMeshComponent::SetForceDisableNanite(bool bInForceDisableNanite)
{
	if (bForceDisableNanite == bInForceDisableNanite)
	{
		return;
	}
	bForceDisableNanite = bInForceDisableNanite;

	// Recreate the proxy iff the decision flips.
	if (SceneProxy != nullptr && SceneProxy->IsNaniteMesh() != ShouldNaniteSkin())
	{
		MarkRenderStateDirty();
	}
}
...
void USkinnedMeshComponent::RegisterLODStreamingCallback(FLODStreamingCallback&& Callback, int32 LODIdx, float TimeoutSecs, bool bOnStreamIn)
{
	if (GetSkinnedAsset())
	{
		...
		GetSkinnedAsset()->RegisterMipLevelChangeCallback(GetPrimitiveComponentInterface(), LODIdx, TimeoutSecs, bOnStreamIn, MoveTemp(Callback));
...
void USkinnedMeshComponent::RegisterLODStreamingCallback(FLODStreamingCallback&& CallbackStreamingStart, FLODStreamingCallback&& CallbackStreamingDone, float TimeoutStartSecs, float TimeoutDoneSecs)
{
	if (GetSkinnedAsset())
	{
		GetSkinnedAsset()->RegisterMipLevelChangeCallback(GetPrimitiveComponentInterface(), TimeoutStartSecs, MoveTemp(CallbackStreamingStart), TimeoutDoneSecs, MoveTemp(CallbackStreamingDone));
		bMipLevelCallbackRegistered = true;
	}
}
...
void USkinnedMeshComponent::BeginDestroy()
{
	...
	FObjectCacheEventSink::NotifySkinnedMeshChanged_Concurrent(GetSkinnedMeshComponentInterface());
#endif // WITH_EDITOR
...
void FSkelMeshComponentLODInfo::SetActiveSkinWeightProfileStack(const FSkinWeightProfileStack& InProfileStack)
{
	ProfileStackRequest = InProfileStack;
}

bool FSkelMeshComponentLODInfo::GetActiveSkinWeightProfileStack(FSkinWeightProfileStack& OutProfileStack) const
{
	OutProfileStack = ProfileStackRequest;
	return !ProfileStackRequest.IsEmpty();
}
	...
	TArray<int32>& OutInvalidBones)
{
	...
	if (bWeightUnderrun)
	{
		UE_LOGF(LogSkinnedMeshComp, Warning, "SetSkinWeightOverride: Too few weights specified.");
	}
}
	...
	const FReferenceSkeleton& RefSkel)
{
	...
	for (int32 SectionIdx = 0; SectionIdx < LODData.RenderSections.Num(); SectionIdx++)
	{
		...
		{
			UE_LOGF(LogSkinnedMeshComp, Warning, "SetSkinWeightOverride: Invalid bones index specified for section %d:", SectionIdx);

			for (int32 BoneIndex : InvalidBones)
			{
				FName BoneName = RefSkel.GetBoneName(BoneIndex);
				UE_LOGF(LogSkinnedMeshComp, Warning, "SetSkinWeightOverride: %d %ls", BoneIndex, *BoneName.ToString());
			}
		}
...
void USkinnedMeshComponent::SetSkinWeightOverride(int32 LODIndex, const TArray<FSkelMeshSkinWeightInfo>& SkinWeights)
{
	...
	if (SkelMeshRenderData != nullptr && LODInfo.IsValidIndex(LODIndex) && SkelMeshRenderData->LODRenderData.IsValidIndex(LODIndex))
	{
		...
		if (SkinWeights.Num() >= ExpectedNumVerts)
		{
			if (SkinWeights.Num() > ExpectedNumVerts)
			{
				UE_LOGF(LogSkinnedMeshComp, Warning, "SetSkinWeightOverride: Too many weights - expected %d, got %d - truncating", ExpectedNumVerts, SkinWeights.Num());
			}
		...
		{
			UE_LOGF(LogSkinnedMeshComp, Warning, "SetSkinWeightOverride: Not enough weights - expected %d, got %d - aborting.", ExpectedNumVerts, SkinWeights.Num());
		}
	}
}
...
bool USkinnedMeshComponent::IsSkinWeightProfileManagementOnRenderThread() const
{
	return bSkinWeightProfileOnRenderThread || IsSkinnedMeshSkinWeightProfilesOnRenderThread();
}
...
{
	// Normalize the profile stack to remove duplicates.
	FSkinWeightProfileStack NormalizedProfileStack(InProfileStack.Normalized());
	
	// If we're not actually changing anything, then just return and say we did, as if the operation succeeded.
	if (NormalizedProfileStack == FSkinWeightProfileStack{CurrentSkinWeightProfileLayers}.Normalized())
	{
		return true;
	}
	...
	if (FSkeletalMeshRenderData* SkelMeshRenderData = GetSkeletalMeshRenderData())
	{
		if (IsSkinWeightProfileManagementOnRenderThread())
		{
			InitLODInfos();
			// Clear skin weight buffer set for all of the LODs
			const int32 LODStartIndex = FMath::Max(0, GSkinWeightProfilesAllowedFromLOD + 1);
			for (int32 LODIndex = LODStartIndex; LODIndex < LODInfo.Num(); ++LODIndex)
			{
				FSkelMeshComponentLODInfo& Info = LODInfo[LODIndex];
				FSkeletalMeshLODRenderData& LODRenderData = SkelMeshRenderData->LODRenderData[LODIndex];
						
				FSkinWeightProfileStack ActiveProfileStack;
				Info.GetActiveSkinWeightProfileStack(ActiveProfileStack);
				if (ActiveProfileStack != NormalizedProfileStack)
				{
					Info.SetActiveSkinWeightProfileStack(NormalizedProfileStack);
					bChanged = true;
				}
			}
			
			if (NormalizedProfileStack.IsEmpty() && bChanged)
			{
				// If skin weight profile is being "reset" i.e set to empty,
				// update the override buffer immediately. 
				UpdateSkinWeightOverrideBuffer();
			}
			else if (bChanged)
			{
		...
		{
			if (NormalizedProfileStack.IsEmpty())
			{
				if (bSkinWeightProfileSet)
				{
					bool bCleared = false;
			...
			else
			{
				...
				for (int32 LODIndex = 0; LODIndex < LODInfo.Num(); ++LODIndex)
				{
					...
					if (LODIndex > GSkinWeightProfilesAllowedFromLOD)
					{
						...
						{
							PRAGMA_DISABLE_DEPRECATION_WARNINGS
							UE::TScopeLock Lock(RenderData.RenderDataMutex);
							PRAGMA_ENABLE_DEPRECATION_WARNINGS

								for (int32 LayerIndex = 0; LayerIndex < FSkinWeightProfileStack::MaxLayerCount; ++LayerIndex)
								{
									bChanged |= RenderData.SkinWeightProfilesData.ContainsProfile(NormalizedProfileStack[LayerIndex]);
								}

							// Retrieve this profile's skin weight buffer
							FSkinWeightVertexBuffer* Buffer = RenderData.SkinWeightProfilesData.GetOverrideBuffer(NormalizedProfileStack);
				...
				if (bChanged)
				{
					...
					else 
					{
								...
								if (WeakMesh.IsValid() && WeakComponent.IsValid())
								{
									...
									if (FSkeletalMeshRenderData * RenderData = WeakMesh->GetResourceForRendering())
									{
										...
										for (int32 Index = 0; Index < NumLODs; ++Index)
										{
											...
											PRAGMA_DISABLE_DEPRECATION_WARNINGS
											UE::TScopeLock Lock(LODRenderData.RenderDataMutex);
											PRAGMA_ENABLE_DEPRECATION_WARNINGS
						...
						if (FSkinWeightProfileManager* Manager = FSkinWeightProfileManager::Get(GetWorld()))
						{
							...
							Manager->RequestSkinWeightProfileStack(NormalizedProfileStack, GetSkinnedAsset(), this, Callback);
							bSkinWeightProfilePending = true;
						}
					}
				}
	...
	NormalizedProfileStack.CopyIntoArray(CurrentSkinWeightProfileLayers);
...
void USkinnedMeshComponent::NotifyIfSkinnedAssetChanged()
{
#if WITH_EDITOR
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	USkinnedAsset* CurrentSkinnedAsset = !SkeletalMesh ? SkinnedAsset : SkeletalMesh;	// Return the SkeletalMesh pointer for backward compatibility until it is retired
	if (KnownSkinnedAsset != CurrentSkinnedAsset)
	{
		KnownSkinnedAsset = CurrentSkinnedAsset;
		FObjectCacheEventSink::NotifySkinnedMeshChanged_Concurrent(GetSkinnedMeshComponentInterface());
	}
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
#endif
...
#if WITH_EDITOR
void USkinnedMeshComponent::OutdatedKnownSkinnedAssetDetected() const
{
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	USkinnedAsset* CurrentSkinnedAsset = !SkeletalMesh ? SkinnedAsset : SkeletalMesh;	// Return the SkeletalMesh pointer for backward compatibility until it is retired
	ensureMsgf(
		KnownSkinnedAsset == CurrentSkinnedAsset,
		TEXT("SkinnedAsset property overwritten for component %s without a call to NotifyIfSkinnedAssetChanged(). KnownSkinnedAsset (%p) != SkinnedAsset (%p - %s)"),
		*GetFullName(),
		KnownSkinnedAsset,
		CurrentSkinnedAsset,
		CurrentSkinnedAsset ? *CurrentSkinnedAsset->GetFullName() : TEXT("nullptr")
		);
	PRAGMA_ENABLE_DEPRECATION_WARNINGS

	// This is a last resort, call the notification now
	USkinnedMeshComponent* MutableThis = const_cast<USkinnedMeshComponent*>(this);
	MutableThis->NotifyIfSkinnedAssetChanged();
}
#endif
