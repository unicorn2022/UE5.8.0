...
void FTextRenderSceneProxy::GetDynamicRayTracingInstances(FRayTracingInstanceCollector& Collector)
{
	...
	if (bRayTracingWithWPO && VertexFactory.GetType()->SupportsRayTracingDynamicGeometry())
	{
		...
		UpdateParams.NumVertices = (uint32)VertexBuffers.PositionVertexBuffer.GetNumVertices();
		UpdateParams.VertexBufferSize = uint32((SIZE_T)VertexBuffers.PositionVertexBuffer.GetNumVertices() * sizeof(FVector3f));
	...
	else if (RayTracingGeometry.IsEvicted())
	{
