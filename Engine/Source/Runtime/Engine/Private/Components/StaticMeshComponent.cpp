...
static UTexture* RecreateMeshPaintTexture(UTexture* InTexture, UPrimitiveComponent* InComponent)
{
	...
	{
		NewTexture = NewObject<UMeshPaintVirtualTexture>(InComponent);
...
void UStaticMeshComponent::OnRegister()
{
	...
	if (FApp::CanEverRender() && IsComponentPSOPrecachingEnabled())
	{
		...
		if (PSOPrecacheComponentData.IsPSOPrecacheCalled() && GetWorld() && GetWorld()->IsGameWorld())
		{
...
bool UStaticMeshComponent::RemapActorTextureStreamingBuiltDataToLevel(const UActorTextureStreamingBuildDataComponent* InActorTextureBuildData)
{
	...
	for (FStreamingTextureBuildInfo& BuildInfo : StreamingTextureData)
	{
		...
		if (TextureLevelIndex == InvalidRegisteredStreamableTexture)
		{
			...
			UE_LOGF(LogStaticMesh, Warning, "Clearing invalid texture streaming built data for %ls", *GetFullName());
	...
	if (FPlatformProperties::HasEditorOnlyData())
	{
		...
		if (!bIgnoreInstanceForTextureStreaming && Mobility == EComponentMobility::Static && GetStaticMesh() && GetStaticMesh()->GetRenderData() && !bHiddenInGame && bCanBuildTextureStreamingData)
		{
			...
			if ((BuildType == TSB_MapBuild) || (BuildType == TSB_ActorBuild))
			{
				...
				{
					UE_LOGF(LogStaticMesh, Warning, "No texture streaming container found : Can't build texture streaming data for %ls", *GetFullName());
					bBuildDataValid = false;
				}
			}
...
UStreamableRenderAsset* UStaticMeshComponent::GetStreamableNaniteAsset() const
{
...
UBodySetup* UStaticMeshComponent::GetBodySetup()
{
	...
	{
		const FBodyInstancePhysicsCreationInputs* Cached = BodyInstance.GetAsyncPhysicsCreationInputs();
		if (ensureAlwaysMsgf(Cached, TEXT("UStaticMeshComponent::GetBodySetup called outside of game thread, expected a valid cache for %s"), *GetFullName()))
		{
		...
		for ( TArray<FStaticMeshComponentLODInfo>::TIterator LODIter( LODData ); LODIter; ++LODIter )
		{
				 ...
				 LODIter.GetIndex() < NumLODs ) 
			{
				...
				{					
					...
					{
						UE_LOGF(LogStaticMesh, Warning, "Level requires re-saving! Outdated vertex color overrides have been discarded for %ls %ls LOD%d. ", *GetFullName(), *GetStaticMesh()->GetFullName(), LODIter.GetIndex());
						CurCompLODInfo.ReleaseOverrideVertexColorsAndBlock();
					}
					else
					{
						UE_LOGF(LogStaticMesh, Warning, "Unable to cache painted data for mesh component. Vertex color overrides will be lost if the mesh is modified. %ls %ls LOD%d.", *GetFullName(), *GetStaticMesh()->GetFullName(), LODIter.GetIndex() );
					}
				}
			}
		}
...
void UStaticMeshComponent::PostEditUndo()
{
	...
	// TODO: should also check !GetStaticMesh()->IsCompiling() here? (UE-365996)
...
void UStaticMeshComponent::PostEditChangeChainProperty(FPropertyChangedChainEvent& PropertyChangedEvent)
{
	NotifyIfStaticMeshChanged();
...
void UStaticMeshComponent::ReleaseMaterialCacheTextures()
{
	...
	{
		if (VirtualTexture->OwningComponent == this)
		{
...
{
	// Ensure persistent textures are ready
	for (TObjectPtr VirtualTexture : MaterialCacheTextures)
	{
		if (!VirtualTexture->GetResource())
		{
...
void UStaticMeshComponent::PostLoad()
{
	...
	if (OverriddenLightMapRes < 4)
	{
		OverriddenLightMapRes = 4;
	}
...
bool UStaticMeshComponent::ShouldCreateRenderState() const
{
	...
	{
		UE_LOGF(LogStaticMesh, Verbose, "ShouldCreateRenderState returned false for %ls (Base class was false)", *GetFullName());
		return false;
	}
	...
	{
		UE_LOGF(LogStaticMesh, Verbose, "ShouldCreateRenderState returned false for %ls (StaticMesh is null)", *GetFullName());
		return false;
	}
	...
	{
		UE_LOGF(LogStaticMesh, Verbose, "ShouldCreateRenderState returned false for %ls (StaticMesh is not ready)", *GetFullName());
		return false;
	}
...
void UStaticMeshComponent::RegisterLODStreamingCallback(FLODStreamingCallback&& Callback, int32 LODIdx, float TimeoutSecs, bool bOnStreamIn)
{
	if (UStaticMesh* Mesh = GetStaticMesh())
	{
		...
		Mesh->RegisterMipLevelChangeCallback(GetPrimitiveComponentInterface(), LODIdx, TimeoutSecs, bOnStreamIn, MoveTemp(Callback));
...
void UStaticMeshComponent::RegisterLODStreamingCallback(FLODStreamingCallback&& CallbackStreamingStart, FLODStreamingCallback&& CallbackStreamingDone, float TimeoutStartSecs, float TimeoutDoneSecs)
{
	if (UStaticMesh* Mesh = GetStaticMesh())
	{
		Mesh->RegisterMipLevelChangeCallback(GetPrimitiveComponentInterface(), TimeoutStartSecs, MoveTemp(CallbackStreamingStart), TimeoutDoneSecs, MoveTemp(CallbackStreamingDone));
		bMipLevelCallbackRegistered = true;
	}
}
...
{
	const int32 NumTexCoords = StaticMesh ? StaticMesh->GetNumTexCoords(0) : 0;
...
void FStaticMeshComponentLODInfo::ExportText(FString& ValueStr)
{
	...
	for(int32 i = 0; i < PaintedVertices.Num(); ++i)
	{
		...
		// Separate each vertex entry with a comma
	...
	if( !StripFlags.IsClassDataStripped( OverrideColorsStripFlag ) )
	{
		...
		if(bLoadVertexColorData)
		{
			...
			if (Ar.IsLoading())
			{
				...
				{
					UE_LOGF(LogStaticMesh, Log, "Loading a staticmesh component that is flag with override vertex color buffer, but the buffer is empty after loading(serializing) it. Resave the map to fix the component override vertex color data.");
