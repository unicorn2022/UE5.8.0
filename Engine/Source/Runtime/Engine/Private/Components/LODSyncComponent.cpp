...
void ULODSyncComponent::OnRegister()
{
	...
	UE_LOGF(LogLODSync, Verbose, "Initialized Sync Component");
...
void ULODSyncComponent::OnUnregister()
{
	...
	UE_LOGF(LogLODSync, Verbose, "Uninitialized Sync Component");
	Super::OnUnregister();
}
			...
			if (PrimComponent)
			{
				...
				if (LODInterface && CompSync.SyncOption != ESyncOption::Disabled)
				{
					...
					if (CompSync.SyncOption == ESyncOption::Drive)
					{
						...
						UE_LOGF(LogLODSync, Verbose, "Adding new component (%ls - LODCount : %d ) to sync.", *CompSync.Name.ToString(), LODCount);
						if (bFindTheMaxLOD)
						{
							// get max lod
							CurrentNumLODs = FMath::Max(CurrentNumLODs, LODCount);
							UE_LOGF(LogLODSync, Verbose, "MaxLOD now is set to (%d ).", CurrentNumLODs);
						}
					}
				}
			}
...
void ULODSyncComponent::UpdateLOD()
{
	...
	if (DriveComponents.Num() > 0)
	{
		...
		if (ForceLODCvar && ForceLODCvar->GetInt() >= 0)
		{
			...
			UE_LOGF(LogLODSync, Verbose, "LOD Sync : Using Cvar r.ForceLod Value to set lodsync [%d]", HighestPriLOD);
		}
		else if (ForcedLOD >= 0 && ForcedLOD < CurrentNumLODs)
		{
			HighestPriLOD = ForcedLOD;
			bHaveValidSetting = true;
			UE_LOGF(LogLODSync, Verbose, "LOD Sync : Using ForcedLOD [%d]", ForcedLOD);
		}
		else
		{
			...
			for (int32 DriveComponentIndex = DriveComponents.Num() - 1; DriveComponentIndex >= 0; DriveComponentIndex--)
			{
				...
				if (Component->WasRecentlyRendered())
				{
					...
					UE_LOGF(LogLODSync, Verbose, "LOD Sync Drivers : %ls (VISIBLE) - Source LOD [%d] RemappedLOD[%d]", *GetNameSafe(Component), DesiredSyncLOD, DesiredLOD);
				...
				UE_LOGF(LogLODSync, Verbose, "LOD Sync Drivers : %ls - Source LOD [%d] RemappedLOD[%d]", *GetNameSafe(Component), DesiredSyncLOD, DesiredLOD);
			}
		}
		...
		if (bHaveValidSetting)
		{
			...
			UE_LOGF(LogLODSync, Verbose, "LOD Sync : Current LOD (%d)", HighestPriLOD);
