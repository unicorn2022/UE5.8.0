...
// Mirror of renderer function, which is inaccessible to the engine component
static bool IsSceneCaptureCubeShareCascadesEnabled(EShaderPlatform Platform)
{
	static const IConsoleVariable* CVarSceneCaptureCubeMobileShareCascades = IConsoleManager::Get().FindConsoleVariable(TEXT("r.SceneCapture.CubeMobileShareCascades"));
	return CVarSceneCaptureCubeMobileShareCascades && CVarSceneCaptureCubeMobileShareCascades->GetBool() && IsMobilePlatform(Platform);
}
...
private:
	...
	virtual float GetMaxShadowReceiverDistance(const FSceneView& View, bool bPrecomputedLightingIsValid) const override
	{
		const float CSMMax = GetCSMMaxDistance(bPrecomputedLightingIsValid, View.MaxShadowCascades);
		const uint32 ClampedFarCount = FMath::Min((uint32)CVarMaxNumFarShadowCascades.GetValueOnAnyThread(), FarShadowCascadeCount);
		if (ClampedFarCount > 0)
		{
			const float FarOverride = CVarFarShadowDistanceOverride.GetValueOnAnyThread();
			const float FarEnd = FarOverride > 0.0f ? FarOverride : FarShadowDistance;
	...
	virtual FSphere GetShadowSplitBoundsDepthRange(const FSceneView& View, FVector ViewOrigin, float SplitNear, float SplitFar, FShadowCascadeSettings* OutCascadeSettings) const override
	{
		...
		const FMatrix& ProjectionMatrix = ViewMatrices.GetViewToClipNoAA();
