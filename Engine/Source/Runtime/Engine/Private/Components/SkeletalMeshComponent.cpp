...
static bool bCVarUninitializeInstanceOnNewSingleNodeInstance = true;
static FAutoConsoleVariableRef CVarUninitializeSingleNodeInstanceOnNewInstance(
	TEXT("a.UninitializeSingleNodeInstanceOnNewInstance"),
	bCVarUninitializeInstanceOnNewSingleNodeInstance,
	TEXT("Whether the current anim instance (if any) should be uninitialized before being assigned a new instance."));
...
struct SkeletalMeshUCSModifiedDataCleanupHelper
{
	...
	{
		static const FName SkeletalMeshPropertyName("SkeletalMesh");
...
public:
	...
	void DoTask(ENamedThreads::Type CurrentThread, const FGraphEventRef& MyCompletionGraphEvent)
	{
		...
		{
			UE_LOG_CONTEXT("SkeletalMeshComponent", FSoftObjectPath(Comp));
	...
	: Super(ObjectInitializer)
{
	...
	EndPhysicsTickFunction.bRunTransactionally = true;
	...
	// End tick runs after every other tick group to wait on the parallel cloth task without pumping the EOF queue.
	ClothEndTickFunction.TickGroup = TG_LastDemotable;
	ClothEndTickFunction.EndTickGroup = TG_LastDemotable;
	ClothEndTickFunction.bCanEverTick = true;
	ClothEndTickFunction.bStartWithTickEnabled = true;
	...
	bDisableAnimDynamicsAnimNode = false;
...
void USkeletalMeshComponent::PostDuplicate(bool bDuplicateForPIE)
{
...
#if WITH_EDITOR
void USkeletalMeshComponent::PostEditImport()
{
	Super::PostEditImport();

	// Update property alias after a duplicate (PostDuplicate not called on actor components)
	RefreshSkeletalMeshAsset();
}
#endif
...
void USkeletalMeshComponent::SetComponentTickEnabled(bool bEnabled)
{
	...
	// Unregister the cloth tick functions when the component's tick is being disabled (they will be re-enabled at the next component tick update once it runs again)
	if (!bEnabled)
	{
		RegisterClothTick(false);
		RegisterClothEndTick(false);
	}
}
...
void USkeletalMeshComponent::RegisterClothEndTick(bool bRegister)
{
	if (bRegister != ClothEndTickFunction.IsTickFunctionRegistered())
	{
		if (bRegister)
		{
			if (SetupActorComponentTickFunction(&ClothEndTickFunction))
			{
				ClothEndTickFunction.Target = this;
				ClothEndTickFunction.AddPrerequisite(this, ClothTickFunction);
			}
		}
		else
		{
			ClothEndTickFunction.UnRegisterTickFunction();
		}
	}
}
...
extern TAutoConsoleVariable<int32> CVarClothPhysicsLODThreshold;
...
bool USkeletalMeshComponent::CanSimulateClothing() const
{
	...
	const int32 LODThreshold = CVarClothPhysicsLODThreshold.GetValueOnAnyThread();
	if (LODThreshold >= 0)
	{
		const int32 LODIndex = GetPredictedLODLevel();
		if (LODIndex < 0 || LODIndex > LODThreshold)
		{
			return false;
		}
	}
...
{
	const bool bShouldRunClothTick = PrimaryComponentTick.IsTickFunctionRegistered() && ShouldRunClothTick();
	RegisterClothTick(bShouldRunClothTick);
	// The end tick owns the wait on the parallel cloth task when the cloth tick function isn't doing it itself.
	// Skip it when the in-tick wait is on (nothing to do here), or when the end-tick path has been disabled via CVar (the wait falls back to OnPreEndOfFrameSync).
	RegisterClothEndTick(bShouldRunClothTick && !ShouldWaitForClothInTickFunction() && ShouldUseClothEndTickWait());
}
...
bool USkeletalMeshComponent::AreBoneTransformsValid() const
{
	return Super::AreBoneTransformsValid() || bBoneTransformsInitialized;
}

void USkeletalMeshComponent::InitializeBeforeRegister()
{
	// Make sure this is done before we call into the Super which will trigger OnCreatePhysicsState
	UpdateHasValidBodies();
}

void USkeletalMeshComponent::InitializeOnRegister()
{
	checkf(!bBoneTransformsInitialized, TEXT("BoneTransforms should not be initialized before skeletal registration setup."));
	...
	// This handles cases where we don't really update the transforms 
	// - SkeletalMeshAsset is invalid 
	// - LeaderPoseComponent is valid
	// - AnimScriptInstance is valid && InitializeAnimScriptInstance returns false
	bBoneTransformsInitialized = true;
}
...
void USkeletalMeshComponent::OnPreRegister()
{
	TRACE_CPUPROFILER_EVENT_SCOPE(USkeletalMeshComponent::OnPreRegister);

	// Temporarily pause the AsyncPhysicsStateJobQueue
	//
	// This is necessary because calling Super::OnPreRegister will schedule an async physics job
	// Pausing new jobs to be added will prevent USkeletalMeshComponent::OnAsyncCreatePhysicsState from being called 
	// until USkeletalMeshComponent::OnPreRegister completes and initializes BoneSpace transforms.
	FPhysScene* PhysScene = GetPhysicsScene();
	check(PhysScene);
	FPhysScene_AsyncPhysicsStateJobQueue::FScopedDeferAsyncPhysicsStateJobs DeferJobs(PhysScene);
...
void USkeletalMeshComponent::OnUnregister()
{
	...
	bBoneTransformsInitialized = false;
...
void USkeletalMeshComponent::InitAnim(bool bForceReinit)
{
	...
	if (GetSkeletalMeshAsset() != nullptr && (IsRegistered() || IsPreRegistering()))
	{
...
bool USkeletalMeshComponent::InitializeAnimScriptInstance(bool bForceReinit, bool bInDeferRootNodeInitialization)
{
	...
	if (SkelMesh && (IsRegistered() || IsPreRegistering()))
	{
		// We may be doing parallel evaluation on the current anim instance
		// Calling this here with true will block this init till that thread completes
		// and it is safe to continue
		...
		else 
		{
			...
			if (bShouldSpawnSingleNodeInstance)
			{
				...
				if (bCVarUninitializeInstanceOnNewSingleNodeInstance && AnimScriptInstance)
				{
					AnimScriptInstance->UninitializeAnimation();
				}
		...
		{
			// It is important to make sure that all post-process related stuff happens after a call to this
			// function, otherwise it may reset linked anim instances that the post-process anim instance creates.
			ResetLinkedAnimInstances();
		...
		if(NeedToSpawnPostPhysicsInstance(bForceReinit))
		{
			...
			if(PostProcessAnimInstance)
			{
				...
				if(FAnimNode_LinkedInputPose* InputNode = PostProcessAnimInstance->GetLinkedInputPoseNode())
				{
					...
					// SetBoneContainer allocates space for bone data but leaves it uninitialized.
...
void USkeletalMeshComponent::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if ( PropertyThatChanged != nullptr )
	{
		...
		if ( PropertyThatChanged->GetFName() == GET_MEMBER_NAME_CHECKED( FSingleAnimationPlayData, AnimToPlay ))
		{
			...
			{
				UE_LOGF(LogAnimation, Warning, "Invalid animation skeleton");
...
void USkeletalMeshComponent::LoadedFromAnotherClass(const FName& OldClassName)
{
	...
	{
		static FName SingleAnimSkeletalComponent_NAME("SingleAnimSkeletalComponent");
...
void USkeletalMeshComponent::TickAnimation(float DeltaTime, bool bNeedsValidRootMotion)
{
	...
	if (GetSkeletalMeshAsset() != nullptr && !bIsCompiling)
	{
		...
		bNeedsQueuedAnimEventsDispatched = !bSuppressNotifyEventDispatch;
...
void USkeletalMeshComponent::TickMontagesOnly(float DeltaTime, bool bNeedsValidRootMotion)
{
	if (!bEnableAnimation)
	{
		if (AnimScriptInstance)
		{
			...
			// Update the pose tick frame counter to avoid double ticking the montages. 
			LastPoseTickFrame = static_cast<uint32>(GFrameCounter);

			AnimScriptInstance->TickMontageOnly(DeltaTime);
		}
		else
		{
			UE_LOGF(LogAnimation, Warning, "SkeletalMeshComponent in '%ls' tried to TickMontagesOnly but there is no valid AnimInstance", *GetNameSafe(GetOwner()));
		}
	}
	else
	{
		UE_LOGF(LogAnimation, Warning, "SkeletalMeshComponent in '%ls' tried to call TickMontagesOnly when animations are enabled. This is likely an error and will be skipped.", *GetNameSafe(GetOwner()));
	}
}
...
void USkeletalMeshComponent::TickPose(float DeltaTime, bool bNeedsValidRootMotion)
{
	...
	if (ShouldTickAnimation())
	{
		...
		{
			UE_LOGF(LogTemp, Display, "%d Ticked %d NotTicked", Ticked.GetValue(), NotTicked.GetValue());
			Ticked.Reset();
			NotTicked.Reset();
		}
	}
...
void USkeletalMeshComponent::ComputeRequiredBones(TArray<FBoneIndexType>& OutRequiredBones, TArray<FBoneIndexType>& OutFillComponentSpaceTransformsRequiredBones, int32 LODIndex, bool bIgnorePhysicsAsset) const
{
	...
	if (!SkelMeshRenderData)
	{
		//No Render Data?
		// Jira UE-64409
		UE_LOGF(LogAnimation, Warning, "Skeletal Mesh asset '%ls' has no render data", *SkelMesh->GetName());
		return;
	}
	...
	{
		//No LODS?
		UE_LOGF(LogAnimation, Warning, "Skeletal Mesh asset '%ls' has no LODs", *SkelMesh->GetName());
		return;
	}
...
void USkeletalMeshComponent::ComputeValidMeshPoseTransforms(TArray<FBoneIndexType>& InRequiredBones, TBitArray<>& OutValidTransforms) const
{
	OutValidTransforms.Init(false, GetNumComponentSpaceTransforms());
	for (int32 i = 0; i < InRequiredBones.Num(); ++i)
	{
		OutValidTransforms[InRequiredBones[i]] = true; 
	}
}
		...
		InAnimInstance->ParallelCanEvaluate(InSkeletalMesh))
	{
		UE_ADD_CRASH_CONTEXT_SCOPE([&](FCrashContextExtendedWriter& Writer)
		{
			const FString Context = FString::Printf(
				TEXT("SkeletalMeshComponent='%s' SkeletalMesh='%s' Skeleton='%s' AnimInstance='%s' bForceRefPose=%s"),
				*GetNameSafe(this),
				*GetNameSafe(InSkeletalMesh),
				*GetNameSafe(InSkeletalMesh ? InSkeletalMesh->GetSkeleton() : nullptr),
				*GetNameSafe(InAnimInstance),
				bInForceRefPose ? TEXT("true") : TEXT("false")
			);
			Writer.AddString(TEXT("AnimationContext"), *Context);
		});
...
UClothingSimulationInteractor* USkeletalMeshComponent::GetClothingSimulationInteractor(const FName ClothingDataName) const
{
	...
	else if (ClothingSimulationInstances.Num())
	{
		...
		UE_CLOGF(
			...
			LogSkeletalMesh,
			Warning,
			"GetClothingSimulationInteractor must be passed a valid Clothing Data name to retrieve a Clothing Simulation Interactor for Skeletal Mesh [%ls], because this asset requires multiple simulation types.",
...
void USkeletalMeshComponent::DoInstanceFinalizeAnimation(bool bWasEvaluated)
{
	if (AnimScriptInstance)
	{
		AnimScriptInstance->FinalizeAnimation(bWasEvaluated);

		for (UAnimInstance* LinkedInstance : LinkedInstances)
		{
			LinkedInstance->FinalizeAnimation(bWasEvaluated);
		}
	}

	if (PostProcessAnimInstance)
	{
		PostProcessAnimInstance->FinalizeAnimation(bWasEvaluated);
	}
}
...
{
#if WITH_EDITOR
	// If we are re-instancing, we will not have run an update before since 
	// bNeedsUpdate == false and UAnimInstance::UpdateAnimation gets skipped
	if (GIsReinstancing)
	{
		return;
	}
...
void USkeletalMeshComponent::PostAnimEvaluation(FAnimationEvaluationContext& EvaluationContext)
{
	...
	if (!IsRegistered() && !IsPreRegistering()) // Notify/Event has caused us to go away so cannot carry on from here
	{
		return;
	}
	...
	if (EvaluationContext.bDoEvaluation || EvaluationContext.bDoInterpolation)
	{
		...
		DoInstanceFinalizeAnimation(EvaluationContext.bDoEvaluation);
	...
	{
		DoInstanceFinalizeAnimation(false);
...
void USkeletalMeshComponent::SetSkeletalMesh(USkeletalMesh* InSkelMesh, bool bReinitPose)
{
...

	{
		...
		else if(IsRegistered() || IsPreRegistering())
		{
			RecalcRequiredBones(GetPredictedLODLevel());
			ResetToRefPose();
			bBoneTransformsInitialized = true;
...
bool USkeletalMeshComponent::AllocateTransformData()
{
	...
	if ( Super::AllocateTransformData() )
	{
		...
		GetMutableValidMeshPoseTransforms().Init(false, GetNumComponentSpaceTransforms());
		bValidMeshPoseTransformsUpToDate = false;
	...
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	GetMutableValidMeshPoseTransforms().Empty();
	return false;
}
...
void USkeletalMeshComponent::SetAnimInstanceClass(class UClass* NewClass)
{
	...
	{
		UE_LOGF(LogAnimation, Warning, "USkeletalMeshComponent::SetAnimInstanceClass: Cannot set anim instance class while animation is still updating/evaluating. Apply a delay or perform the switch elsewhere.");
		return;
	}
	if (NewClass != nullptr)
	{
...
{
	if (!bSuppressNotifyEventDispatch)
	{
		bNeedsQueuedAnimEventsDispatched = true;
	}
	else
	{
		UE_LOGF(LogSkeletalMesh, Warning, "AllowQueuedAnimEventsNextDispatch called when notify dispatch is disabled on mesh %ls. Re-enable dispatch to allow this.", *GetName());
	}	
}
...
void USkeletalMeshComponent::HideBone( int32 BoneIndex, EPhysBodyOp PhysBodyOption)
{
	...
	{
		UE_LOGF(LogSkeletalMesh, Warning, "HideBone[%ls]: Invalid Body Index (%d) has entered. This component doesn't contain buffer for the given body.", *GetPathNameSafe(GetSkeletalMeshAsset()), BoneIndex);
	}
}
...
void USkeletalMeshComponent::UnHideBone( int32 BoneIndex )
{
	...
	{
		UE_LOGF(LogSkeletalMesh, Warning, "UnHideBone[%ls]: Invalid Body Index (%d) has entered. This component doesn't contain buffer for the given body.", *GetPathNameSafe(GetSkeletalMeshAsset()), BoneIndex);
	}
}
...
void USkeletalMeshComponent::MigratePhysicsState(FArchive& Ar)
{
	...
	if (Ar.IsLoading())
	{
		...
		{
			UE_LOGF(LogSkeletalMesh, Verbose, "Migrated an empty physics state (this can be expected) | [%ls] will use its default physics state or the one already had | Migration Context [%ls].", *GetPathNameSafe(this), ToString(ObjMigrationRecvType));
			return;
		}
	...
	else if (Ar.IsSaving())
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogSkeletalMesh, Error, "[%s] Attempted to migrated an object physics state, before processing a previous pending physics state| Discarding pending physics state and sending an empty one...", __func__);
			}
...
void USkeletalMeshComponent::ApplyMigratedPhysicsState(Chaos::FSerializedDataBufferPtr MigratedStateData)
{
	...
	if (RawMigratedData)
	{
		...
		UE_LOGF(LogSkeletalMesh, Verbose, "[%s] Applying migrated physics state | Data context [%ls]", __func__, Chaos::LexToString( MigratedState.GetHeader().DataContext));
		...
		for (FConstraintInstance* ConstraintInstance : Constraints)
		{
			...
			// Some other components may invalidates constraints in skeletal mesh to perform their own simulation (e.g. see UChaosWheeledVehicleMovementComponent::FixupSkeletalMesh())
			// This code ignores invalid constraint data on the receiving server to get around the ensure in Chaos::Private::ExtractJointConstraintDataFromBuffer
			auto ApplyMigratedData = [&MigratedState, ConstraintInstance](Chaos::ESerializedDataContext ContextToApply)
				{
					Chaos::FSerializedDataBufferPtr DataBuffer = MakeUnique<Chaos::FSerializedDataBuffer>();

					MigratedState.ReadElementDataIntoBuffer(*DataBuffer);

					if (!DataBuffer->GetDataAsByteArrayRef().IsEmpty())
					{
						ConstraintInstance->ApplySerializedState(MoveTemp(DataBuffer), ContextToApply);
					}
					else
					{
						UE_LOGF(LogSkeletalMesh, Log, "[%s]: Constraint %ls has empty data when migrated so it is skipped.",__func__, *ConstraintInstance->JointName.ToString());
					}
				};

			if (MigratedState.GetHeader().DataContext == Chaos::ESerializedDataContext::Both)
			{
				ApplyMigratedData(Chaos::ESerializedDataContext::Internal);
				ApplyMigratedData(Chaos::ESerializedDataContext::External);
			}
			else
			{
				ApplyMigratedData(MigratedState.GetHeader().DataContext);
			}
		}
	}
}
...
void USkeletalMeshComponent::SetAnimationMode(EAnimationMode::Type InAnimationMode, bool bForceInitAnimScriptInstance)
{
	if(!bEnableAnimation)
	{
		UE_LOGF(LogAnimation, Warning, "SetAnimationMode: Animation is currently disabled");
		return;
	}
...
void USkeletalMeshComponent::PlayAnimation(class UAnimationAsset* NewAnimToPlay, bool bLooping)
{
	if(!bEnableAnimation)
	{
		UE_LOGF(LogAnimation, Warning, "PlayAnimation: Animation is currently disabled");
		return;
	}
...
void USkeletalMeshComponent::SetAnimation(UAnimationAsset* NewAnimToPlay)
{
	if(!bEnableAnimation)
	{
		UE_LOGF(LogAnimation, Warning, "SetAnimation: Animation is currently disabled");
		return;
	}
	...
	{
		UE_LOGF(LogAnimation, Warning, "Currently in Animation Blueprint mode. Please change AnimationMode to Use Animation Asset");
	}
}
...
void USkeletalMeshComponent::Play(bool bLooping)
{
	if(!bEnableAnimation)
	{
		UE_LOGF(LogAnimation, Warning, "Play: Animation is currently disabled");
		return;
	}
...
void USkeletalMeshComponent::Stop()
{
	if(!bEnableAnimation)
	{
		UE_LOGF(LogAnimation, Warning, "Stop: Animation is currently disabled");
		return;
	}
...
void USkeletalMeshComponent::SetPosition(float InPos, bool bFireNotifies)
{
	if(!bEnableAnimation)
	{
		UE_LOGF(LogAnimation, Warning, "SetPosition: Animation is currently disabled");
		return;
	}
...
void USkeletalMeshComponent::SetPlayRate(float Rate)
{
	if(!bEnableAnimation)
	{
		UE_LOGF(LogAnimation, Warning, "SetPlayRate: Animation is currently disabled");
		return;
	}
...
void USkeletalMeshComponent::OverrideAnimationData(UAnimationAsset* InAnimToPlay, bool bIsLooping /*= true*/, bool bIsPlaying /*= true*/, float Position /*= 0.f*/, float PlayRate /*= 1.f*/)
{
	if(!bEnableAnimation)
	{
		UE_LOGF(LogAnimation, Warning, "OverrideAnimationData: Animation is currently disabled");
		return;
	}
...
FTransform USkeletalMeshComponent::ConvertLocalRootMotionToWorld(const FTransform& InTransform)
{
	...
	UE_LOGF(LogRootMotion, Log,  "ConvertLocalRootMotionToWorld LocalT: %ls, LocalR: %ls, WorldT: %ls, WorldR: %ls.",
...
void USkeletalMeshComponent::ValidateAnimation()
{
	...
	{
		UE_LOGF(LogAnimation, Warning, "SkeletalMesh %ls has no skeleton. This needs to fixed before an animation can be set", *SkelMesh->GetFullName());
	...
	if(AnimationMode == EAnimationMode::AnimationSingleNode)
	{
		if (AnimationData.AnimToPlay && SkelMesh)
		{
			...
			{
				UE_LOGF(LogAnimation, Warning, "Animation %ls is incompatible because it has no skeleton, removing animation from actor.", *AnimationData.AnimToPlay->GetName());
				AnimationData.AnimToPlay = nullptr;
			}
		}
	}
	else if (AnimationMode == EAnimationMode::AnimationBlueprint)
	{
		...
		if (AnimClassInterface && SkelMesh)
		{
			...
			{
				UE_LOGF(LogAnimation, Warning, "AnimBP %ls is incompatible because mesh %ls has no skeleton, removing AnimBP from actor.", *AnimClass->GetName(), *SkelMesh->GetName());
				AnimClass = nullptr;
			}
		}
	}
...
{
	// We are already processing eval and we aren't currently executing it on this thread
	if (IsRunningParallelEvaluation() && ParallelAnimationEvaluationTask->IsAwaitable())
	{
		if (bBlockOnTask)
		{
			...
			if (AutoRTFM::IsTransactional())
			{
				(void)AutoRTFM::Close([&]
					{
						AutoRTFM::OnRetry([strong_this = TObjectPtr<USkeletalMeshComponent>(this), bPerformPostAnimEvaluation]
							{
								UE_LOGF(LogSkeletalMesh, Display, "A call to `HandleExistingParallelEvaluationTask` that is blocking is causing Verse to abort and retry.");
								...
								UE_LOGF(LogSkeletalMesh, Display, "`HandleExistingParallelEvaluationTask` has completed after Verse aborted, and we are now retrying.");
							});
			...
			// We had a valid task handle and now we are done waiting for it
			// Perform completion now
			// Note that because completion happens in a separate task (on the main thread) from the one we waited on
			// it could execute after this call to finalize evaluation. If it does, it should be a no-op operation as
			// it means the skeletal mesh or some other core piece has changed and the completion validates that the
			// data we started with is consistent with the current data.
			// We don't wait for the main thread completion task because it could execute much later, instead we do it inline
			CompleteParallelAnimationEvaluation(bPerformPostAnimEvaluation);
		}
...
void USkeletalMeshComponent::ReinitializeAnimNodes()
{
	if (bRegistered && GetSkeletalMeshAsset() != nullptr)
	{
		if (AnimScriptInstance)
		{
			AnimScriptInstance->InitializeAnimation();
...
void USkeletalMeshComponent::SetAllowRigidBodyAnimNode(bool bInAllow, bool bReinitAnim)
{
	if (bDisableRigidBodyAnimNode == bInAllow)
	{
		...
		if(bReinitAnim)
		{
			...
			ReinitializeAnimNodes();
		}
	}
}

void USkeletalMeshComponent::SetAllowAnimDynamicsAnimNode(bool bInAllow, bool bReinitAnim)
{
	if (bDisableAnimDynamicsAnimNode == bInAllow)
	{
		bDisableAnimDynamicsAnimNode = !bInAllow;
...
void USkeletalMeshComponent::FinalizeBoneTransform() 
{
	...
	if (UE::Anim::bUseSafeMeshPoseIndices && bValidMeshPoseTransformsUpToDate == false)
	{
		ComputeValidMeshPoseTransforms(FillComponentSpaceTransformsRequiredBones, GetMutableValidMeshPoseTransforms());
		bValidMeshPoseTransformsUpToDate = true;
	}
...
bool USkeletalMeshComponent::MoveComponentImpl(const FVector& Delta, const FQuat& NewRotation, bool bSweep, FHitResult* OutHit /*= nullptr*/, EMoveComponentFlags MoveFlags /*= MOVECOMP_NoFlags*/, ETeleportType Teleport /*= ETeleportType::None*/)
{
	...
	{
		// If a skeletal mesh component receives a teleport we should reset any other dynamic simulations
...
bool USkeletalMeshComponent::GetVectorAttribute_Ref(const FName& BoneName, const FName& AttributeName, FVector& OutValue, ECustomBoneAttributeLookup LookupType /*= ECustomBoneAttributeLookup::BoneOnly*/)
{
	return FindAttributeChecked<FVector, FVectorAnimationAttribute>(BoneName, AttributeName, OutValue, OutValue, LookupType);
}

bool USkeletalMeshComponent::GetQuaternionAttribute_Ref(const FName& BoneName, const FName& AttributeName, FQuat& OutValue, ECustomBoneAttributeLookup LookupType /*= ECustomBoneAttributeLookup::BoneOnly*/)
{
	return FindAttributeChecked<FQuat, FQuaternionAnimationAttribute>(BoneName, AttributeName, OutValue, OutValue, LookupType);
}
...
bool USkeletalMeshComponent::GetVectorAttribute(const FName& BoneName, const FName& AttributeName, FVector DefaultValue, FVector& OutValue, ECustomBoneAttributeLookup LookupType /*= ECustomBoneAttributeLookup::BoneOnly*/)
{
	return FindAttributeChecked<FVector, FVectorAnimationAttribute>(BoneName, AttributeName, DefaultValue, OutValue, LookupType);
}

bool USkeletalMeshComponent::GetQuaternionAttribute(const FName& BoneName, const FName& AttributeName, FQuat DefaultValue, FQuat& OutValue, ECustomBoneAttributeLookup LookupType /*= ECustomBoneAttributeLookup::BoneOnly*/)
{
	return FindAttributeChecked<FQuat, FQuaternionAnimationAttribute>(BoneName, AttributeName, DefaultValue, OutValue, LookupType);
}
...
{
	// We may be doing parallel evaluation on the current anim instance
	// Calling this here with true will block until that thread completes
	// and it is safe to continue
	const bool bBlockOnTask = true; // wait on evaluation task so it is safe to swap the buffers
	const bool bPerformPostAnimEvaluation = true; // Do PostEvaluation so we make sure to swap the buffers back. 
	HandleExistingParallelEvaluationTask(bBlockOnTask, bPerformPostAnimEvaluation);
