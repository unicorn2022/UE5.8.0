...
{
	struct FModulatorWeakAccessor
	{
	private:
		TSet<TWeakObjectPtr<USoundModulatorBase>> WeakModulators;

	public:
		FModulatorWeakAccessor(const TSet<TObjectPtr<USoundModulatorBase>>& Modulators)
		{
			Algo::Transform(Modulators, WeakModulators, [](const TObjectPtr<USoundModulatorBase>& Obj)
			{
				return TWeakObjectPtr<USoundModulatorBase>(Obj.Get());
			});
		}

		bool TryGetSafe(TSet<TObjectPtr<USoundModulatorBase>>& OutModulators) const
		{
			OutModulators.Reset();

			auto IsValid = [](const TWeakObjectPtr<USoundModulatorBase>& WeakPtr) { return WeakPtr.IsValid(); };
			if (!Algo::AllOf(WeakModulators, IsValid))
			{
				return false;
			}

			auto AsObjectPtr = [](const TWeakObjectPtr<USoundModulatorBase>& WeakPtr) { return WeakPtr.Get(); };
			Algo::Transform(WeakModulators, OutModulators, AsObjectPtr);
			return true;
		}
	};
} // namespace AudioComponentPrivate
...
void UAudioComponent::BeginDestroy()
{
	...
	{
		UE_LOGF(LogAudio, Verbose, "Audio Component is being destroyed prior to stopping looping sound '%ls' directly.", *Sound->GetFullName());
		Stop();
	}
...
void UAudioComponent::PlayQueuedQuantizedInternal(const UObject* WorldContextObject, FAudioComponentCommandInfo InCommandInfo)
{
	...
	{		
		...
		if (PendingData.CommandID == InCommandInfo.CommandID)
		{
			...
			if(Sound)
			{
				...
				if (!Handle)
				{
					UE_LOGF(LogAudioQuartz, Warning, "Attempting to play Quantized Sound without supplying a Clock Handle");
					bIsValidCommand = false;
				}
				else if (!Handle->DoesClockExist(WorldContextObject))
				{
					UE_LOGF(LogAudioQuartz, Warning, "Clock: '%ls' Does not exist! Cannot play quantized sound: '%ls'", *InternalRequestData.QuantizedRequestData.ClockName.ToString(), *this->Sound->GetName());
					bIsValidCommand = false;
				}
				...
				if (PendingData.bHasBeenStoppedWhileQueued)
				{
					...
					UE_LOGF(LogAudioQuartz, Verbose, "Sound (%ls) to be played (on Clock: %ls) was stopped before being evaluated to play internally", *InternalRequestData.QuantizedRequestData.ClockName.ToString(), *this->Sound->GetName());
					bIsValidCommand = false;
				}
			...
			{
				UE_LOGF(LogAudioQuartz, Warning, "Attempting to play Quantized Sound without supplying a valid Sound to play");
			}
	...
	{
		UE_LOGF(LogAudioQuartz, Warning, "Failed to find command ID '%d' for sound '%ls'.",
...
void UAudioComponent::PlayInternal(const PlayInternalRequestData& InPlayRequestData, USoundBase* InSoundOverride)
{
	...
	if (SoundToPlay)
	{
		UE_LOGF(LogAudio, Verbose, "%g: Playing AudioComponent : '%ls' with Sound: '%ls'. OneShot?: '%ls'", AudioTimeSeconds, *GetFullName(), *SoundToPlay->GetName(), !SoundToPlay->IsLooping() ? TEXT("YES") : TEXT("NO"));
	}
	...
	if (!SoundToPlay)
	{
		UE_LOGF(LogAudio, Verbose, "%g: AudioComponent '%ls' failed to play sound: No sound to play specified.'", World ? World->GetAudioTimeSeconds() : 0.0f, *GetFullName());
		return;
	}
	...
	if (bIsSoundLooping)
	{
		if (bCanPlayMultipleInstances)
		{
			UE_LOGF(LogAudio, Warning, "'Can Play Multiple Instances' disabled: Sound '%ls' set to looping", *SoundToPlay->GetName());
		}
...
void UAudioComponent::Stop()
{
	...
	UE_LOGF(LogAudio, Verbose, "%g: Stopping AudioComponent : '%ls' with Sound: '%ls'", GetAudioTimeSeconds(), *GetFullName(), Sound ? *Sound->GetName() : TEXT("nullptr"));
...
void UAudioComponent::StopDelayed(float DelayTime)
{
	...
	AudioDevice->SendCommandToActiveSounds(AudioComponentID, [DelayTime](FActiveSound& ActiveSound)
	{
		...
		{
			UE_LOGF(LogAudio, Verbose, "%g: Delayed Stop requested for sound '%ls'",
...
void UAudioComponent::SetPaused(bool bPause)
{
	if (bIsPaused != bPause)
	{
		...
		if (IsActive())
		{
			UE_LOGF(LogAudio, Verbose, "%g: Pausing AudioComponent : '%ls' with Sound: '%ls'", GetAudioTimeSeconds(), *GetFullName(), Sound ? *Sound->GetName() : TEXT("nullptr"));
...
void UAudioComponent::SetBusSendEffectInternal(USoundSourceBus* InSourceBus, UAudioBus* InAudioBus, float SendLevel, EBusSendType InBusSendType)
{
	...
	if (IsPlaying())
	{
		if (FAudioDevice* AudioDevice = GetAudioDevice())
		{
			...
			{
				// Preserve existing filter state - this function only changes send level,
				// so don't clobber filter enable/cutoff fields on an existing override entry.
				FSoundSourceBusSendInfo InfoToSend = BusSendInfo;
				TArray<FSoundSourceBusSendInfo> CurrentSends;
				ActiveSound.GetBusSends(InBusSendType, CurrentSends);
				for (const FSoundSourceBusSendInfo& Existing : CurrentSends)
				{
					const bool bSameSourceBus = InfoToSend.SoundSourceBus && (InfoToSend.SoundSourceBus == Existing.SoundSourceBus);
					const bool bSameAudioBus  = InfoToSend.AudioBus && (InfoToSend.AudioBus == Existing.AudioBus);
					if (bSameSourceBus || bSameAudioBus)
					{
						InfoToSend.bEnableLPFCutoff = Existing.bEnableLPFCutoff;
						InfoToSend.LPFCutoff = Existing.LPFCutoff;
						InfoToSend.bEnableHPFCutoff = Existing.bEnableHPFCutoff;
						InfoToSend.HPFCutoff = Existing.HPFCutoff;
						break;
					}
				}
				ActiveSound.SetSourceBusSend(InBusSendType, InfoToSend);
...
void UAudioComponent::SetSourceBusSendPostAttenuation(USoundSourceBus* SoundSourceBus, float SourceBusSendLevel)
{
	SetBusSendEffectInternal(SoundSourceBus, nullptr, SourceBusSendLevel, EBusSendType::PostAttenuation);
}
...
void UAudioComponent::SetAudioBusSendPostAttenuation(UAudioBus* AudioBus, float AudioBusSendLevel)
{
	SetBusSendEffectInternal(nullptr, AudioBus, AudioBusSendLevel, EBusSendType::PostAttenuation);
}
...
{
	struct FModulatorWeakRoutingAccessor
	{
	private:
		TArray<TWeakObjectPtr<UObject>> WeakCollection;
		const FSoundModulationDefaultRoutingSettings Settings;

	public:
		FModulatorWeakRoutingAccessor(const FSoundModulationDefaultRoutingSettings& RoutingSettings)
			: Settings(RoutingSettings)
		{
			auto GatherModulators = [&](const TSet<TObjectPtr<USoundModulatorBase>>& InModulators)
			{
				for (const TObjectPtr<USoundModulatorBase>& Modulator : InModulators)
				{
					if (USoundModulatorBase* ModPtr = Modulator.Get())
					{
						WeakCollection.Add(TWeakObjectPtr<UObject>(ModPtr));
					}
				}
			};

			GatherModulators(RoutingSettings.VolumeModulationDestination.Modulators);
			GatherModulators(RoutingSettings.PitchModulationDestination.Modulators);
			GatherModulators(RoutingSettings.HighpassModulationDestination.Modulators);
			GatherModulators(RoutingSettings.LowpassModulationDestination.Modulators);
		}

		const FSoundModulationDefaultRoutingSettings* GetSafe() const
		{
			auto IsValid = [](const TWeakObjectPtr<UObject>& WeakPtr) { return WeakPtr.IsValid(); };
			if (Algo::AllOf(WeakCollection, IsValid))
			{
				return &Settings;
			}
	...
	AudioDevice->SendCommandToActiveSounds(AudioComponentID, [NewRouting = FModulatorWeakRoutingAccessor(ModulationRouting)](FActiveSound& ActiveSound)
	{
		if (const FSoundModulationDefaultRoutingSettings* SafeSettings = NewRouting.GetSafe())
		{
			ActiveSound.SetNewModulationRouting(*SafeSettings);
		}
	});
...
{
	using namespace AudioComponentPrivate;
	...
	// Tell the active sounds on the component to use the new Modulation Routing
	AudioDevice->SendCommandToActiveSounds(AudioComponentID, [Accessor = FModulatorWeakAccessor(ConvertedModulators), Destination](FActiveSound& ActiveSound)
	{
		TSet<TObjectPtr<USoundModulatorBase>> NewModulators;
		if (Accessor.TryGetSafe(NewModulators))
		{
...
void UAudioComponent::RemoveModulationRouting(const TSet<USoundModulatorBase*>& Modulators, const EModulationDestination Destination)
{
	...
	// Tell the active sounds on the component to remove the given modulators
	AudioDevice->SendCommandToActiveSounds(AudioComponentID, [Accessor = FModulatorWeakAccessor(ConvertedModulators), Destination](FActiveSound& ActiveSound)
	{
		TSet<TObjectPtr<USoundModulatorBase>> ModulatorsToRemove;
		if (Accessor.TryGetSafe(ModulatorsToRemove))
		{
			ActiveSound.RemoveModulationRouting(ModulatorsToRemove, Destination);
		}
	});
}
