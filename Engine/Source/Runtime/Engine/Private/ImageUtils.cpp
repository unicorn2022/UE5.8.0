...
static IImageWrapperModule * GetOrLoadImageWrapperModule()
{
	...
	else
	{
		...
		{
			UE_LOGF(LogImageUtils,Error,"Not on GameThread, cannot load ImageWrapper.  Do on main thread in startup.");
...
{
	TArray64<uint8> Buffer;
	if ( ! FFileHelper::LoadFileToArray(Buffer, Filename, FILEREAD_Silent) )
	{
		return false;
	}
...
bool FImageUtils::ExportTextureSourceToDDS(TArray64<uint8> & OutData, UTexture * Texture, int BlockIndex, int LayerIndex)
{
	...
	if ( ! Source.IsValid() )
	{
		UE_LOGF(LogImageUtils,Warning,"ExportTextureSourceToDDS: Texture does not have source (%ls)",*Texture->GetName());
		return false;
	}
	...
	if ( RawFormat == ERawImageFormat::BGRE8 )
	{
		...
		UE_LOGF(LogImageUtils,Warning,"DDS export to BGRE8 will write the bytes as they were in BGRA, not converted to float linear");
	}

	UE_LOGF(LogImageUtils,Display,"Exporting DDS Dimension=%d SizeX=%d SizeY=%d SizeZ=%d NumMips=%d ArraySize=%d",Dimension, SizeX,SizeY,SizeZ,NumMips,ArraySize);	
	...
	{
		UE_LOGF(LogImageUtils,Warning,"FDDSFile::CreateEmpty (Error=%d)",(int)Error);			
		return false;
	}
	...
	{
		UE_LOGF(LogImageUtils,Warning,"FDDSFile::WriteDDS failed (Error=%d)",(int)Error);			
		return false;
	}
...
bool FImageUtils::ExportRenderTargetToDDS(TArray64<uint8> & OutData, UTextureRenderTarget * TexRT)
{
	...
	{
		UE_LOGF(LogImageUtils,Warning,"ExportRenderTargetToDDS : GetRenderTargetImage failed");
		return false;
	}
...
UTexture2D* FImageUtils::CreateTexture2D(int32 SrcWidth, int32 SrcHeight, const TArray<FColor> &SrcData, UObject* Outer, const FString& Name, const EObjectFlags &Flags, const FCreateTexture2DParameters& InParams)
{
	...
	return Tex2D;
#else
	UE_LOGF(LogImageUtils, Fatal,"CreateTexture2D not supported without WITH_EDITOR.");
	return nullptr;
#endif
}
...
UTexture * FImageUtils::CreateTexture(ETextureClass TextureClass, const FImageView & Image, UObject* Outer, const FString& Name, EObjectFlags Flags, bool DoPostEditChange )
{
	...
	case ETextureClass::Volume:
		...
		break;
	default:
		UE_LOGF(LogImageUtils, Fatal,"CreateTexture invalid TextureClass.");
		return nullptr;
	}
	...
	return Tex;
#else
	UE_LOGF(LogImageUtils, Fatal,"CreateTexture not supported without WITH_EDITOR.");
	return nullptr;
#endif
}
...
{
	UE_LOGF(LogImageUtils, Warning, "HDR file format is very lossy, use EXR or PNG instead." );
...
bool FImageUtils::GetTexture2DSourceImage(UTexture2D* Texture, FImage & OutImage)
{
	...
	UE_LOGF(LogImageUtils,Warning,"GetTexture2DSourceImage from PlatformData not implemented yet");
...
UTexture2D* FImageUtils::ImportFileAsTexture2D(const FString& Filename)
{
	...
	if (FFileHelper::LoadFileToArray(Buffer, *Filename, FILEREAD_Silent))
	{
		...
		if(!NewTexture)
		{
			UE_LOGF(LogImageUtils, Warning, "Error creating texture. %ls is not a supported file format", *Filename)
		}	
	}
	else
	{
		UE_LOGF(LogImageUtils, Warning, "Error creating texture. %ls could not be found", *Filename)
	}
...
UTexture2D* FImageUtils::ImportBufferAsTexture2D(TArrayView64<const uint8> Buffer)
{
	...
	{
		UE_LOGF(LogImageUtils, Warning, "Error creating texture. Couldn't determine the file format");
		return nullptr;
	}
...
UTexture2D* FImageUtils::CreateTexture2DFromImage(const FImageView & Image)
{
	...
	{
		UE_LOGF(LogImageUtils, Warning, "Error in CreateTransient");
		return nullptr;
	}
