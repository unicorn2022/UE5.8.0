...
bool FSkeletalMeshMerge::FinalizeMesh()
{
	...
	if (MaxNumLODs == -1)
	{
		UE_LOGF(LogSkeletalMesh, Warning, "FSkeletalMeshMerge: Invalid source mesh list");
		return false;
	}
	...
	if (Result)
	{
		...
		// Initialize the render data's unified bone map from the LOD storage.
		FSkeletalMeshRenderData* MergeResource = MergeMesh->GetResourceForRendering();
		if (MergeResource)
		{
			MergeResource->InitUnifiedBoneMapFromLODs();
		}
...
void FSkeletalMeshMerge::GenerateLODModel( int32 LODIdx )
{
	...
	// If any source mesh uses unified bone indices, unify the merged output's bone maps too.
	// This ensures the merged mesh is compatible with GPU instance LOD and single-pass skinning.
	{
		bool bAnySourceHasUnifiedBoneMap = false;
		for (USkeletalMesh* SrcMesh : SrcMeshList)
		{
			if (SrcMesh && SrcMesh->GetResourceForRendering() && SrcMesh->GetResourceForRendering()->HasUnifiedBoneMap())
			{
				bAnySourceHasUnifiedBoneMap = true;
				break;
			}
		}

		if (bAnySourceHasUnifiedBoneMap)
		{
			// Build unified bone map from the union of all merged section bone maps.
			TArray<FBoneIndexType> UnifiedBoneMap;
			for (const FSkelMeshRenderSection& Section : MergeLODData.RenderSections)
			{
				UnifiedBoneMap.Append(Section.BoneMap);
			}
			UnifiedBoneMap.Sort();
			UnifiedBoneMap.SetNum(Algo::Unique(UnifiedBoneMap));

			// Remap each section's vertex bone indices from per-section bone map space to unified space,
			// then overwrite the section's bone map with the unified map.
			for (FSkelMeshRenderSection& Section : MergeLODData.RenderSections)
			{
				// Build remap: OldBoneMapIndex -> UnifiedBoneMapIndex
				TArray<uint32> RemapTable;
				RemapTable.SetNumUninitialized(Section.BoneMap.Num());
				for (int32 OldIndex = 0; OldIndex < Section.BoneMap.Num(); ++OldIndex)
				{
					const int32 UnifiedIndex = UnifiedBoneMap.Find(Section.BoneMap[OldIndex]);
					check(UnifiedIndex != INDEX_NONE);
					RemapTable[OldIndex] = (uint32)UnifiedIndex;
				}

				// Remap vertex bone indices in the merged vertex buffer for this section's vertices.
				for (uint32 VertIdx = 0; VertIdx < Section.NumVertices; ++VertIdx)
				{
					FSkinWeightInfo& Weight = MergedSkinWeightBuffer[Section.BaseVertexIndex + VertIdx];

					for (int32 InfluenceIndex = 0; InfluenceIndex < MAX_TOTAL_INFLUENCES; ++InfluenceIndex)
					{
						if (Weight.InfluenceWeights[InfluenceIndex] > 0)
						{
							Weight.InfluenceBones[InfluenceIndex] = (FBoneIndexType)RemapTable[Weight.InfluenceBones[InfluenceIndex]];
						}
					}
				}

				Section.BoneMap = UnifiedBoneMap;
				Section.bUnifiedBoneMap = true;
			}

			// Set the unified bone map on the LOD render data (points into the first section's BoneMap storage).
			if (!MergeLODData.RenderSections.IsEmpty())
			{
				MergeLODData.UnifiedBoneMap = MergeLODData.RenderSections[0].BoneMap;
			}
		}
	}
