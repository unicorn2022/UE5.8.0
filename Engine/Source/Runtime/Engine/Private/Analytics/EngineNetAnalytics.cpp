...
void FNetConnAnalyticsData::SendAnalytics()
{
	...
	if (!(*this == NullVars) && AnalyticsProvider.IsValid())
	{
		...
		UE_LOGF(LogNet, Log, "NetConnection Analytics:");

		UE_LOGF(LogNet, Log, " - OutAckOnlyCount: %llu", OutAckOnlyCount);
		UE_LOGF(LogNet, Log, " - OutKeepAliveCount: %llu", OutKeepAliveCount);
		UE_LOGF(LogNet, Log, " - OutOfOrderPacketsLostCount: %llu", OutOfOrderPacketsLostCount);
		UE_LOGF(LogNet, Log, " - OutOfOrderPacketsRecoveredCount: %llu", OutOfOrderPacketsRecoveredCount);
		UE_LOGF(LogNet, Log, " - OutOfOrderPacketsDuplicateCount: %llu", OutOfOrderPacketsDuplicateCount);


		if (HeadCloseReasons.Num() > 0)
		{
			UE_LOGF(LogNet, Log, " - CloseReasons:");

			for (const FReasonCounter& CurCounter : HeadCloseReasons)
			{
				UE_LOGF(LogNet, Log, "  - %ls: %i", ToCStr(CurCounter.ReasonStr), CurCounter.Counter);
			}
		}

		if (FullCloseReasons.Num() > 0)
		{
			UE_LOGF(LogNet, Log, " - FullCloseReasons:");
		...
		{
			UE_LOGF(LogNet, Log, " - ClientCloseReasons:");
		...
		{
			UE_LOGF(LogNet, Log, " - FullClientCloseReasons:");
		...
		{
			UE_LOGF(LogNet, Log, " - RecoveredFaults:");

			for (TMap<FString, int32>::TConstIterator It(RecoveredFaults); It; ++It)
			{
				UE_LOGF(LogNet, Log, "  - %ls: %i", ToCStr(It.Key()), It.Value());
			}
		}

		if (ProcessedFailedPingAddressesICMP.Num() > 0)
		{
			UE_LOGF(LogNet, Log, " - FailedPingAddressesICMP:");

			for (const FFailedPingCounter& CurCounter : ProcessedFailedPingAddressesICMP)
			{
				UE_LOGF(LogNet, Log, "  - %ls: %i", ToCStr(CurCounter.PingAddress), CurCounter.Counter);
			}
		}

		if (ProcessedFailedPingAddressesUDP.Num() > 0)
		{
			UE_LOGF(LogNet, Log, " - FailedPingAddressesUDP:");
		...
		static const FString EZEventName = "Core.ServerNetConn";
		static const FString EZAttrib_OutAckOnlyCount = "OutAckOnlyCount";
		static const FString EZAttrib_OutKeepAliveCount = "OutKeepAliveCount";
		static const FString EZAttrib_OutOfOrderPacketsLostCount = "OutOfOrderPacketsLostCount";
		static const FString EZAttrib_OutOfOrderPacketsRecoveredCount = "OutOfOrderPacketsRecoveredCount";
		static const FString EZAttrib_OutOfOrderPacketsDuplicateCount = "OutOfOrderPacketsDuplicateCount";
		static const FString EZAttrib_CloseReasons = "CloseReasons";
		static const FString EZAttrib_FullCloseReasons = "FullCloseReasons";
		static const FString EZAttrib_ClientCloseReasons = "ClientCloseReasons";
		static const FString EZAttrib_FullClientCloseReasons = "FullClientCloseReasons";
		static const FString EZAttrib_RecoveredFaults = "RecoveredFaults";
		static const FString EZAttrib_FailedPingICMP = "FailedPingICMP";
		static const FString EZAttrib_FailedPingUDP = "FailedPingUDP";
		static const FString EZAttrib_Reason = "Reason";
		static const FString EZAttrib_Count = "Count";
		static const FString EZAttrib_IP = "IP";
