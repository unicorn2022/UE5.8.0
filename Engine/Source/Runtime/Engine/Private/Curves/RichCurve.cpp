...
void FeaturePointMethod(T& Function , float StartTime,  float EndTime, float StartValue,float Mu, int Depth, int MaxDepth, float& MaxV, float& MinVal)
{
	...
	// Make sure to try and keep our iteration floats within a similar range to avoid incrementing a large float with a small one leading
	// to an infinite loop
	const float RelativeEndTime = EndTime - StartTime;
	for (float f = Mu; f < RelativeEndTime; f += Mu)
	{
		const float CurrentTime = StartTime + f;
		float Value		 = Function.Eval(CurrentTime);
		...
		if (FMath::Sign(CurTangent) != FMath::Sign(PrevTangent))
		{
			...
			float FeaturePointTime = CurrentTime-Mu*2.0f;
			FeaturePointMethod(Function, FeaturePointTime, CurrentTime, Function.Eval(FeaturePointTime), Mu*0.4f,Depth+1, MaxDepth, MaxV, MinVal);
		}
...
FORCEINLINE_DEBUGGABLE static float InterpEval(float InTime, const KeyTimeAdapterType& KeyTimeAdapter, const KeyDataAdapterType& KeyDataAdapter, int32 NumKeys, ERichCurveExtrapolation PreInfinityExtrap, ERichCurveExtrapolation PostInfinityExtrap)
{
	...
	if (Diff > 0.0f)
	{
		...
		if (KeyInterpMode0 != RCCF_Constant)
        {
        ...
        {
			InterpolatedValue = InTime < KeyTime1 ? KeyValue0 : KeyValue1;
        }
	}
	else
	{
