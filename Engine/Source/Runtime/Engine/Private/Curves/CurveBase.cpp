...
TArray<FString> UCurveBase::CreateCurveFromCSVString(const FString& InString)
{
	...
	if (Rows.Num() == 0)
	{
		OutProblems.Add(FString("No data."));
		return OutProblems;
	}
...
void UCurveBase::ImportFromJSONString(const FString& InString, TArray<FString>& OutProblems)
{
	...
	if (!FJsonObjectConverter::UStructToJsonAttributes(GetClass(), this, BackupJSONObject))
	{
...
FString UCurveBase::ExportAsJSONString() const
{
	...
	if (FJsonObjectConverter::UStructToJsonAttributes(GetClass(), this, JSONObject, 0, CPF_Deprecated | CPF_Transient))
	{
