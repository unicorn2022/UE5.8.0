...
public:
		...
		, ProxiedResource(nullptr)
	{
		...
		for (int32 MipIndex = 0; MipIndex <= FirstMipTailIndex; MipIndex++)
		{
			...
			{
				UE_LOGF(LogTexture, Error, "Corrupt texture [%ls]! Missing bulk data for MipIndex=%d", *InOwner->GetFullName(), MipIndex);
			}
			else
			{
	...
	virtual void InitRHI(FRHICommandListBase& RHICmdList) override
	{
		...
		for (int32 MipIndex = 0; MipIndex <= FirstMipTailIndex; MipIndex++)
		{
			for (uint32 ArrayIndex = 0u; ArrayIndex < ArraySize; ++ArrayIndex)
			{
				...
				{
					uint32 DestStride;
					void* TheMipData = RHICmdList.GetAsImmediate().LockTextureCubeFace(TextureCubeRHI, FaceIndex, ArrayIndex, MipIndex, RLM_WriteOnly, DestStride, false);
					GetData(ArrayIndex, FaceIndex, MipIndex, TheMipData, DestStride);
					RHICmdList.GetAsImmediate().UnlockTextureCubeFace(TextureCubeRHI, FaceIndex, ArrayIndex, MipIndex, false);
				}
			}
		}
...
UTextureCubeArray* UTextureCubeArray::CreateTransient(int32 InSizeX, int32 InSizeY, int32 InArraySize, EPixelFormat InFormat, const FName InName)
{
	...
	{
		UE_LOGF(LogTexture, Warning, "Invalid parameters specified for UTextureCubeArray::CreateTransient()");
	}
	return NewTexture;
}
...
const FTexturePlatformData* UTextureCubeArray::GetPlatformData() const
{
	...
	if (PrivatePlatformData && !PrivatePlatformData->IsAsyncWorkComplete())
	{
		...
		UE_LOGF(LogTexture, Log, "Call to GetPlatformData() is forcing a wait on data that is not yet ready.");
...
FTextureResource* UTextureCubeArray::CreateResource()
{
	...
	{
		UE_LOGF(LogTexture, Warning, "%ls contains no miplevels! Please delete.", *GetFullName());
	}
	else if (!FormatInfo.Supported)
	{
		UE_LOGF(LogTexture, Warning, "%ls cannot be created, rhi does not support format %ls.", *GetFullName(), FormatInfo.Name);
	}
	return nullptr;
}
...
ENGINE_API bool UTextureCubeArray::CheckArrayTexturesCompatibility()
{
	...
	for (int32 TextureCmpIndex =1; TextureCmpIndex < SourceTextures.Num(); ++TextureCmpIndex)
	{
		...
		{
			UE_LOGF(LogTexture, Warning, "TextureCubeArray creation failed. Textures %ls and %ls have different sizes.", *TextureName, *TextureNameCmp);
			bError = true;
		}

		if (TextureSourceCmp.GetNumSlices() != NumSlices)
		{
			UE_LOGF(LogTexture, Warning, "TextureCubeArray creation failed. Textures %ls and %ls have different number of faces (some are long/lat, some are not).", *TextureName, *TextureNameCmp);
			bError = true;
		}
	}
...
ENGINE_API bool UTextureCubeArray::CheckLastTextureCompatibility()
{
	// If no last texture exists, return true -- can't have a compatibility mismatch if nothing is there.
	if (!SourceTextures.Num() || !SourceTextures.Last())
	{
		return true;
	}

	// Reject an explicitly invalid last texture
	const UTextureCube* LastTexture = SourceTextures.Last();
	if (!LastTexture->Source.IsValid())
	{
		return false;
	}
	const FTextureSource& LastSource = LastTexture->Source;

	// Check if the last matches the first valid
	for (UTextureCube* SourceTexture : SourceTextures)
	{
		if (SourceTexture && SourceTexture->Source.IsValid())
		{
			const FTextureSource& FirstValidSource = SourceTexture->Source;
			return
				FirstValidSource.GetSizeX() == LastSource.GetSizeX() &&
				FirstValidSource.GetSizeY() == LastSource.GetSizeY() &&
				FirstValidSource.GetNumSlices() == LastSource.GetNumSlices();
		}
	}
...
ENGINE_API bool UTextureCubeArray::UpdateSourceFromSourceTextures(bool bCreatingNewTexture)
{
	...
	{
		// Pull settings from the first valid texture.  We do this before CheckArrayTexturesCompatibility, to support
		// cases where the array is initialized with multiple initially null elements, and the first non-null element
		// is set (CheckArrayTexturesCompatibility will return false if there are any null elements).
		for (UTextureCube* SourceTexture : SourceTextures)
		{
			if (SourceTexture && SourceTexture->Source.IsValid())
			{
				CompressionSettings = SourceTexture->CompressionSettings;
				MipGenSettings = TMGS_NoMipmaps;
				PowerOfTwoMode = ETexturePowerOfTwoSetting::None;
				LODGroup = SourceTexture->LODGroup;
				NeverStream = true;
				break;
			}
		}
	...
	// Detect format and gamma mismatches across source textures, finding a common format that can represent all
	// sources without information loss (ported from UTexture2DArray::UpdateSourceFromSourceTextures).
	ETextureSourceFormat Format = InitialSource.GetFormat();
	bool bSRGB = InitialSource.GetGammaSpace(0) == EGammaSpace::sRGB;
	...
	{
		const FTextureSource& OtherSource = SourceTextures[SourceTextureIndex]->Source;

		if ((OtherSource.GetGammaSpace(0) == EGammaSpace::sRGB) != bSRGB)
		{
		...
		if (Format != OtherSource.GetFormat())
		{
			bMismatchedFormats = true;
			Format = FImageCoreUtils::GetCommonSourceFormat(Format, OtherSource.GetFormat());
		}
	}

	if (bMismatchedGammaSpace)
	{
		UE_LOGF(LogTexture, Log, "TextureCubeArray: mismatched source gamma spaces, converting all to RGBA32F/Linear ...");
	}
	else if (bMismatchedFormats)
	{
		UE_LOGF(LogTexture, Log, "TextureCubeArray: mismatched source pixel formats, converting all to %ls/%s ...",
			ERawImageFormat::GetName(FImageCoreUtils::ConvertToRawImageFormat(Format)), bSRGB ? "sRGB" : "Linear");
	}
	...
	Source.Init(SizeX, SizeY, ArraySize * NumSlices, 1, Format);
	...
	const int64 MipSizePerElement = Source.CalcMipSize(0) / ArraySize;
	...
	for (int32 SourceTexIndex = 0; SourceTexIndex < SourceTextures.Num(); ++SourceTexIndex)
	{
		...
		check( TextureSource.IsValid() ); // checked by CheckArrayTexturesCompatibility
		void* DestSliceData = DestMipData + MipSizePerElement * SourceTexIndex;

		if (TextureSource.GetFormat() == Format && ((TextureSource.GetGammaSpace(0) == EGammaSpace::sRGB) == bSRGB))
		{
			// Fast path: format and gamma match, direct copy.
			TArray64<uint8> RefCubeData;
			TextureSource.GetMipData(RefCubeData, 0);
			check(RefCubeData.Num() == MipSizePerElement);
			FMemory::Memcpy(DestSliceData, RefCubeData.GetData(), MipSizePerElement);
		}
		else
		{
			// Slow path: convert format/gamma via FImage.
			FImage SourceImage;
			TextureSource.GetMipImage(SourceImage, 0);

			if (TextureSource.GetFormat() != Format)
			{
				// When gamma space is mismatched we fall back to TSF_RGBA32F which is always linear,
				// so no separate gamma conversion is needed in that case.
				FImage ConvertedImage;
				SourceImage.CopyTo(ConvertedImage, FImageCoreUtils::ConvertToRawImageFormat(Format), bSRGB ? EGammaSpace::sRGB : EGammaSpace::Linear);
				ConvertedImage.Swap(SourceImage);
			}

			check(SourceImage.RawData.Num() == MipSizePerElement);
			FMemory::Memcpy(DestSliceData, SourceImage.RawData.GetData(), MipSizePerElement);
		}
	}
...
void UTextureCubeArray::GetAssetRegistryTags(FAssetRegistryTagsContext Context) const
{
	...
	// Our sources may be LongLat cubemaps which are single textures
	bool bSourceIsLongLat = Source.IsLongLatCubemap();
	const TCHAR* SourceFaceCount = bSourceIsLongLat ? TEXT("") : TEXT("x6"); 
	int32 ArraySize = bSourceIsLongLat ? Source.GetNumSlices() : Source.GetNumSlices() / 6;
#else
	int32 SizeX = 0;
	int32 SizeY = 0;
	const TCHAR* SourceFaceCount = TEXT(""); 
	int32 ArraySize = 0;
#endif
	const FString Dimensions = FString::Printf(TEXT("%dx%d%s*%d"), SizeX, SizeY, SourceFaceCount, ArraySize);
	...
	if (PropertyName == GET_MEMBER_NAME_CHECKED(UTextureCubeArray, SourceTextures))
	{
		...
		for (int i=0; i < SourceTextures.Num(); i++)
		{
			...
			if (SourceTexture)
			{
				...
				{
					UE_LOGF(LogTexture, Warning, "Texture has no Source, cannot be used [%ls]", *SourceTexture->GetFullName());
		...
		// First valid entry into an empty texture array -- indicate to UpdateSourceFromSourceTextures that we are creating a new texture.
		else if (NumValid == 1)
		{
			// Except in the rare situation where an item was removed for being invalid above, don't treat that as creating a new texture.
			const bool bCreatingNewTexture = NumRemoved == 0;
			UpdateSourceFromSourceTextures(bCreatingNewTexture);
		}
		// Try to update from all source textures.
		else if (UpdateSourceFromSourceTextures(false) == false)
		{
			// If this is a change to the last element, and it's invalid, remove it to make the array valid.  Arrays don't support calling
			// OnShouldFilterAsset on child properties of the array, so this is a backstop to clean up such cases on newly added elements.
			// Invalid edits to an existing last element should be blocked by the filters, so this case should only be reached on an add.
			int32 ChangedIndex = PropertyChangedEvent.GetArrayIndex(PropertyName.ToString());
			int32 LastIndex = SourceTextures.Num() - 1;

			if (ChangedIndex == LastIndex && !CheckLastTextureCompatibility())
			{
