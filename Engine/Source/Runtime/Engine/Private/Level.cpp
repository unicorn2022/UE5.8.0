...
bool GUnregisterTickFunctionsOnClearLevelComponents = true;
static FAutoConsoleVariableRef CVarUnregisterTickFunctionsOnClearLevelComponents(
	TEXT("s.UnregisterTickFunctionsOnClearLevelComponents"),
	GUnregisterTickFunctionsOnClearLevelComponents,
	TEXT("Whether to also unregister actor tick functions when clearing level components"));
...
void FActorFolderSet::Add(UActorFolder* InActorFolder)
{
	...
	if (!IsEmpty() && !GIsFixingActorFolders)
	{
		...
		for (const TObjectPtr<UActorFolder>& Folder : ActorFolders)
		{
			...
			{
				UE_LOGF(LogLevel, Error, "Adding duplicate actor folder %ls with path %ls.", *InActorFolder->GetName(), *AddedFolderPath.ToString());
				break;
			}
		}
	}
	...
	auto ProcessPackage = [&ProcessedPackages, bTrashPackage](UPackage* InPackage, bool bInClearStandaloneFlag = false)
	{
		...
		if (!bWasAlreadyInSet)
		{
			...
			if (bInClearStandaloneFlag || bTrashPackage)
			{
				...
				ForEachObjectWithPackage(InPackage, [](UObject* Object) { Object->ClearFlags(RF_Standalone); return true; }, EGetObjectsFlags::None);
			}
#endif
...
void ULevel::PostInitProperties()
{
	...
	{
		Compatibility->AddCompatibleObject(this);
	}
#endif // WITH_EDITOR
...
void ULevel::AddLoadedActors(const TArray<AActor*>& ActorList, const FTransform* TransformToApply)
{
	...
	FScopedSlowTask SlowTask(ActorsQueue.Num() * 2 + (bAreComponentsCurrentlyRegistered ? ActorsQueue.Num() : 0), LOCTEXT("RegisteringActors", "Registering actors..."));
...
void ULevel::PostLoad()
{
	...
	if (!GetOutermost()->HasAllPackagesFlags(PKG_Cooked))
	{
			...
			&& OwningWorld->WorldType == EWorldType::Editor)
		{
			if (Linker && Linker->IsPackageRelocated())
			{
				...
				UE_LOGF(LogLevel, Error, "The level %ls was moved on disk without the editor knowing it. This might cause some loading issues and some saving issue like an actor potentialy stomping the file of another actor.", *LevelPackageName);
		...
		if (IsUsingExternalActors())
		{
				...
				!Linker->GetInstancingContext().HasTag(ULevel::DontLoadExternalObjectsTag))
			{
				...
				for (int32 i=0; i < ActorPackageNames.Num(); i++)
				{
					...
					UE_CLOGF(!bFoundActor, LogLevel, Error, "Failed to load Actor for External Actor Package %ls", *ActorPackageName);
				}
			}
		}
	}
	...
	if (!GetOutermost()->HasAnyPackageFlags(PKG_PlayInEditor))
	{
		...
		if (LevelScriptBlueprint && OuterWorld && LevelScriptBlueprint->GetFName() != OuterWorld->GetFName())
		{
			...
			LevelScriptBlueprint->Rename(*OuterWorld->GetName(), LevelScriptBlueprint->GetOuter(), REN_DoNotDirty | REN_DontCreateRedirectors | REN_NonTransactional | REN_SkipGeneratedClasses | REN_AllowPackageLinkerMismatch);
		}
	}
...
void ULevel::ClearLevelComponents()
{
	...
	for (int32 ActorIndex = 0; ActorIndex < Actors.Num(); ++ActorIndex)
	{
		...
		if (Actor)
		{
			...
			if(GUnregisterTickFunctionsOnClearLevelComponents)
			{
				// Match tick function unregistration in IncrementalUnregisterComponents to ensure the same behaviour
				// when incremental unregister is not permitted.
...
void ULevel::BeginDestroy()
{
	...
	if (ICompatibilityProvider* Compatibility = GetMutableDataStorageFeature<ICompatibilityProvider>(CompatibilityFeatureName))
	{
		Compatibility->RemoveCompatibleObject(this);
	}
#endif // WITH_EDITOR
...
static void SortActorsHierarchy(TArray<TObjectPtr<AActor>>& Actors, ULevel* Level)
{
	...
	TFunction<int32(AActor*)> CalcAttachDepth = [&DepthMap, &VisitedActors, &CalcAttachDepth, &bFoundWorldSettings, DefaultBrush](AActor* Actor)
	{
		...
		else
		{
			...
			if (Actor->IsA<AWorldSettings>())
			{
				...
				{
					UE_LOGF(LogLevel, Warning, "Detected duplicate WorldSettings actor - UE-62934");
				}
			}
			...
			else if (AActor* ParentActor = Actor->GetAttachParentActor())
			{
				if (VisitedActors.Contains(ParentActor))
				{
					...
					UE_LOGF(LogLevel, Warning, "Found loop in attachment hierarchy: %ls", *VisitedActorLoop);
	...
	{
		UE_LOGF(LogLevel, Warning, "SortActorsHierarchy(%ls) took %f seconds (CalcAttachDepth: %f StableSort: %f)", Level ? *GetNameSafe(Level->GetOutermost()) : TEXT("??"), ElapsedTime, CalcAttachDepthTime, StableSortTime);
	}
...
{
	const bool bWaitForCompletion = Context.GetIncrementalRemoveTimeout().WillNeverExpire();

	auto ProcessPreUnregisteringComponents = [this, bWaitForCompletion]()
	{
...
bool ULevel::IncrementalUnregisterComponents(int32 NumComponentsToUnregister, FUnregisterComponentContext* InContext)
{
	...
	auto IncrementalUnregisterComponentsInternal = [this, NumComponentsToUnregister, bIncrementalUnregisterComponents]()
	{
		...
		while (CurrentActorIndexForUnregisterComponents < Actors.Num())
		{
			...
			CurrentActorIndexForUnregisterComponents++;
			if (bIncrementalUnregisterComponents && NumComponentsUnregistered > NumComponentsToUnregister)
			{
				break;
			}
		}
	...
	if (!bIncrementalUnregisterComponents)
	{
		Context.SetIncrementalRemoveTimeout(UE::FTimeout::Never());
	}
...
uint16 ULevel::RegisterStreamableTexture(UTexture* InTexture)
{
	if (InTexture->LevelIndex == INDEX_NONE)
	{
		...
		if (ExistingTextureIndex != INDEX_NONE)
		{
			...
			UE_LOGF(LogLevel, Warning, "Another streamable texture %ls was already registered with the same guid. Texture %ls needs to be resaved.", *StreamingTextures[ExistingTextureIndex].ToString(), *InTexture->GetPathName());
			InTexture->Modify();
...
uint16 ULevel::RegisterStreamableTexture(const FString& InTextureName, const FGuid& InTextureGuid)
{
	...
	{
		UE_CLOGF(TextureNameIndex != INDEX_NONE, LogLevel, Warning, "Failed to register streamable texture Name = %ls Guid = %ls: An entry already exists for this Name with a different Guid = %ls. Consider rebuilding texture streaming.", *InTextureName, *InTextureGuid.ToString(), *StreamingTextureGuids[TextureNameIndex].ToString());
		UE_CLOGF(TextureGuidIndex != INDEX_NONE, LogLevel, Warning, "Failed to register streamable texture Name = %ls Guid = %ls: An entry already exists for this Guid with a different Name = %ls. Consider modifying & resaving one of these textures (will change its guid) and rebuilding texture streaming.", *InTextureName, *InTextureGuid.ToString(), *StreamingTextures[TextureGuidIndex].ToString());
...
void ULevel::PostEditUndo()
{
	...
	}, EGetObjectsFlags::None, /*ExclusionFlags*/ RF_NoFlags, /* InternalExclusionFlags */ EInternalObjectFlags::Garbage);
...
FName ULevel::GetCookPackageName() const
{
	UWorldPartitionRuntimeCell* Cell = WorldPartitionRuntimeCell.Get();
	return Cell ? Cell->GetCookPackageName() : NAME_None;
}
...
bool ULevel::GetIsLevelPartitionedFromAsset(const FAssetData& Asset)
{
	...
	static const FName NAME_LevelIsPartitioned("LevelIsPartitioned");
...
bool ULevel::GetIsStreamingDisabledFromAsset(const FAssetData& Asset)
{
	...
	static const FName NAME_LevelHasStreamingDisabled("LevelHasStreamingDisabled");
...
bool ULevel::GetWorldExternalActorsReferencesFromAsset(const FAssetData& Asset, TArray<FGuid>& OutWorldExternalActorsReferences)
{
	...
	static const FName NAME_WorldExternalActorsReferences("WorldExternalActorsReferences");
	...
	if (IsUsingActorFolders())
	{
		...
		if (bFixDuplicateFolders)
		{
			...
			for (UActorFolder* FolderToDelete : SortedFoldersToDelete)
			{
				...
				UE_LOGF(LogLevel, Log, "Merging duplicate actor folder %ls.", *OldFolder.GetPath().ToString());
...
void ULevel::OnLevelLoaded()
{
	...
	// 1. Cook commandlet does its own UWorldPartition::Initialize call in FWorldPartitionCookPackageSplitter::GetGenerateList
	...
	if (!IsRunningCookCommandlet() && OwningWorld->HasSubsystem<UWorldPartitionSubsystem>())
	{
		if (UWorldPartition* WorldPartition = GetWorldPartition())
		{
			...
			UE_LOGF(LogWorldPartition, Log, "ULevel::OnLevelLoaded(%ls)(bIsOwningWorldGameWorld=%d, bIsOwningWorldPartitioned=%d, InitializeForMainWorld=%d, InitializeForEditor=%d, InitializeForGame=%d)", 
...
void ULevel::BuildStreamingData(UWorld* World, ULevel* TargetLevel/*=NULL*/, UTexture2D* UpdateSpecificTextureOnly/*=NULL*/)
{
	...
	UE_LOGF(LogLevel, Verbose, "ULevel::BuildStreamingData took %.3f seconds.", FPlatformTime::Seconds() - StartTime);
#else
	UE_LOGF(LogLevel, Fatal,"ULevel::BuildStreamingData should not be called on a console");
#endif
}
...
bool ULevel::SetUseActorFolders(bool bInEnabled, bool bInInteractiveMode)
{
	...
	if (!bInEnabled)
	{
		UE_LOGF(LogLevel, Warning, "Disabling actor folder objects is not supported.");
		if (bInteractiveMode)
		{
	...
	if (GetWorldPartition())
	{
		...
		if (!bIsRunningWorldPartitionBuilderCommandlet)
		{
			...
			UE_LOGF(LogLevel, Warning, "To enable actor folder objects, run WorldPartitionBuilderCommandlet with these options: '-Builder=WorldPartitionResaveActorsBuilder -EnableActorFolders'");
			return false;
		}
	}
		...
		|| !FPackageName::IsValidLongPackageName(LevelPackage->GetName()))
	{
		...
		UE_LOGF(LogLevel, Warning, "You need to save the level before enabling the `Use Actor Folder Objects` option.");
		return false;
	}
...
void ULevel::RepairLevelScript()
{
		...
		OwningWorld && OwningWorld->WorldType == EWorldType::Editor)
	{
		...
		{
			UE_LOGF(LogLevel, Error, "Detected more than one LevelScriptActor in map '%ls'. This can lead to duplicate level blueprint operations during play.", *GetPathName());
...
void ULevel::RegenerateLevelScriptActor()
{
	...
	{
		UE_LOGF(LogLevel, Error, "Skipped regeneration of LevelScriptActor due to blueprint '%ls' having no spawnable class. A probable cause is that the blueprint is deriving from an invalid class, and may not function.", *LevelScriptBlueprint->GetName());
	}
}
	...
	if (LevelScriptActor)
	{
		...
		if (ExtraLSAs.Num() > 0)
		{
			if (UEditorActorSubsystem* EditorActorSubsystem = GEditor->GetEditorSubsystem<UEditorActorSubsystem>())
			{
				...
				{
					UE_LOGF(LogLevel, Log, "Deleting extra LevelScriptActor: %ls", *LSA->GetPathName());
