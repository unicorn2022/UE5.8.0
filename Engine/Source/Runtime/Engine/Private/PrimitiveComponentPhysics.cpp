...
void UPrimitiveComponent::SetPhysicsLinearVelocity(FVector NewVel, bool bAddToCurrent, FName BoneName)
{
	...
	else if (BoneName == NAME_None)
	{
		...
		Interface->SetLinearVelocity(PhysicsObjects, NewVel, bAddToCurrent);
	}
	else if (Chaos::FPhysicsObject* Object = GetPhysicsObjectByName(BoneName))
	{
		FLockedWritePhysicsObjectExternalInterface Interface = FPhysicsObjectExternalInterface::LockWrite({ &Object, 1 });
		Interface->SetLinearVelocity({ &Object, 1 }, NewVel, bAddToCurrent);
	}
}

FVector UPrimitiveComponent::GetPhysicsLinearVelocity(FName BoneName) const
{
	if (FBodyInstance* BI = GetBodyInstance(BoneName))
	{
...
FVector UPrimitiveComponent::GetPhysicsLinearVelocityAtPoint(FVector Point, FName BoneName) const
{
	if (FBodyInstance* BI = GetBodyInstance(BoneName))
	{
...
void UPrimitiveComponent::SyncComponentToRBPhysics()
{
	if(!IsRegistered())
	{
		UE_LOGF(LogPhysics, Log, "SyncComponentToRBPhysics : Component not registered (%ls)", *GetPathName());
		return;
	}
	...
	{
		UE_LOGF(LogPhysics, Log, "SyncComponentToRBPhysics : Missing or invalid BodyInstance (%ls)", *GetPathName());
		return;
	}
...
{
	for (TObjectPtr<USceneComponent> ChildComponent : GetAttachChildren())
	{
		if (ChildComponent.GetResidence() == EResidence::Local)
		{
...
IPhysicsBodyInstanceOwner* UPrimitiveComponent::ResolvePhysicsBodyInstanceOwner(Chaos::FConstPhysicsObjectHandle PhysicsObject)
{
	return this;
}

UObject* UPrimitiveComponent::GetSourceObjectOwner() const
{
	return GetOwner();
}

UPhysicalMaterial* UPrimitiveComponent::GetPhysicsMaterialOverride() const
{
	return BodyInstance.GetPhysMaterialOverride();
}

UMaterialInterface* UPrimitiveComponent::GetPhysicsMaterialBase() const
{
	return IsA<UMeshComponent>() ? GetMaterial(0) : nullptr;
}

bool UPrimitiveComponent::IsPhysicsOwnerMovable() const
{
	return Mobility == EComponentMobility::Movable;
}

bool UPrimitiveComponent::IsPhysicsOwnerSimulatingPhysics() const
{
	bool bBaseIsSimulatingPhysics = false;
	const USceneComponent* AttachParentComponent = this;
	while (!bBaseIsSimulatingPhysics && AttachParentComponent)
	{
		bBaseIsSimulatingPhysics = AttachParentComponent->IsSimulatingPhysics();
...
FVector UPrimitiveComponent::GetPhysicsOwnerVelocity() const
{
	return GetComponentVelocity();
}

UObject* UPrimitiveComponent::GetPhysicsOwnerAttachmentRoot() const
{
	return GetAttachmentRoot();
}

bool UPrimitiveComponent::IsPhysicsObjectWorldGeometry() const
{
	return IsWorldGeometry();
}

FTransform UPrimitiveComponent::GetPhysicsOwnerSocketTransform(FName InSocketName) const
{
	return GetSocketTransform(InSocketName);
}

bool UPrimitiveComponent::DoesSocketExistOnPhysicsOwner(FName InSocketName) const
{
	return DoesSocketExist(InSocketName);
}
