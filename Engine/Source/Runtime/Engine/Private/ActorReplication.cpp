...
static bool GbSendPhysicsAttachRep = true;
static FAutoConsoleVariableRef CVarSendPhysicsAttachRep(TEXT("Net.RepMovement.SendPhysicsAttachRep"), GbSendPhysicsAttachRep, TEXT("Whether to replicate attachment replication info when the actor is simulating physics"), ECVF_ReadOnly);
...
void AActor::OnRep_ReplicatedMovement()
{
	...
	if (RootComponent)
	{
		...
		if (LocalRepMovement.bRepPhysics)
		{
			...
			{
				UE_LOGF(LogNet, Warning, "IsSimulatingPhysics() returned false during physics replication for %ls", *GetName());
			}
#endif
...
bool AActor::IsNetRelevantFor(const AActor* RealViewer, const AActor* ViewTarget, const FVector& SrcLocation) const
{
	...
	if (!RootComponent)
	{
		UE_LOGF(LogNet, Warning, "Actor %ls / %ls has no root component in AActor::IsNetRelevantFor. (Make bAlwaysRelevant=true?)", *GetClass()->GetName(), *GetName() );
		return false;
	}
...
void AActor::GatherCurrentMovement()
{
	if (IsReplicatingMovement() || (RootComponent && RootComponent->GetAttachParent()))
	{
		...
		if (RootPrimComp && RootPrimComp->IsSimulatingPhysics())
		{
			...
			if (GbSendPhysicsAttachRep)
			{
				if (RootComponent->GetAttachParent() != nullptr)
				{
					// Set the attachment info here to prevent it being detached. If the actor was previously not simulating physics
					// but now it is and is attached/welded, the attachment should remain
...
void AActor::GetReplicatedCustomConditionState(FCustomPropertyConditionState& OutActiveState) const
{
	...
	DOREPCUSTOMCONDITION_ACTIVE_FAST(AActor, AttachmentReplication, RootComponent && !RootComponent->GetIsReplicated());
...
{
	AddComponentForReplication(Component, EAddComponentForReplicationBehavior::Immediate);
}

void AActor::AddComponentForReplication(UActorComponent* Component, EAddComponentForReplicationBehavior Type)
{
    ...
    {
		if (Type == EAddComponentForReplicationBehavior::Immediate)
		{
			UE::Net::FReplicationSystemUtil::StartReplicatingActorComponent(this, Component);
		}
		else if (Type == EAddComponentForReplicationBehavior::Deferred)
		{
			UE::Net::FDeferredReplicationSystemCalls::StartReplicatingActorComponentDefer(this, Component);
		}
    }
}
...
void AActor::AddReplicatedSubObject(UObject* SubObject, ELifetimeCondition NetCondition)
{
	...
	UE_CLOGF(Result == FSubObjectRegistry::EResult::NewEntry, LogNetSubObject, Verbose, "%ls (0x%p) added replicated subobject %ls (0x%p) [%ls]", 
...
bool AActor::RemoveReplicatedSubObjectFromList(UObject* SubObject)
{
	...
	UE_CLOGF(bWasRemoved, LogNetSubObject, Verbose, "%ls (0x%p) removed replicated subobject %ls (0x%p)", *GetName(), this, *SubObject->GetName(), SubObject);
	return bWasRemoved;
}
...
void AActor::DestroyReplicatedSubObjectOnRemotePeers(UObject* SubObject)
{
	...
	UE_LOGF(LogNetSubObject, Verbose, "%ls (0x%p) requested to Delete replicated subobject: %ls (0x%p)", *GetName(), this, *SubObject->GetName(), SubObject);
...
void AActor::DestroyReplicatedSubObjectOnRemotePeers(UActorComponent* OwnerComponent, UObject* SubObject)
{
	...
	UE_LOGF(LogNetSubObject, Verbose, "%ls::%ls (0x%p) requested to Delete replicated subobject: %ls (0x%p)", *GetName(), *OwnerComponent->GetName(), OwnerComponent, *SubObject->GetName(), SubObject);
...
void AActor::TearOffReplicatedSubObjectOnRemotePeers(UActorComponent* OwnerComponent, UObject* SubObject)
{
	...
	UE_LOGF(LogNetSubObject, Verbose, "%ls::%ls (0x%p) requested to TearOff replicated subobject: %ls (0x%p)", *GetName(), *OwnerComponent->GetName(), this, *SubObject->GetName(), SubObject);
...
void AActor::TearOffReplicatedSubObjectOnRemotePeers(UObject* SubObject)
{
	...
	UE_LOGF(LogNetSubObject, Verbose, "%ls (0x%p) requested to TearOff replicated subobject: %ls (0x%p)", *GetName(), this, *SubObject->GetName(), SubObject);
...
void AActor::AddActorComponentReplicatedSubObject(UActorComponent* OwnerComponent, UObject* SubObject, ELifetimeCondition NetCondition)
{
	...
	if (ComponentInfo)
	{
		...
		UE_CLOGF(Result == FSubObjectRegistry::EResult::NewEntry, LogNetSubObject, Verbose, "%ls::%ls (0x%p) added replicated subobject %ls (0x%p) [%ls]",
	...
	else
	{
		...
		UE_LOGF(LogNetSubObject, Verbose, "%ls::%ls (0x%p) added replicated subobject %ls (0x%p) [%ls]",
			*GetName(), *OwnerComponent->GetName(), OwnerComponent, *SubObject->GetName(), SubObject, *StaticEnum<ELifetimeCondition>()->GetValueAsString(NetCondition));
	}
}
...
bool AActor::RemoveActorComponentReplicatedSubObjectFromList(UActorComponent* OwnerComponent, UObject* SubObject)
{
	...
	if (FReplicatedComponentInfo* ComponentInfo = ReplicatedComponentsInfo.FindByKey(OwnerComponent))
	{
		...
		UE_CLOGF(bWasRemoved, LogNetSubObject, Verbose, "%ls::%ls (0x%p) removed replicated subobject %ls (0x%p)", *GetName(), *OwnerComponent->GetName(), OwnerComponent, *SubObject->GetName(), SubObject);
...
void AActor::OnReplicationStartedForIris(const FOnReplicationStartedParams& Params)
{
	...
	// Process any RPCs for this actor and its sub-objects (assumes the actor and its sub-objects have already been registered with Iris).
...
void AActor::UpdateOwningNetConnection()
{
	...
	do 
	{
		...
		{
			for (TObjectPtr<AActor>& ActorChildPtr : Actor->Children)
			{
				if (ActorChildPtr.GetResidence() != EResidence::Remote)
				{
					AActor* ActorChild = ActorChildPtr;
					EveryChildren.Push(ActorChild);
				}
			}
