...
namespace FNavigationSystem
{
	...
	bool IsLevelVisibilityChanging(const UObject* Object)
	{
		...
		if (const UActorComponent* ObjectAsComponent = Cast<UActorComponent>(Object))
		{
			Level = ObjectAsComponent->GetComponentLevel();
		}
		else if (const AActor* Actor = Cast<AActor>(Object))
		{
			Level = Actor->GetLevel();
		}
		else if (const UAssetUserData* AssetUserData = Cast<UAssetUserData>(Object))
		{
			Level = AssetUserData->GetTypedOuter<ULevel>();
		}

		return Level ? Level->HasVisibilityChangeRequestPending() : false;
	}
...
bool FNavigationRelevantData::IsCollisionDataValid() const
{
	...
	if (!ensure(bIsValid))
	{
		UE_LOGF(LogNavigation, Error, "NavOctree element has corrupted collision data! Owner:%ls Bounds:%ls", *SourceElement->GetName(), *Bounds.ToString());
		return false;
	}
...
{
	UE_CLOGF(IsMetaArea(), LogNavigation, Warning, "UNavAreaBase::PickAreaClassForAgent called for meta class %ls. Please override PickAreaClass."
		, *(GetClass()->GetName()));
