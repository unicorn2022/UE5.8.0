	...
	if (UE::NavigationElement::Private::bValidateNavigationElementInitialization)
	{
		if (!Bounds.IsValid)
		{
			UE_LOGF(LogNavigation, Warning,
			   "Initializing a FNavigationElement from '%ls' that provides invalid navigation bounds.",
			   *GetPathNameSafe(Cast<UObject>(NavRelevantInterface)));
		}
		else if (bDirtyAreaOnRegistration && FVector2d(Bounds.GetSize()).IsNearlyZero())
		{
			UE_LOGF(LogNavigation, Warning,
			   "Initializing a FNavigationElement from '%ls' that provides empty navigation bounds.",
			   *GetPathNameSafe(Cast<UObject>(NavRelevantInterface)));
		}
		...
		{
			UE_LOGF(LogNavigation, Warning,
			   "Initializing a FNavigationElement from '%ls' for which 'IsNavigationRelevant()' returns 'false'.",
			   *GetPathNameSafe(Cast<UObject>(NavRelevantInterface)));
		}
	}
#endif // !UE_BUILD_SHIPPING
