...
namespace FNavigationSystem
{
	void DiscardNavigationDataChunks(UWorld& InWorld)
	{
		...
		{
			UE_LOGF(LogNavigation, Verbose, "%s for %ls", __FUNCTION__, *GetFullNameSafe(Level));
	...
	FNavigationSystemRunMode FindRunModeFromWorldType(const UWorld& World)
	{
		...
		default:
			UE_LOGF(LogNavigation, Warning, "%s Unhandled world type, defaulting to FNavigationSystemRunMode::InvalidMode.", __FUNCTION__);
			return FNavigationSystemRunMode::InvalidMode;
		}
	}
	...
	void AddNavigationSystemToWorld(UWorld& WorldOwner, const FNavigationSystemRunMode RunMode, UNavigationSystemConfig* NavigationSystemConfig, const bool bInitializeForWorld, const bool bOverridePreviousNavSys)
	{
		...
		UE_LOGF(LogNavigation, Verbose, "%s bOverridePreviousNavSys=%i (NavigationSystemConfig: %ls) (WorldOwner: %ls) (DefaultAgentName: %ls)",
			__FUNCTION__,
			bOverridePreviousNavSys,
			*GetNameSafe(NavigationSystemConfig),
			*WorldOwner.GetOuter()->GetName(),
			NavigationSystemConfig ? *NavigationSystemConfig->DefaultAgentName.ToString() : TEXT("none"));
