...
#include "Net/Core/Connection/RPCDoSDetection.h"
...
static TAutoConsoleVariable<int32> CVarPingPlayerFacingPingType(TEXT("net.PlayerFacingPingType"), int32(EPingType::RoundTripExclFrame),
	TEXT("The preferred ping type reported to the player controller. 1 = RoundTrip (including server frame time), 2 = RoundTripExclFrame, 4 = ICMP, 8 = UDPQoS"));
...
namespace UE::Net::Connection::Private
{
	...
	const FValidateLevelVisibilityResult ValidateLevelVisibility(UWorld* World, const FUpdateLevelVisibilityLevelInfo& LevelVisibility)
	{
		...
		auto FindStreamingLevel = [World, &Result](FName InPackageName)
		{
			...
			Result.StreamingLevel = FLevelUtils::FindStreamingLevel(World, InPackageName);
		...
		FindStreamingLevel(LevelVisibility.PackageName);

		if (!Result.bLevelExists)
		{
			Result.bLevelExists = (!FPackageName::IsMemoryPackage(LevelVisibility.PackageName.ToString()) && FPackageName::DoesPackageExist(LevelVisibility.FileName.ToString()));
		}

		// If the level is not found, try resolving it using the optional LevelVisibility.CookPackageName.
		// This is useful when the in-memory PackageName differs from the cooked package name.
		// A common example is when a PIE client sends PIE memory-generated packages to a cooked server.
		if (!Result.bLevelExists && (!LevelVisibility.CookPackageName.IsNone() && LevelVisibility.CookPackageName != LevelVisibility.PackageName))
		{
			FindStreamingLevel(LevelVisibility.CookPackageName);
		}
...
static bool HasChannelRecordsToConsumeForPacket(const FWrittenChannelsRecord& WrittenChannelsRecord, int32 PacketId);
...
namespace UE::Net::Private
{
	...
	static FTransactionallySafeCriticalSection SharedNetTickMutex; //Shared across all NetConnections to protect sections of the Tick logic that cannot run in parallel
...
void UNetConnection::InitChannelData()
{
	...
	UE_LOGF(LogNet, Log, "%ls setting maximum channels to: %d", *GetNameSafe(this), ChannelSize);
...
void UNetConnection::InitBase(UNetDriver* InDriver,class FSocket* InSocket, const FURL& InURL, EConnectionState InState, int32 InMaxPacket, int32 InPacketOverhead)
{
		...
		FaultRecovery->FaultManager.SetUnhandledResultCallback([](FNetResult&& InResult)
			{
				...
				if (bAlwaysLog || (bUniqueLog && !LoggedResults.Contains(ResultHash)))
				{
					...
					UE_LOGF(LogNet, Warning, "NetConnection FaultManager: Unhandled fault: %ls",
	...
	{
		TSharedPtr<FRPCDoSAnalyticsSenderImpl> RPCDoSAnalyticsSender = MakeShared<FRPCDoSAnalyticsSenderImpl>();

		RPCDoSAnalyticsSender->Init(
		...
		RPCDoS->Init(Driver->GetNetDriverDefinition(), AnalyticsAggregator, RPCDoSAnalyticsSender,
			[this]() -> FString
			{
	...
	if (UReplicationSystem* ReplicationSystem = Driver->GetReplicationSystem())
	{
		...
		if (RPCDoS.IsValid())
		{
			ReplicationSystem->SetConnectionRPCDoS(GetConnectionHandle().GetParentConnectionId(), RPCDoS.Get());
		}
	}
}
...
void UNetConnection::InitSequence(int32 IncomingSequence, int32 OutgoingSequence)
{
	...
	if (InPacketId == -1 && CVarRandomizeSequence.GetValueOnAnyThread() > 0)
	{
		...
		UE_LOGF(LogNet, Verbose, "InitSequence: IncomingSequence: %i, OutgoingSequence: %i, InitInReliable: %i, InitOutReliable: %i", IncomingSequence, OutgoingSequence, InitInReliable, InitOutReliable);
	}
}
...
void UNetConnection::EnableEncryption(const FEncryptionData& EncryptionData)
{
	if (Handler.IsValid())
	{
		UE_LOGF(LogNet, Verbose, "UNetConnection::EnableEncryption, %ls", *Describe());
		...
		{
			UE_LOGF(LogNet, Warning, "UNetConnection::EnableEncryption, encryption component not found!");
		}
	}
}
...
void UNetConnection::EnableEncryptionServer(const FEncryptionData& EncryptionData)
{
	...
	{
		UE_LOGF(LogNet, Log, "UNetConnection::EnableEncryptionServer, connection in invalid state. %ls", *Describe());
	}
}
...
void UNetConnection::SendClientEncryptionAck()
{
	...
	{
		UE_LOGF(LogNet, Log, "UNetConnection::SendClientEncryptionAck, connection in invalid state. %ls", *Describe());
	}
}
...
void UNetConnection::SetEncryptionData(const FEncryptionData& EncryptionData)
{
	if (Handler.IsValid())
	{
		UE_LOGF(LogNet, Verbose, "UNetConnection::SetEncryptionData, %ls", *Describe());
		...
		{
			UE_LOGF(LogNet, Warning, "UNetConnection::SetEncryptionData, encryption component not found!");
		}
	}
}
...
void UNetConnection::Close(FNetResult&& CloseReason)
{
	...
	if (Driver != nullptr && GetConnectionState() != USOCK_Closed)
	{
		...
		UE_IRIS_PARALLEL_EXPR(UE::TScopeLock ScopeLock(GetSharedNetTickMutex()));
		...
		UE_LOGF(LogNet, Log, "%ls", ToCStr(CloseStr.ToString()));
...
void UNetConnection::GracefulClose(FNetResult&& CloseReason)
{
	...
	if (Channels[0] && GetConnectionState() != USOCK_Closed)
	{
		...
		{
			UE_LOGF(LogNet, Log, "Initiating graceful close. CloseReason: %ls. %ls", ToCStr(CloseReason.DynamicToString(UE::Net::ENetResultString::WithChain)), *Describe());
...
void UNetConnection::TryClosePendingGracefulClose()
{
	if (GetConnectionState() == USOCK_Closing)
	{
		...
		{
			UE_LOGF(LogNet, Log, "Graceful close complete, closing connection: %ls", *Describe());
...
void UNetConnection::SendCloseReason(FNetResult&& CloseReason)
{
	...
	{
		UE_LOGF(LogNet, Log, "UNetConnection::SendCloseReason:");
		...
		for (FNetCloseResult::FConstIterator It(CloseReason); It; ++It)
		{
			...
			UE_LOGF(LogNet, Log, " - %ls", ToCStr(It->DynamicToString()));
		}
	}
...
void UNetConnection::HandleReceiveCloseReason(const FString& CloseReasonList)
{
	...
	if (!bReceivedCloseReason)
	{
		...
		if (bValid)
		{
			...
			{
				UE_LOGF(LogNet, Log, "NMT_CloseReason: (%ls Disconnect Reasons) %ls", (bRemoteIsServer ? TEXT("Server") : TEXT("Client")),
						ToCStr(LowLevelGetRemoteAddress(true)));

				for (const FString& CurReason : CloseReasonsArray)
				{
					UE_LOGF(LogNet, Log, " - %ls", ToCStr(CurReason));
				}
			}
			else
			{
...
bool UNetConnection::HandleReceiveNetUpgrade(uint32 RemoteNetworkVersion, EEngineNetworkRuntimeFeatures RemoteNetworkFeatures, UE::Net::ENetUpgradeSource NetUpgradeSource)
{
	...
	if (!bUpgradeSucceeded)
	{
		...
		UE_LOGF(LogNet, Error, "Server is incompatible with the local version of the game: RemoteNetworkVersion=%u vs LocalNetworkVersion=%u, RemoteNetworkFeatures=%ls vs LocalNetworkFeatures=%ls",
...
void UNetConnection::CleanUp()
{
	...
	UE_CLOGF((GCVarLogAllConnectionCleanup || !bWasSocketClosed), LogNet, Log, "UNetConnection::Cleanup: Closing open connection. %ls", *Describe());
	...
	if (CachedNoPawnPlayerController)
	{
		CachedNoPawnPlayerController->Destroy();
		CachedNoPawnPlayerController = nullptr;
	}
}
...
void UNetConnection::DestroyOwningActor()
{
	...
	else
	{
		...
		{
			UE_LOGF(LogNet, Log, "UNetConnection::PendingConnectionLost. %ls bPendingDestroy=%d ", *Describe(), bPendingDestroy);
...
void UChildConnection::RemoveLocalPlayer(UGameInstance* GameInstance, ULocalPlayer* LocalPlayer)
{
...
{
	UE_LOGF(LogNet, Log, "[%ls:%ls] Closing child connection %ls.", *GetNameSafe(GetDriver()), *GetNameSafe(Parent), *GetName());
	...
	if (EnumHasAnyFlags(Flags, UE::Net::ECloseChildFlags::SendCloseMessage))
	{
		if (PlayerController)
		{
			...
			UE_LOGF(LogNet, Log, "[%ls:%ls] Sending NMT_CloseChildConnection with NetPlayerIndex %d.", 
				   *GetNameSafe(GetDriver()), *GetNameSafe(Parent), NetPlayerIndexParam);
		}
		else
		{
			UE_LOGF(LogNet, Warning, "[%ls:%ls] Cannot send NMT_CloseChildConnection because there's no valid player controller.", 
		...
		if (ULocalPlayer* LocalPlayer = PlayerController->GetLocalPlayer())
		{
			if (UWorld* World = GetWorld())
			{
				if (UGameInstance* GameInstance = World->GetGameInstance())
				{
					...
					{
						UE_LOGF(LogNet, Log, "[%ls:%ls] Remove local player %ls.", *GetNameSafe(GetDriver()), *GetNameSafe(Parent), *GetNameSafe(LocalPlayer));

						RemoveLocalPlayer(GameInstance, LocalPlayer);
					}
				}
			}
		}
...
void UChildConnection::SetProxyConnection(bool bEnabled)
{
	UE_LOGF(LogNet, Error, "Cannot set the child connection %ls to be a proxy connection.", *GetName());
}

bool UChildConnection::IsProxyConnection() const
{
	return Parent->IsProxyConnection();
}
	...
	if (bShouldBeVisible == false)
	{
		...
		if (UReplicationSystem* ReplicationSystem = Driver->GetReplicationSystem())
		{
			if (const ULevel* Level = LevelObject->GetLoadedLevel())
			{
				...
				if (UE::Net::FNetObjectGroupHandle GroupHandle = ReplicationBridge->GetContainerGroup(Level); GroupHandle.IsValid())
				{
...
void UNetConnection::UpdateLevelVisibilityInternal(const FUpdateLevelVisibilityLevelInfo& LevelVisibility)
{
	...
	if (LevelVisibility.bIsVisible)
	{
		...
		if (bVerifiedTransaction && VisibilityResult.bLevelExists)
		{
			...
			UE_CLOGF(!FLevelUtils::IsServerStreamingLevelVisible(GetWorld(), LevelVisibility.PackageName), LogPlayerController, Warning, "ServerUpdateLevelVisibility() Added '%ls', but level is not visible on server.", *LevelVisibility.PackageName.ToString());
			
			ClientMakingVisibleLevelNames.Remove(LevelVisibility.PackageName);
			ClientVisibleLevelNames.Add(LevelVisibility.PackageName);
			UE_LOGF(LogPlayerController, Verbose, "ServerUpdateLevelVisibility() Added '%ls'", *LevelVisibility.PackageName.ToString());
		...
		else if (!VisibilityResult.bLevelExists)
		{
			...
			UE_LOGF(LogPlayerController, Warning, "ServerUpdateLevelVisibility() ignored non-existant package. PackageName='%ls', FileName='%ls'",
	...
	else
	{
		...
		UE_LOGF(LogPlayerController, Verbose, "ServerUpdateLevelVisibility() Removed '%ls'", *LevelVisibility.PackageName.ToString());
		if (ReplicationConnectionDriver)
		{
...
void UNetConnection::UpdateContainerVisibility(UObject* Container, bool bVisible)
{
	UpdateContainerVisibilityInternal(Container, bVisible);
}

void UNetConnection::UpdateContainerVisibilityInternal(UObject* Container, bool bVisible)
{
	if (UReplicationSystem* ReplicationSystem = Driver->GetReplicationSystem())
	{
		const UObjectReplicationBridge* ReplicationBridge = ReplicationSystem->GetReplicationBridge();
		UE::Net::FNetObjectGroupHandle ContainerGroupHandle = ReplicationBridge->GetContainerGroup(Container);
		if (ContainerGroupHandle.IsValid())
		{
			ReplicationSystem->SetGroupFilterStatus(ContainerGroupHandle, GetConnectionHandle().GetParentConnectionId(), bVisible ? UE::Net::ENetFilterStatus::Allow : UE::Net::ENetFilterStatus::Disallow);
		}
	}
}
...
void UNetConnection::ReceivedRawPacket( void* InData, int32 Count )
{
	...
	if (Handler.IsValid())
	{
		...
		{
			UE_LOGF(LogNet, Warning, "Packet failed PacketHandler processing.");
		...
		{
			UE_LOGF(LogNet, Warning, "PacketHander isn't fully initialized and also didn't fully consume a packet! This will cause the connection to try to send a packet before the initial packet sequence has been established. Ignoring. Connection: %ls", *Describe());
			return;
		}
	}
...
void UNetConnection::FlushPacketOrderCache(bool bFlushWholeCache/*=false*/)
{
	if (PacketOrderCache.IsSet() && PacketOrderCacheCount > 0)
	{
		...
		while (PacketOrderCacheCount > 0)
		{
			...
			{
				UE_LOGF(LogNet, VeryVerbose, "'Out of Order' Packet Cache, replaying packet with cache index: %i (bFlushWholeCache: %i)", PacketOrderCacheStartIdx, (int32)bFlushWholeCache);
...
void UNetConnection::FlushNet(bool bIgnoreSimulation)
{
	...
	if (SendBuffer.GetNumBits() || HasDirtyAcks || ( Driver->GetElapsedTime() - LastSendTime > Driver->KeepAliveTime && !IsInternalAck() && GetConnectionState() != USOCK_Closed))
	{
		...
		{
			UE_LOGF(LogNet, Log, "Attempting to send data before handshake is complete. %ls", *Describe());
		...
		++OutTotalPackets;
		{
			UE_IRIS_PARALLEL_EXPR(Driver->LockNetDriverStats(true));
			Driver->OutPackets++;
			Driver->OutTotalPackets++;
			UE_IRIS_PARALLEL_EXPR(Driver->UnlockNetDriverStats(true));
		}
		...
		if (!IsReplay())
		{
			...
			{
				UE_LOGF(LogNet, Log, "UNetConnection::FlushNet: QueuedBits overflow detected! QueuedBits: %d, PacketBytes: %d. %ls", QueuedBits, PacketBytes, *Describe());
...
bool UNetConnection::CheckOutgoingPacketEmulation(double CurrentTimeInS, FOutPacketTraits& Traits)
{
	...
	{
		UE_LOGF(LogNet, VeryVerbose, "Dropping outgoing packet at %f", CurrentTimeInS);
		return true;
	}
...
{
	UE_LOGF(LogNetTraffic, Verbose, "   Received ack %i", AckPacketId);
	...
	auto AckChannelFunc = [this, &OutChannelsToClose](int32 AckedPacketId, uint32 ChannelIndex)
	{
		...
		if (Channel)
		{
			...
			for (FOutBunch* OutBunch = Channel->OutRec; OutBunch; OutBunch = OutBunch->Next)
			{
				if (OutBunch->bOpen)
				{
					UE_LOGF(LogNet, VeryVerbose, "Channel %i reset Ackd because open is reliable. ", Channel->ChIndex );
...
{
	UE_LOGF(LogNetTraffic, Verbose, "   Received nak %i", NakPacketId);
...
void UNetConnection::WriteDummyPacketInfo(FBitWriter& Writer)
{
	...
	if (bHasPacketInfoPayload)
	{
		...
		const uint8 bHasServerFrameTime = Driver->IsServer() ? bLastHasServerFrameTime : (CVarPingExcludeFrameTime.GetValueOnAnyThread() > 0 ? 1u : 0u);
...
bool UNetConnection::ReadPacketInfo(FBitReader& Reader, bool bHasPacketInfoPayload, FEngineNetworkCustomVersion::Type EngineNetVer)
{
	...
	for (int32 AckId = OutFirstAckPacketId; AckId <= OutAckPacketId; ++AckId)
	{
		...
		if (PlayerController)
		{
			...
			UE_CLOGF(PlayerController.GetResidence() == EResidence::Remote, LogNet, Warning, "%s: PlayerController is unexpectedly a RemoteObject", __func__);
			if (PlayerController.GetResidence() != EResidence::Remote)
			{
				double PlayerFacingPing = RTTExclFrame;
				switch(EPingType(CVarPingPlayerFacingPingType.GetValueOnAnyThread()))
				{
					case EPingType::ICMP:
						if (NetPing.IsValid())
						{
							PlayerFacingPing = NetPing->GetPingValues(EPingType::ICMP).Current;
						}
						break;
					case EPingType::UDPQoS:
						if (NetPing.IsValid())
						{
							PlayerFacingPing = NetPing->GetPingValues(EPingType::UDPQoS).Current;
						}
						break;
					case EPingType::RoundTrip:
						PlayerFacingPing = RTT;
						break;
					case EPingType::RoundTripExclFrame:
					default:
						PlayerFacingPing = RTTExclFrame;
						break;
				}
				
				PlayerController->UpdatePing(FMath::Max(PlayerFacingPing, 0.f));
			}
		}

		if (NetPing.IsValid())
		{
			NetPing->UpdatePing(EPingType::RoundTrip, CurrentTime, FMath::Max(RTT, 0.0)); 
			NetPing->UpdatePing(EPingType::RoundTripExclFrame, CurrentTime, PacketPing);
		}
...
bool UNetConnection::CheckIncomingPacketEmulation(double CurrentTimeInS, FBitReader& Reader)
{
	if (PacketSimulationSettings.PktIncomingLoss)
	{
		...
		{
			UE_LOGF(LogNet, VeryVerbose, "Dropped incoming packet at %f", CurrentTimeInS);
			return true;
		}
	}
	...
	if (PacketSimulationSettings.PktIncomingLagMin > 0 || PacketSimulationSettings.PktIncomingLagMax > 0)
	{
		...
		UE_LOGF(LogNet, VeryVerbose, "Delaying incoming packet for %f seconds", ExtraLagInSec);
	}
	else if (PacketSimulationSettings.PktIncomingFrameDelay)
	{
		...
		UE_LOGF(LogNet, VeryVerbose, "Delaying incoming packet for %u frames", PacketSimulationSettings.PktIncomingFrameDelay);
...
void UNetConnection::ReceivedPacket( FBitReader& Reader, bool bIsReinjectedPacket, bool bDispatchPacket )
{
	...
	else
	{
		...
		{
			UE_LOGF(LogNet, Warning, "Failed to read PacketHeader");
		...
		if (PacketSequenceDelta > 0)
		{
			...
			if (bFillingPacketOrderCache || (bCheckForMissingSequence && MissingPacketCount > 0 && MissingPacketCount <= MaxMissingPackets))
			{
				...
				if (!CurCachePacket.IsValid())
				{
					UE_LOGF(LogNet, VeryVerbose, "'Out of Order' Packet Cache, caching sequence order '%i' (capacity: %i)", LinearCacheIdx, CacheCapacity);
			...
			auto HandlePacketNotification = [&Header, &ChannelsToClose, this](FNetPacketNotify::SequenceNumberT AckedSequence, bool bDelivered)
			{
				...
				{
					UE_LOGF(LogNet, Error, "LastNotifiedPacketId != AckedSequence");
				...
				if (!FChannelRecordImpl::HasChannelRecordsToConsumeForPacket(ChannelRecord, LastNotifiedPacketId))
				{
					UE_LOGF(LogNet, Error, "Could not find matching record for packet %d", LastNotifiedPacketId);
		...
		else
		{
			...
			if (!PacketOrderCache.IsSet() && CVarNetDoPacketOrderCorrection.GetValueOnAnyThread() != 0)
			{
				...
				{
					UE_LOGF(LogNet, Verbose, "Hit threshold of %i 'out of order' packet sequences. Enabling out of order packet correction.", EnableThreshold);
		...
		{
			UE_LOGF(LogNet, Warning, "Failed to read extra PacketHeader information");
	...
	while( !Reader.AtEnd() && GetConnectionState()!=USOCK_Closed )
	{
		...
		// Process Received data
		{
			...
			else
			{
				...
				{
					UE_LOGF(LogNet, Warning, "Bunch channel index exceeds channel limit");
				...
				else
				{
					if (Channels[Bunch.ChIndex])
					{
						...
						if (Bunch.bOpen && (!Bunch.bPartial || Bunch.bPartialInitial))
						{
							...
							{
								UE_LOGF(LogNetTraffic, Verbose, "Adding channel mapping %d to %d", Bunch.ChIndex, FreeIndex);
								ChannelIndexMap.Add(Bunch.ChIndex, FreeIndex);
								Bunch.ChIndex = FreeIndex;
							}
							else
							{
								UE_LOGF(LogNetTraffic, Warning, "Unable to find free channel index");
								continue;
							}
						}
					}
			...
			else
			{
				if (Bunch.bReliable || Bunch.bOpen)
				{
					...
					if( Reader.IsError() )
					{
						UE_LOGF(LogNet, Warning, "Channel name serialization failed.");
			...
			{
				UE_LOGF(LogNet, Error,
						"Existing channel at index %d with type \"%ls\" differs from the incoming bunch's expected channel type, \"%ls\".",
			...
			{
				UE_LOGF(LogNetDormancy, VeryVerbose, "Received: %ls", *Bunch.ToString());
			}

			if (UE_LOG_ACTIVE(LogNetTraffic,VeryVerbose))
			{
				UE_LOGF(LogNetTraffic, VeryVerbose, "Received: %ls", *Bunch.ToString());
			}
			...
			if(Reader.IsError())
			{
				UE_LOGF(LogNet, Warning, "Bunch header overflowed");
			...
			if (Bunch.bHasPackageMapExports)
			{
				...
				if (Driver->IsServer())
				{
					UE_LOGF(LogNetTraffic, Error, "UNetConnection::ReceivedPacket: Server received bHasPackageMapExports packet.");
				...
				if (IsInternalAck())
				{
					...
					if (Bunch.IsError())
					{
						UE_LOGF(LogNetTraffic, Error, "UNetConnection::ReceivedPacket: Bunch.IsError() after ReceiveNetGUIDBunch. ChIndex: %i",
								Bunch.ChIndex);
					}
				}
			}
			...
			if (Bunch.bReliable)
			{
				UE_LOGF(LogNetTraffic, Verbose, "   Reliable Bunch, Channel %i Sequence %i: Size %.1f+%.1f",
						Bunch.ChIndex, Bunch.ChSequence, (HeaderPos-IncomingStartPos)/8.f, (Reader.GetPosBits()-HeaderPos)/8.f);
			}
			else
			{
				UE_LOGF(LogNetTraffic, Verbose, "   Unreliable Bunch, Channel %i: Size %.1f+%.1f",
						Bunch.ChIndex, (HeaderPos-IncomingStartPos)/8.f, (Reader.GetPosBits()-HeaderPos)/8.f);
			}

			if (Bunch.bOpen)
			{
				UE_LOGF(LogNetTraffic, Verbose, "   bOpen Bunch, Channel %i Sequence %i: Size %.1f+%.1f",
						Bunch.ChIndex, Bunch.ChSequence, (HeaderPos-IncomingStartPos)/8.f, (Reader.GetPosBits()-HeaderPos)/8.f );
			}
			...
			if ( Channels[Bunch.ChIndex] == NULL && ( Bunch.ChIndex != 0 || Bunch.ChName != NAME_Control ) )
			{
				...
				if ( Channels[0] == NULL )
				{
					UE_LOGF(LogNetTraffic, Log,
							"ReceivedPacket: Received non-control bunch before control channel was created. ChIndex: %i, ChName: %ls",
				...
				else if ( PlayerController == NULL && Driver->ClientConnections.Contains( this ) )
				{
					UE_LOGF(LogNet, Warning,
							"ReceivedPacket: Received non-control bunch before player controller was assigned. ChIndex: %i, ChName: %ls",
							Bunch.ChIndex, *Bunch.ChName.ToString());
			...
			{
				UE_LOGF(LogNetTraffic, Log, "ReceivedPacket: Received control channel close before open");
			...
			if (IsInternalAck() && bAllowExistingChannelIndex)
			{
				if (PacketEngineNetVer < FEngineNetworkCustomVersion::ReplayDormancy)
				{
					if (Channel)
					{
						...
						if (bNewlyOpenedActorChannel)
						{
							...
							{
								UE_LOGF(LogNetTraffic, Error,
										"UNetConnection::ReceivedPacket: Unable to read actor GUID for ignored bunch. (Channel %d)",
										Bunch.ChIndex);
							}
						}
							...
							if (Bunch.bClose && (!Bunch.bPartial || Bunch.bPartialFinal))
							{
								...
								if (ActorGUID.IsStatic())
								{
									...
									else
									{
										...
										UE_LOGF(LogNetTraffic, Log,
												"UNetConnection::ReceivedPacket: Unable to find static actor to cleanup for ignored bunch. " "(Channel %d NetGUID %ls)", Bunch.ChIndex, *ActorGUID.ToString());
									}
								}
							}

							UE_LOGF(LogNetTraffic, Log, "Ignoring bunch for already open channel: %i", Bunch.ChIndex);
							continue;
						}
					}
				}
				else
				{
					...
					if (bCloseBunch && ChannelIndexMap.Contains(CachedChIndex))
					{
						...
						UE_LOGF(LogNetTraffic, Verbose, "Removing channel mapping %d to %d", CachedChIndex, Bunch.ChIndex);
			...
			{
				UE_LOGF(LogNetTraffic, Log, "UNetConnection::ReceivedPacket: Received outdated bunch (Channel %d Current Sequence %i)",
			...
			if( !Channel && !Bunch.bReliable )
			{
				...
				if (!ValidUnreliableOpen)
				{
					if ( IsInternalAck() )
					{
						...
						UE_LOGF(LogNetTraffic, Error,
								"      Received unreliable bunch before open with reliable connection (Channel %d Current Sequence %i)",
								Bunch.ChIndex, InReliable[Bunch.ChIndex]);
					}
					else
					{
						// Simply a log (not a warning, since this can happen under normal conditions, like from a re-join, etc)
						UE_LOGF(LogNetTraffic, Log, "      Received unreliable bunch before open (Channel %d Current Sequence %i)",
								Bunch.ChIndex, InReliable[Bunch.ChIndex]);
					}
			...
			if (Channel == nullptr)
			{
				...
				{
					UE_LOGF(LogNetTraffic, Log,
							"      Ignoring Bunch for ChIndex %i, as the channel was already rejected while processing this packet.",
							Bunch.ChIndex);
				...
				{
					UE_LOGF(LogNet, Warning, "ReceivedPacket: Connection unknown channel type (%ls)", *Bunch.ChName.ToString());
				...
				{
					UE_LOGF(LogNetTraffic, Warning, "      Ignoring Bunch Create received from client since only server is allowed to "
						"create this type of channel: Bunch  %i: ChName %ls, ChSequence: %i, bReliable: %i, bPartial: %i, "
						"bPartialInitial: %i, bPartialFinal: %i", Bunch.ChIndex, *Bunch.ChName.ToString(), Bunch.ChSequence,
				...
				if (IsInternalAck() && bIgnoreActorBunches)
				{
					if (Bunch.bOpen && (!Bunch.bPartial || Bunch.bPartialInitial) && (Bunch.ChName == NAME_Actor))
					{
						...
						if (ActorGUID.IsValid() && !ActorGUID.IsDefault())
						{
							...
							{
								UE_LOGF(LogNetTraffic, Verbose, "Adding Channel: %i to ignore list, ignoring guid: %ls",
							...
							else
							{
								...
								{
									UE_LOGF(LogNetTraffic, Verbose, "Removing Channel: %i from ignore list, got new guid: %ls",
											Bunch.ChIndex, *ActorGUID.ToString());
								}
							}
						}
						...
						{
							UE_LOGF(LogNetTraffic, Warning, "Open bunch with invalid actor guid, Channel: %i", Bunch.ChIndex);
						}
					}
					else
					{
						...
						{
							UE_LOGF(LogNetTraffic, Verbose, "Ignoring bunch on channel: %i", Bunch.ChIndex);
							continue;
						}
					}
				}
				...
				UE_LOGF(LogNetTraffic, Log, "      Bunch Create %i: ChName %ls, ChSequence: %i, bReliable: %i, bPartial: %i, " "bPartialInitial: %i, bPartialFinal: %i", Bunch.ChIndex, *Bunch.ChName.ToString(), Bunch.ChSequence,
						(int)Bunch.bReliable, (int)Bunch.bPartial, (int)Bunch.bPartialInitial, (int)Bunch.bPartialFinal);
				...
				if( Driver->Notify == nullptr || !Driver->Notify->NotifyAcceptingChannel( Channel ) )
				{
					...
					UE_LOGF(LogNet, Verbose, "      NotifyAcceptingChannel Failed! Channel: %ls", *Channel->Describe() );
					...
					if( Bunch.ChIndex==0 )
					{
						UE_LOGF(LogNetTraffic, Log, "Channel 0 create failed" );
						SetConnectionState(EConnectionState::USOCK_Closed);
					}
					continue;
				}
			...
			if (Bunch.IsCriticalError() || Bunch.IsError())
			{
				...
				if (bIsServer)
				{
					UE_LOGF(LogNetTraffic, Error, "Received corrupted packet data with SequenceId: %d from client %ls. Disconnecting.", PacketId, *LowLevelGetRemoteAddress());
				...
				{
					UE_LOGF(LogNetTraffic, Error, "Received corrupted packet data with SequenceId: %d from server %ls", PacketId, *LowLevelGetRemoteAddress());
			...
			{
				UE_LOGF(LogNetTraffic, Warning, "Replay cleaning up channel that couldn't be opened: %ls", *Channel->Describe());
				...
				if (Channel->Broken)
				{
					if (UActorChannel* ActorChannel = Cast<UActorChannel>(Channel))
					{
						...
						for (FInBunch* InBunch : ActorChannel->QueuedBunches)
						{
							...
							{
								UE_LOGF(LogNet, Warning, "SetAllowExistingChannelIndex:  Cleaning up broken channel: %ls", *ActorChannel->Describe());
...
void UNetConnection::SetProxyConnection(bool bEnabled)
{
	bIsProxyConnection = bEnabled;

	if (UReplicationSystem* DriverReplicationSystem = Driver->GetReplicationSystem())
	{
		const uint32 ConnectionId = GetConnectionHandle().GetParentConnectionId();
		DriverReplicationSystem->SetProxyConnection(ConnectionId, bEnabled);
	}
}

bool UNetConnection::IsProxyConnection() const
{
	return bIsProxyConnection;
}
...
int32 UNetConnection::SendRawBunch(FOutBunch& Bunch, bool InAllowMerge, const FNetTraceCollector* BunchCollector)
{
	...
	{
		UE_LOGF(LogNetDormancy, VeryVerbose, "Sending: %ls", *Bunch.ToString());
	}

	if (UE_LOG_ACTIVE(LogNetTraffic, VeryVerbose))
	{
		UE_LOGF(LogNetTraffic, VeryVerbose, "Sending: %ls", *Bunch.ToString());
	}
...
UChannel* UNetConnection::CreateChannelByName(const FName& ChName, EChannelCreateFlags CreateFlags, int32 ChIndex)
{
	...
	if (ChIndex == INDEX_NONE)
	{
		...
		if (ChIndex == INDEX_NONE)
		{
			if (!bHasWarnedAboutChannelLimit)
			{
				...
				UE_LOGF(LogNetTraffic, Warning, "No free channel could be found in the channel list (current limit is %d channels) for connection with owner %ls. Consider increasing the max channels allowed using net.MaxChannelSize.", Channels.Num(), *GetNameSafe(OwningActor));
			}
	...
	UE_LOGF(LogNetTraffic, Log, "Created channel %i of type %ls", ChIndex, *ChName.ToString());
...
void UNetConnection::Tick(float DeltaSeconds)
{
	...
	// Acquire a coarse grain lock which protects access to shared state such as NetDriver internals, lag simulation, disconnect logic etc
	// It is unlocked whilst ticking individual channels which is where the most time consuming work is done
	UE_IRIS_PARALLEL_EXPR(UE::TScopeLock ScopeLock(GetSharedNetTickMutex()));
	...
	float EngineTickRate = Driver->CachedEngineMaxTickRate;
	...
	if (bReceiveTimedOut || bGracefulCloseTimedOut)
	{
		...
		{
			UE_LOGF(LogNet, Warning, "%ls", *Error);
	...
	else
	{
		...
		if (CVarTickAllOpenChannels.GetValueOnAnyThread() == 0)
		{
			...
			{
				UE_IRIS_PARALLEL_EXPR(UE::TScopeUnlock<FTransactionallySafeCriticalSection> ScopeUnlock(ChannelsToTick[i]->SupportsParallelTick() ? &GetSharedNetTickMutex() : nullptr ));
		...
		else
		{
			for (int32 i = OpenChannels.Num() - 1; i >= 0; i--)
			{
				UE_IRIS_PARALLEL_EXPR(UE::TScopeUnlock<FTransactionallySafeCriticalSection> ScopeUnlock(OpenChannels[i]->SupportsParallelTick() ? &GetSharedNetTickMutex() : nullptr ));
				if (OpenChannels[i])
				{
					OpenChannels[i]->Tick();
				}
				...
				{
					UE_LOGF(LogNet, Warning, "UNetConnection::Tick: null channel in OpenChannels array. %ls", *Describe());
				}
			}
		}
			...
			for ( int32 ActorChannelIdx = 0; ActorChannelIdx < ActorChannelArray.Num(); ++ActorChannelIdx )
			{
				...
				if ( IsValid(CurChannel) )
				{
					...
					if ( CurChannel->ProcessQueuedBunches() )
					{
						...
						bRemoveChannel = true;
						UE_LOGF( LogNet, VeryVerbose, "UNetConnection::Tick: Removing from KeepProcessingActorChannelBunchesMap. Num: %i", KeepProcessingActorChannelBunchesMap.Num() );
					}
				...
				{
					bRemoveChannel = true;
					UE_LOGF( LogNet, Verbose, "UNetConnection::Tick: Removing from KeepProcessingActorChannelBunchesMap before done processing bunches. Num: %i", KeepProcessingActorChannelBunchesMap.Num() );
				}
	...
	if ( TimeSensitive || (Driver->GetElapsedTime() - LastSendTime) > Driver->KeepAliveTime)
	{
		...
		{
			UE_IRIS_PARALLEL_EXPR(UE::TScopeUnlock<FTransactionallySafeCriticalSection> ScopeUnlock(&GetSharedNetTickMutex()));
			FlushNet();
		}
	}
...
void UNetConnection::HandleClientPlayer( APlayerController *PC, UNetConnection* NetConnection )
{
	...
	UE_LOGF(LogNet, Verbose, "%ls setplayer %ls",*PC->GetName(),*LocalPlayer->GetName());
	LastReceiveTime = Driver->GetElapsedTime();
...
UChildConnection* UNetConnection::CreateChild(int32* OutNetPlayerIndex)
{
	...
	UE_LOGF(LogNet, Log, "[%ls:%ls] Assigning NetPlayerIndex %d to a new child connection.", *GetNameSafe(GetDriver()), *GetName(), NetPlayerIndex);
...
UChildConnection* UNetConnection::CreateChildWithNetPlayerIndex(int32 NetPlayerIndex)
{
	...
	UE_LOGF(LogNet, Log, "[%ls:%ls] Creating child connection %ls NetPlayerIndex %d", *GetNameSafe(GetDriver()), *GetName(), *GetNameSafe(ChildConnection), NetPlayerIndex);
...
void UNetConnection::RemoveChild(UChildConnection* ChildConnection)
{
	...
	UE_LOGF(LogNet, Log, "Removing child connection %ls from parent connection %ls using NetPlayerIndex %d.", *GetNameSafe(ChildConnection), *GetNameSafe(this), NetPlayerIndexToRemove);
...
void UChildConnection::HandleClientPlayer(APlayerController* PC, UNetConnection* NetConnection)
{
	...
	{
		UE_LOGF(LogNet, Error, "Failed to find LocalPlayer for received PlayerController '%ls' with index %d. PlayerControllers:", *PC->GetName(), int32(PC->NetPlayerIndex));
		...
		for (TActorIterator<APlayerController> It(PC->GetWorld()); It; ++It)
		{
			...
			{
				UE_LOGF(LogNet, Log, " - %ls", *It->GetFullName());
			}
		}
#endif
	...
	UE_LOGF(LogNet, Verbose, "%ls setplayer %ls", *PC->GetName(), *NewPlayer->GetName());
	PlayerController = PC;
	OwningActor = PC;
	SetClientHandshakeId(PC->GetClientHandshakeId());
...
void UNetConnection::FlushDormancy(AActor* Actor)
{
	...
	UE_LOGF(LogNetDormancy, Verbose, "FlushDormancy: %ls. Connection: %ls", *Actor->GetName(), *GetName());
	...
	if ( Ch != nullptr )
	{
		UE_LOGF( LogNetDormancy, Verbose, "    Found Channel[%d] '%ls'. Reseting Dormancy. Ch->Closing: %d", Ch->ChIndex, *Ch->Describe(), Ch->Closing );
...
void UNetConnection::SetClientLoginState( const EClientLoginState::Type NewState )
{
	...
	{
		UE_LOGF(LogNet, Verbose, "UNetConnection::SetClientLoginState: State same: %ls", EClientLoginState::ToString( NewState ) );
		return;
	}

	UE_CLOGF((Driver == nullptr || !Driver->DDoS.CheckLogRestrictions()), LogNet, Verbose,
				"UNetConnection::SetClientLoginState: State changing from %ls to %ls",
...
void UNetConnection::SetExpectedClientLoginMsgType(const uint8 NewType)
{
	...
	{
		UE_CLOGF(!bLogRestricted, LogNet, Verbose, "UNetConnection::SetExpectedClientLoginMsgType: Type same: [%d]%ls",
				NewType, FNetControlMessageInfo::IsRegistered(NewType) ? FNetControlMessageInfo::GetName(NewType) : TEXT("UNKNOWN"));

		return;
	}

	UE_CLOGF(!bLogRestricted, LogNet, Verbose,
		"UNetConnection::SetExpectedClientLoginMsgType: Type changing from [%d]%ls to [%d]%ls", 
...
bool UNetConnection::IsClientMsgTypeValid( const uint8 ClientMsgType )
{
	if ( ClientLoginState == EClientLoginState::LoggingIn )
	{
		...
		if ( ClientMsgType != ExpectedClientLoginMsgType )
		{
			...
			UE_LOGF(LogNet, Log, "UNetConnection::IsClientMsgTypeValid FAILED: (ClientMsgType != ExpectedClientLoginMsgType) Remote Address=%ls", *LowLevelGetRemoteAddress());
			return false;
		}
	} 
	else
	{
		...
		if ( ClientMsgType == NMT_Hello || ClientMsgType == NMT_Login )
		{
			...
			UE_LOGF(LogNet, Log, "UNetConnection::IsClientMsgTypeValid FAILED: Invalid msg after being logged in - Remote Address=%ls", *LowLevelGetRemoteAddress());
			return false;
		}
	}
...
bool UNetConnection::TrackLogsPerSecond()
{
	...
	if ( LogCallTotalTime > LOG_AVG_THRESHOLD )
	{
		...
		if ( LogsPerSecond > MAX_LOGS_PER_SECOND_INSTANT )
		{
			...
			UE_LOGF(LogNet, Warning, "TrackLogsPerSecond instant FAILED. LogsPerSecond: %f, RemoteAddr: %ls",
		...
		if ( LogsPerSecond > MAX_LOGS_PER_SECOND_SUSTAINED )
		{
			...
			UE_LOGF(LogNet, Warning,
					"TrackLogsPerSecond: LogsPerSecond > MAX_LOGS_PER_SECOND_SUSTAINED. LogSustainedCount: %i, LogsPerSecond: %f, RemoteAddr: %ls",
			...
			if ( LogSustainedCount > MAX_SUSTAINED_COUNT )
			{
				...
				UE_LOGF(LogNet, Warning, "TrackLogsPerSecond: LogSustainedCount > MAX_SUSTAINED_COUNT. LogsPerSecond: %f, RemoteAddr: %ls",
						(float)LogsPerSecond, *LowLevelGetRemoteAddress());
...
void UNetConnection::ProcessJitter(uint32 PacketJitterClockTimeMS)
{
	...
	//UE_LOGF(LogNetTraffic, Verbose, "Avg Jitter: %f | CurrentJitter: %f | CurrentTimeDelta: %d | PreviousTimeDelta: %d | ReceivedClockMilli %d | LocalClockMilli %d", AverageJitterInMS, CurrentJitter, CurrentJitterTimeDelta, PreviousJitterTimeDelta, PacketJitterClockTimeMS, CurrentClockTimeMilliseconds);
...
void UNetConnection::SendChallengeControlMessage()
{
	...
	{
		UE_LOGF(LogNet, Log, "UWorld::SendChallengeControlMessage: connection in invalid state. %ls", *Describe());
	}
}
...
void UNetConnection::SendChallengeControlMessage(const FEncryptionKeyResponse& Response)
{
	if (GetConnectionState() != USOCK_Invalid && GetConnectionState() != USOCK_Closed && Driver)
	{
		...
		else
		{
			...
			UE_LOGF(LogNet, Warning, "UWorld::SendChallengeControlMessage: encryption failure [%ls] %ls", *ResponseStr, *Response.ErrorMsg);
	...
	{
		UE_LOGF(LogNet, Warning, "UNetConnection::SendChallengeControlMessage: connection in invalid state. %ls", *Describe());
	}
}
...
#if UE_SUPPORT_PARALLEL_IRIS
FTransactionallySafeCriticalSection& UNetConnection::GetSharedNetTickMutex()
{
	return UE::Net::Private::SharedNetTickMutex;
}
#endif // UE_SUPPORT_PARALLEL_IRIS
...
static void	AddSimulatedNetConnections(const TArray<FString>& Args, UWorld* World)
{
	...
	UE_LOGF(LogNet, Display, "Adding %d Simulated Connections...", ConnectionCount);
...
static void	RemoveSimulatedNetConnections(const TArray<FString>& Args, UWorld* World)
{
	...
	UE_LOGF(LogNet, Display, "Removed %d Simulated Connections...", RemovedConnections);
}
	...
	FConsoleCommandWithWorldAndArgsDelegate::CreateLambda([](const TArray<FString>& Args, UWorld* World)
{
	...
	UE_LOGF(LogNet, Display, "ForceOnePacketPerBunch set to %ls", bEnable?TEXT("true"):TEXT("false"));
...
static void	PrintActorReportFunc(const TArray<FString>& Args, UWorld* InWorld)
{
	...
	UE_LOGF(LogNet, Display, "Class Count (includes inheritance)");
	for (auto MapIt : ClassCount)
	{
		UE_LOGF(LogNet, Display, "%ls - %d", *GetNameSafe(MapIt.Key), MapIt.Value);
	}


	UE_LOGF(LogNet, Display, "");
	UE_LOGF(LogNet, Display, "Class Count (actual clases)");
	...
	UE_LOGF(LogNet, Display, "");
	UE_LOGF(LogNet, Display, "Complete Bounding Box: %ls", *BoundingBox.ToString());
	UE_LOGF(LogNet, Display, "                 Size: %ls", *BoundingBox.GetSize().ToString());

	UE_LOGF(LogNet, Display, "");

	for (auto MapIt : DormancyCount)
	{
		UE_LOGF(LogNet, Display, "%ls - %d", *UEnum::GetValueAsString(TEXT("/Script/Engine.ENetDormancy"), MapIt.Key), MapIt.Value);
	}

	UE_LOGF(LogNet, Display, "");
	UE_LOGF(LogNet, Display, "Total Replicated Actor Count: %d", TotalCount);


	UE_LOGF(LogNet, Display, "");
	UE_LOGF(LogNet, Display, "Raw Actor Map: ");
	RawActorPtrMap.Dump(*GLog);

	UE_LOGF(LogNet, Display, "");
	UE_LOGF(LogNet, Display, "Weak Ptr Map: ");
	WeakPtrMap.Dump(*GLog);

	UE_LOGF(LogNet, Display, "");
	UE_LOGF(LogNet, Display, "ObjectKey Map: ");
	ObjKeyMap.Dump(*GLog);
}
...
bool HasChannelRecordsToConsumeForPacket(const FWrittenChannelsRecord& WrittenChannelsRecord, int32 PacketId)
{
	const FWrittenChannelsRecord::FChannelRecordEntryQueue& Record = WrittenChannelsRecord.ChannelRecord;
	if (!Record.IsEmpty())
	{
		FWrittenChannelsRecord::FChannelRecordEntry PacketEntry = Record.Peek();
		return PacketEntry.IsSequence == 1u && PacketEntry.Value == (uint32)PacketId;
	}
