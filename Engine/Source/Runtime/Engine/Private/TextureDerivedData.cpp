...
static bool IsUsingNewDerivedData()
{
	struct FTextureDerivedDataSetting
	{
		FTextureDerivedDataSetting()
		{
			...
			UE_CLOGF(bUseNewDerivedData, LogTexture, Log, "Using new texture derived data builds.");
		}
...
static void SerializeForKey(FArchive& Ar, const FTextureBuildSettings& Settings)
{
	...
	if ( Settings.MaxTextureResolution != FTextureBuildSettings::MaxTextureResolutionDefault && Settings.bUseNewMipFilter )
	{
		...
		{
			TempName = FName("Sliced Resize Bug Fix 03/07/2024");
			Ar << TempName;
		}
	}
	...
	FTextureBuildSettings& OutSettings)
{
	...
	if ( BuiltSizeX > MaxDimension || BuiltSizeY > MaxDimension || BuiltSizeZ > MaxDimension )
	{
		...
		{
			UE_LOGF(LogTexture, Error, "Texture %ls non-volume has huge Z depth!", 
				*Texture.GetPathName());
		...
		{
			UE_LOGF(LogTexture, Warning, "Texture %ls exceeds maximum dimensions : %d x %d x %d > %d , shrinking...", *Texture.GetPathName(),
	...
	{
		UE_LOGF(LogTexture, Error, "Texture %ls failed GetOutputPixelFormatWithFallback (format=%ls)", 
			*Texture.GetPathName(),
	...
	{
		UE_LOGF(LogTexture, Warning, "Texture %ls exceeds maximum size of surface or package: %d x %d x %d x %ls = {%lld,%lld bytes} exceeds limit {%lld,%lld bytes} shrinking...", *Texture.GetPathName(),
	...
	if ( OutSettings.MaxTextureResolution != OriginalMaxTextureResolution )
	{
		...
		const UTextureLODSettings& LODSettings = TargetPlatform->GetTextureLODSettings();
 		const uint32 LODBiasNoCinematics = FMath::Max<int32>(LODSettings.CalculateLODBias(BuiltSizeX, BuiltSizeY, Texture.GetMinTextureSize(), Texture.GetMaxTextureSize(), Texture.LODGroup, Texture.LODBias, 0, Texture.MipGenSettings, OutSettings.bVirtualStreamable, Texture.IsStreamable()), 0);
	...
	)
{
	...
	if (OutSettings.bVirtualStreamable && ! OutSettings.TextureFormatName.IsNone())
	{
		...
		//	this is only used by Android & iOS (changes DXT to ASTC/ETC)
		//  the reason to do it here is we now have bVirtualStreamable, which is not available at the earlier call
		
		// The platform FinalizeVirtualTextureLayerFormat assumes (incorrectly) that it gets non-prefixed names, so remove them :
		...
		// May still have an Oodle prefix, remove that :
		...
		{
			// if the platform FinalizeVirtualTextureLayerFormat changed our format, take that change (with no Oodle) prefix)
			//	otherwise, leave TextureFormatName along (with Oodle prefix)
			// POSSIBLE BUG : if the Platform changed us to a different DXT format that should have Oodle prefix, we lost it here!
...
int32 UTexture::GetMinTextureSize() const
{
	if (VirtualTextureStreaming)
	{
		FVirtualTextureBuildSettings VirtualTextureBuildSettings;
		GetVirtualTextureBuildSettings(VirtualTextureBuildSettings);
		return FVirtualTextureBuildSettings::ClampAndAlignTileSize(VirtualTextureBuildSettings.TileSize);
	}
	return 0;
}
	...
	)
{
	...
	{
		UE_LOGF(LogTexture, Warning, "Unexpected texture build for dynamic texture? (%ls)",*Texture.GetName());
	}
	else
	{
		// unknown TextureType ?
		UE_LOGF(LogTexture, Error, "Unexpected texture build for unknown texture class? (%ls)",*Texture.GetName());
	}
	...
	if ( Texture.RequiresVirtualTexturing() && ! bVirtualTextureStreaming )
	{
		...
		UE_LOGF(LogTexture, Error, "Texture RequiresVirtualTexturing but VT is off (%ls)",*Texture.GetName());
	...
	if (bVirtualTextureStreaming && MipGenSettings == TMGS_NoMipmaps)
	{
		...
		UE_LOGF(LogTexture, Display, "Texture %ls is virtual and has NoMips - forcing to SimpleAverage.", *Texture.GetPathName());
	}
	if (bVirtualTextureStreaming && MipGenSettings == TMGS_LeaveExistingMips)
	{
		for (int32 BlockIndex = 0; BlockIndex < Texture.Source.GetNumBlocks(); BlockIndex++)
		{
			...
			{
				MipGenSettings = TMGS_SimpleAverage;
				UE_LOGF(LogTexture, Warning, "Texture %ls is virtual and has LeaveExistingMips with an incomplete mip chain - forcing to SimpleAverage (Block %d has %d mips, expected %d).", 
					*Texture.GetPathName(),
					BlockIndex,
					Block.NumMips,
					ExpectedNumMips
	...
	{
		UE_LOGF(LogTexture, Warning, "Texture '%ls' has UDIM Blocks, but Virtual Texturing is off for platform '%ls'. Only the first UDIM block will be built for the platform.",
			*Texture.GetPathName(),
	...
	OutBuildSettings.LODBias = TextureLODSettings.CalculateLODBias(SourceSize.X, SourceSize.Y, Texture.GetMinTextureSize(), Texture.GetMaxTextureSize(), Texture.LODGroup, Texture.LODBias, Texture.NumCinematicMipLevels, Texture.MipGenSettings, bVirtualTextureStreaming, Texture.IsStreamable());
	OutBuildSettings.LODBiasWithCinematicMips = TextureLODSettings.CalculateLODBias(SourceSize.X, SourceSize.Y, Texture.GetMinTextureSize(), Texture.GetMaxTextureSize(), Texture.LODGroup, Texture.LODBias, 0, Texture.MipGenSettings, bVirtualTextureStreaming, Texture.IsStreamable());
	...
	{
		UE_LOGF(LogTexture, Warning, "Downscale setting of %f was not used when building texture %ls%ls.", Downscale, *Texture.GetName(),
...
{
	// Beware GetTargetPlatformBuildSettings and GetBuildSettingsForTargetPlatform are almost code dupes of each other; keep in sync!
	//	todo: refactor to dedupe

	check(TargetPlatform != NULL);

	const UTextureLODSettings* LODSettings = UDeviceProfileManager::Get().FindProfile(TargetPlatform->CookingDeviceProfileName())->GetTextureLODSettings();
	...
	)
{
	...
	if (TPM)
	{
		...
		if (Texture.OverrideRunningPlatformName != NAME_None)
		{
			...
			{
				UE_LOGF(LogTexture, Display, "Platform viewing not supported with virtual textures (%ls)", *Texture.GetPathName());
			}
			else if (Texture.Availability == ETextureAvailability::GPU) // only makes sense if encoded!
			{
				...
				if (OverridePlatform)
				{
					UE_LOGF(LogTexture, Display, "Overriding running platform for texture %ls from %ls to %ls", *Texture.GetPathName(), *TargetPlatform->PlatformName(), *OverridePlatform->PlatformName());
...
int64 PutDerivedDataInCache(FTexturePlatformData* DerivedData, const FString& DerivedDataKeySuffix, const FStringView& TextureName, bool bForceAllMipsToBeInlined, bool bReplaceExistingDDC)
{
	...
	for (int32 MipIndex = 0; MipIndex < WritableMipCount; ++MipIndex)
	{
		...
		if (bDDCError)
		{
			UE_LOGF(LogTexture, Fatal, "Error %ls", *LogString);
		}
	...
	)
{
	...
	if ( InTexture.Source.GetNumLayers() == 0 )
	{
		UE_LOGF(LogTexture, Error, "Attempt to cache texture with zero layers [%ls]", *InTexture.GetName());
		return;
	}

	check( InSettingsPerLayerFetchOrBuild != nullptr );
	...
	if (IsUsingNewDerivedData() && InTexture.Source.GetNumLayers() == 1 && !InSettingsPerLayerFetchOrBuild->bVirtualStreamable)
	{
		...
		UE_LOGF(LogTexture, Warning, "Failed to create requested DDC2 build task for texture %ls -- falling back to DDC1", *InTexture.GetName());
	}
...
static bool LoadDerivedStreamingMips(FTexturePlatformData& PlatformData, int32 FirstMipToLoad, int32 MaxMipCount, bool bTouchOnly, FStringView DebugContext, TFunctionRef<void (int32 MipIndex, FSharedBuffer MipData)> Callback)
{
	...
	{
		UE_LOGF(LogTexture, Error, "Attempting to stream in mips for texture that has not generated a supported derived data key format.");
	}
...
bool FTexturePlatformData::TryLoadMipsWithSizes(int32 FirstMipToLoad, void** OutMipData, int64* OutMipSize, FStringView DebugContext)
{
	...
	for (int32 MipIndex = FirstMipToLoad; MipIndex < LoadableMips; ++MipIndex)
	{
		...
		if (BulkDataSize > 0)
		{
			if (OutMipData)
			{
				...
				{
					UE_CLOGF(Mip.BulkData.IsInSeparateFile(), LogTexture, Error, "Loading non-streamed mips from an external bulk file.  This is not desireable.  File %ls",
	...
	{
		UE_LOGF(LogTexture, Verbose, "TryLoadMips failed for %.*ls, NumMipsCached: %d, LoadableMips: %d, FirstMipToLoad: %d",
	...
	const bool bSerializeMipData)
{
	...
	if (bCooked && Ar.IsSaving())
	{
		...
		FirstMipToSerialize = Ar.CookingTarget()->GetTextureLODSettings().CalculateLODBias(Width, Height, Texture->GetMinTextureSize(), Texture->GetMaxTextureSize(), LODGroup, LODBias, 0, MipGenSetting, bIsVirtual, Texture->IsStreamable());
		if (!bIsVirtual)
		{
		...
		if (MipLoadOptions == ETextureMipLoadOptions::OnlyFirstMip)
		{
			...
			if (!bIsVirtual && Texture->GetTextureClass() == ETextureClass::TwoD && NumMips > 1)
			{
					...
					!FMath::IsPowerOfTwo(PlatformData->Mips[FirstMipToSerialize].SizeY))
				{
					...
					UE_LOGF(LogTexture, Warning, "MipLoadOption OnlyFirstMip can't be applied to non pow2 textures, see Downscale option if this is being used as a workaround for LODBias on NoMipMaps. Texture: %ls", *Texture->GetName());
				}
			}
		}
	}
	...
	#endif
		{
			check(Texture && Texture->IsPossibleToStream());
	...
	if (bUsingDerivedData)
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogTexture, Warning, "Invalid pixel format '%ls' for texture '%ls'.", *PixelFormatString, Texture ? *Texture->GetPathName() : TEXT(""));
...
bool UTexture2DArray::GetMipData(int32 InFirstMipToLoad, TArray<FUniqueBuffer, TInlineAllocator<MAX_TEXTURE_MIP_COUNT>>& OutMipData)
{
	...
	check( LocalPlatformData != nullptr );
	...
	if (LocalPlatformData->TryLoadMipsWithSizes(InFirstMipToLoad, MipData, MipSizes, GetPathName()) == false)
	{
		...
		UE_LOGF(LogTexture, Warning, "UTexture2DArray GetMipData failed for %ls (%ls)",
			*GetPathName(), GPixelFormats[GetPixelFormat()].Name);
#if WITH_EDITOR
		if (!GetOutermost()->bIsCookedForEditor)
		{
			ForceRebuildPlatformData();
			LocalPlatformData = GetPlatformData();
			if (LocalPlatformData->TryLoadMipsWithSizes(InFirstMipToLoad, MipData, MipSizes, GetPathName()) == false)
			{
				UE_LOGF(LogTexture, Error, "TryLoadMipsWithSizes still failed after ForceRebuildPlatformData %ls.", *GetPathName());
				return false;
			}
		}
#else // #if WITH_EDITOR
...
bool UTexture2D::GetInitialMipData(int32 FirstMipToLoad, TArrayView<void*> OutMipData, TArrayView<int64> OutMipSize)
{
	...
	{
		// Unable to load mips from the cache. Rebuild the texture and try again.
		UE_LOGF(LogTexture,Warning,"UTexture2D::GetInitialMipData failed for %ls (%ls)",
			*GetPathName(), GPixelFormats[GetPixelFormat()].Name);
#if WITH_EDITOR
		if (!GetOutermost()->bIsCookedForEditor)
		{
			...
			{
				UE_LOGF(LogTexture, Error, "TryLoadMips still failed after ForceRebuildPlatformData %ls.", *GetPathName());
			}
		}
#endif // #if WITH_EDITOR
	}
...
void UTextureCube::GetMipData(int32 FirstMipToLoad, void** OutMipData)
{
	...
	{
		// Unable to load mips from the cache. Rebuild the texture and try again.
		UE_LOGF(LogTexture,Warning,"UTextureCube::GetMipData failed for %ls (%ls)",
			*GetPathName(), GPixelFormats[GetPixelFormat()].Name);
#if WITH_EDITOR
		if (!GetOutermost()->bIsCookedForEditor)
		{
...
void UTexture::CachePlatformData(bool bAsyncCache, bool bAllowAsyncBuild, bool bAllowAsyncLoading, ITextureCompressorModule* Compressor, bool bForceRebuild)
{
	...
	if (PlatformDataLinkPtr)
	{
		...
		if ( FApp::CanEverRender() && CanBuildPlatformData() )
		{
			...
			default:
				{
					UE_LOGF(LogTexture, Fatal, "Invalid encode speed in CachePlatformData");
				}
			}
...
bool UTexture::DownsizeImageUsingTextureSettings(const ITargetPlatform* TargetPlatform, FImage& InOutImage, int32 TargetSize, int32 LayerIndex, bool & OutMadeChanges) const
{
	...
	{
		UE_LOGF(LogTexture, Error, "Invalid parameter supplied to DownsizeImageUsingTextureSettings target size = %d layer index = %d image valid: %ls",
	...
	{
		UE_LOGF(LogTexture, Error, "Invalid layer supplied to DownsizeImageUsingTextureSettings, layer index = %d", LayerIndex);
		return false;
	}
...
{
	// OutSettingPerSupportedFormatPerLayer is an array for platforms that have multiple different texture format modes (eg. Android_Multi)

	// Beware GetTargetPlatformBuildSettings and GetBuildSettingsForTargetPlatform are almost code dupes of each other; keep in sync!
	//	todo: refactor to dedupe
	//GetBuildSettingsForTargetPlatform(*this,TargetPlatform,ETextureEncodeSpeed::Final,...);
