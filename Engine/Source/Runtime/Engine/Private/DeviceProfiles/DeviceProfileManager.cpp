...
void OnCVarDeviceProfileLogActiveDeviceProfile(IConsoleVariable*)
{
	UDeviceProfileManager& DPManager = UDeviceProfileManager::Get();
	UE_LOGF(LogDeviceProfileManager, Log, "ActiveDeviceProfileName= %ls", *DPManager.GetActiveDeviceProfileName());

	TArray<TTuple<FName, FString>> DPChain = DPManager.GetDPChain(DPManager.GetActiveDeviceProfileName(), GConfig);

	for (const auto& DP : DPChain)
	{
		const FName& DPName = DP.Get<0>();
		UE_LOGF(LogDeviceProfileManager, Log, "   %ls", *DPName.ToString());
	}
}

static TAutoConsoleVariable<bool> CVarDeviceProfileLogActiveDeviceProfile(
	TEXT("dp.LogActiveDeviceProfile"),
	false,
	TEXT("Logs the result of GetActiveDeviceProfileName and GetDPChain"),
	FConsoleVariableDelegate::CreateStatic(OnCVarDeviceProfileLogActiveDeviceProfile),
	ECVF_Default);
...
static bool GetFragmentCVars(const FString& FragmentName, const FString& CVarArrayName, TArray<FString>& FragmentCVarsINOUT, FConfigCacheIni* ConfigSystem)
{
	...
	if (ConfigSystem->DoesSectionExist(*FragmentSectionName, GDeviceProfilesIni))
	{
		...
		UE_CLOGF(FragmentCVars.Num() > 0, LogInit, Log, "Including %ls from fragment: %ls", *CVarArrayName, *FragmentName);
		FragmentCVarsINOUT += FragmentCVars;
	}
	else
	{
		UE_LOGF(LogInit, Error, "Could not find device profile fragment %ls.", *FragmentName);
		return false;
	}
	return true;
}
...
TMap<FName, FString> UDeviceProfileManager::GatherDeviceProfileCVars(const FString& DeviceProfileName, EDeviceProfileMode GatherMode)
{
	...
	if (GatherMode == EDeviceProfileMode::DPM_CacheValues || GatherMode == EDeviceProfileMode::DPM_CacheValuesIgnoreMatchingRules )
	{
		...
		if (Profile == nullptr)
		{
			UE_LOGF(LogDeviceProfileManager, Log, "Unable to find DeviceProfile '%ls' for gathering cvars for caching. Aborting....", *DeviceProfileName);
			check(Profile);
	...
	while (bReachedEndOfTree == false)
	{
		...
		if (CurrentSection != nullptr)
		{
			...
			for (int Pass = 0; Pass < 2; Pass++)
			{
				...
				// Iterate over the profile and make sure we do not have duplicate CVars
				{
					...
					for (TArray< FString >::TConstIterator CVarIt(CurrentProfilesCVars); CVarIt; ++CVarIt)
					{
						...
						{
							UE_LOGF(LogDeviceProfileManager, VeryVerbose, "ValidCVars: %ls=%ls", *CVarKey, *CVarValue);
					...
					{
						UE_LOGF(LogDeviceProfileManager, VeryVerbose, "CurrentProfilesCVars: %ls=%ls", *ProcessedCVarIt.Key(), *ProcessedCVarIt.Value());
				...
				for (TArray< FString >::TConstIterator CVarIt(CurrentProfilesCVars); CVarIt; ++CVarIt)
				{
					...
					if ((*CVarIt).Split(TEXT("="), &CVarKey, &CVarValue))
					{
						...
						{
							UE_LOGF(LogDeviceProfileManager, VeryVerbose, "DeviceProfileCVars: %ls=%ls", *CVarKey, *CVarValue);
			...
			{
				BaseDeviceProfileName = NextBaseDeviceProfileName;
				UE_LOGF(LogDeviceProfileManager, Log, "Going up to parent DeviceProfile [%ls]", *BaseDeviceProfileName);
			}
			else
			{
...
void UDeviceProfileManager::SetDeviceProfileCVars(const FString& DeviceProfileName)
{
	...
	else
	{
		...
		{
			static FName AllowScalabilityAtRuntimeName = "dp.AllowScalabilityGroupsToChangeAtRuntime";
		...
		for (TPair<FName, FString>& Pair : DeviceProfileCVars)
		{
			...
			if (CVar)
			{
				...
				UE_LOGF(LogDeviceProfileManager, Log, "Pushing Device Profile CVar: [[%ls:%ls -> %ls]]", *CVarKey, *OldSetting.Value, *CVarValue);
			}
			else
			{
				UE_LOGF(LogDeviceProfileManager, Log, "Creating unregistered Device Profile CVar: [[%ls:%ls]]", *CVarKey, *Pair.Value);
			}
	...
	for (const TPair<FString, FString>& Pair : ConfigRules)
	{
		...
		if (Key.StartsWith("cvar_"))
		{
			...
			UE_LOGF(LogDeviceProfileManager, Log, "Setting ConfigRules Device Profile CVar: [[%ls:%ls]]", *CVarKey, *CVarValue);
	...
	for (int i = 0; i<UE_ARRAY_COUNT(DPCVarTags) ; i++)
	{
		...
		for (const TCHAR* Cursor = FCommandLine::Get(); (Cursor != nullptr) && FParse::Value(Cursor, Tag, DPCVarString, false, &Cursor);)
		{
			...
			for (const FString& DPCVar : DPCVars)
			{
				...
				{
					UE_LOGF(LogDeviceProfileManager, Log, "Setting CommandLine Device Profile CVar: [[%ls:%ls]]", *CVarKey, *CVarValue);
	...
	else
	{
		...
		for (TMap<FString, FPushedCVarSetting>::TIterator It(PushedSettings); It; ++It)
		{
			...
			if (CVar)
			{
				...
				{
					UE_LOGF(LogDeviceProfileManager, Warning, "Popping Device Profile CVar skipped because priority has been overridden to higher than ECVF_SetByDeviceProfile since the last push [[%ls:%ls]]", *It.Key(), *Setting.Value);
				}
				else
				{
					// restore it!
					CVar->SetWithCurrentPriority(*Setting.Value);
					UE_LOGF(LogDeviceProfileManager, Log, "Popping Device Profile CVar: [[%ls:%ls]]", *It.Key(), *Setting.Value);
				}
...
void UDeviceProfileManager::ChangeTaggedFragmentState(FName FragmentTag, bool bNewState)
{
	for (FSelectedFragmentProperties& Fragment : PlatformFragmentsSelected)
	{
		if (Fragment.Tag == FragmentTag)
		{
			...
			{
				UE_LOGF(LogInit, Log, "ChangeTaggedFragmentState: %ls=%d", *FragmentTag.ToString(), bNewState);
...
void UDeviceProfileManager::InitializeCVarsForActiveDeviceProfile(bool bPushSettings, bool bIsDeviceProfilePreview, bool bForceReload)
{
	...
	UE_LOGF(LogInit, Log, "Selected Device Profile: [%ls]", *ActiveProfileName);
...
{	
	...
	UE_LOGF(LogDeviceProfileManager, Log, "ReapplyDeviceProfile applying profile: [%ls]", *BaseProfile->GetName());
	...
	{
		UE_LOGF(LogDeviceProfileManager, Log, "ReapplyDeviceProfile applying override profile: [%ls]", *OverrideProfile->GetName());
...
static void TestProfileForCircularReferences(const FString& ProfileName, const FString& ParentName, const FConfigFile &PlatformConfigFile)
{
	...
	while (!CurrentParent.IsEmpty())
	{
		...
		{
			UE_LOGF(LogDeviceProfileManager, Fatal, "Device Profile %ls has a circular dependency on %ls", *ProfileName, *CurrentParent);
		}
		else
		{
...
UDeviceProfile* UDeviceProfileManager::CreateProfile(const FString& ProfileName, const FString& ProfileType, const FString& InSpecifyParentName, const TCHAR* ConfigPlatform)
{
	...
	if (DeviceProfile == NULL)
	{
		...
		if (!PlatformConfigFile->Contains(ProfileName) && !PlatformConfigFile->Contains(ProfileName + " DeviceProfile"))
		{
			...
			UE_LOGF(LogDeviceProfileManager, Display, "Deviceprofile %ls not found.", *ProfileName);
		}
#endif
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UDeviceProfileManager::LoadProfiles);
	...
	if( !HasAnyFlags( RF_ClassDefaultObject ) )
	{
		...
		{
			TRACE_CPUPROFILER_EVENT_SCOPE(UDeviceProfileManager::FindOrLoadPlatformConfig);
		...
		if (!FPlatformProperties::RequiresCookedData())
		{
			...
			for (int32 PlatformIndex = 0; PlatformIndex < TargetPlatforms.Num(); ++PlatformIndex)
			{
				...
				UDeviceProfile * DP = FindProfile(Platform->CookingDeviceProfileName(), false);
				checkf(DP, TEXT("No Device Profile found for %s"), *Platform->CookingDeviceProfileName());

#if ALLOW_OTHER_PLATFORM_CONFIG
				// disable redirect if we are using this for cooking.
				DP->SetPreviewTextureGroupRedirect(nullptr);
#endif

				const UTextureLODSettings* TextureLODSettingsObj = DP->GetTextureLODSettings();
...
void UDeviceProfileManager::SetPreviewDeviceProfile(UDeviceProfile* DeviceProfile, FName PreviewModeTag)
{
	...
	UE_LOGF(LogDeviceProfileManager, Log, "SetPreviewDeviceProfile Start : %ls %ls", *(DeviceProfile->GetName()), *(PreviewModeTag.ToString()) );
...
#if WITH_EDITOR
	{
		...
		UDeviceProfile * CDP = UDeviceProfileManager::Get().FindProfile(
			TargetPlatform->CookingDeviceProfileName());

		// usually Active == CDP
		// but if not then they must be kept in sync
		if ( CDP != ActiveDeviceProfile )
		{
			CDP->SetPreviewTextureGroupRedirect(PreviewDeviceProfile);
		}
	}
#endif
	...
	UE_LOGF(LogDeviceProfileManager, Log, "SetPreviewDeviceProfile End : %ls %ls", *(DeviceProfile->GetName()), *(PreviewModeTag.ToString()) );
...
void UDeviceProfileManager::RestorePreviewDeviceProfile(FName PreviewModeTag)
{
...
#if WITH_EDITOR
	{
		...
		if ( CDP != ActiveDeviceProfile )
		{
			CDP->SetPreviewTextureGroupRedirect(nullptr);
		}
	}
#endif
...
void UDeviceProfileManager::SetOverrideDeviceProfile(UDeviceProfile* DeviceProfile)
{
	...
	UE_LOGF(LogDeviceProfileManager, Log, "Overriding DeviceProfile to %ls, base device profile %ls", *DeviceProfile->GetName(), *BaseDeviceProfile->GetName() );
...
void UDeviceProfileManager::RestoreDefaultDeviceProfile()
{
	...
	if (BaseDeviceProfile)
	{
		UE_LOGF(LogDeviceProfileManager, Log, "Restoring overridden DP back to %ls", *BaseDeviceProfile->GetName());
...
IDeviceProfileSelectorModule* UDeviceProfileManager::GetPreviewDeviceProfileSelectorModule(FConfigCacheIni* PreviewConfigSystemIn)
{
	...
	if (PreviewConfigSystemIn->GetString(TEXT("DeviceProfileManager"), TEXT("PreviewDeviceProfileSelectionModule"), PreviewDeviceProfileSelectionModuleName, GEngineIni))
	{
		...
		if (IDeviceProfileSelectorModule* DPSelectorModule = FModuleManager::LoadModulePtr<IDeviceProfileSelectorModule>(*PreviewDeviceProfileSelectionModuleName))
		{
...
{
	static FString DeviceProfileSelectionModule;
	static bool bReadModuleNameFromConfig = GConfig->GetString(TEXT("DeviceProfileManager"), TEXT("DeviceProfileSelectionModule"), DeviceProfileSelectionModule, GEngineIni);

	if (bReadModuleNameFromConfig)
	{
...
void UDeviceProfileManager::SetActiveDeviceProfile( UDeviceProfile* DeviceProfile )
{
	...
	UE_LOGF(LogDeviceProfileManager, Verbose, "Available device profiles:");
	for (int32 Idx = 0; Idx < Profiles.Num(); ++Idx)
	{
		...
		UE_LOGF(LogDeviceProfileManager, Verbose, "\t[%p][%p %d] %ls, ", Profile, TextureLODGroupsAddr, NumTextureLODGroups, Profile ? *Profile->GetName() : TEXT("None"));
	}
	...
	UE_LOGF(LogDeviceProfileManager, Log, "Active device profile: [%p][%p %d] %ls", ActiveDeviceProfile, TextureLODGroupsAddr, NumTextureLODGroups, ActiveDeviceProfile ? *ActiveDeviceProfile->GetName() : TEXT("None"));
