...
static FName SRC_Chipset("SRC_Chipset");
static FName SRC_MakeAndModel("SRC_MakeAndModel");
static FName SRC_OSVersion("SRC_OSVersion");
static FName SRC_CommandLine("SRC_CommandLine");
static FName SRC_PrimaryGPUDesc("SRC_PrimaryGPUDesc");
static FName SRC_PreviousRegexMatch("SRC_PreviousRegexMatch");
static FName SRC_PreviousRegexMatch1("SRC_PreviousRegexMatch1");

// comparison operators:
static FName CMP_Equal("==");
static FName CMP_Less("<");
static FName CMP_LessEqual("<=");
static FName CMP_Greater(">");
static FName CMP_GreaterEqual(">=");
static FName CMP_NotEqual("!=");
static FName CMP_Regex("CMP_Regex");
static FName CMP_EqualIgnore("CMP_EqualIgnore");
static FName CMP_LessIgnore("CMP_LessIgnore");
static FName CMP_LessEqualIgnore("CMP_LessEqualIgnore");
static FName CMP_GreaterIgnore("CMP_GreaterIgnore");
static FName CMP_GreaterEqualIgnore("CMP_GreaterEqualIgnore");
static FName CMP_NotEqualIgnore("CMP_NotEqualIgnore");
static FName CMP_Hash("CMP_Hash");
static FName CMP_Or("OR");
static FName CMP_And("AND");
...
static bool EvaluateMatch(IDeviceProfileSelectorModule* DPSelectorModule, const FString& RuleName, const FDPMatchingRulestructBase& rule, FRulePropertiesContainer& RuleProperties, FString& SelectedFragmentsOUT, FOutputDevice* Errors)
{
		...
		for (FString& SRCentry : NewSRCs)
		{
			...
			if (SRCentry.Split(TEXT("="), &CSRCType, &CSRCValue))
			{
				...
				else
				{
					...
					UE_LOGF(LogInit, Verbose, "MatchesRules: Adding source %ls : %ls", *CSRCType, *ExpandedValue);
				}
			}
		}
	}
	...
	if (Expression.Num() == 0)
	{
		UE_LOGF(LogInit, Verbose, "MatchesRules: %ls = true (no rules specified)", *RuleName);
		return true;
	}
	...
	UE_LOGF(LogInit, Verbose, "MatchesRules: rule %ls : %ls", *RuleName, *Line);
	...
	for (const FDPMatchingIfCondition& element : Expression)
	{
		...
		{
			if (element.Operator == FName(")"))
			{
				while (Operators.Num())
				{
					FString PoppedOperator = Operators.Pop();
					if (PoppedOperator != FString("("))
					{
	...
	{
		FString PoppedOperator = Operators.Pop();
		if (PoppedOperator == FString("(") || PoppedOperator == FString(")"))
		{
	...
	for (int i = 0; i < RPNOutput.Num(); i++)
	{
		...
		else
		{
			...
			UE_LOGF(LogInit, Verbose, "MatchesRules: rule %ls evaluating {%ls %ls %ls} = {%ls %ls %ls} = %ls", *RuleName, *A.Value, *RPNOutput[i].Value, *B.Value, *crit.Arg1, *RPNOutput[i].Value, *crit.Arg2, *C.Value);
		}
	}
	FExpressionItem Result = ExpressionStack.Pop();
	UE_LOGF(LogInit, Log, "MatchesRules: rule %ls = %ls", *RuleName, *Result.Value);
...
public:
	...
	{
		UE_LOGF(LogInit, Error, "DeviceProfileMatching: Error while parsing Matching Rules (%ls) : %ls", *Stage, V);
		NumErrors++;
	}
};
...
static TArray<FString> GetMatchedRulesArray(const FString& ParentDP, FConfigCacheIni* ConfigSystem)
{
	...
	FString ArrayName("MatchingRules");
...
static FString LoadAndProcessMatchingRulesFromConfig(const FString& ParentDP, IDeviceProfileSelectorModule* DPSelector, FConfigCacheIni* ConfigSystem)
{
	...
	{
		UE_CLOGF(DPMatchingErrorOutput.NumErrors > 0, LogInit, Warning, "DeviceProfileMatching: %d Warnings(s) encountered while processing MatchedRules for %ls", DPMatchingErrorOutput.NumErrors, *ParentDP);
	}
	else
	{
		UE_CLOGF(DPMatchingErrorOutput.NumErrors > 0, LogInit, Error, "DeviceProfileMatching: %d Error(s) encountered while processing MatchedRules for %ls", DPMatchingErrorOutput.NumErrors, *ParentDP);
	}
...
TArray<FString> UDeviceProfileManager::FindAllReferencedFragmentsFromMatchedRules(const FString& ParentDP, FConfigCacheIni* ConfigSystem)
{
	...
	UE_CLOGF(DPMatchingErrorOutput.NumErrors > 0, LogInit, Error, "DeviceProfileMatching: %d Error(s) encountered while processing FindAllReferencedFragments for %ls", DPMatchingErrorOutput.NumErrors, *ParentDP);
...
TArray<FSelectedFragmentProperties> UDeviceProfileManager::FindMatchingFragments(const FString& ParentDP, FConfigCacheIni* ConfigSystem)
{
...
#endif
	{
		...
		if (ConfigSystem != GConfig)
		{
			...
			UE_CLOGF(!PreviewDPSelector && DPHasMatchingRules(ParentDP, ConfigSystem), LogInit, Warning, "Preview DP %ls contains fragment matching rules, but no preview profile selector was found. The selected fragments for %ls will likely not match the behavior of the intended preview device.", *ParentDP, *ParentDP);
		}
#endif
	...
	UE_CLOGF(!SelectedFragments.IsEmpty(), LogInit, Log, "MatchesRules:Fragment string %ls", *SelectedFragments);
	TArray<FSelectedFragmentProperties> MatchedFragments = FragmentStringToFragmentProperties(SelectedFragments);

	UE_CLOGF(MatchedFragments.Num()>0, LogInit, Log, "MatchesRules: MatchedFragments:");
	for (FSelectedFragmentProperties& MatchedFrag : MatchedFragments)
	{
		UE_CLOGF(MatchedFrag.Tag == NAME_None, LogInit, Log, "MatchesRules: %ls, enabled %d", *MatchedFrag.Fragment, MatchedFrag.bEnabled);
		UE_CLOGF(MatchedFrag.Tag != NAME_None, LogInit, Log, "MatchesRules: %ls=%ls, enabled %d", *MatchedFrag.Tag.ToString(), *MatchedFrag.Fragment, MatchedFrag.bEnabled);
	}
