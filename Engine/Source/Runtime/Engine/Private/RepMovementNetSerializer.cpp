...
void FRepMovementNetSerializer::FNetSerializerRegistryDelegates::OnPreFreezeNetSerializerRegistry()
{
	...
	if (!QuantizedTypeMeetRequirements())
	{
		...
		UE_LOGF(LogIris, Error, "FRepMovementNetSerializer will not be registered");
		return;
	}

	if (!IsRepMovementLayoutAsExpected())
	{
		UE_LOGF(LogIris, Error, "FRepMovement layout is unexpected. Things may not replicate as desired.");
	}
...
bool FRepMovementNetSerializer::FNetSerializerRegistryDelegates::IsRepMovementLayoutAsExpected() const
{
	...
	constexpr int ExpectedStructureSize = 136;
	if (!ensureMsgf(RepMovementStruct->GetStructureSize() == ExpectedStructureSize, TEXT("Unexpected FRepMovement structure size. %d != %d"), RepMovementStruct->GetStructureSize(), ExpectedStructureSize))
	{
		return false;
	}
