...
void FLatentActionManager::ProcessLatentActions(UObject* InObject, float DeltaTime)
{
	...
	if (!ActionsToRemoveMap.IsEmpty())
	{
		...
		for (FActionsForObject::TIterator It(ActionsToRemoveMap); It; ++It)
		{
			...
			{
				static FName TransactionalWorkName("FLatentActionManager::ProcessLatentActions_Remove");
				UE::RemoteExecutor::ExecuteTransactional(TransactionalWorkName, [LatentActionWork]{
						LatentActionWork();
					});
			}
			else
	...
	else if (!ObjectToActionListMap.IsEmpty())
	{
		...
		{	
			auto LatentActionWork = [this, DeltaTime, &ObjIt]()
			{
			...
			if (!AutoRTFM::IsClosed())
			{
				static FName TransactionalWorkName("FLatentActionManager::ProcessLatentActions");
				UE::RemoteExecutor::ExecuteTransactional(TransactionalWorkName, [LatentActionWork]{
						LatentActionWork();
					});
			}
	...
	if (GLatentActionDurationLoggingThreshold > 0.0 && Duration >= GLatentActionDurationLoggingThreshold)
	{
		...
		UE_LOGF(LogScript, Warning, "%ls took longer than %f ms when updating %d latent actions!  Dumping all actions that took longer than %f ms.", ANSI_TO_TCHAR(__FUNCTION__), GLatentActionDurationLoggingThreshold * 1000.0, GLatentActionStats.Num(), GLatentActionMinDurationToLog * 1000.0);
		...
		for (int32 i = 0; i < GLatentActionStats.Num(); ++i)
		{
			...
			{
				UE_LOGF(LogScript, Warning, "Class = %ls, Object = %ls, Duration = %f ms", *GLatentActionStats[i].ClassName.ToString(), *GLatentActionStats[i].ObjectName.ToString(), GLatentActionStats[i].Duration * 1000.0);
			}
		}
	}
#endif // LATENT_ACTION_PROFILING_ENABLED
	...
	for (FLatentResponse::FExecutionInfo& LinkInfo : Response.LinksToExecute)
	{
		if (LinkInfo.LinkID != INDEX_NONE)
		{
			if (UObject* CallbackTarget = LinkInfo.CallbackTarget.Get())
			{
				...
				{
					UE_LOGF(LogScript, Warning, "FLatentActionManager::ProcessLatentActions: Could not find latent action resume point named '%ls' on '%ls' called by '%ls'",
						*LinkInfo.ExecutionFunction.ToString(), *(CallbackTarget->GetPathName()), *(InObject->GetPathName()));
				}
			}
			else
			{
				UE_LOGF(LogScript, Warning, "FLatentActionManager::ProcessLatentActions: CallbackTarget is None.");
			}
		}
	}
}
