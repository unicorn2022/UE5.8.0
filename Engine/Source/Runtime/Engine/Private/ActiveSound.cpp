...
void FActiveSound::AddReferencedObjects(FReferenceCollector& Collector)
{
	...
	for (TArray<FSoundSourceBusSendInfo>& SendArray : BusSendsOverride)
	{
		for (FSoundSourceBusSendInfo& BusSend : SendArray)
		{
			Collector.AddReferencedObject(BusSend.AudioBus);
			Collector.AddReferencedObject(BusSend.SoundSourceBus);
		}
	}

	for (FSoundSubmixSendInfo& SendInfo : SoundSubmixSendsOverride)
	{
		Collector.AddReferencedObject(SendInfo.SoundSubmix);
	}
...
void FActiveSound::SetSourceBusSend(EBusSendType BusSendType, const FSoundSourceBusSendInfo& SendInfo)
{
	...
	{
		if ((SendInfo.SoundSourceBus && Info.SoundSourceBus == SendInfo.SoundSourceBus) ||
			(SendInfo.AudioBus && Info.AudioBus == SendInfo.AudioBus))
		{
			Info.SendLevel = SendInfo.SendLevel;
			Info.bEnableLPFCutoff = SendInfo.bEnableLPFCutoff;
			Info.LPFCutoff = SendInfo.LPFCutoff;
			Info.bEnableHPFCutoff = SendInfo.bEnableHPFCutoff;
			Info.HPFCutoff = SendInfo.HPFCutoff;
	...
	if (Sound)
	{
		...
		for (const FSoundSourceBusSendInfo& SendInfo : BusSendsOverride[(int32)BusSendType])
		{
			...
			for (FSoundSourceBusSendInfo& OutSendInfo : OutSends)
			{
				...
				{
					OutSendInfo.SendLevel = SendInfo.SendLevel;
					OutSendInfo.bEnableLPFCutoff = SendInfo.bEnableLPFCutoff;
					OutSendInfo.LPFCutoff = SendInfo.LPFCutoff;
					OutSendInfo.bEnableHPFCutoff = SendInfo.bEnableHPFCutoff;
					OutSendInfo.HPFCutoff = SendInfo.HPFCutoff;
					bOverridden = true;
					break;
				}
			}
...
void FActiveSound::UpdateInterfaceParameters(const TArray<FListener>& InListeners)
{
	...
	FParameterInterfacePtr SourceLocationInterface = SourceLocationInterface::GetInterface();
	FParameterInterfacePtr ListenerLocationInterface = ListenerLocationInterface::GetInterface();
	FParameterInterfacePtr SourceOcclusionInterface = SourceOcclusionInterface::GetInterface();
	FParameterInterfacePtr WorldTimeInterface = WorldTimeInterface::GetInterface();
	...
	bool bImplementsSourceLocation = Sound->ImplementsParameterInterface(SourceLocationInterface);
	bool bImplementsListenerLocation = Sound->ImplementsParameterInterface(ListenerLocationInterface);
	bool bImplementsSourceOcclusion = Sound->ImplementsParameterInterface(SourceOcclusionInterface);
	bool bImplementsWorldTime = Sound->ImplementsParameterInterface(WorldTimeInterface);
	...
	for (const TPair<UPTRINT, FWaveInstance*>& InstancePair : WaveInstances)
	{
		...
		if (const USoundBase* SoundInstance = Instance->WaveData)
		{
			...
			bImplementsSourceLocation |= SoundInstance->ImplementsParameterInterface(SourceLocationInterface);
			bImplementsListenerLocation |= SoundInstance->ImplementsParameterInterface(ListenerLocationInterface);
			bImplementsSourceOcclusion |= SoundInstance->ImplementsParameterInterface(SourceOcclusionInterface);
			bImplementsWorldTime |= SoundInstance->ImplementsParameterInterface(WorldTimeInterface);
		}
	}

	if (false == (bImplementsAttenuation || bImplementsSpatialization || bImplementsSourceOrientation || bImplementsListenerOrientation 
		|| bImplementsSourceLocation || bImplementsListenerLocation || bImplementsSourceOcclusion || bImplementsWorldTime))
	{
		return;
	}
	...
	// World time is available in preview, so we can add params to the update
	if (bImplementsWorldTime)
	{
		const float CurrentWorldTimeSeconds = World.Get() ? World->GetTimeSeconds() : FApp::GetGameTime();
		const float ElapsedWorldTimeSeconds = PlaybackTimeUnscaled;
		const float WorldTimeWhenPlayedSeconds = CurrentWorldTimeSeconds - PlaybackTimeUnscaled;
		
		ParamsToUpdate.Append(
			{
				{ WorldTimeInterface::Inputs::WorldTimeWhenPlayed,  WorldTimeWhenPlayedSeconds},
				{ WorldTimeInterface::Inputs::CurrentWorldTime,  CurrentWorldTimeSeconds},
				{ WorldTimeInterface::Inputs::ElapsedWorldTime,  ElapsedWorldTimeSeconds}
			});
	}

	// If source is not a Preview Sound, resume adding Audio Params related to the Listener's 3D environment
	if (!IsPreviewSound())
	{
		const FListener& Listener = InListeners[ClosestListenerIndex];
		const FVector ListenerLocation = Listener.bUseAttenuationOverride ? Listener.AttenuationOverride : Listener.Transform.GetLocation();
		const FVector SourceLocation = Transform.GetLocation();
		const FVector SourceDirection = SourceLocation - ListenerLocation;
		const bool bSourceOccluded = bIsOccluded;
		...
		if (bImplementsSourceLocation)
		{
			const float SourceX = SourceLocation.X;
			const float SourceY = SourceLocation.Y;
			const float SourceZ = SourceLocation.Z;

			ParamsToUpdate.Append(
				{
					{ SourceLocationInterface::Inputs::X, SourceX },
					{ SourceLocationInterface::Inputs::Y, SourceY },
					{ SourceLocationInterface::Inputs::Z, SourceZ }
				});
		}

		if (bImplementsListenerLocation)
		{
			const float ListenerX = ListenerLocation.X;
			const float ListenerY = ListenerLocation.Y;
			const float ListenerZ = ListenerLocation.Z;

			ParamsToUpdate.Append(
				{
					{ ListenerLocationInterface::Inputs::X, ListenerX },
					{ ListenerLocationInterface::Inputs::Y, ListenerY },
					{ ListenerLocationInterface::Inputs::Z, ListenerZ }
				});
		}

		if (bImplementsSourceOcclusion)
		{
			ParamsToUpdate.Append(
				{
					{ SourceOcclusionInterface::Inputs::IsOccluded, bSourceOccluded }
				});
		}
	}
...
void FActiveSound::UpdateWaveInstances(TArray<FWaveInstance*> &InWaveInstances, const float DeltaTime)
{
	...
	if (FadeOut == EFadeOut::None || ComponentVolumeFader.IsActive())
	{
		...
		// Allow subsystems one last opportunity to adjust ParseParams
		AudioDevice->SubsystemsAdjustParseParams(*this, ParseParams);
...
void FActiveSound::UpdateAttenuation(float DeltaTime, FSoundParseParameters& ParseParams, int32 ListenerIndex, const FSoundAttenuationSettings* SettingsAttenuationNode)
{
	...
	if (Settings->bEnableSourceDataOverride)
	{
