...
public:
	...
	EShaderFrequencyMask ShaderFrequencyMask = 0;
};
...
{
	UObject* IndexedTexture = Material.GetMatDefaultTextureAtIdx(TextureIndex);
...
inline UTextureCollection* GetIndexedTextureCollection(const FMaterial& Material, int32 TextureCollectionIndex)
{
	...
	if (IndexedTextureCollection == nullptr)
	{
		...
		if (!bWarnedOnce)
		{
			UE_LOGF(LogMaterial, Warning, "%ls: Requesting an invalid TextureIndex! (%u / %u)", *Material.GetFriendlyName(), TextureCollectionIndex, ReferencedTextureCollections.Num());
			bWarnedOnce = true;
		}
	}
