	...
	const FExpressionInput& ShadingModel)
{
	return SubstrateGenerateMaterialTopologyTreeCommon(
		Compiler->GetSubstrateTranslatorData(), ThisExpressionGuid, Parent, OutputIndex,
		EmissiveColor,
		Anisotropy,
		ClearCoatNormal,
		CustomTangent,
...
FSubstrateOperator* UMaterialExpressionSubstrateShadingModels::SubstrateGenerateMaterialTopologyTreeCommon(
	struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, struct FGuid ThisExpressionGuid, class UMaterialExpression* Parent, int32 OutputIndex,
	const FExpressionInput& EmissiveColor,
	const FExpressionInput& Anisotropy,
	const FExpressionInput& ClearCoatNormal,
	const FExpressionInput& CustomTangent,
	const FExpressionInput& ShadingModel)
{
	...
	int32 ThicknessIndex = SubstrateTranslatorData.SubstrateThicknessStackGetThicknessIndex();

	auto AddDefaultWorstCase = [&](ESubstrateBsdfFeature In)
	{
		FSubstrateOperator& SlabOperator = SubstrateTranslatorData.SubstrateCompilationRegisterOperator(SUBSTRATE_OPERATOR_BSDF_LEGACY, SubstrateTranslatorData.SubstrateTreeStackGetPathUniqueId(), ThisExpressionGuid, Parent, SubstrateTranslatorData.SubstrateTreeStackGetParentPathUniqueId());
	...
	FMaterialShadingModelField ShadingModels = SubstrateTranslatorData.GetMaterialShadingModels();
	...
	if ((ShadingModels.CountShadingModels() > 1) || bHasShadingModelFromExpression)
	{
		...
		{
			FSubstrateOperator& Operator = SubstrateTranslatorData.SubstrateCompilationRegisterOperator(SUBSTRATE_OPERATOR_BSDF_LEGACY, SubstrateTranslatorData.SubstrateTreeStackGetPathUniqueId(), ThisExpressionGuid, Parent, SubstrateTranslatorData.SubstrateTreeStackGetParentPathUniqueId());
...
FSubstrateOperator* UMaterialExpressionSubstrateShadingModels::SubstrateGenerateMaterialTopologyTree(struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	return SubstrateGenerateMaterialTopologyTreeCommon(
		SubstrateTranslatorData, this->MaterialExpressionGuid, Parent, OutputIndex,
		EmissiveColor,
		Anisotropy,
		ClearCoatNormal,
		CustomTangent,
...
FSubstrateOperator* UMaterialExpressionSubstrateSlabBSDF::SubstrateGenerateMaterialTopologyTree(
	struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	FSubstrateOperator& SubstrateOperator = SubstrateTranslatorData.SubstrateCompilationRegisterOperator(SUBSTRATE_OPERATOR_BSDF, SubstrateTranslatorData.SubstrateTreeStackGetPathUniqueId(), this->MaterialExpressionGuid, Parent, SubstrateTranslatorData.SubstrateTreeStackGetParentPathUniqueId());
	...
	SubstrateOperator.ThicknessIndex = SubstrateTranslatorData.SubstrateThicknessStackGetThicknessIndex();
...
void UMaterialExpressionSubstrateSimpleClearCoatBSDF::GatherSubstrateMaterialInfo(FSubstrateMaterialInfo& SubstrateMaterialInfo, int32 OutputIndex)
{
	...
	SubstrateMaterialInfo.AddShadingModel(SSM_ClearCoat);
	SubstrateMaterialInfo.AddGuid(MaterialExpressionGuid);
}

FSubstrateOperator* UMaterialExpressionSubstrateSimpleClearCoatBSDF::SubstrateGenerateMaterialTopologyTree(
	struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	FSubstrateOperator& SubstrateOperator = SubstrateTranslatorData.SubstrateCompilationRegisterOperator(SUBSTRATE_OPERATOR_BSDF, SubstrateTranslatorData.SubstrateTreeStackGetPathUniqueId(), this->MaterialExpressionGuid, Parent, SubstrateTranslatorData.SubstrateTreeStackGetParentPathUniqueId());
	SubstrateOperator.BSDFType = SUBSTRATE_BSDF_TYPE_SLAB;
...
{
	FString Caption = "Substrate Volumetric-Fog-Cloud BSDF";
	if (bEmissiveOnly)
	{
...
FSubstrateOperator* UMaterialExpressionSubstrateVolumetricFogCloudBSDF::SubstrateGenerateMaterialTopologyTreeCommon(
	...
	const FExpressionInput& AmbientOcclusion)
{
	FSubstrateOperator& SubstrateOperator = SubstrateTranslatorData.SubstrateCompilationRegisterOperator(SUBSTRATE_OPERATOR_BSDF, SubstrateTranslatorData.SubstrateTreeStackGetPathUniqueId(), ThisExpressionGuid, Parent, SubstrateTranslatorData.SubstrateTreeStackGetParentPathUniqueId());
...
FSubstrateOperator* UMaterialExpressionSubstrateVolumetricFogCloudBSDF::SubstrateGenerateMaterialTopologyTree(struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	return SubstrateGenerateMaterialTopologyTreeCommon(
		SubstrateTranslatorData, this->MaterialExpressionGuid, Parent, OutputIndex,
		EmissiveColor,
...
FSubstrateOperator* UMaterialExpressionSubstrateLightFunction::SubstrateGenerateMaterialTopologyTreeCommon(
	class FMaterialCompiler* Compiler, struct FGuid ThisExpressionGuid, class UMaterialExpression* Parent, int32 OutputIndex)
{
	return SubstrateGenerateMaterialTopologyTreeCommon(Compiler->GetSubstrateTranslatorData(), ThisExpressionGuid, Parent, OutputIndex);
}
...
FSubstrateOperator* UMaterialExpressionSubstrateLightFunction::SubstrateGenerateMaterialTopologyTree(struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	return SubstrateGenerateMaterialTopologyTreeCommon(SubstrateTranslatorData, this->MaterialExpressionGuid, Parent, OutputIndex);
}
...
FSubstrateOperator* UMaterialExpressionSubstratePostProcess::SubstrateGenerateMaterialTopologyTree(struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	return SubstrateGenerateMaterialTopologyTreeCommon(SubstrateTranslatorData, this->MaterialExpressionGuid, Parent, OutputIndex);
}
#endif // WITH_EDITOR
...
FSubstrateOperator* UMaterialExpressionSubstrateUI::SubstrateGenerateMaterialTopologyTree(struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	return SubstrateGenerateMaterialTopologyTreeCommon(SubstrateTranslatorData, this->MaterialExpressionGuid, Parent, OutputIndex);
}
#endif // WITH_EDITOR
...
FSubstrateOperator* UMaterialExpressionSubstrateConvertToDecal::SubstrateGenerateMaterialTopologyTree(
	struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	const bool bUseParameterBlending = true;
	FSubstrateOperator& SubstrateOperator = SubstrateTranslatorData.SubstrateCompilationRegisterOperator(SUBSTRATE_OPERATOR_WEIGHT, SubstrateTranslatorData.SubstrateTreeStackGetPathUniqueId(), this->MaterialExpressionGuid, Parent, SubstrateTranslatorData.SubstrateTreeStackGetParentPathUniqueId(), bUseParameterBlending);
	SubstrateOperator.SubUsage = SUBSTRATE_OPERATOR_SUBUSAGE_DECAL;
	if (SubstrateTranslatorData.GetSubstrateTreeOutOfStackDepthOccurred())
	{
	...
	{
		SubstrateTranslatorData.SubstrateTreeStackPush(this, 0);
		OpA = ChildDecalMaterialExpression->SubstrateGenerateMaterialTopologyTree(SubstrateTranslatorData, this, TracedInput.OutputIndex);
...
{
	FSubstrateOperator& SubstrateOperator = Compiler->SubstrateCompilationGetOperator(Compiler->SubstrateTreeStackGetPathUniqueId());
	const int32 EmissiveColorIdx = CompileWithDefaultFloat3(Compiler, EmissiveColor, 0.0f, 0.0f, 0.0f);
	const int32 TransmittanceColorIdx = CompileWithDefaultFloat3(Compiler, TransmittanceColor, 1.0f, 1.0f, 1.0f);
	const int32 NormalIdx = CompileWithDefaultNormalWS(Compiler, Normal);

	int32 OutputCodeChunk = Compiler->SubstrateUnlitBSDF(
		EmissiveColorIdx,
		TransmittanceColorIdx,
		NormalIdx,
		&SubstrateOperator);
	return OutputCodeChunk;
}
...
FSubstrateOperator* UMaterialExpressionSubstrateUnlitBSDF::SubstrateGenerateMaterialTopologyTree(
	struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	FSubstrateOperator& SubstrateOperator = SubstrateTranslatorData.SubstrateCompilationRegisterOperator(SUBSTRATE_OPERATOR_BSDF, SubstrateTranslatorData.SubstrateTreeStackGetPathUniqueId(), this->MaterialExpressionGuid, Parent, SubstrateTranslatorData.SubstrateTreeStackGetParentPathUniqueId());
	SubstrateOperator.BSDFType = SUBSTRATE_BSDF_TYPE_UNLIT;
...
FSubstrateOperator* UMaterialExpressionSubstrateHairBSDF::SubstrateGenerateMaterialTopologyTree(
	struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	FSubstrateOperator& SubstrateOperator = SubstrateTranslatorData.SubstrateCompilationRegisterOperator(SUBSTRATE_OPERATOR_BSDF, SubstrateTranslatorData.SubstrateTreeStackGetPathUniqueId(), this->MaterialExpressionGuid, Parent, SubstrateTranslatorData.SubstrateTreeStackGetParentPathUniqueId());
...
FSubstrateOperator* UMaterialExpressionSubstrateEyeBSDF::SubstrateGenerateMaterialTopologyTree(
	struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	FSubstrateOperator& SubstrateOperator = SubstrateTranslatorData.SubstrateCompilationRegisterOperator(SUBSTRATE_OPERATOR_BSDF, SubstrateTranslatorData.SubstrateTreeStackGetPathUniqueId(), this->MaterialExpressionGuid, Parent, SubstrateTranslatorData.SubstrateTreeStackGetParentPathUniqueId());
...
FSubstrateOperator* UMaterialExpressionSubstrateSingleLayerWaterBSDF::SubstrateGenerateMaterialTopologyTree(
	struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	FSubstrateOperator& SubstrateOperator = SubstrateTranslatorData.SubstrateCompilationRegisterOperator(SUBSTRATE_OPERATOR_BSDF, SubstrateTranslatorData.SubstrateTreeStackGetPathUniqueId(), this->MaterialExpressionGuid, Parent, SubstrateTranslatorData.SubstrateTreeStackGetParentPathUniqueId());
...
UMaterialExpressionSubstrateToonBSDF::UMaterialExpressionSubstrateToonBSDF(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	struct FConstructorStatics
	{
...
#if WITH_EDITOR
int32 UMaterialExpressionSubstrateToonBSDF::Compile(class FMaterialCompiler* Compiler, int32 OutputIndex)
{
	int32 NormalCodeChunk = CompileWithDefaultNormalWS(Compiler, Normal);
	const FSubstrateRegisteredSharedLocalBasis NewRegisteredSharedLocalBasis = SubstrateCompilationInfoCreateSharedLocalBasis(Compiler, NormalCodeChunk);
	...
	int32 ToonProfileIdCodeChunk = INDEX_NONE;
	if (ToonProfile != nullptr)
	{
		const FName ToonProfileParameterName = ToonProfile::CreateToonProfileParameterName(ToonProfile);
		ToonProfileIdCodeChunk = Compiler->ForceCast(Compiler->ScalarParameter(ToonProfileParameterName, 1.0f), MCT_Float1);
	}
	if (ToonProfileIdCodeChunk == INDEX_NONE)
	{
		ToonProfileIdCodeChunk = Compiler->Constant(0.0f);
	}

	// Unplugged PatternUVs result in UV0 being used, scaled by 10 to make offset/hatching textures visible by default.
	int32 PatternUVsCodeChunk = PatternUVs.GetTracedInput().Expression ? PatternUVs.Compile(Compiler) : Compiler->Mul(Compiler->TextureCoordinate(0, false, false), Compiler->Constant(10.0f));

	int32 OutputCodeChunk = Compiler->SubstrateToonBSDF(
		ToonProfileIdCodeChunk,
		...
		CompileWithDefaultTangentWS(Compiler, Tangent),
		Compiler->GetSubstrateSharedLocalBasisIndexMacro(NewRegisteredSharedLocalBasis),
		!SubstrateOperator.bUseParameterBlending || (SubstrateOperator.bUseParameterBlending && SubstrateOperator.bRootOfParameterBlendingSubTree) ? &SubstrateOperator : nullptr);
...
void UMaterialExpressionSubstrateToonBSDF::GetCaption(TArray<FString>& OutCaptions) const
{
	OutCaptions.Add(TEXT("Substrate Toon BSDF - EXPERIMENTAL"));
}

EMaterialValueType UMaterialExpressionSubstrateToonBSDF::GetOutputValueType(int32 OutputIndex)
{
	return MCT_Substrate;
}

EMaterialValueType UMaterialExpressionSubstrateToonBSDF::GetInputValueType(int32 InputIndex)
{
	switch (InputIndex)
	{
	case 0:
...
bool UMaterialExpressionSubstrateToonBSDF::IsResultSubstrateMaterial(int32 OutputIndex)
{
	return true;
}

void UMaterialExpressionSubstrateToonBSDF::GatherSubstrateMaterialInfo(FSubstrateMaterialInfo& SubstrateMaterialInfo, int32 OutputIndex)
{
	SubstrateMaterialInfo.AddShadingModel(SSM_Toon);
	SubstrateMaterialInfo.AddGuid(MaterialExpressionGuid);

	if (ToonProfile != nullptr)
	{
		SubstrateMaterialInfo.AddToonProfile(ToonProfile);
	}

	if (Anisotropy.IsConnected())
	{
		SubstrateMaterialInfo.AddPropertyConnected(MP_Anisotropy);
	}
}

FSubstrateOperator* UMaterialExpressionSubstrateToonBSDF::SubstrateGenerateMaterialTopologyTree(
	struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	FSubstrateOperator& SubstrateOperator = SubstrateTranslatorData.SubstrateCompilationRegisterOperator(SUBSTRATE_OPERATOR_BSDF, SubstrateTranslatorData.SubstrateTreeStackGetPathUniqueId(), this->MaterialExpressionGuid, Parent, SubstrateTranslatorData.SubstrateTreeStackGetParentPathUniqueId());
	SubstrateOperator.BSDFType = SUBSTRATE_BSDF_TYPE_TOON;
	SubstrateOperator.ThicknessIndex = SubstrateTranslatorData.SubstrateThicknessStackGetThicknessIndex();
	SubstrateOperator.bBSDFWritesEmissive = EmissiveColor.IsConnected();
	SubstrateOperator.BSDFFeatures |= ESubstrateBsdfFeature::Toon;

	if (Anisotropy.IsConnected())
	{
		SubstrateOperator.BSDFFeatures |= ESubstrateBsdfFeature::Anisotropy;
	}
...
FSubstrateOperator* UMaterialExpressionSubstrateHorizontalMixing::SubstrateGenerateMaterialTopologyTree(
	struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	FSubstrateOperator& SubstrateOperator = SubstrateTranslatorData.SubstrateCompilationRegisterOperator(SUBSTRATE_OPERATOR_HORIZONTAL, SubstrateTranslatorData.SubstrateTreeStackGetPathUniqueId(), this->MaterialExpressionGuid, Parent, SubstrateTranslatorData.SubstrateTreeStackGetParentPathUniqueId(), bUseParameterBlending);
	if (SubstrateTranslatorData.GetSubstrateTreeOutOfStackDepthOccurred())
	{
		return &SubstrateOperator; // Out ot stack space, return now to fail the compilation
	}
	...
	{
		SubstrateTranslatorData.SubstrateTreeStackPush(this, 0);
		OpA = ChildAExpression->SubstrateGenerateMaterialTopologyTree(SubstrateTranslatorData, this, TracedInputA.OutputIndex);
		SubstrateTranslatorData.SubstrateTreeStackPop();
		AssignOperatorIndexIfNotNull(SubstrateOperator.LeftIndex, OpA);
	}
	if (ChildBExpression)
	{
		SubstrateTranslatorData.SubstrateTreeStackPush(this, 1);
		OpB = ChildBExpression->SubstrateGenerateMaterialTopologyTree(SubstrateTranslatorData, this, TracedInputB.OutputIndex);
		SubstrateTranslatorData.SubstrateTreeStackPop();
...
FSubstrateOperator* UMaterialExpressionSubstrateVerticalLayering::SubstrateGenerateMaterialTopologyTree(
	struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	FSubstrateOperator& SubstrateOperator = SubstrateTranslatorData.SubstrateCompilationRegisterOperator(SUBSTRATE_OPERATOR_VERTICAL, SubstrateTranslatorData.SubstrateTreeStackGetPathUniqueId(), this->MaterialExpressionGuid, Parent, SubstrateTranslatorData.SubstrateTreeStackGetParentPathUniqueId(), bUseParameterBlending);
	if (SubstrateTranslatorData.GetSubstrateTreeOutOfStackDepthOccurred())
	{
		return &SubstrateOperator; // Out ot stack space, return now to fail the compilation
	}
	...
	if (ChildAExpression)
	{
		SubstrateTranslatorData.SubstrateThicknessStackPush(this, &Thickness);
		SubstrateTranslatorData.SubstrateTreeStackPush(this, 0);
		OpA = ChildAExpression->SubstrateGenerateMaterialTopologyTree(SubstrateTranslatorData, this, TracedInputTop.OutputIndex);
		SubstrateTranslatorData.SubstrateTreeStackPop();
	...
	{
		SubstrateTranslatorData.SubstrateTreeStackPush(this, 1);
		OpB = ChildBExpression->SubstrateGenerateMaterialTopologyTree(SubstrateTranslatorData, this, TracedInputBase.OutputIndex);
		SubstrateTranslatorData.SubstrateTreeStackPop();
		AssignOperatorIndexIfNotNull(SubstrateOperator.RightIndex, OpB);
	}
	CombineFlagForParameterBlending(SubstrateOperator, OpA, OpB);
...
FSubstrateOperator* UMaterialExpressionSubstrateAdd::SubstrateGenerateMaterialTopologyTree(
	struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	FSubstrateOperator& SubstrateOperator = SubstrateTranslatorData.SubstrateCompilationRegisterOperator(SUBSTRATE_OPERATOR_ADD, SubstrateTranslatorData.SubstrateTreeStackGetPathUniqueId(), this->MaterialExpressionGuid, Parent, SubstrateTranslatorData.SubstrateTreeStackGetParentPathUniqueId(), bUseParameterBlending);
	if (SubstrateTranslatorData.GetSubstrateTreeOutOfStackDepthOccurred())
	{
		return &SubstrateOperator; // Out ot stack space, return now to fail the compilation
	}
...
FSubstrateOperator* UMaterialExpressionSubstrateWeight::SubstrateGenerateMaterialTopologyTree(
	struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	FSubstrateOperator& SubstrateOperator = SubstrateTranslatorData.SubstrateCompilationRegisterOperator(SUBSTRATE_OPERATOR_WEIGHT, SubstrateTranslatorData.SubstrateTreeStackGetPathUniqueId(), this->MaterialExpressionGuid, Parent, SubstrateTranslatorData.SubstrateTreeStackGetParentPathUniqueId());
	if (SubstrateTranslatorData.GetSubstrateTreeOutOfStackDepthOccurred())
	{
		return &SubstrateOperator; // Out ot stack space, return now to fail the compilation
	}
...
int32 UMaterialExpressionSubstrateSelect::Compile(class FMaterialCompiler* Compiler, int32 OutputIndex)
{
	...
	{
		const FString ErroneousInput = (!OperatorA && !OperatorB) ? TEXT("A and B") : (!OperatorA ? TEXT("A") : TEXT("B"));
		return Compiler->Errorf(TEXT("Substrate Select node has invalid input %s."), *ErroneousInput);
	}
	if (OperatorA->BSDFRegisteredSharedLocalBasis.NormalCodeChunk == INDEX_NONE || OperatorB->BSDFRegisteredSharedLocalBasis.NormalCodeChunk == INDEX_NONE)
	{
		return Compiler->Errorf(TEXT("Substrate Select node requires both inputs to have a valid local basis. Unlit BSDF does not register a local basis, so it cannot be used with the Select node."));
	}
...
FSubstrateOperator* UMaterialExpressionSubstrateSelect::SubstrateGenerateMaterialTopologyTree(
	struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	FSubstrateOperator& SubstrateOperator = SubstrateTranslatorData.SubstrateCompilationRegisterOperator(SUBSTRATE_OPERATOR_SELECT, SubstrateTranslatorData.SubstrateTreeStackGetPathUniqueId(), this->MaterialExpressionGuid, Parent, SubstrateTranslatorData.SubstrateTreeStackGetParentPathUniqueId(), bUseParameterBlending);
	if (SubstrateTranslatorData.GetSubstrateTreeOutOfStackDepthOccurred())
	{
		return &SubstrateOperator; // Out ot stack space, return now to fail the compilation
	}
	...
	FExpressionInput& ClearCoatNormal, FExpressionInput& CustomTangent)
{
	...
	// normal (e.g., vec2 normal), and avoid crashing the material compilation. The error will still be reported by the compiler up 
	// to the user, but the compilation will succeed.
	// This is done before TransformNormalFromRequestedBasisToWorld because TransformNormalFromRequestedBasisToWorld(INDEX_NONE) would crash inside GetParameterCode.
	if (NormalCodeChunk == INDEX_NONE) 
	{
		NormalCodeChunk = Compiler->VertexNormal();
	}
	...
	if (bHasAnisotropy)
	{
		...
		if (TangentCodeChunk == INDEX_NONE)
		{
			TangentCodeChunk = Compiler->VertexTangent(); // Similar fix as for NormalCodeChunk above.
		}
...
{
	return SubstrateGenerateMaterialTopologyTreeCommon(Compiler->GetSubstrateTranslatorData(), ThisExpressionGuid, Parent, OutputIndex,
		CachedConnectedMaterialAttributesInputs, bShadingModelFromMaterialExpression, bIsEmissiveConnected);
}
...
FSubstrateOperator* UMaterialExpressionSubstrateConvertMaterialAttributes::SubstrateGenerateMaterialTopologyTree(struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	const uint64 Cached = GetConnectedMaterialAttributesInputs(Material);

	return SubstrateGenerateMaterialTopologyTreeCommon(
		SubstrateTranslatorData, MaterialExpressionGuid, Parent, OutputIndex,
