					...
					if (Function)
					{
						...
						else
						{
							...
							*/
							...
							{
								UE_LOGF(LogMaterial, Warning, "%ls not appended because available %ls already contain the base (%ls).", *Function->GetName(), *FunctionType, *BaseFunction->GetName());
								Function = nullptr;
							}
							else
							{
								UE_LOGF(LogMaterial, Warning, "Resolving base (%ls) of %ls for the available %ls list.", *BaseFunction->GetName(), *Function->GetName(), *FunctionType);
...
void UMaterialExpressionLayerStack::RebuildLayerGraph(bool bReportErrors)
{
				...
				for (int32 LayerIndex = NumActiveLayerCallers - 2; LayerIndex >= 0; LayerIndex--)
				{
					...
					for (int32 CallInputIndex = 0; CallInputIndex < CurrentLayer->FunctionInputs.Num(); CallInputIndex++)
					{
						...
						if (!CurrentInput.ExpressionInput || CurrentInputType != MCT_MaterialAttributes)
						{
							...
							{
								UE_LOGF(LogMaterial, Warning, "LayerStack's layer-only graph chain in \"%ls\" is cut short due to missing MaterialAttributes input in %ls.",
	...
	{
		UE_LOGF(LogMaterial, Warning, "Failed to build LayerStack graph for %ls.", Material ? *(Material->GetName()) : TEXT("Unknown"));
	}
}
