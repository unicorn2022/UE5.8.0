...
ENGINE_API extern bool bEnableRestrictiveMaterialMode;
...
static TAutoConsoleVariable<int32> CVarMaterialUseShaderCompilationParameters(
	TEXT("r.Material.UseShaderCompilationParameters"),
	1,
	TEXT("Controls whether the parameters used for determining which shaders to compile are based on translation output or conservative material data."),
...
IMPLEMENT_TYPE_LAYOUT(FCompactionMetaDataSegment);
IMPLEMENT_TYPE_LAYOUT(FCompactUniformExpressionSet);
...
FExpressionOutput* FExpressionInput::GetConnectedOutput() const
{
	if (IsConnected())
	{
		TArray<FExpressionOutput>& ExpressionOutputs = Expression->GetOutputs();
		return ExpressionOutputs.IsValidIndex(OutputIndex) ? &ExpressionOutputs[OutputIndex] : nullptr;
	}
	return nullptr;
}
...
FString FVector2MaterialInput::GetDefaultValue() const
{
	FString DefaultValue;
#if WITH_EDITOR
	DefaultValue = FString("(X=") + FString::SanitizeFloat(Constant.X) + FString(",Y=") + FString::SanitizeFloat(Constant.Y) + FString(")");
#endif
	return DefaultValue;
}
...
FString FVectorMaterialInput::GetDefaultValue() const
{
	FString DefaultValue;
#if WITH_EDITOR
	DefaultValue = FString::SanitizeFloat(Constant.X) + FString(",") + FString::SanitizeFloat(Constant.Y) + FString(",") + FString::SanitizeFloat(Constant.Z);
#endif
	return DefaultValue;
}
...
// As present in UMaterialGraphNode_Root::UpdateInputUseConstant, which fixes up Materials on modification.
// This ensures Materials which have not been fixed up don't cause issues.
static inline bool IsValidPropertyForInlineConst(const EMaterialProperty InProp)
{
	return InProp != MP_WorldPositionOffset && InProp != MP_Anisotropy && InProp != MP_PixelDepthOffset && InProp != MP_Displacement;
}

int32 FColorMaterialInput::CompileWithDefault(class FMaterialCompiler* Compiler, EMaterialProperty Property)
{
	const bool bValidPropertyForInlineConstant = IsValidPropertyForInlineConst(Property);

	if (UseConstant && bValidPropertyForInlineConstant)
	{
...
{ 
	...
	if (bLoadedCookedShaderMapId)
	{
		...
		{
			UE_LOGF(LogMaterial, Fatal, "Tried to access cooked shader map ID from unknown thread");
		}
	}
	else
	{
		...
		OutId.BaseMaterialPathHash = FXxHash64::HashBuffer(Args.BaseMaterialPath.GetCharArray().GetData(), Args.BaseMaterialPath.NumBytesWithoutNull()).Hash;
		...
		OutId.TextureReferencesHash = GetReferencedTexturesHash();
		OutId.ExpressionIncludesHash = GetExpressionIncludesHash();
		OutId.ExternalCodeReferencesHash = GetExternalCodeReferencesHash();
		...
		{
			UE_LOGF(LogMaterial, Error, "FMaterial::GetShaderMapId: TargetPlatform is not null, but a cooked executable cannot target platforms other than its own.");
		}
		OutId.LayoutParams.InitializeForCurrent();

		UE_LOGF(LogMaterial, Error, "Tried to access an uncooked shader map ID in a cooked application");
#endif
	}
}
...
{
	// Conservative estimate based on expressions present in graph.
	// This function is only intended to be used for DDC Key/Iterative Cooking
...
{
	checkSlow(IsInGameThread() || IsInAsyncLoadingThread() || IsInParallelLoadingThread());
...
#if !WITH_EDITOR
		FGuid Id_DEPRECATED;
		UE_LOGF(LogMaterial, Error, "Attempted to serialize legacy material data at runtime, this content should be re-saved and re-cooked");
#endif	
		Ar << Id_DEPRECATED;
...
void FMaterial::SerializeInlineShaderMap(FArchive& Ar, const FName& SerializingAsset)
{
	...
	{
		UE_LOGF(LogShaders, Fatal, "This platform requires cooked packages, and shaders were not cooked into this material %ls.", *GetFriendlyName());
	}
	...
	if (bCooked)
	{
		if (Ar.IsCooking())
		{
			...
			else
			{
				...
				FString InvalidShadermapReason;
				
				if (!GameThreadShaderMap)
				{
					InvalidShadermapReason += TEXT("Shadermap pointer is null.");
				}
				else
				{
					if (!GameThreadShaderMap->CompiledSuccessfully())
					{
						InvalidShadermapReason += TEXT("Shadermap exists but wasn't compiled successfully (yet?)");
					}
					else
					{
						if (HasAnyUsage())
						{
							InvalidShadermapReason += TEXT("Shadermap exists, has usage flags but has no shaders");
						}
						else
						{
							UE_LOGF(LogMaterial, Log, "Material resource (in %ls hierarchy) has no usage flags and will have an empty ShaderMap.",
								*GetFriendlyName());
							bShouldWarn = false;
						}
					}
				...
				if (bShouldWarn)
				{
					UE_LOGF(LogMaterial, Warning, "Cooking a material resource (in %ls hierarchy) that doesn't have a valid ShaderMap! %ls",
						*GetFriendlyName(), *InvalidShadermapReason);
				}
			}
#else
			UE_LOGF(LogMaterial, Fatal, "Internal error: cooking outside the editor is not possible.");
			// unreachable
#endif
		}
		else
		{
			...
			{
				UE_LOGF(LogMaterial, Error, "Loading a material resource %ls with an invalid ShaderMap!", *GetFriendlyName());
			}
		}
	}
}
...
TObjectPtr<UObject> FMaterialResource::GetMatDefaultTextureAtIdx(const int32 InIdx) const
{
	if (MaterialInstance)
	{
		TObjectPtr<UObject> Texture =  MaterialInstance->GetDefaultTextureAtIdx(InIdx);
		if (Texture)
		{
			return Texture;
		}
	...
	if (Material)
	{
		return Material->GetDefaultTextureAtIdx(InIdx);
	}

	return UMaterial::GetDefaultMaterial(MD_Surface)->GetDefaultTextureAtIdx(InIdx);
}

#if WITH_EDITOR
int32 FMaterialResource::GetMatDefaultTextureIdx(TObjectPtr<UObject> InTexture) const
{
	if (MaterialInstance)
	{
		const int32 Idx = MaterialInstance->GetDefaultTextureIdx(InTexture);
		if (Idx != INDEX_NONE)
		{
			return Idx;
		}
	...
	if (Material)
	{
		return Material->GetDefaultTextureIdx(InTexture);
	}

	return UMaterial::GetDefaultMaterial(MD_Surface)->GetDefaultTextureIdx(InTexture);
}

const TArray<TObjectPtr<UObject>>& FMaterialResource::GetMatDefaultTextures() const
{
	if (MaterialInstance)
	{
		const TArray<TObjectPtr<UObject>>& Textures = MaterialInstance->GetDefaultTextures();
		if (Textures.Num() > 0)
		{
			return Textures;
		}
	...
	if (Material)
	{
		return Material->GetDefaultTextures();
	}

	return UMaterial::GetDefaultMaterial(MD_Surface)->GetDefaultTextures();
}
#endif // WITH_EDITOR
...
{
	UE_CLOGF(IsOwnerBeginDestroyed(), LogMaterial, Error, "ReleaseShaderMap called on FMaterial %ls, owner is BeginDestroyed", *GetDebugName());
...
bool FMaterialResource::IsVariableRateShadingAllowed() const { return Material->bAllowVariableRateShading; }
...
{
	return MaterialInstance ? MaterialInstance->GetUsageByFlag(MATUSAGE_EditorCompositing) : Material->GetUsageByFlag(MATUSAGE_EditorCompositing);
}

bool FMaterialResource::IsUsedWithSkeletalMesh() const
{
	return MaterialInstance ? MaterialInstance->GetUsageByFlag(MATUSAGE_SkeletalMesh) : Material->GetUsageByFlag(MATUSAGE_SkeletalMesh);
}

bool FMaterialResource::IsUsedWithGeometryCache() const
{
	return MaterialInstance ? MaterialInstance->GetUsageByFlag(MATUSAGE_GeometryCache) : Material->GetUsageByFlag(MATUSAGE_GeometryCache);
}

bool FMaterialResource::IsUsedWithWater() const
{
	return MaterialInstance ? MaterialInstance->GetUsageByFlag(MATUSAGE_Water) : Material->GetUsageByFlag(MATUSAGE_Water);
}

bool FMaterialResource::IsUsedWithHairStrands() const
{
	return MaterialInstance ? MaterialInstance->GetUsageByFlag(MATUSAGE_HairStrands) : Material->GetUsageByFlag(MATUSAGE_HairStrands);
}

bool FMaterialResource::IsUsedWithLidarPointCloud() const
{
	return MaterialInstance ? MaterialInstance->GetUsageByFlag(MATUSAGE_LidarPointCloud) : Material->GetUsageByFlag(MATUSAGE_LidarPointCloud);
}

bool FMaterialResource::IsUsedWithVirtualHeightfieldMesh() const
{
	return MaterialInstance ? MaterialInstance->GetUsageByFlag(MATUSAGE_VirtualHeightfieldMesh) : Material->GetUsageByFlag(MATUSAGE_VirtualHeightfieldMesh);
}
...
bool FMaterialResource::IsUsedWithNeuralNetworks() const
{
	...
	return MaterialInstance ? MaterialInstance->GetUsageByFlag(MATUSAGE_NeuralNetworks) : Material->GetUsageByFlag(MATUSAGE_NeuralNetworks);
}

bool FMaterialResource::IsUsedWithMeshDeformer() const
{
	return MaterialInstance ? MaterialInstance->GetUsageByFlag(MATUSAGE_MeshDeformer) : Material->GetUsageByFlag(MATUSAGE_MeshDeformer);
}

bool FMaterialResource::IsUsedWithInstancedSkinnedMesh() const
{
	return MaterialInstance ? MaterialInstance->GetUsageByFlag(MATUSAGE_InstancedSkinnedMesh) : Material->GetUsageByFlag(MATUSAGE_InstancedSkinnedMesh);
}
...
bool FMaterialResource::IsUsedWithParticleSystem() const
{
	return MaterialInstance 
		? MaterialInstance->GetUsageByFlag(MATUSAGE_ParticleSprites) || MaterialInstance->GetUsageByFlag(MATUSAGE_BeamTrails)
		: Material->GetUsageByFlag(MATUSAGE_ParticleSprites) || Material->GetUsageByFlag(MATUSAGE_BeamTrails);
}

bool FMaterialResource::IsUsedWithParticleSprites() const
{
	return MaterialInstance ? MaterialInstance->GetUsageByFlag(MATUSAGE_ParticleSprites) : Material->GetUsageByFlag(MATUSAGE_ParticleSprites);
}

bool FMaterialResource::IsUsedWithBeamTrails() const
{
	return MaterialInstance ? MaterialInstance->GetUsageByFlag(MATUSAGE_BeamTrails) : Material->GetUsageByFlag(MATUSAGE_BeamTrails);
}

bool FMaterialResource::IsUsedWithMeshParticles() const
{
	return MaterialInstance ? MaterialInstance->GetUsageByFlag(MATUSAGE_MeshParticles) : Material->GetUsageByFlag(MATUSAGE_MeshParticles);
}

bool FMaterialResource::IsUsedWithNiagaraSprites() const
{
	return MaterialInstance ? MaterialInstance->GetUsageByFlag(MATUSAGE_NiagaraSprites) : Material->GetUsageByFlag(MATUSAGE_NiagaraSprites);
}

bool FMaterialResource::IsUsedWithNiagaraRibbons() const
{
	return MaterialInstance ? MaterialInstance->GetUsageByFlag(MATUSAGE_NiagaraRibbons) : Material->GetUsageByFlag(MATUSAGE_NiagaraRibbons);
}

bool FMaterialResource::IsUsedWithNiagaraMeshParticles() const
{
	return MaterialInstance ? MaterialInstance->GetUsageByFlag(MATUSAGE_NiagaraMeshParticles) : Material->GetUsageByFlag(MATUSAGE_NiagaraMeshParticles);
}

bool FMaterialResource::IsUsedWithStaticLighting() const
{
	return MaterialInstance ? MaterialInstance->GetUsageByFlag(MATUSAGE_StaticLighting) : Material->GetUsageByFlag(MATUSAGE_StaticLighting);
}

bool FMaterialResource::IsUsedWithMorphTargets() const
{
	return MaterialInstance ? MaterialInstance->GetUsageByFlag(MATUSAGE_MorphTargets) : Material->GetUsageByFlag(MATUSAGE_MorphTargets);
}

bool FMaterialResource::IsUsedWithSplineMeshes() const
{
	return MaterialInstance ? MaterialInstance->GetUsageByFlag(MATUSAGE_SplineMesh) : Material->GetUsageByFlag(MATUSAGE_SplineMesh);
}

bool FMaterialResource::IsUsedWithInstancedStaticMeshes() const
{
	return MaterialInstance ? MaterialInstance->GetUsageByFlag(MATUSAGE_InstancedStaticMeshes) : Material->GetUsageByFlag(MATUSAGE_InstancedStaticMeshes);
}

bool FMaterialResource::IsUsedWithGeometryCollections() const
{
	return MaterialInstance ? MaterialInstance->GetUsageByFlag(MATUSAGE_GeometryCollections) : Material->GetUsageByFlag(MATUSAGE_GeometryCollections);
}

bool FMaterialResource::IsUsedWithAPEXCloth() const
{
	return MaterialInstance ? MaterialInstance->GetUsageByFlag(MATUSAGE_Clothing) : Material->GetUsageByFlag(MATUSAGE_Clothing);
}

bool FMaterialResource::IsUsedWithNanite() const
{
	const bool bUsedWithNanite = MaterialInstance ? MaterialInstance->GetUsageByFlag(MATUSAGE_Nanite) : Material->GetUsageByFlag(MATUSAGE_Nanite);
	if (bUsedWithNanite)
	{
		return true;
	}
	...
	if (bNaniteForceEnableMeshes)
	{
		...
		{
			return Material->GetUsageByFlag(MATUSAGE_Nanite);
		}
...
{
	return MaterialInstance ? MaterialInstance->GetUsageByFlag(MATUSAGE_Voxels) : Material->GetUsageByFlag(MATUSAGE_Voxels);
}

bool FMaterialResource::IsUsedWithCurves() const
{
	return MaterialInstance ? MaterialInstance->GetUsageByFlag(MATUSAGE_Curves) : Material->GetUsageByFlag(MATUSAGE_Curves);
}

bool FMaterialResource::IsUsedWithVolumetricCloud() const
{
	return MaterialInstance ? MaterialInstance->GetUsageByFlag(MATUSAGE_VolumetricCloud) : Material->GetUsageByFlag(MATUSAGE_VolumetricCloud);
}

bool FMaterialResource::IsUsedWithHeterogeneousVolumes() const
{
	return MaterialInstance ? MaterialInstance->GetUsageByFlag(MATUSAGE_HeterogeneousVolumes) : Material->GetUsageByFlag(MATUSAGE_HeterogeneousVolumes);
}

bool FMaterialResource::IsUsedWithStaticMesh() const
{
	return MaterialInstance ? MaterialInstance->GetUsageByFlag(MATUSAGE_StaticMesh) : Material->GetUsageByFlag(MATUSAGE_StaticMesh);
}

bool FMaterialResource::HasAnyUsage() const
{
	return MaterialInstance ? (MaterialInstance->GetUsageFlags() != 0) : (Material->GetUsageFlags() != 0);
}
...
{
	return GetMaterialInterface()->IsDisplacementFadeEnabled();
}

FDisplacementFadeRange FMaterialResource::GetDisplacementFadeRange() const
{
	return GetMaterialInterface()->GetDisplacementFadeRange();
}
...
bool FMaterialResource::HasMaterialPropertyConnected(EMaterialProperty In) const
{
	...
	if (Substrate::IsSubstrateEnabled() && bIsSubstrateSupportedDomain)
	{
		...
		return Material->IsPropertyConnected(In);
	}
	else
	{
		switch (In)
...
uint32 FMaterial::AddRef() const
{
	...
	UE_CLOGF(Refs > 5000, LogMaterial, Warning, "FMaterial::AddRef, Suspicious NumDebugRefs %d", Refs);
	return uint32(Refs);
}
...
uint32 FMaterial::Release() const
{
	...
	UE_CLOGF(Refs > 5000, LogMaterial, Warning, "FMaterial::Release, Suspicious NumDebugRefs %d", Refs);
	return uint32(Refs);
}
...
FMaterial::~FMaterial()
{
	...
	UE_CLOGF(NumRemainingRefs > 0u, LogMaterial, Fatal, "%ls Leaked %d refs", *GetDebugName(), NumRemainingRefs);
#endif // UE_CHECK_FMATERIAL_LIFETIME
...
void FMaterial::SetupMaterialEnvironment(
	TConstArrayView<FShaderParametersMetadata*> InUniformBufferStructs,
	const FUniformExpressionSet& InUniformExpressionSet,
	FShaderCompilerEnvironment& OutEnvironment
	) const
{
	// Add the material uniform buffer definition.
	for (const FShaderParametersMetadata* InUniformBufferStruct : InUniformBufferStructs)
	{
		FShaderUniformBufferParameter::ModifyCompilationEnvironment(InUniformBufferStruct->GetShaderVariableName(), *InUniformBufferStruct, GetShaderPlatform(), OutEnvironment);
	}
	...
	if (!Substrate::IsSubstrateEnabled())
	{
		...
		default: 
			UE_LOGF(LogMaterial, Warning, "Unknown material blend mode: %u  Setting to BLEND_Opaque",(int32)GetBlendMode());
			SET_SHADER_DEFINE(OutEnvironment, MATERIALBLENDING_SOLID, 1);
		}
	}
	else
	{
		...
		default:
			UE_LOGF(LogMaterial, Error, "%ls: Unkown Substrate material blend mode could not be converted to Starta. (Asset: %ls) Setting to BLEND_Opaque", *GetFriendlyName(), *GetAssetName());
			SET_SHADER_DEFINE(OutEnvironment, MATERIALBLENDING_SOLID, 1);
		}
	}
	...
	default: 
		UE_LOGF(LogMaterial, Warning, "Unknown material refraction mode: %u  Setting to RM_IndexOfRefraction",(int32)GetRefractionMode());
	...
	switch(GetMaterialDomain())
	{
		...
		default:
			UE_LOGF(LogMaterial, Warning, "Unknown material domain: %u  Setting to MD_Surface",(int32)GetMaterialDomain());
	...
	if (IsTranslucentBlendMode(GetBlendMode()))
	{
		...
		default: 
			UE_LOGF(LogMaterial, Warning, "Unknown lighting mode: %u",(int32)GetTranslucencyLightingMode());
...
#endif
	UE_CLOGF(!ShaderMapId.IsValid(), LogMaterial, Warning, "Invalid shader map ID caching shaders for '%ls', will use default material.", *GetFriendlyName());
#if WITH_EDITOR
	...
	if (!bShaderMapValid)
	{
		...
		if (bContainsInlineShaders || FPlatformProperties::RequiresCookedData() || !AllowShaderCompiling())
		{
			if (bRequiredComplete)
			{
				...
				UE_LOGF(LogMaterial, Fatal,"Failed to find shader map for default material %ls(%ls)! Please make sure cooking was successful (%ls inline shaders, %ls GTSM)",
					*GetFriendlyName(),
					*Instance,
			...
			{
				UE_LOGF(LogMaterial, Log, "Can't compile %ls with cooked content, will use default material instead", *GetFriendlyName());
			}
		...
		else
		{
			...
			if (!bSkipCompilationForODSC)
			{
				...
				UE_LOGF(LogMaterial, Display, "%ls cached shadermap for %ls in %ls, %ls, %ls, %ls (DDC key hash: %ls), compiling. %ls",
					ShaderMapCondition,
					*GetFullPath(),
					*ShaderPlatformName,
...
#else
				UE_LOGF(LogMaterial, Display, "%ls cached shader map for material %ls, compiling. %ls",
					ShaderMapCondition,
					*GetAssetName(),
					IsSpecialEngineMaterial() ? TEXT("Is special engine material.") : TEXT("")
				);
			...
			if (!bShaderMapValid)
			{
				...
				if (IsDefaultMaterial())
				{
					for (int32 ErrorIndex = 0; ErrorIndex < CompileErrors.Num(); ErrorIndex++)
					{
						...
						UE_LOGF(LogMaterial, Warning, "	%ls", *CompileErrors[ErrorIndex]);
					}
					...
					{
						UE_LOGF(LogMaterial, Fatal, "Failed to compile default material %ls!", *GetFriendlyName());
					}
					else
					{
						UE_LOGF(LogMaterial, Error, "Failed to compile default material %ls!", *GetFriendlyName());
					}
				}
#endif // WITH_EDITOR
			}
...
void FMaterial::CacheGivenTypes(const TArray<const FVertexFactoryType*>& VFTypes, const TArray<const FShaderPipelineType*>& PipelineTypes, const TArray<const FShaderType*>& ShaderTypes)
{
	...
	{
		UE_LOGF(LogMaterial, Warning, "Material failed to compile.");
		for (const FString& CompileError : CompileErrors)
		{
			UE_LOGF(LogMaterial, Warning, "%ls", *CompileError);
		}
	...
	{
		UE_LOGF(LogMaterial, Verbose, "Cache given types for a material resource %ls with a complete ShaderMap", *GetFriendlyName());
		return;
	}
		...
		for (int i = 0; i < VFTypes.Num(); ++i)
		{
			...
			if (PipelineType)
			{
				FMeshMaterialShaderType::BeginCompileShaderPipeline(
					...
					this,
					GameThreadShaderMap->GetCompilationOutput(),
			...
			else if (ShaderType->GetTypeForDynamicCast() == FShaderType::EShaderTypeForDynamicCast::Material)
			{
				...
				const FMaterialCompilationOutput& CompilationOutput = GameThreadShaderMap->GetCompilationOutput();
				...
				{
					const bool bShaderShouldCompile = MaterialShaderType->ShouldCompilePermutation(GetShaderPlatform(), FCompilationShaderParameters(this, CompilationOutput), PermutationId, ShaderPermutation);
			...
			else if (ShaderType->GetTypeForDynamicCast() == FShaderType::EShaderTypeForDynamicCast::MeshMaterial)
			{
				...
				for (int32 PermutationId = 0; PermutationId < ShaderType->GetPermutationCount(); ++PermutationId)
				{
					const bool bShaderShouldCompile = MeshMaterialShaderType->ShouldCompilePermutation(GetShaderPlatform(), FCompilationShaderParameters(this, CompilationOutput), VFType, PermutationId, ShaderPermutation);
					if (!bShaderShouldCompile)
					{
						continue;
					}
...
{
	FString MaterialTranslationDDCKeyString = GetMaterialShaderMapKeyString(ShaderMapId, FConservativeShaderParameters(this), GetShaderPlatform(), false);
	FHLSLMaterialTranslator MaterialTranslator(this, OutCompilationOutput, InStaticParameters, GetShaderPlatform(), GetQualityLevel(), ShaderMapId.FeatureLevel, InTargetPlatform, &ShaderMapId.SubstrateCompilationConfig, MaterialTranslationDDCKeyString);
	...
	if (Result == EHLSLMaterialTranslatorResult::RetryWithoutDDC)
	{
		...
		new (&MaterialTranslator) FHLSLMaterialTranslator(this, OutCompilationOutput, InStaticParameters, GetShaderPlatform(), GetQualityLevel(), ShaderMapId.FeatureLevel, InTargetPlatform, &ShaderMapId.SubstrateCompilationConfig, MaterialTranslationDDCKeyString);
...
bool FMaterial::Translate_New(const FMaterialShaderMapId& InShaderMapId,
	...
	TRefCountPtr<FSharedShaderCompilerEnvironment>& OutMaterialEnvironment)
{
	UE_LOGF(LogMaterial, Display, "Translating material '%ls' with the new experimental material translator.", *GetMaterialInterface()->GetFullName());
	...
	FMaterialIRModuleBuilder Builder = {
		.MaterialInterface = GetMaterialInterface(),
...
bool FMaterial::Translate(const FMaterialShaderMapId& InShaderMapId,
	...
	TRefCountPtr<FSharedShaderCompilerEnvironment>& OutMaterialEnvironment)
{
	// Reset the material insights instance if this is a valid material interface.
	if (UMaterialInterface* Interface = GetMaterialInterface())
	{
		Interface->MaterialInsight.Reset(new FMaterialInsights);
	}

	// Use the new translator if the shader map uses it.
	if (InShaderMapId.bUsingNewHLSLGenerator && !bEnableRestrictiveMaterialMode)
	{
	...
	const ITargetPlatform* TargetPlatform)
{
	...
	if(bSuccess)
	{
		...
		{
			// Get this material's override floating point precision setting
			EMaterialFloatPrecisionMode MaterialFloatPrecisionMode = EMaterialFloatPrecisionMode::MFPM_Default;
			const UMaterialInterface* MaterialInterface = GetMaterialInterface();
			if (MaterialInterface)
			{
				MaterialFloatPrecisionMode = MaterialInterface->GetMaterial()->FloatPrecisionMode;
			}

			// Sets the floating point precision to be used for the PreshaderBuffer uniform. This is controlled via this material's floating point override set in the material editor
			EShaderPrecisionModifier::Type ParameterPrecision = (IsMobilePlatform((EShaderPlatform)GetShaderPlatform()) && MaterialFloatPrecisionMode == MFPM_Half_WithShaderParameters) ? EShaderPrecisionModifier::Half : EShaderPrecisionModifier::Float;
			TArray<FShaderParametersMetadata*> UniformBufferStructs = NewCompilationOutput.UniformExpressionSet.CreateBufferStructs(ParameterPrecision);

			SetupMaterialEnvironment(UniformBufferStructs, NewCompilationOutput.UniformExpressionSet, *MaterialEnvironment);
			for (FShaderParametersMetadata* UniformBufferStruct : UniformBufferStructs)
			{
	...
	if (!Shader)
	{
		...
		if (bFatalIfMissing)
		{
			auto noinline_lambda = [&](...) FORCENOINLINE
			{
				...
				const FMaterialCompilationOutput& CompilationOutput = RenderingThreadShaderMap->GetCompilationOutput();
				...
				bool bVFShouldCache = FMeshMaterialShaderType::ShouldCompileVertexFactoryPermutation(GetShaderPlatform(), FCompilationShaderParameters(this, CompilationOutput), VertexFactoryType, ShaderType, ShaderPermutation);
				bool bShaderShouldCache = ShaderType->ShouldCompilePermutation(GetShaderPlatform(), FCompilationShaderParameters(this, CompilationOutput), VertexFactoryType, PermutationId, ShaderPermutation);
				...
				UE_LOGF(LogMaterial, Error,
					"Couldn't find Shader (%ls, %d) for Material Resource %ls!\n" "		RenderMeshShaderMap %d, RenderThreadShaderMap %d\n" "		GameMeshShaderMap %d, GameThreadShaderMap %d, bShaderWasFoundInGameShaderMap %d\n" "		With VF=%ls, Platform=%ls\n" "		ShouldCache: Mat=%u, VF=%u, Shader=%u \n" "		MaterialUsageDesc: %ls",
				...
				if (MeshShaderMap)
				{
					...
					for (const auto& ShaderPair : List)
					{
						...
						UE_LOGF(LogMaterial, Error, "ShaderType found in MaterialMap: %ls", *TypeName);
					}
				}

				UE_LOGF(LogMaterial, Fatal, "Fatal Error Material not found");
			};
			noinline_lambda();
		}
...
bool FMaterial::TryGetShaders(const FMaterialShaderTypes& InTypes, const FVertexFactoryType* InVertexFactoryType, FMaterialShaders& OutShaders) const
{
	...
	const FMaterialCompilationOutput& CompilationOutput = ShaderMap->GetCompilationOutput();

	TArray<FShaderCommonCompileJobPtr> CompileJobs;
	bool bMissingShader = false;

	auto ShouldCacheShaderType = [this, &PermutationFlags, &CompilationOutput](const FShaderType* ShaderType, const FVertexFactoryType* VertexFactoryType, const int32 PermutationId) -> bool {
		...
		{
			return MaterialShader->ShouldCompilePermutation(GetShaderPlatform(), FCompilationShaderParameters(this, CompilationOutput), PermutationId, PermutationFlags);
		}

		// if we are a MeshMaterialShader
		if (const FMeshMaterialShaderType* MeshMaterialShader = ShaderType->GetMeshMaterialShaderType())
		{
			const bool bVFShouldCache = FMeshMaterialShaderType::ShouldCompileVertexFactoryPermutation(GetShaderPlatform(), FCompilationShaderParameters(this, CompilationOutput), VertexFactoryType, ShaderType, PermutationFlags);
			const bool bShaderShouldCache = MeshMaterialShader->ShouldCompilePermutation(GetShaderPlatform(), FCompilationShaderParameters(this, CompilationOutput), VertexFactoryType, PermutationId, PermutationFlags);
						...
						if (!bShaderMapComplete)
						{
							if (InVertexFactoryType)
							{
								FMeshMaterialShaderType::BeginCompileShaderPipeline(
									...
									PermutationFlags, 
									this,
									ShaderMap->GetCompilationOutput(),
	...
	if (!ShaderPipeline)
	{
		if (bFatalIfNotFound)
		{
			auto noinline_lambda = [&](...) FORCENOINLINE
			{
				...
				UE_LOGF(LogMaterial, Error,
					"Couldn't find ShaderPipeline %ls for Material Resource %ls!", ShaderPipelineType->GetName(), *GetFriendlyName());
				...
				for (auto* ShaderType : ShaderPipelineType->GetStages())
				{
					...
					const FCompilationShaderParameters ShaderParams(this, CompilationOutput);

					if (!Shader)
					{
						UE_LOGF(LogMaterial, Error, "Missing %ls shader %ls!", GetShaderFrequencyString(ShaderType->GetFrequency(), false), ShaderType->GetName());
					}
					else if (ShaderType->GetMeshMaterialShaderType())
					{
						...
						bool bVFShouldCache = FMeshMaterialShaderType::ShouldCompileVertexFactoryPermutation(GetShaderPlatform(), ShaderParams, VertexFactoryType, ShaderType, ShaderPermutation);
						bool bShaderShouldCache = ShaderType->GetMeshMaterialShaderType()->ShouldCompilePermutation(GetShaderPlatform(), ShaderParams, VertexFactoryType, kUniqueShaderPermutationId, ShaderPermutation);

						UE_LOGF(LogMaterial, Error, "%ls %ls ShouldCache: Mat=%u, VF=%u, Shader=%u",
					...
					else if (ShaderType->GetMaterialShaderType())
					{
						...
						bool bShaderShouldCache = ShaderType->GetMaterialShaderType()->ShouldCompilePermutation(GetShaderPlatform(), ShaderParams, kUniqueShaderPermutationId, ShaderPermutation);

						UE_LOGF(LogMaterial, Error, "%ls %ls ShouldCache: Mat=%u, NO VF, Shader=%u",
...
FString FMaterialResource::GetMaterialUsageDescription() const
{
	...
	for (int32 MaterialUsageIndex = 0; MaterialUsageIndex < MATUSAGE_MAX; MaterialUsageIndex++)
	{
		if (Material->GetUsageByFlag((EMaterialUsage)MaterialUsageIndex))
		{
			if (!bFirst)
			{
				BaseDescription += FString(",");
			}
			BaseDescription += UMaterialInterface::GetUsageName((EMaterialUsage)MaterialUsageIndex);
			bFirst = false;
		}
	}
	BaseDescription += FString("}");
...
{
	const FConservativeShaderParameters MaterialParameters(this);
...
#if WITH_EDITOR
uint64 FMaterial::GetReferencedTexturesHash() const
{
	FXxHash64Builder HashState;

	const TArray<TObjectPtr<UObject>>& ReferencedTextures = GetMatDefaultTextures();
	...
	for (int32 TextureIndex = 0; TextureIndex < ReferencedTextures.Num(); TextureIndex++)
	{
		...
		HashState.Update(*TextureName, TextureName.NumBytesWithoutNull());
	}
...
uint64 FMaterial::GetExpressionIncludesHash() const
{
	FXxHash64Builder HashState;
...
uint64 FMaterial::GetExternalCodeReferencesHash() const
{
	return GetCachedExpressionData().GetExternalCodeReferencesHash();
}
#endif // WITH_EDITOR
...
bool FMaterial::GetMaterialExpressionSource(FString& OutSource)
{
		...
		if (Source)
		{
			...
			if (CVarMaterialEdPreshaderDumpToHLSL.GetValueOnGameThread())
			{
				...
				for (int32 PreshaderIndex = 0; PreshaderIndex < NewCompilationOutput.UniformExpressionSet.UniformPreshaders.Num(); PreshaderIndex++)
				{
					...
					if (((uint8)PreshaderHeader.Type >= (uint8)UE::Shader::EValueType::Float1 && (uint8)PreshaderHeader.Type <= (uint8)UE::Shader::EValueType::Float4) ||
						((uint8)PreshaderHeader.Type >= (uint8)UE::Shader::EValueType::Int1 && (uint8)PreshaderHeader.Type <= (uint8)UE::Shader::EValueType::Int4) ||
						((uint8)PreshaderHeader.Type >= (uint8)UE::Shader::EValueType::Bool1 && (uint8)PreshaderHeader.Type <= (uint8)UE::Shader::EValueType::Bool4))
					{
						UE::Shader::FType ShaderType(PreshaderHeader.Type);
						...
						SwizzleSuffix = SwizzleTable[PreshaderHeader.BufferOffset % 4][ShaderType.GetNumComponents() - 1];
					}

					OutSource.Appendf(TEXT("// PreshaderBuffer[%d]%s = %s\n"), PreshaderHeader.BufferOffset / 4, SwizzleSuffix, *PreshaderDebug);
				}
	...
	return false;
#else
	UE_LOGF(LogMaterial, Fatal,"Not supported.");
	return false;
#endif
}
...
#else
	UE_LOGF(LogMaterial, Fatal,"GetPreshaderStats is only supported for WITH_EDITOR builds.");
#endif
}
...
bool FMaterial::WritesEveryPixel(bool bShadowPass) const
{
	...
	return WritesEveryPixel(bShadowPass, bVFTypeSupportsNullPixelShader, false);
}
...
bool FMaterial::WritesEveryPixel(bool bShadowPass, bool bVFTypeSupportsNullPixelShader, bool bOpacityMaskIsOpaque) const
{
	bool bLocalStencilDitheredLOD = IsStencilForLODDitherEnabled(GetShaderPlatform());
	return (!IsMasked() || bOpacityMaskIsOpaque)
...
{
	GetShaderTypes(LayoutParams, OutShaderInfo, FConservativeShaderParameters(this));
}
...
{
	const FConservativeShaderParameters MaterialParameters(this);
	GetShaderTypes(LayoutParams, OutShaderInfo, MaterialParameters);
}

void FMaterial::GetShaderTypes(const FPlatformTypeLayoutParameters& LayoutParams, TArray<FDebugShaderTypeInfo>& OutShaderInfo, const FMaterialShaderParameters& InParameters) const
{
	const FMaterialShaderMapLayout& Layout = AcquireMaterialShaderMapLayout(GetShaderPlatform(), GetShaderPermutationFlags(LayoutParams), InParameters);
...
FMaterialUpdateContext::FMaterialUpdateContext(uint32 Options, EShaderPlatform InShaderPlatform)
{
	...
	bSkipRecacheStaticPermutations = (Options & EOptions::SkipRecacheStaticPermutations) != 0;
	bSkipUpdateStaticDrawLists = (Options & EOptions::SkipUpdateStaticDrawLists) != 0;
...
FMaterialUpdateContext::~FMaterialUpdateContext()
{
	...
	const bool bUpdateStaticDrawLists = !bSkipUpdateStaticDrawLists && !ComponentReregisterContext && !ComponentRecreateRenderStateContext && FApp::CanEverRender();
	...
	TFunction<void(UMaterialInstance* MI)> UpdateInstance = [&](UMaterialInstance* MI)
	{
		...
		if (!bSkipRecacheStaticPermutations)
		{
	...
	{
		UE_LOGF(LogMaterial, Verbose,
			   "%.2f seconds spent updating %d materials, %d interfaces, %d instances, %d with static permutations.",
...
void UMaterialInterface::AnalyzeMaterialCompilationInCallback(TFunctionRef<void (FMaterialCompiler*)> InCompilationCallback, FMaterialAnalysisResult& OutResult)
{
	...
	OutResult.bRequiresWorldPosition = MaterialTranslator.bNeedsWorldPositionExcludingShaderOffsets;
...
bool FMaterialInstanceBasePropertyOverrides::operator==(const FMaterialInstanceBasePropertyOverrides& Other)const
{
	return	bOverride_OpacityMaskClipValue == Other.bOverride_OpacityMaskClipValue &&
		...
		bOverride_UsageFlags == Other.bOverride_UsageFlags &&
...
FArchive& FMaterialResourceProxyReader::operator<<(class FName& Name)
{
	...
	{
		UE_LOGF(LogMaterial, Fatal, "FMaterialResourceProxyReader: deserialized an invalid FName, NameIdx=%d, Names.Num()=%d (Offset=%lld, InnerArchive.Tell()=%lld, OffsetToFirstResource=%lld)", 
...
void SetShaderMapsOnMaterialResources_RenderThread(FRHICommandListImmediate& RHICmdList, FMaterialsToUpdateMap& MaterialsToUpdate)
{
	...
	for (int32 FeatureLevelIndex = 0; FeatureLevelIndex < UE_ARRAY_COUNT(bUpdateFeatureLevel); ++FeatureLevelIndex)
	{
		if (bUpdateFeatureLevel[FeatureLevelIndex])
		{
			...
			for (FMaterialRenderProxy* MaterialProxy : FMaterialRenderProxy::GetMaterialRenderProxyMap())
			{
...
void FMaterial::SetShaderMapsOnMaterialResources(const TMap<TRefCountPtr<FMaterial>, TRefCountPtr<FMaterialShaderMap>>& MaterialsToUpdate)
{
	...
	// The parent material of some material in GetMaterialRenderProxyMap may be destroyed before SetShaderMapsOnMaterialResources_RenderThread
	// executes. Iterate over all materials in the proxy map and bump their parent fences so that their IsReadyForFinishDestroy() will return false
	// until the fence executes, which is after the RT task above completes.
	// This guarantees that the parents of all materials in GetMaterialRenderProxyMap are safe from deletion while the RT task executes.
	FScopeLock Locker(&FMaterialRenderProxy::GetMaterialRenderProxyMapLock());
	for (FMaterialRenderProxy* Proxy : FMaterialRenderProxy::GetMaterialRenderProxyMap())
	{
		UMaterialInstance* MI = Cast<UMaterialInstance>(Proxy->GetMaterialInterface());
		while (MI)
		{
			if (MI->Parent)
			{
				MI->Parent->ParentRefFence.BeginFence();
			}
...
FMaterialShaderParametersBase::FMaterialShaderParametersBase(const FMaterial* InMaterial)
{
	...
	bIsUsedWithCurves = InMaterial->IsUsedWithCurves();
	...
	bIsUsedWithMeshDeformer = InMaterial->IsUsedWithMeshDeformer();
	bIsUsedWithInstancedSkinnedMesh = InMaterial->IsUsedWithInstancedSkinnedMesh();
}
...
void FMaterialShaderParameters::InitializeShaderLayoutParamsFromMaterial(const FMaterial* InMaterial)
{
	if (Substrate::IsSubstrateEnabled())
	{
	...
	bWritesEveryPixel = InMaterial->WritesEveryPixel(/*bShadowPass*/false);
	bWritesEveryPixelShadowPass = InMaterial->WritesEveryPixel(/*bShadowPass*/true);
}

void FMaterialShaderParameters::InitializeShaderLayoutParamsFromCompOutput(const FMaterial* InMaterial, const FMaterialCompilationOutput& InCompilationOutput)
{
	bHasAnisotropyConnected = InCompilationOutput.bUsesAnisotropy;
	bHasVertexPositionOffsetConnected = InCompilationOutput.bUsesWorldPositionOffset;
	bHasPixelDepthOffsetConnected = InCompilationOutput.bUsesPixelDepthOffset;
	bHasDisplacementConnected = InCompilationOutput.bUsesDisplacement;
	bMaterialMayModifyMeshPosition = InCompilationOutput.bModifiesMeshPosition;
	bHasPerInstanceCustomData = InCompilationOutput.bUsesPerInstanceCustomData;
	bHasVertexInterpolator = InCompilationOutput.bUsesVertexInterpolator;
	bUsesTemporalResponsiveness = InCompilationOutput.bUsesTemporalResponsiveness;
	bUsesMotionVectorWorldOffset = InCompilationOutput.bUsesMotionVectorWorldOffset;
	
	// Consider whether opacity mask is actually used when calculating WritesEveryPixel().
	// Treat the case where a custom HLSL expression is discarding pixels as equivalent to the use of opacity mask here.
	const bool bVFTypeSupportsNullPixelShader = !InMaterial->IsUsedWithInstancedStaticMeshes();
	const bool bUsesOpacityMaskOrDiscard = InCompilationOutput.bUsesOpacityMask || InCompilationOutput.bUsesCustomHLSLPixelDiscard || (bShouldCastDynamicShadows && InCompilationOutput.bUsesOpacity);
	bWritesEveryPixel = InMaterial->WritesEveryPixel(/*bShadowPass*/false, bVFTypeSupportsNullPixelShader, !bUsesOpacityMaskOrDiscard);
	bWritesEveryPixelShadowPass = InMaterial->WritesEveryPixel(/*bShadowPass*/true, bVFTypeSupportsNullPixelShader, !bUsesOpacityMaskOrDiscard);
}

FCompilationShaderParameters::FCompilationShaderParameters(const FMaterial* InMaterial, const FMaterialCompilationOutput& InCompilationOutput)
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	: FMaterialShaderParameters(InMaterial)
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
{
	static const int32 bUseShaderCompilationParameters = CVarMaterialUseShaderCompilationParameters.GetValueOnAnyThread();
	if (bUseShaderCompilationParameters)
	{
		InitializeShaderLayoutParamsFromCompOutput(InMaterial, InCompilationOutput);
	}
	else
	{
		InitializeShaderLayoutParamsFromMaterial(InMaterial);
	}
}

FConservativeShaderParameters::FConservativeShaderParameters()
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	: FMaterialShaderParameters() {}
PRAGMA_ENABLE_DEPRECATION_WARNINGS

FConservativeShaderParameters::FConservativeShaderParameters(const FMaterial* InMaterial)
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	: FMaterialShaderParameters(InMaterial)
PRAGMA_ENABLE_DEPRECATION_WARNINGS
{
...
void FConservativeShaderParameters::Record(FMaterialKeyGeneratorContext& Context)
{
...
{
	const bool bSubstrateEnabled = Substrate::IsSubstrateEnabled();
	const bool bSubstrateHiddenConversionEnabled = Substrate::IsHiddenMaterialAssetConversionEnabled();
	const bool bSubstrateFrontMaterialPluggedIn = bHasFrontMaterialExpr;
	...
	if (Domain == MD_Volume && Material->GetUsageByFlag(MATUSAGE_SkeletalMesh))
	{
	...
	if (MaterialShadingModels.HasShadingModel(MSM_SingleLayerWater))
	{
		...
		if (!Material->HasAnyExpressionsInMaterialAndFunctionsOfType<UMaterialExpressionSingleLayerWaterMaterialOutput>()
			&& (!bSubstrateEnabled || (bSubstrateHiddenConversionEnabled && !bSubstrateFrontMaterialPluggedIn))
			)
		{
	...
	if (Domain == MD_DeferredDecal && !(IsTranslucentOnlyBlendMode(BlendMode) || BlendMode == BLEND_AlphaComposite || IsModulateBlendMode(BlendMode)))
	{
		...
		const TCHAR* AllowedBlendModes = bSubstrateEnabled ? 
			TEXT("TranslucentGreyTransmittance, TranslucentColorTransmittance, AlphaComposite (Premultiplied Alpha), or ColoredTransmittanceOnly.") :
			TEXT("Translucent, AlphaComposite(Premultiplied Alpha), or Modulate.");

		OutErrors.Add(FString::Printf(TEXT("Material using the DeferredDecal domain can only use the Blend Modes %s"), AllowedBlendModes));
	}
...
const TArray<TObjectPtr<UObject>>& FEditorMaterialResourceBase::GetMatDefaultTextures() const
{
	return ReferencedTextures;
}

int32 FEditorMaterialResourceBase::GetMatDefaultTextureIdx(TObjectPtr<UObject> InTexture) const
{
	return ReferencedTextures.Find(InTexture);
}

TObjectPtr<UObject> FEditorMaterialResourceBase::GetMatDefaultTextureAtIdx(const int32 InIdx) const
{
	if (!ensureAlwaysMsgf(ReferencedTextures.IsValidIndex(InIdx), TEXT("Invalid texture Idx %d requested from Array with %d Elements."), InIdx, ReferencedTextures.Num()))
	{
		return nullptr;
	}

	return ReferencedTextures[InIdx];
}

TArrayView<const TObjectPtr<UObject>> FEditorMaterialResourceBase::GetReferencedTextures() const
{
	return ReferencedTextures;
}

TConstArrayView<TObjectPtr<UTextureCollection>> FEditorMaterialResourceBase::GetReferencedTextureCollections() const
{
	return MakeArrayView(ReferencedTextureCollections);
}

// Helper class for getting translation output from a Material/Material Instance.
class FDebugShaderInfoMaterialResource : public FMaterialResource
{
public:
	FDebugShaderInfoMaterialResource(UMaterial* InMat, UMaterialInstance* InMatInstance, const ERHIFeatureLevel::Type FeatureLevel, const EShaderPlatform ShaderPlatform,
		const EMaterialQualityLevel::Type QualityLevel)
		: FMaterialResource()
	{
		SetMaterial(InMat);
		SetMaterialInstance(InMatInstance);

		SetQualityLevelProperties(ShaderPlatform, QualityLevel);
	}

	virtual ~FDebugShaderInfoMaterialResource() = default;

	bool TryBuildCompilationOutput(FMaterialCompilationOutput& OutCompilationOutput)
	{
		FStaticParameterSet UsedStaticParams;

		if (MaterialInstance)
		{
			MaterialInstance->GetStaticParameterValues(UsedStaticParams);
		}
		else if (Material)
		{
			Material->GetStaticParameterValues(UsedStaticParams);
		}
		else
		{
			UE_LOGF(LogMaterial, Error, "FDebugShaderInfoMaterialResource created without Material or MaterialInstance");
			return false;
		}

		FMaterialShaderMapId ShaderMapId;
		BuildShaderMapId(ShaderMapId, nullptr);

		// TODO: Handle DDC Key creation once new translator supports it
		if (ShaderMapId.bUsingNewHLSLGenerator && !bEnableRestrictiveMaterialMode)
		{
			FMaterialIRModule Module;
			...
			FMaterialIRModuleBuilder Builder = {
				...
				.FeatureLevel = ShaderMapId.FeatureLevel,
				.QualityLevel = GetQualityLevel(),
				.BlendMode = GetBlendMode(),
				.StaticParameters = &UsedStaticParams,
			};
		...
		{
			FString MaterialTranslationDDCKeyString = GetMaterialShaderMapKeyString(ShaderMapId, FConservativeShaderParameters(this), GetShaderPlatform(), false);
			FHLSLMaterialTranslator MaterialTranslator(this, OutCompilationOutput, UsedStaticParams, GetShaderPlatform(), GetQualityLevel(), ShaderMapId.FeatureLevel, nullptr, &ShaderMapId.SubstrateCompilationConfig, MaterialTranslationDDCKeyString);
			EHLSLMaterialTranslatorResult Result = MaterialTranslator.Translate(false);

			return Result == EHLSLMaterialTranslatorResult::Success;
		}
	}

};

TArray<FDebugShaderTypeInfo> GetShadersUsingCompilationParameters_GameThread(UMaterialInterface* InMaterial, const EShaderPlatform InShaderPlatform)
{
	UMaterialInstance* Inst = Cast<UMaterialInstance>(InMaterial);
	FPlatformTypeLayoutParameters LayoutParams;
	LayoutParams.InitializeForPlatform(nullptr);

	FMaterialResource* Resource = InMaterial->GetMaterialResource(InShaderPlatform);
	...
	if (Resource)
	{
		if (Resource->IsGameThreadShaderMapComplete())
		{
			...
			const FMaterialCompilationOutput& CompOutput = ShaderMap->GetCompilationOutput();

			TArray<FDebugShaderTypeInfo> ShaderTypeInfos;
			Resource->GetShaderTypes(LayoutParams, ShaderTypeInfos, FCompilationShaderParameters(Resource, CompOutput));

			return ShaderTypeInfos;
		}

	}
	UMaterial* BaseMaterial = InMaterial->GetMaterial();
	FDebugShaderInfoMaterialResource ShaderInfoResource(BaseMaterial, Inst, GMaxRHIFeatureLevel, InShaderPlatform, EMaterialQualityLevel::Num);

	FMaterialCompilationOutput CompOutput;
	const bool bValidOutput = ShaderInfoResource.TryBuildCompilationOutput(CompOutput);

	TArray<FDebugShaderTypeInfo> ShaderTypeInfos;
	if (bValidOutput)
	{
		ShaderInfoResource.GetShaderTypes(LayoutParams, ShaderTypeInfos, FCompilationShaderParameters(&ShaderInfoResource, CompOutput));
	}
	else
	{
		UE_LOGF(LogMaterial, Display, ("Failed to translate material resource (in %ls hierarchy) for Compilation Output. Returning Conservative Shader Layout"),
			*ShaderInfoResource.GetFriendlyName());

		ShaderInfoResource.GetShaderTypes(LayoutParams, ShaderTypeInfos, FConservativeShaderParameters(&ShaderInfoResource));
	}
