...
EMaterialValueType FMaterialAggregateAttribute::ToMaterialValueType() const
{
	switch (Type)
	{
		...
		case EMaterialAggregateAttributeType::Normal: return MCT_Float3;
		...
		case EMaterialAggregateAttributeType::SubstrateData: return MCT_Substrate;

		case EMaterialAggregateAttributeType::Aggregate:
			return Aggregate.Get() == MaterialAttributesAggregate::Get() ? MCT_MaterialAttributes : MCT_Unknown;
...
{
	// @massimo.tristano We might want to make this constant time
	return Attributes.IndexOfByPredicate([Name](const FMaterialAggregateAttribute& Attribute)
	{
		return Attribute.Name.IsEqual(Name, ENameCase::CaseSensitive);
	});
}
...
#if WITH_EDITOR
namespace MaterialAttributesAggregate
{

static EMaterialAggregateAttributeType MaterialValueTypeToMaterialAggregateAttributeType(EMaterialValueType Type)
{
	switch (Type)
	{
		case MCT_Float:
		...
		case MCT_Substrate: return EMaterialAggregateAttributeType::SubstrateData;
...
static EMaterialAggregateAttributeType GetPropertyType(EMaterialProperty Property)
{
	switch (Property)
	{
		case MP_Normal: return EMaterialAggregateAttributeType::Normal;
		case MP_SubsurfaceColor: return EMaterialAggregateAttributeType::Float4;
		default: return MaterialValueTypeToMaterialAggregateAttributeType(FMaterialAttributeDefinitionMap::GetValueType(Property));
	}
}

static FMaterialAggregateAttribute MakeAttributeFromProperty(EMaterialProperty Property)
{ 
	FMaterialAggregateAttribute Attribute;
	Attribute.Name = FName(FMaterialAttributeDefinitionMap::GetAttributeName(Property));
	Attribute.Type = GetPropertyType(Property);
	...
	Attribute.DefaultValue = (Property == MP_ShadingModel) ? FVector4f(1.0f, 0.0f, 0.0f, 0.0f) : FMaterialAttributeDefinitionMap::GetDefaultValue(Property); // Should that be changed for substrate?
	
	return Attribute;
}

struct FAttributePropertyIndexMap
{
	static const uint32 NumMaxCustomizedUVs = 8;
	...
	static const FAttributePropertyIndexMap& Get()
	{
		static const FAttributePropertyIndexMap Instance;
		return Instance;
	}
...
const TConstArrayView<EMaterialProperty> GetMaterialProperties()
{
	const auto& Properties = FAttributePropertyIndexMap::Get();
	return { Properties.IndexToProperty, Properties.NumAttributes };
}

const UMaterialAggregate* Get()
{
	static const TObjectPtr<UMaterialAggregate> Instance = [] () {
		TObjectPtr<UMaterialAggregate> MA = NewObject<UMaterialAggregate>(GetTransientPackage());
		MA->Rename(TEXT("MaterialAttributes"));
		MA->AddToRoot();
		for (EMaterialProperty Property : GetMaterialProperties())
		{
			MA->Attributes.Add(MakeAttributeFromProperty(Property));
		}
		return MA;
	}();
	return Instance.Get();
...
const FMaterialAggregateAttribute* GetAttribute(EMaterialProperty Property)
{
	int32 PropertyIndex = MaterialPropertyToAttributeIndex( Property);
	if (FAttributePropertyIndexMap::Get().IsValidIndex(PropertyIndex))
	{
		return &Get()->Attributes[PropertyIndex];
	}
	return nullptr;
}

int32 MaterialPropertyToAttributeIndex(EMaterialProperty Property)
{
	check(Property < MP_MAX);
	return (Property < MP_MAX) ? FAttributePropertyIndexMap::Get().PropertyToIndex[Property] : INDEX_NONE;
}

EMaterialProperty AttributeIndexToMaterialProperty(int32 MaterialAttributeIndex)
{
	ensure(FAttributePropertyIndexMap::Get().IsValidIndex(MaterialAttributeIndex));
	return FAttributePropertyIndexMap::Get().IndexToProperty[MaterialAttributeIndex];
}

} // MaterialAttributesAggregate
#endif // #if WITH_EDITOR
