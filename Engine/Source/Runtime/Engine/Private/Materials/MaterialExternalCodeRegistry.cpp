...
void FMaterialExternalCodeEnvironmentDefine::UpdateHash(FXxHash64Builder& Hasher) const
{
	FString NameString = Name.ToString();
	Hasher.Update(*NameString, NameString.NumBytesWithoutNull());
	Hasher.Update(&ShaderFrequency, sizeof(ShaderFrequency));
}
...
void FMaterialExternalCodeDeclaration::UpdateHash(FXxHash64Builder& Hasher) const
{
	const uint8 bIsInlinedByte = bIsInlined ? 1 : 0;
	Hasher.Update(&bIsInlinedByte, sizeof(bIsInlinedByte));

	Hasher.Update(&ReturnType, sizeof(ReturnType));

	FString NameString = Name.ToString();
	Hasher.Update(*NameString, NameString.NumBytesWithoutNull());

	Hasher.Update(*Definition, Definition.NumBytesWithoutNull());
	Hasher.Update(&Derivative, sizeof(Derivative));
	Hasher.Update(&ShaderFrequency, sizeof(ShaderFrequency));

	for (const TEnumAsByte<EMaterialDomain>& InDomain : Domains)
	{
		EMaterialDomain Domain = InDomain.GetValue();
		Hasher.Update(&Domain, sizeof(Domain));
	}
...
void UMaterialExternalCodeCollection::PostInitProperties()
{
	...
	for (const FMaterialExternalCodeDeclaration& ExternalCode : ExternalCodeDeclarations)
	{
		...
		if (InvalidPropertiesList.Len() > 0)
		{
			UE_LOGF(LogMaterial, Error, "External HLSL code declaration '%ls' is invalid: %ls", *ExternalCode.Name.ToString(), *InvalidPropertiesList);
		}
	}
}
...
void MaterialExternalCodeRegistry::BuildMapToExternalDeclarations()
{
	if (const UMaterialExternalCodeCollection* ExternalCodeCollection = GetDefault<UMaterialExternalCodeCollection>())
	{
		...
		for (const FMaterialExternalCodeDeclaration& InExternalCode : ExternalCodeCollection->ExternalCodeDeclarations)
		{
			...
			{
				UE_LOGF(LogMaterial, Fatal, "External HLSL code declarations for materials must not be overloaded, but '%ls' is defined more than once", *InExternalCode.Name.ToString());
			}
