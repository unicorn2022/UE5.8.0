...
bool FMaterialOverrideNanite::FixupLegacySoftReference(UObject* OptionalOwner)
{
	...
	{
		UE_LOGF(LogMaterial, Warning, "MaterialOverrideNanite with owner '%ls' has a legacy override material reference (%ls) but it could not be loaded.", OptionalOwner ? *OptionalOwner->GetPathName() : TEXT("UNKNOWN"), *OverrideMaterialRef.ToString());
		return false;
	}
	...
	if (OverrideMaterialEditor->HasAnyFlags(RF_NeedLoad))
	{
		...
		UE_LOGF(LogMaterial, Display, "Async loading NaniteOverrideMaterial '%ls' for owner '%ls'.", *LongPackageName, OptionalOwner ? *OptionalOwner->GetPathName() : TEXT("UNKNOWN"));
