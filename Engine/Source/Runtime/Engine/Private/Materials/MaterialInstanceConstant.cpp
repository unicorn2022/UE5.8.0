...
void UMaterialInstanceConstant::SetDoubleVectorParameterValueEditorOnly(const FMaterialParameterInfo& ParameterInfo, FVector4d Value)
{
	check(GIsEditor || IsRunningCommandlet());
...
void UMaterialInstanceConstant::SetParameterCollectionParameterValueEditorOnly(const FMaterialParameterInfo& ParameterInfo, UMaterialParameterCollection* Value)
{
	check(GIsEditor || IsRunningCommandlet());
	SetParameterCollectionParameterValueInternal(ParameterInfo, Value);
}
...
{
	// Always use base material textures in Editor, unless we have Layers
	...
	if (!bLoadedCachedExpressionData)
	{
		...
		if (LocalStaticParameters.bHasMaterialLayers)
		{
			...
			LocalCachedExpressionData->AnalyzeMaterial(*BaseMaterial, &MaterialLayers);

			ReferencedDefaultTextures = LocalCachedExpressionData->EditorOnlyData->EditorReferencedDefaultTextures;
		}
