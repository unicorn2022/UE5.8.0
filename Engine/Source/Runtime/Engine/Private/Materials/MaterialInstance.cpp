...
// MaterialEditor flag that controls whether the material editor is in restrictive mode which entails:
//    - MaterialInstances that generate new shader permutations cannot have a cooked parent that is neither a base-material nor a material in the same module as the instance
//    - Force-disables the new experimental material translator
ENGINE_API bool bEnableRestrictiveMaterialMode = false;
...
void FMaterialInstanceRenderProxy::StartCacheUniformExpressions() const
{
	Owner->StartCacheUniformExpressions();
}

void FMaterialInstanceRenderProxy::FinishCacheUniformExpressions() const
{
...
{
	if (MaterialInstance->GetInstanceRenderProxy())
	{
		MaterialInstance->GetInstanceRenderProxy()->CacheUniformExpressions_GameThread(bRecreateUniformBuffer);
	}
}
...
void RecacheMaterialInstanceUniformExpressions(const UMaterialInterface* ParentMaterial, bool bRecreateUniformBuffer)
{
	...
	{
		UE_LOGF(LogMaterial,Verbose,"Recaching MI Uniform Expressions for parent %ls", *ParentMaterial->GetFullName());
		...
		for (TObjectIterator<UMaterialInstance> It(/*AdditionalExclusionFlags = */RF_ClassDefaultObject, /*bIncludeDerivedClasses = */true, /*InInternalExclusionFlags = */EInternalObjectFlags::Garbage); It; ++It)
		{
			...
			do 
			{
				...
				{
					UE_LOGF(LogMaterial,Verbose,"--> %ls", *MaterialInstance->GetFullName());
...
FMaterialInstanceRenderProxy::FMaterialInstanceRenderProxy(UMaterialInstance* InOwner)
...
void FMaterialInstanceRenderProxy::GameThread_Destroy()
{
	FMaterialInstanceRenderProxy* RenderProxy = this;
	ENQUEUE_RENDER_COMMAND(FDestroyMaterialInstanceResourceCommand)(
		[RenderProxy](FRHICommandList& RHICmdList)
		{
			delete RenderProxy;
		});
}
...
const FMaterialRenderProxy* FMaterialInstanceRenderProxy::GetFallback(ERHIFeatureLevel::Type InFeatureLevel) const
{
	if (Parent)
	{
		if (Owner->bHasStaticPermutationResource)
...
const FMaterial* FMaterialInstanceRenderProxy::GetMaterialNoFallback(ERHIFeatureLevel::Type InFeatureLevel) const
{
	checkSlow(IsInParallelRenderingThread());
...
UMaterialInterface* FMaterialInstanceRenderProxy::GetMaterialInterface() const
{
	return Owner;
}

bool FMaterialInstanceRenderProxy::GetParameterValue(EMaterialParameterType Type, const FHashedMaterialParameterInfo& ParameterInfo, FMaterialParameterValue& OutValue, const FMaterialRenderContext& Context) const
{
	checkSlow(IsInParallelRenderingThread());
	...
	{
		const UToonProfile* SPOverrideRT = GetToonProfileOverrideRT();
		for (uint32 It=0,Count=NumToonProfileRT();It<Count;++It)
		{
			...
			{
				check(ParameterInfo.Association == EMaterialParameterAssociation::GlobalParameter);
				OutValue = ToonProfile::GetToonProfileId(SPOverrideRT ? SPOverrideRT : GetToonProfileRT(It));
				bResult = true;
				break;
			}
		}
...
bool FMaterialInstanceRenderProxy::GetUserSceneTextureOverride(FName& InOutName) const
{
	checkSlow(IsInParallelRenderingThread());
...
EBlendableLocation FMaterialInstanceRenderProxy::GetBlendableLocation(const FMaterial* Base) const
{
	check(Base);
	checkSlow(IsInParallelRenderingThread());
...
int32 FMaterialInstanceRenderProxy::GetBlendablePriority(const FMaterial* Base) const
{
	check(Base);
	checkSlow(IsInParallelRenderingThread());
...
void UMaterialInstance::PropagateDataToMaterialProxy()
{
	if (RenderProxy)
	{
		UpdateMaterialRenderProxy(*RenderProxy);
	}
}

void FMaterialInstanceRenderProxy::GameThread_SetParent(UMaterialInterface* ParentMaterialInterface)
{
	...
	if (GameThreadParent != ParentMaterialInterface)
	{
		...
		ENQUEUE_RENDER_COMMAND(InitMaterialInstanceResource)(
			[RenderProxy, ParentMaterialInterface](FRHICommandListImmediate& RHICmdList)
			{
				RenderProxy->Parent = ParentMaterialInterface;
				RenderProxy->InvalidateUniformExpressionCache(true);
			});
...
void FMaterialInstanceRenderProxy::GameThread_UpdateCachedData(const FMaterialInstanceCachedData& CachedData)
{
	ENQUEUE_RENDER_COMMAND(MaterialInstanceResource_UpdateCachedData)(
		[RenderProxy = this, ParentLayerIndexRemap = CachedData.ParentLayerIndexRemap](FRHICommandListImmediate& RHICmdList) mutable
		{
			RenderProxy->ParentLayerIndexRemap = MoveTemp(ParentLayerIndexRemap);
		});
}
...
void FMaterialInstanceRenderProxy::InitMIParameters(FMaterialInstanceParameterSet& ParameterSet)
{
...
void FMaterialInstanceRenderProxy::EvaluateParameterCollections(FRHICommandListBase& RHICmdList, FUniformExpressionCache& OutUniformExpressionCache, const FMaterialRenderContext& Context, FUniformExpressionCacheAsyncUpdater* Updater) const
{
...
void GameThread_UpdateMIParameter(const UMaterialInstance* Instance, const ParameterType& Parameter)
{
	if (FApp::CanEverRender())
	{
		...
		if (Material != nullptr)
		{
			...
			if (Domain != MD_PostProcess && Domain != MD_UI && !Instance->GetUsageByFlag(MATUSAGE_EditorCompositing))
			{
				GetRendererModule().InvalidatePathTracedOutput(PathTracing::EInvalidateReason::UpdateMaterialParameter);
			}
		}
		FMaterialInstanceRenderProxy* RenderProxy = Instance->GetInstanceRenderProxy();
		...
		ENQUEUE_RENDER_COMMAND(SetMIParameterValue)(
			[RenderProxy, ParameterInfo, Value](FRHICommandListImmediate& RHICmdList)
			{
				RenderProxy->RenderThread_UpdateParameter(ParameterInfo, Value);
...
void UMaterialInstance::PostInitProperties()	
{
	...
	{
		RenderProxy = new FMaterialInstanceRenderProxy(this);
...
void GameThread_InitMIParameters(const UMaterialInstance& Instance)
{
	...
	ENQUEUE_RENDER_COMMAND(InitMIParameters)(
		[RenderProxy, Parameters = MoveTemp(ParameterSet)](FRHICommandListImmediate& RHICmdList) mutable
		{
			RenderProxy->InitMIParameters(Parameters);
		});
}
...
{	
	...
	{
		RenderProxy->GameThread_SetParent(SafeParent);
		RenderProxy->GameThread_UpdateCachedData(GetCachedInstanceData());
	}
...
bool UMaterialInstance::GetParameterValue(EMaterialParameterType Type, const FMemoryImageMaterialParameterInfo& ParameterInfo, FMaterialParameterMetadata& OutResult, EMaterialGetParameterValueFlags Flags) const
{
	...
	// Look through the default values if we have not found an override
	if (!bResult && EnumHasAnyFlags(Flags, EMaterialGetParameterValueFlags::CheckNonOverrides))
	{
...
bool UMaterialInstance::GetRefractionSettings(float& OutBiasValue) const
{
	...
	{
		static FName NAME_RefractionDepthBias("RefractionDepthBias");
		ParamInfo.Name = NAME_RefractionDepthBias;
	}
	else
	{
		static FName NAME_RefractionBias("RefractionBias");
...
void UMaterialInstance::GetTextureExpressionValues(const FMaterialResource* MaterialResource, TArray<UTexture*>& OutTextures, TArray<TArray<int32>>* OutIndices, bool bIncludeTextureCollections, bool bAllowOverrides) const
{
	Super::GetTextureExpressionValues(MaterialResource, OutTextures, OutIndices, bIncludeTextureCollections, bAllowOverrides);
...
void UMaterialInstance::GetUsedTextures(TArray<UTexture*>& OutTextures, TOptional<EMaterialQualityLevel::Type> InSpecificQualityLevel, TOptional<EShaderPlatform> InSpecificShaderPlatform) const
{
	...
	if (!FPlatformProperties::IsServerOnly())
	{
		...
		if (BaseMaterial && !BaseMaterial->IsDefaultMaterial())
		{
			...
			TArray<const FMaterialResource*, TInlineAllocator<4>> MatchedResources = GetMaterialResources(MaterialToUse, QualityLevelRange, ShaderPlatformRange);

			for (const FMaterialResource* CurrentResource : MatchedResources)
			{
				constexpr bool bIncludeTextureCollections = true;
				constexpr bool bAllowOverrides = true;
				GetTextureExpressionValues(CurrentResource, OutTextures, nullptr, bIncludeTextureCollections, bAllowOverrides);
			}
		}
		else
		{
		...
		else if (MaterialInstanceToUse)
		{
			...
			if (MaterialResource)
			{
				if (MaterialResource->HasValidGameThreadShaderMap())
				{
					...
					constexpr bool bIncludeTextureCollections = false;
					constexpr bool bAllowOverrides = true;
					GetTextureExpressionValues(MaterialResource, Textures, nullptr, bIncludeTextureCollections, bAllowOverrides);
...
{
	if (!(IsInGameThread() || IsAsyncLoading() || UE::IsSavingPackage()))
	{
		...
		for (const FMaterialTextureParameterInfo& TextureInfo : CurrentResource->GetUniformTextureExpressions(ParameterType))
		{
			...
			if (Texture)
			{
				...
				case EMaterialTextureParameterType::Standard2D:
					...
					{
						UE_LOGF(LogMaterial, Error, "MaterialInstance \"%ls\" parameter '%ls' assigned texture \"%ls\" has invalid type, required 2D texture", *MaterialName, *TextureInfo.GetParameterName().ToString(), *Texture->GetName());
					}
					else if (bShouldValidateVTUsage && (TextureType & MCT_TextureVirtual))
					{
						UE_LOGF(LogMaterial, Error, "MaterialInstance \"%ls\" parameter '%ls' assigned texture \"%ls\" requires non-virtual texture", *MaterialName, *TextureInfo.GetParameterName().ToString(), *Texture->GetName());
					}
					break;
				case EMaterialTextureParameterType::Cube:
					if (!(TextureType & MCT_TextureCube))
					{
						UE_LOGF(LogMaterial, Error, "MaterialInstance \"%ls\" parameter '%ls' assigned texture \"%ls\" has invalid type, required Cube texture", *MaterialName, *TextureInfo.GetParameterName().ToString(), *Texture->GetName());
					}
					break;
				case EMaterialTextureParameterType::Array2D:
					if (!(TextureType & MCT_Texture2DArray))
					{
						UE_LOGF(LogMaterial, Error, "MaterialInstance \"%ls\" parameter '%ls' assigned texture \"%ls\" has invalid type, required texture array", *MaterialName, *TextureInfo.GetParameterName().ToString(), *Texture->GetName());
					}
					break;
				case EMaterialTextureParameterType::ArrayCube:
					if (!(TextureType & MCT_TextureCubeArray))
					{
						UE_LOGF(LogMaterial, Error, "MaterialInstance \"%ls\" parameter '%ls' assigned texture \"%ls\" has invalid type, required texture cube array", *MaterialName, *TextureInfo.GetParameterName().ToString(), *Texture->GetName());
					}
					break;
				case EMaterialTextureParameterType::Volume:
					if (!(TextureType & MCT_VolumeTexture))
					{
						UE_LOGF(LogMaterial, Error, "MaterialInstance \"%ls\" parameter '%ls' assigned texture \"%ls\" has invalid type, required Volume texture", *MaterialName, *TextureInfo.GetParameterName().ToString(), *Texture->GetName());
					}
					break;
				case EMaterialTextureParameterType::Virtual:
					...
					{
						UE_LOGF(LogMaterial, Error, "MaterialInstance \"%ls\" parameter '%ls' assigned texture \"%ls\" requires virtual texture", *MaterialName, *TextureInfo.GetParameterName().ToString(), *Texture->GetName());
					}
					break;
				default:
					checkNoEntry();
	...
	if (!FPlatformProperties::IsServerOnly())
	{
		...
		if (MaterialInstanceToUse && MaterialInstanceToUse->bHasStaticPermutationResource)
		{
			...
			if (CurrentResource)
			{
				constexpr bool bIncludeTextureCollections = false;
				constexpr bool bAllowOverrides = true;
				GetTextureExpressionValues(CurrentResource, OutTextures, &OutIndices, bIncludeTextureCollections, bAllowOverrides);
			}
		}
		...
		{ 
			...
			if (Material)
			{
				...
				if( MaterialResource )
				{
					constexpr bool bIncludeTextureCollections = false;
					constexpr bool bAllowOverrides = true;
					GetTextureExpressionValues(MaterialResource, OutTextures, &OutIndices, bIncludeTextureCollections, bAllowOverrides);
				}
			}
...
uint32 UMaterialInstance::GetUsageFlags() const
{
	return UsageFlags;
}

bool UMaterialInstance::GetUsageByFlag(EMaterialUsage Usage) const
{
	return (UsageFlags & (1u << Usage)) != 0;
}

void UMaterialInstance::SetUsageByFlag(EMaterialUsage Usage, bool NewValue)
{
...
bool UMaterialInstance::SetMaterialUsage(EMaterialUsage Usage)
{
	UMaterial* Material = GetMaterial();
	if (Material == nullptr)
	{
	...
	if (!GetUsageByFlag(Usage))
	{
		// For materials which do not have their UsageFlags set the DefaultMaterial<type> should be used in game
...
#endif
		if (GIsEditor && !bIsGame && Material->bAutomaticallySetUsageInEditor)
		{
			...
			// Open a material update context so this material can be modified safely.
			{
				...
				UpdateContext.AddMaterialInstance(this);
...
#if WITH_EDITOR
			// Rebuild any open material instance editors.
			UMaterialEditingLibrary::RebuildMaterialInstanceEditors(GetMaterial());
#endif
		}
		else
		{
			...
			if ((UsageFlagWarnings & UsageFlagBit) == 0)
			{
				...
				{
					UE_LOGF(LogMaterial, Warning, "     The material instance will recompile every editor launch until resaved.");
				}
...
uint32 UMaterialInstanceDynamic::GetUsageFlags() const
{
	return Parent ? Parent->GetUsageFlags() : 0;
}

bool UMaterialInstanceDynamic::GetUsageByFlag(EMaterialUsage Usage) const
{
	return Parent ? Parent->GetUsageByFlag(Usage) : false;
}

bool UMaterialInstanceDynamic::SetMaterialUsage(EMaterialUsage Usage)
{
	return Parent ? Parent->SetMaterialUsage(Usage) : false;
}
...
void UMaterialInstance::SetStaticComponentMaskParameterValueEditorOnly(const FMaterialParameterInfo& ParameterInfo, FStaticComponentMaskValue Value)
{
	check(GIsEditor || IsRunningCommandlet());
	...
	for (FStaticComponentMaskParameter& Parameter : EditorOnly->StaticParameters.StaticComponentMaskParameters)
	{
	...
	new(EditorOnly->StaticParameters.StaticComponentMaskParameters) FStaticComponentMaskParameter(ParameterInfo, Value.R, Value.G, Value.B, Value.A, true, FGuid());
}
...
#else
	// Discard the MI's cooked shadermap - it was compiled against a parent that is no longer available, so its uniform buffer layout is incompatible with WorldGridMaterial's shaders.
	// Even though we already set BlendMode to BLEND_Opaque, GetMaterialResource() would still serve up the MIC's own StaticPermutationMaterialResources (compiled against the missing parent) 
	// while the render proxy uses WorldGridMaterial's uniform buffers.
	// This produces at draw-time a 'FOpaqueBasePassUniformParameters vs TranslucentBasePass' check failure because the two were compiled against different parents. 
	// Clearing the flag makes GetMaterialResource() delegate entirely to Parent (WorldGridMaterial), so shadermap and uniform buffers come from the same source and stay consistent.
	if (!Parent && bHasStaticPermutationResource)
	{
		UE_LOGF(LogMaterial, Error, "MaterialInstance [%ls] clearing bHasStaticPermutationResource to avoid shadermap/blendmode mismatch with default material.", *GetName());
		bHasStaticPermutationResource = false;
	}
#endif // WITH_EDITORONLY_DATA
...
void UMaterialInstance::UpdateOverridableBaseProperties()
{
	...
	const uint32 ParentUsageFlags = Parent->GetUsageFlags();
	UsageFlags = ParentUsageFlags;
	// Add flags that are overriden and set.
	UsageFlags |= BasePropertyOverrides.bOverride_UsageFlags & BasePropertyOverrides.UsageFlags;
	// Remove flags that are overriden and unset.
	UsageFlags &= ~(BasePropertyOverrides.bOverride_UsageFlags & ~BasePropertyOverrides.UsageFlags);
	// Non overridden flags default to parent value.
	BasePropertyOverrides.UsageFlags |= ~BasePropertyOverrides.bOverride_UsageFlags & ParentUsageFlags;
...
bool UMaterialInstance::IsCachedCookedPlatformDataLoaded( const ITargetPlatform* TargetPlatform ) 
{
	...
	if (UE_LOG_ACTIVE(LogMaterial, VeryVerbose))
	{
		...
		UE_LOGF(LogMaterial, VeryVerbose, "MaterialInstance [%ls] Cook State:", *GetName());
...
void UMaterialInstance::Serialize(FArchive& Ar)
{
	...
	if (Ar.IsSaving() && Ar.IsCooking())
	{
		...
		{
			UpdateReferencedDefaultTextures();
			constexpr bool bKeepProjectPrivateTextures = false;
			StripUnusedDefaultTextures(bKeepProjectPrivateTextures);
		}
		else
		{
			// All textures will be kept in Base materials
	...
	if (Ar.IsLoading() && Ar.CustomVer(FFortniteMainBranchObjectVersion::GUID) < FFortniteMainBranchObjectVersion::AddPhysMatOverrideFlag)
	{
		// Set flag on materials which had Phys Mat already overriden
		bOverridePhysMaterial = bool(PhysMaterial);
	}
#endif
}
...
void UMaterialInstance::PostLoad()
{
	...
	if (IsPSOShaderPreloadingEnabled())
	{
		...
		if (IsUIMaterial() || IsDeferredDecal() || IsPostProcessMaterial() || IsLightFunctionMaterial())
		{
			FGraphEventArray Unused;
			PreloadMaterialShaderMap(GetMaterialResource(GMaxRHIShaderPlatform), Unused);
		}
	}
	else if (IsDeferredDecal() || IsUIMaterial() || IsPostProcessMaterial() || IsLightFunctionMaterial())
	{		
...
bool UMaterialInstance::IsStaticPermutationAllowedForCandidateParent(UMaterialInterface* CandidateParent) const
{
	...
	if (!CandidateParent || !bEnableRestrictiveMaterialMode)
	{
		return true;
	}
	...
	{
		UE_LOGF(LogMaterial, Warning,
			   "MaterialInstance '%ls' cannot allow static parameter permutations because its parent '%ls' ultimately derives from a restricted material.",
			   *this->GetName(),
...
void UMaterialInstance::ValidateStaticPermutationAllowed()
{
	...
	if (bHasStaticPermutationResource && bDisallowStaticParameterPermutations)
	{
		...
		UE_LOGF(LogMaterial, Warning, "Material instance '%ls' with cooked non-user non-base parent material '%ls' is not allowed to create new shader permutations. Setting parent to null.", *GetName(), *PrevParent->GetName());
...
void UMaterialInstance::BeginDestroy()
{
	...
	if (RenderProxy || ResourcesToDestroy.Num() > 0)
	{
		...
		[ResourcesToDestroy = MoveTemp(ResourcesToDestroy), this](FRHICommandListImmediate& RHICmdList) mutable
		{
			if (RenderProxy)
			{
				RenderProxy->MarkForGarbageCollection();
				RenderProxy->ReleaseResource();
			}
			...
			{
				RenderProxy->CancelCacheUniformExpressions();
				bCachingUniformExpressions = false;
			}
...
void UMaterialInstance::FinishDestroy()
{
	if(!HasAnyFlags(RF_ClassDefaultObject))
	{
		if (RenderProxy)
		{
			RenderProxy->GameThread_Destroy();
			RenderProxy = nullptr;
		}
	}
...
bool UMaterialInstance::SetParentInternal(UMaterialInterface* NewParent, bool RecacheShaders)
{
	...
	if (!Parent || Parent != NewParent)
	{
		...
		{
			UE_LOGF(LogMaterial, Warning, "%ls is not a valid parent for %ls as it is already a child of this material instance.",
				   *ParentAsMaterialInstance->GetFullName(),
				   *GetFullName());
		}
		else if (NewParent &&
		...
		{
			UE_LOGF(LogMaterial, Warning, "%ls is not a valid parent for %ls. Only Materials and MaterialInstanceConstants are valid parents for a material instance. Outer is %ls",
...
void UMaterialInstance::SetTextureParameterValueInternal(const FMaterialParameterInfo& ParameterInfo, UTexture* Value)
{
	...
	if (bForceUpdate || ParameterValue->ParameterValue != Value)
	{
		...
		if (Value && ensureMsgf(Value->IsA(UTexture::StaticClass()), TEXT("Expecting a UTexture! Value='%s' class='%s'"), *Value->GetName(), *Value->GetClass()->GetName()))
		{
			...
			// The set of textures (and their sampling scales) used by this instance has changed,
			// so the streaming-texture sampling cache must be rebuilt.
			CacheTexturesSamplingInfo();
...
void UMaterialInstance::ClearParameterValuesInternal(EMaterialInstanceClearParameterFlag Flags)
{
	...
	if (RenderProxy && bUpdateResource)
	{
		FMaterialInstanceRenderProxy* InRenderProxy = RenderProxy;
		ENQUEUE_RENDER_COMMAND(FClearMIParametersCommand)(
			[InRenderProxy](FRHICommandList& RHICmdList)
			{
				InRenderProxy->RenderThread_ClearParameters();
			});
	}
...
void UMaterialInstance::UpdateStaticPermutation(const FStaticParameterSet& NewParameters, FMaterialInstanceBasePropertyOverrides& NewBasePropertyOverrides, const bool bForceStaticPermutationUpdate /*= false*/, FMaterialUpdateContext* MaterialUpdateContext)
{
	...
	const bool bRecacheShaders = (bHasStaticPermutationResource != bWantsStaticPermutationResource) || bParamsHaveChanged || (bBasePropertyOverridesHaveChanged && bWantsStaticPermutationResource) || bForceStaticPermutationUpdate;
	...
	// Not all base property overrides trigger a static permutation but changing them still requires render state recreation.
	const bool bAddToUpdateContext = bRecacheShaders || bBasePropertyOverridesHaveChanged;

	if (bAddToUpdateContext)
	{
...
void UMaterialInstance::SetPermutationParameters(const FStaticParameterSet& InParameters, FMaterialInstanceBasePropertyOverrides& InBasePropertyOverrides)
{
	UMaterialInstanceEditorOnlyData* EditorOnly = GetEditorOnlyData();
	FStaticParameterSet NewParameters = InParameters;

	TrimToOverriddenOnly(NewParameters.StaticSwitchParameters);
	TrimToOverriddenOnly(NewParameters.EditorOnly.StaticComponentMaskParameters);
	
	BasePropertyOverrides = InBasePropertyOverrides;

	UpdateOverridableBaseProperties();

	StaticParametersRuntime = NewParameters.GetRuntime();
	EditorOnly->StaticParameters = NewParameters.EditorOnly;
}
...
{
	for (UObject* UsedObject : GetDefaultTextures())
	{
...
#if WITH_EDITOR
	// Editor-only path. All callers (PostLoad, PostEditChange, ForceRecompileForRendering, static-parameter
	// updates) run on the game thread; CacheTexturesSamplingInfo asserts IsInGameThread() || IsInParallelGameThread()
	// so this call inherits that invariant.
	CacheTexturesSamplingInfo();
#endif
}

void UMaterialInstance::UpdateReferencedDefaultTextures()
{
	const FStaticParameterSet& LocalStaticParameters = GetStaticParameters();
	if (bHasStaticPermutationResource || LocalStaticParameters.bHasMaterialLayers)
	{
...
void UMaterialInstance::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if ((PropertyChangedEvent.ChangeType & EPropertyChangeType::Interactive) == 0)
	{
		...
		if (GIsEditor)
		{
			...
			if (WritesToRuntimeVirtualTexture() || GetCachedExpressionData().bHasMaterialCacheOutput)
			{
...
void UMaterialInstance::GetResourceSizeEx(FResourceSizeEx& CumulativeResourceSize)
{
	...
	if (RenderProxy)
	{
		CumulativeResourceSize.AddDedicatedSystemMemoryBytes(sizeof(FMaterialInstanceRenderProxy));
...
uint64 UMaterialInstance::GetBasePropertyOverridesHash() const
{
	check(IsInGameThread());
	...
	auto GetPropertyOverrideHash = [&](auto InstanceValue, auto MatValue, const FString &HashString)
	{
		...
		if (bOverridden)
		{
			Hash.Update(*HashString, HashString.NumBytesWithoutNull());
	...
	GetPropertyOverrideHash(GetUsageFlags(), Mat->GetUsageFlags(), TEXT("bOverride_UsageFlags"));
...
bool UMaterialInstance::HasOverridenBaseProperties() const
{
	...
	// Note that we only return true for BaseProperties that need to trigger a static permutation.
	return
...
FString UMaterialInstance::GetBasePropertyOverrideString() const
{
	...
	{
		// Only append information for the BaseProperties that cause a static permutation and are checked in HasOverridenBaseProperties().
		...
		BasePropString.Appendf(TEXT("bOverride_UsageFlags_%d "), (GetUsageFlags() != Parent->GetUsageFlags()));
	}
...
bool UMaterialInstance::IsLightFunctionMaterial() const
{
	return Parent ? Parent->IsLightFunctionMaterial() : false;
}
...
{
	// Return the specular profile count form the root material.
	checkSlow(IsInGameThread());
...
uint32 UMaterialInstance::NumToonProfile_Internal() const
{
	// Return the toon profile count form the root material.
	checkSlow(IsInGameThread());
	return Parent ? Parent->NumToonProfile_Internal() : 0;
}

UToonProfile* UMaterialInstance::GetToonProfile_Internal(uint32 Index) const
{
	// Return the Toon profile from the root material.
	checkSlow(IsInGameThread());

	return Parent ? Parent->GetToonProfile_Internal(Index) : 0;
}

UToonProfile* UMaterialInstance::GetToonProfileOverride_Internal() const
{
	// Recursively look for override among the ancestors
	UToonProfile* Out = nullptr;
	if (bOverrideToonProfile && ToonProfileOverride)
	{
		Out = ToonProfileOverride;
	}
	else if (Parent)
	{
		Out = Parent->GetToonProfileOverride_Internal();
	}
	return Out;
}
...
bool UMaterialInstance::TryGetUsedTexturesInCookCache(TArray<TObjectPtr<UObject>>& OutTextures)
{
	OutTextures.Empty();
	...
	for (const FMaterialResource* CurrentResource : MatchedResources)
	{
		...
		for (int32 TypeIndex = 0; TypeIndex < NumMaterialTextureParameterTypes; ++TypeIndex)
		{
			for (int32 TextureIndex = 0; TextureIndex < CurrentSet.GetNumTextures((EMaterialTextureParameterType)TypeIndex); ++TextureIndex)
			{
				const FMaterialTextureParameterInfo& Parameter = CurrentSet.GetTextureParameter((EMaterialTextureParameterType)TypeIndex, TextureIndex);
				FMaterialParameterMetadata SourceValue;
				bool bIsOverriden = false;

				// Do not add overriden default textures
				for (int32 ParamTypeIdx = (int32)EMaterialParameterType::Texture; (ParamTypeIdx <= (int32)EMaterialParameterType::SparseVolumeTexture) && !bIsOverriden; ++ParamTypeIdx)
				{
					bIsOverriden |= GetParameterValue((EMaterialParameterType)ParamTypeIdx, Parameter.ParameterInfo, SourceValue, EMaterialGetParameterValueFlags::CheckInstanceOverrides);
				}
...
bool UMaterialInstance::CalculateTexturesSamplingInfo(UTexture* Texture, FTextureSamplingInfo& OutTextureSamplingInfo) const
{
	const bool bResultIsValid = Super::CalculateTexturesSamplingInfo(Texture, OutTextureSamplingInfo);
	if (!bResultIsValid && Parent)
	{
		return Parent->CalculateTexturesSamplingInfo(Texture, OutTextureSamplingInfo);
	}
...
bool UMaterialInstance::IsRedundant() const
{
	...
	if (Substrate::IsSubstrateEnabled() && NumToonProfile_Internal() > 0 && GetToonProfile_Internal(0) != Parent->GetToonProfile_Internal(0))
	{
		return false;
	}
	...
	|| BasePropertyOverrides.bOverride_UsageFlags)
	{
		return false;
	}
	if (
...
static void FindRedundantMICS(const TArray<FString>& Args)
{
	...
	UE_LOGF(LogConsoleResponse, Display, "----------------------------- %d UMaterialInstance's %d redundant ", MICs.Num(), NumRedundant);
}
...
void UMaterialInstance::DumpDebugInfo(FOutputDevice& OutputDevice) const
{
	if (Parent)
	{
		...
		{
			UE_LOGF(LogConsoleResponse, Display, "    This MIC does not have static permulations, and is therefore is just a version of the parent.");
		}
	}
}

TObjectPtr<UObject> UMaterialInstance::GetDefaultTextureAtIdx(const int32 InIdx, TMicRecursionGuard RecursionGuard)
{
#if WITH_EDITORONLY_DATA
	const TArray<TObjectPtr<UObject>>& UsedTextures = GetDefaultTextures();
#else
	const TArray<TObjectPtr<UObject>>& UsedTextures = ReferencedDefaultTextures;
#endif

	// We also look through the Parents in case there's a texture that has been forced to 
	// remain there because it is Project Private (PKG_NotExternallyReferenceable)

	if (UsedTextures.IsValidIndex(InIdx) && UsedTextures[InIdx] != nullptr)
	{
		return UsedTextures[InIdx];
	}

	if (Parent && !RecursionGuard.Contains(this))
	{
		RecursionGuard.Set(this);
		return Parent->GetDefaultTextureAtIdx(InIdx, RecursionGuard);
	}
...
void UMaterialInstance::GetShaderTypes(EShaderPlatform Platform, const ITargetPlatform* TargetPlatform, TArray<FDebugShaderTypeInfo>& OutShaderInfo)
{
	if (bHasStaticPermutationResource)
	{
		...
		for (FMaterialResource* CurrentResource : StaticPermutationMaterialResources)
		{
			CurrentResource->GetShaderTypes(LayoutParams, OutShaderInfo, FConservativeShaderParameters(CurrentResource));
		}
	}
}
#endif // WITH_EDITOR
...
void UMaterialInstance::OverrideTextureParameterValue(const UTexture* InTextureToOverride, UTexture* OverrideTexture)
{
	...
	if (ParentInstance)
	{
		...
		{
			FMaterialInstanceRenderProxy* LocalRenderProxy = RenderProxy;
			if (LocalRenderProxy)
			{
				ENQUEUE_RENDER_COMMAND(RefreshMIParameterValue)
					(
						[LocalRenderProxy](FRHICommandListImmediate& RHICmdList)
						{
							LocalRenderProxy->CacheUniformExpressions(RHICmdList, false);
						}
				);
			}
#if WITH_EDITOR
