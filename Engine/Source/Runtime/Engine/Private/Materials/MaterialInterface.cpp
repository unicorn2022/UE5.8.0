...
DECLARE_CYCLE_STAT(TEXT("MaterialInterface StripUnusedDefaultTextures"), STAT_MaterialInterface_StripUnusedDefaultTextures, STATGROUP_Shaders);
...
#if WITH_EDITOR
static bool GShouldStripUnusedDefaultTextures = true;
static FAutoConsoleVariableRef CVarStripUnusedDefaultTextures(
	TEXT("r.Material.StripUnusedDefaultTextures"),
	GShouldStripUnusedDefaultTextures,
	TEXT("If enabled, unused Default textures will be discarded at cook time.\n"),
	ECVF_ReadOnly);
#endif
...
static bool SafeRenameEditorOnlyData(UMaterialInterface* MaterialInterface, const FString& EditorOnlyDataName, bool bLogMessageIfRenamed)
{
	...
	if (EditorOnlyWithIncorrectName && EditorOnlyWithIncorrectName != EditorOnlyCurrent)
	{
		...
		{
			UE_LOGF(LogMaterial, Warning, "MaterialInterface '%ls' editor only data ('%ls') could not be marked for garbage bacause it was rooted. Please resave this material.", *MaterialInterface->GetName(), *EditorOnlyWithIncorrectName->GetName());
			return false;
		}
		FName UniqueDummyName = MakeUniqueObjectName(GetTransientPackage(), UMaterialInterfaceEditorOnlyData::StaticClass());
		EditorOnlyWithIncorrectName->Rename(*UniqueDummyName.ToString(), GetTransientPackage(), REN_NonTransactional | REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
	...
	if (EditorOnlyCurrent && EditorOnlyCurrent->GetName() != EditorOnlyDataName)
	{
		...
		{
			UE_LOGF(LogMaterial, Display, "MaterialInterface %ls has a incorrectly name EditorOnlyData '%ls'. This may cause issues when running the editor on cooked data. Trying to rename it to the correct name '%ls'.", *MaterialInterface->GetName(), *EditorOnlyCurrent->GetName(), *EditorOnlyDataName);
		}
...
void UMaterialInterface::Serialize(FArchive& Ar)
{
	...
	if (FArchiveCookContext* CookContext = Ar.GetCookContext())
	{
		...
		// Make sure we are not cooking materials with the new translator flag enabled as it is still experimental.
		const UMaterial* Material = GetMaterial_Concurrent();
		if (Material && Material->IsUsingNewHLSLGenerator())
		{
			UE_LOGF(LogMaterial, Display, "Material '%ls' is marked to use the new material translator. Please refrain from submitting materials using the new translator as it is still experimental.", *GetName());
		}
	}
#endif
...
#if WITH_EDITORONLY_DATA
int32 UMaterialInterface::GetDefaultTextureIdx(UObject* InTexture) const
{
	const FMaterialCachedExpressionData& UsedCachedData = GetCachedExpressionData();
	return UsedCachedData.GetDefaultTextureIdx(InTexture);
}

bool UMaterialInterface::ContainsDefaultTexture(UObject* InTexture) const
{
	return GetDefaultTextureIdx(InTexture) != INDEX_NONE;
}

const TArray<TObjectPtr<UObject>>& UMaterialInterface::GetDefaultTextures() const
{
	const FMaterialCachedExpressionData& UsedCachedData = GetCachedExpressionData();

	// Can happen inside a Material with bLoadedCachedExpressionData and no EditorOnlyData
	if (UsedCachedData.EditorOnlyData->EditorReferencedDefaultTextures.Num() == 0)
	{
		return ReferencedDefaultTextures;
	}

	return UsedCachedData.EditorOnlyData->EditorReferencedDefaultTextures;
}
...
bool UMaterialInterface::IsUsingNewHLSLGenerator() const
{
	...
	if (bEnableRestrictiveMaterialMode || !CVar->GetValueOnAnyThread())
	{
		return false;
	}
...
{
	return IsUsingNewHLSLGenerator();
}
...
{
	const TArray<TObjectPtr<UObject>> Textures = GetDefaultTextures();
	for (UObject* UsedObject : Textures)
	{
...
void UMaterialInterface::GetTextureExpressionValues(const FMaterialResource* MaterialResource, TArray<UTexture*>& OutTextures, TArray<TArray<int32>>* OutIndices, bool bIncludeTextureCollections, bool bAllowOverrides) const
{
	...
	for (int32 TypeIndex = 0; TypeIndex < NumMaterialTextureParameterTypes; ++TypeIndex)
	{
		...
		for (int32 TextureIndex = 0; TextureIndex < UniformExpressions.GetNumTextures((EMaterialTextureParameterType)TypeIndex); TextureIndex++)
		{
			...
			UniformExpressions.GetGameThreadTextureValue((EMaterialTextureParameterType)TypeIndex, TextureIndex, this, *MaterialResource, Texture, bAllowOverrides);
...
TArray<const FMaterialResource*, TInlineAllocator<4>> UMaterialInterface::GetMaterialResources(const UMaterialInterface* InMaterial, const FInt32Range InQualityLevelRange, const FInt32Range InShaderPlatformRange)
{
	TArray<const FMaterialResource*, TInlineAllocator<4>> MatchedResources;
	// Parse all relevant quality and feature levels.
	for (int32 QualityLevelIndex = InQualityLevelRange.GetLowerBoundValue(); QualityLevelIndex <= InQualityLevelRange.GetUpperBoundValue(); ++QualityLevelIndex)
	{
		for (int32 ShaderPlatformIndex = InShaderPlatformRange.GetLowerBoundValue(); ShaderPlatformIndex <= InShaderPlatformRange.GetUpperBoundValue(); ++ShaderPlatformIndex)
		{
			...
			const FMaterialResource* CurrentResource = InMaterial->GetMaterialResource((EShaderPlatform)ShaderPlatformIndex, (EMaterialQualityLevel::Type)QualityLevelIndex);
			if (CurrentResource)
			{
...
{
#if !WITH_EDITOR
	if (const FCachedMaterialRelevance* Cached = CachedMaterialRelevance.TryGet())
	{
		return Cached->Relevance.bUsesWorldPositionOffset;
	}
#endif
...
bool UMaterialInterface::IsUsingMotionVectorWorldOffset_Concurrent(EShaderPlatform InShaderPlatform) const
{
	...
	if (const FCachedMaterialRelevance* Cached = CachedMaterialRelevance.TryGet())
	{
		return Cached->bUsesMotionVectorWorldOffset;
	}
#endif
	if (const UMaterial* Material = GetMaterial_Concurrent())
	{
...
bool UMaterialInterface::IsUsingTemporalResponsiveness_Concurrent(EShaderPlatform InShaderPlatform) const
{
	...
	if (const FCachedMaterialRelevance* Cached = CachedMaterialRelevance.TryGet())
	{
		return Cached->bUsesTemporalResponsiveness;
	}
#endif
	if (const UMaterial* Material = GetMaterial_Concurrent())
	{
...
FMaterialRelevance UMaterialInterface::GetRelevance_Internal(const UMaterial* Material, EShaderPlatform InShaderPlatform) const
{
	...
	const bool bIsSinglePassWaterTranslucent = bIsMobile && bUsesSingleLayerWaterMaterial && IsUsingSingleLayerWaterFallback(InShaderPlatform);
	...
	static const TArray<EMaterialShadingModel> AnisoShadingModels = { MSM_DefaultLit, MSM_ClearCoat };
	bool bUsesAnisotropy = MaterialResource->GetShadingModels().HasAnyShadingModel(AnisoShadingModels) && 
	...
	MaterialRelevance.bUsesSingleLayerWaterMaterial = (bUsesSingleLayerWaterMaterial && !bIsSinglePassWaterTranslucent);
...
#if !WITH_EDITOR
void UMaterialInterface::WarmRelevanceCache(EShaderPlatform InShaderPlatform)
{
	if (!CachedMaterialRelevance.TryGet())
	{
		check(IsInGameThread());
		FCachedMaterialRelevance Cache;
		Cache.Relevance = GetRelevance_Concurrent(InShaderPlatform);
		Cache.bUsesMotionVectorWorldOffset = IsUsingMotionVectorWorldOffset_Concurrent(InShaderPlatform);
		Cache.bUsesTemporalResponsiveness = IsUsingTemporalResponsiveness_Concurrent(InShaderPlatform);
		CachedMaterialRelevance.Set(Cache);
	}
}
#endif // !WITH_EDITOR
...
void UMaterialInterface::AddReferencedObjects(UObject* InThis, FReferenceCollector& Collector)
{
	...
	{
		// Snapshot the TSharedPtr under the read lock and release the lock before invoking the
		// collector. The TSharedPtr keeps the cache alive past lock release, and writers
		// (CacheTexturesSamplingInfo) don't have to wait on the per-texture collector callbacks.
		TSharedPtr<FMaterialCachedTexturesSamplingInfo> CachedSnapshot;
		{
			UE::TReadScopeLock<FCacheRWLock> ReadLock(This->CachedTexturesSamplingInfoLock);
			CachedSnapshot = This->CachedTexturesSamplingInfo;
		}
		if (CachedSnapshot)
		{
			CachedSnapshot->AddReferencedObjects(Collector);
		}
	}
...
void FMaterialCachedTexturesSamplingInfo::AddReferencedObjects(FReferenceCollector& Collector)
{
	for (FTextureSamplingInfo& Info : TextureSamplingInfos)
	{
		Collector.AddReferencedObject(Info.Texture);
	}
}

void FMaterialCachedTexturesSamplingInfo::GetStreamingRenderAssetPrimitiveInfo(
	const FBoxSphereBounds& ComponentBounds, const float ComponentScaling, const FMeshUVChannelInfo& InUVChannelData,
	const EMaterialQualityLevel::Type& MaterialQualityLevel, TArray<FStreamingRenderAssetPrimitiveInfo>& OutStreamingTextures) const
{
	const TArray<uint16>& Mapping = ShaderPlatformMapping.QualityMapping[MaterialQualityLevel];

	for (const uint16& SamplingInfoIndex : Mapping)
	{
		check(TextureSamplingInfos.IsValidIndex(SamplingInfoIndex));
		const FTextureSamplingInfo& TextureSamplingInfo = TextureSamplingInfos[SamplingInfoIndex];
		check(TextureSamplingInfo.Texture);

		const float TextureDensity = [&]
		{
			const float MaterialDensity = TextureSamplingInfo.GetTextureDensity(InUVChannelData);
			// fallback based on FStreamingTextureLevelContext::ProcessMaterial
			if (!MaterialDensity)
			{
				return InUVChannelData.LocalUVDensities[0];
			}
			return MaterialDensity;
		}();

		FStreamingRenderAssetPrimitiveInfo& Info = OutStreamingTextures.AddDefaulted_GetRef();
		Info.RenderAsset = TextureSamplingInfo.Texture;
		...
		// checkSlow rather than ensure: this runs per-texture-per-primitive on every SSAM registration.
		// The invariant is that TextureDensity (from the cache or UV fallback) and ComponentScaling are finite;
		// non-finite values imply an upstream bug worth catching in Debug builds, not worth the per-call cost
		// in Development/Test/Shipping.
		checkSlow(FMath::IsFinite(Info.TexelFactor));
	}
}
...
void UMaterialInterface::AppendToClassSchema(FAppendToClassSchemaContext& Context)
{
	...
	const bool bStripUnusedDefaultTextures = ShouldStripUnusedDefaultTextures();
	const uint8 Byte = bStripUnusedDefaultTextures ? 1 : 0;
...
bool UMaterialInterface::IsLightFunctionMaterial() const
{
	return false;
}
...
UToonProfile* UMaterialInterface::GetToonProfile_Internal(uint32 Index) const
{
	return nullptr;
}

UToonProfile* UMaterialInterface::GetToonProfileOverride_Internal() const
{
	return nullptr;
}
...
uint32 UMaterialInterface::NumToonProfile_Internal() const
{
	return 0u;
}
...
void UMaterialInterface::UpdateMaterialRenderProxy(FMaterialRenderProxy& Proxy)
{
	...
	if (Substrate::IsSubstrateEnabled())
	{
		...
		// Toon profiles
		{
			struct FEntry
			{
				UToonProfile* Profile = nullptr;
				FToonProfileStruct Settings;
				FGuid Guid;
			};
			TArray<FEntry> Entries;
			for (int32 It = 0, Count = NumToonProfile_Internal(); It < Count; ++It)
			{
				FEntry& Entry = Entries.AddDefaulted_GetRef();
				Entry.Profile = GetToonProfile_Internal(It);
				if (Entry.Profile)
				{
					Entry.Settings = Entry.Profile->Settings;
					Entry.Guid = Entry.Profile->Guid;
			...
			UToonProfile* ToonProfileOverride = GetToonProfileOverride_Internal();
			if (Entries.Num() > 0 || ToonProfileOverride)
			{
				ENQUEUE_RENDER_COMMAND(UpdateMaterialRenderProxyToon)(
					[InProxy, Entries, ToonProfileOverride](FRHICommandListImmediate& RHICmdList)
					{
						for (const FEntry& Entry : Entries)
						{
							if (Entry.Profile)
							{
								const uint32 AllocationId = ToonProfile::AddOrUpdateProfile(Entry.Profile, Entry.Guid, Entry.Settings);
								check(AllocationId >= 0 && AllocationId < MAX_TOON_PROFILE_COUNT);
							}
							InProxy->AddToonProfileRT(Entry.Profile);
						}

						if (ToonProfileOverride)
						{
							ToonProfile::AddOrUpdateProfile(ToonProfileOverride, ToonProfileOverride->Guid, ToonProfileOverride->Settings);
						}

						// Set the Toon profile override using the default ToonProfile pointer.
						InProxy->SetToonProfileOverrideRT(ToonProfileOverride);
					});
			}
		}
	}
...
#if WITH_EDITOR
bool UMaterialInterface::ShouldStripUnusedDefaultTextures()
{
	return GShouldStripUnusedDefaultTextures;
}

TArray<const FMaterialResource*, TInlineAllocator<4>> UMaterialInterface::GetCookedCacheMaterialResources() const
{
	TArray<const FMaterialResource*, TInlineAllocator<4>> CookedResources;
	for (const auto& KVP : CachedMaterialResourcesForCooking)
	{
		const TArray<FMaterialResourceForCooking>& PlatformResources = KVP.Value;

		for (const FMaterialResourceForCooking& ResForCooking : PlatformResources)
		{
			if (ResForCooking.Resource)
			{
				CookedResources.Add(ResForCooking.Resource);
			}
		}
	}

	return CookedResources;
}

void UMaterialInterface::StripUnusedDefaultTextures(const bool bInKeepProjectPrivateTextures)
{
	SCOPE_CYCLE_COUNTER(STAT_MaterialInterface_StripUnusedDefaultTextures);

	if (!ShouldStripUnusedDefaultTextures())
	{
		static bool bWarnedOnce = false;
		if (!bWarnedOnce)
		{
			UE_LOGF(LogMaterial, Warning, "Calling StripUnusedDefaultTextures even though ShouldStripUnusedDefaultTextures returns false.");
			bWarnedOnce = true;
		}
	...
	TArray<TObjectPtr<UObject>> UsedTextures;
	const bool bValidUsedTextures = TryGetUsedTexturesInCookCache(UsedTextures);

	if (!bValidUsedTextures)
	{
		if (!bInKeepProjectPrivateTextures)
		{
			// Textures will be kept in Parent material
			ReferencedDefaultTextures.Empty();
		}

		return;
	}

	for (int32 Idx = 0; Idx < ReferencedDefaultTextures.Num(); ++Idx)
	{
		const TObjectPtr<UObject> CurrentTexture = ReferencedDefaultTextures[Idx];
		...
		// Check for project private textures (PKC_NotExternallyReferenceable)
		const UPackage* TexturePackage = CurrentTexture->GetPackage();
		const UPackage* OwnPackage = GetPackage();

		if (!TexturePackage || !OwnPackage)
		{
			continue;
		}

		const bool bUsedTexture = UsedTextures.Contains(CurrentTexture);
		bool bRemoveTexture = !bUsedTexture;
		const bool bIsPackagePrivate = TexturePackage->GetAssetAccessSpecifier() == EAssetAccessSpecifier::Private;

		if (!bInKeepProjectPrivateTextures)
		{
			// We are in a Material Instance, check for private package and mount points
			if (bIsPackagePrivate)
			{
				const FName OwnMountPointName = FPackageName::GetPackageMountPoint(FNameBuilder(OwnPackage->GetFName()).ToView());
				const FName TextureMountPointName = FPackageName::GetPackageMountPoint(FNameBuilder(TexturePackage->GetFName()).ToView());

				if (!OwnMountPointName.IsNone() && !TextureMountPointName.IsNone())
				{
					if (OwnMountPointName != TextureMountPointName)
					{
						// Private package, different mount points -> remove texture
						bRemoveTexture = true;
					}
				}
				else
				{
					// Private package and None mount points -> remove texture
					bRemoveTexture = true;
				}
			}
		}
		...
		{
			// We are in a Material, we only remove if the package is not private
			bRemoveTexture &= !bIsPackagePrivate;
		}

		if (bRemoveTexture)
		{
			ReferencedDefaultTextures[Idx] = nullptr;
		}
	}
}
#endif // WITH_EDITOR
...
{
#if WITH_EDITOR
	UE::TUniqueLock Lock(TextureStreamingDataMutex);
...
float FTextureSamplingInfo::GetTextureDensity(const FMeshUVChannelInfo& UVChannelData) const
{
	float TextureDensity = 0.0f;
	for (int32 Index = 0; Index < MAX_TEXCOORDS; ++Index)
	{
		const float SamplingScale = ChannelMinSamplingScale[Index];
		TextureDensity = FMath::Max<float>(UVChannelData.LocalUVDensities[Index] / SamplingScale, TextureDensity);
	}
	return FMath::IsNearlyZero(TextureDensity) ? 0.0f : TextureDensity;
}
...
FInt32Range UMaterialInterface::BuildQualityLevelRange(TOptional<EMaterialQualityLevel::Type>  InQualityLevel)
{
	FInt32Range QualityLevelRange(0, EMaterialQualityLevel::Num - 1);

	if (InQualityLevel.IsSet())
	{
		EMaterialQualityLevel::Type QualityLevel = InQualityLevel.GetValue();
	...
	return QualityLevelRange;
}

FInt32Range UMaterialInterface::BuildShaderPlatformRange(TOptional<EShaderPlatform> InShaderPlatform)
{
#if WITH_EDITOR
	FInt32Range ShaderPlatformRange(0, EShaderPlatform::SP_NumPlatforms - 1);

	if (InShaderPlatform.IsSet())
	{
		EShaderPlatform ShaderPlatform = InShaderPlatform.GetValue();
...
namespace UE::Private
{
	...
	static FAutoConsoleVariableRef CVarEnableTexturesSamplingStreamingCache(
		...
		TEXT("Enables caching of material texture sampling. Used by FSimpleStreamableAssetManager's primary gather path (FStaticMeshSceneProxy / Nanite::FSceneProxy GetStreamableRenderAssetInfo) and by texture streaming in a cooked build. The cache is automatically enabled when FSimpleStreamableAssetManager is enabled.\n")
		TEXT("Read-only - must be set before the first UMaterialInterface is constructed, because the per-material lock guarding the cache is only allocated when this is enabled."),
		ECVF_ReadOnly);
}

bool UMaterialInterface::IsTexturesSamplingCacheGloballyEnabled()
{
	/* During cooking we do not need a cache, and we cannot relay on GetMaxShaderPlatformChecked
	 * It should be possible to cache the data during the cooking, but we have to do it for TargetPlatform */
	if (IsRunningCookCommandlet())
	{
		return false;
	}
	return UE::Private::GEnableTexturesSamplingStreamingCache || FSimpleStreamableAssetManager::IsEnabled();
}

bool UMaterialInterface::CanCacheTexturesSamplingInfo() const
{
	return IsTexturesSamplingCacheGloballyEnabled();
}

TSharedPtr<FMaterialCachedTexturesSamplingInfo> UMaterialInterface::ComputeCachedTexturesSamplingInfo() const
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UMaterialInterface::ComputeCachedTexturesSamplingInfo);

	check(IsInGameThread() || IsInParallelGameThread());

	TSharedPtr<FMaterialCachedTexturesSamplingInfo> Cache = MakeShared<FMaterialCachedTexturesSamplingInfo>();

	const EShaderPlatform ShaderPlatform = GetMaxShaderPlatformChecked();
	FMaterialCachedTexturesSamplingInfo::FShaderPlatformMapping& ShaderPlatformMapping = Cache->ShaderPlatformMapping;

	// TextureToInfoIndex dedups FTextureSamplingInfo entries across quality levels. CalculateTexturesSamplingInfo
	// is quality-level-independent so the same UTexture* always yields the same Info; keying dedup on the
	// texture pointer avoids the O(n^2) TArray::AddUnique walk for materials with many textures.
	TMap<const UTexture*, int32> TextureToInfoIndex;

	for (EMaterialQualityLevel::Type QualityLevel = EMaterialQualityLevel::Low; QualityLevel < EMaterialQualityLevel::Num; QualityLevel = static_cast<EMaterialQualityLevel::Type>(QualityLevel + 1))
	{
		TArray<uint16>& Mapping = ShaderPlatformMapping.QualityMapping[QualityLevel];
		TArray<UTexture*> Textures;
		GetUsedTextures(Textures, QualityLevel, ShaderPlatform);

		for (UTexture* Texture : Textures)
		{
			/* We will only cache the sampling information for Textures that support streaming
			 * to avoid timing issue instead of relying on dynamic IsStreamable that uses registration status of asset
			 * we will rely on RenderResourceSupportsStreaming that is based on asset properties
			 */
			if (!Texture->RenderResourceSupportsStreaming())
			{
				continue;
			}

			int32 Index = INDEX_NONE;
			if (const int32* ExistingIndex = TextureToInfoIndex.Find(Texture))
			{
				Index = *ExistingIndex;
			}
			else
			{
				// If the texture is referenced but not sampled it will not have an entry
				FTextureSamplingInfo Info{Texture};
				if (!CalculateTexturesSamplingInfo(Texture, Info))
				{
					continue;
				}
				Index = Cache->TextureSamplingInfos.Add(MoveTemp(Info));
				check(Index < TNumericLimits<uint16>::Max());
				TextureToInfoIndex.Add(Texture, Index);
			}

			Mapping.AddUnique(Index);
		}
	}
	return Cache;
}
...
void UMaterialInterface::CacheTexturesSamplingInfo()
{
	...
	// Compute outside the lock; holding the write lock across GetUsedTextures would stall every reader.
	TSharedPtr<FMaterialCachedTexturesSamplingInfo> NewCache = ComputeCachedTexturesSamplingInfo();

	UE::TWriteScopeLock<FCacheRWLock> WriteLock(CachedTexturesSamplingInfoLock);
	CachedTexturesSamplingInfo = MoveTemp(NewCache);
}

TSharedPtr<const FMaterialCachedTexturesSamplingInfo> UMaterialInterface::GetCachedTexturesSamplingInfo() const
{
	// Cache disabled via cvar or in a cook commandlet. Callers tolerate null -- typically by omitting the
	// material from their OutStreamableRenderAssets report, gated with a once-only ensure at the callsite.
	if (!CanCacheTexturesSamplingInfo())
	{
		return nullptr;
	}

	// Fast path: cache already populated. Copy the shared pointer out under the read lock and return.
	{
		UE::TReadScopeLock<FCacheRWLock> ReadLock(CachedTexturesSamplingInfoLock);
		if (CachedTexturesSamplingInfo)
		{
			return CachedTexturesSamplingInfo;
		}
	}

	// Slow path: populate on demand. Populating walks GetUsedTextures + CalculateTexturesSamplingInfo,
	// which are safe on GT/parallel-GT but not on a generic worker. A worker that arrives here before
	// PostLoad / UpdateCachedData has proactively populated the cache falls through to nullptr and
	// the caller omits this material from its streaming report.
	...
	// Double-check-locked publish. A parallel-GT caller may have populated while we computed; in that
	// case keep the existing cache and discard the one we just built.
	UE::TWriteScopeLock<FCacheRWLock> WriteLock(CachedTexturesSamplingInfoLock);
	if (!CachedTexturesSamplingInfo)
	{
		CachedTexturesSamplingInfo = MoveTemp(NewCache);
...
bool UMaterialInterface::CalculateTexturesSamplingInfo(UTexture* Texture, FTextureSamplingInfo& OutTextureSamplingInfo) const
{
	OutTextureSamplingInfo = FTextureSamplingInfo(Texture);
	int32 LowerIndex = INDEX_NONE;
	...
	if (FindTextureStreamingDataIndexRange(Texture->GetFName(), LowerIndex, UpperIndex))
	{
		for (int32 Index = LowerIndex; Index <= UpperIndex; ++Index)
		{
			...
			// Reject non-finite or non-positive sampling scales at source. NaN propagating into
			// ChannelMinSamplingScale would make FTextureSamplingInfo::operator==(default) report
			// distinct for every copy (NaN != NaN bitwise). A zero or near-zero scale would survive
			// IsFinite but produce divide-by-zero (LocalUVDensities[i] / 0.0f -> +Inf) inside
			// FTextureSamplingInfo::GetTextureDensity, tainting Info.TexelFactor with Inf and
			// failing the checkSlow in FMaterialCachedTexturesSamplingInfo::GetStreamingRenderAssetPrimitiveInfo.
			// Predicate matches FMaterialTextureInfo::IsValid()'s SamplingScale > UE_SMALL_NUMBER.
			if (!FMath::IsFinite(Info.SamplingScale) || Info.SamplingScale <= UE_SMALL_NUMBER)
			{
				continue;
			}

			OutTextureSamplingInfo.ChannelMinSamplingScale[Info.UVChannelIndex] = FMath::Min<float>(Info.SamplingScale, OutTextureSamplingInfo.ChannelMinSamplingScale[Info.UVChannelIndex]);
			bIsSampled = true;
		}
	}
	return bIsSampled;
...
FString UMaterialInterface::GetUsageName(EMaterialUsage Usage)
{
	FString UsageName;
	switch (Usage)
	{
	case MATUSAGE_SkeletalMesh: UsageName = TEXT("SkeletalMesh"); break;
	case MATUSAGE_ParticleSprites: UsageName = TEXT("ParticleSprites"); break;
	case MATUSAGE_BeamTrails: UsageName = TEXT("BeamTrails"); break;
	case MATUSAGE_MeshParticles: UsageName = TEXT("MeshParticles"); break;
	case MATUSAGE_NiagaraSprites: UsageName = TEXT("NiagaraSprites"); break;
	case MATUSAGE_NiagaraRibbons: UsageName = TEXT("NiagaraRibbons"); break;
	case MATUSAGE_NiagaraMeshParticles: UsageName = TEXT("NiagaraMeshParticles"); break;
	case MATUSAGE_StaticLighting: UsageName = TEXT("StaticLighting"); break;
	case MATUSAGE_MorphTargets: UsageName = TEXT("MorphTargets"); break;
	case MATUSAGE_SplineMesh: UsageName = TEXT("SplineMeshes"); break;
	case MATUSAGE_InstancedStaticMeshes: UsageName = TEXT("InstancedStaticMeshes"); break;
	case MATUSAGE_GeometryCollections: UsageName = TEXT("GeometryCollections"); break;
	case MATUSAGE_Clothing: UsageName = TEXT("Clothing"); break;
	case MATUSAGE_GeometryCache: UsageName = TEXT("GeometryCache"); break;
	case MATUSAGE_Water: UsageName = TEXT("Water"); break;
	case MATUSAGE_HairStrands: UsageName = TEXT("HairStrands"); break;
	case MATUSAGE_LidarPointCloud: UsageName = TEXT("LidarPointCloud"); break;
	case MATUSAGE_VirtualHeightfieldMesh: UsageName = TEXT("VirtualHeightfieldMesh"); break;
	case MATUSAGE_Nanite: UsageName = TEXT("Nanite"); break;
	case MATUSAGE_Voxels: UsageName = TEXT("Voxels"); break;
	case MATUSAGE_Curves: UsageName = TEXT("Curves"); break;
	case MATUSAGE_VolumetricCloud: UsageName = TEXT("VolumetricCloud"); break;
	case MATUSAGE_HeterogeneousVolumes: UsageName = TEXT("HeterogeneousVolumes"); break;
	case MATUSAGE_StaticMesh: UsageName = TEXT("StaticMesh"); break;
	case MATUSAGE_EditorCompositing: UsageName = TEXT("EditorCompositing"); break;
	case MATUSAGE_NeuralNetworks: UsageName = TEXT("NeuralNetworks"); break;
	case MATUSAGE_MeshDeformer: UsageName = TEXT("MeshDeformer"); break;
	case MATUSAGE_InstancedSkinnedMesh: UsageName = TEXT("InstancedSkinnedMesh"); break;
	default: UE_LOGF(LogMaterial, Fatal, "Unknown material usage: %u", (int32)Usage);
	};
	return UsageName;
}

bool UMaterialInterface::CheckMaterialUsage(EMaterialUsage Usage)
{
	check(IsInGameThread());
	return SetMaterialUsage(Usage);
}

bool UMaterialInterface::CheckMaterialUsage_Concurrent(EMaterialUsage Usage) const
{
	...
	if (NeedsSetMaterialUsage_Concurrent(bUsageSetSuccessfully, Usage))
	{
		if (IsInGameThread())
		{
			bUsageSetSuccessfully = const_cast<UMaterialInterface*>(this)->CheckMaterialUsage(Usage);
		}
		else
		{
			...
			if ((PrevCacheMissMask & UsageFlagBit) == 0u)
			{
				struct FCallSMU
				{
					...
					FCallSMU(UMaterialInterface* InMaterial, EMaterialUsage InUsage)
						: Material(InMaterial)
						, Usage(InUsage)
					{
					}
				...
				UE_LOGF(LogMaterial, Log, "Had to pass SMU back to game thread. Please fix material usage flag %ls on %ls", *GetUsageName(Usage), *GetPathNameSafe(this));

				TSharedRef<FCallSMU, ESPMode::ThreadSafe> CallSMU = MakeShareable(new FCallSMU(const_cast<UMaterialInterface*>(this), Usage));
...
bool UMaterialInterface::NeedsSetMaterialUsage_Concurrent(bool& bOutHasUsage, EMaterialUsage Usage) const
{
	bOutHasUsage = false;

	// Material usage is only relevant for materials that can be applied onto a mesh / use with different vertex factories.
	UMaterial const* BaseMaterial = GetMaterial_Concurrent();
	...
	if (MaterialDomain != MD_Surface && MaterialDomain != MD_DeferredDecal && MaterialDomain != MD_Volume && MaterialDomain != MD_UI)
	{
		return false;
	}

	// Check that the material has been flagged for use with the given usage flag.
	if (!GetUsageByFlag(Usage) && !BaseMaterial->bUsedAsSpecialEngineMaterial)
	{
		const uint32 UsageFlagBit = (1u << Usage);
