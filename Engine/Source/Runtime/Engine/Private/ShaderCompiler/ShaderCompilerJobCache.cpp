...
TRACE_DECLARE_ATOMIC_MEMORY_COUNTER(Shaders_JobCacheDDCBytesReceived, TEXT("Shaders/JobCache/DDCBytesReceived"));
...
struct FShaderJobData
{
	...
	void ResetJobInFlight()
	{
		if (JobInFlight)
		{
#if WITH_EDITOR
			if (JobInFlight->RequestOwner.IsValid())
			{
				JobInFlight->RequestOwner->KeepAlive();
			}
#endif
			JobInFlight = nullptr;
		}
...
public:
	...
	/** Similar to FRefCountedObject AddRef, but not atomic */
	int32 AddRef()
	{
		++NumReferences;
	...
	/** Similar to FRefCountedObject Release, but not atomic */
	int32 Release()
	{
...
public:
	...
	void TransferJobsToLocalWorker(const TArray<FShaderCommonCompileJobPtr>& InJobs);
	void OnSubmitJobComplete();
	...
	void ProcessFinishedJob(FShaderCommonCompileJob* FinishedJob, EShaderCompileJobStatus Status, EShaderCompileJobCompletionMethod Method = EShaderCompileJobCompletionMethod::Unset);
...
private:
	...
	std::atomic<uint32> NumPendingSubmitJobs{ 0 };
	FEventRef AllSubmitJobsCompleteEvent;
...
void FShaderCompileJobCollection::TransferJobsToLocalWorker(const TArray<FShaderCommonCompileJobPtr>& InJobs)
{
	JobsCache->TransferJobsToLocalWorker(InJobs);
}
				...
				if (Job.Id == InId)
				{
					if (Job.JobCacheRef.Block)
					{
						...
						if (DuplicateJob)
						{
							...
							// Unset the execution method on the previously "duplicate" job; this will now be set by whatever executor picks up the job.
							DuplicateJob->JobStatusPtr->SetCompletionMethod(EShaderCompileJobCompletionMethod::Unset);
...
void FShaderJobCache::TransferJobsToLocalWorker(const TArray<FShaderCommonCompileJobPtr>& InJobs)
{
	FWriteScopeLock Locker(JobLock);

	for (const FShaderCommonCompileJobPtr& Job : InJobs)
	{
		// Mark job as begin transferred to avoid duplicate registration as an assigned job
		Job->bTransferred = true;

		// If job was already put into a priority bucket, remove it from the old priority
		if (Job->PendingPriority != EShaderCompileJobPriority::None)
		{
			check(Job->PendingPriority == Job->Priority);
			UnlinkJobWithPriority(*Job);
		...
		// Put job into new priority bucket
		ensure(Job->bInputHashSet);
		Job->Priority = EShaderCompileJobPriority::ForceLocal;
		LinkJobWithPriority(*Job);

		// Reset status and worker type
		Job->UpdateStatus(EShaderCompileJobStatus::Queued);
		Job->CurrentWorker = EShaderCompilerWorkerType::None;
	}
}

void FShaderJobCache::OnSubmitJobComplete()
{
	if (NumPendingSubmitJobs.fetch_sub(1, std::memory_order_acq_rel) == 1)
	{
		AllSubmitJobsCompleteEvent->Trigger();
	}
}
...
void FShaderJobCache::SubmitJob(FShaderCommonCompileJob* Job)
{
	...
	if (!ShaderCompiler::IsJobCacheDebugValidateEnabled())
	{
		...
		if (!Job->PrevLink)
		{
			UE_LOGF(LogShaderCompilers, Display, "Cancelled job 0x%p with pending SubmitJob call.", Job);
		...
		if (LoadContext.HasData() && !Job->bBypassCache)
		{
			Unlink(*Job);		// from PendingSubmitJobTaskJobs
			...
			UE_LOGF(LogShaderCompilers, UE_SHADERCACHE_LOG_LEVEL, "There is already a cached job with the ihash %ls, processing the new one immediately.", *LexToString(InputHash));

			Job->SerializeOutput(LoadContext);

			// finish the job instantly
			ProcessFinishedJob(Job, EShaderCompileJobStatus::Completed, EShaderCompileJobCompletionMethod::CacheHit);
		}
		else
		{
			FShaderJobData& JobData = GetShaderJobData(Job->JobCacheRef);
			// if there's an in flight job for this cache ref already another job with the same input hash is already being worked on
			bool bDuplicateJob = JobData.JobInFlight.IsValid(); 			
			// update the completion method for a duplicate job immediately (so the status reflects that it's a duplicate while it's in queued state)
			// keep it unset if it's a primary job that will be executed; the execution method will be set when it's picked up by a compilation thread
			Job->UpdateStatus(EShaderCompileJobStatus::Queued, bDuplicateJob ? EShaderCompileJobCompletionMethod::Duplicate : EShaderCompileJobCompletionMethod::Unset);
			if (bDuplicateJob)
			{
				UE_LOGF(LogShaderCompilers, UE_SHADERCACHE_LOG_LEVEL, "There is an outstanding job with the ihash %ls, not submitting another one (adding to wait list).", *LexToString(InputHash));
	...
	if (bNewJob)
	{
		...
		if (bJobCacheLocked == false)
		{
			...
			if (!Job->PrevLink)
			{
				UE_LOGF(LogShaderCompilers, Log, "Cancelled job 0x%p with pending SubmitJob call.", Job);
				Job->UpdateStatus(EShaderCompileJobStatus::Cancelled);
				JobLock.WriteUnlock();
				return;
			}
...
void FShaderJobCache::SubmitJobs(const TArray<FShaderCommonCompileJobPtr>& InJobs)
{
	if (InJobs.Num() > 0)
	{
		...
		NumPendingSubmitJobs.fetch_add(InJobs.Num(), std::memory_order_relaxed);
		for (const FShaderCommonCompileJobPtr& Job : InJobs)
		{
			...
			UE::Tasks::Launch(UE_SOURCE_LOCATION, [Job, this]()
				{
					...
					{
						// we do not need to unlink the job here as normal shutdown flow of anything referencing pending shader jobs will call the cancellation path;
						// we just need to ensure that any tasks that still execute after this cancellation occurs decrement the atomic counter and possibly signal the
						// event that we wait on in the destructor (to ensure none of these tasks are outstanding before we start unloading modules)
						OnSubmitJobComplete();
						return;
					}
...
void FShaderJobCache::ProcessFinishedJob(FShaderCommonCompileJob* FinishedJob, EShaderCompileJobStatus Status, EShaderCompileJobCompletionMethod CompletionMethod)
{
	FinishedJob->UpdateStatus(Status, CompletionMethod);
...
void FShaderJobCache::AddToCacheAndProcessPending(FShaderCommonCompileJob* FinishedJob)
{
...

	{
		...
		if (FinishedJob->bSucceeded && !FinishedJob->bBypassCache)
		{
		...
		JobData.ResetJobInFlight();
		FinishedJob->JobCacheRef.Clear();
	}

	if (FinishedDuplicateJobs.Num())
	{
		UE_LOGF(LogShaderCompilers, UE_SHADERCACHE_LOG_LEVEL, "Processed %d outstanding jobs with the same ihash %ls.", FinishedDuplicateJobs.Num(), *LexToString(InputHash));
...
int32 FShaderJobCache::GetPendingJobs(EShaderCompilerWorkerType InWorkerType, EShaderCompileJobPriority InPriority, int32 MinNumJobs, int32 MaxNumJobs, TArray<FShaderCommonCompileJobPtr>& OutJobs)
{
	...
	for (int32 i = 0; i < NumJobs; ++i)
	{
		...
		// Don't register jobs that have already been registered as assigned to a remote worker and then be transferred to a local worker
		if (!Job.bTransferred)
		{
		...
		Job.UpdateStatus(EShaderCompileJobStatus::Pending, (InWorkerType == EShaderCompilerWorkerType::Distributed) ? EShaderCompileJobCompletionMethod::DistributedExecution : EShaderCompileJobCompletionMethod::LocalExecution);
		OutJobs.Add(&Job);
				...
				[this, JobDataPtr = &JobData, DerivedDataPriority, StoredOutputPtr, bCompletedSynchronouslyPtr](UE::DerivedData::FCacheGetResponse&& Response)
				{
					...
					if (Response.Status == UE::DerivedData::EStatus::Ok)
					{
						...
						if (bIsAsync)
						{
							...
							{
								UE_LOGF(LogShaderCompilers, Display, "Cancelled job 0x%p (data 0x%p) with pending DDC hit.", JobDataPtr->JobInFlight.GetReference(), JobDataPtr);
								if (JobDataPtr->JobInFlight)
								{
									JobDataPtr->JobInFlight->UpdateStatus(EShaderCompileJobStatus::Cancelled);
									JobDataPtr->ResetJobInFlight();
								}
								JobLock.WriteUnlock();
								return;
							}
						...
						if (bIsAsync)
						{
							...
							UE_LOGF(LogShaderCompilers, UE_SHADERCACHE_LOG_LEVEL, "Found an async DDC result for job with ihash %ls.", *LexToString(Job->InputHash));
							...
							for (FShaderCommonCompileJob* FinishedJob : FinishedJobs)
							{
								...
								EShaderCompileJobCompletionMethod Method = FinishedJob->JobStatusPtr->GetCompletionMethod();
								// We only update execution method to "DdcHit" for the first job in the chain (of which all subsequent entries are dupes) for tracking purposes
								// Duplicate jobs will have previously had their execution method set to "Duplicate" when queued, so this maintains that
								if (Method == EShaderCompileJobCompletionMethod::Unset)
								{
									Method = EShaderCompileJobCompletionMethod::DdcHit;
								}
								ProcessFinishedJob(FinishedJob, EShaderCompileJobStatus::Completed, Method);
							}

							if (FinishedJobs.Num() > 1)
							{
								UE_LOGF(LogShaderCompilers, UE_SHADERCACHE_LOG_LEVEL, "Processed %d outstanding jobs with the same ihash %ls.", FinishedJobs.Num() - 1, *LexToString(Job->InputHash));
							}
						}
						else
						{
					...
					else
					{
						...
						if (bIsAsync)
						{
							...
							if (!Job->PrevLink)
							{
								UE_LOGF(LogShaderCompilers, Display, "Cancelled job 0x%p (data 0x%p) with pending DDC miss.", Job, JobDataPtr);
								Job->UpdateStatus(EShaderCompileJobStatus::Cancelled);
								JobDataPtr->ResetJobInFlight();
								return;
							}
...
FShaderJobCache::FShaderJobCache(FCriticalSection& InCompileQueueSection)
	...
	, AllSubmitJobsCompleteEvent(EEventMode::ManualReset)
{
...
{
	AllSubmitJobsCompleteEvent->Reset();
	if (NumPendingSubmitJobs.load(std::memory_order_acquire) > 0)
	{
		AllSubmitJobsCompleteEvent->Wait();
	}
	...
	if (JobData.HasOutput() && ShaderCompiler::IsJobCacheDebugValidateEnabled())
	{
		if (OutputHash != JobData.OutputHash)
		{
			...
			UE_LOGF(
				LogShaderCompilers,
				Warning,
				"Job cache validation found output mismatch!\n" "Cached job: %ls\n" "Original job: %ls\n",
			...
			if (ShaderCompiler::IsDumpShaderDebugInfoAlwaysEnabled())
			{
				...
				if (!bOnce)
				{
					UE_LOGF(
						LogShaderCompilers,
						Warning,
						"Enable r.DumpShaderDebugInfo=1 to get debug info paths for the mismatching jobs instead of group names (to allow diffing debug artifacts)");
					bOnce = true;
				}
			}
		}
	...
	else
	{
		...
		if (MemoryBudgetBytes)
		{
			if (CurrentlyAllocatedMemory > MemoryBudgetBytes)
			{
				...
				UE_LOGF(LogShaderCompilers, Display, "Memory overflow, reduced from %.1lf to %.1lf MB.", (double)MemoryBefore / (1024 * 1024), (double)CurrentlyAllocatedMemory / (1024 * 1024));
			}
		}
	}
