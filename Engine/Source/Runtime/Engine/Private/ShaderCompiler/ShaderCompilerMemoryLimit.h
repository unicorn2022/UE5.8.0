...
public:
	...
	/** Caches build of system process ids. Saves overhead if calling TryGetMemoryUsage on separate root process Ids */
	void CacheSystemProcessIds();

	/** Clears the cache of system process Ids. */
	void ClearSystemProcessIdsCache();
...
private:
	...
	/* State of cached system process Ids */
	bool bCachedSystemProcessIds = false;
	...
	/* Populate the system process Id list */
	void RebuildSystemProcessIdList();
};
