...
void FShaderCompileUtilities::ExecuteShaderCompileJob(FShaderCommonCompileJob& Job)
{
	...
	check(Job.GetStatus() != EShaderCompileJobStatus::Completed);
	...
	if (SingleJob)
	{
		...
		CompileShader(ShaderFormats, *SingleJob);
	}
	else
	{
		...
		for (int32 Index = 1; Index < PipelineJob->StageJobs.Num(); ++Index)
		{
			...
			if (!SingleStage)
			{
				UE_LOGF(LogShaderCompilers, Fatal, "Can't nest Shader Pipelines inside Shader Pipeline '%ls'!", PipelineJob->Key.ShaderPipeline->GetName());
			}
			else if (Platform != SingleStage->Input.Target.Platform)
			{
				UE_LOGF(LogShaderCompilers, Fatal, "Mismatched Target Platform %ls while compiling Shader Pipeline '%ls'.", *Format.GetPlainNameString(), PipelineJob->Key.ShaderPipeline->GetName());
			}
		}

		CompileShaderPipeline(ShaderFormats, PipelineJob);
	}

	Job.UpdateStatus(EShaderCompileJobStatus::Completed, EShaderCompileJobCompletionMethod::LocalExecution);
}
