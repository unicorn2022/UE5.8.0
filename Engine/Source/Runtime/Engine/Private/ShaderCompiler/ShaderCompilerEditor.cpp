	...
	FShaderCompileJob* NewJob)
{
	...
	UE_LOGF(LogShaders, Verbose, "	%ls (permutation %d)", ShaderType->GetName(), Key.PermutationId);
	...
	static FString GlobalName("Global");
...
void FGlobalShaderTypeCompiler::BeginCompileShader(const FGlobalShaderType* ShaderType, int32 PermutationId, EShaderPlatform Platform, EShaderPermutationFlags PermutationFlags, TArray<FShaderCommonCompileJobPtr>& NewJobs)
{
	...
	FShaderCompileJob* NewJob = GShaderCompilingManager->PrepareShaderCompileJob(GlobalShaderMapId, FShaderCompileJobKey(ShaderType, Platform, nullptr, PermutationId), EShaderCompileJobPriority::High);
	if (NewJob)
	{
...
void FGlobalShaderTypeCompiler::BeginCompileShaderPipeline(EShaderPlatform Platform, EShaderPermutationFlags PermutationFlags, const FShaderPipelineType* ShaderPipeline, TArray<FShaderCommonCompileJobPtr>& NewJobs)
{
	...
	FShaderPipelineCompileJob* NewPipelineJob = GShaderCompilingManager->PreparePipelineCompileJob(GlobalShaderMapId, FShaderPipelineCompileJobKey(ShaderPipeline, Platform, nullptr, kUniqueShaderPermutationId), EShaderCompileJobPriority::High);
	if (NewPipelineJob)
	{
...
FShader* FGlobalShaderTypeCompiler::FinishCompileShader(const FGlobalShaderType* ShaderType, const FShaderCompileJob& CurrentJob, const FShaderPipelineType* ShaderPipelineType)
{
	...
	if (CurrentJob.bSucceeded)
	{
		...
		uint64 GlobalShaderMapHash;
		{
			FStringView GlobalShaderString = TEXTVIEW("GlobalShaderMap");
			GlobalShaderMapHash = FXxHash64::HashBuffer(GlobalShaderString.GetData(), GlobalShaderString.NumBytes()).Hash;
		}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(GetGlobalShaderMapKey);
...
{
	UE_LOGF(LogShaders, Display, "Recompiling global shaders.");
	...
	const FString& OutputDirectory)
{
	...
	if (TargetPlatform == nullptr)
	{
		UE_LOGF(LogShaders, Display, "Failed to find target platform module for %ls", *Args.PlatformName);
		return;
	}

	const double StartTime = FPlatformTime::Seconds();

	UE_LOGF(LogShaders, Display, "");
	UE_LOGF(LogShaders, Display, "********************************");
	UE_LOGF(LogShaders, Display, "Received compile shader request %ls.", ODSCCmdEnumToString(Args.CommandType));
	...
	UE_LOGF(LogShaders, Verbose, "Loading %d materials...", Args.MaterialsToLoad.Num());
	...
	{
		UE_LOGF(LogShaders, Verbose, "   --> %ls", *Args.MaterialsToLoad[Index]);
		MaterialsToCompile.Add(LoadObject<UMaterialInterface>(nullptr, *Args.MaterialsToLoad[Index]));
	}

	UE_LOGF(LogShaders, Verbose, "  Done!");
	...
	if (Args.ShadersToRecompile.Num() && (Args.MeshMaterialMaps != nullptr))
	{
		...
		{
			if (!GShaderCompilingManager->AllowForIncompleteShaderMaps())
			{
				UE_LOGF(LogShaders, Warning, "AllowForIncompleteShaderMaps is not set to true, ODSC shaders will most likely not be recompiled");
			}
			...
			if (Args.LoadedMaterialsToRecompile)
			{
				for (auto Iter : CompiledShaderMaps)
				{
					...
					{
						UE_LOGF(LogShaders, Warning, "Failed to find Material %ls. Reloading on the client will be skipped", *Iter.Key);
					}
				}
			}
		}
		...
		for (int32 FormatIndex = 0; FormatIndex < DesiredShaderFormats.Num(); FormatIndex++)
		{
			...
			if (ShaderPlatform == Args.ShaderPlatform || Args.ShaderPlatform == SP_NumPlatforms)
			{
					...
					Args.ShaderTypesToLoad.Len() > 0)
				{
					...
					{
						UE_LOGF(LogShaders, Display, "\t%ls...", ShaderType->GetName());
					}
						 ...
						 Args.CommandType == ODSCRecompileCommand::Changed)
				{
					...
					UE_LOGF(LogShaders, Display, "\tFound %d outdated shader types.", OutdatedShaderTypes.Num() + OutdatedShaderPipelineTypes.Num());
	...
	UE_LOGF(LogShaders, Display, "");
	UE_LOGF(LogShaders, Display, "Compiled %u shaders in %.2f seconds.", GShaderCompilerStats->GetTotalShadersCompiled() - StartTotalShadersCompiled, FPlatformTime::Seconds() - StartTime);
...
void ProcessCompiledGlobalShaders(const TArray<FShaderCommonCompileJobPtr>& CompilationResults)
{
	...
	UE_LOGF(LogShaders, Verbose, "Compiled %u global shaders", CompilationResults.Num());
	...
	for (const FString& WarningString : ShaderDiagInfo.UniqueWarnings)
	{
		...
		{
			UE_LOGFMT_NSLOC(LogShaders, Display, "Shaders", "GlobalShaderCompileWarning", "{WarningMessage}", ("WarningMessage", WarningString));
		}
	}
