...
void FShaderCompilerStats::WriteStatSummary()
{
	...
	{
		UE_LOGF(LogShaderCompilers, Display, "=== FShaderJobCache stats%ls ===", AggregatedSuffix);
		UE_LOGF(LogShaderCompilers, Display, "Total job queries %ls, among them cache hits %ls (%.2f%%), DDC hits %ls (%.2f%%), Duplicates %ls (%.2f%%)",
		...
		UE_LOGF(LogShaderCompilers, Display, "Tracking %ls distinct input hashes that result in %ls distinct outputs (%.2f%%)",
		...
		{
			UE_LOGF(LogShaderCompilers, Display, "RAM used: %ls of %ls budget. Usage: %.2f%%",
		...
		{
			UE_LOGF(LogShaderCompilers, Display, "RAM used: %ls, no memory limit set", *FText::AsMemory(Counters.CacheMemUsed, &SizeFormattingOptions, nullptr, EMemoryUnitStandard::IEC).ToString());
		}
	}

	const double TotalTimeAtLeastOneJobWasInFlight = GetTimeShaderCompilationWasActive();

	UE_LOGF(LogShaderCompilers, Display, "=== Shader Compilation stats%ls ===", AggregatedSuffix);
	UE_LOGF(LogShaderCompilers, Display, "Shaders Compiled: %ls", *FormatNumber(TotalCompiled));

	FScopeLock Lock(&CompileStatsLock);	// make a local copy for all the stats?
	UE_LOGF(LogShaderCompilers, Display, "Jobs assigned %ls, completed %ls (%ls)", 
	...
	if (Counters.LargestViolationMemorySizeInBytes > 0)
	{
		UE_LOGF(LogShaderCompilers, Display,
			"Shader compilation violated current memory limit by %d MiB (%lld bytes). Total worker count %d.",
			static_cast<int32>(Counters.LargestViolationMemorySizeInBytes / 1024 / 1024),
			Counters.LargestViolationMemorySizeInBytes,
			Counters.LargestViolationWorkerCount);
	}

	if (Counters.TimesLocalWorkersWereIdle > 0.0)
	{
		UE_LOGF(LogShaderCompilers, Display, "Average time worker was idle: %.2f s", Counters.AccumulatedLocalWorkerIdleTime / Counters.TimesLocalWorkersWereIdle);
	}

	if (Counters.JobsAssigned > 0)
	{
		UE_LOGF(LogShaderCompilers, Display, "Time job spent in pending queue: average %.2f s, longest %.2f s", Counters.AccumulatedPendingTime / (double)Counters.JobsAssigned, Counters.MaxPendingTime);
	}

	if (Counters.JobsCompleted > 0)
	{
		UE_LOGF(LogShaderCompilers, Display, "Job execution time: average %.2f s, max %.2f s", Counters.AccumulatedJobExecutionTime / (double)Counters.JobsCompleted, Counters.MaxJobExecutionTime);
		UE_LOGF(LogShaderCompilers, Display, "Job life time (pending + execution): average %.2f s, max %.2f", Counters.AccumulatedJobLifeTime / (double)Counters.JobsCompleted, Counters.MaxJobLifeTime);
	}
	...
	if (Counters.NumAccumulatedShaderCodes > 0)
	{
		...
		UE_LOGF(LogShaderCompilers, Display, "Shader code size: total %ls, numShaders %ls, average %ls, min %ls, max %ls", *TotalCodeSizeStr, *NumShadersStr, *AvgCodeSizeStr, *MinCodeSizeStr, *MaxCodeSizeStr);
	}

	UE_LOGF(LogShaderCompilers, Display, "Time at least one job was in flight (either pending or executed): %.2f s", TotalTimeAtLeastOneJobWasInFlight);
	...
	if (Counters.LocalJobBatchesSeen > 0 && Counters.DistributedJobBatchesSeen > 0)
	{
		...
		UE_LOGF(LogShaderCompilers, Display, "Jobs were issued in %ls batches (%ls local, %ls distributed), average %.2f jobs/batch (%.2f jobs/local batch. %.2f jobs/distributed batch)",
	...
	{
		UE_LOGF(LogShaderCompilers, Display, "Jobs were issued in %ls batches (only local compilation was used), average %.2f jobs/batch", 
	...
	{
		UE_LOGF(LogShaderCompilers, Display, "Jobs were issued in %ls batches (only distributed compilation was used), average %.2f jobs/batch",
	...
	{
		UE_LOGF(LogShaderCompilers, Display, "Average processing rate: %.2f jobs/sec", (double)Counters.JobsCompleted / TotalTimeAtLeastOneJobWasInFlight);
	}
	...
	if (ShaderTimings.Num())
	{
		...
		UE_LOGF(LogShaderCompilers, Display, "Total thread time: %ls s", *FormatNumber(TotalThreadTimeForAllShaders));
		UE_LOGF(LogShaderCompilers, Display, "Total thread preprocess time: %ls s", *FormatNumber(TotalThreadPreprocessTimeForAllShaders));
		UE_LOGF(LogShaderCompilers, Display, "Percentage time preprocessing: %.2f%%", TotalThreadTimeForAllShaders > 0.0 ? (TotalThreadPreprocessTimeForAllShaders / TotalThreadTimeForAllShaders) * 100.0 : 0.0);

		if (Counters.MaxRemoteAgents > 0)
		{
			UE_LOGF(LogShaderCompilers, Display, "Highest number of remote agents active in parallel: %u (%u active cores peak)", Counters.MaxRemoteAgents, Counters.MaxActiveAgentCores);
		}
		...
		if (TotalTimeAtLeastOneJobWasInFlight > 0.0)
		{
			...
			{
				UE_LOGF(LogShaderCompilers, Display, "Effective parallelization: %.2f (times faster than compiling all shaders on one thread). Compare with number of workers: %d - %f", EffectiveParallelization, GShaderCompilingManager->GetNumLocalWorkers(), EffectiveParallelization/(double)GShaderCompilingManager->GetNumLocalWorkers());
			}
			else
			{
				UE_LOGF(LogShaderCompilers, Display, "Effective parallelization: %.2f (times faster than compiling all shaders on one thread). Distributed compilation was used.", EffectiveParallelization);
			}
		}
		...
		UE_LOGF(LogShaderCompilers, Display, "Top %d most expensive shader types by average time:", MaxShadersToPrint);
		...
		for (TMap<FString, FShaderTimings>::TConstIterator Iter(ShaderTimings); Iter; ++Iter)
		{
			...
			UE_LOGF(LogShaderCompilers, Display, "%60ls (compiled %4d times, average %4.2f sec, max %4.2f sec, min %4.2f sec)", *Iter.Key(), Timings.NumCompiled, Timings.AverageCompileTime, Timings.MaxCompileTime, Timings.MinCompileTime);
		...
		UE_LOGF(LogShaderCompilers, Display, "Top %d shader types by total compile time:", MaxShadersToPrint);
		...
		for (TMap<FString, FShaderTimings>::TConstIterator Iter(ShaderTimings); Iter; ++Iter)
		{
			...
			UE_LOGF(LogShaderCompilers, Display, "%60ls - %.2f%% of total time (compiled %4d times, average %4.2f sec, max %4.2f sec, min %4.2f sec)", 
...
void FShaderCompilerStats::GatherAnalytics(const FString& BaseName, TArray<FAnalyticsEventAttribute>& Attributes)
{
	...
	{
		const FString ChildName = "JobCache_";
...
TSharedPtr<FJsonObject> FShaderCompilerStats::ToJson()
{
	TSharedPtr<FJsonObject> RootObject(MakeShared<FJsonObject>());
	{
		...
		RootObject->SetObjectField(TEXT("Counters"), JsonObject);
	}
	{
		TSharedRef<FJsonObject> JsonObject = MakeShared<FJsonObject>();
		...
		RootObject->SetObjectField(TEXT("MaterialCounters"), JsonObject);
	}
	...
	RootObject->SetArrayField(TEXT("CompileStats"), CompileStatsArray);
	...
	RootObject->SetObjectField(TEXT("ShaderTimings"), ShaderTimingsObject);
	return RootObject;
}
...
void FShaderCompilerStats::RegisterMemoryViolation(const int64 InMemorySizeInBytes, const int32 InWorkerCount)
{
	FScopeLock Lock(&CompileStatsLock);

	if (InMemorySizeInBytes < Counters.LargestViolationMemorySizeInBytes)
		return;

	Counters.LargestViolationMemorySizeInBytes = InMemorySizeInBytes;
	Counters.LargestViolationWorkerCount = InWorkerCount;
}
...
void FShaderCompilerMaterialCounters::WriteStatSummary(const TCHAR* AggregatedSuffix)
{
	...
	UE_LOGF(LogShaderCompilers, Display, "=== Material stats%ls ===", AggregatedSuffix);
	UE_LOGF(LogShaderCompilers, Display, "Materials Cooked:        %d", NumMaterialsCooked);
	UE_LOGF(LogShaderCompilers, Display, "Materials Translated:    %d", MaterialTranslateCalls);
	UE_LOGF(LogShaderCompilers, Display, "Material Total Translate Time: %.2f s", MaterialTranslateTotalTimeSec);
	UE_LOGF(LogShaderCompilers, Display, "Material Translation Only: %.2f s (%d%%)", MaterialTranslateTranslationOnlyTimeSec, CalcTimePercentage(MaterialTranslateTranslationOnlyTimeSec));
	UE_LOGF(LogShaderCompilers, Display, "Material DDC Serialization Only: %.2f s (%d%%)", MaterialTranslateSerializationOnlyTimeSec, CalcTimePercentage(MaterialTranslateSerializationOnlyTimeSec));

	int HitsPercentage = MaterialTranslateCalls ? (int)roundf(float(MaterialCacheHits) / MaterialTranslateCalls * 100) : 0;
	UE_LOGF(LogShaderCompilers, Display, "Material Cache Hits: %d (%d%%)", MaterialCacheHits, HitsPercentage);
}
