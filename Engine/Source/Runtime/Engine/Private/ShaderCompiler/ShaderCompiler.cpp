...
bool GUseMaterialVSUniformBuffer = true;
static FAutoConsoleVariableRef CVarUseMaterialVSUniformBuffer(
	TEXT("r.ShaderCompiler.UseMaterialVSUniformBuffer"),
	GUseMaterialVSUniformBuffer,
	TEXT("If true, translate/compile vertex shaders with a dedicated uniform buffer"),
	ECVF_ReadOnly
);

bool UseMaterialVSUniformBuffer()
{
	return GUseMaterialVSUniformBuffer;
}
...
static FShaderCompilingManager::EDumpShaderDebugInfo GDumpShaderDebugInfo = FShaderCompilingManager::EDumpShaderDebugInfo::OnError;
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FShaderCompilingManager::FShaderCompilingManager);
	...
	if (GConfig->GetBool(TEXT("DevOptions.Shaders"), TEXT("bUseShaderCompilerFromLaunchDir"), bUseShaderCompilerFromLaunchDir, GEngineIni) && bUseShaderCompilerFromLaunchDir)
	{
		...
		{
			UE_LOGF(LogShaderCompilers, Warning, "Using bUseShaderCompilerFromLaunchDir but could not find shader compile worker in LaunchDir - '%ls'.", *FPaths::LaunchDir());
		}
	}
...

	{
		...
		UE_LOGF(LogShaderCompilers, Log, "Guid format shader working directory is %d characters bigger than the processId version (%ls).", ShaderBaseWorkingDirectory.Len() - LegacyShaderWorkingDirectory.Len(), *LegacyShaderWorkingDirectory );
	}

	if (!IFileManager::Get().DeleteDirectory(*ShaderBaseWorkingDirectory, false, true))
	{
		UE_LOGF(LogShaderCompilers, Fatal, "Could not delete the shader compiler working directory '%ls'.", *ShaderBaseWorkingDirectory);
	}
	else
	{
		UE_LOGF(LogShaderCompilers, Log, "Cleaned the shader compiler working directory '%ls'.", *ShaderBaseWorkingDirectory);
	}
	...
	if (GDebugDumpWorkerCrashLog)
	{
		...
		if (!WorkerCrashLogBaseDirectory.IsEmpty() && !IFileManager::Get().DirectoryExists(*WorkerCrashLogBaseDirectory))
		{
			if (!IFileManager::Get().MakeDirectory(*WorkerCrashLogBaseDirectory, true))
			{
				...
				UE_LOGF(LogShaderCompilers, Warning, "Failed to create directory for ShaderCompileWorker crash logs '%ls' (Error Code: %u)", *WorkerCrashLogBaseDirectory, ErrorCode);
			}
		}
	}
	...
	if (bAllowShaderRecompileOnSave)
	{
		if (IDirectoryWatcher* DirectoryWatcher = FModuleManager::LoadModuleChecked<FDirectoryWatcherModule>(TEXT("DirectoryWatcher")).Get())
		{
			...
			{
				UE_LOGF(LogShaderCompilers, Display, "Register directory watchers for shader files.");
				...
				for (const auto& It : ShaderSourceDirectoryMappings)
				{
					...
					DirectoryWatcher->RegisterDirectoryChangedCallback_Handle(
						...
						IDirectoryWatcher::FDirectoryChanged::CreateLambda([](const TArray<FFileChangeData>& InFileChangeDatas) {
							...
							for (const FFileChangeData& It : InFileChangeDatas)
							{
								...
								{
									UE_LOGF(LogShaderCompilers, Display, "Detected change on %ls", *It.Filename);
							...
							if (ChangedShaderFiles.Num())
							{
								...
								UE_LOGF(LogShaderCompilers, Display, "Ready for new shader file changes");
							}
						}),
					...
					{
						UE_LOGF(LogShaderCompilers, Display, "Watching %ls -> %ls", *It.Key, *DirectoryToWatch);
					}
					else
					{
						UE_LOGF(LogShaderCompilers, Error, "Failed to set up directory watcher %ls -> %ls", *It.Key, *DirectoryToWatch);
					}
				}
			}
		}
...
FShaderCompileThreadRunnableBase* FShaderCompilingManager::LaunchRemoteShaderCompilingThread(bool bDelayThreadExecution)
{
	...
	if (BuildDistributionController)
	{
		...
		UE_LOGF(LogShaderCompilers, Display, "Using %ls for shader compilation", *BuildDistributionController->GetName());
	...
	{
		UE_LOGF(LogShaderCompilers, Display, "No distributed shader compiler controller found");
	}
	else if (!AllTargetPlatformSupportsRemoteShaderCompiling())
	{
		UE_LOGF(LogShaderCompilers, Display, "Distributed shader compilation is not supported for all target platforms");
	}
	else
	{
		UE_LOGF(LogShaderCompilers, Display, "Distributed shader compilation is disabled");
	}
...
FShaderCompileThreadRunnableBase* FShaderCompilingManager::LaunchLocalShaderCompilingThread(bool bDelayThreadExecution)
{
	...
	UE_LOGF(LogShaderCompilers, Display, "Using %d local workers for shader compilation", NumShaderCompilingThreads);
	...
	if (GIsBuildMachine)
	{
		...
		{
			UE_LOGF(LogShaderCompilers, Warning, "Only %d SCWs will be spawned, which will result in longer shader compile times.", NumShaderCompilingThreads);
		}
	}

	if (FShaderCompileThreadRunnableBase* Thread = LaunchShaderCompilingThread(MoveTemp(LocalThread), bDelayThreadExecution))
	{
...
void FShaderCompilingManager::OnShaderCompilingThreadStarted(FShaderCompileThreadRunnableBase* /*Thread*/)
{
	// Update assigned priority ranges for all threads
	AssignShaderCompilingThreadJobPriorities();
}

void FShaderCompilingManager::OnShaderCompilingThreadStopped(FShaderCompileThreadRunnableBase* Thread)
{
	// If the manager is shutting down, ignore this notification
	if (bIsManagerShuttingDown)
	{
		return;
	}

	// Remove shader compiling thread from list of active threads
	for (auto Iter = Threads.CreateIterator(); Iter; ++Iter)
	{
		if (Iter->Get() == Thread)
		{
			// Move thread ownership to shutdown task. Its memory needs to be kept alive until the thread has finished.
			// Let the task system handle this deferred deletion, since this notification callback is called within the shader compiling threads and waiting for its compiltion in this function would deadlock.
			checkf((*Iter)->bForceFinish, TEXT("%s is signaled to have stopped but is not put into the right state"), (*Iter)->GetThreadName());

			const FString TaskName = FString::Printf(TEXT("ShutdownTask-%s"), Thread->GetThreadName());
			UE::Tasks::Launch(
				*TaskName,
				[Thread = MoveTemp(*Iter)](){
	...
	// Update assigned priority ranges for the remaining threads
	AssignShaderCompilingThreadJobPriorities();
}

void FShaderCompilingManager::AssignShaderCompilingThreadJobPriorities()
{
	...
	else if (Threads.Num() == 1)
	{
		// A single shader compiling thread must take care of all job priorities
		Threads[0]->SetPriorityRange(EShaderCompileJobPriority::Low, EShaderCompileJobPriority::ForceLocal);
	}
}

FShaderCompilingManager::~FShaderCompilingManager()
{
	bIsManagerShuttingDown = true;
	...
	else if (GIsBuildMachine || bForceUseSCWMemoryPressureLimits)
	{
		...
		if (bFoundEntries)
		{
			...
			if (AvailableMemInGB > 0.0f)
			{
				...
				{
					UE_LOGF(LogShaderCompilers, Display, "Machine has %d GBs of RAM, cooker might take %f GBs, but not enough memory left for the OS! (Requested %f GBs for the OS)", PhysicalGBRam, CookerMemoryUsedInGB, MemoryToLeaveForTheOSInGB);
				}
			}
			else
			{
				UE_LOGF(LogShaderCompilers, Display, "Machine has %d GBs of RAM, but cooker might take %f GBs!", PhysicalGBRam, CookerMemoryUsedInGB);
			}
		...
		{
			UE_LOGF(LogShaderCompilers, Warning, "bForceUseSCWMemoryPressureLimits was set but missing one or more prerequisite setting(s): CookerMemoryUsedInGB, MemoryToLeaveForTheOSInGB, MemoryUsedPerSCWProcessInGB.  Ignoring bForceUseSCWMemoryPressureLimits");
		}
...
bool FShaderCompilingManager::ShouldRecompileToDumpShaderDebugInfo(const FShaderCompilerInput& Input, const FShaderCompilerOutput& Output, bool bSucceeded) const
{
	if (Input.DumpDebugInfoPath.IsEmpty())
	{
		...
		else if (DumpShaderDebugInfo == EDumpShaderDebugInfo::OnErrorOrWarning || Output.bForceDumpDebugInfo)
		{
...
void FShaderCompilingManager::ReleaseJob(FShaderCommonCompileJob* Job)
{
	...
	Job->UpdateStatus(EShaderCompileJobStatus::Released);
...
void FShaderCompilingManager::TransferJobsToLocalWorker(TArray<FShaderCommonCompileJobPtr>& JobsToTransfer)
{
	// Make sure we have a local shader compiling thread
	if (FindShaderCompilingThread(EShaderCompilerWorkerType::LocalThread) == nullptr)
	{
		LaunchLocalShaderCompilingThread();
	}

	FScopeLock Lock(&CompileQueueSection);
	AllJobs.TransferJobsToLocalWorker(JobsToTransfer);
}
	...
	FinishedJob->ForEachSingleShaderJob([this](FShaderCompileJob& SingleJob)
		{
			...
			if (bLogJobCompletionTimes || SingleJob.Output.CompileTime > (double)CVarShaderCompilerLogSlowJobThreshold.GetValueOnAnyThread())
			{
				...
				UE_LOGF(LogShaderCompilers, Display, "Job %ls compile time exceeded threshold (%.3fs)", JobName.ToString(), SingleJob.Output.CompileTime);
			}
		});
...
FProcHandle FShaderCompilingManager::LaunchWorker(const FString& WorkingDirectory, uint32 InParentProcessId, uint32 ThreadId, const FString& WorkerInputFile, const FString& WorkerOutputFile, uint32* OutWorkerProcessId)
{
	...
	TStringBuilder<1024> WorkerParameters;
	WorkerParameters << TEXT("\"") << WorkerAbsoluteDirectory << TEXT("/\" ") << InParentProcessId << TEXT(" ") << ThreadId << " " << WorkerInputFile << " " << WorkerOutputFile << TEXT(" -communicatethroughfile ");

	if (GIsBuildMachine)
	{
		WorkerParameters.Appendf(TEXT(" -TimeToLive=%f -buildmachine"), GBuildWorkerTimeToLive);
	}
	else
	{
		WorkerParameters.Appendf(TEXT(" -TimeToLive=%f"), GRegularWorkerTimeToLive);
	}
	if (PLATFORM_LINUX)
	{
		// suppress log generation as much as possible
		WorkerParameters << TEXT(" -logcmds=\"Global None\"");

		if (UE_BUILD_DEBUG)
		{
			// when running a debug build under Linux, make SCW crash with core for easier debugging
			WorkerParameters << TEXT(" -core");
		}
	}
	FCommandLine::BuildSubprocessCommandLine(ECommandLineArgumentFlags::ProgramContext, false /*bOnlyInherited*/, WorkerParameters);
	...
	WorkerParameters << TEXT(" -trace=default");
#else
	WorkerParameters << TEXT(" -nothreading");
	...
	if (GDebugDumpWorkerCrashLog)
	{
		...
		{
			WorkerParameters.Appendf(TEXT(" -AbsLog=\"%s\""), *FPaths::Combine(WorkerCrashLogBaseDirectory, WorkerLogFilename));
		}
		else
		{
			WorkerParameters.Appendf(TEXT(" -Log=%s"), *WorkerLogFilename);
		}
	}
	...
	{
		WorkerParameters.Appendf(TEXT(" -basedir=\"%s\""), *BaseDirOverride);
	}

	FString BaseFromWorkingDirOverride;
	if (FParse::Value(FCommandLine::Get(), TEXT("BaseFromWorkingDir="), BaseFromWorkingDirOverride))
	{
		WorkerParameters.Appendf(TEXT(" -BaseFromWorkingDir=\"%s\""), *BaseFromWorkingDirOverride);
	}
	...
	else
	{
		...
		else
		{
			...
			if (bFirstLaunch)
			{
				...
				UE_LOGF(LogShaderCompilers, Error, "Couldn't launch %ls! Make sure you build ShaderCompileWorker.", *ShaderCompileWorkerName);
			...
			{
				UE_LOGF(LogShaderCompilers, Fatal, "Couldn't launch %ls!", *ShaderCompileWorkerName);
			}
		}
		...
		if (CompilingShaderMap)
		{
			...
			while (MaterialIndex < MaterialDependencies.Num())
			{
				...
				else if (bSuccess)
				{
					...
					{
						const FName ShaderPlatformName = LegacyShaderPlatformToShaderFormat(CompilingShaderMap->GetShaderPlatform());
						if (ShouldShowShaderWarnings())
						{
							UE_LOGF(LogShaderCompilers, Warning, "Warnings while compiling Material %ls for platform %ls:",
								*Material->GetDebugName(), *ShaderPlatformName.ToString());
						}
						else
						{
							UE_LOGF(LogShaderCompilers, Display, "Diagnostic messages while compiling Material %ls for platform %ls:",
								*Material->GetDebugName(), *ShaderPlatformName.ToString());
						}

						for (const FString& UniqueWarning : ErrorInfo.UniqueWarnings)
						{
							// Use structured logs to avoid build machine string matching heuristic that will cause a cook to fail
							if (ShouldShowShaderWarnings())
							{
								UE_LOGFMT(LogShaderCompilers, Warning, "  {DiagnosticMessage}", UniqueWarning);
							}
							else
							{
								UE_LOGFMT(LogShaderCompilers, Display, "  {DiagnosticMessage}", UniqueWarning);
							}
						}
					}
				}
				else
				{
					...
					if (Material->IsDefaultMaterial())
					{
						...
						if (AreShaderErrorsFatal())
						{
							...
							UE_LOGF(LogShaderCompilers, Fatal, "\n%ls", *ErrorString);
						}
						else
						{
							UE_LOGF(LogShaderCompilers, Error, "\n%ls", *ErrorString);
						}
					}
					...
					// Use structured logs to avoid build machine string matching heuristic that will cause a cook to fail
					UE_LOGFMT(LogShaderCompilers, Warning, "{DiagnosticMessage}", ErrorString);
				}
			...
			if (NumIncompleteMaterials == 0 && (IsMaterialMapDDCEnabled() || bRequiredComplete))
			{
				...
				if (ShaderMapToUseForRendering)
				{
					...
					{
						ShaderMapToUseForRendering->SaveToDerivedDataCache(FConservativeShaderParameters(SingleMaterial));
					}
				}
...
static void BuildErrorStringAndReport(const FShaderDiagnosticInfo& DiagInfo, FString& ErrorString)
{
	...
	for (int32 ErrorIndex = 0; ErrorIndex < DiagInfo.UniqueErrors.Num(); ErrorIndex++)
	{
		...
		{
			UE_LOGF(LogShaderCompilers, Warning, "%ls", *UniqueErrorString);
		}
	...
	for (TMap<int32, FShaderMapFinalizeResults>::TIterator It(CompletedShaderMaps); It; ++It)
	{
		...
		if (!FApp::IsUnattended() && !Results.bAllJobsSucceeded && ReissueForDebugJobs.IsEmpty())
		{
				...
				|| It.Key() == GlobalShaderMapId)
			{
				...
				UE_LOGF(LogShaderCompilers, Warning, "%ls", *ErrorString);
				ErrorString += TEXT("\n\n");
...
void FShaderMapCompileResults::CheckIfHung()
{
	if (!bIsHung)
	{
		...
		if (DurationSoFar >= static_cast<double>(GShaderMapCompilationTimeout))
		{
			...
			UE_LOGF(LogShaderCompilers, Error, "Hung shadermap detected, time spent compiling: %f seconds, NumPendingJobs: %d, FinishedJobs: %d",
				DurationSoFar,
			...
			if (GCrashOnHungShaderMaps)
			{
				...
				GShaderCompilingManager->DumpDebugInfoForEachPendingJob([this](FShaderCommonCompileJob* Job) -> bool {
					if (Job->PendingShaderMap.GetReference() == this && !Job->bSucceeded)
					{
						Job->ForEachSingleShaderJob([](FShaderCompileJob& SingleJob) -> void {
							...
							UE_LOGF(LogShaderCompilers, Log, "Job [%ls] pending for %.1f seconds", *SingleJob.Input.GenerateShaderName(), JobDuration);
						});
						return true;
					}
					return false;
				...
				UE_LOGF(LogShaderCompilers, Fatal, "Crashing on a hung shadermap, time spent compiling: %f seconds, NumPendingJobs: %d, FinishedJobs: %d",
					DurationSoFar,
					NumPendingJobs.GetValue(),
					FinishedJobs.Num()
				);
...
void FShaderCompilingManager::CancelCompilation(const TCHAR* MaterialName, const TArray<int32>& ShaderMapIdsToCancel)
{
	...
	{
		UE_LOGF(LogShaders, Display, "CancelCompilation %ls, Removed %d jobs", MaterialName ? MaterialName : TEXT(""), TotalNumJobsRemoved);
	}
}
...
void FShaderCompilingManager::FinishCompilation(const TCHAR* MaterialName, const TArray<int32>& ShaderMapIdsToFinishCompiling)
{
	...
	UE_LOGF(LogShaders, Verbose, "FinishCompilation %ls %.3fs", MaterialName ? MaterialName : TEXT(""), (float)(EndTime - StartTime));
}

void FShaderCompilingManager::FinishAllCompilation()
{
	// No-op if nothing changed since last call, but required at the beginning for LastNumRemainingAssets to
	// be updated so that the subsequent call can notice that where was something to compile and the amount is back to 0.
	UpdateNumRemainingAssets();
	...
	UE_LOGF(LogShaders, Verbose, "FinishAllCompilation %.3fs", (float)(EndTime - StartTime));
}
...
void FShaderCompilingManager::ProcessAsyncResults(float TimeSlice, bool bBlockOnGlobalShaderCompletion)
{
	...
	if (bBlockOnGlobalShaderCompletion && TimeSlice <= 0 && !IsRunningCookCommandlet())
	{
		...
		{
			UE_LOGF(LogShaders, Warning, "Blocking ProcessAsyncResults for %.1fs, processed %u shader maps, %u being compiled", 
	...
	{
		UE_LOGF(LogShaders, Verbose, "Completed %u async shader maps, %u more pending, %u being compiled",
			NumPendingShaderMaps - PendingFinalizeShaderMaps.Num(), 
...
void GenerateInstancedStereoCode(FString& Result, EShaderPlatform ShaderPlatform)
{
	...
	{
		if (StructIt->GetShaderVariableName() == FString("View"))
		{
			View = *StructIt;
		}

		if (StructIt->GetShaderVariableName() == FString("InstancedView"))
		{
	...
	for (int32 MemberIndex = 0; MemberIndex < StructMembersInstanced.Num(); ++MemberIndex)
	{
		...
		if (ViewMember.GetNumElements() >= 1 && (InstancedViewMember.GetNumElements() >= 2 * ViewMember.GetNumElements()))
		{
			...
			for (uint32 ElementIndex = 0; ElementIndex < ViewMember.GetNumElements(); ElementIndex++)
			{
				Result += FString::Printf(
					TEXT("\tResult.%s[%u] = (%s) InstancedView.%s[ViewIndex * %u + %u];\n"),
					ViewMember.GetName(),
					ElementIndex,
					*ViewMemberTypeName,
					InstancedViewMember.GetName(), 
					ViewMember.GetNumElements(), 
					ElementIndex);
			}
		}
		else if (InstancedViewMember.GetNumElements() > 1 && ViewMember.GetNumElements() == 0)
		{
			...
			Result += FString::Printf(
				TEXT("\tResult.%s = (%s) InstancedView.%s[ViewIndex];\n"),
				ViewMember.GetName(), 
				*ViewMemberTypeName, 
				InstancedViewMember.GetName());
		}
		...
		else
		{
			...
			UE_LOGF(
				LogShaderCompilers, 
				Fatal,
				"Don't know how to copy View buffers' field %ls (NumElements=%d) from InstancedView field %ls (NumElements=%d)",
				ViewMember.GetName(), 
				ViewMember.GetNumElements(), 
				InstancedViewMember.GetName(), 
				InstancedViewMember.GetNumElements());
		}
	}
	Result += "\tFinalizeViewState(Result);\n";
	Result += "\treturn Result;\n";
	...
	)
{
	...
	{
		Input.VSDisallowedUniformBufferNames.Add(TEXT("Material"));
		Input.NonVSDisallowedUniformBufferNames.Add(TEXT("MaterialVS"));
	}
	else
	{
		// Make sure we don't try to include the MaterialVS by mistake
		Input.VSDisallowedUniformBufferNames.Add(TEXT("MaterialVS"));
		Input.NonVSDisallowedUniformBufferNames.Add(TEXT("MaterialVS"));
	}
	...
	if (GDumpShaderDebugInfoBindless)
	{
		...
		switch (FShaderPlatformConfig::GetBindlessConfiguration(ShaderPlatform))
		{
		default:
		case ERHIBindlessConfiguration::Disabled:   Builder.Append(TEXT("Off")); break;
		case ERHIBindlessConfiguration::RayTracing: Builder.Append(TEXT("RT")); break;
		case ERHIBindlessConfiguration::Minimal:    Builder.Append(TEXT("Min")); break;
		case ERHIBindlessConfiguration::All:        Builder.Append(TEXT("All")); break;
		}
	...
	
	{
		...
		if (GDumpShaderDebugInfoShort)
		{
			...
			Input.DebugGroupName.ReplaceInline(TEXT("LocalLights"), TEXT("LL"));
	...
	SET_SHADER_DEFINE(Input.Environment, SUPPORTS_SKYLIGHTCAPTURE_AFFECTS_HEIGHTFOGINSCATTERING, SupportsSkyLightCaptureAffectsHeightFogInScattering(ShaderPlatform) ? 1 : 0);
	...
	if (bUsingMobileRenderer)
	{
		...
		if (bMobileDeferredShading)
		{
			...
			SET_SHADER_DEFINE(Input.Environment, MOBILE_EXTENDED_GBUFFER, MobileUsesExtendedGBuffer((EShaderPlatform)Target.Platform) ? 1 : 0);
		}
		else
		{
	...
	SET_SHADER_DEFINE_AND_COMPILE_ARGUMENT(Input.Environment, PRIMITIVE_HAS_TILEOFFSET_DATA, GPrimitiveHasTileOffsetData);
	...
	if (ShaderFrequencyNeedsInstancedStereoMods(ShaderType))
	{
		...
		{
			UE_LOGF(LogShaderCompilers, Warning, "Instanced stereo rendering is not supported for %ls shader platform.", *ShaderFormatName.ToString());
			bWarningIssued = true;
		}
		...
		{
			UE_LOGF(LogShaderCompilers, Warning, "Multiview rendering is not supported for %ls shader platform.", *ShaderFormatName.ToString());
			bWarningIssued = true;
		}
		if (bWarningIssued)
		{
	...
	// At this point only apply the cvar r.Mobile.FloatPrecisionMode value to global shaders
	if (bUsingMobileRenderer && (ShaderType == nullptr || ShaderType->GetGlobalShaderType() != nullptr))
    {
	...
	if (IsMetalPlatform((EShaderPlatform)Target.Platform))
	{
		...
		// Decide wether to use Metal Native Shader Library or not
		bool bArchive = false;
		...
		
		{
			...
			const bool bIsMac = IsPCPlatform(EShaderPlatform(Target.Platform));
			...
			GConfig->GetBool(RuntimeSettingsClassName, TEXT("MetalOptimizeForSize"), bMetalOptimizeForSize, GEngineIni);
			...
			if (CVarSymbols && CVarExtraData)
			{
				bStripIndividualMetalLibs = bStripIndividualMetalLibs && CVarSymbols->GetInt() == 0 && CVarExtraData->GetInt() == 0;
			}
			Input.Environment.SetCompileArgument(TEXT("METAL_STRIP_INDIVIDUAL_METALLIBS"), bStripIndividualMetalLibs);
			...
			const bool bCanCompileOfflineMetalShaders = Compiler && Compiler->CanCompileBinaryShaders();
			if (!bCanCompileOfflineMetalShaders)
			{
				Input.Environment.CompilerFlags.Add(CFLAG_Debug);
			}
			Input.Environment.SetCompileArgument(TEXT("METAL_ALLOW_RUNTIME_SHADER_COMPILING"), !bCanCompileOfflineMetalShaders || bAllowsRuntimeShaderCompiling);
...

	{
		...
		bool bCCBN = CVar && CVar->GetValueOnAnyThread() != 0 && (!bIsMobilePlatform || !IsMobileDeferredShadingEnabled((EShaderPlatform)Target.Platform) || MobileUsesExtendedGBuffer((EShaderPlatform)Target.Platform));
	...
	const bool bSubstrate = Substrate::IsSubstrateEnabled() && IsSubstrateSupportForShaderPipeline(Input);
	{
		...
		if (bSubstrate)
		{
			...
			// Get the highest closure count accounting for all material quality levels for SUBSTRATE_MATERIAL_CLOSURE_COUNT.
			// This is because it needs to account for the maximum value for all the global shaders being compiled only once. (no permutation per material quality level).
			uint32 SubstrateClosurePerPixel = 1;
			if (!Substrate::IsSubstrateBlendableGBufferEnabled(Target.GetPlatform()))
			{
				for (int8 MaterialQualityLevel = EMaterialQualityLevel::Low; MaterialQualityLevel < EMaterialQualityLevel::Num; ++MaterialQualityLevel)
				{
					SubstrateClosurePerPixel = FMath::Max(SubstrateClosurePerPixel, Substrate::GetClosurePerPixel(Target.GetPlatform(), MaterialQualityLevel));
				}
			}
...

		{
			...
			const bool bExperimentalToonReflectionQuantizationEnabled = bSubstrate && CVar && CVar->GetInt() > 0 ? 1 : 0;
			SET_SHADER_DEFINE(Input.Environment, SUBSTRATE_EXPERIMENTAL_TOON_REFLECTION_QUANTIZATION, bExperimentalToonReflectionQuantizationEnabled ? 1 : 0);
		}

		{
			static IConsoleVariable* CVar = IConsoleManager::Get().FindConsoleVariable(TEXT("r.Substrate.Experimental.ToonUnifiedDiffuse"));
			const bool bExperimentalToonUnifiedDiffuseEnabled = bSubstrate && CVar && CVar->GetInt() > 0 ? 1 : 0;
			SET_SHADER_DEFINE(Input.Environment, SUBSTRATE_EXPERIMENTAL_TOON_UNIFIED_DIFFUSE, bExperimentalToonUnifiedDiffuseEnabled ? 1 : 0);
		}
	}
	...
	{
		const bool bMegaLightsSupported = ShouldCompileMegaLightsShaders((EShaderPlatform)Target.Platform);
		SET_SHADER_DEFINE(Input.Environment, PROJECT_SUPPORTS_MEGALIGHTS, bMegaLightsSupported ? 1 : 0);
	}
	...
	SET_SHADER_DEFINE(Input.Environment, PLATFORM_SUPPORTS_DESCRIPTOR_RANGE, FDataDrivenShaderPlatformInfo::GetSupportsDescriptorRange(ShaderPlatform));
	SET_SHADER_DEFINE(Input.Environment, PLATFORM_SUPPORTS_RAYTRACING_CLUSTER_OPS, FDataDrivenShaderPlatformInfo::GetSupportsRayTracingClusterOps(ShaderPlatform));

	// Differentiate between specialization constants being supported by the platform vs being enabled for a shader.
	SET_SHADER_DEFINE(Input.Environment, PLATFORM_SUPPORTS_SPECIALIZATION_CONSTANTS, FDataDrivenShaderPlatformInfo::GetSupportsSpecializationConstants(ShaderPlatform));
	const bool bEnableSpecializationConstants = Input.Environment.SpecializationConstantValues.Num() > 0;
	SET_SHADER_DEFINE(Input.Environment, ENABLE_SPECIALIZATION_CONSTANTS, bEnableSpecializationConstants ? 1 : 0);
...

	{
		...
		// Set Nanite Curve definitions
		SET_SHADER_DEFINE(Input.Environment, NANITE_CURVE_DATA, bNanitePlatform && NaniteCurvesSupported());
	}
...
static bool ParseShaderCompilerFlags(const TCHAR* InFlagsString, FShaderCompilerFlags& OutCompilerFlags)
{
	...
	{
		UE_LOGF(LogShaderCompilers, Warning, "Unknown shader compiler flags: %ls", *UnknownFlagNameList);
		return false;
	}
...
public:
	...
	void Stop(bool DisplayLog = true)
	{
		if (!bAlreadyStopped)
		{
			...
			if (DisplayLog)
			{
				UE_LOGF(LogShaderCompilers, Warning, "		[%ls] took [%.4f] s",*InfoStr,TimeElapsed);
			}
		}
	}
...
namespace
{
	...
	{
		UE_LOGF(LogShaderCompilers, Display, "ShaderTypeName, Filename, PermutationCount");
		for (TLinkedList<FShaderType*>::TIterator It(FShaderType::GetTypeList()); It; It.Next())
		{
			UE_LOGF(LogShaderCompilers, Display, "%ls, %ls, %d", (*It)->GetName(), (*It)->GetShaderFilename(), (*It)->GetPermutationCount());
		}

		UE_LOGF(LogShaderCompilers, Display, "VertexFactoryTypeName, Filename");
		for (TLinkedList<FVertexFactoryType*>::TIterator It(FVertexFactoryType::GetTypeList()); It; It.Next())
		{
			UE_LOGF(LogShaderCompilers, Display, "%ls, %ls", (*It)->GetName(), (*It)->GetShaderFilename());
		}
	}
...
void GetOutdatedShaderTypes(TArray<const FShaderType*>& OutdatedShaderTypes, TArray<const FShaderPipelineType*>& OutdatedShaderPipelineTypes, TArray<const FVertexFactoryType*>& OutdatedFactoryTypes)
{
	...
	{
		UE_LOGF(LogShaders, Warning, "		Recompiling %ls", OutdatedShaderTypes[TypeIndex]->GetName());
	}
	for (int32 TypeIndex = 0; TypeIndex < OutdatedShaderPipelineTypes.Num(); TypeIndex++)
	{
		UE_LOGF(LogShaders, Warning, "		Recompiling %ls", OutdatedShaderPipelineTypes[TypeIndex]->GetName());
	}
	for (int32 TypeIndex = 0; TypeIndex < OutdatedFactoryTypes.Num(); TypeIndex++)
	{
		UE_LOGF(LogShaders, Warning, "		Recompiling %ls", OutdatedFactoryTypes[TypeIndex]->GetName());
	}
#endif // WITH_EDITOR
}
...
bool RecompileShaders(const TCHAR* Cmd, FOutputDevice& Ar)
{
	...
	if (FPlatformProperties::RequiresCookedData())
	{
		...
		{
			UE_LOGF(LogShaders, Error, "Tried to use recompileshaders but ODSC is not active");
		}
#endif
		return true;
	}
	...
	if (FlagStr.Len() > 0)
	{
		...
		if (FCString::Stricmp(*FlagStr,TEXT("Changed")) == 0)
		{
			...
			{
				UE_LOGF(LogShaderCompilers, Warning, "No Shader changes found.");
			}
		}
		...
		else if (FCString::Stricmp(*FlagStr, TEXT("Material")) == 0)
		{
			...
			if (!bMaterialFound)
			{
				...
				UE_LOGF(LogShaderCompilers, Warning, "Couldn't find Material %ls!", *RequestedMaterialName);
			}
		}
		else if (FCString::Stricmp(*FlagStr, TEXT("All")) == 0)
		{
			...
			for( TObjectIterator<UMaterial> It; It; ++It )
			{
				...
				if( Material )
				{
					UE_LOGF(LogShaderCompilers, Log, "recompiling [%ls]",*Material->GetFullName());
	...
	UE_LOGF(LogShaderCompilers, Warning, "Invalid parameter. \n"
											 "Options are: \n"
...
void VerifyGlobalShaders(EShaderPlatform Platform, const ITargetPlatform* TargetPlatform, bool bLoadedFromCacheFile, const TArray<const FShaderType*>* OutdatedShaderTypes, const TArray<const FShaderPipelineType*>* OutdatedShaderPipelineTypes, const FShaderCompilerFlags& InExtraCompilerFlags)
{
	...
	UE_LOGF(LogMaterial, Verbose, "Verifying Global Shaders for %ls (%ls)", *LegacyShaderPlatformToShaderFormat(Platform).ToString(), *ShaderCompiler::GetTargetPlatformName(TargetPlatform));
	...
	if (bEmptyMap)
	{
		UE_LOGF(LogShaders, Log, "	Empty global shader map, recompiling all global shaders");
	}
	...
	for (TLinkedList<FShaderType*>::TIterator ShaderTypeIt(FShaderType::GetTypeList()); ShaderTypeIt; ShaderTypeIt.Next())
	{
		...
		const bool bOutdated = OutdatedShaderTypes && OutdatedShaderTypes->Contains(GlobalShaderType);

		int32 PermutationCountToCompile = 0;
		GlobalShaderType->ForEachCompiledPermutation(Platform, PermutationFlags, [&](int32 PermutationId)
		{
				...
				if (bOutdated || !GlobalShader.IsValid())
				{
					if (bErrorOnMissing)
					{
                        if (IsMetalPlatform(GMaxRHIShaderPlatform))
                        {
                            ...
                            UE_LOGF(LogShaders, Fatal, "Missing global shader %ls's permutation %i, Please make sure cooking was successful.",
		...
		{
			UE_LOGF(LogShaders, Log, "	%ls (%i out of %i)",
	...
	for (TLinkedList<FShaderPipelineType*>::TIterator ShaderPipelineIt(FShaderPipelineType::GetTypeList()); ShaderPipelineIt; ShaderPipelineIt.Next())
	{
		...
		if (Pipeline->IsGlobalTypePipeline())
		{
				...
				&& (!GlobalShaderMap->HasShaderPipeline(Pipeline) || (OutdatedShaderPipelineTypes && OutdatedShaderPipelineTypes->Contains(Pipeline))))
			{
				...
				if (bErrorOnMissing)
				{
					UE_LOGF(LogShaders, Fatal, "Missing global shader pipeline %ls, Please make sure cooking was successful.", Pipeline->GetName());
				}
				else
				{
#if WITH_EDITOR
					if (!bEmptyMap)
					{
						UE_LOGF(LogShaders, Log, "	%ls", Pipeline->GetName());
					}
...
// Declare dummy global PSO collector which doesn't do anything during collection but list to compile is setup below
// in optimized tasks so shaders can be created in parallel to offload the game thread during startup (5+ shaders)
static const TCHAR* GlobalComputePSOCollectorName = TEXT("GlobalComputePSOCollector");
FRegisterGlobalPSOCollectorFunction RegisterGlobalComputePSOCollector(nullptr, GlobalComputePSOCollectorName);
...
{
	const FPSOPrecacheSettings& PSOPrecacheSettings = UPSOPrecacheSettingsManager::GetSettings();
	if (PSOPrecacheSettings.GlobalShaderMode == 0)
	{
		return;
	}
	...
	{
		FGlobalShaderType* GlobalShaderType = ShaderTypeIt->GetGlobalShaderType();
		if (!GlobalShaderType || (GlobalShaderType->GetFrequency() != SF_Compute && PSOPrecacheSettings.GlobalShaderMode == 1))
		{
			continue;
		}
	...
	if (ComputeShadersToPrecache.Num() > 0)
	{
		...
		{
			UE_LOGF(LogShaders, Display, "Precaching %d global compute shaders", ComputeShadersToPrecache.Num());

			struct FGlobalComputePSOPrecacheData
			{
				FGlobalComputePSOPrecacheData(int Count)
				{
					PSOInitializers.Reserve(Count);
				}

				FPSOPrecacheDataArray PSOInitializers;
				FCriticalSection PSOInitializersCS;
			};
			FGlobalComputePSOPrecacheData* GlobalComputePSOPrecacheData = new FGlobalComputePSOPrecacheData(ComputeShadersToPrecache.Num());			

			auto PrecacheComputePipelineState = [GlobalComputePSOPrecacheData, PSOPrecacheSettings](TShaderRef<FShader>& GlobalShader)
			{
				...
				if (RHIComputeShader)
				{							
					static const int32 GlobalComputePSOCollectorIndex = FGlobalPSOCollectorManager::GetIndex(GlobalComputePSOCollectorName);

					check(!RHIComputeShader->HasSpecializationConstants() || !GlobalShader.GetSpecializationConstants().IsEmpty());
					FComputePipelineStateInitializer ComputeInitializer(RHIComputeShader, GlobalShader.GetSpecializationConstants());
								
					FPSOPrecacheData PSOPrecacheData;
					PSOPrecacheData.Type = FPSOPrecacheData::EType::Compute;
					PSOPrecacheData.ComputeInitializer = ComputeInitializer;
					PSOPrecacheData.InMemoryMode = (PSOPrecacheSettings.bKeepGlobalComputePSOsInMemory && KeepPrecachedPSOsInMemoryForActiveMaterials()) ? EPSOPrecacheInMemoryMode::Cached : EPSOPrecacheInMemoryMode::Uncached;
					#if PSO_PRECACHING_VALIDATE
					PSOPrecacheData.ResourceName = GlobalShader.GetType()->GetName();
					PSOPrecacheData.PSOCollectorIndex = GlobalComputePSOCollectorIndex;
					PSOPrecacheData.VertexFactoryType = nullptr;
					#endif // PSO_PRECACHING_VALIDATE

					FScopeLock ScopeLock(&GlobalComputePSOPrecacheData->PSOInitializersCS);
					GlobalComputePSOPrecacheData->PSOInitializers.Add(MoveTemp(PSOPrecacheData));
				}
			};
			...
			if (GRHIGlobals.SupportsMultithreadedShaderCreation)
			{
				...
				for (TShaderRef<FShader>& GlobalShader : ComputeShadersToPrecache)
				{
					Tasks.Emplace(UE::Tasks::Launch(TEXT("PrecachePSOsForGlobalShaders"),
						[&GlobalShader, PrecacheComputePipelineState]()
						{
							PrecacheComputePipelineState(GlobalShader);
						}
					));
				}
				ENQUEUE_RENDER_COMMAND(WaitOnPSOPrecacheTasks)(
					[ComputeShadersToPrecache = MoveTemp(ComputeShadersToPrecache), Tasks = MoveTemp(Tasks), GlobalComputePSOPrecacheData](FRHICommandListImmediate& RHICmdList)
					{
						UE::Tasks::Wait(Tasks);

						RequestPrecachePSOs(EPSOPrecacheType::Global, GlobalComputePSOPrecacheData->PSOInitializers);
						delete GlobalComputePSOPrecacheData;
					});
			}
			else
			{
				ENQUEUE_RENDER_COMMAND(PrecachePSOsForGlobalShaders)(
					[ComputeShadersToPrecache = MoveTemp(ComputeShadersToPrecache), GlobalComputePSOPrecacheData, PrecacheComputePipelineState](FRHICommandListImmediate& RHICmdList)
					{
						for (TShaderRef<FShader> GlobalShader : ComputeShadersToPrecache)
						{
							PrecacheComputePipelineState(GlobalShader);
	...
	// Set if the global graphics PSOs need to be kept in memory or not
	for (FPSOPrecacheData& PrecacheData : GlobalPSOInitializers)
	{
		PrecacheData.InMemoryMode = (PSOPrecacheSettings.bKeepGlobalGraphicPSOsInMemory && KeepPrecachedPSOsInMemoryForActiveMaterials()) ? EPSOPrecacheInMemoryMode::Cached : EPSOPrecacheInMemoryMode::Uncached;
	}
...
static FPreLoadFile GGlobalShaderPreLoadFile(*(FString("../../../Engine") / TEXT("GlobalShaderCache-SP_") + FPlatformProperties::IniPlatformName() + TEXT(".bin")));
...
{
	return FString("Engine") / TEXT("GlobalShaderCache-") + FDataDrivenShaderPlatformInfo::GetName(Platform).ToString() + TEXT(".bin");
}
...
FString SaveGlobalShaderFile(EShaderPlatform Platform, FString SavePath, class ITargetPlatform* TargetPlatform)
{
	...
	{
		UE_LOGF(LogShaders, Fatal, "Could not save global shader file to '%ls'", *FullPath);
	}
			...
			for (int32 PermutationId = 0; PermutationId < PermutationCount; PermutationId++)
			{
				if (ShouldCacheGlobalShaderTypeName(GlobalShaderType, PermutationId, TypeNameSubstring, Platform, GlobalShaderPermutation))
				{
					if (!GlobalShaderMap->HasShader(GlobalShaderType, PermutationId))
					{
						if (FailureReason)
						{
							FString GlobalShaderTypeName = GlobalShaderType ? GlobalShaderType->GetFName().ToString() : FString("Unknown shader type");
...
static bool TryLoadCookedGlobalShaderMap(EShaderPlatform Platform, FScopedSlowTask& SlowTask)
{
	...
	if (bAllowOverrideGlobalShaders)
	{
		...
		if (!bFileExist)
		{
			UE_LOGF(LogShaders, Display, "%ls doesn't exists", *OverrideGlobalShaderCacheFilename);
		}
		else
		{
			...
			if (bLoadedFromCacheFile)
			{
				UE_LOGF(LogShaders, Display, "%ls has been loaded successfully", *OverrideGlobalShaderCacheFilename);
			}
			else
			{
				UE_LOGF(LogShaders, Error, "%ls failed to load", *OverrideGlobalShaderCacheFilename);
			}
		}
	}
...
void CompileGlobalShaderMap(EShaderPlatform Platform, const ITargetPlatform* TargetPlatform, bool bRefreshShaderMap)
{
	...
	{
		UE_LOGF(LogShaders, Display, "Trying to load global shaders from ODSC ...");
		GODSCManager->TryLoadGlobalShaders(Platform);
		UE_LOGF(LogShaders, Display, "Global shaders from ODSC: %ls", (GGlobalShaderMap[Platform] != nullptr) ? TEXT("success") : TEXT("failed"));
	}
#endif
	...
	if (!GGlobalShaderMap[Platform])
	{
		...
		{
			PrimeGlobalShaderSourceHashCache(Platform, TargetPlatform);
		...
		if (!bLoadedFromCacheFile && !bAllowShaderCompiling)
		{
			...
			{
				UE_LOGF(LogShaders, Error, "%ls", *Message.ToString());
			...
			{
				UE_LOGF(LogShaders, Fatal, "%ls", *Message.ToString());
			}
		}
...
{
	UE_LOGF(LogShaders, Display, "Reloading global shaders...");
...
void SetGlobalShaderCacheOverrideDirectory(const TArray<FString>& Args)
{
	if (Args.Num() == 0)
	{
		UE_LOGF(LogShaders, Error, "Failed to set GGlobalShaderCacheOverrideDirectory without any arguments");
		return; 
	}
	
	GGlobalShaderCacheOverrideDirectory = Args[0];
	UE_LOGF(LogShaders, Log, "GGlobalShaderCacheOverrideDirectory = %ls", *GGlobalShaderCacheOverrideDirectory);
}
...
bool RecompileChangedShadersForPlatform(const FString& PlatformName)
{
	...
	if (TargetPlatform == nullptr)
	{
		UE_LOGF(LogShaders, Display, "Failed to find target platform module for %ls", *PlatformName);
		return false;
	}
	...
	GetOutdatedShaderTypes(OutdatedShaderTypes, OutdatedShaderPipelineTypes, OutdatedFactoryTypes);
	UE_LOGF(LogShaders, Display, "We found %d out of date shader types, %d outdated pipeline types, and %d out of date VF types!", OutdatedShaderTypes.Num(), OutdatedShaderPipelineTypes.Num(), OutdatedFactoryTypes.Num());
	...
	if (bIsValid)
	{
		...
		if (NewGlobalShaderMap)
		{
			...
			else
			{
				...
				UE_LOGF(LogShaderCompilers, Error, "%ls", *ErrorMessage);
#if WITH_ODSC
