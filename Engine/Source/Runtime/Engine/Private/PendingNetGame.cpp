...
void UPendingNetGame::InitNetDriver(UNetDriver* InNetDriver)
{
	...
	if (!GDisallowNetworkTravel)
	{
		...
		{
			UE_LOGF(LogNet, Warning, "Error initializing the pending net driver.  Check the configuration of NetDriverDefinitions and make sure module/plugin dependencies are correct.");
		...
		else
		{
			...
			UE_LOGF(LogNet, Warning, "error initializing the network stack");
	...
	if (NetDriver != nullptr)
	{
		...
		if (ServerConn != nullptr)
		{
			...
			if (!bEncryptionRequirementsFailure)
			{
				...
				UE_LOGF(LogNet, Log, "UPendingNetGame::SendInitialJoin: Sending hello. %ls", *ServerConn->Describe());
			...
			{
				UE_LOGF(LogNet, Error, "UPendingNetGame::SendInitialJoin: EncryptionToken is empty when 'net.AllowEncryption' requires it.");
...
void UPendingNetGame::NotifyControlMessage(UNetConnection* Connection, uint8 MessageType, class FInBunch& Bunch)
{
	...
	UE_LOGF(LogNet, Verbose, "NotifyControlMessage: PendingLevel received: %ls", FNetControlMessageInfo::GetName(MessageType));
#endif
	...
	switch (MessageType)
	{
		...
		case NMT_Failure:
		{
			...
			if (FNetControlMessage<NMT_Failure>::Receive(Bunch, ErrorMsg))
			{
				...
				UE_LOGF(LogNet, Log, "NetConnection::Close() [%ls] [%ls] [%ls] from NMT_Failure %ls",
		...
		case NMT_Welcome:
		{
			...
			if (FNetControlMessage<NMT_Welcome>::Receive(Bunch, URL.Map, GameName, RedirectURL))
			{
				...
				UE_LOGF(LogNet, Log, "Welcomed by server (Level: %ls, Game: %ls)", *URL.Map, *GameName);
		...
		case NMT_EncryptionAck:
		{
			...
			else
			{
				...
				// Force close the session
				UE_LOGF(LogNet, Warning, "%ls: No delegate available to handle encryption ack, disconnecting.", *Connection->GetName());
				Connection->Close();
			}
			break;
		}
		default:
			UE_LOGF(LogNet, Log, " --- Unknown/unexpected message for pending level");
			break;
	}
}
...
void UPendingNetGame::FinalizeEncryptedConnection(const FEncryptionKeyResponse& Response, TWeakObjectPtr<UNetConnection> WeakConnection)
{
	...
	if (Connection)
	{
		if (Connection->GetConnectionState() != USOCK_Invalid && Connection->GetConnectionState() != USOCK_Closed && Connection->Driver)
		{
			...
			else
			{
				...
				UE_LOGF(LogNet, Warning, "UPendingNetGame::FinalizeEncryptedConnection: encryption failure [%ls] %ls", *ResponseStr, *Response.ErrorMsg);
		...
		{
			// This error will be resolved in TickWorldTravel()
			UE_LOGF(LogNet, Warning, "UPendingNetGame::FinalizeEncryptedConnection: connection in invalid state. %ls", *Connection->Describe());
	...
	{
		// This error will be resolved in TickWorldTravel()
		UE_LOGF(LogNet, Warning, "UPendingNetGame::FinalizeEncryptedConnection: Connection is null.");
...
void UPendingNetGame::SetEncryptionKey(const FEncryptionKeyResponse& Response)
{
	...
	{
		UE_LOGF(LogNet, Log, "UPendingNetGame::SetEncryptionKey: net.AllowEncryption is false.");
		return;
	}
	...
	if (NetDriver)
	{
		...
		if (Connection)
		{
			if (Connection->GetConnectionState() != USOCK_Invalid && Connection->GetConnectionState() != USOCK_Closed && Connection->Driver)
			{
				...
				else
				{
					// This error will be resolved in TickWorldTravel()
					FString ResponseStr(LexToString(Response.Response));
					UE_LOGF(LogNet, Warning, "UPendingNetGame::SetEncryptionKey: encryption failure [%ls] %ls", *ResponseStr, *Response.ErrorMsg);
			...
			{
				// This error will be resolved in TickWorldTravel()
				UE_LOGF(LogNet, Warning, "UPendingNetGame::SetEncryptionKey: connection in invalid state. %ls", *Connection->Describe());
				ConnectionError = TEXT("Connection encryption state failure");
				Connection->Close();
			}
		}
		...
		{
			// This error will be resolved in TickWorldTravel()
			UE_LOGF(LogNet, Warning, "UPendingNetGame::SetEncryptionKey: Connection is null.");
			ConnectionError = TEXT("Connection missing during encryption ack");
		}
	}
	else
	{
		// This error will be resolved in TickWorldTravel()
		UE_LOGF(LogNet, Warning, "UPendingNetGame::SetEncryptionKey: NetDriver is null.");
