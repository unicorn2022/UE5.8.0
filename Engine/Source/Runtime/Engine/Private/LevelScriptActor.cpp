...
void ALevelScriptActor::EnableInput(class APlayerController* PlayerController)
{
	...
	{
		UE_LOGF(LogLevel, Warning, "EnableInput on a LevelScript actor can not be specified for only one PlayerController.  Enabling for all PlayerControllers.");
	}
	bInputEnabled = true;
}
...
void ALevelScriptActor::DisableInput(class APlayerController* PlayerController)
{
	if (PlayerController != NULL)
	{
		UE_LOGF(LogLevel, Warning, "DisableInput on a LevelScript actor can not be specified for only one PlayerController.  Disabling for all PlayerControllers.");
	}
	bInputEnabled = false;
}
...
TArray<ALevelScriptActor*> ALevelScriptActor::FindSiblingLevelScriptActors() const
{
	...
	if (!ThisOuter->IsA<UPackage>())
	{
		...
		GetObjectsWithOuter(ThisOuter, AllSiblingObjects, EGetObjectsFlags::None, RF_NoFlags, EInternalObjectFlags::Garbage);
