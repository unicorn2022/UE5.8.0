...
FTaskSyncManager::FTaskSyncManager()
{
	...
	FWorldDelegates::OnPostWorldInitialization.AddRaw(this, &FTaskSyncManager::OnPostWorldInitialization);
#if WITH_EDITOR	
	FEditorDelegates::EndPIE.AddRaw(this, &FTaskSyncManager::EndPIE);
#endif
}
...
void FTaskSyncManager::ReloadRegisteredData()
{
	...
	for (const FSyncPointDescription& SyncPoint : Settings->RegisteredSyncPoints)
	{
		if (SyncPoint.EventType == ESyncPointEventType::Invalid || SyncPoint.ActivationRules == ESyncPointActivationRules::Invalid || !SyncPoint.IsValid())
		{
			...
			UE_CLOGF(!GIsEditor, LogTaskSync, Error, "Cannot register invalid sync point %ls from project settings!", *SyncPoint.RegisteredName.ToString());
...
bool FTaskSyncManager::RegisterNewSyncPoint(const FSyncPointDescription& NewDescription)
{
	...
	if (ensure(NewDescription.IsValid() && !NewDescription.WasLoadedFromSettings()))
	{
		...
		{
			UE_LOGF(LogTaskSync, Error, "Cannot register sync point %ls from source %ls as it already exists!", *NewDescription.RegisteredName.ToString(), *NewDescription.SourceName.ToString());
		}
	}
	else
	{
		UE_LOGF(LogTaskSync, Error, "Cannot register invalid sync point description %ls from source %ls!", *NewDescription.RegisteredName.ToString(), *NewDescription.SourceName.ToString());
	}
...
bool FTaskSyncManager::UnregisterSyncPoint(FName RegisteredName, FName SourceName)
{
	...
	if (ensure(!RegisteredName.IsNone() && !SourceName.IsNone()))
	{
		...
		if (!FoundId)
		{
			UE_LOGF(LogTaskSync, Error, "Cannot unregister sync point %ls from source %ls as it does not exist!", *RegisteredName.ToString(), *SourceName.ToString());
			return false;
		}
		else
		{
			if (const FRegisteredSyncPointData* FoundData = RegisteredDataMap.Find(*FoundId))
			{
				if (FoundData->RegisteredPoint.SourceName == SourceName)
				{
					for (FBatchData& BatchData : BatchList)
					{
						DestroySyncPointIfValid(BatchData, *FoundId);
					}
				...
				{
					UE_LOGF(LogTaskSync, Error, "Cannot unregister sync point %ls from source %ls as it was added by source %ls!", *RegisteredName.ToString(), *SourceName.ToString(), *FoundData->RegisteredPoint.SourceName.ToString());
	...
	{
		UE_LOGF(LogTaskSync, Error, "Cannot unregister invalid sync point %ls from source %ls!", *RegisteredName.ToString(), *SourceName.ToString());
	}
...
FActiveSyncPoint* FTaskSyncManager::GetOrCreateSyncPoint(FBatchData& BatchData, FRegisteredSyncPointData& SyncData)
{
	...
	if (!FoundData)
	{
		...
		case ESyncPointEventType::WorkerThreadTask_HighPriority:
			...
			break;
		default:
			ensureMsgf(0, TEXT("Invalid event type %d on sync point %s!"), EnumToUnderlyingType(Description.EventType), *Description.RegisteredName.ToString());
		}
		...
		case ESyncPointActivationRules::ActivateForAnyWork:
			...
			break;
		default:
			ensureMsgf(0, TEXT("Invalid activation rules %d on sync point %s!"), EnumToUnderlyingType(Description.ActivationRules), *Description.RegisteredName.ToString());
		}
...
bool FTaskSyncManager::DestroySyncPointIfValid(FBatchData& BatchData, FSyncPointId::FInternalId SyncId)
{
	TSharedRef<FActiveSyncPoint>* FoundData = BatchData.SyncPointData.Find(SyncId);

	if (FoundData)
	{
		// Unregister the tick function which will stop it from ticking if it's still pending
		(*FoundData)->UnRegisterTickFunction();

		if (IsTicking() && FWorldContextId(CurrentTickWorld) == BatchData.WorldContext)
		{
			// If we're currently ticking, we need to keep this alive for a frame
			BatchData.SyncPointsToDelete.Add(FoundData->ToSharedPtr());
		}

		// Remove the shared ptr. If this is the only thing keeping it alive, this will destroy it immediately
		BatchData.SyncPointData.Remove(SyncId);
...
void FTaskSyncManager::EndFrame(const UWorld* InWorld)
{
	...
	for (FBatchData& BatchData : BatchList)
	{
		if (BatchData.WorldContext == WorldContext)
		{
			...
			BatchData.SyncPointsToDelete.Reset();
		}
	}
}

void FTaskSyncManager::InitializeForWorld(FWorldContextId WorldContext)
{
	check(IsInGameThread() && WorldContext.IsValid());

	// The batch may already exist, if so just initialize it again
	bool bFoundBatch = false;
	for (FBatchData& BatchData : BatchList)
	{
		if (BatchData.WorldContext == WorldContext)
		{
			bFoundBatch = true;
			InitializeBatchForNewWorld(BatchData);
		}
	}
	...
	if (!bFoundBatch)
	{
		FBatchContextId BatchContext = CreateNewBatch(WorldContext);
		check(BatchList.IsValidIndex(BatchContext.BatchId));
		InitializeBatchForNewWorld(BatchList[BatchContext.BatchId]);
	}
}

void FTaskSyncManager::InitializeBatchForNewWorld(FBatchData& BatchData)
{
	// Make sure all registered sync points are created.
	for (TPair<FSyncPointId::FInternalId, FRegisteredSyncPointData>& DataPair : RegisteredDataMap)
	{
		GetOrCreateSyncPoint(BatchData, DataPair.Value);
	}
}

void FTaskSyncManager::OnPostWorldInitialization(UWorld* World, const UWorld::InitializationValues IVS)
{
	check(World != nullptr);

	FWorldContextId WorldContext(World);
...
#if WITH_EDITOR
void FTaskSyncManager::EndPIE(bool bStartSimulate)
{
	// PIE shutdown doesn't properly call OnWorldContextDestroyed, so iterate the list ourselves
	for (const FWorldContext& Context : GEngine->GetWorldContexts())
	{
		if (Context.WorldType == EWorldType::PIE)
		{
			OnWorldContextRemove(const_cast<FWorldContext&>(Context));
		}
	}
}
#endif
