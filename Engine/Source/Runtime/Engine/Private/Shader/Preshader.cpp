...
void FPreshaderData::WriteValue(const FValue& Value)
{
	...
	check(!Value.Type.IsObject() && !Value.Type.IsStruct());

	Write(Value.Type.ValueType);
...
FPreshaderDataContext::FPreshaderDataContext(const FPreshaderData& InData, TArrayView<float> InResultsBuffer)
	: Ptr(InData.Data.GetData())
	, EndPtr(Ptr + InData.Data.Num())
	, Names(InData.Names)
	, ResultsBuffer(InResultsBuffer)
{}
...
{
	// Preshader2 doesn't include individual preshaders, just a single bytecode stream, so this function shouldn't be called on those.
	check(InContext.IsPreshader2() == false);
}
...
static void EvaluateComponentSwizzle(FPreshaderStack& Stack, FPreshaderDataContext& RESTRICT Data)
{
	...
	case 1:
		...
		break;
	default:
		UE_LOGF(LogMaterial, Fatal, "Invalid number of swizzle elements: %d", NumElements);
		break;
	}
}
...
FPreshaderValue EvaluatePreshader(const FUniformExpressionSet* UniformExpressionSet, const FMaterialRenderContext& Context, FPreshaderStack& Stack, FPreshaderDataContext& RESTRICT Data)
{
	...
	while (Data.Ptr < DataEnd)
	{
		...
		default:
			UE_LOGF(LogMaterial, Fatal, "Unknown preshader opcode %d", (uint8)Opcode);
			break;
		}
	}
...
{
	OutHasher.Update(&bPreshader2, sizeof(bPreshader2));
