...
void LineCheckTracker::SetSpikeMinTraceCount(int32 MinTraceCount)
{
	...
	UE_LOGF(LogSpawn, Log, "Line trace spike count is %d.", TraceCountForSpikeDump);
}
...
void LineCheckTracker::ToggleLineChecks()
{
	...
	UE_LOGF(LogSpawn, Log, "Line tracing is now %ls.", bIsTrackingEnabled ? TEXT("enabled") : TEXT("disabled"));
 ...
 *	  MAC address for spawned actors, assign a default one in this case.
 **/
...
AActor* UWorld::SpawnActor( UClass* Class, FTransform const* UserTransformPtr, const FActorSpawnParameters& SpawnParameters )
{
	...
	if( !Class )
	{
		UE_LOGF(LogSpawn, Warning, "SpawnActor failed because no class was specified" );
		return NULL;
	}
	...
	{
		UE_LOGF(LogSpawn, Warning, "SpawnActor failed because we are in the process of tearing down the world");
		return NULL;
	}
	else if (UserTransformPtr && UserTransformPtr->ContainsNaN())
	{
		UE_LOGF(LogSpawn, Warning, "SpawnActor failed because the given transform (%ls) is invalid", *(UserTransformPtr->ToString()));
		return NULL;
	}
#if !WITH_EDITOR
	else if (SpawnParameters.bForceGloballyUniqueName && !SpawnParameters.Name.IsNone())
	{
		UE_LOGF(LogSpawn, Warning, "SpawnActor failed because a globally unique name was requested and an actor name was provided (%ls)", *SpawnParameters.Name.ToString());
		return NULL;
	}
#else
	if (SpawnParameters.OverridePackage && SpawnParameters.bCreateActorPackage)
	{
		UE_LOGF(LogSpawn, Warning, "SpawnActor failed because both the OverridePackage and bCreateActorPackage are set");
		return nullptr;
	}
#endif
	...
	else if (StaticFindObjectFast(nullptr, LevelToSpawnIn, NewActorName) || ((bNeedGloballyUniqueName != FActorSpawnUtils::IsGloballyUniqueName(NewActorName)) && (SpawnParameters.NameMode == FActorSpawnParameters::ESpawnActorNameMode::Requested)))
	{
		...
		case FActorSpawnParameters::ESpawnActorNameMode::Required_Fatal:
			UE_LOGF(LogSpawn, Fatal, "Cannot generate unique name for '%ls' in level '%ls'.", *NewActorName.ToString(), *LevelToSpawnIn->GetFullName());
			return nullptr;

		case FActorSpawnParameters::ESpawnActorNameMode::Required_ErrorAndReturnNull:
			UE_LOGF(LogSpawn, Error, "Cannot generate unique name for '%ls' in level '%ls'.", *NewActorName.ToString(), *LevelToSpawnIn->GetFullName());
			return nullptr;
	...
	{
		UE_LOGF(LogSpawn, Warning, "Unable to spawn class '%ls' due to client/server context.", *Class->GetName() );
		return NULL;
	}
	...
	if (SpawnParameters.CustomPreSpawnInitialization)
	{
		SpawnParameters.CustomPreSpawnInitialization(Actor);
	}
	...
	{
		UE_LOGF(LogSpawn, Log, "SpawnActor failed because the spawned actor %ls is invalid", *Actor->GetPathName());
		return NULL;
	}
...
bool UWorld::DestroyActor( AActor* ThisActor, bool bNetForce, bool bShouldModifyLevel )
{
	...
	else if (WorldType != EWorldType::Inactive && !IsRunningCommandlet())
	{
		...
		if (!UWorld::WorldTypePreLoadMap.Find(GetOuter()->GetFName()))
		{
			...
			UE_LOGF(LogSpawn, Warning, "UWorld::DestroyActor: World has no context! World: %ls, Actor: %ls", *GetName(), *ThisActor->GetPathName());
		}
	}
...
bool UWorld::FindTeleportSpot(const AActor* TestActor, FVector& TestLocation, FRotator TestRotation)
{
	...
	if ( Adjust.IsNearlyZero() )
	{
		...
		if (MoveComponent && MoveComponent->UpdatedPrimitive)
		{
			...
			{
				UE_LOGF(LogPhysics, Warning, "UWorld::FindTeleportSpot called with an actor that is intersecting geometry. Failed to find new location likely due to "
...
{	
	...
	if (World && PrimComp)
	{
		...
		if (CollisionShape.IsBox() && (Cast<UBoxComponent>(PrimComp) == nullptr))
		{
			...
			{
				UE_LOGF(LogPhysics, Log, "Components must be registered in order to be used in a ComponentOverlapMulti call. PriComp: %ls TestActor: %ls", *PrimComp->GetName(), *TestActor->GetName());
				return false;
			}
		}
		else
		...
		if (bFoundBlockingHit && bComputePenetrationAdjustment)
		{
			...
			for (int32 HitIdx = 0; HitIdx < Overlaps.Num(); HitIdx++)
			{
				...
				if (OverlapComponent && OverlapComponent->GetCollisionResponseToChannel(BlockingChannel) == ECR_Block)
				{
					...
					{
						UE_LOGF(LogPhysics, Log, "OverlapTest says we are overlapping, yet MTD says we're not. Something is wrong");
					...
					if (bSuccess && FMath::IsNearlyZero(MTDResult.Distance))
					{
						...
						{
							// Ignore this overlap.
							UE_LOGF(LogPhysics, Log, "OverlapTest says we are overlapping, yet MTD says we're not (with smaller shape). Ignoring this overlap.");
							NumBlockingHits--;
							continue;
						}
					}
...
bool UWorld::EncroachingBlockingGeometry(const AActor* TestActor, FVector TestLocation, FRotator TestRotation, FVector* ProposedAdjustment)
{
	...
	UMovementComponent* const MoveComponent = TestActor->FindComponentByClass<UMovementComponent>();
	IMovementInterface* MovementInterface = nullptr;
	UPrimitiveComponent* MovedPrimComp = nullptr;
	
	if (MoveComponent)
	{
		MovedPrimComp = MoveComponent->UpdatedPrimitive;
	}
	else
	{
		MovementInterface = TestActor->FindComponentByInterface<IMovementInterface>();
		MovedPrimComp = MovementInterface ? Cast<UPrimitiveComponent>(MovementInterface->GetUpdatedObject()) : nullptr;
	}
			...
			if( StreamingLevel )
			{
				...
				if ( !bSkipFile )
				{
					if ( !bAlreadyLoaded )
					{
						...
						{
							UE_LOGF(LogSpawn, Warning, "Streaming level uses short package name (%ls). Level will not be loaded.", *StreamingLevelWorldAssetPackageName);
						}
					...
					else
					{
						...
						if (LoadedWorld)
						{
							if (ULevel* WorldLevel = LoadedWorld->PersistentLevel)
							{
								...
								{
									UE_LOGF(LogSpawn, Warning, "Streaming level already has a loaded level, but it differs from the one in the associated package (%ls).", *StreamingLevelWorldAssetPackageName);
								}
							}
							else
							{
								UE_LOGF(LogSpawn, Warning, "Streaming level already has a loaded level, but the world in the associated package (%ls) does not have its persistent level set.", *StreamingLevelWorldAssetPackageName);
							}
						}
						else
						{
							UE_LOGF(LogSpawn, Warning, "Streaming level already has a loaded level, but the associated package (%ls) seems to contain no world.", *StreamingLevelWorldAssetPackageName);
						}
					}
				}
			}
