...
FTransform ComputeWorldSpaceTargetTM(
	const USkeletalMeshComponent& SkeletalMeshComponent, 
	const TArrayView<const FTransform> ComponentSpaceTransforms, 
	int32 BoneIndex)
{
	return ComponentSpaceTransforms[BoneIndex] * SkeletalMeshComponent.GetComponentToWorld();
}

FTransform ComputeLocalSpaceTargetTM(
	const USkeletalMeshComponent& SkeletalMeshComponent, 
	...
	int32 BoneIndex)
{
	...
	FTransform AccumulatedDelta = BoneSpaceTransforms[BoneIndex];
	...
	while ((CurBoneIdx = RefSkeleton.GetParentIndex(CurBoneIdx)) != INDEX_NONE)
	{
		...
		AccumulatedDelta = AccumulatedDelta * BoneSpaceTransforms[CurBoneIdx];
	}

	return FTransform::Identity;
}

FTransform ComputeTargetTM(
	...
	int32 BoneIndex)
{
	return PhysAnimData.bIsLocalSimulation 
		? ComputeLocalSpaceTargetTM(SkeletalMeshComponent, PhysAsset, BoneSpaceTransforms, BoneIndex)
		: ComputeWorldSpaceTargetTM(SkeletalMeshComponent, ComponentSpaceTransforms, BoneIndex);
}
...
void UPhysicalAnimationComponent::UpdateTargetActors(ETeleportType TeleportType)
{
	...
	if (PhysAsset && SkeletalMeshComponent->GetSkeletalMeshAsset())
	{
		...
		const TArrayView<const FTransform> ComponentSpaceTransforms = SkeletalMeshComponent->GetEditableComponentSpaceTransforms();

		if (ComponentSpaceTransforms.IsEmpty())
		{
			// Our tick depends on the skeletal mesh primary tick. When physics is disabled (i.e.
			// physics blend = 0), this might have been allowed to run post-physics. Thus, if
			// physics blending is enabled after the ticks have started processing (this can happen
			// from the actor's Blueprint code) then for one tick we might find ourselves running in
			// a post-physics tick group.This can result in this function running at the same time
			// as the secondary EndPhysics SKM tick function that performs a parallel blend back
			// from physics into the SKM component transforms. When this happens, the component
			// transforms will have been moved temporarily into the animation context. Preventing
			// this from happening is difficult (as it would need the tick functions to be updated
			// whilst they are getting processed) so we simply return if the component transforms
			// are missing. Note that setting CVarAnimationDelaysEndGroup
			// ("tick.AnimationDelaysEndGroup") to 0 should prevent us from getting here, as it
			// stops the primary animation tick from being allowed to run after PrePhysics.
			return;
		}
		FPhysicsCommand::ExecuteWrite(SkeletalMeshComponent, [&]()
		{
			const TArrayView<const FTransform> BoneSpaceTransforms = SkeletalMeshComponent->GetBoneSpaceTransformsView();
			for (int32 DataIdx = 0; DataIdx < DriveData.Num(); ++DataIdx)
			{
				const FPhysicalAnimationData& PhysAnimData = DriveData[DataIdx];
				...
				if (FPhysicsActorHandle TargetActor = InstanceData.TargetActor)
				{
					...
					// It's possible the skeletal mesh has changed out from under us. In that case
					// we should probably reset, but at the very least don't do work on non-existent bones
					if (BoneIdx != INDEX_NONE)
					{
						const FTransform TargetTM = ComputeTargetTM(
							PhysAnimData, *SkeletalMeshComponent, *PhysAsset, 
							BoneSpaceTransforms, ComponentSpaceTransforms, BoneIdx);
						// todo(chaos): Calling SetKinematicTarget_AssumesLocked before
						// SetGlobalPose_AssumesLocked is unnecessary for chaos. We should fix this
						// when PhysX is removed.
