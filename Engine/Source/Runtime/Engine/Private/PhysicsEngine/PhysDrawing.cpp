...
void FKTaperedCapsuleElem::DrawTaperedCapsuleSides(FPrimitiveDrawInterface* PDI, const FQuat& CapsuleOrientation, const FVector& InCenter0, const FVector& InCenter1, float InRadius0, float InRadius1, const FColor& Color, bool bInSplitTaperedCylinder)
{
...
static void DrawWireframeTaperedCapsule(class FPrimitiveDrawInterface* PDI, const FColor& Color, const FQuat& CapsuleOrientation, const FVector& Origin, const FVector& XAxis, const FVector& YAxis, const FVector& ZAxis, const float ScaledHalfLength, const float ScaledRadius0, const float ScaledRadius1, const bool bIsOneSided)
{
	const FColor HalfTaperedColor = bIsOneSided ? (FLinearColor(Color) * .2f).ToFColor(true) : Color;
	// Draw top and bottom circles
	const FVector TopEnd = Origin + (ScaledHalfLength * ZAxis);
	const FVector BottomEnd = Origin - (ScaledHalfLength * ZAxis);
	...
	else
	{
		...
		const float OffsetZ0 = FMath::IsNearlyZero(Distance) ? 0.f : ((ScaledRadius0 - ScaledRadius1) * ScaledRadius0) / Distance;
		float TopLimit = FMath::IsNearlyZero(ScaledRadius0) ? 90.f : 90.0f - FMath::RadiansToDegrees(FMath::Asin(OffsetZ0 / ScaledRadius0));
		...
		const float OffsetZ1 = FMath::IsNearlyZero(Distance) ? 0.f : ((ScaledRadius0 - ScaledRadius1) * ScaledRadius1) / Distance;
		...
		// Draw connecty lines
		FKTaperedCapsuleElem::DrawTaperedCapsuleSides(PDI, CapsuleOrientation, TopEnd, BottomEnd, ScaledRadius0, ScaledRadius1, Color, bIsOneSided);
	}
}
...
void FKTaperedCapsuleElem::DrawElemWire(class FPrimitiveDrawInterface* PDI, const FTransform& ElemTM, const FVector& Scale3D, const FColor Color) const
{
	...
	if (IsExtruded())
	{
		const float ScaledHalfWidth = Width * Scale3D.X * .5f;
		const FVector FrontOrigin = Origin + (ScaledHalfWidth * XAxis);
		const FVector BackOrigin = Origin - (ScaledHalfWidth * XAxis);
		DrawWireframeTaperedCapsule(PDI, Color, ElemTM.GetRotation(), FrontOrigin, XAxis, YAxis, ZAxis, ScaledHalfLength, ScaledRadius0, ScaledRadius1, IsOneSidedCollision());
		DrawWireframeTaperedCapsule(PDI, Color, ElemTM.GetRotation(), BackOrigin, XAxis, YAxis, ZAxis, ScaledHalfLength, ScaledRadius0, ScaledRadius1, IsOneSidedCollision());


		// Draw connecty lines
		const FVector TopFrontEnd = FrontOrigin + ScaledHalfLength * ZAxis;
		const FVector TopBackEnd = BackOrigin + ScaledHalfLength * ZAxis;
		PDI->DrawLine(TopFrontEnd + ScaledRadius0 * ZAxis, TopBackEnd + ScaledRadius0 * ZAxis, Color, SDPG_World);
		PDI->DrawLine(TopFrontEnd + ScaledRadius0 * YAxis, TopBackEnd + ScaledRadius0 * YAxis, Color, SDPG_World);
		PDI->DrawLine(TopFrontEnd - ScaledRadius0 * YAxis, TopBackEnd - ScaledRadius0 * YAxis, Color, SDPG_World);

		const FVector BottomFrontEnd = FrontOrigin - ScaledHalfLength * ZAxis;
		const FVector BottomBackEnd = BackOrigin - ScaledHalfLength * ZAxis;
		PDI->DrawLine(BottomFrontEnd - ScaledRadius1 * ZAxis, BottomBackEnd - ScaledRadius1 * ZAxis, Color, SDPG_World);
		PDI->DrawLine(BottomFrontEnd + ScaledRadius1 * YAxis, BottomBackEnd + ScaledRadius1 * YAxis, Color, SDPG_World);
		PDI->DrawLine(BottomFrontEnd - ScaledRadius1 * YAxis, BottomBackEnd - ScaledRadius1 * YAxis, Color, SDPG_World);

	}
	else
	{
		DrawWireframeTaperedCapsule(PDI, Color, ElemTM.GetRotation(), Origin, XAxis, YAxis, ZAxis, ScaledHalfLength, ScaledRadius0, ScaledRadius1, IsOneSidedCollision());
	}
}
...
struct FScopedTaperedCapsuleBuilder
{
	...
	{
		const bool bIsExtrudedCapsule = InTaperedCapsule.IsExtruded();
		...
		const float ScaledHalfWidth = bIsExtrudedCapsule ? InTaperedCapsule.Width * InScale3D.X * .5f : 0.f;;
		...
		const int32 NumSides = DrawCollisionSides + (bIsExtrudedCapsule ? 2 : 0);
		const int32 NumRings = (DrawCollisionSides / 2) + 1;
		...
		// Then rotate this arc NumSides+1 times.
		for (int32 SideIdx = 0; SideIdx < DrawCollisionSides + 1; SideIdx++)
		{
			const FRotator3f ArcRotator(0, 360.f * ((float)SideIdx / DrawCollisionSides), 0);
			const FRotationMatrix44f ArcRot(ArcRotator);
			const float XTexCoord = ((float)SideIdx / DrawCollisionSides);

			const FVector3f XOffset(SideIdx <= DrawCollisionSides / 2 ? -ScaledHalfWidth : ScaledHalfWidth, 0, 0);
			const int32 SideIdxToVIxOffset = (bIsExtrudedCapsule && SideIdx > DrawCollisionSides / 2) ? 1 : 0;
			for (int32 VertIdx = 0; VertIdx < NumRings + 1; VertIdx++)
			{
				const int32 VIx = (NumRings + 1) * (SideIdx + SideIdxToVIxOffset) + VertIdx;

				Verts[VIx].Position = ArcRot.TransformPosition(ArcVerts[VertIdx].Position) + XOffset;
			...
			if (bIsExtrudedCapsule)
			{

				if (SideIdx == DrawCollisionSides / 2)
				{
					...
					for (int32 VertIdx = 0; VertIdx < NumRings + 1; VertIdx++)
					{
						...
						Verts[VIx].Position = ArcRot.TransformPosition(ArcVerts[VertIdx].Position) + FVector3f(ScaledHalfWidth, 0.f, 0.f);
				...
				else if (SideIdx == DrawCollisionSides)
				{
					// Duplicate vertex on second half
					for (int32 VertIdx = 0; VertIdx < NumRings + 1; VertIdx++)
					{
...
void FKConvexElem::DrawElemWire(FPrimitiveDrawInterface* PDI, const FTransform& ElemTM, const float Scale, const FColor Color) const
{
	...
	{
		UE_LOGF(LogPhysics, Log, "FKConvexElem::DrawElemWire : No ConvexMesh, so unable to draw.");
	}
}
...
void FKConvexElem::DrawElemSolid(FPrimitiveDrawInterface* PDI, const FTransform& ElemTM, const float Scale, const FMaterialRenderProxy* MaterialRenderProxy) const
{
	...
	{
		UE_LOGF(LogPhysics, Log, "FKConvexElem::DrawElemSolid : No ConvexMesh, so unable to draw.");
	}
}
...
void FKConvexElem::AddCachedSolidConvexGeom(TArray<FDynamicMeshVertex>& VertexBuffer, TArray<uint32>& IndexBuffer, const FColor VertexColor) const
{
	...
	{
		UE_LOGF(LogPhysics, Log, "FKConvexElem::AddCachedSolidConvexGeom : No ConvexMesh, so unable to draw.");
	}
}
