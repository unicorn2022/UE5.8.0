...
static void OnDumpPhysicalMaterialMaskData(const TArray< FString >& Arguments)
{
	...
	if (Arguments.Num() > 0)
	{
		...
		if (PhysMatMaskAsset.IsValid())
		{
			...
			{
				UE_LOGF(LogPhysicalMaterialMask, Warning, "Could not load PhysicalMaterialMask asset for argument: %ls", *Arguments[0]);
			}
		}
		else
		{
			UE_LOGF(LogPhysicalMaterialMask, Warning, "Could not find PhysicalMaterialMask asset for argument: %ls", *Arguments[0]);
		}
	}
	else
	{
		UE_LOGF(LogPhysicalMaterialMask, Warning, "Command requires a PhysicalMaterialMask asset reference to be specified.");
	}
}
...
void UPhysicalMaterialMask::DumpMaskData()
{
	...
	if (MaskData.Num() == 0)
	{
		UE_LOGF(LogPhysicalMaterialMask, Display, "PhysicalMaterialMask data is empty.");
	}
...
uint32 UPhysicalMaterialMask::GetPhysMatIndex(const TArray<uint32>& MaskData, int32 SizeX, int32 SizeY, int32 AddressX, int32 AddressY, float U, float V)
{
	...
	AdjustCoord(V, static_cast<TextureAddress>(AddressY));
