...
FBox UPhysicsAsset::CalcAABB(const USkinnedMeshComponent* MeshComp, const FTransform& LocalToWorld) const
{
	...
	{
		UE_LOGF(LogPhysics, Log,  "UPhysicsAsset::CalcAABB : Non-uniform scale factor. You will not be able to collide with it.  Turn off collision and wrap it with a blocking volume.  MeshComp: %ls  SkelMesh: %ls", *MeshComp->GetFullName(), MeshComp->GetSkinnedAsset() ? *MeshComp->GetSkinnedAsset()->GetFullName() : TEXT("NULL") );
	}
...
void SanitizeProfilesHelper(const TArray<T*>& SetupInstances, const TArray<FName>& PreProfiles, TArray<FName>& PostProfiles, FPropertyChangedEvent& PropertyChangedEvent, const FName PropertyName, FName& CurrentProfileName, TFunctionRef<void (T*, FName, FName)> RenameFunc, TFunctionRef<void(T*, FName, FName)> DuplicateFunc, TFunctionRef<void(T*, const TArray<FName>& )> UpdateFunc)
{
	...
	if (ArrayIdx != INDEX_NONE)
	{
		if(PropertyChangedEvent.ChangeType != EPropertyChangeType::ArrayMove && PropertyChangedEvent.ChangeType != EPropertyChangeType::ArrayRemove)
		{
			...
			FName NewName = (PostProfiles[ArrayIdx] == NAME_None) ? FName("New") : PostProfiles[ArrayIdx];
...
#if WITH_EDITOR
void UPhysicsAsset::GetAssetRegistryTagMetadata(TMap<FName, FAssetRegistryTagMetadata>& OutMetadata) const
{
	Super::GetAssetRegistryTagMetadata(OutMetadata);

	OutMetadata.Add("Bodies",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UPhysicsAsset", "Bodies_Label", "Bodies"))
		);

	OutMetadata.Add("Constraints",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UPhysicsAsset", "Constraints_Label", "Constraints"))
		);
}
#endif
