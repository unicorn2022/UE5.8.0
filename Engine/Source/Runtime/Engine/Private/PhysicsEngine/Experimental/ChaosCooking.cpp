	...
	namespace Cooking
	{
		...
		Chaos::FTriangleMeshImplicitObjectPtr BuildSingleTrimesh(const FTriMeshCollisionData& Desc, TArray<int32>& OutFaceRemap, TArray<int32>& OutVertexRemap)
		{
			...
			auto LambdaHelper = [&Desc, &FinalVerts, &FinalIndices, &TriMeshParticles, &OutFaceRemap, &OutVertexRemap](auto& Triangles) -> Chaos::FTriangleMeshImplicitObjectPtr
			{
				...
				Chaos::FTriangleMeshImplicitObject::FBuildSettings MeshBuildSettings;
				MeshBuildSettings.PerFaceBounds = Desc.bReducedMemoryRepresentation ? Chaos::FTrimeshBVH::EPerFaceBoundsRepresentation::Byte : Chaos::FTrimeshBVH::EPerFaceBoundsRepresentation::VectorizedFloat;
				constexpr bool bCullsBackFaceRaycast = false;
				Chaos::FTriangleMeshImplicitObjectPtr TriangleMesh( new Chaos::FTriangleMeshImplicitObject(MoveTemp(TriMeshParticles), MoveTemp(Triangles), MoveTemp(MaterialIndices), MoveTemp(OutFaceRemapPtr), MoveTemp(OutVertexRemapPtr),
					bCullsBackFaceRaycast, MeshBuildSettings));
		...
		void BuildTriangleMeshes(TArray<Chaos::FTriangleMeshImplicitObjectPtr>& OutTriangleMeshes, TArray<int32>& OutFaceRemap, TArray<int32>& OutVertexRemap, const FCookBodySetupInfo& InParams)
		{
			...
			auto LambdaHelper = [&](auto& Triangles)
			{
				...
				// Keep the face remap if either related flag is true, to carry it through initial processing. (It will be stripped later from the cook helper's remap array if bSupportFaceRemap is false.)
				bool bNeedFaceRemapOnBVH = InParams.bSupportFaceRemap || InParams.bSupportFaceRemapOnMeshBVH;
				TUniquePtr<TArray<int32>> OutFaceRemapPtr = bNeedFaceRemapOnBVH ? MakeUnique<TArray<int32>>(OutFaceRemap) : nullptr;
				TUniquePtr<TArray<int32>> OutVertexRemapPtr = (InParams.bSupportVertexRemap && Chaos::TriMeshPerPolySupport) ? MakeUnique<TArray<int32>>(OutVertexRemap) : nullptr;
				if (!OutFaceRemapPtr)
				{
					OutFaceRemap.Empty();
				}
				if (!OutVertexRemapPtr)
				{
					OutVertexRemap.Empty();
				}
				Chaos::FTriangleMeshImplicitObject::FBuildSettings MeshBuildSettings;
				MeshBuildSettings.PerFaceBounds = InParams.TriangleMeshDesc.bReducedMemoryRepresentation ? Chaos::FTrimeshBVH::EPerFaceBoundsRepresentation::Byte : Chaos::FTrimeshBVH::EPerFaceBoundsRepresentation::VectorizedFloat;
				constexpr bool bCullsBackFaceRaycast = false;
