			...
			FPhysicsCommand::ExecuteRead(InInstance->GetPhysicsActor(), [&](const FPhysicsActorHandle& Actor)
			{
				...
				if(FPhysicsActorHandle RigidBody = TargetInstance->GetPhysicsActor())
				{
					...
					for (int32 ShapeIdx = 0; ShapeIdx < NumShapes; ShapeIdx++)
					{
						...
						const bool bShapeIsComplex = ShapeFilter.HasFlag(Chaos::EFilterFlags::ComplexCollision);
						const bool bShapeIsSimple = ShapeFilter.HasFlag(Chaos::EFilterFlags::SimpleCollision);
					...
					if (BestHit.Distance < FLT_MAX)
					{
						...
						// Verify the hit is made by doing an overlap test.
						const Chaos::Filter::FQueryFilterData QueryFilter = Chaos::Filter::FQueryTraceFilterBuilder::BuildOverlapAll();

						FTransform StartTM(WorldStart);
						const UWorld* World = InInstance->GetWorld();

						ConvertQueryImpactHit(World, BestHit, OutHit, DeltaMag, QueryFilter, WorldStart, WorldEnd, nullptr, StartTM, true, bExtractPhysMaterial);
						bHitSomething = true;
					}
				}
			});
				...
				for (int32 ShapeIdx = 0; ShapeIdx < NumShapes; ShapeIdx++)
				{
					...
					if ((bSweepComplex && bShapeIsComplex) || (!bSweepComplex && bShapeIsSimple))
					{
						...
						if (bShapeHit && FloatDistance < Hit.Distance)
						{
							...
							ConvertQueryImpactHit(World, Hit, OutHit, DeltaMag, QueryFilter, InStart, InEnd, &QueryGeometry, StartTransform, true, false);
							bSweepHit = true;
						}
					}
				}
...
bool FPhysInterface_Chaos::GetSquaredDistanceToBody(const FBodyInstance* InInstance, const FVector& InPoint, float& OutDistanceSquared, FVector* OutOptPointOnBody)
{
	...
	{
		UE_LOGF(LogPhysics, Verbose, "GetDistanceToBody: Component (%ls) has no simple collision and cannot be queried for closest point.", InInstance->OwnerComponent.Get() ? *(InInstance->OwnerComponent->GetPathName()) : TEXT("NONE"));
	}
