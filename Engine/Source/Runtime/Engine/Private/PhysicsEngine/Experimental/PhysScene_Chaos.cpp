...
bool bLockSceneOnlyToUpdateAcc = false;
FAutoConsoleVariableRef CVar_LockSceneOnlyToUpdateAcc(TEXT("p.LockSceneOnlyToUpdateAcc"), bLockSceneOnlyToUpdateAcc, TEXT("Lock scene only to update the external acceleration structure. The scene won't be lock the entire Synch Body time."));
...
FPhysScene_Chaos::FPhysScene_Chaos(AActor* InSolverActor
	...
	, LastTrailingEventDispatchTime(Chaos::FReal(-1))
#if WITH_EDITOR
	, SingleStepCounter(0)
#endif
#if CHAOS_WITH_PAUSABLE_SOLVER
...
#endif
{
	...
	EventManager->RegisterHandler<Chaos::FTrailingEventData>(Chaos::EEventType::Trailing, this, &FPhysScene_Chaos::HandleTrailingEvents);
...
FPhysScene_Chaos::~FPhysScene_Chaos()
{
	...
	PhysicsReplicationQuantization.Reset();
...
void FPhysScene_Chaos::RemoveObject(FSkeletalMeshPhysicsProxy* InObject)
{
	...
	if(NumRemoved == 0)
	{
		UE_LOGF(LogChaos, Warning, "Attempted to remove an object that wasn't found in its solver's gamethread storage - it's likely the solver has been mistakenly changed.");
	}
...
bool FPhysScene_Chaos::IsComponentMigrated(UPrimitiveComponent* PrimComp) const
{
#if UE_WITH_REMOTE_OBJECT_HANDLE
	return MigratedPrimitiveComponentOwnerIds.Contains(TObjectPtr<AActor>(PrimComp->GetOwner()).GetRemoteId());
#else
	return false;
#endif
}

void FPhysScene_Chaos::ResetComponentOwnerMigratedSet()
{
	MigratedPrimitiveComponentOwnerIds.Reset();
}

void FPhysScene_Chaos::AddToComponentOwnerMigratedSet(const FRemoteObjectId CompOwnerId)
{
	MigratedPrimitiveComponentOwnerIds.Add(CompOwnerId);
}

void FPhysScene_Chaos::RemoveFromComponentOwnerMigratedSet(const FRemoteObjectId CompOwnerId)
{
	MigratedPrimitiveComponentOwnerIds.Remove(CompOwnerId);
}
...
const IPhysicsReplication* FPhysScene_Chaos::GetPhysicsReplication() const
{
	return PhysicsReplication.Get();
}
...
void FPhysScene_Chaos::AddReferencedObjects(FReferenceCollector& Collector)
{
	...
	for (TPair<IPhysicsProxyBase*, TWeakObjectPtr<UPrimitiveComponent>>& Pair : PhysicsProxyToComponentMap)
	{
		if (TObjectPtr<UPrimitiveComponent> Component = Pair.Get<1>().Get())
		{
...
{
	UE_CHAOS_ASYNC_INITBODY_READSCOPELOCK(PhysicsProxyComponentMapsLock);
	if (const TWeakObjectPtr<UPrimitiveComponent>* ComponentWeak = PhysicsProxyToComponentMap.Find(PhysicsProxy))
	{
		const TStrongObjectPtr<UPrimitiveComponent> StrongFoundComp = ComponentWeak->Pin();
		if (ensureAlwaysMsgf(StrongFoundComp.IsValid(), TEXT("GetOwningComponent found primitive component in map which has been garbage collected")))
		{
			return StrongFoundComp.Get();
		}
	}
	return nullptr;
}
...
void FPhysScene_Chaos::HandleTrailingEvents(const Chaos::FTrailingEventData& Event)
{
	QUICK_SCOPE_CYCLE_COUNTER(STAT_HandleGlobalTrailingEvents);

	const float TrailingDataTimestamp = Event.TrailingData.TimeCreated;
	if (Event.TrailingData.bHasGlobalEvent && TrailingDataTimestamp > LastTrailingEventDispatchTime)
	{
		LastTrailingEventDispatchTime = TrailingDataTimestamp;
		TArray<FChaosTrailingEvent> PendingTrailingEvent;
		for (const Chaos::FTrailingData& TrailingDataItem : Event.TrailingData.AllTrailingsArray)
		{
			if ((TrailingDataItem.EmitterFlag & Chaos::EventEmitterFlag::GlobalDispatcher) && TrailingDataItem.Proxy)
			{
				FChaosTrailingEvent& TrailingEvent = PendingTrailingEvent.AddZeroed_GetRef();
				TrailingEvent.Component = GetOwningComponent<UPrimitiveComponent>(TrailingDataItem.Proxy);
				TrailingEvent.Location = TrailingDataItem.Location;
				TrailingEvent.Orientation = TrailingDataItem.Orientation;
				TrailingEvent.LinearVelocity = TrailingDataItem.Velocity;
				TrailingEvent.AngularVelocity = TrailingDataItem.AngularVelocity;
				TrailingEvent.Mass = static_cast<float>(TrailingDataItem.Mass);
				TrailingEvent.LocalBounds = FBox(TrailingDataItem.BoundingBox.Min(), TrailingDataItem.BoundingBox.Max());
				TrailingEvent.Index = TrailingDataItem.TransformGroupIndex;
			}
		}
		if (PendingTrailingEvent.Num() > 0)
		{
			ChaosEventRelay->DispatchPhysicsTrailingEvents(PendingTrailingEvent);
		}
	}
}
...
void FPhysScene_Chaos::OnUpdateWorldPause()
{
	...
	if (bIsWorldPaused != bIsPaused)
	{
		...
		if (PhysDispatcher)
		{
			UE_LOGF(LogFPhysScene_ChaosSolver, Verbose, "FPhysScene_Chaos::OnUpdateWorldPause() pause status changed for actor %ls, bIsPaused = %d", Actor ? *Actor->GetName() : TEXT("None"), bIsPaused);
...
const FRigidBodyState* FPhysScene_Chaos::GetStateFromReplicationCache(UPrimitiveComponent* RootComponent, int& ServerFrame)
{
	...
	{
		return GetStateFromReplicationCache(PhysicsObject, ServerFrame);
	}
	ServerFrame = 0;
	return nullptr;
}

const FRigidBodyState* FPhysScene_Chaos::GetStateFromReplicationCache(Chaos::FPhysicsObjectHandle PhysicsObject, int& ServerFrame)
{
	// Create the physics replication cache if not already created
	CreatePhysicsReplicationCache();
	return PhysicsReplicationCache->GetStateFromReplicationCache(PhysicsObject, ServerFrame);
}
...
void FPhysScene_Chaos::CreatePhysicsReplicationQuantization()
{
	if (!PhysicsReplicationQuantization)
	{
		PhysicsReplicationQuantization = MakeUnique<FPhysicsReplicationQuantization>(this);
	}
}

void FPhysScene_Chaos::CreatePhysicsReplicationCache()
{
	// Create the quantization manager first so its PostSolve sim callback registers
	// before the cache's. Sim callback execution order is registration order.
	CreatePhysicsReplicationQuantization();
...
{
	UE_CHAOS_ASYNC_INITBODY_WRITESCOPELOCK(PhysicsProxyComponentMapsLock);
	const TWeakObjectPtr<UPrimitiveComponent>* ComponentWeak = PhysicsProxyToComponentMap.Find(InObject);
	if (ComponentWeak)
	{
		// Pin() safely resolves the weak pointer (FGCScopeGuard internally), unlike Get() which is not safe off game thread.
		// The strong reference is not retained - ComponentKey is used as a map key only, never dereferenced.
		if (UPrimitiveComponent* ComponentKey = Cast<UPrimitiveComponent>(ComponentWeak->Pin(true).Get()))
		{
			PendingMigratedPhysicsStateByComponent.Remove(ComponentKey);

			TArray<IPhysicsProxyBase*>* ProxyArray = ComponentToPhysicsProxyMap.Find(ComponentKey);
			if (ProxyArray)
			{
				ProxyArray->RemoveSingleSwap(InObject, EAllowShrinking::No);
				if (ProxyArray->Num() == 0)
				{
					ComponentToPhysicsProxyMap.Remove(ComponentKey);
				}
			}
		}
	}
...
FPhysScene_AsyncPhysicsStateJobQueue& FPhysScene_Chaos::GetOrCreateAsyncPhysicsStateJobQueue()
{
	check(IsInGameThread());
	...
	return *AsyncPhysicsStateJobQueue.Get();
}
...
bool FPhysScene_Chaos::PushAsyncCreatePhysicsStateJob(IAsyncPhysicsStateProcessor* Processor)
{
	if (SupportsAsyncPhysicsStateCreation())
	{
		...
		GetOrCreateAsyncPhysicsStateJobQueue().AddJob({ Processor, FPhysScene_AsyncPhysicsStateJobQueue::EJobType::CreatePhysicsState });
		return true;
	}
...
bool FPhysScene_Chaos::PushAsyncDestroyPhysicsStateJob(IAsyncPhysicsStateProcessor* Processor)
{
	if (SupportsAsyncPhysicsStateDestruction())
	{
		...
		GetOrCreateAsyncPhysicsStateJobQueue().AddJob({ Processor, FPhysScene_AsyncPhysicsStateJobQueue::EJobType::DestroyPhysicsState });
		return true;
	}
	return false;
}
...
bool FPhysScene_Chaos::HasAsyncPhysicsStateJobs() const
{
	check(IsInGameThread());
	return (AsyncPhysicsStateJobQueue.IsValid() && !AsyncPhysicsStateJobQueue->IsCompleted());
}
				...
				for (int32 i = 0; i < NumBodies; i++)
				{
					...
					if (!BodyInst->IsInstanceSimulatingPhysics() || Info.TeleportType != ETeleportType::None)
					{
						...
						if (BoneIndex != INDEX_NONE)
						{
							...
							{
								UE_LOGF(LogChaos,Log,"Bone Index %d out of bounds (%d Bones)", BoneIndex, ComponentSpaceTransforms.Num());
							}
						}
					}
				}
...
void FPhysScene_Chaos::OnSyncBodies(Chaos::FPhysicsSolverBase* Solver)
{
			...
			if (FBodyInstance* BodyInstance = ChaosInterface::GetUserData(*(Proxy->GetParticle_LowLevel())))
			{
				...
				if (UPrimitiveComponent* OwnerComponent = BodyInstance->OwnerComponent.Get())
				{
					...
					if (!NewTransform.EqualsNoScale(OwnerComponent->GetComponentTransform()))
					{
						...
						if (bLockSceneOnlyToUpdateAcc)
						{
							FScopedSceneLock_Chaos SceneLock(PhysScene, EPhysicsInterfaceScopedLockType::Write);
	...
	static FName TransactionalDispatcherWorkName("FPhysScene_Chaos::OnSyncBodies-Dispatcher");
...
#if UE_WITH_REMOTE_OBJECT_HANDLE && UE_AUTORTFM
		static FName TransactionalWorkName("FPhysScene_Chaos::OnSyncBodies");
	...
	if(FRewindData* RewindData = Solver->GetRewindData())
	{
		...
		if(NumFrames > 0)
		{
			...
			if(ensure(Solver->GetRewindData()->RewindToFrame(FirstFrame)))
			{
				...
				{
					UE_LOGF(LogChaos,Log,"Resim had %d desyncs",DesyncedParticles.Num());
