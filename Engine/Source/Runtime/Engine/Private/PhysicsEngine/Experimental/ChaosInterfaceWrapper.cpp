...
namespace ChaosInterface
{
	...
	Chaos::FPhysicsObject* GetPhysicsObjectFromUserData(const Chaos::FGeometryParticle& Actor)
	{
		...
		if (UserData)
		{
			Chaos::FPhysicsObject* PhysicsObject = FChaosUserData::Get<Chaos::FPhysicsObject>(UserData);
			if (!PhysicsObject)
			{
				// Check if we appended a custom entity
				FChaosUserEntityAppend* ChaosUserEntityAppend = FChaosUserData::Get<FChaosUserEntityAppend>(UserData);
				if (ChaosUserEntityAppend)
				{
					PhysicsObject = FChaosUserData::Get<Chaos::FPhysicsObject>(ChaosUserEntityAppend->ChaosUserData);
				}
			}
