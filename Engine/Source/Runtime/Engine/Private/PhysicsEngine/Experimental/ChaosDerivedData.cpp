...
{
	GatherKeyData();
	return PluginSpecificCacheKeySuffix;
}

void FChaosDerivedDataCooker::GatherKeyData() const
{
	if (!PluginSpecificCacheKeySuffix.IsEmpty())
	{
		return;
	}

	FString SetupGeometryKey("INVALID");
	...
	PluginSpecificCacheKeySuffix = FString::Printf(TEXT("%s_%s_REAL%d"),
	...
	PluginSpecificCacheKeySuffix.Append(TEXT("_arm64"));
#endif
}
