...
const FRigidBodyState* FPhysicsReplicationCache::GetStateFromReplicationCache(UPrimitiveComponent* RootComponent, int32& OutSolverFrame)
{
	...
	{
		return GetStateFromReplicationCache(PhysicsObject, OutSolverFrame);
	}

	return nullptr;
}

const FRigidBodyState* FPhysicsReplicationCache::GetStateFromReplicationCache(Chaos::FPhysicsObjectHandle PhysicsObject, int32& OutSolverFrame)
{
...
void FPhysicsReplicationCacheAsync::PopulateReplicationCache_Internal()
{
	...
	for (auto It = ReplicationCache_Internal.CreateIterator(); It; ++It)
	{
		...
		{
			// Unregister physics object from caching state
			PendingDeleteFromReplicationCache_Internal.Add(It.Key());
	...
	for (Chaos::FConstPhysicsObjectHandle& PhysicsObject : PendingDeleteFromReplicationCache_Internal)
	{
		ReplicationCache_Internal.Remove(PhysicsObject);
	}
	PendingDeleteFromReplicationCache_Internal.Reset();
