	...
	FPhysicsCommand::ExecuteRead(this, [&]()
	{
		...
		for(int32 i=0; i<InRequiredBones.Num(); i++)
		{
			...
			{	
				...
				{
					UE_LOGF(LogPhysics, Warning, "%ls(Mesh %ls, PhysicsAsset %ls)", 
...
void USkeletalMeshComponent::UpdateKinematicBonesToAnim(const TArray<FTransform>& InSpaceBases, ETeleportType Teleport, bool bNeedsSkinning, EAllowKinematicDeferral DeferralAllowed)
{
	...
	{
		UE_LOGF(LogPhysics, Log, "USkeletalMeshComponent::UpdateKinematicBonesToAnim : Non-uniform scale factor (%ls) can cause physics to mismatch for %ls  SkelMesh: %ls", *MeshScale3D.ToString(), *GetFullName(), GetSkeletalMeshAsset() ? *GetSkeletalMeshAsset()->GetFullName() : TEXT("NULL"));
	}
#endif
			...
			FPhysicsCommand::ExecuteWrite(this, [&]()
			{
				...
				for(int32 i = 0; i < NumBodies; i++)
				{
						...
						(bTeleport || !BodyInst->IsInstanceSimulatingPhysics()))	//If we have a body and it's kinematic, or we are teleporting a simulated body
					{
						...
						{
							FName BodyName = "UNKNOWN";
							if (PhysicsAsset->SkeletalBodySetups.IsValidIndex(i))
							{
								BodyName = PhysicsAsset->SkeletalBodySetups[i]->BoneName;
							}
							UE_LOGF(LogPhysics, Log, "UpdateRBBones: WARNING: Failed to find bone '%ls' need by PhysicsAsset '%ls' in SkeletalMesh '%ls'.", *BodyName.ToString(), *PhysicsAsset->GetName(), *GetSkeletalMeshAsset()->GetName());
						}
						else
						{
							if (BoneIndex >= InSpaceBases.Num())
							{
								...
								UE_LOGF(LogPhysics, Warning, "BoneIndex %d out of range of SpaceBases (Size %d) on PhysicsAsset '%ls' in SkeletalMesh '%ls' for bone '%ls'", BoneIndex, InSpaceBases.Num(), *PhysicsAsset->GetName(), *GetSkeletalMeshAsset()->GetName(), *BodyName.ToString());
								continue;
							}
							...
							if(!BoneTransform.IsValid())
							{
								...
								UE_LOGF(LogPhysics, Warning, "UpdateKinematicBonesToAnim: Trying to set transform with bad data %ls on PhysicsAsset '%ls' in SkeletalMesh '%ls' for bone '%ls'", *BoneTransform.ToHumanReadableString(), *PhysicsAsset->GetName(), *GetSkeletalMeshAsset()->GetName(), *BodyName.ToString());
					...
					else
					{
						...
						if(!bBlendPhysics && BodyInst->PhysicsBlendWeight <= 0.f && BodyInst->BodySetup.IsValid())
						{
							...
							UE_LOGF(LogPhysics, Verbose, "%ls(Mesh %ls, PhysicsAsset %ls, Bone %ls) is simulating, but no blending. ",
