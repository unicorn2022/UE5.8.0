...
bool InitGamePhys()
{
	...
	GPostInitHandle = FCoreDelegates::GetOnPostEngineInit().AddLambda([]()
	{
	...
	UE_LOGF(LogInit, Log, "Physics initialised using underlying interface: %ls", *FPhysicsInterface::GetInterfaceDescription());
...
void TermGamePhys()
{
	...
	{
		FCoreDelegates::GetOnPostEngineInit().Remove(GPostInitHandle);
