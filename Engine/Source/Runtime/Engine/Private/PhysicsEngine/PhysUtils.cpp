...
bool FPhysicsInterface::ExecPhysCommands(const TCHAR* Cmd, FOutputDevice* OutputDevice, UWorld* InWorld)
{
	...
	{
		Chaos::FTrackedGeometryManager::Get().DumpMemoryUsage(Cmd, OutputDevice);
		return true;
	}
	...
	{
		class FChaosArchiveExternalContext : public Chaos::FChaosArchive
		{
		public:
			FChaosArchiveExternalContext(FArchive& ArIn)
				: FChaosArchive(ArIn)
			{
			}

			explicit FChaosArchiveExternalContext(FArchive& ArIn, const TSharedRef<Chaos::FChaosArchiveContext>& InExternalSharedContext)
		...
		TSharedRef<Chaos::FChaosArchiveContext> ArchiveContext = MakeShared<Chaos::FChaosArchiveContext>();
		FChaosArchiveExternalContext Ar(BaseAr, ArchiveContext);
		...
		int64 TotalBytes = 0;
		for (const TPair<FName, Chaos::FChaosArchiveSectionData>& It : MemoryMap)
		{
