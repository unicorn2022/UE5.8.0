...
float ComputeAverageMass_AssumesLocked(Chaos::FPhysicsObject* Body1, Chaos::FPhysicsObject* Body2)
{
	...
	if(NumDynamic > 1)
	{
		AverageMass = AverageMass / NumDynamic;
	}
...
bool GetActorRefs(FBodyInstance* Body1, FBodyInstance* Body2, FPhysicsActorHandle& OutActorRef1, FPhysicsActorHandle& OutActorRef2, UObject* DebugOwner)
{
	...
	bool bActor1InvalidRigidBody = false;
	bool bActor2InvalidRigidBody = false;
	FPhysicsCommand::ExecuteRead(ActorRef1, ActorRef2, [&bActor1InvalidRigidBody, &bActor2InvalidRigidBody, ActorRef1, ActorRef2](const FPhysicsActorHandle& ActorA, const FPhysicsActorHandle& ActorB)
	{
		bActor1InvalidRigidBody = !FPhysicsInterface::IsValid(ActorRef1) || !FPhysicsInterface::IsRigidBody(ActorRef1);
		bActor2InvalidRigidBody = !FPhysicsInterface::IsValid(ActorRef2) || !FPhysicsInterface::IsRigidBody(ActorRef2);
	});
	if(bActor1InvalidRigidBody && bActor2InvalidRigidBody)
	{
...
bool FConstraintInstance::CreateJoint_AssumesLocked(Chaos::FPhysicsObject* Body1, Chaos::FPhysicsObject* Body2)
{
	...
	{
		UE_LOGF(LogPhysics, Log, "FConstraintInstance::CreatePxJoint_AssumesLocked - Invalid 6DOF joint (%ls)", *JointName.ToString());
		return false;
	}
...
FTransform FConstraintInstance::GetRefFrame(EConstraintFrame::Type Frame) const
{
	...
	if(Error > 0.01f)
	{
		UE_LOGF(LogPhysics, Warning,  "FConstraintInstance::GetRefFrame : Contained scale.");
	}
...
// Decomposes InRot into swing and twist about the X axis (the twist-axis convention used by Chaos
// joint constraints). Only the X and W components of InRot contribute to the twist; swing is the
// remainder. Both outputs are normalized. When InRot has no meaningful X component the twist is
// the identity and swing equals InRot.
static void DecomposeSwingTwist(const FQuat& InRot, FQuat& OutSwing, FQuat& OutTwist)
{
	if (FMath::Square(InRot.X) > 1e-12)
	{
		OutTwist = FQuat(InRot.X, 0.0, 0.0, InRot.W);
		OutTwist.Normalize();
	}
	else
	{
	...
	OutSwing = InRot * OutTwist.Inverse();
	OutSwing.Normalize();
}

// Clamps RotSwing in-place to (Swing1Limit, Swing2Limit) using axis-projection linear
// approximation: EffSwing1 = SwingAngle * axis.Z, EffSwing2 = SwingAngle * axis.Y. Each axis is
// clamped independently to match Chaos per-axis limit enforcement. Returns true if clamped.
static bool ClampSwingLinear(FQuat& RotSwing, double Swing1Limit, double Swing2Limit)
{
	if (RotSwing.W >= 1.0 - 1e-6)
	{
		return false;
	}
	...
	FVector SwingAxis;
	double  SwingAngle;
	RotSwing.ToAxisAndAngle(SwingAxis, SwingAngle);

	const double EffSwing1 = SwingAngle * SwingAxis.Z;
	const double EffSwing2 = SwingAngle * SwingAxis.Y;

	if (FMath::Abs(EffSwing1) <= Swing1Limit && FMath::Abs(EffSwing2) <= Swing2Limit)
	{
		return false;
	}

	const double S1_Clamped = FMath::Clamp(EffSwing1, -Swing1Limit, Swing1Limit);
	const double S2_Clamped = FMath::Clamp(EffSwing2, -Swing2Limit, Swing2Limit);
	const double NewAngle   = FMath::Sqrt(FMath::Square(S1_Clamped) + FMath::Square(S2_Clamped));
	RotSwing = NewAngle > 1e-6
		? FQuat(FVector(0.0, S2_Clamped / NewAngle, S1_Clamped / NewAngle), NewAngle)
		: FQuat::Identity;
	return true;
}

// Clamps RotSwing in-place to (Swing1Limit, Swing2Limit) using the exact Chaos swing-angle
// convention: Swing1 = 4*atan2(Z, 1+W), Swing2 = 4*atan2(Y, 1+W). Each axis is clamped
// independently. Reconstruction inverts the formula via a quadratic in W whose discriminant is
// always exactly 4, giving the closed form w = (1-T)/(1+T). Returns true if clamped.
static bool ClampSwingExact(FQuat& RotSwing, double Swing1Limit, double Swing2Limit)
{
	const double EffSwing1 = 4.0 * FMath::Atan2(RotSwing.Z, 1.0 + RotSwing.W);
	const double EffSwing2 = 4.0 * FMath::Atan2(RotSwing.Y, 1.0 + RotSwing.W);
	...
	const double S1_Clamped = FMath::Clamp(EffSwing1, -Swing1Limit, Swing1Limit);
	const double S2_Clamped = FMath::Clamp(EffSwing2, -Swing2Limit, Swing2Limit);
	const double t1    = FMath::Tan(S1_Clamped * 0.25);
	const double t2    = FMath::Tan(S2_Clamped * 0.25);
	const double T     = t1 * t1 + t2 * t2;
	const double W_New = (1.0 - T) / (1.0 + T);
	const double Scale = 1.0 + W_New;
	RotSwing = FQuat(0.0, t2 * Scale, t1 * Scale, W_New);
	RotSwing.Normalize();
	return true;
}

FQuat FConstraintInstance::ClampDriveTargetToLimits(const FQuat& TargetRelRot, bool bExactSwingClamp) const
{
	using namespace Chaos;

	const bool bTwistLimited  = GetAngularTwistMotion()  == EAngularConstraintMotion::ACM_Limited;
	const bool bSwing1Limited = GetAngularSwing1Motion() == EAngularConstraintMotion::ACM_Limited;
	const bool bSwing2Limited = GetAngularSwing2Motion() == EAngularConstraintMotion::ACM_Limited;

	if (!bTwistLimited && !bSwing1Limited && !bSwing2Limited)
	{
		return TargetRelRot;
	}

	// Transform TargetRelRot into constraint frame space.
	//
	// Rot_ChildFrame:  orientation of the constraint frame attached to the child body, expressed
	//                  in child-body-local space. Its X axis is the twist axis; Y and Z are the
	//                  swing axes. (UE calls this Frame1 internally.)
	// Rot_ParentFrame: orientation of the constraint frame attached to the parent body, expressed
	//                  in parent-body-local space. Defines the rest/zero-angle reference.
	//                  (UE calls this Frame2 internally.)
	// Rot_ChildRelParent: rotation of the child frame relative to the parent frame; the pure joint
	//                     angle with both frame offsets factored out. Limits are in this space.
	const FQuat Rot_ChildFrame  = GetRefFrame(EConstraintFrame::Frame1).GetRotation();
	const FQuat Rot_ParentFrame = GetRefFrame(EConstraintFrame::Frame2).GetRotation();
	FQuat Rot_ChildRelParent = Rot_ParentFrame.Inverse() * TargetRelRot * Rot_ChildFrame;
	Rot_ChildRelParent.Normalize();

	FQuat RotSwing_ChildRelParent, RotTwist_ChildRelParent;
	DecomposeSwingTwist(Rot_ChildRelParent, RotSwing_ChildRelParent, RotTwist_ChildRelParent);

	bool bClamped = false;

	if (bTwistLimited)
	{
		const double TwistLimitRad = FMath::DegreesToRadians((double)GetAngularTwistLimit());
		const double TwistAngle    = (double)FPBDJointUtilities::GetTwistAngle(FRotation3(RotTwist_ChildRelParent));
		if (FMath::Abs(TwistAngle) > TwistLimitRad)
		{
			RotTwist_ChildRelParent = FQuat(FVector::XAxisVector, FMath::Clamp(TwistAngle, -TwistLimitRad, TwistLimitRad));
			bClamped = true;
		}
	}
	...
	{
		const double Swing1Limit = bSwing1Limited
			? FMath::DegreesToRadians((double)GetAngularSwing1Limit()) : (double)UE_BIG_NUMBER;
		const double Swing2Limit = bSwing2Limited
			? FMath::DegreesToRadians((double)GetAngularSwing2Limit()) : (double)UE_BIG_NUMBER;

		bClamped |= bExactSwingClamp
			? ClampSwingExact(RotSwing_ChildRelParent, Swing1Limit, Swing2Limit)
			: ClampSwingLinear(RotSwing_ChildRelParent, Swing1Limit, Swing2Limit);
	}
	...
	FQuat RotClamped_ChildRelParent = RotSwing_ChildRelParent * RotTwist_ChildRelParent;
	RotClamped_ChildRelParent.Normalize();
	return Rot_ParentFrame * RotClamped_ChildRelParent * Rot_ChildFrame.Inverse();
}

bool FConstraintInstance::WidenLimitsForDriveTarget(
	...
	float                      WidenMarginDegrees)
{
	...
	const bool bTwistLimited  = DefaultConstraint.GetAngularTwistMotion()  == EAngularConstraintMotion::ACM_Limited;
	const bool bSwing1Limited = DefaultConstraint.GetAngularSwing1Motion() == EAngularConstraintMotion::ACM_Limited;
	const bool bSwing2Limited = DefaultConstraint.GetAngularSwing2Motion() == EAngularConstraintMotion::ACM_Limited;
	...
	// Transform TargetRelRot into constraint frame space (same decomposition as ClampDriveTargetToLimits).
	const FQuat Rot_ChildFrame  = GetRefFrame(EConstraintFrame::Frame1).GetRotation();
	const FQuat Rot_ParentFrame = GetRefFrame(EConstraintFrame::Frame2).GetRotation();
	FQuat Rot_ChildRelParent = Rot_ParentFrame.Inverse() * TargetRelRot * Rot_ChildFrame;
	Rot_ChildRelParent.Normalize();

	FQuat RotSwing, RotTwist;
	DecomposeSwingTwist(Rot_ChildRelParent, RotSwing, RotTwist);

	if (RotSwing.W < 0.0f)
	{
		RotSwing = FQuat(-RotSwing.X, -RotSwing.Y, -RotSwing.Z, -RotSwing.W);

		// Without this, joints whose drive target is near rest pose can have their limits blown open to
		// ~360 degrees instead of being widened by a small margin, causing the joint to become unconstrained.
		// Root cause: the Chaos angle formula 4*atan2(Z, 1+W) (see FPBDJointCachedSolver::InitRotationConstraintsSimd
		// in PBDJointCachedSolverGaussSeidel.cpp) assumes W >= 0. Both q and -q represent the same
		// rotation (quaternion double-cover), so normalizing to the positive hemisphere is safe and
		// prevents 1+W from approaching 0 (which would produce spurious ~360 degree angles).
	}
	...
	if (bTwistLimited)
	{
		const float DefaultDeg    = DefaultConstraint.GetAngularTwistLimit();
		const float TwistAngleDeg =
			FMath::Abs(FMath::RadiansToDegrees((float)FPBDJointUtilities::GetTwistAngle(FRotation3(RotTwist))));
		const float NeededDeg = TwistAngleDeg > DefaultDeg
			? TwistAngleDeg + WidenMarginDegrees : DefaultDeg;
		if (NeededDeg > DefaultDeg)
		{
			bWidened = true;
		}
		if (!FMath::IsNearlyEqual(GetAngularTwistLimit(), NeededDeg, 0.01f))
		{
			SetAngularTwistLimit(EAngularConstraintMotion::ACM_Limited, NeededDeg);
		}
	}

	if (bSwing1Limited || bSwing2Limited)
	{
		// Exact Chaos convention: Swing1 = 4*atan2(Z, 1+W), Swing2 = 4*atan2(Y, 1+W).
		const float Swing1AngleDeg =
			FMath::Abs(FMath::RadiansToDegrees((float)(4.0 * FMath::Atan2((double)RotSwing.Z, 1.0 + (double)RotSwing.W))));
		const float Swing2AngleDeg =
			FMath::Abs(FMath::RadiansToDegrees((float)(4.0 * FMath::Atan2((double)RotSwing.Y, 1.0 + (double)RotSwing.W))));

		if (bSwing1Limited)
		{
			const float DefaultDeg = DefaultConstraint.GetAngularSwing1Limit();
			const float NeededDeg  = Swing1AngleDeg > DefaultDeg
				? Swing1AngleDeg + WidenMarginDegrees : DefaultDeg;
			if (NeededDeg > DefaultDeg)
			{
				bWidened = true;
			}
			if (!FMath::IsNearlyEqual(GetAngularSwing1Limit(), NeededDeg, 0.01f))
			{
				SetAngularSwing1Limit(EAngularConstraintMotion::ACM_Limited, NeededDeg);
			}
		}

		if (bSwing2Limited)
		{
			const float DefaultDeg = DefaultConstraint.GetAngularSwing2Limit();
			const float NeededDeg  = Swing2AngleDeg > DefaultDeg
				? Swing2AngleDeg + WidenMarginDegrees : DefaultDeg;
			if (NeededDeg > DefaultDeg)
			{
				bWidened = true;
			}
			if (!FMath::IsNearlyEqual(GetAngularSwing2Limit(), NeededDeg, 0.01f))
			{
				SetAngularSwing2Limit(EAngularConstraintMotion::ACM_Limited, NeededDeg);
			}
		}
	}
...
void FConstraintInstance::RestoreAngularLimitsToDefault(const FConstraintInstance& DefaultConstraint)
{
	if (DefaultConstraint.GetAngularSwing1Motion() == EAngularConstraintMotion::ACM_Limited)
	{
		const float DefaultDeg = DefaultConstraint.GetAngularSwing1Limit();
		if (!FMath::IsNearlyEqual(GetAngularSwing1Limit(), DefaultDeg, 0.01f))
		{
			SetAngularSwing1Limit(EAngularConstraintMotion::ACM_Limited, DefaultDeg);
		}
	}
	if (DefaultConstraint.GetAngularSwing2Motion() == EAngularConstraintMotion::ACM_Limited)
	{
		const float DefaultDeg = DefaultConstraint.GetAngularSwing2Limit();
		if (!FMath::IsNearlyEqual(GetAngularSwing2Limit(), DefaultDeg, 0.01f))
		{
			SetAngularSwing2Limit(EAngularConstraintMotion::ACM_Limited, DefaultDeg);
		}
	}
	if (DefaultConstraint.GetAngularTwistMotion() == EAngularConstraintMotion::ACM_Limited)
	{
		const float DefaultDeg = DefaultConstraint.GetAngularTwistLimit();
		if (!FMath::IsNearlyEqual(GetAngularTwistLimit(), DefaultDeg, 0.01f))
		{
			SetAngularTwistLimit(EAngularConstraintMotion::ACM_Limited, DefaultDeg);
		}
	}
}
