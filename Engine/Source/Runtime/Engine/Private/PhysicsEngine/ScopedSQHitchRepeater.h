...
struct FScopedSQHitchRepeater
{
	...
	~FScopedSQHitchRepeater()
	{
		if (QueryCallback.bRecordHitches)
		{
			...
			UE_LOGF(LogCollision, Warning, "SceneQueryHitch: took %.3fms with %d calls to PreFilter", DurationInMS, QueryCallback.PreFilterHitchInfo.Num());
			UE_LOGF(LogCollision, Warning, "\t%ls", *HitchDetectionInfo.ToString());
			for (const FCollisionQueryFilterCallback::FPreFilterRecord& Record : QueryCallback.PreFilterHitchInfo)
			{
				UE_LOGF(LogCollision, Warning, "\tPreFilter:%ls, result=%d", *Record.OwnerComponentReadableName, (int32)Record.Result);
			}
