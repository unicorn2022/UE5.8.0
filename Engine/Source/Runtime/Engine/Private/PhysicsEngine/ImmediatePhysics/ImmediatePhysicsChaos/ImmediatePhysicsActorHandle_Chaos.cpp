...
namespace ImmediatePhysics_Chaos
{
		...
		FActorSetup&& ActorSetup)
	{
		...
		if (ParticleHandle != nullptr)
		{
			...
			CollisionData.bQueryCollision = 0;
	...
	void FActorHandle::SetUpdateKinematicFromSimulation(bool bUpdateKinematicFromSimulation)
	{
		using namespace Chaos;
		if (FPBDRigidParticleHandle* Rigid = Handle()->CastToRigidParticle())
		{
			Rigid->SetUpdateKinematicFromSimulation(bUpdateKinematicFromSimulation);
		}
	}

	bool FActorHandle::GetUpdateKinematicFromSimulation() const
	{
		using namespace Chaos;
		if (const FPBDRigidParticleHandle* Rigid = Handle()->CastToRigidParticle())
		{
	...
	void FActorHandle::AddForce(const FVector& Linear, EForceType ForceType)
	{
		using namespace Chaos;
		...
		if (FPBDRigidParticleHandle* Rigid = Handle()->CastToRigidParticle())
		{
			...
			case EForceType::AddForce:
				Rigid->AddForce(Linear);
				break;
			case EForceType::AddAcceleration:
				Rigid->Acceleration() += Linear;
				break;
			case EForceType::AddImpulse:
				Rigid->LinearImpulseVelocity() += Linear * Rigid->InvM();
				break;
			case EForceType::AddVelocity:
				Rigid->LinearImpulseVelocity() += Linear;
				break;
			default:
				break;
			}
	...
	// Calculates the force which, when applied at R (relative to the CoM), results in an acceleration Accel
	// at that location.
	static FVector CalculateForceForAccelAtPoint(
		Chaos::FPBDRigidParticleHandle* Rigid, const FVector& R, const FVector& Accel)
	{
		const double A = Accel.Length();
		if (A > UE_SMALL_NUMBER)
		{
			const FVector Dir = Accel / A;
			// This is the resulting component of linear acceleration in the direction dir that
			// would be achieved by unit force
			const double Response = FVector::DotProduct(
				Dir * Rigid->InvM() + FVector::CrossProduct(Chaos::FParticleUtilities::GetWorldInvInertia(Rigid) * 
					FVector::CrossProduct(R, Dir), R), 
				Dir);
			if (Response > UE_SMALL_NUMBER)
			{
				return Dir * (A / Response);
			}
		}
		return FVector::ZeroVector;
	}

	void FActorHandle::AddForceAtLocation(const FVector& Linear, const FVector& Location, EForceType ForceType)
	{
		using namespace Chaos;

		if (FPBDRigidParticleHandle* Rigid = Handle()->CastToRigidParticle())
		{
			const FVector CoM = FParticleUtilities::GetCoMWorldPosition(Rigid);
			const FVector LocationRelCoM = Location - CoM;
			switch (ForceType)
			{
			case EForceType::AddForce:
			{
				Rigid->AddForce(Linear);
				Rigid->AddTorque(FVector::CrossProduct(LocationRelCoM, Linear));
			}
			break;
			case EForceType::AddAcceleration:
			{
				const FVector DesiredForce = CalculateForceForAccelAtPoint(Rigid, LocationRelCoM, Linear);
				Rigid->AddForce(DesiredForce);
				Rigid->AddTorque(FVector::CrossProduct(LocationRelCoM, DesiredForce));
			}
			break;
			case EForceType::AddImpulse:
			{
				Rigid->LinearImpulseVelocity() += Linear * Rigid->InvM();
				Rigid->AngularImpulseVelocity() += FParticleUtilities::GetWorldInvInertia(Rigid) *
					FVector::CrossProduct(LocationRelCoM, Linear);
			}
			break;
			case EForceType::AddVelocity:
			{
				// The accel->force calculation is the same as velChange->impulse
				const FVector DesiredImpulse = CalculateForceForAccelAtPoint(Rigid, LocationRelCoM, Linear);
				Rigid->LinearImpulseVelocity() += DesiredImpulse * Rigid->InvM();
				Rigid->AngularImpulseVelocity() += FParticleUtilities::GetWorldInvInertia(Rigid) *
					FVector::CrossProduct(LocationRelCoM, DesiredImpulse);
			}
			break;
			default:
				break;
	...
	void FActorHandle::AddTorque(const FVector& Angular, EForceType ForceType)
	{
		using namespace Chaos;
		...
		if (FPBDRigidParticleHandle* Rigid = Handle()->CastToRigidParticle())
		{
			...
			{
			case EForceType::AddForce:
				Rigid->AddTorque(Angular);
				break;
			case EForceType::AddAcceleration:
				Rigid->AngularAcceleration() += Angular;
				break;
			case EForceType::AddImpulse:
				Rigid->AngularImpulseVelocity() += FParticleUtilities::GetWorldInvInertia(Rigid) * Angular;
				break;
			case EForceType::AddVelocity:
				Rigid->AngularImpulseVelocity() += Angular;
				break;
			default:
				break;
			}
	...
	void FActorHandle::SetCCDEnabled(bool bEnable)
	{
		using namespace Chaos;
		if (FPBDRigidParticleHandle* Dynamic = ParticleHandle->CastToRigidParticle())
		{
			Dynamic->SetCCDEnabled(bEnable);
		}
	}

	bool FActorHandle::GetCCDEnabled() const
	{
		using namespace Chaos;
		if (FPBDRigidParticleHandle* Dynamic = ParticleHandle->CastToRigidParticle())
		{
			return Dynamic->CCDEnabled();
		}
		return false;
	}
