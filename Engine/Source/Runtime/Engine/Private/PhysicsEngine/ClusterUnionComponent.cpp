...
UClusterUnionComponent::~UClusterUnionComponent()
{
	ensureMsgf(
		!PhysicsProxy,
		TEXT("ClusterUnionComponent %s destroyed while still attached to physics proxy. "
...
void UClusterUnionComponent::AddComponentToCluster(UPrimitiveComponent* InComponent, const TArray<int32>& BoneIds, bool bRebuildGeometry)
{
	...
	if (Objects.IsEmpty())
	{
		UE_LOGF(LogClusterUnion, Warning, "Trying to add a component [%p] with no physics objects to a cluster union...ignoring", InComponent)
		return;
	}
...
void UClusterUnionComponent::OnCreatePhysicsState()
{
	...
	// Cluster union skips UPrimitiveComponent::OnCreatePhysicsState (where the
	// PhysicsReplicationQuantization registration hook lives). Register here, after
	// PhysicsProxy is initialized. GetPhysicsObjectByName(NAME_None) returns null
	// until then so this must come last.
...
void UClusterUnionComponent::OnDestroyPhysicsState()
{
	...
	// Mirror the OnCreatePhysicsState registration. Safe no op for non replicating actors.
...
void UClusterUnionComponent::OnRep_RigidState()
{
	...
	// Also, if our state is 0 we have not yet receive a valid object state from
...
void UClusterUnionComponent::SetDisabled(const bool Disabled)
{
	if (PhysicsProxy)
	{
		PhysicsProxy->SetDisabled_External(Disabled);
	}
}

bool UClusterUnionComponent::GetDisabled() const
{
	if (PhysicsProxy)
	{
		return PhysicsProxy->GetDisabled_External();
	}

	return false;
}

void UClusterUnionComponent::SetGravityEnabled(const bool Enabled)
{
	if (PhysicsProxy)
	{
		PhysicsProxy->SetGravityEnabled_External(Enabled);
	}
}

bool UClusterUnionComponent::GetGravityEnabled() const
{
	if (PhysicsProxy)
	{
		return PhysicsProxy->GetGravityEnabled_External();
	}

	return false;
}

void UClusterUnionComponent::SetSleepType(const Chaos::ESleepType SleepType)
{
	if (PhysicsProxy)
	{
		PhysicsProxy->SetSleepType_External(SleepType);
	}
}

Chaos::ESleepType UClusterUnionComponent::GetSleepType() const
{
	if (PhysicsProxy)
	{
		return PhysicsProxy->GetSleepType_External();
	}

	return Chaos::ESleepType::MaterialSleep;
}
	
void UClusterUnionComponent::SetRigidState(const Chaos::EObjectStateType ObjectState)
{
	if (PhysicsProxy)
	{
...
Chaos::EObjectStateType UClusterUnionComponent::GetRigidState() const
{
	if (PhysicsProxy)
	{
		return PhysicsProxy->GetObjectState_External();
	}

	return Chaos::EObjectStateType::Uninitialized;
}
