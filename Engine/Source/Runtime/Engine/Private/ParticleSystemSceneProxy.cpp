...
{
	const FVector4 ObjectPostProjectionPositionWithW = View.ViewMatrices.GetWorldToClip().TransformPosition(DynamicData->SystemPositionForMacroUVs);
	...
	if (MacroUVOverride.bOverride)
	{
		...
		{
			UE_LOGF(LogParticles, Error, "MacroUVPosition.ContainsNaN()");
		}
#endif
	}
	...
	if (MacroUVRadius > 0.0f)
	{
		...
		const FVector4 RightPostProjectionPosition = View.ViewMatrices.GetWorldToClip().TransformPosition(MacroUVPosition + MacroUVRadius * View.ViewMatrices.GetTranslatedViewMatrix().GetColumn(0));
		const FVector4 UpPostProjectionPosition = View.ViewMatrices.GetWorldToClip().TransformPosition(MacroUVPosition + MacroUVRadius * View.ViewMatrices.GetTranslatedViewMatrix().GetColumn(1));
		...
		else
		{
			...
			{
				UE_LOGF(LogParticles, Error, "Bad values in FParticleSystemSceneProxy::GetObjectPositionAndScale");
				UE_LOGF(LogParticles, Error, "SystemPositionForMacroUVs: {%.6f, %.6f, %.6f}", DynamicData->SystemPositionForMacroUVs.X, DynamicData->SystemPositionForMacroUVs.Y, DynamicData->SystemPositionForMacroUVs.Z);
				UE_LOGF(LogParticles, Error, "ObjectPostProjectionPositionWithW: {%.6f, %.6f, %.6f, %.6f}", ObjectPostProjectionPositionWithW.X, ObjectPostProjectionPositionWithW.Y, ObjectPostProjectionPositionWithW.Z, ObjectPostProjectionPositionWithW.W);
				UE_LOGF(LogParticles, Error, "RightPostProjectionPosition: {%.6f, %.6f, %.6f, %.6f}", RightPostProjectionPosition.X, RightPostProjectionPosition.Y, RightPostProjectionPosition.Z, RightPostProjectionPosition.W);
				UE_LOGF(LogParticles, Error, "UpPostProjectionPosition: {%.6f, %.6f, %.6f, %.6f}", UpPostProjectionPosition.X, UpPostProjectionPosition.Y, UpPostProjectionPosition.Z, UpPostProjectionPosition.W);
				UE_LOGF(LogParticles, Error, "ObjectNDCPosition: {%.6f, %.6f}", ObjectNDCPosition.X, ObjectNDCPosition.Y);
				UE_LOGF(LogParticles, Error, "RightNDCPosX: %.6f", RightNDCPosX);
				UE_LOGF(LogParticles, Error, "UpNDCPosY: %.6f", UpNDCPosY);
				UE_LOGF(LogParticles, Error, "MacroUVPosition: {%.6f, %.6f, %.6f}", MacroUVPosition.X, MacroUVPosition.Y, MacroUVPosition.Z);
				UE_LOGF(LogParticles, Error, "MacroUVRadius: %.6f", MacroUVRadius);
				UE_LOGF(LogParticles, Error, "DX: %.6f", DX);
				UE_LOGF(LogParticles, Error, "DY: %.6f", DY);
				FVector4 View0 = View.ViewMatrices.GetWorldToView().GetColumn(0);
				FVector4 View1 = View.ViewMatrices.GetWorldToView().GetColumn(1);
				FVector4 View2 = View.ViewMatrices.GetWorldToView().GetColumn(2);
				FVector4 View3 = View.ViewMatrices.GetWorldToView().GetColumn(3);
				UE_LOGF(LogParticles, Error, "View0: {%.6f, %.6f, %.6f, %.6f}", View0.X, View0.Y, View0.Z, View0.W);
				UE_LOGF(LogParticles, Error, "View1: {%.6f, %.6f, %.6f, %.6f}", View1.X, View1.Y, View1.Z, View1.W);
				UE_LOGF(LogParticles, Error, "View2: {%.6f, %.6f, %.6f, %.6f}", View2.X, View2.Y, View2.Z, View2.W);
				UE_LOGF(LogParticles, Error, "View3: {%.6f, %.6f, %.6f, %.6f}", View3.X, View3.Y, View3.Z, View3.W);
				FVector4 ViewProj0 = View.ViewMatrices.GetWorldToClip().GetColumn(0);
				FVector4 ViewProj1 = View.ViewMatrices.GetWorldToClip().GetColumn(1);
				FVector4 ViewProj2 = View.ViewMatrices.GetWorldToClip().GetColumn(2);
				FVector4 ViewProj3 = View.ViewMatrices.GetWorldToClip().GetColumn(3);
				UE_LOGF(LogParticles, Error, "ViewProj0: {%.6f, %.6f, %.6f, %.6f}", ViewProj0.X, ViewProj0.Y, ViewProj0.Z, ViewProj0.W);
				UE_LOGF(LogParticles, Error, "ViewProj1: {%.6f, %.6f, %.6f, %.6f}", ViewProj1.X, ViewProj1.Y, ViewProj1.Z, ViewProj1.W);
				UE_LOGF(LogParticles, Error, "ViewProj2: {%.6f, %.6f, %.6f, %.6f}", ViewProj2.X, ViewProj2.Y, ViewProj2.Z, ViewProj2.W);
				UE_LOGF(LogParticles, Error, "ViewProj3: {%.6f, %.6f, %.6f, %.6f}", ViewProj3.X, ViewProj3.Y, ViewProj3.Z, ViewProj3.W);
			}
		}
	}
}
...
FPrimitiveSceneProxy* UParticleSystemComponent::CreateSceneProxy()
{
	...
	if ((IsActive() == true)/** && (EmitterInstances.Num() > 0)*/ && Template)
	{
		...
		if (!PSOPrecacheComponentData.IsPSOPrecacheCalled())
		{
			if (UPSOPrecacheSettingsManager::GetSettings().ParticlePrecachingTime == 1)
			{
		...
		{
			UE_LOGF(LogParticles, Verbose, "Skipping CreateSceneProxy for UParticleSystemComponent %ls (UParticleSystem PSOs are still compiling)", *GetFullName());
			return nullptr;
		}
		...
		UE_LOGF(LogParticles,Verbose,
			"CreateSceneProxy @ %fs %ls bIsActive=%d", GetWorld()->TimeSeconds,
