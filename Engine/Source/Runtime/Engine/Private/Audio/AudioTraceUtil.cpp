...
namespace Audio::Trace
{
	namespace Private
	{
		const FString MetaSoundSource = "MetaSoundSource";
		const FString SoundWaveProcedural = "SoundWaveProcedural";
		const FString SoundCue = "SoundCue";
		const FString SoundWave = "SoundWave";
		const FString SoundCueTemplate = "SoundCueTemplate";
	...
	namespace Util
	{
		FString GetAudioComponentName(const FActiveSound& InActiveSound)
		{
		...
		void SendActiveSoundEvent(const FActiveSound& InActiveSound, const FString& Event)
		{
			...
			if (bChannelEnabled && InActiveSound.AudioDevice)
			{
				...
				SendEvent(InActiveSound.AudioDevice->DeviceID
						...
						, Util::GetAudioComponentName(InActiveSound));
			}
		}

		void SendEvent(const Audio::FDeviceId AudioDeviceID, const FString& Event, const uint32 AudioObjectID, const FString& AssetPath, const FString& ActorLabel, const FString& ActorIconName, const FString& SoundClassName, const FString& AudioComponentName)
		{
			const bool bChannelEnabled = UE_TRACE_CHANNELEXPR_IS_ENABLED(AudioChannel);

			if (bChannelEnabled)
			{
				UE_TRACE_LOG(Audio, EventLog, AudioChannel)
					...
					<< EventLog.AudioComponentName(*AudioComponentName);
			}
		}
	} // namespace EventLog
