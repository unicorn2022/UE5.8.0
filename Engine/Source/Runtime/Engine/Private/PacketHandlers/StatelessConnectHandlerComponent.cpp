...
namespace UE::Net
{
	...
	/** Initial value of LastClientSendTimestamp, indicating the client hasn't sent anything */
	static constexpr double UninitializedLastClientSendTimestamp = -1.0;
...
StatelessConnectHandlerComponent::StatelessConnectHandlerComponent()
	: HandlerComponent(FName("StatelessConnectHandlerComponent"))
	, Driver(nullptr)
	, HandshakeSecret()
	, ActiveSecret(255)
	...
	, LastClientSendTimestamp(UE::Net::UninitializedLastClientSendTimestamp)
	...
	, LastRemoteHandshakeVersion(static_cast<EHandshakeVersion>(UE::Net::CurrentHandshakeVersion))
{
	...
	if (!MagicHeaderStr.IsEmpty())
	{
		...
		if (HeaderStrLen <= 32)
		{
			...
			{
				UE_LOGF(LogHandshake, Error, "CVar net.MagicHeader must be a binary string, containing only 1's and 0's, e.g.: 00010101. Current string: %ls", *MagicHeaderStr);
		...
		{
			UE_LOGF(LogHandshake, Error, "CVar net.MagicHeader is too long (%i), maximum size is 32 bits: %ls", MagicHeaderStr.Len(), *MagicHeaderStr);
		}
	}
}
...
void StatelessConnectHandlerComponent::SendInitialPacket(EHandshakeVersion HandshakeVersion)
{
	...
	if (Handler->Mode == UE::Handler::Mode::Client)
	{
		...
		{
			UE_LOGF(LogHandshake, Error, "Tried to send handshake connect packet without a server connection.");
		}
	}
}
...
void StatelessConnectHandlerComponent::SendConnectChallenge(FCommonSendToClientParams CommonParams, uint8 ClientSentHandshakePacketCount)
{
	...
	if (Driver != nullptr)
	{
		...
		UE_CLOGF((DDoS == nullptr || !DDoS->CheckLogRestrictions()), LogHandshake, Log,
				"SendConnectChallenge. Timestamp: %f, Cookie: %ls", Timestamp, *FString::FromBlob(Cookie, UE_ARRAY_COUNT(Cookie)));
#endif
																...
																uint8 InCookie[COOKIE_BYTE_SIZE])
{
...
#if !UE_BUILD_SHIPPING
		UE_LOGF(LogHandshake, Log, "SendChallengeResponse. Timestamp: %f, Cookie: %ls", InTimestamp,
	...
	{
		UE_LOGF(LogHandshake, Error, "Tried to send handshake response packet without a server connection.");
	}
}
...
#if !UE_BUILD_SHIPPING
		UE_LOGF(LogHandshake, Log, "SendChallengeAck. InCookie: %ls", *FString::FromBlob(InCookie, COOKIE_BYTE_SIZE));
#endif
...
void StatelessConnectHandlerComponent::SendRestartHandshakeRequest(FCommonSendToClientParams CommonParams)
{
	...
	if (Driver != nullptr)
	{
		...
		UE_CLOGF((DDoS == nullptr || !DDoS->CheckLogRestrictions()), LogHandshake, Verbose, "SendRestartHandshakeRequest.");
#endif
...
void StatelessConnectHandlerComponent::SendToServer(EHandshakeVersion HandshakeVersion, EHandshakePacketType PacketType, FBitWriter& Packet)
{
	if (UNetConnection* ServerConn = (Driver != nullptr ? Driver->ServerConnection.Get() : nullptr))
	{
		...
		if (bRandFail)
		{
			UE_LOGF(LogHandshake, Log, "Triggering random '%ls' packet fail.", ToCStr(LexToString(PacketType)));
		}
...
#endif
		{
			if (Driver->IsNetResourceValid())
			{
				...
				LastClientSendTimestamp = Driver->GetElapsedTime();
			}
			else
			{
				// ignore resent interval, send initial packet on "next frame" as the socket will be ready by then
				LastClientSendTimestamp = Driver->GetElapsedTime() - UE::Net::HandshakeResendInterval + 0.01f;
			}
		}
...
void StatelessConnectHandlerComponent::SetDriver(UNetDriver* InDriver)
{
	...
	else //if (Handler->Mode == Handler::Mode::Client)
	{
		...
		UE_LOGF(LogHandshake, Log, "Stateless Handshake: NetDriverDefinition '%ls' CachedClientID: %u", ToCStr(NetDriverDef),
				CachedClientID);
	}
}
...
void StatelessConnectHandlerComponent::Initialize()
{
	...
	if (Handler->Mode == UE::Handler::Mode::Server)
	{
		...
		{
			UE_LOGF(LogHandshake, Error, "net.CurrentHandshakeVersion value '%i' is invalid. Maximum value: %i", CurrentHandshakeVersion,
		...
		else
		{
			...
			{
				UE_LOGF(LogHandshake, Error, "net.MinHandshakeVersion value '%i' is invalid relative to net.CurrentHandshakeVersion value '%i'. "
					"Minimum value: %u, Maximum value: %u", MinSupportedHandshakeVersion, CurrentHandshakeVersion,
...
bool StatelessConnectHandlerComponent::SupportsParallelConnectionOutgoing() const
{
	return true;
}
...
void StatelessConnectHandlerComponent::Incoming(FBitReader& Packet)
{
		...
		if (bHandshakePacket)
		{
			...
			if (Handler->Mode == UE::Handler::Mode::Client && bHasValidClientID && (bHasValidSessionID || bIsInitialChallengePacket || bIsUpgradePacket))
			{
				if (State == UE::Handler::Component::State::UnInitialized || State == UE::Handler::Component::State::InitializedOnLocal)
				{
					...
					{
#if !UE_BUILD_SHIPPING
						UE_LOGF(LogHandshake, Log, "Ignoring restart handshake request, while already restarted.");
#endif
					}
					// Receiving challenge
					...
					{
#if !UE_BUILD_SHIPPING
						UE_LOGF(LogHandshake, Log, "Cached server SessionID: %u", SessionID);
#endif
					...
					else if (bIsUpgradePacket)
					{
						...
						UE_LOGF(LogHandshake, Log, "Server is running an incompatible version of the game, and has rejected the connection. "
							"Server version '%u', Local version '%u', Server features '%ls', Local features '%ls'.",
				...
				else if (HandshakeData.bRestartHandshake)
				{
					...
					if (ensure(bValidAuthCookie))
					{
						...
						if (!bRestartedHandshake)
						{
							...
							const double DriverElapsedTime = (Driver != nullptr ? Driver->GetElapsedTime() : 0.0);

							// The server may send multiple restart handshake packets, so have a 10 second delay between accepting them
							bPassedDelayCheck = (DriverElapsedTime - LastClientSendTimestamp) > 10.0;
						...
						{
							UE_LOGF(LogHandshake, Log, "Beginning restart handshake process.");
						...
						else if (WithinHandshakeLogLimit())
						{
							if (bRestartedHandshake)
							{
								UE_LOGF(LogHandshake, Log, "Ignoring restart handshake request, while already restarted (this is normal).");
							}
#if !UE_BUILD_SHIPPING
							else if (!bPassedDelayCheck)
							{
								UE_LOGF(LogHandshake, Log, "Ignoring restart handshake request, due to < 10 seconds since last handshake.");
							}
							else // if (!bPassedDualIPCheck)
							{
								UE_LOGF(LogHandshake, Log, "Ignoring restart handshake request, due to recent NetConnection packets.");
							}
#endif
						}
					}
					...
					{
						UE_LOGF(LogHandshake, Log, "Server sent restart handshake request, when we don't have an authorised cookie.");
...
#if !UE_BUILD_SHIPPING
					UE_LOGF(LogHandshake, Log, "Received unexpected post-connect handshake packet - resending ack for LastChallengeSuccessAddress %ls and LastCookie %ls.",
			...
			{
#if !UE_BUILD_SHIPPING
				UE_CLOGF(TrackValidationLogs(), LogHandshake, Log,
						"Incoming: Rejecting handshake packet with invalid session id (%u vs %u) or connection id (%u vs %u).",
...
#if !UE_BUILD_SHIPPING
			UE_LOGF(LogHandshake, Log, "Incoming: Error reading handshake packet.");
#endif
		}
	}
#if !UE_BUILD_SHIPPING
	...
	{
		UE_LOGF(LogHandshake, Log, "Incoming: Error reading session id, connection id and handshake bit from packet.");
	}
#endif
	else if (!bHasValidSessionID || !bHasValidClientID)
	{
#if !UE_BUILD_SHIPPING
		UE_CLOGF(TrackValidationLogs(), LogHandshake, Log,
				"Incoming: Rejecting game packet with invalid session id (%u vs %u) or connection id (%u vs %u).",
				SessionID, CachedGlobalNetTravelCount, ClientID, CachedClientID);
#endif
...
void StatelessConnectHandlerComponent::IncomingConnectionless(FIncomingPacketRef PacketRef)
{
	...
	if (MagicHeader.Num() > 0)
	{
		...
		{
#if !UE_BUILD_SHIPPING
			UE_CLOGF(TrackValidationLogs(), LogNet, Log, "Rejecting packet with invalid magic header '%08X' vs '%08X' (%i bits)",
	...
	if (!bHandshakePacket)
	{
		...
		{
#if !UE_BUILD_SHIPPING
			UE_LOGF(LogHandshake, Log, "IncomingConnectionless: Error reading session id, connection id and handshake bit from packet.");
#endif
		}
...
#if !UE_BUILD_SHIPPING
		UE_CLOGF(DDoS == nullptr || !DDoS->CheckLogRestrictions(), LogHandshake, Log,
			"IncomingConnectionless: Error reading handshake packet.");
#endif
		return;
	}
	...
	if (!bIsValidRequest)
	{
		...
		{
#if !UE_BUILD_SHIPPING
			UE_LOGF(LogHandshake, Log, "IncomingConnectionless: Rejecting packet with invalid session id: %u vs %u.", SessionID, CachedGlobalNetTravelCount);
#endif
			Packet.SetError();
		}
...
bool StatelessConnectHandlerComponent::CheckVersion(const FParsedHandshakeData& HandshakeData, EHandshakeVersion& OutTargetVersion) const
{
	...
	if (!bValidHandshakeVersion || !bValidNetCLVersion || !bIsNetFeatureCompatible)
	{
		...
		UE_CLOGF((DDoS == nullptr || !DDoS->CheckLogRestrictions()), LogHandshake, Log,
				"CheckVersion: Incompatible version. bValidHandshakeVersion: %i, bValidNetVersion: %i, " "GHandshakeEnforceNetworkCLVersion: %i, RemoteMinVersion: %u, RemoteCurVersion: %u, MinSupportedHandshakeVersion: %i, " "CurrentHandshakeVersion: %i, RemoteNetworkVersion: %u, LocalNetworkVersion: %u, RemoteNetworkFeatures: %ls, " "LocalNetworkFeatures: %ls",
...
void StatelessConnectHandlerComponent::Tick(float DeltaTime)
{
	...
	if (Handler->Mode == UE::Handler::Mode::Client)
	{
		if (State != UE::Handler::Component::State::Initialized && LastClientSendTimestamp != UninitializedLastClientSendTimestamp)
		{
			ensureMsgf(Driver, TEXT("StatelessConnectHandlerComponent::Tick: Driver is null."));
			const double CurrentTime = Driver ? Driver->GetElapsedTime() : 0.0;

			const double LastSendTimeDiff = CurrentTime - LastClientSendTimestamp;
			...
			if (LastSendTimeDiff > UE::Net::HandshakeResendInterval)
			{
				...
				{
					UE_LOGF(LogHandshake, Verbose, "Initial handshake packet timeout - resending.");
				...
				{
					UE_LOGF(LogHandshake, Verbose, "Challenge response packet timeout - resending.");
