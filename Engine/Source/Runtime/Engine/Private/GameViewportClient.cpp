...
/** Delegate called at the end of the frame when a non-HDR screenshot is captured */
FOnScreenshotCaptured UGameViewportClient::ScreenshotCapturedDelegate;

/** Delegate called at the end of the frame when an HDR screenshot is captured */
FOnHDRScreenshotCaptured UGameViewportClient::HDRScreenshotCapturedDelegate;
...
#endif
{
	...
	SplitscreenInfo[ESplitScreenType::ThreePlayer_Grid].PlayerData.Add(FPerPlayerSplitscreenData(0.5f, 0.5f, 0.0f, 0.0f));
	SplitscreenInfo[ESplitScreenType::ThreePlayer_Grid].PlayerData.Add(FPerPlayerSplitscreenData(0.5f, 0.5f, 0.5f, 0.0f));
	SplitscreenInfo[ESplitScreenType::ThreePlayer_Grid].PlayerData.Add(FPerPlayerSplitscreenData(0.5f, 0.5f, 0.0f, 0.5f));
	...
	if (HasAnyFlags(RF_ClassDefaultObject) == false)
	{
		...
		StatThermalsData = new FStatThermalsData();
		...
		{
			FSlateApplication::Get().OnRegularWindowMovedOrResized().AddUObject(this, &UGameViewportClient::HandleRegularWindowMovedOrResized);
#if WITH_EDITOR
			if (GIsEditor)
			{
...
UGameViewportClient::~UGameViewportClient()
{
	...
	if (FSlateApplication::IsInitialized())
	{
		FSlateApplication::Get().OnRegularWindowMovedOrResized().RemoveAll(this);
#if WITH_EDITOR
		if (GIsEditor)
		{
	...
	if (StatThermalsData)
	{
		delete StatThermalsData;
		StatThermalsData = NULL;
	}
}
...
TSharedRef<FSceneViewport> UGameViewportClient::CreateViewport(TSharedPtr<SViewport> InViewportWidget)
{
	return FSceneViewport::Create(this, InViewportWidget);
}

FSceneViewport* UGameViewportClient::CreateGameViewport(TSharedPtr<SViewport> InViewportWidget)
{
	FSceneViewport* Result = new FSceneViewport(InViewportWidget);
	Result->SetViewportClient(this);
	return Result;
}
...
void UGameViewportClient::SetEnabledStats(const TArray<FString>& InEnabledStats)
{
	...
	{
		UE_LOGF(LogPlayerManagement, Warning, "WARNING: Stats disabled for non multi-threading platforms");
	}
...
void UGameViewportClient::Init(struct FWorldContext& WorldContext, UGameInstance* OwningGameInstance, bool bCreateNewAudioDevice)
{
	...
	if (FParse::Value(FCommandLine::Get(), TEXT("DefaultViewportMouseCaptureMode="), DefaultViewportMouseCaptureMode))
	{
		...
		if (EnumPtr)
		{
			...
			{
				UE_LOGF(LogInit, Warning, "Unknown DefaultViewportMouseCaptureMode %ls. Command line setting will be ignored.", *DefaultViewportMouseCaptureMode);
			}
		}
	}
...
bool UGameViewportClient::InputChar(FViewport* InViewport, int32 ControllerId, TCHAR Character)
{
	...
	if (!bResult)
	{
		bResult = Super::InputChar(InViewport, ControllerId, Character);
	}
	...
	const uint64 Timestamp)
{
	...
	// route to subsystems that care
	bool bResult = (ViewportConsole ? ViewportConsole->InputTouch(TouchId, Type, TouchLocation, Force, Timestamp) : false);
	if (!bResult)
	{
		ULocalPlayer* const TargetPlayer = GEngine->GetLocalPlayerFromInputDevice(this, TouchId.GetDeviceId());
		if (TargetPlayer && TargetPlayer->PlayerController)
		{
...
bool UGameViewportClient::InputGesture(FViewport* InViewport, const FInputDeviceId DeviceId, EGestureEvent GestureType, EGesturePhase GesturePhase, const FVector2D& GestureDelta, bool bIsDirectionInvertedFromDevice, const uint64 Timestamp)
{
	if (IgnoreInput())
	{
		return false;
	...
	if (TargetPlayer && TargetPlayer->PlayerController)
	{
		bResult = TargetPlayer->PlayerController->InputGesture(DeviceId, GestureType, GesturePhase, GestureDelta, bIsDirectionInvertedFromDevice, Timestamp);
	}
...
{
	TSharedPtr<SWindow> PinnedWindow = GetWindow();
...
TOptional<TSharedRef<SWidget>> UGameViewportClient::MapCursor(FViewport* InViewport, const FCursorReply& CursorReply)
{
	if (bUseSoftwareCursorWidgets)
	{
		if (CursorReply.GetCursorType() != EMouseCursor::None)
		{
			...
			{
				UE_LOGF(LogPlayerManagement, Warning, "UGameViewportClient::MapCursor: Could not find cursor to map to %d.", int32(CursorReply.GetCursorType()));
			}
		}
	}
...
void UGameViewportClient::SetViewport( FSceneViewport* InViewport )
{
	FSceneViewport* PreviousViewport = Viewport;
	Viewport = InViewport;
...
void UGameViewportClient::AddAssociation(FViewport& InViewport)
{
	FSceneViewport* SceneViewport = InViewport.AsSceneViewport();
	if (!SceneViewport || Viewport == SceneViewport)
	{
		return;
	}
	...
	if (Viewport)
	{
		// "Primary" viewport is first-come first-served
		return;
	}
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	SetViewport(SceneViewport); // To be inlined here after deprecation period
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	GameLayerManager->SetSceneViewport(SceneViewport);

	if (TSharedPtr<SViewport> ViewportWidget = SceneViewport->GetViewportWidget().Pin())
	{
		// If the viewport we're drawing to has an overlay as its child, insert instead of replacing (licensee code should not assume this behavior)
		if (TSharedPtr<SWidget> Child = ViewportWidget->GetContent())
		{
			if (Child->GetWidgetClass().GetWidgetType() == SOverlay::StaticWidgetClass().GetWidgetType())
			{
				StaticCastSharedPtr<SOverlay>(Child)->AddSlot(0)
					[
						GameLayerManager->AsWidget()
					];
				return;
			}
		}
		ViewportWidget->SetContent(GameLayerManager->AsWidget());
	}
}

void UGameViewportClient::RemoveAssociation(FViewport& InViewport)
{
	...
	FSceneViewport* SceneViewport = InViewport.AsSceneViewport();
	if (SceneViewport && GameLayerManager.IsValid())
	{
		TSharedPtr<SViewport> SlateViewport = SceneViewport->GetViewportWidget().Pin();
		if (SlateViewport.IsValid())
		{
			if (SlateViewport->GetContent() == GameLayerManager->AsWidget())
			{
				SlateViewport->SetContent(SNullWidget::NullWidget);
			}
			else if (SlateViewport->GetContent().IsValid() && SlateViewport->GetContent()->GetWidgetClass().GetWidgetType() == SOverlay::StaticWidgetClass().GetWidgetType())
			{
				TSharedPtr<SOverlay> Overlay = StaticCastSharedPtr<SOverlay>(SlateViewport->GetContent());
				Overlay->RemoveSlot(GameLayerManager->AsWidget());
			}
		}
	...
	// Without viewport cannot receive MouseLeave to undo the set-to-true from MouseEnter 
...
TSharedPtr<SWindow> UGameViewportClient::GetWindow() const
{
	...
	if (Viewport)
	{
		if (TSharedPtr<SViewport> ViewportWidget = Viewport->GetViewportWidget().Pin())
		{
			TSharedPtr<SWindow> Found = FSlateApplication::Get().FindWidgetWindow(ViewportWidget.ToSharedRef());
			Window = Found.ToWeakPtr();
			return Found;
		}
	}
	return nullptr;
...
void UGameViewportClient::Draw(FViewport* InViewport, FCanvas* SceneCanvas)
{
	...
	static FName CanvasObjectName("CanvasObject");
	...
	static FName DebugCanvasObjectName("DebugCanvasObject");
	...
	if (ViewFamily.EngineShowFlags.VisualizeBuffer && AllowDebugViewmodes())
	{
		...
		if (ICVar)
		{
			static const FName OverviewName = "Overview";
			...
			else
			{
				if (GetBufferVisualizationData().GetMaterial(ModeName) == nullptr)
				{
					...
					UE_LOGF(LogConsoleResponse, Warning, "Buffer visualization mode '%ls' does not exist", *ModeNameString);
	...
	for (FLocalPlayerIterator Iterator(GEngine, MyWorld); Iterator; ++Iterator)
	{
		...
		if (LocalPlayer && LocalPlayer->GetViewportClient() == this)
		{
			...
			if (PlayerController)
			{
				...
				{
					bAnyPlayerCameraCut = bAnyPlayerCameraCut || CameraManager->bGameCameraCutThisFrame;
					// Assumes any other systems that call ULocalPlayer::CalcSceneView(...) and care about camera cuts will do so before this happens.
		...
		if (bStereoRendering)
		{
			...
			if (bUsingRawOutput)
			{
				for (FSceneView* View : Views)
				{
					...
					{
						UE_LOGF(LogViewport, Warning, "Temporal upscaling is unsupported when redirecting primary scaling to the HMD runtime. The HMD runtime's spatial scaling will be used instead.");
						bWarnOnce = false;
					}
	...
	if (!bDisableWorldRendering && PlayerViewMap.Num() > 0 && FSlateApplication::Get().GetPlatformApplication()->IsAllowedToRender())
	{
	...
	if (!bBufferCleared)
	{
		const FLinearColor ClearColor = bTransparentViewportClear ? FLinearColor::Transparent : FLinearColor::Black;

		// clear left
		if( MinX > 0 )
		{
			SceneCanvas->DrawTile(0,0,MinX,InViewport->GetSizeXY().Y,0.0f,0.0f,1.0f,1.f,ClearColor,NULL,false);
		}
		// clear right
		if( MaxX < (uint32)InViewport->GetSizeXY().X )
		{
			SceneCanvas->DrawTile(MaxX,0,InViewport->GetSizeXY().X,InViewport->GetSizeXY().Y,0.0f,0.0f,1.0f,1.f,ClearColor,NULL,false);
		}
		// clear top
		if( MinY > 0 )
		{
			SceneCanvas->DrawTile(MinX,0,MaxX,MinY,0.0f,0.0f,1.0f,1.f,ClearColor,NULL,false);
		}
		// clear bottom
		if( MaxY < (uint32)InViewport->GetSizeXY().Y )
		{
			SceneCanvas->DrawTile(MinX,MaxY,MaxX,InViewport->GetSizeXY().Y,0.0f,0.0f,1.0f,1.f,ClearColor,NULL,false);
		}
	}
	...
	if (IStateStreamManager* Manager = MyWorld->GetStateStreamManager())
	{
		...
		public:
			...
			virtual void DrawText(const FStringView& Text, const FColor& Color) override
			{
			...
			virtual void DrawText(uint32 InX, uint32 InY, const FStringView& Text, const FColor& Color) override
			{
				Canvas->SetDrawColor(Color);
				Canvas->DrawText(GEngine->GetSmallFont(), Text, InX, InY);
			}
		...
		Manager->Game_DebugRender(StateStreamDebugRenderer, 0);
	}
#endif
...
bool UGameViewportClient::ProcessScreenShots(FViewport* InViewport)
{
	...
	if (GIsDumpingMovie || FScreenshotRequest::IsScreenshotRequested() || GIsHighResScreenshot)
	{
		...
		if( bShowUI && FSlateApplication::IsInitialized() )
		{
			...
			if (FScreenshotRequest::ShouldRestrictToGameViewport())
			{
				TSharedPtr<SViewport> GameViewportWidgetPtr = GetGameViewportWidget();
				if (GameViewportWidgetPtr.IsValid())
				{
		...
		if (bScreenshotSuccessful)
		{
			...
			{
				if (HDRScreenshotCapturedDelegate.IsBound() && CVarScreenshotDelegate.GetValueOnGameThread())
				{
					// Ensure that all pixels' alpha is set to 1.0f
					for (FLinearColor& LinearColor : BitmapHDR)
					{
						LinearColor.A = 1.0f;
					}

					// If delegate subscribed, fire it instead of writing out a file to disk
					HDRScreenshotCapturedDelegate.Broadcast(Size.X, Size.Y, BitmapHDR);
				}
				else
				{
...
void UGameViewportClient::Precache()
{
	if(!GIsEditor)
	{
		...
		if (AudioDevice)
		{
			UE_LOGF(LogPlayerManagement, Log, "Precaching sounds...");
			for(TObjectIterator<USoundWave> It;It;++It)
			{
				USoundWave* SoundWave = *It;
				AudioDevice->Precache( SoundWave, false, false, false );
			}
			UE_LOGF(LogPlayerManagement, Log, "Precaching sounds completed...");
		}
	}
	...
	{
		UE_LOGF(LogPlayerManagement, Log, "%5.2f seconds passed since startup.",FPlatformTime::Seconds()-GStartTime);
...
void UGameViewportClient::CloseRequested(FViewport* InViewport)
{
	...
	// Historically, "CloseRequested" means "reset the Viewport reference to null as a signal for the game to shut down"
	if (InViewport)
	{
		RemoveAssociation(*InViewport);
	}
...
{
	UE_LOGF(LogNet, Warning, "Travel Failure: [%ls]: %ls", ETravelFailure::ToString(FailureType), *ErrorString);
}
...
void UGameViewportClient::PeekNetworkFailureMessages(UWorld *InWorld, UNetDriver *NetDriver, ENetworkFailure::Type FailureType, const FString& ErrorString)
{
	...
	{
		UE_LOGF(LogNet, Warning, "Network Failure: %ls[%ls]: %ls", NetDriver ? *NetDriver->NetDriverName.ToString() : TEXT("NULL"), ENetworkFailure::ToString(FailureType), *ErrorString);
...
void UGameViewportClient::UpdateActiveSplitscreenType()
{
	...
	int32 NumPlayers = 0;
	for (ULocalPlayer* LocalPlayer : GEngine->GetGamePlayers(this))
	{
		if (LocalPlayer && LocalPlayer->GetViewportClient() == this)
		{
			++NumPlayers;
	...
	const bool bAllowSplitScreen = !bDisableSplitScreenOverride;

	if (Settings->bUseSplitscreen && bAllowSplitScreen)
	{
		switch (NumPlayers)
		{
		case 0:
		...
		case 3:
			if (bEnablePlayersSplitRT)
			{
				SplitType = ESplitScreenType::ThreePlayer_Grid;
			}
			else
			{
...
void UGameViewportClient::LayoutPlayers()
{
	...
	{
		if (!PlayerList[PlayerIdx] || PlayerList[PlayerIdx]->GetViewportClient() != this)
		{
			continue;
		}
...
{
	if (ensure(ViewportOverlayWidget.IsValid()))
	{
...
void UGameViewportClient::SetMouseCaptureMode(EMouseCaptureMode Mode)
{
	...
	{
		UE_LOGF(LogViewport, Display, "Viewport MouseCaptureMode Changed, %ls -> %ls",
...
void UGameViewportClient::SetMouseLockMode(EMouseLockMode InMouseLockMode)
{
	...
	{
		UE_LOGF(LogViewport, Display, "Viewport MouseLockMode Changed, %ls -> %ls",
...
void UGameViewportClient::SetHideCursorDuringCapture(bool InHideCursorDuringCapture)
{
	...
	{
		UE_LOGF(LogViewport, Display, "Viewport HideCursorDuringCapture Changed, %ls -> %ls",
	...
	if (EngineShowFlags.Collision)
	{
		for (FLocalPlayerIterator It((UEngine*)GetOuter(), World); It; ++It)
		{
			...
			if (PC != NULL && PC->GetPawn() != NULL)
			{
				...
				if (const UObject* MovementBaseObject = PC->GetPawn()->GetMovementBaseObject())
				{
					PC->ClientMessage(FString::Printf(TEXT("Base %s"), *MovementBaseObject->GetName()));
				}
...
{
	if constexpr (FPlatformProperties::SupportsWindowedMode())
	{
		if (!Viewport)
		{
			return true;
		...
		check(CVar);
		EWindowMode::Type FullScreenMode = Viewport->IsFullscreen()
			? EWindowMode::Windowed
			: (((CVar->GetValueOnGameThread() == 0) && Viewport->IsExclusiveFullscreenSupported())
				? EWindowMode::Fullscreen
				: EWindowMode::WindowedFullscreen);

		int32 ResolutionX = GSystemResolution.ResX;
		int32 ResolutionY = GSystemResolution.ResY;
		const int32 MonitorIndex = Viewport->GetCurrentMonitorIndex();
		...
		{
			UGameUserSettings* const UserSettings = GameEngine->GetGameUserSettings();
			if (UserSettings != nullptr)
			{
				...
				if (DisplayMetrics.MonitorInfo.IsValidIndex(MonitorIndex))
				{
					...
					{
						ResolutionX = Monitor.MaxResolution.X;
						ResolutionY = Monitor.MaxResolution.Y;
						if ((ResolutionX == 0) || (ResolutionY == 0))
						{
							ResolutionX = Monitor.NativeWidth;
							ResolutionY = Monitor.NativeHeight;
						}
					}
					else
					{
						const FPlatformRect& DisplayRect = Monitor.DisplayRect;
						ResolutionX = DisplayRect.Right - DisplayRect.Left;
						ResolutionY = DisplayRect.Bottom - DisplayRect.Top;
					}

					UserSettings->SetDisplayProperties(Monitor.ID, MonitorIndex);
				}

				// Ensure that our desired screen size will fit on the display
				UGameEngine::DetermineGameWindowResolution(ResolutionX, ResolutionY, FullScreenMode, true, MonitorIndex);
		...
		if (!bNewModeApplied)
		{
			FSystemResolution::SetMonitorIndex(MonitorIndex);
...
void UGameViewportClient::HandleRegularWindowMovedOrResized(TSharedRef<SWindow> InWindow)
{
	if ((InWindow == GetWindow()) && Viewport)
	{
		UGameEngine* const GameEngine = Cast<UGameEngine>(GEngine);
		if (GameEngine)
		{
			...
			if (UserSettings != nullptr)
			{
				...
				if (DisplayMetrics.MonitorInfo.IsValidIndex(MonitorIndex))
				{
					const FString& DisplayID = DisplayMetrics.MonitorInfo[MonitorIndex].ID;
					if (DisplayID != UserSettings->GetDisplayID())
					{
						UserSettings->SetDisplayProperties(DisplayID, MonitorIndex);

						WindowDisplayChangedDelegate.Broadcast();
					}
				}
			}
		}
...
bool UGameViewportClient::SetHardwareCursor(EMouseCursor::Type CursorShape, FName GameContentPath, FVector2D HotSpot)
{
	...
	if ( !HardwareCursor )
	{
		...
		if ( !HardwareCursor )
		{
			...
			if ( !HardwareCursor )
			{
				UE_LOGF(LogInit, Error, "Failed to load cursor '%ls'", *CursorPath);
				return false;
			}
		}
...
bool UGameViewportClient::LoadAvailableCursorPngs(TArray< TSharedPtr<FPngFileData> >& Results, const FString& InPathToCursorWithoutExtension)
{
	...
	for (const FString& FullCursorPath : PngCursorFiles)
	{
		...
		{
			UE_LOGF(LogInit, Error, "Failed to load cursor '%ls', non-numeric characters in the scale factor.", *FullCursorPath);
			continue;
		}
		...
		{
			UE_LOGF(LogInit, Log, "Loading Cursor '%ls'.", *FullCursorPath);
		}
