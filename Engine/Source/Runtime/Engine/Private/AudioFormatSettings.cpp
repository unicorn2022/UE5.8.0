...
namespace Audio
{
	static bool ShouldAllowHardwareFormats()
	{
		...
		static bool IsAudioLinkEnabled = []() -> bool
		{
			...
			UE_CLOGF(bAvailable,LogAudio, Display, "AudioLink is enabled, disabling hardware AudioFormats.");
			return bAvailable;
		}();
	...
	{		
		if (const IAudioFormat* Format = FindFormat(InWave.FormatName))
		{
			...
			{
				UE_LOGF(LogAudio, Verbose, "Wave '%ls', format '%ls' doesn't support platform '%ls'",
			...
			{
				UE_LOGF(LogAudio, Verbose, "Wave '%ls', format '%ls' doesn't support channel count: '%d'",
	...
	void FAudioFormatSettings::ReadConfiguration(FConfigCacheIni* InConfigSystem, const FString& InConfigFilename)
	{
		...
		{		
			...
			else
			{
				...
				UE_LOGF(LogAudio, Warning, "Audio:AllWaveFormats is not defined, defaulting to built in formats. (%ls)", *MakePrettyArrayToString(AllWaveFormats));
			}
		}
		...
		// FallbackFormat
		{
			...
			else
			{
				...
				UE_LOGF(LogAudio, Warning, "Audio:FallbackFormat is not defined, defaulting to '%ls'.", *FallbackFormat.GetPlainNameString());
			}
			if (!AllWaveFormats.Contains(FallbackFormat) && AllWaveFormats.Num() > 0)
			{
				UE_LOGF(LogAudio, Warning, "FallbackFormat '%ls' not defined in 'AllWaveFormats'. Using first format listed '%ls'", *FallbackFormatString, *AllWaveFormats[0].ToString());
		...
		// PlatformFormat
		{
			...
			else
			{
				...
				UE_LOGF(LogAudio, Warning, "Audio:PlatformFormat is not defined, defaulting to '%ls'.", *PlatformFormat.GetPlainNameString());
			}
			if (!AllWaveFormats.Contains(PlatformFormat))
			{
				UE_LOGF(LogAudio, Warning, "PlatformFormat '%ls' not defined in 'AllWaveFormats'. Using fallback format '%ls'", *PlatformFormatString, *FallbackFormat.ToString());
		...
		// PlatformStreamingFormat
		{
			...
			else
			{
				...
				UE_LOGF(LogAudio, Warning, "Audio:PlatformStreamingFormat is not defined, defaulting to '%ls'.", *PlatformStreamingFormat.GetPlainNameString());
			}
			if (!AllWaveFormats.Contains(PlatformStreamingFormat))
			{
				UE_LOGF(LogAudio, Warning, "PlatformStreamingFormat '%ls' not defined in 'AllWaveFormats'. Using fallback format '%ls'", *PlatformStreamingFormatString, *FallbackFormat.ToString());
				PlatformStreamingFormat = FallbackFormat;
			}
		}

		UE_LOGF(LogAudio, Verbose, "AudioFormatSettings: TargetName='%ls', AllWaveFormats=(%ls), Hints=(%ls), PlatformFormat='%ls', PlatformStreamingFormat='%ls', FallbackFormat='%ls'",
