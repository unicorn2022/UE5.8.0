...
bool UHLODEngineSubsystem::CleanupHLOD(ALODActor* InLODActor)
{
	...
	{
		UE_LOGF(LogEngine, Warning, "Deleting legacy LODActor %ls found in World Partition map. Resave %ls to silence warning.", *InLODActor->GetName(), *InLODActor->GetOutermost()->GetPathName());
		bShouldDestroyActor = true;
	}
	if (InLODActor->GetLevel()->GetWorldSettings()->GetHierarchicalLODSetup().Num() == 0)
	{
		UE_LOGF(LogEngine, Warning, "Deleting LODActor %ls found in map with no HLOD setup or disabled HLOD system. Resave %ls to silence warning.", *InLODActor->GetName(), *InLODActor->GetOutermost()->GetPathName());
		bShouldDestroyActor = true;
	}
	else if (!InLODActor->GetProxy() || InLODActor->GetProxy()->GetMap() != TSoftObjectPtr<UWorld>(InLODActor->GetLevel()->GetTypedOuter<UWorld>()))
	{
		UE_LOGF(LogEngine, Warning, "Deleting LODActor %ls with invalid HLODProxy. Resave %ls to silence warning.", *InLODActor->GetName(), *InLODActor->GetOutermost()->GetPathName());
		bShouldDestroyActor = true;
	}
