...
void AGameModeBase::PreInitializeComponents()
{
	...
	{
		UE_LOGF(LogGameMode, Warning, "No GameStateClass was specified in %ls (%ls)", *GetName(), *GetClass()->GetName());
		GameStateClass = AGameStateBase::StaticClass();
	}
...
bool AGameModeBase::ClearPause()
{
	...
	{
		UE_LOGF(LogGameMode, Log, "Clearing list of UnPause delegates for %ls because game type is not pauseable", *GetFName().ToString());
		Pausers.Reset();
		bPauseCleared = true;
	}
...
{
	UE_LOGF(LogGameMode, Verbose, "Reset %ls", *GetName());
		...
		if (APlayerController* PlayerController = Iterator->Get())
		{
			if (Cast<UNetConnection>(PlayerController->Player) != nullptr)
			{
				...
				if (!UE::GameModeBase::Private::bAllowListenParamSentToClient)
				{
					...
					static FString ListenString = "?listen";
...
bool AGameModeBase::CanServerTravel(const FString& FURL, bool bAbsolute)
{
	...
	{
		UE_LOGF(LogGameMode, Error, "CanServerTravel: FURL %ls Contains illegal character '%%'.", *FURL);
		return false;
	}

	if (FURL.Contains(TEXT(":")) || FURL.Contains(TEXT("\\")))
	{
		UE_LOGF(LogGameMode, Error, "CanServerTravel: FURL %ls blocked, contains : or \\", *FURL);
		return false;
	}
	...
	{
		UE_LOGF(LogGameMode, Log, "CanServerTravel: FURL %ls blocked (%ls)", *FURL, *InvalidPackageError.ToString());
		return false;
	}
...
void AGameModeBase::ProcessServerTravel(const FString& URL, bool bAbsolute)
{
	...
	UE_LOGF(LogGameMode, Log, "ProcessServerTravel: %ls", *URL);
	UWorld* World = GetWorld();
	check(World);
	...
	if (World->WorldType == EWorldType::PIE && bSeamless && !FParse::Param(FCommandLine::Get(), TEXT("MultiprocessOSS")))
	{
		...
		{
			UE_LOGF(LogGameMode, Warning, "ProcessServerTravel: Seamless travel is disabled in PIE, set net.AllowPIESeamlessTravel=1 to enable.");
			bSeamless = false;
		}
	}
...
void AGameModeBase::SwapPlayerControllers(APlayerController* OldPC, APlayerController* NewPC)
{
	...
	{
		UE_LOGF(LogGameMode, Warning, "SwapPlayerControllers: Invalid OldPC, invalid NewPC, or OldPC has no Player!");
	}
}
...
void AGameModeBase::HandleSeamlessTravelPlayer(AController*& C)
{
	...
	if (PC && PC->Player)
	{
		...
		{
			UE_LOGF(LogGameMode, Warning, "HandleSeamlessTravelPlayer: Failed to spawn new PlayerController for %ls (old class %ls)", *PC->GetHumanReadableName(), *PC->GetClass()->GetName());
			PC->Destroy();
			return;
		}
	}
...
{
	if (Options.Contains(FString("SpectatorOnly=1")) && ReplaySpectatorPlayerControllerClass != nullptr)
	{
		return SpawnPlayerControllerCommon(InRemoteRole, FVector::ZeroVector, FRotator::ZeroRotator, ReplaySpectatorPlayerControllerClass);
	}
...
void AGameModeBase::InitSeamlessTravelPlayer(AController* NewController)
{
	...
	{
		UE_LOGF(LogGameMode, Warning, "InitSeamlessTravelPlayer: %ls", *ErrorMessage);
	}
		...
		if (MyWorld->GetStreamingLevels().Num() > 0)
		{
			...
			for (ULevelStreaming* TheLevel : MyWorld->GetStreamingLevels())
			{
				if (TheLevel && TheLevel->CanReplicateStreamingStatus())
				{
					...
					UE_LOGF(LogGameMode, Verbose, "ReplicateStreamingStatus: %ls %i %i %i %ls %i",
...
void AGameModeBase::RestartPlayerAtTransform(AController* NewPlayer, const FTransform& SpawnTransform)
{
	...
	UE_LOGF(LogGameMode, Verbose, "RestartPlayerAtTransform %ls", (NewPlayer && NewPlayer->PlayerState) ? *NewPlayer->PlayerState->GetPlayerName() : TEXT("Unknown"));

	if (MustSpectate(Cast<APlayerController>(NewPlayer)))
	{
		UE_LOGF(LogGameMode, Verbose, "RestartPlayerAtTransform: Tried to restart a spectator-only player!");
		return;
	}
