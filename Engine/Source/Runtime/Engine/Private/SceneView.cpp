...
static TAutoConsoleVariable<float> CVarSceneDepthFudgeFactor(
	TEXT("r.SceneDepthFudgeFactor"),
	1e-13f,
	TEXT("Subtract a tiny number to avoid divide by 0 errors in the shader when a very far distance is decided from the depth buffer."),
...
static TAutoConsoleVariable<int32> CVarMouseCursorX(
	TEXT("r.MouseCursor.X"),
	-1,
	TEXT("Override default debug visualization cursor position. -1 disables override."),
	ECVF_Cheat);

static TAutoConsoleVariable<int32> CVarMouseCursorY(
	TEXT("r.MouseCursor.Y"),
	-1,
	TEXT("Override default debug visualization cursor position. -1 disables override."),
	ECVF_Cheat);
...
float GOrthographicDepthThicknessScale = 0.001f;
...
FVector4f CreateInvDeviceZToWorldZTransform(const FMatrix& ProjMatrix)
{
	...
	if (bIsPerspectiveProjection)
	{
		...
		SubtractValue -= CVarSceneDepthFudgeFactor.GetValueOnAnyThread();
...
void FViewMatrices::Init(const FMinimalInitializer& Initializer)
{
	...
	ViewToClip = AdjustProjectionMatrixForRHI(Initializer.ProjectionMatrix);
	...
	WorldToView = FTranslationMatrix(-LocalViewOrigin) * ViewRotationMatrix;
	...
	ViewToWorld = ViewRotationMatrix.GetTransposed() * FTranslationMatrix(LocalViewOrigin);
	...
	TranslatedWorldToClip = LocalTranslatedViewMatrix * ViewToClip;
	ClipToTranslatedWorld = ClipToView * LocalInvTranslatedViewMatrix;
	...
	if (IsPerspectiveProjection())
	{
		ProjectionScale.X = ScreenXScale * FMath::Abs(ViewToClip.M[0][0]);
		ProjectionScale.Y = FMath::Abs(ViewToClip.M[1][1]);
	...
	else
	{
		...
		{
			int8 Exponent = -FMath::Clamp(FMath::LogX(10, FMath::Abs(ClipToView.M[2][2])), 0, 9);
	...
	if (FirstPersonParams.bUseParameters)
	{
		...
		const FVector ScaleVector = FVector(FOVCorrectionFactor, FOVCorrectionFactor, 1.0f) * FirstPersonParams.Scale;
		WorldToFirstPerson = TranslatedViewMatrix * FScaleMatrix(ScaleVector) * InvTranslatedViewMatrix;
		FirstPersonToWorld = TranslatedViewMatrix * FScaleMatrix(ScaleVector.Reciprocal()) * InvTranslatedViewMatrix;
	}
}
...
void FSceneView::SetupViewFrustum()
{
	if (SceneViewInitOptions.OverrideFarClippingPlaneDistance > 0.0f)
	{
		...
		GetViewFrustumBounds(ViewFrustum, ViewMatrices.GetWorldToClip(), FarPlane, true, false);
	}
	else
	{
		// Derive the view frustum from the view projection matrix.
		GetViewFrustumBounds(ViewFrustum, ViewMatrices.GetWorldToClip(), false);
	}
	...
	bHasNearClippingPlane = ViewMatrices.GetWorldToClip().GetFrustumNearPlane(NearClippingPlane);
...
FSceneView::FSceneView(const FSceneViewInitOptions& InitOptions)
	...
	, SkylightScale(InitOptions.SkylightScale)
	...
	, FeatureLevel(InitOptions.ViewFamily ? InitOptions.ViewFamily->GetFeatureLevel() : GMaxRHIFeatureLevel)
{
	...
	// Query instanced stereo and multi-view state
	{
		...
		{
			UE_LOGF(LogMultiView, Fatal, "Family requires Mobile Multi-View, but it is not supported by the RHI and no fallback is available.");
		}

		if (Aspects.IsSinglePassStereoEnabled())
		{
			checkf((Aspects.IsInstancedStereoEnabled() || Aspects.IsMobileMultiViewEnabled()) &&
				!(Aspects.IsInstancedStereoEnabled() && Aspects.IsMobileMultiViewEnabled()),
				TEXT("If single pass stereo is enabled, either instanced stereo or mobile multi-view needs to be enabled as well but not both."));

			// Planar reflections have bIsSceneCapture == true, but can have instanced stereo views
			const bool bIsSingleViewCapture = (bIsSceneCapture || bIsReflectionCapture) && !bIsPlanarReflection;
			...
			{
				bIsMobileMultiViewEnabled = !bIsSingleViewCapture && Family && Family->bRequireMultiView;
				MultiViewCount = bIsMobileMultiViewEnabled ? 2 : 1;
			}
	...
	bReverseCulling = (Family && Family->Scene) ? ViewMatrices.GetWorldToView().Determinant() < 0.0f : false;
...
int32 FSceneView::GetMultiViewCount() const
{
	return MultiViewCount;
}

bool FSceneView::UsesMobileMultiViewShaders() const
{
	return Aspects.IsMobileMultiViewEnabled();
}

bool FSceneView::UsesInstancedStereoShaders() const
{
	return Aspects.IsInstancedStereoEnabled();
}

bool FSceneView::IsInstancedStereo() const
{
	return bIsInstancedStereoEnabled && IStereoRendering::IsStereoEyeView(*this);
}

FRDGTextureDesc FSceneView::CreateStereoTextureDesc(FIntPoint Size, EPixelFormat Format, FClearValueBinding Clear, ETextureCreateFlags Flags) const
{
	return
		UsesMobileMultiViewShaders() ?
		FRDGTextureDesc::Create2DArray(Size, Format, Clear, Flags, MultiViewCount) :
		FRDGTextureDesc::Create2D(Size, Format, Clear, Flags);
}
...
void FViewMatrices::UpdateViewMatrix(const FVector& ViewLocation, const FRotator& ViewRotation)
{
	...
	WorldToView = FTranslationMatrix(-ViewLocation) * ViewRotationMatrix;
	ViewToWorld = ViewRotationMatrix.GetTransposed() * FTranslationMatrix(ViewLocation);
...
FMatrix FViewMatrices::ScreenToClipProjectionMatrix() const
{
	...
	if (IsPerspectiveProjection())
	{
		return FMatrix(
			FPlane(1, 0, 0, 0),
			FPlane(0, 1, 0, 0),
			FPlane(0, 0, ViewToClip.M[2][2], 1.0f),
			FPlane(0, 0, ViewToClip.M[3][2], 0.0f));
	}
	else
	{
		return FMatrix(
			FPlane(1, 0, 0, 0),
			FPlane(0, 1, 0, 0),
			FPlane(0, 0, ViewToClip.M[2][2], 0.0f),
			FPlane(0, 0, ViewToClip.M[3][2], 1.0f));
	}
}
...
void FViewMatrices::HackOverrideViewMatrixForShadows(const FMatrix& InViewMatrix)
{
	...
	WorldToView = FTranslationMatrix(-ViewOrigin) * InViewMatrix;
...
void FViewMatrices::UpdatePlanarReflectionViewMatrix(const FSceneView& SourceView, const FMirrorMatrix& MirrorMatrix)
{
	...
	const FMatrix LocalViewMatrix(MirrorMatrix * SourceView.ViewMatrices.GetWorldToView());
	...
	WorldToView = FTranslationMatrix(-ViewOrigin) * HMDViewMatrixNoRoll;
	//ViewToWorld = FTranslationMatrix(-WorldToView.GetOrigin()) * WorldToView.RemoveTranslation().GetTransposed();

	ViewToWorld = HMDViewMatrixNoRoll.GetTransposed() * FTranslationMatrix(ViewOrigin);

	WorldToClip = GetWorldToView() * GetViewToClip();
	ClipToWorld = GetClipToView() * ViewToWorld;
...
{
	return ViewMatrices.GetWorldToClip().TransformFVector4(FVector4(WorldPoint,1));
}

FVector FSceneView::ScreenToWorld(const FVector4& ScreenPoint) const
{
	return ViewMatrices.GetClipToWorld().TransformFVector4(ScreenPoint);
}
...
// render target space.  If desired and outside the bounds of the viewport, will clamp to the nearest edge, so a valid result is returned.
FVector4 FSceneView::CursorToScreen(float InX, float InY, float Z, bool bClamped /* = true */) const
{
	float X = InX - UnscaledViewRect.Min.X;
	float Y = InY - UnscaledViewRect.Min.Y; 

	if (bClamped)
	{
		X = FMath::Clamp(X, 0.0f, UnscaledViewRect.Width());
		Y = FMath::Clamp(Y, 0.0f, UnscaledViewRect.Height());
	}
	
	return PixelToScreen(X, Y, Z);
}
...
{
	return ViewMatrices.GetClipToWorld().TransformFVector4(FPlane(ScreenPoint.X * ScreenPoint.W,ScreenPoint.Y * ScreenPoint.W,ScreenPoint.Z * ScreenPoint.W,ScreenPoint.W));
}

void FSceneView::DeprojectFVector2D(const FVector2D& ScreenPos, FVector& out_WorldOrigin, FVector& out_WorldDirection) const
{
	const FMatrix InvViewProjectionMatrix = ViewMatrices.GetClipToWorld();
...
// @param Weight 0..1
void FSceneView::OverridePostProcessSettings(const FPostProcessSettings& Src, float Weight, bool bAllowBlendables)
{
...

	{
		...
		SET_PP(bMegaLights);
		SET_PP(MegaLightsFrontLayerTranslucency);
		...
		SET_PP(VignetteType);
		LERP_PP(VignetteCenter);
		LERP_PP(VignetteColor);
		LERP_PP(VignetteSize);
		LERP_PP(VignetteSoftness);
		SET_PP(VignetteTexture);
		LERP_PP(Sharpen);
		...
		LERP_PP(LumenAmbientOcclusionIntensity);
...
void FSceneView::StartFinalPostprocessSettings(FVector InViewLocation)
{
	...
	// project settings might want to have different defaults
	{
		...
		{
			static const auto CVar = IConsoleManager::Get().FindTConsoleVariableDataBool(TEXT("r.MegaLights.FrontLayerTranslucency.EnableForProject"));
			FinalPostProcessSettings.MegaLightsFrontLayerTranslucency = CVar->GetValueOnGameThread();
		}
...
void FSceneView::EndFinalPostprocessSettings(const FSceneViewInitOptions& ViewInitOptions)
{
	...
	if (Config.bDisplayCaptureRegion && !GIsHighResScreenshot)
	{
		...
		if ((Config.UnscaledCaptureRegion != UnscaledViewRect) && (Config.UnscaledCaptureRegion.Area() > 0) && (State != NULL))
		{
			...
			if (ConfigViewport.IsValid() && Family->RenderTarget == ConfigViewport.Get())
			{
	...
	if (bVisualizationRequired || bBufferDumpingRequired)
	{
		...
		if (BufferVisualizationData.IsDifferentToCurrentOverviewMaterialNames(SelectedMaterialNames))
		{
			...
			while (SelectedMaterialNames.Len())
			{
				...
				{
					UE_LOGF(LogBufferVisualization, Warning, "Unknown material '%ls'", *Left);
				}
...
void FSceneView::ConfigureVisualizeCalibrationSettings()
{
	...
	auto ConfigureCalibrationSettings = [](const FSoftObjectPath& InPath, UMaterialInterface*& OutMaterialInterface, FName& OutMaterialName)
	{
		if (InPath.IsValid())
		{
			...
			{
				UE_LOGF(LogBufferVisualization, Warning, "Error loading material '%ls'", *InPath.ToString());
...
{
	return IsInstancedStereo() && IStereoRendering::IsAPrimaryView(*this);
}
	...
	else
	{
		...
		ViewUniformShaderParameters.SVPositionToTranslatedWorld = FMatrix44f(
			FMatrix(FPlane(Mx, 0, 0, 0),
				FPlane(0, My, 0, 0),
				FPlane(0, 0, 1, 0),
				FPlane(Ax, Ay, 0, 1)) * InViewMatrices.GetClipToTranslatedWorld());
	}
	...
	const FSetupViewUniformParametersInputs& Inputs)
{
	...
	ViewUniformShaderParameters.TranslatedWorldToClip = FMatrix44f(InViewMatrices.GetTranslatedWorldToClip());
	...
	ViewUniformShaderParameters.ViewToClipNoAA = FMatrix44f(InViewMatrices.GetViewToClipNoAA());
	ViewUniformShaderParameters.ClipToView = RelativeMatrices.ClipToView;
	ViewUniformShaderParameters.ClipToTranslatedWorld = FMatrix44f(InViewMatrices.GetClipToTranslatedWorld());
	...
	ViewUniformShaderParameters.PrevViewToClip = FMatrix44f(InPrevViewMatrices.GetViewToClip());
	ViewUniformShaderParameters.PrevClipToView = RelativeMatrices.PrevClipToView;
	ViewUniformShaderParameters.PrevTranslatedWorldToClip = FMatrix44f(InPrevViewMatrices.GetTranslatedWorldToClip());
	...
	ViewUniformShaderParameters.ScreenToTranslatedWorld = 
		FMatrix44f(InViewMatrices.GetScreenToClipMatrix() * InViewMatrices.GetClipToTranslatedWorld());

	ViewUniformShaderParameters.MobileMultiviewShadowTransform = 
		FMatrix44f(InViewMatrices.GetScreenToClipMatrix() * InViewMatrices.GetClipToTranslatedWorld() * FTranslationMatrix(-InViewMatrices.GetPreViewTranslation()));

	ViewUniformShaderParameters.MobileMultiviewDecalTransform =
		FMatrix44f(FTranslationMatrix(InViewMatrices.GetPreViewTranslation()) * InViewMatrices.GetTranslatedWorldToClip());

	ViewUniformShaderParameters.PrevScreenToTranslatedWorld = 
		FMatrix44f(InPrevViewMatrices.GetScreenToClipMatrix() * InPrevViewMatrices.GetClipToTranslatedWorld());
	...
	{
		FVector DeltaTranslation = InPrevViewMatrices.GetPreViewTranslation() - InViewMatrices.GetPreViewTranslation();
		FMatrix InvViewProj = InViewMatrices.GetClipToView() * InViewMatrices.GetTranslatedViewMatrix().GetTransposed();
		FMatrix PrevViewProj = FTranslationMatrix(DeltaTranslation) * InPrevViewMatrices.GetTranslatedViewMatrix() * InPrevViewMatrices.GetViewToClip();

		ViewUniformShaderParameters.ClipToPrevClipWithAA = FMatrix44f(InvViewProj * PrevViewProj);		// LWC_TODO: Precision loss?
	}

	ViewUniformShaderParameters.FirstPersonTransform = FMatrix44f(InViewMatrices.GetWorldToFirstPerson());
	ViewUniformShaderParameters.PrevFirstPersonTransform = FMatrix44f(InPrevViewMatrices.GetWorldToFirstPerson());
	ViewUniformShaderParameters.InvFirstPersonTransform = FMatrix44f(InViewMatrices.GetFirstPersonToWorld());
	...
	if (CVarMouseCursorX.GetValueOnRenderThread() >= 0 && CVarMouseCursorY.GetValueOnRenderThread() >= 0)
	{
		LockedCursorPos.X = CVarMouseCursorX.GetValueOnRenderThread();
		LockedCursorPos.Y = CVarMouseCursorY.GetValueOnRenderThread();
	}
...
FSceneViewFamily::~FSceneViewFamily()
{
	...
	if (MegaFiltersInterface)
	{
		// IMegaFilters* is only defined in renderer's private header because no backward compatibility is provided between major version.
		delete reinterpret_cast<ISceneViewFamilyExtention*>(MegaFiltersInterface);
	}
	...
	: FSceneViewFamily(static_cast<const FSceneViewFamily&>(InViewFamily))
{
	...
	check(MegaFiltersInterface == nullptr);
...
EDebugViewShaderMode FSceneViewFamily::ChooseDebugViewShaderMode() const
{
	...
	else if (EngineShowFlags.StreamingTextureDeficit
	      || EngineShowFlags.StreamingTextureResidency
	      || EngineShowFlags.StreamingMeshLODDeficit
	      || EngineShowFlags.StreamingMeshLODResidency)
	{
		return DVSM_StreamingDeficit;
	}
	return DVSM_None;
}
