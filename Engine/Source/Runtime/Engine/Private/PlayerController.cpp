	...
	: Super(ObjectInitializer)
{
	...
	TickOffsetBufferTime = NetworkPhysicsCvars::TickOffsetBufferTime;
...
void APlayerController::ClientUpdateLevelStreamingStatus_Implementation(FName PackageName, bool bNewShouldBeLoaded, bool bNewShouldBeVisible, bool bNewShouldBlockOnLoad, int32 LODIndex, FNetLevelVisibilityTransactionId TransactionId, bool bNewShouldBlockOnUnload)
{
	...
	{
		UE_LOGF(LogStreaming, Log, "Unable to find streaming object %ls", *PackageName.ToString() );
	}
}
...
bool APlayerController::ServerUpdateLevelVisibility_Validate(const FUpdateLevelVisibilityLevelInfo& LevelVisibility)
{
	...
	{
		UE_LOGF(LogPlayerController, Warning, "ServerUpdateLevelVisibility() Invalid package name: %ls (%ls)", *LevelVisibility.PackageName.ToString(), *Reason.ToString());
		return false;
	}
...
void APlayerController::ServerUpdateContainerVisibility_Implementation(UObject* Container, bool bVisible)
{
	if (UNetConnection* Connection = Cast<UNetConnection>(Player))
	{
		Connection->UpdateContainerVisibility(Container, bVisible);
	}
}
...
FString APlayerController::ConsoleCommand(const FString& Cmd, bool bWriteToLog)
{
	if (Player != nullptr)
	{
		...
		{
			static FName TransactionalWorkName("APlayerController::ConsoleComand");
...
void APlayerController::ServerNotifyLoadedWorld_Implementation(FName WorldPackageName)
{
	...
	UE_LOGF(LogPlayerController, Verbose, "APlayerController::ServerNotifyLoadedWorld_Implementation: Client loaded %ls", *WorldPackageName.ToString());
...
void APlayerController::ClientRetryClientRestart_Implementation(APawn* NewPawn)
{
	...
	UE_LOGF(LogPlayerController, Verbose, "ClientRetryClientRestart_Implementation %ls, AcknowledgedPawn: %ls", *GetNameSafe(NewPawn), *GetNameSafe(AcknowledgedPawn));
...
{
	UE_LOGF(LogPlayerController, Verbose, "ClientRestart_Implementation %ls", *GetNameSafe(NewPawn));
...
void APlayerController::SpawnDefaultHUD()
{
	...
	UE_LOGF(LogPlayerController, Verbose, "SpawnDefaultHUD");
...
void APlayerController::OnActorChannelOpen(FInBunch& InBunch, UNetConnection* Connection)
{
	...
	if (Connection->Driver != NULL && Connection == Connection->Driver->ServerConnection)
	{
		...
		if (bIsParentConnection)
		{
			...
			// sets the reserved bandwidth on the bandwidth manager for gameplay in Kbps
			FDownlinkBandwidthManager& DBManager = FDownlinkBandwidthManager::Get();
			DBManager.SetGameplayBandwidthCap(Connection->Driver->MaxInternetClientRate / UE::BandwidthManager::KilobitsToBytesConversionValue);
		}
		else
		{
			...
			{
				UE_LOGF(LogNet, Log, "[%ls:%ls] Binding player controller %ls to child connection %ls", 
...
{
	FString SMod = S;
	static FName NAME_Say = FName("Say");
...
void APlayerController::ServerExec(const FString& Msg)
{
	...
	if (Msg.Len() > 128)
	{
		UE_LOGF(LogPlayerController, Warning, "APlayerController::ServerExec. Msg too big for network RPC. Truncating to 128 character");
	}
...
bool APlayerController::InputKey(const FInputKeyParams& Params)
{
	...
	{
		UE_LOGF(LogPlayerInput, Warning, "Call to the deprecated version of UPlayerInput::InputKey will no longer consider the YZ components of the input delta. Use paired key axis instead.");
	}
...
{
	return InputTouch(FTouchId(DeviceId, static_cast<ETouchIndex::Type>(Handle)), Type, TouchLocation, Force, Timestamp);
}

bool APlayerController::InputTouch(const FTouchId TouchId, const ETouchType::Type Type, const FVector2D& TouchLocation, const float Force,
	const uint64 Timestamp)
{
	bool bResult = false;
	if (PlayerInput)
	{
		bResult = PlayerInput->InputTouch(TouchId, Type, TouchLocation, Force, Timestamp);
		...
		if (bEnableTouchEvents || bEnableTouchOverEvents)
		{
			...
			ProcessTouchHitResult(TouchId, Type, TouchLocation, HitResult, bHit);
		}
	}
...
void APlayerController::ProcessTouchHitResult(const FInputDeviceId DeviceId, uint32 Handle, ETouchType::Type Type, const FVector2D& TouchLocation,
	const FHitResult& HitResult, bool bHit)
{
	ProcessTouchHitResult(FTouchId(DeviceId, static_cast<ETouchIndex::Type>(Handle)), Type, TouchLocation, HitResult, bHit);
}

void APlayerController::ProcessTouchHitResult(const FTouchId TouchId, const ETouchType::Type Type, const FVector2D& TouchLocation,
	const FHitResult& HitResult, bool bHit)
{
	if (GetHUD())
	{
	...
	ETouchEventFamilies EnabledFamilies = ETouchEventFamilies::None;
	if (bEnableTouchEvents)
	{
		EnabledFamilies |= ETouchEventFamilies::BeginEnd;
	}
	if (bEnableTouchOverEvents)
	{
		EnabledFamilies |= ETouchEventFamilies::Over;
	}
	if (EnabledFamilies != ETouchEventFamilies::None)
	{
		UPrimitiveComponent::DispatchTouchHitResult(TouchId, Type, HitResult, bHit, this, EnabledFamilies, CurrentTouchablePrimitives);
	}
}
...
bool APlayerController::InputGesture(const FInputDeviceId DeviceId, EGestureEvent GestureType, EGesturePhase GesturePhase, const FVector2D& GestureDelta, bool bIsDirectionInvertedFromDevice, const uint64 Timestamp)
{
	bool bResult = false;
	...
	if (PlayerInput)
	{
		...
		switch (GestureType)
		{
		case EGestureEvent::Pan:      GestureKey = EKeys::Gesture_Pan; break;
		case EGestureEvent::Magnify:  GestureKey = EKeys::Gesture_Pinch; break;
		case EGestureEvent::Rotate:   GestureKey = EKeys::Gesture_Rotate; break;
		case EGestureEvent::Tap:      GestureKey = EKeys::Gesture_Tap; break;
		case EGestureEvent::LongPress: GestureKey = EKeys::Gesture_LongPress; break;
		case EGestureEvent::Flick:    GestureKey = EKeys::Gesture_Flick; break;
		...
		switch (GesturePhase)
		{
		case EGesturePhase::Started: InputEvent = IE_Pressed; break;
		case EGesturePhase::Update:  InputEvent = IE_Repeat; break;
		case EGesturePhase::Ended:   InputEvent = IE_Released; break;
		default: return false;
		}
		bResult = PlayerInput->InputGesture(FInputKeyEventArgs(DeviceId, GestureKey, InputEvent, GestureDelta, Timestamp));
	}
...
void APlayerController::SetMotionControlsEnabled(bool bEnabled)
{
	...
	{
		UE_LOGF(LogPlayerController, Warning, "Player bEnableMotionControls has been set to true, but motion controls are disabled in the input settings! See UInputSettings::bEnableMotionControls");
	}
}
...
void APlayerController::SetShowMouseCursor(bool bShow)
{
	...
	{
		UE_LOGF(LogViewport, Display, "Player bShowMouseCursor Changed, %ls -> %ls",
...
#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
	{
		for (const UInputComponent* IC : InputStack)
		{
			LastUsedInputStack.Add(IC);
		}
	}
#endif
...
void APlayerController::SpawnPlayerCameraManager()
{
	...
	{
		UE_LOGF(LogPlayerController, Log,  "Couldn't Spawn PlayerCameraManager for Player!!" );
	}
}
...
float APlayerController::GetDeprecatedInputYawScale() const
{
	...
	{
		UE_LOGF(LogPlayerController, Warning, "Attempting to access legacy input scales without the setting enabled! See UInputSettings::bEnableLegacyInputScales.");
		return 1.0f;
	}
}
...
void APlayerController::ServerRestartPlayer_Implementation()
{
	...
	if ( IsInState(NAME_Inactive) || (IsInState(NAME_Spectating) && bPlayerIsWaiting) )
	{
		...
		if (!IsValid(GameMode))
		{
			UE_LOGF(LogPlayerController, Warning, "[APlayerController::ServerRestartPlayer_Implementation] Player Controller '%ls' requested restart but the game mode is null! Nothing will happen.", *GetNameSafe(this));
			return;
		}
...
void APlayerController::DisplayDebug(class UCanvas* Canvas, const FDebugDisplayInfo& DebugDisplay, float& YL, float& YPos)
{
	...
	if (DebugDisplay.IsDisplayOn(NAME_Camera))
	{
		if (PlayerCameraManager != nullptr)
		{
			DisplayDebugManager.DrawString(FString("<<<< CAMERA >>>>"));
			PlayerCameraManager->DisplayDebug(Canvas, DebugDisplay, YL, YPos);
		}
		else
		{
			DisplayDebugManager.SetDrawColor(FColor::Red);
			DisplayDebugManager.DrawString(FString("<<<< NO CAMERA >>>>"));
		}
	}
	if ( DebugDisplay.IsDisplayOn(NAME_Input) )
	{
		...
		DisplayDebugManager.DrawString(FString("<<<< INPUT STACK >>>"));
		...
		{
			DisplayDebugManager.SetDrawColor(FColor::Red);
			DisplayDebugManager.DrawString(FString("NO INPUT"));
		}
	}
	...
	if ( Player != NULL )
	{
		if ( IsSplitscreenPlayer() )
		{
			...
			if ( LP != NULL )
			{
				...
				{
					UE_LOGF(LogPlayerController, Warning, "(%ls) APlayerController::%ls:GetSplitscreenPlayerByIndex: requested player at invalid index! PlayerIndex:%i NumLocalPlayers:%i",
			...
			else if ( RemoteConnection != NULL )
			{
				if ( GetNetMode() == NM_Client )
				{
					...
					UE_LOGF(LogPlayerController, Warning, "(%ls) APlayerController::%ls:GetSplitscreenPlayerByIndex: CALLED ON CLIENT WITH VALID REMOTE NETCONNECTION!",
				...
				else
				{
					...
					{
						UE_LOGF(LogPlayerController, Log, "(%ls) APlayerController::%ls:GetSplitscreenPlayerByIndex: %ls IS NOT THE PRIMARY CONNECTION AND HAS NO CHILD CONNECTIONS!", *GetName(), *GetStateName().ToString(), *Player->GetName());
					}
				}
			}
			else
			{
				UE_LOGF(LogPlayerController, Log, "(%ls) APlayerController::%ls:GetSplitscreenPlayerByIndex: %ls IS NOT A ULocalPlayer* AND NOT A RemoteConnection! (No valid UPlayer* reference)", *GetName(), *GetStateName().ToString(), *Player->GetName());
			}
		}
	}
	else
	{
		UE_LOGF(LogPlayerController, Log, "(%ls) APlayerController::%ls:GetSplitscreenPlayerByIndex: %ls", *GetName(), *GetStateName().ToString(), TEXT("NULL value for Player!"));
	}
...
int32 APlayerController::GetSplitscreenPlayerCount() const
{
	...
	if ( IsSplitscreenPlayer() )
	{
		if ( Player != NULL )
		{
			...
			{
				UE_LOGF(LogPlayerController, Log, "(%ls) APlayerController::%ls:GetSplitscreenPlayerCount NOT A ULocalPlayer* AND NOT A RemoteConnection!", *GetName(), *GetStateName().ToString());
			}
		}
		else
		{
			UE_LOGF(LogPlayerController, Log, "(%ls) APlayerController::%ls:GetSplitscreenPlayerCount called without a valid UPlayer* value!", *GetName(), *GetStateName().ToString());
		}
	}
	...
	else
	{
		...
		if (FeedbackAction == nullptr && Action == EDynamicForceFeedbackAction::Start)
		{
			if (ActionHandle > 0)
			{
				...
				{
					UE_LOGF(LogPlayerController, Error, "Specifying an ID to start a dynamic force feedback with that has not yet been assigned is unsafe. No action has been started.");
				}
			}
			else
			{
...
void APlayerController::PlayHapticEffect(UHapticFeedbackEffect_Base* HapticEffect, EControllerHand Hand, float Scale, bool bLoop)
{
	if (HapticEffect)
	{
		...
		case EControllerHand::HMD:
			...
			break;
		default:
			UE_LOGF(LogPlayerController, Warning, "Invalid hand specified (%d) for haptic feedback effect %ls", (int32)Hand, *HapticEffect->GetName());
			break;
		}
	}
}
...
void APlayerController::SetHapticsByValue(const float Frequency, const float Amplitude, EControllerHand Hand)
{
	...
	{
		UE_LOGF(LogPlayerController, Warning, "Invalid hand specified (%d) for setting haptic feedback values (F: %f A: %f)", (int32)Hand, Frequency, Amplitude);
		return;
	}
...
#if UE_WITH_REMOTE_OBJECT_HANDLE
static void UpdateNetConnectionControllerPosition(UNetConnection* NetConnection)
{
	AActor* ControllerActor = Cast<AActor>(NetConnection->PlayerController);
	bool bNoPawnPlayerControllerPositionSet = false;
	if (ControllerActor)
	{
		if (NetConnection->ViewTarget.GetResidence() == EResidence::Remote)
		{
			UE_LOGF(LogPlayerController, Warning, "UpdateNetConnectionControllerPosition: Incoming ViewTarget is remote.");
		}
		else
		{
			if (APlayerController* PlayerController = Cast<APlayerController>(NetConnection->ViewTarget))
			{
				FVector TrueViewLocation;
				FRotator ViewRotation;
				PlayerController->GetPlayerViewPoint(TrueViewLocation, ViewRotation);
				ControllerActor->SetActorLocation(TrueViewLocation);
				UE_LOGF(LogPlayerController, Verbose, "[%ls] UpdateNetConnectionControllerPosition: Setting NoPawnPlayerController %ls position to %ls using position from PlayerController %ls",
					*FRemoteServerId::GetLocalServerId().ToString(),
					*ControllerActor->GetName(), *(TrueViewLocation.ToString()), *PlayerController->GetName());
				bNoPawnPlayerControllerPositionSet = true;
			}
			else if (AActor* TargetActor = Cast<AActor>(NetConnection->ViewTarget))
			{
				FVector TrueViewLocation = TargetActor->GetActorLocation();
				ControllerActor->SetActorLocation(TrueViewLocation);
				UE_LOGF(LogPlayerController, Verbose, "[%ls] UpdateNetConnectionControllerPosition: Setting NoPawnPlayerController %ls position to %ls using position from PlayerCharacter %ls",
					*FRemoteServerId::GetLocalServerId().ToString(),
					*ControllerActor->GetName(), *(TrueViewLocation.ToString()), *TargetActor->GetName());
				bNoPawnPlayerControllerPositionSet = true;
			}
		}
	}
	if (!bNoPawnPlayerControllerPositionSet)
	{
		UE_LOGF(LogPlayerController, Warning, "[%ls] UpdateNetConnectionControllerPosition: Failed to set position of NoPawnPlayerController %ls",
			*FRemoteServerId::GetLocalServerId().ToString(),
			*GetNameSafe(NetConnection->PlayerController));
	}
}

void APlayerController::PostMigrate(const FUObjectMigrationContext& MigrationContext)
{
	auto RemoveFromReplicationSystem = [](UNetDriver* NetDriver, AActor* Actor)
	{
		if (NetDriver->GetReplicationSystem())
		{
			FStopReplicatingActorParams StopRepParams(EEndPlayReason::Destroyed);
			StopRepParams.StopReplicatingReason = UE::Net::EStopReplicatingReason::ProxyReuse;
			UE::Net::FDeferredReplicationSystemCalls::StopReplicatingActorDefer(Actor, StopRepParams);
		}
		else
		{
	...
	auto AddToReplicationSystem = [](UNetDriver* NetDriver, AActor* Actor)
	{
		if (NetDriver->GetReplicationSystem())
		{
			UE::Net::FDeferredReplicationSystemCalls::StartReplicatingActorDefer(Actor);
		}
		else
		{
	...
	// APlayerController::NetConnection is set to null when serialized between servers so it should be null here. This function
	// is expected to set a new value of NetConnection if and when required.
	ensure(NetConnection == nullptr);

	// Make sure we have the prerequisites
	UWorld* World = GetWorld();
	UNetDriver* NetDriver = World ? World->GetNetDriver() : nullptr;
	if (!ensureMsgf(NetDriver, TEXT("%s::%hs: World or NetDriver not set."), *GetName(), __FUNCTION__))
	{
		return;
	}

	// Update the NetConnection on the player controller because it will be set to null when serialized. It's important that the NetConnection variable
	// is always updated, even when borrowed, because there is code that depends on that value and won't work if it's null.
	// We need to do this before Super::PostMigrate because RPCs will not Queue without Player set.
	if (MigrationContext.MigrationSide == EObjectMigrationSide::Receive)
	{
		// Set NetConnection to the local connection that matches the player id in the migrated player controller.
		NetConnection = NetDriver->FindConnection(CachedConnectionPlayerId);
			
		if (!ensureMsgf(NetConnection, TEXT("APlayerController::PostMigrate: Failed to find a NetConnection associated with player id %s"), *CachedConnectionPlayerId.ToDebugString()))
		{
			return;
		}

		// Set Player at the same time as NetConnection because it's used in APlayerController::GetConnection().
	...
	if (MigrationContext.MigrationSide == EObjectMigrationSide::Receive)
	{
		...
		UE_LOGF(LogPlayerController, Verbose, "[%ls] APlayerController::PostMigrate [%ls]: %ls bHasOwnership=%d MigrationSide=Receive RecvType=%ls PlayerId=%ls", 
		...
		const bool bAttachControllerToConnection = (MigrationRecvType == EObjectMigrationRecvType::AssignedOwnership);
		if (bAttachControllerToConnection)
		{	
			// It's assumed the current connection for the player id should be using an ANoPawnPlayerController.
			ensure(NetConnection->PlayerController);
			ensure(NetConnection->PlayerController.IsA<ANoPawnPlayerController>());
			
			ANoPawnPlayerController* PrevNoPawnPlayerController = Cast<ANoPawnPlayerController>(NetConnection->PlayerController);
			if (PrevNoPawnPlayerController)
			{
				// Detach the network connection and its player controller.
				NetConnection->PlayerController = nullptr;
				NetConnection->OwningActor = nullptr;
				PrevNoPawnPlayerController->NetConnection = nullptr;
				PrevNoPawnPlayerController->Player = nullptr;

				// Update the cached ANoPawnPlayerController to account for cases where this connection was established with an
				// instance of ANoPawnPlayerController outside of APlayerController::PostMigrate() (e.g. a connection was opened
				// with NMT_JoinNoPawn or NMT_JoinNoPawnSplit). Without this code we might lose reference to an instance of 
				// ANoPawnPlayerController and create a controller leak.
				NetConnection->CachedNoPawnPlayerController = PrevNoPawnPlayerController;

				// Attach this migrated player controller to the network connection.
				NetConnection->PlayerController = this;
				NetConnection->OwningActor = this;

				// Update the values of this migrated player controller that are associated with the connection.
				NetPlayerIndex = PrevNoPawnPlayerController->NetPlayerIndex;
				SetClientHandshakeId(NetConnection->GetClientHandshakeId());
				LocalPlayerConnectionIdentifier = PrevNoPawnPlayerController->LocalPlayerConnectionIdentifier;
				CachedConnectionPlayerId = PrevNoPawnPlayerController->CachedConnectionPlayerId;

				// Stop replicating the old ANoPawnPlayerController since it's no longer used.
				RemoveFromReplicationSystem(NetDriver, PrevNoPawnPlayerController);

				UpdateOwningNetConnection();

				UE_LOGF(LogPlayerController, Verbose, "[%ls] APlayerController::PostMigrate [%ls]: Binding %ls to connection %ls",
					   *FRemoteServerId::GetLocalServerId().ToString(), *FRemoteObjectId(this).ToString(),
					   *GetName(), *NetConnection->GetName());

				// There should only be one registered controller in the world for this connection and associated player.
				World->RemoveController(PrevNoPawnPlayerController);
	...
	else if (MigrationContext.MigrationSide == EObjectMigrationSide::Send)
	{
		...
		UE_LOGF(LogPlayerController, Verbose, "[%ls] APlayerController::PostMigrate [%ls]: %ls bHasOwnership=%d MigrationSide=Send SendType=%ls PlayerId=%ls",
		...
		const bool bDetachControllerToConnection = (MigrationSendType == EObjectMigrationSendType::ReassignOwnership);

		if (bDetachControllerToConnection)
		{
			// Lookup the local connection that matches the player id in the player controller being migrated away.
			UNetConnection* PrevNetConnection = NetDriver->FindConnection(CachedConnectionPlayerId);
		
			if (!ensureMsgf(PrevNetConnection, TEXT("APlayerController::PostMigrate: Failed to find a connection associated with player id %s"), *CachedConnectionPlayerId.ToDebugString()))
			{
				return;
			}

			if (!ensureMsgf(PrevNetConnection->PlayerController == this, TEXT("APlayerController::PostMigrate: Connection doesn't match player controller being migrated away.")))
			{
				return;
			}

			if (ensure(PrevNetConnection->PlayerController))
			{
				// It's assumed this connection has a player controller associated that is not an instance of ANoPawnPlayerController.
				ensure(!PrevNetConnection->PlayerController.IsA<ANoPawnPlayerController>());
				ensure(PrevNetConnection->PlayerController != PrevNetConnection->CachedNoPawnPlayerController);

				// Detach the network connection and its player controller.
				PrevNetConnection->PlayerController->NetConnection = nullptr;
				PrevNetConnection->PlayerController->Player = nullptr;
				PrevNetConnection->PlayerController = nullptr;
				PrevNetConnection->OwningActor = nullptr;

				auto BindAndInitializeConnectionAndController = [](UNetConnection* Connection, ANoPawnPlayerController* NoPawnPlayerController, const APlayerController* OldPlayerController)
				{
					// Bind the connection with the ANoPawnPlayerController.
					Connection->PlayerController = NoPawnPlayerController;
					Connection->OwningActor = NoPawnPlayerController;
					NoPawnPlayerController->NetConnection = Connection;
					NoPawnPlayerController->Player = Connection;

					// Make sure that the ANoPawnPlayerController has the correct connection related state.
					NoPawnPlayerController->CachedConnectionPlayerId = OldPlayerController->CachedConnectionPlayerId;
					NoPawnPlayerController->NetPlayerIndex = OldPlayerController->NetPlayerIndex;
					NoPawnPlayerController->SetClientHandshakeId(Connection->GetClientHandshakeId());
					NoPawnPlayerController->LocalPlayerConnectionIdentifier = OldPlayerController->LocalPlayerConnectionIdentifier;
				};

				// If the cached no pawn player controller was created in a different world, destroy it so that it will be re-created. This
				// can happen during seamless travel and the world is destroyed but the connection remains open.
				if (PrevNetConnection->CachedNoPawnPlayerController)
				{
					if (PrevNetConnection->CachedNoPawnPlayerController->GetWorld() != World)
					{
						UE_LOGF(LogPlayerController, Verbose, "APlayerController::PostMigrate(): Destroying cached instance ANoPawnPlayerController because it was spawned in a different world.");

						PrevNetConnection->CachedNoPawnPlayerController->Destroy();
						PrevNetConnection->CachedNoPawnPlayerController = nullptr;
					}
				}

				// If there is no cached instances of ANoPawnPlayerController, make sure one is spawned.
				if (PrevNetConnection->CachedNoPawnPlayerController == nullptr)
				{
					UE_LOGF(LogPlayerController, Verbose, "APlayerController::PostMigrate(): Spawning a new instance of ANoPawnPlayerController for connection %ls", *PrevNetConnection->GetName());

					FActorSpawnParameters SpawnInfo;
					SpawnInfo.ObjectFlags |= RF_Transient;
					SpawnInfo.bDeferConstruction = true;
					ANoPawnPlayerController* NewNoPawnPlayerController = World->SpawnActor<ANoPawnPlayerController>(ANoPawnPlayerController::StaticClass(), FVector(), FRotator(), SpawnInfo);
					if (ensure(NewNoPawnPlayerController))
					{
						// Attach this new instance of ANoPawnPlayerController to the network connection.
						BindAndInitializeConnectionAndController(PrevNetConnection, NewNoPawnPlayerController, this);

						// Initial configuration for the ANoPawnPlayerController.
						NewNoPawnPlayerController->SetRole(ROLE_Authority);
						NewNoPawnPlayerController->SetReplicates(true);
						NewNoPawnPlayerController->SetAutonomousProxy(true);

						// Finalizing spawning of the controller.
						// When using Iris this will call AActor::UpdateOwningNetConnection() and StartReplication() (immediately and not deferred).
						NewNoPawnPlayerController->FinishSpawning(FTransform());

						// Cache the instances of ANoPawnPlayerController to avoid respawning it on player migration.
						PrevNetConnection->CachedNoPawnPlayerController = NewNoPawnPlayerController;
					}
				}
				else
				{
					ensure(PrevNetConnection->CachedNoPawnPlayerController->IsA<ANoPawnPlayerController>());
				
					ANoPawnPlayerController* CachedNoPawnPlayerController = PrevNetConnection->CachedNoPawnPlayerController;

					// Attach this cached instance of ANoPawnPlayerController to the network connection.
					BindAndInitializeConnectionAndController(PrevNetConnection, CachedNoPawnPlayerController, this);
				
					// Start replicating the ANoPawnPlayerController.
					AddToReplicationSystem(NetDriver, CachedNoPawnPlayerController);

					// Since the player controller is being re-used, ensure that its initial state is the same as if it were just spawned.
					//
					// NOTE: This currently commented out because PostMigrate() can be called while Iris is replicating and Iris will ensure
					//		 when calling AActor::UpdateOwningNetConnection() during replication. This seems to be okay for now since
					//		 none of the state from ANoPawnPlayerController is changing between migrations.
					//CachedNoPawnPlayerController->SetRole(ROLE_Authority);
					//CachedNoPawnPlayerController->SetReplicates(true); // Calls AActor::UpdateOwningNetConnection().
					//CachedNoPawnPlayerController->SetAutonomousProxy(true);
				}

				// Update the position of the ANoPawnPlayerController during migration, so it will be correct when used for relevancy check. 
				UpdateNetConnectionControllerPosition(PrevNetConnection);

				UE_LOGF(LogPlayerController, Verbose, "[%ls] APlayerController::PostMigrate [%ls]: Replacing %ls with new player controller %ls on connection %ls",
					   *FRemoteServerId::GetLocalServerId().ToString(), *FRemoteObjectId(this).ToString(),
				...
				World->AddController(PrevNetConnection->PlayerController);
			}
		}
	}
}
	...
	if (ULocalPlayer* LocalPlayer = Cast<ULocalPlayer>(Player))
	{
		...
		if (bEnableTouchOverEvents)
		{
			for (int32 TouchIndexInt = 0; TouchIndexInt < EKeys::NUM_TOUCH_KEYS; ++TouchIndexInt)
			{
				...
				const FTouchId TouchId(PlayerInput->GetTouchStates()[FingerIndex].DeviceId, FingerIndex);
				UPrimitiveComponent::DispatchTouchOverEvents(TouchId, this, HitResult, PreviousComponent, CurrentComponent);
					...
					if (ServerData->ServerTimeStamp != 0.f)
					{
						...
						if (ServerData->bTriggeringForcedUpdates)
						{
							if (ServerData->ServerTimeStamp > ServerData->ServerTimeLastForcedUpdate)
							{
								...
								UE_LOGF(LogNetPlayerMovement, Log, "Movement detected, resetting forced update state (ServerTimeStamp %.6f > ServerTimeLastForcedUpdate %.6f) Pawn: %ls", ServerData->ServerTimeStamp, ServerData->ServerTimeLastForcedUpdate, *GetNameSafe(GetPawn()));
							...
							else
							{
								...
								if (PawnTimeSinceForcingUpdates > PawnTimeForcedUpdateMaxDuration)
								{
									...
									UE_LOGF(LogNetPlayerMovement, Log, "Setting bForcedUpdateDurationExceeded=true (PawnTimeSinceForcingUpdates %.6f > PawnTimeForcedUpdateMaxDuration %.6f) (bLastRequestNeedsForcedUpdates:%d) Pawn: %ls", PawnTimeSinceForcingUpdates, PawnTimeForcedUpdateMaxDuration, (int32)ServerData->bLastRequestNeedsForcedUpdates, *GetNameSafe(GetPawn()));
						...
						{
							//UE_LOGF(LogPlayerController, Warning, "ForcedMovementTick. PawnTimeSinceUpdate: %f, DeltaSeconds: %f, DeltaSeconds+: %f", PawnTimeSinceUpdate, DeltaSeconds, DeltaSeconds+0.06f);
...
void APlayerController::ClientTravelInternal_Implementation(const FString& URL, ETravelType TravelType, bool bSeamless, FGuid MapPackageGuid)
{
	...
	else
	{
		if (bSeamless)
		{
			UE_LOGF(LogPlayerController, Warning, "Unable to perform seamless travel because TravelType was %i, not TRAVEL_Relative", int32(TravelType));
		}
		// Do the travel.
...
ASpectatorPawn* APlayerController::SpawnSpectatorPawn()
{
	...
	if ((GetSpectatorPawn() == nullptr) && IsLocalController())
	{
		...
		if (AGameStateBase const* const GameState = World->GetGameState())
		{
			if (UClass* SpectatorClass = GameState->SpectatorClass)
			{
				...
				if (SpawnedSpectator)
				{
					...
					UE_LOGF(LogPlayerController, Verbose, "Spawned spectator %ls [server:%d]", *GetNameSafe(SpawnedSpectator), GetNetMode() < NM_Client);
				}
				else
				{
					UE_LOGF(LogPlayerController, Warning, "Failed to spawn spectator with class %ls", *GetNameSafe(SpectatorClass));
				}
			}
		}
		else
		{
			...
			UE_LOGF(LogPlayerController, Verbose, "NULL GameState when trying to spawn spectator!");
		}
	}
...
void APlayerController::UpdateStateInputComponents()
{
	...
	if (StateName == NAME_Inactive && IsLocalController())
	{
		...
		{
			static const FName InactiveStateInputComponentName("PC_InactiveStateInputComponent0");
...
void APlayerController::EndSpectatingState()
{
	if ( PlayerState != NULL )
	{
		...
		{
			UE_LOGF(LogPlayerController, Warning, "Spectator only UPlayer* leaving spectating state");
		}
...
void APlayerController::GetInputTouchState(ETouchIndex::Type FingerIndex, float& LocationX, float& LocationY, bool& bIsCurrentlyPressed) const
{
	...
	if (PlayerInput)
	{
		...
		{
			const FTouchState& State = PlayerInput->GetTouchStates()[FingerIndex];
			LocationX = State.ViewportRelativeLocation.X;
			LocationY = State.ViewportRelativeLocation.Y;
			bIsCurrentlyPressed = State.Pressure != 0;
		}
		else
		{
			bIsCurrentlyPressed = false;
			UE_LOGF(LogPlayerController, Warning, "Requesting information for invalid finger index.");
		}
	}
	else
	{
...
void APlayerController::EnableInput(class APlayerController* PlayerController)
{
	...
	{
		UE_LOGF(LogPlayerController, Error, "EnableInput can only be specified on a PlayerController for itself");
	}
}
...
void APlayerController::DisableInput(class APlayerController* PlayerController)
{
	...
	{
		UE_LOGF(LogPlayerController, Error, "DisableInput can only be specified on a PlayerController for itself");
	}
}
...
{
	static const FString DisplayName = "Base";
	return DisplayName;
}
#endif	// UE_ENABLE_DEBUG_DRAWING
...
{
	static const FString DebugName = "UI Only (Input will only be consumed by the UI, not the player!)";
	return DebugName;
}
#endif	// UE_ENABLE_DEBUG_DRAWING
...
{
	static const FString DisplayName = "Game and UI";
	return DisplayName;
}
#endif	// UE_ENABLE_DEBUG_DRAWING
...
{
	static const FString DisplayName = "Game Only (Input will only be consumed by the player, not UI)";
	return DisplayName;
}
#endif	// UE_ENABLE_DEBUG_DRAWING
...
{
#if UE_SERVER && UE_AUTORTFM && UE_WITH_REMOTE_OBJECT_HANDLE
	if (!AutoRTFM::IsClosed())
	{
		if (PlayerCameraManager.GetResidence() != EResidence::Local)
		{
			UE_LOGF(LogPlayerController, VeryVerbose, "%s: Skipping due to PlayerCameraManager being non-local", __FUNCTION__);
			return;
		}
	}
#endif
...
{
	UE_LOGF(LogPlayerController, Warning, "APlayerController::ClientRepObjRef repped: %ls", Object ? *Object->GetName() : TEXT("NULL") );
}
...
void APlayerController::SetTickOffsetBufferTime(float NewBufferTime)
{
	TickOffsetBufferTime = FMath::Max(0.0f, NewBufferTime);
};
...
void APlayerController::ServerSendLatestAsyncPhysicsTimestamp_Implementation(FAsyncPhysicsTimestamp Timestamp)
{
	...
	if (FMath::IsNearlyZero(AsyncFixedTimeStepSize))
	{
		return;
	}
	...
	const int32 BufferTickSize = FMath::CeilToInt((TickOffsetBufferTime * 0.001f) / AsyncFixedTimeStepSize);
		...
		|| CurrentTime - NetworkPhysicsTickOffsetDesyncAccumulatedTime > (NetworkPhysicsCvars::TickOffsetCorrectionTimeLimit * 0.001f))
	{
		...
		UE_LOGF(LogPlayerController, Log, "APlayerController::ServerSendLatestAsyncPhysicsTimestamp_Implementation. Sync physics tick with client. ClientFrame: %d, ServerFrame: %d, BufferSize: %d, PredictedFrame: %d)"
...
#if DEBUG_NETWORK_PHYSICS
	UE_LOGF(LogPlayerController, Log, "APlayerController::ClientSetupNetworkPhysicsTimestamp_Implementation. ClientFrame: %d, ServerFrame: %d, NetworkPhysicsTickOffset: %d)"
...
{
	UE_LOGF(LogNet, Log, "%ls replication for connections using %ls (previously %ls).",
...
#if UE_WITH_REMOTE_OBJECT_HANDLE
FRemoteServerId ANoPawnPlayerController::GetPreferredRemoteServer() const
{
	// NoPawnPlayerControllers are server-specific & never migrate servers
	return UE::RemoteObject::Handle::GetOwnerServerId(this);
}
#endif
