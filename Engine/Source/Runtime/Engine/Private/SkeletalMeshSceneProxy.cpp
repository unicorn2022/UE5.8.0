...
static TAutoConsoleVariable<int32> CVarMeshCardRepresentationSkeletalMesh(
	...
	FConsoleVariableDelegate::CreateLambda([](IConsoleVariable* InVariable) { FSkinnedMeshComponentRecreateRenderStateContext Context(nullptr); }),
...
FSkeletalMeshSceneProxy::FSkeletalMeshSceneProxy(const USkinnedMeshComponent* Component, FSkeletalMeshRenderData* InRenderData)
	: FSkeletalMeshSceneProxy(FSkinnedMeshSceneProxyDesc(Component), InRenderData)
{ }

FSkeletalMeshSceneProxy::FSkeletalMeshSceneProxy(const FSkinnedMeshSceneProxyDesc& InMeshDesc, FSkeletalMeshRenderData* InRenderData, int32 InMinLODLevel)
	: FSkeletalMeshSceneProxy(InMeshDesc, InRenderData)
{ }
...
FSkeletalMeshSceneProxy::FSkeletalMeshSceneProxy(const FSkinnedMeshSceneProxyDesc& InMeshDesc, FSkeletalMeshRenderData* InRenderData)
		...
		,	MinLODLevel(InMeshDesc.MinLODLevel)
		,	ForceStreamedLOD(InMeshDesc.ForceStreamedLOD)
		...
		,	WPODisableDistance(static_cast<float>(InMeshDesc.WorldPositionOffsetDisableDistance))
#if RHI_RAYTRACING
...
#endif
{
...
#if RHI_RAYTRACING
	bHasRayTracingRepresentation = RenderData->bSupportRayTracing && (bRenderStatic || CVarRayTracingSupportSkeletalMeshes.GetValueOnAnyThread() != 0);
	...
	const bool bUsePSOPrecacheFallbackMaterial = InMeshDesc.UsePSOPrecacheFallbackMaterial();
	...
	bool bAnySectionHasSkelMeshUsage = false;
	bool bAnySectionHasMeshDeformerUsage = false;
	LODSections.Reserve(RenderData->LODRenderData.Num());
	LODSections.AddZeroed(RenderData->LODRenderData.Num());
	for(int32 LODIdx=MinLODLevel; LODIdx < RenderData->LODRenderData.Num(); LODIdx++)
	{
		...
		for(int32 SectionIndex = 0;SectionIndex < LODData.RenderSections.Num();SectionIndex++)
		{
			...
			bool bUseDefaultMaterial = (GForceDefaultMaterial && Material && !IsTranslucentBlendMode(*Material));
			if (bUsePSOPrecacheFallbackMaterial || bUseDefaultMaterial)
			{
				Material = (bUseDefaultMaterial || UPSOPrecacheSettingsManager::GetFallbackMaterial() == nullptr) ? UMaterial::GetDefaultMaterial(MD_Surface) : UPSOPrecacheSettingsManager::GetFallbackMaterial();
				MaterialRelevance |= Material->GetRelevance(GetScene().GetShaderPlatform());
			}

			// validate usage based on technique
			const bool bUsesMeshDeformer = InMeshDesc.GetMeshDeformerInstanceForLOD(LODIdx) != nullptr;
			const EMaterialUsage RequiredUsage = bUsesMeshDeformer ? MATUSAGE_MeshDeformer : MATUSAGE_SkeletalMesh;
			bAnySectionHasMeshDeformerUsage |= RequiredUsage == MATUSAGE_MeshDeformer;
			bAnySectionHasSkelMeshUsage |= RequiredUsage == MATUSAGE_SkeletalMesh;
			bool bValidUsage = Material && Material->CheckMaterialUsage_Concurrent(RequiredUsage);

			// Instanced skinned meshes use inline GPU skinning and need their own usage flag
			// to ensure shader permutations are compiled even when r.SkinCache.CompileShaders=2.
			if (bInstancedSkinnedMesh)
			{
				bValidUsage &= Material && Material->CheckMaterialUsage_Concurrent(MATUSAGE_InstancedSkinnedMesh);
			}
			...
			{
				UE_CLOGF(Material && !bValidUsage, LogSkeletalMesh, Warning,
					"Material with missing usage flag was applied to skeletal mesh %ls",
			...
			if (SectionOverlayMaterial != nullptr)
			{
				if (!SectionOverlayMaterial->CheckMaterialUsage_Concurrent(MATUSAGE_SkeletalMesh))
				{
					...
					UE_LOGF(LogSkeletalMesh, Warning, "Overlay material per section with missing usage flag was applied to skeletal mesh[%ls] LOD %d, section index %d.", *InMeshDesc.GetSkinnedAsset()->GetPathName(), LODIdx, SectionIndex);
					FMaterialOverlayHelper::ForceMaterial(MaterialSlotsOverlayMaterial, UseMaterialIndex, SectionOverlayMaterial);
				}
				else if (bUsePSOPrecacheFallbackMaterial)
				{
					SectionOverlayMaterial = nullptr;
					FMaterialOverlayHelper::ForceMaterial(MaterialSlotsOverlayMaterial, UseMaterialIndex, SectionOverlayMaterial);
				}
	...
	{
		bool bInvalidUsage = false;
		if (bAnySectionHasSkelMeshUsage && !OverlayMaterial->CheckMaterialUsage_Concurrent(MATUSAGE_SkeletalMesh))
		{
			bInvalidUsage = true;
			UE_LOGF(LogSkeletalMesh, Warning, "Overlay material with missing skel mesh usage flag was applied to skeletal mesh %ls", *InMeshDesc.GetSkinnedAsset()->GetPathName());
		}
		if (bAnySectionHasMeshDeformerUsage && !OverlayMaterial->CheckMaterialUsage_Concurrent(MATUSAGE_MeshDeformer))
		{
			bInvalidUsage = true;
			UE_LOGF(LogSkeletalMesh, Warning, "Overlay material with missing mesh deformer usage flag was applied to skeletal mesh %ls", *InMeshDesc.GetSkinnedAsset()->GetPathName());
		}
		if (bInvalidUsage || bUsePSOPrecacheFallbackMaterial)
		{
	...
	// True by constructors of proxy types which support gathering of UStreamableAssets
	bImplementsStreamableAssetGathering = true;

	EnableGPUSceneSupportFlags();
	bMobileGPUScene = UseGPUScene(GetScene().GetShaderPlatform()) && PlatformGPUSceneUsesUniformBufferView(GetScene().GetShaderPlatform());
	bVFRequiresPrimitiveUniformBuffer |= bMobileGPUScene;
...
void FSkeletalMeshSceneProxy::GetStreamableRenderAssetInfo(const FBoxSphereBounds& InPrimitiveBounds, TArray<FStreamingRenderAssetPrimitiveInfo>& OutStreamableRenderAssets) const
{
	constexpr float ComponentScaling = 1.0f;
	const EMaterialQualityLevel::Type QualityLevel = GetCurrentMaterialQualityLevelChecked();
	for (const FLODSectionElements& LOD : LODSections)
	{
		for (const FSectionElementInfo& Section : LOD.SectionElements)
		{
			if (const UMaterialInterface* ShadingMaterial = Section.Material)
			{
				const static FMeshUVChannelInfo Fallback(1.0f);
				const FMeshUVChannelInfo* UVChannelData = SkinnedAsset->GetUVChannelData(Section.UseMaterialIndex);
				const FMeshUVChannelInfo* UsedUVChannelData = UVChannelData ? UVChannelData : &Fallback;

				if (TSharedPtr<const FMaterialCachedTexturesSamplingInfo> CachedSamplingInfo = ShadingMaterial->GetCachedTexturesSamplingInfo())
				{
					CachedSamplingInfo->GetStreamingRenderAssetPrimitiveInfo(InPrimitiveBounds, ComponentScaling, *UsedUVChannelData, QualityLevel, OutStreamableRenderAssets);
				}
				else
				{
	...
	if (SkinnedAsset->IsStreamable())
	{
		// Positive texel factor = component sphere bound diameter (mesh texel factors aren't UV densities, so bAffectedByComponentScale must be false).
		// Negative texel factor encodes a forced streamed LOD index for the streaming manager: ForceStreamedLOD must be < LODNum
		// so the subtraction yields a strictly negative value. USkinnedMeshComponent::SetForceStreamedLOD clamps to [0, NumLODs-1],
		// but FSkinnedMeshSceneProxyDesc::ForceStreamedLOD is a plain int32 field with no clamp, and direct desc writers
		// (e.g. FastGeo serialization) bypass that path. Clamp defensively here.
		const int32 LODNum = SkinnedAsset->GetLODNum();
		if (LODNum > 0)
		{
		    ensureMsgf(ForceStreamedLOD < LODNum, TEXT("ForceStreamedLOD (%d) >= LODNum (%d) on %s; clamping to LODNum-1 to keep TexelFactor strictly negative."), ForceStreamedLOD, LODNum, *SkinnedAsset->GetName());
		    const float TexelFactor = ForceStreamedLOD >= 0
			    ? static_cast<float>(FMath::Min(ForceStreamedLOD, LODNum - 1) - LODNum)
			    : InPrimitiveBounds.SphereRadius * 2.0f;
		    const bool bAllowInvalidTexelFactorWhenUnregistered = false;
		    const bool bAffectedByComponentScale = false;
		    OutStreamableRenderAssets.Add(FStreamingRenderAssetPrimitiveInfo(SkinnedAsset, InPrimitiveBounds, TexelFactor, PackedRelativeBox_Identity, bAllowInvalidTexelFactorWhenUnregistered, bAffectedByComponentScale));
		}
	}
}
...
void FSkeletalMeshSceneProxy::DrawStaticElements(FStaticPrimitiveDrawInterface* PDI)
{
	...
	if (!HasViewDependentDPG())
	{
		...
		for (int32 LODIndex = MinLODLevel; LODIndex < NumLODs; ++LODIndex)
		{
			const FSkeletalMeshLODRenderData& LODData = RenderData->LODRenderData[LODIndex];
...
void FSkeletalMeshSceneProxy::GetMeshElementsConditionallySelectable(const TArray<const FSceneView*>& Views, const FSceneViewFamily& ViewFamily, bool bInSelectable, uint32 VisibilityMap, FMeshElementCollector& Collector) const
{
	...
	{
#if DO_CHECK
		UE_LOGF(LogSkeletalMesh, Warning, "Skeletal mesh %ls has no valid LODs for rendering.", *GetResourceName().ToString());
#endif
	}
	else
	{
	...
	FMeshBatch& OutMeshBatch) const
{
	...
	BatchElement.bForceInstanceCulling = bMobileGPUScene && VertexFactory && !VertexFactory->GetType()->SupportsGPUSkinPassThrough();

	if (bUseGpuLodSelection)
	{
		BatchElement.MaxScreenSize = MeshObject->GetScreenSize(LODIndex);
		BatchElement.MinScreenSize = 0.0f;
		if (LODIndex < MAX_STATIC_MESH_LODS - 1)
		{
			BatchElement.MinScreenSize = MeshObject->GetScreenSize(LODIndex + 1);
		}
	}
}
...
void FSkeletalMeshSceneProxy::CreateBaseMeshBatch(const FSceneView* View, const FSkeletalMeshLODRenderData& LODData, const int32 LODIndex, const int32 SectionIndex, const FSectionElementInfo& SectionElementInfo, FMeshBatch& Mesh, ESkinVertexFactoryMode VFMode) const
{
	...
	BatchElement.NumPrimitives = Section.NumTriangles;
	BatchElement.PrimitiveUniformBuffer = GetUniformBuffer();
	BatchElement.bForceInstanceCulling = bMobileGPUScene && Mesh.VertexFactory && !Mesh.VertexFactory->GetType()->SupportsGPUSkinPassThrough();
}
...
uint8 FSkeletalMeshSceneProxy::GetStreamingLODWantedDeficit() const
{
	if (SkinnedAsset)
	{
		const FStreamableRenderResourceState State = SkinnedAsset->GetStreamableResourceState();
		const uint8 WantedLODs = SkinnedAsset->GetCachedNumWantedLODs();
		if (WantedLODs > 0 && State.NumResidentLODs > 0)
		{
			return static_cast<uint8>(FMath::Max(0, (int32)WantedLODs - (int32)State.NumResidentLODs));
		}
	}
	return 0;
}
...
{
	const int32 LODIndex = MeshObject->GetLOD();
	check(LODIndex >= MinLODLevel);
	return FDesiredLODLevel::CreateFixed(LODIndex);
}
		...
		if (VisibilityMap & (1 << ViewIndex))
		{
			...
			if (ActiveOverlayMaterial != nullptr && OverlayMaterialMaxDrawDistance >= 0.f)
			{
				...
				if (bAddOverlay)
				{
					...
					GetOverlayMeshBatch(LODData, ActiveOverlayMaterial, Mesh, OverlayMeshBatch);
...
{
	if (IsRayTracingEnabled() && bHasRayTracingRepresentation && bRayTraceStatic)
	{
...
void FSkeletalMeshSceneProxy::GetStaticRayTracingInstances(FStaticRayTracingInstances& OutRayTracingInstances)
{
	const ESceneDepthPriorityGroup PrimitiveDPG = GetStaticDepthPriorityGroup();
	const bool bIgnorePreviewFilter = true;

	OutRayTracingInstances.LODs.SetNum(NumLODs);
	...
	for (int32 LODIndex = MinLODLevel; LODIndex < NumLODs; ++LODIndex)
	{
		...
		if (LODSections.Num() > 0 && LODData.GetNumVertices() > 0)
		{
			...
			for (FSkeletalMeshSectionIter Iter(LODIndex, *MeshObject, LODData, LODSection, bIgnorePreviewFilter); Iter; ++Iter)
			{
				...
				RayTracingMaterials.Add(MeshElement);
				...
				if (SectionOverlayMaterial != nullptr)
				{
					...
					RayTracingMaterials.Add(OverlayMeshBatch);
				}
			}
			...
			FRayTracingInstance& OutRayTracingInstance = OutRayTracingInstances.LODs[LODIndex].Instances.AddDefaulted_GetRef();
			OutRayTracingInstance.Geometry = &LODData.StaticRayTracingGeometry;
			OutRayTracingInstance.Materials = MoveTemp(RayTracingMaterials);
		}
	}
}
...
{
	if (!ensure(CVarRayTracingSkeletalMeshes.GetValueOnRenderThread() && CVarRayTracingSupportSkeletalMeshes.GetValueOnRenderThread()))
	{
		return;
	}
	...
	//#hwrt_todo (FORT-944645): verify why this condition is not fulfilled sometimes
	if (!ensureMsgf(LODIndex == RayTracingGeometry->LODIndex,
		TEXT("Mismatch between MeshObject ray tracing LOD and RayTracingGeometry (%s)."), *MeshObject->GetDebugName().ToString()))
	{
		return;
	}
	...
	//#hwrt_todo (UE-113617): verify why this condition is not fulfilled sometimes
	if (!ensure(LODSection.SectionElements.Num() == RayTracingGeometry->GetInitializer().Segments.Num()))
	{
		return;
	}
	...
	if (CollectorMeshBatches.IsEmpty())
	{
		return;
	}
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
		...
		UpdateParams.NumVertices = LODData.GetNumVertices();
		UpdateParams.VertexBufferSize = LODData.GetNumVertices() * (uint32)sizeof(FVector3f);
		UpdateParams.NumTriangles = RayTracingGeometry->GetInitializer().TotalPrimitiveCount;
		UpdateParams.Geometry = RayTracingGeometry;
		UpdateParams.Buffer = MeshObject->GetRayTracingDynamicVertexBuffer();
	...
	RayTracingInstance.NumTransforms = 1; // Instance transforms are copied from GPUScene while building ray tracing instance buffer.
...
void FSkeletalMeshSceneProxy::DebugDrawSkeleton(int32 ViewIndex, FMeshElementCollector& Collector, const FEngineShowFlags& EngineShowFlags) const
{
	...
	if (!MeshObject || !MeshObject->GetComponentSpaceTransforms())
	{
		return;
	}
	...
	const int32 NumBones = FMath::Min(ComponentSpaceTransforms.Num(), RefSkeleton.GetNum());
...
bool FSkeletalMeshSceneProxy::GetInstanceWorldPositionOffsetDisableDistance(float& OutWPODisableDistance) const
{
	OutWPODisableDistance = WPODisableDistance;
	return WPODisableDistance > 0.f;
}

void FSkeletalMeshSceneProxy::SetWorldPositionOffsetDisableDistance_GameThread(int32 NewValue)
{
	check(IsInGameThread());
	ENQUEUE_RENDER_COMMAND(CmdSetSkelMeshWPODisableDistance)(
		[this, NewValue](FRHICommandList&)
		{
			const float NewFloatValue = static_cast<float>(NewValue);
			const bool bUpdate = !FMath::IsNearlyEqual(WPODisableDistance, NewFloatValue);
			const bool bToggleZero = bUpdate && (WPODisableDistance == 0.f || NewFloatValue == 0.f);
			if (bUpdate)
			{
				WPODisableDistance = NewFloatValue;
				if (FPrimitiveSceneInfo* PrimitiveSceneInfo = GetPrimitiveSceneInfo())
				{
					GetScene().RequestUniformBufferUpdate(*PrimitiveSceneInfo);
					GetScene().RequestGPUSceneUpdate(*PrimitiveSceneInfo, EPrimitiveDirtyState::ChangedOther);
					if (bToggleZero)
					{
						GetRendererModule().RequestStaticMeshUpdate(PrimitiveSceneInfo);
					}
				}
			}
		});
