...
void FDistanceFieldAsyncQueue::OnAssetPostCompile(const TArray<FAssetCompileData>& CompiledAssets)
{
	for (const FAssetCompileData& CompileData : CompiledAssets)
	{
		...
		{
			const UStaticMesh* StaticMesh = Cast<UStaticMesh>(CompileData.Asset.Get());
			ProcessPendingTasksForMesh(StaticMesh);
		}
	}
}

#if WITH_EDITOR
void FDistanceFieldAsyncQueue::FinalizeAsyncTask(FAsyncDistanceFieldTask* InTask, FObjectCacheContextScope& InObjectCacheScope) const
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FDistanceFieldAsyncQueue::FinalizeAsyncTask);
	...
	if (InTask->StaticMesh)
	{
		check(!InTask->StaticMesh->IsCompiling());

		InTask->GeneratedVolumeData->bAsyncBuilding = false;
		...
		RenderData->LODResources[0].DistanceFieldData = InTask->GeneratedVolumeData;
		...
		while (PlatformRenderData)
		{
			...
			{
				*PlatformRenderData->LODResources[0].DistanceFieldData = *InTask->GeneratedVolumeData;
				// The old bulk data assignment operator doesn't copy over flags
				PlatformRenderData->LODResources[0].DistanceFieldData->StreamableMips.ResetBulkDataFlags(InTask->GeneratedVolumeData->StreamableMips.GetBulkDataFlags());
			}
...

		{
			...
			InTask->StaticMesh->GetRenderData()->LODResources[0].DistanceFieldData->Serialize(Ar, InTask->StaticMesh);
		...
		if (!InTask->bGeneratedCardRepresentation)
		{
			BeginCacheMeshCardRepresentation(
				InTask->StaticMesh,
				InTask->StaticMesh->GetPlatformStaticMeshRenderData(InTask->StaticMesh, InTask->TargetPlatform),
				InTask->DDCKey,
				MoveTemp(InTask->SourceMeshData),
				MoveTemp(InTask->PartData));
		}
	}
}
#endif
...
{
	GDistanceFieldAsyncQueue->Build(&Task);
}
...
void FDistanceFieldAsyncQueue::ProcessPendingTasksForMesh(const UStaticMesh *InStaticMesh)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FDistanceFieldAsyncQueue::ProcessPendingTasksForMesh);
	...
	for (auto It = PendingTasks.CreateIterator(); It; ++It)
	{
		...
		if ((Task->StaticMesh == InStaticMesh || Task->GenerateSource == InStaticMesh) &&
			(Task->GenerateSource == nullptr || !Task->GenerateSource->IsCompiling()) && 
			(Task->StaticMesh == nullptr || !Task->StaticMesh->IsCompiling()))
		{
			StartBackgroundTask(Task);
			It.RemoveCurrent();
		}
...
#else
	UE_LOGF(LogStaticMesh,Fatal,"Tried to build a distance field without editor support (this should have been done during cooking)");
#endif
}
...
int32 FDistanceFieldAsyncQueue::GetNumOutstandingTasks(const UStaticMesh* InStaticMesh) const
{
	FScopeLock Lock(&CriticalSection);

	// Pending tasks are already included in referenced tasks
	...
	{
		const FAsyncDistanceFieldTask* Task = *It;
...
void FDistanceFieldAsyncQueue::BlockUntilBuildComplete(UStaticMesh* StaticMesh, bool bWarnIfBlocked)
{
	...
	do 
	{
		ProcessAsyncTasksForMesh(StaticMesh);
	...
	{
		UE_LOGF(LogStaticMesh, Display, "Main thread blocked for %.3fs for async distance field build of %ls to complete!  This can happen if the mesh is rebuilt excessively.",
...
void FDistanceFieldAsyncQueue::ProcessAsyncTasks(bool bLimitExecutionTime)
{
	...
	while (!bLimitExecutionTime || (FPlatformTime::Seconds() - StartTime) < MaxProcessingTime)
	{
		...
		FinalizeAsyncTask(Task, ObjectCacheScope);
...
void FDistanceFieldAsyncQueue::ProcessAsyncTasksForMesh(const UStaticMesh* InStaticMesh, bool bLimitExecutionTime)
{
#if WITH_EDITOR
	TRACE_CPUPROFILER_EVENT_SCOPE(FDistanceFieldAsyncQueue::ProcessAsyncTasksForMesh);
	...
	while (!bLimitExecutionTime || (FPlatformTime::Seconds() - StartTime) < MaxProcessingTime)
	{
		FAsyncDistanceFieldTask* Task = nullptr;
		{
			...
			{
				FAsyncDistanceFieldTask* TaskToCheck = *CompletedTaskIt;
...
void FDistanceFieldAsyncQueue::Shutdown()
{
	...
	UE_LOGF(LogStaticMesh, Log, "Abandoning remaining async distance field tasks for shutdown");
	ThreadPool.Reset();
}
...
public:
	...
	{
		return IsFeatureLevelSupported(Parameters.Platform, ERHIFeatureLevel::SM5) && ShouldCompileDistanceFieldShaders(Parameters.Platform);
	}
