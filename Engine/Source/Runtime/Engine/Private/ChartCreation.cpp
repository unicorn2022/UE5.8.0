...
void FDumpFPSChartToEndpoint::DumpChart(double InWallClockTimeFromStartOfCharting, FString InMapName, FString InDeviceProfileName)
{
	...
	{
		UE_LOGF(LogChartCreation, Log, "Weirdness: wall clock time (%f) is smaller than total frame time (%f)", WallClockTimeFromStartOfCharting, TotalTime);
	}
...
struct FDumpFPSChartToLogEndpoint : public FDumpFPSChartToEndpoint
{
	...
	virtual void PrintToEndpoint(const FString& Text) override
	{
		UE_LOGF(LogChartCreation, Log, "%ls", *Text);
	}
};
...
void FPerformanceTrackingChart::Reset(const FDateTime& InStartTime)
{
	...
	StartTemperature = -1.0f;
	StopTemperature = -1.0f;
	StartThermalState = EDeviceThermalState::Unsupported;
	StopThermalState = EDeviceThermalState::Unsupported;
...
void FPerformanceTrackingChart::AccumulateWith(const FPerformanceTrackingChart& Chart)
{
	...
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	StopTemperature = Chart.StopTemperature;
	StopThermalState = Chart.StopThermalState;
...
{
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	StartTemperatureLevel = FPlatformMisc::GetDeviceTemperatureLevel(); // Deprecated in 5.8 in favor of the two stats below.
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	StartTemperature = FPlatformMisc::GetDeviceTemperature();
	StartThermalState = FPlatformMisc::GetDeviceThermalState();
...
{
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	StopTemperatureLevel = FPlatformMisc::GetDeviceTemperatureLevel(); // Deprecated in 5.8 in favor of the two stats below.
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	StopTemperature = FPlatformMisc::GetDeviceTemperature();
	StopThermalState = FPlatformMisc::GetDeviceThermalState();
...
void FPerformanceTrackingChart::DumpChartsToLogFile(double WallClockElapsed, const TArray<const FPerformanceTrackingChart*>& Charts, const FString& InMapName, const FString& LogFileName)
{
	...
	if (FArchive* OutputFile = IFileManager::Get().CreateDebugFileWriter(*LogFileName, FILEWRITE_Append))
	{
		...
		UE_LOGF(LogProfilingDebugging, Warning, "FPS Chart (logfile) saved to %ls", *AbsolutePath);
...
void FPerformanceTrackingChart::DumpChartToAnalyticsParams(const FString& InMapName, TArray<struct FAnalyticsEventAttribute>& InParamArray, bool bIncludeClientHWInfo, bool bIncludeHistograms) const
{
	...
	if ((GetTotalTime() > 0.0) && (GetNumFrames() > 0))
	{
		...
		if (bIncludeClientHWInfo)
		{
			...
			InParamArray.Add(FAnalyticsEventAttribute(TEXT("StartTemperature"), StartTemperature));
			InParamArray.Add(FAnalyticsEventAttribute(TEXT("StopTemperature"), StopTemperature));
			InParamArray.Add(FAnalyticsEventAttribute(TEXT("StartThermalState"), (int)StartThermalState));
			InParamArray.Add(FAnalyticsEventAttribute(TEXT("StopThermalState"), (int)StopThermalState));
...
IPerformanceDataConsumer::FFrameData FPerformanceTrackingSystem::AnalyzeFrame(float DeltaSeconds)
{
	...
	if (ShouldExcludeIdleTimeFromCharts())
	{
		...
		{
			UE_LOGF(LogChartCreation, Warning, "Idle time for this frame (%f) is larger than delta between FPSChart ticks (%f)", ThisFrameIdleTime, DeltaSeconds);
		}
	}
...
void FPerformanceTrackingSystem::StartCharting()
{
	...
	UE_LOGF(LogChartCreation, Log, "Started creating FPS charts at %f seconds", FPSChartStartTime);
}
...
void FPerformanceTrackingSystem::StopCharting()
{
	...
	UE_LOGF(LogChartCreation, Log, "Stopped creating FPS charts at %f seconds", FPSChartStopTime);
}
...
void UEngine::AddPerformanceDataConsumer(TSharedPtr<IPerformanceDataConsumer> Consumer)
{
	...
	UE_LOGF(LogChartCreation, Verbose, "AddPerformanceDataConsumer DataConsumers Count:%d", ActivePerformanceDataConsumers.Num());
}
...
void UEngine::RemovePerformanceDataConsumer(TSharedPtr<IPerformanceDataConsumer> Consumer)
{
	...
	UE_LOGF(LogChartCreation, Verbose, "RemovePerformanceDataConsumer DataConsumers Count:%d", ActivePerformanceDataConsumers.Num());
}
...
// 	}
	...
	if (FFileHelper::LoadFileToString(FPSChart, *HTMLFilename))
	{
		...
		const FString HeaderSeparator = "<UE4></UE4>";
	...
	UE_LOGF(LogProfilingDebugging, Warning, "FPS Chart (HTML) saved to %ls", *IFileManager::Get().ConvertToAbsolutePathForExternalAppForRead(*HTMLFilename));
}
