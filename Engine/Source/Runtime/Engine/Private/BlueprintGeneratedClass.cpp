...
namespace UE::Runtime::Engine::Private
{
	struct FBlueprintGeneratedClassUtils
	{
		...
		static bool ShouldInitializePropertyDuringPostConstruction(const FProperty& Property)
		{
			...
			const bool bIsForcePostConstructLink = Property.HasAnyPropertyFlags(CPF_ForcePostConstructLink);

			// Skip config and forced post construct link properties as they're already in the PostConstructLink chain. Also skip transient properties if they contain a reference to an instanced subobjects (as those should not be initialized from defaults).
			if (!bIsConfigProperty && !bIsForcePostConstructLink &&(!bIsTransientProperty || !Property.ContainsInstancedObjectProperty()))
			{
				return true;
			}
			...
			if (!ObjValue)
			{
				...
				if (ObjValue && ObjValue->IsDefaultSubobject() && ObjValue->HasAllFlags(RF_DefaultSubObject))
				{
					...
					if (CurrentValue)
					{
						...
						{
							UE_LOGF(LogBlueprint, Warning, "%ls: Detected a NULL reference value for the class member named (%ls). Changes to this property may not be restored on load. Check to see if any changes need to be re-applied, then re-save the asset to fix this warning.", *InOuter->GetPathName(), *InProperty->GetName());
					...
					else
					{
						...
						UE_LOGF(LogBlueprint, Warning, "%ls: Missing a default subobject instance named \'%ls\'. This should have been instanced at construction time for \'%ls\'.", *InOuter->GetPathName(), *ObjValue->GetName(), *InProperty->GetOwnerStruct()->GetName());
					}
				}
			}
...
void UBlueprintGeneratedClass::GetAssetRegistryTags(FAssetRegistryTagsContext Context) const
{
	 ...
	 */
	if (UE::IsSavingPackage() && !IsRunningCookCommandlet())
	{
...
UClass* UBlueprintGeneratedClass::GetAuthoritativeClass()
{
 	if (nullptr == ClassGeneratedBy)
 	{
		...
		{
			UE_LOGF(LogBlueprint, Fatal, "UBlueprintGeneratedClass::GetAuthoritativeClass: ClassGeneratedBy is null. class '%ls'", *GetPathName());
		}
 	}
...
{
	UE::TScopeLock SerializeAndPostLoadLock(SerializeAndPostLoadCritical);
		...
		auto BuildCachedPropertyDataLambda = [BPGCName](FBlueprintCookedComponentInstancingData& CookedData, UActorComponent* SourceTemplate, FString CompVarName)
		{
			if (CookedData.bHasValidCookedData)
			{
				...
				else
				{
					...
					UE_LOGF(LogBlueprint, Warning, "BPComp fast path (%ls.%ls) : Invalid source template. Will use slow path for dynamic instancing.", *BPGCName, *CompVarName);
...
void UBlueprintGeneratedClass::PostInitInstance(UObject* InObj, FObjectInstancingGraph* InstanceGraph)
{
	GetSuperClass()->PostInitInstance(InObj, InstanceGraph);
}

void UBlueprintGeneratedClass::PostLoadInstance(UObject* InObj)
{
	GetSuperClass()->PostLoadInstance(InObj);
}
...
UInheritableComponentHandler* UBlueprintGeneratedClass::GetInheritableComponentHandler(const bool bCreateIfNecessary)
{
	...
	{
		InheritableComponentHandler = NewObject<UInheritableComponentHandler>(this, FName("InheritableComponentHandler"));
	}
	...
	if (SimpleConstructionScript && !IsChildOf<ALevelScriptActor>())
	{
		...
		while (Class)
		{
			...
			else if (UInheritableComponentHandler* ICH = Class->GetInheritableComponentHandler())
			{
				...
				{
					// We must preload here to avoid creating exports with an uninitialized archetype
					ICH->PreloadAll();
				}
		...
		if(Archetype == nullptr)
		{
			// We failed to find an archetype by name - but the ICH mangles
			// the DefaultSceneRoot name *and* tries to cull unused templates
			// so do a relaxed compare to find the real default scene root:
			static const FName DefaultSceneRootSCS("DefaultSceneRoot_GEN_VARIABLE");
			static const FName ICHDefaultSceneRoot("ICH-DefaultSceneRoot_GEN_VARIABLE");
			if(ArchetypeName == DefaultSceneRootSCS || ArchetypeName == ICHDefaultSceneRoot)
			{
				Class = const_cast<UBlueprintGeneratedClass*>(this);
				while (Class && !Archetype)
				{
					// look for an SCS template:
					if(const USimpleConstructionScript* SCS = Class->SimpleConstructionScript)
					{
						if(const USCS_Node* Node = SCS->GetDefaultSceneRootNode())
						{
							// Assets may have orphaned default scene root nodes - only honor
							// them if they still have a spot in the root nodes list:
							if(SCS->GetRootNodes().Contains(Node))
							{
								Archetype = Node->ComponentTemplate;
							}
						}
					}
...
void UBlueprintGeneratedClass::BindDynamicDelegates(const UClass* ThisClass, UObject* InInstance)
{
	...
	{
		UE_LOGF(LogBlueprint, Warning, "BindComponentDelegates: '%ls' is not an instance of '%ls'.", *InInstance->GetName(), *ThisClass->GetName());
		return;
	}
...
void UBlueprintGeneratedClass::UnbindDynamicDelegates(const UClass* ThisClass, UObject* InInstance)
{
	...
	if (!InInstance->IsA(ThisClass))
	{
		UE_LOGF(LogBlueprint, Warning, "UnbindDynamicDelegates: '%ls' is not an instance of '%ls'.", *InInstance->GetName(), *ThisClass->GetName());
		return;
	}
...
void UBlueprintGeneratedClass::CreatePersistentUberGraphFrame(UObject* Obj, bool bCreateOnlyIfEmpty, bool bSkipSuperClass, UClass* OldClass) const
{
	...
	if (Obj && UsePersistentUberGraphFrame() && UberGraphFramePointerProperty && UberGraphFunction)
	{
		...
		if (!PointerToUberGraphFrame->RawPointer)
		{
			...
			{
				UE_LOGF(LogBlueprint, Verbose, "Function '%ls' is not ready to create frame for '%ls'",
...
void UBlueprintGeneratedClass::DestroyPersistentUberGraphFrame(UObject* Obj, bool bSkipSuperClass) const
{
	...
	if (Obj && UsePersistentUberGraphFrame() && UberGraphFramePointerProperty && UberGraphFunction)
	{
		...
		{
			UE_LOGF(LogBlueprint, Log, "Object '%ls' had no Uber Graph Persistent Frame", *GetPathNameSafe(Obj));
		}
	}
	...
	if (HasAnyFlags(RF_BeingRegenerated))
	{
		...
		if(bCooked)
		{
			...
			while (const UBlueprintGeneratedClass* ParentBPGC = Cast<const UBlueprintGeneratedClass>(CurrentBPGC->GetSuperClass()))
			{
				...
				{
					UE_LOGF(LogBlueprint, Warning, "%ls: found an uncooked class (%ls) as an ancestor of a cooked class (%ls) in the hierarchy.",
						*this->GetPathName(),
...
void UBlueprintGeneratedClass::PreSaveRoot(FObjectPreSaveRootContext ObjectSaveContext)
{
	...
	{
		BP->GeneratedClassPreSaveRoot(ObjectSaveContext);
	}
}
...
void UBlueprintGeneratedClass::PostSaveRoot(FObjectPostSaveRootContext ObjectSaveContext)
{
	...
	if (UBlueprint* BP = Cast<UBlueprint>(ClassGeneratedBy))
	{
		BP->GeneratedClassPostSaveRoot(ObjectSaveContext);
	}	
}
#endif
...
void UBlueprintGeneratedClass::Link(FArchive& Ar, bool bRelinkExistingProperties)
{
	...
	// If our parent class needs to be instanced on load, this class should also inherit that trait.
	bNeedsPostLoadSubobjectInstancing = GetSuperClass()->RequiresPostLoadSubobjectInstancing();
}
	...
	virtual void HandleObjectReference(UObject*& Object, const UObject* Referencer, const FProperty* ReferencingProperty) override
	{
		...
		if (!bIsValidObjectReference)
		{
			if (const UFunction* UberGraphFunction = Cast<UFunction>(Referencer))
			{
				...
				UE_LOGF(LogUObjectGlobals, Log, "PersistentFrame: Addr=0x%016llx, Size=%d%ls",
...
{
	static const FName UberGraphFrameName("UberGraphFrame");
