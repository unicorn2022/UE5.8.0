...
void UBlueprintCore::Serialize(FArchive& Ar)
{
		...
		&& (Ar.UEVer() != VER_UE4_BLUEPRINT_SKEL_TEMPORARY_TRANSIENT))
	{
		...
		if( SkeletonGeneratedClass )
		{
			...
			GetObjectsWithOuter(SkeletonGeneratedClass, SubObjs, EGetObjectsFlags::IncludeNestedObjects);
...
void UBlueprint::PreSave(FObjectPreSaveContext ObjectSaveContext)
{
	...
	if (!TargetPlatform || TargetPlatform->HasEditorOnlyData())
	{
		...
		FBlueprintEditorUtils::RecompileBeforeSaveIfNeeded(this);
	}
}
...
bool UBlueprint::RenameGeneratedClasses(const TCHAR* NewName, UObject* NewOuter, ERenameFlags Flags)
{
	...
	{
		const auto TryFreeName = [](const FString& NewClassString, UObject* NewOuter, ERenameFlags InFlags)
			{
				if (UObject* ExistingObject = StaticFindObjectFast(UObject::StaticClass(), NewOuter, *NewClassString))
				{
					const FName NewName = MakeUniqueObjectName(NewOuter, ExistingObject->GetClass(), *NewClassString);
					ExistingObject->Rename(*(NewName.ToString()), NewOuter, InFlags | REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
				}
			};
		...
		const auto CheckRedirectors = [](FName NewClassName, UClass* OldClass, UObject* NewOuter)
			{
				if (UObjectRedirector* Redirector = FindObjectFast<UObjectRedirector>(NewOuter, NewClassName))
				{
					...
					if (bFoundRedirector)
					{
						Redirector->Rename(nullptr, GetTransientPackage(), REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
					}
				}
			};
		...
		{
			const FString NewGenClassNameString = NewGenClassName.ToString();

			TryFreeName(NewGenClassNameString, NewTopLevelObjectOuter, Flags);
			CheckRedirectors(NewGenClassName, GeneratedClass, NewTopLevelObjectOuter);
			const bool bMovedOK = GeneratedClass->Rename(*NewGenClassNameString, NewTopLevelObjectOuter, Flags);
			if (!bMovedOK)
			{
				return false;
			}
		...
		{
			const FString NewSkelClassNameString = NewSkelClassName.ToString();

			TryFreeName(NewSkelClassNameString, NewTopLevelObjectOuter, Flags);
			CheckRedirectors(NewSkelClassName, SkeletonGeneratedClass, NewTopLevelObjectOuter);
			const bool bMovedOK = SkeletonGeneratedClass->Rename(*NewSkelClassNameString, NewTopLevelObjectOuter, Flags);
			if (!bMovedOK)
			{
				return false;
			}
	...
	while(NewObjects.Num())
	{
		...
		for(UObject* ReachableObject : ReachableObjects) //-V1078
		{
			...
			else
			{
				...
				TArray<UObject*> SubObjs;
				GetObjectsWithOuter(ReachableObject, SubObjs, EGetObjectsFlags::IncludeNestedObjects, RF_Transient, EInternalObjectFlags::Garbage);
...
#if WITH_EDITOR
void UBlueprint::GetAssetRegistryTagMetadata(TMap<FName, FAssetRegistryTagMetadata>& OutMetadata) const
{
	Super::GetAssetRegistryTagMetadata(OutMetadata);

	OutMetadata.Emplace(FBlueprintTags::ParentClassPath, FAssetRegistryTagMetadata()
		.SetDisplayName(NSLOCTEXT("UBlueprint", "ParentClass", "Parent Class"))
		.SetTooltip(NSLOCTEXT("UBlueprint", "ParentClassTooltip", "The parent class of this blueprint.")));

	OutMetadata.Emplace(FBlueprintTags::NativeParentClassPath, FAssetRegistryTagMetadata()
		.SetDisplayName(NSLOCTEXT("UBlueprint", "NativeParentClass", "Native Parent Class"))
		.SetTooltip(NSLOCTEXT("UBlueprint", "NativeParentClassTooltip", "The native parent class of this blueprint.")));

	OutMetadata.Emplace(FBlueprintTags::NumReplicatedProperties, FAssetRegistryTagMetadata()
		.SetDisplayName(NSLOCTEXT("UBlueprint", "NumReplicatedProperties", "Replicated Properties"))
		.SetTooltip(NSLOCTEXT("UBlueprint", "NumReplicatedPropertiesTooltip", "The number of replicated properties in this blueprint.")));

	OutMetadata.Emplace(FBlueprintTags::IsDataOnly, FAssetRegistryTagMetadata()
		.SetDisplayName(NSLOCTEXT("UBlueprint", "IsDataOnly", "Is Data Only"))
		.SetTooltip(NSLOCTEXT("UBlueprint", "IsDataOnlyTooltip", "Whether this is a data-only blueprint.")));

	OutMetadata.Emplace(FBlueprintTags::NumNativeComponents, FAssetRegistryTagMetadata()
		.SetDisplayName(NSLOCTEXT("UBlueprint", "NumNativeComponents", "Native Components"))
		.SetTooltip(NSLOCTEXT("UBlueprint", "NumNativeComponentsTooltip", "The number of native components in this blueprint.")));

	OutMetadata.Emplace(FBlueprintTags::NumBlueprintComponents, FAssetRegistryTagMetadata()
		.SetDisplayName(NSLOCTEXT("UBlueprint", "NumBlueprintComponents", "Blueprint Components"))
		.SetTooltip(NSLOCTEXT("UBlueprint", "NumBlueprintComponentsTooltip", "The number of blueprint components in this blueprint.")));
}
#endif
...
void UBlueprint::BeginCacheForCookedPlatformData(const ITargetPlatform *TargetPlatform)
{
	...
	if (GeneratedClass && GeneratedClass->IsChildOf<AActor>())
	{
		...
		{
			UE_LOGF(LogBlueprint, Log, "%ls: Cooked %d component(s) in %.02g ms", *GetName(), NumCookedComponents, (FPlatformTime::Seconds() - StartTime) * 1000.0);
		}
	}
}
...
ETimelineSigType UBlueprint::GetTimelineSignatureForFunctionByName(const FName& FunctionName, const FName& ObjectPropertyName)
{
	...
	if(ObjectPropertyName != NAME_None)
	{
		...
		if(ObjProperty == NULL)
		{
			UE_LOGF(LogBlueprint, Log, "GetTimelineSignatureForFunction: Object Property '%ls' not found.", *ObjectPropertyName.ToString());
	...
	if(Function == NULL)
	{
		UE_LOGF(LogBlueprint, Log, "GetTimelineSignatureForFunction: Function '%ls' not found in class '%ls'.", *FunctionName.ToString(), *UseClass->GetName());
		return ETS_InvalidSignature;
	}
	...
	//UE_LOGF(LogBlueprint, Log, "GetTimelineSignatureForFunction: No SkeletonGeneratedClass in Blueprint '%ls'.", *GetName());
	//return ETS_InvalidSignature;
}
...
bool UBlueprint::ChangeOwnerOfTemplates()
{
	...
	public:
		FUniqueNewNameHelper(const FString& Name, UObject* Outer) : isValid(false)
		{
			...
			{
				check(Outer);
				UE_LOGF(LogBlueprint, Warning, "ChangeOwnerOfTemplates: Cannot generate a deterministic new name. Old name: %ls New outer: %ls", *Name, *Outer->GetName());
			}
		}
	...
	if (BPGClass)
	{
		...
		for( auto CompIt = ComponentTemplates.CreateIterator(); CompIt; ++CompIt )
		{
			...
			if (Component)
			{
				...
				{
					const bool bRenamed = Component->Rename(*Component->GetName(), BPGClass, REN_DoNotDirty | REN_AllowPackageLinkerMismatch);
					ensure(bRenamed);
		...
		for( auto CompIt = Timelines.CreateIterator(); CompIt; ++CompIt )
		{
			...
			if (Template)
			{
				if(Template->GetOuter() == this)
				{
					...
					const bool bRenamed = Template->Rename(*UTimelineTemplate::TimelineVariableNameToTemplateName(Template->GetFName()), BPGClass, REN_DoNotDirty | REN_AllowPackageLinkerMismatch);
					ensure(bRenamed);
					bIsStillStale |= !bRenamed;
		...
		for (auto Curve : Curves)
		{
			...
			{
				const bool bRenamed = Curve->Rename(FUniqueNewNameHelper(Curve->GetName(), BPGClass).Get(), BPGClass, REN_DoNotDirty | REN_AllowPackageLinkerMismatch);
				ensure(bRenamed);
				bIsStillStale |= !bRenamed;
			}
		}
		...
		if(USimpleConstructionScript* SCS = SimpleConstructionScript)
		{
			...
			{
				const bool bRenamed = SCS->Rename(FUniqueNewNameHelper(SCS->GetName(), BPGClass).Get(), BPGClass, REN_DoNotDirty | REN_AllowPackageLinkerMismatch);
				ensure(bRenamed);
				bIsStillStale |= !bRenamed;
				bMigratedOwner = true;
			}
			...
			for (USCS_Node* SCSNode : SCS->GetAllNodes())
			{
				...
				{
					const bool bRenamed = Component->Rename(FUniqueNewNameHelper(Component->GetName(), BPGClass).Get(), BPGClass, REN_DoNotDirty | REN_AllowPackageLinkerMismatch);
					ensure(bRenamed);
					bIsStillStale |= !bRenamed;
					bMigratedOwner = true;
				}
	...
	{
		UE_LOGF(LogBlueprint, Log, "ChangeOwnerOfTemplates: No BlueprintGeneratedClass in %ls", *GetName());
	}
...
void UBlueprint::BroadcastChanged()
{
	if(bBeingCompiled)
	{
		return;
...
void UBlueprint::GeneratedClassPreSaveRoot(FObjectPreSaveRootContext ObjectSaveContext)
{
}

void UBlueprint::GeneratedClassPostSaveRoot(FObjectPostSaveRootContext ObjectSaveContext)
{
}
