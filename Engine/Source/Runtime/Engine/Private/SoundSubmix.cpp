...
void USoundSubmixBase::IncrementLoadRefCount()
{
	const int32 OldRefCount = RefCount.fetch_add(1, std::memory_order_relaxed);
	if (OldRefCount == 0)
	{
		AddToRoot();
	}
...
void USoundSubmixBase::DecrementLoadRefCount()
{
	const int32 OldRefCount = RefCount.fetch_sub(1, std::memory_order_relaxed);
	if (OldRefCount == 1 && IsRooted())
	{
		RemoveFromRoot();
	}
}
...
void USoundSubmixBase::PostLoad()
{
	...
	if (ClearBrokenSubmixAssetsCVar)
	{
		for (int32 ChildIndex = ChildSubmixes.Num() - 1; ChildIndex >= 0; ChildIndex--)
		{
			...
			if (USoundSubmixWithParentBase* CastedChildSubmix = Cast<USoundSubmixWithParentBase>(ChildSubmix))
			{
				...
				{
					UE_LOGF(LogAudio, Warning, "Submix had a child submix that didn't explicitly mark this submix as a parent!");
	...
	// Dynamic submixes register via DynamicConnect, skip them on asset load.
	const bool bIsDynamicSubmix = IsDynamic(true /* bIncludeAncestors */);

	if (AudioDeviceManager && !bIsTemp && !bIsDynamicSubmix)
	{
		AudioDeviceManager->RegisterSoundSubmix(this);
	}
}
...
{	
	...
	{
		UE_LOGF(LogAudio, Warning, "Submix (DynamicConnect): World Context is null for [%ls]", *GetName());
		return false;
	}
	...
	if (!World)
	{
		UE_LOGF(LogAudio, Warning, "Submix (DynamicConnect): World is null for [%ls]", *GetName());		
		return false;
	}
...
bool USoundSubmixWithParentBase::DynamicConnect(FAudioDeviceHandle Handle, USoundSubmixBase* InParent)
{
	if (!IsDynamic(false /* bIncludeAncestors */))
	{
		...
		UE_CLOGF(DynamicAncestor, LogAudio, Warning, "Submix (DynamicConnect): Dynamic Flag not set for [%ls], you need its ancestor [%ls]. Call FindDynamicAncestor on this submix to find it. Ignoring...", *GetName(), *DynamicAncestor->GetName());
		UE_CLOGF(!DynamicAncestor, LogAudio, Warning, "Submix (DynamicConnect): Dynamic Flag not set for [%ls] or any of its parents, ignoring... ", *GetName());
		return false;
	}
	if (!Handle.IsValid())
	{
		UE_LOGF(LogAudio, Warning, "Submix (DynamicConnect): No valid audio device in this world for [%ls]", *GetName());
		return false;
	}
	...
	{
		UE_LOGF(LogAudio, Warning, "Submix (DynamicConnect): Soundfield submixes do not support dynamic connections. Submix=%ls, Parent=%ls ", *GetName(), *GetNameSafe(InParent));
		return false;
	}
	
	if (InParent && !SubmixUtils::AreSubmixFormatsCompatible(this, InParent))
	{
		UE_LOGF(LogAudio, Warning, "Submix (DynamicConnect): Submix connot be connected are they are incompatible [%ls] and [%ls]", *GetName(), *GetNameSafe(InParent));
		return false;
	}
	...
	{
		UE_LOGF(LogAudio, Warning, "Submix (DynamicConnect): Submix [%ls] is already part of the graph", *GetName());
		return false;
	}
	...
	if (CurrentParent != InParent)
	{
		...
		if (CurrentParent)
		{
			...
			UE_LOGF(LogAudio, Verbose, "Submix (DynamicConnect): Reparenting [%ls] and removing from it's parent [%ls]", *GetName(), *CurrentParent->GetName());
		}
		...
		if (CurrentParent)
		{
			...
			UE_LOGF(LogAudio, Verbose, "Submix (DynamicConnect): Reparenting [%ls] and adding to it's new parent [%ls]", *GetName(), *CurrentParent->GetName());
		...
		SubmixUtils::ForEachStaticChildRecursive(
			...
			{
				UE_LOGF(LogAudio, Verbose, "Submix (DynamicConnect): Registering [%ls] with AudioDevice [%u]", *Iter->GetName(), Id);
	...
	UE_CLOGF(InParent, LogAudio, Warning, "Submix (DynamicConnect): Submix [%ls] was already connected to [%ls]", *GetName(), *GetNameSafe(CurrentParent));
	UE_CLOGF(InParent == nullptr, LogAudio, Warning, "Submix (DynamicConnect): Connecting a [%ls] to a null parent, but our parent is already null", *GetName());
	return false;
...
bool USoundSubmixWithParentBase::DynamicDisconnect(const UObject* WorldContextObject)
{
	if (!WorldContextObject)
	{
		UE_LOGF(LogAudio, Warning, "Submix (DynamicDisconnect): World Context is null for [%ls]", *GetName());
		return false;
	}
	...
	if (!World)
	{
		UE_LOGF(LogAudio, Warning, "Submix (DynamicDisconnect): World is null for [%ls]", *GetName());		
		return false;
	}
...
bool USoundSubmixWithParentBase::DynamicDisconnect(FAudioDeviceHandle Handle)
{
	if (!Handle.IsValid())
	{
		UE_LOGF(LogAudio, Warning, "Submix (DynamicDisconnect): No valid audio device in this world for [%ls]", *GetName());
		return false;
	}

	if (!IsDynamic(false /* bIncludeAncestors */))
	{
		UE_LOGF(LogAudio, Warning, "Submix (DynamicDisconnect): Dynamic Flag not set for [%ls] ignoring.", *GetName());
		return false;
	}
	...
	if (CurrentParent)
	{
		UE_LOGF(LogAudio, Verbose, "Submix (DynamicDisconnect): Removing [%ls] from it's parent [%ls]", *GetName(), *CurrentParent->GetName());
		...
		UE_LOGF(LogAudio, Verbose, "Submix (DynamicDisconnect): Unregistering [%ls] with AudioDevice [%u]", *GetName(), Id);

		return true;
	}

	UE_LOGF(LogAudio, Warning, "Submix (DynamicDisconnect): Submix was not connected to any dynamic parent [%ls]", *GetName());
	return false;
}
...
void UEndpointSubmix::PostLoad()
{
	...
	if (!EndpointTypeNames.Contains(EndpointType))
	{
		...
		UE_LOGF(LogAudio, Warning, "UEndpointSubmix [%ls] has endpoint type [%ls] which is not currently currently enabled. Changing to [%ls]",
...
void USoundfieldSubmix::PostLoad()
{
	...
	if (!FactoryNames.Contains(SoundfieldEncodingFormat))
	{
		...
		UE_LOGF(LogAudio, Warning, "SoundfieldSubmix [%ls] has Encoding format [%ls] which is not currently currently enabled. Changing to [%ls]",
...
void USoundfieldEndpointSubmix::PostLoad()
{
	...
	if (!SoundfieldEndpointTypeNames.Contains(SoundfieldEndpointType))
	{
		...
		UE_LOGF(LogAudio, Warning, "USoundfieldEndpointSubmix [%ls] has endpoint type [%ls] which is not currently currently enabled. Changing to [%ls]",
...
void USoundfieldEndpointSubmix::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	if (PropertyChangedEvent.Property != nullptr)
	{
		static const FName NAME_SoundfieldFormat("SoundfieldEndpointType");
	...
	if (Version < FSoundSubmixCustomVersion::MigrateModulatedSendProperties)
	{
		...
		auto ConvertToModulatedDb = [this](const float InValue, const float InDefault, const float InDefaultModulationValue, FSoundModulationDestinationSettings& OutModulator, const TCHAR* InParamName) 
		{
				...
				FMath::IsNearlyEqual(OutModulator.Value, InDefaultModulationValue))
			{
				...
				UE_LOGF(LogAudio, Display, "SoundSubmix::HandleVersionMigration, ConvertToModulatedDb, Asset = %ls, %ls = %2.2f dB from %2.2f", *GetName(), InParamName, OutModulator.Value, InValue);
			}
		};
	...
	if (Version < FSoundSubmixCustomVersion::ConvertLinearModulatorsToDb)
	{
		auto ConvertToDb = [this](FSoundModulationDestinationSettings& OutValue, const TCHAR* InName) 
		{
			...
			if (OutValue.Value > 0.0f)
			{
				...
				UE_LOGF(LogAudio, Display, "SoundSubmix::HandleVersionMigration, ConvertToDb, Asset = %ls, %ls = %2.2f dB from %2.2f", *GetName(), InName, dbValue, OutValue.Value)
