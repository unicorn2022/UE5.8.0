	...
	int32 RemovedCount = 
		PendingConstructionScriptActors.RemoveAll(
			[InWorld](TWeakObjectPtr<AActor>& InActor)
			{
				...
				if (Actor == nullptr)
				{
					UE_LOGF(LogActorDeferredScriptManager, VeryVerbose, "Removed an actor from deferred construction script because its not valid anymore");
					return true;
				}
				else if (Actor->GetWorld() == InWorld)
				{
					UE_LOGF(LogActorDeferredScriptManager, VeryVerbose, "Removed %ls from deferred construction script because its world is being cleaned up", *InActor->GetPathName());
					return true;
				}
...
{
	UE_LOGF(LogActorDeferredScriptManager, VeryVerbose, "Adding actor %ls for deferred construction script", *InActor->GetPathName());
...
void FActorDeferredScriptManager::ProcessAsyncTasks(bool bLimitExecutionTime)
{
	...
	if (NumLeftToProcess > 0)
	{
		...
		UE_LOGF(LogActorDeferredScriptManager, VeryVerbose, "Starting to process at index %d", CurrentIndex);
		...
		for (; NumLeftToProcess > 0; --NumLeftToProcess, ++CurrentIndex)
		{
			...
			if (!bHasTimeLeft)
			{
				...
				UE_LOGF(LogActorDeferredScriptManager, VeryVerbose, "No time left, will continue at index %d", CircularIndex);
			...
			{
				UE_LOGF(LogActorDeferredScriptManager, VeryVerbose, "Processing index %d (%ls)", CircularIndex, *Actor->GetPathName());
				...
				if (bIsStaticMeshesCompiling)
				{
					if (ULevel* Level = Actor->GetLevel())
					{
						if (DoesLevelHaveCompilationLeft(Level))
						{
							...
							UE_LOGF(LogActorDeferredScriptManager, VeryVerbose, "Skipping index %d (%ls) because it's level outer still has static meshes compilation pending", CircularIndex, *Actor->GetPathName());
							continue;
				...
				UE_LOGF(LogActorDeferredScriptManager, VeryVerbose, "Rerunning construction script for index %d (%ls)", CircularIndex, *Actor->GetPathName());
			...
			{
				UE_LOGF(LogActorDeferredScriptManager, VeryVerbose, "Removing index %d because its actor is not valid anymore", CircularIndex);
			}
			...
			UE_LOGF(LogActorDeferredScriptManager, VeryVerbose, "Removing index %d", CircularIndex);
			PendingConstructionScriptActors.RemoveAtSwap(CircularIndex, 1, AllowShrinking);
			CurrentIndex--;

			if (PendingConstructionScriptActors.IsEmpty())
			{
				UE_LOGF(LogActorDeferredScriptManager, VeryVerbose, "Finished processing construction scripts");
			}
		}
		if (EnteredPackage)
		{
