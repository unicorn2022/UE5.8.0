...
bool ParseWaveFormatHeader(const uint8* InSrcBufferData, uint32 InSrcBufferDataSize, FWaveFormatInfo& OutHeader)
{
	...
	{
		UE_LOGF(LogAudio, Error, "Failed to parse wave format header. Buffer data was null or empty.");
		return false;
	}
	...
	{
		UE_LOGF(LogAudio, Error, "Beginning of wave file was not \"RIFF\"");
		return false;
	}
	...
	{
		UE_LOGF(LogAudio, Error, "First wave RIFF chunk is not a \"WAVE\" type");
		return false;
	}
	...
	while (CurrByte < InSrcBufferDataSize)
	{
		...
		switch (ChunkHeader.ChunkId)
		{
			...
			default:
			{
				UE_LOGF(LogAudio, Warning, "Wave file contained unknown RIFF chunk type (%d)", ChunkHeader.ChunkId);
			}
			break;
