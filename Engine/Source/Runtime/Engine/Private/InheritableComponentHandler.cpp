...
UActorComponent* UInheritableComponentHandler::CreateOverridenComponentTemplate(const FComponentKey& Key)
{
	...
	if (!BestArchetype)
	{
		UE_LOGF(LogBlueprint, Warning, "CreateOverridenComponentTemplate '%ls': cannot find archetype for component '%ls' from '%ls'",
	...
	for (int32 Index = 0; Index < Records.Num();)
	{
		...
		FComponentOverrideRecord& Record = Records[Index];
		{
			...
			if (ComponentKey.RefreshVariableName())
			{
				...
				UE_LOGF(LogBlueprint, Log, "ValidateTemplates '%ls': variable old name '%ls' new name '%ls'",
			...
			if (IsRecordValid(Record))
			{
				...
				else
				{
					...
					UE_LOGF(LogBlueprint, Log, "ValidateTemplates '%ls': overridden template is unnecessary and will be removed - component '%ls' from '%ls'",
						*GetPathNameSafe(this), *VarName.ToString(), *GetPathNameSafe(ComponentKey.GetComponentOwner()));
				}
			}
			else
			{
				UE_LOGF(LogBlueprint, Log, "ValidateTemplates '%ls': overridden template is invalid and will be removed - component '%ls' from '%ls' (this can happen when a class was recently reparented)",
					*GetPathNameSafe(this), *VarName.ToString(), *GetPathNameSafe(ComponentKey.GetComponentOwner()));
			}
		}
...
void UInheritableComponentHandler::FixComponentTemplateName(UActorComponent* ComponentTemplate, const FName NewName)
{
	...
	{
		ExistingObject->Rename(nullptr, nullptr, REN_AllowPackageLinkerMismatch | REN_DontCreateRedirectors);
	}

	// Now that we're sure there are no collisions with other records, we can safely rename this one to its new name.
	ComponentTemplate->Rename(*NewName.ToString(), nullptr, REN_AllowPackageLinkerMismatch | REN_DontCreateRedirectors);
}
