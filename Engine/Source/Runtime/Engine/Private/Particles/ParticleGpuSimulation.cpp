...
public:
	...
	const TCHAR* ParticlePositionTextureName = TEXT("FParticleStatePosition");
	const TCHAR* ParticleVelocityTextureName = TEXT("FParticleStateVelocity");
	...
	virtual void InitRHI(FRHICommandListBase& RHICmdList) override
	{
		...
		PositionTextureRHI = CreateStateTextureRHI(RHICmdList, ParticlePositionTextureName, GetParticlePositionFormat());

		// 16-bit per channel RGBA texture for velocity.
		check(!IsValidRef(VelocityTextureRHI));
		VelocityTextureRHI = CreateStateTextureRHI(RHICmdList, ParticleVelocityTextureName, GetParticleVelocityFormat());
		...
		FName PositionTextureName(ParticlePositionTextureName, ParticleStateIndex + 1);
		FName VelocityTextureName(ParticleVelocityTextureName, ParticleStateIndex + 1);
		PositionTextureRHI->SetName(PositionTextureName);
		VelocityTextureRHI->SetName(VelocityTextureName);
		RHICmdList.BindDebugLabelName(PositionTextureRHI, ParticlePositionTextureName);
		RHICmdList.BindDebugLabelName(VelocityTextureRHI, ParticleVelocityTextureName);

		// Clear textures at creation time to ensure GPU residency is established immediately.
		ClearRenderTarget(RHICmdList.GetAsImmediate(), PositionTextureRHI);
		ClearRenderTarget(RHICmdList.GetAsImmediate(), VelocityTextureRHI);
		RHICmdList.GetAsImmediate().Transition({
			FRHITransitionInfo(PositionTextureRHI, ERHIAccess::RTV, ERHIAccess::SRVMask),
			FRHITransitionInfo(VelocityTextureRHI, ERHIAccess::RTV, ERHIAccess::SRVMask)
		});
		bTexturesCleared = true;
	}
	...
	void ResizeRHI(FRHICommandListImmediate& RHICmdList, int32 SizeX, int32 SizeY)
	{
		...
		FTextureRHIRef NewPositionTextureRHI = CreateStateTextureRHI(RHICmdList, ParticlePositionTextureName, GetParticlePositionFormat());
		FTextureRHIRef NewVelocityTextureRHI = CreateStateTextureRHI(RHICmdList, ParticleVelocityTextureName, GetParticleVelocityFormat());
		...
		{
			{
				...
				FRHITexture* NewRenderTargets[] = { NewPositionTextureRHI, NewVelocityTextureRHI };
				FRHIRenderPassInfo RenderPassInfo(UE_ARRAY_COUNT(NewRenderTargets), NewRenderTargets, ERenderTargetActions::Clear_Store);
				RHICmdList.BeginRenderPass(RenderPassInfo, TEXT("GPUParticlesClearStateTextures"));
				RHICmdList.EndRenderPass();
			}

			RHICmdList.Transition({
				FRHITransitionInfo(NewPositionTextureRHI, ERHIAccess::RTV, ERHIAccess::CopyDest),
				FRHITransitionInfo(PositionTextureRHI, ERHIAccess::SRVMask, ERHIAccess::CopySrc),
				FRHITransitionInfo(NewVelocityTextureRHI, ERHIAccess::RTV, ERHIAccess::CopyDest),
				FRHITransitionInfo(VelocityTextureRHI, ERHIAccess::SRVMask, ERHIAccess::CopySrc) });
			...
			RHICmdList.Transition({
				FRHITransitionInfo(NewPositionTextureRHI, ERHIAccess::CopyDest, ERHIAccess::SRVMask),
				FRHITransitionInfo(NewVelocityTextureRHI, ERHIAccess::CopyDest, ERHIAccess::SRVMask) });
		}
...
public:
	...
	const TCHAR* ParticleAttributeTextureName = TEXT("FParticleAttributesTexture");
	...
	{
		const FRHITextureCreateDesc Desc =
			FRHITextureCreateDesc::Create2D(ParticleAttributeTextureName)
			.SetExtent(RenderTargetSize.X, RenderTargetSize.Y)
	...
	void ResizeRHI(FRHICommandListImmediate& RHICmdList, int32 SizeX, int32 SizeY)
	{
		...
		ClearRenderTarget(RHICmdList, NewAttributesTextureRHI);

		RHICmdList.Transition({
			FRHITransitionInfo(NewAttributesTextureRHI, ERHIAccess::RTV, ERHIAccess::CopyDest),
			FRHITransitionInfo(TextureRHI, ERHIAccess::SRVMask, ERHIAccess::CopySrc) });
		...
		if (RHISupportsGPUParticles() && GFXCascadeGpuSpriteRenderingEnabled)
		{
				...
				&& Simulation->VertexBuffer.ParticleCount > 0)
			{
				...
				if (bAllowSorting && SortMode == PSORTMODE_DistanceToView)
				{
					...
					{
						UE_LOGF(LogParticles, Warning, "Cascade doesn't support gpu sorting with particle gpu simulation resources resizing");
					}
					else
					{ 
...
FGPUSpriteParticleEmitterInstance(IParticleEmitterInstanceOwner& InComponent, FFXSystem* InFXSystem, FGPUSpriteEmitterInfo& InEmitterInfo)
...
#if TRACK_TILE_ALLOCATIONS
		UE_LOGF(LogParticles,VeryVerbose,
			"%ls|%ls|0x%016x [Init] %d tiles",
	...
	virtual void Tick(float DeltaSeconds, bool bSuppressSpawning) override
	{
		...
		if (bEnabled)
		{
			...
			if (!bPreventSpawning && bValidEmitterTime && bValidLoop)
			{
				...
				{
					UE_LOGF(LogParticles,Warning,"Spawning %d GPU particles in one frame[%d]: %ls/%ls",
						NewParticleCount,
						GFrameNumber,
	...
	virtual void ForceUpdateBoundingBox() override
	{
		if ( !GIsEditor )
		{
			UE_LOGF(LogParticles, Warning, "ForceUpdateBoundingBox called on a GPU sprite emitter outside of the Editor!" );
			return;
		}
...
#if TRACK_TILE_ALLOCATIONS
					UE_LOGF(LogParticles, VeryVerbose,
						"%ls|%ls|0x%016p [ReleaseSimulationResources] %d tiles",
						*Component.GetName(), *Component.Template->GetName(), (PTRINT)this, AllocatedTiles.Num());
#endif // #if TRACK_TILE_ALLOCATIONS
				}
			}
			...
			if (FreeParticlesInTile <= 0)
			{
				...
				else
				{
					...
					if (NewTile == INDEX_NONE)
					{
						...
						UE_LOGF(LogParticles,Warning,
							"Failed to allocate tiles for %ls! %d new particles truncated to %d.",
...
void DumpTileAllocations()
{
	for (TMap<FFXSystem*,TSet<FGPUSpriteParticleEmitterInstance*> >::TConstIterator It(GPUSpriteParticleEmitterInstances); It; ++It)
	{
		...
		UE_LOGF(LogParticles,Display,"---------- GPU Particle Tile Allocations : FXSystem=0x%016x ----------", (PTRINT)FXSystem);
		for (TSet<FGPUSpriteParticleEmitterInstance*>::TConstIterator It(Emitters); It; ++It)
		{
			...
			UE_LOGF(LogParticles,Display,
				"%ls|%ls|0x%016x %d tiles",
				*Emitter->Component->GetName(),*Emitter->Component->Template->GetName(),(PTRINT)Emitter, TileCount);
			TotalAllocatedTiles += TileCount;
		}

		UE_LOGF(LogParticles,Display,"---");
		UE_LOGF(LogParticles,Display,"Total Allocated: %d", TotalAllocatedTiles);
		UE_LOGF(LogParticles,Display,"Free (est.): %d", FXSystem->GetParticleSimulationResources()->GetAllocatedTileCount() - TotalAllocatedTiles);
		if (FXSystem)
		{
			UE_LOGF(LogParticles,Display,"Free (actual): %d", FXSystem->GetParticleSimulationResources()->GetFreeTileCount());
			UE_LOGF(LogParticles,Display,"Leaked: %d", FXSystem->GetParticleSimulationResources()->GetAllocatedTileCount() - TotalAllocatedTiles - FXSystem->GetParticleSimulationResources()->GetFreeTileCount());
		}
	}
}
...
{
	UE_LOGF(LogParticles,Verbose,
		"Destroying %d GPU particle simulations for FXSystem 0x%p",
		GPUSimulations.Num(),
		this
		);
	...
	)
{
	...
	// Helper to handle transitions the current state render targets only when required
	bool bIsCurrentTextureAccessRTV = false;

	const auto PrepareCurrentRenderTargetsAccess =
		[&]()
		{
			if (bIsCurrentTextureAccessRTV == false)
			{
				bIsCurrentTextureAccessRTV = true;
				RHICmdList.Transition({
	...
	const auto FinalizeCurrentRenderTargetsAccess =
		[&]()
		{
			if (bIsCurrentTextureAccessRTV)
			{
				RHICmdList.Transition({
					FRHITransitionInfo(CurrentStateTextures.PositionTextureRHI, ERHIAccess::RTV, ERHIAccess::SRVMask),
					FRHITransitionInfo(CurrentStateTextures.VelocityTextureRHI, ERHIAccess::RTV, ERHIAccess::SRVMask)
				});
			}
		};

	{
		...
		if (!CurrentStateTextures.bTexturesCleared)
		{
			...
			FRHIRenderPassInfo RenderPassInfo(UE_ARRAY_COUNT(CurrentStateRenderTargets), CurrentStateRenderTargets, ERenderTargetActions::Clear_Store);
			RHICmdList.BeginRenderPass(RenderPassInfo, TEXT("PrepareGPUSimulation_GPUParticlesClearStateTextures"));
			RHICmdList.EndRenderPass();
	...
	{
		PrepareCurrentRenderTargetsAccess();
	...
	FinalizeCurrentRenderTargetsAccess();
