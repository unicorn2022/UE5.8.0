	...
	const FSceneView* View, const FMatrix& LocalToWorld, FParticleOrder* ParticleOrder) const
{
	...
	if (SortMode == PSORTMODE_ViewProjDepth)
	{
		for (int32 ParticleIndex = 0; ParticleIndex < ParticleCount; ParticleIndex++)
		{
			...
			if (bLocalSpace)
			{
				InZ = View->ViewMatrices.GetWorldToClip().TransformPosition(LocalToWorld.TransformPosition(Particle.Location)).W;
			}
			else
			{
				InZ = View->ViewMatrices.GetWorldToClip().TransformPosition(Particle.Location).W;
			}
...
void FDynamicSpriteEmitterDataBase::RenderDebug(const FParticleSystemSceneProxy* Proxy, FPrimitiveDrawInterface* PDI, const FSceneView* View, bool bCrosses) const
{
	...
	FMatrix CameraToWorld = View->ViewMatrices.GetViewToWorld();
	...
	FAsyncBufferFillData& Data) const
{
	...
	{
		UE_LOGF(LogParticles, Warning, "Panic logging.  Allocated %u bytes for Resource: %ls, Owner: %ls", InVertexCount * InVertexSize, *Proxy->GetResourceName().ToString(), *Proxy->GetOwnerName().ToString())
	}
	...
	if( bUsesDynamicParameter )
	{
		...
		*DynamicParameterAllocation = DynamicVertexBuffer.Allocate( InVertexCount * InDynamicParameterVertexStride );
		...
		if (DynamicVertexBuffer.IsRenderAlarmLoggingEnabled())
		{
			UE_LOGF(LogParticles, Warning, "Panic logging.  Allocated %u bytes for Resource: %ls, Owner: %ls", InVertexCount * InDynamicParameterVertexStride, *Proxy->GetResourceName().ToString(), *Proxy->GetOwnerName().ToString())
		}
...
void FDynamicSpriteEmitterData::GetDynamicMeshElementsEmitter(const FParticleSystemSceneProxy* Proxy, const FSceneView* View, const FSceneViewFamily& ViewFamily, int32 ViewIndex, FMeshElementCollector& Collector) const
{
	...
	if (bValid && SourceData && GFXCascadeSpriteRenderingEnabled)
	{
		if (SourceData->EmitterRenderMode == ERM_Normal)
		{
			...
			const uint32 InstanceFactor = View->IsInstancedStereo() ? 2 : 1;
			...
			if (DynamicVertexBuffer.IsRenderAlarmLoggingEnabled())
			{
				UE_LOGF(LogParticles, Warning, "Panic logging.  Allocated %u bytes for Resource: %ls, Owner: %ls", InstanceFactor * ParticleCount * VertexSize * NumVerticesPerParticleInBuffer, *Proxy->GetResourceName().ToString(), *Proxy->GetOwnerName().ToString())
			}
			...
			if (bUsesDynamicParameter)
			{
				...
				if (DynamicVertexBuffer.IsRenderAlarmLoggingEnabled())
				{
					UE_LOGF(LogParticles, Warning, "Panic logging.  Allocated %u bytes for Resource: %ls, Owner: %ls", InstanceFactor * ParticleCount * DynamicParameterVertexSize * NumVerticesPerParticleInBuffer, *Proxy->GetResourceName().ToString(), *Proxy->GetOwnerName().ToString())
				}
			}
	...
	if (bValid && GFXCascadeMeshRenderingEnabled)
	{
		if (Source.EmitterRenderMode == ERM_Normal)
		{
			...
			if (bUsesDynamicParameter)
			{
				...
				if (DynamicVertexBuffer.IsRenderAlarmLoggingEnabled())
				{
					UE_LOGF(LogParticles, Warning, "Panic logging.  Allocated %u bytes for Resource: %ls, Owner: %ls", InstanceFactor * ParticleCount * DynamicParameterVertexStride, *Proxy->GetResourceName().ToString(), *Proxy->GetOwnerName().ToString());
				}
			}
			...
			for (int32 LODIndex = 0; LODIndex < 1; LODIndex++)
			{
				...
				for (int32 SectionIndex = 0; SectionIndex < LODModel.Sections.Num(); SectionIndex++)
				{
					...
					if (Section.NumTriangles == 0)
					{
						...
						UE_LOGF(LogParticles, Warning, "Mesh section %i has 0 triangles (LOD %i)", SectionIndex, GetMeshLODIndexFromProxy(Proxy));
						continue;
					}

					if (MaterialProxy == nullptr)
					{
						UE_LOGF(LogParticles, Warning, "Mesh section %i has a null material proxy (LOD %i)", SectionIndex, GetMeshLODIndexFromProxy(Proxy));
						continue;
					}
				...
				if (TotalTriangles == 0)
				{
					UE_LOGF(LogParticles, Warning, "WARNING: particle mesh LOD %i has no triangles, but is reported as a valid LOD!", GetMeshLODIndexFromProxy(Proxy));
				}
			}
		}
	...
	) const
{
	...
	const FVector PreviousViewDirection = PreviousViewMatrices.GetWorldToView().GetColumn(2);
	...
	const FSceneView* View )
{
	...
	{
		CameraUp	= -View->ViewMatrices.GetClipToWorld().TransformVector(FVector(1.0f,0.0f,0.0f)).GetSafeNormal();
		CameraRight	= -View->ViewMatrices.GetClipToWorld().TransformVector(FVector(0.0f,1.0f,0.0f)).GetSafeNormal();
	}
	else
	{
...
int32 FDynamicBeam2EmitterData::FillVertexData_NoNoise(FAsyncBufferFillData& Me) const
{
	...
	FMatrix CameraToWorld = Me.View->ViewMatrices.GetViewToWorld();
	FVector	ViewOrigin = CameraToWorld.GetOrigin();
	int32 TessFactor = Source.InterpolationPoints ? Source.InterpolationPoints : 1;
...
void FDynamicTrailsEmitterData::GetDynamicMeshElementsEmitter(const FParticleSystemSceneProxy* Proxy, const FSceneView* View, const FSceneViewFamily& ViewFamily, int32 ViewIndex, FMeshElementCollector& Collector) const
{
	...
	else
	{
		...
		{
			UE_LOGF(LogParticles, Log, "Data.OutTriangleCount = %4d vs. SourcePrimCount = %4d", OutTriangleCount, SourcePointer->PrimitiveCount);
			...
			for (int32 ParticleIdx = 0; ParticleIdx < SourcePointer->ActiveParticleCount; ParticleIdx++)
			{
				...
				UE_LOGF(LogParticles, Log, "Trail %2d has %5d triangles", TrailPayload->TrailIndex, TrailPayload->TriangleCount);
				CheckTriangleCount += TrailPayload->TriangleCount;
				CheckTrailCount++;
			}
			UE_LOGF(LogParticles, Log, "Total 'live' trail count = %d", CheckTrailCount);
			UE_LOGF(LogParticles, Log, "\t%5d triangles total (not counting degens)", CheckTriangleCount);
		}
#endif
...
int32 FDynamicRibbonEmitterData::FillVertexData(struct FAsyncBufferFillData& Data) const
{
	...
	FMatrix CameraToWorld = Data.View->ViewMatrices.GetViewToWorld();
	...
	for (int32 ParticleIdx = 0; ParticleIdx < Source.ActiveParticleCount; ParticleIdx++)
	{
		...
		while (TrailPayload)
		{
			...
			if (InterpCount > 1)
			{
				...
				FVector PrevTangent = (FVector)PrevTrailPayload->Tangent;
