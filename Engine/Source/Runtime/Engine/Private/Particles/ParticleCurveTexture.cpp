...
FTexelAllocation FParticleCurveTexture::AddCurve(const TArray<FColor>& CurveSamples)
{
	...
	if (FApp::CanEverRender())
	{
		if (CurveSamples.Num())
		{
			...
			UE_LOGF(LogParticles, Warning, "FParticleCurveTexture: Failed to allocate %d texels for a curve (may need to increase the size of GParticleCurveTextureSizeX or GParticleCurveTextureSizeY).", CurveSamples.Num());
