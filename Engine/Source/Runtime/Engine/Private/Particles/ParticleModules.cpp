...
bool UParticleModule::ConvertFloatDistribution(UDistributionFloat* FloatDist, UDistributionFloat* SourceFloatDist, float Percentage)
{
	...
	{
		UE_LOGF(LogParticles, Log, "UParticleModule::ConvertFloatDistribution> Invalid distribution?");
		return false;
	}
...
bool UParticleModule::ConvertVectorDistribution(UDistributionVector* VectorDist, UDistributionVector* SourceVectorDist, float Percentage)
{
	...
	{
		UE_LOGF(LogParticles, Log, "UParticleModule::ConvertVectorDistribution> Invalid distribution?");
		return false;
	}
	...
	if (DupObject)
	{
		...
		if (bGenerateModuleData)
		{
			if (NewModule->GenerateLODModuleValues(this, Percentage, DestLODLevel) == false)
			{
				FString NameDump;
				GetName(NameDump);
				UE_LOGF(LogParticles, Log, "ERROR - GenerateFromLODLevel - Failed to generate LOD module values for %ls!", *NameDump);
				NewModule = NULL;
			}
		}
	}
...
private:
	...
	FArchive& operator<<( class UObject*& Obj )
	{
		if (Obj && Obj->IsA<UDistribution>())
		{
			...
			{
				UE_LOGF(LogParticles, Verbose, "Bad Module Distribution %ls not in %ls (resave asset will fix this)", *GetFullNameSafe(Obj), *GetFullNameSafe(Outer));
				UObject *New = (UObject*)FindObjectWithOuter(Outer, Obj->GetClass(), Obj->GetFName());
				if (New)
				{
					UE_LOGF(LogParticles, Verbose, "      Replacing with Found %ls", *GetFullNameSafe(New));
				}
				else
				{
					New = NewObject<UObject>(Outer, Obj->GetClass(), Obj->GetFName(), RF_NoFlags, Obj);
					UE_LOGF(LogParticles, Verbose, "      Replacing with New %ls", *GetFullNameSafe(New));
				}
				Obj = New;
			}
...
void UParticleModuleOrientationAxisLock::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (LODLevel)
	{
		...
		UE_LOGF(LogParticles, Warning, "UParticleModuleOrientationAxisLock has an incorrect outer... run FixupEmitters on package %ls",
	...
	{
		if (PropertyThatChanged->GetFName() == FName("LockAxisFlags"))
		{
...
void UParticleModuleRequired::Serialize(FStructuredArchive::FRecord Record)
{
	...
	if (UnderlyingArchive.CustomVer(FRenderingObjectVersion::GUID) >= FRenderingObjectVersion::MovedParticleCutoutsToRequiredModule)
	{
		...
		{
			UE_LOGF(LogParticles, Fatal, "This platform requires cooked packages, and this SubUV animation does not contain cooked data %ls.", *GetName());
		}
...
void UParticleModuleRequired::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyThatChanged)
	{
		if (PropertyThatChanged->GetFName() == FName("MaxDrawCount"))
		{
			if (MaxDrawCount >= 0)
			{
				bUseMaxDrawCount = true;
		...
		else if (AlphaThreshold > 0 && PropertyThatChanged->GetFName() == FName("Material"))
		{
			GetDefaultCutout();
		}
	}
...
void UParticleModuleTypeDataMesh::OnMeshChanged()
{
	...
	{
		// The outer is incorrect - warn the user and handle it
		UE_LOGF(LogParticles, Warning, "UParticleModuleTypeDataMesh has an incorrect outer... run FixupEmitters on package %ls",
			*(OuterObj->GetOutermost()->GetPathName()));
		OuterObj = LODLevel->GetOuter();
		UParticleEmitter* Emitter = Cast<UParticleEmitter>(OuterObj);
		check(Emitter);
	...
	FProperty* MeshProperty = FindFProperty<FProperty>(UParticleModuleTypeDataMesh::StaticClass(), FName("Mesh"));
...
void UParticleModuleTypeDataMesh::PreEditChange(FProperty* PropertyThatWillChange)
{
	...
	if ( (PropertyThatWillChange != nullptr) && (PropertyThatWillChange->GetFName() == FName("Mesh")) )
	{
		if ( GIsEditor && (Mesh != nullptr) )
		{
...
void UParticleModuleTypeDataMesh::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyThatChanged)
	{
		if (PropertyThatChanged->GetFName() == FName("Mesh"))
		{
			OnMeshChanged();
		}
		if (GIsEditor && (Mesh != nullptr))
...
FParticleEmitterInstance* UParticleModuleTypeDataGpu::CreateInstance(UParticleEmitter* InEmitterParent, IParticleEmitterInstanceOwner& InComponent)
{
	UE_LOGF(LogParticles,Verbose,
		"Create GPU Sprite Emitter @ %fs %ls", InComponent.GetWorldTimeSeconds(),
