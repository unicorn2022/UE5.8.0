...
{
	UE_LOGF(LogParticles, Fatal,"UParticleEmitter::CreateInstance is pure virtual"); 
	return NULL; 
}
		...
		while (CheckIndex >= 0)
		{
			if (CurrModule->IsUsedInLODLevel(CheckIndex))
			{
				...
				else
				{
					...
					{
						UE_LOGF(LogParticles, Warning, "\t\tMismatched module count at %2d in %ls", LODIndex, *(Emitter->GetPathName()));
						Result = -1;
					}
					else
					{
	...
	for (int32 LODIndex = 0; LODIndex < LODLevels.Num(); LODIndex++)
	{
		...
		if (LODLevel)
		{
			...
			if (LODLevel->SpawnModule == NULL)
			{
				...
				if (PSys)
				{
					UE_LOGF(LogParticles, Warning, "LODLevel %d was not converted to spawn module - forcing: %ls", 
		...
		for (int32 LODIndex = 0; (LODIndex < LODLevels.Num()) && !bWarned; LODIndex++)
		{
			...
			if (LODLevel)
			{
				...
				if (Module)
				{
					...
					{
						UE_LOGF(LogParticles, Warning, "UParticleModule %ls has an incorrect outer on %ls... run FixupEmitters on package %ls (%ls)",
							*(Module->GetPathName()), 
							*(EmitterOuter->GetPathName()),
							*(OuterObj->GetOutermost()->GetPathName()),
							*(GetOutermost()->GetPathName()));
						UE_LOGF(LogParticles, Warning, "\tModule Outer..............%ls", *(OuterObj->GetPathName()));
						UE_LOGF(LogParticles, Warning, "\tModule Outermost..........%ls", *(Module->GetOutermost()->GetPathName()));
						UE_LOGF(LogParticles, Warning, "\tEmitter Outer.............%ls", *(EmitterOuter->GetPathName()));
						UE_LOGF(LogParticles, Warning, "\tEmitter Outermost.........%ls", *(GetOutermost()->GetPathName()));
						bWarned = true;
					}
				}
...
{
	UE_LOGF(LogParticles, Log, "UParticleEmitter::AddEmitterCurvesToEditor> Should no longer be called...");
	return;
}
...
int32 UParticleEmitter::CreateLODLevel(int32 LODLevel, bool bGenerateModuleData)
{
	...
	if (NextHighestLODLevel)
	{
		...
		{
			UE_LOGF(LogParticles, Warning, "Failed to generate LOD level %d from level %d", LODLevel, NextHighestLODLevel->Level);
		}
	}
	else
	{
...
bool UParticleEmitter::AutogenerateLowestLODLevel(bool bDuplicateHighest)
{
	...
	if (LODLevels.Num() == 1)
	{
		...
		{
			UE_LOGF(LogParticles, Warning, "Failed to generate LOD level %d from LOD level 0", 1);
			return false;
		}
...
bool UParticleEmitter::CalculateMaxActiveParticleCount()
{
		...
		if (LODLevel && LODLevel->bEnabled)
		{
			...
			if (LODMaxAPC > CurrMaxAPC)
			{
				if (LODIndex > 0)
				{
					...
					{
						//UE_LOGF(LogParticles, Log, "MaxActiveParticleCount Discrepancy?\n\tLOD %2d, Emitter %16ls", LODIndex, *GetName());
					}
				}
	...
	if ((GIsEditor == true) && (CurrMaxAPC > 500))
	{
		...
		UE_LOGF(LogParticles, Warning, "MaxCount = %4d for Emitter %ls (%ls)",
...
void UParticleEmitter::CreateStatID() const
{
	...
	FString LongName;
	LongName = FString("Emitter") / OuterName.ToString() / EmitterName.ToString();
