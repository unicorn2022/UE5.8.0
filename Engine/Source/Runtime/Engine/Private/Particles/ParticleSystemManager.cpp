...
// 		}
// 		//UE_LOGF(LogParticles, Warning, "| Ticking Concurrent Batch %ls |", *Ticked);
...
void FParticleSystemWorldManager::OnWorldInit(UWorld* World, const UWorld::InitializationValues IVS)
{
	...
	UE_LOGF(LogParticles, Verbose, "| OnWorldInit | WorldMan: %p | World: %p | %ls |", NewWorldMan, World, *World->GetFullName());
}
...
void FParticleSystemWorldManager::OnWorldCleanup(UWorld* World, bool bSessionEnded, bool bCleanupResources)
{
	...
	{
		UE_LOGF(LogParticles, Verbose, "| OnWorldCleanup | WorldMan: %p | World: %p | %ls |", *WorldMan, World, *World->GetFullName());
		delete (*WorldMan);
...
void FParticleSystemWorldManager::OnPreWorldFinishDestroy(UWorld* World)
{
	if (FParticleSystemWorldManager** WorldMan = WorldManagers.Find(World))
	{
		UE_LOGF(LogParticles, Verbose, "| OnPreWorldFinishDestroy | WorldMan: %p | World: %p | %ls |", *WorldMan, World, *World->GetFullName());
		delete (*WorldMan);
		WorldManagers.Remove(World);
	}
}
...
void FParticleSystemWorldManager::OnWorldBeginTearDown(UWorld* World)
{
	if (FParticleSystemWorldManager** WorldMan = WorldManagers.Find(World))
	{
		UE_LOGF(LogParticles, Verbose, "| OnWorldBeginTearDown | WorldMan: %p | World: %p | %ls |", *WorldMan, World, *World->GetFullName());
		delete (*WorldMan);
		WorldManagers.Remove(World);
	}
}
...
{
	UE_LOGF(LogParticles, Verbose, "| FParticleSystemWorldManager::~FParticleSystemWorldManager() | %p |", this);
	Cleanup();
}
...
void FParticleSystemWorldManager::AddReferencedObjects(FReferenceCollector& Collector)
{
	if (World)
	{
		//UE_LOGF(LogParticles, Warning, "| AddReferencedObjects - %d - 0x%p - %ls |", ManagedPSCs.Num(), World, *World->GetFullName());
		...
		for (int32 PSCIndex = 0; PSCIndex < PendingRegisterPSCs.Num(); ++PSCIndex)
		{
			...
			{
				//UE_LOGF(LogParticles, Warning, "| Add Pending PSC Ref %d - 0x%p |", PSCIndex, PendingRegisterPSCs[PSCIndex]);
	...
	{
		//UE_LOGF(LogParticles, Warning, "| AddReferencedObjects - NULL WORLD |", ManagedPSCs.Num(), World, *World->GetFullName());
	}
}
...
{
	UE_LOGF(LogParticles, Verbose, "| HandlePostGarbageCollect | WorldMan: %p |", this);
...
{
	UE_LOGF(LogParticles, Verbose, "| FParticleSystemWorldManager::Cleanup() | %p |", this);
...
bool FParticleSystemWorldManager::RegisterComponent(UParticleSystemComponent* PSC)
{
	...
	if (Handle == INDEX_NONE)
	{
		if (!PSC->IsPendingManagerAdd())
		{
			...
			UE_LOGF(LogParticles, Verbose, "| Register New: %p | Man: %p | %d | %ls", PSC, this, Handle, *PSC->Template->GetName());
			return true;
		}
		else
		{
			UE_LOGF(LogParticles, Verbose, "| Register Existing Pending PSC: %p | Man: %p | %d | %ls", PSC, this, Handle, *PSC->Template->GetName());
			return false;
		}
	}
	else if(!PSC->IsPendingManagerAdd())
	{
		...
		if(TickData.bPendingUnregister)
		{
			...
			UE_LOGF(LogParticles, Verbose, "| Re-Register Pending Unregister PSC: %p | Man: %p | %d | %ls", PSC, this, Handle, *PSC->Template->GetName());
		}
		else
		{

			UE_LOGF(LogParticles, Verbose, "| Register Existing PSC: %p | Man: %p | %d | %ls", PSC, this, Handle, *PSC->Template->GetName());
		}
...
void FParticleSystemWorldManager::UnregisterComponent(UParticleSystemComponent* PSC)
{
	...
	if (Handle != INDEX_NONE)
	{
		if (PSC->IsPendingManagerAdd())
		{
			UE_LOGF(LogParticles, Verbose, "| UnRegister Pending PSC: %p | Man: %p | %d | %ls", PSC, this, Handle, *PSC->Template->GetName());
		...
		else
		{
			...
			UE_LOGF(LogParticles, Verbose, "| UnRegister PSC: %p | Man: %p | %d | %ls", PSC, this, Handle, *PSC->Template->GetName());
		}
	}
...
void FParticleSystemWorldManager::AddPSC(UParticleSystemComponent* PSC)
{
	if (IsValid(PSC))  // Don't add PSC if it has been marked for deletion
	{
		...
		UE_LOGF(LogParticles, Verbose, "| Add PSC - PSC: %p | Man: %p | %d | %d |Num: %d |", ManagedPSCs[Handle].Get(), this, Handle, TickList[TickData.TickListHandle], ManagedPSCs.Num());
	}
}
...
void FParticleSystemWorldManager::RemovePSC(int32 PSCIndex)
{
	...
	UE_LOGF(LogParticles, Verbose, "| Remove PSC - PSC: %p | Man: %p | %d |Num: %d |", ManagedPSCs[PSCIndex].Get(), this, PSCIndex, ManagedPSCs.Num());

#if PSC_MAN_USE_STATIC_TICK_LISTS
	FTickList& TickList = TickData.bCanTickConcurrent ? TickLists_Concurrent[(int32)TickData.TickGroup] : TickLists_GT[(int32)TickData.TickGroup];

	UE_LOGF(LogParticles, Verbose, "| Remove PSC - PSC: %p | Man: %p | %d | %d |Num: %d |", ManagedPSCs[PSCIndex].Get(), this, PSCIndex, TickList[TickData.TickListHandle], ManagedPSCs.Num());
...
{
	//UE_LOGF(LogParticles, Verbose, "| ---- PSC World Manager Tick ----- | TG %ls | World: %p - %ls |", *TickGroupEnum->GetNameByValue(TickGroup).ToString(), World, *World->GetFullName());
	...
	//UE_LOGF(LogParticles, Warning, "| Queue Concurrent Ticks | TG: %ls |", *TickGroupEnum->GetNameByValue(TickGroup).ToString());
...
void FParticleSystemWorldManager::HandleManagerEnabled()
{
	if (GbEnablePSCWorldManager != bCachedParticleWorldManagerEnabled)
	{
		...
		if (bCachedParticleWorldManagerEnabled)
		{
			//UE_LOGF(LogParticles, Warning, "| Enabling Particle System World Manager |");
		...
		{
			//UE_LOGF(LogParticles, Warning, "| Disabling Particle System World Manager |");
...
void FParticleSystemWorldManager::Dump()
{
	...
	UE_LOGF(LogParticles, Log, "|	   	               Managed Particle System Component Tick State Info                                |");
	UE_LOGF(LogParticles, Log, "|-------------------------------------------------------------------------------------------------------|");
	for (int32 Handle = 0; Handle < ManagedPSCs.Num(); ++Handle)
	{
		...
		UE_LOGF(LogParticles, Log, "| %d | %ls |0x%p | Active: %d | Sig: %ls | Vis: %d | Num: %d | %ls | Prereq: 0x%p - %ls |",
...
{
	return FName("ParticleSystemManager");
}
