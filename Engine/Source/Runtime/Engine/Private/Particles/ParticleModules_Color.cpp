...
void UParticleModuleColor::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	{
		if (PropertyThatChanged->GetFName() == FName("bClampAlpha"))
		{
			...
			if (LODLevel)
			{
				...
				UE_LOGF(LogParticles, Warning, "UParticleModuleColor has an incorrect outer... run FixupEmitters on package %ls",
...
void UParticleModuleColorOverLife::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyThatChanged)
	{
		if (PropertyThatChanged->GetFName() == FName("bClampAlpha"))
		{
			...
			{
				// The outer is incorrect - warn the user and handle it
				UE_LOGF(LogParticles, Warning, "UParticleModuleColorOverLife has an incorrect outer... run FixupEmitters on package %ls",
					*(OuterObj->GetOutermost()->GetPathName()));
				OuterObj = LODLevel->GetOuter();
				UParticleEmitter* Emitter = Cast<UParticleEmitter>(OuterObj);
				check(Emitter);
...
bool UParticleModuleColorScaleOverLife::PerformCustomMenuEntry(int32 InEntryIndex)
{
	if (GIsEditor == true)
	{
		if (InEntryIndex == 0)
		{
			UE_LOGF(LogParticles, Log, "Setup color scale over life for particle param!");
			...
			if (ColorDist)
			{
				ColorDist->ParameterName = FName("InstanceColorScaleOverLife");
			...
			if (AlphaDist)
			{
				AlphaDist->ParameterName = FName("InstanceAlphaScaleOverLife");
