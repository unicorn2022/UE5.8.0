...
void UParticleModuleLocationBoneSocket::ValidateLODLevels(UParticleEmitter* Emitter, int32 iModule)
{
	...
	if (bRequiresValidate)
	{
		...
		for (int32 iLOD=1; iLOD < NumLODLevels; ++iLOD)
		{
			...
			{
				UE_LOGF(LogParticles, Warning, "UParticleModuleLocationBoneSocket: ParticleSystem '%ls' contains an Emitter '%ls' LOD '%d' with inconsistent SourceLocations that could lead to a crash or unexpected behaviour.  Fixing by copying the Highest LOD SourceLocations to LOD.", *Emitter->GetOuter()->GetFullName(), *Emitter->GetEmitterName().ToString(), iLOD);
...
void UParticleModuleLocationSkelVertSurface::GetSkeletalMeshComponentSource(const FContext& Context, FModuleLocationVertSurfaceInstancePayload* InstancePayload)
{
	...
	if (!InstancePayload->SourceComponent.IsValid() || bChangedActor)
	{
		...
		else
		{
			...
			UE_LOGF(LogParticles, Warning, "Attempting to use Cascade SkelVertSurface module on mesh without valid data.");
			UE_LOGF(LogParticles, Warning, "Likely due to CPU side buffers being stripped with r.FreeSkeletalMeshBuffers=1.");
			UE_LOGF(LogParticles, Warning, "This emitter should probably be culled for this platform using detail mode.");
			UE_LOGF(LogParticles, Warning, "Mesh: %ls", NewSkelMeshComp ? *NewSkelMeshComp->GetSkeletalMeshAsset()->GetFullName() : TEXT("NULL"));
			UE_LOGF(LogParticles, Warning, "Comp: %ls", *Owner->Component.GetFullName());
			UE_LOGF(LogParticles, Warning, "System: %ls", *Owner->Component.GetTemplate()->GetFullName());
			UE_LOGF(LogParticles, Warning, "----------------------------------------------------------------------");
