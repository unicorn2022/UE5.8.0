	...
	if (ActiveParticles > 0)
	{
		...
		for (int32 ParticleIdx = ActiveParticles - 1; ParticleIdx >= 0; ParticleIdx--)
		{
				...
				)
			{
				...
				UE_LOGF(LogParticles, Log, "Killing Particle %4d - Next = %4d, Prev = %4d, Type = %8ls", 
					CurrentIndex, 
	...
	FTrailsBaseTypeDataPayload*& OutTrailData)
{
	...
	{
		// Wrong direction!
		UE_LOGF(LogParticles, Warning, "GetParticleInTrail: END particle will always be in the NEXT direction!");
	}
	if ((InGetOption == GET_Start) && (InGetDirection == GET_Next))
	{
		// Wrong direction!
		UE_LOGF(LogParticles, Warning, "GetParticleInTrail: START particle will always be in the PREV direction!");
	}
	...
	)
{
	...
	if (TRAIL_EMITTER_IS_ONLY(StartTrailData->Flags))
	{
		...
		{
			UE_LOGF(LogParticles, Log, "BAD SPAWN TIME! Curr %8.6f (%9ls), Start %8.6f (%9ls), %ls (%ls)", 
				TrailData->SpawnTime, 
				...
				if ((int32)ActiveParticles > LocalMaxParticleInTrailCount)
				{
					if (Component.HasWorld())
					{
						...
						if (MaxCount < ActiveParticles)
						{
							...
							UE_LOGF(LogParticles, Log, "%ls", *ErrorMessage);
						}
					}
				}
		...
		case PET2SRCM_Particle:
			{
				...
				if (SourceEmitter && SourceEmitter->ParticleIndices && SourceEmitter->ActiveParticles > 0)
				{
					...
					if (SourceParticle != nullptr)
					{
						...
						{
							UE_LOGF(LogParticles, Warning, "TrailIdx: %d", InTrailIdx);
							UE_LOGF(LogParticles, Warning, "SourceEmitterParticleIndex: %d", SourceEmitterParticleIndex);
							UE_LOGF(LogParticles, Warning, "ActiveParticles: %d", SourceEmitter->ActiveParticles);

							UE_LOGF(LogParticles, Warning, "==============ParticleIndices=================");
							for (int32 i = 0; i < SourceEmitter->ActiveParticles; ++i)
							{
								UE_LOGF(LogParticles, Warning, "%d: %d", i, SourceEmitter->ParticleIndices[i]);
							}

							UE_LOGF(LogParticles, Warning, "==============ParticleData=================");
							for (int32 i = 0; i < SourceEmitter->ActiveParticles; ++i)
							{
								UE_LOGF(LogParticles, Warning, "-- Particle %d --", i);
								if (FBaseParticle* DumpParticle = SourceEmitter->GetParticleDirect(i))
								{
									UE_LOGF(LogParticles, Warning, "Location:{%6.4f, %6.4f, %6.4f}", DumpParticle->Location.X, DumpParticle->Location.Y, DumpParticle->Location.Z);
									UE_LOGF(LogParticles, Warning, "OldLocation:{%6.4f, %6.4f, %6.4f}", DumpParticle->OldLocation.X, DumpParticle->OldLocation.Y, DumpParticle->OldLocation.Z);
									UE_LOGF(LogParticles, Warning, "BaseVelocity:{%6.4f, %6.4f, %6.4f}", DumpParticle->BaseVelocity.X, DumpParticle->BaseVelocity.Y, DumpParticle->BaseVelocity.Z);
									UE_LOGF(LogParticles, Warning, "Velocity:{%6.4f, %6.4f, %6.4f}", DumpParticle->Velocity.X, DumpParticle->Velocity.Y, DumpParticle->Velocity.Z);
									UE_LOGF(LogParticles, Warning, "BaseSize:{%6.4f, %6.4f, %6.4f}", DumpParticle->BaseSize.X, DumpParticle->BaseSize.Y, DumpParticle->BaseSize.Z);
									UE_LOGF(LogParticles, Warning, "Size:{%6.4f, %6.4f, %6.4f}", DumpParticle->Size.X, DumpParticle->Size.Y, DumpParticle->Size.Z);
									UE_LOGF(LogParticles, Warning, "RelativeTime: %6.4f", DumpParticle->RelativeTime);
									UE_LOGF(LogParticles, Warning, "OneOverMaxLifetime: %6.4f", DumpParticle->OneOverMaxLifetime);
									UE_LOGF(LogParticles, Warning, "Rotation: %6.4f", DumpParticle->Rotation);
									UE_LOGF(LogParticles, Warning, "BaseRotationRate: %6.4f", DumpParticle->BaseRotationRate);
									UE_LOGF(LogParticles, Warning, "RotationRate: %6.4f", DumpParticle->RotationRate);
									UE_LOGF(LogParticles, Warning, "Flags: %d", DumpParticle->Flags);
									UE_LOGF(LogParticles, Warning, "Color:{%6.4f, %6.4f, %6.4f, %6.4f}", DumpParticle->Color.R, DumpParticle->Color.G, DumpParticle->Color.B, DumpParticle->Color.A);
									UE_LOGF(LogParticles, Warning, "BaseColor:{%6.4f, %6.4f, %6.4f, %6.4f}", DumpParticle->BaseColor.R, DumpParticle->BaseColor.G, DumpParticle->BaseColor.B, DumpParticle->BaseColor.A)
								}
								else
								{
									UE_LOGF(LogParticles, Warning, "Dump Particle was NULL");
								}
							}
...
bool FParticleRibbonEmitterInstance::FillReplayData(FDynamicEmitterReplayDataBase& OutData)
{
	...
	{
#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
		UE_LOGF(LogParticles, Warning, "RIBBON   : FillReplayData failed.");
		UE_LOGF(LogParticles, Warning, "\tIndexCount (%d) exceeds allowed value (%d).", IndexCount, MAX_TRAIL_INDICES);
		UE_LOGF(LogParticles, Warning, "\tActiveParticleCount = %d.", ActiveParticles);
		UE_LOGF(LogParticles, Warning, "\tTriangleCount = %d.", TriangleCount);
		UE_LOGF(LogParticles, Warning, "\tTrailCount = %d.", TrailCount);
		UE_LOGF(LogParticles, Warning, "\t%ls", Component.GetTemplate() ? 
	...
	if (TriangleCount <= 0)
	{
		if ((ActiveParticles > 0) && (ActiveParticles != HeadOnlyParticles))
		{
			...
			{
// 				UE_LOGF(LogParticles, Warning, "TRAIL: GetDynamicData -- TriangleCount == 0 (APC = %4d) for PSys %ls",
// 					ActiveParticles, 
...
//	else
	{
		...
		case ETrailWidthMode_FromSecond:
		{
			...
			break;
		default:
			UE_LOGF(LogParticles, Fatal, "Invalid Width Mode for trail: %u", (uint32)WidthMode);
			break;
		}
		...
		else
		{
			...
			if (TRAIL_EMITTER_IS_ONLY(StartTrailData->Flags))
			{
				...
				if (TrailData->SpawnTime < StartTrailData->SpawnTime)
				{
					UE_LOGF(LogParticles, Log, "BAD SPAWN TIME! Curr %8.6f, Start %8.6f", TrailData->SpawnTime, StartTrailData->SpawnTime);
				}
...
bool FParticleAnimTrailEmitterInstance::FillReplayData( FDynamicEmitterReplayDataBase& OutData )
{
	...
	{
#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
		UE_LOGF(LogParticles, Warning, "ANIMTRAIL: FillReplayData failed.");
		UE_LOGF(LogParticles, Warning, "\tIndexCount (%d) exceeds allowed value (%d).", IndexCount, MAX_TRAIL_INDICES);
		UE_LOGF(LogParticles, Warning, "\tActiveParticleCount = %d.", ActiveParticles);
		UE_LOGF(LogParticles, Warning, "\tTriangleCount = %d.", TriangleCount);
		UE_LOGF(LogParticles, Warning, "\tTrailCount = %d.", TrailCount);
...
{
	UE_LOGF(LogParticles, Log, "%d| Particle %4d - Next = %4d, Prev = %4d, Type = %8ls, Life=%.5f",
		TrailIndex,
		CurrentIndex,
		TRAIL_EMITTER_GET_NEXT(TrailData->Flags),
		TRAIL_EMITTER_GET_PREV(TrailData->Flags),
	...
	UE_LOGF(LogParticles, Log, "TI=%4d - TC=%4d, SpT=%.5f, SpD= %.5f, TU=%.4f, SpTP=%4d, RIntpC=%4d, PSF=%.4f, %ls, %ls",
...
void FParticleAnimTrailEmitterInstance::PrintTrails()
{
	...
	{
		UE_LOGF(LogParticles, Log, "===========================================");
		UE_LOGF(LogParticles, Log, "Active: %d", ActiveParticles);
		UE_LOGF(LogParticles, Log, "===========================================");
...
void FParticleTrailsEmitterInstance_Base::DumpCircularTrailsSpam()
{
	...
	{
		UE_LOGF(LogParticles, Warning, "Circular links in Anim Trail discovered. \nPSys: %ls\nEmitter: %ls", *Component.GetTemplate()->GetFullName(), *SpriteTemplate->GetEmitterName().ToString());
