...
static TAutoConsoleVariable<int32> CVarMobileVirtualTextures(
	FCVarDeprecationArgs{ TEXT("5.8"), EShadowCVarBehavior::Ensure, TEXT("r.Mobile.VirtualTextures is deprecated, use r.VirtualTextures in platform specific engine .ini files instead")},
	TEXT("r.Mobile.VirtualTextures"),
	0,
	TEXT("[DEPRECATED] Enable virtual texture suppport on mobile platforms."),
	ECVF_RenderThreadSafe | ECVF_ReadOnly
);
...
{
	return UseVirtualTexturing(TargetPlatform);
}
...
void UTexture::ValidateSettingsAfterImportOrEdit(bool * pRequiresNotifyMaterials)
{
	...
	if (Source.IsValid()) // we can have an empty source if the last source in a texture2d array is removed via the editor.
	{
		if ( MipGenSettings == TMGS_LeaveExistingMips && PowerOfTwoMode != ETexturePowerOfTwoSetting::None )
		{
			...
			UE_LOGF(LogTexture, Display, "Texture padding or resizing cannot be used with LeaveExistingMips, disabled. (%ls)", *GetName());
		...
		if ( PowerOfTwoMode == ETexturePowerOfTwoSetting::ResizeToSpecificResolution && !this->IsA<UTexture2D>() )
		{
			...
			UE_LOGF(LogTexture, Display, "Currently resizing is only supported for Texture2D, forcing PowerOfTwoMode to None. (%ls)", *GetName());
		...
		if ( VirtualTextureStreaming && ! IsVirtualTexturingEnabled() )
		{
			...
			UE_LOGF(LogTexture, Display, "Texture %ls was VT, but VT is off on the project, turning off on texture.", *GetName());
		...
		if ( bLargeTextureMustBeVT && ! VirtualTextureStreaming && ( MaxTextureSize == 0 || MaxTextureSize > RHIMaximumDimension ) )
		{
			if ( IsVirtualTexturingEnabled() )
			{
				...
				{
					UE_LOGF(LogTexture, Display, "Large Texture %ls Dimension=%d changed to VT; to disable VT set MaxTextureSize first", *GetName(), LargerSourceDimension);
					VirtualTextureStreaming = true;
					bRequiresNotifyMaterials = true;
				}
				else
				{
					UE_LOGF(LogTexture, Warning, "Large Texture %ls Dimension=%d needs to be VT but is not 2d, changing MaxTextureSize", *GetName(), LargerSourceDimension);
			...
			{
				UE_LOGF(LogTexture, Warning, "Large Texture %ls Dimension=%d must be VT but VirtualTextures are disabled, changing MaxTextureSize", *GetName(), LargerSourceDimension);
		...
		if (VirtualTextureStreaming)
		{
			...
			if (!bIsPowerOfTwo)
			{
				...
				{
					UE_LOGF(LogTexture, Warning, "VirtualTextureStreaming not supported for \"%ls\", texture size is not a power-of-2", *GetName());
					VirtualTextureStreaming = false;
					bRequiresNotifyMaterials = true;
				}
			}
			...
			{
				UE_LOGF(LogTexture, Warning, "VirtualTextureStreaming is not compatible with ColorLookupTable LODGroup as virtual textures require mips (%ls)", *GetName());
				VirtualTextureStreaming = false;
				bRequiresNotifyMaterials = true;
			}
			...
			{
				UE_LOGF(LogTexture, Display, "Virtual textures require mips and MipGenSettings is NoMipmaps: Forcing to SimpleAverage (%ls)", *GetName());
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
		...
		else if ( PropertyName == CompositeTextureName )
		{
			...
			{
				UE_LOGF(LogTexture, Warning, "Cycle detected: Requested composite texture has this texture as its composite texture!");
...
void UTexture::Serialize(FArchive& Ar)
{
	...
	if (Ar.IsCooking() && VirtualTextureStreaming)
	{
		...
		{
			UE_LOGF(LogTexture, Display, "%ls is marked for virtual streaming but virtual texture streaming is not available.", *GetPathName());
		}
	}
	...
	if (Ar.IsLoading())
	{
		...
		{
			UE_LOGF(LogTexture, Display, "Texture \"%ls\" has CompressionFormat=PNG but not bPNGCompressed, assuming texture is actually uncompressed.", *GetPathName());
		...
		if ( Source.bPNGCompressed_DEPRECATED )
		{
			...
			{
				UE_LOGF(LogTexture, Warning, "Texture \"%ls\" has CompressionFormat=%d=%ls with bPNGCompressed, ignoring bPNGCompressed.", 
			...
			|| Source.GetFormat() == TSF_RGBE8_DEPRECATED )
		{
			...
			UE_LOGF(LogTexture, Warning, "TextureSource is a deprecated RB swapped format, needs reimport!: %ls", *GetPathName());
		...
		{
			UE_LOGF(LogTexture, Warning, "CompositeTextureMode was invalid in uasset, disabled.  Fix content and re-save : %ls", *GetPathName());
...
#if WITH_EDITOR
void UTexture::GetAssetRegistryTagMetadata(TMap<FName, FAssetRegistryTagMetadata>& OutMetadata) const
{
	Super::GetAssetRegistryTagMetadata(OutMetadata);

	OutMetadata.Add("SourceCompression",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UTexture", "SourceCompression_Label", "Source Compression"))
		);

	OutMetadata.Add("SourceFormat",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UTexture", "SourceFormat_Label", "Source Format"))
		);

	OutMetadata.Add("IsSourceValid",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UTexture", "IsSourceValid_Label", "Is Source Valid"))
		);

	OutMetadata.Add("PowerOfTwoMode",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UTexture", "PowerOfTwoMode_Label", "Power Of Two Mode"))
		);

	OutMetadata.Add("MipGenSettings",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UTexture", "MipGenSettings_Label", "Mip Gen Settings"))
		);

	OutMetadata.Add("MaxTextureSize",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UTexture", "MaxTextureSize_Label", "Max Texture Size"))
		);
}
#endif
...
FStreamableRenderResourceState UTexture::GetResourcePostInitState(const FTexturePlatformData* PlatformData, bool bAllowStreaming, int32 MinRequestMipCount, int32 MaxMipCount, bool bSkipCanBeLoaded) const
{
	...
	if ( NumMips < NumOfNonStreamingMips )
	{
		...
		if ( NumOfNonStreamingMips > MaxRuntimeMipCount )
		{
			...
			UE_LOGF(LogTexture, Warning, "NumOfNonStreamingMips > MaxRuntimeMipCount. (%ls)", *GetName());
...
FSharedBuffer FTextureSource::Decompress(IImageWrapperModule* ) const
{
	...
	{
		UE_LOGF(LogTexture,Warning,"Decompressed buffer does not match expected size : %lld != %lld",
...
FMutableMemoryView FTextureSource::LockMipInternal(int32 BlockIndex, int32 LayerIndex, int32 MipIndex, ELockState RequestedLockState, FImageInfo & OutImageInfo)
{
	...
	if (NumLockedMips > 0 && RequestedLockState != LockState)
	{
		...
		else
		{
			...
			UE_LOGF(LogTexture,Error, "LockMip cannot lock for write when previously locked for read [%ls]", 
				Owner ? *Owner->GetFullName() : *TornOffOwnerName);	
	...
	{
		UE_LOGF(LogTexture,Error,"Mip Data is too small : %lld < %lld+%lld", LockedMipData.GetSize(),MipOffset,MipSize); 
		LockedMipData.Reset();
		return FMutableMemoryView();
	}
...
void FTextureSource::UnlockMip(int32 BlockIndex, int32 LayerIndex, int32 MipIndex)
{
	...
	if (NumLockedMips == 0u)
	{
		...
		if (LockState == ELockState::ReadWrite)
		{
			UE_CLOGF(CompressionFormat == TSCF_JPEG, LogTexture, Warning, "Call to FTextureSource::UnlockMip will cause texture source to lose it's jpeg storage format");
...
FTextureSource::FMipData FTextureSource::GetMipData(IImageWrapperModule* )
{
	...
	{
		// failed!
		UE_LOGF(LogTexture, Error, "LockMipReadOnly failed in GetMipData");
...
EGammaSpace FTextureSource::GetGammaSpace(int LayerIndex) const
{
	...
	else
	{
		...
		{
			UE_LOGF(LogTexture, Error, "Torn off texture source doesn't have gamma copied!");
			return EGammaSpace::Linear;
		}
...
FSharedBuffer FTextureSource::TryDecompressData() const
{
	...
	else if (NumLayers == 1 && NumSlices == 1 && Blocks.Num() == 0)
	{
		...
		if ( ! FImageUtils::DecompressImage(Payload.GetData(), Payload.GetSize(), Image) )
		{
			...
			UE_LOGF(LogTexture, Display, "TryDecompressData failed: LayerSize = %lld PayloadSize = %lld",LayerSize,PayloadSize);
			UE_LOGF(LogTexture, Display, "TryDecompressData failed: LoadedMainStreamObjectVersion = %d, TextureSourceVirtualization = %d, VolumetricCloudReflectionSampleCountDefaultUpdate = %d",
			...
			if ( LayerSize == PayloadSize )
			{
				...
				UE_LOGF(LogTexture, Warning, "TryDecompressData data marked compressed appears to be uncompressed?");
				return Payload;
			}
			else
			{
				UE_LOGF(LogTexture, Error, "TryDecompressData failed to return uncompressed data");
				return FSharedBuffer();
			}
		}
		...
		if ( Image.Format != RawFormat )
		{
			...
			UE_LOGF(LogTexture, Warning, "TryDecompressData unexpected format conversion?");
	...
	{
		UE_LOGF(LogTexture, Warning, "Compressed source art is in an invalid format NumLayers:(%d) NumSlices:(%d) NumBlocks:(%d)",
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FTextureSource::ImportCustomProperties);
		...
		if (PayloadSize > (uint64)0)
		{
			...
			if (DestData)
			{
				...
				while (FChar::IsHexDigit(*SourceText))
				{
					...
					{
						uint8 Value = 0;
						// Don't use FCString::Strtoi as it can be pathologically slow on some platforms because of the StringCast conversion to convert from TCHAR to char. 
						while (FChar::IsHexDigit(*SourceText))
						{
							Value = (Value << 4) | TCharToNibble(*SourceText++);
						}
						DestData[Index++] = Value;
					}
					else
					{
						// If there are more hex tokens despite the Index having reached PayloadSize, then the input is malformed. We still want to advance the parsing in such to avoid looping infintely
						while (FChar::IsHexDigit(*SourceText))
						{
							++SourceText;
						}
				...
				if (NumTokensParsed != PayloadSize)
				{
					Warn->Log(*FText::Format(NSLOCTEXT("UnrealEd", "Importing_TextureSource_SyntaxError", "Syntax Error : num tokens: {0} vs. payload size: {1}"), { NumTokensParsed, PayloadSize}).ToString());
					bSuccess = false;
				}
			}
			else
	...
	if ( bHasFormat )
	{
		if ( ! bOodleTextureSdkVersionIsNone )
		{
			...
			else
			{
				...
				UE_CALL_ONCE( [&](){
					UE_LOGF(LogTexture, Verbose, "AlternateTextureCompression/TextureCompressionFormatWithVersion not specified, using %ls.", *TextureCompressionFormat);
				} );
			}
		}
		...
		if ( TextureFormatModule )
		{
			...
			{
				UE_CALL_ONCE( [&](){
						UE_LOGF(LogTexture, Warning, "AlternateTextureCompression specified, Module found, but no TextureFormat : %ls.", *TextureCompressionFormat);
				} );
			}
		}
		else
		{
			UE_CALL_ONCE( [&](){
				UE_LOGF(LogTexture, Warning, "AlternateTextureCompression specified but Module not found: %ls.", *TextureCompressionFormat);
			} );
		}
	}
#endif
...
void UTexture::GetBuiltTextureSize(const ITargetPlatformSettings* TargetPlatformSettings, const ITargetPlatformControls* TargetPlatformControls, int32 & OutSizeX, int32 & OutSizeY, int32& OutSizeZ ) const
{
	...
	if ( Source.IsValid() && TargetPlatformSettings )
	{
		...
		const uint32 LODBiasNoCinematics = FMath::Max<int32>(LODSettings.CalculateLODBias(SizeX, SizeY, GetMinTextureSize(), GetMaxTextureSize(), LODGroup, LODBias, 0, MipGenSettings, bVirtualTextureStreaming, IsStreamable()), 0);
	...
	bool bSupportCompressedVolumeTexture, int32 Unused_BlockSize, bool bSupportFilteredFloat32Textures )
{
	...
	static FName NameDXT1("DXT1");
	static FName NameDXT5("DXT5");
	static FName NameDXT5n("DXT5n");
	static FName NameAutoDXT("AutoDXT");
	static FName NameBC4("BC4");
	static FName NameBC5("BC5");
	static FName NameBGRA8("BGRA8");
	static FName NameXGXR8("XGXR8");
	static FName NameG8("G8");
	static FName NameG16("G16");
	static FName NameVU8("VU8");
	static FName NameRGBA16F("RGBA16F");
	static FName NameRGBA32F("RGBA32F");
	static FName NameR16F("R16F");
	static FName NameR32F("R32F");
	static FName NameBC6H("BC6H");
	static FName NameBC7("BC7");
	static FName NameR5G6B5("R5G6B5");
	static FName NameA1RGB555("A1RGB555");
		...
		( TextureFormatName == NameR32F || TextureFormatName == NameRGBA32F ) )
	{
		...
		if ( Filter != ETextureSamplerFilter::Point )
		{
			...
			UE_LOGF(LogTexture, Display, "32 bit float texture changed to 16F because Filter is not Nearest and !bSupportFilteredFloat32Textures : %ls", *Texture->GetPathName());
...
void UTexture::Blueprint_GetTextureSourceDiskAndMemorySize(int64 & OutDiskSize,int64 & OutMemorySize) const
{
	...
	UE_LOGF(LogTexture, Error, "Blueprint_GetTextureSourceDiskAndMemorySize can only be called WITH_EDITORONLY_DATA. (%ls)", *GetName());
#endif
}
...
bool UTexture::ComputeTextureSourceChannelMinMax(FLinearColor & OutColorMin, FLinearColor & OutColorMax) const
{
	...
	UE_LOGF(LogTexture, Error, "ComputeTextureSourceChannelMinMax failed to GetMipImage. (%ls)", *GetName());
	return false;
#else
	UE_LOGF(LogTexture, Error, "ComputeTextureSourceChannelMinMax can only be called WITH_EDITORONLY_DATA. (%ls)", *GetName());
	return false;
#endif
}
...
bool FTextureSource::UpdateChannelMinMaxFromIncomingTextureData(FMemoryView InNewTextureData)
{
	...
	for (int32 LayerIndex = 0; LayerIndex < NumLayers; LayerIndex++)
	{
		...
		for (int32 BlockIndex = 0; BlockIndex < GetNumBlocks(); BlockIndex++)
		{
			...
			{
				UE_LOGF(LogTexture, Error, "Invalid mip size in texture source init: passed in size doesn't accomodate all mips!");
				ResetLayerColorInfo();
				return false;
			}
		} // end each block
