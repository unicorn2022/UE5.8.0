...
{
	//Cached value for UWorldPartitionHLODRuntimeSubsystem::IsWarmupEnabled
	static bool bWarmupEnabled = true;
}

static void HLODRuntimeSubsystemCVarSinkFunction()
{
	// Wait to evaluate if warmup is enabled until after RHI is initialized. Sink functions get called very early in engine init
	if (!GIsRHIInitialized)
	{
		return;
	}

	// Check if any cvars affecting warmup have changed
	bool bNewWarmupEnabled = UWorldPartitionHLODRuntimeSubsystem::IsWarmupEnabled();
	if (FHLODRuntimeSubsystem::bWarmupEnabled == bNewWarmupEnabled)
	{
		return;
	}

	// Update the static cached value and every subsystem instance
	FHLODRuntimeSubsystem::bWarmupEnabled = bNewWarmupEnabled;
	for (UWorldPartitionHLODRuntimeSubsystem* HLODRuntimeSubsystem : TObjectRange<UWorldPartitionHLODRuntimeSubsystem>(RF_ClassDefaultObject | RF_ArchetypeObject, false, EInternalObjectFlags::Garbage))
	{
...
UWorldPartitionHLODRuntimeSubsystem::UWorldPartitionHLODRuntimeSubsystem()
	: UWorldSubsystem()
	, bCachedShouldPerformWarmup(FHLODRuntimeSubsystem::bWarmupEnabled)
{
}
...
bool UWorldPartitionHLODRuntimeSubsystem::IsWarmupEnabled()
{
...
void UWorldPartitionHLODRuntimeSubsystem::OnWorldPartitionInitialized(UWorldPartition* InWorldPartition)
{
	if (InWorldPartition && InWorldPartition->IsStreamingEnabled())
	{
		...
		if (InWorldPartition && InWorldPartition->RuntimeHash)
		{
			...
			{
				UE_LOGF(LogHLODRuntimeSubsystem, Verbose, "Registering cell %ls - %ls", *Cell->GetGuid().ToString(), *Cell->GetDebugName());
				...
				if (InWorldPartition->HasStandaloneHLOD())
				{
					...
					if (TSet<IWorldPartitionHLODObject*>* StandaloneHLODObjects = StandaloneHLODObjectsReferencingUnloadedCells.Find(Cell->GetGuid()))
					{
						for (IWorldPartitionHLODObject* HLODObject : (*StandaloneHLODObjects))
						{
							...
							UE_LOGF(LogHLODRuntimeSubsystem, Verbose, "Attached Standalone HLOD %ls to cell %ls", *HLODObject->GetHLODNameOrLabel(), *Cell->GetGuid().ToString());
						}
...
void UWorldPartitionHLODRuntimeSubsystem::OnWorldPartitionUninitialized(UWorldPartition* InWorldPartition)
{
	if (InWorldPartition && InWorldPartition->IsStreamingEnabled())
	{
			...
			for (const TPair<FGuid, FCellData>& It : WorldPartitionHLODRuntimeData.CellsData)
			{
				...
				for (IWorldPartitionHLODObject* HLODObject : CellData.LoadedHLODs)
				{
					if (HLODObject->IsStandalone())
					{
						...
						HLODObject->SetVisibility(UWorldPartitionHLODRuntimeSubsystem::WorldPartitionHLODEnabled);
						UE_LOGF(LogHLODRuntimeSubsystem, Verbose, "Detached Standalone HLOD %ls from cell %ls", *HLODObject->GetHLODNameOrLabel(), *CellGuid.ToString());
					}
				}
			}
		}
		...
		{
			UE_LOGF(LogHLODRuntimeSubsystem, Verbose, "Unregistering cell %ls - %ls", *Cell->GetGuid().ToString(), *Cell->GetDebugName());
			return true;
		});
#endif
...
void UWorldPartitionHLODRuntimeSubsystem::OnExternalStreamingObjectInjected(URuntimeHashExternalStreamingObjectBase* ExternalStreamingObject)
{
	...
	if (WorldPartitionHLODRuntimeData)
	{
		...
		{
			UE_LOGF(LogHLODRuntimeSubsystem, Verbose, "Registering external cell %ls - %ls", *Cell.GetGuid().ToString(), *Cell.GetDebugName());
...
void UWorldPartitionHLODRuntimeSubsystem::OnExternalStreamingObjectRemoved(URuntimeHashExternalStreamingObjectBase* ExternalStreamingObject)
{
	...
	if (WorldPartitionHLODRuntimeData)
	{
		ExternalStreamingObject->ForEachStreamingCells([WorldPartitionHLODRuntimeData](const UWorldPartitionRuntimeCell& Cell)
		{
			UE_LOGF(LogHLODRuntimeSubsystem, Verbose, "Unregistering external cell %ls - %ls", *Cell.GetGuid().ToString(), *Cell.GetDebugName());
...
void UWorldPartitionHLODRuntimeSubsystem::RegisterHLODObject(IWorldPartitionHLODObject* InWorldPartitionHLOD)
{
		...
		if (const UWorldPartition* WorldPartition = FHLODRuntimeSubsystem::GetWorldPartition(InWorldPartitionHLOD))
		{
			if (const FGuid* CustomHLODSourceCellGuidPtr = WorldPartition->RuntimeHash->GetCustomHLODActorSourceCellOverride(InWorldPartitionHLOD->GetCustomHLODGuid()))
			{
				...
				if (FCellData* CustomHLODSourceCellData = GetCellData(WorldPartition, CustomHLODSourceCellGuid))
				{
					...
					UE_LOGF(LogHLODRuntimeSubsystem, Verbose, "Registering Custom HLOD %ls for overriden source cell '%ls'", *InWorldPartitionHLOD->GetHLODNameOrLabel(), *CustomHLODSourceCellGuid.ToString());
				...
				{
					UE_LOGF(LogHLODRuntimeSubsystem, Log, "Override source cell '%ls' not found for Custom HLOD actor %ls", *CustomHLODSourceCellGuid.ToString(), *InWorldPartitionHLOD->GetHLODNameOrLabel());
				}
			}
			else
			{
				UE_LOGF(LogHLODRuntimeSubsystem, Log, "Override source cell not found for Custom HLOD actor %ls", *InWorldPartitionHLOD->GetHLODNameOrLabel());
			}
		}
	}
	else if (FCellData* SourceCellData = GetCellData(InWorldPartitionHLOD))
	{
		UE_LOGF(LogHLODRuntimeSubsystem, Verbose, "Registering HLOD %ls for cell %ls", *InWorldPartitionHLOD->GetHLODNameOrLabel(), *InWorldPartitionHLOD->GetSourceCellGuid().ToString());
	...
	{	
		...
		if (const FGuid* StandaloneHLODSourceCellsGuidPtr = WorldPartition->RuntimeHash->GetStandaloneHLODActorSourceCellOverride(InWorldPartitionHLOD->GetStandaloneHLODGuid()))
		{
			...
			{
				// Overriden source cell found in WorldPartitionHLODRuntimeData. Add the HLOD Actor to its LoadedHLODs.
				UE_LOGF(LogHLODRuntimeSubsystem, Verbose, "Registering Standalone HLOD %ls for overriden source cell '%ls'", *InWorldPartitionHLOD->GetHLODNameOrLabel(), *StandaloneHLODSourceCellGuid.ToString());
			...
			else
			{
				...
				UE_LOGF(LogHLODRuntimeSubsystem, Verbose, "Registering Standalone HLOD %ls referencing currently not loaded overriden source cell '%ls'", *InWorldPartitionHLOD->GetHLODNameOrLabel(), *StandaloneHLODSourceCellGuid.ToString());
		...
		else if (TObjectPtr<UWorldPartition>* StandaloneWorldPartitionPtr = StandaloneHLODCellToWorldPartitionMap.Find(InWorldPartitionHLOD->GetSourceCellGuid()))
		{
			...
			if (FCellData* StandaloneHLODSourceCellData = GetCellData(StandaloneWorldPartition, InWorldPartitionHLOD->GetSourceCellGuid()))
			{
				...
				UE_LOGF(LogHLODRuntimeSubsystem, Verbose, "Registering Standalone HLOD %ls for Standalone LI source cell '%ls'", *InWorldPartitionHLOD->GetHLODNameOrLabel(), *InWorldPartitionHLOD->GetSourceCellGuid().ToString());
		...
		else
		{
			...
			UE_LOGF(LogHLODRuntimeSubsystem, Verbose, "Registering Standalone HLOD %ls referencing currently not loaded cell '%ls'", *InWorldPartitionHLOD->GetHLODNameOrLabel(), *InWorldPartitionHLOD->GetSourceCellGuid().ToString());
	...
	{
		UE_LOGF(LogHLODRuntimeSubsystem, Verbose, "Found HLOD %ls referencing nonexistent cell '%ls'", *InWorldPartitionHLOD->GetHLODNameOrLabel(), *InWorldPartitionHLOD->GetSourceCellGuid().ToString());
...
void UWorldPartitionHLODRuntimeSubsystem::UnregisterHLODObject(IWorldPartitionHLODObject* InWorldPartitionHLOD)
{
		...
		if (const UWorldPartition* WorldPartition = FHLODRuntimeSubsystem::GetWorldPartition(InWorldPartitionHLOD))
		{
			if (const FGuid* CustomHLODSourceCellGuidPtr = WorldPartition->RuntimeHash->GetCustomHLODActorSourceCellOverride(InWorldPartitionHLOD->GetCustomHLODGuid()))
			{
				...
				if (FCellData* CustomHLODSourceCellData = GetCellData(WorldPartition, CustomHLODSourceCellGuid))
				{
					...
					UE_LOGF(LogHLODRuntimeSubsystem, Verbose, "Unregistering Custom HLOD %ls for overriden source cell %ls", *InWorldPartitionHLOD->GetHLODNameOrLabel(), *CustomHLODSourceCellGuid.ToString());
	...
	{
		UE_LOGF(LogHLODRuntimeSubsystem, Verbose, "Unregistering HLOD %ls for cell %ls", *InWorldPartitionHLOD->GetHLODNameOrLabel(), *InWorldPartitionHLOD->GetSourceCellGuid().ToString());
	...
	else if (InWorldPartitionHLOD->IsStandalone())
	{
		...
		if (const FGuid* SourceCellGuidPtr = WorldPartition->RuntimeHash->GetStandaloneHLODActorSourceCellOverride(InWorldPartitionHLOD->GetStandaloneHLODGuid()))
		{
			...
			{
				// Overriden source cell found. Remove the HLOD object from its LoadedHLODs.
				UE_LOGF(LogHLODRuntimeSubsystem, Verbose, "Unregistering Standalone HLOD %ls for overriden source cell %ls", *InWorldPartitionHLOD->GetHLODNameOrLabel(), *SourceCellGuid.ToString());
			...
			else
			{
				...
				UE_LOGF(LogHLODRuntimeSubsystem, Verbose, "Unregistering Standalone HLOD %ls for currently not loaded overriden source cell %ls", *InWorldPartitionHLOD->GetHLODNameOrLabel(), *SourceCellGuid.ToString());
		...
		else if (TObjectPtr<UWorldPartition>* StandaloneWorldPartitionPtr = StandaloneHLODCellToWorldPartitionMap.Find(InWorldPartitionHLOD->GetSourceCellGuid()))
		{
			...
			if (FCellData* SourceCellData = GetCellData(StandaloneWorldPartition, InWorldPartitionHLOD->GetSourceCellGuid()))
			{
				...
				UE_LOGF(LogHLODRuntimeSubsystem, Verbose, "Unregistering Standalone HLOD %ls for Standalone LI source cell %ls", *InWorldPartitionHLOD->GetHLODNameOrLabel(), *InWorldPartitionHLOD->GetSourceCellGuid().ToString());
		...
		else
		{
			...
			UE_LOGF(LogHLODRuntimeSubsystem, Verbose, "Unregistering Standalone HLOD %ls for currently not loaded cell %ls", *InWorldPartitionHLOD->GetHLODNameOrLabel(), *InWorldPartitionHLOD->GetSourceCellGuid().ToString());
...
void UWorldPartitionHLODRuntimeSubsystem::OnCellShown(const UWorldPartitionRuntimeCell* InCell)
{
	if (FCellData* CellData = GetCellData(InCell))
	{
		...
		{
			UE_LOGF(LogHLODRuntimeSubsystem, Verbose, "Cell shown - %ls - hiding %d HLOD objects", *InCell->GetGuid().ToString(), CellData->LoadedHLODs.Num());

			for (IWorldPartitionHLODObject* HLODObject : CellData->LoadedHLODs)
			{
				UE_LOGF(LogHLODRuntimeSubsystem, Verbose, "\t\t* %ls", *HLODObject->GetHLODNameOrLabel());
...
void UWorldPartitionHLODRuntimeSubsystem::OnCellHidden(const UWorldPartitionRuntimeCell* InCell)
{
	if (FCellData* CellData = GetCellData(InCell))
	{
		...
		if (!CellData->LoadedHLODs.IsEmpty())
		{
			UE_LOGF(LogHLODRuntimeSubsystem, Verbose, "Cell hidden - %ls - showing %d HLOD objects", *InCell->GetGuid().ToString(), CellData->LoadedHLODs.Num());
...
{
	// If warmup is disabled by cvar/platform setting, no warmup is needed
	if (!FHLODRuntimeSubsystem::bWarmupEnabled)
	{
		return false;
	}
