...
UHLODLayer::UHLODLayer(const FObjectInitializer& ObjectInitializer)
	...
	, EditorLoadingBehavior(EHLODLayerEditorLoadingBehavior::Default)
#endif
{
}
...
void UHLODLayer::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(UHLODLayer, ParentLayer))
	{
		...
		while (CurHLODLayer)
		{
			...
			if (bHLODLayerWasAlreadyInSet)
			{
				...
				UE_LOGF(LogHLODLayer, Error, "Circular HLOD parent chain detected: HLODLayer=%ls ParentLayer=%ls", *GetName(), *ParentLayer->GetName());
				break;
			}
		...
		{
			UE_LOGF(LogHLODLayer, Error, "Invalid HLOD settings. Parent layer %ls will reuse private materials created for this HLOD layer. Common error is to use an \"instancing\" parent layer, or a \"merged\" parent layer set to not merge materials. Change the type of HLOD generated by the parent layer or assign a new parent layer.", *ParentLayer->GetPathName());
			bInvalidParentLayer = true;
		}
	...
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(UHLODLayer, EditorLoadingBehavior))
	{
		// Warn when enabling loading for Instancing layers, as this can have performance/memory implications
		if (EditorLoadingBehavior == EHLODLayerEditorLoadingBehavior::ForceLoaded && LayerType == EHLODLayerType::Instancing)
		{
			FSuppressableWarningDialog::FSetupInfo Info(
				LOCTEXT("InstancingHLODLoadWarning_Message", "Loading Instancing HLOD actors in the editor can have significant performance and memory implications, as this may lead to a lot of assets being loaded.\n\nAre you sure you want to enable editor loading for this HLOD layer?"),
				LOCTEXT("InstancingHLODLoadWarning_Title", "Warning: Loading Instancing HLOD Layer"),
				TEXT("InstancingHLODLayerEditorLoadingWarning")
			);
			Info.ConfirmText = LOCTEXT("InstancingHLODLoadWarning_Confirm", "Yes");
			Info.CancelText = LOCTEXT("InstancingHLODLoadWarning_Cancel", "No");
		...
		// Refresh all loaders to re-evaluate which HLODs should be loaded
...
void UHLODLayer::ComputeHLODHash(FHLODHashBuilder& InHashBuilder) const
{
	...
	if (bForceRayTracingFarField)
	{
		InHashBuilder.HashField(bForceRayTracingFarField, GET_MEMBER_NAME_CHECKED(UHLODLayer, bForceRayTracingFarField));
	}
}

bool UHLODLayer::MustBeLoadedOnServer() const
{
	return HLODModifierClass && HLODModifierClass->GetDefaultObject<UWorldPartitionHLODModifier>()->MustBeLoadedOnServer();
}
