...
{
	const FString HLODSuffix = TEXT("_HLOD");
	...
	if (HLODActorsPtr)
	{
		...
		{
			int32 Position = HLODActor->GetActorLabel().Find(HLODSuffix, ESearchCase::IgnoreCase, ESearchDir::FromEnd);
			check(Position != INDEX_NONE);
	...
	else
	{
		...
		TMap<int32, FActorContainerID> HLODIndexToContainer;
		...
		for (const FString& Package : Packages)
		{
			...
			int32 Position = PackageName.Find(HLODSuffix, ESearchCase::IgnoreCase, ESearchDir::FromEnd);
			check(Position != INDEX_NONE); // Guaranteed by LevelPackagePrefix returned by GetStandaloneHLODFolderPathAndPackagePrefix
			FString NameSuffix = PackageName.RightChop(Position);
			FString ActorLabel = InStandaloneHLODActorParams.ActorLabel + NameSuffix;
			int32 HLODIndex = FCString::Atoi(*NameSuffix.RightChop(HLODSuffix.Len()));
			...
			UWorldPartitionRuntimeHash* WorldHash = GetWorld()->GetWorldPartition() ? GetWorld()->GetWorldPartition()->RuntimeHash.Get() : nullptr;
			if (!WorldHash)
			{
				UE_LOGF(LogStandaloneHLODSubsystem, Log, "RuntimeHash not found in World %ls", *GetWorld()->GetName());
				return;
			}
			...
			{
				UE_LOGF(LogStandaloneHLODSubsystem, Log, "Couldn't resolve Runtime Grid for %ls, HLODIndex %d", *WorldHash->GetDefaultGrid().ToString(), HLODIndex);
				return;
			}
			...
			HLODIndexToContainer.Add(HLODIndex, HLODActor->GetLevelInstanceID().GetContainerID());
		}

		// Create a mapping from HLOD container to container with child HLOD actors
		// e.g. container with HLOD1 actors maps to the container with HLOD0 actors
		HLODIndexToContainer.KeySort(TLess<int32>());
		const FActorContainerID* ChildContainer = nullptr;
		for (const TPair<int32, FActorContainerID>& Entry : HLODIndexToContainer)
		{
			if (ChildContainer)
			{
				HLODContainerToChildHLODActorsContainer.Add(Entry.Value, *ChildContainer);
			}
			ChildContainer = &Entry.Value;
		}
	}
}
...
void UWorldPartitionStandaloneHLODSubsystem::ForEachStandaloneHLODActorWithLevelInstanceGuid(TFunctionRef<void(AWorldPartitionStandaloneHLOD*, const FGuid& LevelInstanceGuid)> Func) const
{
	for (const TPair<FGuid, TArray<AWorldPartitionStandaloneHLOD*>>& Pair : StandaloneHLODActors)
	{
		const FGuid& LevelInstanceGuid = Pair.Key;
		const TArray<AWorldPartitionStandaloneHLOD*>& HLODActors = Pair.Value;
		for (AWorldPartitionStandaloneHLOD* HLODActor : HLODActors)
		{
			Func(HLODActor, LevelInstanceGuid);
		}
	}
}
...
bool UWorldPartitionStandaloneHLODSubsystem::GetChildHLODActorsContainer(const FActorContainerID& InContainerID, FActorContainerID& OutChildContainerID) const
{
	if (const FActorContainerID* ChildContainerID = HLODContainerToChildHLODActorsContainer.Find(InContainerID))
	{
		OutChildContainerID = *ChildContainerID;
		return true;
	}
	return false;
}
