...
TSubclassOf<UHLODInstancedStaticMeshComponent> UHLODBuilder::GetInstancedStaticMeshComponentClass()
{
	...
	if (!ISMClass)
	{
		...
		UE_LOGF(LogHLODBuilder, Error, "Could not resolve the class specified for HLODInstancedStaticMeshComponentClass. Config value was %ls", *ConfigValue);
...
TSubclassOf<UHLODInstancedSkinnedMeshComponent> UHLODBuilder::GetInstancedSkinnedMeshComponentClass()
{
	...
	if (!ISMClass)
	{
		...
		UE_LOGF(LogHLODBuilder, Error, "Could not resolve the class specified for HLODInstancedSkinnedMeshComponentClass. Config value was %ls", *ConfigValue);
...
TArray<UActorComponent*> UHLODBuilder::BatchInstances(const TArray<UActorComponent*>& InSourceComponents, TFunctionRef<bool(const FBox&)> InFilterFunc)
{
	...
	{
		UE_LOGF(LogHLODBuilder, Warning, "UHLODBuilder::BatchInstances - Excluding %d unsupported components:", FilteredOutComponents.Num());
		for (UActorComponent* FilteredOutComponent : FilteredOutComponents)
		{
			UE_LOGF(LogHLODBuilder, Warning, "    -> (%ls) %ls", *FilteredOutComponent->GetClass()->GetName(), *FilteredOutComponent->GetName());
		}
	}
	...
	auto IsAssetValidForInstancing = [](UActorComponent* InComponent, UObject* InObj)
	{
		...
		{
			UE_LOGF(LogHLODBuilder, Warning, "Instanced HLOD source component %ls points to a %ls mesh, ignoring.", *InComponent->GetPathName(), !InObj ? TEXT("null") : bPrivateAsset ? TEXT("private") : TEXT("transient"));
			return false;
		}
...
static bool ShouldBatchComponent(UActorComponent* ActorComponent)
{
	...
	if (UPrimitiveComponent* PrimitiveComponent = Cast<UPrimitiveComponent>(ActorComponent))
	{
		...
		case EHLODBatchingPolicy::MeshSection:
		{
			...
			UE_LOGF(LogHLODBuilder, Display, "EHLODBatchingPolicy::MeshSection is not yet supported by the HLOD builder, falling back to EHLODBatchingPolicy::Instancing for %ls.", *LogDetails);
			break;
		}
		default:
			checkNoEntry();
