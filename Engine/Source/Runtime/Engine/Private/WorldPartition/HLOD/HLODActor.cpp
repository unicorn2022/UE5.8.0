...
FWorldPartitionHLODActorEditorRegisteredDelegate AWorldPartitionHLOD::HLODActorEditorRegisteredDelegate;
FWorldPartitionHLODActorEditorUnregisteredDelegate AWorldPartitionHLOD::HLODActorEditorUnregisteredDelegate;
#endif
...
void AWorldPartitionHLOD::PreSave(FObjectPreSaveContext ObjectSaveContext)
{
	...
	if (ObjectSaveContext.IsCooking() && IsStandalone())
	{
		StandaloneHLODGuid = GetActorInstanceGuid();
	}
}
#endif

void AWorldPartitionHLOD::PostInitProperties()
{
	Super::PostInitProperties();

	if (HasAnyFlags(RF_ClassDefaultObject) && ExactCast<AWorldPartitionHLOD>(this))
	{
		CVarWorldPartitionHLODForceDisableShadows->OnChangedDelegate().RemoveAll(this);
		CVarWorldPartitionHLODForceDisableShadows->OnChangedDelegate().AddUObject(this, &AWorldPartitionHLOD::OnForceDisableShadowsCVarChanged);
	}
}

void AWorldPartitionHLOD::BeginDestroy()
{
	if (HasAnyFlags(RF_ClassDefaultObject) && ExactCast<AWorldPartitionHLOD>(this))
	{
		CVarWorldPartitionHLODForceDisableShadows->OnChangedDelegate().RemoveAll(this);
...
void AWorldPartitionHLOD::OnForceDisableShadowsCVarChanged(IConsoleVariable* CVar)
{
	const bool bShouldCastShadows = (CVar != nullptr) ? !CVar->GetBool() : true;
	...
	for (const FWorldContext& WorldContext : GEngine->GetWorldContexts())
	{
		...
		for (TActorIterator<AWorldPartitionHLOD> It(World); It; ++It)
		{
			...
			if (IsValid(HLODActor))
			{
				HLODActor->ApplyCastShadowToComponents(bShouldCastShadows);
			}
		}
	}
}

void AWorldPartitionHLOD::ApplyCastShadowToComponents(bool bInShouldCastShadows)
{
	ForEachComponent<UPrimitiveComponent>(false, [bInShouldCastShadows](UPrimitiveComponent* PrimitiveComponent)
	{
		PrimitiveComponent->SetCastShadow(bInShouldCastShadows);
	});
}
...
void AWorldPartitionHLOD::PreRegisterAllComponents()
{
	...
	{
		ApplyCastShadowToComponents(false);
	}
...
void AWorldPartitionHLOD::PostRegisterAllComponents()
{
	...
	if (UWorld* World = GetWorld())
	{
		...
		{
			HLODActorEditorRegisteredDelegate.Broadcast(this);
		}
	}
#endif
}
...
void AWorldPartitionHLOD::PostUnregisterAllComponents()
{
	...
	if (UWorld* World = GetWorld())
	{
		if (World->IsEditorWorld())
		{
			HLODActorEditorUnregisteredDelegate.Broadcast(this);
		}
	}
#endif
...
void AWorldPartitionHLOD::OnWorldCleanup(UWorld* InWorld, bool bSessionEnded, bool bCleanupResources)
{
	...
	if (World == InWorld)
	{
		...
		{
			UPackage* HLODResourcesPackage = GetHLODResourcesPackage();
			...
			{
				if (Asset->GetPackage() == HLODResourcesPackage)
				{
...
UPackage* AWorldPartitionHLOD::GetHLODResourcesPackage() const
{
	if (!HLODResourcesPackagePath.IsNone())
	{
		// If HLODResourcesPackagePath is set, HLOD resources are in that package
		return FindPackage(nullptr, *HLODResourcesPackagePath.ToString());
	}
	// If HLODResourcesPackagePath is not set, HLOD resources are in the actor's package
	return GetPackage();
}
...
bool AWorldPartitionHLOD::BuildHLOD(bool bForceBuild)
{
	bool bBuiltHLOD = false;
	...
	if (IWorldPartitionHLODUtilities* WPHLODUtilities = WPHLODUtilitiesModule != nullptr ? WPHLODUtilitiesModule->GetUtilities() : nullptr)
	{
		...
		BuildParams.HLODActor = this;
		BuildParams.bForceBuild = bForceBuild;
		bBuiltHLOD = WPHLODUtilities->BuildHLOD(BuildParams);
	}
...
bool AWorldPartitionHLOD::ShouldBuildHLOD() const
{
	bool bDirty = false;
	...
	{
		FHLODBuildParams BuildParams;
		BuildParams.HLODActor = const_cast<AWorldPartitionHLOD*>(this);
		BuildParams.bForceBuild = false;
		BuildParams.bTestOnly = true;
		bDirty = WPHLODUtilities->BuildHLOD(BuildParams);
	}
...
FWorldPartitionHLODActorEditorRegisteredDelegate& AWorldPartitionHLOD::GetHLODActorEditorRegisteredDelegate()
{
	return HLODActorEditorRegisteredDelegate;
}

FWorldPartitionHLODActorEditorUnregisteredDelegate& AWorldPartitionHLOD::GetHLODActorEditorUnregisteredDelegate()
{
	return HLODActorEditorUnregisteredDelegate;
}
...
void AWorldPartitionHLOD::UpdateHLODBuildReportContent()
{
	HLODBuildReport.Reset();
	...
	for (const TObjectPtr<UHLODRebuildPolicyData>& HLODRebuildPolicyData : HLODRebuildPolicyDataSet)
	{
		FString HLODPolicyDataString = HLODRebuildPolicyData->ToString();
		if (!HLODPolicyDataString.IsEmpty())
		{
			HLODBuildReport += HLODPolicyDataString.TrimEnd();
			HLODBuildReport += LINE_TERMINATOR;
		}
	}
...
TArray<UObject*> AWorldPartitionHLOD::ExportHLODAssets(const FExportHLODAssetsParams& ExportHLODAssetsParams, FString& OutErrorMessage) const
{
	...
	auto ShouldExportAsset = [ResourcesPackage = GetHLODResourcesPackage()](const UObject* InAsset)
	{
		// Skip null entries and assets that are not in the HLOD package (no need for export)
		return InAsset && InAsset->IsInPackage(ResourcesPackage);
	};
...
const FHLODRebuildPolicyDataSet& AWorldPartitionHLOD::GetHLODRebuildPolicyDataSet() const
{
	return HLODRebuildPolicyDataSet;
}

void AWorldPartitionHLOD::SetHLODRebuildPolicyDataSet(const FHLODRebuildPolicyDataSet& InHLODRebuildPolicyDataSet)
{
	HLODRebuildPolicyDataSet = InHLODRebuildPolicyDataSet;

	for (TObjectPtr<UHLODRebuildPolicyData>& HLODRebuildPolicyData : HLODRebuildPolicyDataSet)
	{
		HLODRebuildPolicyData->Rename(nullptr, this, REN_None);
	}

	UpdateHLODBuildReportContent();
}
