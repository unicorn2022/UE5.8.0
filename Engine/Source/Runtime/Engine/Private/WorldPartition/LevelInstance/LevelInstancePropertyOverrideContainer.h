...
class ULevelInstancePropertyOverrideContainer : public UActorDescContainer
{
	...
	virtual ~ULevelInstancePropertyOverrideContainer() = default;
...
private:
	...
	FWorldPartitionActorDesc* GetTransientOverrideActorDesc(const FGuid& InActorGuid, const FActorContainerPath& InContainerPath = FActorContainerPath()) const;
	...
	void OnBaseContainerActorDescPreUpdate(FWorldPartitionActorDesc* InActorDesc);
	...
	TMap<FActorContainerPath, TMap<FGuid, TUniquePtr<FWorldPartitionActorDesc>>> PerContainerTransientOverrideActorDescs;
#endif
};
