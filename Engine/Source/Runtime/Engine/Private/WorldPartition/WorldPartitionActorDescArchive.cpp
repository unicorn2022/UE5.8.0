	...
	if (IsLoading())
	{
		auto TryRedirectClass = [](FTopLevelAssetPath& InOutClassPath, bool bNativeClass)
		{
			if (InOutClassPath.IsValid())
			{
				...
				if (!bNativeClass)
				{
					FSoftObjectPath RedirectedClassPath(InOutClassPath);
	...
	if (!BaseDesc)
	{
		...
		if (!BaseDesc)
		{
			if (IsLoading())
			{
				...
				UE_LOGF(LogWorldPartition, Log, "Can't find class descriptor '%ls' for loading '%ls', using '%ls'", *ClassPath.ToString(), *ActorDesc->GetActorSoftPath().ToString(), *BaseDesc->GetActorSoftPath().ToString());
			}
			else
			{
				UE_LOGF(LogWorldPartition, Log, "Can't find class descriptor '%ls' for saving '%ls'", *ClassPath.ToString(), *ActorDesc->GetActorSoftPath().ToString());
			}
		}
	}
