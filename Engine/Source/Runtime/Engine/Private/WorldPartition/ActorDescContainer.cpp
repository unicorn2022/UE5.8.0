...
void UActorDescContainer::Initialize(const FInitializeParams& InitParams)
{
	...
	if (!ContainerPackageName.IsNone() && !FPackageName::IsTempPackage(ContainerPackageName.ToString()))
	{
		...
		if (InitParams.bShouldGatherInternalActors)
		{
	...
	UE_LOGF(LogWorldPartition, Verbose, "Parsed actor descriptor container package '%ls': %d external actors, %d internal actors", *InitParams.PackageName.ToString(), ExternalAssets.Num(), InternalAssets.Num());
	...
	TMap<FGuid, TUniquePtr<FWorldPartitionActorDesc>> ValidActorDescs;
	{
		...
		for (const FAssetData& Asset : ExternalAssets)
		{
			...
			{
				UE_LOGF(LogWorldPartition, Warning, "Invalid actor descriptor for actor '%ls' from package '%ls'", *Asset.GetObjectPathString(), *Asset.PackageName.ToString());
				InvalidActors.Emplace(Asset);
			} 
			else if (!ActorDesc->GetNativeClass().IsValid())
			{
				UE_LOGF(LogWorldPartition, Warning, "Invalid actor native class: Actor: '%ls' (guid '%ls') from package '%ls'",
			...
			{
				UE_LOGF(LogWorldPartition, Warning, "Unknown actor base class `%ls`: Actor: '%ls' (guid '%ls') from package '%ls'",
			...
			{
				UE_LOGF(LogWorldPartition, Warning, "Duplicate actor descriptor in package `%ls`: Actor: '%ls' -> Existing actor '%ls'", 
					*ActorDesc->GetActorPackage().ToString(), 
					*ActorDesc->GetActorNameString(), 
			...
			else if (TUniquePtr<FWorldPartitionActorDesc>* ExistingActorDescPtr = ValidActorDescs.Find(ActorDesc->GetGuid()))
			{
				...
				UE_LOGF(LogWorldPartition, Warning, "Duplicate actor descriptor guid `%ls`: Actor: '%ls' from package '%ls' -> Existing actor '%ls' from package '%ls'", 
					*ActorDesc->GetGuid().ToString(), 
					*ActorDesc->GetActorNameString(), 
					*ActorDesc->GetActorPackage().ToString(),
		...
		for (const FString& InternalAsset : InternalAssets)
		{
			...
			if (!ActorDesc.IsValid())
			{
				UE_LOGF(LogWorldPartition, Warning, "Invalid actor descriptor for actor '%ls' from package '%ls'", *ActorDescInitParams.PathName.ToString(), *InitParams.PackageName.ToString());
			} 
			else if (!ActorDesc->GetNativeClass().IsValid())
			{
				UE_LOGF(LogWorldPartition, Warning, "Invalid actor native class: Actor: '%ls' (guid '%ls') from package '%ls'",
...
void UActorDescContainer::ReassignActorDesc(TUniquePtr<FWorldPartitionActorDesc>& DescToUpdate, TFunctionRef<void(TUniquePtr<FWorldPartitionActorDesc>& ActorDesc)> UpdateAction)
{
	// 1 - Prepare users of the actor desc
	OnActorDescPreUpdate(DescToUpdate.Get());
	// 2 - Apply the transform, which is a function that depends on the context of the change, and which may have created a new ActorDesc and destroyed the old one, updating the passed-in unique ptr
	UpdateAction(DescToUpdate);
	// 3 - Update the pointer when needed, on all container instances, before proceeding with computing actor hashes, etc. because this may involve using other containers
	OnActorDescUpdating(DescToUpdate.Get());
	// 4 - Carry on processing the new actor desc
	OnActorDescUpdated(DescToUpdate.Get());
}

void UActorDescContainer::UpdateActorDesc(FWorldPartitionActorDesc* ActorDesc, TFunctionRef<void(FWorldPartitionActorDesc* ActorDesc)> UpdateAction)
{
	// 1 - Prepare users of the actor desc
	OnActorDescPreUpdate(ActorDesc);
	// 2 - Apply the transform, which is a function that depends on the context of the change, and which may have created a new ActorDesc and destroyed the old one, updating the passed-in unique ptr
	UpdateAction(ActorDesc);
	// 3 - Carry on processing the modified actor desc
	OnActorDescUpdated(ActorDesc);
}
...
void UActorDescContainer::RegisterActorDescriptor(FWorldPartitionActorDesc* InActorDesc)
{
	...
	UE_LOGF(LogWorldPartition, Verbose, "\tRegistered actor descriptor '%ls'", *InActorDesc->ToString(FWorldPartitionActorDesc::EToStringMode::Verbose));
}
...
void UActorDescContainer::UnregisterActorDescriptor(FWorldPartitionActorDesc* InActorDesc)
{
	...
	if (!ActorsByName.Remove(InActorDesc->GetActorName()))
	{
		...
		for (auto& [ActorName, ActorDesc] : ActorsByName)
		{
			...
			{
				UE_LOGF(LogWorldPartition, Log, "Removed actor '%ls' from container '%ls' with unexpected name `%ls`", *InActorDesc->GetActorNameString(), *ContainerPackageName.ToString(), *ActorName.ToString());
		...
		UE_CLOGF(!bRemoved, LogWorldPartition, Log, "Missing actor '%ls' from container '%ls'", *InActorDesc->GetActorNameString(), *ContainerPackageName.ToString());
	}
}
...
{
	FStringView ActorName;
	FStringView ActorContext;
	if (!UE::String::SplitLastChar(FStringView(ActorPath), TEXT('.'), ActorContext, ActorName, ESearchCase::CaseSensitive))
	{
		ActorName = ActorPath;
	}
		...
		if (const AActor* Actor = Cast<AActor>(Object))
		{
			if (ShouldHandleActorEvent(Actor))
			{
				...
				if (TUniquePtr<FWorldPartitionActorDesc>* ExistingActorDesc = GetActorDescriptor(Actor->GetActorGuid()))
				{
					...
					ReassignActorDesc(*ExistingActorDesc,
						[Actor](TUniquePtr<FWorldPartitionActorDesc>& ActorDesc)
						{
							// This changes the actor desc pointer
							FWorldPartitionActorDescUtils::UpdateActorDescriptorFromActor(Actor, ActorDesc);
						}
					);
...
void UActorDescContainer::OnObjectsReplaced(const TMap<UObject*, UObject*>& InOldToNewObjectMap)
{
	OnObjectsReplacedEvent.Broadcast(InOldToNewObjectMap);
}
...
void UActorDescContainer::OnEditorActorReplaced(AActor* InOldActor, AActor* InNewActor)
{
	OnEditorActorReplacedEvent.Broadcast(InOldActor, InNewActor);
}
	...
	if (ActorPackages.Num())
	{
		...
		for (const FAssetData& Asset : Assets)
		{
			...
			if (NewActorDesc.IsValid() && NewActorDesc->GetNativeClass().IsValid())
			{
				...
				{
					ReassignActorDesc(*ExistingActorDesc,
						[&NewActorDesc](TUniquePtr<FWorldPartitionActorDesc>& ActorDesc)
						{
							// This changes the actor desc pointer
							FWorldPartitionActorDescUtils::UpdateActorDescriptorFromActorDescriptor(NewActorDesc, ActorDesc);
						}
					);
				}
			}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UActorDescContainer::RegisterEditorDelegates);
	...
	FEditorDelegates::OnEditorActorReplaced.AddUObject(this, &UActorDescContainer::OnEditorActorReplaced);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UActorDescContainer::UnregisterEditorDelegates);
...
void UActorDescContainer::OnActorDescPreUpdate(FWorldPartitionActorDesc* ActorDesc)
{
	// Pass to prepare for change to the ActorDesc
	OnActorDescPreUpdateEvent.Broadcast(ActorDesc);
}

void UActorDescContainer::OnActorDescUpdating(FWorldPartitionActorDesc* ActorDesc)
{
	// Pass to update all pointers, because the previous pointer is now deleted
...
{
	// Pass to process the new ActorDesc, which may include using other containers which must also point to the updated ActorDesc
...
void UActorDescContainer::ForEachChildActorDesc(const FGuid& InActorGuid, TFunctionRef<void(const FGuid&)> InFunc)
{
	TSet<FGuid> CycleDetector;

	TFunction<void(const FGuid&)> ForEachChildActorDescInternal = [&ForEachChildActorDescInternal, Func = InFunc, &CycleDetector, this](const FGuid& ActorGuid)
	{
		bool bAlreadyVisited = false;
		CycleDetector.Add(ActorGuid, &bAlreadyVisited);
		if (bAlreadyVisited)
		{
		...
		if (const TSet<FGuid>* Children = ParentActorToChildrenMap.Find(ActorGuid))
		{
			for (const FGuid& ChildGuid : *Children)
			{
				if (ensure(ChildGuid.IsValid()))
				{
					Func(ChildGuid);
					ForEachChildActorDescInternal(ChildGuid);
				}
			}
		}
	};

	ForEachChildActorDescInternal(InActorGuid);
}

void UActorDescContainer::PropagateActorToWorldUpdate(FWorldPartitionActorDesc* ActorDesc)
{
	ForEachChildActorDesc(ActorDesc->GetGuid(), [this](const FGuid& ChildActorGuid)
	{
		if (FWorldPartitionActorDesc* ChildActor = GetActorDesc(ChildActorGuid))
		{
			UpdateActorDesc(ChildActor,
				[](FWorldPartitionActorDesc* ActorDesc)
				{
					ActorDesc->UpdateActorToWorld();
				}
			);
		}
	});
