...
static void SavePackage(UPackage* Package, ISourceControlHelper* SourceControlHelper)
{
	...
	else
	{
		...
		{
			UE_LOGF(LogWorldPartitionRuntimeSpatialHashHLOD, Error, "Error saving package %ls.", *Package->GetName());
			check(0);
		}
	}
}
...
static TArray<FGuid> GenerateHLODActorsForGrid(UWorldPartition* WorldPartition, const IStreamingGenerationContext* StreamingGenerationContext, const UWorldPartition::FSetupHLODActorsParams& Params, const FSpatialHashRuntimeGrid& RuntimeGrid, uint32 HLODLevel, FHLODCreationContext& Context, ISourceControlHelper* SourceControlHelper, const TArray<const IStreamingGenerationContext::FActorSetInstance*>& ActorSetInstances, TArray<FName>& NewActors, TArray<FName>& DirtyActors, const FSpatialHashSettings& Settings)
{
	...
	UE_LOGF(LogWorldPartitionRuntimeSpatialHashHLOD, Verbose, "%ls %ls...", *LOCTEXT("BuildingHLODsForGrid", "Building HLODs for grid").ToString(), *RuntimeGrid.GridName.ToString());
	...
	PartitionedActors.ForEachCells([&](const FSquare2DGridHelper::FGridLevel::FGridCell& GridCell)
	{
		...
		for (const FSquare2DGridHelper::FGridLevel::FGridCellDataChunk& GridCellDataChunk : GridCell.GetDataChunks())
		{
			...
			if (bShouldGenerateHLODActors)
			{
				...
				UE_LOGF(LogWorldPartitionRuntimeSpatialHashHLOD, Verbose, "%ls %ls -> %ls...", *CellName, *LOCTEXT("CreatingHLODCell", "Creating HLOD for cell").ToString() , *CellCoord.ToString());

				TRACE_CPUPROFILER_EVENT_SCOPE_TEXT(*CellCoord.ToString());

				UE_LOGF(LogWorldPartitionRuntimeSpatialHashHLOD, Display, "[%d / %d] %ls %ls...", (int32)SlowTask.CompletedWork + 1, (int32)SlowTask.TotalAmountOfWork, *LOCTEXT("ProcessingCell", "Processing cell").ToString(), *CellName);
				...
				CreationParams.Filters = Params.Filters;
...
static const FName HLODGridActorInUseTag = "HLODGridActorInUse";
...
bool UWorldPartitionRuntimeSpatialHash::SetupHLODActors(const IStreamingGenerationContext* StreamingGenerationContext, const UWorldPartition::FSetupHLODActorsParams& Params) const
{
	...
	if (!Grids.Num())
	{
		UE_LOGF(LogWorldPartitionRuntimeSpatialHashHLOD, Error, "Invalid partition grids setup");
		return false;
	}
	...
	StreamingGenerationContext->ForEachActorSetInstance([&GridsMapping, &GridActorSetInstances](const IStreamingGenerationContext::FActorSetInstance& ActorSetInstance)
	{
		...
		if (!FoundIndex)
		{
			...
			UE_LOGF(LogWorldPartitionRuntimeSpatialHashHLOD, Log, "Invalid partition grid '%ls' referenced by actor cluster", *ActorSetInstance.RuntimeGrid.ToString());
		}
	...
	{
		UE_LOGF(LogWorldPartitionRuntimeSpatialHashHLOD, Display, "\t%ls actors (%d):", StatType, Actors.Num());
		for (FName ActorName : Actors)
		{
			UE_LOGF(LogWorldPartitionRuntimeSpatialHashHLOD, Display, "\t\t%ls:", *ActorName.ToString());
		}
	};

	UE_LOGF(LogWorldPartitionRuntimeSpatialHashHLOD, Display, "SetupHLODActors stats:");
		...
		for (FActorDescContainerInstanceCollection::TConstIterator<ASpatialHashRuntimeGridInfo> Iterator(InWorldPartition); Iterator; ++Iterator)
		{
			if (ASpatialHashRuntimeGridInfo* SpatialHashRuntimeGridInfo = Cast<ASpatialHashRuntimeGridInfo>(Iterator->GetActor()))
			{
				if (!SpatialHashRuntimeGridInfo->ActorHasTag(HLODGridActorInUseTag))
				{
					...
					if (bDeleted)
					{
						UE_LOGF(LogWorldPartitionRuntimeSpatialHashHLOD, Display, "Deleting %ls...", *PackageName);
					}
					else
					{
						UE_LOGF(LogWorldPartitionRuntimeSpatialHashHLOD, Error, "Failed to delete %ls", *PackageName);
					}
				}
			}
		}
