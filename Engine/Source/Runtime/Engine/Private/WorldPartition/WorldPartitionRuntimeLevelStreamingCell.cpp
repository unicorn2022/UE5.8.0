...
#if WITH_EDITOR
FName UWorldPartitionRuntimeLevelStreamingCell::GetCookPackageName() const
{
	if (UWorld* World = GetOwningWorld())
	{
		const FString CookPackageName = FString::Printf(TEXT("%s/%s/%s"), *UWorld::RemovePIEPrefix(World->GetPackage()->GetName()), FPackageName::GetGeneratedPackageSubPath(), *GetName());
		return FName(CookPackageName);
	}
	return NAME_None;
}
...
bool UWorldPartitionRuntimeLevelStreamingCell::CreateAndSetLevelStreaming(const TSoftObjectPtr<UWorld>& InWorldAsset, const FTransform& InInstanceTransform) const
{
	...
	{
		UE_LOGF(LogWorldPartition, Warning, "UWorldPartitionRuntimeLevelStreamingCell::CreateAndSetLevelStreaming can't create an already existing UWorldPartitionLevelStreamingDynamic object named %ls", *LevelStreamingName.ToString());
		return false;
	}
...
bool UWorldPartitionRuntimeLevelStreamingCell::IsVisible() const
{
	if (LevelStreaming)
	{
		// Use GetLevelStreamingState() which is protected by CurrentStateLock,
		// unlike GetLevelStreamingStatus() which reads LoadedLevel without synchronization.
		return LevelStreaming->GetLevelStreamingState() == ELevelStreamingState::LoadedVisible;
	}
	return false;
}
...
bool UWorldPartitionRuntimeLevelStreamingCell::OnPrepareGeneratorPackageForCook(TArray<UPackage*>& OutModifiedPackages)
{
	...
	if (GetActorCount() > 0)
	{
		...
		FWorldPartitionLevelHelper::FLoadActorsParams Params = FWorldPartitionLevelHelper::FLoadActorsParams()
			...
			.SetLoadAsync(true, &AsyncRequestIDs)
		...
		while (!AsyncRequestIDs.IsEmpty())
		{
			TArray<int32> ToFlush = MoveTemp(AsyncRequestIDs);
			FlushAsyncLoading(ToFlush);
		}
...
UWorldPartitionLevelStreamingDynamic* UWorldPartitionRuntimeLevelStreamingCell::GetOrCreateLevelStreaming() const
{
	...
	if (LevelStreaming)
	{
		LevelStreaming->OnLevelShown.AddUniqueDynamic(const_cast<UWorldPartitionRuntimeLevelStreamingCell*>(this), &UWorldPartitionRuntimeLevelStreamingCell::OnLevelShown);
		LevelStreaming->OnLevelHidden.AddUniqueDynamic(const_cast<UWorldPartitionRuntimeLevelStreamingCell*>(this), &UWorldPartitionRuntimeLevelStreamingCell::OnLevelHidden);
	}
