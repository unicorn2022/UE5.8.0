...
FAutoConsoleVariableRef UWorldPartition::CVarUseMakingVisibleTransactionRequests(
	...
	TEXT("Changing the value while the game is considered experimental."),
	FConsoleVariableDelegate::CreateStatic(&*WorldPartition::OnVisibilityTransactionCVarChanged),
	WorldPartition::ECVF_Runtime_ReadOnly);
...
TMap<FName, FString> GetDataLayersDumpString(const UWorldPartition* WorldPartition)
{
	...
	{
		DataLayersDumpString.FindOrAdd(DataLayerInstance->GetFName()) = FString::Format(TEXT("{0}{1})"), { DataLayerInstance->GetDataLayerShortName(), DataLayerInstance->GetFName().ToString() });
		return true;
	});
				...
				if (!World->IsGameWorld())
				{
					if (UWorldPartition* WorldPartition = World->GetWorldPartition())
					{
						...
						for (const FString& ActorPath : ActorPaths)
						{
							...
							{
								UE_LOGF(LogWorldPartition, Log, "%ls", *GetActorDescDumpString(ActorDescInstance, DataLayersDumpString));
							}
						}
					}
				}
...
UWorldPartition::FWorldPartitionEditorGameWorldEvent UWorldPartition::OnPrepareEditorGameWorldForPIE;
UWorldPartition::FWorldPartitionEditorGameWorldEvent UWorldPartition::OnShutdownEditorGameWorldForPIE;
...
void UWorldPartition::PrepareEditorGameWorld()
{
	...
	if (bIsPIE)
	{
		OnPrepareEditorGameWorldForPIE.Broadcast(this);
	}
...
void UWorldPartition::ShutdownEditorGameWorld()
{
	...
	if (bIsPIE)
	{
		...
		RuntimeHash->ShutdownEditorGameWorld();
		OnShutdownEditorGameWorldForPIE.Broadcast(this);
		bIsPIE = false;
	}
}
...
void UWorldPartition::PostEditChangeChainProperty(FPropertyChangedChainEvent& PropertyChangedEvent)
{
	...
	static FName NAME_RuntimeCellsTransformerStack("RuntimeCellsTransformerStack");
	...
	if (PropertyName == GET_MEMBER_NAME_CHECKED(FRuntimeCellTransformerInstance, Class))
	{
		...
		TransformerInstance.Instance = TransformerInstance.Class ? NewObject<UWorldPartitionRuntimeCellTransformer>(this, TransformerInstance.Class, NAME_None, RF_Transactional) : nullptr;
	...
	// Recreate EditorHash if it was externally nulled (e.g. by Valkyrie "Reset Property to Default")
	if (!EditorHash)
	{
		if (UWorld* OuterWorld = GetWorld())
		{
			...
			virtual FArchive& operator<<(UObject*& InObj) override
			{
				if (InObj && (InObj != Root) && !InObj->IsTemplate())
				{
					if (AActor* Actor = Cast<AActor>(InObj))
					{
						...
						{
							InObj = nullptr;
							UE_LOGF(LogWorldPartition, Log, "Cleared actor editor-only reference '%ls' -> '%ls'.", *Root->GetActorNameOrLabel(), *Actor->GetActorNameOrLabel());							
						}
					}
...
void UWorldPartition::Initialize(UWorld* InWorld, const FTransform& InTransform)
{
	...
	if (bIsEditor || bIsGame || bIsPIEWorldTravel || bIsDedicatedServer)
	{
		...
		ContainerInitParams.bShouldGatherInternalActors = false;
...
void UWorldPartition::Uninitialize()
{
	...
	if (IsInitialized())
	{
		...
		UE_LOGF(LogWorldPartition, Log, "UWorldPartition::Uninitialize : World = %ls", *GetTypedOuter<UWorld>()->GetPathName());
...
UWorldPartition* UWorldPartition::CreateOrRepairWorldPartition(AWorldSettings* WorldSettings, TSubclassOf<UWorldPartitionEditorHash> EditorHashClass, TSubclassOf<UWorldPartitionRuntimeHash> RuntimeHashClass)
{
	...
	if (!WorldPartition)
	{
		WorldPartition = NewObject<UWorldPartition>(WorldSettings, NAME_None, RF_Transactional);
...
void UWorldPartition::InvalidateVisiblityTransactionRequestCachedVariables()
{
	bCachedUseMakingVisibleTransactionRequests.Reset();
	bCachedUseMakingInvisibleTransactionRequests.Reset();
}
...
void UWorldPartition::Tick(float DeltaSeconds)
{
	...
	if (RuntimeCellsTransformerStackTimes.Num())
	{
		...
		if (RuntimeCellsTransformerStackDumpTime > 10.0f)
		{
			...
			UE_LOGF(LogWorldPartition, Log, "Runtime cells transformer stack per-cell stats:");
			for (TPair<UClass*, TPair<double, int32>>& StatsPair : RuntimeCellsTransformerStackTimes)
			{
				UE_LOGF(LogWorldPartition, Log, "\t%ls: %ls", *StatsPair.Key->GetName(), *FPlatformTime::PrettyTime(StatsPair.Value.Key / (double)StatsPair.Value.Value));
			}
...
{
	UE_CLOGF(!bStreamingInEnabled, LogWorldPartition, Warning, "UWorldPartition::DisableStreamingIn called while streaming was already disabled.");
	bStreamingInEnabled = false;
}

void UWorldPartition::EnableStreamingIn()
{
	UE_CLOGF(bStreamingInEnabled, LogWorldPartition, Warning, "UWorldPartition::EnableStreamingIn called while streaming was already enabled.");
	bStreamingInEnabled = true;
}
		...
		TFunction<void(const UActorDescContainerInstance*, uint32)> DumpContainerInstanceActors = [&ActorDescInstances, &DumpContainerInstanceActors](const UActorDescContainerInstance* ContainerInstance, uint32 Depth)
		{
			...
			for (const FWorldPartitionActorDescInstance* ActorDescInstance : SortedActorDescInstances)
			{
				...
				if (ActorDescInstance->IsChildContainerInstance())
				{
					...
					{
						UE_LOGF(LogWorldPartition, Log, "%ls%ls=%d", FCString::Tab(Depth), *ContainerInstanceDesc.ContainerInstance->GetContainerPackage().ToString(), ContainerInstanceDesc.ContainerInstance->GetActorsByGuid().Num());
...
void UWorldPartition::AppendAssetRegistryTags(FAssetRegistryTagsContext Context) const
{
	...
	if (WorldExternalActorReferences.Num())
	{
		...
		Context.AddTag(FAssetRegistryTag(NAME_WorldExternalActorsReferences, FString(StringBuilder), FAssetRegistryTag::TT_Hidden));
...
UActorDescContainerInstance* UWorldPartition::RegisterActorDescContainerInstance(const UActorDescContainerInstance::FInitializeParams& InParams)
{
	...
	{	
		...
		InitParams.bShouldGatherInternalActors = InParams.bShouldGatherInternalActors;
		...
		InitParams.FilterActorDescFunc = [this, &WorldDataLayerActorsDesc, &InParams](const FWorldPartitionActorDesc* ActorDesc)
		{
			...
			if (ActorDesc->GetActorNativeClass()->IsChildOf<AWorldDataLayers>())
			{
				...
				{
					UE_LOGF(LogWorldPartition, Warning, "Found WorldDataLayers actor '%ls' (%ls), but another WorldDataLayers actor '%ls' (%ls) already exists. Actor will be ignored, clean up the invalid actor to remove the warning.",
			...
			{
				UE_LOGF(LogWorldPartition, Warning, "Found existing actor descriptor guid `%ls`: Actor: '%ls' from package '%ls'",
...
bool UWorldPartition::ShouldStripRuntimeCellByTransformerStack(const UWorldPartitionRuntimeCell* InCell) const
{
	for (const FRuntimeCellTransformerInstance& TransformerInstance : RuntimeCellsTransformerStack)
	{
		if (TransformerInstance.Class && TransformerInstance.ShouldStripRuntimeCell(InCell))
		{
			UE_LOGF(LogWorldPartition, Log, "Stripping runtime cell '%ls' by transformer '%ls'",
				*InCell->GetDebugName(), *TransformerInstance.Class->GetName());
			return true;
		}
	}
	return false;
...
void UWorldPartition::ApplyRuntimeCellsTransformerStack(UWorldPartitionRuntimeCell* InCell)
{
	if (!RuntimeCellsTransformerStack.IsEmpty())
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(ApplyRuntimeCellsTransformerStack_RuntimeCell);
		...
		for (const FRuntimeCellTransformerInstance& TransformerInstance : RuntimeCellsTransformerStack)
		{
			if (TransformerInstance.Class)
			{
				...
				TransformerInstance.TransformRuntimeCell(InCell);
...
void UWorldPartition::ApplyRuntimeCellsTransformerStack(ULevel* InLevel)
{
	if (!RuntimeCellsTransformerStack.IsEmpty())
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(ApplyRuntimeCellsTransformerStack_Level);
...

		{
			...
			TFunction<UObject*(UObject*)> ResolveFinalReplacement = [&CombinedReplacementMap, &FinalReplacementMap, &CurrentlyResolving, &ResolveFinalReplacement](UObject* Object) -> UObject*
			{
				...
				if (bIsAlreadyInSet)
				{
					UE_LOGF(LogWorldPartition, Log, "Detected cycle in object remapping during ApplyRuntimeCellsTransformerStack at %ls, ignoring", *GetNameSafe(Object));
					return nullptr;
				}
