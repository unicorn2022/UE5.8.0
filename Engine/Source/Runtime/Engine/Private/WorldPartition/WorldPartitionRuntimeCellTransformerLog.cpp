...
void UWorldPartitionRuntimeCellTransformerLog::PreTransform(ULevel* InLevel)
{
	...
	{
		UE_LOGF(LogWorldPartition, Log, "Level %ls PreTransform Content:", *InLevel->GetPackage()->GetName());
...
void UWorldPartitionRuntimeCellTransformerLog::PostTransform(ULevel* InLevel)
{
	...
	if (bOnlyLogDifferences)
	{
		...
		{
			UE_LOGF(LogWorldPartition, Log, "Level %ls PostTransform Delta:", *InLevel->GetPackage()->GetName());
	...
	{
		UE_LOGF(LogWorldPartition, Log, "Level %ls PostTransform Content:", *InLevel->GetPackage()->GetName());
...
void UWorldPartitionRuntimeCellTransformerLog::DumpLevelContentStats(ULevel* InLevel, const TMap<UClass*, int32>& InClassNumInstances)
{
	...
	{
		UE_LOGF(LogWorldPartition, Log, "\tNum Actors: %d", InLevel->Actors.Num());
		UE_LOGF(LogWorldPartition, Log, "\tActor Class Breakdown:");
		for (auto& [Class, NumInstances] : InClassNumInstances)
		{
			if (NumInstances)
			{
				UE_LOGF(LogWorldPartition, Log, "\t\t%ls: %d", *Class->GetName(), NumInstances);
			}
		}
	}
}
