...
static FName NAME_ActorMetaDataClass("ActorMetaDataClass");
static FName NAME_ActorMetaData("ActorMetaData");
...
TUniquePtr<FWorldPartitionActorDesc> FWorldPartitionActorDescUtils::GetActorDescriptorFromInitParams(const FActorDescInitParams& InActorDescInitParams, FName InPackageName)
{
	...
	{
		UE_LOGF(LogWorldPartition, Warning, "Invalid class for actor guid `%ls` ('%ls') from package '%ls'", *NewActorDesc->GetGuid().ToString(), *NewActorDesc->GetActorName().ToString(), *NewActorDesc->GetActorPackage().ToString());
