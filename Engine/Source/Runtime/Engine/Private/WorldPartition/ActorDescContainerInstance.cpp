...
UActorDescContainerInstance::FActorDescInstanceAddedToContainerEvent UActorDescContainerInstance::OnActorDescInstanceAddedToContainer;
UActorDescContainerInstance::FActorDescInstanceRemovedFromContainerEvent UActorDescContainerInstance::OnActorDescInstanceRemovedFromContainer;
...
void UActorDescContainerInstance::RegisterContainer(const FInitializeParams& InParams)
{
	...
	ContainerInitParams.bShouldGatherInternalActors = InParams.bShouldGatherInternalActors;
...
void UActorDescContainerInstance::Initialize(const FInitializeParams& InParams)
{
	...
	for (FActorDescList::TIterator<> It(Container); It; ++It)
	{
		...
		if (bIsInstanced)
		{
			...
			if (LongActorPackageName != OuterWorldContainerPackageName)
			{
...
{
	// Ensure that a non-deferred context is currently being used, because otherwise the actor desc instance and container deletion resulting from this call will risk the context
	// use deleted memory. If this check fails, a fix would be to flush the current FWorldPartitionLoadingContext::IContext through a new static method, after having forced unregistrations below.
	check(!FWorldPartitionLoadingContext::IsDeferringRawObjects());
...
bool UActorDescContainerInstance::ShouldRegisterDelegates() const
{
	...
	return (!OwningWorld || !OwningWorld->IsGameWorld()) && !IsRunningCookCommandlet();
}

void UActorDescContainerInstance::RegisterDelegates()
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UActorDescContainerInstance::RegisterDelegates);
	...
	{
		Container->OnObjectsReplacedEvent.AddUObject(this, &UActorDescContainerInstance::OnObjectsReplaced);
		Container->OnEditorActorReplacedEvent.AddUObject(this, &UActorDescContainerInstance::OnEditorActorReplaced);
	}
	...
	Container->OnActorDescPreUpdateEvent.AddUObject(this, &UActorDescContainerInstance::OnActorDescPreUpdate);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UActorDescContainerInstance::UnregisterDelegates);

	check(Container);
	Container->OnObjectsReplacedEvent.RemoveAll(this);
	Container->OnEditorActorReplacedEvent.RemoveAll(this);
	Container->OnActorDescAddedEvent.RemoveAll(this);
	Container->OnActorDescRemovedEvent.RemoveAll(this);
	Container->OnActorDescPreUpdateEvent.RemoveAll(this);
...
FWorldPartitionActorDescInstance* UActorDescContainerInstance::AddActorDescInstance(FWorldPartitionActorDescInstance&& InActorDescInstance)
{
	...
	OnActorDescInstanceAddedToContainer.Broadcast(this, NewActorDescInstance);
...
void UActorDescContainerInstance::RemoveActorDescInstance(TUniquePtr<FWorldPartitionActorDescInstance>* InActorDescInstance)
{
	...
	OnActorDescInstanceRemovedFromContainer.Broadcast(this, ActorDescInstance);
	...
	// This actor desc might have bIsRegisteringOrUnregistering set to true, meaning it is currently stored in the Registrations array in a FDeferred loading context.
	// RemoveActorDescriptor would delete it without removing it from the array, leading to dangling pointer access.
	FWorldPartitionLoadingContext::CancelOperationIfAny(ActorDescInstance);
...
void UActorDescContainerInstance::OnActorDescPreUpdate(FWorldPartitionActorDesc* InActorDesc)
{
	...
	if (TUniquePtr<FWorldPartitionActorDescInstance>* ActorDescInstanceUniquePtr = GetActorDescInstancePtr(InActorDesc->GetGuid()))
	{
		check(ActorDescInstanceUniquePtr->IsValid());
		FWorldPartitionActorDescInstance* ActorDescInstance = ActorDescInstanceUniquePtr->Get();
		...
		if (UWorldPartition* WorldPartition = GetOuterWorldPartition())
		{
			WorldPartition->OnActorDescInstanceUpdating(ActorDescInstance);
		}

		ActorDescInstance->OnPreUpdate();

		OnActorDescInstanceUpdatingEvent.Broadcast(ActorDescInstance);
	}
}

void UActorDescContainerInstance::OnActorDescUpdating(FWorldPartitionActorDesc* InActorDesc)
{
	// See comment in UActorDescContainerInstance::OnActorDescPreUpdate
