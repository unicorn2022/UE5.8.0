...
TArray<ICookPackageSplitter::FGeneratedPackage> FWorldPartitionCookPackageSplitter::GetGenerateList(const UPackage* OwnerPackage)
{
	...
	UE_LOGF(LogWorldPartition, Display, "[Cook] Gathering packages to cook from generators for owner object %ls.", *GetFullNameSafe(World));
	...
	UE_CLOGF(!bIsSuccess, LogWorldPartition, Warning, "[Cook] Errors while gathering packages to took from generators for owner object %ls.", *GetFullNameSafe(World));

	UE_LOGF(LogWorldPartition, Display, "[Cook] Gathered %u packages to generate from %u Generators.", CookContext.NumPackageToGenerate(), CookContext.NumGenerators());

	TArray<ICookPackageSplitter::FGeneratedPackage> PackagesToGenerate;
	BuildPackagesToGenerateList(PackagesToGenerate);

	UE_LOGF(LogWorldPartition, Display, "[Cook] Sending %u packages to be generated.", PackagesToGenerate.Num());
	UE_LOGF(LogWorldPartition, Display, "[Cook] Debug(GetGenerateList) : OwnerWorld=%ls", *GetFullNameSafe(World));

	return PackagesToGenerate;
}

TOptional<bool> FWorldPartitionCookPackageSplitter::RequiresEmptyPackageBeforePopulate(const UPackage* Package)
{
	if (CookContext.IsGeneratingPackage(Package))
	{
		return true;
	}
...
bool FWorldPartitionCookPackageSplitter::PopulateGeneratedPackage(ICookPackageSplitter::FPopulateContext& PopulateContext)
{
	...
	UE_LOGF(LogWorldPartition, Verbose, "[Cook][PopulateGeneratedPackage] Processing %ls", *FWorldPartitionCookPackage::MakeGeneratedFullPath(GeneratedPackage.GeneratedRootPath, GeneratedPackage.RelativePath));
	...
	{
		UE_LOGF(LogWorldPartition, Error, "[Cook][PopulateGeneratedPackage] Could not find WorldPartitionCookPackage for %ls", *FWorldPartitionCookPackage::MakeGeneratedFullPath(GeneratedPackage.GeneratedRootPath, GeneratedPackage.RelativePath));
		bIsSuccess = false;
	}

	UE_LOGF(LogWorldPartition, Verbose, "[Cook][PopulateGeneratedPackage] Gathered %u modified packages for %ls", ModifiedPackages.Num() , *FWorldPartitionCookPackage::MakeGeneratedFullPath(GeneratedPackage.GeneratedRootPath, GeneratedPackage.RelativePath));
...
bool FWorldPartitionCookPackageSplitter::PopulateGeneratorPackage(ICookPackageSplitter::FPopulateContext& PopulateContext)
{
	...
	UE_LOGF(LogWorldPartition, Display, "[Cook][PopulateGeneratorPackage] Processing %u packages", GeneratedPackages.Num());
	...
	UE_LOGF(LogWorldPartition, Display, "[Cook][PopulateGeneratorPackage] Gathered %u modified packages", ModifiedPackages.Num());
	PopulateContext.ReportKeepReferencedPackages(ModifiedPackages);

	return bIsSuccess;
}

TOptional<bool> FWorldPartitionCookPackageSplitter::SkipSaveGeneratorPackage(const UPackage* OwnerPackage)
{
	// If we have EDL cook filters, we are trying to cook the EDLs in isolation. Do not save the map even though it was loaded.
	if (!UWorldPartitionSettings::ShouldCookEDLGenerator(OwnerPackage))
	{
		return true;
	}
	return TOptional<bool>();
...
{
	UE_LOGF(LogWorldPartition, Display, "[Cook] Debug(TearDown): OwnerWorld=%ls", *GetFullNameSafe(World));
	...
	for (const IWorldPartitionCookPackageGenerator* CookPackageGenerator : CookContext.GetCookPackageGenerators())
	{
		if (const TArray<FWorldPartitionCookPackage*>* CookPackages = CookContext.GetCookPackages(CookPackageGenerator))
		{
			...
			for (const FWorldPartitionCookPackage* CookPackage : *CookPackages)
			{
				...
				CookContext.RegisterGeneratedPackage(CookPackage->GetFullGeneratedPath());
				UE_LOGF(LogWorldPartition, Log, "Adding Cell %ls with GenerationHash: %ls to packages to generate", *CookPackage->GetFullGeneratedPath(), *LexToString(CookPackage->GenerationHash))
