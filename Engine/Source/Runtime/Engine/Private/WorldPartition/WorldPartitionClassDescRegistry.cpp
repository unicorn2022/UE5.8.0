...
{
	check(IsInitialized());
	UE::TScopeLock Lock(ClassDescLock);
	PrefetchClassDescsInternal(InClassPaths);
}

void FWorldPartitionClassDescRegistry::PrefetchClassDescsInternal(const TArray<FTopLevelAssetPath>& InClassPaths)
{
	...
	Algo::Transform(InClassPaths, ClassPaths, [this](const FTopLevelAssetPath& ClassPath) { return RedirectClassPathInternal(ClassPath); });
	...
	Algo::ForEach(Assets, [this, &AssetRegistry, &ParentClassPathsMap, &SortBlueprintAssetsByNameLength, &GetBlueprintAssets](FAssetData& AssetData)
	{
		...
		while (AssetData.IsRedirector())
		{
			...
			{
				UE_LOGF(LogWorldPartition, Warning, "Failed to follow class redirector for '%ls'", *AssetData.ToSoftObjectPath().ToString());
			...
			{
				UE_LOGF(LogWorldPartition, Warning, "Failed to find redirected assets for '%ls' from '%ls'", *AssetData.ToSoftObjectPath().ToString(), *AssetClassPath.ToString());
				AssetData = FAssetData();
				break;
			}
			...
			if (bRedirectAlreadyRegistered)
			{
				...
				UE_LOGF(LogWorldPartition, Warning, "Redirector loop detected for '%ls' from '%ls':\n%ls", *AssetData.ToSoftObjectPath().ToString(), *AssetClassPath.ToString(), *ClassRedirectsLoop);
				AssetData = FAssetData();
				break;
			}
	...
	if (!ParentClassPathsMap.IsEmpty())
	{
		...
		PrefetchClassDescsInternal(ParentClassPaths);

		for (auto& [ClassPath, ParentClassPath] : ParentClassPathsMap)
		{
			ParentClassMap.Add(ClassPath, RedirectClassPathInternal(ParentClassPath));
		}
	}
...
bool FWorldPartitionClassDescRegistry::IsRegisteredClass(const FTopLevelAssetPath& InClassPath) const
{
	check(IsInitialized());
	UE::TScopeLock Lock(ClassDescLock);
	const FTopLevelAssetPath ClassPath = RedirectClassPathInternal(InClassPath);
...
const FWorldPartitionActorDesc* FWorldPartitionClassDescRegistry::GetClassDescDefaultInternal(const FTopLevelAssetPath& InClassPath) const
{
...
const FWorldPartitionActorDesc* FWorldPartitionClassDescRegistry::GetClassDescDefault(const FTopLevelAssetPath& InClassPath) const
{
	check(IsInitialized());
	UE::TScopeLock Lock(ClassDescLock);
	return GetClassDescDefaultInternal(InClassPath);
}
...
const FWorldPartitionActorDesc* FWorldPartitionClassDescRegistry::GetClassDescDefaultForActor(const FTopLevelAssetPath& InClassPath) const
{
	check(IsInitialized());
	UE::TScopeLock Lock(ClassDescLock);
	return GetClassDescDefaultInternal(RedirectClassPathInternal(InClassPath));
}
...
const FWorldPartitionActorDesc* FWorldPartitionClassDescRegistry::GetClassDescDefaultForClass(const FTopLevelAssetPath& InClassPath) const
{
	check(IsInitialized());
	UE::TScopeLock Lock(ClassDescLock);
	const FTopLevelAssetPath ParentClassPath = ParentClassMap.FindRef(RedirectClassPathInternal(InClassPath));
	return GetClassDescDefaultInternal(ParentClassPath);
}
...
void FWorldPartitionClassDescRegistry::PrefetchClassDesc(UClass* InClass)
{
	for (UClass* ParentClass = InClass; !ParentClass->IsNative(); ParentClass = ParentClass->GetSuperClass())
	{
		...
		if (!ClassByPath.Contains(ParentClassPath))
		{
			...
			{
				PrefetchClassDescsInternal({ ParentClassPath });
			}
...
{
	const FWorldPartitionActorDesc* UpdatedClassDesc = nullptr;
			...
			if (Cast<UPackage>(Blueprint->GetOuter()))
			{
				...
				if (UBlueprintGeneratedClass* BlueprintGeneratedClass = Cast<UBlueprintGeneratedClass>(Blueprint->GeneratedClass))
				{
					...
					if (BlueprintGeneratedClass->IsChildOf<AActor>())
					{
						...
						UpdatedClassDesc = UpdateClassDescriptor(Blueprint, false);
					}
				}
			}
		}
	...
	if (UpdatedClassDesc)
	{
		ClassDescriptorUpdatedEvent.Broadcast(UpdatedClassDesc);
	}
}
		...
		if (!InPropertyChangedEvent.Property)
		{
			if (UBlueprint* Blueprint = Cast<UBlueprint>(InObject))
			{
				...
				if (Blueprint->IsUpToDate() && Blueprint->GeneratedClass && Blueprint->GeneratedClass->IsChildOf<AActor>())
				{
					...
					UpdatedClassDesc = UpdateClassDescriptor(InObject, true);
				}
			}
		}
	}
...
const FWorldPartitionActorDesc* FWorldPartitionClassDescRegistry::UpdateClassDescriptor(UObject* InObject, bool bOnlyIfExists)
{
	check(IsInitialized());
	...
	if (TUniquePtr<FWorldPartitionActorDesc>** ExistingClassDesc = ClassByPath.Find(ClassPath))
	{
		...
		else
		{
			...
			// Return the updated descriptor so the caller can broadcast after releasing the lock
			return (*ExistingClassDesc)->Get();
		}
	}
...
FTopLevelAssetPath FWorldPartitionClassDescRegistry::RedirectClassPathInternal(const FTopLevelAssetPath& InClassPath) const
{
...
{
	UE::TScopeLock Lock(ClassDescLock);
	return RedirectClassPathInternal(InClassPath);
}
#endif
