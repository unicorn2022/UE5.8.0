...
static_assert(std::is_same<std::underlying_type<EDataLayerRuntimeState>::type, uint8>::value);
static_assert(std::is_same<std::underlying_type<EWorldPartitionRuntimeCellState>::type, uint8>::value);

static_assert(static_cast<uint8>(EWorldPartitionRuntimeCellState::Unloaded) == static_cast<uint8>(EDataLayerRuntimeState::Unloaded));
static_assert(static_cast<uint8>(EWorldPartitionRuntimeCellState::Loaded) == static_cast<uint8>(EDataLayerRuntimeState::Loaded));
static_assert(static_cast<uint8>(EWorldPartitionRuntimeCellState::Activated) == static_cast<uint8>(EDataLayerRuntimeState::Activated));
...
void UWorldPartitionRuntimeCell::SetDataLayers(const TArray<const UDataLayerInstance*>& InDataLayerInstances)
{
	...
	Algo::Transform(SortedDataLayerInstances, SortedDataLayerInstanceNames, [](const UDataLayerInstance* DataLayerInstance) { return DataLayerInstance->GetFName(); });
	if (SortedDataLayerInstances.Num() > 0)
	{
...
void UWorldPartitionRuntimeCell::DumpStateLog(FHierarchicalLogArchive& Ar) const
{
	...
	if (TArray<const UDataLayerInstance*> DataLayerInstances = GetDataLayerInstances(); DataLayerInstances.Num() > 0)
	{
		...
		{
			Ar.Printf(TEXT("%s"), *DataLayerInstance->GetDataLayerFullName());
		}
	}
...
EDataLayerRuntimeState UWorldPartitionRuntimeCell::GetCellEffectiveWantedState(const FWorldPartitionStreamingContext& Context) const
{
	if (!HasDataLayers())
	{
		UE::TWriteScopeLock WriteLock(EffectiveWantedStateLock);
		return EffectiveWantedState = EDataLayerRuntimeState::Activated;
	}
	...
	// Epoch mismatch - resolve outside lock, then cache under write lock
	const EDataLayerRuntimeState ResolvedState = Context.ResolveDataLayerRuntimeState(DataLayers);
	UE::TWriteScopeLock WriteLock(EffectiveWantedStateLock);
	EffectiveWantedState = ResolvedState;
...
{
	return GetDataLayersInline().Contains(DataLayerInstance->GetFName());
}
