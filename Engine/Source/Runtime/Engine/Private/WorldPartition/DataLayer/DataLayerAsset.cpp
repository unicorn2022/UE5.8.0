...
FText UDataLayerAsset::GetDisplayNameText() const
{
	if (IsPrivate())
	{
		if (const UDataLayerInstance* DataLayerInstance = GetTypedOuter<UDataLayerInstance>())
		{
...
void UDataLayerAsset::SetLoadFilter(EDataLayerLoadFilter InLoadFilter)
{
	check(DataLayerType == EDataLayerType::Runtime || !IsPrivate());
	if (CanEditDataLayerType())
	{
		LoadFilter = InLoadFilter;
	}
}
