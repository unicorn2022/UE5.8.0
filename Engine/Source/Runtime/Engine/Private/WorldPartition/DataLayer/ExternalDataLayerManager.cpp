...
bool UExternalDataLayerManager::RegisterExternalStreamingObjectForGameWorld(const UExternalDataLayerAsset* InExternalDataLayerAsset)
{
	...
	{
		UE_LOGF(LogWorldPartition, Warning, "[EDL: %ls] External streaming object already registered.", *InExternalDataLayerAsset->GetName());
		return false;
	}
	...
	{
		UE_LOGF(LogWorldPartition, Error, "[EDL: %ls] No external streaming object found.", *InExternalDataLayerAsset->GetName());
		return false;
	}
	...
	{
		UE_LOGF(LogWorldPartition, Error, "[EDL: %ls] No external streaming object found in package %ls.", *InExternalDataLayerAsset->GetName(), *ExternalStreamingObjectPackagePath);
		return false;
	}
...
bool UExternalDataLayerManager::UnregisterExternalStreamingObjectForGameWorld(const UExternalDataLayerAsset* InExternalDataLayerAsset)
{
	...
	if (!ExternalStreamingObjects.Contains(InExternalDataLayerAsset))
	{
		UE_LOGF(LogWorldPartition, Error, "[EDL: %ls] External streaming object not registered.", *InExternalDataLayerAsset->GetName());
		return false;
	}
...
bool UExternalDataLayerManager::InjectExternalDataLayer(const UExternalDataLayerAsset* InExternalDataLayerAsset)
{
	...
	{
		UE_LOGF(LogWorldPartition, Error, "[EDL %ls] External Data Layer already added.", *InExternalDataLayerAsset->GetName());
		return false;
	}
...
bool UExternalDataLayerManager::InjectIntoGameWorld(const UExternalDataLayerAsset* InExternalDataLayerAsset)
{
	...
	if (!ExternalStreamingObject)
	{
		UE_LOGF(LogWorldPartition, Log, "[EDL: %ls] No external streaming object found. No content will be injected.", *InExternalDataLayerAsset->GetName());
		return false;
	}
	...
	if (!WorldDataLayers)
	{
		UE_LOGF(LogWorldPartition, Error, "[EDL: %ls] Injection failed : No WorldDataLayers found for World %ls.", *InExternalDataLayerAsset->GetName(), *InjectedWorld->GetName());
		return false;
	}
	...
	{
		UE_LOGF(LogWorldPartition, Error, "[EDL: %ls] Injection failed : Adding External Data Layer failed for World %ls.", *InExternalDataLayerAsset->GetName(), *InjectedWorld->GetName());
		return false;
	}
	...
	if (!WorldPartition->InjectExternalStreamingObject(ExternalStreamingObject))
	{
		...
		UE_LOGF(LogWorldPartition, Error, "[EDL: %ls] Injection failed : Injecting external streaming object failed for World %ls.", *InExternalDataLayerAsset->GetName(), *InjectedWorld->GetName());
		return false;
	}

	UE_LOGF(LogWorldPartition, Log, "[EDL: %ls] Injection succeeded for World %ls.", *InExternalDataLayerAsset->GetName(), *InjectedWorld->GetName());
	return true;
}
...
bool UExternalDataLayerManager::RemoveFromGameWorld(const UExternalDataLayerAsset* InExternalDataLayerAsset)
{
	...
	{
		UE_LOGF(LogWorldPartition, Error, "[EDL: %ls] Failed to find External Data Layer Instance.", *InExternalDataLayerAsset->GetName());
		return false;
	}
...
#endif
		UE_LOGF(LogWorldPartition, Error, "[EDL: %ls] Failed to find external streaming object.", *InExternalDataLayerAsset->GetName());
		bSuccess = false;
	}
	else
	{
		...
		{
			UE_LOGF(LogWorldPartition, Error, "[EDL: %ls] Failed to remove external streaming object from World %ls.", *InExternalDataLayerAsset->GetName(), *InjectedWorld->GetName());
			bSuccess = false;
		}
		...
		{
			UE_LOGF(LogWorldPartition, Error, "[EDL: %ls] Failed to remove External Data Layer from World %ls.", *InExternalDataLayerAsset->GetName(), *InjectedWorld->GetName());
			bSuccess = false;
		}

		UE_CLOGF(bSuccess, LogWorldPartition, Log, "[EDL: %ls] Successfully removed from World %ls.", *InExternalDataLayerAsset->GetName(), *InjectedWorld->GetName());
	}
	return bSuccess;
}
...
bool UExternalDataLayerManager::RemoveExternalDataLayer(const UExternalDataLayerAsset* InExternalDataLayerAsset)
{
	...
	if (GetTypedOuter<UWorld>()->IsGameWorld())
	{
		...
		UE_CLOGF(!bSuccess, LogWorldPartition, Error, "[EDL: %ls] Failed to find External Data Layer in the world.", *InExternalDataLayerAsset->GetName());
	}
#if WITH_EDITOR
	if (!GetTypedOuter<UWorld>()->IsGameWorld() || IsRunningGameOrInstancedWorldPartition())
	{
...
UActorDescContainerInstance* UExternalDataLayerManager::RegisterExternalDataLayerActorDescContainer(const UExternalDataLayerAsset* InExternalDataLayerAsset)
{
	...
	InitParams.FilterActorDescFunc = [this, InExternalDataLayerAsset, &ExternalDataLayerAssetSoftObjectPath](const FWorldPartitionActorDesc* ActorDesc)
	{
		...
		{
			UE_LOGF(LogWorldPartition, Warning, "[EDL %ls] Actor Desc %ls External Data Layer '%ls' doesn't match its container External Data Layer '%ls'.",
...
bool UExternalDataLayerManager::RegisterExternalDataLayerInstance(UExternalDataLayerInstance* InExternalDataLayerInstance)
{
	...
	if (!ExistingExternalDataLayerInstance)
	{
		...
		{
			UE_LOGF(LogWorldPartition, Error, "[EDL %ls] Failed to register External Data Layer Instance.", *ExternalDataLayerAsset->GetName());
			return false;
		}
	}
...
bool UExternalDataLayerManager::UnregisterExternalDataLayerActorDescContainer(const UExternalDataLayerAsset* InExternalDataLayerAsset)
{
	...
	if (ActorDescContainerInstance)
	{
		...
		{
			UE_LOGF(LogWorldPartition, Error, "[EDL: %ls] Failed to unregister the ActorDescContainer of this External Data Layer.", *InExternalDataLayerAsset->GetName());
			bSuccess = false;
		}
	}
	...
	if (UExternalDataLayerInstance* ExternalDataLayerInstance = GetExternalDataLayerInstance(InExternalDataLayerAsset))
	{
		...
		{
			UE_LOGF(LogWorldPartition, Error, "[EDL: %ls] Failed to remove External Data Layer.", *InExternalDataLayerAsset->GetName());
			bSuccess = false;
		}
	}
...
bool UExternalDataLayerManager::UnregisterExternalDataLayerInstance(UExternalDataLayerInstance* InExternalDataLayerInstance)
{
	...
	{
		UE_LOGF(LogWorldPartition, Error, "[EDL: %ls] Failed to unregister External Data Layer Instance.", *ExternalDataLayerAsset->GetName());
		return false;
	}
	return true;
}
...
URuntimeHashExternalStreamingObjectBase* UExternalDataLayerManager::CreateExternalStreamingObjectUsingStreamingGeneration(const UExternalDataLayerAsset* InExternalDataLayerAsset)
{
	...
	{
		DataLayerInstance->Rename(nullptr, ExternalStreamingObject, REN_DontCreateRedirectors | REN_DoNotDirty | REN_NonTransactional | REN_AllowPackageLinkerMismatch);
