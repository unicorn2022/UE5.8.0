...
bool UExternalDataLayerInstance::PerformAddActor(AActor* InActor) const
{
	...
	{
		UE_LOGF(LogWorldPartition, Error, "[EDL: %ls] Can't move actor %ls Package %ls from EDL %ls.", *GetDataLayerShortName(), *InActor->GetActorNameOrLabel(), *InActor->GetPackage()->GetName(), *ActorExternalDataLayer->GetName());
		return false;
	}
	...
	UE_LOGF(LogWorldPartition, Error, "[EDL: %ls] Failed to add actor %ls to EDL %ls.", *GetDataLayerShortName(), *InActor->GetActorNameOrLabel(), *ExternalDataLayer->GetName());
	return false;
}
