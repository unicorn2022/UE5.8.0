...
{
	static const FName NAME_DataLayerInstanceName("DataLayerInstanceName");
	static const FName NAME_DataLayerInstanceParentName("DataLayerInstanceParentName");
	static const FName NAME_DataLayerInstanceAssetPath("DataLayerInstanceAssetPath");
	static const FName NAME_DataLayerInstanceIsPrivate("DataLayerInstanceIsPrivate");
	static const FName NAME_DataLayerInstanceIsIncludedInActorFilterDefault("DataLayerInstanceIsIncludedInActorFilterDefault");
	static const FName NAME_DataLayerInstancePrivateDataLayerSupportsActorFilter("DataLayerInstancePrivateDataLayerSupportsActorFilter");
	static const FName NAME_DataLayerInstancePrivateShortName("DataLayerInstancePrivateShortName");
	static const FName NAME_DataLayerInstanceInitialRuntimeState("DataLayerInstanceInitialRuntimeState");
};
...
void UDataLayerInstance::GetAssetRegistryTags(FAssetRegistryTagsContext Context) const
{
	...
	if (IsPackageExternal())
	{
		...
		Context.AddTag(FAssetRegistryTag(DataLayerInstance::NAME_DataLayerInstanceName, *GetFName().ToString(), FAssetRegistryTag::TT_Hidden));
		if (GetParent())
		{
			Context.AddTag(FAssetRegistryTag(DataLayerInstance::NAME_DataLayerInstanceParentName, *GetParent()->GetFName().ToString(), FAssetRegistryTag::TT_Hidden));
		}
		...
		Context.AddTag(FAssetRegistryTag(DataLayerInstance::NAME_DataLayerInstanceInitialRuntimeState, *StaticEnum<EDataLayerRuntimeState>()->GetNameStringByValue(static_cast<int64>(InitialRuntimeState)), FAssetRegistryTag::TT_Hidden));
	}
}
...
bool UDataLayerInstance::GetAssetRegistryInfoFromPackage(const FAssetData& InAsset, FDataLayerInstanceDesc& OutDataLayerInstanceDesc)
{
	...
	if (InAsset.GetTagValue(DataLayerInstance::NAME_DataLayerInstanceInitialRuntimeState, Value))
	{
		OutDataLayerInstanceDesc.InitialRuntimeState = static_cast<EDataLayerRuntimeState>(StaticEnum<EDataLayerRuntimeState>()->GetValueByNameString(Value));
	}
...
{
	return GetOuterWorldDataLayers()->GetDataLayerRuntimeStateByName(GetFName());
}

EDataLayerRuntimeState UDataLayerInstance::GetEffectiveRuntimeState() const
{
	return GetOuterWorldDataLayers()->GetDataLayerEffectiveRuntimeStateByName(GetFName());
}
