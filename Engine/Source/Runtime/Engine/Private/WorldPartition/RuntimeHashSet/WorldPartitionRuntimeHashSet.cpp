...
static bool GRuntimeHashSetWorldAssetStreamingIncludeHLODFlagInCellHash = true;
static FAutoConsoleVariableRef CVarRuntimeHashSetWorldAssetStreamingIncludeHLODFlagInCellHash(
	TEXT("wp.Runtime.HashSet.WorldAssetStreaming.IncludeHLODFlagInCellHash"),
	GRuntimeHashSetWorldAssetStreamingIncludeHLODFlagInCellHash,
	TEXT("Used to choose whether HLOD flag should be included in cell hash generated during world asset streaming generation."));
...
#if WITH_EDITOR
		UE_LOGF(LogWorldPartition, Verbose, "CreatePartitionsSpatialIndex: %ls used %ls", *DebugName, *FGenericPlatformMemory::PrettyMemory(StaticIndexAllocatedSize));
#endif
	}
}
...
TOptional<FFixedGridInfo> UWorldPartitionRuntimeHashSet::GetFixedGridInfo(FName InGridName) const
{
	const URuntimePartition* Partition = ResolveRuntimePartition(InGridName, /*bMainPartitionLayer*/ true);
	return Partition ? Partition->GetFixedGridInfo() : TOptional<FFixedGridInfo>{};
}
...
void UWorldPartitionRuntimeHashSet::ForEachStreamingCellsQuery(const FWorldPartitionStreamingQuerySource& QuerySource, TFunctionRef<bool(const UWorldPartitionRuntimeCell*)> Func, FWorldPartitionQueryCache* QueryCache) const
{
	...
	auto ForEachNonSpatiallyLoadedCells = [this, &ShouldAddCell, &QuerySource, &Func](const TArray<TObjectPtr<UWorldPartitionRuntimeCell>>& InNonSpatiallyLoadedCells)
	{
		if (!bQueryNonSpatialCells)
		{
			return true;
		}
		for (const UWorldPartitionRuntimeCell* Cell : InNonSpatiallyLoadedCells)
		{
...
void UWorldPartitionRuntimeHashSet::ForEachStreamingCellsSources(const TArray<FWorldPartitionStreamingSource>& Sources, TFunctionRef<bool(const UWorldPartitionRuntimeCell*, EStreamingSourceTargetState)> Func, const FWorldPartitionStreamingContext& InContext) const
{
	...
	FDynamicLoadingRangeScales LoadingRangeScales
	{
#if WITH_EDITOR
		Context.GetDynamicLoadingRangeScales()
#endif // WITH_EDITOR
	};
	...
	for (const FWorldPartitionStreamingSource& Source : Sources)
	{
		...
		for (FName GridName : TargetGrids)
		{
			if (const TArray<const FRuntimePartitionStreamingData*>* StreamingDataList = RuntimeSpatiallyLoadedDataGridMap.Find(GridName))
			{
				...
				{
					Source.ForEachShape(StreamingData->GetLoadingRange(), false, [this, &Source, StreamingData, &Context, &Func, &LoadingRangeScales](const FSphericalSector& Shape)
					{
						...
						auto ForEachIntersectingElement = [&StreamingData, &Source, &ForEachIntersectingElementFunc, &LoadingRangeScales]<typename ShapeType>(ShapeType& Shape)
						{
							...
							if (StreamingData->SpatialIndex.IsValid())
							{
								if (Source.bForce2D)
								{
									Shape.Radius = ShapeRadius * LoadingRangeScales.Scale2D;
									StreamingData->SpatialIndexForce2D->ForEachIntersectingElement(Shape, ForEachIntersectingElementFunc);
								}
								else
								{
									Shape.Radius = ShapeRadius * LoadingRangeScales.Scale3D;
						...
						if (Shape.IsSphere())
						{
							FStaticSpatialIndex::FSphere ShapeSphere(Shape.GetCenter(), Shape.GetRadius());
							ForEachIntersectingElement(ShapeSphere);
						}
						else
						{
							FStaticSpatialIndex::FCone ShapeCone(Shape.GetCenter(), Shape.GetAxis(), Shape.GetRadius(), Shape.GetAngle());
...
FGuid UWorldPartitionRuntimeHashSet::RegisterWorldAssetStreaming(const UWorldPartition::FRegisterWorldAssetStreamingParams& InParams)
{
	...
	auto CreateStreamingCell = [&InParams, &SourceCellGuid, StreamingObject, this](const URuntimePartition* TargetPartition, const TSoftObjectPtr<UWorld>& WorldAsset, bool bIsHLODPass) -> bool
	{
		...
		if (GRuntimeHashSetWorldAssetStreamingIncludeHLODFlagInCellHash)
		{
			ArMD5 << bIsHLODPass;
		}
			...
			CreateRuntimeCell(UWorldPartitionRuntimeLevelStreamingCell::StaticClass(), UWorldPartitionRuntimeCellDataHashSet::StaticClass(), CellName, InParams.CellInstanceSuffix, StreamingObject)))
		{
			...
			{
				UE_LOGF(LogWorldPartition, Error, "Error creating streaming cell %ls for world asset %ls at %ls", *RuntimeCell->GetName(), *WorldAsset.ToString(), *InParams.Transform.ToString());
				return false;
			}
		}		
		else
...
void UWorldPartitionRuntimeHashSet::PostEditChangeChainProperty(FPropertyChangedChainEvent& PropertyChangedEvent)
{
	...
	static FName NAME_RuntimePartitions("RuntimePartitions");
	static FName NAME_HLODSetups("HLODSetups");
	static FName NAME_HLODLayers("HLODLayers");
	...
	if (PropertyName == GET_MEMBER_NAME_CHECKED(FRuntimePartitionDesc, Name))
	{
		...
		if (RuntimePartitionDesc.HLODSetups.IsValidIndex(HLODSetupsIndex))
		{
			...
			// Rename HLOD if it clashes with another one
			auto FindUniqueHLODName = [this, &RuntimePartitionDesc](FName TestedName, int32 SkipIndex)->FName
			{
				bool FoundDuplicate;
				do
				{
					...
					{
						if (CurHLODSetupsIndex != SkipIndex)
						{
							if (TestedName == RuntimePartitionDesc.HLODSetups[CurHLODSetupsIndex].Name)
							{
								FoundDuplicate = true;
								TestedName.SetNumber(TestedName.GetNumber() + 1);
								break;
							}
						}
					}
			...
			RuntimePartitionHLODSetup.Name = FindUniqueHLODName(RuntimePartitionHLODSetup.Name, HLODSetupsIndex);
		...
		{
			// Rename layer if it clashes with another one
			auto FindUniquePartitionName = [this](FName TestedName, int32 SkipIndex)->FName
			{
				bool FoundDuplicate;
				do
				{
					...
					{
						if (CurRuntimePartitionIndex != SkipIndex)
						{
							if (TestedName == RuntimePartitions[CurRuntimePartitionIndex].Name)
							{
								FoundDuplicate = true;
								TestedName.SetNumber(TestedName.GetNumber() + 1);
								break;
			...
			RuntimePartitionDesc.Name = FindUniquePartitionName(RuntimePartitionDesc.Name, RuntimePartitionIndex);
			RuntimePartitionDesc.MainLayer->Name = RuntimePartitionDesc.Name;
		}
	}
...
void UWorldPartitionRuntimeHashSet::PostLoad()
{
	...
	// Clear streaming data that could have inadvertently been serialized while saving in PIE when loading from disk (need to keep it when coming from PIE duplication).
	if (GetLinker())
	{
...
void UWorldPartitionRuntimeHashSet::FixupHLODSetup(FRuntimePartitionDesc& RuntimePartition)
{
	...
	{
		// By default, HLOD partitions are only loaded on the clients.
		// However, some HLOD layer configurations might requires them to be loaded on the server (for example, HLOD destruction
		// requires HLOD actors data to be replicated).
		auto HLODPartitionMustBeLoadedOnServer = [](const FRuntimePartitionHLODSetup& HLODSetup) -> bool
		{
			return Algo::AnyOf(HLODSetup.HLODLayers, [](const UHLODLayer* HLODLayer)
			{
				return HLODLayer && HLODLayer->MustBeLoadedOnServer();
			});			
		};
		...
		for (int32 HLODSetupIndex = 0; HLODSetupIndex < RuntimePartition.HLODSetups.Num(); )
		{
			...
			if (HLODSetup.PartitionLayer)
			{
				...
				// Only load HLOD on clients unless the HLOD setup requires them on the server too.
				HLODSetup.PartitionLayer->bClientOnlyVisible = !HLODPartitionMustBeLoadedOnServer(HLODSetup);
			}
		...
		if (IWorldPartitionEditorModule::Get().GetRequireExplicitHLODLayerPartitionAssignation())
		{
			...
			PersistentHLODSetup.PartitionLayer->bClientOnlyVisible = !HLODPartitionMustBeLoadedOnServer(PersistentHLODSetup);
		}
	}
}
...
FName UWorldPartitionRuntimeHashSet::ResolveRuntimeGridNameForHLODLayer(FName RuntimeGrid, const FSoftObjectPath& HLODLayerPath) const
{
	if (const URuntimePartition* RuntimePartition = ResolveRuntimePartitionForHLODLayer(RuntimeGrid, HLODLayerPath))
	{
		if (!RuntimePartition->SupportsHLODs())
		{
			return NAME_None;
		}

		return FName(RuntimeGrid.ToString() + TEXT(":") + RuntimePartition->Name.ToString());
	}
	return NAME_None;
}

bool UWorldPartitionRuntimeHashSet::CopyGridSetupFrom(const UWorldPartitionRuntimeHash* Other)
{
	if (const UWorldPartitionRuntimeHashSet* OtherHashSet = Cast<UWorldPartitionRuntimeHashSet>(Other))
	{
		RuntimePartitions.Empty();

		for (const FRuntimePartitionDesc& SourcePartitionDesc : OtherHashSet->RuntimePartitions)
		{
			FRuntimePartitionDesc& PartitionDesc = RuntimePartitions.AddDefaulted_GetRef();
			PartitionDesc.Name = SourcePartitionDesc.Name;
			PartitionDesc.MainLayer = DuplicateObject<URuntimePartition>(SourcePartitionDesc.MainLayer, this);

			for (const FRuntimePartitionHLODSetup& SourceHLODSetup : SourcePartitionDesc.HLODSetups)
			{
				...
				HLODSetup.PartitionLayer = DuplicateObject<URuntimePartition>(SourceHLODSetup.PartitionLayer, this);
			}
		}
		return true;
	}
...
{
	const int32 PreviousSpatialCellCount = StreamingData.SpatiallyLoadedCells.Num();
	...
	// See URuntimeHashSetExternalStreamingObject::AddReferencedObjects - cells in spatial indices are not tracked via ARO in non-editor builds, will lead to deallocated mem access
	checkf(StreamingData.SpatiallyLoadedCells.Num() == PreviousSpatialCellCount || !StreamingData.HasSpatialIndex(),
		TEXT("Spatial cells removed after SpatialIndex creation."));
}
