...
bool UWorldPartitionRuntimeHashSet::Draw2D(FWorldPartitionDraw2DContext& DrawContext) const
{
	...
	float DynamicLoadingRangeScale2D = 1.f;
	float DynamicLoadingRangeScale3D = 1.f;
#if WITH_EDITOR
	if (UWorldPartitionSubsystem* WorldPartitionSubsystem = UWorld::GetSubsystem<UWorldPartitionSubsystem>(GetWorld()))
	{
		...
		DynamicLoadingRangeScale2D = Scales.Scale2D;
		DynamicLoadingRangeScale3D = Scales.Scale3D;
	}
#endif // WITH_EDITOR

	auto DrawStreamingData = [this, &WorldPartitionTransform, DynamicLoadingRangeScale2D, DynamicLoadingRangeScale3D](const FRuntimePartitionStreamingData* StreamingData, const FBox2D& Region2D, const FBox2D& GridScreenBounds, TFunctionRef<FVector2D(const FVector2D&, bool)> InWorldToScreen, FWorldPartitionDraw2DContext& DrawContext)
	{
		const UWorldPartition* WorldPartition = GetOuterUWorldPartition();
		const TArray<FWorldPartitionStreamingSource>& Sources = WorldPartition->GetStreamingSources();
		...
		for (const FWorldPartitionStreamingSource& Source : Sources)
		{
			...
			Source.ForEachShape(StreamingData->GetLoadingRange(), StreamingData->Name, true, [&Color, &WorldToScreen, &GridScreenBounds, &DrawContext, &Source, DynamicLoadingRangeScale2D, DynamicLoadingRangeScale3D, this](const FSphericalSector& Shape)
			{
				check(!Shape.IsNearlyZero())

				float DynamicLoadingRangeScale = (Source.bForce2D) ? DynamicLoadingRangeScale2D : DynamicLoadingRangeScale3D;
				...
				const FVector Startup = FRotator(0, -0.5f * Angle, 0).RotateVector(Shape.GetScaledAxis() * DynamicLoadingRangeScale);
				...
				DrawContext.PushDrawSegment(GridScreenBounds, WorldToScreen(Center2D), WorldToScreen(Center2D + Axis * Shape.GetRadius() * DynamicLoadingRangeScale), Color, 4);
			});
...
void UWorldPartitionRuntimeHashSet::Draw3D(const TArray<FWorldPartitionStreamingSource>& Sources) const
{
	...
	ForEachStreamingData([this, &Sources, VisualizeMode, &DataLayerDebugColors, ContentBundleManager, OwningWorld, &WorldPartitionTransform, &DebugDrawCell3D, DynamicLoadingRangeScale2D, DynamicLoadingRangeScale3D](const FRuntimePartitionStreamingData& StreamingData)
	{
		if (StreamingData.SpatialIndex)
		{
			for (const FWorldPartitionStreamingSource& Source : Sources)
			{
				Source.ForEachShape(StreamingData.GetLoadingRange(), StreamingData.Name, false, [this, &StreamingData, VisualizeMode, &DataLayerDebugColors, ContentBundleManager, OwningWorld, &WorldPartitionTransform, &DebugDrawCell3D, &Source, DynamicLoadingRangeScale2D, DynamicLoadingRangeScale3D](const FSphericalSector& Shape)
				{
					...
					if (Shape.IsSphere())
					{
						const FStaticSpatialIndex::FSphere Sphere(Shape.GetCenter(), Shape.GetRadius() * DynamicLoadingRangeScale);
						ForEachIntersectingElement(Sphere);
					}
					else
					{
						const FStaticSpatialIndex::FCone Cone(Shape.GetCenter(), Shape.GetAxis(), Shape.GetRadius() * DynamicLoadingRangeScale, Shape.GetAngle());
		...
		if (StreamingData.SpatialIndex2D.IsValid())
		{
			for (const FWorldPartitionStreamingSource& Source : Sources)
			{
				...
				const FBox SourceBounds = Source.CalcBounds(StreamingData.GetLoadingRange() * DynamicLoadingRangeScale2D, StreamingData.Name, bCalcBoundsIn2D);
...
bool UWorldPartitionRuntimeHashSet::ContainsRuntimeHash(const FString& Name) const
{
	const FRuntimePartitionDesc* FoundRuntimePartitionDesc = RuntimePartitions.FindByPredicate([&Name](const FRuntimePartitionDesc& RuntimePartitionDesc)
		{
			return RuntimePartitionDesc.Name.ToString().Equals(Name, ESearchCase::IgnoreCase);
		});

	return FoundRuntimePartitionDesc != nullptr;
}
