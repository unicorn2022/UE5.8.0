...
void UWorldPartitionRuntimeCellData::AppendStreamingSourceInfo(const FWorldPartitionStreamingSource& Source, const FSphericalSector& SourceShape, const FWorldPartitionStreamingContext& Context, bool bBlockOnSlowLoading) const
{
	...
	if (bBlockOnSlowLoading && Source.bBlockOnSlowLoading)
	{
		...
		const double BlockOnSlowStreamingRatio = FMath::SafeDivide(FMath::Sqrt(CachedMinSquareDistanceToBlockingSource), SourceShape.GetRadius());
	...
	const double SlowStreamingRatio = FMath::SafeDivide(FMath::Sqrt(CachedMinSquareDistanceToSource), SourceShape.GetRadius());
	...
	const double SourceDistanceRatio = FMath::Clamp(FMath::SafeDivide(ContentBounds.ComputeSquaredDistanceToPoint(SourceShape.GetCenter()), FMath::Square(SourceShape.GetRadius())), 0.0f, 1.0f);

	// Compute final cell priority for this source
	const double SortingPriority = SourceDistanceRatio * SourceCosAngleRatio;
