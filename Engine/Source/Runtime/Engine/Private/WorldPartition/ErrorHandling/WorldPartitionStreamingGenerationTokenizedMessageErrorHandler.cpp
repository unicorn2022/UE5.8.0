...
void ITokenizedMessageErrorHandler::OnRejectedActorDescViewMutator(const IWorldPartitionActorDescInstanceView& ActorDescView, const FActorDescViewMutator& RejectedOverrides, EActorDescViewMutatorRejectedReason Reason)
{
	TSharedRef<FTokenizedMessage> Message = FTokenizedMessage::Create(EMessageSeverity::Warning);
	Message->AddToken(FTextToken::Create(LOCTEXT("TokenMessage_WorldPartition_MutatorRejected", "Mutator override rejected: cannot set actor")))
		->AddToken(FActorToken::Create(ActorDescView.GetActorSoftPath().ToString(), ActorDescView.GetGuid(), FText::FromString(GetActorName(ActorDescView))));

	switch (Reason)
	{
	case EActorDescViewMutatorRejectedReason::DivergesClusterRuntimeGrid:
		Message->AddToken(FTextToken::Create(LOCTEXT("TokenMessage_WorldPartition_MutatorRejected_RuntimeGridTo", "RuntimeGrid to")))
			->AddToken(FTextToken::Create(FText::FromName(RejectedOverrides.RuntimeGrid.GetValue())))
			->AddToken(FTextToken::Create(LOCTEXT("TokenMessage_WorldPartition_MutatorRejected_WouldSplitCluster", "because it would split a reference-connected cluster")));
		break;
	case EActorDescViewMutatorRejectedReason::DivergesClusterIsSpatiallyLoaded:
		Message->AddToken(FTextToken::Create(LOCTEXT("TokenMessage_WorldPartition_MutatorRejected_IsSpatiallyLoadedTo", "IsSpatiallyLoaded to")))
			->AddToken(FTextToken::Create(RejectedOverrides.bIsSpatiallyLoaded.GetValue()
				? LOCTEXT("TokenMessage_WorldPartition_True", "true")
				: LOCTEXT("TokenMessage_WorldPartition_False", "false")))
			->AddToken(FTextToken::Create(LOCTEXT("TokenMessage_WorldPartition_MutatorRejected_WouldSplitCluster", "because it would split a reference-connected cluster")));
		break;
	}

	Message->AddToken(FMapErrorToken::Create(TEXT("WorldPartition_ActorDescViewMutatorRejected_CheckForErrors")));
	HandleTokenizedMessage(MoveTemp(Message));
}
