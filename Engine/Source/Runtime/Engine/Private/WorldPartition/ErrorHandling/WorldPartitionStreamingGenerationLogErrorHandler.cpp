...
{
	static const FString SpatiallyLoadedActor("Spatially loaded actor");
	static const FString NonSpatiallyLoadedActor("Non-spatially loaded actor");
...
{
	UE_ASSET_LOG(LogWorldPartition, Log, DataLayerInstance, TEXT("Data Layer Instance %s and Data Layer Instance %s are both referencing Data Layer Asset %s"), *DataLayerInstance->GetFName().ToString(), *ConflictingDataLayerInstance->GetFName().ToString(), *DataLayerInstance->GetAsset()->GetFullName());
}
...
void FStreamingGenerationLogErrorHandler::OnRejectedActorDescViewMutator(const IWorldPartitionActorDescInstanceView& ActorDescView, const FActorDescViewMutator& RejectedOverrides, EActorDescViewMutatorRejectedReason Reason)
{
	switch (Reason)
	{
	case EActorDescViewMutatorRejectedReason::DivergesClusterRuntimeGrid:
		UE_ASSET_LOG_ACTORDESCVIEW(LogWorldPartition, Log, ActorDescView, TEXT("Mutator override rejected: cannot set actor %s RuntimeGrid to %s because it would split a reference-connected cluster"),
			*GetActorName(ActorDescView), *RejectedOverrides.RuntimeGrid.GetValue().ToString());
		break;
	case EActorDescViewMutatorRejectedReason::DivergesClusterIsSpatiallyLoaded:
		UE_ASSET_LOG_ACTORDESCVIEW(LogWorldPartition, Log, ActorDescView, TEXT("Mutator override rejected: cannot set actor %s IsSpatiallyLoaded to %s because it would split a reference-connected cluster"),
			*GetActorName(ActorDescView), RejectedOverrides.bIsSpatiallyLoaded.GetValue() ? TEXT("true") : TEXT("false"));
		break;
	}
}
#endif
