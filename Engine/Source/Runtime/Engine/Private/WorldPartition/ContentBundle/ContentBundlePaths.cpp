...
FString ContentBundlePaths::GetRelativeLevelFolder(const FContentBundleBase& ContentBundle)
{
	...
	UE_CLOGF(!FPackageName::IsValidPath(WorldPackageName), LogContentBundle, Warning, "Invalid world package name '%ls' (maybe not yet mounted), Content Bundle won't work as expected", *WorldPackageName);

	FString PackageRoot, PackagePath, PackageName;
	FPackageName::SplitPackageName(WorldPackageName, &PackageRoot, &PackagePath, &PackageName);
