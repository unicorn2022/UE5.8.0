...
void FContentBundleEditor::DoInjectContent()
{
	...
	if (bCreatedContainerPath)
	{
		...
		InitParams.FilterActorDescFunc = [&](const FWorldPartitionActorDesc* ActorDesc)
		{
			...
			{
				UE_LOGF(LogContentBundle, Display, "%ls Container '%ls' : Actor '%ls' object path doesn't match injected world path '%ls'",
		...
		{
			UE_CLOGF(ActorDescContainerInstance->GetActorDescInstanceCount() > 0, LogContentBundle, Log, "%ls ExternalActors in %ls found. %u actors were injected", *ContentBundle::Log::MakeDebugInfoString(*this), *ActorDescContainerInstance->GetExternalActorPath(), ActorDescContainerInstance->GetActorDescInstanceCount());
		...
		{
			UE_LOGF(LogContentBundle, Error, "%ls Failed to register actor desc container with %ls", *ContentBundle::Log::MakeDebugInfoString(*this), *ActorDescContainerPackage);
			SetStatus(EContentBundleStatus::FailedToInject);
		}
	}
	else
	{
		UE_LOGF(LogContentBundle, Error, "%ls Failed to build Container Package Path using %ls", *ContentBundle::Log::MakeDebugInfoString(*this), *GetInjectedWorld()->GetPackage()->GetName());
		SetStatus(EContentBundleStatus::FailedToInject);
	}
...
bool FContentBundleEditor::AddActor(AActor* InActor)
{
	...
	{
		UE_LOGF(LogContentBundle, Verbose, "%ls Adding first actor to content bundle.", *ContentBundle::Log::MakeDebugInfoString(*this));
		SetStatus(EContentBundleStatus::ContentInjected);
	}
	...
	UE_LOGF(LogContentBundle, Verbose, "%ls Added new actor %ls, ActorCount:  %u. Package %ls.", *ContentBundle::Log::MakeDebugInfoString(*this), *InActor->GetActorNameOrLabel(), GetActorCount(), *InActor->GetPackage()->GetName());
...
void FContentBundleEditor::InjectBaseContent()
{
	...
	UE_LOGF(LogContentBundle, Log, "%ls Injecting Base Content", *ContentBundle::Log::MakeDebugInfoString(*this));
...
void FContentBundleEditor::GenerateStreaming(TArray<FString>* OutPackageToGenerate, bool bIsPIE)
{
	...
	UE_LOGF(LogContentBundle, Log, "%ls Generated streaming cells. %u cells were generated", *ContentBundle::Log::MakeDebugInfoString(*this), CellCount);
	...
	if (bIsPIE)
	{
		...
		{
			UE_LOGF(LogContentBundle, Error, "%ls Failed to store streaming object. PIE duplication will not work.", *ContentBundle::Log::MakeDebugInfoString(*this));
		}
...
bool FContentBundleEditor::GatherPackagesToCook(class IWorldPartitionCookPackageContext& CookContext)
{
	...
	// Do not cook ContentBundles when building with (exclusive) EDL filters
	if (HasCookedContent() && UWorldPartitionSettings::GetEDLCookFilters().IsEmpty())
	{
		ExternalStreamingObject->ForEachStreamingCells([this, &bIsSuccess, &CookContext](UWorldPartitionRuntimeCell& RuntimeCell)
		{
			...
			{
				UE_LOGF(LogContentBundle, Error, "%ls[Cook] Failed to add cell package %ls in cook context.", *ContentBundle::Log::MakeDebugInfoString(*this), *RuntimeCell.GetPackageNameToCreate());
				bIsSuccess = false;
			}
		});
		...
		{
			UE_LOGF(LogContentBundle, Error, "%ls[Cook] Failed to add streaming object package in cook context.", *ContentBundle::Log::MakeDebugInfoString(*this));
			bIsSuccess = false;
		}
	}
...
bool FContentBundleEditor::PopulateGeneratorPackageForCook(class IWorldPartitionCookPackageContext& CookContext, const TArray<FWorldPartitionCookPackage*>& PackagesToCook, TArray<UPackage*>& OutModifiedPackages)
{
	...
	if (HasCookedContent())
	{
		UE_LOGF(LogContentBundle, Log, "%ls[Cook] Populating Generator Package. %u Packages", *ContentBundle::Log::MakeDebugInfoString(*this), PackagesToCook.Num());
		...
		for (const FWorldPartitionCookPackage* CookPackage : PackagesToCook)
		{
			if (CookPackage->Type == FWorldPartitionCookPackage::EType::Level)
			{
				...
				if (UWorldPartitionRuntimeCell* Cell = MatchingCell ? *MatchingCell : nullptr)
				{
					...
					{
						UE_LOGF(LogContentBundle, Error, "%ls[Cook] Failed to prepare cell with package %ls for cook.", *ContentBundle::Log::MakeDebugInfoString(*this), *CookPackage->RelativePath);
						bIsSuccess = false;
					}
				}
				else
				{
					UE_LOGF(LogContentBundle, Error, "%ls[Cook] Could not find cell for package %ls while populating generator packages.", *ContentBundle::Log::MakeDebugInfoString(*this), *CookPackage->RelativePath);
					bIsSuccess = false;
				}
			}
		}
...
bool FContentBundleEditor::PopulateGeneratedPackageForCook(class IWorldPartitionCookPackageContext& CookContext, const FWorldPartitionCookPackage& PackageToCook, TArray<UPackage*>& OutModifiedPackages)
{
	...
	UE_LOGF(LogContentBundle, Log, "%ls[Cook] Populating Generated Package %ls", *ContentBundle::Log::MakeDebugInfoString(*this), *PackageToCook.RelativePath);
	...
	if (PackageToCook.Type == FWorldPartitionCookPackage::EType::Level)
	{
				...
				if (!Cell->IsAlwaysLoaded())
				{
					...
					if (Cell->OnPopulateGeneratedPackageForCook(CookContext, PackageToCook.GetPackage(), OutModifiedPackages))
					{
						...
						if (CellWorld != nullptr)
						{
							...
							{
								UE_LOGF(LogContentBundle, Warning, "%ls[Cook] Failed to Remap cell in level for package %ls.", *ContentBundle::Log::MakeDebugInfoString(*this), *PackageToCook.RelativePath);
								bIsSuccess = false;
							}
						}
						else
						{
							UE_LOGF(LogContentBundle, Warning, "%ls[Cook] Failed to retrieve cell world from package %ls.", *ContentBundle::Log::MakeDebugInfoString(*this), *PackageToCook.RelativePath);
							bIsSuccess = false;
						}
					}
					else
					{
						UE_LOGF(LogContentBundle, Error, "%ls[Cook] Failed to populate cell package %ls.", *ContentBundle::Log::MakeDebugInfoString(*this), *PackageToCook.RelativePath);
						bIsSuccess = false;
					}
				}
				else
				{
					UE_LOGF(LogContentBundle, Warning, "%ls[Cook] Cell %ls is flagged always loaded. Content Bundles cells should never be always loaded. It will not be populated and be empty at runtime.",
		...
		{
			UE_LOGF(LogContentBundle, Error, "%ls[Cook] Could not find cell for package %ls while populating generated pacakges.", *ContentBundle::Log::MakeDebugInfoString(*this), *PackageToCook.RelativePath);
			bIsSuccess = false;
		}
	}
	else
	{
		...
		{
			UE_LOGF(LogContentBundle, Error, "%ls[Cook] Failed to rename streaming object package.", *ContentBundle::Log::MakeDebugInfoString(*this));
			bIsSuccess = false;
		}
	}
...
{
	UE_LOGF(LogContentBundle, Verbose, "%ls Added actor %ls to container, ActorCount: %u. Package %ls.", 
...
{
	UE_LOGF(LogContentBundle, Verbose, "%ls Removed actor %ls from container, ActorCount:  %u. Package %ls.", 
		*ContentBundle::Log::MakeDebugInfoString(*this), *ActorDescInstance->GetActorLabelOrName().ToString(), GetActorCount(), *ActorDescInstance->GetActorPackage().ToString());
...
{
	UE_LOGF(LogContentBundle, Verbose, "%ls Removed unsaved actor %ls, ActorCount: %u. Package %ls.",
