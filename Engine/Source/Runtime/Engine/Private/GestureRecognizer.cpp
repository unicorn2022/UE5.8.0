...
void FGestureRecognizer::DetectGestures(TArrayView<const UE::FTouchState> TouchStates, UPlayerInput* PlayerInput, float DeltaTime)
{
	// how many fingers currently held down?
	int32 TouchCount = 0;
	for (int32 TouchIndex = 0; TouchIndex < TouchStates.Num(); TouchIndex++)
	{
		if (TouchStates[TouchIndex].Pressure != 0)
		{
			TouchCount++;
		}
	}
	...
	if (PreviousTouchCount != 0 || TouchCount != 0)
	{
		...
		for (int32 Index = 0; Index < UE_ARRAY_COUNT(AnchorPoints); Index++)
		{
			...
			{
				AnchorPoints[Index] = FVector2D(TouchStates[Index].ViewportRelativeLocation);
			}
		}
		...
		if (TouchCount >= 2)
		{
			...
			FVector2D CurrentPinchPoint_Start(TouchStates[0].ViewportRelativeLocation);
			FVector2D CurrentPinchPoint_End(TouchStates[1].ViewportRelativeLocation);
			...
			*CurrentAlpha = NewDistanceSq / AnchorDistanceSq;
			...
			float NewAngle = FMath::Atan2(CurrentPinchPoint_Start.Y - CurrentPinchPoint_End.Y, CurrentPinchPoint_Start.X - CurrentPinchPoint_End.X);
		...
		else if (PreviousTouchCount == 1 && TouchCount == 1)
		{
			...
			const FVector2f& Loc0 = TouchStates[0].ViewportRelativeLocation;
			FlickCurrent = FVector2D(Loc0);
			FlickTime += DeltaTime;
		}
