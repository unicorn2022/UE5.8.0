...
namespace PreviewScene::Private
{
	bool bForceGCOnPreviewSceneExit = true;
	FAutoConsoleVariableRef CVarForceGCOnPreviewSceneExit(TEXT("r.ForceGCOnPreviewSceneExit"), bForceGCOnPreviewSceneExit, TEXT(""), ECVF_Default);
	...
	, bForceAllUsedMipsResident(CVS.bForceMipsResident)
{
	...
	PreviewWorld->InitializeNewWorld(UWorld::InitializationValues()
										...
										.AllowLumenPrimitiveTrackingInPreviewWorld(CVS.bAllowLumenPrimitiveTrackingInPreviewWorld));
...
void FPreviewScene::Uninitialize()
{
	...
	if (LocalPreviewWorld && GEngine)
	{
		...
		if (PreviewScene::Private::bForceGCOnPreviewSceneExit)
		{
