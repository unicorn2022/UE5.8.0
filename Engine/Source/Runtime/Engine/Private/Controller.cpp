...
void AController::SetControlRotation(const FRotator& NewRotation)
{
	...
	{
		//UE_LOGF(LogPlayerController, Log, "Skipping SetControlRotation %ls for %ls (Pawn %ls)", *NewRotation.ToString(), *GetNameSafe(this), *GetNameSafe(GetPawn()));
	}
}
...
void AController::PawnPendingDestroy(APawn* inPawn)
{
	...
	{
		UE_LOGF(LogController, Log, "PawnPendingDestroy while inactive %ls", *GetName());
	}
...
#if UE_WITH_REMOTE_OBJECT_HANDLE
FRemoteServerId AController::GetPreferredRemoteServer() const
{
	TObjectPtr<APawn> PawnPtr = GetPawn();  // TObjectPtr by value, no auto-pull
	if (!AutoRTFM::IsClosed() && PawnPtr.GetResidence() == EResidence::Remote)
	{
		// We are outside of a transaction (probably push migration step) and the
		// Pawn is already on another server — that IS its preferred server
		FRemoteServerId PawnServerId = UE::RemoteObject::Handle::GetOwnerServerId(PawnPtr.GetRemoteId());
		if (PawnServerId.IsValid())
		{
			return PawnServerId;
		}
	}
	else if (APawn* LocalPawn = PawnPtr.Get())
	{
		// Pawn is local — ask where it WANTS to be
		return LocalPawn->GetPreferredRemoteServer();
	}

	// No Pawn — stay where we are (don't use spatial delegate; controllers
	// without pawns have no meaningful location to determine preferred server)
...
void AController::InitPlayerState()
{
	if ( GetNetMode() != NM_Client )
	{
		...
		if (GameMode != NULL)
		{
			...
			{
				UE_LOGF(LogPlayerController, Log, "AController::InitPlayerState: the PlayerStateClass of game mode %ls is null, falling back to APlayerState.", *GameMode->GetName());
