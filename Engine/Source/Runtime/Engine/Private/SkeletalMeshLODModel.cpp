...
FArchive& operator<<(FArchive& Ar, FSkelMeshSection& S)
{
	...
	if (Ar.CustomVer(FSkeletalMeshCustomVersion::GUID) >= FSkeletalMeshCustomVersion::CombineSectionWithChunk)
	{
		...
		if (Ar.IsLoading() && Ar.CustomVer(FSkeletalMeshCustomVersion::GUID) < FSkeletalMeshCustomVersion::SaveNumVertices)
		{
			...
			{
				UE_LOGF(LogSkeletalMesh, Warning, "Cannot set FSkelMeshSection::NumVertices for older content, loading in non-editor build.");
				S.NumVertices = 0;
			}
		}
		...
		if (Ar.CustomVer(FSkeletalMeshCustomVersion::GUID) < FSkeletalMeshCustomVersion::CombineSoftAndRigidVerts)
		{
			...
			{
				UE_LOGF(LogSkeletalMesh, Error, "Legacy NumSoftVerts + NumRigidVerts != SoftVertices.Num()");
			}
		}
...
void FSkeletalMeshLODModel::GetVertices(TArray<FSoftSkinVertex>& Vertices, const FSkeletalMeshUnifiedBoneIndices* UnifiedBoneIndices) const
{
	...
	// validate NumVertices is correct
	{
		...
		if (TotalSoftVertices != NumVertices)
		{
			...
			UE_LOGF(LogSkeletalMesh, Fatal, "NumVertices (%i) != TotalSoftVertices (%i)", NumVertices, TotalSoftVertices);
		}
	}

	// Initialize the vertex data
	// All chunks are combined into one (rigid first, soft next)
	FSoftSkinVertex* DstVertices = (FSoftSkinVertex*)Vertices.GetData();

	for (const FSkelMeshSection& Section : Sections)
	{
		if (UnifiedBoneIndices && UnifiedBoneIndices->IsValid())
		{
			for (int32 VertexIndex = 0; VertexIndex < Section.SoftVertices.Num(); ++VertexIndex)
			{
				FSoftSkinVertex& DstVertex = DstVertices[VertexIndex];
				DstVertex = Section.SoftVertices[VertexIndex];
				UnifiedBoneIndices->RemapVertex(Section.BoneMap, DstVertex);
			}
		}
		else
		{
			FMemory::Memcpy(DstVertices, Section.SoftVertices.GetData(), Section.SoftVertices.Num() * sizeof(FSoftSkinVertex));
		}

		DstVertices += Section.SoftVertices.Num();
	}
}
...
void FSkeletalMeshLODModel::GetNonClothVertices(TArray<FSoftSkinVertex>& OutVertices, const FSkeletalMeshUnifiedBoneIndices* UnifiedBoneIndices) const
{
	...
	// Initialize the vertex data
	// All chunks are combined into one (rigid first, soft next)
	FSoftSkinVertex* DstVertices = (FSoftSkinVertex*)OutVertices.GetData();
	for (const FSkelMeshSection& Section : Sections)
	{
		if (Section.ClothingData.AssetGuid.IsValid())
		{
...
void FSkeletalMeshLODModel::UpdateChunkedSectionInfo(const FString& SkeletalMeshName)
{
	...
	for (int32 LODModelSectionIndex = 0; LODModelSectionIndex < LODModelSectionNum; ++LODModelSectionIndex)
	{
		...
		if (bIsOldChunkingSection && Section.MaterialIndex == LastMaterialIndex && !Section.ClothingData.AssetGuid.IsValid())
		{
