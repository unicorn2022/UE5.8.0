...
{
	static const FName Reset("Reset");
	static const FName HitBox("HitBox");
	static const FName Animation("Animation");
	static const FName Physics("Physics");
}
...
void AHUD::DrawDebugTextList()
{
	if ((DebugTextList.Num() > 0) && (DebugCanvas != nullptr) && (DebugCanvas->Canvas != nullptr))
	{
		...
		for (int32 Idx = 0; Idx < DebugTextList.Num(); Idx++)
		{
			...
			UE_LOGF(LogHUD, VeryVerbose, "[%f] %ls: \"%ls\" Loc: [%ls] "
				, GetWorld()->GetTimeSeconds()
				, *GetNameSafe(DebugTextList[Idx].SrcActor)
				, *DebugTextList[Idx].DebugText
				, *WorldTextLoc.ToString());
...
void AHUD::AddHitBox(FVector2D Position, FVector2D Size, FName Name, bool bConsumesInput, int32 Priority)
{
	...
	{
		UE_LOGF(LogHUD, Warning, "Failed to add hitbox named %ls as a hitbox with this name already exists", *Name.ToString());
	}
}
