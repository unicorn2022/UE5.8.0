...
void FMultiSizeIndexContainer::CopyIndexBuffer(const TArray<uint32>& NewArray)
{
	...
	else
	{
		...
		for (int32 i = 0; i < NewArray.Num(); ++i)
		{
			...
			{
				UE_LOGF(LogSkeletalMesh, Warning, "Attempting to copy %u into a uint16 index buffer - this value will overflow to %u, use RebuildIndexBuffer to create a uint32 index buffer!", NewArray[i], (uint16)NewArray[i]);
			}
#endif
