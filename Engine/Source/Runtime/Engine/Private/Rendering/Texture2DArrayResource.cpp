...
static const FName NAME_FTexture2DArrayResource("FTexture2DArrayResource");
...
: FStreamableTextureResource(InOwner, InOwner->GetPlatformData(), InState, false)
{
	...
	if (InOwner->GetMipData(State.LODCountToAssetFirstLODIdx(State.NumRequestedLODs), AllMipsData) == false)
	{
		...
		UE_LOGF(LogTexture, Fatal, "Corrupt texture [%ls]! Unable to load mips (bulk data missing)", *TextureName.ToString());
		return;
	}
...
void FTexture2DArrayResource::CreateTexture(FRHICommandListBase& RHICmdList)
{
	...
	{		
		for (uint32 ArrayIndex = 0; ArrayIndex < SizeZ; ++ArrayIndex)
		{
			...
			{
				UE_LOGF(LogTexture, Warning, "Failed to lock texture 2d array mip/slice %d / %d (%ls)", MipIndex, ArrayIndex, *TextureName.ToString());
			}
