...
static const FName NAME_FTexture3DResource("FTexture3DResource");
...
{
	if (!GRHIGlobals.SupportsTexture3DAnisotropicFiltering)
	{
		MaxAniso = 1;
	}
		...
		InitialData.GetMipData() + FirstLODIdx, InitialData.GetMipSize() + FirstLODIdx, InOwner->GetPathName()))
	{
		...
		if (GRHIGlobals.UseTexture3DBulkData)
		{
			for (int32 MipIndex = FirstLODIdx; MipIndex < InState.MaxNumLODs; ++MipIndex)
			{
				...
				UE_LOGF(LogTexture,Verbose,"FTexture3DResource::FTexture3DResource %d : %dx%dx%d : MipBytes=%d",
	...
	if (!GRHIGlobals.UseTexture3DBulkData) 
	{
		...
		for (int32 RHIMipIdx = 0; RHIMipIdx < State.NumRequestedLODs; ++RHIMipIdx)
		{
			...
			if (MipData)
			{
				...
				// RHICmdList.UpdateTexture3D crashes on some platforms at engine initialization time.
				...
				UE_LOGF(LogTexture,Verbose,"FTexture3DResource::CreateTexture:UpdateTexture3D %d : %dx%dx%d : RowStride=%d, SliceStride=%d, MipBytes=%d",
