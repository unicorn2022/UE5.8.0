...
{
	return IsFeatureLevelSupported(InShaderPlatform, ERHIFeatureLevel::ES3_1) && Substrate::IsSpecularProfileEnabled(InShaderPlatform);
}
...
public:
	...
	class FComponentCount : SHADER_PERMUTATION_SPARSE_INT("PERMUTATION_COMPONENT_COUNT", 3, 4);
	using FPermutationDomain = TShaderPermutationDomain<FProcedural, FComponentCount>;
...
public:
	...
	class FComponentCount : SHADER_PERMUTATION_SPARSE_INT("PERMUTATION_COMPONENT_COUNT", 3, 4);
	using FPermutationDomain = TShaderPermutationDomain<FReadWriteOnArray, FComponentCount>;
	...
	static bool ShouldCompilePermutation(const FGlobalShaderPermutationParameters& Parameters)
	{
		if (IsSpecularProfileSupport(Parameters.Platform))
		{
			const FPermutationDomain PermutationVector(Parameters.PermutationId);
			return !IsOpenGLPlatform(Parameters.Platform) || !PermutationVector.Get<FReadWriteOnArray>();
		}
		return false;
	}
	static void ModifyCompilationEnvironment(const FGlobalShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment)
...
IPooledRenderTarget* FSpecularProfileTextureManager::GetAtlasTexture(FRDGBuilder& GraphBuilder, EShaderPlatform ShaderPlatform)
{
	...
	if (GSpecularProfileTextureAtlas == nullptr)
	{
		...
		// Check if we can do typed UAV loads on pixel formats with optional UAV load support
		const bool bR11G11B10Supported = GPixelFormats[PF_FloatR11G11B10].Supported && UE::PixelFormat::HasCapabilities(PF_FloatR11G11B10, EPixelFormatCapabilities::TextureFilterable | EPixelFormatCapabilities::UAV);
		const bool bR11G11B10TypedLoadSupported = bR11G11B10Supported && UE::PixelFormat::HasCapabilities(PF_FloatR11G11B10, EPixelFormatCapabilities::TypedUAVLoad);
		const bool bRGBA16TypedLoadSupported = UE::PixelFormat::HasCapabilities(PF_FloatRGBA, EPixelFormatCapabilities::TypedUAVLoad);

		// Certain platforms don't support read/write on the same texture array, even if this done safely on different slices.
		// We fall back to an SRV load during convolution if we can't support this, or if neither pixel format supports typed UAV loads.
		const bool bConvolveSupportsTypedUAVLoad = !IsOpenGLPlatform(ShaderPlatform) && (bR11G11B10TypedLoadSupported || bRGBA16TypedLoadSupported);

		const EPixelFormat Format = bConvolveSupportsTypedUAVLoad ?
			(bR11G11B10TypedLoadSupported ? PF_FloatR11G11B10 : PF_FloatRGBA) :
			(bR11G11B10Supported ? PF_FloatR11G11B10 : PF_FloatRGBA);

		const int32 ComponentCount = GPixelFormats[Format].NumComponents;
		...
		for (uint32 LayerIt = 0; LayerIt < LayerCount; ++LayerIt)
		{
			...
			PermutationVector.Set<FSpecularProfileCopyCS::FComponentCount>(ComponentCount);
		...
		if (LayerCount > 0)
		{
			...
			if (!bConvolveSupportsTypedUAVLoad)
			{
			...
			PermutationVector.Set<FSpecularProfileConvolveCS::FComponentCount>(ComponentCount);
