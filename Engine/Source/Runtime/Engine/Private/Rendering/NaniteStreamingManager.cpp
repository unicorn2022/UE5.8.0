...
static FAutoConsoleVariableRef CVarNaniteStreamingDynamicallyGrowAllocations(
	...
	TEXT("Determines if root page allocations are allowed to grow dynamically from initial allocation set by r.Nanite.Streaming.NumInitialRootPages"),
	ECVF_RenderThreadSafe | ECVF_ReadOnly
);
...
void FStreamingManager::ResetStreamingStateCPU()
{
	...
	// TODO: Need to do anything about ModifiedResources / InstalledPages / UninstalledPages here?
...
void FStreamingManager::UpdatePageConfiguration()
{
	...
	{
		UE_LOGF(LogNaniteStreaming, Fatal, "Streaming pool size (%dMB) must be smaller than the largest allocation supported by the graphics hardware (%dMB)", StreamingPoolSizeInMB, MaxPoolSizeInMB);
	}
...
void FStreamingManager::Add( FResources* Resources )
{
	...
	if (Resources->RuntimeResourceID == INDEX_NONE)
	{
		...
		const uint32 NumBoneIndexDwords = Resources->BoneIndices.Num();
		const uint32 NumAssemblyTransforms = Resources->AssemblyTransforms.Num();
		const uint32 NumAssemblyTransformDwords = NumAssemblyTransforms * NANITE_ASSEMBLY_TRANSFORM_SIZE_DWORDS;
		const uint32 NumBoneAttachmentDataDwords = Resources->AssemblyBoneAttachmentData.Num();
		const uint32 TotalHierarchyDwords = NumHierarchyNodeDwords + NumBoneIndexDwords + NumAssemblyTransformDwords + NumBoneAttachmentDataDwords;

		Resources->HierarchyOffset = Hierarchy.Allocator.Allocate(TotalHierarchyDwords);
		Resources->AssemblyTransformOffset = NumAssemblyTransformDwords > 0 ? Resources->HierarchyOffset + NumHierarchyNodeDwords + NumBoneIndexDwords : MAX_uint32;
		Resources->NumAssemblyTransforms = NumAssemblyTransforms;
		...
		{
			UE_LOGF(LogNaniteStreaming, Fatal, "Out of root pages. Increase the initial root page allocation (r.Nanite.Streaming.NumInitialRootPages) or allow it to grow dynamically (r.Nanite.Streaming.DynamicallyGrowAllocations).");
		}
		...
		if ((uint32)Resources->RootPageIndex >= MaxRootPages)
		{
			...
			UE_LOGF(LogNaniteStreaming, Fatal, "Cannot allocate more root pages %d/%d. Pool resource has grown to maximum size of %dMB.\n"
...
void FStreamingManager::Remove( FResources* Resources )
{
	...
	if (Resources->RuntimeResourceID != INDEX_NONE)
	{
		...
		const uint32 BaseRootPageIndex = Resources->RootPageIndex;
		const uint32 NumRootPages = Resources->NumRootPages;
		ClusterPageData.Allocator.Free(BaseRootPageIndex, NumRootPages);
		...
		const uint32 VirtualPageRangeStart = RootPageInfos[BaseRootPageIndex].VirtualPageRangeStart;
		...
		for (uint32 LocalPageIndex = 0; LocalPageIndex < NumRootPages; LocalPageIndex++)
		{
			const uint32 RootPageIndex = BaseRootPageIndex + LocalPageIndex;
			const uint32 GPUPageIndex = MaxStreamingPages + RootPageIndex;

			FRootPageInfo& RootPageInfo = RootPageInfos[RootPageIndex];

			if (RootPageInfo.FixupChunk)
			{
				FPageInfo PageInfo{ GPUPageIndex, RootPageInfo.FixupChunk->Header.NumClusters, Resources->RuntimeResourceID };
				if (InstalledPages.Remove(PageInfo) == 0)
				{
					UninstalledPages.Add(PageInfo);
				}
			}
...
static bool VerifyFixupChunk(const FFixupChunk& FixupChunk, const FResources& Resources, bool bFatal)
{
	...
	if (!bValid)
	{
		if (bFatal)
		{
			UE_LOGF(LogNaniteStreaming, Fatal,
				"Encountered a corrupt fixup chunk for resource (%ls). Magic: %4X. This should never happen.",
		...
		{
			UE_LOGF(LogNaniteStreaming, Error,
				"Encountered a corrupt fixup chunk for resource (%ls). Magic: %4X. This should never happen.",
				GetNaniteResourceName(Resources),
				FixupChunk.Header.Magic
			);
		...
		if (ResidentPage.Key.RuntimeResourceID != INDEX_NONE)
		{
			...
			for (uint32 i = 0; i < FixupChunk.Header.NumGroupFixups; i++)
			{
				...
				if (bInstalled != bSatisfied)
				{
					...
					UE_LOGF(LogNaniteStreaming, Verbose, "FixupVerifyState failed for page GPUpage %u: Key: (%x, %u) Group: %u, Installed: %u, Satisfied: %u, PageDependencies: %ls",
...
void FStreamingManager::UninstallResidentPage(uint32 GPUPageIndex, uint32 NumStreamingPages, const TSet<uint32>* NoWriteGPUPages, bool bApplyFixup)
{
	...
	if (ResidentPage.Key.RuntimeResourceID != INDEX_NONE)
	{
		...
		const uint32 NumClusters = ResidentPageFixupChunks[GPUPageIndex]->Header.NumClusters;

		Resources.NumResidentClusters -= NumClusters;
		...
		{
			FPageInfo PageInfo{ GPUPageIndex, NumClusters, ResidentPage.Key.RuntimeResourceID };
			if (InstalledPages.Remove(PageInfo) == 0)
			{
				UninstalledPages.Add(PageInfo);
			}
...
void FStreamingManager::InstallReadyPages( uint32 NumReadyOrSkippedPages )
{
	...
	UE_LOGF(LogNaniteStreaming, Verbose, "InstallReadyPages: %u", NumReadyOrSkippedPages);
	...
	// Must be processed in PendingPages order so FFixupChunks are loaded when we need them.
	{
		...
		for (uint32 LocalPageIndex = 0; LocalPageIndex < NumReadyOrSkippedPages; LocalPageIndex++)
		{
			...
			UE_LOGF(LogNaniteStreaming, Verbose, "Install GPUPage: %u, InstallKey: (%x, %u), ResidentKey: (%x, %u)",
			...
			if (!RootPageInfo)
			{
				UE_LOGF(LogNaniteStreaming, Verbose, "Skip install. Resource no longer exists.");
				continue;	// Resource no longer exists. Skip resource install.
			}

			if (RootPageInfo->bInvalidResource)
			{
				UE_LOGF(LogNaniteStreaming, Verbose, "Skip install. Resource is marked invalid.");
				continue;
			}
			...
			{
				UE_LOGF(LogNaniteStreaming, Verbose, "Skip install. FixupChunk is invalid. Marking resource as invalid.");
			...
			InstalledPages.Add(FPageInfo{ PendingPage.GPUPageIndex, FixupChunk->Header.NumClusters, PendingPage.InstallKey.RuntimeResourceID });
			...
			{
				UE_LOGF(LogNaniteStreaming, Verbose, "Skip upload.");
			}
...
FRDGBuffer* FStreamingManager::ResizePoolAllocationIfNeeded(FRDGBuilder& GraphBuilder)
{
	...
	if (bResetStreamingState)
	{
...

		{
			...
			for (uint32 RootPageIndex = 0; RootPageIndex < (uint32)RootPageInfos.Num(); RootPageIndex++)
			{
				...
				if (RootPageInfo.RuntimeResourceID != INDEX_NONE && RootPageInfo.FixupChunk)	// FixupChunk when the resource has been added, but ProcessNewResources hasn't run yet
				{
					ApplyFixups(*RootPageInfo.FixupChunk, *RootPageInfo.Resources, nullptr, MaxStreamingPages, MAX_uint32, RootPageInfo.VirtualPageRangeStart, false, false, true);

					FPageInfo PageInfo;
					PageInfo.GPUPageIndex = OldMaxStreamingPages + RootPageIndex; // Old GPUPageIndex
					PageInfo.NumClusters = RootPageInfo.FixupChunk->Header.NumClusters;
					PageInfo.RuntimeResourceID = RootPageInfo.RuntimeResourceID;
					...
					PageInfo.GPUPageIndex = MaxStreamingPages + RootPageIndex; // New GPUPageIndex
					InstalledPages.Add(PageInfo);
				}
			}
	...
	{
		ClusterPageDataBuffer = ResizeByteAddressBufferIfNeededWithCurrentLLMTag(GraphBuilder, ClusterPageData.DataBuffer, AllocatedPagesSize, TEXT("Nanite.StreamingManager.ClusterPageData"));		
		if (!bClusterPageDataAllocated && (MaxStreamingPages != OldMaxStreamingPages))
		{
			// Even if GPU data is just allocated for the first time, we need to reset CPU state if MaxStreamingPages changed
			ResetStreamingStateCPU();
		}
...
void FStreamingManager::ProcessNewResources(FRDGBuilder& GraphBuilder, FRDGBuffer* ClusterPageDataBuffer)
{
	...
	for (FResources* Resources : PendingAdds)
	{
		...
		for (uint32 LocalPageIndex = 0; LocalPageIndex < Resources->NumRootPages; LocalPageIndex++)
		{
			...
			InstalledPages.Add(FPageInfo{ GPUPageIndex, NumClusters, Resources->RuntimeResourceID });
		}
		...
		const uint32 BoneIndicesSizeDwords = Resources->BoneIndices.Num();
		...
		if (BoneIndicesSizeDwords > 0)
		{
			Hierarchy.UploadBuffer.Add(Resources->HierarchyOffset + HierarchyNodeSizeDwords, Resources->BoneIndices.GetData(), BoneIndicesSizeDwords);
		}
		if (AssemblyTransformSizeDwords > 0)
		{
			Hierarchy.UploadBuffer.Add(Resources->HierarchyOffset + HierarchyNodeSizeDwords + BoneIndicesSizeDwords, Resources->AssemblyTransforms.GetData(), AssemblyTransformSizeDwords);
		}
		if (AssemblyBoneAttachmentSizeDwords > 0)
		{
			Hierarchy.UploadBuffer.Add(Resources->HierarchyOffset + HierarchyNodeSizeDwords + BoneIndicesSizeDwords + AssemblyTransformSizeDwords, Resources->AssemblyBoneAttachmentData.GetData(), AssemblyBoneAttachmentSizeDwords);
		}
		...
		Resources->BoneIndices.Empty();
		Resources->AssemblyTransforms.Empty();
		Resources->AssemblyBoneAttachmentData.Empty();
	#endif
	}
		...
		for( uint32 i = 0; i < NumPendingPages && NumReadyOrSkippedPages < MaxPageInstallsPerUpdate; i++ )
		{
...
#if WITH_EDITOR
			// Handle any DDC request that completed
			if (PendingPage.DCCRequestCompleted.exchange(false, std::memory_order_acquire))
			{
				PendingPage.State = PendingPage.SharedBuffer.IsNull() ? FPendingPage::EState::DDC_Failed : FPendingPage::EState::DDC_Ready;
			}
			...
			if (PendingPage.State == FPendingPage::EState::DDC_Ready)
			{
				if (PendingPage.RetryCount > 0)
				{
					...
					if (Resources)
					{
						UE_LOGF(LogNaniteStreaming, Log, "Nanite DDC retry succeeded for '%ls' (Page %d) after %d attempts.", *Resources->ResourceName, PendingPage.InstallKey.PageIndex, PendingPage.RetryCount);
					}
				}
			}
			...
			else if (PendingPage.State == FPendingPage::EState::DDC_Failed)
			{
				...
				if (Resources)
				{
					...
					{
						UE_LOGF(LogNaniteStreaming, Log, "Nanite DDC request failed for '%ls' (Page %d).", *Resources->ResourceName, PendingPage.InstallKey.PageIndex);
					}
					
					if (GNaniteStreamingNumRetries < 0 || PendingPage.RetryCount <= (uint32)GNaniteStreamingNumRetries)
					{
						UE_LOGF(LogNaniteStreaming, Log, "Retrying Nanite DDC request for '%ls' (Page %d).", *Resources->ResourceName, PendingPage.InstallKey.PageIndex);
					...
					{
						UE_LOGF(LogNaniteStreaming, Warning, "Too many Nanite DDC retries for '%ls' (Page %d). Giving up and marking resource invalid.", *Resources->ResourceName, PendingPage.InstallKey.PageIndex);
				...
				if (PendingPage.Request.IsCompleted())
				{
					if (!PendingPage.Request.IsOk())
					{
						...
						if (Resources)	// If the resource is gone, no need to do anything as the page will be ignored by InstallReadyPages
						{
							...
							{
								UE_LOGF(LogNaniteStreaming, Warning, "IO Request failed. RuntimeResourceID: %8X, Offset: %d, Size: %d.", PendingPage.InstallKey.RuntimeResourceID, PageStreamingState.BulkOffset, PageStreamingState.BulkSize);
							}

							if (GNaniteStreamingNumRetries < 0 || PendingPage.RetryCount <= (uint32)GNaniteStreamingNumRetries)
							{
								UE_LOGF(LogNaniteStreaming, Log, "Retrying IO request RuntimeResourceID: %8X, Offset: %d, Size: %d.", PendingPage.InstallKey.RuntimeResourceID, PageStreamingState.BulkOffset, PageStreamingState.BulkSize);
							...
							{
								UE_LOGF(LogNaniteStreaming, Warning, "Too many Nanite IO request retries for RuntimeResourceID: %8X, Offset: %d, Size: %d. Giving up and marking resource invalid.", PendingPage.InstallKey.RuntimeResourceID, PageStreamingState.BulkOffset, PageStreamingState.BulkSize);
								RootPageInfos[Resources->RootPageIndex].bInvalidResource = true;
							}
						}
					}
					else
					{
						if (PendingPage.RetryCount > 0)
						{
							...
							{
								const FPageStreamingState& PageStreamingState = Resources->PageStreamingStates[PendingPage.InstallKey.PageIndex];
								UE_LOGF(LogNaniteStreaming, Log, "Nanite IO request retry succeeded for RuntimeResourceID: %8X, Offset: %d, Size: %d after %d attempts.", PendingPage.InstallKey.RuntimeResourceID, PageStreamingState.BulkOffset, PageStreamingState.BulkSize, PendingPage.RetryCount);
							}
						}
					}
...
void FStreamingManager::SanityCheckStreamingRequests(const FGPUStreamingRequest* StreamingRequestsPtr, const uint32 NumStreamingRequests)
{
	...
	for (uint32 Index = 0; Index < NumStreamingRequests; Index++)
	{
		...
		{
			UE_LOGF(LogNaniteStreaming, Fatal, "Validation of Nanite streaming request failed! The magic doesn't match. This likely indicates an issue with the GPU readback.");
		}
		...
		{
			UE_LOGF(LogNaniteStreaming, Fatal, "Validation of Nanite streaming request failed! Single readback has data from multiple frames. Is there a race condition on the readback, a missing streaming update or is GPUScene being updated mid-frame?");
		}
		...
		if (Resources)
		{
			...
			{
				UE_LOGF(LogNaniteStreaming, Fatal, "Validation of Nanite streaming request failed! Page lookup key is empty.");
			}

			if (!GPURequest.ResourcePageRangeKey.HasStreamingPages())
			{
				UE_LOGF(LogNaniteStreaming, Fatal, "Validation of Nanite streaming request failed! Request has no streaming pages.");
			}
			
			if (!Resources->IsValidPageRangeKey(GPURequest.ResourcePageRangeKey))
			{
				UE_LOGF(LogNaniteStreaming, Fatal, "Validation of Nanite streaming request failed! Page lookup key is not valid.");
			}
		}
	}
}
	...
	do
	{
		...
		if (RootPageInfo && !RootPageInfo->bInvalidResource)
		{
			if (Priority == 0u || Priority > NANITE_MAX_PRIORITY_BEFORE_PARENTS)
			{
				if (bVerify)
				{
					UE_LOGF(LogNaniteStreaming, Warning, "Invalid priority %u of request for resource (%8X, %ls).", Priority, RuntimeResourceID, GetNaniteResourceName(*RootPageInfo->Resources));
				}
				continue;
			}
			...
			if (!PageRangeKey.IsMultiRange())
			{
				...
				{
					UE_LOGF(LogNaniteStreaming, Warning, "Invalid page range request (%d-%d) for resource (%8X, %ls) which has %d root pages and %d total pages.",
						StartPage, EndPage - 1,
			...
			else
			{
				...
				if (EndRange <= (uint32)Resources->PageRangeLookup.Num())
				{
					Resources->ForEachPage(
						...
						[this, RuntimeResourceID, Priority, VirtualPageRangeStart, &UpdatePagePriority, RootPageInfo, bVerify](uint32 PageIndex)
						{
							...
							{
								UE_LOGF(LogNaniteStreaming, Warning, "Invalid page request (%d) for resource (%8X, %ls) which has %d root pages and %d total pages.",
									PageIndex,
									RuntimeResourceID, GetNaniteResourceName(*RootPageInfo->Resources),
									RootPageInfo->NumRootPages, RootPageInfo->NumTotalPages);
							}
				...
				{
					UE_LOGF(LogNaniteStreaming, Warning, "Invalid page multi range request (%d-%d) for resource (%8X, %ls) which has %d page range lookups.",
...
void FStreamingManager::AsyncUpdate()
{
	...
	if (AsyncState.GPUStreamingRequestsPtr)
	{
				...
				for( const FPageKey& SelectedKey : SelectedPages )
				{
...
#endif
					{
						...
						if (!PendingPageStagingAllocator->TryAllocate(PageStreamingState.BulkSize, AllocatedOffset))
						{
							...
							// UE_LOGF(LogNaniteStreaming, Verbose, "This should be a rare event.");
							break;
						}
		...
		if (NumLegacyRequestsIssued > 0 && bUsingPakFiles)
		{
			...
			if (!bHasWarned)
			{
				UE_LOGF(LogNaniteStreaming, Warning, "PERFORMANCE WARNING: Nanite is issuing IO requests using the legacy IO path. Expect slower streaming and higher CPU overhead. "
...
void FStreamingManager::RequestNanitePages(TArray<uint32>&& InRequestStream)
{
	check(IsInRenderingThread());
	check(!AsyncState.bUpdateActive);
	if (GNaniteStreamingExplicitRequests)
	{
		if (PendingExplicitRequests.Num() == 0)
		{
			PendingExplicitRequests = MoveTemp(InRequestStream);
		}
		else
		{
			PendingExplicitRequests.Append(InRequestStream.GetData(), InRequestStream.Num());
		}
	}
}
...
#if WITH_EDITOR
uint64 FStreamingManager::GetRequestRecordBuffer(TMap<uint64, uint32>& OutRequestData)
{
	check(IsInRenderingThread());
	check(!AsyncState.bUpdateActive);
	
	OutRequestData.Reset();
	...
	OutRequestData.Reserve(PageRequestRecordMap.Num());
	...
	for (const TPair<FPageKey, uint32>& MapEntry : PageRequestRecordMap)
	{
		...
		if (Resources)
		{	
			const uint64 PageId = (((uint64)Resources->PersistentHash) << 32) | (uint64)MapEntry.Key.PageIndex;
			// One to one correspondance between OutRequestData and PageRequestRecordMap so can add without needing to combine.
			OutRequestData.Add(PageId, MapEntry.Value);
		}
	}
...
void FStreamingManager::PackRequestsToStream(TMap<uint64, uint32> const& InPageIdAndPriorities, TArray<uint32>& OutRequestStream)
{
	...
	{
		Requests.Add(FStreamingRequest{ FPageKey{ (uint32)(Entry.Key >> 32), (uint32)(Entry.Key) }, Entry.Value });
	}
	Requests.Sort();
	...
	// As there are often many requests per resource, this encoding can safe upwards of half of the total DWORDs.
	{
		...
		for (const FStreamingRequest& Request : Requests)
		{
			...
			{
				OutRequestStream[WriteIndex++] = Request.Key.RuntimeResourceID;
			}
			else
			{
				OutRequestStream[WriteIndex - 1] |= 1; // Mark resource repeat bit in previous packed dword
 			}
			...
			OutRequestStream[WriteIndex++] = Packed;
		}
...
void FStreamingManager::RequestDDCData(TConstArrayView<FCacheGetChunkRequest> DDCRequests)
{
	...
	GetCache().GetChunks(DDCRequests, *RequestOwner,
		[this](FCacheGetChunkResponse&& Response)
		{
			...
			{
				PendingPage.SharedBuffer.Reset();
			}
			PendingPage.DCCRequestCompleted.store(true, std::memory_order_release);
		});
}
