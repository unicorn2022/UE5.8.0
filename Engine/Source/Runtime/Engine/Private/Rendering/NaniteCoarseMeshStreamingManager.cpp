...
namespace Nanite
{
	...
	void FCoarseMeshStreamingManager::NotifyPrimitiveDetached(const IPrimitiveComponent* Primitive)
	{
		UnregisterComponent(Primitive);
	}

	void FCoarseMeshStreamingManager::NotifyPrimitiveUpdated(const IPrimitiveComponent* Primitive)
	{
		bool bCheckStaticMeshChanged = true;
		RegisterComponent(Primitive, bCheckStaticMeshChanged);
	}

	void FCoarseMeshStreamingManager::NotifyPrimitiveUpdated_Concurrent(const IPrimitiveComponent* Primitive)
	{
	...
	void FCoarseMeshStreamingManager::UnregisterRenderAsset(UStreamableRenderAsset* RenderAsset)
	{
		...
		TSet<const IPrimitiveComponent*>* RegisteredComponents = RegisteredComponentsMap.Find(RenderAsset);
		...
		if (RegisteredComponents)
		{
			...
			TSet<const IPrimitiveComponent*>* ComponentsToRelease = RequestReleaseComponentsMap.Find(RenderAsset);
			for (const IPrimitiveComponent* Component : *RegisteredComponents)
			{
				bool bRequestReleased = ComponentsToRelease && ComponentsToRelease->Contains(Component);
				if (!bRequestReleased)
				{
					Component->GetStreamingState().SetAttachedToCoarseMeshStreamingManager(false);
				}
			}
		}
	...
	void FCoarseMeshStreamingManager::ProcessNotifiedActor(const AActor* Actor, ENotifyMode NotifyMode)
	{
		...
		for (const UPrimitiveComponent* Primitive : Primitives)
		{
			...
			{
				bool bCheckStaticMeshChanged = true;
				RegisterComponent(Primitive->GetPrimitiveComponentInterface(), bCheckStaticMeshChanged);
			}
			else if (NotifyMode == ENotifyMode::Unregister)
			{
				UnregisterComponent(Primitive->GetPrimitiveComponentInterface());
			}
		}
	}

	void FCoarseMeshStreamingManager::RegisterComponent(const IPrimitiveComponent* Primitive, bool bCheckStaticMeshChanged)
	{		
		UStreamableRenderAsset* Asset = Primitive->GetStreamableNaniteAsset();
		...
		if (bCheckStaticMeshChanged && StreamingState.IsAttachedToCoarseMeshStreamingManager())
		{
		...
		if (StreamingState.IsAttachedToCoarseMeshStreamingManager())
		{
			return;
		}
		...
		TSet<const IPrimitiveComponent*>* ComponentsToRelease = RequestReleaseComponentsMap.Find(StaticMesh);
		if (ComponentsToRelease && ComponentsToRelease->Contains(Primitive))
		{
			ComponentsToRelease->Remove(Primitive);
			bNeedToAdd = false;
		}
		TSet<const IPrimitiveComponent*>& RegisteredComponents = RegisteredComponentsMap.FindOrAdd(StaticMesh);
		if (bNeedToAdd)
		{
		...
		// Mark tracked
		StreamingState.SetAttachedToCoarseMeshStreamingManager(true);
	}

	void FCoarseMeshStreamingManager::UnregisterComponent(const IPrimitiveComponent* Primitive)
	{
		UStreamableRenderAsset* Asset = Primitive->GetStreamableNaniteAsset();
		...
		if (!Primitive->GetStreamingState().IsAttachedToCoarseMeshStreamingManager())
		{
			return;
		}
	...
	void FCoarseMeshStreamingManager::UnregisterComponentInternal(const IPrimitiveComponent* Primitive, UStreamableRenderAsset* RenderAsset)
	{
		TSet<const IPrimitiveComponent*>& Components = RequestReleaseComponentsMap.FindOrAdd(RenderAsset);
		...
		// Mark untracked
		Primitive->GetStreamingState().SetAttachedToCoarseMeshStreamingManager(false);
	}
	...
	void FCoarseMeshStreamingManager::RequestUpdateCachedRenderState(const UStreamableRenderAsset* RenderAsset)
	{
		...
		TSet<const IPrimitiveComponent*>* Components = RegisteredComponentsMap.Find(RenderAsset);
		if (Components)
		{
			for (const IPrimitiveComponent* Component : *Components)
			{
	...
	void FCoarseMeshStreamingManager::UpdateResourceStreaming(float DeltaTime, bool bProcessEverything)
	{
		...
		for (auto Iter = RequestReleaseComponentsMap.CreateIterator(); Iter ; ++Iter)
		{
			...
			if (RegisteredComponents) 
			{
				TSet<const IPrimitiveComponent*>& ComponentsToRelease = Iter.Value();
				for (const IPrimitiveComponent* Component : ComponentsToRelease)
				{
