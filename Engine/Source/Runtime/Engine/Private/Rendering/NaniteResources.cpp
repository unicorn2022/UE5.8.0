...
DECLARE_LOG_CATEGORY_CLASS(LogNanite, Log, All);
...
static TAutoConsoleVariable<int32> CVarNaniteAllowMeshShaders(
	TEXT("r.Nanite.AllowMeshShaders"),
	1,
	TEXT("Whether to enable support for Nanite mesh shaders (if supported)"),
	ECVF_RenderThreadSafe | ECVF_ReadOnly);
...
static TAutoConsoleVariable<int32> CVarNaniteAllowTranslucency(
	TEXT("r.Nanite.AllowTranslucency"),
	0,
	TEXT("Whether to enable support for Nanite translucency (Heavy WIP / R&D)"),
	ECVF_RenderThreadSafe | ECVF_ReadOnly);
...
static TAutoConsoleVariable<int32> CVarNaniteAllowCurves(
	TEXT("r.Nanite.AllowCurves"),
	0,
	TEXT("Whether to enable support for Nanite curve rasterization"),
	ECVF_RenderThreadSafe | ECVF_ReadOnly);
...
FAutoConsoleVariableRef CVarNaniteProxyRenderMode(
	...
	TEXT(" 1: Disable rendering if Nanite is enabled on a mesh but is unsupported, except for static mesh editor toggle.")
	TEXT(" 2: Disable rendering if Nanite is enabled on a mesh but is unsupported.\n"),
	FConsoleVariableDelegate::CreateLambda([](IConsoleVariable* InVariable)
		{
			FGlobalComponentRecreateRenderStateContext Context;
		}),
...
static TAutoConsoleVariable<int32> CVarNaniteMaterialQualityDisableMask(
	TEXT("r.Nanite.MinMaterialQuality"),
	0,
	TEXT("Setting material quality below this value disables nanite support.\n")
	TEXT("Note that EMaterialQualityLevel isn't ordered by quality (High=1, Medium=2).\n")
	TEXT("But but for interpreting this cvar we will rearrage so that Medium=1 and High=2."),
...
{
	const ERayTracingMode RequestedMode = (ERayTracingMode)GNaniteRayTracingMode;
	ERayTracingMode Mode = RequestedMode;

	static ERayTracingMode PreviousRequestedMode = ERayTracingMode::Fallback;
	if (RequestedMode == ERayTracingMode::CLAS && !GRHIGlobals.RayTracing.SupportsClusterOps)
	{
		if (PreviousRequestedMode != RequestedMode)
		{
			UE_LOGF(LogNanite, Warning, "Unable to use ERayTracingMode::CLAS because device doesn't support ray tracing cluster operations.");
		}

		Mode = ERayTracingMode::Fallback;
	}
	PreviousRequestedMode = RequestedMode;
...
bool ShouldCookStreamablePagesAsOptional(const ITargetPlatform* TargetPlatform)
{
	if (TargetPlatform)
	{
		return GetDefault<URendererSettings>()->bNaniteStreamableBulkDataOptional.GetValueForPlatform(*TargetPlatform->IniPlatformName());
	}
...
FArchive& operator<<(FArchive& Ar, FPackedHierarchyNode& Node)
{
	for (uint32 i = 0; i < NANITE_MAX_BVH_NODE_FANOUT; i++)
	{
		...
		Ar << Node.Misc2[ i ].NumBones_BoneIndicesOffset;
	}
...
FResourcePrimitiveInfo::FResourcePrimitiveInfo(const FResources& Resources) :
	...
	AssemblyTransformCount(Resources.NumAssemblyTransforms)
{
}
...
void FResources::SerializeInternal(FArchive& Ar, UObject* Owner, bool bCooked)
{
	...
	{
		check(Ar.IsObjectReferenceCollector() || !HasStreamingData() || StreamablePages.GetBulkDataSize() > 0);

	#if WITH_EDITOR
		const bool bOptional = bCooked && Ar.IsSaving() && ShouldCookStreamablePagesAsOptional(Ar.CookingTarget());
		const uint32 OriginalBulkDataFlags = StreamablePages.GetBulkDataFlags();
		if (bOptional)
		{
			StreamablePages.SetBulkDataFlags(BULKDATA_OptionalPayload);
		}
	#endif
		...
		if (bOptional)
		{
			StreamablePages.ResetBulkDataFlags(OriginalBulkDataFlags);
		}
	#endif
	}
...
void FResources::GetResourceSizeEx(FResourceSizeEx& CumulativeResourceSize) const
{
	...
	CumulativeResourceSize.AddDedicatedSystemMemoryBytes(AssemblyTransforms.GetAllocatedSize());
	CumulativeResourceSize.AddDedicatedSystemMemoryBytes(AssemblyBoneAttachmentData.GetAllocatedSize());
	CumulativeResourceSize.AddDedicatedSystemMemoryBytes(PageRangeLookup.GetAllocatedSize());
	CumulativeResourceSize.AddDedicatedSystemMemoryBytes(BoneIndices.GetAllocatedSize());
}
...
{
	static const auto CVarNaniteMaxPixelsPerEdge = IConsoleManager::Get().FindTConsoleVariableDataFloat(TEXT("r.Nanite.MaxPixelsPerEdge"));
...
void FSceneProxyBase::OnMaterialsUpdated(bool bOverrideMaterialRelevance)
{
	...
	for (auto& MaterialSection : MaterialSections)
	{
		...
		if (!bOverrideMaterialRelevance)
		{
			...
			MaterialSection.bTranslucent = IsTranslucentBlendMode(ShadingMaterial->GetBlendMode());
		}
		...
		else if (bVertexProgrammableRaster || bPixelProgrammableRaster || MaterialSection.bTranslucent)
		{
		...
		bHasCurves |= MaterialSection.bHasCurves;
...
#endif
{
	...
	bHasRuntimeCardRepresentationData = false;
	...
	for (int32 SectionIndex = 0; SectionIndex < MeshSections.Num(); ++SectionIndex)
	{
		...
		if (ShadingMaterial == nullptr || ProxyDesc.UsePSOPrecacheFallbackMaterial())
		{
			UMaterialInterface* FallbackMaterial = (ProxyDesc.UsePSOPrecacheFallbackMaterial() && UPSOPrecacheSettingsManager::GetFallbackMaterial()) ? UPSOPrecacheSettingsManager::GetFallbackMaterial() : UMaterial::GetDefaultMaterial(MD_Surface);
			ShadingMaterial = MaterialSection.bHidden ? GEngine->NaniteHiddenSectionMaterial.Get() : FallbackMaterial;
		}

		MaterialSection.ShadingMaterialProxy = ShadingMaterial->GetRenderProxy();
		MaterialSection.bTranslucent = MaterialAudit.HasTranslucency(MaterialSection.MaterialIndex);
	}
	...
	if (ProxyDesc.ShouldCreateMaterialCacheProxy())
	{
		for (UMaterialCacheVirtualTexture* MaterialCacheTexture : ProxyDesc.MaterialCacheTextures)
		{
			if (FMaterialCacheVirtualTextureRenderProxy* RenderProxy = MaterialCacheTexture->CreateRenderProxy(
				ProxyDesc.MaterialCacheUVRegion,
				ProxyDesc.MaterialCacheTextureCoordinateIndex
			))
			{
...
FSceneProxy::~FSceneProxy()
{
	...
	if (bHasRuntimeCardRepresentationData)
	{
			...
			for (int32 InstanceIndex = 0; InstanceIndex < InstanceCount; InstanceIndex++)
			{
				...
				if ((bDrawSimpleCollision || bDrawSimpleWireframeCollision) && BodySetup)
				{
					if (FMath::Abs(InstanceToWorld.Determinant()) < UE_SMALL_NUMBER)
					{
						...
						//UE_LOGF(LogNanite, Log, "Zero scaling not supported (%ls)", *StaticMesh->GetPathName());
					}
					else
					{
...
{
	// TODO: check CVarRayTracingNaniteProxyMeshes here instead of during GetRayTracingPrimitiveFlags(...)
...
#if WITH_EDITOR
	if (RenderData->RayTracingProxy->PreviewLODLevel >= 0)
	{
...
void FSceneProxy::CreateDynamicRayTracingGeometries(FRHICommandListBase& RHICmdList)
{
	...
	{
		FRayTracingGeometryInitializer Initializer = RayTracingLODs[LODIndex].RayTracingGeometry->GetInitializer();
...
void FSceneProxy::GetDynamicRayTracingInstances_Internal(FRayTracingInstanceCollector& Collector, FRWBuffer* DynamicVertexBuffer, bool bUpdateRayTracingGeometry)
{
	...
	if (InstanceCount > 1)
	{
		...
		{
			bWarnOnce = false;
			UE_LOGF(LogStaticMesh, Warning, "Nanite instanced static mesh using World Position Offset not supported in ray tracing yet (%ls).", *StaticMesh->GetPathName());
		}

		return;
	}

	// Instance transforms are copied from GPUScene while building ray tracing instance buffer.
	...
	if (bUseDynamicGeometry && bUpdateRayTracingGeometry)
	{
		...
		UpdateParams.MeshBatchesView = CachedRayTracingMaterials;
		UpdateParams.NumVertices = NumVertices;
		UpdateParams.VertexBufferSize = NumVertices * (uint32)sizeof(FVector3f);
		UpdateParams.NumTriangles = DynamicRayTracingGeometry->GetInitializer().TotalPrimitiveCount;
		UpdateParams.Geometry = DynamicRayTracingGeometry;
		UpdateParams.Buffer = DynamicVertexBuffer;
...
ERayTracingPrimitiveFlags FSceneProxy::GetRayTracingPrimitiveFlags()
{
	if (bDynamicRayTracingGeometry)
	{
	...
	const ERayTracingMode NaniteRayTracingMode = GetRayTracingMode();
	const bool bIsRayTracingFarField = IsRayTracingFarField();

	// try and find the first valid RT geometry index
	const int32 ValidLODIndex = GetFirstValidRaytracingGeometryLODIndex(NaniteRayTracingMode);
	if (ValidLODIndex == INDEX_NONE)
	{
...
void FSceneProxy::GetStaticRayTracingInstances(FStaticRayTracingInstances& OutRayTracingInstances)
{
	const ERayTracingMode NaniteRayTracingMode = GetRayTracingMode();
	const bool bUsingNaniteRayTracing = NaniteRayTracingMode != ERayTracingMode::Fallback;
	...
	const FRayTracingGeometry* StaticRayTracingGeometry = nullptr;
	TArray<FMeshBatch> RayTracingMaterials;
	if (bUsingNaniteRayTracing)
	{
		StaticRayTracingGeometry = nullptr;

		SetupRayTracingMaterials(RayTracingMaterials);
	}
	else
	{
		FStaticMeshRayTracingProxyLODArray& RayTracingLODs = RenderData->RayTracingProxy->LODs;
		StaticRayTracingGeometry = RayTracingLODs[ValidLODIndex].RayTracingGeometry;

		SetupFallbackRayTracingMaterials(ValidLODIndex, RayTracingMaterials);
	}

	if (RayTracingMaterials.IsEmpty())
	{
		return; // don't output an instance if it has no materials
	}

	FRayTracingInstance& RayTracingInstance = StaticRayTracingInstancesLOD.Instances.AddDefaulted_GetRef();
	RayTracingInstance.Geometry = StaticRayTracingGeometry;
	RayTracingInstance.Materials = MoveTemp(RayTracingMaterials);
	// Instance transforms are copied from GPUScene while building ray tracing instance buffer.
...
#endif
{
	...
	bEvaluateWorldPositionOffset = InMeshDesc.EvaluateWorldPositionOffset();
	InstanceWPODisableDistance = static_cast<uint32>(FMath::Max(0, InMeshDesc.WorldPositionOffsetDisableDistance));
	...
	for (int32 SectionIndex = 0; SectionIndex < MeshSections.Num(); ++SectionIndex)
	{
		...
		//
		// The index used to fetch the shading material is kept in a local variable instead of updating 
		// MaterialSection.MaterialIndex, because MaterialSection.MaterialIndex needs to match the original 
		// material index baked into the cluster triangles.
		int32 UseMaterialIndex = MaterialSection.MaterialIndex;
		{
			if (SectionIndex < MeshInfo.LODMaterialMap.Num() && SkinnedAsset->IsValidMaterialIndex(MeshInfo.LODMaterialMap[SectionIndex]))
			{
				UseMaterialIndex = MeshInfo.LODMaterialMap[SectionIndex];
				UseMaterialIndex = FMath::Clamp(UseMaterialIndex, 0, SkinnedAsset->GetNumMaterials());
			}
		}
		...
		MaterialSection.bHidden = MeshObject->IsMaterialHidden(FirstLODIndex, UseMaterialIndex) || MeshSection.bDisabled;

		// If the material is NULL, or isn't flagged for use with skeletal meshes, it will be replaced by the default material.
		UMaterialInterface* ShadingMaterial = InMeshDesc.GetMaterial(UseMaterialIndex);
		...
		if (ShadingMaterial == nullptr || !bValidUsage || InMeshDesc.UsePSOPrecacheFallbackMaterial())
		{
			UMaterialInterface* FallbackMaterial = (InMeshDesc.UsePSOPrecacheFallbackMaterial() && UPSOPrecacheSettingsManager::GetFallbackMaterial()) ? UPSOPrecacheSettingsManager::GetFallbackMaterial() : UMaterial::GetDefaultMaterial(MD_Surface);
			ShadingMaterial = MaterialSection.bHidden ? GEngine->NaniteHiddenSectionMaterial.Get() : FallbackMaterial;
		}

		MaterialSection.ShadingMaterialProxy = ShadingMaterial->GetRenderProxy();

		MaterialSection.LocalUVDensities = MaterialAudit.GetLocalUVDensities(UseMaterialIndex);
...
void FSkinnedSceneProxy::GetDynamicRayTracingInstances(FRayTracingInstanceCollector& Collector)
{
	...
	check(LODData.RenderSections.Num() == RayTracingGeometry->GetInitializer().Segments.Num());
...
ERayTracingPrimitiveFlags FSkinnedSceneProxy::GetRayTracingPrimitiveFlags()
{
...
void FSkinnedSceneProxy::GetStaticRayTracingInstances(FStaticRayTracingInstances& OutRayTracingInstances)
{
	if (bDynamicRayTracingGeometry)
	{
		// Skip Nanite implementation and use base implementation instead
		Super::GetStaticRayTracingInstances(OutRayTracingInstances);
		return;
	}
	...
	else
	{
		...
		StaticRayTracingGeometry = &RenderData->LODRenderData[LODIndex].StaticRayTracingGeometry;

		SetupFallbackRayTracingMaterials(LODIndex, /*bUseStaticRayTracingGeometry*/ true, /*bWillCacheInstance*/ true, RayTracingMaterials);
	}
...
bool FSkinnedSceneProxy::GetInstanceWorldPositionOffsetDisableDistance(float& OutWPODisableDistance) const
{
	OutWPODisableDistance = static_cast<float>(InstanceWPODisableDistance);
	return InstanceWPODisableDistance != 0;
}

void FSkinnedSceneProxy::SetWorldPositionOffsetDisableDistance_GameThread(int32 NewValue)
{
	const uint32 ClampedNewValue = static_cast<uint32>(FMath::Max(0, NewValue));
	ENQUEUE_RENDER_COMMAND(CmdSetSkinnedNaniteWPODisableDistance)(
		[this, ClampedNewValue](FRHICommandList&)
		{
			const bool bUpdatePrimitiveData = InstanceWPODisableDistance != ClampedNewValue;
			const bool bUpdateDrawCmds = bUpdatePrimitiveData && (InstanceWPODisableDistance == 0 || ClampedNewValue == 0);

			if (bUpdatePrimitiveData)
			{
				InstanceWPODisableDistance = ClampedNewValue;
				GetScene().RequestUniformBufferUpdate(*GetPrimitiveSceneInfo());
				GetScene().RequestGPUSceneUpdate(*GetPrimitiveSceneInfo(), EPrimitiveDirtyState::ChangedOther);
				if (bUpdateDrawCmds)
				{
...
{
	static const bool bAllowTranslucent = CVarNaniteAllowTranslucency.GetValueOnAnyThread() != 0;
	return IsOpaqueOrMaskedBlendMode(BlendMode) ||
		(bAllowTranslucent && IsTranslucentOnlyBlendMode(BlendMode)); // GW-TODO: IsTranslucentBlendMode support? (Additive, Modulate, etc?)
}
...
void FNaniteVertexFactory::ModifyCompilationEnvironment(const FVertexFactoryShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment)
{
	...
	const bool bIsRayTracingHitGroup = Parameters.ShaderType->GetFrequency() == SF_RayHitGroup;
	const bool bUseNaniteUniformBuffers = !bIsRayTracingHitGroup;
	...
	const bool bBasepass = FCString::Stristr(Parameters.ShaderType->GetShaderFilename(), TEXT("BasePass")) != nullptr; // TODO: Only basepass for now
	const bool bBindless = UseNaniteBindlessShading(Parameters.Platform) && bBasepass;
	...
	OutEnvironment.SetDefine(TEXT("NANITE_RAY_TRACING"), bIsRayTracingHitGroup);
...
void Nanite::FSceneProxyBase::GetStreamableRenderAssetInfo(const FBoxSphereBounds& InPrimitiveBounds, TArray<FStreamingRenderAssetPrimitiveInfo>& OutStreamableRenderAssets) const
{
	...
	constexpr float ComponentScaling = 1.0f;
	const EMaterialQualityLevel::Type QualityLevel = GetCurrentMaterialQualityLevelChecked();

	for (const FMaterialSection& Section : MaterialSections)
	{
		// Precache / in-flight proxies can still have a null ShadingMaterialProxy before ResetToDefaultMaterial
		// or ShadingMaterial->GetRenderProxy() has populated the section. Skip defensively rather than
		// deref-crash. The outer UMaterialInterface null check below also tolerates GetMaterialInterface()
		// returning null once we have a proxy.
		...
		if (const UMaterialInterface* ShadingMaterial = Section.ShadingMaterialProxy->GetMaterialInterface())
		{
			...
			if (TSharedPtr<const FMaterialCachedTexturesSamplingInfo> CachedSamplingInfo = ShadingMaterial->GetCachedTexturesSamplingInfo())
			{
				CachedSamplingInfo->GetStreamingRenderAssetPrimitiveInfo(InPrimitiveBounds, ComponentScaling, UVChannelData, QualityLevel, OutStreamableRenderAssets);
			}
			else
			{
				// Unexpected - Cache unavailable
				// Skip this material, the streamer won't see this material's textures.
				ensureMsgf(IsRunningCookCommandlet(), TEXT("No cached texture sampling info for material %s; skipping its textures in the streaming report."), *ShadingMaterial->GetName());
			}
		}
	}
}
...
TMulticastDelegate<void(const Nanite::FSkinnedSceneProxy*)> Nanite::FSkinnedSceneProxyDelegates::OnCreateRenderThreadResources;
TMulticastDelegate<void(const Nanite::FSkinnedSceneProxy*)> Nanite::FSkinnedSceneProxyDelegates::OnDestroyRenderThreadResources;
