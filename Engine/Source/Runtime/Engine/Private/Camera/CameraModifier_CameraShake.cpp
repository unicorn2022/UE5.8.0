	...
	if( ActiveShakes.Num() > 0 )
	{
		...
		for (int32 i = ActiveShakes.Num() - 1; i >= 0; i--)
		{
			...
			if (ShakeInfo.ShakeInstance == nullptr || ShakeInfo.ShakeInstance->IsFinished() || ShakeInfo.ShakeSource.IsStale())
			{
				...
				UE_LOGF(LogCameraShake, Verbose, "UCameraModifier_CameraShake::ModifyCamera Removing obsolete shake %ls", *GetNameSafe(ShakeInfo.ShakeInstance));
...
UCameraShakeBase* UCameraModifier_CameraShake::AddCameraShake(TSubclassOf<UCameraShakeBase> ShakeClass, const FAddCameraShakeParams& Params)
{
	...
	UE_LOGF(LogCameraShake, Verbose, "UCameraModifier_CameraShake::AddCameraShake %ls", *GetNameSafe(ShakeClass));
	...
	if (ShakeClass != nullptr)
	{
		...
		if (NewInst)
		{
			...
			if (bReplacedNull == false)
			{
				...
				UE_LOGF(LogCameraShake, Verbose, "UCameraModifier_CameraShake::AddCameraShake %ls Active Instance Added", *GetNameSafe(ShakeInfo.ShakeInstance));
	...
	for (int32 i = 0; i < ActiveShakes.Num(); ++i)
	{
		...
		if (ShakeInfo.ShakeInstance == ShakeInst)
		{
			...
			if (bImmediately)
			{
				...
				SaveShakeInExpiredPoolIfPossible(ShakeInfo);
				ActiveShakes.RemoveAt(i, 1);
				UE_LOGF(LogCameraShake, Verbose, "UCameraModifier_CameraShake::RemoveCameraShake %ls", *GetNameSafe(ShakeInfo.ShakeInstance));
			}
			break;
		}
	}
	...
	for (int32 i = ActiveShakes.Num()- 1; i >= 0; --i)
	{
		...
		if (ShakeInst != nullptr && (ShakeInst->GetClass()->IsChildOf(ShakeClass)))
		{
			...
			if (bImmediately)
			{
				...
				SaveShakeInExpiredPoolIfPossible(ShakeInfo);
				ActiveShakes.RemoveAt(i, 1);
				UE_LOGF(LogCameraShake, Verbose, "UCameraModifier_CameraShake::RemoveAllCameraShakesOfClass %ls", *GetNameSafe(ShakeInfo.ShakeInstance));
			}
		}
	}
}
	...
	for (int32 i = ActiveShakes.Num() - 1; i >= 0; --i)
	{
		...
		if (ShakeInfo.ShakeSource.Get() == SourceComponent && ShakeInfo.ShakeInstance != nullptr)
		{
			...
			if (bImmediately)
			{
				...
				SaveShakeInExpiredPoolIfPossible(ShakeInfo);
				ActiveShakes.RemoveAt(i, 1);
				UE_LOGF(LogCameraShake, Verbose, "UCameraModifier_CameraShake::RemoveAllCameraShakesFromSource %ls", *GetNameSafe(ShakeInfo.ShakeInstance));
			}
		}
	}
}
	...
	for (int32 i = ActiveShakes.Num() - 1; i >= 0; --i)
	{
				...
				ShakeInfo.ShakeInstance->GetClass()->IsChildOf(ShakeClass))
		{
			...
			if (bImmediately)
			{
				...
				SaveShakeInExpiredPoolIfPossible(ShakeInfo);
				ActiveShakes.RemoveAt(i, 1);
				UE_LOGF(LogCameraShake, Verbose, "UCameraModifier_CameraShake::RemoveAllCameraShakesOfClassFromSource %ls", *GetNameSafe(ShakeInfo.ShakeInstance));
			}
		}
	}
}
