	...
	: ActiveImpl(nullptr)
{
	...
	{
		UE_LOGF(LogCameraPhotography, Log, "Photography camera created.  Provider=%ls, Supported=%d", ActiveImpl->GetProviderName(), ActiveImpl->IsSupported());
...
FCameraPhotographyManager::~FCameraPhotographyManager()
{
	if (ActiveImpl.IsValid())
	{
		UE_LOGF(LogCameraPhotography, Log, "Photography camera destroyed.  Provider=%ls, Supported=%d", ActiveImpl->GetProviderName(), ActiveImpl->IsSupported());		
		ActiveImpl.Reset();
	}	
}
