				...
				for (FConfigSection::TConstIterator It(*MaterialSection); It; ++It)
				{
					...
					if( FParse::Value( *It.Value().GetValue(), TEXT("Material="), MaterialName, true ) )
					{
						...
						if (Material)
						{
							...
							bool bEnableOnMobile = true;
							FParse::Bool(*It.Value().GetValue(), TEXT("EnableOnMobile="), bEnableOnMobile);
							...
							Rec.bEnableOnMobile = bEnableOnMobile;
...
bool FBufferVisualizationData::GetMaterialEnableOnMobile(FName InMaterialName) const
{
	if (const Record* Result = GetRecord(InMaterialName))
	{
		return Result->bEnableOnMobile;
	}
	else
	{
		return false;
