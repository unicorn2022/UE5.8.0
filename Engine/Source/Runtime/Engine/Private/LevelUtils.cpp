...
bool FLevelUtils::IsLevelVisible(const ULevel* Level)
{
	...
	static const FName NAME_TransLevelMoveBuffer("TransLevelMoveBuffer");
...
void FLevelUtils::ApplyLevelTransform(const FLevelUtils::FApplyLevelTransformParams& TransformParams)
{
	...
	if (bTransformActors)
	{
		...
		const auto ApplyLevelTransformToCompDirect = [&TransformParams](AActor* Actor, USceneComponent* Comp)
		{
			if (Comp->GetAttachParent() == nullptr)
			{
				Comp->SetRelativeLocation_Direct(TransformParams.LevelTransform.TransformPosition(Comp->GetRelativeLocation()));
				Comp->SetRelativeRotation_Direct(TransformParams.LevelTransform.TransformRotation(Comp->GetRelativeRotation().Quaternion()).Rotator());
				Comp->SetRelativeScale3D_Direct(TransformParams.LevelTransform.GetScale3D() * Comp->GetRelativeScale3D());
			}
			else if (Comp->IsUsingAbsoluteLocation() || Comp->IsUsingAbsoluteRotation() || Comp->IsUsingAbsoluteScale())
			{
		...
		const auto ApplyLevelTransformToCompRelative = [&TransformParams](AActor* Actor, USceneComponent* Comp)
		{
			if (Comp->GetAttachParent() == nullptr)
			{
				Comp->SetRelativeLocationAndRotation(TransformParams.LevelTransform.TransformPosition(Comp->GetRelativeLocation()), TransformParams.LevelTransform.TransformRotation(Comp->GetRelativeRotation().Quaternion()));
				Comp->SetRelativeScale3D(TransformParams.LevelTransform.GetScale3D() * Comp->GetRelativeScale3D());
			}
			else if (Comp->IsUsingAbsoluteLocation() || Comp->IsUsingAbsoluteRotation() || Comp->IsUsingAbsoluteScale())
			{
				if (Comp->IsUsingAbsoluteLocation() || Comp->IsUsingAbsoluteRotation())
				{
					const FVector NewLoc = Comp->IsUsingAbsoluteLocation()
						? TransformParams.LevelTransform.TransformPosition(Comp->GetRelativeLocation())
						: Comp->GetRelativeLocation();
					const FQuat NewRot = Comp->IsUsingAbsoluteRotation()
						? TransformParams.LevelTransform.TransformRotation(Comp->GetRelativeRotation().Quaternion())
		...
		if (TransformParams.Actor)
		{
			...
			{
				if (TransformParams.Actor->GetRootComponent())
				{
					TInlineComponentArray<USceneComponent*> Components;
					TransformParams.Actor->GetComponents(Components);
					for (USceneComponent* Comp : Components)
					{
						ApplyLevelTransformToCompDirect(TransformParams.Actor, Comp);
					}
				}
			}
			else
			{
				if (TransformParams.Actor->GetRootComponent())
				{
					...
					for (USceneComponent* Comp : Components)
					{
						ApplyLevelTransformToCompRelative(TransformParams.Actor, Comp);
					}
				}
			}
#if WITH_EDITOR
			...
			for (AActor* Actor : TransformParams.Level->Actors)
			{
				if (Actor)
				{
					if (Actor->GetRootComponent())
					{
						...
						for (USceneComponent* Comp : Components)
						{
							ApplyLevelTransformToCompDirect(Actor, Comp);
						}
					}
				}
			}
			...
			for (AActor* Actor : TransformParams.Level->Actors)
			{
				if (Actor)
				{
					if (Actor->GetRootComponent())
					{
						...
						for (USceneComponent* Comp : Components)
						{
							ApplyLevelTransformToCompRelative(Actor, Comp);
						}
					}
				}
			}
