...
void FStaticMeshSceneProxyDesc::InitializeFromStaticMeshComponent(const UStaticMeshComponent* InComponent)
{
	...
	MaterialCacheUVRegion = InComponent->MaterialCacheUVRegion;
	MaterialCacheTextureCoordinateIndex = InComponent->MaterialCacheUVCoordinateIndex;
}
...
{
	return !MaterialCacheTextures.IsEmpty();
}
...
{
	// Prefer the GT-resolved cache when available. Callers that build the desc on a thread
	// where UTexture::GetResource() is not safe (e.g. FastGeo's cooked async render-state
	// worker) populate this on the game thread before dispatching the worker.
	if (CachedMeshPaintTextureResource)
	{
		return CachedMeshPaintTextureResource;
	}
