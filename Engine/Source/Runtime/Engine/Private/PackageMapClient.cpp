...
bool UPackageMapClient::SerializeObject( FArchive& Ar, UClass* Class, UObject*& Object, FNetworkGUID *OutNetGUID)
{
	...
	{
		UE_LOGF(LogNetPackageMap, Log, "Serialized Object %ls", *Object->GetName());
	}
#endif
	...
	if (Ar.IsSaving())
	{
		...
		if (!NetGUID.IsDefault() && Object && ShouldSendFullPath(Object, NetGUID))
		{
			...
			{
				UE_LOGF( LogNetPackageMap, Verbose, "Failed to export in ::SerializeObject %ls", *Object->GetName() );
			}
		}
	...
	else if (Ar.IsLoading())
	{
		...
		double LoadTime = 0.0;
		{
			...
			{
				Ar.SetError();
				UE_LOGF(LogNetPackageMap, Warning, "UPackageMapClient::SerializeObject: InternalAck GUID broken.");
				return false;
			}
#endif
			...
			{
				UE_LOGF(LogNetPackageMap, Warning, "Using None instead of replicated reference to %ls because the level it's in has not been made visible", *Object->GetFullName());
				Object = NULL;
			}
			...
			{
				UE_LOGF(LogNetPackageMap, Warning, "Forged object: got %ls, expecting %ls", *Object->GetFullName(), *Class->GetFullName());
				Object = NULL;
			}
			...
			UE_LOGF(LogNetPackageMap, VeryVerbose, "UPackageMapClient::SerializeObject Serialized Object %ls as <%ls>", Object ? *Object->GetPathName() : TEXT("NULL"), *NetGUID.ToString());
		}
		...
		{
			UE_LOGF(LogNetPackageMap, Warning, "Long net serialize: %fms, Serialized Object %ls", (float)LoadTime * 1000.0f, *GetNameSafe(Object));
		}
#endif
...
bool UPackageMapClient::WriteObject( FArchive& Ar, UObject* ObjOuter, FNetworkGUID NetGUID, FString ObjName )
{
	...
	UE_LOGF(LogNetPackageMap, Log, "WroteObject %ls NetGUID <%ls>", *ObjName, *NetGUID.ToString() );

	if (NetGUID.IsStatic() && !NetGUID.IsDefault() && !NetGUIDHasBeenAckd(NetGUID))
	{
		if ( !ExportNetGUID( NetGUID, NULL, ObjName, ObjOuter ) )
		{
			UE_LOGF( LogNetPackageMap, Warning, "Failed to export in ::WriteObject %ls", *ObjName );
		}
	}
...
bool UPackageMapClient::SerializeNewActor(FArchive& Ar, class UActorChannel *Channel, class AActor*& Actor)
{
	...
	UE_LOGF( LogNetPackageMap, VeryVerbose, "SerializeNewActor START" );
...
#if UE_WITH_REMOTE_OBJECT_HANDLE
	UE_LOGF(LogNetPackageMapRemoteObject, Log, "UPackageMapClient::SerializeNewActor: Actor=%ls NetGUID=%ls",
		   *GetNameSafe(NewObj), *NetGUID.ToString());
#endif

	if ( Ar.IsError() )
	{
		UE_LOGF( LogNetPackageMap, Error, "UPackageMapClient::SerializeNewActor: Ar.IsError after SerializeObject 1" );
		return false;
	}
	...
	if ( Ar.AtEnd() && NetGUID.IsDynamic() )
	{
		...
		{
			UE_LOGF( LogNetPackageMap, Error, "UPackageMapClient::SerializeNewActor: bIsClosingChannel == 0 : %ls [%ls]", *GetNameSafe(Actor), *NetGUID.ToString() );
			Ar.SetError();
			return false;
		}

		UE_LOGF( LogNetPackageMap, Log, "UPackageMapClient::SerializeNewActor:  Skipping full read because we are deleting dynamic actor: %ls", *GetNameSafe(Actor) );
	...
	if ( NetGUID.IsDynamic() )
	{
		...
		if ( ArchetypeNetGUID.IsValid() && Archetype == NULL )
		{
			...
			{
				UE_LOGF( LogNetPackageMap, Error, "UPackageMapClient::SerializeNewActor. Unresolved Archetype GUID. Path: %ls, NetGUID: %ls.", *ExistingCacheObjectPtr->PathName.ToString(), *ArchetypeNetGUID.ToString() );
			}
			else
			{
				UE_LOGF( LogNetPackageMap, Error, "UPackageMapClient::SerializeNewActor. Unresolved Archetype GUID. Guid not registered! NetGUID: %ls.", *ArchetypeNetGUID.ToString() );
			}
		}
			...
			if ( Actor == NULL )
			{
				if ( Archetype )
				{
					...
					if (SpawnLevel == nullptr || SpawnLevel->GetWorld() != nullptr)
					{
						...
						{
							UE_LOGF(LogNetPackageMap, Warning, "SerializeNewActor: Failed to spawn actor for NetGUID: %ls, Channel: %d", *NetGUID.ToString(), Channel->ChIndex);
						}
					}
					else
					{
						UE_LOGF(LogNetPackageMap, Log, "SerializeNewActor: Actor level has invalid world (may be streamed out). NetGUID: %ls, Channel: %d", *NetGUID.ToString(), Channel->ChIndex);
					}
				}
				else
				{
					UE_LOGF(LogNetPackageMap, Error, "UPackageMapClient::SerializeNewActor Unable to read Archetype for NetGUID %ls / %ls", *NetGUID.ToString(), *ArchetypeNetGUID.ToString() );
				}
			}
		}
	}
	else if ( Ar.IsLoading() && Actor == NULL )
	{
		...
		UE_CLOGF(!Connection->IsReplay(), LogNetPackageMap, Log, "SerializeNewActor: Failed to find static actor: FullNetGuidPath: %ls, Channel: %d", *GuidCache->FullNetGUIDPath(NetGUID), Channel->ChIndex);
	...
	UE_LOGF( LogNetPackageMap, Log, "SerializeNewActor END: Finished Serializing. Actor: %ls, FullNetGUIDPath: %ls, Channel: %d, IsLoading: %i, IsDynamic: %i", Actor ? *Actor->GetName() : TEXT("NULL"), *GuidCache->FullNetGUIDPath( NetGUID ), Channel->ChIndex, (int)Ar.IsLoading(), (int)NetGUID.IsDynamic() );
	...
	const FExportFlags &	ExportFlags )
{
	...
	if ( CacheObject != NULL )
	{
		...
		{
			UE_LOGF( LogNetPackageMap, Warning, "SanityCheckExport: CacheObject->OuterGUID != ExpectedOuterGUID. NetGUID: %ls, Object: %ls, Expected: %ls", *NetGUID.ToString(), *Object->GetPathName(), *ExpectedPathName );
		}
	}
	else
	{
		UE_LOGF( LogNetPackageMap, Warning, "SanityCheckExport: CacheObject == NULL. NetGUID: %ls, Object: %ls, Expected: %ls", *NetGUID.ToString(), *Object->GetPathName(), *ExpectedPathName );
	}

	if ( Object->GetName() != ExpectedPathName )
	{
		UE_LOGF( LogNetPackageMap, Warning, "SanityCheckExport: Name mismatch. NetGUID: %ls, Object: %ls, Expected: %ls", *NetGUID.ToString(), *Object->GetPathName(), *ExpectedPathName );
	}
	...
	if ( Object->GetOuter() != ExpectedOuter )
	{
		...
		UE_LOGF( LogNetPackageMap, Warning, "SanityCheckExport: Outer mismatch. Object: %ls, NetGUID: %ls, Current: %ls, Expected: %ls", *Object->GetPathName(), *NetGUID.ToString(), *CurrentOuterName, *ExpectedOuterName );
	}
	...
	{
		UE_LOGF( LogNetPackageMap, Warning, "SanityCheckExport: Package type mismatch. Object:%ls, NetGUID: %ls", *Object->GetPathName(), *NetGUID.ToString() );
	}
}
...
FNetworkGUID UPackageMapClient::InternalLoadObject( FArchive & Ar, UObject *& Object, const int32 InternalLoadObjectRecursionCount )
{
	...
	{
		UE_LOGF( LogNetPackageMap, Warning, "InternalLoadObject: Hit recursion limit." );
		Ar.SetError(); 
		Object = NULL;
		return FNetworkGUID(); 
	} 
...
#if UE_WITH_REMOTE_OBJECT_HANDLE
		UE_LOGF(LogNetPackageMapRemoteObject, Log, "Looking up NetGUID %ls in the local cache and found '%ls'", *NetGUID.ToString(), *GetNameSafe(Object));

		if (Object == nullptr)
		{
			FRemoteObjectId RemoteObjectId = FNetGUIDCache::GetRemoteObjectIdFromNetworkGUID(NetGUID);
			if (UE::RemoteObject::Handle::GetResidence(RemoteObjectId) != EResidence::Local)
			{
				if (AutoRTFM::IsTransactional())
				{
					UE_LOGF(LogNetPackageMapRemoteObject, Log, "Dereferencing a remote NetGUID %ls (%ls).", *NetGUID.ToString(), *RemoteObjectId.ToString());
					TObjectPtr<UObject> RemoteObjectPtr = TObjectPtr<UObject>(FObjectPtr(RemoteObjectId));
					Object = RemoteObjectPtr.Get();
				}
				else
				{
					UE_LOGF(LogNetPackageMapRemoteObject, Log, "Dereferencing a remote NetGUID %ls (%ls) that is outside a transaction.", *NetGUID.ToString(), *RemoteObjectId.ToString());
				}
			}
		}
#endif

		UE_LOGF(LogNetPackageMap, VeryVerbose, "InternalLoadObject loaded %ls from NetGUID <%ls>", Object ? *Object->GetFullName() : TEXT( "NULL" ), *NetGUID.ToString() );
	}
	...
	if ( ExportFlags.bHasPath )
	{
		...
		if ( ExportFlags.bHasNetworkChecksum )
		{
			...
			UE_LOGF(LogNetPackageMap, Verbose, "%ls has network checksum %u", *ObjectName, NetworkChecksum);
		}
		...
		if ( Ar.IsError() )
		{
			UE_LOGF( LogNetPackageMap, Error, "InternalLoadObject: Failed to load path name" );
			Object = NULL;
			return NetGUID;
		}
		...
		if (NetGUID.IsDefault())
		{
			...
			{
				UE_LOGF( LogNetPackageMap, Log, "InternalLoadObject: couldn't find outer for non-package object. GUID: %ls, ObjectName: %ls", *NetGUID.ToString(), *ObjectName );
				Object = nullptr;
				return NetGUID;
			}
			...
			if ( Object == NULL )
			{
				UE_LOGF( LogNetPackageMap, Warning, "UPackageMapClient::InternalLoadObject: Unable to resolve default guid from client: ObjectName: %ls, ObjOuter: %ls ", *ObjectName, ObjOuter != NULL ? *ObjOuter->GetPathName() : TEXT( "NULL" ) );
				return NetGUID;
			}

			if (!IsValid(Object))
			{
				UE_LOGF( LogNetPackageMap, Warning, "UPackageMapClient::InternalLoadObject: Received reference to invalid object from client: ObjectName: %ls, ObjOuter: %ls ", *ObjectName, ObjOuter != NULL ? *ObjOuter->GetPathName() : TEXT( "NULL" ) );
				Object = NULL;
				return NetGUID;
			}
			...
			if ( NetworkChecksum != 0 && GuidCache->NetworkChecksumMode == FNetGUIDCache::ENetworkChecksumMode::SaveAndUse && !CVarIgnoreNetworkChecksumMismatch.GetValueOnAnyThread() )
			{
				...
				if (CompareNetworkChecksum != NetworkChecksum )
				{
					...
					UE_LOGF( LogNetPackageMap, Error, "%ls", *ErrorStr);
					Object = NULL;
			...
			if ( bIsPackage )
			{
				...
				if ( Package == NULL )
				{
					UE_LOGF( LogNetPackageMap, Error, "UPackageMapClient::InternalLoadObject: Default object not a package from client: ObjectName: %ls, ObjOuter: %ls ", *ObjectName, ObjOuter != NULL ? *ObjOuter->GetPathName() : TEXT( "NULL" ) );
					Object = NULL;
					return NetGUID;
				}
			}
		...
		{
			UE_LOGF( LogNetPackageMap, Warning, "UPackageMapClient::InternalLoadObject: Server could not resolve non default guid from client. ObjectName: %ls, ObjOuter: %ls ", *ObjectName, ObjOuter != NULL ? *ObjOuter->GetPathName() : TEXT( "NULL" ) );
			return NetGUID;
		}
		...
		{
			UE_LOGF( LogNetPackageMap, Warning, "InternalLoadObject: Unable to resolve object from path. Path: %ls, Outer: %ls, NetGUID: %ls", *ObjectName, ObjOuter ? *ObjOuter->GetPathName() : TEXT( "NULL" ), *NetGUID.ToString() );
		}
	}
	else if ( Object == NULL && !GuidCache->ShouldIgnoreWhenMissing( NetGUID ) )
	{
		UE_LOGF( LogNetPackageMap, Warning, "InternalLoadObject: Unable to resolve object. FullNetGUIDPath: %ls", *GuidCache->FullNetGUIDPath( NetGUID ) );
	}
...
bool UPackageMapClient::ExportNetGUID( FNetworkGUID NetGUID, UObject* Object, FString PathName, UObject* ObjOuter )
{
	...
	for ( int32 NumTries = 0; NumTries < 2; NumTries++ )
	{
		...
		{
			UE_LOGF( LogNetPackageMap, Fatal, "ExportNetGUID - CurrentExportNetGUIDs.Num() != 0 (%ls).", Object ? *Object->GetName() : *PathName );
			return false;
		}
		...
		if ( CurrentExportNetGUIDs.Num() == 0 )
		{
			...
			UE_LOGF( LogNetPackageMap, Warning, "ExportNetGUID - InternalWriteObject no GUID's were exported: %ls ", Object ? *Object->GetName() : *PathName );
		...
		if ( ExportNetGUIDCount == 0 || NumTries == 1 )
		{
			...
			UE_LOGF( LogNetPackageMap, Fatal, "ExportNetGUID - Failed to serialize NetGUID into single bunch. (%ls)", Object ? *Object->GetName() : *PathName  );
			return false;
		}
...
void UPackageMapClient::ExportNetGUIDHeader()
{
	...
	{
		UE_LOGF(LogNetPackageMap, Verbose, "ExportNetGUIDHeader: ");
		for (auto It = CurrentExportBunch->ExportNetGUIDs.CreateIterator(); It; ++It)
		{
			UE_LOGF(LogNetPackageMap, Verbose, "  NetGUID: %ls", *It->ToString());
		}
	}
	...
	{
		UE_LOGF(LogNetPackageMap, Warning, "Attempted to export a NetGUID Bunch with no NetGUIDs!");
	}
...
void UPackageMapClient::ReceiveNetGUIDBunch( FInBunch &InBunch )
{
	...
	{
		UE_LOGF( LogNetPackageMap, Error, "UPackageMapClient::ReceiveNetGUIDBunch: NumGUIDsInBunch > MaxSerializedNetGuids (%d / %d)", NumGUIDsInBunch, UE::Net::MaxSerializedNetGuids);
		InBunch.SetError();
		return;
	}
	...
	UE_LOGF(LogNetPackageMap, Log, "UPackageMapClient::ReceiveNetGUIDBunch %d NetGUIDs. PacketId %d. ChSequence %d. ChIndex %d", NumGUIDsInBunch, InBunch.PacketId, InBunch.ChSequence, InBunch.ChIndex );
	...
	while( NumGUIDsRead < NumGUIDsInBunch )
	{
		...
		if ( InBunch.IsError() )
		{
			UE_LOGF( LogNetPackageMap, Error, "UPackageMapClient::ReceiveNetGUIDBunch: InBunch.IsError() after InternalLoadObject" );
			return;
		}
		NumGUIDsRead++;
	}
...
void UPackageMapClient::SerializeNetFieldExportGroupMap(FArchive& Ar, bool bClearPendingExports)
{
	...
	else
	{
		...
		if (Ar.IsError())
		{
			UE_LOGF(LogNetPackageMap, Error, "UPackageMapClient::SerializeNetFieldExportGroupMap - Archive error while reading NumNetFieldExportGroups");
			return;
		}
		...
		{
			Ar.SetError();
			UE_LOGF(LogNetPackageMap, Error, "UPackageMapClient::SerializeNetFieldExportGroupMap - NumNetFieldExportGroups exceeds MaxSerializedNetExportGroups (%u / %d)", NumNetFieldExportGroups, UE::Net::MaxSerializedNetExportGroups);
			return;
		}
		...
		for (uint32 i = 0; i < NumNetFieldExportGroups; ++i)
		{
			...
			if (Ar.IsError())
			{
				UE_LOGF(LogNetPackageMap, Error, "UPackageMapClient::SerializeNetFieldExportGroupMap - Archive error while loading FNetFieldExportGroup, Index: %u", i);
				return;
			}
...
void UPackageMapClient::ReceiveNetFieldExportsCompat(FInBunch &InBunch)
{
	if (!Connection->IsInternalAck())
	{
		UE_LOGF(LogNetPackageMap, Error, "ReceiveNetFieldExportsCompat: connection is not a replay connection.");
		InBunch.SetError();
		return;
	}
	...
	{
		UE_LOGF(LogNetPackageMap, Error, "UPackageMapClient::ReceiveNetFieldExportsCompat - NumExportGroups exceeds MaxSerializedNetExportGroups (%u / %d)", NumExportGroups, UE::Net::MaxSerializedNetExportGroups);
		InBunch.SetError();
		return;
	}
	...
	for (int32 i = 0; i < (int32)NumExportGroups; i++)
	{
		...
		if (InBunch.IsError())
		{
			UE_LOGF(LogNetPackageMap, Error, "UPackageMapClient::ReceiveNetFieldExportsCompat - Error serializing export path index.");
			return;
		}
		...
		if (InBunch.ReadBit() == 1)
		{
			...
			if (InBunch.IsError())
			{
				UE_LOGF(LogNetPackageMap, Error, "UPackageMapClient::ReceiveNetFieldExportsCompat - Error serializing export path.");
				return;
			}
			...
			{
				UE_LOGF(LogNetPackageMap, Error, "UPackageMapClient::ReceiveNetFieldExportsCompat - NumExportsInGroup exceeds MaxSerializedNetExportsPerGroup (%d / %d)", NumExportsInGroup, UE::Net::MaxSerializedNetExportsPerGroup);
				InBunch.SetError();
				return;
			}
		...
		{
			UE_LOGF(LogNetPackageMap, Error, "ReceiveNetFieldExports: Invalid NetFieldExport Handle '%i', Max '%i'.",
...
void UPackageMapClient::ReceiveNetFieldExports(FArchive& Archive)
{
	...
	{
		UE_LOGF(LogNetPackageMap, Error, "UPackageMapClient::ReceiveNetFieldExports - NumNetExports exceeds MaxSerializedNetExportGroups (%u / %d)", NumNetExports, UE::Net::MaxSerializedNetExportGroups);
		Archive.SetError();
		return;
	}
	...
	for (int32 i = 0; i < (int32)NumNetExports; i++)
	{
		...
		if (!!WasExported)
		{
			...
			{
				UE_LOGF(LogNetPackageMap, Warning, "UPackageMapClient::ReceiveNetFieldExports - NumExportsInGroup exceeds MaxSerializedNetExportsPerGroup (%u / %d)", NumExportsInGroup, UE::Net::MaxSerializedNetExportsPerGroup);
				Archive.SetError();
				return;
			}
		...
		if (NetFieldExportGroup)
		{
			...
			{
				UE_LOGF(LogNetPackageMap, Error, "ReceiveNetFieldExports: Invalid NetFieldExportHandle '%i', Max '%i'", Export.Handle, Exports.Num());
			}
		}
		else
		{
			UE_LOGF(LogNetPackageMap, Error, "ReceiveNetFieldExports: Unable to find NetFieldExportGroup for export. Export.Handle=%i, Export.Name=%ls, PathNameIndex=%lu, WasExported=%d, Archive.IsError()=%d",
...
void UPackageMapClient::ReceiveNetExportGUIDs(FArchive& Archive)
{
	...
	{
		UE_LOGF(LogNetPackageMap, Error, "UPackageMapClient::ReceiveNetExportGUIDs: NumGUIDs > MaxSerializedReplayNetGuids (%u / %d)", NumGUIDs, UE::Net::MaxSerializedReplayNetGuids);
		Archive.SetError();
		return;
	}
...
void UPackageMapClient::AppendExportBunches(TArray<FOutBunch *>& OutgoingBunches)
{
	...
	if (ExportBunches.Num() > 0)
	{
		if (UE_LOG_ACTIVE(LogNetPackageMap,Verbose))
		{
			UE_LOGF(LogNetPackageMap, Verbose, "AppendExportBunches. ExportBunches: %d, ExportNetGUIDCount: %d", ExportBunches.Num(), ExportNetGUIDCount);
...
void UPackageMapClient::ReceiveCustomExportsBunch(FInBunch &InBunch)
{
	...
	while (bHasExportsToRead && !InBunch.IsError())
	{
		...
		UE_LOGF(LogNetToken, VeryVerbose, "%s: Imported token %ls", __func__, *ImportedNetToken.ToString());
...
void UPackageMapClient::ReceivedAck( const int32 AckPacketId )
{
	...
	{
		UE_LOGF(LogNetToken, VeryVerbose, "%s: Acked token %ls", __func__, *(It.Value().ToString()));
...
bool UPackageMapClient::NetGUIDHasBeenAckd(FNetworkGUID NetGUID)
{
	...
	if ( AckPacketId == GUID_PACKET_ACKED )
	{
		...
		UE_LOGF( LogNetPackageMap, Verbose, "NetGUID <%ls> is fully ACKd (AckPacketId: %d <= Connection->OutAckPacketIdL %d) ", *NetGUID.ToString(), AckPacketId, Connection->OutAckPacketId );
		return true;
	}
...
void UPackageMapClient::HandleUnAssignedObject(UObject* Obj)
{
	...
	if ( !NetGUID.IsDefault() && ShouldSendFullPath( Obj, NetGUID ) )
	{
		...
		{
			UE_LOGF( LogNetPackageMap, Verbose, "Failed to export in ::HandleUnAssignedObject %ls", Obj ? *Obj->GetName() : TEXT("NULL") );
		}
	}
}
...
void UPackageMapClient::LogDebugInfo( FOutputDevice & Ar )
{
	for ( auto It = GuidCache->NetGUIDLookup.CreateIterator(); It; ++It )
	{
		...
		Ar.Logf(TEXT("%s"), *Str);
		UE_LOGF(LogNetPackageMap, Log, "%ls", *Str);
	}
}
...
bool UPackageMapClient::PrintExportBatch()
{
	...
	if (FNetGUIDCache::IsHistoryEnabled())
	{
		for (auto It = GuidCache->History.CreateIterator(); It; ++It)
		{
			...
			UE_LOGF(LogNetPackageMap, Warning, "<%ls> - %ls", *NetGUID.ToString(), *Str);
		}
	}
#endif

	UE_LOGF(LogNetPackageMap, Warning, "\n\n");
	if ( CurrentExportBunch != NULL )
	{
		for (auto It = CurrentExportBunch->ExportNetGUIDs.CreateIterator(); It; ++It)
		{
			UE_LOGF(LogNetPackageMap, Warning, "  CurrentExportBunch->ExportNetGUIDs: %ls", *It->ToString());
		}
	}
	...
	for (auto It = CurrentExportNetGUIDs.CreateIterator(); It; ++It)
	{
		UE_LOGF(LogNetPackageMap, Warning, "  CurrentExportNetGUIDs: %ls", *It->ToString());
	}
...
void UPackageMapClient::AddUnmappedNetGUIDReference(FNetworkGUID UnmappedGUID)
{
	...
	if (bRemapStableSubobjects && GuidCache)
	{
		...
		if (CacheObject && CacheObject->OuterGUID.IsValid() && !CacheObject->PathName.IsNone())
		{
			...
			UE_LOGF(LogNetPackageMap, VeryVerbose, "Adding unmapped stably-named inner object NetGUID to tracking map: %ls. With outer: %ls", *GuidCache->Describe(UnmappedGUID), *GuidCache->Describe(CacheObject->OuterGUID));
		}
	}
}
...
void UPackageMapClient::RemoveUnmappedNetGUIDReference(FNetworkGUID NetGUID)
{
	...
	if (bRemapStableSubobjects && GuidCache)
	{
		...
		if (CacheObject)
		{
			...
			if (FoundInners)
			{
				UE_LOGF(LogNetPackageMap, VeryVerbose, "Removing stably-named inner GUID from tracking map: %ls. With outer: %ls", *GuidCache->Describe(NetGUID), *GuidCache->Describe(CacheObject->OuterGUID));
...
bool FNetGUIDCache::SupportsObject( const UObject* Object, const TWeakObjectPtr<UObject>* WeakObjectPtr ) const
{
	...
	if (bDisplayWarning)
	{
		UE_LOGF(LogNetPackageMap, Warning, "FNetGUIDCache::SupportsObject: %ls NOT Supported.", *Object->GetFullName());
		//UE_LOGF( LogNetPackageMap, Warning, "   %ls", *DebugContextString );
	}
	return false;
}
...
FNetworkGUID FNetGUIDCache::GetOrAssignNetGUID(UObject* Object, const TWeakObjectPtr<UObject>* WeakObjectPtr)
{
	...
	if (!Object || !SupportsObject(Object, &WeakObject))
	{
		...
		UE_LOGF(LogNetPackageMap, Verbose, "GetOrAssignNetGUID: Object is not supported. Object %ls", *GetPathNameSafe(Object));
		return FNetworkGUID();
	}
	...
	if (NetGUID.IsValid())
	{
		...
		if (bReadOnly)
		{
			...
			UE_LOGF(LogNetPackageMap, Warning, "GetOrAssignNetGUID: Attempt to reassign read-only guid. FullNetGUIDPath: %ls", *FullNetGUIDPath(NetGUID));
	...
	if (!bIsNetGUIDAuthority)
	{
		...
		UE_LOGF(LogNetPackageMap, Verbose, "GetOrAssignNetGUID: NetGUIDLookup did not contain object on client, returning default. Object %ls", *Object->GetPathName());
...
FNetworkGUID FNetGUIDCache::AssignNewNetGUID_Server( UObject* Object )
{
	...
	UE_LOGF(LogNetPackageMapRemoteObject, Log, "Assigning object %ls to use NetGUID %ls based on remote object id %ls",
		   *Object->GetName(),
		   *NewNetGuid.ToString(),
...
FNetworkGUID FNetGUIDCache::AssignNewNetGUIDFromPath_Server( const FString& PathName, UObject* ObjOuter, UClass* ObjClass )
{
	...
	UE_LOGF(LogNetPackageMapRemoteObject, Log, "Assigning object %ls from path %ls to use NetGUID %ls based on remote object id %ls",
		   *ObjOuter->GetName(),
		   *PathName,
		   *NewNetGuid.ToString(),
		   *RemoteObjectId.ToString());
...
void FNetGUIDCache::RegisterNetGUID_Client( const FNetworkGUID& NetGUID, const UObject* Object )
{
	...
	UE_LOGF( LogNetPackageMap, Log, "RegisterNetGUID_Client: NetGUID: %ls, Object: %ls", *NetGUID.ToString(), Object ? *Object->GetName() : TEXT( "NULL" ) );
	...
	if ( ExistingCacheObjectPtr )
	{
		...
		{
			UE_LOGF( LogNetPackageMap, Warning, "RegisterNetGUID_Client: Guid with pathname. FullNetGUIDPath: %ls", *FullNetGUIDPath( NetGUID ) );
		}
		...
		{
			UE_LOGF( LogNetPackageMap, Warning, "RegisterNetGUID_Client: Reassigning NetGUID <%ls> to %ls (was assigned to object %ls)", *NetGUID.ToString(), Object ? *Object->GetPathName() : TEXT( "NULL" ), OldObject ? *OldObject->GetPathName() : TEXT( "NULL" ) );
		}
		else
		{
			UE_LOGF( LogNetPackageMap, Verbose, "RegisterNetGUID_Client: Reassigning NetGUID <%ls> to %ls (was assigned to object %ls)", *NetGUID.ToString(), Object ? *Object->GetPathName() : TEXT( "NULL" ), OldObject ? *OldObject->GetPathName() : TEXT( "NULL" ) );
		}
	...
	if ( ExistingNetworkGUIDPtr )
	{
		...
		UE_LOGF( LogNetPackageMap, Warning, "Changing NetGUID on object %ls from <%ls:%ls> to <%ls:%ls>", Object ? *Object->GetPathName() : TEXT( "NULL" ), *ExistingNetworkGUIDPtr->ToString(), ExistingNetworkGUIDPtr->IsDynamic() ? TEXT("TRUE") : TEXT("FALSE"), *NetGUID.ToString(), NetGUID.IsDynamic() ? TEXT("TRUE") : TEXT("FALSE") );
...
void FNetGUIDCache::RegisterNetGUIDFromPath_Client( const FNetworkGUID& NetGUID, const FString& PathName, const FNetworkGUID& OuterGUID, const uint32 NetworkChecksum, const bool bNoLoad, const bool bIgnoreWhenMissing )
{
	...
	UE_LOGF( LogNetPackageMap, Log, "RegisterNetGUIDFromPath_Client: NetGUID: %ls, PathName: %ls, OuterGUID: %ls", *NetGUID.ToString(), *PathName, *OuterGUID.ToString() );
	...
	if ( ExistingCacheObjectPtr != NULL )
	{
		...
		{
			UE_LOGF( LogNetPackageMap, Warning, "FNetGUIDCache::RegisterNetGUIDFromPath_Client: Path mismatch. Path: %ls, Expected: %ls, NetGUID: %ls", *PathName, *ExistingCacheObjectPtr->PathName.ToString(), *NetGUID.ToString() );
		...
		{
			UE_LOGF( LogNetPackageMap, Warning, "FNetGUIDCache::RegisterNetGUIDFromPath_Client: Outer mismatch. Path: %ls, Outer: %ls, Expected: %ls, NetGUID: %ls", *PathName, *OuterGUID.ToString(), *ExistingCacheObjectPtr->OuterGUID.ToString(), *NetGUID.ToString() );
		...
		if ( ExistingCacheObjectPtr->Object != NULL )
		{
			...
			{
				UE_LOGF( LogNetPackageMap, Warning, "FNetGUIDCache::RegisterNetGUIDFromPath_Client: Netguid mismatch. Path: %ls, NetGUID: %ls, Expected: %ls", *PathName, *NetGUID.ToString(), *CurrentNetGUID.ToString() );
...
void FNetGUIDCache::RegisterNetGUIDFromPath_Server( const FNetworkGUID& NetGUID, const FString& PathName, const FNetworkGUID& OuterGUID, const uint32 NetworkChecksum, const bool bNoLoad, const bool bIgnoreWhenMissing )
{
	...
	UE_LOGF( LogNetPackageMap, Log, "RegisterNetGUIDFromPath_Server: NetGUID: %ls, PathName: %ls, OuterGUID: %ls", *NetGUID.ToString(), *PathName, *OuterGUID.ToString() );
	...
	if ( ExistingCacheObjectPtr != nullptr )
	{
		...
		if ( ExistingCacheObjectPtr->PathName.ToString() != PathName )
		{
			UE_LOGF( LogNetPackageMap, Warning, "FNetGUIDCache::RegisterNetGUIDFromPath_Server: Path mismatch. Path: %ls, Expected: %ls, NetGUID: %ls", *PathName, *ExistingCacheObjectPtr->PathName.ToString(), *NetGUID.ToString() );
		...
		if ( ExistingCacheObjectPtr->OuterGUID != OuterGUID )
		{
			UE_LOGF( LogNetPackageMap, Warning, "FNetGUIDCache::RegisterNetGUIDFromPath_Server: Outer mismatch. Path: %ls, Outer: %ls, Expected: %ls, NetGUID: %ls", *PathName, *OuterGUID.ToString(), *ExistingCacheObjectPtr->OuterGUID.ToString(), *NetGUID.ToString() );
			ErrorStr = FString::Printf(TEXT("Outer mismatch. Path: %s, Outer: %s, Expected: %s, NetGUID: %s"), *PathName, *OuterGUID.ToString(), *ExistingCacheObjectPtr->OuterGUID.ToString(), *NetGUID.ToString());
			bOuterMismatch = true;
		}
...
void FNetGUIDCache::ValidateAsyncLoadingPackage(FNetGuidCacheObject& CacheObject, const FNetworkGUID NetGUID)
{
	...
	{
		UE_LOGF(LogNetPackageMap, Log, "ValidateAsyncLoadingPackage: Already async loading package with a different NetGUID. Path: %ls, original NetGUID: %ls, new NetGUID: %ls",
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	{
		UE_LOGF(LogNetPackageMap, Log, "ValidateAsyncLoadingPackage: Already async loading package. Path: %ls, NetGUID: %ls", *CacheObject.PathName.ToString(), *NetGUID.ToString());
	}
	...
	if (FPendingAsyncLoadRequest const * const PendingLoadRequest = PendingAsyncLoadRequests.Find(PackageName))
	{
		...
		for (FNetworkGUID NetGUIDToProcess : PendingLoadRequest->NetGUIDs)
		{
			if (FNetGuidCacheObject* CacheObject = ObjectLookup.Find(NetGUIDToProcess))
			{
				...
				{
					UE_LOGF(LogNetPackageMap, Error, "AsyncPackageCallback: Package wasn't pending. Path: %ls, NetGUID: %ls", *PackageName.ToString(), *NetGUIDToProcess.ToString());
				}
				...
				if (bIsBroken)
				{
					...
					UE_LOGF(LogNetPackageMap, Error, "AsyncPackageCallback: Package FAILED to load. Path: %ls, NetGUID: %ls", *PackageName.ToString(), *NetGUIDToProcess.ToString());
				}
			...
			{
				UE_LOGF(LogNetPackageMap, Error, "AsyncPackageCallback: Could not find net guid. Path: %ls, NetGUID: %ls", *PackageName.ToString(), *NetGUIDToProcess.ToString());
			}
		}
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	{
		UE_LOGF(LogNetPackageMap, Error, "AsyncPackageCallback: Could not find package. Path: %ls", *PackageName.ToString());
	}
}
...
UObject* FNetGUIDCache::GetObjectFromNetGUID( const FNetworkGUID& NetGUID, const bool bIgnoreMustBeMapped )
{
...
#if UE_WITH_REMOTE_OBJECT_HANDLE
		&& (UE::RemoteObject::Handle::GetResidence(FWeakObjectPtr(CacheObjectPtr->Object).GetRemoteId()) != EResidence::Remote)
#endif
		)
	{
	...
	if ( CacheObjectPtr->OuterGUID.IsValid() )
	{
		...
		if ( OuterCacheObject == NULL )
		{
			...
			{
				UE_LOGF( LogNetPackageMap, Error, "GetObjectFromNetGUID: Static outer not registered. FullNetGUIDPath: %ls", *FullNetGUIDPath( NetGUID ) );
		...
		{
			UE_LOGF( LogNetPackageMap, Error, "GetObjectFromNetGUID: Outer is broken. FullNetGUIDPath: %ls", *FullNetGUIDPath( NetGUID ) );
			CacheObjectPtr->bIsBroken = 1;	// Set this to 1 so that we don't keep spamming
			return NULL;
		}
		...
		if ( ObjOuter == NULL )
		{
			...
			{
				UE_LOGF( LogNetPackageMap, Error, "GetObjectFromNetGUID: Failed to find outer. FullNetGUIDPath: %ls", *FullNetGUIDPath( NetGUID ) );
			}
	...
	{
		UE_LOGF( LogNetPackageMap, Error, "GetObjectFromNetGUID: Outer is null or package is not fully loaded.  FullNetGUIDPath: %ls Outer: %ls", *FullNetGUIDPath( NetGUID ), *GetFullNameSafe(ObjOuter) );
	}
	...
	if ( Object == NULL && !CacheObjectPtr->bNoLoad )
	{
		if (bIsNetGUIDAuthority)
		{
			...
			UE_LOGF(LogNetPackageMap, Warning, "GetObjectFromNetGUID: Server re-loading object (might have been GC'd). FullNetGUIDPath: %ls", *FullNetGUIDPath(NetGUID));
		}
		...
		if ( bIsPackage )
		{
			...
			if ( ShouldAsyncLoad() )
			{
				if (!PendingAsyncLoadRequests.Contains(CacheObjectPtr->PathName))
				{
					...
					UE_LOGF(LogNetPackageMap, Log, "GetObjectFromNetGUID: Async loading package. Path: %ls, NetGUID: %ls", *CacheObjectPtr->PathName.ToString(), *NetGUID.ToString());
				}
				else
				{
		...
		else
		{
			...
			if ( ShouldAsyncLoad() )
			{
				UE_LOGF( LogNetPackageMap, Error, "GetObjectFromNetGUID: Forced blocking load. Path: %ls, NetGUID: %ls", *CacheObjectPtr->PathName.ToString(), *NetGUID.ToString() );
			}
		}
	}
	...
	if ( Object == NULL )
	{
		...
		{
			CacheObjectPtr->bIsBroken = 1;	// Set this to 1 so that we don't keep spamming
			UE_LOGF( LogNetPackageMap, Error, "GetObjectFromNetGUID: Failed to resolve path. FullNetGUIDPath: %ls", *FullNetGUIDPath( NetGUID ) );
		}
	...
	if ( bIsPackage )
	{
		...
		if ( Package == NULL )
		{
			...
			UE_LOGF( LogNetPackageMap, Error, "GetObjectFromNetGUID: Object is not a package but should be! Path: %ls, NetGUID: %ls", *CacheObjectPtr->PathName.ToString(), *NetGUID.ToString() );
			return NULL;
		}
			...
			&& !Package->HasAnyPackageFlags( TreatAsLoadedFlags )) //TODO: dependencies of CompiledIn could still be loaded asynchronously. Are they necessary at this point??
		{
			if (ShouldAsyncLoad() && Package->HasAnyInternalFlags(EInternalObjectFlags_AsyncLoading))
			{
				...
				UE_LOGF(LogNetPackageMap, Log, "GetObjectFromNetGUID: Listening to existing async load. Path: %ls, NetGUID: %ls", *CacheObjectPtr->PathName.ToString(), *NetGUID.ToString());
			...
			else if (!GbNetCheckNoLoadPackages || !CacheObjectPtr->bNoLoad)
			{
				...
				UE_LOGF(LogNetPackageMap, Log, "GetObjectFromNetGUID: Blocking load of %ls, NetGUID: %ls", *CacheObjectPtr->PathName.ToString(), *NetGUID.ToString());
	...
	if ( CacheObjectPtr->NetworkChecksum != 0 && !CVarIgnoreNetworkChecksumMismatch.GetValueOnAnyThread() )
	{
		...
		if (CacheObjectPtr->NetworkChecksum != NetworkChecksum )
		{
			if ( NetworkChecksumMode == ENetworkChecksumMode::SaveAndUse )
			{
				...
				UE_LOGF( LogNetPackageMap, Warning, "%ls", *ErrorStr );
			...
			{
				UE_LOGF( LogNetPackageMap, Verbose, "GetObjectFromNetGUID: Network checksum mismatch. FullNetGUIDPath: %ls, %u, %u", *FullNetGUIDPath( NetGUID ), CacheObjectPtr->NetworkChecksum, NetworkChecksum );
			}
		}
	}
	...
	{
		UE_LOGF(LogNetPackageMap, Verbose, "GetObjectFromNetGUID: Forcing object to NULL since level is not loaded yet. Object: %ls", *GetFullNameSafe(Object));
		return NULL;
	}
	...
	if (bIsNotReadOnlyOrAllowRemap || !NetGUIDLookup.Contains(Object))
	{
		...
		{
			UE_LOGF( LogNetPackageMap, Warning, "GetObjectFromNetGUID: Attempt to reassign read-only guid. FullNetGUIDPath: %ls", *FullNetGUIDPath( NetGUID ) );
...
void FNetGUIDCache::GenerateFullNetGUIDPath_r( const FNetworkGUID& NetGUID, FString& FullPath ) const
{
	...
	if ( CacheObject->Object.IsValid() )
	{
		...
		{
			UE_LOGF( LogNetPackageMap, Warning, "GenerateFullNetGUIDPath_r: Name mismatch! %ls != %ls", *CacheObject->PathName.ToString(), *CacheObject->Object->GetName() );	
		}
...
FArchive& operator<<(FArchive& Ar, FNetFieldExportGroup& C)
{
	...
	if (Ar.IsLoading())
	{
		...
		{
			UE_LOGF(LogNetPackageMap, Error, "FNetFieldExportGroup - NumNetFieldExports exceeds MaxSerializedNetExportsPerGroup (%d / %d)", NumNetFieldExports, UE::Net::MaxSerializedNetExportsPerGroup);
			Ar.SetError();
			return Ar;
		}
	...
	for (auto It = QueuedBunchObjectReferences.CreateIterator(); It; ++It)
	{
		...
		if (ObjectRefPtr)
		{
			...
			if (ObjectRefPtr->Object)
			{
				...
				if (!ObjectRefPtr->Object)
				{
					UE_LOGF(LogNetPackageMap, Warning, "FNetGUIDCache::CollectReferences: QueuedBunchObjectReference was killed by GC. NetGUID=%ls", *It.Key().ToString());
				}
			}
		}
		else
...
static void	ListNetGUIDExports()
{
	...
	for (TObjectIterator<UPackageMapClient> PmIt; PmIt; ++PmIt)
	{
		...
		{
			UE_LOGF(LogNetPackageMap, Warning, "NetGUID <%ls> - %d", *(It.Key().ToString()), It.Value() );	
		}
		UE_LOGF(LogNetPackageMap, Warning, "-----------------------");
	}			
}
