...
FString UCurveTable::GetTableAsString() const
{
	...
	{
		Result += FString("No data in row curve!\n");
	}
	return Result;
}
...
void UCurveTable::RenameRow(const FName& CurveName, const FName& NewCurveName)
{
...
void UCurveTable::DeleteRow(const FName& CurveName)
{
...
TArray<FString> UCurveTable::CreateTableFromCSVString(const FString& InString, ERichCurveInterpMode InterpMode)
{
	...
	if (Rows.Num() <= 1)
	{
		OutProblems.Add(FString("Too few rows."));
		return OutProblems;
	}
...
#if WITH_EDITORONLY_DATA
	ImportCurveInterpMode = InterpMode;
...
TArray<FString> UCurveTable::CreateTableFromJSONString(const FString& InString, ERichCurveInterpMode InterpMode)
{
	...
	for (int32 RowIdx = 0; RowIdx < ParsedTableRows.Num(); ++RowIdx)
	{
		...
		// Get row name
		static const FString RowNameJsonKey = "Name";
		...
		{
			// Skip the name entry
			if (FStringView(ParsedTableRowEntry.Key) == RowNameJsonKey)
			{
				continue;
			}
...
FRealCurve* FCurveTableRowHandle::GetCurve(const FString& ContextString, bool bWarnIfNotFound) const
{
	if (CurveTable == nullptr)
	{
		if (RowName != NAME_None)
		{
			UE_CLOGF(bWarnIfNotFound, LogCurveTable, Warning, "FCurveTableRowHandle::FindRow : No CurveTable for row %ls (%ls).", *RowName.ToString(), *ContextString);
		}
		return nullptr;
	}
