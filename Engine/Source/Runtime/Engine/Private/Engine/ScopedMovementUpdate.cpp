...
, bIsGroupDeferedUpdate(ScopeBehavior == EScopedUpdate::DeferredGroupUpdates)
{
	if (IsValid(Component))
	{
		...
		if (ScopeBehavior == EScopedUpdate::ImmediateUpdates)
		{
			...
			if (OuterDeferredScope && OuterDeferredScope->bDeferUpdates)
			{
				if (s_ScopedWarningCount < 100 || (GFrameCounter & 31) == 0)
				{
					...
					UE_LOGF(LogScopedMovementUpdate, Error, "FScopedMovementUpdate attempting to use immediate updates within deferred scope, will use deferred updates instead.");
				}
		...
		{
			UE_LOGF(LogScopedMovementUpdate, Warning, "FScopedMovementUpdate attempting to use group updates within an ungrouped scope, will use deferred updates instead.");
