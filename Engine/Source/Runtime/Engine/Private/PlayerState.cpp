...
#if UE_WITH_REMOTE_OBJECT_HANDLE
FRemoteServerId APlayerState::GetPreferredRemoteServer() const
{
	if (!AutoRTFM::IsClosed() && PawnPrivate.GetResidence() == EResidence::Remote)
	{
		// We are not in a transaction (probably during push migration) and our Pawn is non-local
		// So the Pawn must have already migrated, choose that server.
		FRemoteServerId PawnServerId = UE::RemoteObject::Handle::GetOwnerServerId(PawnPrivate.GetRemoteId());
		if (PawnServerId.IsValid())
		{
			return PawnServerId;
		}
	}
	else if (APawn* LocalPawn = PawnPrivate.Get())
	{
		return LocalPawn->GetPreferredRemoteServer();
	}
	// No Pawn — stay where we are (don't use spatial delegate, we have no location)
