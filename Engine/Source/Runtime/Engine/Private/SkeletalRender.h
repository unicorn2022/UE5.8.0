...
/**
* Forward kinematic update of bones removed at a given LOD level.
* Fixes frozen transforms in the pose buffer for bones that were LOD'd out.
* @param	MeshObject - mesh object that caches the bones-to-remove list
* @param	PoseBuffer - reference-to-local matrices to fix up
* @param	InDynamicData - dynamic data providing component space transforms
* @param	SkinnedAsset - asset providing ref skeleton and inverse ref pose
* @param	LODIndex - LOD level whose removed bones should be fixed
* @param	bUsePreviousTransforms - if true, reads previous component space transforms instead of current
*/
ENGINE_API void UpdateBonesRemovedByLOD(FSkeletalMeshObject* MeshObject, TArray<FMatrix44f>& PoseBuffer, const FSkinnedMeshSceneProxyDynamicData& InDynamicData, const USkinnedAsset* SkinnedAsset, int32 LODIndex, bool bUsePreviousTransforms);
...
/** Returns true if skin weight profile management is to be done on the render thread exclusively */
ENGINE_API bool IsSkinnedMeshSkinWeightProfilesOnRenderThread();