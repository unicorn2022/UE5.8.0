...
namespace
{
	...
	static const FName TriggerCollisionProfileName("Trigger");
}
...
void ATriggerSphere::EditorApplyScale(const FVector& DeltaScale, const FVector* PivotLocation, bool bAltDown, bool bShiftDown, bool bCtrlDown)
{
	...
	float ScaleToApply = ModifiedScale.GetMax();
	const float MinScale = ModifiedScale.GetMin();

	// Use scale with the greater length
	if (FMath::Abs(MinScale) > ScaleToApply)
	{
		ScaleToApply = MinScale;
	}

	USphereComponent * SphereComponent = CastChecked<USphereComponent>(GetRootComponent());
	SphereComponent->SetSphereRadius(FMath::Max<float>(0.0f, SphereComponent->GetUnscaledSphereRadius() + ScaleToApply));
}
#endif	//WITH_EDITOR
