...
{
#if WITH_EDITOR
	return PrimComponent && PrimComponent->bAlwaysAllowTranslucentSelect;
#else
	return false;
#endif
}
...
FContentComparisonHelper::FContentComparisonHelper()
{
	...
	if (RefTypes != NULL)
	{
		for( FConfigSectionMap::TConstIterator It(*RefTypes); It; ++It )
		{
			...
			UE_LOGF(LogEngineUtils, Log, "Adding class of interest: %ls", *RefType);
		}
	}
}
		...
		for( TObjectIterator<UClass> It; It; ++It )
		{
				...
				(TheAssetClass->HasAnyClassFlags(CLASS_Abstract) == false))
			{
				...
				for (int32 CheckIdx = 0; CheckIdx < IgnoreBaseClasses.Num(); CheckIdx++)
				{
					...
					{
// 						UE_LOGF(LogEngineUtils, Warning, "Skipping class derived from other content comparison class...");
// 						UE_LOGF(LogEngineUtils, Warning, "\t%ls derived from %ls", *TheAssetClass->GetFullName(), *CheckClass->GetFullName());
						bSkipIt = true;
					}
				}
				if (bSkipIt == false)
	...
	{
		UE_LOGF(LogEngineUtils, Warning, "Failed to find class: %ls", *InBaseClassName);
		return false;
	}

#if 0
	// Log them all out
	UE_LOGF(LogEngineUtils, Log, "CompareClasses on %ls", *InBaseClassName);
	for (TMap<FString,TArray<FContentComparisonAssetInfo>>::TIterator It(ClassToAssetsMap); It; ++It)
	{
		...
		UE_LOGF(LogEngineUtils, Log, "\t%ls", *ClassName);
		for (int32 AssetIdx = 0; AssetIdx < AssetList.Num(); AssetIdx++)
		{
			FContentComparisonAssetInfo& Info = AssetList(AssetIdx);

			UE_LOGF(LogEngineUtils, Log, "\t\t%ls,%f", *(Info.AssetName), Info.ResourceSize/1024.0f);
		}
	}
#endif
	...
	else if (AssetTable != NULL)
	{
		...
		UE_LOGF(LogEngineUtils, Warning, "Failed to open output stream in asset table!");
	}
