...
void UTimelineTemplate::PostDuplicate(bool bDuplicateForPIE)
{
	...
	for(TArray<struct FTTFloatTrack>::TIterator It = FloatTracks.CreateIterator();It;++It)
	{
		...
		{
			UE_LOGF(LogBlueprint, Warning, "Timeline %ls Track %ls in %ls has an invalid curve.  Please fix!", *GetVariableName().ToString(), *Track.GetTrackName().ToString(), *GetPathNameSafe(GetOuter()));
		}
	}
