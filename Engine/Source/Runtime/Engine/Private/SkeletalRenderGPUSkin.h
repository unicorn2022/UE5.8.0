...
*/
class FDynamicSkelMeshObjectDataGPUSkin final : public TPooledSkeletalMeshDynamicData<FDynamicSkelMeshObjectDataGPUSkin>
{
...
*/
class FMorphVertexBufferPool : public FRefCountedObject
{
public:
	...
	/**
	 * Non-asserting variant of GetMorphVertexBufferForReading. Returns nullptr if the underlying
	 * VertexBufferRHI isn't valid yet
	 */
	const FMorphVertexBuffer* TryGetMorphVertexBufferForReading(bool bPrevious) const;
	FMorphVertexBuffer& GetMorphVertexBufferForWriting();

private:
	/** Resolves which MorphVertexBuffers[] slot a reader should observe */
	uint32 GetReadingIndex(bool bPrevious) const;
...
public:
	...
	FSkinWeightVertexBufferInfo GetSkinWeightVertexBuffer_RenderThread(int32 LODIndex) const override;
...
private:
	...
	struct FSkeletalMeshObjectLOD
	{
		...
		void InitResources(
			...
			ERHIFeatureLevel::Type FeatureLevel,
			FGPUSkinPassthroughVertexFactory::EVertexAttributeFlags VertexAttributeMask,
			EBoneTransformStorageMode BoneTransformStorageMode,
		...
		/** Unified bone buffer across the entire LOD when the mesh supports it. */
		...
		const FSkinWeightVertexBuffer* MeshObjectWeightBuffer = nullptr;
		FSkinWeightVertexBufferInfo MeshObjectWeightBuffer_RenderThread;
		...
		void UpdateSkinWeights(const FSkelMeshComponentLODInfo* CompLODInfo, EBoneTransformStorageMode BoneTransformStorageMode);
		...
		void GetVertexBuffers(
			...
			EBoneTransformStorageMode StorageMode);
	...
	FDynamicSkelMeshObjectDataGPUSkin* PreviousDynamicData = nullptr;
	...
	EBoneTransformAllocationMode BoneTransformAllocationMode = EBoneTransformAllocationMode::VertexFactory;
	bool bMorphResourcesInitialized = false;
	bool bMorphNeedsUpdate = false;

	/** Cached scene extension bone data, set in UpdateSceneExtensionHeader, reset in ProcessUpdatedDynamicData. */
	mutable struct
	{
		FRHIShaderResourceView* TransformBufferSRV = nullptr;
		uint32 TransformBufferOffset = 0;
		uint32 MaxTransformCount = 0;
		uint32 CurrentTransformSlot = 0;
