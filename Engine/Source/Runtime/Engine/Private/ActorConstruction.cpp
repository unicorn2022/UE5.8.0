...
void AActor::ResetPropertiesForConstruction()
{
	...
	const FName RandomStreamName("RandomStream");
	...
	for (const TPair<UActorComponent*,int32>& ComponentAndDepth : ComponentDepthMap)
	{
		...
		if (Component)
		{
			...
			if (bDestroyComponent)
			{
				...
				FName const NewObjectName = MakeUniqueObjectName(GetTransientOuterForRename(Component->GetClass()), GetClass(), NewBaseName);
				Component->Rename(*NewObjectName.ToString(), GetTransientOuterForRename(Component->GetClass()), REN_DontCreateRedirectors|REN_NonTransactional|REN_DoNotDirty);
...
{
	UFunction* UCS = GetClass()->FindFunctionByName(FName("UserConstructionScript")/*UEdGraphSchema_K2::FN_UserConstructionScript*/);
...
void AActor::RerunConstructionScripts()
{
	...
	if (bAllowReconstruction)
	{
		...
		if (bUseRootComponentProperties)
		{
			...
			{
				// ComponentToWorld must be up-to-date for the detach below, and to get OldTransform
				...
				if (RootComponent->GetAttachParent() != nullptr && RootComponent->IsCreatedByConstructionScript())
				{
					...
					if (Parent == this)
					{
						UE_LOGF(LogActor, Warning, "RerunConstructionScripts: RootComponent (%ls) attached to another component in this Actor (%ls).", *RootComponent->GetPathName(), *Parent->GetPathName());
						Parent = nullptr;
					}
					...
					if (Info.AttachParentName != NAME_None)
					{
						...
						if (!ChildRoot->GetAttachParent())
						{
							UE_LOGF(LogBlueprint, Warning,
								"Couldn't find a component named \'%ls\' when reattaching. Attaching to root component instead.",
...
{
#if WITH_EDITOR
	TOptional<UE::Cook::FCookRandScope> CookRandScope;
	if (IsRunningCookCommandlet())
	{
		// Notify blueprint functions called by the UserConstructScript that support CookRand to return deterministic
		// results based on our seed. Overwrite an existing CookRand if it exists so that we always get the same
		// guids/rands even if called from other UserConstruction scripts or CookRand environments.
		int32 Seed = UE::Cook::FCookRand::ConstructSeed(GetPathName());
		CookRandScope.Emplace(Seed, UE::Cook::ECookRandScope::Overwrite);
	}
#endif
...
UActorComponent* AActor::CreateComponentFromTemplate(UActorComponent* Template, FName InName)
{
	...
	if (Template != nullptr)
	{
		...
		{
			UE_LOGF(LogBlueprint, Log, "%ls: CreateComponentFromTemplate() - %ls \'%ls\' completed in %.02g ms", *GetName(), *Template->GetClass()->GetName(), *InName.ToString(), (FPlatformTime::Seconds() - StartTime) * 1000.0);
		}
#endif
	}
	return NewActorComp;
...
UActorComponent* AActor::CreateComponentFromTemplateData(const FBlueprintCookedComponentInstancingData* TemplateData, FName InName)
{
	...
	if (TemplateData != nullptr && TemplateData->ComponentTemplateClass != nullptr)	// some components (e.g. UTextRenderComponent) are not loaded on a server (or client). Handle that gracefully, but we ideally shouldn't even get here (see UEBP-175).
	{
		...
		if (CVarLogBlueprintComponentInstanceCalls.GetValueOnGameThread())
		{
			UE_LOGF(LogBlueprint, Log, "%ls: CreateComponentFromTemplateData() - %ls \'%ls\' completed in %.02g ms", *GetName(), *TemplateData->ComponentTemplateClass->GetName(), *InName.ToString(), (FPlatformTime::Seconds() - StartTime) * 1000.0);
		}
#endif
	}
	return NewActorComp;
...
UActorComponent* AActor::AddComponent(FName TemplateName, bool bManualAttachment, const FTransform& RelativeTransform, const UObject* ComponentTemplateContext, bool bDeferredFinish)
{
	if (const UWorld* World = GetWorld())
	{
		...
		{
			UE_LOGF(LogActor, Warning, "AddComponent failed for actor: [%ls] with param TemplateName: [%ls] because we are in the process of tearing down the world"
				, *GetName()
	...
	{
		UE_LOGF(LogActor, Warning, "AddComponent failed for actor: [%ls] with param TemplateName: [%ls] because world == nullptr"
			, *GetName()
			, *TemplateName.ToString());
		return nullptr;
	}
...
UActorComponent* AActor::AddComponentByClass(TSubclassOf<UActorComponent> Class, bool bManualAttachment, const FTransform& RelativeTransform, bool bDeferredFinish)
{
	...
	if (const UWorld* World = GetWorld())
	{
		if (World->bIsTearingDown)
		{
			UE_LOGF(LogActor, Warning, "AddComponentByClass failed for actor: [%ls] with param Class: [%ls] because we are in the process of tearing down the world"
				, *GetName()
				, *GetNameSafe(Class));
			return nullptr;
		}
	...
	{
		UE_LOGF(LogActor, Warning, "AddComponentByClass failed for actor: [%ls] with param Class: [%ls] because world == nullptr"
			, *GetName()
			, *GetNameSafe(Class));
		return nullptr;
	}
