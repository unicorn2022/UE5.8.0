...
static void ListUnbuiltHLODActors(const TArray<FString>& Args, UWorld* World)
{
	...
	for (TActorIterator<ALODActor> HLODIt(World); HLODIt; ++HLODIt)
	{
		...
		if (!Actor->IsBuilt() && Actor->HasValidLODChildren())
		{
			...
			UE_LOGF(LogHLOD, Warning, "HLOD %ls is unbuilt (HLOD level %i)", *ActorPathName, Actor->LODLevel);
		}
	}

	UE_LOGF(LogHLOD, Warning, "%d HLOD actor(s) were unbuilt", NumUnbuilt);
...
static UMaterialInterface* GetImposterMaterial(const UStaticMeshComponent* InComponent)
{
	...
	{
		UE_LOGF(LogHLOD, Warning, "Imposter's static mesh %ls has multiple mesh sections for its lowest LOD", *StaticMesh->GetName());
	}
...
void ALODActor::PreSave(FObjectPreSaveContext ObjectSaveContext)
{
	...
	if(!IsBuilt(true))
	{
		UE_LOGF(LogHLOD, Log, "HLOD actor %ls in map %ls is not built. Meshes may not match.", *GetName(), *GetOutermost()->GetName());
	}
}
