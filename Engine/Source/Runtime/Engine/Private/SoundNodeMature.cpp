		...
		*RequiresInitialization = 0;
		...
		for( int32 i = 0; i < ChildNodes.Num() ; ++i )
		{
			if( ChildNodes[ i ] )
			{
				...
				{
					UE_LOGF(LogAudio, Warning, "SoundNodeMature(%ls) has a child which is not eventually linked to a sound node wave", *GetPathName() );
				}
			}
		}
		...
		else
		{
			...
			{
				UE_LOGF(LogAudio, Warning, "SoundNodeMature(%ls): GEngine->bAllowMatureLanguage is false, no non-mature child sound exists", *GetPathName() );
			}
		}
	}
