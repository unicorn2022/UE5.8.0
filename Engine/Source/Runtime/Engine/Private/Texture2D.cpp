...
float UTexture2D::GetAverageBrightness(bool bIgnoreTrueBlack, bool bUseGrayscale)
{
	...
	{
		UE_LOGF(LogTexture, Log, "No SourceArt available for %ls", *GetPathName());
	}
...
#if WITH_EDITOR
void UTexture2D::GetAssetRegistryTagMetadata(TMap<FName, FAssetRegistryTagMetadata>& OutMetadata) const
{
	Super::GetAssetRegistryTagMetadata(OutMetadata);

	OutMetadata.Add("Dimensions",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UTexture2D", "Dimensions_Label", "Dimensions"))
		);

	OutMetadata.Add("HasAlphaChannel",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UTexture2D", "HasAlphaChannel_Label", "Has Alpha Channel"))
		);

	OutMetadata.Add("Format",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UTexture2D", "Format_Label", "Format"))
		);
}
#endif
...
void UTexture2D::PostLinkerChange()
{
	...
	if( !HasAnyFlags( RF_BeginDestroyed | RF_NeedLoad | RF_NeedPostLoad ) && !IsUnreachable() && !HasAnyInternalFlags(EInternalObjectFlags_AsyncLoading) )
	{
		...
		else
		{
			...
			if (!NumMips)
			{
				...
				if (bIsServerCookedPackage)
				{
					...
					UE_LOGF(LogTexture, Log, "%ls contains no miplevels! Please delete. (Format: %d)", *GetFullName(), (int)PixelFormat);
				}
				else
				{
					UE_LOGF(LogTexture, Warning, "%ls contains no miplevels! This could happen if this texture is a thumbnail and hasn't been generated (Format: %d)", *GetFullName(), (int)PixelFormat);
				}
#else
				UE_LOGF(LogTexture, Error, "%ls contains no miplevels! Please delete. (Format: %d)", *GetFullName(), (int)PixelFormat);
#endif
				
			}
			else if (!GPixelFormats[PixelFormat].Supported)
			{
				UE_LOGF(LogTexture, Error, "%ls is %ls [raw type %d] which is not supported.", *GetFullName(), GPixelFormats[PixelFormat].Name, static_cast<int32>(PixelFormat));
			}
			else if (NumMips == 1 && FMath::Max(GetSizeX(), GetSizeY()) > (int32)GetMax2DTextureDimension())
			{
				UE_LOGF(LogTexture, Warning, "%ls cannot be created, exceeds this rhi's maximum dimension (%d) and has no mip chain to fall back on.", *GetFullName(), GetMax2DTextureDimension());
			}
			else
			{
...
UTexture2D* UTexture2D::CreateTransientFromImage(const FImage* InImage, const FName InName)
{
	...
	default:
		{
			UE_LOGF(LogTexture, Error, "Invalid raw image format in UTexture2D::CreateTransientFromImage()");
			return nullptr;
		}
	}
...
UTexture2D* UTexture2D::CreateTransient(int32 InSizeX, int32 InSizeY, EPixelFormat InFormat, const FName InName, TConstArrayView64<uint8> InImageData)
{
	...
	{
		UE_LOGF(LogTexture, Warning, "Negative size specified for UTexture2D::CreateTransient()");
		return nullptr;
	}
	...
	{
		UE_LOGF(LogTexture, Warning, "Size specified isn't valid for block-based pixel format in UTexture2D::CreateTransient()");
		return nullptr;
	}
	...
	{
		UE_LOGF(LogTexture, Warning, "Size specified overflows in UTexture2D::CreateTransient()");
		return nullptr;
	}
	...
	{
		UE_LOGF(LogTexture, Warning, "Image data provided is incorrect size (%llu provided, %llu wanted) in UTexture2D::CreateTransient()", InImageData.Num(), BytesForImage);
		return nullptr;
	}
...
int32 UTexture2D::Blueprint_GetSizeX() const
{
	...
	if (!GetSizeX())
	{
		...
		const int32 CookedLODBias = LODSettings->CalculateLODBias(Source.SizeX, Source.SizeY, GetMinTextureSize(), GetMaxTextureSize(), LODGroup, LODBias, 0, MipGenSettings, IsCurrentlyVirtualTextured(), IsStreamable());
...
void UTexture2D::UpdateTextureRegions(int32 MipIndex, uint32 NumRegions, const FUpdateTextureRegion2D* Regions, uint32 SrcPitch, uint32 SrcBpp, uint8* SrcData, TFunction<void(uint8* SrcData, const FUpdateTextureRegion2D* Regions)> DataCleanupFunc)
{
	if (IsCurrentlyVirtualTextured())
	{
		UE_LOGF(LogTexture, Log, "UpdateTextureRegions called for %ls which is virtual.", *GetPathName());
		return;
	}
	...
	{
		UE_LOGF(LogTexture, Log, "UpdateTextureRegions called for %ls without calling TemporarilyDisableStreaming", *GetPathName());
	}
...
void FVirtualTexture2DResource::InitializeEditorResources(FRHICommandListBase& RHICmdList, IVirtualTexture* InVirtualTexture)
{
	...
	if (GIsEditor && !IsRunningCommandlet())
	{
		...
		else
		{
			...
			const ETextureCreateFlags RHITexCreateFlags = ETextureCreateFlags::ShaderResource | (this->TexCreateFlags & TexCreate_SRGB);
			...
			if (FRHITexture* ReferencedTexture = TextureReferenceRHI->GetReferencedTexture())
			{
				...
				if (ExistingDesc.Format == PixelFormat && ExistingDesc.GetSize() == FIntVector(MipWidthInTiles * TileSizeInPixels, MipHeightInTiles * TileSizeInPixels, 1) &&
					ExistingDesc.Flags == RHITexCreateFlags )
				{
