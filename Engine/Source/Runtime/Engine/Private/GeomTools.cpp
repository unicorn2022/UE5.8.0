...
bool FGeomTools::TriangulatePoly(TArray<FClipSMTriangle>& OutTris, const FClipSMPolygon& InPoly, bool bKeepColinearVertices)
{
	...
	while(true)
	{
		...
		else
		{
			...
			if(!bFoundEar)
			{
				UE_LOGF(LogGeomTools, Log, "Triangulation of poly failed.");
				OutTris.Empty();
				return false;
			}
		}
...
bool FGeomTools2D::ArePolygonsValid(const TArray<TArray<FVector2D>>& Polygons)
{
			...
			for (int VertexA = 0; VertexA < PolygonA.Num(); ++VertexA)
			{
				...
				for (int VertexB = 0; VertexB < PolygonB.Num(); ++VertexB)
				{
					...
					if (DetS != 0.0f && DetT != 0.0f)
					{
						...
						{
							// Edges intersect
							UE_LOGF(LogGeomTools, Log, "Edges in polygon %d and %d intersect", PolygonIndexA, PolygonIndexB);
							return false;
						}
					}
				}
	...
	//				{
	//					// Edges intersect
	//					UE_LOGF(LogGeomTools, Log, "Polygon %d is self intersecting", PolygonIndex);
	//					return false;
	//				}
	//			}
	//		}
