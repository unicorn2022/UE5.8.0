...
void ULevelInstanceSubsystem::UpdateStreamingStateInternal()
{
	...
	if (LevelInstancesToLoadOrUpdate.Num())
	{
		...
		UE_LOGF(LogLevelInstance, Log, "Loaded %ls levels in %ls seconds", *FText::AsNumber(LevelInstancesToLoadOrUpdateCopy.Num()).ToString(), *FText::AsNumber(ElapsedTime).ToString());
#endif
	}
...
void ULevelInstanceSubsystem::OnWorldCleanup(UWorld* InWorld, bool bSessionEnded, bool bCleanupResources)
{
	if (InWorld == GetWorld() && !InWorld->IsGameWorld() && bCleanupResources)
	{
		...
		if (LevelInstanceEdit.IsValid())
		{
			ILevelInstanceInterface* LI = LevelInstanceEdit->GetLevelInstance();
			BroadcastOnLevelInstanceEditModeChanged(LI, ILevelInstanceEditorModule::ELevelInstanceEditMode::None);
		}
...
void ULevelInstanceSubsystem::RegisterLoadedLevelStreamingLevelInstanceEditor(ULevelStreamingLevelInstanceEditor* LevelStreaming)
{
	if (!bIsCreatingLevelInstance)
	{
		...
		BroadcastOnLevelInstanceEditModeChanged(LevelInstance, ILevelInstanceEditorModule::ELevelInstanceEditMode::Original);
	}
}
...
void ULevelInstanceSubsystem::Tick()
{
	...
	if (!GetWorld()->IsGameWorld())
	{
		...
		if (GEditor && GEditor->GetEditorWorldContext().World() == GetWorld())
		{
...
bool ULevelInstanceSubsystem::MoveActorsToLevel(const TArray<AActor*>& ActorsToRemove, ULevel* DestinationLevel, TArray<AActor*>* OutActors /*= nullptr*/) const
{
	...
	{
		UE_LOGF(LogLevelInstance, Warning, "Failed to move actors out of Level Instance because not all actors could be moved");
		return false;
	}
...
ILevelInstanceInterface* ULevelInstanceSubsystem::CreateLevelInstanceFrom(const TArray<AActor*>& ActorsToMove, const FNewLevelInstanceParams& CreationParams)
{
	...
	{
		UE_LOGF(LogLevelInstance, Warning, "Failed to create Level Instance : %ls", *Reason.ToString());
		return nullptr;
	}
	...
	if (!LevelStreaming)
	{
		UE_LOGF(LogLevelInstance, Warning, "Failed to create new Level");
		return nullptr;
	}
	...
	else
	{
		...
		{
			UE_LOGF(LogLevelInstance, Warning, "Failed to create packed level blueprint. Creating non blueprint packed level instance instead.");
	...
	TArray<AActor*>* OutMovedActors /* = nullptr */, ELevelInstanceBreakFlags Flags /* = None */)
{
	...
	UE_LOGF(LogLevelInstance, Log, "Break took %ls seconds (%ls actors)", *FText::AsNumber(ElapsedTime).ToString(), *FText::AsNumber(MovedActors.Num()).ToString());
	...
	TArray<AActor*>& OutMovedActors, ELevelInstanceBreakFlags Flags)
{
	if (Levels > 0)
	{
		...
		{
			UE_LOGF(LogLevelInstance, Warning, "Failed to completely break Level Instance because some children have Level Scripts.");
		...
		{
			UE_LOGF(LogLevelInstance, Warning, "Failed to break Level Instance because not all actors could be moved");
			return;
		}
...
void ULevelInstanceSubsystem::BroadcastOnLevelInstanceEditModeChanged(ILevelInstanceInterface* LevelInstance, ILevelInstanceEditorModule::ELevelInstanceEditMode NewMode)
{
	...
	if (ILevelInstanceEditorModule* EditorModule = FModuleManager::Get().GetModulePtr<ILevelInstanceEditorModule>("LevelInstanceEditor"))
	{
		EditorModule->BroadcastOnLevelInstanceEditModeChanged(LevelInstance, NewMode);
	}
}
...
ULevelInstanceSubsystem::FLevelsToRemoveScope::~FLevelsToRemoveScope()
{
	if (Levels.Num() > 0)
	{
		...
		double ElapsedTime = FPlatformTime::Seconds() - StartTime;
		UE_LOGF(LogLevelInstance, Log, "Unloaded %ls levels in %ls seconds", *FText::AsNumber(Levels.Num()).ToString(), *FText::AsNumber(ElapsedTime).ToString());
	}
}
...
bool ULevelInstanceSubsystem::CanEditLevelInstancePropertyOverrides(const ILevelInstanceInterface* LevelInstance, FText* OutReason) const
{
	...
	if (!ULevelInstanceSettings::Get()->IsLevelInstancePropertyOverrideSettingEnabled())
	{
		if (OutReason)
		{
			*OutReason = LOCTEXT("LevelInstanceNotEnabledPropertyOverrides", "Check the \"Enable Level Instance Property Overrides\" Editor Setting");
		}
		return false;
	}

	if (!ULevelInstanceSettings::Get()->IsPropertyOverridePolicyDefined())
	{
		if (OutReason)
		{
			*OutReason = LOCTEXT("LevelInstanceProvidePolicyClass", "Provide an override policy class in your DefaultEngine.ini file.\nFor ex. under [/Script/Engine.LevelInstanceSettings], add PropertyOverridePolicyClass=/Script/Engine.LevelInstancePropertyOverrideSamplePolicy");
		}
		return false;
	}
...
bool ULevelInstanceSubsystem::CommitLevelInstanceInternal(TUniquePtr<FLevelInstanceEdit>& InLevelInstanceEdit, bool bDiscardEdits, bool bDiscardOnFailure, TSet<FName>* DirtyPackages)
{
	...
	BroadcastOnLevelInstanceEditModeChanged(LevelInstance, ILevelInstanceEditorModule::ELevelInstanceEditMode::None);
...
void ULevelInstanceSubsystem::EditLevelInstancePropertyOverrides(ILevelInstanceInterface* LevelInstance, AActor* ContextActor)
{
	...
	// Load Edit LevelInstance level
	LevelInstance = GetLevelInstance(LevelInstanceID);
	BroadcastOnLevelInstanceEditModeChanged(LevelInstance, ILevelInstanceEditorModule::ELevelInstanceEditMode::Override);

	ULevelStreamingLevelInstanceEditorPropertyOverride::Load(LevelInstance);
...
bool ULevelInstanceSubsystem::CommitLevelInstancePropertyOverridesInternal(TUniquePtr<FPropertyOverrideEdit>& InPropertyOverrideEdit, bool bDiscardEdits)
{
	...
	BroadcastOnLevelInstanceEditModeChanged(LevelInstanceWithOverrides, ILevelInstanceEditorModule::ELevelInstanceEditMode::None);
