...
void ULevelStreamingLevelInstance::ResetLevelInstanceLoaders()
{
	...
	if (UWorld* OuterWorld = LoadedLevel ? LoadedLevel->GetTypedOuter<UWorld>() : nullptr)
	{
		...
		// Reset loaders for objects outered to LoadedLevel before detaching the OuterWorld linker.
		// ResetLoaders might trigger async loads, which can in turn cause further loads that require the OuterWorld linker to complete.
		// This might happen, for example, when subobjects are replaced in PostLoad.
		// If GC occurs between that replacement and the ResetLoaders call, the original object will be reloaded.
...
ULevelStreamingLevelInstance* ULevelStreamingLevelInstance::LoadInstance(ILevelInstanceInterface* LevelInstance)
{
	...
	{
		UE_LOGF(LogLevelInstance, Error, "Failed to load LevelInstance Actor '%ls' because that would cause a loop. Run Map Check for more details.", *LevelInstanceActor->GetPathName());
		return nullptr;
	}
	...
	{
		UE_LOGF(LogLevelInstance, Error, "Failed to load LevelInstance Actor '%ls' because it refers to an invalid package ('%ls'). Run Map Check for more details.", *LevelInstanceActor->GetPathName(), *LevelInstance->GetWorldAsset().GetLongPackageName());
		return nullptr;
	}
#endif
...
void ULevelStreamingLevelInstance::UnloadInstance(ULevelStreamingLevelInstance* LevelStreaming)
{
	...
	if (LevelStreaming->IsEditorWorldMode())
	{
		...
		ForEachObjectWithOuterBreakable(OuterWorld, [&bResetTrans](UObject* Obj)
		{
			...
			{
				bResetTrans = true;
				UE_LOGF(LogLevelInstance, Warning, "Found RF_Transactional object '%ls' while unloading Level Instance.", *Obj->GetPathName());
				return false;
			}
			return true;
		}, EGetObjectsFlags::IncludeNestedObjects);
