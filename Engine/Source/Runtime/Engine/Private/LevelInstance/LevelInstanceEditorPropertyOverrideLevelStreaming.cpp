...
void ULevelStreamingLevelInstanceEditorPropertyOverride::Unload(ULevelStreamingLevelInstanceEditorPropertyOverride* LevelStreaming)
{
	if (ULevel* LoadedLevel = LevelStreaming->GetLoadedLevel())
	{
		...
		// ObjectReferences holds hard refs to archetype actors for GC protection during
		// the editing session. Release them now so the archetype world package can be fully GC'd
		// by UnloadArchetypeWorld; without this the package persists and the next call to
		// LoadArchetypeWorld would fire: check(!FindPackage(nullptr, *BasePackageName)).
		LevelStreaming->ObjectReferences.Reset();
