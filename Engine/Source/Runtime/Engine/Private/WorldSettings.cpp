...
/** Allows globally disabling world bound checks. */
static int32 GDisableWorldBoundsChecks = 0;
static FAutoConsoleVariableRef CVarActorWorldBoundsOverride(
	TEXT("WorldBounds.DisableChecks"),
	GDisableWorldBoundsChecks,
	TEXT("If set to 1, will ignore all worlds bounds checks."));
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	bUseAutomaticUGCNavigation = false;
...
#if WITH_EDITOR
bool AWorldSettings::GetDynamicLoadingRangeScalingEnable() const
{
	// The per-project, per-user settings are stored for the Editor world, not the PIE world.
	UWorld* EditorWorld = (GEditor) ? GEditor->GetEditorWorldContext().World() : nullptr;

	// DynamicLoadingRangeScaling is enabled in WorldSettings, but can also be disabled via a per-user, per-map override
	return DynamicLoadingRangeScaling.bEnabled && EditorWorld && !GetDefault<UWorldPartitionEditorPerProjectUserSettings>()->GetForceDisableDynamicLoadingRangeScaling(EditorWorld);
}
#endif // WITH_EDITOR
...
bool AWorldSettings::AreWorldBoundsChecksEnabled() const
{
	return bEnableWorldBoundsChecks && !GDisableWorldBoundsChecks;
}
	...
	if (PropertyName == GET_MEMBER_NAME_CHECKED(AWorldSettings, DefaultReverbSettings) || PropertyName == GET_MEMBER_NAME_CHECKED(AWorldSettings, DefaultAmbientZoneSettings))
{
	...
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(AWorldSettings, NavigationSystemConfig))
	{
		...
		if (World && World->PersistentLevel)
		{
			...
			if (NavigationSystemConfig)
			{
				FNavigationSystem::AddNavigationSystemToWorld(*World, FNavigationSystemRunMode::EditorMode, NavigationSystemConfig);
			}
		}
	}
...
void AWorldSettings::AdjustNumberOfBookmarks()
{
	...
	{
		UE_LOGF(LogWorldSettings, Warning, "%ls: MaxNumberOfBookmarks cannot be below 0 (Value=%d). Defaulting to 10", *GetPathNameSafe(this), MaxNumberOfBookmarks);
		MaxNumberOfBookmarks = NumMappedBookmarks;
	}

	if (MaxNumberOfBookmarks < BookmarkArray.Num())
	{
		UE_LOGF(LogWorldSettings, Warning, "%ls: MaxNumberOfBookmarks set below current number of bookmarks. Clearing %d bookmarks.", *GetPathNameSafe(this), BookmarkArray.Num() - MaxNumberOfBookmarks);
	}
...
void AWorldSettings::SanitizeBookmarkClasses()
{
	if (UClass* ExpectedClass = GetDefaultBookmarkClass().Get())
	{
		...
		{
			UE_LOGF(LogWorldSettings, Warning, "%ls: Bookmarks found with invalid classes", *GetPathNameSafe(this));
		}
	}
	else
	{
		UE_LOGF(LogWorldSettings, Warning, "%ls: Invalid bookmark class, clearing existing bookmarks.", *GetPathNameSafe(this));
		DefaultBookmarkClass = UBookMark::StaticClass();
