...
static bool bEnableTransactionalTimers = true;
static FAutoConsoleVariableRef CVarTimerManagerEnableTransactionalTimers(
	TEXT("TimerManager.EnableTransactionalTimers"),
	bEnableTransactionalTimers,
	TEXT("If false, the AutoRTFM transactional timer path will be disabled."));

#ifndef UE_ENABLE_DUMPALLTIMERLOGSTHRESHOLD
#define UE_ENABLE_DUMPALLTIMERLOGSTHRESHOLD !UE_BUILD_SHIPPING
#endif

// Verbose debug logging - especially useful for debugging transactional behavior.
#define UE_TIMER_MANAGER_ENABLE_VERBOSE_LOGGING 0
#if UE_TIMER_MANAGER_ENABLE_VERBOSE_LOGGING
static_assert(!UE_BUILD_SHIPPING);
static unsigned GTimerManagerDebugNesting = 0;
static const char * GTimerManagerDebugNestingIndentation = "                                ";
#define UE_TIMER_MANAGER_DEBUG_LOG(MSG, ...) \
	UE_LOG(LogEngine, Log, TEXT("%hs:%-4d %hs %hs " MSG), __FILE__, __LINE__, \
		AutoRTFM::IsClosed() ? "[closed]" : "[open]  ", \
		GTimerManagerDebugNestingIndentation + (32 - 2 * GTimerManagerDebugNesting), \
		##__VA_ARGS__)
#define UE_TIMER_MANAGER_DEBUG_SCOPE(MSG, ...) \
	UE_TIMER_MANAGER_DEBUG_LOG(MSG " -- begin", ##__VA_ARGS__) \
	GTimerManagerDebugNesting++; \
	ON_SCOPE_EXIT { GTimerManagerDebugNesting--; UE_TIMER_MANAGER_DEBUG_LOG(MSG " -- end", ##__VA_ARGS__); }
#else
#define UE_TIMER_MANAGER_DEBUG_LOG(MSG, ...)
#define UE_TIMER_MANAGER_DEBUG_SCOPE(MSG, ...)
#endif
...
struct FTimerSourceList
{
	...
	{
		FString FunctionNameStr = "NotBound!";
		FString ObjectNameStr = "NotBound!";
	...
	void DumpEntries()
	{
		...
		UE_LOGF(LogEngine, Log, "Listing Expired Timer Stats for OwningGameInstance=%ls%ls", *GetPathNameSafe(OwningGameInstance), *AdditionalInfoAboutGameInstance);
		
		UE_LOGF(LogEngine, Log, "TotalExpires,RateAvg,PeakInFrame,ObjectOrClass,FunctionName");
		...
		{
			UE_LOGF(LogEngine, Log, "%llu,%.2f,%u,%ls", KVP.Value.TotalCount, KVP.Value.SumApproxRate / KVP.Value.TotalCount, KVP.Value.MaxPerFrame, *KVP.Key);
		}
...
namespace
{
	...
	/**
	* Returns true if the TimerData target is locally resident. When a target is locally resident, you can
	* execute the TimerDelegate without causing (an initial) migration. The delegate itself may cause other migrations.
	*/
	bool IsTimerTargetResident(const FTimerUnifiedDelegate& TimerDelegate)
	{
		using namespace UE::RemoteObject::Handle;

		FRemoteObjectId RemoteObjectId = UE::RemoteObject::Handle::GetRemoteObjectId([&TimerDelegate]()
		{
			const UObject* TargetObject;
			switch (TimerDelegate.VariantDelegate.GetIndex())
			{
				case FTimerDelegateVariant::IndexOfType<FTimerDelegate>():
					TargetObject = TimerDelegate.VariantDelegate.Get<FTimerDelegate>().GetUObject();
					break;

				case FTimerDelegateVariant::IndexOfType<FTimerDynamicDelegate>():
					TargetObject = TimerDelegate.VariantDelegate.Get<FTimerDynamicDelegate>().GetUObject();
					break;

				case FTimerDelegateVariant::IndexOfType<FTimerFunction>():
					// fallthrough -- TFunction does not know about an underlying UObject, so we always consider it local.
				default:
		...
		return GetResidence(RemoteObjectId) == EResidence::Local;
	}
	#endif
}
	...
	, OwningGameInstance(nullptr)
{
	...
	FModuleManager::Get().OnLibraryUnmount().AddRaw(this, &FTimerManager::RemoveExpiredTimers);
}

FTimerManager::~FTimerManager()
{
	FModuleManager::Get().OnLibraryUnmount().RemoveAll(this);
...
{
	UE_LOGF(LogEngine, Warning, "TimerManager %p on crashing delegate called, dumping extra information", this);

	UE_LOGF(LogEngine, Log, "------- %d Active Timers (including expired) -------", ActiveTimerHeap.Num());
	...
	UE_LOGF(LogEngine, Log, "------- %d Expired Active Timers -------", ExpiredActiveTimerCount);

	UE_LOGF(LogEngine, Log, "------- %d Paused Timers -------", PausedTimerSet.Num());
	...
	UE_LOGF(LogEngine, Log, "------- %d Pending Timers -------", PendingTimerSet.Num());
	...
	UE_LOGF(LogEngine, Log, "------- %d Total Timers -------", PendingTimerSet.Num() + PausedTimerSet.Num() + ActiveTimerHeap.Num() - ExpiredActiveTimerCount);

	UE_LOGF(LogEngine, Warning, "TimerManager %p dump ended", this);
}
...
{
	UE_TIMER_MANAGER_DEBUG_SCOPE("InternalSetTimer(InOutHandle: %s, InDelegate: %s)", *InOutHandle.ToString(), *InDelegate.ToString());
...
#if UE_AUTORTFM
		if (bEnableTransactionalTimers && AutoRTFM::IsClosed())
		{
			// Reserve a slot in Timers for this transactional timer.
			NewTimerHandle = GenerateReservedHandleForTransaction();

			// Add the timer to TransactionalTimers.
			NewTimerData.ExpireTime = FirstDelay;
			NewTimerData.Handle = NewTimerHandle;
			TransactionalTimers.Add(NewTimerHandle, FTransactionalTimer
				{
					.NewData = MoveTemp(NewTimerData),
					.Status = HasBeenTickedThisFrame() ? FTransactionalTimer::Active : FTransactionalTimer::Pending,
				});
			MaybeRegisterTransactionalCallbacks();
		}
		else
#endif  // UE_AUTORTFM
...
{
	UE_TIMER_MANAGER_DEBUG_SCOPE("InternalSetTimerForNextTick(InDelegate: %s)", *InDelegate.ToString());
	...
	if (bEnableTransactionalTimers && AutoRTFM::IsClosed())
	{
		...
		// Add the timer to TransactionalTimers.
		TransactionalTimers.FindOrAdd(NewTimerHandle, FTransactionalTimer
			{
				.NewData = MoveTemp(NewTimerData),
				.Status = bQueueForCurrentFrame ? FTransactionalTimer::Active : FTransactionalTimer::Pending,
			});
		MaybeRegisterTransactionalCallbacks();
	}
	else
	...
	UE_TIMER_MANAGER_DEBUG_LOG("NewTimerHandle: %s", *NewTimerHandle.ToString());
	return NewTimerHandle;
}

void FTimerManager::InternalClearTimer(FTimerHandle InHandle)
{
	UE_TIMER_MANAGER_DEBUG_LOG("InternalClearTimer(InHandle: %s)", *InHandle.ToString());
	...
	if (bEnableTransactionalTimers && AutoRTFM::IsClosed())
	{
		// Add or replace the transactional timer with the canceled status.
		FTransactionalTimer& Timer = TransactionalTimers.FindOrAdd(InHandle);
		Timer.NewData.Reset();
		Timer.Status = FTransactionalTimer::Canceled;
		MaybeRegisterTransactionalCallbacks();
		return;
	}
#endif  // UE_AUTORTFM
	...
	UE_TIMER_MANAGER_DEBUG_LOG("Status: %d", static_cast<int>(Data.Status));
	switch (Data.Status)
	{
		...
		case ETimerStatus::ReservedForTransaction:
			// Only the on-commit or on-abort handler may remove a
			// transactionally-reserved timer.
			check(AutoRTFM::IsCommitting() || AutoRTFM::IsAborting());
			RemoveTimer(InHandle);
			break;
...
{
	UE_TIMER_MANAGER_DEBUG_SCOPE("InternalClearAllTimers(Object: %p)", Object);
	...
	if (bEnableTransactionalTimers && AutoRTFM::IsClosed())
	{
		for (TTuple<FTimerHandle, FTransactionalTimer>& It : TransactionalTimers)
		{
			if (TOptional<FTimerData>& NewData = It.Value.NewData; NewData.IsSet())
			{
				if (NewData->TimerDelegate.GetBoundObject() == Object)
				{
					It.Value.NewData.Reset();
					It.Value.Status = FTransactionalTimer::Canceled;
				}
			}
		}
	}
...
{
	UE_TIMER_MANAGER_DEBUG_SCOPE("PauseTimer(InHandle: %s)", *InHandle.ToString());
	...
	if (bEnableTransactionalTimers && AutoRTFM::IsClosed())
	{
		// Add or replace the transactional timer with the paused status (if it
		// didn't exist with a canceled status).
		FTransactionalTimer& Timer = TransactionalTimers.FindOrAdd(InHandle);
		if (Timer.Status != FTransactionalTimer::Canceled)
		{
			Timer.Status = FTransactionalTimer::Paused;
		}
		MaybeRegisterTransactionalCallbacks();
		return;
	}
	...
	UE_TIMER_MANAGER_DEBUG_LOG("Status: %d", PreviousStatus);
...
{
	UE_TIMER_MANAGER_DEBUG_SCOPE("UnPauseTimer(InHandle: %s)", *InHandle.ToString());
	...
	if (bEnableTransactionalTimers && AutoRTFM::IsClosed())
	{
		// Add or replace the transactional timer with the pending status (if it
		// didn't exist with a canceled status).
		FTransactionalTimer& Timer = TransactionalTimers.FindOrAdd(InHandle);
		if (Timer.Status != FTransactionalTimer::Canceled)
		{
			Timer.Status = HasBeenTickedThisFrame() ? FTransactionalTimer::Active : FTransactionalTimer::Pending;
		}
		MaybeRegisterTransactionalCallbacks();
		return;
	}
...
{
	AutoRTFM::UnreachableIfClosed("FTimerManager::Tick cannot be called from the closed");

	SCOPED_NAMED_EVENT(FTimerManager_Tick, FColor::Orange);
	CSV_SCOPED_TIMING_STAT_EXCLUSIVE(TimerManager);
	UE_TIMER_MANAGER_DEBUG_SCOPE("FTimerManager::Tick");
	...
	UE_SUPPRESS(LogEngine, Warning,
	{
		if (DumpAllTimerLogsThreshold > 0 && ActiveTimerHeap.Num() > DumpAllTimerLogsThreshold)
		{
			...
			if(!bAlreadyLogged)
			{
				...
				UE_LOGF(LogEngine, Warning, "Number of active Timers (%d) has exceeded DumpAllTimerLogsThreshold (%d)!  Dumping all timer info to log:", ActiveTimerHeap.Num(), DumpAllTimerLogsThreshold);
	...
	while (ActiveTimerHeap.Num() > 0)
	{
		...
		{
			UE_TIMER_MANAGER_DEBUG_LOG("Removing top timer (%s) with status ActivePendingRemoval", *TopHandle.ToString());
		...
		if (InternalTime > Top->ExpireTime)
		{
...
#if UE_WITH_REMOTE_OBJECT_HANDLE && UE_AUTORTFM
				bool bExecuteTransactionally = false;
				FName WorkName;
				switch (Top->TimerDelegate.VariantDelegate.GetIndex())
				{
				case FTimerDelegateVariant::IndexOfType<FTimerDelegate>():
					WorkName = FName{ TEXT("TimerManager::Execute FTimerDelegate") };
					bExecuteTransactionally = true;
					break;
				case FTimerDelegateVariant::IndexOfType<FTimerDynamicDelegate>():
					WorkName = FName{ TEXT("TimerManager::Execute FTimerDynamicDelegate") };
					bExecuteTransactionally = true;
					break;
				default:
					break;
				}
				if (bExecuteTransactionally)
				{
					UE_TIMER_MANAGER_DEBUG_SCOPE("ExecuteTransactional(%s, %s)", *CurrentlyExecutingTimer.ToString(), *Top->TimerDelegate.ToString());
					UE::RemoteExecutor::ExecuteTransactional(WorkName, [this, Handle = CurrentlyExecutingTimer]
					{
						checkf(Handle == CurrentlyExecutingTimer,
							TEXT("CurrentlyExecutingTimer changed between retries: Was=%s, Now=%s"),
							*Handle.ToString(), *CurrentlyExecutingTimer.ToString());
						FTimerData* Top = FindTimer(CurrentlyExecutingTimer);
						checkf(Top && !WillRemoveTimerAssert(CurrentlyExecutingTimer), TEXT("RemoveTimer(CurrentlyExecutingTimer) - due to fail before Execute()"));
...
#endif
				{
					UE_TIMER_MANAGER_DEBUG_SCOPE("Execute non-transactionally(%s, %s)", *CurrentlyExecutingTimer.ToString(), *Top->TimerDelegate.ToString());
					checkf(!WillRemoveTimerAssert(CurrentlyExecutingTimer),
						TEXT("RemoveTimer(Top = %p, CurrentlyExecutingTimer = %s) - due to fail before Execute()"),
						Top, *CurrentlyExecutingTimer.ToString());
					Top->TimerDelegate.Execute();
				}

				// Update Top pointer, in case it has been invalidated by the Execute call
				Top = FindTimer(CurrentlyExecutingTimer);
				checkf(!Top || !WillRemoveTimerAssert(CurrentlyExecutingTimer),
					TEXT("RemoveTimer(Top = %p, CurrentlyExecutingTimer = %s) - due to fail after Execute()"),
					Top, *CurrentlyExecutingTimer.ToString());
			...
			if (DumpTimerLogsThreshold > 0.f && !bDumpTimerLogsThresholdExceeded)
			{
				...
				if (DeltaT >= DumpTimerLogsThreshold)
				{
                    ...
                    ++NbExpiredTimers;
					UE_LOGF(LogEngine, Log, "TimerManager's time threshold of %.2fms exceeded with a deltaT of %.4f, dumping current timer data.", DumpTimerLogsThreshold, DeltaT);
					...
					{
						UE_LOGF(LogEngine, Log, "There was no timer data for the first timer after exceeding the time threshold!");
					}
				}
			}
			...
			{
				#if UE_WITH_REMOTE_OBJECT_HANDLE
				// IsBound can cause a migration (and we are not inside a transaction).
				// If the Delegate's Target is non-Resident, then we should reschedule and only do IsBound() when it's Resident.
				const bool bForceLoopingTimerReschedule = !IsTimerTargetResident(Top->TimerDelegate);
				#else
				constexpr bool bForceLoopingTimerReschedule = false;
				#endif
				...
				if (Top->bLoop && (bForceLoopingTimerReschedule || !Top->bRequiresDelegate || Top->TimerDelegate.IsBound()))
				{
				...
				{
					UE_TIMER_MANAGER_DEBUG_LOG("Removing timer (%s) after execution", *CurrentlyExecutingTimer.ToString());
	...
	if (NbExpiredTimers > MaxExpiredTimersToLog)
	{
		UE_LOGF(LogEngine, Log, "TimerManager's caught %d Timers exceeding the time threshold. Only the first %d were logged.", NbExpiredTimers, MaxExpiredTimersToLog);
	}
...
void FTimerManager::ListTimers() const
{
	...
	UE_LOGF(LogEngine, Log, "------- %d Active Timers -------", ValidActiveTimers.Num());
	for (const FTimerData* Data : ValidActiveTimers)
	{
		check(Data);
		DescribeFTimerDataSafely(*GLog, *Data);
	...
	UE_LOGF(LogEngine, Log, "------- %d Total Timers -------", PendingTimerSet.Num() + PausedTimerSet.Num() + ValidActiveTimers.Num());
}

FTimerHandle FTimerManager::AddTimer(FTimerData&& TimerData)
{
	UE_TIMER_MANAGER_DEBUG_SCOPE("AddTimer()");
	...
	{
		AddObjectToTimers(Result, TimerIndicesByObjectKey);
	}

	return Result;
}

void FTimerManager::AddObjectToTimers(FTimerHandle TimerHandle, const void *TimerIndicesByObjectKey)
{
	UE_TIMER_MANAGER_DEBUG_SCOPE("AddObjectToTimers(TimerHandle: %s, TimerIndicesByObjectKey: %p)", *TimerHandle.ToString(), TimerIndicesByObjectKey);

	checkSlow(TimerIndicesByObjectKey)
	TSet<FTimerHandle>& HandleSet = ObjectToTimers.FindOrAdd(TimerIndicesByObjectKey);

	bool bAlreadyExists = false;
	HandleSet.Add(TimerHandle, &bAlreadyExists);
	checkf(!bAlreadyExists, TEXT("A timer with this handle and object has already been added! (%s)"), *TimerHandle.ToString());
}

void FTimerManager::RemoveTimer(FTimerHandle Handle)
{
	UE_TIMER_MANAGER_DEBUG_SCOPE("RemoveTimer(Handle: %s)", *Handle.ToString());
	...
	{
		UE_TIMER_MANAGER_DEBUG_LOG("TimerIndicesByObjectKey: %p", TimerIndicesByObjectKey);
...
#if UE_AUTORTFM
	if (bEnableTransactionalTimers && AutoRTFM::IsTransactional() && TransactionalTimers.Find(Handle))
	{
		// Removing a timer in the transactional open that has been modified in
		// the transaction. The timer slot cannot be released until the
		// transaction is complete, so change the status to ReservedForTransaction.
		FTimerData Reserved{};
		Reserved.Handle = Handle;
		Reserved.Status = ETimerStatus::ReservedForTransaction;
		Timers[Handle.GetIndex()] = MoveTemp(Reserved);
	}
	else
#endif  // UE_AUTORTFM
	{
...
void FTimerManager::RemoveExpiredTimers()
{
	TArray<FTimerHandle> HandlesToClear;

	for (const FTimerData& TimerData : Timers)
	{
		if (!TimerData.TimerDelegate.IsBound())
		{
			HandlesToClear.Add(TimerData.Handle);
		}
	}	

	for (FTimerHandle Handle : HandlesToClear)
	{
		int32 IndexIndex = ActiveTimerHeap.Find(Handle);
...
#if UE_AUTORTFM
void FTimerManager::MaybeRegisterTransactionalCallbacks()
{
	if (bRegisteredTransactionalCallbacks)
	{
		return;
	}

	bRegisteredTransactionalCallbacks = true;

	AutoRTFM::OnCommit([this]
	{
		UE_TIMER_MANAGER_DEBUG_SCOPE("OnCommit()");
		
		for (TTuple<FTimerHandle, FTransactionalTimer>& It : TransactionalTimers)
		{
			const FTimerHandle Handle = It.Key;
			FTransactionalTimer& TransactionalTimer = It.Value;
			UE_TIMER_MANAGER_DEBUG_LOG("TransactionalTimer: {%s, Handle: %s, Status: %d}",
				TransactionalTimer.NewData ? TEXT("with-data") : TEXT("no-data"),
				*Handle.ToString(), (int)TransactionalTimer.Status);

			if (TransactionalTimer.NewData)
			{
				// Timer was created during the transaction.
				// Move the transactional timer to the non-transactional Timers array.
				FTimerData& TimerData = Timers[Handle.GetIndex()];
				check(Handle == TimerData.Handle);
				check(Handle == TransactionalTimer.NewData->Handle);
				TimerData = MoveTemp(*TransactionalTimer.NewData);

				switch (TransactionalTimer.Status)
				{
				case FTransactionalTimer::Active:
					TimerData.ExpireTime += InternalTime;
					TimerData.Status = ETimerStatus::Active;
					ActiveTimerHeap.HeapPush(TimerData.Handle, FTimerHeapOrder(Timers));
					break;

				case FTransactionalTimer::Pending:
					TimerData.Status = ETimerStatus::Pending;
					PendingTimerSet.Add(TimerData.Handle);
					break;

				case FTransactionalTimer::Paused:
					TimerData.Status = ETimerStatus::Paused;
					PausedTimerSet.Add(Handle);
					break;

				case FTransactionalTimer::Canceled:
					checkf(false, TEXT("FTransactionalTimer::NewData should be empty when Status is Canceled"));
					break;
				}

				if (const void* TimerIndicesByObjectKey = TimerData.TimerDelegate.GetBoundObject())
				{
					TimerData.TimerIndicesByObjectKey = TimerIndicesByObjectKey;
					AddObjectToTimers(Handle, TimerIndicesByObjectKey);
				}
			}
			else
			{
				...
				case FTransactionalTimer::Pending:
				{
					// The code in this block mirrors UnPauseTimer() but with
					// the HasBeenTickedThisFrame() conditional replaced with
					// TransactionalTimer.Status
					FTimerData* TimerToUnPause = FindTimer(Handle);
					if (TimerToUnPause && TimerToUnPause->Status == ETimerStatus::Paused)
					{
						// Move it out of paused list and into proper TArray
						if (TransactionalTimer.Status == FTransactionalTimer::Active)
						{
							// Convert from time remaining back to a valid ExpireTime
							TimerToUnPause->ExpireTime += InternalTime;
							TimerToUnPause->Status = ETimerStatus::Active;
		...
		TransactionalTimers.Empty();
		bRegisteredTransactionalCallbacks = false;
	});

	AutoRTFM::OnPreAbort([this]
	{
		UE_TIMER_MANAGER_DEBUG_SCOPE("OnPreAbort()");
		
		// Remove transactionally reserved timers.
		for (auto& It : TransactionalTimers)
		{
			const FTimerHandle& Handle = It.Key;
			const int32 Index = Handle.GetIndex();
			FTimerData* const Timer = FindTimer(Handle);
			if (Timer && Timer->Status == ETimerStatus::ReservedForTransaction)
			{
				RemoveTimer(Handle);
			}
		}
		// Note: TransactionalTimers & bRegisteredTransactionalCallbacks will be
		// cleared automatically by AutoRTFM memory rollback.
	});
}

FTimerHandle FTimerManager::GenerateReservedHandleForTransaction()
{
	FTimerData Reservation;
	Reservation.Status = ETimerStatus::ReservedForTransaction;
	int32 NewIndex = Timers.Add(MoveTemp(Reservation));
	FTimerHandle Handle = GenerateHandle(NewIndex);
	Timers[NewIndex].Handle = Handle;
	return Handle;
}
#endif  // UE_AUTORTFM
...
#if UE_TIMER_MANAGER_ENABLE_VERBOSE_LOGGING
#undef UE_TIMER_MANAGER_DEBUG_LOG
#undef UE_TIMER_MANAGER_DEBUG_SCOPE
#endif
#undef UE_TIMER_MANAGER_ENABLE_VERBOSE_LOGGING
