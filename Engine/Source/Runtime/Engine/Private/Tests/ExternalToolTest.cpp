...
bool FWaitForProcessToCompleteLatentCommand::Update()
{
	if( ProcessInfo.ProcessHandle.IsValid() )
	{
		...
		if( ProcessOutput.Len() > 0 )
		{
			...
			{
				UE_LOGF(LogBaseAutomationTests, Log, "%ls", *OutputLines[i]);
			}
		}
		...
		if( ReturnCode != 0 )
		{
			UE_LOGF(LogBaseAutomationTests, Error, "External Tool failed with error code: %i",ReturnCode);
		}
	}
...
bool FRunExternalToolTest::RunTest(const FString& Parameters)
{
	...
	if( Commands.Num() != 3 )
	{
		UE_LOGF(LogBaseAutomationTests, Error, "ERROR Parsing commands for external tool: %ls",*Parameters);
		return false;
	}
	...
	{
		UE_LOGF(LogBaseAutomationTests, Error, "Failed to launch executable (%ls) for external tool %ls",*Executable, *Parameters);
		FPlatformProcess::ClosePipe(ProcessInfo.ReadPipe, ProcessInfo.WritePipe);
		return false;
	}
