...
#if WITH_AUTOMATION_TESTS
		UE_LOGF(LogEngineAutomationTests, Verbose, "CVar '%ls' was set with the value '%ls'.", *ConsoleVariableName, *Value);
...
#if WITH_AUTOMATION_TESTS
			UE_LOGF(LogEngineAutomationTests, Verbose, "CVar '%ls' was restored with the original value '%ls'.", *ConsoleVariableName, *OriginalValue);
#endif // WITH_AUTOMATION_TESTS
		}
...
namespace AutomationCommon
{
	...
	FAutomationScreenshotData BuildScreenshotData(const FString& MapOrContext, const FString& TestName, const FString& ScreenShotName, int32 Width, int32 Height)
	{
		...
		Data.Commit = FEngineVersion::Current().HasChangelist() ? FString::FromInt(FEngineVersion::Current().GetChangelist()) : FString();
	...
	TArray<uint8> CaptureFrameTrace(const FString& MapOrContext, const FString& TestName)
	{
			...
			&& FModuleManager::Get().IsModuleLoaded("RenderDocPlugin"))
		{
			...
			UE_LOGF(LogEngineAutomationTests, Log, "Taking Frame Trace: %ls", *TempCaptureFilePath);
			...
			{
				UE_LOGF(LogEngineAutomationTests, Warning, "Failed taking frame trace: %ls", *TempCaptureFilePath);
			}
		}
	...
	public:
		...
		bool Update() override
		{
			if (!Initiate)
			{
				...
				UE_LOGF(LogEditorAutomationTests, Log, "Requesting image %ls to be compared.", *ComparisonParameters.ScreenshotPath);
...
{
	UE_LOGF(LogEditorAutomationTests, Log, "%ls", *LogText);
	return true;
}
...
bool FLoadGameMapCommand::Update()
{
	...
	UE_LOGF(LogEngineAutomationTests, Log, "Loading Map Now. '%ls'", *MapName);
...
bool FWaitForInteractiveFrameRate::Update()
{
	...
	{
		UE_LOGF(LogEngineAutomationLatentCommand, Log, "FWaitForInteractiveFrameRate: FApp::CanEverRender() == false. Passing wait immediately.");
		return true;
	}

	if (StartTimeOfWait == 0)
	{
		UE_LOGF(LogEngineAutomationLatentCommand, Log, "FWaitForInteractiveFrameRate: Starting wait for framerate of >= %.f FPS", DesiredFrameRate);
	...
	{
		UE_LOGF(LogEngineAutomationLatentCommand, Display, "FWaitForInteractiveFrameRate: After %.f secs Current: %.02f, Average: %.02f, Desired: %.02f", ElapsedWaitTime, CurrentFrameRate, AverageFrameRate, DesiredFrameRate);
	...
	*/
	...
	if (!AtDesiredFramerate)
	{
		if (StartTimeOfAcceptableFrameRate > 0)
		{
			...
			UE_LOGF(LogEngineAutomationLatentCommand, Display, "FWaitForInteractiveFrameRate: Dropped below minimal framerate of %.f after %.02f secs. Now %.02f FPS ", 
	...
	else
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogEngineAutomationLatentCommand, Log, "FWaitForInteractiveFrameRate: Hit %.02f FPS for %.f seconds after %.f seconds of waiting", AverageFrameRate, TimeAtFrameRate, ElapsedWaitTime);
				return true;
			}
		}
	}
	...
	if (ElapsedWaitTime >= MaxWaitTime)
	{
		...
		UE_LOGF(LogEngineAutomationLatentCommand, Display, "%ls", *Msg);
	...
	if (TimeNow - LastReportTime >= kReportInterval)
	{
		if (AtDesiredFramerate)
		{
			UE_LOGF(LogEngineAutomationLatentCommand, Display, "FWaitForInteractiveFrameRate: Waited %.f seconds. Will timeout in %.f. Current FPS=%.f for %.02f secs", 
				ElapsedWaitTime, MaxWaitTime- ElapsedWaitTime, AverageFrameRate, TimeAtFrameRate);
		}
		else
		{
			UE_LOGF(LogEngineAutomationLatentCommand, Display, "FWaitForInteractiveFrameRate: Waited %.f seconds. Will timeout in %.f. Current FPS=%.f ", 
...
{
	UE_LOGF(LogEngineAutomationLatentCommand, Log, "Executing the console command: '%ls'", *ExecCommand);
...
bool FStreamAllResourcesLatentCommand::Update()
{
	...
	{
		UE_LOGF(LogEngineAutomationLatentCommand, Warning, "StreamAllResources() waited for %.2fs but %d resources are still in flight.", Time - LocalStartTime, GStreamAllResourcesStillInFlight);
	}
	else
	{
		UE_LOGF(LogEngineAutomationLatentCommand, Log, "StreamAllResources() waited for %.2fs (max duration: %.2f).", Time - LocalStartTime, Duration);
	}
...
bool FExecWorldStringLatentCommand::Update()
{
	...
	UE_LOGF(LogEngineAutomationLatentCommand, Log, "Running Exec Command. '%ls'", *ExecCommand);
...
bool FWaitForShadersToFinishCompilingInGame::Update()
{
	...
	if (ShadersCompiling || AssetsCompiling)
	{
		if (TimeNow - LastReportTime > 5.0)
		{
			...
			if (AssetsCompiling)
			{
				UE_LOGF(LogEditorAutomationTests, Log, "Waiting for %i assets to finish.", FAssetCompilingManager::Get().GetNumRemainingAssets());
			}
		}
...
{
	UE_LOGF(LogEngineAutomationTests, Log, "%ls", *LogText);
	return true;
}
...
bool FProjectMapsCycleTest::RunTest(const FString& Parameters)
{
	...
	if (FParse::Value(FCommandLine::Get(), TEXT("map="), ParsedMapName))
	{
		...
		for (const FString& Map : MapList)
		{
				...
				FPaths::GetExtension(ActualName, /*bIncludeDot=*/true) == FPackageName::GetMapPackageExtension())
			{
				...
				UE_LOGF(LogEngineAutomationTests, Display, "Found Map %ls on command line. Cycle Test will be use this map", *ActualName);
			}
			else
			{
				UE_LOGF(LogEngineAutomationTests, Fatal, "Cound not find package for Map '%ls' specified on command line.", *ActualName);
			}
		}
	}
	...
	if (CycleMaps.Num() == 0)
	{
	...
	#endif
		{
			...
			if (MapSettings->GetGameDefaultMap().Len())
			{
				...
				UE_LOGF(LogEngineAutomationTests, Display, "No MapsToCycle specified in DefaultEngine.ini [/Script/Engine.AutomationTestSettings]. Using GameStartup Map %ls", *StartupMap);
	...
	if (CycleMaps.Num() == 0)
	{
		UE_LOGF(LogEngineAutomationTests, Fatal, "No automation or default maps are configured for cycling!");
	}
