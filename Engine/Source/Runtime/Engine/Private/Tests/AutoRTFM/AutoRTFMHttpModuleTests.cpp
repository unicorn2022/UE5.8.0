...
bool FAutoRTFMHttpModuleNetworking::RunTest(const FString& Parameters)
{
	...
	auto IssueHttpRequest = [&] AUTORTFM_ENABLE (FHttpModule& Module) -> void
	{
...
bool FAutoRTFMHttpRequestPreservesSettings::RunTest(const FString& Parameters)
{
	...
	auto DoCheck = [&](AUTORTFM_IMPLICIT_ENABLE auto&& SetterFn, AUTORTFM_IMPLICIT_ENABLE auto&& GetterFn) -> void
	{
	...
	DoCheck(
		...
		[&](IHttpRequest& Request)
		{
			...
			TestTrueExpr(Request.GetEffectiveURL() == FString());
			TestTrueExpr(Request.GetVerb() == FString("GET"));
