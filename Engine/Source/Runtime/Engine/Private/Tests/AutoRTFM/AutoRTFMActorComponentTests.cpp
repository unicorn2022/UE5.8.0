...
{
	FName Name = "MyObjectToBeReplaced";
...
AUTORTFM_ACTOR_COMPONENT_TEST(ReconstructActorThenObtainWeakPtr) // SOL-7678
{
	AutoRTFM::Testing::Abort([&]
	{
		...
		// with the AutoRTFM sanitizer disabled. This is open-write is slightly
		// iffy, but can be considered part of the UObject construction / reconstruction
		...
		// The AutoRTFM sanitizer would catch the closed write followed by the
		// open write. The applied fix for this is to disable the AutoRTFM
		// sanitizer on this open, and to explicitly record the write. To the
		// AutoRTFM sanitizer this behaves like a regular closed write, despite
		// the logic using atomic CAS which is not permitted in the closed.
