...
UAsyncLoadingTests_Shared::FOnConstructDelegate UAsyncLoadingTests_Shared::OnConstruct;
...
void FLoadingTestsScope::CreateObjects()
{
	...
	Package4 = CreatePackage();
	Object4 = NewObject<UAsyncLoadingTests_Shared>(Package4, ObjectName, RF_Public | RF_Standalone);
}
...
{
	// Remove any asset path redirections that were registered for test packages
	// (e.g., from ObjectRedirectors created by Rename operations and picked up by the asset registry).
	// These persist in GRedirectCollector across tests and would corrupt soft object path resolution.
#if WITH_EDITOR
	for (const FString& PackageName : PackageNames)
	{
		FSoftObjectPath ObjectPath(PackageName + TEXT(".") + ObjectName);
		GRedirectCollector.RemoveAssetPathRedirection(ObjectPath);
	}
#endif

	GarbageCollect();

	// Delete on-disk package files to avoid stale assets leaking between tests
	for (const FString& PackageName : PackageNames)
	{
	...
	UAsyncLoadingTests_Shared::OnConstruct.Unbind();
