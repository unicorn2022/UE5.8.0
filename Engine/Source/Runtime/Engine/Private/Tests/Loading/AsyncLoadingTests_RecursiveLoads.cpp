...
bool FLoadingTests_RecursiveLoads_FromSerialize::RunTest(const FString& Parameters)
{
	...
	UAsyncLoadingTests_Shared::OnSerialize.BindLambda(
		...
		{
			// Loading from serialize is not supported in the multithreaded workers as it can lead to deadlocks.
			// Classes that end up doing it should be added to the .ini (search ParallelPreloadExclusions for examples) to prevent them from running wide.
...
bool FLoadingTests_RecursiveLoads_FullFlushFrom_Serialize::RunTest(const FString& Parameters)
{
	...
	AddExpectedMessage(TEXT("to avoid deadlock when it was recursively flushed by package"), EAutomationExpectedErrorFlags::Contains);
...
/**
 * These tests validate loading an object that loads itself synchronously during postload.
 */
IMPLEMENT_CUSTOM_SIMPLE_AUTOMATION_TEST(
	FLoadingTests_RecursiveLoads_LoadSelfFromPostLoad_Direct,
	FLoadingTests_RecursiveLoads_Base,
	TEXT("System.Engine.Loading.RecursiveLoads.LoadSelfFromPostLoad_Direct"),
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter
)
bool FLoadingTests_RecursiveLoads_LoadSelfFromPostLoad_Direct::RunTest(const FString& Parameters)
{
	FLoadingTestsScope LoadingTestScope(this,
		[](FLoadingTestsScope& TestScope)
		{
			TestScope.Object1->SoftReference = TestScope.Object1;
		}
	);
	...
	LoadPackage(nullptr, LoadingTestScope.PackagePath1, LOAD_None);

	UAsyncLoadingTests_Shared* LocalObject1 = FindObject<UAsyncLoadingTests_Shared>(nullptr, LoadingTestScope.ObjectPath1);
	TestTrue(TEXT("The object should have been properly loaded recursively"), LocalObject1 != nullptr);
	if (LocalObject1)
	{
		TestFalse(TEXT("The object should have been properly loaded recursively"), LocalObject1->HasAnyFlags(RF_NeedLoad | RF_NeedPostLoad));
	}
...
IMPLEMENT_CUSTOM_SIMPLE_AUTOMATION_TEST(
	FLoadingTests_RecursiveLoads_LoadSelfFromPostLoad_Indirect,
	FLoadingTests_RecursiveLoads_Base,
	TEXT("System.Engine.Loading.RecursiveLoads.LoadSelfFromPostLoad_Indirect"),
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter
)
bool FLoadingTests_RecursiveLoads_LoadSelfFromPostLoad_Indirect::RunTest(const FString& Parameters)
{
	FLoadingTestsScope LoadingTestScope(this,
		[](FLoadingTestsScope& TestScope)
		{
			// Soft-circular reference back to Object1
			TestScope.Object1->SoftReference = TestScope.Object2;
			TestScope.Object2->HardReference = TestScope.Object1;
		}
	);
	...
	UAsyncLoadingTests_Shared* LocalObject2 = FindObject<UAsyncLoadingTests_Shared>(nullptr, LoadingTestScope.ObjectPath2);
	TestTrue(TEXT("The object should have been properly loaded recursively"), LocalObject2 != nullptr);
	if (LocalObject2)
	{
		TestFalse(TEXT("The object should have been properly loaded recursively"), LocalObject2->HasAnyFlags(RF_NeedLoad | RF_NeedPostLoad));
	}
