...
{
	const FString DummyTestName = "Automation.DummyTestVariable";
...
#if DUMP_EXPECTED_TRANSFORMS
		UE_LOGF(LogTemp, Log, "\tFTransform(");
		UE_LOGF(LogTemp, Log, "\t\tFQuat(%.8ff, %.8ff, %.8ff, %.8ff),", ChildActor->GetActorQuat().X, ChildActor->GetActorQuat().Y, ChildActor->GetActorQuat().Z, ChildActor->GetActorQuat().W);
		UE_LOGF(LogTemp, Log, "\t\tFVector(%.8ff, %.8ff, %.8ff),", ChildActor->GetActorLocation().X, ChildActor->GetActorLocation().Y, ChildActor->GetActorLocation().Z);
		UE_LOGF(LogTemp, Log, "\t\tFVector(%.8ff, %.8ff, %.8ff)", ChildActor->GetActorScale3D().X, ChildActor->GetActorScale3D().Y, ChildActor->GetActorScale3D().Z);
		UE_LOGF(LogTemp, Log, "\t),");
#endif
...
constexpr auto ExampleTag = "[TestExampleTag][MustPass]";
...
constexpr auto ComplexTags1 = "[TestExampleTagComplex][FirstExample][MustPass]";
constexpr auto ComplexTags2 = "[TestExampleTagComplex][SecondExample][MustPass]";
