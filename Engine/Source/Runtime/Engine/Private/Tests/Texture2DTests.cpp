...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FTexture2DMissingPreEditChange, "System.Engine.Texture2D.MissingPreEditChange", TestFlags)
bool FTexture2DMissingPreEditChange::RunTest(const FString& Parameters)
{
	constexpr int32 TextureSize = 2 * 1024;
	constexpr int32 NumIteration = 3;
	...
	auto DoTest =
		[this](bool bVirtualStreaming)
		{
			...
			for (int32 Iteration = 0; Iteration < NumIteration; ++Iteration)
			{
				TRACE_CPUPROFILER_EVENT_SCOPE(FTexture2DMissingPreEditChange::Iteration);
				...
				{
					TRACE_CPUPROFILER_EVENT_SCOPE(FTexture2DMissingPreEditChange::PreEditChange);
					TestTexture->PreEditChange(nullptr);
				}
				...
				{
					TRACE_CPUPROFILER_EVENT_SCOPE(FTexture2DMissingPreEditChange::WriteSource);
					TestTexture->Source.Init(TextureSize, TextureSize, 1, 1, TSF_BGRA8);
					int32* Data = (int32*)TestTexture->Source.LockMip(0);
				...
				// This will start a build async
				{
					TRACE_CPUPROFILER_EVENT_SCOPE(FTexture2DMissingPreEditChange::PostEditChange);
					TestTexture->PostEditChange();
				}
				
				// Overwrite source again without calling PreEditChange, this should log some errors
				{
					TRACE_CPUPROFILER_EVENT_SCOPE(FTexture2DAsyncCompileCancelation::OverwriteWriteSource);
					TestTexture->Source.Init(TextureSize, TextureSize, 1, 1, TSF_BGRA8);
					int32* Data = (int32*)TestTexture->Source.LockMip(0);
				...
				{
					TRACE_CPUPROFILER_EVENT_SCOPE(FTexture2DMissingPreEditChange::GC);
					CollectGarbage(GARBAGE_COLLECTION_KEEPFLAGS);
				}
	...
	AddExpectedMessage(TEXT("missing call to PreEditChange"), EAutomationExpectedErrorFlags::Contains, 0 /* Must be seen at least once for the test to pass*/);
	{
		// Normal and VT textures have different cancelation points
		DoTest(false /*bVirtualStreaming*/);
		DoTest(true  /*bVirtualStreaming*/);
