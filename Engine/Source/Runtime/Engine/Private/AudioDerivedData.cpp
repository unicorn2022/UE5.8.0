...
static FString GetSoundWaveRawDataHash(const USoundWave& InWave)
{
	const USoundWave::FEditorAudioBulkData& RawData = InWave.RawData;
	return LexToString(RawData.RawData.GetPayloadId());
}

static FString GetWaveformTransformationHash(const USoundWave& InWave)
{
	// Hash the SoundWave waveform transformations 
	FString WaveformTransformationHash;

	const TArray<TObjectPtr<UWaveformTransformationBase>>& Transformations = InWave.Transformations;
	
	for (const TObjectPtr<UWaveformTransformationBase>& Transformation : Transformations)
	{
		if (Transformation)
		{
			WaveformTransformationHash += Transformation->GetTransformationHash();
		}
	}

	return WaveformTransformationHash;
}
	...
	)
{
	...
	UE_LOGF(LogAudio, Verbose, "%ls  Derived Data: %d bytes", *LogString, RawDerivedData.Num());
...
class FStreamedAudioCacheDerivedDataWorker : public FNonAbandonableTask
{
	                        ...
	                        const uint32 InMaxChunkLengthInFrames) const
	{
		...
		{	
			...
			if (bOverSize || bOverLength)
			{
				...
				UE_LOGF(LogAudio, Verbose, "Adding Chunk %d because (%ls): Blocks=%d (%u bytes), AudioFrames=%u (%2.2f seconds), SeekTableEntries=%d (%d bytes), ChunkSize=%u bytes, PercentFull=%2.2f, Remaining=%u bytes",
			...
			UE_LOGF(LogAudio, VeryVerbose, "\tAdding Block: Chunk_Block_Count=%d, BlockSize=%d, ChunkSize=%lld", BlockCount, BlockSize, Current - ChunkStart);
		}
		...
		if (Current - ChunkStart)
		{
			...
			UE_LOGF(LogAudio, Verbose, "Adding FINAL Chunk %d because (%ls): Blocks=%d (%u bytes), AudioFrames=%u (%2.2f seconds), SeekTableEntries=%d (%d bytes), ChunkSize=%u bytes, PercentFull=%2.2f, Remaining=%u bytes",
	...
	void BuildStreamedAudio()
	{
		...
		if (AudioFormat)
		{
			...
			if (GetCompressedData(CookOutputs))
			{
				...
				if (SplitDataForStreaming(AudioFormat, CookOutputs, ChunkBuffers, ZerothChunkSize,
				                          ...
				                          MAX_uint32))
				{
					UE_LOGF(LogAudio, Display, "Chunk stats for (%ls: Duration=%2.2f secs, Channels=%d), Settings(FirstChunkSize=%u frames, InlineFirst=%ls, MaxChunkSize=%d), Chunks=%d, Chunk0=%d bytes, Chunk1=%d bytes, ChunkN=%d)",
					...
					{
						UE_LOGF(LogAudio, Display, "Sound Wave %ls is very large, requiring %d chunks.", *SoundWaveFullName, ChunkBuffers.Num());
					}
					...
					if (ChunkBuffers.Num() > 0)
					{
						...
						{
							UE_LOGF(LogAudioDerivedData, Warning, "While building split chunk for streaming: Raw PCM data already being written to. Chunk Index: 0 SoundWave: %ls ", *SoundWaveFullName);
						}
					...
					for (int32 ChunkIndex = 1; ChunkIndex < ChunkBuffers.Num(); ++ChunkIndex)
					{
						...
						if (NewChunk->BulkData.IsLocked())
						{
							UE_LOGF(LogAudioDerivedData, Warning, "While building split chunk for streaming: Raw PCM data already being written to. Chunk Index: %d SoundWave: %ls ", ChunkIndex, *SoundWaveFullName);
						}
				...
				else
				{
					...
					if (NewChunk->BulkData.IsLocked())
					{
						UE_LOGF(LogAudioDerivedData, Warning, "While building single-chunk streaming SoundWave: Raw PCM data already being written to. SoundWave: %ls ", *SoundWaveFullName);
					}
			...
			{
				UE_LOGF(LogAudio, Display, "Failed to retrieve compressed data for format %ls and soundwave %ls.",
		...
		{
			UE_LOGF(LogAudio, Display, "Failed to build %ls derived data for %ls",
				*AudioFormatName.GetPlainNameString(),
				*SoundWavePath
				);
		}
	...
	for (int32 ChunkIndex = 0; ChunkIndex < Chunks.Num(); ++ChunkIndex)
	{
		...
		if (Chunk.DerivedDataKey.IsEmpty() == false)
		{
			...
			if (bLoadedFromDDC)
			{
				...
				{
					UE_LOGF(LogAudioDerivedData, Warning, "In TryInlineChunkData: Raw PCM data already being written to. Chunk: %d DDC Key: %ls ", ChunkIndex, *DerivedDataKey);
				}
...
int32 FStreamedAudioPlatformData::GetChunkFromDDC(int32 ChunkIndex, uint8** OutChunkData, bool bMakeSureChunkIsLoaded /* = false */)
{
	if (GetNumChunks() == 0)
	{
		UE_LOGF(LogAudioDerivedData, Display, "No streamed audio chunks found!");
		return 0;
	}
...
void FStreamedAudioPlatformData::Serialize(FArchive& Ar, USoundWave* Owner)
{
	...
	if (Ar.IsCooking() && Ar.IsSaving() && Ar.GetCookContext() && Ar.GetCookContext()->GetCookTagList() && Owner)
	{
		...
		const FName Format = CookingTarget->GetWaveFormat(Owner);

		/* note: must have "Diff_" prefix for the diff tool to pick up the tag as important for diffing */
		CookTags->Add(Owner, "Diff_10_RawDataHash", GetSoundWaveRawDataHash(*Owner));
		CookTags->Add(Owner, "Diff_20_WaveTransformHash", FString(GetWaveformTransformationHash(*Owner)));
		CookTags->Add(Owner, "Diff_30_FormatHash", Format.ToString());
		CookTags->Add(Owner, "Diff_40_SoundWaveHash", FString(GetSoundWaveHash(*Owner, CookingTarget)));
		CookTags->Add(Owner, "Diff_50_CompressedDataGuid", Owner->CompressedDataGuid.ToString());
	}
#endif
...
public:
	...
	{
		UE_LOGF(LogAudioDerivedData, Display, "%ls", *InMessage.ToString());
	}
};
...
static void ResampleWaveData(Audio::FAlignedFloatBuffer& WaveData, int32 NumChannels, float SourceSampleRate, float DestinationSampleRate)
{
	...
	{
		UE_LOGF(LogAudioDerivedData, Error, "Resampling operation failed.");
	}

	double TimeDelta = FPlatformTime::Seconds() - StartTime;
	UE_LOGF(LogAudioDerivedData, Display, "Resampling file from %d to %d took %2.2f seconds.", (int32)SourceSampleRate, (int32)DestinationSampleRate, TimeDelta);
}
	...
	const FName InFormatName, const int32 InCurrentSampleRate)
{
	...
	if (InSampleRateOverride > 0 && InSampleRateOverride != InCurrentSampleRate)
	{
		...
		{
			UE_LOGF(LogAudio, Warning,  "SampleRate Override '%d' not supported by AudioFormat '%ls'", InSampleRateOverride, *InFormatName.ToString());
			bLogOnce = true;
		}
	}
...
static void CookSimpleWave(const FAudioCookInputs& Inputs, FAudioCookOutputs& Outputs)
{
	...
	{
		UE_LOGF(LogAudioDerivedData, Warning, "LPCM data failed to load for sound %ls", *Inputs.SoundFullName);
	}
	else if (!WaveInfo.ReadWaveHeader(RawWaveData, RawDataSize, 0))
	{
		...
		UE_LOGF(LogAudioDerivedData, Warning, "Only mono or stereo 16 bit waves allowed: %ls (%d bytes)", *Inputs.SoundFullName, RawDataSize);
	}
	else
	{
	...
	if(!Input.Num())
	{
		UE_LOGF(LogAudioDerivedData, Warning, "Can't cook %ls because there is no source LPCM data", *Inputs.SoundFullName );
		return;
	}
	...
	if (bNeedsFloatConversion)
	{
		...
		if(bNeedsToApplyWaveTransformation)
		{
			...
			UE_CLOGF(WaveSampleRate != TransformationInfo.SampleRate, LogAudioDerivedData, Warning, "Wave transformations which alter the sample rate are not supported. Cooked audio for %ls may be incorrect", *Inputs.SoundFullName);
			WaveSampleRate = TransformationInfo.SampleRate;

			UE_CLOGF(NumChannels != TransformationInfo.NumChannels, LogAudioDerivedData, Error, "Wave transformations which alter number of channels are not supported. Cooked audio for %ls may be incorrect", *Inputs.SoundFullName);
		...
		if (MaxValue > 1.0f)
		{
			UE_LOGF(LogAudioDerivedData, Display, "Audio clipped during cook: This asset will be normalized by a factor of 1/%f. Consider attenuating the above asset.", MaxValue);
	...
	UE_CLOGF(Inputs.CompressionQuality != QualityInfo.Quality, LogAudioDerivedData,
		Display, "Compression Quality for %ls will be modified from %d to %d, with modifier [%.2f] ",
	...
	static const FName NAME_BINKA("BINKA");
		...
		Inputs.BaseFormat == NAME_BINKA)
	{
		...
		UE_LOGF(LogAudioDerivedData, Warning, "[%ls] High sample rate wave (%d) with Bink Audio - perf waste - high frequencies are discarded by Bink Audio (like most perceptual codecs).", *Inputs.SoundFullName, WaveSampleRate);
	}

	// Cook the data.
	if (!Inputs.Compressor->Cook(Inputs.BaseFormat, Input, QualityInfo, Outputs.EncodedData))
	{
		UE_LOGF(LogAudioDerivedData, Warning, "Cooking sound failed: %ls", *Inputs.SoundFullName);
	}
...
static void CookSurroundWave(const FAudioCookInputs& Inputs, FAudioCookOutputs& Outputs)
{
	...
	{
		UE_LOGF(LogAudioDerivedData, Warning, "Cooking surround sound failed: %ls, Failed to load virtualized bulkdata payload", *Inputs.SoundFullName);
		return;
	}
	...
	if (SampleDataSize == 0)
	{
		UE_LOGF(LogAudioDerivedData, Warning, "Cooking surround sound failed: %ls", *Inputs.SoundFullName );
		return;
	}
	...
	{
		UE_LOGF(LogAudioDerivedData, Warning, "No format available for a %d channel surround sound: %ls", ChannelCount, *Inputs.SoundFullName );
		return;
	}
	...
	if(bNeedsDeinterleave)
	{
		...
		if(bContainsTransformations)
		{
			...
			UE_CLOGF(WaveSampleRate != TransformationInfo.SampleRate, LogAudioDerivedData, Warning, "Wave transformations which alter the sample rate are not supported. Cooked audio for %ls may be incorrect", *Inputs.SoundFullName);
			UE_CLOGF(ChannelCount != TransformationInfo.NumChannels, LogAudioDerivedData, Error, "Wave transformations which alter number of channels are not supported. Cooked audio for %ls may be incorrect", *Inputs.SoundFullName);
	...
	UE_LOGF(LogAudioDerivedData, Display, "Cooking %d channels for: %ls", ChannelCount, *Inputs.SoundFullName);
...
bool FDerivedAudioDataCompressor::Build(TArray<uint8>& OutData)
{
	...
	if (!CookInputs->Compressor)
	{
		UE_LOGF(LogAudioDerivedData, Warning, "Could not find audio format to cook: %ls", *CookInputs->BaseFormat.ToString());
		return false;
	}
...
{
	GetWorkingSoundWaveData()->RunningPlatformData = FStreamedAudioPlatformData();
}
...
void USoundWave::SerializeCookedPlatformData(FArchive& Ar)
{
...
#endif // #if WITH_EDITORONLY_DATA
	{
		...
		GetWorkingSoundWaveData()->RunningPlatformData.Serialize(Ar, this);
	}
}
...
void USoundWave::CachePlatformData(bool bAsyncCache)
{
	...
	if (GetWorkingSoundWaveData()->RunningPlatformData.DerivedDataKey != DerivedDataKey)
	{
		...
		GetWorkingSoundWaveData()->RunningPlatformData.Cache(*this, CompressionOverrides, AudioFormat, CacheFlags, GetRunningPlatform());
	}
}
...
void USoundWave::FinishCachePlatformData()
{
	...
	GetWorkingSoundWaveData()->RunningPlatformData.FinishCache();
	...
	if (!GAllowCookedDataInEditorBuilds && GetWorkingSoundWaveData()->RunningPlatformData.GetNumChunks())
	{
		FString DerivedDataKey;
		FName AudioFormat = GetWaveFormatForRunningPlatform(*this);
		const FPlatformAudioCookOverrides* CompressionOverrides = GetCookOverridesForRunningPlatform();
		...
		UE_CLOGF(GetWorkingSoundWaveData()->RunningPlatformData.DerivedDataKey != DerivedDataKey, LogAudio, Warning, "Audio was cooked with the DDC key %ls but should've had the DDC key %ls. the cook overrides/codec used may be incorrect.", *GetWorkingSoundWaveData()->RunningPlatformData.DerivedDataKey, *DerivedDataKey);
	}
#endif
}
...
void USoundWave::ForceRebuildPlatformData()
{
	...
	GetWorkingSoundWaveData()->RunningPlatformData.Cache(
		*this,
		CompressionOverrides,
