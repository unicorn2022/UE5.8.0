	...
	if (WaveInstance->GetUseSpatialization())
	{
		...
		if (WaveInstance->NonSpatializedRadiusStart > 0.0f)
		{
			...
			if (Params.Distance > 0.0f)
			{
				...
				//UE_LOGF(LogTemp, Log, "Distance: %.2f, NonSpatializedRadiusStart: %.2f, NonSpatializedRadiusEnd: %.2f, NonSpatializedAmount: %.2f", Params.Distance, WaveInstance->NonSpatializedRadiusStart, WaveInstance->NonSpatializedRadiusEnd, Params.NonSpatializedAmount);
...
void FWaveInstance::NotifyFinished( const bool bStopped )
{
	if( !bAlreadyNotifiedHook )
	{
		...
		if( !bIsStarted )
		{
			UE_LOGF(LogAudio, Warning, "Received finished notification from waveinstance that hasn't started!" );
		}
...
struct FRiffBroadcastAudioExtension 
{
	...
	uint8 Reserved[180];			// 180 bytes, reserved for future use, set to “NULL” 
...
bool FWaveModInfo::ReadWaveInfo( const uint8* WaveData, int32 WaveDataSize, FString* ErrorReason, bool InHeaderDataOnly, void** OutFormatHeader)
{
	...
	if (!InHeaderDataOnly && IsFormatSupported())
	{
		...
		// Truncate any bad sample data, does not affect exported data
		// Corrupt data remains present in the raw wave data
		if (pBlockAlign && *pBlockAlign > 0)
		{
			const uint16 BlockAlign = *pBlockAlign; // NumChannels * BytesPerSample
			const uint32 ExtraBytes = SampleDataSize % BlockAlign;

			if (ExtraBytes > 0)
			{
				UE_LOGF(LogAudio, Warning, "Wave data chunk has %u invalid byte(s), truncating invalid block of compressed data", ExtraBytes);

				SampleDataEnd = SampleDataEnd - ExtraBytes;
				SampleDataSize = SampleDataEnd - SampleDataStart;
			}
		}
	...
	*pWaveDataSize = INTEL_ORDER32(*pWaveDataSize);
	...
	if (RiffChunk != nullptr)
	{
		...
		// Guard against a crafted NumCuePoints that would trigger integer overflow in TArray allocation.
		// ChunkDataSize covers the NumCuePoints field (4 bytes) plus the cue point array entries.
		const uint8* CuePointDataStart = (uint8*)RiffChunk + sizeof(FRiffCueChunk);

		// Number of bytes for cue section reported by the wave file
		const uint32 NumCueDataBytes = CueChunk->ChunkDataSize >= sizeof(uint32) ? CueChunk->ChunkDataSize - sizeof(uint32) : 0;

		// Maximum number of cue points that could fit in the reported number of bytes.
		const uint32 MaxCuePointsByChunk = NumCueDataBytes / sizeof(FRiffCuePointChunk);

		// Maximum number of cue points that could fit before the end of the file
		const uint32 MaxCuePointsByFile = CuePointDataStart < (const uint8*)WaveDataEnd
			? static_cast<uint32>(((const uint8*)WaveDataEnd - CuePointDataStart) / sizeof(FRiffCuePointChunk))
			: 0;

		// Take the min of all these to make sure the reported number of cue points can't break out of the RIFF cue chunk or the file as a whole
		const uint32 SafeNumCuePoints = FMath::Min(FMath::Min(CueChunk->NumCuePoints, MaxCuePointsByChunk), MaxCuePointsByFile);

		if (SafeNumCuePoints != CueChunk->NumCuePoints)
		{
			UE_LOG(LogAudio, Warning, TEXT("FWaveModInfo::ReadWaveInfo: WAV cue chunk NumCuePoints (%u) exceeds available data; clamping to %u. File may be malformed."),
				CueChunk->NumCuePoints, SafeNumCuePoints);
		}

		WaveCues.Reset(SafeNumCuePoints);

		// Get to the first cue point chunk
		FRiffCuePointChunk* CuePointChunks = (FRiffCuePointChunk*)CuePointDataStart;
		for (uint32 CuePointId = 0; CuePointId < SafeNumCuePoints; ++CuePointId)
		{
			FWaveCue NewCue;
	...
	if (RiffChunk != nullptr)
	{
		...
		if(NumSamplesSinceMidnight > 0 && FmtChunk && FmtChunk->nSamplesPerSec)
		{
			...
			// BextChunk char arrays have a fixed length and might not be null terminated or padded
			// Safely retrieve either the first null termination or the safe max length of the char array
			FFileHelper::BufferToString(TimecodeInfo->Description, BextChunk->Description, sizeof(BextChunk->Description));
			FFileHelper::BufferToString(TimecodeInfo->OriginatorDescription, BextChunk->Originator, sizeof(BextChunk->Originator));
			FFileHelper::BufferToString(TimecodeInfo->OriginatorDate, BextChunk->OriginationDate, sizeof(BextChunk->OriginationDate));
			FFileHelper::BufferToString(TimecodeInfo->OriginatorReference, BextChunk->OriginatorReference, sizeof(BextChunk->OriginatorReference));
			FFileHelper::BufferToString(TimecodeInfo->OriginatorTime, BextChunk->OriginationTime, sizeof(BextChunk->OriginationTime));
	...
	if (TimecodeInfo.IsValid() && RiffChunk != nullptr)
	{
		...
		if (RiffXml.IsValid())
		{
			const FString SpeedTag("SPEED");
			const FString TimecodeRateTag("TIMECODE_RATE");
			const FString TimecodeRateDelimiter("/");
			const FString TimecodeFlagTag("TIMECODE_FLAG");
			const FString DropFrameFlag("DF");
...
void SerializeWaveFile(TArray<uint8>& OutWaveFileData, const uint8* InPCMData, const int32 NumBytes, const int32 NumChannels, const int32 SampleRate)
{
	...
	int32 BlockAlign = NumChannels * 2;
