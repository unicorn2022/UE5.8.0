...
extern const TCHAR* GNoRefBiasQualityLevelCVarName;
extern const TCHAR* GNoRefBiasQualityLevelScalabilitySection;
	...
	: Super(ObjectInitializer)
{
	...
	NoRefStreamingLODBias.Default = -1;
...
int32 USkeletalMeshLODSettings::SetLODSettingsToMesh(USkeletalMesh* InMesh) const
{
	if (InMesh)
	{
		...
		InMesh->SetNoRefStreamingLODBias(NoRefStreamingLODBias);
...
int32 USkeletalMeshLODSettings::SetLODSettingsFromMesh(USkeletalMesh* InMesh)
{
	...
	if (InMesh)
	{
		...
		NoRefStreamingLODBias = InMesh->GetNoRefStreamingLODBias();
#if WITH_EDITORONLY_DATA
