...
namespace AsyncCompilationHelpers
{
		...
		FName InExperimentalSettingsName)
	{
		...
		{
			// Early at boot, UEditorExperimentalSettings is unavailable, go straight to the ini file instead.
			bool bIsEnabled = false;
			if (GConfig->GetBool(TEXT("/Script/UnrealEd.EditorExperimentalSettings"), *InExperimentalSettingsName.ToString(), bIsEnabled, GEditorPerProjectIni))
			{
	...
	void DumpStallStacks()
	{
		...
		for (FStackData* StackData : Stacks)
		{
			...
			UE_LOGF(LogAsyncCompilation, Display, "Async Compilation Stall Stack: (Count: %llu, Time: %ls)\n%ls", StackData->Count, *FPlatformTime::PrettyTime(FPlatformTime::ToSeconds64(StackData->Cycles)), ANSI_TO_TCHAR(HumanReadableString));
		}

		if (TotalCount)
		{
			UE_LOGF(LogAsyncCompilation, Display, "Async Compilation Stall Stack: (Total Count: %llu, Total Time: %ls)", TotalCount, *FPlatformTime::PrettyTime(FPlatformTime::ToSeconds64(TotalCycles)));
		}
	}
#endif // WITH_EDITOR
}
