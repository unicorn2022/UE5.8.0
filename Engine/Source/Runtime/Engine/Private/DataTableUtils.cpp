		...
		if(EnumIndex == INDEX_NONE)
		{
			...
			for(int32 EnumEntryIndex = 0; EnumEntryIndex < Enum->NumEnums(); ++EnumEntryIndex)
			{
				...
				if ((FriendlyEnumEntryName.ToString() == InString) || (GetSourceString(FriendlyEnumEntryName) == InString))
				{
					...
					{
						UE_LOGF(LogDataTable, Warning, "Could not a find matching enum entry for '%ls', but did find a matching display name. Will import using the enum entry corresponding to that display name ('%ls')", *InString, *StringToImport);
					}
	...
	if(InProp && IsSupportedTableProperty(InProp))
	{
		...
		else
		{
			if(InString.Len() >= 2 && InString[0] == '(' && InString[InString.Len() - 1] == ')')
			{
				...
				{
					UE_LOGF(LogDataTable, Warning, "%ls - Array is %d elements large, but we have %d values to import", *InProp->GetName(), InProp->ArrayDim, Values.Num());
				}
			...
			{
				UE_LOGF(LogDataTable, Warning, "%ls - Malformed array string. It must start with '(' and end with ')'", *InProp->GetName());
			}
		}
	}
