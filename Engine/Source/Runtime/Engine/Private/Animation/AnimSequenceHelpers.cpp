 ...
 void ExtractBoneTransform(const FRawAnimSequenceTrack& RawTrack, FTransform& OutTransform, int32 KeyIndex)
{
	...
	{
		UE_LOGF(LogAnimation, Log, "GetBoneTransform : No anim data in track!");
...
bool CopyNotifies(const UAnimSequenceBase* SourceAnimSeq, UAnimSequenceBase* DestAnimSeq, bool bShowDialogs, bool bDeleteNotifies)
{
	...
	if (DestAnimSeq->GetPlayLength() < SourceAnimSeq->GetPlayLength())
	{
		for (int32 NotifyIndex = 0; NotifyIndex < SourceAnimSeq->Notifies.Num(); ++NotifyIndex)
		{
			...
			{
				UE_LOGF(LogAnimation, Warning, "Animation Notify trigger time %f falls outside of the destination animation sequence its length %f, notify will not be copied.", SrcNotifyEvent.GetTriggerTime(), DestAnimSeq->GetPlayLength());
...
bool Compression::CompressRawAnimSequenceTrack(FRawAnimSequenceTrack& RawTrack, int32 NumberOfKeys, FName ErrorName, float MaxPosDiff, float MaxAngleDiff, float MaxScaleDiff)
{
	...
	if (!bPosTrackIsValid)
	{
		UE_LOGF(LogAnimation, Warning, "Found non valid position track for %ls, %d frames, instead of %d. Chopping!", *ErrorName.ToString(), RawTrack.PosKeys.Num(), NumberOfKeys);
		bRemovedKeys = true;
	...
	if (!bRotTrackIsValid)
	{
		UE_LOGF(LogAnimation, Warning, "Found non valid rotation track for %ls, %d frames, instead of %d. Chopping!", *ErrorName.ToString(), RawTrack.RotKeys.Num(), NumberOfKeys);
		bRemovedKeys = true;
	...
	{
		UE_LOGF(LogAnimation, Warning, "Found non valid scaling track for %ls, %d frames, instead of %d. Chopping!", *ErrorName.ToString(), RawTrack.ScaleKeys.Num(), NumberOfKeys);
		bRemovedKeys = true;
