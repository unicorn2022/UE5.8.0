...
namespace MontageCVars
{
	...
	static bool bCleanupMontageInstancesOnDestroy = true;
	static FAutoConsoleVariableRef CVarCleanupMontageInstancesOnDestroy(
		TEXT("a.Montage.CleanupMontageInstancesOnDestroy"),
		bCleanupMontageInstancesOnDestroy,
		TEXT("Whether any montages still alive on BeginDestroy should be cleaned up."));
}

namespace UE::Anim::Private
{
	static int32 NextNotifyInstanceID = 0;
	static int32 GetNextNotifyInstanceID()
	{
		if (NextNotifyInstanceID == TNumericLimits<int32>::Max())
		{
			NextNotifyInstanceID = 0;
			return NextNotifyInstanceID;
		}
		else
		{
			return NextNotifyInstanceID++;
	...
	static uint32 NextAnimInstanceID = 0;
	static uint32 GetNextAnimInstanceID()
	{
		return NextAnimInstanceID++;
	}
}

/** Used to determine notify dispatch behaviour when sources (montage, anim graph) may tick at different times within a frame. */
enum class EAnimNotifyDispatchPolicy : int32
{
	/** Previous behavior - update active notify buffers and dispatch from any source, even if that source hasn't updated before the dispatch request. */
	AllSources = 0,
	
	/**
	 *  Only dispatch from sources that HAVE updated before the dispatch request.
	 *  
	 *  Anim graph notifies/states are not triggered/ticked if the anim graph hasn't updated yet this tick.
	 *  Active notify states from skipped sources remain valid (useful in MoveAutonomous / multi-tick scenarios).
	 */
	UpdatedSourcesOnly = 1,
};

namespace AnimNotifyCVars
{
	static int32 bApplyUnfilteredMontageAnimNotifiesOnAutonomousTickPose = 1;
	FAutoConsoleVariableRef CVarApplyUnfilteredMontageAnimNotifiesOnAutonomousTickPose(
		TEXT("a.Notifies.ApplyUnfilteredOnAutonomousTickPose"),
		bApplyUnfilteredMontageAnimNotifiesOnAutonomousTickPose,
		TEXT("Whether to apply unfiltered montage notifies when ticking character pose during autonomous move updates. \n")
		TEXT("0: Disable, 1: Enable"),
		ECVF_Default);
	
	static int32 NotifiesDispatchPolicy = static_cast<int32>(EAnimNotifyDispatchPolicy::UpdatedSourcesOnly);
	static FAutoConsoleVariableRef CVarNotifiesDispatchPolicy(
		TEXT("a.Notifies.DispatchPolicy"),
		NotifiesDispatchPolicy,
		TEXT("Controls notify dispatch behavior when it's source (montage, anim graph) may tick at different times within a frame.\n")
			TEXT("  0 = AllSources: Update active notify buffers and dispatch from any source, even if that source has not updated yet this tick (previous behavior).\n")
			TEXT("  1 = UpdatedSourcesOnly: Only dispatch from sources that have updated this tick. Anim graph notifies/states are not triggered/ticked if the graph has not updated yet, ")
			TEXT("but active notify states from skipped sources remain valid. Useful for multi-tick scenarios such as MoveAutonomous."),
		ECVF_Default);
}
...
#endif
{
	...
	bHasDispatchedAnimGraphNotifiesThisTick = false;
	bHasDispatchedMontageNotifiesThisTick = false;
...
void UAnimInstance::InitializeMontageOnly()
{
	bUninitialized = false;
	AnimInstanceID = UE::Anim::Private::GetNextAnimInstanceID();
	USkeletalMeshComponent* OwnerComponent = GetSkelMeshComponent();
...
void UAnimInstance::InitializeAnimation(bool bInDeferRootNodeInitialization)
{
	...
	if (IAnimClassInterface* AnimBlueprintClass = IAnimClassInterface::GetFromClass(GetClass()))
	{
		...
		// Update any game thread pre-event graph property access calls so that these variables are available
		// for any anim nodes that call GetEvaluateGraphExposedInputs from Initialize_AnyThread
...
#if ANIM_TRACE_ENABLED
				TickRecord.SourceProxy = &GetProxyOnAnyThread<FAnimInstanceProxy>();
#endif
...
void UAnimInstance::TickMontageOnly(float DeltaSeconds)
{
	ClearQueuedAnimEvents(true);

	// Clear out any invalid montage instances 
...
void UAnimInstance::PreUpdateAnimation(float DeltaSeconds)
{
	...
	if (USkeletalMeshComponent* SkelMeshComp = GetSkelMeshComponent(); SkelMeshComp && !SkelMeshComp->PoseTickedThisFrame())
	{
		bHasDispatchedMontageNotifiesThisTick = false;
		bHasDispatchedAnimGraphNotifiesThisTick = false;
	}
...
void UAnimInstance::PostUpdateAnimation()
{
	...
	FGuardValue_Bitfield(bPostUpdatingAnimation, true);
#endif
...
{
	// Acquire the proxy as we need to query notifies and delta time
	FAnimInstanceProxy& Proxy = GetProxyOnGameThread<FAnimInstanceProxy>();
	USkeletalMeshComponent* SkelMeshComp = GetSkelMeshComponent();
	
	// When ticking the character pose via the CharacterMovementComponent we need to manually apply the montage notifies after each replicated move since it can queue unfiltered montage notifies that will get skipped otherwise. 
	if (SkelMeshComp && SkelMeshComp->bIsAutonomousTickPose && AnimNotifyCVars::bApplyUnfilteredMontageAnimNotifiesOnAutonomousTickPose)
	{
		NotifyQueue.ApplyMontageNotifies(Proxy);
	}
	...
	// We need to determine which anim update path's notifies have we triggered in the case we get multiple dispatch request per frame
	// (from the pose being ticked in CharacterMovementComponent) to avoid clearing the ActiveNotify arrays after parallel anim eval is done.
	if (SkelMeshComp)
	{
		bHasDispatchedMontageNotifiesThisTick = SkelMeshComp->bIsAutonomousTickPose && SkelMeshComp->PoseTickedThisFrame();
		bHasDispatchedAnimGraphNotifiesThisTick = SkelMeshComp->bIsAutonomousTickPose && !NeedsUpdate() && SkelMeshComp->PoseTickedThisFrame();
	}
}
...
void UAnimInstance::FinalizeAnimation(bool bWasEvaluated)
{
	LLM_SCOPE(ELLMTag::Animation);
	NativeFinalizeAnimation(bWasEvaluated);
}
...
void UAnimInstance::NativeFinalizeAnimation(bool bWasEvaluated)
{
}
...
void OutputTickRecords(const TArray<FAnimTickRecord>& Records, UCanvas* Canvas, float Indent, const int32 HighlightIndex, FLinearColor TextColor, FLinearColor HighlightColor, FLinearColor InactiveColor, FDisplayDebugManager& DisplayDebugManager, bool bFullBlendspaceDisplay)
{
	for (int32 PlayerIndex = 0; PlayerIndex < Records.Num(); ++PlayerIndex)
	{
		...
		if (HighlightIndex != INDEX_NONE)
		{
			...
			TConstArrayView<FAnimSyncMarker> AuthoredMarkerNames;
			
			if (const UAnimSequence* AnimSequence = Cast<UAnimSequence>(Player.SourceAsset))
			{
				AuthoredMarkerNames = AnimSequence->AuthoredSyncMarkers;  
			}
			else if (const UAnimMontage* AnimMontage = Cast<UAnimMontage>(Player.SourceAsset))
			{
				AuthoredMarkerNames = AnimMontage->MarkerData.AuthoredSyncMarkers;
			}
			else if (const UBlendSpace* BlendSpace = Cast<UBlendSpace>(Player.SourceAsset))
			{
				AuthoredMarkerNames = BlendSpace->GetAuthoredSyncMarkers();
			}
			
			if (!AuthoredMarkerNames.IsEmpty())
			{
				if (AuthoredMarkerNames.IsValidIndex(Player.MarkerTickRecord->PreviousMarker.MarkerIndex))
				{
					PrevMarkerName = AuthoredMarkerNames[Player.MarkerTickRecord->PreviousMarker.MarkerIndex].MarkerName; 
				}

				if (AuthoredMarkerNames.IsValidIndex(Player.MarkerTickRecord->NextMarker.MarkerIndex))
				{
...
void UAnimInstance::DisplayDebug(class UCanvas* Canvas, const FDebugDisplayInfo& DebugDisplay, float& YL, float& YPos)
{
	...
	static FName CAT_SyncGroups("SyncGroups");
	static FName CAT_Montages("Montages");
	static FName CAT_Graph("Graph");
	static FName CAT_Curves("Curves");
	static FName CAT_Notifies("Notifies");
	static FName CAT_FullAnimGraph("FullGraph");
	static FName CAT_FullBlendspaceDisplay("FullBlendspaceDisplay");
			...
			if (PrevChainID != Line.ChainID)
			{
				...
				if (IndentLineStartCoord.IsValidIndex(VerticalLineIndex))
				{
					...
					if (FMath::IsNearlyEqual(LineStartCoord.X, DisplayDebugManager.GetXPos()))
					{
						...
						Canvas->K2_DrawLine(FVector2D(StartX, CurrLineYBase), FVector2D(EndX, CurrLineYBase), 1.0f, ActiveColor);

						// vertical line
						Canvas->K2_DrawLine(FVector2D(StartX, LineStartCoord.Y), FVector2D(StartX, CurrLineYBase), 1.0f, ActiveColor);
					}
				}
...
void UAnimInstance::Serialize(FArchive& Ar)
{
	...
	// There's speculation we wanted 'if (!IsPersistent())' here, but changing this will require bumping the serialization version,
	// so we're keeping this always-serialized until we find a reason not to.
	// if (!Ar.IsLoading() || !Ar.IsSaving()) [This should always return true]
	Ar << GetProxyOnAnyThread<FAnimInstanceProxy>().GetRequiredBones();

	if (Ar.IsMigratingRemoteObjects())
	{
		uint8 BitFields = 0;
		BitFields |= (bUpdateAnimationEnabled << 0);
		BitFields |= (bUninitialized << 1);
		BitFields |= (bHasDispatchedAnimGraphNotifiesThisTick << 2);
		BitFields |= (bHasDispatchedMontageNotifiesThisTick << 3);

		Ar << BitFields;

		if (Ar.IsLoading())
		{
			bUpdateAnimationEnabled 				= (BitFields & (1 << 0)) ? 1 : 0;
			bUninitialized 							= (BitFields & (1 << 1)) ? 1 : 0;
			bHasDispatchedAnimGraphNotifiesThisTick = (BitFields & (1 << 2)) ? 1 : 0;
			bHasDispatchedMontageNotifiesThisTick 	= (BitFields & (1 << 3)) ? 1 : 0;
		}

		// These are guard flags, so don't serialize them
		//Ar << bInitializing;
		//Ar << bPostUpdatingAnimation;
		//Ar << bUpdatingAnimation;
	}
}
...
void UAnimInstance::BeginDestroy()
{
	...
	if (MontageCVars::bCleanupMontageInstancesOnDestroy)
	{
		if (MontageInstances.Num() > 0)
		{
			for (int32 Index = 0; Index < MontageInstances.Num(); ++Index)
...
void UAnimInstance::TriggerAnimNotifies(float DeltaSeconds, ENotifyTriggerMode Mode)
{
	SCOPE_CYCLE_COUNTER(STAT_AnimTriggerAnimNotifies);
	USkeletalMeshComponent* SkelMeshComp = GetSkelMeshComponent();

	// This function can be called from outsode of anim evaluation (e.g. the ability system) and seems like it may call into 
	// code that uses the mem stack allocator. Conditionally construct a mark if there is none.
	TOptional<FMemMark> MaybeMark;
	FMemStack& MemStack = FMemStack::Get();
	if (MemStack.GetNumMarks() == 0)
	{
		MaybeMark.Emplace(MemStack);
	}
	...
	// Override modes (filters).
	const bool bForceMontageOnly = Mode == ENotifyTriggerMode::ForceMontageOnly;
	const bool bForceAnimGraphOnly = Mode == ENotifyTriggerMode::ForceAnimGraphOnly;

	// Filter based on override selection
	if (bForceMontageOnly || bForceAnimGraphOnly)
	{
		TArray<FAnimNotifyEventReference, FAnimStackAllocator> FilteredAnimNotifies;
		FilteredAnimNotifies.Reserve(NotifyQueue.AnimNotifies.Num());
		
		for (FAnimNotifyEventReference& Notify : NotifyQueue.AnimNotifies)
		{
			const bool bIsFromMontage = Notify.GetContextData<UE::Anim::FAnimNotifyMontageInstanceContext>() != nullptr;
			const bool bFilterGraphNotify = !bIsFromMontage && bForceMontageOnly;
			const bool bFilterMontageNotify = bIsFromMontage && bForceAnimGraphOnly;

			if (bFilterGraphNotify || bFilterMontageNotify)
			{
				continue;
			}

			FilteredAnimNotifies.Push(Notify);
		}

		NotifyQueue.AnimNotifies = MoveTemp(FilteredAnimNotifies);
	}
	
	const bool bDispatchFromSourcesOnly = static_cast<EAnimNotifyDispatchPolicy>(AnimNotifyCVars::NotifiesDispatchPolicy) == EAnimNotifyDispatchPolicy::UpdatedSourcesOnly && Mode == ENotifyTriggerMode::Default;

	// We need to skip triggering notifies if we haven't updated the source (due to parallel update) or if we have already processed the notifies this tick (multiple anim updates/dispatches only allowed from autonomous tick/move requests). 
	const bool bSkipNotifiesSourcedFromAnimGraph = bDispatchFromSourcesOnly && (NeedsUpdate() || (bHasDispatchedAnimGraphNotifiesThisTick && !SkelMeshComp->bIsAutonomousTickPose));
	const bool bSkipNotifiesSourcedFromMontages = bDispatchFromSourcesOnly && (bHasDispatchedMontageNotifiesThisTick && !SkelMeshComp->bIsAutonomousTickPose);
	...
	for (int32 Index=0; Index<NotifyQueue.AnimNotifies.Num(); Index++)
	{
		...
		{
			// Filter any notify states that are still active from the old tracking arrays to avoid triggering their respective being/end events.
			if (AnimNotifyEvent->NotifyStateClass)
			{
				const UE::Anim::FAnimNotifyMontageInstanceContext* QueuedMontageNotifyContext = NotifyQueue.AnimNotifies[Index].GetContextData<UE::Anim::FAnimNotifyMontageInstanceContext>();
				const UE::Anim::FAnimNotifyAssetPlayerInstanceContext* QueuedAssetPlayerNotifyContext = NotifyQueue.AnimNotifies[Index].GetContextData<UE::Anim::FAnimNotifyAssetPlayerInstanceContext>();
				
				bool bFoundAlreadyActiveNotify = false;
				for (int32 ActiveNotifyStateIndex = 0; ActiveNotifyStateIndex < ActiveAnimNotifyState.Num(); ++ActiveNotifyStateIndex)
				{
					// Check if the queued notify matches an already-active notify state
					const bool bIsMatchingNotifyEvent = ActiveAnimNotifyState[ActiveNotifyStateIndex] == *AnimNotifyEvent;
					if (bIsMatchingNotifyEvent)
					{
						check(ActiveAnimNotifyState.Num() == ActiveAnimNotifyEventReference.Num());
						const FAnimNotifyEventReference& ActiveEventReference = ActiveAnimNotifyEventReference[ActiveNotifyStateIndex];
						
						// When NoMergeOnConcurrentPlay is set, require a matching source instance ID to prevent concurrent animation plays from sharing a single notify state.
						const bool bRequiresSourceInstanceIDMatching = (AnimNotifyEvent->NotifyStateClass->NotifyStateBehaviorFlags & static_cast<uint8>(EAnimNotifyStateBehaviorFlags::NoMergeOnConcurrentPlay)) != 0;
						bool bHasMatchingSourceInstanceID = false;
						if (bRequiresSourceInstanceIDMatching)
						{
							if (QueuedMontageNotifyContext)
							{
								if (const UE::Anim::FAnimNotifyMontageInstanceContext* ActiveMontageNotifyContext = ActiveEventReference.GetContextData<UE::Anim::FAnimNotifyMontageInstanceContext>())
								{
									bHasMatchingSourceInstanceID = QueuedMontageNotifyContext->MontageInstanceID == ActiveMontageNotifyContext->MontageInstanceID;
								}
							}
							else if (QueuedAssetPlayerNotifyContext)
							{
								if (const UE::Anim::FAnimNotifyAssetPlayerInstanceContext* ActiveAssetInstanceNotifyContext = ActiveEventReference.GetContextData<UE::Anim::FAnimNotifyAssetPlayerInstanceContext>())
								{
									bHasMatchingSourceInstanceID = QueuedAssetPlayerNotifyContext->AssetPlayerInstanceID == ActiveAssetInstanceNotifyContext->AssetPlayerInstanceID;
								}
							}
						}

						if (!bRequiresSourceInstanceIDMatching || (bRequiresSourceInstanceIDMatching && bHasMatchingSourceInstanceID))
						{
							// Notify already exists, so we should re-use the same Instance ID.
							NotifyQueue.AnimNotifies[Index].SetNotifyInstanceID(ActiveAnimNotifyEventReference[ActiveNotifyStateIndex].GetNotifyInstanceID());
							ActiveAnimNotifyState.RemoveAtSwap(ActiveNotifyStateIndex, EAllowShrinking::No);
							ActiveAnimNotifyEventReference.RemoveAtSwap(ActiveNotifyStateIndex, EAllowShrinking::No);
							bFoundAlreadyActiveNotify = true;
							break;
						}
					}
				}
				
				if (!bFoundAlreadyActiveNotify)
				{
					// New notify. Generate unique ID for this instance.
					NotifyQueue.AnimNotifies[Index].SetNotifyInstanceID(UE::Anim::Private::GetNextNotifyInstanceID());
	...
	for (int32 Index = 0; Index < ActiveAnimNotifyState.Num(); ++Index)
	{
		...
		const bool bIsFromMontage = EventReference.GetContextData<UE::Anim::FAnimNotifyMontageInstanceContext>() != nullptr;
		const bool bShouldSkipAnimGraphNotify = bSkipNotifiesSourcedFromAnimGraph && !bIsFromMontage;
		const bool bShouldSkipMontageNotify = bSkipNotifiesSourcedFromMontages && bIsFromMontage;
		
		// Any notify state we skip from being considered as ended needs to be re-added to our new notify array to be able to properly keep track of its lifetime.
		if (bShouldSkipAnimGraphNotify || bShouldSkipMontageNotify)
		{
			NewActiveAnimNotifyState.Add(AnimNotifyEvent);
			FAnimNotifyEventReference& NewEventRef = NewActiveAnimNotifyEventReference.Add_GetRef(EventReference);
			NewEventRef.SetNotify(&NewActiveAnimNotifyState.Top());
		}
		else
		{
	...
	for (int32 Index = 0; Index < ActiveAnimNotifyState.Num(); Index++)
	{
		...
		// Only tick notifies states if their source is active.
		const bool bIsFromMontage = EventReference.GetContextData<UE::Anim::FAnimNotifyMontageInstanceContext>() != nullptr;
		const bool bShouldSkipAnimGraphNotify = bSkipNotifiesSourcedFromAnimGraph && !bIsFromMontage;
		const bool bShouldSkipMontageNotify = bSkipNotifiesSourcedFromMontages && bIsFromMontage;
		const bool bShouldTickNotifyState = !bShouldSkipMontageNotify || !bShouldSkipAnimGraphNotify; 

		if (ShouldTriggerAnimNotifyState(AnimNotifyEvent.NotifyStateClass) && bShouldTickNotifyState)
		{
#if WITH_EDITOR
			// Prevent firing notifies in animation editors if requested 
			if((SkelMeshComp && !SkelMeshComp->IsA<UDebugSkelMeshComponent>()) || AnimNotifyEvent.NotifyStateClass->ShouldFireInEditor())
	...
	if (AnimNotifyEvent && (AnimNotifyEvent->NotifyStateClass == NULL))
	{
		...
		if (AnimNotifyEvent->Notify != nullptr)
		{
...
#endif
			{
				...
				EventReference.SetNotifyInstanceID(UE::Anim::Private::GetNextNotifyInstanceID());
				...
				if (InAnimInstance == this || InAnimInstance->bReceiveNotifiesFromLinkedInstances)
				{
					...
					if (Function)
					{
						...
						else
						{
							...
							UE_LOGF(LogAnimNotify, Warning, "Anim notifier named %ls, but the parameter number does not match or not of the correct type", *FuncName.ToString());
						}
					}
				}
			};
...
bool UAnimInstance::GetCurveValueWithDefault(FName CurveName, float DefaultValue, float& OutValue) const
{
...
FName UAnimInstance::GetCurrentStateName(int32 MachineIndex) const
{
	if (const IAnimClassInterface* AnimBlueprintClass = IAnimClassInterface::GetFromClass(GetClass()))
	{
		...
		if ((MachineIndex >= 0) && (MachineIndex < AnimNodeProperties.Num()))
		{
			...
			const FStructProperty* MachineInstanceProperty = AnimNodeProperties[InstancePropertyIndex];
			checkSlow(MachineInstanceProperty->Struct->IsChildOf(FAnimNode_StateMachine::StaticStruct()));

			const FAnimNode_StateMachine* MachineInstance = MachineInstanceProperty->ContainerPtrToValuePtr<FAnimNode_StateMachine>(this);
	...
	for (int32 InstanceIndex = 0; InstanceIndex < MontageInstances.Num(); InstanceIndex++)
	{
		...
		if (MontageInstance && IsValid(MontageInstance->Montage))
		{
			...
			if (MontageInstance.IsValid())
			{
				...
				UE_LOGF(LogAnimMontage, Warning, "%d. Montage (%ls), DesiredWeight(%0.2f), CurrentWeight(%0.2f), BlendingTime(%0.2f)",
...
const TMap<FName, FInertializationRequest>& UAnimInstance::GetActiveInertializationRequests() const
{
	return SlotGroupInertializationRequestDataMap;
}

void UAnimInstance::ClearActiveInertializationRequests()
{
	SlotGroupInertializationRequestDataMap.Reset();
}
...
void UAnimInstance::TriggerQueuedMontageEvents()
{
	...
	// we're copying event containers in case they get changed during broadcast delegate calls 
	// we preferred a stack copy using TInlineAllocator allocated arrays rather than a MoveTemp to avoid reallocating the original arrays
	// also, we cannot use TMemStackAllocator, since we cannot guarantee broadcast delegate calls will not reallocate memstack containers

	constexpr int32 PreallocatedEvents = 4;

	if (QueuedMontageBlendingOutEvents.Num() > 0)
	{
		TArray<FQueuedMontageBlendingOutEvent, TInlineAllocator<PreallocatedEvents>> QueuedMontageBlendingOutEventsCopy(QueuedMontageBlendingOutEvents);
		QueuedMontageBlendingOutEvents.Reset();

		for (const FQueuedMontageBlendingOutEvent& MontageBlendingOutEvent : QueuedMontageBlendingOutEventsCopy)
		{
			TriggerMontageBlendingOutEvent(MontageBlendingOutEvent);
		}
	}
	...
	{
		TArray<FQueuedMontageBlendedInEvent, TInlineAllocator<PreallocatedEvents>> QueuedMontageBlendedInEventsCopy(QueuedMontageBlendedInEvents);
		QueuedMontageBlendedInEvents.Reset();

		for (const FQueuedMontageBlendedInEvent& MontageBlendedInEvent : QueuedMontageBlendedInEventsCopy)
		{
			TriggerMontageBlendedInEvent(MontageBlendedInEvent);
		}
	}
	...
	{
		TArray<FQueuedMontageSectionChangedEvent, TInlineAllocator<PreallocatedEvents>> QueuedMontageSectionChangedEventsCopy(QueuedMontageSectionChangedEvents);
		QueuedMontageSectionChangedEvents.Reset();

		for (const FQueuedMontageSectionChangedEvent& MontageSectionChangedEvent : QueuedMontageSectionChangedEventsCopy)
		{
			TriggerMontageSectionChangedEvent(MontageSectionChangedEvent);
		}
	}
	...
	{
		TArray<FQueuedMontageEndedEvent, TInlineAllocator<PreallocatedEvents>> QueuedMontageEndedEventsCopy(QueuedMontageEndedEvents);
		QueuedMontageEndedEvents.Reset();

		for (const FQueuedMontageEndedEvent& MontageEndedEvent : QueuedMontageEndedEventsCopy)
		{
			TriggerMontageEndedEvent(MontageEndedEvent);
		}
	}
...
float UAnimInstance::Montage_PlayInternal(UAnimMontage* MontageToPlay, const FMontageBlendSettings& BlendInSettings, float InPlayRate /*= 1.f*/, EMontagePlayReturnType ReturnValueType /*= EMontagePlayReturnType::MontageLength*/, float InTimeToStartMontageAt /*= 0.f*/, bool bStopAllMontages /*= true*/)
{
	...
	if (MontageToPlay && (MontageToPlay->GetPlayLength() > 0.f) && MontageToPlay->HasValidSlotSetup())
	{
		if (CurrentSkeleton && MontageToPlay->GetSkeleton())
		{
			...
			UE_LOGF(LogAnimMontage, Verbose, "Montage_Play: AnimMontage: %ls,  (DesiredWeight:%0.2f, Weight:%0.2f)",
		...
		{
			UE_LOGF(LogAnimMontage, Warning, "Playing a Montage (%ls) for the wrong Skeleton (%ls) instead of (%ls).",
...
FComputeDeltaFrameTime* UAnimInstance::Montage_GetComputeFrameDeltaTimeDelegate(const UAnimMontage* Montage)
{
	if (Montage)
	{
		FAnimMontageInstance* MontageInstance = GetActiveInstanceForMontage(Montage);
		if (MontageInstance)
		{
			return &MontageInstance->OnComputeDeltaFrameTime;
		}
	}
	else
	{
...
void UAnimInstance::StopAllMontages(float BlendOut)
{
	...
	{
		if (!MontageInstances.IsValidIndex(Index))
		{
			// Adding a check here to avoid indexing past the end of the array. This could potentially occur if a
			// montage has been removed from the array during this loop iteration.
			continue;
		}
...
void UAnimInstance::ClearMontageInstanceReferences(FAnimMontageInstance& InMontageInstance)
{
	...
	else
	{
		...
		{
			UE_LOGF(LogAnimation, Warning, "%ls: null montage found in the montage instance array!!", *GetName());
		}
	}
...
{
	// This can be called from Blueprint, make sure no async work is ongoing
	if (USkeletalMeshComponent* MeshComp = GetSkelMeshComponent())
	{
		HandleExistingParallelEvaluationTask(MeshComp);
	}
...
void UAnimInstance::PerformLinkedLayerOverlayOperation(TSubclassOf<UAnimInstance> InClass, TFunctionRef<UClass*(UClass* InClass, FAnimNode_LinkedAnimLayer*)> InClassSelectorFunction, bool bInDeferSubGraphInitialization)
{
	...
	if(bInitializing)
	{
		UE_LOGF(LogAnimation, Warning, "Performing linked layer operations in initialization may produce unexpected results, please use the BlueprintLinkedAnimationLayersInitialized event to perform linked layer operations.");
	}
#endif
	...
	if (IAnimClassInterface* AnimBlueprintClass = IAnimClassInterface::GetFromClass(GetClass()))
	{
		...
		if (NewClass && IAnimClassInterface::GetFromClass(NewClass) == nullptr)
		{
			...
			UE_LOGF(LogAnimation, Warning, "Performing linked layer operations with an invalid layer class: %ls", *NewClass->GetName());
			return;
		}
...
void UAnimInstance::AnimDrawDebugOnScreenMessage(const FString& DebugMessage, const FColor& Color, const FVector2D& TextScale, ESceneDepthPriorityGroup DepthPriority)
{
	GetProxyOnAnyThread<FAnimInstanceProxy>().AnimDrawDebugOnScreenMessage(DebugMessage, Color, TextScale, DepthPriority);
}

void UAnimInstance::AnimDrawDebugInWorldMessage(const FString& DebugMessage, const FVector& TextLocation, const FColor& Color, float TextScale)
{
	GetProxyOnAnyThread<FAnimInstanceProxy>().AnimDrawDebugInWorldMessage(DebugMessage, TextLocation, Color, TextScale);
}

void UAnimInstance::AnimDrawDebugLine(const FVector& StartLoc, const FVector& EndLoc, const FColor& Color, bool bPersistentLines, float LifeTime, float Thickness, ESceneDepthPriorityGroup DepthPriority)
{
	GetProxyOnAnyThread<FAnimInstanceProxy>().AnimDrawDebugLine(StartLoc, EndLoc, Color, bPersistentLines, LifeTime, Thickness, DepthPriority);
}

void UAnimInstance::AnimDrawDebugDirectionalArrow(const FVector& LineStart, const FVector& LineEnd, float ArrowSize, const FColor& Color, bool bPersistentLines, float LifeTime, float Thickness, ESceneDepthPriorityGroup DepthPriority)
{
	GetProxyOnAnyThread<FAnimInstanceProxy>().AnimDrawDebugDirectionalArrow(LineStart, LineEnd, ArrowSize, Color, bPersistentLines, LifeTime, Thickness, DepthPriority);
}

void UAnimInstance::AnimDrawDebugSphere(const FVector& Center, float Radius, int32 Segments, const FColor& Color, bool bPersistentLines, float LifeTime, float Thickness, ESceneDepthPriorityGroup DepthPriority)
{
	GetProxyOnAnyThread<FAnimInstanceProxy>().AnimDrawDebugSphere(Center, Radius, Segments, Color, bPersistentLines, LifeTime, Thickness, DepthPriority);
}

void UAnimInstance::AnimDrawDebugCoordinateSystem(FVector const& AxisLoc, FRotator const& AxisRot, float Scale, bool bPersistentLines, float LifeTime, float Thickness, ESceneDepthPriorityGroup DepthPriority)
{
	GetProxyOnAnyThread<FAnimInstanceProxy>().AnimDrawDebugCoordinateSystem(AxisLoc, AxisRot, Scale, bPersistentLines, LifeTime, Thickness, DepthPriority);
}

void UAnimInstance::AnimDrawDebugPlane(const FTransform& BaseTransform, float Radii, const FColor& Color, bool bPersistentLines, float LifeTime, float Thickness, ESceneDepthPriorityGroup DepthPriority)
{
	GetProxyOnAnyThread<FAnimInstanceProxy>().AnimDrawDebugPlane(BaseTransform, Radii, Color, bPersistentLines, LifeTime, Thickness, DepthPriority);
}

void UAnimInstance::AnimDrawDebugPoint(const FVector& Loc, float Size, const FColor& Color, bool bPersistentLines, float LifeTime, ESceneDepthPriorityGroup DepthPriority)
{
	GetProxyOnAnyThread<FAnimInstanceProxy>().AnimDrawDebugPoint(Loc, Size, Color, bPersistentLines, LifeTime, DepthPriority);
}

void UAnimInstance::AnimDrawDebugCircle(const FVector& Center, float Radius, int32 Segments, const FColor& Color, const FVector& UpVector, bool bPersistentLines, float LifeTime, ESceneDepthPriorityGroup DepthPriority, float Thickness)
{
	GetProxyOnAnyThread<FAnimInstanceProxy>().AnimDrawDebugCircle(Center, Radius, Segments, Color, UpVector, bPersistentLines, LifeTime, DepthPriority, Thickness);
}

void UAnimInstance::AnimDrawDebugCone(const FVector& Center, float Radius, const FVector& Direction, float AngleWidth, float AngleHeight, int32 Segments, const FColor& Color, bool bPersistentLines, float LifeTime, ESceneDepthPriorityGroup DepthPriority, float Thickness)
{
	GetProxyOnAnyThread<FAnimInstanceProxy>().AnimDrawDebugCone(Center, Radius, Direction, AngleWidth, AngleHeight, Segments, Color, bPersistentLines, LifeTime, DepthPriority, Thickness);
}

void UAnimInstance::AnimDrawDebugCapsule(const FVector& Center, float HalfHeight, float Radius, const FRotator& Rotation, const FColor& Color, bool bPersistentLines, float LifeTime, float Thickness, ESceneDepthPriorityGroup DepthPriority)
{
	GetProxyOnAnyThread<FAnimInstanceProxy>().AnimDrawDebugCapsule(Center, HalfHeight, Radius, Rotation, Color, bPersistentLines, LifeTime, Thickness, DepthPriority);
}
...
{
	UE_LOGF(LogAnimation, Error, "%ls: LockAIResources is no longer supported. Please use LockAIResourcesWithAnimation instead.", *GetName());	
}

void UAnimInstance::UnlockAIResources(bool bUnlockMovement, bool UnlockAILogic)
{
	UE_LOGF(LogAnimation, Error, "%ls: UnlockAIResources is no longer supported. Please use UnlockAIResourcesWithAnimation instead.", *GetName());
}

void UAnimInstance::ResetSync()
{
	GetProxyOnAnyThread<FAnimInstanceProxy>().ResetSync();
}
...
void UAnimInstance::UpdateMontageEvaluationData()
{
	...
	UE_LOGF(LogAnimMontage, Verbose, "UpdateMontageEvaluationData Starting: Owner: %ls",	*GetNameSafe(GetOwningActor()));
	...
	for (FAnimMontageInstance* MontageInstance : MontageInstances)
	{
		...
		{
			UE_LOGF(LogAnimMontage, Verbose, "UpdateMontageEvaluationData : AnimMontage: %ls,  (DesiredWeight:%0.2f, Weight:%0.2f)",
		...
		if(bThisObjectWasReinstanced)
		{
			...
			if(MeshComponent && MeshComponent->GetSkeletalMeshAsset())
			{
				if (UWorld* World = GetWorld())
				{
					...
					// Reset interactors which might have been used in the blueprint
					for (FClothingSimulationInstance& ClothingSimulationInstance : MeshComponent->GetClothingSimulationInstances())
					{
