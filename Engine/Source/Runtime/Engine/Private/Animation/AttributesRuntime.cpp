...
void Attributes::AccumulateAttributesWithOverrideWeights(const FStackAttributeContainer& SourceAttributes, FStackAttributeContainer& OutAttributes, const TMap<FAttributeId, float>& AttributeWeightOverrides, float Weight, EAdditiveAnimationType AdditiveType)
{
	QUICK_SCOPE_CYCLE_COUNTER(STAT_AccumulateAttributesWithOverrideWeights);
	...
	{
		const IAttributeBlendOperator* Operator = AttributeTypes::GetTypeOperator(WeakScriptStruct);
		FAttributeBlendData AttributeBlendData = FAttributeBlendData::SingleAdditiveContainerWithOverrideWeights(Array, AttributeWeightOverrides, Weight, AdditiveType, WeakScriptStruct.Get());
		Operator->AccumulateWithOverrideWeights(AttributeBlendData, &OutAttributes);
	}	
}
