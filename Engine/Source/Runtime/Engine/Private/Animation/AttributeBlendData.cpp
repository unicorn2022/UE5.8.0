...
namespace UE { namespace Anim {
	...
	FAttributeBlendData::FAttributeBlendData(const TArrayView<const FStackAttributeContainer* const> SourceAttributes, const TMap<UE::Anim::FAttributeId, float>& InAttributeWeightOverrides,  const float InUniformWeight, EAdditiveAnimationType InAdditiveType, const UScriptStruct* AttributeScriptStruct) : UniformWeight(InUniformWeight), AdditiveType(InAdditiveType)
	{
		AttributeWeightOverrides = InAttributeWeightOverrides;
	...
	void FAttributeBlendData::ProcessAttributes(const FStackAttributeContainer& AttributeContainers, int32 SourceAttributesIndex, const UScriptStruct* AttributeScriptStruct)
	{
		...
		auto AddAttributeToSet = [this](FAttributeSet& AttributeSet, const uint8* DataPtr, int32 WeightIndex)
		{
			...
			// Only track highest-weighted container when using container-level weights (not per-bone)
			if (HasContainerWeights())
			{
