...
void UTransformTrajectoryBlueprintLibrary::DebugDrawTrajectory(const FTransformTrajectory& Trajectory, const UObject* Owner, const FLogCategoryBase& Category, ELogVerbosity::Type Verbosity, const float DebugThickness, float HeightOffset, bool bDisplayVelocity)
{
	...
	if (LastIndex >= 0)
	{
		for (int32 Index = 0; ; ++Index)
		{
			...
			FString VelocityDebugStr;
			if (Index > 0 && bDisplayVelocity)
			{
				float DeltaTime = Trajectory.Samples[Index].TimeInSeconds - Trajectory.Samples[Index - 1].TimeInSeconds;
				if (DeltaTime > 0.f)
				{
					Velocity = (Trajectory.Samples[Index].Position - Trajectory.Samples[Index - 1].Position) / DeltaTime;
					VelocityDebugStr = FString::Printf(TEXT("Velocity: %.3f"), Velocity.Size());
				}
			}

			CurrentEntry->AddSegment(Pos, Pos + X * Scale, Category.GetCategoryName(), Verbosity, IsPast ? FColor::Red : FColor::Blue, VelocityDebugStr, DebugThickness);
