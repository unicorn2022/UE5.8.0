				...
				Controller->OpenBracket(LOCTEXT("FFortniteMainBranchObjectVersion::FixUpNoneNameAnimationCurves_Bracket","FFortniteMainBranchObjectVersion::FixUpNoneNameAnimationCurves"), bDoNotTransactAction);
				{
					...
					for (int32 Index = 0; Index < FloatCurves.Num(); ++Index)
					{
						...
						if (Curve.GetName() == NAME_None)
						{
							...
							UE_LOGF(LogAnimation, Warning, "[AnimSequence %ls] contains invalid curve name \'None\'. Renaming this to %ls. Please fix this curve in the editor. ", *GetFullName(), *Curve.GetName().ToString());
	...
	if (Context.IsLeader())
	{
		...
		if (DeltaTime != 0.f)
		{
			...
			else
			{
				...
				UE_LOGF(LogAnimMarkerSync, Log, "Leader (%ls) (normal advance)  - PreviousTime (%0.2f), CurrentTime (%0.2f), MoveDelta (%0.2f), Looping (%d) ", *GetName(), PreviousTime, CurrentTime, DeltaTime, Instance.bLooping ? 1 : 0);
			}
		}
	...
	else
	{
		...
		if (Instance.bCanUseMarkerSync)
		{
			...
			else
			{
				...
				FAnimationRuntime::AdvanceTime(Instance.bLooping, DeltaTime, CurrentTime, GetPlayLength());
				UE_LOGF(LogAnimMarkerSync, Log, "Follower (%ls) (normal advance) - PreviousTime (%0.2f), CurrentTime (%0.2f), MoveDelta (%0.2f), Looping (%d) ", *GetName(), PreviousTime, CurrentTime, DeltaTime, Instance.bLooping ? 1 : 0);
			}
		}
		else
		{
			...
			UE_LOGF(LogAnimMarkerSync, Log, "Follower (%ls) (normalized position advance) - PreviousTime (%0.2f), CurrentTime (%0.2f), MoveDelta (%0.2f), Looping (%d) ", *GetName(), PreviousTime, CurrentTime, DeltaTime, Instance.bLooping ? 1 : 0);
		}
...
void UAnimSequenceBase::TickByMarkerAsFollower(FMarkerTickRecord &Instance, FMarkerTickContext &MarkerContext, float& CurrentTime, float& OutPreviousTime, const float MoveDelta, const bool bLooping, const UMirrorDataTable* MirrorTable) const
{
	...
	UE_LOGF(LogAnimMarkerSync, Log, "Follower (%ls) (TickByMarker) PreviousTime(%0.2f) CurrentTime(%0.2f) MoveDelta(%0.2f) Looping(%d) %ls", *GetName(), OutPreviousTime, CurrentTime, MoveDelta, bLooping ? 1 : 0, *MarkerContext.ToString());
}
...
void UAnimSequenceBase::TickByMarkerAsLeader(FMarkerTickRecord& Instance, FMarkerTickContext& MarkerContext, float& CurrentTime, float& OutPreviousTime, const float MoveDelta, const bool bLooping, const UMirrorDataTable* MirrorTable) const
{
	...
	UE_LOGF(LogAnimMarkerSync, Log, "Leader (%ls) (TickByMarker) PreviousTime(%0.2f) CurrentTime(%0.2f) MoveDelta(%0.2f) Looping(%d) %ls", *GetName(), OutPreviousTime, CurrentTime, MoveDelta, bLooping ? 1 : 0, *MarkerContext.ToString());
}
		...
		if (!FMath::IsNearlyEqual(GetPlayLength(), ParentSeqBase->GetPlayLength()))
		{
			...
			else
			{
				UE_LOGF(LogTemp, Display,
						"Child %ls %ls has incompatible frame rate with Parent %ls. Child: %ls, Parent: %ls. Reimport the child animation with a compatible frame rate.",
...
void UAnimSequenceBase::Serialize(FArchive& Ar)
{
	...
	{
		UE_LOGF(LogAnimation, Fatal, "This package (%ls) was saved with a version that had to be backed out and is no longer able to be loaded.", *GetPathNameSafe(this));
	}
