...
void FReferenceSkeleton::Remove(const FName InBoneName, const bool bRemoveChildren)
{
	...
	{
		UE_LOGF(LogAnimation, Error, "Remove: '%ls' not found.", *InBoneName.ToString());
		return;
	}
	...
	if (RawParentIndex == INDEX_NONE)
	{
		...
		if (NumRoots == 1)
		{
			UE_LOGF(LogAnimation, Error, "Remove: cannot remove root bone as it's the only one in the hierarchy.");
			return;
		}
	}
...
{	
	...
	{
		UE_LOGF(LogAnimation, Error, "SetParent: bone '%ls' not found.", *InBoneName.ToString());
		return INDEX_NONE;
	}
	...
	{
		UE_LOGF(LogAnimation, Error, "SetParent: parent bone '%ls' not found.", *InParentName.ToString());
		return INDEX_NONE;
	}
	...
	while (ParentParentIndex > INDEX_NONE)
	{
		...
		{ // we do not allow to reparent to one of bone's children
			UE_LOGF(LogAnimation, Error, "SetParent: cannot parent '%ls' to one of its children.", *InBoneName.ToString());
			return INDEX_NONE;
		}
...
void FReferenceSkeleton::RebuildRefSkeleton(const USkeleton* Skeleton, bool bRebuildNameMap)
{
	...
	FinalRefBoneInfo = TArray<FMeshBoneInfo>(RawRefBoneInfo, NumVirtualBones); //-V781
	...
	if (NumVirtualBones > 0)
	{
		...
		for (int32 VirtualBoneIdx = 0; VirtualBoneIdx < NumVirtualBones; ++VirtualBoneIdx)
		{
			...
			const int32 SourceIndex = GetRawSourceBoneIndex(Skeleton, VB.SourceBoneName);	//-V595
...
#if WITH_EDITOR
		// Notify the skeleton that we changed
		Skeleton->HandleReferenceSkeletonChange();
#endif
	}
}
...
void FReferenceSkeleton::RemoveDuplicateBones(const UObject* Requester, TArray<FBoneIndexType> & DuplicateBones)
{
	...
	for (int32 BoneIndex = NumBones - 1; BoneIndex >= 0; BoneIndex--)
	{
		...
		else
		{
			...
			UE_LOGF(LogAnimation, Warning, "RemoveDuplicateBones: duplicate bone name (%ls) detected for (%ls)! Indices: %d and %d. Removing the latter.",
	...
	if (bRemovedBones || (RawNameToIndexMap.Num() == 0))
	{
		...
		if (!Skeleton)
		{
			...
			{
				UE_LOGF(LogAnimation, Warning, "RemoveDuplicateBones: Object supplied as requester (%ls) needs to be either Skeleton or SkeletalMesh", *GetFullNameSafe(Requester));
			}
		}
...
void FReferenceSkeleton::RebuildNameToIndexMap()
{
	...
	for (int32 BoneIndex = 0; BoneIndex < NumBones; BoneIndex++)
	{
		...
		{
			UE_LOGF(LogAnimation, Warning, "RebuildNameToIndexMap: Bone with no name detected for index: %d", BoneIndex);
		}
	}
...
void FReferenceSkeleton::EnsureParentsExist(TArray<FBoneIndexType>& InOutBoneSortedArray) const
{
	...
	if (NumBones == 0)
	{
		if (bWarn && !InOutBoneSortedArray.IsEmpty())
		{
			UE_LOGF(LogAnimation, Log, "FReferenceSkeleton::EnsureParentsExist, RefSkeleton is empty. Removing all bones from InOutBoneSortedArray.");
		}
		InOutBoneSortedArray.Empty();
		return;
	}
	
	// remove invalid indices
	InOutBoneSortedArray.RemoveAll([bWarn, NumBones](const FBoneIndexType BoneIndex)
	{
		const bool bRemove = BoneIndex >= NumBones;	
		if (bWarn && bRemove)
		{
			UE_LOGF(LogAnimation, Log,
				"FReferenceSkeleton::EnsureParentsExist, invalid BoneIndex[%d / %d]. Removing bone from InOutBoneSortedArray.",
				BoneIndex, NumBones);	
		}
		return bRemove;
	});
	
	if (InOutBoneSortedArray.IsEmpty())
	{
		return;
	}
	...
	BoneExists.Reset();
	BoneExists.Init(false, NumBones);
	...
	while (i < InOutBoneSortedArray.Num())
	{
		...
		{			
			...
			if (BoneExists.IsValidIndex(ParentIndex) && !BoneExists[ParentIndex])
			{
