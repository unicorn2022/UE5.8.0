...
// 	}
	...
	if( bEnableLogging )
	{
		UE_LOGF(LogAnimationCompression, Log, "Enumerate End Effectors for %ls", *Skeleton->GetFName().ToString());
	}
	for ( int32 BoneIndex = 0 ; BoneIndex < OutBoneData.Num() ; ++BoneIndex )
	{
		...
		if ( BoneData.IsEndEffector() )
		{
			...
			if( bEnableLogging )
			{
				UE_LOGF(LogAnimationCompression, Log, "\t %ls bKeyEndEffector: %d", *BoneData.Name.ToString(), BoneData.bKeyEndEffector);
			}
		}
	}
#if 0
	UE_LOGF(LogAnimationCompression, Log,  "====END EFFECTORS:" );
	...
	UE_LOGF(LogAnimationCompression, Log,  "====NUM EFFECTORS %i(%i)", NumEndEffectors, OutBoneData(0).Children.Num() );
	UE_LOGF(LogAnimationCompression, Log,  "====NON END EFFECTORS:" );
	for ( int32 BoneIndex = 0 ; BoneIndex < OutBoneData.Num() ; ++BoneIndex )
	{
		const FBoneData& BoneData = OutBoneData(BoneIndex);
		if ( !BoneData.IsEndEffector() )
	...
	UE_LOGF(LogAnimationCompression, Log,  "===================" );
#endif
}
...
// 	UE_LOGF(LogAnimation, Log, "GetBindPoseAtom BoneIndex: %d, OutBoneAtom: %ls", BoneIndex, *OutBoneAtom.ToString());
// #endif
}
...
FString FAnimationUtils::GetAnimationCompressionFormatString(AnimationCompressionFormat InFormat)
{
	...
	{
	case ACF_None:
		return FString("ACF_None");
	case ACF_Float96NoW:
		return FString("ACF_Float96NoW");
	case ACF_Fixed48NoW:
		return FString("ACF_Fixed48NoW");
	case ACF_IntervalFixed32NoW:
		return FString("ACF_IntervalFixed32NoW");
	case ACF_Fixed32NoW:
		return FString("ACF_Fixed32NoW");
	case ACF_Float32NoW:
		return FString("ACF_Float32NoW");
	case ACF_Identity:
		return FString("ACF_Identity");
	default:
		UE_LOGF(LogAnimationCompression, Warning, "AnimationCompressionFormat was not found:  %i", static_cast<int32>(InFormat) );
	}
...
FString FAnimationUtils::GetAnimationKeyFormatString(AnimationKeyFormat InFormat)
{
	...
	case AKF_ConstantKeyLerp:
		return FString("AKF_ConstantKeyLerp");
	case AKF_VariableKeyLerp:
		return FString("AKF_VariableKeyLerp");
	case AKF_PerTrackCompression:
		return FString("AKF_PerTrackCompression");
	default:
		UE_LOGF(LogAnimationCompression, Warning, "AnimationKeyFormat was not found:  %i", static_cast<int32>(InFormat) );
	}
...
static void EnsureDependenciesAreLoaded(UObject* Object)
{
	...
	{
		UE_LOGF(LogAnimationCompression, Warning, "EnsureDependenciesAreLoaded: [Preload] %ls", *Object->GetPathName());
	}
	...
	{
		UE_LOGF(LogAnimationCompression, Warning, "EnsureDependenciesAreLoaded: [ConditionalPostLoad] %ls", *Object->GetPathName());
	...
	for (UObject* Dependency : ObjectReferences)
	{
		...
		{
			UE_LOGF(LogAnimationCompression, Warning, "EnsureDependenciesAreLoaded: [Preload Dependency] %ls", *Dependency->GetPathName());
		}
		...
		{
			UE_LOGF(LogAnimationCompression, Warning, "EnsureDependenciesAreLoaded: [ConditionalPostLoad Dependency] %ls", *Dependency->GetPathName());
...
UObject* GetDefaultAnimationCompressionSettings(const TCHAR* IniValueName, bool bIsFatal)
{
	...
	if (Value == nullptr)
	{
		if (bIsFatal)
		{
			UE_LOGF(LogAnimationCompression, Fatal, "Couldn't find default compression setting for '%ls' under '[Animation.DefaultObjectSettings]'", IniValueName);
		}
		else
		{
			UE_LOGF(LogAnimationCompression, Warning, "Couldn't find default compression setting for '%ls' under '[Animation.DefaultObjectSettings]'", IniValueName);
		}
	...
	if (DefaultCompressionSettings == nullptr)
	{
		if (bIsFatal)
		{
			UE_LOGF(LogAnimationCompression, Fatal, "Couldn't load default compression settings asset with path '%ls'", *CompressionSettingsName);
		}
		else
		{
			UE_LOGF(LogAnimationCompression, Warning, "Couldn't load default compression settings asset with path '%ls'", *CompressionSettingsName);
		}
...
void FAnimationUtils::ExtractTransformFromTrack(const FRawAnimSequenceTrack& RawTrack, double Time, int32 NumFrames, double SequenceLength, EAnimInterpolationType Interpolation, FTransform &OutAtom)
{
	...
	// 	UE_LOGF(LogAnimation, Log, " *  *  *  Position. PosKeyIndex1: %3d, PosKeyIndex2: %3d, Alpha: %f", PosKeyIndex1, PosKeyIndex2, Alpha);
	// 	UE_LOGF(LogAnimation, Log, " *  *  *  Rotation. RotKeyIndex1: %3d, RotKeyIndex2: %3d, Alpha: %f", RotKeyIndex1, RotKeyIndex2, Alpha);
