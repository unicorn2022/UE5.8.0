...
float FAnimNotifyEvent::GetEndTriggerTime() const
{
	...
	{
		UE_LOGF(LogAnimNotify, Log, "Anim Notify %ls is non state, but has an EndTriggerTimeOffset %f!", *NotifyName.ToString(), EndTriggerTimeOffset);
	}
...
{
	CollectMarkersInRange(PrevPosition, NewPosition, OutMarkersPassedThisTick, TotalDeltaMove, nullptr);
}

extern FName MarkerOrMirroredName(FName MarkerToFind, const UMirrorDataTable* MirrorTable);

void FMarkerSyncData::CollectMarkersInRange(float PrevPosition, float NewPosition, TArray<FPassedMarker>& OutMarkersPassedThisTick, float TotalDeltaMove, const UMirrorDataTable* MirrorDataTable)
{
	for (const FAnimSyncMarker& Marker : AuthoredSyncMarkers)
	{
		if (Marker.Time >= PrevPosition && Marker.Time < NewPosition)
		{
			...
			OutMarkersPassedThisTick[PassedMarker].PassedMarkerName = MarkerOrMirroredName(Marker.MarkerName, MirrorDataTable);
