...
{
	return FSkinWeightProfilesData::AllowCPU();
}

bool FSkinWeightProfileManager::HandleDelayedLoads()
{
	return FSkinWeightProfilesData::HandleDelayedLoads();
}
	...
	)
{
	...
	if (USkeletalMesh* const Mesh = Cast<USkeletalMesh>(SkinnedAsset))
	{
		...
		UE_LOGF(LogSkinWeightProfileManager, Display, "RequestSkinWeightProfile [%ls | %ls]", *Mesh->GetName(), *Requester->GetName());
...
void FSkinWeightProfileManager::CancelSkinWeightProfileRequest(UObject* Requester)
{
	...
	for (FSetProfileRequest& Request : PendingSetProfileRequests)
	{
		if (Request.IdentifyingObject == Requester || Request.WeakSkeletalMesh == Requester)
		{
			...
			UE_LOGF(LogSkinWeightProfileManager, Display, "Cancel [%ls]", *Requester->GetName());			
		}
	}
}
...
{	
	...
	if (PendingSetProfileRequests.Num())
	{
		...
		// Release stream-out protection for meshes whose readback has progressed past
		// the danger window
		TickReleaseStreamOutProtection_GameThread();
		...
		for (int32 RequestIndex = 0; RequestIndex < PendingSetProfileRequests.Num(); ++RequestIndex)
		{
			...
			{	
				...
				if (bAllBuffersReady)
				{
					...
					{
						UE_LOGF(LogSkinWeightProfileManager, Display, "Callback [%ls | %ls]", *SkeletalMesh->GetName(), *Request.IdentifyingObject->GetName());
	...
	{
		// Drain any leftover protections even if no requests are pending this tick.
		TickReleaseStreamOutProtection_GameThread();
		TickFunction.SetTickFunctionEnable(false);
	}
}

void FSkinWeightProfileManager::RegisterStreamOutProtection_AsyncSafe(USkinnedAsset* SkinnedAsset)
{
	if (!SkinnedAsset)
	{
		return;
	}
	FScopeLock Lock(&ProtectedSkinnedAssetsCS);
	ProtectedSkinnedAssets.Add(TWeakObjectPtr<USkinnedAsset>(SkinnedAsset));
}

void FSkinWeightProfileManager::TickReleaseStreamOutProtection_GameThread()
{
	using EReadbackStatus = FSkinWeightProfilesData::EReadbackStatus;

	FScopeLock Lock(&ProtectedSkinnedAssetsCS);
	if (ProtectedSkinnedAssets.IsEmpty())
	{
		return;
	}
	...
	for (auto It = ProtectedSkinnedAssets.CreateIterator(); It; ++It)
	{
		USkinnedAsset* Asset = It->Get();

		// GC'd: streaming manager already cleaned up via RemoveStreamingRenderAsset.
		if (!Asset)
		{
			It.RemoveCurrent();
			continue;
		...
		if (!RenderData)
		{
			RAMgr.UnregisterStreamOutProtectedAsset(Asset);
			It.RemoveCurrent();
			continue;
		}

		// Protection must hold across two windows:
		//  1. PendingGPUReadback   — set on GT/Async; the RT lambda has not yet recorded
		//                            FRHICommandCopyToStagingBuffer onto the RHI cmd list.
		//  2. GPUReadbackEnqueued  — RT lambda recorded the Copy, but the RHI thread has
		//                            not executed it yet (and the GPU certainly hasn't
		//                            completed it). A StreamOut-batched Replace landing on
		//                            the cmd list before the Copy executes nulls the
		//                            source's UnderlyingResource and crashes the readback.
		//
		// Status transitions GPUReadbackEnqueued -> DataReadbackEnqueued only after
		// BufferReadback->IsReady() (RHI fence signaled, GPU done with the Copy). Past
		// that point the staging buffer holds the data and the source can be replaced.
		bool bAnyStillPending = false;
		for (const FSkeletalMeshLODRenderData& LODData : RenderData->LODRenderData)
		{
			const EReadbackStatus Status = LODData.SkinWeightProfilesData.GetReadbackStatus();
			if (Status == EReadbackStatus::PendingGPUReadback ||
				Status == EReadbackStatus::GPUReadbackEnqueued)
			{
				...
				for (const int32 LODIndex : Request.LODIndices)
				{
					...
					else
					{
						if (SkinweightData.IsPendingReadback())
						{
							...
							{
								// Atomically test "no pending streaming op" and protect against
								// StreamOut for the duration of the readback. 
								USkeletalMesh* MutableMesh = const_cast<USkeletalMesh*>(SkeletalMesh);
								IRenderAssetStreamingManager& RAMgr = IStreamingManager::Get().GetRenderAssetStreamingManager();
								if (RAMgr.TryRegisterStreamOutProtectedAsset(MutableMesh))
								{
									SkinweightData.EnqueueGPUReadback();
									Owner->RegisterStreamOutProtection_AsyncSafe(MutableMesh);
								}
								// else: pending streaming op in flight; leave status at None and retry next tick.
							}
