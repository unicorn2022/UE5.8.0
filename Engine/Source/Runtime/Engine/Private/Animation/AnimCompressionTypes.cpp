...
#if WITH_EDITOR
int32 FCompressibleAnimData::GUseHighFidelityAdditiveCurveCompression = 0;
static FAutoConsoleVariableRef CVarUseHighFidelityAdditiveCurveCompression(TEXT("a.Compression.UseHighFidelityAdditiveCurves"), FCompressibleAnimData::GUseHighFidelityAdditiveCurveCompression, TEXT("When enabled, uses improved additive curve baking that preserves source curve fidelity for non-AnimScaled types and correctly determines interpolation modes for AnimScaled."));
...
void FCompressibleAnimData::BakeOutAdditiveIntoRawData(const FFrameRate& SampleRate, TArray<FBoneAnimationTrack>& ResampledTrackData, TArray<FFloatCurve>& FloatCurves)
{
...

	{
		...
		TMap<FName, TArray<float, FAnimStackAllocator>> PerFrameCurveValues;
		for (int32 KeyIndex = 0; KeyIndex < SampledKeys; ++KeyIndex)
		{
			...
			if (GUseHighFidelityAdditiveCurveCompression && AnimSequence->RefPoseType == EAdditiveBasePoseType::ABPT_AnimScaled)
			{
				// Store results of float curve evaluation(s), non-singular pose additive type only - used for further processing
				Curve.ForEachElement([&PerFrameCurveValues, KeyIndex, SampledKeys](const UE::Anim::FCurveElement& InElement)
				{
					TArray<float, FAnimStackAllocator>& Values = PerFrameCurveValues.FindOrAdd(InElement.Name);
					if (Values.IsEmpty())
					{
						Values.SetNumZeroed(SampledKeys);
					}
					Values[KeyIndex] = InElement.Value;
				});
			}
			else
			{
				const double PreviousKeyTime = SampleRate.AsSeconds(KeyIndex - 1);
			    const FFrameTime CurrentFrameTime = KeyIndex;
		...
		if (GUseHighFidelityAdditiveCurveCompression)
		{
			// Can skip any processing and copy float curves directly (offset with constant base animation curve value for RefPose/LocalAnimFrame/AnimFrame) 
			if (AnimSequence->RefPoseType != EAdditiveBasePoseType::ABPT_AnimScaled)
			{
				const TArray<FFloatCurve>& SourceCurves = DataModelInterface->GetFloatCurves();
				for (const FFloatCurve& SourceCurve : SourceCurves)
				{
					FFloatCurve* RawCurve = FindOrAddCurve(FloatCurves, SourceCurve.GetName());
					RawCurve->FloatCurve = SourceCurve.FloatCurve;

					if (AnimSequence->RefPoseType == EAdditiveBasePoseType::ABPT_LocalAnimFrame)
					{
						const double BaseTime = AnimSequence->GetTimeAtFrame(AnimSequence->RefFrameIndex);
						const float BaseValue = AnimSequence->EvaluateCurveData(SourceCurve.GetName(), BaseTime, true);
						if (!FMath::IsNearlyZero(BaseValue))
						{
							for (FRichCurveKey& Key : RawCurve->FloatCurve.Keys)
							{
								Key.Value -= BaseValue;
							}
						}
					}
					else if (AnimSequence->RefPoseType == EAdditiveBasePoseType::ABPT_AnimFrame && AdditiveBaseAnimation)
					{
						const double BaseTime = AdditiveBaseAnimation->GetTimeAtFrame(AnimSequence->RefFrameIndex);
						const float BaseValue = AdditiveBaseAnimation->EvaluateCurveData(SourceCurve.GetName(), BaseTime, true);
						if (!FMath::IsNearlyZero(BaseValue))
						{
							for (FRichCurveKey& Key : RawCurve->FloatCurve.Keys)
							{
			...
			// Otherwise do sub-sampling to produce linear/constant keyed curve values
			else if (AnimSequence->RefPoseType == EAdditiveBasePoseType::ABPT_AnimScaled)
			{
				// Helper to evaluate a single additive curve value at an arbitrary time using per-curve EvaluateCurveData, matches ConvertToAdditive
				auto EvaluateAdditiveCurveAtTime = [AnimSequence, AdditiveBaseAnimation](FName CurveName, double Time) -> float
				{
					constexpr bool bEvaluateRawCurveData = true;
					const float SourceValue = AnimSequence->EvaluateCurveData(CurveName, Time, bEvaluateRawCurveData);
					check(AdditiveBaseAnimation);
					const double Fraction = AnimSequence->GetPlayLength() > 0.0 ? FMath::Clamp(Time / AnimSequence->GetPlayLength(), 0.0, 1.0) : 0.0;
					const double BaseTime = AdditiveBaseAnimation->GetPlayLength() * Fraction;
					return SourceValue - AdditiveBaseAnimation->EvaluateCurveData(CurveName, BaseTime, bEvaluateRawCurveData);
				};

				for (const TPair<FName, TArray<float, FAnimStackAllocator>>& Pair : PerFrameCurveValues)
				{
					const FName& CurveName = Pair.Key;
					const TArray<float, FAnimStackAllocator>& FrameValues = Pair.Value;

					FFloatCurve* RawCurve = nullptr;

					for (int32 SampledFrameIndex = 0; SampledFrameIndex < FrameValues.Num(); ++SampledFrameIndex)
					{
						const float CurveWeight = FrameValues[SampledFrameIndex];
						const double CurrentKeyTime = SampleRate.AsSeconds(SampledFrameIndex);
						const double PreviousKeyTime = SampleRate.AsSeconds(SampledFrameIndex - 1);

						if (!RawCurve && !FMath::IsNearlyZero(CurveWeight))
						{
							RawCurve = FindOrAddCurve(FloatCurves, CurveName);
						}
						...
						if (RawCurve)
						{
							...
							if (!bHasKeys)
							{
								// Add pre (leading) key of 0
								if (SampledFrameIndex > 0)
								{
									RawCurve->UpdateOrAddKey(0.f, PreviousKeyTime);
								}
								
								// Add current non-zero key
								RawCurve->UpdateOrAddKey(CurveWeight, CurrentKeyTime);
							}
							
							// Compare against previously set key value
							else if (IsNewKeyDifferent(RawCurve->FloatCurve.GetLastKey(), CurveWeight))
							{
								RawCurve->UpdateOrAddKey(CurveWeight, CurrentKeyTime);
								TArray<FRichCurveKey>& CurveKeys = RawCurve->FloatCurve.Keys;
								if (CurveKeys.Num() > 1)
								{
									...
									// Sample at quarter and three-quarter points. Default to Linear; set to Constant only when both samples are held at PrevKey.Value
									const double KeySpan = CurrentKeyTime - PrevKey.Time;
									const double QuarterTime = PrevKey.Time + KeySpan * 0.25;
									const double ThreeQuarterTime = PrevKey.Time + KeySpan * 0.75;

									const float QuarterSample = EvaluateAdditiveCurveAtTime(CurveName, QuarterTime);
									const float ThreeQuarterSample = EvaluateAdditiveCurveAtTime(CurveName, ThreeQuarterTime);

									const bool bQuarterIsConstant = FMath::IsNearlyEqual(QuarterSample, PrevKey.Value, UE_KINDA_SMALL_NUMBER);
									const bool bThreeQuarterIsConstant = FMath::IsNearlyEqual(ThreeQuarterSample, PrevKey.Value, UE_KINDA_SMALL_NUMBER);

									if (bQuarterIsConstant && bThreeQuarterIsConstant)
									{
										PrevKey.InterpMode = RCIM_Constant;
									}
									else
            ...
            {
				ensureAlwaysMsgf(false, TEXT("Invalid additive animation type value %i"), (int32)AnimSequence->RefPoseType);
            }
		}
	}
}
...
bool FCompressibleAnimData::FetchData(const ITargetPlatform* InPlatform)
{
#if WITH_EDITOR
	if (bDataFetched)
	{
	...
	// GC can mark sequence as unreachable during mark phase which makes TWeakObjPtr return nullptr. Cannot control all downstream calls of .Get() so use scope-guard instead
	FGCScopeGuard GCScopeGuard;
	
	// Could have been cancelled after reachability phase 
	if (IsCancelled())
	{		
		return false;
	}
	...
	// Attempt to retrieve animation sequence, if null check whether it is actually null or just unreachable (GC-ed but not yet destroyed)
	const UAnimSequence* AnimSequence = WeakSequence.Get();
	if (!AnimSequence)
	{
		if ( WeakSequence.GetEvenIfUnreachable() != nullptr)
		{
			UE_LOGF(LogAnimation, Display, "%ls was marked as garbage, but compression task has not been cancelled yet - skipping.", *FullName);
		}
		else
		{
			UE_LOGF(LogAnimation, Display, "%ls became null, invalid state - skipping.", *FullName);
		}
	...
	if (!Skeleton)
	{
		UE_LOGF(LogAnimation, Display, "Invalid Skeleton while trying to fetch to-compress animation data - skipping %ls.", *AnimSequence->GetPathName());
		return false;
	}
...
{
	// Multi-threaded async loading does not enforce pre-load dependencies specified during cook
	// and instead requires explicit dependencies
...
void FCompressedAnimSequence::SerializeCompressedData(FArchive& Ar, bool bDDCData, UObject* DataOwner, USkeleton* Skeleton, UAnimBoneCompressionSettings* BoneCompressionSettings, UAnimCurveCompressionSettings* CurveCompressionSettings, bool bCanUseBulkData)
{
	...
	if (Ar.IsLoading())
	{
		...
		bool bBoneDataLoadSuccessful = true;
		...
		if (bUseBulkDataForLoad)
		{
			...
			if (Size == 0)
			{
				// If we have no bone data, there is nothing to load
				bIsDataPresent = true;
			}
			else
			{
				// We expect some data
				bIsDataPresent = OwnedPtr->GetPointer() != nullptr;
			}

			if (ensureMsgf(bIsDataPresent, TEXT("Compressed animation data failed to load")))
			{
#if WITH_EDITOR
			...
			{
				// Memory mapping failed, this is unexpected
				// We'll clear everything and output a warning below
				bBoneDataLoadSuccessful = false;
			}
		...
		// We need them to be fully loaded in order to be able to look up the codec used to compress our data within
		// The codec typically lives with the compressed data which ends up in the DDC, as such we cannot maintain
		// a stable UObject reference. We use a lookup mechanism instead.
		...
		if (!BoneCompressionCodec && BoneCodecDDCHandle.Len() > 0)
		{
			UE_LOGF(LogAnimation, Warning, "%ls: Failed to find bone compression codec for DDC handle '%ls'. Compressed bone data will not be available.", *DataOwner->GetPathName(), *BoneCodecDDCHandle);
		}

		if (!CurveCompressionCodec && CurveCodecPath.Len() > 0)
		{
			UE_LOGF(LogAnimation, Warning, "%ls: Failed to find curve compression codec for codec path '%ls'. Compressed curve data will not be available.", *DataOwner->GetPathName(), *CurveCodecPath);
		}

		if (BoneCompressionCodec != nullptr && bBoneDataLoadSuccessful)
		{
		...
		if (!bBoneDataLoadSuccessful)
		{
			{
				// Still consume the codec-side archive bytes to keep Ar position correct.
				TUniquePtr<ICompressedAnimData> Throwaway(BoneCompressionCodec->AllocateAnimData());
				Throwaway->SerializeCompressedData(DataOwner, Ar);
			}

			CompressedDataStructure = nullptr;
			BoneCompressionCodec = nullptr;
			CompressedByteStream.Empty(0);
			CompressedTrackToSkeletonMapTable.Empty(0);

#if !NO_LOGGING
			// Warn once per process so the failure is visible without spamming every frame.
			static bool bHasWarnedMissingCompressedData = false;
			if (!bHasWarnedMissingCompressedData)
			{
				bHasWarnedMissingCompressedData = true;
				UE_LOG(LogAnimation, Error,
					TEXT("Failed to serialize bone data on load for AnimSequence '%s'; returning ref pose. ")
					TEXT("Likely cause: a memory-mapped bulk-data chunk failed to install (IoStore On-Demand). ")
					TEXT("Subsequent occurrences this session will be suppressed."),
					*DataOwner->GetPathName());
			}
#endif
		}

#if WITH_EDITOR
		// Mark as containing valid compressed data. Covers all deserialization paths (cooked load, DDC fetch, duplication).
		bHasValidCompressedData = true;
#endif
	}
...
#else
			UE_LOGF(LogAnimation, Fatal, "Can't save animation as bulk data in non-editor builds!");
#endif
		}
		else
		{
...
{
#if WITH_EDITOR
	// Flag ensures data was fetched or locally compressed, catches default constructed FCompressedAnimSequence false-positive
	return (bHasValidCompressedData || AnimSequence->GetOutermost()->HasAnyPackageFlags(PKG_Cooked)) && IsBoneDataValid(AnimSequence, bLogInformation) && IsCurveDataValid(AnimSequence, bLogInformation);
#else
	return true;
#endif
}
