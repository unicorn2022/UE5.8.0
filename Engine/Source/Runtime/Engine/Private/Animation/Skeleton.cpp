...
#if WITH_EDITOR
const FName USkeleton::AnimNotifyTag = FName("AnimNotifyList");
const FString USkeleton::AnimNotifyTagDelimiter = TEXT(";");

const FName USkeleton::AnimSyncMarkerTag = FName("AnimSyncMarkerList");
const FString USkeleton::AnimSyncMarkerTagDelimiter = TEXT(";");

const FName USkeleton::CurveNameTag = FName("CurveNameList");
const FString USkeleton::CurveTagDelimiter = TEXT(";");

const FName USkeleton::AttributeTag = FName("AttributeList");

const FName USkeleton::CompatibleSkeletonsNameTag = FName("CompatibleSkeletonList");
...
const FName FAnimSlotGroup::DefaultGroupName = FName("DefaultGroup");
const FName FAnimSlotGroup::DefaultSlotName = FName("DefaultSlot");
...
{
	const FString VirtualBonePrefix("VB ");
...
void USkeleton::Serialize( FArchive& Ar )
{
	...
	{
		UE_LOGF(LogAnimation, Warning, "Skeleton '%ls' has not been saved since version 'VER_UE4_SKELETON_GUID_SERIALIZATION' This asset will not cook deterministically until it is resaved.", *GetPathName());
		RegenerateGuid();
	}
	else
	{
...
#if WITH_EDITORONLY_DATA
	// Drain the legacy FSmartNameContainer payload from source assets predating
	// RemovedSmartNameContainerPayload. Editor-only: cooked runtime never loads stale packages.
	if (Ar.UEVer() >= VER_UE4_SKELETON_ADD_SMARTNAMES)
	{
		Ar.UsingCustomVersion(FUE5ReleaseStreamObjectVersion::GUID);
		if (Ar.CustomVer(FUE5ReleaseStreamObjectVersion::GUID)
				< FUE5ReleaseStreamObjectVersion::RemovedSmartNameContainerPayload)
		{
			PRAGMA_DISABLE_DEPRECATION_WARNINGS
...
bool USkeleton::IsCompatibleMesh(const USkinnedAsset* InSkinnedAsset, bool bDoParentChainCheck) const
{
	...
	for (int32 MeshBoneIndex=0; MeshBoneIndex<NumBones; MeshBoneIndex++)
	{
		...
		if( SkeletonBoneIndex != INDEX_NONE )
		{
			...
			{
				UE_LOGF(LogAnimation, Verbose, "%ls : Hierarchy does not match.", *MeshBoneName.ToString());
				return false;
			}
		}
		else
		{
			...
			if( SkeletonBoneIndex == INDEX_NONE )
			{
				UE_LOGF(LogAnimation, Verbose, "%ls : Missing joint on skeleton.  Make sure to assign to the skeleton.", *MeshBoneName.ToString());
				return false;
			}
...
void USkeleton::BuildLinkupData(const USkinnedAsset* InSkinnedAsset, FSkeletonToMeshLinkup& NewMeshLinkup)
{
	...
	bool bPopulatedMissingBoneNames = false;
#endif
	...
	for (int32 MeshBoneIndex = 0; MeshBoneIndex < NumMeshBones; MeshBoneIndex++)
	{
		...
		if (SkeletonBoneIndex == INDEX_NONE)
		{
			...
			// Gather remaining missing bone data
			// In case the merge succeeds, we need to gather this data before the merge to populate the message log correctly 
			if (!bPopulatedMissingBoneNames)
			{
				MissingBoneNames.Add(MeshBoneName.ToString());
				for (int32 CheckMeshBoneIndex = MeshBoneIndex + 1; CheckMeshBoneIndex < NumMeshBones; ++CheckMeshBoneIndex)
				{
					const FName MissingMeshBoneName = MeshRefSkel.GetBoneName(CheckMeshBoneIndex);
					if (SkeletonRefSkel.FindBoneIndex(MissingMeshBoneName) == INDEX_NONE)
					{
						MissingBoneNames.Add(MissingMeshBoneName.ToString());
					}
				}
				bPopulatedMissingBoneNames = true;
			}
			
			// Re-add all SkelMesh bones to the Skeleton.
			const bool bSuccessfulMerge = MergeAllBonesToBoneTree(InSkinnedAsset);
			...
			{
				// If the merge failed we only display the current missing bone as this error will be hit again for future missing bones. 
				// If the merge succeeded, we don't expect this error to be hit again so we display all missing bone names. 
				const FString AllMissingBonesNames = bSuccessfulMerge ? FString::Join(MissingBoneNames, TEXT(",")) : MeshBoneName.ToString();
				...
				Message->AddToken(FTextToken::Create(FText::Format(LOCTEXT("SkeletonBuildLinkupMissingBones4", " Missing bone name(s): {0}"), FText::FromString(AllMissingBonesNames))));
				LoadErrors.Open();
			...
			if (!bSpawnedErrorMessage)
			{
				...
				UE_LOGF(LogAnimation, Error, "USkeleton::BuildLinkup: The Skeleton %ls, is missing bones that SkeletalMesh %ls needs. MeshBoneName %ls",
...
bool USkeleton::MergeBonesToBoneTree(const USkinnedAsset* InSkinnedAsset, const TArray<int32> & RequiredRefBones, bool bShowProgress /*= true*/)
{
	...
	else
	{
		...
		if( IsCompatibleMesh(InSkinnedAsset, bDoParentChainCheck) )
		{
			...
			const FReferenceSkeleton& InputRefSkeleton = InSkinnedAsset->GetRefSkeleton();
			const TArray<FMeshBoneInfo>& InputBoneInfos = InputRefSkeleton.GetRefBoneInfo();
			
			// Exclude bones who do not have a parent.
			TArray<int32> FilteredRequiredBones;
			FAnimationRuntime::ExcludeBonesWithNoParents(RequiredRefBones, InputRefSkeleton, FilteredRequiredBones);
			...
			// Two modifier passes: add bones that dont already exist, then set bone parents that have changed
			{
				...
				{
					const FName& InputBoneName = InputRefSkeleton.GetBoneName(MeshBoneIndex);
					const int32 SkeletonBoneIndex = ReferenceSkeleton.FindRawBoneIndex(InputBoneName);
					
					// Bone doesn't already exist. Add it.
					if( SkeletonBoneIndex == INDEX_NONE )
					{
						FMeshBoneInfo NewMeshBoneInfo = InputBoneInfos[MeshBoneIndex];
						// Fix up ParentIndex for our new Skeleton.
						...
						{
							NewMeshBoneInfo.ParentIndex = ReferenceSkeleton.FindRawBoneIndex(InputRefSkeleton.GetBoneName(InputRefSkeleton.GetParentIndex(MeshBoneIndex)));
						}

						RefSkelModifier.Add(NewMeshBoneInfo, InputRefSkeleton.GetRefBonePose()[MeshBoneIndex]);
						BoneTree.AddDefaulted(1);
						bShouldHandleHierarchyChange = true;
					}
					else
					{
						// handle case-sensitive only change
						const FName& BoneName = ReferenceSkeleton.GetBoneName(SkeletonBoneIndex);
						if (InputBoneName == BoneName && !InputBoneName.IsEqual(BoneName, ENameCase::CaseSensitive))
						{
							RefSkelModifier.Rename(BoneName, InputBoneName);
						}
					}
				}
			}

			{
				...
				for (const int32 MeshBoneIndex : FilteredRequiredBones)
				{
					const FName BoneName = InputRefSkeleton.GetBoneName(MeshBoneIndex);
					...
					if(SkeletonBoneIndex != INDEX_NONE)
					{
						...
						const int32 MeshParentIndex = InputRefSkeleton.GetParentIndex(MeshBoneIndex);
						const int32 SkeletonParentIndex = ReferenceSkeleton.GetRawParentIndex(SkeletonBoneIndex);
						const FName MeshParentName = MeshParentIndex != INDEX_NONE ? InputRefSkeleton.GetBoneName(MeshParentIndex) : NAME_None;
...
void USkeleton::UpdateRetargetSource( const FName Name )
{
	...
	if (PoseFound)
	{
		...
		{
			UE_LOGF(LogAnimation, Warning, "Reference Mesh for Retarget Source %ls has been removed.", *GetName());
		}
	}
}
...
void USkeleton::HandleReferenceSkeletonChange() const
{
	TRACE_CPUPROFILER_EVENT_SCOPE(USkeleton::HandleReferenceSkeletonChange);
	OnReferenceSkeletonChanged.Broadcast();
}
...
void USkeleton::RegisterOnReferenceSkeletonChanged(const FOnReferenceSkeletonChanged& Delegate)
{
	OnReferenceSkeletonChanged.Add(Delegate);
}

void USkeleton::UnregisterOnReferenceSkeletonChanged(FDelegateUserObject Unregister)
{
	OnReferenceSkeletonChanged.RemoveAll(Unregister);
}
...
void USkeleton::HandleVirtualBoneChanges()
{
	...
	auto NeedsReRegistration = [this, &SkeletalMeshTransacting](const ISkinnedMeshComponent* InMeshComponent)
	{
		if (!InMeshComponent)
		{
			return false;
	...
	for (ISkinnedMeshComponent* Component : ObjectCacheScope.GetContext().GetSkinnedMeshComponents())
	{
		if (NeedsReRegistration(Component))
		{
			Component->PostSkeletalHierarchyChange();
		}
	}
