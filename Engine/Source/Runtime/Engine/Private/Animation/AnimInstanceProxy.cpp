...
const FName NAME_AnimBlueprintLog("AnimBlueprintLog");
const FName NAME_PIELog("PIE");
const FName NAME_Evaluate("Evaluate");
const FName NAME_Update("Update");
const FName NAME_AnimGraph("AnimGraph");

bool GWarnOnDuplicateSlotNode = true;
static FAutoConsoleVariableRef CVarWarnOnDuplicateSlotNode(
	TEXT("a.WarnOnDuplicateSlotNode"),
	GWarnOnDuplicateSlotNode,
	TEXT("1 = Warn when duplicate slot nodes exist in the anim graph. 0 = off"));
...
void FAnimInstanceProxy::RegisterSlotNodeWithAnimInstance(const FName& SlotNodeName)
{
	...
	if (SlotNameToTrackerIndex.Contains(SlotNodeName))
	{
		if (GWarnOnDuplicateSlotNode)
		{
			...
			{
				UE_LOGF(LogAnimation, Warning, "SLOTNODE: '%ls' in animation instance class %ls already exists. Remove duplicates from the animation graph for this class.", *SlotNodeName.ToString(), *ClassNameString);
			}
		}
		return;
	}
...
void FAnimInstanceProxy::UpdateAnimation()
{
	...
	UE_LOG_CONTEXT("AnimBlueprint", FTopLevelAssetPath(GetAnimInstanceObject() ? GetAnimInstanceObject()->GetClass() : nullptr));
	...
	// Now that we have updated our asset players we record their tick records from read buffer (meaning they have up-to-date info).
#if ANIM_TRACE_ENABLED
	{
		for (const TTuple<FName, FAnimGroupInstance>& TickRecordGroup : Sync.GetSyncGroupMapRead())
		{
		...
		for (const FAnimTickRecord& TickRecord : Sync.GetUngroupedActivePlayersRead())
		{
			TRACE_ANIM_TICK_RECORD(Context, TickRecord)
		}
	}
...
void FAnimInstanceProxy::SlotEvaluatePoseWithBlendProfiles(const FName& SlotNodeName, const FAnimationPoseData& SourceAnimationPoseData, float InSourceWeight, FAnimationPoseData& OutBlendedAnimationPoseData, float InBlendWeight)
{
	...
	for (int32 Index = 0; Index < GetMontageEvaluationData().Num(); ++Index)
	{
		...
		if (Montage && Montage->IsValidSlot(SlotNodeName))
		{
			...
			// Sum up the total weights per bone for non-additive poses.
			if (AdditiveAnimType == AAT_None)
			{
				for (int32 BoneIndex = 0; BoneIndex < NumBones; ++BoneIndex)
				{
		...
		if (Montage->IsValidSlot(SlotNodeName))
		{
			...
			if (UE_TRACE_CHANNELEXPR_IS_ENABLED(AnimationChannel))
			{
				...
				if (const FAnimSegment* const AnimSegment = AnimTrack->GetSegmentAtTime(ClampedTime))
				{
					...
					TickRecord.SourceProxy = Context.AnimInstanceProxy;
...
void FAnimInstanceProxy::GetSlotWeight(const FName& SlotNodeName, float& out_SlotNodeWeight, float& out_SourceWeight, float& out_TotalNodeWeight) const
{
...
#endif
#if ENABLE_ANIM_LOGGING
				UE_LOGF(LogAnimation, Verbose, "GetSlotWeight : Owner: %ls, AnimMontage: %ls,  (DesiredWeight:%0.2f, Weight:%0.2f)",
	...
	else if (TotalDesiredWeight == 1.f && TotalSum < 1.f - ZERO_ANIMWEIGHT_THRESH)
	{
		...
		UE_LOGF(LogAnimation, Warning, "[%ls] Montage has less weight. Blending in?(%f)", *SlotNodeName.ToString(), TotalSum);
	}
#endif
