...
bool FStreamingAnimationData::Initialize(UAnimStreamable* InStreamableAnim, FAnimationStreamingManager* InAnimationStreamingManager)
{
	...
	{
		UE_LOGF(LogAnimation, Error, "Failed to initialize streaming animation due to lack of anim or serialized stream chunks. '%ls'", *InStreamableAnim->GetFullName());
		return false;
	}
...
void FStreamingAnimationData::BeginPendingRequests(const TArray<uint32>& IndicesToLoad, const TArray<uint32>& IndicesToFree)
{
	...
	for (uint32 ChunkIndex : IndicesToLoad)
	{
		...
		{
			UE_CLOGF(ChunkStorage.CompressedAnimData != nullptr, LogAnimation, Fatal, "Existing compressed data for streaming animation chunk.");
			UE_CLOGF(ChunkStorage.IORequest, LogAnimation, Fatal, "Streaming animation chunk already has IORequest.");
			
			int64 ChunkSize = Chunk.BulkData.GetBulkDataSize();
			ChunkStorage.RequestStart = FPlatformTime::Seconds();
			UE_LOGF(LogAnimation, Warning, "Anim Streaming Request Started %ls Chunk:%i At:%.3f\n", *StreamableAnim->GetName(), ChunkIndex, ChunkStorage.RequestStart);
			...
			{
				UE_LOGF(LogAnimation, Error, "Animation streaming read request failed.");
			}
		}
	}
}
...
void FAnimationStreamingManager::OnAsyncFileCallback(FStreamingAnimationData* StreamingAnimData, int32 ChunkIndex, int64 ReadSize, IBulkDataIORequest* ReadRequest, bool bWasCancelled)
{
	...
	if (Mem)
	{
		...
		{
			UE_LOGF(LogAnimation, Warning, "Streaming anim loading slower than needed ChunkIndex %i - Length: %.3f Load Duration:%.3f Anim:%ls\n", ChunkIndex, SequenceLength, RequestDuration, *StreamingAnimData->StreamableAnim->GetName());
		}
	...
	else
	{
		...
		UE_LOGF(LogAnimation, Warning, "Streaming anim failed to load chunk: %i Load Duration:%.3f, Anim:%ls WasCancelled: %ls\n", ChunkIndex, RequestDuration, *StreamingAnimData->StreamableAnim->GetName(), WasCancelledText);
		...
		if (AnimData->LoadedChunkIndices.Contains(ChunkIndex))
		{
			if (const FLoadedAnimationChunk* Chunk = Algo::FindBy(AnimData->LoadedChunks, ChunkIndex, &FLoadedAnimationChunk::Index))
			{
				if (Chunk->CompressedAnimData == nullptr)
				{
					...
					UE_LOGF(LogAnimation, Warning, "No Animation Data for loaded chunk: %i, Anim: %ls Request timer : %.3f", ChunkIndex, *Anim->GetFullName(), RequestTimer);
				}
				return Chunk->CompressedAnimData;
			}
			else
			{
				UE_LOGF(LogAnimation, Warning, "Unable to find requested Chunk: %i, Anim: %ls - Is in LoadedChunkIndices however", ChunkIndex, *Anim->GetFullName());
			}
		}
		else
		{
			UE_LOGF(LogAnimation, Warning, "Requested Previously Unknown Chunk: %i, Anim: %ls",  ChunkIndex, *Anim->GetFullName());
		}
	}
