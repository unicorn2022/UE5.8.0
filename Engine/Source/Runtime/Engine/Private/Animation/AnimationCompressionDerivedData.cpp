		...
		Owner.LaunchTask(TEXT("AnimationSequenceSerialize"), [this, Name = Response.Name, Key = Response.Key, Value = MoveTemp(Response.Value)]
		{
...

			{
				...
				if (UAnimSequence* AnimSequence = WeakAnimSequence.Get())
				{
					...
					{
						UE_LOGF(LogAnimationCompression, Warning, "Fetched invalid compressed animation data for %ls", *CompressibleAnimPtr->FullName);
					...
					{
						UE_LOGF(LogAnimationCompression, Verbose, "Fetched compressed animation data for %ls", *CompressibleAnimPtr->FullName);
...
{	
	...
	UE_LOGF(LogAnimationCompression, Display, "Building compressed animation data for %ls (Required Memory Estimate: %.2f MB)",
	...
	if (DataToCompress.FetchData(TargetPlatform))
	{
		if (Owner.IsCanceled())
		{
			return false;
		...
		if (!Owner.IsCanceled() && AnimSequence)
		{
			UE_COMPRESSED_DATA_WRITE_SCOPE(AnimSequence);

			// All compression and OutData mutations happen under the write lock
			// to prevent readers from seeing partial/intermediate state
			...
			if (bBoneCompressionOk && bCurveCompressionOk && !Owner.IsCanceled())
			{
			...
			else if (!bBoneCompressionOk || !bCurveCompressionOk)
			{
				UE_LOGF(LogAnimationCompression, Error, "Failed to generate compressed animation data for %ls with bone compression scheme %ls [%i] and curve compression scheme %ls [%i] for target platform %ls", *CompressibleAnimPtr->FullName, *BoneCompressionName, bBoneCompressionOk ? 1 : 0, *CurveCompressionName, bCurveCompressionOk ? 1 : 0, *TargetPlatform->DisplayName().ToString());
			}
		}
		else if (!AnimSequence)
		{
			UE_LOGF(LogAnimationCompression, Warning, "Failed to apply compressed animation data for %ls as it has previously been garbage collected", *CompressibleAnimPtr->FullName);
		}
	}
	else
	{
		UE_LOGF(LogAnimationCompression, Log, "Failed to fetch FCompressibleAnimData data for %ls", *CompressibleAnimPtr->FullName);
	}
...
void FAnimationSequenceAsyncCacheTask::LaunchCompressionTask(const UE::FSharedString& Name, const UE::DerivedData::FCacheKey& Key)
{
	Owner.LaunchTask(TEXT("AnimationSequenceCompression"), [this, Name, Key]
	{
		...
		if (const UAnimSequence* AnimSequence = WeakAnimSequence.Get())
		{
			if (!CompressedData->IsBoneDataValid(AnimSequence, true) || !CompressedData->IsCurveDataValid(AnimSequence, true))
			{
				CompressedData->Reset();
				UE_LOGF(LogAnimationCompression, Warning, "Generated invalid compressed animation data for %ls", *CompressibleAnimPtr->FullName);
			}
			else
			{
				CompressedData->bHasValidCompressedData = true;
				...
				UE_LOGF(LogAnimationCompression, Display, "Storing compressed animation data for %ls, at %ls/%ls", *Name, *FString(Key.Bucket.ToString()), *LexToString(Key.Hash));
...
void FAnimationSequenceAsyncCacheTask::CalculateRequiredMemoryEstimate()
{
	...
	if (const UAnimSequence* AnimSequence = WeakAnimSequence.Get())
	{
			...
			for (TObjectPtr<UAnimBoneCompressionCodec> Codec : BoneCompressionSettings->Codecs)
			{
				if (Codec)
				{
					...
					if (PeakMemoryEstimate < 0)
					{
						...
						UE_LOGF(LogAnimationCompression, Warning, "Got invalid memory usage estimate from codec %ls for %ls. This can negatively affect the time compression takes.", *Codec->GetFullName(), *AnimSequence->GetFullName());
						RequiredMemory = -1;
						break;
					}
		...
		{
			if (CurveCompressionSettings->Codec)
			{
				const int64 PeakMemoryEstimate = CurveCompressionSettings->Codec->EstimateCompressionMemoryUsage(*AnimSequence);
				if (PeakMemoryEstimate < 0)
				{
					// Assume the worst and default to the default behavior when no estimate is given.
					UE_LOGF(LogAnimationCompression, Warning, "Got invalid memory usage estimate from codec %ls for %ls. This can negatively affect the time compression takes.", *CurveCompressionSettings->Codec->GetFullName(), *AnimSequence->GetFullName());
					RequiredMemory = -1;
				}
				else
				{
