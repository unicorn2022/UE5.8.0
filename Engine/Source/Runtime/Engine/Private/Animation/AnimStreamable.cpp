...
void UAnimStreamable::Serialize(FArchive& Ar)
{
	...
	{
		UE_LOGF(LogAnimation, Fatal, "This platform requires cooked packages, and animation data was not cooked into %ls.", *GetFullName());
	}
	...
	if(ensureMsgf(ChunkIndex != INDEX_NONE, TEXT("Could not get valid chunk with Time %.2f for Streaming Anim %s"), ExtractionContext.CurrentTime, *GetFullName()))
	{
...
#else
			UE_LOGF(LogAnimation, Warning, "Failed to get streamed compressed data Time: %.2f, ChunkIndex:%i, Anim: %ls", ExtractionContext.CurrentTime, 0, *GetFullName());
			return;
#endif
		}
		...
		if (!CompressedData)
		{
			...
			while (!CompressedData)
			{
				...
				{
					//Cannot get fallback
					UE_LOGF(LogAnimation, Warning, "Failed to get ANY streamed compressed data Time: %.2f, ChunkIndex:%i, Anim: %ls", ExtractionContext.CurrentTime, ChunkIndex, *GetFullName());
					return;
				}
			...
			UE_LOGF(LogAnimation, Warning, "Failed to get streamed compressed data Time: %.2f, ChunkIndex:%i - Using Chunk %i Anim: %ls", ExtractionContext.CurrentTime, ChunkIndex, FallbackChunkIndex, *GetFullName());
#endif
		}
...
void UAnimStreamable::RequestCompressedData(const ITargetPlatform* Platform)
{
	...
	for (uint32 ChunkIndex = 0; ChunkIndex < NumChunks; ++ChunkIndex)
	{
		...
		RequestCompressedDataForChunk(ChunkDDCKey, PlatformData.Chunks[ChunkIndex], ChunkIndex, FrameStart, FrameEnd, Platform);
	}
...
float UAnimStreamable::GetChunkSizeSeconds(const ITargetPlatform* Platform) const
{
	...
	//UE_LOGF(LogAnimation, Log, "Anim Chunk Size for platform %ls : %.2f\n", *Platform->DisplayName().ToString(), CVarPlatformChunkSizeSeconds);
...
void UAnimStreamable::RequestCompressedDataForChunk(const FString& ChunkDDCKey, FAnimStreamableChunk& Chunk, const int32 ChunkIndex, const uint32 FrameStart, const uint32 FrameEnd, const ITargetPlatform* Platform)
{
	...
	TArray<uint8> OutData;
	{
		...
		FDerivedDataAnimationCompression* AnimCompressor = new FDerivedDataAnimationCompression(TEXT("StreamAnim"), ChunkDDCKey);
