...
UE_TRACE_CHANNEL_DEFINE(AnimationChannel, "Animation tick records, skeletal mesh poses, and animation graph evaluation.");

UE_TRACE_EVENT_BEGIN(Animation, TickRecord3)
	...
	UE_TRACE_EVENT_FIELD(uint32, SyncGroupNameId)
	UE_TRACE_EVENT_FIELD(bool, IsSyncGroupLeader)
	UE_TRACE_EVENT_FIELD(float, SyncLeaderScore)
	UE_TRACE_EVENT_FIELD(float, TimeToPrevMarker)
	UE_TRACE_EVENT_FIELD(float, TimeToNextMarker)
	UE_TRACE_EVENT_FIELD(int32, PrevMarkerIndex)
	UE_TRACE_EVENT_FIELD(int32, NextMarkerIndex)
...
void FAnimTrace::OutputAnimTickRecord(const FAnimationBaseContext& InContext, const FAnimTickRecord& InTickRecord)
{
	...
	{
		// Use the source proxy for attribution if the tick record originated from a different proxy (e.g. a linked anim graph), 
		// while keeping the context proxy for sync group lookup since tick records always live in the main proxy's sync regardless of which linked graph created them.
		FAnimInstanceProxy* AttributionProxy = (InTickRecord.SourceProxy && InTickRecord.SourceProxy != InContext.AnimInstanceProxy) ? InTickRecord.SourceProxy : InContext.AnimInstanceProxy;
		UObject* AnimInstance = AttributionProxy->GetAnimInstanceObject();
		...
		// Figure out which sync group is this tick record located, if any at all.
		const TMap<FName, FAnimGroupInstance>& GroupedTickRecords = InContext.AnimInstanceProxy->GetSyncGroupMapRead();
		
		static FName NoSyncGroupName(EName::None);
		FName SyncGroupName = NoSyncGroupName;
		bool IsSyncGroupLeader = false;
		
		for (const TTuple<FName, FAnimGroupInstance>& TickRecordGroup : GroupedTickRecords)
		{
			int32 GroupIdx = 0;
			for (const FAnimTickRecord& TickRecord : TickRecordGroup.Value.ActivePlayers)
			{
				if (&TickRecord == &InTickRecord)
				{
					SyncGroupName = TickRecordGroup.Key;
					IsSyncGroupLeader = GroupIdx == TickRecordGroup.Value.GroupLeaderIndex;
				}
				++GroupIdx;
			}
		}

		uint32 SyncGroupNameId = FAnimTrace::OutputName(SyncGroupName);
		
		UE_TRACE_LOG(Animation, TickRecord3, AnimationChannel)
			<< TickRecord3.Cycle(FPlatformTime::Cycles64())
			<< TickRecord3.RecordingTime(FObjectTrace::GetWorldElapsedTime(AnimInstance->GetWorld()))
			<< TickRecord3.AnimInstanceId(FObjectTrace::GetObjectId(AnimInstance))
			<< TickRecord3.AssetId(FObjectTrace::GetObjectId(InTickRecord.SourceAsset))
			<< TickRecord3.NodeId(InContext.GetCurrentNodeId())
			<< TickRecord3.BlendWeight(InTickRecord.EffectiveBlendWeight)
			<< TickRecord3.PlaybackTime(PlaybackTime)
			<< TickRecord3.RootMotionWeight(InTickRecord.RootMotionWeightModifier)
			<< TickRecord3.PlayRate(InTickRecord.PlayRateMultiplier)
			<< TickRecord3.BlendSpacePositionX(BlendSpacePositionX)
			<< TickRecord3.BlendSpacePositionY(BlendSpacePositionY)
			<< TickRecord3.BlendSpaceFilteredPositionX(BlendSpaceFilteredPositionX)
			<< TickRecord3.BlendSpaceFilteredPositionY(BlendSpaceFilteredPositionY)
			<< TickRecord3.FrameCounter(FObjectTrace::GetObjectWorldTickCounter(AnimInstance))
			<< TickRecord3.Looping(InTickRecord.bLooping)
			<< TickRecord3.IsBlendSpace(bIsBlendSpace)
			<< TickRecord3.SyncGroupNameId(SyncGroupNameId)
			<< TickRecord3.IsSyncGroupLeader(IsSyncGroupLeader)
			<< TickRecord3.SyncLeaderScore(InTickRecord.LeaderScore)
			<< TickRecord3.PrevMarkerIndex(InTickRecord.MarkerTickRecord ? InTickRecord.MarkerTickRecord->PreviousMarker.MarkerIndex : -1)
			<< TickRecord3.NextMarkerIndex(InTickRecord.MarkerTickRecord ? InTickRecord.MarkerTickRecord->NextMarker.MarkerIndex : -1)
			<< TickRecord3.TimeToPrevMarker(InTickRecord.MarkerTickRecord ? InTickRecord.MarkerTickRecord->PreviousMarker.TimeToMarker : 0)
			<< TickRecord3.TimeToNextMarker(InTickRecord.MarkerTickRecord ? InTickRecord.MarkerTickRecord->NextMarker.TimeToMarker : 0);
	}
}
