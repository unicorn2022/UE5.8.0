...
void FAnimNode_AssetPlayerBase::Initialize_AnyThread(const FAnimationInitializeContext& Context)
{
	...
	{
		AssetPlayerInstanceID = HashCombine(GetTypeHash(AnimInstance->AnimInstanceID), GetTypeHash(Context.GetCurrentNodeId()));
		bHasValidAssetPlayerInstanceID = true;
	}
}
...
void FAnimNode_AssetPlayerBase::CreateTickRecordForNode(const FAnimationUpdateContext& Context, UAnimSequenceBase* Sequence, bool bLooping, float PlayRate, bool bIsEvaluator)
{
	...
	if (bHasValidAssetPlayerInstanceID)
	{
		TickRecord.MakeContextData<UE::Anim::FAnimNotifyAssetPlayerInstanceContext>(AssetPlayerInstanceID);
	}
