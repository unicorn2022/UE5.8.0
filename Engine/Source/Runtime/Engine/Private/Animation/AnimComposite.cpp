...
void UAnimComposite::PostLoad()
{
	...
	for (const FAnimSegment& AnimSegment : AnimationTrack.AnimSegments)
	{
		...
		{
			UE_LOGF(LogAnimation, Warning, "AnimComposite (%ls) contains a Segment for which the playable length %f is out-of-sync with the represented AnimationSequence its length %f (%ls). Please up-date the segment and resave.", *GetFullName(), (AnimSegment.AnimEndTime - AnimSegment.AnimStartTime), AnimSegment.GetAnimReference()->GetPlayLength(), *AnimSegment.GetAnimReference()->GetFullName());
		}
	}
#endif
}
			...
			else
			{
				...
				{					
					...
					{						
						...
						{
							UE_LOGF(LogAnimation, Warning, "Frame rate of animation %ls (%ls) is incompatible with other animations in Animation Composite %ls - underlying frame-rate will be set to %ls:%ls", *Base->GetName(), *BaseFrameRate.ToPrettyText().ToString(), *GetName(), *Super::GetSamplingFrameRate().ToPrettyText().ToString(), *AssetString);
						}
				...
				{
					UE_LOGF(LogAnimation, Warning, "Invalid frame rate %ls for %ls in %ls", *BaseFrameRate.ToPrettyText().ToString(), *Base->GetName(), *GetName());
				}
			}			
		}	
	}
