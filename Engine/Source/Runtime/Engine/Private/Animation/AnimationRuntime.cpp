...
void FAnimationRuntime::BlendTwoPosesTogetherInPlace(FAnimationPoseData& InOutPoseOneData, const FAnimationPoseData& SourcePoseTwoData, const float WeightOfPoseOne)
{
	FCompactPose& InOutPose = InOutPoseOneData.GetPose();
	FBlendedCurve& InOutCurve = InOutPoseOneData.GetCurve();
	UE::Anim::FStackAttributeContainer& InOutAttributes = InOutPoseOneData.GetAttributes();

	const float WeightOfPoseTwo = 1.f - WeightOfPoseOne;

	// Scale first pose in-place, then accumulate second pose
	BlendPose<ETransformBlendMode::Overwrite>(InOutPose, InOutPose, WeightOfPoseOne);
	BlendPose<ETransformBlendMode::Accumulate>(SourcePoseTwoData.GetPose(), InOutPose, WeightOfPoseTwo);

	// Ensure that all of the resulting rotations are normalized
	InOutPose.NormalizeRotations();

	// Use 3 args Lerp over 2 args in-place LerpTo as this is more performant and the memory overhead is negligible
	FBlendedCurve TempCurve;
	TempCurve.Lerp(InOutCurve, SourcePoseTwoData.GetCurve(), WeightOfPoseTwo);
	InOutCurve = MoveTemp(TempCurve);

	// Use a temporary for attributes to avoid aliasing
	UE::Anim::FStackAttributeContainer TempAttributes;
	UE::Anim::Attributes::BlendAttributes({ InOutPoseOneData.GetAttributes(), SourcePoseTwoData.GetAttributes() }, { WeightOfPoseOne, WeightOfPoseTwo }, { 0, 1 }, TempAttributes);
	InOutAttributes = MoveTemp(TempAttributes);
}
...
void FAnimationRuntime::ExcludeBonesWithNoParents(const TArray<int32>& BoneIndices, const FReferenceSkeleton& RefSkeleton, TArray<int32>& FilteredRequiredBones)
{
	...
	for (int32 Index=0; Index<BoneIndices.Num(); Index++)
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogAnimation, Warning, "ExcludeBonesWithNoParents: Filtering out bone (%ls) since parent (%ls) is missing", 
...
void FAnimationRuntime::ConvertCSTransformToBoneSpace(const FTransform& ComponentTransform, FCSPose<FCompactPose>& MeshBases, FTransform& InOutCSBoneTM, FCompactPoseBoneIndex BoneIndex, EBoneControlSpace Space)
{
	switch( Space )
	{
		...
		default:
			UE_LOGF(LogAnimation, Warning, "ConvertCSTransformToBoneSpace: Unknown BoneSpace %d", (int32)Space);
			break;
	}
}
...
void FAnimationRuntime::ConvertBoneSpaceTransformToCS(const FTransform& ComponentTransform, FCSPose<FCompactPose>& MeshBases, FTransform& InOutBoneSpaceTM, FCompactPoseBoneIndex BoneIndex, EBoneControlSpace Space)
{
	switch( Space )
	{
		...
		default:
			UE_LOGF(LogAnimation, Warning, "ConvertBoneSpaceTransformToCS: Unknown BoneSpace %d", (int32)Space);
			break;
	}
}
...
{
	FBoneContainerRetargetBoneTransformContext Context(RequiredBones);
	RetargetBoneTransform(SourceSkeleton, SourceName, RetargetTransforms, BoneTransform, SkeletonBoneIndex, BoneIndex, &Context, bIsBakedAdditive);
}

void FAnimationRuntime::RetargetBoneTransform(const USkeleton* SourceSkeleton, const FName& SourceName, const TArray<FTransform>& RetargetTransforms, FTransform& BoneTransform, const int32 SkeletonBoneIndex, const FCompactPoseBoneIndex& BoneIndex, const FAnimationRuntimeRetargetBoneTransformContext* InContext, const bool bIsBakedAdditive)
{
	check(InContext);
	...
	if (SourceSkeleton)
	{
		...
		const USkeleton* TargetSkeleton = InContext->GetSkeleton(bEvenIfUnreachable);
		...
		const int32 TargetSkeletonBoneIndex = InContext->GetSkeletonBoneIndex(BoneIndex.GetInt());
		...
		const EBoneTranslationRetargetingMode::Type RetargetMode = FAnimationRuntime::GetBoneTranslationRetargetingMode(
			...
			SourceSkeleton,
			TargetSkeleton,
			InContext->IsRetargetingDisabled());
		...
		switch (RetargetMode)
		{
			case EBoneTranslationRetargetingMode::AnimationScaled:
			{
				...
				if (RetargetTransforms.IsValidIndex(SourceSkeletonBoneIndex))
				{
					...
					{
						const float TargetTranslationLength = InContext->GetRefPoseTransform(BoneIndex.GetInt()).GetTranslation().Size();
			...
			{
				BoneTransform.SetTranslation(bIsBakedAdditive ? FVector::ZeroVector : InContext->GetRefPoseTransform(BoneIndex.GetInt()).GetTranslation());
				break;
			}
			...
			case EBoneTranslationRetargetingMode::AnimationRelative:
			{
				...
				if (!bIsBakedAdditive)
				{
					...
					const FTransform& RefPoseTransform = InContext->GetRefPoseTransform(BoneIndex.GetInt());
					...
					else
					{
						...
						{
							UE_LOGF(LogAnimation, Error, "RetargetBoneTransform: Trying to retarget an additive bone (%ls) from SourceName (%ls) with Source Skeleton (%ls) and Target Skeleton (%ls) that does not exist at RetargetTransforms Array. Please check the animation and its Base Pose Aimation and re-save them.",
			...
			case EBoneTranslationRetargetingMode::OrientAndScale:
			{
				if (!bIsBakedAdditive)
				{
					const FRetargetSourceCachedData& RetargetSourceCachedData = InContext->GetRetargetSourceCachedData(SourceName, SkeletonRemapping, RetargetTransforms);
					...
					if ((OrientAndScaleDataArray.Num() > 0) && (CompactPoseIndexToOrientAndScaleIndex.Num() == InContext->GetNumBonesInContext()))
					{
