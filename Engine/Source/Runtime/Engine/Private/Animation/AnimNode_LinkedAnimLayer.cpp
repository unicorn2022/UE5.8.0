...
void FAnimNode_LinkedAnimLayer::HandleObjectsReinstanced_Impl(UObject* InSourceObject, UObject* InTargetObject, const TMap<UObject*, UObject*>& OldToNewInstanceMap)
{
	...
	if(UseLegacyAnimInstanceReinstancingBehavior == nullptr || !UseLegacyAnimInstanceReinstancingBehavior->GetBool())
	{
		...
		// Skip if the owning mesh component is not registered. During BP reinstancing,
		// linked anim layer instances can survive on trashed/orphaned components (the old
		// pawn was destroyed but the linked AnimInstance is still subscribed to
		// OnObjectsReinstanced). The component's bone data was cleared in OnUnregister
		// but the mesh reference remains, so the existing mesh-only check is insufficient.
