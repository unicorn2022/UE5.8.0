	...
	if (InSequenceBase->HasCurveData(InName, false))
	{
		if (const UAnimSequence* InSequence = Cast<UAnimSequence>(InSequenceBase))
		{
			if (InSequence->IsCurveCompressedDataValid())
			{
				...
				{
					UE_LOGF(LogAnimation, Warning, "Trying to use FAnimCurveBufferAccess on an animation that is not compressed with UniformIndexable, this is not supported and will not work in cooked builds. Animation: %ls", *InSequence->GetFullName());
				}
			}
		}
