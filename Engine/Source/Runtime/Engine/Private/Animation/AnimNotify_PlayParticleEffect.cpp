...
UParticleSystemComponent* UAnimNotify_PlayParticleEffect::SpawnParticleSystem(class USkeletalMeshComponent* MeshComp, class UAnimSequenceBase* Animation)
{
	...
	if (PSTemplate)
	{
		...
		{
			UE_LOGF(LogParticles, Warning, "Particle Notify: Anim '%ls' tried to spawn infinitely looping particle system '%ls'. Spawning suppressed.", *GetNameSafe(Animation), *GetNameSafe(PSTemplate));
			return ReturnComp;
		}
	...
	{
		UE_LOGF(LogParticles, Warning, "Particle Notify: Particle system is null for particle notify '%ls' in anim: '%ls'", *GetNotifyName(), *GetPathNameSafe(Animation));
	}
