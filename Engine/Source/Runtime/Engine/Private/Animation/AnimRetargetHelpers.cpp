...
ERetargetSourceAssetStatus CheckRetargetSourceAssetDataImpl(const AssetType* InAsset)
{
	...
	if (InAsset->RetargetSourceAssetReferencePose.Num() > 0)
	{
		if (AssetDataStatus != AssetRegistry::EExists::Unknown)
		{
			...
			{
				UE_LOGF(LogAnimation, Warning, "Asset [%ls] references a missing Retarget Source Asset [%ls]. Retarget Reference Pose has [%d] elements. Please, add a correct retarget source asset and resave."
					, *InAsset->GetFullName()
					, *(RetargetSourceAsset.GetLongPackageName() + FString("/") + RetargetSourceAsset.GetAssetName())
...
void CheckRetargetSourceAssetData(bool bFixAssets, bool bWantsFullScan, const TArray<FString>& IncludedPaths, const TArray<FString>& ExcludedPaths)
{
	...
	if (IncludedPaths.Num() > 0)
	{
		...
		{
			UE_LOGF(LogAnimation, Log, "Check Retarget Source Assets scan folder [%ls].", *IncludedPath);
	...
	{
		UE_LOGF(LogAnimation, Warning, "Check Retarget Source Assets will scan all folders (this might take some time and require a lot of memory).");
	}
	...
	UE_LOGF(LogAnimation, Log, "Check Retarget Source Assets found [%d] assets.", Assets.Num());
	...
	if (Assets.Num() > 0 && ExcludedPaths.Num() > 0)
	{
		...
		{
			UE_LOGF(LogAnimation, Log, "Check Retarget Source Assets Excluded folder : [%ls].", *ExcludedPath);
	...
	UE_LOGF(LogAnimation, Log, "Check Retarget Source Assets after filtering exclusions : [%d] assets.", Assets.Num());

	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	const FName SearchableName = GET_MEMBER_NAME_CHECKED(UAnimSequence, RetargetSourceAsset);
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	const FString AssetNone("None");
	...
	for (int32 Idx = 0; Idx < NumAssets; Idx++)
	{
		...
		Feedback.EnterProgressFrame(1, 
			FText::Format(LOCTEXT("ScanningRetargetSourceAssetsProgress", "Assets Left [{0}] - {1}"), 
				NumAssets - Idx,
				FText::FromString((AssetData.PackagePath.ToString() + FString("/") + AssetData.AssetName.ToString()))));
...
static FAutoConsoleCommand CheckRetargetSourceAssetDataCmd(
	...
	FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& Args)
		{
			static const FString IncludedPathsSwitch = "Include=";
			static const FString ExcludedPathsSwitch = "Exclude=";
