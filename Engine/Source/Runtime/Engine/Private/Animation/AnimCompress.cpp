...
FCompressionMemorySummary::~FCompressionMemorySummary()
{
	if (bEnabled && bUsed)
	{
		...
		UE_LOGF(LogAnimationCompression, Display, "Top 10 Worst Bone Errors:");
		WorstBoneError.LogErrorStat();
		UE_LOGF(LogAnimationCompression, Display, "Top 10 Worst Average Animation Errors:");
	...
	bool IncludeKeyTable)
{
	...
	else
	{
		...
		if (NumTracks == 0)
		{
			UE_LOGF(LogAnimationCompression, Warning, "When compressing %ls: no key-reduced data", *CompressibleAnimData.Name);
		}
		...
		for (int32 TrackIndex = 0; TrackIndex < NumTracks; ++TrackIndex)
		{
			...
			if (NumKeysTrans == 0)
			{
				UE_LOGF(LogAnimationCompression, Warning, "When compressing %ls track %i: no translation keys", *CompressibleAnimData.Name, TrackIndex);
			}
			...
			{
				UE_LOGF(LogAnimationCompression, Warning, "When compressing %ls track %i: no rotation keys", *CompressibleAnimData.Name, TrackIndex);
			}
			...
			if (bHasScale)
			{
				...
				{
					UE_LOGF(LogAnimationCompression, Warning, "When compressing %ls track %i: no Scale keys", *CompressibleAnimData.Name, TrackIndex);
				}
