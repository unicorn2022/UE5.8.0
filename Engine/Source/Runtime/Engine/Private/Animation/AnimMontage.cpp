...
{
	static FName TimeStretchCurveName("MontageTimeStretchCurve");
}
...
namespace MontageCVars
{
	...
	static bool bUseForcedFromPositionAsSubStepOrigin = true;
	static FAutoConsoleVariableRef CVarUseForcedFromPositionAsSubStepOrigin(
		TEXT("a.Montage.UseForcedFromPositionAsSubStepOrigin"),
		bUseForcedFromPositionAsSubStepOrigin,
		TEXT(" When true (default), if a ForcedNextFromPosition is set during Montage Advance, use that forced position as the sub-step start point rather than the stale cached Position."));
	
	static bool bUseForcedSequencerMontageLoopBehavior = false;
	static FAutoConsoleVariableRef CVarUseForcedSequencerMontageLoopBehavior(
		TEXT("a.Montage.UseForcedSequencerMontageLoopBehavior"),
		bUseForcedSequencerMontageLoopBehavior,
		TEXT(" When true, if a montage is updated from Sequencer by utilising forced positions also handle wrapping correctly when the Sequencer loops"));
...
int32 UAnimMontage::AddAnimCompositeSection(FName InSectionName, float StartTime)
{
	...
	{
		UE_LOGF(LogAnimMontage, Warning, "AnimCompositeSection : %ls(%ls) already exists. Choose different name.", 
		...
		for (FSlotAnimationTrack& Slot : SlotAnimTracks)
		{
			for (FAnimSegment& Segment : Slot.AnimTrack.AnimSegments)
			{
				if (UAnimSequenceBase* AnimReference = Segment.GetAnimReference())
				{
					...
					{
						UE_LOGF(LogAnimation, Warning, "[Montage %ls] has RootMotionEnabled, but [AnimationSequence %ls] has not been saved after setting the flag. Please open the Montage and the AnimationSequence and save the AnimationSequence as this will generate non determistic cooks.", *GetFullName(), *AnimReference->GetFullName());
					}
#endif // WITH_EDITOR
...
FTransform UAnimMontage::ExtractRootMotionFromTrackRange(float StartTrackPosition, float EndTrackPosition, const FAnimExtractContext& Context) const
{
	...
	UE_LOGF(LogRootMotion, Log,  "\tUAnimMontage::ExtractRootMotionForTrackRange RootMotionTransform: Translation: %ls, Rotation: %ls"
	...
	if (NumAnimTracks > 1)
	{
		...
		if (MySkeleton)
		{
			...
			for (int32 TrackIndex = 1; TrackIndex < NumAnimTracks; TrackIndex++)
			{
				...
				{
					UE_LOGF(LogAnimMontage, Warning, "Montage '%ls' not properly setup. Slot named '%ls' is already used in this Montage. All slots must be unique",
				...
				{
					UE_LOGF(LogAnimMontage, Warning, "Montage '%ls' not properly setup. Slot's group '%ls' is different than the Montage's group '%ls'. All slots must belong to the same group.",
	...
	if (Context.IsLeader())
	{
		...
		if (MoveDelta != 0.f)
		{
			if (Instance.bCanUseMarkerSync && Instance.MarkerTickRecord && Context.CanUseMarkerPosition())
			{
				...
				UE_LOGF(LogAnimMarkerSync, Log, "Montage Leading SyncGroup: %ls(%ls) Start [%ls], End [%ls]", *GetNameSafe(this), *SyncGroup.ToString(), *MarkerTickContext.GetMarkerSyncStartPosition().ToString(), *MarkerTickContext.GetMarkerSyncEndPosition().ToString());
			}
		}
...
void UAnimMontage::CollectMarkers()
{
	...
	if (SyncGroup != NAME_None)
	{
		...
		{
			UE_LOGF(LogAnimMontage, Warning, "Montage's sync slot track index is invalid. Make sure to use a valid slot track index, otherwise this Montage will use old sync markers or not use marker-based syncing at all.")
		}
	}
}
...
void FAnimMontageInstance::Stop(const FMontageBlendSettings& InBlendOutSettings, bool bInterrupt)
{
	if (Montage)
	{
		UE_LOGF(LogAnimMontage, Verbose, "Montage.Stop Before: AnimMontage: %ls,  (DesiredWeight:%0.2f, Weight:%0.2f)",
	...
	if (Montage != nullptr)
	{
		UE_LOGF(LogAnimMontage, Verbose, "Montage.Stop After: AnimMontage: %ls,  (DesiredWeight:%0.2f, Weight:%0.2f)",
			*Montage->GetName(), GetDesiredWeight(), GetWeight());
	}
}
...
void FAnimMontageInstance::Terminate()
{
	...
	if (AnimInstance.IsValid())
	{
		...
		FAnimMontageInstance* SelfPtr = this;
		const uint32 LastMontageFlushFrame = AnimInstanceLocal->GetLastMontageFlushFrame();
		...
		// Safety check if the anim instance is still valid after triggering montage end events 
		if (!AnimInstanceLocal.IsValid())
		{
			return;
		}

		// UninitializeAnimation() empties the MontagesInstances array thus we need to exit early to not use freed memory.
		if (!AnimInstanceLocal->IsInitialized() && MontageCVars::bEarlyOutMontageWhenUninitialized)
		{
			return;
		}

		// Prone to have being free self from trigger montage events.
		if (LastMontageFlushFrame != AnimInstanceLocal->GetLastMontageFlushFrame() && AnimInstanceLocal->MontageInstances.Find(SelfPtr) == INDEX_NONE && MontageCVars::bEarlyOutMontageWhenUninitialized)
		{
			return;
		}
	...
	UE_LOGF(LogAnimMontage, Verbose, "Terminating: AnimMontage: %ls", *GetNameSafe(OldMontage));
}
...
bool FAnimMontageInstance::JumpToSectionName(FName const & SectionName, bool bEndOfSection)
{
	...
	UE_LOGF(LogAnimMontage, Warning, "JumpToSectionName %ls bEndOfSection: %d failed for Montage %ls",
...
bool FAnimMontageInstance::SetNextSectionID(int32 const & SectionID, int32 const & NewNextSectionID)
{
	...
	UE_LOGF(LogAnimMontage, Warning, "SetNextSectionName %ls to %ls failed for Montage %ls",
...
void FAnimMontageInstance::OnMontagePositionChanged(FName const & ToSectionName) 
{
	...
	{
		UE_LOGF(LogAnimMontage, Warning, "Changing section on Montage (%ls) to '%ls' during blend out. This can cause incorrect visuals!",
...
void FAnimMontageInstance::UpdateWeight(float DeltaTime)
{
	if ( IsValid() )
	{
		...
		UE_LOGF(LogAnimMontage, Verbose, "UpdateWeight: AnimMontage: %ls,  (DesiredWeight:%0.2f, Weight:%0.2f, PreviousWeight: %0.2f)",
			*Montage->GetName(), GetDesiredWeight(), GetWeight(), PreviousWeight);
		UE_LOGF(LogAnimMontage, Verbose, "Blending Info: BlendOption : %d, AlphaLerp : %0.2f, BlendTime: %0.2f",
...
EMontageSubStepResult FMontageSubStepper::Advance(float& InOut_P_Original, const FBranchingPointMarker** OutBranchingPointMarkerPtr)
{
	...
	if (MontageInstance->ForcedNextToPosition.IsSet())
	{
		...
		if (MontageCVars::bUseForcedSequencerMontageLoopBehavior 
			&& MontageInstance->ForcedLoopBoundaryPosition.IsSet() && MontageInstance->ForcedLoopResumePosition.IsSet())
		{
			const float BoundaryPosition = MontageInstance->ForcedLoopBoundaryPosition.GetValue();
			const float ResumePosition = MontageInstance->ForcedLoopResumePosition.GetValue();
			const float TotalDelta = (BoundaryPosition - InOut_P_Original) + (NewPosition - ResumePosition);
			
			DeltaMove = BoundaryPosition - InOut_P_Original;
			PlayRate = TotalDelta / TimeRemaining;
			bPlayingForward = (TotalDelta >= 0.f);
		}
		else
		{
	...
	else if (MontageInstance->OnComputeDeltaFrameTime.IsBound())
	{
		// Note that the compute delta frame time ignores play rate, at least for now.
		DeltaMove = MontageInstance->OnComputeDeltaFrameTime.Execute(MontageInstance->GetInstanceID(), InOut_P_Original, TimeRemaining);
		if (FMath::IsNearlyZero(DeltaMove))
		{
			return EMontageSubStepResult::NotMoved;
		}

		// we only set playrate for a sign check later, otherwise unused.
		PlayRate = DeltaMove / TimeRemaining;
		bPlayingForward = (DeltaMove >= 0.f); // *could* go backwards a bit. since music clock is not purely coupled with gameclock. 
		TimeStretchMarkerIndex = INDEX_NONE;
	}
	else
	{
		...
		if (bPlaying)
		{
			...
			*/
			...
			while (bPlaying && MontageSubStepper.HasTimeRemaining() && (++NumIterations < MaxIterations))
			{
				...
				const float PreviousSubStepPosition = (MontageCVars::bUseForcedFromPositionAsSubStepOrigin && ForcedNextFromPosition.IsSet())
					? ForcedNextFromPosition.GetValue()
					: Position;
				...
				if (bHaveMoved)
				{
					if (bDidUseMarkerSyncThisTick)
					{
						Montage->MarkerData.CollectMarkersInRange(PreviousSubStepPosition, Position, MarkersPassedThisTick, SubStepDeltaMove, nullptr);
					}
					...
					if (bExtractRootMotion && AnimInstance.IsValid() && !IsRootMotionDisabled())
					{
						...
						UE_LOGF(LogRootMotion, Log, "\tFAnimMontageInstance::Advance ExtractedRootMotion: %ls, AccumulatedRootMotion: %ls, bBlendRootMotion: %d"
				...
				// After the first loop segment has been processed and events have been handled,
				// clear out the loop data and update the position for the next iteration 
				if (MontageCVars::bUseForcedSequencerMontageLoopBehavior 
					&& ForcedLoopBoundaryPosition.IsSet() && ForcedLoopResumePosition.IsSet())
				{
					Position = ForcedLoopResumePosition.GetValue();
					ForcedNextFromPosition = ForcedLoopResumePosition.GetValue();
					ForcedLoopBoundaryPosition.Reset();
					ForcedLoopResumePosition.Reset();
				}
				...
				{
					// If the montage has not advanced because ForcedNextFromPosition and ForcedNextToPosition are set to the same value, we need to keep any active notify state alive
					// (This can happen for example in frame-locked Sequencer setups with unstable frame times)
					if (ForcedNextFromPosition.IsSet() && ForcedNextToPosition.IsSet() 
						&& ForcedNextFromPosition.GetValue() == ForcedNextToPosition.GetValue())
					{
						const float ForcedPosition = ForcedNextFromPosition.GetValue();
						HandleEvents(ForcedPosition, ForcedPosition, nullptr);
					}
...
UAnimMontage* FAnimMontageInstance::SetSequencerMontagePosition(FName SlotName, UAnimInstance* AnimInstance, int32& InOutInstanceId, UAnimSequenceBase* InAnimSequence, float InFromPosition, float InToPosition, float Weight, bool bLooping, bool bInPlaying)
{
	...
	if (AnimInst)
	{
		...
		if (MontageInstanceToUpdate)
		{
			...
			if (bInPlaying)
			{
				// If the forced from position is still set, it means we either haven't advanced the montage yet (e.g. because of URO) 
				// or the sequencer is looping and calling this function twice in one frame before the montage had a chance to advance 
				if (MontageCVars::bUseForcedSequencerMontageLoopBehavior 
					&& MontageInstanceToUpdate->ForcedNextFromPosition.IsSet() && MontageInstanceToUpdate->ForcedNextToPosition.IsSet())
				{
					const float ForcedFromPosition = MontageInstanceToUpdate->ForcedNextFromPosition.GetValue();
					const bool bPlayingForwards = InToPosition >= InFromPosition; 
					const bool bIsLooping = bPlayingForwards ? (ForcedFromPosition > InToPosition) : (ForcedFromPosition < InToPosition);

					// If we detected that a loop is being performed, cache the loop boundary positions 
					// so we can evaluate them correctly when advancing the montage 
					if (bIsLooping)
					{
						// This is the location in the animation that marks the end of that section 
						MontageInstanceToUpdate->ForcedLoopBoundaryPosition = MontageInstanceToUpdate->ForcedNextToPosition.GetValue();
					
						// This is the location we want to resume from after hitting the loop boundary 
						MontageInstanceToUpdate->ForcedLoopResumePosition = InFromPosition;
					}
					
					// Update the from position to the previous from position to account for the full range of update that needs to happen
					// Even if no loop is detected we want to update the from position to allow full range evaluation if sequencer happened to tick
					// more frequently than the montage advanced (e.g. if URO is active)
					InFromPosition = ForcedFromPosition;
				}
	...
	{
		UE_LOGF(LogSkeletalMesh, Warning, "Invalid animation configuration when attempting to set animation possition with : %ls", *InAnimSequence->GetName());
	}
...
UAnimMontage* UAnimMontage::CreateSlotAnimationAsDynamicMontage_WithBlendSettings(UAnimSequenceBase* Asset, FName SlotNodeName, const FMontageBlendSettings& BlendInSettings, const FMontageBlendSettings& BlendOutSettings, float InPlayRate, int32 LoopCount, float InBlendOutTriggerTime)
{
	...
	{
		// user warning
		UE_LOGF(LogAnimMontage, Warning, "PlaySlotAnimationAsDynamicMontage: Invalid input asset(%ls). If Montage, please use Montage_Play", *GetNameSafe(Asset));
		return nullptr;
	}
	...
	{
		// user warning
		UE_LOGF(LogAnimMontage, Warning, "SlotNode Name is required. Make sure to add Slot Node in your anim graph and name it.");
		return nullptr;
	}
	...
	{
		UE_LOGF(LogAnimMontage, Warning, "This animation isn't supported to play as montage");
		return nullptr;
	}
				...
				else
				{
					if (BaseFrameRate.IsValid())
					{
						...
						else if (TargetRate != BaseFrameRate && !BaseFrameRate.IsMultipleOf(TargetRate))
						{
							...
							{
								UE_LOGF(LogAnimation, Warning, "Frame rate of animation %ls (%ls) is incompatible with other animations in Animation Montage %ls - underlying frame-rate will be set to %ls:%ls", *Base->GetName(), *BaseFrameRate.ToPrettyText().ToString(), *GetName(), *Super::GetSamplingFrameRate().ToPrettyText().ToString(), *AssetString);
							}
					...
					{
						UE_LOGF(LogAnimMontage, Warning, "Invalid frame rate %ls for %ls in %ls", *BaseFrameRate.ToPrettyText().ToString(), *Base->GetName(), *GetName());
					}
				}			
			}	
		}			
