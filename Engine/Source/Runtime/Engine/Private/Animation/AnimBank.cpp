...
bool FAnimBankBuildAsyncCacheTask::BuildData(const UE::FSharedString& Name, const UE::DerivedData::FCacheKey& Key)
{
	...
	*Data = {};
	...
	{
		UE_LOGF(LogAnimBank, Error, "Cannot find a valid skinned asset to build the animation bank asset.");
		return false;
	}
	...
	for (int32 ItemIndex = 0; ItemIndex < AnimBank->Sequences.Num(); ++ItemIndex)
	{
		...
		{
			UE_LOGF(LogAnimBank, Error, "Animation bank referenced sequence is missing compressed data!");
			return false;
		}
		...
		{
			TRACE_CPUPROFILER_EVENT_SCOPE(FAnimBankBuildAsyncCacheTask::BuildData);
			FBoneContainer BoneContainer(BoneIndices, UE::Anim::FCurveFilterSettings(), *AnimBank->Asset);

			int32 KeyIndex = 0;

			for (uint32 Frame = 0; Frame < BankEntry.FrameCount; ++Frame)
			{
				TRACE_CPUPROFILER_EVENT_SCOPE(FAnimBankBuildAsyncCacheTask::Frame);
		...
		{
			UE_LOGF(LogAnimBank, Error, "BankEntry contains NaN in sampled bounds!");
			return false;
		}
...
FIoHash UAnimBank::CreateDerivedDataKeyHash(const ITargetPlatform* TargetPlatform)
{
	...
	FGuid AnimBankVersionGuid(0x51842261, 0xDBAF4D8C, 0xB14ABA3D, 0xC9A914B5);
...
float UAnimBank::GetSequencePlayLength(int32 SequenceIndex) const
{
	...
	if (IsCompiling())
	{
		...
		{
			if (Sequences[SequenceIndex].Sequence)
			{
...
#endif
	if (Data.Entries.IsValidIndex(SequenceIndex))
	{
		const float SampleInterval = 1.0f / ANIM_BANK_SAMPLE_RATE;
		return static_cast<float>(Data.Entries[SequenceIndex].FrameCount - 1) * SampleInterval;
	}

	return 0.0f;
}

bool UAnimBank::IsSequenceLooping(int32 SequenceIndex) const
{
#if WITH_EDITORONLY_DATA
	if (IsCompiling())
	{
		if (Sequences.IsValidIndex(SequenceIndex))
		{
			if (Sequences[SequenceIndex].Sequence)
			{
				return Sequences[SequenceIndex].bLooping;
			}
		}
	}
	else
	...
	if (Data.Entries.IsValidIndex(SequenceIndex))
	{
		return Data.Entries[SequenceIndex].IsLooping();
	}
...
static float GetAnimBankTrackLoopedPosition(float Position, float TrackLength)
{
	if (TrackLength >= 0.0f)
	{
		Position = TrackLength == 0.0f ? 0.0f : FMath::Fmod(Position, TrackLength);

		// Returns a negative range if input is negative.
		if (Position < 0.0f)
		{
...
void FAnimBankTrackPackedData::Update(float SequenceLength, bool bLooping, float DeltaTime)
{
	if (bAutoPlay)
	{
		PreviousPosition = Position;
		Position += PlayRate * DeltaTime;
		bHasPreviousPosition = false;
		Wrap(SequenceLength, bLooping);
	}
}

void FAnimBankTrackPackedData::Wrap(float SequenceLength, bool bLooping)
{
	const EAnimSequenceTrackLoopMode LoopModeEnum = GetLoopMode();

	if (LoopModeEnum == EAnimSequenceTrackLoopMode::Loop)
	{
		Position         = GetAnimBankTrackLoopedPosition(Position, SequenceLength);
		PreviousPosition = GetAnimBankTrackLoopedPosition(PreviousPosition, SequenceLength);
	}
	else
	{
		Position         = FMath::Clamp(Position, 0.0f, SequenceLength);
		PreviousPosition = FMath::Clamp(PreviousPosition, 0.0f, SequenceLength);
	}
}

void FAnimBankTrackPackedData::Serialize(FArchive& Ar)
{
	constexpr auto MakeBitMask = [] (uint32 Bits) -> uint32 { return (Bits >= 32) ? 0xffffffffu : ((1u << Bits) - 1u); };

	constexpr uint32 ItemIndexBits = 20;
	constexpr uint32 ItemIndexMask = MakeBitMask(ItemIndexBits);

	constexpr uint32 LoopModeShift = ItemIndexBits;
	constexpr uint32 LoopModeBits  = 2;
	...
	constexpr uint32 HasPreviousPositionShift = AutoPlayShift + AutoPlayBits;
	constexpr uint32 HasPreviousPositionBits  = 1;
	constexpr uint32 HasPreviousPositionMask  = MakeBitMask(HasPreviousPositionBits);

	uint32 PackedBits;
	if (Ar.IsSaving())
	{
		PackedBits =
			   (ItemIndex & ItemIndexMask)
			| ((LoopMode  & LoopModeMask ) << LoopModeShift)
			| ((bAutoPlay & AutoPlayMask ) << AutoPlayShift)
			| ((bHasPreviousPosition & HasPreviousPositionMask) << HasPreviousPositionShift);
		Ar << PackedBits;
		Ar << Position;
	...
	{
		Ar << PackedBits;
		ItemIndex =  PackedBits & ItemIndexMask;
		LoopMode  = (PackedBits >> LoopModeShift) & LoopModeMask;
		bAutoPlay = (PackedBits >> AutoPlayShift) & AutoPlayMask;
		bHasPreviousPosition = (PackedBits >> HasPreviousPositionShift) & HasPreviousPositionMask;
		Ar << Position;
...
FAnimBankRenderData::~FAnimBankRenderData()
{
	check(Proxies.Num() == 0);
}

void FAnimBankRenderData::InsertProxy(FInstancedSkinningSceneExtensionProxy* Proxy)
{
	Proxies.FindOrAdd(Proxy);
}

void FAnimBankRenderData::RemoveProxy(FInstancedSkinningSceneExtensionProxy* Proxy)
{
	Proxies.Remove(Proxy);
}

void FAnimBankRenderData::Patch(FAnimBankTrackPool::FPatch&& Patch)
{
	...
	Tracks.Patch<FAnimBankTrackPackedData>(MoveTemp(Patch),
		[&](FAnimBankTrackPackedData& Dst, const FAnimBankTrackPackedData& Src, bool bWasActive)
	{
		Dst = Src;
	});
}
...
FAnimBankTransformProviderProxy::FAnimBankTransformProviderProxy(TConstArrayView<FAnimBankItem> InAnimBankItems, FInstancedSkinningSceneExtensionProxy* InSceneExtensionProxy)
	: AnimBankItems(InAnimBankItems)
	, TrackAllocator(new FAnimBankTrackPool)
	, Tracks(TrackAllocator.Get())
	, SceneExtensionProxy(InSceneExtensionProxy)
{
	TrackAllocator->Reserve(AnimBankItems.Num());

	for (int32 ItemIndex = 0; ItemIndex < AnimBankItems.Num(); ++ItemIndex)
	{
		const FAnimBankItem& Item = AnimBankItems[ItemIndex];
		...
		Data.Position             = BankEntry.Position;
		Data.PlayRate             = BankEntry.PlayRate;
		Data.LoopMode             = BankEntry.IsLooping() ? EAnimSequenceTrackLoopMode::Loop : EAnimSequenceTrackLoopMode::Clamp;

		TrackAllocator->AllocateTrack(FAnimBankTrackPackedData::Pack(Data));
	}
}

FAnimBankTransformProviderProxy::FAnimBankTransformProviderProxy(TConstArrayView<FAnimBankItem> InAnimBankItems, FInstancedSkinningSceneExtensionProxy* InSceneExtensionProxy, FAnimBankRenderData& InRenderData)
	: AnimBankItems(InAnimBankItems)
...
void FAnimBankTransformProviderProxy::CreateRenderThreadResources(FSceneInterface& Scene, FRHICommandListBase& RHICmdList)
{
	if (RenderData)
	{
	...
	AnimBankTransformProvider = Scene.GetAnimBankTransformProvider();

	if (AnimBankTransformProvider)
	{
		AnimBankTransformProvider->RegisterAnimBank(this);
	}
}

void FAnimBankTransformProviderProxy::DestroyRenderThreadResources()
{
	if (RenderData)
	{
	...
	if (AnimBankTransformProvider)
	{
		AnimBankTransformProvider->UnregisterAnimBank(this);
		AnimBankTransformProvider = nullptr;
	}
}
...
uint32 UAnimBankData::GetUniqueAnimationCount() const
{
...
bool UAnimBankData::IsAnimBankValid(FInstancedSkinningSceneExtensionProxy* ExtensionProxy) const
{
	for (const FAnimBankItem& Item : AnimBankItems)
	{
		if (Item.BankAsset == nullptr /* || Item.BankAsset->Asset != ExtensionProxy->GetSkinnedAsset() */)
		{
			return false;
		}
...
FTransformProviderRenderProxy* UAnimBankData::CreateRenderProxy(FInstancedSkinningSceneExtensionProxy* SceneExtensionProxy) const
{
	if (IsAnimBankValid(SceneExtensionProxy))
	{
		return new FAnimBankTransformProviderProxy(AnimBankItems, SceneExtensionProxy);
	}
	return nullptr;
}
...
float UAnimBankData::GetSequencePlayLength(int32 ItemIndex) const
{
	if (AnimBankItems.IsValidIndex(ItemIndex))
	{
		const FAnimBankItem& AnimBankItem = AnimBankItems[ItemIndex];

		if (const UAnimBank* BankAsset = AnimBankItem.BankAsset)
		{
			return BankAsset->GetSequencePlayLength(AnimBankItem.SequenceIndex);
		}
	}
	return 0.0f;
}
...
UInstancedSkinnedMeshComponent* FSkinnedMeshComponentDescriptorBase::CreateComponent(UObject* Outer, FName Name, EObjectFlags ObjectFlags) const
{
	UInstancedSkinnedMeshComponent* ISKMComponent = NewObject<UInstancedSkinnedMeshComponent>(Outer, ComponentClass, Name, ObjectFlags);

	InitComponent(ISKMComponent);

	return ISKMComponent;
}
...
void FSkinnedMeshComponentDescriptorBase::InitFrom(const UInstancedSkinnedMeshComponent* Template, bool bInitBodyInstance)
{
	...
	bVisibleInReflections = Template->bVisibleInReflections;
	...
	bRenderStatic = Template->bRenderStatic;
...
void FSkinnedMeshComponentDescriptorBase::InitComponent(UInstancedSkinnedMeshComponent* Component) const
{
	...
	Component->SetVisibleFlag(bVisible);
	Component->bRenderStatic = bRenderStatic;
	Component->bEvaluateWorldPositionOffset = bEvaluateWorldPositionOffset;
	Component->BodyInstance.CopyBodyInstancePropertiesFrom(&BodyInstance);
	Component->WorldPositionOffsetDisableDistance = WorldPositionOffsetDisableDistance;
	Component->NanitePixelProgrammableDistance = NanitePixelProgrammableDistance;
	Component->DetailMode = DetailMode;
#if WITH_EDITOR
...
bool FSkinnedMeshComponentDescriptorBase::operator==(const FSkinnedMeshComponentDescriptorBase& Other) const
{
	return
		...
		bVisibleInReflections == Other.bVisibleInReflections &&
