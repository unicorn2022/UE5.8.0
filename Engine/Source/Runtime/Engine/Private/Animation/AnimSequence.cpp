...
static const FName AssetRegistryTag_CompressionRatio(TEXT("Compression Ratio"));
static const FName AssetRegistryTag_SourceFrameRate(TEXT("Source Frame Rate"));
static const FName AssetRegistryTag_NumberOfFrames(TEXT("Number of Frames"));
static const FName AssetRegistryTag_NumberOfKeys(TEXT("Number of Keys"));
static const FName AssetRegistryTag_CompressedSize(TEXT("Compressed Size (KB)"));
static const FName AssetRegistryTag_TargetFrameRate(TEXT("Target Frame Rate"));
...
void UAnimSequence::GetAssetRegistryTags(FAssetRegistryTagsContext Context) const
{
	...
	{
		Context.AddTag(FAssetRegistryTag(AssetRegistryTag_CompressionRatio, FString::Printf(TEXT("%.03f"), (double)GetApproxCompressedSize() / (double)GetUncompressedRawSize()), FAssetRegistryTag::TT_Numerical));
		Context.AddTag(FAssetRegistryTag(AssetRegistryTag_SourceFrameRate, FString::Printf(TEXT("%.2f"), DataModelInterface->GetFrameRate().AsDecimal()), FAssetRegistryTag::TT_Numerical));
		Context.AddTag(FAssetRegistryTag(AssetRegistryTag_NumberOfFrames, FString::Printf(TEXT("%.i"), DataModelInterface->GetNumberOfFrames()), FAssetRegistryTag::TT_Numerical));
		Context.AddTag(FAssetRegistryTag(AssetRegistryTag_NumberOfKeys, FString::Printf(TEXT("%i"), DataModelInterface->GetNumberOfKeys()), FAssetRegistryTag::TT_Numerical));
	}
	else
	{
		Context.AddTag(FAssetRegistryTag(AssetRegistryTag_SourceFrameRate, TEXT("0.0"), FAssetRegistryTag::TT_Numerical));
		Context.AddTag(FAssetRegistryTag(AssetRegistryTag_NumberOfFrames, TEXT("0"), FAssetRegistryTag::TT_Numerical));
		Context.AddTag(FAssetRegistryTag(AssetRegistryTag_NumberOfKeys, TEXT("0"), FAssetRegistryTag::TT_Numerical));
	}
	...
	Context.AddTag(FAssetRegistryTag(AssetRegistryTag_CompressedSize, FString::Printf(TEXT("%.02f"), (float)GetApproxCompressedSize() / 1024.0f), FAssetRegistryTag::TT_Numerical));
	Context.AddTag(FAssetRegistryTag(AssetRegistryTag_TargetFrameRate, FString::Printf(TEXT("%.2f"), PlatformTargetFrameRate.Default.AsDecimal()), FAssetRegistryTag::TT_Numerical));
	Super::GetAssetRegistryTags(Context);
}

#if WITH_EDITOR

void UAnimSequence::GetAssetRegistryTagMetadata(TMap<FName, FAssetRegistryTagMetadata>& OutMetadata) const
{
	OutMetadata.Add(AssetRegistryTag_CompressionRatio, FAssetRegistryTagMetadata().SetDisplayName(LOCTEXT("CompressionRatio", "Compression Ratio")));
	OutMetadata.Add(AssetRegistryTag_SourceFrameRate, FAssetRegistryTagMetadata().SetDisplayName(LOCTEXT("SourceFrameRate", "Source Frame Rate")));
	OutMetadata.Add(AssetRegistryTag_NumberOfFrames, FAssetRegistryTagMetadata().SetDisplayName(LOCTEXT("NumberOfFrames", "Number of Frames")));
	OutMetadata.Add(AssetRegistryTag_NumberOfKeys, FAssetRegistryTagMetadata().SetDisplayName(LOCTEXT("NumberOfKeys", "Number of Keys")));
	OutMetadata.Add(AssetRegistryTag_CompressedSize, FAssetRegistryTagMetadata().SetDisplayName(LOCTEXT("CompressedSize", "Compressed Size (KB)")));
	OutMetadata.Add(AssetRegistryTag_TargetFrameRate, FAssetRegistryTagMetadata().SetDisplayName(LOCTEXT("TargetFrameRate", "Target Frame Rate")));
}
...
void UAnimSequence::Serialize(FArchive& Ar)
{
	...
	if (Ar.IsLoading() && Ar.CustomVer(FUE5MainStreamObjectVersion::GUID) < FUE5MainStreamObjectVersion::RenamingAnimationNumFrames)
	{
		...
		{
			UE_LOGF(LogAnimation, Warning, "Animation %ls needs resaving - Invalid number of keys %i stored according to maximum number animation data track keys. Setting new number of keys %i.", *GetName(), NumberOfKeys, MaxNumberOfTrackKeys);
			NumberOfKeys = MaxNumberOfTrackKeys;
		}

		UpdateFrameRate_DeprecatedLoad();
		...
		if (RawAnimationData.Num())
		{
			...
			{
				UE_LOGF(LogAnimation, Warning, "Animation %ls needs resaving - Inaccurate sequence length %5.5f according to number of frames and frame rate (delta of %f). Setting new length %5.5f.", *GetName(), SequenceLength, SampledTime - SequenceLength, SampledTime);
...
void UAnimSequence::UpdateFrameRate_DeprecatedLoad()
{
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	const int32 NumberOfFrames = FMath::Max(NumberOfKeys - 1, 1);
...
void UAnimSequence::PostLoad()
{
	...
	if (IsValidAdditive())
	{
		if (RefPoseSeq && RefPoseSeq->GetSkeleton() == nullptr)
		{
			...
			UE_LOGF(LogAnimation, Warning, "Animation %ls - Invalid additive animation base animation (%ls)", *GetName(), *RefPoseSeq->GetName());
			RefPoseSeq = nullptr;
		}
	}
	...
	if (GetOutermost()->HasAnyPackageFlags(PKG_Cooked | PKG_FilterEditorOnly))
	{
		...
		if (const USkeleton* CurrentSkeleton = GetSkeleton())
		{
			...
			{
				UE_LOGF(LogAnimation, Fatal, "No animation compression exists for sequence %ls (%ls)", *GetName(), (GetOuter() ? *GetOuter()->GetFullName() : *GetFullName()) );
			}
		}
	}
...
void ShowResaveMessage(const UAnimSequence* Sequence)
{
	...
	{
		UE_LOGF(LogAnimation, Log, "Resave Animation Required(%ls, %ls): Fixing track data and recompressing.", *GetNameSafe(Sequence), *Sequence->GetPathName());
...
bool UAnimSequence::HasCompressedDataForHash(const FIoHash& InKeyHash) const
{
	...
	if (InKeyHash.IsZero())
	{
		UE_LOGF(LogAnimation, Warning, "HasCompressedDataForHash: Zero key hash %ls ", *GetName());
		return false;
	}
	...
	if(const TUniquePtr<FCompressedAnimSequence>* CompressedDataPtr = DataByPlatformKeyHash.Find(InKeyHash))
	{
		...
		{
			UE::TScopeLock LockGuard(CacheTasksMutex);
...
bool UAnimSequence::IsCachedCookedPlatformDataLoaded(const ITargetPlatform* TargetPlatform)
{
	...
	if (KeyHash.IsZero())
	{
		UE_LOGF(LogAnimation, Warning, "Zero key hash compressed animation data for %ls requested platform %ls", *GetName(), *TargetPlatform->PlatformName());
		return true;
	}
...
FTransform UAnimSequence::ExtractRootMotionFromRange(double StartTime, double EndTime, const FAnimExtractContext& ExtractionContext) const
{
	...
	{
		UE_LOGF(LogAnimation, Error, "ExtractRootMotionFromRange for %ls failed, Skeleton == nullptr.", *GetName());
...
void UAnimSequence::SetRetargetSourceAsset(USkeletalMesh* InRetargetSourceAsset)
{
	...
	{
		UE_LOGF(LogAnimation, Error, "Error, Transient asset [%ls] can not be assigned as Retarget Source for AnimSequence [%ls]. Please, use a non transient asset as retarget surce."
...
void UAnimSequence::OnAnimModelLoaded()
{
	...
	if (ShouldDataModelBeValid())
	{
		...
		{
			UE_LOGF(LogAnimation, Warning, "No animation data exists for sequence %ls (%ls)", *GetName(), (GetOuter() ? *GetOuter()->GetFullName() : *GetFullName()) );
...
void UAnimSequence::FlagDependentAnimationsAsRawDataOnly() const
{
	for (TObjectIterator<UAnimSequence> Iter; Iter; ++Iter)
	{
		...
		/** Ignore self, as data will already be recompressed at higher-level callsite */
		if (Seq->HasAllFlags(RF_LoadCompleted) && Seq != this && Seq->RefPoseSeq == this)
		{
			Seq->ClearAllCompressionData();
			Seq->BeginCacheDerivedDataForCurrentPlatform();
		}
	}
}
	...
	if (bPlayingForwards)
	{
		...
		while (true)
		{
			...
			if (CurrentMoveDelta > NextMarker.TimeToMarker)
			{
				...
				MarkersPassed[PassedMarker].PassedMarkerName = MarkerOrMirroredName(NextSyncMarker.MarkerName, MirrorTable);
	...
	else
	{
		...
		while (true)
		{
			...
			if (CurrentMoveDelta < PrevMarker.TimeToMarker)
			{
				...
				// Record that we just passed a marker.
				const int32 PassedMarker = MarkersPassed.Add(FPassedMarker());
				MarkersPassed[PassedMarker].PassedMarkerName = MarkerOrMirroredName(PrevSyncMarker.MarkerName, MirrorTable);
				MarkersPassed[PassedMarker].DeltaTimeWhenPassed = CurrentMoveDelta;
...
void UAnimSequence::MoveAttributesToModel()
{
	...
	if (TargetSkeleton && PerBoneCustomAttributeData.Num())
	{
		...
		for (const FCustomAttributePerBoneData& PerBoneData : PerBoneCustomAttributeData)
		{
			...
			if (BoneName != NAME_None)
			{
				auto ProcessCustomAttribute = [BoneName, this](const FCustomAttribute& Attribute)
				{
					...
					case EVariantTypes::Float:
					{
						...
						{
							UE_LOGF(LogAnimation, Warning, "Failed to upgrade float attribute %ls for bone %ls", *Attribute.Name.ToString(), *BoneName.ToString());
					...
					case EVariantTypes::Int32:
					{
						...
						{
							UE_LOGF(LogAnimation, Warning, "Failed to upgrade integer attribute %ls for bone %ls", *Attribute.Name.ToString(), *BoneName.ToString());
						}
					...
					case EVariantTypes::String:
					{
						...
						{
							UE_LOGF(LogAnimation, Warning, "Failed to upgrade string attribute %ls for bone %ls", *Attribute.Name.ToString(), *BoneName.ToString());
						}
						break;
					}
			...
			{
				UE_LOGF(LogAnimation, Warning, "Failed to upgrade custom attributes for bone index %i as no such bone index existing in the Skeleton %ls", PerBoneData.BoneTreeIndex, *TargetSkeleton->GetName());
			}
		}
...
// Opt-in binary path: archive-based key generation (skips intermediate FString).
static bool GAnimSequenceUseBinaryDDCKey = false;
static FAutoConsoleVariableRef CVarAnimSequenceUseBinaryDDCKey(
	TEXT("a.AnimSequence.UseBinaryDDCKey"),
	GAnimSequenceUseBinaryDDCKey,
	TEXT("When true, UAnimSequence uses archive-based DDC key generation (skips intermediate FString allocation). Default false uses the legacy CreateDerivedDataKeyString path."));
...
FIoHash UAnimSequence::CreateDerivedDataKeyHash(const ITargetPlatform* TargetPlatform) const
{
	...
	if (GAnimSequenceUseBinaryDDCKey)
	{
		// Write directly to the hasher.
		FMemoryHasherBlake3 Writer;
		if (!CreateDerivedDataKey(TargetPlatform, Writer))
		{
			return FIoHash::Zero;
		}
	...
	{
		// Legacy: build readable key string then hash.
...
bool UAnimSequence::CreateDerivedDataKey(const ITargetPlatform* TargetPlatform, FArchive& Output) const
{
    const USkeleton* CurrentSkeleton = GetSkeleton();
    if (CurrentSkeleton == nullptr)
    {
        UE_LOGF(LogAnimation, Warning, "Animation Compression request for %ls failed, Skeleton == nullptr.", *GetName());
        return false;
    }

    if (BoneCompressionSettings == nullptr || CurveCompressionSettings == nullptr || VariableFrameStrippingSettings == nullptr)
    {
        UE_LOGF(LogAnimation, Warning, "Animation Compression request for %ls failed, BoneCompressionSettings == nullptr || CurveCompressionSettings == nullptr || VariableFrameStrippingSettings == nullptr.", *GetName());
        return false;
    }

    const bool bIsValidAdditive = IsValidAdditive();

    bool bPerformFrameStripping = GPerformFrameStripping == 1;
    bool bPerformFrameStrippingOnOddNumberedFrames = GPerformFrameStrippingOddFramedAnimations == 1;
    {
    ...
    // FArchive::operator<< takes a non-const ref; pass-by-value gives us a writable local.
    auto Write = [&Output](auto Value) { Output << Value; };

    Write(CompressCommandletVersion);
    Write(GetDataModel()->GenerateGuid());
    Write(CurrentSkeleton->GetGuid());
    Write(CurrentSkeleton->GetVirtualBoneGuid());

    // Additive type bundle — encode as raw bytes; absent additive guid emits nothing (length differentiates).
    Write(bIsValidAdditive ? (uint8)AdditiveAnimType : (uint8)0);
    Write(bIsValidAdditive ? (uint8)RefPoseType : (uint8)0);
    Write(RefFrameIndex);
    if (bIsValidAdditive && RefPoseSeq)
    {
        Write(RefPoseSeq->GetDataModel()->GenerateGuid());
    }

    Write(CompressionErrorThresholdScale);
    Write(bPerformFrameStripping);

    BoneCompressionSettings->PopulateDDCKey(UE::Anim::Compression::FAnimDDCKeyArgs(*this, TargetPlatform), Output);
    CurveCompressionSettings->PopulateDDCKey(Output);
    VariableFrameStrippingSettings->PopulateDDCKey(UE::Anim::Compression::FAnimDDCKeyArgs(*this, TargetPlatform), Output);

    if (bIsValidAdditive)
    {
        // Additive sequences are compressed in re-targeted space, as such we need to include the re-targeting transforms in our key
        for (FTransform RetargetTransform : GetRetargetTransforms())
        {
            Write(RetargetTransform);
        }
    }
    ...
    {
        Write(Socket->BoneName);
    }

    if (FCompressibleAnimData::GUseHighFidelityAdditiveCurveCompression)
    {
        Write(FCompressibleAnimData::GUseHighFidelityAdditiveCurveCompression);
    }

    const FFrameRate FrameRate = UE::Anim::Compression::GetCompressionFrameRate(*this, TargetPlatform);
    Write(FrameRate.Numerator);
    Write(FrameRate.Denominator);
    Write(UE::Anim::Compression::AnimationCompressionVersionString);
...
FString UAnimSequence::CreateDerivedDataKeyString(const ITargetPlatform* TargetPlatform) const
{
	...
	if (FCompressibleAnimData::GUseHighFidelityAdditiveCurveCompression)
	{
		ArcToHexString.Ar << FCompressibleAnimData::GUseHighFidelityAdditiveCurveCompression;
	}
...
FIoHash UAnimSequence::BeginCacheDerivedData(const ITargetPlatform* TargetPlatform)
{
	...
	{
		UE_LOGF(LogAnimation, Warning, "Animation Compression request for %ls was blocked, bBlockCompressionRequests == true.", *GetName());
		return FIoHash::Zero;
	}
	...
	FCompressedAnimSequence* TargetData = nullptr;
	{
		...
		{
			UE_LOGF(LogAnimation, Warning, "Animation Compression request for %ls was blocked, Residency.", *GetName());
			return FIoHash::Zero;
		}
...
void UAnimSequence::RequestResidency(const ITargetPlatform* InPlatform, uint32 InReferencerHash)
{
	...
	{
		UE_CLOGF(false, LogAnimationCompression, Display, "RequestResidency platform data [KeyHash]: %ls %ls %x %ls", *InPlatform->DisplayName().ToString(), *LexToString(KeyHash), InReferencerHash, *GetPathName());
...
void UAnimSequence::ReleaseResidency(const ITargetPlatform* InPlatform, uint32 InReferencerHash)
{
	...
	TArray<uint32> ReferencerHashes;
	{
		...
		verify(ResidencyReferencerHashes.Remove(InReferencerHash) == 1);
		verify(PlatformHashToReferencers.Remove(KeyHash, InReferencerHash) == 1);
		
		// Check if there are any references left
		PlatformHashToReferencers.MultiFind(KeyHash, ReferencerHashes);

	
		UE_CLOGF(false, LogAnimationCompression, Display, "ReleaseResidency platform data [KeyHash]: %ls %ls %x %ls", *InPlatform->DisplayName().ToString(), *LexToString(KeyHash), InReferencerHash, *GetPathName());
	}
	...
	if (bHasInflightTasks)
	{
		...
		for (auto It = CacheTasksByKeyHash.CreateIterator(); It; ++It)
		{
			...
			else
			{
				...
				{
					// Failed to compress
					UE_LOGF(LogAnimationCompression, Display, "Failed to finish async Animation Compression task for %ls, as the generated data is not valid.", *GetName());
					ResetData();
				}
			}
