...
protected:
	...
	void CompressRotation_16_16_16(const FRotationTrack& RotationData, float ZeroingThreshold)
	{
		...
		for (int32 i = 0; i < NumKeys; ++i)
		{
			...
			{
				UE_LOGF(LogAnimationCompression, Log, "Error: Loss of normalization!");
				UE_LOGF(LogAnimationCompression, Log, "  Track: %i, Key: %i", 0, i);
				UE_LOGF(LogAnimationCompression, Log, "  Q : %ls", *Q.ToString());
				UE_LOGF(LogAnimationCompression, Log, "  Q': %ls", *DecompressedQ.ToString());
				UE_LOGF(LogAnimationCompression, Log, " XYZ: %i, %i, %i", X, Y, Z);
			}
	...
	void CompressRotation_11_11_10(const FRotationTrack& RotationData, float ZeroingThreshold)
	{
		...
		for (int32 i = 0; i < NumKeys; ++i)
		{
			...
			if (!DecompressedQ.IsNormalized())
			{
				...
				UE_LOGF(LogAnimationCompression, Log, "  Q : %ls", *Q.ToString());
				UE_LOGF(LogAnimationCompression, Log, "  Q': %ls", *DecompressedQ.ToString());
				UE_LOGF(LogAnimationCompression, Log, " XYZ: %f, %f, %f, %f", QRenorm.X, QRenorm.Y, QRenorm.Z, QRenorm.W);
				UE_LOGF(LogAnimationCompression, Log, " Mins(%f, %f, %f)   Maxs(%f, %f,%f)", KeyBounds.Min.X, KeyBounds.Min.Y, KeyBounds.Min.Z, KeyBounds.Max.X, KeyBounds.Max.Y, KeyBounds.Max.Z);
			}
			check(DecompressedQ.IsNormalized());
			const float Error = FQuat4f::ErrorAutoNormalize(Q, DecompressedQ);
			MaxError = FMath::Max(MaxError, Error);
...
public:
	...
	FPerTrackCompressor(int32 InCompressionType, const FTranslationTrack& TranslationData, const FPerTrackParams& Params)
	{
		...
		case ACF_IntervalFixed32NoW:
			...
			//case ACF_Float32NoW:
		default:
			UE_LOGF(LogAnimationCompression, Fatal,"Unsupported translation compression format");
			break;
		}
	...
	FPerTrackCompressor(int32 InCompressionType, const FRotationTrack& RotationData, const FPerTrackParams& Params)
	{
		...
		case ACF_Float32NoW:
			...
			break;
		default:
			UE_LOGF(LogAnimationCompression, Fatal,"Unsupported rotation compression format");
			break;
		}
	...
	FPerTrackCompressor(int32 InCompressionType, const FScaleTrack& ScaleData, const FPerTrackParams& Params)
	{
		...
		case ACF_IntervalFixed32NoW:
			...
			//case ACF_Float32NoW:
		default:
			UE_LOGF(LogAnimationCompression, Fatal,"Unsupported Scale compression format");
			break;
		}
...
// 			float ThresholdS_DueS = (TrackError.MaxErrorInScaleDueToScale > UE_SMALL_NUMBER) ? (PerturbationProbeSize / TrackError.MaxErrorInScaleDueToScale) : 1.0f;
		...
		UE_LOGF(LogAnimationCompression, Warning, "   Compressed track %i, Trans=%ls_%i (#keys=%i, err=%f), Rot=%ls_%i (#keys=%i, err=%f)  (height=%i max pos=%f, angle=%f)", 
			TrackIndex,
