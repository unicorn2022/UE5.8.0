...
void UAnimNotify_PlaySound::Notify(class USkeletalMeshComponent* MeshComp, class UAnimSequenceBase* Animation, const FAnimNotifyEventReference& EventReference)
{
	...
	if (Sound && MeshComp)
	{
		if (!Sound->IsOneShot())
		{
			UE_LOGF(LogAudio, Warning, "PlaySound notify: Anim %ls tried to play a sound asset which is not a one-shot: '%ls'. Spawning suppressed.", *GetNameSafe(Animation), *GetNameSafe(Sound));
			return;
		}
...
void UAnimNotify_PlaySound::PostLoad()
{
	...
	{
		// Sound MUST follow its owner if it follows one of its bones (edit condition prevents bFollow to become false in this case).
		bFollowEditable = false;
		bFollow = true;
	}
	else
	{
		// If no bone name is given, the user can choose whether it follows the root bone or not by setting bFollow themselves.
		bFollowEditable = true;
	}
}

void UAnimNotify_PlaySound::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);
	
	if (PropertyChangedEvent.GetMemberPropertyName() == GET_MEMBER_NAME_CHECKED(ThisClass, AttachName))
	{
		if (!AttachName.IsNone())
		{
			// Sound MUST follow its owner if it follows one of its bones (edit condition prevents bFollow to become false in this case).
