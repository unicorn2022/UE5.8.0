...
static FAutoConsoleVariableDeprecated CVarCullAllInVertexShader_Deprecated(TEXT("foliage.CullAllInVertexShader"), TEXT("foliage.CullAllInVertexShader_Unused"), TEXT("5.8"));
static TAutoConsoleVariable<int32> CVarCullAllInVertexShader(
	TEXT("foliage.CullAllInVertexShader_Unused"),
	0,
	TEXT("This cvar is no longer implemented as the support would add overhead and complexity."));
	...
	virtual bool Exec_Runtime(class UWorld* InWorld, const TCHAR* Cmd, FOutputDevice& Ar)
	{
		if(FParse::Command(&Cmd, TEXT("LIST ISM PHYSICS")))
		{
			...
			for(TObjectIterator<UInstancedStaticMeshComponent> It; It; ++It)
			{
				...
				{
					const TArray<FBodyInstance*>& Bodies = ISMComponent->GetInstanceBodies();
					if(Bodies.Num() == 0)
					{
						continue;
					}
...
{
	return DoesPlatformUseInstancedStaticMeshVertexFactory(Parameters.Platform)
			&& (Parameters.MaterialParameters.bIsUsedWithInstancedStaticMeshes || Parameters.MaterialParameters.bIsSpecialEngineMaterial)
...
{
	OutEnvironment.SetDefine(TEXT("IS_INSTANCED_STATIC_MESH_VF"), TEXT("1"));
...
void FInstancedStaticMeshVertexFactory::GetPSOPrecacheVertexFetchElements(EVertexInputStreamType VertexInputStreamType, FVertexDeclarationElementList& Elements)
{
	...
	if (GMaxRHIFeatureLevel > ERHIFeatureLevel::ES3_1 || !bMobileUsesGPUScene)
	{
		uint8 StreamIndex = Elements.Last().StreamIndex + 1;

		// InstanceOriginComponent
		Elements.Add(FVertexElement(StreamIndex++, 0, VET_Float4, 8, 0, false));

		// InstanceTransformComponent
		Elements.Add(FVertexElement(StreamIndex, 0, VET_Float4, 9, 0, false));
		Elements.Add(FVertexElement(StreamIndex, 8, VET_Float4, 10, 0, false));
		Elements.Add(FVertexElement(StreamIndex++, 16, VET_Float4, 11, 0, false));

		// InstanceLightmapAndShadowMapUVBiasComponent
		Elements.Add(FVertexElement(StreamIndex++, 16, VET_Short4N, 12, 0, false));
	}
}
...
void FInstancedStaticMeshVertexFactory::InitRHI(FRHICommandListBase& RHICmdList)
{
	...
	const bool bCanUseGPUScene = UseGPUScene(GShaderPlatformForFeatureLevel[ThisFeatureLevel]);
	...
	FVertexStreamList& Streams)
{
	...
	const bool bCanUseGPUScene = UseGPUScene(GShaderPlatformForFeatureLevel[FeatureLevel]);
	const bool bMobileUsesGPUScene = MobileSupportsGPUScene();
...
void FInstancedStaticMeshSceneProxy::GetDynamicMeshElements(const TArray<const FSceneView*>& Views, const FSceneViewFamily& ViewFamily, uint32 VisibilityMap, FMeshElementCollector& Collector) const
{
	...
	bool bBatchRenderSelection = bSelectionRenderEnabled && IsSelected();
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		if (VisibilityMap & (1 << ViewIndex))
		{
...

			{
				...
				FInstancedStaticMeshVFLooseUniformShaderParametersRef LooseUniformBuffer = CreateLooseUniformBuffer(View, /*InstancedLODRange=*/0, LODIndex, EUniformBufferUsage::UniformBuffer_SingleFrame);
				...
				for (int32 SectionIndex = 0; SectionIndex < LODModel.Sections.Num(); SectionIndex++)
				{
					...
					for (int32 BatchIndex = 0; BatchIndex < NumBatches; BatchIndex++)
					{
						...
						if (GetMeshElement(LODIndex, BatchIndex, SectionIndex, GetDepthPriorityGroup(View), bBatchRenderSelection, true, MeshElement))
						{
							...
							MeshElement.bUseSelectionOutline = bBatchRenderSelection;
							MeshElement.bUseWireframeSelectionColoring = bBatchRenderSelection;
...
static uint32 GetTypeHash(const FInstancedStaticMeshVFLooseUniformShaderParameters& Params)
{
	...
	HashParameters = HashCombine(HashParameters, GetTypeHash(Params.InstanceDitherParameters.InstancingTranslatedWorldViewOriginOne));
	HashParameters = HashCombine(HashParameters, GetTypeHash(Params.InstanceDitherParameters.InstancingTranslatedWorldViewOriginZero));
	HashParameters = HashCombine(HashParameters, GetTypeHash(Params.InstanceDitherParameters.InstancingViewZCompareOne));
	HashParameters = HashCombine(HashParameters, GetTypeHash(Params.InstanceDitherParameters.InstancingViewZCompareZero));
	HashParameters = HashCombine(HashParameters, GetTypeHash(Params.InstanceDitherParameters.InstancingViewZConstant));
...
static bool operator== (const FInstancedStaticMeshVFLooseUniformShaderParameters& A, const FInstancedStaticMeshVFLooseUniformShaderParameters& B)
{
	...
	return A.InstancingFadeOutParams == B.InstancingFadeOutParams
		&& A.InstanceDitherParameters.InstancingTranslatedWorldViewOriginOne == B.InstanceDitherParameters.InstancingTranslatedWorldViewOriginOne
		&& A.InstanceDitherParameters.InstancingTranslatedWorldViewOriginZero == B.InstanceDitherParameters.InstancingTranslatedWorldViewOriginZero
		&& A.InstanceDitherParameters.InstancingViewZCompareOne == B.InstanceDitherParameters.InstancingViewZCompareOne
		&& A.InstanceDitherParameters.InstancingViewZCompareZero == B.InstanceDitherParameters.InstancingViewZCompareZero
		&& A.InstanceDitherParameters.InstancingViewZConstant == B.InstanceDitherParameters.InstancingViewZConstant;
}
...
FLocalVertexFactoryInstanceDitherParameters FInstancedStaticMeshSceneProxy::GetInstanceDitherParameters(const FSceneView* View, uint32 InstancedLODRange, uint32 InstancedLODIndex) const
{
	FLocalVertexFactoryInstanceDitherParameters Result = MakeNullLocalVertexFactoryInstanceDitherParameters();

	// InstancedLODRange is only set for HierarchicalInstancedStaticMeshes
	if (InstancedLODRange)
	{
		...
		check(View);
		int32 FirstLOD = UserData_AllInstances.MinLOD;

		int32 DebugMin = FMath::Min(CVarMinLOD.GetValueOnRenderThread(), UserData_AllInstances.MeshRenderData->LODResources.Num() - 1);
		if (DebugMin >= 0)
		{
			FirstLOD = FMath::Max(FirstLOD, DebugMin);
		}

		FBoxSphereBounds ScaledBounds = UserData_AllInstances.MeshRenderData->Bounds.TransformBy(FTransform(FQuat::Identity, FVector::ZeroVector, UserData_AllInstances.AverageInstancesScale));
		...
		for (int32 SampleIndex = 0; SampleIndex < 2; SampleIndex++)
		{
			...
			if (MinSize > 0.0)
			{
				FinalCull = ComputeBoundsDrawDistance(MinSize, SphereRadius, View->ViewMatrices.GetViewToClip()) * LODScale;
			}
			...
			if (int(InstancedLODIndex) < UserData_AllInstances.MeshRenderData->LODResources.Num() - 1)
			{
				float NextCut = ComputeBoundsDrawDistance(UserData_AllInstances.MeshRenderData->ScreenSize[InstancedLODIndex + 1].GetValue(), SphereRadius, View->ViewMatrices.GetViewToClip()) * LODScale;
			...
			{
				float CurCut = ComputeBoundsDrawDistance(UserData_AllInstances.MeshRenderData->ScreenSize[InstancedLODIndex].GetValue(), SphereRadius, View->ViewMatrices.GetViewToClip()) * LODScale;
				if (CurCut < FinalCull)
				{
		...
		Result.InstancingViewZCompareZero = InstancingViewZCompareZero;
		Result.InstancingViewZCompareOne = InstancingViewZCompareOne;
		Result.InstancingViewZCompareOne.W = 1.0f;
		Result.InstancingViewZConstant = InstancingViewZConstant;
		Result.InstancingTranslatedWorldViewOriginZero = InstancingTranslatedWorldViewOriginZero;
		Result.InstancingTranslatedWorldViewOriginOne = InstancingTranslatedWorldViewOriginOne;
	}

	return Result;
}

FVector2f FInstancedStaticMeshSceneProxy::CalcInstancingFadeOutParams() const
{
	const float MaxDrawDistanceScale = GetCachedScalabilityCVars().ViewDistanceScale;
	const float StartDistance = UserData_AllInstances.StartCullDistance * MaxDrawDistanceScale;
	const float EndDistance = UserData_AllInstances.EndCullDistance * MaxDrawDistanceScale;

	FVector2f Result = FVector2f(StartDistance, 0.f);
	if (EndDistance > 0.0f)
	{
		if (EndDistance > StartDistance)
		{
			Result.Y = 1.f / (float)(EndDistance - StartDistance);
		}
		else
		{
			Result.Y = 1.f;
...
FInstancedStaticMeshVFLooseUniformShaderParametersRef FInstancedStaticMeshSceneProxy::CreateLooseUniformBuffer(const FSceneView* View, uint32 InstancedLODRange, uint32 InstancedLODIndex, EUniformBufferUsage UniformBufferUsage) const
{
	FInstancedStaticMeshVFLooseUniformShaderParameters LooseParameters;

	LooseParameters.InstanceDitherParameters = GetInstanceDitherParameters(View, InstancedLODRange, InstancedLODIndex);
	const FVector2f InstancingFadeOutParams = CalcInstancingFadeOutParams();
	LooseParameters.InstancingFadeOutParams = FVector4f(InstancingFadeOutParams.X, InstancingFadeOutParams.Y, 1.0f, 1.0f);
...
#endif
	if (DoesPlatformUseInstancedStaticMeshVertexFactory(GetScene().GetShaderPlatform()))
	{
		InstancedRenderData = MakeUnique<FInstancedStaticMeshRenderData>(&InProxyDesc, InFeatureLevel);
	}
...
void FInstancedStaticMeshSceneProxy::SetupProxy(const FInstancedStaticMeshSceneProxyDesc& InProxyDesc)
{
	...
	if (OverlayMaterial != nullptr)
	{
		if (!OverlayMaterial->CheckMaterialUsage_Concurrent(MATUSAGE_InstancedStaticMeshes))
		{
			...
			UE_LOGF(LogStaticMesh, Error, "Overlay material with missing usage flag was applied to instanced static mesh %ls", *InProxyDesc.GetStaticMesh()->GetPathName());
		}
	}
	...
	const bool bUseGPUScene = UseGPUScene(GetScene().GetShaderPlatform());
...
bool FInstancedStaticMeshSceneProxy::CanSetupInstancedMeshBatch(int32 LODIndex) const
{
	if (InstancedRenderData)
	{
		...
		check(LODLooseUniformBuffers.Num() == GetNumRenderDataVertexFactories());
		return LODLooseUniformBuffers.Contains(LODIndex) && LODIndex < GetNumRenderDataVertexFactories();
	}
	return LODIndex < GetNumRenderDataVertexFactories();
}
...
bool FInstancedStaticMeshSceneProxy::CanSetupRayTracingInstancedMeshBatch(int32 LODIndex) const
{
	...
	{
		return LODIndex < RenderData->RayTracingProxy->LODVertexFactories->Num();
	}
}
...
void FInstancedStaticMeshSceneProxy::CreateRenderThreadResources(FRHICommandListBase& RHICmdList)
{
	...
	if (InstancedRenderData)
	{
		...
		{
			FInstancedStaticMeshVFLooseUniformShaderParametersRef LooseUniformBuffer = CreateLooseUniformBuffer(nullptr, 0, i, EUniformBufferUsage::UniformBuffer_MultiFrame);
	...
	RayTracingLooseUniformBuffer = CreateLooseUniformBuffer(nullptr, /*InstancedLODRange*/ 0, /*InstancedLODIndex*/ 0, EUniformBufferUsage::UniformBuffer_MultiFrame);
}
...
bool FInstancedStaticMeshSceneProxy::GetInstancingFadeOutParams(FVector2f& OutInstancingFadeOutParams) const
{
	if (UserData_AllInstances.StartCullDistance > 0)
	{
		OutInstancingFadeOutParams = CalcInstancingFadeOutParams();;
		return true;
	}
	return false;
}
...
int32 FInstancedStaticMeshSceneProxy::GetNumRenderDataVertexFactories() const
{
	if (InstancedRenderData)
	{
		return InstancedRenderData->VertexFactories.Num();
	}
	else
	{
		return RenderData->LODVertexFactories.Num();
	}
}
...
void FInstancedStaticMeshSceneProxy::GetDynamicRayTracingInstances(FRayTracingInstanceCollector& Collector)
{
	...
	TArray<FMeshBatch>& RayTracingMeshBatches = Collector.AllocateMeshBatchArray();

	{
		int32 SectionCount = 0;
		if (InstancedRenderData)
		{
			SectionCount = (*InstancedRenderData->RayTracingProxyLODs)[LODIndex].Sections->Num();
		}
		else
		{
			SectionCount = RenderData->RayTracingProxy->LODs[LODIndex].Sections->Num();
		}
		...
		for (int32 SectionIdx = 0; SectionIdx < SectionCount; ++SectionIdx)
		{
			...
			if (bResult)
			{
				if (InstancedRenderData)
				{
					auto VF = RenderData->RayTracingProxy->bUsingRenderingLODs ? &InstancedRenderData->VertexFactories[LODIndex] : &InstancedRenderData->RayTracingProxyVertexFactories[LODIndex];
					check(MeshBatch.VertexFactory == VF);
				}
				else
				{
					auto VF = RenderData->RayTracingProxy->bUsingRenderingLODs ? &RenderData->LODVertexFactories[LODIndex].VertexFactory : &(*RenderData->RayTracingProxy->LODVertexFactories)[LODIndex].VertexFactory;
					check(MeshBatch.VertexFactory == VF);
				}
	...
	StaticRayTracingInstance.MaterialsView = RayTracingMeshBatches;
	...
	for (FVisibleInstance VisibleInstance : VisibleInstances)
	{
		...
		const bool bSupportsRayTracingDynamicGeometry = InstancedRenderData 
			? InstancedRenderData->RayTracingProxyVertexFactories[LODIndex].GetType()->SupportsRayTracingDynamicGeometry()
			: (*RenderData->RayTracingProxy->LODVertexFactories)[LODIndex].VertexFactory.GetType()->SupportsRayTracingDynamicGeometry();
		if (bDynamicRayTracingGeometry && bSupportsRayTracingDynamicGeometry)
		{
			...
			if (ActiveInstances[DynamicInstanceIdx] == INDEX_NONE)
			{
				...
				RayTracingInstance.MaterialsView = RayTracingMeshBatches;
				...
				UpdateParams.MeshBatchesView = RayTracingMeshBatches;
				UpdateParams.bUsingIndirectDraw = false;
				UpdateParams.NumVertices = NumVertices;
				UpdateParams.VertexBufferSize = uint32((SIZE_T)NumVertices * sizeof(FVector3f));
				UpdateParams.NumTriangles = DynamicData.Geometry.GetInitializer().TotalPrimitiveCount;
				UpdateParams.Geometry = &DynamicData.Geometry;
				UpdateParams.Buffer = nullptr;
				UpdateParams.bApplyWorldPositionOffset = true;
				UpdateParams.InstanceId = InstanceIndex;
				UpdateParams.WorldToInstance = (FMatrix44f) InstanceToWorld.Inverse();
...
void FInstancedStaticMeshSceneProxy::SetupRayTracingDynamicInstances(FRHICommandListBase& RHICmdList, int32 NumDynamicInstances, int32 LODIndex)
{
	...
	for (int32 Item = 0; Item < NumDynamicInstances; Item++)
	{
		...
		FRayTracingGeometryInitializer Initializer = RayTracingLOD.RayTracingGeometry->GetInitializer();
...
void FInstancedStaticMeshSceneProxy::GetRayTracingWPOConfig(bool& bOutHasRayTracingRepresentation, bool& bOutDynamicRayTracingGeometry) const
{
	...
	{
		UE_LOGF(LogStaticMesh, Warning, "Dynamic ray tracing geometry not currently supported for GPU instanced static meshes (%ls). Falling back to static ray tracing geometry.", *StaticMesh->GetPathName());
	...
	, InstancePhysicsBodies(MakeUnique<FInstancedMeshComponentBodies>(this))
{
	...
	bWalkableSlopeOverrideCachedGT = false;
#endif
...
const TArray<FBodyInstance*>& UInstancedStaticMeshComponent::GetInstanceBodies() const
{
	return InstancePhysicsBodies->GetBodies();
}
...
FPrimitiveSceneProxy* UInstancedStaticMeshComponent::CreateStaticMeshSceneProxy(Nanite::FMaterialAudit& NaniteMaterials, bool bCreateNanite)
{
	...
	{
		UE_LOGF(LogStaticMesh, Verbose, "Skipping CreateSceneProxy for UInstancedStaticMeshComponent %ls (UInstancedStaticMeshComponent PSOs are still compiling)", *GetFullName());
		return nullptr;
	}
...
void UInstancedStaticMeshComponent::BuildInstanceDataDeltaChangeSetCommon(FISMInstanceUpdateChangeSet &ChangeSet)
{
	...
	if (ChangeSet.Flags.bHasPerInstanceRandom)
	{
		...
		ChangeSet.GeneratePerInstanceRandomIds =
			...
			AdditionalRandomSeeds = AdditionalRandomSeeds](TArray<float> &InstanceRandomIDs)
			{
				FInstanceDataHelpers::GenerateInstanceRandomIDs(InstancingRandomSeed, AdditionalRandomSeeds, InstanceRandomIDs);
			};
	}
...
void UInstancedStaticMeshComponent::InitInstanceBody(int32 InstanceIdx, FBodyInstance* InstanceBodyInstance, FBodyInstance* ReferenceBodyInstance, bool bRuntimeBodyInstance, UBodySetup* BodySetup)
{
	if (!GetStaticMesh())
	{
		UE_LOGF(LogStaticMesh, Warning, "Unable to create a body instance for %ls in Actor %ls. No StaticMesh set.", *GetName(), GetOwner() ? *GetOwner()->GetName() : TEXT("?"));
		return;
	}

	check(InstanceIdx < PerInstanceSMData.Num());
	check(InstanceIdx < InstancePhysicsBodies->Num());
	check(InstanceBodyInstance);
	check(ReferenceBodyInstance);
	check(BodySetup);

	const FTransform InstanceTransform = FTransform(PerInstanceSMData[InstanceIdx].Transform) * GetComponentTransform();
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	InstancePhysicsBodies->InitInstanceBody(InstanceBodyInstance, ReferenceBodyInstance, InstanceIdx, InstanceTransform,
		bRuntimeBodyInstance, BodySetup);	
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
}
...
void UInstancedStaticMeshComponent::CreateAllInstanceBodies()
{
	...
	if (!BodySetup)
	{
		UE_LOGF(LogStaticMesh, Warning, "Instance Static Mesh Component unable to create InstanceBodies!");
		return;
	}

	TArray<FTransform> WorldTransforms;
	WorldTransforms.SetNumUninitialized(NumBodies);
	for (int32 i = 0; i < NumBodies; ++i)
	{
		WorldTransforms[i] = FTransform(PerInstanceSMData[i].Transform) * GetComponentTransform();
	}

#if WITH_EDITOR
	const bool bSkipWalkableSlopeOverride = bWalkableSlopeOverrideCachedGT;
#else
	constexpr bool bSkipWalkableSlopeOverride = false;
#endif
	InstancePhysicsBodies->CreateAll(WorldTransforms, bSkipWalkableSlopeOverride);
}
...
void UInstancedStaticMeshComponent::OnAsyncDestroyPhysicsStateBegin_GameThread(TSet<UObject*>& OutRootedObjects)
{
	USceneComponent::OnAsyncDestroyPhysicsStateBegin_GameThread(OutRootedObjects);

	// Call UnRegisterForCollisionEvents on game thread before going async.
	// TermBody skips UnRegisterForCollisionEvents when called from a worker thread (see FBodyInstance::TermBody contract).
...
#if WITH_EDITOR
void UInstancedStaticMeshComponent::OnAsyncCreatePhysicsStateBegin_GameThread(TSet<UObject*>& OutRootedObjects)
{
	// GT-side capture of WalkableSlopeOverride from the (potentially designer-mutable) BodySetup.
	// The worker step (UActorComponent::OnAsyncCreatePhysicsState fallthrough -> OnCreatePhysicsState
	// -> CreateAllInstanceBodies -> InstancePhysicsBodies->CreateAll) checks bWalkableSlopeOverrideCachedGT
	// and skips its own read.
	...
	bWalkableSlopeOverrideCachedGT = true;

	Super::OnAsyncCreatePhysicsStateBegin_GameThread(OutRootedObjects);
}
#endif

void UInstancedStaticMeshComponent::OnAsyncCreatePhysicsStateEnd_GameThread()
{
	...
	for (FBodyInstance* Body : *InstancePhysicsBodies)
	{
		if (Body)
		{
...
TArray<int32> UInstancedStaticMeshComponent::AddInstancesInternal(TConstArrayView<FTransform> InstanceTransforms, bool bShouldReturnIndices, bool bWorldSpace, bool bUpdateNavigation)
{
	...
	bool bNeedsNavigationRegistration = false;
	...
	for (const FTransform& InstanceTransform : InstanceTransforms)
	{
		...
		if (bHasPreviousTransforms)
		{
			// Copy in the current transform - it is somewhat redundant should probably add tracking instead to ensure it gets set, or pass in a struct that can initialize the whole instance 
			// (same with custom data where we store a mob of zeroes)
			PerInstancePrevTransform.Emplace(NewInstanceData.Transform);
		}
		...
		if (bNavigationUpdateRequired)
		{
			...
			if (GetInstanceCount() == 1)
			{
				bNeedsNavigationRegistration = true;
			}
	...
	if (bNavigationUpdateRequired)
	{
		if (bNeedsNavigationRegistration)
		{
			FNavigationSystem::RegisterComponent(*this);
		}
...
bool UInstancedStaticMeshComponent::RemoveInstanceInternal(int32 InstanceIndex, bool InstanceAlreadyRemoved, bool bForceRemoveAtSwap, const bool bUpdateNavigation)
{
	...
	// When InstanceAlreadyRemoved, the entry has already been removed from PerInstanceSMData
	// (e.g., by the property editor), so the index may be past the end of the already-shrunk
	// array. Account for this by checking against the original (pre-removal) count.
	const bool bValidInstance = InstanceAlreadyRemoved
		? (InstanceIndex >= 0 && InstanceIndex <= PerInstanceSMData.Num())
		: PerInstanceSMData.IsValidIndex(InstanceIndex);
	...
	if (bValidInstance)
	{
		...
		else if (NumCustomDataFloats > 0)
		{
			// When the instance was already removed (e.g., by the property editor), only PerInstanceSMData
			// was updated; PerInstanceSMCustomData still contains the stale entry and must be cleaned up.
			// Use ordered removal to match the ordered removal performed on PerInstanceSMData by the caller.
			PerInstanceSMCustomData.RemoveAt(InstanceIndex * NumCustomDataFloats, NumCustomDataFloats);
		}
	}

	if (bHasPreviousTransforms && PerInstancePrevTransform.IsValidIndex(InstanceIndex))
	{
		if (bUseRemoveAtSwap)
		{
	...
	if (bPhysicsStateCreated && InstancePhysicsBodies->IsValidIndex(InstanceIndex))
	{
		...
		{
			InstancePhysicsBodies->RemoveAt(InstanceIndex);
		}
	}
...
{
	if (PerInstanceSMData.IsValidIndex(InstanceBodyIndex))
	{
...
void UInstancedStaticMeshComponent::ForEachInstanceMatrix(TFunctionRef<void(const FMatrix&)> Func) const
{
...
void UInstancedStaticMeshComponent::SetupNewInstanceData(FInstancedStaticMeshInstanceData& InOutNewInstanceData, int32 InInstanceIndex, const FTransform& InInstanceTransform)
{
	...
	{
		const FTransform WorldTransform = InInstanceTransform * GetComponentTransform();
...
void UInstancedStaticMeshComponent::OnActorEnableCollisionChanged()
{
	Super::OnActorEnableCollisionChanged();
	
	for (FBodyInstance* InstanceBody : *InstancePhysicsBodies)
	{
		if (InstanceBody)
		{
			InstanceBody->UpdatePhysicsFilterData();
		}
	}
}
...
void UInstancedStaticMeshComponent::PostLoad()
{
	...
	if ((PerInstanceSMData.Num() * NumCustomDataFloats) != PerInstanceSMCustomData.Num() && PerInstanceSMData.Num() > 0)
	{
		...
		{
			UE_LOGF(LogStaticMesh, Warning, "%ls has %d instances, and %d NumCustomDataFloats, but no PerInstanceSMCustomData.  Allocating %d custom floats.", *GetFullName(), PerInstanceSMData.Num(), NumCustomDataFloats, PerInstanceSMData.Num() * NumCustomDataFloats);
		...
		else
		{
			...
			UE_LOGF(LogStaticMesh, Warning, "%ls has %d instances, and %d NumCustomDataFloats, but has %d PerInstanceSMCustomData.  Allocating %d custom floats to match.", *GetFullName(), PerInstanceSMData.Num(), NumCustomDataFloats, PerInstanceSMCustomData.Num(), PerInstanceSMData.Num() * NumCustomDataFloats);
...
{
	bool bHaveHit = InstancePhysicsBodies->LineTrace(OutHit, Start, End, Params.bTraceComplex, Params.bReturnPhysicalMaterial);
...
bool UInstancedStaticMeshComponent::ComponentOverlapComponentImpl(class UPrimitiveComponent* PrimComp, const FVector Pos, const FQuat& Quat, const FCollisionQueryParams& Params)
{
	...
	{
		UE_LOGF(LogCollision, Warning, "ComponentOverlapComponent : (%ls) Does not support InstancedStaticMesh with Physics Asset", *PrimComp->GetPathName());
		return false;
	}
...
void UInstancedStaticMeshComponent::GetResourceSizeEx(FResourceSizeEx& CumulativeResourceSize)
{
	...
	// component stuff
	CumulativeResourceSize.AddDedicatedSystemMemoryBytes(InstancePhysicsBodies->GetBodies().GetAllocatedSize());
	for (FBodyInstance* Body : *InstancePhysicsBodies)
	{
		if (Body != nullptr && Body->IsValidBodyInstance())
		{
			Body->GetBodyInstanceResourceSizeEx(CumulativeResourceSize);
		}
	}
...
void UInstancedStaticMeshComponent::SelectInstance(bool bInSelected, int32 InInstanceIndex, int32 InInstanceCount)
{
	...
	if (InInstanceCount > 0 && DeletionState == EInstanceDeletionReason::NotDeleting)
	{
		...
		// Reset the flags if all are false (this is also used to ensure per primitive selection works as intended since it relies on this being cleared when per instance selection is disabled).
		if (SelectedInstances.Find(true) == INDEX_NONE)
		{
