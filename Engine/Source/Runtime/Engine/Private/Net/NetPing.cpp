...
FNetPing::FOnAlternatePingAddressReceivedSignature FNetPing::OnAlternatePingAddressReceivedDelegate;
...
static int32 GNetPingMaxOverrideCount = 8;

static FAutoConsoleVariableRef CVarNetPingMaxOverrideCount(
	TEXT("net.NetPingMaxOverrideCount"), GNetPingMaxOverrideCount,
	TEXT("Specifies the maximum number of times the ping address may be overridden before being ignored"));
	...
	if (UNLIKELY(LastPingTypesCVar != PingTypesCVar))
	{
		...
		for (FString CurPingAndAvg : PingTypeList)
		{
			...
			if (CurPingAndAvg.Split(TEXT("="), &CurPingType, &CurPingAvg))
			{
				...
				{
					UE_LOGF(LogNet, Error, "FNetPing: Invalid 'net.NetPingTypes' entry: PingType: %ls (Valid: %i), "
							"PingAverageType: %ls (Valid: %i)", ToCStr(CurPingType), (int32)(PingTypeVal != INDEX_NONE),
							ToCStr(CurPingAvg), (int32)(PingAvgVal != INDEX_NONE))
				}
			}
			else
			{
				UE_LOGF(LogNet, Error, "FNetPing: Invalid 'net.NetPingTypes' entry '%ls' (Full CVar: %ls)",
	...
	if (EnumHasAnyFlags(PingTypes, PingType))
	{
			...
			CurStorageBase.PingTimeouts >= static_cast<uint32>(CVarNetPingTimeoutDisableThreshold.GetValueOnAnyThread()))
		{
			...
			if (PingType == EPingType::ICMP)
			{
				...
				UE_LOGF(LogNet, Warning, "FNetPing: ICMP ping failed with 100%% timeout after '%i' tries. Disabling. "
					"Ping address: %ls", CurStorageBase.PingTimeouts, ToCStr(FailureAddress));
			...
			else if (PingType == EPingType::UDPQoS)
			{
				...
				UE_LOGF(LogNet, Warning, "FNetPing: UDPQoS ping failed with 100%% timeout after '%i' tries. Disabling. "
					"Ping address: %ls", CurStorageBase.PingTimeouts, ToCStr(FailureAddress));
...
void FNetPing::TickRealtime(double CurTimeSeconds)
{
	...
	if (!!GNetPingDebugDump && (CurTimeSeconds - LastDebugPingDump) > 5.0)
	{
		...
		UE_LOGF(LogNet, Log, "FNetPing Debug Dump:");

		if (EnumHasAnyFlags(PingTypes, EPingType::RoundTrip))
		{
			FPingValues CurPingValues = GetPingValues(EPingType::RoundTrip);

			UE_LOGF(LogNet, Log, " - RoundTrip: Min: %f, Max: %f, Current: %f", CurPingValues.Min, CurPingValues.Max,
		...
		if (EnumHasAnyFlags(PingTypes, EPingType::RoundTripExclFrame))
		{
			...
			UE_LOGF(LogNet, Log, " - RoundTripExclFrame: Min: %f, Max: %f, Current: %f", CurPingValues.Min, CurPingValues.Max,
					CurPingValues.Current);
		}
		...
		if (EnumHasAnyFlags(PingTypes, EPingType::ICMP))
		{
			...
			UE_LOGF(LogNet, Log, " - ICMP: Min: %f, Max: %f, Current: %f, Timeouts: %i, Timeout Percent: %f",
		...
		if (EnumHasAnyFlags(PingTypes, EPingType::UDPQoS))
		{
			...
			UE_LOGF(LogNet, Log, " - UDP: Min: %f, Max: %f, Current: %f, Timeouts: %i, Timeout Percent: %f",
					CurPingValues.Min, CurPingValues.Max, CurPingValues.Current, CurStorageBase.PingTimeouts,
					TimeoutPercent);
		}
	}
				...
				if (MessageType == ENetPingControlMessage::SetPingAddress)
				{
					if (FNetPing* NetPing = Driver->ServerConnection->GetNetPing())
					{
						...
						if (MessageStr.ParseIntoArray(Args, TEXT("=")) == 2 && Args[0].IsNumeric())
						{
							...
							{
								NetPing->ServerSetPingAddress(PingType, PingAddress, ENetPingTargetEndpoint::DefaultEndpoint);
							}
						}
					}
				}
				else if (MessageType == ENetPingControlMessage::SetAlternatePingAddress)
				{
					if (FNetPing* NetPing = Driver->ServerConnection->GetNetPing())
					{
						TArray<FString> Args;
						...
						if (MessageStr.ParseIntoArray(Args, TEXT("=")) == 2 && Args[0].IsNumeric())
						{
							...
							if (IsValidPingType(PingType) && IsAddrValid(PingAddress))
							{
								NetPing->ServerSetPingAddress(PingType, PingAddress, ENetPingTargetEndpoint::AlternateEndpoint);

								if (NetPing->HasAlternatePingAddress(PingType))
								{
									OnAlternatePingAddressReceivedDelegate.Broadcast(Connection, PingType, PingAddress);
								}
							}
						}
					}
...
void FNetPing::ServerSetPingAddress(EPingType PingType, FString PingAddress, ENetPingTargetEndpoint TargetEndpoint /*= ENetPingTargetEndpoint::DefaultEndpoint*/)
{
	if (OverridePingAddressCount >= GNetPingMaxOverrideCount)
	{
		UE_LOGF(LogNet, Warning, "ServerSetPingAddress: OverridePingAddressCount %d exceeds GNetPingMaxOverrideCount: %d", OverridePingAddressCount, GNetPingMaxOverrideCount);
		return;
	}
	...
	if (EnumHasAnyFlags(PingTypes, PingType))
	{
		...
		if (PingType == EPingType::ICMP)
		{
			...
			{
				if (TargetEndpoint != ENetPingTargetEndpoint::DefaultEndpoint)
				{
					UE_LOGF(LogNet, Log, "ServerSetPingAddress: EPingType::ICMP PingAddress: %ls", ToCStr(PingAddress));
				...
				{
					UE_LOGF(LogNet, Warning, "ServerSetPingAddress: EPingType::ICMP does not currently support alternate ping endpoints");
				}
			}
			else
			{
				UE_LOGF(LogNet, Warning, "ServerSetPingAddress: EPingType::ICMP should not specify a port number: %ls",
						ToCStr(PingAddress));
			}
		}
		...
		else if (PingType == EPingType::UDPQoS)
		{
			if (PingAddress.Len() > 0)
			{
				...
				bool bUseAlternateAddress = (TargetEndpoint == ENetPingTargetEndpoint::AlternateEndpoint);

				FString & OverridePingAddress = bUseAlternateAddress ? AlternatePingUDPQoSAddress : OverridePingUDPQoSAddress;
				int32 & OverridePingPort = bUseAlternateAddress ? AlternatePingUDPQoSPort : OverridePingUDPQosPort;

				if (PingAddress.ParseIntoArray(IPPort, TEXT(":")) <= 2)
				{
					if (IPPort[0].Len() > 0)
					{
						OverridePingAddress = IPPort[0];
					}

					if (IPPort.Num() == 2 && IPPort[1].IsNumeric())
					{
						OverridePingPort = FCString::Atoi(ToCStr(IPPort[1]));
					}

					UE_LOGF(LogNet, Log, "ServerSetPingAddress: EPingType::UDPQoS PingAddress: %ls, PingPort: %i",
							ToCStr(OverridePingAddress), OverridePingPort);

					ResetPingValues(EPingType::UDPQoS);
				}
				else
				{
					UE_LOGF(LogNet, Warning, "ServerSetPingAddress: EPingType::UDPQoS PingAddress split into too many components: %ls",
							ToCStr(PingAddress));
				}
			}
		}
...
void FNetPing::UseAlternatePingAddress(EPingType PingType)
{
	if (PingType == EPingType::UDPQoS && !AlternatePingUDPQoSAddress.IsEmpty() &&
		EnumHasAnyFlags(PingTypes, PingType))
	{
		OverridePingUDPQoSAddress = AlternatePingUDPQoSAddress;

		if (AlternatePingUDPQoSPort != INDEX_NONE)
		{
			OverridePingUDPQosPort = AlternatePingUDPQoSPort;
		}

		UE_LOGF(LogNet, Log, "UseAlternatePingAddress: EPingType::UDPQoS PingAddress: %ls, PingPort: %i",
...
bool FNetPing::HasAlternatePingAddress(EPingType PingType) const
{
	if (PingType == EPingType::UDPQoS)
	{
		return !AlternatePingUDPQoSAddress.IsEmpty();
	}
