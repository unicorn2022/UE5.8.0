	...
	PreStartNextReplay(Params.PreStartNextPlaylistReplayDelegate)
{
	...
	for (int32 i = Playlist.Num() - 1; i >= 0; --i)
	{
		...
		if (Replay.RemoveFromStart(QuoteString) || Replay.RemoveFromEnd(QuoteString))
		{
			...
			UE_LOGF(LogDemo, Log, "FReplayPlaylistTracker: Removed mismatched quotes from %ls", *Params.Playlist[i]);
		}
		...
		if (Replay.Len() == 0)
		{
			UE_LOGF(LogDemo, Log, "FReplayPlaylistTracker: Removed invalid replay %ls", *Params.Playlist[i]);
	...
	if (Playlist.Num() == 0)
	{
		UE_LOGF(LogDemo, Warning, "FReplayPlaylistTracker: No valid replays in playlist.")
	}
}
	...
	if (Playlist.Num() > 0 && CurrentReplay < Playlist.Num())
	{
		...
		if (bStartedPlayingSuccessfully)
		{
			...
			if (UDemoNetDriver * LocalDemoNetDriver = World->GetDemoNetDriver())
			{
				...
				UE_LOGF(LogDemo, Log, "FReplayPlaylistTracker::Start: Successfully started Replay=%ls, CurrentReplayIdx=%d", *Playlist[CurrentReplay], CurrentReplay);
				return true;
			}
			else
			{
				UE_LOGF(LogDemo, Error, "FReplayPlaylistTracker::Start: Unable to retrieve DemoNetDriver, replay may be started, but playlist won't continue");
			}
		}
		else
		{
			UE_LOGF(LogDemo, Warning, "FReplayPlaylistTracker::Start: Failed to play replay.");
		}
	}
	...
	if (UGameInstance * LocalGameInstance = GameInstance.Get())
	{
		...
		if (CurrentReplay < Playlist.Num())
		{
			if (PreStartNextReplay.IsBound())
			{
				...
				{
					UE_LOGF(LogDemo, Log, "FReplayPlaylistTracker::PlayNextReplay: Updated World Override, NewWorldOverride=%ls, OldWorldOverride=%ls",
				...
				{
					UE_LOGF(LogDemo, Log, "FReplayPlaylistTracker::PlayNextReplay: Updated AdditionalOptions, NewOptions=[%ls], OldOptions=[%ls]",
