...
namespace UE::Net::Private
{
	...
	static FName ReceivedBunchTransactionName("UDataStreamChannel::ReceivedBunch");
}
...
void UDataStreamChannel::ReceivedBunch(FInBunch& Bunch)
{
	...
	{
		UE::RemoteExecutor::ExecuteTransactional(UE::Net::Private::ReceivedBunchTransactionName, [this, &SerializationContext]()
		{
...
void UDataStreamChannel::Tick()
{
	...
	const bool bWroteData = WriteData(EDataStreamWriteMode::Full);

#if UE_NET_IRIS_ALLOW_DEBUG_DATA
	// If we have debugdata, we write it as separate packets.
	if (DataStreamManager->HasDebugDataToSend() && !IsPacketWindowFull())
	{
		// Need to flush
		if (bWroteData)
		{
		...
		WriteData(EDataStreamWriteMode::DebugData);
	}
#endif
}

bool UDataStreamChannel::WriteData(UE::Net::EDataStreamWriteMode WriteMode)
{
	using namespace UE::Net;
	...
	const SIZE_T RecordCount = WriteRecords.Count();
	...
	check(Connection);
	UNetDriver* ConnDriver = Connection->GetDriver();
	UReplicationSystem* RepSystem = ConnDriver ? ConnDriver->GetReplicationSystem() : nullptr;
	ensure(RepSystem);

	//Get a NetStatsContext that is unique per-thread so locking NetStats is not required for the rest of this function
	UE::Net::FScopedNetStatsContext ScopedNetStatsContext(RepSystem);
	...
	auto WriteDataFunction = [this, &bNeedsPreSendFlush, &BeginWriteParams, MaxBunchBits, MaxBytes, &ScopedNetStatsContext]() -> UDataStream::EWriteResult
	{
		...
		SerializationContext.SetNetStatsContext(ScopedNetStatsContext.GetRawPtr());
		...
		const int32 PreSendBunchQueuedBits = Connection->QueuedBits + Connection->SendBuffer.GetNumBits();

		constexpr bool bAllowMerging = false;
		const FPacketIdRange PacketIds = SendBunch(&OutBunch, bAllowMerging);

		// Ugly trick to allow DebugData to not count towards bandwidth limit
		if (BeginWriteParams.WriteMode == EDataStreamWriteMode::DebugData)
		{
			Connection->QueuedBits = PreSendBunchQueuedBits - Connection->SendBuffer.GetNumBits();
		}
	...
	return WriteRecords.Count() != RecordCount;
}
...
{
	return FString("DataStream: ") + UChannel::Describe();
}
...
bool UDataStreamChannel::IsPacketWindowFull() const
{
	...
	{
		UE_LOGF(LogIris, Verbose, "Packet window full.");
		return true;
	}
