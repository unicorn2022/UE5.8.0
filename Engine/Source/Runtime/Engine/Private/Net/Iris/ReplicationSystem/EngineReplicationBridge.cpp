...
// Due to how it is setup in legacy replication system default is to replicate child subobjects before the actorcomponents
static bool bDefaultReplicateActorComponentSubobjectsAfterComponent = false;
static FAutoConsoleVariableRef CVarDefaultReplicateActorComponentSubobjectsAfterComponent(
		TEXT("net.Iris.DefaultReplicateActorComponentSubobjectsAfterComponent"),
		bDefaultReplicateActorComponentSubobjectsAfterComponent,
		TEXT("If set to true, ChildSubObjects of ActorComponents will replicate after component. Default is false.")
);

static int32 MaxNetRefHandleErrorDiagnosticMessages = 5;
static FAutoConsoleVariableRef CVarMaxNetRefHandleErrorDiagnosticMessages(
	TEXT("net.Iris.MaxNetRefHandleErrorDiagnosticMessages"),
	MaxNetRefHandleErrorDiagnosticMessages,
	TEXT("Maximum number of NMT_IrisNetRefHandleErrorWithDiagnosticMessage control messages a client may send to the server. Once exceeded the bridge falls back to NMT_IrisNetRefHandleError (no diagnostic string).")
);
...
{
	const UEngineReplicationBridge* EngineReplicationBridge = CastChecked<const UEngineReplicationBridge>(Bridge);
	if (!EngineReplicationBridge->AllowExecutePreReplication())
	{
		return;
	}
	
	UNetDriver* NetDriver = EngineReplicationBridge->GetNetDriver();
...
void UEngineReplicationBridge::Deinitialize()
{
	...
	bProtocolMismatchMessageSent = false;
	SentDiagnosticProtocolIds.Empty();
	NumSentDiagnosticMessages = 0;
}
...
UE::Net::FNetRefHandle UEngineReplicationBridge::StartReplicatingActor(AActor* Actor)
{
	...
	if (!AllowReplicationOfActorsWithAnyRole())
	{
	...
	// Need to call this to get any netfactory override
	...
	FNetRefHandle ActorRefHandle = StartReplicatingRootObject(Actor, FactoryId);

	if (!ActorRefHandle.IsValid())
	{
		const bool bIsFrontendProxy = (GetReplicationSystem()->GetProxyType() == EProxyType::Frontend);

		// If the replication system is a frontend proxy this condition can be encountered if attempting to start replicating
		// an actor that isn't found in the shared object reference cache (i.e. it wasn't replicated to a corresponding backend
		// proxy cache).
		UE_CLOGF(bIsFrontendProxy, LogIris, Warning, "Failed to create NetRefHandle for Actor Named %ls", ToCStr(Actor->GetName()));

		// If the replication system isn't a frontend proxy then this condition is unexpected and will ensure.
		ensureMsgf(bIsFrontendProxy, TEXT("Failed to create NetRefHandle for Actor Named %s"), ToCStr(Actor->GetName()));

		return FNetRefHandle::GetInvalid();
	}

    UE_CLOGF(Actor->bAlwaysRelevant, LogIris, Verbose, "StartReplicatingActor of AlwaysRelevant actor %ls", *PrintObjectFromNetRefHandle(ActorRefHandle));
	...
	AddRootObjectToContainerGroup(Actor, Actor->GetLevel());
...
UE::Net::FNetRefHandle UEngineReplicationBridge::StartReplicatingComponent(FNetRefHandle RootObjectHandle, UActorComponent* SubObject)
{
	...
	if (!ReplicatedComponentHandle.IsValid())
	{
		...
		// TODO: Add control over this from ActorComponent and support it from legacy replication system as well. JIRA UE-237066
		const FSubObjectReplicationParams Params
		{ 
			.RootObjectHandle = RootObjectHandle,
			.ChildSubObjectsReplicationOrder = UE::Net::Private::bDefaultReplicateActorComponentSubobjectsAfterComponent ? EChildSubObjectsReplicationOrder::AfterParent : EChildSubObjectsReplicationOrder::BeforeParent
		};
	...
	// Begin replication for any SubObjects registered by the component
	{
		...
		{
			.RootObjectHandle = RootObjectHandle,
			.ParentSubObjectHandle = ReplicatedComponentHandle,
		};
	...
	CancelDeferredComponent(SubObject);
...
void UEngineReplicationBridge::StopReplicatingActor(AActor* Actor, const FStopReplicatingActorParams& Params)
{
	...
	UE_LOGF(LogIris, Verbose, "StopReplicatingActor %ls. Reason %ls ", *PrintObjectFromNetRefHandle(RefHandle), LexToString(Params.StopReplicatingReason));
...
EEndReplicationFlags UEngineReplicationBridge::ConvertToEndReplicationFlags(UE::Net::EStopReplicatingReason StopReplicatingReason) const
{
	...
	if (StopReplicatingReason == EStopReplicatingReason::Destroyed || StopReplicatingReason == EStopReplicatingReason::ProxyReuse)
	{
	...
	if (StopReplicatingReason == EStopReplicatingReason::ProxyReuse)
	{
		EndReplicationFlags |= EEndReplicationFlags::ProxyReuse;
	}
...
void UEngineReplicationBridge::StopReplicatingComponent(UActorComponent* ActorComponent, EEndReplicationFlags EndReplicationFlags)
{
	...
	CancelDeferredComponent(ActorComponent);

	FNetRefHandle ComponentHandle = GetReplicatedRefHandle(ActorComponent, EGetRefHandleFlags::EvenIfGarbage);
	if (ComponentHandle.IsValid())
	{
		UE_LOGF(LogIris, Verbose, "StopReplicatingComponent for %ls %ls.", *GetNameSafe(ActorComponent), *ComponentHandle.ToString());
...
void UEngineReplicationBridge::SetNetDriver(UNetDriver* const InNetDriver)
{
	...
	if (InNetDriver != nullptr)
	{
		...
		{
			const bool bRequiredChannelIsConfigured = InNetDriver->ChannelDefinitions.FindByPredicate(
				[RequiredChannelName](const FChannelDefinition& rhs)
				{
		...
		bAllowExecutePreReplication = InNetDriver->IsPreReplicationEnabled();
		bAllowReplicationOfActorsWithAnyRole = InNetDriver->IsReplicationOfActorsWithAnyRoleEnabled();
	}
}
...
bool UEngineReplicationBridge::ObjectContainerHasFinishedLoading(UObject* Object) const
{
...
void UEngineReplicationBridge::StartReplicatingActorDefer(AActor* Instance)
{
	if (bIsFlushDeferredRunning)
	{
		UE_LOGF(LogIris, Warning, "Cannot call UEngineReplicationBridge::StartReplicatingActorDefer() for %ls while flushing.", *GetNameSafe(Instance));
		return;
	}

	FDeferredStartStop* Def = DeferredStartStopCallsActors.Find(Instance);
	if (Def == nullptr)
	{
		// If this is the first call for this actor make sure it hasn't already started replication so it can be started.
		const bool bCanCallStart = !GetReplicatedRefHandle(Instance).IsValid();
		if (!bCanCallStart)
		{
			UE_LOGF(LogIris, Warning, "UEngineReplicationBridge::StartReplicatingActorDefer: First call for %ls failed because it's already replicating.", *GetNameSafe(Instance));
			return;
		}

		Def = &DeferredStartStopCallsActors.Add(Instance);
	}

	check(Def);

	const int32 NewCount = (Def->Count + 1);
	
	// Prevent deferring multiple, consecutive calls to start replication for the same actor.
	if (NewCount <= 1)
	{
		Def->Count = NewCount;
		UE_LOGF(LogIris, Verbose, "UEngineReplicationBridge::StartReplicatingActorDefer: %ls", *GetNameSafe(Instance));
	}
	else
	{
		UE_LOGF(LogIris, Warning, "UEngineReplicationBridge::StartReplicatingActorDefer: Already deferred this call for %ls", *GetNameSafe(Instance));
	}
}

void UEngineReplicationBridge::StopReplicatingActorDefer(AActor* Instance, const FStopReplicatingActorParams& Params)
{
	if (bIsFlushDeferredRunning)
	{
		UE_LOGF(LogIris, Warning, "Cannot call UEngineReplicationBridge::StopReplicatingActorDefer() for %ls while flushing.", *GetNameSafe(Instance));
		return;
	}
	...
	if (Def == nullptr)
	{
		// If this is the first call for this actor make sure it's already started replication so it can be stopped.
		const bool bCanCallStop = GetReplicatedRefHandle(Instance).IsValid();
		if (!bCanCallStop)
		{
			UE_CLOGF(Instance->GetIsReplicated(), LogIris, Warning, "UEngineReplicationBridge::StopReplicatingActorDefer: First call for %ls failed because it's already not replicating.", *GetNameSafe(Instance));
			return;
		}
	...
	// Prevent deferring multiple, consecutive calls to stop replication for the same actor.
	if (NewCount >= -1)
	{
		Def->Count = NewCount;
		Def->StopReplicatingActorParams = Params;
		UE_LOGF(LogIris, Verbose, "UEngineReplicationBridge::StopReplicatingActorDefer: %ls", *GetNameSafe(Instance));
	}
	else
	{
		UE_LOGF(LogIris, Warning, "UEngineReplicationBridge::StopReplicatingActorDefer: Already deferred this call for %ls", *GetNameSafe(Instance));
	}
}

void UEngineReplicationBridge::StartReplicatingComponentDefer(const UObject* RootObject, UActorComponent* Component)
{
	if (bIsFlushDeferredRunning)
	{
		UE_LOGF(LogIris, Warning, "Cannot call UEngineReplicationBridge::StartReplicatingComponentDefer() for %ls while flushing.", *GetNameSafe(Component));
		return;
	}

	FDeferredStartStop* Def = DeferredStartStopCallsComponents.Find(Component);
	if (Def == nullptr)
	{
		// If this is the first call for this component, make sure it hasn't already started replication so it can be started.
		const bool bCanCallStart = !GetReplicatedRefHandle(Component).IsValid();
		if (!bCanCallStart)
		{
			UE_LOGF(LogIris, Warning, "UEngineReplicationBridge::StartReplicatingComponentDefer: First call for %ls failed because it's already replicating.", *GetNameSafe(Component));
			return;
		}

		Def = &DeferredStartStopCallsComponents.Add(Component);
	}
	...
	// Prevent deferring multiple, consecutive calls to start replication for the same component.
	if (NewCount <= 1)
	{
		Def->Count = NewCount;
		Def->RootObject = RootObject;
		UE_LOGF(LogIris, Verbose, "UEngineReplicationBridge::StartReplicatingComponentDefer: %ls", *GetNameSafe(Component));
	}
	else
	{
		UE_LOGF(LogIris, Warning, "UEngineReplicationBridge::StartReplicatingComponentDefer: Already deferred this call for %ls", *GetNameSafe(Component));
	}
}

void UEngineReplicationBridge::StopReplicatingComponentDefer(UActorComponent* Component, EEndReplicationFlags EndReplicationFlags)
{
	if (bIsFlushDeferredRunning)
	{
		UE_LOGF(LogIris, Warning, "Cannot call UEngineReplicationBridge::StopReplicatingComponentDefer() for %ls while flushing.", *GetNameSafe(Component));
		return;
	}
	...
	if (Def == nullptr)
	{
		// If this is the first call for this component, make sure it's already started replication so it can be stopped.
		const bool bCanCallStop = GetReplicatedRefHandle(Component).IsValid();
		if (!bCanCallStop)
		{
			UE_CLOGF(Component->GetIsReplicated(), LogIris, Warning, "UEngineReplicationBridge::StopReplicatingComponentDefer: First call for %ls failed because it's already not replicating.", *GetNameSafe(Component));
			return;
		}
	...
	// Prevent deferring multiple, consecutive calls to stop replication for the same component.
	if (NewCount >= -1)
	{
		Def->Count = NewCount;
		Def->StopReplicatingComponentFlags = EndReplicationFlags;
		UE_LOGF(LogIris, Verbose, "UEngineReplicationBridge::StopReplicatingComponentDefer: %ls", *GetNameSafe(Component));
	}
	else
	{
		UE_LOGF(LogIris, Warning, "UEngineReplicationBridge::StopReplicatingComponentDefer: Already deferred this call for %ls", *GetNameSafe(Component));
	}
}

void UEngineReplicationBridge::FlushDeferred()
{
	bIsFlushDeferredRunning = true;

	if (DeferredStartStopCallsActors.Num() > 0 || DeferredStartStopCallsComponents.Num() > 0)
	{
		UE_LOGF(LogIris, Verbose, "UEngineReplicationBridge::FlushDeferred: Actors=%d Components=%d", DeferredStartStopCallsActors.Num(), DeferredStartStopCallsComponents.Num());
	}

	// Actor replication needs to be processed before component replication in case the component depends
	// on an owning actor that hasn't started replicating yet.

	for (TPair<TObjectKey<AActor>, FDeferredStartStop>& Pair : DeferredStartStopCallsActors)
	{
		TObjectKey ActorKey = Pair.Key;
		FDeferredStartStop& Def = Pair.Value;

		// No start or stop was called, or the number of start and stop calls cancel out (i.e. do nothing).
		if (Def.Count == 0)
		{
			continue;
		}
		...
		// Call UEngineReplicationBridge::StartReplicatingActor()
		if (Def.Count > 0)
		{
			UE::Net::FNetRefHandle NewNetRefHandle = StartReplicatingActor(Actor);
			if (NewNetRefHandle.IsValid())
			{
				Actor->OnReplicationStartedForIris(AActor::FOnReplicationStartedParams());
			}
				
			UE_LOGF(LogIris, Verbose, "UEngineReplicationBridge::FlushDeferred: Called StartReplicatingActor for %ls (success=%d)", *Actor->GetName(), NewNetRefHandle.IsValid());
		}
				
		// Call UEngineReplicationBridge::StopReplicatingActor()
		else if (Def.Count < 0)
		{
			// Cache locally in case the DeferredStartStopCallsActors map is changed as a side-effect of the remaining logic.
			FStopReplicatingActorParams StopReplicatingActorParams = Def.StopReplicatingActorParams;

			AActor::FOnStopReplicationParams OnStopReplicationParams = { .StopReplicatingParams = StopReplicatingActorParams };
			Actor->OnStopReplicationForIris(OnStopReplicationParams);
			StopReplicatingActor(Actor, StopReplicatingActorParams);

			UE_LOGF(LogIris, Verbose, "UEngineReplicationBridge::FlushDeferred: Called StopReplicatingActor for %ls", *Actor->GetName());
		}
	}

	for (TPair<TObjectKey<UActorComponent>, FDeferredStartStop>& Pair : DeferredStartStopCallsComponents)
	{
		TObjectKey<UActorComponent> ActorComponentKey = Pair.Key;
		FDeferredStartStop& Def = Pair.Value;
		...
		UActorComponent* ActorComponent = ActorComponentKey.ResolveObjectPtr();
		if (!ActorComponent)
		{
		...
		// Call UEngineReplicationBridge::StartReplicatingComponent()
		if (Def.Count > 0)
		{
			UObject* RootObject = Def.RootObject.ResolveObjectPtr();
			if (RootObject)
			{
				FNetRefHandle RootObjectHandle = GetReplicatedRefHandle(RootObject);
				if (!RootObjectHandle.IsValid())
				{
					UE_LOGF(LogIris, Verbose, "UEngineReplicationBridge::FlushDeferred: Cannot find handle for component's root object %ls", *RootObject->GetName());
					continue;
				}

				UE::Net::FNetRefHandle NewNetRefHandle = StartReplicatingComponent(RootObjectHandle, ActorComponent);
				if (NewNetRefHandle.IsValid())
				{
					ActorComponent->OnReplicationStartedForIris(UActorComponent::FOnReplicationStartedParams());
				}

				UE_LOGF(LogIris, Verbose, "UEngineReplicationBridge::FlushDeferred: Called StartReplicatingComponent for %ls (success=%d)", *ActorComponent->GetName(), NewNetRefHandle.IsValid());
			}
		}

		// Call UEngineReplicationBridge::StopReplicatingComponent()
		else if (Def.Count < 0)
		{
			ActorComponent->OnStopReplicationForIris(UActorComponent::FOnStopReplicationParams());
			StopReplicatingComponent(ActorComponent, Def.StopReplicatingComponentFlags);

			UE_LOGF(LogIris, Verbose, "UEngineReplicationBridge::FlushDeferred: Called StopReplicatingComponent for %ls", *ActorComponent->GetName());
		}
	}

	bIsFlushDeferredRunning = false;

	DeferredStartStopCallsActors.Reset();
	DeferredStartStopCallsComponents.Reset();
}

void UEngineReplicationBridge::OnProtocolMismatchDetected(FNetRefHandle ObjectHandle, const TArray<uint8>& CDOStateBytes)
{
	Super::OnProtocolMismatchDetected(ObjectHandle, CDOStateBytes);
	...
	if (NetDriver && NetDriver->ServerConnection)
	{
		...
		if (!bProtocolMismatchMessageSent && CDOStateBytes.Num() > 0)
		{
			// We must const_cast because Send requires non-const. CDOStateBytes is NOT modified.
			FNetControlMessage<NMT_IrisProtocolMismatchWithCDOState>::Send(NetDriver->ServerConnection, RawHandleId, const_cast<TArray<uint8>&>(CDOStateBytes));
			bProtocolMismatchMessageSent = true;
		}
		else
		{
...
void UEngineReplicationBridge::OnProtocolMismatchReported(FNetRefHandle RefHandle, uint32 ConnectionId, const TArray<uint8>& ClientCDOStateBytes)
{
	Super::OnProtocolMismatchReported(RefHandle, ConnectionId, ClientCDOStateBytes);
	...
	if (NetDriver && NetDriver->IsServer())
	{
		...
		if (IsClassCritical(ObjectClass))
		{
			if (UNetConnection* ClientConnection = NetDriver->GetConnectionByHandle(UE::Net::FConnectionHandle(ConnectionId)))
			{
				...
				UE_LOGF(LogIris, Error, "%ls: Closing connection due to: %ls", ToCStr(ClientConnection->Describe()), ToCStr(ErrorMsg));
				{
...
void UEngineReplicationBridge::SendErrorWithNetRefHandle(UE::Net::ENetRefHandleError ErrorType, FNetRefHandle RefHandle, uint32 ConnectionId, TConstArrayView<const FNetRefHandle> ExtraNetRefHandle, const UE::Net::FNetErrorContext& ErrorContext)
{
	if (NetDriver)
	{
		if (UNetConnection* ClientConnection = NetDriver->GetConnectionByHandle(UE::Net::FConnectionHandle(ConnectionId)))
		{
			...
			const FString& DiagnosticMessage = ErrorContext.GetErrorDiagnostic();
			const UE::Net::FReplicationProtocolIdentifier ProtocolId = ErrorContext.GetErrorProtocolId();

			const bool bAlreadySentForProtocol = SentDiagnosticProtocolIds.Contains(ProtocolId);
			const bool bGlobalCapReached = NumSentDiagnosticMessages >= UE::Net::Private::MaxNetRefHandleErrorDiagnosticMessages;

			if (DiagnosticMessage.IsEmpty() || bAlreadySentForProtocol || bGlobalCapReached)
			{
				FNetControlMessage<NMT_IrisNetRefHandleError>::Send(ClientConnection, ErrorType, HandleId, ArrayIds);
			}
			else
			{
				FNetControlMessage<NMT_IrisNetRefHandleErrorWithDiagnosticMessage>::Send(ClientConnection, ErrorType, HandleId, ArrayIds, const_cast<FString&>(DiagnosticMessage));
				SentDiagnosticProtocolIds.Add(ProtocolId);
				++NumSentDiagnosticMessages;
			}
		}
		else
		{
			UE_LOGF(LogIris, Error, "UEngineReplicationBridge::SendErrorWithNetRefHandle could not find Connection for id:%u", ConnectionId);
		}
	}
}
...
void UEngineReplicationBridge::ActorChangedLevel(const AActor* Actor, const ULevel* PreviousLevel)
{
	...
	if (PreviousLevel)
	{
		...
		UE::Net::FNetObjectGroupHandle PreviousContainerGroup = GetContainerGroup(PreviousLevel);
		if (GetReplicationSystem()->IsInGroup(PreviousContainerGroup, ActorRefHandle))
		{
			UE_LOG_BRIDGEID(LogIris, Verbose, TEXT("ActorChangedLevel: removing %s from GroupIndex: %u PreviousLevel: %s"), *ActorRefHandle.ToString(), PreviousContainerGroup.GetGroupIndex(), ToCStr(PreviousLevelPackageName.ToString()));
			GetReplicationSystem()->RemoveFromGroup(PreviousContainerGroup, ActorRefHandle);
		}
		else if (PreviousContainerGroup.IsValid())
		{
			UE_LOG_BRIDGEID(LogIris, Warning, TEXT("ActorChangedLevel: %s not found in GroupIndex: %u PreviousLevel: %s"), *ActorRefHandle.ToString(), PreviousContainerGroup.GetGroupIndex(), ToCStr(PreviousLevelPackageName.ToString()));
		}
	}
...
{
	AddRootObjectToContainerGroup(RootObject, Level);
}
PRAGMA_ENABLE_DEPRECATION_WARNINGS

void UEngineReplicationBridge::AddRootObjectToContainerGroup(const UObject* RootObject, const UObject* Container)
{
	...
	if (const ULevel* Level = Cast<ULevel>(Container))
	{
		if (UE::Net::Private::bAlwaysCreateLevelFilteringGroupsForPersistentLevels || !Level->IsPersistentLevel() || (Level != NetDriver->GetWorld()->PersistentLevel))
		{
			const UPackage* const LevelPackage = Level->GetOutermost();
			const FString PackageName = LevelPackage->GetName();

			UE::Net::FNetObjectGroupHandle LevelGroup = GetContainerGroup(Level);
			if (!LevelGroup.IsValid())
			{
				...
				LevelGroup = CreateContainerGroup(Level, *LevelGroupPackageUniqueName);
	...
	else if (const UPackage* Package = Cast<UPackage>(Container))
	{
		UE::Net::FNetObjectGroupHandle ContainerGroup = GetContainerGroup(Package);

		if (!ContainerGroup.IsValid())
		{
			const FName ContainerName = Package->GetFName();
			ContainerGroup = CreateContainerGroup(Package, ContainerName);

			UE_LOG_BRIDGEID(LogIris, Verbose, TEXT("Created new GroupIndex: %u for Level: %s using Package Unique Name: %s"), ContainerGroup.GetGroupIndex(), ToCStr(ContainerName.ToString()), ToCStr(ContainerName.ToString()));
		}

		const UE::Net::FNetRefHandle RootObjectRefHandle = GetReplicatedRefHandle(RootObject);

		// Add object to group
		UE_LOG_BRIDGEID(LogIris, Verbose, TEXT("Added %s to GroupIndex: %u Container: %s"), *RootObjectRefHandle.ToString(), ContainerGroup.GetGroupIndex(), ToCStr(Package->GetPackage()->GetName()));
		GetReplicationSystem()->AddToGroup(ContainerGroup, RootObjectRefHandle);
	}
	else
	{
		ensureMsgf(false, TEXT("Iris requires replicated container group to be a ULevel or a UPackage, Container: %s"), ToCStr(GetFullNameSafe(Container)));
	}
}
...
bool UEngineReplicationBridge::AllowExecutePreReplication() const
{
	return bAllowExecutePreReplication;
}

bool UEngineReplicationBridge::AllowReplicationOfActorsWithAnyRole() const
{
	return bAllowReplicationOfActorsWithAnyRole;
}

void UEngineReplicationBridge::CancelDeferredActor(AActor* Actor)
{
	if (bIsFlushDeferredRunning)
	{
		return;
	...
	UE_CLOGF(DeferredStartStopCallsActors.Contains(Actor), LogIris, Verbose, "UEngineReplicationBridge::CancelDeferredActor: Cancelling deferred %ls.", *GetNameSafe(Actor));
	DeferredStartStopCallsActors.Remove(Actor);
}

void UEngineReplicationBridge::CancelDeferredComponent(UActorComponent* ActorComponent)
{
	if (bIsFlushDeferredRunning)
	{
		return;
	...
	UE_CLOGF(DeferredStartStopCallsComponents.Contains(ActorComponent), LogIris, Verbose, "UEngineReplicationBridge::CancelDeferredComponent: Cancelling deferred %ls.", *GetNameSafe(ActorComponent));
	DeferredStartStopCallsComponents.Remove(ActorComponent);
}