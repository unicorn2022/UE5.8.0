...
namespace UE::Net::Private
{
	...
	static FName NetActorFactoryName("NetActorFactory");
...
void UNetActorFactory::FillRootObjectReplicationParams(const UE::Net::FRootObjectReplicationParamsContext& Context, UE::Net::FRootObjectReplicationParams& OutParams)
{
	using namespace UE::Net;

	AActor* Actor = Cast<AActor>(Context.Object);
	if (!Actor)
	{
		UE_LOGF(LogIris, Error, "UNetActorFactory only supports AActor derived classes. Got %ls", ToCStr(GetNameSafe(Context.Object ? Context.Object->GetClass() : nullptr)));
		ensureMsgf(false, TEXT("UNetActorFactory only supports AActor derived classes. Got %s"), ToCStr(GetNameSafe(Context.Object ? Context.Object->GetClass() : nullptr)));
		return;
	}

	AActor::FFillReplicationParamsContext ActorParamsContext;
	FActorReplicationParams ActorParams;
	Actor->FillReplicationParams(ActorParamsContext, ActorParams);

	OutParams.bNeedsPreUpdate = true,
		OutParams.bNeedsWorldLocationUpdate = true,
		OutParams.bIsDormant = Actor->NetDormancy > DORM_Awake,
		OutParams.StaticPriority = (Actor->bAlwaysRelevant || Actor->bOnlyRelevantToOwner) ? Actor->NetPriority : 0.0f,
		OutParams.PollFrequency = Actor->GetNetUpdateFrequency();

#if !UE_BUILD_SHIPPING
	ensureMsgf(!(Actor->bAlwaysRelevant || Actor->bOnlyRelevantToOwner) || OutParams.StaticPriority >= 1.0f, TEXT("Very low NetPriority %.02f for always relevant or owner relevant Actor %s. Set it to 1.0f or higher."), Actor->NetPriority, ToCStr(Actor->GetName()));
#endif
	...
	if (ActorParams.FilterType != FActorReplicationParams::ConfigFilter)
	{
		...
		case FActorReplicationParams::DefaultSpatial:
			OutParams.ExplicitDynamicFilterName = Bridge->GetDefaultSpatialFilterName();
			break;
		...
		case FActorReplicationParams::ExplicitFilter:
			...
			OutParams.ExplicitDynamicFilterName = ActorParams.ExplicitDynamicFilterName;
			break;
		}
	}
}
...
UNetObjectFactory::FInstantiateResult UNetActorFactory::InstantiateReplicatedObjectFromHeader(const FInstantiateContext& Context, const UE::Net::FNetObjectCreationHeader* Header)
{
	...
	if (!BaseHeader->IsDynamic())
	{
		...
		if (!Actor)
		{
			...
			{
				const FString FailedObjectReferenceStr = ActorBridge->DescribeObjectReference(StaticHeader->ObjectReference, Context.ResolveContext);
				UE_LOGF(LogIris, Error, "UNetActorFactory::InstantiateNetObjectFromHeader Failed to resolve ObjectReference: %ls . Could not find static actor.", *FailedObjectReferenceStr);
				InstantiateResult.FailureDiagnosticMessage = FString::Printf(TEXT("Static actor not found. ObjectReference: %s"), *FailedObjectReferenceStr);
			}
			return InstantiateResult;
		}

		UE_LOGF(LogIris, Verbose, "UNetActorFactory::InstantiateNetObjectFromHeader Found static Actor: %ls using ObjectReference: %ls", ToCStr(Actor->GetPathName()), *ActorBridge->DescribeObjectReference(StaticHeader->ObjectReference, Context.ResolveContext));
	...
	if (BaseHeader->IsPreregistered())
	{
		...
		if (!FoundActor)
		{
			...
			if (UE_LOG_ACTIVE(LogIris, Error))
			{
				UE_LOGF(LogIris, Error, "UNetActorFactory::InstantiateReplicatedObjectFromHeader Unable to find pre-registered actor: %ls", *Context.Handle.ToString());
				InstantiateResult.FailureDiagnosticMessage = FString::Printf(TEXT("Pre-registered actor not found. Handle: %s"), *Context.Handle.ToString());
			}
			return InstantiateResult;
		}
	...
	if (!Archetype)
	{
		...
		if (UE_LOG_ACTIVE(LogIris, Error))
		{
			const FString FailedArchetypeReferenceStr = Bridge->DescribeObjectReference(DynamicHeader->ArchetypeReference, Context.ResolveContext);
			UE_LOGF(LogIris, Error, "UNetActorFactory::InstantiateNetObjectFromHeader Unable to instantiate actor, failed to resolve archetype: %ls", *FailedArchetypeReferenceStr);
			InstantiateResult.FailureDiagnosticMessage = FString::Printf(TEXT("Actor archetype not found. Archetype: %s"), *FailedArchetypeReferenceStr);
		}
		return InstantiateResult;
	}
	...
	if (!Actor)
	{
		...
		if (UE_LOG_ACTIVE(LogIris, Error))
		{
			const FString ArchetypeNameStr = GetNameSafe(Archetype);
			const FString ObjectReferenceStr = Bridge->DescribeObjectReference(DynamicHeader->ArchetypeReference, Context.ResolveContext);
			ensureMsgf(Actor, TEXT("UNetActorFactory::InstantiateNetObjectFromHeader SpawnActor failed. Used Archetype: %s ObjectReference: %s"), *ArchetypeNameStr, *ObjectReferenceStr);
			InstantiateResult.FailureDiagnosticMessage = FString::Printf(TEXT("SpawnActor failed. Archetype: %s, ObjectReference: %s"), *ArchetypeNameStr, *ObjectReferenceStr);
		}
		else
		{
			// We still want to ensure if the LogIris Error is disabled, but only construct the strings if ensures are enabled.
			ensureMsgf(Actor, TEXT("UNetActorFactory::InstantiateNetObjectFromHeader SpawnActor failed. Used Archetype: %s ObjectReference: %s"), *GetNameSafe(Archetype), *Bridge->DescribeObjectReference(DynamicHeader->ArchetypeReference, Context.ResolveContext));
		}
		return InstantiateResult;
	}
	...
	UE_LOGF(LogIris, Verbose, "UNetActorFactory::InstantiateNetObjectFromHeader Spawned Actor: %ls using Archetype: %ls", *Actor->GetPathName(), *GetNameSafe(Archetype));
...
void UNetActorFactory::PostInstantiation(const FPostInstantiationContext& Context)
{
	...
	// OnActorChannelOpen
	{
		...
		{
			UE_LOGF(LogIris, Error, "UNetActorFactory::PostInstantiation deserialization error in OnActorChannelOpen for Actor: %ls", *Actor->GetPathName());
			check(false);
			return;
		}
	}
...
void UNetActorFactory::SubObjectDetachedFromReplication(const FDetachContext& Context, const FSubObjectDetachContext& SubObjectContext)
{
	using namespace UE::Net;
	...
	const bool bWillBeDestroyed = !bIsStable;
	...
	if (AActor* ActorOwner = Cast<AActor>(SubObjectContext.RootObject))
	{
		ActorOwner->OnSubobjectDestroyFromReplication(Context.DetachedInstance);
	}
}

void UNetActorFactory::DetachedFromReplication(const FDetachContext& Context, const TOptional<FSubObjectDetachContext>& SubObjectContext)
{
	using namespace UE::Net;

	AActor* Actor = Cast<AActor>(Context.DetachedInstance);
	if (!Actor)
	{
		// Nothing to do since the actor instance has been detached already
		return;
	}
	...
	const bool bIsDynamic = !Actor->IsNameStableForNetworking();
	const bool bCanBeDestroyed = (bIsDynamic || Context.Reason == EDetachReason::StaticDestroyed) && NetDriver->ShouldClientDestroyActor(Actor);

	if (Context.Reason == EDetachReason::TornOff)
	{
		if (!NetDriver->ShouldClientDestroyTearOffActors())
		{
			NetDriver->ClientSetActorTornOff(Actor);
		}
		else if (bCanBeDestroyed)
		{
...
TOptional<UNetActorFactory::FWorldInfoData> UNetActorFactory::GetWorldInfo(const FWorldInfoContext& Context) const
{
	...
	if (!Actor)
	{
		...
		UE_LOGF(
			LogIris,
			VeryVerbose,
			"UNetActorFactory::GetWorldInfo requested for non-existent actor (likely destroyed): %ls | NetRefHandle: %ls",
