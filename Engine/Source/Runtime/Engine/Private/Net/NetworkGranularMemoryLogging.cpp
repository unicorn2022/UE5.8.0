...
namespace UE::Net::Private::GranularMemoryTracking
{
	...
	struct FNetworkMemoryTrackingScope
	{
		...
		void GenerateRows(const FString& Prefix, TArray<FString>& OutRows)
		{
			...
			static const FString CountName = "Count";
	...
	struct FNetworkMemoryTrackingScopeStack
	{
		...
		{
			static const FString UnknownScopeName("UNKNOWN");
	...
	{
		return CVarUseGranularNetworkTracking.GetValueOnAnyThread() && Ar.IsCountingMemory() && FString("FArchiveCountMem").Equals(Ar.GetArchiveName());
	}
	...
	void FScopeMarker::LogCustomWork(const FString& WorkName, const uint64 Bytes) const
	{
		if (ScopeStack)
		{
			// UE_LOGF(LogNet, Log, "%ls: %ls is %llu bytes", *ScopeName, *WorkName, Bytes);
