...
bool FNetworkCongestionControl::IsReadyToSend(double Timestamp) const
{
	...
	UE_LOGF(LogNetTraffic, VeryVerbose,
		"BytesInFlight: %d, PacketsInFlight: %d, CongestionWindow: %f, MinRTT: %f",
...
void FNetworkTrafficAnalyzer::OnSend(const FSeqSample& SeqSample)
{
	...
	{
		UE_LOGF(LogNet, Error, "Sending out of order packet, expected: %d, actual: %d", (OutSeq + SeqT(1)).Get(), Seq.Get());
		return;
	}
	...
	UE_LOGF(LogNet, VeryVerbose, "FNetworkTrafficAnalyzer::OnSend, BytesInFlight: %d, "
...
void FNetworkTrafficAnalyzer::OnAck(const FAckSample& AckSample)
{
	...
	UE_LOGF(LogNet, VeryVerbose, "FNetworkTrafficAnalyzer::OnAck, Ack: %d, AckNum: %d",
		Ack.Get(), AckNum);
	...
	UE_LOGF(LogNet, VeryVerbose, "FNetworkTrafficAnalyzer::OnAck, BytesInFlight: %d, "
