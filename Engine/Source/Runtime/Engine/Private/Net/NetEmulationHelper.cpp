...
FConsoleCommandWithWorldArgsAndOutputDeviceDelegate::CreateStatic([](const TArray<FString>& Args, UWorld* World, FOutputDevice& Output)
{
	...
	NetDriver->SendRPCDel.BindLambda([DropPercentage](AActor* Actor, UFunction* Function, void* Parameters, FOutParmRec* OutParms, FFrame* Stack, UObject* SubObject, bool& bOutBlockSendRPC)
	{
		if ((Function->FunctionFlags & FUNC_NetReliable) == 0)
		{
			...
			if (SubObject)
			{
				UE_LOGF(LogNetTraffic, Log, "      Dropped unreliable RPC %ls::%ls : %ls", *GetFullNameSafe(Actor), *GetNameSafe(SubObject), *GetNameSafe(Function));
			}
			else
			{
				UE_LOGF(LogNetTraffic, Log, "      Dropped unreliable RPC %ls : %ls", *GetFullNameSafe(Actor), *GetNameSafe(Function));
			}
		}
	});

	UE_LOGF(LogNetTraffic, Warning, "Will start dropping %.2f%% of all unreliable RPCs", DropPercentage);
}));
...
FConsoleCommandWithWorldArgsAndOutputDeviceDelegate::CreateStatic([](const TArray<FString>& Args, UWorld* World, FOutputDevice& Output)
{
	...
	if (Args.Num() < 1)
	{
		UE_LOGF(LogNet, Warning, "No class name parameter passed to NetEmulation.DropUnreliableOfActorClass");
		return;
	}
	...
	UE_LOGF(LogNetTraffic, Warning, "Will start dropping %.2f%% of all unreliable RPCs of actors of class: %ls", DropPercentage, *ClassNameParam);
}));
...
FConsoleCommandWithWorldArgsAndOutputDeviceDelegate::CreateStatic([](const TArray<FString>& Args, UWorld* World, FOutputDevice& Output)
{
	...
	if (Args.Num() < 1)
	{
		UE_LOGF(LogNet, Warning, "No RPC name parameter passed to NetEmulation.DropUnreliableRPC");
		return;
	}
	...
	UE_LOGF(LogNetTraffic, Warning, "Will start dropping %.2f%% of all unreliable RPCs named: %ls", DropPercentage, *RPCNameParam);
}));
...
FConsoleCommandWithWorldArgsAndOutputDeviceDelegate::CreateStatic([](const TArray<FString>& Args, UWorld* World, FOutputDevice& Output)
{
	...
	if (Args.Num() < 1)
	{
		UE_LOGF(LogNet, Warning, "No SubObject name parameter passed to NetEmulation.DropUnreliableOfSubObjectClass");
		return;
	}
	...
	UE_LOGF(LogNetTraffic, Warning, "Will start dropping %.2f%% of all unreliable RPCs for subobjects: %ls", DropPercentage, *SubObjectClassNameParam);
}));
...
bool FPacketSimulationSettings::LoadEmulationProfile(const TCHAR* ProfileName)
{
	...
	if (!bSectionExists)
	{
		UE_LOGF(LogNet, Log, "EmulationProfile [%ls] was not found in %ls. Packet settings were not changed", *SectionName, *GEngineIni);
		return false;
	}
	...
	for (const FString& ConfigVar : SectionConfigs)
	{
		...
		if (ConfigVar.Split(TEXT("="), &VarName, &VarValue))
		{
			...
			{
				UE_LOGF(LogNet, Warning, "FPacketSimulationSettings::LoadEmulationProfile could not find property named %ls", *VarName);
			}
		}
	}
