...
{
	UE_IRIS_PARALLEL_EXPR(UE::TWriteScopeLock LockGuard(NetworkMetricLock));
...
bool UNetworkMetricsDatabase::SetFloat(const FName MetricName, float Value)
{
	...
	{
		UE_LOGF(LogNetworkMetrics, Warning, "UNetworkMetricsDatabase::SetFloat Cannot find float metric %ls.", *MetricName.ToString());
		return false;
	}
}
...
bool UNetworkMetricsDatabase::SetMinFloat(const FName MetricName, float Value)
{
	...
	{
		UE_LOGF(LogNetworkMetrics, Warning, "UNetworkMetricsDatabase::SetMinFloat Cannot find float metric %ls.", *MetricName.ToString());
		return false;
	}
}
...
bool UNetworkMetricsDatabase::SetMaxFloat(const FName MetricName, float Value)
{
	...
	{
		UE_LOGF(LogNetworkMetrics, Warning, "UNetworkMetricsDatabase::SetMaxFloat Cannot find float metric %ls.", *MetricName.ToString());
		return false;
	}
}
...
{
	UE_IRIS_PARALLEL_EXPR(UE::TReadScopeLock LockGuard(NetworkMetricLock));
	UE::Net::FNetworkMetric<float>* Metric = MetricFloats.Find(MetricName);
	if (Metric)
	{
		return Metric->Value;
	...
	{
		UE_LOGF(LogNetworkMetrics, Warning, "UNetworkMetricsDatabase::GetFloat Cannot find float metric %ls.", *MetricName.ToString());
		return 0.0f;
	}
}
...
bool UNetworkMetricsDatabase::SetInt(const FName MetricName, int64 Value)
{
	...
	{
		UE_LOGF(LogNetworkMetrics, Warning, "UNetworkMetricsDatabase::SetInt Cannot find integer metric %ls.", *MetricName.ToString());
		return false;
	}
}
...
bool UNetworkMetricsDatabase::SetMinInt(const FName MetricName, int64 Value)
{
	...
	{
		UE_LOGF(LogNetworkMetrics, Warning, "UNetworkMetricsDatabase::SetMinInt Cannot find integer metric %ls.", *MetricName.ToString());
		return false;
	}
}
...
bool UNetworkMetricsDatabase::SetMaxInt(const FName MetricName, int64 Value)
{
	...
	{
		UE_LOGF(LogNetworkMetrics, Warning, "UNetworkMetricsDatabase::SetMaxInt Cannot find integer metric %ls.", *MetricName.ToString());
		return false;
	}
}
...
bool UNetworkMetricsDatabase::IncrementInt(const FName MetricName, int64 Value)
{
	...
	{
		UE_LOGF(LogNetworkMetrics, Warning, "UNetworkMetricsDatabase::IncrementInt Cannot find integer metric %ls.", *MetricName.ToString());
		return false;
	}
}
...
int64 UNetworkMetricsDatabase::GetInt(const FName MetricName)
{
	...
	{
		UE_LOGF(LogNetworkMetrics, Warning, "UNetworkMetricsDatabase::GetInt Cannot find integer metric %ls.", *MetricName.ToString());
		return 0;
	}
}
...
void UNetworkMetricsDatabase::Register(const FName MetricName, TWeakObjectPtr<UNetworkMetricsBaseListener> Reporter)
{
	...
	{
		UE_LOGF(LogNetworkMetrics, Warning, "UNetworkMetricsDatabase::Register: Cannot register metric %ls with invalid listener.", *MetricName.ToString());
		return;
	}
	...
	if (ensureMsgf(MetricType, TEXT("Cannot find metric %s to register listener."), *MetricName.ToString()))
	{
		...
		for (TInstancedStruct<FNetworkMetricsMutator>& Mutator : Reporter->GetMutators())
		{
			if (ensure(Mutator.IsValid()))
			{
				...
				{
					UE_LOGF(LogNetworkMetrics, Warning, "UNetworkMetricsDatabase::Register: Cannot register metric %ls whose name collides with a registered mutator for listener %ls.", *MetricName.ToString(), *Reporter->GetClass()->GetName());
...
void UNetworkMetricsDatabase::RegisterMutator(TWeakObjectPtr<UNetworkMetricsBaseListener> Listener, const TInstancedStruct<FNetworkMetricsMutator>& Mutator)
{
	...
	{
		UE_LOGF(LogNetworkMetrics, Warning, "UNetworkMetricsDatabase::RegisterMutator: Cannot register mutator %ls with invalid listener.", *Mutator.Get().GetName().ToString());
		return;
	}
	...
	for (const FName MutatorMetricName : Mutator.Get().GetAllMetricNames())
	{
		...
		{
			UE_LOGF(LogNetworkMetrics, Warning, "UNetworkMetricsDatabase::RegisterMutator: Cannot register mutator %ls with an unknown metric '%ls'.", *Mutator.Get().GetName().ToString(), *MutatorMetricName.ToString());
			return;
		}
	}
	...
	for (TInstancedStruct<FNetworkMetricsMutator>& ExistingMutator : Listener->GetMutators())
	{
		if (ensure(ExistingMutator.IsValid()))
		{
			...
			{
				UE_LOGF(LogNetworkMetrics, Warning, "UNetworkMetricsDatabase::RegisterMutator: Cannot register mutator %ls whose name collides with an existing mutator for listener %ls.", *Mutator.Get().GetName().ToString(), *Listener->GetClass()->GetName());
	...
	if (ListenerMetrics)
	{
		for (const FNameAndType& NameAndType : *ListenerMetrics)
		{
			...
			{
				UE_LOGF(LogNetworkMetrics, Warning, "UNetworkMetricsDatabase::RegisterMutator: Cannot register mutator %ls whose name collides with a registered metric for listener %ls.", *Mutator.Get().GetName().ToString(), *Listener->GetClass()->GetName());
...
void UNetworkMetricsLog::Report(const UE::Net::FNetworkMetricSnapshot& Snapshot)
{
	...
	{
		MetricsStr += FString::Printf(TEXT("%s=%lld "), *Metric.Name.ToString(), Metric.Value);
	}
	...
	UE_CLOGF(!MetricsStr.IsEmpty(), LogNetworkMetrics, Log, "Network Metrics: %ls", *MetricsStr);
