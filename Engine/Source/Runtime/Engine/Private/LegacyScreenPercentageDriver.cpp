...
void OnScreenPercentageChange(IConsoleVariable* Var)
{
	...
	// Check whether dynamic resolution is overriding r.ScreenPercentage.
	{
		...
		{
			UE_LOGF(LogEngine, Display, "r.ScreenPercentage=%f is ignored because overriden by dynamic resolution settings (r.DynamicRes.OperationMode).", SP);
			return;
		}
		else if (DynResInfo.Status == EDynamicResolutionStatus::DebugForceEnabled)
		{
			UE_LOGF(LogEngine, Display, "r.ScreenPercentage=%f is ignored because overriden by dynamic resolution settings (r.DynamicRes.TestScreenPercentage).", SP);
			return;
		}
	}
}
...
float FStaticResolutionFractionHeuristic::ResolveResolutionFraction() const
{
	...
	if (EffectiveScreenPercentageMode == EScreenPercentageMode::BasedOnDisplayResolution)
	{
		...
		if (!bInitPixelCount)
		{
			...
			if (!bSuccess)
			{
				UE_LOGF(LogEngine, Fatal, "Failed to load Rendering.AutoScreenPercentage ini section.");
			}
