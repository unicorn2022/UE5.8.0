...
{
	// WorkingMinDesiredLODLevel and MinLODLevel could be a LOD that's not loaded, so we need to clamp it to the first loaded LOD in the render data.
	return FMath::Max3<int32>(WorkingMinDesiredLODLevel, MinLODLevel, SkeletalMeshRenderData->CurrentFirstLODIdx);
}
