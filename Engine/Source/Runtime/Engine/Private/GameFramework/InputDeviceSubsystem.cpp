...
namespace UE
{
	static bool bShouldUpdatedDeviceSubsystemOnConnection = true;
	static FAutoConsoleVariableRef CVarShouldUpdatedDeviceSubsystemOnConnection(TEXT("input.bShouldUpdatedDeviceSubsystemOnConnection"),
		bShouldUpdatedDeviceSubsystemOnConnection,
		TEXT("If true, then upon receiving a device connection event, the UInputDeviceSubsystem will attempt to update the latest known metadata about a given device."));

	static bool bShouldUpdateDeviceSubsystemOnDescriptorChanged = true;
	static FAutoConsoleVariableRef CVarShouldUpdateDeviceSubsystemOnDescriptorChanged(TEXT("input.bShouldUpdateDeviceSubsystemOnDescriptorChanged"),
		bShouldUpdateDeviceSubsystemOnDescriptorChanged,
		TEXT("If true, then UInputDeviceSubsystem will route FInputDeviceRegistry::OnInputDeviceDescriptorChanged broadcasts into SetMostRecentlyUsedHardwareDevice ")
		TEXT("(which fires OnInputHardwareDeviceChanged). Set to false to disable the descriptor-change-driven path on a specific platform if it causes issues; ")
		TEXT("the device-connection path (input.bShouldUpdatedDeviceSubsystemOnConnection) is unaffected."));
}
...
class FInputDeviceSubsystemProcessor : public IInputProcessor
{
	...
	void UpdateLatestDevice(const FKey Key, const FInputDeviceId InDeviceId)
	{
		...
		{
			if (const TOptional<FInputDeviceDescriptor> Desc = FInputDeviceRegistry::FindDescriptor(InDeviceId))
			{
				if (const FHardwareDeviceIdentifier* Hardware = UInputPlatformSettings::Get()->GetHardwareDeviceForClassName(Desc->HardwareDeviceIdentifier))
				{
				...
				{
					SubSystem->SetMostRecentlyUsedHardwareDevice(InDeviceId, { Desc->InputDeviceName, Desc->HardwareDeviceIdentifier });
				}
			}
...
void UInputDeviceSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	IPlatformInputDeviceMapper::Get().GetOnInputDeviceHardwareInfoAvailable().AddUObject(this, &UInputDeviceSubsystem::HandleDeviceConnectionChange);

	// Listen for descriptor changes on the registry so we can republish the hardware identifier into
	// our own latest-device tracking. RegisterDevice already filters to only fire this delegate when
	// a descriptor's contents actually change, so by routing through SetMostRecentlyUsedHardwareDevice
	// (which fires OnInputHardwareDeviceChanged) we collapse "device connection" and "device descriptor
	// refresh" into a single delegate that gameplay code can subscribe to.
	FInputDeviceRegistry::OnInputDeviceDescriptorChanged().AddUObject(this, &UInputDeviceSubsystem::HandleInputDeviceDescriptorChanged);
...
void UInputDeviceSubsystem::Deinitialize()
{
	...
	IPlatformInputDeviceMapper::Get().GetOnInputDeviceHardwareInfoAvailable().RemoveAll(this);
	FInputDeviceRegistry::OnInputDeviceDescriptorChanged().RemoveAll(this);
}
...
FInputDevicePropertyHandle UInputDeviceSubsystem::ActivateDeviceProperty(UInputDeviceProperty* Property, const FActivateDevicePropertyParams& Params)
{
	if (!Property)
	{
		UE_LOGF(LogInputDeviceProperties, Error, "Invalid Property passed into ActivateDeviceProperty! Nothing will happen.");
		return FInputDevicePropertyHandle::InvalidHandle;
	}

	if (!Params.UserId.IsValid())
	{
		UE_LOGF(LogInputDeviceProperties, Error, "Invalid Platform User Id '%d' given to ActivateDeviceProperty! Nothing will happen.", Params.UserId.GetInternalId());
		return FInputDevicePropertyHandle::InvalidHandle;
	}
...
FInputDevicePropertyHandle UInputDeviceSubsystem::ActivateDevicePropertyOfClass(TSubclassOf<UInputDeviceProperty> PropertyClass, const FActivateDevicePropertyParams& Params)
{
	if (!PropertyClass)
	{
		UE_LOGF(LogInputDeviceProperties, Error, "Invalid PropertyClass passed into ActivateDeviceProperty! Nothing will happen.");
		return FInputDevicePropertyHandle::InvalidHandle;
	}
...
void UInputDeviceSubsystem::RemoveDevicePropertyHandles(const TSet<FInputDevicePropertyHandle>& HandlesToRemove)
{
	...
	{
		UE_LOGF(LogInputDeviceProperties, Warning, "Provided an empty set of handles to remove. Nothing will happen.");
	}
}
...
void UInputDeviceSubsystem::HandleDeviceConnectionChange(EInputDeviceConnectionState NewState, FPlatformUserId User, FInputDeviceId DeviceId)
{
	if (!UE::bShouldUpdatedDeviceSubsystemOnConnection)
	{
		return;
	}

	// Not all systems will have a device descriptor available right away upon connection yet
	const TOptional<FInputDeviceDescriptor> Desc = FInputDeviceRegistry::FindDescriptor(DeviceId);

	UE_LOGF(LogInputDeviceProperties, Log, "Input device connection change: NewState=%ls, DeviceId=%d, PlatformUser=%d, Device Info=%ls::%ls",
		*UEnum::GetValueAsString(NewState), DeviceId.GetId(), User.GetInternalId(),
		Desc.IsSet() ? *Desc->InputDeviceName.ToString() : TEXT("(unknown)"),
		Desc.IsSet() ? *Desc->HardwareDeviceIdentifier.ToString() : TEXT("(unknown)"));

	// When a new device is connected, we can attempt to populate the subsystem with its connection info.
	// This way it is easier to build menus or respond to specific devices on their connection states
	if (NewState != EInputDeviceConnectionState::Connected)
	{
		return;
	}

	// We need a valid descriptor to proceed past this point
	if (!Desc)
	{
		return;
	}
	...
	if (const FHardwareDeviceIdentifier* Hardware = UInputPlatformSettings::Get()->GetHardwareDeviceForClassName(Desc->HardwareDeviceIdentifier))
	{
		SetMostRecentlyUsedHardwareDevice(DeviceId, *Hardware);
	}
	else
	{
		SetMostRecentlyUsedHardwareDevice(DeviceId, { Desc->InputDeviceName, Desc->HardwareDeviceIdentifier });
	}
}

void UInputDeviceSubsystem::HandleInputDeviceDescriptorChanged(const FInputDeviceDescriptor& Descriptor)
{
	// Kill switch — defaults true. Lets a platform / project disable the descriptor-change-driven
	// SetMostRecentlyUsedHardwareDevice path without rebuilding, in case it causes a live issue.
	// The other update paths (HandleDeviceConnectionChange, the input-preprocessor's UpdateLatestDevice)
	// are independent and continue to function regardless.
	if (!UE::bShouldUpdateDeviceSubsystemOnDescriptorChanged)
	{
		return;
	}

	// Convert the registry descriptor into the FHardwareDeviceIdentifier shape the rest of the
	// subsystem speaks. This is the same translation HandleDeviceConnectionChange does — it prefers
	// the project-config-mapped FHardwareDeviceIdentifier when one is registered for the descriptor's
	// HardwareDeviceIdentifier name, otherwise synthesizes a minimal one from the descriptor's fields.
	if (const FHardwareDeviceIdentifier* Hardware = UInputPlatformSettings::Get()->GetHardwareDeviceForClassName(Descriptor.HardwareDeviceIdentifier))
	{
		SetMostRecentlyUsedHardwareDevice(Descriptor.HardwareDeviceHandle, *Hardware);
	}
	else
	{
		SetMostRecentlyUsedHardwareDevice(Descriptor.HardwareDeviceHandle, { Descriptor.InputDeviceName, Descriptor.HardwareDeviceIdentifier });
	}
}
...
{
	UE_LOGF(LogInputDeviceProperties, Verbose, "SetMostRecentlyUsedHardwareDevice: DeviceId=%d, Hardware=%ls",
		InDeviceId.GetId(), *InHardwareId.ToString());

	// Capture the previous hardware identifier for this device BEFORE we overwrite it so we can
	// detect "same DeviceId, different hardware" 
	// This case must also fire OnInputHardwareDeviceChanged so downstream consumers re-query
	// device capabilities, even though the user's "most recent device id" hasn't changed.
	const FHardwareDeviceIdentifier* PreviousHardwareForDevice = LatestInputDeviceIdentifiers.Find(InDeviceId);
	const bool bHardwareIdentifierChanged = (PreviousHardwareForDevice == nullptr) || (*PreviousHardwareForDevice != InHardwareId);
	...
	const bool bUserLatestDeviceChanged = (LatestUserData.LatestDeviceId != InDeviceId);
	...
	// Fire the delegate when EITHER (a) the user just switched to a different device, or (b) the
	// device itself reports new hardware. Both are device-identity changes that gameplay code may
	// care about.
	//
	// The (b) "hardware identifier changed for the same DeviceId" branch is gated by the same
	// kill switch as HandleInputDeviceDescriptorChanged. When that CVar is false the behavior of
	// this function reduces to the original (fire only when LatestDeviceId for the user changes),
	// so disabling input.bShouldUpdateDeviceSubsystemOnDescriptorChanged fully restores the
	// pre-change semantics — both at the registry-routing entry point AND inside this setter.
	const bool bRequiresDelegateUpdate = bUserLatestDeviceChanged
		|| (UE::bShouldUpdateDeviceSubsystemOnDescriptorChanged && bHardwareIdentifierChanged);

	if (bRequiresDelegateUpdate)
	{
		OnInputHardwareDeviceChangedNative.Broadcast(OwningUserId, InDeviceId);
