...
void FCharacterNetworkSerializationPackedBitsNetSerializer::Deserialize(FNetSerializationContext& Context, const FNetDeserializeArgs& Args)
{
	...
	if (bHasDataBits)
	{
		...
		if (NumDataBits > MaxNumDataBits)
		{
			...
			UE_LOGF(LogNetPlayerMovement, Error, "FCharacterNetworkSerializationPackedBits::Deserialize: Invalidating move due to NumBits (%u) exceeding allowable limit (%u). See NetPackedMovementMaxBits.", NumDataBits, MaxNumDataBits);
...
void FCharacterNetworkSerializationPackedBitsNetSerializer::Quantize(FNetSerializationContext& Context, const FNetQuantizeArgs& Args)
{
	...
	if (NumDataBits > MaxNumDataBits)
	{
		...
		UE_LOGF(LogNetPlayerMovement, Error, "FCharacterNetworkSerializationPackedBits::Quantize: Invalidating move due to NumBits (%u) exceeding allowable limit (%u). See NetPackedMovementMaxBits.", NumDataBits, MaxNumDataBits);
		NumDataBits = 0U;
		ensureMsgf(false, TEXT("Invalidating move due to NumBits exceeding allowable limit"));
	}
