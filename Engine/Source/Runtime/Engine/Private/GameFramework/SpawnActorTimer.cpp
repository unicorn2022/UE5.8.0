...
void FSpawnActorTimer::SpawnActorTimerCmdFunc(const TArray<FString>& Args)
{
	if(Args.Num() == 1)
	{
		if (Args[0].Compare(FString("start"), ESearchCase::IgnoreCase) == 0)
		{
			Get().Start();
		}
		else if(Args[0].Compare(FString("stop"), ESearchCase::IgnoreCase) == 0)
		{
			Get().Stop();
		}
	}
...
void FSpawnActorTimer::Start()
{
	if(!bIsRecordingSpawnActorTimes)
	{
		...
		UE_LOGF(LogSpawnActorTimer, Log, "Starting SpawnActorTimer");
	}
}
...
{
	UE_LOGF(LogSpawnActorTimer, Log, "===============");
	...
	{
		UE_LOGF(LogSpawnActorTimer, Log, "%ls,%f,%f,%d", *AggStat.ClassName.ToString(), AggStat.AverageSpawnTime, AggStat.TotalSpawnTime, AggStat.NumSpawned);
	}

	UE_LOGF(LogSpawnActorTimer, Log, "%d Spawns over %d frames", SpawnActorInfos.Num(), FrameCount);
	UE_LOGF(LogSpawnActorTimer, Log, "Average spawn time per frame: %f", (SpawnActorTotalTime/(double)FrameCount) * 1000.f);
	UE_LOGF(LogSpawnActorTimer, Log, "===============");
}
	...
	if(bIsRecordingSpawnActorTimes && ActorName != NAME_None)
	{
		...
		if(SpawnActorType != ESpawnActorTimingType::FinishSpawning)
		{
			...
			if(SpawnActorType == ESpawnActorTimingType::SpawnActorDeferred)
			{
				...
				{
					UE_LOGF(LogSpawnActorTimer, Warning, "Trying to start deferred spawn for '%ls', but already an incomplete entry!", *ActorName.ToString());
				}
				else
				{
