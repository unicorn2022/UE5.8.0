...
void FMallocLeakReporter::Start(int32 FilterSize/*=0*/, float ReportOnTime /*= 0.0f*/)
{
	...
	UE_LOGF(LogLeakDetector, Error, "Cannot track leaks, MALLOC_LEAKDETECTION=%d, PLATFORM_USES_FIXED_GMalloc_CLASS=%d (should be set as 1 & 0 in your Game<Config>Target.cs file)",
		MALLOC_LEAKDETECTION, PLATFORM_USES_FIXED_GMalloc_CLASS);

#else
	UE_LOGF(LogLeakDetector, Log, "Started Tracking Allocations > %d KiB", FilterSize / 1024);
...
void FMallocLeakReporter::Stop()
{
	...
	UE_LOGF(LogLeakDetector, Log, "Stopped tracking allocations");
...
int32 FMallocLeakReporter::WriteReports(const uint32 ReportFlags /*=EReportOption::ReportAll*/)
{
	...
	if (LeakCount > 0)
	{
		UE_LOGF(LogLeakDetector, Log, "Found %d leaks, report written to %ls", LeakCount, *LeakName);

		ReportDelegate.Broadcast(ReportCount, LeakCount);
	}
	else
	{
		UE_LOGF(LogLeakDetector, Log, "No leaks found");
	}
...
#else
	UE_LOGF(LogLeakDetector, Log, "Cannot report leaks. MALLOC_LEAKDETECTION=0");
	return 0;
#endif // MALLOC_LEAKDETECTION
}
...
static FAutoConsoleCommand LeakReporterStartCommand(
	...
	FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& Args) {
		...
		UE_LOGF(LogConsoleResponse, Display, "Tracking allocations >= %d KiB and reporting every %f seconds", Size, ReportTime);
	})
);
...
static FAutoConsoleCommand LeakReporterStopCommand(
	...
	FConsoleCommandDelegate::CreateLambda([]() {
		...
		UE_LOGF(LogConsoleResponse, Display, "Stopped tracking allocations.");
	})
);
...
static FAutoConsoleCommand LeakReporterClearCommand(
	TEXT("mallocleak.clear"),
		...
		UE_LOGF(LogConsoleResponse, Display, "Cleared recorded data.");
	})
);
...
static FAutoConsoleCommand LeakReporterReportCommand(
	...
	FConsoleCommandDelegate::CreateLambda([]() {
		...
		UE_LOGF(LogConsoleResponse, Display, "Wrote out memory reports");
	})
);
