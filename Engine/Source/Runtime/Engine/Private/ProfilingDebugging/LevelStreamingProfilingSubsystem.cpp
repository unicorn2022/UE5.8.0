...
void ULevelStreamingProfilingSubsystem::Deinitialize() 
{
	...
	{
		UE_LOGF(LogLevelStreamingProfiling, Log, "Waiting for report writing task to complete during Deinitialize for safety");
...
TUniquePtr<ULevelStreamingProfilingSubsystem::FActiveLevel> ULevelStreamingProfilingSubsystem::MakeActiveLevel(const ULevelStreaming* StreamingLevel, ELevelState InitialState, ULevel* LoadedLevel)
{
	...
	else
	{
		...
		case ELevelState::RemovedFromWorld: 
			UE_LOGF(LogLevelStreamingProfiling, Error, "Entered initial state %ls without loaded level. %ls %ls", 
...
void ULevelStreamingProfilingSubsystem::StartTracking()
{
	if (!ReportWritingTask.IsCompleted())
	{
		UE_LOGF(LogLevelStreamingProfiling, Warning, "StartTracking called while writing previous report - waiting for completion.");
		ReportWritingTask.Wait();
	}
	
	if (bIsTracking)
	{
		UE_LOGF(LogLevelStreamingProfiling, Warning, "StartTracking called while already tracking");
	}
	else
	{
		bIsTracking = true;

		UE_LOGF(LogLevelStreamingProfiling, Log, "Starting to track level streaming events");
...
void ULevelStreamingProfilingSubsystem::StopTrackingAndReport()
{
	...
	if (!bIsTracking)
	{
		UE_LOGF(LogLevelStreamingProfiling, Warning, "StopTrackingAndReport called without StartTracking");
		return;
	}

	bIsTracking = false;
	UE_LOGF(LogLevelStreamingProfiling, Log, "Reporting level streaming stats");
...
void ULevelStreamingProfilingSubsystem::OnLevelStreamingTargetStateChanged(UWorld* World, const ULevelStreaming* StreamingLevel, ULevel* LevelIfLoaded, ELevelStreamingState CurrentState, ELevelStreamingTargetState PrevTarget, ELevelStreamingTargetState NewTarget)
{
	...
	UE_LOGF(LogLevelStreamingProfiling, Verbose, "OnLevelStreamingTargetStateChanged: %ls %ls. %ls: %ls -> %ls (LoadedLevel: %ls)",
		*StreamingLevel->GetName(), 
		*StreamingLevel->GetWorldAssetPackageName(),
	...
	case ELevelStreamingTargetState::LoadedNotVisible:
		...
		case ELevelStreamingState::MakingVisible:
			return;
		default:
			UE_LOGF(LogLevelStreamingProfiling, Warning, "Unexpected desired state LoadedNotVisible in current state %ls", ::EnumToString(CurrentState));
			return;
		}
	case ELevelStreamingTargetState::LoadedVisible:
		...
		default:
			UE_LOGF(LogLevelStreamingProfiling, Warning, "Unexpected desired state LoadedVisible in current state %ls", ::EnumToString(CurrentState));
			return;
		}
...
{	
	if (FCsvProfiler::Get()->IsCapturing())
	{
		...
		CSV_CUSTOM_STAT(LevelStreaming, NumActorsPendingPurge, FLevelStreamingGCHelper::GetNumActorsPendingPurge(), ECsvCustomStatOp::Set);
	}
}
#endif
...
void ULevelStreamingProfilingSubsystem::OnLevelStreamingStateChanged(UWorld* World, const ULevelStreaming* StreamingLevel, ULevel* LevelIfLoaded, ELevelStreamingState PrevState, ELevelStreamingState NewState)
{
	...
	UE_LOGF(LogLevelStreamingProfiling, Verbose, "OnLevelStreamingStateChanged: %ls %ls. %ls -> %ls (LoadedLevel: %ls)",
		*StreamingLevel->GetName(), 
		*StreamingLevel->GetWorldAssetPackageName(),
	...
	case ELevelStreamingState::Unloaded: 
		...
		{
			UE_LOGF(LogLevelStreamingProfiling, Verbose, "Ignoring transition to Unloaded from previous state %ls", ::EnumToString(PrevState));
		}
		break;
	...
	case ELevelStreamingState::LoadedNotVisible: 
		...
		case ELevelStreamingState::Unloaded:
			...
			{
				UE_LOGF(LogLevelStreamingProfiling, Verbose, "Ignoring transition to LoadedNotVisible from previous state %ls because LoadedLevel is not yet available",
					::EnumToString(PrevState));
			}
			break;
		default:
			UE_LOGF(LogLevelStreamingProfiling, Verbose, "Ignoring transition to LoadedNotVisible from previous state %ls", ::EnumToString(PrevState));
			break;
	...
	case ELevelStreamingState::LoadedVisible: 
		...
		{
			UE_LOGF(LogLevelStreamingProfiling, Verbose, "Ignoring transition to LoadedVisible from previous state %ls", ::EnumToString(PrevState));
		}
		break;
	case ELevelStreamingState::MakingVisible: 
	case ELevelStreamingState::MakingInvisible: 
...
void ULevelStreamingProfilingSubsystem::OnLevelQueuedForLoading(UWorld* World, const ULevelStreaming* StreamingLevel)
{
	...
	if (Level.IsValid())
	{
		...
		UE_LOGF(LogLevelStreamingProfiling, Error, "Streaming level %ls queued for loading while already active.", 
			*StreamingLevel->GetWorldAssetPackageName());
	}
	UE_LOGF(LogLevelStreamingProfiling, Verbose, "Streaming level %ls: %ls queued for loading", 
		*StreamingLevel->GetName(), 
		*StreamingLevel->GetWorldAssetPackageName());
...
void ULevelStreamingProfilingSubsystem::OnLevelUnqueuedForLoading(UWorld* World, const ULevelStreaming* StreamingLevel)
{
	...
	UE_LOGF(LogLevelStreamingProfiling, Verbose, "Streaming level %ls: %ls unqueued for loading", 
		*StreamingLevel->GetName(), 
		*StreamingLevel->GetWorldAssetPackageName());
	int32 NumRemoved = ActiveLevels.Remove(StreamingLevel);
	if (NumRemoved == 0)
	{	
		UE_LOGF(LogLevelStreamingProfiling, Verbose, "Streaming level %ls: %ls unqueued for loading when it wasn't tracked", 
			*StreamingLevel->GetName(), 
			*StreamingLevel->GetWorldAssetPackageName());
	}
}
...
void ULevelStreamingProfilingSubsystem::OnStreamingLevelRemoved(UWorld* World, const ULevelStreaming* StreamingLevel)
{
	...
	if (NumRemoved != 0)
	{
		UE_LOGF(LogLevelStreamingProfiling, Verbose, "Streaming level %ls: %ls removed entirely", 
			*StreamingLevel->GetName(), 
			*StreamingLevel->GetWorldAssetPackageName());
	}
}
...
void ULevelStreamingProfilingSubsystem::OnLevelStartedAsyncLoading(UWorld* World, const ULevelStreaming* StreamingLevel)
{
	...
	if (!Level.IsValid())
	{
		UE_LOGF(LogLevelStreamingProfiling, Verbose, "Streaming level %ls started loading while untracked.", 
			*StreamingLevel->GetWorldAssetPackageName());
		Level = MakeActiveLevel(StreamingLevel, ELevelState::QueuedForLoading);
	}
	...
	{
		UE_LOGF(LogLevelStreamingProfiling, Error, "Streaming level %ls started loading in unexpected state %ls",
			*StreamingLevel->GetWorldAssetPackageName(), EnumToString(Level->State));
	}
	UE_LOGF(LogLevelStreamingProfiling, Verbose, "Streaming level %ls: %ls started async loading", 
		*StreamingLevel->GetName(), 
		*StreamingLevel->GetWorldAssetPackageName());
...
void ULevelStreamingProfilingSubsystem::OnLevelFinishedAsyncLoading(UWorld* World, const ULevelStreaming* StreamingLevel, ULevel* LoadedLevel)
{
	...
	if (!Level.IsValid())
	{
		UE_LOGF(LogLevelStreamingProfiling, Verbose, "Streaming level %ls finished loading while untracked.", 
			*StreamingLevel->GetWorldAssetPackageName());
	...
	if (Level->State == ELevelState::QueuedForLoading)
	{
		UE_LOGF(LogLevelStreamingProfiling, Verbose, "Streaming level %ls: %ls finished async loading while queued - must have been found in memory.", 
			*StreamingLevel->GetName(), 
			*StreamingLevel->GetWorldAssetPackageName());
	...
	{
		UE_LOGF(LogLevelStreamingProfiling, Error, "Streaming level %ls finished loading in unexpected state %ls",
			*StreamingLevel->GetWorldAssetPackageName(), EnumToString(Level->State));
	}

	if (!LoadedLevel)
	{
		UE_LOGF(LogLevelStreamingProfiling, Verbose, "Streaming level %ls: %ls cancelled or failed async loading", 
			*StreamingLevel->GetName(), 
			*StreamingLevel->GetWorldAssetPackageName());
	...
	UE_LOGF(LogLevelStreamingProfiling, Verbose, "Streaming level %ls: %ls finished async loading", 
		*StreamingLevel->GetName(), 
		*StreamingLevel->GetWorldAssetPackageName());
...
void ULevelStreamingProfilingSubsystem::OnLevelQueuedForAddToWorld(UWorld* World, const ULevelStreaming* StreamingLevel, ULevel* LoadedLevel)
{
	...
	if (!Level.IsValid())
	{
		...
		UE_LOGF(LogLevelStreamingProfiling, Verbose, "Streaming level %ls queued for adding to world while untracked.", 
			*StreamingLevel->GetWorldAssetPackageName());
	...
	{
		UE_LOGF(LogLevelStreamingProfiling, Error, "Streaming level %ls queued for adding to world in unexpected state %ls",
			*StreamingLevel->GetWorldAssetPackageName(), EnumToString(Level->State));
	}

	UE_LOGF(LogLevelStreamingProfiling, Verbose, "Streaming level %ls: %ls queued for adding to world", 
		*StreamingLevel->GetName(), 
		*StreamingLevel->GetWorldAssetPackageName());
	double Time = FPlatformTime::Seconds();
	LevelStats[Level->StatsIndex].bValid = true;
...
void ULevelStreamingProfilingSubsystem::OnLevelUnqueuedForAddToWorld(UWorld* World, const ULevelStreaming* StreamingLevel, ULevel* LoadedLevel)
{
	...
	{
		// Level loaded before we started profiling 
		UE_LOGF(LogLevelStreamingProfiling, Verbose, "Streaming level %ls unqueued for adding to world while untracked.", 
			*StreamingLevel->GetWorldAssetPackageName());
		Level = MakeActiveLevel(StreamingLevel, ELevelState::Loaded, LoadedLevel);
	}
	...
	{
		UE_LOGF(LogLevelStreamingProfiling, Error, "Streaming level %ls unqueud for adding to world in unexpected state %ls",
			*StreamingLevel->GetWorldAssetPackageName(), EnumToString(Level->State));
	}

	UE_LOGF(LogLevelStreamingProfiling, Verbose, "Streaming level %ls: %ls unqueued for adding to world", 
		*StreamingLevel->GetName(), 
		*StreamingLevel->GetWorldAssetPackageName());
	double Time = FPlatformTime::Seconds();
	LevelStats[Level->StatsIndex].bValid = true;
...
void ULevelStreamingProfilingSubsystem::OnLevelStartedAddToWorld(UWorld* World, const ULevelStreaming* StreamingLevel, ULevel* LoadedLevel)
{
	...
	{
		// Level loaded before we started profiling 
		UE_LOGF(LogLevelStreamingProfiling, Verbose, "Streaming level %ls started adding to world while untracked.", 
			*StreamingLevel->GetWorldAssetPackageName());
	...
	if (Level->State != ELevelState::QueuedForAddToWorld)
	{
		UE_LOGF(LogLevelStreamingProfiling, Error, "Streaming level %ls started adding to world in unexpected state %ls",
			*StreamingLevel->GetWorldAssetPackageName(), EnumToString(Level->State));
	}

	UE_LOGF(LogLevelStreamingProfiling, Verbose, "Streaming level %ls: %ls started adding to world", 
		*StreamingLevel->GetName(), 
		*StreamingLevel->GetWorldAssetPackageName());
	double Time = FPlatformTime::Seconds();
	LevelStats[Level->StatsIndex].bValid = true;
...
void ULevelStreamingProfilingSubsystem::OnLevelFinishedAddToWorld(UWorld* World, const ULevelStreaming* StreamingLevel, ULevel* LoadedLevel)
{
	...
	if (!Level.IsValid())
	{
		...
		UE_LOGF(LogLevelStreamingProfiling, Verbose, "Streaming level %ls finished adding to world while untracked.", 
			*StreamingLevel->GetWorldAssetPackageName());
	...
	{
		UE_LOGF(LogLevelStreamingProfiling, Error, "Streaming level %ls finished adding to world in unexpected state %ls",
			*StreamingLevel->GetWorldAssetPackageName(), EnumToString(Level->State));
	}

	UE_LOGF(LogLevelStreamingProfiling, Verbose, "Streaming level %ls: %ls finished adding to world", 
		*StreamingLevel->GetName(), 
		*StreamingLevel->GetWorldAssetPackageName());
	double Time = FPlatformTime::Seconds();
	LevelStats[Level->StatsIndex].bValid = true;
	...
	if (World && World->GetWorldPartition())
	{
		...
		if (CellBounds.IsValid)
		{
			...
			{
				UE_LOGF(LogLevelStreamingProfiling, Verbose, "Streaming level %ls %ls streamed in at distance %.0f from cell bounds", 
		...
		if (ContentBounds.IsValid)
		{
			...
			if (MinDistance != MAX_dbl)
			{
				UE_LOGF(LogLevelStreamingProfiling, Verbose, "Streaming level %ls %ls streamed in at distance %.0f from content bounds", 
					*StreamingLevel->GetName(), *StreamingLevel->GetWorldAssetPackageName(), MinDistance);
...
void ULevelStreamingProfilingSubsystem::OnLevelQueuedForRemoveFromWorld(UWorld* World, const ULevelStreaming* StreamingLevel, ULevel* LoadedLevel)
{
	...
	{
		// Level loaded before we started profiling 
		UE_LOGF(LogLevelStreamingProfiling, Verbose, "Streaming level %ls queued for removing from world while untracked.", 
			*StreamingLevel->GetWorldAssetPackageName());
	...
	{
		UE_LOGF(LogLevelStreamingProfiling, Error, "Streaming level %ls queued for removing from world in unexpected state %ls",
			*StreamingLevel->GetWorldAssetPackageName(), EnumToString(Level->State));
	}

	UE_LOGF(LogLevelStreamingProfiling, Verbose, "Streaming level %ls: %ls queued for removing from world", 
		*StreamingLevel->GetName(), 
		*StreamingLevel->GetWorldAssetPackageName());
	double Time = FPlatformTime::Seconds();
	LevelStats[Level->StatsIndex].bValid = true;
...
void ULevelStreamingProfilingSubsystem::OnLevelUnqueuedForRemoveFromWorld(UWorld* World, const ULevelStreaming* StreamingLevel, ULevel* LoadedLevel)
{
	...
	{
		// Level loaded before we started profiling 
		UE_LOGF(LogLevelStreamingProfiling, Verbose, "Streaming level %ls unqueued for removing from world while untracked.", 
			*StreamingLevel->GetWorldAssetPackageName());
	...
	{
		UE_LOGF(LogLevelStreamingProfiling, Error, "Streaming level %ls unqueued for removing from world in unexpected state %ls",
			*StreamingLevel->GetWorldAssetPackageName(), EnumToString(Level->State));
	}

	UE_LOGF(LogLevelStreamingProfiling, Verbose, "Streaming level %ls: %ls unqueued for removing from world", 
		*StreamingLevel->GetName(), 
		*StreamingLevel->GetWorldAssetPackageName());
	double Time = FPlatformTime::Seconds();
	LevelStats[Level->StatsIndex].bValid = true;
...
void ULevelStreamingProfilingSubsystem::OnLevelStartedRemoveFromWorld(UWorld* World, const ULevelStreaming* StreamingLevel, ULevel*)
{
	...
	if (Level->State != ELevelState::QueuedForRemoveFromWorld)
	{
		UE_LOGF(LogLevelStreamingProfiling, Error, "Streaming level %ls started removing from world in unexpected state %ls",
			*StreamingLevel->GetWorldAssetPackageName(), EnumToString(Level->State));
	}

	UE_LOGF(LogLevelStreamingProfiling, Verbose, "Streaming level %ls: %ls started removing from world", 
		*StreamingLevel->GetName(), 
		*StreamingLevel->GetWorldAssetPackageName());
	double Time = FPlatformTime::Seconds();
	LevelStats[Level->StatsIndex].bValid = true;
...
void ULevelStreamingProfilingSubsystem::OnLevelFinishedRemoveFromWorld(UWorld* World, const ULevelStreaming* StreamingLevel, ULevel*)
{
	...
	UE_LOGF(LogLevelStreamingProfiling, Verbose, "Streaming level %ls: %ls finished removing from world", 
		*StreamingLevel->GetName(), 
		*StreamingLevel->GetWorldAssetPackageName());
	double Time = FPlatformTime::Seconds();
	LevelStats[Level->StatsIndex].bValid = true;
