...
static void TraceScreenshotCommandCallback(const TArray<FString>& Args)
{
	...
	{
		UE_LOGF(LogConsoleResponse, Warning, "Invalid arguments. Usage: Trace.Screenshot [Name] [bShowUI]");
		return;
	}
							 ...
							 TChannelType OpaqueAlphaValue)
{
	...
	UE_LOGF(LogCore, Display, "Tracing Screenshot \"%ls\" taken with size: %d x %d", *ScreenshotName, InSizeX, InSizeY);
