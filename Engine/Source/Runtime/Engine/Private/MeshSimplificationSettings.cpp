...
#if WITH_EDITOR
	// As of this writing (Oct 28, 2025), MeshReductionInterface and MeshUtilities have a
	// circular dependency. Each tries to load the other, but it turns out that
	// MeshUtilities loading MeshReductionInterface works out properly, but MeshReductionInterface
	// loading MeshUtilities does not quite work, since FMeshUtilities::StartupModule()
	// sets its VersionString incorrectly when invoked from inside MeshReductionInterface
	// startup.
	//
	// Explicitly load MeshUtilities first for now to sidestep this mess.
	FModuleManager::Get().LoadModule("MeshUtilities");
