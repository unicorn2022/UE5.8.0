...
static TAutoConsoleVariable<float> CVarDemoIncreaseRepPrioritizeThreshold(TEXT("demo.IncreaseRepPrioritizeThreshold"), 0.9f, TEXT("The % of Replicated to Prioritized actors at which prioritize time will be decreased."));
static TAutoConsoleVariable<float> CVarDemoDecreaseRepPrioritizeThreshold(TEXT("demo.DecreaseRepPrioritizeThreshold"), 0.7f, TEXT("The % of Replicated to Prioritized actors at which prioritize time will be increased."));
static TAutoConsoleVariable<float> CVarDemoMinimumRepPrioritizeTime(TEXT("demo.MinimumRepPrioritizePercent"), 0.3f, TEXT("Minimum percent of time that must be spent prioritizing actors, regardless of throttling."));
static TAutoConsoleVariable<float> CVarDemoMaximumRepPrioritizeTime(TEXT("demo.MaximumRepPrioritizePercent"), 0.7f, TEXT("Maximum percent of time that may be spent prioritizing actors, regardless of throttling."));
...
{
	static FName SkipTimeInSecondsTask("SkipTimeInSecondsTask");
	static FName JumpToLiveReplayTask("JumpToLiveReplayTask");
	static FName GotoTimeInSecondsTask("GotoTimeInSecondsTask");
	static FName FastForwardLevelsTask("FastForwardLevelsTask");
};
...
namespace DemoNetDriverRecordingPrivate
{
	...
	static TAutoConsoleVariable<float> CVarDemoMaximumRecDestructionInfoTime(TEXT("demo.MaximumRecDestructionInfoTime"), 0.2f, TEXT("Maximum percentage of frame to use replicating destruction infos, if per frame limit is enabled."));
...
struct FDemoBudgetLogHelper
{
		...
		if (FirstWarningTime != 0.f)
		{
			...
			if (Time - FirstWarningTime > DemoNetDriverRecordingPrivate::WarningTimeInterval)
			{
				if (UE_LOG_ACTIVE(LogDemo, Log))
				{
					...
					UE_LOGF(LogDemo, Log, "%ls", *FString::Join(LogLines, TEXT("\n")));
				}
	...
	void MarkFrameOverBudget(EBudgetCategory Category, ::UE::Core::TCheckedFormatString<FString::FmtCharType, T...> Format, T... Args)
	{
		...
		{
			UE_LOGF(LogDemo, Log, "%ls", *FString::Printf(Format, Args...));
			return;
		}
...
public:
	...
	virtual bool Tick() override
	{
		...
		if (bHasNewReplayTime || bTimeExpired)
		{
			if (bTimeExpired)
			{
				UE_LOGF(LogDemo, Warning, "FJumpToLiveReplayTask::Tick: Too much time since last live update.");
			}
...
public:
		...
		if (!Driver.IsValid())
	{
		...
		{
			UE_LOGF(LogDemo, Warning, "FGotoTimeInSecondsTask::CheckpointReady: Failed to go to checkpoint.");
...
public:
	...
	void CheckpointReady(const FGotoResult& Result)
	{
		...
		if (!Result.WasSuccessful())
		{
			UE_LOGF(LogDemo, Warning, "FFastForwardLevelsTask::CheckpointReady: Failed to get checkpoint.");
		}
	}
...
{
	UE_LOGF(LogDemo, Verbose, "UDemoNetDriver::AddReplayTask. Name: %ls", *NewTask->GetName().ToString());
...
bool UDemoNetDriver::ProcessReplayTasks()
{
	...
	if (!ActiveReplayTask.IsValid() && QueuedReplayTasks.Num() > 0)
	{
		...
		UE_LOGF(LogDemo, Verbose, "UDemoNetDriver::ProcessReplayTasks. Name: %ls", *ActiveReplayTask->GetName().ToString());
	...
	if (GetDuplicateLevelID() == -1)
	{
		...
		else
		{
			...
			if (WorldContext == nullptr)
			{
				...
				UE_LOGF(LogDemo, Error, "UDemoNetDriver::InitConnect: %ls", *Error);
				ReplayHelper.NotifyReplayError(EReplayResult::InitConnect);
				return false;
			}
...
void UDemoNetDriver::TickFlushInternal(float DeltaSeconds)
{
	...
	if (FileAr == nullptr)
	{
		UE_LOGF(LogDemo, Error, "UDemoNetDriver::TickFlush: FileAr == nullptr");
	...
	if (DemoElapsedTime > AVG_FLUSH_TIME_IN_SECONDS && RecordCountSinceFlush > 0)
	{
		...
		{
			UE_LOGF(LogDemo, Verbose, "UDemoNetDriver::TickFlush: SLOW FRAME. Avg: %2.2f, Max: %2.2f, Actors: %i", AvgTimeMS, MaxRecordTimeMS, GetNetworkObjectList().GetActiveObjects().Num());
		}
...
void UDemoNetDriver::TickDispatch(float DeltaSeconds)
{
	...
	if (FileAr == nullptr)
	{
		UE_LOGF(LogDemo, Error, "UDemoNetDriver::TickDispatch: FileAr == nullptr");
		ReplayHelper.NotifyReplayError(EReplayResult::MissingArchive);
		return;
	}
...
void UDemoNetDriver::StopDemo()
{
	if (!IsRecording() && !IsPlaying())
	{
		UE_LOGF(LogDemo, Log, "StopDemo: No demo is playing");
		ClearReplayTasks();
	...
	UE_LOGF(LogDemo, Log, "StopDemo: Demo %ls stopped at frame %d", *ReplayHelper.DemoURL.Map, GetDemoFrameNum());
...
void UDemoNetDriver::TickDemoRecordFrame(float DeltaSeconds)
{
...

	{
		...
		auto PrioritizeDestructionInfos = [ClientConnection, this, &HasConsiderTimeBeenExhausted]()
		{
			...
			for (auto DestroyedOrDormantGUID = ClientConnection->GetDestroyedStartupOrDormantActorGUIDs().CreateIterator(); DestroyedOrDormantGUID; ++DestroyedOrDormantGUID)
			{
				...
				{
					UE_LOGF(LogDemo, Verbose, "Consider time exhaused prioritizing destruction infos.");
					break;
				}
			}
		};
		...
		if (!HasConsiderTimeBeenExhausted())
		{
			...
			struct FActorRemoveInfo
			{
				AActor* Actor = nullptr;
				bool bIsValidActor = false;
			};
			TArray<FActorRemoveInfo, TInlineAllocator<128>> ActorsToRemove;
			...
			for (const TSharedPtr<FNetworkObjectInfo>& ObjectInfo : ActiveObjectSet)
			{
				...
				if (GetDemoCurrentTime() > ActorInfo->NextUpdateTime)
				{
					...
					{
						ActorsToRemove.Add({.Actor = Actor, .bIsValidActor = false});
						continue;
					}
					...
					{
						ActorsToRemove.Add({.Actor = Actor, .bIsValidActor = true});
						continue;
					}
					...
					{
						UE_LOGF(LogNet, Verbose, "DemoNetDriver::ReplicateActor ignored the Actor since he was not BeginPlay yet: %ls", *GetPathNameSafe(Actor));
						continue;
					}
				...
				if (HasConsiderTimeBeenExhausted())
				{
					UE_LOGF(LogDemo, Verbose, "Consider time exhaused while iterating the active object list [%d/%d]", ProcessedCount, ActiveObjectSet.Num());
					break;
				}
			}

			{
				...
				for (FActorRemoveInfo& RemoveInfo : ActorsToRemove)
				{
					if (RemoveInfo.bIsValidActor)
					{
						RemoveNetworkActor(RemoveInfo.Actor);
					}
					else
					{
						RemoveInvalidNetworkActor(RemoveInfo.Actor);
					}
				}
			}
		}
		...
		if (bLateDestructionInfos)
		{
			...
			{
				UE_LOGF(LogDemo, Verbose, "Consider time exhausted without processing destruction infos");
			}
		}
		else
		{
	...
	if (HasLevelStreamingFixes())
	{
		...
		{
			UE_LOGF(LogDemo, Verbose, "bPrioritizeActors and HasLevelStreamingFixes are both enabled. This will undo some prioritization work.");
		}
...
bool UDemoNetDriver::ReplicatePrioritizedActor(const FActorPriority& ActorPriority, FRepActorsParams& Params)
{
	...
	if (bDestructionInfo)
	{
		...
		if (!bHasDestructionInfoTimeBeenExhausted)
		{
			...
			if (Channel)
			{
				UE_LOGF(LogDemo, Verbose, "TickDemoRecord creating destroy channel for NetGUID <%ls,%ls> Priority: %d", *DestructionInfo->NetGUID.ToString(), *DestructionInfo->PathName, ActorPriority.Priority);
	...
	{
		UE_LOGF(LogDemo, Warning, "TickDemoRecord: prioritized actor entry should have either an actor or a destruction info");
	}
	...
	if (Params.TimeLimitSeconds > 0.f)
	{
		...
		{
			UE_LOGF(LogDemo, Verbose, "Actor %ls took more than 95%% of maximum desired recording time. Actor: %.3fms. Max: %.3fms.",
...
void UDemoNetDriver::PauseChannels(const bool bPause)
{
	...
	UE_LOGF(LogDemo, Verbose, "PauseChannels: %d", bChannelsArePaused);
...
void UDemoNetDriver::OnRefreshHeaderCompletePrivate(const FDownloadHeaderResult& Result, int32 LevelIndex)
{
	...
	if (Result.WasSuccessful())
	{
		...
		if (ReplayHelper.ReadPlaybackDemoHeader(Error))
		{
			...
			{
				UE_LOGF(LogDemo, Error, "UDemoNetDriver::OnDownloadHeaderComplete: LevelIndex %d not in range of level names of size: %d", LevelIndex, ReplayHelper.PlaybackDemoHeader.LevelNamesAndTimes.Num());
				ReplayHelper.NotifyReplayError(EReplayResult::Corrupt);
			}
		}
		else
		{
			UE_LOGF(LogDemo, Error, "UDemoNetDriver::OnDownloadHeaderComplete: ReadPlaybackDemoHeader header failed with error %ls.", *Error);
			// ReadPlaybackDemoHeader will have already called NotifyPlaybackError internally on failure
		}
	}
	else
	{
		UE_LOGF(LogDemo, Error, "UDemoNetDriver::OnDownloadHeaderComplete: Downloading header failed.");
		ReplayHelper.NotifyReplayError(EReplayResult::Corrupt);
	}
}
...
bool UDemoNetDriver::ShouldSkipPlaybackPacket(const FPlaybackPacket& Packet)
{
	if (HasLevelStreamingFixes() && Packet.SeenLevelIndex != 0)
	{
		...
		{
			UE_LOGF(LogDemo, Warning, "ShouldSkipPlaybackPacket encountered a packet with an invalid seen level index.");
		}
	}
...
bool UDemoNetDriver::ProcessPacket(const uint8* Data, int32 Count)
{
	...
	if (ServerConnection == nullptr || ServerConnection->GetConnectionState() == USOCK_Closed)
	{
		...
		UE_LOGF(LogDemo, Error, "UDemoNetDriver::ProcessPacket: ReceivedRawPacket closed connection");
...
void UDemoNetDriver::GotoTimeInSeconds(const float TimeInSeconds, const FOnGotoTimeDelegate& InOnGotoTimeDelegate)
{
	...
	UE_LOGF(LogDemo, Log, "GotoTimeInSeconds: %2.2f", TimeInSeconds);
...
{
	UE_LOGF(LogDemo, Log, "UDemoNetDriver::JumpToEndOfLiveReplay.");
...
void UDemoNetDriver::FinalizeFastForward(const double StartTime)
{
	...
	UE_LOGF(LogDemo, Log, "Fast forward took %.2f seconds.", FastForwardTotalSeconds);
}
...
bool UDemoNetDriver::SpawnSplitscreenViewer(ULocalPlayer* NewPlayer, UWorld* InWorld)
{
	...
	{
		UE_LOGF(LogDemo, Warning, "UDemoNetDriver::SpawnSplitscreenViewer: Local Player or World is invalid!");
		return false;
	}

	if (ClientConnections.Num() == 0 && ServerConnection == nullptr)
	{
		UE_LOGF(LogDemo, Error, "UDemoNetDriver::SpawnSplitscreenViewer: This netdriver has no demo connection data");
		return false;
	}
	...
	{
		UE_LOGF(LogDemo, Warning, "UDemoNetDriver::SpawnSplitscreenViewer: Unable to create new splitscreen controller");
		return false;
	}
...
{
	UE_LOGF(LogDemo, Log, "Attempting to remove splitscreen viewer!");
...
int32 UDemoNetDriver::CleanUpSplitscreenConnections(bool bDeleteOwner)
{
	...
	UE_LOGF(LogDemo, Log, "Cleaned up %d splitscreen connections, owner deletion: %ls", NumSplitscreenConnectionsCleaned, bDeleteOwner ? TEXT("enabled") : TEXT("disabled"));
...
void UDemoNetDriver::ReplayStreamingReady(const FStartStreamingResult& Result)
{
	...
	if (!Result.bRecording)
	{
		...
		if (SkipToLevelIndexOption)
		{
			...
			{
				UE_LOGF(LogDemo, Warning, "ReplayStreamingReady: SkipToLevelIndex was invalid: %d", Index);
			}
		}
		...
		if (CVarDemoJumpToEndOfLiveReplay.GetValueOnGameThread() != 0)
		{
			if (ReplayHelper.ReplayStreamer->IsLive() && ReplayHelper.ReplayStreamer->GetTotalDemoTime() > 15 * 1000)
			{
				...
				{
					UE_LOGF(LogDemo, Log, "UDemoNetDriver::ReplayStreamingReady: Deferring checkpoint until next available time.");
		...
		if (UE_LOG_ACTIVE(LogDemo, Log))
		{
			...
			UE_LOGF(LogDemo, Log, "ReplayStreamingReady: playing back replay [%ls] %ls, which was recorded on engine version %ls with flags [%ls]",
			...
			{
				UE_LOGF(LogDemo, Log, "ReplayStreamingReady: replay was recorded with: MinHz: %0.2f MaxHz: %0.2f FrameMS: %0.2f CheckpointMS: %0.2f Platform: [%ls] Config: [%ls] Target: [%ls]",
...
void UDemoNetDriver::RespawnNecessaryNetStartupActors(TArray<AActor*>& SpawnedActors, ULevel* Level /* = nullptr */)
{
	...
	for (auto RollbackIt = RollbackNetStartupActors.CreateIterator(); RollbackIt; ++RollbackIt)
	{
		...
		if (Actor)
		{
			...
			{
				UE_LOGF(LogDemo, Log, "RespawnNecessaryNetStartupActors: NetStartupRollbackActor name doesn't match original: %ls, %ls", *Actor->GetFullName(), *RollbackIt.Key());
			}
			...
			for (UObject* ObjRef : RollbackActor.ObjReferences)
			{
				if (!IsValid(ObjRef))
				{
					...
					UE_LOGF(LogDemo, Warning, "RespawnNecessaryNetStartupActors: Rollback actor reference was gc'd, skipping state restore: %ls", *GetFullNameSafe(Actor));
					break;
				}
			}
			...
			// BeginPlay
			{
				...
				QUICK_SCOPE_CYCLE_COUNTER(Stat_PostNetInit);
...
void UDemoNetDriver::PrepFastForwardLevels()
{
	...
	for (TWeakObjectPtr<UObject>& WeakLevel : ReplayHelper.NewStreamingLevelsThisFrame)
	{
		...
		if (ULevel* Level = CastChecked<ULevel>(WeakLevel.Get()))
		{
			...
			{
				UE_LOGF(LogDemo, Warning, "FastForwardLevels - NewStreamingLevel found in Pending list! %ls", *GetFullName(Level));
				continue;
			}
...
bool UDemoNetDriver::ProcessFastForwardPackets(TArrayView<FPlaybackPacket> Packets, const TSet<int32>& LevelIndices)
{
	...
	for (FPlaybackPacket& Packet : Packets)
	{
		...
		{
			UE_LOGF(LogDemo, Warning, "ProcessFastForwardPackets: Skipping packet with no seen level index");
			continue;
		}
		...
		if (ReplayHelper.SeenLevelStatuses.IsValidIndex(Packet.SeenLevelIndex - 1))
		{
			...
			if (bCareAboutLevel)
			{
				...
				{
					UE_LOGF(LogDemo, Warning, "FastForwardLevel failed to process packet");
					return false;
				}
			}
		}
		...
		{
			UE_LOGF(LogDemo, Warning, "FastForwardLevel could not process packet with invalid seen level index");
		}
	}
...
bool UDemoNetDriver::FastForwardLevels(const FGotoResult& GotoResult)
{
...

	{
		...
		if (CheckpointArchive->TotalSize() != 0)
		{
			...
			do 
			{
				...
				*CheckpointArchive << LevelIndex;
				...
				{
					UE_LOGF(LogDemo, Warning, "UDemoNetDriver::FastForwardLevels: Failed to read packets from Checkpoint.");
		...
		{
			UE_LOGF(LogDemo, Warning, "UDemoNetDriver::FastForwardLevels: Failed to read packets from Stream.");
...
bool UDemoNetDriver::LoadCheckpoint(const FGotoResult& GotoResult)
{
	...
	{
		UE_LOGF(LogDemo, Log, "LoadCheckpoint: SpectatorController is null and a valid GUID for null was found in the GuidCache. SpectatorController = %ls",
		...
		*GotoCheckpointArchive << NumValues;
		...
		for (int32 i = 0; i < NumValues; i++)
		{
				...
				*GotoCheckpointArchive << bExported;
				...
				else
				{
					...
					else
					{
						...
						UE_LOGF(LogDemo, Error, "Invalid guid path table index while serializing checkpoint.");
						break;
					}
				}
			}
			...
			*GotoCheckpointArchive << Flags;
			...
			{
				UE_LOGF(LogDemo, Error, "Guid cache serialization error while loading checkpoint.");
				return true;
			}
		}
		...
		for (FActorIterator ActorIt( World ); ActorIt; ++ActorIt)
		{
			...
			if (ReplayHelper.PlaybackDeletedNetStartupActors.Contains(FullName))
			{
				if (CurrentActor->bReplayRewindable)
				{
					...
					UE_LOGF(LogDemo, Warning, "Replay Rewindable Actor found in the DeletedNetStartupActors. Replay may show artifacts (%ls)", *FullName);
					continue;
				}
				...
				UE_LOGF(LogDemo, Verbose, "LoadCheckpoint: deleting startup actor %ls", *FullName);
...
bool UDemoNetDriver::ComparePropertyState(const FDemoSavedPropertyState& State) const
{
	...
	if (IsRecording())
	{
		for (const FDemoSavedRepObjectState& ObjectState : State)
		{
			...
			{
				UE_LOGF(LogDemo, Warning, "A replicated object was destroyed or marked pending kill since its state was saved!");
				bWasDifferent = true;
			}
		}
	}
...
void UDemoNetConnection::LowLevelSend(void* Data, int32 CountBits, FOutPacketTraits& Traits)
{
	...
	if (CountBytes == 0)
	{
		UE_LOGF(LogDemo, Warning, "UDemoNetConnection::LowLevelSend: Ignoring empty packet.");
		return;
	}
	...
	if (!DemoDriver)
	{
		UE_LOGF(LogDemo, Warning, "UDemoNetConnection::LowLevelSend: No driver found.");
		return;
	}

	if (CountBytes > FReplayHelper::MAX_DEMO_READ_WRITE_BUFFER)
	{
		UE_LOGF(LogDemo, Fatal, "UDemoNetConnection::LowLevelSend: CountBytes > MAX_DEMO_READ_WRITE_BUFFER.");
	}
	...
	{
		UE_LOGF(LogDemo, Warning, "UDemoNetConnection::LowLevelSend: Missing rep context.");
	}
}
...
void UDemoNetDriver::NotifyActorDestroyed(AActor* Actor, bool IsSeamlessTravel, EChannelCloseReason CloseReason)
{
	...
	if (bActorRewindable && !IsSeamlessTravel && !bIsRecording)
	{
		if (bNetStartup || !TrackedRewindActorsByGUID.Contains(GetNetGuidCache()->NetGUIDLookup.FindRef(Actor)))
		{
			...
			UE_LOGF(LogDemo, Warning, "Replay Rewindable Actor destroyed during playback. Replay may show artifacts (%ls)", *Actor->GetFullName());
		}
	}
	...
	if (World)
	{
		for (const FNetworkGUID& NetGUID : TrackedRewindActorsByGUID)
		{
			if (FNetGuidCacheObject* CacheObject = GetNetGuidCache()->ObjectLookup.Find(NetGUID))
			{
				...
				{
					UE_LOGF(LogDemo, Warning, "CleanupOutstandingRewindActors - Invalid object for %ls, skipping.", *NetGUID.ToString());
					continue;
				}
			}	
			else
			{
				UE_LOGF(LogDemo, Warning, "CleanupOutstandingRewindActors - CacheObject not found for %ls", *NetGUID.ToString());
			}
		}
	}
...
void UDemoNetDriver::QueueNetStartupActorForRollbackViaDeletion(AActor* Actor)
{
	...
	{
		UE_LOGF(LogDemo, Warning, "Attempted to queue a Replay Rewindable Actor for Rollback Via Deletion. Replay may have artifacts (%ls)", *GetFullNameSafe(Actor));
		return;
	}
...
{
	UE_LOGF(LogDemo, Warning, "NotifyDemoPlaybackFailure: %ls", EDemoPlayFailure::ToString(FailureType));
...
void UDemoNetDriver::AdjustConsiderTime(const float ReplicatedPercent)
{
	if (MaxDesiredRecordTimeMS > 0.f)
	{
		...
		if (ReplicatedPercent > IncreaseThreshold)
		{
			...
			UE_LOGF(LogDemo, Verbose, "AdjustConsiderTime: RecordBuildConsiderAndPrioritizeTimeSlice is now %0.1f", RecordBuildConsiderAndPrioritizeTimeSlice)
		}
...
bool UDemoPendingNetGame::LoadMapCompleted(UEngine* Engine, FWorldContext& Context, bool bInLoadedMapSuccessfully, const FString& LoadMapError)
{
	...
	if (!bLoadedMapSuccessfully)
	{
		...
		UE_LOGF(LogDemo, Error, "UDemoPendingNetGame::HandlePostLoadMap: LoadMap failed: %ls", *LoadMapError);
