...
void FSplineMeshComponentDescriptorBase::InitFrom(const UStaticMeshComponent* Template, bool bInitBodyInstance)
{
	...
	const USplineMeshComponent* SplineTemplate = Cast<USplineMeshComponent>(Template);

	// Disregard the template class if it does not stem from a spline mesh component
	if (SplineTemplate)
	{
		ComponentClass = SplineTemplate->GetClass();
	}
	...
	bNeverNeedsCookedCollisionData = SplineTemplate ? SplineTemplate->GetbNeverNeedsCookedCollisionData() : false;
...
bool FSplineMeshComponentDescriptorBase::operator==(const FSplineMeshComponentDescriptorBase& Other) const
{
	...
	bNeverNeedsCookedCollisionData == Other.bNeverNeedsCookedCollisionData &&
#if WITH_EDITORONLY_DATA
...
void FSplineMeshComponentDescriptorBase::InitComponent(USplineMeshComponent* SplineMeshComponent) const
{
	...
	SplineMeshComponent->bVisibleInReflections = bVisibleInReflections;
	...
	SplineMeshComponent->SetbNeverNeedsCookedCollisionData(bNeverNeedsCookedCollisionData);
