...
UTextureCube* UTextureCube::CreateTransient(int32 InSizeX, int32 InSizeY, EPixelFormat InFormat, const FName InName)
{
	...
	{
		UE_LOGF(LogTexture, Warning, "Invalid parameters specified for UTextureCube::CreateTransient()");
	}
	return NewTexture;
}
...
void UTextureCube::Serialize(FArchive& Ar)
{
	...
	if (Ar.IsLoading() && Source.GetNumSlices() == 1 && MaxTextureSize == 0 && Ar.CustomVer(FReleaseObjectVersion::GUID) < FReleaseObjectVersion::LonglatTextureCubeDefaultMaxResolution)
	{
		...
		MaxTextureSize = 512;
		UE_LOGF(LogTexture, Log, "Default maximum texture size for cubemaps generated from long-lat sources has been changed from 512 to unlimited. In order to preserve old behavior for '%ls', its maximum texture size has been explicitly set to 512.", *GetPathName());
	}
...
void UTextureCube::GetAssetRegistryTags(FAssetRegistryTagsContext Context) const
{
	...
	// Our source may be a LongLat cubemap which is a single texture
	const TCHAR* SourceFaceCount = Source.IsLongLatCubemap() ? TEXT("") : TEXT("x6");
#else
	int32 SizeX = 0;
	int32 SizeY = 0;
	...
	const FString Dimensions = FString::Printf(TEXT("%dx%d%s"), SizeX, SizeY, SourceFaceCount);
	...
	virtual void InitRHI(FRHICommandListBase& RHICmdList) override
	{
		...
		for( int32 FaceIndex=0; FaceIndex<6; FaceIndex++ )
		{
			for(int32 MipIndex=0; MipIndex < NumMips; MipIndex++)
			{
				...
				{
					uint32 DestStride;
					void* TheMipData = RHICmdList.GetAsImmediate().LockTextureCubeFace( TextureCubeRHI, FaceIndex, 0, MipIndex, RLM_WriteOnly, DestStride, false );
					GetData( FaceIndex, MipIndex, TheMipData, DestStride );
					RHICmdList.GetAsImmediate().UnlockTextureCubeFace( TextureCubeRHI, FaceIndex, 0, MipIndex, false );
				}
			}
		}
...
bool UTextureCube::NeedsLoadForTargetPlatform(const ITargetPlatform* TargetPlatform) const
{
	...
	{
		const static FName EncodedHDR("EncodedHDR");
