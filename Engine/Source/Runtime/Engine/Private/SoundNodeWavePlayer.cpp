...
void USoundNodeWavePlayer::LoadAsset(bool bAddToRoot)
{
	if (IsAsyncLoadingMultithreaded())
	{
		...
		if (SoundWave == nullptr)
		{
			...
			{
				UE_LOGF(LogAudio, VeryVerbose, " '%ls:%ls', Async loading...", *GetNameSafe(GetOuter()), *GetName());
				bAsyncLoading = true;
	...
	{
		UE_LOGF(LogAudio, VeryVerbose, "'%ls:%ls', DOING SYNCRONOUS loading... %ls",
...
void USoundNodeWavePlayer::ParseNodes( FAudioDevice* AudioDevice, const UPTRINT NodeWaveInstanceHash, FActiveSound& ActiveSound, const FSoundParseParameters& ParseParams, TArray<FWaveInstance*>& WaveInstances )
{
	if (bAsyncLoading)
	{
		UE_LOGF(LogAudio, Verbose, "Asynchronous load of %ls not complete in USoundNodeWavePlayer::ParseNodes, will attempt to play later.", *GetFullNameSafe(this));
	...
	{
		UE_LOGF(LogAudio, Warning, "Asynchronous load of %ls required in USoundNodeWavePlayer::ParseNodes when we attempted to play, likely because the quality node was changed.", *GetFullNameSafe(this));
