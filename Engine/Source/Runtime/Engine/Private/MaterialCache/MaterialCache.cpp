...
static TAutoConsoleVariable CVarMaterialCacheAllocationMaxTileCountX(
	TEXT("r.MaterialCache.AllocationMaxTileCountWidth"),
	1024,
	TEXT("Maximum number of tiles (per axis) of each allocation"),
	ECVF_ReadOnly
);
...
FIntPoint GetMaterialCacheTileCount(float TriangleArea, float TexelSize, TOptional<FIntPoint> MaxTileCount)
{	
	float TexelCountUniform = TriangleArea / TexelSize;
	float TileCountUniform  = TexelCountUniform / FMath::Square(static_cast<float>(GetMaterialCacheTileWidth()));
	float TileCountX        = FMath::Max(1u, FMath::Sqrt(TileCountUniform));
	
	TileCountX = FMath::RoundUpToPowerOfTwo(TileCountX);
	
	if (!MaxTileCount)
	{
		MaxTileCount = FIntPoint(CVarMaterialCacheAllocationMaxTileCountX->GetInt());
	}
	
	return FIntPoint(TileCountX, TileCountX).ComponentMin(MaxTileCount.GetValue());
}
