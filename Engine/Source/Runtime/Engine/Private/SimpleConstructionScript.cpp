...
{
	void ConformArchetypeVisibility(UObject* Template)
	{
		if(!Template->HasAllFlags(RF_Public))
		{
			return;
		}
		...
		{
			// In theory it might be good to propagate to all subobjects of
			// the template object, but I'm being conservative for now:
			if(SubObject->HasAnyFlags(RF_ArchetypeObject))
			{
				if(!SubObject->HasAllFlags(RF_Public))
				{
					UE_LOGFMT(LogBlueprint, Warning, "Conforming subobject flags: {subobject}", SubObject->GetPathName());
				}
...
template <typename T>
class TSimpleConstructionScriptMutexScope
{
private:
	T& Mutex;
public:
	TSimpleConstructionScriptMutexScope(T& InMutex)
		: Mutex(InMutex)
	{
		if (!Mutex.TryLock())
		{
			TRACE_CPUPROFILER_EVENT_SCOPE(SimpleConstructionScriptMutexContention);
			Mutex.Lock();
		}
	}

	~TSimpleConstructionScriptMutexScope()
	{
		Mutex.Unlock();
	}
};
...
void USimpleConstructionScript::PreloadChain()
{
	...
	TSimpleConstructionScriptMutexScope ScopeLock(Mutex);
...
void USimpleConstructionScript::PostLoad()
{
	...
	if (!Blueprint)
	{
		...
		UE_LOGF(LogBlueprint, Warning, "USimpleConstructionScript::PostLoad() '%ls' cannot find its owner blueprint", *GetPathName());
		return;
	}
#endif // WITH_EDITOR

	// Protect AllNodes/RootNodes/DefaultSceneRootNode against concurrent reads from ALT
	// (via FindArchetype -> FindSCSNode/FindSCSNodeByGuid/PreloadChain).
	TSimpleConstructionScriptMutexScope ScopeLock(Mutex);
					...
					for (USCS_Node* Node : RootNodes)
					{
						if(Node->ParentComponentOrVariableName == NAME_None)
						{
							...
							if (USceneComponent* SceneComponentTemplate = Cast<USceneComponent>(Node->ComponentTemplate))
							{
								...
								{
									UE_LOGF(LogBlueprint, Warning, "%ls: Found non-native root component custom scale for %ls (%ls) saved prior to being usable; reverting to default scale.", *BPGeneratedClass->GetName(), *Node->GetVariableName().ToString(), *ComponentRelativeScale3D.ToString());
		...
		void FixupOrphanedNodes(USCS_Node* SceneRootNodeIn, USceneComponent* RootComponentTemplate, const bool bThisOwnsSceneRootIn)
		{
			...
			for (USCS_Node* Orphan : OrphanedNodes)
			{
				...
				auto AddToRootSet = [&RootNodeListRef](USCS_Node* Node)
				{
					...
					{
						UE_LOGF(LogBlueprint, Warning, "Found orphaned component ('%ls') and added it to the Blueprint's root set. Please validate the component hierarchy is as wanted and resave.",
	...
	private:
		...
		void FixupParentage(USCS_Node* Node)
		{
			if (PendingParent)
			{
				...
				{
					UE_LOGF(LogBlueprint, Warning, "Reparenting the '%ls' component (now nested under '%ls)' - possible cyclic linkage? Please validate the component hierarchy and resave the Blueprint.",
						*Node->GetVariableName().ToString(),
				...
				{
					UE_LOGF(LogBlueprint, Warning, "The '%ls' component is being removed from the root set and nested under '%ls' - possible cyclic linkage? Please validate the component hierarchy and resave the Blueprint.",
						*Node->GetVariableName().ToString(),
						*PendingParent->GetVariableName().ToString()
					);
				}
...
void USimpleConstructionScript::FixupRootNodeParentReferences()
{
	...
	{
		UE_LOGF(LogBlueprint, Warning, "USimpleConstructionScript::FixupRootNodeParentReferences() - owner class is NULL; skipping.");
	...
	for (int32 NodeIndex=0; NodeIndex < RootNodes.Num(); ++NodeIndex)
	{
		...
		if(RootNode->ParentComponentOrVariableName != NAME_None)
		{
			...
			if(!bWasFound)
			{
				UE_LOGF(LogBlueprint, Warning, "USimpleConstructionScript::FixupRootNodeParentReferences() - Couldn't find %ls parent component '%ls' for '%ls' in BlueprintGeneratedClass '%ls' (it may have been removed)", RootNode->bIsParentComponentNative ? TEXT("native") : TEXT("inherited"), *RootNode->ParentComponentOrVariableName.ToString(), *RootNode->GetVariableName().ToString(), *BPGeneratedClass->GetName());
...
void USimpleConstructionScript::RemoveNodeAndPromoteChildren(USCS_Node* Node)
{
	...
	else
	{
		...
		{
#if WITH_EDITOR
			UE_LOGF(LogBlueprint, Error, "RemoveNodeAndPromoteChildren(%ls) failed to find a parent node in Blueprint %ls, attaching children to the root", *Node->GetName(), *GetBlueprint()->GetPathName());
#endif
...
// Evicts any orphaned _GEN_VARIABLE object that occupies VariableName's slot in TargetOuter (e.g. a
// template left behind by an incomplete undo/redo), then renames ComponentTemplate into that slot.
static void RenameComponentTemplateIntoClass(UActorComponent* ComponentTemplate, UObject* TargetOuter, FName VariableName)
{
	const FString FullTemplateName = VariableName.ToString() + USimpleConstructionScript::ComponentTemplateNameSuffix;
	UObject* Collision = FindObject<UObject>(TargetOuter, *FullTemplateName);
	while (Collision)
	{
		Collision->Rename(nullptr, GetTransientPackage(), REN_DoNotDirty | REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
		Collision = FindObject<UObject>(TargetOuter, *FullTemplateName);
	}
	ComponentTemplate->Rename(*FullTemplateName, TargetOuter, REN_DoNotDirty | REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
}
...
USCS_Node* USimpleConstructionScript::CreateNode(UClass* NewComponentClass, FName NewComponentVariableName)
{
	...
	RenameComponentTemplateIntoClass(NewComponentTemplate, Blueprint->GeneratedClass, NewComponentVariableName);
...
USCS_Node* USimpleConstructionScript::CreateNodeAndRenameComponent(UActorComponent* NewComponentTemplate)
{
	...
	// evicting any orphaned template that may occupy the slot.
	RenameComponentTemplateIntoClass(NewComponentTemplate, GetBlueprint()->GeneratedClass, NewComponentVariableName);
...
#if WITH_EDITOR
			UE::Private::ConformArchetypeVisibility(Node->ComponentTemplate);
#endif//WITH_EDITOR
		}
	}
}
