		...
		if (GConfig)
		{
			...
			for (FConfigSection::TConstIterator It(*PackageRedirects); It; ++It)
			{
				...
				if (It.Key() == TEXT("K2ParamRedirects"))
				{
					...
					{
						UE_LOGF(LogBlueprint, Warning, "Unequal lengths for old and new param values for  param redirect '%ls' to '%ls'.", *(OldParam.ToString()), *(NewParam.ToString()));
					}

					if (CustomValueMapping.Len() > 0 && (OldParamValuesList.Num() > 0 || NewParamValuesList.Num() > 0))
					{
						UE_LOGF(LogBlueprint, Warning, "Both Custom and Automatic param value remapping specified for param redirect '%ls' to '%ls'.  Only Custom will be applied.", *(OldParam.ToString()), *(NewParam.ToString()));
					}
					...
					for (int32 i = FMath::Min(OldParamValuesList.Num(), NewParamValuesList.Num()) - 1; i >= 0; --i)
					{
						...
						{
							UE_LOGF(LogBlueprint, Warning, "Duplicate old param value '%ls' for param redirect '%ls' to '%ls'.", *(OldParamValuesList[i]), *(OldParam.ToString()), *(NewParam.ToString()));
						}
					}
				}			
			}
...
TFieldType* FMemberReference::ResolveMemberImpl(UClass* SelfScope, TFieldTypeClass* FieldClass, const bool bAlwaysFollowRedirects, const bool bIsUFunctionOrMulticastDelegate) const
{
	...
	{
		UE_LOGF(LogBlueprint, Warning, "FMemberReference::ResolveMember (%ls) bSelfContext == true, but no scope supplied!", *MemberName.ToString());
	}
	...
	else
	{
		...
		else if (MemberName.ToString().EndsWith(HEADER_GENERATED_DELEGATE_SIGNATURE_SUFFIX))
		{
			...
			if (ReturnField != nullptr)
			{
				UE_LOGF(LogBlueprint, Display, "Generic delegate signature ref (%ls). Explicitly setting it to: '%ls'. Make sure this is correct (there could be multiple native delegate types with this name).", *MemberName.ToString(), *ReturnField->GetPathName());
	...
	while (TestRemapClass != nullptr)
	{
		...
		if (NewRedirectName != OldRedirectName)
		{
			...
			if (OldRedirectName.OuterName != NewRedirectName.OuterName)
			{
				...
				if (!SearchClass)
				{
					UE_LOGF(LogBlueprint, Log, "UK2Node:  Unable to find updated field name for '%ls' on unknown class '%ls'.", *InitialName.ToString(), *ClassName);
					return nullptr;
				}
			}
		}
		...
		if (NewFieldName != NAME_None)
		{
			...
			if (NewField != nullptr)
			{
				...
				{
					UE_LOGF(LogBlueprint, Log, "UK2Node:  Unable to update field. Remapped field '%ls' in not owned by given scope. Scope: '%ls', Owner: '%ls'.", *InitialName.ToString(), *InitialScope->GetName(), *NewFieldName.ToString());
				}
				else
				{
					UE_LOGF(LogBlueprint, Log, "UK2Node:  Fixed up old field '%ls' to new name '%ls' on class '%ls'.", *InitialName.ToString(), *NewFieldName.ToString(), *SearchClass->GetName());
					return NewField;
				}
			}
			else
			{
				UE_LOGF(LogBlueprint, Log, "UK2Node:  Unable to find updated field name for '%ls' on class '%ls'.", *InitialName.ToString(), *SearchClass->GetName());
			}
