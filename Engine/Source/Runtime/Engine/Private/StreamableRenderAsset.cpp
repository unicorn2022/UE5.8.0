...
const TCHAR* GNoRefBiasQualityLevelCVarName = TEXT("r.Streaming.NoRefLODBiasQualityLevel");
const TCHAR* GNoRefBiasQualityLevelScalabilitySection = TEXT("ViewDistanceQuality");
	...
	: Super(ObjectInitializer)
{
	...
	{
		// Pre-publish: no other thread has a reference to this asset yet, so ordering is trivial.
		// The wrapper's operator= performs a release-store (default), which is more than enough.
		SSAMAssetHandle = FSimpleStreamableAssetManager::AllocateAssetHandle();
	}
...
void UStreamableRenderAsset::RegisterMipLevelChangeCallback(IPrimitiveComponent* Component, int32 LODIndex, float TimeoutSecs, bool bOnStreamIn, FLODStreamingCallback&& Callback)
{
...
void UStreamableRenderAsset::RegisterMipLevelChangeCallback(IPrimitiveComponent* Component, float TimeoutStartSecs, FLODStreamingCallback&& CallbackStreamingStart, float TimeoutDoneSecs, FLODStreamingCallback&& CallbackStreamingDone)
{
	check(IsInGameThread());
...
void UStreamableRenderAsset::RemoveMipLevelChangeCallback(IPrimitiveComponent* Component)
{
	check(IsInGameThread());
...
void UStreamableRenderAsset::RemoveMipLevelChangeCallback(UPrimitiveComponent* Component)
{
	if (Component)
	{
		RemoveMipLevelChangeCallback(Component->GetPrimitiveComponentInterface());
	}
}
...
void UStreamableRenderAsset::TickStreaming(ETickStreamingFlags Flags, TArray<UStreamableRenderAsset*>* DeferredTickCBAssets)
{
...
#if WITH_EDITOR
			if (StreamableRenderAsset::bAllowUpdateResourceSize && GIsEditor && EnumHasAnyFlags(Flags, ETickStreamingFlags::SendCompletionEvents))
			{
	...
	if (EnumHasAnyFlags(Flags, ETickStreamingFlags::SendCompletionEvents))
	{
...
bool UStreamableRenderAsset::HasPendingInitOrStreaming() const 
{
	if (!!PendingUpdate)
	{
		return true;
	...
	return HasPendingRenderResourceInitialization();
}
...
void UStreamableRenderAsset::WaitForPendingInitOrStreaming(ETickStreamingFlags Flags)
{
	...
	while (HasPendingInitOrStreaming())
	{
...
void UStreamableRenderAsset::WaitForStreaming(ETickStreamingFlags Flags)
{
	// Complete pending streaming so that the asset can executing new requests if needed.
	WaitForPendingInitOrStreaming(Flags);
...
void UStreamableRenderAsset::BeginDestroy()
{
	...
	if (FSimpleStreamableAssetManager::IsEnabled())
	{
		// Atomically capture-and-clear the SSAM handle and queue it for release. With SSAM
		// enabled the handle is always valid here: the ctor allocates one, and UnregisterAsset
		// preserves the "alive asset has a valid handle" invariant by swapping in a fresh
		// replacement before queueing the old one for release -- so BeginDestroy still has a
		// handle to reclaim even if UnregisterAsset already ran.
		//
		// The atomic Exchange must run from the open (AutoRTFM compiler cannot emit a closed
		// clone over std::atomic ops); OnCommit must be registered from the closed (OnCommit
		// from an open nest runs immediately, defeating the deferral). The reconstruction path
		// snapshots the full UObject memory via RecordOpenWrite before BeginDestroy, so the
		// Exchange is implicitly rolled back on abort. The queue push mutates global SSAM state
		// outside that snapshot, so it must be deferred via OnCommit -- which is a no-op outside
		// transactions and skipped on abort.
		const FSSAMAssetHandle Captured = AutoRTFM::Open([&]
		{
			return SSAMAssetHandle.Exchange(FSSAMAssetHandle{});
		});

		AutoRTFM::OnCommit([Captured]
		{
			FSimpleStreamableAssetManager::QueueRegisteredAssetHandleRelease(Captured);
		});
	}
}
