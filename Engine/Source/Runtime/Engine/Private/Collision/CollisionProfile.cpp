...
constexpr uint32 MaxNumCollisionChannels = 64;
...
FName UCollisionProfile::CustomCollisionProfileName = FName("Custom");
...
ENGINE_API const FName UCollisionProfile::NoCollision_ProfileName = FName("NoCollision");
ENGINE_API const FName UCollisionProfile::BlockAll_ProfileName = FName("BlockAll");
ENGINE_API const FName UCollisionProfile::OverlapAll_ProfileName = FName("OverlapAll");
ENGINE_API const FName UCollisionProfile::PhysicsActor_ProfileName = FName("PhysicsActor");
ENGINE_API const FName UCollisionProfile::BlockAllDynamic_ProfileName = FName("BlockAllDynamic");
ENGINE_API const FName UCollisionProfile::Pawn_ProfileName = FName("Pawn");
ENGINE_API const FName UCollisionProfile::Vehicle_ProfileName = FName("Vehicle");
ENGINE_API const FName UCollisionProfile::DefaultProjectile_ProfileName = FName("DefaultProjectile");
...
void UCollisionProfile::LoadProfileConfig(bool bForceInit)
{
	...
	for (auto Iter=Profiles.CreateConstIterator(); Iter; ++Iter)
	{
		...
		{
			UE_LOGF(LogCollisionProfile, Error, "Profiles contain invalid name : %ls is reserved for internal use", *CustomCollisionProfileName.ToString());
	...
	FString EngineTraceChannel	= "EngineTraceChannel";
	FString GameTraceChannel	= "GameTraceChannel";
	...
	check (Struct);
	static const FString DisplayNameKey = "DisplayName";
	static const FString ScriptNameKey = "ScriptName";
	static const FString TraceType = "TraceQuery";
	static const FString TraceValue = "1";
	static const FString HiddenMeta = "Hidden";
	...
	static const FString Prefix = "ECC_";

	auto DisplayNameToScriptName = 
		[](const FString& DisplayName, FStringView DeprecatedScriptName = FStringView())
		{
			FString ScriptName = DisplayName;
			ScriptName.RemoveSpacesInline();
			if (!ScriptName.StartsWith(Prefix))
			{
				ScriptName.InsertAt(0, Prefix);
			}
			if (!DeprecatedScriptName.IsEmpty())
			{
				ScriptName += TEXT(';');
				ScriptName += DeprecatedScriptName;
			}
			return ScriptName;
		};
	...
	for (int32 ChannelResponseIndex = 0; ChannelResponseIndex < DefaultChannelResponses.Num(); ++ChannelResponseIndex)
	{
		...
		if ( IS_VALID_COLLISIONCHANNEL(EnumIndex) )
		{
			if ( CustomChannel.Name != NAME_None )
			{
				...
				{
					UE_LOGF(LogCollisionProfile, Warning, "Cannot map multiple responses to the same collision channel (%d); ignoring '%ls' ", EnumIndex, *DisplayValue);
				...
				FString ScriptValue = DisplayNameToScriptName(DisplayValue);
			...
			{
				// missing name
				UE_LOGF(LogCollisionProfile, Warning, "Name can't be empty for Channel (%d) ", EnumIndex );
			}
		...
		else
		{
			...
			UE_LOGF(LogCollisionProfile, Warning, "Default Setup doesn't allow for predefined engine channels (%d) ", EnumIndex);
		}
	}
			...
			if ( !DisplayName.IsEmpty() )
			{
				...
				if (Enum->GetMetaData(*TraceType, EnumIndex) == TraceValue)
				{
					...
					if (ensure(TraceTypeIndex != INDEX_NONE))
					{
						...
						TraceTypeEnum->SetMetaData(*ScriptNameKey, ScriptName.IsEmpty() ? *DisplayNameToScriptName(DisplayName, TraceTypeEnum->GetNameStringByIndex(TraceTypeIndex)) : *ScriptName, TraceTypeIndex);
				...
				else
				{
					...
					if (ensure(ObjectTypeIndex != INDEX_NONE))
					{
						...
						ObjectTypeEnum->SetMetaData(*ScriptNameKey, ScriptName.IsEmpty() ? *DisplayNameToScriptName(DisplayName, ObjectTypeEnum->GetNameStringByIndex(ObjectTypeIndex)) : *ScriptName, ObjectTypeIndex);
	...
	for(auto Iter = CollisionChannelRedirects.CreateConstIterator(); Iter; ++Iter)
	{
		...
		{
			// print error 
			UE_LOGF(LogCollisionProfile, Warning, "CollisionChannel Redirects : Name Can't be none (%ls: %ls)", *OldName.ToString(), *NewName.ToString());
		}
	}
	...
	for(auto Iter = ProfileRedirects.CreateConstIterator(); Iter; ++Iter)
	{
		...
		if(OldName!= NAME_None && NewName != NAME_None)
		{
			...
			{
				// print error
				UE_LOGF(LogCollisionProfile, Warning, "ProfileRedirect (%ls : %ls) - New Name (\'%ls\') isn't found ",
	...
	for (int32 ProfileIndex = 0; ProfileIndex<ProfileList.Num(); ++ProfileIndex)
	{
		...
		if (Template.ObjectTypeName!=NAME_None)
		{
			...
			if (EnumIndex != INDEX_NONE)
			{
				...
				{
					UE_LOGF(LogCollisionProfile, Warning, "Profile (%ls) ObjectTypeName (%ls) is Trace Type. You can set Object Type Channel to Object Type.",
			...
			{
				UE_LOGF(LogCollisionProfile, Warning, "Profile (%ls) ObjectTypeName (%ls) is invalid. ", 
					*Template.Name.ToString(), *Template.ObjectTypeName.ToString());
...
int32 UCollisionProfile::LoadCustomResponses(FCollisionResponseTemplate& Template, const UEnum* CollisionChannelEnum, TArray<FResponseChannel>& CustomResponses) const
{
	...
	for (auto ChannelIter = CustomResponses.CreateIterator(); ChannelIter; ++ChannelIter)
	{
		...
		{
			// print error
			UE_LOGF(LogCollisionProfile, Warning, "Profile (%ls) - Custom Channel Name = \'%ls\' hasn't been found", 
...
void UCollisionProfile::SaveCustomResponses(FCollisionResponseTemplate& Template) const
{
	...
	for(int32 Index=0; Index<MaxNumCollisionChannels; ++Index)
	{
