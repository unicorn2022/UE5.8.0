...
static bool ConvertOverlappedShapeToImpactHit(const UWorld* World, const THitLocation& Hit, const FVector& StartLoc, const FVector& EndLoc, FHitResult& OutResult, const FPhysicsGeometry& Geom, const FTransform& QueryTM, const Chaos::Filter::FQueryFilterData& QueryFilter, bool bReturnPhysMat);
...
static void CheckHitResultNormal(const FHitResult& OutResult, const TCHAR* Message, const FVector& Start=FVector::ZeroVector, const FVector& End = FVector::ZeroVector, const FPhysicsGeometry* Geom=nullptr, const FVector& NormalPreSafeNormalize = FVector::ZeroVector)
{
	...
	{
		UE_LOGF(LogPhysics, Warning, "(%ls) Non-normalized OutResult.Normal from hit conversion: %ls (Component- %ls)", Message, *OutResult.Normal.ToString(), *OutResult.Component->GetFullName());
		UE_LOGF(LogPhysics, Warning, "Start Loc(%ls), End Loc(%ls), Hit Loc(%ls), ImpactNormal(%ls) NormalPreSafeNormalize(%ls)", *Start.ToString(), *End.ToString(), *OutResult.Location.ToString(), *OutResult.ImpactNormal.ToString(), *NormalPreSafeNormalize.ToString() );
		if (Geom)
		{
			if (GetType(*Geom) == ECollisionShapeType::Capsule)
			{
				const FPhysicsCapsuleGeometry& Capsule = (FPhysicsCapsuleGeometry&)*Geom;
				UE_LOGF(LogPhysics, Warning, "Capsule radius (%f), Capsule Halfheight (%f)", GetRadius(Capsule), GetHalfHeight(Capsule));
			}
		}
...
static void SetHitResultFromShapeAndFaceIndex(const FPhysicsShape& Shape,  const FPhysicsActor& Actor, const uint32 FaceIndex, const FVector& HitLocation, FHitResult& OutResult, bool bReturnPhysMat)
{
	...
	else if (const FBodyInstance* BodyInst = GetUserData(Actor))
	{
		...
		// Use GetActorReferenceWithWelding() instead of GetPhysicsActor() so that
		// welded children resolve to their parent's PhysicsObject rather than returning null.
		// When a body is welded, its shapes are merged into the parent particle,
		// so the parent's PhysicsObject is the correct one for force/impulse application.
		const FPhysicsActorHandle& PhysicsActor = BodyInst->GetActorReferenceWithWelding();
		OutResult.PhysicsObject = PhysicsActor ? PhysicsActor->GetPhysicsObject() : nullptr;
	}
	else if (Chaos::FPhysicsObject* PhysObject = GetPhysicsObjectFromUserData(Actor))
	{
		OutResult.PhysicsObject = PhysObject;
		OutResult.Item = Chaos::FPhysicsObjectInterface::GetId(PhysObject);
		OutResult.BoneName = Chaos::FPhysicsObjectInterface::GetName(PhysObject);
	}
	else
	{
...
template <typename THitLocation>
EConvertQueryResult ConvertQueryImpactHitImp(const UWorld* World, const THitLocation& Hit, FHitResult& OutResult, float CheckLength, const Chaos::Filter::FQueryFilterData& QueryFilter, const FVector& StartLoc, const FVector& EndLoc, const FPhysicsGeometry* Geom, const FTransform& QueryTM, bool bReturnFaceIndex, bool bReturnPhysMat)
{
	using namespace ChaosInterface;
...
#if WITH_EDITOR
	if(bReturnFaceIndex && World && World->IsGameWorld())
	{
	...
	const Chaos::Filter::FShapeFilterData ShapeFilter = GetShapeFilterData(HitShape);
	...
	if (bUseReturnedPoint)
	{
		...
		if (Position.ContainsNaN() && GTActor && GTShape)
		{
			...
			UE_LOGF(LogCore, Error, "ConvertQueryImpactHit() NaN details:\n>> Actor:%ls (%ls)\n>> Component:%ls\n>> Item:%d\n>> BoneName:%ls\n>> Time:%f\n>> Distance:%f\n>> Location:%ls\n>> bIsBlocking:%d\n>> bStartPenetrating:%d",
...
EConvertQueryResult ConvertQueryImpactHit(const UWorld* World, const FHitLocation& Hit, FHitResult& OutResult, float CheckLength, const Chaos::Filter::FQueryFilterData& QueryFilter, const FVector& StartLoc, const FVector& EndLoc, const FPhysicsGeometry* Geom, const FTransform& QueryTM, bool bReturnFaceIndex, bool bReturnPhysMat)
{
...
EConvertQueryResult ConvertQueryImpactHit(const UWorld* World, const ChaosInterface::FPTLocationHit& Hit, FHitResult& OutResult, float CheckLength, const Chaos::Filter::FQueryFilterData& QueryFilter, const FVector& StartLoc, const FVector& EndLoc, const FPhysicsGeometry* Geom, const FTransform& QueryTM, bool bReturnFaceIndex, bool bReturnPhysMat)
{
	return ConvertQueryImpactHitImp(World, Hit, OutResult, CheckLength, QueryFilter, StartLoc, EndLoc, Geom, QueryTM, bReturnFaceIndex, bReturnPhysMat);
}

template <typename HitType>
EConvertQueryResult ConvertTraceResults(bool& OutHasValidBlockingHit, const UWorld* World, int32 NumHits, HitType* Hits, float CheckLength, const Chaos::Filter::FQueryFilterData& QueryFilter, TArray<FHitResult>& OutHits, const FVector& StartLoc, const FVector& EndLoc, const FPhysicsGeometry* Geom, const FTransform& QueryTM, float MaxDistance, bool bReturnFaceIndex, bool bReturnPhysMat)
{
...
EConvertQueryResult ConvertTraceResults(bool& OutHasValidBlockingHit, const UWorld* World, int32 NumHits, Hit* Hits, float CheckLength, const Chaos::Filter::FQueryFilterData& QueryFilter, FHitResult& OutHit, const FVector& StartLoc, const FVector& EndLoc, const FPhysicsGeometry* Geom, const FTransform& QueryTM, float MaxDistance, bool bReturnFaceIndex, bool bReturnPhysMat)
{
	const FVector Dir = (EndLoc - StartLoc).GetSafeNormal();
...
template EConvertQueryResult ConvertTraceResults<FHitSweep>(bool& OutHasValidBlockingHit, const UWorld* World, int32 NumHits, FHitSweep* Hits, float CheckLength, const Chaos::Filter::FQueryFilterData& QueryFilter, TArray<FHitResult>& OutHits, const FVector& StartLoc, const FVector& EndLoc, const FPhysicsGeometry* Geom, const FTransform& QueryTM, float MaxDistance, bool bReturnFaceIndex, bool bReturnPhysMat);
template EConvertQueryResult ConvertTraceResults<FHitSweep>(bool& OutHasValidBlockingHit, const UWorld* World, int32 NumHits, FHitSweep* Hits, float CheckLength, const Chaos::Filter::FQueryFilterData& QueryFilter, FHitResult& OutHit, const FVector& StartLoc, const FVector& EndLoc, const FPhysicsGeometry* Geom, const FTransform& QueryTM, float MaxDistance, bool bReturnFaceIndex, bool bReturnPhysMat);
template EConvertQueryResult ConvertTraceResults<ChaosInterface::FPTSweepHit>(bool& OutHasValidBlockingHit, const UWorld* World, int32 NumHits, ChaosInterface::FPTSweepHit* Hits, float CheckLength, const Chaos::Filter::FQueryFilterData& QueryFilter, TArray<FHitResult>& OutHits, const FVector& StartLoc, const FVector& EndLoc, const FPhysicsGeometry* Geom, const FTransform& QueryTM, float MaxDistance, bool bReturnFaceIndex, bool bReturnPhysMat);
template EConvertQueryResult ConvertTraceResults<ChaosInterface::FPTSweepHit>(bool& OutHasValidBlockingHit, const UWorld* World, int32 NumHits, ChaosInterface::FPTSweepHit* Hits, float CheckLength, const Chaos::Filter::FQueryFilterData& QueryFilter, FHitResult& OutHit, const FVector& StartLoc, const FVector& EndLoc, const FPhysicsGeometry* Geom, const FTransform& QueryTM, float MaxDistance, bool bReturnFaceIndex, bool bReturnPhysMat);
template EConvertQueryResult ConvertTraceResults<FHitRaycast>(bool& OutHasValidBlockingHit, const UWorld* World, int32 NumHits, FHitRaycast* Hits, float CheckLength, const Chaos::Filter::FQueryFilterData& QueryFilter, TArray<FHitResult>& OutHits, const FVector& StartLoc, const FVector& EndLoc, const FPhysicsGeometry* Geom, const FTransform& QueryTM, float MaxDistance, bool bReturnFaceIndex, bool bReturnPhysMat);
template EConvertQueryResult ConvertTraceResults<FHitRaycast>(bool& OutHasValidBlockingHit, const UWorld* World, int32 NumHits, FHitRaycast* Hits, float CheckLength, const Chaos::Filter::FQueryFilterData& QueryFilter, FHitResult& OutHit, const FVector& StartLoc, const FVector& EndLoc, const FPhysicsGeometry* Geom, const FTransform& QueryTM, float MaxDistance, bool bReturnFaceIndex, bool bReturnPhysMat);
template EConvertQueryResult ConvertTraceResults<ChaosInterface::FPTRaycastHit>(bool& OutHasValidBlockingHit, const UWorld* World, int32 NumHits, ChaosInterface::FPTRaycastHit* Hits, float CheckLength, const Chaos::Filter::FQueryFilterData& QueryFilter, TArray<FHitResult>& OutHits, const FVector& StartLoc, const FVector& EndLoc, const FPhysicsGeometry* Geom, const FTransform& QueryTM, float MaxDistance, bool bReturnFaceIndex, bool bReturnPhysMat);
template EConvertQueryResult ConvertTraceResults<ChaosInterface::FPTRaycastHit>(bool& OutHasValidBlockingHit, const UWorld* World, int32 NumHits, ChaosInterface::FPTRaycastHit* Hits, float CheckLength, const Chaos::Filter::FQueryFilterData& QueryFilter, FHitResult& OutHit, const FVector& StartLoc, const FVector& EndLoc, const FPhysicsGeometry* Geom, const FTransform& QueryTM, float MaxDistance, bool bReturnFaceIndex, bool bReturnPhysMat);
...
static bool ConvertOverlappedShapeToImpactHit(const UWorld* World, const THitLocation& Hit, const FVector& StartLoc, const FVector& EndLoc, FHitResult& OutResult, const FPhysicsGeometry& Geom, const FTransform& QueryTM, const Chaos::Filter::FQueryFilterData& QueryFilter, bool bReturnPhysMat)
{
	...
	if (bValidPosition)
	{
		...
		else
		{
			...
			UE_LOGF(LogPhysics, Verbose, "Warning: ConvertOverlappedShapeToImpactHit: MTD returned NaN :( position: %ls", *HitPosition.ToString());
		}
	}
	else
	{
	...
	else
	{
		...
		if (!bFiniteNormal)
		{
			UE_LOGF(LogPhysics, Verbose, "Warning: ConvertOverlappedShapeToImpactHit: MTD returned NaN :( normal: %ls", *HitNormal.ToString());
		}
	}
...
void ConvertQueryOverlap(const FPhysicsShape& Shape, const FPhysicsActor& Actor, FOverlapResult& OutOverlap, const Chaos::Filter::FQueryFilterData& QueryFilter)
{
	using namespace ChaosInterface;
	...
	else if (const FBodyInstance* BodyInst = GetUserData(Actor))
	{
		...
		// so the parent's PhysicsObject is the correct one for force/impulse application.
		const FPhysicsActorHandle& PhysicsActor = BodyInst->GetActorReferenceWithWelding();
		OutOverlap.PhysicsObject = PhysicsActor ? PhysicsActor->GetPhysicsObject() : nullptr;
...
#if UE_WITH_REMOTE_OBJECT_HANDLE
		ItemIndex  = BodyInst->InstanceBodyIndex;
#else
		if (const IPhysicsBodyInstanceOwner* BodyInstanceOwner = IPhysicsBodyInstanceOwner::GetPhysicsBodyInstanceOwnerFromOverlapResult(OutOverlap))
		{
			const bool bUseBodyIndex = BodyInstanceOwner->IsMultiBodyOverlap();
			ItemIndex = bUseBodyIndex ? BodyInst->InstanceBodyIndex : INDEX_NONE;
		}
#endif
		OutOverlap.SetItemIndex(ItemIndex);
	}
	else
	{
		// Currently geom collections are registered with a primitive component user data, but maybe custom should be adapted
...
bool IsBlocking(const FPhysicsShape& Shape, const Chaos::Filter::FQueryFilterData& QueryFilter)
{
	using namespace ChaosInterface;

	// See if this is a 'blocking' hit
	const Chaos::Filter::FShapeFilterData ShapeFilter = GetShapeFilterData(Shape);
	const ECollisionQueryHitType HitType = FCollisionQueryFilterCallback::CalcQueryHitType(QueryFilter, ShapeFilter);
...
bool ConvertOverlapResultsImp(int32 NumOverlaps, THitOverlap* OverlapResults, const Chaos::Filter::FQueryFilterData& QueryFilter, TArray<FOverlapResult>& OutOverlaps)
{
...
bool ConvertOverlapResults(int32 NumOverlaps, FHitOverlap* OverlapResults, const Chaos::Filter::FQueryFilterData& QueryFilter, TArray<FOverlapResult>& OutOverlaps)
{
	return ConvertOverlapResultsImp(NumOverlaps, OverlapResults, QueryFilter, OutOverlaps);
}

bool ConvertOverlapResults(int32 NumOverlaps, ChaosInterface::FPTOverlapHit* OverlapResults, const Chaos::Filter::FQueryFilterData& QueryFilter, TArray<FOverlapResult>& OutOverlaps)
{
	return ConvertOverlapResultsImp(NumOverlaps, OverlapResults, QueryFilter, OutOverlaps);
}
