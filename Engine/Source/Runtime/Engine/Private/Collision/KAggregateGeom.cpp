...
FBox FKTaperedCapsuleElem::CalcAABB(const FTransform& BoneTM, float Scale) const
{
	...
	// Get extruded tapered capsule axis direction
	const FVector3f ExtrudedAxis = (FVector3f)ElemTM.GetScaledAxis(EAxis::X);
	// Get abs of that vector
	const FVector3f AbsExtrudedAxis(FMath::Abs(ExtrudedAxis.X), FMath::Abs(ExtrudedAxis.Y), FMath::Abs(ExtrudedAxis.Z));
	// Scale by width of sphyl
	const FVector3f AbsExtrudedDist = (Scale * 0.5f * Width) * AbsExtrudedAxis;


	const FVector3f MaxPos = TaperedCapsuleCenter + AbsDist + AbsExtrudedDist;
	const FVector3f MinPos = TaperedCapsuleCenter - AbsDist - AbsExtrudedDist;
...
static void GetTriIndicesUsingEdge(const int32 Edge0, const int32 Edge1, const TArray<int32>& TriData, int32& Tri0Index, int32& Tri1Index)
{
	...
	for(int32 i=0; i<NumTris; i++)
	{
		if( TriHasEdge(TriData[(i*3)+0], TriData[(i*3)+1], TriData[(i*3)+2], Edge0, Edge1) )
		{
			...
			{
				UE_LOGF(LogPhysics, Log,  "GetTriIndicesUsingEdge : 3 tris share an edge." );
			}
		}
	}
}
...
bool FKConvexElem::HullFromPlanes(const TArray<FPlane>& InPlanes, const TArray<FVector>& SnapVerts, float InSnapDistance)
{
	...
	{
		UE_LOGF(LogPhysics, Log,  "Total Polygon Area invalid: %f", TotalPolyArea );
		return false;
	}
