...
bool BuildQueryShape(const FPhysicsGeometryCollection& GeometryCollection, Chaos::FQueryShape& OutQueryShape)
{
	...
	switch (ShapeType)
	{
		...
		default:
		{
			UE_LOGF(LogChaos, Warning, "PhysicsQueryHandler: Invalid shape type. Shape type %d is not yet supported", (int)ShapeType);
		}
	}
	return false;
}
...
struct TSQTraits
{
	...
	static typename TEnableIf<T == ESweepOrRay::Raycast, void>::Type SceneTrace(const AccelContainerType& Container, const GeomInputsType& GeomInputs, const FVector& Dir, float DeltaMag, const FTransform& StartTM, HitBufferType& HitBuffer, EHitFlags OutputFlags, const ChaosInterface::FSceneQueryCommonParams& CommonParams)
	{
		Chaos::Private::LowLevelRaycast(Container, StartTM.GetLocation(), Dir, DeltaMag, HitBuffer, OutputFlags, CommonParams);
	}

	//SceneTrace - sweep
	template <typename AccelContainerType, typename GeomInputsType, ESweepOrRay T = GeometryQuery>
	static typename TEnableIf<T == ESweepOrRay::Sweep, void>::Type SceneTrace(const AccelContainerType& Container, const GeomInputsType& GeomInputs, const FVector& Dir, float DeltaMag, const FTransform& StartTM, HitBufferType& HitBuffer, EHitFlags OutputFlags, const ChaosInterface::FSceneQueryCommonParams& CommonParams)
	{
		Chaos::Private::LowLevelSweep(Container, *GeomInputs.GetGeometry(), StartTM, Dir, DeltaMag, HitBuffer, OutputFlags, CommonParams);
	}
...
bool TSceneCastCommonImpWithRetryRequest(const UWorld* World, typename Traits::OutHitsType& OutHits, const GeomInputsType& GeomInputs, const FVector Start, const FVector End, ECollisionChannel TraceChannel, const struct FCollisionQueryParams& Params, const struct FCollisionResponseParams& ResponseParams, const struct FCollisionObjectQueryParams& ObjectParams, const AccelContainerType& AccelContainer, bool& bOutRequestRetry, FCollisionQueryParams& OutRetryParams)
{
	...
	if (Traits::IsSweep() || DeltaMag > 0.f)
	{
		...
		{
			const ChaosInterface::FSceneQueryCommonParams CommonParams(QueryCallback, QueryFilterData, Traits::GetQueryFlags(), DebugParams);
			...
			do
			{
				...
				{
					Traits::SceneTrace(AccelContainer.GetSpatialAcceleration(), GeomInputs, Dir, DeltaMag, StartTM, HitchRepeater.GetBuffer(), Traits::GetHitFlags(), CommonParams);
				}
				else
				{
					Traits::SceneTrace(PhysScene, GeomInputs, Dir, DeltaMag, StartTM, HitchRepeater.GetBuffer(), Traits::GetHitFlags(), CommonParams);
				}
		...
		{
			bool bSuccess = ConvertTraceResults(bBlockingHit, World, NumHits, Traits::GetHits(HitBufferSync), DeltaMag, QueryFilterData, OutHits, Start, End, GeomInputs.GetGeometry(), StartTM, MinBlockingDistance, Params.bReturnFaceIndex, Params.bReturnPhysicalMaterial) == EConvertQueryResult::Valid;

			if (!bSuccess)
			{
				// We don't need to change bBlockingHit, that's done by ConvertTraceResults if it removed the blocking hit.
				UE_LOGF(LogCollision, Error, "%ls%ls resulted in a NaN/INF in PHit!", Traits::IsRay() ? TEXT("Raycast") : TEXT("Sweep"), Traits::IsMulti() ? TEXT("Multi") : (Traits::IsSingle() ? TEXT("Single") : TEXT("Test")));
#if ENABLE_NAN_DIAGNOSTIC
				UE_LOGF(LogCollision, Error, "--------TraceChannel : %d", (int32)TraceChannel);
				UE_LOGF(LogCollision, Error, "--------Start : %ls", *Start.ToString());
				UE_LOGF(LogCollision, Error, "--------End : %ls", *End.ToString());
				if (Traits::IsSweep())
				{
					UE_LOGF(LogCollision, Error, "--------GeomRotation : %ls", *(GeomInputs.GetGeometryOrientation()->ToString()));
				}
				UE_LOGF(LogCollision, Error, "--------%ls", *Params.ToString());
#endif
			}
					...
					else
					{
						...
						if (ClusterUnionHit.bIsClusterUnion)
						{
							...
							else if (AActor* ClusterUnionActor = OutHits.GetActor())
							{
								...
								const Chaos::Filter::FInstanceData& FilterInstanceData = Traits::GetHits(HitBufferSync)->Shape->GetFilterInstanceData();
								uint32 ActorIDFromShape = FilterInstanceData.GetOwnerId();
								OutRetryParams.AddIgnoredActor(ActorIDFromShape);
								if (ActorIDFromShape != ClusterUnionActor->GetUniqueID())
								{
									UE_LOGF(LogChaos, Warning, "TSceneCastCommonImpWithRetryRequest: Incorrect Shape Actor ID detected");
								}								
								bOutRequestRetry = true;
								return false;
							}
...
bool TSceneCastCommonImp(const UWorld* World, typename Traits::OutHitsType& OutHits, const GeomInputsType& GeomInputs, const FVector Start, const FVector End, ECollisionChannel TraceChannel, const struct FCollisionQueryParams& Params, const struct FCollisionResponseParams& ResponseParams, const struct FCollisionObjectQueryParams& ObjectParams, const AccelContainerType& AccelContainer)
{
	...
	{
		UE_LOGF(LogChaos, Warning, "TSceneCastCommonImp: Potential Infinite Loop Detected");
		bReturnResult = false;
	}
...
bool GeomOverlapMultiImp(const UWorld* World, const FPhysicsGeometry& Geom, const CollisionAnalyzerType& CollisionAnalyzerShape, const FTransform& GeomPose, TArray<FOverlapResult>& OutOverlaps, ECollisionChannel TraceChannel, const struct FCollisionQueryParams& Params, const struct FCollisionResponseParams& ResponseParams, const struct FCollisionObjectQueryParams& ObjectParams, const AccelContainerType& AccelContainer)
{
	...
	if (GeomType == ECollisionShapeType::Sphere || GeomType == ECollisionShapeType::Capsule || GeomType == ECollisionShapeType::Box || GeomType == ECollisionShapeType::Convex)
	{
		...
		{
			const FSceneQueryCommonParams CommonParams(QueryCallback, QueryFilterData, QueryFlags, DebugParams);
			{
				...
				do
				{
					if constexpr (AccelContainerType::HasAccelerationStructureOverride())
					{
						Chaos::Private::LowLevelOverlap(AccelContainer.GetSpatialAcceleration(), Geom, GeomPose, HitchRepeater.GetBuffer(), CommonParams);
					}
					else
					{
						Chaos::Private::LowLevelOverlap(PhysScene, Geom, GeomPose, HitchRepeater.GetBuffer(), CommonParams);
					}
				} while(HitchRepeater.RepeatOnHitch());
			...
			if(InfoType == EQueryInfo::GatherAll)	//if we are gathering all we need to actually convert to UE format
			{
				...
				if(NumHits > 0)
				{
					bHaveBlockingHit = ConvertOverlapResults(NumHits, OverlapBuffer.GetHits(), QueryFilterData, OutOverlaps);
	...
	{
		UE_LOGF(LogCollision, Log, "GeomOverlapMulti : unsupported shape - only supports sphere, capsule, box");
	}
