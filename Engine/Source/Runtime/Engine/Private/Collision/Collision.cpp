...
namespace CollisionResponseConsoleCommands
{
	...
	void ListCollisionProfileNames()
	{
		...
		for (TSharedPtr<FName> NamePtr : ProfileNameList)
		{
			...
			UE_LOGF(LogCollisionCommands, Log, "%2d: %ls", Index++, *TemplateName.ToString());
		}
	}
	...
	void ListCollisionChannelNames()
	{
		...
		if (ChannelEnum)
		{
			for (int32 ChannelValue = 0; ChannelValue < ECollisionChannel::ECC_MAX; ChannelValue++)
			{
				...
				UE_LOGF(LogCollisionCommands, Log, "%2d: %ls (%ls)", ChannelValue, *ChannelShortName, *ChannelDisplayName);
			}
		}
	}
	...
	void ListComponentsWithResponseToProfile(const ECollisionResponse RequiredResponse, const FName& ProfileToCheck)
	{
		...
		{
			UE_LOGF(LogCollisionCommands, Warning, "Could not find collision profile '%ls'. Use 'Collision.ListProfiles' to see a full list of available profiles.", *ProfileToCheck.ToString());
			return;
		}
		...
		if (Results.Num() > 0)
		{
			...
			UE_LOGF(LogCollisionCommands, Log, "%ls", *Output);
			const int32 TotalLen = Output.Len() + 16;
			const FString LineMarker = FillString(TCHAR('-'), TotalLen);
			UE_LOGF(LogCollisionCommands, Log, "%ls", *LineMarker);
			...
			for (UPrimitiveComponent* Comp : Results)
			{
				...
				if (Outer)
				{
					...
					if (RequiredResponse == ECollisionResponse::ECR_Overlap)
					{
						UE_LOGF(LogCollisionCommands, Log, "%3d, %-14ls, %-*ls, %-*ls, %-*ls, %ls",
					...
					{
						UE_LOGF(LogCollisionCommands, Log, "%3d, %-*ls, %-*ls, %-*ls, %ls",
		...
		UE_LOGF(LogCollisionCommands, Log, "Found %d components with '%ls' response to profile '%ls'.", Results.Num(), *ResponseStrings[(int32)RequiredResponse], *ProfileToCheck.ToString());
	}
	...
	void Parse_ListComponentsWithResponseToProfile(const TArray<FString>& Args, UWorld* World)
	{
		if (Args.Num() < 2)
		{
			UE_LOGF(LogCollisionCommands, Warning, "Usage: 'Collision.ListComponentsWithResponseToProfile <Response> <Profile>'.");
			UE_LOGF(LogCollisionCommands, Warning, "  Response: %ls", *GetCommaSeparatedList(ResponseStrings));
			UE_LOGF(LogCollisionCommands, Warning, "  Profile:  Collision profile name or index. Use 'Collision.ListProfiles' to see a full list.");
			return;
		}
		...
		{
			UE_LOGF(LogCollisionCommands, Warning, "Unknown response '%ls'. Must be one of %ls.", *ResponseString, *GetCommaSeparatedList(ResponseStrings));
			return;
		}
	...
	void ListProfilesWithResponseToChannel(const ECollisionResponse RequiredResponse, const ECollisionChannel TestChannel)
	{
		...
		if (Results.Num() > 0)
		{
			...
			{
				UE_LOGF(LogCollisionCommands, Log, "%ls", *ResultName.ToString());
			}
		}
		// Display Summary
		check(RequiredResponse < ECollisionResponse::ECR_MAX);
		...
		UE_LOGF(LogCollisionCommands, Log, "----------------------------------------------------------------------");
		UE_LOGF(LogCollisionCommands, Log, "Found %d profiles with '%ls' response to channel '%ls' ('%ls')", Results.Num(), *ResponseStrings[(int32)RequiredResponse], *ChannelName, *ChannelDisplayName);
	}
	...
	void Parse_ListProfilesWithResponseToChannel(const TArray<FString>& Args, UWorld* World)
	{
		if (Args.Num() < 2)
		{
			UE_LOGF(LogCollisionCommands, Warning, "Usage: 'Collision.ListProfilesWithResponseToChannel <Response> <Channel>'.");
			UE_LOGF(LogCollisionCommands, Warning, "  Response: %ls", *GetCommaSeparatedList(ResponseStrings));
			UE_LOGF(LogCollisionCommands, Warning, "  Profile:  Collision channel name or index. Use 'Collision.ListChannels' to see a full list.");
			return;
		}
		...
		{
			UE_LOGF(LogCollisionCommands, Warning, "Unknown channel '%ls. Use 'Collision.ListChannels' to see a full list.'", *ChannelNameString);
			return;
		}
	...
	void ListObjectsWithCollisionComplexity(ECollisionTraceFlag Complexity)
	{
		...
		if (Results.Num() > 0)
		{
			...
			// Get max column widths for some data
			const FString ComplexityHeading = "Collision Complexity";
		...
		// Display Summary
		UE_LOGF(LogCollisionCommands, Log, "Found %d components with '%ls' collision complexity.", Results.Num(), *ComplexityStrings[int32(Complexity)]);
	}
	...
	void Parse_ListObjectsWithCollisionComplexity(const TArray<FString>& Args, UWorld* World)
	{
		if (Args.Num() < 1)
		{
			UE_LOGF(LogCollisionCommands, Warning, "Usage: 'Collision.ListObjectsWithCollisionComplexity <Complexity>'.");
			UE_LOGF(LogCollisionCommands, Warning, "  Complexity: %ls", *GetCommaSeparatedList(ComplexityStrings));
			return;
		}
		...
		if (Complexity == CTF_MAX)
		{
			UE_LOGF(LogCollisionCommands, Warning, "Unknown complexity '%ls'. Must be one of %ls.", *ComplexityString, *GetCommaSeparatedList(ComplexityStrings));
			return;
		}
