...
void FTexture2DStreamIn_DDC::DoCreateAsyncDDCRequests(const FContext& Context)
{
	...
	if (const FTexturePlatformData* PlatformData = Context.Texture->GetPlatformData())
	{
		...
		if (bUsingDDC1)
		{
			for (int32 MipIndex = PendingFirstLODIdx; MipIndex < CurrentFirstLODIdx && !IsCancelled(); ++MipIndex)
			{
				...
				{
					UE_LOGF(LogTexture, Error, "Attempting to stream data that is already loaded for mip %d of %ls.",
		...
		{
			UE_LOGF(LogTexture, Error, "Attempting to stream data in an unsupported cache format for mips [%d, %d) of %ls.",
	...
	{
		UE_LOGF(LogTexture, Error, "Attempting to stream data that has not been generated yet for mips [%d, %d) of %ls.",
			PendingFirstLODIdx, CurrentFirstLODIdx, *Context.Texture->GetPathName());
		MarkAsCancelled();
	}
}
	...
	for (int32 MipIndex = PendingFirstLODIdx; MipIndex < CurrentFirstLODIdx && !IsCancelled(); ++MipIndex)
	{
		...
		if (Status.Buffer)
		{
			...
			if (Status.Buffer.GetSize() == ExpectedMipSize)
			{
				...
				check(MipData[MipIndex].Size >= Status.Buffer.GetSize());
