	...
	const FTextureUpdateSyncOptions& SyncOptions)
{
	...
	// Step (1) : Create the texture on the render thread using AsyncReallocateTexture2D. Wait for the RHI to signal the operation is completed through the FThreadSafeCounter.
	if (!IntermediateTextureRHI)
	{
		...
		IntermediateTextureRHI = RHICmdList.AsyncReallocateTexture2D(
			Texture2DRHI,
	...
	// Step (2) : Finalize the texture state through FinalizeAsyncReallocateTexture2D and lock the new mips.
	else
	{
		...
		for (int32 MipIndex = PendingFirstLODIdx; MipIndex < CurrentFirstLODIdx; ++MipIndex)
		{
			...
			MipInfo.DataSize = 0;
#endif
			MipInfo.DestData = RHICmdList.LockTexture2D(IntermediateTextureRHI, MipIndex - PendingFirstLODIdx, RLM_WriteOnly, MipInfo.RowPitch, false, bFlushRHIThread);
...
void FTexture2DMipAllocator_AsyncReallocate::UnlockNewMips()
{
	...
	if (IntermediateTextureRHI)
	{
		...
		{
			RHICmdList.UnlockTexture2D(IntermediateTextureRHI, MipIndex, false, bFlushRHIThread);
		}
