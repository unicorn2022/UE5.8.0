...
bool FDynamicRenderAssetInstanceManager::IsReferenced(const IPrimitiveComponent* Component) const
{ 
	return Component && Component->GetStreamingState().IsAttachedToStreamingManagerAsDynamic() && (StateSync.GetState()->HasComponentReferences(Component) || PendingComponents.Contains(Component));
}
...
void FDynamicRenderAssetInstanceManager::IncrementalUpdate(FRemovedRenderAssetArray& RemovedRenderAssets, float Percentage)
{
	...
	for (const IPrimitiveComponent* Component : PendingComponents)
	{
		check(Component);

		State->RemoveComponent(Component, &RemovedRenderAssets);
		Component->GetStreamingState().SetAttachedToStreamingManagerAsDynamic(false);
		// Re-enable updates now that the component is out of the pending list.
		Component->GetStreamingState().SetIgnoreStreamingManagerUpdate(false);
	}
	...
	if (PendingComponents.Num())
	{
		...
		if (FRenderAssetStreamingManager::IsParallelForAllowedDuringIncrementalUpdate(ToProcessComponentCount, MinBatchSize))
		{
			...
			ParallelFor(TEXT("PreAddComponentsIgnoreBounds"), ToProcessComponentCount, MinBatchSize, [&](int32 Index)
			{
				...
				const IPrimitiveComponent* Component = PendingComponents[Index];
				check(Component);
				if (!Component->GetStreamingState().IsAttachedToStreamingManagerAsDynamic() && CanManage(Component) && Component->IsRegistered() && (!Component->IsRenderStateCreated() || Component->GetSceneProxy()))
				{
			...
			});
			{
				for (FRenderAssetInstanceState::FPreAddComponentIgnoreBoundsPayload& Payload : Payloads)
				{
					...
					{
						Payload.Component->GetStreamingState().SetAttachedToStreamingManagerAsDynamic(true);
					}
				}
			}
		}
		else
		{
			for (const IPrimitiveComponent* Component : PendingComponents)
			{
				...
				if (!Component->GetStreamingState().IsAttachedToStreamingManagerAsDynamic() && CanManage(Component) && Component->IsRegistered() && (!Component->IsRenderStateCreated() || Component->GetSceneProxy()))
				{
					...
					{
						Component->GetStreamingState().SetAttachedToStreamingManagerAsDynamic(true);
					}
				}
			}
		}
...
void FDynamicRenderAssetInstanceManager::OnCreateViewDone(FRenderAssetInstanceView* InView)
{
	...
	// All of the data is dirty now since we just got uninitialized data.
	DirtyIndex = 0;
...
void FDynamicRenderAssetInstanceManager::OnPreGarbageCollect(FRemovedRenderAssetArray& RemovedRenderAssets)
{
	...
	{
		const IPrimitiveComponent* Primitive = PendingComponents[Index];
		check(Primitive);
		...
		if (!Primitive->IsRegistered() || !CanManage(Primitive))
		{
			PendingComponents.RemoveAtSwap(Index);
			Primitive->GetStreamingState().SetIgnoreStreamingManagerUpdate(false);
			--Index;
			...
			Primitive->GetStreamingState().SetAttachedToStreamingManagerAsDynamic(false);
		}
	}
}

bool FDynamicRenderAssetInstanceManager::CanManage(const IPrimitiveComponent* Component) const
{
	if (!Component)
	{
		return false;
	}
	const UObject* ComponentObject = Component->GetUObject();
	return IsValid(ComponentObject) && !ComponentObject->HasAnyFlags(RF_BeginDestroyed|RF_FinishDestroyed);
}
...
EAddComponentResult FDynamicRenderAssetInstanceManager::Add(const IPrimitiveComponent* Component, FStreamingTextureLevelContext& LevelContext, float MaxAllowedUIDensity)
{
	...
	{
		FRenderAssetOwnerStreamingState& StreamingState = Component->GetStreamingState();
		...
		if (!StreamingState.IsIgnoreStreamingManagerUpdate())
		{
			// Ignore any further updates while the component is in the pending list.
			StreamingState.SetIgnoreStreamingManagerUpdate(true);

			// Postpone so that we don't have to sync the state.
			PendingComponents.Add(Component);

			// Notify attached since it is now referred in PendingComponents
			StreamingState.SetAttachedToStreamingManagerAsDynamic(true);
		}
...
void FDynamicRenderAssetInstanceManager::Remove(const IPrimitiveComponent* Component, FRemovedRenderAssetArray* RemovedRenderAssets)
{
	check(!Component || !Component->GetUObject() || Component->GetUObject()->IsValidLowLevelFast());
	if (Component && Component->GetStreamingState().IsAttachedToStreamingManagerAsDynamic())
	{
