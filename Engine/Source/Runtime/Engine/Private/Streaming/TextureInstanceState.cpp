...
int32 FRenderAssetInstanceState::AddBounds(const IPrimitiveComponent* Component)
{
	checkf(bIsDynamicInstanceState, TEXT("This version of AddBounds should only be called by the dynamic instance manager."));
	FBoxSphereBounds BoundsCopy = Component->GetBounds();
	BoundsCopy.SphereRadius = Component->GetStreamingScale();
	return AddBounds(BoundsCopy, PackedRelativeBox_Identity, Component, Component->GetLastRenderTimeOnScreen(), BoundsCopy.Origin, 0, 0, FLT_MAX);
}

int32 FRenderAssetInstanceState::AddBounds(const FBoxSphereBounds& Bounds, uint32 PackedRelativeBox, const IPrimitiveComponent* InComponent, float LastRenderTime, const FVector4& RangeOrigin, float MinDistanceSq, float MinRangeSq, float MaxRangeSq)
{
	check(InComponent);
...
void FRenderAssetInstanceState::AddElement(const IPrimitiveComponent* InComponent, const UStreamableRenderAsset* InAsset, int InBoundsIndex, float InTexelFactor, bool InForceLoad, int32*& ComponentLink)
{
...
void FRenderAssetInstanceState::AddRenderAssetElements(const IPrimitiveComponent* Component, const TArray<FPreAddRenderAssetElement>& RenderAssetElements, int32 BoundsIndex, int32*& ComponentLink)
{
	for (const FPreAddRenderAssetElement& Element : RenderAssetElements)
	{
		AddElement(Component, Element.RenderAsset, BoundsIndex, Element.MergedTexelFactor, Component->IsMipStreamingForced(), ComponentLink);
	}
}
...
void FRenderAssetInstanceState::AddRenderAssetElements(const IPrimitiveComponent* Component, const TArrayView<FStreamingRenderAssetPrimitiveInfo>& RenderAssetInstanceInfos, int32 BoundsIndex, int32*& ComponentLink)
{
	...
	{
		AddElement(Component, RenderAsset, BoundsIndex, TexelFactor, Component->IsMipStreamingForced(), ComponentLink);
	});
}
...
void FRenderAssetInstanceState::PreAddComponent(const IPrimitiveComponent* Component, FStreamingTextureLevelContext& LevelContext, float MaxAllowedUIDensity, FPreAddComponentPayload& OutPreAddPayload)
{
...
EAddComponentResult FRenderAssetInstanceState::AddComponent(const IPrimitiveComponent* Component, FStreamingTextureLevelContext& LevelContext, float MaxAllowedUIDensity)
{
	return AddComponentInternal(Component, LevelContext, MaxAllowedUIDensity, this, nullptr);
}

EAddComponentResult FRenderAssetInstanceState::AddComponentInternal(const IPrimitiveComponent* Component, FStreamingTextureLevelContext& LevelContext, float MaxAllowedUIDensity, FRenderAssetInstanceState* State, FPreAddComponentPayload* OutPreAddPayload)
{
	...
	check(Component);
	TArray<FStreamingRenderAssetPrimitiveInfo> RenderAssetInstanceInfos = LevelContext.GetStreamingRenderAssetInfoWithNULLRemoval(Component);
	...
	else
	{
		...
		for (int32 InfoIndex = 0; InfoIndex < RenderAssetInstanceInfos.Num();)
		{
			...
			if (State)
			{
				const int32 BoundsIndex = State->AddBounds(Info.Bounds, PackedRelativeBox_Identity, Component, Component->GetLastRenderTimeOnScreen(), Component->GetBounds().Origin, MinDistanceSq, MinRangeSq, MaxRangeSq);
				State->AddRenderAssetElements(Component, TArrayView<FStreamingRenderAssetPrimitiveInfo>(RenderAssetInstanceInfos.GetData() + InfoIndex, NumOfBoundReferences), BoundsIndex, ComponentLink);
			}
			else
			{
				check(OutPreAddPayload);
				FPreAddBounds& OutBounds = OutPreAddPayload->Bounds.Emplace_GetRef(Info.Bounds, PackedRelativeBox_Identity, false, Component->GetBounds().Origin, MinDistanceSq, MinRangeSq, MaxRangeSq);
				UE::Private::PreAddRenderAssetElements(TArrayView<FStreamingRenderAssetPrimitiveInfo>(RenderAssetInstanceInfos.GetData() + InfoIndex, NumOfBoundReferences), OutBounds.RenderAssets);
			}
...
void FRenderAssetInstanceState::PreAddComponentIgnoreBounds(const IPrimitiveComponent* Component, FStreamingTextureLevelContext& LevelContext, FPreAddComponentIgnoreBoundsPayload& OutPayload)
{
...
EAddComponentResult FRenderAssetInstanceState::AddComponentIgnoreBounds(const IPrimitiveComponent* Component, FStreamingTextureLevelContext& LevelContext)
{
	return AddComponentIgnoreBoundsInternal(Component, LevelContext, this, nullptr);
}

EAddComponentResult FRenderAssetInstanceState::AddComponentIgnoreBoundsInternal(const IPrimitiveComponent* Component, FStreamingTextureLevelContext& LevelContext, FRenderAssetInstanceState* State, FPreAddComponentIgnoreBoundsPayload* OutPreAddPayload)
{
...
void FRenderAssetInstanceState::RemoveComponent(const IPrimitiveComponent* Component, FRemovedRenderAssetArray* RemovedRenderAssets)
{
	int32 ElementIndex = INDEX_NONE;
...
bool FRenderAssetInstanceState::RemoveComponentReferences(const IPrimitiveComponent* Component)
{
...
void FRenderAssetInstanceState::UpdateBounds(const IPrimitiveComponent* Component)
{
	checkf(bIsDynamicInstanceState, TEXT("Bounds shouldn't be updated after creation unless the instances are dynamic"));

	if (int32* ComponentLink = ComponentMap.Find(Component))
	{
		int32 ElementIndex = *ComponentLink;
		while (ElementIndex != INDEX_NONE)
		{
			...
			if (Element.BoundsIndex != INDEX_NONE)
			{
				...
				Bounds4[Element.BoundsIndex / 4].FullUpdate(Element.BoundsIndex % 4, Bounds.Origin, Bounds.BoxExtent, Component->GetStreamingScale(), Component->GetLastRenderTimeOnScreen());
			}
			ElementIndex = Element.NextComponentLink;
		}
	}
...
bool FRenderAssetInstanceState::UpdateBounds(int32 BoundIndex)
{
	...
	const IPrimitiveComponent* Component = ensure(Bounds4Components.IsValidIndex(BoundIndex)) ? Bounds4Components[BoundIndex] : nullptr;
	if (Component)
	{
		const FBoxSphereBounds& Bounds = Component->GetBounds();
...
void FRenderAssetInstanceState::UpdateLastRenderTimeAndMaxDrawDistance(int32 BoundIndex)
{
	...
	if (Component)
	{
		...
		const IPrimitiveComponent* LODParent = Component->GetLODParentPrimitive();
		if (LODParent)
		{
			const float MaxRangeSq = FRenderAssetInstanceView::GetMaxDrawDistSqWithLODParent(
				Component->GetBounds().Origin,
				LODParent->GetBounds().Origin,
				LODParent->GetMinDrawDistance(),
				LODParent->GetBounds().SphereRadius);
...
int32 FRenderAssetInstanceState::CheckRegistrationAndUnpackBounds(TArray<const IPrimitiveComponent*>& RemovedComponents)
{
	for (int32 BoundIndex : BoundsToUnpack)
	{
		if (Bounds4Components.IsValidIndex(BoundIndex))
		{
			if (const IPrimitiveComponent* Component = Bounds4Components[BoundIndex])
			{
				...
				if (Component->IsRegistered() && (!Component->IsRenderStateCreated() || Component->GetSceneProxy()))
				{
...
bool FRenderAssetInstanceState::MoveBound(int32 SrcBoundIndex, int32 DstBoundIndex)
{
	...
	if (Bounds4Components.IsValidIndex(DstBoundIndex) && Bounds4Components.IsValidIndex(SrcBoundIndex) && !Bounds4Components[DstBoundIndex] && Bounds4Components[SrcBoundIndex])
	{
		...
		const IPrimitiveComponent* Component = Bounds4Components[SrcBoundIndex];
