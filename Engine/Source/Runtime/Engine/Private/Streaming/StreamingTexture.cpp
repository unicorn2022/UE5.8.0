...
FStreamingRenderAsset::FStreamingRenderAsset(
	...
	, SSAMAssetHandle(InRenderAsset->GetSSAMAssetHandle())
	, RenderAssetType(InRenderAsset->GetRenderAssetType())
{
	...
	SetFlag(FStreamingRenderAssetFlags::HasUpdatePending, InRenderAsset && InRenderAsset->bHasStreamingUpdatePending);
	SetFlag(FStreamingRenderAssetFlags::WasSSAMRegistered, FSimpleStreamableAssetManager::IsAssetRegistered(SSAMAssetHandle));

	SetFlag(FStreamingRenderAssetFlags::ForceFullyLoadHeuristic, false);
	SetFlag(FStreamingRenderAssetFlags::UseUnkownRefHeuristic, false);
	NumMissingMips = 0;
	SetFlag(FStreamingRenderAssetFlags::LooksLowRes, false);
	SetFlag(FStreamingRenderAssetFlags::MissingTooManyMips, false);
...
void FStreamingRenderAsset::UpdateStaticData(const FRenderAssetStreamingSettings& Settings)
{
	...
	if (RenderAsset)
	{
		...
		if (IsTexture())
		{
			...
			SetFlag(FStreamingRenderAssetFlags::LoadWithHigherPriority, Settings.HighPriorityLoad_Texture[LODGroup]);
			SetFlag(FStreamingRenderAssetFlags::IsTerrainTexture, (TextureLODGroup == TEXTUREGROUP_Terrain_Heightmap || TextureLODGroup == TEXTUREGROUP_Terrain_Weightmap));
			SetFlag(FStreamingRenderAssetFlags::IsVirtualTexture, ResourceState.bSupportsVirtualStreaming);
		}
		else
		{
			...
			BoostFactor = 1.f;
			SetFlag(FStreamingRenderAssetFlags::LoadWithHigherPriority, false);
			SetFlag(FStreamingRenderAssetFlags::IsTerrainTexture, false);
			SetFlag(FStreamingRenderAssetFlags::IsVirtualTexture, false);
...
void FStreamingRenderAsset::UpdateDynamicData(TConstArrayView<int32> NumStreamedMips, const FRenderAssetStreamingSettings& Settings, TArray<UStreamableRenderAsset*>* DeferredTickCBAssets)
{
	...
	if (RenderAsset)
	{
		...
		const FStreamableRenderResourceState ResourceState = UpdateStreamingStatus(DeferredTickCBAssets);
		...
		const bool bForceFullyLoadFlag = RenderAsset->ShouldMipLevelsBeForcedResident();
		SetFlag(FStreamingRenderAssetFlags::ForceFullyLoad, bForceFullyLoadFlag);

		SetFlag(FStreamingRenderAssetFlags::IgnoreStreamingMipBias, RenderAsset->bIgnoreStreamingMipBias);

		const int32 NumCinematicMipLevels = (bForceFullyLoadFlag && RenderAsset->bUseCinematicMipLevels) ? RenderAsset->NumCinematicMipLevels : 0;
		...
		check(LODGroup < NumStreamedMips.Num());
	...
	{
		SetFlag(FStreamingRenderAssetFlags::ForceFullyLoad, false);
		SetFlag(FStreamingRenderAssetFlags::IgnoreStreamingMipBias, false);
		ResidentMips = 0;
		RequestedMips = 0;
		MinAllowedMips = 0;
		MaxAllowedMips = 0;
...
FStreamableRenderResourceState FStreamingRenderAsset::UpdateStreamingStatus(TArray<UStreamableRenderAsset*>* DeferredTickCBAssets)
{
	FStreamableRenderResourceState ResourceState;

	if (RenderAsset)
	{
		RenderAsset->TickStreaming(UStreamableRenderAsset::ETickStreamingFlags::SendCompletionEvents, DeferredTickCBAssets);
...
int32 FStreamingRenderAsset::GetWantedMipsFromSize(float Size, float InvMaxScreenSizeOverAllViews) const
{
	...
	else
	{
		...
		return GetFlag(FStreamingRenderAssetFlags::UseUnkownRefHeuristic) ? FMath::Max(Result - NoRefLODBias, MinAllowedMips) : Result;;
	}
}
...
void FStreamingRenderAsset::SetPerfectWantedMips_Async(
	...
	const FRenderAssetStreamingSettings& Settings)
{
	SetFlag(FStreamingRenderAssetFlags::ForceFullyLoadHeuristic, (MaxSize == FLT_MAX || MaxSize_VisibleOnly == FLT_MAX));
	SetFlag(FStreamingRenderAssetFlags::LooksLowRes, InLooksLowRes); // Things like lightmaps, HLOD and close instances.
	NormalizedScreenSize = 0.f;
	...
	if (GetFlag(FStreamingRenderAssetFlags::IsTerrainTexture) || GetFlag(FStreamingRenderAssetFlags::ForceFullyLoadHeuristic) || GetFlag(FStreamingRenderAssetFlags::LooksLowRes))
	{
...
int64 FStreamingRenderAsset::UpdateRetentionPriority_Async(bool bPrioritizeMesh)
{
	...
	if (RenderAsset)
	{
		...
		const bool bIsTerrainTextureFlag = GetFlag(FStreamingRenderAssetFlags::IsTerrainTexture);
		const bool bForceFullyLoadHeuristicFlag = GetFlag(FStreamingRenderAssetFlags::ForceFullyLoadHeuristic);
		const bool bLooksLowResFlag = GetFlag(FStreamingRenderAssetFlags::LooksLowRes);
		const bool bShouldKeep = bIsTerrainTextureFlag || bForceFullyLoadHeuristicFlag || (bLooksLowResFlag && !bIsHuge);
		...
		if (GetFlag(FStreamingRenderAssetFlags::LoadWithHigherPriority) || bIsSmall)	RetentionPriority += 256; // Try to keep character of small texture as they don't pay off.
...
bool FStreamingRenderAsset::UpdateLoadOrderPriority_Async(const FRenderAssetStreamingSettings& Settings)
{
	...
	if (ResidentMips < VisibleWantedMips && VisibleWantedMips < BudgetedMips && BudgetedMips >= Settings.MinMipForSplitRequest && !bIsTerrainTextureFlag)
	{
	...
	if (RenderAsset && WantedMips != RequestedMips)
	{
		...
		const bool bMustLoadFirst		= GetFlag(FStreamingRenderAssetFlags::ForceFullyLoadHeuristic) || bIsTerrainTextureFlag || GetFlag(FStreamingRenderAssetFlags::LoadWithHigherPriority);

		if (WantedMips > RequestedMips)
		{
			const bool bMipIsImportant = WantedMips - ResidentMips > (GetFlag(FStreamingRenderAssetFlags::LooksLowRes) || IsMesh() ? 1 : 2);

			// Only consider visible assets to reduce the number of high priority requests
			SetFlag(FStreamingRenderAssetFlags::MissingTooManyMips, bIsVisible && bMipIsImportant && Settings.LowResHandlingMode != FRenderAssetStreamingSettings::LRHM_DoNothing);
...
{
	SetFlag(FStreamingRenderAssetFlags::CachedForceFullyLoadHeuristic, GetFlag(FStreamingRenderAssetFlags::ForceFullyLoadHeuristic));
...
void FStreamingRenderAsset::StreamWantedMips_Internal(FRenderAssetStreamingManager& Manager, bool bUseCachedData)
{
	if (RenderAsset && !RenderAsset->HasPendingInitOrStreaming())
	{
		...
		const uint32 bLocalForceFullyLoadHeuristic = bUseCachedData ? GetFlag(FStreamingRenderAssetFlags::CachedForceFullyLoadHeuristic) : GetFlag(FStreamingRenderAssetFlags::ForceFullyLoadHeuristic);
		...
		RenderAsset->CachedNumWantedLODs = static_cast<uint8>(LocalWantedMips);
#endif
		...
		if (LocalWantedMips != ResidentMips)
		{
			if (LocalWantedMips < ResidentMips)
			{
				// Protection-aware StreamOut: if an external consumer holds the asset,
				// this returns false without invoking StreamOut. The async task will
				// keep producing stream-out requests so the next tick after release
				// picks this up.
				if (!Manager.StreamOutProtected(RenderAsset, LocalWantedMips))
				{
					return;
				}
			}
			...
			{
				const bool bShouldPrioritizeAsyncIORequest = (bLocalForceFullyLoadHeuristic || GetFlag(FStreamingRenderAssetFlags::IsTerrainTexture) || GetFlag(FStreamingRenderAssetFlags::LoadWithHigherPriority) || IsMissingTooManyMips()) && LocalWantedMips <= LocalVisibleWantedMips;
