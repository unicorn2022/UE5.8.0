...
void FStaticMeshStreamIn::CreateBuffers(const FContext& Context)
{
	...
	StreamingRHICmdList->SwitchPipeline(ERHIPipeline::Graphics);
	{
		...
		for (int32 LODIdx = PendingFirstLODIdx; LODIdx < CurrentFirstLODIdx; ++LODIdx)
		{
			...
			{
#if DO_CHECK
				CheckRayTracingGeometryInitializer(LODResource, ERayTracingGeometryInitializerType::StreamingDestination, LODResource.RayTracingGeometry->GetInitializer());
#endif
	...
	if (IsRayTracingAllowed() && Context.Mesh->bSupportRayTracing)
	{
		...
		for (int32 LODIndex = PendingFirstLODIdx; LODIndex < CurrentFirstLODIdx; ++LODIndex)
		{
			...
			if (LODResource.GetNumVertices() > 0 && LODResource.RayTracingGeometry != nullptr && !LODResource.RayTracingGeometry->IsEvicted())
			{
				...
				CheckRayTracingGeometryInitializer(LODResource, ERayTracingGeometryInitializerType::Rendering, LODResource.RayTracingGeometry->GetInitializer());
...
void FStaticMeshStreamOut::CheckReferencesAndDiscardCPUData(const FContext& Context)
{
	...
	if (Mesh && RenderData)
	{
		...
		if (NumExternalReferences > PreviousNumberOfExternalReferences && NumReferenceChecks > 0)
		{
			...
			UE_LOGF(LogContentStreaming, Warning, "[%ls] Streamed out LODResources got referenced while in pending stream out.", *Mesh->GetName());
		}
	}
	...
	else
	{
		...
		{
			UE_LOGF(LogContentStreaming, Warning, "[%ls] Streamed out LODResources references are not getting released.", *Mesh->GetName());
		}
...
void FStaticMeshStreamIn_IO::ReportIOError(const FContext& Context)
{
	...
	if (bFailedOnIOError && Context.Mesh)
	{
		...
		UE_LOGF(LogContentStreaming, Warning, "[%ls] StaticMesh stream in request failed due to IO error (LOD %d-%d).", *Context.Mesh->GetName(), PendingFirstLODIdx, CurrentFirstLODIdx - 1);
	}
}
		...
		for (int32 LODIdx = PendingFirstLODIdx; LODIdx < CurrentFirstLODIdx; ++LODIdx)
		{
			...
			if (Ar.IsError())
			{
				UE_LOGF(LogContentStreaming, Error,
					"[%ls] StaticMesh stream in failed due to possibly corrupted data. LOD %d %d-%d. BulkData %#x offset %lld size %lld flags %#x.",
					*Mesh->GetPathName(),
					LODIdx,
					PendingFirstLODIdx,
				...
				GLog->Flush();
				UE_LOGF(LogContentStreaming, Fatal, "Possibly corrupted static mesh LOD data detected.");
#endif
			}
		}
