 ...
 */
class FStaticRenderAssetInstanceManager
{
public:
	...
	FORCEINLINE int32 CheckRegistrationAndUnpackBounds(TArray<const IPrimitiveComponent*>& RemovedComponents) { return StateSync.SyncAndGetState()->CheckRegistrationAndUnpackBounds(RemovedComponents); }
	...
	FORCEINLINE bool IsReferenced(const IPrimitiveComponent* Component) const { return StateSync.GetState()->HasComponentReferences(Component); }

	/** Invoke a callable for each referenced component. */
	template<typename CallableT>
	void ForEachReferencedComponent(CallableT&& Callable)
	{
		StateSync.GetState()->ForEachReferencedComponent(Forward<CallableT>(Callable));
	}
	...
	EAddComponentResult Add(const IPrimitiveComponent* Component, FStreamingTextureLevelContext& LevelContext, float MaxAllowedUIDensity);
	...
	/** Remove a component, the RemovedRenderAssets is the list of assets not referred anymore. */
