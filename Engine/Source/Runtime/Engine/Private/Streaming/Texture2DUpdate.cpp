...
void FTexture2DUpdate::DoAsyncReallocate(FRHICommandListImmediate& RHICmdList, const FContext& Context)
{
...
void FTexture2DUpdate::DoConvertToVirtualWithNewMips(FRHICommandListImmediate& RHICmdList, const FContext& Context)
{
	check(Context.CurrentThread == TT_Render);
...
bool FTexture2DUpdate::DoConvertToNonVirtual(FRHICommandListImmediate& RHICmdList, const FContext& Context)
{
	check(Context.CurrentThread == TT_Render);
...
void FTexture2DUpdate::DoFinishUpdate(FRHICommandListImmediate& RHICmdList, const FContext& Context)
{
	check(Context.CurrentThread == TT_Render);
	...
	if (IntermediateTextureRHI && Context.Resource)
	{
		if (!IsCancelled())
		{
			Context.Resource->FinalizeStreaming(RHICmdList, IntermediateTextureRHI);
