...
static void ValidateMipBulkDataSize(const UTexture2D& Texture, int32 MipSizeX, int32 MipSizeY, int32 MipIndex, int64 ActualMipSize, int64& BulkDataSize)
{
...
void FTexture2DStreamIn_IO::SetIORequests(const FContext& Context)
{
	...
	for (int32 MipIndex = PendingFirstLODIdx; MipIndex < CurrentFirstLODIdx && !IsCancelled(); ++MipIndex)
	{
		...
		if (BulkDataSize > 0)
		{
			...
			ValidateMipBulkDataSize(*Context.Texture, MipMap.SizeX, MipMap.SizeY, MipIndex, MipData[MipIndex].Size, BulkDataSize);
...
void FTexture2DStreamIn_IO::ReportIOError(const FContext& Context)
{
	...
	if (bFailedOnIOError && Context.Texture)
	{
		...
		UE_LOGF(LogContentStreaming, Warning, "[%ls] Texture stream in request failed due to IO error (Mip %d-%d).", *Context.Texture->GetName(), ResourceState.AssetLODBias + PendingFirstLODIdx, ResourceState.AssetLODBias + CurrentFirstLODIdx - 1);
	}
}
