...
struct FBounds4
{
	 ...
	 *  When the bounds row is owned by FSimpleStreamableAssetManager (SSAM), this field always carries
	 *  StreamingScaleFactor (i.e., the ComponentScale interpretation) regardless of whether the instance is
	 *  static or dynamic; the legacy "sphere radius for static" interpretation does not apply on SSAM-owned
	 *  rows. Consumers that branch on static vs dynamic must account for the SSAM ownership case. */
	...
	void UnpackBounds(int32 Index, const IPrimitiveComponent* Component);
...
class FRenderAssetInstanceView : public UE::Private::FQueryableRefCountedObject
{
public:
	...
	struct FElement
	{
		...
		const IPrimitiveComponent* Component; // Which component this relates too
	...
	public:
		...
		FORCEINLINE const IPrimitiveComponent* GetComponent() const { return State.Elements[CurrElementIndex].Component; }
	...
	static void GetDistanceAndRange(const IPrimitiveComponent* Component, const FBoxSphereBounds& RenderAssetInstanceBounds, float& MinDistanceSq, float& MinRangeSq, float& MaxRangeSq);

	static void OutputToLog(const FBoxSphereBounds& Bounds, int32 BoundsIndex, const TArray<FBounds4>& Bounds4, float TexelFactor, bool bForceLoad, float MaxNormalizedSize, float MaxNormalizedSize_VisibleOnly, const TCHAR* Prefix, const IPrimitiveComponent* Component = nullptr);
