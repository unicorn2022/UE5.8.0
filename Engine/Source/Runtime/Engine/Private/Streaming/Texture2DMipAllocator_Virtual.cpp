	...
	const FTextureUpdateSyncOptions& SyncOptions)
{
	...
	else
	{
		...
		for (int32 MipIndex = PendingFirstLODIdx; MipIndex < CurrentFirstLODIdx; ++MipIndex)
		{
			...
			MipInfo.DestData = RHICmdList.LockTexture2D(IntermediateTextureRHI, MipIndex, RLM_WriteOnly, MipInfo.RowPitch, false, bFlushRHIThread);
...
void FTexture2DMipAllocator_Virtual::UnlockNewMips(FRHICommandListImmediate& RHICmdList)
{
	// Unlock any locked mips.
	if (IntermediateTextureRHI)
	{
		...
		{
			RHICmdList.UnlockTexture2D(IntermediateTextureRHI, MipIndex, false, CVarFlushRHIThreadOnSTreamingTextureLocks.GetValueOnAnyThread() > 0 );
		}
		LockedMipIndices.Empty();
	}
}

void FTexture2DMipAllocator_Virtual::DoConvertToVirtualWithNewMips(FRHICommandListImmediate& RHICmdList, const FTextureUpdateContext& Context)
{
	if (Context.Resource)
	{
