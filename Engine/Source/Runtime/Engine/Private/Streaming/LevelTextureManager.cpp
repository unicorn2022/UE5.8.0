...
{ 
	auto DetachComponent = [](const IPrimitiveComponent* PrimitiveComponent)
	{
		if (PrimitiveComponent)
		{
			// Check that this component was not already destroyed.
			check(!PrimitiveComponent->GetUObject() || PrimitiveComponent->GetUObject()->IsValidLowLevelFast());

			// Don't check as there can be duplicates in PendingComponents
			//check(PrimitiveComponent->IsAttachedToStreamingManagerAsStatic());

			// A component can only be referenced in one level, so if it was here, we can clear the flag
			PrimitiveComponent->GetStreamingState().SetAttachedToStreamingManagerAsStatic(false);
		}
	};

	StaticInstances.ForEachReferencedComponent(DetachComponent);
	Algo::ForEach(UnprocessedComponents, DetachComponent);
	Algo::ForEach(PendingComponents, DetachComponent);
...
void FLevelRenderAssetManager::SetAsStatic(FDynamicRenderAssetInstanceManager& DynamicComponentManager, const IPrimitiveComponent* Primitive, bool bIsConcurrent)
{
	check(Primitive);
	FRenderAssetOwnerStreamingState& StreamingState = Primitive->GetStreamingState();

	StreamingState.SetAttachedToStreamingManagerAsStatic(true);
	if (StreamingState.IsHandledByStreamingManagerAsDynamic())
	{
		...
		StreamingState.SetHandledByStreamingManagerAsDynamic(false);
	}
}


void FLevelRenderAssetManager::SetAsDynamic(FDynamicRenderAssetInstanceManager& DynamicComponentManager, FStreamingTextureLevelContext& LevelContext, const IPrimitiveComponent* Primitive, bool bIsConcurrent)
{
	check(Primitive);
	FRenderAssetOwnerStreamingState& StreamingState = Primitive->GetStreamingState();
...
void FLevelRenderAssetManager::IncrementalBuild(FDynamicRenderAssetInstanceManager& DynamicComponentManager, FStreamingTextureLevelContext& LevelContext, bool bForceCompletion, int64& NumStepsLeft)
{
	...
	case EStaticBuildStep::ProcessActors:
	{
		...
		{
			TArray<const IPrimitiveComponent*> UnprocessedComponents{};
			int32 NumSteps = 0;
		};
		...
		ParallelForWithTaskContext(TEXT("ProcessActors"), Contexts, Level->Actors.Num(), MinBatchSize, [this, &DynamicComponentManager, &LevelContext, bIsParallelForAllowed](FProcessActorContext& Context, int32 Index)
		{
			if (const AActor* Actor = Level->Actors[Index])
			{
				...
				Actor->ForEachComponent<UPrimitiveComponent>(false, [this, &DynamicComponentManager, &Context, &LevelContext, &NumSteps, &bIsStaticActor, bIsParallelForAllowed]
				(UPrimitiveComponent* Primitive)
				{
					if (bIsStaticActor && Primitive->Mobility == EComponentMobility::Static)
					{
						SetAsStatic(DynamicComponentManager, Primitive->GetPrimitiveComponentInterface(), bIsParallelForAllowed);
						Context.UnprocessedComponents.Push(Primitive->GetPrimitiveComponentInterface());
					}
					else
					{
						SetAsDynamic(DynamicComponentManager, LevelContext, Primitive->GetPrimitiveComponentInterface(), bIsParallelForAllowed);
					}
					++NumSteps;
				});
	...
	case EStaticBuildStep::ProcessComponents:
	{
		...
		if (FRenderAssetStreamingManager::IsParallelForAllowedDuringIncrementalUpdate(ToProcessComponentCount, MinBatchSize) && StaticInstances.CanAddComponent())
		{
			...
			// Because LevelContext is not thread-safe when ComponentBuildData is valid, we force not to use built data
			{
				...
				ParallelFor(TEXT("PreAddComponents"), ToProcessComponentCount, MinBatchSize, [&](int32 Index)
				{
					...
					const IPrimitiveComponent* Primitive = UnprocessedComponents[UnprocessedComponentCount - Index - 1];
		...
		else
		{
			...
			{
				const IPrimitiveComponent* Primitive = UnprocessedComponents.Pop(EAllowShrinking::No);
	...
	case EStaticBuildStep::WaitForRegistration:
		if (Level->bIsVisible)
		{
			...
			TArray<const IPrimitiveComponent*> RemovedPrimitives;
			NumStepsLeft -= (int64)StaticInstances.CheckRegistrationAndUnpackBounds(RemovedPrimitives);
			for (const IPrimitiveComponent* Primitive : RemovedPrimitives)
			{
				SetAsDynamic(DynamicComponentManager, LevelContext, Primitive);
			}
			...
			else
			{
				...
				{
					const IPrimitiveComponent* Primitive = PendingComponents.Pop(EAllowShrinking::No);
