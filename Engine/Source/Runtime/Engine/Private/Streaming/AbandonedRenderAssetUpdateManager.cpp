...
FAbandonedRenderAssetUpdateManager::~FAbandonedRenderAssetUpdateManager()
{
	UE_CLOGF(!AbandonedRenderAssetUpdates.IsEmpty(),
		LogContentStreaming,
		Warning,
		"FAbandonedRenderAssetUpdateManager shutdown before processing %d render asset updates", AbandonedRenderAssetUpdates.Num());
...
{
	UE_CLOGF(!IsInGameThread(), LogContentStreaming, Fatal, "FAbandonedRenderAssetUpdateManager::OnAbandoned is expected to only be called on the game thread");
	RenderAssetUpdate->OnAbandoned();
	AbandonedRenderAssetUpdates.Add(RenderAssetUpdate);
	UE_LOGF(LogContentStreaming, Log, "FAbandonedRenderAssetUpdateManager Abandoned pending render asset update [Name:%ls]", *OwningRenderAsset->GetName());
}
...
void FAbandonedRenderAssetUpdateManager::TickAbandoned()
{
	...
	UE_CLOGF(!IsInGameThread(), LogContentStreaming, Fatal, "FAbandonedRenderAssetUpdateManager::TickAbandoned is expected to only tick on the game thread");
