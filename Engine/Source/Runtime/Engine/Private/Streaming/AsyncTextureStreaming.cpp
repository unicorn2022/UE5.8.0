...
void FAsyncRenderAssetStreamingData::UpdatePerfectWantedMips_Async(FStreamingRenderAsset& StreamingRenderAsset, const FRenderAssetStreamingSettings& Settings, bool bOutputToLog) const
{
	...
	{
		if (StreamingRenderAsset.LastRenderTime < 300 || StreamingRenderAsset.GetFlag(FStreamingRenderAssetFlags::ForceFullyLoad))
		{
	...
	else
	{
		...
		if (FSimpleStreamableAssetManager::IsAssetRegistered(StreamingRenderAsset.SSAMAssetHandle))
		{
			...
			if (!bAlreadyAtMaximumResolution)
			{
				...
				FSimpleStreamableAssetManager::GetRenderAssetScreenSize(AssetType, StreamingRenderAsset.SSAMAssetHandle, TmpMaxSize, TmpMaxVisibleOnly, TmpMaxNumForcedLODs, MaxAssetSize, StreamingRenderAsset.MaxAllowedMips, bOutputToLog ? TEXT("Static") : nullptr);
		...
		// SSAM removals don't flow through FRenderAssetStreamingManager::SetRenderAssetsRemovedTimestamp
		// the way legacy component/level removals do, so InstanceRemovedTimestamp would sit at its
		// -FLT_MAX default for SSAM-managed assets and bUseUnkownRefHeuristic would fire the instant
		// MaxSize drops to zero, skipping the 5 sec grace period meant to cover transient ref churn
		// (component recreation, level transitions). Edge-detect the "SSAM registered -> not registered"
		// transition here using the handle we already carry, and stamp the timestamp when we observe
		// the flip. The previous-pass bit lives on the streaming asset so SSAM stays unaware of the
		// streaming manager and the handle table doesn't grow a per-slot field that only the asset
		// side would ever use.
		//
		// InstanceRemovedTimestamp is FStreamingRenderAsset::FAtomicTimestamp (relaxed-atomic double)
		// because this write happens on the mip-calc task while legacy GT writers in
		// FRenderAssetStreamingManager can be stamping concurrently. Both writers use
		// FApp::GetCurrentTime(); whichever store wins yields a valid recent-removal time.
		const bool bIsSSAMRegistered = FSimpleStreamableAssetManager::IsAssetRegistered(StreamingRenderAsset.SSAMAssetHandle);
		if (StreamingRenderAsset.GetFlag(FStreamingRenderAssetFlags::WasSSAMRegistered) && !bIsSSAMRegistered)
		{
			StreamingRenderAsset.InstanceRemovedTimestamp = FApp::GetCurrentTime();
		}
		StreamingRenderAsset.SetFlag(FStreamingRenderAssetFlags::WasSSAMRegistered, bIsSSAMRegistered);
		...
		bool bUseUnkownRefHeuristic = MaxSize == 0 && MaxSize_VisibleOnly == 0 && !MaxNumForcedLODs && StreamingRenderAsset.LastRenderTime < TimeSinceRemoved - 5.f;
		if (bUseUnkownRefHeuristic)
		{
			if (Settings.bMipCalculationEnablePerLevelList)
			{
				bUseUnkownRefHeuristic = !bCulled;
			}
			else
			{
			...
			if (bUseUnkownRefHeuristic && StreamingRenderAsset.LastRenderTime < 90.0f && ((Settings.DropMips != 3 && Settings.DropMips != 4) || AssetType == EStreamableRenderAssetType::Texture))
			{
				if (bOutputToLog) UE_LOGF(LogContentStreaming, Log,  "  UnkownRef");
		...
		StreamingRenderAsset.SetFlag(FStreamingRenderAssetFlags::UseUnkownRefHeuristic, bUseUnkownRefHeuristic);

		// TODO: for meshes, how to determine whether they are HLOD?
		if (StreamingRenderAsset.GetFlag(FStreamingRenderAssetFlags::ForceFullyLoad)
		...
		{
			if (bOutputToLog) UE_LOGF(LogContentStreaming, Log,  "  Forced FullyLoad");
		...
		{
			if (bOutputToLog) UE_LOGF(LogContentStreaming, Log,  "  HLOD Strategy");
...
void FRenderAssetStreamingMipCalcTask::UpdatePendingStreamingStatus_Async()
{
	...
	for (int32 AssetIndex = 0; AssetIndex < StreamingRenderAssets.Num() && !IsAborted(); ++AssetIndex)
	{
		...
		if (StreamingTexture.GetFlag(FStreamingRenderAssetFlags::HasUpdatePending) != StreamingTexture.HasUpdatePending(bIsStreamingPaused, HasAnyView()))
		{
...
void FRenderAssetStreamingMipCalcTask::UpdateStats_Async()
{
	...
	for (const FStreamingRenderAsset& StreamingRenderAsset : StreamingRenderAssets)
	{
		...
		if (GIsEditor && StreamingRenderAsset.GetFlag(FStreamingRenderAssetFlags::ForceFullyLoadHeuristic))
		{
			Stats.ForcedMips += UsedSize;
		}
		else if (StreamingRenderAsset.GetFlag(FStreamingRenderAssetFlags::UseUnkownRefHeuristic))
		{
		...
		else
		{
			...
			else // VisibleWantedSize < UsedSize
			{
				...
				if (StreamingRenderAsset.GetFlag(FStreamingRenderAssetFlags::ForceFullyLoadHeuristic)
