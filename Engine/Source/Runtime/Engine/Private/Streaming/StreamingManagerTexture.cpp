...
static int32 GRenderAssetStreamingEnableStreamOutProtection = 1;
static FAutoConsoleVariableRef CVarStreamingEnableStreamOutProtection(
	TEXT("r.Streaming.EnableStreamOutProtection"),
	GRenderAssetStreamingEnableStreamOutProtection,
	TEXT("Whether streamable render assets can be added to the streaming manager's stream-out protection list."),
	ECVF_Default);
...
,	LastWorldUpdateTime_MipCalcTask(LastWorldUpdateTime)
{
	...
	{
		UE_LOGF(LogContentStreaming, Log, "Textures will NEVER stream out!");
	}
	...
	for ( int32 Idx=0; Idx < StreamingRenderAssets.Num(); ++Idx )
	{
		...
		if (StreamingRenderAsset.RenderAsset)
		{
			...
			if ( TimeLeft >= 0.0f )
			{
				...
				UE_LOGF(LogContentStreaming, Log, "Canceling forced texture: %ls (had %.1f seconds left)", *StreamingRenderAsset.Texture->GetFullName(), TimeLeft );
#endif
			}
		}
	}
...
bool FRenderAssetStreamingManager::StreamOutRenderAssetData( int64 RequiredMemorySize )
{
	...
	for (int32 PriorityIndex = PrioritizedRenderAssets.Num() - 1; PriorityIndex >= 0 && MemoryDropped < RequiredMemorySize; --PriorityIndex)
	{
		...
		// Protection-aware StreamOut: held by external consumer (e.g. SkinWeightProfile readback)
		// returns false. Next eviction pass picks the asset up after release.
		if (StreamOutProtected(StreamingRenderAsset.RenderAsset, StreamingRenderAsset.MinAllowedMips))
		{
	...
	UE_LOGF(LogContentStreaming, Log, "Streaming out texture memory! Saved %.2f MB.", float(MemoryDropped)/1024.0f/1024.0f);
	return true;
}
...
void FRenderAssetStreamingManager::TickFastResponseAssets()
{
	...
	for (TSet<UStreamableRenderAsset*>::TIterator It(FastResponseRenderAssets); It; ++It)
	{
		...
		if (Asset.ResidentMips == Asset.RequestedMips && Asset.ResidentMips < Asset.MaxAllowedMips)
		{
			...
			Asset.SetFlag(FStreamingRenderAssetFlags::HasUpdatePending, true);
...
void FRenderAssetStreamingManager::ProcessAddedRenderAssets()
{
	...
	for (UStreamableRenderAsset* Asset : PendingStreamingRenderAssets)
	{
		// Could be null if it was removed after being added.
		if (Asset)
		{
			Asset->StreamingIndex = StreamingRenderAssets.Num();
			TConstArrayView<int32> NumStreamedMips = GetNumStreamedMipsArray(Asset->GetRenderAssetType());
			new (StreamingRenderAssets) FStreamingRenderAsset(Asset, NumStreamedMips, Settings);
		}
	}
...
void FRenderAssetStreamingManager::ConditionalUpdateStaticData()
{
		...
		PreviousSettings != Settings)
	{
		...
		if (!FSimpleStreamableAssetManager::IsEnabled()
				&& (bDebugSettingsChanged || PreviousSettings.MaterialQualityLevel != Settings.MaterialQualityLevel))
		{
			...
			DynamicComponentManager.ForEachReferencedComponent(
				[this](const IPrimitiveComponent* PrimitiveComponent)
				{
					NotifyPrimitiveUpdated_Concurrent(PrimitiveComponent);
				});
		}
...
void FRenderAssetStreamingManager::RemoveStreamingRenderAsset( UStreamableRenderAsset* RenderAsset )
{
	...
	{
		// Ensure the protected array can never hold a stale pointer.
		FRWScopeLock Lock(StreamOutProtectedAssetsLock, SLT_Write);
		StreamOutProtectedAssets.RemoveSingleSwap(RenderAsset, EAllowShrinking::No);
	}

	STAT(GatheredStats.CallbacksCycles += FPlatformTime::Cycles();)
}

bool FRenderAssetStreamingManager::TryRegisterStreamOutProtectedAsset(UStreamableRenderAsset* Asset)
{
	if (!Asset)
	{
		return false;
	...
	// HasPendingInitOrStreaming() is tested under the same lock that StreamOut paths
	// take (read), so concurrent StreamOut paths are either:
	//  - blocked waiting for this write lock and will see Asset in the array, or
	//  - already finished — in which case HasPendingInitOrStreaming() reports true
	//    and we bail.
	...
	if (GRenderAssetStreamingEnableStreamOutProtection)
	{
		StreamOutProtectedAssets.AddUnique(Asset);
	}
	return true;
}

void FRenderAssetStreamingManager::UnregisterStreamOutProtectedAsset(UStreamableRenderAsset* Asset)
{
	if (!Asset)
	{
		return;
	...
	FRWScopeLock Lock(StreamOutProtectedAssetsLock, SLT_Write);
	StreamOutProtectedAssets.RemoveSingleSwap(Asset, EAllowShrinking::No);
}

bool FRenderAssetStreamingManager::StreamOutProtected(UStreamableRenderAsset* Asset, int32 NewMipCount)
{
	FRWScopeLock Lock(StreamOutProtectedAssetsLock, SLT_ReadOnly);
	if (StreamOutProtectedAssets.Contains(Asset))
	{
		return false;
	}
	return Asset->StreamOut(NewMipCount);
}
...
void FRenderAssetStreamingManager::NotifyActorDestroyed( AActor* Actor )
{
	...
	for (FLevelRenderAssetManager* LevelManager : LevelRenderAssetManagers)
	{
		if (LevelManager!=nullptr && (!Level || LevelManager->GetLevel() == Level))
		{
			...
			{
				LevelManager->RemoveComponentReferences(Component->GetPrimitiveComponentInterface(), RemovedRenderAssets);
			}
		}
	}
	...
	for (UPrimitiveComponent* Component : Components)
	{
		...
		DynamicComponentManager.Remove(Component->GetPrimitiveComponentInterface(), &RemovedRenderAssets);

		// Reset this now as we have finished iterating over the levels
		Component->SetAttachedToStreamingManagerAsStatic(false);
	}
...
void FRenderAssetStreamingManager::RemoveStaticReferences(const IPrimitiveComponent* Primitive)
{
	if (FSimpleStreamableAssetManager::IsEnabled())
	{
		return;
	...
	check(Primitive);
	FRenderAssetOwnerStreamingState& StreamingState = Primitive->GetStreamingState();

	if (StreamingState.IsAttachedToStreamingManagerAsStatic())
	{
		FRemovedRenderAssetArray RemovedRenderAssets;
		...
		StreamingState.SetAttachedToStreamingManagerAsStatic(false);
 ...
 */
void FRenderAssetStreamingManager::NotifyPrimitiveDetached( const IPrimitiveComponent* Primitive )
{
	if (FSimpleStreamableAssetManager::IsEnabled())
	{
		return;
	...
	if (!Primitive || !Primitive->GetStreamingState().IsAttachedToStreamingManager())
	{
		return;
	}
	...
	FRemovedRenderAssetArray RemovedRenderAssets;
		...
		UE_LOGF(LogContentStreaming, Log, "NotifyPrimitiveDetached(0x%08x \"%ls\"), IsRegistered=%d", SIZE_T(Primitive), *Primitive->GetReadableName(), Primitive->IsRegistered());
#endif
	...
	if (StreamingState.IsAttachedToStreamingManagerAsStatic())
	{
		...
		{
			const UObject* PrimitiveObject = Primitive->GetUObject();
			// Unless in editor, we don't want to remove reference in static level data when toggling visibility.
			if (GIsEditor || !IsValid(PrimitiveObject) || PrimitiveObject->HasAnyFlags(RF_BeginDestroyed | RF_FinishDestroyed))
			{
				check(!LevelRenderAssetManagersLock);
				for (FLevelRenderAssetManager* LevelManager : LevelRenderAssetManagers)
				{
...
void FRenderAssetStreamingManager::InvalidateFileCache()
{
	FScopeLock ScopeLock(&MountedStateDirtyFilesCS);
...
void FRenderAssetStreamingManager::NotifyPrimitiveUpdated( const IPrimitiveComponent* Primitive )
{
	if (FSimpleStreamableAssetManager::IsEnabled() || !Primitive)
	{
		return;
	}
	...
	if (IsInGameThread() && bUseDynamicStreaming && Primitive && !StreamingState.IsIgnoreStreamingManagerUpdate())
	{
		UE::TScopeLock ScopeLock(CriticalSection);
		...
		if (StreamingState.IsHandledByStreamingManagerAsDynamic())
		{
		...
		else if (StreamingState.IsAttachedToStreamingManagerAsStatic())
		{
			...
			StreamingState.SetHandledByStreamingManagerAsDynamic(true);
			bUpdatePrimitive = !bHasRenderStateUpdateScheduled;
		}
		else
		{
 ...
 */
void FRenderAssetStreamingManager::NotifyPrimitiveUpdated_Concurrent( const IPrimitiveComponent* Primitive )
{
	if (FSimpleStreamableAssetManager::IsEnabled())
	{
		return;
...
void FRenderAssetStreamingManager::UpdateIndividualRenderAsset( UStreamableRenderAsset* RenderAsset )
{
	...
	const TConstArrayView<int32> NumStreamedMips = GetNumStreamedMipsArray(StreamingRenderAsset->RenderAssetType);
	StreamingRenderAsset->UpdateDynamicData(NumStreamedMips, Settings);

	if (StreamingRenderAsset->GetFlag(FStreamingRenderAssetFlags::ForceFullyLoad)) // Somewhat expected at this point.
	{
 ...
 */
void FRenderAssetStreamingManager::UpdateStreamingRenderAssets(int32 StageIndex, int32 NumUpdateStages, bool bAsync)
{
	...
	if (GAllowParallelUpdateStreamingRenderAssets)
	{
		struct FPacket
		{
			FPacket(int32 InStartIndex, int32 InEndIndex, TArrayView<FStreamingRenderAsset> InStreamingRenderAssets)
		...
		for (int32 i = 0; i < NumThreadTasks; ++i)
		{
			...
			Packets.Emplace(Start, Start + NumAssetsToProcess, AsyncUnsafeStreamingRenderAssets);
		...
		ParallelFor(TEXT("RenderAssetStreaming"), Packets.Num(), 1, [this, &Packets](int32 PacketIndex)
		{
			...
			for (int32 Index = Packets[PacketIndex].StartIndex; Index < Packets[PacketIndex].EndIndex; ++Index)
			{
				...
				StreamingRenderAsset.UpdateDynamicData(NumStreamedMips, Settings, &Packets[PacketIndex].LocalDeferredTickCBAssets); // We always use the Deferred CBs when doing the ParallelFor since those CBs are not thread safe.
				...
				{
					Packets[PacketIndex].MipIOBandwidth += StreamingRenderAsset.GetSize(StreamingRenderAsset.ResidentMips) - StreamingRenderAsset.GetSize(PreviousResidentMips);
				}
#endif
			}
		...
		for (const FPacket& Packet : Packets)
		{
			...
			STAT(GatheredStats.MipIOBandwidth += Packet.MipIOBandwidth;)
		}
	}
	else
	{
		for (int32 Index = StartIndex; Index < EndIndex; ++Index)
		{
			...
			StreamingRenderAsset.UpdateDynamicData(NumStreamedMips, Settings, bAsync ? &DeferredTickCBAssets : nullptr);
...
void FRenderAssetStreamingManager::StreamRenderAssets( bool bProcessEverything )
{
	...
	for (int32 AssetIndex : AsyncTask.GetPendingUpdateDirties())
	{
		if (StreamingRenderAssets.IsValidIndex(AssetIndex) && !VisibleFastResponseRenderAssetIndices.Contains(AssetIndex))
		{
			...
			StreamingRenderAsset.SetFlag(FStreamingRenderAssetFlags::HasUpdatePending, bNewState);
			if (StreamingRenderAsset.RenderAsset)
			{
	...
	for (TSet<int32>::TConstIterator It(VisibleFastResponseRenderAssetIndices); It; ++It)
	{
		...
		if (StreamingRenderAssets.IsValidIndex(Idx))
		{
			...
			Asset.SetFlag(FStreamingRenderAssetFlags::IgnoreStreamingMipBias, true);
...
{	
	if (CVarStreamingUseFixedPoolSize.GetValueOnGameThread() == 0)
	{
		...
		{
			UE_LOGF(LogContentStreaming,Log,"Texture pool size now %d MB", int32(TexturePoolSize/1024/1024));
...
void FRenderAssetStreamingManager::LogViewLocationChange()
{
	...
	{
		UE_LOGF(LogContentStreaming, Log, "Texture streaming view location is now %ls.", bIsLocationOverridden ? TEXT("OVERRIDDEN") : TEXT("normal") );
...
public:
	FUpdateStreamingRenderAssetsTask(FRenderAssetStreamingManager* InManager, int32 InStageIdx, int32 InNumUpdateStages)
		: Manager(InManager)
		, StageIdx(InStageIdx)
...
void FRenderAssetStreamingManager::UpdateResourceStreaming( float DeltaTime, bool bProcessEverything/*=false*/ )
{
	...
	if (NumRenderAssetProcessingStages <= 0 || bProcessEverything)
	{
		...
		// Update Thread Data
		SetLastUpdateTime();
		UpdateStreamingRenderAssets(0, 1);
	...
	else if (ProcessingStage <= NumRenderAssetProcessingStages)
	{
		...
		if (bOverlappedExecution)
		{
			...
			.ConstructAndDispatchWhenReady(this, ProcessingStage - 1, NumRenderAssetProcessingStages);
		}
		else
		{
...
int32 FRenderAssetStreamingManager::BlockTillAllRequestsFinished( float TimeLimit /*= 0.0f*/, bool bLogResults /*= false*/ )
{
	...
	while (ensure(!IsAssetStreamingSuspended()))
	{
		...
		{
			const bool bValid = StreamingRenderAsset.UpdateStreamingStatus().IsValid();
		...
		else
		{
			if (bLogResults)
			{
				...
				{
					UE_LOGF(LogContentStreaming, Log, "Blocking on texture streaming: %.1f ms (%d still in flight)", BlockedMillis, NumOfInFlights);
				}
			}
...
static void GatherComponentsFromView(
	...
	const TFunction<bool(const IPrimitiveComponent*)>& ShouldChoose,
	TArray<const IPrimitiveComponent*>& OutComps)
{
	if (View)
	{
		for (FRenderAssetInstanceView::FRenderAssetLinkConstIterator It = View->GetElementIterator(RenderAsset); It; ++It)
		{
			const IPrimitiveComponent* Comp = It.GetComponent();
...
void FRenderAssetStreamingManager::GetAssetComponents(const UStreamableRenderAsset* RenderAsset, TArray<const IPrimitiveComponent*>& OutComps, TFunction<bool(const IPrimitiveComponent*)> ShouldChoose)
{
...
void FRenderAssetStreamingManager::AddRenderedTextureStats(TMap<FString, FRenderedTextureStats>& OutRenderedTextureStats)
{
	...
	for (const FStreamingRenderAsset& StreamingRenderAsset : AsyncUnsafeStreamingRenderAssets)
	{
		...
		if (RenderAsset == nullptr || StreamingRenderAsset.GetFlag(FStreamingRenderAssetFlags::UseUnkownRefHeuristic)
...
bool FRenderAssetStreamingManager::HandleListStreamingRenderAssetsCommand( const TCHAR* Cmd, FOutputDevice& Ar )
{
	...
	for ( int32 Idx=0; Idx < StreamingRenderAssets.Num(); ++Idx)
	{
		...
		if (!StreamingRenderAsset.RenderAsset) continue;
		if (bShouldOnlyListUnkownRef && !StreamingRenderAsset.GetFlag(FStreamingRenderAssetFlags::UseUnkownRefHeuristic)) continue;
	...
	for (TMap<FString, int32>::TConstIterator It(SortedRenderAssets); It; ++It)
	{
		...
		UE_LOGF(LogContentStreaming, Log, "%ls [%d] : %ls",
		...
		if (AssetType == EStreamableRenderAssetType::Texture && TexturePlatformData && *TexturePlatformData)
		{
			...
			{
				UE_LOGF(LogContentStreaming, Log, "    Current=%ls  Wanted=%ls MaxAllowed=%ls LastRenderTime=%.3f BudgetBias=%d Group=%ls",
			...
			{
				UE_LOGF(LogContentStreaming, Log, "    Current=%ls Wanted=%ls MaxAllowed=%ls BudgetBias=%d Group=%ls",
					*OutputMipsString(CurrentMipIndex), *OutputMipsString(WantedMipIndex), *OutputMipsString(MaxAllowedMipIndex),
...
#endif
			UE_LOGF(LogContentStreaming, Log, "    CurrentLOD=%d WantedLOD=%d MaxAllowedLOD=%d NumLODs=%d NumForcedLODs=%d LastRenderTime=%ls BudgetBias=%d Group=%ls",
				CurrentMipIndex,
				WantedMipIndex,
				MaxAllowedMipIndex,
...
bool FRenderAssetStreamingManager::HandleLODGroupsCommand( const TCHAR* Cmd, FOutputDevice& Ar )
{
	...
	// Output stats.
	{
		UE_LOGF(LogContentStreaming, Log, "Texture memory usage:");
		...
		for (int32 GroupIndex = 0; GroupIndex < TEXTUREGROUP_MAX; ++GroupIndex)
		{
			...
			if (Stat.NumStreamingTextures || Stat.NumNonStreamingTextures || Stat.NumNoResourceTextures)
			{
				...
				UE_LOGF(LogContentStreaming, Log, "%34ls: NumStreamingTextures=%4d { Current=%8.1f KB, Wanted=%8.1f KB, OnDisk=%8.1f KB }, NumNonStreaming=%4d { Size=%8.1f KB }, NumWithNoResource=%4d",
	...
	if (InvestigateAssetName.Len())
	{
		...
		for (int32 i = 0; i < AssetIndices.Num(); ++i)
		{
			...
			if (bInvestigateAll || AssetName.Contains(InvestigateAssetName))
			{
				...
				UE_LOGF(LogContentStreaming, Log, "%ls: %ls", FStreamingRenderAsset::GetStreamingAssetTypeStr(AssetType), *AssetName);
				...
				{
					UE_LOGF(LogContentStreaming, Log, "  LOD group:       %ls [Bucket=%ls]", *LODGroupName, BucketNames[(int32)FPlatformMemory::GetMemorySizeBucket()]);
				}
				else
				{
					UE_LOGF(LogContentStreaming, Log, "  LOD group:       %ls [Unkown Bucket]", *LODGroupName);
				}
				if (Texture && Texture->GetRunningPlatformData() && *Texture->GetRunningPlatformData())
				{
					UE_LOGF(LogContentStreaming, Log, "  Format:          %ls", GPixelFormats[(*Texture->GetRunningPlatformData())->PixelFormat].Name);
				}
				if (RenderAsset->bGlobalForceMipLevelsToBeResident)
				{
					UE_LOGF(LogContentStreaming, Log, "  Force all mips:  bGlobalForceMipLevelsToBeResident");
				}
				else if (RenderAsset->bForceMiplevelsToBeResident)
				{
					UE_LOGF(LogContentStreaming, Log, "  Force all mips:  bForceMiplevelsToBeResident");
				}
				else if (RenderAsset->ShouldMipLevelsBeForcedResident())
				{
					...
					UE_LOGF(LogContentStreaming, Log, "  Force all mips:  %.1f seconds left", FMath::Max(TimeLeft, 0.0f));
				}
				else if (StreamingRenderAsset.GetFlag(FStreamingRenderAssetFlags::ForceFullyLoadHeuristic))
				{
					UE_LOGF(LogContentStreaming, Log, "  Force all mips:  bForceFullyLoad");
				}
				else if (ResourceState.MaxNumLODs == 1)
				{
					UE_LOGF(LogContentStreaming, Log, "  Force all mips:  No mip-maps");
				}
				...
				if (Texture && !Texture->IsCurrentlyVirtualTextured() && Texture->GetRunningPlatformData() && *Texture->GetRunningPlatformData())
				{
					...
					if (Texture2D)
					{
						UE_LOGF(LogContentStreaming, Log, "  Current size [2D Mips]: %dx%d [%d]", TextureMips[CurrentMipIndex].SizeX, TextureMips[CurrentMipIndex].SizeY, StreamingRenderAsset.ResidentMips);
						UE_LOGF(LogContentStreaming, Log, "  Wanted size [2D Mips]:  %dx%d [%d]", TextureMips[WantedMipIndex].SizeX, TextureMips[WantedMipIndex].SizeY, StreamingRenderAsset.GetPerfectWantedMips());
					}
					else if (VolumeTexture)
					{
						UE_LOGF(LogContentStreaming, Log, "  Current size [3D Mips]: %dx%dx%d [%d]", TextureMips[CurrentMipIndex].SizeX, TextureMips[CurrentMipIndex].SizeY, TextureMips[CurrentMipIndex].SizeZ, StreamingRenderAsset.ResidentMips);
						UE_LOGF(LogContentStreaming, Log, "  Wanted size [3D Mips]:  %dx%dx%d [%d]", TextureMips[WantedMipIndex].SizeX, TextureMips[WantedMipIndex].SizeY, TextureMips[CurrentMipIndex].SizeZ, StreamingRenderAsset.GetPerfectWantedMips());
					}
					else if (Texture2DArray)
					{
						UE_LOGF(LogContentStreaming, Log, "  Current size [2D Array Mips]: %dx%d*%d [%d]", TextureMips[CurrentMipIndex].SizeX, TextureMips[CurrentMipIndex].SizeY, TextureMips[CurrentMipIndex].SizeZ, StreamingRenderAsset.ResidentMips);
						UE_LOGF(LogContentStreaming, Log, "  Wanted size [2D Array Mips]:  %dx%d*%d [%d]", TextureMips[WantedMipIndex].SizeX, TextureMips[WantedMipIndex].SizeY, TextureMips[CurrentMipIndex].SizeZ, StreamingRenderAsset.GetPerfectWantedMips());
					}
				}
				else
				{
					UE_LOGF(LogContentStreaming, Log, "  Current LOD index: %d", CurrentMipIndex);
					UE_LOGF(LogContentStreaming, Log, "  Wanted LOD index: %d", WantedMipIndex);
				}
				UE_LOGF(LogContentStreaming, Log, "  Allowed mips:        %d-%d [%d]", StreamingRenderAsset.MinAllowedMips, StreamingRenderAsset.MaxAllowedMips, ResourceState.MaxNumLODs);
				UE_LOGF(LogContentStreaming, Log, "  LoadOrder Priority:  %d", StreamingRenderAsset.LoadOrderPriority);
				UE_LOGF(LogContentStreaming, Log, "  Retention Priority:  %d", StreamingRenderAsset.RetentionPriority);
				UE_LOGF(LogContentStreaming, Log, "  Boost factor:        %.1f", StreamingRenderAsset.BoostFactor);
				...
				FString BiasDesc;
				{
					...
					if (!Settings.bUseAllMips)
					{
						...
						if (RenderAsset->NumCinematicMipLevels && !(StreamingRenderAsset.GetFlag(FStreamingRenderAssetFlags::ForceFullyLoad) && RenderAsset->bUseCinematicMipLevels))
						{
				...
				UE_LOGF(LogContentStreaming, Log, "  Mip bias:			  %d %ls", ResourceState.MaxNumLODs - StreamingRenderAsset.MaxAllowedMips, !BiasDesc.IsEmpty() ? *BiasDesc : TEXT("") );

				if (InWorld && !GIsEditor)
				{
					UE_LOGF(LogContentStreaming, Log, "  Time: World=%.3f LastUpdate=%.3f ", InWorld->GetTimeSeconds(), LastWorldUpdateTime);
				}
				...
				for (int32 ViewIndex = 0; ViewIndex < StreamingData.GetViewInfos().Num(); ViewIndex++)
				{
					...
					UE_LOGF(LogContentStreaming, Log, "  View%d: Position=(%ls) ScreenSize=%f MaxEffectiveScreenSize=%f Boost=%f", ViewIndex, *ViewInfo.ViewOrigin.ToString(), ViewInfo.ScreenSize, Settings.MaxEffectiveScreenSize, ViewInfo.BoostFactor);
				}
