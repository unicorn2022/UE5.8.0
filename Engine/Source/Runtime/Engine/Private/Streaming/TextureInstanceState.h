...
public:
	...
	{
		const IPrimitiveComponent* Component;
	...
	static void PreAddComponent(const IPrimitiveComponent* Component, FStreamingTextureLevelContext& LevelContext, float MaxAllowedUIDensity, FPreAddComponentPayload& OutPayload);
	static void PreAddComponentIgnoreBounds(const IPrimitiveComponent* Component, FStreamingTextureLevelContext& LevelContext, FPreAddComponentIgnoreBoundsPayload& OutPayload);

	// Will also remove bounds
	EAddComponentResult AddComponent(const IPrimitiveComponent* Component, FStreamingTextureLevelContext& LevelContext, float MaxAllowedUIDensity);
	...
	EAddComponentResult AddComponentIgnoreBounds(const IPrimitiveComponent* Component, FStreamingTextureLevelContext& LevelContext);
	EAddComponentResult AddComponentIgnoreBounds(const FPreAddComponentIgnoreBoundsPayload& Payload);

	FORCEINLINE bool HasComponentReferences(const IPrimitiveComponent* Component) const { return ComponentMap.Contains(Component); }
	...
	void RemoveComponent(const IPrimitiveComponent* Component, FRemovedRenderAssetArray* RemovedRenderAssets);
	...
	bool RemoveComponentReferences(const IPrimitiveComponent* Component);
	...
	void ForEachReferencedComponent(CallableT&& Callable) const
	{
		for (const TPair<const IPrimitiveComponent*, int32>& Component : ComponentMap)
		{
			Callable(Component.Key);
		}
	}

	void UpdateBounds(const IPrimitiveComponent* Component);
	...
	int32 CheckRegistrationAndUnpackBounds(TArray<const IPrimitiveComponent*>& RemovedComponents);
...
private:
	...
	void AddElement(const IPrimitiveComponent* Component, const UStreamableRenderAsset* Asset, int BoundsIndex, float TexelFactor, bool bForceLoad, int32*& ComponentLink);
	...
	int32 AddBounds(const FBoxSphereBounds& Bounds, uint32 PackedRelativeBox, const IPrimitiveComponent* Component, float LastRenderTime, const FVector4& RangeOrigin, float MinDistanceSq, float MinRangeSq, float MaxRangeSq);
	FORCEINLINE int32 AddBounds(const IPrimitiveComponent* Component);
	void RemoveBounds(int32 Index);

	EAddComponentResult AddComponentInternal(const FPreAddComponentPayload& Payload);
	static EAddComponentResult AddComponentInternal(const IPrimitiveComponent* Component, FStreamingTextureLevelContext& LevelContext, float MaxAllowedUIDensity, FRenderAssetInstanceState* State, FPreAddComponentPayload* OutPreAddPayload);
	static EAddComponentResult AddComponentIgnoreBoundsInternal(const IPrimitiveComponent* Component, FStreamingTextureLevelContext& LevelContext, FRenderAssetInstanceState* State, FPreAddComponentIgnoreBoundsPayload* OutPreAddPayload);

	void AddRenderAssetElements(const IPrimitiveComponent* Component, const TArray<FPreAddRenderAssetElement>& RenderAssetElements, int32 BoundsIndex, int32*& ComponentLink);
	void AddRenderAssetElements(const IPrimitiveComponent* Component, const TArrayView<FStreamingRenderAssetPrimitiveInfo>& RenderAssetInstanceInfos, int32 BoundsIndex, int32*& ComponentLink);
...
private:
	 ...
	 */
	TArray<const IPrimitiveComponent*> Bounds4Components;
	...
	TMap<const IPrimitiveComponent*, int32> ComponentMap;
