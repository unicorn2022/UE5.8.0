...
static int32 GGPUSkinBoneTransformAllocationMode = 1;
static FAutoConsoleVariableRef CVarGPUSkinBoneTransformAllocationMode(
	TEXT("r.GPUSkin.BoneTransformAllocationMode"),
	GGPUSkinBoneTransformAllocationMode,
	TEXT("Where to prefer to allocate bone transforms for non-Nanite skeletal meshes.")
	TEXT(" 0 - Vertex Factory: Per-section bone buffers are allocated by the Vertex Factory all the time")
	TEXT(" 1 - Scene:          Bone transforms are allocated in the scene extension when possible."),
...
EBoneTransformAllocationMode GetBoneTransformAllocationMode()
{
	return GGPUSkinBoneTransformAllocationMode == 0 || !IsGPUSkinSceneExtensionEnabled()
		? EBoneTransformAllocationMode::VertexFactory
		: EBoneTransformAllocationMode::Scene;
}

bool GGPUSkinSceneExtensionEnabled = true;
static FAutoConsoleVariableRef CVarGPUSkinUseSceneExtension(
	TEXT("r.GPUSkin.UseSceneExtension"),
	GGPUSkinSceneExtensionEnabled,
	TEXT("When set, allows GPU skin skeletal meshes to be registered with the skinning scene extension."),
	ECVF_ReadOnly
);

bool IsGPUSkinSceneExtensionEnabled()
{
	return GGPUSkinSceneExtensionEnabled;
}
...
inline ESkeletalMeshGPUSkinTechnique GetGPUSkinTechnique(USkinnedMeshComponent* MeshComponent, FSkeletalMeshRenderData* RenderData, int32 LODIndex, ERHIFeatureLevel::Type FeatureLevel)
{
	...
	if (MeshComponent)
	{
		...
		else if (!AreInlineSkinningShadersEnabled(GetFeatureLevelShaderPlatform(FeatureLevel)) || MeshComponent->IsSkinCacheAllowed(LODIndex, FeatureLevel))
		{
...
inline ESkeletalMeshGPUSkinTechnique GetGPUSkinTechnique(const FSkinnedMeshSceneProxyDesc& InMeshDesc, FSkeletalMeshRenderData* RenderData, int32 LODIndex, ERHIFeatureLevel::Type FeatureLevel)
{
	...
	else if (!AreInlineSkinningShadersEnabled(GetFeatureLevelShaderPlatform(FeatureLevel)) || InMeshDesc.IsSkinCacheAllowed(LODIndex))
	{
		GPUSkinTechnique = ESkeletalMeshGPUSkinTechnique::GPUSkinCache;
...
uint32 FMorphVertexBufferPool::GetReadingIndex(bool bPrevious) const
{
	uint32 Index = 0;
	if (bDoubleBuffer)
	{
...
{
	const uint32 Index = GetReadingIndex(bPrevious);
...
const FMorphVertexBuffer* FMorphVertexBufferPool::TryGetMorphVertexBufferForReading(bool bPrevious) const
{
	const uint32 Index = GetReadingIndex(bPrevious);
	if (!MorphVertexBuffers[Index].VertexBufferRHI.IsValid())
	{
		return nullptr;
	}
	return &MorphVertexBuffers[Index];
...
void FSkeletalMeshUpdatePacketGPUSkin::Add(FSkeletalMeshObjectGPUSkin* MeshObject, FDynamicSkelMeshObjectDataGPUSkin* DynamicData)
{
	...
	if (MeshObject->PreviousDynamicData)
	{
		DynamicDatasToFree.Emplace(MeshObject->PreviousDynamicData);
	}
...
{
	BoneTransformStorageMode = EBoneTransformStorageMode::BoneMap;
	...
	BoneTransformAllocationMode = ::GetBoneTransformAllocationMode();
	...
	for (int32 LODIndex=0; LODIndex < SkeletalMeshRenderData->LODRenderData.Num(); LODIndex++)
	{
		...
		FSkeletalMeshObjectLOD& LOD = LODs.Emplace_GetRef(SkeletalMeshRenderData, LODIndex, InFeatureLevel, RecreateMorphVertexBuffer, GPUSkinTechnique);

		bool bHasValidSections = false;

		for (const FSkelMeshRenderSection& Section : GetRenderSections(LODIndex))
		{
			if (Section.IsValid())
			{
		...
		// Fall back to vertex factory buffer allocation mode if any of the LOD's require it or if an LOD has no valid sections in it.
		if (!bHasValidSections || GPUSkinTechnique != ESkeletalMeshGPUSkinTechnique::Inline)
		{
			BoneTransformAllocationMode = EBoneTransformAllocationMode::VertexFactory;
		}
	}
...
{
	FDynamicSkelMeshObjectDataGPUSkin::Release(DynamicData);
	FDynamicSkelMeshObjectDataGPUSkin::Release(PreviousDynamicData);
	DynamicData = nullptr;
...
void FSkeletalMeshObjectGPUSkin::InitResources(const FSkinnedMeshSceneProxyDesc& InMeshDesc)
{
	...
	for( int32 LODIndex=0;LODIndex < LODs.Num();LODIndex++ )
	{
		...
		if (SkelLOD.SkelMeshRenderData && SkelLOD.SkelMeshRenderData->LODRenderData.IsValidIndex(LODIndex) && SkelLOD.SkelMeshRenderData->LODRenderData[LODIndex].GetNumVertices() > 0)
		{
			...
			else if (SkelLOD.GPUSkinTechnique == ESkeletalMeshGPUSkinTechnique::GPUSkinCache || (FGPUSkinCache::IsGPUSkinCacheRayTracingSupported(FeatureLevel) && SkelLOD.SkelMeshRenderData->bSupportRayTracing))
			{
			...
			SkelLOD.InitResources(GPUSkinCache, MeshLODInfo, CompLODInfo, FeatureLevel, VertexAttributeMask, BoneTransformStorageMode, BoneTransformAllocationMode);
		}
	}
	...
	const FExternalMorphWeightData& InExternalMorphWeightData)
{
	...
	const int32 NumBones = InDynamicData.bUseReferenceToLocalMatrices ? InDynamicData.ReferenceToLocalMatrices.Num() : InDynamicData.ComponentSpaceTransforms.Num();
	FDynamicSkelMeshObjectDataGPUSkin* NewDynamicData = FDynamicSkelMeshObjectDataGPUSkin::Acquire(NumBones);
...
void FSkeletalMeshObjectGPUSkin::UpdateSkinWeightBuffer(const TArrayView<const FSkelMeshComponentLODInfo> InLODInfo)
{
	for (int32 LODIndex = 0; LODIndex < LODs.Num(); LODIndex++)
	{
		...
		if (SkelLOD.SkelMeshRenderData->LODRenderData[LODIndex].GetNumVertices() > 0)
		{
			...
			SkelLOD.UpdateSkinWeights(CompLODInfo, BoneTransformStorageMode);
		}
	}
...
{
	return FGPUSkinCache::IsGPUSkinCacheEnabled(FeatureLevel) &&
...
void FSkeletalMeshObjectGPUSkin::UpdateBufferData(FRHICommandList& RHICmdList, EGPUSkinCacheEntryMode Mode)
{
	...
	if (BoneTransformAllocationMode == EBoneTransformAllocationMode::VertexFactory)
	{
		...
		if (Mode == EGPUSkinCacheEntryMode::Raster && DynamicData->PreviousBoneTransformUpdateMode != EPreviousBoneTransformUpdateMode::None)
		{
		...
		if (LOD.UnifiedBoneBuffer)
		{
			if (!PreviousReferenceToLocalMatrices.IsEmpty())
			{
				...
				LOD.UnifiedBoneBuffer->Update(RHICmdList, OwnerName, PreviousReferenceToLocalMatrices, bPrevious);
			}
		...
		for (int32 SectionIdx = 0; SectionIdx < Sections.Num(); SectionIdx++)
		{
			...
			{
				VertexFactory->UpdateUniformBuffer(RHICmdList, nullptr, nullptr);
			}
		}
	}
}
...
void FSkeletalMeshObjectGPUSkin::ProcessUpdatedDynamicData(FRHICommandList& RHICmdList, EGPUSkinCacheEntryMode Mode)
{
	...
	// Invalidate cached scene extension bone data until next call to UpdateSceneExtensionHeader.
	SceneExtensionBoneBuffer = {};
	...
	if (Mode == EGPUSkinCacheEntryMode::Raster)
	{
		...
		if (GEnableMorphTargets && LODData.GetNumVertices() > 0)
		{
			if (bHasWeightedActiveMorphs || bHasExternalMorphs)
			{
			...
			// The update is still set to true when transitioning from morphs to no-morphs. We need to clear out the previous buffer contents.
			else if (bMorphNeedsUpdate && LOD.MorphVertexBufferPool->IsInitialized())
			{
				MorphVertexBuffer = &LOD.MorphVertexBufferPool->GetMorphVertexBufferForWriting();
				MorphVertexBuffer->bNeedsInitialClear = true;
	...
	const bool bGPUSkinCacheEnabled = GPUSkinCache && DynamicData->GPUSkinTechnique == ESkeletalMeshGPUSkinTechnique::GPUSkinCache && FGPUSkinCache::IsGPUSkinCacheEnabled(FeatureLevel);
	...
	{
		bool bReleasedSkinCacheEntry = false;

		if (SkinCacheEntry)
		{
			FGPUSkinCache::Release(SkinCacheEntry);
			bReleasedSkinCacheEntry = true;
		}
	...
	if (LOD.UnifiedBoneBuffer)
	{
		LOD.UnifiedBoneBuffer->FlipBuffers();
	...
	for (int32 SectionIdx = 0; SectionIdx < Sections.Num(); SectionIdx++)
	{
		...
		if (BoneTransformAllocationMode == EBoneTransformAllocationMode::VertexFactory)
		{
			const auto AllocateBoneBuffer = [&](bool bPrevious)
			{
				FVertexBufferAndSRV& Buffer = ShaderData.GetBoneBufferForWriting(bPrevious);
				if (LOD.UnifiedBoneBuffer)
				{
					Buffer = LOD.UnifiedBoneBuffer->GetBuffer(bPrevious);
				}
				else
				{
					ShaderData.AllocateBoneBuffer(RHICmdList, VertexFactory->GetBoneBufferSize(), Buffer);
				}
			};
	...
	FRayTracingGeometry& RayTracingGeometry, bool bAnySegmentUsesWorldPositionOffset, FSkeletalMeshObject* MeshObject)
{
	if (IsRayTracingEnabled() && MeshObject->bSupportRayTracing)
	{
		...
		bool bRequireRecreatingRayTracingGeometry = LODIndex != RayTracingGeometry.LODIndex
			...
			|| RayTracingGeometry.GetInitializer().Segments.Num() == 0;

		if (!bRequireRecreatingRayTracingGeometry)
		{
			for (const FRayTracingGeometrySegment& Segment : RayTracingGeometry.GetInitializer().Segments)
			{
		...
		{
			check(LODModel.RenderSections.Num() == RayTracingGeometry.GetInitializer().Segments.Num());
...
FSkeletalMeshObjectGPUSkin::FSkeletalMeshObjectLOD::FSkeletalMeshObjectLOD(
	FSkeletalMeshRenderData* InSkelMeshRenderData,
	int32 InLOD,
	ERHIFeatureLevel::Type InFeatureLevel,
	FMorphVertexBufferPool* InRecreateBufferPool,
	ESkeletalMeshGPUSkinTechnique InSkinTechnique)
...
inline FSkinWeightVertexBufferInfo GetSkinWeightVertexBufferInfo(const FSkinWeightVertexBuffer* Buffer)
{
	FSkinWeightVertexBufferInfo Info;

	if (Buffer)
	{
		Info.InputWeightStreamSRV		= Buffer->GetDataVertexBuffer()->GetSRV();
		Info.InputWeightLookupStreamSRV = Buffer->GetLookupVertexBuffer()->GetSRV();
		Info.InputWeightIndexSize       = Buffer->GetBoneIndexByteSize() | (Buffer->GetBoneWeightByteSize() << 8);
		Info.InputWeightStride          = Buffer->GetConstantInfluencesVertexStride();
		Info.MaxBoneInfluences          = Buffer->GetMaxBoneInfluences();
		Info.BoneInfluenceType          = Buffer->GetBoneInfluenceType();
		Info.bUse16BitBoneIndex         = Buffer->Use16BitBoneIndex();
		Info.bUse16BitBoneWeight        = Buffer->Use16BitBoneWeight();
	}

	return Info;
}

void FSkeletalMeshObjectGPUSkin::FSkeletalMeshObjectLOD::UpdateSkinWeights(const FSkelMeshComponentLODInfo* CompLODInfo, EBoneTransformStorageMode InBoneTransformStorageMode)
{
	...
	if (CompLODInfo)
	{
		...
		if (MeshObjectWeightBuffer != NewMeshObjectWeightBuffer)
		{
			...
			GetVertexBuffers(VertexBuffers, LODData, InBoneTransformStorageMode);

			FSkeletalMeshObjectLOD* Self = this;

			ENQUEUE_RENDER_COMMAND(UpdateSkinWeightsGPUSkin)(UE::RenderCommandPipe::SkeletalMesh,
				[NewMeshObjectWeightBuffer, VertexBuffers, Self](FRHICommandList& RHICmdList) mutable
			{
				Self->MeshObjectWeightBuffer_RenderThread = GetSkinWeightVertexBufferInfo(VertexBuffers.SkinWeightVertexBuffer);
	...
	FMorphVertexBuffer& InMorphVertexBuffer)
{
	...
	const int32 RecomputeTangentsMode = FGPUSkinCache::GetGPUSkinCacheRecomputeTangentsMode(FeatureLevel);
	bool bAllSectionsDoGPURecomputeTangent = bGPUSkinCacheEnabled && RecomputeTangentsMode > 0;
	if (bAllSectionsDoGPURecomputeTangent && RecomputeTangentsMode == 2)
	{
...
const FVertexFactory* FSkeletalMeshObjectGPUSkin::GetSkinVertexFactory(const FSceneView* View, int32 LODIndex, int32 ChunkIdx, ESkinVertexFactoryMode VFMode) const
{
	...
	// Return the passthrough vertes factory for Skin Cache.
	if (DynamicData->GPUSkinTechnique == ESkeletalMeshGPUSkinTechnique::GPUSkinCache)
	{
		check(FGPUSkinCache::IsEntryValid(SkinCacheEntry, ChunkIdx));
		
		return LOD.GPUSkinVertexFactories.PassthroughVertexFactory.Get();
	}

	// Shouldn't get here if inline shaders were disabled.
	check(AreInlineSkinningShadersEnabled(GetFeatureLevelShaderPlatform(FeatureLevel)))
...
FSkinWeightVertexBufferInfo FSkeletalMeshObjectGPUSkin::GetSkinWeightVertexBuffer_RenderThread(int32 LODIndex) const
{
...
FSkinningSceneExtensionProxy* FSkeletalMeshObjectGPUSkin::CreateSceneExtensionProxy(const USkinnedAsset* InSkinnedAsset, bool bAllowScaling) const
{
	if (IsGPUSkinSceneExtensionEnabled() && BoneTransformAllocationMode == EBoneTransformAllocationMode::Scene)
	{
...
void FSkeletalMeshObjectGPUSkin::UpdateSceneExtensionHeader(FRHICommandListBase& RHICmdList, const FSkinningHeader& SkinningHeader, FRHIShaderResourceView* TransformDataBufferSRV) const
{
	check(DynamicData);
	const FSkeletalMeshObjectLOD& LOD = LODs[DynamicData->LODIndex];
	check(BoneTransformAllocationMode == EBoneTransformAllocationMode::Scene);

	// Cache for GetSectionBoneBuffer.
	SceneExtensionBoneBuffer.TransformBufferSRV = TransformDataBufferSRV;
	SceneExtensionBoneBuffer.TransformBufferOffset = SkinningHeader.TransformBufferOffset;
	SceneExtensionBoneBuffer.MaxTransformCount = SkinningHeader.MaxTransformCount;
	SceneExtensionBoneBuffer.CurrentTransformSlot = SkinningHeader.CurrentTransformSlot;

	for (const TUniquePtr<FGPUBaseSkinVertexFactory>& VertexFactory : LOD.GPUSkinVertexFactories.VertexFactories)
	{
		if (VertexFactory)
		{
...
void InitGPUSkinVertexFactoryComponents(FGPUSkinDataType* VertexFactoryData, const FSkeletalMeshObjectGPUSkin::FVertexFactoryBuffers& VertexBuffers, FGPUBaseSkinVertexFactory* VertexFactory)
{
	...
	FGPUBaseSkinVertexFactory::FShaderDataType* ShaderData = VertexFactory ? &VertexFactory->GetShaderData() : nullptr;
	...
	bool bUnifiedBoneBuffer)
{
	...
	const FGPUBaseSkinVertexFactory::FInitializer Initializer
	{
		...
		, .bUnifiedBoneBuffer = bUnifiedBoneBuffer
	};
	...
	FPSOPrecacheVertexFactoryDataList& VertexFactoryDataList)
{
	...
	FVertexFactoryBuffers VertexBuffers;
	SkeletalMeshObjectLOD.GetVertexBuffers(VertexBuffers, LODRenderData, EBoneTransformStorageMode::Direct);
	...
	if (GPUSkinTechnique != ESkeletalMeshGPUSkinTechnique::Inline || FGPUSkinCache::IsGPUSkinCacheRayTracingSupported(InFeatureLevel))
	{
	...
	// Only add GPU skin cache vertex factory if run inline (not GPUSkinCache or MeshDeformer)
	if (GPUSkinTechnique == ESkeletalMeshGPUSkinTechnique::Inline)
	{
 ...
 */
void FSkeletalMeshObjectGPUSkin::FSkeletalMeshObjectLOD::GetVertexBuffers(FVertexFactoryBuffers& OutVertexBuffers, FSkeletalMeshLODRenderData& LODData, EBoneTransformStorageMode StorageMode)
{
	...
	bool bUnifiedBoneBuffer)
{
	...
	{
		const int32 NumBones = Section.BoneMap.Num();
		...
		else if (Section.HasClothingData() && bClothEnabled)
		{
			...
			CreateVertexFactoryCloth(RHICmdList, VertexFactories, Passthrough, VertexBuffers, InFeatureLevel, VertexAttributeMask, NumBones, BoneOffset, Section.BaseVertexIndex, NumInfluencesPerVertex, bUsedForPassthroughVertexFactory, bUnifiedBoneBuffer);
		}
		else
		{
			CreateVertexFactory(RHICmdList, VertexFactories, Passthrough, VertexBuffers, InFeatureLevel, VertexAttributeMask, NumBones, BoneOffset, Section.BaseVertexIndex, bUsedForPassthroughVertexFactory, bUnifiedBoneBuffer);
		}

		BoneOffset += Section.IsValid() && !bUnifiedBoneBuffer ? Section.BoneMap.Num() : 0;
	}
}
...
void FSkeletalMeshObjectGPUSkin::FSkeletalMeshObjectLOD::InitResources(
	...
	ERHIFeatureLevel::Type InFeatureLevel,
	FGPUSkinPassthroughVertexFactory::EVertexAttributeFlags VertexAttributeMask,
	EBoneTransformStorageMode InBoneTransformStorageMode,
	EBoneTransformAllocationMode InBoneTransformAllocationMode)
{
	check(SkelMeshRenderData);
	check(SkelMeshRenderData->LODRenderData.IsValidIndex(LODIndex));
	const bool bHasUnifiedBoneMap = SkelMeshRenderData->HasUnifiedBoneMap();
	...
	{
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		UE::TScopeLock Lock(LODData.RenderDataMutex);
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	if (InBoneTransformAllocationMode == EBoneTransformAllocationMode::VertexFactory && bHasUnifiedBoneMap)
	{
		UnifiedBoneBuffer = MakeUnique<FUnifiedBoneBuffer>(SkelMeshRenderData->GetUnifiedBoneMap());
	}

	ENQUEUE_RENDER_COMMAND(FSkeletalMeshObjectLOD_InitResources)(UE::RenderCommandPipe::SkeletalMesh,
		[this, InGPUSkinCache, VertexBuffers, &LODData, InFeatureLevel, VertexAttributeMask, bHasUnifiedBoneMap](FRHICommandList& RHICmdList)
	{
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		UE::TScopeLock Lock(LODData.RenderDataMutex);
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
				
		MeshObjectWeightBuffer_RenderThread = GetSkinWeightVertexBufferInfo(VertexBuffers.SkinWeightVertexBuffer);
		GPUSkinVertexFactories.InitVertexFactories(RHICmdList, VertexBuffers, LODData.RenderSections, InFeatureLevel, VertexAttributeMask, GPUSkinTechnique, bHasUnifiedBoneMap);
...
FSkeletalMeshObject::FBoneTransforms FSkeletalMeshObjectGPUSkin::GetBoneTransforms() const
{
...
FSkeletalMeshObject::FBoneBufferSRV FSkeletalMeshObjectGPUSkin::GetSectionBoneBuffer(int32 InLODIndex, int32 InSectionIndex, bool bPreviousFrame) const
{
	...
	const FGPUBaseSkinVertexFactory* BaseVertexFactory = GetBaseSkinVertexFactory(InLODIndex, InSectionIndex);
	if (!BaseVertexFactory)
	{
		return Result;
	}

	if (BoneTransformAllocationMode == EBoneTransformAllocationMode::Scene)
	{
		// Scene skinning extension path: shared transform buffer with combined offset.
		Result.SRV = SceneExtensionBoneBuffer.TransformBufferSRV;
		Result.Offset = SceneExtensionBoneBuffer.TransformBufferOffset
			+ ((bPreviousFrame ? 1u : 0u) ^ SceneExtensionBoneBuffer.CurrentTransformSlot) * SceneExtensionBoneBuffer.MaxTransformCount
			+ BaseVertexFactory->GetBoneOffset();
	}
	else
	{
		const FGPUBaseSkinVertexFactory::FShaderDataType& ShaderData = BaseVertexFactory->GetShaderData();
	...
	const FExternalMorphWeightData& InExternalMorphWeightData)
{
	...
	{
		UE_LOGF(LogSkeletalGPUSkinMesh, Fatal,
			"Skeletal mesh %ls, LOD %d is not set to use the mesh deformer skin technique, but the component deformer instance is valid. " "This means a mesh deformer was added but the skeletal mesh object was not recreated.",
