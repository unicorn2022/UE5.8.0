...
template<typename BufferType, int ElementSize, EPixelFormat PixelFormat>
void UpdateInternalBuffer(FRHICommandListBase& RHICmdList, const TCHAR* DebugName, const uint32 ElementCount, const BufferType* InputData, FRWBuffer& OutputBuffer, const bool bInitField = false, ERHIAccess InitAccess = ERHIAccess::SRVMask)
{
	if (ElementCount > 0 && InputData)
	{
		...
		{
			LLM_SCOPE(ELLMTag::Physics);
			OutputBuffer.Initialize(RHICmdList, DebugName, sizeof(BufferType), BufferCount, PixelFormat, InitAccess, BUF_Static);
		}
...
public:
	...
	{
		return !IsMobilePlatform(Parameters.Platform) && IsFeatureLevelSupported(Parameters.Platform, ERHIFeatureLevel::SM5);
	}
					...
					if (MaxBound.X > MinBound.X && MaxBound.Y > MinBound.Y && MaxBound.Z > MinBound.Z)
					{
						...
						if (LocalMin.X < LocalMax.X && LocalMin.Y < LocalMax.Y && LocalMin.Z < LocalMax.Z)
						{
							...
							const float CellsRatio = FieldInfos.CellsOffsets[CellIndex + 1] / FMath::Pow(FieldInfos.ClipmapResolution, 3.0);
								...
								/*UE_LOGF(LogGlobalField, Log, "Field Bounds[%d][%d] = %d %d %d -> %d %d %d | Cells Ratio = %f | %d", TargetType, ClipmapIndex,
	...
	const TArray<FVector4>& BoundsMinDatas, const TArray<FVector4>& BoundsMaxDatas, const TStaticArray<int32, EFieldPhysicsType::Field_PhysicsType_Max + 1>& BoundsOffsetsDatas)
{
	...
	if (FieldInfos.bBuildClipmap)
	{
		...
		}
		{
			...
			RHICmdList.Transition(FRHITransitionInfo(this->ClipmapBuffer.UAV, ERHIAccess::UAVCompute, ERHIAccess::SRVMask));
		}
	}
}
...
void FPhysicsFieldInstance::InitInstance( const TArray<EFieldPhysicsType>& TargetTypes, const bool bBuildClipmap, const bool bNeedsResource)
{
	...
	if (!FieldResource && bNeedsResource)
	{
...
void UPhysicsFieldComponent::SendRenderDynamicData_Concurrent()
{
	...
	for (uint32 FieldIndex = 0; FieldIndex < 2; ++FieldIndex)
	{
		...
		if (LocalInstance)
		{
			...
			if (bIsDebugBuffer && TransientLifetime > 0.0f)
			{
...
void UPhysicsFieldComponent::OnRegister()
{
	...
	{
		const EShaderPlatform ShaderPlatfom = GetWorld()->Scene->GetShaderPlatform();
		bNeedsResource = !IsMobilePlatform(ShaderPlatfom) && IsFeatureLevelSupported(ShaderPlatfom, ERHIFeatureLevel::SM5);
	}
	...
	for (uint32 FieldIndex = 0; FieldIndex < 2; ++FieldIndex)
	{
		...
		if (!LocalInstance)
		{
			...
			if (GPhysicsFieldSingleTarget == 1)
			{
				...
				LocalInstance->InitInstance(TargetTypes, bBuildClipmaps[FieldIndex], bNeedsResource);
			}
			else
			{
