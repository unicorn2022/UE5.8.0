...
void UInputSettings::RemoveInvalidKeys()
{
	...
	for (const FInputActionKeyMapping& KeyMapping : ActionMappings)
	{
		...
		{
			UE_LOGF(LogInput, Warning, "Action %ls uses invalid key %ls.", *KeyMapping.ActionName.ToString(), *KeyMapping.Key.ToString());
	...
	if (InvalidIndices.Num())
	{
		...
		{
			UE_LOGF(LogInput, Warning, "Use -RemoveInvalidKeys to remove instances of these keys from the action mapping.");
		}
	}
}
...
void UInputSettings::PostInitProperties()
{
	...
	for (const FInputActionKeyMapping& KeyMapping : ActionMappings)
	{
		...
		{
			UE_LOGF(LogInput, Warning, "Action %ls uses deprecated key %ls.", *KeyMapping.ActionName.ToString(), *KeyMapping.Key.ToString());
		}
	}
	...
	for (const FInputAxisKeyMapping& KeyMapping : AxisMappings)
	{
		if (KeyMapping.Key.IsDeprecated())
		{
			UE_LOGF(LogInput, Warning, "Axis %ls uses deprecated key %ls.", *KeyMapping.AxisName.ToString(), *KeyMapping.Key.ToString());
		}
	}
...
void UInputSettings::AddInternationalConsoleKey()
{
	...
	if (ConsoleKeys.Num() >= 1 && ConsoleKeys[0] == EKeys::Tilde)
	{
		...
		// We need to use this call because it gives the actual layout, rather than the locale
		if (GetKeyboardLayoutNameA(KeyboardLayoutName))
		{
			FKey DefaultConsoleKey = EKeys::Tilde;

			const int32 KeyboardLayoutId = strtol(KeyboardLayoutName, nullptr, 16);

			switch (PRIMARYLANGID(LOWORD(KeyboardLayoutId)))
			{
			case LANG_FRENCH:
			case LANG_HUNGARIAN:
			...
			{
				ConsoleKeys.AddUnique(DefaultConsoleKey);
			}
		}
	}
#endif
...
TArray<FHardwareDeviceIdentifier> UInputPlatformSettings::GetAllHardwareDeviceIdentifiers()
{
	TArray<FHardwareDeviceIdentifier> Result;

	// Always include the built-in defaults.
	Result.AddUnique(FHardwareDeviceIdentifier::DefaultKeyboardAndMouse);
	Result.AddUnique(FHardwareDeviceIdentifier::DefaultGamepad);
	Result.AddUnique(FHardwareDeviceIdentifier::DefaultMobileTouch);
