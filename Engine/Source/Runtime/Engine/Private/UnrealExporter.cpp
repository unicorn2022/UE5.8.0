...
bool UExporter::ExportToArchive( UObject* Object, UExporter* InExporter, FArchive& Ar, const TCHAR* FileType, int32 FileIndex )
{
	...
	if( !Exporter )
	{
		UE_LOGF(LogExporter, Warning, "No %ls exporter found for %ls", FileType, *Object->GetFullName() );
		return( false );
	}
...
bool UExporter::RunAssetExportTask(class UAssetExportTask* Task)
{
	...
	if (Exporter)
	{
		if (UObject* Object = Task->Object.Get())
		{
			if (!Object->IsA(Exporter->SupportedClass))
			{
				...
				UE_LOGF(LogExporter, Warning, "%ls", *Task->Errors.Last());
				return false;
			}
		}
	}
	...
	else if( Exporter->bText )
	{
		...
		if (!TextBuffer.IsValid())
		{
			...
			{
				UE_LOGF(LogExporter, Warning, "Failed to create file output device... defaulting to string buffer");
			}
		...
		else
		{
			...
			else
			{
				if ( !Task->bReplaceIdentical )
				{
					...
					{
						UE_LOGF(LogExporter, Log, "Not replacing %ls because identical", *Task->Filename);
						return true;
					}
					if( Task->bPrompt )
					{
...
#endif
					UE_LOGF(LogExporter, Error, "%ls", *FString::Printf(TEXT("Error exporting %s: couldn't open file '%s'"), *(Task->Object->GetFullName()), *Task->Filename));
					return false;
				}
				return true;
			}
		...
		for( int32 i = 0; i < FileCount; i++ )
		{
			...
			if(ExportToArchive(Task->Object, Exporter, Buffer, *Extension, i))
			{
				...
				if(!Task->bReplaceIdentical)
				{
					...
					{
						UE_LOGF(LogExporter, Log,  "Not replacing %ls because identical", *UniqueFilename );
						return true;
					}
					if(Task->bPrompt)
					{
				...
				if( !FFileHelper::SaveArrayToFile( Buffer, *UniqueFilename ) )
				{
					...
					UE_LOGF(LogExporter, Error, "%ls", *Task->Errors.Last());
					return false;
				}
			}
		}
...
void UExporter::ExportObjectInner(const FExportObjectInnerContext* Context, UObject* Object, FOutputDevice& Ar, uint32 PortFlags)
{
	...
	else
	{
		...
		GetObjectsWithOuter(Object, TempInners, EGetObjectsFlags::None, RF_NoFlags, EInternalObjectFlags::Garbage);
	}
...
(
	...
	FOverriddenPropertySet* OverriddenProperties,
	const FString&	PropertyPrefix = FString()
)
{
	check(Struct != NULL);
	...
	for (FProperty* Property = Struct->PropertyLink; Property; Property = Property->PropertyLinkNext)
	{
		...
		{
			SanitizedPropertyName = PropertyPrefix + "." + SanitizedPropertyName;
		}
		...
		for (int32 PropertyArrayIndex=0; PropertyArrayIndex<Property->ArrayDim; PropertyArrayIndex++)
		{
			...
			if (ArrayProperty != nullptr)
			{
				...
				FStructProperty* InnerStructProperty = CastField<FStructProperty>(InnerProp);
				if (InnerStructProperty != nullptr)
				{
					checkSlow(InnerStructProperty->Struct);
					StructDefaults = (uint8*)FMemory::Malloc(InnerStructProperty->Struct->GetStructureSize());
					InnerStructProperty->InitializeValue(StructDefaults);
				}
				ON_SCOPE_EXIT
				{
					if (StructDefaults)
					{
						InnerStructProperty->DestroyValue(StructDefaults);
						FMemory::Free(StructDefaults);
					}
				};

				void* Arr = Property->ContainerPtrToValuePtr<void>(ValuePtr, PropertyArrayIndex);
				FScriptArrayHelper ArrayHelper(ArrayProperty, Arr);

				void* DiffArr = nullptr;
				if (DiffStruct)
				{
					DiffArr = Property->ContainerPtrToValuePtrForDefaults<void>(DiffStruct, Diff, PropertyArrayIndex);
				}
				...
				if (OverriddenProperties)
				{
					...
					Operation = FOverridableSerializationLogic::GetOverriddenPropertyOperation(PortFlags, &Chain, nullptr, Arr, DiffArr);
				}
				...
				if (Operation == EOverriddenPropertyOperation::Modified)
				{
					if (const FOverriddenPropertyNode* ArrayOverriddenPropertyNode = OverriddenProperties->GetOverriddenPropertyNode(&Chain))
					{
						...
						// Only array of instanced subobjects are handled here as like a set where the matching key is done using the archetype
						...
						bool bExternalized = false;

						if (FOverridableSerializationLogic::ShouldPropertyShadowSerializeSubObject(ArrayProperty))
						{
							const int32 ArrayNum = ArrayHelper.Num();
							for (int i = 0; i < ArrayNum; i++)
							{
						...
						for (const FOverriddenPropertyNode& SubNode: ArrayOverriddenPropertyNode->GetSubPropertyNodes())
						{
							...
							case EOverriddenPropertyOperation::Remove:
								{
									const int32 DefaultIndex = FindObject(SubNode.GetNodeID(), DiffArrayHelper);
						...
						if(bExternalized)
						{
							Out.Logf(TEXT("%s%s<%s>=%s\r\n"), FCString::Spc(Indent), *SanitizedPropertyName, *GetOverriddenOperationString(EOverriddenPropertyOperation::Externalized), TEXT("\"\""));
						}
						...
						auto ExportArray = [&](TArray<int32> ExportIndices, const EOverriddenPropertyOperation Operation)
						{
							for (int32 i : ExportIndices)
						{
							...
							HandleExportObject(InnerProp, ArrayHelper.GetRawPtr(i), ArrayExportObjectProp);
								...
								Out.Logf(TEXT("%s%s<%s>=%s\r\n"), FCString::Spc(Indent), *SanitizedPropertyName, *GetOverriddenOperationString(Operation), *Value);
						}
						};

						ExportArray(AddedIndices, EOverriddenPropertyOperation::Add);
						ExportArray(ShadowIndices, EOverriddenPropertyOperation::SubObjectsShadowing);
					}
					continue;
				}
			}
			...
			if (MapProperty != nullptr)
			{
				...
				void* Map = Property->ContainerPtrToValuePtr<void>(ValuePtr, PropertyArrayIndex);
				void* DiffMap = nullptr;
				if (DiffStruct)
				{
					DiffMap = Property->ContainerPtrToValuePtrForDefaults<void>(DiffStruct, Diff, PropertyArrayIndex);
				}
				...
				if (Operation == EOverriddenPropertyOperation::Modified)
				{
					if (const FOverriddenPropertyNode* MapOverriddenPropertyNode = OverriddenProperties->GetOverriddenPropertyNode(&Chain))
					{
						...
						TArray<int32> AddedIndices;
						TArray<int32> ShadowedIndices;

						if (FOverridableSerializationLogic::ShouldPropertyShadowSerializeSubObject(MapProperty))
						{
							...
							for (int32 i : ExportIndices)
						{
							...
							FString Key;
								MapHelper.KeyProp->ExportTextItem_Direct(Key, MapHelper.GetKeyPtr(i), /*DefaultValue*/nullptr, Parent, ExportFlags, ExportRootScope);

							FString Value;
								MapHelper.ValueProp->ExportTextItem_Direct(Value, MapHelper.GetValuePtr(i), /*DefaultValue*/nullptr, Parent, ExportFlags, ExportRootScope);
						...
						ExportArray(ModifiedIndices, EOverriddenPropertyOperation::Modified);
						ExportArray(AddedIndices, EOverriddenPropertyOperation::Add);
						ExportArray(ShadowedIndices, EOverriddenPropertyOperation::SubObjectsShadowing);
					}
			...
			// Recursively call this method for struct in the overridable serialization case
			if (StructProperty != nullptr && Struct == DiffStruct && OverriddenProperties)
						{
				FOverridableTextPortPropertyPathScope ScopePath(StructProperty);

				ExportProperties_Overrides(
					Context, 
					Out, 
					StructProperty->Struct, 
					Property->ContainerPtrToValuePtr<uint8>(ValuePtr, PropertyArrayIndex), 
					Indent, 
					StructProperty->Struct,
					Diff ? Property->ContainerPtrToValuePtr<uint8>(Diff, PropertyArrayIndex) : nullptr,
					Parent,
					PortFlags,
					ExportRootScope,
			...
			uint8* DiffData = (DiffStruct && Property->IsInContainer(DiffStruct->GetPropertiesSize())) ? Diff : NULL;
			if (Property->ExportText_InContainer(PropertyArrayIndex, Value, ValuePtr, DiffData, Parent, ExportFlags, ExportRootScope))
			{
				HandleExportObject(Property, Property->ContainerPtrToValuePtr<uint8>(ValuePtr, PropertyArrayIndex), ExportObjectProp);

				FString OverridableOperation;
				if (OverriddenProperties)
				{
					const EOverriddenPropertyOperation Operation = FOverridableSerializationLogic::GetOverriddenPropertyOperationForPortText(ValuePtr, DiffData, PortFlags);
...
)
{
	if (FOverridableSerializationLogic::HasCapabilities(FOverridableSerializationLogic::ECapabilities::T3DSerialization))
	{
		...
		ExportProperties_Overrides(Context, Out, ObjectClass, Object, Indent, DiffClass, Diff, Parent, PortFlags, ExportRootScope, OverriddenProperties);
...
void DumpComponents(UObject *Object)
{
	...
	else
	{
		...
		UE_LOGF(LogExporter, Log, "%ls", *Output);
	}
}
...
void DumpObject(const TCHAR* Label, UObject* Object)
{
	...
	UE_LOGF(LogExporter, Display, "%ls", Label);
	UE_LOGF(LogExporter, Display, "%ls", *ExportedText);
}
