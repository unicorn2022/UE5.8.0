...
{
	checkf(Result.bRecording, TEXT("FReplayRecordHelper::StartRecordingComplete: bRecording is false. You may need to enable localReplay.UsePlayerAccountIdPostfix."));
	...
	{
		UE_LOGF(LogDemo, Warning, "FReplayRecordHelper::StartRecordingComplete: Failed");
...
void FReplayHelper::WriteNetworkDemoHeader(UNetConnection* Connection)
{
	...
	{
		UE_LOGF(LogDemo, Error, "WriteNetworkDemoHeader: Header archive is invalid.");
	}
}
...
APlayerController* FReplayHelper::CreateSpectatorController(UNetConnection* Connection)
{
	...
	{
		UE_LOGF(LogDemo, Error, "FReplayHelper::CreateDemoPlayerController: Failed to load demo spectator class.");
		return nullptr;
	}
	...
	{
		UE_LOGF(LogDemo, Error, "FReplayHelper::CreateDemoPlayerController: Failed to spawn demo spectator.");
		return nullptr;
	}
...
bool FReplayHelper::ReadPlaybackDemoHeader(FString& Error)
{
	...
	if (!FileAr)
	{
		...
		UE_LOGF(LogDemo, Error, "FReplayHelper::ReadPlaybackDemoHeader: %ls", *Error);
	...
	if (FileAr->IsError())
	{
		Error = FString("Demo file is corrupt");
		UE_LOGF(LogDemo, Error, "FReplayHelper::ReadPlaybackDemoHeader: %ls", *Error);
	...
	{
		UE_LOGF(LogDemo, Error, "FReplayHelper::ReadPlaybackDemoHeader: Replay has delta checkpoints but streamer does not support them.");
	...
	if (!Error.IsEmpty())
	{
		UE_LOGF(LogDemo, Error, "FReplayHelper::ReadPlaybackDemoHeader: (Game Specific) %ls", *Error);
...
void FReplayHelper::TickRecording(float DeltaSeconds, UNetConnection* Connection)
{
	...
	if (FileAr == nullptr)
	{
		UE_LOGF(LogDemo, Error, "FReplayHelper::TickRecording: FileAr == nullptr");
...
void FReplayHelper::SaveCheckpoint(UNetConnection* Connection)
{
	...
	UE_LOGF(LogDemo, Log, "Starting checkpoint. Networked Actors: %i", NetworkObjectList.GetAllObjects().Num());
...
void FReplayHelper::WriteDemoFrame(UNetConnection* Connection, FArchive& Ar, TArray<FQueuedDemoPacket>& QueuedPackets, float FrameTime, EWriteDemoFrameFlags Flags)
{
	...
	else
	{
		...
		for (uint32 i = 0; i < NumStreamingLevels; i++)
		{
			...
			UE_LOGF(LogDemo, Log, "WriteDemoFrame: StreamingLevel: %ls, %ls", *PackageName, *PackageNameToLoad);
		}
		...
		if (UObject* Object = Pair.Key.Get())
		{
			if (FExternalDataWrapper* ExternalData = ExternalDataMap.Find(Object))
			{
				if (ExternalData->NumBits > 0)
				{
					...
					{
						UE_LOGF(LogDemo, Warning, "SaveExternalData: Discarding external data for object with no net guid: %ls", *GetNameSafe(Object));
					}
				}
			}
		}
...
void FReplayHelper::CacheNetGuids(UNetConnection* Connection)
{
	if (Connection && Connection->Driver)
	{
		...
		UE_LOGF(LogDemo, Verbose, "CacheNetGuids: %d, %.1f ms", NumValues, (FPlatformTime::Seconds() - StartTime) * 1000);
	}
}
...
bool FReplayHelper::SetExternalDataForObject(UNetConnection* Connection, UObject* OwningObject, const uint8* Src, const int32 NumBits)
{
	...
	{
		UE_LOGF(LogDemo, Warning, "SetExternalDataForObject: Discarding external data for object, already exists: %ls", *GetNameSafe(OwningObject));
	}
...
bool FReplayHelper::ReplicateActor(AActor* Actor, UNetConnection* Connection, bool bMustReplicate)
{
	...
	{
		UE_LOGF(LogDemo, Error, "DemoReplicateActor: bMustReplicate is true but nothing was sent: %ls", *GetNameSafe(Actor));
	}
...
bool FReplayHelper::ReadDemoFrame(UNetConnection* Connection, FArchive& Ar, TArray<FPlaybackPacket>& InPlaybackPackets, const bool bForLevelFastForward, const FArchivePos MaxArchiveReadPos, float* OutTime)
{
	...
	if (Ar.IsError())
	{
		UE_LOGF(LogDemo, Error, "FReplayHelper::ReadDemoFrame: Archive Error");
	...
	else
	{
		...
		for (uint32 i = 0; i < NumStreamingLevels; ++i)
		{
			...
			UE_LOGF(LogDemo, Log, "ReadDemoFrame: Loading streamingLevel: %ls, %ls", *PackageName, *PackageNameToLoad);
		}
	}

	if (Ar.IsError())
	{
		UE_LOGF(LogDemo, Error, "FReplayHelper::ReadDemoFrame: Failed to read demo ServerDeltaTime");
		NotifyReplayError(EReplayResult::Serialization);
		return false;
	}
...

	{
		...
		while ((MaxArchiveReadPos == 0) || (Ar.Tell() < MaxArchiveReadPos))
		{
			...
			{
				UE_LOGF(LogDemo, Error, "FReplayHelper::ReadDemoFrame: ReadPacket failed.");
				NotifyReplayError(EReplayResult::Serialization);
				return false;
			}
...
const FReplayHelper::EReadPacketState FReplayHelper::ReadPacket(FArchive& Archive, TArray<uint8>& OutBuffer, const EReadPacketMode Mode)
{
	...
	{
		UE_LOGF(LogDemo, Error, "FReplayHelper::ReadPacket: Failed to read demo OutBufferSize");
	...
	{
		UE_LOGF(LogDemo, Error, "FReplayHelper::ReadPacket: OutBufferSize < 0");
		return EReadPacketState::Error;
	}
	...
	if (UNLIKELY(Archive.IsError()))
	{
		UE_LOGF(LogDemo, Error, "FReplayHelper::ReadPacket: Failed to read demo file packet");
		return EReadPacketState::Error;
	}
...
void FReplayHelper::AddOrUpdateEvent(const FString& Name, const FString& Group, const FString& Meta, const TArray<uint8>& Data)
{
	...
	UE_LOGF(LogDemo, Verbose, "AddOrUpdateEvent %ls.%ls. Total: %i, Time: %2.2f", *Group, *Name, Data.Num(), float(SavedTimeMS));
}
		...
		if (UActorChannel* Channel = Connection->FindActorChannelRef(Actor))
		{
			if (Channel->ChIndex != INDEX_NONE)
			{
				...
				if (int32* PendingIndex = CheckpointSaveContext.PendingActorToIndex.Find(Actor))
				{
					...
					{
						UE_LOGF(LogDemo, Verbose, "Removing actor while it is still in the PendingCheckpointActors list: %ls", *GetNameSafe(Actor));
...
void FReplayHelper::NotifyActorDestroyed(UNetConnection* Connection, AActor* Actor)
{
	...
	if (bNetStartup)
	{
		...
		UE_CLOGF(bActorRewindable, LogDemo, Warning, "Replay Rewindable Actor destroyed during recording. Replay may show artifacts (%ls)", *FullName);

		UE_LOGF(LogDemo, VeryVerbose, "NotifyActorDestroyed: adding actor to deleted startup list: %ls", *FullName);
...
UE::Net::EHandleNetResult FReplayResultHandler::HandleNetResult(UE::Net::FNetResult&& InResult)
{
	...
	UE_LOGF(LogDemo, Error, "FReplayResultHandler::HandleNetResult:");

	for (FNetResult::FConstIterator It(InResult); It; ++It)
	{
		UE_LOGF(LogDemo, Error, " - %ls", ToCStr(It->DynamicToString()));
	}
