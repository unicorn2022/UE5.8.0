...
void UHierarchicalInstancedStaticMeshComponent::FClusterBuilder::BuildTreeAndBufferAsync(ENamedThreads::Type CurrentThread, const FGraphEventRef& MyCompletionGraphEvent)
{
	...
	{
		UE_LOGF(LogStaticMesh, Warning, "BuildTree Debug Delay %5.1f (CVar foliage.DebugBuildTreeAsyncDelayInSeconds)", GDebugBuildTreeAsyncDelayInSeconds);
...
bool UHierarchicalInstancedStaticMeshComponent::FClusterTree::PrintLevel(int32 NodeIndex, int32 Level, int32 CurrentLevel, int32 Parent)
{
	...
	{
		UE_LOGF(LogConsoleResponse, Display, "Level %2d  Parent %3d",
			Level,
			Parent
			);
		FVector Extent(Node.BoundMax - Node.BoundMin);
		UE_LOGF(LogConsoleResponse, Display, "    Bound (%5.1f, %5.1f, %5.1f) [(%5.1f, %5.1f, %5.1f) - (%5.1f, %5.1f, %5.1f)]",
			Extent.X, Extent.Y, Extent.Z, 
			Node.BoundMin.X, Node.BoundMin.Y, Node.BoundMin.Z, 
			Node.BoundMax.X, Node.BoundMax.Y, Node.BoundMax.Z
			);
		UE_LOGF(LogConsoleResponse, Display, "    children %3d [%3d,%3d]   instances %3d [%3d,%3d]",
...
{
	UE_LOGF(LogConsoleResponse, Display, "Running Foliage test.");
...
{
#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
	UE_LOGF(LogConsoleResponse, Display, "Freezing Foliage Culling.");
	GDebugTag++;
...
{
#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
	UE_LOGF(LogConsoleResponse, Display, "Unfreezing Foliage Culling.");
	GDebugTag++;
	GCaptureDebugRuns = 0;
#endif
}
...
#endif
{
	...
	UserData_AllInstances.AverageInstancesScale = InComponent->GetAverageScale();
}
...
void FHierarchicalStaticMeshSceneProxy::SetupOcclusion(UHierarchicalInstancedStaticMeshComponent* InComponent)
{
	...
	if (NumNodes < 2)
	{
		...
		if (ClusterTree.Num())
		{
			//UE_LOGF(LogTemp, Display, "No SubOcclusion %d inst", 1 + ClusterTree[0].LastInstance - ClusterTree[0].FirstInstance);
		}
	}
	else
	{
		//int32 NumPerNode = (1 + ClusterTree[0].LastInstance - ClusterTree[0].FirstInstance) / NumNodes;
		//UE_LOGF(LogTemp, Display, "Occlusion level %d   %d inst / node", NumNodes, NumPerNode);
...
{
	const FInstancingUserData* PassUserData;
	const FSceneView* View;
	int32 ViewIndex;
	bool bSelectionRenderEnabled;
	bool bBatchRenderSelection;
	bool bIsWireframe;
...
void FHierarchicalStaticMeshSceneProxy::FillDynamicMeshElements(const FSceneView* View, FMeshElementCollector& Collector, const FFoliageElementParams& ElementParams, const FFoliageRenderInstanceParams& Params) const
{
	...
	const int32 NumVertexFactories = GetNumRenderDataVertexFactories();

	int32 OnlyLOD = FMath::Min<int32>(CVarOnlyLOD.GetValueOnRenderThread(), NumVertexFactories - 1);
	int32 FirstLOD = FMath::Max((OnlyLOD < 0) ? 0 : OnlyLOD, static_cast<int32>(this->GetCurrentFirstLODIdx_Internal()));
	int32 LastLODPlusOne = (OnlyLOD < 0) ? NumVertexFactories : (OnlyLOD+1);
	...
	for (int32 LODIndex = FirstLOD; LODIndex < LastLODPlusOne; LODIndex++)
	{
		...
		{
			FInstancedStaticMeshVFLooseUniformShaderParametersRef LooseUniformBuffer;
			if (InstancedRenderData)
			{
				LooseUniformBuffer = CreateLooseUniformBuffer(View, InstancedLODRange, LODIndex, EUniformBufferUsage::UniformBuffer_SingleFrame);
			}
			...
			for (int32 SectionIndex = 0; SectionIndex < TotalNumSections; SectionIndex++)
			{
				...
				if (bUseGPUScene)
				{
					...
					if (!FStaticMeshSceneProxy::GetMeshElement(LODIndex, 0, SectionIndex, GetDepthPriorityGroup(ElementParams.View), ElementParams.bBatchRenderSelection, true, MeshBatch))
					{
						continue;
					}
					...
					MeshBatch.bUseSelectionOutline = ElementParams.bBatchRenderSelection;
					MeshBatch.bUseWireframeSelectionColoring = ElementParams.bBatchRenderSelection;
					...
					// For dither LOD to work we need to override the VF UB with per-view dynamic data
					else if (bDitherLODEnabled)
					{
						const FLocalVertexFactory* VF = static_cast<const FLocalVertexFactory*>(MeshBatch.VertexFactory);
						FLocalVertexFactoryInstanceDitherParameters InstanceDitherParameters = GetInstanceDitherParameters(View, InstancedLODRange, LODIndex);
						MeshBatchElement.VertexFactoryUserData = CreateLocalVFUniformBuffer(VF, VF->GetLightMapCoordinateIndex(), nullptr, 0, 0, &InstanceDitherParameters, EUniformBufferUsage::UniformBuffer_SingleFrame);
					}

					MeshBatchElement.UserData = ElementParams.PassUserData;
					...
					int32 TotalInstances = bDitherLODEnabled ? Params.TotalMultipleLODInstances[LODIndex] : Params.TotalSingleLODInstances[LODIndex];
					{
						...
						if (GFrameNumberRenderThread_CaptureFoliageRuns == GFrameNumberRenderThread)
						{
							...
							{
								UE_LOGF(LogStaticMesh, Display, "lod:%1d/%1d   section:%1d/%1d   runs:%4d   inst:%8d   tris:%9lld   cast shadow:%1d   cull:-NONE!!-   shadow:%1d     %ls %ls",
									LODIndex, NumVertexFactories, SectionIndex, TotalNumSections, RunArray.Num() / 2,
							...
							{
								UE_LOGF(LogStaticMesh, Display, "lod:%1d/%1d   section:%1d/%1d   runs:%4d   inst:%8d   tris:%9lld   cast shadow:%1d   cull:%8.0f   shadow:%1d     %ls %ls",
									LODIndex, NumVertexFactories, SectionIndex, TotalNumSections, RunArray.Num() / 2,
				...
				else if (ensure(InstancedRenderData))
				{
					ensure(DoesPlatformUseInstancedStaticMeshVertexFactory(GetScene().GetShaderPlatform()));
					int32 NumBatches = 1;
					int32 CurrentRun = 0;
					...
					for (int32 BatchIndex = 0; BatchIndex < NumBatches; BatchIndex++)
					{
						...
						if (!FStaticMeshSceneProxy::GetMeshElement(LODIndex, 0, SectionIndex, GetDepthPriorityGroup(ElementParams.View), ElementParams.bBatchRenderSelection, true, MeshElement))
						{
							continue;
						}
						...
						BatchElement0.UserData = ElementParams.PassUserData;
						...
						MeshElement.bUseSelectionOutline = ElementParams.bBatchRenderSelection;
						MeshElement.bUseWireframeSelectionColoring = ElementParams.bBatchRenderSelection;
						...
						if (!bDidStats)
						{
							...
							if (GFrameNumberRenderThread_CaptureFoliageRuns == GFrameNumberRenderThread)
							{
								...
								{
									UE_LOGF(LogStaticMesh, Display, "lod:%1d/%1d   section:%1d/%1d   runs:%4d   inst:%8d   tris:%9lld   cast shadow:%1d   cull:-NONE!!-   shadow:%1d     %ls %ls",
										LODIndex, InstancedRenderData->VertexFactories.Num(), SectionIndex, TotalNumSections, RunArray.Num() / 2,
...
void FHierarchicalStaticMeshSceneProxy::GetDynamicMeshElements(const TArray<const FSceneView*>& Views, const FSceneViewFamily& ViewFamily, uint32 VisibilityMap, FMeshElementCollector& Collector) const
{
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		if (VisibilityMap & (1 << ViewIndex))
		{
			...
			ElementParams.PassUserData = &UserData_AllInstances;
			ElementParams.bBatchRenderSelection = ElementParams.bSelectionRenderEnabled && IsSelected();
			...
			ElementParams.FeatureLevel = GetScene().GetFeatureLevel();
			...
			if (ClusterTree.Num())
			{
				...
				else
				{
					...
					{
						FMatrix LocalViewProjForCulling = GetLocalToWorld() * View->ViewMatrices.GetWorldToClip();
				...
				{
					float Distance = ComputeBoundsDrawDistance(RenderData->ScreenSize[LODIndex].GetValue(), SphereRadius, View->ViewMatrices.GetViewToClip()) * LODScale;
...
void UHierarchicalInstancedStaticMeshComponent::ApplyComponentInstanceData(FInstancedStaticMeshComponentInstanceData* InstancedMeshData)
{
	...
	// Force rebuild, since anything may have changed in the ApplyComponentInstanceData.
	BuildTreeIfOutdated(/*Async*/false, /*ForceUpdate*/true);
}
...
void UHierarchicalInstancedStaticMeshComponent::PostBuildStats()
{
#if 0
	const TArray<FClusterNode>& ClusterTree = *ClusterTreePtr;
	FString MeshName = GetStaticMesh() ? GetStaticMesh()->GetPathName() : FString("null");
	...
	UE_LOGF(LogStaticMesh, Display, "Built a foliage hierarchy with %d instances, %d nodes, %f instances / leaf (desired %d) and %d verts in LOD0. Grass? %d    %ls", NumInst, ClusterTree.Num(), ActualInstancesPerLeaf(), DesiredInstancesPerLeaf(), GetVertsForLOD(0), bIsGrass), *MeshName);
#endif
}
...
void UHierarchicalInstancedStaticMeshComponent::ApplyBuildTreeAsync(ENamedThreads::Type CurrentThread, const FGraphEventRef& MyCompletionGraphEvent, TSharedRef<FClusterBuilder, ESPMode::ThreadSafe> Builder, double StartTime)
{
	...
	if (bConcurrentChanges)
	{
		...
		UE_LOGF(LogStaticMesh, Verbose, "Discarded foliage hierarchy of %d elements build due to concurrent removal (%.1fs)", Builder->Result->InstanceReorderTable.Num(), (float)(FPlatformTime::Seconds() - StartTime));
...
void UHierarchicalInstancedStaticMeshComponent::GetInstanceTransforms(TArray<FMatrix>& InstanceTransforms, FVector const& Offset) const
{
	...
	UE_LOGF(LogStaticMesh, Verbose, "Copied %d transforms in %.3fs.", Num, float(FPlatformTime::Seconds() - StartTime));
}
...
FPrimitiveSceneProxy* UHierarchicalInstancedStaticMeshComponent::CreateSceneProxy()
{
	...
	{
		UE_LOGF(LogStaticMesh, Verbose, "Skipping CreateSceneProxy for UHierarchicalInstancedStaticMeshComponent %ls (UHierarchicalInstancedStaticMeshComponent PSOs are still compiling)", *GetFullName());
		return nullptr;
	}
...
{
#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
	UE_LOGF(LogConsoleResponse, Display, "Rebuild Foliage Trees");
#endif
