...
bool FWorldCookPackageSplitter::RequiresEmptyPackageBeforePopulate(const UPackage* Package)
{
	TOptional<bool> Result;

	ForEachCookPackageSubSplitters(
		[Package, &Result](FWorldCookPackageSplitter::ISubSplitter* WorldCookPackageSplitter)
		{
			TOptional<bool> SubResult = WorldCookPackageSplitter->RequiresEmptyPackageBeforePopulate(Package);
			check(!SubResult.IsSet() || !Result.IsSet());
			Result = SubResult;
		});
	...
	return ICookPackageSplitter::RequiresEmptyPackageBeforePopulate(Package);
}
	...
	const UPackage* OwnerPackage, const UObject* OwnerObject)
{
	...
	ForEachCookPackageSubSplitters(
		[this, &Manifest, &SkipSaveGeneratorPackage, OwnerPackage, OwnerWorld](FWorldCookPackageSplitter::ISubSplitter* WorldCookPackageSplitter)
		{
			...
			// Any sub-splitter can disable save of the map
			TOptional<bool> SkipSave = WorldCookPackageSplitter->SkipSaveGeneratorPackage(OwnerPackage);
			if (SkipSave.IsSet())
			{
				if (SkipSaveGeneratorPackage.IsSet() && SkipSave.GetValue() != SkipSaveGeneratorPackage.GetValue())
				{
					UE_LOGF(LogWorldPartition, Error, "[Cook][ReportGenerationManifest] At least 2 sub-splitters disagree on whether to skip the generator package save, which also means 2 sub-splitters are attempting to handle the same package. It will stay enabled as a consequence.");
					SkipSaveGeneratorPackage = false; // Do not skip, as this will avoid missing packages and be easier to debug for unconcerned users.
				}
				else
				{
					SkipSaveGeneratorPackage = SkipSave.GetValue();
				}
			}
		});

	// If any sub-splitter requested that we skip the generator package save, and none requested that we keep it, disable the save. Otherwise the default is to save.
	Manifest.bCookSaveGeneratorPackage = !SkipSaveGeneratorPackage.Get(false);
