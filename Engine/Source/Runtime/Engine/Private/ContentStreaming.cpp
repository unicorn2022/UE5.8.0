			...
			if ( FCString::Stristr(AssetName, *TrackedAssetName) != NULL )
			{
				if ( bEnableLogging )
				{
						 ...
						 bIsDestroying )
					{
						...
						UE_LOGF(LogContentStreaming, Log, "%ls: \"%ls\", ResidentMips: %d/%d, RequestedMips: %d, WantedMips: %d, Boost: %.1f (%ls)",
...
void IStreamingManager::SetupViewInfos( float DeltaTime )
{
...
#if STREAMING_LOG_VIEWCHANGES
	{
		...
		for ( int32 ViewIndex = 0; ViewIndex < CurrentViewInfos.Num(); ++ViewIndex )
		{
			...
			if ( !bFound )
			{
				UE_LOGF(LogContentStreaming, Log, "Adding location: X=%.1f, Y=%.1f, Z=%.1f (override=%d, boost=%.1f)", ViewInfo.ViewOrigin.X, ViewInfo.ViewOrigin.Y, ViewInfo.ViewOrigin.Z, ViewInfo.bOverrideLocation, ViewInfo.BoostFactor );
			}
		}
		...
		for ( int32 PrevView=0; PrevView < GPrevViewLocations.Num(); ++PrevView )
		{
			...
			if ( !bFound )
			{
				...
				UE_LOGF(LogContentStreaming, Log, "Removing location: X=%.1f, Y=%.1f, Z=%.1f (override=%d, boost=%.1f)", PrevViewInfo.ViewOrigin.X, PrevViewInfo.ViewOrigin.Y, PrevViewInfo.ViewOrigin.Z, PrevViewInfo.bOverrideLocation, PrevViewInfo.BoostFactor );
			}
		}
...
FStreamingManagerCollection::~FStreamingManagerCollection()
{
	...
	UE_CLOGF(StreamingManagers.Num() > 0, LogContentStreaming, Display, "There are %d unreleased StreamingManagers", StreamingManagers.Num());
}
...
void FStreamingManagerCollection::AddLevel( ULevel* Level )
{
	...
	UE_LOGF(LogContentStreaming, Log, "FStreamingManagerCollection::AddLevel(\"%ls\")", *Level->GetOutermost()->GetName());
#endif
...
{
#if STREAMING_LOG_LEVELS
	UE_LOGF(LogContentStreaming, Log, "FStreamingManagerCollection::RemoveLevel(\"%ls\")", *Level->GetOutermost()->GetName());
#endif
...
void FStreamingManagerCollection::NotifyPrimitiveDetached( const IPrimitiveComponent* Primitive )
{
	// Route to streaming managers.
	for( int32 ManagerIndex=0; ManagerIndex<StreamingManagers.Num(); ManagerIndex++ )
	{
...
void FStreamingManagerCollection::NotifyPrimitiveUpdated( const IPrimitiveComponent* Primitive )
{
	// Route to streaming managers.
	for( int32 ManagerIndex=0; ManagerIndex<StreamingManagers.Num(); ManagerIndex++ )
	{
...
void FStreamingManagerCollection::NotifyPrimitiveUpdated_Concurrent( const IPrimitiveComponent* Primitive )
{
	// Route to streaming managers.
	for( int32 ManagerIndex=0; ManagerIndex<StreamingManagers.Num(); ManagerIndex++ )
	{
...
FAudioChunkHandle::FAudioChunkHandle(const uint8* InData, uint32 NumBytes, const TSharedRef<const FSoundWaveData>&  InSoundWave, const FName& SoundWaveName, uint32 InChunkIndex, uint64 InCacheLookupID)
	: CachedData(InData)
	, CachedDataNumBytes(NumBytes)
	, CorrespondingWaveName(SoundWaveName)
	, CorrespondingWaveGuid(InSoundWave->GetGUID())
	, ChunkIndex(InChunkIndex)
#if WITH_EDITOR
	, CorrespondingWave(InSoundWave)
	, ChunkRevision(InSoundWave->GetCurrentChunkRevision())
#endif
{
}
...
{
	TSharedPtr<const FSoundWaveData, ESPMode::ThreadSafe> SoundWaveDataPtr = CorrespondingWave.Pin();
...
bool IAudioStreamingManager::RequestChunk(const TSharedRef<const FSoundWaveData>& SoundWave, uint32 ChunkIndex,
	TFunction<void(EAudioChunkLoadResult)> OnLoadCompleted, ENamedThreads::Type ThreadToCallOnLoadCompletedOn, bool bForImmediatePlayback)
{
	static bool bDidWarn = false;
	if (!bDidWarn)
	{
		UE_LOG(LogContentStreaming, Warning, TEXT("This virtual function must be overridden, and will be pure virtual in a future release."));
		bDidWarn = true;
	}
	
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	return RequestChunk(FSoundWaveProxy::CreateNonUpdatingCopy(SoundWave), ChunkIndex, OnLoadCompleted, ThreadToCallOnLoadCompletedOn, bForImmediatePlayback);
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
}

FAudioChunkHandle IAudioStreamingManager::GetLoadedChunk(const TSharedRef<const FSoundWaveData>& SoundWave, uint32 ChunkIndex, bool bBlockForLoad, bool bForImmediatePlayback) const
{
	static bool bDidWarn = false;
	if (!bDidWarn)
	{
	...
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	return GetLoadedChunk(FSoundWaveProxy::CreateNonUpdatingCopy(SoundWave), ChunkIndex, bBlockForLoad, bForImmediatePlayback);
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
}

void IAudioStreamingManager::HandleStarvation(const TSharedPtr<FSoundWaveProxy>& SoundWave, const uint32 ChunkIndex, const uint32 InBytesProduced,
	const uint32 InBytesExpected)
{
	if (ensure(SoundWave.IsValid()))
	{
		HandleStarvation(SoundWave->GetSoundWaveDataRef(), ChunkIndex, InBytesProduced, InBytesExpected);
	}
}

FAudioChunkHandle IAudioStreamingManager::BuildChunkHandle(const uint8* InData, uint32 NumBytes, const TSharedRef<const FSoundWaveData>&  InSoundWaveData, const FName& SoundWaveName, uint32 InChunkIndex, uint64 InCacheLookupID)
{
	return FAudioChunkHandle(InData, NumBytes, InSoundWaveData, SoundWaveName, InChunkIndex, InCacheLookupID);
}

FAudioChunkHandle IAudioStreamingManager::BuildChunkHandle(const uint8* InData, uint32 NumBytes, const FSoundWaveProxyPtr& InSoundWave,
	const FName& SoundWaveName, uint32 InChunkIndex, uint64 CacheLookupID)
{
	if (ensure(InSoundWave.IsValid()))
	{
		return BuildChunkHandle(InData, NumBytes, InSoundWave->GetSoundWaveDataRef(), SoundWaveName, InChunkIndex, CacheLookupID);
	}
...
{
	UE_LOGF(LogContentStreaming, Warning, "This function is now deprecated on IAudioStreamingManager and no-ops.");
}

void IStreamingManager::NotifyPrimitiveDetached(const UPrimitiveComponent* Primitive)
{
	NotifyPrimitiveDetached(Primitive->GetPrimitiveComponentInterface());
}

void IStreamingManager::NotifyPrimitiveUpdated(const UPrimitiveComponent* Primitive)
{
	NotifyPrimitiveUpdated(Primitive->GetPrimitiveComponentInterface());
}

void IStreamingManager::NotifyPrimitiveUpdated_Concurrent(const UPrimitiveComponent* Primitive)
{
	NotifyPrimitiveUpdated_Concurrent(Primitive->GetPrimitiveComponentInterface());
}
