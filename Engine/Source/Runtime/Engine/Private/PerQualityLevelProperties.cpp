...
namespace QualityLevelProperty
{
	...
	static FString QualityLevelMappingStr = "QualityLevelMapping";
	...
	FSupportedQualityLevelArray PerPlatformOverrideMapping(FString& InPlatformName, UObject* RequestingAsset)
	{
		...
		for (const FName& EnginePlatformName : EnginePlatforms)
		{
			...
			if (EngineSettings.GetString(TEXT("SystemSettings"), *QualityLevelMappingStr, MappingStr))
			{
				...
				if (Value == INDEX_NONE)
				{
					UE_LOGF(LogCore, Warning, "Bad QualityLevelMapping input value in %lsEngine.ini. "
						"Needs to be one of [low,medium,high,epic,cinematic].",
			...
			{
				UE_LOGF(LogCore, Warning, "Didn't find QualityLevelMapping in %lsEngine.ini, for platform %ls that was requested by %ls. "
					"Need to define QualityLevelMapping under the [SystemSettings] section. All perplatform MinLOD will not be converted to PerQuality.",
