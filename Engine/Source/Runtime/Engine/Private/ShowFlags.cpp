...
int32 FEngineShowFlags::FindIndexByName(const TCHAR* Name, const TCHAR* CommaSeparatedNames)
{
	...
	if (CommaSeparatedNames == nullptr)
	{
		...
		#define SHOWFLAG_ALWAYS_ACCESSIBLE(a,...) if(Search == UE_STRINGIZE(a)) { return (int32)SF_##a; }
...
FString FEngineShowFlags::FindNameByIndex(uint32 InIndex)
{
	...
	#define SHOWFLAG_ALWAYS_ACCESSIBLE(a,...) case SF_##a: Name = UE_STRINGIZE(a); break;
...
{
	#define SHOWFLAG_ALWAYS_ACCESSIBLE(a,...) case SF_##a: Out += UE_STRINGIZE(a); break;
	switch (InIndex)
	{
		#include "ShowFlagsValues.inl" // IWYU pragma: keep
		default:
...
void ApplyViewMode(EViewModeIndex ViewModeIndex, bool bPerspective, FEngineShowFlags& EngineShowFlags)
{
	...
	switch(ViewModeIndex)
	{
		...
		case VMI_StreamingTextureDeficit:
		case VMI_StreamingTextureResidency:
		case VMI_StreamingMeshLODDeficit:
		case VMI_StreamingMeshLODResidency:
			bPostProcessing = false;
			break;
	...
	EngineShowFlags.SetVisualizeMegaLights(ViewModeIndex == VMI_VisualizeMegaLights);
	...
	EngineShowFlags.SetStreamingTextureDeficit(ViewModeIndex == VMI_StreamingTextureDeficit);
	EngineShowFlags.SetStreamingTextureResidency(ViewModeIndex == VMI_StreamingTextureResidency);
	EngineShowFlags.SetStreamingMeshLODDeficit(ViewModeIndex == VMI_StreamingMeshLODDeficit);
	EngineShowFlags.SetStreamingMeshLODResidency(ViewModeIndex == VMI_StreamingMeshLODResidency);
...
void EngineShowFlagOverride(EShowFlagInitMode ShowFlagInitMode, EViewModeIndex ViewModeIndex, FEngineShowFlags& EngineShowFlags, bool bCanDisableTonemapper)
{
	...
	// Some view modes want some features off or on (no state)
	{
		...
		if( ViewModeIndex == VMI_Unlit ||
			...
			ViewModeIndex == VMI_StreamingTextureDeficit ||
			ViewModeIndex == VMI_StreamingTextureResidency ||
			ViewModeIndex == VMI_StreamingMeshLODDeficit ||
			ViewModeIndex == VMI_StreamingMeshLODResidency)
		{
		...
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		if (ViewModeIndex == VMI_Lit_Wireframe)
		{
			EngineShowFlags.SetMeshEdges(true);
		}
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
	}
	...
	if (EngineShowFlags.VisualizeColorGrading)
	{
		EngineShowFlags.SetCompositeEditorPrimitives(false);
		EngineShowFlags.SetCompositeDebugPrimitives(false);
		EngineShowFlags.SetAudioRadius(false);
		EngineShowFlags.SetEditor(false);
...
EViewModeIndex FindViewMode(const FEngineShowFlags& EngineShowFlags)
{
	...
	else if (EngineShowFlags.VisualizeMegaLights)
	{
		return VMI_VisualizeMegaLights;
	}
	...
	{
		return VMI_StreamingTextureDeficit;
	}
	else if (EngineShowFlags.StreamingTextureResidency)
	{
		return VMI_StreamingTextureResidency;
	}
	else if (EngineShowFlags.StreamingMeshLODDeficit)
	{
		return VMI_StreamingMeshLODDeficit;
	}
	else if (EngineShowFlags.StreamingMeshLODResidency)
	{
		return VMI_StreamingMeshLODResidency;
	}
...
const TCHAR* GetViewModeName(EViewModeIndex ViewModeIndex)
{
	switch (ViewModeIndex)
	{
		...
		case VMI_VisualizeMegaLights:		return TEXT("VisualizeMegaLights");
		...
		case VMI_StreamingTextureDeficit:	return TEXT("StreamingTextureDeficit");
		case VMI_StreamingTextureResidency:	return TEXT("StreamingTextureResidency");
		case VMI_StreamingMeshLODDeficit:	return TEXT("StreamingMeshLODDeficit");
		case VMI_StreamingMeshLODResidency:	return TEXT("StreamingMeshLODResidency");
	}
	return TEXT("");
}
