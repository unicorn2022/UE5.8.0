...
public:
	...
	virtual FIntPoint GetSizeXY() const
	{
		...
		{
			return ViewRect_RenderThread.Size();
		}
		else
		{
			return ViewRect_GameThread.Size();
		}
	}
	...
	void SetViewRect_GameThread(const FIntRect& InViewRect)
	{
		ViewRect_GameThread = InViewRect;
	}

	void SetViewRect_RenderThread(const FIntRect& InViewRect)
	{
		ViewRect_RenderThread = InViewRect;
	}

	/** Gets the viewport rect for the render target */
	const FIntRect& GetViewRect_GameThread() const 
	{
		return ViewRect_GameThread;
	}

	const FIntRect& GetViewRect_RenderThread() const 
	{
		return ViewRect_RenderThread;
	}

private:
	FRDGTexture* RDGTexture = nullptr;
	FIntRect ViewRect_GameThread;
	FIntRect ViewRect_RenderThread;
};
...
void FDebugCanvasDrawer::BeginRenderingCanvas( const FIntRect& CanvasRect )
{
	if( CanvasRect.Size().X > 0 && CanvasRect.Size().Y > 0 )
	{
		...
		// Caching the ViewRect for the game thread before passing the canvas to the render thread
		if (CanvasToRender.IsValid() && CanvasToRender->IsUsingInternalTexture())
		{
			RenderTarget->SetViewRect_GameThread(FIntRect(FIntPoint(0, 0), CanvasToRender->GetParentCanvasSize()));
		}
		else
		{
			RenderTarget->SetViewRect_GameThread(CanvasRect);
		}
...
void FDebugCanvasDrawer::Draw_RenderThread(FRDGBuilder& GraphBuilder, const FDrawPassInputs& Inputs)
{
	...
	if (RenderThreadCanvas.IsValid())
	{
		...
		if (RenderThreadCanvas->IsUsingInternalTexture())
		{
			...
			{
				UE_LOGF(LogProfilingDebugging, Warning, "No layer resource or HMD swapchain available for stereo debug layer");
			}
		}
		...
		{
			RenderThreadCanvas->SetRenderTargetRect(RenderTarget->GetViewRect_RenderThread());
		}
...
void FDebugCanvasDrawer::SetRenderThreadCanvas( const FIntRect& InCanvasRect, FCanvasPtr& Canvas, FTextureRenderTargetResource* InLayerResource )
{
	...
	{
		RenderTarget->SetViewRect_RenderThread(FIntRect(FIntPoint(0, 0), Canvas->GetParentCanvasSize()));
	}
	else
	{
		RenderTarget->SetViewRect_RenderThread(InCanvasRect);
	}
...
bool SDebugCanvas::CustomPrepass(float LayoutScaleMultiplier)
{
