...
int32 SGameLayerManager::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	...
	int32 ResultLayer = SCompoundWidget::OnPaint(Args, AllottedGeometry, MyCullingRect, OutDrawElements, LayerId, InWidgetStyle, bParentEnabled);

#if WITH_SLATE_NAVIGATION_DEBUGGING
	ResultLayer = FSlateNavigationDebugger::Get().Paint(OutDrawElements, ResultLayer);
#endif
...
{
	UE_LOGF(LogSlate, Log, "Updating window title bar state: %ls mode, drag %ls, window buttons %ls, title bar %ls",
...
TSharedRef<SWidget> SGameLayerManager::AsWidget()
{
	return AsShared();
}

bool SGameLayerManager::CustomPrepass(float LayoutScaleMultiplier)
{
	SetIsProjectContentParent(true);
	return true;
}
