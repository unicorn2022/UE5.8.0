...
void FMeshBudgetProjectSettingsUtils::SetLodGroupForStaticMesh(UStaticMesh* StaticMesh)
{
	...
	for (const FStaticMeshBudgetInfo& MeshBudgetInfo : MeshBudgetProjectSettings->StaticMeshBudgetInfos)
	{
		if (!LodGroupNames.Contains(MeshBudgetInfo.LodGroupName))
		{
			...
			{
				UE_LOGF(LogMeshBudget, Warning, "MeshBudgetProjectSettings MeshBudgetInfo [%ls] LOD group name do not exist add it to the ini file. Skipping this budget... ", *MeshBudgetInfo.LodGroupName.ToString());
			}
	...
	{
		UE_LOGF(LogMeshBudget, Error, "MeshBudgetProjectSettings Cannot assign any lod group to static mesh, the budget data is not configure properly in the project settings. ");
	}
