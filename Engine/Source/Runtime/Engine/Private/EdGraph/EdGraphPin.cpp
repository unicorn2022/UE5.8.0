...
void UEdGraphPin::AssignByRefPassThroughConnection(UEdGraphPin* InTargetPin)
{
	if (InTargetPin)
	{
		...
		{
			UE_LOGF(LogBlueprint, Warning, "Pin '%ls' is owned by node '%ls' and pin '%ls' is owned by '%ls', they must be owned by the same node!", *GetName(), *GetOwningNode()->GetName(), *InTargetPin->GetName(), *InTargetPin->GetOwningNode()->GetName());
		}
		else if (Direction == InTargetPin->Direction)
		{
			UE_LOGF(LogBlueprint, Warning, "Both pin '%ls' and pin '%ls' on node '%ls' go in the same direction, one must be an input and the other an output!", *GetName(), *InTargetPin->GetName(), *GetOwningNode()->GetName());
		}
		else if (!PinType.bIsReference && !InTargetPin->PinType.bIsReference)
		{
			...
			UE_LOGF(LogBlueprint, Warning, "Input pin '%ls' is attempting to by-ref pass-through to output pin '%ls' on node '%ls', however neither pin is by-ref!", *InputPin->GetName(), *OutputPin->GetName(), *InputPin->GetOwningNode()->GetName());
		}
		else if (!PinType.bIsReference)
		{
			UE_LOGF(LogBlueprint, Warning, "Pin '%ls' on node '%ls' is not by-ref but it is attempting to pass-through to '%ls'", *GetName(), *GetOwningNode()->GetName(), *InTargetPin->GetName());
		}
		else if (!InTargetPin->PinType.bIsReference)
		{
			UE_LOGF(LogBlueprint, Warning, "Pin '%ls' on node '%ls' is not by-ref but it is attempting to pass-through to '%ls'", *InTargetPin->GetName(), *InTargetPin->GetOwningNode()->GetName(), *GetName());
		}
		else
		{
...
{
#if WITH_EDITOR
	UEdGraphNode* OwnerNode = GetOwningNodeUnchecked();
	return OwnerNode ? OwnerNode->GetSchema() : nullptr;
#else
	return nullptr;
#endif	//WITH_EDITOR
}
...
{
#ifdef TRACK_PINS
	PinAllocationTracking.Emplace(this, InOwningNode ? InOwningNode->GetName() : FString("UNOWNED"));
#endif //TRACK_PINS
}
				...
				else
				{
					...
					if(Ar.IsTransacting() && !GEditor->Trans->IsObjectTransacting(LocalOwningNode))
					{
						...
						*/
						...
						{
							// case 3:
							UE_LOGF(LogBlueprint, Warning, "Pin link in transaction buffer is corrupt - this occurs because node mutations are not recorded in the transaction buffer. Node links will be lost");
							PinRef = nullptr;
							bRetVal = false;
...
{
	static FName GameplayTagName = "GameplayTag";
	static FName GameplayTagContainerName = "GameplayTagContainer";
	static FName GameplayTagsPathName = "/Script/GameplayTags";
