...
int32 MeshCardRepresentation::GetAxisAlignedDirectionIndex(const FVector3f& Direction)
{
	const FVector3f DirectionAbs = Direction.GetAbs();

	if (DirectionAbs == FVector3f::XAxisVector)
	{
		return Direction.X > 0.0f ? 1 : 0;
	}
	else if (DirectionAbs == FVector3f::YAxisVector)
	{
		return Direction.Y > 0.0f ? 3 : 2;
	}
	else if (DirectionAbs == FVector3f::ZAxisVector)
	{
		return Direction.Z > 0.0f ? 5 : 4;
	}
	else
	{
		return INDEX_NONE;
...
#define CARDREPRESENTATION_DERIVEDDATA_VER TEXT("BE3395EF-F023-4F1D-9827-1DD3C88E16DC")
...
FAsyncCardRepresentationTask* BeginCacheMeshCardRepresentationInternal(
	...
	FSourceMeshDataForDerivedDataTask&& OptionalSourceMeshData,
	TArray<FSignedDistanceFieldBuildPartData>&& OptionalPartData,
	FDistanceFieldVolumeData* DistanceFieldData = nullptr)
{
	...
	if (CVarCards->GetValueOnAnyThread() != 0)
	{
		if (RenderData.LODResources.IsValidIndex(0))
		{
			...
			{
				const FStaticMeshSourceModel& SourceModel = StaticMeshAsset->GetSourceModel(0);
				...
				// Version the key against changes in the source model contents.
				if (SourceModel.GetMeshDescriptionBulkData())
				{
					Key += SourceModel.GetMeshDescriptionBulkData()->GetUnversionedIdString();
				}
	...
	FCardRepresentationData& OutRenderData)
{
	...
	if (!MeshCardRepresentation::IsDebugMode() && GetDerivedDataCacheRef().GetSynchronous(*InDDCKey, DerivedData, Mesh->GetPathName()))
	{
		...
		if (Ar.IsError())
		{
			UE_LOGF(LogStaticMesh, Error, "Error while deserializing Mesh Card derived data for %ls from DDC (key %ls)", *Mesh->GetPathName(), *InDDCKey);
		}
	}
	else
	{
		...
		NewTask->DistanceFieldData = DistanceFieldData;
		...
		else if (Mesh->IsNaniteAssembly())
		{
			...
			for (int32 PartIndex = 0; PartIndex < NumParts; ++PartIndex)
			{
				...
				{
					UE_LOGF(LogStaticMesh, Warning, "Nanite Assembly doesn't have a valid part reference. See previous line(s) for details.");
				}
			}
...
void FCardRepresentationData::CacheDerivedData(
	...
	TArray<FSignedDistanceFieldBuildPartData>&& OptionalPartData)
{
	FAsyncCardRepresentationTask* NewTask = CreateCardRepresentationTask(InDDCKey, Mesh, GenerateSource, nullptr, MaxLumenMeshCards, bGenerateDistanceFieldAsIfTwoSided, MoveTemp(OptionalSourceMeshData), MoveTemp(OptionalPartData), *this);
...
void FMeshCardsBuildData::FitToNewBounds(const FBox& NewBounds)
{
	const FVector OldOrigin = Bounds.GetCenter();
	const FVector OldExtent = Bounds.GetExtent();
	const FVector NewOrigin = NewBounds.GetCenter();
	// Make sure the new bounds isn't empty. Matches the behavior in BuildMeshCards()
	const FVector NewExtent = FVector::Max(NewBounds.GetExtent() + 1.0, FVector::OneVector);

	const FVector3f OldToNewScale(NewExtent / OldExtent);

	for (FLumenCardBuildData& Card : CardBuildData)
	{
		const FVector3f CardSpaceScale = Card.OBB.RotateLocalToCard(OldToNewScale).GetAbs();
		
		Card.OBB.Extent *= CardSpaceScale;
		Card.OBB.Origin = (Card.OBB.Origin - FVector3f(OldOrigin)) * OldToNewScale + FVector3f(NewOrigin);
	}

	Bounds = FBox(NewOrigin - NewExtent, NewOrigin + NewExtent);
}
...
{
	GCardRepresentationAsyncQueue->Build(&Task);
}
...
void FCardRepresentationAsyncQueue::ProcessPendingTasksForMesh(const UStaticMesh* InStaticMesh)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FCardRepresentationAsyncQueue::ProcessPendingTasksForMesh);
	...
	{
		FAsyncCardRepresentationTask* Task = *It;
		if ((Task->StaticMesh == InStaticMesh || Task->GenerateSource == InStaticMesh) &&
			(Task->GenerateSource == nullptr || Task->GenerateSource->IsCompiling() == false))
		{
			StartBackgroundTask(Task);
			It.RemoveCurrent();
		}
...
#else
	UE_LOGF(LogStaticMesh,Fatal,"Tried to build a card representation without editor support (this should have been done during cooking)");
#endif
}
...
int32 FCardRepresentationAsyncQueue::GetNumOutstandingTasks(const UStaticMesh* InStaticMesh) const
{
	FScopeLock Lock(&CriticalSection);
	if (!ReferencedTasks.Num())
	{
		return 0;
	}

	int32 Count = 0;
	for (auto It = ReferencedTasks.CreateConstIterator(); It; ++It)
	{
		const FAsyncCardRepresentationTask* Task = *It;
		if (InStaticMesh == Task->GenerateSource || InStaticMesh == Task->StaticMesh)
		{
			++Count;
		}
	}
...
void FCardRepresentationAsyncQueue::BlockUntilBuildComplete(UStaticMesh* InStaticMesh, bool bWarnIfBlocked)
{
	...
	do 
	{
		ProcessAsyncTasksForMesh(InStaticMesh);
	...
	{
		UE_LOGF(LogStaticMesh, Display, "Main thread blocked for %.3fs for async card representation build of %ls to complete!  This can happen if the mesh is rebuilt excessively.",
...
void FCardRepresentationAsyncQueue::Build(FAsyncCardRepresentationTask* Task)
{
#if WITH_EDITOR
	COOK_STAT(auto Timer = CardRepresentationCookStats::UsageStats.TimeSyncWork());
	COOK_STAT(Timer.TrackCyclesOnly());
	...
	if (Task->StaticMesh && Task->GenerateSource)
	{
		...
		Task->bSuccess = MeshUtilities->GenerateCardRepresentationData(
			...
			MeshData,
			Task->DistanceFieldData ? Task->DistanceFieldData : Task->GenerateSource->GetRenderData()->LODResources[0].DistanceFieldData,
...
void FCardRepresentationAsyncQueue::ProcessAsyncTasks(bool bLimitExecutionTime)
{
	...
	FObjectCacheContextScope ObjectCacheScope; // Declared here for caching optimization purposes
	...
	while (!bLimitExecutionTime || (FPlatformTime::Seconds() - StartTime) < MaxProcessingTime)
	{
		...
		FinalizeAndDeleteTask(Task, ObjectCacheScope);
	}
...
void FCardRepresentationAsyncQueue::ProcessAsyncTasksForMesh(const UStaticMesh* InStaticMesh, bool bLimitExecutionTime)
{
#if WITH_EDITOR
	TRACE_CPUPROFILER_EVENT_SCOPE(FCardRepresentationAsyncQueue::ProcessAsyncTasksForMesh);

	ProcessPendingTasksForMesh(InStaticMesh);

	FObjectCacheContextScope ObjectCacheScope; // Declared here for mem/caching optimization purposes
	const double MaxProcessingTime = 0.016f;
	double StartTime = FPlatformTime::Seconds();
	bool bMadeProgress = false;
	while (!bLimitExecutionTime || (FPlatformTime::Seconds() - StartTime) < MaxProcessingTime)
	{
		FAsyncCardRepresentationTask* Task = nullptr;
		{
			// Try and find a task that matches the static mesh
			FScopeLock Lock(&CriticalSection);
			for (auto CompletedTaskIt = CompletedTasks.CreateIterator(); CompletedTaskIt; ++CompletedTaskIt)
			{
				FAsyncCardRepresentationTask* TaskToCheck = *CompletedTaskIt;

				if (TaskToCheck != nullptr && (TaskToCheck->StaticMesh == InStaticMesh || TaskToCheck->GenerateSource == InStaticMesh))
				{
					Task = TaskToCheck;
					CompletedTaskIt.RemoveCurrent();
					verify(ReferencedTasks.Remove(Task));
					break;
				}
			}
...
#if WITH_EDITOR
void FCardRepresentationAsyncQueue::FinalizeAndDeleteTask(FAsyncCardRepresentationTask* InTask, FObjectCacheContextScope& InObjectCacheScope)
{
	check(InTask);
	check(IsInGameThread()); // Timer wont work properly outside of gamethread.

	if (!InTask->bSuccess)
	{
		CancelAndDeleteTask(TSet<FAsyncCardRepresentationTask*>({ InTask }));
		return;
	}
	...
	if (InTask->AsyncTask)
	{
		InTask->AsyncTask->EnsureCompletion();
		InTask->AsyncTask.Reset();
	}

	// Editor 'force delete' can null any UObject pointers which are seen by reference collecting (eg UProperty or serialized)
	if (InTask->StaticMesh && InTask->bSuccess)
	{
		check(!InTask->StaticMesh->IsCompiling());
		
		FStaticMeshRenderData* RenderData = InTask->StaticMesh->GetRenderData();
		...
		RenderData->LODResources[0].CardRepresentationData = InTask->GeneratedCardRepresentation;

		// Any already created render state needs to be dirtied
		if (RenderData->IsInitialized())
		{
			for (IStaticMeshComponent* Component : InObjectCacheScope.GetContext().GetStaticMeshComponents(InTask->StaticMesh))
			{
		...
		while (PlatformRenderData)
		{
			...
			{
				*PlatformRenderData->LODResources[0].CardRepresentationData = *InTask->GeneratedCardRepresentation;
			}
		...
		if (!MeshCardRepresentation::IsDebugMode())
		{
			...
			Ar << *(InTask->StaticMesh->GetRenderData()->LODResources[0].CardRepresentationData);
			GetDerivedDataCacheRef().Put(*InTask->DDCKey, DerivedData, InTask->StaticMesh->GetPathName());
...
void FCardRepresentationAsyncQueue::Shutdown()
{
	...
	UE_LOGF(LogStaticMesh, Log, "Abandoning remaining async card representation tasks for shutdown");
	ThreadPool.Reset();
}
