...
TAutoConsoleVariable<int32> CVarClothPhysicsLODThreshold(TEXT("p.ClothPhysics.LODThreshold"), -1, TEXT("Max LOD that cloth simulation is allowed to run on. Use -1 to disable."), ECVF_Scalability);
...
static TAutoConsoleVariable<int32> CVarClothPhysicsUseEndTickWait(TEXT("p.ClothPhysics.UseEndTickWait"), 1, TEXT("If 1 (default), wait on the parallel cloth/deformable task in a dedicated TG_LastDemotable tick function. If 0, fall back to waiting in OnPreEndOfFrameSync (kill switch). Has no effect when p.ClothPhysics.WaitForParallelClothTask is 1."));
...
static TAutoConsoleVariable<bool> CVarClothPhysicsOnUpdateTransformTeleport(TEXT("p.ClothPhysics.OnUpdateTransformTeleport"), true, TEXT("Allow transform changes to trigger cloth teleports."));
...
static bool GEnableSkeletalMeshComponentAsyncInitBody = false;
static FAutoConsoleVariableRef CVarEnableSkeletalMeshComponentAsyncInitBody(TEXT("p.SkeletalMeshComponent.EnableAsyncInitBody"), GEnableSkeletalMeshComponentAsyncInitBody, TEXT("Enables SkeletalMeshComponent's body instances to be initialized outside of game thread (default is false)."), ECVF_ReadOnly);
...
{
	return FName("SkeletalMeshComponentClothTick");
}

void FSkeletalMeshComponentClothEndTickFunction::ExecuteTick(float DeltaTime, enum ELevelTick TickType, ENamedThreads::Type CurrentThread, const FGraphEventRef& MyCompletionGraphEvent)
{
	QUICK_SCOPE_CYCLE_COUNTER(FSkeletalMeshComponentClothEndTickFunction_ExecuteTick);
	if (Target)
	{
		Target->HandleExistingParallelClothSimulation();
	}
}

FString FSkeletalMeshComponentClothEndTickFunction::DiagnosticMessage()
{
	if (Target)
	{
		return Target->GetFullName() + TEXT("[ClothEndTick]");
	}
	return TEXT("<NULL>[ClothEndTick]");
}

FName FSkeletalMeshComponentClothEndTickFunction::DiagnosticContext(bool bDetailed)
{
	return FName("SkeletalMeshComponentClothEndTick");
}
...
{
	return FName("SkeletalMeshComponentEndPhysicsTick");
}
...
void USkeletalMeshComponent::CreateBodySetup()
{
	...
	if (GetSkeletalMeshAsset())
	{
		...
		BodySetup->CookedFormatProvider = SkeletalMeshConst;
		BodySetup->bHasCookedFormatProvider = true;
	}
...
void USkeletalMeshComponent::InitArticulated(FPhysScene* PhysScene)
{
	...
	if (Bodies.Num() > 0)
	{
		UE_LOGF(LogSkeletalMesh, Log, "USkeletalMeshComponent::InitArticulated : Bodies already created (%ls) - call TermArticulated first.", *GetPathName());
		return;
	}
	...
	{
		UE_LOGF(LogSkeletalMesh, Verbose, "Skipping PhysicsAsset creation on dedicated server (%ls : %ls) %ls", *GetNameSafe(GetOuter()), *GetName(), *PhysicsAsset->GetName());
		return;
	}
	...
	{
		UE_LOGF(LogSkeletalMesh, Log, "USkeletalMeshComponent::InitArticulated : Could not find root physics body: '%ls'", *GetPathName());
		return;
	}
	...
	{
		UE_LOGF(LogSkeletalMesh, Log, "USkeletalMeshComponent::InitArticulated : Too many shapes to create aggregate, Max: %u, This: %d", AggregateMaxSize, NumShapes);
	}
...
void USkeletalMeshComponent::InstantiatePhysicsAsset(const UPhysicsAsset& PhysAsset, const FVector& Scale3D, TArray<FBodyInstance*>& OutBodies, TArray<FConstraintInstance*>& OutConstraints, FPhysScene* PhysScene, USkeletalMeshComponent* OwningComponent, int32 UseRootBodyIndex, const FPhysicsAggregateHandle& UseAggregate) const
{
	...
	{
		const bool bExpectedBoneTransformsValidity = IsPreRegistering() || IsPreRegistered() || IsRegistered();
		checkf(bExpectedBoneTransformsValidity == AreBoneTransformsValid(), TEXT("Component bone transforms are inconsistent with the component's registration state."));
	...
	for(int32 BodyIdx = 0; BodyIdx < NumOutBodies; BodyIdx++)
	{
		...
		if(BoneIndex != INDEX_NONE)
		{
			...
			else
			{
				...
				if(PhysScene != nullptr && !CVarEnableRagdollPhysics.GetValueOnAnyThread())	//We only limit creation of the global physx scenes and not assets related to immedate mode
				{
					continue;
				}
			}
	...
	for(int32 ConstraintIdx = 0; ConstraintIdx < NumOutConstraints; ConstraintIdx++)
	{
		...
		if(Body1 && Body2)
		{
			...
			auto ValidateBody = [this](const FBodyInstance* InBody, const FName& InBoneName)
			{
				...
				{
					// Disable log for now.
					// UE_LOGF(LogSkeletalMesh, Warning, "USkeletalMeshComponent::InitArticulated : Unable to initialize constraint (%ls) -  Body Invalid %ls.", *(this->GetPathName()), *(InBoneName.ToString()));
					return false;
				}
...
void USkeletalMeshComponent::TermArticulated()
{
	...
	const bool bIsAsyncDestroyPhysicsStateRunning = IsAsyncDestroyPhysicsStateRunning();
	const bool bPrepareAsyncPayload = bIsAsyncDestroyPhysicsStateRunning && bIsGameThread;
	const bool bUseAsyncPayload = bIsAsyncDestroyPhysicsStateRunning && !bIsGameThread;
	check(!bPrepareAsyncPayload || !bUseAsyncPayload);
	...
	auto TermArticulatedInternal = [this, bPrepareAsyncPayload, bUseAsyncPayload]()
	{
		TArray<FBodyInstance*>& InOutBodies = bUseAsyncPayload ? AsyncDestroyPhysicsStatePayload : Bodies;
		TArray<FConstraintInstance*>& InOutConstraints = bUseAsyncPayload ? AsyncDestroyConstraintsPayload : Constraints;
		FPhysicsAggregateHandle& InOutAggregate = bUseAsyncPayload ? AggregatePayload : Aggregate;

		// We shut down the physics for each body and constraint here. The actual UObjects will get GC'd.
		for (int32 i = 0; i < InOutBodies.Num(); i++)
		{
			if (ensure(InOutBodies[i]))
			{
				// There seems to be scenarios where there is a body that still has an active weld parent.
				if (!ensure(InOutBodies[i]->WeldParent == nullptr))
				{
					InOutBodies[i]->WeldParent->UnWeld(InOutBodies[i]);
				}

				// Asynchronous destruction is done in OnAsyncDestroyPhysicsState
				if (!bPrepareAsyncPayload)
				{
					InOutBodies[i]->TermBody();
					delete InOutBodies[i];
				}
			}
		}
		...
		if (!bPrepareAsyncPayload)
		{
			for (int32 i = 0; i < InOutConstraints.Num(); i++)
			{
				check(InOutConstraints[i]);
				InOutConstraints[i]->TermConstraint();
				delete InOutConstraints[i];
			}

			InOutConstraints.Empty();
			InOutBodies.Empty();

			// releasing Aggregate, it shouldn't contain any Bodies now, because they are released above
			if (InOutAggregate.IsValid())
			{
				check(FPhysicsInterface::GetNumActorsInAggregate(InOutAggregate) == 0);
				FPhysicsInterface::ReleaseAggregate(InOutAggregate);
			}
		}
		else
		{
			// Prepare payload for asynchronous destruction
			check(AsyncDestroyConstraintsPayload.IsEmpty());
			AsyncDestroyConstraintsPayload = MoveTemp(Constraints);

			check(AsyncDestroyPhysicsStatePayload.IsEmpty());
			AsyncDestroyPhysicsStatePayload = MoveTemp(Bodies);

			check(!AggregatePayload.IsValid());
			AggregatePayload = MoveTemp(Aggregate);
		}
	};

	if (bPrepareAsyncPayload)
	{
		// No need to lock scene when preparing asynchronous destruction payloads
		TermArticulatedInternal();
	}
	else
	{
		FPhysicsCommand::ExecuteWrite(this, [&TermArticulatedInternal]()
		{
			TermArticulatedInternal();
		});
	}
...
bool USkeletalMeshComponent::AllowsAsyncPhysicsStateCreation() const
{
	check(FPhysScene::SupportsAsyncPhysicsStateCreation());

	// Note: AutoWeld is disabled by default for PhysicsAsset's BodyInstances
	return GEnableSkeletalMeshComponentAsyncInitBody && (!BodyInstance.bAutoWeld || !Cast<UPrimitiveComponent>(GetAttachParent()));
}

bool USkeletalMeshComponent::AllowsAsyncPhysicsStateDestruction() const
{
	check(FPhysScene::SupportsAsyncPhysicsStateDestruction());
	return AllowsAsyncPhysicsStateCreation();
}

void USkeletalMeshComponent::OnAsyncCreatePhysicsStateBegin_GameThread(TSet<UObject*>& OutRootedObjects)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(USkeletalMeshComponent::OnAsyncCreatePhysicsStateBegin_GameThread);

	if (bEnablePerPolyCollision == false)
	{
		USceneComponent::OnAsyncCreatePhysicsStateBegin_GameThread(OutRootedObjects);
	}
	else
	{
		CreateBodySetup();
...
bool USkeletalMeshComponent::OnAsyncCreatePhysicsState(const UE::FTimeout& TimeOut)
{
	// Don't call Super::OnAsyncCreatePhysicsState as we don't want UActorComponent::OnAsyncCreatePhysicsState
	// to call OnCreatePhysicsState (which would call USkeletalMeshComponent::OnCreatePhysicsState).
	TRACE_CPUPROFILER_EVENT_SCOPE(USkeletalMeshComponent::OnAsyncCreatePhysicsState);
	check(AllowsAsyncPhysicsStateCreation());

	check(AreBoneTransformsValid());

	if (bEnablePerPolyCollision == false)
	{
		InitArticulated(GetWorld()->GetPhysicsScene());

		// Don't call Super (skip PrimitiveComponent)
		USceneComponent::OnCreatePhysicsState();
	}
	else
	{
		// If we're doing per poly we'll use the body instance of the primitive component
...
void USkeletalMeshComponent::OnAsyncCreatePhysicsStateEnd_GameThread()
{
	TRACE_CPUPROFILER_EVENT_SCOPE(USkeletalMeshComponent::OnAsyncCreatePhysicsStateEnd_GameThread);

	if (bEnablePerPolyCollision == false)
	{
		// Don't call Super (skip PrimitiveComponent)
		USceneComponent::OnAsyncCreatePhysicsStateEnd_GameThread();
...
void USkeletalMeshComponent::OnAsyncDestroyPhysicsStateBegin_GameThread(TSet<UObject*>& OutRootedObjects)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(USkeletalMeshComponent::OnAsyncDestroyPhysicsStateBegin_GameThread);
	...
	DestroyPhysicsStateInternal();

	Super::OnAsyncDestroyPhysicsStateBegin_GameThread(OutRootedObjects);
}

bool USkeletalMeshComponent::OnAsyncDestroyPhysicsState(const UE::FTimeout& Timeout)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(USkeletalMeshComponent::OnAsyncDestroyPhysicsState);

	if (bEnablePerPolyCollision == false)
	{
		check(Bodies.IsEmpty());
		check(Constraints.IsEmpty());
		check(!Aggregate.IsValid());
...
void USkeletalMeshComponent::TermBodiesBelow(FName ParentBoneName)
{
	...
	if(PhysicsAsset && GetSkeletalMeshAsset() && Bodies.Num() > 0)
	{
		...
		{
			UE_LOGF(LogSkeletalMesh, Log, "TermBodiesBelow: ParentBoneName '%ls' is invalid", *ParentBoneName.ToString());
			return;
		}
...
void USkeletalMeshComponent::SetAllMotorsAngularPositionDrive(bool bEnableSwingDrive, bool bEnableTwistDrive, bool bSkipCustomPhysicsType)
{
	...
	for(int32 i=0; i<Constraints.Num(); i++)
	{
		...
		Constraints[i]->SetOrientationDriveSLERP(bEnableTwistDrive || bEnableSwingDrive);
	}
}
...
void USkeletalMeshComponent::SetNamedMotorsAngularVelocityDrive(bool bEnableSwingDrive, bool bEnableTwistDrive, const TArray<FName>& BoneNames, bool bSetOtherBodiesToComplement)
{
	...
	for(int32 i=0; i<Constraints.Num(); i++)
	{
		...
		if( BoneNames.Contains(Instance->GetChildBoneName()) )
		{
			...
			Constraints[i]->SetAngularVelocityDriveSLERP(bEnableTwistDrive || bEnableSwingDrive);
		}
		else if( bSetOtherBodiesToComplement )
		{
			Constraints[i]->SetAngularVelocityDriveTwistAndSwing(!bEnableTwistDrive, !bEnableSwingDrive);
...
void USkeletalMeshComponent::OnUpdateTransform(EUpdateTransformFlags UpdateTransformFlags, ETeleportType Teleport)
{
	...
	UpdateClothTransform(CVarClothPhysicsOnUpdateTransformTeleport.GetValueOnGameThread() ? Teleport : ETeleportType::None);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(USkeletalMeshComponent::OnCreatePhysicsState);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(USkeletalMeshComponent::OnDestroyPhysicsState);
...
void USkeletalMeshComponent::DestroyPhysicsStateInternal()
{
	...
	if (UseSceneProxy)
	{
		...
		for (FBodyInstance* BI : Bodies)
		{
			if (BI && BI->IsValidBodyInstance())
			{
				...
				{
					UE_LOGF(LogSkeletalMesh, Log, "SkeletalMeshComponent : (%d) Bone index out of bounds", BoneIndex);
					continue;
				}
...
void USkeletalMeshComponent::UpdateMeshForBrokenConstraints()
{
	...
	DEBUGBROKENCONSTRAINTUPDATE(UE_LOGF(LogSkeletalMesh, Log, "%3.3f UpdateMeshForBrokenConstraints", GetWorld()->GetTimeSeconds());)
	...
	for(int32 ConstraintInstIndex = 0; ConstraintInstIndex < Constraints.Num(); ConstraintInstIndex++)
	{
		...
		if( ConstraintInst && ConstraintInst->IsTerminated() )
		{
			...
			DEBUGBROKENCONSTRAINTUPDATE(UE_LOGF(LogSkeletalMesh, Log, "  Found Broken Constraint: (%d) %ls", JointBoneIndex, *PhysicsAsset->ConstraintSetup(ConstraintInstIndex)->JointName.ToString());)
			...
			for(int32 BodySetupIndex = 0; BodySetupIndex < PhysicsAsset->SkeletalBodySetups.Num(); BodySetupIndex++)
			{
				...
				{
					DEBUGBROKENCONSTRAINTUPDATE(UE_LOGF(LogSkeletalMesh, Log, "    Found Child Bone: (%d) %ls", BoneIndex, *PhysicsAssetBodySetup->BoneName.ToString());)
					...
					if( ChildBodyInst )
					{
						...
						{
							DEBUGBROKENCONSTRAINTUPDATE(UE_LOGF(LogSkeletalMesh, Log, "      Unfixing body.");)
					...
					if( ChildConstraintInst )
					{
						...
						{
							DEBUGBROKENCONSTRAINTUPDATE(UE_LOGF(LogSkeletalMesh, Log, "      Turning off LinearPositionDrive.");)
							ChildConstraintInst->SetLinearPositionDrive(false, false, false);
						}
						if( ChildConstraintInst->IsLinearVelocityDriveEnabled() )
						{
							DEBUGBROKENCONSTRAINTUPDATE(UE_LOGF(LogSkeletalMesh, Log, "      Turning off LinearVelocityDrive.");)
							ChildConstraintInst->SetLinearVelocityDrive(false, false, false);
						}
						if( ChildConstraintInst->IsAngularOrientationDriveEnabled() )
						{
							DEBUGBROKENCONSTRAINTUPDATE(UE_LOGF(LogSkeletalMesh, Log, "      Turning off AngularPositionDrive.");)
							ChildConstraintInst->SetOrientationDriveTwistAndSwing(false, false);
						}
						if( ChildConstraintInst->IsAngularVelocityDriveEnabled() )
						{
							DEBUGBROKENCONSTRAINTUPDATE(UE_LOGF(LogSkeletalMesh, Log, "      Turning off AngularVelocityDrive.");)
...
int32 USkeletalMeshComponent::ForEachBodyBelow(FName BoneName, bool bIncludeSelf, bool bSkipCustomType, TFunctionRef<void(FBodyInstance*)> Func)
{
	...
	else
	{
		...
		{
			// GetBodyIndicesBelow does not take into account the actual Bodies we've created.
			if (!Bodies.IsValidIndex(BodyIdx))
			{
				UE_LOGF(LogSkeletalMesh, Warning, "%s: assumed we had BodyIdx %d but we only have %d Bodies", __FUNCTION__, BodyIdx, Bodies.Num());
				break;
			}
...
void USkeletalMeshComponent::SetPhysicsAsset(UPhysicsAsset* InPhysicsAsset, bool bForceReInit)
{
	...
	if(bForceReInit || InPhysicsAsset != GetPhysicsAsset())
	{
		...
		if(GetSkeletalMeshAsset())
		{
			...
			{
			//	UE_LOGF(LogSkeletalMesh, Warning, "Creating Physics State (%ls : %ls)", *GetNameSafe(GetOuter()),  *GetName());			
...
bool USkeletalMeshComponent::ComponentOverlapComponentImpl(class UPrimitiveComponent* PrimComp,const FVector Pos,const FQuat& Quat,const struct FCollisionQueryParams& Params)
{
	...
	{
		UE_LOGF(LogCollision, Warning, "ComponentOverlapComponent : (%ls) Does not support skeletalmesh with Physics Asset", *PrimComp->GetPathName());
		return false;
	}
...
void USkeletalMeshComponent::RecreateClothingActors()
{
	...
	if (bIsWorldReadyForSimulation && bAllowClothActors && CVarEnableClothPhysics.GetValueOnGameThread())
	{
		...
		{
			UE_LOGF(LogSkeletalMesh, Log, "USkeletalMeshComponent: Recreating Clothing Actors for '%ls' with '%ls'", *GetName(), *SkelMesh->GetName());
...
void USkeletalMeshComponent::RemoveClothCollisionSources(USkeletalMeshComponent* InSourceComponent)
{
	...
	{
		const int32 NumRemoved = ClothCollisionSources.RemoveAll([InSourceComponent](const FClothCollisionSource& InCollisionSource)
			{
		...
		if (NumRemoved)
		{
			// Drop the tick prerequisite now that this source no longer contributes collisions
			ClothTickFunction.RemovePrerequisite(InSourceComponent, InSourceComponent->PrimaryComponentTick);
...
void USkeletalMeshComponent::RemoveClothCollisionSource(USkeletalMeshComponent* InSourceComponent, UPhysicsAsset* InSourcePhysicsAsset)
{
	...
	if (InSourceComponent && InSourcePhysicsAsset)
	{
		const int32 NumRemoved = ClothCollisionSources.RemoveAll([InSourceComponent, InSourcePhysicsAsset](const FClothCollisionSource& InCollisionSource)
			{
		...
		if (NumRemoved)
		{
			// Only drop the tick prerequisite when no remaining entry still references this source component
			bool bRemovePrerequisite = true;
			for (const FClothCollisionSource& ClothCollisionSource : ClothCollisionSources)
			{
				if (ClothCollisionSource.SourceComponent.Get() == InSourceComponent)
				{
					bRemovePrerequisite = false;
					break;
				}
			}
			if (bRemovePrerequisite)
			{
				ClothTickFunction.RemovePrerequisite(InSourceComponent, InSourceComponent->PrimaryComponentTick);
			}
			UpdateClothTransform();
...
{
#if WITH_CLOTH_COLLISION_DETECTION
	if (ClothCollisionSources.Num())
	{
		// Drop tick prerequisites for any still-valid sources before clearing the array
		for (const FClothCollisionSource& ClothCollisionSource : ClothCollisionSources)
		{
			if (ClothCollisionSource.SourceComponent.IsValid())
			{
				ClothTickFunction.RemovePrerequisite(ClothCollisionSource.SourceComponent.Get(), ClothCollisionSource.SourceComponent.Get()->PrimaryComponentTick);
			}
		}
...
{
	if (!ShouldUseClothEndTickWait()
		&& ClothingSimulationInstances.Num()
		&& CVarEnableClothPhysics.GetValueOnGameThread()
		&& !ShouldWaitForClothInTickFunction())
	{
		return true;
	}
...
bool USkeletalMeshComponent::ShouldUseClothEndTickWait() const
{
	return CVarClothPhysicsUseEndTickWait.GetValueOnAnyThread() != 0;
}
...
const TMap<int32, FClothSimulData>& USkeletalMeshComponent::GetCurrentClothingData_GameThread() const
{
	...
	if (!ShouldWaitForClothInTickFunction())
	{
		...
		UE_LOGF(LogSkeletalMesh, Warning, "Use of USkeletalMeshComponent::GetCurrentClothingData_GameThread requires that property bWaitForParallelClothTask be set to true");
...
{
	// Called during EndOfFrameUpdates, usually in a parallel-for loop, where waiting on the task here would re-enter EOF state via the busy wait.
	// With p.ClothPhysics.UseEndTickWait=1 (default), FSkeletalMeshComponentClothEndTickFunction (TG_LastDemotable) waits before EOF runs, so the task is complete by the time this is called.
	// With p.ClothPhysics.UseEndTickWait=0 (kill switch), OnPreEndOfFrameSync runs the wait earlier in SendAllEndOfFrameUpdatesInternal.
	// Either way, this never blocks; if the task isn't complete yet we return the empty dataset.
