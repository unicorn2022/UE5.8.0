...
void FMeshVertexPainter::PaintVerticesLerpAlongAxis(UStaticMeshComponent* StaticMeshComponent, const FLinearColor& StartColor, const FLinearColor& EndColor, EVertexPaintAxis Axis, bool bConvertToSRGB)
{
	...
	{
#if WITH_EDITOR
		UE_LOGF(LogStaticMesh,
			Warning,
			"bAllowCPUAccess should be set to true for StaticMesh '%ls' before calling PaintVerticesLerpAlongAxis(), otherwise the mesh geometry data isn't accessible when packed.",
			*StaticMesh->GetName());
#else
		UE_LOGF(LogStaticMesh,
			Error,
			"bAllowCPUAccess must be set to true for StaticMesh '%ls' before calling PaintVerticesLerpAlongAxis(), otherwise the mesh geometry data isn't accessible when packed.",
			*StaticMesh->GetName());
		return;
#endif
	}
