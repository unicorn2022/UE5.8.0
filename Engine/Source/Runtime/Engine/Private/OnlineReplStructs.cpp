...
namespace
{
	static const FString InvalidUniqueNetIdStr = "INVALID";
}
...
void FUniqueNetIdRepl::MakeReplicationDataV1()
{
	...
	if (ensure(Length > 0))
	{
		...
		//UE_LOGF(LogNet, VeryVerbose, "bEvenChars: %d EncodedSize: %d bIsNumeric: %d bIsPadded: %d", bEvenChars, EncodedSize32, bIsNumeric, bIsPadded);
		...
		if (EnumHasAllFlags(EncodingFlags, EUniqueIdEncodingFlags::IsEncoded))
		{
			...
			//UE_LOGF(LogNet, VeryVerbose, "HexEncoded UniqueId, serializing %d bytes", ReplicationBytes.Num());
		}
		else
		{
			...
			Writer << Contents;
			//UE_LOGF(LogNet, VeryVerbose, "Normal UniqueId, serializing %d bytes", ReplicationBytes.Num());
		}
	}
	else
	{
...
bool FUniqueNetIdRepl::NetSerialize(FArchive& Ar, UPackageMap* Map, bool& bOutSuccess)
{
	...
	if (Ar.IsSaving())
	{
		...
		//UE_LOGF(LogNet, Warning, "UID Save: Bytes: %d Success: %d", ReplicationBytes.Num(), bOutSuccess);
	}
	else if (Ar.IsLoading())
	{
		...
		{
			UE_LOGF(LogNet, Warning, "Error serializing unique id");
		}
	}
...
void FUniqueNetIdRepl::NetSerializeLoadV1Encoded(FArchive& Ar, const EUniqueIdEncodingFlags EncodingFlags, bool& bOutSuccess)
{
	...
	if (bValidTypeHash)
	{
			...
			if (EncodedSize > 0)
			{
				...
				if (!Ar.IsError())
				{
					...
					if (Contents.Len() > 0)
					{
						...
						{
							UE_LOGF(LogNet, Warning, "Error with unique id type");
						}
					}
				}
				else
				{
					UE_LOGF(LogNet, Warning, "Error with encoded unique id contents");
				}
			}
			else
			{
				UE_LOGF(LogNet, Warning, "Empty Encoding!");
			}
		...
		{
			UE_LOGF(LogNet, Warning, "Error with encoded unique id size");
		}
	}
	else
	{
		UE_LOGF(LogNet, Warning, "Error with encoded type hash");
	}
}
...
void FUniqueNetIdRepl::NetSerializeLoadV1Unencoded(FArchive& Ar, const EUniqueIdEncodingFlags EncodingFlags, bool& bOutSuccess)
{
	...
	if (bValidTypeHash)
	{
		...
		{
			UE_LOGF(LogNet, Warning, "Error with unencoded unique id");
		}
	}
	else
	{
			...
			if (const TCHAR* NewBuffer1 = FPropertyHelpers::ReadToken(Buffer, Token))
			{
				...
				if (IdContents != InvalidUniqueNetIdStr)
				{
					...
					if (!UOnlineEngineInterface::Get()->IsLoaded(OnlineSubsystemName))
					{
						...
						UE_CLOGF(!bLogged, LogNet, Log, "Attempted to ImportText to FUniqueNetIdRepl while OSS is not loaded. OSS:%ls Parent:%ls", *OnlineSubsystemName.ToString(), *GetPathNameSafe(Parent));
						bLogged = true;
					}
...
#define CHECK_REPL_VALIDITY(IdOne, TheBool) \
	...
	{
		UE_LOGF(LogNet, Warning, "EmptyId is valid: %ls", *EmptyIdIn.ToString());
		bSetupSuccess = false;
	}
	...
	{
		UE_LOGF(LogNet, Warning, "UserId input %ls != UserId output %ls", UserId.IsValid() ? *UserId->ToString() : *InvalidUniqueNetIdStr, *ValidIdIn.ToString());
		bSetupSuccess = false;
	}
...
#define WAYTOOLONG TEXT("deadbeef")
	...
	static FName NAME_CustomOSS("MyCustomOSS");
	...
	if (bSetupSuccess)
	{
		// Regular Serialization (persistent/disk based using FString)
		{
			...
			{
				UE_LOGF(LogNet, Warning, "EmptyId %ls should have been invalid", *EmptyIdOut->ToDebugString());
				bRegularSerializationSuccess = false;
			}

			if (EmptyIdIn != EmptyIdOut)
			{
				UE_LOGF(LogNet, Warning, "EmptyId In/Out mismatch");
				bRegularSerializationSuccess = false;
			}
		...
		// Network serialization (network/transient using MakeReplicationData)
		{
			...
			{
				UE_LOGF(LogNet, Warning, "There were %d encoding failures", EncodingFailures);
			...
			if (bNetworkSerializationSuccess)
			{
				...
				{
					UE_LOGF(LogNet, Warning, "There were %d decoding failures", DecodingFailures);
					bNetworkSerializationSuccess = false;
				}
		...
		// TextItem serialization
		{
			...
			{
				UE_LOGF(LogNet, Warning, "There were %d export failures", ExportFailures);
			...
			if (bTextItemSerializationSuccess)
			{
				...
				if (ImportFailures > 0)
				{
					UE_LOGF(LogNet, Warning, "There were %d import failures", ImportFailures);
					bTextItemSerializationSuccess = false;
				}
		...
		!NativePlatformService.IsEmpty())
	{
		...
		{
			UE_LOGF(LogNet, Warning, "PlatformUserId input %ls != PlatformUserId output %ls", PlatformUserId.IsValid() ? *PlatformUserId->ToString() : *InvalidUniqueNetIdStr, *ValidPlatformIdIn.ToString());
		...
		if (bPlatformSerializationSuccess)
		{
			...
			if (EncodingFailures > 0)
			{
				UE_LOGF(LogNet, Warning, "There were %d platform encoding failures", EncodingFailures);
				bPlatformSerializationSuccess = false;
			}
			...
			if (DecodingFailures > 0)
			{
				UE_LOGF(LogNet, Warning, "There were %d platform decoding failures", DecodingFailures);
				bPlatformSerializationSuccess = false;
			}
	...
	UE_LOGF(LogNet, Log, "TestUniqueIdRepl tests:");
	UE_LOGF(LogNet, Log, "	Setup: %ls", bSetupSuccess ? TEXT("PASS") : TEXT("FAIL"));
	UE_LOGF(LogNet, Log, "	Normal: %ls", bRegularSerializationSuccess ? (bSetupSuccess ? TEXT("PASS") : TEXT("SKIPPED")) : TEXT("FAIL"));
	UE_LOGF(LogNet, Log, "	Network: %ls", bNetworkSerializationSuccess ? (bSetupSuccess ? TEXT("PASS") : TEXT("SKIPPED")) : TEXT("FAIL"));
	UE_LOGF(LogNet, Log, "	TextItem: %ls", bTextItemSerializationSuccess ? (bSetupSuccess ? TEXT("PASS") : TEXT("SKIPPED")) : TEXT("FAIL"));
	UE_LOGF(LogNet, Log, "	Platform: %ls", bPlatformSerializationSuccess ? (bSetupSuccess ? TEXT("PASS") : TEXT("SKIPPED")) : TEXT("FAIL"));
	UE_LOGF(LogNet, Log, "	JSON: %ls", bJSONSerializationSuccess ? (bSetupSuccess ? TEXT("PASS") : TEXT("SKIPPED")) : TEXT("FAIL"));
