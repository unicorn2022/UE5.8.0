...
static void ToggleShadowIndexBuffers()
{
	...
	UE_LOGF(LogStaticMesh,Log,"Optimized shadow index buffers %ls",GUseShadowIndexBuffer ? TEXT("ENABLED") : TEXT("DISABLED"));
...
static void ToggleReversedIndexBuffers()
{
	...
	UE_LOGF(LogStaticMesh,Log,"Reversed index buffers %ls",GUseReversedIndexBuffer ? TEXT("ENABLED") : TEXT("DISABLED"));
	FGlobalComponentReregisterContext ReregisterContext;
}
...
static void ToggleForceDefaultMaterial()
{
	...
	UE_LOGF(LogStaticMesh,Log,"Force default material %ls",GForceDefaultMaterial ? TEXT("ENABLED") : TEXT("DISABLED"));
	FGlobalComponentReregisterContext ReregisterContext;
}
...
#endif
{
	...
	bool bUsePSOPrecacheFallbackMaterial = InProxyDesc.UsePSOPrecacheFallbackMaterial();
	...
	{
		MaterialRelevance |= UPSOPrecacheSettingsManager::GetFallbackMaterial() ? UPSOPrecacheSettingsManager::GetFallbackMaterial()->GetRelevance(ShaderPlatform) : FMaterialRelevance();
	}
	...
	if (bUsePSOPrecacheFallbackMaterial)
	{
	...
	if (InProxyDesc.ShouldCreateMaterialCacheProxy())
	{
		for (UMaterialCacheVirtualTexture* MaterialCacheTexture : InProxyDesc.MaterialCacheTextures)
		{
			if (FMaterialCacheVirtualTextureRenderProxy* RenderProxy = MaterialCacheTexture->CreateRenderProxy(
				InProxyDesc.MaterialCacheUVRegion,
				InProxyDesc.MaterialCacheTextureCoordinateIndex
			))
			{
...
uint8 FStaticMeshSceneProxy::GetStreamingLODWantedDeficit() const
{
	if (StaticMesh)
	{
		const FStreamableRenderResourceState State = StaticMesh->GetStreamableResourceState();
		const uint8 WantedLODs = StaticMesh->GetCachedNumWantedLODs();
...
void FStaticMeshSceneProxy::GetDynamicMeshElements(const TArray<const FSceneView*>& Views, const FSceneViewFamily& ViewFamily, uint32 VisibilityMap, FMeshElementCollector& Collector) const
{
	...
	if (EngineShowFlags.StaticMeshes && bDrawMesh)
	{
		...
		for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
		{
			...
			const bool bLODColoration = View->Family->EngineShowFlags.LODColoration;
								...
								for (int32 BatchIndex = 0; BatchIndex < NumBatches; BatchIndex++)
								{
									...
									if (GetMeshElement(LODIndex, BatchIndex, SectionIndex, SDPG_World, bSectionIsSelected, true, MeshElement))
									{
										...
										if (MeshElement.bDitheredLODTransition && LODMask.IsDithered())
										{
											...
											if(bLODColoration)
											{ 
												if (LODIndex == LODMask.LODIndex0)
												{
													MeshElement.CachedLODDither = FMeshBatch::EDitherState::FadeOut;
												}
												else if (LODIndex == LODMask.LODIndex1)
												{
													MeshElement.CachedLODDither = FMeshBatch::EDitherState::FadeIn;
												}
											}
#endif										
										}
		...
		if (VisibilityMap & (1 << ViewIndex))
		{
			...
			if((bDrawSimpleCollision || bDrawSimpleWireframeCollision) && BodySetup)
			{
				if(FMath::Abs(GetLocalToWorld().Determinant()) < UE_SMALL_NUMBER)
				{
					...
					//UE_LOGF(LogStaticMesh, Log, "Zero scaling not supported (%ls)", *StaticMesh->GetPathName());
				}
				else
				{
...
void FStaticMeshSceneProxy::GetStaticRayTracingInstances(FStaticRayTracingInstances& OutRayTracingInstances)
{
	const int32 NumBatches = GetNumMeshBatches();
	...
	for (int32 LODIndex = RayTracingMinLOD; LODIndex < NumLODs; ++LODIndex)
	{
		...
		for (int32 BatchIndex = 0; BatchIndex < NumBatches; BatchIndex++)
		{
			for (int32 SectionIndex = 0; SectionIndex < RayTracingLOD.Sections->Num(); SectionIndex++)
			{
				...
				if (bResult)
				{
					...
					RayTracingMaterials.Add(MeshBatch);
				}
			}
		}
		...
		OutRayTracingInstance.Geometry = RayTracingLOD.RayTracingGeometry;
...
FRaytracingVFAttributeData FStaticMeshSceneProxy::GetRaytracingVFAttributeData(const FSceneView* View)
{
	FRaytracingVFAttributeData Out;

	if (!bSupportRayTracing || !bHasRayTracingRepresentation || !IsRayTracingRelevant())
	{
		return Out;
	}

	if (!RenderData || !RenderData->RayTracingProxy || !RenderData->RayTracingProxy->LODVertexFactories)
	{
		return Out;
	}

	const int32 LODIndex = (View && RenderData->RayTracingProxy->bUsingRenderingLODs) ? FMath::Max(GetLOD(View), (int32)GetCurrentFirstLODIdx_RenderThread()) : 0;
	const FStaticMeshVertexFactories& VertexFactories = (*RenderData->RayTracingProxy->LODVertexFactories)[LODIndex];
	FRaytracingVFSectionData& Section = Out.Sections.Emplace_GetRef();
	Section.UV.BufferSRV = VertexFactories.VertexFactory.GetTextureCoordinatesSRV();
	Section.UV.NumCoordinates = VertexFactories.VertexFactory.GetNumTexcoords();
...
void FStaticMeshSceneProxy::GetDynamicRayTracingInstances(FRayTracingInstanceCollector& Collector)
{
...

	{
		...
		if (bEvaluateWPO && (*RenderData->RayTracingProxy->LODVertexFactories)[LODIndex].VertexFactory.GetType()->SupportsRayTracingDynamicGeometry())
		{
			...
			UpdateParams.Buffer = nullptr; // Use the shared vertex buffer - needs to be updated every frame
	...
	, bUsesMeshModifyingMaterials(false)
{
	...
	for(int32 SectionIndex = 0;SectionIndex < LODModel.Sections.Num();SectionIndex++)
	{
		...
		bool bUseDefaultMaterial = (GForceDefaultMaterial && SectionInfo.Material && !IsTranslucentBlendMode(*SectionInfo.Material));
		if (bUsePSOPrecacheFallbackMaterial || bUseDefaultMaterial)
		{
			UMaterial* FallbackMaterial = (bUseDefaultMaterial || UPSOPrecacheSettingsManager::GetFallbackMaterial() == nullptr) ? UMaterial::GetDefaultMaterial(MD_Surface) : UPSOPrecacheSettingsManager::GetFallbackMaterial();			
			SectionInfo.Material = FallbackMaterial;
			if (SectionInfo.OverlayMaterial)
			{
				SectionInfo.OverlayMaterial = FallbackMaterial;
			}
		}

		// If there isn't an applied material, or if the material doesn't have static mesh usage or if we need static lighting and the material doesn't support it, then fall back to the default material.
		if (!SectionInfo.Material || 
			!SectionInfo.Material->CheckMaterialUsage_Concurrent(MATUSAGE_StaticMesh) ||
			(bHasSurfaceStaticLighting && !SectionInfo.Material->CheckMaterialUsage_Concurrent(MATUSAGE_StaticLighting)))
		{
...
FStaticMeshSceneProxy::FRayTracingLODInfo::FRayTracingLODInfo(const FStaticMeshSceneProxyDesc& InProxyDesc, int32 LODIndex)
{
	...
	for(int32 SectionIndex = 0;SectionIndex < LOD.Sections->Num();SectionIndex++)
	{
		...
		if (bUsePSOPrecacheFallbackMaterial || bUseDefaultMaterial)
		{
			SectionInfo.Material = (bUseDefaultMaterial || UPSOPrecacheSettingsManager::GetFallbackMaterial() == nullptr) ? UMaterial::GetDefaultMaterial(MD_Surface) : UPSOPrecacheSettingsManager::GetFallbackMaterial();
		}
