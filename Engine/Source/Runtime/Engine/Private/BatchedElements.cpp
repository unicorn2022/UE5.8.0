...
void FBatchedElements::AddTriangleExtensive(int32 V0,int32 V1,int32 V2,FBatchedElementParameters* BatchedElementParameters,const FTexture* Texture,ESimpleElementBlendMode BlendMode, const FDepthFieldGlowInfo& GlowInfo)
{
	...
	if(!MeshElement)
	{
		...
		{
			UE_LOGF(LogBatchedElements, Warning, "Omitting FBatchedElements::AddTriangle due to sparce vertices V0=%i,V1=%i,V2=%i",V0,V1,V2);
		}
		else
		{
...
void FBatchedElements::AddReserveLines(int32 NumLines, bool bDepthBiased, bool bThickLines)
{
	...
	{
		ThickLines.Reserve(ThickLines.Num() + NumLines);
	}
}
...
void FBatchedElements::DrawPointElements(FRHICommandList& RHICmdList, const FMatrix& Transform, const uint32 ViewportSizeX, const uint32 ViewportSizeY, const FVector& CameraX, const FVector& CameraY, const uint32 NumInstances) const
{
	...
	if( Points.Num() > 0 )
	{
		...
		{
			UE_LOGF(LogBatchedElements, Error, "Too many points. Will overflow uint32 buffer size. NumPoints: %d", Points.Num());
			return;
		}
...
bool FBatchedElements::Draw(FRHICommandList& RHICmdList, const FMeshPassProcessorRenderState& DrawRenderState, ERHIFeatureLevel::Type FeatureLevel, const FSceneView& View, bool bHitTesting, float Gamma /* = 1.0f */, EBlendModeFilter::Type Filter /* = EBlendModeFilter::All */, uint32 NumInstances /* = 1 */) const
{
	...
	const FMatrix& WorldToClip = View.ViewMatrices.GetWorldToClip();
	const FMatrix& ClipToWorld = View.ViewMatrices.GetClipToWorld();
	...
	if( HasPrimsToDraw() )
	{
			...
			&& (Filter & EBlendModeFilter::OpaqueAndMasked))
		{
			...
			if( LineVertices.Num() > 0 )
			{
				...
				{
					UE_LOGF(LogBatchedElements, Error, "Too many line vertices. Will overflow uint32 buffer size. LineVertices: %d", LineVertices.Num());
					return false;
				}
			...
			if ( ThickLines.Num() > 0 )
			{
				...
				const bool bIsPerspective = View.ViewMatrices.GetViewToClip().M[3][3] < 1.0f ? true : false;
				if (!bIsPerspective)
				{
					OrthoZoomFactor = 1.0f / View.ViewMatrices.GetViewToClip().M[0][0];
				}
		...
		if( Sprites.Num() > 0 )
		{
			...
			if (ValidSpriteCount > 0)
			{
				...
				{
					UE_LOGF(LogBatchedElements, Error, "Too many sprites. Will overflow uint32 buffer size. ValidSpriteCount: %d", ValidSpriteCount);
					return false;
				}
		...
		if( MeshElements.Num() > 0)
		{
			...
			{
				UE_LOGF(LogBatchedElements, Error, "Too many mesh vertices. Will overflow uint32 buffer size. MeshVertices.Num(): %d", MeshVertices.Num());
				return false;
			}
