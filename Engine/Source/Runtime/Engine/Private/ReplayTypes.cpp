...
void FNetworkDemoHeader::SetDefaultNetworkVersions()
{
	...
	EngineNetworkProtocolVersion = FNetworkVersion::GetNetworkProtocolVersion(FEngineNetworkCustomVersion::Guid);
	GameNetworkProtocolVersion = FNetworkVersion::GetNetworkProtocolVersion(FGameNetworkCustomVersion::Guid);
...
FArchive& operator<<(FArchive& Ar, FNetworkDemoHeader& Header)
{
	...
	{
		UE_LOGF(LogDemo, Error, "Header.Magic != NETWORK_DEMO_MAGIC");
		Ar.SetError();
		return Ar;
	}
	...
	{
		UE_LOGF(LogDemo, Error, "Header.Version < MinSupportedVersion. Header.Version: %i, MinSupportedVersion: %i", Header.Version, FReplayCustomVersion::MinSupportedVersion);
		Ar.SetError();
		return Ar;
	}
	...
	if (Ar.IsLoading())
	{
		...
		for (const FCustomVersionDifference& Diff : VersionDiffs)
		{
			...
			{
				UE_LOGF(LogDemo, Error, "Replay was saved with a custom version that is not present. Tag %ls Version %d", *Diff.Version->Key.ToString(), Diff.Version->Version);
				Ar.SetError();
				return Ar;
			}
			else if (Diff.Type == ECustomVersionDifference::Invalid)
			{
				UE_LOGF(LogDemo, Error, "Replay was saved with an invalid custom version. Tag %ls Version %d", *Diff.Version->Key.ToString(), Diff.Version->Version);
				Ar.SetError();
				return Ar;
			}
			else if (Diff.Type == ECustomVersionDifference::Newer)
			{
				...
				UE_LOGF(LogDemo, Error, "Replay was saved with a newer custom version than the current. Tag %ls Name '%ls' ReplayVersion %d  MaxExpected %d",
	...
	else
	{
		if (Ar.IsLoading())
		{
			...
			// and not FEngineNetworkCustomVersion for versioning. Therefore, pre-LWC replays
			// that did not save the EUnrealEngineObjectUE5Version will try to read doubles
			// from these vectors instead of floats.
			//
			// However, FEngineNetworkCustomVersion::SerializeDoubleVectorsAsDoubles was
			...
			// between when FEngineNetworkCustomVersion::SerializeDoubleVectorsAsDoubles and LARGE_WORLD_COORDINATES
