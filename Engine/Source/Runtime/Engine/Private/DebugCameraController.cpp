...
ASpectatorPawn* ADebugCameraController::SpawnSpectatorPawn()
{
	...
	if (GetSpectatorPawn() == nullptr && IsLocalController())
	{
		...
		if (GameState)
		{
			...
			{
				UE_LOGF(LogPlayerController, Warning, "Failed to spawn spectator with class %ls", GameState->SpectatorClass ? *GameState->SpectatorClass->GetName() : TEXT("NULL"));
			}
		}
		else
		{
...
void ADebugCameraController::CycleViewMode()
{
	...
	if (UGameViewportClient* GameViewportClient = GetWorld()->GetGameViewport())
	{
		...
		{
			UE_LOGF(LogPlayerController, Warning, "Debug camera controller settings must specify at least one view mode for view mode cycling.");
			return;
		}
		...
		if (NextViewModeIndex != CurrViewModeIndex)
		{
			...
			{
				FString Cmd("VIEWMODE ");
				Cmd += NextViewModeName;
				GameViewportClient->ConsoleCommand(Cmd);
			}
			else
			{
				UE_LOGF(LogPlayerController, Warning, "Invalid view mode index %d.", NextViewModeIndex);
			}
		}
...
TArray<FString> ADebugCameraController::GetBufferVisualizationOverviewTargets()
{
	...
	{
		UE_LOGF(LogPlayerController, Warning, "Console variable r.BufferVisualizationOverviewTargets is not found.");
	}
...
void ADebugCameraController::SetBufferVisualizationFullMode(bool bFullMode)
{
	if (bEnableBufferVisualizationFullMode != bFullMode)
	{
		...
		{
			UE_LOGF(LogPlayerController, Verbose, "Console variable %ls does not exist.", FBufferVisualizationData::GetVisualizationTargetConsoleCommandName());
		}
	}
}
