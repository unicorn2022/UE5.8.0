...
static void ConstructResolvedSettings(ReadyMask InReadyMask, UDeveloperSettings* InSettings, FResolvedTextureEncodingSettings* OutResolvedSettings)
{
	...
	OutResolvedSettings->Project.bSharedLinearTextureEncoding = ProjectSettings->bSharedLinearTextureEncoding;
	{
		...
		if (FParse::Bool(FCommandLine::Get(), TEXT("-ForceSharedLinearTextureEncoding="), CmdLineSLE))
		{
			...
			UE_LOGF(LogTextureEncodingSettings, Display, "Shared linear texture encoding forced to %ls via command line.", *LexToString(CmdLineSLE));
		}
	}
	...
	// Determine what encode speed to use
	{
		...
		if (FParse::Value(FCommandLine::Get(), TEXT("-ForceTextureEncodeSpeed="), CmdLineSpeed))
		{
			...
			if (Value == INDEX_NONE)
			{
				UE_LOGF(LogTextureEncodingSettings, Error, "Invalid value for ForceTextureEncodeSpeed, ignoring. Valid values are the ETextureEncodeSpeed enum (Final, FinalIfAvailable, Fast)");
			}
			else
			{
				OutResolvedSettings->EncodeSpeed = (ETextureEncodeSpeed)Value;
				UE_LOGF(LogTextureEncodingSettings, Display, "Texture Encode Speed forced to %ls via command line.", *EncodeSpeedEnum->GetNameStringByValue(Value));
				return;
			}
		}
		...
		if (OutResolvedSettings->User.ForceEncodeSpeed != ETextureEncodeSpeedOverride::Disabled)
		{
			...
			UE_LOGF(LogTextureEncodingSettings, Display, "Texture Encode Speed forced to %ls via user settings.", *EncodeSpeedEnum->GetNameStringByValue((int64)OutResolvedSettings->EncodeSpeed));
			return;
		}
		...
		if (GIsEditor && !IsRunningCommandlet())
		{
			...
			UE_LOGF(LogTextureEncodingSettings, Display, "Texture Encode Speed: %ls (editor).", *EncodeSpeedEnum->GetNameStringByValue((int64)OutResolvedSettings->EncodeSpeed));
		}
		else
		{
			OutResolvedSettings->EncodeSpeed = OutResolvedSettings->Project.CookUsesSpeed;
			UE_LOGF(LogTextureEncodingSettings, Display, "Texture Encode Speed: %ls (cook).", *EncodeSpeedEnum->GetNameStringByValue((int64)OutResolvedSettings->EncodeSpeed));
		}
	}
	...
	// Log encode speeds
	{
		...
		{
			FinalRDOString = FString("Off");
		}

		UE_LOGF(LogTextureEncodingSettings, Display, "Oodle Texture Encode Speed settings: Fast: RDO %ls Lambda=%d, Effort=%ls Final: RDO %ls Lambda=%d, Effort=%ls", \
	...
	UE_LOGF(LogTextureEncodingSettings, Display, "Shared linear texture encoding: %ls", OutResolvedSettings->Project.bSharedLinearTextureEncoding ? TEXT("Enabled") : TEXT("Disabled"));
}
