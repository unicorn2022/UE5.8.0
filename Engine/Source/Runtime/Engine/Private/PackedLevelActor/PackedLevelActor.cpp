...
void APackedLevelActor::RerunConstructionScripts()
{
	...
	if (GetWorld() && GetWorld()->IsGameWorld())
	{
		...
		if (PackedVersion != GetClass()->GetDefaultObject<APackedLevelActor>()->PackedVersion)
		{
			...
			UE_LOGF(LogLevelInstance, Verbose, "RerunConstructionScript was executed on %ls (%ls) because its version (%ls) doesn't match latest version (%ls). Resaving this actor will fix this",
				*GetPathName(),
