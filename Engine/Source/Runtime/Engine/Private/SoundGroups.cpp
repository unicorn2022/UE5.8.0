...
void USoundGroups::Initialize() const
{
	...
	{
		UE_LOGF(LogAudio, Warning, "Missing default SoundGroup profile. Creating default with no decompression.");
	...
	for (const auto& It : SoundGroupMap)
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogAudio, Warning, "Custom Game SoundGroup profile for %ls defined but no display name supplied.", *SoundGroupEnum->GetDisplayNameTextByValue(It.Key).ToString());
			}
			SoundGroupEnum->RemoveMetaData(TEXT("Hidden"), It.Key);
		}
	}
...
const FSoundGroup& USoundGroups::GetSoundGroup(const ESoundGroup SoundGroup) const
{
	...
	if (SG == NULL)
	{
		...
		UE_LOGF(LogAudio, Warning, "Requested SoundGroup %ls does not have defined profile.  Using SOUNDGROUP_Default.", *SoundGroupEnum->GetDisplayNameTextByValue(SoundGroup).ToString());
