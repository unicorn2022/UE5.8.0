...
namespace UE::StringTable
{
	...
	void GatherForLocalization(const FString& SourceLocation, const FStringTableConstRef& StringTable, FPropertyLocalizationDataGatherer& PropertyLocalizationDataGatherer, const EPropertyLocalizationGathererTextFlags GatherTextFlags)
	{
		...
		StringTable->EnumerateSourceStrings([&SourceLocation, &StringTable, &FindOrAddTextData](const FString& InKey, const FString& InSourceString, const FString& InDevNotes) -> bool
		{
	...
	void UnregisterForGCUnderPath(FStringView PluginPackageRoot)
	{
		KeepAliveStringTables.RemoveAllSwap(
			[&PluginPackageRoot](const TObjectPtr<UStringTable>& InStringTableAsset)
			{
				...
				{
					UE_LOGF(LogStringTable, Log, "Removing string table '%ls' from keep-alive list", *InStringTableAsset->GetName());
					return true;
				}
...
private:
	...
	{
		return IsInGameThread() && !IsGarbageCollecting() && !UE::IsSavingPackage();
	}
