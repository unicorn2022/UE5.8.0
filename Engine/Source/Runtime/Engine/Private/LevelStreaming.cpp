...
{
	static void OnVisibilityTransactionCVarChanged(IConsoleVariable* CVar)
	{
		if (GEngine)
		{
			for (const FWorldContext& Context : GEngine->GetWorldContexts())
			{
				...
				{
					World->InvalidateVisiblityTransactionRequestCachedVariables();
				}
			}
		}
	}
	...
	FAutoConsoleVariableRef CVarDefaultAllowClientUseMakingInvisibleTransactionRequests(
		...
		FConsoleVariableDelegate::CreateStatic(&*LevelStreamingCVars::OnVisibilityTransactionCVarChanged),
		ECVF_Default);
	...
	static bool bForceFlushLoadsBeforeAddToWorld = false;
	FAutoConsoleVariableRef CVarForceFlushLoadsBeforeAddToWorld(
		TEXT("LevelStreaming.ForceFlushLoadsBeforeAddToWorld"),
		bForceFlushLoadsBeforeAddToWorld,
		TEXT("Force a full FlushAsyncLoads() call before AddToWorld in MakingVisible.\n")
		TEXT("0: Disable, 1: Enable"),
		ECVF_Default);
}
...
void FStreamLevelAction::UpdateOperation(FLatentResponse& Response)
{
	...
	UE_LOGF(LogLevelStreaming, Verbose, "FStreamLevelAction::UpdateOperation() LevelName %ls, bIsLevelValid %d", *LevelName.ToString(), (int32)bIsLevelValid);
	if (bIsLevelValid)
	{
...
{	
	if (LevelStreamingObject)
	{
		...
		if (bLoading)
		{
			UE_LOGF(LogStreaming, Log, "Streaming in level %ls (%ls)...",*LevelStreamingObject->GetName(),*LevelStreamingObject->GetWorldAssetPackageName());
		...
		{
			UE_LOGF(LogStreaming, Log, "Streaming out level %ls (%ls)...",*LevelStreamingObject->GetName(),*LevelStreamingObject->GetWorldAssetPackageName());
		...
		if (UWorld* LevelWorld = LevelStreamingObject->GetWorld())
		{
			...
			UE_LOGF(LogLevel, Log, "ActivateLevel %ls %i %i %i",
	...
	{
		UE_LOGF(LogLevel, Warning, "Failed to find streaming level object associated with '%ls'", *LevelName.ToString() );
	}
}
...
void ULevelStreaming::PostLoad()
{
	...
	if ( !PIESession && PackageName_DEPRECATED != NAME_None )
	{
		...
		{
			UE_LOGF(LogLevelStreaming, Display, "Invalid streaming level package name (%ls). Only long package names are supported. This streaming level may not load or save properly.", *DeprecatedPackageNameString);
		}
	}
#endif
	...
	if (!IsValidStreamingLevel())
	{
		...
		UE_LOGF(LogLevelStreaming, Display, "Failed to find streaming level package file: %ls. This streaming level may not load or save properly.", *WorldPackageName);
#if WITH_EDITOR
		if (GIsEditor)
		{
	...
	if (Ar.IsLoading())
	{
		...
		if (Ar.IsPersistent() && !bIsRunningPIE)
		{
			...
			if (bHasMismatchedPackageName)
			{
				...
				UE_LOGF(LogLevelStreaming, Warning, "WorldAsset (%ls) and PackageNameToLoad (%ls) point to different streaming levels. PackageNameToLoad will be set to none. Resave this map (%ls) to remove warnings.",
...
void ULevelStreaming::ServerUpdateLevelVisibility(bool bIsVisible, bool bTryMakeVisible, FNetLevelVisibilityTransactionId TransactionId)
{
	if (IsConcernedByNetVisibilityTransactionAck())
	{
		...
		for (FConstPlayerControllerIterator Iterator = World->GetPlayerControllerIterator(); Iterator; ++Iterator)
		{
			...
			UNetConnection* NetConnection = IsValid(PlayerController) && !PlayerController->IsPendingKillPending() ? PlayerController->GetNetConnection() : nullptr;
			if (NetConnection && NetConnection->GetUChildConnection() == nullptr && NetConnection->GetConnectionState() == USOCK_Open)
			{
				...
				// No need to loop on all player controllers as ServerUpdateLevelVisibility only needs to be done once per connection
				break;
			}
		}
	}
	...
	case ELevelStreamingState::MakingVisible:
		if (ensure(LoadedLevel))
		{
			...
			else
			{
				...
				if (LevelStreamingCVars::bForceFlushLoadsBeforeAddToWorld)
				{
					// This is intended as a temporary workaround (CORO-8867). A proper fix is being tracked via CORO-8868.
					FlushAsyncLoading();
				}
...
void ULevelStreaming::PrepareLoadedLevel(ULevel* InLevel, UPackage* InLevelPackage, int32 InPIEInstanceID)
{
	...
	if (ensure(LevelOwningWorld))
	{
		if (LoadedLevel && (LevelOwningWorld->IsLevelMakingVisible(LoadedLevel) || LevelOwningWorld->IsLevelMakingInvisible(LoadedLevel)))
		{
			...
			UE_LOGF(LogLevelStreaming, Verbose, "Delaying setting result of async load new level %ls, because current loaded level still processing visibility request", *InLevelPackage->GetName());
		}
		else
		{
			check(PendingUnloadLevel == nullptr);
...
bool ULevelStreaming::ValidateUniqueWorldAsset(UWorld* PersistentWorld)
{
	...
	for (ULevelStreaming* OtherLevel : PersistentWorld->GetStreamingLevels())
	{
		...
		if (OtherLevel->WorldAsset == WorldAsset)
		{
			...
			{
				UE_LOGF(LogLevelStreaming, Warning, "Streaming Level '%ls' uses same destination for level ('%ls') as '%ls'. Level cannot be loaded again and this StreamingLevel will be flagged as failed to load.", *GetPathName(), *WorldAsset.GetLongPackageName(), *OtherLevel->GetPathName());
...
bool ULevelStreaming::RequestLevel(UWorld* PersistentWorld, bool bAllowLevelLoadRequests, EReqLevelBlock BlockPolicy)
{
    ...
    {
		UE_LOGF(LogLevelStreaming, Verbose, "Delaying load of new level %ls, because %ls still processing visibility request.", *DesiredPackageName.ToString(), *CachedLoadedLevelPackageName.ToString());
		return false;
	}
	...
	if ( LevelPackage == nullptr && PersistentWorld->IsPlayInEditor() )
	{
		...
		if (bShouldDuplicate)
		{
			...
			else if (PersistentWorld->WorldComposition == NULL) // In world composition streaming levels are not loaded by default
			{
				...
				{
					UE_LOGF(LogLevelStreaming, Log, "World to duplicate for PIE '%ls' not found. Attempting load.", *NonPrefixedLevelName);
				}
				else
				{
					UE_LOGF(LogLevelStreaming, Warning, "Unable to duplicate PIE World: '%ls'", *NonPrefixedLevelName);
				}
			}
		}
	}
	...
	if (LevelPackage)
	{
		if (World != nullptr)
		{
			if (!IsValid(World))
			{
				...
				UE_LOGF(LogLevelStreaming, Verbose, "RequestLevel: World is pending kill %ls", *DesiredPackageName.ToString());
				return false;
			}
			...
			{
				UE_LOGF(LogLevelStreaming, Error, "World exists but PersistentLevel doesn't for %ls, most likely caused by reference to world of unloaded level and GC setting reference to null while keeping world object", *World->GetOutermost()->GetName());
				UE_LOGF(LogLevelStreaming, Error, "Most likely caused by reference to world of unloaded level and GC setting reference to null while keeping world object. Referenced by:");
			...
			{
				UE_LOGF(LogLevelStreaming, Verbose, "Trying to reuse level %ls but ShouldReuseUnloadedButStillAroundLevels returns false. Trash the current package and reload.", *LevelPackage->GetName());
				TrashObject(LevelPackage);
				return false;
			}
	...
	if (bAllowLevelLoadRequests)
	{
		...
		TRACE_BEGIN_REGION(*WriteToString<256>(TEXT("Load: "), FNameBuilder(DesiredPackageName)), TEXT("LevelStreaming"));
		...
		if (BlockPolicy == AlwaysBlock || (ShouldBeAlwaysLoaded() && BlockPolicy != NeverBlock))
		{
			if (IsAsyncLoading())
			{
				UE_LOGF(LogStreaming, Display, "ULevelStreaming::RequestLevel(%ls) is flushing async loading", *DesiredPackageName.ToString());
			}
...
void ULevelStreaming::AsyncLevelLoadComplete(const FName& InPackageName, UPackage* InLoadedPackage, EAsyncLoadingResult::Type Result)
{
	...
	if (InLoadedPackage)
	{
		...
		if (World)
		{
			...
			{
				UE_LOGF(LogLevelStreaming, Warning, "Couldn't find ULevel object in package '%ls'", *InPackageName.ToString() );
			}
		}
		else
		{
			...
			{
				UE_LOGF(LogLevelStreaming, Warning, "Failed to find a world in package '%ls'", *InPackageName.ToString());
				SetCurrentState(ELevelStreamingState::FailedToLoad);
	...
	{
		UE_LOGF(LogLevelStreaming, Warning, "Failed to load package '%ls'", *InPackageName.ToString());
		if (!FPackageName::DoesPackageExist(PackagePath, &PackagePath))
		{
			UE_LOGF(LogLevelStreaming, Error, "Couldn't find file for package %ls.", *PackagePath.GetDebugName());
		}
		SetCurrentState(ELevelStreamingState::FailedToLoad);
 		SetShouldBeLoaded(false);
	}
	...
	TRACE_END_REGION(*WriteToString<256>(TEXT("Load: "), FNameBuilder(InPackageName)));
}
...
ULevelStreaming* ULevelStreaming::CreateInstance(const FString& InstanceUniqueName)
{
	...
	if (InWorld)
	{
		...
		{
			UE_LOGF(LogStreaming, Warning, "Provided streaming level instance name is not unique: %ls", *InstanceUniquePackageName.ToString());
		}
	}
	...
	if (bNeedsUniqueTest)
	{
		...
		if (ULevelStreaming* const* ExistingLevelStreaming = Params.World->GetStreamingLevels().FindByPredicate([&ModifiedLevelPackageName](ULevelStreaming* LS) { return LS && LS->GetWorldAssetPackageFName() == ModifiedLevelPackageName; }))
		{
				...
				StreamingLevel->LevelTransform.Equals(Params.LevelTransform))
			{
				...
				UE_LOGF(LogLevelStreaming, Verbose, "LoadLevelInstance found existing StreamingLevel for LevelPackageName:%ls", *ModifiedLevelPackageName.ToString());
				return StreamingLevel;
			}
			else
			{
				UE_LOGF(LogLevelStreaming, Error, "LoadLevelInstance called with a name that already exists, returning nullptr. LevelPackageName:%ls", *ModifiedLevelPackageName.ToString());
				return nullptr;
			}
		}
	}
