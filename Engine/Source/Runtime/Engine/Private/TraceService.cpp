...
FTraceService::FTraceService(const TSharedPtr<IMessageBus>& InBus)
{
	...
	if (InBus.IsValid())
	{
		MessageEndpoint = FMessageEndpoint::Builder("FTraceService", InBus.ToSharedRef())
			...
			.Handling<FTraceControlSecure>(this, &FTraceService::OnSecure)
#endif
...
#if UE_TRACE_ALLOW_SECURE_TRACING 
		// MessageEndpoint->Subscribe<FTraceControlSecure>();
#endif
	}
}
...
#if UE_TRACE_ALLOW_SECURE_TRACING 
	Response->Capabilities.Add(FTraceCapability{ TEXT("SecureConnection") });
#endif
...
void FTraceService::OnSecure(const FTraceControlSecure& Message, const TSharedRef<IMessageContext>& Context)
{
#if UE_TRACE_ALLOW_SECURE_TRACING 

	// Generate a random authorization token
	FGuid AuthToken = FGuid::NewGuid();
	FArrayWriter AuthData;
	AuthData << AuthToken;
	uint32 AuthTokenSize = AuthData.Num();
	
	// Setup the security options
	FString Identity = FGuid::NewGuid().ToString(); // Random identity
	FTraceAuxiliary::FSecureOptions Options;
	Options.AuthorizationToken = AuthData;
	Options.TimeoutSeconds = 30.0f;
	Options.Identity = TCHAR_TO_ANSI(*Identity);

	const bool bSuccess = FTraceAuxiliary::StartSecure(Options);
	...
	const auto Response = FMessageEndpoint::MakeMessage<FTraceControlSecureResponse>();
	Response->Host = FTraceAuxiliary::GetSecureHost();
	Response->Port = FTraceAuxiliary::GetSecurePort();
	Response->AuthorizationToken = AuthData;
	Response->CertificateAuthority = FTraceAuxiliary::GetSecureCertificate();
	if (!Response->CertificateAuthority.IsEmpty())
	{
		Response->Mode = ETraceSecureMode::SelfSignedCertAndAuthToken;
	}
	else
	{
		Response->Mode = ETraceSecureMode::AuthToken;
