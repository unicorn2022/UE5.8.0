...
namespace UE::Private::FontFace
{
	const FString& GetStandaloneFontFaceDirectory()
	{
		static const FString FontFaceDirectory = FPaths::Combine(FPaths::ProjectIntermediateDir(), TEXT("FontFaces"));
		return FontFaceDirectory;
	}
}

static bool GSimulateCookedFontFilesInStandalone = false;
FAutoConsoleVariableRef CVarSimulateCookedFontFilesInStandalone(
	TEXT("UI.Slate.Font.SimulateCookedFontFilesInStandalone"),
	GSimulateCookedFontFilesInStandalone,
	TEXT("If true when running in standalone, the embedded font face data will be written to the Intermediate project folder and loaded on demand. This simulates the loading of .ufont files in a cooked build."));
...
EFontRasterizationMode GetDeviceFontFaceDistanceFieldRasterizationMode()
{
	...
	UE_LOGF(LogFontFace, Warning, "Unexpected value of CVar UI.SlateSDFText.RasterizationMode - falling back to Bitmap.");
...
#if WITH_EDITOR
	// For the regular Editor, all font faces need to be loaded for editing and the font data is embedded.
	// In standalone, we write out the font data for lazily loaded fonts to the Intermediate project folder and replace the filename of the font face.
	// This causes lazily loaded fonts to actually be loaded from disk to simulate the behavior in a cooked build.
	if (!GIsEditor && !GIsServer && GSimulateCookedFontFilesInStandalone && LoadingPolicy == EFontLoadingPolicy::LazyLoad)
	{
		if (FontFaceData->HasData())
		{
			FString Filename = GetStandaloneFilename();
			FString DirectoryPath = FPaths::GetPath(Filename);
			if (!IPlatformFile::GetPlatformPhysical().DirectoryExists(*DirectoryPath))
			{
				UE_LOGF(LogFontFace, Display, "Standalone font face directory '%ls' does not exist. Creating directory.", *DirectoryPath);
				if (!IPlatformFile::GetPlatformPhysical().CreateDirectoryTree(*DirectoryPath))
				{
					UE_LOGF(LogFontFace, Warning, "Unable to create standalone font face directory '%ls'. Font face '%ls' will not be written and async font loading of font face will be skipped.", *DirectoryPath, *Filename);
					return;
				}
			}

			if (FFileHelper::SaveArrayToFile(FontFaceData->GetData(), *Filename))
			{
				UE_LOGF(LogFontFace, Display, "Written font face file '%ls' to simulate loading of cooked font files in standalone mode.", *Filename);
				SourceFilename = MoveTemp(Filename);
				// We clear the data in the FFontFaceData so we force a synchronous load of the data 
				TArray<uint8> EmptyData;
				FontFaceData->SetData(MoveTemp(EmptyData));
			}
			else
			{
				UE_LOGF(LogFontFace, Warning, "Failed to write font face file '%ls' to simulate loading of cooked font files in standalone mode.", *Filename);
			}
		}
	}
#endif
...
#if WITH_EDITOR
FString UFontFace::GetStandaloneFilename() const
{
	// This should ever only be called when in standalone 
	ensure(!GIsEditor);
	// This implementation follows UFontFace::GetCookedFilename. If that implementation changes, it is best to also adapt this to the change 
	FString PackageName = GetOutermost()->GetName();
	PackageName = FPackageName::GetLocalizedPackagePath(PackageName);

	return FPaths::Combine(UE::Private::FontFace::GetStandaloneFontFaceDirectory(), PackageName) / GetName() + TEXT(".ufont");
}
	...
	TFunctionRef<void(const TCHAR* Filename, void* Data, int64 Size)> WriteAdditionalFile)
{
	if (LoadingPolicy != EFontLoadingPolicy::Inline)
	{
		...
		{
			UE_LOGF(LogFontFace, Warning, 
				"The font asset '%ls' contains nested font faces which can cause issues for iterative cook-on-the-fly."
