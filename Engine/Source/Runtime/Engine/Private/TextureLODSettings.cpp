...
int32 TextureGroupMaxQualityLevel = -1;
FAutoConsoleVariableRef CVarTextureGroupMaxQualityLevel(
	TEXT("r.TextureGroup.MaxQualityLevel"),
	TextureGroupMaxQualityLevel,
	TEXT("Used to apply texture quality bias to all textures and to limit TextureGroupQualityLevel. (default=-1)"),
	ECVF_Default
);

int32 TextureGroupQualityLevel = -1;
FAutoConsoleVariableRef CVarTextureGroupQualityLevel(
	TEXT("r.TextureGroup.QualityLevel"),
	TextureGroupQualityLevel,
	TEXT("Used to apply texture quality bias to streamable textures at runtime. Clamped by r.TextureGroup.MaxQualityLevel. (default=-1)"),
	ECVF_Default
);
...
void FTextureLODGroup::SetupGroup()
{
	...
	if (!GIsEditor)
	{
		...
		if (TextureGroupMaxQualityLevel >= 0 && TextureGroupMaxQualityLevel < 4)
		{
			int32 MaxSizes[4];
			ComputeMaxSizesForQualityLevels(MaxSizes);
			if (MaxSizes[TextureGroupMaxQualityLevel] != -1)
			{
				MaxLODSize = MaxSizes[TextureGroupMaxQualityLevel];
			}
		}
	}
...
void FTextureLODGroup::ComputeMaxSizesForQualityLevels(int32 OutMaxSizes[4]) const
{
	// We don't want to implicitly generate quality if no quality was set for this group
	if (MaxLODSize_Low == -1 &&
		MaxLODSize_Medium == -1 &&
		MaxLODSize_High == -1 &&
		MaxLODSize_Epic == -1)
	{
		OutMaxSizes[0] = OutMaxSizes[1] = OutMaxSizes[2] = OutMaxSizes[3] = -1;
		return;
	}

	// Copy or compute fallbacks for sizes
	OutMaxSizes[0] = MaxLODSize_Low == -1 ? OptionalMaxLODSize : MaxLODSize_Low;
	// In case none of the values are set
	if (OutMaxSizes[0] == -1)
	{
		OutMaxSizes[0] = 128;
	}
	OutMaxSizes[1] = MaxLODSize_Medium == -1 ? OutMaxSizes[0] << 1 : MaxLODSize_Medium;
	OutMaxSizes[2] = MaxLODSize_High == -1 ? OutMaxSizes[1] << 1 : MaxLODSize_High;
	OutMaxSizes[3] = MaxLODSize_Epic == -1 ? MaxLODSize : FMath::Min(MaxLODSize_Epic, MaxLODSize);

	// Ensure size[n] is not higher than size[n + 1]
	OutMaxSizes[2] = FMath::Min(OutMaxSizes[2], OutMaxSizes[3]);
	OutMaxSizes[1] = FMath::Min(OutMaxSizes[1], OutMaxSizes[2]);
	OutMaxSizes[0] = FMath::Min(OutMaxSizes[0], OutMaxSizes[1]);
}
...
{	
	...
	int32 TextureMinSize = 0;
	int32 TextureMaxSize = 0;

#if WITH_EDITORONLY_DATA
	MipGenSetting = Texture->MipGenSettings;
	TextureMinSize = Texture->GetMinTextureSize();
	TextureMaxSize = Texture->GetMaxTextureSize();
	...
	return CalculateLODBias( FMath::RoundToInt(Width), FMath::RoundToInt(Height), TextureMinSize, TextureMaxSize, Texture->LODGroup, Texture->LODBias, bIncCinematicMips ? Texture->NumCinematicMipLevels : 0, MipGenSetting, Texture->IsCurrentlyVirtualTextured(), Texture->IsStreamable());
}

int32 UTextureLODSettings::CalculateLODBias(int32 Width, int32 Height, int32 MinSize, int32 MaxSize, int32 LODGroup, int32 LODBias, int32 NumCinematicMipLevels, TextureMipGenSettings InMipGenSetting, bool bVirtualTexture, bool bCurrentlyStreamableTexture) const
{	
	...
	// Clamp the accumulated UsedLodBias to account for MinSize.
	if (MinSize > 0)
	{
		const int32 MaxLODBias = FMath::Max((int32)FMath::CeilLogTwo(FMath::Min(Width, Height)) - (int32)FMath::CeilLogTwo(MinSize), 0);
		UsedLODBias = FMath::Min(UsedLODBias, MaxLODBias);
	}
	...
	int32 MaxLODSize = LODGroupInfo.MaxLODSize;
	if (!GIsEditor)
	{
		// Non-streamable textures effectively bake in the bias once created and don't respond to changes, which means we can't apply bias that can change at runtime
		if (bCurrentlyStreamableTexture && !bVirtualTexture && TextureGroupQualityLevel >= 0 && TextureGroupQualityLevel < 4)
		{
			int32 QualityLevel = TextureGroupQualityLevel;
			if (TextureGroupMaxQualityLevel >= 0)
			{
				QualityLevel = FMath::Min(QualityLevel, TextureGroupMaxQualityLevel);
			}

			int32 MaxSizes[4];
			LODGroupInfo.ComputeMaxSizesForQualityLevels(MaxSizes);
			if (MaxSizes[QualityLevel] != -1)
			{
				MaxLODSize = MaxSizes[QualityLevel];
			}
		}
	}

	int32 MinLOD = FMath::CeilLogTwo(LODGroupInfo.MinLODSize);
	int32 MaxLOD = FMath::CeilLogTwo(MaxLODSize);
...
int32 UTextureLODSettings::CalculateNumOptionalMips(int32 LODGroup, const int32 Width, const int32 Height, const int32 NumMips, const int32 MinMipToInline, TextureMipGenSettings InMipGenSetting) const
{
	...
	int32 OptionalMaxLODSize = LODGroupInfo.OptionalMaxLODSize;
	if (TextureGroupOptionalQualityLevel >= 0 && TextureGroupOptionalQualityLevel < 4)
	{
		int32 MaxSizes[4];
		LODGroupInfo.ComputeMaxSizesForQualityLevels(MaxSizes);
		if (MaxSizes[TextureGroupOptionalQualityLevel] != -1)
		{
			OptionalMaxLODSize = MaxSizes[TextureGroupOptionalQualityLevel];
		}
	}

	int32 OptionalLOD = FMath::Min<int32>(FMath::CeilLogTwo(OptionalMaxLODSize) + 1, NumMips);
