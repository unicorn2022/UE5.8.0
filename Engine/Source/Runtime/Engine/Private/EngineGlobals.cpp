...
void ReattachMaterialInstances(const TArray<FString>& Args)
{
	...
	UE_LOGF(LogConsoleResponse, Display, "Reattach.MaterialInstances:");
	...
	for( TObjectIterator<UMaterialInstanceConstant> MaterialIt; MaterialIt; ++MaterialIt )
	{
		if(Args.Num() == 1)
		{
			UE_LOGF(LogConsoleResponse, Display, "   %ls", *MaterialIt->GetName());
...
void ReattachMaterials(const TArray<FString>& Args)
{
	...
	UE_LOGF(LogConsoleResponse, Display, "Reattach.Materials:");
...
void ReattachComponents(const TArray<FString>& Args)
{
	if(Args.Num() != 1)
	{
		UE_LOGF(LogConsoleResponse, Warning, "Reattach.Components: missing class name parameter");
		return;
	}

	UE_LOGF(LogConsoleResponse, Display, "Reattach.Components:");
		...
		Class->IsChildOf(UActorComponent::StaticClass()) )
	{
		for( FThreadSafeObjectIterator It(Class); It; ++It )
		{
			...
			if( ActorComponent )
			{
				UE_LOGF(LogConsoleResponse, Display, "   Component: %ls", 
	...
	{
		UE_LOGF(LogConsoleResponse, Warning, "Reattach.Components: No objects with the class name '%ls' found", *Args[0]);
	}
