		...
		for (auto It = IndexDelta.GetIterator(); It; ++It)
		{
			...
			if (!ChangeSet.InstanceAttributeTracker.TestFlag<FInstanceAttributeTracker::EFlag::Added>(ToIndex))
			{
				...
				if (InstanceIdIndexMap.IsValidId(InstanceId))
				{
					...
					CondMove(ProxyData.Flags.bHasPerInstanceCustomData, ProxyData.InstanceCustomData, FromIndex, ToIndex, ProxyData.NumCustomDataFloats);
					CondMove(ProxyData.Flags.bHasPerInstanceRandom, ProxyData.InstanceRandomIDs, FromIndex, ToIndex);
					CondMove(ProxyData.Flags.bHasPerInstanceLMSMUVBias, ProxyData.InstanceLightShadowUVBias, FromIndex, ToIndex);
					CondMove(ProxyData.Flags.bHasPerInstanceSkinningData, ProxyData.InstanceSkinningData, FromIndex, ToIndex);
	#if WITH_EDITOR
					CondMove(ProxyData.Flags.bHasPerInstanceEditorData, ProxyData.InstanceEditorData, FromIndex, ToIndex);
	#endif
				}
			}
		}
...
void FInstanceDataManager::SkinningDataChanged(int32 InstanceIndex)
{
	LOG_INST_DATA(TEXT("SkinningDataChanged(IDX: %d)"), InstanceIndex);
	MarkChangeHelper<EChangeFlag::SkinningDataChanged>(InstanceIndex);
}
...
bool FInstanceDataManager::FlushChanges(FInstanceDataManagerSourceDataDesc&& ComponentData)
{
	...
	if (GetState() == ETrackingState::Optimized)
	{
		...
		if (!bLoggedOnce)
		{
			UE_LOGF(LogTemp, Log, "Discarded PrecomputedOptimizationData");
			bLoggedOnce = true;
		}
#endif
	...
	ETrackingState SuccessorTrackingState = ETrackingState::Tracked;
	{
		...
		if (bNeedFullUpdate)
		{
			...
			{
				if (PrecomputedOptimizationData->PrimitiveLocalToWorldHash == FInstanceDataHelpers::HashPrimitiveLocalToWorld(ChangeSet.PrimitiveToRelativeWorld, ChangeSet.PrimitiveWorldSpaceOffset))
				{
...
void FInstanceDataManager::BeginCacheForCookedPlatformData(const ITargetPlatform* TargetPlatform, FInstanceDataManagerSourceDataDesc&& ComponentData)
{
	...
	if (bShouldBuild && ComponentData.BuildChangeSet)
	{
		...
		UE_LOGF(LogTemp, Log, "Build Instance Spatial Hashes (%.2fms), Instances: %d, Hashes: %d, Remap Size: %d", FPlatformTime::ToMilliseconds( EndtTime - StartTime ), ComponentData.NumInstances, PrecomputedOptimizationData->Hashes.Num(), PrecomputedOptimizationData->ProxyIndexToComponentIndexRemap.Num());
	}	
}
...
{
	FInstanceDataHelpers::FSpatialHashResult SpatialHashResult = FInstanceDataHelpers::BuildSpatialHashData(ChangeSet.NumSourceInstances,
	...
	Result.PrimitiveLocalToWorldHash = FInstanceDataHelpers::HashPrimitiveLocalToWorld(ChangeSet.PrimitiveToRelativeWorld, ChangeSet.PrimitiveWorldSpaceOffset);
	return Result;
}
