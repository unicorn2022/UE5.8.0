...
TArray<FText> FillViewModeDisplayNames()
{
	...
	for (int32 Index = 0; Index < VMI_Unknown + 1; ++Index)
	{
		...
		{
			ViewModeDisplayNames.Emplace(LOCTEXT("UViewModeUtils_VMI_BrushWireframe", "Wireframe only"));
		}
		// Wireframe w/ BSP
		...
		else if (ViewModeIndex == VMI_VisualizeMegaLights)
		{
			ViewModeDisplayNames.Emplace(LOCTEXT("UViewModeUtils_VMI_VisualizeMegaLights", "MegaLights Visualization"));
		}
		...
		// Streaming Deficit modes
		else if (ViewModeIndex == VMI_StreamingTextureDeficit)
		{
			ViewModeDisplayNames.Emplace(LOCTEXT("UViewModeUtils_VMI_StreamingTextureDeficit", "Streaming Texture Deficit"));
		}
		else if (ViewModeIndex == VMI_StreamingTextureResidency)
		{
			ViewModeDisplayNames.Emplace(LOCTEXT("UViewModeUtils_VMI_StreamingTextureResidency", "Streaming Texture Residency"));
		}
		else if (ViewModeIndex == VMI_StreamingMeshLODDeficit)
		{
			ViewModeDisplayNames.Emplace(LOCTEXT("UViewModeUtils_VMI_StreamingMeshLODDeficit", "Streaming Mesh LOD Deficit"));
		}
		else if (ViewModeIndex == VMI_StreamingMeshLODResidency)
		{
			ViewModeDisplayNames.Emplace(LOCTEXT("UViewModeUtils_VMI_StreamingMeshLODResidency", "Streaming Mesh LOD Residency"));
		}
...
FName GetViewModeIconName(const EViewModeIndex InViewMode)
{
	...
	static FName VisualizeMegaLightsIcon("EditorViewport.VisualizeMegaLightsMode");
