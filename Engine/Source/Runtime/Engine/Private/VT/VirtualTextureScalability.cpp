...
namespace VirtualTextureScalability
{
	...
	static TAutoConsoleVariable<int32> CVarVTMaxUploadsPerFrameStreamingInEditor(
		TEXT("r.VT.MaxUploadsPerFrameInEditor.Streaming"),
		500,
		TEXT("Max number of page uploads per frame in editor for streaming VT.")
		TEXT("Zero means no separate budget. Negative means no limit.\n"),
		ECVF_RenderThreadSafe | ECVF_Scalability
	);

	static TAutoConsoleVariable<int32> CVarMaxPagesProducedPerFrameInEditor(
		TEXT("r.VT.MaxTilesProducedPerFrameInEditor"),
		500,
		TEXT("Max number of pages that can be produced per frame when in editor"),
		ECVF_RenderThreadSafe | ECVF_Scalability
	);
	...
	static TAutoConsoleVariable<float> CVarVTMaxGatherTimePerFrame(
		TEXT("r.VT.MaxGatherTimePerFrame"),
		0,
		TEXT("Time slice limit in milliseconds used to limit time spent gathering virtual texture requests per frame.\n")
		TEXT("0 (default) disables."),
		ECVF_RenderThreadSafe | ECVF_Scalability
	);

	static TAutoConsoleVariable<float> CVarVTMaxSubmitTimePerFrame(
		TEXT("r.VT.MaxSubmitTimePerFrame"),
		0,
		TEXT("Time slice limit in milliseconds used to limit time spent submitting virtual texture requests per frame.\n")
		TEXT("0 (default) disables."),
		ECVF_RenderThreadSafe | ECVF_Scalability
	);
	...
	{
#if WITH_EDITOR
		// Vary setting in editor because we rely on reactive updates while editing.
...
#if WITH_EDITOR
		// Vary setting in editor because we rely on reactive updates while editing.
		Budget = GIsEditor ? CVarVTMaxUploadsPerFrameStreamingInEditor.GetValueOnAnyThread() : Budget;
#endif
		return (Budget < 0) ? MAX_int32 : Budget;
	}
...
#if WITH_EDITOR
		// Vary setting in editor because we rely on reactive updates while editing.
		return GIsEditor ? CVarMaxPagesProducedPerFrameInEditor.GetValueOnAnyThread() : CVarMaxPagesProducedPerFrame.GetValueOnAnyThread();
#else
	...
	{
#if WITH_EDITOR
		// Vary setting in editor because we rely on reactive updates while editing. eg GPULightmass.
	...
	float GetMaxGatherTimePerFrame()
	{
#if WITH_EDITOR
		if (GIsEditor)
		{
...
#endif
		return CVarVTMaxGatherTimePerFrame.GetValueOnAnyThread() * 0.001f;
	}

	float GetMaxSubmitTimePerFrame()
	{
#if WITH_EDITOR
		if (GIsEditor)
		{
...
#endif
		return CVarVTMaxSubmitTimePerFrame.GetValueOnAnyThread() * 0.001f;
	}
