...
namespace VirtualTexture
{
	...
	void Recreate()
	{
		UE_LOGF(LogVirtualTexturing, Display, "Recreating virtual texture pools.");
		...
		if (IMaterialCacheTagProvider* TagProvider = GetRendererModule().GetMaterialCacheTagProvider())
		{
			TagProvider->NotifyAllTagSceneInvalidation();
		}
	}

	void Recreate(TConstArrayView< TEnumAsByte<EPixelFormat> > InFormat)
	{
		UE_LOGF(LogVirtualTexturing, Display, "Recreating virtual texture pools for formats.");
		...
		for (TObjectIterator<UMaterialCacheVirtualTexture> It(RF_ClassDefaultObject, true, EInternalObjectFlags::Garbage); It; ++It)
		{
			...
			{
				if (Layout.Layers[i].StorageFormat != InFormat[i])
				{
					bFormatsMatch = false;
					break;
				}
