...
static TAutoConsoleVariable<int32> CVarVTMaxUploadMemory(
	...
	TEXT("0 or negative values disable the restriction."),
	ECVF_RenderThreadSafe
);
...
static TAutoConsoleVariable<int32> CVarMaxUploadRequests(
	...
	TEXT("0 (default) or negative values disable the restriction."),
	ECVF_RenderThreadSafe);
...
{
	const bool bSupportPersistentBuffer = (GRHISupportsDirectGPUMemoryLock && GRHISupportsUpdateFromBufferTexture);
	UploadType = bSupportPersistentBuffer ? EVTUploadType::PersistentBuffer : UploadType = EVTUploadType::StagingCopy;
}
	...
	for (int PoolIndex = 0; PoolIndex < Pools.Num(); ++PoolIndex)
	{
		...
		for (uint32 Index = 0u; Index < BatchCount; ++Index)
		{
			...
			else
			{
				...
				GraphBuilder.UseInternalAccessMode(TileEntry.Texture);
...
{
	const int32 MaxUploadRequests = CVarMaxUploadRequests.GetValueOnRenderThread();
	const uint64 MaxUploadMemoryBytes = (uint64)FMath::Max(CVarVTMaxUploadMemory.GetValueOnRenderThread(), 0) * 1024u * 1024u;

	if (MaxUploadRequests > 0 && PendingUpload.Num() + PendingRelease.Num() > MaxUploadRequests)
	{
		return false;
	}
	if (MaxUploadMemoryBytes > 0 && TileAllocator.TotalAllocatedBytes() > MaxUploadMemoryBytes)
	{
		return false;
	}
	return true;
