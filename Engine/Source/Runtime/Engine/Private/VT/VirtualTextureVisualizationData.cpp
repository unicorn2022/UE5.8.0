...
EVirtualTextureVisualizationMode FVirtualTextureVisualizationData::SetModeUsingConsoleCommand(EVirtualTextureVisualizationMode InMode)
{
	EVirtualTextureVisualizationMode PreviousMode = EVirtualTextureVisualizationMode::None;
	if (IConsoleVariable* ConsoleVariable = IConsoleManager::Get().FindConsoleVariable(GetVisualizeConsoleCommandName()))
	{
		FString PreviousModeString = ConsoleVariable->GetString();
		if (const FModeRecord* Record = ModeArray.FindByPredicate([PreviousModeString](const FModeRecord& Record) { return Record.ModeString == PreviousModeString; }))
		{
			PreviousMode = Record->ModeID;
		}
		
		FString ModeString; // Empty string will implement EVirtualTextureVisualizationMode::None
		if (const FModeRecord* Record = ModeArray.FindByPredicate([InMode](const FModeRecord& Record) { return Record.ModeID == InMode; }))
		{
			ModeString = Record->ModeString;
		}
		ConsoleVariable->AsVariable()->SetWithCurrentPriority(*ModeString);
	}
	return PreviousMode;
}
