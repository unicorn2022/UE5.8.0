...
struct FPixelDataRectangle
{
	...
	*/
	...
	void Save(FString BaseFileName, IImageWrapperModule* ImageWrapperModule)
	{
		...
		{
			UE_LOGF(LogVirtualTexturing,Warning,"Couldn't save to : %ls",*BaseFileName);
		}
	}
#endif
...
bool FVirtualTextureBuilderDerivedInfo::InitializeFromBuildSettings(const FTextureSourceData& InSourceData, const FTextureBuildSettings* InSettingsPerLayer)
{
	...
	if (BuildSettingsLayer0.PowerOfTwoMode == ETexturePowerOfTwoSetting::ResizeToSpecificResolution)
	{
			...
			(BuildSettingsLayer0.ResizeDuringBuildY && BuildSettingsLayer0.ResizeDuringBuildY < TileSize))
		{
			...
			UE_LOGF(LogVirtualTexturing, Warning, "InitializeFromBuildSettings failed : Explicit resize to smaller than tile size (%d) not supported (%d x %d) [%ls]",
		...
		for (const auto& SourceBlock : InSourceData.Blocks)
		{
			...
			if (!FMath::IsPowerOfTwo(SourceBlock.SizeX) || !FMath::IsPowerOfTwo(SourceBlock.SizeY) || SourceBlock.SizeX < TileSize || SourceBlock.SizeY < TileSize)
			{
				...
				{
					UE_LOGF(LogVirtualTexturing, Warning, "InitializeFromBuildSettings failed : Both resized width and height (%d x %d) must be set if any block is smaller than tile size (%d) [%ls]",
	...
	{
		UE_LOGF(LogVirtualTexturing,Warning,"InitializeFromBuildSettings failed : Block dimensions not power of 2 (%d x %d) [%ls]",
			BlockSizeX,BlockSizeY,*InSourceData.TextureFullName);

		return false;
	}

	// Ensure block size is at least 1 tile.
	BlockSizeScaleX = 1;
	BlockSizeScaleY = 1;
	while (BlockSizeX < TileSize) { BlockSizeX *= 2; BlockSizeScaleX *= 2; }
	while (BlockSizeY < TileSize) { BlockSizeY *= 2; BlockSizeScaleY *= 2; }
	...
	if (vLogMaxSize > VIRTUALTEXTURE_LOG2_MAX_PAGETABLE_SIZE)
	{
		...
		UE_LOGF(LogVirtualTexturing, Warning,
			"InitializeFromBuildSettings failed: VT dimensions (%lld x %lld) are too large - too many tiles (%lld x %lld) [%ls]",
			FullSizeX, FullSizeY,
	...
	{
		UE_LOGF(LogVirtualTexturing,Warning,"InitializeFromBuildSettings failed : dimensions exceed VTCanvasMaxDimension "
			"(%d x %d = %lld) (%d x %d = %lld) [%ls]",
	...
	{
		UE_LOGF(LogVirtualTexturing,Warning,"InitializeFromBuildSettings failed : total pixel count over 4G "
			"(%d x %d = %lld) [%ls]",
...
void FVirtualTextureDataBuilder::BuildLayerBlocks(FSlowTask& BuildTask, uint32 LayerIndex, const FVirtualTextureSourceLayerData& LayerData, FTextureSourceData& SourceData, FTextureSourceData& CompositeSourceData, bool bAllowAsync)
{
	...
	for (int32 BlockIndex = 0; BlockIndex < NumBlocks; ++BlockIndex)
	{
		...
		// in case image is smaller than tile size, we need PowerOfTwoMode adjusted size to correctly calculate MipBias & LocalBlockSizeScaleX/Y in a while loop below
		...
		int32 LocalBlockSizeScaleX = DerivedInfo.BlockSizeScaleX;
		int32 LocalBlockSizeScaleY = DerivedInfo.BlockSizeScaleY;
		while (AdjustedSizeX * LocalBlockSizeScaleX < TileSize || AdjustedSizeY * LocalBlockSizeScaleY < TileSize)
		{
			check(BlockData.MipBias > 0u);
			--BlockData.MipBias;
			LocalBlockSizeScaleX *= 2;
			LocalBlockSizeScaleY *= 2;
		}
		...
		if (LocalBlockSizeScaleX == 1 && LocalBlockSizeScaleY == 1)
		{
			uint32 NumMipsInTail, ExtData;
		...
		else
		{
			...
			for (const FImage& SrcMip : SourceMips)
			{
				...
				SrcMip.ResizeTo(*ScaledMip, SrcMip.SizeX * LocalBlockSizeScaleX, SrcMip.SizeY * LocalBlockSizeScaleY, SrcMip.Format, GammaSpace);
			}
...
void FVirtualTextureDataBuilder::BuildTiles(const TArray<FVTSourceTileEntry>& TileList, uint32 LayerIndex, FLayerData& GeneratedData)
{
	...
	if (bCompressionError)
	{
		...
		UE_LOGF(LogVirtualTexturing, Fatal, "Failed build tile");
	}
	else
	{
		...
		UE_LOGF(LogVirtualTexturing, Verbose, "VT CodecPayloadSize = %d TilePayloadTotalSize = %lld", CodecPayloadSize, TilePayloadTotalSize );
	}
}
...
bool FVirtualTextureDataBuilder::PushDataToChunk(const TArray<FVTSourceTileEntry>& Tiles, const TArray<FLayerData>& LayerData)
{
	...
	{
		UE_LOGF(LogVirtualTexturing,Error,"Cannot build VT; data bigger than 4 GB : %lld",TotalSize);
		return false;
	}
