...
namespace MeshPaintVirtualTexture
{
	...
	{
		return CVarMeshPaintVirtualTextureUseCompression.GetValueOnAnyThread();
	}
	...
	static void GetSceneUniformParams(IAllocatedVirtualTexture* InAllocatedVT, FUniformParams& OutParams)
	{
		...
		OutParams.PackedUniform.W = FMath::AsUInt((float)pPageSize * RcpPhysicalTextureSize);
	}
	...
	FUintVector2 GetTextureDescriptor(FTextureResource* InTextureResource, uint32 InOptionalCoordinateIndex)
	{
		...
		const uint32 PageTable32Bit = AllocatedVT->GetPageTableFormat() == EVTPageTableFormat::UInt32 ? 1 : 0;
		...
		Descriptor.Y = WidthInPages | (HeightInPages << 12) | (MaxLevel << 24) | PageTable32Bit << 28 | (InOptionalCoordinateIndex << 30);
	...
	: UTexture2D(ObjectInitializer)
{
	...
	// Mesh paint textures usually don't tile. We should default to clamping borders during texture build.
	AddressX = TA_Clamp;
	AddressY = TA_Clamp;
...
void UMeshPaintVirtualTexture::UpdateResourceWithParams(EUpdateResourceFlags InFlags)
{
	...
	// Notify that we are done updating resource
