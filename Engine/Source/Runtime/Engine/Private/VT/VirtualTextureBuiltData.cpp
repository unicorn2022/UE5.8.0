...
bool FVirtualTextureBuiltData::ValidateData(FStringView const& InDDCDebugContext, bool bValidateCompression) const
{
	...
	for (int32 ChunkIndex = 0; bResult && ChunkIndex < Chunks.Num(); ++ChunkIndex)
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogTexture, Log, "Virtual Texture %ls failed to retrieve DDC data (%ls) for chunk %d", *TextureName, *Chunk.DerivedDataKey, ChunkIndex);
				bResult = false;
				break;
			}
		...
		{
			UE_LOGF(LogTexture, Error, "Virtual Texture %ls has invalid size %lld for chunk %d", *TextureName, ChunkDataSize, ChunkIndex);
			bResult = false;
			break;
		}
		...
		{
			UE_LOGF(LogTexture, Error, "Virtual Texture %ls has invalid hash for chunk %d", *TextureName, ChunkIndex);
			bResult = false;
			break;
		}
				...
				for (uint32 LayerIndex = 0u; LayerIndex < GetNumLayers(); ++LayerIndex)
				{
					...
					if (VTCodec == EVirtualTextureCodec::ZippedGPU_DEPRECATED)
					{
						...
						if (NextTileOffset > TileOffset)
						{
							...
							{
								UE_LOGF(LogTexture, Error, "Virtual Texture %ls failed to validate compression for chunk %d", *TextureName, ChunkIndex);
								bResult = false;
								break;
							}
						}
