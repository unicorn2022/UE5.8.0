...
FString AGameMode::GetNetworkNumber()
{
	...
	return NetDriver ? NetDriver->LowLevelGetNetworkNumber() : FString();
}
...
void AGameMode::Tick(float DeltaSeconds)
{
	...
	if (GetMatchState() == MatchState::WaitingToStart)
	{
		...
		if (ReadyToStartMatch())
		{
			UE_LOGF(LogGameMode, Log, "GameMode returned ReadyToStartMatch");
			StartMatch();
		}
	}
	if (GetMatchState() == MatchState::InProgress)
	{
		...
		if (ReadyToEndMatch())
		{
			UE_LOGF(LogGameMode, Log, "GameMode returned ReadyToEndMatch");
			EndMatch();
		}
	}
}
...
{
	UE_LOGF(LogGameMode, Log, ">> GameMode::HandleSeamlessTravelPlayer: %ls ", *C->GetName());
	...
	if (PC && PC->GetClass() != PCClassToSpawn)
	{
		if (PC->Player != nullptr)
		{
			...
			if (NewPC == nullptr)
			{
				UE_LOGF(LogGameMode, Warning, "Failed to spawn new PlayerController for %ls (old class %ls)", *PC->GetHumanReadableName(), *PC->GetClass()->GetName());
				PC->Destroy();
				return;
			}
			else
...
bool AGameMode::CanServerTravel(const FString& URL, bool bAbsolute)
{
	...
	{
		UE_LOGF(LogGameMode, Log, "Not traveling because of network error");
		return false;
	}
