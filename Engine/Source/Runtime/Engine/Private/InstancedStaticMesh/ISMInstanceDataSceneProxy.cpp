...
{
	bUseLegacyRenderingPath = !UseGPUScene(ShaderPlatform);
}
...
void FISMCInstanceDataSceneProxyLegacyReordered::Update(FISMInstanceUpdateChangeSet&& ChangeSet)
{
	...
	if (bLegacyReordered && !LegacyInstanceReorderTable.IsEmpty())
	{
		...
		{
			if (ProxyData.VisibleInstances.IsValidIndex(InstanceIndex))
			{
