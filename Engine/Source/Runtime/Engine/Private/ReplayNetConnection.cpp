...
void UReplayNetConnection::LowLevelSend(void* Data, int32 CountBits, FOutPacketTraits& Traits)
{
	...
	if (CountBytes == 0)
	{
		UE_LOGF(LogDemo, Warning, "UReplayNetConnection::LowLevelSend: Ignoring empty packet.");
		return;
	}

	if (CountBytes > MAX_REPLAY_PACKET)
	{
		UE_LOGF(LogDemo, Fatal, "UReplayNetConnection::LowLevelSend: CountBytes > MAX_REPLAY_PACKET.");
	}
	...
	{
		UE_LOGF(LogDemo, Warning, "UReplayNetConnection::LowLevelSend - Missing rep context.");
	}
}
