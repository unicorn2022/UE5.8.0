...
static float GDebugLODDitherSpeed = -1.0f;
static FAutoConsoleVariableRef CVarForceLODDitherSpeed(
	TEXT("lod.DebugLODDitherSpeed"),
	GDebugLODDitherSpeed,
	TEXT("Force a LOD Dither transition with the next LOD, and have it looping. Useful to debug LOD dithering without moving the camera."),
...
bool ShouldCompileDistanceFieldShaders(EShaderPlatform ShaderPlatform)
{
	...
	return DoesPlatformSupportDistanceFields(ShaderPlatform) 
		&& IsUsingDistanceFields(ShaderPlatform) 
		&& CVarGenerateDF->GetValueOnAnyThread() != 0;
}

bool ShouldCompileDistanceFieldAOShaders(EShaderPlatform ShaderPlatform)
{
...
{
	if (GDebugLODDitherSpeed > 0.0f)
	{
		return FMath::Frac(LastRenderTime * GDebugLODDitherSpeed);
	}
...
void FRayTracingInstanceCollector::AddRayTracingInstance(int32 ViewIndex, FRayTracingInstance Instance)
{
	...
	RayTracingInstances.AddElement(MoveTemp(Tmp));
}

void FRayTracingInstanceCollector::AddReferencedGeometry(RayTracing::FGeometryHandle Geometry)
{
...
void FRayTracingInstanceCollector::AddRayTracingGeometryUpdate(int32 ViewIndex, FRayTracingDynamicGeometryUpdateParams Params)
{
	...
	RayTracingGeometriesToUpdate.AddElement(MoveTemp(UpdateRequest));
}
...
FMeshElementCollector::FMeshElementCollector(ERHIFeatureLevel::Type InFeatureLevel, FSceneRenderingBulkObjectAllocator& InBulkAllocator, ECommitFlags InCommitFlags) :
	...
	bUseGPUScene(UseGPUScene(GShaderPlatformForFeatureLevel[FeatureLevel]))
{
}
...
{
	return ComputeBoundsScreenRadiusSquared(Origin, SphereRadius, View.GetTemporalLODOrigin(SampleIndex), View.ViewMatrices.GetViewToClip());
}

float ComputeBoundsScreenRadiusSquared(const FVector4& Origin, const float SphereRadius, const FSceneView& View)
{
	return ComputeBoundsScreenRadiusSquared(Origin, SphereRadius, View.ViewMatrices.GetViewOrigin(), View.ViewMatrices.GetViewToClip());
}

float ComputeBoundsScreenSize( const FVector4& Origin, const float SphereRadius, const FSceneView& View )
{
	return ComputeBoundsScreenSize(Origin, SphereRadius, View.ViewMatrices.GetViewOrigin(), View.ViewMatrices.GetViewToClip());
}
...
{
	return ComputeBoundsScreenSize(Origin, SphereRadius, View.GetTemporalLODOrigin(SampleIndex), View.ViewMatrices.GetViewToClip());
}
...
FLODMask ComputeLODForMeshes(const TArray<class FStaticMeshBatchRelevance>& StaticMeshRelevances, const FSceneView& View, const FVector4& Origin, float SphereRadius, int32 ForcedLODLevel, float& OutScreenRadiusSquared, int8 CurFirstLODIdx, float ScreenSizeScale, bool bDitheredLODTransition)
{
	...
	if (ForcedLODLevel >= 0)
	{
		...
		// force LOD dither transition with the next LOD
		if (GDebugLODDitherSpeed > 0.0f && NumMeshes > 0 && bDitheredLODTransition && StaticMeshRelevances[0].bDitheredLODTransition && MaxLOD != 0)
		{
			int32 LOD0 = FMath::Clamp(ForcedLODLevel, MinLOD, MaxLOD);
			int32 LOD1 = FMath::Clamp(LOD0 + 1, MinLOD, MaxLOD);
			LODToRender.SetLODSample(LOD0, 0);
			LODToRender.SetLODSample(LOD1, 1);
		}
		else
		{
			...
			for (int32 SampleIndex = 0; SampleIndex < 2; SampleIndex++)
			{
				...
				for (int32 MeshIndex = NumMeshes - 1; MeshIndex >= 0; --MeshIndex)
				{
					...
					if (Mesh.ScreenSize > 0.0f && !Mesh.bOverlayMaterial)
					{
						...
						MaxLOD = FMath::Max<int32>(MaxLOD, Mesh.GetLODIndex());
					}
				}
				...
				// force LOD dither transition with the next LOD
				if (GDebugLODDitherSpeed > 0.0f && LODToRender.IsValid() && LODToRender.LODIndex0 == LODToRender.LODIndex1)
				{
					int32 LOD0 = LODToRender.LODIndex0;
					int32 LOD1 = FMath::Min(LOD0 + 1, MaxLOD);
					LODToRender.SetLODSample(LOD0, 0);
					LODToRender.SetLODSample(LOD1, 1);				
				}
			}
...
FViewUniformShaderParameters::FViewUniformShaderParameters()
{
	...
	PhysicsFieldNodesParams = GWhiteVertexBufferWithSRV->ShaderResourceViewRHI;
	PhysicsFieldNodesOffsets = GWhiteVertexBufferWithSRV->ShaderResourceViewRHI;
	PhysicsFieldTargetsOffsets = GWhiteVertexBufferWithSRV->ShaderResourceViewRHI;
	...
	ShadingEnergyGGXMaxSpecTexture = GWhiteTextureWithSRV->TextureRHI;
	...
	// Toon profiles
	ToonProfileTextureSizeAndInvSize = FVector4f(1.f, 1.f, 1.f, 1.f);
	ToonProfileDitherPatternSizeAndInvSize = FVector2f(1.f, 1.f);
	ToonProfileTexture = GBlackArrayTexture->TextureRHI;
	ToonProfileSampler = TStaticSamplerState<SF_Bilinear, AM_Clamp, AM_Clamp, AM_Clamp>::GetRHI();
...
{
	const bool bPrecomputedLightingParametersFromGPUScene = UseGPUScene(GShaderPlatformForFeatureLevel[FeatureLevel]) && bCanUsePrecomputedLightingParametersFromGPUScene;
...
void FMeshBatch::PreparePrimitiveUniformBuffer(const FPrimitiveSceneProxy* PrimitiveSceneProxy, ERHIFeatureLevel::Type FeatureLevel)
{
	...
	if (!UseGPUScene(GShaderPlatformForFeatureLevel[FeatureLevel]) || FeatureLevel == ERHIFeatureLevel::ES3_1)
	{
...
bool FMeshBatch::Validate(const FPrimitiveSceneProxy* PrimitiveSceneProxy, ERHIFeatureLevel::Type FeatureLevel) const
{
	...
	const bool bUseGPUScene = UseGPUScene(GShaderPlatformForFeatureLevel[FeatureLevel]);
