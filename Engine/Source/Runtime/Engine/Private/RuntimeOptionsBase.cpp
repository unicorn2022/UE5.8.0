...
void URuntimeOptionsBase::RegisterSupportedConsoleVariables(bool bDuringReload)
{
	...
	for (const FProperty* Property : TFieldRange<const FProperty>(GetClass()))
	{
		...
		{
			UE_LOGF(LogRuntimeOptionsBase, Error, "A conflicting CVar '%ls' already exists when trying to register a runtime option for the property %ls%ls::%ls",
				*FullyQualifiedName,
...
void URuntimeOptionsBase::PostReloadConfig(FProperty* PropertyThatWasLoaded)
{
	...
	{
		UE_LOGF(LogRuntimeOptionsBase, Verbose, "After %ls%ls::PostReloadConfig:", GetClass()->GetPrefixCPP(), *GetClass()->GetName());
		for (const FProperty* Property : TFieldRange<const FProperty>(GetClass()))
		{
			FString	Value;
			Property->ExportText_InContainer(0, Value, this, this, this, 0);

			UE_LOGF(LogRuntimeOptionsBase, Verbose, "\t%ls%ls::%ls = %ls", GetClass()->GetPrefixCPP(), *GetClass()->GetName(), *Property->GetName(), *Value);
		}
	}
}
