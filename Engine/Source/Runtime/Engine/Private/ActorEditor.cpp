...
{	
	...
	if( Info  )
	{
		UE_LOGF( LogActor, Warning, "--%ls--", Info );
	}
	else
	{
		UE_LOGF( LogActor, Warning, "--------------------------------------------------" );
	}
	UE_LOGF( LogActor, Warning, "--------------------------------------------------" );
	UE_LOGF( LogActor, Warning, "Actor [%p] (%ls)", this, *GetFName().ToString() );
	...
	{
		UE_LOGF( LogActor, Warning, "Actor has no root." );		
	}
	UE_LOGF( LogActor, Warning, "--------------------------------------------------" );
}
...
void AActor::DebugShowOneComponentHierarchy( USceneComponent* SceneComp, int32& NestLevel, bool bShowPosition )
{
	...
	if( OwnerActor )
	{
		UE_LOGF(LogActor, Warning, "%lsSceneComp [%p] (%ls) Owned by %ls %ls", *Nest, SceneComp, *SceneComp->GetFName().ToString(), *OwnerActor->GetFName().ToString(), *PosString );
	}
	else
	{
		UE_LOGF(LogActor, Warning, "%lsSceneComp [%p] (%ls) No Owner", *Nest, SceneComp, *SceneComp->GetFName().ToString() );
	}
	if( SceneComp->GetAttachParent())
	{
		...
		UE_LOGF(LogActor, Warning, "%lsAttachParent [%p] (%ls) %ls", *Nest, SceneComp->GetAttachParent(), *SceneComp->GetAttachParent()->GetFName().ToString(), *PosString );
	}
	else
	{
		UE_LOGF(LogActor, Warning, "%ls[NO PARENT]", *Nest );
	}
	...
	{
		UE_LOGF(LogActor, Warning, "%ls[NO CHILDREN]", *Nest );
	}
}
...
bool AActor::InternalPostEditUndo()
{
	...
	if (OldClass->HasAnyClassFlags(CLASS_NewerVersionExists))
	{
		...
		{
			UE_LOGF(LogActor, Warning, "WARNING: %ls is out of date and is the same as its AuthoritativeClass during PostEditUndo!", *OldClass->GetName());
		};
...
void AActor::EditorApplyTranslation(const FVector& DeltaTranslation, bool bAltDown, bool bShiftDown, bool bCtrlDown)
{
	...
	{
		UE_LOGF(LogActor, Warning, "WARNING: EditorApplyTranslation %ls has no root component", *GetName() );
	}
}
...
void AActor::EditorApplyRotation(const FRotator& DeltaRotation, bool bAltDown, bool bShiftDown, bool bCtrlDown)
{
	...
	{
		UE_LOGF(LogActor, Warning, "WARNING: EditorApplyRotation %ls has no root component", *GetName() );
	}
}
...
void AActor::EditorApplyMirror(const FVector& MirrorScale, const FVector& PivotLocation)
{
	...
	if( RootComponent != NULL )
	{
		GetRootComponent()->SetWorldRotation(NewRot.Rotator());
	...
	{
		UE_LOGF(LogActor, Warning, "WARNING: EditorApplyMirror %ls has no root component", *GetName() );
	}
}
...
void AActor::SetActorLabel(const FString& NewActorLabelDirty, bool bMarkDirty)
{
	...
	{
		//Invalid actor name
		UE_LOGF(LogActor, Warning, "SetActorLabel failed: %ls", *OutErrorMessage.ToString());
	}
...
void AActor::FixupActorFolder()
{
	...
	if (!IsUsingActorFolders(this))
	{
		if (FolderGuid.IsValid())
		{
			UE_LOGF(LogActor, Warning, "Actor folder %ls for actor %ls encountered when not using actor folders", *FolderGuid.ToString(), *GetName());
			FolderGuid = FGuid();
		}
	}
	else
	{
		...
		{
			UE_LOGF(LogActor, Warning, "Actor folder path %ls for actor %ls encountered when using actor folders", *FolderPath.ToString(), *GetName());
...
ELevelInstanceType AActor::GetLevelInstanceType() const
{
	// ChildActorComponent Actors forward the call their ParentActor 
	if (AActor* ParentActor = GetParentActor())
	{
		return ParentActor->GetLevelInstanceType();
	}

	return LevelInstanceType;
}

ELevelInstanceFlags AActor::GetLevelInstanceFlags() const
{
	// ChildActorComponent Actors forward the call their ParentActor 
	if (AActor* ParentActor = GetParentActor())
	{
		return ParentActor->GetLevelInstanceFlags();
	}

	return LevelInstanceFlags;
}
...
void AActor::ForceLoadDataLayerAssets() const
{
...
TArray<FName> AActor::GetDataLayerInstanceNames() const
{
	...
	{
		DataLayerInstanceNames.Add(DataLayerInstance->GetFName());
	}
...
void AActor::FixupDataLayers(bool bRevertChangesOnLockedDataLayer /*= false*/)
{
	...
	if (IsTemplate())
	{
		// Fixup old data: Template actors should never have an ExternalDataLayerAsset
	...
	if (IsRunningCookCommandlet())
	{
		...
		Algo::Transform(CleanedUpDataLayers, DataLayerAssets, [](const UDataLayerAsset* DataLayerAsset) { return TSoftObjectPtr<UDataLayerAsset>(const_cast<UDataLayerAsset*>(DataLayerAsset)); });
	}
}
...
bool FAssignActorDataLayer::AddDataLayerAsset(AActor* InActor, const UDataLayerAsset* InDataLayerAsset)
{
	...
	if (const UExternalDataLayerAsset* ExternalDataLayerAsset = Cast<UExternalDataLayerAsset>(InDataLayerAsset))
	{
		...
		UE_CLOGF(InActor->ExternalDataLayerAsset != ExternalDataLayerAsset, LogActor, Warning, "Trying to assign external data layer %ls on actor %ls while %ls is already assigned.", 
