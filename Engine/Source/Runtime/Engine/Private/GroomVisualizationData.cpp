...
EGroomViewMode GetGroomViewMode(const FSceneView& View)
{
	...
	if (IsGroomEnabled())
	{
		...
		if (CVarGroomViewMode)
		{
			...
			case 41: return EGroomViewMode::Bounds;
			default: break;
			}
		}
...
const TCHAR* GetGroomViewModeName(EGroomViewMode In)
{
	...
	case EGroomViewMode::Bounds:					return TEXT("Bounds");
	}
	return TEXT("None");
}
...
void FGroomVisualizationData::Initialize()
{
	...
	if (!bIsInitialized && IsGroomEnabled())
	{
		...
		AddVisualizationMode(ModeMap, true, EGroomViewMode::Bounds,						LOCTEXT("Bounds", "Bounds"),								LOCTEXT("BoundsDesc", "Bounds"));
