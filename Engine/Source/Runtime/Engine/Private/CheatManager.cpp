...
{
	UE_LOGF(LogCheatManager, Log, "Fabricate %ls", *ClassName );
	...
	if ( !bSpawnedActor )
	{
		UE_LOGF(LogCheatManager, Warning, "Failed to summon %ls. Reason: %ls", *ClassName, *FailureReason);
	}
}
	...
	for (FActorIterator It(GetWorld()); It; ++It)
	{
		...
		if (IsValid(A))
		{
			if ( A->GetFName() == ActorName )
			{
				...
				static FName NAME_ThirdPerson = FName("ThirdPerson");
...
{
	return FDebugCameraManager::Get().IsDebugCameraActive(GetOuterAPlayerController());
}

void UCheatManager::EnableDebugCamera()
{
	FDebugCameraManager::Get().EnableDebugCamera(GetOuterAPlayerController());
}

void UCheatManager::DisableDebugCamera()
{
	FDebugCameraManager::Get().DisableDebugCamera(GetOuterAPlayerController());
}
		...
		if(PC)
		{
			...
			if (bDebugCapsuleSweep)
			{
				...
				if(bHit)
				{
					...
					UE_LOGF(LogCollision, Log, "Collision component (%ls) : Actor (%ls)", *GetNameSafe(Result.Component.Get()), *Result.HitObjectHandle.GetName());
				}
			}
		}
	}
	...
	if(PC)
	{
		...
		for (FThreadSafeObjectIterator Iter(AVolume::StaticClass()); Iter; ++Iter)
		{
			...
			if (Volume->GetClass()->GetDefaultObject() != Volume)
			{
				...
				UE_LOGF(LogCheatManager, Log, "Distance to (%ls) is %0.2f", *Volume->GetName(), Distance);
			}
		}
	}
#endif
...
{
	UE_LOGF(LogCheatManager, Display, "");
	UE_LOGF(LogCheatManager, Display, "-------------------------------------------------------------");
	UE_LOGF(LogCheatManager, Display, "");
...
{
	UE_LOGF(LogCheatManager, Log,  "BugItGo to: %ls %ls", *TheLocation.ToString(), *TheRotation.ToString() );
...
void UCheatManager::CheatScript(FString ScriptName)
{
	...
	{
		UE_LOGF(LogCheatManager, Warning, "Can't find section 'CheatScript.%ls' in DefaultGame.ini", *ScriptName);
	}
}
...
void UCheatManager::DestroyServerStatReplicator()
{
	...
	{
		UE_LOGF(LogCheatManager, Log, "Destroying stat replicator (%ls)", *GameModeBase->ServerStatReplicator->GetName());
