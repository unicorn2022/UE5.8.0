...
void FNetworkProfiler::EnableTracking( bool bShouldEnableTracking )
{
	...
	{
		UE_LOGF(LogNet, Log, "Network Profiler: ENABLED");
	}
...
void FNetworkProfiler::SetCurrentConnection( UNetConnection* Connection )
{
	if ( bIsTrackingEnabled && Connection != nullptr )
	{
		...
		{
			static const FString InvalidConnection = FString("Invalid Connection");
...
void FNetworkProfiler::TrackSessionChange( bool bShouldContinueTracking, const FURL& InURL )
{
	...
	{
		UE_LOGF( LogNet, Log, "Network Profiler: TrackSessionChange.  InURL: %ls", *InURL.ToString() );
		...
		if( FileWriter )
		{	
			UE_LOGF( LogNet, Log, "Network Profiler: Closing session file for '%ls'", *CurrentHeader.GetURL() );

			if ( !bHasNoticeableNetworkTrafficOccured )
			{
				UE_LOGF( LogNet, Warning, "Network Profiler: Nothing important happened" );
			}
		...
		if( bShouldContinueTracking )
		{
			...
			UE_LOGF(LogNet, Log, "Network Profiler: Creating session file at location %ls", *FinalFileName);
...
bool FNetworkProfiler::Exec( UWorld * InWorld, const TCHAR* Cmd, FOutputDevice & Ar )
{
	...
	if ( bShouldTrackingBeEnabled && FileWriter == nullptr ) 
	{
		...
		{
			UE_LOGF(LogNet, Warning, "FNetworkProfiler::Exec: FAILED to create file writer!");
			EnableTracking( false );
		}
	}
