...
void AGameState::ReceivedGameModeClass()
{
	...
	{
		UE_LOGF(LogGameState, Error, "Mixing AGameState with AGameModeBase is not compatible. Change AGameModeBase subclass (%ls) to derive from AGameMode, or make both derive from Base", *GameModeClass->GetName());
	}
}
