...
#endif
			{
				UE_LOGF(LogDataTable, Warning, "%ls", *ErrorMsg.ToString());
			}
		}
		bLeaveEmpty = true;
		...
		for (const UDataTable* ParentTable : ParentTables)
		{
			...
			if (ParentTable->RowStruct != RowStruct)
			{
				if (bWarnOnInvalidChildren)
				{
					...
					UE_LOGF(LogDataTable, Error, "Composite tables must have the same row struct as their parent tables. Composite Table: %ls, Composite Row Struct: %ls, Parent Table: %ls, Parent Row Struct: %ls.", *GetName(), *CompositeRowStructName, *ParentTable->GetName(), *ParentRowStructName);
				}
				continue;
			}
...
const TArray<UDataTable*>& UCompositeDataTable::GetParentTables() const
{
	return ParentTables;
}
