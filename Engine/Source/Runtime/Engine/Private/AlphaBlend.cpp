...
FAlphaBlend::FAlphaBlend(float NewBlendTime) 
	: CustomCurve(nullptr)
	, BlendTime(FMath::Max(NewBlendTime, 0.0f))
	, BeginValue(0.0f)
	, DesiredValue(1.0f)
...
void FAlphaBlendArgs::PostSerialize(const FArchive& Ar)
{
	if (Ar.IsLoading() && Ar.CustomVer(FUE5ReleaseStreamObjectVersion::GUID) < FUE5ReleaseStreamObjectVersion::AlphaBlendNegativeBlendTimeGuard)
	{
		BlendTime = FMath::Max(BlendTime, 0.0f);
	}
}
...
void FAlphaBlend::PostSerialize(const FArchive& Ar)
{
	if (Ar.IsLoading() && Ar.CustomVer(FUE5ReleaseStreamObjectVersion::GUID) < FUE5ReleaseStreamObjectVersion::AlphaBlendNegativeBlendTimeGuard)
	{
		BlendTime = FMath::Max(BlendTime, 0.0f);
...
FAlphaBlend::FAlphaBlend(const FAlphaBlendArgs& InArgs)
	...
	, BlendTime(FMath::Max(InArgs.BlendTime, 0.0f))
	, BeginValue(0.0f)
	, DesiredValue(1.0f)
...
FAlphaBlendArgs::FAlphaBlendArgs(const struct FAlphaBlend& InAlphaBlend)
	...
	, BlendTime(FMath::Max(InAlphaBlend.GetBlendTime(), 0.0f))
