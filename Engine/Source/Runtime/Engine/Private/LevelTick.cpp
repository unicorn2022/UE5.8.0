...
TRACE_DECLARE_INT_COUNTER(UObjectCount, TEXT("UObject Count"));
...
void FDetailedTickStats::DumpStats()
{
	...
	&& ((FApp::GetCurrentTime() - LastTimeOfLogDump) > MinTimeBetweenLogDumps) )
	{
		...
		UE_LOGF(LogLevel, Log, "Per object stats, frame # %llu", (uint64)GFrameCounter);
		for( int32 i=0; i<SortedTickStats.Num(); i++ )
		{
			const FTickStats& TickStats = SortedTickStats[i];
			if( i<NumObjectsToReport )
			{
				UE_LOGF(LogLevel, Log, "%5.2f ms, %4i instances, avg cost %5.3f, %ls", 1000 * TickStats.TotalTime, TickStats.Count, (TickStats.TotalTime/TickStats.Count) * 1000, *TickStats.ObjectPathName ); 
			}
			Totals.TotalTime += TickStats.TotalTime;
			Totals.Count	 += TickStats.Count;
		}
		UE_LOGF(LogLevel, Log, "Total time spent %ls %4i instances: %5.2f", *OperationPerformed, Totals.Count, Totals.TotalTime * 1000 );
		...
		if (GLogDetailedDumpStats)
		{
			...
			UE_LOGF(LogLevel, Log, "Detailed object stats, frame # %i", GFrameCounter);
			for( int32 i=0; i<SortedTickStatsDetailed.Num(); i++ )
			{
				...
				{
					UE_LOGF(LogLevel, Log, "avg cost %5.3f, %ls %ls",(TickStats.TotalTime/TickStats.Count) * 1000, *TickStats.ObjectPathName, *TickStats.ObjectDetailedInfo ); 
				}
				Totals.TotalTime += TickStats.TotalTime;
				Totals.Count	 += TickStats.Count;
			}
...
public:
	...
	FVisibleLevelStreamingSettings( EStreamingVolumeUsage Usage )
	{
		...
		case SVB_BlockingOnLoad:
			...
			break;
		default:
			UE_LOGF(LogLevel, Fatal,"Unsupported usage %i",(int32)Usage);
		}
	}
...
void UWorld::ProcessLevelStreamingVolumes(FVector* OverrideViewLocation)
{
	...
	{
		if (Iterator->GetResidence() != EResidence::Remote)
		{
...
void UWorld::SendAllEndOfFrameUpdatesInternal(EEndOfFrameUpdateLocation Location)
{
	...
	// Process OnPreEndOfFrameSync BEFORE moving the component arrays into the context.
	// OnPreEndOfFrameSync can trigger WaitUntilTaskCompletes (e.g. cloth simulation), which pumps
	// game-thread tasks. Those tasks can call MarkActorComponentForNeededEndOfFrameUpdate, which
	// validates that already-marked components still exist at their stored array index. If we moved
	// the arrays first, those indices would point into the moved-away (context-owned) data, causing
	// a check failure. Processing sync first keeps the arrays in place and indices valid.
	//
	// We use a draining loop because OnPreEndOfFrameSync (or tasks pumped during it) may mark
	// additional components for sync. The TSet is copied to a local TArray for safe iteration,
	// then drained until no new sync components remain.
	{
		auto DrainSyncSet = [](TSet<TObjectPtr<UActorComponent>>& SyncSet)
		{
			while (!SyncSet.IsEmpty())
			{
				TSet<TObjectPtr<UActorComponent>> LocalSync = MoveTemp(SyncSet);
				SyncSet.Reset();

				for (UActorComponent* Component : LocalSync)
				{
					if (Component)
					{
		...
		DrainSyncSet(ComponentsThatNeedPreEndOfFrameSync_AsyncTick);

		if (!bAsyncInTick)
		{
			DrainSyncSet(ComponentsThatNeedPreEndOfFrameSync);
		}
	}
...
static void RecordWorldCountsToCSV(UWorld* World, bool bDoingActorTicks)
{
	if(FCsvProfiler::Get()->IsCapturing())
	{
		...
		if (CVarRecordActorCountsToCSV.GetValueOnAnyThread())
		{
			...
			static FName TotalActorCountStatName("TotalActorCount");
		...
		static const FName TotalObjectCountStatName("Total");
...
void UWorld::Tick( ELevelTick TickType, float DeltaSeconds )
{
	...
	TRACE_COUNTER_SET(UObjectCount, GUObjectArray.GetObjectArrayNumMinusAvailable());
	...
	if(ThisFramePawnSpawns.Num() > 1 && IsGameWorld() && !GIsServer && GEngine->bCheckForMultiplePawnsSpawnedInAFrame )
	{
		...
		UE_LOGF(LogLevel, Warning, "%ls", *WarningMessage );
		// print out the pawns that were spawned
		for(int32 i=0; i<ThisFramePawnSpawns.Num(); i++)
		{
			UE_LOGF(LogLevel, Warning, "%ls", *ThisFramePawnSpawns[i]);
		}
...
void UWorld::CleanupActors()
{
	...
	{
		// Don't compact actors array for levels that are currently in the process of being made visible or invisible
		// as the code that spreads this work across several frames relies on the actor count not changing as it keeps
