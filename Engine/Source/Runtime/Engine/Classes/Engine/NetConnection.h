...
enum { MAX_PACKET_RELIABLE_SEQUENCE_HEADER_BITS = 32 /*PackedHeader*/ + 16 /*HistorySequence*/ + FNetPacketNotify::SequenceHistoryT::MaxSizeInBits };
...
private:
	...
	/** 
	 * Cache a pointer to an instance of ANoPawnPlayerController to avoid having to re-spawn it when changing this 
	 * connection from a normal connection with a pawn and player controller to a no pawn connection.
	 *
	 * This is currently only used by APlayerController::PostMigrate() when migrating players between servers.
	 */
	UPROPERTY(transient)
	TObjectPtr<class ANoPawnPlayerController> CachedNoPawnPlayerController;
...
public:
	...
	FTransactionallySafeCriticalSection& GetSharedNetTickMutex();
...
public:
	...
	/** Called by PlayerController to tell connection about client container visibility change */
	ENGINE_API void UpdateContainerVisibility(UObject* Container, bool bVisible);
...
public:
	...
	*/
	...
	/** Set to true if this connection's remote endpoint is a proxy. */
	ENGINE_API virtual void SetProxyConnection(bool bEnabled);
	ENGINE_API virtual bool IsProxyConnection() const;

private:
	void RestoreRemappedChannel(const int32 ChIndex);

	bool bIsProxyConnection = false;
...
private:
	...
	/** Called by PlayerController to tell connection about client container visibility change */
	void UpdateContainerVisibilityInternal(UObject* Container, bool bVisible);
	...
	*/
	...
	// Required by APlayerController::PostMigrate() to read and update CachedNoPawnPlayerController.
