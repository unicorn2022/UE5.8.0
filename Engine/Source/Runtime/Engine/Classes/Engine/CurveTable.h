	...
	, public FCurveOwnerInterface
{
	...
	DECLARE_TS_MULTICAST_DELEGATE(FOnCurveTableChanged);
	...
	ENGINE_API void RenameRow(const FName& CurveName, const FName& NewCurveName);

	/** Remove a curve row from the table.  Note the associated curve will be deleted. */
	ENGINE_API void DeleteRow(const FName& CurveName);
...
public:
	...
	/** Interpolation mode persisted for reimport. */
	UPROPERTY(VisibleAnywhere, Category=ImportSettings)
	TEnumAsByte<ERichCurveInterpMode> ImportCurveInterpMode = RCIM_None;
	...
	FRealCurve* FindCurve(FName RowName, const FString& ContextString, bool bWarnIfNotFound = true) const
	{
		if (RowName.IsNone())
		{
			UE_CLOGF(bWarnIfNotFound, LogCurveTable, Warning, "UCurveTable::FindCurve : NAME_None is invalid row name for CurveTable '%ls' (%ls).", *GetPathName(), *ContextString);
			return nullptr;
		}
		...
		{
			UE_CLOGF(bWarnIfNotFound, LogCurveTable, Warning, "UCurveTable::FindCurve : Row '%ls' not found in CurveTable '%ls' (%ls).", *RowName.ToString(), *GetPathName(), *ContextString);
			return nullptr;
		}
	...
	FRichCurve* FindRichCurve(FName RowName, const FString& ContextString, bool bWarnIfNotFound = true) const
	{
		...
		{
			UE_LOGF(LogCurveTable, Error, "UCurveTable::FindCurve : Using FindRichCurve on CurveTable '%ls' (%ls) that is storing simple curves.", *GetPathName(), *ContextString);
			return nullptr;
		}
	...
	FSimpleCurve* FindSimpleCurve(FName RowName, const FString& ContextString, bool bWarnIfNotFound = true) const
	{
		...
		{
			UE_LOGF(LogCurveTable, Error, "UCurveTable::FindCurve : Using FindSimpleCurve on CurveTable '%ls' (%ls) that is storing rich curves.", *GetPathName(), *ContextString);
			return nullptr;
		}
