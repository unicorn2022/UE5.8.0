...
public:
	...
	/** 
	 * Removes scan paths. 
	 * Used by the GameFeaturePlugins system to reset the database to the previous state when plugin unregisters.
	 * @param PrimaryAssetType Type of asset to look for
	 * @param Paths				List of file system paths to remove
	 * @param BaseClass			Base class of all loaded assets, must match the type that were previously added
	 * @param bHasBlueprintClasses	Flag, must match what was previously added
	 * @param bIsEditorOnly		Flag, must match what was previously added
	 */
	...
	/** 
	 * Removes the whole primary asset type. 
	 * Used by GameFeaturePlugins system to remove types, that are exclusive to the plugin itself.
	 * @param PrimaryAssetType Type of asset to look for
	 */
	ENGINE_API virtual void RemovePrimaryAssetType(FPrimaryAssetType PrimaryAssetType);

	/** 
	 * Removes tracking of primary assets from the management database.
	 * Used by GameFeaturePlugins system to unregister plugin's primary assets.
	 * @param PrimaryAssetType Type of asset to look for
	 * @param RootPath Mount point path, containing leading and tail path separators "/".
	 */
	ENGINE_API virtual void RemovePrimaryAssetsForTypeInMountPoint(FPrimaryAssetType PrimaryAssetType, FStringView RootPath);
...
public:
	...
	/** Gets list of all FPrimaryAssetId for a primary asset type beginning with a specific path */
	ENGINE_API virtual void GetPrimaryAssetsInPath(FPrimaryAssetType PrimaryAssetType, FStringView Path, TArray<FPrimaryAssetId>& PrimaryAssetIdList) const;
	...
	TSharedPtr<FStreamableHandle> DownloadAssetList(
		const TArray<FSoftObjectPath>& AssetList,
		FAssetManagerLoadParams&& LoadParams,
		FString&& DebugName = FString("DownloadAssetList"),
		UE::FSourceLocation&& Location = UE::FSourceLocation::Current());
	/** rvalue reference overload for Asset List */
