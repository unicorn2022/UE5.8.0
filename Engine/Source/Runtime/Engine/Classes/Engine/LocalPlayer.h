...
public:
	...
	/** The UGameViewportClient for the UGameInstance that this player belongs to. */
	UPROPERTY()
	TObjectPtr<class UGameViewportClient> ViewportClient;

	/** The primary viewport containing this player's view (different from ViewportClient if override is set). */
	ENGINE_API UScriptViewportClient* GetViewportClient() const;

	/** Declares that a different viewport client will render this player.  Pass nullptr to render in the primary viewport. */
	ENGINE_API void OverrideViewportClient(class UScriptViewportClient* InViewportClient);
...
private:
	...
	UPROPERTY()
	TObjectPtr<UScriptViewportClient> ViewportClientOverride;
