...
BEGIN_GLOBAL_SHADER_PARAMETER_STRUCT(FInstancedStaticMeshVFLooseUniformShaderParameters, ENGINE_API)
	SHADER_PARAMETER_STRUCT_INCLUDE(FLocalVertexFactoryInstanceDitherParameters, InstanceDitherParameters)
...
public:
	...
	~FInstancedStaticMeshSceneProxy() = default;
	...
	ENGINE_API virtual bool GetInstancingFadeOutParams(FVector2f& OutInstancingFadeOutParams) const override;
...
protected:
	ENGINE_API FLocalVertexFactoryInstanceDitherParameters GetInstanceDitherParameters(const FSceneView* View, uint32 InstancedLODRange, uint32 InstancedLODIndex) const;
	ENGINE_API FVector2f CalcInstancingFadeOutParams() const;
	ENGINE_API FInstancedStaticMeshVFLooseUniformShaderParametersRef CreateLooseUniformBuffer(const FSceneView* View, uint32 InstancedLODRange, uint32 InstancedLODIndex, EUniformBufferUsage UniformBufferUsage) const;
	...
	/** Helper function to get the number of VFs in the render data */
	int32 GetNumRenderDataVertexFactories() const;
...
public:
	...
	{
		return FString("InstancedSMLightingMapping");
	}
