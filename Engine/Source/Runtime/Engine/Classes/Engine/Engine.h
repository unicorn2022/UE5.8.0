...
struct FNetDriverDefinition
{
	...
	/** Runs the Server Net Connection Tick in parallel */
	UPROPERTY()
	bool bRunParallelConnectionTick;
	...
	FNetDriverDefinition() :
		...
		bRunParallelConnectionTick(false)
	{
	}
};
...
struct FIrisNetDriverConfig 
{
	...
	/**
	 * Sets the preferred replication system for this NetDriver. This is the one that will be used unless it's overridden
	 * by bCanUseIris, UGameInstance::GetDesiredReplicationSystem, or AGameModeBase::GameNetDriverReplicationSystem.
	 */
	UPROPERTY()
	EReplicationSystem PreferredReplicationSystem = EReplicationSystem::Default;

	/**
	 * If true, and Iris is used as the replication system then NetConnection::Tick can be run in parallel on the server for this NetDriver
	 */
	UPROPERTY()
	bool bCanUseParallelNetConnectionTick = false;
};
...
public:
	...
	/** Use to specify the navigation system config override class used in UEFN. */
	UPROPERTY(globalconfig, meta = (MetaClass = "/Script/NavigationSystem.NavSystemConfigOverride", DisplayName = "SceneGraph Navigation System Override Class Name"))
	FSoftClassPath SceneGraphNavigationSystemOverrideClassName;

	/** Use to specify the navmesh bounds volume class used in UEFN. */
	UPROPERTY(globalconfig, meta = (MetaClass = "/Script/NavigationSystem.NavMeshBoundsVolume", DisplayName = "SceneGraph NavMesh Bounds Volume Class Name"))
	FSoftClassPath SceneGraphNavMeshBoundsVolumeClassName;
	...
	/** The colors used for streaming deficit debug view modes (green=fully streamed, red=high deficit). */
	UPROPERTY(globalconfig)
	TArray<FLinearColor> StreamingDeficitColors;
	...
	*/
	...
	/** Texture used for maximum/peak value of GGX specular reflection lobe */
	UPROPERTY()
	TObjectPtr<class UTexture2D> GGXMaxSpecEnergyTexture;
	/** Path of the texture used for maximum/peak value of GGX specular reflection lobe */
	UPROPERTY(globalconfig)
	FSoftObjectPath GGXMaxSpecEnergyTextureName;
	...
	/** Hair DualScattering (3D volume texture) */
	UPROPERTY()
	TObjectPtr<class UVolumeTexture> HairDualScatteringTexture;

	/** Path of the hair DualScattering volume texture */
	UPROPERTY(globalconfig)
	FSoftObjectPath HairDualScatteringTextureName;

	/** Hair Directional Alebdo (3D volume texture) */
	UPROPERTY()
	TObjectPtr<class UVolumeTexture> HairDirectionalAlbedoTexture;

	/** Path of the hair Directional Alebdo volume texture */
	UPROPERTY(globalconfig)
	FSoftObjectPath HairDirectionalAlbedoTextureName;

	/** Hair Coverage (2D texture) */
	UPROPERTY()
	TObjectPtr<class UTexture2D> HairCoverageTexture;

	/** Path of the hair Coverage texture */
	UPROPERTY(globalconfig)
	FSoftObjectPath HairCoverageTextureName;
...
public:
	...
	*/
	...
	/** Delay loading Hair LUT textures until they are needed by the renderer. */
	ENGINE_API void LoadHairLUTTextures();
...
public:
	...
	/**
	 * Disables garbage collection until the time limit or free memory threshold are exceeded, or ResumeGarbageCollection is called
	 * Cvars gc.PauseGCTimeLimitSeconds and gc.PauseGCFreeMemThresholdMB can be used to specify time and memory thresholds for
	 * auto-resume.
	 *
	 */
	ENGINE_API void PauseGarbageCollection();

	/**
	 * Re-enables garbage collection following a call to PauseGarbageCollection
	 *
	 * @return true if garbage collection was paused or false otherwise
	 */
	ENGINE_API bool ResumeGarbageCollection();

	/**
	 * Checks if GC is paused
	 *
	 * @return true if garbage collection is paused or false otherwise
	 */
	ENGINE_API bool IsGarbageCollectionPaused();
...
public:
	...
	/** Returns true if the netdriver will run server NetConnection::Tick in parallel (only when using Iris) */
	ENGINE_API bool WillNetDriverUseParallelNetConnectionTick(const FWorldContext& Context, const FName InNetDriverDefinition, const FName InNetDriverName) const;
	...
	/** Register a viewport to be drawn each frame. */
	ENGINE_API void RegisterViewport(TSharedRef<FViewport> Viewport);

	/** Unregister a viewport to stop drawing to it. */
	ENGINE_API void UnregisterViewport(TSharedRef<FViewport> Viewport);

	/**
	 * Redraws all viewports.
	 * @param	bShouldPresent	Whether we want this frame to be presented
	 */
	ENGINE_API virtual void RedrawViewports(bool bShouldPresent = true);
...
private:
	...
	ENGINE_API bool ToggleStatThermals(UWorld* World, FCommonViewportClient* ViewportClient, const TCHAR* Stream = nullptr);
#if !UE_BUILD_SHIPPING
	...
	ENGINE_API int32 RenderStatThermals(UWorld* World, FViewport* Viewport, FCanvas* Canvas, int32 X, int32 Y, const FVector* ViewLocation = nullptr, const FRotator* ViewRotation = nullptr);
...
protected:
	/** Viewports to draw each frame. */
	TArray<TSharedRef<FViewport>> ActiveViewports;
	UE::FMutex ActiveViewportsMutex;
};
