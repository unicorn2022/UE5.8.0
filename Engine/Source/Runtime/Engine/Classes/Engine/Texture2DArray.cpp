...
UTexture2DArray* UTexture2DArray::CreateTransient(int32 InSizeX, int32 InSizeY, int32 InArraySize, EPixelFormat InFormat, const FName InName)
{
	...
	{
		UE_LOGF(LogTexture, Warning, "Invalid parameters specified for UTexture2DArray::CreateTransient()");
	}
	return NewTexture;
}
...
ENGINE_API bool UTexture2DArray::UpdateSourceFromSourceTextures(bool bCreatingNewTexture)
{
	...
	if (SourceTextures.Num() > 0)
	{
		...
		if (bMismatchedSize)
		{
			UE_LOGF(LogTexture, Log, "Mismatched sizes of the source textures, resizing all the array elements to %dx%d%ls ...",
		...
		{
			UE_LOGF(LogTexture, Log, "Mismatched source gamma spaces, converting all to RGBA32F/Linear ...");
		}
		else if (bMismatchedFormats)
		{
			UE_LOGF(LogTexture, Log, "Mismatched source pixel formats, converting all to %ls/%ls ...",
