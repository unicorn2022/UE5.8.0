...
#ifndef UE_ENABLE_STREAMABLE_JIT_ASYNC_LOADING
#define UE_ENABLE_STREAMABLE_JIT_ASYNC_LOADING 0
#endif

class IStreamableAsyncLoader;
...
struct FStreamableHandle : public TSharedFromThis<FStreamableHandle>
{
	...
	bool IsUsingJustInTimeAsyncLoader()
	{
		return bUseJustInTimeAsyncLoader;
	}
	...
	/**
	 * Set priority (useful to update remaining async loads to a different priority than the handle was created with).
	 * This only takes affect when using just in time async loading.
	 * @see bUseJustInTimeAsyncLoader
	 */
	ENGINE_API void SetPriority(TAsyncLoadPriority InPriority);
...
private:
	...
	/** If true, use the JIT async loader when queueing requests */
	bool bUseJustInTimeAsyncLoader = false;
};
...
struct FStreamableAsyncLoadParams
{
	...
	/** [optional] If true, async load requests will be trickled to the async loader instead of queueing all at once, while continuing to queue subsequent requests as processed on the AsyncLoadingThread */
	/** Useful for not overloading the async loading queue and if improved control over prioritization/cancellation is needed (remaining requests are queued at updated handle priorities and only if necessary/not cancelled) */
	bool bUseJustInTimeAsyncLoader = false;
...
private:
	...
	struct FStreamable* StreamInternal(const FSoftObjectPath& Target, TAsyncLoadPriority Priority, TSharedRef<FStreamableHandle> Handle, TSharedRef<IStreamableAsyncLoader> AsyncLoader);

	void CancelJITLoadsRecursively(TSharedRef<FStreamableHandle> Handle);
