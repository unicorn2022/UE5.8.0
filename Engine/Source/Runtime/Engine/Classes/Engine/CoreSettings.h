...
/** Whether to kick off incremental GC when there are over the specified amount of actors (from levels) still waiting to be purged. */
extern ENGINE_API int32 GLevelStreamingContinuouslyIncrementalGCWhileActorsPendingPurge;
...
/** In a low memory situation, will kick off a 'soft' GC if the number of actors (from streaming levels) pending purge meets or exceeds this count. */
extern ENGINE_API int32 GLevelStreamingLowMemoryActorsPendingPurgeCount;
