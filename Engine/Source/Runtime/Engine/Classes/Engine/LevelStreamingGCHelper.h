...
struct FLevelStreamingGCHelper
{
	...
	/**
	 * @return	The number of actors (from levels) pending a purge by the garbage collector
	 */
	static ENGINE_API int32 GetNumActorsPendingPurge();

	/**
	 * Trash a package, making sure it will be reloaded if needed again by level streaming
	 */
	UE_DEPRECATED(5.8, "TrashPackage is deprecated.")
...
private:
	...
	/** The number of unloaded actors prepared for the next GC (used by GetNumActorsPendingPurge). */
	static int32 NumberOfPreparedStreamedOutActorsForGC;

	/** The number of actors that should be unloaded */
	static int32 ActorsPendingUnload;
};	
