...
public:
	...
	/** set to true if we want to split the render of each player view into separate Viewports/RT */
	uint32 bEnablePlayersSplitRT:1;
protected:
	/**
...
public:
	...
	ENGINE_API virtual TSharedRef<FSceneViewport> CreateViewport(TSharedPtr<SViewport> InViewportWidget);
	UE_DEPRECATED(5.8, "Use CreateViewport instead")
	ENGINE_API virtual FSceneViewport* CreateGameViewport(TSharedPtr<SViewport> InViewportWidget) final;
...
public:
	...
	ENGINE_API virtual bool InputTouch(FViewport* const InViewport, const FTouchId TouchId, const ETouchType::Type Type, const FVector2D& TouchLocation, const float Force, const uint64 Timestamp) override;
	...
	ENGINE_API virtual bool InputGesture(FViewport* Viewport, const FInputDeviceId DeviceId, EGestureEvent GestureType, EGesturePhase GesturePhase, const FVector2D& GestureDelta, bool bIsDirectionInvertedFromDevice, const uint64 Timestamp) override;
...
public:
	 ...
	 */
	UE_DEPRECATED(5.8, "UGameViewportClient::SetViewportFrame and FViewportFrame are deprecated. Use FSceneViewport/SetViewport.")
	inline void SetViewportFrame(FSceneViewport* InViewportFrame)
	{
PRAGMA_DISABLE_DEPRECATION_WARNINGS
		SetViewport(InViewportFrame);
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	 ...
	 */
	UE_DEPRECATED(5.8, "If you must perform actions in response to the creation + assignment of this client's ISlateViewport, do so by overriding CreateGameViewport or AddAssociation")
	ENGINE_API virtual void SetViewport(FSceneViewport* InViewportFrame) final;

	UE_DEPRECATED(5.8, "Call Bind and use overlay created by UGameViewportClient, instead of creating one externally.")
	ENGINE_API void SetViewportOverlayWidget(TSharedPtr<SWindow> InWindow, TSharedRef<SOverlay> InViewportOverlayWidget);

	UE_DEPRECATED(5.8, "Call Bind and use layer manager created by UGameViewportClient, instead of creating one externally.")
	...
	ENGINE_API TSharedPtr<SWindow> GetWindow() const;
	...
	*/
	...
	/** Accessor for delegate called when a non-HDR screenshot is captured */
	static FOnScreenshotCaptured& OnScreenshotCaptured()
	{
		return ScreenshotCapturedDelegate;
	}

	/** Accessor for delegate called when an HDR screenshot is captured */
	...
	void SetCurrentMegaLightsVisualizationMode(FName NewMegaLightsVisualizationMode) { CurrentMegaLightsVisualizationMode = NewMegaLightsVisualizationMode; }
	FName GetCurrentMegaLightsVisualizationMode() const { return CurrentMegaLightsVisualizationMode; }
	...
	ENGINE_API virtual void AddAssociation(FViewport& InViewport) override;

	ENGINE_API virtual void RemoveAssociation(FViewport& InViewport) override;
...
public:
	...
	union
	{
		...
		UE_DEPRECATED(5.8, "ViewportFrame is deprecated. Use the Viewport member instead.")
		FSceneViewport* ViewportFrame;
	};
	...
	/**
	 * Get a ptr to the stat thermals data for this viewport
	 */
	virtual FStatThermalsData* GetStatThermalsData() const override
	{
		return StatThermalsData;
	}
	...
	*/
	...
	/** Accessor for delegate called when the window moves to another display */
	FOnWindowDisplayChanged& OnWindowDisplayChanged()
	{
		return WindowDisplayChangedDelegate;
	}
...
private:
	...
	/** Delegate handler for when a window is moved or resized */
	ENGINE_API void HandleRegularWindowMovedOrResized(TSharedRef<SWindow> InWindow);
...
private:
	/** Cached Slate window that contains this viewport's SViewport widget. */
	mutable TWeakPtr<SWindow> Window;
	...
	TSharedPtr<IGameLayerManager> GameLayerManager;
	...
	/** Current MegaLights visualization mode for this game viewport */
	...
	/** Delegate called when the window moves to another display */
	FOnWindowDisplayChanged WindowDisplayChangedDelegate;
	...
	/** Data needed to display thermal state when STAT THERMALS is enabled */
	FStatThermalsData* StatThermalsData;
