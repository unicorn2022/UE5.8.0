...
public:
	...
	ENGINE_API FStreamingTextureLevelContext(EMaterialQualityLevel::Type InQualityLevel, const IPrimitiveComponent* Primitive);
	ENGINE_API ~FStreamingTextureLevelContext();

	void SetForceNoUseBuiltData(bool bValue) { bForceNoUseBuiltData = bValue; }

	ENGINE_API TArray<FStreamingRenderAssetPrimitiveInfo> GetStreamingRenderAssetInfoWithNULLRemoval(const IPrimitiveComponent* Component);
	...
	ENGINE_API void ProcessMaterial(const FBoxSphereBounds& ComponentBounds, const FPrimitiveMaterialInfo& MaterialData, float ComponentScaling, TArray<FStreamingRenderAssetPrimitiveInfo>& OutStreamingTextures, bool bIsComponentBuildDataValid = false);
	UE_DEPRECATED(5.8, "The UPrimitiveComponent argument is no longer required")
	void ProcessMaterial(const FBoxSphereBounds& ComponentBounds, const FPrimitiveMaterialInfo& MaterialData, float ComponentScaling, TArray<FStreamingRenderAssetPrimitiveInfo>& OutStreamingTextures, bool bIsComponentBuildDataValid, const UPrimitiveComponent* DebugComponent)
	{
		ProcessMaterial(ComponentBounds, MaterialData, ComponentScaling, OutStreamingTextures, bIsComponentBuildDataValid);
	}
...
typedef TFunction<void(IPrimitiveComponent*, UStreamableRenderAsset*, ELODStreamingCallbackResult)> FLODStreamingCallback;
