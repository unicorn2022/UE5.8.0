...
1. All changes to Texture properties must be wrapped in PreEditChange/PostEditChange , eg :
  ...
  The PostEditChange will automatically UpdateResource when needed, you should generally not call
  Modify or UpdateResource manually, Pre/PostEdit will do that for you.

  Changes to Texture->Source should be inside a PreEdit/PostEdit in the same way.

  If possible, do all your changes inside a single PreEdit/PostEdit scope.

2. Before doing anything that relies on a texture having its PlatformData async build finished, you should call

  Texture->BlockOnAnyAsyncChange();

  (use this instead of FinishCompilation or calling to the FTextureCompilingManager directly)

  In some cases you might need to also call WaitForPendingInitOrStreaming.

3. When creating or filling a texture with new data, call :

  SetModernSettingsForNewOrChangedTexture();

  Basically, do this any time the texture is fresh data and you don't need to try to keep the bits identical
  to old data because you are hoping to keep patches small.  The UTexture default constructor sets things up
  with legacy values that we don't recommend, they are just that way to avoid unnecessary patches.
  Whenever posible, use SetModernSettingsForNewOrChangedTexture.

  If a texture is freshly imported, use ApplyDefaultsForNewlyImportedTextures which calls SetModernSettingsForNewOrChangedTexture
  and also sets up other typical defaults for imported textures.

4. When possible use the more modern APIs for working with pixel data in UTexture, such as FMipLock and FImage
  rather than raw arrays of U8.  For pixel format conversions, resizes, etc. use FImageCore.
...
struct FTextureSourceColorSettings
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = ColorManagement, meta = (EditCondition = "ColorSpace == ETextureColorSpace::TCS_Custom", EditConditionHides))
...
public:
	...
	*/
	...
	/**
	 * Returns true when the texture's SRGB state is determined at runtime rather than
	 * being authored on the asset (e.g. UMediaTexture, which adopts the currently playing
	 * sample's sRGB flag). When true, material sampler type validation should accept
	 * Color and LinearColor variants interchangeably, since the asset's serialized SRGB
	 * may not reflect what will be sampled at runtime.
	 */
	...
	*/
	...
	/** Returns the minimum size that width/height/depth will be clamped to when generating this texture. */
	ENGINE_API int32 GetMinTextureSize() const;
	/** Returns the maximum size that width/height/depth will be clamped to when generating this texture. */
