...
struct FTextureLODGroup
{
	...
	/** Cap the number of mips such that the largest mip is this big. Has no effect for textures with no mip chain. Used in combination with r.TextureGroup.QualityLevel, r.TextureGroup.MaxQualityLevel and TextureGroupOptionalQualityLevel. Not used for virtual textures. */
	UPROPERTY()
	int32 MaxLODSize_Low = -1;

	/** Cap the number of mips such that the largest mip is this big. Has no effect for textures with no mip chain. Used in combination with r.TextureGroup.QualityLevel, r.TextureGroup.MaxQualityLevel and TextureGroupOptionalQualityLevel. Not used for virtual textures. */
	UPROPERTY()
	int32 MaxLODSize_Medium = -1;

	/** Cap the number of mips such that the largest mip is this big. Has no effect for textures with no mip chain. Used in combination with r.TextureGroup.QualityLevel, r.TextureGroup.MaxQualityLevel and TextureGroupOptionalQualityLevel. Not used for virtual textures. */
	UPROPERTY()
	int32 MaxLODSize_High = -1;

	/** Cap the number of mips such that the largest mip is this big. Has no effect for textures with no mip chain. Used in combination with r.TextureGroup.QualityLevel, r.TextureGroup.MaxQualityLevel and TextureGroupOptionalQualityLevel. Not used for virtual textures. */
	UPROPERTY()
	int32 MaxLODSize_Epic = -1;
	...
	UPROPERTY()
	EVTProducerPriority VirtualTextureStreamingPriority = EVTProducerPriority::Normal;
	...
	ENGINE_API void ComputeMaxSizesForQualityLevels(int32 OutMaxSizes[4]) const;
};
...
public:
	...
	/**
	 * Calculates and returns the LOD bias based on texture LOD group, LOD bias and minimum/maximum size.
	 *
	 ...
	 * @param	MinSize						Min size to clamp width or height of the texture
	 * @param	MaxSize						Max size to clamp width or height of the texture
	...
	* @param	bCurrentlyStreamableTexture	If texture is currently streamable (if it's not, then quality levels don't apply to it)
	 * @return	LOD bias
	 */
	ENGINE_API int32 CalculateLODBias(int32 Width, int32 Height, int32 MinSize, int32 MaxSize, int32 LODGroup, int32 LODBias, int32 NumCinematicMipLevels, TextureMipGenSettings MipGenSetting, bool bVirtualTexture, bool bCurrentlyStreamableTexture = false) const;

	UE_DEPRECATED(5.8, "Call newer version of function that takes MinSize.")
	int32 CalculateLODBias(int32 Width, int32 Height, int32 MaxSize, int32 LODGroup, int32 LODBias, int32 NumCinematicMipLevels, TextureMipGenSettings MipGenSetting, bool bVirtualTexture, bool bCurrentlyStreamableTexture = false) const
	{
		return CalculateLODBias(Width, Height, 0, MaxSize, LODGroup, LODBias, NumCinematicMipLevels, MipGenSetting, bVirtualTexture, bCurrentlyStreamableTexture);
	}
...
public:
	...
	/** If set to a valid value, use MaxLODSize for the given quality instead of OptionalMaxLODSize during cook time. */
	UPROPERTY(EditAnywhere, config, Category="Texture LOD Settings")
	int32 TextureGroupOptionalQualityLevel = -1;
