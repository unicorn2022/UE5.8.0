...
namespace NSSkeletalMeshSourceFileLabels
{
	...
	{
		static FString GeoAndSkinningName("All");
		return GeoAndSkinningName;
	}

	inline const FString& GeometryMetaDataValue()
	{
		static FString GeometryName("Geometry");
		return GeometryName;
	}

	inline const FString& SkinningMetaDataValue()
	{
		static FString SkinningName("SkinningWeights");
		return SkinningName;
	}
...
#if WITH_EDITOR
	/** Notification when something is about to be changed */
	DECLARE_MULTICAST_DELEGATE(FOnPreMeshChange);
...
public:
	...
	*/
	 ...
	 *    is still successful.
	 */
	ENGINE_API bool CommitMeshDescription(const int32 InLODIndex, const FCommitMeshDescriptionParams& InParams = FCommitMeshDescriptionParams());
	 ...
	 *  \param bInAlwaysMarkPackageDirty Always mark the owning package dirty, even if there was not a transaction open to store into.
	...
	/** Generally clear data (source models, skeleton, physics, materials, etc) off the skeletal mesh */
...
public:
	...
	TArray<FSkeletalMaterialMinimalInfo> SkeletalMaterialsInfoCache;
	mutable FCriticalSection MaterialsInfoCriticalSection;
	ENGINE_API void UpdateSkeletalMaterialsInfoCache(const TArray<FSkeletalMaterial>& InMaterials);
	ENGINE_API const FString ExportSkeletalMaterialsInfo() const;
	...
	const TArray<FSkeletalMaterialMinimalInfo> GetCopyOfSkeletalMaterialsInfo() const
	{
		FScopeLock Lock(&MaterialsInfoCriticalSection);
		return SkeletalMaterialsInfoCache;
	}

	ENGINE_API static const TArray<FSkeletalMaterialMinimalInfo> ImportSkeletalMaterialsInfo(const FString& InData);
...
private:
	...
	void CacheClothDependencies();

	/** Cache of SkinnedAsset dependencies accessible during the async build. */
	TArray<TWeakObjectPtr<USkinnedAsset>> ClothDependencyCache;
#endif
...
public:
	...
	UPROPERTY(EditAnywhere, Category = AnimationRig, BlueprintGetter = GetDefaultAnimatingRig, BlueprintSetter = SetDefaultAnimatingRig, meta = (AllowedClasses = "/Script/ControlRigDeveloper.ControlRigEditorAssetInterface,/Script/ControlRig.ControlRigRuntimeAssetInterface"))
	...
	ENGINE_API void SetRefSkeleton(const FReferenceSkeleton& InRefSkeleton);
...
public:
...
#if WITH_EDITOR
	/** If the given section of the specified LOD has a clothing asset, unbind it and remove it from the asset array if it is no longer used. */
	...
	*/
	...
	UPROPERTY(VisibleAnywhere, BlueprintGetter = GetMeshClothingAssets, BlueprintSetter = SetMeshClothingAssets, Category = Clothing)
...
public:
	...
	const FOnPreMeshChange& GetOnPreMeshChange() const { return OnPreMeshChange; }
	FOnPreMeshChange& GetOnPreMeshChange() { return OnPreMeshChange; }
...
#endif
#if WITH_EDITOR
	FOnPreMeshChange OnPreMeshChange;
...
public:
	...
	*/
	...
	ENGINE_API bool RemoveMorphTargets(TConstArrayView<FName> InMorphTargetNames, bool bInRebuildRenderMesh = false);

	/**
	 * Remove a single morph target from all LODs on this mesh.
	 * @param InMorphTargetName The name of the morph target to remove.
	 * @param bInRebuildRenderMesh Set to true if rebuilding the render mesh as a part of the operation is desired. 
	 *    This is needed if the result of the morph removal on the mesh is to be visible afterward. 
	 * @return true if the morph target was successfully removed. 
	 */
	UFUNCTION(BlueprintCallable, Category = Mesh, meta=(DevelopmentOnly))
	bool RemoveMorphTarget(const FName InMorphTargetName, bool bInRebuildRenderMesh = false)
	{
		return RemoveMorphTargets({InMorphTargetName}, bInRebuildRenderMesh);
	}
...
#if WITH_EDITOR
	ENGINE_API bool RemoveSkinWeightProfiles(TConstArrayView<FName> InProfileName, bool bInRebuildRenderMesh = false);
 	
	/** Remove the named skin weight profile from this skeletal mesh. 
	 *  @param InProfileName The name of the profile to remove.
	 *  @param bInRebuildRenderMesh Set to true if rebuilding the render mesh as a part of the operation is desired. 
	 *    This is needed if the result of the skin weight profile removal on the mesh is to be visible afterward. 
	 *  @return true if the named profile was found and removed. 
	 */
	UFUNCTION(BlueprintCallable, Category = Mesh, meta=(DevelopmentOnly))
	bool RemoveSkinWeightProfile(const FName InProfileName, bool bInRebuildRenderMesh = false)
	{
		return RemoveSkinWeightProfiles({InProfileName}, bInRebuildRenderMesh);
	}
#endif
...
protected:
	...
	/** Additional deformers that will be applied to this mesh at runtime. Skeletal Mesh needs this collection of deformers to make sure it cooks any extra data required by these deformers */
	UE_DEPRECATED(5.8, "This must be protected for async build, always use the accessors even internally.")
