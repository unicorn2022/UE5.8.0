	...
	: public UObject
{
	...
	DECLARE_TS_MULTICAST_DELEGATE(FOnDataTableChanged);
...
public:
	...
	/** 
	 * Get the value in the datatable at the given row and column. 
	 * The provided function should get the value from the property type and the cell data
	 */
	template<typename TPropertyType, typename TReturnType>
	const TReturnType* GetCellValue(const FName& RowName, const FName& ColumnName, TFunction<const TReturnType*(const TPropertyType*, const uint8*)> GetPropertyValueFromCell)
	{
		const FProperty* Property = FindTableProperty(ColumnName);
		if (!Property)
		{
			return nullptr;
		}
		
		const TPropertyType* PropertyType = CastField<TPropertyType>(Property);
		if (!PropertyType)
		{
			return nullptr;
		}
		
		const uint8* RowData = FindRowUnchecked(RowName);
		if (!RowData)
		{
			return nullptr;
		}

		const uint8* CellData = Property->ContainerPtrToValuePtr<uint8>(RowData, 0);
		if (!CellData)
		{
			return nullptr;
		}

		return GetPropertyValueFromCell(PropertyType, CellData);
	}
...
struct FDataTableRowHandle
{
	...
	T* GetRow(const TCHAR* ContextString) const
	{
		if(DataTable == nullptr)
		{
			...
			{
				UE_LOGF(LogDataTable, Warning, "FDataTableRowHandle::GetRow : No DataTable for row %ls (%ls).", *RowName.ToString(), ContextString);
			}
			return nullptr;
		}
...
struct FDataTableCategoryHandle
{
	...
	void GetRows(TArray<T*>& OutRows, const FString& ContextString) const
	{
		...
		if (DataTable == nullptr)
		{
			...
			{
				UE_LOGF(LogDataTable, Warning, "FDataTableCategoryHandle::GetRows : No DataTable for row %ls (%ls).", *RowContents.ToString(), *ContextString);
			}
		...
		if (ColumnName == NAME_None)
		{
			if (RowContents != NAME_None)
			{
				UE_LOGF(LogDataTable, Warning, "FDataTableCategoryHandle::GetRows : No Column selected for row %ls (%ls).", *RowContents.ToString(), *ContextString);
			}
