...
// Specifies how the constraint responds when the angular drive target would violate the joint's angular limits.
// Without a response, the drive and the limit constraint compete, producing spurious torques.
UENUM(BlueprintType)
enum class EAngularDriveLimitViolationResponse : uint8
{
	// Drive target rotation is left unchanged. The drive and the limit constraint may compete.
	None       = 0,
	// Project the drive target onto the joint limits using a linear swing-angle approximation.
	// Accurate for small angles; exact for pure Swing1 or pure Swing2 rotations.
	ClampLinear = 1,
	// Project the drive target onto the joint limits using the exact Chaos 4*atan2 formula.
	// More accurate near the limit boundary but slightly more expensive than ClampLinear.
	ClampExact  = 2,
	// Widen the joint's angular limits to accommodate the drive target instead of clipping the
	// target. Limits are never narrowed below their authored defaults; they are restored each
	// frame when the target moves back inside. Has no effect on locked axes.
	WidenLimits = 3,
};
...
struct FAngularDriveConstraint
{
	...
	// Specifies how the constraint responds when the drive target rotation would violate the joint's
	// angular limits. Without a response, the drive and the limit constraint compete, producing
	// spurious torques.
	UPROPERTY(EditAnywhere, Category = AngularMotor)
	EAngularDriveLimitViolationResponse LimitViolationResponse = EAngularDriveLimitViolationResponse::None;
