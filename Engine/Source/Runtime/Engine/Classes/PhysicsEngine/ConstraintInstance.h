...
public:
	...
	*/
	...
	/**
	 * Projects TargetRelRot (child frame expressed in parent-bone-local space) onto the joint's
	 * angular limits so the drive target is always feasible. Returns the clamped rotation in the
	 * same space. If bExactSwingClamp is true, uses the exact Chaos 4*atan2 swing convention to
	 * match what the physics solver enforces; otherwise a linear axis-projection approximation is
	 * used, which is cheaper but may leave a small residual at the boundary.
	 */
	ENGINE_API FQuat ClampDriveTargetToLimits(const FQuat& TargetRelRot, bool bExactSwingClamp) const;

	/**
	 * Widens this constraint's angular limits so that TargetRelRot sits inside them without
	 * clipping. DefaultConstraint provides the authored minimum values; limits are never narrowed
	 * below those defaults. WidenMarginDegrees is added when the target exceeds a default so the
	 * drive target sits clearly inside the constraint boundary.
	 * Returns true if any axis was left wider than its authored default (i.e. widening was needed),
	 * false if all axes are at their defaults or the axis is locked/free.
	 */
	ENGINE_API bool WidenLimitsForDriveTarget(
		const FQuat&               TargetRelRot,
		const FConstraintInstance& DefaultConstraint,
		float                      WidenMarginDegrees = 0.5f);

	/**
	 * Restores this constraint's angular limits to the authored values stored in DefaultConstraint.
	 * Only touches axes whose motion type is ACM_Limited in DefaultConstraint; does nothing when
	 * the live value already matches the default.
	 */
	ENGINE_API void RestoreAngularLimitsToDefault(const FConstraintInstance& DefaultConstraint);
