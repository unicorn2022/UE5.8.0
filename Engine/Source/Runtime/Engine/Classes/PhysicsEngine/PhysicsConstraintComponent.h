...
public:
	...
	ENGINE_API void GetConstrainedBodyIds(TPair<FRemoteServerId, FRemoteServerId>& OwnerServerIds, TPair<FRemoteObjectId, FRemoteObjectId>& ObjectIds) const;
	ENGINE_API bool AreConstrainedComponentsInSameActor() const;
	ENGINE_API void AddReInitDelegatesToComponents();
	ENGINE_API void InitComponentConstraintWithoutUpdatingFrames();
	ENGINE_API void CheckAndUpdateConstraintPT();
	bool GetIsBrokenLocal() const { return bIsBrokenLocal; };
#endif
	...
	ENGINE_API void ReInitConstraintAndApplyPhysicsState(FPhysScene* PhysScene, float DeltaTime);
...
private:
	...
	ENGINE_API bool CanSendMigratedPhysicsState(const struct FUObjectMigrationContext* MigrationContext, EObjectMigrationSendType ObjMigrationSendType) const;

	void CheckAndUnbindPhysicsStateDelegates(bool bCheckIfBound = true);

	void UnBindPreTickDelegate();
