...
struct FKAggregateGeom
{
	...
	void VisitSimElements(CallableType&& InCallable) const
	{
		...
		for(const FKConvexElem& Elem : ConvexElems)
		{
			InCallable(Elem);
		}
	}
	...
	FKShapeElem* GetElement(const EAggCollisionShape::Type Type, const int32 Index)
	{
		...
		case EAggCollisionShape::SkinnedTriangleMesh:
			...
			break;
		default:
			// Failure is OK/returned/handled
			break;
		}
		return nullptr;
	}
	...
	*/
	...
	ENGINE_API FVector::FReal GetScaledVolumeForMassContribution(const FVector& Scale3D) const;
