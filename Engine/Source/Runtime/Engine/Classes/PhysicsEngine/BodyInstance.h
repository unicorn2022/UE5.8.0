...
using FBodyInstancePhysicsCreationInputsPtr = TSharedPtr<FBodyInstancePhysicsCreationInputs, ESPMode::ThreadSafe>;

// Cache used when creating physics body instances
struct FBodyInstancePhysicsCreationInputs
{
	void GetReferencedObjects(TSet<UObject*>& OutReferencedObjects);

	// Cached body setup resolved on GT
	UBodySetup* ResolvedBodySetup = nullptr;

	// Cached simple material raw ptr
	UPhysicalMaterial* ResolvedSimpleMaterial = nullptr;

	// Cached complex materials and masks
	TArray<UPhysicalMaterial*> ResolvedComplexMaterials;
	TArray<FPhysicalMaterialMaskParams> ResolvedComplexMasks;

	// Pre-computed collision filter data (avoids OwnerComponent/SourceObject weak ptr access on worker thread)
	FBodyCollisionData ResolvedCollisionData;

	// Pre-computed initial velocity for InitDynamicProperties (avoids OwnerComponent dereference on worker thread)
	FVector ResolvedInitialLinearVelocity = FVector::ZeroVector;
	bool bResolvedComponentAwake = false;

	// Cached owner component pointer for use as a map key ONLY.
	// Do NOT dereference - use only for AddToComponentMaps / RegisterForCollisionEvents.
	UPrimitiveComponent* OwnerComponentMapKey = nullptr;
};
...
{
	UE_API FInitBodiesHelper(TArray<FBodyInstance*, InAllocatorType>& InBodies, TArray<FTransform, InAllocatorType>& InTransforms, class UBodySetup* InBodySetup,
...
struct FInitBodiesHelperWithData : public FInitBodiesHelperBase
{
	...
	: FInitBodiesHelperBase(OwnedBodies, OwnedTransforms, InBodySetup, InPrimitiveComp, InRBScene, InSpawnParams, InAggregate, InBodyInstanceOwner), OwnedBodies(MoveTemp(InBodies)), OwnedTransforms(MoveTemp(InTransforms))
	{
		//Compute all the needed constants
	...
	: FInitBodiesHelperBase(OwnedBodies, OwnedTransforms, InHelper.BodySetup, InHelper.PrimitiveComp, InHelper.PhysScene, InHelper.SpawnParams, InHelper.Aggregate), OwnedBodies(InHelper.OwnedBodies), OwnedTransforms(InHelper.OwnedTransforms, InHelper.BodyInstanceOwner)
	{
		ensure(false);
	}
...
public:
	...
	*/
	 ...
	 * Can be called from a worker thread IF:
	 * - UnRegisterForCollisionEvents was already called on the game thread (or bNotifyRigidBodyCollision is false)
	 * - The caller has exclusive ownership of this FBodyInstance (e.g., ISM bodies moved to async payload)
...
public:
	// Cache resolved Inputs used by physics state creation (currently only used by async mode)
	ENGINE_API const FBodyInstancePhysicsCreationInputsPtr& CachePhysicsCreationInputs(UPrimitiveComponent* InOwnerComponent, IPhysicsBodyInstanceOwner* InBodyInstanceOwner = nullptr);
	
	// Clear cached physics creation inputs
	void ClearCachedPhysicsCreationInputs() { PhysicsCreationInputs.Reset(); }

	/**
	 * Returns the cached physics creation inputs when called from a worker thread (not game thread, not parallel game thread).
	 * Returns nullptr on the game thread or if no cache is available.
	 * Use this to safely branch between cached data (off-GT) and live data (on-GT).
	 */
	ENGINE_API const FBodyInstancePhysicsCreationInputs* GetAsyncPhysicsCreationInputs() const;
...
private:
	...
	static ENGINE_API void BuildBodyFilterData(const FBodyInstance* InInstance, UPrimitiveComponent* InOwnerComp, UObject* InSourceObj, UBodySetup* InSetup, FBodyCollisionFilterData& OutFilterData, const int32 InShapeIndex = INDEX_NONE);

	FBodyInstancePhysicsCreationInputsPtr PhysicsCreationInputs;
};
