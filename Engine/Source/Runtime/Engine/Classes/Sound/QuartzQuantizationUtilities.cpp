...
namespace Audio
{
	...
	void FQuartzClockTickRate::SetFramesPerTick(int32 InNewFramesPerTick)
	{
		...
		{
			UE_LOGF(LogAudioQuartz, Warning, "Quartz Metronme requires at least 1 frame per tick, clamping request");
	...
	void IQuartzQuantizedCommand::OnQueued(const FQuartzQuantizedCommandInitInfo& InCommandInitInfo)
	{
		...
		UE_LOGF(LogAudioQuartz, Verbose, "OnQueued() called for quantized event type: [%ls]", *GetCommandName().ToString());
	...
	void IQuartzQuantizedCommand::FailedToQueue(FQuartzQuantizedRequestData& InGameThreadData)
	{
		...
		UE_LOGF(LogAudioQuartz, Verbose, "FailedToQueue() called for quantized event type: [%ls]", *GetCommandName().ToString());
		FailedToQueueCustom();
	}
	...
	void IQuartzQuantizedCommand::AboutToStart()
	{
		...
		UE_LOGF(LogAudioQuartz, Verbose, "AboutToStart() called for quantized event type: [%ls]", *GetCommandName().ToString());
		AboutToStartCustom();
	}
	...
	void IQuartzQuantizedCommand::OnFinalCallback(int32 InNumFramesLeft)
	{
		...
		UE_LOGF(LogAudioQuartz, Verbose, "OnFinalCallback() called for quantized event type: [%ls]", *GetCommandName().ToString());
	...
	void IQuartzQuantizedCommand::OnClockPaused()
	{
		...
		UE_LOGF(LogAudioQuartz, Verbose, "OnClockPaused() called for quantized event type: [%ls]", *GetCommandName().ToString());
		OnClockPausedCustom();
	}
	...
	void IQuartzQuantizedCommand::OnClockStarted()
	{
		...
		UE_LOGF(LogAudioQuartz, Verbose, "OnClockStarted() called for quantized event type: [%ls]", *GetCommandName().ToString());
	...
	void IQuartzQuantizedCommand::Cancel()
	{
		...
		UE_LOGF(LogAudioQuartz, Verbose, "Cancel() called for quantized event type: [%ls]", *GetCommandName().ToString());
		CancelCustom();
	}
	...
	bool FQuartzQuantizedCommandHandle::Cancel()
	{
		...
		{
			UE_LOGF(LogAudioQuartz, Verbose, "OnQueued() called for quantized event type: [%ls]", *CommandPtr->GetCommandName().ToString());
