...
namespace Audio
{
	...
	public:
		...
		// InSoundWaveTransformationTarget is used to overwrite SoundWave data with its own transformations. (ex. marker data)
		ENGINE_API USoundWave* SynchronouslyWriteSoundWave(const TSampleBuffer<>& InSampleBuffer, const FString* FileName = nullptr, const FString* FilePath = nullptr, TObjectPtr<USoundWave> InSoundWaveTransformationTarget = nullptr);
