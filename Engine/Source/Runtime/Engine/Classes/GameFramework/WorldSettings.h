...
/** Settings for dynamically increasing the loading range in PIE after BeginPlay - makes PIE startup quicker on large maps */
USTRUCT()
struct FDynamicLoadingRangeScaling
{
	GENERATED_BODY()

	/** Enable for this map, unless user force disabled using bForceDisableDynamicLoadingRangeScaling. */
	UPROPERTY(EditAnywhere, Category = WorldPartitionSetup, AdvancedDisplay)
	bool bEnabled = false;

	/** Start scaling at which ratio, from 0.f to 100.f. 100.f is equivalent to disabling the feature. 0.f is not recommended if floor or terrain tiles are streamed, as the pawn will have nothing to stand on initially. */
	UPROPERTY(EditAnywhere, Category = WorldPartitionSetup, AdvancedDisplay, meta=(EditCondition = "bEnabled", Units="%", ClampMin = "0.0", ClampMax = "100.0", UIMin = "0.0", UIMax = "100.0"))
	float InitialDynamicLoadingRangeScaling = 100.f;

	/** Delay before scaling starts, to prevent additional world streaming while initial resources are also streaming in mips, LODs, etc. */
	UPROPERTY(EditAnywhere, Category = WorldPartitionSetup, AdvancedDisplay, meta=(EditCondition = "bEnabled", Units="s", ClampMin = "0.0", UIMin = "0.0"))
	float DynamicLoadingRangeScalingDelay = 0.f;

	/** How long to scale for after the delay, in seconds. */
	UPROPERTY(EditAnywhere, Category = WorldPartitionSetup, AdvancedDisplay, meta=(EditCondition = "bEnabled", Units="s", ClampMin = "0.0", UIMin = "0.0"))
	float DynamicLoadingRangeScalingDuration = 10.f;
};
...
#if WITH_EDITOR
	bool GetDynamicLoadingRangeScalingEnable() const;
#endif // WITH_EDITOR
...
public:
	/** If set to true, use new UGC navigation (added through scene graph component). */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, config, Category=AI, AdvancedDisplay, meta=(DisplayName = "Use Automatic UGC Navigation"))
	uint8 bUseAutomaticUGCNavigation:1;
	...
	*/
	...
	// This property is customized in FWorldPartitionDetails
	UPROPERTY()
	FDynamicLoadingRangeScaling DynamicLoadingRangeScaling;
...
public:
	...
	ENGINE_API bool AreWorldBoundsChecksEnabled() const;
