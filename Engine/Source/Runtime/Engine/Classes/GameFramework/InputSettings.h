	...
	: public UObject
{
	...
	/**
	 * If true, then before creating an IInputDeviceModule, check for a list of "preferred"
	 * input API names set by the user in their GGameUserSettingsIni.
	 *
	 * This will check the following value:
	 * 
	 *		; In GGameUserSettingsIni
	 *		[GamepadAPIPreference]
	 *		PreferredAPI=Example,API,List
	 *
	 *	The "PreferredAPI" config value is a comma seperated string value which can be modified in your gameplay code.
	 *	If the value is not set, or empty, then the default value of "DefaultPreferredInputAPIList" will be checked.
	 *
	 * If the module is not contained in the comma-seperated list of preferred APIs,
	 * then the input API will not be used.
	 * 
	 * This may be a useful setting for your project if you would like to provide users with multiple options
	 * for their Input API to choose from to enjoy the best compatibility for their device. Because this setting
	 * is config driven, it will allow you to package your game with all the input plugins you would like to support
	 * enabled, but determine which to use at runtime, similar to how many projects allow you to select between different
	 * rendering APIs. 
	 *
	 * This feature is only for Desktop platforms, and is experimental in 5.8
	 */
	UE_EXPERIMENTAL(5.8, "This setting is experimental")
	UPROPERTY(config, EditAnywhere, Category = "Input", meta=(InlineEditConditionToggle))
	uint8 bEnablePreferredInputAPIPreferences : 1;

	/**
	 * This feature is only for Desktop platforms, and is experimental in 5.8
	 */
	UE_EXPERIMENTAL(5.8, "This setting is experimental")
	UPROPERTY(config, EditAnywhere, Category = "Input", AdvancedDisplay, meta=(EditCondition="bEnablePreferredInputAPIPreferences"))
	FString DefaultPreferredInputAPIList;
...
public:
	...
	*/
	...
	/**
	 * Returns all FHardwareDeviceIdentifier types known across all platform configurations.
	 * Unlike GetAllHardwareDeviceNames, this returns the full structs (including InputClassName
	 * and PrimaryDeviceType) so callers can build descriptors for device simulation.
	 */
	static ENGINE_API TArray<FHardwareDeviceIdentifier> GetAllHardwareDeviceIdentifiers();
#endif	// WITH_EDITOR
