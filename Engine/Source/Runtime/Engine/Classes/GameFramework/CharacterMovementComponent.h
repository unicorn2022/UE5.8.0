...
public:
	...
	*/
	...
	UE_DEPRECATED(5.8, "Movement functions using UPrimitiveComponents have been deprecated (GetLastServerMovementBase included). Use GetLastServerMovementBaseInterfaceData instead.")
	UPrimitiveComponent* GetLastServerMovementBase() const { return Cast<UPrimitiveComponent>(LastMovementBaseInterfaceData.PhysicsObjectOwner.Get()); }

	/** Accessor to the Last Server Movement Base Interface Data, only relevant during server update*/
	const FMovementBaseInterfaceData* GetLastServerMovementBaseInterfaceData() const { return &LastMovementBaseInterfaceData; }
...
private:
	...
	UE_DEPRECATED(5.8, "This property has been deprecated. Please use LastMovementBaseInterfaceData instead.")
	TWeakObjectPtr<UPrimitiveComponent> LastServerMovementBase = nullptr;
	
	/** Remember last server movement base interface data so we can detect mounts/dismounts and respond accordingly. */
	FMovementBaseInterfaceData LastMovementBaseInterfaceData;
...
public:
	 ...
	 * Note: This only prevents the server from generating corrections. To also prevent the client from applying
	 * in-flight corrections, use bClientIgnoreMovementCorrections alongside this flag.
	 * @see bClientIgnoreMovementCorrections, bServerAcceptClientAuthoritativePosition, ServerCheckClientError()
	 */
	...
	/**
	 * When true, the client will not apply server movement corrections (skips ClientAdjustPosition).
	 * Typically used together with bIgnoreClientMovementErrorChecksAndCorrection, which prevents the
	 * server from generating corrections in the first place.
	 */
	UPROPERTY(Category="Character Movement", EditAnywhere, BlueprintReadWrite)
	uint8 bClientIgnoreMovementCorrections:1;
...
public:
	...
	*/
	...
	UE_DEPRECATED(5.8, "GetMovementBase has been deprecated. Use GetMovementBaseObject cast if necessary instead.")
	UFUNCTION(BlueprintCallable, Category="Pawn|Components|CharacterMovement", meta=(DeprecatedFunction="Note", DeprecationMessage="GetMovementBase has been deprecated. Use GetMovementBaseObject cast if necessary instead."))
	ENGINE_API UPrimitiveComponent* GetMovementBase() const;

	/** Return Uobject we are based on (standing and walking on). */
	UFUNCTION(BlueprintCallable, Category="Pawn|Components|CharacterMovement")
	ENGINE_API UObject* GetMovementBaseObject() const;
	
	/** Returns struct containing references and functionality for UObject we're using as a MovementBase. Uses CharacterOwner to get relevant movement base data */
	ENGINE_API const FMovementBaseInterfaceData* GetMovementBaseInterfaceData() const;
	
	/** Returns mutable struct containing references and functionality for UObject we're using as a MovementBase. Uses CharacterOwner to get relevant movement base data */
	ENGINE_API FMovementBaseInterfaceData* GetMovementBaseInterfaceData_Mutable();
	...
	UE_DEPRECATED(5.8, "Movement functions using UPrimitiveComponents have been deprecated. Use functions using the FMovementBaseInterfaceData struct instead.")
	ENGINE_API virtual void SetBase(UPrimitiveComponent* NewBase, const FName BoneName = NAME_None, bool bNotifyActor=true);
	
	// TODO NS: Should this be a pointer?
...
public:
	 ...
	 */	
	ENGINE_API void RevertMove(const FVector& OldLocation, FMovementBaseInterfaceData* OldMovementBaseInterfaceData, const FVector& InOldBaseLocation, const FFindFloorResult& OldFloor, bool bFailMove);
...
protected:
	...
	*/
	 ...
	 */
	ENGINE_API virtual void ServerMoveHandleClientError(float ClientTimeStamp, float DeltaTime, const FVector& Accel, const FVector& RelativeClientLocation, FMovementBaseInterfaceData* ClientMovementBaseInterfaceData, FName ClientBaseBoneName, uint8 ClientMovementMode);
	 ...
	 */
	ENGINE_API virtual bool ServerCheckClientError(float ClientTimeStamp, float DeltaTime, const FVector& Accel, const FVector& ClientWorldLocation, const FVector& RelativeClientLocation, FMovementBaseInterfaceData* ClientMovementBaseInterfaceData, FName ClientBaseBoneName, uint8 ClientMovementMode);
	 ...
	 */
	ENGINE_API virtual bool ServerExceedsAllowablePositionError(float ClientTimeStamp, float DeltaTime, const FVector& Accel, const FVector& ClientWorldLocation, const FVector& RelativeClientLocation, FMovementBaseInterfaceData* ClientMovementBaseInterfaceData, FName ClientBaseBoneName, uint8 ClientMovementMode);
	 ...
	 */
	ENGINE_API virtual bool ServerShouldUseAuthoritativePosition(float ClientTimeStamp, float DeltaTime, const FVector& Accel, const FVector& ClientWorldLocation, const FVector& RelativeClientLocation, FMovementBaseInterfaceData* ClientMovementBaseInterfaceData, FName ClientBaseBoneName, uint8 ClientMovementMode);
...
public:
	...
	*/
	...
	ENGINE_API virtual void ClientAdjustPosition_Implementation(float TimeStamp, FVector NewLoc, FVector NewVel, FMovementBaseInterfaceData* NewMovementBaseInterfaceData, FName NewBaseBoneName, bool bHasBase, bool bBaseRelativePosition, uint8 ServerMovementMode, TOptional<FRotator> OptionalRotation = TOptional<FRotator>());
	...
	ENGINE_API virtual void ClientVeryShortAdjustPosition_Implementation(float TimeStamp, FVector NewLoc, FMovementBaseInterfaceData* NewMovementBaseInterfaceData, FName NewBaseBoneName, bool bHasBase, bool bBaseRelativePosition, uint8 ServerMovementMode);
	...
	ENGINE_API virtual void ClientAdjustRootMotionPosition_Implementation(float TimeStamp, float ServerMontageTrackPosition, FVector ServerLoc, FVector_NetQuantizeNormal ServerRotation, float ServerVelZ, FMovementBaseInterfaceData* ServerMovementBaseInterfaceData, FName ServerBoneName, bool bHasBase, bool bBaseRelativePosition, uint8 ServerMovementMode);
...
public:
	...
	UE_DEPRECATED(5.8, "This property has been deprecated. Please use StartMovementBaseInterfaceData instead.")
	TWeakObjectPtr<UPrimitiveComponent> StartBase;
	FMovementBaseInterfaceData StartMovementBaseInterfaceData;
	FName StartBoneName;
	...
	UE_DEPRECATED(5.8, "This property has been deprecated. Please use EndMovementBaseInterfaceData instead.")
	TWeakObjectPtr<UPrimitiveComponent> EndBase;
	FMovementBaseInterfaceData EndMovementBaseInterfaceData;
	FName EndBoneName;
