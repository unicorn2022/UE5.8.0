...
struct FRepRootMotionMontage
{
	...
	~FRepRootMotionMontage() = default;
	FRepRootMotionMontage(const FRepRootMotionMontage&) = default;
	FRepRootMotionMontage(FRepRootMotionMontage&&) = default;
	FRepRootMotionMontage& operator=(const FRepRootMotionMontage&) = default;
	FRepRootMotionMontage& operator=(FRepRootMotionMontage&&) = default;
	...
	UE_DEPRECATED(5.8, "This property has been deprecated. Please use PhysicsObjectOwner instead and use that to create a FMovementBaseInterfaceData. See ACharacter::OnRep_ReplicatedBasedMovement() for an example of this.")
	UPROPERTY(meta = (DeprecatedProperty, DeprecationMessage="Please use PhysicsObjectOwner instead and use that to create a FMovementBaseInterfaceData."))
	TObjectPtr<UPrimitiveComponent> MovementBase = nullptr;

	/** PhysicsObjectOwner for BodyInstance used as movement base */
	UPROPERTY()
	TObjectPtr<UObject> PhysicsObjectOwner = nullptr;
...
{
	// Start deprecated movement base utility functions
	/** Determine whether MovementBase can possibly move. */
	UE_DEPRECATED(5.8, "MovementBaseUtility functions using UPrimitiveComponents have been deprecated. Use functions using the FMovementBaseInterfaceData struct instead.")
	...
	// End deprecated movement base utility functions


	/** Determine whether MovementBase can possibly move. */
	ENGINE_API bool IsDynamicBase(const FMovementBaseInterfaceData* MovementBase);

	/** Determine whether MovementBase is simulating or attached to a simulating object. */
	ENGINE_API bool IsSimulatedBase(const FMovementBaseInterfaceData* MovementBase);

	/** Ensure that BasedObjectTick ticks after NewBase */
	ENGINE_API void AddTickDependency(FTickFunction& BasedObjectTick, FMovementBaseInterfaceData* MovementBase);

	/** Remove tick dependency of BasedObjectTick on OldBase */
	ENGINE_API void RemoveTickDependency(FTickFunction& BasedObjectTick, FMovementBaseInterfaceData* MovementBase);
	
	/** Determine if we should use relative positioning when based on a component (because it may move). */
	inline bool UseRelativeLocation(const FMovementBaseInterfaceData* MovementBase)
	{
		return IsDynamicBase(MovementBase);
	}

	/** Get the velocity of the MovementBase, first checking the ComponentVelocity and falling back to the physics velocity if necessary. */
	ENGINE_API FVector GetMovementBaseVelocity(const FMovementBaseInterfaceData* MovementBase, const FName BoneName);

	/** Get the tangential velocity at WorldLocation for the MovementBase. */
	ENGINE_API FVector GetMovementBaseTangentialVelocity(const FMovementBaseInterfaceData* MovementBase, const FName BoneName, const FVector& WorldLocation);
	
	/** Get the transform (local-to-world) for the given MovementBase, optionally at the location of a bone. Returns false if MovementBase is nullptr, or if BoneName is not a valid bone. */
	ENGINE_API bool GetMovementBaseTransform(const FMovementBaseInterfaceData* MovementBase, const FName BoneName, FVector& OutLocation, FQuat& OutQuat);

	/** Convert a local location to a world location for a given MovementBase. Returns false if MovementBase is nullptr, or if BoneName is not a valid bone. Scaling is ignored. */
	ENGINE_API bool TransformLocationToWorld(const FMovementBaseInterfaceData* MovementBase, const FName BoneName, const FVector& LocalLocation, FVector& OutLocationWorldSpace);

	/** Convert a world location to a local location for a given MovementBase, optionally at the location of a bone. Returns false if MovementBase is nullptr, or if BoneName is not a valid bone. Scaling is ignored. */
	ENGINE_API bool TransformLocationToLocal(const FMovementBaseInterfaceData* MovementBase, const FName BoneName, const FVector& WorldSpaceLocation, FVector& OutLocalLocation);

	/** Convert a local direction to a world direction for a given MovementBase. Returns false if MovementBase is nullptr, or if BoneName is not a valid bone. Scaling is ignored. */
	ENGINE_API bool TransformDirectionToWorld(const FMovementBaseInterfaceData* MovementBase, const FName BoneName, const FVector& LocalDirection, FVector& OutDirectionWorldSpace);

	/** Convert a world direction to a local direction for a given MovementBase, optionally relative to the orientation of a bone. Returns false if MovementBase is nullptr, or if BoneName is not a valid bone. Scaling is ignored. */
	ENGINE_API bool TransformDirectionToLocal(const FMovementBaseInterfaceData* MovementBase, const FName BoneName, const FVector& WorldSpaceDirection, FVector& OutLocalDirection);

	/** Returns true if movement base data passed in matches. Even if both are invalid. */
	ENGINE_API bool DoesMovementBaseDataMatch(const FMovementBaseInterfaceData* MovementBaseDataA, const FMovementBaseInterfaceData* MovementBaseDataB);
	
	/** Returns true if MovementBaseData is valid (both not null and underlying FMovementBaseInterfaceData::IsValid check) */
	ENGINE_API bool IsMovementBaseDataValid(const FMovementBaseInterfaceData* MovementBaseData);
	
	/** Returns FMovementBaseInterfaceData with movement base data extracted from the PhysicsObjectOwner passed in. */
	ENGINE_API FMovementBaseInterfaceData GetMovementBaseDataFromPhysicsOwner(UObject* PhysicsObjectOwner);

	/** Returns FMovementBaseInterfaceData with movement base data extracted from the HitResult passed in. */
	ENGINE_API FMovementBaseInterfaceData GetMovementBaseDataFromHitResult(const FHitResult* HitResult);
}
...
struct FBasedMovementInfo
{
	...
	~FBasedMovementInfo() = default;
	FBasedMovementInfo(const FBasedMovementInfo&) = default;
	FBasedMovementInfo(FBasedMovementInfo&&) = default;
	FBasedMovementInfo& operator=(const FBasedMovementInfo&) = default;
	FBasedMovementInfo& operator=(FBasedMovementInfo&&) = default;
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	UE_DEPRECATED(5.8, "This property has been deprecated. Please use MovementBaseInterfaceData instead.")
	UPROPERTY(meta = (DeprecatedProperty, DeprecationMessage="Use MovementBaseInterfaceData instead."))
	TObjectPtr<UPrimitiveComponent> MovementBase = nullptr;
	...
	/** Struct containing Movement base functionality and relevant data for the UObject we are based on */
	UPROPERTY()
	...
	{
		return MovementBaseUtility::UseRelativeLocation(&MovementBaseInterfaceData);
	}
	...
	{
		return (!MovementBaseInterfaceData.IsValid()) && bServerHasBaseComponent;
	}
};
...
public:
	...
	UE_DEPRECATED(5.8, "Movement base functions using UPrimitiveComponents have been deprecated. Use functions using the FMovementBaseInterfaceData struct instead.")
	ENGINE_API virtual void SetBase(UPrimitiveComponent* NewBase, const FName BoneName = NAME_None, bool bNotifyActor=true);

	/** Sets the component the Character is walking on, used by CharacterMovement walking movement to be able to follow dynamic objects. */
	ENGINE_API virtual void SetBase(FMovementBaseInterfaceData* MovementBaseInterfaceData, const FName BoneName = NAME_None, bool bNotifyActor=true);
...
protected:
	/** CharacterMovement bIsActive replicated for simulated proxies. */
	UPROPERTY(Transient, ReplicatedUsing=OnRep_IsCharacterMovementActive)
	uint8 bProxyIsCharacterMovementActive : 1;

	ENGINE_API void SetProxyCharacterMovementActive(bool bInActive);

	UFUNCTION()
	ENGINE_API virtual void OnRep_IsCharacterMovementActive();
...
public:
	...
	*/
	...
	UE_DEPRECATED(5.8, "GetMovementBase is deprecated. Use GetMovementBaseObject or GetMovementBaseInterfaceData instead.")
	virtual UPrimitiveComponent* GetMovementBase() const override final { return Cast<UPrimitiveComponent>(GetMovementBaseObject()); }
	
	virtual UObject* GetMovementBaseObject() const override final { return BasedMovement.MovementBaseInterfaceData.GetMovementBaseObject(); }
	ENGINE_API virtual const FMovementBaseInterfaceData* GetMovementBaseInterfaceData() const override final;
	ENGINE_API virtual FMovementBaseInterfaceData* GetMovementBaseInterfaceData_Mutable() override final;
...
public:
	...
	/** 
	 * By default this function returns false, which prevents jumping while in falling state (in many cases, we use this to consume the first jump), 
     * but subclasses can override this behavior to allow jump to happen while falling under specific conditions they may care about 
	 */
	ENGINE_API virtual bool CanJumpWhileFalling() const;
	...
	/** Sets bOnlyAllowAutonomousTickPose based on the character's current state */
	ENGINE_API void UpdateMeshAllowAutonomousTickPose();
};
