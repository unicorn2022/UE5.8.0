...
namespace UE
{
	struct FTouchState
	{
		...
		FVector2f ViewportRelativeLocation = FVector2f(-1.0f, -1.0f);
		float Pressure = 0.0f;
	};
}
...
public:
	...
	UE_DEPRECATED(5.8, "Use GetTouchStates() instead")
	FVector Touches[EKeys::NUM_TOUCH_KEYS];

	/** The most recent state reported for each finger. */
	TArrayView<const UE::FTouchState> GetTouchStates() const
	{
		return TouchStates;
	}
...
public:
	...
	/** Handles a gesture input event with 2D value (e.g. pan delta). Returns true. */
	ENGINE_API bool InputGesture(const FInputKeyEventArgs& Params);
...
protected:
	...
	TStaticArray<UE::FTouchState, EKeys::NUM_TOUCH_KEYS> TouchStates;
