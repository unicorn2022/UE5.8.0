...
struct FActorLastRenderTime
{
	...
	UE_DEPRECATED(5.8, "Use HasAlwaysVisibleComponents/IncrementAlwaysVisibleComponents/DecrementAlwaysVisibleComponents instead.")
	std::atomic_int32_t NumAlwaysVisibleComponents = 0;

	bool HasAlwaysVisibleComponents() const
	{
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		return NumAlwaysVisibleComponents.load(std::memory_order_relaxed) > 0;
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
	}

	void IncrementAlwaysVisibleComponents()
	{
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		NumAlwaysVisibleComponents.fetch_add(1, std::memory_order_relaxed);
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
	}

	/** Returns the previous count (before decrement). */
	int32 DecrementAlwaysVisibleComponents()
	{
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		return NumAlwaysVisibleComponents.fetch_sub(1, std::memory_order_relaxed);
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
	}
...
private:
	...
	enum class EAddComponentForReplicationBehavior : uint8
	{
		Immediate,
		Deferred,
		NeverAdd
...
public:
	...
	virtual bool ShouldForceDuplicateInPIE() const { return bForceDuplicateInPIE; }
#endif
...
public:
	...
	{
		return GetLevelInstanceType() == ELevelInstanceType::LevelInstanceEdit;
	}
	...
	{
		return GetLevelInstanceType() == ELevelInstanceType::LevelInstancePropertyOverride;
	}
	...
	{
		return EnumHasAnyFlags(GetLevelInstanceFlags(), ELevelInstanceFlags::IsInEditHierarchy);
	}

	/** If true, this actor or one of its components has property overrides applied. */
	bool HasLevelInstancePropertyOverrides() const
	{
		return EnumHasAnyFlags(GetLevelInstanceFlags(), ELevelInstanceFlags::HasPropertyOverrides);
	}
	...
	{
		return EnumHasAnyFlags(GetLevelInstanceFlags(), ELevelInstanceFlags::HasEditablePropertyOverrides);
	}

	/** If true, the actor belongs to a level instance. */
	bool IsInLevelInstance() const
	{
		return GetLevelInstanceType() != ELevelInstanceType::None;
	}

	/** Whether this actor belongs to a level instance or not and what type of level instance. */
	ENGINE_API ELevelInstanceType GetLevelInstanceType() const;

	/** Flags related to level instances for this actor. */
	ENGINE_API ELevelInstanceFlags GetLevelInstanceFlags() const;
#endif
...
#if WITH_EDITORONLY_DATA
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	UE_DEPRECATED(5.8, "Use DataLayerAssets instead.")
	UPROPERTY(meta = (DeprecatedProperty, DeprecationMessage = "Use DataLayerAssets instead."))
...
public:
	...
	ENGINE_API void ForceLoadDataLayerAssets() const;
	...
	static const FName GetExternalDataLayerAssetPropertyName() { return GET_MEMBER_NAME_CHECKED(AActor, ExternalDataLayerAsset); }

	UE_DEPRECATED(5.8, "FActorDataLayer is no longer in use.")
	static const FName GetDataLayerPropertyName() { return FName(); }
	...
	// This getter is kept solely to enable UDataLayerToAssetCommandlet for conversions of old data
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	UE_DEPRECATED(all, "Convert DataLayer using UDataLayerToAssetCommandlet and use GetDataLayerAssets() instead")
...
private:
	...
	ENGINE_API virtual bool SupportsIncrementalPreUnregisterComponents() const;
...
public:
	...
	/**
	 * Returns the server this actor should live on for push migration (ownership change).
	 * Override for class-specific behavior (e.g., Controller follows Pawn).
	 * Default:
	 *  - If the Actor is local and movable, use GetPreferredServerDelegate for game-specific implementation.
	 *  - Otherwise, report the current owner server (no change).
	 */
	ENGINE_API virtual FRemoteServerId GetPreferredRemoteServer() const;

	/**
	 * Static delegate for plugins or games to provide an implementation (typically a spatial-partition-based server lookup).
	 * Called by the default GetPreferredRemoteServer() implementation.
	 */
	static ENGINE_API TDelegate<FRemoteServerId(const AActor*)> GetPreferredServerDelegate;
#endif
...
protected:
	...
	*/
	...
	void BeginReplication(const FActorReplicationParams& Params) {}
	...
	// Required by both these classes to call OnReplicationStartedForIris().
	friend struct UE::Net::FReplicationSystemUtil;
	friend class UEngineReplicationBridge;
...
private:
	...
	ENGINE_API void ResetOwnedComponents(EAddComponentForReplicationBehavior Type);
...
private:
	...
	void AddComponentForReplication(UActorComponent* Component, EAddComponentForReplicationBehavior Type);
