...
public:
	...
	UFUNCTION(Reliable, Server, SealedEvent)
	ENGINE_API void ServerUpdateContainerVisibility(UObject* Container, bool bVisible);
...
#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
	// The current input component stack which is built every frame in APlayerController::BuildInputStack.
	// This is useful for building any external debug tools where you have access to the player controller
	// It is not recommended to access this outside of debug tooling because this is re-built every frame.
	TConstArrayView<TWeakObjectPtr<const UInputComponent>> Debug_GetMostRecentInputStack() const { return { LastUsedInputStack }; }
#endif
...
#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
	// Debug tracking of the most recent *complete* input stack.
	TArray<TWeakObjectPtr<const UInputComponent>> LastUsedInputStack;
#endif
...
public:
	...
	UE_DEPRECATED(5.8, "Use the overload that uses FTouchId instead")
	ENGINE_API virtual bool InputTouch(
		...
		const uint64 Timestamp) final;

	/**
	 * Allow the player-controller to process a touch event received by the viewport-client.
	 * If the player-controller uses the event, it should return true.  If true is returned, the viewport-client will not offer it to the next
	 * candidate.
	 * 
	 * @param	TouchId - Identifier unique to this ongoing touch (i.e. device + finger).
	 * @param	Type - What kind of touch event this is (see ETouchType).
	 * @param	TouchLocation - Viewport-relative position of the touch.
	...
	/** Handles a gesture input event, maps EGestureEvent to FKey and forwards to PlayerInput */
	ENGINE_API virtual bool InputGesture(const FInputDeviceId DeviceId, EGestureEvent GestureType, EGesturePhase GesturePhase, const FVector2D& GestureDelta, bool bIsDirectionInvertedFromDevice, const uint64 Timestamp);
...
protected:
	...
	UE_DEPRECATED(5.8, "Use the overload with FTouchId instead")
	ENGINE_API virtual void ProcessTouchHitResult(const FInputDeviceId DeviceId, uint32 Handle, ETouchType::Type Type, const FVector2D& TouchLocation, const FHitResult& HitResult, bool bHit) final;

	ENGINE_API virtual void ProcessTouchHitResult(const FTouchId TouchId, const ETouchType::Type Type, const FVector2D& TouchLocation, const FHitResult& HitResult, bool bHit);
...
private:
	...
	* This is 0 on the server */
	...
	/** Time to add to the tick offset, works as a buffer on the server to handle fluctuating network conditions */
	float TickOffsetBufferTime;
...
public:
	...
	*/
	...
	/** Get the current buffer added to the physics tick-offset */
	float GetTickOffsetBufferTime() const { return TickOffsetBufferTime; }

	/** Set a new buffer time to add to the physics tick-offset */
	void SetTickOffsetBufferTime(float NewBufferTime);
...
#if UE_WITH_REMOTE_OBJECT_HANDLE
	ENGINE_API virtual FRemoteServerId GetPreferredRemoteServer() const override;
#endif
