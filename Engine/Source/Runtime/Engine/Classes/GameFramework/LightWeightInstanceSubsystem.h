...
struct FLightWeightInstanceSubsystem
{
	...
	ENGINE_API ADEPRECATED_LightWeightInstanceManager* FindLightWeightInstanceManager(const FActorInstanceHandle& Handle) const;

	// Returns the instance manager that handles actors of type ActorClass in level Level
	ENGINE_API ADEPRECATED_LightWeightInstanceManager* FindLightWeightInstanceManager(UClass& ActorClass, UWorld& World, const FVector& InPos, const UDataLayerInstance* DataLayer = nullptr) const;

	// Returns the instance manager that handles instances of type Class that live in Level
	ENGINE_API ADEPRECATED_LightWeightInstanceManager* FindOrAddLightWeightInstanceManager(UClass& ActorClass, UWorld& World, const FVector& InPos, const UDataLayerInstance* DataLayer = nullptr);
	...
	ENGINE_API bool AddManager(ADEPRECATED_LightWeightInstanceManager* Manager);

	// Remove a manager from the subsystem, thread safe.
	ENGINE_API bool RemoveManager(ADEPRECATED_LightWeightInstanceManager* Manager);
...
protected:
	...
	ENGINE_API int32 GetManagerIndex(const ADEPRECATED_LightWeightInstanceManager* Manager) const;

	// Returns the light weight instance manager at index Index
	ENGINE_API const ADEPRECATED_LightWeightInstanceManager* GetManagerAt(int32 Index) const;

private:
	// TODO: preallocate the size of this based on a config variable
	TArray<ADEPRECATED_LightWeightInstanceManager*> LWInstanceManagers;
