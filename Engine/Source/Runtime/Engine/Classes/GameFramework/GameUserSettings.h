...
public:
	...
	UFUNCTION(BlueprintCallable, Category = Settings)
	ENGINE_API int32 GetLandscapeQuality() const;
	...
	static ENGINE_API void RequestResolutionChange(int32 InResolutionX, int32 InResolutionY, EWindowMode::Type InWindowMode, bool bInDoOverrides = true, int32 MonitorIndex = -1);
	...
	UFUNCTION(BlueprintCallable, Category = Settings)
	ENGINE_API void SetMaximumHDRDisplayNits(int32 DisplayNits);

	/** Gets the maximum luminance that we want to display. */
	UFUNCTION(BlueprintCallable, Category = Settings)
	ENGINE_API int32 GetMaximumHDRDisplayNits() const;

	/** Sets if HDR calibration is used (only used for platforms without system HDR calibration) */
	UFUNCTION(BlueprintCallable, Category = Settings)
	ENGINE_API void SetHDRCalibrationUsed(bool bUseCalibration);

	/** Gets if HDR calibration is used (only used for platforms without system HDR calibration) */
	UFUNCTION(BlueprintCallable, Category = Settings)
	ENGINE_API bool IsHDRCalibrationUsed() const;

	/** Sets HDR paper white to control scene brightness, also UI unless bIsHDRUILuminanceSeparate is true */
	UFUNCTION(BlueprintCallable, Category = Settings)
	ENGINE_API void SetHDRPaperWhiteNits(float Nits);

	/** Gets HDR paper white to control scene brightness, also UI unless bIsHDRUILuminanceSeparate is true */
	UFUNCTION(BlueprintCallable, Category = Settings)
	ENGINE_API float GetHDRPaperWhiteNits() const;

	/** Sets the HDR UI luminance value that is used if luminance is separate*/
	UFUNCTION(BlueprintCallable, Category = Settings)
	ENGINE_API void SetHDRUILuminanceNits(float Nits);

	/** Gets the HDR UI luminance value that is used if luminance is separate  */
	UFUNCTION(BlueprintCallable, Category = Settings)
	ENGINE_API float GetHDRUILuminanceNits() const;

	/** Sets whether HDR UI should use its own value or use paper white */
	UFUNCTION(BlueprintCallable, Category = Settings)
	ENGINE_API void SetHDRUILuminanceSeparate(bool bIndependent);

	/** Gets whether HDR UI should use its own value or use paper white */
	UFUNCTION(BlueprintCallable, Category = Settings)
	ENGINE_API bool IsHDRUILuminanceSeparate() const;

	/** Sets if HDR is enabled (ignored if support is unavailable) */
	UFUNCTION(BlueprintCallable, Category = Settings, meta = (DisplayName = "Set HDR Enabled"))
	ENGINE_API void SetHDREnabled(bool bEnableHDR);

	UFUNCTION(BlueprintCallable, Category = Settings, meta = (DisplayName = "Is HDR Enabled"))
	ENGINE_API bool IsHDREnabled() const;

	/**
	 * @return True if Lumen Global Illumination is allowed by scalability and supported by the device. This does not take into account whether Lumen is enabled in the project settings, or a Post Process Volume.
	 * This is useful for the game to setup exposure depending on whether interiors can be darkened by skylight shadowing or not.
	 */
	UFUNCTION(BlueprintCallable, Category = Settings, meta = (DisplayName = "Is Global Illumination Allowed", WorldContext = "WorldContextObject"))
	ENGINE_API bool IsGlobalIlluminationAllowed(UObject* WorldContextObject) const;
...
protected:
	...
	/** String that attempts to uniquely identify a display even if FDisplayMetrics::MonitorInfo is reordered. */
	UPROPERTY(config)
	FString DisplayID;

	/** Index of a display in FDisplayMetrics::MonitorInfo. */
	UPROPERTY(config)
	int32 DisplayIndex;

	/** Last user confirmed display by ID. */
	UPROPERTY(config)
	FString LastUserConfirmedDisplayID;

	/** Last user confirmed display by index. */
	UPROPERTY(config)
	int32 LastUserConfirmedDisplayIndex;
	...
	/** Dynamic resolution frame target */
	UPROPERTY(config)
	float DynamicResolutionFrameTarget;
	...
	/** Whether to display output in HDR if support is available */
	UPROPERTY(config)
	bool bUseHDRDisplayOutput;

	/** Whether to override platform HDR display information */
	UPROPERTY(config)
	bool bUseHDRDisplayCalibration;

	/** The maximum nits of which the HDR display is capable (application should offer calibration flow) */
	UPROPERTY(config)
	...
	/** Paper white luminance to control UI brightness in HDR if bIsHDRUILuminanceSeparate is true */
	UPROPERTY(config)
	float HDRUILuminanceNits;

	/** Whether to override HDRPaperWhiteNits with HDRUILuminanceNits for UI brightness */
	UPROPERTY(config)
	bool bIsHDRUILuminanceSeparate;
...
public:
	...
	/**
	 * We call this to notify UI to refresh relevant settings.
	 */
	DECLARE_MULTICAST_DELEGATE(FOnGameUserSettingsVideoRevert);
	FOnGameUserSettingsVideoRevert OnGameUserSettingsVideoRevert;
