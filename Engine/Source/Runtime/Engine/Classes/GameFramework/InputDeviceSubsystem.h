...
/** Native delegate for non-UObject listeners for when new input device metadata is available. */
DECLARE_MULTICAST_DELEGATE_TwoParams(FHardwareInputDeviceChangedNative, FPlatformUserId, FInputDeviceId);
...
public:
	...
	*/
	...
	/** Native companion to OnInputHardwareDeviceChanged — supports AddRaw/AddWeakLambda for non-UObject listeners. */
	FHardwareInputDeviceChangedNative OnInputHardwareDeviceChangedNative;
	
protected:

	void HandleDeviceConnectionChange(EInputDeviceConnectionState NewState, FPlatformUserId User, FInputDeviceId DeviceId);

	/**
	 * Handler subscribed to FInputDeviceRegistry::OnInputDeviceDescriptorChanged. Routes a descriptor
	 * change into SetMostRecentlyUsedHardwareDevice so OnInputHardwareDeviceChanged becomes the single
	 * delegate gameplay code needs to subscribe to in order to be notified of device-identity changes
	 * (initial connect, descriptor refresh as the platform layer finalizes hardware metadata, etc).
	 */
	void HandleInputDeviceDescriptorChanged(const struct FInputDeviceDescriptor& Descriptor);
