...
class UDataTableFunctionLibrary : public UBlueprintFunctionLibrary
{
	 ...
	 * @param CurveTable The Curve Table to export.
	 * @return The CurveTable contents encoded as a string.
	 */
	UFUNCTION(BlueprintCallable, Category = "DataTable")
	static ENGINE_API FString CurveTableToJson(UCurveTable* CurveTable);

	/**
	 * Convert a curve table to JSON.
	 */
	UFUNCTION(BlueprintCallable, Category = "DataTable")
	static ENGINE_API TArray<FString> JsonToCurveTable(UCurveTable* CurveTable, const FString& InString);

	/**
	 * Adds a simple curve to a curve table.
	 * @param CurveTable The Curve Table to modify
	 * @param RowName The name of the new row
	 * @return True if the operation was successful.
	 */
	UFUNCTION(BlueprintCallable, Category = "DataTable")
	static ENGINE_API bool AddSimpleCurveToTable(UCurveTable* CurveTable, FName RowName);

	/**
	 * Returns the names of the rows in the curve table.
	 * @param CurveTable The Curve Table to query
	 * @return A list of names of rows.
	 */
	UFUNCTION(BlueprintCallable, Category = "DataTable")
	static ENGINE_API TArray<FName> GetCurveTableRowNames(UCurveTable* CurveTable);

	/**
	 * Adds a simple curve to a curve table.
	 * @param CurveTable The Curve Table to modify
	 * @param RowName The name of the row to modify
	 * @param DefaultValue The default value of the row.
	 * @return True if the operation was successful.
	 */
	UFUNCTION(BlueprintCallable, Category = "DataTable")
	static ENGINE_API bool SetCurveTableRowDefault(UCurveTable* CurveTable, FName RowName, float DefaultValue);
	
	/**
	 * Adds a key to a row in a CurveTable
	 * @param CurveTable The Curve Table to modify.
	 * @param RowName The name of the row to modify.
	 * @param InTime The time of the new key.
	 * @param InValue The value of the new key.
	 * @return True if the operation was successful.
	 */
	UFUNCTION(BlueprintCallable, Category = "DataTable")
	static ENGINE_API bool AddCurveTableKey(UCurveTable* CurveTable, FName RowName, float InTime, float InValue);

	/**
	 * Returns a list of keys in a row of a SimpleCurve table.
	 * @param CurveTable The Curve Table to query.
	 * @param RowName The name of the row to query.
	 * @return The list of keys in the curve, or empty if the curve doesn't exist.
	 */
	UFUNCTION(BlueprintCallable, Category = "DataTable")
	static ENGINE_API TArray<FSimpleCurveKey> GetCurveTableKeys(UCurveTable* CurveTable, FName RowName);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "DataTable")
	static ENGINE_API bool RemoveCurveTableRow(UCurveTable* CurveTable, FName RowName);
	
	/**
	 * Renames a curve table row.
	 * @param CurveTable The Curve Table to modify
	 * @param RowName The name of the row to rename
	 * @param NewRowName The new name of the row
	 * @return True if the operation was successful.
	 */
	UFUNCTION(BlueprintCallable, Category = "DataTable")
	static ENGINE_API bool RenameCurveTableRow(UCurveTable* CurveTable, const FName& RowName, const FName& NewRowName);
