...
struct FMovementProperties
{
	...
	/** If true, this Pawn is capable of flying. By default, Pawn can only use NavData with matching bCanFly values.*/
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category=MovementProperties)
	uint8 bCanFly:1;
...
struct FNavigationFindMoveAlongSurfaceWork
{
	...
	/** If set, stores the result of the underlying find surface move call
	 *  TRUE if the underlying find surface move call succeeded -- in this case ResultNavLocation will store the navmesh surface location and poly where it ends
	 *  FALSE if the underlying find surface move call failed */
	TOptional<bool> bResult;

	FNavigationFindMoveAlongSurfaceWork(const FNavLocation& InStartLocation = FNavLocation(), const FVector& InDesiredEndLocation = FNavigationSystem::InvalidLocation)
		: StartLocation(InStartLocation), ResultNavLocation(), DesiredEndLocation(InDesiredEndLocation), bResult()
	{}
};
