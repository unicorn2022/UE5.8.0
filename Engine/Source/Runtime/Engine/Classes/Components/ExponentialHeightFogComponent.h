...
class UExponentialHeightFogComponent : public USceneComponent
{
	...
	/** When >0 and when r.SupportsSkyLightCaptureAffectsHeightFogInScattering is true, this will make the sky light capture inject scattered light into the height fog. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = InscatteringTexture, meta = (DisplayName = "Sky Light Capture In-Scattering Strength", UIMin = "0", UIMax = "1", ClampMin = "0"))
	float SkyLightCaptureAffectsHeightFogStrength;

	/** This is the roughness at which the sky light capture cubemap will be sample to simulate scattering dispersion due to a phase function. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = InscatteringTexture, meta = (DisplayName = "Sky Light Capture In-Scattering Roughness", UIMin = "0", UIMax = "1", ClampMin = "0", ClampMax = "1"))
	float SkyLightCaptureAffectsHeightFogRoughness;
	...
	/**
	 * Enable FSSS (Fog Screen Space Scattering) to approximate multiple scattering.
	 */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Fog Screen Space Scattering - EXPERIMENTAL", AdvancedDisplay, meta = (DisplayName = "Enable Fog Screen Space Scattering"))
	bool bEnableFSSS;

	/**
	 * The amount of scene color injected into the fog screen space scattering technique.
	 * When 0, only the fog luminance will be scattered.
	 * When 1, fog and scene luminance will be scattered.
	 * When above 1, more scene color can be injected to increase the brightness of the multiple scattering effect, even when the fog is not too intense. We cannot scatter more than the scene color however.
	 */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Fog Screen Space Scattering - EXPERIMENTAL", AdvancedDisplay, meta = (DisplayName = "Scene Color Scattering Amount Scale", UIMin = "0", UIMax = "1", ClampMin = "0"))
	float FSSSSceneColorScatteringAmountScale;

	/**
	 * Apply a power over the amount of scattered scene luminance after it is evaluated using Scene Color Scattering Amount Scale.
	 * A value smaller than 1 can show more scattering where the fog is less intense.
	 */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Fog Screen Space Scattering - EXPERIMENTAL", AdvancedDisplay, meta = (DisplayName = "Scene Color Scattering Amount Power", UIMin = "0.01", UIMax = "2", ClampMin = "0"))
	float FSSSSceneColorScatteringAmountPower;

	/**
	 * The scale of the spread for the fog multiple scattering. This basically scales mip level sampled from the blurred screen space FSSS texture.
	 * Increasing will make scene and thicker fog look fuzzier.
	 * But too high values can cause artifacts such as halos around depth discontinuities.
	 */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Fog Screen Space Scattering - EXPERIMENTAL", AdvancedDisplay, meta = (DisplayName = "Spread Scale", UIMin = "0", UIMax = "1", ClampMin = "0"))
	float FSSSSpreadScale;

	/**
	 * When generating the the blurred screen space FSSS texture, this controls how much of the blurred lower mip levels is injected back into the sharper higher mip levels.
	 * A higher value will result in a blurrier result. But when = 1, multiple scattering will be a single average color of the FSSS texture.
	 */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Fog Screen Space Scattering - EXPERIMENTAL", AdvancedDisplay, meta = (DisplayName = "Blur Control", UIMin = "0", UIMax = "1", ClampMin = "0", ClampMax = "1"))
	float FSSSBlurControl;
