...
public:
	...
	ENGINE_API FRegisterComponentContext(UWorld* InWorld);
	ENGINE_API ~FRegisterComponentContext();
	...
	UE_DEPRECATED(5.8, "AsyncRegisterLevelContext has been removed.")
	FRegisterComponentContext& SetAsyncRegisterLevelContext(struct FAsyncRegisterLevelContext* Value);
...
private:
	...
	/**
	* Whether this component will receive a call to AsyncPhysicsTickComponent (in code) and ReceiveAsyncPhysicsTick (a Blueprint implementable event).
    * These functions will execute on the game thread and are allowed to read and write both Physics Thread (PT) and Game Thread (GT) Data .
	* Performance Warning: This will cause both threads to halt and force a full sync of data between PT and GT. The PT will block until the GT is done executing all AsyncPhysicsTick functions.
	*/
	...
	// When set to true the component will remove itself from Iris in EndPlay(). If set to false it's
	// assumed that the component will be responsible for removing itself from Iris.
	uint8 bCallStopReplicationInEndPlay : 1 = true;
#endif
...
public:
	...
	*/
	...
	/** Returns the physics scene associated with this component */
	ENGINE_API FPhysScene* GetPhysicsScene() const;
