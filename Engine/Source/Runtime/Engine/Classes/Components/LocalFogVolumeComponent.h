...
class ULocalFogVolumeComponent : public USceneComponent
{
	...
	/** Controls the emissive color of this fog volume. (Unit = Luminance) */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, interp, Category = "Shading")
	FLinearColor FogEmissive = FLinearColor::Black;

	/** Controls the distance after which this volume starts to appear. (Unit = Centimeter) This distance is clamped to be greater than r.LocalFogVolume.GlobalStartDistance. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, interp, Category = "Shading", meta = (DisplayName = "Start Distance", UIMin = "0.0", ClampMin = 0.0))
	...
	UFUNCTION(BlueprintCallable, Category = "Rendering")
	ENGINE_API void SetFogStartDistance(float NewValue);
	UFUNCTION(BlueprintCallable, Category = "Rendering")
