...
public:
	...
	* If CaptureSource uses FinalColor, tiling will be ignored and a Warning message will be logged	*/
	...
	/** Render with main view camera.  Enables Main View Family.  Temporal AA jitter is matched with main view if Main View Resolution is enabled and no Resolution Divisor is set. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category=PassInfo)
	...
	{
		return bMainViewResolution || (ShouldRenderInMainRenderer() && (!UserSceneTextureBaseColor.IsNone() || !UserSceneTextureNormal.IsNone() || !UserSceneTextureSceneColor.IsNone()));
	}

	inline bool ShouldRenderWithMainViewFamily() const
	{
		return bMainViewFamily || bMainViewCamera || ShouldRenderWithMainViewResolution();
	}
