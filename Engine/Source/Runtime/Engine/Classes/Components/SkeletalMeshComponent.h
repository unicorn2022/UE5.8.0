...
template<>
struct TStructOpsTypeTraits<FSkeletalMeshComponentClothEndTickFunction> : public TStructOpsTypeTraitsBase2<FSkeletalMeshComponentClothEndTickFunction>
{
	enum
	{
		WithCopy = false
...
public:
	...
	// Blocks on the evaluation task (if necessary) and then returns a copy of the bone space
	// transforms. A copy is returned because the original array can/will be modified during the
	// evaluation, which can be running in a parallel task.
	ENGINE_API TArray<FTransform> GetBoneSpaceTransforms();

	// Blocks on the evaluation task (if necessary) and then returns bone space transforms as an
	// array view.
...
private:
	 ...
	 * This property is not safe to access directly during evaluation - see GetBoneSpaceTransforms() etc
	 */
	TArray<FTransform> BoneSpaceTransforms;
...
public:
     ...
     * @param OutValue (reference) Retrieved attribute value if found
	...
	*/
	...
	/**
	 * Get vector type attribute value.
	...
	*/
	UFUNCTION(BlueprintCallable, Category = CustomAttributes)
	ENGINE_API bool GetVectorAttribute_Ref(const FName& BoneName, const FName& AttributeName, UPARAM(ref) FVector& OutValue, ECustomBoneAttributeLookup LookupType = ECustomBoneAttributeLookup::BoneOnly);

	/**
	 * Get quaternion type attribute value.
	...
	*/
	UFUNCTION(BlueprintCallable, Category = CustomAttributes)
	ENGINE_API bool GetQuaternionAttribute_Ref(const FName& BoneName, const FName& AttributeName, UPARAM(ref) FQuat& OutValue, ECustomBoneAttributeLookup LookupType = ECustomBoneAttributeLookup::BoneOnly);
	...
	*/
	UFUNCTION(BlueprintCallable, Category = CustomAttributes)
	ENGINE_API bool GetVectorAttribute(const FName& BoneName, const FName& AttributeName, FVector DefaultValue, FVector& OutValue, ECustomBoneAttributeLookup LookupType = ECustomBoneAttributeLookup::BoneOnly);
	...
	*/
	UFUNCTION(BlueprintCallable, Category = CustomAttributes)
	ENGINE_API bool GetQuaternionAttribute(const FName& BoneName, const FName& AttributeName, FQuat DefaultValue, FQuat& OutValue, ECustomBoneAttributeLookup LookupType = ECustomBoneAttributeLookup::BoneOnly);
...
private:
	...
	/** Disable Anim Dynamics animation nodes and play original animation without simulation */
	UPROPERTY(EditAnywhere, Category = Physics)
	uint8 bDisableAnimDynamicsAnimNode:1;
...
public:
	...
	UE_DEPRECATED(5.8, "bCollideWithAttachedChildren is deprecated. Use CollisionSources to add external collisions instead.")
	UPROPERTY(BlueprintReadWrite, Category = Clothing, meta=(DeprecatedProperty, DeprecationMessage="CollideWithAttachedChildren is deprecated. Use CollisionSources to add external collisions instead."))
	...
	* It will instead be called from Autonomous networking updates. See ACharacter. */
	...
	/** Prevent notify events from being dispatched for this mesh. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = SkeletalMesh)
	uint8 bSuppressNotifyEventDispatch : 1;
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	/** 
	 * If true, next time that ConditionallyDispatchQueuedAnimEvents() is called it will trigger any 
	 * queued anim notifies available. Only to be used when not performing a regular anim instance update.
	 */
...
public:
	...
	*/
	...
	/**
	 * Sets whether or not to allow Anim Dynamics animation nodes for this component
	 */
	UFUNCTION(BlueprintCallable, Category = "Components|SkeletalMesh")
	ENGINE_API void SetAllowAnimDynamicsAnimNode(bool bInAllow, bool bReinitAnim = true);

	UFUNCTION(BlueprintCallable, Category = "Components|SkeletalMesh")
	bool GetAllowAnimDynamicsAnimNode() const { return !bDisableAnimDynamicsAnimNode; }

	/**
	 * Initialise animation nodes.
	 */
	UFUNCTION(BlueprintCallable, Category = "Components|SkeletalMesh")
	ENGINE_API void ReinitializeAnimNodes();
...
public:
	...
	FSkeletalMeshComponentClothEndTickFunction ClothEndTickFunction;
...
private:
	...
	friend FSkeletalMeshComponentClothEndTickFunction;
	...
	/** Whether the parallel cloth task wait is owned by the TG_LastDemotable end tick. Returns false to fall back to the legacy OnPreEndOfFrameSync wait. */
	bool ShouldUseClothEndTickWait() const;
...
private:
	...
	/** Called by OnRegister/OnPreRegister before calling Super. */
	void InitializeBeforeRegister();

	/** Called by OnRegister/OnPreRegister after calling Super. */
	void InitializeOnRegister();

	/** Shared code between synchronous and asynchronous phyiscs destruction. */
	void DestroyPhysicsStateInternal();
...
public:
	...
	*/
	...
	/** Tick only montages on the main anim instance to support legacy montages in UAF */
	ENGINE_API void TickMontagesOnly(float DeltaTime, bool bNeedsValidRootMotion);
	...
	*/
	...
	/** Computes a dense bit array of valid bone transforms, based on a list of required bones.
	 * The bit array is sized to the full number of bones of our RefSkeleton.
	 * This can be used to check our component space bone transforms for validity, without iterating over the full required bones list.
	 */
	ENGINE_API void ComputeValidMeshPoseTransforms(TArray<FBoneIndexType>& InRequiredBones, TBitArray<>& OutValidTransforms) const;
...
public:
	...
	//Handle registering our post cloth wait tick function (TG_LastDemotable)
	void RegisterClothEndTick(bool bRegister);
...
protected:
	ENGINE_API virtual bool AreBoneTransformsValid() const override;
...
public:
	...
	*/
	...
	/** Enable or Disable AngularPositionDrive. If motor is in SLERP mode it will be turned on if either bEnableSwingDrive OR bEnableTwistDrive are enabled. In Twist and Swing mode the twist and the swing can be controlled individually.*/
	UFUNCTION(BlueprintCallable, Category = "Physics")
	...
	/** Enable or Disable AngularVelocityDrive. If motor is in SLERP mode it will be turned on if either bEnableSwingDrive OR bEnableTwistDrive are enabled. In Twist and Swing mode the twist and the swing can be controlled individually.*/
	UFUNCTION(BlueprintCallable, Category = "Physics")
	...
	/** Enable or Disable AngularPositionDrive based on a list of bone names. If motor is in SLERP mode it will be turned on/off if either bEnableSwingDrive OR bEnableTwistDrive are enabled.   */
...
protected:
	...
	UE_DEPRECATED(5.8, "CopyClothCollisionsToChildren has been deprecated. Use CollisionSources to add external collisions instead.")
	void CopyClothCollisionsToChildren() {}

	/** copy children's cloth collisions to parent, where parent means this component */
	UE_DEPRECATED(5.8, "CopyChildrenClothCollisionsToParent has been deprecated. Use CollisionSources to add external collisions instead.")
	...
	UE_DEPRECATED(5.8, "FindClothCollisions has been deprecated. Use CollisionSources to add external collisions instead.")
	void FindClothCollisions(FClothCollisionData& OutCollisions) const {}
...
public:
	...
	virtual ENGINE_API void ParallelAnimationEvaluation();
...
protected:
	...
	// Handle post animation on all anim instances, whether or not an evaluation took place 
	ENGINE_API void DoInstanceFinalizeAnimation(bool bWasEvaluated);
...
private:
	...
	UPROPERTY(EditAnywhere, Category = AnimationRig, BlueprintGetter = GetDefaultAnimatingRigOverride, BlueprintSetter = SetDefaultAnimatingRigOverride, meta = (AllowedClasses = "/Script/ControlRigDeveloper.ControlRigBlueprint, /Script/ControlRig.ControlRigBlueprintGeneratedClass"), meta = (editcondition = "bOverrideDefaultAnimatingRig"))
...
private:
	...
	TArray<FConstraintInstance*> AsyncDestroyConstraintsPayload;
	FPhysicsAggregateHandle AggregatePayload;
...
private:
	bool bBoneTransformsInitialized = false;
};
