...
public:
	...
	UFUNCTION(BlueprintCallable, Category="Rendering|Material")
	ENGINE_API class UMaterialInterface* GetOverlayMaterial(bool bGetMaterialSlot = false, int32 SlotIndex = 0) const;

	/** Change the overlay material used by this instance. If SetMaterialSlot is enabled, the relative slot material is changed. */
	UFUNCTION(BlueprintCallable, Category="Rendering|Material")
	ENGINE_API void SetOverlayMaterial(class UMaterialInterface* NewOverlayMaterial, bool bSetMaterialSlot = false, int32 SlotIndex = 0);
...
protected:
	...
	ENGINE_API void SetParameterValueOnMaterials(const FName ParameterName
...
protected:
	...
	TSortedMap<FName, FMaterialParameterCache, FDefaultAllocator, FNameFastLess> MaterialSlotsOverlayMaterialParameterCache;
};
