...
protected:
	...
	/** Use a minimum material quality to determine if we enable the virtual texture on mobile. */
	UPROPERTY(EditAnywhere, AdvancedDisplay, Category = RuntimeVirtualTexture, Meta = (InlineEditConditionToggle))
	bool bUseMinMaterialQualityMobile = false;

	/** The minimum material quality for which we enable the virtual texture on mobile. Only affects In-Game and PIE. */
	UPROPERTY(EditAnywhere, AdvancedDisplay, Category = RuntimeVirtualTexture, Meta = (EditCondition = "bUseMinMaterialQualityMobile"))
	ERuntimeVirtualTextureMaterialQuality MinInGameMaterialQualityMobile = ERuntimeVirtualTextureMaterialQuality::Low;
