...
#include "ActorSkinnedMeshComponentInterface.h"
...
namespace UE::Anim
{
	...
	ENGINE_API extern bool bUseSafeMeshPoseIndices;
}
...
struct FSkelMeshComponentLODInfo
{
	...
	FSkinWeightVertexBuffer* OverrideSkinWeights = nullptr;

	/** Vertex buffer used to override skin weights from one of the profiles */
	FSkinWeightVertexBuffer* OverrideProfileSkinWeights = nullptr;
	
	ENGINE_API FSkelMeshComponentLODInfo();
	ENGINE_API ~FSkelMeshComponentLODInfo();

	ENGINE_API void SetActiveSkinWeightProfileStack(const FSkinWeightProfileStack& InProfileStack);
	ENGINE_API bool GetActiveSkinWeightProfileStack(FSkinWeightProfileStack& OutProfileStack) const;
...
private:
	...
	FSkinWeightProfileStack ProfileStackRequest;
};
...
class USkinnedMeshComponent : public UMeshComponent, public ILODSyncInterface, public IClothSimulationDataProvider
{
	...
	UE_DECLARE_COMPONENT_ACTOR_INTERFACE(SkinnedMeshComponent)
...
#if WITH_EDITOR
	/** Used to track down when SkinnedAsset has been modified for notification purpose */
	USkinnedAsset* KnownSkinnedAsset = nullptr;
#endif

#if WITH_EDITOR
	/** Called when the SkinnedAsset property gets overwritten without us knowing about it */
	ENGINE_API void OutdatedKnownSkinnedAssetDetected() const;
#endif

protected:
	ENGINE_API void NotifyIfSkinnedAssetChanged();
...
public:
	...
	/** Returns the MeshDeformerInstanceSettings pointer. (can be nullptr) */
	ENGINE_API UMeshDeformerInstanceSettings* GetMeshDeformerInstanceSettings() const;
...
private:
	ENGINE_API void UpdateSkinWeightProfileReadbackIfNeeded(int32 LODIndex);
	...
	/**
	 * Mesh Pose (sized to our skinned asset's ref skeleton) bit array of valid bone indices. 
	 * This should be populated by derived classes to avoid children copying invalid transforms through i.e. leader pose/copy pose.
	 */
	TBitArray<> ValidMeshPoseTransforms;
...
public:
	...
	TArray<int32>& GetMutableLeaderBoneMap() { return LeaderBoneMap; }
...
public:
	...
	/** Forces this component to use fallback (non-Nanite) rendering even if the asset has Nanite resources. */
	UPROPERTY(EditAnywhere, AdvancedDisplay, BlueprintReadWrite, Category = Rendering)
	...
	bool IsDisallowNanite() const { return bDisallowNanite; }
	bool IsForceDisableNanite() const { return bForceDisableNanite; }
	int32 GetWorldPositionOffsetDisableDistance() const { return WorldPositionOffsetDisableDistance; }
	bool EvaluateWorldPositionOffset() const { return bEvaluateWorldPositionOffset; }
...
protected:
	...
	/** used to cache our valid mesh pose transform list for the current animation update. */
	uint8 bValidMeshPoseTransformsUpToDate : 1;
	...
	/** Whether actual Skin Weight Profile buffer is currently pending override */
	uint8 bSkinWeightProfileBufferOverridePending : 1;
public:
	/** Do skin weight profile management on render thread exclusively */
	UPROPERTY(EditAnywhere, AdvancedDisplay, BlueprintReadWrite, Category = SkeletalMesh)
	uint8 bSkinWeightProfileOnRenderThread: 1;
...
public:
	...
	ENGINE_API void SetEvaluateWorldPositionOffset(bool bNewValue);

	/** Set the distance beyond which World Position Offset is suppressed for this component (0 disables). */
	UFUNCTION(BlueprintCallable, Category = "Rendering|LOD")
	...
	*/
	...
	ENGINE_API bool IsSkinCacheAllowed(int32 LodIdx, ERHIFeatureLevel::Type FeatureLevel) const;
...
public:
	...
	ENGINE_API virtual void OnRenderAssetFirstLodChange(const UStreamableRenderAsset* RenderAsset, int32 FirstLodIndex) override;
	...
	*/
	...
	bool IsSkinWeightProfileManagementOnRenderThread() const;
...
protected:
	...
	inline TBitArray<>& GetMutableValidMeshPoseTransforms() { return ValidMeshPoseTransforms; }
...
public:
	 ...
	 */
	UE_DEPRECATED(5.8, "Duplicate transform control will be removed.")
...
public:
	...
	inline const TBitArray<>& GetValidMeshPoseTransforms() const { return ValidMeshPoseTransforms; }
...
protected:
	/**
	 * Returns whether the bone transforms are valid.
	 */
	ENGINE_API virtual bool AreBoneTransformsValid() const;
...
public:
	...
	/**
	 * Tests if BoneName is child of ParentBoneName.
	 *
	 * @param BoneName Name of the bone
	 * @param ParentBone Name to check
	 *
	 * @return true if child (strictly, not same). false otherwise
	 * Note - will return false if ChildBoneIndex is the same as ParentBoneIndex i.e. must be strictly a child.
	 */
	UFUNCTION(BlueprintCallable, Category="Components|SkinnedMesh")
...
public:
		...
		bWasRenderStateCreated(Component->IsRenderStateCreated())
	{
		if (bWasRenderStateCreated)
		{
			...
			{
				UE_LOGF(LogSkeletalMesh, Warning, "Created a FRenderStateRecreator with an unregistered component: %ls", *Component->GetPathName());
			}
