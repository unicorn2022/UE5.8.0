 ...
 */
UCLASS(ClassGroup=Utility, ShowCategories = (Mobility), HideCategories = (Activation, Physics, Collision, HLOD, LOD, TextureStreaming, Navigation, RayTracing, Lighting, Rendering, Mobile), meta=(BlueprintSpawnableComponent), MinimalAPI)
...
public:
	...
	/** Get the input key of the control point of the spline at the specified index. */
	UFUNCTION(BlueprintCallable, Category = Spline)
	ENGINE_API float GetInputKeyValueAtSplinePoint(int32 PointIndex) const;

	/** Set the input key of the control point of the spline at the specified index. */
	UFUNCTION(BlueprintCallable, Category = Spline)
	ENGINE_API void SetInputKeyValueAtSplinePoint(int32 PointIndex, float InputKey, bool bUpdateSpline = true);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Spline|Metadata")
	ENGINE_API int32 AddFloatPropertyAtSplineInputKey(float InKey, float Value, FName PropertyName);
...
public: // Deprecated Interface
	...
	UE_DEPRECATED(5.8, "SynchronizeSplines is deprecated.")
	void SynchronizeSplines() {}

	UE_DEPRECATED(5.8, "PopulateFromLegacy is deprecated.")
	void PopulateFromLegacy() {}

	UE_DEPRECATED(5.8, "SetFloatPropertyAtSplineInputKey is deprecated, please use AddFloatPropertyAtSplineInputKey")
	UFUNCTION(BlueprintCallable, Category = "Spline|Metadata", meta = (DeprecatedFunction, DeprecationMessage = "Please use AddFloatPropertyAtSplineInputKey"))
	int32 SetFloatPropertyAtSplineInputKey(float InKey, float Value, FName PropertyName) const { return INDEX_NONE; }
...
private:
	...
	/** Whether or not FSplineCurves is the authoritative data. If false, we assume Spline is implemented as FTangentSpline. */
	ENGINE_API bool ShouldUseSplineCurves() const;
	...
	static ENGINE_API const FInterpCurveVector DummyPointPosition;
	static ENGINE_API const FInterpCurveQuat DummyPointRotation;
	static ENGINE_API const FInterpCurveVector DummyPointScale;
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	ENGINE_API bool Validate() const;
	...
	// FSplinePointDetails will access ShouldUseSplineCurves for enabling/disabling some editing functionality
	friend class FSplinePointDetails;

	// AFunctionalSplineTestBase accesses private methods for functional testing.
	friend class AFunctionalSplineTestBase;
};
