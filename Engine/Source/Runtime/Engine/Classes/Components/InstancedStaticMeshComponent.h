...
private:
	...
	/** Set in OnAsyncCreatePhysicsStateBegin_GameThread to signal that BodyInstance.WalkableSlopeOverride
	 *  was pre-cached from BodySetup on GT, so the worker-side FInstancedMeshComponentBodies::CreateAll
	 *  must skip its own (potentially racy) read of BodySetup->WalkableSlopeOverride during PIE.
	 *  Cleared in OnAsyncCreatePhysicsStateEnd_GameThread. In cooked, BodySetup is immutable
	 *  post-PostLoad and the worker read is uncontested, so this flag is unneeded. */
	uint8 bWalkableSlopeOverrideCachedGT : 1;
#endif
...
public:
	...
	*/
	...
	/** Iterates on instance transform (FMatrix) */
...
public:
	...
	/** Returns the per-instance physics bodies array. */
	ENGINE_API const TArray<FBodyInstance*>& GetInstanceBodies() const;

	UE_DEPRECATED(5.8, "This member is no longer used. Use GetInstanceBodies() instead.")
...
public:
	...
	ENGINE_API virtual void PartialNavigationUpdate(int32 InstanceIdx) final;

	/** Request to navigation system to update only part of navmesh occupied specified instances transforms. */
	ENGINE_API virtual void PartialNavigationUpdates(TConstArrayView<FTransform> InstanceTransforms) final;
...
protected:
	...
	UE_DEPRECATED(5.8, "Navigation bounds are now lazily updated and no longer require explicit calls to update them.")
	...
	UE_DEPRECATED(5.8, "Use InstancePhysicsBodies instead.")
