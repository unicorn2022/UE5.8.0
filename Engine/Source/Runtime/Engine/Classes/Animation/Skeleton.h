...
public:
	...
	UE_DEPRECATED(5.7, "CachedSoftObjectPtr is no longer used and has been deprecated.")
...
protected:
...
#if WITH_EDITOR
	DECLARE_TS_MULTICAST_DELEGATE( FOnSkeletonHierarchyChangedMulticaster );
	FOnSkeletonHierarchyChangedMulticaster OnSkeletonHierarchyChanged;

	DECLARE_TS_MULTICAST_DELEGATE( FOnReferenceSkeletonChangedMulticaster );
	FOnReferenceSkeletonChangedMulticaster OnReferenceSkeletonChanged;
	...
	/** Call this when the reference skeleton has changed to fix dependent assets */
	ENGINE_API void HandleReferenceSkeletonChange() const;

public:
	typedef FOnSkeletonHierarchyChangedMulticaster::FDelegate FOnSkeletonHierarchyChanged;
	typedef FOnReferenceSkeletonChangedMulticaster::FDelegate FOnReferenceSkeletonChanged;

	/** Registers a delegate to be called after the skeleton hierarchy has changed */
	...
	/** Registers a delegate to be called after the reference skeleton has changed */
	ENGINE_API void RegisterOnReferenceSkeletonChanged(const FOnReferenceSkeletonChanged& Delegate);
	ENGINE_API void UnregisterOnReferenceSkeletonChanged(FDelegateUserObject Unregister);
