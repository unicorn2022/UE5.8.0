...
class UAnimationSettings : public UDeveloperSettings
{
	...
	*/
	...
	/** When a mirror pair is not found map to self instead of none.  */
	UPROPERTY(config, EditAnywhere, Category = Mirroring)
	bool bOnMirrorPairFindReplaceFailedMapToSelf = true;

	/** When enabled, automatically validates the mirror data table when it is opened in the editor. Only shows a notification if issues are found. */
	UPROPERTY(config, EditAnywhere, Category = Mirroring)
	bool bValidateMirrorDataTableOnOpen = true;
