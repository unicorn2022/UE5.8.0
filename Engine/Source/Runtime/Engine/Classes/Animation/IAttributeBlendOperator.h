...
namespace UE
{
	namespace Anim
	{
		...
		{
		public:
			virtual ~IAttributeBlendOperator() = default;
			...
			/** Invoked when an attribute container A is accumulated into container B with additional per attribute weight overrides */
			virtual void AccumulateWithOverrideWeights(const FAttributeBlendData& BlendData, FStackAttributeContainer* OutAttributes) const = 0;
