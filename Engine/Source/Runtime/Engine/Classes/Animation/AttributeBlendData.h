...
namespace UE
{
	...
	{	
		...
		protected:
			...
			static FAttributeBlendData SingleAdditiveContainerWithOverrideWeights(const TArrayView<const FStackAttributeContainer* const> SourceAttributes, const TMap<UE::Anim::FAttributeId, float>& InAttributeWeightOverrides, const float InUniformWeight, EAdditiveAnimationType AdditiveType, const UScriptStruct* AttributeScriptStruct)
			{
				ensure(SourceAttributes.Num() == 1);
				return FAttributeBlendData(SourceAttributes, InAttributeWeightOverrides, InUniformWeight, AdditiveType, AttributeScriptStruct);
			}
		...
		private:
			...
			// Additive accumulate using a single weight plus optional per attribute override weights 
			ENGINE_API FAttributeBlendData(TArrayView<const FStackAttributeContainer* const> SourceAttributes, const TMap<UE::Anim::FAttributeId, float>& InAttributeWeightOverrides, const float InUniformWeight, EAdditiveAnimationType InAdditiveType, const UScriptStruct* AttributeScriptStruct);
		...
		private:
			...
			/* Contains optional per attribute weight overrides */
			TMap<UE::Anim::FAttributeId, float> AttributeWeightOverrides;
		...
		public:
			...
			public:
				...
				const float GetWeightOverride(const FAttributeId& InAttribute) const
				{
					const float* FoundWeight = Data.AttributeWeightOverrides.Find(InAttribute);
					if (FoundWeight)
					{
						return *FoundWeight; 
					}
