...
public:
	...
	*/
	...
	/**
	* Sets keys for an existing bone animation track with the provided name. Broadcasts a EAnimDataModelNotifyType::TrackChanged notify if successful.
...
protected:
	...
	virtual bool ShouldTransact(bool bInLocalShouldTransact) const = 0;
...
// Add this to subclasses for change transactor. Note, this code uses UnrealEd types and this file is in Engine
// Needs to include "ChangeTransactor.h"
#define DECLARE_TRANSACTION_CONTROLLER \
		...
		{ \
			if (UE::FChangeTransactor::CanTransactChanges() && ShouldTransact(bCondition)) \
			{  \
		...
		{	 \
			return MakeUnique<IAnimationDataController::FScopedBracket>(this, Description, UE::FChangeTransactor::CanTransactChanges() && ShouldTransact(bCondition));	 \
		} \
