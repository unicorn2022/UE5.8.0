...
USTRUCT()
struct FPoseAssetInfluenceForPose
{
	GENERATED_USTRUCT_BODY()
...
/**
 * Intermediary Interface class to be implemented to extract information from a pose asset.
 * This is used for UPoseAsset::GetAnimationPose, for example.
 */
...
public:
	...
	/**
	 * The number of pose curves available in this context
	 * @return The number of pose curves available in this context
	 */
	virtual int32 NumPoses() const = 0;

	/**
	 * Retrieves the name of the pose given an index.
	 * @param InPoseIndexInContext The index of the pose to return the name of
	 * @return The name of the pose given the index
	 */
	virtual const FName& GetPoseName(const int32& InPoseIndexInContext) const = 0;

	/**
	 * Retrieves the index of the pose in the pose asset given the index of the pose within the context.
	 * @param InPoseIndexInContext The index of the pose within the context
	 * @return The index of the pose in the pose asset
	 */
	virtual int32 GetPoseIndexInPoseAsset(const int32& InPoseIndexInContext) const = 0;

	/**
	 * Retrieves the weight / value of a pose in the context
	 * @param InPoseIndexInContext The index of the pose in the context
	 * @return The weight / value of a pose in the context
	 */
	virtual float GetPoseWeight(const int32& InPoseIndexInContext) const = 0;

	/**
	 * Retrieves the pose stored in the context as a FPoseCurve
	 * @param InPoseIndexInContext The index of the pose in the context
	 * @return The FPoseCurve representing the stored pose in the context
	 */
	virtual FPoseCurve GetPoseCurve(const int32& InPoseIndexInContext) const
	{
		return FPoseCurve(
			GetPoseIndexInPoseAsset(InPoseIndexInContext),
			GetPoseName(InPoseIndexInContext),
			GetPoseWeight(InPoseIndexInContext)
		);
	}

	/**
	 * Returns the number of curves stored in the context
	 * @return The number of curves stored in the context
	 */
	virtual int32 NumCurves() const = 0;

	/**
	 * Blends an existing blended curve onto this context given a weight
	 * @param InCurve The curve to blend onto this context
	 * @param InWeight The weight to use when blending
	 */
	virtual void BlendPoseCurve(const FBlendedCurve& InCurve, float InWeight) = 0;

	/**
	 * Returns the target skeleton this context is using
	 * @param bEvenIfUnreachable If true the skeleton will be returned using a most expensive lookup if the cached weakptr is invalid
	 * @return The target skeleton this context is using
	 */
	virtual const USkeleton* GetTargetSkeleton(bool bEvenIfUnreachable = false) const = 0;

	/**
	 * Returns true if a given bone is required to be computed.
	 * @param InOutputBoneIndex The bone index within the context
	 * @return True if a given bone is required to be computed.
	 */
	virtual bool IsOutputBoneRequired(const int32& InOutputBoneIndex) const = 0;

	/**
	 * Returns the bone index within the output pose of this context given the skeleton pose bone index
	 * @param InSkeletonBoneIndex The index of the bone in the skeleton's pose
	 * @return The bone index within the output pose
	 */
	virtual int32 GetOutputPoseBoneIndexFromSkeletonPoseBoneIndex(const int32& InSkeletonBoneIndex) const = 0;

	/**
	 * Returns the curve filter to apply when blending curves (or nullptr)
	 * @return The curve filter to apply when blending curves (or nullptr)
	 */
	virtual const UE::Anim::FCurveFilter* GetCurveFilter() const = 0;

	/**
	 * Resets the output pose to additive identity (with zero scale)
	 */
	virtual void ResetPoseToAdditiveIdentity() = 0;

	/**
	 * Resets the output pose to the reference pose
	 */
	virtual void ResetPoseToRefPose() = 0;

	/**
	 * Returns the transform within the output pose given a bone index
	 * @param InOutputBoneIndex The index of the bone within this context
	 * @return The transform within the output pose given a bone index
	 */
	virtual FTransform& GetOutputTransform(const int32& InOutputBoneIndex) = 0;

	/**
	 * Returns the context to use for FAnimationRuntime::RetargetBoneTransform
	 * @return The context to use for FAnimationRuntime::RetargetBoneTransform
	 */
	virtual const FAnimationRuntimeRetargetBoneTransformContext* GetRetargetBoneTransformContext() const = 0;
};
...
private:
	...
	ENGINE_API void BlendPoseCurve(const FPoseData* PoseData, FPoseAssetExtractionContext* InOutContext, float Weight) const;
...
public: 
	...
	*/
	...
	ENGINE_API bool GetAnimationPose(struct FAnimationPoseData& OutAnimationPoseData, const FAnimExtractContext& ExtractionContext, const bool bIgnoreSourceAnimationCurves = false, const bool bAllowNegativeWeights = false) const;
	ENGINE_API bool GetAnimationPose(FPoseAssetExtractionContext* InOutContext, const bool bIgnoreSourceAnimationCurves = false, const bool bAllowNegativeWeights = false) const;
...
public:
...
#if WITH_EDITOR
	// Return full / absolute curve values. Will do conversion if PoseAsset is Additive. 
	ENGINE_API const TArray<float>& GetFullCurveValues(const int32 PoseIndex) const;
#endif
	// Return relative / additive curve values. Will give access to raw data and won't do conversion.
	// This is the same as GetCurveValues
	ENGINE_API const TArray<float>& GetRawCurveValues(const int32 PoseIndex) const;
	...
	ENGINE_API const TArray<FTransform>& GetFullPose(int32 PoseIndex) const;
#endif

	// Return local (local space, additive) pose. Will give access to raw data and won't do conversion 
	ENGINE_API bool GetRawPose(int32 PoseIndex, TArray<FTransform>& OutTransforms) const;
	ENGINE_API const TArray<FTransform>& GetRawPose(int32 PoseIndex) const;
	...
	ENGINE_API const FTransform& GetLocalSpaceTransform(int32 TrackIndex, int32 PoseIndex = 0) const;
	...
	ENGINE_API bool GetBasePoseTransform(TArray<FTransform>& OutBasePose, TArray<float>& OutBaseCurve) const;
	ENGINE_API bool GetTrackInfluencesForPose(const int32 PoseIndex, TArray<FPoseAssetInfluenceForPose>& OutInfluences) const;
	ENGINE_API bool GetTrackInfluencesForPoses(const TArrayView<const FPoseCurve>& InPoses, TArray<FPoseAssetInfluenceForPose>& OutInfluences) const;
	ENGINE_API bool ConvertToFullPose(const int32 PoseIndex, TArray<FTransform>& OutFullPose, TArray<FPoseAssetInfluenceForPose>& OutInfluences, TArray<float>& OutFullCurves) const;
	ENGINE_API bool ConvertToFullPose(const int32 PoseIndex, TArray<FTransform>& OutFullPose, TArray<FPoseAssetInfluenceForPose>& OutInfluences) const;
	ENGINE_API bool ConvertToFullPose(const int32 PoseIndex, TArray<float>& OutFullCurves) const;
