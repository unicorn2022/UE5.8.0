...
namespace UE
{
	namespace Anim
	{
		...
		public:
			...
			virtual void AccumulateWithOverrideWeights(const FAttributeBlendData& BlendData, FStackAttributeContainer* OutAttributes) const final { AccumulateWithOverrideWeights_Internal<AttributeType>(BlendData, OutAttributes); }
		...
		protected:
			...
			/* Accumulate operation for blendable attribute types with optional per attribute weight overrides */
			template <typename Type>
			inline typename TEnableIf<TAttributeTypeTraits<Type>::IsBlendable, void>::Type AccumulateWithOverrideWeights_Internal(const FAttributeBlendData& BlendData, FStackAttributeContainer* OutAttributes) const
			{
				BlendData.ForEachUniqueAttribute<AttributeType>([OutAttributes](typename FAttributeBlendData::template TSingleIterator<AttributeType>& It) -> void
				{
					while (It.Next())
					{
						...
						float AttributeWeight = It.GetWeightOverride(Identifier);
						if (AttributeWeight <= -1.0)
						{
							AttributeWeight = It.GetWeight();
						}
			...
			/* Accumulate operation for non-blendable attribute types with optional per attribute weight overrides */
			template <typename Type>
			inline typename TEnableIf<!TAttributeTypeTraits<Type>::IsBlendable, void>::Type AccumulateWithOverrideWeights_Internal(const FAttributeBlendData& BlendData, FStackAttributeContainer* OutAttributes) const
			{
				BlendData.ForEachUniqueAttribute<AttributeType>([OutAttributes](typename FAttributeBlendData::template TSingleIterator<AttributeType>& It) -> void
		...
		public:
			...
			virtual void AccumulateWithOverrideWeights(const FAttributeBlendData& BlendData, FStackAttributeContainer* OutAttributes) const final
			{
				BlendData.ForEachUniqueAttribute([&, OutAttributes](FAttributeBlendData::TSingleRawIterator& It) -> void
				{
					while (It.Next())
