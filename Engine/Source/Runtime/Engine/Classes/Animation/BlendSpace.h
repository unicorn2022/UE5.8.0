...
struct FBlendSample
{
	...
	{
		return (Other.Animation == Animation && Other.SampleValue == SampleValue && Other.bUseSingleFrameForBlending == bUseSingleFrameForBlending && bMirror == Other.bMirror && Other.FrameIndexToSample == FrameIndexToSample && FMath::IsNearlyEqual(Other.RateScale, RateScale));
	}
...
public:
	...
	ENGINE_API TConstArrayView<FAnimSyncMarker> GetAuthoredSyncMarkers() const;
...
protected:
	...
	*/
	...
	UE_DEPRECATED(5.8, "Please use TickFollowerSamples() that takes in the blendspace's tick record.")
	...
	void TickFollowerSamples(TArray<FBlendSampleData> &SampleDataList, const int32 HighestWeightIndex, FAnimAssetTickContext &Context, 
...
private:
	...
	/** Gets the appropriate mirror data table for tick record's blend sample. */
	const UMirrorDataTable* GetEffectiveMirrorDataTableForSample(const FAnimTickRecord& InInstance, const FBlendSample& InBlendSample) const;
	
	/** Gets the appropriate mirror data table for a blend sample taking into account any upstream mirror requests */
	const UMirrorDataTable* GetEffectiveMirrorDataTableForSample(const FBlendSample& InBlendSample, const UMirrorDataTable* InUpStreamMirrorDataTable = nullptr) const;
...
public:
	...
	*/
	...
	/** Enable to use the old method for calculating the sample point's effective animation length.
	The legacy method was to calculate each sample's effective animation length and perform a weighted blend to calculate the animation length for the preview.
	This means the result is non-linear with respect to each sample's play rate as play rate is the inverse of animation length.
	The new method is to perform a weighted blend on the play rates directly and then derive the animation length.
	*/
	UPROPERTY(EditAnywhere, Category = Animation)
	bool bUseLegacySamplePointAnimationLengthCalculations = false;
	...
	*/
	...
	// Used to mirror any blend samples with bMirror set to true
	UPROPERTY(Category=Mirroring, EditAnywhere)
