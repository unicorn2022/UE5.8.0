...
#include "AnimEnums.h"
#include "Animation/AnimTraceDefines.h"
...
struct FBlendSampleData
{
	...
	static void ENGINE_API NormalizeDataWeight(const TArrayView<FBlendSampleData*>& SampleDataList);
};
...
struct FAnimTickRecord
{
	...
	// Source proxy for deferred tracing - captures the originating anim instance proxy at tick record
	// creation time so that linked graph tick records are correctly attributed during the deferred trace loop.
	// Raw pointer is safe here: proxy lifetime is tied to its UAnimInstance which outlives the tick record.
