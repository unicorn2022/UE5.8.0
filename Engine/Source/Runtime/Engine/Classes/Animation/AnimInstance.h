...
// Experimental 5.8, FComputeDeltaFrameTime is experimental, and may change in the future.
DECLARE_DELEGATE_RetVal_ThreeParams(float, FComputeDeltaFrameTime, int32 /*InstanceId*/, float /*CurrentTime*/, float /*DeltaTime*/)
...
class UAnimInstance : public UObject
{
	...
	UE_DEPRECATED(5.7, "Public access to bNeedsUpdate is deprecated, and it will become private in a future release. Please update code to use the NeedsUpdate() getter instead.")
...
private:
	...
	uint8 bUpdateAnimationEnabled : 1;
	...
	uint8 bPostUpdatingAnimation : 1;

	/** Used to guard against recursive calls to UpdateAnimation */
	uint8 bUpdatingAnimation : 1;
#endif

	/** Used to prevent firing notifies queued from the anim graph if we have already done the animation update this tick (non-autonomous). */
	uint8 bHasDispatchedAnimGraphNotifiesThisTick : 1;

	/** Used to prevent firing notifies queued from montages if we have already done the montage animation update this tick (non-autonomous). */
	uint8 bHasDispatchedMontageNotifiesThisTick : 1;
...
public:	
	...
	/**  Experimental: Returns the delegate if Montage is found, otherwise returns first one found */
	UE_EXPERIMENTAL(5.8, "Experimental custom way to drive the montage frame time forward, API may change in the future")
	ENGINE_API FComputeDeltaFrameTime* Montage_GetComputeFrameDeltaTimeDelegate(const UAnimMontage* Montage = nullptr);
...
public:
	...
	/**  Returns active inertialization requests on the anim instance (does not forward the call to the proxy), used for legacy montage support in UAF */
	ENGINE_API const TMap<FName, FInertializationRequest>& GetActiveInertializationRequests() const;
	
	/**  Clears all active inertialization requests on the anim instance (does not forward the call to the proxy), used for legacy montage support in UAF */
	ENGINE_API void ClearActiveInertializationRequests();
...
public:
	...
	bool IsPostUpdatingAnimation() const { return static_cast<bool>(bPostUpdatingAnimation); }

	/** Is this animation currently running update */
	bool IsUpdatingAnimation() const { return static_cast<bool>(bUpdatingAnimation); }
#endif
...
public:
	...
	*/
	UFUNCTION(BlueprintPure, Category="Animation|Curves", meta=(BlueprintThreadSafe))
	ENGINE_API bool GetCurveValueWithDefault(FName CurveName, float DefaultValue, float& OutValue) const;
	...
	ENGINE_API FName GetCurrentStateName(int32 MachineIndex) const;
	...
	UFUNCTION(BlueprintCallable, Category = "Animation|Synchronization")
	ENGINE_API void ResetSync();
...
public:
	...
	* See Class UAnimBlueprintPostCompileValidation. */
	...
	{
		/**
		 * Uses the global dispatch policy (a.Notifies.DispatchPolicy) for eligibility:
		 *   1: only trigger notifies from sources (anim graph or montage) that updated before the trigger request.
		 *   0: trigger notifies from all sources.
		 */
		Default,

		/** Override: Only trigger notifies queued by the anim graph. Ignores global dispatch policy. */
		ForceAnimGraphOnly,

		/** Override: Only trigger notifies queued by montages. Ignores global dispatch policy. */
		ForceMontageOnly,

		/** Override: Trigger notifies from all sources (anim graph/montages). Ignores global dispatch policy. */
		ForceAllSources,
	};
	...
	ENGINE_API void InitializeMontageOnly();
	...
	// Called post evaluation if an evaluation took place
	ENGINE_API void PostEvaluateAnimation();
	// Called after PostEvaluateAnimation, whether or not an evaluation took place
	ENGINE_API void FinalizeAnimation(bool bWasEvaluated);
	...
	// Native Finalize override point
	ENGINE_API virtual void NativeFinalizeAnimation(bool bWasEvaluated);
...
public:
	...
	/** ID for this anim instance */
	uint32 AnimInstanceID = 0;
...
public:
	...
	*/
	...
	ENGINE_API void TriggerAnimNotifies(float DeltaSeconds, ENotifyTriggerMode Mode = ENotifyTriggerMode::Default);
...
public:
	ENGINE_API void TickMontageOnly(float DeltaSeconds);
