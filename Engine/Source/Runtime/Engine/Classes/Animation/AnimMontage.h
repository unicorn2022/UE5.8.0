...
/* Delegate allows a c++ client to get a callback when it's time to advance the montage frame time.
 * It then returns the DeltaTime to use. Currently used to drive montages according to a music clock timeline
 * rather than advancing through world time. Note that playrate both at the montage and segment level
 * are basically ignored by this system, because the current use does not require it.
 */
...
struct FAnimMontageInstance
{
	...
	UE_EXPERIMENTAL(5.8, "FComputeDeltaFrameTime is experimental, and may change in the future.")
	FComputeDeltaFrameTime OnComputeDeltaFrameTime;
...
private:
	...
	TOptional<float> ForcedLoopBoundaryPosition;
	TOptional<float> ForcedLoopResumePosition;
