...
void FUserDefinedChords::SaveChords() const
{
	if( Chords.IsValid() )
	{
		...
		for(const TPair<FUserDefinedChordKey, FInputChord>& ChordInfo : *Chords)
		{
			...
			ChordInfoObj->SetField( TEXT("BindingContext"), MakeShareable( new FJsonValueString( ChordInfo.Key.BindingContext.ToString() ) ) );
			ChordInfoObj->SetField( TEXT("CommandName"), MakeShareable( new FJsonValueString( ChordInfo.Key.CommandName.ToString() ) ) );
			ChordInfoObj->SetField(TEXT("ChordIndex"), MakeShareable(new FJsonValueNumber(static_cast<uint8>(ChordInfo.Key.ChordIndex))));
			ChordInfoObj->SetField( TEXT("Control"), MakeShareable( new FJsonValueBoolean( ChordInfo.Value.NeedsControl() ) ) );
			ChordInfoObj->SetField( TEXT("Alt"), MakeShareable( new FJsonValueBoolean( ChordInfo.Value.NeedsAlt() ) ) );
			ChordInfoObj->SetField( TEXT("Shift"), MakeShareable( new FJsonValueBoolean( ChordInfo.Value.NeedsShift() ) ) );
			ChordInfoObj->SetField( TEXT("Command"), MakeShareable( new FJsonValueBoolean( ChordInfo.Value.NeedsCommand() ) ) );
			ChordInfoObj->SetField( TEXT("Key"), MakeShareable( new FJsonValueString( ChordInfo.Value.Key.ToString() ) ) );
		...
		// Disable file operations to prevent the .ini from being deleted when all the sections are emptied below
		const bool bFileOperationsDisabled = GConfig->AreFileOperationsDisabled();
		if (!bFileOperationsDisabled)
		{
			GConfig->DisableFileOperations();
		}

		// Remove all sections not related to keybinds...
		TArray<FString> SectionNames;
		if (GConfig->GetSectionNames(GEditorKeyBindingsIni, SectionNames))
		{
			for (const FString& SectionKey : SectionNames)
			{
				GConfig->EmptySection(*SectionKey, GEditorKeyBindingsIni);
			}
		}
		...
		// Flush the .ini to ensure all new key bindings are saved
		const bool bRemoveFromCache = false;
		GConfig->Flush(bRemoveFromCache, GEditorKeyBindingsIni);
	}
}
	...
	for (uint32 i = 0; i < static_cast<uint8>(EMultipleKeyBindingIndex::NumChords); ++i)
	{
		...
		if (ExistingInfo.IsValid())
		{
			if (ExistingInfo->CommandName != InCommandInfo->CommandName)
			{
				...
				UE_LOGF(LogSlate, Warning, "The command '%ls.%ls' has the same default chord as '%ls.%ls' [%ls]",
		...
		else if (bFoundUserDefinedChord)
		{
			...
			if (ExisingCommandName != NAME_None)
			{
				...
				if (*PreviousInfo->ActiveChords[i] != PreviousInfo->DefaultChords[i])
				{
					...
					UE_LOGF(LogSlate, Error, "Duplicate user defined chords found: [%ls,%ls].  Chord for %ls being removed", *InCommandInfo->GetLabel().ToString(), *PreviousInfo->GetLabel().ToString(), (ExistingInfo  ? *ExistingInfo->GetLabel().ToString() : TEXT("")));
				}
...
void FInputBindingManager::PrintAllInputCommands(bool bBoundOnly)
{
	...
	UE_LOGF(LogSlate, Log, "UIContext List  ---------------");
	...
	{	
		...
		if (!bBoundOnly)
		{
			...
			{
				UE_LOGF(LogSlate, Log, "%ls", *ContextName.ToString());
			}
		}
	}

	UE_LOGF(LogSlate, Log, "UICommand List ---------------");
	for (const FName& ContextName : ContextNames)
	{
		...
		{
			UE_LOGF(LogSlate, Log, "%ls.%ls", *ContextName.ToString(), *CommandName);
		}
	}
#endif // WITH_EDITOR
}
