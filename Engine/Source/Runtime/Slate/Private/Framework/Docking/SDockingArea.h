...
public:
	 ...
	 */
	SLATE_API bool TryOpenSidebarDrawer(TSharedRef<SDockTab> TabToModify) const;

	/**
	 * Attempts to dismiss a sidebar drawer that might be open for the tab
	 *
	 * @true if the drawer was dismissed, false if the tab is pinned or not in a drawer
	 */
	SLATE_API bool TryDismissSidebarDrawer(TSharedRef<SDockTab> TabToModify) const;

	/**
	 * Attempts to toggle a sidebar drawer that may own the given tab
	 *
	 * @true if the drawer toggled, false if not found or some other setting prevented toggling
	 */
	SLATE_API bool TryToggleSidebarDrawer(TSharedRef<SDockTab> TabToModify) const;
