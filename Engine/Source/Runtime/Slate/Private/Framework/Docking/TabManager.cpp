...
static const FString UE_TABMANAGER_OPENED_TAB_STRING = "OpenedTab";
static const FString UE_TABMANAGER_CLOSED_TAB_STRING = "ClosedTab";
static const FString UE_TABMANAGER_SIDEBAR_TAB_STRING = "SidebarTab";
static const FString UE_TABMANAGER_INVALID_TAB_STRING = "InvalidTab";
...
TSharedRef<FTabManager::FLayoutNode> FTabManager::FLayout::NewFromString_Helper( TSharedPtr<FJsonObject> JsonObject )
{
	struct local
	{
		static FTabManager::FArea::EWindowPlacementInternal PlacementFromString( const FString& AsString )
		{
			if (AsString == GET_ENUMERATOR_NAME_STRING_VIEW_CHECKED(FTabManager::FArea::EWindowPlacementInternal, Placement_NoWindow))
			{
				return FTabManager::FArea::EWindowPlacementInternal::Placement_NoWindow;
			}
			else if (AsString == GET_ENUMERATOR_NAME_STRING_VIEW_CHECKED(FTabManager::FArea::EWindowPlacementInternal, Placement_Automatic))
			{
				return FTabManager::FArea::EWindowPlacementInternal::Placement_Automatic;
			}
			else if (AsString == GET_ENUMERATOR_NAME_STRING_VIEW_CHECKED(FTabManager::FArea::EWindowPlacementInternal, Placement_Specified))
			{
				return FTabManager::FArea::EWindowPlacementInternal::Placement_Specified;
			}
			else if (AsString == GET_ENUMERATOR_NAME_STRING_VIEW_CHECKED(FTabManager::FArea::EWindowPlacementInternal, Placement_ParentSpecified))
			{
				return FTabManager::FArea::EWindowPlacementInternal::Placement_ParentSpecified;
			}
			else
			{
		...
		{
			static const FString Orient_Horizontal_Str = "Orient_Horizontal";
			static const FString Orient_Vertical_Str = "Orient_Vertical";
	...
	if (NodeType == TEXT("Area"))
	{
		...
		FTabManager::FArea::EWindowPlacementInternal WindowPlacement = local::PlacementFromString( JsonObject->GetStringField(TEXT("WindowPlacement")) );
		switch( WindowPlacement )
		{
			default:
			case FTabManager::FArea::EWindowPlacementInternal::Placement_NoWindow:
			{
				NewArea = FTabManager::NewPrimaryArea();
			}
			break;

			case FTabManager::FArea::EWindowPlacementInternal::Placement_Automatic:
			{
			...
			case FTabManager::FArea::EWindowPlacementInternal::Placement_ParentSpecified:
			case FTabManager::FArea::EWindowPlacementInternal::Placement_Specified:
			{
				...
				if (WindowPlacement == FTabManager::FArea::EWindowPlacementInternal::Placement_ParentSpecified)
				{
					NewArea->WindowPlacement = FTabManager::FArea::EWindowPlacementInternal::Placement_ParentSpecified;
				}
			}
			break;
		}
	...
	else if ( NodeType == TEXT("Stack") )
	{
		...
		{
			double MinSize = 0.0;
			if (JsonObject->TryGetNumberField(TEXT("MinSize"), MinSize))
			{
				NewStack->SetMinSize((float)MinSize);
			}
		}
...
FString FTabManager::FLayout::ToString() const
{
	...
	{
		UE_LOGF(LogSlate, Error, "Failed save layout as Json string: %ls", *GetLayoutName().ToString());
	}
...
TSharedRef<FJsonObject> FTabManager::FLayout::PersistToString_Helper(const TSharedRef<FLayoutNode>& NodeToPersist)
{
	...
	if ( NodeAsArea.IsValid() )
	{
		...
		if ( NodeAsArea->WindowPlacement == FArea::EWindowPlacementInternal::Placement_Automatic )
		{
			JsonObj->SetStringField( TEXT("WindowPlacement"), GET_ENUMERATOR_NAME_STRING_CHECKED(FArea::EWindowPlacementInternal, Placement_Automatic) );
		...
		else if (NodeAsArea->WindowPlacement == FArea::EWindowPlacementInternal::Placement_NoWindow)
		{
			JsonObj->SetStringField( TEXT("WindowPlacement"), GET_ENUMERATOR_NAME_STRING_CHECKED(FArea::EWindowPlacementInternal, Placement_NoWindow) );
		}
		else if ( NodeAsArea->WindowPlacement == FArea::EWindowPlacementInternal::Placement_Specified 
			|| NodeAsArea->WindowPlacement == FArea::EWindowPlacementInternal::Placement_ParentSpecified )
		{
			if (NodeAsArea->WindowPlacement == FArea::EWindowPlacementInternal::Placement_Specified)
			{
				JsonObj->SetStringField(TEXT("WindowPlacement"), GET_ENUMERATOR_NAME_STRING_CHECKED(FArea::EWindowPlacementInternal, Placement_Specified) );
			}
			else
			{
				JsonObj->SetStringField(TEXT("WindowPlacement"), GET_ENUMERATOR_NAME_STRING_CHECKED(FArea::EWindowPlacementInternal, Placement_ParentSpecified) );
			}
	...
	else if ( NodeAsStack.IsValid() )
	{
		...
		if (NodeAsStack->MinSize != 0.f)
		{
			JsonObj->SetNumberField( TEXT("MinSize"), NodeAsStack->MinSize );
		}
...
void FTabManager::FLayout::ProcessExtensions(const FLayoutExtender& Extender)
{
	...
	for (FTabManager::FStack* Stack : AllTabs.AllStacks)
	{
		...
		// Check if any of the extended tabs were marked as needing to be foregrounded
		const FTabId ForegroundTab = Extender.FindFirstForegroundTab(Stack->GetExtensionId());
		if (ForegroundTab.TabType != NAME_None)
		{
			Stack->SetForegroundTab(ForegroundTab);
		}
	}
}
...
TSharedRef<FTabManager::FLayout> FTabManager::PersistLayout() const
{
	...
	for ( int32 LiveAreaIndex=0; LiveAreaIndex < DockAreas.Num(); ++LiveAreaIndex )
	{
		...
		if ( PersistedNode.IsValid() )
		{
			...
			if (PersistedNode->WindowPlacement == FArea::EWindowPlacementInternal::Placement_NoWindow)
			{
				ensure( !PersistentLayout->PrimaryArea.IsValid() );
				PersistentLayout->PrimaryArea = PersistedNode;

				if (this == &FGlobalTabmanager::Get().Get())
				{
					// Convert the area placement to use the ParentSpecified mode so that the global that manager can restore the main frame window
					if (TSharedPtr<SWindow> ParentWindow = ChildDockingArea->GetParentWindow())
					{
						UE::Slate::Private::SetAreaNodeWindowPlacementValues(PersistedNode, *ParentWindow.Get());
						PersistedNode->WindowPlacement = FArea::EWindowPlacementInternal::Placement_ParentSpecified;
					}
				}
			}
		}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FTabManager::RestoreFrom);
	...
	for (int32 AreaIndex=0; AreaIndex < Layout->Areas.Num(); ++AreaIndex )
	{
		...
		// Use in the placement has a fallback for old layout where the primary area might not have been saved
		const bool bIsPrimaryArea = ThisArea == Layout->GetPrimaryArea() || ThisArea->WindowPlacement == FArea::EWindowPlacementInternal::Placement_NoWindow;
		...
		if ( bShouldCreate )
		{
			...
			if (bHasValidOpenTabs)
			{
				...
				if (!RestoredDockArea)
				{
					if (bIsPrimaryArea)
					{
						UE_LOGF(LogSlate, Warning, "Primary area was not valid for RestoreAreaOutputCanBeNullptr = %d.", int(RestoreAreaOutputCanBeNullptr));
					}
	...
	{
		UE_LOGF(LogSlate, Warning, "FTabManager::RestoreFrom(): RestoreAreaOutputCanBeNullptr was set to EOutputCanBeNullptr::Never but"
...
void FTabManager::PopulateTabSpawnerMenu( FMenuBuilder &PopulateMe, const FName& TabType )
{
	...
	{
		UE_LOGF(LogSlate, Warning, "PopulateTabSpawnerMenu failed to find entry for %ls", *(TabType.ToString()));
	}
}
...
TSharedPtr<SDockTab> FTabManager::TryToggleSidebarTab(const FTabId& TabId)
{
	...
	{
		const TSharedPtr<SDockingArea> DockingArea = ExistingTab->GetDockArea();
		if (DockingArea.IsValid())
		{
			bToggled = DockingArea->TryToggleSidebarDrawer(ExistingTab.ToSharedRef());
		}
	}

	return bToggled ? ExistingTab : TSharedPtr<SDockTab>();
}

TSharedPtr<SDockTab> FTabManager::TryDismissSidebarTab(const FTabId& TabId)
{
	if (!InvokeTab_CanInvokeTab(TabId))
	{
		return {};
	...
	if (ExistingTab.IsValid())
	{
		...
		if (DockingArea.IsValid())
		{
			bDismissed = DockingArea->TryDismissSidebarDrawer(ExistingTab.ToSharedRef());
		}
	}

	return bDismissed ? ExistingTab : TSharedPtr<SDockTab>();
}
...
bool FTabManager::InvokeTab_CanInvokeTab(const FTabId& TabId) const
{
	...
	{
		UE_LOGF(LogTabManager, Warning, "Cannot spawn tab for '%ls'", *(TabId.ToString()));
		return false;
	}
...
TSharedPtr<SDockTab> FTabManager::InvokeTab_FindOrReuseExistingTab(const FTabId& TabId) const
{
	...
	if (!Spawner.IsValid())
	{
		UE_LOGF(LogTabManager, Warning, "Cannot spawn tab because no spawner is registered for '%ls'", *(TabId.ToString()));
	}
	else
	{
...
#if PLATFORM_MAC && ALPHA_BLENDED_WINDOWS
			// this is needed to allow Slate-rendered Metal views to show up on XCode's view debugger
			// and for multi-view rendering to function properly
...
TSharedPtr<SDockingTabStack> FTabManager::AttemptToOpenTab( const FTabId& ClosedTabId, bool bForceOpenWindowIfNeeded )
{
	...
	if ( CollapsedAreaWithMatchingTabIndex != INDEX_NONE )
	{
		...
		if (FGlobalTabmanager::Get() != SharedThis(this) &&
			...
			CollapsedAreaWithMatchingTab->WindowPlacement != FTabManager::FArea::EWindowPlacementInternal::Placement_NoWindow)
		{
...
void FTabManager::OpenUnmanagedTab(FName PlaceholderId, const FSearchPreference& SearchPreference, const TSharedRef<SDockTab>& UnmanagedTab)
{
	...
	else
	{
		...
		{
			UE_LOGF(LogTabManager, Warning, "Unable to insert tab '%ls'.", *(PlaceholderId.ToString()));
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FTabManager::RestoreArea_Helper);
	...
	{
		const bool bDockNeedsNewWindow = NodeAsArea.IsValid()
			&& !(NodeAsArea->WindowPlacement == FArea::EWindowPlacementInternal::Placement_NoWindow 
				|| NodeAsArea->WindowPlacement == FArea::EWindowPlacementInternal::Placement_ParentSpecified);
		...
		if ( bDockNeedsNewWindow )
		{
			...
			const bool bAutoPlacement = (NodeAsArea->WindowPlacement == FArea::EWindowPlacementInternal::Placement_Automatic);
		...
		{
			if (NodeAsArea->WindowPlacement == FArea::EWindowPlacementInternal::Placement_ParentSpecified)
			{
				if (ParentWindow)
				{
					/**
					 * The SWindow api doesn't help here as some function use the DPI resolution and some don't, but this out of scope for this change.
					 * This code should be refactored to be shared with the window logic also it doesn't support multi DPI scale factor across multiple monitor
					 */
					const float DPIScaleFactor = ParentWindow->GetDPIScaleFactor();
					UE::Slate::FDeprecateVector2DResult WindowSize = ParentWindow->GetWindowSizeFromClientSize(NodeAsArea->UnscaledWindowSize * DPIScaleFactor, DPIScaleFactor);
					FVector2D WindowPosition = NodeAsArea->UnscaledWindowPosition * DPIScaleFactor;
					...
					{
						// The window is out of bound of the virtual display rect just use the preferred work area
						FSlateRect PreferredWorkArea = FSlateApplicationBase::Get().GetPreferredWorkArea();
						WindowSize = PreferredWorkArea.GetSize();
						WindowPosition.X = PreferredWorkArea.Left;
						WindowPosition.Y = PreferredWorkArea.Top;
					}

					ParentWindow->ReshapeWindow(WindowPosition, WindowSize);
					if (NodeAsArea->bIsMaximized)
					{
...
bool FTabManager::IsAllowedTabType(const FName TabType) const
{
	...
	if (!bIsAllowed)
	{
		UE_LOGF(LogSlate, Verbose, "Disallowed Tab: %ls", *TabType.ToString());
	}
	return bIsAllowed;
}
...
void FTabManager::ToggleSidebarOpenTabs(const TSet<FTabId>& InExceptions)
{
	bool bHasSidebarTabs = !TemporarilySidebaredTabs.IsEmpty();

	// Check if TemporarilySidebaredTabs only contains InExceptions
	if (bHasSidebarTabs)
	{
		bool bHasOnlyExceptions = true;

		for (const TWeakPtr<SDockTab>& WeakTab : TemporarilySidebaredTabs)
		{
			if (TSharedPtr<SDockTab> Tab = WeakTab.Pin())
			{
				const FTabId TabId = Tab->GetLayoutIdentifier();
				if (!InExceptions.Contains(TabId))
				{
					bHasOnlyExceptions = false;
					break;
				}
			}
		}

		if (bHasOnlyExceptions)
		{
			bHasSidebarTabs = false;
		}
	}
		...
		for (int32 AreaIndex = 0; AreaIndex < DockAreas.Num(); ++AreaIndex)
		{
			...
			if (SomeDockArea.IsValid() && SomeDockArea->CanHaveSidebar())
			{
				...
				for (TSharedRef<SDockTab>& Tab : AllTabs)
				{
					const FTabId TabId = Tab->GetLayoutIdentifier();
					if (!InExceptions.Contains(TabId)
						&& IsTabAllowedInSidebar(Tab->GetLayoutIdentifier())
						&& !SomeDockArea->IsTabInSidebar(Tab)
						&& Tab->GetParentDockTabStack()->CanMoveTabToSideBar(Tab))
					{
	...
	{
		for (auto It = TemporarilySidebaredTabs.CreateIterator(); It; ++It)
		{
			if (TSharedPtr<SDockTab> Tab = It->Pin())
			{
				const FTabId TabId = Tab->GetLayoutIdentifier();
				if (!InExceptions.Contains(TabId))
				{
	...
	OnToggleTemporarilySidebaredTabs.Broadcast(!bHasSidebarTabs, InExceptions);
}
...
TSharedPtr<SDockTab> FTabManager::SpawnTab(const FTabId& TabId, const TSharedPtr<SWindow>& ParentWindow, const bool bCanOutputBeNullptr)
{
	...
	if (bSpawningAllowedBySpawner && !NewTabWidget.IsValid())
	{
		...
		if (!bCanOutputBeNullptr)
		{
			UE_LOGF(LogSlate, Log,
				"The tab \"%ls\" attempted to spawn in layout '%ls' but failed for some reason. An \"unrecognized tab\" will be returned instead.", *StringToDisplay, *ActiveLayoutName.ToString()
			);
			...
			const FTabId UnrecognizedId(FName("Unrecognized"), ETabIdFlags::None);
		...
		{
			UE_LOGF(LogSlate, Log,
				"The tab \"%ls\" attempted to spawn in layout '%ls' but failed for some reason. It will not be displayed.", *StringToDisplay, *ActiveLayoutName.ToString()
			);
		}
	}
...
bool FTabManager::HasSidebaredTabs() const
{
	return !TemporarilySidebaredTabs.IsEmpty();
}
...
{
	FName LevelEditorTabId = FName("LevelEditor");
	FName HomeScreenTabId = FName("HomeScreen");
...
TArray<TSharedPtr<SWindow>> FGlobalTabmanager::GetDockableWindows() const
{
	TArray<TSharedPtr<SWindow>> Windows;
	Windows.Reserve(DockAreas.Num());
	for (const TWeakPtr<SDockingArea>& DockingAreaWeak : DockAreas)
	{
		if (const TSharedPtr<SDockingArea> DockArea = DockingAreaWeak.Pin())
		{
			if (TSharedPtr<SWindow> Window = DockArea->GetParentWindow())
			{
				Windows.Add(Window);
			}
		}
...
FGlobalTabmanager::FWindowMajorTabsInfo FGlobalTabmanager::GetDockableWindowMajorTabsInfo(const TSharedRef<SWindow>& InWindow) const
{
	FWindowMajorTabsInfo TabsInfo;
	TabsInfo.Window = InWindow;
	if (const TSharedPtr<SDockingArea> DockingArea = GetDockingAreaForWindow(InWindow))
	{
		TArray<TSharedRef<SDockingTabStack>> TabStacks;
		GetAllStacks(DockingArea.ToSharedRef(), TabStacks);

		for (const TSharedRef<SDockingTabStack>& TabStack : TabStacks)
		{
			TSlotlessChildren<SDockTab> Tabs = TabStack->GetTabs();
			for (int32 Index = 0; Index < Tabs.Num(); Index++)
			{
				TSharedRef<SDockTab> Tab = StaticCastSharedRef<SDockTab>(Tabs.GetChildAt(Index));
				TabsInfo.DockTabs.Add(Tab);

				if (Tab->IsForeground())
				{
					TabsInfo.ActiveTab = Tab;
				}
			}
		}
	}
	...
	{
		UE_LOGF(LogSlate, Warning, "Could not find docking area for window %ls", *InWindow->GetTitle().ToString())
	}
	return TabsInfo;
}
...

{
	...
	// The panel drawer only support Major tab a the moment so don't bother with any other type of tab.
	if (BackgroundedTabManager && BackgroundedTab && BackgroundedTab->GetTabRole() == ETabRole::MajorTab)
	{
		if (TSharedPtr<SDockingArea> BackgroundTabbDockingArea = BackgroundedTab->GetDockArea())
		{
			BackgroundTabbDockingArea->SetPanelDrawerArea({});
		}
	}
	...
	if (ForegroundTab && ForegroundTabManager)
	{
		...
		{
			// Don't try to forward a panel drawer from the tab nmanager to a tab if it doesn't have a panel drawer.
			if (Primary->HasPanelDrawer())
			{
				if (TSharedPtr<SDockingArea> ForegroundTabDockingArea = ForegroundTab->GetDockArea())
				{
					ForegroundTabDockingArea->SetPanelDrawerArea(Primary->GetPanelDrawerArea());
				}
			}
		}
	}
