...
void SPanelDrawerArea::SetupOpenedLayout()
{
	...
	[
		SNew(SSplitter)
		.Orientation(Orient_Horizontal)
		.UpdateAttributes(ESlateAttributeInitialization::AfterConstruct)
		+ SSplitter::Slot()
...
{
	return PanelDrawerData ? PanelDrawerData->Size.MainContentCoefficient : 1.f;
}

float SPanelDrawerArea::GetPanelDrawerCoefficient() const
{
	return PanelDrawerData ? PanelDrawerData->Size.PanelDrawerCoefficient : 0.f;
}
