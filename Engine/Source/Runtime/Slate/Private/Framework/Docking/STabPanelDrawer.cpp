...
void STabPanelDrawer::Construct(const FArguments& InArgs, const TSharedRef<SDockTab> InTab)
{
	...
	[
		...
		.HAlign(HAlign_Fill)
		[
			SAssignNew(TabLabelOverlay, SOverlay)
			+ SOverlay::Slot()
			.Padding(TAttribute<FMargin>::Create(TAttribute<FMargin>::FGetter::CreateSP(this, &STabPanelDrawer::GetTabLabelOverlayPadding)))
			.VAlign(VAlign_Center)
			.HAlign(HAlign_Center)
			[
				SNew(SBox)
				.MaxDesiredWidth(this, &STabPanelDrawer::GetTabLabelOverlayMaxSize)
				[
					// Label sub HBox
					SNew(SHorizontalBox)
			...
			[
				...
				.VAlign(VAlign_Center)
				[
					SAssignNew(OpenTabInNewWindowButton, SButton)
				...
				.VAlign(VAlign_Center)
				[
					SAssignNew(ButtonSpacer, SSpacer)
...
FMargin STabPanelDrawer::GetTabLabelOverlayPadding() const
{
	if (DisplayedTab)
	{
		FMargin TabLabelMargin = DisplayedTab->GetTabPadding();
		const float LargestSide = FMath::Max(TabLabelMargin.Left, TabLabelMargin.Right);
		TabLabelMargin.Left = LargestSide;
		TabLabelMargin.Right = LargestSide;
		return TabLabelMargin;
	}

	return {};
}

FOptionalSize STabPanelDrawer::GetTabLabelOverlayMaxSize() const
{
	TSharedPtr<SWidget> TabLabelOverlayAsShared = TabLabelOverlay.Pin();
	if (TabLabelOverlayAsShared && DisplayedTab)
	{
		const FGeometry& PaintGeo = TabLabelOverlayAsShared->GetPaintSpaceGeometry();
		float TotalWidth = PaintGeo.GetLocalSize().X;
		if (TotalWidth > 0.f)
		{
			FMargin TabLabelMargin = DisplayedTab->GetTabPadding();
			float LargestSide = FMath::Max(TabLabelMargin.Left, TabLabelMargin.Right);

			if (TSharedPtr<SWidget> OpenTabInNewWindowButtonAsShared = OpenTabInNewWindowButton.Pin())
			{
				LargestSide += OpenTabInNewWindowButtonAsShared->GetPaintSpaceGeometry().GetLocalSize().X;
			}

			if (TSharedPtr<SWidget> ButtonSpacerAsShared = ButtonSpacer.Pin())
			{
				// Double the size for to use the same spacing between the most left button and the label
				LargestSide += ButtonSpacerAsShared->GetPaintSpaceGeometry().GetLocalSize().X * 2;
			}

			if (TSharedPtr<SWidget> DismissButtonAsShared = DismissButton.Pin())
			{
				LargestSide += DismissButtonAsShared->GetPaintSpaceGeometry().GetLocalSize().X;
			}

			return FOptionalSize(FMath::Max(0.f, TotalWidth - LargestSide * 2));
		}
	}
