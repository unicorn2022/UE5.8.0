...
void SDockingTabStack::Construct( const FArguments& InArgs, const TSharedRef<FTabManager::FStack>& PersistentNode  )
{
	...
	NodeMinSize = PersistentNode->GetMinSize();
	...
	{
		ShowHideTabWell = FCurveSequence(0,0.15f);
...
{
	// If the new widget is a drawer within the tab, the tab does not need to be activated.
	// This prevents the focus being reset back to the containing tab and instantly closing the drawer.
	if (NewWidgetPath.IsValid())
	{
		if (TSharedPtr<SWidget> LastWidget = NewWidgetPath.GetLastWidget())
		{
			if (LastWidget->GetType() == STabDrawer::StaticWidgetClass().GetWidgetType())
			{
				return;
			}
		}
...
void SDockingTabStack::ShowContextMenu(const TSharedRef<SDockTab>& InContextTab, const FWidgetPath& InWidgetPath)
{
	FSlateApplication::Get().PushMenu(
		AsShared(), 
		InWidgetPath, 
		MakeContextMenu(InContextTab), 
		FSlateApplication::Get().GetCursorPos(), 
		FPopupTransitionEffect::ContextMenu
	);
	
	InContextTab->FlashTab();
}
...
{
	CloseTab(TabWell->GetForegroundTab());
}

void SDockingTabStack::CloseTab(TSharedPtr<SDockTab> InTab)
{
	if (InTab.IsValid())
	{
		InTab->RequestCloseTab();
	}
}

void SDockingTabStack::CloseTabsInDirectionFromForegroundTab(ETabsToClose TabsToClose, ECloseTabsDirection Direction)
{
	CloseTabsInDirectionFromTab(TabWell->GetForegroundTab(), TabsToClose, Direction);
}

void SDockingTabStack::CloseTabsInDirectionFromTab(TSharedPtr<SDockTab> InTab, ETabsToClose InTabsToClose, ECloseTabsDirection InDirection)
{
	if (!InTab.IsValid())
	{
		return;
	...
	const int32 IndexOfTab = TabWell->GetTabIndex(InTab);
	if (IndexOfTab == INDEX_NONE)
	{
		return;
	}

	const int32 Increment = GetIncrementFromCloseTabsDirection(InDirection);

	int32 DestroyIndex = IndexOfTab + Increment;
	while (DestroyIndex >= 0 && DestroyIndex < TabWell->GetNumTabs())
	{
		...
		const bool bCanClose =
			(InTabsToClose == ETabsToClose::CloseAllTabs) ||
			(InTabsToClose == ETabsToClose::CloseDocumentTabs && VisualTabRole == ETabRole::DocumentTab) ||
			(InTabsToClose == ETabsToClose::CloseDocumentAndMajorTabs && (VisualTabRole == ETabRole::DocumentTab || VisualTabRole == ETabRole::MajorTab));
		...
		if (!bCanClose || !Tab->RequestCloseTab() || InDirection == ECloseTabsDirection::Left)
		{
...
{
	CloseAllButTab(TabWell->GetForegroundTab(), TabsToClose);
}

void SDockingTabStack::CloseAllButTab(TSharedPtr<SDockTab> InTab, ETabsToClose InTabsToClose)
{
	if (!InTab.IsValid())
	{
		return;
	...
	while ((TabWell->GetNumTabs() > 1) && (DestroyIndex < TabWell->GetNumTabs()))
	{
		...
		if ((Tab == InTab) || !bCanClose || !Tab->RequestCloseTab())
		{
			++DestroyIndex;
		}
	}
...
{
	if (TabWell.IsValid() && MouseEvent.GetEffectingButton() == EKeys::RightMouseButton)
	{
		if (const TSharedPtr<SDockTab> ForegroundTab = TabWell->GetForegroundTab())
		{
			const FWidgetPath WidgetPath = MouseEvent.GetEventPath() != nullptr ? *MouseEvent.GetEventPath() : FWidgetPath();
			ShowContextMenu(ForegroundTab.ToSharedRef(), WidgetPath);
			return FReply::Handled();
		}
	}
...
TSharedRef<SWidget> SDockingTabStack::MakeContextMenu(const TSharedPtr<SDockTab>& InTab)
{
	if (!InTab.IsValid())
	{
	...
	FMenuBuilder MenuBuilder( bCloseAfterSelection, nullptr, TSharedPtr<FExtender>(), bCloseSelfOnly, &FCoreStyle::Get() );
	{
		MenuBuilder.BeginSection("DockingTabStackOptions", LOCTEXT("TabOptionsHeading", "Options") );
		{
			...
			if (IsTabAllowedInSidebar(InTab))
			{
				MenuBuilder.AddMenuEntry(
					...
					FSlateIcon(),
					FUIAction(
						FExecuteAction::CreateSP(this, &SDockingTabStack::MoveTabToSidebar, InTab),
						FCanExecuteAction::CreateSP(this, &SDockingTabStack::CanMoveTabToSidebar, InTab)
					)
				);
			}
		}
		...
		MenuBuilder.BeginSection("DockingTabStackCloseTabs");
		{
			MenuBuilder.AddMenuEntry(
				...
				LOCTEXT("CloseTabTooltip", "Close this tab."),
				FSlateIcon(),
				FUIAction(
					FExecuteAction::CreateSP(this, &SDockingTabStack::CloseTab, InTab),
					FCanExecuteAction::CreateSP(this, &SDockingTabStack::CanCloseTab, InTab)
				)
			);
			...
			MenuBuilder.AddMenuEntry(
				...
				FSlateIcon(),
				FUIAction(
					FExecuteAction::CreateSP( this, &SDockingTabStack::CloseTabsInDirectionFromTab, InTab, TabsToClose, ECloseTabsDirection::Left),
					FCanExecuteAction::CreateSP( this, &SDockingTabStack::CanCloseTabsInDirectionOfTab, InTab, ECloseTabsDirection::Left)
				)
			);
			...
			MenuBuilder.AddMenuEntry(
				...
				FSlateIcon(),
				FUIAction(
					FExecuteAction::CreateSP( this, &SDockingTabStack::CloseTabsInDirectionFromTab, InTab, TabsToClose, ECloseTabsDirection::Right),
					FCanExecuteAction::CreateSP( this, &SDockingTabStack::CanCloseTabsInDirectionOfTab, InTab, ECloseTabsDirection::Right)
				)
			);

			MenuBuilder.AddMenuEntry(
				LOCTEXT("CloseOtherTabs", "Close Other Tabs"),
				LOCTEXT("CloseOtherTabsTooltip", "Closes all tabs except for the active tab."),
				FSlateIcon(),
				FUIAction(
					FExecuteAction::CreateSP( this, &SDockingTabStack::CloseAllButTab, InTab, TabsToClose ),
					FCanExecuteAction::CreateSP(this, &SDockingTabStack::CanCloseAllButTab, InTab)
				)
			);
		}
		MenuBuilder.EndSection();

		// Let the tab extend its own context menu
		InTab->ExtendContextMenu(MenuBuilder);
	}
...
void SDockingTabStack::MoveTabToSidebar(TSharedPtr<SDockTab> InTab)
{
	if (InTab.IsValid())
	{
		MoveTabToSidebar(InTab.ToSharedRef());
	}
}
...
{
	return CanCloseTab(TabWell->GetForegroundTab());
}

bool SDockingTabStack::CanCloseTab(TSharedPtr<SDockTab> InTab) const
{
	return InTab.IsValid() && InTab->CanCloseTab();
}
...
{
	return CanCloseTabsInDirectionOfTab(TabWell->GetForegroundTab(), Direction);
}

bool SDockingTabStack::CanCloseTabsInDirectionOfTab(TSharedPtr<SDockTab> InTab, ECloseTabsDirection InDirection) const
{
	if (!InTab.IsValid())
	{
		return false;
	...
	const int32 TabIndex = TabWell->GetTabIndex(InTab);
	if (TabIndex == INDEX_NONE)
	{
		return false;
	}

	const ETabRole VisualTabRole = InTab->GetVisualTabRole();
	if (VisualTabRole == ETabRole::DocumentTab || VisualTabRole == ETabRole::MajorTab)
	{
		const TArray< TSharedRef<SDockTab> > MyTabs = this->GetTabs().AsArrayCopy();
		const int32 Increment = GetIncrementFromCloseTabsDirection(InDirection);
		for (int32 Index = TabIndex + Increment; Index >= 0 && Index < MyTabs.Num(); Index += Increment)
		{
			TSharedRef<SDockTab> Tab = MyTabs[Index];
			if (Tab->CanCloseTab())
			{
				return true;
			}
...
bool SDockingTabStack::CanCloseAllButForegroundTab() const
{
	...
	return CanCloseAllButTab(TabWell->GetForegroundTab());
}

bool SDockingTabStack::CanCloseAllButTab(TSharedPtr<SDockTab> InTab) const
{
	if (!InTab.IsValid())
	{
		return false;
...
bool SDockingTabStack::CanMoveTabToSidebar(TSharedPtr<SDockTab> InTab) const
{
	if (InTab.IsValid())
	{
		return CanMoveTabToSideBar(InTab.ToSharedRef());
	}
...
TSharedPtr<SDockTab> SDockingTabStack::GetSelectedTabFromWell()
{
	if (TabWell.IsValid())
	{
		return TabWell->GetForegroundTab();
	}
	else
	{
		return nullptr;
...
{
	const TSlotlessChildren<SDockTab> ChildTabs = GetTabs();

	const int32 NumTabs = ChildTabs.Num();
	if (NumTabs <= 0)
	{
		return SSplitter::FractionOfParent;
	}

	const TSharedPtr<SDockingSplitter> ParentSplitter = ParentNodePtr.Pin();
	const TArrayView<const TSharedRef<SDockTab>> ChildTabsArray = MakeArrayView(&ChildTabs[0], ChildTabs.Num());

	const bool bAllTabsAllowAutosize = Algo::AllOf(ChildTabsArray
		,[&ParentSplitter](const TSharedRef<SDockTab>& InTab)
	{
		const EDockTabAutosizePolicy Policy = InTab->GetAutosizePolicy();

		// No parent splitter: treat like generic autosize check
		if (!ParentSplitter.IsValid())
		{
			return Policy != EDockTabAutosizePolicy::None;
		}

		const EOrientation ParentOrientation = ParentSplitter->GetOrientation();

		switch (Policy)
		{
		default:
		case EDockTabAutosizePolicy::None:
			return false;
		case EDockTabAutosizePolicy::LayoutAxis:
			return true;
		case EDockTabAutosizePolicy::HorizontalOnly:
			return ParentOrientation == Orient_Horizontal;
		case EDockTabAutosizePolicy::VerticalOnly:
			return ParentOrientation == Orient_Vertical;
		}
	});

	return bAllTabsAllowAutosize ? SSplitter::SizeToContent : SSplitter::FractionOfParent;
}
