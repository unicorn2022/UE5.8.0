...
FSlateTextLayout::FSlateTextLayout(SWidget* InOwner, FTextBlockStyle InDefaultTextStyle)
	...
	, bIsPassword(false)
	, FontFacesLoadingPaintPolicy(EFontFacesLoadingPaintPolicy::DoNotPaint)
{
...
{
	// The layout pass would have already started shaping all of the text in this text layout and caching the shaped text as part of measurement 
	// We should know by this point whether any of the text runs are waiting on a font to load
	if (AreAnyFontFacesLoading())
	{
		if (GetFontFacesLoadingPaintPolicy() == EFontFacesLoadingPaintPolicy::DoNotPaint)
		{
			return LayerId;
		}
	}
...
void FSlateTextLayout::SetOnAllFontFacesFinishLoading(const FSimpleDelegate& InAllFontFacesFinishLoading)
{
	SWidget& OwnerWidget = Children.GetOwner();
	TSharedPtr< FSlateTextLayoutPendingFontFacesLoadingMetaData> MetaData = OwnerWidget.GetMetaData<FSlateTextLayoutPendingFontFacesLoadingMetaData>();
	if (MetaData)
	{
		MetaData->OnAllFontFacesFinishLoading = InAllFontFacesFinishLoading;
	}
	// No point creating a new metadata and adding it to the widget if the passed in delegate isn't bound 
	else if (InAllFontFacesFinishLoading.IsBound())
	{
		TSharedRef<FSlateTextLayoutPendingFontFacesLoadingMetaData> NewMetaData = MakeShared<FSlateTextLayoutPendingFontFacesLoadingMetaData>();
		NewMetaData->OnAllFontFacesFinishLoading = InAllFontFacesFinishLoading;
		OwnerWidget.AddMetadata(NewMetaData);
	}
}

void FSlateTextLayout::HandleAllFontFacesFinishLoading()
{
	TSharedPtr<FSlateTextLayoutPendingFontFacesLoadingMetaData> MetaData = Children.GetOwner().GetMetaData<FSlateTextLayoutPendingFontFacesLoadingMetaData>();
	if (MetaData)
	{
		MetaData->OnAllFontFacesFinishLoading.ExecuteIfBound();
	}
}

EFontFacesLoadingPaintPolicy FSlateTextLayout::GetFontFacesLoadingPaintPolicy() const
{
	return FontFacesLoadingPaintPolicy;
}

void FSlateTextLayout::SetFontFacesLoadingPaintPolicy(EFontFacesLoadingPaintPolicy InFontFacesLoadingPaintPolicy)
{
	FontFacesLoadingPaintPolicy = InFontFacesLoadingPaintPolicy;
}
