...
FShapedGlyphSequenceRef FShapedTextCache::AddShapedText(const FCachedShapedTextKey& InKey, const TCHAR* InText, TArray<TWeakPtr<FFreeTypeFace>>* OutLoadingFontFaces)
{
...
FShapedGlyphSequenceRef FShapedTextCache::AddShapedText(const FCachedShapedTextKey& InKey, const TCHAR* InText, const TextBiDi::ETextDirection InTextDirection, TArray<TWeakPtr<FFreeTypeFace>>* OutLoadingFontFaces)
{
	const TSharedPtr<FSlateFontCache> FontCache = FontCachePtr.Pin();
...
FShapedGlyphSequenceRef FShapedTextCache::FindOrAddShapedText(const FCachedShapedTextKey& InKey, const TCHAR* InText, TArray<TWeakPtr<FFreeTypeFace>>* OutLoadingFontFaces)
{
	FShapedGlyphSequencePtr ShapedText = FindShapedText(InKey);

	if (!ShapedText.IsValid())
	{
		ShapedText = AddShapedText(InKey, InText, OutLoadingFontFaces);
	}

	return ShapedText.ToSharedRef();
}

FShapedGlyphSequenceRef FShapedTextCache::FindOrAddShapedText(const FCachedShapedTextKey& InKey, const TCHAR* InText, const TextBiDi::ETextDirection InTextDirection, TArray<TWeakPtr<FFreeTypeFace>>* OutLoadingFontFaces)
{
	FShapedGlyphSequencePtr ShapedText = FindShapedText(InKey);

	if (!ShapedText.IsValid())
	{
		ShapedText = AddShapedText(InKey, InText, InTextDirection, OutLoadingFontFaces);
	}
...
FVector2D ShapedTextCacheUtil::MeasureShapedText(const FShapedTextCacheRef& InShapedTextCache, const FCachedShapedTextKey& InRunKey, const FTextRange& InMeasureRange, const TCHAR* InText, TArray<TWeakPtr<FFreeTypeFace>>* OutLoadingFontFaces)
{
	...
	FShapedGlyphSequenceRef ShapedText = InShapedTextCache->FindOrAddShapedText(InRunKey, InText, OutLoadingFontFaces);
	...
	if (!MeasuredWidth.IsSet())
	{
		...
		ShapedText = InShapedTextCache->FindOrAddShapedText(MeasureKey, InText, OutLoadingFontFaces);
...
int8 ShapedTextCacheUtil::GetShapedGlyphKerning(const FShapedTextCacheRef& InShapedTextCache, const FCachedShapedTextKey& InRunKey, const int32 InGlyphIndex, const TCHAR* InText, TArray<TWeakPtr<FFreeTypeFace>>* OutLoadingFontFaces)
{
	...
	if (!Kerning.IsSet())
	{
		...
		ShapedText = InShapedTextCache->FindOrAddShapedText(KerningKey, InText, OutLoadingFontFaces);
...
FShapedGlyphSequenceRef ShapedTextCacheUtil::GetShapedTextSubSequence(const FShapedTextCacheRef& InShapedTextCache, const FCachedShapedTextKey& InRunKey, const FTextRange& InTextRange, const TCHAR* InText, const TextBiDi::ETextDirection InTextDirection, TArray<TWeakPtr<FFreeTypeFace>>* OutLoadingFontFaces)
{
	...
	FShapedGlyphSequencePtr ShapedText = InShapedTextCache->FindOrAddShapedText(InRunKey, InText, OutLoadingFontFaces);
	...
	if (InRunKey.TextRange != InTextRange)
	{
		...
		else
		{
			...
			else
			{
				...
				ShapedText = InShapedTextCache->FindOrAddShapedText(SubSequenceKey, InText, InTextDirection, OutLoadingFontFaces);
			}
		}
	}
