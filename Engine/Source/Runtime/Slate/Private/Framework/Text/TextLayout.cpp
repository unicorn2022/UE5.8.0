...
{
	const FRunTextContext RunTextContext(TextShapingMethod, Line.TextBaseDirection, Line.ShapedTextCache, &LoadingFontFaces);
...
void FTextLayout::CreateLineViewBlocks( int32 LineModelIndex, const int32 StopIndex, const float WrappedLineWidth, const TOptional<float>& JustificationWidth, int32& OutRunIndex, int32& OutRendererIndex, int32& OutPreviousBlockEnd, TArray< TSharedRef< ILayoutBlock > >& OutSoftLine )
{
	...
	const FRunTextContext RunTextContext(TextShapingMethod, LineModel.TextBaseDirection, LineModel.ShapedTextCache, &LoadingFontFaces);
...
void FTextLayout::EndLayout()
{
	...
	if (LoadingFontFaces.IsEmpty())
	{
		// We just finished loading all fonts 
		if (bLoadingFontFacesOnPreviousLayout)
		{
			HandleAllFontFacesFinishLoading();
			bLoadingFontFacesOnPreviousLayout = !LoadingFontFaces.IsEmpty();
		}
	}
	else
	{
		// We just started loading necessary font faces 
		if (!bLoadingFontFacesOnPreviousLayout)
		{
			bLoadingFontFacesOnPreviousLayout = !LoadingFontFaces.IsEmpty();
		}
...
FTextLayout::FTextLayout()
	...
	, bLoadingFontFacesOnPreviousLayout(false)
{
}

void FTextLayout::UpdateIfNeeded()
{
	// GSlateLayoutGeneration is incremented when font faces are finally done async lazily loaded
	// This re-measures and lays out the text accordingly
...
void FTextLayout::UpdateLayout()
{
	...
	// We clear all loading font faces every re-layout so that it can be populated anew during shaping 
	LoadingFontFaces.Empty();
	ClearView();
	BeginLayout();
...
bool FTextLayout::AreAnyFontFacesLoading() const
{
	return !LoadingFontFaces.IsEmpty();
}
