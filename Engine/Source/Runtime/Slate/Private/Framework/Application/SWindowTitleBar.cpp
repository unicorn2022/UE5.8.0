...
TSharedRef<SWidget> SWindowTitleBar::MakeTitleBarContent( TSharedPtr<SWidget> CenterContent, EHorizontalAlignment CenterContentAlignment )
{
	...
	// Extension content lives in its own FillWidth slot so it clips before
	// the window buttons (AutoWidth, right) and the app icon + menus (AutoWidth, left) do.
	TSharedPtr<SWidget> ExtensionContent = SNew(SBox)
		.Visibility(EVisibility::SelfHitTestInvisible)
		[
			SNew(SHorizontalBox)
			+ SHorizontalBox::Slot()
	...
	ExtensionContent->SlatePrepass();
	RightContent->SlatePrepass();

	FVector2D LeftSize = LeftContent->GetDesiredSize();
	FVector2D ExtensionSize = ExtensionContent->GetDesiredSize();
	FVector2D ButtonsSize = RightContent->GetDesiredSize();
	FVector2D RightSize = FVector2D(ExtensionSize.X + ButtonsSize.X, FMath::Max(ExtensionSize.Y, ButtonsSize.Y));
	...
	return 
		...
		[
			...
			// Foreground: nested layout with explicit clip priority.
			// 1. RightContent (outer AutoWidth): never clips.
			// 2. ExtensionContent (inner FillWidth + ClipToBounds): clips first.
			// 3. LeftContent (inner AutoWidth, clipped by outer SBox): clips last.
			+ SOverlay::Slot()
			[
				SNew(SHorizontalBox)
				.Visibility(EVisibility::SelfHitTestInvisible)

				// Outer fill slot: absorbs all available space left of the buttons.
				// ClipToBounds ensures LeftContent cannot bleed into the button area.
				+ SHorizontalBox::Slot()
				.FillWidth(1.0f)
				.VAlign(VAlign_Top)
				[
					...
					[
						...
						// LeftContent: AutoWidth inside the clipped container.
						// Only clips once the ExtensionContent area is fully gone.
						+ SHorizontalBox::Slot()
						.AutoWidth()
						.HAlign(HAlign_Left)
						.VAlign(VAlign_Top)
						...
						// ExtensionContent: FillWidth so it clips before LeftContent is affected.
						+ SHorizontalBox::Slot()
						.FillWidth(1.0f)
						.HAlign(HAlign_Right)
						.VAlign(VAlign_Top)
						[
							...
							.Clipping(EWidgetClipping::ClipToBounds)
							[
								ExtensionContent.ToSharedRef()
							]
						]
					]
				]

				// RightContent: AutoWidth so it never clips.
				+ SHorizontalBox::Slot()
				.AutoWidth()
				.HAlign(HAlign_Right)
				.VAlign(VAlign_Top)
