...
struct FScopedNavigationTransition
{
	...
	~FScopedNavigationTransition()
	{
		...
		// Clear any pending navigation context - valid only for this navigation event.
		if (User.IsValid())
		{
			FSlateApplication::Get().ClearPendingNavigationContext(User->GetUserIndex());
		}
	}
...
void FSlateApplication::OverridePlatformApplication(TSharedPtr<class GenericApplication> InPlatformApplication)
{
	...
	{
		UE_LOGF(LogSlate, Warning, "OverridePlatformApplication: Cannot override cursor with invalid value");
	}
}
...
void FSlateApplication::InitHighDPI(const bool bForceEnable)
{
	...
	if (CVar)
	{
		...
		if (bEnableHighDPI)
		{
			...
			// If the Slate application already exists, its display metrics cache was
			// populated before DPI awareness was set and needs to be updated.
			if (FSlateApplication::IsInitialized())
			{
				FDisplayMetrics DisplayMetrics;
				FSlateApplication::Get().GetDisplayMetrics(DisplayMetrics);
				FSlateApplication::Get().OnVirtualDesktopSizeChanged(DisplayMetrics);
			}
		}
	}
}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FSlateApplication::InitializeRenderer);
	Renderer = InRenderer;
...
void FSlateApplication::SetCursorPos(const FVector2D& MouseCoordinate)
{
	...
	{
		LastPlatformCursorPosition = UE::Slate::CastToVector2f(PlatformApplication->Cursor->GetPosition());
	}
}
	...
	if (TSharedPtr<FSlateUser> SlateUser = GetUser(CursorUserIndex))
	{
		...
		if (bIsUsingPlatformCursor != bUsePlatformCursor)
		{
			if (PlatformApplication && PlatformApplication->Cursor)
			{
				...
				UE_LOGF(LogSlate, Log, "User[%d] UsePlatformCursorForCursorUser(%ls)", SlateUser->GetUserIndex(), bUsePlatformCursor ? TEXT("true") : TEXT("false"));
			}
		}
	}
}
...
#if WITH_SLATE_DEBUGGING
	FSlateDebugging::ResetInvalidationPanelCount();
#endif
...
void FSlateApplication::TickPlatform(float DeltaTime)
{
...

	{
		...
		// Skip the software gesture detector when native platform gesture recognizers are active.
...
FWidgetPath FSlateApplication::LocateWidgetInWindow(UE::Slate::FDeprecateVector2DParameter ScreenspaceMouseCoordinate, const TSharedRef<SWindow>& Window, bool bIgnoreEnabledStatus, int32 UserIndex) const
{
	...
	if (bAcceptsInput && Window->IsScreenspaceMouseWithin(ScreenspaceMouseCoordinate))
	{
		...
		if (TransformFullscreenMouseInput && !GIsEditor && Window->GetWindowMode() == EWindowMode::Fullscreen)
		{
			...
			const FVector2f WindowSize = Window->GetSizeInScreen();
			...
			CursorPosition -= PositionInScreen;
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FSlateApplication::AddWindow);
...
void FSlateApplication::AddModalWindow( TSharedRef<SWindow> InSlateWindow, const TSharedPtr<const SWidget> InParentWidget, bool bSlowTaskWindow )
{
	...
	{
		UE_LOGF(LogSlate, Warning, "A modal window tried to take control while running in unattended script mode. The window was canceled.");
...
#if WITH_EDITOR
	FCoreDelegates::PreSlateModalWithContext.Broadcast(
		FCoreDelegates::FModalWindowContext{ 
			.bIsSlowTaskWindow = bSlowTaskWindow,
			.WindowIdentifier = (std::intptr_t)(&InSlateWindow.Get()) });
PRAGMA_DISABLE_DEPRECATION_WARNINGS
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	if( InParentWidget.IsValid() )
	{
		...
		{
			UE_LOGF(LogSlate, Warning, "Modal Window fail to open as a native child. The path to the parent widget (%ls) could not be found", *InParentWidget->ToString());
...
TSharedPtr<IMenu> FSlateApplication::PushMenu(const TSharedRef<SWidget>& InParentWidget, const FWidgetPath& InOwnerPath, const TSharedRef<SWidget>& InContent, const UE::Slate::FDeprecateVector2DParameter& SummonLocation, const FPopupTransitionEffect& TransitionEffect, const bool bFocusImmediately, const UE::Slate::FDeprecateVector2DParameter& SummonLocationSize, TOptional<EPopupMethod> Method, const bool bIsCollapsedByParent, const int32 FocusUserIndex)
{
	...
	UE_LOGF(LogSlate, Warning, "Menu could not be pushed.  A path to the parent widget(%ls) could not be found", *InParentWidget->ToString());
...
#if !WITH_EDITOR
	UE_LOGF(LogSlate, Log, "Request Window '%ls' being destroyed", *InWindowToDestroy->GetTitle().ToString() );
#endif
	struct local
	{
...
void FSlateApplication::TryDumpNavigationConfig(TSharedPtr<FNavigationConfig> InNavigationConfig) const
{
	...
	{
		UE_LOGF(LogSlate, Log, "Navigation Config Change:\n%ls", *InNavigationConfig->ToString());
		...
		UE_LOGF(LogSlate, Log, "--- Navigation Config Changing Callstack ---");
		UE_LOGF(LogSlate, Log, "Script Stack:\n%ls", *ScriptStack);
		UE_LOGF(LogSlate, Log, "Callstack:\n%ls", ANSI_TO_TCHAR(DumpCallstack));
	}
}
#endif // WITH_SLATE_DEBUGGING
...
void FSlateApplication::SetPendingNavigationContext(uint32 UserIndex, TSharedPtr<ISlateMetaData> InContext)
{
	PendingNavigationContexts.Add(UserIndex, MoveTemp(InContext));
}

void FSlateApplication::ClearPendingNavigationContext(uint32 UserIndex)
{
	PendingNavigationContexts.Remove(UserIndex);
}
...
bool FSlateApplication::SetUserFocus(FSlateUser& User, const FWidgetPath& InFocusPath, const EFocusCause InCause)
{
...
#endif
			UE_LOGF(LogSlate, Warning, "Ignoring SetUserFocus because it's not an active modal Window (user %i not set to %ls.", User.GetUserIndex(), *InFocusPath.GetLastWidget()->ToString());
			return false;
		}
	}
	...
	if (InFocusPath.IsValid())
	{
		//UE_LOGF(LogSlate, Warning, "Focus for user %i seeking focus path:\n%ls", InUserIndex, *InFocusPath.ToString());
		...
		for (int32 WidgetIndex = InFocusPath.Widgets.Num() - 1; WidgetIndex >= 0; --WidgetIndex)
		{
			...
			if (WidgetToFocus.Widget->SupportsKeyboardFocus())
			{
				...
				{
					//UE_LOGF(LogSlate, Warning, "--Focus Has Not Changed--");
					return false;
				}
	...
	//UE_LOGF(LogSlate, Warning, "Focus for user %i set to %ls.", User.GetUserIndex(), NewFocusedWidget.IsValid() ? *NewFocusedWidget->ToString() : TEXT("Invalid"));
	...
	if (OldFocusedWidget.IsValid())
	{
		...
		if (OldFocusedWidget->CanSupportFocus() && OldFocusedWidget->SupportsKeyboardFocus())
		{
			OldFocusedWidget->Invalidate(EInvalidateWidgetReason::Paint);
		}
#endif
#if WITH_SLATE_DEBUGGING
...
#if PLATFORM_UI_NEEDS_FOCUS_OUTLINES
		if (NewFocusedWidget->CanSupportFocus() && NewFocusedWidget->SupportsKeyboardFocus())
		{
			NewFocusedWidget->Invalidate(EInvalidateWidgetReason::Paint);
		}
#endif
#if WITH_SLATE_DEBUGGING
...
void FSlateApplication::ProcessReply( const FWidgetPath& CurrentEventPath, const FReply& TheReply, const FWidgetPath* WidgetsUnderMouse, const FPointerEvent* InMouseEvent, const uint32 UserIndex )
{
	...
	if (TheReply.GetNavigationDestination().IsValid() || TheReply.GetNavigationType() != EUINavigation::Invalid)
	{
		...
		{
			if (!GSlateEnableGamepadEditorNavigation && TheReply.GetNavigationGenesis() == ENavigationGenesis::Controller && !NavigationSource.GetLastWidget()->GetPersistentState().IsProjectContent())
			{
				// Gamepad navigation while not in project content, do nothing as specified by GSlateEnableGamepadEditorNavigation
			}
...
void FSlateApplication::EnterDebuggingMode()
{
	...
	auto AddNotification = [Self=this](const FText& SubText)
	{
		...
		UE_LOGF(LogSlate, Warning, "Enter Debugging Mode failed.");
...
#if WITH_EDITOR
	// If we enter debugging mode to debug a widget in the main editor window,
	// the editor will become mostly unresponsive as it stops ticking the main editor window.
	// Otherwise allow debugging within tick.
	if (bIsTicking)
	{
		TSharedPtr<SWindow> RootWindow = FGlobalTabmanager::Get()->GetRootWindow();
		TSharedPtr<SWidget> ContextWidgetPinned = CurrentDebugContextWidget.Pin();
		TSharedPtr<SWindow> DebugWindow = ContextWidgetPinned.IsValid() ? FindWidgetWindow(ContextWidgetPinned.ToSharedRef()) : TSharedPtr<SWindow>();
		if (DebugWindow.IsValid() && DebugWindow == RootWindow)
		{
			AddNotification(INVTEXT("Tried to enter debugging mode inside Tick in the main editor window.\n\n"
				"Restart PIE using New Editor Window instead of Selected Viewport (or undock the viewport tab) and try again."));
			return;
		}
	}
	...
	if (TSharedPtr<SWidget> CurrentDebugContextWidgetPinned = CurrentDebugContextWidget.Pin())
	{
		...
		{
			UE_LOGF(LogSlate, Warning, "EnterDebuggingMode without blocking the window Paint. That may start a new Paint on the same Window while the previous Paint is not completed.");
		}
	}
...
void FSlateApplication::SetIsFakeTouch(bool bInIsFakingTouch)
{
	bIsFakingTouch = bInIsFakingTouch;
}

bool FSlateApplication::GetIsFakeTouch() const
{
...
void TakeScreenshotCommon(const TSharedRef<SWidget>& Widget, const FIntRect& InnerWidgetArea, FIntRect& ScreenshotRect, 
						  SWindow* WidgetWindow)
{
	...
	FSlateApplication::Get().OnPreScreenshot().Broadcast(Widget);
}
...
TSharedPtr<FSlateUser> FSlateApplication::GetUser(FPlatformUserId PlatformUser)
{
	...
	{
		UE_LOGF(LogSlate, Warning, "SlateApplication::GetUser called with an invalid platform user! Defaulting to 0");
	}
...
TSharedRef<FSlateUser> FSlateApplication::RegisterNewUser(FPlatformUserId PlatformUserId, bool bIsVirtual)
{
	...
	UE_LOGF(LogSlate, Log, "Slate User Registered.  User Index %d, Is Virtual User: %d", UserIndex, bIsVirtual);
...
void FSlateApplication::UnregisterUser(int32 UserIndex)
{
	if ( UserIndex < Users.Num() )
	{
		UE_LOGF(LogSlate, Log, "Slate User Unregistered.  User Index %d", UserIndex);

		ClearUserFocus(UserIndex, EFocusCause::SetDirectly);
		ClearPendingNavigationContext(UserIndex);
...
FPointerEvent FSlateApplication::TransformPointerEvent(const FPointerEvent& PointerEvent, const TSharedPtr<SWindow>& Window) const
{
	...
	if (Window)
	{
		if (TransformFullscreenMouseInput && !GIsEditor && Window->GetWindowMode() == EWindowMode::Fullscreen)
		{
			...
			const FVector2f PositionInScreen = Window->GetPositionInScreen();
			const FVector2f Ratio = WindowSize / DisplaySize;
			FVector2f Position = PointerEvent.GetScreenSpacePosition();
			Position -= PositionInScreen;
			Position *= Ratio;
			Position += PositionInScreen;
			FVector2f LastPosition = PointerEvent.GetLastScreenSpacePosition();
			LastPosition -= PositionInScreen;
			LastPosition *= Ratio;
			LastPosition += PositionInScreen;
			TransformedPointerEvent = FPointerEvent(PointerEvent, Position, LastPosition);
		}
	}
...
bool FSlateApplication::OnMouseDown( const TSharedPtr< FGenericWindow >& PlatformWindow, const EMouseButtons::Type Button, const FVector2D CursorPos )
{
	...
	// Update PressedMouseButtons before constructing the event so that the value-copy
	// snapshot in FPointerEvent reflects the post-press state. The matching Add inside
	// ProcessMouseButtonDownEvent becomes a no-op for this path (TSet is idempotent)
	// but is still needed for touch events that call ProcessMouseButtonDownEvent directly.
	if (GetUserIndexForMouse() == CursorUserIndex)
	{
...
#if PLATFORM_MAC
	void* ActiveWindow = FMacPlatformApplicationMisc::GetActiveWindowPtr();
	...
	)
	{
		...
		if ( !bIsVirtualInteraction )
		{
			...
			if ( bNeedToActivateWindow || ( TopLevelWindow.IsValid() && TopLevelWindow->GetNativeWindow()->GetOSWindowHandle() != ActiveWindow ) )
			{
				...
				if ( !bIsContextMenu && bIsDetectingLMBDrag && ActiveWindow == FMacPlatformApplicationMisc::GetActiveWindowPtr())
				{
...
FReply FSlateApplication::RoutePointerUpEvent(const FWidgetPath& WidgetsUnderPointer, const FPointerEvent& PointerEvent)
{
	...
	if (PointerEvent.GetEffectingButton() == EKeys::LeftMouseButton && !bIsDragDropping)
	{
		...
		else if (FPlatformApplicationMisc::GetActiveWindowPtr() == nullptr)
		{
			FPlatformApplicationMisc::ActivateApplication();
		}
	}
...
bool FSlateApplication::OnMouseDoubleClick( const TSharedPtr< FGenericWindow >& PlatformWindow, const EMouseButtons::Type Button, const FVector2D CursorPos )
{
	...
	// Update PressedMouseButtons before constructing the event — same reasoning as OnMouseDown.
	if (GetUserIndexForMouse() == CursorUserIndex)
	{
		PressedMouseButtons.Add(Key);
	}
...
bool FSlateApplication::OnMouseUp( const EMouseButtons::Type Button, const FVector2D CursorPos )
{
	...
	// Update PressedMouseButtons before constructing the event so the value-copy snapshot
	// reflects the post-release state (button is no longer down). The matching Remove inside
	// ProcessMouseButtonUpEvent becomes a no-op for this path but is still needed for touch
	// events routed via ProcessTouchEndedEvent -> ProcessMouseButtonUpEvent directly.
	if (GetUserIndexForMouse() == CursorUserIndex)
	{
	...
	if ( InGestureEvent )
	{
		...
		default:
			bShouldProcessEvent = (InGestureEvent->GetGestureDelta() != FVector2f::ZeroVector) ||
				(InGestureEvent->GetGesturePhase() == EGesturePhase::Started) ||
				(InGestureEvent->GetGesturePhase() == EGesturePhase::Ended);
			break;
		}
	}
	else
...
EUINavigation FSlateApplication::NavigateFromWidget(const uint32 InUserIndex, TSharedPtr<SWidget> InNavigationSource, const TArray<EUINavigation>& InNavigationTypes)
{
	...
	{
		UE_LOGF(LogSlate, Log, "Failed to navigate from widget %ls. No widget currently in Focus", *(InNavigationSource->ToString()));
	...
	UE_LOGF(LogSlate, Log, "Failed to navigate from widget %ls. No neighboring widgets could be reached.", *(InNavigationSource->ToString()));
	return EUINavigation::Invalid;
}

FSlateApplication::FNavigationResult FSlateApplication::CalculateDestinationWidget(const FWidgetPath& NavigationSource, const FNavigationReply& NavigationReply, EUINavigation NavigationType, uint32 UserIndex, const FArrangedWidget& BoundaryWidget) const
{
	if (!NavigationSource.IsValid())
	{
		return FNavigationResult();
	}

	FNavigationResult Result;
	...
	if ( NavigationReply.GetBoundaryRule() == EUINavigationRule::Explicit )
	{
		...
		{
			Result.DestinationWidget = NavigationReply.GetFocusRecipient();
			Result.bAlwaysHandleNavigationAttempt = true;

#if WITH_SLATE_DEBUGGING
			Result.NavigationMethod = ESlateDebuggingNavigationMethod::Explicit;
#endif
		}
#if WITH_SLATE_DEBUGGING
		else
		{
			...
			UE_LOGF(LogSlate, VeryVerbose, "Could not Explicitly navigate to widget '%ls' because '%ls'", *FReflectionMetaData::GetWidgetDebugInfo(FocusRecipient), Reason);
		}
#endif
	}
	else if ( NavigationReply.GetBoundaryRule() == EUINavigationRule::Custom )
	{
		...
		if ( FocusDelegate.IsBound() )
		{
			...
			Result.DestinationWidget = FocusDelegate.Execute(NavigationType);
			Result.bAlwaysHandleNavigationAttempt = true;

#if WITH_SLATE_DEBUGGING
			Result.NavigationMethod = ESlateDebuggingNavigationMethod::CustomDelegateBound;
#endif
		}
		else
		{
#if WITH_SLATE_DEBUGGING
			Result.NavigationMethod = ESlateDebuggingNavigationMethod::CustomDelegateUnbound;
#endif
		}
	}
	else
	{
		...
		if (NavigationType == EUINavigation::Next || NavigationType == EUINavigation::Previous)
		{
			...
			checkf(NewFocusedWidgetPath.IsValid(), TEXT("This was assuming the `NewFocusedWidgetPath` is valid already"));

			// Resolve the Widget Path
			FArrangedWidget& NewFocusedArrangedWidget = NewFocusedWidgetPath.Widgets.Last();
			Result.DestinationWidget = NewFocusedArrangedWidget.Widget;

#if WITH_SLATE_DEBUGGING
			Result.NavigationMethod = ESlateDebuggingNavigationMethod::NextOrPrevious;
#endif
		}
		else
		{
			...
			Result.DestinationWidget = NavigationSource.GetDeepestWindow()->GetHittestGrid().FindNextFocusableWidget(FocusedArrangedWidget, NavigationType, NavigationReply, BoundaryWidget, UserIndex);

#if WITH_SLATE_DEBUGGING
			Result.NavigationMethod = ESlateDebuggingNavigationMethod::HitTestGrid;
#endif
		}
	}
...
bool FSlateApplication::AttemptNavigation(const FWidgetPath& NavigationSource, const FNavigationEvent& NavigationEvent, const FNavigationReply& NavigationReply, const FArrangedWidget& BoundaryWidget)
{
	...
	const EUINavigation NavigationType = NavigationEvent.GetNavigationType();
	const uint32 UserIndex = NavigationEvent.GetUserIndex();
...
#if WITH_SLATE_DEBUGGING
	FSlateDebugging::BroadcastAttemptNavigation(NavigationEvent, NavigationReply, NavigationSource, DestinationResult.DestinationWidget, DestinationResult.NavigationMethod);
#endif

	FScopedNavigationTransition ScopedNavigationTransition(NavigationType, GetUser(UserIndex));

	return ExecuteNavigation(NavigationSource, DestinationResult.DestinationWidget, NavigationEvent.GetUserIndex(), DestinationResult.bAlwaysHandleNavigationAttempt);
}
...
bool FSlateApplication::HandleTouchGesture( FInputDeviceId DeviceId, EGestureEvent GestureType, EGesturePhase GesturePhase, const FVector2D &Delta, const float Value, bool bIsDirectionInvertedFromDevice )
{
...
{
	// Software gesture path (e.g. Mac trackpad via GestureRecognizer) has no device ID.
	// Use device 0 to preserve the original UserIndex=0 routing behavior.
	return HandleTouchGesture(FInputDeviceId::CreateFromInternalId(0), GestureType, EGesturePhase::Update, Delta, MouseWheelDelta, bIsDirectionInvertedFromDevice);
}

bool FSlateApplication::OnNativeGestureBegin(EGestureEvent GestureType, const FVector2D& Delta, float Value, bool bIsDirectionInvertedFromDevice, FPlatformUserId PlatformUserId, FInputDeviceId DeviceId)
{
	return HandleTouchGesture(DeviceId, GestureType, EGesturePhase::Started, Delta, Value, bIsDirectionInvertedFromDevice);
}

bool FSlateApplication::OnNativeGestureUpdate(EGestureEvent GestureType, const FVector2D& Delta, float Value, bool bIsDirectionInvertedFromDevice, FPlatformUserId PlatformUserId, FInputDeviceId DeviceId)
{
	return HandleTouchGesture(DeviceId, GestureType, EGesturePhase::Update, Delta, Value, bIsDirectionInvertedFromDevice);
}

bool FSlateApplication::OnNativeGestureEnd(EGestureEvent GestureType, const FVector2D& Delta, float Value, bool bIsDirectionInvertedFromDevice, FPlatformUserId PlatformUserId, FInputDeviceId DeviceId)
{
	return HandleTouchGesture(DeviceId, GestureType, EGesturePhase::Ended, Delta, Value, bIsDirectionInvertedFromDevice);
}
...
bool FSlateApplication::OnTouchStarted( const TSharedPtr< FGenericWindow >& PlatformWindow, const FVector2D& Location, float Force, int32 TouchIndex, FPlatformUserId PlatformUserId, FInputDeviceId DeviceId )
{
	...
	if (TouchIndex >= (int32)ETouchIndex::CursorPointerIndex)
	{
		...
		UE_LOGF(LogSlate, Warning, "Maximum Touch Index Exceeded, %d, the maximum index allowed is %d", TouchIndex, (((int32)ETouchIndex::CursorPointerIndex) - 1));
#endif
		return false;
	}
...
void FSlateApplication::OnRegularWindowMovedOrResized(const TSharedRef<FGenericWindow>& PlatformWindow)
{
	TSharedPtr< SWindow > SlateWindow = FSlateWindowHelper::FindWindowByPlatformWindow(SlateWindows, PlatformWindow);

	if (SlateWindow.IsValid() && SlateWindow->IsRegularWindow())
	{
		OnRegularWindowMovedOrResizedEvent.Broadcast(SlateWindow.ToSharedRef());
	}
}
...
{
	//UE_LOGF(LogSlate, Warning, "Window being %ls: %p", ActivateEvent.GetActivationType() == FWindowActivateEvent::EA_Deactivate ? TEXT("Deactivated") : TEXT("Activated"), &(ActivateEvent.GetAffectedWindow().Get()));
...
EDropEffect::Type FSlateApplication::OnDragDrop(const TSharedPtr< FGenericWindow >& Window)
{
	...
	if (GetCursorUser()->IsDragDropping())
	{
		// Update PressedMouseButtons before constructing the synthesized mouse-up event so
		// the value-copy snapshot reflects the post-release state. Same reasoning as OnMouseUp.
		if (GetUserIndexForMouse() == CursorUserIndex)
		{
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FSlateApplication::InitializeAsStandaloneApplication);
...
void FSlateApplication::SetWidgetReflector(const TSharedRef<IWidgetReflector>& WidgetReflector)
{
	...
	if ( DebugObjectAccessDelegate.IsBound() )
	{
		WidgetReflector->SetDebugObjectAccessDelegate(DebugObjectAccessDelegate);
	}
	...
	Parent->GetAllChildren()->ForEachWidget([&bResult, Parent, &AllWidgets](SWidget& ChildWidget)
		{
			if (&ChildWidget != &SNullWidget::NullWidget.Get())
			{
				...
				{
					bResult = false;
					UE_LOGF(LogSlate, Warning, "The widget '%ls' is owned by more than one parent. 1:'%ls' 2:'%ls'."
				...
				{
					bResult = false;
					UE_LOGF(LogSlate, Warning, "The widget '%ls' has the wrong parent.", *FReflectionMetaData::GetWidgetDebugInfo(ChildWidget));
				}
				...
				if (ChildWidget.GetVisibility().IsVisible() && (bInsideInvalidationRoot || ChildWidget.Debug_GetLastPaintFrame() == LastPaintFrame))
				{
					...
					if (bIsChildDeferredPaint == bIsDeferredPaint)
					{
						...
						if (!bLayerInIsValid || !bLayerOutIsValid)
						{
							...
							{
								bResult = false;
								UE_LOGF(LogSlate, Warning, "The widget '%ls' LayerId is invalid. Parent: [%d,%d] Child: [%d,%d]."
									, *FReflectionMetaData::GetWidgetDebugInfo(ChildWidget)
									, InLayerId
									, OutLayerId
									, ChildInLayerId
