...
static bool bEnableTooltipForceFieldDraw = false;
FAutoConsoleVariableRef CVarEnableTooltipForceFieldDraw(
	TEXT("Slate.EnableTooltipForceFieldDraw"),
	bEnableTooltipForceFieldDraw,
	TEXT("If turned on, the rectangles of effective tooltip force fields will be visualized."));
#endif
	...
	, Cursor(InCursor)
{
	...
	UE_LOGF(LogSlate, Log, "New Slate User Created. Platform User Id %d, User Index %d, Is Virtual User: %d", PlatformUser.GetInternalId(), UserIndex, IsVirtualUser());
	...
	, Cursor(InCursor)
{
	...
	UE_LOGF(LogSlate, Log, "New Slate User Created.  Platform User Id %d,  Old User Index: %d  , Is Virtual User: %d", PlatformUser.GetInternalId(), UserIndex, IsVirtualUser());
...
{
#if WITH_SLATE_DEBUGGING
	ClearForceFieldDebug();
#endif

	UE_LOGF(LogSlate, Log, "Slate User Destroyed.  User Index %d, Is Virtual User: %d", UserIndex, IsVirtualUser());
}
...
void FSlateUser::ReleaseCapture(uint32 PointerIndex)
{
	if (const FWeakWidgetPath* CaptorPath = PointerCaptorPathsByIndex.Find(PointerIndex))
	{
		...
		TSharedPtr<SWidget> CaptorWidget = CaptorPath->GetLastWidget().Pin();
		CaptorPath = nullptr;

		// Make sure we don't recursively come back here, in case OnMouseCaptureLost causes
		// release capture again.
...
void FSlateUser::SetPointerPosition(uint32 PointerIndex, int32 PosX, int32 PosY)
{
	...
	{
		UE_LOGF(LogSlate, Verbose, "SlateUser [%d] moving cursor @ (%d, %d)", UserIndex, PosX, PosY );
...
void FSlateUser::ShowTooltip(const TSharedRef<IToolTip>& InTooltip, const UE::Slate::FDeprecateVector2DParameter& InLocation, TSharedPtr<SWindow> OwnerWindow)
{
	CloseTooltip();

	ActiveTooltipInfo.Tooltip = InTooltip;

	// Establish the tooltip content in the window (per-owner for correct Wayland parenting)
	TSharedRef<SWindow> TooltipWindow = GetOrCreateTooltipWindow(OwnerWindow);
	TooltipWindowPtr = TooltipWindow;	
...
void FSlateUser::CloseTooltip()
{
	...
	// Hide the tooltip window (don't destroy it - we'll reuse it for that owner)
...
void FSlateUser::LockCursorInternal(const FWidgetPath& WidgetPath)
{
	...
	if (NativeWindow && (NativeWindow->IsForegroundWindow() || NativeWindow->SupportsDynamicCursorLock()))
	{
		...
		if (FSlateApplication::Get().GetTransformFullscreenMouseInput() && !GIsEditor && NativeWindow->GetWindowMode() == EWindowMode::Fullscreen)
		{
			...
			const FVector2f DisplaySize = CachedDisplayMetrics.MonitorInfo.IsEmpty()
				...
				: WidgetPath.GetWindow()->GetFullScreenInfo().GetSize2f();
			FVector2f DisplayDistortion = DisplaySize / SlateClipRect.GetSize();

			SlateClipRect.Right = ((SlateClipRect.Right - SlateClipRect.Left) * DisplayDistortion.X) + SlateClipRect.Left;
			SlateClipRect.Bottom = ((SlateClipRect.Bottom - SlateClipRect.Top) * DisplayDistortion.Y) + SlateClipRect.Top;
		}

		// Note: Slate uses floating point bounds, but the cursor needs integers. Round the clip rect inwardly, to guarantee it remains inside the
		// widget.  Truncating the left and/or top would cause the clip rect to fall outside the geometry in cases where the widget's left/top
		// coordinates are fractional.
		RECT ClipRect;
		ClipRect.left = FMath::RoundToPositiveInfinity(SlateClipRect.Left);
		ClipRect.top = FMath::RoundToPositiveInfinity(SlateClipRect.Top);
		ClipRect.right = FMath::RoundToNegativeInfinity(SlateClipRect.Right);
		ClipRect.bottom = FMath::RoundToNegativeInfinity(SlateClipRect.Bottom);
...
TSharedRef<SWindow> FSlateUser::GetOrCreateTooltipWindow(TSharedPtr<SWindow> OwnerWindow)
{
	// Platforms without per-toplevel parent constraints share one tooltip window across
	// owners -- avoids spawning a Vulkan viewport per owner.
	if (!FSlateApplication::Get().GetPlatformApplication()->RequiresPerWindowTooltips())
	{
		OwnerWindow.Reset();
	}

	// Look up existing tooltip window for this owner
	if (OwnerWindow)
	{
		TWeakPtr<SWindow>* FoundTooltip = TooltipWindowsByOwner.Find(OwnerWindow);
		if (FoundTooltip && FoundTooltip->IsValid())
		{
			return FoundTooltip->Pin().ToSharedRef();
		}
	}
	else
	{
		// No owner -- check for a previously created ownerless tooltip
		TWeakPtr<SWindow> NullKey;
		TWeakPtr<SWindow>* FoundTooltip = TooltipWindowsByOwner.Find(NullKey);
		if (FoundTooltip && FoundTooltip->IsValid())
		{
			return FoundTooltip->Pin().ToSharedRef();
		}
	...
	// Create a new tooltip window for this owner
	TSharedRef<SWindow> NewTooltipWindow = SWindow::MakeToolTipWindow();
	if (OwnerWindow)
	{
		TooltipWindowsByOwner.Add(OwnerWindow, NewTooltipWindow);
	}
	else
	{
		TooltipWindowsByOwner.Add(nullptr, NewTooltipWindow);
	}

	// Plumb OwnerWindow through to the native layer when per-toplevel parents are required.
	// Otherwise the platform falls back to SDL_GetMouseFocus(), which can disagree with the
	// Slate owner when the cursor briefly sits over a different toplevel at creation time.
	if (OwnerWindow.IsValid()
		&& FSlateApplication::Get().GetPlatformApplication()->RequiresPerWindowTooltips())
	{
		FSlateApplication::Get().AddWindowAsNativeChild(NewTooltipWindow, OwnerWindow.ToSharedRef(), /*bShowImmediately =*/false);
	}
	else
	{
...
#if WITH_SLATE_DEBUGGING
void FSlateUser::ClearForceFieldDebug()
{
	if (!DebugForceFieldRects.IsEmpty())
	{
		DebugForceFieldRects.Reset();
		FSlateDebugging::PaintDebugElements.RemoveAll(this);
	}
}

void FSlateUser::HandlePaint(const FPaintArgs& InArgs, const FGeometry& InAllottedGeometry,
	FSlateWindowElementList& InOutDrawElements, int32& InOutLayerId)
{
	for (const FSlateRect& Rect : DebugForceFieldRects)
	{
		FSlateDrawElement::MakeBox(
			...
			InAllottedGeometry.ToPaintGeometry(
				Rect.GetSize2f(),
				FSlateLayoutTransform(Rect.GetTopLeft2f() - InArgs.GetWindowToDesktopTransform())
			),
...
{
	UE_CLOGF(PointerPositionsByIndex.Contains(TouchEvent.GetPointerIndex()), LogSlate, Error, "SlateUser [%d] notified of a touch starting for pointer [%d] without finding out it ever ended.", TouchEvent.GetUserIndex(), TouchEvent.GetPointerIndex());
...
void FSlateUser::UpdateTooltip(const FMenuStack& MenuStack, bool bCanSpawnNewTooltip)
{
	...
	if (bCheckForTooltipChanges)
	{
		...
		{
			// Don't query tooltip windows themselves for tooltips
			bool bIsOverTooltipWindow = false;
			for (const TPair<TWeakPtr<SWindow>, TWeakPtr<SWindow>>& Pair : TooltipWindowsByOwner)
			{
				if (WidgetsUnderCursor.GetWindow() == Pair.Value.Pin())
				{
					bIsOverTooltipWindow = true;
					break;
				}
			}
			if (!bIsOverTooltipWindow)
			{
	...
	for (int32 WidgetIndex = WidgetsToQueryForTooltip.Widgets.Num() - 1; WidgetIndex >= 0; --WidgetIndex)
	{
		...
		{
			const FSlateRect WidgetRect = ArrangedWidget->Geometry.GetLayoutBoundingRect();
			if (!ForceFieldRect.IsSet())
			{
				ForceFieldRect = WidgetRect;
			}
			else
			{
				...
				ForceFieldRect = ForceFieldRect->Expand(WidgetRect);
			}
			
			// Expand based on the attribute to specify a custom zone
			TOptional<FSlateRect> ExpansionRect = CurWidget->GetToolTipForceFieldExpansion();
			if (ExpansionRect.IsSet())
			{
				ForceFieldRect = ForceFieldRect->Expand(ExpansionRect.GetValue());
			}
			
			ForceFieldRect = (1.0f / DPIScaleFactor) * ForceFieldRect.GetValue();

#if WITH_SLATE_DEBUGGING
			if (bEnableTooltipForceFieldDraw)
			{
				DebugForceFieldRects.Add(WidgetRect);
			}
#endif
		}
	}
	...
	if (!DebugForceFieldRects.IsEmpty())
	{
		FSlateDebugging::PaintDebugElements.AddRaw(this, &FSlateUser::HandlePaint);
	}
#endif

	TSharedPtr<SWidget> NewTooltipVisualizer;
	TSharedPtr<IToolTip> ActiveTooltip = ActiveTooltipInfo.Tooltip.Pin();
	bool bTooltipChanged = NewTooltip != ActiveTooltip;
	if (bTooltipChanged)
	{
	...
	if (ForceFieldRect.IsSet() && !ActiveTooltipInfo.WasInteractive)
	{
		...
		{
			// If in the bottom right corner there's no space for the tooltip, let's try to shift it upwards instead
			const FSlateRect WorkArea = FSlateApplication::Get().GetPreferredWorkArea();
			if (DesiredLocation.X + TooltipShift.X + TooltipSize.X > WorkArea.Right &&
				DesiredLocation.Y + TooltipShift.Y + TooltipSize.Y > WorkArea.Bottom)
			{
				TooltipShift.Y = ForceFieldRect->Top - SlateDefs::TooltipOffsetFromForceField.Y - TooltipSize.Y - DesiredLocation.Y;
			}
	...
	// On platforms where tooltips are parented to a specific toplevel window, detect when the
	// source window changes (e.g. mouse moved from main window to asset browser) and recreate
	// the tooltip so it belongs to the correct window.
	if (SlateApp.GetPlatformApplication()->RequiresPerWindowTooltips())
	{
		TSharedPtr<SWindow> CurrentSourceWindow = WidgetsToQueryForTooltip.IsValid()
			? WidgetsToQueryForTooltip.GetWindow() : TSharedPtr<SWindow>();
		if (CurrentSourceWindow.IsValid() && ActiveTooltipInfo.SourceWindow.IsValid() &&
			CurrentSourceWindow != ActiveTooltipInfo.SourceWindow.Pin())
		{
			ResetTooltipWindow();
			bTooltipChanged = true;
		}
		if (CurrentSourceWindow.IsValid())
		{
			ActiveTooltipInfo.SourceWindow = CurrentSourceWindow;
		}
	}
	...
	if (bTooltipChanged)
	{
		...
		if (NewTooltip || (ActiveTooltip && !ActiveTooltip->IsInteractive()))
		{
			...
			if (NewTooltip && bCanSpawnNewTooltip)
			{
				...
				{
					ShowTooltip(NewTooltip.ToSharedRef(), DesiredLocation, WidgetsToQueryForTooltip.GetWindow());
...
{
	// Destroy all per-owner tooltip windows
	for (TPair<TWeakPtr<SWindow>, TWeakPtr<SWindow>>& Pair : TooltipWindowsByOwner)
	{
...
{
	// Check if this window is ANY of our tooltip windows
	bool bIsTooltipWindow = false;
	for (const TPair<TWeakPtr<SWindow>, TWeakPtr<SWindow>>& Pair : TooltipWindowsByOwner)
	{
		if (WindowToTest == Pair.Value.Pin())
		{
