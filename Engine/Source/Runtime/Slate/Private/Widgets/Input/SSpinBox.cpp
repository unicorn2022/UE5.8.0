...
inline void SSpinBox<NumericType>::PrivateRegisterAttributes(FSlateAttributeInitializer& AttributeInitializer)
{
	SLATE_ADD_MEMBER_ATTRIBUTE_DEFINITION(AttributeInitializer, bDragging, EInvalidateWidgetReason::Paint);
	...
	SLATE_ADD_MEMBER_ATTRIBUTE_DEFINITION(AttributeInitializer, Delta, EInvalidateWidgetReason::Paint);
	SLATE_ADD_MEMBER_ATTRIBUTE_DEFINITION(AttributeInitializer, MinValue, EInvalidateWidgetReason::Paint);
	SLATE_ADD_MEMBER_ATTRIBUTE_DEFINITION(AttributeInitializer, MaxValue, EInvalidateWidgetReason::Paint);
	SLATE_ADD_MEMBER_ATTRIBUTE_DEFINITION(AttributeInitializer, MinSliderValue, EInvalidateWidgetReason::Paint);
	SLATE_ADD_MEMBER_ATTRIBUTE_DEFINITION(AttributeInitializer, MaxSliderValue, EInvalidateWidgetReason::Paint);

	SLATE_ADD_MEMBER_ATTRIBUTE_DEFINITION(AttributeInitializer, SliderExponent, EInvalidateWidgetReason::Paint);
	SLATE_ADD_MEMBER_ATTRIBUTE_DEFINITION(AttributeInitializer, SliderExponentNeutralValue, EInvalidateWidgetReason::Paint);
}

template <typename NumericType>
SSpinBox<NumericType>::SSpinBox()
	: ValueAttribute(*this, 0)
	, Delta(*this, 0)
	, SliderExponent(*this, 1.f)
	, SliderExponentNeutralValue(*this)
	, MinValue(*this, TOptional<NumericType>{})
	, MaxValue(*this, TOptional<NumericType>{})
	, MinSliderValue(*this, TOptional<NumericType>{})
	, MaxSliderValue(*this, TOptional<NumericType>{})
	, bDragging(*this, false)
{
}
...
SSpinBox<NumericType>::~SSpinBox()
{
	if (bDragging.Get() || PointerDraggingSliderIndex != INDEX_NONE)
	{
		CancelMouseCapture();
	}
}
...
void SSpinBox<NumericType>::Construct(const FArguments& InArgs)
{
	...
	ValueAttribute.Assign(*this, InArgs._Value, ESlateAttributeBindAction::Initialize);
	...
	MinValue.Assign(*this, InArgs._MinValue, ESlateAttributeBindAction::Initialize);
	MaxValue.Assign(*this, InArgs._MaxValue, ESlateAttributeBindAction::Initialize);
	MinSliderValue.Assign(*this, (InArgs._MinSliderValue.Get().IsSet()) ? InArgs._MinSliderValue : MinValue.Get(), ESlateAttributeBindAction::Initialize);
	MaxSliderValue.Assign(*this, (InArgs._MaxSliderValue.Get().IsSet()) ? InArgs._MaxSliderValue : MaxValue.Get(), ESlateAttributeBindAction::Initialize);
	...
	SliderExponent.Assign(*this, InArgs._SliderExponent);

	bIsSliderExponentNeutralValueSet = InArgs._SliderExponentNeutralValue.IsSet();
	SliderExponentNeutralValue.Assign(*this, InArgs._SliderExponentNeutralValue);

	DistanceDragged = 0.0f;
	PreDragValue = NumericType();

	Delta.Assign(*this, InArgs._Delta);
	ShiftMultiplier = InArgs._ShiftMultiplier;
	CtrlMultiplier = InArgs._CtrlMultiplier;
	ShiftAltMultiplier = InArgs._ShiftAltMultiplier;
	CtrlAltMultiplier = InArgs._CtrlAltMultiplier;
	...
	FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	...
	const bool bActiveFeedback = bDragging.Get() || IsInTextMode();
	...
	if (!bUnlimitedSpinRange)
	{
		...
		{
			if (bIsSliderExponentNeutralValueSet && SliderExponentNeutralValue.Get() > GetMinSliderValue() && SliderExponentNeutralValue.Get() < GetMaxSliderValue())
			{
...
void SSpinBox<NumericType>::ApplySliderMaxValueChanged(float SliderDeltaToAdd, bool UpdateOnlyIfHigher)
{
	...
	if (MaxSliderValue.Get().IsSet())
	{
		...
		if ((NewMaxSliderValue + (NumericType)SliderDeltaToAdd > GetMaxSliderValue() && UpdateOnlyIfHigher) || !UpdateOnlyIfHigher)
		{
			...
			if (!MaxSliderValue.IsBound(*this)) // simple value so we can update it without breaking the mechanic otherwise it must be handle by the callback implementer
			{
...
{
	check(SupportDynamicSliderMinValue.Get());

	NumericType NewMinSliderValue = std::numeric_limits<NumericType>::min();

	if (MinSliderValue.Get().IsSet())
	{
		...
		if ((NewMinSliderValue + (NumericType)SliderDeltaToAdd < GetMinSliderValue() && UpdateOnlyIfLower) || !UpdateOnlyIfLower)
		{
			...
			if (!MinSliderValue.IsBound(*this)) // simple value so we can update it without breaking the mechanic otherwise it must be handle by the callback implementer
			{
...
FReply SSpinBox<NumericType>::OnMouseMove(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	if (PointerDraggingSliderIndex == MouseEvent.GetPointerIndex() && bEnableSlider)
	{
		...
		else
		{
			...
			if (!bUnlimitedSpinRange)
			{
				bool HasValidExponentNeutralValue = bIsSliderExponentNeutralValueSet && SliderExponentNeutralValue.Get() > GetMinSliderValue() && SliderExponentNeutralValue.Get() < GetMaxSliderValue();
			...
			else
			{
				...
				if (LinearDeltaSensitivity.IsSet() && LinearDeltaSensitivity.Get() != 0 && Delta.Get() > 0)
				{
...
FReply SSpinBox<NumericType>::OnMouseWheel(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	if (bEnableWheel && PointerDraggingSliderIndex == INDEX_NONE && HasKeyboardFocus())
	{
		...
		// Apply multiplier based on mouse event
		Step *= GetInputEventMultiplier(MouseEvent);
...
{
	if (!bDragging.Get() && (InFocusEvent.GetCause() == EFocusCause::Navigation || InFocusEvent.GetCause() == EFocusCause::SetDirectly))
	{
		EnterTextMode();
...
{ return ValueAttribute.ToAttribute(*this); }
...
template <typename NumericType>
void SSpinBox<NumericType>::SetValue(TAttribute<NumericType> InValueAttribute, const bool bShouldCommit)
{
	ValueAttribute.Assign(*this, MoveTemp(InValueAttribute), ESlateAttributeBindAction::Initialize);
	const NumericType LocalValueAttribute = ValueAttribute.Get();
...
template <typename NumericType>
void SSpinBox<NumericType>::SetMinValue(TAttribute<TOptional<NumericType>> InMinValue)
{
	MinValue.Assign(*this, MoveTemp(InMinValue), ESlateAttributeBindAction::Initialize);
	UpdateIsSpinRangeUnlimited();
}
...
template <typename NumericType>
void SSpinBox<NumericType>::SetMaxValue(TAttribute<TOptional<NumericType>> InMaxValue)
{
	MaxValue.Assign(*this, MoveTemp(InMaxValue), ESlateAttributeBindAction::Initialize);
	UpdateIsSpinRangeUnlimited();
}

template <typename NumericType>
bool SSpinBox<NumericType>::IsMinSliderValueBound() const
{ return MinSliderValue.IsBound(*this); }
...
template <typename NumericType>
void SSpinBox<NumericType>::SetMinSliderValue(TAttribute<TOptional<NumericType>> InMinSliderValue)
{
	MinSliderValue.Assign(*this, InMinSliderValue.Get().IsSet() ? MoveTemp(InMinSliderValue) : MinValue.Get(), ESlateAttributeBindAction::Initialize);
	UpdateIsSpinRangeUnlimited();
}

template <typename NumericType>
bool SSpinBox<NumericType>::IsMaxSliderValueBound() const
{ return MaxSliderValue.IsBound(*this); }
...
template <typename NumericType>
void SSpinBox<NumericType>::SetMaxSliderValue(TAttribute<TOptional<NumericType>> InMaxSliderValue)
{
	MaxSliderValue.Assign(*this, InMaxSliderValue.Get().IsSet() ? MoveTemp(InMaxSliderValue) : MaxValue.Get(), ESlateAttributeBindAction::Initialize);
	UpdateIsSpinRangeUnlimited();
}
...
template <typename NumericType>
void SSpinBox<NumericType>::SetSliderExponent(TAttribute<float> InSliderExponent)
{ SliderExponent.Assign(*this, MoveTemp(InSliderExponent)); }
...
template <typename NumericType>
void SSpinBox<NumericType>::SetMinDesiredWidth(TAttribute<float> InMinDesiredWidth)
{ MinDesiredWidth = MoveTemp(InMinDesiredWidth); }
...
void SSpinBox<NumericType>::CommitValue(NumericType NewValue, double NewSpinValue, ECommitMethod CommitMethod, ETextCommit::Type OriginalCommitInfo, const bool bShouldCommit)
{
	...
	if (!ValueAttribute.IsBound(*this))
	{
		ValueAttribute.Assign(*this, NewValue);
	}

	ValueAttribute.UpdateNow(*this);
...
template <typename NumericType>
float SSpinBox<NumericType>::GetInputEventMultiplier(const FInputEvent& InEvent) const
{
	float Multiplier = 1.f;

	if (InEvent.IsShiftDown())
	{
		if (InEvent.IsAltDown())
		{
			Multiplier = ShiftAltMultiplier.Get();
		}
		else
		{
			Multiplier = ShiftMultiplier.Get();
		}
	}
	else if (InEvent.IsControlDown())
	{
		if (InEvent.IsAltDown())
		{
			Multiplier = CtrlAltMultiplier.Get();
		}
		else
		{
			Multiplier = CtrlMultiplier.Get();
		}
	}
...
double SSpinBox<NumericType>::GetDefaultStepSize(const FInputEvent& InputEvent)
{
	...
	// Apply multiplier based on mouse event
	Step *= GetInputEventMultiplier(InputEvent);
