...
void SComboButton::Construct( const FArguments& InArgs )
{
	...
	const FMargin DownArrowMargin = [&InArgs]
	{
		if (InArgs._HasDownArrow)
		{
			if (InArgs._ButtonContent.Widget == SNullWidget::NullWidget)
			{
				// If there is no button content, the down arrow sits alone.
				// Consistent left/right padding makes it the most visually pleasing.
				FMargin Result = InArgs._ComboButtonStyle->DownArrowPadding;
				const float SidePadding = FMath::Min(Result.Left, Result.Right); 
				Result.Right = SidePadding;
				Result.Left = SidePadding;
				return Result;
			}
			return InArgs._ComboButtonStyle->DownArrowPadding;
		}
		return FMargin(0);
	}();
	...
	// Repel tooltips from appearing on the button and menu content when opened...
	EnableToolTipForceField(true);
	SetToolTipForceFieldExpansion(TAttribute<TOptional<FSlateRect>>::CreateSPLambda(this, [this]()->TOptional<FSlateRect>
		{
			if (IsOpen() && MenuContent.IsValid())
			{
				return MenuContent->GetCachedGeometry().GetLayoutBoundingRect();	
			}
