...
FReply SDockTab::OnMouseButtonDown( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
	if ( !HasMouseCapture() )
	{
		...
		{
			// Show context menu for this tab otherwise let tab well handle it
			if (const TSharedPtr<SDockingTabStack> DockingTabStack = GetParentDockTabStack())
			{
				const FWidgetPath WidgetPath = MouseEvent.GetEventPath() != nullptr ? *MouseEvent.GetEventPath() : FWidgetPath();
				DockingTabStack->ShowContextMenu(SharedThis(this), WidgetPath);
				return FReply::Handled();
			}

			// let the tab well bring up the context menu or whatever it wants to do with the right click.
...
bool SDockTab::IsInSidebar() const
{
	if (TSharedPtr<SDockingArea> DockingArea = GetDockArea())
	{
		return DockingArea->IsTabInSidebar(SharedThis(const_cast<SDockTab*>(this)));
	}
...
SDockTab::SDockTab()
	...
	, AutosizePolicy(EDockTabAutosizePolicy::None)
...
void SDockTab::Construct( const FArguments& InArgs )
{
	...
	this->AutosizePolicy = (InArgs._ShouldAutosize && InArgs._AutosizePolicy == EDockTabAutosizePolicy::None)
		? EDockTabAutosizePolicy::LayoutAxis : InArgs._AutosizePolicy; // Map old arg to new arg
...
void SDockTab::CheckTabAllowed()
{
	if (!LayoutIdentifier.TabType.IsNone())
	{
		...
		if (Manager)
		{
			...
			{
				UE_LOGF(LogSlate, Verbose, "Closing tab because type does not pass filter: %ls", *LayoutIdentifier.TabType.ToString());
				RequestCloseTab();
			}
		}
	}
...
FSlateColor SDockTab::GetIconColor() const
{
	...
	{
		return IconColor.Get().CopyWithNewOpacity(0.7f);
	}
...
{
	return AutosizePolicy != EDockTabAutosizePolicy::None;
}

void SDockTab::SetShouldAutosize(const bool bNewShouldAutosize)
{
	AutosizePolicy = bNewShouldAutosize ? EDockTabAutosizePolicy::LayoutAxis : EDockTabAutosizePolicy::None;
}

EDockTabAutosizePolicy SDockTab::GetAutosizePolicy() const
{
	return AutosizePolicy;
}

void SDockTab::SetAutosizePolicy(const EDockTabAutosizePolicy InNewPolicy)
{
	AutosizePolicy = InNewPolicy;
}
