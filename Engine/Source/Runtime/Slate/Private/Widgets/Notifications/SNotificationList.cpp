...
protected:
	...
	TSharedPtr<SRichTextBlock> MySubTextBlock;
...
public:
	...
	*/
	...
	TSharedRef<SWidget> ConstructInternals( const FArguments& InArgs ) 
	{
		...
		.AutoHeight()
		[
			SAssignNew(MySubTextBlock, SRichTextBlock)
			.Text(SubText)
			.TextStyle(FAppStyle::Get(), "NotificationList.WidgetText")
...
TSharedRef<SNotificationItem> SNotificationList::AddNotification(const FNotificationInfo& Info)
{
	...
	if (FSlateNotificationManager::Get().AreNotificationsAllowed())
	{
		...
		if (Info.DefaultState != SNotificationItem::ECompletionState::CS_None)
		{
			NewItem->SetCompletionState(Info.DefaultState);
		}
	}
	else
	{
