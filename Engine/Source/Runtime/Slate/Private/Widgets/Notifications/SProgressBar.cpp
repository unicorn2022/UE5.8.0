...
SLATE_IMPLEMENT_WIDGET(SProgressBar)
void SProgressBar::PrivateRegisterAttributes(FSlateAttributeInitializer& AttributeInitializer)
{
	struct FInvalidation
	{
		static void UpdateMarquee(SWidget& OwningWidget)
		{
			static_cast<SProgressBar&>(OwningWidget).bNeedsMarqueeTimerUpdate = true;
		}
	};
	
	SLATE_ADD_MEMBER_ATTRIBUTE_DEFINITION(AttributeInitializer, Percent, EInvalidateWidgetReason::Paint)
		.OnValueChanged(FSlateAttributeDescriptor::FAttributeValueChangedDelegate::CreateStatic(FInvalidation::UpdateMarquee));
	
	SLATE_ADD_MEMBER_ATTRIBUTE_DEFINITION(AttributeInitializer, FillColorAndOpacity, EInvalidateWidgetReason::Paint);
	SLATE_ADD_MEMBER_ATTRIBUTE_DEFINITION(AttributeInitializer, BorderPadding, EInvalidateWidgetReason::Layout);
}

SProgressBar::SProgressBar()
	: Percent(*this, TOptional<float>())
	, FillColorAndOpacity(*this, FLinearColor::White)
	, BorderPadding(*this, FVector2D(0,0))
{
}
...
void SProgressBar::Construct( const FArguments& InArgs )
{
	...
	FillColorAndOpacity.Assign(*this, InArgs._FillColorAndOpacity);
	BorderPadding.Assign(*this, InArgs._BorderPadding);
...
{
	Percent.Assign(*this, MoveTemp(InPercent));
}
...
{
	FillColorAndOpacity.Assign(*this, MoveTemp(InFillColorAndOpacity));
}

void SProgressBar::SetBorderPadding(TAttribute< FVector2D > InBorderPadding)
{
	BorderPadding.Assign(*this, MoveTemp(InBorderPadding));
}
...
{
	if (bNeedsMarqueeTimerUpdate)
	{
		const_cast<SProgressBar*>(this)->UpdateMarqueeActiveTimer();
	}
...
{
	if (!Percent.Get().IsSet() || Style->EnableFillAnimation)
	{
		// If percent is not set or fill animation is enabled then set the timer
		if (!ActiveTimerHandle.IsValid())
		{
			// If we already had a handle, there is no need to register a new one.
			ActiveTimerHandle = RegisterActiveTimer(0.f, FWidgetActiveTimerDelegate::CreateSP(this, &SProgressBar::ActiveTick));
		}
	}
	...
	bNeedsMarqueeTimerUpdate = false;
}
