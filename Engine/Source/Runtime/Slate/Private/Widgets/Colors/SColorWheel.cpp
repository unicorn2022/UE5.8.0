...
void SColorWheel::Construct(const FArguments& InArgs)
{
	...
	bProcessMouseMoveImmediate = InArgs._ProcessMouseMoveImmediate;
...
FReply SColorWheel::OnMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	if (MouseEvent.GetEffectingButton() == EKeys::LeftMouseButton)
	{
		...
		bHighPrecision = MouseEvent.IsControlDown();
		...
		FReply ReturnReply = FReply::Handled().CaptureMouse(SharedThis(this)).SetUserFocus(SharedThis(this), EFocusCause::Mouse);

		if (bHighPrecision)
		{
			ReturnReply = ReturnReply.UseHighPrecisionMouseMovement(SharedThis(this));
		}
...
FReply SColorWheel::OnMouseMove(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	if (bProcessMouseMoveImmediate)
	{
		ProcessMouseAction(MyGeometry, MouseEvent, true);
	}
	else
	{
		LastGeometry = MyGeometry;
		LastMouseEvent = MouseEvent;
	}

	bHighPrecision = MouseEvent.IsControlDown();
	if (bHighPrecision && !FSlateApplicationBase::Get().GetPlatformApplication()->IsUsingHighPrecisionMouseMode())
	{
...
void SColorWheel::OnFinishedPointerInput()
{
	if (bDragging && !LastMouseEvent.GetCursorDelta().IsNearlyZero() && !bProcessMouseMoveImmediate)
	{
		ProcessMouseAction(LastGeometry, LastMouseEvent, true);
		LastGeometry = FGeometry();
		LastMouseEvent = FPointerEvent();
	}
}
