...
int32 SHeaderRow::FindColumnIndex(const FName& InColumnId) const
{
	for (int32 iColumn = 0; iColumn < Columns.Num(); iColumn++ )
	{
		const FColumn& Column = Columns[iColumn];
...
void SHeaderRow::AddColumns( TConstArrayView<FColumn::FArguments> NewColumnArgsArray )
{
	Columns.Reserve(Columns.Num() + NewColumnArgsArray.Num());

	const bool bHasFillerColumn = (Columns.Num() > 0 && Columns[Columns.Num() - 1].ColumnId == NAME_None);

	for (FColumn::FArguments const& NewColumnArgs : NewColumnArgsArray)
	{
		SHeaderRow::FColumn* NewColumn = new SHeaderRow::FColumn(NewColumnArgs);
		check(NewColumn->ColumnId != NAME_None);

		const int32 InsertIndex = bHasFillerColumn ? Columns.Num() - 1 : Columns.Num();
		Columns.Insert(NewColumn, InsertIndex);
	}
...
void SHeaderRow::RemoveColumns(const int32 StartIdx, const int32 Count)
{
	Columns.RemoveAt(StartIdx, Count);

	ColumnsChanged.Broadcast( SharedThis( this ) );
	RegenerateWidgets();
}

void SHeaderRow::SwapColumns(const int32 FirstColumnIdx, const int32 SecondColumnIdx)
{
	if (Columns.IsValidIndex(FirstColumnIdx) && Columns.IsValidIndex(SecondColumnIdx) && FirstColumnIdx != SecondColumnIdx)
	{
		Columns.Swap(FirstColumnIdx, SecondColumnIdx);
			...
			if ( SomeColumn.ShouldGenerateWidget.Get(true) && SomeColumn.bIsVisible )
			{
				...
				case EColumnSizeMode::Fixed:
				{
					...
					Splitter->AddSlot()
						...
						[
							SNew(SBox)
							.WidthOverride(SomeColumn.GetWidth() - SplitterHandleSize)
							[
								NewHeader
							]
						];
