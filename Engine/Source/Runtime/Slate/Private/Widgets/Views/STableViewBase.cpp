...
SLATE_IMPLEMENT_WIDGET(STableViewBase)
void STableViewBase::PrivateRegisterAttributes(FSlateAttributeInitializer& AttributeInitializer)
{
	SLATE_ADD_MEMBER_ATTRIBUTE_DEFINITION(AttributeInitializer, SelectionModeAttribute, EInvalidateWidgetReason::None)
		.OnValueChanged(FSlateAttributeDescriptor::FAttributeValueChangedDelegate::CreateLambda([](SWidget& Widget)
		{
			STableViewBase* TableView = static_cast<STableViewBase*>(&Widget);
			const ESelectionMode::Type OldSelectionMode = TableView->PreviousSelectionMode;
			TableView->PreviousSelectionMode = TableView->GetSelectionMode();
			TableView->OnSelectionModeChanged(OldSelectionMode, TableView->GetSelectionMode());
		}));
	SLATE_ADD_MEMBER_ATTRIBUTE_DEFINITION(AttributeInitializer, BackgroundBrushAttribute, EInvalidateWidgetReason::Layout);
}
...
TSharedRef<SWidget> STableViewBase::ConstructVerticalShadowBox(TSharedRef<SWidget> Content)
{
	return SNew(SOverlay)
		...
		[
			...
			SAssignNew(TopLeftShadowImage, SImage)
			.Visibility(ShadowBoxStyle ? EVisibility::HitTestInvisible : EVisibility::Collapsed)
			.ColorAndOpacity(this, &STableViewBase::GetStartShadowOpacity)
			.Image(ShadowBoxStyle ? &ShadowBoxStyle->TopShadowBrush : nullptr)
		]
		+ SOverlay::Slot()
		.HAlign(HAlign_Fill)
		...
		[
			...
			SAssignNew(BottomRightShadowImage, SImage)
			.Visibility(ShadowBoxStyle ? EVisibility::HitTestInvisible : EVisibility::Collapsed)
			.ColorAndOpacity(this, &STableViewBase::GetEndShadowOpacity)
			.Image(ShadowBoxStyle ? &ShadowBoxStyle->BottomShadowBrush : nullptr)
		];
}
...
TSharedRef<SWidget> STableViewBase::ConstructHorizontalShadowBox(TSharedRef<SWidget> Content)
{
	return SNew(SOverlay)
		...
		[
			...
			.Visibility(ShadowBoxStyle ? EVisibility::HitTestInvisible : EVisibility::Collapsed)
			.ColorAndOpacity(this, &STableViewBase::GetStartShadowOpacity)
			.Image(ShadowBoxStyle ? &ShadowBoxStyle->LeftShadowBrush : nullptr)
		]
		+ SOverlay::Slot()
		.HAlign(HAlign_Right)
		.VAlign(VAlign_Fill)
		[
			...
			.Visibility(ShadowBoxStyle ? EVisibility::HitTestInvisible : EVisibility::Collapsed)
			.ColorAndOpacity(this, &STableViewBase::GetEndShadowOpacity)
			.Image(ShadowBoxStyle ? &ShadowBoxStyle->RightShadowBrush : nullptr)
		];
}
...
void STableViewBase::ConstructChildren( const TAttribute<float>& InItemWidth, const TAttribute<float>& InItemHeight, const TAttribute<EListItemAlignment>& InItemAlignment, const TSharedPtr<SHeaderRow>& InHeaderRow, const TSharedPtr<SScrollBar>& InScrollBar, EOrientation InScrollOrientation, const FOnTableViewScrolled& InOnTableViewScrolled, const FScrollBarStyle* InScrollBarStyle, const bool bInPreventThrottling )
{
	...
	UE_CLOGF(InScrollOrientation != Orientation, LogSlate, Error, "STableViewBase does not support horizontal scrolling when displaying a header row");
...
int32 STableViewBase::OnPaint( const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled ) const
{
	...
	const FSlateBrush* BackgroundBrushResource = GetBackgroundBrush();
...
STableViewBase::STableViewBase( ETableViewMode::Type InTableViewMode )
	...
	, SelectionModeAttribute(*this, ESelectionMode::Multi)
	, PreviousSelectionMode(ESelectionMode::Multi)
	, BackgroundBrushAttribute(*this, FStyleDefaults::GetNoBrush())
{
...
{
	const int32 NumItemsBeingObserved = GetNumItemsBeingObserved();
	const float FractionalScrollOffsetInItems = NumItemsBeingObserved == 0 ? 0.f : (DesiredScrollOffset + GetScrollRateInItems() * ScrollByAmountInSlateUnits) / NumItemsBeingObserved;
...
void STableViewBase::SetBackgroundBrush(TAttribute<const FSlateBrush*> InBackgroundBrush)
{
	BackgroundBrushAttribute.Assign(*this, TAttribute<FSlateBrushCached>::CreateLambda(
		[BackgroundBrush = MoveTemp(InBackgroundBrush)]()
		{
			return FSlateBrushCached(BackgroundBrush.Get(nullptr));
		}));
}
...
void STableViewBase::SetShadowBoxStyle(const FScrollBoxStyle* InShadowBoxStyle)
{
	if (ShadowBoxStyle != InShadowBoxStyle)
	{
		ShadowBoxStyle = InShadowBoxStyle;
		if (TopLeftShadowImage)
		{
			const FSlateBrush* Image = nullptr;
			if (ShadowBoxStyle)
			{
				Image = Orientation == Orient_Vertical ? &ShadowBoxStyle->TopShadowBrush : &ShadowBoxStyle->LeftShadowBrush;
			}
			TopLeftShadowImage->SetImage(Image);
			TopLeftShadowImage->SetVisibility(ShadowBoxStyle ? EVisibility::HitTestInvisible : EVisibility::Collapsed);
		}
		if (BottomRightShadowImage)
		{
			const FSlateBrush* Image = nullptr;
			if (ShadowBoxStyle)
			{
				Image = Orientation == Orient_Vertical ? &ShadowBoxStyle->BottomShadowBrush : &ShadowBoxStyle->RightShadowBrush;
			}
			BottomRightShadowImage->SetImage(Image);
			BottomRightShadowImage->SetVisibility(ShadowBoxStyle ? EVisibility::HitTestInvisible : EVisibility::Collapsed);
		}
	}
}
