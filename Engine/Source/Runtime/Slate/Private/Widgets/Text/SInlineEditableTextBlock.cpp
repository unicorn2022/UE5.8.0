...
{
	SLATE_ADD_MEMBER_ATTRIBUTE_DEFINITION(AttributeInitializer, MaximumLengthAttribute, [](const SWidget& OwningWidget)
	{
		const SInlineEditableTextBlock& InlineTextBlock = static_cast<const SInlineEditableTextBlock&>(OwningWidget);
		if (InlineTextBlock.IsInEditMode())
		{
			const bool bIsProcessingAttributeUpdate = InlineTextBlock.GetProxyHandle().IsValid(InlineTextBlock) &&
				InlineTextBlock.GetProxyHandle().GetInvalidationRootHandle().GetInvalidationRoot() &&
				InlineTextBlock.GetProxyHandle().GetInvalidationRootHandle().GetInvalidationRoot()->IsProcessingAttributeUpdate();

			if (bIsProcessingAttributeUpdate)
			{
				// We want to retrigger an OnTextChanged event with the text of the edited textbox to revalidate the text.
				// It cannot be retriggered right away while attributes are updated if OnTextChanged ends up creating or destroying the error
				// message as this would lead to a crash. We also cannot register a timer at this point, so we delay until next tick.
				bool bHadError = InlineTextBlock.HasTextBoxError();
				FText ErrorMessage;
				bool bHasError = !InlineTextBlock.IsTextAllowed(InlineTextBlock.GetEditableText(), ErrorMessage);

				if (bHadError != bHasError)
				{
					// a popup widget would end up being created or destroyed, so we need to delay its creation/destruction
					InlineTextBlock.bNeedsMaximumLengthChecked = true;
					return EInvalidateWidgetReason::Paint;
				}
			}
			
			const_cast<SInlineEditableTextBlock&>(InlineTextBlock).OnTextChanged(InlineTextBlock.GetEditableText());
		}
		return EInvalidateWidgetReason::None;
	});
}

SInlineEditableTextBlock::SInlineEditableTextBlock()
	: MaximumLengthAttribute(*this)
{
}
...
void SInlineEditableTextBlock::Construct( const FArguments& InArgs )
{
	...
	SetMaximumLength(InArgs._MaximumLength);
}

int32 SInlineEditableTextBlock::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect,	FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	if (bNeedsMaximumLengthChecked)
	{
		const_cast<SInlineEditableTextBlock*>(this)->OnTextChanged(GetEditableText());
	}
...
void SInlineEditableTextBlock::EnterEditingMode()
{
	if(!bIsReadOnly.Get() && FSlateApplication::Get().HasAnyMouseCaptor() == false)
	{
		if(TextBlock->GetVisibility() == EVisibility::Visible)
		{
			...
			// We retrigger an OnTextChanged event to revalidate the text
...
void SInlineEditableTextBlock::SetMaximumLength(TAttribute<int32> InMaximumLength)
{
	MaximumLengthAttribute.Assign(*this, MoveTemp(InMaximumLength));
}
...
{
	if (!bDelayedLeftClickEntersEditMode || DoubleSelectDelay == 0.0f || !MouseEvent.IsMouseButtonDown(EKeys::LeftMouseButton) || MouseEvent.IsControlDown() || MouseEvent.IsShiftDown())
	{
	...
	if (IsSelected.IsBound())
	{
		...
		{
			ActiveTimerHandle = RegisterActiveTimer(DoubleSelectDelay, FWidgetActiveTimerDelegate::CreateSP(this, &SInlineEditableTextBlock::TriggerEditMode));
		}
	}
	else
	{
...
{
	if (DoubleSelectDelay == 0.0f)
	{
		if (IsSelected.IsBound())
		{
			if (IsSelected.Execute() && !bIsReadOnly.Get() && !FSlateApplication::Get().IsDragDropping())
			{
				EnterEditingMode();
			}
		}
		else if (HasKeyboardFocus() && !bIsReadOnly.Get())
		{
			EnterEditingMode();
		}
...
void SInlineEditableTextBlock::OnTextChanged(const FText& InText)
{
	if(IsInEditMode())
	{
		...
		if(!IsTextAllowed(InText, OutErrorMessage))
		{
...
bool SInlineEditableTextBlock::IsTextAllowed(const FText& InText, FText& OutErrorMessage) const
{
	return FTextEditHelper::VerifyTextLength(InText, OutErrorMessage, MaximumLengthAttribute.Get()) &&
...
FText SInlineEditableTextBlock::GetEditableText() const
{
	return 
#if WITH_FANCY_TEXT
	bIsMultiLine ? MultiLineTextBox->GetText() :
...
bool SInlineEditableTextBlock::HasTextBoxError() const
{
	return 
#if WITH_FANCY_TEXT
	bIsMultiLine ? MultiLineTextBox->HasError() :
#endif //WITH_FANCY_TEXT
				 TextBox->HasError();
}
