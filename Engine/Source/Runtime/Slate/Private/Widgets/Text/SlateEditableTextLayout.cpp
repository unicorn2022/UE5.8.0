 ...
 * i.e. never forget to call EndEditTransaction();
 */
...
public:
	...
	{
		EditableTextLayout->BeginEditTransaction();
	}
...
void FSlateEditableTextLayout::Construct(TAttribute<FText> InInitialText, FTextBlockStyle InTextStyle, const TOptional<ETextShapingMethod>& InTextShapingMethod, const TOptional<ETextFlowDirection>& InTextFlowDirection, const FCreateSlateTextLayout& InCreateSlateTextLayout, TSharedRef<ITextLayoutMarshaller> InTextMarshaller, TSharedRef<ITextLayoutMarshaller> InHintTextMarshaller)
{
	...
	const TSharedRef<SWidget> OwnerSlateWidget = OwnerWidget->GetSlateWidget();

	Marshaller = MoveTemp(InTextMarshaller);
	HintMarshaller = MoveTemp(InHintTextMarshaller);
	TextStyle = MoveTemp(InTextStyle);
	HintTextStyle = TextStyle;
	TextLayout = CreateSlateTextLayout.Execute(&OwnerSlateWidget.Get(), TextStyle);
	...
	BoundText.Assign(OwnerSlateWidget.Get(), MoveTemp(InInitialText), ESlateAttributeBindAction::Initialize);
	PreviousText = BoundText.Get();
	...
	// Set the initial text - the same as SetText, but doesn't call OnTextChanged as that can cause a crash during construction
	{
		...
		const FText& InitialTextToSet = BoundText.Get();
...
void FSlateEditableTextLayout::SetText(TAttribute<FText> InText)
{
	bForceBoundTextReview = true;
	BoundText.Assign(OwnerWidget->GetSlateWidget().Get(), MoveTemp(InText), FText::GetEmpty(), ESlateAttributeBindAction::Initialize);

	if (bForceBoundTextReview)
	{
		// We would only arrive here if OnBoundTextChanged did not trigger (because the BoundText technically did not change),
		// but we still want to make sure the EditableText is aligning so we call it manually
		OnBoundTextChanged();
	}
}

int32 FSlateEditableTextLayout::GetTextLineCount() const
{
...
void FSlateEditableTextLayout::SetHintText(TAttribute<FText> InHintText)
{
	const TSharedRef<SWidget> OwnerSlateWidget = OwnerWidget->GetSlateWidget();

	HintText.Assign(OwnerSlateWidget.Get(), MoveTemp(InHintText), FText::GetEmpty(), ESlateAttributeBindAction::Initialize);

	// If we have hint text that is either non-empty or bound to a delegate, we'll also need to make the hint text layout
	if (HintText.IsBound(OwnerSlateWidget.Get()) || !HintText.Get().IsEmpty())
	{
		HintTextLayout = MakeUnique<FSlateTextBlockLayout>(&OwnerSlateWidget.Get(), HintTextStyle, TextLayout->GetTextShapingMethod(), TextLayout->GetTextFlowDirection(), CreateSlateTextLayout, HintMarshaller.ToSharedRef(), nullptr);
...
void FSlateEditableTextLayout::SetSearchText(TAttribute<FText> InSearchText)
{
	BoundSearchText.Assign(OwnerWidget->GetSlateWidget().Get(), MoveTemp(InSearchText));
}
...
void FSlateEditableTextLayout::SetTextStyle(const FTextBlockStyle& InTextStyle)
{
	...
	// Using Layout as changing text block size can affect the size.
...
void FSlateEditableTextLayout::SetHintTextOpacity(TAttribute<float> InOpacity)
{
	HintTextOpacity.Assign(OwnerWidget->GetSlateWidget().Get(), MoveTemp(InOpacity));
}

float FSlateEditableTextLayout::GetHintTextOpacity() const
{
	return HintTextOpacity.Get();
}

void FSlateEditableTextLayout::SetIsReadOnly(TAttribute<bool> InIsReadOnly)
{
	bIsReadOnlyAttribute.Assign(OwnerWidget->GetSlateWidget().Get(), MoveTemp(InIsReadOnly));
}

bool FSlateEditableTextLayout::GetIsReadOnly() const
{
	return bIsReadOnlyAttribute.Get();
}
...
{
	const ETextShapingMethod NewShapingMethod = InTextShapingMethod.Get(GetDefaultTextShapingMethod());
	if (NewShapingMethod != TextLayout->GetTextShapingMethod())
	{
		TextLayout->SetTextShapingMethod(NewShapingMethod);
...
{
	const ETextFlowDirection NewFlowDirection = InTextFlowDirection.Get(GetDefaultTextFlowDirection());
	if (NewFlowDirection != TextLayout->GetTextFlowDirection())
	{
		TextLayout->SetTextFlowDirection(NewFlowDirection);

		OwnerWidget->GetSlateWidget()->Invalidate(EInvalidateWidget::Layout);
	}
}

void FSlateEditableTextLayout::SetTextWrapping(TAttribute<float> InWrapTextAt, TAttribute<bool> InAutoWrapText, TAttribute<ETextWrappingPolicy> InWrappingPolicy)
{
	const TSharedRef<SWidget> OwnerSlateWidget = OwnerWidget->GetSlateWidget();

	WrapTextAt.Assign(OwnerSlateWidget.Get(), MoveTemp(InWrapTextAt));
	AutoWrapText.Assign(OwnerSlateWidget.Get(), MoveTemp(InAutoWrapText));
	WrappingPolicy.Assign(OwnerSlateWidget.Get(), MoveTemp(InWrappingPolicy));
}

void FSlateEditableTextLayout::SetWrapTextAt(TAttribute<float> InWrapTextAt)
{
	WrapTextAt.Assign(OwnerWidget->GetSlateWidget().Get(), MoveTemp(InWrapTextAt));
}

void FSlateEditableTextLayout::SetAutoWrapText(TAttribute<bool> InAutoWrapText)
{
	AutoWrapText.Assign(OwnerWidget->GetSlateWidget().Get(), MoveTemp(InAutoWrapText));
}

void FSlateEditableTextLayout::SetWrappingPolicy(TAttribute<ETextWrappingPolicy> InWrappingPolicy)
{
	WrappingPolicy.Assign(OwnerWidget->GetSlateWidget().Get(), MoveTemp(InWrappingPolicy));
}

void FSlateEditableTextLayout::SetMargin(TAttribute<FMargin> InMargin)
{
	Margin.Assign(OwnerWidget->GetSlateWidget().Get(), MoveTemp(InMargin));
}

void FSlateEditableTextLayout::SetJustification(TAttribute<ETextJustify::Type> InJustification)
{
	Justification.Assign(OwnerWidget->GetSlateWidget().Get(), MoveTemp(InJustification));
}

ETextJustify::Type FSlateEditableTextLayout::GetJustification() const
{
	return Justification.Get();
}

void FSlateEditableTextLayout::SetLineHeightPercentage(TAttribute<float> InLineHeightPercentage)
{
	LineHeightPercentage.Assign(OwnerWidget->GetSlateWidget().Get(), MoveTemp(InLineHeightPercentage));
}

void FSlateEditableTextLayout::SetApplyLineHeightToBottomLine(TAttribute<bool> InApplyLineHeightToBottomLine)
{
	ApplyLineHeightToBottomLine.Assign(OwnerWidget->GetSlateWidget().Get(), MoveTemp(InApplyLineHeightToBottomLine));
}

void FSlateEditableTextLayout::SetOverflowPolicy(const TOptional<ETextOverflowPolicy>& InOverflowPolicy)
{
...
{
	const FText& TextToSet = BoundText.Get();
...
bool FSlateEditableTextLayout::RefreshImpl(const FText* InTextToSet, const bool bForce)
{
	...
	if (InTextToSet && (bForce || !BoundTextLastTick.IdenticalTo(*InTextToSet)))
	{
		...
		if (bForce || !BoundTextLastTick.IsDisplayStringEqualTo(*InTextToSet))
		{
			...
			bHasSetText = SetEditableText(*InTextToSet, bForce);
		}
...
bool FSlateEditableTextLayout::HandleFocusReceived(const FFocusEvent& InFocusEvent)
{
	...
	OwnerWidget->OnBeginTextEdit(BoundText.Get());
	return true;
}
...
void FSlateEditableTextLayout::BeginEditTransaction()
{
...
void FSlateEditableTextLayout::EndEditTransaction()
{
	...
	if (bHasTextChanged)
	{
		...
		// Let outsiders know that the text content has been changed
		BroadcastOnTextChanged(EditedText);
		...
		{
			{
				...
				{
					// Let outsiders know that the text content has been changed
					BroadcastOnTextChanged(UndoState.Text);
				}
...
void FSlateEditableTextLayout::SaveText(const FText& TextToSave)
{
	...
	if (!BoundText.IsBound(OwnerSlateWidget.Get()))
	{
		BoundText.Assign(OwnerSlateWidget.Get(), TextToSave);
	}
}
...
void FSlateEditableTextLayout::LoadText()
{
	...
	{
		bForceBoundTextReview = true;
		BoundText.UpdateNow(OwnerSlateWidget.Get());
		if (bForceBoundTextReview)
		{
			// We would only arrive here if OnBoundTextChanged did not trigger (because the BoundText technically did not change),
			// but we still want to make sure the EditableText is aligning so we call it manually
...
{
	return OwnerWidget->GetSlateWidget()->HasAnyUserFocus().IsSet() && CursorLineHighlighter->GetDrawCursor();
}
...
void FSlateEditableTextLayout::UpdateTextChangedByVirtualKeyboard()
{
	if (bTextChangedByVirtualKeyboard)
	{
		...
		TextLayout->UpdateIfNeeded();
		// Let outsiders know that the text content has been changed
		BroadcastOnTextChanged(GetEditableText());
		bTextChangedByVirtualKeyboard = false;
	}
}
...
void FSlateEditableTextLayout::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	...
	const bool bTextChangedByVKThisTick = bTextChangedByVirtualKeyboard;
	...
	// Prevent changes to selection when text did not change this tick as this cause the caret to disappear and only appear when new text is entered due to the SelectionChanged being received before the new text.
	if (bSelectionChangedExternally && bTextChangedByVKThisTick)
	{
		bSelectionChangedExternally = false;
	...
	{
		const FText& SearchTextToSet = BoundSearchText.Get();
	...
	const EVisibility ScrollBarVisibilityOverride = ((AutoWrapText.Get() && CachedSize.IsZero()) || !OwnerWidget->IsMultiLineTextEdit()) ? EVisibility::Collapsed : EVisibility::Visible;

	// Try and make sure that the line containing the cursor is in view
	// Prevent scrolling if Virtual Keyboard changed text this tick to prevent using stale AllottedGeometry. Scroll will be applied properly on the next tick.
	if (PositionToScrollIntoView.IsSet() && !bTextChangedByVKThisTick)
	{
...

	{
		...
		ScrollOffset.X = OwnerWidget->UpdateAndClampHorizontalScrollBar(ViewOffset, ViewFraction, ScrollBarVisibilityOverride);
	}

	{
		...
		ScrollOffset.Y = OwnerWidget->UpdateAndClampVerticalScrollBar(ViewOffset, ViewFraction, ScrollBarVisibilityOverride);
	}

	TextLayout->SetVisibleRegion(AllottedGeometry.Size, FVector2D(ScrollOffset) * TextLayout->GetScale());
}

int32 FSlateEditableTextLayout::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	// Update the auto-wrap size now that we have computed paint geometry; won't take effect until text frame
	// Note: This is done here rather than in Tick(), because Tick() doesn't get called while resizing windows, but OnPaint() does
	const FVector2f PreviousCachedSize = CachedSize;
	CachedSize = FVector2f(AllottedGeometry.GetLocalSize());

	// If CacheSize is set for the first time, we relayout to avoid a one frame jump for non left justifications.
	if (PreviousCachedSize == FVector2f::Zero())
	{
	...
	if (TextLayout->IsEmpty() && HintTextLayout.IsValid())
	{
		...
		FLinearColor ThisColorAndOpacity = TextStyle.ColorAndOpacity.GetColor(InWidgetStyle);
		ThisColorAndOpacity.A = GetHintTextOpacity();
		HintTextStyle.ColorAndOpacity = ThisColorAndOpacity;
...
void FSlateEditableTextLayout::SetCachedSize(const FGeometry& AllottedGeometry)
{
	CachedSize = FVector2f(AllottedGeometry.GetLocalSize());
}
...
FVector2D FSlateEditableTextLayout::ComputeDesiredSize(float LayoutScaleMultiplier) const
{
	...
	if (TextLayout->IsEmpty() && HintTextLayout.IsValid())
	{
		...
		const FVector2D HintTextSize = HintTextLayout->ComputeDesiredSize(
			FSlateTextBlockLayout::FWidgetDesiredSizeArgs(HintText.Get(), FText::GetEmpty(), WrappingWidth, AutoWrapText.Get(), WrappingPolicy.Get(), ETextTransformPolicy::None, MarginValue, LineHeightPercentage.Get(), ApplyLineHeightToBottomLine.Get(), Justification.Get()),
...
void FSlateEditableTextLayout::FVirtualKeyboardEntry::SetTextFromVirtualKeyboard(const FText& InNewText, ETextEntryType TextEntryType)
{
	...
	const TSharedRef<SWidget> OwnerSlateWidget = OwnerLayout->OwnerWidget->GetSlateWidget();
	...
	if (!OwnerLayout->BoundText.IsBound(OwnerSlateWidget.Get()))
	{
		OwnerLayout->BoundText.Assign(OwnerSlateWidget.Get(), InNewText);
	}
...
void FSlateEditableTextLayout::FTextInputMethodContext::SetTextInRange(const uint32 BeginIndex, const uint32 Length, const FString& InString)
{
	...
	if (HasTextChanged)
	{
		...
		OwnerLayout->BroadcastOnTextChanged(EditedText);
	}
}
...
void FSlateEditableTextLayout::FTextInputMethodContext::BeginComposition()
{
	...
	if (!bIsComposing)
	{
		...
		OwnerLayout->BeginEditTransaction();
		OwnerLayout->UpdateCursorHighlight();
	}
}
...
void FSlateEditableTextLayout::SetOnAllFontFacesFinishLoadingDelegate(const FSimpleDelegate& InAllFontFacesFinishLoadingDelegate)
{
	if (TextLayout)
	{
		TextLayout->SetOnAllFontFacesFinishLoading(InAllFontFacesFinishLoadingDelegate);
	}
}

void FSlateEditableTextLayout::SetFontFacesLoadingPaintPolicy(EFontFacesLoadingPaintPolicy InFontFacesLoadingPaintPolicy)
{
	if (TextLayout)
	{
		TextLayout->SetFontFacesLoadingPaintPolicy(InFontFacesLoadingPaintPolicy);
	}
}

void FSlateEditableTextLayout::BroadcastOnTextChanged(const FText& InText)
{
	// We update the PreviousText so future text change events will trigger as expected.
	// This is needed because this OnTextChanged event can be broadcasted from a few places
	PreviousText = InText;
	OwnerWidget->OnTextChanged(InText);
}

void FSlateEditableTextLayout::OnBoundTextChanged()
{
	const TSharedRef<SWidget> Widget = OwnerWidget->GetSlateWidget();

	// During construction, we do not want to trigger events.
	// If we have focus, the bound text might have changed but the EditableText prevails and we don't want to replace it
	if (!OwnerWidget->IsWidgetConstructed() || (Widget->HasAnyUserFocus().IsSet() && !bForceBoundTextReview))
	{
		bForceBoundTextReview = false;
		return;
	}

	const FText NewBoundText = BoundText.Get();
	...
	const bool bForceRefresh = !EditableText.ToString().Equals(NewBoundText.ToString(), ESearchCase::CaseSensitive);

	// Only emit the "text changed" event if the text has actually been changed
	const bool bHasTextChanged = Widget->HasAnyUserFocus().IsSet()
		? !NewBoundText.ToString().Equals(EditableText.ToString(), ESearchCase::CaseSensitive)
		: !NewBoundText.ToString().Equals(PreviousText.ToString(), ESearchCase::CaseSensitive);

	if (RefreshImpl(&NewBoundText, bForceRefresh))
	{
		// Make sure we move the cursor to the end of the new text if we had keyboard focus, but only if we set it via SetText or LoadText
		...
		if (bHasTextChanged)
		{
			BroadcastOnTextChanged(NewBoundText);
		}
	}
...
void FSlateEditableTextLayout::OnBoundSearchTextChanged(SWidget& Widget, FSlateEditableTextLayout& SlateTextLayout)
{
	const FText& SearchTextToSet = SlateTextLayout.BoundSearchText.Get();
	SlateTextLayout.BoundSearchTextLastTick = FTextSnapshot(SearchTextToSet);
	SlateTextLayout.BeginSearch(SearchTextToSet);
}

void FSlateEditableTextLayout::OnIsReadOnlyChanged(SWidget& Widget, FSlateEditableTextLayout& SlateTextLayout)
{
	SlateTextLayout.CursorLineHighlighter->SetDrawCursor(!SlateTextLayout.bIsReadOnlyAttribute.Get());
}
