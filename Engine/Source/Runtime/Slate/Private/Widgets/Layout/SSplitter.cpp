...
SLATE_IMPLEMENT_WIDGET(SSplitter)
void SSplitter::PrivateRegisterAttributes(FSlateAttributeInitializer& AttributeInitializer)
{
...
SSplitter::FSlot::FSlot()
 ...
 , SizingRule(*this, FractionOfParent)
 , SizeValue(*this, 1.f)
 , MinSizeValue(*this, 0.f)
{
}
...
void SSplitter::FSlot::Construct(const FChildren& SlotOwner, FSlotArguments&& InArgs)
{
	...
	SetSizingRule( MoveTemp(InArgs._SizeRule) );
	SetSizeValue( MoveTemp(InArgs._Value) );
...
void SSplitter::FSlot::RegisterAttributes(FSlateWidgetSlotAttributeInitializer& AttributeInitializer)
{
	TWidgetSlotWithAttributeSupport<FSlot>::RegisterAttributes(AttributeInitializer);

	SLATE_ADD_SLOT_ATTRIBUTE_DEFINITION(FSlot, AttributeInitializer, SizingRule, EInvalidateWidgetReason::Layout);
	SLATE_ADD_SLOT_ATTRIBUTE_DEFINITION(FSlot, AttributeInitializer, SizeValue, EInvalidateWidgetReason::Layout);
	SLATE_ADD_SLOT_ATTRIBUTE_DEFINITION(FSlot, AttributeInitializer, MinSizeValue, EInvalidateWidgetReason::Layout);
}

void SSplitter::FSlot::SetSizeValue( TAttribute<float> InValue )
{
	SizeValue.Assign(*this, MoveTemp(InValue));
}

void SSplitter::FSlot::SetSizingRule( TAttribute<ESizeRule> InSizeRule ) 
{
	SizingRule.Assign(*this, MoveTemp(InSizeRule));
}

void SSplitter::FSlot::SetMinSize(TAttribute<float> InMinSize)
{
	MinSizeValue.Assign(*this, MoveTemp(InMinSize));
}
