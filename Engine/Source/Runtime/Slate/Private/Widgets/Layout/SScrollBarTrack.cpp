...
void SScrollBarTrack::Construct(const FArguments& InArgs)
{
	...
	Children.AddSlot(MoveTemp(FSlot::FSlotArguments(MakeUnique<FSlot>())
	[
		SNew(SOverlay) // We need a parent widget to control the visibility
		+ SOverlay::Slot()
		[
	...
	// Ensure the visibilities are set as expected
	InvalidateIsNeeded(!IsNeeded());
}
...
void SScrollBarTrack::SetSizes(float InThumbOffsetFraction, float InThumbSizeFraction)
{
	...
	if (InThumbSizeFraction == 0.0f && !bIsAlwaysVisible)
	{
		InThumbSizeFraction = 1.0f;
	}
	else if (InThumbSizeFraction > 1.0f && bIsAlwaysVisible)
	{
		InThumbSizeFraction = 0.0f;
	}

	if (OffsetFraction != InThumbOffsetFraction || ThumbSizeFraction != InThumbSizeFraction)
	{
		const bool bWasNeeded = IsNeeded();
		OffsetFraction = InThumbOffsetFraction;
		ThumbSizeFraction = InThumbSizeFraction;

		InvalidateIsNeeded(bWasNeeded);
	}
}
...
void SScrollBarTrack::SetIsAlwaysVisible(bool InIsAlwaysVisible)
{
	if (bIsAlwaysVisible != InIsAlwaysVisible)
	{
		...
		// SetSizes is called because it can adjust the ThumbSizeFraction value
		SetSizes(OffsetFraction, ThumbSizeFraction);
		
		InvalidateIsNeeded(bWasNeeded);
	}
}

void SScrollBarTrack::InvalidateIsNeeded(bool bWasNeeded)
{
	Invalidate(EInvalidateWidget::Layout);
	if (IsNeeded() != bWasNeeded)
	{
		const EVisibility NewVisibility = IsNeeded() ? EVisibility::SelfHitTestInvisible : EVisibility::Hidden;
		Children[BOTTOM_SLOT_INDEX].GetWidget()->SetVisibility(NewVisibility);
		Children[THUMB_SLOT_INDEX].GetWidget()->SetVisibility(NewVisibility);
	}
}
