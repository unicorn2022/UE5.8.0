...
namespace ScrollBoxFocus
{
static void EmplaceArrangedWidget(const FGeometry& MyGeometry, TSharedRef<SWidget> InParentWidget, TSharedRef<SWidget> InChildWidget, TMap<TSharedRef<SWidget>, FArrangedWidget>& OutWidgetsToArranged)
{
	FArrangedChildren ArrangedChildren(EVisibility::Visible);
	InParentWidget->ArrangeChildren(MyGeometry, ArrangedChildren);
	if (FArrangedWidget* FoundSelfArranged = ArrangedChildren.GetInternalArray().FindByPredicate([InChildWidget](const FArrangedWidget& WidgetIt) { return InChildWidget == WidgetIt.Widget; }))
	{
		OutWidgetsToArranged.Emplace(FoundSelfArranged->Widget, *FoundSelfArranged);
	}
}

static void AppendFocusableChildren(
	const FGeometry& MyGeometry,
	TSharedRef<SWidget> InParentWidget,
	TSharedRef<SWidget> InRuleWidget,
	const uint32 InUserIndex,
	const bool bIsRootCall,
	TSharedPtr<SWidget>& OutUserFocusedWidget,
	TMap<TSharedRef<SWidget>, FArrangedWidget>& OutWidgetsToArranged,
	FHittestGrid& OutHitTestGrid)
{
	FChildren* InWidgetChildren = InParentWidget->GetChildren();
	if (InWidgetChildren)
	{
		for (int32 SlotIndex = 0; SlotIndex < InWidgetChildren->Num(); ++SlotIndex)
		{
			TSharedRef<SWidget> Child = InWidgetChildren->GetChildAt(SlotIndex);

			if (Child->GetVisibility().IsVisible())
			{
				const bool bIsRuleWidget = Child == InRuleWidget;
				const bool bSupportsKeyboardFocus = Child->SupportsKeyboardFocus();

				if (bIsRuleWidget)
				{
					EmplaceArrangedWidget(MyGeometry, InParentWidget, Child, OutWidgetsToArranged);
				}
				else if (bSupportsKeyboardFocus)
				{
					OutHitTestGrid.AddWidget(&Child.Get(), 0, 0, FSlateInvalidationWidgetSortOrder());
					const bool bHasUserFocus = Child->HasUserFocus(InUserIndex).IsSet() || Child->HasUserFocusedDescendants(InUserIndex);
					if (bHasUserFocus)
					{
						ensureMsgf(!OutUserFocusedWidget.IsValid(), TEXT("This should only be set once"));
						OutUserFocusedWidget = Child.ToSharedPtr();

						EmplaceArrangedWidget(MyGeometry, InParentWidget, Child, OutWidgetsToArranged);
					}
				}

				// We only want to add Focusable Children that are children of the Rule Widget, i.e. the ScrollBox.
				// We do need to start one widget higher in the hiearchy to arrange the ScrollBox/Rule widget itself
				// If we added this child widget as keyboard focus already, we do NOT need to search it's children
				const bool bIsChildThatDoesNotSupportKeyBoardFocus = !bIsRootCall && !bSupportsKeyboardFocus;
				if (bIsRuleWidget || bIsChildThatDoesNotSupportKeyBoardFocus)
				{
					AppendFocusableChildren(MyGeometry, Child, InRuleWidget, InUserIndex, false, OutUserFocusedWidget, OutWidgetsToArranged, OutHitTestGrid);
				}
			}
		}
	}
...
static TSharedPtr<SWidget> GetNextWidget(
	const FGeometry& MyGeometry,
	const UE::Slate::FDeprecateVector2DResult& InHitTestGridSize,
	TSharedRef<SWidget> InRuleWidget,
	const uint32 InUserIndex,
	const EUINavigation InNavigation,
	FNavigationMethod& InNavigationMethod)
{
	TSharedPtr<SWidget> ReturnWidget;

	TSharedPtr<SWidget> ParentWidget = InRuleWidget->GetParentWidget();
	if (ParentWidget.IsValid())
	{
		TSharedPtr<SWidget> StartingWidget; // This is the current user focused widget
		TMap<TSharedRef<SWidget>, FArrangedWidget> WidgetsToArranged;
		FHittestGrid HitTestGrid;
		{
			UE::Slate::FDeprecateVector2DResult RectTopLeft = InRuleWidget->GetTickSpaceGeometry().GetRenderBoundingRect().GetTopLeft();		
			HitTestGrid.SetHittestArea(RectTopLeft, InHitTestGridSize);
		}

		constexpr bool bIsRootCall = true;
		AppendFocusableChildren(
			MyGeometry,
		...
		if (StartingWidget.IsValid())
		{
			const TSharedPtr<FNavigationMetaData> FocusWidgetNavMetaData = StartingWidget->GetMetaData<FNavigationMetaData>();
			const TSharedPtr<SWidget> ExplicitNextWidget = FocusWidgetNavMetaData.IsValid() ? FocusWidgetNavMetaData->GetFocusRecipient(InNavigation).Pin() : nullptr;
			if (ExplicitNextWidget.IsValid())
			{
				return ExplicitNextWidget;
			}
		}

		const bool bRuleWidgetArranged = WidgetsToArranged.Contains(InRuleWidget);
		const bool bUserFocusedWidgetArranged = StartingWidget.IsValid() && WidgetsToArranged.Contains(StartingWidget.ToSharedRef());
		const bool bFoundArrangedWidgets = bRuleWidgetArranged && bUserFocusedWidgetArranged;
		if (bFoundArrangedWidgets)
		{
			InNavigationMethod.Initialize(&HitTestGrid, nullptr);
			
			const FNavigationReply FocusedNavigationReply = StartingWidget->OnNavigation(MyGeometry, InNavigation);
			const FArrangedWidget& ArrangedStartingWidget = WidgetsToArranged[StartingWidget.ToSharedRef()];
			const FArrangedWidget& ArrangedRuleWidget = WidgetsToArranged[InRuleWidget];

			ReturnWidget = InNavigationMethod.FindNextFocusableWidget(ArrangedStartingWidget, InNavigation, FocusedNavigationReply, ArrangedRuleWidget, InUserIndex);
		}
	}
...
SLATE_IMPLEMENT_WIDGET(SScrollPanel)
void SScrollPanel::PrivateRegisterAttributes(FSlateAttributeInitializer& AttributeInitializer)
{
	FSlateWidgetSlotAttributeInitializer Initializer = SLATE_ADD_PANELCHILDREN_DEFINITION(AttributeInitializer, Children);
	SScrollBox::FSlot::RegisterAttributes(Initializer);
}

void SScrollPanel::Construct(const FArguments& InArgs, TArray<SScrollBox::FSlot::FSlotArguments> InSlots, TSharedRef<SScrollBox> InScrollBox)
{
	OwningScrollBox = InScrollBox;
	PhysicalOffset = 0;
...
int32 SScrollPanel::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	const bool bHasNavigationMethod = OwningScrollBox.IsValid() && OwningScrollBox.Pin()->HasNavigationMetaData();
	if (bHasNavigationMethod)
	{
		// Use the content size of the scrollbox to navigate to offscreen rendered widgets
		const FVector2D ScrollPanelSize = FVector2D::Max(AllottedGeometry.Size, GetArrangedSize());
		const FGeometry ScrollBoxGeometry = AllottedGeometry.MakeChild(ScrollPanelSize, FSlateLayoutTransform());
		const FSlateRect BoundingRect = OwningScrollBox.IsValid() ? OwningScrollBox.Pin()->GetHitTestBoundingRect() : GetHitTestBoundingRect();

		return SPanel::OnPaint(Args, ScrollBoxGeometry, BoundingRect, OutDrawElements, LayerId, InWidgetStyle, bParentEnabled);
	}
	else
	{
...
void SScrollBox::Construct( const FArguments& InArgs )
{
	...
	bAllowsRightClickDragScrolling = InArgs._ScrollBarRightClickDragAllowed;
	...
	OnScrollBoxFocusChanging = InArgs._OnFocusChanging;
	...
	SAssignNew(ScrollPanel, SScrollPanel, MoveTemp(const_cast<TArray<FSlot::FSlotArguments>&>(InArgs._Slots)), StaticCastSharedRef<SScrollBox>(AsShared()))
	...
	[
		...
		[
			...
			[
				...
				SAssignNew(TopLeftShadowHint, SImage)
			...
			[
				...
				SAssignNew(BottomRightShadowHint, SImage)
				.Visibility(EVisibility::HitTestInvisible)
...
{
	TWeakPtr<SWidget> WidgetWeak = WidgetToScrollIntoView;

	ScrollIntoViewRequest = [this, WidgetWeak, InAnimateScroll, InDestination, InScrollPadding](FGeometry AllottedGeometry)
	{
		if (WidgetWeak.IsValid())
		{
			InternalScrollDescendantIntoView(AllottedGeometry, WidgetWeak.Pin(), InAnimateScroll, InDestination, InScrollPadding);
		}
	};
...
bool SScrollBox::InternalScrollDescendantIntoView(const FGeometry& MyGeometry, const TSharedPtr<SWidget>& WidgetToFind, bool InAnimateScroll, EDescendantScrollDestination InDestination, float InScrollPadding)
{
	...
	if (WidgetToFind.IsValid())
	{
		...
		if (!WidgetGeometry)
		{
			UE_LOGF(LogSlate, Warning, "Unable to scroll to descendant as it's not a child of the scrollbox");
		}
		else
		{
...
void SScrollBox::SetStyle(const FScrollBoxStyle* InStyle)
{
	...
	{
		Style = InStyle;
		TopLeftShadowHint->SetImage(Orientation == Orient_Vertical ? &Style->TopShadowBrush : &Style->LeftShadowBrush);
		BottomRightShadowHint->SetImage(Orientation == Orient_Vertical ? &Style->BottomShadowBrush : &Style->RightShadowBrush);
		InvalidateStyle();
	}
}
...
{
	TSharedPtr<FNavigationMetaData> NavMetaData = GetMetaData<FNavigationMetaData>();
	if (NavMetaData.IsValid())
	{
		TInstancedStruct<FNavigationMethod>& NavigationMethodInstance = NavMetaData->GetMutableNavigationMethodStruct();
		FNavigationMethod* NavigationMethod = NavigationMethodInstance.IsValid() ? NavigationMethodInstance.GetMutablePtr() : nullptr;
		if (NavigationMethod)
		{
			TSharedPtr<SWidget> NextFocusableChild = ScrollBoxFocus::GetNextWidget(MyGeometry, GetScrollPanelContentSize(), AsShared(), InNavigationEvent.GetUserIndex(), InNavigationEvent.GetNavigationType(), *NavigationMethod);
	...
	return OnNavigation_Default(MyGeometry, InNavigationEvent);
}

FNavigationReply SScrollBox::OnNavigation_Default(const FGeometry& MyGeometry, const FNavigationEvent& InNavigationEvent)
{
...
FSlateRect SScrollBox::GetHitTestBoundingRect() const
{
	const FGeometry WidgetGeo = GetPaintSpaceGeometry();
	FSlateRect RenderRect = WidgetGeo.GetRenderBoundingRect();

	const FVector2f ContentSize = GetScrollPanelContentSize();
	const FVector2f TransformedContentSize = WidgetGeo.GetAccumulatedLayoutTransform().TransformVector(ContentSize);

	RenderRect.Bottom = RenderRect.Top + TransformedContentSize.Y;
	RenderRect.Right = RenderRect.Left + TransformedContentSize.X;

	if (Orientation == EOrientation::Orient_Vertical)
	{
		RenderRect.Top -= DesiredScrollOffset;
	}
	else
	{
		RenderRect.Left -= DesiredScrollOffset;
	}

	return RenderRect;
}

void SScrollBox::OnAddedMetadata(const TSharedRef<ISlateMetaData>& AddedMetaData)
{
	if (AddedMetaData->IsOfType<FNavigationMetaData>())
	{
		bNavigationDataExists = true;
	}
}

void SScrollBox::OnRemovedMetadata(const TSharedRef<ISlateMetaData>& RemovedMetaData)
{
	if (RemovedMetaData->IsOfType<FNavigationMetaData>())
	{
		bNavigationDataExists = false;
	}
}
...
void SScrollBox::OnFocusChanging(const FWeakWidgetPath& PreviousFocusPath, const FWidgetPath& NewWidgetPath, const FFocusEvent& InFocusEvent)
{
	...
	OnScrollBoxFocusChanging.ExecuteIfBound(PreviousFocusPath, NewWidgetPath);
}
