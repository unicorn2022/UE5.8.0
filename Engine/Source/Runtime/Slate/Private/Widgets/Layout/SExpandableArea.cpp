...
void SExpandableArea::SetStyle(const FExpandableAreaStyle* InStyle)
{
	check(InStyle);
	WidgetStyle = InStyle;
	CollapsedImage = &WidgetStyle->CollapsedImage;
	ExpandedImage = &WidgetStyle->ExpandedImage;
	RolloutCurve = FCurveSequence(0.0f, WidgetStyle->RolloutAnimationSeconds, ECurveEaseFunction::CubicOut);

	// If it should be visible, show it now
	if (!bAreaCollapsed)
	{
		RolloutCurve.JumpToEnd();
	}
