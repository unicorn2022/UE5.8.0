...
public:
	...
	*/
	...
	/** Returns true if there are any font faces that are currently loading for the text layout . Else returns false. */
	SLATE_API bool AreAnyFontFacesLoading() const;
protected:
	...
	*/
	...
	/**
	 * Called when it is detected during a layout pass that all async loading font faces for this text layout have finished loading.
	 */
	virtual void HandleAllFontFacesFinishLoading() {}
private:
...
protected:
	...
	/** The array of unique font faces still loading for the text. */
	TArray<TWeakPtr<class FFreeTypeFace>> LoadingFontFaces;
	...
	bool ApplyLineHeightToBottomLine : 1;

	/** True if there were any font faces that were detected to be async loading in the previous layout pass. Else false. */
	bool bLoadingFontFacesOnPreviousLayout : 1;
};
