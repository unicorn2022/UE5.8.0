...
public:
	...
	/** Delegate called when we are about to take a screenshot of a SWidget. */
	...
	/**
	 * Sets a typed context payload for the current navigation event, keyed by user index.
	 * Intended to be written during OnNavigation and read during OnFocusReceived on the
	 * destination widget. Automatically cleared at the end of each navigation event.
	 */
	SLATE_API void SetPendingNavigationContext(uint32 UserIndex, TSharedPtr<ISlateMetaData> InContext);

	/**
	 * Returns the pending navigation context for the given user cast to the requested type.
	 * Returns nullptr if no context was set or the stored type does not match.
	 */
	template <typename T>
	...
	/** Clears the pending navigation context for the given user. */
	...
	*/
	...
	/** @param AccessDelegate The delegate to pass along to the widget reflector */
	void SetWidgetReflectorDebugAccessDelegate(FAccessDebugObject AccessDelegate)
	{
		DebugObjectAccessDelegate = AccessDelegate;
	}
...
public:
	...
	/** Setter and getter for the touch simulation */
...
private:
	...
	/** Per-user navigation context payloads. Written during OnNavigation, read during OnFocusReceived.
	 * Cleared automatically at the end of each navigation event by FScopedNavigationTransition.
	 */
	...
	/** Delegate for accessing debug objects, to pass to any widget inspectors. */
	...
	/** Delegate for when we are about to take a screenshot of a Slate widget */
	...
	/**
	 * Called when a regular window is moved or resized
	 */
