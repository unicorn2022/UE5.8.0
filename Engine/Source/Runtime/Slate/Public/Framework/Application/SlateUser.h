...
public:
	...
	SLATE_API void ShowTooltip(const TSharedRef<IToolTip>& InTooltip, const UE::Slate::FDeprecateVector2DParameter& InSpawnLocation, TSharedPtr<SWindow> OwnerWindow = nullptr);
	...
	SLATE_API bool HasValidFocusPath() const;
	SLATE_API const FWeakWidgetPath& GetWeakFocusPath() const;
...
private:
	...
	SLATE_API TSharedRef<SWindow> GetOrCreateTooltipWindow(TSharedPtr<SWindow> OwnerWindow = nullptr);

#if WITH_SLATE_DEBUGGING
	SLATE_API void ClearForceFieldDebug();
	SLATE_API void HandlePaint(const FPaintArgs& InArgs, const FGeometry& InAllottedGeometry, FSlateWindowElementList& InOutDrawElements, int32& InOutLayerId);

	TArray<FSlateRect> DebugForceFieldRects;
#endif
	...
	/** Per-owner tooltip windows. On Wayland, tooltips are xdg_popups that must be parented
	 *  to the correct toplevel. Each toplevel gets its own tooltip window. */
	TMap<TWeakPtr<SWindow>, TWeakPtr<SWindow>> TooltipWindowsByOwner;
	/** The currently active tooltip window (from TooltipWindowsByOwner) */
	...
	struct FActiveTooltipInfo
	{
		...
		/** The toplevel window that sourced the tooltip. Used to detect when the tooltip
		 *  needs to be recreated for a different parent (required on Wayland). */
		TWeakPtr<SWindow> SourceWindow;
	};
