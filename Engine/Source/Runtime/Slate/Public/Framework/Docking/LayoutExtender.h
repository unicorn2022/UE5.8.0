...
public:
	 ...
	 * @param TabToAdd				The new tab definition
	 * @param bShouldBeForegrounded	Whether the tab should be foregrounded, first foreground tab found when applying extension will be used. So competing foregrounded tabs are not handled.
	 */
	SLATE_API void ExtendStack(FName ExtensionId, ELayoutExtensionPosition Position, FTabManager::FTab TabToAdd, bool bShouldBeForegrounded=false);
	...
	FTabId FindFirstForegroundTab(FName ExtensionID) const
	{
		TArray<FExtendedTab, TInlineAllocator<2>> Extensions;
		StackExtensions.MultiFind(ExtensionID, Extensions, true);
		...
		for (FExtendedTab& Extension : Extensions)
		{
			if (Extension.bShouldBeForegrounded)
			{
				return Extension.TabToAdd.TabId;
			}
		}
...
private:
	...
	struct FExtendedTab
	{
		FExtendedTab(ELayoutExtensionPosition InPosition, const FTabManager::FTab& InTabToAdd, bool bInShouldBeForegrounded=false)
			: Position(InPosition), TabToAdd(InTabToAdd), bShouldBeForegrounded(bInShouldBeForegrounded)
		{}
		...
		/** Whether the tab should be foregrounded, competing foregrounded tabs are not handled */ 
		bool bShouldBeForegrounded;
	};
