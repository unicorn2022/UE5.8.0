...
template <>
struct TWidgetTypeTraits<class STableViewBase>
{
...
{
	SLATE_DECLARE_WIDGET_API(STableViewBase, SCompoundWidget, SLATE_API)
public:

	virtual bool IsTableView() const { return true; }
	...
	SLATE_API void SetBackgroundBrush(TAttribute<const FSlateBrush*> InBackgroundBrush);
...
public:
	...
	SLATE_API void SetShadowBoxStyle(const FScrollBoxStyle* InShadowBoxStyle);
	const FScrollBoxStyle* GetShadowBoxStyle() const 
	{
		return ShadowBoxStyle;
	}
...
protected:
	...
	*/
	...
	/**
	 * Automatically called when the Selection Mode has changed
	 */
	virtual void OnSelectionModeChanged(ESelectionMode::Type OldSelectionMode, ESelectionMode::Type NewSelectionMode) {}
	...
	/** Called when the user has started to touch. */
	virtual void NotifyTouchStart() = 0;

	/** Called when the user has scrolled the list some amount. */
	virtual void NotifyTouchMove() = 0;

	/** Called when the user has lifted its finger after scrolling. */
	virtual void NotifyTouchEnd() = 0;
...
protected:
	...
	{
		return SelectionModeAttribute.Get();
	}
	void Private_SetSelectionMode(TAttribute<ESelectionMode::Type> InSelectionMode)
	{
		SelectionModeAttribute.Assign(*this, MoveTemp(InSelectionMode));
	}
	TSlateAttributeRef<ESelectionMode::Type> GetSelectionModeAttribute() const
	{
		return TSlateAttributeRef<ESelectionMode::Type>{SharedThis(this), SelectionModeAttribute};
	}

	const FSlateBrush* GetBackgroundBrush() const
	{
		return BackgroundBrushAttribute.Get().GetImage();
	}
	TSlateAttributeRef<FSlateBrushCached> GetBackgroundBrushAttribute() const
	{
		return TSlateAttributeRef<FSlateBrushCached>{SharedThis(this), BackgroundBrushAttribute};
	}
...
protected:
	...
	/** The ShadowImage widget for either the Top or the Left shadow depending on the Orientation */
	TSharedPtr< class SImage > TopLeftShadowImage;
	/** The ShadowImage widget for either the Bottom or the Right shadow depending on the Orientation */
	TSharedPtr< class SImage > BottomRightShadowImage;
	...
	UE_DEPRECATED(5.8, "Use Private_SetSelectionMode / GetSelectionMode / GetSelectionModeAttribute")
	TSlateDeprecatedTAttribute<ESelectionMode::Type> SelectionMode;
#endif
	...
	UE_DEPRECATED(5.8, "Use SetBackgroundBrush / GetBackgroundBrush / GetBackgroundBrushAttribute")
	FDeprecatedInvalidatableBrushAttribute BackgroundBrush;
#endif
...
public:
	...
	float GetOverscroll() const { return Overscroll.GetOverscroll(GetTickSpaceGeometry()); }
...
private:
	...
	TSlateAttribute<ESelectionMode::Type> SelectionModeAttribute;
	ESelectionMode::Type PreviousSelectionMode;

	TSlateAttribute<FSlateBrushCached> BackgroundBrushAttribute;
