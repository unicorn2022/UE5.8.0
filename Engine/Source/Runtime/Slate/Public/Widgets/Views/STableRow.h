...
class STableRow : public ITableRow, public SBorder
{
	...
	SLATE_DECLARE_WIDGET(STableRow, SBorder)
	static inline SLATE_IMPLEMENT_WIDGET(STableRow)
public:
...
public:
	...
	*/
	...
	{
		if (GetItemDropZone().IsSet())
		{
			if (PaintDropIndicatorEvent.IsBound())
			{
				return PaintDropIndicatorEvent.Execute(GetItemDropZone().GetValue(), Args, AllottedGeometry, MyCullingRect, OutDrawElements, LayerId, InWidgetStyle, bParentEnabled);
			}
			else
			{
				return OnPaintDropIndicator(GetItemDropZone().GetValue(), Args, AllottedGeometry, MyCullingRect, OutDrawElements, LayerId, InWidgetStyle, bParentEnabled);
			}
		}
	...
	{
		SetItemDropZone(TOptional<EItemDropZone>());
	...
	virtual FReply OnDragOver(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent) override
	{
		if ( OnCanAcceptDrop.IsBound() )
		{
			...
			SetItemDropZone([ItemHoverZone, DragDropEvent, this]()
			{
				TSharedRef< ITypedTableView<ItemType> > OwnerTable = OwnerTablePtr.Pin().ToSharedRef();
				if (const TObjectPtrWrapTypeOf<ItemType>* MyItemPtr = GetItemForThis(OwnerTable))
				{
	...
	STableRow()
		...
		, ItemDropZoneAttribute(*this, TOptional<EItemDropZone>())
	{
#if WITH_ACCESSIBILITY
...
protected:
	const TOptional<EItemDropZone>& GetItemDropZone() const { return ItemDropZoneAttribute.Get(); }
	void SetItemDropZone(TOptional<EItemDropZone> InItemDropZone) { ItemDropZoneAttribute.Assign(*this, MoveTemp(InItemDropZone)); }
	TSlateAttributeRef<TOptional<EItemDropZone>> GetItemDropZoneAttribute() const
	{
		return TSlateAttributeRef<TOptional<EItemDropZone>>{SharedThis(this), ItemDropZoneAttribute};
	}
...
protected:
	...
	UE_DEPRECATED(5.8, "Use GetItemDropZone / SetItemDropZone / GetItemDropZoneAttribute.")
...
private:
	...
	/** Are we currently dragging/dropping over this item? */
	TSlateAttribute<TOptional<EItemDropZone>> ItemDropZoneAttribute;
};

template<typename ItemType>
inline void STableRow<ItemType>::PrivateRegisterAttributes(FSlateAttributeInitializer& AttributeInitializer)
{
	SLATE_ADD_MEMBER_ATTRIBUTE_DEFINITION(AttributeInitializer, ItemDropZoneAttribute, EInvalidateWidgetReason::Paint);
}
