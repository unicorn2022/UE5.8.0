...
public:
	...
	/** @return the index of the column associated to the specified name */
	SLATE_API int32 FindColumnIndex( const FName& InColumnId ) const;

	/** Adds a column to the header */
	SLATE_API void AddColumn( const FColumn::FArguments& NewColumnArgs );
	SLATE_API void AddColumns( TConstArrayView<FColumn::FArguments> NewColumns );
	...
	SLATE_API void RemoveColumns( const int32 StartIdx, const int32 Count );

	/** Swap two columns at the specified indices */
	SLATE_API void SwapColumns( const int32 FirstColumnIdx, const int32 SecondColumnIdx );
