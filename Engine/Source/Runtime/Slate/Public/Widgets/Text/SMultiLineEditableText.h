...
template <>
struct TWidgetTypeTraits<class SMultiLineEditableText>
{
...
{
	SLATE_DECLARE_WIDGET_API(SMultiLineEditableText, SWidget, SLATE_API)
public:
		...
		, _FontFacesLoadingPaintPolicy(EFontFacesLoadingPaintPolicy::DoNotPaint)
	{
		...
		/** Whether to wrap text automatically based on the widget's computed horizontal space.  IMPORTANT: Using automatic wrapping can result
			in visual artifacts, as the computed wrapped size will be at least one frame late!  Consider using WrapTextAt instead.  The initial
	...
	{
		return const_cast<const SMultiLineEditableText*>(this)->GetTextLineCount();
	}
	/** Get the number of Text Lines */
	 ...
	 */
	SLATE_API void SetClearTextSelectionOnFocusLoss(TAttribute<bool> InClearTextSelectionOnFocusLoss);
	...
	/** Get the runs that are currently selected, some of which may be only partially selected */
...
protected:
	...
	/** True if we want the text control to lose focus on a text commit/revert events */
