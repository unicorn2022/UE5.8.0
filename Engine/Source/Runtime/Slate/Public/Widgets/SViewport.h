...
namespace UE::Slate::Viewport
{
	/**
	 * Calculates the scaling factor needed to transform from local Slate geometry to viewport pixel geometry
	 * @param InGeometry The viewport's Slate geometry
	 * @param InViewportSize The viewport's render size
	 * @return A non-uniform scaling factor to convert from local Slate coordinates to viewport pixel coordinates
	 */
	SLATE_API FVector2D GetViewportLocalToPixelScale(const FGeometry& InGeometry, const FVector2D& InViewportSize);
}