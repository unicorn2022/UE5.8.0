...
template <>
struct TWidgetTypeTraits<class SMenuAnchor>
{
...
{
	SLATE_DECLARE_WIDGET_API(SMenuAnchor, SPanel, SLATE_API)
public:
...
protected:
	...
	/**
	 * Checks if any parent of the current SMenuAnchor is hidden (in which case this returns EVisibility::Collapsed).
	 * If all the ancestors are visible, this returns the value of the OriginalVisibilityAttribute.
	 */
	SLATE_API EVisibility GetMenuContentVisibility(TAttribute<EVisibility> OriginalVisibilityAttribute) const;

	static SLATE_API TArray<TWeakPtr<IMenu>> OpenApplicationMenus;

	TSlateAttributeRef<EMenuPlacement> GetPlacementAttribute() const
	{
		return TSlateAttributeRef<EMenuPlacement>{SharedThis(this), PlacementAttribute};
	}
...
protected:
	...
	UE_DEPRECATED(5.8, "Use SetMenuPlacement / GetPlacementAttribute")
	TSlateDeprecatedTAttribute<EMenuPlacement> Placement;
#endif
...
private:
	/** How should the popup be placed relative to the anchor. */
	TSlateAttribute<EMenuPlacement> PlacementAttribute;
};
