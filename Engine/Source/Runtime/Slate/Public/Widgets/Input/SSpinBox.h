...
template <typename NumericType>
struct TWidgetTypeTraits<SSpinBox<NumericType>>
{
...
{
	SLATE_DECLARE_WIDGET_API(SSpinBox, SCompoundWidget, SLATE_API)
	static inline SLATE_IMPLEMENT_WIDGET(SSpinBox)
public:
	...
	SLATE_BEGIN_ARGS(SSpinBox<NumericType>)
		...
		, _ShiftAltMultiplier(100.f)
		, _CtrlAltMultiplier(0.01f)
	...
	SLATE_STYLE_ARGUMENT(FSpinBoxStyle, Style)
		...
		/** Multiplier to use when shift+alt is held down */
		SLATE_ATTRIBUTE(float, ShiftAltMultiplier)
		/** Multiplier to use when ctrl+alt is held down */
		SLATE_ATTRIBUTE(float, CtrlAltMultiplier)
	...
	SLATE_API void SetValue(TAttribute<NumericType> InValueAttribute, const bool bShouldCommit = false);

	/** See the MinValue attribute */
	SLATE_API NumericType GetMinValue() const;

	SLATE_API void SetMinValue(TAttribute<TOptional<NumericType>> InMinValue);

	/** See the MaxValue attribute */
	SLATE_API NumericType GetMaxValue() const;

	SLATE_API void SetMaxValue(TAttribute<TOptional<NumericType>> InMaxValue);
	...
	SLATE_API void SetMinSliderValue(TAttribute<TOptional<NumericType>> InMinSliderValue);
	...
	SLATE_API void SetMaxSliderValue(TAttribute<TOptional<NumericType>> InMaxSliderValue);
	...
	SLATE_API void SetSliderExponent(TAttribute<float> InSliderExponent);
...
private:
	/** Returns the specific step multiplier based on the modifier keys pressed */
	float GetInputEventMultiplier(const FInputEvent& InEvent) const;
	...
	TSlateAttribute<NumericType> ValueAttribute;
	...
	TSlateAttribute<NumericType> Delta;

	// Step multipliers
	TAttribute<float> ShiftMultiplier;
	TAttribute<float> CtrlMultiplier;
	TAttribute<float> ShiftAltMultiplier;
	TAttribute<float> CtrlAltMultiplier;

	TAttribute<int32> LinearDeltaSensitivity;
	TSlateAttribute<float> SliderExponent;
	TSlateAttribute<NumericType> SliderExponentNeutralValue;
	TSlateAttribute< TOptional<NumericType> > MinValue;
	TSlateAttribute< TOptional<NumericType> > MaxValue;
	TSlateAttribute< TOptional<NumericType> > MinSliderValue;
	TSlateAttribute< TOptional<NumericType> > MaxSliderValue;
	...
	TSlateAttribute<bool> bDragging;

	/** True when the SliderExponentNeutralValue attribute is set */
	bool bIsSliderExponentNeutralValueSet;
