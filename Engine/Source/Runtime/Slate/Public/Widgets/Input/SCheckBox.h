...
template<>
struct TWidgetTypeTraits<class SCheckBox>
{
...
{
	SLATE_DECLARE_WIDGET_API(SCheckBox, SCompoundWidget, SLATE_API)
public:
...
protected:
	SLATE_API void SetPaddingOverride(TAttribute<FMargin> InPaddingOverride);
	TSlateAttributeRef<FMargin> GetPaddingOverrideAttribute() const
	{
		return TSlateAttributeRef<FMargin>{SharedThis(this), PaddingOverrideAttribute};
	}

	SLATE_API void SetForegroundColorOverride(TAttribute<FSlateColor> InForegroundColorOverride);
	TSlateAttributeRef<FSlateColor> GetForegroundColorOverrideAttribute() const
	{
		return TSlateAttributeRef<FSlateColor>{SharedThis(this), ForegroundColorOverrideAttribute};
	}

	SLATE_API void SetBorderBackgroundColorOverride(TAttribute<FSlateColor> InBorderBackgroundColorOverride);
	TSlateAttributeRef<FSlateColor> GetBorderBackgroundColorOverrideAttribute() const
	{
		return TSlateAttributeRef<FSlateColor>{SharedThis(this), BorderBackgroundColorOverrideAttribute};
	}

	/** The Sound to play when the check box is hovered  */
	void SetHoveredSound(TOptional<FSlateSound> InHoveredSound);
	FSlateSound GetHoveredSound() const;

	/** The Sound to play when the check box is checked */
	void SetCheckedSound(TOptional<FSlateSound> InCheckedSound);
	FSlateSound GetCheckedSound() const;

	/** The Sound to play when the check box is unchecked */
	void SetUncheckedSound(TOptional<FSlateSound> InUncheckedSound);
	FSlateSound GetUncheckedSound() const;

private:
	/**
	 * Returns the Foreground Color of this widget.
	 * Used to avoid overriding SCompoundWidget::GetForegroundColor for invalidation to work properly.
	 */
	FSlateColor GetForegroundColorInternal() const;
...
protected:
	...
	UE_DEPRECATED(5.8, "Use SetPaddingOverride / OnGetPadding / GetPaddingOverrideAttribute")
	TSlateDeprecatedTAttribute<FMargin> PaddingOverride;
	/** Overrides foreground color in the widget style, if set */
	UE_DEPRECATED(5.8, "Use SetForegroundColorOverride / GetForegroundColor / GetForegroundColorOverrideAttribute")
	TSlateDeprecatedTAttribute<FSlateColor> ForegroundColorOverride;
	/** Overrides border background color in the widget style, if set */
	UE_DEPRECATED(5.8, "Use SetBorderBackgroundColorOverride / OnGetBorderBackgroundColor / GetBorderBackgroundColorOverrideAttribute")
	TSlateDeprecatedTAttribute<FSlateColor> BorderBackgroundColorOverride;
#endif
...
#if WITH_EDITORONLY_DATA
	/** The Sound to play when the check box is hovered  */
	UE_DEPRECATED(5.8, "Use GetHoveredSound / SetHoveredSound")
	FSlateSound HoveredSound;

	/** The Sound to play when the check box is checked */
	UE_DEPRECATED(5.8, "Use GetCheckedSound / SetCheckedSound")
	FSlateSound CheckedSound;

	/** The Sound to play when the check box is unchecked */
	UE_DEPRECATED(5.8, "Use GetUncheckedSound / SetUncheckedSound")
...
private:
	/** Overrides padding in the widget style, if set */
	TSlateAttribute<FMargin> PaddingOverrideAttribute;
	bool bIsPaddingAttrSet = false;
	
	/** Overrides foreground color in the widget style, if set */
	TSlateAttribute<FSlateColor> ForegroundColorOverrideAttribute;
	/** Overrides border background color in the widget style, if set */
	TSlateAttribute<FSlateColor> BorderBackgroundColorOverrideAttribute;
	bool bIsBorderBackgroundColorOverrideAttrSet = false;

	/** The Sound to play when the check box is hovered  */
	TOptional<FSlateSound> HoveredSoundOverride;

	/** The Sound to play when the check box is checked */
	TOptional<FSlateSound> CheckedSoundOverride;

	/** The Sound to play when the check box is unchecked */
	TOptional<FSlateSound> UncheckedSoundOverride;
};
