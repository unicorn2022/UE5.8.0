...
{
	extern SLATE_API bool bMouseButtonHandlingV2;
}
...
public:
	...
	void Construct( const FArguments& InArgs, const TSharedRef<STableViewBase>& InOwnerTable )
	{
		STableRow< OptionType >::Construct(
			...
			.bAllowPreselectedItemActivation(SlateComboRow::bMouseButtonHandlingV2)
			.Content()
			[
	...
	{
		if (!STableRow<OptionType>::bAllowPreselectedItemActivation)
		{
...
public:
	...
	SLATE_BEGIN_ARGS( SComboBox )
		...
		, _IsFocusable( true )
		, _ReselectionTriggersOnSelectionChanged(false)
		{}
		...
		*/
		...
		/** Whether OnSelectionChanged event gets triggered when selecting the item that's already selected */
		SLATE_ARGUMENT(bool, ReselectionTriggersOnSelectionChanged)
	...
	void Construct( const FArguments& InArgs )
	{
		...
		this->bReselectionTriggersOnSelectionChanged = InArgs._ReselectionTriggersOnSelectionChanged;
		...
		// Repel tooltips from appearing on the list item we are hovering...
		ComboListView->EnableToolTipForceField(true);
...
private:
	...
	void OnSelectionChanged_Internal( NullableOptionType ProposedSelection, ESelectInfo::Type SelectInfo )
	{
		...
		if(SelectInfo != ESelectInfo::OnNavigation)
		{
			...
			if (bReselectionTriggersOnSelectionChanged || ProposedSelection != SelectedItem)
			{
...
private:
	...
	// Whether OnSelectionChanged event gets triggered when selecting the item that's already selected
	bool bReselectionTriggersOnSelectionChanged;
