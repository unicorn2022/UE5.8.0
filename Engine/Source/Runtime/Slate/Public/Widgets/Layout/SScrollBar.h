...
template <>
struct TWidgetTypeTraits<class SScrollBar>
{
...
{
	SLATE_DECLARE_WIDGET_API(SScrollBar, SBorder, SLATE_API)
public:
	...
	SLATE_API void SetUserVisibility(TAttribute<EVisibility> InUserVisibility);
	 ...
	 * Even if true, tracks will only be visible if the brush from the style has an ImageSize.
	 * See FScrollBarStyle::VerticalBottomSlotImage and equivalents
	 */
...
protected:
	...
	TSlateAttributeRef<EVisibility> GetUserVisibilityAttribute() const
	{
		return TSlateAttributeRef<EVisibility>{SharedThis(this), UserVisibilityAttribute};
	}

#if WITH_EDITORONLY_DATA
	/** The scrollbar's visibility as specified by the user. Will be compounded with internal visibility rules. */
	UE_DEPRECATED(5.7, "Use SetUserVisibility / GetUserVisibilityAttribute")
	TSlateDeprecatedTAttribute<EVisibility> UserVisibility;
#endif

	const FScrollBarStyle* Style = nullptr;
...
private:
	bool bIsThicknessAttrSet = false;

	/** The scrollbar's visibility as specified by the user. Will be compounded with internal visibility rules. */
	TSlateAttribute<EVisibility> UserVisibilityAttribute;
};
