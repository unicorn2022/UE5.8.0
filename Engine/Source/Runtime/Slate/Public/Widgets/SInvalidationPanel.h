...
#if UE_SLATE_WITH_DYNAMIC_INVALIDATION
	/**
	 * Returns true when the invalidation panel should use Dynamic Invalidation.
	 * Dynamic Invalidation means that the widget's content will only be cached if all descendant widgets support invalidation.
	 * Dynamic Invalidation can be globally disabled or forced. Use `CanUseDynamicInvalidation` to check if this invalidation panel will actually
	 * use Dynamic Invalidation
	**/
	SLATE_API bool GetUseDynamicInvalidation() const;
	SLATE_API void SetUseDynamicInvalidation(bool InUseDynamicInvalidation);

	/**
	 * Checks if this invalidation panel will actually use Dynamic Invalidation, depending on the CVar `Slate.DynamicInvalidation.Options`
	 * If Dynamic Invalidation is disabled, this will return false.
	 * If this invalidation panel is not dynamic, or if Dynamic Invalidation is forced Static, this will return false.
	 * If Dynamic Invalidation is enabled, this will return the same value as `GetUseDynamicInvalidation`.
	 * If Dynamic Invalidation is forced, this will return true.
	 */
	SLATE_API bool CanUseDynamicInvalidation() const;

	SLATE_API virtual bool SupportsInvalidationRecursive(EInvalidationStrategy InInvalidationStrategy) const override;
#endif

	SLATE_API virtual bool CanCacheThisFrame() const override;
