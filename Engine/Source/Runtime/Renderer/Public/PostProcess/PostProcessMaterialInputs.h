...
struct FPostProcessMaterialInputs
{
	...
	*/
	...
	/**
	 * When true, the AddPostProcessMaterialPass can execute outside of the regular render post-processing stack.
	 * This offers a leaner alternative to DrawMaterialToRenderTarget, where bindings can be assigned using regular
	 * post-process inputs instead of named texture parameters.
	 * 
	 * Other bindings that require a fully initialized rendering pipeline are skipped.
	 * The caller remains responsible for providing valid (dummy) SceneTextures & ViewUniformBuffer.
	 */
