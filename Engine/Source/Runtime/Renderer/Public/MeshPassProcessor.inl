...
static bool ShouldMaterialAllowHardwareVariableRateShading(const FMaterial& RESTRICT MaterialResource, bool bUsesPixelDepthOffset)
{
	return MaterialResource.IsVariableRateShadingAllowed()
		&& !MaterialResource.IsMasked()		// When using pixel discard, coarse shading causes the whole block to get discarded resulting in noticeable artifacts
		&& !bUsesPixelDepthOffset;			// When writing depth from the pixel shader, coarse shading causes the whole block to output the same depth value resulting in noticeable artifacts
}
	...
	const ShaderElementDataType& ShaderElementData)
{
	...
	PipelineState.bAllowVariableRateShading = ShouldMaterialAllowHardwareVariableRateShading(MaterialResource, MaterialResource.MaterialUsesPixelDepthOffset_RenderThread()) && HardwareVariableRateShadingSupportedByScene();
	...
	TArray<FPSOPrecacheData>& PSOInitializers)
{
	...
	else
	{
		...
		MinimalPipelineStateInitializer.bAllowVariableRateShading = ShouldMaterialAllowHardwareVariableRateShading(MaterialResource, MaterialResource.MaterialUsesPixelDepthOffset_GameThread()) && HardwareVariableRateShadingSupportedByPlatform(GMaxRHIShaderPlatform);

		// NOTE: AsGraphicsPipelineStateInitializer will create the RHIShaders internally if they are not cached yet
		// Always pass false for shader creation - this function is only called during PSO precache collection.
		// bRequired above is for FPSOPrecacheData::bRequired (PSO compilation priority) which is a separate concern.
		// If any shaders fail to load, skip subsequent work and early out.

		// FORT-1109249 : temporarily forcing required == true to ensure shaders are loaded when PSO Precache tasks are created.
		// PSO precache misses will occur if shaders are not loaded as precache tasks will be silently dropped.
		FGraphicsPipelineStateInitializer PipelineStateInitializer = MinimalPipelineStateInitializer.AsGraphicsPipelineStateInitializer(/*false*/);
...
#if PSO_PRECACHING_VALIDATE
	PSOPrecacheData.ResourceName = MaterialResource.GetAssetName();
...
void DrawDynamicMeshPass(const FSceneView& View, FRHICommandList& RHICmdList, const LambdaType& BuildPassProcessorLambda, bool bForceStereoInstancingOff = false)
{
	...
	const uint32 InstanceFactor = (!bForceStereoInstancingOff && View.IsInstancedStereo() && IStereoRendering::IsAPrimaryView(View)) ? 2 : 1;
