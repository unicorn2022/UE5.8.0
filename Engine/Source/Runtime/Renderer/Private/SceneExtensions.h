...
public:
	...
	virtual ~ISceneExtension() = default;
...
{
public:
	virtual ~ISceneExtensionUpdater() = default;
	...
	virtual void PostGPUSceneUpdate(FRDGBuilder& GraphBuilder) {}
};
...
public:
	...
	virtual ~ISceneExtensionRenderer() = default;

	virtual void Begin(FSceneRendererBase* InRenderer) { }
	virtual void End() {}

	/**
	 * Called before LaunchVisibilityTasks to allow scene extensions to prepare data for use during these steps.
	 * Note that because of the visibility tasks being launched as early as possible, this can happen before the end of FScene::Update, 
	 * Notably, it happens (when done early) _after_ SceneExtensionsUpdaters.PostSceneUpdate and _before_ GPU-Scene update, in the callback Parameters.Callbacks.PostStaticMeshUpdate. 
	 * Thus, you can always rely on the updaters to have processed scene updates, but nothing after that sequencing point.
	 */
	virtual void PreVisibilityTasks(FRDGBuilder& GraphBuilder) {}
...
public:
	virtual ~ISceneExtensionFactory() = default;
...
public:
	...
	void PostGPUSceneUpdate(FRDGBuilder& GraphBuilder)
	{
		for (auto Updater : Updaters) { Updater->PostGPUSceneUpdate(GraphBuilder); }
	}
...
public:
	...
	/**
	 * Called before LaunchVisibilityTasks to allow scene extensions to prepare data for use during these steps.
	 * For details see ISceneExtensionRenderer::PreVisibilityTasks
	 */
	void PreVisibilityTasks(FRDGBuilder& GraphBuilder)
	{
		ValidateAdvanceCallbackStage(ECallbackStage::PreVisibilityTasks);
		for (auto Renderer : Renderers) { Renderer->PreVisibilityTasks(GraphBuilder); }
	}	
...
public:
	...
	virtual ~TSceneExtensionRegistration() = default;
...
#define DECLARE_SCENE_EXTENSION(ModuleExport, ClassName) \
	public: \
		ModuleExport static int32 GetExtensionID(); \
	private: \
...
#define IMPLEMENT_SCENE_EXTENSION(ClassName) \
	int32 ClassName::GetExtensionID() { return ExtensionRegistration.GetExtensionID();  } \
