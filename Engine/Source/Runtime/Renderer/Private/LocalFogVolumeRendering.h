...
{
	uint32 LocalFogVolumeInstanceCount;			// Total count in the scene
	uint32 LocalFogVolumeInstanceCountFinal;	// Count after culling on CPU has happened from GetLocalFogVolumeViewSortingData.
	FLocalFogVolumeGPUInstanceData* LocalFogVolumeGPUInstanceData;
	FVector* LocalFogVolumeCenterPos;
	FVector2f* LocalFogVolumeStartDistanceAndUniformScale;
...
struct FLocalFogVolumeViewData
{
	...
	float DepthBoundStartDistance = 0.0f;
...
float GetLocalFogVolumeViewStartDistance(const FViewInfo& View);	// Account for global and all local fog volumes start distances.
