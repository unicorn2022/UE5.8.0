...
class FNaniteVisualizeCS : public FNaniteGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(uint32, NumShadingGroups)
...
public:
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_RDG_BUFFER_SRV(Buffer<uint>, EditorSelectedHitProxyIds)
		SHADER_PARAMETER_RDG_BUFFER_SRV(ByteAddressBuffer, ShadingBinData)
		SHADER_PARAMETER_RDG_BUFFER_SRV(ByteAddressBuffer, StreamingDeficitData)
		SHADER_PARAMETER_ARRAY(FLinearColor, StreamingDeficitColors, [NumLODColorationColors])
	...
	static void ModifyCompilationEnvironment(const FGlobalShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment)
	{
		...
		OutEnvironment.SetDefine(TEXT("DEBUG_VIEW_STREAMING_DEFICIT"),	(uint32)Nanite::EDebugViewMode::StreamingDeficit);
...
)
{
...

	{
		...
		PassParameters->NumShadingGroups = Nanite::GGlobalResources.NumShadingGroups;
...
)
{
	...
	if (Scene && Views.Num() > 0 && VisualizationData.IsActive() && EngineShowFlags.VisualizeNanite)
	{
		...
		if (ensure(Views.Num() == Results.Num()))
		{
			for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
			{
				...
				if (View.ShouldRenderView())
				{
				...
				#if WITH_EDITOR
					uint32 HitProxyIdCount = 0;
					if (const Nanite::FEditorSceneExtension* EditorExtension = Scene->GetExtensionPtr<Nanite::FEditorSceneExtension>())
					{
						HitProxyIdCount = EditorExtension->GetSelectionGroup(Nanite::FEditorSceneExtension::ESelectionGroup::Selected).HitProxyIds.Num();
					}
					...
					for (FVisualizeResult& Visualization : Data.Visualizations)
					{
						...
						PassParameters->EditorSelectedHitProxyIds = Nanite::GetEditorSelectedHitProxyIdsSRV(GraphBuilder, *Scene);
	...
	if (PickingBuffer != nullptr)
	{
		...
		if (PickingBufferNumPending < MaxPickingBuffers)
		{
			...
			{
				static const FName PickingFeedbackName("Nanite.PickingFeedback");
...
/** Build a per-primitive buffer of streaming deficit indices (0-7) for the Nanite debug view shader. */
static FRDGBufferRef BuildStreamingDeficitBuffer(FRDGBuilder& GraphBuilder, const FScene& Scene, const FViewInfo& View)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(Nanite_BuildStreamingDeficitBuffer);

	const FEngineShowFlags& ShowFlags = View.Family->EngineShowFlags;
	const bool bTextureDeficit = ShowFlags.StreamingTextureDeficit;
	const bool bTextureResidency = ShowFlags.StreamingTextureResidency;
	const bool bMeshDeficit = ShowFlags.StreamingMeshLODDeficit;
	const bool bMeshResidency = ShowFlags.StreamingMeshLODResidency;
	const bool bIsTexture = bTextureDeficit || bTextureResidency;
	const bool bUseWantedMips = bTextureDeficit || bMeshDeficit;

	const int32 MaxPrimitiveIndex = Scene.GetMaxPersistentPrimitiveIndex();
	if (MaxPrimitiveIndex <= 0)
	{
		return GSystemTextures.GetDefaultByteAddressBuffer(GraphBuilder, 4u);
	}

	TArray<uint32> DeficitData;
	DeficitData.SetNumZeroed(MaxPrimitiveIndex);

	TMap<const FMaterialRenderProxy*, int32> MaterialCache;

	TConstArrayView<FPrimitiveSceneProxy*> Proxies = Scene.GetPrimitiveSceneProxies();
	for (int32 PackedIdx = 0; PackedIdx < Proxies.Num(); ++PackedIdx)
	{
		const FPrimitiveSceneProxy* Proxy = Proxies[PackedIdx];
		if (!Proxy)
		{
			continue;
		}
		...
		const int32 PersistentIdx = SceneInfo->GetPersistentIndex().Index;
		if (PersistentIdx < 0 || PersistentIdx >= MaxPrimitiveIndex)
		{
			continue;
		}
		...
		if (bIsTexture)
		{
			// Helper to accumulate deficit from a material render proxy
			auto AccumulateMaterialDeficit = [&](const FMaterialRenderProxy* MatProxy)
			{
				if (!MatProxy)
				{
					return;
				}
				if (int32* Cached = MaterialCache.Find(MatProxy))
				{
					Deficit = FMath::Max(Deficit, *Cached);
				}
				else
				{
					const FMaterial* Material = MatProxy->GetMaterialNoFallback(View.GetFeatureLevel());
					if (!Material)
					{
						return;
					}
					int32 MatDeficit = ComputeTextureStreamingDeficitForMaterial(*MatProxy, *Material, bUseWantedMips);
					MaterialCache.Add(MatProxy, MatDeficit);
					Deficit = FMath::Max(Deficit, MatDeficit);
				}
			};

			// Nanite proxies store materials in MaterialSections, not in StaticMeshes
			if (Proxy->IsNaniteMesh())
			{
				const Nanite::FSceneProxyBase* NaniteProxy = static_cast<const Nanite::FSceneProxyBase*>(Proxy);
				for (const Nanite::FSceneProxyBase::FMaterialSection& Section : NaniteProxy->GetMaterialSections())
				{
					AccumulateMaterialDeficit(Section.ShadingMaterialProxy);
				}
			}
			else
			{
				...
				{
					AccumulateMaterialDeficit(Mesh.MaterialRenderProxy);
				}
			}
		}
		else if (bMeshDeficit)
		{
			Deficit = Proxy->GetStreamingLODWantedDeficit();
		}
		else if (bMeshResidency)
		{
			Deficit = Proxy->GetCurrentFirstLODIdx_RenderThread();
		}

		DeficitData[PersistentIdx] = static_cast<uint32>(FMath::Clamp(Deficit, 0, NumLODColorationColors - 1));
	}

	FRDGBufferRef Buffer = GraphBuilder.CreateBuffer(FRDGBufferDesc::CreateByteAddressDesc(MaxPrimitiveIndex * sizeof(uint32)), TEXT("Nanite.StreamingDeficitData"));
	GraphBuilder.QueueBufferUpload(Buffer, DeficitData.GetData(), MaxPrimitiveIndex * sizeof(uint32));
	return Buffer;
}
...
)
{
	...
	{
		UE_CALL_ONCE([](){ UE_LOGF(LogNanite, Error, "Platform does not support Nanite debug view shaders"); });
		return;
	}
...
#if !UE_BUILD_SHIPPING
	if (DebugViewMode == EDebugViewMode::StreamingDeficit)
	{
		PassParameters->StreamingDeficitData = GraphBuilder.CreateSRV(BuildStreamingDeficitBuffer(GraphBuilder, Scene, View));
		const TArray<FLinearColor>& Colors = GEngine->StreamingDeficitColors;
		for (int32 i = 0; i < NumLODColorationColors; ++i)
		{
			PassParameters->StreamingDeficitColors[i] = (i < Colors.Num()) ? Colors[i] : FLinearColor::Black;
		}
	}
	else
#endif
	{
		PassParameters->StreamingDeficitData = GraphBuilder.CreateSRV(GSystemTextures.GetDefaultByteAddressBuffer(GraphBuilder, 4u));
		FMemory::Memzero(PassParameters->StreamingDeficitColors);
	}
