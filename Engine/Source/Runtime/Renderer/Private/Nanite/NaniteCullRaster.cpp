...
{	
	...
	const EOutputBufferMode RasterMode = Pipeline == ERasterPipeline::Shadows ? EOutputBufferMode::DepthOnly : EOutputBufferMode::VisBuffer;
	const bool bHasVirtualShadowMapArray = Pipeline == ERasterPipeline::Shadows && UseVirtualShadowMaps(ShaderPlatform, FeatureLevel); // true during shadow pass
	const bool bIsMaterialCache = Pipeline == ERasterPipeline::MaterialCache && IsMaterialCacheSupported(ShaderPlatform);
	...
	TArray<FPSOPrecacheData>& PSOInitializers)
{
	...
	CollectRasterPSOInitializersForPipeline(SceneTexturesConfig.FeatureLevel, RasterMaterial, RasterPipeline, PSOCollectorIndex, ERasterPipeline::Primary, PreCacheParams.bDefaultMaterial, PSOInitializers);
	CollectRasterPSOInitializersForPipeline(SceneTexturesConfig.FeatureLevel, RasterMaterial, RasterPipeline, PSOCollectorIndex, ERasterPipeline::Shadows, PreCacheParams.bDefaultMaterial, PSOInitializers);
}
	...
	TArray<FPSOPrecacheData>& OutPSOPrecacheData)
{
	...
	// Collect for primary & shadows
	CollectRasterPSOInitializersForPipeline(FeatureLevel, RasterMaterial, RasterPipeline, PSOCollectorIndex, ERasterPipeline::Primary, bDefaultMaterial, OutPSOPrecacheData);
	CollectRasterPSOInitializersForPipeline(FeatureLevel, RasterMaterial, RasterPipeline, PSOCollectorIndex, ERasterPipeline::Shadows, bDefaultMaterial, OutPSOPrecacheData);	
}
				...
				if (HWShaderBundle != nullptr)
				{
					auto RecordDispatches = [&](FRHICommandDispatchGraphicsShaderBundle& Command)
					{
						...
						for (const int32 GroupIndex : DispatchList.Indirections)
						{
							...
							const bool bCullModeNone = FirstRasterizerPass.RasterPipeline.bIsTwoSided;
	...
	bool bMainPass)
{
	...
	FRDGPass* HWTrianglesPass = GraphBuilder.AddPass(
		...
		[ClusterPassParameters, &DispatchContext, ViewRect, &SceneView = SceneView, bMainPass, HardwarePath, PSOCollectorIndex, RenderFlags = RenderFlags, FeatureLevel](FRDGAsyncTask, FRHICommandList& RHICmdList)
		{
