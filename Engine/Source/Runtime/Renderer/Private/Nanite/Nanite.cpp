...
class FCalculateShadingStatsCS : public FNaniteGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_RDG_BUFFER_SRV(ByteAddressBuffer, ShadingDispatchArgs)
	END_SHADER_PARAMETER_STRUCT()
};
...
void ListStatFilters(FSceneRenderer* SceneRenderer)
{
	...
	{
		UE_LOGF(LogNanite, Warning, "** Available Filters **");

		// Primary view is always available.
		UE_LOGF(LogNanite, Warning, "Primary");
		...
		if (bListShadows)
		{
			...
			{
				UE_LOGF(LogNanite, Warning, "VirtualShadowMaps");
			}
		}
		...
		if (bListShadows)
		{
			...
			{
				UE_LOGF(LogNanite, Warning, "ShadowAtlas%d", AtlasIndex);
			}
		}
			...
			for (int32 CubemapIndex = 0; CubemapIndex < ShadowCubeMaps.Num(); CubemapIndex++)
			{
				...
				if (ProjectedShadowInfo->bNaniteGeometry && ProjectedShadowInfo->CacheMode != SDCM_MovablePrimitivesOnly)
				{
					...
					for (int32 CubemapFaceIndex = 0; CubemapFaceIndex < 6; CubemapFaceIndex++)
					{
						...
						UE_LOGF(LogNanite, Warning, "Shadow Cube Map: %ls", *CubeFaceFilterName);
					}
				}
			}
		}
...
)
{
	...
	if (VisualizationData.IsActive())
	{
		...
		Nanite::GGlobalResources.NumShadingGroups = NumShadingGroups;
	...
	if (GNaniteShowStats != 0 && Nanite::GGlobalResources.GetStatsBufferRef())
	{
		...
		const bool bBindless = (ShadeBinning.ThreadGroupData != nullptr);
		...
		PassParameters->NumShadingGroups = bBindless ? NumShadingGroups : 0xFFFFFFFFu;
		...
		PassParameters->ShadingDispatchArgs = GraphBuilder.CreateSRV(ShadeBinning.ShadingDispatchArgs);
