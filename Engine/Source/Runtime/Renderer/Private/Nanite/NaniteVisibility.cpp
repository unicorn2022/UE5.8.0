	...
	bool bAsync)
{
	...
	if (!bShouldTest)
	{
		for (const FNaniteVisibility::FShadingBin& ShadingBin : References.ShadingBins)
		{
			...
			const bool bCurveVisible = ShadingBin.Curve != 0xFFFFu ? (bool)Query->ShadingBinVisibility[int32(ShadingBin.Curve)] : true;
			if (!bTriangleVisible || !bVoxelVisible || !bCurveVisible) // Shading bin reference is not marked visible
			{
				bShouldTest = true;
				break;
			}
			...
			if (bPrimitiveVisible)
			{
				...
				if (Query->bCullShadingBins)
				{
					for (const FNaniteVisibility::FShadingBin& ShadingBin : References.ShadingBins)
					{
						...
						if (ShadingBin.Curve != 0xFFFFu)
						{
							Query->ShadingBinVisibility[int32(ShadingBin.Curve)] = true;
						}
					}
				}
			}
