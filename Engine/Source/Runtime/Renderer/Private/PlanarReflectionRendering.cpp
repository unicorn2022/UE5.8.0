	...
	const FUpdatePlanarReflectionContentsInputs& Inputs)
{
	...
	UE_CLOGF(FSceneCaptureLogUtils::bEnableSceneCaptureLogging, LogSceneCapture, Log, "Running UpdatePlanarReflectionContents_RenderThread.");
...
extern void BuildProjectionMatrix(FIntPoint RenderTargetSize, float FOV, float InNearClippingPlane, float InFarClippingPlane, FMatrix& ProjectionMatrix);
...
void FScene::UpdatePlanarReflectionContents(UPlanarReflectionComponent* CaptureComponent, FSceneRenderer& ParentSceneRenderer, ISceneRenderBuilder& SceneRenderBuilder)
{
	...
	for (int32 ViewIndex = 0; ViewIndex < ParentSceneRenderer.Views.Num() && ViewIndex < GMaxPlanarReflectionViews; ++ViewIndex)
	{
		...
		const FMatrix ViewMatrix(MirrorMatrix * View.ViewMatrices.GetWorldToView());
		...
		if (!View.IsPerspectiveProjection() || (CaptureComponent->ExtraFOV == 0.f && ParentSceneRenderer.Views.Num() > 1))
		{
			...
			ProjectionMatrix = View.ViewMatrices.GetViewToClip();
		}
		else
		{
			const float HalfFOV = FMath::Atan(1.0f / View.ViewMatrices.GetViewToClip().M[0][0]);
			BuildProjectionMatrix(View.UnscaledViewRect.Size(), HalfFOV + FMath::DegreesToRadians(CaptureComponent->ExtraFOV), GNearClippingPlane, GNearClippingPlane, ProjectionMatrix);
		}
	...
	ESetupViewFamilyFlags SetupViewFamilyFlags = ESetupViewFamilyFlags::CaptureSceneColor | ESetupViewFamilyFlags::IsPlanarReflection;
	...
	FString EventName = "PlanarReflection";
...
void FScene::RemovePlanarReflection(UPlanarReflectionComponent* Component) 
{
	...
	ENQUEUE_RENDER_COMMAND(FRemovePlanarReflectionCommand)(
		[SceneProxy, Scene](FRHICommandListImmediate& RHICmdList)
		{
			...
			// Need to clear out all planar reflection references on removal to avoid dangling pointers.
			for (int32 PrimitiveIndex = 0; PrimitiveIndex < Scene->Primitives.Num(); ++PrimitiveIndex)
			{
				Scene->Primitives[PrimitiveIndex]->RemoveCachedPlanarReflection(SceneProxy);
			}
		});
}
