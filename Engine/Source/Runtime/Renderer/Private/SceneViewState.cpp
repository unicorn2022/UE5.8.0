...
static uint64 GetTextureGPUSizeBytes(const FTextureRHIRef& Target, bool bLogSizes)
{
	...
	if (bLogSizes && Size)
	{
		UE_LOGF(LogRenderer, Log, "LogSizes\tTexture\t0x%p\t%ls\t%llu", Target.GetReference(), *Target->GetName().ToString(), Size);
	}
	return Size;
}
...
static uint64 GetRenderTargetGPUSizeBytes(const TRefCountPtr<IPooledRenderTarget>& Target, bool bLogSizes)
{
	...
	if (bLogSizes && Size)
	{
		UE_LOGF(LogRenderer, Log, "LogSizes\tRenderTarget\t0x%p\t%ls\t%llu", Target.GetReference(), Target->GetDesc().DebugName, Size);
	}
	return Size;
}
...
static uint64 GetBufferGPUSizeBytes(const TRefCountPtr<FRDGPooledBuffer>& Buffer, bool bLogSizes)
{
	...
	if (bLogSizes && Size)
	{
		...
		UE_LOGF(LogRenderer, Log, "LogSizes\tBuffer\t0x%p\t%ls\t%llu", Buffer.GetReference(), Name ? Name : TEXT("UNKNOWN"), Size);
	}
	return Size;
}
...
static uint64 GetTextureReadbackGPUSizeBytes(const FRHIGPUTextureReadback* TextureReadback, bool bLogSizes)
{
	...
	if (bLogSizes && Size)
	{
		UE_LOGF(LogRenderer, Log, "LogSizes\tTextureReadback\t0x%p\t%ls\t%llu", TextureReadback, *TextureReadback->GetName().ToString(), Size);
	}
	return Size;
}
...
static uint64 GetBufferReadbackGPUSizeBytes(const FRHIGPUBufferReadback* BufferReadback, bool bLogSizes)
{
	...
	if (bLogSizes && Size)
	{
		UE_LOGF(LogRenderer, Log, "LogSizes\tBufferReadback\t%p\t%ls\t%llu", BufferReadback, *BufferReadback->GetName().ToString(), Size);
	}
	return Size;
}
...
uint64 FPreviousViewInfo::GetGPUSizeBytes(bool bLogSizes) const
{
	uint64 TotalSize =
		...
		FSSSHistory.GetGPUSizeBytes(bLogSizes) +
	...
	for (const TPair<FLightComponentId, TSharedPtr<FScreenSpaceDenoiserHistory>>& ShadowHistoryIt : ShadowHistories)
	{
	...
	if (ThirdPartyTemporalUpscalerHistory)
	{
		...
		if (bLogSizes)
		{
			UE_LOGF(LogRenderer, Log, "LogSizes\tThirdPartyTemporalUpscaler\t%ls\t%llu", ThirdPartyTemporalUpscalerHistory->GetDebugName(), ThirdPartyTemporalUpscalerHistory->GetGPUSizeBytes());
		}
	}

	if (MegaFiltersHistory)
	{
		TotalSize += MegaFiltersHistory->GetGPUSizeBytes();
		if (bLogSizes)
		{
			UE_LOGF(LogRenderer, Log, "LogSizes\tMegaFiltersHistory\t%ls\t%llu", MegaFiltersHistory->GetDebugName(), MegaFiltersHistory->GetGPUSizeBytes());
		}
	}
...
uint64 FMegaLightsViewState::GetGPUSizeBytes(bool bLogSizes) const
{
	...
	Out += FrontLayerTranslucency.GetGPUSizeBytes(bLogSizes);
	Out += GetBufferGPUSizeBytes(PrevLightPower, bLogSizes);
	return Out;
}
...
uint64 FMegaLightsViewState::FResources::GetGPUSizeBytes(bool bLogSizes) const
{
	return
		...
		+ GetBufferGPUSizeBytes(HiddenLightHashHistory, bLogSizes)
		+ GetBufferGPUSizeBytes(VolumeVisibleLightHashHistory, bLogSizes)
		+ GetBufferGPUSizeBytes(VolumeHiddenLightHashHistory, bLogSizes)
		+ GetBufferGPUSizeBytes(TranslucencyVolume0VisibleLightHashHistory, bLogSizes)
		+ GetBufferGPUSizeBytes(TranslucencyVolume1VisibleLightHashHistory, bLogSizes)
		+ GetBufferGPUSizeBytes(TranslucencyVolume0HiddenLightHashHistory, bLogSizes)
		+ GetBufferGPUSizeBytes(TranslucencyVolume1HiddenLightHashHistory, bLogSizes)
		+ GetBufferGPUSizeBytes(FrozenLightData, bLogSizes)
		+ GetBufferGPUSizeBytes(GeneralPurposeState, bLogSizes);
}
...
uint64 FStochasticLightingViewState::GetGPUSizeBytes(bool bLogSizes) const
{
	return
		...
		GetRenderTargetGPUSizeBytes(SceneClosureHistory, bLogSizes) +
		GetRenderTargetGPUSizeBytes(FrontLayerTranslucencyDepthHistory, bLogSizes) +
		GetRenderTargetGPUSizeBytes(FrontLayerTranslucencyNormalHistory, bLogSizes);
}
...
uint64 FTranslucencyLightingViewState::GetGPUSizeBytes(bool bLogSizes) const
{
	...
	for (int32 Index = 0; Index < TVC_MAX; Index++)
	{
		...
		TotalSize += GetRenderTargetGPUSizeBytes(HistorySpecularDirection[Index], bLogSizes);
	}
...
uint64 FSceneViewState::GetGPUSizeBytes(bool bLogSizes) const
{
	...
	for (const TPair<FLightComponentId, TUniquePtr<FTemporalAAHistory>>& LightShaftBloomIt : LightShaftBloomHistoryRTs)
	{
	...
	void (*TextureFunction)(FRHICommandListImmediate&, FSceneViewStateSystemMemoryMirror&, FSceneViewState&, TRefCountPtr<IPooledRenderTarget>&))
{
	...
	TextureFunction(RHICmdList, SystemMemoryMirror, ViewState, ViewState.StochasticLighting.SceneClosureHistory);
...
void FSceneViewState::SystemMemoryMirrorBackup(FSceneViewStateSystemMemoryMirror* SystemMemoryMirror)
{
	...
	{
		{
			RHI_BREADCRUMB_EVENT_STAT(RHICmdList, SystemMemoryBackup, "SystemMemoryBackup");
...
void FSceneViewState::SystemMemoryMirrorRestore(FSceneViewStateSystemMemoryMirror* SystemMemoryMirror)
{
	...
	{
		RHI_BREADCRUMB_EVENT_STAT(RHICmdList, SystemMemoryRestore, "SystemMemoryRestore");
...
void FSceneViewState::AddReferencedObjects(FReferenceCollector& Collector)
{
	Collector.AddReferencedObjects(MIDPool);
}

UMaterialInstanceDynamic* FSceneViewState::GetReusableMID(class UMaterialInterface* InSource)
{		
...
void FSceneViewState::ClearMIDPool(FStringView MidParentRootPath)
{
	check(IsInGameThread());
...
void FSceneViewState::ActivateFrozenViewMatrices(FSceneView& SceneView)
{
...
void FSceneViewState::RestoreUnfrozenViewMatrices(FSceneView& SceneView)
{
	auto* ViewState = static_cast<FSceneViewState*>(SceneView.State);
	if (ViewState->bIsFrozen)
	{
