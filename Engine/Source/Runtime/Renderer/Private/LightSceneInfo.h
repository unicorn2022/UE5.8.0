...
public:
	...
	// Note: these are provided for TSceneUpdateCommandQueue compatibility
	using FPersistentId = FLightSceneId;
...
public:
	...
	{
		return GetTypeHash(LightSceneInfo->Id);
	}

	bool IsAValidShadowPointLight(const FViewInfo& View, const FVisibleLightInfoArray& VisibleLightInfos, EShaderPlatform ShaderPlatform) const;

	bool SetupMobileMovableLocalSpotLightShadowParameters(const FViewInfo& View, const FVisibleLightInfoArray& VisibleLightInfos, FMobileMovableLocalLightShadowParameters& MobileMovableLocalLightShadowParameters) const;
...
/**
 * Array type (sparse) for storing FLightSceneInfoCompact in the renderer.
 */
using FSceneLightInfoArray = TLightSparseArray<FLightSceneInfoCompact, TAlignedSparseArrayAllocator<alignof(FLightSceneInfoCompact)>>;
