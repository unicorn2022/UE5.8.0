...
static void CreateSplitScreenDebugViewFamilies(const FSceneViewFamily& InFamily, TConstArrayView<FSceneViewFamily*>& OutFamilies, TArray<FSceneViewFamily*, TFixedAllocator<2>>& OutFamiliesStorage)
{
	...
	for (int32 ViewIndex = 0; ViewIndex < 2; ViewIndex++)
	{
		...
		if (ViewIndex == 1)
		{
			FSceneViewState* SourceViewState = InFamily.Views[0]->State->GetConcreteViewState();
			
			if (SourceViewState->SplitScreenDebugViewState == nullptr)
			{
				SourceViewState->SplitScreenDebugViewState = new FSceneViewState(InFamily.GetFeatureLevel(), nullptr);
			}

			FSceneViewState* SecondViewState = SourceViewState->SplitScreenDebugViewState;

			// Propagate this user writable field between FSceneViewState
			SecondViewState->SequencerState = SourceViewState->SequencerState;

			// Add or remove optional Lumen scene for second view state
			if (CVarSplitScreenDebugLumenScene.GetValueOnGameThread())
			{
				SecondViewState->AddLumenSceneData(ViewParents[ViewIndex]->Scene, 1.0f);
			}
			else
			{
				SecondViewState->RemoveLumenSceneData(ViewParents[ViewIndex]->Scene);
			}

			InitOptions.SceneViewStateInterface = SecondViewState;
		}
	...
	for (FOp Op : Ops)
	{
		...
		case FOp::EType::Render:
		{
			...
			ENQUEUE_RENDER_COMMAND(SceneRenderBuilder_Render)([this, &RenderNode] (FRHICommandListImmediate& RHICmdList)
			{
				...
				{
					VISUALIZE_TEXTURE_BEGIN_VIEW(Scene->GetFeatureLevel(), Renderer.Views[0].GetViewKey(), FunctionInputs.FullPath, Renderer.Views[0].bIsSceneCapture, Renderer.Views[0].bIsReflectionCapture);
				}
