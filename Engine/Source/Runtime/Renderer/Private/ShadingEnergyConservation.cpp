...
public:
	...
	virtual void ReleaseRHI()
	{
		...
		GGXMaxSpecEnergyTexture.SafeRelease();
	}
...
namespace ShadingEnergyConservationData
{
	...
	static TRefCountPtr<IPooledRenderTarget> CreateTexture2D(FRHICommandListImmediate& RHICmdList, TObjectPtr<class UTexture2D>& InCPUTexture, EPixelFormat InFormat, const TCHAR* InName)
	{
		...
		check(InFormat == PF_A16B16G16R16 || InFormat == PF_R8G8B8A8 || InFormat == PF_G16R16 || InFormat == PF_R8G8 || InFormat == PF_G16 || InFormat == PF_R8);
		const uint32 ComponentCount = (InFormat == PF_A16B16G16R16 || InFormat == PF_R8G8B8A8) ? 4u : ((InFormat == PF_G16R16 || InFormat == PF_R8G8) ? 2u: 1u);
		const bool b8bit = InFormat == PF_R8G8B8A8 || InFormat == PF_R8G8 || InFormat == PF_R8;
		...
		for (int32 y = 0; y < DataSize.Y; ++y)
		{
			...
			{		
				...
				if (b8bit)
				{
					...
					if (ComponentCount > 2)
					{
						Dest[2] = uint8(FMath::Clamp(float(Value.B) * 0xFF, 0u, 0xFF));
						Dest[3] = uint8(FMath::Clamp(float(Value.A) * 0xFF, 0u, 0xFF));
					}
				}
				else
				{
					...
					if (ComponentCount > 2)
					{
						Dest[2] = uint16(FMath::Clamp(float(Value.B) * 0xFFFF, 0u, 0xFFFF));
						Dest[3] = uint16(FMath::Clamp(float(Value.A) * 0xFFFF, 0u, 0xFFFF));
					}
				}
			}
		}
...
class FBuildShadingEnergyConservationTableCS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D, Output3Texture2D)
...
void Init(FRDGBuilder& GraphBuilder, TArray<FViewInfo>& Views)
{
	...
	const int Size2DMaxLUT = 32; // Must match GGX_MAX_LUT_RESOLUTION

	bool bNeedEnergyConservationLutData = false;
	for (auto& View : Views)
	{
		bNeedEnergyConservationLutData |= GetSettings(View).bNeedData;
		if (bNeedEnergyConservationLutData)
		{
			break;
		}
	}
	...
	const bool bRGBA16Supported = GPixelFormats[PF_A16B16G16R16].Supported && UE::PixelFormat::HasCapabilities(PF_A16B16G16R16, EPixelFormatCapabilities::TextureFilterable);
	...
	const EPixelFormat MaxSpecFormat = bRuntimeGeneration ? PF_FloatRGBA : (bRGBA16Supported ? PF_A16B16G16R16 : PF_R8G8B8A8);
	const EPixelFormat DiffFormat = bR16Supported ? PF_G16 : PF_R8;

	const bool bBuildTable =
		Out.GGXMaxSpecEnergyTexture == nullptr ||	// This texture is always loaded on GPU as it needs to always be available for Toon shading.
		bRuntimeGeneration ||						// A developer is running runtime generation
		(bNeedEnergyConservationLutData && 
			(
			...
			Out.GGXMaxSpecEnergyTexture == nullptr ||
...

	{
		if (bBuildTable)
		{
			...
			if (bRuntimeGeneration)
			{
				...
				FRDGTextureRef GGXMaxSpecEnergyTexture	= GraphBuilder.CreateTexture(FRDGTextureDesc::Create2D(FIntPoint(Size2D, Size2D),         MaxSpecFormat, FClearValueBinding::None, TexCreate_ShaderResource | TexCreate_UAV), TEXT("Shading.GGXMaxSpecEnergy"),ERDGTextureFlags::MultiFrame);
			
				const uint32 NumSamples = 1u << 14u; 
				auto GlobalShaderMap = GetGlobalShaderMap(GMaxRHIShaderPlatform);
				...
				// GGX
				{
					...
					TShaderMapRef<FBuildShadingEnergyConservationTableCS> ComputeShader(GlobalShaderMap, PermutationVector);
				...
				{
					FBuildShadingEnergyConservationTableCS::FPermutationDomain PermutationVector;
					PermutationVector.Set<FBuildShadingEnergyConservationTableCS::FEnergyTableDim>(FBuildShadingEnergyConservationTableCS::EEnergyTableType::GGXMaxSpecular);
					TShaderMapRef<FBuildShadingEnergyConservationTableCS> ComputeShader(GlobalShaderMap, PermutationVector);
					FBuildShadingEnergyConservationTableCS::FParameters* PassParameters = GraphBuilder.AllocParameters<FBuildShadingEnergyConservationTableCS::FParameters>();
					PassParameters->NumSamples = NumSamples;
					PassParameters->EnergyTableResolution = Size2DMaxLUT;
					PassParameters->Output3Texture2D = GraphBuilder.CreateUAV(FRDGTextureUAVDesc(GGXMaxSpecEnergyTexture, 0));
					FComputeShaderUtils::AddPass(
						GraphBuilder,
						RDG_EVENT_NAME("ShadingEnergyConservation::BuildTable(GGXMaxSpec)"),
						ComputeShader,
						PassParameters,
						FComputeShaderUtils::GetGroupCount(FIntPoint(Size2DMaxLUT, Size2DMaxLUT), FComputeShaderUtils::kGolden2DGroupSize));
				}
				...
				Out.GGXMaxSpecEnergyTexture = GraphBuilder.ConvertToExternalTexture(GGXMaxSpecEnergyTexture);
			}
			else
			{
				...
				check(MaxSpecFormat == PF_A16B16G16R16 || MaxSpecFormat == PF_R8G8B8A8);
				...
				// LUTs that are always loaded.
				Out.GGXMaxSpecEnergyTexture = ShadingEnergyConservationData::CreateTexture2D(GraphBuilder.RHICmdList, GEngine->GGXMaxSpecEnergyTexture,      MaxSpecFormat, TEXT("Shading.GGXMaxSpecEnergyTexture"));
				...
				if (!Out.GGXMaxSpecEnergyTexture) { Out.GGXMaxSpecEnergyTexture = GSystemTextures.WhiteDummy; }
			}
		}
	}
}
...
FShadingEnergyConservationData GetData(const FViewInfo& View)
{
	...
	Out.GGXMaxSpecEnergyTexture = GShadingEnergyConservationResources.GGXMaxSpecEnergyTexture;
	return Out;
}
