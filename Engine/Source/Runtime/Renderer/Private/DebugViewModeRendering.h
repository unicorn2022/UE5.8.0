...
private:
	...
	/** Per-proxy cache for texture streaming deficit. Keyed on proxy (not FMaterial*) because
	    material instances sharing the same compiled material can have different texture overrides. */
	TMap<const FMaterialRenderProxy*, int32> TextureStreamingDeficitCache;
};
