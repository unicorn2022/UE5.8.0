...
class FDistanceFieldShadowingUpsamplePS : public FGlobalShader
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(float, ShadowFadeFraction)
		SHADER_PARAMETER(float, MinDepth)
		SHADER_PARAMETER(float, MaxDepth)
	END_SHADER_PARAMETER_STRUCT()
...
class FShadowTileVS : public FGlobalShader
{
	...
	static bool ShouldCompilePermutation(const FGlobalShaderPermutationParameters& Parameters)
	{
		return UseShadowIndirectDraw(Parameters.Platform) && ShouldCompileDistanceFieldShaders(Parameters.Platform);
	}
...
bool SupportsDistanceFieldShadows(ERHIFeatureLevel::Type FeatureLevel, EShaderPlatform ShaderPlatform)
{
	return GDistanceFieldShadowing 
		...
		&& ShouldCompileDistanceFieldShaders(ShaderPlatform);
}
...
{
	if (!SceneUpdateInputs.HasAnyShowFlags([] (const FEngineShowFlags& EngineShowFlags) { return EngineShowFlags.DynamicShadows && EngineShowFlags.RayTracedDistanceFieldShadows; }) ||
	...
	const FLightTileIntersectionParameters& LightTileIntersectionParameters)
{
...

	{
		...
		PassParameters->ScreenToView = FMatrix44f(View.ViewMatrices.GetScreenToClipMatrix() * View.ViewMatrices.GetClipToTranslatedWorld() * View.ViewMatrices.GetOverriddenTranslatedViewMatrix());
	...
	FTiledShadowRendering* TiledShadowRendering)
{
	...
	if (RayTracedShadowsTexture.IsValid())
	{
		...
		PassParameters->PS.ShadowFadeFraction = FadeAlphas[ViewIndex];
		...
		if (bRunTiled)
		{
				...
				[this, &View, VertexShader, PixelShader, ScissorRect, bProjectingForForwardShading, PassParameters, bForceRGBModulation](FRDGAsyncTask, FRHICommandList& RHICmdList)
			{
				...
				if (bDirectionalLight)
				{
					SetDepthBoundsTest(RHICmdList, CascadeSettings.SplitNear - CascadeSettings.SplitNearFadeRegion, CascadeSettings.SplitFar, View.ViewMatrices.GetViewToClip());
				}
