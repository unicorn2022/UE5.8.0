...
FLightSceneInfo::FLightSceneInfo(FLightSceneProxy* InProxy, bool InbVisible)
	...
	, bPrecomputedLightingIsValid(InProxy->IsPrecomputedLightingValid())
	, bVisible(InbVisible)
	, bEnableLightShaftBloom(InProxy->IsLightShaftBloomEnabled())
	, BloomScale(InProxy->GetBloomScale())
	, BloomThreshold(InProxy->GetBloomThreshold())
	, BloomMaxBrightness(InProxy->GetBloomMaxBrightness())
	, BloomTint(InProxy->GetBloomTint())
	, NumUnbuiltInteractions(0)
	, bCreatePerObjectShadowsForDynamicObjects(Proxy->ShouldCreatePerObjectShadowsForDynamicObjects())
	, Scene(static_cast<FScene*>(InProxy->GetSceneInterface()))
{
}
...
bool FLightSceneInfo::IsAValidShadowPointLight(const FViewInfo& View, const FVisibleLightInfoArray& VisibleLightInfos, EShaderPlatform ShaderPlatform) const
{
	...
	bool bShouldCastPointShadow = View.Family->EngineShowFlags.DynamicShadows
		&& GetShadowQuality() > 0
		&& VisibleLightInfos[GetPersistentIndex()].AllProjectedShadows.Num() > 0
		&& VisibleLightInfos[GetPersistentIndex()].AllProjectedShadows.Last()->bAllocated;

	return bShouldCastPointShadow;
}

bool FLightSceneInfo::SetupMobileMovableLocalSpotLightShadowParameters(const FViewInfo& View, const FVisibleLightInfoArray& VisibleLightInfos, FMobileMovableLocalLightShadowParameters& MobileMovableLocalLightShadowParameters) const
{
...
uint32 FLightSceneInfo::PackExtraData(bool bAllowStaticLighting, bool bLightFunction, bool bMegaLight, bool bClusteredDeferredSupported) const
{
	...
	const uint32 UseTransmission = Proxy->Transmission() ? 1 : 0;
	Result |= UseTransmission << LIGHT_EXTRA_DATA_BIT_OFFSET_USE_TRANSMISSION;
