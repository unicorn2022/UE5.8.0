...
class FPrintIlluminanceMeterPS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(int32, SkyLightCubemapMaxResolution)
		SHADER_PARAMETER(int32, HardwareCubemapMaxResolution)
		SHADER_PARAMETER(int32, CurrentCubemapResolution)
...
FScreenPassTexture ProcessAndRenderIlluminanceMeter(FRDGBuilder& GraphBuilder, const FViewInfo& View, FScreenPassTexture& ScreenPassSceneColor)
{
	...
	// 3- Sum up each faces and display illuminance on screen
	{
		...
		PassParameters->CurrentCubemapResolution = SkyLightTextureRHI ? SkyLightTextureRHI->GetDesc().Extent.X : 0;
		PassParameters->HardwareCubemapMaxResolution = GetMaxCubeTextureDimension();
		static auto* SkyLightCubemapMaxResolutionCVar = IConsoleManager::Get().FindConsoleVariable(TEXT("r.SkyLight.CubemapMaxResolution"));
		PassParameters->SkyLightCubemapMaxResolution = SkyLightCubemapMaxResolutionCVar ? SkyLightCubemapMaxResolutionCVar->GetInt() : 0;
