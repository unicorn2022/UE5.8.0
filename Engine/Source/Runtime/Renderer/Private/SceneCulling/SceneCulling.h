...
{
	FCullingVolume() = default;
	explicit FCullingVolume(const FConvexVolume& InConvexVolume) 
		: ConvexVolume(InConvexVolume)
	{
	}

	FConvexVolume ConvexVolume;
	// Negative translation to add to the tested location prior to testing the ConvexVolume.
	FVector3d WorldToVolumeTranslation = FVector3d::ZeroVector; 
	...
	bool IntersectBox(const FVector& Origin, const FVector& BoxExtent) const
	{
		if (ConvexVolume.Planes.IsEmpty())
		{
			return FMath::SphereAABBIntersection(Sphere, FBox(Origin - BoxExtent, Origin + BoxExtent));
		}
		return ConvexVolume.IntersectBox(Origin + WorldToVolumeTranslation, BoxExtent);
	}
};
...
public:
	...
	/**
	 * Provides info to build cpu-side instance culling structures.
	 */
	class FPrimitiveCullingInfo
	{
	private:
		// Offset into a semi-compact range of bits (aligned to full word boundaries to avoid race conditions during async updates.
		uint32 HashGroupVisibilityOffset = 0u;
		uint32 HashGroupLODOffset = 0u;
		uint32 NumItems = 0u;
		float MaxInstanceDrawDistance = -1.0f;
		FPersistentPrimitiveIndex PersistentPrimitiveId;
	...
	public:
		...
		TArrayView<const FInstanceSceneDataBuffers::FCompressedSpatialHashItem> Items;
		uint32 GetNumWords() const { return FMath::DivideAndRoundUp(NumItems, uint32(NumBitsPerDWORD));}
	};

	/**
	 * Scene culling info for a given primitive for CPU-side culling.
	 */
	FPrimitiveCullingInfo* GetCullingInfo(FPersistentPrimitiveIndex PersistentPrimitiveIndex)
	{
		return CPUCullingInfos.Find(PersistentPrimitiveIndex);			
	}
private:

	// Returns the number of bits required to store the state for all hierarchical culling ranges.
	int32 GetMaxHashGroupVisibilityOffset() const { return MaxHashGroupVisibilityWord * NumBitsPerDWORD; }

	// Returns the number of bits required to store the state for all hierarchical culling ranges.
	int32 GetMaxHashGroupLODOffset() const { return MaxHashGroupLODOffset; }

	bool IsSmallCullingVolume(const FCullingVolume& CullingVolume) const;

	void FinalizeUpdateAndClear(FRDGBuilder& GraphBuilder, bool bPublishStats);
	...
	bool bHierarchicalCPUCullingEnabled = false;
	...
	// Used to allocate ranges of bits for the cpu-side instance culling state.
	int32 MaxHashGroupVisibilityWord = -1;
	int32 MaxHashGroupLODOffset = -1;
	Experimental::TRobinHoodHashMap<FPersistentPrimitiveIndex, FPrimitiveCullingInfo> CPUCullingInfos;
};
