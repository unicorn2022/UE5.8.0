...
static TAutoConsoleVariable<bool> CVarSceneCullingHierarchicalCPUCulling(
	TEXT("r.SceneCulling.HierarchicalCPUCulling"), 
	true, 
	TEXT("Use to enable hierarchical CPU culling, which works for instanced things that have pre computed instance bounds."),
...
static TAutoConsoleVariable<bool> CVarSceneCullingHierarchicalCPUCullingProjectEnabled(
	TEXT("r.SceneCulling.HierarchicalCPUCulling.ProjectEnabled"), 
	true, 
	TEXT("Read-only switch to turn off the CPU culling machinery for the project, allows the whole scene extension to be disabled for non-nanite platforms."),
	ECVF_RenderThreadSafe | ECVF_ReadOnly);
...
#define BUILDER_LOG_SCOPE(Fmt, ...) if (GBuilderForLogging) { GBuilderForLogging->AddLog(FString::Printf(TEXT(Fmt), ##__VA_ARGS__)); } FIHLoggerScopeHelper UE_JOIN(IHLoggerScopeHelper, __LINE__)
...
static bool UseHierarchicalCPUCulling()
{
	return CVarSceneCullingHierarchicalCPUCulling.GetValueOnAnyThread() && CVarSceneCullingHierarchicalCPUCullingProjectEnabled.GetValueOnAnyThread();
}

bool FSceneCulling::ShouldCreateExtension(FScene& Scene)
{
	// If CPU culling is enabled for the project, we must always create the extension such that we can toggle at runtime, since we don't track if ShouldCreateExtension changes.
	return CVarSceneCullingHierarchicalCPUCullingProjectEnabled.GetValueOnAnyThread() || DoesPlatformSupportNanite(GetFeatureLevelShaderPlatform(Scene.GetFeatureLevel()));
}
...
/**
 * Clamp the level to the provided minimum. That is if the level of Loc is below MinLevel change it to MinLevel but not if already above.
 */
inline FSceneCulling::FLocation64 ClampToMinLevel(const FSceneCulling::FLocation64& Loc, int32 MinLevel)
{
	if (Loc.Level < MinLevel)
	{
		return RenderingSpatialHash::ToLevel(Loc, MinLevel);
	}
	return Loc;
};
...
public:
		...
		, SpatialHashMap(InSceneCulling.SpatialHash.GetHashMap())
	{
		...
		AddLog(FString("Log-Scope-Begin - ") + SceneTag);
		LogIndent(1);
#endif
	}
			...
			for (; ChunkOffset < EndChunkOffset && NumToRemove > 0; ++ChunkOffset)
			{
				...
				else
				{
					...
					else
					{
						...
						BUILDER_LOG_LIST("Retained(%d):", FMath::CountBits64(RetainedMask));
	...
	template <typename HashCallbackFuncType>
	void ProcessPrecomputed(const TArray<FInstanceSceneDataBuffers::FCompressedSpatialHashItem>& Hashes, int32 InstanceDataOffset, int32 MinLevel, HashCallbackFuncType HashCallbackFunc)
	{
		// This should be guaranteed by InstanceDataFlags.bHasCompressedSpatialHash never being true if there are no hashes.
		check(!Hashes.IsEmpty());
		int32 InstanceDataOffsetCurrent = InstanceDataOffset;
		int32 NumInstances = 0;
		RenderingSpatialHash::FLocation64 CurrentLoc = ClampToMinLevel(Hashes[0].Location, MinLevel);
		// Bias the the hash to the current min level and add all consecutive now-identical hashes 
		for (const FInstanceSceneDataBuffers::FCompressedSpatialHashItem& Item : Hashes)
		{
			RenderingSpatialHash::FLocation64 HML = ClampToMinLevel(Item.Location, MinLevel);

			if (HML != CurrentLoc)
			{
				HashCallbackFunc(CurrentLoc, InstanceDataOffsetCurrent, NumInstances);

				CurrentLoc = HML;
				InstanceDataOffsetCurrent += NumInstances;
				NumInstances = 0;
	...
	SC_FORCEINLINE void AddPrecomputed(FPrimitiveState& NewPrimitiveState, FPersistentPrimitiveIndex PersistentPrimitiveId, int32 InstanceDataOffset, TSharedPtr<FInstanceSceneDataImmutable, ESPMode::ThreadSafe>&& InstanceSceneDataImmutable)
	{
		BUILDER_LOG_LIST("Add AddPrecomputed");
		const TArray<FInstanceSceneDataBuffers::FCompressedSpatialHashItem>& Hashes = InstanceSceneDataImmutable->GetCompressedInstanceSpatialHashes();
		ProcessPrecomputed(Hashes, InstanceDataOffset, SceneCulling.SpatialHash.GetFirstLevel(), 
			[&](const RenderingSpatialHash::FLocation64& CurrentLoc, int32 InstanceDataOffsetCurrent, int32 NumInstances)
		{
			int32 CellIndex = AddRange(CurrentLoc, InstanceDataOffsetCurrent, NumInstances, EUpdateFrequencyCategory::Static);
			BUILDER_LOG_LIST_APPEND("(%d, %d, %d)", CellIndex, InstanceDataOffsetCurrent, NumInstances);
		});
	...
	SC_FORCEINLINE void AddInstances(FPersistentPrimitiveIndex PersistentId, FPrimitiveSceneInfo* PrimitiveSceneInfo)
	{
		...
		switch(NewPrimitiveState.State)
		{
			...
			case FPrimitiveState::Precomputed:
			{
				...
				AddPrecomputed(NewPrimitiveState, PersistentId, InstanceDataOffset, MoveTemp(InstanceSceneDataImmutable));
			}
			break;
	...
	inline void RemovePrecomputed(FSceneCulling::FPrimitiveState& PrimitiveState, FPersistentPrimitiveIndex PersistentPrimitiveId)
	{
		BUILDER_LOG("FPrimitiveState::Precomputed");
		check(PrimitiveState.InstanceSceneDataImmutable);

		BUILDER_LOG_LIST("RemovePrecomputed");
		ProcessPrecomputed(PrimitiveState.InstanceSceneDataImmutable->GetCompressedInstanceSpatialHashes(), 0/*unused*/, SceneCulling.SpatialHash.GetFirstLevel(), 
			[&](const RenderingSpatialHash::FLocation64& CurrentLoc, int32 /*InstanceDataOffsetCurrent*/, int32 NumInstances)
		{
			int32 CellIndex = GetCellIndex(CurrentLoc);
			BUILDER_LOG_LIST_APPEND("(%d, %d)", CellIndex, NumInstances);
			MarkCellForRemove(CellIndex, NumInstances, EUpdateFrequencyCategory::Static);
		});
	}
	...
	inline void MarkInstancesForRemoval(FPersistentPrimitiveIndex PersistentPrimitiveIndex, FPrimitiveSceneInfo *PrimitiveSceneInfo)
	{
		...
		{
			RemovePrecomputed(PrimitiveState, PersistentPrimitiveIndex);
		}
	...
	SC_FORCEINLINE void UpdateInstances(FPersistentPrimitiveIndex PersistentPrimitiveIndex, FPrimitiveSceneInfo* PrimitiveSceneInfo)
	{
		...
		else if (PrevPrimitiveState.State == FPrimitiveState::Precomputed)
		{
			...
			RemovePrecomputed(PrevPrimitiveState, PersistentPrimitiveIndex);
		...
		if (bNeedsAdd)
		{
			...
			switch(NewPrimitiveState.State)
			{
				...
				case FPrimitiveState::Precomputed:
				{
					...
					SC_SCOPED_NAMED_EVENT_DETAIL(SceneCulling_Post_AddInstances_Precomputed, FColor::Emerald);
					TSharedPtr<FInstanceSceneDataImmutable, ESPMode::ThreadSafe> InstanceSceneDataImmutable = InstanceSceneDataBuffers->GetImmutable();
					AddPrecomputed(NewPrimitiveState, PersistentPrimitiveIndex, InstanceDataOffset, MoveTemp(InstanceSceneDataImmutable));
				}
				break;
				case FPrimitiveState::UnCullable:
				{
...

		{
			...
			FRDGBuffer* ExplicitChunkBoundsBufferRDG = nullptr;
			{
				...
				if (!DirtyChunkBoundsData.IsEmpty())
				{
					...
					PassParameters->GPUSceneParameters = SceneCulling.Scene.GPUScene.GetShaderParameters(GraphBuilder);
					...
					auto ComputeShader = GetGlobalShaderMap(SceneCulling.Scene.GetShaderPlatform())->GetShader<FComputeExplicitChunkBounds_CS>();
		...
		if (CVarValidateGPUData.GetValueOnRenderThread() != 0)
		{
			...
			SceneCulling.InstanceIdsBuffer.ValidateGPUData(GraphBuilder, TConstArrayView<const uint32>(SceneCulling.PackedCellData), 
				[this, FreeChunkBits = MoveTemp(FreeChunkBits)](int32 Index, int32 HostValue, int32 GPUValue) 
				{
					...
					{
						UE_LOGF(LogTemp, Error, "PackedCellData[%d] != InstanceIdsBuffer[%d], %d, %d", Index, Index, HostValue, GPUValue);
					}
...
public:
	...
	inline void AddLog(const FString &Item)
	{
		if (bIsLoggingEnabled)
		{
			...
			UE_LOGF(LogTemp, Log, "%ls%ls", *Indent, *Item);
		}
	}
...
FSceneCulling::FSceneCulling(FScene& InScene)
	...
	, bHierarchicalCPUCullingEnabled(UseHierarchicalCPUCulling())
...
{	
	...
	{
		FString ShaderPlatformString = LexToString(Scene.GetShaderPlatform());
		FString SceneWorldNameString = Scene.GetFullWorldName();
		UE_LOGF(LogRenderer, Log, "SceneCulling instance hierarchy is disabled as UseNanite(%ls) returned false, for scene: '%ls'.", *ShaderPlatformString, *SceneWorldNameString);
	}
#endif
...
void FSceneCulling::FUpdater::PreSceneUpdate(FRDGBuilder& GraphBuilder, const FScenePreUpdateChangeSet& ChangeSet)
{
	LLM_SCOPE_BYTAG(SceneCulling);

	SceneCulling.FinalizeUpdateAndClear(GraphBuilder, false);

	if (SceneCulling.bHierarchicalCPUCullingEnabled)
	{
		// TODO, run async?
		bool bWasAnyRemoved = false;
		for (FPersistentPrimitiveIndex Id : ChangeSet.RemovedPrimitiveIds)
		{
			bWasAnyRemoved = SceneCulling.CPUCullingInfos.Remove(Id) || bWasAnyRemoved;

		}
		if (bWasAnyRemoved)
		{
			// Reset to trigger recompute
			SceneCulling.MaxHashGroupVisibilityWord = -1;
			SceneCulling.MaxHashGroupLODOffset = -1;
		}	
	}
...
void FSceneCulling::FUpdater::PostSceneUpdate(FRDGBuilder& GraphBuilder, const FScenePostUpdateChangeSet& ChangeSet)
{
	...
	auto AddOrUpdateHierarchicalCPUCUlling = [&](FPersistentPrimitiveIndex PersistentPrimitiveId, FPrimitiveSceneInfo* PrimitiveSceneInfo) -> bool
	{
		if (PrimitiveSceneInfo->GetInstanceDataHeader().Flags.bHasCompressedSpatialHash && PrimitiveSceneInfo->GpuLodInstanceRadius > 0.0f && !PrimitiveSceneInfo->Proxy->IsNaniteMesh())
		{
			// Note: this causes stall, can we move precomputed ref to Header?
			if (const FInstanceSceneDataBuffers* InstanceSceneDataBuffers = PrimitiveSceneInfo->GetInstanceSceneDataBuffers())
			{
				if (TSharedPtr<FInstanceSceneDataImmutable, ESPMode::ThreadSafe> InstanceSceneDataImmutable = InstanceSceneDataBuffers->GetImmutable())
				{
					const TArray<FInstanceSceneDataBuffers::FCompressedSpatialHashItem>& Hashes = InstanceSceneDataImmutable->GetCompressedInstanceSpatialHashes();
					FPrimitiveCullingInfo* PrimitiveCullingInfo = SceneCulling.CPUCullingInfos.FindOrAdd(PersistentPrimitiveId, FPrimitiveCullingInfo{});

					// Maintain reference to avoid keep Hashes alive
					PrimitiveCullingInfo->InstanceSceneDataImmutable = MoveTemp(InstanceSceneDataImmutable);
					PrimitiveCullingInfo->NumItems = Hashes.Num();
					PrimitiveCullingInfo->HashGroupVisibilityOffset = -1;
					PrimitiveCullingInfo->PersistentPrimitiveId = PersistentPrimitiveId;
					PrimitiveCullingInfo->Items = PrimitiveCullingInfo->InstanceSceneDataImmutable->GetCompressedInstanceSpatialHashes();
					
					FVector2f MinMaxDrawDistance;
					if (PrimitiveSceneInfo->Proxy->GetInstanceDrawDistanceMinMax(MinMaxDrawDistance))
					{
						PrimitiveCullingInfo->MaxInstanceDrawDistance = MinMaxDrawDistance.Y;
					}
	...
	bool bNewHierarchicalCPUCullingEnabled = UseHierarchicalCPUCulling();
	if (bNewHierarchicalCPUCullingEnabled && !SceneCulling.bHierarchicalCPUCullingEnabled)
	{
		// Make sure we are starting from empty
		check(SceneCulling.MaxHashGroupVisibilityWord == -1);
		check(SceneCulling.MaxHashGroupLODOffset == -1);
		check(SceneCulling.CPUCullingInfos.Num() == 0);

		for (FPrimitiveSceneInfo* PrimitiveSceneInfo : SceneCulling.Scene.Primitives)
		{
			AddOrUpdateHierarchicalCPUCUlling(PrimitiveSceneInfo->GetPersistentIndex(), PrimitiveSceneInfo);
		}
		SceneCulling.bHierarchicalCPUCullingEnabled = true;
	}
	else if (!bNewHierarchicalCPUCullingEnabled && SceneCulling.bHierarchicalCPUCullingEnabled)
	{
		// reset everything when it is turned off.
		SceneCulling.MaxHashGroupVisibilityWord = -1;
		SceneCulling.MaxHashGroupLODOffset = -1;
		SceneCulling.CPUCullingInfos.Empty();
		SceneCulling.bHierarchicalCPUCullingEnabled = false;
	}
	else if (SceneCulling.bHierarchicalCPUCullingEnabled)
	{
		// TODO, run async?
		ChangeSet.PrimitiveUpdates.ForEachUpdateCommand(ESceneUpdateCommandFilter::AddedUpdated, EPrimitiveUpdateDirtyFlags::All, [&](const FPrimitiveUpdateCommand& Cmd)
		{
			FPersistentPrimitiveIndex PersistentPrimitiveId = Cmd.GetPersistentId();
			FPrimitiveSceneInfo* PrimitiveSceneInfo = Cmd.GetSceneInfo();
			bool bWasAddedOrUpdated = false;
			if (!PrimitiveSceneInfo->Proxy->IsNaniteMesh())
			{
				bWasAddedOrUpdated = AddOrUpdateHierarchicalCPUCUlling(PersistentPrimitiveId, PrimitiveSceneInfo);
				// It was an update, but it no longer is a valid (e.g., spatial hashes were removed)
				if (!bWasAddedOrUpdated && !Cmd.IsAdd())
				{
					SceneCulling.CPUCullingInfos.Remove(PersistentPrimitiveId);
				}
			}
			// Make sure we're not leaving any that are not supposed to have an entry in the map
			check(bWasAddedOrUpdated || !SceneCulling.CPUCullingInfos.Find(PersistentPrimitiveId));
		});

	}
	
	// Derived state, regardless of previous state
	if (SceneCulling.bHierarchicalCPUCullingEnabled)
	{
		// Something has changed wrt the allocation of the bits in the bit vector, recompute the offsets as needed
		if (SceneCulling.MaxHashGroupVisibilityWord < 0)
		{
			uint32 Offset = 0u;
			uint32 LODOffset = 0u;
			for (auto& CPUCullingInfoItem : SceneCulling.CPUCullingInfos)
			{
				FPrimitiveCullingInfo& PrimitiveCullingInfo = CPUCullingInfoItem.Value;
				PrimitiveCullingInfo.HashGroupVisibilityOffset = Offset;
				PrimitiveCullingInfo.HashGroupLODOffset = LODOffset;
				Offset += PrimitiveCullingInfo.GetNumWords();
				LODOffset += PrimitiveCullingInfo.NumItems;
			}
			SceneCulling.MaxHashGroupVisibilityWord = Offset;
			SceneCulling.MaxHashGroupLODOffset = LODOffset;
		}
	}
...
void FSceneCulling::FinalizeUpdateAndClear(FRDGBuilder& GraphBuilder, bool bPublishStats)
{
	LLM_SCOPE_BYTAG(SceneCulling);
	...
	if (ActiveUpdaterImplementation.IsValid())
	{
		...
		ActiveUpdaterImplementation->UploadToGPU(GraphBuilder);
...
void FSceneCulling::EndUpdate(FRDGBuilder& GraphBuilder, bool bPublishStats)
{
	if (bIsEnabled)
	{
		FinalizeUpdateAndClear(GraphBuilder, bPublishStats);
#if DO_CHECK
