...
bool IsDynamicInstancingEnabled(ERHIFeatureLevel::Type FeatureLevel)
{
	return CVarMeshDrawCommandsDynamicInstancing.GetValueOnRenderThread() > 0
		&& UseGPUScene(GShaderPlatformForFeatureLevel[FeatureLevel]);
}
			...
			if (!View.bIsReflectionCapture && !View.bIsSceneCapture )
			{
				...
				if ((GAreScreenMessagesEnabled && !GEngine->bSuppressMapWarnings) && bViewHasWarnings)
				{
						...
						(FCanvas& Canvas)
					{
						...
						{
							FString NaniteError = "Nanite is used in the scene but not supported by your graphics hardware and/or driver. Meshes will not render using Nanite.";
