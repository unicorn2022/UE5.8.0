...
bool AreDecalsEnabled(const FViewInfo& View)
{
	const FSceneViewFamily& ViewFamily = *View.Family;
	return ViewFamily.EngineShowFlags.Decals && !ViewFamily.EngineShowFlags.VisualizeLightCulling && !ViewFamily.EngineShowFlags.HitProxies &&
		(!View.CustomRenderPass || View.CustomRenderPass->GetRenderMode() == FCustomRenderPassBase::ERenderMode::DepthAndBasePass);
}

bool IsDBufferEnabled(const FSceneViewFamily& ViewFamily, EShaderPlatform ShaderPlatform)
{
	return IsUsingDBuffers(ShaderPlatform) && !ViewFamily.EngineShowFlags.ShaderComplexity;
}
	...
	EDecalRenderStage DecalRenderStage)
{
	...
	{
		Substrate::BindSubstratePublicGlobalUniformParameters(GraphBuilder, SceneTextures.SubstrateSceneData.Get(), Parameters->SubstratePublic);
	}
	else
	{
	...
	if (!bIsMobile)
	{
		PassTextures.GBufferA = SceneTextures.GBufferA;
		PassTextures.GBufferB = SceneTextures.GBufferB;
		PassTextures.GBufferC = SceneTextures.GBufferC;
		PassTextures.GBufferE = SceneTextures.GBufferE;
	}
	...
	FDeferredDecalPassParameters& PassParameters)
{
	...
	case EDecalRenderTargetMode::DBuffer:
	{
		...
		FDBufferTextures& DBufferTextures = *Textures.DBufferTextures;

		const bool bUseTextureArrays = View.UsesMobileMultiViewShaders();
		const ERenderTargetLoadAction LoadAction = (bUseTextureArrays ? DBufferTextures.bDBufferTexArrayProduced : DBufferTextures.bDBufferProduced)
			? ERenderTargetLoadAction::ELoad
			: ERenderTargetLoadAction::EClear;

		if (bUseTextureArrays)
		{
			if (DBufferTextures.DBufferATexArray)
			{
				DBufferTextures.bDBufferTexArrayProduced = true;
			}
		}
		else
		{
			if (DBufferTextures.DBufferA)
			{
				DBufferTextures.bDBufferProduced = true;
			}
		}
		...
		const EDecalDBufferMaskTechnique DBufferMaskTechnique = GetDBufferMaskTechnique(View.Family->GetShaderPlatform());
	...
	TArray<FPSOPrecacheData>& PSOInitializers)
{
	...
	EDecalRenderTargetMode DecalRenderTargetMode = DecalRendering::GetRenderTargetMode(ShaderPlatform, DecalBlendDesc, DecalRenderStage);
	...
	if (!DecalRendering::SetupShaderState(FeatureLevel, Material, DecalRenderStage, GraphicsPSOInit.BoundShaderState, false))
	{
		return;
	}
	...
	EDecalRenderStage DecalRenderStage)
{
	...
	if (bVisibleDecalsInView)
	{
		...
		if (SortedDecalCount > 0)
		{
			...
			EDecalRenderTargetMode LastRenderTargetMode = DecalRendering::GetRenderTargetMode(ShaderPlatform, SortedDecals[0]->BlendDesc, DecalRenderStage);

			for (; SortedDecalIndex < SortedDecalCount; ++SortedDecalIndex, ++NumDecals)
			{
				const EDecalRenderTargetMode RenderTargetMode = DecalRendering::GetRenderTargetMode(ShaderPlatform, SortedDecals[SortedDecalIndex]->BlendDesc, DecalRenderStage);
