...
bool ShouldRenderScreenSpaceAmbientOcclusion(const FViewInfo& View, bool bLumenWantsSSAO)
{
	...
	bEnabled = View.FinalPostProcessSettings.AmbientOcclusionIntensity > 0
		...
		&& (FSSAOHelper::GetNumAmbientOcclusionLevels() != 0)
...
{
	return FSSAOHelper::IsAmbientOcclusionCompute() ? ESSAOType::ECS : ESSAOType::EPS;
}
...
FSSAOCommonParameters GetSSAOCommonParameters(
	...
	bool bAllowGBufferRead)
{
	...
	CommonParameters.SceneTextures = GetSceneTextureShaderParameters(View);
...
{
	const int32 DecalViewOffset = DecalVisibility ? DecalVisibility->GetViewOffset(Views) : INDEX_NONE;

	if (DecalViewOffset != INDEX_NONE)
	{
		for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
		{
			DecalVisibility->Finish(DecalViewOffset, ViewIndex);
		}
	}
}
...
void FCompositionLighting::ProcessBeforeBasePass(FRDGBuilder& GraphBuilder, FDBufferTextures& DBufferTextures, FInstanceCullingManager& InstanceCullingManager, const FSubstrateSceneData& SubstrateSceneData)
{
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
	{
		...
		const bool bEnableDecals = bRunDbufferBeforeBasePass && DecalViewOffset != INDEX_NONE && DecalVisibility->HasStage(DecalViewOffset, ViewIndex, EDecalRenderStage::BeforeBasePass);
		...
		if (bEnableSSAO)
		{
			FSSAOCommonParameters Parameters = GetSSAOCommonParameters(GraphBuilder, View, ViewConfig.Levels, false);
...
void FCompositionLighting::ProcessAfterBasePass(FRDGBuilder& GraphBuilder, FInstanceCullingManager& InstanceCullingManager, EProcessAfterBasePassMode Mode, const FSubstrateSceneData& SubstrateSceneData)
{
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
	{
		...
		{
			AddDeferredDecalPass(GraphBuilder, View, DecalVisibility->FinishRelevantDecals(DecalViewOffset, ViewIndex, Stage), DecalPassTextures, InstanceCullingManager, Stage);
		};

		if (DecalViewOffset != INDEX_NONE)
		{
			if (DecalVisibility->HasStage(DecalViewOffset, ViewIndex, EDecalRenderStage::BeforeLighting) && Mode != EProcessAfterBasePassMode::SkipBeforeLightingDecals)
			{
				// We can disable this pass if using DBuffer decals
				// Decals are before AmbientOcclusion so the decal can output a normal that AO is affected by
				FDeferredDecalPassTextures DecalPassTextures = GetDeferredDecalPassTextures(GraphBuilder, View, SceneTextures, nullptr, EDecalRenderStage::BeforeLighting);
				AddDeferredDecalPassLambda(DecalPassTextures, EDecalRenderStage::BeforeLighting);
			}

			if (DecalVisibility->HasStage(DecalViewOffset, ViewIndex, EDecalRenderStage::Emissive) && Mode != EProcessAfterBasePassMode::OnlyBeforeLightingDecals)
			{
				// DBuffer decals with emissive component
				FDeferredDecalPassTextures DecalPassTextures = GetDeferredDecalPassTextures(GraphBuilder, View, SceneTextures, nullptr, EDecalRenderStage::Emissive);
		...
		if (!IsForwardShadingEnabled(View.GetShaderPlatform()) && Mode != EProcessAfterBasePassMode::OnlyBeforeLightingDecals)
		{
			if (ViewConfig.Levels > 0)
			{
				...
				if (ViewConfig.GTAOType == EGTAOType::EAsyncHorizonSearch || ViewConfig.GTAOType == EGTAOType::EAsyncCombinedSpatial)
				{
					...
					FGTAOCommonParameters Parameters = GetGTAOCommonParameters(GraphBuilder, View, ViewConfig.GTAOType);
					...
					ensureMsgf(
						DecalViewOffset == INDEX_NONE || !DecalVisibility->HasStage(DecalViewOffset, ViewIndex, EDecalRenderStage::AmbientOcclusion),
				...
				else
				{
					...
					{
						FSSAOCommonParameters Parameters = GetSSAOCommonParameters(GraphBuilder, View, ViewConfig.Levels, true);
						AmbientOcclusion = AddPostProcessingAmbientOcclusion(GraphBuilder, View, Parameters, FinalTarget);
					}

					if (DecalViewOffset != INDEX_NONE)
					{
						FDeferredDecalPassTextures DecalPassTextures = GetDeferredDecalPassTextures(GraphBuilder, View, SceneTextures, nullptr, EDecalRenderStage::AmbientOcclusion);
...
void FCompositionLighting::ProcessAfterOcclusion(FRDGBuilder& GraphBuilder)
{
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
	{
		...
		if (ViewConfig.GTAOType == EGTAOType::EAsyncCombinedSpatial || ViewConfig.GTAOType == EGTAOType::EAsyncHorizonSearch)
		{
			...
			FGTAOCommonParameters CommonParameters = GetGTAOCommonParameters(GraphBuilder, View, ViewConfig.GTAOType);
