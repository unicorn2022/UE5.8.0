...
static TAutoConsoleVariable<int32> CVarAmbientOcclusionFramebufferCompression(
	TEXT("r.AmbientOcclusion.FramebufferCompression"),
	0,
	TEXT("Whether to use framebuffer compression for AO texture if platform supports it.\n"),
	ECVF_RenderThreadSafe
);
...
bool FSSAOHelper::IsAmbientOcclusionCompute()
{
	return CVarAmbientOcclusionCompute.GetValueOnAnyThread() >= 1;
}
...
bool FSSAOHelper::IsAmbientOcclusionAsyncCompute(const FViewInfo& View, uint32 AOPassCount)
{
	...
	if(IsAmbientOcclusionCompute() && (AOPassCount == 1))
	{
...
uint32 FSSAOHelper::ComputeAmbientOcclusionPassCount(const FViewInfo& View)
{
...

	{
		...
		if (IsAmbientOcclusionCompute() || IsForwardShadingEnabled(View.GetShaderPlatform()))
		{	
			if (CVarLevel<0)
			{
				CVarLevel = 1;
...
bool FSSAOHelper::IsDepthBoundsTestEnabled()
{
	return GSupportsDepthBoundsTest && CVarAmbientOcclusionDepthBoundsTest.GetValueOnRenderThread() != 0;
}
	...
	EAOTechnique AOTechnique)
{
	...
	const FIntPoint RandomizationSize = GSystemTextures.SSAORandomization->GetDesc().Extent * InputViewport.Extent / OutputViewport.Extent;
...
class FAmbientOcclusionSmoothPS : public FGlobalShader
{
public:
	DECLARE_GLOBAL_SHADER(FAmbientOcclusionSmoothPS);
	SHADER_USE_PARAMETER_STRUCT(FAmbientOcclusionSmoothPS, FGlobalShader);
...
IMPLEMENT_GLOBAL_SHADER(FAmbientOcclusionSmoothPS, "/Engine/Private/PostProcessAmbientOcclusion.usf", "MainSSAOSmoothPS", SF_Pixel);
	...
	FScreenPassRenderTarget Output)
{
	...
	if (SSAOType == ESSAOType::EPS)
	{
		FAmbientOcclusionSmoothPS::FParameters* PassParameters = GraphBuilder.AllocParameters<FAmbientOcclusionSmoothPS::FParameters>();
		...
		TShaderMapRef<FAmbientOcclusionSmoothPS> PixelShader(View.ShaderMap);
...
BEGIN_SHADER_PARAMETER_STRUCT(FAmbientOcclusionParameters, )
	...
	SHADER_PARAMETER(uint32, SSAO_ShadingModelsExcludeMask)
...
struct FAmbientOcclusionPermutations
{
	...
	class FUseSMExcludeMaskDim	: SHADER_PERMUTATION_BOOL("USE_SM_EXCLUDE_MASK");
	class FShaderQualityDim		: SHADER_PERMUTATION_INT("SHADER_QUALITY", 5);

	using FPermutationDomain = TShaderPermutationDomain<FUseUpsampleDim, FUseAoSetupAsInputDim, FUseSMExcludeMaskDim, FOutputDepthDim, FShaderQualityDim>;

	static bool ShouldCompilePermutation(const FGlobalShaderPermutationParameters& Parameters, EShaderFrequency Frequency)
	{
		...
		if (!IsMobilePlatform(Parameters.Platform))
		{
			...
			if (PermutationVector.Get<FUseSMExcludeMaskDim>() || PermutationVector.Get<FOutputDepthDim>())
			{
				return false;
			}
		}

		// SM mask requires access to GBuffer, only pixel shader version can do it
		const bool bSMAccess = !IsMobilePlatform(Parameters.Platform) || (IsMobileDeferredShadingEnabled(Parameters.Platform) && MobileUsesGBufferCustomData(Parameters.Platform));
		if (PermutationVector.Get<FUseSMExcludeMaskDim>() && (Frequency != SF_Pixel || !bSMAccess))
		{
			return false;
		}

		// OUTPUT_DEPTH is only used in a specific setup with a bilateral upsample
		if (PermutationVector.Get<FOutputDepthDim>() &&
			(PermutationVector.Get<FUseUpsampleDim>() || PermutationVector.Get<FUseAoSetupAsInputDim>()))
		{
			return false;
		}
	...
	static FPermutationDomain BuildPermutationVector(bool bDoUpsample, bool bAOSetupAsInput, int32 ShaderQuality, bool bOutputDepth, uint32 SMExcludeMask)
	{
		FPermutationDomain PermutationVector;
		PermutationVector.Set<FUseUpsampleDim>(bDoUpsample);
		PermutationVector.Set<FUseAoSetupAsInputDim>(bAOSetupAsInput);
		PermutationVector.Set<FShaderQualityDim>(ShaderQuality);
		PermutationVector.Set<FOutputDepthDim>(bOutputDepth);
		PermutationVector.Set<FUseSMExcludeMaskDim>(SMExcludeMask != 0);
...
public:
	...
	using FPermutationDomain = FAmbientOcclusionPermutations::FPermutationDomain;
	...
	{
		return FAmbientOcclusionPermutations::ShouldCompilePermutation(Parameters, SF_Pixel);
	}
...
public:
	...
	static bool ShouldCompilePermutation(const FGlobalShaderPermutationParameters& Parameters)
	{
		return FAmbientOcclusionPermutations::ShouldCompilePermutation(Parameters, SF_Compute);
	}
	...
	EPixelFormat IntermediateFormatOverride)
{
	...
	if (UE::PixelFormat::HasCapabilities(OutputDesc.Format, EPixelFormatCapabilities::LossyCompressible) 
		&& CVarAmbientOcclusionFramebufferCompression.GetValueOnRenderThread() != 0)
	{
		OutputDesc.Flags |= TexCreate_LossyCompressionLowBitrate;
	}
	...
	bool bAOSetupAsInput)
{
	...
	bool bDoUpsample = DownsampledAO.IsValid();
	if (CommonParameters.MobileHalfResolutionSetting > 0)
	{
		// these are not supported with half-res SSAO
		bDoUpsample = false;
		bAOSetupAsInput = false;
	}
	
	FAmbientOcclusionParameters SharedParameters;
	SharedParameters.View = View.GetShaderParameters();
	SharedParameters.SceneTextures = CommonParameters.SceneTextures;
	...
	if (DownsampledAO.IsValid())
	{
		...
		SharedParameters.SSAO_DownsampledAOUVViewportMin = (ViewportMin + 0.5f) * SharedParameters.SSAO_DownsampledAOInverseSize;
		SharedParameters.SSAO_DownsampledAOUVViewportMax = (ViewportMax - 0.5f) * SharedParameters.SSAO_DownsampledAOInverseSize;
	}
	else
	{
	...
	{
		// Downscaled 
		SharedParameters.SSAO_SvPositionScaleBias = FVector2f(2, -0.5);
	}
	else
	{
	...
	SharedParameters.SSAO_ShadingModelsExcludeMask = CommonParameters.MobileShadingModelsExcludeMask;
	...
	if (AOType == ESSAOType::ECS || AOType == ESSAOType::EAsyncCS)
	{
		...
		FAmbientOcclusionCS::FPermutationDomain PermutationVector = FAmbientOcclusionPermutations::BuildPermutationVector(
			bDoUpsample, 
			bAOSetupAsInput, 
			CommonParameters.ShaderQuality, 
			CommonParameters.MobileHalfResolutionSetting > 1,
			0// SMExcludeMask
		);
	...
	{
		const bool bFullRes = (CommonParameters.SceneTexturesViewport == OutputViewport);
		...
		FAmbientOcclusionPS::FPermutationDomain PermutationVector = FAmbientOcclusionPermutations::BuildPermutationVector(
			bDoUpsample, 
			bAOSetupAsInput, 
			CommonParameters.ShaderQuality, 
			CommonParameters.MobileHalfResolutionSetting > 1,
			CommonParameters.MobileShadingModelsExcludeMask
		);
