...
public:
	 ...
	 */
	bool HasVirtualShadowMap(FLightSceneId LightId) const { return CommonSetups[LightId].bHasVirtualShadowMap; }

	/** 
	 * returns true if the given light has a clipmap VSM set up for any view
	 */
	bool HasVirtualClipMap(FLightSceneId LightId) const { return CommonSetups[LightId].bHasVirtualShadowMap && CommonSetups[LightId].bIsDirectional; }
	...
	TArray<FRDGBufferRef> HairTransmittanceMaskBits;
...
private:
	...
	struct FDirectionalLightShadowFrameSetup
	{
		...
		FDirectionalLightShadowFrameSetup(FLightSceneId InLightId, uint32 InViewMask) 
			: LightId(InLightId)
			, ViewMask(InViewMask)
		{
		}
	...
	FDirectionalLightShadowFrameSetup* FindDirectional(const FLightSceneId LightId, int32 ViewIndex);
