...
void FShadowScene::FUpdater::PostLightsUpdate(FRDGBuilder& GraphBuilder, const FLightSceneChangeSet& LightSceneChangeSet)
{
	...
	if (WorkSize > 0)
	{
		...
		ShadowScene.SceneChangeUpdateTask = GraphBuilder.AddSetupTask([this, LightSceneChangeSet]
		{
			...
			auto UpdateLight = [&](FLightSceneId Id)
			{
			...
			for (FLightSceneId LightId : LightSceneChangeSet.AddedLightIds)
			{ 
...
void FShadowScene::FUpdater::PreSceneUpdate(FRDGBuilder& GraphBuilder, const FScenePreUpdateChangeSet& ChangeSet)
{
...
void FShadowScene::UpdateForRenderedFrame(FRDGBuilder& GraphBuilder)
{
	...
	for (FSceneLightBitArray::FIterator BitIt(ActiveLights); BitIt; ++BitIt)
	{
		FLightSceneId Id = BitIt.GetIndex();
...
float FShadowScene::GetLightMobilityFactor(FLightSceneId LightId) const 
{ 
	SceneChangeUpdateTask.Wait();
