...
static void UpdateGlobalDistanceFieldViewOrigin(const FViewInfo& View, bool bLumenEnabled)
{
	...
	if (View.ViewState && (View.ViewState->GlobalDistanceFieldData->bFirstFrame || View.ViewState->GlobalDistanceFieldData->UpdateFrame != View.Family->FrameNumber))
	{
		...
		{
			CameraVelocityOffset = FVector(0.0f, 0.0f, 0.0f);
		}
	}
}

void UpdateGlobalDistanceFieldViewOrigin(const FSceneRenderUpdateInputs& SceneUpdateInputs)
{
	SceneUpdateInputs.ForEachView([&](const FSceneRenderer* Renderer, FViewInfo& View)
		{
			...
			UpdateGlobalDistanceFieldViewOrigin(View, bUseLumenGlobalDistanceFieldSettings);
			return true;
		});
}
...
class FCullObjectsToClipmapCS : public FGlobalShader
{
	...
	{
		return ShouldCompileDistanceFieldAOShaders(Parameters.Platform);
	}
...
class FGlobalDistanceFieldDebugCS : public FGlobalShader
{
	...
	static bool ShouldCompilePermutation(const FGlobalShaderPermutationParameters& Parameters)
	{
		return ShouldCompileDistanceFieldAOShaders(Parameters.Platform) && ShaderPrint::IsSupported(Parameters.Platform);
	}
...
void UpdateGlobalDistanceFieldVolume(
	...
	FGlobalDistanceFieldInfo& GlobalDistanceFieldInfo)
{
	RDG_RHI_EVENT_SCOPE_STAT(GraphBuilder, GlobalDistanceFieldUpdate, "GlobalDistanceFieldUpdate");
