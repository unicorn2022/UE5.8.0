...
void FSplineMeshSceneUpdater::PostGPUSceneUpdate(FRDGBuilder& GraphBuilder)
{
	if (SceneData->NumRegisteredPrimitives() == 0)
	{
	...
	if (NeededSize > SPLINE_MESH_TEXTURE_MAX_DIMENSION)
	{
		...
		if (!SceneData->bOverflowError)
		{
			UE_LOGF(LogRenderer, Error,
				"Too many spline meshes have been registered with the scene. The spline mesh texture has grown " "to its max size (%dx%d - see r.SplineMesh.BakeToTexture.MaxDimension) and has ran out of space. " "Expect some spline meshes to render incorrectly.",
	...
	bool bForceUpdate)
{
	...
	auto ComputeShader = GetGlobalShaderMap(Scene.GetShaderPlatform())->GetShader<FSplineMeshTextureFillCS>();
