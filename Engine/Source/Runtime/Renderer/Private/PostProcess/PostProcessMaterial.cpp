...
static EMaterialCustomDepthPolicy GetMaterialCustomDepthPolicy(const FMaterialRenderProxy* MaterialRenderProxy, const FMaterial* Material)
{
	...
	if (Material->IsStencilTestEnabled() && IsPostProcessStencilTestAllowed())
	{
		...
		{
			FString AssetNameString = Material->GetAssetName();
			UE_LOGF(LogRenderer, Warning, "PostProcessMaterial %ls uses stencil test, but stencil not allocated. Set r.CustomDepth to 3 to allocate custom stencil.", *AssetNameString);
		}
		else if (MaterialRenderProxy->GetBlendableLocation(Material) == BL_SceneColorAfterTonemapping)
		{
			...
			FString AssetNameString = Material->GetAssetName();
			UE_LOGF(LogRenderer, Warning, "PostProcessMaterial %ls uses stencil test, but is set to blend After Tonemapping. This is not supported.", *AssetNameString);
		}
		else
		{
	...
	const FMaterialShaderMap* MaterialShaderMap)
{
	...
	// Substrate requires the full rendering pipeline to be initialized.
	// Skip for standalone passes running outside the normal post-process pipeline.
	if (!Inputs.bStandalonePass)
	{
	...
	PostProcessMaterialParameters->SceneDepthWithoutSingleLayerWaterTexture = GSystemTextures.GetBlackDummy(GraphBuilder);
...
{
	// Runtime reflection captures have a special case to support a singleton post processing material, even though
	// post processing isn't enabled in the show flags.
	if (View.IsRuntimeReflectionCapture())
	{
		return CVarPostProcessingDisableMaterials.GetValueOnRenderThread() == 0;
	}
...
{
	if (!Material.IsPostProcessMaterial() || !UPSOPrecacheSettingsManager::GetSettings().bPostProcessingMaterial)
	{
		return;
	}
	...
	auto AddPSOInitializer = [&](bool bManualStencilTest)
	{
		...
		FRHIVertexShader* VertexShaderRHI = VertexShader.GetVertexShader(false);
		FRHIPixelShader* PixelShaderRHI = PixelShader.GetPixelShader(false);
