...
struct FTAAPassParameters
{
	...
	// Not null if this texture should be used as the half resolution render target when bDownsample=true.
	FRDGTexture* DownsampleRenderTarget = nullptr;
	// Mip level to write to when writing to DownsampleRenderTarget.
	uint8 DownsampleRenderTargetMipLevel = 0;
	...
	// Optional override for stencil source (when depth has been swapped but stencil lives elsewhere, e.g., translucency early velocity pass)
	FRDGTexture* StencilDepthTexture = nullptr;
...
/** Returns whether TSR needs the bits set by translucent clipped depth pass*/
...
struct FTSRPassConfig
{
	...
	bool  MotionBlurValidInCurrentFrame = true;
...
UE::Renderer::Private::IMegaFilters::FOutputs AddMegaFiltersPass(
	FRDGBuilder& GraphBuilder, 
	const FViewInfo& View,
	const TCHAR* Name,
	const UE::Renderer::Private::IMegaFilters::FInputs& Inputs);

FRDGTextureRef AddMegaFiltersTextureCopy(
    FRDGBuilder& GraphBuilder,
    const FViewInfo& View,
    FRDGTextureRef SourceTexture,
    UE::Renderer::Private::EMegaFiltersPassType PassType,
