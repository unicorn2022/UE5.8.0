...
public:
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(float, bShowTileBorder)
...
FScreenPassTexture AddVisualizeBufferPass(FRDGBuilder& GraphBuilder, const FViewInfo& View, const FVisualizeBufferInputs& Inputs)
{
	...
	for (int32 TileIndex = 0; TileIndex < Inputs.Tiles.Num(); ++TileIndex)
	{
		...
		PassParameters->InvGamma = Tile.bNeedsGammaCorrection ? (1.0f / GEngine->GetDisplayGamma()) : 1.0f;
		PassParameters->bShowTileBorder = 1.0f;
...
{
	return GetActiveBufferVisualizationMaterial(View) != nullptr
		|| IsVisualizeGBufferOverviewEnabled(View)
		|| IsVisualizeGBufferDumpToFileEnabled(View)
		|| IsVisualizeGBufferDumpToPipeEnabled(View);
}

UMaterial* GetActiveBufferVisualizationMaterial(const FViewInfo& View)
{
	if (View.Family->EngineShowFlags.VisualizeBuffer && GetBufferVisualizationData().IsInitialized())
	{
...
{
	FPostProcessMaterialInputs VisualizeBufferPPMInputs = PostProcessMaterialInputs;

	if (UMaterial* VisMaterial = GetActiveBufferVisualizationMaterial(View))
	{
		if (VisMaterial->BlendableLocation == Location || Location == EBlendableLocation::BL_MAX)
		{
			VisualizeBufferPPMInputs.bAllowSceneColorInputAsOutput = false;
			Context.Buffer = AddPostProcessMaterialPass(GraphBuilder, View, VisualizeBufferPPMInputs, VisMaterial);
		}
	}

	if (IsVisualizeGBufferOverviewEnabled(View)
		|| IsVisualizeGBufferDumpToFileEnabled(View)
		|| IsVisualizeGBufferDumpToPipeEnabled(View))
	{
		const FFinalPostProcessSettings& PostProcessSettings = View.FinalPostProcessSettings;
		
		int NumMaterials = PostProcessSettings.BufferVisualizationOverviewMaterials.Num();
		for (int Index = 0; Index < NumMaterials; ++Index)
		{
			UMaterialInterface* VisMaterialInterface = PostProcessSettings.BufferVisualizationOverviewMaterials[Index];
			UMaterial* VisMaterial = VisMaterialInterface ? VisMaterialInterface->GetMaterial() : nullptr;

			if (VisMaterial && (VisMaterial->BlendableLocation == Location || Location == EBlendableLocation::BL_MAX))
			{
				const TSharedPtr<FImagePixelPipe, ESPMode::ThreadSafe>* OutputPipe = PostProcessSettings.BufferVisualizationPipes.Find(VisMaterialInterface->GetFName());
				...
				{
					VisualizeBufferPPMInputs.OutputFormat = PF_A32B32G32R32F;
				}
				else
				{
					// Respect the r.PostProcessingColorFormat cvar just like the main rendering path
					EPixelFormat OutputFormat = OverridePostProcessingColorFormat(bOutputInHDR ? PF_FloatRGBA : PF_Unknown);
					VisualizeBufferPPMInputs.OutputFormat = OutputFormat;
				}
				VisualizeBufferPPMInputs.bAllowSceneColorInputAsOutput = false;

				Context.OverviewBuffers.SetNum(NumMaterials);
				Context.OverviewBuffers[Index] = AddPostProcessMaterialPass(GraphBuilder, View, VisualizeBufferPPMInputs, VisMaterialInterface);
			}
		}
	}
}

static bool RunsBeforeTonemapping(UMaterialInterface* MaterialInterface)
{
	UMaterial* BaseMaterial = MaterialInterface->GetMaterial();
	EBlendableLocation BlendableLocation = MaterialInterface->GetBlendableLocation(BaseMaterial);

	bool bMaterialRunsBeforeTonemapping = false;
	switch (BlendableLocation)
	{
		case BL_SceneColorBeforeDOF:
		case BL_SceneColorAfterDOF:
		case BL_TranslucencyAfterDOF:
		case BL_SSRInput:
		case BL_SceneColorBeforeBloom:
			bMaterialRunsBeforeTonemapping = true;
			break;
		case BL_ReplacingTonemapper:
		case BL_SceneColorAfterTonemapping:
			bMaterialRunsBeforeTonemapping = false;
			break;
		default: ensure(0);
	}

	return bMaterialRunsBeforeTonemapping;
}

FScreenPassTexture AddVisualizeGBufferPass(
	FRDGBuilder& GraphBuilder,
	const FViewInfo& View,
	const FVisualizeGBufferInputs& Inputs,
	FVisualizeBufferContext& Context)
{
	const FFinalPostProcessSettings& PostProcessSettings = View.FinalPostProcessSettings;

	const bool bOverview = IsVisualizeGBufferOverviewEnabled(View);
	const bool bDumpToFile = IsVisualizeGBufferDumpToFileEnabled(View);
	const bool bDumpToPipe = IsVisualizeGBufferDumpToPipeEnabled(View);
	const bool bActiveBufferVisualizationMaterial = GetActiveBufferVisualizationMaterial(View) != nullptr;

	check(Inputs.SceneColor.IsValid());
	check(bActiveBufferVisualizationMaterial || bDumpToFile || bOverview || PostProcessSettings.BufferVisualizationPipes.Num() > 0);

	if (bActiveBufferVisualizationMaterial)
	{
		if (Context.Buffer.IsValid())
		{
			...
			bool bMaterialRunsBeforeTonemapping = RunsBeforeTonemapping(GetActiveBufferVisualizationMaterial(View));
			...
			PassParameters->Output = GetScreenPassTextureViewportParameters(OutputViewport);
			PassParameters->RenderTargets[0] = Output.GetRenderTargetBinding();
			PassParameters->InputTexture = Context.Buffer.Texture;
			PassParameters->InputSampler = BilinearClampSampler;
			PassParameters->SelectionColor = FLinearColor::Transparent;
			PassParameters->InvGamma = bMaterialRunsBeforeTonemapping ? (1.0f / GEngine->GetDisplayGamma()) : 1.0f;
			PassParameters->bShowTileBorder = 0.0f;

			const FScreenPassTextureViewport InputViewport(Context.Buffer);

			TShaderMapRef<FScreenPassVS> VertexShader(View.ShaderMap);
			TShaderMapRef<FVisualizeBufferPS> PixelShader(View.ShaderMap);
			FRHIBlendState* BlendState = TStaticBlendState<CW_RGB, BO_Add, BF_SourceAlpha, BF_InverseSourceAlpha>::GetRHI();

			AddDrawScreenPass(GraphBuilder, RDG_EVENT_NAME("VisualizeBuffer"), View, OutputViewport, InputViewport, VertexShader, PixelShader, BlendState, PassParameters);

			return Output;
		}
	}
	else if(!Context.OverviewBuffers.IsEmpty())
	{
		...
		for (int Index = 0; Index < NumMaterials; ++Index)
		{
			UMaterialInterface* MaterialInterface = PostProcessSettings.BufferVisualizationOverviewMaterials[Index];
			FScreenPassTexture Output = Context.OverviewBuffers[Index];

			if (!MaterialInterface || !Output.IsValid())
			{
			...
			if (bOverview)
			{
				...
				bool bMaterialRunsBeforeTonemapping = RunsBeforeTonemapping(MaterialInterface);
				...
				const FName MaterialFName = MaterialInterface->GetFName();
				if (!bIsMobilePlatform || GetBufferVisualizationData().GetMaterialEnableOnMobile(MaterialFName))
				{
					Tile.Label = GetBufferVisualizationData().GetMaterialDisplayName(MaterialFName).ToString();
				}
				else
				{
					const FText NotImplementedOnMobile = NSLOCTEXT("PostProcessVisualizeBuffer", "NotImplementedOnMobile", "(Not implemented on Mobile)");
					Tile.Label = FText::Format(
						NSLOCTEXT("PostProcessVisualizeBuffer", "TileNotImplementedOnMobile", "{0} {1}"),
						GetBufferVisualizationData().GetMaterialDisplayName(MaterialFName),
						NotImplementedOnMobile).ToString();
				}

				// VisualizeGBufferOverviewPass runs after tonemapping, so any materials that were expecting to run before
				// need to be tonemapped manually.
				Tile.bNeedsGammaCorrection = bMaterialRunsBeforeTonemapping;
