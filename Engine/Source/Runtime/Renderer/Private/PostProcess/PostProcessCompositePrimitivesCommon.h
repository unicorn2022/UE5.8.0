...
struct FCompositePrimitiveInputs
{
	...
	// [Optional] Size (in pixels) of the dithering pattern applied to occluded primitives.
	float OccludedDitheringPatternSize = 2.0f;

	// [Optional] The strength of the dithering applied to occluded primitives.
	float OccludedDithering = 0.8f;

	// [Optional] The brightness of occluded primitives.
...
public:
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_RDG_TEXTURE_SRV(Texture2D<float>, EditorPrimitivesDepth)
		SHADER_PARAMETER_RDG_TEXTURE_SRV(Texture2D<uint2>, EditorPrimitivesStencil)
		...
		SHADER_PARAMETER(float, OccludedDitheringPatternSize)
		SHADER_PARAMETER(float, OccludedDithering)
		SHADER_PARAMETER(float, OccludedBrightness)
		RENDER_TARGET_BINDING_SLOTS()
	END_SHADER_PARAMETER_STRUCT()

	// SDPG_World means "with occlusion", SDPG_Foreground means "without occlusion". But, SDPG_Foreground can be drawn fully opaque, or with dithering on 'occluded' areas (e.g. gizmos).
	// The stencil flag marks which areas should *not* receive dithering.
	static constexpr uint32 EditorStencilFlagForegroundNoDithering = 0x1;
	...
	static void ModifyCompilationEnvironment(const FGlobalShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment)
	{
		...
		OutEnvironment.SetDefine(TEXT("EDITOR_STENCIL_FLAG_FOREGROUND_NO_DITHERING"), EditorStencilFlagForegroundNoDithering);
	}
};
