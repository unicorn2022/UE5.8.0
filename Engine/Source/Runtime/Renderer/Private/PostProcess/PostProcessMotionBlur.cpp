...
BEGIN_SHADER_PARAMETER_STRUCT(FVecocityTileTextureSRVs, )
	// The NON_PIXEL shader parameter will still generate proper SRVCompute value in compute passes
	SHADER_PARAMETER_RDG_TEXTURE_NON_PIXEL_SRV_ARRAY(Texture2D, Textures, [FVelocityFlattenTextures::kMaxVelocityTileTextureCount])
		...
		AddDrawCanvasPass(GraphBuilder, RDG_EVENT_NAME("VisualizeMotionBlurOverlay"), View, Output,
			[&View](FCanvas& Canvas)
			{
				...
				Line = FString::Printf(TEXT("View=%.4x PrevView=%.4x"),
					View.ViewMatrices.GetWorldToView().ComputeHash() & 0xffff,
					View.PrevViewInfo.ViewMatrices.GetWorldToView().ComputeHash() & 0xffff);
