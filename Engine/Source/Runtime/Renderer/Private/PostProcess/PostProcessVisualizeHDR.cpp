...
FScreenPassTexture AddVisualizeHDRPass(FRDGBuilder& GraphBuilder, const FViewInfo& View, const FVisualizeHDRInputs& Inputs)
{
	...
	// HistogramTexture (desktop) and HistogramBuffer (mobile) may both be null when auto exposure is disabled
	// we substitute dummy resources in that case, so the overlay still renders, just with an empty histogram.
	checkf(!(Inputs.HistogramTexture && Inputs.HistogramBuffer), TEXT("VisualizeHDR cannot have both HistogramTexture and HistogramBuffer set."));
	...
	PassParameters->HistogramTexture = Inputs.HistogramTexture ? Inputs.HistogramTexture : GSystemTextures.GetBlackDummy(GraphBuilder);
	PassParameters->HistogramBuffer = Inputs.HistogramBuffer ? Inputs.HistogramBuffer : GraphBuilder.CreateSRV(GSystemTextures.GetDefaultBuffer(GraphBuilder, 4), PF_R32_UINT);
