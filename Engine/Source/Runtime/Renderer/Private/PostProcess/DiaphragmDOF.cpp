...
TAutoConsoleVariable<int32> CVarGatherMaxMipMapCount(
	TEXT("r.DOF.Gather.MaxMipMapCount"),
	4,
	TEXT("Set a maximum amount of mipmaps to calculate in the reduction phase.\n")
	TEXT("These are then used in the gather step to reduce noise in areas with a large but sparse blurring kernel."),
	ECVF_Scalability | ECVF_RenderThreadSafe);
...
TAutoConsoleVariable<float> CVarSOFHistoryRejectionBoost(
	TEXT("r.DOF.TAA.SOFHistoryRejectionBoost"),
	0.0,
	TEXT("In the recombine pass, reduce the strength of denoising for slight-out-of-focus areas.\n")
	TEXT("Reasonable values are 0 to 0.125.\n")
	TEXT("If 0, this is disabled.\n")
	TEXT(" (Default: 0)."),
	ECVF_RenderThreadSafe);

TAutoConsoleVariable<int32> CVarPreferLowerBitDepth(
	TEXT("r.DOF.PreferLowerBitDepth"),
	0,
	TEXT("When enabled, prefers intermediate textures with lower bit depth.\n")
	TEXT("This improves memory usage and bandwidth, at the cost of quality."),
	ECVF_Scalability | ECVF_RenderThreadSafe);
...
class FDiaphragmDOFCocFlattenCS : public FDiaphragmDOFShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		SHADER_PARAMETER(FVector2f, Stride)
		SHADER_PARAMETER(FVector4f, GatherInputExtentAndInvExtent)
		SHADER_PARAMETER(FVector4f, GatherInputViewRectUV)
...
class FDiaphragmDOFReduceCS : public FDiaphragmDOFShader
{
	...
	class FReduceMipCount : SHADER_PERMUTATION_RANGE_INT("DIM_REDUCE_MIP_COUNT", 0, kMaxMipLevelCount + 1);
	...
	static int32 GetMaxReductionMipLevelCount(bool bSupportAlphaChannel, bool bMip0Disabled)
	{
		// Can only have 8 UAVs, but need to output 3x UAV for hybird scatter + 2 UAV per mips for RGBA + SeparateCoc.
		int32 MaxLevelsWithAlphaChannel = bMip0Disabled ? 3 : 2;
		return bSupportAlphaChannel ? MaxLevelsWithAlphaChannel : kMaxMipLevelCount;
	}

	/** Returns the nearest number of mip levels the reduce pass is able to output */
	static int32 GetNearestMipLevelReductionCount(int ReductionCount, bool bSupportAlphaChannel, bool bMip0Disabled)
	{
		int32 MaxCount = GetMaxReductionMipLevelCount(bSupportAlphaChannel, bMip0Disabled);
		ReductionCount = FMath::Min(ReductionCount, MaxCount);
	...
	static bool ShouldCompilePermutation(const FGlobalShaderPermutationParameters& Parameters)
	{
		...
		bool bAlphaChannel = PermutationVector.Get<FDiaphragmDOFShader::FAlphaChannelDim>();
		bool bDisableOutputMip0 = PermutationVector.Get<FDisableOutputMip0>();
		...
		int32 ReduceMipCount = PermutationVector.Get<FReduceMipCount>();
		if (GetNearestMipLevelReductionCount(ReduceMipCount, bAlphaChannel, bDisableOutputMip0) != ReduceMipCount)
		{
			return false;
		}
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		SHADER_PARAMETER(FIntPoint, PassViewSize)
		...
		SHADER_PARAMETER(FVector4f, QuarterResGatherInputExtentAndInvExtent)
		SHADER_PARAMETER(FVector4f, QuarterResGatherInputViewRectUV)
...
class FDiaphragmDOFRecombineCS : public FDiaphragmDOFShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(float, MaxRecombineAbsCocRadius)
		SHADER_PARAMETER(float, SOFHistoryRejectionBoost)

		SHADER_PARAMETER(FVector4f, SceneColorExtentAndInvExtent)
		SHADER_PARAMETER(FVector4f, SceneColorViewRectUV)
...
bool DiaphragmDOF::IsEnabled(const FViewInfo& View)
{
	...
	return
		...
		View.FinalPostProcessSettings.DepthOfFieldEnabled &&
...
bool DiaphragmDOF::AddPasses(
	...
	FRDGTextureRef OriginalSceneDepthForStencil)
{
	...
	const bool bPreferLowerBitDepth = CVarPreferLowerBitDepth.GetValueOnRenderThread() != 0;
	const bool bRGBBufferSeparateCocBuffer = (
		...
		// Maintain quality
		(bPreferLowerBitDepth || SceneColorFormat == PF_FloatR11G11B10));
	...
	FIntRect FullResGatherInputViewRect = View.ViewRect;
	FDOFGatherInputTextures HalfResGatherInputTextures;
	FIntRect HalfResGatherInputViewRect = View.ViewRect / 2;
	...
	// Setup at lower resolution from full resolution scene color and scene depth.
	{
		...
		if (!bOutputHalfResolution)
		{
			...
			HalfResGatherInputViewRect = FullResGatherInputViewRect;
		}
	}
	...
	if (IsTemporalAccumulationBasedMethod(View.AntiAliasingMethod) && ViewState)
	{
		...
		TAAParameters.StencilDepthTexture = OriginalSceneDepthForStencil;
		...
		HalfResGatherInputViewRect = TAAParameters.OutputViewRect;
		HalfResGatherInputViewRect.Min = FIntPoint::DivideAndRoundUp(HalfResGatherInputViewRect.Min, TAAParameters.ResolutionDivisor);
		HalfResGatherInputViewRect.Max = FIntPoint::DivideAndRoundUp(HalfResGatherInputViewRect.Max, TAAParameters.ResolutionDivisor);
	...
	FDOFTileClassificationTextures TileClassificationTextures;
	{
		...
		FDOFTileClassificationTextures FlattenedTileClassificationTextures;
		{
			...
			const FIntPoint HalfResExtent = HalfResGatherInputDescs.SceneColor.Extent;
			const FVector4f HalfResExtentAndInvExtent
			{
				float(HalfResExtent.X), float(HalfResExtent.Y), 1.0f / HalfResExtent.X, 1.0f / HalfResExtent.Y
			};
			const FVector4f HalfResViewRectUV
			{
				FVector2f(HalfResGatherInputViewRect.Min) / HalfResExtent, 
				(FVector2f(HalfResGatherInputViewRect.Max) - 0.5f) / HalfResExtent
			};

			FDiaphragmDOFCocFlattenCS::FParameters* PassParameters = GraphBuilder.AllocParameters<FDiaphragmDOFCocFlattenCS::FParameters>();
			PassParameters->CommonParameters = CommonParameters;

			PassParameters->Stride = FVector2f(
				PreprocessViewSize.X / float(GatheringViewSize.X),
				PreprocessViewSize.Y / float(GatheringViewSize.Y));
			PassParameters->GatherInputExtentAndInvExtent = HalfResExtentAndInvExtent;
			PassParameters->GatherInputViewRectUV = HalfResViewRectUV;
	...
	FRDGBufferRef BackgroundScatterDrawListBuffer = nullptr;
	{
		...
		const int32 MipLevelsRequested = FMath::CeilToInt(FMath::Log2(MaxBluringRadius * 0.5 / HalfResRingCount)) + (bUseLowAccumulatorQuality ? 1 : 0);
		const int32 MipLevelCount = FMath::Clamp(MipLevelsRequested, 1, CVarGatherMaxMipMapCount.GetValueOnRenderThread());
		...
		{
			QuarterResGatherInputDescs = HalfResGatherInputDescs;
			QuarterResGatherInputDescs.SceneColor.Extent /= 2;
			QuarterResGatherInputDescs.SeparateCoc.Extent /= 2;
		}
		const FIntPoint QuarterResExtent = QuarterResGatherInputDescs.SceneColor.Extent;
		const FVector4f QuarterResExtentAndInvExtent
		{
			float(QuarterResExtent.X), float(QuarterResExtent.Y), 1.0f / QuarterResExtent.X, 1.0f / QuarterResExtent.Y
		};
		const FIntRect QuarterResGatherInputViewRect{ FIntPoint::ZeroValue, HalfResGatherInputViewRect.Size() / 2 }; // rebase to (0,0) and halve size
		const FVector4f QuarterResViewRectUV
		{
			FVector2f(QuarterResGatherInputViewRect.Min) / QuarterResExtent, 
			(FVector2f(QuarterResGatherInputViewRect.Max) - 0.5f) / QuarterResExtent
		};
		...
		{
			const bool bMip0Disabled = false;
			int32 ProcessingMipLevelCount = FDiaphragmDOFReduceCS::GetNearestMipLevelReductionCount(MipLevelCount, IsPostProcessingWithAlphaChannelSupported(), bMip0Disabled);
			...
			PassParameters->PassViewSize = PassViewSize;
			...
			PassParameters->QuarterResGatherInputExtentAndInvExtent = QuarterResExtentAndInvExtent;
			PassParameters->QuarterResGatherInputViewRectUV = QuarterResViewRectUV;
		...
		while (ReducedMipLevelCount < MipLevelCount)
		{
			...
			PassParameters->GatherInputExtentAndInvExtent = FVector4f(GatherInputExtent.X / InputMipLevelPow2, GatherInputExtent.Y / InputMipLevelPow2, InputMipLevelPow2 / GatherInputExtent.X, InputMipLevelPow2 / GatherInputExtent.Y);
			PassParameters->GatherInputViewRectUV = ViewRectUV;
	...
	FRDGTextureRef NewSceneColor;
	{
		...
		const FIntPoint FullResExtent = FullResGatherInputDescs.SceneColor.Extent;
		const FVector4f FullResExtentAndInvExtent
		{
			float(FullResExtent.X), float(FullResExtent.Y), 1.0f / FullResExtent.X, 1.0f / FullResExtent.Y
		};
		const FVector4f FullResViewRectUV
		{
			FVector2f(FullResGatherInputViewRect.Min) / FullResExtent, 
			(FVector2f(FullResGatherInputViewRect.Max) - 0.5f) / FullResExtent
		};
		...
		PassParameters->SOFHistoryRejectionBoost = CVarSOFHistoryRejectionBoost.GetValueOnRenderThread();
		...
		PassParameters->SceneColorExtentAndInvExtent = FullResExtentAndInvExtent;
		PassParameters->SceneColorViewRectUV = FullResViewRectUV;
