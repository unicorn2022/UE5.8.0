...
struct FMobileBloomSetupInputs
{
	...
	FIntPoint OutputFixedTextureSize;
...
{
	FIntPoint			BloomDownOutputFixedTextureExtent;
	FIntPoint			BloomDownOutputViewExtent;
...
struct FMobileBloomUpInputs
{
	...
	FIntPoint BloomUpOutputFixedTextureExtent;
	FIntPoint BloomUpOutputViewExtent;
	FVector2D ScaleAB;
	FVector4f TintA;
	FVector4f TintB;
};
...
struct FMobileEyeAdaptationSetupInputs
{
	...
	// When AEM_Basic is active, run the histogram CS so VisualizeHDR has bucket data.
	bool bGenerateHistogramForVisualize = false;
};

struct FMobileEyeAdaptationSetupOutputs
{
	FRDGBufferSRVRef EyeAdaptationSetupSRV;
	// Histogram buffer for the VisualizeHDR overlay.
