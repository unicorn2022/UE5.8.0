...
namespace
{
	...
	TAutoConsoleVariable<int32> CVarSSSSeparatedDiffuseEnable(
		TEXT("r.SSS.SeparatedDiffuse.Enable"),
		1,
		TEXT("Whether store diffuse luminance in a separated texture\n")
		TEXT(" 0: stores in A channel of SceneColor if RGBA format, checkerboard of SceneColor if RGB format:\n")
		TEXT(" 1: Force the luminance to store in the separated texture.\n"),
		ECVF_RenderThreadSafe);

	TAutoConsoleVariable<int32> CVarSSSSeparatedDiffuseSupported(
		TEXT("r.SSS.SeparatedDiffuse.Supported"),
		0,
		TEXT("Whether separated diffuse texture is supported for subsurface scattering in GBuffer format path.\n")
		TEXT("When it's on, a separated texture will be created and bound inevitably. Should use with SceneColor format RGB for quality and performance\n")
		TEXT("It is not compatible to Substrate experimental IsOpaqueRoughRefraction feature.\n"),
		ECVF_ReadOnly);

	TAutoConsoleVariable<int32> CVarSSSSeparatedDiffuseDebug(
		TEXT("r.SSS.SeparatedDiffuse.Debug"),
		0,
		TEXT("Whether the separated diffuse texture is meaningful when non-RGB format is enabled.\n"),
		ECVF_RenderThreadSafe);

	TAutoConsoleVariable<int32> CVarSSSDebugPixel(
		TEXT("r.SSS.DebugPixel"),
		0,
		TEXT("Debug the pixel value and recombine process in full resolution. Need to enable shaderprint with r.ShaderPrint 1.\n"),
		ECVF_RenderThreadSafe);
...
{
	const bool bEnabled = CVarSubsurfaceScattering.GetValueOnAnyThread() != 0 && EngineShowFlags.Lighting;
...
bool IsSubsurfaceRequiredForView(const FViewInfo& View)
{
	...
	const bool bSubsurfaceEnabled = IsSubsurfaceEnabled(View.Family->EngineShowFlags);
...
{
	return PlatformSupportsSeparatedSubsurfaceDiffuse(ShaderPlatform)
		&& IsUsingGBuffers(ShaderPlatform)
		&& !Substrate::IsOpaqueRoughRefractionEnabled(ShaderPlatform);
}
...
bool IsSubsurfaceCheckerboardFormat(EPixelFormat SceneColorFormat, EShaderPlatform ShaderPlatform)
{
	if (Substrate::IsOpaqueRoughRefractionEnabled(ShaderPlatform))
	{
		// With this mode, specular and subsurface colors are correctly separated so checkerboard is not required.
		return false;
	}
	const int SeparatedDiffuseEnable = CVarSSSSeparatedDiffuseEnable.GetValueOnAnyThread();
	const bool bSeparatedDiffuseSupported = UseSeparatedSubsurfaceDiffuse(ShaderPlatform);
	if (SeparatedDiffuseEnable > 0 && bSeparatedDiffuseSupported)
	{
		// Force the luminance to store in the separated texture
		return false;
	}
...
{
	return IsSubsurfaceCheckerboardFormat(SceneColorFormat, View.GetShaderPlatform());
}

bool IsSeparatedDiffuseLuminanceEnabled(const FEngineShowFlags& EngineShowFlags, EPixelFormat SceneColorFormat, EShaderPlatform ShaderPlatform)
{
	const int bSeparatedDiffuseEnable = UseSeparatedSubsurfaceDiffuse(ShaderPlatform);
	const bool bEnabled = IsSubsurfaceEnabled(EngineShowFlags);

	const bool bIsCheckerboardEnabled = IsSubsurfaceCheckerboardFormat(SceneColorFormat, ShaderPlatform);
	const bool bColorFormatDoesNotHaveAlpha = SceneColorFormat != PF_A32B32G32R32F && SceneColorFormat!= PF_FloatRGBA;
	const bool bDebugSeparatedDiffuse = CVarSSSSeparatedDiffuseDebug.GetValueOnAnyThread() != 0;
	return bEnabled && bSeparatedDiffuseEnable && !bIsCheckerboardEnabled && (bColorFormatDoesNotHaveAlpha || bDebugSeparatedDiffuse);
}
...
public:
	...
	static void ModifyCompilationEnvironment(const FGlobalShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment)
	{
		FSubsurfaceShader::ModifyCompilationEnvironment(Parameters, OutEnvironment);
		OutEnvironment.SetDefine(TEXT("SUBSURFACE_BURLEY_COMPUTE"), 1);
		OutEnvironment.SetDefine(TEXT("SUBSURFACE_DIFFUSE_OPERATION"),/*DIFFUSE_LUMINANCE_READ*/ 0);
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		SHADER_PARAMETER_RDG_UNIFORM_BUFFER(FSubstrateGlobalUniformParameters, Substrate)
		SHADER_PARAMETER_STRUCT_INCLUDE(FSubsurfaceDiffuseReadParameters, SubsurfaceDiffuseParameters)
		SHADER_PARAMETER_STRUCT_INCLUDE(FSubsurfaceParameters, Subsurface)
...
// Calculate the Luminance after the base pass to get the Luminance of the scene color
class FSubsurfaceDiffuseInitPassPS : public FSubsurfaceShader
{
	DECLARE_GLOBAL_SHADER(FSubsurfaceDiffuseInitPassPS);
	SHADER_USE_PARAMETER_STRUCT(FSubsurfaceDiffuseInitPassPS, FSubsurfaceShader);
...
IMPLEMENT_GLOBAL_SHADER(FSubsurfaceDiffuseInitPassPS, "/Engine/Private/PostProcessSubsurfaceDiffuse.usf", "SubsurfaceDiffuseInitPassPS", SF_Pixel);

// Debug pass: prints SSS reconstruction + recombine data at cursor via ShaderPrint
class FSubsurfaceDebugCS : public FSubsurfaceShader
{
	DECLARE_GLOBAL_SHADER(FSubsurfaceDebugCS);
	SHADER_USE_PARAMETER_STRUCT(FSubsurfaceDebugCS, FSubsurfaceShader);
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_SAMPLER(SamplerState, SubsurfaceSampler1)
		SHADER_PARAMETER_STRUCT_INCLUDE(FSubsurfaceDiffuseReadParameters, SubsurfaceDiffuseParameters)
		SHADER_PARAMETER(float, ViewResolutionFraction)
		SHADER_PARAMETER(int, SceneColorHasAlpha)
	END_SHADER_PARAMETER_STRUCT()

	class FDimensionCheckerboard : SHADER_PERMUTATION_BOOL("SUBSURFACE_PROFILE_CHECKERBOARD");
	class FDimensionQuality : SHADER_PERMUTATION_ENUM_CLASS("SUBSURFACE_RECOMBINE_QUALITY", FSubsurfaceRecombinePS::EQuality);
	using FPermutationDomain = TShaderPermutationDomain<FDimensionCheckerboard, FDimensionQuality>;

	static bool ShouldCompilePermutation(const FGlobalShaderPermutationParameters& Parameters)
	{
		return FSubsurfaceShader::ShouldCompilePermutation(Parameters) && ShaderPrint::IsSupported(Parameters.Platform);
	}
	...
	static void ModifyCompilationEnvironment(const FGlobalShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment)
	{
		...
		OutEnvironment.SetDefine(TEXT("SUBSURFACE_DEBUG_PASS"), 1);
		OutEnvironment.SetDefine(TEXT("SUBSURFACE_DIFFUSE_OPERATION"), /*DIFFUSE_LUMINANCE_READ*/ 0);
		OutEnvironment.SetDefine(TEXT("SUBSURFACE_RECOMBINE_MODE"), /*SUBSURFACE_RECOMBINE_MODE_FULLRES*/ 0);
		OutEnvironment.SetDefine(TEXT("SSS_SUPPORT_SEPARATED_SUBSURFACE_DIFFUSE"), PlatformSupportsSeparatedSubsurfaceDiffuse(Parameters.Platform) ? 1 : 0);
	}
};

IMPLEMENT_GLOBAL_SHADER(FSubsurfaceDebugCS, "/Engine/Private/PostProcessSubsurface.usf", "SubsurfaceDebugCS", SF_Compute);
	...
	FSubsurfaceDiffuseWriteParameters& OutParameters)
{
	...
	bool bSeparatedSubsurfaceDiffuseEnabled = 
		IsSeparatedDiffuseLuminanceEnabled(
			View.Family->EngineShowFlags,
			SceneTextures.Color.Target->Desc.Format,
			View.GetShaderPlatform());

	if (SeparatedSubsurfaceDiffuse)
	{
		if (bSeparatedSubsurfaceDiffuseEnabled)
		{
			OutParameters.RWSeparatedSubsurfaceDiffuseLuminance = GraphBuilder.CreateUAV(SeparatedSubsurfaceDiffuse);
			OutParameters.bWriteSeparatedSubsurfaceDiffuse = 1;
		}
		else
		{
			FRDGTextureRef Dummy = GraphBuilder.CreateTexture(
				FRDGTextureDesc::Create2D(FIntPoint(1, 1), PF_R16F, FClearValueBinding::None,
					TexCreate_RenderTargetable | TexCreate_ShaderResource | TexCreate_UAV),
				TEXT("DummyScatterableLuminance"));

			OutParameters.RWSeparatedSubsurfaceDiffuseLuminance = GraphBuilder.CreateUAV(Dummy);
			OutParameters.bWriteSeparatedSubsurfaceDiffuse = 0;
		}
	}
	else
	{
		OutParameters.RWSeparatedSubsurfaceDiffuseLuminance = nullptr;
		OutParameters.bWriteSeparatedSubsurfaceDiffuse = 0;
	}
}
	...
	FSubsurfaceDiffuseReadParameters& OutParameters)
{
	...
	if (SeparatedSubsurfaceDiffuse)
	{
		if (bSeparatedSubsurfaceDiffuseEnabled)
		{
			OutParameters.SeparatedSubsurfaceDiffuseLuminance = GraphBuilder.CreateSRV(SeparatedSubsurfaceDiffuse);
			OutParameters.bUseSeparatedSubsurfaceDiffuse = 1;
		}
		else
		{
			FRDGTextureRef Dummy = GraphBuilder.CreateTexture(
				FRDGTextureDesc::Create2D(FIntPoint(1, 1), PF_R16F, FClearValueBinding::None,
					TexCreate_RenderTargetable | TexCreate_ShaderResource | TexCreate_UAV),
				TEXT("DummySeparatedSubsurfaceDiffuseLuminance"));

			AddClearUAVPass(GraphBuilder,GraphBuilder.CreateUAV(Dummy),FLinearColor::White);
			OutParameters.SeparatedSubsurfaceDiffuseLuminance = GraphBuilder.CreateSRV(Dummy);
			OutParameters.bUseSeparatedSubsurfaceDiffuse = 0;
		}
	}
	else
	{
		OutParameters.SeparatedSubsurfaceDiffuseLuminance = nullptr;
		OutParameters.bUseSeparatedSubsurfaceDiffuse = 0;
	}
}
	...
	TArray<FSubsurfaceTiles>& OutPerViewTiles)
{
	...
	FRDGTextureRef SeparatedSubsurfaceDiffuse = SceneTextures.SeparatedSubsurfaceDiffuse;
	OutPerViewTiles.SetNum(Views.Num());

	RDG_EVENT_SCOPE_STAT(GraphBuilder, SubsurfaceScattering, "SSS::InitSeparatedDiffuse");

	auto ComputeSeparatedScatteringLuminance = [&](const FViewInfo& View, FSubsurfaceTiles& Tiles, 
		ERenderTargetLoadAction SeparatedDiffuseLoadAction){

		const ESubsurfaceMode SubsurfaceMode = GetSubsurfaceModeForView(View);
		const bool bByPassSubsurface = SubsurfaceMode == ESubsurfaceMode::Bypass;
		...
		// 1. Build SSS tiles in Init pass
		// Should not build tiles if bypassing as a whole pass will be initiated.
		if (!bByPassSubsurface)
		{
			Tiles = ClassifySSSTiles(GraphBuilder, View, SceneTextures, SceneViewportParameters, SubsurfaceViewportParameters, bHalfRes);
		}
		// 2. Tile-driven PS copy: SceneColor luminance -> SeparatedSubsurfaceDiffuse
		{
			FSubsurfaceDiffuseInitPassPS::FParameters* PassParameters = GraphBuilder.AllocParameters<FSubsurfaceDiffuseInitPassPS::FParameters>();
			PassParameters->ViewUniformBuffer = View.ViewUniformBuffer;
			PassParameters->SceneColorTexture = SceneColorTexture;
			if (!bByPassSubsurface)
			{
				PassParameters->TileParameters = GetSubsurfaceTileParameters(SubsurfaceViewport, Tiles, FSubsurfaceTiles::ETileType::All);
			}
			PassParameters->RenderTargets[0] = FRenderTargetBinding(SeparatedSubsurfaceDiffuse, SeparatedDiffuseLoadAction);

			TShaderMapRef<FSubsurfaceTilePassVS> VertexShader(View.ShaderMap);
			TShaderMapRef<FSubsurfaceDiffuseInitPassPS> PixelShader(View.ShaderMap);

			FRHIBlendState* BlendState = TStaticBlendState<CW_RGBA, BO_Add, BF_One, BF_Zero>::GetRHI();
			FRHIDepthStencilState* DepthStencilState = TStaticDepthStencilState<false, CF_Always>::GetRHI();

			// ByPassing Subsurface still requires the whole diffuse in recombine pass.
			AddSubsurfaceTiledScreenPass(
				GraphBuilder,
				RDG_EVENT_NAME("SSS::InitSeparatedDiffuse(%s) %dx%d",
					bByPassSubsurface ? TEXT("Full") : TEXT("Tiled"),
					View.ViewRect.Width(), 
				...
				DepthStencilState,
				SceneViewport,
				FSubsurfaceTiles::ETileType::All,
				bByPassSubsurface /*bShouldFallbackToFullScreenPass*/);
		}
	};

	// Initialize the separated scattering luminance from the base pass, indirect lighting contribution.
	// We build tiles here if possible.
	if (SeparatedSubsurfaceDiffuse)
	{
		...
		ERenderTargetLoadAction SeparatedDiffuseLoadAction = ERenderTargetLoadAction::EClear;
		for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
		{
			...
			bool bSeparatedSubsurfaceDiffuseEnabled = IsSeparatedDiffuseLuminanceEnabled( 
				View.Family->EngineShowFlags,
				SceneColorTexture->Desc.Format,
				View.GetShaderPlatform());

			if (bIsSubsurfaceView && bSeparatedSubsurfaceDiffuseEnabled)
			{
				ComputeSeparatedScatteringLuminance(View, OutPerViewTiles[ViewIndex], SeparatedDiffuseLoadAction);
				SeparatedDiffuseLoadAction = ERenderTargetLoadAction::ELoad;
			}
		}
	}
}
...
void AddSubsurfaceViewPass(
	FRDGBuilder& GraphBuilder,
	const FViewInfo& View,
	const FSubsurfaceTiles* PrebuiltTiles,
	const FSceneTextures& SceneTextures,
	...
	ERenderTargetLoadAction SceneColorTextureLoadAction)
{
	...
	if (SubsurfaceMode != ESubsurfaceMode::Bypass)
	{
		...
		if (PrebuiltTiles && PrebuiltTiles->IsValid())
		{
			Tiles = *PrebuiltTiles;
		}
		else
		...
		// Call the indirect setup (with tile classification)
		{
			...
			for (int PassIndex = 0; PassIndex < NumOfSubsurfaceSetupPass; ++PassIndex)
			{
				...
				SetupSubsurfaceDiffuseReadParameters(GraphBuilder, View, PassParameters->SubsurfaceDiffuseParameters);
		...
		// Major pass to combine Burley and Separable
		{
			...
			{
				{	TEXT("PassOne_Burley"),	SetupTexture, SubsurfaceSubpassOneTex, SHADER::ESubsurfaceType::BURLEY	 , SHADER::ESubsurfacePass::PassOne}, //Burley main pass
				{	TEXT("PassTwo_SepHon"),	SetupTexture, SubsurfaceSubpassOneTex, SHADER::ESubsurfaceType::SEPARABLE, SHADER::ESubsurfacePass::PassOne}, //Separable horizontal
				{ TEXT("PassThree_SepVer"),	SubsurfaceSubpassOneTex, SubsurfaceSubpassTwoTex, SHADER::ESubsurfaceType::SEPARABLE, SHADER::ESubsurfacePass::PassTwo}, //Separable Vertical
				{	 TEXT("PassFour_BVar"),	SubsurfaceSubpassOneTex, SubsurfaceSubpassTwoTex, SHADER::ESubsurfaceType::BURLEY	 , SHADER::ESubsurfacePass::PassTwo}  //Burley Variance
			};
			...
			for (int PassIndex = 0; PassIndex < NumOfSubsurfacePass; ++PassIndex)
			{
				...
				FComputeShaderUtils::AddPass(
					GraphBuilder,
					RDG_EVENT_NAME("SSS::%s(%s %s %s %s%s)",
						PassInfo.Name,
	...
	// Debug: print SSS reconstruction + recombine at cursor when in full res.
	const bool bDebugPixel = CVarSSSDebugPixel.GetValueOnRenderThread() != 0;
	if (ShaderPrint::IsEnabled() && ShaderPrint::IsValid(View.ShaderPrintData) 
		&& !bHalfRes && SubsurfaceMode != ESubsurfaceMode::Bypass && bDebugPixel)
	{
		ShaderPrint::RequestSpaceForCharacters(2048);
		
		typedef FSubsurfaceDebugCS SHADER;
		...
		if (SubsurfaceSubpassTwoTex)
		{
			PassParameters->SubsurfaceInput1 = GetSubsurfaceInput(SubsurfaceSubpassTwoTex, SubsurfaceViewportParameters);
		}
		else
		{
			PassParameters->SubsurfaceInput1 = GetSubsurfaceInput(SystemTextures.Black, SubsurfaceViewportParameters);
		}

		PassParameters->SubsurfaceSampler1 = TStaticSamplerState<SF_Bilinear, AM_Border, AM_Border, AM_Border>::GetRHI();

		SetupSubsurfaceDiffuseReadParameters(GraphBuilder,View,PassParameters->SubsurfaceDiffuseParameters);
		PassParameters->ViewResolutionFraction = View.ViewRect.Width() / (float)View.UnscaledViewRect.Width();

		const bool bColorFormatHasAlpha = bReadSeparatedSubSurfaceSceneColor ? false : 
			(SceneColorTexture->Desc.Format == PF_A32B32G32R32F || SceneColorTexture->Desc.Format == PF_FloatRGBA);
		PassParameters->SceneColorHasAlpha = bColorFormatHasAlpha ? 1 : 0;

		SHADER::FPermutationDomain PermutationVectors;
		PermutationVectors.Set<SHADER::FDimensionCheckerboard>(bCheckerboard);
		PermutationVectors.Set<SHADER::FDimensionQuality>(FSubsurfaceRecombinePS::GetQuality(View));
		TShaderMapRef<SHADER> ComputeShader(View.ShaderMap, PermutationVectors);

		FIntPoint ViewSize = View.ViewRect.Size();
		FComputeShaderUtils::AddPass(
			GraphBuilder,
			RDG_EVENT_NAME("SSS::Debug"),
			ComputeShader,
			PassParameters,
			FComputeShaderUtils::GetGroupCount(ViewSize, FIntPoint(8, 8)));
	}
	...
	FRDGTextureRef SceneColorTextureOutput)
{
	...
	for (uint32 ViewIndex = 0; ViewIndex < ViewCount; ++ViewIndex)
	{
		...
		if (bIsSubsurfaceView)
		{
			...
			const FViewInfo& View = Views[ViewIndex];
			const bool bHasValidTile = (ViewIndex < (uint32)PrebuiltTiles.Num()) && PrebuiltTiles[ViewIndex].IsValid();
			const FSubsurfaceTiles* ViewTiles = bHasValidTile ? &PrebuiltTiles[ViewIndex] : nullptr;

			AddSubsurfaceViewPass(GraphBuilder, View, ViewTiles, SceneTextures, SceneColorTextureOutput, SceneColorTextureLoadAction);
	...
	const TArray<FSubsurfaceTiles>& PrebuiltTiles)
{
	...
	AddSubsurfacePass(GraphBuilder, Views, PrebuiltTiles, ViewMask, SceneTextures, SceneTextures.Color.Target);
