...
TAutoConsoleVariable<int32> CVarTSRShadingRejectionMode(
	TEXT("r.TSR.ShadingRejection.Mode"), 1,
	TEXT(".\n")
	TEXT("  0: More responsiveness and less ghosting but tends to have more blocky artifacts.\n")
	TEXT("  1: Better stability and shading rejection control but tends to have ghosting.\n"),
	ECVF_RenderThreadSafe);
...
TAutoConsoleVariable<int32> CVarTSRThinGeometryDetection(
	...
	TEXT("Define if we should perform another pass to detect thin geometries (sub-pixel in frame buffer) either due to sampling algorithm before TSR or the geometry being too thin (Default = 0). ")
...
TAutoConsoleVariable<int32> CVarTSRThinGeometryCoverageShadingRange(
	TEXT("r.TSR.ThinGeometryDetection.Coverage.ShadingRange"), 3,
	TEXT("The shading model range regarded as thin geometry and accumulate the coverage.\n")
	TEXT("0: Two-sided foliage only.\n")
	TEXT("1: Two-sided foliage and hair.\n")
	TEXT("2: All shading models except unlit.\n")
	TEXT("3: All shading models except unlit participate in coverage. Foliage gets full relaxation dilation.\n")
	TEXT("	Regular materials get lightweight edge-only relaxation (no dilation).\n")
	TEXT("	Requires \"r.TSR.ShadingRejection.Flickering 1\" or \"r.TSR.ThinGeometryDetection.Coverage.MinKeepLineContrast\" > 0.\n")
	TEXT("	Otherwise falls back to 0 (foliage only). (Default)\n"),
	ECVF_RenderThreadSafe);

TAutoConsoleVariable<float> CVarTSRThinGeometryCoverageMaxRelaxationWeight(
	TEXT("r.TSR.ThinGeometryDetection.Coverage.MaxRelaxationWeight"), 0.037f,
	TEXT("The max history clamping box relaxation weight due to thin geometry detection (0 to 1)"),
	ECVF_RenderThreadSafe);

TAutoConsoleVariable<float> CVarTSRThinGeometryCoverageMinKeepLineContrast(
	TEXT("r.TSR.ThinGeometryDetection.Coverage.MinKeepLineContrast"), 0.30f,
	TEXT("The minimum luminance contrast needed to keep a detected intensity line after normalization"),
	ECVF_RenderThreadSafe);

TAutoConsoleVariable<float> CVarTSRThinGeometryHighContrastLineFadeRate(
	TEXT("r.TSR.ThinGeometryDetection.HighContrastLineFadeRate"), 0.1f,
	TEXT("How quickly reprojected high contrast line markers fade out outside validated thin geometry regions. 0=never, 1=instant (default=0.1)."),
	ECVF_RenderThreadSafe);

TAutoConsoleVariable<float> CVarTSRThinGeometryHighContrastLineFadeRateInsideRegion(
	TEXT("r.TSR.ThinGeometryDetection.HighContrastLineFadeRateInsideRegion"), 0.03f,
	TEXT("How quickly reprojected high contrast line markers fade out inside validated thin geometry regions. 0=never, 1=instant (default=0.03)."),
	ECVF_RenderThreadSafe);

TAutoConsoleVariable<float> CVarTSRThinGeometryHighContrastLineWeight(
	TEXT("r.TSR.ThinGeometryDetection.HighContrastLineWeight"), 0.6f,
	TEXT("Relaxation weight applied to detected high contrast lines when no cluster-based relaxation is active."),
	ECVF_RenderThreadSafe);
...
TAutoConsoleVariable<float> CVarTSRThinGeometryDetectionErrorMultiplier(
	...
	TEXT("Scales the depth-based edge detection threshold. Higher values require larger depth discontinuities to classify as thin geometry edges.\n")
	TEXT("Reduce for more sensitive detection at higher upscale ratios (Default = 200)."),
	ECVF_RenderThreadSafe);

TAutoConsoleVariable<int32> CVarTSRThinGeometryDetectionAntiFlickering(
	TEXT("r.TSR.ThinGeometryDetection.AntiFlickering"), 1,
	TEXT("Thin geometry detection can sometimes fail. Whether the anti-flickering logic considers the thin geometry detection pass."),
	ECVF_RenderThreadSafe);

TAutoConsoleVariable<float> CVarTSRThinGeometryDetectionRejectTranslucency(
	TEXT("r.TSR.ThinGeometryDetection.RejectTranslucency"), 0.6f,
	TEXT("Translucency ratio threshold above which all thin geometry detection is rejected for a pixel in the next frame (0 to disable)."),
	ECVF_RenderThreadSafe);
...
class FTSRRejectShadingCS : public FTSRConvolutionNetworkShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(float, ExposureOffsetFactor)
		SHADER_PARAMETER(int32, RejectShadingMode)
		SHADER_PARAMETER(FVector2f, TotalVariationCountInvThreshold)
		SHADER_PARAMETER(int32, ThinGeometryAffectMoireError)
...
class FTSRDetectThinGeometryCS : public FTSRConvolutionNetworkShader
{
	...
	class FVaryingRangeDetectionDim : SHADER_PERMUTATION_BOOL("DIM_VARYING_RANGE_DETECTION");
	class FIntensityLineDetectionDim : SHADER_PERMUTATION_BOOL("DIM_INTENSITY_LINE_DETECTION");
	class FRejectTranslucencyDim : SHADER_PERMUTATION_BOOL("DIM_REJECT_TRANSLUCENCY_DETECTION");
	using FPermutationDomain = TShaderPermutationDomain<FTSRConvolutionNetworkShader::FPermutationDomain, FVaryingRangeDetectionDim, FIntensityLineDetectionDim, FRejectTranslucencyDim>;
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(float, MinKeepLineContrast)
		SHADER_PARAMETER(float, HighContrastLineFadeRateInsideRegion)
		SHADER_PARAMETER(float, HighContrastLineFadeRate)
		SHADER_PARAMETER(float, HighContrastLineWeight)

		SHADER_PARAMETER_RDG_TEXTURE(Texture2D, SceneDepthTexture)
		SHADER_PARAMETER_RDG_TEXTURE_SRV(Texture2D, CurrentCoverageTexture)
		SHADER_PARAMETER_RDG_TEXTURE_SRV(Texture2D, FlickeringLumaTexture)
	...
	static FPermutationDomain RemapPermutation(FPermutationDomain PermutationVector)
	{
		...
		// If intensity line detection is on, we always need varying range. So disable it if not varying range.
		if (PermutationVector.Get<FIntensityLineDetectionDim>() && !PermutationVector.Get<FVaryingRangeDetectionDim>())
		{
			PermutationVector.Set<FIntensityLineDetectionDim>(false);
		}
		return PermutationVector;
	}
...
class FTSRWeightRelaxationCS : public FTSRConvolutionNetworkShader
{
	DECLARE_GLOBAL_SHADER(FTSRWeightRelaxationCS);
	SHADER_USE_PARAMETER_STRUCT(FTSRWeightRelaxationCS, FTSRConvolutionNetworkShader);

	class FRejectTranslucencyDim : SHADER_PERMUTATION_BOOL("DIM_REJECT_TRANSLUCENCY_DETECTION");
	using FPermutationDomain = TShaderPermutationDomain<FTSRConvolutionNetworkShader::FPermutationDomain, FRejectTranslucencyDim>;
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters,)
		...
		SHADER_PARAMETER(int32, ThinGeometryTextureIndex)
		SHADER_PARAMETER(float, MaxRelaxationWeight)
		SHADER_PARAMETER(float, TranslucencyRejectionThreshold)
		SHADER_PARAMETER(int32, bIntensityLineDetectionEnabled)
...
IMPLEMENT_GLOBAL_SHADER(FTSRMeasureFlickeringLumaCS, "/Engine/Private/TemporalSuperResolution/TSRMeasureFlickeringLuma.usf", "MeasureFlickeringLumaCS", SF_Compute);
IMPLEMENT_GLOBAL_SHADER(FTSRMeasureThinGeometryCoverageCS, "/Engine/Private/TemporalSuperResolution/TSRMeasureCoverage.usf", "MeasureCoverageCS", SF_Compute);
IMPLEMENT_GLOBAL_SHADER(FTSRClearPrevTexturesCS,     "/Engine/Private/TemporalSuperResolution/TSRClearPrevTextures.usf",     "ClearPrevTexturesCS", SF_Compute);
IMPLEMENT_GLOBAL_SHADER(FTSRDilateVelocityCS,        "/Engine/Private/TemporalSuperResolution/TSRDilateVelocity.usf",        "DilateVelocityCS", SF_Compute);
IMPLEMENT_GLOBAL_SHADER(FTSRDetectThinGeometryCS,	 "/Engine/Private/TemporalSuperResolution/TSRDetectThinGeometry.usf",	 "DetectThinGeometryCS", SF_Compute);
IMPLEMENT_GLOBAL_SHADER(FTSRWeightRelaxationCS, 	 "/Engine/Private/TemporalSuperResolution/TSRWeightRelaxation.usf",		 "WeightRelaxationCS", SF_Compute);
IMPLEMENT_GLOBAL_SHADER(FTSRDecimateHistoryCS,       "/Engine/Private/TemporalSuperResolution/TSRDecimateHistory.usf",       "DecimateHistoryCS", SF_Compute);
IMPLEMENT_GLOBAL_SHADER(FTSRRejectShadingCS,         "/Engine/Private/TemporalSuperResolution/TSRRejectShading.usf",         "RejectShadingCS", SF_Compute);
IMPLEMENT_GLOBAL_SHADER(FTSRSpatialAntiAliasingCS,   "/Engine/Private/TemporalSuperResolution/TSRSpatialAntiAliasing.usf",   "SpatialAntiAliasingCS", SF_Compute);
IMPLEMENT_GLOBAL_SHADER(FTSRUpdateHistoryCS,         "/Engine/Private/TemporalSuperResolution/TSRUpdateHistory.usf",         "UpdateHistoryCS", SF_Compute);
IMPLEMENT_GLOBAL_SHADER(FTSRResolveHistoryCS,        "/Engine/Private/TemporalSuperResolution/TSRResolveHistory.usf",        "ResolveHistoryCS", SF_Compute);
IMPLEMENT_GLOBAL_SHADER(FTSRVisualizeCS,             "/Engine/Private/TemporalSuperResolution/TSRVisualize.usf",             "VisualizeCS", SF_Compute);
...
bool NeedTSRTranslucentClippedDepthPass(const FViewInfo& View)
{
	return GetMainTAAPassConfig(View) == EMainTAAPassConfig::TSR &&
		View.Family->EngineShowFlags.PostProcessing;
}
...
{
	return ((CVarTSRFlickeringEnable.GetValueOnRenderThread() != 0 && CVarTSRFlickeringPeriod.GetValueOnRenderThread() != 0.0f)) ||
		(CVarTSRThinGeometryCoverageMinKeepLineContrast.GetValueOnRenderThread() > 0 && ShouldAddTSRMainThinGeometryCoveragePass());
}
...
void AddTSRMeasureThinGeometryCoverage(FRDGBuilder& GraphBuilder, FGlobalShaderMap* ShaderMap, const FSceneTextures& SceneTextures, const FScreenPassTexture& ThinGeometryCoverage)
{
	...
	EThinGeometryShadingRange ShadingRange = GetShadingRange();
	...
	FComputeShaderUtils::AddPass(
		...
		RDG_EVENT_NAME("TSR MeasureThinGeometryCoverage(#%d%s) %dx%d", 
			...
			GetShadingRangeName(ShadingRange),
			ViewRect.Width(), 
			ViewRect.Height()),
...
FTSRPassConfig GetTSRMainPassConfig(const FViewInfo& View)
{
	...
	{
		static const IConsoleVariable* CVar = IConsoleManager::Get().FindConsoleVariable(TEXT("r.MotionVectorSimulation"));
		bool MotionVectorSimulation = CVar ? CVar->GetInt() != 0 : false;
		PassConfig.MotionBlurValidInCurrentFrame = (MotionVectorSimulation || (!View.bCameraCut && !View.bPrevTransformsReset));
	}
#if UE_BUILD_OPTIMIZED_SHOWFLAGS
	...
	FTSRHistory& OutputHistory)
{
	...
	FIntPoint HistoryGuideExtent;
	{
		...
		FIntPoint MaxRenderingViewSize = FSceneRenderer::ApplyResolutionFraction(View.UnconstrainedViewRect.Size(), ResolutionFractionUpperBound);
	...
	static FName OverviewName = FName("Overview");
	static FName PerformanceOverviewName = FName("PerformanceOverview");
	...
	bool bAllocateOutputMip1 = PassInputs.bGenerateOutputMip1;
	bool bWriteOutputMip1 = PassInputs.bGenerateOutputMip1 && PassConfig.MotionBlurValidInCurrentFrame;

	FTSRHistoryTextures History;
	const int32 HistoryColorGuideSliceCountWithoutResurrection = bSupportsAlpha ? 2 : 1;
	{
		{
			bool bRequires2Mips = HistorySize == OutputRect.Size() && bAllocateOutputMip1;
	...
	int32 ThinGeometryTextureIndex = 2;
	{
		...
		PermutationVector.Set<FTSRDilateVelocityCS::FThinGeometryEdgeReprojectionDim>(PassConfig.ThinGeometryDetectionEnable);
	...
	if (PassConfig.ThinGeometryDetectionEnable)
	{
		const float MinKeepLineContrast = FMath::Clamp(CVarTSRThinGeometryCoverageMinKeepLineContrast.GetValueOnRenderThread(), 0.0f, 1.0f);
		const float TranslucencyRejectionThreshold = FMath::Clamp(CVarTSRThinGeometryDetectionRejectTranslucency.GetValueOnRenderThread(), 0.0f, 1.0f); 
		const bool EnableVaryingRangeDetection = (GetShadingRange() == EThinGeometryShadingRange::VaryingRange)
			&& ((FlickeringFramePeriod > 0.0f) || MinKeepLineContrast != 0.0f);
		{
			...
			PermutationVector.Set<FTSRDetectThinGeometryCS::FVaryingRangeDetectionDim>(EnableVaryingRangeDetection);
			PermutationVector.Set<FTSRConvolutionNetworkShader::FPermutationDomain>(ConvolutionNetworkPermutationVector);
			PermutationVector.Set<FTSRDetectThinGeometryCS::FIntensityLineDetectionDim>(MinKeepLineContrast != 0.0f);
			PermutationVector.Set<FTSRDetectThinGeometryCS::FRejectTranslucencyDim>(TranslucencyRejectionThreshold != 0.0f);
			...
			PassParameters->MinKeepLineContrast = MinKeepLineContrast;
			PassParameters->HighContrastLineFadeRateInsideRegion = FMath::Clamp(CVarTSRThinGeometryHighContrastLineFadeRateInsideRegion.GetValueOnRenderThread(), 0.0f, 1.0f);
			PassParameters->HighContrastLineFadeRate = FMath::Clamp(CVarTSRThinGeometryHighContrastLineFadeRate.GetValueOnRenderThread(), 0.0f, 1.0f);
			PassParameters->HighContrastLineWeight = FMath::Clamp(CVarTSRThinGeometryHighContrastLineWeight.GetValueOnRenderThread(), 0.0f, 1.0f);
			PassParameters->SceneDepthTexture = PassInputs.SceneDepth.Texture;
			...
			// Coverage texture
			{
				if (bCameraCut)
				{
					PassParameters->ReprojectedHistoryCoverageTexture = GraphBuilder.CreateSRV(BlackDummy);
					PassParameters->CurrentCoverageTexture = GraphBuilder.CreateSRV(FRDGTextureSRVDesc(BlackDummy));
					PassParameters->FlickeringLumaTexture = GraphBuilder.CreateSRV(FRDGTextureSRVDesc(WhiteDummy));
				}
				else
				{
					...
					if (PassInputs.FlickeringInputTexture.IsValid())
					{
						...
						if (MinKeepLineContrast != 0.0f && ArrayIndex == 1)
						{
							PassParameters->FlickeringLumaTexture = GraphBuilder.CreateSRV(
								FRDGTextureSRVDesc::CreateForSlice(PassInputs.FlickeringInputTexture.Texture, /* SliceIndex = */ ArrayIndex - 1));
						}
						else
						{
							PassParameters->FlickeringLumaTexture = GraphBuilder.CreateSRV(FRDGTextureSRVDesc(WhiteDummy));
		...
		if(CVarTSRThinGeometryDetectionWeightRelaxation.GetValueOnRenderThread() != 0)
		{
			...
			FTSRWeightRelaxationCS::FPermutationDomain PermutationVector;
			PermutationVector.Set<FTSRConvolutionNetworkShader::FPermutationDomain>(ConvolutionNetworkPermutationVector);
			PermutationVector.Set<FTSRWeightRelaxationCS::FRejectTranslucencyDim>(TranslucencyRejectionThreshold != 0.0f);
			PermutationVector = FTSRWeightRelaxationCS::RemapPermutation(PermutationVector);

			const int32 GroupTileSize = 32;
			const int32 TileOverscan = 0;//FMath::Clamp(CVarTSRShadingTileOverscan.GetValueOnRenderThread(), 3, GroupTileSize / 2 - 1);
			const int32 TileSize = GroupTileSize - 2 * TileOverscan;

			FTSRWeightRelaxationCS::FParameters* PassParameters = GraphBuilder.AllocParameters<FTSRWeightRelaxationCS::FParameters>();
			PassParameters->CommonParameters = CommonParameters;
			PassParameters->TileOverscan = TileOverscan;
			PassParameters->ThinGeometryTextureIndex = ThinGeometryTextureIndex;
			PassParameters->MaxRelaxationWeight = FMath::Clamp(CVarTSRThinGeometryCoverageMaxRelaxationWeight.GetValueOnRenderThread(), 0.0f, 1.0f);
			PassParameters->TranslucencyRejectionThreshold = TranslucencyRejectionThreshold;
			PassParameters->bIntensityLineDetectionEnabled = MinKeepLineContrast != 0.0f;
			...
			TShaderMapRef<FTSRWeightRelaxationCS> ComputeShader(View.ShaderMap, PermutationVector);
			FComputeShaderUtils::AddPass(
				GraphBuilder,
	...
	FRDGTextureSRVRef MoireHistoryTexture = nullptr;
	{
		...
		PassParameters->RejectShadingMode = CVarTSRShadingRejectionMode.GetValueOnRenderThread();
		PassParameters->TotalVariationCountInvThreshold = FVector2f(
			(1.0 - FMath::Pow(1.0 - 0.05 /*MinBlendFinal*/, FlickeringFramePeriod)),
			(1.0 - FMath::Pow(1.0 - 0.05 /*MinBlendFinal*/, 3.0f * FlickeringFramePeriod))); // Total variation count for thin geometry region.
		PassParameters->ThinGeometryAffectMoireError = CVarTSRThinGeometryDetectionAntiFlickering.GetValueOnRenderThread();
	...
	FRDGTextureSRVRef SceneColorOutputEighthResTextureSRV = nullptr;
	{
		...
		if (bWriteOutputMip1 && HistorySize == OutputRect.Size())
		{
	...
	if (HistorySize != OutputRect.Size())
	{
		...
		{
			FIntPoint MipClampedOutputExtent = FIntPoint(FMath::Max(OutputExtent.X, bAllocateOutputMip1 ? 2 : 1), 
				                                         FMath::Max(OutputExtent.Y, bAllocateOutputMip1 ? 2 : 1));
			FRDGTextureDesc OutputDesc = FRDGTextureDesc::Create2D(
				MipClampedOutputExtent,
				ColorFormat,
				FClearValueBinding::None,
				/* InFlags = */ TexCreate_ShaderResource | TexCreate_UAV | TexCreate_RenderTargetable,
				/* NumMips = */ bAllocateOutputMip1 ? 2 : 1);
		...
		FComputeShaderUtils::AddPass(
			GraphBuilder,
			RDG_EVENT_NAME("TSR ResolveHistory(#%d WaveSize=%d%s%s%s) %dx%d",
				PermutationVector.ToDimensionValueId(),
