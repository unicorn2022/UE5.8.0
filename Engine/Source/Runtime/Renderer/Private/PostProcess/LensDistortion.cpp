...
TAutoConsoleVariable<float> CVarLensDistortionPaniniD(
	...
	TEXT("Note: the projection is automatically disabled when the view FOV is too wide for Panini to\n")
	TEXT("represent (beyond ~140 degrees at D=1); the LUT cannot be built and the effect is skipped."),
	ECVF_RenderThreadSafe);
...
FLensDistortionLUT FPaniniProjectionConfig::GenerateLUTPasses(FRDGBuilder& GraphBuilder, const FViewInfo& View) const
{
	...
	/** 
	* Panini is undefined at/near a 180 degree FOV.
	* Bail out with a disabled LUT so callers (which gate on FLensDistortionLUT::IsEnabled) simply skip the pass.
	*/
	if (!FMath::IsFinite(ScreenPosToPaniniFactor.X) || !FMath::IsFinite(ScreenPosToPaniniFactor.Y))
	{
		return FLensDistortionLUT();
	}
	...
	float ResolutionFraction;
	{
		...
		if (!FMath::IsFinite(ResolutionFraction) || ResolutionFraction <= 1.0f || ResolutionFraction >= 2.0f)
		{
			return FLensDistortionLUT();
		}
	}
