...
class FPathTracingDebugRG : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		// extra parameters required for path compacting kernel
		SHADER_PARAMETER(int32, FirstBounce)
		SHADER_PARAMETER_RDG_BUFFER_UAV(RWStructuredBuffer<FPathTracingDebugPackedPathState>, PathStateData)
		SHADER_PARAMETER_RDG_BUFFER_UAV(RWBuffer<uint>, ActivePaths)
		SHADER_PARAMETER_RDG_BUFFER_UAV(RWBuffer<uint>, NextActivePaths)
		SHADER_PARAMETER_RDG_BUFFER_UAV(RWBuffer<uint>, NumPathStates)
...
void FDeferredShadingSceneRenderer::PreparePathTracing(const FViewInfo& View, const FScene& Scene, TArray<FRHIRayTracingShader*>& OutRayGenShaders)
{
		...
		&& PathTracing::ShouldCompilePathTracingShadersForProject(ViewFamily.GetShaderPlatform()))
	{
		...
		{
			{
				FPathTracingDebugRG::FPermutationDomain PermutationVector = GetPathTracingDebugRGPermutation(View, Scene, true, false, DebugMode);
			...
			if (DebugMode == PATH_TRACER_DEBUG_VIZ_PRIMARY_RAYS)
			{
				...
				{
					FPathTracingDebugRG::FPermutationDomain PermutationVector = GetPathTracingDebugRGPermutation(View, Scene, false, true, DebugMode);
					auto RayGenShader = GetGlobalShaderMap(ViewFamily.GetShaderPlatform())->GetShader<FPathTracingDebugRG>(PermutationVector);
					OutRayGenShaders.Add(RayGenShader.GetRayTracingShader());
				}
				if (CompactionDepth > 0)
				{
					FPathTracingDebugRG::FPermutationDomain PermutationVector = GetPathTracingDebugRGPermutation(View, Scene, false, false, DebugMode);
					auto RayGenShader = GetGlobalShaderMap(ViewFamily.GetShaderPlatform())->GetShader<FPathTracingDebugRG>(PermutationVector);
					OutRayGenShaders.Add(RayGenShader.GetRayTracingShader());
				}
			}
		...
		else
		{
			...
			{
				// camera ray kernel
				FPathTracingRG::FPermutationDomain PermutationVector = GetPathTracingRGPermutation(View, Scene, true, false);
			...
			if (CompactionDepth != 1)
			{
				...
				FPathTracingRG::FPermutationDomain PermutationVector = GetPathTracingRGPermutation(View, Scene, false, true);
				auto RayGenShader = GetGlobalShaderMap(ViewFamily.GetShaderPlatform())->GetShader<FPathTracingRG>(PermutationVector);
				OutRayGenShaders.Add(RayGenShader.GetRayTracingShader());
			}
			if (CompactionDepth > 0)
			{
				...
				FPathTracingRG::FPermutationDomain PermutationVector = GetPathTracingRGPermutation(View, Scene, false, false);
				auto RayGenShader = GetGlobalShaderMap(ViewFamily.GetShaderPlatform())->GetShader<FPathTracingRG>(PermutationVector);
				OutRayGenShaders.Add(RayGenShader.GetRayTracingShader());
			}
			{
	...
	FPathTracingResources& PathTracingResources)
{
	...
	if (int DebugMode = GetPathTracingVisualizationMode(); DebugMode >= 0)
	{
		...
		FRDGBuffer* PathStateData = nullptr;
		{
			...
			PathStateData = GraphBuilder.CreateBuffer(FRDGBufferDesc::CreateStructuredDesc(sizeof(FPathTracingDebugPackedPathState), NumPaths), TEXT("PathTracer.PathStateData"));
		}
		...
		for (int SampleIndex = 0; SampleIndex < SamplesPerPixel; SampleIndex++)
		{
			...
			for (int Bounce = 0; Bounce <= CompactionDepth; Bounce++)
			{
				...
				GraphBuilder.AddPass(
					RDG_EVENT_NAME("Path Tracing Visualize [%d/%d] - Bounce %d%s", SampleIndex + 1, SamplesPerPixel, Bounce, bUseCompaction ? "" : "+"),
					PassParameters,
