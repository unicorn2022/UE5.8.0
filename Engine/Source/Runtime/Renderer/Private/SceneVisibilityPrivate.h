...
private:
	...
	FSceneCullingRendererHierarchicalCullingQuery* SceneCullingQuery = nullptr;
	...
	bool bSLWUsesMotionVectorWorldOffset = false;
...
private:
	FRelevancePacket* CreateRelevancePacket()
	{
		...
		return Packets.Emplace_GetRef(new FRelevancePacket(TaskData, View, ViewIndex, ViewData, SceneCullingQuery, MarkMasks, PrerequisitesTask));
	}
...
public:
	...
	{
		FProcessVisitor(FGPUOcclusionPacket& InPacket, FGlobalDynamicVertexBuffer& InDynamicVertexBuffer)
			: Packet(InPacket)
...
public:
	FGPUOcclusionSerial(FVisibilityViewPacket& InViewPacket)
		...
		, ProcessVisitor(Packet, DynamicVertexBuffer)
	{}
