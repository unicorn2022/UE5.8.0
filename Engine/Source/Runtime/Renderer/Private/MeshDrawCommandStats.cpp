			...
			if (bShowStats)
			{
				...
				if (StatCollection)
				{
					for (const FCollectionCategory& Category : StatCollection->Categories)
					{
						...
						if (DrawData && DrawData->PrimitiveCount > 0)
						{
							...
							if (Count > 0)
							{
								...
								OutMessages.Add(Severity, FText::FromString(FString::Printf(TEXT("%5" UINT64_FMT "K / %5lldK - %s (%s)"),
									NumberToK(Count),
									NumberToK(Budget),
					...
					if (TotalTracked > 0)
					{
						...
						OutMessages.Add(Severity, FText::FromString(FString::Printf(TEXT("%5lldK - Total Tracked"), NumberToK(TotalTracked))));
					}
				}
				...
				for (const TPair<FName, FDrawData>& Pair : UntrackedDrawData)
				{
					...
					if (Count > 0)
					{
						OutMessages.Add(FCoreDelegates::EOnScreenMessageSeverity::Warning, FText::FromString(FString::Printf(TEXT("\t%5lldK - %s%s"), NumberToK(Count), *(Name.ToString()), *PassFriendlyNames)));
				...
				{
					OutMessages.Add(FCoreDelegates::EOnScreenMessageSeverity::Warning, FText::FromString(FString::Printf(TEXT("%5lldK - Total Untracked"), NumberToK(TotalUntracked))));
				}
				...
				if (TotalBudget)
				{
					...
					OutMessages.Add(Severity, FText::FromString(FString::Printf(TEXT("%5lldK / %5lldK - Total"), NumberToK(TotalTracked + TotalUntracked), NumberToK(TotalBudget))));
				}
				else
				{
					OutMessages.Add(FCoreDelegates::EOnScreenMessageSeverity::Info, FText::FromString(FString::Printf(TEXT("\t%5lldK - Total"), NumberToK(TotalCount))));
				}
			}
		});
}
