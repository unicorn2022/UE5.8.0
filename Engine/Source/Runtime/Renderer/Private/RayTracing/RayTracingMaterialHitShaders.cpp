...
public:
	...
	static bool ShouldCompilePermutation(const FMeshMaterialShaderPermutationParameters& Parameters)
	{
		...
		static const auto CVarRayTracingUseTextureLod = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.RayTracing.UseTextureLod"));
		const bool bRayTracingUseTextureLod = CVarRayTracingUseTextureLod && CVarRayTracingUseTextureLod->GetValueOnAnyThread() != 0;
		...
		return IsSupportedVertexFactoryType(Parameters.VertexFactoryType)
			...
			&& bRayTracingUseTextureLod == UseRayConeTextureLod
	...
	TShaderRef<FMaterialCHS>& OutRayHitGroupShader)
{
	...
	{
		static const auto CVarRayTracingUseTextureLod = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.RayTracing.UseTextureLod"));
		const bool bUseTextureLOD = CVarRayTracingUseTextureLod && CVarRayTracingUseTextureLod->GetValueOnAnyThread() != 0;
	...
	bool& bOutIsUsingFallbackRTPSO)
{
	...
	if (PipelineState)
	{
        ...
        Scene->bLastFrameUsedFallback = false;
	}
	else
	{
...
#if WITH_EDITOR
        static const FText Message = NSLOCTEXT("Renderer", "RayTracingPSONotReady", "Ray Tracing PSO is not ready yet, using fallback.");
		OnGetOnScreenMessages.AddLambda([](FScreenMessageWriter& ScreenMessageWriter) -> void
        {
            ScreenMessageWriter.DrawLine(Message);
        });
        if (!Scene->bLastFrameUsedFallback)
        {
            UE_LOGF(LogRenderer, Log, "%ls", *Message.ToString());
            Scene->bLastFrameUsedFallback = true;
        }
#endif
	}
...
void FDeferredShadingSceneRenderer::SetupMaterialRayTracingHitGroupBindings(FRDGBuilder& GraphBuilder, FViewInfo& View, TConstArrayView<FRayTracingShaderBindingData> RayTracingShaderBindings)
{
		...
		bEnableMaterials, bEnableShadowMaterials, &RayTracingSBT = Scene->RayTracingSBT, &RayTracingMeshCommands = Scene->CachedRayTracingMeshCommands](const FRayTracingShaderBindingData& DirtyShaderBinding, FRayTracingLocalShaderBindingWriter* BindingWriter)
	{
		...
		if (bIsMeshDecalShader)
		{
			...
			Binding.ShaderIndexInPipeline = HiddenMeshDecalHitGroupIndex;
