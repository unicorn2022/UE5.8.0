...
public:
	...
	FRHIShaderResourceView* GetPersistentInlineBindingDataBufferSRV(FRayTracingPersistentShaderBindingTableID PersistentSBTID) const
	{ 
		FRHIShaderResourceView* InlineBindingDataBufferSRV = nullptr;
		if (PersistentSBTs.IsValidIndex(PersistentSBTID))
		{
			InlineBindingDataBufferSRV = PersistentSBTs[PersistentSBTID].InlineBindingDataPooledBuffer->GetSRV();
		}
		return InlineBindingDataBufferSRV;
	}
	...
	RENDERER_API void FreeStaticRange(FRayTracingSBTAllocation* Allocation);
	 ...
	 */	
	RENDERER_API void FlushAllocationsToClear(FRHICommandList& RHICmdList);
...
private:
	...
	struct FAllocationKey
	{
		...
		{
			return HashCombine(HashCombine(GetTypeHash(Key.Geometry), GetTypeHash(Key.Flags)), GetTypeHash(Key.SegmentCount));
		}
	};
	...
	const uint32 NumShaderSlotsPerGeometrySegment = 0;					//< Number of slots per geometry segment (engine wide fixed)
	...
	TIndirectArray<FRayTracingSBTAllocation> DynamicAllocationPool;      //< Dynamic allocation pool (for faster allocations)
	uint32 NumUsedDynamicAllocations = 0;                                //< Number of dynamic allocations currently used
	...
	uint32 StartDynamicRangeOffsetToMarkDirty = 0;						 //< Start SBT record offset that needs to be marked dirty
};
