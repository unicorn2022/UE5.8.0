...
#include "MegaLights/MegaLightsInternal.h"
...
void RenderHairBasePass(
	...
	TArray<FViewInfo>& Views,
	FInstanceCullingManager& InstanceCullingManager,
	TSharedPtr<FMegaLightsFrameTemporaries> MegaLightsContext)
{
	for (FViewInfo& View : Views)
	{
		const bool bIsViewCompatible = IsHairStrandsEnabled(EHairStrandsShaderType::Strands, View.GetShaderPlatform());
		...
		// Late composition of hair into to custom depth (avoid extra rasterization by reusing primary raster outputs)
		if (SceneTextures.CustomDepth.Depth && EnumHasAllFlags(SceneTextures.SetupMode, ESceneTextureSetupMode::CustomDepth))
		{
			if (HairStrands::HasViewHairStrandsData(View))
			{
				HairStrands::DrawCustomDepth(GraphBuilder, View, View.ViewRect, SceneTextures.CustomDepth.Depth);
			}
		}
	}

	// Megalights registers early Scene.HZB / Scene.ClosestHZB into FMegaLightsParameters. When IsHairVisibilityUpdateSceneHZBEnabled() is enabled, 
	// new scene HZB textures are created. The Megalights context needs to be patched with these new HZB texture
	if (MegaLightsContext && IsHairVisibilityUpdateSceneHZBEnabled())
	{
		MegaLightsContext->UpdateHZB();
	}
}
...
TRDGUniformBufferRef<FHairStrandsViewUniformParameters> CreateDefaultHairStrandsViewUniformBuffer(FRDGBuilder& GraphBuilder)
{
...
TRDGUniformBufferRef<FVirtualVoxelParameters> CreateDefaultHairStrandsVoxelUniformBuffer(FRDGBuilder& GraphBuilder)
{
	FRDGBufferSRVRef DummyBuffer = GraphBuilder.CreateSRV(GSystemTextures.GetDefaultBuffer(GraphBuilder, 4u, 0u), PF_R32_UINT);
	FRDGBufferSRVRef DummyNodeDescBuffer = GraphBuilder.CreateSRV(GSystemTextures.GetDefaultStructuredBuffer(GraphBuilder, sizeof(FPackedVirtualVoxelNodeDesc), 0u));

	FVirtualVoxelParameters* Out = GraphBuilder.AllocParameters<FVirtualVoxelParameters>();
	Out->Common.PageCountResolution = FIntVector::ZeroValue;
	Out->Common.CPUMinVoxelWorldSize = 0.f;
	Out->Common.PageTextureResolution = FIntVector::ZeroValue;
	Out->Common.PageCount = 0;
	Out->Common.PageResolution = 0;
	Out->Common.PageResolutionLog2 = 0;
	Out->Common.PageIndexCount = 0;
	Out->Common.IndirectDispatchGroupSize = 0;
	Out->Common.NodeDescCount = 0;
	Out->Common.JitterMode = 0;
	Out->Common.DensityScale = 0;
	Out->Common.DensityScale_AO = 0;
	Out->Common.DensityScale_Shadow = 0;
	Out->Common.DensityScale_Transmittance = 0;
	Out->Common.DensityScale_Environment = 0;
	Out->Common.DensityScale_Raytracing = 0;
	Out->Common.DepthBiasScale_Shadow = 0;
	Out->Common.DepthBiasScale_Transmittance = 0;
	Out->Common.DepthBiasScale_Environment = 0;
	Out->Common.SteppingScale_Shadow = 0;
	Out->Common.SteppingScale_Transmittance = 0;
	Out->Common.SteppingScale_Environment = 0;
	Out->Common.SteppingScale_Raytracing = 0;
	Out->Common.HairCoveragePixelRadiusAtDepth1 = 0;
	Out->Common.Raytracing_ShadowOcclusionThreshold = 0;
	Out->Common.Raytracing_SkyOcclusionThreshold = 0;
	Out->Common.TranslatedWorldOffset = FVector3f::ZeroVector;
	Out->Common.TranslatedWorldOffsetStereoCorrection = FVector3f::ZeroVector;
	Out->Common.AllocationFeedbackEnable = 0;
	Out->Common.AllocatedPageCountBuffer = DummyBuffer;
	Out->Common.PageIndexBuffer = DummyBuffer;
	Out->Common.PageIndexCoordBuffer = DummyBuffer;
	Out->Common.NodeDescBuffer = DummyNodeDescBuffer;
	Out->Common.CurrGPUMinVoxelSize = DummyBuffer;
	Out->Common.NextGPUMinVoxelSize = DummyBuffer;
	Out->PageTexture = GSystemTextures.GetVolumetricBlackDummy(GraphBuilder);
	return GraphBuilder.CreateUniformBuffer(Out);
}
...
bool HasHairStrandsVisible(TArrayView<const FViewInfo> Views)
{
	for (const FViewInfo& View : Views)
	{
