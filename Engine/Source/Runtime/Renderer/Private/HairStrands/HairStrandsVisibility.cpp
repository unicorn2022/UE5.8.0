...
#include "HairStrands/HairStrandsForwardRaster.h"
#include "HairStrands/HairStrandsLUT.h"
#include "HairStrands/HairStrandsMacroGroup.h"
#include "HairStrands/HairStrandsTile.h"
...
static FAutoConsoleVariableRef CVarHairStrands_Selection_CoverageThreshold(TEXT("r.HairStrands.Selection.CoverageThreshold"), GHairStrands_Selection_CoverageThreshold, TEXT("Coverage threshold for making hair strands outline selection and custom depth finer"));
...
int32 GHairVisibilityUpdateSceneHZB = 0;
static FAutoConsoleVariableRef CVarHairVisibilityUpdateSceneHZB(TEXT("r.HairStrands.Visibility.UpdateSceneHZB"), GHairVisibilityUpdateSceneHZB, TEXT("Update scene HZB with hair data (experimental)."));
...
void SetUpViewHairRenderInfo(const FViewInfo& ViewInfo, bool bEnableMSAA, FVector4f& OutHairRenderInfo, uint32& OutHairRenderInfoBits, uint32& OutHairComponents, float FinalResolutionScale)
{
	...
	const FMinHairRadiusAtDepth1 MinHairRadius = ComputeMinStrandRadiusAtDepth1(
		FIntPoint(EffectiveViewRect.Width(), EffectiveViewRect.Height()), ViewInfo.FOV, HairVisibilitySampleCount, RasterizationScaleOverride, ViewInfo.ViewMatrices.GetOrthoDimensions().X, FinalResolutionScale);
...
void SetUpViewHairRenderInfo(const FViewInfo& ViewInfo, FVector4f& OutHairRenderInfo, uint32& OutHairRenderInfoBits, uint32& OutHairComponents, float FinalResolutionScale)
{
	SetUpViewHairRenderInfo(ViewInfo, IsMsaaEnabled(ViewInfo.GetShaderPlatform()), OutHairRenderInfo, OutHairRenderInfoBits, OutHairComponents, FinalResolutionScale);
}
...
bool IsHairVisibilityUpdateSceneHZBEnabled()
{
	return GHairVisibilityUpdateSceneHZB > 0;
}
...
//Used by DeferredLightGlobalPSOCollector
EPixelFormat GetHairLightSampleFormat()
{
	return FHairLightSampleClearPS::GetHairLightSampleFormat();
}
...
protected:
	...
	{
		return IsCompatibleWithHairVisibility(Parameters) && HairStrands::IsHairStrandsVF(Parameters.VertexFactoryType);
	}
...
public:
	...
	static bool ShouldCompilePermutation(const FMeshMaterialShaderPermutationParameters& Parameters)
	{
		return IsCompatibleWithHairStrands(Parameters.Platform, Parameters.MaterialParameters) && HairStrands::IsHairStrandsVF(Parameters.VertexFactoryType);
	}
...
static bool ShouldRenderHairStrands(ERHIFeatureLevel::Type FeatureLevel, const FMaterial& Material, const FVertexFactoryType* VFType, bool bPrimitiveRenderInPass)
{
	...
	const bool bIsHairStrandsFactory = HairStrands::IsHairStrandsVF(VFType);
	...
	FRDGBufferRef IndirectArgBuffer)
{
	...
	auto MaterialPass = [&](FRDGTextureRef RenderTarget, EHairMaterialPassFilter Filter)
	{
...

		{
			...
			SetUpViewHairRenderInfo(*ViewInfo, bEnableMSAA, ViewInfo->CachedViewUniformShaderParameters->HairRenderInfo, ViewInfo->CachedViewUniformShaderParameters->HairRenderInfoBits, ViewInfo->CachedViewUniformShaderParameters->HairComponents, ViewInfo->Family->FinalResolutionScale);
...
public:
	...
	static bool ShouldCompilePermutation(const FMeshMaterialShaderPermutationParameters& Parameters)
	{
		if (!HairStrands::IsHairStrandsVF(Parameters.VertexFactoryType))
		{
			return false;
		}
...
#if RHI_RAYTRACING
static FRDGTextureRef CreateLightChannelMaskTexture(FRDGBuilder& GraphBuilder, const FIntPoint& Resolution)
{
	...
	const FRDGTextureRef& SceneDepthTexture)
{
	...
	FRDGTextureRef OutLightChannelMask = CreateLightChannelMaskTexture(GraphBuilder, View.ViewRect.Size());
	...
	FRDGTextureRef& OutLightChannelMask)
{
	...
	if (bLightingChannel)
	{
		OutLightChannelMask = CreateLightChannelMaskTexture(GraphBuilder, View.ViewRect.Size());
	}
#else
	...
	if (IsHairVisibilityUpdateSceneHZBEnabled())
	{
		BuildHZB(
			GraphBuilder,
			OutDepthTexture,
	...
	FRDGTextureRef& OutFurthestHZBTexture)
{
	...
	BuildHZB(
		...
		HairDepthTexture,
		/* VisBufferTexture = */ nullptr,
		FSceneRendererBase::GetHZBViewRect(View),
		View.GetFeatureLevel(),
		View.GetShaderPlatform(),
		TEXT("HZBHairClosest"),
	...
	for (const FHairStrandsMacroGroupData& MacroGroup : MacroGroupDatas)
	{
		...
		for (const FHairStrandsMacroGroupData::PrimitiveInfo& PrimitiveInfo : PrimitiveSceneInfos)
		{
			...
			if (bClassification || bUseHWRaster)
			{
				...
				Parameters->VS.RadiusAtDepth1 = ComputeMinStrandRadiusAtDepth1(Resolution, ViewInfo.FOV, 1, -1.0f, ViewInfo.ViewMatrices.GetOrthoDimensions().X, ViewInfo.Family->FinalResolutionScale).Primary;
...
///////////////////////////////////////////////////////////////////////////////////////////////////
// Selection outline, HitProxyId, CustomDepth

class FHairStrandsEmitDepthPS : public FGlobalShader
{
	DECLARE_GLOBAL_SHADER(FHairStrandsEmitDepthPS);
	SHADER_USE_PARAMETER_STRUCT(FHairStrandsEmitDepthPS, FGlobalShader);

	class FColor : SHADER_PERMUTATION_BOOL("PERMUTATION_COLOR");
	class FTile : SHADER_PERMUTATION_BOOL("PERMUTATION_TILE");
	using FPermutationDomain = TShaderPermutationDomain<FColor, FTile>;

	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		SHADER_PARAMETER(float, CoverageThreshold)
		SHADER_PARAMETER(FVector2f, InvViewportResolution)
		SHADER_PARAMETER(uint32, MaxMaterialCount)
		SHADER_PARAMETER(uint32, bHasValidHairOnlyDepth)
		SHADER_PARAMETER(uint32, bDiscardIfZero)
		SHADER_PARAMETER_STRUCT_REF(FViewUniformShaderParameters, View)
		...
		SHADER_PARAMETER_RDG_BUFFER_SRV(Buffer<uint>, ActiveMaterialIdBuffer)
		SHADER_PARAMETER_STRUCT_INCLUDE(FHairStrandsTilePassVS::FParameters, TileData)
		RENDER_TARGET_BINDING_SLOTS()
	END_SHADER_PARAMETER_STRUCT()
	...
	{
		FGlobalShader::ModifyCompilationEnvironment(Parameters, OutEnvironment);
		OutEnvironment.SetDefine(TEXT("SHADER_EMIT_DEPTH"), 1);
	}
};
IMPLEMENT_GLOBAL_SHADER(FHairStrandsEmitDepthPS, "/Engine/Private/HairStrands/HairStrandsHitProxy.usf", "EmitPS", SF_Pixel);

enum class EHairStrandsEmitDepthMode : uint8
{
	Selection,
	HitProxy,
	CustomDepth,
...
static void AddHairStrandsEmitDepthPass(
	FRDGBuilder& GraphBuilder,
	const FViewInfo& View,
	const FIntRect& ViewportRect,
	const FHairStrandsMacroGroupDatas& MacroGroupDatas,
	const EHairStrandsEmitDepthMode Mode,
	const FHairStrandsTiles& TileData,
	FRDGTextureRef VisNodeIndex,
	FRDGBufferRef VisNodeData,
	FRDGTextureRef CoverageTexture,
	FRDGTextureRef HairOnlyDepthTexture,
	FRDGTextureRef OutHitProxyTexture,
	FRDGTextureRef OutEmitDepthTexture)
{
	...
	TArray<uint32> ActiveMaterialId;
	ActiveMaterialId.Init(0u, View.HairStrandsMeshElements.Num());
	for (const FHairStrandsMacroGroupData& MacroGroupData : MacroGroupDatas)
	{
		for (const FHairStrandsMacroGroupData::PrimitiveInfo& PrimitiveInfo : MacroGroupData.PrimitivesInfos)
		{
			if (ActiveMaterialId.IsValidIndex(PrimitiveInfo.MaterialId) && PrimitiveInfo.PrimitiveSceneProxy)
			{
				uint32 Value = 0u;
				switch (Mode)
				{
					case EHairStrandsEmitDepthMode::Selection   : Value = PrimitiveInfo.PrimitiveSceneProxy->IsSelected() ? 1u : 0u; break;
					case EHairStrandsEmitDepthMode::CustomDepth : Value = PrimitiveInfo.PrimitiveSceneProxy->ShouldRenderCustomDepth() ? 1u : 0u; break;
					case EHairStrandsEmitDepthMode::HitProxy    : Value = PrimitiveInfo.Mesh ? PrimitiveInfo.Mesh->BatchHitProxyId.GetColor().DWColor() : 0u; break;
				}
				ActiveMaterialId[PrimitiveInfo.MaterialId] = Value;
			}
		}
	}
	
	FRDGBufferRef ActiveMaterialIdBuffer = CreateUploadBuffer(GraphBuilder, TEXT("Hair.ActiveMaterialIdBuffer"), sizeof(uint32), ActiveMaterialId.Num(), ActiveMaterialId.GetData(), sizeof(uint32) * ActiveMaterialId.Num());
	auto* PassParameters = GraphBuilder.AllocParameters<FHairStrandsEmitDepthPS::FParameters>();
	PassParameters->View = View.ViewUniformBuffer;
	PassParameters->CoverageThreshold = FMath::Clamp(GHairStrands_Selection_CoverageThreshold, 0.f, 1.f);
	PassParameters->MaxMaterialCount = ActiveMaterialId.Num();
	PassParameters->InvViewportResolution = FVector2f(1.f/ViewportRect.Width(), 1.f/ViewportRect.Height());
	PassParameters->bDiscardIfZero = Mode != EHairStrandsEmitDepthMode::HitProxy ? 1u : 0u;
	...
	PassParameters->ActiveMaterialIdBuffer = GraphBuilder.CreateSRV(ActiveMaterialIdBuffer, PF_R32_UINT);
	if (PassParameters->HairOnlyDepthTexture == nullptr)
	{
		PassParameters->HairOnlyDepthTexture = GSystemTextures.GetDepthDummy(GraphBuilder);
	}
	...
	switch (Mode)
	{
		case EHairStrandsEmitDepthMode::Selection:
		{
			PassParameters->RenderTargets.DepthStencil = FDepthStencilBinding(OutEmitDepthTexture, ERenderTargetLoadAction::ELoad, FExclusiveDepthStencil::DepthWrite_StencilWrite);
			DepthStencilState = TStaticDepthStencilState<true, CF_DepthNearOrEqual, true, CF_Always, SO_Keep, SO_Keep, SO_Replace>::GetRHI(); 
			StencilRef = 3;
			PassName = TEXT("Selection");
			bUseTile = false;
		}
		break;
		case EHairStrandsEmitDepthMode::CustomDepth:
		{
			PassParameters->RenderTargets.DepthStencil = FDepthStencilBinding(OutEmitDepthTexture, ERenderTargetLoadAction::ELoad, FExclusiveDepthStencil::DepthWrite_StencilNop);
			DepthStencilState = TStaticDepthStencilState<true, CF_DepthNearOrEqual, false>::GetRHI(); 
			StencilRef = 0xFF;
			PassName = TEXT("CustomDepth");
			bUseTile = true;
		}
		break;
		case EHairStrandsEmitDepthMode::HitProxy:
		{
			PassParameters->RenderTargets[0] = FRenderTargetBinding(OutHitProxyTexture, ERenderTargetLoadAction::ELoad);
			PassParameters->RenderTargets.DepthStencil = FDepthStencilBinding(OutEmitDepthTexture, ERenderTargetLoadAction::ELoad, FExclusiveDepthStencil::DepthWrite_StencilWrite);
			DepthStencilState = TStaticDepthStencilState<true, CF_DepthNearOrEqual>::GetRHI();
			StencilRef = 0xFF;
			PassName = TEXT("HitProxy");
			bUseTile = true;
		}
		break;
		default: check(false);
	...
	FHairStrandsEmitDepthPS::FPermutationDomain PermutationVector;
	PermutationVector.Set<FHairStrandsEmitDepthPS::FColor>(Mode == EHairStrandsEmitDepthMode::HitProxy);
	PermutationVector.Set<FHairStrandsEmitDepthPS::FTile>(bUseTile);
	auto PixelShader = View.ShaderMap->GetShader<FHairStrandsEmitDepthPS>(PermutationVector);
	if (!bUseTile)
	{
		...
		FPixelShaderUtils::AddFullscreenPass(
			GraphBuilder,
			View.ShaderMap,
			RDG_EVENT_NAME("HairStrands::EmitDepth(%s)", PassName),
			PixelShader,
			PassParameters,
			ViewportRect,
	...
	else
	{
		...
		GraphBuilder.AddPass(
			...
			PassParameters,
			ERDGPassFlags::Raster,
			[PassParameters, TileVertexShader, PixelShader, Viewport, TileType, DepthStencilState](FRDGAsyncTask, FRHICommandList& RHICmdList)
		{
...
// Draw hair strands custom depth value
void DrawCustomDepth(FRDGBuilder& GraphBuilder, const FViewInfo& View, const FIntRect& ViewportRect, FRDGTextureRef CustomDepthTexture)
{
	AddHairStrandsEmitDepthPass(
		GraphBuilder,
		View,
		ViewportRect,
		View.HairStrandsViewData.MacroGroupDatas,
		EHairStrandsEmitDepthMode::CustomDepth,
...
void DrawEditorSelection(FRDGBuilder& GraphBuilder, const FViewInfo& View, const FIntRect& ViewportRect, FRDGTextureRef SelectionDepthTexture)
{
	...
	AddHairStrandsEmitDepthPass(
		...
		ViewportRect,
		View.HairStrandsViewData.MacroGroupDatas,
		EHairStrandsEmitDepthMode::Selection,
		View.HairStrandsViewData.VisibilityData.TileData,
		View.HairStrandsViewData.VisibilityData.NodeIndex,
		View.HairStrandsViewData.VisibilityData.NodeVisData,
		View.HairStrandsViewData.VisibilityData.CoverageTexture,
	...
	FRDGTextureRef HitProxyDepthTexture)
{
	...
	AddHairStrandsEmitDepthPass(
		...
		View.ViewRect,
		MacroGroupDatas,
		EHairStrandsEmitDepthMode::HitProxy,
		TileData,
		VisNodeIndex,
		VisNodeData,
		CoverageTexture,
