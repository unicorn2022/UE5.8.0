...
static TAutoConsoleVariable<int32> CVarHairStrandsRaytracingEnableLinearSweptSpheres(
	TEXT("r.HairStrands.Raytracing.EnableLinearSweptSpheres"), 1,
	TEXT("Enable Linear Swept Spheres for hair strand raytracing (if the current hardware supports it)."),
	ECVF_RenderThreadSafe | ECVF_ReadOnly);
...
bool IsHairStrandsLinearSweptSpheresEnabled()
{
	// Are LSS primitives supported on the currently running device and are they enable for hair strands in this project?
	return GRHIGlobals.RayTracing.SupportsLinearSweptSpheres && CVarHairStrandsRaytracingEnableLinearSweptSpheres.GetValueOnAnyThread() != 0;
}
...
bool IsHairStrandsVF(const FMeshBatch* Mesh)
{
	if (Mesh)
	{
		return IsHairStrandsVF(Mesh->VertexFactory->GetType());
	}
	return false;
}
...
bool IsHairStrandsVF(const FVertexFactoryType* InVFType)
{
	// NOTE: The VF is defined in a different module, so we must search for it by name
	static const FVertexFactoryType* VFTypeA = FVertexFactoryType::GetVFByName(TEXT("FHairStrandsVertexFactory"));
	static const FVertexFactoryType* VFTypeB = FVertexFactoryType::GetVFByName(TEXT("FLinearSweptSpheresHairStrandsVertexFactory"));
	#if !UE_BUILD_SHIPPING
	if (IsGroomEnabled())
	{
		check(VFTypeA != nullptr);
		check(VFTypeB != nullptr);
	}
	#endif
	return InVFType == VFTypeA || InVFType == VFTypeB;
}
