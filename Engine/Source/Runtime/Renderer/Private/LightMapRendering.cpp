...
FMobileSingleLayerWaterPolicy::FMobileSingleLayerWaterPolicy()
{
}

bool FMobileSingleLayerWaterPolicy::ShouldCompilePermutation(const FMeshMaterialShaderPermutationParameters& Parameters)
{
	return IsMobilePlatform(Parameters.Platform) 
		&& !IsOpenGLPlatform(Parameters.Platform) // not enough samplers
		&& IsMobileDeferredShadingEnabled(Parameters.Platform)
		&& !FReadOnlyCVARCache::MobileAllowFramebufferFetch(Parameters.Platform)
		&& Parameters.MaterialParameters.ShadingModels.HasShadingModel(MSM_SingleLayerWater);
}

void FMobileSingleLayerWaterPolicy::ModifyCompilationEnvironment(const FMaterialShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment)
{
	OutEnvironment.SetDefine(TEXT("DIRECTIONAL_LIGHT_CSM"), TEXT("1"));
	OutEnvironment.SetDefine(TEXT("MOBILE_SINGLELAYERWATER_FALLBACK"), 0u);
	OutEnvironment.SetDefine(TEXT("MOBILE_SSR_ENABLED"), 1u); // should it be optional?
}

void FMobileSingleLayerWaterPolicy::GetVertexShaderBindings(
	const FPrimitiveSceneProxy* PrimitiveSceneProxy,
	const ElementDataType& ShaderElementData,
	const VertexParametersType* VertexShaderParameters,
	FMeshDrawSingleShaderBindings& ShaderBindings)
...
void FMobileSingleLayerWaterPolicy::GetPixelShaderBindings(
	const FPrimitiveSceneProxy* PrimitiveSceneProxy,
	const ElementDataType& ShaderElementData,
	const PixelParametersType* PixelShaderParameters,
	FMeshDrawSingleShaderBindings& ShaderBindings)
