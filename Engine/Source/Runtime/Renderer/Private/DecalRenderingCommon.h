...
namespace DecalRendering
{
	...
	/** Get the render target mode that a decal uses for a given stage at runtime and at cook time. Can return EDecalRenderTargetMode::None if there is no valid render target mode. */
	...
	/** Get the render target mode string */
