	...
	bool bCameraCut)
{
	...
	if (!bValid)
	{
		UE_LOGF(LogRenderer, Warning, "Warning: A viewport of resolution 0x0 was specified - VolumetricCloud not rendered.");
		return;
	}
...

	{
		...
		if (VolumetricReconstructRT[CurrentRT].IsValid() && FIntPoint(CurrentTargetResVec.X, CurrentTargetResVec.Y) != CurrentTargetRes)
		{
			...
			VolumetricReconstructRTForCloudOnTranslucent.SafeRelease();
		}
...
FRDGTextureRef FVolumetricRenderTargetViewStateData::GetOrCreateVolumetricReconstructRTForCloudOnTranslucent(FRDGBuilder& GraphBuilder)
{
	check(VolumetricReconstructRTResolution != FIntPoint::ZeroValue); // check that initialization has been done at least once

	if (!VolumetricReconstructRTForCloudOnTranslucent.IsValid())
	{
		FPooledRenderTargetDesc Desc = FPooledRenderTargetDesc::Create2DDesc(
		...
		GRenderTargetPool.FindFreeElement(GraphBuilder.RHICmdList, Desc, VolumetricReconstructRTForCloudOnTranslucent, TEXT("VolumetricRenderTarget.Reconstruct.WithFog"));
	}

	return GraphBuilder.RegisterExternalTexture(VolumetricReconstructRTForCloudOnTranslucent);
}

TRefCountPtr<IPooledRenderTarget> FVolumetricRenderTargetViewStateData::GetVolumetricReconstructRTForCloudOnTranslucent()
{
	return VolumetricReconstructRTForCloudOnTranslucent;
}
...
class FReconstructVolumetricRenderTargetPS : public FGlobalShader
{
	...
	using FPermutationDomain = TShaderPermutationDomain<FHistoryAvailable, FReprojectionBoxConstraint, FCloudMinAndMaxDepth, FApplyFog, FApplyLocalFogVolume>;
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(FVector2f, SourceViewRectMin)
		SHADER_PARAMETER(FVector2f, SourceViewRectSize)
	...
	bool bWaitFinishFence)
{
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		...
		const bool bShouldVolumetricCloudsApplyFogDuringUpsamplingForCloudOnTranslucent = ShouldVolumetricCloudsApplyFogDuringUpsamplingForCloudOnTranslucent(ViewInfo);
		...
		PermutationVector.Set<FReconstructVolumetricRenderTargetPS::FApplyFog>(bShouldVolumetricCloudsApplyFogDuringUpsamplingForCloudOnTranslucent && ShouldRenderFog(*ViewInfo.Family));
		PermutationVector.Set<FReconstructVolumetricRenderTargetPS::FApplyLocalFogVolume>(bShouldVolumetricCloudsApplyFogDuringUpsamplingForCloudOnTranslucent && ViewInfo.LocalFogVolumeViewData.GPUInstanceCount > 0);
		...
		if (bMinMaxDepth)
		{
			...
			{
				PassParameters->RenderTargets[3] = FRenderTargetBinding(VolumetricCloudRT.GetOrCreateVolumetricReconstructRTForCloudOnTranslucent(GraphBuilder), ERenderTargetLoadAction::ENoAction);
			}
		}
		else
		{
			...
			if (bShouldVolumetricCloudsApplyFogDuringUpsamplingForCloudOnTranslucent)
			{
				PassParameters->RenderTargets[2] = FRenderTargetBinding(VolumetricCloudRT.GetOrCreateVolumetricReconstructRTForCloudOnTranslucent(GraphBuilder), ERenderTargetLoadAction::ENoAction);
			}
		}
		...
		PassParameters->SourceViewRectMin = ViewInfo.ViewRect.Min;
		PassParameters->SourceViewRectSize = ViewInfo.ViewRect.Size();
...
class FComposeVolumetricRTOverScenePS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(FVector2f, ViewSeparateFogDepthOriginOffset)
...
{
	if (!Scene || !AnyViewRequiresProcessing(Views))
	{
		return;
	}
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		...
		const bool bShouldVolumetricCloudsApplyFogDuringUpsampling = ShouldVolumetricCloudsApplyFogDuringUpsampling(ViewInfo);

		// Check if we should apply fog separate composition on clouds
		check(!FogSeparateCompositionViewResources || FogSeparateCompositionViewResources->Num() > ViewIndex);
		const bool bApplyFogSeparateComposition = FogSeparateCompositionViewResources
			&& (*FogSeparateCompositionViewResources)[ViewIndex].FogSeparateCompositionTexture0 != nullptr
			&& (*FogSeparateCompositionViewResources)[ViewIndex].FogSeparateCompositionTexture1 != nullptr;
		FRDGTextureRef SceneSeparateColorTextureOverride = bApplyFogSeparateComposition ? (*FogSeparateCompositionViewResources)[ViewIndex].FogSeparateCompositionTexture0 : nullptr;
		...
		PermutationVector.Set<FComposeVolumetricRTOverScenePS::FApplyFog>(bShouldVolumetricCloudsApplyFogDuringUpsampling && (ViewInfo.Family->Scene->HasAnyExponentialHeightFog() || ViewInfo.LocalFogVolumeViewData.GPUInstanceCount > 0) && ShouldRenderFog(*ViewInfo.Family));
		PermutationVector.Set<FComposeVolumetricRTOverScenePS::FApplyLocalFogVolume>(bShouldVolumetricCloudsApplyFogDuringUpsampling && ViewInfo.LocalFogVolumeViewData.GPUInstanceCount > 0);
		...
		if (SceneSeparateColorTextureOverride)
		{
			PassParameters->RenderTargets[0] = FRenderTargetBinding(SceneSeparateColorTextureOverride, ERenderTargetLoadAction::ELoad);
			PassParameters->ViewSeparateFogDepthOriginOffset = FVector2f(ViewInfo.ViewRect.Min);
		}
		else
		{
			PassParameters->RenderTargets[0] = FRenderTargetBinding(SceneColorTexture, ERenderTargetLoadAction::ELoad);
			PassParameters->ViewSeparateFogDepthOriginOffset = FVector2f(0.0f, 0.0f);
		}
		...
		if (SceneSeparateColorTextureOverride)
		{
			FRHIBlendState* AlphaBlendMode = TStaticBlendState<CW_RGBA, BO_Add, BF_One, BF_SourceAlpha, BO_Add, BF_Zero, BF_SourceAlpha>::GetRHI(); // multiply transmitance of clouds into alpha.

			FIntRect SceneSeparateColorRect = ViewInfo.ViewRect;
			SceneSeparateColorRect -= ViewInfo.ViewRect.Min; // Make it relative to 0,0 

			FPixelShaderUtils::AddFullscreenPass<FComposeVolumetricRTOverScenePS>(
				GraphBuilder, ViewInfo.ShaderMap, RDG_EVENT_NAME("VolumetricComposeCloudOverScene"), PixelShader, PassParameters, SceneSeparateColorRect,
				AlphaBlendMode);
		}
		else
		{
		...
		if (bSupportsAlpha && VolumetricCloudRT.GetHoldoutValid() && !SceneSeparateColorTextureOverride)
		{
	...
	const FMinimalSceneTextures& SceneTextures)
{
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		...
		if (bShouldVolumetricCloudsApplyFogDuringUpsampling)
		{
			PassParameters->FogStruct = CreateFogUniformBuffer(GraphBuilder, ViewInfo);
			PassParameters->LFV = ViewInfo.LocalFogVolumeViewData.UniformParametersStruct;
		}
