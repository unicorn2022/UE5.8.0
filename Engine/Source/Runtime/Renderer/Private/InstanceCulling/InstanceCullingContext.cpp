	...
	int32& NewPassVisibleMeshDrawCommandsNum)
{
	...
	// Used to detect when we change from direct to indirect and break auto instancing there. This is a workaround for the fact that we need an indirect draw to support GPU culling and when MDCs are identical they could batch together otherwise.
	...
	for (int32 DrawCommandIndex = 0; DrawCommandIndex < NumDrawCommandsIn; ++DrawCommandIndex)
	{
		...
		const bool bChangedFromDirectToIndirect = !bPrevUseIndirectDraw && bUseIndirectDraw;
		...
		if (bCompactIdenticalCommands && CurrentStateBucketId != -1 && bMeshDrawCommandMatch && !bChangedFromDirectToIndirect)
		{
