...
static TAutoConsoleVariable<int32> CVarSSGILeakFreeReprojection(
	...
	TEXT("[DEPRECATED] Whether use a more expensive but leak free reprojection of previous frame's scene color.\n"),
	ECVF_RenderThreadSafe);

static TAutoConsoleVariable<int32> CVarSSGIQuality(
	TEXT("r.SSGI.Quality"), 4,
	TEXT("[DEPRECATED] Quality setting to control number of ray shot with SSGI, between 1 and 4 (defaults to 4).\n"),
	ECVF_Scalability | ECVF_RenderThreadSafe);
...
static TAutoConsoleVariable<int32> CVarSSGIRejectUncertainRays(
	...
	TEXT("[DEPRECATED] Rejects the screen space ray if it was uncertain due to going behind screen geometry."),
	ECVF_Scalability | ECVF_RenderThreadSafe);

static TAutoConsoleVariable<int32> CVarSSGITerminateCertainRay(
	FCVarDeprecationArgs{ TEXT("5.8") },
	TEXT("r.SSGI.TerminateCertainRay"), 1,
	TEXT("[DEPRECATED] Optimisations that if the screen space ray is certain and didn't find any geometry, don't fallback on otehr tracing technic."),
	ECVF_Scalability | ECVF_RenderThreadSafe);

static TAutoConsoleVariable<float> CVarSSGISkyDistance(
	FCVarDeprecationArgs{ TEXT("5.8") },
	TEXT("r.SSGI.SkyDistance"), 10000000,
	TEXT("[DEPRECATED] Distance of the sky in KM."),
	ECVF_Scalability | ECVF_RenderThreadSafe);
	...
	if (TiledScreenSpaceReflection == nullptr)
	{
		...
		else
		{
				...
				[PassParametersPS, &View, PixelShader, SSRStencilPrePass](FRDGAsyncTask, FRHICommandList& RHICmdList)
			{
				...
				{
					// Clobers the stencil to pixel that should not compute SSR
					GraphicsPSOInit.DepthStencilState = TStaticDepthStencilState<false, CF_Always, true, CF_NotEqual, SO_Keep, SO_Keep, SO_Keep>::GetRHI();
				}
