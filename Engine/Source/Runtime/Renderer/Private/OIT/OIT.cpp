	...
	FOITData& OITData)
{
	...
	FComputeShaderUtils::AddPass(
		GraphBuilder,
		RDG_EVENT_NAME("Translucency::OITDebug(Pixel,%s)", !!(OITData.PassType & OITPass_SeparateTranslucency) ? TEXT("SeparateTransluency") : TEXT("Regular")),
		ComputeShader,
		Parameters,
		FIntVector(1, 1, 1));
}
...
{
	if (!OutColorTexture && !OutLightingTexture) return;

	const bool bValidSceneColor = OutColorTexture && OutColorTexture->Desc.NumSamples == 1;
	const bool bModulate = OutLightingTexture && OutLightingTexture->Desc.NumSamples == 1 && OutTransmittanceTexture && OutTransmittanceTexture->Desc.NumSamples == 1;

	if (!(bValidSceneColor || bModulate) ||
	...
	FComputeShaderUtils::AddPass(
		GraphBuilder,
		RDG_EVENT_NAME("Translucency::OITCombine(%s)", !!(OITData.PassType & OITPass_SeparateTranslucency) ? TEXT("SeparateTransluency") : TEXT("Regular")),
		ComputeShader,
		Parameters,
		DispatchCount);
...
public:
	...
	{
		return ShaderPrint::IsSupported(InPlatform);
	}
	...
	FOITDebugData& DebugData)
{
	...
	// 1. Scan the primitive and assign each primitive to a slice
	{
		...
		const FBoxSphereBounds& ViewBounds = Bounds.TransformBy(View.ViewMatrices.GetWorldToView());
		...
		Parameters->LocalToView				= FMatrix44f(MeshBatch.Proxy->GetLocalToWorld() * View.ViewMatrices.GetWorldToView());
		...
		if (bDebugEnable)
		{
			Parameters->ViewToWorld		= FMatrix44f(View.ViewMatrices.GetWorldToView().Inverse());	 // LWC_TODO: Precision loss?
...
namespace OIT
{
	...
	FOITData CreateOITData(FRDGBuilder& GraphBuilder, const FViewInfo& View, EOITPassType PassType)
	{
		...
		if (PassTypeBits & OITPass_SeparateTranslucency) Out.SupportedPass |= OITPass_SeparateTranslucency;
