...
static bool ShouldDraw(const FMaterial& Material, bool bMaterialUsesAnisotropy, bool bMaterialIsSubstrateToon)
{
	const bool bIsNotTranslucent = IsOpaqueOrMaskedBlendMode(Material);
	return bMaterialUsesAnisotropy && bIsNotTranslucent && (Material.GetShadingModels().HasAnyShadingModel({ MSM_DefaultLit, MSM_ClearCoat }) || (Material.GetShadingModels().HasAnyShadingModel({ MSM_Strata }) && bMaterialIsSubstrateToon));
}
...
{
	bool bResult = true;
	if (ShouldDraw(Material, Material.MaterialUsesAnisotropy_RenderThread(), uint16(Material.MaterialGetSubstrateMaterialBsdfFeatures_RenderThread() & ESubstrateBsdfFeature::Toon) > 0))
	{
...
{
	if (ShouldDraw(Material, Material.MaterialUsesAnisotropy_GameThread(), uint16(Material.MaterialGetSubstrateMaterialBsdfFeatures_GameThread() & ESubstrateBsdfFeature::Toon) > 0) &&
