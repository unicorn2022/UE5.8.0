...
public:
	...
	{		
		...
		OutEnvironment.SetDefine(TEXT("MOBILE_SSR_ENABLED"), (!bDeferredShadingEnabled && AreMobileScreenSpaceReflectionsEnabled(Parameters.Platform)) ? 1u : 0u);
...
template <EMobileLocalLightSetting LocalLightSetting, typename LightMapPolicyType>
...
{
	using FVertexShaderType = TMobileBasePassVSPolicyParamType<LightMapPolicyType>;
	using FPixelShaderType = TMobileBasePassPSPolicyParamType<LightMapPolicyType>;

	FMaterialShaderTypes ShaderTypes;
	ShaderTypes.AddShaderType<TMobileBasePassVS<LightMapPolicyType>>();

	switch (ColoredTransmittanceFallback)
	{
	default:
	case EMobileTranslucentColorTransmittanceMode::DEFAULT:
		ShaderTypes.AddShaderType<TMobileBasePassPS<LightMapPolicyType, LocalLightSetting, EMobileTranslucentColorTransmittanceMode::DEFAULT>>();
		break;
	case EMobileTranslucentColorTransmittanceMode::SINGLE_SRC_BLENDING:
		ShaderTypes.AddShaderType<TMobileBasePassPS<LightMapPolicyType, LocalLightSetting, EMobileTranslucentColorTransmittanceMode::SINGLE_SRC_BLENDING>>();
		break;
	}
