...
void FIndirectLightingCache::FinalizeCacheUpdates(FRHICommandListBase& RHICmdList, FScene* Scene, FSceneRenderer& Renderer, FILCUpdatePrimTaskData& TaskData)
{
	...
	FinalizeUpdateInternal_RenderThread(RHICmdList, Scene, Renderer, TaskData.OutBlocksToUpdate, TaskData.OutTransitionsOverTimeToUpdate, TaskData.OutPrimitivesToUpdateStaticMeshes);
}

void FIndirectLightingCache::UpdateCache(FRHICommandListBase& RHICmdList, FScene* Scene, FSceneRenderer& Renderer, bool bAllowUnbuiltPreview)
{
	...
	FinalizeUpdateInternal_RenderThread(RHICmdList, Scene, Renderer, BlocksToUpdate, TransitionsOverTimeToUpdate, PrimitivesToUpdateStaticMeshes);
}
...
void FIndirectLightingCache::FinalizeUpdateInternal_RenderThread(FRHICommandListBase& RHICmdList, FScene* Scene, FSceneRenderer& Renderer, TMap<FIntVector, FBlockUpdateInfo>& BlocksToUpdate, const TArray<FIndirectLightingCacheAllocation*>& TransitionsOverTimeToUpdate, TArray<FPrimitiveSceneInfo*>& PrimitivesToUpdateStaticMeshes)
{
	...
	{
		{
			...
			UpdateBlocks(RHICmdList, Scene, Renderer.Views.GetData(), BlocksToUpdate);
		}
...
void FIndirectLightingCache::UpdateBlocks(FRHICommandListBase& RHICmdList, FScene* Scene, FViewInfo* DebugDrawingView, TMap<FIntVector, FBlockUpdateInfo>& BlocksToUpdate)
{
	...
	{
		UpdateBlock(RHICmdList, Scene, DebugDrawingView, It.Value());
	}
}
...
void FIndirectLightingCache::UpdateBlock(FRHICommandListBase& RHICmdList, FScene* Scene, FViewInfo* DebugDrawingView, FBlockUpdateInfo& BlockInfo)
{
	...
	if (CanIndirectLightingCacheUseVolumeTexture(GetFeatureLevel()) && !BlockInfo.Allocation->bPointSample)
	{
		...
		RHICmdList.UpdateTexture3D(GetTexture0(), 0, UpdateRegion, BlockInfo.Block.TexelSize * FormatSize, BlockInfo.Block.TexelSize * BlockInfo.Block.TexelSize * FormatSize, (const uint8*)Texture0Data.GetData());
		RHICmdList.UpdateTexture3D(GetTexture1(), 0, UpdateRegion, BlockInfo.Block.TexelSize * FormatSize, BlockInfo.Block.TexelSize * BlockInfo.Block.TexelSize * FormatSize, (const uint8*)Texture1Data.GetData());
		RHICmdList.UpdateTexture3D(GetTexture2(), 0, UpdateRegion, BlockInfo.Block.TexelSize * FormatSize, BlockInfo.Block.TexelSize * BlockInfo.Block.TexelSize * FormatSize, (const uint8*)Texture2Data.GetData());
	}
	else
	{
