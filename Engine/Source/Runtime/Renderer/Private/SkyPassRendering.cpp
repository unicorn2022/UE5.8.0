...
void FSkyPassMeshProcessor::CollectPSOInitializersInternal(T& SkyPassShaders, const FSceneTexturesConfig& SceneTexturesConfig, const FMaterial& Material, const FPSOPrecacheVertexFactoryData& VertexFactoryData, const FPSOPrecacheParams& PreCacheParams, TArray<FPSOPrecacheData>& PSOInitializers)
{
	...
	static auto CVarRealTimeReflectionCaptureDepthBufferStatic = IConsoleManager::Get().FindConsoleVariable(TEXT("r.SkyLight.RealTimeReflectionCapture.DepthBuffer"));
	const bool bUseDepthBuffer = CVarRealTimeReflectionCaptureDepthBufferStatic ? CVarRealTimeReflectionCaptureDepthBufferStatic->GetInt() > 0 : false;

	FGraphicsPipelineRenderTargetsInfo RenderTargetsInfo;
	SetupGBufferRenderTargetInfo(SceneTexturesConfig, RenderTargetsInfo, bUseDepthBuffer);
	...
	// Also generate with depth write when dpeth buffer is enabled which is used during CaptureSkyMeshReflection
	{
		...
		FExclusiveDepthStencil::Type BasePassDepthStencilAccess_Sky = bUseDepthBuffer ? FExclusiveDepthStencil::Type(SceneBasePassDepthStencilAccess | FExclusiveDepthStencil::DepthWrite)
			: FExclusiveDepthStencil::Type(SceneBasePassDepthStencilAccess & ~FExclusiveDepthStencil::DepthWrite);
