...
void AddViewDepthCopyPSPass(FRDGBuilder& GraphBuilder, const FViewInfo& View, FRDGTextureRef SourceSceneDepthTexture, FRDGTextureRef DestinationDepthTexture, bool bKeepDestStencil = false);

enum class EDepthCopyStencilMode : uint8
{
	Clear,  // Zero stencil - PS path
	Keep,   // Preserve destination stencil - allows fast CS+HTILE path when available
};

void AddViewDepthCopyPass(FRDGBuilder& GraphBuilder, const FViewInfo& View, FRDGTextureRef SourceDepthTexture, FRDGTextureRef DestinationDepthTexture, EDepthCopyStencilMode StencilMode = EDepthCopyStencilMode::Clear);
