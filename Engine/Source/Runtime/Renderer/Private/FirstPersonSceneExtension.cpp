...
void FFirstPersonSceneExtension::FUpdater::PreSceneUpdate(FRDGBuilder& GraphBuilder, const FScenePreUpdateChangeSet& ChangeSet)
{
	...
	for (const FPrimitiveSceneInfo* PrimitiveSceneInfo : SceneExtension.GetFirstPersonPrimitives())
	{
		...
		for (int32 ViewIndex = 0; ViewIndex < NumViews; ++ViewIndex)
		{
			...
			if (View.PrimitiveVisibilityMap[PrimitiveIndex])
			{
				...
				const FBoxSphereBounds ConservativeBounds = Bounds.TransformBy(View.ViewMatrices.GetWorldToFirstPerson()) + Bounds;
