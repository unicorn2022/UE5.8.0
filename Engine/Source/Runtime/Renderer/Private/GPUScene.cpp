...
void FGPUScene::SetEnabled(EShaderPlatform InShaderPlatform)
{
	ShaderPlatform = InShaderPlatform;
	bIsEnabled = UseGPUScene(ShaderPlatform);
}
...
void FGPUScene::UpdateInternal(FRDGBuilder& GraphBuilder, FRDGExternalAccessQueue& ExternalAccessQueue, const UE::Tasks::FTask& UpdateTaskPrerequisites, const FUpdateFromComputeCommands& UpdatesFromCompute, FSceneExtensionsUpdaters& SceneExtensionsUpdaters)
{
	...
	check(bIsEnabled == UseGPUScene(Scene.GetShaderPlatform()));
	...
	bUseTiledInstanceDataLayout = CVarGPUSceneInstanceDataTileSizeLog2.GetValueOnRenderThread() >= 0 && GRHIGlobals.ReservedResources.Supported;
	...
	auto SkinningUpdater = SceneExtensionsUpdaters.GetUpdaterPtr<FSkinningSceneExtension::FUpdater>();

	if (!UpdatesFromCompute.IsEmpty() || SkinningUpdater)
	{
		...
		if (SkinningUpdater)
		{
			SkinningUpdater->ResolveAttachments(GraphBuilder, Params);
		}
	}
}
...
void FGPUScene::UploadGeneral(FRDGBuilder& GraphBuilder, const FRegisteredBuffers &BufferState, FRDGExternalAccessQueue* ExternalAccessQueue, const FUploadDataSourceAdapter& UploadDataSourceAdapter, const UE::Tasks::FTask& PrerequisiteTask)
{
	...
	ensure(bIsEnabled == UseGPUScene(Scene.GetShaderPlatform()));
