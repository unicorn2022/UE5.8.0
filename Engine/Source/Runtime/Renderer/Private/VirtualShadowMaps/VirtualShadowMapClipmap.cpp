...
TAutoConsoleVariable<int32> CVarMarkCoarsePagesDirectional(
	...
	TEXT(" 0 - Disabled\n")
	TEXT(" 1 - Always mark the last clipmap level and levels indicated by First/LastCoarseLevel.\n")
	TEXT(" 2 - Performance Mode: Suppress dynamic invalidations in coarse pages due to geometry changes (e.g., moving objects or WPO or animation).\n"),
	ECVF_Scalability | ECVF_RenderThreadSafe
);
	  ...
	  DependentView(InDependentView)
{
	...
	if (bIsOrthographicCamera)
	{
		 ...
		 */	
		CameraViewportWidth = FMath::Max(FMath::CeilToInt(CameraViewMatrices.GetClipToView().M[0][0] * 2.0f), CameraViewportWidth);
	}
	...
	if (bIsOrthographicCamera && CVarOrthoVSMProjectViewOrigin.GetValueOnRenderThread())
	{
		...
		*/
			...
			&& DependentView && DependentView->Family && DependentView->Family->Scene)
		{
			...
			{
				FVector ViewForward = CameraViewMatrices.GetWorldToView().GetColumn(2);
				FCollisionObjectQueryParams ObjectParams = FCollisionObjectQueryParams(FCollisionObjectQueryParams::InitType::AllObjects);
				
				FCollisionQueryParams CollisionParams(FName("OrthoCamera_VSMTrace"), true);
				...
				if (World->LineTraceSingleByObjectType(
					Hit,		//result
					WorldOrigin,	//start
					WorldOrigin + ViewForward * FMath::Abs(CameraViewMatrices.GetClipToView().M[2][2]), //end
	...
	if (bIsOrthographicCamera && CVarOrthoVSMEstimateClipmapLevels.GetValueOnRenderThread())
	{
		...
		* subtracting a level.
		*/
		int32 OrthoFirstLevel = FMath::FloorToInt(FMath::Log2(static_cast<float>(CameraViewMatrices.GetClipToView().M[0][0])));
	...
	for (int32 ClipmapIndex = 0; ClipmapIndex < LevelCount; ++ClipmapIndex)
	{
		...
		const float CullingEndDistance = GetLevelRadius(static_cast<float>(AbsoluteLevel) - ResolutionLodBias); 
		...
		if (Config.bHasPrefilteredDistant)
		{
			ensureMsgf(Config.bForceInvalidate && Config.bCullDynamicTightly, TEXT("The prefiltered distant path overloads the DynamicDepthCullRange parameters so it will not work correctly unless bForceInvalidate and bCullDynamicTightly are set."));
			Level.DynamicDepthCullRange.X = Level.ViewToClip.TransformPosition(FVector(0.0, 0.0, -PrevCullDistance)).Z;
			Level.DynamicDepthCullRange.Y = Level.ViewToClip.TransformPosition(FVector(0.0, 0.0, -CullingEndDistance)).Z;
			PrevCullDistance = CullingEndDistance;
		}
...
uint32 PackClipmapLevelAndCount(int32 FirstLevel, int32 ClipmapIndex, int32 ClipmapLevelCountRemaining)
{
	int32 ClipmapLevel = FirstLevel + ClipmapIndex;
	check(ClipmapLevel + VSM_PACKED_CLIP_LEVEL_BIAS >= 0);
	check(ClipmapLevelCountRemaining >= 0 && ClipmapLevelCountRemaining <= 0xFF);
	check(ClipmapIndex >= 0 && ClipmapIndex <= 0xFF);

	return (uint32(ClipmapLevel + VSM_PACKED_CLIP_LEVEL_BIAS) << 16u) 
		| (uint32(ClipmapIndex) << 8u)
		| uint32(ClipmapLevelCountRemaining);
}
...
FVirtualShadowMapProjectionShaderData FVirtualShadowMapClipmap::ComputeProjectionShaderData(int32 ClipmapIndex) const
{
	...
	const FVector3f ClipmapWorldOriginOffset(WorldOrigin + PreViewTranslation);
	...
	FMatrix TranslatedWorldToShadowUVMatrix = CalcTranslatedWorldToShadowUVMatrix(WorldToLightViewRotationMatrix, Level.ViewToClip);
	Data.TranslatedWorldToShadowUVMatrix = FMatrix44f(TranslatedWorldToShadowUVMatrix);
	Data.ShadowUVToTranslatedWorldMatrix = FMatrix44f(TranslatedWorldToShadowUVMatrix.Inverse());
	...
	Data.ClipmapWorldOriginOffset = ClipmapWorldOriginOffset;
	int32 ClipmapLevel = FirstLevel + ClipmapIndex;
	...
	Data.LightId = GetLightSceneInfo().Id.GetIndex();

	if (Config.bHasPrefilteredDistant)
	{
		Data.DynamicDepthCullRange = Level.DynamicDepthCullRange;
		Data.Flags |= VSM_PROJ_FLAG_HAS_PREFILTERED_DISTANT;
	}
	...
	if (CVarMarkCoarsePagesDirectional.GetValueOnRenderThread() == 2)
	{
		Data.Flags |= VSM_PROJ_FLAG_FORCE_CACHE_DYNAMIC_COARSE;
	}
