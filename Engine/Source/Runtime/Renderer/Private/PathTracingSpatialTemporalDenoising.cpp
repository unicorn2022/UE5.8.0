...
namespace {
	...
	TAutoConsoleVariable<int32> CVarPathTracingDenoiserPrepassRankedLuminanceVariance(
		...
		TEXT("0: use default luminance to estimate variance (Most clarity but may preserve some noise if variance is too high).\n")
		TEXT("1: Use channel ranked luminance when calculating variance (better in high variance and low motion scene, e.g., dawn, dusk, or low lighting).\n"),
		ECVF_RenderThreadSafe
	);
...
{	
	...
	for (int TextureId = 0; TextureId < kNumberOfTexturesPerPass; ++TextureId)
	{
		...
		{
			{TEXT("Blur0"),	Context.DistanceTextures[TextureId],						  TempBuffer},
			{TEXT("Blur1"),							 TempBuffer, Context.DistanceTextures[TextureId]}
		};
		...
		for (int i = 0; i < BlurSize; ++i) 
		{
			for (int PassIndex = 0; PassIndex < NumOfBlurPass; ++PassIndex)
			{
				...
				FComputeShaderUtils::AddPass(
					GraphBuilder,
					RDG_EVENT_NAME("PathTracing::Denoising::%s %dx%d Mip=%d (DistPatch %d/%d)",
	...
	FPathTracingSpatialTemporalDenoisingContext& SpatialTemporalDenoisingContext)
{
	...
	if (SpatialDenoiserType == ESpatialDenoiserType::SPATIAL_DENOISER_PLUGIN)
	{
		...
		if (bApplyTemporalDenoiser)
		{
			...
			{
				UE_LOGF(LogPathTracingDenoising, Log, "Using temporal denoising for frame %d", SpatialTemporalDenoisingContext.FrameIndex);
