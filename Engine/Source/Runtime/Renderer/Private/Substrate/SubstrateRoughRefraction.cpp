...
bool IsOpaqueRoughRefractionEnabled(TArrayView<const FViewInfo> Views)
{
	...
	TArrayView<const FViewInfo> Views)
{
	...
	const bool bOpaqueRoughRefractionEnabled = IsOpaqueRoughRefractionEnabled(Views);
	...
	// Opaque rough refraction support is not implemented for Megalights. Issue a warning into the log
	static bool bWarningIssued = false;
	if (ViewCount > 0 && Views[0].Family && MegaLights::IsEnabled(*Views[0].Family) && !bWarningIssued)
	{
		UE_LOGF(LogRenderer, Warning, "Substrate::OpaqueRoughRefraction is enabled at the same time as MegaLights, which is currently not supported with opaque rough refraction. The visual output might be incorrect.");
		bWarningIssued = true;
	}
		...
		// Sample variance as a function of roughness
		{
			...
			// Read back the data on CPU
			{
					...
					[RoughRefracDataBuffer, RoughRefracDataByteSize](FRHICommandListImmediate& RHICmdList)
				{
					...
					static const FName FenceName("DumpGPU.BufferFence");
					...
					if (RoughRefracData)
					{
						...
						if (!bDataDumpDone)
						{
							...
							FString Filename = "RoughRefracData.txt";
