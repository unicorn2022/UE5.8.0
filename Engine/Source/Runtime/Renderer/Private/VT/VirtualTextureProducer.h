...
public:
	...
	/**
	 * Record that a page at MipLevel was sampled for the producer identified by PackedProducerHandle.
	 * Only updates producers that were registered with bTrackSamplingHistory=true.
	 */
	void UpdateSamplingHistory(uint32 PackedProducerHandle, uint8 MipLevel, uint32 Frame);

	/**
	 * Returns the LastFramePerMip array for the given producer, or an empty view if not tracking.
	 * Entry [i] is the last frame mip i was sampled; 0 means never sampled.
	 */
	TConstArrayView<uint32> GetLastFramePerMip(const FVirtualTextureProducerHandle& Handle) const;
	...
	/** 
	 * Notify producers that all requests have been completed. 
	 * Only notifies when r.VT.EnableNotifyCompleted=1 and for producers with bNotifyCompleted=true.
	 */
...
private:
	struct FProducerEntry
	{
		...
		/** Per-mip last-sampled frame. Empty when bTrackSamplingHistory was not set at registration; otherwise sized to MaxLevel+1. */
		TArray<uint32> LastFramePerMip;
	};
	...
	FProducerEntry const* GetEntry(const FVirtualTextureProducerHandle& Handle) const;
