	...
	, Space(nullptr)
{
	...
	const uint32 FullWidthInTiles = BlockWidthInTiles * WidthInBlocks;
	const uint32 FullHeightInTiles = BlockHeightInTiles * HeightInBlocks;
	MaxLevel = FMath::Min(MaxLevel, FMath::CeilLogTwo(FMath::Min(FullWidthInTiles, FullHeightInTiles)));

	MaxLevel = FMath::Min(MaxLevel, VIRTUALTEXTURE_LOG2_MAX_PAGETABLE_SIZE - 1u);

	// Clamp AdaptiveBias so that each block retains at least 1 tile after shifting.
	const uint32 BlockMinDim = FMath::Min(BlockWidthInTiles, BlockHeightInTiles);
	Description.AllocatedAdaptiveBias = FMath::Min<uint8>(Description.AllocatedAdaptiveBias, FMath::FloorLog2(FMath::Max(1u, BlockMinDim)));
	
	// Clamp AdaptiveBias so that MaxLevel -= AdaptiveBias cannot underflow.
	// A producer may have fewer mips than the block size would suggest.
	Description.AllocatedAdaptiveBias = FMath::Min<uint8>(Description.AllocatedAdaptiveBias, MaxLevel);
	
	// Re-clamp MaxLevel using allocator dimensions
	// The allocator requires CeilLogTwo(AllocMaxDim) >= AllocMaxLevel.
	// This can fail for non-power-of-2 tile counts where shifting by AdaptiveBias loses bits.
	const uint32 AllocMinDim = FMath::Max(1u, FMath::Min(FullWidthInTiles >> Description.AllocatedAdaptiveBias, FullHeightInTiles >> Description.AllocatedAdaptiveBias));
	MaxLevel = FMath::Min(MaxLevel, Description.AllocatedAdaptiveBias + FMath::CeilLogTwo(AllocMinDim));
	
	// Adjust Width/Height/MaxLevel for our adaptive bias.
	// Producer-facing operations will need to use AdaptiveBias back to map to the full resolution texture.
	MaxLevel -= Description.AllocatedAdaptiveBias;
	BlockWidthInTiles = BlockWidthInTiles >> Description.AllocatedAdaptiveBias;
	BlockHeightInTiles = BlockHeightInTiles >> Description.AllocatedAdaptiveBias;
	...
	// Physical pool needs to evict all pages that belong to this VT
	{
		...
		for (int32 PageTableIndex = 0u; PageTableIndex < UniquePageTableLayers.Num(); ++PageTableIndex)
		{
			if (UniquePageTableLayers[PageTableIndex].PhysicalSpace)
			{
				...
				{
					// Clean up page pool before release. Pages should all be unmapped, but may still be allocated. 
					// The physical space can be recycled later, so it's state needs to be clean.
					check(UniquePageTableLayers[PageTableIndex].PhysicalSpace->GetPagePool().GetNumMappedPages() == 0);
					UniquePageTableLayers[PageTableIndex].PhysicalSpace->GetPagePool().EvictAllPages(System);
		...
		for (uint32 LayerIndex = 0; LayerIndex < Space->GetNumPageTableLayers(); ++LayerIndex)
		{
			...
			if (MappedPages.Num() > 0)
			{
				...
				UE_LOGF(LogVirtualTexturing, Warning, "%ls", Message.ToString());
			}
		}
#endif // DO_CHECK
	}
...
void FAllocatedVirtualTexture::LockOrUnlockTiles(FVirtualTextureSystem* InSystem, bool bLock) const
{
	...
	for (int32 ProducerIndex = 0u; ProducerIndex < UniqueProducers.Num(); ++ProducerIndex)
	{
		...
		if (Producer && Producer->GetDescription().bPersistentHighestMip)
		{
			...
			// Add AdaptiveBias to get full-VT MaxLevel for producer interaction.
			const uint32 AdjustedMaxLevel = MaxLevel + Description.AllocatedAdaptiveBias;
			check(MipBias <= AdjustedMaxLevel);

			const uint32 Local_vLevel = AdjustedMaxLevel - MipBias;
			checkf(Local_vLevel <= Producer->GetMaxLevel(), TEXT("Invalid Local_vLevel %d for VT producer %s, Producer MaxLevel %d, MipBias %d, AllocatedVT AdjustedMaxLevel %d"),
				Local_vLevel,
...
bool FAllocatedVirtualTexture::TryMapLockedTiles(FVirtualTextureSystem* InSystem) const
{
	...
	for (int32 PageTableLayerIndex = 0u; PageTableLayerIndex < UniquePageTableLayers.Num(); ++PageTableLayerIndex)
	{
		...
		// Note that Local_vLevel is in producer space and must match what LockOrUnlockTiles locked ((MaxLevel+AdaptiveBias) - MipBias).
		const uint32 AdjustedMaxLevel = MaxLevel + Description.AllocatedAdaptiveBias;
		const uint32 WidthInTiles = Producer->GetWidthInTiles();
		const uint32 HeightInTiles = Producer->GetHeightInTiles();
		const uint32 Local_vLevel = FMath::Min(Producer->GetMaxLevel(), AdjustedMaxLevel - UniqueProducer.MipBias);
		const uint32 MipScaleFactor = (1u << Local_vLevel);
		...
		for (uint32 TileY = 0u; TileY < RootHeightInTiles; ++TileY)
		{
			for (uint32 TileX = 0u; TileX < RootWidthInTiles; ++TileX)
			{
				...
				if (pAddress == ~0u)
				{
					...
					if (pAddress != ~0u)
					{
						PagePool.MapPage(Space, PageTableLayer.PhysicalSpace, PageTableLayerIndex, MaxLevel, MaxLevel, vAddress, AdjustedMaxLevel + LocalMipBias, pAddress);
					}
					else
					{
						bHasMissingTiles = true;
						// Mark page table entry as invalid if we can't map.
						PageMap.InvalidateUnmappedRootPage(Space, PageTableLayer.PhysicalSpace, PageTableLayerIndex, MaxLevel, MaxLevel, vAddress, AdjustedMaxLevel + LocalMipBias);
					}
				}
			}
		}
	...
	{
		UE_LOGF(LogVirtualTexturing, Verbose, "Failed to map lowest resolution mip for AllocatedVT %ls (%d frames)", *Description.Name.ToString(), InSystem->GetFrame() - FrameAllocated);
	}
...
uint32 FAllocatedVirtualTexture::CalculatePersistentHash(FAllocatedVTDescription const& InDesc, FVirtualTextureProducer* const* InProducers) const
{
	...
	Hash = HashCombine(Hash, GetTypeHash(InDesc.AllocatedAdaptiveBias));
...
void FAllocatedVirtualTexture::GetPackedPageTableUniform(FUintVector4* OutUniform) const
{
	...
	const uint32 PageTable32Bit = (GetPageTableFormat() == EVTPageTableFormat::UInt32) ? 1 : 0;
	...
	checkSlow(Description.AllocatedAdaptiveBias < 16u);
	...
	OutUniform[1].Z = MaxLevel | (AdaptiveLevelBias << 4) | (PageTable32Bit << 8) | (Description.AllocatedAdaptiveBias << 9);
