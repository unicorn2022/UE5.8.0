...
void FVirtualTextureAllocator::DumpToConsole(bool verbose)
{
	for (int32 BlockID = SortedIndices.Num() - 1; BlockID >= 0; BlockID--)
	{
		...
		UE_LOGF(LogVirtualTexturing, Display, "Block: vAddress %i,%i, size: %ix%i (tiles),  ", X, Y, Size, Size);
		if (Block.VT != nullptr)
		{
			if (verbose)
			{
				UE_LOGF(LogVirtualTexturing, Display, "%p", Block.VT);
			}
		...
		else
		{
			if (verbose)
			{
				UE_LOGF(LogVirtualTexturing, Display, "NULL VT");
			}
		}
	}
}
