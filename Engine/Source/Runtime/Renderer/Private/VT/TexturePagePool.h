...
public:
	...
	/**
	 * Reset the page pool. This can be used to flush any caches.
	 */
	...
	*/
	 ...
	 * @param OldAllocatedAdaptiveBias The allocated adaptive bias of the old virtual texture, used for level bounds checking
	 * @param vLevelChange_vAddress Applied physical level change, used for base remapping
	 * @param vLevelChange_Producer Applied producer (i.e., local) level change
	 * @param Frame The current frame, used for LRU updates
	 */
	void RemapPages(
		FVirtualTextureSystem* System, uint8 SpaceID, FVirtualTexturePhysicalSpace* PhysicalSpace,
		FVirtualTextureProducerHandle const& ProducerHandleOld, uint32 OldVirtualAddress,
		FVirtualTextureProducerHandle const& ProducerHandleNew, uint32 NewVirtualAddress,
		int32 OldAllocatedAdaptiveBias,
		int32 vLevelChange_vAddress, int32 vLevelChange_Producer, uint32 Frame
	);
	...
	/** Get the packed producer handle of the page allocated at the given physical address */
	inline uint32 GetPackedProducerHandleForAddress(uint16 pAddress) const { check(Pages[pAddress].PackedValue != 0u); return Pages[pAddress].PackedProducerHandle; }
