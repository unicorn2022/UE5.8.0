...
void FSkinnedMeshDebugViewExtension::RenderSkeletons(FRDGBuilder& GraphBuilder, const FSceneView& View, const FScreenPassRenderTarget& Output)
{
	...
	{
		FBatchedPrimitiveParameters* UniformParameters = GraphBuilder.AllocParameters<FBatchedPrimitiveParameters>();
		UniformParameters->Data = GraphBuilder.CreateSRV(GSystemTextures.GetDefaultStructuredBuffer(GraphBuilder, 4));
		PassParameters->VSParameters.BatchedPrimitive = GraphBuilder.CreateUniformBuffer(UniformParameters);
	}
