...
static TAutoConsoleVariable<int32> CVarLumenSceneStats(
	TEXT("r.LumenScene.GPUDrivenUpdate.Stats"),
	0,
...
float LumenScene::GetCardTexelDensity(float SurfaceCacheResolution)
{
	return CVarLumenSceneCardTexelDensityScale.GetValueOnRenderThread() * SurfaceCacheResolution * (GLumenFastCameraMode ? .2f : 1.0f);
}
...
int32 LumenScene::GetCardMinResolution(bool bOrthographicCamera, float LumenSceneDetail, float SurfaceCacheResolution)
{
	int32 CardMinResolution = 0;
	if (bOrthographicCamera)
	{
		CardMinResolution = CVarOrthoLumenSceneMinCardResolution.GetValueOnRenderThread();
	}
	if (CardMinResolution <= 0)
	{
		CardMinResolution = CVarLumenSceneCardMinResolution.GetValueOnRenderThread();
	}

	CardMinResolution = FMath::Clamp(FMath::RoundToInt((CardMinResolution / LumenSceneDetail) * SurfaceCacheResolution), 1, 1024);

	return CardMinResolution;
}
...
void LumenScene::GPUDrivenUpdate(FRDGBuilder& GraphBuilder, const FScene* Scene, TArray<FViewInfo>& Views, const FLumenSceneFrameTemporaries& FrameTemporaries)
{
...

	{
		...
		PassParameters->CardTexelDensity = LumenScene::GetCardTexelDensity(LumenSceneData.SurfaceCacheResolution);
		...
		PassParameters->MinCardResolution = LumenScene::GetCardMinResolution(bHasOrthographicView, LumenSceneDetail, LumenSceneData.SurfaceCacheResolution);
		for (int32 OriginIndex = 0; OriginIndex < NumViewOrigins; ++OriginIndex)
		{
