...
static TAutoConsoleVariable<int32> CVarLumenSceneDirectLightingHardwareRayTracingAsyncCompute(
	TEXT("r.LumenScene.DirectLighting.HardwareRayTracing.AsyncCompute"),
	1,
	TEXT("Enables async compute for hardware ray tracing for Lumen direct lighting (Default = 1)"),
	ECVF_Scalability | ECVF_RenderThreadSafe
);
...
class FLumenDirectLightingHardwareRayTracing : public FLumenHardwareRayTracingShaderBase
{
	...
	using FPermutationDomain = TShaderPermutationDomain<FLumenHardwareRayTracingShaderBase::FBasePermutationDomain, FForceTwoSided, FEnableFarFieldTracing, FEnableHeightfieldProjectionBias, FStochastic>;
	...
	ERDGPassFlags ComputePassFlags)
{
	...
	if (CVarLumenSceneDirectLightingHardwareRayTracingAsyncCompute.GetValueOnAnyThread() == 0)
	{
