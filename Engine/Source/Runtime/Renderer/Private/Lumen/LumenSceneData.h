...
{
	static constexpr uint32 SizeInPagesNumBits = 5;
	static constexpr uint32 ResLevelNumBits = 4;
	static constexpr uint32 PageTableSpanSizeNumBits = 9;

	uint32 bLocked : 1 = false;
	uint32 SizeInPagesX : SizeInPagesNumBits = 0;
	uint32 SizeInPagesY : SizeInPagesNumBits = 0;
	uint32 ResLevelX : ResLevelNumBits = 0;
	uint32 ResLevelY : ResLevelNumBits = 0;

	uint32 PageTableSpanSize : PageTableSpanSizeNumBits = 0;
	int32 PageTableSpanOffset = -1;
	
	static_assert(Lumen::MaxMipSizeInPages < (1u << SizeInPagesNumBits), "Not enough bits for max SizeInPagesX/Y");
	static_assert(Lumen::MaxResLevel < (1u << ResLevelNumBits), "Not enough bits for max ResLevelX/Y");
	static_assert(Lumen::MaxMipSizeInPages * Lumen::MaxMipSizeInPages < (1u << PageTableSpanSizeNumBits), "Not enough bits for max PageTableSpanSize");
...
class FLumenCardCullingInfo
{
public:
	uint32 bVisible : 1 = false;
	uint32 AxisZIndex : 2;
	
	// Requested res level based on distance. Actual allocated res level may be lower if atlas is out of space. Used for stats
	uint32 DesiredLockedResLevel : 8 = 0;

	// Similar to DesiredLockedResLevel but only updated on successful allocation from the atlas. Used to adjust surface cache request priorities
	uint32 DesiredLockedResLevelOnLastAlloc : 8 = 0;

	FVector3f MeshCardsSpaceCenter;
	FVector3f MeshCardsSpaceExtent;

	FLumenCardCullingInfo() = default;

	void Initialize(const FLumenCard& Card);

	void SetTransform(const FLumenCard& Card);

	float GetMaxCardExtent() const;

	int32 SetDesiredLockedResLevel(int32 NewLevel, const class FLumenCard& Card);
};

class FLumenCard
{
public:
	static constexpr uint32 DilationModeNumBits = 1;

	static_assert(uint32(ELumenCardDilationMode::Num) <= (1u << DilationModeNumBits), "Not enough bits for DilationMode");

	FLumenCard() = default;
	~FLumenCard();

	uint32 bHeightfield : 1 = false;
	uint32 bFarField : 1 = false;
	uint32 bAxisXFlipped : 1 = false;
	uint32 DilationMode : DilationModeNumBits = uint32(ELumenCardDilationMode::Disabled);

	uint32 LocalOBBAxisX : 3 = 1;
	uint32 LocalOBBAxisY : 3 = 3;
	uint32 LocalOBBAxisZ : 3 = 5;
	FVector3f LocalOBBOrigin = FVector3f::ZeroVector;
	FVector3f LocalOBBExtent = FVector3f::ZeroVector;

	FVector3f WorldOBBAxisX = FVector3f::XAxisVector;
	FVector3f WorldOBBAxisY = FVector3f::YAxisVector;
	FVector3f WorldOBBAxisZ = FVector3f::ZAxisVector;
	FVector WorldOBBOrigin = FVector::ZeroVector;

	FVector3f LocalToWorldScale = FVector3f::ZeroVector;
	...
	uint8 IndexInMeshCards = UINT8_MAX;
	uint8 IndexInBuildData = UINT8_MAX;
	uint8 AxisAlignedDirectionIndex = UINT8_MAX;
	// Derived from CardAspect = WorldOBB.Extent.X / WorldOBB.Extent.Y, which can't change during reallocation
	uint8 PackedResLevelXYBias = 0;
	...
	void GetSurfaceStats(const FLumenCardCullingInfo& CardCullingInfo, const TSparseSpanArray<FLumenPageTableEntry>& PageTable, FSurfaceStats& Stats) const;
	...
	ELumenCardDilationMode GetDilationMode() const
	{
		return ELumenCardDilationMode(DilationMode);
	}

	FLumenCardOBBf GetLocalOBB() const;
	FLumenCardOBBd GetWorldOBB() const;
	FLumenCardOBBf GetMeshCardsOBB() const;

private:
	void SetLocalOBB(const FLumenCardOBBf& In);

	FVector3f RotateCardToLocal(const FVector3f& CardPosition) const;
	FVector3f RotateCardToLocalNoFlip(const FVector3f& CardPosition) const;
	FVector3f RotateLocalToCard(const FVector3f& LocalPosition) const;
	FVector3f RotateLocalToCardNoFlip(const FVector3f& LocalPosition) const;

	void ComputeAndSetResLevelXYBias(float CardAspect);
};
...
{
private:
	static constexpr SIZE_T PrimitivesArrayElementSize = sizeof(FPrimitiveSceneInfo*);

	union
	{
		FPrimitiveSceneInfo* PrimitiveInlined = nullptr;
		FPrimitiveSceneInfo** PrimitivesArray;
	};

	uint32 NumPrimitives : 24 = 0;
	uint32 bValidMeshCards : 1 = false;
	uint32 bFarField : 1 = false;
	uint32 bHeightfield : 1 = false;
	uint32 bEmissiveLightSource : 1 = false;
	uint32 bOpaqueOrMasked : 1 = true;
	/** Note that non-landscape proxies may be represented through a "heightfield"-like projection */
	uint32 bLandscape : 1 = false;

	uint32 PrimitivesArrayMax : 24 = 0;
	// Use 4 bits to match FLumenCardGPUData::FillData but should really only need 3 bits
	uint32 LightingChannelMask : 4 = 0xF;
...
public:
	...
	FLumenPrimitiveGroup() = default;
	~FLumenPrimitiveGroup();

	FLumenPrimitiveGroup(const FLumenPrimitiveGroup& Other);
	FLumenPrimitiveGroup(FLumenPrimitiveGroup&& Other);

	FLumenPrimitiveGroup& operator=(const FLumenPrimitiveGroup& Other);
	FLumenPrimitiveGroup& operator=(FLumenPrimitiveGroup&& Other);

	void AddPrimitive(FPrimitiveSceneInfo* PrimitiveSceneInfo);
	void RemovePrimitiveAtSwap(int32 Index, EAllowShrinking AllowShrinking = EAllowShrinking::Default);
	TConstArrayView<FPrimitiveSceneInfo*> GetPrimitives() const;

	SIZE_T GetPrimitivesArrayAllocatedSize() const
	{
		return PrimitivesArrayMax * PrimitivesArrayElementSize;
	}

	void SetHasValidMeshCards(bool bValue) { bValidMeshCards = bValue; }
	bool HasValidMeshCards() const { return bValidMeshCards; }

	void SetIsFarField(bool bValue) { bFarField = bValue; }
	bool IsFarField() const { return bFarField; }

	void SetIsHeightfield(bool bValue) { bHeightfield = bValue; }
	bool IsHeightfield() const { return bHeightfield; }

	void SetIsEmissiveLightSource(bool bValue) { bEmissiveLightSource = bValue; }
	bool IsEmissiveLightSource() const { return bEmissiveLightSource; }

	void SetIsOpaqueOrMasked(bool bValue) { bOpaqueOrMasked = bValue; }
	bool IsOpaqueOrMasked() const { return bOpaqueOrMasked; }

	void SetIsLandscape(bool bValue) { bLandscape = bValue; }
	bool IsLandscape() const { return bLandscape; }

	void SetLightingChannelMask(uint8 InLightingChannelMask) { LightingChannelMask = InLightingChannelMask; }
...
private:
	bool HasPrimitivesArray() const
	{
		return NumPrimitives > 1u;
	}

	void FreePrimitivesArray();
};
...
struct FLumenPrimitiveGroupCullingInfo
{
	...
	FLumenPrimitiveGroupCullingInfo(const FRenderBounds& Bounds, const FLumenPrimitiveGroup& PrimitiveGroup, int32 InPrimitiveGroupIndex)
		: bVisible(false)
		, bValidMeshCards(PrimitiveGroup.HasValidMeshCards())
		, bFarField(PrimitiveGroup.IsFarField())
		, bEmissiveLightSource(PrimitiveGroup.IsEmissiveLightSource())
		, bOpaqueOrMasked(PrimitiveGroup.IsOpaqueOrMasked())
		, NumInstances(0)
...
struct FLumenSceneFrameTemporaries
{
	...
	TArray<FRDGTextureRef, TInlineAllocator<LUMEN_MAX_VIEWS>> ScreenProbeGatherRadianceCacheIndirectionTextures;
...
public:
	...
	TSparseSpanArray<FLumenCardCullingInfo> CardCullingInfos;
	...
	// Number of surface cache texels wanted by cards passed culling
	int32 NumDesiredSurfaceCacheTexels = 0;
	...
	FLumenSceneData(EShaderPlatform ShaderPlatform, EWorldType::Type WorldType, bool bAllowTrackAllPrimitivesInPreviewWorld);
	...
	void AllocateRadiosityCardAtlases(FRDGBuilder& GraphBuilder, FLumenSceneFrameTemporaries& FrameTemporaries);
	...
	void UpdateSurfaceCacheFeedback(FFeedbackData Data, const TArray<FVector, TInlineAllocator<LUMEN_MAX_VIEWS>>& LumenSceneCameraOrigins, TArray<FSurfaceCacheRequest>& MeshCardsUpdate, const FViewFamilyInfo& ViewFamily, int32 RequestHistogram[Lumen::NumDistanceBuckets]);
	...
	void UpdatePrimitiveGroupGPUBufferEntry(int32 PrimitiveGroupIndex);
	...
	void UpdatePrimitiveGroupCullingInfo(const FLumenPrimitiveGroup& PrimitiveGroup, const FRenderBounds& NewWorldBounds, bool bForcePrimitiveLevel = false, bool bAdditive = false);
...
private:
	...
	bool RecaptureCardPage(FLumenCardRenderer& LumenCardRenderer, FLumenSurfaceCacheAllocator& CaptureAtlasAllocator, FRHIGPUMask GPUMask, int32 PageTableIndex);
