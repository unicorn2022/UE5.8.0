...
class FReflectionClearTracesCS : public FGlobalShader
{
	...
	class FClearTraceThroughput : SHADER_PERMUTATION_BOOL("CLEAR_TRACE_THROUGHPUT");
	using FPermutationDomain = TShaderPermutationDomain<FCleatTraceMaterialId, FClearTraceThroughput>;

	static bool ShouldCompilePermutation(const FGlobalShaderPermutationParameters& Parameters)
	{
		FPermutationDomain PermutationVector(Parameters.PermutationId);

		if (PermutationVector.Get<FCleatTraceMaterialId>() && PermutationVector.Get<FClearTraceThroughput>())
		{
			return false;
		}
...
class FReflectionCompactTracesCS : public FGlobalShader
{
	...
	using FPermutationDomain = TShaderPermutationDomain<FTraceCompactionMode, FWaveOpWaveSize, FThreadGroupSize>;
	...
	bool bSortByMaterial)
{
	...
	const uint32 CompactionThreadGroupSize = FReflectionCompactTracesCS::GetThreadGroupSize(GLumenReflectionTraceCompactionGroupSizeInTiles);
...

	{
		...
		if (Lumen::UseWaveOps(View.GetShaderPlatform()) && GLumenReflectionTraceCompactionWaveOps != 0)
		{
			// Prefer wave32 as for some reason it's faster in practice
		...
		PermutationVector.Set<FReflectionCompactTracesCS::FWaveOpWaveSize>(WaveOpWaveSize);
		...
		FComputeShaderUtils::AddPass(
			GraphBuilder,
			RDG_EVENT_NAME("CompactTraces WaveOpSize:%d ThreadGroupSize%u", WaveOpWaveSize, CompactionThreadGroupSize),
			ComputePassFlags,
			ComputeShader,
			PassParameters,
	...
	EHZBType HZBType)
{
	...
	Parameters.HZBParameters = GetHZBParameters(GraphBuilder, View, HZBType);
...
{
	{
		...
		PermutationVector.Set<FReflectionClearTracesCS::FClearTraceThroughput>(false);
...
{
	{
		...
		PermutationVector.Set<FReflectionClearTracesCS::FClearTraceThroughput>(true);
		auto ComputeShader = View.ShaderMap->GetShader<FReflectionClearTracesCS>(PermutationVector);
