...
TAutoConsoleVariable<int32> CVarLumenTranslucencyVolumeShareRadianceCacheWithOpaque(
	TEXT("r.Lumen.TranslucencyVolume.ShareRadianceCacheWithOpaque"),
	0,
	TEXT("Whether to reuse the same Radiance Cache as the opaque final gather, instead of creating a new one just for translucency GI. Costs less but gives less control over probe spacing."),
	ECVF_Scalability | ECVF_RenderThreadSafe
);

TAutoConsoleVariable<int32> CVarLumenTranslucencyVolumeShareRadianceCacheWithOpaqueClipmapBias(
	TEXT("r.Lumen.TranslucencyVolume.ShareRadianceCacheWithOpaque.ClipmapBias"),
	3,
	TEXT("Bias applied to the Radiance Cache clipmap that would have been used for opaque surfaces. This is needed to avoid overflowing the opaque Radiance Cache, since the Translucency Volume GI will mark everywhere inside the camera frustum, not just near surfaces."),
	ECVF_Scalability | ECVF_RenderThreadSafe
);
...
namespace LumenTranslucencyVolumeRadianceCache
{
	...
	int32 GetNumMipmaps()
	{
		return FMath::Clamp(CVarTranslucencyVolumeRadianceCacheNumMipmaps.GetValueOnRenderThread(), 1, LumenRadianceCache::MaxMipmaps);
	}
	...
	LumenRadianceCache::FRadianceCacheInputs SetupRadianceCacheInputs(const FViewInfo& View)
	{
		...
		Parameters.ProbeAtlasResolutionInProbes = LumenRadianceCache::GetProbeAtlasResolutionInProbes(CVarTranslucencyVolumeRadianceCacheProbeAtlasResolutionInProbes.GetValueOnRenderThread());
	...
	int32 GetClipmapBias()
	{
		if (CVarLumenTranslucencyVolumeShareRadianceCacheWithOpaque.GetValueOnRenderThread() > 0)
		{
			return CVarLumenTranslucencyVolumeShareRadianceCacheWithOpaqueClipmapBias.GetValueOnRenderThread();
		}

		return 0;
	}

	bool ShareWithOpaque()
	{
		return CVarLumenTranslucencyVolumeShareRadianceCacheWithOpaque.GetValueOnRenderThread() != 0;
	}
};
	...
	const FLumenTranslucencyGIVolume& LumenTranslucencyGIVolume)
{
	...
	Parameters.RadianceCacheInterpolationParameters.RadianceProbeIndirectionTexture = OrBlack3DUintIfNull(GraphBuilder, Parameters.RadianceCacheInterpolationParameters.RadianceProbeIndirectionTexture);
	Parameters.RadianceCacheInterpolationParameters.RadianceCacheFinalRadianceAtlas = OrBlack2DIfNull(GraphBuilder, Parameters.RadianceCacheInterpolationParameters.RadianceCacheFinalRadianceAtlas);
	Parameters.RadianceCacheInterpolationParameters.RadianceCacheFinalSkyVisibilityAtlas = OrBlack2DIfNull(GraphBuilder, Parameters.RadianceCacheInterpolationParameters.RadianceCacheFinalSkyVisibilityAtlas);
	Parameters.RadianceCacheInterpolationParameters.RadianceCacheFinalIrradianceAtlas = OrBlack2DIfNull(GraphBuilder, Parameters.RadianceCacheInterpolationParameters.RadianceCacheFinalIrradianceAtlas);
	Parameters.RadianceCacheInterpolationParameters.RadianceCacheProbeOcclusionAtlas = OrBlack2DIfNull(GraphBuilder, Parameters.RadianceCacheInterpolationParameters.RadianceCacheProbeOcclusionAtlas);
	Parameters.RadianceCacheInterpolationParameters.RadianceCacheProbeValidAtlas = OrBlack2DIfNull(GraphBuilder, Parameters.RadianceCacheInterpolationParameters.RadianceCacheProbeValidAtlas);
	Parameters.RadianceCacheInterpolationParameters.RadianceCacheDepthAtlas = OrBlack2DIfNull(GraphBuilder, Parameters.RadianceCacheInterpolationParameters.RadianceCacheDepthAtlas);
	
	if (!Parameters.RadianceCacheInterpolationParameters.ProbeWorldOffset)
	{
		Parameters.RadianceCacheInterpolationParameters.ProbeWorldOffset = GraphBuilder.CreateSRV(GSystemTextures.GetDefaultBuffer(GraphBuilder, sizeof(FVector4f)), PF_A32B32G32R32F);
	}
	...
	// UV scaling parameters for dynamic resolution support
	Parameters.TranslucencyGIScreenToResourceUV = LumenTranslucencyGIVolume.ScreenToResourceUV;
	Parameters.TranslucencyGIScreenToResourceMaxUV = LumenTranslucencyGIVolume.ScreenToResourceMaxUV;
...
FLumenTranslucencyLightingVolumeParameters GetTranslucencyLightingVolumeParameters(FRDGBuilder& GraphBuilder, const FViewInfo& View)
{
	...
	const FIntPoint GridResourceSizeXY = FIntPoint::DivideAndRoundUp(View.GetSceneTexturesConfig().Extent, TranslucencyFroxelGridPixelSize);
	...
	Parameters.TranslucencyGIGridResourceSize = FIntVector(GridResourceSizeXY.X, GridResourceSizeXY.Y, FMath::Max(GridSizeZ, 1));

	// UV scaling parameters for dynamic resolution support
	Parameters.TranslucencyGIScreenToResourceUV = FVector2f(Parameters.TranslucencyGIGridSize.X, Parameters.TranslucencyGIGridSize.Y) / FVector2f(Parameters.TranslucencyGIGridResourceSize.X, Parameters.TranslucencyGIGridResourceSize.Y);
	Parameters.TranslucencyGIScreenToResourceMaxUV = (FVector2f(Parameters.TranslucencyGIGridSize.X, Parameters.TranslucencyGIGridSize.Y) - FVector2f(0.5f, 0.5f)) / FVector2f(Parameters.TranslucencyGIGridResourceSize.X, Parameters.TranslucencyGIGridResourceSize.Y);
...
void MarkRadianceProbesUsedByTranslucencyVolume(
	FRDGBuilder& GraphBuilder,
	...
	ERDGPassFlags ComputePassFlags)
{
	...
	PassParameters->RadianceCacheClipmapBias = ClipmapBias;
	...
	ERDGPassFlags ComputePassFlags)
{
	...
	FComputeShaderUtils::AddPass(
		GraphBuilder,
		RDG_EVENT_NAME("%s %ux%u", bTraceFromVolume ? TEXT("TraceGlobalSDFAndInterpolateRadianceCache") : TEXT("InterpolateRadianceCache"), TranslucencyVolumeTracingOctahedronResolution, TranslucencyVolumeTracingOctahedronResolution),
		ComputePassFlags,
		ComputeShader,
		PassParameters,
		GroupSize);
	...
	ERDGPassFlags ComputePassFlags)
{
	if (CVarLumenTranslucencyVolume.GetValueOnRenderThread())
	{
		...
		const FMatrix44f UnjitteredPrevWorldToClip = FMatrix44f(View.PrevViewInfo.ViewMatrices.GetWorldToView() * View.PrevViewInfo.ViewMatrices.ComputeProjectionNoAAMatrix());		// LWC_TODO: Precision loss?
...

		{
			...
			FLumenTranslucencyLightingVolumeTraceSetupParameters TraceSetupParameters;
			{
				...
				TraceSetupParameters.RadianceCacheClipmapBias = LumenTranslucencyVolumeRadianceCache::GetClipmapBias();
			}
			...
			FIntVector PrevTranslucencyGridSize(TranslucencyGridSize);
			FIntVector PrevTranslucencyGridResourceSize(TranslucencyGridSize);

			if (View.ViewState && View.ViewState->Lumen.TranslucencyVolume0)
			{
				if (View.ViewState->Lumen.PrevTranslucencyGridSize != FIntVector::ZeroValue)
				{
					PrevTranslucencyGridSize = View.ViewState->Lumen.PrevTranslucencyGridSize;
				}

				PrevTranslucencyGridResourceSize.X = View.ViewState->Lumen.TranslucencyVolume0->GetDesc().Extent.X;
				PrevTranslucencyGridResourceSize.Y = View.ViewState->Lumen.TranslucencyVolume0->GetDesc().Extent.Y;
				PrevTranslucencyGridResourceSize.Z = View.ViewState->Lumen.TranslucencyVolume0->GetDesc().Depth;
			...
			//Using a pessimistic Resource Size (View.GetSceneTexturesConfig().Extent) only to create the textures, in order to avoid engine low level reallocation when dynamic resolution is used
			const FIntVector TranslucencyGridResourceSize = VolumeParameters.TranslucencyGIGridResourceSize;

			FRDGTextureDesc LumenTranslucencyGIDesc0(FRDGTextureDesc::Create3D(TranslucencyGridResourceSize, LightingDataFormat, FClearValueBinding::Black, TexCreate_ShaderResource | TexCreate_UAV | TexCreate_3DTiling));
			FRDGTextureDesc LumenTranslucencyGIDesc1(FRDGTextureDesc::Create3D(TranslucencyGridResourceSize, PF_FloatRGBA, FClearValueBinding::Black, TexCreate_ShaderResource | TexCreate_UAV | TexCreate_3DTiling));
			...
			FRDGTextureRef TranslucencyGIVolumeNewHistory1 = GraphBuilder.CreateTexture(LumenTranslucencyGIDesc1, TEXT("Lumen.TranslucencyVolume.SHLightingNewHistory1"));
...

			{
				...
				const bool bUseTemporalReprojection =
					...
					&& PrevTranslucencyGridResourceSize == VolumeParameters.TranslucencyGIGridResourceSize
					&& PrevTranslucencyGridSize == TranslucencyGridSize
					&& TranslucencyGIVolumeHistory0
					&& TranslucencyGIVolumeHistory0->Desc.Format == LightingDataFormat;
			...
			if (View.ViewState && !View.bStatePrevViewInfoIsReadOnly)
			{
				...
				View.ViewState->Lumen.PrevTranslucencyGridSize = TranslucencyGridSize;
			}
			...
			View.GetOwnLumenTranslucencyGIVolume().ScreenToResourceUV = VolumeParameters.TranslucencyGIScreenToResourceUV;
			View.GetOwnLumenTranslucencyGIVolume().ScreenToResourceMaxUV = VolumeParameters.TranslucencyGIScreenToResourceMaxUV;
		}
	}
}
