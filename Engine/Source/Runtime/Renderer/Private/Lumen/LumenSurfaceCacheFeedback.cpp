...
void FLumenSceneData::UpdateSurfaceCacheFeedback(FFeedbackData FeedbackData, const TArray<FVector, TInlineAllocator<LUMEN_MAX_VIEWS>>& LumenSceneCameraOrigins, TArray<FSurfaceCacheRequest>& SurfaceCacheRequests, const FViewFamilyInfo& ViewFamily, int32 RequestHistogram[Lumen::NumDistanceBuckets])
{
		...
		for (int32 FeedbackElementIndex = 0; FeedbackElementIndex < NumFeedbackElements; ++FeedbackElementIndex)
		{
				...
				&& Cards.IsAllocated(CardIndex))
			{
				...
				else
				{
					...
					{
						DistanceSquared = FMath::Min(DistanceSquared, CardWorldOBB.ComputeSquaredDistanceToPoint(CameraOrigin));
					}
...
void FDeferredShadingSceneRenderer::FinishGatheringLumenSurfaceCacheFeedback(FRDGBuilder& GraphBuilder, const FViewInfo& View, FLumenSceneFrameTemporaries& FrameTemporaries, const FMinimalSceneTextures& SceneTextures)
{
	...
	if (LumenSceneData)
	{
		if (FrameTemporaries.IndirectLightingAtlas && FrameTemporaries.RadiosityNumFramesAccumulatedAtlas && LumenRadiosity::IsEnabled(*View.Family))
		{
		...
		{
			LumenSceneData->IndirectLightingAtlas = nullptr;
			LumenSceneData->RadiosityNumFramesAccumulatedAtlas = nullptr;
		}
	}
}