...
namespace Lumen
{
	...
	{
#if RHI_RAYTRACING
		static IConsoleVariable* LumenFinalGatherMethod = IConsoleManager::Get().FindConsoleVariable(TEXT("r.Lumen.FinalGatherMethod"));
		return IsRayTracingEnabled()
			&& Lumen::UseHardwareRayTracing(ViewFamily)
			&& (LumenFinalGatherMethod->GetInt() == 2)
...
{
	if (Lumen::GetFinalGatherMethod(*View.Family, ShaderPlatform) == ELumenFinalGatherMethod::ReSTIRGather)
	{
		{
	...
	FLumenScreenSpaceBentNormalParameters& ScreenSpaceBentNormalParameters)
{
	...
	ReSTIRParameters.DownsampledSceneDepth = FrameTemporaries.StochasticLighting.DownsampledSceneDepth2x2.CreateSharedRT(GraphBuilder, DownsampledSceneDepthDesc, ReservoirViewExtent, TEXT("Lumen.ReSTIRGather.DownsampledSceneDepth"));
	...
	ReSTIRParameters.DownsampledWorldNormal = FrameTemporaries.StochasticLighting.DownsampledWorldNormal2x2.CreateSharedRT(GraphBuilder, DownsampledWorldNormalDesc, ReservoirViewExtent, TEXT("Lumen.ReSTIRGather.DownsampledWorldNormal"));
	...
	if (GLumenReSTIRGatherShortRangeAmbientOcclusion != 0 && ViewFamily.EngineShowFlags.LumenShortRangeAmbientOcclusion)
	{
		...
		ScreenSpaceBentNormalParameters = ComputeScreenSpaceShortRangeAO(GraphBuilder, Scene, View, FrameTemporaries, SceneTextures, LightingChannelsTexture, BlueNoise, FVector2f(MaxScreenTraceFraction), DownsampleFactor, 1.0f, ComputePassFlags);
	}
