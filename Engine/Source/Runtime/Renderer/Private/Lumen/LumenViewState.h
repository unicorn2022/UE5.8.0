...
public:
	...
	void AddCrossGPUTransfers(uint32 SourceGPUIndex, uint32 DestGPUIndex, TArray<FTransferResourceParams>& OutTransfers)
	{
		...
		TRANSFER_LUMEN_RESOURCE(ProbeInterpolationMisses);
		TRANSFER_LUMEN_RESOURCE(ProbeAdaptiveIndices);
		TRANSFER_LUMEN_RESOURCE(ProbeValid);
...
public:
	...
	// Translucency
	FIntVector PrevTranslucencyGridSize = FIntVector::ZeroValue;
	...
	void SafeRelease()
	{
		...
		PrevTranslucencyGridSize = FIntVector::ZeroValue;
	}
