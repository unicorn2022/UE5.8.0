...
static TAutoConsoleVariable<int32> CVarLumenScreenProbeSpatialFilterDisocclusionMaxFrames(
	TEXT("r.Lumen.ScreenProbeGather.SpatialFilter.DisocclusionMaxFrames"),
	4,
	TEXT("Number of history frames to apply probe spatial filtering on history disocclusion.")
	TEXT("0 disables any disocclusion spatial filtering."),
	ECVF_Scalability | ECVF_RenderThreadSafe
);

static TAutoConsoleVariable<float> CVarLumenScreenProbeSpatialFilterDisocclusionFraction(
	TEXT("r.Lumen.ScreenProbeGather.SpatialFilter.DisocclusionFraction"),
	0.4f,
	TEXT("Fraction of pixels below DisocclusionMaxFrames to apply stronger spatial filtering on probe."),
	ECVF_Scalability | ECVF_RenderThreadSafe
);
...
{
	float GetSpatialFilterDisocclusionMaxFrames()
	{
		return CVarLumenScreenProbeSpatialFilterDisocclusionMaxFrames.GetValueOnRenderThread();
	}

	float GetSpatialFilterDisocclusionFraction()
	{
		return CVarLumenScreenProbeSpatialFilterDisocclusionFraction.GetValueOnRenderThread();
	}

	bool UseDisocclusionSpatialFilter()
	{
		return GetSpatialFilterDisocclusionMaxFrames() > 0;
	}
}
...
class FScreenProbeFilterGatherTracesCS : public FGlobalShader
{
	...
	using FPermutationDomain = TShaderPermutationDomain<FScreenProbeExtraAO, FScreenProbeDisocclusion>;
	...
	ERDGPassFlags ComputePassFlags)
{
	...
	if (LumenScreenProbeGather::UseProbeSpatialFilter() && GLumenScreenProbeSpatialFilterHalfKernelSize > 0)
	{
		for (int32 PassIndex = 0; PassIndex < GLumenScreenProbeSpatialFilterNumPasses; PassIndex++)
		{
			...
			PermutationVector.Set<FScreenProbeFilterGatherTracesCS::FScreenProbeDisocclusion>(LumenScreenProbeGather::UseDisocclusionSpatialFilter());
	...
	const EScreenProbeIrradianceFormat ScreenProbeIrradianceFormat = LumenScreenProbeGather::GetScreenProbeIrradianceFormat(View.Family->EngineShowFlags, View.FinalPostProcessSettings.LumenAmbientOcclusionIntensity);
