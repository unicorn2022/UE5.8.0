...
static TAutoConsoleVariable<float> CVarLumenRadiosityHardwareRayTracingBias(
	TEXT("r.LumenScene.Radiosity.HardwareRayTracing.Bias"),
	0.1f,
	TEXT("Bias along the ray direction, useful when the Ray Tracing geometry doesn't match the GBuffer (Nanite Proxy geometry)"),
	ECVF_Scalability | ECVF_RenderThreadSafe
);

static TAutoConsoleVariable<float> CVarLumenRadiosityHardwareRayTracingNormalBias(
	TEXT("r.LumenScene.Radiosity.HardwareRayTracing.NormalBias"),
	0.1f,
...
void LumenRadiosity::ClearSurfaceCacheAtlas(FRDGBuilder& GraphBuilder, const FLumenSceneFrameTemporaries& FrameTemporaries, const FGlobalShaderMap* GlobalShaderMap)
{
...
class FLumenRadiosityHardwareRayTracing : public FLumenHardwareRayTracingShaderBase
{
	...
	class FProbeOcclusion : SHADER_PERMUTATION_BOOL("PROBE_OCCLUSION");
	using FPermutationDomain = TShaderPermutationDomain<FLumenHardwareRayTracingShaderBase::FBasePermutationDomain, FAvoidSelfIntersectionsMode, FProbeOcclusion>;
...
class FLumenRadiosityIntegrateCS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<UNORM float>, RWRadiosityNumFramesAccumulatedAtlas)
	...
	ERDGPassFlags ComputePassFlags)
{
	...
	if (Lumen::UseHardwareRayTracedRadiosity(*FirstView.Family))
	{
		...
		for (int32 OriginIndex = 0; OriginIndex < NumViewOrigins; ++OriginIndex)
		{
			...
			PassParameters->RayTracingBias = CVarLumenRadiosityHardwareRayTracingBias.GetValueOnRenderThread();
			PassParameters->RayTracingNormalBias = CVarLumenRadiosityHardwareRayTracingNormalBias.GetValueOnRenderThread();
	...
	ERDGPassFlags ComputePassFlags)
{
	...
	{
		if (LumenSceneData.bFinalLightingAtlasContentsValid && CardUpdateContext.MaxUpdateTiles > 0)
		{
	...
	{
		LumenSceneData.RadiosityTraceRadianceAtlas = nullptr;
		LumenSceneData.RadiosityTraceHitDistanceAtlas = nullptr;
		LumenSceneData.RadiosityProbeSHRedAtlas = nullptr;
		LumenSceneData.RadiosityProbeSHGreenAtlas = nullptr;
		LumenSceneData.RadiosityProbeSHBlueAtlas = nullptr;
	}
}
