	...
	const LumenSceneDirectLighting::FLightDataParameters& LumenLightData)
{
	...
	FLumenSceneLightingStochasticParameters CommonParameters;
	{
		...
		for (int32 OriginIndex = 0; OriginIndex < NumViewOrigins; ++OriginIndex)
		{
			...
			GET_SCALAR_ARRAY_ELEMENT(CommonParameters.ViewExposure, OriginIndex) = ViewOrigin.LastEyeAdaptationExposure;
		}
			...
			for (const int32 StandaloneLightIndex : LightingTaskData->StandaloneLightIndices)
			{
				...
				{					
					...
					GraphBuilder.AddPass(
						RDG_EVENT_NAME("StandaloneLight::Evaluate(LF,%s)", *LightName),
						PassParameters,
						ComputePassFlags,
				...
				else
				{
					...
					FComputeShaderUtils::AddPass(
						GraphBuilder,
						RDG_EVENT_NAME("StandaloneLight::Evaluate(%s)", *LightName),
						ComputeShader,
						PassParameters,
						CardTilePerLightArgs,
