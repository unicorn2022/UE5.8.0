...
FAutoConsoleVariableRef GVarLumenScreenProbeGatherHierarchicalScreenTraces(
	...
	TEXT("Whether to use HZB tracing for SSGI instead of fixed step count intersection.  HZB tracing is much more accurate, in particular not missing thin features, but is slower."),
	ECVF_Scalability | ECVF_RenderThreadSafe
);
...
static TAutoConsoleVariable<bool> GVarLumenScreenProbeGatherHierarchicalScreenTracesSkipHairHits(
	TEXT("r.Lumen.ScreenProbeGather.ScreenTraces.HZBTraversal.SkipHairHits"),
	false,
	TEXT("Whether to allow screen traces to hit hair shading model.  Can be used to work around aliasing from high frequency hair cards geometry."),
	ECVF_Scalability | ECVF_RenderThreadSafe
);

static TAutoConsoleVariable<bool> GVarLumenScreenProbeGatherHierarchicalScreenTracesSkipUnlitHits(
	TEXT("r.Lumen.ScreenProbeGather.ScreenTraces.HZBTraversal.SkipUnlitHits"),
	false,
	TEXT("Whether to allow screen traces to hit unlit shading model. Can be used to work around strong emissives causing noise."),
	ECVF_Scalability | ECVF_RenderThreadSafe
);
...
namespace LumenScreenProbeGather
{
	...
	{
		return !GVarLumenScreenProbeGatherHierarchicalScreenTracesSkipHairHits.GetValueOnRenderThread();
	}
}
...
class FScreenProbeTraceScreenTexturesCS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(uint32, SkipUnlitHits)
		SHADER_PARAMETER_STRUCT_INCLUDE(FScreenProbeParameters, ScreenProbeParameters)
...
class FScreenProbeCompactTracesCS : public FGlobalShader
{
	...
	static EShaderPermutationPrecacheRequest ShouldPrecachePermutation(const FGlobalShaderPermutationParameters& Parameters)
	{
		...
		if (PermutationVector.Get<FWaveOps>() &&
			((GRHIGlobals.MinimumWaveSize > 32) || (GRHIGlobals.MaximumWaveSize < 32)))
		{
			return EShaderPermutationPrecacheRequest::NotUsed;
		}
...
class FScreenProbeTraceVoxelsCS : public FGlobalShader
{
	...
	static bool ShouldCompilePermutation(const FGlobalShaderPermutationParameters& Parameters)
	{
		...
		// Can't disable entire FScreenProbeTraceVoxelsCS as it's used for compositing radiance cache and sky in HWRT mode
		if (PermutationVector.Get<FTraceVoxels>() && !ShouldCompileDistanceFieldShaders(Parameters.Platform))
		{
			return false;
		}
	...
	ERDGPassFlags ComputePassFlags)
{
	...
	if (bTraceScreen)
	{
		auto TraceScreen = [&]()
		{
			...
			PassParameters->HZBScreenTraceParameters = SetupHZBScreenTraceParameters(GraphBuilder, View, SceneTextures, bHZBTraversal ? EHZBType::ClosestHZB : EHZBType::FurthestHZB);
			...
			PassParameters->SkipHairHits = GVarLumenScreenProbeGatherHierarchicalScreenTracesSkipHairHits.GetValueOnRenderThread() ? 1 : 0;
			PassParameters->SkipUnlitHits = GVarLumenScreenProbeGatherHierarchicalScreenTracesSkipUnlitHits.GetValueOnRenderThread() ? 1 : 0;
	...
	auto TraceVoxels = [&]()
	{
		...
		FComputeShaderUtils::AddPass(
			GraphBuilder,
			bNeedTraceHairVoxel ? RDG_EVENT_NAME("TraceGlobalSDFAndApplyRadianceCache and HairStrands") : RDG_EVENT_NAME("TraceGlobalSDFAndApplyRadianceCache"),
			ComputePassFlags,
			ComputeShader,
			PassParameters,
			CompactedTraceParameters.IndirectArgs,
