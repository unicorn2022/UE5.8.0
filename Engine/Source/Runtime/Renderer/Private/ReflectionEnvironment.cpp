...
float FCaptureComponentSceneState::ComputeCurrentFade(double Now, float Duration) const
{
	if (FadeStartValue == FadeTargetValue)
	{
		return FadeTargetValue;
	}
	if (Duration <= 0.0f)
	{
		return FadeTargetValue;
	}
	const float Elapsed = static_cast<float>(Now - FadeStartTime);
	const float Delta = Elapsed / Duration;
	const float Direction = (FadeTargetValue >= FadeStartValue) ? 1.0f : -1.0f;
	const float Value = FadeStartValue + Direction * Delta;
	return FMath::Clamp(Value,
		FMath::Min(FadeStartValue, FadeTargetValue),
		FMath::Max(FadeStartValue, FadeTargetValue));
}
...
void FReflectionEnvironmentSceneData::ResizeCubemapArrayGPU(uint32 InMaxCubemaps, uint32 InNumReserved, int32 InCubemapSize)
{
	check(IsInRenderingThread());
	check(InNumReserved <= InMaxCubemaps);

	const int32 NewRegularEnd = (int32)InMaxCubemaps - (int32)InNumReserved;
	...
	{
		CubemapArraySlotsUsed.Init(false, NewRegularEnd);
		CubemapArray.UpdateMaxCubemaps(InMaxCubemaps, InNumReserved, InCubemapSize);
		return;
	}

	const int32 OldNumReserved = CubemapArray.GetNumReservedCubemaps();
	const int32 OldRegularEnd = CubemapArray.GetMaxRegularCubemaps();

	// Generate a remapping table for regular slot positions.  IndexRemapping applies to the regular slots, ignoring reserved
	// slots at the end of the cube map array used for smooth blending of refreshed captures.  Runtime reflection captures can add
	// elements to CubemapArraySlotsUsed, exceeding the original GetMaxRegularCubemaps(), so it's necessary to take the maximum.
	// In particular, runtime captures allocate new slots in the "PruneReflectionCaptures" render command BEFORE this function is
	// called, while baked captures allocate their slots AFTER this call in CaptureOrUploadReflectionCapture.  The reserved slots
	// at the tail of the array are separately remapped below.
	const int32 RegularRemapCount = FMath::Max(OldRegularEnd, CubemapArraySlotsUsed.Num());
	TArray<int32> IndexRemapping;
	IndexRemapping.Reserve(RegularRemapCount);
	int32 Count = 0;
	for (int i = 0; i < RegularRemapCount; i++)
	{
	...
	// Build the remapping for reserved elements at the tail of the array.
	TArray<int32> ReservedTailRemapping;
	ReservedTailRemapping.Reserve(OldNumReserved);
	for (int32 i = 0; i < OldNumReserved; i++)
	{
		// Note that the number of reserved elements may have been reduced if the CVar shrank, so we need to check for
		// elements no longer in range, and give them an invalid remap index.
		ReservedTailRemapping.Add(i < (int32)InNumReserved ? NewRegularEnd + i : -1);
	}

	// Apply the reserved-tail remap to RuntimeSmoothBlendSlots.  Each occupied entry's SourceCubemapArrayIndex stores the
	// physical slice index of its source cubemap.  Drop the blend if the slot is being evicted by a CVar shrink.
	for (int32 i = 0; i < RuntimeSmoothBlendSlots.Num(); i++)
	{
		FSmoothBlendEntry& Entry = RuntimeSmoothBlendSlots[i];
		if (Entry.SourceCubemapArrayIndex != INDEX_NONE)
		{
			const int32 LogicalReservedIndex = Entry.SourceCubemapArrayIndex - OldRegularEnd;
			check(LogicalReservedIndex >= 0 && LogicalReservedIndex < OldNumReserved);
			Entry.SourceCubemapArrayIndex = ReservedTailRemapping[LogicalReservedIndex];
			if (Entry.SourceCubemapArrayIndex == INDEX_NONE)
			{
				Entry.OwningComponent = nullptr;
				Entry.BlendStartTime = -1.0;
			}
		}
	}
	RuntimeSmoothBlendSlots.SetNum((int32)InNumReserved);

	CubemapArray.ResizeCubemapArrayGPU(InMaxCubemaps, InNumReserved, InCubemapSize, IndexRemapping, ReservedTailRemapping);
}
...
void FReflectionEnvironmentSceneData::Reset(FScene* Scene)
{
	...
	RuntimeCaptureBudgetCandidates.Empty();
	...
	ENQUEUE_RENDER_COMMAND(ReflectionEnvironmentSceneDataReset)(
		[this](FRHICommandList& RHICmdList)
		{
			...
			PreChangeSortOrder.Empty();
			NumBoxCaptures = 0;
			NumSphereCaptures = 0;
			RuntimeSmoothBlendSlots.Empty();
		});
}

void FReflectionEnvironmentSceneData::SetRegisteredReflectionCapturesHasChanged()
{
	bRegisteredReflectionCapturesHasChanged = true;

	// Snapshot the current sort order (key data only) for post-change sort order comparison in UpdateReflectionSceneData.
	if (PreChangeSortOrder.IsEmpty())
	{
		for (const FReflectionCaptureSortData& Entry : SortedCaptures)
		{
			PreChangeSortOrder.Add(Entry);
		}
	}
}

void FReflectionEnvironmentCubemapArray::ResizeCubemapArrayGPU(uint32 InMaxCubemaps, uint32 InNumReserved, int32 InCubemapSize, const TArray<int32>& IndexRemapping, const TArray<int32>& ReservedTailRemapping)
{
	...
	const int32 OldMaxCubemaps = (int32)MaxCubemaps;
	const int32 OldRegularEnd = (int32)MaxCubemaps - (int32)NumReservedCubemaps;
	MaxCubemaps = InMaxCubemaps;
	NumReservedCubemaps = InNumReserved;
...

	{
		...
		// Copy the regular cubemaps, remapping the elements as necessary
		const int32 RegularSourceCount = FMath::Min(OldRegularEnd, IndexRemapping.Num());
		for (int32 SourceCubemapIndex = 0; SourceCubemapIndex < RegularSourceCount; SourceCubemapIndex++)
		{
			const int32 DestCubemapIndex = IndexRemapping[SourceCubemapIndex];
		...
		// Copy the reserved tail cubemaps, remapping the elements as necessary
		for (int32 ReservedTailIndex = 0; ReservedTailIndex < ReservedTailRemapping.Num(); ReservedTailIndex++)
		{
			const int32 SourceCubemapIndex = OldRegularEnd + ReservedTailIndex;
			const int32 DestCubemapIndex = ReservedTailRemapping[ReservedTailIndex];
			if (DestCubemapIndex != -1)
			{
...
void FReflectionEnvironmentCubemapArray::UpdateMaxCubemaps(uint32 InMaxCubemaps, uint32 InNumReserved, int32 InCubemapSize)
{
	check(InMaxCubemaps > 0);
	check(InCubemapSize > 0);
	check(InNumReserved <= InMaxCubemaps);
