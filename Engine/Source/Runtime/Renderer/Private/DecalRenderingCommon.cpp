...
static_assert(DecalRendering::DecalRenderTargetMode_NumBits == (int32)EDecalRenderTargetMode::Num);
...
namespace DecalRendering
{
	...
	/** Used at cook and runtime time to determine whether decals support normal G-Buffer sampling (requires Deferred MultiPass mode). */
	static bool MobileSupportDecalNormalSampling(const FStaticShaderPlatform Platform)
	{
		return IsMobileDeferredShadingEnabled(Platform) && !MobileAllowFramebufferFetch(Platform);
	}
	...
	void FinalizeBlendDesc(EShaderPlatform Platform, FDecalBlendDesc& Desc)
	{
		...
		const bool bIsMobileNormalBlendable = bIsDBufferPlatform || (bIsMobileDeferredPlatform && MobileUsesExtendedGBuffer(Platform) && !IsStaticLightingAllowed());
		...
		if (bIsMobilePlatform && !bIsMobileNormalBlendable)
		{
	...
	uint8 GetDecalRenderTargetModeMask(const FMaterial& Material, ERHIFeatureLevel::Type FeatureLevel)
	{
		...
		for (int32 DecalRenderStageNum = 0; DecalRenderStageNum < (int32)EDecalRenderStage::Num; ++DecalRenderStageNum)
		{
			...
			{
				EDecalRenderTargetMode DecalRenderTargetMode = GetRenderTargetMode(ShaderPlatform, DecalBlendDesc, DecalRenderStage);
	...
	EDecalRenderTargetMode GetRenderTargetMode(EShaderPlatform Platform, FDecalBlendDesc DecalBlendDesc, EDecalRenderStage DecalRenderStage)
	{
		...
		case EDecalRenderStage::MobileBeforeLighting:
			return (MobileSupportDecalNormalSampling(Platform) && !DecalBlendDesc.bWriteNormal) ? EDecalRenderTargetMode::SceneColorAndGBufferNoNormal : EDecalRenderTargetMode::SceneColorAndGBuffer;
	...
	const TCHAR* GetRenderTargetModeName(EDecalRenderTargetMode DecalRenderTargetMode)
	{
		switch (DecalRenderTargetMode)
		{
		case EDecalRenderTargetMode::DBuffer:
			return TEXT("DBuffer");
		case EDecalRenderTargetMode::SceneColorAndGBuffer:
			return TEXT("SceneColorAndGBuffer");
		case EDecalRenderTargetMode::SceneColorAndGBufferNoNormal:
			return TEXT("SceneColorAndGBufferNoNormal");
		case EDecalRenderTargetMode::SceneColor:
	...
	void ModifyCompilationEnvironment(EShaderPlatform Platform, FDecalBlendDesc DecalBlendDesc, EDecalRenderStage DecalRenderStage, FShaderCompilerEnvironment& OutEnvironment)
	{
		...
		const EDecalRenderTargetMode RenderTargetMode = GetRenderTargetMode(Platform, DecalBlendDesc, DecalRenderStage);
