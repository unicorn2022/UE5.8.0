...
// if the detailed necessity check (bShouldPrepareForMaterialsOrNiagara) succeeds, this cvar can be used
// to control whether the GDF is prepared, regardless of whether DFAO is being used
int32 GGlobalDistanceFieldDetailedNecessitySufficient = 0;
FAutoConsoleVariableRef CVarGlobalDistanceFieldDetailedSufficient(
	TEXT("r.GlobalDistanceField.DetailedNecessitySufficient"),
	GGlobalDistanceFieldDetailedNecessitySufficient,
	TEXT("Whether detailed necessity checks in FSceneRenderer::ShouldPrepareGlobalDistanceField() is sufficient to enabled GDF."),
	ECVF_RenderThreadSafe
);
...
{
	return ShouldCompileDistanceFieldAOShaders(ShaderPlatform);
}
...
void GenerateBestSpacedVectors()
{
	...
	if (bApplyRepulsion && !bGenerated)
	{
		...
		for (int32 i = 0; i < UE_ARRAY_COUNT(OriginalSpacedVectors9); i++)
		{
			UE_LOGF(LogDistanceField, Log, "FVector(%f, %f, %f),", OriginalSpacedVectors9[i].X, OriginalSpacedVectors9[i].Y, OriginalSpacedVectors9[i].Z);
		}
...
{
#if !NO_LOGGING
	UE_LOGF(LogRenderer, Log, "Shared GPU memory (excluding render targets)");

	if (Scene->DistanceFieldSceneData.NumObjectsInBuffer > 0)
	{
		UE_LOGF(LogRenderer, Log, "   Scene Object data %.3fMb", Scene->DistanceFieldSceneData.GetCurrentObjectBuffers()->GetSizeBytes() / 1024.0f / 1024.0f);
	}
#endif // !NO_LOGGING
}
...
bool ShouldPrepareGlobalDistanceField(const FSceneRenderer* Renderer)
{
	...
	if (GGlobalDistanceFieldDetailedNecessitySufficient && bShouldPrepareForMaterialsOrNiagara)
	{
		return true;
	}
...
{
	if (GDistanceFieldAOApplyToStaticIndirect && ShouldRenderDistanceFieldAO(Views, ViewFamily.EngineShowFlags, AnyViewHasGIMethodSupportingDFAO()) && ShouldRenderDistanceFieldLighting(Scene->DistanceFieldSceneData, Views))
	{
...
bool ShouldRenderDistanceFieldAO(TConstArrayView<FViewInfo> Views, const FEngineShowFlags& EngineShowFlags, bool bAnyViewHasGIMethodSupportingDFAO)
{
	...
	return EngineShowFlags.DistanceFieldAO
		&& bAnyViewHasGIMethodSupportingDFAO
		&& !bShouldRenderRTAO
