...
bool FDistanceFieldSceneData::ResizeIndirectionAtlasIfNeeded(FRDGBuilder& GraphBuilder, FGlobalShaderMap* GlobalShaderMap, FRDGTexture*& OutTexture)
{
	...
	if (!IndirectionAtlas || (CurrentSize != DesiredSize) || !Relocations.IsEmpty())
	{
		...
		UE_LOGF(LogDistanceField, Log, "New indirection table size: %ls (%ls required)", *IndirectionAtlas->GetDesc().GetSize().ToString(), *IndirectionAtlasLayout.GetSize().ToString());
		return true;
	}
	...
	TArray<FSetElementId>& DistanceFieldAssetRemoves)
{
...

	{
		...
		// Refresh the snapshot used by HasPendingStreaming() before the AsyncUpdate task below appends NewReadRequests into ReadRequests on a worker thread.
		// Render-thread callers running in parallel with that task (e.g. ComputeUpdateRegionsAndUpdateViewState) read the cached value to avoid racing with the append.
		bHasPendingStreaming = (ReadRequests.Num() + NewReadRequests.Num()) > 0;
...
void FDistanceFieldSceneData::ListMeshDistanceFields(bool bDumpAssetStats) const
{
	...
	UE_LOGF(LogDistanceField, Log,
		"Mesh Distance Field Atlas %ux%ux%u = %.1fMb (%.1fMb target max), with %.1fMb free, %.1fMb block allocator waste, Indirection Table %.1fMb, Indirection Texture %.1fMb", 
		AtlasDimensions.X,
		AtlasDimensions.Y,
		AtlasDimensions.Z,
	...
	{
		UE_LOGF(LogDistanceField, Log, "   Bricks at Mip%u: %.1fMb, %.1f%%", 
			ReversedMipIndex,
	...
	{
		UE_LOGF(LogDistanceField, Log, "");
		UE_LOGF(LogDistanceField, Log, "Dumping mesh distance fields for %u mesh assets", AssetStats.Num());
		UE_LOGF(LogDistanceField, Log, "   Memory Mb, Loaded Mips / Wanted Mips, Mip0 Resolution, Asset Name");

		for (int32 EntryIndex = 0; EntryIndex < AssetStats.Num(); EntryIndex++)
		{
			const FMeshDistanceFieldStats& MeshStats = AssetStats[EntryIndex];

			UE_LOGF(LogDistanceField, Log, "   %.2fMb, %u%ls, %dx%dx%d, %ls", 
