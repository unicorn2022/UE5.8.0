...
struct FTriangleLabelAttribCopyInfo
{
	TTriangleAttributesConstRef<FName> MeshDescAttribRef;
	FDynamicMeshTriangleLabelAttribute* DynaMeshAttribRef = nullptr;
};
...
void FMeshDescriptionToDynamicMesh::Convert(const FMeshDescription* MeshIn, FDynamicMesh3& MeshOut, bool bCopyTangents )
{
	...
	{
		UE_LOGF(LogTemp, Warning, "FMeshDescriptionToDynamicMesh: MeshDescription verts %d polys %d instances %d", NumVertices, NumPolygons, NumVtxInstances);
	}
	...
	TArray<FTriangleLabelAttribCopyInfo> TriangleLabelAttributes;
	...
	if (!bDisableAttributes)
	{
		...
		// find triangle label attribs
		const TAttributesSet<FTriangleID>& TriAttribsSet = MeshIn->TriangleAttributes();
		TriAttribsSet.ForEach([&TriAttribsSet, &TriangleLabelAttributes, &MeshOut](const FName AttributeName, auto AttributesRef)
		{
			if (FSkeletalMeshAttributes::IsReservedAttributeName(AttributeName)) return;

			if (TriAttribsSet.template HasAttributeOfType<FName>(AttributeName))
			{
				FDynamicMeshTriangleLabelAttribute* NewTriangleLabelAttrib = new FDynamicMeshTriangleLabelAttribute(&MeshOut);
				MeshOut.Attributes()->AttachTriangleLabelAttribute(AttributeName, NewTriangleLabelAttrib);
				TriangleLabelAttributes.Add({TriAttribsSet.GetAttributesRef<FName>(AttributeName), NewTriangleLabelAttrib});
			}
		});
	}
	...
	if (!bDisableAttributes)
	{
		...
		// initialize triangle labels
		for (const FTriangleLabelAttribCopyInfo& TriLabelAttrInfo: TriangleLabelAttributes)
		{
			auto TriangleLabelTask = UE::Tasks::Launch(UE_SOURCE_LOCATION, [&, TriLabelAttrInfo]()
			{
				if (ensure(TriLabelAttrInfo.DynaMeshAttribRef))
				{
					for (int32 tid : MeshOut.TriangleIndicesItr())
					{
						FTriangleID SourceTriangleID = TriIDMap[tid];
						TriLabelAttrInfo.DynaMeshAttribRef->SetValue(tid, TriLabelAttrInfo.MeshDescAttribRef.Get(SourceTriangleID));
					}
				}
			});
			Pending.Add(MoveTemp(TriangleLabelTask));
		}
	...
	if (bPrintDebugMessages)
	{
		UE_LOGF(LogTemp, Warning, "FMeshDescriptionToDynamicMesh:  Conversion Timing: Triangles %fs   Attributbes %fs",
		...
		UE_LOGF(LogTemp, Warning, "FMeshDescriptionToDynamicMesh:  FDynamicMesh verts %d triangles %d (primary) uvs %d normals %d", MeshOut.MaxVertexID(), MeshOut.MaxTriangleID(), NumUVs, NumNormals);
	}
