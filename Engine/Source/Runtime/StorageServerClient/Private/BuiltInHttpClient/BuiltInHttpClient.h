...
public:
	...
	// Probe whether a pooled socket is still usable. Returns true only when the
	// connection is open and idle (no buffered data, no peer-closed state). A busy
	// server can reap idle keep-alive connections, so pools must check before reusing.
...
public:
	...
	virtual void SetPersistentHeaders(TArrayView<const FAnsiString> InHeaders) override;
