...
#include "BuiltInHttpClient/BuiltInHttpClientUDSSocket.h"
#endif
...
bool FStorageServerConnection::Initialize(TArrayView<const FString> HostAddresses, const int32 Port, const FAnsiStringView& InBaseURI, TArrayView<const FAnsiString> InPersistentHeaders)
{
	BaseURI = InBaseURI;
	PersistentHeaders = InPersistentHeaders;
	...
	for (const FString& HostAddress : SortedHostAddresses)
	{
		...
		HttpClient->SetPersistentHeaders(PersistentHeaders);
		CurrentHostAddr = HostAddress;
		if (HandshakeRequest())
		{
			UE_LOGF(LogStorageServerConnection, Display, "Zen store connection established to %ls:%i.", *CurrentHostAddr, Port);
			SetupCacheStrategy();
			return true;
		}
	}
...
TIoStatusOr<FStorageServerConnection::Workspaces> FStorageServerConnection::GetWorkspaces()
{
	...
	TIoStatusOr<FIoBuffer> RequestResult = ResultTuple.IoStatus;
...
TIoStatusOr<FString> FStorageServerConnection::CreateShare(const FString& WorkspaceId, const FString& SharePath, const FString& Alias)
{
	...
	TIoStatusOr<FIoBuffer> Result = ResultTuple.IoStatus;
	if (Result.IsOk())
	{
...
void FStorageServerConnection::PackageStoreRequest(TFunctionRef<void(FPackageStoreEntryResource&&)> Callback)
{
	...
	if (Result.IsOk())
	{
		EStorageServerContentType ContentType = ResultTuple.ContentType;
	...
	{
		UE_LOGF(LogStorageServerConnection, Fatal, "Failed to read oplog from storage server. '%ls'", *Result.Status().ToString());
	}
}
...
void FStorageServerConnection::FileManifestRequest(TFunctionRef<void(FIoChunkId Id, FStringView Path, int64 RawSize)> Callback)
{
	...
	{
		UE_LOGF(LogStorageServerConnection, Fatal, "Failed to read file manifest from storage server. '%ls'", *Result.Status().ToString());
	}
}
...
int64 FStorageServerConnection::ChunkSizeRequest(const FIoChunkId& ChunkId)
{
	...
	{
		UE_LOGF(LogStorageServerConnection, Fatal, "Failed to get chunk size from storage server. '%ls'", *Result.Status().ToString());
	}
...
)
{
	...
	if (bCacheAvailable && CacheStrategy.IsValid() && CacheStrategy->ReadChunk(ChunkId, Offset, Size, TOptional<FIoBuffer>(), CacheChunkBuffer, CacheContentType))
	{
		...
		ResultTuple = IStorageServerHttpClient::FResult{
			.IoStatus       = TIoStatusOr<FIoBuffer>(CacheChunkBuffer),
	...
	else
	{
		...
		else
		{
			...
			FIoStatus ResultStatus = ReadChunkBatchRequest(ChunkBatchRequests, [&](FIoChunkId Id, EStorageServerContentType MimeType, FIoBuffer Data, const TOptional<uint64>& ModTag)
				{
					...
					Data.MakeOwned();
					ResultTuple = IStorageServerHttpClient::FResult{
						.IoStatus       = TIoStatusOr<FIoBuffer>(Data),
			...
			{
				ResultTuple = IStorageServerHttpClient::FResult{
					.IoStatus       = TIoStatusOr<FIoBuffer>(ResultStatus),
	...
	if (ResultBuffer.IsOk())
	{
		...
		{
			ZenCacheMiss(ResultTuple.IoStatus.ValueOrDie().GetSize());
			CacheStrategy->CacheChunk(ChunkId, Offset, Size, ResultTuple.IoStatus.ValueOrDie(), ResultTuple.ContentType, ResultModTag);
		}
...
FIoStatus FStorageServerConnection::ReadChunkBatchRequest(const TArray<FChunkBatchRequestEntry>& Chunks, TFunctionRef<void(FIoChunkId Id, EStorageServerContentType MimeType, FIoBuffer Data, const TOptional<uint64>& ModTag)> OnResponse, bool bSkipData)
{
	...
	else if (Chunks.Num() > 1)
	{
		...
		for (const FChunkBatchRequestEntry& Chunk: Chunks)
		{
			...
			{
				UE_LOGF(LogStorageServerConnection, Fatal, "Multiple requests to the same chunkid are not supported (%ls is already present)", *LexToString(Chunk.ChunkId));
	...
	TIoStatusOr<FIoBuffer> Result = HttpResult.IoStatus;
	...
	{
		UE_LOGF(LogStorageServerConnection, Fatal, "Read incorrect header magic from server response - expected %x, got %x", 0xaa77aacc, Header.Magic);
		return FIoStatus::Unknown;
	}
...
bool FStorageServerConnection::IsUDSSocketAddress(const FString Address)
{
	return Address.EndsWith(TEXT(".sock"));
}
...
TUniquePtr<IStorageServerHttpClient> FStorageServerConnection::CreateHttpClient(const FString Address, const int32 Port)
{
	...
	UE_LOGF(LogStorageServerConnection, Display, "Creating zen store connection to %ls:%i (\"%ls\").", *Address, Port, *HostName);
...
#if PLATFORM_HAS_BSD_SOCKET_FEATURE_WINSOCKETS
	else if (IsUDSSocketAddress(Address))
	{
		SocketPool = MakeUnique<FBuiltInHttpClientUDSSocketPool>(Address);
		// The request will be forwarded by the system that reads from the socket to the right target server
		// Hence the host name in this case doesn't matter. We overwrite it to something basic such as localhost
		// so that the socket path such as 'I:/sockets/zenstore.sock' doesn't mess up the URI generation. 
		HostName = "localhost"; 
	}
#endif
	else
...
TSharedPtr<FInternetAddr> FStorageServerConnection::StringToInternetAddr(const FString HostAddr, const int32 Port)
{
	...
	else if (IsUDSSocketAddress(HostAddr))
	{
		return Result;
	}
	...
	else if (!HostAddr.IsEmpty() && HostAddr[0] == TEXT('[') && HostAddr[HostAddr.Len() - 1] == TEXT(']'))
	{
		...
		{
			UE_LOGF(LogStorageServerConnection, Warning, "Ignoring storage server host IPV6 address on platform that doesn't support IPV6: %ls", *HostAddr);
...
bool FStorageServerConnection::ShouldRetryRequest(const IStorageServerHttpClient::FResult& Result)
{
	// Success doesn't need a retry.
	if (Result.IoStatus.IsOk())
	{
		return false;
	}

	// 4xx responses encode a deterministic client-side failure (e.g. 400 Bad Request,
	// 401 Unauthorized, 404 Not Found). Retrying the same request will produce the same
	// response, so treat the whole 4xx range as terminal and let the caller report it.
	if (Result.HttpStatusCode >= 400 && Result.HttpStatusCode < 500)
	{
		return false;
	}

	// Everything else - transport errors (HttpStatusCode == 0, e.g. send/recv failed
	// before getting a response) and 5xx server errors - is potentially transient. Let
	// the caller retry with backoff.
	return true;
}

bool FStorageServerConnection::HandshakeRequest()
{
	// Handshakes are done with a limited per-attempt connection timeout so that we can
	// find out if the destination is unreachable in a timely manner. Multiple attempts
	// (with bounded backoff) tolerate a server that's briefly busy with a full accept
	// backlog at startup time.
	const float ConnectionTimeoutSeconds = 2.0f;
	...
	for (uint32 Attempt = 0; Attempt < MaxAttempts; Attempt++)
	{
		if (Attempt > 0)
		{
			// 50ms, 100ms, 200ms, 400ms, 500ms, 500ms, ... cap at 500ms. Total worst-case
			// backoff across 10 attempts is ~3.25s.
			const int32 Shift = FMath::Min<int32>((int32)Attempt - 1, 4);
			const float BackoffSeconds = FMath::Min(0.5f, 0.05f * (float)(1 << Shift));
		...
		// Bail out early on a deterministic non-retryable response (e.g. any 4xx).
		// Burning the remaining attempts on something the server has already definitively
		// said no to is wasted time and noise in the logs.
		if (!ShouldRetryRequest(ResultTuple))
		{
			break;
		}
	}
...
{
	TIoStatusOr<FIoBuffer> Result = ResultTuple.IoStatus;
	EStorageServerContentType MimeType = ResultTuple.ContentType;
	if (!Result.IsOk())
	{
...
bool FStorageServerChunkBatchRequest::Issue(TFunctionRef<void(uint32 ChunkCount, uint32* ChunkIndices, uint64* ChunkSizes, FStorageServerResponse& ChunkDataStream)> OnResponse)
{
	...
	if (!Socket)
	{
		UE_LOGF(LogStorageServerConnection, Fatal, "Failed to send chunk batch request to storage server.");
		return false;
	}
	FStorageServerResponse Response(Owner, *Socket);
	if (!Response.IsOk())
	{
		UE_LOGF(LogStorageServerConnection, Fatal, "Failed to read chunk batch from storage server. '%ls'", *Response.GetErrorMessage());
		return false;
	}
	...
	{
		UE_LOGF(LogStorageServerConnection, Fatal, "Invalid magic in chunk batch response from storage server.");
		return false;
	}
	Response << ChunkCount;
	if (ChunkCount > INT32_MAX)
	{
		UE_LOGF(LogStorageServerConnection, Fatal, "Invalid chunk count in chunk batch response from storage server.");
		return false;
	}
	Response << Reserved1;
	Response << Reserved2;
