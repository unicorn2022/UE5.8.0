...
{
	UE_LOGF(LogStorageServerPlatformFile, Verbose, "IterateDirectory '%ls'", *Path);
...
{
	UE_LOGF(LogStorageServerPlatformFile, Verbose, "IterateDirectoryRecursively '%ls'", *Path);
...
public:
	...
	FStorageServerEngineStartupPrecache(FStorageServerConnection& InConnection)
		...
		, bUseBatchedRequests(false) // Turned off batch requests until a stall can be diagnosed and fixed
	{
	...
	{
		// the worker thread has most likely completed already but no harm in shutting down cleanly
...
private:
	virtual uint32 Run() override
	{
		...
		while (!bExitImmediately)
		{
			...
			PrecacheItems();
			if (bExitWhenComplete && !bExitImmediately)
			{
	...
	UE::FManualResetEvent HasWork;
	UE::FManualResetEvent IsCompleted;
...
public:
	...
	virtual int64 Size() override
	{
		if (FileSize < 0)
		{
			...
			{
				UE_LOGF(LogStorageServerPlatformFile, Warning, "Failed to obtain size of file '%ls'", *Filename);
				FileSize = 0;
			}
		}
		return FileSize;
...
FStorageServerPlatformFile::FStorageServerPlatformFile()
{
	if (UE::IsUsingZenPakFileStreaming())
	{
		...
		ServerProjectDirView = FStringView(TEXT(UE_STRINGIZE(UE_PROJECT_NAME)) TEXT("/"));
	}
}
...
TUniquePtr<FArchive> FStorageServerPlatformFile::TryFindProjectStoreMarkerFile(IPlatformFile* Inner) const
{
	...
	for (const FString& ProjectStorePath : PotentialProjectStorePaths)
	{
		...
		{
			UE_LOGF(LogStorageServerPlatformFile, Display, "Found '%ls'", *ProjectMarkerPath);
	...
	if (UE::IsUsingZenPakFileStreaming() && !WorkspaceSharePath.IsEmpty())
	{
		...
		if (QueryConnection.Initialize(HostAddrs, HostPort, "/ws"))
		{
			...
			if (WorkspacesResponse.IsOk())
			{
				...
				UE_LOGF(LogStorageServerPlatformFile, Error, "Failed to to resolve or create workspace share path '%ls' from %ls", *WorkspaceSharePath, *FString(QueryConnection.GetHostAddr()));
			}
			else
			{
				UE_LOGF(LogStorageServerPlatformFile, Error, "Failed to get list of workspaces from %ls. Status: %ls", *FString(QueryConnection.GetHostAddr()), *WorkspacesResponse.Status().ToString());
			}
		}
		else
		{
			UE_LOGF(LogStorageServerPlatformFile, Error, "Failed to connect to %ls to get list of workspace shares", *FString(QueryConnection.GetHostAddr()));
		}
	}
	if (!BaseURI.IsEmpty())
	{
...
bool FStorageServerPlatformFile::ShouldBeUsed(IPlatformFile* Inner, const TCHAR* CmdLine) const
{
	...
	if (ProjectStoreMarkerReader != nullptr)
	{
		...
		if (FJsonSerializer::Deserialize(Reader, ProjectStoreObject) && ProjectStoreObject.IsValid())
		{
			...
			if (ProjectStoreObject->TryGetObjectField(TEXT("zenserver"), ZenServerObjectPtr) && (ZenServerObjectPtr != nullptr))
			{
				...
				UE_LOGF(LogStorageServerPlatformFile, Display, "Using connection settings from ue.projectstore: HostAddrs='%ls' and HostPort='%d'", *FString::Join(HostAddrs, TEXT("+")), HostPort);
			}
		}
		else
		{
			UE_LOGF(LogStorageServerPlatformFile, Error, "Failed to Deserialize ue.projectstore!'");
		}
	}

	FString Host;
	if (FParse::Value(CmdLine, TEXT("-ZenStoreHost="), Host))
	{
		UE_LOGF(LogStorageServerPlatformFile, Display, "Adding connection settings from command line: -ZenStoreHost='%ls'", *Host);
	...
	{
		UE_LOGF(LogStorageServerPlatformFile, Display, "Using connection settings from command line: -ZenStorePort='%d'", HostPort);
	}
...
bool FStorageServerPlatformFile::Initialize(IPlatformFile* Inner, const TCHAR* CmdLine)
{
	...
	if (HostAddrs.Num() > 0)
	{
		...
		if (ProjectStoreMarkerReader != nullptr)
		{
			...
			if (FJsonSerializer::Deserialize(Reader, ProjectStoreObject) && ProjectStoreObject.IsValid())
			{
				...
				if (ProjectStoreObject->TryGetObjectField(TEXT("zenserver"), ZenServerObjectPtr) && (ZenServerObjectPtr != nullptr))
				{
					...
					const TSharedPtr<FJsonObject>* HostAuthObjectPtr = nullptr;
					if (ZenServerObject->TryGetObjectField(TEXT("hostauth"), HostAuthObjectPtr) && HostAuthObjectPtr != nullptr)
					{
						const TSharedPtr<FJsonObject>& HostAuthObject = *HostAuthObjectPtr;
						FString AuthType;
						if (HostAuthObject->TryGetStringField(TEXT("type"), AuthType))
						{
							if (AuthType == TEXT("password"))
							{
								FString Username;
								FString Password;
								HostAuthObject->TryGetStringField(TEXT("username"), Username);
								HostAuthObject->TryGetStringField(TEXT("password"), Password);
								FString Credentials = FString::Printf(TEXT("%s:%s"), *Username, *Password);
								FString Encoded = FBase64::Encode(Credentials);
								AuthHeaders.Add(FAnsiString::Printf("Authorization: Basic %s", TCHAR_TO_ANSI(*Encoded)));
								UE_LOGF(LogStorageServerPlatformFile, Display, "Using password authentication from ue.projectstore");
							}
						}
					}

					UE_LOGF(LogStorageServerPlatformFile, Display, "Using settings from ue.projectstore: ServerProject='%ls' and ServerPlatform='%ls'", *ServerProject, *ServerPlatform);
				}
		...
		{
			UE_LOGF(LogStorageServerPlatformFile, Display, "Using settings from command line: -ZenStoreProject='%ls'", *ServerProject);
		}
		if (FParse::Value(CmdLine, TEXT("-ZenStorePlatform="), ServerPlatform))
		{
			UE_LOGF(LogStorageServerPlatformFile, Display, "Using settings from command line: -ZenStorePlatform='%ls'", *ServerPlatform);
		}
		if (FParse::Value(CmdLine, TEXT("-ZenStoreBaseURI="), BaseURI))
		{
			UE_LOGF(LogStorageServerPlatformFile, Display, "Using settings from command line: -ZenStoreBaseURI='%ls'", *BaseURI);
		}
		if (FParse::Value(CmdLine, TEXT("-ZenWorkspaceSharePath="), WorkspaceSharePath))
		{
			UE_LOGF(LogStorageServerPlatformFile, Display, "Using settings from command line: -ZenWorkspaceSharePath='%ls'", *WorkspaceSharePath);
		}
...
void FStorageServerPlatformFile::InitializeConnection()
{
	...
	if (Connection->Initialize(HostAddrs, HostPort, MakeBaseURI(), AuthHeaders))
	{
#if WITH_STORAGE_SERVER_STARTUP_FILE_CACHE
		...
		else
		{
			...
			UE_LOGF(LogStorageServerPlatformFile, Fatal, "Failed to get file list from Zen at '%.*ls'", HostAddr.Len(), HostAddr.GetData());
		}
	}
	else if (bAbortOnConnectionFailure)
	{
		...
		{
			FString FailedConnectionTitle = "Failed to connect";
		...
		UE_LOGF(LogStorageServerPlatformFile, Error, "Failed to initialize connection to any of [ %ls ]",
			*FString::Join(HostAddrs, TEXT(", ")));
		Connection.Reset();
	...
	{
		// One reason the connection can fail is if the oplog has not yet been created during a new cook:
		// we will get the message "oplog <PlatformName> not found in project <ProjectName>".
		UE_LOGF(LogStorageServerPlatformFile, Display, "Failed to initialize connection to any of [ %ls ]",
			*FString::Join(HostAddrs, TEXT(", ")));
		Connection.Reset();
	}
}
...
TSharedPtr<IPackageStoreBackend> FStorageServerPlatformFile::CreatePackageStoreBackend() const
{
#if WITH_COTF
	if (CookOnTheFlyServerConnection)
	{
		return MakeShared<FCookOnTheFlyPackageStoreBackend>(*CookOnTheFlyServerConnection.Get());
	}
	else
#endif
	{
		return MakeShared<FStorageServerPackageStoreBackend>(*Connection.Get());
	}
}

TSharedPtr<IIoDispatcherBackend> FStorageServerPlatformFile::CreateIoDispatcherBackend() const
{
	return MakeShared<FStorageServerIoDispatcherBackend>(*Connection.Get());
}
...
public:
	...
	virtual bool Visit(const TCHAR* FilenameOrDirectory, DataType StatData) override
	{
		...
		bKeepIterating = bKeepIterating && RealVisitor.Visit(FilenameOrDirectory, StatData);
		...
		TFunctionRef<bool(const TCHAR* Directory, ParentVisitorClass& Visitor)> RemoteOperation)
	{
		...
		bool bLocalDirectoryExists = false;
		bool bRemoteDirectoryExists = false;
		if (GPreferLocalForNonAssets)
		{
			bool bExistsAndKeepIterating = LowLevelOperation(Directory, *this);
			if (!bKeepIterating)
			{
				return false;
			}
			bLocalDirectoryExists = bExistsAndKeepIterating;
		}
		...
		if (bIterateOnServer)
		{
			bool bExistsAndKeepIterating = RemoteOperation(*StorageServerDirectory, *this);
			if (!bKeepIterating)
			{
				return false;
			}
			bRemoteDirectoryExists = bExistsAndKeepIterating;
		}
		...
		return bKeepIterating && (bLocalDirectoryExists || bRemoteDirectoryExists);
	}
...
bool FStorageServerPlatformFile::IterateDirectory(const TCHAR* Directory, IPlatformFile::FDirectoryVisitor& Visitor)
{
	...
	return MergedVisitor.PerformMergedIteration(Directory,
		[this](const TCHAR* Directory, FDirectoryVisitor& InMergedVisitor)
		{
			return LowerLevel ? LowerLevel->IterateDirectory(Directory, InMergedVisitor) : false;
		},
		[this](const TCHAR* Directory, FDirectoryVisitor& InMergedVisitor)
		{
			return ServerToc.IterateDirectory(Directory, [this, &InMergedVisitor]
				(const FIoChunkId& FileChunkId, const TCHAR* FilenameOrDirectory, int64 RawSize)
				{
					...
					return InMergedVisitor.CallShouldVisitAndVisit(*LocalPath, bDirectory);
				});
		}
	);
}
...
bool FStorageServerPlatformFile::IterateDirectoryRecursively(const TCHAR* Directory, IPlatformFile::FDirectoryVisitor& Visitor)
{
	...
	return MergedVisitor.PerformMergedIteration(Directory,
		[this](const TCHAR* Directory, FDirectoryVisitor& InMergedVisitor)
		{
			return LowerLevel ? LowerLevel->IterateDirectoryRecursively(Directory, InMergedVisitor) : false;
		},
		[this](const TCHAR* Directory, FDirectoryVisitor& InMergedVisitor)
		{
			return ServerToc.IterateDirectoryRecursively(Directory, [this, &InMergedVisitor]
				(const FIoChunkId& FileChunkId, const TCHAR* FilenameOrDirectory, int64 RawSize)
				{
					TStringBuilder<1024> LocalPath;
					bool bConverted = MakeLocalPath(FilenameOrDirectory, LocalPath);
...
bool FStorageServerPlatformFile::IterateDirectoryStat(const TCHAR* Directory, FDirectoryStatVisitor& Visitor)
{
	...
	return MergedVisitor.PerformMergedIteration(Directory,
		[this](const TCHAR* Directory, FDirectoryStatVisitor& InMergedVisitor)
		{
			return LowerLevel ? LowerLevel->IterateDirectoryStat(Directory, InMergedVisitor) : false;
		},
		[this](const TCHAR* Directory, FDirectoryStatVisitor& InMergedVisitor)
		{
			return ServerToc.IterateDirectory(Directory, [this, &InMergedVisitor]
				(const FIoChunkId& FileChunkId, const TCHAR* ServerFilenameOrDirectory, int64 RawSize)
				{
					TStringBuilder<1024> LocalPath;
					...
					return InMergedVisitor.CallShouldVisitAndVisit(*LocalPath, FileStatData);
				});
		}
	);
}
...
FOpenMappedResult FStorageServerPlatformFile::OpenMappedEx2(const TCHAR* Filename, EOpenMappedFlags OpenOptions, int64 MaximumSize)
{
	if (LowerLevel && IsNonServerFilenameAllowed(Filename))
	{
...
FString FStorageServerPlatformFile::GetFilenameOnDisk(const TCHAR* Filename)
{
	...
	if (MakeStorageServerPath(Filename, StorageServerFilename) && ServerToc.FileExists(*StorageServerFilename))
	{
		UE_LOGF(LogStorageServerPlatformFile, Warning, "Attempting to get disk filename of remote file '%ls'", Filename);
		return Filename;
	}
...
bool FStorageServerPlatformFile::IsNonServerFilenameAllowed(FStringView InFilename)
{
	...
	if (!HostAddrs.IsEmpty() && (LowerLevel == &IPlatformFile::GetPlatformPhysical()))
	{
		...
		if (bRelative)
		{
			...
			UE_CLOGF(!bAllowed, LogStorageServerPlatformFile, VeryVerbose,
				"Access to file '%.*ls' is limited to server contents due to file extension being listed in ExcludedNonServerExtensions.",
...
void FStorageServerPlatformFile::OnCookOnTheFlyMessage(const UE::Cook::FCookOnTheFlyMessage& Message)
{
	switch (Message.GetHeader().MessageType)
	{
		...
		{
			UE_LOGF(LogCookOnTheFly, Verbose, "Received '%ls' message", LexToString(Message.GetHeader().MessageType));
			...
			{
				UE_LOGF(LogCookOnTheFly, Verbose, "Adding file '%ls'", *Filenames[Idx]);
