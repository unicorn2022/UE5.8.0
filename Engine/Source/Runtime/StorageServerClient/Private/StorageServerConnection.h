...
public:
	...
	bool Initialize(TArrayView<const FString> HostAddresses, const int32 Port, const FAnsiStringView& InBaseURI, TArrayView<const FAnsiString> InPersistentHeaders = {});
...
private:
	...
	TArray<FAnsiString> PersistentHeaders;
	...
	static bool IsUDSSocketAddress(const FString Address);
	...
	// Returns true if the given request result represents a transient failure that is
	// worth retrying. The intent is to retry transport-level failures and 5xx server
	// responses while leaving 4xx alone (they encode deterministic client-side errors
	// like "Not Found" or "Bad Request" that an identical retry would only repeat).
	static bool ShouldRetryRequest(const IStorageServerHttpClient::FResult& Result);
