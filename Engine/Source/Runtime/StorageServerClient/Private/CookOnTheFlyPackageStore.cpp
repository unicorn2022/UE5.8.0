...
void FCookOnTheFlyPackageStoreBackend::GetCookedPackagesFromServer()
{
	...
	if (Response.IsOk())
	{
		...
		UE_LOGF(LogCookOnTheFly, Log, "Got '%d' cooked and '%d' failed packages from server",
	...
	{
		UE_LOGF(LogCookOnTheFly, Warning, "Failed to send 'GetCookedPackages' request");
	}
}
...
void FCookOnTheFlyPackageStoreBackend::GetAllAvailablePackagesFromServer()
{
	...
	if (Response.IsOk())
	{
		...
		UE_LOGF(LogCookOnTheFly, Display, "Received list of %d total on-disk packages from server", GetAllOnDiskPackagesResponse.PackageIds.Num());

		AddOnDiskPackages(GetAllOnDiskPackagesResponse.PackageIds);
	}
	else
	{
		UE_LOGF(LogCookOnTheFly, Warning, "Failed to send 'GetAllOnDiskPackages' request");
	}
}
...
void FCookOnTheFlyPackageStoreBackend::SendCookRequest(TArray<TPair<FPackageId, FName>> PackageIdsAndNames)
{
	...
	{
		UE_LOGF(LogCookOnTheFly, Warning, "Failed to send cook package(s) request");
	...
	TArray<TPair<FPackageId, FName>> PackageIdsAndNames)
{
	...
	for (FPackageId FailedPackageId : FailedPackageIds)
	{
		...
		if (EntryInfo.Status != EPackageStoreEntryStatus::Missing)
		{
			...
			{
				FailedName = FName("<Unknown>");
			}

			UE_LOGF(LogCookOnTheFly, Warning, "0x%ls [Failed]. Failed to cook package %ls.",
	...
	for (FPackageStoreEntryResource& Entry : Entries)
	{
		...
		UE_LOGF(LogCookOnTheFly, Verbose, "0x%ls '%ls' [OK] (Cooked/Failed='%d/%d')",
...
void FCookOnTheFlyPackageStoreBackend::OnCookOnTheFlyMessage(const UE::Cook::FCookOnTheFlyMessage& Message)
{
	...
	switch (Message.GetHeader().MessageType)
	{
		case UE::Cook::ECookOnTheFlyMessage::PackagesCooked:
		{
			...
			UE_LOGF(LogCookOnTheFly, Verbose, "Received 'PackagesCooked' message, Cooked='%d', Failed='%d'",
...
void FCookOnTheFlyPackageStoreBackend::CheckActivity()
{
		...
		TimeSinceLastWarning > TimeBetweenWarning)
	{
		...
		UE_LOGF(LogCookOnTheFly, Log, "No server response in '%.2lf' seconds", TimeSinceLastServerActivity);

		UE_LOGF(LogCookOnTheFly, Log, "=== Pending Packages ===");
		{
			FReadScopeLock ReadLock(EntriesLock);
			for (const auto& KeyValue : PackageIdToEntryInfo)
			{
				...
				if (EntryInfo.Status == EPackageStoreEntryStatus::Pending)
				{
					UE_LOGF(LogCookOnTheFly, Log, "0x%ls", *LexToString(KeyValue.Key));
				}
			}
		}
	}
