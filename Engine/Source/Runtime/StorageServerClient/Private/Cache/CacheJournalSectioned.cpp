...
namespace StorageServer
{
	...
	bool FCacheJournalSectioned::LoadCacheJournal()
	{
...

		{
			...
			{
				UE_LOGF(LogCacheJournal, Warning, "Journal file has wrong magic number, the journal is corrupt");
				InvalidateAll();
				return false;
			}
			...
			{
				UE_LOGF(LogCacheJournal, Warning, "Journal file is of different version %d, supported %d", Header.Version, JOURNAL_VERSION);
				InvalidateAll();
				return false;
			}
		}
		...
		while ((ReadCursor + FJournalPageHeader::SerializedSize < JournalFileHandle->Size()) && (PageCount < Header.PageCount))
		{
			...
			{
				UE_LOGF(LogCacheJournal, Warning, "Failed to read page header from section in journal file");
				return false;
			}
			...
			{
				UE_LOGF(LogCacheJournal, Warning, "Section has wrong magic number, the journal is corrupt");
				InvalidateAll();
				return false;
			}
			...
			{
				UE_LOGF(LogCacheJournal, Warning, "Section has wrong size, the journal is corrupt");
				InvalidateAll();
				return false;
			}
			...
			{
				UE_LOGF(LogCacheJournal, Warning, "Failed to read page from section in journal file.");
				InvalidateAll();
				return false;
			}
			...
			{
				UE_LOGF(LogCacheJournal, Warning, "Page has invalid data hash, the journal is corrupt.");
				InvalidateAll();
				return false;
			}
			...
			case EPageType::ChunkInfo:
				...
				break;
			default:
				UE_LOGF(LogCacheJournal, Warning, "Unknown page type in journal file.");
				InvalidateAll();
				return false;
			}
	...
	void FCacheJournalSectioned::ValidateJournal()
	{
		...
		{
			UE_LOGF(LogCacheJournal, Warning, "There were %d invalid zen cache journal entries.", InvalidChunkIds.Num());
		}

		if (DuplicateChunkIds.Num() > 0)
		{
			UE_LOGF(LogCacheJournal, Warning, "There were %d duplicate chunk id entries.", DuplicateChunkIds.Num());
		}
	}
	...
	bool FJournalPageBase::Flush(IFileHandle* JournalFile, TArray<uint8>& SerializationBuffer)
	{
		...
		if (!ensureMsgf(SerializationBuffer.Num() + FJournalPageHeader::SerializedSize <= PageSize, TEXT("Page %u serialized to %i bytes > max size of %" INT64_FMT), EnumToUnderlyingType(PageType), SerializationBuffer.Num() + FJournalPageHeader::SerializedSize, PageSize))
		{
			return false;
		}
