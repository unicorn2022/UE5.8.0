...
namespace UE::Private::UDynamicMeshLocal
{
	static void DisplayCriticalWarningMessage(const FText& InMessage, float ExpireDuration = 5.0f)
	{
		...
		UE_LOGF(LogGeometry, Warning, "%ls", *InMessage.ToString());
	}
}
...
void UDynamicMesh::ImportCustomProperties(const TCHAR* SourceText, FFeedbackContext* Warn)
{
	...
	if (FParse::Command(&SourceText, TEXT("MeshData")))	
	{
		...
		UE_LOGF(LogGeometry, Warning, "UDynamicMesh text-based property serialization incomplete, generating box as placeholder. Try increasing geometry.DynamicMesh.TextBasedDupeTriThreshold, or geometry.DynamicMesh.DupeStashTimeout.")
		FMinimalBoxMeshGenerator BoxGen;
		BoxGen.Box = UE::Geometry::FOrientedBox3d(FVector3d::Zero(), 50.0 * FVector3d::One());
...
UDynamicMesh* UDynamicMeshPool::RequestMesh()
{
	...
	{
		UE_LOGF(LogGeometry, Warning, "UDynamicMeshPool Threshold of %d Allocated Meshes exceeded! Releasing references to all current meshes and forcing a garbage collection.", CVarDynamicMeshPoolMaxPoolSizeThreshold.GetValueOnAnyThread());
