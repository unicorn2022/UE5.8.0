...
void FBaseDynamicMeshSceneProxy::GetDynamicRayTracingInstances(FRayTracingInstanceCollector& Collector)
{
	...
	for (FMeshRenderBufferSet* BufferSet : Buffers)
	{
		...
		{
			ensure(BufferSet->PrimaryRayTracingGeometry.GetInitializer().IndexBuffer.IsValid());
		...
		{
			ensure(BufferSet->SecondaryRayTracingGeometry.GetInitializer().IndexBuffer.IsValid());
...
{
	ensure(RayTracingGeometry.GetInitializer().IndexBuffer.IsValid());
