										   ...
										   EDynamicMeshAttributeChangeFlags ChangeFlags)
{
	...
	{
		MeshObject->EditMesh(EditFunc, ChangeType, ChangeFlags);
...
{
	FastNotifyDecompositionSetsUpdated([this, &Triangles]() -> TArray<int32>
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(SimpleDynamicMeshComponent_FastTriangleVertexUpdate_FindSets);
...
void UDynamicMeshComponent::FastNotifyVerticesUpdated(const TArray<int32>& Vertices, EMeshRenderAttributeFlags UpdatedAttributes)
{
	FastNotifyDecompositionSetsUpdated([this, &Vertices]() -> TArray<int32>
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(SimpleDynamicMeshComponent_FastVertexUpdate_FindSets);
		// compute list of sets to update
		TArray<int32> UpdatedBuffers;
		...
		ParallelFor(Vertices.Num(), [&](int32 k)
		{
			...
			Mesh->EnumerateVertexTriangles(VID, [this, &BufferFlags](int32 TID)
			{
				int32 SetID = Decomposition->GetGroupForTriangle(TID);
				BufferFlags[SetID].store(true, std::memory_order_relaxed);
			});
		});
		for (int32 k = 0; k < NumBuffers; ++k)
		{
...
void UDynamicMeshComponent::FastNotifyDecompositionSetsUpdated(TFunctionRef<TArray<int32>(void)> GetUpdatedDecompositionIndices, EMeshRenderAttributeFlags UpdatedAttributes)
{
	if (!ensure(MeshObject))
	{
		return;
	...
	{
		// compute list of sets to update
		TArray<int32> UpdatedSets = GetUpdatedDecompositionIndices();
		...
		if (bPositions)
		{
			UpdateBoundsCalc = Async(DynamicMeshComponentAsyncExecTarget, [this]()
			{
				TRACE_CPUPROFILER_EVENT_SCOPE(SimpleDynamicMeshComponent_FastROIUpdate_AsyncBoundsUpdate);
				UpdateLocalBounds();
			});
		}
		...
		{
			TRACE_CPUPROFILER_EVENT_SCOPE(SimpleDynamicMeshComponent_FastROIUpdate_ApplyUpdate);
		...
		if (bPositions)
		{
			TRACE_CPUPROFILER_EVENT_SCOPE(SimpleDynamicMeshComponent_FastROIUpdate_FinalPositionsUpdate);
			MarkRenderTransformDirty();
			UpdateBoundsCalc.Wait();
			UpdateBounds();
		}
...
void UDynamicMeshComponent::SetDynamicMesh(UDynamicMesh* NewMesh)
{
	...
	NewMesh->Rename( nullptr, this, REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
	MeshObject = NewMesh;
	MeshObjectChangedHandle = MeshObject->OnMeshChanged().AddUObject(this, &UDynamicMeshComponent::OnMeshObjectChanged);
