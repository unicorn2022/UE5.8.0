...
public:
	...
	/**
	 * This function updates vertex positions/attributes of existing SceneProxy render buffers if possible, for the given vertices.
	 * If a FMeshRenderDecomposition has not been explicitly set, call is forwarded to FastNotifyVertexAttributesUpdated()
	 */
	GEOMETRYFRAMEWORK_API void FastNotifyVerticesUpdated(const TArray<int32>& Vertices, EMeshRenderAttributeFlags UpdatedAttributes);

private:
	// Helper to implement common functionality of above FastNotify-region-Updated methods
	void FastNotifyDecompositionSetsUpdated(TFunctionRef<TArray<int32>(void)> GetUpdatedDecompositionIndices, EMeshRenderAttributeFlags UpdatedAttributes);
public:
