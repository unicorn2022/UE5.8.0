...
void UAudioAnalyzerNRT::AnalyzeAudio()
{
	...
	if (nullptr != Sound)
	{
		...
		{
			UE_LOGF(LogAudioAnalyzer, Warning, "Soundwave '%ls' is procedural. NRT audio analysis is not currently supported for this.", *Sound->GetFullName());
			SetResult(nullptr);
			return;
		}
		...
		{
			UE_LOGF(LogAudioAnalyzer, Warning, "Soundwave '%ls' has multi-channel audio (channels greater than 2). Audio analysis is not currently supported for this yet.", *Sound->GetFullName());
			SetResult(nullptr);
			return;
		}
		...
		{
			UE_LOGF(LogAudioAnalyzer, Error, "Could not analyze audio due to failed import of sound wave data from Soundwave '%ls'.", *Sound->GetFullName());
			SetResult(nullptr);
			return;
		}
		...
		{
			UE_LOGF(LogAudioAnalyzer, Error, "Failed to parse the raw imported data for '%ls' for analysis.", *Sound->GetFullName());
			SetResult(nullptr);
			return;
		}
