...
void UAudioAnalyzer::StartAnalyzing(const Audio::FDeviceId InAudioDeviceId, UAudioBus* AudioBusToAnalyze)
{
	...
	{
		UE_LOGF(LogAudioAnalyzer, Error, "Unable to analyze audio without an audio bus to analyze.");
		return;
	}
	...
	{
		UE_LOGF(LogAudioAnalyzer, Error, "Unable to analyze audio with a null audio device manager.");
		return;
	}
	...
	if (!MixerDevice)
	{
		UE_LOGF(LogAudioAnalyzer, Error, "Audio analyzer only works with the audio mixer.");
		return;
	}
