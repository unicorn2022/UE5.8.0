...
void FChaosEngineInterface::ClearWakeEvent_AssumesLocked(const FPhysicsActorHandle& InActorReference)
{
	InActorReference->GetGameThreadAPI().ClearEvents();
}
...
void FChaosEngineInterface::AddImpulseAtLocation_AssumesLocked(const FPhysicsActorHandle& InActorReference,const FVector& InImpulse,const FVector& InLocation, bool bIsInternal)
{
	if (ensure(FChaosEngineInterface::IsValid(InActorReference)))
	{
		if (bIsInternal)
		{
			...
			//UE_LOGF(LogTemp, Warning, "AddImpulseAtLocation_AssumesLocked Impulse = %ls | State = %d", *InImpulse.ToString(), InActorReference->GetPhysicsThreadAPI()->ObjectState());
		}
		else
		{
...
void FChaosEngineInterface::AddForceAtPosition_AssumesLocked(const FPhysicsActorHandle& InActorReference, const FVector& Force, const FVector& Position, bool bAllowSubstepping, bool bIsLocalForce, bool bIsInternal)
{
	if (ensure(FChaosEngineInterface::IsValid(InActorReference)))
	{
		if (bIsInternal)
		{
			...
			//UE_LOGF(LogTemp, Warning, "AddForceAtPosition_AssumesLocked Force = %ls | State = %d", *Force.ToString(), InActorReference->GetPhysicsThreadAPI()->ObjectState());
		}
		else
		{
...
void FChaosEngineInterface::SerializeParticleStateToBuffer(const FPhysicsActorHandle& InActorHandle, Chaos::FSerializedDataBuffer& OutSerializedData, Chaos::ESerializedDataContext DataContext)
{
	...
	{
		UE_LOGF(LogChaos, Warning, "[%s]: Failed to serialize particle | Invalid Actor Handle.", __func__);
		return;
	}
	...
	auto SerializeInternalStateToBuffer = [InActorHandle](FPBDRigidsSolver& Solver, FSerializedDataBuffer& OutData)
	{
		...
		else
		{
			...
			else
			{
				...
				UE_LOGF(LogChaos, Warning, "[%s]: Failed to get a valid Internal physics state. Falling back to the external state | Owner [%ls].", __func__, *GetNameSafe(InActorHandle->GetOwner()));
			}
		}
	};
			...
			default:
				{
					...
					UE_LOGF(LogChaos, Warning, "[%s]: Failed to serialize particle | Unsupported Data Context [%ls].", __func__, Chaos::LexToString(DataContext));
					break;
				}
		}
	}
...
void FChaosEngineInterface::ApplySerializedStateToParticle(const FPhysicsActorHandle& InActorHandle, Chaos::FSerializedDataBufferPtr&& InSerializedData, Chaos::ESerializedDataContext DataContext)
{
	...
	if (FPBDRigidsSolver* Solver = InActorHandle->GetSolver<FPBDRigidsSolver>())
	{
		...
		case ESerializedDataContext::Invalid:
		default:
			UE_LOGF(LogChaos, Warning, "[%s]: Failed to apply serialized state to particle | Unsupported Data Context [%ls].", __func__, Chaos::LexToString(DataContext));
			break;
		}
	}
}
...
void FChaosEngineInterface::SerializeConstraintStateToBuffer(const FPhysicsConstraintHandle& ConstraintHandle, Chaos::FSerializedDataBuffer& OutSerializedData, Chaos::ESerializedDataContext DataContext)
{
	...
	{
		UE_LOGF(LogChaos, Warning, "[%s]: Failed to constraint particle | Invalid Actor Handle.", __func__);
		return;
	}
	...
	if (FPBDRigidsSolver* Solver = Proxy ? Proxy->GetSolver<FPBDRigidsSolver>() : nullptr)
	{
		...
		auto SerializeInternalStateToBuffer = [&ConstraintHandle](IPhysicsProxyBase* Proxy, FPBDRigidsSolver& Solver, FSerializedDataBuffer& OutData)
		{
			...
			else
			{
				...
				else
				{
					...
					UE_LOGF(LogChaos, Warning, "[%s]: Failed to get a valid Internal physics state. Falling back to the external state | Owner [%ls].", __func__, *GetNameSafe(Proxy->GetOwner()));
				}
			}
		};
		...
		default:
			{
				// Note: We don't support serialize data from both states into a single buffer yet. But we do support apply the same state to External and Internal during deserialization if ESerializedDataContext::Both is used.
				UE_LOGF(LogChaos, Warning, "[%s]: Failed to serialize constraint | Unsupported Data Context [%ls].", __func__, Chaos::LexToString(DataContext));
				break;
			}
		}
	}
...
void FChaosEngineInterface::CreateActor(const FActorCreationParams& InParams,FPhysicsActorHandle& Handle, UObject* InOwner)
{
	...
	{
		FSpatialAccelerationIdx SpatialIndex{0, static_cast<std::underlying_type_t<ESpatialAccelerationCollectionBucketInnerIndex>>(ESpatialAccelerationCollectionBucketInnerIndex::Default) };
		if (AccelerationStructureSplitStaticAndDynamic == 1)
		{
			if (AccelerationStructureIsolateQueryOnlyObjects == 1)
			{
				...
				{
					SpatialIndex = FSpatialAccelerationIdx{0, static_cast<std::underlying_type_t<ESpatialAccelerationCollectionBucketInnerIndex>>(ESpatialAccelerationCollectionBucketInnerIndex::DefaultQueryOnly)};
				}
				else if (!InParams.bStatic && InParams.bQueryOnly)
				{
					SpatialIndex = FSpatialAccelerationIdx{ 0, static_cast<std::underlying_type_t<ESpatialAccelerationCollectionBucketInnerIndex>>(ESpatialAccelerationCollectionBucketInnerIndex::DynamicQueryOnly)};
				}
				else if (!InParams.bStatic && !InParams.bQueryOnly)
				{
					SpatialIndex = FSpatialAccelerationIdx{ 0, static_cast<std::underlying_type_t<ESpatialAccelerationCollectionBucketInnerIndex>>(ESpatialAccelerationCollectionBucketInnerIndex::Dynamic) };
				}
			}
			else
			{
...
void FChaosEngineInterface::ReleaseActor(FPhysicsActorHandle& Handle,FChaosScene* InScene,bool bNeverDerferRelease)
{
	if(!Handle)
	{
		UE_LOGF(LogChaos, Verbose, "Attempting to release an actor with a null handle");
