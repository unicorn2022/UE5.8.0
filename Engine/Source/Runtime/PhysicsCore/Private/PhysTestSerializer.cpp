...
FPhysTestSerializer::FPhysTestSerializer()
	...
	ChaosContext(MakeShared<Chaos::FChaosArchiveContext>())
{
}
...
void FPhysTestSerializer::Serialize(const TCHAR* FilePrefix)
{
	...
	if (File)
	{
		UE_LOGF(LogPhysicsCore, Log, "PhysTestSerialize File: %ls", *UseFileName);
		Serialize(*File);
	}
	else
	{
		UE_LOGF(LogPhysicsCore, Warning, "Could not create PhysTestSerialize file(%ls)", *UseFileName);
	}
}

void FPhysTestSerializer::Serialize(FArchive& Ar)
{
	TUniquePtr<Chaos::FChaosArchive> ChaosAr = MakeUnique<Chaos::FChaosArchive>(Ar);
	
	if (!Ar.IsLoading())
	{
		//make sure any context we had set is restored before writing out sqcapture
		ChaosAr = MakeUnique<FArchiveWithExternalContext>(Ar, ChaosContext);
	}

	static const FName TestSerializerName = "PhysTestSerializer";

	{
		Chaos::FChaosArchiveScopedMemory ScopedMemory(*ChaosAr, TestSerializerName, false);
		int Version = 1;
		Ar << Version;
		Ar << bDiskDataIsChaos;
	...
	if (Ar.IsLoading())
	{
		...
		ChaosAr = MakeUnique<FArchiveWithExternalContext>(Ar, ChaosContext);	//make sure any context we created during load is used for sqcapture
	}
...
void FPhysTestSerializer::SetPhysicsData(Chaos::FPBDRigidsEvolution& Evolution)
{
	...
	TSharedRef<Chaos::FChaosArchiveContext> NewExternalContext = MakeShared<Chaos::FChaosArchiveContext>();
	FArchiveWithExternalContext ChaosAr(Ar, NewExternalContext);
	Evolution.Serialize(ChaosAr);
	ChaosContext = NewExternalContext;
