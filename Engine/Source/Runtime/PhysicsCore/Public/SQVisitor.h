...
struct TSQVisitor : public Chaos::ISpatialVisitor<TPayload, Chaos::FReal>
{
	...
	TSQVisitor(const FVector& InStartPoint, const FVector& InDir, ChaosInterface::FSQHitBuffer<THitType>& InHitBuffer, EHitFlags InOutputFlags,
		const ChaosInterface::FSceneQueryCommonParams& InCommonParams)
		: StartPoint(InStartPoint)
		, Dir(InDir)
		, OutputFlags(InOutputFlags)
		, bAnyHit(false)
		, DebugParams(InCommonParams.DebugParams)
		, HitFaceNormal(Chaos::FVec3::ZeroVector)
		, HitBuffer(InHitBuffer)
		, QueryFlags(InCommonParams.Flags)
		, QueryFilterData(InCommonParams.FilterData)
		, QueryCallback(InCommonParams.GetQueryCallback())
		, QueryCenter(StartTM.GetLocation())
	{
		QueryGeomWorldBounds = FAABB3(QueryCenter, QueryCenter);
		bAnyHit = QueryFlags & FChaosQueryFlag::eANY_HIT;
	}

	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	UE_DEPRECATED(5.8, "Use the overload that takes a ChaosInterface::FSceneQueryCommonParams instead")
	TSQVisitor(const FVector& InStartPoint, const FVector& InDir, ChaosInterface::FSQHitBuffer<THitType>& InHitBuffer, EHitFlags InOutputFlags,
		...
		: TSQVisitor(InStartPoint, InDir, InHitBuffer, InOutputFlags, ChaosInterface::FSceneQueryCommonParams(InQueryCallback, InQueryFilterData, InDebugParams))
	{
	}
	...
	TSQVisitor(const FTransform& InStartTM, const FVector& InDir, ChaosInterface::FSQHitBuffer<THitType>& InHitBuffer, EHitFlags InOutputFlags,
		const QueryGeometryType& InQueryGeom, const FAABB3& InQueryGeomWorldBounds, const ChaosInterface::FSceneQueryCommonParams& InCommonParams)
		: Dir(InDir)
		, OutputFlags(InOutputFlags)
		, bAnyHit(false)
		, DebugParams(InCommonParams.DebugParams)
	...
	TSQVisitor(const FTransform& InStartTM, const FVector& InDir, ChaosInterface::FSQHitBuffer<THitType>& InHitBuffer, EHitFlags InOutputFlags,
		...
		: TSQVisitor(InStartTM, InDir, InHitBuffer, InOutputFlags, InQueryGeom, InQueryGeomWorldBounds, ChaosInterface::FSceneQueryCommonParams(InQueryCallback, InQueryFilterData, InDebugParams))
	{
	}
	...
	TSQVisitor(const FTransform& InWorldTM, ChaosInterface::FSQHitBuffer<THitType>& InHitBuffer,
		const ChaosInterface::FQueryFilterData& InQueryFilterData, ICollisionQueryFilterCallbackBase& InQueryCallback, const QueryGeometryType& InQueryGeom, const FAABB3& InQueryGeomWorldBounds, const ChaosInterface::FQueryDebugParams& InDebugParams)
		: TSQVisitor(InWorldTM, InHitBuffer, InQueryGeom, InQueryGeomWorldBounds, ChaosInterface::FSceneQueryCommonParams(InQueryCallback, InQueryFilterData, InDebugParams))
	{
	}
	...
	virtual bool PrePreFilter(const TPayload& Payload) const override
	{
		return Payload.PrePreFilter(QueryFilterData);
	}
...
private:
	...
	bool Visit(const Chaos::TSpatialVisitorData<TPayload>& Instance, Chaos::FQueryFastData* CurData)
	{
		...
		if constexpr (bGTData)
		{
			...
			{
				UE_LOGF(LogChaos, Warning, "Query Dangling handle detected on game Thread. Payload Id: %d, Particle Id: %d", Payload.UniqueIdx().Idx, GeometryParticle->UniqueIdx().Idx);
		...
		else
		{
			...
			{
				UE_LOGF(LogChaos, Warning, "Query Dangling handle detected on Physics Thread. Payload Id: %d, Particle Id: %d", Payload.UniqueIdx().Idx, GeometryParticle->UniqueIdx().Idx);
		...
		for (int32 ShapeIndex = 0; ShapeIndex < Shapes.Num(); ++ShapeIndex)
		{
			...
			ECollisionQueryHitType HitType = QueryFlags & FChaosQueryFlag::ePREFILTER ? QueryCallback.PreFilter(QueryFilterData, *Shape, *GeometryParticle) : ECollisionQueryHitType::Block;
			...
			if (HitType != ECollisionQueryHitType::None)
			{
				...
				if (bHit)
				{
					...
					if (bAcceptHit)
					{
						...
						HitType = QueryFlags & FChaosQueryFlag::ePOSTFILTER ? QueryCallback.PostFilter(QueryFilterData, Hit) : HitType;
	...
	const Chaos::Filter::FQueryFilterData QueryFilterData;
	const QueryGeometryType* QueryGeom = nullptr;
	ICollisionQueryFilterCallback& QueryCallback;
...
struct TBPVisitor : public Chaos::ISpatialVisitor<TPayload, Chaos::FReal>
{
	...
	TBPVisitor(const FTransform& InWorldTM, ChaosInterface::FSQHitBuffer<THitType>& InHitBuffer,
		const QueryGeometryType& InQueryGeom, const ChaosInterface::FSceneQueryCommonParams& InCommonParams)
		...
		, StartTM(InWorldTM)
	{
		bAnyHit = QueryFlags & FPhysicsQueryFlag::eANY_HIT;
	}
	...
	TBPVisitor(const FTransform& InWorldTM, ChaosInterface::FSQHitBuffer<THitType>& InHitBuffer,
		const ChaosInterface::FQueryFilterData& InQueryFilterData, ICollisionQueryFilterCallbackBase& InQueryCallback, const QueryGeometryType& InQueryGeom, const ChaosInterface::FQueryDebugParams& InDebugParams)
		: TBPVisitor(InWorldTM, InHitBuffer, InQueryGeom, ChaosInterface::FSceneQueryCommonParams(InQueryCallback, InQueryFilterData, InDebugParams))
	{
	}
	PRAGMA_ENABLE_DEPRECATION_WARNINGS

	virtual ~TBPVisitor() = default;
...
private:
	...
	bool Visit(const Chaos::TSpatialVisitorData<TPayload>& Instance, Chaos::FQueryFastData* CurData)
	{
		...
		for (int32 ShapeIndex = 0; ShapeIndex < Shapes.Num(); ++ShapeIndex)
		{
			...
			ECollisionQueryHitType HitType = QueryFlags & FPhysicsQueryFlag::ePREFILTER ? QueryCallback.PreFilter(QueryFilterData, *Shape, *GeometryParticle) : ECollisionQueryHitType::Block;
			if (HitType != ECollisionQueryHitType::None)
			{
				const bool bBlocker = (HitType == ECollisionQueryHitType::Block || bAnyHit || HitBuffer.WantsSingleResult());
				Hit.Shape = Shape.Get();
...
void SweepHelper(const QueryGeomType& QueryGeom, const Chaos::ISpatialAcceleration<TPayload, Chaos::FReal, 3>& SpatialAcceleration, const FTransform& StartTM, const FVector& Dir, const float DeltaMagnitude, ChaosInterface::FSQHitBuffer<TSweepHit>& HitBuffer, EHitFlags OutputFlags, const ChaosInterface::FSceneQueryCommonParams& InCommonParams)
{
	...
	if (ensure(bIsValid))
	{
		TSQVisitor<QueryGeomType, TPayload, TSweepHit, std::is_same<TSweepHit, FSweepHit>::value> SweepVisitor(StartTM, Dir, HitBuffer, OutputFlags, QueryGeom, Bounds, InCommonParams);
...
void OverlapHelper(const QueryGeomType& QueryGeom, const Chaos::ISpatialAcceleration<TPayload, Chaos::FReal, 3>& SpatialAcceleration, const FTransform& GeomPose, ChaosInterface::FSQHitBuffer<TOverlapHit>& HitBuffer, const ChaosInterface::FSceneQueryCommonParams& InCommonParams)
{
	using namespace Chaos;
	using namespace ChaosInterface;
	...
	if (ensure(bIsValid))
	{
		...
		bool bSkipNarrowPhase = InCommonParams.Flags & FPhysicsQueryFlag::eSKIPNARROWPHASE;

		constexpr bool bGTData = std::is_same<TOverlapHit, FOverlapHit>::value;
		if (bSkipNarrowPhase)
		{
			TBPVisitor<QueryGeomType, TPayload, TOverlapHit, bGTData> OverlapVisitor(GeomPose, HitBuffer, QueryGeom, InCommonParams);
			SpatialAcceleration.Overlap(Bounds, OverlapVisitor);
		}
		else
		{
			TSQVisitor<QueryGeomType, TPayload, TOverlapHit, bGTData> OverlapVisitor(GeomPose, HitBuffer, QueryGeom, Bounds, InCommonParams);
			SpatialAcceleration.Overlap(Bounds, OverlapVisitor);
		}
		HitBuffer.DecFlushCount();
	}
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
template <typename QueryGeomType, typename TSweepHit, typename TPayload>
UE_DEPRECATED(5.8, "Use the overload that takes ChaosInterface::FSceneQueryCommonParams instead")
void SweepHelper(const QueryGeomType& QueryGeom, const Chaos::ISpatialAcceleration<TPayload, Chaos::FReal, 3>& SpatialAcceleration, const FTransform& StartTM, const FVector& Dir, const float DeltaMagnitude, ChaosInterface::FSQHitBuffer<TSweepHit>& HitBuffer, EHitFlags OutputFlags, const ChaosInterface::FQueryFilterData& QueryFilterData, ICollisionQueryFilterCallbackBase& QueryCallback, const ChaosInterface::FQueryDebugParams& DebugParams)
{
	...
	SweepHelper(QueryGeom, SpatialAcceleration, StartTM, Dir, DeltaMagnitude, HitBuffer, OutputFlags, CommonParams);
}
...
{
	const ChaosInterface::FSceneQueryCommonParams CommonParams(QueryCallback, QueryFilterData, DebugParams);
	OverlapHelper(QueryGeom, SpatialAcceleration, GeomPose, HitBuffer, CommonParams);
}
PRAGMA_ENABLE_DEPRECATION_WARNINGS
