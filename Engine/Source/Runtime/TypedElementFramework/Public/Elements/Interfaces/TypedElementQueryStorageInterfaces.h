...
namespace UE::Editor::DataStorage
{
	 ...
	 */
	struct ICommonQueryContext : public 
		Queries::ITableInfo<
		Queries::IDynamicColumnInfo<
		Queries::IContextCapability>>
	{
		...
		/**
		 * Defers a relation change until the end of the current chunk loop.
		 * Removes any existing relation of this type where Target is the Subject,
		 * then creates a new one with NewObject as the Object.
		 * Pass InvalidRowHandle for NewObject to remove the existing relation without creating a new one.
		 * Safe to call from within a processor callback — does not call ICoreProvider directly.
		 */
		virtual void SetRelation(
			RelationTypeHandle Type,
			RowHandle Target,
			RowHandle NewObject) = 0;
	...
	// query callbacks/context. Please keep both in sync, except for new entries, until the old query callbacks/context has been fully 
	// deprecated.
...
namespace UE::Editor::DataStorage
{
	...
	{
		const UScriptStruct* DynamicColumnType = FindDynamicColumnType<DynamicColumnTemplate>(Identifier);
		return DynamicColumnType ? HasColumn(Row, DynamicColumnType) : false;
	}
	...
	const DynamicColumnTemplate* ICommonQueryWithEnvironmentContext::GetColumn(const FName& Identifier) const
	{
		...
		const UScriptStruct* StructInfo = FindDynamicColumnType(*DynamicColumnTemplate::StaticStruct(), Identifier);
		if (StructInfo)
		{
