...
#include "Elements/Framework/TypedElementSearcher.h"
...
public:
	...
	/**
     * Constructs a list of classes that are used to convert the column data into a searchable string on the rows with the column(s)
     * associated with this widget constructor. A widget constructor can provide multiple searching constructors which will
     * allow the same list of rows to be searched in different ways.
     */
    TYPEDELEMENTFRAMEWORK_API virtual TArray<TSharedPtr<const UE::Editor::DataStorage::FColumnSearcherInterface>> ConstructColumnSearchers(
        UE::Editor::DataStorage::ICoreProvider* DataStorage,
        UE::Editor::DataStorage::IUiProvider* DataStorageUi,
        const UE::Editor::DataStorage::FMetaDataView& Arguments);
...
protected:
	...
	/**
	 * Helper function to perform a function on all column types for this widget (includes matched dynamic templates) 
	 */
	TYPEDELEMENTFRAMEWORK_API void ForAllColumnTypes(TFunctionRef<void(TNotNull<const UScriptStruct*>)> Function) const;
};
...
namespace UE::Editor::DataStorage
{
	...
	concept PropertyClassType = std::is_base_of_v<FProperty, PropertyClass>;
	...
	public:
		...
		using FWidgetConstructorPtr = TSharedPtr<FTypedElementWidgetConstructor>;
		using WidgetCreatedCallback = TFunctionRef<void(const TSharedRef<SWidget>& NewWidget, UE::Editor::DataStorage::RowHandle Row)>;
		using WidgetConstructionCallback = TFunctionRef<bool(FWidgetConstructorPtr, TConstArrayView<TWeakObjectPtr<const UScriptStruct>>)>;
		...
		using PropertySearcherConstructorCallback = 
			TFunction<TSharedPtr<const FColumnSearcherInterface>(TWeakObjectPtr<const UScriptStruct> ColumnType, const FProperty& Property)>;
		...
		/** Get the hierarchy handle that stores widget purposes relationships */
		virtual FHierarchyHandle GetWidgetPurposeHierarchyHandle() const = 0;
		...
		virtual void CreateWidgetConstructors(RowHandle PurposeRow, 
			const FMetaDataView& Arguments, const WidgetConstructionCallback& Callback) = 0;
		...
		/**
		 * @section Searcher generation
		 * Widget constructors can provide a custom searcher object that will allow Slate columns to convert Column data to an FString
		 * that the searcher supports. If none is provided however, TEDS UI will generate one based on the properties in a column that
		 * are marked with the meta data "Searchable". Out of the box TEDS UI supports string property types (FName, FString, FText), but
		 * additional properties can be registered with the functions below as well. Note that this is mechanic is exclusively for properties
		 * that are used in the dynamic generation of searchers. For complex searching that involves data specific knowledge or combines
		 * multiple values it's highly recommended to create a custom searcher and attach it to the appropriate widget constructor.
		 */

		/** Build searchers for the provided list of columns */
		virtual void GeneratePropertySearchers(TArray<TSharedPtr<const FColumnSearcherInterface>>& Results,
		...
		virtual void RegisterSearcherGeneratorForProperty(const FFieldClass* PropertyType, 
			PropertySearcherConstructorCallback PropertySearcherConstructor) = 0;

		/** Register a function to handle converting a property into a searcher. */
		template<PropertyClassType PropertyClass>
		void RegisterSearcherGeneratorForProperty(PropertySearcherConstructorCallback PropertySearcherConstructor);

		/** Unregister a function to handle converting a property into a searcher. */
		virtual void UnregisterSearcherGeneratorForProperty(const FFieldClass* PropertyType) = 0;
		...
		// Old widget constructor callback that takes in a UniquePtr, all new code should use the new WidgetConstructionCallback instead
		using WidgetConstructorCallback UE_DEPRECATED(5.8, "Use the new WidgetConstructionCallback alias instead") =
			TFunctionRef<bool(TUniquePtr<FTypedElementWidgetConstructor>, TConstArrayView<TWeakObjectPtr<const UScriptStruct>>)>;

		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		
		UE_DEPRECATED(5.8, "Use the overload that uses the new WidgetConstructionCallback instead")
		virtual void CreateWidgetConstructors(RowHandle PurposeRow, 
	...
	template<PropertyClassType PropertyClass>
	void IUiProvider::RegisterSearcherGeneratorForProperty(PropertySearcherConstructorCallback PropertySearcherConstructor)
	{
		RegisterSearcherGeneratorForProperty(PropertyClass::StaticClass(), PropertySearcherConstructor);
	}

	template<PropertyClassType PropertyClass>
	void IUiProvider::UnregisterSearcherGeneratorForProperty()
	{
		UnregisterSearcherGeneratorForProperty(PropertyClass::StaticClass());
	}

	template<PropertyClassType PropertyClass>
	void IUiProvider::RegisterSorterGeneratorForProperty(PropertySorterConstructorCallback PropertySorterConstructor)
	{
		RegisterSorterGeneratorForProperty(PropertyClass::StaticClass(), PropertySorterConstructor);
	}

	template<PropertyClassType PropertyClass>
	void IUiProvider::UnregisterSorterGeneratorForProperty()
	{
		UnregisterSorterGeneratorForProperty(PropertyClass::StaticClass());
	}
}