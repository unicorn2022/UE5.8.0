...
namespace UE::Editor::DataStorage::Queries::Private
{
	...
	constexpr bool TArgumentInfo<Args...>::HasFlags()
	{
		using NthArg = typename TTupleElement<Index, TTuple<Args...>>::Type;
		return EnumHasAllFlags(TArgument<NthArg>::Flags, Flags);
	}
	...
	template<typename... Args>
	void TArgumentInfo<Args...>::SetContext(ArgumentList& Arguments, const IContextContract& Contract)
	{
		if constexpr (ContextIndex >= 0)
		{
			SetContextUnguarded<ContextIndex>(Arguments, Contract);
	...
	template<typename... Args>
	constexpr bool TArgumentInfo<Args...>::IsContextConst()
	{
		if constexpr (ContextIndex >= 0)
		{
			return HasFlags<ContextIndex, EArgumentFlags::IsConst>();
		}
		else
		{
			return false;
	...
	template<typename... Args>
	template<int32 Index>
	void TArgumentInfo<Args...>::SetContextUnguarded(ArgumentList& Arguments, const IContextContract& Contract)
	{
		Arguments.template Get<Index>() = typename TTupleElement<Index, ArgumentList>::Type(Contract);
	}
