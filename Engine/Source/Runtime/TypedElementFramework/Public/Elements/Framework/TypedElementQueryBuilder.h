 ...
 *		Query = Select(FName("Example Callback"), Info, 
...
namespace UE::Editor::DataStorage::Queries
{
	...
	// ============================================================================
	// Relation Predicate Factory Functions
	// ============================================================================

	TYPEDELEMENTFRAMEWORK_API FQueryDescription::FRelationPredicate IsSubjectOf(RelationTypeHandle RelationType);
	TYPEDELEMENTFRAMEWORK_API FQueryDescription::FRelationPredicate IsSubjectOf(RelationTypeHandle RelationType, RowHandle Object);
	TYPEDELEMENTFRAMEWORK_API FQueryDescription::FRelationPredicate IsObjectOf(RelationTypeHandle RelationType);
	TYPEDELEMENTFRAMEWORK_API FQueryDescription::FRelationPredicate IsObjectOf(RelationTypeHandle RelationType, RowHandle Subject);
	TYPEDELEMENTFRAMEWORK_API FQueryDescription::FRelationPredicate IsDescendantOf(RelationTypeHandle RelationType, RowHandle Ancestor, bool bIncludeSelf = false);
	TYPEDELEMENTFRAMEWORK_API FQueryDescription::FRelationPredicate IsAncestorOf(RelationTypeHandle RelationType, RowHandle Descendant, bool bIncludeSelf = false);

	// ============================================================================
	// FQueryExpression — composite expression of column conditions and relation predicates.
	//
	// Build one with operator&& between:
	//   - TColumn<T>() or FConditions  (column conditions, evaluated at archetype level)
	//   - IsDescendantOf / IsSubjectOf / etc. free functions (relation predicates, per-entity)
	// All conditions are AND-composed.
	//
	// Example:
	//   using namespace UE::Editor::DataStorage::Queries;
	//   Select(...).Where(TColumn<FMyColumn>() && IsDescendantOf(MyRelation, RootRow)).Compile()
	// ============================================================================

	/**
	 * Composite query expression holding both column conditions (evaluated at archetype selection time
	 * via SetArchetypeMatchOverride) and relation predicates (evaluated per-entity after chunk selection).
	 * Column and relation conditions are AND-composed with each other.
	 */
	struct FQueryExpression
	{
		/** Column conditions. Evaluated at archetype level. Absent when only relation predicates are combined. */
		TOptional<FConditions> ColumnConditions;
		/** Relation predicates AND-composed with each other and with ColumnConditions. */
		TArray<FQueryDescription::FRelationPredicate, TInlineAllocator<2>> RelationAtoms;
	};

	// ---- Produce FQueryExpression: column atom + relation predicate ----
	TYPEDELEMENTFRAMEWORK_API FQueryExpression operator&&(FColumnBase Lhs, const FQueryDescription::FRelationPredicate& Rhs);
	TYPEDELEMENTFRAMEWORK_API FQueryExpression operator&&(const FQueryDescription::FRelationPredicate& Lhs, FColumnBase Rhs);
	// ---- Produce FQueryExpression: FConditions + relation predicate ----
	TYPEDELEMENTFRAMEWORK_API FQueryExpression operator&&(const FConditions& Lhs, const FQueryDescription::FRelationPredicate& Rhs);
	TYPEDELEMENTFRAMEWORK_API FQueryExpression operator&&(const FQueryDescription::FRelationPredicate& Lhs, const FConditions& Rhs);
	// ---- Produce FQueryExpression: relation predicate + relation predicate ----
	TYPEDELEMENTFRAMEWORK_API FQueryExpression operator&&(const FQueryDescription::FRelationPredicate& Lhs, const FQueryDescription::FRelationPredicate& Rhs);
	// ---- Extend an existing FQueryExpression ----
	TYPEDELEMENTFRAMEWORK_API FQueryExpression operator&&(FQueryExpression Lhs, const FQueryDescription::FRelationPredicate& Rhs);
	TYPEDELEMENTFRAMEWORK_API FQueryExpression operator&&(FQueryExpression Lhs, FColumnBase Rhs);
	TYPEDELEMENTFRAMEWORK_API FQueryExpression operator&&(FQueryExpression Lhs, const FConditions& Rhs);
	...
	public:
		...
		/**
		 * Filter to rows that are subjects in relations of the given type.
		 * E.g., IsSubjectOf(SubclassOfType) finds all rows that inherit from something.
		 */
		TYPEDELEMENTFRAMEWORK_API FSimpleQuery& IsSubjectOf(RelationTypeHandle RelationType);

		/**
		 * Filter to rows that are subjects in relations of the given type where the object
		 * is the specified row. E.g., IsSubjectOf(SubclassOfType, UObject) finds direct subclasses of UObject.
		 */
		TYPEDELEMENTFRAMEWORK_API FSimpleQuery& IsSubjectOf(RelationTypeHandle RelationType, RowHandle Object);

		/**
		 * Filter to rows that are objects in relations of the given type.
		 * E.g., IsObjectOf(SubclassOfType) finds all rows that have something inheriting from them.
		 */
		TYPEDELEMENTFRAMEWORK_API FSimpleQuery& IsObjectOf(RelationTypeHandle RelationType);

		/**
		 * Filter to rows that are objects in relations of the given type where the subject
		 * is the specified row. E.g., IsObjectOf(SubclassOfType, UDerivedClass) finds UDerivedClass's parent.
		 */
		TYPEDELEMENTFRAMEWORK_API FSimpleQuery& IsObjectOf(RelationTypeHandle RelationType, RowHandle Subject);
		...
		/**
		 * Filter to rows that are descendants of the specified ancestor in the given hierarchical relation.
		 * Uses interval encoding for O(1) evaluation per row.
		 * @param RelationType Must be a hierarchical relation type (HierarchyMode != EHierarchyMode::Disabled)
		 * @param Ancestor The ancestor row to test against
		 * @param bIncludeSelf If true, the ancestor itself will also match
		 */
		TYPEDELEMENTFRAMEWORK_API FSimpleQuery& IsDescendantOf(RelationTypeHandle RelationType, RowHandle Ancestor, bool bIncludeSelf = false);

		/**
		 * Filter to rows that are ancestors of the specified descendant in the given hierarchical relation.
		 * @param RelationType Must be a hierarchical relation type (HierarchyMode != EHierarchyMode::Disabled)
		 * @param Descendant The descendant row to test against
		 * @param bIncludeSelf If true, the descendant itself will also match
		 */
		TYPEDELEMENTFRAMEWORK_API FSimpleQuery& IsAncestorOf(RelationTypeHandle RelationType, RowHandle Descendant, bool bIncludeSelf = false);
	...
	public:
		...
		// Query Condition API
		TYPEDELEMENTFRAMEWORK_API FQueryConditionQuery Where(Queries::FConditions Condition);
		TYPEDELEMENTFRAMEWORK_API FQueryConditionQuery Where(Queries::FQueryExpression Expression);
	...
	AUTORTFM_INFER DirectQueryCallback CreateDirectQueryCallbackBinding(Function&& Callback);
	template<typename Function>
	AUTORTFM_INFER SubqueryCallback CreateSubqueryCallbackBinding(Function&& Callback);
