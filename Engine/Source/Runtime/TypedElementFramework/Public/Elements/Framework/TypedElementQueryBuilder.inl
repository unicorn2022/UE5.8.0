...
namespace UE::Editor::DataStorage::Queries
{
	...
	namespace Private
	{
			...
			, public FContextInfo<SourceContext, TargetContext>
		{
			...
			AUTORTFM_INFER void Call(SourceContext& Context, const CallerType& Caller)
			{
				...
				if constexpr (SuperColumn::bArePointerColumns || std::is_pointer_v<RowHandleType>)
				{
					...
					{
						std::apply([this, Rows = Rows.GetData(), &Caller] AUTORTFM_ENABLE (auto&&... Column)
							{
								if constexpr (AUTORTFM_MODE_OF_CALL(Caller(this->ContextWrapper, Rows, Column...)) == autortfm_mode_disable) //-V501
								{
				...
				else
				{
					for (RowHandle Row : Rows)
					{
						if constexpr (sizeof...(Args) > 0)
						{
							std::apply([this, Row, &Caller] AUTORTFM_ENABLE (auto&&... Column)
								{
									if constexpr (AUTORTFM_MODE_OF_CALL(Caller(this->ContextWrapper, Row, *Column...)) == autortfm_mode_disable) //-V501
									{
										AutoRTFM::UnreachableIfClosed();
									}
		...
		AUTORTFM_INFER void CallQueryFunction(const FQueryDescription& Description, SourceContext& Context,
		...
		AUTORTFM_INFER void CallQueryFunction_Expand(const FQueryDescription& Description, SourceContext& Context,
