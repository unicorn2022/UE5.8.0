...
public:
	...
	void LogReference() const
	{
		...
		for (uint32 CallstackIndex = TypedHandleRefTrackingSkipCount; CallstackIndex < CallstackDepth; ++CallstackIndex)
		{
			...
			UE_LOGF(LogCore, Error, "%ls", ANSI_TO_TCHAR(CallstackText));
		}
	}
...
public:
	...
	void LogReferences() const
	{
		...
		UE_LOGF(LogCore, Error, "External Element References:");
		...
		{
			UE_LOGF(LogCore, Error, "Destruction requested by:");
	...
	void StoreDestructionRequestCallstack()
	{
		...
		if (DestructionRequestCallstack)
		{
			...
			DestructionRequestCallstack->LogReference();
			UE_LOGF(LogCore, Error, "===============================================");
			UE_LOGF(LogCore, Fatal, "Element has already had its destruction callstack set! (see above)");
		}
#endif	// DO_CHECK
...
public:
	...
	void LogExternalReferencesOnDestruction() const
	{
		...
		{
#if UE_TYPED_ELEMENT_HAS_REFTRACKING
			UE_LOGF(LogCore, Warning, "Element '%ls' is still externally referenced when being destroyed. Ref-count: %d;", *GetDebugId(), LocalRefCount);
			if (References)
			{
			...
			{
				UE_LOGF(LogCore, Warning, "Enable CVar 'TypedElements.EnableReferenceTracking' for Element reference tracking.");
			}
#else	// UE_TYPED_ELEMENT_HAS_REFTRACKING
			UE_LOGF(LogCore, Warning, "Element '%ls' is still externally referenced when being destroyed. Ref-count: %d", *GetDebugId(), LocalRefCount);
			UE_LOGF(LogCore, Warning, "UE_TYPED_ELEMENT_HAS_REFTRACKING is disabled. Enable it, recompile and enable CVar 'TypedElements.EnableReferenceTracking' for reference tracking.");
#endif	// UE_TYPED_ELEMENT_HAS_REFTRACKING
		}
#endif	// DO_CHECK
	}
...
public:
	...
	TTypedElementInternalData<void>& AddDataForElement(const FTypedHandleTypeId InTypeId, FTypedHandleElementId& InOutElementId)
	{
		...
		TTypedElementInternalData<void>& InternalData = InternalDataArray[InternalDataArrayIndex];
		InternalData.Initialize(InTypeId, InOutElementId);
		return InternalData;
	}
