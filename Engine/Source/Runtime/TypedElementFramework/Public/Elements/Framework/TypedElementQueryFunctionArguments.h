...
namespace UE::Editor::DataStorage::Queries::Private
{
	...
	template<typename T> concept ContextType = std::is_base_of_v<FQueryContext, T> || std::is_base_of_v<FConstQueryContext, T>;
	...
	struct TArgument<T>
	{
		static_assert(!std::is_reference_v<T> && !std::is_pointer_v<T>, "The context can only be passed in by value.");
		...
		constexpr static EArgumentFlags Flags =
			...
			(std::is_base_of_v<FConstQueryContext, BaseType> ? EArgumentFlags::IsConst : EArgumentFlags::IsMutable) |
	...
	private:
		...
		template<int32 Index, EArgumentFlags Flags>
		constexpr static bool HasFlags();
	...
	public:
		...
		static void SetContext(ArgumentList& Arguments, const IContextContract& Contract);
		constexpr static bool IsContextConst();
	...
	private:
		...
		template<int32 Index>
		static void SetContextUnguarded(ArgumentList& Arguments, const IContextContract& Contract);
		template<int32 Index>
