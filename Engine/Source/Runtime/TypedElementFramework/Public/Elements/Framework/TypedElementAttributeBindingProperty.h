...
namespace UE::Editor::DataStorage
{
	namespace Private
	{
		...
		// Concept for a conversion function used by the attribute binder to bind a lambda to a TFunction accepting a bool argument representing if
		// the given column is present on the row
        template <typename FunctionType>
        concept AttributeBinderColumnPresenceInvocable = std::is_invocable_v<std::decay_t<FunctionType>, const bool>;
