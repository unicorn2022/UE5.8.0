...
namespace UE::Editor::DataStorage
{
    ...
    TAttribute<AttributeType> FAttributeBinder::BindColumnsPresence(TFunction<AttributeType(const bool)> InConverter)
    {
    	if(DataStorage)
    	{
    		return TAttribute<AttributeType>::CreateLambda(
    			...
    			{
    				return Convertor(Storage->HasColumns<ColumnTypes...>(Row));
    			});
    	}
    	return TAttribute<AttributeType>();
    }
	...
	template <TColumnType... ColumnTypes, typename FunctionType>
		requires DataStorage::Private::AttributeBinderColumnPresenceInvocable<FunctionType>
	auto FAttributeBinder::BindColumnsPresence(FunctionType&& InConverter)
	{
		// Deduce the attribute type from the return value of the converter function
		using AttributeType = decltype(InConverter(std::declval<const bool>()));
    	
		return BindColumnsPresence<AttributeType, ColumnTypes...>(TFunction<AttributeType(const bool)>(MoveTemp(InConverter)));
	}

	template <typename AttributeType>
	TAttribute<AttributeType> FAttributeBinder::BindColumnsPresence(Queries::FConditions InConditions, TFunction<AttributeType(const bool)> InConverter)
	{
		if(DataStorage)
		{
			if (!InConditions.IsCompiled())
			{
				InConditions.Compile(Queries::FEditorStorageQueryConditionCompileContext(DataStorage));
			}
			return TAttribute<AttributeType>::CreateLambda(
				[
				...
				{
					return Convertor(Storage->MatchesColumns(Row, Conditions));
				});
		}
		return TAttribute<AttributeType>();
	}
	...
	template <typename FunctionType>
		requires DataStorage::Private::AttributeBinderColumnPresenceInvocable<FunctionType>
	auto FAttributeBinder::BindColumnsPresence(Queries::FConditions InConditions, FunctionType&& InConverter)
	{
		// Deduce the attribute type from the return value of the converter function
		using AttributeType = decltype(InConverter(std::declval<const bool>()));
    	
		return BindColumnsPresence<AttributeType>(InConditions, TFunction<AttributeType(const bool)>(MoveTemp(InConverter)));
	}
	...
	template <typename InRetValType, typename... ParamTypes, TDataColumnType ColumnType>
	TDelegate<InRetValType(ParamTypes...)> FAttributeBinder::BindEvent(TSharedPtr<TDelegate<InRetValType(ParamTypes...)>> ColumnType::* InVariable)
	{
		using namespace DataStorage::Private;

		TProperty<TSharedPtr<TDelegate<InRetValType(ParamTypes...)>>> Prop;
		Prop.Bind(InVariable);
			...
			Row = TargetRow](ParamTypes&&... Params)
		{
			if (ColumnType* Column = GetColumn<ColumnType>(Storage, Row))
			{
				TSharedPtr<TDelegate<InRetValType(ParamTypes...)>>& SharedDelegate = Property.Get(*Column);
				if (SharedDelegate && SharedDelegate->IsBound())
				{
					return SharedDelegate->Execute(Forward<ParamTypes>(Params)...);
				}
			}
			return InRetValType();
		});
	...
	template <typename InRetValType, typename... ParamTypes, TDynamicColumnTemplate ColumnType>
	TDelegate<InRetValType(ParamTypes...)> FAttributeBinder::BindEvent(const FName& InIdentifier, TSharedPtr<TDelegate<InRetValType(ParamTypes...)>> ColumnType::* InVariable)
	{
		using namespace DataStorage::Private;
	...
	TAttribute<InRetValType> FAttributeBinder::BindEventAttribute(
		TSharedPtr<TDelegate<InRetValType()>> ColumnType::* InVariable,
		InRetValType DefaultValue)
	{
		using namespace DataStorage::Private;

		TProperty<TSharedPtr<TDelegate<InRetValType()>>> Prop;
		Prop.Bind(InVariable);

		return TAttribute<InRetValType>::CreateLambda([Property = MoveTemp(Prop), Storage = DataStorage,
			Row = TargetRow, Default = MoveTemp(DefaultValue)]()
		{
			if (ColumnType* Column = GetColumn<ColumnType>(Storage, Row))
			{
				TSharedPtr<TDelegate<InRetValType()>>& SharedDelegate = Property.Get(*Column);
				if (SharedDelegate && SharedDelegate->IsBound())
				{
					return SharedDelegate->Execute();
				}
			}
			return Default;
		});
		...
		InRetValType DefaultValue)
	{
		...
		return TAttribute<InRetValType>::CreateLambda([Property = MoveTemp(Prop), Storage = DataStorage,
			Row = TargetRow, Identifier = InIdentifier, Default = MoveTemp(DefaultValue)]()
		{
			if (ColumnType* Column = GetColumn<ColumnType>(Storage, Row, Identifier))
			{
				TSharedPtr<TDelegate<InRetValType()>>& SharedDelegate = Property.Get(*Column);
