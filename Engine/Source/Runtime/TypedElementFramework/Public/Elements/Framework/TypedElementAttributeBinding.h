...
namespace UE::Editor::DataStorage
{
	...
	public:
		 ...
		 * Example:
		 * FAttributeBinder Binder(RowHandle);
		 * TAttribute<FString> TestAttribute(Binder.BindData(&FTypedElementLabelColumn::Label, FName("Id")))
		 */
		...
		*  @param InIdentifier The identifier for this column if it is a dynamic column
		...
		/**
         * Bind the presence of a TEDS tag or column to a slate attribute (instead of a single member variable). This allows you to derive an
         * attribute from the presence of a tag on a row
         *
         * @param InConverter Conversion function to convert from a bool (aka bHasColumn) -> AttributeType
         * @return A TAttribute bound to the row, column pair specified
         *
         * Example:
         *			const TAttribute TestHasIntAttribute(Binder.BindColumnsPresence<FTestHasIntTag>([](bool bHasColumn)
         *			{
         *				return bHasColumn ? Image1 : Image2;
         *			}));
         */
        template <typename AttributeType, TColumnType... ColumnTypes>
        TAttribute<AttributeType> BindColumnsPresence(TFunction<AttributeType(const bool)> InConverter);
		...
		auto BindColumnsPresence(FunctionType&& InConverter);
		
		/**
		 * Overload for the BindColumnPresence to accept FConditions instead of TColumnTypes
		 * @param InConditions Conditions to check the row if it matches (can include dynamic columns)
		 * @param InConverter Conversion function to convert from a bool (aka bHasColumn) -> AttributeType
		 * @return A TAttribute bound to the row, column pair specified
		 */
		template <typename AttributeType>
		TAttribute<AttributeType> BindColumnsPresence(Queries::FConditions InConditions, TFunction<AttributeType(const bool)> InConverter);
		...
		template <typename FunctionType>
			requires DataStorage::Private::AttributeBinderColumnPresenceInvocable<FunctionType>
		auto BindColumnsPresence(Queries::FConditions InConditions, FunctionType&& InConverter);
		...
		// BindEvent overrides for TSharedPtr<TDelegate> since raw delegates inside of columns are unsafe because they are not trivially relocatable
		template <typename InRetValType, typename... ParamTypes, TDataColumnType ColumnType>
		TDelegate<InRetValType(ParamTypes...)> BindEvent(TSharedPtr<TDelegate<InRetValType(ParamTypes...)>> ColumnType::* InVariable);

		template <typename InRetValType, typename... ParamTypes, TDynamicColumnTemplate ColumnType>
		TDelegate<InRetValType(ParamTypes...)> BindEvent(const FName& InIdentifier, TSharedPtr<TDelegate<InRetValType(ParamTypes...)>> ColumnType::* InVariable);
		...
		TAttribute<InRetValType> BindEventAttribute(
			...
			InRetValType DefaultValue = InRetValType{});
