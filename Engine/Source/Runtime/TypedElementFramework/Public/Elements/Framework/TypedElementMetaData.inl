...
namespace UE::Editor::DataStorage
{
	...
	template<typename Column>
	FMetaDataEntryView FMetaDataView::FindForColumnOrGeneric(FName AttributeName) const
	{
		return FindForColumnOrGeneric(Column::StaticStruct(), AttributeName);
	}

	template<typename... Columns>
	FMetaDataEntryView FMetaDataView::FindForColumnsOrGeneric(FName AttributeName) const
	{
		const TWeakObjectPtr<const UScriptStruct> ColumnStructs[] = { Columns::StaticStruct()... };
		return FindForColumnsOrGeneric(ColumnStructs, AttributeName);
	}
