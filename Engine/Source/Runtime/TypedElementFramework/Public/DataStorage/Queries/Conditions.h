...
namespace Queries
{
	...
	protected:
		FColumnBase() = default;
		explicit FColumnBase(TWeakObjectPtr<const UScriptStruct> ColumnTypeInfo, const FName& InIdentifier)
			: TypeInfo(ColumnTypeInfo)
			, Identifier(InIdentifier)
		{}
	};

	template<typename T, TVariantName Id = TVariantName<0>()>
	struct TColumn final : public FColumnBase
	{
		explicit TColumn() : FColumnBase(T::StaticStruct(), NAME_None) {}
		explicit TColumn(const FName& Identifier) : FColumnBase(T::StaticStruct(), Identifier) {}
	};

	template<typename T, TVariantName Id> requires (decltype(Id)::Size > 0)
	struct TColumn<T, Id> final : public FColumnBase
	{
		explicit TColumn() : FColumnBase(T::StaticStruct(), []()
			{
				static FName Name = Id.GetName();
				return Name;
			}()) {}
	};

	template<>
	struct TColumn<void> final : public FColumnBase
	{
		explicit TColumn(TWeakObjectPtr<const UScriptStruct> ColumnTypeInfo) : FColumnBase(ColumnTypeInfo, NAME_None) {}
		explicit TColumn(TWeakObjectPtr<const UScriptStruct> ColumnTypeInfo, const FName& Identifier)
			: FColumnBase(ColumnTypeInfo, Identifier)
		{
		}
	};
	TColumn(TWeakObjectPtr<const UScriptStruct>) -> TColumn<void>;
	TColumn(TWeakObjectPtr<const UScriptStruct>, const FName&) -> TColumn<void>;

	template<typename T>
	concept TColumnDescType = std::is_base_of_v<FColumnBase, T>;
