...
namespace UE::Editor::DataStorage
{
	...
	const UScriptStruct* FColumnMetaData::GetColumnType() const
	{
		return ColumnType;
	}
	...
	FMetaDataEntryView FMetaDataView::FindForColumnOrGeneric(TWeakObjectPtr<const UScriptStruct> Column, FName AttributeName) const
	{
		FMetaDataEntryView ColumnMetaData = FindForColumn(Column, AttributeName);
		return ColumnMetaData.IsSet() ? ColumnMetaData : FindGeneric(AttributeName);
	}

	FMetaDataEntryView FMetaDataView::FindForColumnsOrGeneric(const TConstArrayView<TWeakObjectPtr<const UScriptStruct>> Columns, FName AttributeName) const
	{
		for (const TWeakObjectPtr<const UScriptStruct>& Column : Columns)
		{
			FMetaDataEntryView ColumnMetaData = FindForColumn(Column, AttributeName);
			if (ColumnMetaData.IsSet())
			{
				return ColumnMetaData;
			}
		}
		return FindGeneric(AttributeName);
	}
	...
	FColumnsMetaDataView::FColumnsMetaDataView(TConstArrayView<TSharedPtr<FColumnMetaData>> InColumnsMetaData)
		: ColumnsMetaData(InColumnsMetaData)
	{
	}
		...
		TWeakObjectPtr<const UScriptStruct> Column, FName AttributeName) const
	{
		...
		const TSharedPtr<FColumnMetaData>* MetaData = ColumnsMetaData.FindByPredicate([Column](const TSharedPtr<FColumnMetaData>& MetaData)
			{
				return MetaData && MetaData->GetColumnType() == Column;
			});
