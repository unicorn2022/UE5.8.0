...
{
	UE_LOGF(LogWebBrowserTexture, VeryVerbose, "FWebBrowserTextureResource:FWebBrowserTextureResource %d %d", OwnerDim.X, OwnerDim.Y);
}
...
{
	UE_LOGF(LogWebBrowserTexture, VeryVerbose, "FWebBrowserTextureResource:ClearTexture");
...
void FWebBrowserTextureResource::CopySample(FRHICommandListBase& RHICmdList, const TSharedPtr<FWebBrowserTextureSample, ESPMode::ThreadSafe>& Sample, const FLinearColor& ClearColor)
{
	...
	{
		UE_LOGF(LogWebBrowserTexture, VeryVerbose, "FWebBrowserTextureResource:CopySample 1");
		// use sample's texture as the new render target.
		if (TextureRHI != SampleTexture2D)
		{
			UE_LOGF(LogWebBrowserTexture, VeryVerbose, "FWebBrowserTextureResource:CopySample 11");
	...
	{
		UE_LOGF(LogWebBrowserTexture, VeryVerbose, "FWebBrowserTextureResource:CopySample 2");
		...
		UE_LOGF(LogWebBrowserTexture, VeryVerbose, "WebBrowserTextureResource:CopySample: %d x %d", SampleDim.X, SampleDim.Y);

		// copy sample data to output render target
		FUpdateTextureRegion2D Region(0, 0, 0, 0, SampleDim.X, SampleDim.Y);
		RHICmdList.UpdateTexture2D(RenderTargetTextureRHI.GetReference(), 0, Region, Sample->GetStride(), (uint8*)Sample->GetBuffer());
	}
	Cleared = false;
}
...
{
	UE_LOGF(LogWebBrowserTexture, VeryVerbose, "FWebBrowserTextureResource:UpdateResourceSize");
...
void FWebBrowserTextureResource::UpdateTextureReference(FRHICommandListBase& RHICmdList, FRHITexture* NewTexture)
{
	...
	UE_LOGF(LogWebBrowserTexture, VeryVerbose, "FWebBrowserTextureResource:UpdateTextureReference: %d x %d", OwnerDim.X, OwnerDim.Y);
}
