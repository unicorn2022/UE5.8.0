...
void FFragmentType::ToString(const void* Payload, FStringBuilderBase& OutStringBuilder) const
{
	...
	const FStringView StringToEncode = OutStringBuilder.ToString();
	FString EncodedString = FGenericPlatformHttp::UrlEncode(StringToEncode);
	OutStringBuilder = EncodedString;
	...
	FStringView StringRepresentation = FStringView(OutStringBuilder).RightChop(StartPos);
...
{
	FString DecodedString = FGenericPlatformHttp::UrlDecode(InString);
	return (*InstanceBindings.TryParseString)(Payload, DecodedString, Params);
}
