...
private:
	static UGameplayDebuggerLocalController* GetController(UWorld* InWorld)
	{
		...
		UE_CLOGF(Controller == nullptr, LogConsoleResponse, Error, "GameplayDebugger not available");
		return Controller;
	}
	...
	static void ToggleCategory(const TArray<FString>& Args, UWorld* InWorld)
	{
		...
		if (Args.Num() != 1)
		{
			UE_LOGF(LogConsoleResponse, Error, "Missing category index parameter. Usage: gdt.ToggleCategory <CategoryIdx>");
			return;
		}

		if (!Args[0].IsNumeric())
		{
			UE_LOGF(LogConsoleResponse, Error, "Must provide numerical value as index. Usage: gdt.ToggleCategory <CategoryIdx>");
			return;
		}
		...
		{
			UE_LOGF(LogConsoleResponse, Error, "Requires a category index in the active row [0..%d]. Usage: gdt.ToggleCategory CategoryIndex", MaxSlotIdx);
			return;
		}
	...
	static void EnableCategoryName(const TArray<FString>& Args, UWorld* InWorld)
	{
		...
		{
			UE_CLOGF(Controller != nullptr && Controller->CachedReplicator == nullptr, 
				LogConsoleResponse, Error, "Failed due to CachedReplicator being Null");
			return;
		}

		if (Args.Num() < 1)
		{
			UE_LOGF(LogConsoleResponse, Error, "Missing category name parameter. Usage: gdt.EnableCategory <CategoryNamePattern> [Enable]");
			return;
		}
	...
	static void SetFontSize(const TArray<FString>& Args, UWorld* InWorld)
	{
		if (Args.Num() != 1)
		{
			UE_LOGF(LogConsoleResponse, Error, "Missing \'fontSize\' parameter. Usage: gdt.fontsize <fontSize>");
			return;
		}

		if (!Args[0].IsNumeric())
		{
			UE_LOGF(LogConsoleResponse, Error, "Must provide numerical value as \'fontSize\'. Usage: gdt.fontsize <fontSize>");
			return;
		}
