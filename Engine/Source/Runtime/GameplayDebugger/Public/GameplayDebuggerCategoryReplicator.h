...
protected:
	...
	GAMEPLAYDEBUGGER_API virtual UPlayer* GetNetOwningPlayer() override;
	GAMEPLAYDEBUGGER_API virtual UPlayer* GetNetOwningPlayerAnyRole() override;
...
protected:
	...
	UE_DEPRECATED(5.8, "ServerSetCategoryEnabled by CategoryId does not work when server/client mismatch indices, use ServerSetCategoryNameEnabled")
	UFUNCTION(Server, Reliable, WithValidation, meta = (CallInEditor = "true"))
	GAMEPLAYDEBUGGER_API void ServerSetCategoryEnabled(int32 CategoryId, bool bEnable);

	UFUNCTION(Server, Reliable, WithValidation, meta = (CallInEditor = "true"))
	GAMEPLAYDEBUGGER_API void ServerSetCategoryNameEnabled(FName CategoryName, bool bEnable);

	/** helper function for replicating input for category handlers */
	UE_DEPRECATED(5.8, "ServerSendCategoryInputEvent by CategoryId does not work when server/client mismatch indices, use ServerSendCategoryNameInputEvent")
	UFUNCTION(Server, Reliable, WithValidation, meta = (CallInEditor = "true"))
	GAMEPLAYDEBUGGER_API void ServerSendCategoryInputEvent(int32 CategoryId, int32 HandlerId);

	UFUNCTION(Server, Reliable, WithValidation, meta = (CallInEditor = "true"))
	GAMEPLAYDEBUGGER_API void ServerSendCategoryNameInputEvent(FName CategoryName, int32 HandlerId);
