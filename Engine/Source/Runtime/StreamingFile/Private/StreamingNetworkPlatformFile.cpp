...
bool FStreamingNetworkPlatformFile::ShouldBeUsed(IPlatformFile* Inner, const TCHAR* CmdLine) const
{
	...
	if (bResult)
	{
		...
		if (FPlatformMisc::AllowLocalCaching())
		{
			...
			{
				UE_LOGF(LogStreamingPlatformFile, Display, "Platform supports local caching, but requires explicitly specifying -AllowCaching to use it.");
			}
			bResult = bResult || !bAllowCaching;
		}
		UE_CLOGF(bResult, LogStreamingPlatformFile, Display, "Using streaming network file system.");
	}
...
bool FStreamingNetworkPlatformFile::InitializeInternal(IPlatformFile* Inner, const TCHAR* HostIP)
{
	...
	if (HostIP == nullptr)
	{
		UE_LOGF(LogStreamingPlatformFile, Error, "No Host IP specified in the commandline.");
		bIsUsable = false;
	...
	{
		UE_LOGF(LogStreamingPlatformFile, Display, "Overriding file server port: %d", OverridePort);
	...
	if(SendPayloadAndReceiveResponse(Payload,Response))
	{
		...
		if (TestFileHandle != nullptr)
		{
			...
			{
				UE_LOGF(LogStreamingPlatformFile, Fatal, "Could not read test file %ls.", *TestSyncFile);
			}
		...
		{
			UE_LOGF(LogStreamingPlatformFile, Fatal, "Could not open test file %ls.", *TestSyncFile);
		}
