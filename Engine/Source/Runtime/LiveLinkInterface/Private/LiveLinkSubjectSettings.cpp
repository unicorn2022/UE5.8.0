	...
	else
	{
		...
		for (int32 Index = 0; Index < PreProcessors.Num(); ++Index)
		{
			if (ULiveLinkFramePreProcessor* PreProcessor = PreProcessors[Index])
			{
				...
				{
					UE_LOGF(LogLiveLinkSubjectSettings, Warning, "Role '%ls' is not supported by pre processors '%ls'", *RoleClass->GetName(), *PreProcessor->GetName());
		...
		if (InterpolationProcessor)
		{
			...
			{
				UE_LOGF(LogLiveLinkSubjectSettings, Warning, "Role '%ls' is not supported by interpolation '%ls'", *RoleClass->GetName(), *InterpolationProcessor->GetName());
				InterpolationProcessor = nullptr;
				bValidProcessors = false;
			}
		}
		...
		for (int32 Index = 0; Index < Translators.Num(); ++Index)
		{
			if (ULiveLinkFrameTranslator* Translator = Translators[Index])
			{
				...
				{
					UE_LOGF(LogLiveLinkSubjectSettings, Warning, "Role '%ls' is not supported by translator '%ls'", *RoleClass->GetName(), *Translator->GetName());
		...
		if (Remapper)
		{
			...
			{
				UE_LOGF(LogLiveLinkSubjectSettings, Warning, "Role '%ls' is not supported by remapper '%ls', only %ls is supported", *RoleClass->GetName(), *Remapper->GetName(), *Remapper->GetSupportedRole()->GetName());
				Remapper = nullptr;
				bValidProcessors = false;
			}
		}
