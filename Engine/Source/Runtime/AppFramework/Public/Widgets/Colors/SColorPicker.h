...
public:
	...
	struct FEyeDropperButtonArgs
	{
		...
		TDelegate<void(FLinearColor)> OnValueChanged;
		...
		TDelegate<void(bool)> OnComplete;

		/** Sets the display Gamma setting - used to correct colors sampled from the screen */
		TAttribute<float> DisplayGamma = TAttribute<float>();
	};

	/** Creates a standalone eye dropper button that functions identically to the button within the color picker */
	static APPFRAMEWORK_API TSharedRef<SButton> CreateStandaloneEyeDropperButton(const FEyeDropperButtonArgs& InArgs);
