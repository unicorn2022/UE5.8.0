...
void SColorThemesViewer::SetCurrentColorTheme(TSharedPtr<FColorTheme> NewTheme)
{
	...
	if (FPaths::FileExists(GEditorPerProjectIni) && CurrentlySelectedThemePtr.IsValid())
	{
		GConfig->SetString(TEXT("ColorPickerUI"), TEXT("CurrentTheme"), *CurrentlySelectedThemePtr.Pin()->Name, GEditorPerProjectIni);
	}
...
void SColorThemesViewer::LoadColorThemesFromIni()
{
	...
	if (FPaths::FileExists(GEditorPerProjectIni))
	{
		const FString ThemeName = GConfig->GetStr(TEXT("ColorPickerUI"), TEXT("CurrentTheme"), GEditorPerProjectIni);

		if (TSharedPtr<FColorTheme>* CurrentTheme = Algo::FindBy(ColorThemes, ThemeName, &FColorTheme::Name))
		{
			CurrentlySelectedThemePtr = *CurrentTheme;
		}
	}
}
