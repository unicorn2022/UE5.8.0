										...
										+ SHorizontalBox::Slot()
											...
											[
												SAssignNew(ColorWheel, SColorWheel)
													...
													.ProcessMouseMoveImmediate(bOnlyRefreshOnMouseUp || bOnlyRefreshOnOk)
											]
...
TSharedRef<SButton> SColorPicker::CreateStandaloneEyeDropperButton(const FEyeDropperButtonArgs& InArgs)
{
	return SNew(SEyeDropperButton)
		.OnValueChanged(InArgs.OnValueChanged)
		.OnBegin(InArgs.OnBegin)
		.OnComplete(InArgs.OnComplete)
		.DisplayGamma(InArgs.DisplayGamma);
}
	...
	else
	{
		...
		else if (bNeedToAddWindow)
		{
			if (Args.ParentWidget.IsValid())
			{
				...
				if (FSlateApplication::Get().GeneratePathToWidgetUnchecked(Args.ParentWidget.ToSharedRef(), WidgetPath))
				{
