...
namespace Audio
{
	...
	bool FMixerPlatformAudioUnit::InitializeHardware()
	{
		...
		if (!Success)
		{
			UE_LOGF(LogAudioMixerAudioUnit, Display, "Error setting sample rate.");
		}
		...
		if (!Success)
		{
			UE_LOGF(LogAudioMixerAudioUnit, Display, "Error while setting spatial experience to bypass for VisionOS.");
		}
#endif
		...
		UE_LOGF(LogAudioMixerAudioUnit, Display, "Device Sample Rate: %f", GraphSampleRate);
		...
		if (!Success)
		{
			UE_LOGF(LogAudioMixerAudioUnit, Display, "Error starting audio session.");
		}
		
		UE_LOGF(LogAudioMixerAudioUnit, Display, "Bytes per submitted buffer: %d", BytesPerSubmittedBuffer);
		...
		Status = AUGraphSetNodeInputCallback(AudioUnitGraph,
											 ...
											 0,
											 &InputCallback);
		UE_CLOGF(Status != noErr, LogAudioMixerAudioUnit, Error, "Failed to set input callback for audio output node");
	...
	void FMixerPlatformAudioUnit::ResumeContext()
	{
		if (SuspendCounter > 0)
		{
			...
			UE_LOGF(LogAudioMixerAudioUnit, Display, "Resuming Audio");
			bSuspended = false;
		}
	}
	...
	void FMixerPlatformAudioUnit::SuspendContext()
	{
		...
		if (SuspendCounter == 0)
		{
			...
			UE_LOGF(LogAudioMixerAudioUnit, Display, "Suspending Audio");
			bSuspended = true;
		}
	}
	...
	{
		UE_LOGF(LogAudioMixerAudioUnit, Log, "%ls", InLogOutput);
		if (bTeardown)
		{
			TeardownHardware();
		}
	...
	void FMixerPlatformAudioUnit::GrowCircularBufferIfNeeded(const int32 InNumSamplesPerRenderCallback, const int32 InNumSamplesPerDeviceCallback)
	{
		...
		if (CircularOutputBuffer.GetCapacity() < MaxCircularBufferCapacity)
		{
			...
			UE_LOGF(LogAudioMixerAudioUnit, Display, "Growing iOS circular buffer to %i bytes.", MaxCircularBufferCapacity);
		}
	}
