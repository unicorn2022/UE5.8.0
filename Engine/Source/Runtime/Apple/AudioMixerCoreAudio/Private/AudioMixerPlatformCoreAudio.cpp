...
namespace Audio
{
	...
	bool FMixerPlatformCoreAudio::InitializeHardware()
	{
		...
		if(Status != 0)
		{
			UE_LOGF(LogAudioMixerAudioUnit, Warning, "Error querying Audio Device ID: %i", Status);
		}
		...
		if(Status == 0)
		{
			UE_LOGF(LogAudioMixerAudioUnit, Display, "Sample Rate: %f", GraphSampleRate);
		}
		else
		{
			UE_LOGF(LogAudioMixerAudioUnit, Warning, "Error querying Sample Rate: %i", Status);
		}
