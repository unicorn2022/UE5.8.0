...
IMPLEMENT_MODULE(FDefaultModuleImpl, MacMenu);
...
public:
	...
	virtual void RegisterCommands() override
	{
		...
		UI_COMMAND( Close, "Close", "Close", EUserInterfaceActionType::None, FInputChord( FInputChord() ) )
		...
		//Step 3: assign the widgets and setup the menu state.
		{
			...
			for (TMap<FMacMenu*, TSharedPtr<TArray<FMacMenuItemState>>>::TIterator It(GCachedMenuState); It; ++It)
			{
				...
				if (!Menu.MultiBox.IsValid())
				{
					...
					{
						Widget = *Value;
						if (Widget->GetType() == FName("SMultiBoxWidget"))
						{
						...
						else
						{
							...
							UE_LOGF(LogMac, Warning, "Unsupported type of menu widget in FSlateMacMenu::UpdateCachedState(): %ls, %ls, %ls",
...
{
	if (Content->GetType() == FName("STextBlock"))
	{
		return Content;
	}
