...
public:
	...
	void EndDrawingViewport(FMetalViewport* Viewport, const FRHIPresentArgs& InPresentArgs);
	...
	*/
	...
	/** Update the event to capture all GPU work so far enqueued by this encoder. */
	void SignalSharedEvent(MTLSharedEventPtr Event, uint32_t SignalCount);
	
	/** Prevent further GPU work until the event is reached. */
	void WaitForSharedEvent(MTLSharedEventPtr Event, uint32_t SignalCount);
