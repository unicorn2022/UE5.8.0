	...
	, TotalTextureSize  (0)
{
	...
	if (bIsMSAARequired)
	{
        ...
        UE_LOGF(LogMetal, Verbose, "Creating MSAA %d x %d %ls surface", CreateDesc.Extent.X, CreateDesc.Extent.Y, EnumHasAnyFlags(CreateDesc.Flags, TexCreate_RenderTargetable) ? TEXT("Color") : TEXT("Depth"));
        
		if (MSAATexture.get() == nullptr)
		{
            UE_LOGF(LogMetal, Fatal, "Failed to create MSAA texture");
		}
		...
		OnDeleteFunction(MoveTemp(InOnDeleteFunction))
{
	...
	if (IsMetalBindlessEnabled())
	{
		...
		BindlessHandle = BindlessDescriptorManager->AllocateDescriptor(ERHIDescriptorType::TextureSRV, InTexture.get());
	}
#endif
}
...
uint32 FMetalSurface::GetMemorySize()
{
	...
	{
		uint32 NumFaces = (GetDesc().IsTextureCube() ? 6 : 1);
		TotalSize += GetMipSize(MipIndex, NULL, false) * GetDesc().ArraySize * NumFaces;
	}
...
MTLTexturePtr FMetalSurface::GetDrawableTexture()
{
	...
	{
		check(Viewport);
		Texture = NS::RetainPtr(Viewport->GetDrawableTexture(EMetalViewportAccessRHI, this));
...
{
	UE_LOGF(LogMetal, Fatal, "RHIAsyncCreateTexture2D is not supported");
...
void FMetalRHICommandContext::RHICopyTexture(FRHITexture* SourceTextureRHI, FRHITexture* DestTextureRHI, const FRHICopyTextureInfo& CopyInfo)
{
    ...
    {
        UE_LOGF(LogMetal, Error, "RHICopyTexture Source (UnrealEngine %d: MTL %d) <-> Destination (UnrealEngine %d: MTL %d) texture format mismatch", (uint32)SourceTextureRHI->GetFormat(), (uint32)MetalSrcTexture->Texture->pixelFormat(), (uint32)DestTextureRHI->GetFormat(), (uint32)MetalDestTexture->Texture->pixelFormat());
    }
}
