...
FVertexShaderRHIRef FMetalDynamicRHI::RHICreateVertexShader(const FRHICreateShaderDesc& CreateShaderDesc)
{
    MTL_SCOPED_AUTORELEASE_POOL;
    
	FMetalVertexShader* Shader = new FMetalVertexShader(*Device, CreateShaderDesc);
	return Shader;
}

FPixelShaderRHIRef FMetalDynamicRHI::RHICreatePixelShader(const FRHICreateShaderDesc& CreateShaderDesc)
{
    MTL_SCOPED_AUTORELEASE_POOL;
    
	FMetalPixelShader* Shader = new FMetalPixelShader(*Device, CreateShaderDesc);
	return Shader;
}

FGeometryShaderRHIRef FMetalDynamicRHI::RHICreateGeometryShader(const FRHICreateShaderDesc& CreateShaderDesc)
{
    MTL_SCOPED_AUTORELEASE_POOL;
#if PLATFORM_SUPPORTS_GEOMETRY_SHADERS
    FMetalGeometryShader* Shader = new FMetalGeometryShader(*Device, CreateShaderDesc);
    return Shader;
#else
	return nullptr;
#endif
...
FComputeShaderRHIRef FMetalDynamicRHI::RHICreateComputeShader(const FRHICreateShaderDesc& CreateShaderDesc)
{
    MTL_SCOPED_AUTORELEASE_POOL;
    return new FMetalComputeShader(*Device, CreateShaderDesc, MTLLibraryPtr());
}

#if PLATFORM_SUPPORTS_MESH_SHADERS
FMeshShaderRHIRef FMetalDynamicRHI::RHICreateMeshShader(const FRHICreateShaderDesc& CreateShaderDesc)
{
    MTL_SCOPED_AUTORELEASE_POOL;

    return new FMetalMeshShader(*Device, CreateShaderDesc);
}

FAmplificationShaderRHIRef FMetalDynamicRHI::RHICreateAmplificationShader(const FRHICreateShaderDesc& CreateShaderDesc)
{
    MTL_SCOPED_AUTORELEASE_POOL;

    return new FMetalAmplificationShader(*Device, CreateShaderDesc);
}
#endif

#if METAL_RHI_RAYTRACING
FRayTracingShaderRHIRef FMetalDynamicRHI::RHICreateRayTracingShader(const FRHICreateShaderDesc& CreateShaderDesc, EShaderFrequency ShaderFrequency)
{
	MTL_SCOPED_AUTORELEASE_POOL;
	
	return new FMetalRayShader(*Device, CreateShaderDesc, ShaderFrequency);
}
...
{
	FString METAL_MAP_EXTENSION(".metalmap");
    ...
    auto SerializeShaderCode = [](FMetalShaderLibrary::FShaderCodeArrayType& Array, FArchive& Ar)
    {
	   ...
	   {
		   UE_LOGF(LogMetal, Fatal, "mmapped array must be serialized via FStaticMemoryReader");
	   }
#endif
	};
	...
	{
		UE_LOGF(LogMetal, Display, "mmapping %ls, %d bytes", *BinaryShaderFile, MemOwner->MappedCacheFile->GetFileSize());
	...
	if(BinaryShaderAr == nullptr)
	{
		...
		{
			UE_LOGF(LogMetal, Display, "emulating mmapping %ls, %d bytes!", *BinaryShaderFile, FileData.Num());
        ...
        *BinaryShaderAr << SerializedShaders;
        ...
        if (Header.Format == ShaderFormatName.GetPlainNameString())
        {
            ...
            for (uint32 i = 0; i < Header.NumLibraries; i++)
			{
				...
				TUniquePtr<FMetalShaderLibrary::FShaderLibDataOwner> Data = MakeUnique<FMetalShaderLibrary::FShaderLibDataOwner>();
#if USE_MMAPPED_SHADERARCHIVE
				MapShaderLibraryFile(MetalLibraryFilePath, *Data);
#endif				

				TUniquePtr<FMetalShaderLibrary::FLazyMetalLib> LazyLibrary = MakeUnique<FMetalShaderLibrary::FLazyMetalLib>(MetalLibraryFilePath, MoveTemp(Data));
		...
		{
		    UE_LOGF(LogMetal, Display, "Unknown shader format for %ls!", *LibName);
		}
    }
    else
    {
        UE_LOGF(LogMetal, Display, "No .metalmap file found for %ls!", *LibName);
    }
