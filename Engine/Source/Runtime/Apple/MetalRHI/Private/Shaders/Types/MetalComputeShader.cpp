...
FMetalComputeShader::FMetalComputeShader(FMetalDevice& Device, const FRHICreateShaderDesc& CreateShaderDesc, MTLLibraryPtr InLibrary)
...
void FMetalComputeShader::ReleaseMetalObjects()
{
	Pipeline.Reset();
	TMetalBaseShader::ReleaseMetalObjects();
}
...
FMetalShaderPipelinePtr FMetalComputeShader::GetPipeline()
{
	...
	if (!Pipeline)
	{
		...
		if (Kernel.get() == nullptr)
		{
			...
			UE_LOGF(LogRHI, Fatal, "Failed to create compute kernel: %ls", *NSStringToFString(Error->description()));
		}
