...
public:
	...
	{
	public:
		FLazyMetalLib(FString FilePath, TUniquePtr<FShaderLibDataOwner> Data);
		~FLazyMetalLib();
		
		FLazyMetalLib(FLazyMetalLib&& Other) = delete;
		FLazyMetalLib(FLazyMetalLib const&) = delete;
		FLazyMetalLib& operator=(FLazyMetalLib const&) = delete;
		
		MTLLibraryPtr EnsureLoaded(FMetalDevice& MetalDevice);

	private:
		TUniquePtr<FShaderLibDataOwner> Data;
		FString FilePath;
		UE::FMutex LibraryMutex;
		std::atomic<MTL::Library*> Library = nullptr;
	};
