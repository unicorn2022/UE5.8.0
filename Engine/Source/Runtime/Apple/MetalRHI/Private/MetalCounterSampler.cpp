...
MTL::CounterSampleBuffer* FMetalCounterSampler::SwapOrAllocateBuffer(uint32_t SampleSize, uint32_t& OutOffset)
{
	...
	if(Offset > Size || !SampleBuffer)
	{
		...
		else
		{
			...
			if (!bLoggedWarning)
			{
				UE_LOGF(LogMetal, Warning, "We have run out of Metal counter samplers which will negatively affect the GPU stats, you can try disabling Blit counter samplers with rhi.Metal.SampleBlitEncoderTimings=0");
				bLoggedWarning = true;
			}
		}
