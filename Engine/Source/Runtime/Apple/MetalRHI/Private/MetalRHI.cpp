...
static void ValidateTargetedRHIFeatureLevelExists(EShaderPlatform Platform)
{
	...
	{
		GConfig->GetBool(TEXT("/Script/IOSRuntimeSettings.IOSRuntimeSettings"), TEXT("bSupportsMetalMobileSM5"), bSupportsShaderPlatform, GEngineIni);
	}
	else if (Platform == SP_METAL_SM6_IOS)
	{
		GConfig->GetBool(TEXT("/Script/IOSRuntimeSettings.IOSRuntimeSettings"), TEXT("bSupportsMetalMobileSM6"), bSupportsShaderPlatform, GEngineIni);
	}
#endif
	...
	: Device(FMetalDevice::CreateDevice())
{
	...
	{
		// A M2+ Mac Apple GPU is required for SM6 Apple8, on iOS we need Apple9 or above
#if PLATFORM_MAC
		bSupportsSM6 = MTLDevice->supportsFamily(MTL::GPUFamilyApple8);
#else
		bSupportsSM6 = MTLDevice->supportsFamily(MTL::GPUFamilyApple9);
#endif
		bSupportsBindless = MTLDevice->supportsFamily(MTL::GPUFamilyApple7); 
	}
	
#if PLATFORM_IOS
	if (!MTLDevice->supportsFamily(MTL::GPUFamilyApple3))
	{
		static const FText UnsupportedGPUFamilyApple9 = NSLOCTEXT("MetalRHI", "UnsupportedA8FeatureSet", "This device does not support the Apple family 3 GPU features (Apple A9+), which is required as the minimum for this engine version.");
		UE_LOGF(LogMetal, Log, "%ls", *UnsupportedGPUFamilyApple9.ToString());
		FMessageDialog::Open(EAppMsgType::Ok, UnsupportedGPUFamilyApple9); 
		FPlatformMisc::RequestExit(true, TEXT("FMetalDynamicRHI::FMetalDynamicRHI.UnsupportedA8FeatureSet"));
	}
        
	GRHISupportsDrawIndirect = true;
	GRHISupportsPixelShaderUAVs = true;
	GRHISupportsRWTextureBuffers = MTLDevice->supportsFamily(MTL::GPUFamilyApple4);
	...
	bool bProjectSupportsMetalMobileSM5 = false;
	bool bProjectSupportsMetalMobileSM6 = false;
	bool bProjectSupportsMetal          = false;

	static const TCHAR* const IOSSettingsSection = TEXT("/Script/IOSRuntimeSettings.IOSRuntimeSettings");
	GConfig->GetBool(IOSSettingsSection, TEXT("bSupportsMetalMobileSM5"), bProjectSupportsMetalMobileSM5, GEngineIni);
	GConfig->GetBool(IOSSettingsSection, TEXT("bSupportsMetalMobileSM6"), bProjectSupportsMetalMobileSM6, GEngineIni);
	GConfig->GetBool(IOSSettingsSection, TEXT("bSupportsMetal"),          bProjectSupportsMetal,          GEngineIni);

	const bool bRequestedSM6 = (RequestedFeatureLevel == ERHIFeatureLevel::SM6) || (!bRequestedFeatureLevel && FParse::Param(FCommandLine::Get(), TEXT("metalmobilesm6")));
	const bool bRequestedSM5 = (RequestedFeatureLevel == ERHIFeatureLevel::SM5) || (!bRequestedFeatureLevel && FParse::Param(FCommandLine::Get(), TEXT("metalmobilesm5")));
	const bool bForceES3_1   = FParse::Param(FCommandLine::Get(), TEXT("es31"));

	// Resolve the concrete shader platform for each feature level on this compile target.
	#if PLATFORM_TVOS
		constexpr EShaderPlatform SM6Platform  = SP_NumPlatforms;       // not supported on tvOS
		constexpr EShaderPlatform SM5Platform  = SP_METAL_SM5_TVOS;
		constexpr EShaderPlatform ES31Platform = SP_METAL_ES3_1_TVOS;
	#elif WITH_IOS_SIMULATOR
		constexpr EShaderPlatform SM6Platform  = SP_NumPlatforms;       // not supported on simulator
		constexpr EShaderPlatform SM5Platform  = SP_NumPlatforms;       // not supported on simulator
		constexpr EShaderPlatform ES31Platform = SP_METAL_SIM;
	#else
		constexpr EShaderPlatform SM6Platform  = SP_METAL_SM6_IOS;
		constexpr EShaderPlatform SM5Platform  = SP_METAL_SM5_IOS;
		constexpr EShaderPlatform ES31Platform = SP_METAL_ES3_1_IOS;
	#endif

	// Select the highest supported feature level, in priority order.
	if (SM6Platform != SP_NumPlatforms && bProjectSupportsMetalMobileSM6 && bRequestedSM6 && !bForceES3_1 && bSupportsSM6)
	{
		ValidateTargetedRHIFeatureLevelExists(SM6Platform);
		GMaxRHIShaderPlatform = SM6Platform;
		GMaxRHIFeatureLevel   = ERHIFeatureLevel::SM6;
	}
	else if (SM5Platform != SP_NumPlatforms && bProjectSupportsMetalMobileSM5 && bRequestedSM5 && !bForceES3_1
		&& (UE::RHICore::GetBindlessConfigurationOnStartup(SM5Platform) == ERHIBindlessConfiguration::Disabled || bSupportsBindless))
	{
		ValidateTargetedRHIFeatureLevelExists(SM5Platform);
		GMaxRHIShaderPlatform = SM5Platform;
		GMaxRHIFeatureLevel   = ERHIFeatureLevel::SM5;
	}
	else if (bProjectSupportsMetal
		&& (UE::RHICore::GetBindlessConfigurationOnStartup(ES31Platform) == ERHIBindlessConfiguration::Disabled || bSupportsBindless))
	{
		ValidateTargetedRHIFeatureLevelExists(ES31Platform);
		GMaxRHIShaderPlatform = ES31Platform;
		GMaxRHIFeatureLevel   = ERHIFeatureLevel::ES3_1;
	}
	else
	{
		static const FText NoCompatibleMetalFormats = NSLOCTEXT("MetalRHI", "NoCompatibleMetalFormats", "This project has been built with project settings that are unsupported by this device");
		UE_LOGF(LogMetal, Log, "%ls", *NoCompatibleMetalFormats.ToString());
		FMessageDialog::Open(EAppMsgType::Ok, NoCompatibleMetalFormats);
		FPlatformMisc::RequestExit(true, TEXT("FMetalDynamicRHI::FMetalDynamicRHI.NoCompatibleMetalFormats"));
	}
	...
	check(GPUs.Num());
	FMacPlatformMisc::FGPUDescriptor const& GPUDesc = GPUs[0];
	...
	// At this point this should be valid because we checked it while creating the FMetalDevice
	check(MTLDevice->supportsFamily(MTL::GPUFamilyApple7));
	...
	bool const bRequestedSM5 = RequestedFeatureLevel == ERHIFeatureLevel::SM5 ||
                               (!bRequestedFeatureLevel && FParse::Param(FCommandLine::Get(),TEXT("metalsm5")));
                                
	if(bRequestedSM6 && !bSupportsSM6)
	{
		if(!GRHIAdapterName.Contains("M1"))
		{
			UE_LOGF(LogMetal, Warning, "SM6 is enabled but is not supported on this system, falling back to SM5");
		}
		else
		{
			UE_LOGF(LogMetal, Warning, "To use SM6 on this system, please ensure you are running Mac OS 15. Falling back to SM5");
		}
	}
	...
	GRHIGlobals.bSupportsBindless = UE::RHICore::GetBindlessConfigurationOnStartup(GMaxRHIShaderPlatform) != ERHIBindlessConfiguration::Disabled;
	GRHIGlobals.MaxSimultaneousUAVs = FMath::Min(METAL_MAX_TEXTURES, METAL_MAX_BUFFERS);
	GRHIGlobals.MaxTextureSamplers = GRHIGlobals.bSupportsBindless ? MTLDevice->maxArgumentBufferSamplerCount() : 16;
	...
	if (GRHIGlobals.bSupportsBindless)
	{
		...
		Device->ForceHeapSupport();
	}
	
	// Must call post init after we have set flags on device
	Device->PostInit();
	...
	GRHIGlobals.ParallelRenderPasses.Supported = bSupportsParallel;
	GRHIGlobals.ParallelRenderPasses.ParentWaitForChildTranslate = true;
	GRHIGlobals.ParallelRenderPasses.ChildWaitForParentBegin = true;
	...
	GSupportsShaderFramebufferFetch = !PLATFORM_MAC && GMaxRHIShaderPlatform != SP_METAL_SM5_IOS && GMaxRHIShaderPlatform != SP_METAL_SM6_IOS && GMaxRHIShaderPlatform != SP_METAL_SM5_TVOS;
	...
	GRHIMaxDispatchThreadGroupsPerDimension.X = MAX_int32;
	GRHIMaxDispatchThreadGroupsPerDimension.Y = MAX_int32;
	GRHIMaxDispatchThreadGroupsPerDimension.Z = MAX_int32;
	...
	if (MTLDevice->supportsFamily(MTL::GPUFamilyApple2) && !MTLDevice->supportsFamily(MTL::GPUFamilyApple3))
	{
		UE_LOGF(LogMetal, Display, "Detected GPUFamilyApple2 device.");
		EnumRemoveFlags(GPixelFormats[PF_A2B10G10R10].Capabilities, EPixelFormatCapabilities::UAV);
		EnumRemoveFlags(GPixelFormats[PF_FloatRGB].Capabilities, EPixelFormatCapabilities::UAV);
		EnumRemoveFlags(GPixelFormats[PF_FloatR11G11B10].Capabilities, EPixelFormatCapabilities::UAV);
		EnumRemoveFlags(GPixelFormats[PF_R9G9B9EXP5].Capabilities, EPixelFormatCapabilities::UAV);
	}
	...
	if(GPUDesc.GPUVendorId == GRHIVendorId)
	{
		UE_LOGF(LogMetal, Display,  "      Vendor ID: %d", GPUDesc.GPUVendorId);
		UE_LOGF(LogMetal, Display,  "      Device ID: %d", GPUDesc.GPUDeviceId);
		UE_LOGF(LogMetal, Display,  "      VRAM (MB): %d", GPUDesc.GPUMemoryMB);
	}
	else
	{
		UE_LOGF(LogMetal, Warning,  "GPU descriptor (%ls) from IORegistry failed to match Metal (%ls)", *FString(GPUDesc.GPUName), *GRHIAdapterName);
	}
#endif
	...
	if(Device->SupportsFeature(EMetalFeaturesRayTracing))
	{
		if (!FParse::Param(FCommandLine::Get(), TEXT("noraytracing")))
		{
			...
			GRHISupportsRayTracingShaders = RHISupportsRayTracingShaders(GMaxRHIShaderPlatform) && UE::RHICore::AreRayTracingShadersEnabled();
...
void FMetalRHICommandContext::RHIBeginBreadcrumbGPU(FRHIBreadcrumbNode* Breadcrumb)
{
	...
	if (ShouldEmitBreadcrumbs())
	{
		MTL_SCOPED_AUTORELEASE_POOL;
		{
			...
			CurrentEncoder.PushDebugGroup(NS::String::string(TCHAR_TO_UTF8(Name), NS::UTF8StringEncoding));
		}
	}
#endif
...
{
#if PLATFORM_MAC
	CGDisplayModeRef DisplayMode = FMacPlatformMiscEx::GetSupportedDisplayMode(kCGDirectMainDisplay, Width, Height);
	if (DisplayMode)
	{
...
#else
	UE_LOGF(LogMetal, Warning,  "RHIGetSupportedResolution unimplemented!");
#endif
}
...
bool FMetalDynamicRHI::RHIGetAvailableResolutions(FScreenResolutionArray& Resolutions, bool bIgnoreRefreshRate)
{
	...
	return true;
#else
	UE_LOGF(LogMetal, Warning,  "RHIGetAvailableResolutions unimplemented!");
	return false;
#endif
}
...
MTL::Device* FMetalDynamicRHI::RHIGetDevice()
{
...
{
	// There is no "default"/"immediate" context in Metal now, just return a regular (i.e. parallel) RHI context.
	// Forward this call through the GDynamicRHI pointer so the validation layer can still do its per-context setup.
	return static_cast<IRHICommandContext*>(&GDynamicRHI->RHIGetCommandContext(ERHIPipeline::Graphics)->GetLowestLevelContext());
}
...
IRHIComputeContext* FMetalDynamicRHI::RHIGetCommandContext(ERHIPipeline Pipeline)
{
	...
	if (!Context)
	{
		Context = new FMetalRHICommandContext(*Device);
	}
...
IRHICommandContext* FMetalDynamicRHI::RHIGetParallelCommandContext(FRHIParallelRenderPassInfo const& ParallelRenderPass)
{
...
void FMetalDynamicRHI::RHIWriteResourceGPUAddresses(FRHICommandListBase& RHICmdList, uint32 GPUIndex, TConstArrayView<FRHIResource*> Resources, uint64* AddressesPtr)
{
	RHICmdList.EnqueueLambda([AddressesPtr, Resources](FRHICommandListBase&)
		{
			...
			for (int32 ResourceIndex = 0; ResourceIndex < Resources.Num(); ++ResourceIndex)
			{
				...
				{
					const FMetalRHIBuffer* Buffer = ResourceCast(static_cast<const FRHIBuffer*>(Resource));
					AddressesPtr[ResourceIndex] = Buffer->GetCurrentBufferOrNull()->GetGPUAddress();
				}
				...
				{
					const FMetalRayTracingGeometry* RayTracingGeometry = ResourceCast(static_cast<const FRHIRayTracingGeometry*>(Resource));
					AddressesPtr[ResourceIndex] = RayTracingGeometry->GetGPUAddress();
				}
#endif
				else
				{
