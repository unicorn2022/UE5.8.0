...
int32 GBindlessResourceDescriptorHeapSize = 1 << 23; // 8MB
...
int32 GBindlessSamplerDescriptorHeapSize = 1 << 16; // 64k
...
void FMetalBindlessDescriptorManager::Init()
{
	...
	GRHIGlobals.DescriptorTypeGroups.Emplace(StandardResources.GetTypeMask());
	GRHIGlobals.DescriptorTypeGroups.Emplace(SamplerResources.GetTypeMask());
...
void FMetalBindlessDescriptorManager::UpdateDescriptor(FRHICommandListBase& RHICmdList, FRHIDescriptorHandle DescriptorHandle, IRDescriptorTableEntry Descriptor, EMetalDescriptorUpdateType InUpdateType)
{
	const EMetalDescriptorUpdateType UpdateType = !GIsRHIInitialized ? EMetalDescriptorUpdateType::Immediate : InUpdateType;
...
{
	IRDescriptorTableEntry Descriptor = CreateDescriptor(Texture);
	UpdateDescriptor(RHICmdList, DescriptorHandle, Descriptor, UpdateType);
}

void FMetalBindlessDescriptorManager::UpdateDescriptor(FRHICommandListBase& RHICmdList, FRHIDescriptorHandle DescriptorHandle, FMetalBuffer* Buffer, EMetalDescriptorUpdateType UpdateType)
{
	IRDescriptorTableEntry Descriptor = CreateDescriptor(Buffer);
	UpdateDescriptor(RHICmdList, DescriptorHandle, Descriptor, UpdateType);
}
