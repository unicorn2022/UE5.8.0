...
static_assert(Offset_BlendStateEnd < 64 && Offset_End < 128, "Offset_BlendStateEnd must be <= 64 && Offset_End < 128");
...
struct FMetalGraphicsPipelineKey
{
	...
	FShaderHash VertexFunction;
#if PLATFORM_SUPPORTS_MESH_SHADERS
	FShaderHash MeshFunction;
	FShaderHash AmplificationFunction;
#endif
	FShaderHash PixelFunction;

	template<typename Type>
	inline void SetHashValue(uint32 Offset, uint32 NumBits, Type Value)
	{
		if (Offset < Offset_BlendStateEnd)
		{
...
static NS::URL* PipelineCacheSaveLocation()
{
	...
	if (!dir)
	{
		UE_LOGF(LogMetal, Error, "Metal Pipeline Cache: Unable to find Documents folder");
	}
...
static void InitializeMetalPipelineCache(FMetalDevice& Device)
{
	...
	if (strCacheMode.Compare(TEXT("recreate"), ESearchCase::IgnoreCase) == 0)
	{
        ...
        UE_LOGF(LogMetal, Log, "Metal Pipeline Cache: recreate PSO cache");
	}
	else if (strCacheMode.Compare(TEXT("append"), ESearchCase::IgnoreCase) == 0)
	{
		GPSOCacheMode = CacheMode::Append;
        UE_LOGF(LogMetal, Log, "Metal Pipeline Cache: append to PSO cache");
	}
	else if (strCacheMode.Compare(TEXT("use"), ESearchCase::IgnoreCase) == 0)
	{
		GPSOCacheMode = CacheMode::Use;
        UE_LOGF(LogMetal, Log, "Metal Pipeline Cache: use PSO cache");
	}
	else
	{
		GPSOCacheMode = CacheMode::Ignore;
        UE_LOGF(LogMetal, Log, "Metal Pipeline Cache: ignore PSO cache");
	}
		
	if (GPSOCacheMode != CacheMode::Ignore)
	{
		NS::URL* url = PipelineCacheSaveLocation();
        UE_LOGF(LogMetal, Log, "Metal Pipeline Cache: pso cache save location will be: %ls", *FString(url->fileSystemRepresentation()));
		...
		if (err)
		{
			UE_LOGF(LogMetal, Error, "Error adding Pipeline Functions to Binary Archive: %ls", *FString(url->fileSystemRepresentation()));
		}
        archDesc->release();
	}
}
...
static void RelatePipelineStateToCache(const MTL::RenderPipelineDescriptor* PipelineDesc, NS::UInteger * outPipelineOpts)
{
	if (GPSOBinaryArchive && (GPSOCacheMode != CacheMode::Ignore))
	{
		if (GPSOCacheMode == CacheMode::Recreate || GPSOCacheMode == CacheMode::Append)
		{
			...
			if (err)
			{
				UE_LOGF(LogMetal, Warning, "Metal Pipeline Cache: Error adding Pipeline Functions to Binary Archive: %ls", *NSStringToFString(err->localizedDescription()));
			}
            else if(bAddedBinaryPSO)
            {
                ...
                {
                    UE_LOGF(LogMetal, Log, "Metal Pipeline Cache: Harvested PSO count: %d", GPSOHarvestCount);
                }
            }
		}
	}
...
{
	UE_LOGF(LogMetal, Log, "Metal Pipeline Cache: requesting PSO save...");
	{
		if (GPSOBinaryArchive && (GPSOCacheMode == CacheMode::Recreate || GPSOCacheMode == CacheMode::Append))
		{
			NS::URL* url = PipelineCacheSaveLocation();
			UE_LOGF(LogMetal, Log, "Metal Pipeline Cache: Serialize harvested PSOs to: %ls", *FString(url->fileSystemRepresentation()));
            UE_LOGF(LogMetal, Log, "Metal Pipeline Cache: Serialized PSO Count: %d", GPSOHarvestCount);
            
			NS::Error* err = nullptr;
			GPSOBinaryArchive->serializeToURL(url, &err);
			if (err)
			{
				UE_LOGF(LogMetal, Error, "Metal Pipeline Cache: Error Serializing binary archive: %ls", *NSStringToFString(err->localizedDescription()));
			}
		}
	}
}
...
static FMetalShaderPipelinePtr CreateMTLRenderPipeline(FMetalDevice& Device, bool const bSync, FMetalGraphicsPipelineKey const& Key, const FGraphicsPipelineStateInitializer& Init, FMetalGraphicsPipelineState const* State)
{
	...
	)
    {
		...
		UE_CLOGF((Pipeline->RenderPipelineState.get() == nullptr), LogMetal, Error, "Failed to generate a pipeline state object: %ls", *NSStringToFString(Error->description()));
		UE_CLOGF((Pipeline->RenderPipelineState.get() == nullptr), LogMetal, Error, "Vertex shader: %ls", *NSStringToFString(VertexShader->GetSourceCode()));
		UE_CLOGF((Pipeline->RenderPipelineState.get() == nullptr), LogMetal, Error, "Pixel shader: %ls", PixelShader ? *NSStringToFString(PixelShader->GetSourceCode()) : TEXT("NULL"));
		UE_CLOGF((Pipeline->RenderPipelineState.get() == nullptr), LogMetal, Error, "Descriptor: %ls", *NSStringToFString(RenderPipelineDesc->description()));
		UE_CLOGF((Pipeline->RenderPipelineState.get() == nullptr), LogMetal, Error, "Vertex Descriptor: %ls", *NSStringToFString(VertexDecl->Layout.VertexDesc->description()));
		UE_CLOGF((Pipeline->RenderPipelineState.get() == nullptr), LogMetal, Error, "Masked Vertex Descriptor: %ls", *NSStringToFString(MaskedVertexDesc->description()));
		UE_CLOGF((Pipeline->RenderPipelineState.get() == nullptr), LogMetal, Error, "Failed to generate a render pipeline state object:\n\n %ls\n\n", *NSStringToFString(Error->localizedDescription()));
...
#if METAL_DEBUG_OPTIONS
		// Reset the pipeline descriptor to release MTLFunction/MTLLibrary references it retains.
		Pipeline->RenderDesc.reset();
        ...
        if (GFrameCounter > 3)
        {
            UE_LOGF(LogMetal, Verbose, "Created a hitchy pipeline state for hash %llx %llx %llx", (uint64)Key.RenderPipelineHash.RasterBits, (uint64)(Key.RenderPipelineHash.TargetBits), (uint64)Key.VertexDescriptorHash.VertexDescHash);
        }
#endif
    }
    ...
    else if (vertexFunction && geometryFunction)
	{
        ...
        UE_CLOGF((Pipeline->RenderPipelineState.get() == nullptr), LogMetal, Error, "Failed to generate a pipeline state object: %ls", *NSStringToFString(Error->description()));
        UE_CLOGF((Pipeline->RenderPipelineState.get() == nullptr), LogMetal, Error, "Mesh shader: %ls", *NSStringToFString(GeometryShader->GetSourceCode()));
        UE_CLOGF((Pipeline->RenderPipelineState.get() == nullptr), LogMetal, Error, "Object shader: %ls", *NSStringToFString(VertexShader->GetSourceCode()));
        UE_CLOGF((Pipeline->RenderPipelineState.get() == nullptr), LogMetal, Error, "Pixel shader: %ls", PixelShader ? *NSStringToFString(PixelShader->GetSourceCode()) : TEXT("NULL"));
        UE_CLOGF((Pipeline->RenderPipelineState.get() == nullptr), LogMetal, Error, "Descriptor: %ls", *NSStringToFString(MeshPipelineDesc->description()));
        UE_CLOGF((Pipeline->RenderPipelineState.get() == nullptr), LogMetal, Error, "Failed to generate a mesh render pipeline state object:\n\n %ls\n\n", *NSStringToFString(Error->localizedDescription()));
    ...
    else if (meshFunction)
    {
        ...
        UE_CLOGF((Pipeline->RenderPipelineState.get() == nullptr), LogMetal, Error, "Failed to generate a pipeline state object: %ls", *NSStringToFString(Error->description()));
        UE_CLOGF((Pipeline->RenderPipelineState.get() == nullptr), LogMetal, Error, "Mesh shader: %ls", *NSStringToFString(MeshShader->GetSourceCode()));
        UE_CLOGF((Pipeline->RenderPipelineState.get() == nullptr), LogMetal, Error, "Object shader: %ls", AmplificationShader ? *NSStringToFString(AmplificationShader->GetSourceCode()) : TEXT("NULL"));
        UE_CLOGF((Pipeline->RenderPipelineState.get() == nullptr), LogMetal, Error, "Pixel shader: %ls", PixelShader ? *NSStringToFString(PixelShader->GetSourceCode()) : TEXT("NULL"));
        UE_CLOGF((Pipeline->RenderPipelineState.get() == nullptr), LogMetal, Error, "Descriptor: %ls", *NSStringToFString(MeshPipelineDesc->description()));
        UE_CLOGF((Pipeline->RenderPipelineState.get() == nullptr), LogMetal, Error, "Failed to generate a mesh render pipeline state object:\n\n %ls\n\n", *NSStringToFString(Error->localizedDescription()));
...
#if PLATFORM_IOS
	ApplicationWillDeactivateDelegateHandle = FCoreDelegates::ApplicationWillDeactivateDelegate.AddStatic(&FShaderPipelineCache::PauseBatching);
	ApplicationHasReactivatedDelegateHandle = FCoreDelegates::ApplicationHasReactivatedDelegate.AddStatic(&FShaderPipelineCache::ResumeBatching);
...
{
	FCoreDelegates::ApplicationWillDeactivateDelegate.Remove(ApplicationWillDeactivateDelegateHandle);
	FCoreDelegates::ApplicationHasReactivatedDelegate.Remove(ApplicationHasReactivatedDelegateHandle);
