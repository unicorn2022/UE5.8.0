...
void FMetalStateCache::SetScissorRect(bool const bEnable, MTL::ScissorRect const& Rect)
{
	...
	{
		Scissor[0].width = FMath::Max((Scissor[0].x + Scissor[0].width <= FMath::RoundToInt32(FrameBufferSize.width)) ? Scissor[0].width : FMath::RoundToInt32(FrameBufferSize.width) - Scissor[0].x, (NS::UInteger)0u);
		Scissor[0].height = FMath::Max((Scissor[0].y + Scissor[0].height <= FMath::RoundToInt32(FrameBufferSize.height)) ? Scissor[0].height : FMath::RoundToInt32(FrameBufferSize.height) - Scissor[0].y, (NS::UInteger)0u);
	}
	...
	if (NeedsToSetRenderTarget(InRenderTargets))
	{
		...
		for (uint32 RenderTargetIndex = 0; RenderTargetIndex < MaxSimultaneousRenderTargets; RenderTargetIndex++)
		{
			...
			if (RenderTargetIndex < NumColorRenderTargets && RenderPassInfo.ColorRenderTargets[RenderTargetIndex].RenderTarget != nullptr)
			{
				...
				{
#if PLATFORM_MAC
					UE_LOG(LogMetal, Warning, TEXT("Failed to acquire backbuffer drawable texture for color render target %u; skipping render-pass setup for this frame."), RenderTargetIndex);
#endif
		...
		if (RenderPassInfo.DepthStencilRenderTarget.DepthStencilTarget != nullptr)
		{
			...
			else if (SampleCount != DepthSampleCount)
            {
				...
				{
					UE_LOGF(LogMetal, Error, "If we support separate targets the high level should always give us matching counts");
					bLogged = true;
				}
            }
...
void FMetalStateCache::CacheHeapResidency(MTL::Heap* InHeap, EMetalShaderStages const Frequency)
{
	HeapsUsedByStage[Frequency].Add(InHeap);
}
...
void FMetalStateCache::IRMakeSRVResident(EMetalShaderStages const Frequency, FMetalShaderResourceView* SRV)
{
    ...
    switch (SRV->GetMetalType())
    {
        ...
        case FMetalResourceViewBase::EMetalType::AccelerationStructure:
        {
			...
			CacheOrSkipResourceResidencyUpdate(AccelerationStructure->GetHitContributionsBuffer()->GetMTLBuffer(), Frequency, true);
...
void FMetalStateCache::IRBindPackedUniforms(EMetalShaderStages const Frequency, int32 Index, uint8 const* Bytes, const uint32 Size, FMetalBufferPtr& Buffer)
{
	...
	check((uint32)Index < TopLevelABNumEntry);
...
void FMetalStateCache::SetShaderTexture(EMetalShaderStages const Frequency, MTL::Texture* Texture, NS::UInteger const Index, MTL::ResourceUsage const Usage)
{
	...
	UE_CLOGF(Texture->storageMode() == MTL::StorageModeMemoryless, LogMetal, Fatal, "FATAL: Attempting to bind a memoryless texture. Stage %u Index %llu Texture %ls", Frequency, Index, *NSStringToFString(Texture->description()));
#endif
...
template<EMetalShaderStages Frequency, MTL::FunctionType FunctionType>
void FMetalStateCache::FlushUsedResources(FMetalCommandEncoder* Encoder)
{
...
{ 	
	FlushUsedResources<Frequency, FunctionType>(Encoder);
	...
	FMetalStaticSamplers* StaticSamplersTable = Device.GetStaticSamplers();
	const uint64 StaticSamplersTableVA = StaticSamplersTable->GetGPUAddress();

	check(Shader->Bindings.RSNumCBVs < TopLevelABNumEntry);
	CBVTable[Frequency][Shader->Bindings.RSNumCBVs] = StaticSamplersTableVA;
	
    // Bind CBV Table
    Encoder->SetShaderBytes(FunctionType, (const uint8*)CBVTable[Frequency], sizeof(uint64) * (Shader->Bindings.RSNumCBVs+1), kIRArgumentBufferBindPoint);
}
                ...
                switch(Binding->type())
                {
                    case MTL::BindingTypeBuffer:
                    {
                        ...
                        if (Pipeline->ResourceMask[Frequency].BufferMask & (1 << Binding->index()))
                        {
                            ...
                            {
                                bOK = false;
                                UE_LOGF(LogMetal, Warning, "Unbound buffer at Metal index %u which will crash the driver: %ls", (uint32)Binding->index(), *NSStringToFString(Binding->description()));
                            }
                        }
                        break;
                    }
                    ...
                    case MTL::BindingTypeTexture:
                    {
                        ...
                        {
                            bOK = false;
                            UE_LOGF(LogMetal, Warning, "Unbound texture at Metal index %u which will crash the driver: %ls", (uint32)Binding->index(), *NSStringToFString(Binding->description()));
                        }
                        else if (ShaderTextures[Frequency].Textures[Binding->index()]->textureType() != TextureBinding->textureType())
                        {
                            bOK = false;
                            UE_LOGF(LogMetal, Warning, "Incorrect texture type bound at Metal index %u which will crash the driver: %ls\n%ls",
                    ...
                    case MTL::BindingTypeSampler:
                    {
                        ...
                        {
                            bOK = false;
                            UE_LOGF(LogMetal, Warning, "Unbound sampler at Metal index %u which will crash the driver: %ls", (uint32)Binding->index(), *NSStringToFString(Binding->description()));
                        }
                        break;
                    }
                    default:
                ...
                switch(Arg->type())
                {
                    case MTL::ArgumentTypeBuffer:
                    {
                        ...
                        if (NSStringToFString(Arg->name()) != TEXT("BufferSizes") && Arg->index() != SideTableIndex)
                        {
                            ...
                            {
                                bOK = false;
                                UE_LOGF(LogMetal, Warning, "Unbound buffer at Metal index %u which will crash the driver: %ls", (uint32)Arg->index(), *NSStringToFString(Arg->description()));
                            }
                        }
                        break;
                    }
                    ...
                    case MTL::ArgumentTypeTexture:
                    {
                        ...
                        {
                            bOK = false;
                            UE_LOGF(LogMetal, Warning, "Unbound texture at Metal index %u which will crash the driver: %ls", (uint32)Arg->index(), *NSStringToFString(Arg->description()));
                        }
                        else if (ShaderTextures[Frequency].Textures[Arg->index()]->textureType() != Arg->textureType())
                        {
                            bOK = false;
                            UE_LOGF(LogMetal, Warning, "Incorrect texture type bound at Metal index %u which will crash the driver: %ls\n%ls", (uint32)Arg->index(), *NSStringToFString(Arg->description()), *NSStringToFString(ShaderTextures[Frequency].Textures[Arg->index()]->description()));
                        }
                        break;
                    }
                    case MTL::ArgumentTypeSampler:
                    {
                        ...
                        {
                            bOK = false;
                            UE_LOGF(LogMetal, Warning, "Unbound sampler at Metal index %u which will crash the driver: %ls", (uint32)Arg->index(), *NSStringToFString(Arg->description()));
                        }
                        break;
                    }
                    default:
...
void FMetalStateCache::Validate()
{
    ...
    if (!bOK)
    {
        UE_LOGF(LogMetal, Error, "Metal Validation failures for vertex shader:\n%ls", Pipeline->VertexSource ? *NSStringToFString(Pipeline->VertexSource) : TEXT("nullptr"));
    }
    ...
    if (!bOK)
    {
        UE_LOGF(LogMetal, Error, "Metal Validation failures for fragment shader:\n%ls", Pipeline->FragmentSource ? *NSStringToFString(Pipeline->FragmentSource) : TEXT("nullptr"));
    }
#endif
}
