...
void* FMetalRHIBuffer::Lock(FRHICommandListBase& RHICmdList, EResourceLockMode InLockMode, uint32 Offset, uint32 InSize)
{
	...
	else
	{
		...
		if (Mode == MTL::StorageModePrivate)
		{
			...
			FRHICommandListImmediate& ImmediateCmdList = RHICmdList.GetAsImmediate();
			
			ImmediateCmdList.EnqueueLambda([this, InTransferBuffer=TransferBuffer](FRHICommandListBase& ExecutingCmdList)
			{
				FMetalRHICommandContext& Context = FMetalRHICommandContext::Get(ExecutingCmdList);
				Context.CopyFromBufferToBuffer(GetCurrentBuffer(), 0, InTransferBuffer, 0, GetCurrentBuffer()->GetLength());
			});
		...
		else if(Mode == MTL::StorageModeManaged)
		{
			...
			ImmediateCmdList.EnqueueLambda([this](FRHICommandListBase& ExecutingCmdList)
			{
				FMetalRHICommandContext& Context = FMetalRHICommandContext::Get(ExecutingCmdList);
