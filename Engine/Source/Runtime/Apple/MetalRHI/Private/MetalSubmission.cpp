...
FDynamicRHI::FRHIFinalizeContextsResult FMetalDynamicRHI::RHIFinalizeContexts(FRHIFinalizeContextsArgs&& Args)
{
	MTL_SCOPED_AUTORELEASE_POOL;
	
	FRHIFinalizeContextsResult Result;
	
	if (FMetalRHIUploadContext* UploadContext = static_cast<FMetalRHIUploadContext*>(Args.UploadContext))
	{
		...
		if (Commands.Num())
		{
			Result.UploadCommandList = new FMetalFinalizedCommands(MoveTemp(Commands));
		}
	}
	
	for (ERHIPipeline Pipeline : MakeFlagsRange(ERHIPipeline::All))
	{
		if (FMetalRHICommandContext* Context = static_cast<FMetalRHICommandContext*>(Args.Contexts[Pipeline]))
		{
			FMetalFinalizedCommands Commands;
			Context->Finalize(Commands);
			Context->ResetContext();

			MetalCommandContextPool.Push(Context);
		
			Result.CommandLists[Pipeline] = new FMetalFinalizedCommands(MoveTemp(Commands));
		}
	}
					...
					for (int32 Index = 0; Index < Payload->CommandBuffersToExecute.Num(); Index++)
					{
						...
						for (id<MTLFunctionLog> log : logs)
						{
							...
							if(FunctionLog)
							{
								...
								if(DebugLocation)
								{
									UE_LOGF(LogMetal, Warning, "Shader Validation Log:\n functionName: %ls\nencoder: %ls\nline: %lld\ncolumn: %lld", *NSStringToFString(DebugLocation->functionName()), *NSStringToFString(FunctionLog->encoderLabel()), DebugLocation->line(), DebugLocation->column());
								}
							}
						}
#endif
	...
	ForEachQueue([&](FMetalCommandQueue& CurrentQueue)
	{
		while (FMetalPayload* Payload = CurrentQueue.PendingInterrupt.Peek())
		{
			...
			{	
				...
				{
					FMetalCommandQueue::HandleMetalCommandBufferFailure(CompletedBuffer);
				}
			}
			...
			if(Payload->SignalCommandBuffer)
			{
				...
				{
					FMetalCommandQueue::HandleMetalCommandBufferFailure(SignalBuffer);
				}
			}
			...
			// Resolve breadcrumb timestamps into the event stream before signalling sync points.
