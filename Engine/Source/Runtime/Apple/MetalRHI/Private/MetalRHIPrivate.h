...
#ifndef METAL_USE_METAL_SHADER_CONVERTER
#define METAL_USE_METAL_SHADER_CONVERTER 0
#endif
...
#define NOT_SUPPORTED(Func) UE_LOGF(LogMetal, Fatal, "'%ls' is not supported", TEXT(Func));
...
template<typename TRHIType>
static FORCEINLINE const typename TMetalResourceTraits<TRHIType>::TConcreteType* ResourceCast(const TRHIType* Resource)
{
	return static_cast<const typename TMetalResourceTraits<TRHIType>::TConcreteType*>(Resource);
}
...
static FORCEINLINE const FMetalSurface* ResourceCast(const FRHITexture* Texture)
{
	unimplemented();
	return nullptr;
}
