...
static int32 GMetalUseEventForGPUFence = 1;
static FAutoConsoleVariableRef CVarMetalUseEventForGPUFence(
	TEXT("rhi.Metal.UseEventForGPUFence"),
	GMetalUseEventForGPUFence,
	TEXT("When enabled use an Event to signal GPUFence completion to save splitting command buffers default: on"));
...
FMetalGPUFence::FMetalGPUFence(FMetalDevice& InDevice, FName InName)
	: FRHIGPUFence(InName),
	  Device(&InDevice)
{
	if(GMetalUseEventForGPUFence)
	{
		SharedEvent = Device->CreateSharedEvent();
	}
}

FMetalGPUFence::~FMetalGPUFence()
{
	if(GMetalUseEventForGPUFence)
	{
		FMetalDynamicRHI::Get().DeferredDelete([Event=SharedEvent]{});
	}
}
...
bool FMetalGPUFence::Poll() const
{
	...
	if (SharedEvent && SharedEvent->signaledValue() > 0)
	{
		return true;
	}
...
void FMetalGPUFence::Wait(FRHICommandListImmediate& RHICmdList, FRHIGPUMask GPUMask) const
{
	if(!Poll())
	{
		...
		if (SharedEvent)
		{
			static const uint64_t FENCE_EVENT_WAIT_TIMEOUT = 1000000;
			bSuccess = SharedEvent->waitUntilSignaledValue(1, FENCE_EVENT_WAIT_TIMEOUT);
		}
		if(!bSuccess && SyncPoint)
		{
			SyncPoint->Wait();
		}
	}	
...
void FMetalDynamicRHI::RHIWriteGPUFence_TopOfPipe(FRHICommandListBase& RHICmdList, FRHIGPUFence* FenceRHI)
{
	...
	RHICmdList.EnqueueLambda([Fence, SyncPoint = Fence->SyncPoint](FRHICommandListBase& CmdList)
	{
		...
		if(Fence->SharedEvent)
		{
			Context.SignalSharedEvent(Fence->SharedEvent, 1);
		}
		else
		{
...
{
    MTL_SCOPED_AUTORELEASE_POOL;
	return new FMetalGPUFence(*Device, Name);
}
