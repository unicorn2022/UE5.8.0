...
FAutoConsoleVariableRef CVarQueryUseControlPrecisionBit(
	TEXT("rhi.Metal.QueryUseControlPrecisionBit"),
	GQueryUseControlPrecisionBit,
	TEXT("Whether to use VisibilityResultModeCounting (if supported) when doing a render query. (Default : 1 (on), 0 (off))\n"),
	ECVF_ReadOnly
);
...
{
	if (GIsMetalInitialized && Buffer && Pool)
	{
...
void FMetalRHIRenderQuery::Begin(FMetalRHICommandContext* Context)
{
	...
	switch (Type)
	{
		case RQT_Occlusion:
		{
			...
			if (Device.SupportsFeature(EMetalFeaturesCountingQueries) && (GQueryUseControlPrecisionBit > 0))
			{
