...
static bool GBackgroundHttpUseExistingNonResumableTasks = true;
static FAutoConsoleVariableRef CVarBackgroundHttpUseExistingNonResumableTasks(
	TEXT("ios.BackgroundHttp.bUseExistingNonResumableTasks"),
	GBackgroundHttpUseExistingNonResumableTasks,
	TEXT("Make getTasksWithCompletionHandler use existing non resumable tasks"),
	ECVF_Default
);

// This is now disabled as there is a known race condition wrt async cancellation
// and any other code managing the same download id task, will be re-enabled when
// that is fixed.
static bool GBackgroundHttpUseCancelByProducingResumeData = false;
static FAutoConsoleVariableRef CVarBackgroundHttpUseCancelByProducingResumeData(
	TEXT("ios.BackgroundHttp.bUseCancelByProducingResumeData"),
	GBackgroundHttpUseCancelByProducingResumeData,
	TEXT("Use cancelByProducingResumeData in RecreateDownload to try reuse existing downloaded data"),
	ECVF_Default
);

static bool GBackgroundHttpReorderCDNsByReachability = true;
static FAutoConsoleVariableRef CVarBackgroundHttpReorderCDNsByReachability(
	TEXT("ios.BackgroundHttp.bReorderCDNsByReachability"),
	GBackgroundHttpReorderCDNsByReachability,
	TEXT("Reorder CDNs by reachability at runtime (true, by default). Otherwise, use Request URLList order."),
	ECVF_Default
);
...
// Force cancel all pending downloads. See ClearPreExistingRequestsAtStartup()
static bool bCancelExistingDownloads = false;
...
static void LogIOSBackgroundDownloadMessage(NSString* Message)
{
	...
	else if (UE_LOG_ACTIVE(LogIOSBackgroundDownload, Log))
	{
		...
		UE_LOGF(LogIOSBackgroundDownload, Log, "%ls", *MessageWrapper);
	}
}
...
+ (FBackgroundNSURLSessionDownloadTaskData* _Nonnull)TaskDataWithURLs:(NSArray<__kindof NSURL*>*)URLs WithRetryCount:(NSInteger)RetryCount WithExpectedResultSize:(uint64)ExpectedResultSize WithRequestID:(NSString*)RequestID;
...
NSString* const SerializationKeyRequestID = @"i";
...
+ (FBackgroundNSURLSessionDownloadTaskData* _Nullable)TaskDataFromSerializedString:(NSString*)SerializedData
{
	...
	NSString* RequestID = [Dict valueForKey:SerializationKeyRequestID];

	if (Error != nil || Version == nil || Version.intValue != 1 || CDNs == nil || Path == nil || RetryCountPerURL == nil || RequestID == nil)
	{
		UE_DNLD_LOG(@"Failed to deserialize task state '%@' due to '%@', %u, %u, %u, %u, %u, %u, %u, %u",
			SerializedData,
...
- (NSString *)ToSerializedString
{
	...
	[Dict setValue:self.RequestID forKey:SerializationKeyRequestID];
...
@property (atomic) BOOL AllowConstrained;
...
- (NSString*)GetTempPathForURL:(NSURL* _Nonnull)URL WithRequestID:(NSString*)RequestID;
- (NSMutableArray<__kindof NSURL*>*)ReorderCDNsByReachability:(NSMutableArray<__kindof NSURL*>*)URLs;
- (NSMutableArray<__kindof NSURL*>*)OrderURLs:(NSArray<__kindof NSURL*>*)Candidates ByCDNs:(NSArray<__kindof FBackgroundNSURLCDNInfo*>*)CDNs;
...
- (NSUInteger)CreateOrFindDownloadForURLs:(NSArray<__kindof NSString*>*)URLStrings WithPriority:(float)Priority WithExpectedResultSize:(uint64)ExpectedResultSize WithRequestID:(NSString*)RequestID;
...
- (void)GetActiveDownloadIDs:(TSet<uint64>&)OutActiveIDs;
...
- (void)SetDownloadResult:(NSInteger)HTTPCode IsDiskSpaceError:(BOOL)bIsDiskSpaceError WithTempFile:(NSString*)TempFile ForDownload:(NSURLSessionDownloadTask*)Task;
...
@implementation FBackgroundNSURLSession
{
	...
	NSObject* _CDNProbeGate; // Serializes HEAD probes in ReorderCDNsByReachability
	...
	bool _bUseExistingNonResumableTasks;
	bool _bUseCancelByProducingResumeData;
...
#if !UE_BUILD_SHIPPING
NSProgressUserInfoKey const NSProgressDownloadStartTime = @"com.epicgames.nsprogress.starttime";
#endif
...
- (void)Initialize
{
	...
	_bUseExistingNonResumableTasks = GBackgroundHttpUseExistingNonResumableTasks; // pass getTasksWithCompletionHandler tasks even if they are not in resumable state
	_bUseCancelByProducingResumeData = GBackgroundHttpUseCancelByProducingResumeData; // use [task cancelByProducingResumeData...] to restart tasks if true, use [task cancel] if false
	...
	GConfig->GetBool(TEXT("BackgroundHttp.iOSSettings"), TEXT("bUseExistingNonResumableTasks"), _bUseExistingNonResumableTasks, GEngineIni);
	GConfig->GetBool(TEXT("BackgroundHttp.iOSSettings"), TEXT("bUseCancelByProducingResumeData"), _bUseCancelByProducingResumeData, GEngineIni);
	...
	UE_DNLD_LOG(@"bCDNReorderByPingTime=%u", _bCDNReorderByPingTime ? 1 : 0);
	UE_DNLD_LOG(@"bUseExistingNonResumableTasks=%u", _bUseExistingNonResumableTasks ? 1 : 0);
	UE_DNLD_LOG(@"bUseCancelByProducingResumeData=%u", _bUseCancelByProducingResumeData ? 1 : 0);
	...
	_CDNProbeGate = [NSObject new];
	_ForegroundStaleDownloadCheckTimer = nil;
	_bAnyTaskDidCompleteWithError = false;

	// Never allow cellular & constrained downloads unless we get explicit opt-in from the user.
	self.AllowCellular    = NO;
	self.AllowConstrained = NO;
	...
	[_Session getTasksWithCompletionHandler:^(NSArray<__kindof NSURLSessionDataTask*>*, NSArray<__kindof NSURLSessionUploadTask*>*, NSArray<__kindof NSURLSessionDownloadTask*>* Downloads)
	{
		...
		if (Downloads != nil)
		{
			for (NSURLSessionDownloadTask* ExistingTask in Downloads)
			{
				...
				if (_bUseExistingNonResumableTasks == false && !bCanRestartTask)
				{
					UE_DNLD_LOG(@"Skipping tracking for existing download task with taskIdentifier %lu because it's not in resumable state (%lu)", ExistingTask.taskIdentifier, ExistingTask.state);
					continue;
				}
				...
				UE_DNLD_LOG(@"Existing task '%@' with taskIdentifier %lu with DownloadId %lu and state %lu", ExistingTask.taskDescription, ExistingTask.taskIdentifier, DownloadId, ExistingTask.state);
			}
		}
...
- (void)dealloc
{
	...
	if (_CDNProbeGate != nil)
	{
		[_CDNProbeGate release];
		_CDNProbeGate = nil;
	}
...
- (NSString*)GetTempPathForURL:(NSURL* _Nonnull)URL WithRequestID:(NSString*)RequestID
{
	@synchronized (self)
	{
		...
		const FString& TempFileName = HelperRef->FindOrAddTempFilenameMappingForURL(TaskURL, RequestID);
		...
		const FString DestinationDir = FPaths::GetPath(ConvertedPath);
		PlatformFile.CreateDirectory(*DestinationDir);

		return ConvertedPath.GetNSString();
	}
}

// Orders candidate URLs by their matching CDN, just appending any candidate without a match.
- (NSMutableArray<__kindof NSURL*>*)OrderURLs:(NSArray<__kindof NSURL*>*)Candidates ByCDNs:(NSArray<__kindof FBackgroundNSURLCDNInfo*>*)CDNs
{
	// Array sizes are assumed small enough that hashmap is not needed

	NSMutableArray<__kindof NSURL*>* Result = [NSMutableArray arrayWithCapacity:[Candidates count]];

	for (FBackgroundNSURLCDNInfo* Info in CDNs)
	{
...
- (NSMutableArray<__kindof NSURL*>*)ReorderCDNsByReachability:(NSMutableArray<__kindof NSURL*>*)URLs
{
	...
	// Fast path: re-uses existing CDN info.
	@synchronized (_CDNInfo)
	{
		if ([_CDNInfo count] != 0)
		{
			return [self OrderURLs:URLs ByCDNs:_CDNInfo];
		}
	}

	// We only allow a single-fligh CDN info update.
	@synchronized (_CDNProbeGate)
	{
		// A prior gate holder may have already published the info.
		@synchronized (_CDNInfo)
		{
			if ([_CDNInfo count] != 0)
			{
		...
		// Requests will put results into a local Pending state, which is later
		// published into _CDNInfo.
		NSMutableArray<__kindof FBackgroundNSURLCDNInfo*>* Pending = [NSMutableArray array];

		std::shared_ptr<std::atomic<int32>> InflightCount = std::make_shared<std::atomic<int32>>((int32)URLs.count);
		std::shared_ptr<std::promise<void>> Done = std::make_shared<std::promise<void>>();
		std::shared_ptr<std::atomic<bool>> AcceptWrites = std::make_shared<std::atomic<bool>>(true);
		...
		for (NSURL* URL in URLs)
		{
			...
			{
				// Fast reject — handler may fire after wait_for; skipping the decrement is fine since Done is no longer awaited.
				if (!AcceptWrites->load())
				{
					return;
				}
				...
				if (InflightCount->fetch_sub(1) == 1)
				{
		...
		Done->get_future().wait_for(std::chrono::milliseconds(_HeadRequestTimeout));
		UE_DNLD_LOG(@"Finished waiting for CDN reachability");

		AcceptWrites->store(false);
		[_HeadRequestSession resetWithCompletionHandler:^{}];

		// AcceptWrites was flipped above, so any handler entering Pending now reads
		// false and skips. So either task added info or won't touch it (in a case of
		// timeout).
		@synchronized (Pending)
		{
			for (NSUInteger URLIndex = 0; URLIndex < [URLs count]; URLIndex++)
			{
				...
				for (FBackgroundNSURLCDNInfo* Info in Pending)
				{
					if ([Info.CDNHost isEqualToString:URL.host])
					{
			...
			[Pending sortUsingComparator:^NSComparisonResult(FBackgroundNSURLCDNInfo* _Nonnull A, FBackgroundNSURLCDNInfo* _Nonnull B)
			{
			...
			{
				FBackgroundNSURLCDNInfo* Info = [Pending objectAtIndex:i];
		...
		NSMutableArray<__kindof NSURL*>* Result = [self OrderURLs:URLs ByCDNs:Pending];

		// _CDNInfo is write-once, published under the  _CDNProbeGate.
		@synchronized (_CDNInfo)
		{
			[_CDNInfo addObjectsFromArray:Pending];
		}
...
- (NSArray<__kindof FBackgroundNSURLCDNInfo*>*)GetCDNInfo
{
	@synchronized (_CDNInfo)
	{
		return [[_CDNInfo copy] autorelease];
	}
}
...
- (NSURLSessionDownloadTask*)CreateDownloadForURL:(NSURL* _Nonnull)URL WithPriority:(float)Priority WithTaskData:(FBackgroundNSURLSessionDownloadTaskData* _Nonnull)TaskData;
{
	...
	URLRequest.allowsConstrainedNetworkAccess = self.AllowConstrained;
	URLRequest.allowsExpensiveNetworkAccess = self.AllowCellular;
#if !UE_BUILD_SHIPPING
	if ([URL.host hasSuffix:@".akamaized.net"])
	{
		[URLRequest setValue:@"akamai-x-cache-on, akamai-x-cache-remote-on, akamai-x-check-cacheable" forHTTPHeaderField:@"Pragma"];
	}
#endif
...
- (NSUInteger)CreateOrFindDownloadForURLs:(NSArray<__kindof NSString*>*)URLStrings WithPriority:(float)Priority WithExpectedResultSize:(uint64)ExpectedResultSize WithRequestID:(NSString*)RequestID
{
	...
	if (GBackgroundHttpReorderCDNsByReachability)
	{
		URLs = [self ReorderCDNsByReachability:URLs];
	}

	// Serialize current settings
	FBackgroundNSURLSessionDownloadTaskData* TaskData = [FBackgroundNSURLSessionDownloadTaskData TaskDataWithURLs:URLs WithRetryCount:_RetryResumeDataLimit WithExpectedResultSize:ExpectedResultSize WithRequestID:RequestID];
	...
	if (AssetPath != nil)
	{
		@synchronized(_AllDownloads)
		{
			...
			[_AllDownloads enumerateKeysAndObjectsUsingBlock:^(NSNumber* IterKey, NSURLSessionDownloadTask* IterTask, BOOL* IterStop)
			 {
				...
				{
					FBackgroundNSURLSessionDownloadTaskData* IterTaskData = [FBackgroundNSURLSessionDownloadTaskData TaskDataFromSerializedString:IterTask.taskDescription];
					if (TaskData == nil || [IterTaskData.RequestID isEqualToString:RequestID])
					{
			...
			{				
				UE_DNLD_LOG(@"Found existing download task for path '%@' requestID '%@' with DownloadId %lu", AssetPath, RequestID, ExistingDownloadId);
...
- (void)SetCurrentDownloadedBytes:(uint64)DownloadedBytes ForTask:(NSURLSessionDownloadTask*)Task
{
	if (Task != nil)
	{
#if !UE_BUILD_SHIPPING // only needed for GetDownloadDebugText
		if ([Task.progress.userInfo objectForKey:NSProgressDownloadStartTime] == nil)
		{
			[Task.progress setUserInfoObject:[NSNumber numberWithDouble:[[NSDate date] timeIntervalSince1970]] forKey:NSProgressDownloadStartTime];
		}
#endif
...
- (uint64)GetCurrentDownloadedBytes:(NSUInteger)DownloadId
{
	...
	if (Task != nil)
	{
		...
		else // if didWriteData was not invoked yet
		{
			// TODO maybe we can use this for everything?
...
- (void)GetActiveDownloadIDs:(TSet<uint64>&)OutActiveIDs
{
	@synchronized(_AllDownloads)
	{
		...
		{
			// NSURLSessionTaskStateRunning means "started, not suspended/cancelled" — it does NOT mean
			// the task has an active TCP connection. With MaximumConnectionsPerHost, NSURLSession internally
			// queues tasks beyond the connection limit and still reports them as Running.
			// Use two signals to determine if a task has a real connection:
			//   - countOfBytesReceived > 0: body data is flowing
			//   - countOfBytesExpectedToReceive > 0 and != unknown: response headers received (connection
			//     established, server responded) even if no body data yet
			const bool bHasReceivedData = Task.countOfBytesReceived > 0;
			const bool bHasReceivedHeaders = Task.countOfBytesExpectedToReceive > 0 && Task.countOfBytesExpectedToReceive != NSURLSessionTransferSizeUnknown;
			if (Task.state == NSURLSessionTaskStateRunning && (bHasReceivedData || bHasReceivedHeaders))
			{
				OutActiveIDs.Add(Key.unsignedLongLongValue);
			}
		}];
	}
}
...
- (void)RecreateDownload:(NSUInteger)DownloadId ShouldResetRetryCount:(bool)ResetRetryCount
{
	...
	FBackgroundNSURLSessionDownloadTaskData* OldTaskData = [FBackgroundNSURLSessionDownloadTaskData TaskDataFromSerializedString:OldTask.taskDescription];
	...
	if (OldTaskData == nil || NewTaskData == nil)
	{
		return;
	}
	...
	// Remove task data from this download task, otherwise didCompleteWithError might retry the request.
	[OldTaskData Cancel];
	OldTask.taskDescription = [OldTaskData ToSerializedString];

	// Must retain otherwise EnsureTaskIsNotTracked will free the object.
	[OldTask retain];

	// We're done with this task.
	[self EnsureTaskIsNotTracked:OldTask];

	// If permissions changed, we need to recreate task from scratch skipping the resume data.
	// That is unfortunate, but otherwise we might recreate a task _after_ user allowed us to
	// use cellular/constrained network, which would have old permissions and not be able to
	// download.
	NSURLRequest* Request = OldTask.currentRequest != nil ? OldTask.currentRequest : OldTask.originalRequest;
	const bool bIsResumable =
		(Request == nil) || (
		Request.allowsCellularAccess           == self.AllowCellular    &&
		Request.allowsConstrainedNetworkAccess == self.AllowConstrained &&
		Request.allowsExpensiveNetworkAccess   == self.AllowCellular);

	if (_bUseCancelByProducingResumeData && bIsResumable)
	{
		// Will invoke didCompleteWithError if task is incomplete.
		[OldTask cancelByProducingResumeData:^(NSData * _Nullable ResumeData)
		{
			if (ResumeData != nil)
			{
				UE_DNLD_LOG(@"RecreateDownload OldTask cancelByProducingResumeData for DownloadId %lu returned resume data", DownloadId);
				
				NSURLSessionDownloadTask* NewTask = [self CreateDownloadForResumeData:ResumeData WithPriority:OldTaskPriority WithTaskData:NewTaskData];
			...
			{
				UE_DNLD_LOG(@"RecreateDownload OldTask cancelByProducingResumeData for DownloadId %lu did not return resume data", DownloadId);
				
				// start without resume data, will drop any already downloaded data
...
- (void)SwitchToForegroundSession
{
	UE_DNLD_LOG([self IsForegroundSession]
		? @"SwitchToForegroundSession: already in foreground"
		: @"SwitchToForegroundSession: switching from background");

	bool bDiscretionary = true;
	double TimeoutIntervalForRequest = 120.0;
	double TimeoutIntervalForResource = 60.0 * 60.0;
	...
	Configuration.allowsExpensiveNetworkAccess = YES;
	Configuration.allowsConstrainedNetworkAccess = YES;
	Configuration.networkServiceType = NSURLNetworkServiceTypeDefault; // always default in foreground
	// TODO enable if entitlement is provided
	// Configuration.multipathServiceType = NSURLSessionMultipathServiceTypeAggregate;
	Configuration.HTTPMaximumConnectionsPerHost = _MaximumConnectionsPerHost; // TODO maybe x2 the number?
	Configuration.timeoutIntervalForRequest = TimeoutIntervalForRequest;
	Configuration.timeoutIntervalForResource = TimeoutIntervalForResource;

	NSURLSession* OldSession = _Session;
	_Session = [[NSURLSession sessionWithConfiguration:Configuration delegate:self delegateQueue:nil] retain];
	UE_DNLD_LOG(@"SwitchToForegroundSession: new foreground session created");

	[self RecreateDownloads];
...
- (BOOL)IsForegroundSession
{
	// Default sessions have a nil identifier; background-identifier sessions are non-nil.
	return _Session.configuration.identifier == nil;
}
...
- (NSString*)GetDownloadDebugText:(NSUInteger)DownloadId
{
	...
	NSString* Description = [NSString stringWithFormat:@"%llu:%@", (uint64)DownloadId, Task.currentRequest.URL.absoluteString];

	NSString* DetailMode = [self IsForegroundSession] ? @"FG" : @"BG";

	NSNumber* ResultStatusCode = [Task.progress.userInfo objectForKey:NSProgressDownloadResultStatusCode];
	NSString* DetailResult = ResultStatusCode != nil ?
		[NSString stringWithFormat:@" Done:%i", (int32)ResultStatusCode.integerValue]
		: @"";

	NSString* DetailCache = @" NoResp";
	if ([Task.response isKindOfClass:[NSHTTPURLResponse class]])
	{
		NSHTTPURLResponse* Response = (NSHTTPURLResponse*)Task.response;

		NSString* Cache  = [Response valueForHTTPHeaderField:@"X-Epic-Cache"]
						?: [Response valueForHTTPHeaderField:@"X-Cache"]
						?: [Response valueForHTTPHeaderField:@"CF-Cache-Status"];
		NSString* Age    = [Response valueForHTTPHeaderField:@"Age"];
		NSString* Via    = [Response valueForHTTPHeaderField:@"Via"];
		NSString* Server = [Response valueForHTTPHeaderField:@"Server"];

		DetailCache = 
			  Cache != nil  ? [NSString stringWithFormat:@" Cache:%@", Cache]
			: Age != nil    ? [NSString stringWithFormat:@" Age:%@s", Age]
			: Via != nil    ? [NSString stringWithFormat:@" Via:%@", Via]
			: Server != nil ? [NSString stringWithFormat:@" Srv:%@", Server]
			: @" NoCacheHdr";
	}

	NSNumber* CompletedBytes = [Task.progress.userInfo objectForKey:NSProgressDownloadCompletedBytes];
	NSString* DetailBytes = CompletedBytes != nil ?
		[NSString stringWithFormat:@" %.2fMB", (double)CompletedBytes.unsignedLongLongValue / (1024.0 * 1024.0)]
		: @"";

	NSNumber* StartTime = [Task.progress.userInfo objectForKey:NSProgressDownloadStartTime];
	NSString* DetailSpeed = @"";
	if (StartTime != nil && CompletedBytes != nil)
	{
		double Elapsed = [[NSDate date] timeIntervalSince1970] - StartTime.doubleValue;
		if (Elapsed > 0.0)
		{
			double MBps = (double)CompletedBytes.unsignedLongLongValue / Elapsed / (1024.0 * 1024.0);
			DetailSpeed = [NSString stringWithFormat:@" %.2fMB/s", MBps];
		}
	}
	
	return [NSString stringWithFormat:@"%@\n%@%@%@%@%@", Description, DetailMode, DetailResult, DetailBytes, DetailSpeed, DetailCache];
}
#endif
...
- (void)ReplaceTrackedTaskWith:(NSURLSessionDownloadTask*)NewTask ForDownloadId:(NSUInteger)DownloadId
{
	@synchronized(_AllDownloads)
	{
		...
		// in case if all downloads were drained, we need to restart a timer as a new task was inserted
		[self StartCheckingForStaleDownloads];
	}
}
...
- (void)SetDownloadResult:(NSInteger)HTTPCode IsDiskSpaceError:(BOOL)bIsDiskSpaceError WithTempFile:(NSString*)TempFile ForDownload:(NSURLSessionDownloadTask*)Task
{
	...
	if (bIsDiskSpaceError)
		Result = EBackgroundHttpRequestCompletionResult::OutOfDiskSpaceError;
	else if (!bDownloadSuccess)
		Result = EBackgroundHttpRequestCompletionResult::GenericError;
	else
		Result = EBackgroundHttpRequestCompletionResult::Success;
	
	FBackgroundURLSessionHandler::OnDownloadCompletedExtended.Broadcast(DownloadId, Result);
}
...
static bool IsOutOfDiskSpaceError(NSError* Error, int Depth = 0)
{
	if (!Error)
	{
		return false;
	...
	// prevent theoretical cyclic references that would never end; number is arbitrarily chosen
	if (Depth >= 5)
	{
		return false;
	}
	...
	if (Underlying)
	{
		return IsOutOfDiskSpaceError(Underlying, Depth + 1);
	}
...
- (void)URLSession:(NSURLSession*)Session task:(NSURLSessionTask*)GenericTask didCompleteWithError:(NSError*)Error
{
	...
	const bool bOutOfDiskSpaceError = IsOutOfDiskSpaceError(Error);
	...
	if (!bOutOfDiskSpaceError && bIsTrackedTask && TaskData != nil)
	{
	...
	{
		UE_DNLD_LOG(@"didCompleteWithError, task '%@' with taskIdentifier %lu failed due to [%@:%ld] '%@', has no retry data or no next url, failing request (bOutOfDiskSpaceError=%d)", Task.taskDescription, Task.taskIdentifier, Error.domain, Error.code, LocalizedDescription, bOutOfDiskSpaceError);
		...
		[self SetDownloadResult:StatusCode IsDiskSpaceError:bOutOfDiskSpaceError WithTempFile:nil ForDownload:Task];
	}
}
...
- (void)URLSession:(NSURLSession*)Session downloadTask:(NSURLSessionDownloadTask*)Task didFinishDownloadingToURL:(NSURL*)Location
{
	...
	FBackgroundNSURLSessionDownloadTaskData* TaskData = [FBackgroundNSURLSessionDownloadTaskData TaskDataFromSerializedString:Task.taskDescription];
	NSString* RequestID = TaskData != nil ? TaskData.RequestID : @"";

	NSString* DestinationPath = [self GetTempPathForURL:Task.originalRequest.URL WithRequestID:RequestID];
	...
	if (TaskData != nil && TaskData.ExpectedResultSize > 0)
	{
		...
		if (Attributes != nil && AttributesError == nil)
		{
			...
			{
				[self SetDownloadResult:HTTPStatusCodeErrorServer IsDiskSpaceError:false WithTempFile:nil ForDownload:Task];
	...
	if (Error != nil)
	{
		[self SetDownloadResult:HTTPStatusCodeErrorServer IsDiskSpaceError:IsOutOfDiskSpaceError(Error) WithTempFile:nil ForDownload:Task];
	...
	{
		[self SetDownloadResult:HTTPStatusCodeSuccessCreated IsDiskSpaceError:false WithTempFile:DestinationPath ForDownload:Task];
...
FBackgroundURLSessionHandler::FOnDownloadCompletedExtended FBackgroundURLSessionHandler::OnDownloadCompletedExtended;
...
void FBackgroundURLSessionHandler::AllowConstrained(bool bAllow)
{
	@autoreleasepool
	{
		const BOOL bCurrentValue = [FBackgroundNSURLSession Shared].AllowConstrained;
		const BOOL bNewValue = bAllow ? YES : NO;
		if (bCurrentValue == bNewValue)
		{
			return;
		...
		[[FBackgroundNSURLSession Shared] setAllowConstrained:bNewValue];
		[[FBackgroundNSURLSession Shared] RecreateDownloads];
	}
}

uint64 FBackgroundURLSessionHandler::CreateOrFindDownload(const TArray<FString>& URLs, const float Priority, BackgroundHttpFileHashHelperRef HelperRef, const uint64 ExpectedResultSize, const FString& RequestID)
{
	@autoreleasepool
	{
		...
		return [[FBackgroundNSURLSession Shared] CreateOrFindDownloadForURLs:URLArray WithPriority:Priority WithExpectedResultSize:ExpectedResultSize WithRequestID:RequestID.GetNSString()];
	}
}
...
void FBackgroundURLSessionHandler::GetActiveDownloadIDs(TSet<uint64>& OutActiveIDs)
{
	@autoreleasepool
	{
		OutActiveIDs.Reset();
		[[FBackgroundNSURLSession Shared] GetActiveDownloadIDs:OutActiveIDs];
	}
}
...
void FBackgroundURLSessionHandler::CleanupOldTempDownloads()
{
	IPlatformFile& PlatformFile = FPlatformFileManager::Get().GetPlatformFile();
	const FString TempPath = FBackgroundHttpFileHashHelper::GetTemporaryRootPath();
	UE_LOGF(LogIOSBackgroundDownload, VeryVerbose, "CleanupOldTempDownloads: cleaning up %ls", *TempPath);
	if (PlatformFile.DirectoryExists(*TempPath)) 
	{
		const FString BranchName = BuildSettings::GetBranchName();
		UE_LOGF(LogIOSBackgroundDownload, VeryVerbose, "CleanupOldTempDownloads: keeping directory: %ls", *BranchName);
		PlatformFile.IterateDirectory(*TempPath, [&BranchName, &PlatformFile](const TCHAR* FilenameOrDirectory, bool bIsDirectory) -> bool {
			UE_LOGF(LogIOSBackgroundDownload, VeryVerbose, "CleanupOldTempDownloads: considering: %ls | %d", FilenameOrDirectory, (int)bIsDirectory);
			if (bIsDirectory)
			{
				const FString DirectoryName = FPaths::GetCleanFilename(FilenameOrDirectory);
				if (DirectoryName != TEXT("URLMap") && DirectoryName != BranchName)
				{
					UE_LOGF(LogIOSBackgroundDownload, VeryVerbose, "CleanupOldTempDownloads: deleting: %ls", FilenameOrDirectory);
...
void FBackgroundURLSessionHandler::ClearPreExistingRequestsAtStartup(bool bEnable)
{
	bCancelExistingDownloads = bEnable;	
}

#if !UE_BUILD_SHIPPING
void BackgroundURLSessionHandler_SwitchToForegroundSession() // not public API at this point
{
	@autoreleasepool
	{
		[[FBackgroundNSURLSession Shared] SwitchToForegroundSession];
	}
}
#endif
