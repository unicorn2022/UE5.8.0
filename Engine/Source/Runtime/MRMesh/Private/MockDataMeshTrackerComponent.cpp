...
void UMockDataMeshTrackerComponent::ConnectMRMesh(UMRMeshComponent* InMRMeshPtr)
{
	if (!InMRMeshPtr)
	{
		UE_LOGF(LogMockMeshDataTracker, Warning,
			"MRMesh given is not valid. Ignoring this connect.");
		return;
	}
	else if (MRMesh)
	{
		UE_LOGF(LogMockMeshDataTracker, Warning,
			"MeshTrackerComponent already has a MRMesh connected.  Ignoring this connect.");
		return;
	}
	else if (InMRMeshPtr->IsConnected())
	{
		UE_LOGF(LogMockMeshDataTracker, Warning,
			"MRMesh is already connected to a UMockDataMeshTrackerComponent. Ignoring this connect.");
		return;
	}
	else
	{
...
void UMockDataMeshTrackerComponent::DisconnectMRMesh(class UMRMeshComponent* InMRMeshPtr)
{
	...
	{
		UE_LOGF(LogMockMeshDataTracker, Warning,
			"MeshTrackerComponent MRMesh is already disconnected. Ignoring this disconnect.");
		return;
	}
	else if (InMRMeshPtr != MRMesh)
	{
		UE_LOGF(LogMockMeshDataTracker, Warning,
			"MeshTrackerComponent MRMesh given is not the MRMesh connected. "
...
void UMockDataMeshTrackerComponent::PostEditChangeProperty(FPropertyChangedEvent& e)
{
	...
	{
		UE_LOGF(LogMockMeshDataTracker, Log, "PostEditChangeProperty is changing MLMeshingSettings");
	}
...
void UMockDataMeshTrackerComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction *ThisTickFunction)
{
	...
	if (ScanWorld && CurrentTime > LastUpdateTime + UpdateInterval)
	{
		...
		if (MockDataPattern == 0)
		{
			...
			UE_LOGF(LogMockMeshDataTracker, Log, "TickComponent is updating Add: %i Update: %i Remove: %i", AddBlockIndex, UpdateBlockIndex, RemoveBlockIndex);
		...
		else
		{
			...
			UE_LOGF(LogMockMeshDataTracker, Log, "TickComponent is adding 4 blocks");
			UpdateBlock(0);
			UpdateBlock(1);
			UpdateBlock(2);
			UpdateBlock(3);
...
void UMockDataMeshTrackerComponent::UpdateBlock(int32 BlockIndex)
{
	...
	switch (VertexColorMode)
	{
		case EMeshTrackerVertexColorMode::Confidence:
		{
			...
			{
				UE_LOGF(LogMockMeshDataTracker, Warning, "MeshTracker vertex color mode is Confidence "
		...
		case EMeshTrackerVertexColorMode::Block:
		{
			...
			{
				UE_LOGF(LogMockMeshDataTracker, Warning, "MeshTracker vertex color mode is Block but "
