...
bool FPcmAudioInfoHybrid::AbsPositionToChunkIndexAndOffset(const uint64 InAbsPosition, uint32& OutChunkIndex, uint32& OutChunkOffset) const
{
	...
	if (const TSharedPtr<const FSoundWaveData>& Wave = GetStreamingSoundWaveData(); Wave.IsValid() && Wave->IsStreaming())
	{
...
bool FPcmAudioInfoHybrid::ParseHeader(const uint8* InSrcBufferData, uint32 InSrcBufferDataSize, FSoundQualityInfo* QualityInfo)
{
	...
	const bool bParseJustHeader = GetStreamingSoundWaveData().IsValid() && GetStreamingSoundWaveData()->IsStreaming();
	...
	{
		UE_LOGF(LogAudio, Error, "%s WaveInfo.ReadWaveInfo Failed", __func__);
		return false;
	}
	if (!FormatHeader)
	{
		UE_LOGF(LogAudio, Error, "%s WaveInfo.ReadInfo Failed to return a Header", __func__);
		return false;
	}
	...
	{
		UE_LOGF(LogAudio, Error, "%s HeaderSize=%u", __func__, HeaderSize);
		return false;	
	}
	...
	if (NumChannels <= 0)
	{
		UE_LOGF(LogAudio, Error, "%s NumChannels=%u", __func__, NumChannels);
		return false;
	}
	const uint32 SampleRate = *WaveInfo.pSamplesPerSec;
	if (SampleRate == 0)
	{
		UE_LOGF(LogAudio, Error, "%s SampleRate=%u", __func__, SampleRate);
		return false;	
	}
	...
	if (!Impl.IsValid())
	{
		UE_LOGF(LogAudio, Error, "%s, Failed to Create Impl for %u", __func__, *WaveInfo.pFormatTag);
		return false;
	}
...
{
	check(TotalSamplesPerChannel > 0);
