...
namespace ADPCM
{
	...
	bool DecodeBlock(const uint8* EncodedADPCMBlock, int32 BlockSize, int16* DecodedPCMData)
	{
		...
		{
			UE_LOGF(LogAudio, Error, "Decoding ADPCM block resulted in bad CoefficientIndex (%d). BlockSize: %d, ReadIndex: %d", CoefficientIndex, BlockSize, ReadIndex);
			return false;
		}
		else
		{
	...
	bool DecodeBlock(const uint8* EncodedADPCMBlock, const int32 BlockSize, int16* DecodedPCMData, const int32 DecodedPCMSizeSamples)
	{
		...
		{
			UE_LOGF(LogAudio, Error, "Decoding ADPCM block has insufficient space to decode to: DecodedPCMSize=%d", DecodedPCMSizeSamples);
			return false;
		}
