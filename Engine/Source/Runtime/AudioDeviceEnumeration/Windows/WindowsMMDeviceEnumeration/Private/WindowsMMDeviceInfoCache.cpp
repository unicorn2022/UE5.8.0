...
namespace Audio
{
	...
	void FWindowsMMDeviceCache::Refresh()
	{
		UE_LOGF(LogAudioEnumeration, Display, "Refreshing FWindowsMMDeviceCache");
		// Hold the write lock across both phases so a reader landing between
		// the two cannot observe a fully repopulated Cache map paired with
		// stale DefaultRenderId / DefaultCaptureId arrays.
		FWriteScopeLock WriteLock(CacheMutationLock);
		EnumerateEndpoints_NoLock();
		EnumerateDefaults_NoLock();
	}

PRAGMA_DISABLE_DEPRECATION_WARNINGS
	void FWindowsMMDeviceCache::EnumerateEndpoints()
	{
		// Deprecated wrapper: takes the lock then forwards to the no-lock implementation.
		FWriteScopeLock WriteLock(CacheMutationLock);
		EnumerateEndpoints_NoLock();
	}
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	bool FWindowsMMDeviceCache::EnumerateChannelMask(uint32 InMask, FCacheEntry& OutInfo)
	{
		...
		{
			UE_CLOGF(WindowsMMCvarUtils::ShouldLogDeviceSwaps(), LogAudioEnumeration, Warning, "FWindowsMMDeviceCache: Did not find the channel type flags for audio device '%ls'. Reverting to a default channel ordering.", *OutInfo.FriendlyName);
	...
	bool FWindowsMMDeviceCache::EnumerateDeviceProps(const TComPtr<IMMDevice>& InDevice, FCacheEntry& OutInfo)
	{
		...
		if (SUCCEEDED(InDevice->OpenPropertyStore(STGM_READ, &PropertyStore)))
		{
			...
			else
			{
				...
				UE_CLOGF(DeviceState == DEVICE_STATE_ACTIVE, LogAudioEnumeration, Warning, "FWindowsMMDeviceCache: Failed to get Format for active device '%ls'", *OutInfo.FriendlyName);
			}
		}
	...
	bool FWindowsMMDeviceCache::EnumerateHardwareTopology(const TComPtr<IMMDevice>& InDevice, FCacheEntry& OutInfo)
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogAudioEnumeration, Verbose, "FWindowsMMDeviceCache::EnumerateHardwareTopology failed to fetch IDeviceTopology: 0x%x", Result);
			return false;
		}
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogAudioEnumeration, Verbose, "FWindowsMMDeviceCache::EnumerateHardwareTopology failed to fetch connector: 0x%x", Result);
			return false;
		}
		...
		{
			UE_LOGF(LogAudioEnumeration, Verbose, "FWindowsMMDeviceCache::EnumerateHardwareTopology failed to fetch render filter Id: 0x%x", Result);
			return false;
		}
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogAudioEnumeration, Verbose, "FWindowsMMDeviceCache::EnumerateHardwareTopology failed to fetch render node: 0x%x", Result);
			return false;
		}
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogAudioEnumeration, Verbose, "FWindowsMMDeviceCache::EnumerateHardwareTopology failed to open property store: 0x%x", Result);
			return false;
		}
		...
		{
			UE_LOGF(LogAudioEnumeration, Verbose, "FWindowsMMDeviceCache::EnumerateHardwareTopology failed to fetch hardward Id: 0x%x", Result);
			return false;
		}
	...
	void FWindowsMMDeviceCache::EnumerateEndpoints_NoLock()
	{
		SCOPED_NAMED_EVENT(FWindowsMMDeviceCache_EnumerateEndpoints, FColor::Blue);

		// Caller must hold CacheMutationLock for write.
		...
		if (DeviceEnumerator)
		{
				...
				for (uint32 i = 0; i < DeviceCount; ++i)
				{
					...
					if (SUCCEEDED(DeviceCollection->Item(i, &Device)) && Device)
					{
						...
						if (SUCCEEDED(Device->GetId(&DeviceIdString.StringPtr)) && DeviceIdString)
						{
							...
							UE_LOGF(LogAudioEnumeration, Verbose, "FWindowsMMDeviceCache: %ls Device '%ls' ID='%ls'",
			...
			// Finally, Replace cache with new one. Caller already holds the write lock.
			Cache = MoveTemp(NewCache);
		}
	}

	void FWindowsMMDeviceCache::EnumerateDefaults_NoLock()
	{
		if (!DeviceEnumerator.IsValid())
		{
			UE_LOGF(LogAudioEnumeration, Warning, "FWindowsMMDeviceCache::EnumerateDefaults() NULL device enumerator");
			return;
		}
		...
		auto GetDefaultDeviceID = [this](EDataFlow InDataFlow, ERole InRole, FName& OutDeviceId) -> bool
		{
			...
			if (SUCCEEDED(DeviceEnumerator->GetDefaultAudioEndpoint(InDataFlow, InRole, &DefaultDevice)))
			{
				...
				{
					UE_LOGF(LogAudioEnumeration, Display, "FWindowsMMDeviceCache::EnumerateDefaults() NULL default device for InDataFlow: %d InRole: %d", InDataFlow, InRole);
				}
			}
			return bSuccess;
		};

		// Get defaults (render, capture). Caller must hold CacheMutationLock for write.
		...
		for (int32 i = 0; i < ERole_enum_count; ++i)
		{
			...
			{
				UE_CLOGF(!DeviceIdName.IsNone(), LogAudioEnumeration, Display, "FWindowsMMDeviceCache: Default Render Role='%ls', Device='%ls'", ToString((EAudioDeviceRole)i), *GetFriendlyName(DeviceIdName));
				DefaultRenderId[i] = DeviceIdName;
			}
			if (GetDefaultDeviceID(eCapture, static_cast<ERole>(i), DeviceIdName))
			{
				UE_CLOGF(!DeviceIdName.IsNone(), LogAudioEnumeration, Display, "FWindowsMMDeviceCache: Default Capture Role='%ls', Device='%ls'", ToString((EAudioDeviceRole)i), *GetFriendlyName(DeviceIdName));
	...
	void FWindowsMMDeviceCache::OnDeviceAdded(const FString& DeviceId, bool bIsRender)
	{
		if (TOptional<FCacheEntry> NewDeviceEntry = BuildCacheEntry(DeviceId))
		{
			...
			if (FCacheEntry* Entry = Cache.Find(NewDeviceEntry->DeviceId))
			{
				*Entry = MoveTemp(*NewDeviceEntry);
				UE_LOGF(LogAudioEnumeration, Warning, "FWindowsMMDeviceCache::OnDeviceAdded: Device already in cache. Replacing entry for DeviceID='%ls'", *DeviceId);
			}
			else
			{
		...
		{
			UE_LOGF(LogAudioEnumeration, Warning, "FWindowsMMDeviceCache::OnDeviceAdded: Failed to add DeviceID='%ls' to cache. ", *DeviceId);
		}
	}
	...
	void FWindowsMMDeviceCache::OnDeviceRemoved(const FString& DeviceId, bool)
	{
		...
		UE_CLOGF(!Cache.Contains(DeviceIdName), LogAudioEnumeration, Warning, "FWindowsMMDeviceCache::OnDeviceRemoved: DeviceId='%ls' was not in the cache. ", *DeviceId);
	...
	void FWindowsMMDeviceCache::OnDeviceStateChanged(const FString& DeviceId, const EAudioDeviceState InState, bool)
	{
		...
		if (FCacheEntry* Entry = Cache.Find(DeviceIdName))
		{
			...
			UE_CLOGF(WindowsMMCvarUtils::ShouldLogDeviceSwaps(), LogAudioEnumeration, Verbose, "FWindowsMMDeviceCache: DeviceName='%ls' - DeviceID='%ls' state changed from '%ls' to '%ls'.",
	...
	void FWindowsMMDeviceCache::InvalidateDevice(FName InDeviceId)
	{
		// Called when a consumer observes that a device reported Active is actually unreachable.
		// Cheap, surgical: flip one entry Active -> NotPresent under per-entry write lock so the
		// next swap-retry iteration sees it correctly and stops looping on the zombie ID.
		if (InDeviceId.IsNone())
		{
			return;
		}

		// Map read-lock is sufficient because we are not adding or removing entries; we are only
		// mutating an existing entry's State. The per-entry write lock serializes that mutation.
		FReadScopeLock ReadLock(CacheMutationLock);

		if (FCacheEntry* Entry = Cache.Find(InDeviceId))
		{
			FWriteScopeLock WriteLock(Entry->MutationLock);
			if (Entry->State == EAudioDeviceState::Active)
			{
				UE_LOGF(LogAudioEnumeration, Warning, "FWindowsMMDeviceCache::InvalidateDevice: FriendlyName='%ls' DeviceID='%ls' marked NotPresent; caller observed it unreachable.",
					*Entry->FriendlyName, *InDeviceId.ToString());
				Entry->State = EAudioDeviceState::NotPresent;
			}
		}
	}
	...
	void FWindowsMMDeviceCache::OnFormatChanged(const FString& InDeviceId, const FFormatChangedData& InFormat)
	{
		...
		if (FCacheEntry* Found = Cache.Find(DeviceName))
		{
			...
			{
				UE_CLOGF(WindowsMMCvarUtils::ShouldLogDeviceSwaps(), LogAudioEnumeration, Verbose, "FWindowsMMDeviceCache: DeviceID='%ls', Name='%ls' changed default format from %d channels to %d.", *InDeviceId, *EntryCopy.FriendlyName, EntryCopy.NumChannels, InFormat.NumChannels);
			...
			{
				UE_CLOGF(WindowsMMCvarUtils::ShouldLogDeviceSwaps(), LogAudioEnumeration, Verbose, "FWindowsMMDeviceCache: DeviceID='%ls', Name='%ls' changed default format from %dhz to %dhz.", *InDeviceId, *EntryCopy.FriendlyName, EntryCopy.SampleRate, InFormat.SampleRate);
				EntryCopy.SampleRate = InFormat.SampleRate;
				bDirty = true;
			}
			if (EntryCopy.ChannelBitmask != InFormat.ChannelBitmask)
			{
				UE_CLOGF(WindowsMMCvarUtils::ShouldLogDeviceSwaps(), LogAudioEnumeration, Verbose, "FWindowsMMDeviceCache: DeviceID='%ls', Name='%ls' changed default format from 0x%x to 0x%x bitmask", *InDeviceId, *EntryCopy.FriendlyName, EntryCopy.ChannelBitmask, InFormat.ChannelBitmask);
			...
			{
				UE_CLOGF(WindowsMMCvarUtils::ShouldLogDeviceSwaps(), LogAudioEnumeration, Verbose, "FWindowsMMDeviceCache: Channel Change, DeviceID='%ls', Name='%ls' OLD=[%ls]", *InDeviceId, *EntryCopy.FriendlyName, *ToFString(EntryCopy.OutputChannels));
				EnumerateChannelMask(InFormat.ChannelBitmask, EntryCopy);
				UE_CLOGF(WindowsMMCvarUtils::ShouldLogDeviceSwaps(), LogAudioEnumeration, Verbose, "FWindowsMMDeviceCache: Channel Change, DeviceID='%ls', Name='%ls' NEW=[%ls]", *InDeviceId, *EntryCopy.FriendlyName, *ToFString(EntryCopy.OutputChannels));
			}
	...
	FName FWindowsMMDeviceCache::GetDefaultOutputDevice(EAudioDeviceRole InRole) const
	{
		FReadScopeLock MapReadLock(CacheMutationLock);

		if (ensure(InRole >= EAudioDeviceRole::Console && InRole < EAudioDeviceRole::COUNT))
		{
			return DefaultRenderId[(int32)InRole];
		}
		return NAME_None;
	}
