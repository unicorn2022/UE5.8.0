...
struct FWindowsMMDeviceCache : IAudioMixerDeviceChangedListener, IAudioPlatformDeviceInfoCache
{
	...
	UE_DEPRECATED(5.8, "EnumerateEndpoints is no longer publicly callable; use Refresh() to rebuild the cache atomically.")
	UE_API void EnumerateEndpoints();

	UE_DEPRECATED(5.8, "EnumerateDefaults is no longer publicly callable; use Refresh() to rebuild the cache atomically.")
	...
	UE_API virtual void InvalidateDevice(FName InDeviceId) override;

	UE_API virtual FName GetDefaultOutputDevice(EAudioDeviceRole InRole) const override;
...
private:
	/** Re-enumerates all endpoints into the cache. Caller MUST hold CacheMutationLock for write. */
	UE_API void EnumerateEndpoints_NoLock();

	/** Re-reads default render/capture device IDs. Caller MUST hold CacheMutationLock for write. */
	UE_API void EnumerateDefaults_NoLock();
