...
// the DOM json parser is case insensitive on keys names
Json::FValue::ConstMemberIterator FindMemberCaseInsensitive(Json::FConstObject Object, const TCHAR* FieldName)
{
	const int32 FieldNameLength = FCString::Strlen(FieldName);

	for (Json::FValue::ConstMemberIterator MemberIt = Object.MemberBegin(); MemberIt != Object.MemberEnd(); ++MemberIt)
	{
		if (FieldNameLength != MemberIt->name.GetStringLength())
		{
			continue;
		}

		if (FCString::Strnicmp(FieldName, MemberIt->name.GetString(), FieldNameLength) == 0)
		{
			return MemberIt;
		}
	}
...
{
	Json::FValue::ConstMemberIterator It = FindMemberCaseInsensitive(Object, FieldName);
...
static inline bool TryGetIntHelper(Json::FConstObject Object, const TCHAR* FieldName, const TCHAR* OutTypeName, T& Out)
{
	...
	FValue::ConstMemberIterator It = FindMemberCaseInsensitive(Object, FieldName);
	if (It == Object.MemberEnd())
	{
		return false;
	}
