	...
	const TCHAR JsonText[] = TEXT(R"json(
{
	...
	"EnableSceneGraphAnimation": true,
	...
	REQUIRE(Descriptor.bEnableSceneGraphAnimation == true);
...
TEST_CASE_NAMED(FProjectsLoadingRapidJsonPluginLoadingCaseInsensitiveKeys, "System::Engine::Projects::Loading::RapidJsonPluginLoading::CaseInsensitiveKeys", "[ApplicationContextMask][SmokeFilter]")
{
	const TCHAR JsonText[] = TEXT(R"json(
{
	"aaa": 1,
	...
	REQUIRE(TryGetNumberField(*RapidRoot, TEXT("Aaa"), AAA));
	REQUIRE(TryGetNumberField(*RapidRoot, TEXT("Bbb"), BBB));
	REQUIRE(TryGetNumberField(*RapidRoot, TEXT("Ccc"), CCC));
