...
void CheckForDeprecatedPlugin(FPlugin& Plugin)
{
	...
	{
		UE_LOGF(LogPluginManager, Display, "The Plugin %ls has been marked deprecated for %ls and will be removed soon. Please update your dependencies.", *Plugin.GetName(), *DeprecatedEngineVersion);
	}
}
...
namespace DiscoveredPluginMapUtils
{
	...
	static void DiscardAllSuppressedVersions(FDiscoveredPluginMap::ValueType& PluginMapValue)
	{
...
const FString& EnumToString(const EPluginType& InPluginType)
{
	...
	static const FString Engine("Engine");
	static const FString Enterprise("Enterprise");
	static const FString Project("Project");
	static const FString External("External");
	static const FString Mod("Mod");
	static const FString ErrorString("Unknown plugin type, has EPluginType been changed?");
...
void FPluginManager::RefreshPluginsList()
{
	...
	for (FDiscoveredPluginMap::TIterator Iter(AllPlugins); Iter; ++Iter)
	{
		...
		else
		{
			...
			DiscoveredPluginMapUtils::DiscardAllSuppressedVersions(Iter.Value());
		}
	}
...
bool FPluginManager::AddToPluginsList(const FString& PluginFilename, FText* OutFailReason /*= nullptr*/)
{
	...
	else
	{
		...
		UE_LOGF(LogPluginManager, Error, "%ls", *FailReason.ToString());
		if (OutFailReason)
		{
...
bool FPluginManager::RemoveFromPluginsList(const FString& PluginFilename, FText* OutFailReason /*= nullptr*/)
{
	...
	const uint32 PluginNameHash = GetTypeHash(FoundPlugin->GetName());
	PluginsToConfigure.RemoveByHash(PluginNameHash, FoundPlugin->GetName());
	...
	TArray<FString>* OutPluginSources)
{
...
#endif
	{
		UE_LOGF(LogPluginManager, Verbose, "No *.upluginmanifest files found, looking for *.uplugin files instead.")
	...
	else
	{
		...
		for (const FString& ManifestFileName : ManifestFileNames)
		{
			...
			UE_LOGF(LogPluginManager, Verbose, "Reading plugin manifest: %ls", *ManifestFileName);
			if (OutPluginSources)
			{
			...
			{
				UE_LOGF(LogPluginManager, Fatal, "%ls", *FailReason.ToString());
			}
	...
	for (TSharedRef<FPlugin> Child : ChildPlugins)
	{
		...
		if (ParentPtr != nullptr)
		{
			...
			for (const FModuleDescriptor& ChildModule : Child->GetDescriptor().Modules)
			{
				...
				if (!bFound)
				{
					...
					UE_LOGF(LogPluginManager, Log, "Adding extension module %ls from %ls to %ls", *ChildModule.Name.ToString(), *Child->Name, *Parent->Name);
				}
		...
		{
			UE_LOGF(LogPluginManager, Error, "Child plugin %ls was not named properly. It should be in the form <ParentPlugin>_<Platform>.uplugin.", *Child->GetDescriptorFileName());
		}
	}
		...
		SlowTask_ReadPlugins.EnterProgressFrame();
		{
			...
			for (int32 Index = 0, Num = FileNames.Num(); Index < Num; ++Index)
			{
				...
				else
				{
					...
					UE_LOGF(LogPluginManager, Error, "Failed to load Plugin (%ls); %ls", *FullPath, *Context.FailureReason.ToString());
				}
			}
		}
	}
...
void FPluginManager::CreatePluginObject(const FString& FileName, const FPluginDescriptor& Descriptor, const EPluginType Type, FDiscoveredPluginMap& Plugins, TArray<TSharedRef<FPlugin>>& ChildPlugins)
{
	...
	FString FullPath = FPaths::ConvertRelativePathToFull(FileName);
	UE_LOGF(LogPluginManager, Verbose, "Read plugin descriptor for %ls, from %ls", *Plugin->GetName(), *FullPath);
	...
	{
		UE_LOGF(LogPluginManager, Display, "By default, prioritizing project plugin (%ls) over the corresponding engine version (%ls).", *Plugin->FileName, *(*ExistingPlugin)->FileName);
	...
	else if ((*ExistingPlugin)->FileName != Plugin->FileName)
	{
		...
		{
			UE_LOGF(LogPluginManager, Display, "A version of the '%ls' plugin has already been enabled (%ls); prioritizing that over the newly discovered version (%ls).", *Plugin->Name, *(*ExistingPlugin)->FileName, *Plugin->FileName);
		...
		else if ((*ExistingPlugin)->Type == EPluginType::Project && Type == EPluginType::Engine)
		{
			...
			UE_LOGF(LogPluginManager, Display, "By default, prioritizing project plugin (%ls) over the corresponding engine version (%ls).", *(*ExistingPlugin)->FileName, *Plugin->FileName);
			DiscoveredPluginMapUtils::InsertPluginIntoMap(Plugins, Plugin->GetName(), Plugin, DiscoveredPluginMapUtils::EInsertionType::AsSuppressedPlugin);
		}
		else
		{
			...
			if (ExistingVersion != Descriptor.Version)
			{
				UE_LOGF(
					LogPluginManager,
					Display,
					"By default, prioritizing newer version (v%d) of '%ls' plugin, over older version (v%d).",
			...
			{
				UE_LOGF(LogPluginManager, Warning, "The same version (v%d) of plugin '%ls' exists at '%ls' and '%ls' - second location will be ignored.", ExistingVersion, *Plugin->GetName(), *(*ExistingPlugin)->FileName, *Plugin->FileName);
			}
		}
	}
...
FPluginManager::FLeakedPackagesFoundEvent& FPluginManager::OnLeakedPackagesFound()
{
	return LeakedPackagesFoundEvent;
}
...
static TUniquePtr<FTargetReceipt> FindMatchingTargetFile (const FString& ReceiptWildcard)
{
	...
	for (const FString& TargetFileWithoutPath : AllTargetFilesWithoutPath)
	{
		...
		if (Receipt->Read(TargetFile))
		{
			if (Receipt->TargetType == FApp::GetBuildTargetType() && Receipt->Configuration == FApp::GetBuildConfiguration())
			{
				...
				{
					UE_LOGF(LogPluginManager, Log, "Found matching target receipt: %ls", *TargetFile);
					return Receipt;
				}
	...
	if (BestMatch.Value)
	{
		UE_LOGF(LogPluginManager, Log, "Found target receipt: %ls", *BestMatch.Key);
	}
	else
	{
		UE_LOGF(LogPluginManager, Log, "Unable to find target receipt in path: %ls", *ReceiptWildcard);
	}
...
void FPluginManager::FindCompileTimePlugins(FPluginDiscoveryContext& Context)
{
...
#if READ_TARGET_ENABLED_PLUGINS_FROM_RECEIPT
	{
		...
		UE_LOGF(LogPluginManager, Log, "Looking for build plugins target receipt");
...
bool FPluginManager::FindTargetPlugins(FPluginDiscoveryContext& Context)
{
	...
	if (!FParse::Param(FCommandLine::Get(), TEXT("NoEnginePlugins")))
	{
...

		{
			...
			UE_LOGF(LogPluginManager, Log, "Looking for enabled plugins target receipt");
...
#else
		{
			...
			for (const FStringView& TargetEnabledPlugin : std::initializer_list<FStringView>{UBT_TARGET_ENABLED_PLUGINS})
			{
		...
		}
		{
			...
			for (const FStringView& TargetDisabledPlugin : std::initializer_list<FStringView>{UBT_TARGET_DISABLED_PLUGINS})
			{
...
void FPluginManager::MarkEnabledPlugins(FPluginDiscoveryContext& Context)
{
	...
	for (TPair<FString, FPlugin*>& Pair : Context.EnabledPlugins)
	{
		...
		{
			UE_LOGF(LogPluginManager, Display, "Skipping load of '%ls'.", *Plugin.Name);
			continue;
		}
#endif // !IS_MONOLITHIC && !UE_MERGED_MODULES
...
void FPluginManager::ProcessEnabledPlugins(FPluginDiscoveryContext& Context)
{
	...
	ChangeTracker.bSkipLoadingAlreadyLoadedFiles = true;
	...
	for (TSharedRef<FPlugin> PluginPtr : PluginsArray)
	{
		...
		UE_LOGF(LogPluginManager, Log, "Mounting %ls plugin %ls", *EnumToString(Plugin.Type), *Plugin.GetName());
		UE_LOGF(LogPluginManager, Verbose, "Plugin path: %ls", *Plugin.FileName);
		...
		DynamicLayerPriority LayerPrio = Plugin.GetType() == EPluginType::Engine ? DynamicLayerPriority::EnginePlugin : DynamicLayerPriority::ProjectPlugin;
		FConfigCacheIni::RegisterPlugin(PluginName, Plugin.GetBaseDir(), Plugin.GetExtensionBaseDirs(), LayerPrio, bIncludePluginNameInBranchName);
			...
			if (Plugin.Descriptor.bCanContainContent)
			{
				...
				if (FPlatformProperties::RequiresCookedData())
				{
					...
					for (const FString& PakPath : FoundPaks)
					{
						...
						{
							UE_LOGF(LogPluginManager, Warning, "PAK file (%ls) could not be mounted because MountPak is not bound", *PakPath);
						}
					}
				}
			}
	...
	TMap<FString, FPlugin*>& EnabledPlugins, FStringView SourceOfPluginRequest, const TSet<FString>& AllowedOptionalDependencies)
{
		...
		case EConfigurePluginResultCode::PluginMissing:
			{
				...
				{
					UE_LOGF(LogPluginManager, Display, "Downloading '%ls' plugin from marketplace (%ls).", *ResultInfo.PluginReference->Name, *ResultInfo.PluginReference->MarketplaceURL);
					return false;
				}
				...
				{
					UE_LOGF(LogPluginManager, Display, "Disabled plugin '%ls' due to missing plugin '%ls', continuing.", *FirstReference.Name, *ResultInfo.PluginReference->Name);
					return true;
				}
			}
			break;
		...
		case EConfigurePluginResultCode::PluginSealed:
			{
				...
				{
					UE_LOGF(LogPluginManager, Display, "Disabled plugin '%ls' due to sealed dependency on plugin '%ls', continuing.", *FirstReference.Name, *ResultInfo.PluginReference->Name);
					return true;
				}
			}
			break;
		...
		case EConfigurePluginResultCode::DisallowedDependency:
			{
				...
				{
					UE_LOGF(LogPluginManager, Display, "Disabled plugin '%ls' due to disallowed dependency on plugin '%ls', continuing.", *FirstReference.Name, *ResultInfo.PluginReference->Name);
					return true;
				}
			}
			break;
	...
	if (bFatalError)
	{
		UE_LOGF(LogPluginManager, Fatal, "%ls", *ErrorMessage);
	}
	else
	{
		UE_LOGF(LogPluginManager, Error, "%ls", *ErrorMessage);
	}
	return false;
}
	...
	FConfigurePluginResultInfo& OutResultInfo)
{
	if (EnabledPlugins.Contains(FirstReference.Name))
	{
		...
		if (FirstReference.RequestedVersion.IsSet())
		{
			UE_CLOGF(
				...
				"Requested explicit version (v%d) of plugin '%ls', but a different version (v%d) was already enabled by another source.",
	...
	if (FirstReference.TargetsToExplicitlyLoad.Contains(TargetType))
	{
		UE_LOGF(LogPluginManager, Verbose, "Ignoring plugin '%ls' for being set to load-on-demand in project.", *FirstReference.Name);
		return true;
	}
	...
	for (int32 Idx = 0; Idx < NewPluginQueue.Num(); Idx++)
	{
		...
		{
			UE_LOGF(LogPluginManager, Verbose, "Ignoring plugin '%ls' for platform/configuration", *Reference.Name);
			continue;
		}
		...
		{
			UE_LOGF(LogPluginManager, Verbose, "Ignoring plugin '%ls' due to unsupported platform", *Reference.Name);
			continue;
		}
		...
		if (PluginPtr && Reference.RequestedVersion.IsSet())
		{
			...
			if (!FoundVersion)
			{
				UE_LOGF(LogPluginManager, Warning, "Failed to find specific version (v%d) of plugin '%ls'. Other versions exist, but the explicit version requested was missing.",
			...
			{
				UE_LOGF(LogPluginManager, Warning, "A different version of the %ls plugin (v%d : '%ls') is already enabled. Cannot switch to v%d (%ls) while another version is active.",
		...
		if (PluginPtr == nullptr)
		{
			...
			{
				UE_LOGF(LogPluginManager, Verbose, "Ignored optional reference to '%ls' plugin; plugin was not found.", *Reference.Name);
				continue;
			}
		...
		{
			UE_LOGF(LogPluginManager, Verbose, "Plugin '%ls' was disabled by platform.", *Reference.Name);
			continue;
		}
		...
		{
			UE_LOGF(LogPluginManager, Verbose, "Ignoring plugin '%ls' due to unsupported platform in plugin descriptor", *Reference.Name);
			continue;
		}
		...
		{
			UE_LOGF(LogPluginManager, Verbose, "Ignoring plugin '%ls' due to absence from the supported programs list", *Reference.Name);
			continue;
		}
		...
		{
			UE_LOGF(LogPluginManager, Verbose, "Ignoring plugin '%ls' due to not being an Enterpise project", *Reference.Name);
			continue;
		}

#if IS_MONOLITHIC || UE_MERGED_MODULES
		// In monolithic or merged-modular builds, if the plugin is optional, and any modules are not compiled in this build, do not load it. This makes content compiled with plugins enabled compatible with builds where they are disabled.
		if (Reference.bOptional)
		{
			...
			{
				UE_LOGF(LogPluginManager, Verbose, "Ignored optional reference to '%ls' plugin; plugin's modules were not found.", *Reference.Name);
				continue;
			}
		}
#endif // IS_MONOLITHIC || UE_MERGED_MODULES
		...
		{
			UE_LOGF(LogPluginManager, Verbose, "Ignored optional reference to '%ls' plugin; plugin was not built by target.", *Reference.Name);
			continue;
		}
#endif
		...
		for (const FPluginReferenceDescriptor& NextReference : Plugin.Descriptor.Plugins)
		{
			...
			{
				UE_LOGF(LogPluginManager, Verbose, "Ignored optional reference to '%ls' plugin from '%ls' plugin; plugin was not built by target.", *NextReference.Name, *Plugin.GetName());
				continue;
			}
#endif
...
bool FPluginManager::IsPluginCompatible(const FPlugin& Plugin)
{
	if (Plugin.Descriptor.EngineVersion.Len() > 0)
	{
		...
		{
			UE_LOGF(LogPluginManager, Warning, "Engine version string in %ls could not be parsed (\"%ls\")", *Plugin.FileName, *Plugin.Descriptor.EngineVersion);
			return true;
		}
		...
		{
			UE_LOGF(LogPluginManager, Warning, "Plugin '%ls' requires engine version '%ls' and may not be compatible with the current current engine version '%ls'", *Plugin.Name, *Plugin.Descriptor.EngineVersion, *FEngineVersion::CompatibleWith().ToString());
			return false;
		}
	}
	return true;
...
bool FPluginManager::TryLoadModulesForPlugin( const FPlugin& Plugin, const ELoadingPhase::Type LoadingPhase ) const
{
	...
	if( !FailureMessage.IsEmpty() )
	{
		if (bAllPluginsEnabledViaCommandLine)
		{
			UE_LOGF(LogPluginManager, Display, "%ls", *FailureMessage.ToString());
		}
		else
		{
			UE_LOGF(LogPluginManager, Error, "%ls", *FailureMessage.ToString());
		}
		if (!bAllPluginsEnabledViaCommandLine)
		{
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FPluginManager::LoadModulesForEnabledPlugins);
	...
	else
	{
		...
		if (bPreloadedBinaries && LoadingPhase == ELoadingPhase::PostConfigInit)
		{
			...
			for (const FDiscoveredPluginMap::ElementType& PluginPair : AllPlugins)
			{
				...
				{
					UE_LOGF(LogPluginManager, Display, "Skipping loading modules for %ls. Plugin is not enabled for current target.", *PluginPair.Key);
					continue;
				}
	...
	UE_CLOGF(LastCompletedLoadingPhase != ELoadingPhase::None && LastCompletedLoadingPhase > LoadingPhase,
		LogPluginManager, Error, "LoadModulesForEnabledPlugins called on phase %d after already being called on later phase %d.",
	...
	UE_CLOGF((LastCompletedLoadingPhase == ELoadingPhase::None && LoadingPhase != ELoadingPhase::EarliestPossible)
		|| (LastCompletedLoadingPhase != ELoadingPhase::None
			&& static_cast<int32>(LoadingPhase) > static_cast<int32>(LastCompletedLoadingPhase) + 1),
		LogPluginManager, Error,
		"LoadModulesForEnabledPlugins called on phase %d after last being called on phase %d, which skipped phase %d.",
		static_cast<int32>(LoadingPhase), static_cast<int32>(LastCompletedLoadingPhase),
...
bool FPluginManager::MountExplicitlyLoadedPluginLocalizationData(const FString& PluginName, TUniqueFunction<void(bool bLoadedLocalization, const FString& PluginName)>&& Notification)
{
	...
	if (!Plugin.IsValid() || !Plugin->bIsMounted)
	{
		...
		UE_LOGF(LogPluginManager, Error, "Cannot mount plugin localization for '%ls' as the plugin is unknown or not mounted. Did you forget to call MountExplicitlyLoadedPlugin?", *PluginName);
		if (Notification)
		{
	...
	{
		// Not supported
		UE_LOGF(LogPluginManager, Warning, "Cannot mount plugin localization for '%ls' as the plugin isn't explicitly loaded.", *PluginName);
		if (Notification)
		{
			Notification(false, PluginName);
		}
	...
	{
		// Already loaded
		UE_LOGF(LogPluginManager, Verbose, "Ignoring request to mount plugin localization for '%ls' as it was already referenced (ref-count: %d -> %d).", *PluginName, Plugin->ExplicitlyLoadedLocalizationDataMountedRefCount, Plugin->ExplicitlyLoadedLocalizationDataMountedRefCount + 1);
	...
	{
		// Nothing to load
		UE_LOGF(LogPluginManager, Verbose, "Ignoring request to mount plugin localization for '%ls' as the plugin has no localization targets defined.", *PluginName);
		if (Notification)
		{
			Notification(false, PluginName);
		}
	...
	UE_LOGF(LogPluginManager, Log, "Mounting plugin localization for '%ls'...", *PluginName);
...
bool FPluginManager::UnmountExplicitlyLoadedPluginLocalizationData(const FString& PluginName, TUniqueFunction<void(bool bUnloadedLocalization, const FString& PluginName)>&& Notification)
{
	...
	{
		// Does not exist or is not mounted
		UE_LOGF(LogPluginManager, Error, "Cannot unmount plugin localization for '%ls' as the plugin is unknown or not mounted. Did you forget to call MountExplicitlyLoadedPlugin?", *PluginName);
		if (Notification)
		{
			Notification(false, PluginName);
		}
	...
	{
		// Not supported
		UE_LOGF(LogPluginManager, Warning, "Cannot unmount plugin localization for '%ls' as the plugin isn't explicitly loaded.", *PluginName);
		if (Notification)
		{
			Notification(false, PluginName);
		}
	...
	if (Plugin->ExplicitlyLoadedLocalizationDataMountedRefCount > 1)
	{
		...
		UE_LOGF(LogPluginManager, Verbose, "Ignoring request to unmount plugin localization for '%ls' as it is still referenced (ref-count: %d -> %d).", *PluginName, Plugin->ExplicitlyLoadedLocalizationDataMountedRefCount, Plugin->ExplicitlyLoadedLocalizationDataMountedRefCount - 1);
		--Plugin->ExplicitlyLoadedLocalizationDataMountedRefCount;
		if (Notification)
		{
			Notification(false, PluginName);
	...
	{
		// Already unloaded
		UE_LOGF(LogPluginManager, Verbose, "Ignoring request to unmount plugin localization for '%ls' as the localization data was not mounted.", *PluginName);
		if (Notification)
		{
			Notification(false, PluginName);
		}
	...
	{
		// Nothing to load
		UE_LOGF(LogPluginManager, Verbose, "Ignoring request to unmount plugin localization for '%ls' as the plugin has no localization targets defined.", *PluginName);
		if (Notification)
		{
			Notification(false, PluginName);
		}
	...
	if (IsEngineExitRequested())
	{
		...
		UE_LOGF(LogPluginManager, Verbose, "Ignoring request to unmount plugin localization for '%ls' as the engine is exiting.", *PluginName);
		if (Notification)
		{
			Notification(false, PluginName);
		}
	...
	UE_LOGF(LogPluginManager, Log, "Unmounting plugin localization for '%ls'...", *PluginName);
...
bool FPluginManager::TryMountExplicitlyLoadedPluginVersion(TSharedRef<FPlugin>* AllPlugins_PluginPtr)
{
	...
	if (AllPlugins_PluginPtr && (*AllPlugins_PluginPtr)->Descriptor.bExplicitlyLoaded)
	{
		...
		{
			UE_LOGF(LogPluginManager, Error, "Cannot mount plugin '%ls' with another version (%ls) already mounted.", *(*AllPlugins_PluginPtr)->FileName, *CurrentPriorityPlugin->FileName);
		}
		else
		{
#if WITH_EDITOR
...
void FPluginManager::MountPluginFromExternalSource(const TSharedRef<FPlugin>& Plugin, ELoadingPhase::Type MaxLoadingPhase)
{
	...
	if (bHasContentOrVerse)
	{
		...
		{
			UE::PluginManager::Private::CoreUObjectPluginHandler->OnPluginLoad(*Plugin);
		}

		if (NewPluginMountedEvent.IsBound())
		{
			TRACE_CPUPROFILER_EVENT_SCOPE_STR("OnNewPluginMounted");
			NewPluginMountedEvent.Broadcast(*Plugin);
		}

		FString ContentDir = Plugin->GetContentDir();
		{ 
			TRACE_CPUPROFILER_EVENT_SCOPE_STR("RegisterMountPoint");
			RegisterMountPointDelegate.Execute(Plugin->GetMountedAssetPath(), ContentDir);
		}
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE_STR("Mount Localization Paths");
		TArray<FString> AdditionalLocResPaths;
		PluginLocalizationUtils::GetLocalizationPathsForPlugin(*Plugin, AdditionalLocResPaths);
		FTextLocalizationManager::Get().HandleLocalizationTargetsMounted(AdditionalLocResPaths);
	}
	...
	if (Plugin->Descriptor.Modules.Num() > 0)
	{
		TRACE_CPUPROFILER_EVENT_SCOPE_STR("Load Plugin Modules");
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE_STR("OnNewPluginContentMounted");
		NewPluginContentMountedEvent.Broadcast(*Plugin);
	}
...
FName FPluginManager::PackageNameFromModuleName(FName ModuleName)
{
	...
	for (FDiscoveredPluginMap::TIterator Iter(AllPlugins); Iter; ++Iter)
	{
		...
		for (int Idx = 0; Idx < Modules.Num(); Idx++)
		{
			...
			{
				UE_LOGF(LogPluginManager, Verbose, "Module %ls belongs to Plugin %ls and we assume that is the name of the package with the UObjects is /Script/%ls", *ModuleName.ToString(), *Plugin->Name, *Plugin->Name);
	...
	if (!Plugin->Descriptor.bIsPluginExtension)
	{
		for (const FModuleDescriptor& Module : Plugin->Descriptor.Modules)
		{
			if (const TSharedRef<IPlugin>* FoundPlugin = ModuleNameToPluginMap.Find(Module.Name))
			{
				...
				{
					UE_LOGF(LogPluginManager, Display, "Module %ls from plugin %ls is already associated with plugin %ls (maybe because the plugin should be using bIsPluginExtension)", *Module.Name.ToString(), *Plugin->GetName(), *(*FoundPlugin)->GetName());
				}
			}
			else
			{
