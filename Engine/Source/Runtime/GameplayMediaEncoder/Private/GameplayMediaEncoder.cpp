...
bool FGameplayMediaEncoder::RegisterListener(IGameplayMediaEncoderListener* Listener)
{
	...
	if(bFirstListener)
	{
		UE_LOGF(GameplayMediaEncoder, Log, "Registering the first listener");
		if(!Start())
		{
			return false;
		}
...
void FGameplayMediaEncoder::UnregisterListener(IGameplayMediaEncoderListener* Listener)
{
	...
	{
		UE_LOGF(GameplayMediaEncoder, Log, "Unregistered the last listener");
		Stop();
	}
}
...
bool FGameplayMediaEncoder::Initialize()
{
	...
	if(VideoEncoder)
	{
		UE_LOGF(GameplayMediaEncoder, Log, "Already initialized");
		return true;
	}
	...
	{
		UE_LOGF(GameplayMediaEncoder, Error, "No audio encoder for aac found");
		return false;
	}
	...
	if(!AudioEncoder)
	{
		UE_LOGF(GameplayMediaEncoder, Error, "Could not create audio encoder");
		return false;
	}
	...
	{
		UE_LOGF(GameplayMediaEncoder, Error, "Could not initialize audio encoder");
		return false;
	}
	...
	UE_LOGF(GameplayMediaEncoder, Log, "GameplayMediaEncoder.ResY = %d", VideoConfig.Height);
	...
	{
		UE_LOGF(GameplayMediaEncoder, Fatal, "GameplayMediaEncoder.ResY can only have a value of 720 or 1080");
		return false;
	}
	...
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	{
		...
		UE_LOGF(GameplayMediaEncoder, Log, "Uncapping FPS");
	}
	else
	{
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		VideoConfig.Framerate = FMath::Clamp(VideoConfig.Framerate, (uint32)MinVideoFPS, (uint32)MaxVideoFPS);
		UE_LOGF(GameplayMediaEncoder, Log, "Capping FPS %u", VideoConfig.Framerate);
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	if(GDynamicRHI)
	{
		...
		{
			PRAGMA_DISABLE_DEPRECATION_WARNINGS
			VideoEncoderInput = AVEncoder::FVideoEncoderInput::CreateForD3D11(GetID3D11DynamicRHI()->RHIGetDevice(), true, IsRHIDeviceAMD());
			PRAGMA_ENABLE_DEPRECATION_WARNINGS
		}
		else if (RHIType == ERHIInterfaceType::D3D12)
		{
			PRAGMA_DISABLE_DEPRECATION_WARNINGS
			VideoEncoderInput = AVEncoder::FVideoEncoderInput::CreateForD3D12(GetID3D12DynamicRHI()->RHIGetDevice_NoMGPU(), true, IsRHIDeviceNVIDIA());
			PRAGMA_ENABLE_DEPRECATION_WARNINGS
		}
		else if (RHIType == ERHIInterfaceType::Vulkan)
		{
			PRAGMA_DISABLE_DEPRECATION_WARNINGS
			VideoEncoderInput = AVEncoder::FVideoEncoderInput::CreateForVulkan(GetIVulkanDynamicRHI()->RHIGetVkDevice(), true);
			PRAGMA_ENABLE_DEPRECATION_WARNINGS
		}
		else
#endif
		{
			UE_LOGF(GameplayMediaEncoder, Error, "Video encoding is not supported with the current Platform/RHI combo.");
			return false;
		}
	}
	...
	{
		UE_LOGF(GameplayMediaEncoder, Error, "No video encoders found. Check if relevent encoder plugins have been enabled for this project.");
		return false;
	}
	...
	if (!VideoEncoder)
	{
		UE_LOGF(GameplayMediaEncoder, Error, "No H264 video encoder found. Check if relevent encoder plugins have been enabled for this project.");
		return false;
	}
	...
	if(!VideoEncoder)
	{
		UE_LOGF(GameplayMediaEncoder, Fatal, "Creating video encoder failed.");
		return false;
	}
...
bool FGameplayMediaEncoder::Start()
{
	if(StartTime != 0)
	{
		UE_LOGF(GameplayMediaEncoder, Log, "Already running");
		return true;
	}

	if(!VideoEncoder)
	{
		UE_LOGF(GameplayMediaEncoder, Log, "Not initialized yet , so also performing a Intialize()");
		if(!Initialize())
		{
			return false;
		}
...
void FGameplayMediaEncoder::Stop()
{
	...
	if(StartTime == 0)
	{
		UE_LOGF(GameplayMediaEncoder, Log, "Not running");
		return;
	}
...
void FGameplayMediaEncoder::Shutdown()
{
	if(StartTime != 0)
	{
		UE_LOGF(GameplayMediaEncoder, Log, "Currently running, so also performing a Stop()");
		Stop();
	}
...
void FGameplayMediaEncoder::OnNewSubmixBuffer(const USoundSubmix* OwningSubmix, float* AudioData, int32 NumSamples, int32 NumChannels, const int32 SampleRate, double /*AudioClock*/)
{
	...
	if(SampleRate != HardcodedAudioSamplerate)
	{
		...
		if(!bAudioFormatChecked)
		{
			...
			UE_LOGF(GameplayMediaEncoder, Error, "Audio SampleRate needs to be %d HZ, current value is %d. VideoRecordingSystem won't record audio", HardcodedAudioSamplerate, SampleRate);
		}
		return;
	}
...
void FGameplayMediaEncoder::OnFrameBufferReady(SWindow& SlateWindow, ISlateViewportProvider& ViewportProvider)
{
	CSV_SCOPED_TIMING_STAT(GameplayMediaEncoder, OnBackBufferReady);
	check(IsInRenderingThread());
	ProcessVideoFrame(ViewportProvider.GetBackBufferResource());
}
...
void FGameplayMediaEncoder::ProcessAudioFrame(const float* AudioData, int32 NumSamples, int32 NumChannels, int32 SampleRate)
{
	...
	{
		UE_LOGF(GameplayMediaEncoder, Warning, "Audio clock falling behind real time clock by %.3f seconds. Ajusting audio clock", Now.GetTotalSeconds() - AudioClock);
...
void FGameplayMediaEncoder::ProcessVideoFrame(const FTextureRHIRef& FrameBuffer)
{
	...
	if(bDoFrameSkipping)
	{
		...
		{
			UE_LOGF(GameplayMediaEncoder, Verbose, "Framerate control dropped captured frame");
			return;
		}
	}
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
{
	...
	if(!BackBuffers.Contains(InputFrame))
	{
		...
		if (RHIType == ERHIInterfaceType::D3D11)
		{
			FTextureRHIRef Texture = FRHICommandListImmediate::Get().CreateTexture(Desc);
		...
		UE_LOGF(LogTemp, Log, "%d backbuffers currently allocated", BackBuffers.Num());
#else
		unimplemented();
