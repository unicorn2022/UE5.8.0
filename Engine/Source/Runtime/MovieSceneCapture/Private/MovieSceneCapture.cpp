...
const FName UMovieSceneCapture::MovieSceneCaptureUIName = FName("MovieSceneCaptureUIInstance");
...
void UMovieSceneCapture::Initialize(TSharedPtr<FSceneViewport> InSceneViewport, int32 PIEInstance)
{
	...
	// Apply command-line overrides
	{
		...
		if( FParse::Value( FCommandLine::Get(), TEXT( "-MovieFolder=" ), OutputPathOverride ) )
		{
			...
			if (!Settings.OutputDirectory.Path.Contains(TEXT("{")))
			{
				if (!IFileManager::Get().DirectoryExists(*Settings.OutputDirectory.Path))
				{
					...
					{
						UE_LOGF(LogMovieSceneCapture, Error, "Invalid output directory: %ls.", *Settings.OutputDirectory.Path);
					}
				}
				else if (IFileManager::Get().IsReadOnly(*Settings.OutputDirectory.Path))
				{
					UE_LOGF(LogMovieSceneCapture, Error, "Read only output directory: %ls.", *Settings.OutputDirectory.Path);
				}
			}
		}
			...
			|| FParse::Value( FCommandLine::Get(), TEXT( "-ImageCaptureProtocol=" ), ImageProtocolOverrideString ) )
		{
			...
			if (!OverrideClass)
			{
				UE_LOGF(LogMovieSceneCapture, Error, "Unrecognized image capture type (-MovieFormat or -ImageCaptureProtocol): %ls.", *ImageProtocolOverrideString);
			}
			else
			{
		...
		if (FParse::Value( FCommandLine::Get(), TEXT( "-AudioCaptureProtocol=" ), AudioProtocolOverrideString ) )
		{
			...
			if (!OverrideClass)
			{
				UE_LOGF(LogMovieSceneCapture, Error, "Unrecognized audio capture type (-AudioCaptureProtocol): %ls.", *AudioProtocolOverrideString);
			}
			else
			{
		...
		if ( FParse::Value( FCommandLine::Get(), TEXT( "-MovieFrameRate=" ), FrameRateOverrideString ) )
		{
			...
			{
				UE_LOGF(LogMovieSceneCapture, Error, "Unrecognized capture frame rate: %ls. Defaulting to sequence frame rate.", *FrameRateOverrideString);
			}
			else
			{
...
void UMovieSceneCapture::CaptureThisFrame(float DeltaSeconds)
{
	...
	if (CaptureStrategy->ShouldPresent(CachedMetrics.ElapsedSeconds, CachedMetrics.Frame))
	{
		...
		UE_LOGF(LogMovieSceneCapture, Verbose, "Captured frame: %d", CachedMetrics.Frame);
		++CachedMetrics.Frame;
	}
}
...
void FFixedTimeStepCaptureStrategy::OnStop()
{
	...
	{
		UE_LOGF(LogTemp, Warning, "FApp::CurrentTime() is ahead of platform time due to Fixed Timestep. Fixing. AppCurrentTime: %f PlatformCurrentTime: %f", FApp::GetCurrentTime(), FPlatformTime::Seconds());
