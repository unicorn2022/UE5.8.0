...
void UImageSequenceProtocol::OnLoadConfigImpl(FMovieSceneCaptureSettings& InSettings)
{
	...
	if (!bHasFrameFormat)
	{
		...
		UE_LOGF(LogMovieSceneCapture, Display, "Automatically appended .{frame} to the format string as specified format string did not provide a way to differentiate between frames via {frame} or {shot_frame}!");
	}
...
{
	{
		...
		if (FParse::Value(FCommandLine::Get(), TEXT("-CaptureGamut="), CaptureGamutString))
		{
			...
			{
				UE_LOGF(LogMovieSceneCapture, Warning, "The value for the command -CaptureGamut is invalid, using default value instead!")
			}
		}
	}
