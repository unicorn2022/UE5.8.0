...
public:
	...
	virtual bool StartCaptureImpl() override
	{
		...
		{
			UE_LOGF(LogTemp, Warning, "Audio Recording Resumed");
	...
	virtual void PauseCaptureImpl() override
	{
		...
		UE_LOGF(LogTemp, Warning, "Audio Recording Paused. Adding: %f seconds to GameRecording. Adding: %f seconds to Platform Recording.",
	...
	virtual void BeginFinalizeImpl() override
	{
		...
		if(!FMath::IsNearlyZero(Difference, 0.05))
		{
			...
			UE_LOGF(LogMovieSceneCapture, Warning, "Game Time is out of sync with Platform Time during audio recording. This is usually an indication that the sequence could not play back at full speed, and audio will most likely be desynchronized. Platform Time took %f seconds longer than Game Time.", Difference);
		}
	}
