...
FLazyName UMediaPlayer::MediaInfoNameVideoDecoderProvider(TEXT("VideoDecoderProvider"));
FLazyName UMediaPlayer::MediaInfoNameAudioDecoderProvider(TEXT("AudioDecoderProvider"));
...
bool UMediaPlayer::CanPlaySource(UMediaSource* MediaSource)
{
	...
	UE_LOGF(LogMediaAssets, Verbose, "%ls.CanPlaySource %ls", *GetFName().ToString(), *MediaSource->GetFName().ToString());
...
{
	UE_LOGF(LogMediaAssets, Verbose, "%ls.CanPlayUrl %ls", *GetFName().ToString(), *Url);
...
{
	UE_LOGF(LogMediaAssets, VeryVerbose, "%ls.Close", *GetFName().ToString());
		...
		for(const TPair<FString, TArray<TUniquePtr<IMediaMetadataItem>>>& Data : *MetadataPtr)
		{
			...
			for(const TUniquePtr<IMediaMetadataItem>& Item : Data.Value)
			{
				if(const IMediaMetadataItem* ItemPtr = Item.Get())
				{
					...
					default:
						UE_LOGF(LogMediaAssets, Warning, "GetMediaMetadataItems() unhandled FVariant type!");
					}
...
TOptional<FTimespan> UMediaPlayer::GetTrackDuration(EMediaPlayerTrack TrackType, int32 TrackIndex) const
{
	return PlayerFacade->GetTrackDuration((EMediaTrackType)TrackType, TrackIndex);
}
...
{
	UE_LOGF(LogMediaAssets, Verbose, "%ls.Next", *GetFName().ToString());
...
bool UMediaPlayer::OpenFile(const FString& FilePath)
{
	...
	UE_LOGF(LogMediaAssets, Verbose, "%ls.OpenFile %ls", *GetFName().ToString(), *FilePath);
...
bool UMediaPlayer::OpenPlaylistIndex(UMediaPlaylist* InPlaylist, int32 Index)
{
	...
	{
		UE_LOGF(LogMediaAssets, Warning, "%ls.OpenPlaylistIndex called with null MediaPlaylist", *GetFName().ToString());
		return false;
	}

	UE_LOGF(LogMediaAssets, Verbose, "%ls.OpenSource %ls %i", *GetFName().ToString(), *InPlaylist->GetFName().ToString(), Index);
	...
	{
		UE_LOGF(LogMediaAssets, Warning, "%ls.OpenPlaylistIndex called with invalid PlaylistIndex %i", *GetFName().ToString(), Index);
		return false;
	}
	...
	{
		UE_LOGF(LogMediaAssets, Error, "Failed to validate media source %ls (%ls)", *MediaSource->GetName(), *MediaSource->GetUrl());
		return false;
	}
...
bool UMediaPlayer::OpenSourceInternal(UMediaSource* MediaSource, const FMediaPlayerOptions* PlayerOptions)
{
	...
	if (MediaSource == nullptr)
	{
		UE_LOGF(LogMediaAssets, Warning, "%ls.OpenSource called with null MediaSource", *GetFName().ToString());
		return false;
	}

	UE_LOGF(LogMediaAssets, Verbose, "%ls.OpenSource %ls", *GetFName().ToString(), *MediaSource->GetFName().ToString());
	...
	if (Playlist == nullptr)
	{
		UE_LOGF(LogMediaAssets, Error, "Failed to create playlist on opening media source %ls (%ls)", *MediaSource->GetName(), *MediaSource->GetUrl());
		return false;
	}
...
bool UMediaPlayer::OpenUrl(const FString& Url)
{
	...
	UE_LOGF(LogMediaAssets, Verbose, "%ls.OpenUrl %ls", *GetFName().ToString(), *Url);
...
{
	UE_LOGF(LogMediaAssets, Verbose, "%ls.Pause", *GetFName().ToString());
...
{
	UE_LOGF(LogMediaAssets, Verbose, "%ls.Play", *GetFName().ToString());
...
{
	UE_LOGF(LogMediaAssets, Verbose, "%ls.Previous", *GetFName().ToString());
...
{
	UE_LOGF(LogMediaAssets, Verbose, "%ls.Reopen", *GetFName().ToString());
	if (Playlist == nullptr)
	{
		return false;
	}
...
{
	UE_LOGF(LogMediaAssets, Verbose, "%ls.Rewind", *GetFName().ToString());
...
{
	UE_LOGF(LogMediaAssets, VeryVerbose, "%ls.Seek %ls", *GetFName().ToString(), *Time.ToString(TEXT("%h:%m:%s.%t")));
...
bool UMediaPlayer::Scrub(const FTimespan& Time)
{
	...
	{
		UE_LOGF(LogMediaAssets, VeryVerbose, "%ls.Scrub %ls", *GetFName().ToString(), *Time.ToString(TEXT("%h:%m:%s.%t")));
...
{
	UE_LOGF(LogMediaAssets, Verbose, "%ls.SelectTrack %ls %i", *GetFName().ToString(), *UEnum::GetValueAsString(TEXT("MediaAssets.EMediaPlayerTrack"), TrackType), TrackIndex);
...
{
	UE_LOGF(LogMediaAssets, VeryVerbose, "%ls.SetBlockOnTime %ls", *GetFName().ToString(), *Time.ToString(TEXT("%h:%m:%s.%t")));
...
{
	UE_LOGF(LogMediaAssets, VeryVerbose, "%ls.SetBlockOnRange [%ls, %ls]", *GetFName().ToString(),
...
{
	UE_LOGF(LogMediaAssets, Verbose, "%ls.SetDesiredPlayerName %ls", *GetFName().ToString(), *PlayerName.ToString());
...
bool UMediaPlayer::SetLooping(bool Looping)
{
	...
	UE_LOGF(LogMediaAssets, Verbose, "%ls.SetLooping %ls", *GetFName().ToString(), *(Looping ? CoreTexts.True : CoreTexts.False).ToString());
...
{
	UE_LOGF(LogMediaAssets, Verbose, "%ls.SetMediaOptions", *GetFName().ToString());
...
{
	UE_LOGF(LogMediaAssets, Verbose, "%ls.SetRate %f", *GetFName().ToString(), Rate);
...
{
	UE_LOGF(LogMediaAssets, Verbose, "%ls.SetTrackFormat %ls %i %i", *GetFName().ToString(), *UEnum::GetValueAsString(TEXT("MediaAssets.EMediaPlayerTrack"), TrackType), TrackIndex, FormatIndex);
...
{
	UE_LOGF(LogMediaAssets, Verbose, "%ls.SetVideoTrackFrameRate %i %i %f", *GetFName().ToString(), TrackIndex, FormatIndex, FrameRate);
...
bool UMediaPlayer::SetViewField(float Horizontal, float Vertical, bool Absolute)
{
	...
	UE_LOGF(LogMediaAssets, Verbose, "%ls.SetViewField %f %f %ls", *GetFName().ToString(), Horizontal, Vertical, *(Absolute ? CoreTexts.True : CoreTexts.False).ToString());
...
bool UMediaPlayer::SetViewRotation(const FRotator& Rotation, bool Absolute)
{
	...
	UE_LOGF(LogMediaAssets, Verbose, "%ls.SetViewRotation %ls %ls", *GetFName().ToString(), *Rotation.ToString(), *(Absolute ? CoreTexts.True : CoreTexts.False).ToString());
...
{
	UE_LOGF(LogMediaAssets, Verbose, "%ls.SetTimeDelay %ls", *GetFName().ToString(), *InTimeDelay.ToString());
...
{
	UE_LOGF(LogMediaAssets, VeryVerbose, "%ls.SetPlaybackTimeRange [%ls, %ls]", *GetFName().ToString(),
...
{
	UE_LOGF(LogMediaAssets, Verbose, "%ls.PausePIE", *GetFName().ToString());
...
{
	UE_LOGF(LogMediaAssets, Verbose, "%ls.ResumePIE", *GetFName().ToString());
...
void UMediaPlayer::RegisterWithMediaModule()
{
	...
	{
		UE_LOGF(LogMediaAssets, Warning, "Failed to register media player '%ls' due to module 'Media' not being loaded yet.", *GetName());
	}
}
...
public:
		...
		, Options(InOptions)
	{
		if (InMediaSource)
		{
			...
			{
				UE_LOGF(LogMediaAssets, Warning, "Open Media Latent: Failed initial open: %ls", *URL);
				bSawError = true;
			}
		}
		else
		{
			UE_LOGF(LogMediaAssets, Warning, "Open Media Latent: Failed initial open because no media source given");
			bSawError = true;
		}
	}
	...
	{
		UE_LOGF(LogMediaAssets, Verbose, "Open Media Latent: Saw event: %ls", *MediaUtils::EventToString(Event));
	...
	virtual void UpdateOperation(FLatentResponse& Response) override
	{
		...
		{
			UE_LOGF(LogMediaAssets, Warning, "Open Media Latent: Saw media open failed event. %ls", *URL);
		...
		{
			UE_LOGF(LogMediaAssets, Warning, "Open Media Latent: Media player object was deleted. %ls", *URL);
			FailedOperation(Response);
			return;
		}
		...
		{
			UE_LOGF(LogMediaAssets, Warning, "Open Media Latent: Media player is in Error state. %ls", *URL);
			FailedOperation(Response);
			return;
		}
		...
		{
			UE_LOGF(LogMediaAssets, Warning, "Open Media Latent: Media player is closed. %ls", *URL);
			FailedOperation(Response);
			return;
		}
		...
		{
			UE_LOGF(LogMediaAssets, Verbose, "Open Media Latent: Is preparing ...");
		}
		else if (MediaPlayer->IsReady())
		{
			UE_LOGF(LogMediaAssets, Verbose, "Open Media Latent: IsReady() ... %ls", *URL);
			...
			if (bSawMediaOpened)
			{
				if (!Options.SeekTime.IsZero())
				{
					...
					{
						UE_LOGF(LogMediaAssets, Verbose, "Open Media Latent: Triggering output pin after seek completed. Success: %d, %ls", OutSuccess, *URL);
						OutSuccess = true;
						Response.FinishAndTriggerIf(true, ExecutionFunction, OutputLink, CallbackTarget);
						return;
					}
					else
					{
						...
						{
							UE_LOGF(LogMediaAssets, Warning, "Open Media Latent: Media player seeking to time out of bounds. Seek: %ls, Duration: %ls, URL: %ls",
								*Options.SeekTime.ToString(), *MediaPlayer->GetDuration().ToString(), *URL);
							FailedOperation(Response);
							return;
						}
						UE_LOGF(LogMediaAssets, Verbose, "Open Media Latent: Waiting for seek completed event ...");
					}
				}
				else
				{
					UE_LOGF(LogMediaAssets, Verbose, "Open Media Latent: Triggering output pin after opened event. Success: %d, %ls", OutSuccess, *URL);
					OutSuccess = true;
					Response.FinishAndTriggerIf(true, ExecutionFunction, OutputLink, CallbackTarget);
					return;
				}
			...
			{
				UE_LOGF(LogMediaAssets, Verbose, "Open Media Latent: Waiting for opened event ...");
			}
		}
		else
		{
			UE_LOGF(LogMediaAssets, Verbose, "Open Media Latent: Waiting for IsReady() ...");
		}
		...
		{
			UE_LOGF(LogMediaAssets, Warning, "Open Media Latent: Timed out. %ls", *URL);
			FailedOperation(Response);
			return;
		}
	}
