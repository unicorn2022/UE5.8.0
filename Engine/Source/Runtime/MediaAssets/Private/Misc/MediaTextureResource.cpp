...
static int32 GMediaCaptureTexture = 0;
static FAutoConsoleVariableRef CVarMediaTextureTriggerCapture(
	TEXT("r.Media.CaptureTexture"),
	GMediaCaptureTexture,
	TEXT("Trigger a GPU capture for the media texture resource update (debug/development builds only).")
);
#endif
...
{
	/** Calculate the maximum number of mips for the given texture dimension in pixels. */
	static int32 CalcMaxNumMips(const FIntPoint& InDim)
	{
		const int32 MaxDimension = FMath::Max(InDim.X, InDim.Y);

		// FloorLog2NonZero requires a positive non-zero value.
		if (MaxDimension <= 0)
		{
			return 1;
		}

		const uint32 NumMips = 1u + FMath::FloorLog2NonZero(static_cast<uint32>(MaxDimension));
		return static_cast<int32>(NumMips);
	}
	...
	uint8 ClampNumMips(uint8 InNumMips, const FIntPoint& InDim)
	{
		if (InNumMips > 1u)
		{
			const uint8 MaxNumMips = static_cast<uint8>(FMath::Min(GMaxTextureMipCount, CalcMaxNumMips(InDim)));
...
namespace MediaTextureResourceHelpers
{
	...
	/**
	 * Helper function to calculate the stride of the sample buffer
	 * using mip0's stride up to the point where there is 1 pixel left.
	 */
	uint32 CalcSampleStrideForMipLevel(const TSharedPtr<IMediaTextureSample, ESPMode::ThreadSafe>& InSample, uint8 InMipLevel)
	{
		const uint32 MipStride = InSample->GetStride() >> InMipLevel;
		if (MipStride > 0)
		{
			return MipStride;
		}
		
		// Calculate the smallest stride for this format (for 1 pixel).
		const EPixelFormat PixelFormat = GetPixelFormat(InSample);
		const FPixelFormatInfo& PixelFormatInfo = GPixelFormats[PixelFormat];
		return PixelFormatInfo.GetBlockCountForWidth(1) * PixelFormatInfo.BlockBytes;
	}

	/**
	 * Updates the destination RHI texture from the sample's memory buffers. 
	 */
	void UpdateTextureFromSample(
		FRHICommandListImmediate& InRhiCmdList,
		const TSharedPtr<IMediaTextureSample, ESPMode::ThreadSafe>& InSample,
		uint8 InNumMips,
		const TRefCountPtr<FRHITexture>& InDestTexture)
	{
		const uint8* Data = (const uint8*)InSample->GetBuffer();
		check(Data);
		const FPixelFormatInfo& FormatInfo = GPixelFormats[GetPixelFormat(InSample)];
		const FIntPoint SampleDim = InSample->GetDim();

		for (uint8 MipLevel = 0; MipLevel < InNumMips; ++MipLevel)
		{
			const uint32 Width = FMath::Max(1, SampleDim.X >> MipLevel);
			const uint32 Height = FMath::Max(1, SampleDim.Y >> MipLevel);
			const uint32 Stride = CalcSampleStrideForMipLevel(InSample, MipLevel);

			// Don't let a zero stride go through rhi, it causes a delayed error that is hard to track.
			if (!ensure(Stride != 0))
			{
				break;
			}
			...
			InRhiCmdList.UpdateTexture2D(InDestTexture, MipLevel, Region, Stride, Data);

			const uint32 HeightUnit = (Height + FormatInfo.BlockSizeY - 1) / FormatInfo.BlockSizeY;
...
#if UE_BUILD_DEBUG || UE_BUILD_DEVELOPMENT
			// Placed inside the UseSample branch to guarantee the capture of a valid frame sample update
			RenderCaptureInterface::FScopedCapture RenderCapture(GMediaCaptureTexture > 0, &RHICmdList, TEXT("FMediaTextureResource::Render"));
			GMediaCaptureTexture = 0;
#endif
			...
			if (UseSample)
			{
				...
				if (IMediaTextureSampleColorConverter* Converter = Sample->GetMediaTextureSampleColorConverter())
				{
					...
					if (bRecreateOutputTarget && TextureRef)
					{
						...
						OutputTarget->SetName(TEXT("MediaTextureColorConversionOutput"));
				...
				// Notify extensions the media texture has been updated with a new sample.
				FMediaDelegates::OnSampleRendered_RenderThread.Broadcast(this, Sample);
			}
		}
	}
...
bool FMediaTextureResource::RequiresConversion(const TSharedPtr<IMediaTextureSample, ESPMode::ThreadSafe>& Sample, uint8 InTargetNumMips) const
{
	...
	// Any reason color conversion must run?
	...
	{
		// Color space mismatch with the working color space.
		if (!Sample->GetSourceColorSpace().Equals(DestinationCS))
		{
			return true;
		}

		// Non-unity HDR nits normalization is applied as a scale on the CSTransform inside
		// GetColorSpaceConversionMatrixForSample, so the conversion pass must run even when
		// the color space and encoding would otherwise allow us to skip it.
		const float NormalizationFactor = Sample->GetHDRNitsNormalizationFactor();
...
void FMediaTextureResource::ConvertSample(FRHICommandListImmediate& RHICmdList, const TSharedPtr<IMediaTextureSample, ESPMode::ThreadSafe>& Sample, const FLinearColor& ClearColor, uint8 InTargetNumMips)
{
	...
	FRHITexture* InputTexture = nullptr;
	{
		...
		else
		{
			...
			MediaTextureResourceHelpers::UpdateTextureFromSample(RHICmdList, Sample, SampleNumMips, InputTarget);
	...
	else
	{
		...
		else
		{
			...
			if (!Converter || !Converter->Convert(RHICmdList, RenderTargetTextureRHI, Hints))
			{
				...
				MediaTextureResourceHelpers::UpdateTextureFromSample(RHICmdList, Sample, SampleNumMips, RenderTargetTextureRHI);
			}
