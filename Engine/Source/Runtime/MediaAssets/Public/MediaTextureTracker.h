...
// Workaround to avoid having a comma in the type for the multicast macro.
using FMediaTextureTrackerObjectPtr = TSharedPtr<FMediaTextureTrackerObject, ESPMode::ThreadSafe>;

/** Fires on register/unregister of a tracker object. Game thread only. Texture may be null on unregister if GC'd. */
DECLARE_MULTICAST_DELEGATE_TwoParams(FOnMediaTextureTrackerObjectsChanged, TObjectPtr<UMediaTexture> /*Texture*/, const FMediaTextureTrackerObjectPtr& /*Info*/);
...
struct FMediaTextureTrackerObject
{
	...
	/** Optional consumer-supplied tile-visibility provider; takes precedence over VisibleMipsTilesCalculations when set. */
	TSharedPtr<IMediaTileVisibilityProvider, ESPMode::ThreadSafe> TileVisibilityProvider;
};
...
public:
	...
	virtual ~FMediaTextureTracker() = default;
	 ...
	 *
	 * @param InInfo Object that has the texture.
	 * @param InTexture Texture to unregister. May be null if the caller has lost the
	 *        pointer (e.g. the texture was garbage-collected before the owner was destroyed);
	 *        in that case the tracker scans for InInfo across all registered textures.
	 */
	...
	const TArray<TWeakObjectPtr<UMediaTexture>>& GetTextures()
	{
		...
		return MediaTextures;
	}

	/** Delegate fired after a tracker object has been added for a texture. Subscribe to pick up late registrations. */
	FOnMediaTextureTrackerObjectsChanged& OnObjectRegistered()
	{
		check(IsInGameThread());
		return ObjectRegisteredDelegate;
	}

	/** Delegate fired after a tracker object has been removed for a texture. */
	FOnMediaTextureTrackerObjectsChanged& OnObjectUnregistered()
	{
		check(IsInGameThread());
		return ObjectUnregisteredDelegate;
	}
...
protected:
	...
	FOnMediaTextureTrackerObjectsChanged ObjectRegisteredDelegate;
	FOnMediaTextureTrackerObjectsChanged ObjectUnregisteredDelegate;
};
