...
public:
	...
	/**
	 * Gets the duration of a specific track, if available.
	 * Availability depends on the media format and player implementation.
	 * If unset only the overall media duration can be used.
	 *
	 * @return An optional time span representing the duration.
	 * @see GetDuration
	 */
	MEDIAASSETS_API TOptional<FTimespan> GetTrackDuration(EMediaPlayerTrack TrackType, int32 TrackIndex) const;
...
public:
	...
	/** Video decoder provider. String. */
	static MEDIAASSETS_API FLazyName MediaInfoNameVideoDecoderProvider;
	/** Audio decoder provider. String. */
	static MEDIAASSETS_API FLazyName MediaInfoNameAudioDecoderProvider;
