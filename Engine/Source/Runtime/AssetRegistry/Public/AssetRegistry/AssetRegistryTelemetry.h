...
namespace UE::Telemetry::AssetRegistry
{
    ...
    // Information about asset registry scanning either as the initial scan when the process starts,
    // or when an additional mount point is added. See the bIsInitialScan field to disambiguate.
    // Note that for subsequent scans, the times other than TotalSearchDurationSeconds will be cumulative totals including previous work.
    ...
    struct FGatherTelemetry
    {
        ...
        // True if this was the initial startup scan - this should be true only once per process.        
        bool bIsInitialScan = false;
    };
