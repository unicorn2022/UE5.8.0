...
struct FAssetRegistryLoadOptions
{
	...
	/**
	 *  If non-empty, called on every FAssetData, AssetPackageData, and FDependsNode loaded from
	 * disk, keeping them only if ShouldKeepPackage returns true on their packagename.
	 */
	TFunction<bool(FName PackageName)> ShouldKeepPackage;
	/**
	 * Optional pointer to the FStore previously created by Load, in cases where the same AssetRegistry.bin
	 * file is being reloaded. Caution: If this Store is not identical to the one that would be loaded from disk,
	 * chaotic crashes can occur during loading or afterwards during reading of AsssetDatas.
	 * If non-null, this FStore will be used for all TagData lookups and the serialization of the bytes for it
	 * on disk will be skipped.
	 */
	...
	/** 
	 * If true, AssetDatas, AssetPackageDatas and DependsNodes will be individually allocated rather than allocated in a single block,
	 * so that they can be individually freed when their mountpoint is unloaded.
	 */
	bool bMountPointsRequireSeparateAllocationBlocks = false;
};

/** Extra data about the load of an AssetRegistryState from disk that is optionally returned from Load. */
struct FAssetRegistryLoadResults
{
...
public:
	...
	/** Removes existing package datas, passed in as FAssetIdentifier(PackageName) for internal performance. */
	ASSETREGISTRY_API bool RemovePackageDatas(const TSet<FAssetIdentifier>& PackageNames);
	...
	/**
	 * Similar to InitializeFromExisting, but it moves the allocations over from InState into *this rather than
	 * copying them. It also shares the InState's existing FStore (if any) rather than creating a new one.
	 * This function is used for faster consumption of states loaded internally by the AssetRegistry.
	 * This function is less general than InitializeFromExisting to reduce complexity; it only supports
	 * EInitializationMode::OnlyUpdateNew and does not support filter flags on the input Options; all instances
	 * of FAssetDatas, FAssetPackageDatas, and FDependsNodes are kept
	 * The InState is reset to empty by this function.
	 */
	void ConsumeExisting(FAssetRegistryState&& Existing, const FAssetRegistrySerializationOptions& Options,
	 ...
	 * This is more efficient than calling InitializeFromExisting and then PruneAssetData.
	 *
	...
	/** Filter a given AssetData's Tags to remove Tags that are filtered out by the filtering rules in Options. */
	ASSETREGISTRY_API static void FilterTags(FAssetData& InOutAssetData,
		const FAssetRegistrySerializationOptions& Options);
...
private:
	...
	void SetAssetDatas(TConstArrayView<FAssetData*> AssetDatas, const FAssetRegistryLoadOptions& Options);
	...
	/** Removes the depends node and updates the dependencies to no longer contain it as a referencer. */
	ASSETREGISTRY_API bool RemoveDependsNode(const FAssetIdentifier& Identifier);

	enum class ERemoveDependsFlags
	{
		None,
		/**
		 * If passed in, RemoveDependsNode functions will remove nodes that were not in the remove request
		 * but that had links to the removed nodes and no longer have any links after the removals.
		 */
		RemoveOrphans = 1 << 0,
	};
	FRIEND_ENUM_CLASS_FLAGS(ERemoveDependsFlags);

	/** Removes a set of depends nodes and updates their dependencies to no longer contain them as a referencer. */
	ASSETREGISTRY_API bool RemoveDependsNodes(const TSet<FAssetIdentifier>& Identifiers,
		ERemoveDependsFlags Flags = ERemoveDependsFlags::None);
	...
	void LoadDependenciesImpl(FArchive& Ar, const FAssetRegistryLoadOptions& Options, FAssetRegistryVersion::Type Version);
...
ENUM_CLASS_FLAGS(FAssetRegistryState::ERemoveDependsFlags);
