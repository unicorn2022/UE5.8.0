...
public:
	 ...
	 * @param bFailIfLockHeld If the AssetRegistry is currently being written to by another thread this 
	 *	      function will return EExists::Unknown rather than waiting for safe read access.
	 * @return Enum return code
	 */
	virtual UE::AssetRegistry::EExists TryGetAssetPackageData(FName PackageName,
		FAssetPackageData& OutAssetPackageData, bool bFailIfLockHeld = false) const = 0;
	 ...
	 */
	virtual UE::AssetRegistry::EExists TryGetAssetPackageData(FName PackageName,
		FAssetPackageData& OutAssetPackageData, FName& OutCorrectCasePackageName, bool bFailIfLockHeld = false) const = 0;
	...
	* or cache writing has been disabled such as when using -NoAssetRegistryCacheWrite. */
	...
	virtual void AppendState(const FAssetRegistryState& InState,
		UE::AssetRegistry::EAppendMode AppendMode = UE::AssetRegistry::EAppendMode::Append) = 0;

	// ActiveMountsLoading: save memory when using cooked AssetRegistries by removing AssetRegistryData
	// for MountPoints that are unmounted. Only beneficial if your game unloads mountpoints during some periods,
	// and only usable if your game does not require knowledge of the assets in those mountpoints when unloaded.
	// Currently only enablable for ClientOnly, since ClientOnly is the configuration that runs on the most memory
	// constrained platforms. Enable by setting Engine.ini:[AssetRegistry]:bActiveMountsLoadingEnabled_Client=true,
	// and calling IAssetRegistry->ActiveMountsInitialize() when ready to start unloading at startup.
	// By default, ActiveMountsLoading runs in auto mode and unloading is triggered whenever
	// FPackageName::OnMountPointDismounted is called, but alternatively you can set 
	// Engine.ini:[AssetRegistry]:bActiveMountsLoadingAuto_Client=false and call ActiveMountsUnloadAsync and
	// ActiveMountsLoadAsync explicitly.

	/**
	 * Returns true if ActiveMountsIsEnabled in the current configuration. Returns true even if ActiveMountsInitialize
	 * has not yet been called. If not enabled, functions with AssetRegistry side effects will such as
	 * ActiveMountsRegister* will still execute the side-effects, but all behavior specific to unloading and reloading
	 * will be skipped by the ActiveMounts functions.
	 */
	virtual bool ActiveMountsIsEnabled() = 0;
	/**
	 * Called once per process to activate ActiveMountsLoading. Calls to ActiveMountsLoadAsync/ActiveMountsUnloadAsync
	 * before this call are ignored.
	 * This function also forces the parsing of the premade AssetRegistry: MountPoints in the premade AssetRegistry
	 * state will be searched at this point in FPackageName or FPlugins; any mountpoints used by the premade that are
	 * not mounted and not findable as a plugin should be registered before this via FPackageName::FindOrAddMountPoint.
	 * Any mountpoints not findable will log an error and AssetDatas under them will always be loaded.
	 */
	virtual void ActiveMountsInitialize() = 0;

	/**
	 * Registers an AssetRegistry.bin file on disk for ActiveMountsLoading, and also immediately loads its
	 * data into the AssetRegistry with AppendState. The default premade AssetRegistry.bin is automatically
	 * registered by the AssetRegistry and does not need this call.
	 * If ActiveMountsLoading is disabled, this function behaves the same as AppendState.
	 * Returns with no action if AssetRegistryState can not be loaded from the given file.
	 * This function also forces the parsing of the premade AssetRegistry, @see ActiveMountsInitialize.
	 * 
	 * @return True if the AssetRegistryState could be loaded from the given file, else false.
	 */
	virtual bool ActiveMountsRegisterAndLoadFilePathSynchronous(FStringView StateFilePath) = 0;
	/**
	 * Runs ActiveMountsRegisterAndLoadFilePathSynchronous on an async thread and calls OnComplete when done.
	 * OnComplete is called from an async thread, and in game error cases might be called after IAssetRegistry
	 * is shutdown. Caller should handle thread safety and should check for IAssetRegistry::Get != nullptr
	 * if it is used in their callback.
	*/
	virtual void ActiveMountsRegisterAndLoadFilePathAsync(FStringView StateFilePath,
		...
		UE::Tasks::ETaskPriority TaskPriority = UE::Tasks::ETaskPriority::Default) = 0;
	/**
	 * Requests that AssetDatas under the given MountPoints (looked up via FPackageName::FindOrAddMountPoint) be
	 * reloaded into the AssetRegistry. This only affects MountPoints previously unloaded via auto unload or 
	 * ActiveMountsUnloadAsync, and is only possible for AssetDatas in the premade AssetRegistry or in an AssetRegistry
	 * file registered via an ActiveMountsRegister function. Calls OnComplete once the load completes.
	 * 
	 * @param OnComplete Optional delegate: void OnComplete(bool bCanceled). Called when the load is complete.
	 *        bCanceled is set to true iff ActiveMountsLoading is not enabled or for any requested MountPoint the
	 *        load failed or was overridden by an unload request. If true, test ActiveMountsIsEnabled and call
	 *        ActiveMountsIsLoaded to test state of each mountpoint.
	 *        OnComplete is called from an async thread, caller should handle thread safety in their callback.
	 */
	virtual void ActiveMountsLoadAsync(TConstArrayView<FStringView> MountPointLongPackageNames,
		TFunction<void(bool bCanceled)> OnComplete = TFunction<void(bool bCanceled)>()) = 0;
	/**
	 * Requests that all AssetDatas that have been unloaded by ActiveMounts be reloaded. Calls OnComplete once the
	 * load completes.
	 *
	 * @param OnComplete Optional delegate: void OnComplete(bool bCanceled). Called when the load is complete.
	 *        bCanceled is set to true iff ActiveMountsLoading is not enabled or for any MountPoint the load failed
	 *        or was overridden by an unload request. 
	 *        OnComplete is called from an async thread, caller should handle thread safety in their callback.
	 */
	...
	/**
	 * Requests that AssetDatas under the given MountPoints (looked up via FPackageName::FindOrAddMountPoint) be
	 * unloaded from the AssetRegistry. This is only possible for AssetDatas in the premade AssetRegistry or in an
	 * AssetRegistry file registered via an ActiveMountsRegister function. Calls OnComplete once the unload completes.
	 *
	 * @param OnComplete Optional delegate: void OnComplete(bool bCanceled). Called when the unload is complete.
	 *        bCanceled is set to true iff ActiveMountsLoading is not enabled or for any requested MountPoint the
	 *        unload failed or was overridden by a load request. If true, test ActiveMountsIsEnabled and call
	 *        ActiveMountsIsLoaded to test state of each mountpoint.
	 *        OnComplete is called from an async thread, caller should handle thread safety in their callback.
	 */
	virtual void ActiveMountsUnloadAsync(TConstArrayView<FStringView> MountPointLongPackageNames,
		TFunction<void(bool bCanceled)> OnComplete = TFunction<void(bool bCanceled)>()) = 0;
	/**
	 * Requests that AssetDatas under MountPoints that are available for unloading via the premade
	 * AssetRegistry or in an AssetRegistry registered via an ActiveMountsRegister function be unloaded if they
	 * are not currently mounted with FPackageName. Calls OnComplete once the unload completes.
	 *
	 * @param OnComplete Optional delegate: void OnComplete(bool bCanceled). Called when the unload is complete.
	 *        bCanceled is set to true iff ActiveMountsLoading is not enabled or for any MountPoint the unload failed
	 *        or was overridden by a load request. 
	 *        OnComplete is called from an async thread, caller should handle thread safety in their callback.
	 */
	virtual void ActiveMountsUnloadAllUnmountedAsync(
		TFunction<void(bool bCanceled)> OnComplete = TFunction<void(bool bCanceled)>()) = 0;
	/**
	 * Returns whether the given MountPoint (looked up via FPackageName::FindOrAddMountPoint) is loaded in the
	 * ActiveMounts system. Returns false if currently loading or unloading or if unloaded. Only valid if
	 * ActiveMountsLoader is enabled and the MountPoint is used by the PreMade AssetRegistry or a registered
	 * AssetRegistry file. Returns false for all other cases.
	 */
	virtual bool ActiveMountsIsLoaded(FStringView MountPointLongPackageName) = 0;
	/**
	 * Returns whether ActiveMountsLoading is in progress for any MountPoint. Returns false if ActiveMountsLoading is
	 * not enabled.
	 */
	virtual bool ActiveMountsIsInProgress() = 0;
	/**
	 * Call the given callback when all requested ActiveMountsLoading loads and unloads complete and the system
	 * becomes idle. The callback is called outside of the lock, so by the time it is called, a new request may
	 * have come in and the system might be in progress again. Intended use of this function is to guard entry into a
	 * state that relies on AssetRegistry queries after issuing a set of ActiveMounts loads or unloads.
	 * If the system is already idle (ActiveMountsIsInProgress returns false), the callback is called immediately.
	 * If ActiveMountsLoading is not enabled, the callback is called immediately.
	 * The callback might be called from an async thread; caller should handle thread safety in their callback.
	 */
	virtual void ActiveMountsReportWhenIdle(TFunction<void()> OnIdle) = 0;
	...
	/**
	 * Request to pause paths mounting.
	 * Can be used by the calling code to temporarily pause loading of data inside the AssetRegistry in response
	 * to mounted MountPoints (e.g. ActiveMountsLoading). All mounting requests will be executed in one batch at the
	 * end of that scope.
	 */
	struct FSuspendPathsMounting
	{
		UE_NODISCARD_CTOR FSuspendPathsMounting() { IAssetRegistry::GetChecked().SuspendPathsMounting(); }
		~FSuspendPathsMounting() { IAssetRegistry* AR = IAssetRegistry::Get(); if (AR) AR->ResumePathsMounting(); }
	};

	/**
	 * Request to pause paths dismounting.
	 * Can be used by the calling code to temporarily pause clearing of data inside the AssetRegistry.
	 * All unmounting requests will be executed in one batch at the end of that scope.
	 */
	struct FSuspendPathsDismounting
	{
		UE_NODISCARD_CTOR FSuspendPathsDismounting() { IAssetRegistry::GetChecked().SuspendPathsDismounting(); }
		~FSuspendPathsDismounting() { IAssetRegistry* AR = IAssetRegistry::Get(); if (AR) { AR->ResumePathsDismounting(); } }
	};

protected:

	// Implementation methods for FSuspendPathsMounting and FSuspendPathsDismounting
	virtual void SuspendPathsMounting() {};
	virtual void ResumePathsMounting() {};
	virtual void SuspendPathsDismounting() {};
	virtual void ResumePathsDismounting() {};
	...
	// Functions specifically for calling from cooker's FAssetRegistryGenerator
	friend class FAssetRegistryGenerator;

	/**
	 * Copy all manage dependencies in the global AssetRegistry that target any assets that exist in InOutState into
	 * the manage dependencies of InOutState.
	 */
	virtual void CopyManageDependenciesToState(FAssetRegistryState& InOutState) const = 0;
