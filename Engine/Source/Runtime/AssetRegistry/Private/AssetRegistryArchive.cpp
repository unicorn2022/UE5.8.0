	...
	for (FAssetBundleEntry& Entry : Entries)
	{
		...
		if (Version < FAssetRegistryVersion::RemoveAssetPathFNames)
		{
			for (FTopLevelAssetPath& Path : Entry.AssetPaths)
			{
				...
				Path = FTopLevelAssetPath(AssetPathName.ToString());
			}
		}
		else
		{
...
FAssetRegistryWriter::~FAssetRegistryWriter()
{
	...
	UE::AssetRegistry::AlignPosInArchive(MemWriter);	// make sure the data before store leaves it in the aligned position
	int64 StoreStart = MemWriter.TotalSize();
	...
	// make sure the Store is aligned
	UE::AssetRegistry::AlignPosInArchive(TargetAr);
	TargetAr.Serialize(MemWriter.GetData() + StoreStart, MemWriter.TotalSize() - StoreStart);
	// Body does not need to be aligned because we copy all the data out of it rather than pointing to its memory-mapped position
...
FAssetRegistryReader::FAssetRegistryReader(FArchive& Inner,
	TSharedPtr<UE::AssetRegistry::FMemoryMappedFile> InBackingFile,
	const FixedTagPrivate::FWeakStorePtr& PreviousLoadedStore,
	int32 NumWorkers, FAssetRegistryHeader Header)
	: FArchiveProxy(Inner)
{
	check(IsLoading());
	...
	// Load older versions for compatibility
	if (UNLIKELY(Header.Version < FAssetRegistryVersion::MemoryMappedTagDataStore))
	{
		Names = LoadNameBatch(Inner, ENameBatchLoadingFlags::RespectOrder);
		Tags = FixedTagPrivate::LoadStore(*this, Header.Version, nullptr);
	}
	else
	{
		TRefCountPtr<const FixedTagPrivate::FStore> PreviousStore = PreviousLoadedStore.Pin();
		if (NumWorkers > 0 && !PreviousStore) // Multithreaded loading does not yet support PreviousStore
		{
			...
			StoreLoader.SetStoreBackingFile(InBackingFile);
		...
		else
		{
			...
			if (PreviousStore)
			{
				Tags = MoveTemp(PreviousStore);
				FixedTagPrivate::SkipBytesStore(*this, Header.Version);
			}
			else
			{
				Tags = FixedTagPrivate::LoadStore(*this, Header.Version, InBackingFile);
			}
		}
	}
}
...
const TRefCountPtr<const FixedTagPrivate::FStore>& FAssetRegistryReader::GetTags() const
{
	return Tags;
}
...
bool FAssetRegistryTagSerializationTest::RunTest(const FString& Parameters)
{
...

	{
		...
		FAssetRegistryReader RegistryReader(DataReader, nullptr, FixedTagPrivate::FWeakStorePtr(), 0, Header);
		RegistryReader.WaitForTasks();
