...
{
public:
	/// @param InBackingFile shared pointer to a class that allows mapping the underlying file into memory
	/// @param NumWorkers > 0 for parallel loading
	FAssetRegistryReader(FArchive& Inner, TSharedPtr<UE::AssetRegistry::FMemoryMappedFile> InBackingFile,
		const FixedTagPrivate::FWeakStorePtr& PreviousLoadedStore, int32 NumWorkers, FAssetRegistryHeader Header);
	~FAssetRegistryReader();
	const TRefCountPtr<const FixedTagPrivate::FStore>& GetTags() const;
