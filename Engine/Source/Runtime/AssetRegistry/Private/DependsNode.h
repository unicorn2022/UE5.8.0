...
public:
	...
	/** Replace nodes in referencers and dependencies with OldNodeToNewNode[Target]; remove any that remap to null. */
	void RemapLinks(const TMap<FDependsNode*, FDependsNode*>& OldNodeToNewNode)
	{
		RemapLinks([&OldNodeToNewNode](FDependsNode* Target)
			{
				FDependsNode*const* NewNodePtr = OldNodeToNewNode.Find(Target);
				return NewNodePtr ? *NewNodePtr : nullptr;
			});
	}
	/** Replace nodes in referencers and dependencies with the Remapper[Target]; remove any that Remap to null. */
	void RemapLinks(const TUniqueFunction<FDependsNode* (FDependsNode*)>& Remapper);
	/** Replace nodes in dependencies with the Remapper[Target]; remove any that Remap to null. */
	void RemapDependencies(const TUniqueFunction<FDependsNode* (FDependsNode*)>& Remapper);
	/** Replace nodes in referencers with the Remapper[Target]; remove any that Remap to null. */
	void RemapReferencers(const TUniqueFunction<FDependsNode* (FDependsNode*)>& Remapper);
...
private:
	...
	uint32 PackageIsSorted : 1 = 1;
	uint32 SearchableNameIsSorted : 1 = 1;
	uint32 ManageIsSorted : 1 = 1;
	uint32 ReferencersIsSorted : 1 = 1;
	uint32 DependenciesInitialized : 1 = 0;
	uint32 bScriptPath : 1 = 0;
	/** Used to control FDependsNodeList shrinking behaviour */
	uint32 bAllowShrinking : 1 = 1;
...
inline void FDependsNode::Empty()
{
