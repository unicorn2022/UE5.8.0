	...
	*this << PackageFileSummary;
		...
		IsEnforcePackageCompatibleVersionCheck())
	{
		...
		UE_LOGF(LogAssetRegistry, Warning,
			"Package is unloadable: %ls. Reason: Package was saved unversioned and the current process does not support loading unversioned packages.",
			*PackageFilename);
		...
		IsEnforcePackageCompatibleVersionCheck())
	{
		// Log a warning rather than an error. Linkerload gracefully handles this case.
		UE_LOGF(LogAssetRegistry, Warning,
			"Package is unloadable: %ls. Reason: Version is too old. Min Version: %i, Package Version: %i.",
		...
		IsEnforcePackageCompatibleVersionCheck())
	{
		// Log a warning rather than an error. Linkerload gracefully handles this case.
		UE_LOGF(LogAssetRegistry, Warning,
			"Package is unloadable: %ls. Reason: Version is too new. Engine Version: %i, Package Version: %i.",
		...
		IsEnforcePackageCompatibleVersionCheck())
	{
		// Log a warning rather than an error. Linkerload gracefully handles this case.
		UE_LOGF(LogAssetRegistry, Warning,
			"Package is unloadable: %ls. Reason: LicenseeVersion is too new. Licensee Version: %i, Package Licensee Version: %i.",
	...
	for (FCustomVersionDifference Diff : Diffs)
	{
		...
		else if (Diff.Type == ECustomVersionDifference::Newer)
		{
			if (IsEnforcePackageCompatibleVersionCheck())
			{
				...
				UE_LOGF(LogAssetRegistry, Error,
					"Package is unloadable: %ls. Reason: Custom version is too new; the package has newer custom version of %ls: Package: %d, HeadCode: %d.",
...
#define UE_PACKAGEREADER_CORRUPTPACKAGE_WARNING(MessageKey, PackageFileName) \
	...
	{\
		...
		UE_LOGF(LogAssetRegistry, Warning, "%ls", \
...
bool FPackageReader::ReadImportTypeHierarchies(TMap<FSoftObjectPath, UE::Serialization::Private::FImportTypeHierarchy>& OutTypeHierarchies)
{
	// Skip export map as there should be no paths including exports in import type hierarchies. 
	// If there are and the export map has not been serialized, warnings will be logged.
	if (!SerializeNameMap() || !SerializeImportMap())
	{
		return false;
	}

	if (!StartSerializeSection(PackageFileSummary.ImportTypeHierarchiesOffset))
	{
		UE_LOGFMT_LOC(LogAssetRegistry, Warning,"ImportTypeHierarchies_BadOffset",
			"Package is unloadable: {FileName}. Reason: ImportTypeHierarchiesOffset {Offset} was invalid. ",
			("FileName", PackageFilename), ("Offset", PackageFileSummary.ImportTypeHierarchiesOffset));
		return false;
	}
	...
	*this << Count;
	if (IsError())
	{
		UE_LOGFMT_LOC(LogAssetRegistry, Warning,"ImportTypeHierarchies_CountSerializeFail",
			"Package is unloadable: {FileName}. Reason: Failed to serialize import type hierarchy count. ",
			("FileName", PackageFilename));
		return false;
	}
	if (Count != PackageFileSummary.ImportTypeHierarchiesCount)
	{
		UE_LOGFMT_LOC(LogAssetRegistry, Warning,"ImportTypeHierarchies_CountMismatch",
			"Package is unloadable: {FileName}. Reason: ImportTypeHierarchiesCount {HeaderCount} did not match embedded count {EmbeddedCount}. ",
			("FileName", PackageFilename), ("HeaderCount", PackageFileSummary.ImportTypeHierarchiesCount), ("EmbeddedCount", Count));
		return false;
	}

	bool bSuccess = true;
	for (int32 i=0; i < PackageFileSummary.ImportTypeHierarchiesCount; ++i)
	{
		TPair<FPackageIndex, UE::Serialization::Private::FImportTypeHierarchy> Pair;
		*this << Pair;
		if (IsError())
		{
			UE_LOGFMT_LOC(LogAssetRegistry, Warning,"ImportTypeHierarchies_SerializeFail",
				"Package is unloadable: {FileName}. Reason: Failed to serialize import type hierarchy at index {Index}. ",
				("FileName", PackageFilename), ("Index", i));
			return false;
		}

		TOptional<FSoftObjectPath> Path = LinkerIndexToSoftObjectPath(Pair.Key);
		if (Path.IsSet())
		{
			OutTypeHierarchies.Add(Path.GetValue(), MoveTemp(Pair.Value));
		}
		else
		{
			UE_LOGFMT_LOC(LogAssetRegistry, Warning,"ImportTypeHierarchies_BadPath",
				"Package is unloadable: {FileName}. Reason: Package index {Index} could not be resolved to a path. ",
				("FileName", PackageFilename), ("Index", Pair.Key.ForDebugging()));
			bSuccess = false;
		}
	}
...
bool FPackageReader::ReadCellExports(TArray<FCellExport>& OutExports)
{
	if (PackageFileSummary.CellExportCount == 0)
	{
		return true;
	}

	if (!StartSerializeSection(PackageFileSummary.CellExportOffset))
	{
		UE_LOGFMT_LOC(LogAssetRegistry, Warning,"CellExports_BadOffset",
			"Package is unloadable: {FileName}. Reason: CellExportOffset {Offset} was invalid. ",
			("FileName", PackageFilename), ("Offset", PackageFileSummary.CellExportOffset));
		return false;
	}

	OutExports.Reserve(PackageFileSummary.CellExportCount);
	for (int32 i=0; i < PackageFileSummary.CellExportCount; ++i)
	{
		FCellExport& Export = OutExports.Emplace_GetRef();
		*this << Export;

		if (IsError())
		{
			UE_LOGFMT_LOC(LogAssetRegistry, Warning,"CellExport_SerializeFailed",
				"Package is unloadable: {FileName}. Reason: Failed to serialize cell export at index {Index}. ",
				("FileName", PackageFilename), ("Index", i));
			return false;
		}
	}
...
bool FPackageReader::ReadCellImports(TArray<FCellImport>& OutImports)
{
	if (PackageFileSummary.CellImportCount == 0)
	{
		return true;
	}

	if (!StartSerializeSection(PackageFileSummary.CellImportOffset))
	{
		UE_LOGFMT_LOC(LogAssetRegistry, Warning,"CellImports_BadOffset",
			"Package is unloadable: {FileName}. Reason: CellImportOffset {Offset} was invalid. ",
			("FileName", PackageFilename), ("Offset", PackageFileSummary.CellImportOffset));
		return false;
	}

	OutImports.Reserve(PackageFileSummary.CellImportCount);
	for (int32 i=0; i < PackageFileSummary.CellImportCount; ++i)
	{
		FCellImport& Import = OutImports.Emplace_GetRef();
		*this << Import;

		if (IsError())
		{
			UE_LOGFMT_LOC(LogAssetRegistry, Warning,"CellImport_SerializeFailed",
				"Package is unloadable: {FileName}. Reason: Failed to serialize cell import at index {Index}. ",
				("FileName", PackageFilename), ("Index", i));
			return false;
		}
	}
...
bool FPackageReader::ReadSearchableNames(TMap<FTopLevelAssetPath, TArray<FName>>& OutSearchableNames)
{
	...
	bool bSuccess = true;
	for (const TPair<FPackageIndex, TArray<FName>>& Pair : SearchableNames.SearchableNamesMap)
	{
		FPackageIndex Index = Pair.Key;
		TArray<FName> PathElements;
		while (!Index.IsNull())
		{
			FObjectResource* Obj = nullptr;
			if (Index.IsImport() && ImportMap.IsValidIndex(Index.ToImport()))
			{
				Obj = &ImportMap[Index.ToImport()];
			}
			else if (Index.IsExport() && ExportMap.IsValidIndex(Index.ToExport()))
			{
				Obj = &ExportMap[Index.ToExport()];
			}  
			if (Obj)
			{
				PathElements.Add(Obj->ObjectName);
				Index = Obj->OuterIndex;
			}
			else 
			{
				UE_LOGFMT_LOC(LogAssetRegistry, Warning,"SearchableNamePathInvalid",
					"Package is unloadable: {FileName}. Reason: SearchableName path was not valid. Index: {Index} ",
					("FileName", PackageFilename), ("Index", Pair.Key.ForDebugging()));
				bSuccess = false;
				break;
			}
		...
		if (!Index.IsNull())
		{
			// Exited loop early above
		}
		else if (PathElements.Num() == 2)
		{
			OutSearchableNames.Add(FTopLevelAssetPath(PathElements[1], PathElements[0]), Pair.Value);
		}
		else if (PathElements.Num() == 1)
		{
			OutSearchableNames.Add(FTopLevelAssetPath(PathElements[0], FName()), Pair.Value);
		}
		else 
		{
			UE_LOGFMT_LOC(LogAssetRegistry, Warning,"SearchableNamePathTooLong",
				"Package is unloadable: {FileName}. Reason: SearchableName object was not a top level asset. Path: {Path} ",
				("FileName", PackageFilename), ("Path", PathElements));
			bSuccess = false;
		}
	}
...
bool FPackageReader::ReadWorldTileInfo(FWorldTileInfo& OutInfo)
{
	if (PackageFileSummary.WorldTileInfoDataOffset == 0)
	{
		return true;
	}

	if (!StartSerializeSection(PackageFileSummary.WorldTileInfoDataOffset))
	{
		UE_LOGFMT_LOC(LogAssetRegistry, Warning,"WorldTileInfo_BadOffset",
			"Package is unloadable: {FileName}. Reason: WorldTileInfoDataOffset {Offset} was invalid. ",
			("FileName", PackageFilename), ("Offset", PackageFileSummary.WorldTileInfoDataOffset));
		return false;
	}
	...
	*this << OutInfo;
	if (IsError())
	{
		UE_LOGFMT_LOC(LogAssetRegistry, Warning,"WorldTileInfo_SerializeFail",
			"Package is unloadable: {FileName}. Reason: Failed to serialize WorldTileInfo. ",
			("FileName", PackageFilename));
		return false;
	}
...
bool FPackageReader::ReadDataResourceMap(TArray<TTuple<FSoftObjectPath, FObjectDataResource>>& OutDataResources)
{
	// Data resources should never be associated with imports, but there may be a case in which an export is
	// an inner of an import, e.g. external packages
	if (!SerializeNameMap() || !SerializeExportMap() || !SerializeImportMap())
	{
		return false;
	}

	if (!StartSerializeSection(PackageFileSummary.DataResourceOffset))
	{
		UE_LOGFMT_LOC(LogAssetRegistry, Warning,"DataResourceMap_BadOffset",
			"Package is unloadable: {FileName}. Reason: DataResourceOffset {Offset} was invalid. ",
			("FileName", PackageFilename), ("Offset", PackageFileSummary.DataResourceOffset));
		return false;
	}

	TArray<FObjectDataResource> DataResources;
	FObjectDataResource::Serialize(*this, DataResources);
	if (IsError())
	{
		UE_LOGFMT_LOC(LogAssetRegistry, Warning,"DataResourceMap_SerializeFail",
			"Package is unloadable: {FileName}. Reason: Failed to serialize DataResourceMap. ",
			("FileName", PackageFilename));
		return false;
	}

	bool bSuccess = true;
	for (int32 i=0; i < DataResources.Num(); ++i)
	{
		const FObjectDataResource& DataResource = DataResources[i];
		TOptional<FSoftObjectPath> Path = LinkerIndexToSoftObjectPath(DataResource.OuterIndex);
		if (Path.IsSet())
		{
			OutDataResources.Emplace(*Path, DataResource);
		}
		else
		{
			UE_LOGFMT_LOC(LogAssetRegistry, Warning,"DataResourceMap_BadObjectPath",
				"Package is unloadable: {FileName}. Reason: Failed to get object path for data resource {Index}. ",
				("FileName", PackageFilename), ("Index", i));
			bSuccess = false;
		}
	}
...
#if WITH_METADATA
bool FPackageReader::ReadMetaData(FMetaData& OutMetaData)
{
	...
	if (!StartSerializeSection(PackageFileSummary.MetaDataOffset))
	{
		UE_LOGFMT_LOC(LogAssetRegistry, Warning,"MetaDataInvalidOffset",
			"Package is unloadable: {FileName}. Reason: Invalid metadata offset {Offset}",
			("FileName", PackageFilename), ("Offset", PackageFileSummary.MetaDataOffset));
		return false;
	}

	int32 NumObjectMetaDataMap = 0;
	int32 NumRootMetaDataMap = 0;

	*this << NumObjectMetaDataMap;
	*this << NumRootMetaDataMap;
	if (IsError())
	{
		UE_LOGFMT_LOC(LogAssetRegistry, Warning,"MetaDataFailedToReadCounts",
			"Package is unloadable: {FileName}. Reason: Failed to read metadata counts",
			("FileName", PackageFilename));
		return false;
	}

	for (int32 i=0; i < NumObjectMetaDataMap; ++i)
	{
		TPair<FSoftObjectPath, TMap<FName, FString>> ObjectMetaData;
		*this << ObjectMetaData;
		if (IsError())
		{
			UE_LOGFMT_LOC(LogAssetRegistry, Warning,"MetaDataFailedToReadObjectMetaData",
				"Package is unloadable: {FileName}. Reason: Failed to read object metadata. Index: {Index}",
				("FileName", PackageFilename), ("Index", i));
			return false;
		}
		OutMetaData.ObjectMetaDataMap.Add(ObjectMetaData.Key, ObjectMetaData.Value);
	}

	for (int32 i=0; i < NumRootMetaDataMap; ++i)
	{
		TPair<FName, FString> RootMetaData;
		*this << RootMetaData;
		if (IsError())
		{
			UE_LOGFMT_LOC(LogAssetRegistry, Warning,"MetaDataFailedToReadRootMetaData",
				"Package is unloadable: {FileName}. Reason: Failed to read root metadata. Index: {Index}",
				("FileName", PackageFilename), ("Index", i));
			return false;
		}
		OutMetaData.RootMetaDataMap.Add(RootMetaData.Key, RootMetaData.Value);
	}
...
bool FPackageReader::ReadPackageTrailer(UE::FPackageTrailer& OutTrailer)
{
	...
	{
		UE_LOGFMT_LOC(LogAssetRegistry, Warning,"PackageTrailer_BadOffset",
			"Package is unloadable: {FileName}. Reason: Bad offset for PayloadTocOffset {Offset}. ",
			("FileName", PackageFilename), ("Offset", PackageFileSummary.PayloadTocOffset));
		return false;
	}

	if (!OutTrailer.TryLoad(*this))
	{
		UE_LOGFMT_LOC(LogAssetRegistry, Warning,"PackageTrailer_SerializeFail",
			"Package is unloadable: {FileName}. Reason: Failed to deserialize trailer. ",
			("FileName", PackageFilename));
		return false;
	}
	...
	TArray<TPair<FName, UE::AssetRegistry::EExtraDependencyFlags>>& InOutExtraPackageDependencies)
{
	...
	{
		UE_LOGF(LogPackageRelocation, Verbose, "Detected relocated package (%.*ls). The package was saved as (%ls).",
...
static FName CoreUObjectPackageName("/Script/CoreUObject");
static FName ScriptStructName("ScriptStruct");
...
FArchive& FPackageReader::operator<<( FName& Name )
{
	...
	{
		UE_LOGF(LogAssetRegistry, Warning,
			"Package is unloadable: %ls. Reason: Bad name index %i/%i when reading package.",
...
// Potentially look up by index into the soft object path table as with LinkerLoad
FArchive& FPackageReader::operator<<(FSoftObjectPath& Value)
{
	FArchive& Ar = *this;
	// Comment taken from FLinkerLoad:
	...
	if (SoftObjectPathMap.Num() > 0)
	{
		...
		if (SoftObjectPathMap.IsValidIndex(SoftObjectPathIndex))
		{
			Value = SoftObjectPathMap[SoftObjectPathIndex];
		...
		{
			UE_LOGFMT_LOC(LogAssetRegistry, Warning,"SoftObjectPathInvalidIndex",
				"Package is unloadable: {FileName}. Reason: Bad soft object path index. Index: {Index}/{Count}",
				("FileName", PackageFilename), ("Index", SoftObjectPathIndex), ("Count", SoftObjectPathMap.Num()));
...
namespace UE::AssetRegistry
{
		...
		const TArray<FObjectImport>* InImports = nullptr, const TArray<FObjectExport>* InExports = nullptr)
	{
		...
		{
			UE_LOGF(LogAssetRegistry, Error, "Failed to find an import or export matching asset class short name \"%ls\".", *AssetClassName);
		...
		TArray<FAssetData*>& OutAssetDataList, EReadPackageDataMainErrorCode& OutError, const TArray<FObjectImport>* InImports, const TArray<FObjectExport>* InExports)
	{
		...
		for (int32 ObjectIdx = 0; ObjectIdx < DeserializePackageData.ObjectCount; ++ObjectIdx)
		{
			...
			if (!bFullObjectPath)
			{
				...
				{
					UE_LOGF(LogAssetRegistry, Warning,
						"Package is loadable but its AssetRegistry data is corrupt: %ls. Reason: Cannot make FAssetData for sub object %ls.",
			...
			{
				UE_LOGF(LogAssetRegistry, Warning,
					"Package is loadable but has invalid data; resave the package! Package: %ls. Reason: Export %ls is invalid.",
					*PackageName, *ObjectPackageData.ObjectPath);
				continue;
			}
...
TOptional<FSoftObjectPath> FPackageReader::LinkerIndexToSoftObjectPath(FPackageIndex Index)
{
	if (Index.IsNull())
	{
	...
	// Most paths will be 2-3 elements but there may be some deeply nested subobjects in certain asset types.
	TArray<FName, TInlineAllocator<8>> PathElements; 

	auto GetObjectResource = [this](FPackageIndex I) -> FObjectResource*
	{
		if (I.IsImport())
		{
			if (ImportMap.IsValidIndex(I.ToImport()))
			{
				return &ImportMap[I.ToImport()];
			}
		}
		else 
		{
			if (ExportMap.IsValidIndex(I.ToExport()))
			{
				return &ExportMap[I.ToExport()];
			}
		}
		UE_LOGFMT_LOC(LogAssetRegistry, Warning,"ConstructObjectPath_PackageIndexOutOfRange",
			"Package is unloadable: {FileName}. Reason: Bad object index {Index}. ",
			("FileName", PackageFilename), ("Index", I.ForDebugging()));
		return nullptr;
	};

	// Simultaneously check for loops with another pointer moving at twice the rate
	FPackageIndex P1 = Index;
	FPackageIndex P2 = Index;
	while (!P1.IsNull())
	{
		FObjectResource* Obj = GetObjectResource(P1);
		if (!Obj)
		{
			return {};
		}
		...
		if (P1.IsNull())
		{
			// If P1 terminates we know there is no loop and we have all path elements that we want.
			break;
		}

		// Advance P2 by two steps, if it terminates we know there is no loop so we can stop advancing it
		for (int32 i=0; i <2; ++i)
		{
			if (!P2.IsNull())
			{
				FObjectResource* Obj2 = GetObjectResource(P2);
				if (!ensureMsgf(Obj2, TEXT("Unexpected null object resource traversing object path")))
				{
					return {};
				}
				P2 = Obj2->OuterIndex;
		...
		// If P1 did not terminate and P2 is at the same place as P1, we have a loop
		if (P1 == P2)
		{
			UE_LOGFMT_LOC(LogAssetRegistry, Warning,"PackageIndexOuterLoop",
				"Package is unloadable: {FileName}. Reason: Loop detected in outer chain of object index {Index}. ",
				("FileName", PackageFilename), ("Index", Index.ForDebugging()));
			return {};
		}
	}

	// PathElements was populated from leaf to root, reverse it before handling any of these cases for clarity
	Algo::Reverse(PathElements);
	if (PathElements.Num() == 2)
	{
		return FSoftObjectPath(FTopLevelAssetPath(PathElements[0], PathElements[1]));
	}
	else if (PathElements.Num() == 1)
	{
		return FSoftObjectPath(FTopLevelAssetPath(PathElements[0], FName()));
	}
	else 
	{
		// Technically some delimiters ought to be : but we can't do that without using type information
		TStringBuilder<512> Builder;
		Builder.Join(PathElements, '.'); 
