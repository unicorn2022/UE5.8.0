...
void FAssetDataDiscoveryCache::LoadAndUpdateCache()
{
		...
		(WriteEnabled == EFeatureEnabled::Never && GGatherSettings.IsDiscoveryCacheWriteEnabled() != EFeatureEnabled::Never))
	{
		...
		UE_LOGF(LogAssetRegistry, Display,
			"PlatformFileJournal is not available on volume '%ls' of project directory '%ls', so AssetDiscovery cache will not be %ls. Unavailability reason:\n\t%ls",
	...
	bool bReportCacheErrorDiagnostics = false;
	for (TPair<FString, FCachedVolumeInfo>& VolumePair : CachedVolumes)
	{
		...
		for (TMap<FString, FCachedDirScanDir>::TIterator DirIter(VolumeInfo.Dirs); DirIter; ++DirIter)
		{
			...
			if (DirData.bCacheValid)
			{
				if (DirData.JournalHandle != FileJournalFileHandleInvalid)
				{
					KnownDirectories.Add(DirData.JournalHandle, DirName);
				}
				else
				{
					// We have serialized DirData that doesn't have a valid handle and thus we can't check
					// if the directory has been modified. Mark it as invalid so it can be rescanned (same 
					// effect as appending to ModifiedDirectories but avoids the overhead)
		...
		if (!VolumeInfo.bJournalAvailable)
		{
			UE_LOGF(LogAssetRegistry, Warning,
				"PlatformFileJournal is not available on volume '%ls'. AssetRegistry discovery of files on this volume will be uncached. Unavailability reason:\n\t%ls",
		...
		else
		{
			...
			{
				UE_LOGF(LogAssetRegistry, Display,
					"PlatformFileJournal journal has wrapped for volume '%ls'. AssetRegistry discovery of files on this volume will be uncached. Notes on wrapping:" "\r\n%ls",
					*VolumeName, *VolumeInfo.LastError);
			...
			{
				UE_LOGF(LogAssetRegistry, Warning,
					"PlatformFileJournal is not available for volume '%ls'. AssetRegistry discovery of files on this volume will be uncached. Unavailability reason:" "\n\t%ls",
					*VolumeName, *VolumeInfo.LastError);
				bReportCacheErrorDiagnostics = true;
	...
	if (bReportCacheErrorDiagnostics)
	{
#if WITH_EDITOR
		...
		UE_LOGF(LogAssetRegistry, Warning, "Saving a copy of %ls to %ls for debug diagnostics.", *CachePath, *NewCachePath);
		if (!IFileManager::Get().Move(*NewCachePath, *CachePath, true /*Replace*/, true /*EvenIfReadOnly*/))
		{
			UE_LOGF(LogAssetRegistry, Warning, "Failed creating a copy of %ls at %ls", *CachePath, *NewCachePath);
		}
#endif
	}
}
...
void FAssetDataDiscoveryCache::SaveCache()
{
	...
	if (!Ar)
	{
		UE_LOGF(LogAssetRegistry, Warning, "Could not write to DiscoveryCacheFile %ls.", *Filename);
		return;
	}
	Compressed.Save(*Ar);
}
...
bool FAssetDataDiscoveryCache::TryReadCacheFile()
{
	...
	if (!bCacheValid)
	{
		...
		{
			UE_LOGF(LogAssetRegistry, Display,
				"Version mismatch for AssetDiscovery cache %ls. AssetRegistry discovery of files will be uncached.",
				*Filename);
		}
		else if (bCacheCorrupt)
		{
			UE_LOGF(LogAssetRegistry, Warning,
				"Corrupt AssetDiscovery cache %ls. AssetRegistry discovery of files will be uncached.",
				*Filename);
		}
		else
		{
			UE_LOGF(LogAssetRegistry, Display,
				"No AssetDiscovery cache present at %ls. AssetRegistry discovery of files will be uncached.",
				*Filename);
		}
		CachedVolumes.Empty();
		return false;
...
void FAssetDataDiscoveryCache::QueueConsume()
{
	...
	while (ScanQueueDirFullDatas.Dequeue(ScanDirPair))
	{
		...
		// Confirm our handle is valid otherwise mark the cache as invalid
		ScanDir.bCacheValid = ScanDir.JournalHandle != FileJournalFileHandleInvalid;
...
void FCachedVolumeInfo::RemoveInvalidDirs()
{
	for (TMap<FString, FCachedDirScanDir>::TIterator It = Dirs.CreateIterator(); It; ++It)
	{
		const FCachedDirScanDir& ScanDir = It->Value;
		if (!ScanDir.bCacheValid || ScanDir.JournalHandle == FileJournalFileHandleInvalid)
		{
			It.RemoveCurrent();
		}
	}
