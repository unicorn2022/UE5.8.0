...
void FAssetRegistryState::Reset()
{
	...
	// ensure(NumAssets == 0);
	UE_CLOGF(NumAssets != 0, LogAssetRegistry, Display,
		"AssetRegistryState::Reset: NumAssets does not match the number of CachedAssets entries. Leaking some allocations.");
	...
	{
		// Delete all PackageDatas in the cache
	...
	const TSet<int32> ChunksToKeep, const FAssetRegistrySerializationOptions& Options)
{
	...
	for (const TPair<FAssetIdentifier, FDependsNode*>& Pair : ExistingState.CachedDependsNodes)
	{
		...
		{
			// If we're removing this Node because it is a redirector, store its target
			// in RedirectCache so that referencers can redirect to the target instead.
			ExistingState.ResolveRedirector(Node, ExistingState.CachedAssets, RedirectCache);
		}
	}
	...
	ParallelFor(OldNodeToNewNodeMaxId, [&OldNodeToNewNode, &ReferencerPairs, &RedirectCache](int32 Index)
		{
			FSetElementId Id = FSetElementId::FromInteger(Index);
			if (OldNodeToNewNode.IsValidId(Id))
			{
				...
				OldNode->IterateOverDependencies([&, OldNode, NewNode](FDependsNode* InDependency,
					...
					bool bDuplicate)
					{
						...
						if (!NewDependency)
						{
							// Check for a redirected target; redirectors might be pruned but their targets might not be.
							FDependsNode** RedirectedInDependency = RedirectCache.Find(InDependency);
							if (RedirectedInDependency && *RedirectedInDependency != InDependency)
							{
								NewDependency = OldNodeToNewNode.Find(*RedirectedInDependency);
							}
						}
						if (NewDependency)
						{
	...
	FAssetRegistryAppendResult* OutAppendResult)
{
	...
	// and we want to work on it on multiple threads. But we can't add them to *this simultaneously because we can't append
	...
	ParallelFor(AssetDataMapMaxIndex,
		[&ConditionalAddDuplicatedAssetToQueue](int32 Index)
		{
...
void FAssetRegistryState::ConsumeExisting(FAssetRegistryState&& Consume,
	const FAssetRegistrySerializationOptions& Options,
	FAssetRegistryAppendResult* OutAppendResult)
{
	// Move over AssetDatas
	TArray<FAssetData*> AssetsToDeallocate;
	for (FAssetData* AssetData : Consume.CachedAssets)
	{
		if (CachedAssets.Find(FCachedAssetKey(AssetData)))
		{
			AssetsToDeallocate.Add(AssetData);
			continue;
		}
		...
		if (OutAppendResult)
		{
			OutAppendResult->AddedAssets.Add(AssetData);
		}
	}

	// Move over PackageDatas
	TArray<FAssetPackageData*> PackageDatasToDeallocate;
	for (const TPair<FName, FAssetPackageData*>& Pair : Consume.CachedPackageData)
	{
		if (!Pair.Value)
		{
			continue;
		}
		FAssetPackageData*& ExistingData = CachedPackageData.FindOrAdd(Pair.Key);
		if (ExistingData)
		{
			PackageDatasToDeallocate.Add(Pair.Value);
			continue;
		}
		ExistingData = Pair.Value;
		NumPackageData++;
	}

	// Move over DependsNodes, more complicated because of the links.
	// First, move over DependsNodes, and create a map from OldNode to NewNode, either OldNode to itself because keep
	// it or OldNode to the existing node that takes priority over it.
	TArray<FDependsNode*> NodesToDeallocate;
	TMap<FDependsNode*, FDependsNode*> OldNodeToNewNode;
	OldNodeToNewNode.Reserve(Consume.CachedDependsNodes.Num());
	for (const TPair<FAssetIdentifier, FDependsNode*>& Pair : Consume.CachedDependsNodes)
	{
		FDependsNode* ConsumedNode = Pair.Value;
		FDependsNode*& ExistingNode = CachedDependsNodes.FindOrAdd(Pair.Key);
		if (ExistingNode == nullptr)
		{
			ExistingNode = ConsumedNode;
			NumDependsNodes++;
		}
		else
		{
			NodesToDeallocate.Add(ConsumedNode);
		}
		OldNodeToNewNode.Add(ConsumedNode, ExistingNode);
	}

	// Second, remap:
	// Add mirrored two-way links for any links between an old node we are keeping and an old node we are not
	// keeping: copy over the link from the old node we are not keeping into the existing node that takes
	// priority over it.
	for (const TPair<FDependsNode*, FDependsNode*>& Pair : OldNodeToNewNode)
	{
		FDependsNode* OldSourceNode = Pair.Key;
		FDependsNode* NewSourceNode = Pair.Value;
		// We are keeping a node only if it maps to itself in FDependsNode*
		if (NewSourceNode == OldSourceNode)
		{
			// For any dependencies or references from the kept old node to a replaced old node, copy the replaced old node's
			// mirrored reference or dependency onto the new node that replaces it.
			OldSourceNode->IterateOverDependencies([OldSourceNode, &OldNodeToNewNode]
			(FDependsNode* OldTargetNode,
				...
				bool bDuplicate)
				{
					if (!bDuplicate)
					{
						FDependsNode** NewTargetNode = OldNodeToNewNode.Find(OldTargetNode);
						if (NewTargetNode && *NewTargetNode != OldTargetNode)
						{
							(*NewTargetNode)->AddReferencer(OldSourceNode);
						}
					}
				});
			OldSourceNode->IterateOverReferencers([OldSourceNode, &OldNodeToNewNode]
			(FDependsNode* OldReferencer)
				{
					FDependsNode** NewReferencerPtr = OldNodeToNewNode.Find(OldReferencer);
					FDependsNode* NewReferencer = NewReferencerPtr ? *NewReferencerPtr : nullptr;
					if (NewReferencer && NewReferencer != OldReferencer)
					{
						// We have a remapped referencer and we need to add a dependency from the remapped version of that referencer
						// to OldSourceNode. We have to look up what kind of dependency to add from that old referencer.
						OldReferencer->IterateOverDependencies([OldSourceNode, NewReferencer](
							FDependsNode* OldTargetOfOldRefencer,
							UE::AssetRegistry::EDependencyCategory Category,
							UE::AssetRegistry::EDependencyProperty Properties,
							bool bDuplicate)
							{
								if (OldTargetOfOldRefencer == OldSourceNode)
								{
									NewReferencer->AddDependency(OldSourceNode, Category, Properties);
								}
							});
					}
				});

			// Now that we've read what we need to from the old targets of the old links on this kept old node, remap all those
			// links to old targets to instead point to the new remapped targets.
			OldSourceNode->RemapLinks(OldNodeToNewNode);

			// ConsumeExisting, like InitializeFromExisting, sets consumed nodes to sorted. We also set AllowShrinking to true.
			// We use this contract because Consume and InitializeFromExisting are almost exclusively used when not gathering,
			// so we should optimize for read and for memory rather than write speed.
			OldSourceNode->SetAllowShrinking(true);
			OldSourceNode->SetIsDependencyListSorted(UE::AssetRegistry::EDependencyCategory::All, true);
			OldSourceNode->SetIsReferencersSorted(true);
		}
	}

	// Steal all of the preallocated buffers; we don't currently track whether we are not using them.
	for (FAssetData* Buffer : Consume.PreallocatedAssetDataBuffers)
	{
		this->PreallocatedAssetDataBuffers.Add(Buffer);
	}
	for (FDependsNode* Buffer : Consume.PreallocatedDependsNodeDataBuffers)
	{
		this->PreallocatedDependsNodeDataBuffers.Add(Buffer);
	}
	for (FAssetPackageData* Buffer : Consume.PreallocatedPackageDataBuffers)
	{
		this->PreallocatedPackageDataBuffers.Add(Buffer);
	}

	// Reset the Consume state, but only delete its data that we didn't steal.
	if (Consume.PreallocatedAssetDataBuffers.Num())
	{
		Consume.PreallocatedAssetDataBuffers.Empty();
	}
	else
	{
		for (FAssetData* AssetData : AssetsToDeallocate)
		{
			delete AssetData;
		}
	}
	...
	if (Consume.PreallocatedDependsNodeDataBuffers.Num())
	{
		Consume.PreallocatedDependsNodeDataBuffers.Empty();
		// Clear memory on the OldNodes, since they are staying around unused.
		for (FDependsNode* DependsNode : NodesToDeallocate)
		{
			DependsNode->Empty();
		}
	}
	...
	Consume.NumDependsNodes = 0;

	if (Consume.PreallocatedPackageDataBuffers.Num())
	{
		Consume.PreallocatedPackageDataBuffers.Empty();
	}
	else
	{
		for (FAssetPackageData* AssetPackageData : PackageDatasToDeallocate)
		{
			delete AssetPackageData;
		}
	}
	Consume.NumPackageData = 0;

	// Clear the containers on the consumed state; we can't call Reset since it also deletes the elements.
	Consume.CachedAssetsByPackageName.Empty();
	Consume.CachedAssetsByPath.Empty();
	Consume.CachedAssetsByClass.Empty();
#if UE_ASSETREGISTRY_CACHEDASSETSBYTAG
	Consume.CachedAssetsByTag.Empty();
#else
	Consume.CachedClassesByTag.Empty();
#endif
	Consume.CachedDependsNodes.Empty();
	Consume.CachedPackageData.Empty();
#if UE_ASSETREGISTRY_INDIRECT_ASSETDATA_POINTERS
	Consume.IndirectAssetDataArrays.Empty();
#endif
	Consume.CachedAssets.Empty();
...
void FAssetRegistryState::Prune(const FAssetRegistryPruneOptions& PruneOptions)
{
	...
	// RemapNodes is passed to RemapLinks for all nodes we are keeping. Nodes that we are keeping map to themselves.
	// Nodes that we are not keeping eventually either map to either null, or to a redirect that we are keeping.
	// While constructing it, all nodes we are not keeping map to null, and then we do a pass looking for redirects
	// and replace those nulls with redirects.
	TMap<FDependsNode*, FDependsNode*> RemapNodes;

	CachedDependsNodes.GenerateValueArray(AllDependsNodes);
	KeepAssetDataIndex.SetNum(AllAssetData.Num());
	RemapNodes.Reserve(AllDependsNodes.Num());
	for (FDependsNode* DependsNode : AllDependsNodes)
	{
		RemapNodes.Add(DependsNode, nullptr);
	}

	ParallelFor(AllAssetData.Num(),
		[&AllAssetData, &KeepAssetDataIndex, &RemapNodes, &NumRemovedAssetDatas,
		...
		(int32 AssetDataIndex)
		{
			...
			if (bKeepDependsNode)
			{
				FDependsNode** DependsNodePtr = CachedDependsNodes.Find(AssetData->PackageName);
				// We are keeping this node, so set its remap node to itself. 
				...
				// AtomicStore_Relaxed only supports integer types, so we have to cast our pointer and value to an integertype.
				if (DependsNodePtr)
				{
					FDependsNode* DependsNode = *DependsNodePtr;
					FDependsNode*& TargetDependsNode = RemapNodes.FindChecked(DependsNode);
					FPlatformAtomics::AtomicStore_Relaxed((PTRINT*)&TargetDependsNode, (PTRINT)DependsNode);
				}

				FPrimaryAssetId PrimaryAssetId = AssetData->GetPrimaryAssetId();
				if (PrimaryAssetId.IsValid())
				{
					DependsNodePtr = CachedDependsNodes.Find(PrimaryAssetId);
					if (DependsNodePtr)
					{
						FDependsNode* DependsNode = *DependsNodePtr;
						FDependsNode*& TargetDependsNode = RemapNodes.FindChecked(DependsNode);
	...
	int32 MaxId = RemapNodes.GetMaxIndex();
	ParallelFor(MaxId, [&RemapNodes, &Options, &PruneOptions](int32 Index)
		{
			FSetElementId SetId = FSetElementId::FromInteger(Index);
			if (!RemapNodes.IsValidId(SetId))
			{
				return;
			}
			TPair<FDependsNode*, FDependsNode*>& RemapPair = RemapNodes.Get(SetId);

			FDependsNode* DependsNode = RemapPair.Key;
			FDependsNode*& RemapNode = RemapPair.Value;
			bool bKeepDependsNode = DependsNode == RemapNode; // kept DependsNodes map to themselves.
			...
			if (Id.IsValue())
			{
				...
				bKeepDependsNode = !Options.bFilterSearchableNames;
			}
			else if (Id.PrimaryAssetType.IsValid())
			{
				...
				if (!PrimaryAssetId.IsValid())
				{
					UE_LOGF(LogAssetRegistry, Warning, "FAssetRegistryState::Prune: Unknown type of DependsNode: %ls", *Id.ToString());
					bKeepDependsNode = false;
				}
				else
				{
					...
					bKeepDependsNode |= !PruneOptions.bRemoveDependenciesWithoutPackages ||
			...
			else if (Id.IsPackage())
			{
				...
				bKeepDependsNode |= FPackageName::IsScriptPackage(WriteToString<256>(Id.PackageName));
			}
			...
			RemapNode = bKeepDependsNode ? DependsNode : nullptr;
		});

	// For pruned nodes, check for redirectors. If the redirection target is not pruned, mark that the pruned node maps
	// to the redirection target. Redirectors have to be searched for before removing any pruned asset datas, because
	// we use that AssetData information to check whether each asset is a redirector.
	TMap<FDependsNode*, FDependsNode*> RedirectCache;
	for (TPair<FDependsNode*, FDependsNode*>& RemapPair : RemapNodes)
	{
		if (!RemapPair.Value)
		{
			FDependsNode* DependsNode = RemapPair.Key;
			FDependsNode* TargetNode = ResolveRedirector(DependsNode, CachedAssets, RedirectCache);
			if (TargetNode != DependsNode)
			{
				bool bKeepTargetNode = (RemapNodes.FindChecked(TargetNode) == TargetNode);
				if (bKeepTargetNode)
				{
					RemapPair.Value = TargetNode;
				}
			}
		}
	}

	// Now that we have finished all our use for the pruned AssetDatas, remove them.
	...
	TMpscQueue<TPair<FDependsNode*, FDependsNode*>> ReferencersToAdd;
	TMpscQueue<FDependsNode*> PotentialOrphans;

	ParallelFor(MaxId, [&RemapNodes, &ReferencersToAdd, &PotentialOrphans, &Options, &PruneOptions](int32 Index)
		{
			FSetElementId SetId = FSetElementId::FromInteger(Index);
			if (!RemapNodes.IsValidId(SetId))
			{
			...
			TPair<FDependsNode*, FDependsNode*>& RemapPair = RemapNodes.Get(SetId);
			bool bKeepDependsNode = RemapPair.Key == RemapPair.Value; // kept DependsNodes map to themselves.
			if (bKeepDependsNode) 
			{
				FDependsNode* DependsNode = RemapPair.Key;
				DependsNode->RemapDependencies(
					[DependsNode, &RemapNodes, &ReferencersToAdd, &PotentialOrphans]
					(FDependsNode* ExistingDependsNode)
					{
						FDependsNode* Remapped = RemapNodes.FindChecked(ExistingDependsNode);

						// Remapped == Existing: it's a keeper.
						// Remapped == nullptr: pruned and either not redirected or target is also pruned; remove it.
						if (Remapped != ExistingDependsNode && Remapped != nullptr)
						{
							// Remapped == some other node: Apply the remap.
							// 
							// Since we are adding Remapped to DependsNode->Dependencies, we also need to add
							// DependsNode to Remapped->Referencers; we have to do that after leaving the
							// ParallelFor to avoid data race on Remapped->Referencers.
							// 
							// We don't worry about removing the Referencer from ExistingDependsNode, because we
							// are deleting that node and do not plan to read its Referencers anymore.
							ReferencersToAdd.Enqueue(TPair<FDependsNode*, FDependsNode*>{ Remapped, DependsNode });
						}
						return Remapped;
					});
				DependsNode->RemapReferencers([&RemapNodes](FDependsNode* ExistingDependsNode)
					{
						// Drop references from deleted nodes, no need to handle remapping them.
						FDependsNode* Remapped = RemapNodes.FindChecked(ExistingDependsNode);
						if (Remapped == ExistingDependsNode)
						{
							return ExistingDependsNode;
						}
						return (FDependsNode*)nullptr;
					});

				// Mark for remove any currently orphaned depends nodes. Wait until after ReferencersToAdd is
				// processed to decide whether they are orphaned, since ReferencersToAdd may add links to them.
				if (DependsNode->GetConnectionCount() == 0)
				{
					PotentialOrphans.Enqueue(DependsNode);
				}
			}
		});
	TPair<FDependsNode*, FDependsNode*> ReferencerPair;
	while (ReferencersToAdd.Dequeue(ReferencerPair))
	{
		ReferencerPair.Key->AddReferencer(ReferencerPair.Value);
	}
	FDependsNode* Orphan;
	while (PotentialOrphans.Dequeue(Orphan))
	{
		if (Orphan->GetConnectionCount() == 0)
		{
			RemapNodes.FindChecked(Orphan) = nullptr;
		}
	}
	...
	for (TPair<FDependsNode*, FDependsNode*>& RemapPair : RemapNodes)
	{
		bool bKeepDependsNode = RemapPair.Key == RemapPair.Value;
		if (!bKeepDependsNode)
		{
			FDependsNode* DependsNode = RemapPair.Key;
			...
			{
				DependsNode->ClearDependencies();
				DependsNode->ClearReferencers();
			}
		}
	}
}
...
FName FAssetRegistryState::GetFirstPackageByName(FStringView PackageName) const
{
	...
	if (LongPackageNames.Num() > 1)
	{
		...
		UE_LOGF(LogAssetRegistry, Warning,
			"GetFirstPackageByName('%.*ls') is returning '%ls', but it also found '%ls'%ls.",
...
TArray<FAssetData*> FAssetRegistryState::GetMutableAssetsSortedByObjectPath()
{
	...
	UE::FLinearBlockAllocator SharedAllocator(nullptr, 0U, ESPMode::NotThreadSafe);
	...
	ParallelForWithTaskContext(TEXT("GetMutableAssetsSortedByObjectPath"), ThreadAllocators, SortDatas.Num(),
		...
		{
			// ParallelFor passes return value into the FLinearBlockAllocatorThreadAccessor constructor.
...
bool FAssetRegistryState::Save(FArchive& OriginalAr, const FAssetRegistrySerializationOptions& Options)
{
	...
	else
	{
		...
		TUniqueFunction<int32(FDependsNode*, bool bAsReferencer)> GetSerializeIndexFromNode =
			[this, &DependsIndexMap](FDependsNode* InDependency, bool bAsReferencer)
		{
			if (!InDependency)
			{
				return -1;
...
{
	FAssetRegistryLoadResults Results;
	bool bResult = Load(OriginalAr, Options, Results);
	if (OutVersion)
	{
		*OutVersion = Results.Version;
	}
	return bResult;
}

bool FAssetRegistryState::Load(FArchive& OriginalAr, const FAssetRegistryLoadOptions& Options,
	FAssetRegistryLoadResults& OutResults)
{
	LLM_SCOPE(ELLMTag::AssetRegistry);
	FAssetRegistryHeader Header;
	Header.SerializeHeader(OriginalAr);
	OutResults.Version = Header.Version;
	OutResults.Store = FixedTagPrivate::FWeakStorePtr();
	...
	{
		FAssetRegistryReader Reader(OriginalAr, nullptr /* BackingFile */,
			Options.Store, Options.ParallelWorkers, Header);
		...
		Reader.WaitForTasks();
		OutResults.Store = FixedTagPrivate::FWeakStorePtr(Reader.GetTags());
	}
	...
	const FAssetRegistryLoadOptions& Options)
{
	...
	TArray<FAssetData*> AllocatedAssets;
	if (!Options.bMountPointsRequireSeparateAllocationBlocks)
	{
		AllocatedAssets.SetNumUninitialized(LocalNumAssets);
		...
		for (int32 Index = 0; Index < LocalNumAssets; ++Index)
		{
			AllocatedAssets[Index] = &PreallocatedAssetDataBuffer[Index];
		}
	}
	else
	{
		AllocatedAssets.SetNumZeroed(LocalNumAssets);
	}

	for (FAssetData*& NewAssetData : AllocatedAssets)
	{
		if (!NewAssetData)
		{
			NewAssetData = new FAssetData();
		}
		...
		if (Options.ShouldKeepPackage && !Options.ShouldKeepPackage(NewAssetData->PackageName))
		{
			if (!PreallocatedAssetDataBuffers.Num())
			{
				delete NewAssetData;
			}
			NewAssetData = nullptr;
		}
	}
	SetAssetDatas(AllocatedAssets, Options);
	...
	if (LocalNumPackageData > 0)
	{
		...
		for (int32 PackageDataIndex = 0; PackageDataIndex < LocalNumPackageData; PackageDataIndex++)
		{
			FName PackageName;
			Ar << PackageName;
			FAssetPackageData* NewPackageData = nullptr;
			if (!Options.bLoadPackageData ||
				(Options.ShouldKeepPackage && !Options.ShouldKeepPackage(PackageName)))
			{
			...
			else
			{
				...
				{
					NewPackageData = new FAssetPackageData();
				}
				CachedPackageData.Add(PackageName, NewPackageData);
				++NumPackageData;
			}
			check(NewPackageData);
	...
	FAssetRegistryVersion::Type Version)
{
	...
	{
		// ShouldKeepPackage is not supported in this legacy loading path
		check(!Options.ShouldKeepPackage);
		// bMountPointsRequireSeparateAllocationBlocks is not supported in this legacy loading path
		check(!Options.bMountPointsRequireSeparateAllocationBlocks);
	...
	else
	{
		...
		if (Options.bLoadDependencies)
		{
			if (Version == FAssetRegistryVersion::LatestVersion)
			{
				LoadDependenciesImpl<true>(Ar, Options, Version);
			}
			else
			{
				LoadDependenciesImpl<false>(Ar, Options, Version);
			}
		}
...
void FAssetRegistryState::LoadDependenciesImpl(FArchive& Ar, const FAssetRegistryLoadOptions& Options,
	FAssetRegistryVersion::Type Version)
{
	...
	TArray<FDependsNode*> AllocatedNodes;
	AllocatedNodes.SetNumUninitialized(LocalNumDependsNodes);
	if (!Options.bMountPointsRequireSeparateAllocationBlocks)
	{
		...
		for (int32 Index = 0; Index < LocalNumDependsNodes; ++Index)
		{
			AllocatedNodes[Index] = &PreallocatedDependsNodeDataBuffer[Index];
		}
	}
	else
	{
		for (FDependsNode*& DependsNode : AllocatedNodes)
		{
			DependsNode = new FDependsNode();
		}
	}
	CachedDependsNodes.Reserve(LocalNumDependsNodes);

	TUniqueFunction<FDependsNode*(int32)> GetNodeFromSerializeIndex =
		[&AllocatedNodes, LocalNumDependsNodes](int32 Index) -> FDependsNode*
	{
	...
	{
		for (FDependsNode* RemoveNode : RemoveNodes)
		{
			RemoveNode->IterateOverDependencies([&RemoveNodes, RemoveNode](FDependsNode* Dependency,
				UE::AssetRegistry::EDependencyCategory Category,
				UE::AssetRegistry::EDependencyProperty Properties,
				bool bDuplicate)
				{
					if (!bDuplicate && !RemoveNodes.Contains(Dependency))
					{
						Dependency->RemoveReferencer(RemoveNode);
					}
				});
			RemoveNode->IterateOverReferencers([&RemoveNodes, RemoveNode](FDependsNode* Referencer)
				{
					if (!RemoveNodes.Contains(Referencer))
					{
						Referencer->RemoveDependency(RemoveNode);
					}
				});
			RemoveNode->Empty();
		}
...
SIZE_T FAssetRegistryState::GetAllocatedSize(bool bLogDetailed) const
{
	...
	if (bLogDetailed)
	{
		UE_LOGF(LogAssetRegistry, Log, "AssetData Count: %d", CachedAssets.Num());
	...
	if (bLogDetailed)
	{
		UE_LOGF(LogAssetRegistry, Log, "Dependency Node Count: %d", CachedDependsNodes.Num());
	...
	if (bLogDetailed)
	{
		UE_LOGF(LogAssetRegistry, Log, "PackageData Count: %d", CachedPackageData.Num());
...
FDependsNode* FAssetRegistryState::ResolveRedirector(FDependsNode* InDependency,
													 ...
													 TMap<FDependsNode*, FDependsNode*>& InCache) const
{
...
void FAssetRegistryState::SetAssetDatas(TConstArrayView<FAssetData*> AssetDatas, const FAssetRegistryLoadOptions& Options)
{
	using namespace UE::AssetRegistry::Private;

	UE_CLOGF(NumAssets != 0, LogAssetRegistry, Fatal,
		"Can only load into empty asset registry states. Load into temporary and append using InitializeFromExisting() instead.");
...
void FAssetRegistryState::AddAssetData(FAssetData* AssetData)
{
	...
	if (bAlreadyInSet)
	{
		UE_LOGF(LogAssetRegistry, Error, "AddAssetData called with ObjectPath %ls which already exists. "
			"This will overwrite and leak the existing AssetData.", *FCachedAssetKey(*AssetData).ToString());
	}
	else
	{
		++NumAssets;
...
void FAssetRegistryState::AddTagsToAssetData(const FSoftObjectPath& InObjectPath, FAssetDataTagMap&& InTagsAndValues)
{
	...
	{
		UE_LOGF(LogAssetRegistry, Warning,
			"AddTagsToAssetData called with asset data that doesn't exist! Tags not added. ObjectPath: %ls",
...
void FAssetRegistryState::FilterTags(const FAssetRegistrySerializationOptions& Options)
{
	...
	{
#endif
		FilterTags(AssetData, Options);
		// Add the AssetData to all its CachedAssetsByTag/CachedClassesByTag keys even if FilterTags did not change
		// anything, because we are reconstructing all CachedAssetsByTag.
		for (auto TagIt = AssetData.TagsAndValues.CreateConstIterator(); TagIt; ++TagIt)
		{
#if UE_ASSETREGISTRY_CACHEDASSETSBYTAG
...
void FAssetRegistryState::FilterTags(FAssetData& InOutAssetData, const FAssetRegistrySerializationOptions& Options)
{
	FAssetDataTagMap LocalTagsAndValues;
	FilterTags(InOutAssetData.TagsAndValues, LocalTagsAndValues,
		Options.CookFilterlistTagsByClass.Find(InOutAssetData.AssetClassPath), Options);
	if (LocalTagsAndValues != InOutAssetData.TagsAndValues)
	{
		InOutAssetData.TagsAndValues = FAssetDataTagMapSharedView(MoveTemp(LocalTagsAndValues));
	}
	InOutAssetData.TaggedAssetBundles = FAssetRegistryState::FilterBundles(InOutAssetData.TaggedAssetBundles, Options);
}
...
void FAssetRegistryState::UpdateAssetData(FAssetData* AssetData, FAssetData&& NewAssetData, bool* bOutModified)
{
	...
	if (OldKey != NewKey)
	{
		...
		if (NumRemoved == 0)
		{
			UE_LOGF(LogAssetRegistry, Error,
				"UpdateAssetData called on AssetData %ls that is not present in the AssetRegistry.",
	...
	*AssetData = MoveTemp(NewAssetData);
	...
	if (OldKey != NewKey)
	{
		...
		{
			UE_LOGF(LogAssetRegistry, Error,
				"UpdateAssetData called with a change in ObjectPath from Old=\"%ls\" to New=\"%ls\", " "but the new ObjectPath is already present with another AssetData. This will overwrite and leak the existing AssetData.",
	...
	TBitArray<BitArrayAllocator>& bOutRemovedAssetDatas, TBitArray<BitArrayAllocator>& bOutRemovedPackageDatas)
{
	...
	{
		UE_CLOGF(!It.GetValue(), LogAssetRegistry, Error,
			"RemoveAssetData called on AssetData %ls that is not present in the AssetRegistry.",
...
bool FAssetRegistryState::RemoveDependsNodes(const TSet<FAssetIdentifier>& Identifiers,
	ERemoveDependsFlags Flags)
{
	using namespace UE::Tasks;

	const bool bRemoveOrphans = EnumHasAnyFlags(Flags, ERemoveDependsFlags::RemoveOrphans);
	...
	TMpscQueue<FDependsNode*> OrphansToRemoveQueue;
	TMap<FDependsNode*, TSet<FDependsNode*>> RemovedReferences;
	FTask PopulateTask = Launch(TEXT("RemoveDependsNode_PopulateSchedule"),
		[&NodesToRemove, &RemovedReferences, bRemoveOrphans, &OrphansToRemoveQueue]()
		{
			for (FDependsNode* Node : NodesToRemove)
			{
				...
				for (FDependsNode* ReferencerNode : ReferencerNodes)
				{
					...
					{
						ReferencerNode->RemoveDependency(Node);
						if (bRemoveOrphans && ReferencerNode->GetConnectionCount() == 0)
						{
							OrphansToRemoveQueue.Enqueue(ReferencerNode);
						}
					}
				}
			}
	...
	FTask RemoveTask = Launch(TEXT("RemoveDependsNode_RemoveReferencerSchedule"),
		[&RemovedReferences, bRemoveOrphans, &OrphansToRemoveQueue]()
		{
			...
			ParallelFor(TEXT("RemoveDependsNode_RemoveReferencerParallel"), DependenciesToUpdate.Num(), 1, 
				[&DependenciesToUpdate, &RemovedReferences, bRemoveOrphans, &OrphansToRemoveQueue](int32 Index)
				{
					...
					if (bRemoveOrphans && DependencyNode->GetConnectionCount() == 0)
					{
						OrphansToRemoveQueue.Enqueue(DependencyNode);
					}
	...
	const bool bDeleteRemovedNodes = PreallocatedDependsNodeDataBuffers.Num() == 0;
	FTask DeleteTask;
	if (bDeleteRemovedNodes)
	{
	...
	if (!OrphansToRemoveQueue.IsEmpty())
	{
		// It should be impossible to have duplicates in OrphansToRemove, but guarantee that with Algo::Unique.
		TArray<FDependsNode*> OrphansToRemove;
		FDependsNode* Orphan;
		while (OrphansToRemoveQueue.Dequeue(Orphan))
		{
			OrphansToRemove.Add(Orphan);
		}
		Algo::Sort(OrphansToRemove);
		int32 PreUniqueNum = OrphansToRemove.Num();
		OrphansToRemove.SetNum(Algo::Unique(OrphansToRemove));
		if (OrphansToRemove.Num() != PreUniqueNum)
		{
			UE_LOGFMT(LogAssetRegistry, Error,
				"Logic error in FAssetRegistryState::RemoveDependsNodes, OrphansToRemove had duplicates. PreUniqueNum={PreUniqueNum}, UniqueNum={UniqueNum}.",
				PreUniqueNum, OrphansToRemove.Num());
		}

		// We had to wait for RemoveTask to complete, which was running in parallel with DeleteTask, so we
		// can't lump these in to DeleteTask. Do a pass over them now to remove and delete them.
		for (FDependsNode* Node : OrphansToRemove)
		{
			int32 NumRemoved = CachedDependsNodes.Remove(Node->GetIdentifier());
			NumDependsNodes -= NumRemoved;
			if (bDeleteRemovedNodes)
			{
				delete Node;
			}
...
bool FAssetRegistryState::RemovePackageDatas(const TSet<FAssetIdentifier>& PackageNames)
{
	// Call the templated function (templated based on TSet allocator) for the default TSet allocator.
	return RemovePackageData(PackageNames);
}
...
void FAssetRegistryState::Dump(const TArray<FString>& Arguments, TArray<FString>& OutPages, int32 LinesPerPage) const
{
	...
	if (bAllFields || Arguments.Contains(TEXT("PackageData")))
	{
		...
		for (const FName& Key : Keys)
		{
			...
			PageBuffer << FIoHash();
#endif
			PageBuffer.Appendf(TEXT(" : %" INT64_FMT " bytes"), PackageData->DiskSize);
			AddLine();
		}
...
bool FAssetRegistryAssetPathStringsTest::RunTest(const FString& Parameters)
{
	...
	const FName TopLevelOuter = "/Path/To/PackageName";
	const FName DirectSubObjectOuter = "/Path/To/PackageName.OuterName";
	const FName SubSubObjectOuter = "/Path/To/PackageName.OuterName:SubOuterName";
	const FName AssetName = "AssetName";
