...
public:
	...
	*/
	...
	/** Similar to AppendState, but it moves rather than copies. @see FAssetRegistryState::ConsumeExisting. */
	void ConsumeState(Impl::FEventContext& EventContext, FAssetRegistryState&& InState, bool bEmitAssetEvents = false);
...
#endif
	void OnMultipleMountPointsMounted(UAssetRegistryImpl& UARI, Impl::FEventContext& EventContext,
	...
	void OnMultipleMountPointsDismounted(UAssetRegistryImpl& UARI, Impl::FEventContext& EventContext,
		Impl::FActiveMountEvents& ActiveMountEvents, UE::AssetRegistry::FInterfaceWriteScopeLock& ProofOfWriteLock,
	 ...
	 */
	void LogSearchDiagnostics(double StartTime, bool bInitialScan);
	...
	void GetProcessLoadedAssetsBatch(TQueue<TWeakObjectPtr<const UObject>, EQueueMode::Mpsc>& LoadedAssetsToProcess,
		TArray<const UObject*>& OutLoadedAssets, uint32 BatchSize, bool bUpdateDiskCacheAfterLoad);
	 ...
	 */
	void ConditionalLoadPremadeAssetRegistry(UE::AssetRegistry::FInterfaceWriteScopeLock& ScopeLock,
		UE::AssetRegistry::Impl::FEventContext& EventContext);

	/** Unload specified MountPoints, helper for OnMultipleMountPointsDismounted and others. */
	void UnloadMountPoints(Impl::FEventContext& EventContext,
...
private:
	...
	void CachePathsFromState(Impl::FEventContext& EventContext, const FAssetRegistryState& InState,
		const FAssetRegistrySerializationOptions* Options = nullptr);
	...
	void LoadPremadeAssetRegistry(UAssetRegistryImpl& UARI, Impl::FEventContext& Context,
		Impl::FActiveMountEvents& ActiveMountEvents, FInterfaceWriteScopeLock& ProofOfWriteLock,
...
// Normalization for LongPackageNames that identify a directory. Ensures it ends with a trailing forward-slash.
FString CreateStandardLongPackageNameDirectory(FStringView LongPackageName);
// Normalization for LongPackageNames that should not end in a trailing slash.
