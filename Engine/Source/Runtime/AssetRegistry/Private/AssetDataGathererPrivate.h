 ...
 */
class FScanDir : public FRefCountedObject
{
public:
...
private:
	...
	struct FDirToScanData
	{
		...
		// TStringBuilder<N> cannot be memmoved, so use TStringBuilder<0> without a buffer.
		TStringBuilder<0> DirLocalAbsPath;
		TStringBuilder<0> DirLongPackageName;
...
public:
	...
	static constexpr uint32 CacheSerializationMagic = 0xC2832E98; // Versioning and integrity checking
	...
	const FString& GetLegacyCacheBaseFilename() const;
	const FString& GetAssetRegistryCacheRootFolder() const;
	TArray<FString> FindShardedCacheFiles() const;
	TArray<FString> FindLegacyShardedCacheFiles() const;

private:
	FString CacheBaseFilename;
	FString LegacyCacheBaseFilename;
...
enum class ECacheCleanupContext
{
	PreLoad,
	PostWrite,
};
...
inline bool FGatherSettings::IsInitialized() const
{
	return bInitialized;
}
...
inline FString FGatherSettings::GetCacheDirectory() const
{
	return GetAssetRegistryCacheRootFolder() / TEXT("CachedAssetRegistry");
}
...
inline const FString& FGatherSettings::GetLegacyCacheBaseFilename() const
{
	return LegacyCacheBaseFilename;
}
