...
void FNonBufferingReadOnlyArchive::OpenFileLowLevel(const TCHAR* InFilename)
{
	...
	{
		UE_CLOGF(!IsSilent(), LogAssetRegistry, Warning, "OpenFile failed: File:%ls, Error:%ls",
...
bool FNonBufferingReadOnlyArchive::InternalPrecache(int64 PrecacheOffset, int64 PrecacheSize)
{
	...
	{
		UE_CLOGF(!IsSilent(), LogAssetRegistry, Warning, 
			"Buffer is not large enough to satisfy the requested read. BufferBase=%lld BufferSize=%lld BufferReadAlignment=%lld ReadCount=%lld Pos=%lld", BufferBase, BufferSize, BufferReadAlignment, ReadCount, Pos);
		return false;
	}
	...
	if (Count != ReadCount && Count != (Size - BufferBase))
	{
		...
		UE_CLOGF(!IsSilent(), LogAssetRegistry, Warning, "ReadFile failed: Count=%lld Pos=%lld Size=%lld BufferBase=%lld ReadCount=%lld BufferReadAlignment=%lld LastError=%u: %ls",
...
void FNonBufferingReadOnlyArchive::Serialize(void* V, int64 Length)
{
	...
	while (Length > 0)
	{
		...
		if (Copy <= 0 || bReadUncached)
		{
			...
			{
				SetError();
				UE_CLOGF(!IsSilent(), LogAssetRegistry, Warning, "ReadFile failed during precaching for file %ls", *Filename);
				return;
			}
			bReadUncached = false;
			...
			{
				SetError();
				UE_CLOGF(!IsSilent(), LogAssetRegistry, Error, "ReadFile beyond EOF %lld+%lld/%lld for file %ls",
