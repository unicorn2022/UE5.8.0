...
{
	UE_LOGF(LogAssetRegistry, Log, "*** Printing DependsNode: %ls ***", *GetIdentifier().ToString());
	UE_LOGF(LogAssetRegistry, Log, "*** Dependencies:");
	PrintDependencies();
	UE_LOGF(LogAssetRegistry, Log, "*** Referencers:");
	PrintReferencers();
}
...
template <uint32 FlagWidth>
void RemapLinks(const TUniqueFunction<FDependsNode* (FDependsNode*)>& Remap,
	TArray<FDependsNode*>& Dependencies,
	TBitArray<>* FlagBits,
	bool IsSorted,
	EAllowShrinking ShrinkPolicy)
{
	for (FDependsNode*& OldNode : Dependencies)
	{
		OldNode = Remap(OldNode);
	}
	::RemoveAll<FlagWidth>([](const FDependsNode* Node)
		{
			return Node == nullptr;
		},
		Dependencies, FlagBits, false /* IsSorted */, ShrinkPolicy);
	if (IsSorted)
	{
		::SortDependencyList<FlagWidth>(Dependencies, FlagBits, ShrinkPolicy);
	}
}

void FDependsNode::RemapLinks(const TUniqueFunction<FDependsNode* (FDependsNode*)>& Remap)
{
	RemapDependencies(Remap);
	RemapReferencers(Remap);
}

void FDependsNode::RemapDependencies(const TUniqueFunction<FDependsNode* (FDependsNode*)>& Remap)
{
	const EAllowShrinking ShrinkPolicy = bAllowShrinking ? EAllowShrinking::Yes : EAllowShrinking::No;
	::RemapLinks<PackageFlagWidth>(Remap, PackageDependencies, &PackageFlags, PackageIsSorted, ShrinkPolicy);
	::RemapLinks<SearchableNameFlagWidth>(Remap, NameDependencies, nullptr, SearchableNameIsSorted, ShrinkPolicy);
	::RemapLinks<ManageFlagWidth>(Remap, ManageDependencies, &ManageFlags, ManageIsSorted, ShrinkPolicy);
}

void FDependsNode::RemapReferencers(const TUniqueFunction<FDependsNode* (FDependsNode*)>& Remap)
{
	const EAllowShrinking ShrinkPolicy = bAllowShrinking ? EAllowShrinking::Yes : EAllowShrinking::No;
	::RemapLinks<0>(Remap, Referencers, nullptr, ReferencersIsSorted, ShrinkPolicy);
}
...
void FDependsNode::PrintDependenciesRecursive(const FString& Indent, TSet<const FDependsNode*>& VisitedNodes) const
{
	...
	{
		UE_LOGF(LogAssetRegistry, Log, "%lsNULL", *Indent);
	}
	else if ( VisitedNodes.Contains(this) )
	{
		UE_LOGF(LogAssetRegistry, Log, "%ls[CircularReferenceTo]%ls", *Indent, *GetIdentifier().ToString());
	}
	else
	{
		UE_LOGF(LogAssetRegistry, Log, "%ls%ls", *Indent, *GetIdentifier().ToString());
