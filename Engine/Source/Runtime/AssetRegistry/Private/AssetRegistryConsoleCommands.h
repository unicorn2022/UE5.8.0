...
public:
	...
	void GetByName(const TArray<FString>& Args)
	{
		if ( Args.Num() < 1 )
		{
			UE_LOGF(LogAssetRegistry, Log, "Usage: AssetRegistry.GetByName PackageName");
			return;
		}
		...
		UE_LOGF(LogAssetRegistry, Log, "GetAssetsByPackageName for %ls:", *AssetPackageName.ToString());
	...
	void GetByPath(const TArray<FString>& Args)
	{
		if ( Args.Num() < 1 )
		{
			UE_LOGF(LogAssetRegistry, Log, "Usage: AssetRegistry.GetByPath Path");
			return;
		}
		...
		UE_LOGF(LogAssetRegistry, Log, "GetAssetsByPath for %ls:", *AssetPath.ToString());
		for (int32 AssetIdx = 0; AssetIdx < AssetData.Num(); ++AssetIdx)
		{
			AssetData[AssetIdx].PrintAssetData();
		}
	...
	void GetByClass(const TArray<FString>& Args)
	{
		if ( Args.Num() < 1 )
		{
			UE_LOGF(LogAssetRegistry, Log, "Usage: AssetRegistry.GetByClass ClassPathname");
			return;
		}
		...
		if (!ClassPathName.IsNull())
		{
			...
			UE_LOGF(LogAssetRegistry, Log, "GetAssetsByClass for %ls:", *ClassPathName.ToString());
			for (int32 AssetIdx = 0; AssetIdx < AssetData.Num(); ++AssetIdx)
			{
				AssetData[AssetIdx].PrintAssetData();
			}
		...
		{
			UE_LOGF(LogAssetRegistry, Error, "not a valid class path name (E.g. /Script/Engine.Actor)");
		}
	}
	...
	void GetByTag(const TArray<FString>& Args)
	{
		if ( Args.Num() < 2 )
		{
			UE_LOGF(LogAssetRegistry, Log, "Usage: AssetRegistry.GetByTag TagName TagValue");
			return;
		}
		...
		UE_LOGF(LogAssetRegistry, Log, "GetAssetsByTagValues for Tag'%ls' and Value'%ls':", *Args[0], *Args[1]);
		for (int32 AssetIdx = 0; AssetIdx < AssetData.Num(); ++AssetIdx)
		{
			AssetData[AssetIdx].PrintAssetData();
		}
	...
	void GetDependencies(const TArray<FString>& Args)
	{
 		if ( Args.Num() < 1 )
 		{
 			UE_LOGF(LogAssetRegistry, Log, "Usage: AssetRegistry.GetDependencies PackageName");
 			return;
 		}
 		...
 		{
			UE_LOGF(LogAssetRegistry, Log, "Dependencies for %ls:", *PackageName.ToString());
			for ( auto DependencyIt = Dependencies.CreateConstIterator(); DependencyIt; ++DependencyIt )
			{
				UE_LOGF(LogAssetRegistry, Log, "   %ls", *(*DependencyIt).ToString());
			}
 		}
 		else
 		{
 			UE_LOGF(LogAssetRegistry, Log, "Could not find dependency data for %ls:", *PackageName.ToString());
 		}
	}
	...
	void GetReferencers(const TArray<FString>& Args)
	{
		if ( Args.Num() < 1 )
		{
			UE_LOGF(LogAssetRegistry, Log, "Usage: AssetRegistry.GetReferencers ObjectPath");
			return;
		}
		...
		{
			UE_LOGF(LogAssetRegistry, Log, "Referencers for %ls:", *PackageName.ToString());
			for ( auto ReferencerIt = Referencers.CreateConstIterator(); ReferencerIt; ++ReferencerIt )
			{
				UE_LOGF(LogAssetRegistry, Log, "   %ls", *(*ReferencerIt).ToString());
			}
		}
		else
		{
			UE_LOGF(LogAssetRegistry, Log, "Could not find referencer data for %ls:", *PackageName.ToString());
		}
	}
	...
	void FindInvalidUAssets(const TArray<FString>& Args)
	{
		...
		UE_LOGF(LogAssetRegistry, Log, "Invalid UAssets:");
		...
		for (int32 AssetIdx = 0; AssetIdx < AllAssets.Num(); ++AssetIdx)
		{
				...
				false /*bMatchCaseOnDisk*/, &PackagePath) != FPackageName::EPackageLocationFilter::None)
			{
				if (PackagePath.GetHeaderExtension() == EPackageExtension::Asset && !AssetData.IsUAsset())
				{
					...
					UE_LOGF(LogAssetRegistry, Log, "%ls", *AssetData.GetObjectPathString());
				}
			}
		}
	}
	...
	void ScanPath(const TArray<FString>& Args)
	{
		...
		if (InPath.IsEmpty())
		{
			UE_LOGF(LogAssetRegistry, Log, "Usage: AssetRegistry.ScanPath [-forcerescan] [-ignoreDenyLists] [-asfile] [-asdir] FileOrDirectoryPath");
			return;
		}
	...
	void DumpAllocatedSize(const TArray<FString>& Args)
	{
		...
		UE_LOGF(LogAssetRegistry, Log, "Total %2.f mb", double(Size) / 1024.0 / 1024.0);
	}
	...
	void DumpState(const TArray<FString>& Args)	
	{
		...
		{
			UE_LOGF(LogAssetRegistry, Error, "No arguments for asset registry dump.");
			return;
		}
		...
		for (const FString& Page : Pages)
		{
			if( bDoLog )
			{
				UE_LOGF(LogAssetRegistry, Log, "%ls", *Page);
			}
		...
		UE_LOGF(LogAssetRegistry, Display, "Dumped asset registry state to %ls.", *Path);
#else
		UE_LOGF(LogAssetRegistry, Error, "Asset registry dumping is disabled by compilation flags.");
#endif
	}
