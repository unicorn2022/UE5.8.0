...
bool bMemoryMappedCacheFiles = true;
static FAutoConsoleVariableRef CVarMemoryMappedCacheFiles(
	TEXT("AssetRegistry.MemoryMappedCacheFiles"),
	bMemoryMappedCacheFiles,
	TEXT("If true, asset registry cache file will be memory mapped, reducing memory usage"));
...
{
#if WITH_EDITOR
	FString EditorGameScansAR = "Sync";
...
void FGatherSettings::Initialize()
{
	...
	FString AssetRegistryDiscoveryCacheStr = "Default";
	...
	if (DiscoverySetting == EFeatureEnabledReadWrite::Invalid)
	{
		UE_LOGF(LogAssetRegistry, Error,
			"Invalid text \"%ls\" for Engine.ini:[AssetRegistry]:AssetRegistryDiscoveryCache. Expected \"Never\", \"Default\", or \"AlwaysWrite\".",
	...
	{
		bAsyncEnabled = false;
		UE_LOGF(LogAssetRegistry, Warning, "Requested asynchronous asset data gather, but threading support is disabled. Performing a synchronous gather instead!");
	}

	const TCHAR* CacheBaseName = bGatherDependsData ? TEXT("CachedAssetRegistry") : TEXT("CachedAssetRegistryNoDeps");
	LegacyCacheBaseFilename = AssetRegistryCacheRootFolder / CacheBaseName;
	CacheBaseFilename = GetCacheDirectory() / CacheBaseName;
...
// Returns true if the filename's base (no extension) is an old-style sharded cache binary:
// CachedAssetRegistry_N (exactly one underscore, followed by a pure-digit shard number).
// New-style names have multiple underscores: CachedAssetRegistry_N_YYYYMMDDTHHMMSS_XXXX.
static bool IsOldStyleShardBinaryName(FStringView BaseName)
{
	// Old-style names have exactly one underscore (the separator before the shard number).
	// New-style names have at least three underscores (shard, timestamp, hex suffix).
	int32 FirstUnderscore = INDEX_NONE;
	if (!BaseName.FindChar(TEXT('_'), FirstUnderscore))
	{
		return false;
	}
	// If there's a second underscore after the first, it's new-style.
	FStringView AfterFirst = BaseName.Mid(FirstUnderscore + 1);
	int32 SecondUnderscore = INDEX_NONE;
	if (AfterFirst.FindChar(TEXT('_'), SecondUnderscore))
	{
		return false; // Multiple underscores -> new-style
	}
	// Single underscore: the suffix must be a non-empty sequence of digits (the shard number).
	if (AfterFirst.IsEmpty())
	{
		return false;
	}
	for (TCHAR C : AfterFirst)
	{
		if (!FChar::IsDigit(C))
		{
			return false;
...
// Reads the binary filename stored in a .ref file. Returns empty string on any failure.
static FString ReadCacheRefFile(const FString& RefPath)
{
	FString Content;
	if (!FFileHelper::LoadFileToString(Content, *RefPath, FFileHelper::EHashOptions::None, FILEREAD_Silent))
	{
		return FString();
	}
...
// Atomically writes a binary filename (relative) to a .ref file via a temp file + rename.
static bool WriteCacheRefFile(const FString& RefPath, const FString& BinaryRelName)
{
	const FString TempPath = RefPath + TEXT(".tmp");
	if (!FFileHelper::SaveStringToFile(BinaryRelName, *TempPath, FFileHelper::EEncodingOptions::ForceUTF8WithoutBOM))
	{
		UE_LOGF(LogAssetRegistry, Warning, "Failed to write cache ref file %ls", *TempPath);
		return false;
	}
	if (!IFileManager::Get().Move(*RefPath, *TempPath))
	{
		UE_LOGF(LogAssetRegistry, Warning, "Failed to rename cache ref file %ls -> %ls", *TempPath, *RefPath);
...
// Returns a unique binary filename for a given shard.
// Format: CacheBase_N_GUID.bin
static FString MakeShardBinaryFilename(const FString& CacheBaseFilename, int32 Shard)
{
	return FString::Printf(TEXT("%s_%d_%s.bin"),
		*CacheBaseFilename, Shard,
		*FGuid::NewGuid().ToString(EGuidFormats::Digits));
}

// Returns the .ref filename for a given shard: CacheBase_N.ref
static FString GetShardRefFilename(const FString& CacheBaseFilename, int32 Shard)
{
	return FString::Printf(TEXT("%s_%d.ref"), *CacheBaseFilename, Shard);
}

// Attempts to delete cache binary files that are not referenced by any .ref file.
// Delete failures are silently skipped (e.g. file still in use).
void CleanupOrphanedCacheFiles(const FString& CacheBaseFilename, ECacheCleanupContext Context)
{
	const FString Directory = FPaths::GetPath(CacheBaseFilename);

	// Collect all binary filenames (lowercase) currently referenced by .ref files
	TArray<FString> RefFilenames;
	IFileManager::Get().FindFiles(RefFilenames, *(CacheBaseFilename + TEXT("_*.ref")), true, false);

	TSet<FString> ReferencedBinaryNames;
	for (const FString& RefFilename : RefFilenames)
	{
		FString BinaryRelName = ReadCacheRefFile(Directory / RefFilename);
		if (!BinaryRelName.IsEmpty())
		{
			ReferencedBinaryNames.Add(BinaryRelName.ToLower());
		}
	}

	UE_LOGF(LogAssetRegistry, Display, "CleanupOrphanedCacheFiles (%ls): %d .ref files found, %d referenced binaries",
		Context == ECacheCleanupContext::PreLoad ? TEXT("PreLoad") : TEXT("PostWrite"),
		RefFilenames.Num(), ReferencedBinaryNames.Num());

	// Delete any leftover .tmp files from interrupted writes (always safe to remove)
	TArray<FString> TmpFiles;
	IFileManager::Get().FindFiles(TmpFiles, *(CacheBaseFilename + TEXT("_*.tmp")), true, false);
	for (const FString& TmpFilename : TmpFiles)
	{
		IFileManager::Get().Delete(*(Directory / TmpFilename), false, false, true);
	}

	// Try to delete binary files that are not referenced by any .ref
	TArray<FString> AllBinaries;
	IFileManager::Get().FindFiles(AllBinaries, *(CacheBaseFilename + TEXT("_*.bin")), true, false);

	int32 DeletedCount = 0;
	int32 SkippedReferencedCount = 0;
	int32 SkippedOldStyleCount = 0;
	int32 FailedDeleteCount = 0;

	for (const FString& BinaryFilename : AllBinaries)
	{
		// Skip if still referenced
		if (ReferencedBinaryNames.Contains(BinaryFilename.ToLower()))
		{
			UE_LOGF(LogAssetRegistry, Verbose, "  %ls: kept (referenced by .ref)", *BinaryFilename);
			++SkippedReferencedCount;
			continue;
		}

		FStringView BaseName = FPathViews::GetBaseFilename(FStringView(BinaryFilename));
		if (IsOldStyleShardBinaryName(BaseName))
		{
			if (Context == ECacheCleanupContext::PreLoad)
			{
				// PreLoad: keep old-style bins - FindShardedCacheFiles needs them for migration fallback.
				UE_LOGF(LogAssetRegistry, Verbose, "  %ls: kept (old-style, needed for migration fallback)", *BinaryFilename);
				++SkippedOldStyleCount;
				continue;
			}
			// PostWrite: all shards have .ref files now, old-style bins are fully superseded.
		}

		// Deletion may fail if the file is still in use; that is expected and fine.
		if (IFileManager::Get().Delete(*(Directory / BinaryFilename), false, false, true))
		{
			UE_LOGF(LogAssetRegistry, Display, "  %ls: deleted (orphaned, unreferenced by any .ref)", *BinaryFilename);
			++DeletedCount;
		}
		else
		{
			++FailedDeleteCount;
			UE_LOGF(LogAssetRegistry, Display,
				"  %ls: delete failed (orphaned but still in use)",
				*BinaryFilename);
		}
	}

	UE_LOGF(LogAssetRegistry, Display,
		"CleanupOrphanedCacheFiles: %d total binaries, %d referenced (kept), %d old-style pre-migration (kept), %d orphans deleted, %d orphans locked (kept)",
		AllBinaries.Num(), SkippedReferencedCount, SkippedOldStyleCount, DeletedCount, FailedDeleteCount);

	// Also clean up legacy cache files in the old location (Intermediate/CachedAssetRegistry_N.bin).
	// In PostWrite context all shards have .ref files, so legacy files are fully superseded.
	// In PreLoad context keep them if there are no .ref files (migration fallback may still need them).
	const FString& LegacyBase = GGatherSettings.GetLegacyCacheBaseFilename();
	if (!LegacyBase.IsEmpty() && LegacyBase != CacheBaseFilename)
	{
		const FString LegacyDir = FPaths::GetPath(LegacyBase);
		TArray<FString> LegacyBinaries;
		IFileManager::Get().FindFiles(LegacyBinaries, *(LegacyBase + TEXT("_*.bin")), true, false);

		if (LegacyBinaries.Num() > 0)
		{
			bool bShouldDeleteLegacy = (Context == ECacheCleanupContext::PostWrite) || (RefFilenames.Num() > 0);
			if (bShouldDeleteLegacy)
			{
				int32 LegacyDeletedCount = 0;
				int32 LegacyFailedCount = 0;
				for (const FString& LegacyBin : LegacyBinaries)
				{
					if (IFileManager::Get().Delete(*(LegacyDir / LegacyBin), false, false, true))
					{
						++LegacyDeletedCount;
					}
					else
				...
				UE_LOGF(LogAssetRegistry, Display,
					"CleanupOrphanedCacheFiles: legacy location %ls: %d deleted, %d locked (kept)",
					*LegacyDir, LegacyDeletedCount, LegacyFailedCount);
			}
			else
			{
				UE_LOGF(LogAssetRegistry, Display,
					"CleanupOrphanedCacheFiles: legacy location %ls: %d files kept (no .ref files yet, needed for migration fallback)",
					*LegacyDir, LegacyBinaries.Num());
			}
		}

		// Also delete legacy monolithic cache file (CachedAssetRegistry.bin) if it exists
		IFileManager::Get().Delete(*(LegacyBase + TEXT(".bin")), false, false, true);
	}
}

TArray<FString> FGatherSettings::FindShardedCacheFiles() const
{
	const FString Directory = FPaths::GetPath(GetCacheBaseFilename());
	TArray<FString> Result;

	// Primary path: find all .ref files and resolve them to binary paths
	TArray<FString> RefFilenames;
	IFileManager::Get().FindFiles(RefFilenames, *(GetCacheBaseFilename() + TEXT("_*.ref")), true, false);

	TSet<int32> HandledShards;
	for (const FString& RefFilename : RefFilenames)
	{
		const FString RefPath = Directory / RefFilename;
		const FString BinaryRelName = ReadCacheRefFile(RefPath);
		if (BinaryRelName.IsEmpty())
		{
			UE_LOGF(LogAssetRegistry, Warning, "Cache ref file is empty or unreadable: %ls", *RefPath);
			continue;
		}
		const FString BinaryPath = Directory / BinaryRelName;
		if (!IFileManager::Get().FileExists(*BinaryPath))
		{
			UE_LOGF(LogAssetRegistry, Warning,
				"Cache ref file %ls points to missing binary %ls, shard will be regenerated",
				*RefPath, *BinaryPath);
			continue;
		}
		Result.Add(BinaryPath);

		// Track which shard number this .ref covers (for migration fallback below)
		FStringView RefBase = FPathViews::GetBaseFilename(FStringView(RefFilename));
		int32 LastUnderscore = INDEX_NONE;
		RefBase.FindLastChar(TEXT('_'), LastUnderscore);
		if (LastUnderscore != INDEX_NONE)
		{
			int32 ShardNum = 0;
			LexFromString(ShardNum, RefBase.Mid(LastUnderscore + 1));
			HandledShards.Add(ShardNum);
		}
	}

	// Migration fallback: include old-style _N.bin files for shards not yet covered by a .ref
	int32 MigrationCount = 0;
	TArray<FString> OldStyleBinaries;
	IFileManager::Get().FindFiles(OldStyleBinaries, *(GetCacheBaseFilename() + TEXT("_*.bin")), true, false);
	for (const FString& BinaryFilename : OldStyleBinaries)
	{
		FStringView BaseName = FPathViews::GetBaseFilename(FStringView(BinaryFilename));
		if (!IsOldStyleShardBinaryName(BaseName))
		{
			continue; // new-style binary; covered by the .ref lookup above
		}
		int32 LastUnderscore = INDEX_NONE;
		BaseName.FindLastChar(TEXT('_'), LastUnderscore);
		int32 ShardNum = 0;
		LexFromString(ShardNum, BaseName.Mid(LastUnderscore + 1));
		if (!HandledShards.Contains(ShardNum))
		{
			UE_LOGF(LogAssetRegistry, Display, "FindShardedCacheFiles: migration fallback for shard %d -> %ls", ShardNum, *BinaryFilename);
			Result.Add(Directory / BinaryFilename);
			++MigrationCount;
		}
	}

	UE_LOGF(LogAssetRegistry, Display, "FindShardedCacheFiles: %d via .ref, %d via migration fallback, %d total",
		Result.Num() - MigrationCount, MigrationCount, Result.Num());

	return Result;
}

TArray<FString> FGatherSettings::FindLegacyShardedCacheFiles() const
{
	TArray<FString> CachePaths;
	IFileManager::Get().FindFiles(CachePaths, *(GetLegacyCacheBaseFilename() + TEXT("_*.bin")), /* Files */ true, /* Directories */ false);
	if (CachePaths.Num())
	{
		FString Directory = FPaths::GetPath(GetLegacyCacheBaseFilename());
...
// Attempts to lock a system-wide mutex guarding the writing of AssetRegistry cache files. If already locked, no waiting occurs.
// Users should verify the lock state after calling this function.
[[nodiscard]] UE::FPlatformSystemWideMutex TryLockSystemWideCacheFilesWrites()
{
	...
	return UE::FPlatformSystemWideMutex(Builder.ToString());
}
#endif
...
TArray<FCachePayload> LoadCacheFiles(TConstArrayView<FString> CacheFilenames, bool bCacheWriteEnabled);
void DeleteCacheFilesWithinLock();
...
FScanDir* FScanDir::GetControllingDir(FStringView InRelPath, bool bIsDirectory, const FInherited& ParentData, FInherited& OutData, FString& OutRelPath)
{
	...
	if (InRelPath.IsEmpty())
	{
		if (!bIsDirectory)
		{
			UE_LOGF(LogAssetRegistry, Warning, "GetControllingDir called on %ls with !bIsDirectory, but we have it recorded as a directory. Returning null.", *GetLocalAbsPath());
			OutData = FInherited();
			OutRelPath.Reset();
			return nullptr;
		}
...
void FMountDir::AddChildMount(FMountDir* ChildMount)
{
	...
	{
		UE_LOGF(LogAssetRegistry, Warning,
			"AssetDataGatherer directory %.*ls has already started scanning when a new mountpoint was added under it at %.*ls. "
			"Assets in the new mount point may exist twice in the AssetRegistry under two different package names.",
...
void FMountDir::RemoveChildMount(FMountDir* ChildMount)
{
	...
	{
		UE_LOGF(LogAssetRegistry, Warning,
			"AssetDataGatherer directory %.*ls has already started scanning when it was removed and merged into its parent mount at %.*ls. "
			"Assets in the new mount point may exist twice in the AssetRegistry under two different package names.",
	...
	{
		{
				...
				for (FDirToScanData& Data : TArrayView<FDirToScanData>(DirToScanDatas.GetData(), DirToScanDatasNum))
				{
					if (Data.bScanned)
					{
						...
						else if (Data.ScanDir->IsScanInFlightInvalidated())
						{
							...
							UE_LOGF(LogAssetRegistry, Display, "ScanDir %ls has been invalidated (Current State: %ls). Will not scan.", *Data.ScanDir->GetLocalAbsPath(), (Data.ScanDir->IsValid() ? TEXT("valid") : TEXT("invalid")));
						}
						else
						{
			...
			if (DirToScanDatasNum == 0)
			{
				...
				if (LocalNumDirectoriesToScan != 0)
				{
					...
					UE_LOGF(LogAssetRegistry, Warning, "FAssetDataDiscovery::SetIsIdle(true) called when NumDirectoriesToScan == %d.\n" "First incomplete scandir: %ls", LocalNumDirectoriesToScan, Incomplete ? *Incomplete->GetLocalAbsPath() : TEXT("<NoneFound>"));
				}
		...
		[this](FDirToScanBuffer& ScanBuffer, int32 DirToScanDatasIndex)
		{
			...
			(const TCHAR* IterFilename, bool bIsDirectory, const FDateTime& ModificationTime, bool bIsReadOnly, FFileJournalFileHandle JournalHandle, bool bIsReparsePoint)
			{
				...
				if (!FPathViews::TryMakeChildPathRelativeTo(IterFilename, Data.DirLocalAbsPath, RelPath))
				{
					...
					{
						UE_LOGF(LogAssetRegistry, Warning,
							"IterateDirectory returned unexpected result %ls which is not a child of the requested path %ls.",
				...
				{
					UE_LOGF(LogAssetRegistry, Warning,
						"IterateDirectory returned unexpected result %ls which is not a direct child of the requested path %ls.",
						IterFilename, Data.DirLocalAbsPath.ToString());
					return true;
				}
				int32 DirLongPackageRootNameLen = Data.DirLongPackageName.Len();
				...
				else
				{
					...
					if (bRunOnce)
					{
							...
							}, &OutError))
						{
							...
							UE_LOGF(LogAssetRegistry, Warning, "Failed to FileJournalIterateDirectory, failing to cache this directory due to:\n  %ls", *OutError);
...
void FAssetDataDiscovery::SetIsIdle(bool bInIsIdle, double& TickStartTime)
{
	...
	if (!IsSynchronous())
	{
		if (bIsIdle)
		{
			...
			UE_LOGF(LogAssetRegistry, Verbose,
				"Discovery took %0.4f seconds to add %d files, Cumulative=%0.4f seconds to add %d.",
...
void FAssetDataDiscovery::GetAndTrimSearchResults(bool& bOutIsComplete, TArray<FString>& OutDiscoveredPaths, FFilesToSearch& OutFilesToSearch, int32& OutNumPathsToSearch)
{
	...
	{
		UE_LOGF(LogAssetRegistry, Warning, "FAssetDataDiscovery::GetAndTrimSearchResults is returning bIsIdle=true while OutNumPathsToSearch=%d.",
			OutNumPathsToSearch);
	}
}
...
bool TryFixSymlinkCapitalization(FString& LocalAbsPath, FString& CorrectedCapitalization)
{
	// try to find the last junction / symlink 
	FString Path = FString(FPathViews::GetPath(LocalAbsPath));
	while (!Path.IsEmpty() && !IFileManager::Get().IsSymlink(*Path))
	{
		Path = FString(FPathViews::GetPath(Path));
	}

	// We couldn't find a symlink on the path, we don't know why the capitalization is wrong.
	if (Path.IsEmpty())
	{
		return false;
	}

	FStringView LocalAbsPathAfterJunction(LocalAbsPath);
	LocalAbsPathAfterJunction = LocalAbsPathAfterJunction.RightChop(Path.Len());

	FStringView CorrectedCapitalizationAfterJunction(CorrectedCapitalization);
	CorrectedCapitalizationAfterJunction = CorrectedCapitalizationAfterJunction.RightChop(CorrectedCapitalizationAfterJunction.Len() - LocalAbsPathAfterJunction.Len());

	// we expect something like: [junction]/foo/bar.uasset <-> [fullpath]/Foo/BAR.uasset
	if (!LocalAbsPathAfterJunction.Equals(CorrectedCapitalizationAfterJunction, ESearchCase::IgnoreCase))
	{
		//unexpected but no need for a separate error
		return false;
	}

	int LocalAbsPathIt = LocalAbsPath.Len() - 1;
	int CorrectedCapitalizationIt = CorrectedCapitalization.Len() - 1;
	for (int i = 0; i < LocalAbsPathAfterJunction.Len(); ++i)
	{
		LocalAbsPath[LocalAbsPathIt--] = CorrectedCapitalization[CorrectedCapitalizationIt--];
	}
...
void FPathExistence::LoadExistenceData()
{
	...
	if (StatData.bIsValid)
	{
		...
		else // if the case insensitive comparison above failed, there might be junction / symlink in the path
		{
			if (!TryFixSymlinkCapitalization(LocalAbsPath, CorrectedCapitalization))
			{
				UE_LOGF(LogAssetRegistry, Warning,
					"FPathExistence failed to gather correct capitalization from disk for %ls, because GetFilenameOnDisk returned non-matching filename '%ls'.",
	...
	bool bForceRescan, bool bIgnoreDenyListScanFilters)
{
	...
	bool bTickOwner = false;
	{
		...
		for (FPathExistence& QueryPath : QueryPaths)
		{
			...
			if (!MountDir)
			{
				UE_LOGF(LogAssetRegistry, Log, "SetPropertiesAndWait called on %ls which is not in a mounted directory. Call will be ignored.",
			...
			{
				UE_LOGF(LogAssetRegistry, Verbose, "SetPropertiesAndWait called on %ls which is not monitored. Call will be ignored.",
					*QueryPath.GetLocalAbsPath());
				continue;
			}
			...
			if (PathType == FPathExistence::EType::MissingParentDir)
			{
				...
				UE_LOGF(LogAssetRegistry, Verbose, "SetPropertiesAndWait called on non-existent path %ls. Call will be ignored.",
					*QueryPath.GetLocalAbsPath());
				continue;
			}
...
void FAssetDataDiscovery::PrioritizeSearchPath(const FString& LocalAbsPath, EPriority Priority)
{
	...
	if (!MountDir)
	{
		UE_LOGF(LogAssetRegistry, Warning, "FAssetDataGatherer::PrioritizeSearchPath called on unmounted path %.*ls. Call will be ignored.",
	...
	const FSetPathProperties& InProperties)
{
	...
	{
		UE_LOGF(LogAssetRegistry, Warning,
			"FAssetDataGatherer::SetDirectoryProperties called on unmounted path %ls. Call will be ignored.",
			*QueryPath.GetLocalAbsPath());
		return;
	}
...
void FAssetDataDiscovery::AddMountPoint(const FString& LocalAbsPath, FStringView LongPackageName, bool& bOutAlreadyExisted)
{
	CHECK_IS_NOT_LOCKED_CURRENT_THREAD(ResultsLock);
	FGathererScopeLock TreeScopeLock(&TreeLock);
	AddMountPointInternal(LocalAbsPath, LongPackageName, bOutAlreadyExisted);
...
// this namespace contains deprecated code
namespace AssetRegistryBackwardCompat
{
...
TOptional<FBlockHeader> LoadBlockHeader(FMemoryView Data)
{
	...
	{
		UE_LOGF(LogAssetRegistry, Warning, "Wrong block magic (0x%x)", Header.Magic);
		return {};
	}
...
class FChecksumArchiveBase : public FArchiveProxy
{
	...
	bool LoadBlock()
	{
		...
		{
			UE_LOGF(LogAssetRegistry, Warning, "Couldn't read block header");
			return false;
		}
		...
		if (TOptional<FBlockHeader> Header = LoadBlockHeader(MakeMemoryView(HeaderData)))
		{
			...
			if (InnerArchive.IsError())
			{
				UE_LOGF(LogAssetRegistry, Warning, "Couldn't read block data");
				return false;
			}
			else if (CalculateBlockChecksum(Block.GetRemaining()) != Header->Checksum)
			{
				UE_LOGF(LogAssetRegistry, Warning, "Wrong block checksum");
				return false;
			}
...
protected:
	...
	void Load(FMutableMemoryView Data)
	{
		...
		for (uint64 Size = Block.GetRemainingSize(); Size < Data.GetSize(); Size = Block.GetRemainingSize())
		{
			...
			if (!LoadBlock())
			{
				UE_LOGF(LogAssetRegistry, Warning, "Integrity check failed, '%ls' cache will be discarded", *InnerArchive.GetArchiveName());
				SetError();
				return;
			}
		}
...
private:
	...
	virtual void Serialize(void* V, int64 Len) override
	{
		...
		while (CurrentBlock.GetRemainingSize() < Out.GetSize())
		{
			...
			if (!NextBlock)
			{
				UE_LOGF(LogAssetRegistry, Warning, "Integrity check failed, '%ls' cache will be discarded", *FileName);
				SetError();
				return;
			}
	...
	FORCENOINLINE static TOptional<FMemoryView> LoadNextBlock(FMemoryViewReader& In)
	{
		...
		if (TOptional<FBlockHeader> Header = LoadBlockHeader(In.Load(sizeof(FBlockHeader))))
		{
			...
			{
				UE_LOGF(LogAssetRegistry, Warning, "Incomplete block");
				return {};
			}
...
private:
	...
	void DelayedInitialize()
	{
		...
		if (GGatherSettings.IsPreloadGatherCache())
		{
			...
			{
				CachePaths = GGatherSettings.FindLegacyShardedCacheFiles();
				if (CachePaths.Num())
				{
					UE_LOGF(LogAssetRegistry, Display, "Preload: found %d legacy cache files for migration", CachePaths.Num());
				}
			}
			if (CachePaths.Num())
			{
	...
	{
		LLM_SCOPE(ELLMTag::AssetRegistry);
		CleanupOrphanedCacheFiles(GGatherSettings.GetCacheBaseFilename(), ECacheCleanupContext::PreLoad);
		GatherCachePayloads = LoadCacheFiles(Paths, bCacheWriteEnabled);
	}
...
FAssetDataGatherer::FAssetDataGatherer(UAssetRegistryImpl& InRegistryImpl)
	...
	, DelayedSaveAsyncCacheStartTime(0.0)
	, CurrentSearchTime(0.)
...
FAssetDataGatherer::~FAssetDataGatherer()
{
	...
	// Best-effort cleanup of orphaned binaries. Files still memory-mapped by this
	// or another process will fail to delete and be cleaned up on next startup.
	UE::AssetDataGather::Private::CleanupOrphanedCacheFiles(
		UE::AssetDataGather::Private::GGatherSettings.GetCacheBaseFilename(),
		UE::AssetDataGather::Private::ECacheCleanupContext::PostWrite);
}
...
void FAssetDataGatherer::OnAdditionalMountSearchCompleted()
{
	...
	// If we haven't done so already, request a delayed save of the cache.
	// Since additional mounts may be added in quick succession we want to save the cache but only
	// after a brief period of time (i.e. FGatherSettings::MinSecondsToElapseBeforeCacheWrite) 
	// to avoid saving the entire cache multiples times in a row.
	RequestDelayedAsyncCacheSave();
...
uint32 FAssetDataGatherer::Run()
{
	...
	while (!IsStopped || IsCacheSaveReady())
	{
		bool bShouldRequestAssetRegistryTick = false;
		ETickResult TickResult = InnerTickLoop(false /* bInSynchronousTick */, true /* bContributeToCacheSave */,
			-1. /* EndTimeSeconds */, &bShouldRequestAssetRegistryTick);

		if (bShouldRequestAssetRegistryTick)
		{
...
FAssetDataGatherer::ETickResult FAssetDataGatherer::InnerTickLoop(bool bInSynchronousTick,
	bool bContributeToCacheSave, double EndTimeSeconds, bool* bShouldRequestAssetRegistryTick)
{
	using namespace UE::AssetDataGather::Private;
	...
	TArray<TPair<FName, FDiskCachedAssetData*>> AssetsToSave;
	{
		...
		for (;;)
		{
			ETickResult TickResult = TickInternal(TickStartTime, bPollDiscovery, bShouldRequestAssetRegistryTick);
			if (EndTimeSeconds > 0. && FPlatformTime::Seconds() > EndTimeSeconds)
			{
	...
	if (bShouldSaveCache)
	{
		TrySaveCacheFile(AssetsToSave, true /*bRequestSaveOnFailure*/);
	}
	return Result;
}
...
bool FAssetDataGatherer::ShouldBackgroundGatherThreadPauseGathering(bool& bOutGathererIsIdle)
{
	...
	return !IsStopped && !IsCacheSaveReady() && (IsGatheringPaused || bIsIdle);
}

UE::AssetRegistry::Impl::EGatherStatus FAssetDataGatherer::TryTickOnBackgroundThread()
{
	// TickOnBackgroundThread calls this->GetAndTrimSearchResults, which enters ResultsLock and might enter TickLock,
	// so assert that we are not already holding them.
...
bool FAssetDataGatherer::TrySaveCacheFile(const TArray<TPair<FName,FDiskCachedAssetData*>>& AssetsToSave, bool bRequestSaveOnFailure)
{
	using namespace UE::AssetDataGather::Private;
	...
	bool bSuccess = true;
	FString CorruptTagText;
	bool bCorruptTagDetected = false;
	...
	TArray<TArray<TPair<FName, FDiskCachedAssetData*>>> DataPerShard;
	{
		...
		for (const TPair<FName, FDiskCachedAssetData*>& Entry : AssetsToSave)
		{
			...
			// UE-372209: Work-around a not-yet-diagnosed AssetRegistry corruption bug that is allowing empty strings
			// to be present in some AssetRegistry tag values. Delete the old cache and skip saving the cache if that
			// has occurred; we don't know the extent of the corruption.
			for (const FAssetData& AssetData : Entry.Value->AssetDataList)
			{
				if (bCorruptTagDetected)
				{
					break;
				}
				...
				{
					if (TagAndValue.Value.GetStorageString().IsEmpty())
					{
						bCorruptTagDetected = true;
						CorruptTagText = FString::Printf(
							TEXT("AssetRegistry corruption detected: the TagValue on an AssetData is empty string, which is invalid. ")
							TEXT("The existing cache will be deleted and the new cache save will be skipped.")
							TEXT("\n\tAsset: '%s'")
							TEXT("\n\tKey: '%s', Value: '%s', IsSet: %s, IsFixed: %s"),
							*AssetData.ToSoftObjectPath().ToString(), *TagAndValue.Key.ToString(), *TagAndValue.Value.GetStorageString(),
							*LexToString(TagAndValue.Value.IsSet()), *LexToString(TagAndValue.Value.IsFixed()));
						break;
					}
				}
			}
	...
	if (CurrentTime < LocalLastCacheWriteTime + WaitTimeBeforeReopen)
	{
		FPlatformProcess::Sleep(static_cast<float>(LocalLastCacheWriteTime + WaitTimeBeforeReopen - CurrentTime));
	}

#if CACHEFILE_SYSTEMWIDE_WRITELOCK
	// Attempt to get exclusive write access to the cache files. If we cannot, skip writing
	// and try again next tick. Failing to get the lock here means another process 
	// is currently writing so we avoid the possibility of never writing cache files 
	UE::FPlatformSystemWideMutex CacheFilesLock = TryLockSystemWideCacheFilesWrites();
	if (!CacheFilesLock.IsValid())
	{
		bSuccess = false;
		if (bRequestSaveOnFailure && !IsStopped.load(std::memory_order_relaxed))
		{
			RequestAsyncCacheSave();
		}
		// Intentionally fall-through to perform necessary cleanup
	}
	else
#endif
	{
		if (ensureMsgf(!bCorruptTagDetected, TEXT("%s"), *CorruptTagText))
		{
			IFileManager::Get().MakeDirectory(*GGatherSettings.GetCacheDirectory(), true);

			// Write each shard to a new uniquely-named binary file, then update its .ref.
			ParallelFor(CacheShards, [this, &TotalCacheSize, &DataPerShard](int32 Shard)
			{
				TArray<TPair<FName, FDiskCachedAssetData*>>& ShardData = DataPerShard[Shard];
				FString BinaryFilename = MakeShardBinaryFilename(GGatherSettings.GetCacheBaseFilename(), Shard);
				int64 CacheSize = SaveCacheFileInternal(BinaryFilename, ShardData);
				if (CacheSize > 0)
				{
					FString RefFilename = GetShardRefFilename(GGatherSettings.GetCacheBaseFilename(), Shard);
					if (WriteCacheRefFile(RefFilename, FPaths::GetCleanFilename(BinaryFilename)))
					{
						TotalCacheSize += CacheSize;
					}
					else
					{
						// .ref update failed; delete the orphaned binary so it doesn't accumulate.
						// The previous .ref still points to a valid (older) binary for this shard.
						IFileManager::Get().Delete(*BinaryFilename, false, false, true);
					}
				}
			}, EParallelForFlags::BackgroundPriority);

			UE_LOGF(LogAssetRegistry, Display, "Asset registry cache written as %.1f MiB to %ls_*.ref",
				static_cast<float>(TotalCacheSize) / 1024.f / 1024.f, *GGatherSettings.GetCacheBaseFilename());

			// Delete any excess .ref files if the number of shards was reduced
			{
				const FString CacheDirectory = FPaths::GetPath(GGatherSettings.GetCacheBaseFilename());
				TArray<FString> ExistingRefFiles;
				IFileManager::Get().FindFiles(ExistingRefFiles, *(GGatherSettings.GetCacheBaseFilename() + TEXT("_*.ref")), true, false);
				for (const FString& RefFilename : ExistingRefFiles)
				{
					FStringView BaseName = FPathViews::GetBaseFilename(FStringView(RefFilename));
					if (int32 Index; BaseName.FindLastChar('_', Index))
					{
						int32 ShardNum = 0;
						LexFromString(ShardNum, BaseName.Mid(Index + 1));
						if (ShardNum >= CacheShards)
						{
							IFileManager::Get().Delete(*(CacheDirectory / RefFilename), false, false, true);
						}
					}
				}
				// Old-style _N.bin files (including excess shards) are cleaned up by
				// CleanupOrphanedCacheFiles(PostWrite) which runs after the write lock is released.
			}
		}
		else
		{
			// Corruption detected, delete the cache files
			UE::AssetDataGather::Private::DeleteCacheFilesWithinLock();
			// Report this as a "successful" write, so that we stop attempting to write the cache files.
			bSuccess = true;
		}
	...
	// Best-effort cleanup of orphaned binary files outside the write lock.
	CleanupOrphanedCacheFiles(GGatherSettings.GetCacheBaseFilename(), ECacheCleanupContext::PostWrite);

	FScopedGatheringPause ScopedPause(*this);
	FGathererScopeLock TickScopeLock(&TickLock);
	check(bIsSavingAsyncCache);
	bIsSavingAsyncCache = false;
...
FAssetDataGatherer::ETickResult FAssetDataGatherer::TickInternal(double& TickStartTime, bool bPollDiscovery, bool* bShouldRequestAssetRegistryTick)
{
	using namespace UE::AssetDataGather::Private;
	...
	if (bShouldRequestAssetRegistryTick)
	{
		*bShouldRequestAssetRegistryTick = false;
	}
	...
	if (bLoadCache)
	{
...
#if UE_EDITOR
		bGatherCacheWriteEnabled = GGatherSettings.IsGatherCacheWriteEnabled();
		if (GGatherSettings.IsPreloadGatherCache())
		{
...
#endif
		{
			...
			if (CachePaths.IsEmpty())
			{
				...
				if (CachePaths.Num())
				{
					UE_LOGF(LogAssetRegistry, Display, "Found %d legacy cache files for migration from %ls",
						CachePaths.Num(), *GGatherSettings.GetLegacyCacheBaseFilename());
				}
			}
			Payloads = LoadCacheFiles(CachePaths, bGatherCacheWriteEnabled);
		}
		ConsumeCacheFiles(MoveTemp(Payloads));
		UE_LOGF(LogAssetRegistry, Display, "AssetDataGatherer spent %.3fs loading caches from %ls.",
			FPlatformTime::Seconds() - CacheLoadStartTime, *GGatherSettings.GetCacheDirectory());
	...
	for (FGatheredPathData& AssetFileData : LocalFilesToSearch)
	{
		...
		if (DiskCachedAssetData)
		{
			...
			{
				UE_LOGF(LogAssetRegistry, Display, "Cached dependency data for package '%ls' is invalid. Discarding cached data.", *PackageName.ToString());
				DiskCachedAssetData = nullptr;
			}
		}
	...
	if (bShouldRequestAssetRegistryTick)
	{
		*bShouldRequestAssetRegistryTick = !bInWaitBatch && !CompletedReadContexts.IsEmpty();
	}
...
bool FAssetDataGatherer::ReadAssetFile(const FString& AssetLongPackageName, const FString& AssetFilename, TArray<FAssetData*>& AssetDataList, FPackageDependencyData& DependencyData, TArray<FString>& CookedPackageNamesWithoutAssetData, bool& OutCanRetry) const
{
	...
	if (!PackageReader.OpenPackageFile(AssetLongPackageName, AssetFilename, &OpenPackageResult))
	{
		...
		if (OpenPackageResult == FPackageReader::EOpenPackageResult::CustomVersionMissing)
		{
			...
			if (!bAllowRetry)
			{
				UE_LOGF(LogAssetRegistry, Display, "Package %ls uses an unknown custom version and cannot be loaded for the AssetRegistry", *AssetFilename);
			}
		}
		else
		{
...
void FAssetDataGatherer::AddToCache(FName PackageName, FDiskCachedAssetData* DiskCachedAssetData)
{
	...
	if (ValueInMap != DiskCachedAssetData)
	{
		...
		if (DiskCachedAssetData->Extension != ValueInMap->Extension)
		{
			...
			UE_LOGF(LogAssetRegistry, Display, "Multiple files exist with the same package name %ls but different extensions (%ls and %ls). " "This is invalid and will cause errors; merge or rename or delete one of the files.",
...
{
	bool bOtherThreadIsSavingAsyncCache = false;
	bool bWasCacheEnabled = false;
	bool bShouldSaveCache = false;
	TArray<TPair<FName, FDiskCachedAssetData*>> AssetsToSave;
	{
		FGathererScopeLock TickScopeLock(&TickLock);
		// Must be done before we disable writing
		TryReserveSaveCache(bShouldSaveCache, AssetsToSave);

		bWasCacheEnabled = bCacheWriteEnabled || bCacheReadEnabled;
		bCacheWriteEnabled = false;
		bCacheReadEnabled = false;
		bOtherThreadIsSavingAsyncCache = bIsSavingAsyncCache && !bShouldSaveCache;
	}
	...
	// Saves are executed outside of the lock, but they indicate they are in progress by setting bIsSavingAsyncCache=true.
	// bIsSavingAsyncCache cannot be set to true after bCacheWriteEnabled=false which we set above, so starvation should not be possible.
	while (bOtherThreadIsSavingAsyncCache)
	{
		constexpr float WaitForSaveCompleteTime = .001f;
		FPlatformProcess::Sleep(WaitForSaveCompleteTime);
		FGathererScopeLock TickScopeLock(&TickLock);
		bOtherThreadIsSavingAsyncCache = bIsSavingAsyncCache;
	}

	if (bShouldSaveCache)
	{
		// If this save fails, we don't re-attempt it and accept we couldn't write out the latest state
		TrySaveCacheFile(AssetsToSave);
	}

	{
		FGathererScopeLock TickScopeLock(&TickLock);
		// bIsSavingAsyncCache should now be false. We finished and set it to false if we were asked to save the cache,
		// and no other thread should be able to set it to true because we set bCacheWriteEnabled=false in the same critical
		// section that we used to test for bShouldSaveCache.,
		check(!bIsSavingAsyncCache);
		NewCachedAssetDataMap.Empty();
		DiskCachedAssetDataMap.Empty();
...
bool FAssetDataGatherer::IsCacheSaveReady() const
{
	using namespace UE::AssetDataGather::Private;

	double SaveAsyncCacheStartTime = 0.0;
	if (bIsSavingAsyncCache || !IsCacheSaveRequested(&SaveAsyncCacheStartTime))
	{
		return false;
	}

	if (!IsStopped.load(std::memory_order_relaxed) && (FPlatformTime::Seconds() - SaveAsyncCacheStartTime) < FGatherSettings::MinSecondsToElapseBeforeCacheWrite)
	{
		// A delayed save request was made but we haven't waited long enough to start the save yet so we aren't ready.
		// However if we are stopped, ignore the timer and handle the save request immediately
		return false;
	}
	return true;
}
...
int64 FAssetDataGatherer::SaveCacheFileInternal(const FString& CacheFilename, const TArray<TPair<FName,FDiskCachedAssetData*>>& AssetsToSave)
{
	...
	{
		{
			UE::THashingArchiveProxy<FBlake3> FileArHash(*FileAr);

			using namespace UE::AssetDataGather::Private;
			uint64 CurrentVersion = FGatherSettings::CurrentVersion;
			FileArHash << CurrentVersion;
	...
	#if ALLOW_NAME_BATCH_SAVING
			{
				...
				FAssetRegistryWriter Ar(FAssetRegistryWriterOptions(), FileArHash);
				UE::AssetDataGather::Private::SerializeCacheSave(Ar, AssetsToSave);
			}
	#else		
			static_assert(false, "Cannot save asset registry cache in this configuration");
	#endif
			// write the integrity checksum
			FBlake3Hash Hash = FileArHash.GetHash();
			FileAr->Serialize(&Hash, sizeof(Hash));
		}
	...
	{
		UE_LOGF(LogAssetRegistry, Error, "Failed to open file for write %ls", *TempFilename);
		return 0;
	}
}
...
void SerializeCacheSave(FAssetRegistryWriter& Ar, const TArray<TPair<FName,FDiskCachedAssetData*>>& AssetsToSave)
{
	...
	UE_LOGF(LogAssetRegistry, Verbose, "Asset data gatherer serialized in %0.6f seconds", FPlatformTime::Seconds() - SerializeStartTime);
#endif
}
...
TArray<FCachePayload> LoadCacheFiles(TConstArrayView<FString> InCacheFilenames, bool bCacheWriteEnabled)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(LoadCacheFiles);
	TArray<FCachePayload> Results;
	if (UNLIKELY(InCacheFilenames.IsEmpty()))
	{
		return MoveTemp(Results);
	}
	Results.AddDefaulted(InCacheFilenames.Num());

	// Binary cache files are never overwritten (write-once, named with timestamp+suffix), so memory-mapping
	// them is safe even on builder machines. Only the escape hatch flag can disable it.
	static bool bNoAssetRegistryMemoryMapping = FParse::Param(FCommandLine::Get(), TEXT("NoAssetRegistryMemoryMapping"));
	if (bNoAssetRegistryMemoryMapping)
	{
		UE::AssetDataGather::Private::bMemoryMappedCacheFiles = false;
	}

	UE_LOGF(LogAssetRegistry, Display, "LoadCacheFiles: loading %d cache files, memory-mapping=%ls",
		InCacheFilenames.Num(),
		UE::AssetDataGather::Private::bMemoryMappedCacheFiles ? TEXT("enabled") : TEXT("disabled"));

	auto DoLoad = [](FArchive& FilemappedReader, TSharedPtr<UE::AssetRegistry::FMemoryMappedFile> BackingFile, FAssetRegistryVersion::Type VersionToRead = FAssetRegistryVersion::LatestVersion)
		{
			...
			// The discovery cache always includes editor-only data.
			FAssetRegistryHeader Header;
			Header.Version = VersionToRead;
			Header.bFilterEditorOnlyData = false;

			FAssetRegistryReader RegistryReader(FilemappedReader,
				UE::AssetDataGather::Private::bMemoryMappedCacheFiles ? BackingFile : nullptr,
				FixedTagPrivate::FWeakStorePtr(), NumAsyncWorkers, Header);
			return RegistryReader.IsError() ? FCachePayload() : SerializeCacheLoad(RegistryReader);
		};

	auto VerifyIntegrity = [](FArchive* Ar)
		{
			// same size as in FMD5Hash::HashFileFromArchive
			constexpr int32 kLocalScratchSize = 64 * 1024;
			TArray<uint8> LocalScratch;
			LocalScratch.SetNumUninitialized(kLocalScratchSize);

			// we're going to hash all the file except the last N bytes which hold the hash we need to compare
			constexpr int32 kHashSize = sizeof(FBlake3Hash);
			const int64 SizeToHash = Ar->TotalSize() - kHashSize;

			if (SizeToHash > 0)
			{
				int64 Position = 0;
				FBlake3Hash ComputedHash = [&]()
					{
						THashingArchiveProxy<FBlake3> HashingAr(*Ar);
						// Read in BufferSize chunks
						while (Position < SizeToHash)
						{
							const auto ReadNum = FMath::Min(SizeToHash - Position, (int64)LocalScratch.Num());
							HashingAr.Serialize(LocalScratch.GetData(), ReadNum);

							Position += ReadNum;
						};

						return HashingAr.GetHash();
					}();

				int64 HashToRead = Ar->TotalSize() - Position;
				if (HashToRead == sizeof(ComputedHash))
				{
					FBlake3Hash ReadHash;
					Ar->Serialize(&ReadHash, sizeof(ReadHash));
					Ar->Seek(0);	// return to the beginning so if verification succeeds, we can proceed to read the file as usual
					return (ReadHash == ComputedHash);
				}
			}
	...
	{
		const int32 NumTasks = InCacheFilenames.Num();
		const int32 MaxNumThreads = FGatherSettings::GARGatherCacheParallelism;
		...
		ParallelFor(TEXT("AssetDataGatherer::LoadCacheFiles"), NumTasks, MinBatchSize,
			[&DoLoad, &InCacheFilenames, &Results, &VerifyIntegrity](int32 Index)
		{
			const FString& CacheFilename = InCacheFilenames[Index];

			FCachePayload Payload;
			bool bVerifiedOk = false;

			// First, verify the file and refuse to read if hashes do not agree
			// We do it before memory mapping the file to avoid bringing into the process memory areas of the file
			// that we do not expect to touch, and would have to explicitly evict later.
			TUniquePtr<FArchive> Ar(IFileManager::Get().CreateFileReader(*CacheFilename));
			if (Ar)
			{
				bVerifiedOk = VerifyIntegrity(Ar.Get());
			}

			// now read the file if it is valid
			if (bVerifiedOk)
			{
				TSharedPtr<UE::AssetRegistry::FMemoryMappedFile> File = MakeShared<UE::AssetRegistry::FMemoryMappedFile>(*CacheFilename);

				// we already know (from the verification) that Ar is non-empty
				uint64 Version;
				*Ar << Version;

				if (Version == FGatherSettings::CurrentVersion)
				{
					Payload = DoLoad(*Ar, File);
					UE_CLOGF(Payload.bSucceeded, LogAssetRegistry, Display,
						"Asset registry cache read as %.1f MiB from %ls (memory-mapped=%ls)",
						static_cast<float>(Ar->TotalSize()) / 1024.f / 1024.f, *CacheFilename,
						UE::AssetDataGather::Private::bMemoryMappedCacheFiles ? TEXT("yes") : TEXT("no"));
					UE_CLOGF(!Payload.bSucceeded, LogAssetRegistry, Warning,
						"There was an error loading the asset registry cache from %ls.",
						*CacheFilename);
				}
				else
				{
					// We do not support backwards compatibility after FAssetRegistryVersion::MemoryMappedTagDataStore
					UE_LOGF(LogAssetRegistry, Display,
						"Asset registry older version (%" UINT64_FMT ") cache is too old to read; discarding %ls and regathering its data.",
						Version, *CacheFilename);
				}
			}
			else // check if it was a past version
			{
				Ar.Reset();
				...
				if (File)
				{
					AssetRegistryBackwardCompat::FMemoryViewReader FileReader(File->GetWholeFileView());
					TOptional<uint64> VersionOpt = FileReader.TryLoad<uint64>();
					// all later versions should have a valid integrity hash, so if we're here with a later version archive, it is genuinely corrupted and this is not a compat issue.
					if (VersionOpt.IsSet() && static_cast<FAssetRegistryVersion::Type>(VersionOpt.GetValue()) < FAssetRegistryVersion::MemoryMappedTagDataStore)
					{
						FAssetRegistryVersion::Type Version = static_cast<FAssetRegistryVersion::Type>(VersionOpt.GetValue());
						// For a cooked editor, we don't support any older caches
						#if UE_EDITOR && !UE_IS_COOKED_EDITOR
						// For uncooked editor, we only support backwards compatibility for a single version
						if (Version >= FAssetRegistryVersion::ManageDependenciesCookRule)
						{
							AssetRegistryBackwardCompat::FChecksumViewReader ChecksummingReader(MoveTemp(FileReader), *CacheFilename);
							Payload = DoLoad(ChecksummingReader, nullptr, Version);	// cannot pass the mmaped file, as we cannot map the data in old format
							UE_CLOGF(Payload.bSucceeded, LogAssetRegistry, Display,
								"Asset registry older version (%d) cache read as %.1f MiB from %ls (memory-mapped=no, old format).",
								Version,
								static_cast<float>(FileReader.GetTotalSize()) / 1024.f / 1024.f, *CacheFilename);
							UE_CLOGF(!Payload.bSucceeded, LogAssetRegistry, Warning,
								"There was an error loading the asset registry cache in old format (version %d) from %ls.",
								Version,
								*CacheFilename);
						}
						else
						...
						{
							UE_LOGF(LogAssetRegistry, Display,
								"Asset registry older version (%d) cache is too old to read; discarding %ls and regathering its data.",
								Version, *CacheFilename);
						}
					}
					else
					{
						UE_LOGF(LogAssetRegistry, Warning, "Corrupted AssetRegistry cache file %ls",
							*CacheFilename
						);
					}
				}
	...
	{
		ParallelFor(InCacheFilenames.Num(), [&DoLoad, &InCacheFilenames, &Results, &VerifyIntegrity](int32 Index)
		{
			const FString& CacheFilename = InCacheFilenames[Index];

			FCachePayload Payload;
			TUniquePtr<FArchive> FileAr(IFileManager::Get().CreateFileReader(*CacheFilename, FILEREAD_Silent));
			// old archive versions (predating the hash) may fail verification, but this path only supports current version anyway
			if (FileAr && !FileAr->IsError() && FileAr->TotalSize() > sizeof(uint64) && VerifyIntegrity(FileAr.Get()))
			{
				uint64 Version = 0;
				*FileAr << Version;
				if (Version == FGatherSettings::CurrentVersion)
				{
					Payload = DoLoad(*FileAr, nullptr);
					UE_CLOGF(Payload.bSucceeded, LogAssetRegistry, Display,
						"Asset registry cache read as %.1f MiB from %ls (memory-mapped=no, platform unsupported)",
...
void DeleteCacheFilesWithinLock()
{
	// Delete binary files discovered via .ref indirection and old-style migration fallback
	...
	// Also delete all .ref files and any orphaned binaries not reachable via FindShardedCacheFiles
	const FString CacheBase = GGatherSettings.GetCacheBaseFilename();
	const FString Directory = FPaths::GetPath(CacheBase);

	TArray<FString> RefFiles;
	IFileManager::Get().FindFiles(RefFiles, *(CacheBase + TEXT("_*.ref")), true, false);
	for (const FString& RefFile : RefFiles)
	{
		IFileManager::Get().Delete(*(Directory / RefFile));
	}

	TArray<FString> AllBinaries;
	IFileManager::Get().FindFiles(AllBinaries, *(CacheBase + TEXT("_*.bin")), true, false);
	for (const FString& BinaryFile : AllBinaries)
	{
		IFileManager::Get().Delete(*(Directory / BinaryFile));
	}

	// Also delete legacy location files (Intermediate/CachedAssetRegistry_N.bin)
	TArray<FString> LegacyCacheFiles = GGatherSettings.FindLegacyShardedCacheFiles();
	for (const FString& LegacyCacheFile : LegacyCacheFiles)
	{
		IFileManager::Get().Delete(*LegacyCacheFile);
	}
}
...
void FAssetDataGatherer::SetIsIdle(bool bInIsIdle, double& TickStartTime)
{
	...
	if (bIsIdle)
	{
		...
		if (!bFinishedInitialDiscovery && IsEngineStartupModuleLoadingComplete())
		{
			...
			UE_LOGF(LogAssetRegistry, Verbose, "Initial scan took %0.6f seconds (found %d cached assets, and loaded %d)",
...
void FFileReadScheduler::ProcessReadContexts(TArray<TUniquePtr<UE::AssetDataGather::Private::FReadContext>>&& ReadContexts, bool bIsTimeSliced)
{
	...
	for (int32 i = 0; i < ReadContexts.Num(); ++i)
	{
		...
		auto ProcessPackageFile = [this](FPackageReader& PackageReader, FPackageReader::EOpenPackageResult OpenPackageResult, TUniquePtr<FReadContext>& ReadContext)
			{
				if (OpenPackageResult != FPackageReader::EOpenPackageResult::Success)
				{
					...
					if (OpenPackageResult == FPackageReader::EOpenPackageResult::CustomVersionMissing)
					{
						...
						if (!bAllowRetry)
						{
							UE_LOGF(LogAssetRegistry, Display, "Package %ls uses an unknown custom version and cannot be loaded for the AssetRegistry", *ReadContext->AssetFileData.LocalAbsPath);
						}
					}
					else
					{
		...
		{
			UE_LOGF(LogAssetRegistry, Warning, "Package %ls could not be opened during gathering. Package will be ignored.",
...
void FFileReadScheduler::Schedule(const TArray<FString>& PriorityDirectories)
{
	...
	for (TPair<FString, TArray<TUniquePtr<FReadContext>>>& Pair : DirectoryReadsToSchedule)
	{
		const FString& Directory = Pair.Key;
		TArray<TUniquePtr<FReadContext>> ReadContexts = MoveTemp(Pair.Value);
		ScheduleDirectory(ReadConcurrencyLimiter, Directory, MoveTemp(ReadContexts), false /*bUnblockImmediately*/);
	}
	DirectoryReadsToSchedule.Empty();
}
...
void FFileReadScheduler::Shrink()
{
	...
	DirectoryReadsToSchedule.Shrink();
}
...
// ============================================================================
// Tests for cache ref-file indirection (write-once binaries + .ref pointers)
// ============================================================================
#include "Misc/AutomationTest.h"

#if WITH_DEV_AUTOMATION_TESTS

IMPLEMENT_SIMPLE_AUTOMATION_TEST(FCacheRefFileLifecycleTest,
	"System.AssetRegistry.CacheRefFile.Lifecycle",
	EAutomationTestFlags_ApplicationContextMask | EAutomationTestFlags::EngineFilter);

bool FCacheRefFileLifecycleTest::RunTest(const FString& Parameters)
{
	using namespace UE::AssetDataGather::Private;

	// --- Setup: create an isolated temp directory ---
	const FString TempDir = FPaths::ProjectIntermediateDir() / FString::Printf(TEXT("CacheRefLifecycleTest_%s"), *FGuid::NewGuid().ToString());
	...
	const FString CacheBase = TempDir / TEXT("TestCache");

	// Helper: write a valid cache binary file identical to SaveCacheFileInternal's format.
	auto WriteCacheBinary = [this](const FString& TargetPath, const TArray<TPair<FName, FDiskCachedAssetData*>>& Assets) -> bool
	{
		const FString TempPath = TargetPath + TEXT(".tmp");
		{
			TUniquePtr<FArchive> FileAr(IFileManager::Get().CreateFileWriter(*TempPath, 0));
			if (!FileAr)
			{
				AddError(FString::Printf(TEXT("Failed to create file writer for %s"), *TempPath));
				return false;
			}
			UE::THashingArchiveProxy<FBlake3> HashAr(*FileAr);
			uint64 Version = FGatherSettings::CurrentVersion;
			HashAr << Version;
			{
				FAssetRegistryWriter Writer(FAssetRegistryWriterOptions(), HashAr);
				UE::AssetDataGather::Private::SerializeCacheSave(Writer, Assets);
			}
			FBlake3Hash Hash = HashAr.GetHash();
			FileAr->Serialize(&Hash, sizeof(Hash));
		}
		return IFileManager::Get().Move(*TargetPath, *TempPath);
	};

	// Helper: count .bin files matching the cache base pattern
	auto CountBinFiles = [&CacheBase]() -> int32
	{
		TArray<FString> Files;
		IFileManager::Get().FindFiles(Files, *(CacheBase + TEXT("_*.bin")), true, false);
		return Files.Num();
	};

	// Helper: count .ref files
	auto CountRefFiles = [&CacheBase]() -> int32
	{
		TArray<FString> Files;
		IFileManager::Get().FindFiles(Files, *(CacheBase + TEXT("_*.ref")), true, false);
		return Files.Num();
	};

	// --- Step 1: Build test asset data ---
	FDiskCachedAssetData AssetA(FDateTime::Now(), FDateTime::Now(), FName(TEXT("uasset")));
	FDiskCachedAssetData AssetB(FDateTime::Now(), FDateTime::Now(), FName(TEXT("uasset")));

	TArray<TPair<FName, FDiskCachedAssetData*>> TestAssets;
	TestAssets.Emplace(FName(TEXT("/Test/PackageA")), &AssetA);
	TestAssets.Emplace(FName(TEXT("/Test/PackageB")), &AssetB);

	// --- Step 2: Write binary A + .ref ---
	const FString BinaryPathA = MakeShardBinaryFilename(CacheBase, 0);
	TestTrue(TEXT("Write binary A"), WriteCacheBinary(BinaryPathA, TestAssets));

	const FString RefPath = GetShardRefFilename(CacheBase, 0);
	WriteCacheRefFile(RefPath, FPaths::GetCleanFilename(BinaryPathA));

	TestEqual(TEXT("Step 2: 1 .bin file"), CountBinFiles(), 1);
	TestEqual(TEXT("Step 2: 1 .ref file"), CountRefFiles(), 1);
	TestEqual(TEXT("Step 2: .ref points to A"), ReadCacheRefFile(RefPath), FPaths::GetCleanFilename(BinaryPathA));

	// --- Step 3: Load and verify round-trip ---
	{
		TArray<FCachePayload> Payloads = LoadCacheFiles({BinaryPathA}, false);
		TestEqual(TEXT("Step 3: LoadCacheFiles returned 1 payload"), Payloads.Num(), 1);
		TestTrue(TEXT("Step 3: Load succeeded"), Payloads[0].bSucceeded);
		TestEqual(TEXT("Step 3: 2 assets loaded"), Payloads[0].NumAssets, 2);
	}

	// --- Step 4: Write binary B, update .ref ---
	const FString BinaryPathB = MakeShardBinaryFilename(CacheBase, 0);
	TestTrue(TEXT("Write binary B"), WriteCacheBinary(BinaryPathB, TestAssets));
	WriteCacheRefFile(RefPath, FPaths::GetCleanFilename(BinaryPathB));

	// --- Step 5: Assert two .bin files, .ref points to B ---
	TestEqual(TEXT("Step 5: 2 .bin files"), CountBinFiles(), 2);
	TestEqual(TEXT("Step 5: 1 .ref file"), CountRefFiles(), 1);
	TestEqual(TEXT("Step 5: .ref points to B"), ReadCacheRefFile(RefPath), FPaths::GetCleanFilename(BinaryPathB));

	// --- Step 6: Write binary C, update .ref ---
	const FString BinaryPathC = MakeShardBinaryFilename(CacheBase, 0);
	TestTrue(TEXT("Write binary C"), WriteCacheBinary(BinaryPathC, TestAssets));
	WriteCacheRefFile(RefPath, FPaths::GetCleanFilename(BinaryPathC));

	TestEqual(TEXT("Step 6: 3 .bin files"), CountBinFiles(), 3);

	// --- Step 7: Run cleanup; A and B are orphaned, C is referenced ---
	CleanupOrphanedCacheFiles(CacheBase, ECacheCleanupContext::PostWrite);

	TestEqual(TEXT("Step 7: 1 .bin file remains"), CountBinFiles(), 1);
	TestEqual(TEXT("Step 7: 1 .ref file remains"), CountRefFiles(), 1);
	TestTrue(TEXT("Step 7: Binary C still exists"), IFileManager::Get().FileExists(*BinaryPathC));
	TestFalse(TEXT("Step 7: Binary A deleted (orphan)"), IFileManager::Get().FileExists(*BinaryPathA));
	TestFalse(TEXT("Step 7: Binary B deleted (orphan)"), IFileManager::Get().FileExists(*BinaryPathB));
	TestEqual(TEXT("Step 7: .ref points to C"), ReadCacheRefFile(RefPath), FPaths::GetCleanFilename(BinaryPathC));
