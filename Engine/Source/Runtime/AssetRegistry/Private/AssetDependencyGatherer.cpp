	...
	if (Context.bInterfaceGatherDependenciesCalled)
	{
		...
		if (!bCalledOnce)
		{
			...
			UE_LOGF(LogAssetRegistry, Warning, 
				"%s is overriding a deprecated function, GatherDependencies(const FAssetData& , const FAssetRegistryState&, TArray&, TArray&). " "Override GatherDependencies that takes an FGatherDependenciesContext instead. " "Deprecation handling will be removed in a future version and your code will no longer compile.",
				InGathererClass);
