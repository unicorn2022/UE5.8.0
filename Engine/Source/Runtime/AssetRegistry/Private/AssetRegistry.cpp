...
{
	const FName WildcardFName("*");
	const FTopLevelAssetPath WildcardPathName(TEXT("/*"), TEXT("*"));

	const FName Stage_ChunkCountFName("Stage_ChunkCount");
	const FName Stage_ChunkSizeFName("Stage_ChunkSize");
	const FName Stage_ChunkCompressedSizeFName("Stage_ChunkCompressedSize");
	const FName Stage_ChunkInstalledSizeFName("Stage_ChunkInstalledSize");
	const FName Stage_ChunkStreamingSizeFName("Stage_ChunkStreamingSize");
	const FName Stage_ChunkOptionalSizeFName("Stage_ChunkOptionalSize");
...
namespace UE::AssetRegistry::Impl
{
	...
	float MaxSecondsPerTickBackgroundThread = -1.f;
	static FAutoConsoleVariableRef CVarAssetRegistryMaxSecondsPerTickBackgroundThread(
		...
		TEXT("Maximum amount of time allowed for Asset Registry processing, in seconds, per iteration on the background thread. Negative are treated as no maximum."));
	...
	const FName DestinationObjectFName("DestinationObject");
}
...
public:
	...
	virtual UE::AssetRegistry::EExists TryGetAssetPackageData(FName PackageName, class FAssetPackageData& OutPackageData, bool bFailIfLockHeld) const override
	{
		FName OutCorrectCasePackageName;
		return TryGetAssetPackageData(PackageName, OutPackageData, OutCorrectCasePackageName, bFailIfLockHeld);
	}
	
	virtual UE::AssetRegistry::EExists TryGetAssetPackageData(FName PackageName, class FAssetPackageData& OutPackageData, FName& OutCorrectCasePackageName, bool bFailIfLockHeld) const override
	{
		IAssetRegistry* AssetRegistry = IAssetRegistry::Get();
		if (!AssetRegistry)
		{
		...
		return AssetRegistry->TryGetAssetPackageData(PackageName, OutPackageData, OutCorrectCasePackageName, bFailIfLockHeld);
	}
...
void FAssetRegistryImpl::ConsumeOrDeferPreloadedPremade(UAssetRegistryImpl& UARI, Impl::FEventContext& EventContext,
	Impl::FActiveMountEvents& ActiveMountEvents, FInterfaceWriteScopeLock& ProofOfWriteLock)
{
	...
	{
		FPreloader::FConsumeFunction ConsumeFromAsyncThread = [this, &UARI](Premade::ELoadResult LoadResult,
			UE::AssetRegistry::Impl::FAssetProviderInitData&& StateInitData)
		{
			Impl::FEventContext EventContext;
			Impl::FActiveMountEvents ActiveMountEvents;
			{
				UE::AssetRegistry::FInterfaceWriteScopeLock InterfaceScopeLock(UARI.InterfaceLock);
				AsyncConsumer.Consume(UARI, EventContext, ActiveMountEvents, InterfaceScopeLock, LoadResult,
					MoveTemp(StateInitData));
			}
			UARI.Broadcast(EventContext);
			ActiveMountEvents.Broadcast();
		};
		auto ConsumeOnCurrentThread = [ConsumeFromAsyncThread](Premade::ELoadResult LoadResult,
			UE::AssetRegistry::Impl::FAssetProviderInitData&& StateInitData) mutable
		{
			Async(EAsyncExecution::TaskGraph, [LoadResult, StateInitData=MoveTemp(StateInitData),
				ConsumeFromAsyncThread=MoveTemp(ConsumeFromAsyncThread)]() mutable
			{
				ConsumeFromAsyncThread(LoadResult, MoveTemp(StateInitData));
			});
		};
	...
	{
		GPreloader.Consume(
			[this, &UARI, &EventContext, &ProofOfWriteLock, &ActiveMountEvents](Premade::ELoadResult LoadResult,
				UE::AssetRegistry::Impl::FAssetProviderInitData&& StateInitData)
			{
				LoadPremadeAssetRegistry(UARI, EventContext, ActiveMountEvents, ProofOfWriteLock, LoadResult,
					MoveTemp(StateInitData));
			});
	}
}
...
EChunkProgressReportingType::Type GetChunkAvailabilityProgressType(EAssetAvailabilityProgressReportingType::Type ReportType)
{
	...
	default:
		ChunkReportType = EChunkProgressReportingType::PercentageComplete;
		UE_LOGF(LogAssetRegistry, Error, "Unsupported assetregistry report type: %i", (int)ReportType);
		break;
	}
...
struct FInitializeContext
{
	...
	FActiveMountEvents ActiveMountEvents;
	...
	: Super(ObjectInitializer)
{
	...
	Context.ActiveMountEvents.Broadcast();
}
...
void FAssetRegistryImpl::LoadPremadeAssetRegistry(UAssetRegistryImpl& UARI, Impl::FEventContext& EventContext,
	Impl::FActiveMountEvents& ActiveMountEvents, FInterfaceWriteScopeLock& ProofOfWriteLock,
	Premade::ELoadResult LoadResult, Impl::FAssetProviderInitData&& StateInitData)
{
	...
	if (SerializationOptions.bSerializeAssetRegistry)
	{
		...
		{
			UARI.ActiveMountsLoader.InitialProviderReport(UARI, ActiveMountEvents, StateInitData);
			
			if (State.GetNumAssets() == 0)
			{
				// ActiveMounts: Handle the possibility that ActiveMountsInitialize has already been called and if so
				// parse the premade's state now.
				UARI.ActiveMountsLoader.InitialProviderConditionalRegister(UARI, ActiveMountEvents, StateInitData.ARState);

				// Add the premade state into the global AssetRegistry
				State = MoveTemp(StateInitData.ARState);
				...
				if (bEmitAssetEvents)
				{
					...
					{
						EventContext.AssetEvents.Emplace(AssetData,
							UE::AssetRegistry::Impl::FEventContext::EEvent::Added);
					});
				}
			}
			else
			{
				// This is unexpected, we should not have called AppendState or in any other way added Assets to the
				// AssetRegistry before loading the premade AssetRegistry. However, we can handle it with only minor
				// performance issues.
				// ActiveMounts usually parses the premade's ARState during ActiveMountsInitialize by reading the
				// global AssetRegistry's GuardedData.State, but that state already contains some other data, so we
				// need to instead parse the premade's state directly.
				UARI.ActiveMountsLoader.InitialProviderOnReadyForRegister(UARI, ActiveMountEvents,
					Impl::EInitialProviderRegisterReason::EarlierState);
				UARI.ActiveMountsLoader.InitialProviderConditionalRegister(UARI, ActiveMountEvents, StateInitData.ARState);

				// Add the premade state into the global AssetRegistry
				if (State.GetNumAssets() < StateInitData.ARState.GetNumAssets())
				{
				...
				{
					AppendState(EventContext, StateInitData.ARState,
						FAssetRegistryState::EInitializationMode::OnlyUpdateNew, bEmitAssetEvents);
				}
			}
		...
		{
			UE_CLOGF(FPlatformProperties::RequiresCookedData() && (IsRunningGame() || IsRunningDedicatedServer()),
				LogAssetRegistry, Error, "Failed to load premade asset registry. LoadResult == %d.", static_cast<int32>(LoadResult));
		}
	}

	{
		...
		for (const TSharedRef<IPlugin>& ContentPlugin : ContentPlugins)
		{
			if (ContentPlugin->CanContainContent())
			{
				...
				if (UARI.ActiveMountsLoader.TryLoadStateAndRegisterAssetProvider(UARI, ActiveMountEvents,
					ProofOfWriteLock, PakPlatformFile, PluginAssetRegistry, PluginState))
				{
#if WITH_EDITOR
					/*
...
void FAssetRegistryImpl::Initialize(UE::AssetRegistry::FInterfaceWriteScopeLock& ScopeLock, Impl::FInitializeContext& Context)
{
	...
	Context.UARI.ActiveMountsLoader.InitializeConfig();

	ConsumeOrDeferPreloadedPremade(Context.UARI, Context.Events, Context.ActiveMountEvents, ScopeLock);
	...
	UE_LOGF(LogAssetRegistry, Log, "FAssetRegistry took %0.4f seconds to start up", StartupDuration);
...
void UAssetRegistryImpl::InitializeEvents(UE::AssetRegistry::Impl::FInitializeContext& Context)
{
	...
	FPackageName::OnMountPointMounted().AddUObject(this, &UAssetRegistryImpl::OnMountPointMounted);
	FPackageName::OnMountPointDismounted().AddUObject(this, &UAssetRegistryImpl::OnMountPointDismounted);
	...
	FCoreDelegates::GetOnPostEngineInit().AddUObject(this, &UAssetRegistryImpl::OnPostEngineInit);
		...
		if (!ClassName.IsEmpty() && !TagName.IsEmpty())
		{
			...
			else
			{
				...
				else if (FPackageName::IsShortPackageName(ClassName))
				{
					...
					UE_CLOGF(ClassPathName.IsNull(), LogAssetRegistry, Warning, "Failed to convert short class name \"%ls\" when parsing ini [AssetRegistry] CookedTagsWhitelist or CookedTagsBlacklist", *ClassName);
				}
				else
				{
				...
				if (!ClassPathName.IsValid())
				{
					UE_LOGF(LogAssetRegistry, Warning, 
						"There is a rule to %ls tag \"%ls\" for class \"%ls\", which is not a valid class path. Ignoring.",
	...
	bool* bAreRulesResolved)
{
	...
	if (RuleIt->ClassPath == UE::AssetRegistry::WildcardPathName)
	{
		...
		for (++RuleIt; RuleIt; ++RuleIt)
		{
			...
			UE_CLOGF(!bSuppressWarnings && !bIsDuplicate, LogAssetRegistry, Warning, 
				"There is a wildcard rule to %ls tag \"%.*ls\", " "but there is another rule to %ls that tag for class \"%ls\". " "Only wildcard rule is accepted. Ignoring another rule.",
	...
	for (; RuleIt; ++RuleIt)
	{
		...
		else
		{
			...
			UE_CLOGF(!bSuppressWarnings && !bIsDuplicate, LogAssetRegistry, Warning,
				"There is a rule to %ls tag \"%.*ls\" for class \"%ls\". Class is unknown. Ignoring.",
				RuleIt->bInclude ? TEXT("include") : TEXT("exclude"),
				TagName.Len(), TagName.GetData(),
				*RuleIt->ClassPath.ToString()
			);
...
void UAssetRegistryImpl::OnPreExit()
{
	...
	ActiveMountsLoader.Shutdown(*this, ActiveMountEvents);
	ActiveMountEvents.Broadcast();
}
...
void UAssetRegistryImpl::FinishDestroy()
{
	...
	}
	{
		...
		FPackageName::OnMountPointMounted().RemoveAll(this);
		FPackageName::OnMountPointDismounted().RemoveAll(this);
	...
	Impl::FClassInheritanceContext& InheritanceContext, bool bSynchronousSearch)
{
	...
	{
		CSV_TRACE_REGION_BEGIN(TEXT("Scan"), TEXT("AssetRegistry"));
	...
	{
		UE_CLOGF(!bSynchronousSearch, LogAssetRegistry, Warning, "SearchAllAssets: Gatherer is in synchronous mode; forcing bSynchronousSearch=true.");
	...
	for (;;)
	{
		...
		if (!IsTickActive(Status) && Status != EGatherStatus::WaitingForEvents)
		{
			if (Status == EGatherStatus::UnableToProgress)
			{
				UE_LOGF(LogAssetRegistry, Display,
					"UAssetRegistryImpl::WaitForCompletion exiting without completing because TickGatherer returned UnableToProgress. "
					"IsInGameThread() == %ls; IsEngineStartupModuleLoadingComplete() == %ls",
			...
			else if (Status == EGatherStatus::Complete && bInitialSearchStarted)
			{
				...
				if (!GuardedData.IsInitialSearchCompleted())
				{
					UE_LOGF(LogAssetRegistry, Error,
						"Exiting from UAssetRegistryImpl::WaitForCompletion but IsInitialSearchCompleted is still false."
							"EventContext.bHasSentFileLoadedEventBroadcast == %ls; IsInGameThread() == %ls",
		...
		else if (Status == EGatherStatus::WaitingForEvents)
		{
			...
			if (!CanBroadcastEvents())
			{
				...
				UE_LOGF(LogAssetRegistry, Warning,
					"Exiting from UAssetRegistryImpl::WaitForCompletion with gather almost completed " "but missing the final broadcast of events, so IsInitialSearchCompleted is still false. " "We cannot execute the broadcast because %ls.", Reason);
				break;
			} 
		}
...
static FTopLevelAssetPath TryConvertShortTypeNameToPathName(FName ClassName)
{
	...
	if (ClassName != NAME_None)
	{
		...
		UE_CLOGF(ClassPathName.IsNull(), LogClass, Error, "Failed to convert short class name %ls to class path name.", *ShortClassName);
	}
...
UE::AssetRegistry::EExists UAssetRegistryImpl::TryGetAssetPackageData(const FName PackageName, FAssetPackageData& OutAssetPackageData, bool bFailIfLockHeld) const
{
	FName OutCorrectCasePackageName;
	return TryGetAssetPackageData(PackageName, OutAssetPackageData, OutCorrectCasePackageName, bFailIfLockHeld);
}

UE::AssetRegistry::EExists UAssetRegistryImpl::TryGetAssetPackageData(const FName PackageName, FAssetPackageData& OutAssetPackageData, FName& OutCorrectCasePackageName, bool bFailIfLockHeld) const
{
	if (!InterfaceLock.TryReadLock())
	{
		if (bFailIfLockHeld)
		{
			return UE::AssetRegistry::EExists::Unknown;
		}
		InterfaceLock.ReadLock();
	}
	ON_SCOPE_EXIT
	{
		InterfaceLock.ReadUnlock();
	};
...
bool UAssetRegistryImpl::GetAllAssets(TArray<FAssetData>& OutAssetData, bool bIncludeOnlyOnDiskAssets) const
{
	...
	UE_LOGF(LogAssetRegistry, VeryVerbose, "GetAllAssets completed in %0.4f seconds", FPlatformTime::Seconds() - GetAllAssetsStartTime);
	return true;
}
...
bool UAssetRegistryImpl::EnumerateAllAssets(TFunctionRef<bool(const FAssetData&)> Callback, UE::AssetRegistry::EEnumerateAssetsFlags InEnumerateFlags) const
{
	...
	TRACE_CPUPROFILER_EVENT_SCOPE_STR("UAssetRegistryImpl::EnumerateAllAssets");
	...
	if (EnumHasAnyFlags(InEnumerateFlags, EEnumerateAssetsFlags::Parallel))
	{
		...
		TMpscQueue<FAssetData> OnDiskAssetDatasQueue;
		TArray<FAssetData, TInlineAllocator<128>> OnDiskAssetDatas;
		{
			UE::AssetRegistry::FInterfaceReadScopeLock InterfaceScopeLock(InterfaceLock);
			GuardedData.EnumerateAllDiskAssets(PackageNamesToSkip,
				[&OnDiskAssetDatasQueue](const FAssetData& AssetData)
				{
					OnDiskAssetDatasQueue.Enqueue(AssetData);
					return true;
				}, InEnumerateFlags);
			OnDiskAssetDatas.Reserve(GuardedData.State.GetNumAssets());
		}
		FAssetData AssetData;
		while (OnDiskAssetDatasQueue.Dequeue(AssetData))
		{
			OnDiskAssetDatas.Add(MoveTemp(AssetData));
		}
...
void UAssetRegistryImpl::GetPackagesByName(FStringView PackageName, TArray<FName>& OutPackageNames) const
{
	UE::AssetRegistry::FInterfaceReadScopeLock InterfaceScopeLock(InterfaceLock);
	const FAssetRegistryState& State = GuardedData.GetState();
	UE_CLOGF(GuardedData.IsInitialSearchStarted() && !GuardedData.IsInitialSearchCompleted(), LogAssetRegistry, Warning,
		"GetPackagesByName has been called before AssetRegistry gather is complete and it does not wait. " "The search may return incomplete results.");
...
FName UAssetRegistryImpl::GetFirstPackageByName(FStringView PackageName) const
{
	...
	bool bSearchAllAssets;
	{
		...
		const FAssetRegistryState& State = GuardedData.GetState();
		UE_CLOGF(GuardedData.IsInitialSearchStarted() && !GuardedData.IsInitialSearchCompleted(), LogAssetRegistry, Warning,
			"GetFirstPackageByName has been called before AssetRegistry gather is complete and it does not wait. " "The search may fail to find the package.");
	...
	if (!GIsEditor && !bSearchAllAssets)
	{
		...
		{
			UE_LOGF(LogAssetRegistry, Warning,
				"GetFirstPackageByName is being called in `-game` to resolve partial package name. " "This may cause a slow scan on disk. " "Consider using the fully qualified package name for better performance. ");
	...
	if (GIsEditor)
	{
		...
		if (bVerifyNegativeResults && !AssetPackageData)
		{
			...
			if (FPackageName::InternalDoesPackageExistEx(PackageNameStr, FPackageName::EPackageLocationFilter::Any, 
				false /*bMatchCaseOnDisk*/, &PackagePath) != FPackageName::EPackageLocationFilter::None)
			{
				UE_LOGF(LogAssetRegistry, Warning, "Package %ls exists on disk but does not exist in the AssetRegistry", *PackageNameStr);
		...
		if (OutExtension)
		{
			if (AssetPackageData->Extension == EPackageExtension::Unspecified || AssetPackageData->Extension == EPackageExtension::Custom)
			{
				...
				{
					UE_LOGF(LogAssetRegistry, Error,
						"UAssetRegistryImpl::DoesPackageExistOnDisk failed to find the extension for %ls. The package exists in the AssetRegistry but does not exist on disk.",
						*PackageNameStr);
	...
	TArray<FTopLevelAssetPath>& OutAncestorClassNames) const
{
	...
	else
	{
		...
		{
			UE_LOGF(LogAssetRegistry, Error, "IsChildClass exceeded max inheritance depth. There is probably an infinite loop of parent classes.");
			bFoundClass = false;
		}
	}
	...
	if (!ensureAlwaysMsgf(InFilter.ClassNames.Num() == 0, TEXT("Asset Registry Filter using ClassNames instead of ClassPaths. First class name: \"%s\""), *InFilter.ClassNames[0].ToString()))
	{
		...
		for (FName ClassName : InFilter.ClassNames)
		{
			if (!ClassName.IsNone())
			{
				...
				{
					UE_LOGF(LogAssetRegistry, Error, "Failed to resolve class path for short class name \"%ls\" when compiling asset registry filter", *ClassName.ToString());
				}
			}
		}
	}
	...
	UE::AssetRegistry::EScanFlags InScanFlags)
{
	...
	UE_LOGF(LogAssetRegistry, Verbose, "ScanPathsSynchronous completed scanning %ls to find %d assets in %0.4f seconds", *PathsString,
...
void UAssetRegistryImpl::AssetTagsFinalized(const UObject& FinalizedAsset)
{
	...
	LLM_SCOPE(ELLMTag::AssetRegistry);
	AddLoadedAssetToProcess(FinalizedAsset);
#endif
}
...
UE::AssetRegistry::Impl::EGatherStatus UAssetRegistryImpl::TickOnBackgroundThread()
{
	...
	do
	{
		...
		if (GatheredDataProcessingLock.TryLock())
		{
			...
			UE::AssetRegistry::Impl::FTickContext TickContext(GuardedData, EventContext, InheritanceContext);
			if (UE::AssetRegistry::Impl::MaxSecondsPerTickBackgroundThread > 0.f)
			{
...
void FAssetRegistryImpl::UpdateMaxSecondsPerFrame()
{
	...
	if (bInitialSearchStarted && !IsInitialSearchCompleted())
	{
		...
		if (bBlockingInitialLoad)
		{
			...
			{
				UE_LOGF(LogAssetRegistry, Display,
					"EditorPerProjectUserSettings.ini:[AssetRegistry]:BlockingInitialLoad=true, setting AssetRegistry load to blocking. The editor will not be interactive until the initial scan completes.");
			}
		}
	}
	if (GlobalGatherer)
...
Impl::EGatherStatus FAssetRegistryImpl::TickGatherer(Impl::FTickContext& TickContext)
{
	...
	{
		UE_LOGF(LogAssetRegistry, Verbose, "### Background search completed in %0.4f seconds", SearchTime);
	}
			...
			&& TickContext.bHandleDeferred)
		{
				...
				(TickContext.bIsInGameThread && (!DeferredAssetsForGameThread.IsEmpty() || !DeferredDependenciesForGameThread.IsEmpty())))
			{
				...
				if (bRetryAssetGathering)
				{
					...
					{
						UE_LOGF(LogAssetRegistry, Error, "Runaway loop detected in handling of deferred assets");
	...
	State.EnumerateAllMutableAssets([](FAssetData& AssetData)
		{
			...
			for (; BundleIndex < NumBundles; ++BundleIndex)
			{
				...
				for (; AssetPathIndex < NumAssetPaths; ++AssetPathIndex)
				{
					...
					{
						UE_LOGF(LogAssetRegistry, Warning,
							"AssetBundleEntry Name %ls.%ls[%d] was redirected from %ls -> %ls, but the redirected target is not a TopLevelAssetPath, and AssetBundleEntry Names must be TopLevelAssetPaths. Ignoring the redirector.",
...
void FAssetRegistryImpl::OnInitialSearchCompleted(Impl::FEventContext& EventContext)
{
	...
	LogSearchDiagnostics(InitialSearchStartTime, /* bInitialScan */ true);
	CSV_TRACE_REGION_END(TEXT("Scan"), TEXT("AssetRegistry"));
...
void FAssetRegistryImpl::OnAdditionalMountSearchCompleted(Impl::FEventContext& EventContext)
{
	...
	LogSearchDiagnostics(AdditionalMountSearchStartTime, /* bInitialScan */ false);
	CSV_TRACE_REGION_END(TEXT("Additional Mount Search"), TEXT("AssetRegistry")); // Matching CSV_TRACE_REGION_BEGIN in OnMountPointMounted
...
void FAssetRegistryImpl::LogSearchDiagnostics(double StartTime, bool bInitialScan)
{
	...
	Telemetry.bIsInitialScan = bInitialScan;
	FTelemetryRouter::Get().ProvideTelemetry(Telemetry);
#if !NO_LOGGING
	...
	UE_LOGF(LogAssetRegistry, Log, "%ls", *Message);

	if (bVerboseLogging)
	{
		UE_LOGF(LogAssetRegistry, Verbose, "TagMemoryUse:");
		...
		for (const TPair<FTopLevelAssetPath, int64>& Pair : TagSizeByClass)
		{
			...
			UE_LOGF(LogAssetRegistry, Verbose, "%ls: %.1f MB",
...
void FAssetRegistryImpl::SetDirectoriesWatchedByPackage(FName PackageName, TConstArrayView<FString> Directories)
{
	...
	for (const FString& DirectoryLocalPathOrLongPackageName : Directories)
	{
		...
		{
			UE_LOGF(LogAssetRegistry, Warning,
				"AddDirectoryReferencer called with LongPackageName %ls that cannot be mapped to a local path. Ignoring it.",
...
bool UAssetRegistryImpl::ActiveMountsIsEnabled()
{
	return ActiveMountsLoader.IsEnabled();
}

void UAssetRegistryImpl::ActiveMountsInitialize()
{
	using namespace UE::AssetRegistry;
	...
	UE::AssetRegistry::Impl::FActiveMountEvents ActiveMountEvents;
	{
		LLM_SCOPE(ELLMTag::AssetRegistry);
		UE::AssetRegistry::FInterfaceWriteScopeLock InterfaceScopeLock(InterfaceLock);
		ActiveMountsLoader.OnActiveMountsInitialize(*this, ActiveMountEvents, InterfaceScopeLock);
	}
	ActiveMountEvents.Broadcast();
}

bool UAssetRegistryImpl::ActiveMountsRegisterAndLoadFilePathSynchronous(FStringView StateFilePath)
{
	if (StateFilePath.IsEmpty())
	{
		ensureMsgf(false, TEXT("AppendStateForActiveMounts called with an invalid empty StateFilePath."));
		return false;
	}
	...
	UE::AssetRegistry::Impl::FActiveMountEvents ActiveMountEvents;
	LLM_SCOPE(ELLMTag::AssetRegistry);
	bool bResult = ActiveMountsLoader.TryRegisterAssetProviderAndAppendStateOutsideARLock(*this, EventContext,
		ActiveMountEvents, StateFilePath);
	Broadcast(EventContext);
	ActiveMountEvents.Broadcast();
	return bResult;
}

void UAssetRegistryImpl::ActiveMountsRegisterAndLoadFilePathAsync(FStringView StateFilePath,
	TFunction<void(bool bSucceeded)> OnComplete, UE::Tasks::ETaskPriority TaskPriority)
{
	if (StateFilePath.IsEmpty())
	{
		ensureMsgf(false, TEXT("AppendStateForActiveMounts called with an invalid empty StateFilePath."));
	...
	UE::Tasks::Launch(UE_SOURCE_LOCATION,
		[StateFilePath = FString(StateFilePath), OnComplete = MoveTemp(OnComplete)]
		{
			bool bSuccess = false;
			IAssetRegistry* GlobalAR = IAssetRegistry::Get();
			if (GlobalAR)
			{
				bSuccess = GlobalAR->ActiveMountsRegisterAndLoadFilePathSynchronous(StateFilePath);
			}
			if (OnComplete)
			{
				OnComplete(bSuccess);
...
void UAssetRegistryImpl::ActiveMountsLoadAsync(TConstArrayView<FStringView> MountPointLongPackageNames,
	TFunction<void(bool bCanceled)> OnComplete)
{
	using namespace UE::AssetRegistry;

	LLM_SCOPE(ELLMTag::AssetRegistry);
	TArray<TRefCountPtr<UE::PackageName::IMountPoint>> MountPoints;
	MountPoints.Reserve(MountPointLongPackageNames.Num());
	for (FStringView MountPointName : MountPointLongPackageNames)
	{
		TRefCountPtr<UE::PackageName::IMountPoint> MountPoint =
			FPackageName::FindMountPointByRootPackageName(MountPointName);
		if (MountPoint)
		{
			MountPoints.Add(MoveTemp(MountPoint));
		}
	}

	UE::AssetRegistry::Impl::FActiveMountEvents ActiveMountEvents;
	ActiveMountsLoader.LoadAsync(*this, ActiveMountEvents, MountPoints, MoveTemp(OnComplete));
	ActiveMountEvents.Broadcast();
}

void UAssetRegistryImpl::ActiveMountsReloadAllAsync(TFunction<void(bool bCanceled)> OnComplete)
{
	UE::AssetRegistry::Impl::FActiveMountEvents ActiveMountEvents;
	ActiveMountsLoader.ReloadAllAsync(*this, ActiveMountEvents, MoveTemp(OnComplete));
	ActiveMountEvents.Broadcast();
}

void UAssetRegistryImpl::ActiveMountsUnloadAsync(TConstArrayView<FStringView> MountPointLongPackageNames,
	TFunction<void(bool bCanceled)> OnComplete)
{
	using namespace UE::AssetRegistry;
	...
	UE::AssetRegistry::Impl::FActiveMountEvents ActiveMountEvents;
	ActiveMountsLoader.UnloadAsync(*this, ActiveMountEvents, MountPoints, MoveTemp(OnComplete));
	ActiveMountEvents.Broadcast();
}

void UAssetRegistryImpl::ActiveMountsUnloadAllUnmountedAsync(TFunction<void(bool bCanceled)> OnComplete)
{
	UE::AssetRegistry::Impl::FActiveMountEvents ActiveMountEvents;
	ActiveMountsLoader.UnloadAllUnmountedAsync(*this, ActiveMountEvents, MoveTemp(OnComplete));
	ActiveMountEvents.Broadcast();
}

bool UAssetRegistryImpl::ActiveMountsIsLoaded(FStringView MountPointLongPackageName)
{
	using namespace UE::AssetRegistry;

	TRefCountPtr<UE::PackageName::IMountPoint> MountPoint =
		FPackageName::FindMountPointByRootPackageName(MountPointLongPackageName);
	if (!MountPoint)
	{
		return false;
	}
	...
	LLM_SCOPE(ELLMTag::AssetRegistry);
	UE::AssetRegistry::Impl::FActiveMountEvents ActiveMountEvents;
	bool bResult = ActiveMountsLoader.IsLoaded(*this, ActiveMountEvents, MountPoint);
	ActiveMountEvents.Broadcast();
	return bResult;
}

bool UAssetRegistryImpl::ActiveMountsIsInProgress()
{
	return ActiveMountsLoader.IsInProgress(*this);
}

void UAssetRegistryImpl::ActiveMountsReportWhenIdle(TFunction<void()> OnIdle)
{
	UE::AssetRegistry::Impl::FActiveMountEvents ActiveMountEvents;
	ActiveMountsLoader.ReportWhenIdle(*this, ActiveMountEvents, MoveTemp(OnIdle));
	ActiveMountEvents.Broadcast();
}
	...
	FAssetRegistryState::EInitializationMode Mode, bool bEmitAssetEvents)
{
	...
	CachePathsFromState(EventContext, InState, &Options);
...
void FAssetRegistryImpl::ConsumeState(Impl::FEventContext& EventContext, FAssetRegistryState&& InState,
	bool bEmitAssetEvents)
{
	FAssetRegistryAppendResult LocalAppendResult;
	FAssetRegistryAppendResult* AppendResultPtr = bEmitAssetEvents ? &LocalAppendResult : nullptr;

#if WITH_EDITOR
	const FAssetRegistrySerializationOptions& InOptions = DevelopmentSerializationOptions;
	//Development options have all filters disabled, so there is no need to resolve runtime class filter options
#else
	FAssetRegistrySerializationOptions& InOptions = SerializationOptions;
	const bool bSuppressWarnings = !UE::AssetRegistry::Utils::HasEngineModuleLoaded();
	Utils::UpdateSerializationOptions(InOptions, CachedParsedFilterRules, bSuppressWarnings);
#endif

	// Since we're moving assets rather than copying them, we do not filter them by tags and we do not remove any
	// any entries from their tags.
	FAssetRegistrySerializationOptions ConsumeOptions = InOptions;
	ConsumeOptions.bFilterAssetDataWithNoTags = false;
	ConsumeOptions.CookFilterlistTagsByClass.Empty();

	CachePathsFromState(EventContext, InState, &ConsumeOptions);

	State.ConsumeExisting(
		MoveTemp(InState),
		ConsumeOptions,
		AppendResultPtr);
...
void FAssetRegistryImpl::CachePathsFromState(Impl::FEventContext& EventContext, const FAssetRegistryState& InState,
	const FAssetRegistrySerializationOptions* InOptions)
{
	...
	{
#if WITH_EDITOR
		InOptions = &DevelopmentSerializationOptions;
#else
		InOptions = &SerializationOptions;
#endif
	}
	const FAssetRegistrySerializationOptions& Options = *InOptions;
...
void FAssetRegistryImpl::GetAllocatedSize(bool bLogDetailed, SIZE_T& StateSize, SIZE_T& StaticSize, SIZE_T& SearchSize) const
{
	...
	if (bIsTempCachingEnabled && !bIsTempCachingAlwaysEnabled)
	{
		...
		UE_LOGF(LogAssetRegistry, Warning, "Asset Registry Temp caching enabled, wasting memory: %lldk", TempCacheMem / 1024);
	}
...
void UAssetRegistryImpl::CopyManageDependenciesToState(FAssetRegistryState& InOutState) const
{
	using namespace UE::AssetRegistry;
	...
	{
		FDependsNode* ManagerNode = Pair.Value;
		Dependencies.Reset();
		ManagerNode->GetDependencies(Dependencies, EDependencyCategory::Manage);
		// Remove dependencies to packages that do not exist in InOutState, unless they are PrimaryAssets. The
		// AssetRegistryGenerator's state does not pre-add the PrimaryAsset nodes, this function needs to add them.
		// AddDependencies will create the node if it does not already exist.
		Dependencies.RemoveAllSwap([&InOutState](const FAssetDependency& AssetDependency)
			{
				if (AssetDependency.AssetId.GetPrimaryAssetId().IsValid())
				{
					return false;
				}
				if (InOutState.CachedPackageData.Contains(AssetDependency.AssetId.PackageName))
				{
					return false;
				}
				return true;
		...
		{
			InOutState.AddDependencies(Pair.Key, Dependencies);
		}
	}
}
	...
	, bIgnoreInvalidPathWarning(!!(InScanFlags & UE::AssetRegistry::EScanFlags::IgnoreInvalidPathWarning))
{
	...
	if (bIgnoreDenyListScanFilters && !bForceRescan)
	{
		...
		UE_LOGF(LogAssetRegistry, Warning, "ScanPathsSynchronous: bIgnoreDenyListScanFilters==true is only valid when bForceRescan==true. Setting bForceRescan=true.");
		bForceRescan = true;
		bLogCallstack = true;
	}
	...
	for (const FString& InFile : InFiles)
	{
		...
		else if (!FPackageName::TryConvertToMountedPath(InFile, &LocalPath, &PackageName, nullptr, nullptr, &Extension, &FlexNameType))
		{
			...
			{
				UE_LOGF(LogAssetRegistry, Warning, "ScanPathsSynchronous: %ls is not in a mounted path, will not scan.", *InFile);
				bLogCallstack = true;
			}
			continue;
		}
		if (FPackageName::IsTempPackage(PackageName))
		{
			if (!bIgnoreInvalidPathWarning)
			{
				UE_LOGF(LogAssetRegistry, Warning, "ScanPathsSynchronous: %ls is in the /Temp path, will not scan.", *InFile);
				bLogCallstack = true;
			}
			continue;
		}
	...
	for (const FString& InDir : InDirs)
	{
		...
		else if (!FPackageName::TryConvertToMountedPath(InDir, &LocalPath, &PackageName, nullptr, nullptr, &Extension, &FlexNameType))
		{
			if (!bIgnoreInvalidPathWarning)
			{
				UE_LOGF(LogAssetRegistry, Warning, "ScanPathsSynchronous: %ls is not in a mounted path, will not scan.", *InDir);
				bLogCallstack = true;
			}
			continue;
		}
		if (FPackageName::IsTempPackage(PackageName))
		{
			if (!bIgnoreInvalidPathWarning)
			{
				UE_LOGF(LogAssetRegistry, Warning, "ScanPathsSynchronous: %ls is in the /Temp path, will not scan.", *InDir);
				bLogCallstack = true;
			}
			continue;
		}
...
void FAssetRegistryImpl::ScanPathsSynchronous(FInterfaceWriteScopeLock* ScopeLock, Impl::FScanPathContext& Context)
{
	...
	auto IsInRequestedDir = [&Context](const FAssetData& AssetData)
		{
			TStringBuilder<NAME_SIZE> PackageNameStr;
			...
			return Context.PackageFiles.ContainsByHash(GetTypeHash(PackageName), PackageName);
		};
				...
				auto AddGatheredAssets = [&IsInRequestedDir, OutFoundAssets](TSet<FSoftObjectPath>& InOutGatheredAssets, const TMultiMap<FName, TUniquePtr<FAssetData>>& AssetMap)
					{
						for (TMultiMap<FName, TUniquePtr<FAssetData>>::TConstIterator It = AssetMap.CreateConstIterator(); It; ++It)
						{
							...
							if (OutFoundAssets)
							{
								...
								{
									UE_LOGF(LogAssetRegistry, VeryVerbose, "FAssetRegistryImpl::ScanPathsSynchronous: Found Asset: %ls",
...
FAssetData* FAssetRegistryImpl::ResolveAssetIdCollision(FAssetData& A, FAssetData& B)
{
	...
	FString FileNameB;
	UE_LOGF(LogAssetRegistry, Warning,
		"Invalid duplicate copies of ExternalActor %ls. Resolve by deleting the package that is invalid. Chosing alphabetically for this process."
		"\n\tDiscarding: %ls"
		"\n\tKeeping:    %ls",
	...
	FTopLevelAssetPath& OutRequestedGameThreadClass)
{
	...
	while (!AssetClass)
	{
		if (++CycleCount > CycleCountBeforeCycleDetection)
		{
			...
			if (bExists)
			{
				...
				UE_LOGF(LogAssetRegistry, Warning,
					"Cycle detected in ParentClasses, ObjectRedirectors, or CoreRedirects, starting from AssetClassPath %ls. Skipping AssetRegistry normalization of assets with that class." "\n\tCycle: { %ls }",
	...
	if (!AssetClass)
	{
		...
		if (bLogMissingClassWarnings)
		{
			...
			UE_LOGF(LogAssetRegistry, Verbose,
				"Unable to PostLoadAssetRegistryTags for '%ls' because ancestor class '%ls' cannot be found. %ls", 
	...
	if (!Context.PostLoadCouldComplete())
	{
		...
		if (!bCanCreateObjects)
		{
			...
			UE_LOGF(LogAssetRegistry, Display,
				"Unable to PostLoadAssetRegistryTags for '%ls' because class '%ls' ThreadedPostLoadAssetRegistryTags requires GameThread to load its CDO, but we need it immediately when off the game thread.",
		...
		else
		{
			...
			{
				UE_LOGF(LogAssetRegistry, Display,
					"Unable to PostLoadAssetRegistryTags for '%ls' because class '%ls' ThreadedPostLoadAssetRegistryTags returned !PostLoadCouldComplete.",
					*AssetData->GetObjectPathString(), *AssetClass->GetPathName());
			}
...
bool FAssetRegistryImpl::ShouldSkipGatheredAsset(FAssetData& AssetData)
{
	...
	if (UE::String::FindFirst(PackageNameStr, ExternalActorsFolderName) != INDEX_NONE)
	{
		...
		{
			UE_LOGF(LogAssetRegistry, Verbose,
				"Invalid ExternalActor: Package %ls is an ExternalActor package but is not in the expected root path for ExternalActors /%.*ls/%.*ls. Ignoring this actor.",
		...
		if (bAllowValidation && !PackageNameRelPathAfterExternalActorRoot.StartsWith(ObjectPathRelPath))
		{
			...
			UE_LOGF(LogAssetRegistry, Verbose,
				"Invalid ExternalActor: Package %ls is an ExternalActor package but its path does not match the expected path %ls created from its objectpath %ls. Ignoring this actor.",
	...
	EDeferFlag DeferFlag)
{
	...
	for (TMultiMap<FName, TUniquePtr<FAssetData>>::TIterator Iter(AssetResults); Iter && !bInterrupted; ++Iter)
	{
		...
		if (bVerifyMountPointAfterGather)
		{
			...
			{
				UE_LOGF(LogAssetRegistry, Display,
					"AssetRegistry: An asset has been loaded with an invalid mount point: '%ls', Mount Point: '%ls'. Ignoring the asset.",
...
#if WITH_EDITOR
		bool bCouldPostLoad = false;
		FTopLevelAssetPath RequestedGameThreadClass;
		{
			// Take the FGCScopeGuard for as short a duration as possible to avoid blocking 
			// the game thread for long if it needs to collect garbage. The GCLock doesn't 
			// have a means to signal to our interruption context like the InterfaceLock can.
			FGCScopeGuard Guard;
			bCouldPostLoad = TryPostLoadAssetRegistryTags(BackgroundResult.Get(), DeferFlag, RequestedGameThreadClass);
		}
		if (!bCouldPostLoad)
		{
	...
	Impl::FInterruptionContext& InOutInterruptionContext, TOptional<TSet<FString>>& MountPointsForVerifyAfterGather)
{
	...
	while (PathResults.Num() > 0)
	{
		...
		{
			UE_LOGF(LogAssetRegistry, Display,
				"AssetRegistry: A path has been loaded with an invalid mount point: '%ls', Mount Point: '%ls'. Ignoring the path.",
				*Path, *PackageRoot);
		}
	...
	TOptional<TSet<FString>>& MountPointsForVerifyAfterGather)
{
		...
		for (TMultiMap<FName, FPackageDependencyData>::TIterator Iter(DependsResults); Iter && !bInterrupted; ++Iter)
		{
			...
			else
			{
				if (bLocalVerifyMountPointAfterGather)
				{
					...
					{
						UE_LOGF(LogAssetRegistry, Display,
							"AssetRegistry: DependencyData has been loaded with an invalid mount point: '%ls', Mount Point: '%ls'. Ignoring the DependencyData.",
							*PackageNameString, *PackageRoot);
						continue;
					}
				}

				FAssetIdentifier SourceNodeIdentifier(SourceNodePackageName);
				FDependsNode* SourceNode = State.CreateOrFindDependsNode(SourceNodeIdentifier, &bSourceNodeAlreadyExisted);
				...
				{
					// The node was created by an earlier package registering SourceNode as a dependency, and it may
					// have had a mismatching capitalization that was saved into the package with the dependency.
					// Force the capitalization of the DependsNode for SourceNodePackage to match SourceNodePackage's
					// capitalization (which comes from its filepath on disk) if they are different.
					if (SourceNode->GetPackageName().GetDisplayIndex() != SourceNodePackageName.GetDisplayIndex())
					{
						State.CachedDependsNodes.Remove(SourceNode->GetIdentifier());
						SourceNode->SetIdentifier(SourceNodeIdentifier);
						State.CachedDependsNodes.Add(SourceNode->GetIdentifier(), SourceNode);
					}
				}
	...
	ParallelFor(TEXT("DependencyGatheredData_SortNodes"), ModifiedDependNodes.GetMaxIndex(), 1, 
		[&ModifiedDependNodes, bShouldSortReferencers = ShouldSortReferencers()] (int Index)
		{
			FSetElementId Id = FSetElementId::FromInteger(Index);
			if (!ModifiedDependNodes.IsValidId(Id))
			{
				return;
			}
			FDependsNode* Node = ModifiedDependNodes.Get(Id);
	...
	TRingBuffer<FString>& CookedPackageNamesWithoutAssetDataResults, Impl::FInterruptionContext& InOutInterruptionContext)
{
	...
	if (ShouldProcessCookedPackages.bShouldProcess)
	{
		while (CookedPackageNamesWithoutAssetDataResults.Num() > 0)
		{
			...
			UE_LOGF(LogAssetRegistry, Warning, "CookedPackageNamesWithoutAssetDataGathered : Deprecated in 5.6 due to poor performance, will be removed in a future version, contact Epic if you need this functionality.");
		}
	...
	for (int32 FileIdx = 0; FileIdx < FileChangesProcessed.Num(); ++FileIdx)
	{
		if (FileChangesProcessed[FileIdx].Action == FFileChangeData::FCA_RescanRequired)
		{
			if (bInitialSearchStarted && !IsInitialSearchCompleted()) // Initial search only
			{
				...
				UE_LOGF(LogAssetRegistry, Log, "FAssetRegistry ignoring rescan request for %ls during startup", *FileChangesProcessed[FileIdx].Filename);
			}
			else
			{
		...
		if (bIsValidPackage)
		{
			...
			case FFileChangeData::FCA_Added:
				...
				UE_LOGF(LogAssetRegistry, Verbose, "File was added to content directory: %ls", *File);
				break;
			...
			case FFileChangeData::FCA_Modified:
				...
				bAddedOrCreated = true;
				UE_LOGF(LogAssetRegistry, Verbose, "File changed in content directory: %ls", *File);
				break;
			...
			case FFileChangeData::FCA_Removed:
				...
				UE_LOGF(LogAssetRegistry, Verbose, "File was removed from content directory: %ls", *File);
				break;
			}
			if (bAddedOrCreated && CachedEmptyPackages.Contains(LongPackageFName))
			{
				UE_LOGF(LogAssetRegistry, Warning, "%ls: package was marked as deleted in editor, but has been modified on disk. It will once again be returned from AssetRegistry queries.",
					*File);
				RemoveEmptyPackage(LongPackageFName);
			}
		}
		else if (bIsValidPackageName)
		{
			...
			if (FAssetDataGatherer::IsVerseFile(File))
			{
				...
				case FFileChangeData::FCA_Added:
					...
					NewFiles.AddUnique(File);
					UE_LOGF(LogAssetRegistry, Verbose, "Verse file was added to content directory: %ls", *File);
					break;
				...
				case FFileChangeData::FCA_Removed:
					...
					UE_LOGF(LogAssetRegistry, Verbose, "Verse file was removed from content directory: %ls", *File);
					break;
				}
			}
			else
			{
				...
				case FFileChangeData::FCA_Added:
				{
					...
					{
						NewDirs.Add(File);
						UE_LOGF(LogAssetRegistry, Verbose, "Directory was added to content directory: %ls", *File);
					}
					break;
				}
				case FFileChangeData::FCA_Removed:
				{
					...
					UE_LOGF(LogAssetRegistry, Verbose, "Directory was removed from content directory: %ls", *File);
					break;
				}
				default:
					break;
	...
	Impl::FClassInheritanceContext& InheritanceContext, FString& DirPath, int64 BeforeTimeStamp)
{
	...
	else
	{
		...
		for (FString& MountedLongPackageName : ContentRoots)
		{
			...
			{
				FString NormalizeMountedFilePath = UE::AssetRegistry::CreateStandardFilename(MountedFilePath);
	...
	ParallelFor(NumDirs, [this, &DirPathsAndPackageNames, &Results, &BeforeDateTime](int32 DirIndex)
		{
			...
			FPackageName::IteratePackagesInDirectory(LocalPath,
				...
				(const TCHAR* Filename, const FFileStatData& StatData)
				{
					...
					FStringView RelPath;
					FString NormalizedFilename = UE::AssetRegistry::CreateStandardFilename(FStringView(Filename));
...
{
	LLM_SCOPE(ELLMTag::AssetRegistry);
	AddLoadedAssetToProcess(*AssetLoaded);
}

void UAssetRegistryImpl::AddLoadedAssetToProcess(const UObject& AssetLoaded)
{
	LoadedAssetsToProcess.Enqueue(&AssetLoaded);
}
	...
	UE::AssetRegistry::Impl::EGatherStatus Status, UE::AssetRegistry::Impl::FInterruptionContext& InOutInterruptionContext)
{
...

	{
		LLM_SCOPE(ELLMTag::AssetRegistry);
		UE::AssetRegistry::FInterfaceWriteScopeLock InterfaceScopeLock(InterfaceLock);
		GuardedData.GetProcessLoadedAssetsBatch(LoadedAssetsToProcess, BatchObjects, BatchSize, bUpdateDiskCacheAfterLoad);
	...
	while (BatchObjects.Num() > 0)
	{
		...
		LLM_SCOPE(ELLMTag::AssetRegistry);
		UE::AssetRegistry::FInterfaceWriteScopeLock InterfaceScopeLock(InterfaceLock);
		GuardedData.PushProcessLoadedAssetsBatch(EventContext, LoadedAssetsToProcess, BatchAssetDatas,
...
void FAssetRegistryImpl::GetProcessLoadedAssetsBatch(TQueue<TWeakObjectPtr<const UObject>, EQueueMode::Mpsc>& LoadedAssetsToProcess, TArray<const UObject*>& OutLoadedAssets, uint32 BatchSize,
	bool bUpdateDiskCacheAfterLoad)
{
	...
	TWeakObjectPtr<const UObject> AssetToProcess;
	while (OutLoadedAssets.Num() < static_cast<int32>(BatchSize) && LoadedAssetsToProcess.Dequeue(AssetToProcess))
	{
		const UObject* LoadedAsset = AssetToProcess.Get();
		if (!LoadedAsset)
		{
		...
		FString LocalPath;
		if (UNLIKELY(!FPackageName::TryConvertLongPackageNameToFilename(LoadedAsset->GetPackage()->GetName(), LocalPath)))
		{
			continue;
		}
	...
	TArrayView<FAssetData> LoadedAssetDatas, TArrayView<const UObject*> UnprocessedFromBatch)
{
	...
	for (const UObject* Obj : UnprocessedFromBatch)
	{
		LoadedAssetsToProcess.Enqueue(Obj);
	}
}
...
void UAssetRegistryImpl::OnMountPointMounted(const TRefCountPtr<UE::PackageName::IMountPoint>& MountPoint)
{
	if (SuspendMountingCounter.load(std::memory_order_acquire) > 0)
	{
		FScopeLock Guard(&MountingCriticalSection);
		ListOfMountedMountPoints.Emplace(MountPoint);
		return;
	}

	OnMultipleMountPointsMounted(TConstArrayView<TRefCountPtr<UE::PackageName::IMountPoint>>(&MountPoint, 1));
}

void UAssetRegistryImpl::OnMultipleMountPointsMounted(TConstArrayView<TRefCountPtr<UE::PackageName::IMountPoint>> MountedMountPoints)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(AssetRegistry_OnMultipleMountPointsMounted);
	LLM_SCOPE(ELLMTag::AssetRegistry);
	...
	if (UE::AssetRegistry::Impl::IsDirectoryWatcherEnabled())
	{
		...
		for (const TRefCountPtr<UE::PackageName::IMountPoint>& MountPoint : MountedMountPoints)
		{
			if (DirectoryWatcher)
			{
				// Make sure the directory exits on disk so that the OS level DirectoryWatcher can be used to monitor it.
				IPlatformFile::GetPlatformPhysical().CreateDirectoryTree(*WriteToString<512>(MountPoint->GetLocalPathStandard()));
			}
		}
	}
#endif
	...
	UE::AssetRegistry::Impl::FActiveMountEvents ActiveMountEvents;
	{
		...
		UE::AssetRegistry::Impl::FClassInheritanceBuffer InheritanceBuffer;
		GetInheritanceContextWithRequiredLock(InterfaceScopeLock, InheritanceContext, InheritanceBuffer);
		GuardedData.OnMultipleMountPointsMounted(*this, EventContext, ActiveMountEvents,
			InheritanceContext, InterfaceScopeLock, MountedMountPoints);

		// Listen for directory changes in this content path
#if WITH_EDITOR
		for (const TRefCountPtr<UE::PackageName::IMountPoint>& MountPoint : MountedMountPoints)
		{
			const FString StandardFileSystemPath = UE::AssetRegistry::CreateStandardFilename(MountPoint->GetLocalPathStandard());
			// In-game doesn't listen for directory changes
			if (DirectoryWatcher && !IsDirAlreadyWatchedByRootWatchers(StandardFileSystemPath))
			{
				const FString AssetPathWithTrailingSlash = UE::AssetRegistry::CreateStandardLongPackageNameDirectory(MountPoint->GetLongPackageName());
...
void FAssetRegistryImpl::OnMultipleMountPointsMounted(UAssetRegistryImpl& UARI, Impl::FEventContext& EventContext,
	Impl::FActiveMountEvents& ActiveMountEvents,
	Impl::FClassInheritanceContext& InheritanceContext,
	UE::AssetRegistry::FInterfaceWriteScopeLock& ScopeLock, 
	TConstArrayView<TRefCountPtr<UE::PackageName::IMountPoint>> MountedMountPoints)
{
	for (const TRefCountPtr<UE::PackageName::IMountPoint>& MountPoint : MountedMountPoints)
	{
		FString FileSystemPath(MountPoint->GetLocalPathStandard());
		const FString AssetPathWithTrailingSlash = UE::AssetRegistry::CreateStandardLongPackageNameDirectory(MountPoint->GetLongPackageName());
		...
		if (GlobalGatherer.IsValid() && bSearchAllAssets)
		{
			...
			else
			{
				if (!IsGathering())
				{
					CSV_TRACE_REGION_BEGIN(TEXT("Additional Mount Search"), TEXT("AssetRegistry")); // Matching CSV_TRACE_REGION_END in OnAdditionalMountSearchCompleted
				...
				GlobalGatherer->AddMountPoint(MountPoint->GetLocalPathStandard(), MountPoint->GetLongPackageName());
				GlobalGatherer->SetIsOnAllowList(MountPoint->GetLocalPathStandard(), true);
			}
		}
	}

	UARI.ActiveMountsLoader.OnMountPointsMounted(UARI, ActiveMountEvents, MountedMountPoints);
}

}

void UAssetRegistryImpl::OnMountPointDismounted(const TRefCountPtr<UE::PackageName::IMountPoint>& MountPoint)
{
	if (SuspendDismountingCounter.load(std::memory_order_acquire) > 0)
	{
		FScopeLock Guard(&MountingCriticalSection);
		ListOfDismountedMountPoints.Emplace(MountPoint);
		return;
	}

	OnMultipleMountPointsDismounted(TConstArrayView<TRefCountPtr<UE::PackageName::IMountPoint>>(&MountPoint, 1));
}

void UAssetRegistryImpl::OnMultipleMountPointsDismounted(TConstArrayView<TRefCountPtr<UE::PackageName::IMountPoint>> DismountedMountPoints)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(AssetRegistry_OnMultipleMountPointsDismounted);
#if WITH_EDITOR
	FDirectoryWatcherModule& DirectoryWatcherModule = FModuleManager::LoadModuleChecked<FDirectoryWatcherModule>(TEXT("DirectoryWatcher"));
	IDirectoryWatcher* DirectoryWatcher = nullptr;
	if (UE::AssetRegistry::Impl::IsDirectoryWatcherEnabled())
	...
	UE::AssetRegistry::Impl::FActiveMountEvents ActiveMountEvents;
	{
		LLM_SCOPE(ELLMTag::AssetRegistry);
		UE::AssetRegistry::FInterfaceWriteScopeLock InterfaceScopeLock(InterfaceLock);
		GuardedData.OnMultipleMountPointsDismounted(*this, EventContext, ActiveMountEvents,
			InterfaceScopeLock, DismountedMountPoints);

		// Stop listening for directory changes in this content path
#if WITH_EDITOR
		for (const TRefCountPtr<UE::PackageName::IMountPoint>& MountPoint : DismountedMountPoints)
		{
			const FString StandardFileSystemPath = UE::AssetRegistry::CreateStandardFilename(MountPoint->GetLocalPathStandard());
			if (DirectoryWatcher && !IsDirAlreadyWatchedByRootWatchers(StandardFileSystemPath))
			{
				// Make sure OnDirectoryChangedDelegateHandles key is symmetrical with the one used in OnMountPointMounted
				FString AssetPathWithTrailingSlash = UE::AssetRegistry::CreateStandardLongPackageNameDirectory(MountPoint->GetLongPackageName());
...
void FAssetRegistryImpl::OnMultipleMountPointsDismounted(UAssetRegistryImpl& UARI,
	Impl::FEventContext& EventContext, Impl::FActiveMountEvents& ActiveMountEvents,
	UE::AssetRegistry::FInterfaceWriteScopeLock& ProofOfWriteLock,
	TConstArrayView<TRefCountPtr<UE::PackageName::IMountPoint>> DismountedMountPoints)
{
	UARI.ActiveMountsLoader.OnMountPointsDismounted(UARI, ActiveMountEvents, ProofOfWriteLock, DismountedMountPoints);

	// For the GlobalGatherer we remove all input MountPoints; we don't need to keep information about their gather.
	// But for the FAssetData stored inside our AssetRegistryState, for some mount points
	// we are configured to keep those assets in memory even when dismounted.
	TArray<TRefCountPtr<UE::PackageName::IMountPoint>> MountPointsToRemoveAssetsFrom;
	MountPointsToRemoveAssetsFrom.Reserve(DismountedMountPoints.Num());
	for (const TRefCountPtr<UE::PackageName::IMountPoint>& MountPoint : DismountedMountPoints)
	{
		if (GlobalGatherer.IsValid())
		{
			GlobalGatherer->RemoveMountPoint(MountPoint->GetLocalPathStandard());
		}

		// The input AssetPaths of the unmounted MountPoints are unnormalized and might have a trailing slash,
		// but we are comparing them by exact string inside our containers 
		// and need to normalize them to not have a trailing slash.
		FName AssetPathNoTrailingSlashFName = UE::AssetRegistry::CreateStandardLongPackageFNameNoTrailingSlash(
			MountPoint->GetLongPackageName());
		if (PersistentMountPoints.Contains(AssetPathNoTrailingSlashFName))
		{
			// This path is marked to never remove its AssetDatas.
			continue;
		}

		MountPointsToRemoveAssetsFrom.Add(MountPoint);
	}
	UnloadMountPoints(EventContext, MountPointsToRemoveAssetsFrom);
}

void FAssetRegistryImpl::UnloadMountPoints(Impl::FEventContext& EventContext,
	TConstArrayView<TRefCountPtr<UE::PackageName::IMountPoint>> MountPoints)
{
	// Collect the list of paths from given list of roots
	TSet<FName> AssetPathsToRemoveAssetsFrom;
	TArray<FName> MountPointRootNoTrailingSlashFNames;

	const bool bRecurse = true;
	MountPointRootNoTrailingSlashFNames.Reserve(MountPoints.Num());
	for (const TRefCountPtr<UE::PackageName::IMountPoint>& MountPoint : MountPoints)
	{
		FName AssetPathNoTrailingSlashFName = UE::AssetRegistry::CreateStandardLongPackageFNameNoTrailingSlash(
			MountPoint->GetLongPackageName());
		CachedPathTree.GetSubPaths(AssetPathNoTrailingSlashFName, AssetPathsToRemoveAssetsFrom, bRecurse);
		AssetPathsToRemoveAssetsFrom.Add(AssetPathNoTrailingSlashFName);
		MountPointRootNoTrailingSlashFNames.Add(AssetPathNoTrailingSlashFName);
	}

	// Remove all cached assets and Verse files found in the mountpoints.
	{
		TArray<FAssetData*> AllAssetDataToRemove;
		for (FName PathName : AssetPathsToRemoveAssetsFrom)
		{
			// Gather assets
	...
	// Remove the root paths of the mountpoints.
	for (FName AssetPathNoTrailingSlashFName : MountPointRootNoTrailingSlashFNames)
	{
		const bool bEvenIfAssetsStillExist = true;
		RemoveAssetPath(EventContext, AssetPathNoTrailingSlashFName, bEvenIfAssetsStillExist);
	}
}
...
void FAssetRegistryImpl::AddCachedBPClassParent(const FTopLevelAssetPath& ClassPath, const FTopLevelAssetPath& NotYetRedirectedParentPath)
{
	...
	{
		UE_LOGF(LogAssetRegistry, Error,
			"Class redirect exists from %ls -> %ls, which is invalid because ClassNames must be TopLevelAssetPaths. " "Redirect will be ignored in AssetRegistry queries.",
...
void UAssetRegistryImpl::SuspendPathsMounting()
{
	SuspendMountingCounter.fetch_add(1, std::memory_order_acq_rel);
}

void UAssetRegistryImpl::ResumePathsMounting()
{
	if (SuspendMountingCounter.fetch_sub(1, std::memory_order_acq_rel) == 1)
	{
		TArray<TRefCountPtr<UE::PackageName::IMountPoint>> LocalList;
		{
			FScopeLock Guard(&MountingCriticalSection);
			LocalList = MoveTemp(ListOfMountedMountPoints);
		}
		if (!LocalList.IsEmpty())
		{
			OnMultipleMountPointsMounted(LocalList);
		}
	}
}

void UAssetRegistryImpl::SuspendPathsDismounting()
{
	SuspendDismountingCounter.fetch_add(1, std::memory_order_acq_rel);
}

void UAssetRegistryImpl::ResumePathsDismounting()
{
	if (SuspendDismountingCounter.fetch_sub(1, std::memory_order_acq_rel) == 1)
	{
		TArray<TRefCountPtr<UE::PackageName::IMountPoint>> LocalList;
		{
			FScopeLock Guard(&MountingCriticalSection);
			LocalList = MoveTemp(ListOfDismountedMountPoints);
		}
		if (!LocalList.IsEmpty())
		{
			OnMultipleMountPointsDismounted(LocalList);
		}
	}
}
...
void PrintAssetRegistryManagementPathsPackageDebugInfo(FDependsNode* Node, UE::AssetRegistry::FSetManageReferencesNodeDataMap* NodeData)
{
	if (Node)
	{
		UE_LOGF(LogAssetRegistry, Display, "SetManageReferences is printing out the reference chain that caused '%ls' to be managed", *GAssetRegistryManagementPathsPackageDebugName);
		...
		for (;;)
		{
			...
			UE_LOGF(LogAssetRegistry, Display, "  %ls", *ReferencingNode->GetIdentifier().ToString());
			if (AllVisitedNodes.Contains(ReferencingNode))
			{
				UE_LOGF(LogAssetRegistry, Display, "  ... (Circular reference back to %ls)", *ReferencingNode->GetPackageName().ToString());
				break;
			}
	...
	{
		UE_LOGF(LogAssetRegistry, Warning, "Node with AssetRegistryManagementPathsPackageDebugName '%ls' was not found", *GAssetRegistryManagementPathsPackageDebugName);
	}
}
#endif // WITH_EDITOR
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FAssetRegistryImpl::SetManageReferences);
	if (!Context.ManagerMap)
	{
		UE_LOGF(LogAssetRegistry, Error, "SetManageReferences: Context.ManagerMap must not be null.");
		FDebug::DumpStackTraceToLog(ELogVerbosity::Warning);
		return;
	}
	if (!Context.ShouldSetManager.IsSet())
	{
		UE_LOGF(LogAssetRegistry, Error, "SetManageReferences: Context.ShouldSetManager must be a bound pointer.");
		FDebug::DumpStackTraceToLog(ELogVerbosity::Warning);
		return;
	}
	...
	for (const TPair<FAssetIdentifier, FAssetIdentifier>& Pair : *Context.ManagerMap)
	{
		...
		if (!ManagedNode)
		{
			UE_LOGF(LogAssetRegistry, Warning, "Cannot set %ls to manage asset %ls because %ls does not exist!", 
...
void UAssetRegistryImpl::Broadcast(UE::AssetRegistry::Impl::FEventContext& EventContext, bool bAllowFileLoadedEvent)
{
	...
	if (EventContext.RequiredLoads.Num())
	{
		...
		{
			RequestIds.Add(LoadPackageAsync(*RequiredLoad));
		}
	...
	if (EventContext.bFileLoadedEventBroadcast || EventContext.bKnownGathersCompleteEventBroadcast)
	{
		...
		if (bLocalFileLoadedEventBroadcast)
		{
			...
			UE_LOGF(LogAssetRegistry, Display, "Starting OnFilesLoaded.Broadcast");
			FileLoadedEvent.Broadcast();
			ScanEndedEvent.Broadcast();
			UE_LOGF(LogAssetRegistry, Display, "Completed OnFilesLoaded.Broadcast: %.3fs",
				FPlatformTime::Seconds() - BroadcastTimeStart);
...
bool ShouldSearchAllAssetsAtStart()
{
	...
	if (GIsEditor)
	{
		...
		else
		{
			...
			FString CommandletToken("commandlet");
...
FString CreateStandardFilename(FStringView InPath)
{
	return CreateStandardFilename(FString(InPath));
}
...
FString CreateStandardLongPackageNameDirectory(FStringView LongPackageName)
{
	FString Result(LongPackageName);
	if (!Result.EndsWith(TEXT("/"), ESearchCase::CaseSensitive))
	{
		Result = Result + TEXT("/");
	}
	return Result;
...
FName CreateStandardLongPackageFNameNoTrailingSlash(FStringView LongPackageName)
{
	FStringView ResultNoTrailingSlash = LongPackageName;
	if (ResultNoTrailingSlash.EndsWith(TEXT("/"), ESearchCase::CaseSensitive))
	{
		// We don't want a trailing slash here as it could interfere with RemoveAssetPath
		ResultNoTrailingSlash.LeftChopInline(1);
	}
	return FName(ResultNoTrailingSlash);
}
