	...
	: RootParametersSize(0)
{
...
#if WITH_NVAPI
	if (QBSS.bNeedsNvIntrinsicsSpace)
	{
		check(RootParameterCount < MaxRootParameters);

		// Must use a DescriptorTable since structured buffers with counters can't be bound via root descriptors
	...
	{
		UE_LOGF(LogD3D12RHI, Verbose, "Root signature created where the root parameters take up %u DWORDS. Using more than %u DWORDs can negatively impact performance depending on the hardware and root parameter usage.", RootParametersSize, SizeWarningThreshold);
	}
#endif
...
void FD3D12RootSignature::Init(const D3D12_VERSIONED_ROOT_SIGNATURE_DESC& InDesc, ERootSignatureType InRootSignatureType)
{
	...
	{
		UE_LOGF(LogD3D12RHI, Fatal, "D3DX12SerializeVersionedRootSignature failed with error %ls", ANSI_TO_TCHAR(Error->GetBufferPointer()));
	}
