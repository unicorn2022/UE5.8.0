...
struct FD3D12QuantizedBoundShaderState
{
	...
	union
	{
		struct
		{
			...
			uint8 bNeedsNvIntrinsicsSpace : 1;
...
inline bool D3D12DoesShaderFrequencySupportUAV(EShaderFrequency ShaderFrequency)
{
	return ShaderFrequency == SF_Pixel
		|| ShaderFrequency == SF_Vertex
		|| ShaderFrequency == SF_Compute
#if PLATFORM_SUPPORTS_MESH_SHADERS
		|| ShaderFrequency == SF_Mesh
		|| ShaderFrequency == SF_Amplification
#endif
		;
}
