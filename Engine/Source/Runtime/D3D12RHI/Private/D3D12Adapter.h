...
struct FD3D12DeviceBasicInfo
{
	...
	bool                        bUMA;
	bool                        bCacheCoherentUMA;
	bool                        bSupportsGPUUploadHeaps;
...
struct FD3D12AdapterDesc
{
	...
	/** Whether the GPU is tile-based. */
	bool bTileBasedRenderer = false;

	/** Whether the GPU has unified memory. This can be used to identify integrated GPU as well. */
	bool bUMA = false;

	/** Whether the UMA device has cache coherency between the GPU and CPU.  */
	bool bCacheCoherentUMA = false;

	/** Whether the device supports GPU Upload Heaps  */
	bool bSupportsGPUUploadHeaps = false;
...
public:
	...
	static void CreateDXGIFactory(TRefCountPtr<IDXGIFactory2>& DxgiFactory2, bool bWithDebug);
	...
	const FRHIMemoryStats& CollectMemoryStats();
	FORCEINLINE const FRHIMemoryStats& GetMemoryStats() const { return MemoryStats; }
	...
	/**
	 * Controls the heuristics used by Windows Video Memory Manager to demote resources from GPU physical memory when it is
	 * overcommitted or fragmented. The D3D12 RHI performs access tracking and manual residency management, however it is
	 * still possible to overcommit under some conditions on low-VRAM GPUs.
	 * Render targets, depth buffers and UAVs are assigned high residency priority by default. 
	 * To differentiate between high priority resources, the size is packed into low 16 bits of the priority value (in units of 10MB).
	 * If ResourceSize parameter is specified, the final priority value will be packed in a mannner consistent with the
	 * default priority assignment. Current priority value is stored in the Pageable private data. The size bits will be
	 * preserved if priority changes but the size is not provided in subsequent calls to this function.
	 * See https://learn.microsoft.com/en-us/windows/win32/direct3d12/residency.
	*/
	void SetResidencyPriority(ID3D12Pageable* Pageable, D3D12_RESIDENCY_PRIORITY HeapPriority,
							  uint32 GPUIndex, TOptional<uint64> ResourceSize = {});
