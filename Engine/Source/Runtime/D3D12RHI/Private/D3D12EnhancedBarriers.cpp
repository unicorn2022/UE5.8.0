...
#define PLATFORM_REQUIRES_ENHANCED_BARRIERS_GFX_ONLY_READ_BITS_HACK !(D3D12RHI_HAS_LAYOUT_DIRECT_QUEUE_GENERIC_READ_COMPUTE_QUEUE_ACCESSIBLE)
...
#define PLATFORM_REQUIRES_LAYOUT_DEPTH_STENCIL_WRITE_NOT_COMPATIBLE_WITH_ACCESS_DEPTH_STENCIL_READ !(PLATFORM_WINDOWS)
...
#if D3D12RHI_HAS_LAYOUT_VIDEO_QUEUE_COMMON
		ENUMVAL(D3D12_BARRIER_LAYOUT_VIDEO_QUEUE_COMMON),
#endif
#if D3D12RHI_HAS_LAYOUT_DIRECT_QUEUE_GENERIC_READ_COMPUTE_QUEUE_ACCESSIBLE
		ENUMVAL(D3D12_BARRIER_LAYOUT_DIRECT_QUEUE_GENERIC_READ_COMPUTE_QUEUE_ACCESSIBLE),
#endif
#if D3D12RHI_COMMON_AND_PRESENT_ARE_DISTINCT
		ENUMVAL(D3D12_BARRIER_LAYOUT_PRESENT),
#endif
		ENUMVAL(D3D12_BARRIER_LAYOUT_UNDEFINED));
}

static bool ResourceIsSimultaneousAccess(
	const FD3D12ResourceDesc& InDesc)
{
	return EnumHasAnyFlags(InDesc.Flags, D3D12_RESOURCE_FLAG_ALLOW_SIMULTANEOUS_ACCESS);
}
	...
	const FD3D12Resource* InResource)
{
	if (InResource)
	{
		return ResourceIsSimultaneousAccess(InResource->GetDesc());
	}
	return false;
}
...
// The below tables may be variably sized based on platform properties. This lets them 
// vary in size without the ugly arithmetic necessary to explicitly calculate and specify
// the size in the type based on those platform properties. It takes an array as input,
// rather than the values themselves, to allow for a trailing comma.
template <typename T, uint32 Size>
static constexpr TStaticArray<T, Size> MakeStaticArray(const T(&InValues)[Size])
{
	// Used to capture the indices
	return []
		<uint32... Indices>
		(const T(&InValues)[Size], TIntegerSequence<uint32, Indices...>)
		{
			return TStaticArray<T, Size>(InValues[Indices]...);
		}(InValues, TMakeIntegerSequence<uint32, Size>{});
}
...
static constexpr
TStaticArray AccessCompatibleSync = MakeStaticArray<D3D12_BARRIER_SYNC>({
...
// Simultaneous access textures can be accessed using any access bits except these.
// This info is rather hidden in the current version of the spec. It appears in the
// section titled "Equivalent D3D12_BARRIER_ACCESS bit for each D3D12_RESOURCE_STATES bit"
static constexpr
D3D12_BARRIER_ACCESS SimultaneousAccessIncompatibleAccess =
	D3D12_BARRIER_ACCESS_DEPTH_STENCIL_WRITE
	| D3D12_BARRIER_ACCESS_DEPTH_STENCIL_READ
	| D3D12_BARRIER_ACCESS_RAYTRACING_ACCELERATION_STRUCTURE_READ
	| D3D12_BARRIER_ACCESS_RAYTRACING_ACCELERATION_STRUCTURE_WRITE;
...
static constexpr
TStaticArray LayoutCompatibleAccess = MakeStaticArray<D3D12_BARRIER_ACCESS>({
...
static constexpr
TStaticArray DirectQueueCompatibleLayouts = MakeStaticArray<D3D12_BARRIER_LAYOUT>({
	D3D12_BARRIER_LAYOUT_COMMON,
	D3D12_BARRIER_LAYOUT_GENERIC_READ,
	D3D12_BARRIER_LAYOUT_RENDER_TARGET,
	D3D12_BARRIER_LAYOUT_UNORDERED_ACCESS,
	...
	D3D12_BARRIER_LAYOUT_RESOLVE_DEST,
	D3D12_BARRIER_LAYOUT_SHADING_RATE_SOURCE,
	// @TODO - Not listed in the spec, but logic tells us it's compatible
	D3D12_BARRIER_LAYOUT_DIRECT_QUEUE_COMMON,
	D3D12_BARRIER_LAYOUT_DIRECT_QUEUE_GENERIC_READ,
	D3D12_BARRIER_LAYOUT_DIRECT_QUEUE_UNORDERED_ACCESS,
	D3D12_BARRIER_LAYOUT_DIRECT_QUEUE_SHADER_RESOURCE,
...
static constexpr
TStaticArray ComputeQueueCompatibleLayouts = MakeStaticArray<D3D12_BARRIER_LAYOUT>({
	D3D12_BARRIER_LAYOUT_COMMON,
	D3D12_BARRIER_LAYOUT_GENERIC_READ,
	D3D12_BARRIER_LAYOUT_UNORDERED_ACCESS,
	D3D12_BARRIER_LAYOUT_SHADER_RESOURCE,
	...
	const FD3D12Resource* InResource)
{
	...
	if (ResourceIsSimultaneousAccess(InResource))
	{
		return !EnumHasAnyFlags(InAccess, SimultaneousAccessIncompatibleAccess);
	}
	...
	ERHIPipeline InPipeline)
{
	for (const T& Barrier : InBarriers)
	{
		...
		if constexpr (std::is_same_v<D3D12_TEXTURE_BARRIER, T>)
		{
			...
			check(AccessIsCompatibleWithLayout(Barrier.AccessBefore, Barrier.LayoutBefore, Barrier.pResource));
			check(AccessIsCompatibleWithLayout(Barrier.AccessAfter, Barrier.LayoutAfter, Barrier.pResource));
		}
	}
	...
	const FD3D12Texture* InTexture)
{
	...
	if (InTexture && ResourceIsSimultaneousAccess(InTexture->GetResource()))
	{
		// Simultaneous access textures have an immutable layout
		return D3D12_BARRIER_LAYOUT_UNDEFINED;
	}
	...
	if (EnumOnlyContainsFlags(InD3D12Access, ED3D12Access::ReadOnlyMask))
	{
		...
		case ERHIPipeline::All:
			if (EnumHasAnyFlags(InD3D12Access, GfxOnlyGenericReadBits))
			{
				...
				return D3D12_BARRIER_LAYOUT_DIRECT_QUEUE_GENERIC_READ;
#elif D3D12RHI_HAS_LAYOUT_DIRECT_QUEUE_GENERIC_READ_COMPUTE_QUEUE_ACCESSIBLE
...
static const ID3D12Resource* GetID3D12ResourcePtr(
	const FD3D12Resource* ResourcePtr)
{
	return ResourcePtr->GetResource();
}

static const ID3D12Resource* GetID3D12ResourcePtr(
	const ID3D12Resource* ResourcePtr)
{
	return ResourcePtr;
}
...
static FString GetResourceName(
	const ID3D12Resource* ResourcePtr)
{
	return GetD312ObjectName(const_cast<ID3D12Resource*>(ResourcePtr));
}

static FString ConvertSubresourceToString(
	uint32 Subresource)
{
	return (Subresource == D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES) ?
		TEXT("All") :
		FString::FromInt(Subresource);
}

static bool ShouldLogBarrier(
	const FString& ResourceName)
{
	const FString& Filter = GD3D12LogBarriersResourceFilter;
	return Filter.IsEmpty() || ResourceName.Contains(Filter);
}
...
static void LogTextureBarrier(
	...
	bool bDiscard)
{
	const FString ResourceName = GetResourceName(pResource);
	if (!ShouldLogBarrier(ResourceName))
	{
		return;
	}
...
D3D12 Texture Barrier
  ...
  |         Discard: %u
	...
	*ResourceName, GetID3D12ResourcePtr(pResource),
	ENUM_STRING_AND_VALUE(SyncBefore),
	ENUM_STRING_AND_VALUE(SyncAfter),
	ENUM_STRING_AND_VALUE(AccessBefore),
	ENUM_STRING_AND_VALUE(AccessAfter),
	...
	*ConvertSubresourceToString(Subresource),
...
#endif // D3D12RHI_BARRIER_LOGGING
...
#if D3D12RHI_BARRIER_LOGGING
	if (GD3D12LogBarriers & D3D12LogBarriersWhenBatched)
	{
...
#if D3D12RHI_BARRIER_LOGGING
		if (GD3D12LogBarriers & D3D12LogBarriersWhenFlushed)
		{
	...
	const FD3D12ResourceDesc& InDesc)
{
	...
	if (bIsBuffer)
	{
		// Buffers don't have a layout
		return D3D12_BARRIER_LAYOUT_UNDEFINED;
	}

	return ResourceIsSimultaneousAccess(InDesc) ?
		// Simultaneous access textures must be created in the COMMON layout
		D3D12_BARRIER_LAYOUT_COMMON :
	...
	const FRHITransitionCreateInfo& CreateInfo)
{
	...
	if (bCreateFence)
	{
		...
		for (uint32 Index : FRHIGPUMask::All())
		{
			...
			{
				DeviceSyncPoints[Pipeline] = FD3D12SyncPoint::Create(ED3D12SyncPointType::GPUOnly, UE_D3D12_STATIC_FNAME("Transition"));
			}
		}
	}
