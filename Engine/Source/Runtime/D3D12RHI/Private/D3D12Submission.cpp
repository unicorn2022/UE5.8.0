...
FDynamicRHI::FRHIFinalizeContextsResult FD3D12DynamicRHI::RHIFinalizeContexts(FRHIFinalizeContextsArgs&& Args)
{
	checkf(Args.UploadContext == nullptr, TEXT("D3D12 RHI does not implement upload contexts."));
	...
	FDynamicRHI::FRHIFinalizeContextsResult Result;
	for (ERHIPipeline Pipeline : MakeFlagsRange(ERHIPipeline::All))
	{
		if (FD3D12CommandContextBase* ContextBase = static_cast<FD3D12CommandContextBase*>(Args.Contexts[Pipeline]))
		{
			FD3D12FinalizedCommands FinalizedCommands;
			if (FD3D12CommandContextRedirector* Redirector = ContextBase->AsRedirector())
			{
				for (uint32 GPUIndex : Redirector->GetPhysicalGPUMask())
				{
					FinalizeContext(Redirector->GetSingleDeviceContext(GPUIndex), FinalizedCommands);
				}
			...
			{
				FinalizeContext(static_cast<FD3D12CommandContext*>(ContextBase), FinalizedCommands);
			}

			if (FinalizedCommands.Num())
			{
				Result.CommandLists[Pipeline] = new FD3D12FinalizedCommands(MoveTemp(FinalizedCommands));
			}
		}
	}
...
FD3D12DynamicRHI::FProcessResult FD3D12DynamicRHI::ProcessSubmissionQueue()
{
	...
	IF_RENDER_TRACING_ENABLED(RenderTracing::D3D12::ProcessSubmissionQueueEnter());
	...
	{
		bProgress = false;
		EnumRemoveFlags(Result.Status, EQueueStatus::Pending);
			...
			{
				{
					...
					while (Payload->SyncPointsToWait.Index < Payload->SyncPointsToWait.Num())
					{
						...
						if (!SyncPoint->ResolvedFence.IsSet())
						{
							...
							IF_RENDER_TRACING_ENABLED(RenderTracing::D3D12::SubmitYieldOnUnresolvedSyncPoint(CurrentQueue, Payload, SyncPoint));
							return;
						}
					...
					while (Payload->ManualFencesToWait.Index < Payload->ManualFencesToWait.Num())
					{
						FD3D12PayloadBase::FManualFence& ManualFence = Payload->ManualFencesToWait[Payload->ManualFencesToWait.Index];
						if (ManualFence.bHasSignalled != nullptr && !(*ManualFence.bHasSignalled))
						{
							// The CPU-side submit action for a manual fence has not happened yet.
							// Don't process this payload until that has, otherwise we can deadlock due to inversion of submission order.
							// Skip processing this queue, and move on to the next. We will retry later when
							// further work is submitted, which may contain the fence we need.
							Result.Status |= EQueueStatus::Pending;
							IF_RENDER_TRACING_ENABLED(RenderTracing::D3D12::SubmitYieldOnUnresolvedManualFence(CurrentQueue, Payload, ManualFence.Fence, ManualFence.Value));
							return;
						}

						Payload->ManualFencesToWait.Index++;
						bProgress = true;
					}
	...
	IF_RENDER_TRACING_ENABLED(RenderTracing::D3D12::ProcessSubmissionQueueExit(static_cast<uint8>(Result.Status)));
	return Result;
}
...
uint64 FD3D12Queue::FinalizePayload(bool bRequiresSignal, FPayloadArray& PayloadsToHandDown)
{
	...
	for (FD3D12SyncPointRef& SyncPoint : PayloadToSubmit->SyncPointsToSignal)
	{
		...
		IF_RENDER_TRACING_ENABLED(RenderTracing::D3D12::SubmitResolveSyncPoint(*this, PayloadToSubmit, SyncPoint, Fence.NextCompletionValue));
...
void FD3D12DynamicRHI::FlushBatchedPayloads(FD3D12Queue::FPayloadArray& PayloadsToSubmit)
{
	...
	auto Wait = [this](FD3D12Payload* Payload)
	{
		...
		for (auto& [LocalFence, Value] : Payload->QueueFencesToWait)
		{
			...
			IF_RENDER_TRACING_ENABLED(RenderTracing::D3D12::SubmitWaitQueueFence(Payload, Queue, *LocalFence.OwnerQueue, Value));
			VERIFYD3D12RESULT(Queue.D3DCommandQueue->Wait(LocalFence.D3DFence, Value));
		}

		// Wait for manual fences
		for (auto& [LocalFence, Value, bFenceSignalled] : Payload->ManualFencesToWait)
		{
			checkf(!bFenceSignalled.IsValid() || *bFenceSignalled, TEXT("The signal operation for a provided manual D3D12 fence has not yet happened."));
			IF_RENDER_TRACING_ENABLED(RenderTracing::D3D12::SubmitWaitManualFence(Payload, Queue, LocalFence, Value));
	...
	auto Flush = [&]()
	{
		...
		IF_RENDER_TRACING_ENABLED(RenderTracing::D3D12::SubmitPlatformCommandLists(
			Queue,
			MakeConstArrayView<FD3D12Payload*>(&PayloadsToSubmit[FirstPayload], LastPayload - FirstPayload),
			D3DCommandLists
		));
	...
	auto Signal = [this](FD3D12Payload* Payload)
	{
		...
		for (auto& [ManualFence, Value, bFenceSignalled] : Payload->ManualFencesToSignal)
		{
			IF_RENDER_TRACING_ENABLED(RenderTracing::D3D12::SubmitSignalManualFence(Payload, Queue, ManualFence, Value));

			VERIFYD3D12RESULT(Queue.D3DCommandQueue->Signal(ManualFence, Value));
			if (bFenceSignalled)
			{
				*bFenceSignalled = true;
			}
		}
		...
		if (Payload->RequiresQueueFenceSignal())
		{
			...
			IF_RENDER_TRACING_ENABLED(RenderTracing::D3D12::SubmitSignalQueueFence(Payload, Queue, Payload->CompletionFenceValue));
		...
		if (Payload->PresentArgs.IsValid())
		{
			Payload->PresentArgs.DrawingViewport->PresentOnSubmissionThread(Payload->PresentArgs);
		}
...
void FD3D12SyncPoint::Wait() const
{
	...
	if (!GraphEvent->IsComplete())
	{
		...
		UE_TRACE_LOG_SCOPED_T(Cpu, SyncPoint_Wait, CpuChannel)
			<< SyncPoint_Wait.WaitingForName(*GetDebugName().ToString());
...
void FD3D12DynamicRHI::ProcessInterruptQueueOnGPUCrash(HRESULT D3DResult)
{
	// This function will not return.

	// We know this function was called due to a GPU crash, so let the thread know.
	GGPUD3DResult = D3DResult;
	GGPUCrashDetected.store(true);
...
FD3D12DynamicRHI::FProcessResult FD3D12DynamicRHI::ProcessInterruptQueue()
{
	...
	IF_RENDER_TRACING_ENABLED(RenderTracing::D3D12::ProcessInterruptQueueEnter());
	...
	ForEachQueue([&](FD3D12Queue& CurrentQueue)
	{
		while (FD3D12Payload** PayloadPtr = CurrentQueue.PendingInterrupt.Peek())
		{
			...
			IF_RENDER_TRACING_ENABLED(RenderTracing::D3D12::InterruptQueueFenceSignaled(CurrentQueue, Payload, CompletedFenceValue, LastSignaledFenceValue));
	...
	if (GGPUCrashDetected.load())
	{
		...
		TerminateOnGPUCrash(GGPUD3DResult);
	}

	IF_RENDER_TRACING_ENABLED(RenderTracing::D3D12::ProcessInterruptQueueExit(static_cast<uint8>(Result.Status)));
	return Result;
}
