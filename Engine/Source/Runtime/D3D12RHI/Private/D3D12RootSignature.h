...
public:
	...
	inline uint32 UAVRDTBindSlot(EShaderFrequency ShaderStage) const
	{
		switch (ShaderStage)
		{
		case SF_Vertex: return BindSlotMap[VS_UAVs];
#if PLATFORM_SUPPORTS_MESH_SHADERS
		case SF_Mesh: return BindSlotMap[MS_UAVs];
		case SF_Amplification: return BindSlotMap[AS_UAVs];
#endif
		case SF_Pixel: return BindSlotMap[ALL_UAVs];

		case SF_NumFrequencies:
		case SF_Compute: return BindSlotMap[ALL_UAVs];
