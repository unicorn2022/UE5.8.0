...
void FD3D12CommandContext::RHICopyToStagingBuffer(FRHIBuffer* SourceBufferRHI, FRHIStagingBuffer* StagingBufferRHI, uint32 Offset, uint32 NumBytes)
{
	...
	
	{
		...
		{
			UE_LOGF(LogD3D12RHI, Warning, "RHICopyToStagingBuffer cannot be used on the RayTracing Accelleration structure %ls", *SourceBufferRHI->GetName().GetPlainNameString());
		}
		else
		{
...
void FD3D12CommandContext::RHISetViewport(float MinX, float MinY, float MinZ, float MaxX, float MaxY, float MaxZ)
{
	...
	if (Viewport.Width > 0 && Viewport.Height > 0)
	{
		...
		RHISetScissorRect(true, FMath::RoundToZero(MinX), FMath::RoundToZero(MinY), FMath::RoundFromZero(MaxX), FMath::RoundFromZero(MaxY));
	}
}
...
struct FD3D12ResourceBinder
{
	...
	void SetParameter(uint32 BufferIndex, uint32 BaseIndex, uint32 NumBytes, const uint8* NewValue)
	{
		checkSlow(BufferIndex == 0);
		ConstantBuffer.UpdateConstant(NewValue, BaseIndex, NumBytes);
	}
	...
	void SetBindlessHandle(const FRHIDescriptorHandle& Handle, uint32 Offset)
	{
		if (Handle.IsValid())
		{
			...
			SetParameter(0, Offset, 4, reinterpret_cast<const uint8*>(&BindlessIndex));
		}
	}
#endif
	...
	void SetDescriptorRange(FRHIDescriptorRange* DescriptorRange, uint32 Offset)
	{
		if (bBindlessResources || bBindlessSamplers)
		{
			if (FD3D12DescriptorRange* D3D12DescriptorRange = FD3D12CommandContext::RetrieveObject<FD3D12DescriptorRange>(DescriptorRange, GpuIndex))
			{
				FRHIDescriptorRangeShaderData ShaderData = D3D12DescriptorRange->GetShaderData();
				SetParameter(0, Offset, sizeof(ShaderData), reinterpret_cast<const uint8*>(&ShaderData));

				// Make sure the referenced resources are resident before use.
				Context.StateCache.QueueBindlessSRVs(Frequency, D3D12DescriptorRange->AllSrvs);
				Context.StateCache.QueueBindlessUAVs(Frequency, D3D12DescriptorRange->AllUavs);
			}
		}
	}
	...
	, TArrayView<const FRHIShaderParameterResource> InBindlessParameters)
{
	...
	{
		Binder.SetParameter(Parameter.BufferIndex, Parameter.BaseIndex, Parameter.ByteSize, &InParametersData[Parameter.ByteOffset]);
	}
	...
	for (const FRHIShaderParameterResource& Parameter : InBindlessParameters)
	{
		if (FRHIResource* Resource = Parameter.Resource)
		{
			...
			else if (Parameter.Type == FRHIShaderParameterResource::EType::DescriptorRange)
			{
				Binder.SetDescriptorRange(static_cast<FRHIDescriptorRange*>(Resource), Parameter.Index);
			}
			else
			{
	...
	for (const FRHIShaderParameterResource& Parameter : InResourceParameters)
	{
		...
		{
			if (D3D12DoesShaderFrequencySupportUAV(ShaderFrequency))
			{
				Binder.SetUAV(static_cast<FRHIUnorderedAccessView*>(Parameter.Resource), Parameter.Index, true);
			}
			else
			{
				checkf(false, TEXT("UAV set on unsupported shader frequency: %s"), GetShaderFrequencyString(ShaderFrequency));
			}
		}
	}
	...
	for (const FRHIShaderParameterResource& Parameter : InResourceParameters)
	{
		...
		case FRHIShaderParameterResource::EType::UniformBuffer:
			BindUniformBuffer(Context, Shader, ShaderFrequency, Parameter.Index, FD3D12CommandContext::RetrieveObject<FD3D12UniformBuffer>(Parameter.Resource, Binder.GpuIndex));
			break;
#if PLATFORM_SUPPORTS_BINDLESS_RENDERING
		case FRHIShaderParameterResource::EType::ResourceCollection:
...
)
{
	...
	{
		check(Method == ERHIShaderBundleMethod::Native);
...
void FD3D12CommandContext::RHIClearMRTImpl(bool* bClearColorArray, int32 NumClearColors, const FLinearColor* ClearColorArray, bool bClearDepth, float Depth, bool bClearStencil, uint32 Stencil)
{
	...
	if (!bSupportsFastClear || !bClearCoversEntireSurface)
	{
		...
		{
			UE_LOGF(LogD3D12RHI, Warning, "RHIClearMRTImpl: Using non-fast clear path! This has performance implications");
			UE_LOGF(LogD3D12RHI, Warning, "       Viewport: Width %d, Height: %d", static_cast<LONG>(FMath::RoundToInt(Viewport.Width)), static_cast<LONG>(FMath::RoundToInt(Viewport.Height)));
			UE_LOGF(LogD3D12RHI, Warning, "   Scissor Rect: Width %d, Height: %d", ScissorRect.right, ScissorRect.bottom);
		}
	}
	...
	if (ClearDSV)
	{
		...
		{
			UE_LOGF(LogD3D12RHI, Warning, "RHIClearMRTImpl: Asking to clear a DSV that does not store depth.");
		}
		...
		{
			UE_LOGF(LogD3D12RHI, Warning, "RHIClearMRTImpl: Asking to clear a DSV that does not store stencil.");
		}
	}
