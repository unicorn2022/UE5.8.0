...
{
	check(D3D12DoesShaderFrequencySupportUAV(ShaderStage));
...
D3D12_GPU_DESCRIPTOR_HANDLE FD3D12DescriptorCache::BuildSRVTable(EShaderFrequency ShaderStage, const FD3D12RootSignature* RootSignature, FD3D12ShaderResourceViewCache& Cache, const SRVSlotMask& SlotsNeededMask, uint32 SlotsNeeded, uint32& HeapSlot)
{
	...
	for (uint32 SlotIndex = 0; SlotIndex < SlotsNeeded; SlotIndex++)
	{
		if (FD3D12ShaderResourceView* SRV = SRVs[SlotIndex])
		{
			...
			if (RayTracingScene)
			{
				RayTracingScene->UpdateResidency(Context, true);
			}
#endif
		}
		else
...
void FD3D12DescriptorCache::SetConstantBufferViews(EShaderFrequency ShaderStage, const FD3D12RootSignature* RootSignature, FD3D12ConstantBufferCache& Cache, CBVSlotMask SlotsNeededMask, uint32 SlotsNeeded, uint32& HeapSlot)
{
	...
	if (ShaderStage == SF_Compute)
	{
		...
		{
			UE_LOGF(LogD3D12RHI, Error, "SetComputeRootDescriptorTable call exceed max 64 slots: %d ", RDTIndex);
		}
	}
	else
	{
		...
		{
			UE_LOGF(LogD3D12RHI, Error, "SetGraphicsRootDescriptorTable call exceed max 64 slots: %d ", RDTIndex);
		}
	}
...
void FD3D12DescriptorCache::SetRootConstantBuffers(EShaderFrequency ShaderStage, const FD3D12RootSignature* RootSignature, FD3D12ConstantBufferCache& Cache, CBVSlotMask SlotsNeededMask, FD3D12StateCache* StateCache)
{
	...
	for (uint32 SlotIndex = 0; SlotIndex < RDCBVsNeeded; SlotIndex++)
	{
		...
		if (CurrentGPUVirtualAddress == 0)
		{
			...
			if (Shader == nullptr)
			{
				UE_LOGF(LogD3D12RHI, Fatal, "Missing uniform buffer at slot %u, stage %ls and no shader is bound.", SlotIndex, GetShaderFrequencyString(ShaderStage));
			...
			if (ExpectedLayoutHash != 0)
			{
				...
				UE_LOGF(LogD3D12RHI, Fatal, "Missing uniform buffer at slot %u, stage %ls. Please check the high level drawing code. Hashes: %ls.", SlotIndex, GetShaderFrequencyString(ShaderStage), *ShaderHashList);
				return;
			}
		}
		...
		{
			UE_LOGF(LogD3D12RHI, Error, "%ls call exceed max 64 slots: %d ", (ShaderStage == SF_Compute) ? TEXT("SetComputeRootConstantBufferView") : TEXT("SetGraphicsRootConstantBufferView"), BaseIndex + SlotIndex);
		}
...
bool FD3D12DescriptorCache::SwitchToContextLocalViewHeap()
{
	...
	{
		UE_LOGF(LogD3D12RHI, Log, "This should only happen in the Editor where it doesn't matter as much. If it happens in game you should increase the device global heap size!");
...
void FD3D12OnlineHeap::CleanupResources()
{
	Heap.SafeRelease();
}
...
bool FD3D12GlobalOnlineSamplerHeap::RollOver()
{
	...
	check(false);
	UE_LOGF(LogD3D12RHI, Fatal, "Global Descriptor heaps can't roll over!");
	return false;
}
...
bool FD3D12SubAllocatedOnlineHeap::AllocateBlock()
{
	...
	else
	{
		...
		UE_LOGF(LogD3D12RHI, Warning, "Descriptor cache ran out of sub allocated descriptor blocks! Moving to Context local View heap strategy");
...
bool FD3D12LocalOnlineHeap::RollOver()
{
	...
	{
		UE_LOGF(LogD3D12RHI, Log, "OnlineHeap RollOver Detected. Increase the heap size to prevent creation of additional heaps");
