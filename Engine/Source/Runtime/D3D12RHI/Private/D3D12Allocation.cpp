...
static int32 GD3D12UploadHeapBigBlockPoolSize = 32 * 1024 * 1024; //On a game with Nanite disabled this value could be lowered
...
static int32 GD3D12UploadHeapPoolAllocationStrategy = 0;
#else
static int32 GD3D12UploadHeapPoolAllocationStrategy = 1;
#endif

static FAutoConsoleVariableRef CVarD3D12UploadHeapPoolAllocationStrategy(
	TEXT("d3d12.UploadHeap.PoolAllocationStrategy"),
	GD3D12UploadHeapPoolAllocationStrategy,
	TEXT("Pool allocation strategy")
	TEXT("0 - Immediate: a new pool will be allocated on the spot when needed")
	TEXT("1 - Async: the system will try to pre allocate overflow pools in advance"),
	ECVF_ReadOnly);

static int32 GD3D12UploadHeapOverFlowPoolsCount = 1;
static FAutoConsoleVariableRef CVarD3D12UploadHeapOverFlowPoolsCount(
	TEXT("d3d12.UploadHeap.OverFlowPoolsCount"),
	GD3D12UploadHeapOverFlowPoolsCount,
	TEXT("Max OverFlow Pools count to pre-allocate"),
	ECVF_ReadOnly);

#if D3D12RHI_PLATFORM_HAS_UNIFIED_MEMORY
static int32 GD3D12DefaultBufferPoolAllocationStrategy = 0;
#else
static int32 GD3D12DefaultBufferPoolAllocationStrategy = 1;
#endif

static FAutoConsoleVariableRef CVarD3D12DefaultBufferPoolAllocationStrategy(
	TEXT("d3d12.DefaultBuffer.PoolAllocationStrategy"),
	GD3D12DefaultBufferPoolAllocationStrategy,
	TEXT("Pool allocation strategy")
	TEXT("0 - Immediate: a new pool will be allocated on the spot when needed")
	TEXT("1 - Async: the system will try to pre allocate overflow pools in advance"),
	ECVF_ReadOnly);

static int32 GD3D12DefaultBufferOverFlowPoolsCount = 1;
static FAutoConsoleVariableRef CVarD3D12DefaultBufferPoolOverFlowPoolsCount(
	TEXT("d3d12.DefaultBuffer.OverFlowPoolsCount"),
	GD3D12DefaultBufferOverFlowPoolsCount,
	TEXT("Max OverFlow Pools count to pre-allocate"),
	ECVF_ReadOnly);
...
static ERHIPoolAllocationStrategy GetPoolAllocationStrategy(uint32 AllocationStrategyIndex)
{
	// 0 = ERHIPoolAllocationStrategy::Immediate
	// 1 = ERHIPoolAllocationStrategy::Async
	return (AllocationStrategyIndex == 0) ? ERHIPoolAllocationStrategy::Immediate : ERHIPoolAllocationStrategy::Async;
}
...
void FD3D12BuddyAllocator::Initialize()
{
	...
	if (AllocationStrategy == EResourceAllocationStrategy::kPlacedResource)
	{
		...
		CA_SUPPRESS(6286);
		const bool bShouldTrackResource = (GNumExplicitGPUsForRendering == 1) || IsGPUOnly(HeapProps.Type);

#if ENABLE_RESIDENCY_MANAGEMENT
		if (!GD3D12StartResourceResident && bShouldTrackResource)
		{
			Desc.Flags |= D3D12_HEAP_FLAG_CREATE_NOT_RESIDENT;
		}
#endif
...
uint32 FD3D12BuddyAllocator::AllocateBlock(uint32 order)
{
	...
	if (order > MaxOrder)
	{
		...
		UE_LOGF(LogD3D12RHI, Fatal, "Buddy Allocator cant allocate a block that large (order %d)", order);
	}
...
FD3D12MultiBuddyAllocator::FD3D12MultiBuddyAllocator(FD3D12Device* ParentDevice,
	...
	, PoolAllocationStrategy(InPoolAllocationStrategy)
	, OverFlowPoolsCount(InOverFlowPoolsCount)
{
#if UE_MEMORY_TRACE_ENABLED
...
{
	if (PoolAllocationStrategy == ERHIPoolAllocationStrategy::Async)
	{
		CS.Lock();

		// No Pools, needs initialization
		if (Allocators.IsEmpty())
		{
			Initialize();
		}

		// Iterate over Pool and find one where we can allocate from 
		...
		{
			// Allocation failed, grab a ready OverFlow pool and try allocating from it
			if (OverFlowAllocators.Num())
			{
				Allocators.Add(OverFlowAllocators.Pop());
				AllocationSuccess = Allocators.Last()->TryAllocate(SizeInBytes, Alignment, ResourceLocation);
			}

			// Allocation failed and either no OverFlow pool is ready or allocation failed from it as well, we need to allocate a new Pool immediately
			// we want to avoid this case as much as possible, consider increasing OverFlowPoolsCount if this happens
			if (!AllocationSuccess)
			{
		...
		// Check if we needs to start a new async Pool Allocation if not in progress
		if ((OverFlowAllocators.Num() < OverFlowPoolsCount) && (PoolAllocationStatus == ERHIPoolAllocationStatus::Idle))
		{
			PoolAllocationStatus = ERHIPoolAllocationStatus::Allocating;

			CS.Unlock();

			AsyncTask(ENamedThreads::AnyBackgroundHiPriTask, [this]()
			{
				NewAllocatorAsyncAllocate();
			});
		}
		else
		{
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FD3D12MultiBuddyAllocator::CreateNewAllocator);
...
void FD3D12MultiBuddyAllocator::NewAllocatorAsyncAllocate()
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FD3D12MultiBuddyAllocator::NewAllocatorAsyncAllocate);

	FD3D12BuddyAllocator* NewBuddyAllocator = CreateNewAllocator(MaximumAllocationSizeForPooling);

	{
		FScopeLock Lock(&CS);

		check(PoolAllocationStrategy == ERHIPoolAllocationStrategy::Async);
		
		if (PoolAllocationStatus == ERHIPoolAllocationStatus::Allocating)
		{
			OverFlowAllocators.Add(NewBuddyAllocator);
		}
		else
		{
			delete NewBuddyAllocator;
		}

		PoolAllocationStatus = ERHIPoolAllocationStatus::Idle;
	}
}
...
void FD3D12MultiBuddyAllocator::Initialize()
{
	if (PoolAllocationStrategy == ERHIPoolAllocationStrategy::Async)
	{
		Allocators.Add(CreateNewAllocator(MaximumAllocationSizeForPooling));

		check(OverFlowPoolsCount > 0);
...
void FD3D12MultiBuddyAllocator::ReleaseAllResources()
{
	...
	{
		FScopeLock Lock(&CS);
		PoolAllocationStatus = ERHIPoolAllocationStatus::ShutDown;
	}

	for (int32 c = 0; c < OverFlowAllocators.Num(); c++)
	{
		if (OverFlowAllocators[c] != nullptr)
		{
			OverFlowAllocators[c]->Destroy();
			delete(OverFlowAllocators[c]);
		}
	}
...
FD3D12UploadHeapAllocator::FD3D12UploadHeapAllocator(FD3D12Adapter* InParent, FD3D12Device* InParentDevice, const FString& InName)
	...
	, SmallBlockAllocator(
		...
		, TraceHeapId
		, GetPoolAllocationStrategy(GD3D12UploadHeapPoolAllocationStrategy)
		, GD3D12UploadHeapOverFlowPoolsCount)
...
{
	SmallBlockAllocator.Initialize();
	BigBlockAllocator.Initialize(ERHIPoolResourceTypes::Buffers);
}
...
FD3D12BufferPool* FD3D12DefaultBufferAllocator::CreateBufferPool(D3D12_HEAP_TYPE InHeapType, D3D12_RESOURCE_FLAGS InResourceFlags, EBufferUsageFlags InBufferUsage, ED3D12ResourceStateMode InResourceStateMode, uint32 Alignment)
{
	...
	ERHIPoolAllocationStrategy PoolAllocationStrategy = GetPoolAllocationStrategy(GD3D12DefaultBufferPoolAllocationStrategy);
	uint16 OverFlowPoolCount = GD3D12DefaultBufferOverFlowPoolsCount;
	...
	if (EnumHasAnyFlags(InitConfig.InitialD3D12Access, ED3D12Access::BVHRead | ED3D12Access::BVHWrite))
	{
		...
		PoolAllocationStrategy = ERHIPoolAllocationStrategy::Immediate;
		OverFlowPoolCount = 0;
	}
#endif // D3D12_RHI_RAYTRACING

	FD3D12BufferPool* NewPool = new FD3D12PoolAllocator(Device, VisibleNodes, InitConfig, Name, AllocationStrategy, PoolSize, PoolAlignment, MaxAllocationSize, FreeListOrder, bDefragEnabled, TraceHeapId, PoolAllocationStrategy, OverFlowPoolCount);
	...
	FD3D12MultiBuddyAllocator* Allocator = new FD3D12MultiBuddyAllocator(Device,
		...
		MinBlockSize,
		TraceHeapId,
		ERHIPoolAllocationStrategy::Immediate,
		0
		);
...
{	
...

	{
		...
		FD3D12PoolAllocator* ReadOnly4KPool = new FD3D12PoolAllocator(Device, GetVisibilityMask(), InitConfig, Name, AllocationStrategy, PoolSize, D3D12_SMALL_RESOURCE_PLACEMENT_ALIGNMENT, MaxAllocationSize, FreeListOrder, bDefragEnabled, TraceHeapId, ERHIPoolAllocationStrategy::Immediate, 0);
		ReadOnly4KPool->Initialize(ERHIPoolResourceTypes::NonRTDSTextures);
...

	{
		...
		FD3D12PoolAllocator* ReadOnlyPool = new FD3D12PoolAllocator(Device, GetVisibilityMask(), InitConfig, Name, AllocationStrategy, PoolSize, D3D12_DEFAULT_RESOURCE_PLACEMENT_ALIGNMENT, MaxAllocationSize, FreeListOrder, bDefragEnabled, TraceHeapId, ERHIPoolAllocationStrategy::Immediate, 0);
		ReadOnlyPool->Initialize(ERHIPoolResourceTypes::NonRTDSTextures);
...

	{
		...
		FD3D12PoolAllocator* RTPool = new FD3D12PoolAllocator(Device, GetVisibilityMask(), InitConfig, Name, AllocationStrategy, PoolSize, D3D12_DEFAULT_RESOURCE_PLACEMENT_ALIGNMENT, MaxAllocationSize, FreeListOrder, bDefragEnabled, TraceHeapId, ERHIPoolAllocationStrategy::Immediate, 0);
		RTPool->Initialize(ERHIPoolResourceTypes::NonRTDSTextures);
...

	{
		...
		bool bDefragEnabled = false;
		FD3D12PoolAllocator* UAVPool = new FD3D12PoolAllocator(Device, GetVisibilityMask(), InitConfig, Name, AllocationStrategy, PoolSize, D3D12_DEFAULT_RESOURCE_PLACEMENT_ALIGNMENT, MaxAllocationSize, FreeListOrder, bDefragEnabled, TraceHeapId, ERHIPoolAllocationStrategy::Immediate, 0);
		UAVPool->Initialize(ERHIPoolResourceTypes::NonRTDSTextures);
