...
TRefCountPtr<FD3D12Resource> FD3D12DynamicRHI::GetStagingTexture(FRHITexture* TextureRHI, FIntRect InRect, FIntRect& StagingRectOUT, FReadSurfaceDataFlags InFlags, D3D12_PLACED_SUBRESOURCE_FOOTPRINT& ReadbackHeapDesc, uint32 GPUIndex)
{
	...
	else
	{
		...
		const bool bSubrect = (Rect.left != 0 || Rect.top != 0 || Rect.right != SourceDesc.Width || Rect.bottom != SourceDesc.Height);
		...
		RHICmdList.Transition(FRHITransitionInfo(TextureRHI, ERHIAccess::Unknown, ERHIAccess::CopySrc, EResourceTransitionFlags::IgnoreAfterState));

		RHICmdList.EnqueueLambda(
		[Texture, GPUIndex, Rect, bSubrect, DestCopyLocation = MoveTemp(DestCopyLocation), SourceCopyLocation = MoveTemp(SourceCopyLocation), TempTexture2D = CopyTemp(TempTexture2D)](FRHICommandList& ExecutingCmdList)
		{
			...
			const D3D12_BOX* RectPtr = nullptr; // API prefers NULL for entire texture.
			if (bSubrect)
			{
			...
			Context.CopyTextureRegionChecked(
				...
				RectPtr,
				Texture->GetFormat(),
				TempTexture2D->GetName(),		// DestName
				Texture->GetName()				// SourceName
			);

			Context.UpdateResidency(Texture->GetResource());
			Context.UpdateResidency(TempTexture2D);
		});
...
{	
	...
	{
		RHICmdList.EnqueueLambda(
		[Texture, GPUIndex, SampleIndex, InRect, Rect, NonMSAADesc, NonMSAATexture2D, NonMSAARTV, StagingTexture2D, DestCopyLocation = MoveTemp(DestCopyLocation), SourceCopyLocation = MoveTemp(SourceCopyLocation)](FRHICommandListBase& ExecutingCmdList) mutable
		{
			FD3D12CommandContext& Context = FD3D12CommandContext::Get(ExecutingCmdList, GPUIndex);

			// Resolve the sample to the non-MSAA render target.
			Context.ResolveTextureUsingShader<FResolveSingleSamplePS>(
				ResourceCast(Texture->GetTexture2D()),
				NULL,
				&NonMSAARTV,
				NULL,
				NonMSAADesc,
				FResolveRect(InRect.Min.X, InRect.Min.Y, InRect.Max.X, InRect.Max.Y),
				FResolveRect(0, 0, InRect.Width(), InRect.Height()),
				SampleIndex
				);
			...
			Context.CopyTextureRegionChecked(
				...
				&Rect,
				Texture->GetFormat(),
				StagingTexture2D->GetName(),		// DestName
				Texture->GetName()					// SourceName
			);
...
void FD3D12DynamicRHI::RHIReadSurfaceFloatData(FRHITexture* TextureRHI, FIntRect InRect, TArray<FFloat16Color>& OutData, FReadSurfaceDataFlags InFlags)
{
	...
	FRHICommandListImmediate& RHICmdList = FRHICommandListImmediate::Get();
	RHICmdList.EnqueueLambda(
	[Texture, GPUIndex, Rect, DestCopyLocation = MoveTemp(DestCopyLocation), SourceCopyLocation = MoveTemp(SourceCopyLocation), TempTexture2D = CopyTemp(TempTexture2D)](FRHICommandListBase& ExecutingCmdList)
	{
		FD3D12CommandContext& Context = FD3D12CommandContext::Get(ExecutingCmdList, GPUIndex);
...
void FD3D12DynamicRHI::RHIRead3DSurfaceFloatData(FRHITexture* TextureRHI, FIntRect InRect, FIntPoint ZMinMax, TArray<FFloat16Color>& OutData, FReadSurfaceDataFlags InFlags)
{
	...
	FRHICommandListImmediate& RHICmdList = FRHICommandListImmediate::Get();
	RHICmdList.EnqueueLambda(
	[Texture, GPUIndex, Rect, DestCopyLocation = MoveTemp(DestCopyLocation), SourceCopyLocation = MoveTemp(SourceCopyLocation), TempTexture3D = CopyTemp(TempTexture3D)](FRHICommandListBase& ExecutingCmdList)
	{
		FD3D12CommandContext& Context = FD3D12CommandContext::Get(ExecutingCmdList, GPUIndex);
		...
		Context.CopyTextureRegionChecked(
			...
			&Rect,
			Texture->GetFormat(),
			TempTexture3D->GetName(),		// DestName
			Texture->GetName()				// SourceName
		);

		Context.UpdateResidency(Texture->GetResource());
		Context.UpdateResidency(TempTexture3D);
	});
