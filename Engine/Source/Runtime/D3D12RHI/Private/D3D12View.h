...
public:
	...
	FD3D12ConstantBufferView(FD3D12Device* InParent, FD3D12ConstantBufferView* FirstLinkedObject, ERHIDescriptorType InDescriptorType = ERHIDescriptorType::Invalid);
	void CreateView(const FResourceInfo& InResource, uint32 InOffset, uint32 InAlignedSize);
	void UpdateView(const FD3D12ContextArray& Contexts, const FResourceInfo& InResource, uint32 InOffset, uint32 InAlignedSize);
