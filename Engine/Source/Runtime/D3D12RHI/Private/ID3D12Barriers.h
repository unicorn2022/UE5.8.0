...
inline int32 GD3D12LogBarriers = 0;
inline FAutoConsoleVariableRef CVarD3D12LogBarriers(
	TEXT("d3d12.LogBarriers"),
	GD3D12LogBarriers,
	TEXT("Whether to log barriers. Use d3d12.LogBarriersResourceFilter to filter by resource name.\n")
	TEXT("0 - No logging\n")
	TEXT("1 - Log when batched\n")
	TEXT("2 - Log when flushed\n")
	TEXT("3 - Log when batched and when flushed"));

enum ED3D12LogBarriersFlags : decltype(GD3D12LogBarriers)
{
	D3D12LogBarriersNever       = 0,
	D3D12LogBarriersWhenBatched = 1u << 0,
	D3D12LogBarriersWhenFlushed = 1u << 1,
};

// Do not access the CVar directly as that will create a race.
// Instead use this variable for which updates are sync'd via the callback
inline FString GD3D12LogBarriersResourceFilter = "";
inline FAutoConsoleVariable CVarD3D12LogBarriersResourceFilter(
	TEXT("d3d12.LogBarriersResourceFilter"),
	TEXT(""),
	TEXT("Which resources to log barriers for. Set to empty to log everything."),
	FConsoleVariableDelegate::CreateStatic(
		[](IConsoleVariable* CVarFilter)
		{
			GD3D12LogBarriersResourceFilter = CVarFilter->GetString();
		}));
