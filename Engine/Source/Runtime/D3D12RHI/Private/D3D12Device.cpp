...
void FD3D12Device::CleanupResources()
{
	...
	GlobalSamplerHeap.CleanupResources();
...
void FD3D12Device::BlockUntilIdle()
{
	...
	{
		FD3D12SyncPointRef SyncPoint = FD3D12SyncPoint::Create(ED3D12SyncPointType::GPUAndCPU, UE_D3D12_STATIC_FNAME("BlockUntilGPUIdle"));
...
D3D12_RESOURCE_ALLOCATION_INFO FD3D12Device::GetResourceAllocationInfoUncached(const FD3D12ResourceDesc& InDesc)
{
...
#endif
	{
		...
		if (Result.SizeInBytes == UINT64_MAX)
		{
			...
			UE_LOGF(LogD3D12RHI, Error, "D3D12 GetResourceAllocationInfo failed - likely a resource was requested that has invalid allocation info (e.g. is an invalid texture size)");
			UE_LOGF(LogD3D12RHI, Error, "     W %llu H %d depth %d mips %d pf %d", InDesc.Width, InDesc.Height, InDesc.DepthOrArraySize, InDesc.MipLevels, InDesc.PixelFormat);
		}
	}
	return Result;
}
