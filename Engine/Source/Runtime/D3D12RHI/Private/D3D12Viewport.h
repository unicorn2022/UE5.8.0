...
public:
	...
	/** Schedule Present from the RHI Thread to the Submission Thread
	* Returns true if Present was correctly setup.
	*/
	bool SchedulePresent(class FD3D12CommandContextBase* BaseContext, const FRHIPresentArgs& InPresentArgs);
	
	/** Presents the swap chain on the Submission Thread
	*/
	void PresentOnSubmissionThread(FD3D12PresentArgs& PresentArgs);

	/** Wait for the last Present to be called from the Submission Thread
	*/
	void WaitForLastPresent() const;

	/** Signal that the Presents on the device has been completed and the backbuffer index has been update
	*/
	 ...
	 */
	bool EndRendering(class FD3D12CommandContextBase& Context, const FRHIPresentArgs& InPresentArgs);
	...
	FD3D12Texture* GetBackBuffer_RHIThread(bool bWaitForLastPresent = true) const;

	FD3D12Texture* GetSDRBackBuffer_RHIThread(bool bWaitForLastPresent = true) const
	{
	...
	void AdvanceExpectedBackBufferIndex_RenderThread(bool bPresent);

	void OnResumeRendering();
	void OnSuspendRendering();

	bool PresentedLastFrame(){ return bLastFramePresented; }

private:
	bool IsPresentAllowed();
	bool CheckSwapChain();
	void SyncToSubmissionThread(FD3D12CommandContextBase& ContextBase);
	...
	void EnsureColorSpace(DXGI_COLOR_SPACE_TYPE NewColorSpace);
