...
void UE::D3D12Descriptors::CopyDescriptor(FD3D12Device* Device, FD3D12DescriptorHeap* TargetHeap, FD3D12DescriptorIndex DstIndex, D3D12_CPU_DESCRIPTOR_HANDLE SrcCpuHandle)
{
	const D3D12_CPU_DESCRIPTOR_HANDLE DestCpuHandle = TargetHeap->GetCPUSlotHandle(DstIndex);
...
void UE::D3D12Descriptors::CopyDescriptor(FD3D12Device* Device, FD3D12DescriptorHeap* TargetHeap, FRHIDescriptorHandle DstHandle, D3D12_CPU_DESCRIPTOR_HANDLE SrcCpuHandle)
{
	...
	{
		checkSlow(Translate(TargetHeap->GetType()) == Translate(DstHandle.GetType()));
		CopyDescriptor(Device, TargetHeap, DstHandle.GetIndex(), SrcCpuHandle);
	}
}
...
void UE::D3D12Descriptors::CopyDescriptors(FD3D12Device* Device, FD3D12DescriptorHeap* TargetHeap, FD3D12DescriptorHeap* SourceHeap, TConstArrayView<FD3D12DescriptorIndex> Indices)
{
	// Use regular CPU Profile without dynamic value - almost same overhead for string formatting as actual Copy
	TRACE_CPUPROFILER_EVENT_SCOPE_STR("CopyDescriptors HeapToHeap Scattered");
	//SCOPED_NAMED_EVENT_F(TEXT("CopyDescriptors HeapToHeap Scattered (%d)"), FColor::Turquoise, Indices.Num());

	const int32 NumDescriptors = Indices.Num();
	const D3D12_DESCRIPTOR_HEAP_TYPE D3DHeapType = Translate(TargetHeap->GetType());
	...
	{
		DstDescriptors[Index] = TargetHeap->GetCPUSlotHandle(Indices[Index]);
		SrcDescriptors[Index] = SourceHeap->GetCPUSlotHandle(Indices[Index]);
		CopyCounts[Index] = 1;
	}
...
FD3D12DescriptorHeap::FD3D12DescriptorHeap(FD3D12DescriptorManager* InManager, uint32 InOffset, uint32 InNumDescriptors)
	: FD3D12DescriptorHeap(InManager->GetHeap(), InOffset, InNumDescriptors)
{
	Manager = InManager;
}
...
FD3D12DescriptorHeap* FD3D12DescriptorManager::AllocateSubheap(uint32 InNumDescriptors)
{
	if (TOptional<FRHIDescriptorAllocation> Allocation = Allocate(InNumDescriptors))
	{
		return new FD3D12DescriptorHeap(this, Allocation->StartIndex, Allocation->Count);
	}

	return nullptr;
}

void FD3D12DescriptorManager::FreeSubheap(FD3D12DescriptorHeap* InHeap)
{
	Free(FRHIDescriptorAllocation(InHeap->GetOffset(), InHeap->GetNumDescriptors()));
}
...
{
	for (const FD3D12DescriptorHeapPtr& Heap : Heaps)
	{
		checkf(!Heap, TEXT("FD3D12OnlineDescriptorManager destroyed without CleanupResources being called."));
	}
}
...
{
	for (FD3D12DescriptorHeapPtr& Heap : Heaps)
	{
		Heap.SafeRelease();
	}
}
...
FD3D12DescriptorHeap* FD3D12DescriptorHeapManager::AllocateHeap(const TCHAR* InDebugName, ERHIDescriptorHeapType InHeapType, uint32 InNumDescriptors, ED3D12DescriptorHeapFlags InHeapFlags)
{
	for (FD3D12DescriptorManager& GlobalHeap : GlobalHeaps)
	{
		...
		{
			if (FD3D12DescriptorHeap* NewHeap = GlobalHeap.AllocateSubheap(InNumDescriptors))
			{
				return NewHeap;
			}
