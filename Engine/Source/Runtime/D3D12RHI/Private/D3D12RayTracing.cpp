...
// This is required to avoid redundant code static analysis warnings
...
DECLARE_DWORD_ACCUMULATOR_STAT(TEXT("Procedural primitives in all BL acceleration structures"), STAT_D3D12RayTracingProceduralPrimitivesBLAS, STATGROUP_D3D12RayTracing);
DECLARE_DWORD_ACCUMULATOR_STAT(TEXT("LinearSweptSpheres in all BL acceleration structures"), STAT_D3D12RayTracingLinearSweptSpheresBLAS, STATGROUP_D3D12RayTracing);
...
#if WITH_NVAPI
DEFINE_ENUM_FLAG_OPERATORS(NVAPI_D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAGS_EX);
#endif
...
static void DumpRayTracingGeometries(EDumpRayTracingGeometryMode Mode, int32 NumEntriesToShow, const FString& NameFilter, bool bCSV, FBufferedOutputDevice& BufferedOutput)
{
	...
	FName CategoryName("D3D12RayTracing");
...
static DXGI_FORMAT TranslateRayTracingVertexElementType(EVertexElementType Type)
{
	switch (Type)
	{
		case VET_Float4:
...
static D3D12_RAYTRACING_GEOMETRY_FLAGS TranslateSegmentToGeometryFlags(const FRayTracingGeometrySegment& Segment)
{
...
)
{
	...
	if (FAILED(hr))
	{
		...
		UE_LOGF(LogD3D12RHI, Warning, "Failed to create raytracing RTPSO - add '-d3ddebug' to the command line to get more info on RTPSO compilation errors from the debug device");
...
void FD3D12RayTracingCompactionRequestHandler::RequestCompact(FD3D12RayTracingGeometry* InRTGeometry)
{
	...
	check(InRTGeometry->PerGPUData[GPUIndex].AccelerationStructureBuffer);
...
void FD3D12RayTracingCompactionRequestHandler::Update(FD3D12CommandContext& Context)
{
	...
	for (FD3D12RayTracingGeometry* RTGeometry : PendingRequests)
	{
		...
		FD3D12ResourceLocation& ResourceLocation = RTGeometry->PerGPUData[GPUIndex].AccelerationStructureBuffer.GetReference()->ResourceLocation;
	...
	if (ActiveRequests.Num() > 0)
	{
		...
		Context.UpdateResidency(PostBuildInfoBuffer->GetResource());
...
public:
	...
	struct FEntry
	{
		...
		D3D12_EXISTING_COLLECTION_DESC GetCollectionDesc()
		{
			...
			{
				UE_LOGF(LogD3D12RHI, Fatal, "Trying to add a raytracing state collection which failed to compile to a RTPSO collection.");
			}
	...
	public:
		...
		FShaderCompileTask(
			...
			, CacheKey(InCacheKey)
			, Device(InDevice)
			, RayTracingDevice(InDevice->GetParentAdapter()->GetD3DDevice5())
		...
		void DoTask(ENamedThreads::Type CurrentThread, const FGraphEventRef& MyCompletionGraphEvent)
		{
			...
			{
				UE_LOGF(LogD3D12RHI, Fatal, "Failed to create a required RTPSO pipeline state collection");
			}
			...
			if (Entry.CompileTimeMS >= 1000.0f)
			{
				...
				UE_LOGF(LogD3D12RHI, Log, "Compiled %ls for RTPSO in %.2f ms.", OriginalEntryPoints[0], Entry.CompileTimeMS);
			}
		}
		...
		bool* bOutCacheHit = nullptr)
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogD3D12RHI, Fatal, "Failed to create a required RTPSO collection");
			}
		}
...
private:
	...
	void CompareData(uint32 Offset, const void* InData, uint32 InDataSize)
	{
		...
		if (FMemory::Memcmp(CurrentData, InBytes, InDataSize) != 0)
		{
			...
			UE_LOGF(LogD3D12RHI, Error, "CompareData mismatch at buffer offset %u + %u (absolute byte %u), comparing %u bytes. Dumping %d bytes from first diff:",
				Offset, FirstDiff, Offset + FirstDiff, InDataSize, BytesToDump);
			UE_LOGF(LogD3D12RHI, Error, "  Current:  %ls", *BytesToHex(CurrentData + FirstDiff, BytesToDump));
			UE_LOGF(LogD3D12RHI, Error, "  Expected: %ls", *BytesToHex(InBytes + FirstDiff, BytesToDump));
...
public:
	...
	D3D12_DISPATCH_RAYS_DESC GetDispatchRaysDesc(FD3D12Device* InDevice, const FD3D12ShaderIdentifier& RayGenShaderIdentifier, FD3D12Resource*& OutRayGenUploadResource) const
	{
		...
		OutRayGenUploadResource = UploadResourceLocation.GetResource();
	...
	struct FRecordUpdateUniformBufferListener : public ID3D12UniformBufferUpdateListener
	{
		...
		void Init(uint32 WorkerIndex, FD3D12UniformBuffer* InUniformBuffer, uint32 InShaderTableOffset, uint32 InRecordIndex, uint32 InOffsetWithinRootSignature)
		{
...
#if ENABLE_RESIDENCY_MANAGEMENT
			CurrentResource = UniformBuffer->ResourceLocation.GetResource();
			if (CurrentResource)
			{
		...
		virtual void RemoveListener(FD3D12UniformBuffer* InUpdatedUniformBuffer) final override
		{
			...
			// TODO: investigate if current resource be removed from reference resource list as well here?
		}

		virtual void UniformBufferUpdated(const FD3D12ContextArray& Contexts, FD3D12UniformBuffer* InUpdatedUniformBuffer) final override
		{
			check(UniformBuffer == InUpdatedUniformBuffer);
...
#if ENABLE_RESIDENCY_MANAGEMENT
			FD3D12Resource* NewResource = UniformBuffer->ResourceLocation.GetResource();
			...
			if (NewResource != CurrentResource)
			{
				...
				// Using 0 as consolidated worker index data which gets merged during Commit of the shader binding table
				if (CurrentResource)
				{
	...
	struct FRecordUpdateRayTracingGeometryListener : public ID3D12RayTracingGeometryUpdateListener
	{
		...
		virtual void HitGroupParametersUpdated(FD3D12RayTracingGeometry* InGeometry) final override
		{
			...
			FD3D12HitGroupSystemParameters SystemParameters = Geometry->PerGPUData[GPUIndex].HitGroupSystemParameters[SegmentIndex];
	...
	void SetHitGroupGeometrySystemParameters(uint32 WorkerIndex, ERayTracingLocalShaderBindingType BindingType, uint32 RecordIndex, const FD3D12RayTracingGeometry* Geometry, uint32 GeometrySegmentIndex, uint32 UserData)
	{	
		FD3D12HitGroupSystemParameters SystemParameters;

		if (Geometry != nullptr)
		{
			SystemParameters = Geometry->PerGPUData[Device->GetGPUIndex()].HitGroupSystemParameters[GeometrySegmentIndex];
		}
		else
		{
			FMemory::Memset(&SystemParameters, 0, sizeof(SystemParameters));
		}
		...
		D3D12_GPU_VIRTUAL_ADDRESS& OutGPUVirtualAddress)
	{
		...
		else
		{
			...
			if (Lifetime == ERayTracingShaderBindingTableLifetime::Persistent)
			{
				...
				#if ENABLE_RESIDENCY_MANAGEMENT
					UpdatePersistentReferencedResource(WorkerIndex, RecordData.LooseParameterResourceLocation->GetResource(), 1);
				#endif // ENABLE_RESIDENCY_MANAGEMENT
				...
				else if (RecordData.LooseParameterResourceLocation)
				{
					...
					FD3D12Resource* CurrentResource = RecordData.LooseParameterResourceLocation->GetResource();

					RecordData.LooseParameterResourceLocation->Clear();
					FD3D12ResourceLocation::TransferOwnership(*RecordData.LooseParameterResourceLocation, ResourceLocation);

				#if ENABLE_RESIDENCY_MANAGEMENT
					FD3D12Resource* NewResource = RecordData.LooseParameterResourceLocation->GetResource();
					if (CurrentResource != NewResource)
					{
						UpdatePersistentReferencedResource(WorkerIndex, CurrentResource, -1);
						UpdatePersistentReferencedResource(WorkerIndex, NewResource, 1);
					}
				#endif // ENABLE_RESIDENCY_MANAGEMENT
				}
			}
			...
			{			
				AddDynamicReferencedResource(WorkerIndex, ResourceLocation.GetResource());
			}
#endif // ENABLE_RESIDENCY_MANAGEMENT
		}
	...
	void RemoveUpdateUniformBufferListener(uint32 RecordIndex, FRecordUpdateUniformBufferListener* Listener)
	{
		...
		for (int32 Index = 0; Index < HitRecordData[RecordIndex].UpdateUniformBufferListeners.Num(); ++Index)
		{
			if (HitRecordData[RecordIndex].UpdateUniformBufferListeners[Index] == Listener)
			{
				...
				// Index is 0 since the work has been consolidated
				Listener->Reset(0);
				delete Listener;
	...
	struct FShaderRecordCacheKey
	{
		...
		FShaderRecordCacheKey(uint32 InNumUniformBuffers, FRHIUniformBuffer* const* InUniformBuffers, uint32 InShaderIndex)
		{
			...
			Hash = FShaderHash::HashBuffer(UniformBuffers, DataSizeInBytes).Hash;
		}
...
static void CreateSpecializedStateObjects(
	...
	const TMap<FShaderHash, int32>& RayGenShaderIndexByHash,
	...
	Device(Device)
{
	...
	ID3D12Device5* RayTracingDevice = Device->GetParentAdapter()->GetD3DDevice5();
		...
		(FD3D12RayTracingShader* Shader, FD3D12RayTracingPipelineCache::ECollectionType CollectionType)
	{
		...
		{
			UE_LOGF(LogD3D12RHI, Warning, "Raytracing shader with with entry point %ls, name %ls and ShaderBindingLayout hash %u doesn't match the RTPSO ShaderBindingLayout hash %u.",
	...
	if (BasePipeline && GRayTracingSpecializeStateObjects == 0)
	{
		...
		else
		{
			...
			ID3D12Device7* Device7 = Device->GetParentAdapter()->GetD3DDevice7();
	...
	else
	{
		...
		if (StateObject == nullptr)
		{
			UE_LOGF(LogD3D12RHI, Fatal, "Failed to a create raytracing pipeline state");
		}
	}
...
static bool GeometryUsesLinearSweptSpheres(const FRayTracingGeometryInitializer& Initializer)
{
	return Initializer.GeometryType == RTGT_LinearSweptSpheres;
}

static void TranslateRayTracingGeometryDescs(const FRayTracingGeometryInitializer& Initializer, TArrayView<D3D12_RAYTRACING_GEOMETRY_DESC> Output)
{
	check(Output.Num() == Initializer.Segments.Num());
	check(!GeometryUsesLinearSweptSpheres(Initializer));
	...
	for (int32 SegmentIndex = 0; SegmentIndex < Initializer.Segments.Num(); ++SegmentIndex)
	{
		...
		D3D12_RAYTRACING_GEOMETRY_DESC& Desc = Output[SegmentIndex];
		Desc = {}; // zero-initialize

		Desc.Flags = TranslateSegmentToGeometryFlags(Segment);
		...
		case RTGT_Triangles:
			...
			Desc.Triangles.VertexFormat = TranslateRayTracingVertexElementType(Segment.VertexBufferElementType);
			checkf(Desc.Triangles.VertexFormat != DXGI_FORMAT_UNKNOWN, TEXT("Unknown vertex format %d - not currently implemented"), int(Desc.Triangles.VertexFormat));

			if (Initializer.IndexBuffer)
			{
				uint32 IndexStride = Initializer.IndexBuffer->GetStride();
				check(Initializer.IndexBuffer->GetSize() >=
					  (Segment.FirstPrimitive + Segment.NumPrimitives) * FD3D12RayTracingGeometry::IndicesPerTriangle * IndexStride + Initializer.IndexBufferOffset);
				...
				Desc.Triangles.IndexCount = Segment.NumPrimitives * FD3D12RayTracingGeometry::IndicesPerTriangle;
...
#if WITH_NVAPI
static void TranslateRayTracingGeometryDescsEx(const FRayTracingGeometryInitializer& Initializer, TArrayView<NVAPI_D3D12_RAYTRACING_GEOMETRY_DESC_EX> Output)
{
	check(GeometryUsesLinearSweptSpheres(Initializer)); // This function is only for LSS primitives, otherwise use main D3D path
	check(Output.Num() == Initializer.Segments.Num());
	...
	for (int32 SegmentIndex = 0; SegmentIndex < Initializer.Segments.Num(); ++SegmentIndex)
	{
		...
		NVAPI_D3D12_RAYTRACING_GEOMETRY_DESC_EX& Desc = Output[SegmentIndex];
		Desc = {}; // zero-initialize

		Desc.flags = TranslateSegmentToGeometryFlags(Segment);

		// RTGT_LinearSweptSpheres (only case for now)
		{
			Desc.type = NVAPI_D3D12_RAYTRACING_GEOMETRY_TYPE_LSS_EX;

			checkf(Segment.VertexBufferStride >= sizeof(FVector4f), TEXT("LSS vertex buffer must contain at least float4 that defines control point."));

			uint32 IndexStride = Initializer.IndexBuffer->GetStride();
			check(Initializer.IndexBuffer->GetSize() >=
				(Segment.FirstPrimitive + Segment.NumPrimitives) * FD3D12RayTracingGeometry::IndicesPerLinearSweptSphere * IndexStride + Initializer.IndexBufferOffset);

			Desc.lss = {};
			Desc.lss.vertexCount = Segment.MaxVertices;
			Desc.lss.indexCount = Segment.NumPrimitives * FD3D12RayTracingGeometry::IndicesPerLinearSweptSphere;
			Desc.lss.primitiveCount = Segment.NumPrimitives;
			Desc.lss.vertexPositionBuffer.StartAddress = 0;
			Desc.lss.vertexPositionBuffer.StrideInBytes = 4 * sizeof(float);
			Desc.lss.vertexPositionFormat = DXGI_FORMAT_R32G32B32_FLOAT;
			Desc.lss.vertexRadiusBuffer.StartAddress = 3 * sizeof(float);
			Desc.lss.vertexRadiusBuffer.StrideInBytes = 4 * sizeof(float);
			Desc.lss.vertexRadiusFormat = DXGI_FORMAT_R32_FLOAT;
			Desc.lss.indexFormat = DXGI_FORMAT_R32_UINT;
			Desc.lss.indexBuffer.StartAddress = 0;
			Desc.lss.indexBuffer.StrideInBytes = sizeof(uint32);
			Desc.lss.endcapMode = NVAPI_D3D12_RAYTRACING_LSS_ENDCAP_MODE_NONE;
			Desc.lss.primitiveFormat = NVAPI_D3D12_RAYTRACING_LSS_PRIMITIVE_FORMAT_LIST;
		}
...
FRayTracingAccelerationStructureSize FD3D12DynamicRHI::RHICalcRayTracingGeometrySize(const FRayTracingGeometryInitializer& Initializer)
{
...
#if WITH_NVAPI
	TArray<NVAPI_D3D12_RAYTRACING_GEOMETRY_DESC_EX, TInlineAllocator<1>> GeometryDescsEx;
	if (GeometryUsesLinearSweptSpheres(Initializer))
	{
		GeometryDescsEx.SetNumUninitialized(Initializer.Segments.Num());
		TranslateRayTracingGeometryDescsEx(Initializer, GeometryDescsEx);
	}
	else
#endif
	{
	...
	for (DXGI_FORMAT IndexFormat : IndexFormats)
	{
...
#if WITH_NVAPI
		for (NVAPI_D3D12_RAYTRACING_GEOMETRY_DESC_EX& GeometryDescEx : GeometryDescsEx)
		{
			check(GeometryDescEx.type == NVAPI_D3D12_RAYTRACING_GEOMETRY_TYPE_LSS_EX);
			GeometryDescEx.lss.indexFormat = IndexFormat;
		}
#endif
		...
		for (uint32 GPUIndex = 0; GPUIndex < GNumExplicitGPUsForRendering; ++GPUIndex)
		{
			...
			if (GeometryUsesLinearSweptSpheres(Initializer))
			{
				NVAPI_D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_INPUTS_EX InputsEx = {};
				InputsEx.type = PrebuildDescInputs.Type;
				InputsEx.flags = static_cast<NVAPI_D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAGS_EX>(PrebuildDescInputs.Flags);
				InputsEx.descsLayout = PrebuildDescInputs.DescsLayout;
				InputsEx.geometryDescStrideInBytes = sizeof(NVAPI_D3D12_RAYTRACING_GEOMETRY_DESC_EX);
				InputsEx.instanceDescs = 0ULL;
				InputsEx.numDescs = GeometryDescsEx.Num();
				InputsEx.pGeometryDescs = GeometryDescsEx.GetData();

				NVAPI_GET_RAYTRACING_ACCELERATION_STRUCTURE_PREBUILD_INFO_EX_PARAMS PrebuildParams = {};
				PrebuildParams.version = NVAPI_GET_RAYTRACING_ACCELERATION_STRUCTURE_PREBUILD_INFO_EX_PARAMS_VER;
				PrebuildParams.pDesc = &InputsEx;
				PrebuildParams.pInfo = &PrebuildInfo;

				ID3D12Device5* RayTracingDevice = Adapter.GetD3DDevice5();
				NvAPI_Status status = NvAPI_D3D12_GetRaytracingAccelerationStructurePrebuildInfoEx(RayTracingDevice, &PrebuildParams);
...
static TRefCountPtr<FD3D12Buffer> CreateRayTracingBuffer(FD3D12Adapter* Adapter, uint32 GPUIndex, uint64 Size, ERayTracingBufferType Type, const FDebugName& DebugName)
{
	...
	else if (Type == ERayTracingBufferType::Scratch)
	{
		...
		Adapter->SetResidencyPriority(HeapResource, D3D12_RESIDENCY_PRIORITY_HIGH, GPUIndex, Size);
	}
	else
	{
		checkNoEntry();
	...
	: FRHIRayTracingGeometry(InInitializer), FD3D12AdapterChild(Adapter)
{
	...
	FOREACH_GPU(GPUIndex < (uint32)PerGPUData.Num(),
	{
		FPerGPUState& PerGPUState = PerGPUData[GPUIndex];

		PerGPUState.AccelerationStructureBuffer = CreateRayTracingBuffer(Adapter, GPUIndex, SizeInfo.ResultSize, ERayTracingBufferType::AccelerationStructure, DebugName);
		PerGPUState.AccelerationStructureBuffer->SetOwnerName(OwnerName);

		INC_MEMORY_STAT_BY(STAT_D3D12RayTracingUsedVideoMemory, PerGPUState.AccelerationStructureBuffer->GetSize());
		INC_MEMORY_STAT_BY(STAT_D3D12RayTracingBLASMemory, PerGPUState.AccelerationStructureBuffer->GetSize());
		if (Initializer.bAllowUpdate)
		{
			INC_MEMORY_STAT_BY(STAT_D3D12RayTracingDynamicBLASMemory, PerGPUState.AccelerationStructureBuffer->GetSize());
		}
		else
		{
			INC_MEMORY_STAT_BY(STAT_D3D12RayTracingStaticBLASMemory, PerGPUState.AccelerationStructureBuffer->GetSize());
		}
	});

	switch (Initializer.GeometryType)
	{
		case RTGT_Triangles:          INC_DWORD_STAT_BY(STAT_D3D12RayTracingTrianglesBLAS           , Initializer.TotalPrimitiveCount); break;
		case RTGT_Procedural:         INC_DWORD_STAT_BY(STAT_D3D12RayTracingProceduralPrimitivesBLAS, Initializer.TotalPrimitiveCount); break;
		case RTGT_LinearSweptSpheres: INC_DWORD_STAT_BY(STAT_D3D12RayTracingLinearSweptSpheresBLAS  , Initializer.TotalPrimitiveCount); break;
	}
	...
	if (SourceData != nullptr)
	{
		...
		RHICmdList.EnqueueLambda([this, SrcResourceLoc = MoveTemp(SrcResourceLoc), bForRendering](FRHICommandListBase& ExecutingCmdList)
		{
			FOREACH_GPU(GPUIndex < (uint32)PerGPUData.Num(),
			{
				...
				FD3D12Buffer* AccelerationStructure = PerGPUState.AccelerationStructureBuffer;
				...
				Context.UpdateResidency(AccelerationStructure->GetResource());
...
void FD3D12RayTracingGeometry::Swap(FD3D12RayTracingGeometry& Other)
{
	FOREACH_GPU(GPUIndex < (uint32)PerGPUData.Num(),
	{
		::Swap(PerGPUData[GPUIndex].AccelerationStructureBuffer, Other.PerGPUData[GPUIndex].AccelerationStructureBuffer);
		::Swap(PerGPUData[GPUIndex].bIsAccelerationStructureDirty, Other.PerGPUData[GPUIndex].bIsAccelerationStructureDirty);
	});
	...
	DebugName = !Initializer.DebugName.IsNone() ? Initializer.DebugName : FName("BLAS");
...
void FD3D12RayTracingGeometry::ReleaseUnderlyingResource()
{
	...
	{
		FPerGPUState& PerGPUState = PerGPUData[GPUIndex];
		if (PerGPUState.bHasPendingCompactionRequests)
		{
			check(PerGPUState.AccelerationStructureBuffer);
			FD3D12Device* Device = PerGPUState.AccelerationStructureBuffer.GetReference()->GetParentDevice();
			bool bRequestFound = Device->GetRayTracingCompactionRequestHandler()->ReleaseRequest(this);
			check(bRequestFound);
			PerGPUState.bHasPendingCompactionRequests = false;
		}
	});
	...
	{
		FPerGPUState& PerGPUState = PerGPUData[GPUIndex];
		PerGPUState.HitGroupSystemIndexBufferSRV.Reset();
		PerGPUState.HitGroupSystemSegmentVertexBufferSRVs.Empty();

		UnregisterAsRenameListener(GPUIndex);
	});

	for (FPerGPUState& GPUState : PerGPUData)
	{
		if (GPUState.AccelerationStructureBuffer)
		{
			DEC_MEMORY_STAT_BY(STAT_D3D12RayTracingUsedVideoMemory, GPUState.AccelerationStructureBuffer->GetSize());
			DEC_MEMORY_STAT_BY(STAT_D3D12RayTracingBLASMemory, GPUState.AccelerationStructureBuffer->GetSize());

			ERayTracingAccelerationStructureFlags BuildFlags = GetRayTracingAccelerationStructureBuildFlags(Initializer);
			if (EnumHasAllFlags(BuildFlags, ERayTracingAccelerationStructureFlags::AllowUpdate))
			{
				DEC_MEMORY_STAT_BY(STAT_D3D12RayTracingDynamicBLASMemory, GPUState.AccelerationStructureBuffer->GetSize());
			}
			else
			{
				DEC_MEMORY_STAT_BY(STAT_D3D12RayTracingStaticBLASMemory, GPUState.AccelerationStructureBuffer->GetSize());
			}
		}
	}
	...
	if (Initializer.Type != ERayTracingGeometryInitializerType::StreamingSource)
	{
		switch (Initializer.GeometryType)
		{
			case RTGT_Triangles:          DEC_DWORD_STAT_BY(STAT_D3D12RayTracingTrianglesBLAS           , Initializer.TotalPrimitiveCount); break;
			case RTGT_Procedural:         DEC_DWORD_STAT_BY(STAT_D3D12RayTracingProceduralPrimitivesBLAS, Initializer.TotalPrimitiveCount); break;
			case RTGT_LinearSweptSpheres: DEC_DWORD_STAT_BY(STAT_D3D12RayTracingLinearSweptSpheresBLAS  , Initializer.TotalPrimitiveCount); break;
		}
	...
	for (FPerGPUState& GPUState : PerGPUData)
	{
		GPUState.AccelerationStructureBuffer.SafeRelease();
		GPUState.HitGroupSystemParameters.Empty();
	}
...
bool FD3D12RayTracingGeometry::AllocateBufferSRVs(uint32 InGPUIndex)
{
	bool bAnyBuffersUpdated = false;

	FPerGPUState& PerGPUState = PerGPUData[InGPUIndex];

	// Procedural doesn't need any SRVs for index buffer
	if (Initializer.IndexBuffer && (Initializer.GeometryType == RTGT_Triangles || Initializer.GeometryType == RTGT_LinearSweptSpheres)
#if !DO_CHECK
		// if checks are not enabled then simply early out when HitGroupSystemIndexBufferSRV is already created because we don't support updating them
		&& PerGPUState.HitGroupSystemIndexBufferSRV == nullptr		
#endif	
		)
	{
		...
		const uint32 PerPrimitiveIndexCount = (Initializer.GeometryType == RTGT_Triangles) ? IndicesPerTriangle : IndicesPerLinearSweptSphere;
		...
		SRVDesc.Buffer.NumElements = FMath::Max((uint32)1, ((MaxPrimitiveCount * PerPrimitiveIndexCount * IndexBuffer->GetStride()) + 3) >> 2u);
		SRVDesc.Buffer.Flags = D3D12_BUFFER_SRV_FLAG_RAW;
		SRVDesc.Buffer.StructureByteStride = 0;

#if DO_CHECK
		if (PerGPUState.HitGroupSystemIndexBufferSRV)
		{
			// Validate that current index buffer view hasn't changed
			D3D12_SHADER_RESOURCE_VIEW_DESC CurrentSRVDesc = PerGPUState.HitGroupSystemIndexBufferSRV->GetD3DDesc();
			check(SRVDesc.Buffer.FirstElement == CurrentSRVDesc.Buffer.FirstElement &&
				SRVDesc.Buffer.NumElements == CurrentSRVDesc.Buffer.NumElements &&
				IndexBuffer == PerGPUState.HitGroupSystemIndexBufferSRV->GetShaderResource());
		}
		else
#endif // DO_CHECK
		{
			PerGPUState.HitGroupSystemIndexBufferSRV = MakeShared<FD3D12ShaderResourceView>(GetParentAdapter()->GetDevice(InGPUIndex), InGPUIndex > 0 ? PerGPUData[0].HitGroupSystemIndexBufferSRV.Get() : nullptr, ERHIDescriptorType::BufferSRV);
			PerGPUState.HitGroupSystemIndexBufferSRV->CreateView(IndexBuffer, SRVDesc, FD3D12ShaderResourceView::EFlags::None);
		
			bAnyBuffersUpdated = true;
		}
	}

	if (PerGPUState.HitGroupSystemSegmentVertexBufferSRVs.Num() != Initializer.Segments.Num())
	{		
		PerGPUState.HitGroupSystemSegmentVertexBufferSRVs.SetNum(Initializer.Segments.Num());
	}
	...
	for (int32 SegmentIndex = 0; SegmentIndex < Initializer.Segments.Num(); ++SegmentIndex)
	{
		...
		bool bCreateNewSRV = true;
		FD3D12ShaderResourceView* CurrentVertexBufferSRV = SegmentIndex < PerGPUState.HitGroupSystemSegmentVertexBufferSRVs.Num() ? PerGPUState.HitGroupSystemSegmentVertexBufferSRVs[SegmentIndex].Get() : nullptr;
		if (CurrentVertexBufferSRV)
		{
			D3D12_SHADER_RESOURCE_VIEW_DESC CurrentSRVDesc = CurrentVertexBufferSRV->GetD3DDesc();
			if (SRVDesc.Buffer.FirstElement == CurrentSRVDesc.Buffer.FirstElement &&
				SRVDesc.Buffer.NumElements == CurrentSRVDesc.Buffer.NumElements &&
				CurrentVertexBufferSRV->GetShaderResource() == VertexBuffer)
			{
				bCreateNewSRV = false;
			}
		}
		...
		if (bCreateNewSRV)
		{
			...
			if (InGPUIndex > 0)
			{
				if (PerGPUData[0].HitGroupSystemSegmentVertexBufferSRVs.Num() > SegmentIndex)
				{
					FirstLinkedObject = PerGPUData[0].HitGroupSystemSegmentVertexBufferSRVs[SegmentIndex].Get();
				}
			}
			...
			PerGPUState.HitGroupSystemSegmentVertexBufferSRVs[SegmentIndex] = VertexBufferSRV;

			bAnyBuffersUpdated = true;
		}
	}

	return bAnyBuffersUpdated;
}
...
void FD3D12RayTracingGeometry::RegisterAsRenameListener(uint32 InGPUIndex)
{
	...
	check(!PerGPUState.bRegisteredAsRenameListener);
	...
	PerGPUState.bRegisteredAsRenameListener = true;
}
...
void FD3D12RayTracingGeometry::UnregisterAsRenameListener(uint32 InGPUIndex)
{
	...
	if (!PerGPUState.bRegisteredAsRenameListener)
	{
		return;
	}
	...
	PerGPUState.bRegisteredAsRenameListener = false;
}
...
void FD3D12RayTracingGeometry::UpdateResidency(FD3D12CommandContext& CommandContext)
{
	...
	CommandContext.UpdateResidency(PerGPUData[GPUIndex].AccelerationStructureBuffer->GetResource());
}
...
void FD3D12RayTracingGeometry::SetupHitGroupSystemParameters(uint32 InGPUIndex)
{
	...
	bool bAnyBuffersUpdated = true;
	check(BuffersValid(InGPUIndex));
	if (bBindless)
	{
		bAnyBuffersUpdated = AllocateBufferSRVs(InGPUIndex);
	}

	if (bAnyBuffersUpdated)
	{
		FPerGPUState& PerGPUState = PerGPUData[InGPUIndex];

		TArray<FD3D12HitGroupSystemParameters>& HitGroupSystemParametersForThisGPU = PerGPUState.HitGroupSystemParameters;
		...
		const uint32 LocalIndicesPerPrimitive = Initializer.GeometryType == RTGT_LinearSweptSpheres ? IndicesPerLinearSweptSphere : IndicesPerTriangle;
		for (int32 SegmentIndex = 0; SegmentIndex < Initializer.Segments.Num(); ++SegmentIndex)
		{
			const FRayTracingGeometrySegment& Segment = Initializer.Segments[SegmentIndex];
			FD3D12Buffer* VertexBuffer = FD3D12DynamicRHI::ResourceCast(Segment.VertexBuffer.GetReference(), InGPUIndex);
			...
			if (bBindless)
			{
				SystemParameters.BindlessHitGroupSystemVertexBuffer = PerGPUState.HitGroupSystemSegmentVertexBufferSRVs[SegmentIndex]->GetBindlessHandle().GetIndex();
			}
			else
			{
			...
			if (IndexBuffer != nullptr)
			{
				if (bBindless)
				{
					SystemParameters.BindlessHitGroupSystemIndexBuffer = PerGPUState.HitGroupSystemIndexBufferSRV->GetBindlessHandle().GetIndex();
				}
				else
				{
					SystemParameters.IndexBuffer = IndexBuffer->ResourceLocation.GetGPUVirtualAddress();
				}
				SystemParameters.RootConstants.IndexBufferOffsetInBytes = Initializer.IndexBufferOffset + IndexStride * Segment.FirstPrimitive * LocalIndicesPerPrimitive;
...
void FD3D12RayTracingGeometry::CreateAccelerationStructureBuildDesc(FD3D12CommandContext& CommandContext, EAccelerationStructureBuildMode BuildMode, D3D12_GPU_VIRTUAL_ADDRESS ScratchBufferAddress, D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_DESC& OutDesc, TArrayView<D3D12_RAYTRACING_GEOMETRY_DESC>& OutGeometryDescs) const
{
	...
	// Make sure we have the right amount of space reserved
	check(OutGeometryDescs.Num() == Initializer.Segments.Num());
	checkf(BuffersValid(GPUIndex), TEXT("Index & vertex buffers should be valid (not streamed out) when building the acceleration structure '%s' (owner '%s')"),
		DebugName.IsNone() ? TEXT("UNKNOWN") : *DebugName.ToString(),
		OwnerName.IsNone() ? TEXT("UNKNOWN") : *OwnerName.ToString());

	FD3D12Buffer* IndexBuffer = CommandContext.RetrieveObject<FD3D12Buffer>(Initializer.IndexBuffer.GetReference());
		
	const TArray<FD3D12HitGroupSystemParameters>& HitGroupSystemParametersForThisGPU = PerGPUData[GPUIndex].HitGroupSystemParameters;
	check(HitGroupSystemParametersForThisGPU.Num() == Initializer.Segments.Num());

	ERayTracingAccelerationStructureFlags BuildFlags = GetRayTracingAccelerationStructureBuildFlags(Initializer);

	// Fill in the GeometryDesc
	TranslateRayTracingGeometryDescs(Initializer, OutGeometryDescs);

	// second pass to actually set the current VB/IB buffer addresses
	for (int32 SegmentIndex = 0; SegmentIndex < Initializer.Segments.Num(); ++SegmentIndex)
	{
		...
		{
		case RTGT_Triangles:
			check(Desc.Triangles.VertexFormat == TranslateRayTracingVertexElementType(Segment.VertexBufferElementType));
			// #dxr_todo UE-72160: support various vertex buffer layouts (fetch/decode based on vertex stride and format)
			checkf(Segment.VertexBufferElementType == VET_Float3 ||
				   Segment.VertexBufferElementType == VET_Float4, TEXT("Only VET_Float3 and Float4 are currently implemented and tested. Other formats will be supported in the future."));
			...
			if (IndexBuffer)
			{
				check(Desc.Triangles.IndexCount <= Segment.NumPrimitives * FD3D12RayTracingGeometry::IndicesPerTriangle);
	...
	BuildDesc.DestAccelerationStructureData = PerGPUData[GPUIndex].AccelerationStructureBuffer->ResourceLocation.GetGPUVirtualAddress();
	BuildDesc.ScratchAccelerationStructureData = ScratchBufferAddress;
	BuildDesc.SourceAccelerationStructureData = bIsUpdate
		? PerGPUData[GPUIndex].AccelerationStructureBuffer->ResourceLocation.GetGPUVirtualAddress()
...
#if WITH_NVAPI
void FD3D12RayTracingGeometry::CreateAccelerationStructureBuildDescEx(FD3D12CommandContext& CommandContext, EAccelerationStructureBuildMode BuildMode, D3D12_GPU_VIRTUAL_ADDRESS ScratchBufferAddress, NVAPI_D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_DESC_EX& OutDescEx, TArrayView<NVAPI_D3D12_RAYTRACING_GEOMETRY_DESC_EX>& OutGeometryDescsEx) const
{
	check(GeometryUsesLinearSweptSpheres(GetInitializer()));
	if (Initializer.IndexBuffer)
	{
		checkf(Initializer.IndexBuffer->GetStride() == 2 || Initializer.IndexBuffer->GetStride() == 4, TEXT("Index buffer must be 16 or 32 bit."));
	}
	...
	// Make sure we've reserved the right amount of space
	check(OutGeometryDescsEx.Num() == Initializer.Segments.Num());
	checkf(BuffersValid(GPUIndex), TEXT("Index & vertex buffers should be valid (not streamed out) when building the acceleration structure '%s' (owner '%s')"),
		DebugName.IsNone() ? TEXT("UNKNOWN") : *DebugName.ToString(),
		OwnerName.IsNone() ? TEXT("UNKNOWN") : *OwnerName.ToString());
	...
	TranslateRayTracingGeometryDescsEx(Initializer, OutGeometryDescsEx);

	ERayTracingAccelerationStructureFlags BuildFlags = GetRayTracingAccelerationStructureBuildFlags(Initializer);
	for (int32 SegmentIndex = 0; SegmentIndex < Initializer.Segments.Num(); ++SegmentIndex)
	{
		NVAPI_D3D12_RAYTRACING_GEOMETRY_DESC_EX& Desc = OutGeometryDescsEx[SegmentIndex];
		const FRayTracingGeometrySegment& Segment = Initializer.Segments[SegmentIndex];
		const FD3D12HitGroupSystemParameters& SystemParameters = HitGroupSystemParametersForThisGPU[SegmentIndex];

		FD3D12Buffer* VertexBuffer = CommandContext.RetrieveObject<FD3D12Buffer>(Segment.VertexBuffer.GetReference());

		check(Desc.type == NVAPI_D3D12_RAYTRACING_GEOMETRY_TYPE_LSS_EX);

		Desc.lss.indexCount = Segment.bEnabled ? Segment.NumPrimitives * IndicesPerLinearSweptSphere : 0;

		check(IndexBuffer != nullptr);
		
		Desc.lss.indexFormat = (IndexStride == 4 ? DXGI_FORMAT_R32_UINT : DXGI_FORMAT_R16_UINT);
		Desc.lss.indexBuffer.StartAddress = IndexBuffer->ResourceLocation.GetGPUVirtualAddress() + SystemParameters.RootConstants.IndexBufferOffsetInBytes;
		Desc.lss.indexBuffer.StrideInBytes = sizeof(uint32);
		Desc.lss.vertexPositionBuffer.StartAddress = VertexBuffer->ResourceLocation.GetGPUVirtualAddress() + Segment.VertexBufferOffset;
		Desc.lss.vertexPositionBuffer.StrideInBytes = Segment.VertexBufferStride;
		Desc.lss.vertexRadiusBuffer.StartAddress = Desc.lss.vertexPositionBuffer.StartAddress + 3 * sizeof(float);
		Desc.lss.vertexRadiusBuffer.StrideInBytes = Segment.VertexBufferStride;
	}

	NVAPI_D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAGS_EX LocalBuildFlagsEx = static_cast<NVAPI_D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAGS_EX>(TranslateRayTracingAccelerationStructureFlags(BuildFlags));
	...
	if (bIsUpdate)
	{
		...
		LocalBuildFlagsEx |= NVAPI_D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_PERFORM_UPDATE_EX;
	}
	...
	NVAPI_D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_INPUTS_EX InputsEx = {};
	InputsEx.type = D3D12_RAYTRACING_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL;
	InputsEx.flags = LocalBuildFlagsEx;
	InputsEx.descsLayout = D3D12_ELEMENTS_LAYOUT_ARRAY;
	InputsEx.geometryDescStrideInBytes = sizeof(NVAPI_D3D12_RAYTRACING_GEOMETRY_DESC_EX);
	InputsEx.instanceDescs = 0ULL;
	InputsEx.numDescs = OutGeometryDescsEx.Num();
	InputsEx.pGeometryDescs = OutGeometryDescsEx.GetData();
	...
	ID3D12Device5* RayTracingDevice = CommandContext.GetParentDevice()->GetParentAdapter()->GetD3DDevice5();
	NvAPI_Status status = NvAPI_D3D12_GetRaytracingAccelerationStructurePrebuildInfoEx(RayTracingDevice, &PrebuildParams);
	check(status == NVAPI_OK);
	...
	OutDescEx.inputs.flags = LocalBuildFlagsEx;
	OutDescEx.inputs.type = D3D12_RAYTRACING_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL;
	OutDescEx.inputs.numDescs = OutGeometryDescsEx.Num();
	OutDescEx.inputs.descsLayout = D3D12_ELEMENTS_LAYOUT_ARRAY;
	OutDescEx.inputs.geometryDescStrideInBytes = sizeof(NVAPI_D3D12_RAYTRACING_GEOMETRY_DESC_EX);
	OutDescEx.inputs.pGeometryDescs = OutGeometryDescsEx.GetData();

	OutDescEx.destAccelerationStructureData = PerGPUData[GPUIndex].AccelerationStructureBuffer->ResourceLocation.GetGPUVirtualAddress();
	OutDescEx.scratchAccelerationStructureData = ScratchBufferAddress;
	OutDescEx.sourceAccelerationStructureData = bIsUpdate
		? PerGPUData[GPUIndex].AccelerationStructureBuffer->ResourceLocation.GetGPUVirtualAddress()
		: D3D12_GPU_VIRTUAL_ADDRESS(0);
}
#endif
...
void FD3D12RayTracingGeometry::CompactAccelerationStructure(FD3D12CommandContext& CommandContext, uint32 InGPUIndex, uint64 InSizeAfterCompaction)
{
	...
	check(PerGPUState.bHasPendingCompactionRequests);
	PerGPUState.bHasPendingCompactionRequests = false;
	...
	DEC_MEMORY_STAT_BY(STAT_D3D12RayTracingUsedVideoMemory, PerGPUState.AccelerationStructureBuffer->GetSize());
	DEC_MEMORY_STAT_BY(STAT_D3D12RayTracingBLASMemory, PerGPUState.AccelerationStructureBuffer->GetSize());
	DEC_MEMORY_STAT_BY(STAT_D3D12RayTracingStaticBLASMemory, PerGPUState.AccelerationStructureBuffer->GetSize());

	UnregisterD3D12RayTracingGeometry(this);

	// Move old AS into this temporary variable which gets released when this function returns
	TRefCountPtr<FD3D12Buffer> OldAccelerationStructure = PerGPUState.AccelerationStructureBuffer;

	PerGPUState.AccelerationStructureBuffer = CreateRayTracingBuffer(CommandContext.GetParentAdapter(), InGPUIndex, InSizeAfterCompaction, ERayTracingBufferType::AccelerationStructure, DebugName);
	PerGPUState.AccelerationStructureBuffer->SetOwnerName(OwnerName);
	...
	CommandContext.UpdateResidency(PerGPUState.AccelerationStructureBuffer->GetResource());

	CommandContext.RayTracingCommandList()->CopyRaytracingAccelerationStructure(
		PerGPUState.AccelerationStructureBuffer->ResourceLocation.GetGPUVirtualAddress(),
...
void FD3D12RayTracingScene::ReleaseBuffer()
{
	for (FPerGPUState& GPUState : PerGPUData)
	{
		...
		{
			DEC_MEMORY_STAT_BY(STAT_D3D12RayTracingUsedVideoMemory, GPUState.AccelerationStructureBuffer->GetSize());
			DEC_MEMORY_STAT_BY(STAT_D3D12RayTracingTLASMemory, GPUState.AccelerationStructureBuffer->GetSize());
		}

		GPUState.AccelerationStructureBuffer = nullptr;
	}
}
...
void FD3D12RayTracingScene::BindBuffer(FRHIBuffer* InBuffer, uint32 InBufferOffset)
{
	...
	FOREACH_GPU(GPUIndex < (uint32)PerGPUData.Num(),
	{
		...
		if (PerGPUState.AccelerationStructureBuffer)
		{
			DEC_MEMORY_STAT_BY(STAT_D3D12RayTracingUsedVideoMemory, PerGPUState.AccelerationStructureBuffer->GetSize());
			DEC_MEMORY_STAT_BY(STAT_D3D12RayTracingTLASMemory, PerGPUState.AccelerationStructureBuffer->GetSize());
		}

		PerGPUState.AccelerationStructureBuffer = FD3D12CommandContext::RetrieveObject<FD3D12Buffer>(InBuffer, GPUIndex);

		INC_MEMORY_STAT_BY(STAT_D3D12RayTracingUsedVideoMemory, PerGPUState.AccelerationStructureBuffer->GetSize());
		INC_MEMORY_STAT_BY(STAT_D3D12RayTracingTLASMemory, PerGPUState.AccelerationStructureBuffer->GetSize());
	});
...
#if ENABLE_RESIDENCY_MANAGEMENT
void FD3D12RayTracingScene::UpdateResidencyTracking(FD3D12CommandContext& CommandContext)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UpdateTLASResidency);

	const uint32 GPUIndex = CommandContext.GetGPUIndex();

	FPerGPUState& GPUState = PerGPUData[GPUIndex];

	// Build lookup set of current geometries
	TSet<FRHIRayTracingGeometry*> CurrentGeometrySet;
	CurrentGeometrySet.Reserve(ReferencedGeometries.Num());
	for (const TRefCountPtr<FRHIRayTracingGeometry>& GeomRef : ReferencedGeometries)
	{
		CurrentGeometrySet.Add(GeomRef.GetReference());
	}

	// Helpers: increment/decrement ref counts for a resource array
	auto IncrementRefCounts = [](TMap<const FD3D12Resource*, int32>& RefCounts, TConstArrayView<const FD3D12Resource*> Resources)
		{
			for (const FD3D12Resource* Resource : Resources)
			{
				++RefCounts.FindOrAdd(Resource, 0);
			}
		};

	auto DecrementRefCounts = [](TMap<const FD3D12Resource*, int32>& RefCounts, TConstArrayView<const FD3D12Resource*> Resources)
		{
			for (const FD3D12Resource* Resource : Resources)
			{
				int32& RefCount = RefCounts.FindChecked(Resource);
				if (--RefCount == 0)
				{
					RefCounts.Remove(Resource);
				}
			}
		};

	// Remove stale entries from tracked geometries
	for (auto It = GPUState.TrackedGeometries.CreateIterator(); It; ++It)
	{
		if (!CurrentGeometrySet.Contains(It.Key()))
		{
			DecrementRefCounts(GPUState.ResourceRefCounts, MakeConstArrayView(&It.Value().CachedASResource, 1));
			DecrementRefCounts(GPUState.DispatchResourceRefCounts, It.Value().CachedDispatchResources);
			It.RemoveCurrent();
		}
	}

	// Helper: resolve a geometry's D3D12 resources for this GPU
	auto ResolveGeometryResources = [&CommandContext, GPUIndex](FD3D12RayTracingGeometry* Geometry,
			FGeometryResidencyInfo& OutResidencyInfo)
		{
			OutResidencyInfo.CachedASResource = Geometry->PerGPUData[GPUIndex].AccelerationStructureBuffer->GetResource();
			
			OutResidencyInfo.CachedDispatchResources.Reset();
			...
			if (GRHIGlobals.RayTracing.SupportsShaders || GRHIGlobals.RayTracing.RequiresInlineRayTracingSBT)
			{
				if (Geometry->Initializer.IndexBuffer)
				{
					...
					OutResidencyInfo.CachedDispatchResources.Add(IndexBuffer->GetResource());
				}
				...
				for (const FRayTracingGeometrySegment& Segment : Geometry->Initializer.Segments)
				{
					...
					{
						FD3D12Buffer* VertexBuffer = CommandContext.RetrieveObject<FD3D12Buffer>(Segment.VertexBuffer.GetReference());
						OutResidencyInfo.CachedDispatchResources.Add(VertexBuffer->GetResource());
					}
				}
			}
		};

	// Process additions and mutations
	for (int32 Index = 0; Index < ReferencedGeometries.Num(); ++Index)
	{
		FRHIRayTracingGeometry* GeometryRHI = ReferencedGeometries[Index];
		FD3D12RayTracingGeometry* Geometry = FD3D12DynamicRHI::ResourceCast(GeometryRHI);

		checkf(!Geometry->IsDirty(GPUIndex),
		...
		if (GRHIGlobals.RayTracing.SupportsShaders || GRHIGlobals.RayTracing.RequiresInlineRayTracingSBT)
		{
			checkf(Geometry->BuffersValid(GPUIndex),
				TEXT("Index and vertex buffers for all geometries must be valid (streamed in) when adding geometry to the top level acceleration structure for the scene"));
		}

		FGeometryResidencyInfo* ExistingInfo = GPUState.TrackedGeometries.Find(GeometryRHI);

		if (!ExistingInfo)
		{
			// Added geometry - resolve and track
			FGeometryResidencyInfo Info;
			Info.ResourceGeneration = Geometry->ResourceGeneration;
			ResolveGeometryResources(Geometry, Info);

			IncrementRefCounts(GPUState.ResourceRefCounts, MakeConstArrayView(&Info.CachedASResource, 1));
			IncrementRefCounts(GPUState.DispatchResourceRefCounts, Info.CachedDispatchResources);

			GPUState.TrackedGeometries.Add(GeometryRHI, MoveTemp(Info));
		}
		else if (ExistingInfo->ResourceGeneration != Geometry->ResourceGeneration)
		{
			// Mutated geometry - swap resources
			DecrementRefCounts(GPUState.ResourceRefCounts, MakeConstArrayView(&ExistingInfo->CachedASResource, 1));
			DecrementRefCounts(GPUState.DispatchResourceRefCounts, ExistingInfo->CachedDispatchResources);

			ExistingInfo->ResourceGeneration = Geometry->ResourceGeneration;
			ResolveGeometryResources(Geometry, *ExistingInfo);

			IncrementRefCounts(GPUState.ResourceRefCounts, MakeConstArrayView(&ExistingInfo->CachedASResource, 1));
			IncrementRefCounts(GPUState.DispatchResourceRefCounts, ExistingInfo->CachedDispatchResources);
		}
		// else: unchanged - skip entirely
	}

	// Rebuild flat arrays from tracked resources + transient referenced buffers, deduplicating by residency handle to minimize UpdateResidency calls.
	{
		Experimental::TSherwoodSet<FD3D12ResidencyHandle*> UniqueResidencyHandles;

		auto AddUniqueResource = [&UniqueResidencyHandles](const FD3D12Resource* Resource, TArray<const FD3D12Resource*>& OutResources)
			{
		...
		GPUState.ResourcesToMakeResident.Reset(GPUState.ResourceRefCounts.Num() + ReferencedBuffers.Num());
		for (const auto& Pair : GPUState.ResourceRefCounts)
		{
			AddUniqueResource(Pair.Key, GPUState.ResourcesToMakeResident);
		}

		for (FRHIBuffer* ReferencedBuffer : ReferencedBuffers)
		{
			FD3D12Buffer* Buffer = CommandContext.RetrieveObject<FD3D12Buffer>(ReferencedBuffer);
			AddUniqueResource(Buffer->GetResource(), GPUState.ResourcesToMakeResident);
		}

		GPUState.DispatchResourcesToMakeResident.Reset(GPUState.DispatchResourceRefCounts.Num());
		for (const auto& Pair : GPUState.DispatchResourceRefCounts)
		{
			AddUniqueResource(Pair.Key, GPUState.DispatchResourcesToMakeResident);
		}
	}
}
#endif // ENABLE_RESIDENCY_MANAGEMENT

void FD3D12RayTracingScene::UpdateResidency(FD3D12CommandContext& CommandContext, bool bIncludeDispatchResources) const
{
#if ENABLE_RESIDENCY_MANAGEMENT
	const uint32 GPUIndex = CommandContext.GetGPUIndex();
	const FPerGPUState& PerGPUState = PerGPUData[GPUIndex];
	CommandContext.UpdateResidency(PerGPUState.AccelerationStructureBuffer->GetResource());
	for (const FD3D12Resource* Resource : PerGPUState.ResourcesToMakeResident)
	{
		CommandContext.UpdateResidency(Resource);
	}
	if (bIncludeDispatchResources)
	{
		for (const FD3D12Resource* Resource : PerGPUState.DispatchResourcesToMakeResident)
		{
			CommandContext.UpdateResidency(Resource);
		}
	}
	...
	D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_DESC& OutBuildDesc)
{
...
#if ENABLE_RESIDENCY_MANAGEMENT
	Scene.UpdateResidencyTracking(CommandContext);
#endif // ENABLE_RESIDENCY_MANAGEMENT

	// Make necessary resources resident

	TRefCountPtr<FD3D12Buffer>& AccelerationStructureBuffer = Scene.PerGPUData[GPUIndex].AccelerationStructureBuffer;
	...
	Scene.UpdateResidency(CommandContext, false);
...

	{
		...
		checkf(BufferAddress % GRHIRayTracingAccelerationStructureAlignment == 0,
			TEXT("TLAS buffer (plus offset) must be aligned to %d bytes."),
			GRHIRayTracingAccelerationStructureAlignment);

		checkf(ScratchAddress % GRHIRayTracingScratchBufferAlignment == 0,
			TEXT("TLAS scratch buffer (plus offset) must be aligned to %d bytes."),
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(D3D12::BuildRaytracingAccelerationStructure);
...
#if WITH_NVAPI
void FD3D12CommandContext::BuildAccelerationStructuresInternalEx(const TArrayView<NVAPI_D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_DESC_EX> DescArray)
{
	for (const NVAPI_D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_DESC_EX& Desc : DescArray)
	{
		NVAPI_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_EX_PARAMS Params = { 0 };
		Params.version = NVAPI_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_EX_PARAMS_VER;
		Params.pDesc = &Desc;

		NvAPI_Status status = NvAPI_D3D12_BuildRaytracingAccelerationStructureEx(GraphicsCommandList4().Get(), &Params);
		check(status == NVAPI_OK);
	}
}
#endif
...
#if WITH_NVAPI
	TArray<NVAPI_D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_DESC_EX, TInlineAllocator<32>> BuildDescsEx;
	BuildDescsEx.Reserve(Params.Num());
#endif
	...
	for (int32 i = 0; i < Params.Num(); i++)
	{
		...
		if (Geometry->IsDirty(GPUIndex))
		{
			...
			checkf(ScratchBufferRequiredSize + ScratchBufferOffset <= ScratchBufferSize,
				TEXT("BLAS scratch buffer size is %d bytes with offset %d (%d bytes available), but the build requires %lld bytes. "),
			...
			checkf(ScratchBufferAddress % GRHIRayTracingAccelerationStructureAlignment == 0,
				TEXT("BLAS scratch buffer (plus offset) must be aligned to %d bytes."),
				GRHIRayTracingAccelerationStructureAlignment);

#if WITH_NVAPI
			if (GeometryUsesLinearSweptSpheres(Geometry->GetInitializer()))
			{
				const uint32 NumGeometryDescs = Geometry->GetInitializer().Segments.Num();
				NVAPI_D3D12_RAYTRACING_GEOMETRY_DESC_EX* LocalGeometryDescsMemoryEx = (NVAPI_D3D12_RAYTRACING_GEOMETRY_DESC_EX*)FMemStack::Get().Alloc(NumGeometryDescs * sizeof(NVAPI_D3D12_RAYTRACING_GEOMETRY_DESC_EX), alignof(NVAPI_D3D12_RAYTRACING_GEOMETRY_DESC_EX));
				TArrayView<NVAPI_D3D12_RAYTRACING_GEOMETRY_DESC_EX> LocalGeometryDescsEx = MakeArrayView(LocalGeometryDescsMemoryEx, NumGeometryDescs);

				NVAPI_D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_DESC_EX& BuildDescEx = BuildDescsEx.AddZeroed_GetRef();
				Geometry->CreateAccelerationStructureBuildDescEx(*this, P.BuildMode, ScratchBufferAddress, BuildDescEx, LocalGeometryDescsEx);
			}
			else
#endif
			{
...
#if WITH_NVAPI
	BuildAccelerationStructuresInternalEx(BuildDescsEx);
#endif
	...
	for (const FRayTracingGeometryBuildParams& P : Params)
	{
		...
		if (Geometry->IsDirty(GPUIndex))
		{
			...
			if (ShouldCompactAfterBuild(GeometryBuildFlags))
			{
				...
				Geometry->PerGPUData[GPUIndex].bHasPendingCompactionRequests = true;
			}
...
struct FD3D12RayTracingGlobalResourceBinder
{
	...
	{
		D3D12RayTracingScene->UpdateResidency(CommandContext, true);
	}
	...
	ResourceBinderType& Binder)
{
	...
	struct FBindings
	{
		...
		void SetResourceCollection(FRHIResourceCollection* ResourceCollection, uint8 Index)
		{
			...
			// Ensure all member SRVs referenced by the collection buffer are resident.
			for (FD3D12ShaderResourceView* MemberSRV : D3D12ResourceCollection->AllSrvs)
			{
				Binder.AddReferencedShaderResource(MemberSRV->GetShaderResource());

				if (FD3D12RayTracingScene* ReferencedRayTracingScene = MemberSRV->GetRayTracingScene())
				{
					ReferencedRayTracingScenes.Add(ReferencedRayTracingScene);
				}
			}

			// Resolve texture references to their current textures and ensure residency.
			for (FD3D12RHITextureReference* TextureReference : D3D12ResourceCollection->AllTextureReferences)
			{
				if (TextureReference)
				{
					Binder.AddReferencedTexture(TextureReference);
				}
			}
		}
		...
		if (FRHIResource* Resource = ShaderParameterResource.Resource)
		{
			switch (ShaderParameterResource.Type)
			{
				...
				case FRHIShaderParameterResource::EType::ResourceCollection:
					Bindings.SetResourceCollection(static_cast<FRHIResourceCollection*>(Resource), BindlessParameterIndex);
					break;
			}
		}
	}
...
static void DispatchRays(FD3D12CommandContext& CommandContext,
	...
	FD3D12Resource* RayGenUploadResource = nullptr,
	FD3D12Buffer* ArgumentBuffer = nullptr, uint32 ArgumentOffset = 0)
{
	...
	if (ArgumentBuffer)
	{
		...
		CommandContext.UpdateResidency(DispatchRaysDescBufferResource);
		CommandContext.UpdateResidency(UploadResourceLocation.GetResource());
		CommandContext.UpdateResidency(ArgumentBuffer->GetResource());
	...
	if (bResourcesBound)
	{
		// Ensure residency for the ray gen
		if (RayGenUploadResource)
		{
			CommandContext.UpdateResidency(RayGenUploadResource);
		}
	...
	uint32 Width, uint32 Height)
{
	...
	FD3D12Resource* RayGenUploadResource = nullptr;
	D3D12_DISPATCH_RAYS_DESC DispatchDesc = ShaderTableForDevice->GetDispatchRaysDesc(GetParentDevice(), RayGenShaderIdentifier, RayGenUploadResource);

	DispatchDesc.Width = Width;
	DispatchDesc.Height = Height;
	DispatchDesc.Depth = 1;

	DispatchRays(*this, GlobalResourceBindings, Pipeline, RayGenShaderIndex, ShaderTableForDevice, DispatchDesc, QueueType, RayGenUploadResource);
}
	...
	FRHIBuffer* ArgumentBuffer, uint32 ArgumentOffset)
{
	...
	FD3D12Resource* RayGenUploadResource = nullptr;
	D3D12_DISPATCH_RAYS_DESC DispatchDesc = ShaderTableForDevice->GetDispatchRaysDesc(GetParentDevice(), RayGenShaderIdentifier, RayGenUploadResource);
	DispatchRays(*this, GlobalResourceBindings, Pipeline, RayGenShaderIndex, ShaderTableForDevice, DispatchDesc, QueueType, RayGenUploadResource, RetrieveObject<FD3D12Buffer>(ArgumentBuffer), ArgumentOffset);
}
	...
	uint32 WorkerIndex)
{
	...
	ShaderTable->SetHitGroupGeometrySystemParameters(WorkerIndex, BindingType, RecordIndex, Geometry, GeometrySegmentIndex, UserData);
	...
	ERayTracingBindingType BindingType)
{
	...
	auto BindingTask = [Bindings, Device = Device, ShaderTableForDevice, Pipeline, BindingType](const FTaskContext& Context, int32 CurrentIndex)
	{
		...
		if (BindingType == ERayTracingBindingType::HitGroup)
		{
			...
			{
				//UE_LOGF(LogD3D12RHI, Log, "Set hit record data for RecordIndex %d on SBT %#016llx with mode: %d", Binding.RecordIndex, ShaderTableForDevice, Binding.BindingType);
			...
			{
				//UE_LOGF(LogD3D12RHI, Log, "Clear hit record data for RecordIndex %d on SBT %#016llx", Binding.RecordIndex, ShaderTableForDevice);
	...
	ParallelForWithExistingTaskContext(TEXT("RHISetBindingsOnShaderBindingTable_ParallelFor"), MakeArrayView(TaskContexts), NumBindings, ItemsPerTask, BindingTask);
