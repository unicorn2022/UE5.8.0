...
static void ValidateScissorRect(const D3D12_VIEWPORT& Viewport, const D3D12_RECT& ScissorRect)
{
	...
	bScissorRectValid = bScissorRectValid && ScissorRect.left >= (LONG)FMath::RoundToZero(Viewport.TopLeftX);
	bScissorRectValid = bScissorRectValid && ScissorRect.top >= (LONG)FMath::RoundToZero(Viewport.TopLeftY);
	bScissorRectValid = bScissorRectValid && ScissorRect.right <= (LONG)FMath::RoundFromZero(Viewport.TopLeftX + Viewport.Width);
	bScissorRectValid = bScissorRectValid && ScissorRect.bottom <= (LONG)FMath::RoundFromZero(Viewport.TopLeftY + Viewport.Height);
	...
	ensureMsgf(bScissorRectValid,
		TEXT("Scissor invalid with current Viewport. Scissor: [left:%li, top:%li, right:%li, bottom:%li]. Viewport: [left:%.2f, top:%.2f, right:%.2f, bottom:%.2f]")
			, ScissorRect.left
			, ScissorRect.top
			, ScissorRect.right
			, ScissorRect.bottom
...
void FD3D12StateCache::ApplyResourceTables(const FD3D12RootSignature* const pRootSignature, uint32 StartStage, uint32 EndStage)
{
	...
	uint32 NumUAVs[SF_NumStandardFrequencies] = {};
	...
	for (uint32 iTries = 0; iTries < 2; ++iTries)
	{
		for (uint32 Stage = StartStage; Stage < EndStage; ++Stage)
		{
			...
			if (bUAVs)
			{
				const UAVSlotMask CurrentShaderUAVRegisterMask = BitMask<UAVSlotMask>(PipelineState.Common.CurrentShaderUAVCounts[Stage]);
				CurrentShaderDirtyUAVSlots[Stage] = CurrentShaderUAVRegisterMask & PipelineState.Common.UAVCache.DirtySlotMask[Stage];
				if (CurrentShaderDirtyUAVSlots[Stage])
				{
					check(D3D12DoesShaderFrequencySupportUAV(static_cast<EShaderFrequency>(Stage)));
					...
					if (ResourceBindingTier <= D3D12_RESOURCE_BINDING_TIER_2)
					{
						...
						NumUAVs[Stage] = pRootSignature->MaxUAVCount(Stage);
					}
					else
					{
						NumUAVs[Stage] = PipelineState.Common.CurrentShaderUAVCounts[Stage];
					}

					check(NumUAVs[Stage] > 0 && NumUAVs[Stage] <= MAX_UAVS);
					NumViews += NumUAVs[Stage];
				}
			}
	...
	if (bUAVs)
	{
		...
		FD3D12UnorderedAccessViewCache& UAVCache = PipelineState.Common.UAVCache;

		for (uint32 Index = StartStage; Index < EndStage; Index++)
		{
			if (CurrentShaderDirtyUAVSlots[Index])
			{
				const EShaderFrequency ShaderFrequency = static_cast<EShaderFrequency>(Index);

				const D3D12_GPU_DESCRIPTOR_HANDLE BindDescriptor = DescriptorCache.BuildUAVTable(ShaderFrequency, pRootSignature, UAVCache, CurrentShaderDirtyUAVSlots[Index], NumUAVs[Index], ViewHeapSlot);
				DescriptorCache.SetUAVTable(ShaderFrequency, pRootSignature, UAVCache, NumUAVs[Index], BindDescriptor);
			}
		}
	}
...
// This requires the debug layer
	...
	UE_LOGF(LogD3D12RHI, Log, "*** VerifyResourceStates requires the debug layer ***", this);
	return true;
#else
	...
	if (!bWithD3DDebug)
	{
		UE_LOGF(LogD3D12RHI, Fatal, "*** AssertResourceStates requires the debug layer ***");
		return false;
	}
...
void FD3D12StateCache::SetUAV(EShaderFrequency ShaderStage, uint32 SlotIndex, FD3D12UnorderedAccessView* UAV)
{
...
void FD3D12StateCache::SetShaderResourceView(EShaderFrequency ShaderFrequency, FD3D12ShaderResourceView* SRV, uint32 ResourceIndex)
{
	...
	// If we are binding an SRV that isn't actually backed by a resource, we shouldn't act as if we are non-null.
	// Fall back to the 'null' binding path.
	if (SRV && SRV->GetResource() == nullptr)
	{
		SRV = nullptr;
	}
