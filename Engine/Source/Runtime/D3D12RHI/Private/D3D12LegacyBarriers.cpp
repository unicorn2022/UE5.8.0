	...
	ED3D12QueueType QueueType)
{
	...
	const bool bCheckResourceName = !Filter.IsEmpty();
	...
	for (int32 i = 0; i < Barriers.Num(); i++)
	{
		...
		case D3D12_RESOURCE_BARRIER_TYPE_TRANSITION:
		{
			...
			if (bShouldLog)
			{
				...
				if (!bCheckResourceName || BarrierResourceName.Contains(Filter))
				{
					UE_LOGF(LogD3D12RHI, Log, "*** BARRIER (CmdList: %016llX, Queue: %ls) %u/%u: %ls %016llX (Sub: %u), %ls -> %ls", pCommandList, GetD3DCommandQueueTypeName(QueueType), i + 1, Barriers.Num(),
						*BarrierResourceName,
		...
		case D3D12_RESOURCE_BARRIER_TYPE_UAV:
			{
				...
				if (!bCheckResourceName || BarrierResourceName.Contains(Filter))
				{
					UE_LOGF(LogD3D12RHI, Log, "*** BARRIER (CmdList: %016llX, Queue: %ls) %u/%u: UAV Barrier %ls", pCommandList, GetD3DCommandQueueTypeName(QueueType), i + 1, Barriers.Num(), *BarrierResourceName);
				}
			}
			break;
		...
		case D3D12_RESOURCE_BARRIER_TYPE_ALIASING:
			{
				...
				if (!bCheckResourceName || BarrierResourceNameBefore.Contains(Filter) || BarrierResourceNameAfter.Contains(Filter))
				{
					UE_LOGF(LogD3D12RHI, Log, "*** BARRIER (CmdList: %016llX, Queue: %ls) %u/%u: Aliasing Barrier, %016llX %ls -> %016llX %ls", pCommandList, GetD3DCommandQueueTypeName(QueueType), i + 1, Barriers.Num(), currentBarrier.Aliasing.pResourceBefore, *BarrierResourceNameBefore, currentBarrier.Aliasing.pResourceAfter, *BarrierResourceNameAfter);
				}
			}
			break;
		...
		const FRHITransitionCreateInfo& CreateInfo)
{
	...
	if (bAsyncToAllPipelines)
	{
		...
		{
			if (EnumHasAnyFlags(TransitionInfo.AccessAfter, ERHIAccess::GraphicsReadableMask))
			{
	...
	TRefCountPtr<ID3D12Resource>& OutResource)
{
	...
	if (InDesc.SupportsUncompressedUAV())
	{
		checkf(InInitialD3D12Access == ED3D12Access::Common, TEXT("RESOURCE_STATE_COMMON is required for castable resources (Given: %d)"), EnumToUnderlyingType(InInitialD3D12Access));
	...
	FD3D12ContextCommon& Context)
{
	...
	{
		LogResourceBarriers(
			MakeConstArrayView<D3D12_RESOURCE_BARRIER>({Barrier}),
			Context.GetCommandList().BaseCommandList().Get(),
	...
	uint32 Subresource)
{
	...
	if (Barriers.Num())
	{
		// Check if the new transition is for the same (sub)resource as the prior barrier, which also must be a transition.
		FD3D12ResourceBarrier& Last = Barriers.Last();
		if (Last.Type == D3D12_RESOURCE_BARRIER_TYPE_TRANSITION
			...
			// This should always be true (assuming the above are also true) but we don't want to obscure bugs by assuming it
			&& Before == Last.Transition.StateAfter)
		{
			...
			if (After == Last.Transition.StateBefore)
			{
				Barriers.RemoveAt(Barriers.Num() - 1);
				return -1;
			}
			// No work was done between this and the last transition so we can replace the existing transition's after state with
			// the new after state, effectively merging the two barriers. This commonly happens during copies since we "push" and 
			// then "pop" the copy states and often there's no additional work done between that pop (e.g. transition from the copy
			// state back to whatever state the resource was in before) and the next transition.
			else
			{
	...
	FD3D12QueryAllocator& TimestampAllocator)
{
	...
	for (int32 BatchStart = 0, BatchEnd = 0; BatchStart < Barriers.Num(); BatchStart = BatchEnd)
	{
		...
		if (GD3D12LogBarriers & D3D12LogBarriersWhenFlushed)
		{
			...
			LogResourceBarriers(ConstArrayView, CommandList.BaseCommandList().Get(), CommandList.QueueType);
		}
#endif
	...
	bool& bUAVBarrier)
{
	for (const FRHITransitionInfo& Info : TransitionData->TransitionInfos)
	{
		...
		// Edge case to handle graphics state transitions on the async compute pipe that needs to be removed with EB
		// Transitions to read-only Gfx bits get handled on the Gfx pipe
		...
		if (TransitionData->bAsyncToAllPipelines && EnumHasAllFlags(ResourceState.AccessAfter, ERHIAccess::SRVMask) && (QueueType == ED3D12QueueType::Direct))
		{
	...
	uint32 InSubresourceIndex)
{
	...
	{
		UE_LOGF(LogRHI, Error, "Incompatible Transition State for Resource %ls - %ls", *InResource->GetName().ToString(), *IncompatibilityReason);
	}
#endif
