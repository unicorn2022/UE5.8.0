	...
	else // Commit / grow case
	{
		...
		while (ReservedResourceData->NumCommittedTiles < NumRequiredCommitTiles)
		{
			...
			else
			{
				...
				{
					Adapter->SetResidencyPriority(D3DHeap, D3D12_RESIDENCY_PRIORITY_HIGH, GPUIndex, NewHeapDesc.SizeInBytes);
				}
...
#if ENABLE_RESIDENCY_MANAGEMENT
	const bool bMultiGPUEnabled = (GNumExplicitGPUsForRendering > 1);
	bRequiresResidencyTracking = !bMultiGPUEnabled || IsGPUOnly(HeapDesc.Properties.Type, &HeapDesc.Properties);
#endif // ENABLE_RESIDENCY_MANAGEMENT
	...
	bool bVerifyHResult)
{
	...
	{
		UE_LOGF(LogD3D12RHI, Display, "D3D12 CreateCommittedResource failed with params:\n\tHeap Type: %d\n\tHeap Flags: %d\n\tResource Dimension: %d\n\tResource Width: %llu\n\tResource Height: %u\n\tArray Size: %u\n\tMip Levels: %u\n\tFormat: %d\n\tResource Flags: %d",
	...
	bool bVerifyHResult)
{
	...
	{
		UE_LOGF(LogD3D12RHI, Display, "D3D12 CreateReservedResource failed with params:\n\tResource Dimension: %d\n\tResource Width: %llu\n\tResource Height: %u\n\tFormat: %d\n\tResource Flags: %d",
	...
	bool bVerifyHResult)
{
	...
	{
		UE_LOGF(LogD3D12RHI, Display, "D3D12 CreatePlacedResource failed with params:\n\tHeap Type: %d\n\tHeap Flags: %d\n\tResource Dimension: %d\n\tResource Width: %llu\n\tResource Height: %u\n\tFormat: %d\n\tResource Flags: %d",
...
void FD3D12ResourceLocation::TransferOwnership(FD3D12ResourceLocation& Destination, FD3D12ResourceLocation& Source)
{
	...
	{
		// Note: pool allocated locations handle the copy themselves to prevent data races when updating their linked list.
	...
	ED3D12Access& OutD3D12Access)
{
	...
	UE_TRACE_METADATA_SCOPE_ASSET_FNAME(CurrentResource->GetName(), FName("FD3D12ResourceLocation::OnAllocationMoved"), NAME_None);
...
void FD3D12DynamicRHI::RHIWriteResourceGPUAddresses(FRHICommandListBase& RHICmdList, uint32 GPUIndex, TConstArrayView<FRHIResource*> Resources, uint64* AddressesPtr)
{
	RHICmdList.EnqueueLambda([AddressesPtr, Resources, GPUIndex](FRHICommandListBase&)
		{
			...
			for (int32 ResourceIndex = 0; ResourceIndex < Resources.Num(); ++ResourceIndex)
			{
				...
				{
					const FD3D12Buffer* Buffer = ResourceCast(static_cast<const FRHIBuffer*>(Resource), GPUIndex);
					AddressesPtr[ResourceIndex] = Buffer->GetGPUVirtualAddress();
				}
				...
				{
					const FD3D12RayTracingGeometry* RayTracingGeometry = ResourceCast(static_cast<const FRHIRayTracingGeometry*>(Resource));
					AddressesPtr[ResourceIndex] = RayTracingGeometry->GetGPUVirtualAddress(GPUIndex);
				}
#endif
				else
				{
