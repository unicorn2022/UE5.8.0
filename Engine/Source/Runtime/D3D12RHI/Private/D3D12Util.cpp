...
{
	if (FD3D12Adapter* const D3D12Adapter = ChosenAdapter.Get())
	{
...
static FString GetD3D12ErrorString(HRESULT ErrorCode, ID3D12Device* Device)
{
	...
	if (ErrorCode == DXGI_ERROR_DEVICE_REMOVED && Device)
	{
		...
		ErrorCodeText += FString(" with Reason: ") + GetD3D12DeviceHungErrorString(hResDeviceRemoved);
	}
...
static bool LogDREDData(ID3D12Device* Device, bool bTrackingAllAllocations, D3D12_GPU_VIRTUAL_ADDRESS& OutPageFaultGPUAddress)
{
	...
	if (Dred.Data.IsValid())
	{
		...
		{
			UE_LOGF(LogD3D12RHI, Error, "DRED: Last tracked GPU operations:");
			...
			while (Node && Node->pLastBreadcrumbValue)
			{
				...
				if (LastCompletedOp != Node->BreadcrumbCount && LastCompletedOp != 0)
				{
					...
					UE_LOGF(LogD3D12RHI, Error, "DRED: Commandlist \"%ls\" on CommandQueue \"%ls\", %d completed of %d", Node->pCommandListDebugNameW, Node->pCommandQueueDebugNameW, LastCompletedOp, Node->BreadcrumbCount);
					TracedCommandLists++;
					...
					for (int32 Op = FirstOp; Op <= LastOp; ++Op)
					{
						...
						const TCHAR* OpName = (BreadcrumbOp < UE_ARRAY_COUNT(BreadcrumbOpNames)) ? BreadcrumbOpNames[BreadcrumbOp] : TEXT("Unknown Op");
						UE_LOGF(LogD3D12RHI, Error, "\tOp: %d, %ls%ls%ls", Op, OpName, *ContextStr, (Op + 1 == LastCompletedOp) ? TEXT(" - LAST COMPLETED") : TEXT(""));
					}
			...
			{
				UE_LOGF(LogD3D12RHI, Error, "DRED: No command list found with active outstanding operations (all finished or not started yet).");
			}
		}
		else
		{
			UE_LOGF(LogD3D12RHI, Error, "DRED: No breadcrumb head found.");
		}
		...
		if (SUCCEEDED(Dred.Data->GetPageFaultAllocationOutput(&DredPageFaultOutput)) && DredPageFaultOutput.PageFaultVA != 0)
		{
			...
			UE_LOGF(LogD3D12RHI, Error, "DRED: PageFault at VA GPUAddress \"0x%llX\"", (long long)DredPageFaultOutput.PageFaultVA);
			
			const D3D12_DRED_ALLOCATION_NODE* Node = DredPageFaultOutput.pHeadExistingAllocationNode;
			if (Node)
			{
				UE_LOGF(LogD3D12RHI, Error, "DRED: Active objects with VA ranges that match the faulting VA:");
				while (Node)
				{
					...
					if (!bTrackingAllAllocations || Node->ObjectNameW)
					{
						...
						UE_LOGF(LogD3D12RHI, Error, "\tName: %ls (Type: %ls)", Node->ObjectNameW, AllocTypeName);
					}
					Node = Node->pNext;
				}
			}
			...
			if (Node)
			{
				UE_LOGF(LogD3D12RHI, Error, "DRED: Recent freed objects with VA ranges that match the faulting VA:");
				while (Node)
				{
					// See comments above
					if (!bTrackingAllAllocations || Node->ObjectNameW)
		...
		{
			UE_LOGF(LogD3D12RHI, Error, "DRED: No PageFault data.");
		}
...
void LogPageFaultData(FD3D12Adapter* InAdapter, FD3D12Device* InDevice, D3D12_GPU_VIRTUAL_ADDRESS InPageFaultAddress)
{
	...
	UE_LOGF(LogD3D12RHI, Error, "PageFault: PageFault at VA GPUAddress \"0x%llX\" (GPU %d)", (long long)InPageFaultAddress, InDevice->GetGPUIndex());
	...
	UE_LOGF(LogD3D12RHI, Error, "PageFault: Last completed frame ID: %lld (cached: %lld) - Current frame ID: %lld", ActualFenceValue, CachedFenceValue, NextFenceValue);
	UE_LOGF(LogD3D12RHI, Error, "PageFault: Logging all resource enabled: %ls", InAdapter->IsTrackingAllAllocations() ? TEXT("Yes") : TEXT("No"));
	...
	UE_LOGF(LogD3D12RHI, Error, "PageFault: Found %d active tracked resources in %3.2f MB range of page fault address", OverlappingResources.Num(), CheckRangeRadius / (1024.0f * 1024));
	if (OverlappingResources.Num() > 0)
	{
		...
		for (uint32 Index = 0; Index < PrintCount; ++Index)
		{
			...
			UE_LOGF(LogD3D12RHI, Error, "\tGPU Address: [0x%llX .. 0x%llX] - Size: %lld bytes, %3.2f MB - Distance to page fault: %lld bytes, %3.2f MB - Transient: %d - Name: %ls - Desc: %ls",
	...
	UE_LOGF(LogD3D12RHI, Error, "PageFault: Found %d active heaps containing page fault address", OverlappingHeaps.Num());
	for (int32 Index = 0; Index < OverlappingHeaps.Num(); ++Index)
	{
		FD3D12Heap* Heap = OverlappingHeaps[Index];
		UE_LOGF(LogD3D12RHI, Error, "\tGPU Address: \"0x%llX\" - Size: %3.2f MB - Name: %ls",
	...
	UE_LOGF(LogD3D12RHI, Error, "PageFault: Found %d released resources containing the page fault address during last 100 frames", ReleasedResources.Num());
	if (ReleasedResources.Num() > 0)
	{
		...
		for (uint32 Index = 0; Index < PrintCount; ++Index)
		{
			...
			UE_LOGF(LogD3D12RHI, Error, "\tGPU Address: [0x%llX .. 0x%llX] - Size: %lld bytes, %3.2f MB - FrameID: %4lld - DefragFree: %d - Transient: %d - Heap: %d - Name: %ls - Desc: %ls",
...
{	
	const FRHIMemoryStats& MemoryStats = InAdapter->GetMemoryStats();

	UE_LOGF(LogD3D12RHI, Error, "Video Memory Stats from frame ID %lld:", InAdapter->GetMemoryStatsUpdateFrame());
	UE_LOGF(LogD3D12RHI, Error, "\tLocal Budget:\t%7.2f MB", MemoryStats.BudgetLocal / (1024.0f * 1024));
	UE_LOGF(LogD3D12RHI, Error, "\tLocal Used:\t%7.2f MB", MemoryStats.UsedLocal / (1024.0f * 1024));
	UE_LOGF(LogD3D12RHI, Error, "\tSystem Budget:\t%7.2f MB", MemoryStats.BudgetSystem / (1024.0f * 1024));
	UE_LOGF(LogD3D12RHI, Error, "\tSystem Used:\t%7.2f MB", MemoryStats.UsedSystem / (1024.0f * 1024));
}
...
void FD3D12DynamicRHI::TerminateOnOutOfMemory(ID3D12Device* InDevice, HRESULT D3DResult, bool bCreatingTextures)
{
	...
	// if InDevice == nullptr, just pick the chosen adapter
	if (!Adapter && ChosenAdapter.IsValid())
	{
		check(!InDevice);
		Adapter = ChosenAdapter.Get();
	}

	if (Adapter)
	{
		const auto& CurrentMemoryStats = Adapter->GetMemoryStats();
		FCoreDelegates::GetGPUOutOfMemoryDelegate().Broadcast(CurrentMemoryStats.BudgetLocal, CurrentMemoryStats.UsedLocal);
	}
	...
	UE_LOGF(LogD3D12RHI, Fatal, "Out of video memory trying to allocate a rendering resource");
	if (!bGPUCrashOOM)
	{
	...
	UE_LOGF(LogInit, Fatal, "Out of video memory trying to allocate a rendering resource");
#endif // !PLATFORM_WINDOWS
}
...
void FD3D12DynamicRHI::OutputGPUCrashReport(FTextBuilder& ErrorMessage)
{
	...
	// Log which devices were removed and their reason strings
	{
		...
		UE_LOGF(LogD3D12RHI, Error, "GPU crash detected:%ls\r\n", *RemovedReasons);
	}
	...
	// Log shader asserts / prints
	{
		...
		{
			UE_LOGF(LogD3D12RHI, Error, "Shader diagnostic messages and asserts:%ls\r\n", *ShaderDiagnostics);
		}
	}
	...
	ForEachDevice(nullptr, [&](FD3D12Device* Device)
	{
		...
		if (!LogDREDData<FDred_1_2>(Device->GetDevice(), bIsTrackingAllAllocations, PageFaultAddress))
		{
			...
			{
				UE_LOGF(LogD3D12RHI, Error, "DRED: could not find DRED data (might not be enabled or available). Run with -dred or -gpucrashdebugging to enable dred if available.");
			}						
		}
...
void FD3D12DynamicRHI::TerminateOnGPUCrash(TOptional<HRESULT> D3DResult)
{
	// Include the D3D HRESULT that alerted the application of the GPU crash and the device removal reason, if present.
	// In some cases, such as when the runtime signals the device is lost through fence values, there is no HRESULT
	// but there might be a device removal reason.
	if (D3DResult.IsSet())
	{
		FGenericCrashContext::SetEngineData(TEXT("GPUCrash.D3DResult"), FString::FromInt(*D3DResult));
	}

	TConstArrayView<FD3D12Device*> Devices = GetAdapter().GetDevices();
	for (FD3D12Device* Device : Devices)
	{
		HRESULT Reason = Device->GetDevice()->GetDeviceRemovedReason();
		if (Reason != S_OK)
		{
			FGenericCrashContext::SetEngineData(TEXT("GPUCrash.D3DDeviceRemovedReason"), FString::FromInt(Reason));

			// Just include one device's removal reason in the crash report payload.
			break;
		}
	}

#if WITH_NVAPI_RAYTRACING_VALIDATION
	FlushRaytracingValidationMessages();
#endif
	...
	{
		UE_LOGF(LogD3D12RHI, D3D12RHI_GPU_CRASH_LOG_VERBOSITY, "%ls", *ErrorMessage.ToText().ToString());
	}
...
void FD3D12DynamicRHI::HandleFailedD3D12Result(HRESULT D3DResult, ID3D12Device* Device, bool bCreatingTextures, const TCHAR* Message)
{
	auto LogMessage = [Message](bool bFatal)
	{
		if (bFatal)
		{
			UE_LOGF(LogD3D12RHI, Fatal, "%ls", Message);
		}
		else
		{
			UE_LOGF(LogD3D12RHI, Error, "%ls", Message);
		}
	};
	...
	else if (D3DResult == DXGI_ERROR_DEVICE_REMOVED || D3DResult == DXGI_ERROR_DEVICE_HUNG || D3DResult == DXGI_ERROR_DEVICE_RESET)
	{
		...
		else
		{
			...
			// This function does not return.
			ProcessInterruptQueueOnGPUCrash(D3DResult);
		}
	}
	else
	{
...
namespace D3D12RHI
{
	...
	void VerifyD3D12CreateTextureResult(HRESULT D3DResult, const ANSICHAR* Code, const ANSICHAR* Filename, uint32 Line, const D3D12_RESOURCE_DESC& TextureDesc, ID3D12Device* Device)
	{
		...
		FD3D12DynamicRHI::GetD3DRHI()->HandleFailedD3D12Result(D3DResult, Device, true, *FString::Printf(
			TEXT("%s failed \n at %s:%u \n with error %s, \n Size=%llix%ix%i Format=%s(0x%08X), NumMips=%i, Flags=%s"),
			ANSI_TO_TCHAR(Code),
			ANSI_TO_TCHAR(Filename),
			Line,
			*ErrorString,
...
bool NeedsNvIntrinsicsSpace(const FD3D12ShaderData& ShaderData)
{
#if D3D12RHI_NEEDS_VENDOR_EXTENSIONS
	for (const FShaderCodeVendorExtension& Extension : ShaderData.VendorExtensions)
	{
...
)
{
	if (ShaderData)
	{
		...
		OutQBSS.bNeedsNvIntrinsicsSpace |= NeedsNvIntrinsicsSpace(*ShaderData);
	}
...
#if PLATFORM_SUPPORTS_MESH_SHADERS
	QuantizeBoundShaderStateCommon(QBSS, FD3D12DynamicRHI::ResourceCast(BSS.GetMeshShader()),          ResourceBindingTier, SV_Mesh, true /*bAllowUAVs*/);
	QuantizeBoundShaderStateCommon(QBSS, FD3D12DynamicRHI::ResourceCast(BSS.GetAmplificationShader()), ResourceBindingTier, SV_Amplification, true /*bAllowUAVs*/);
#endif
...
const FD3D12RootSignature* FD3D12Adapter::GetGlobalRayTracingRootSignature(const FRHIShaderBindingLayout& ShaderBindingLayout)
{
	...
	QBSS.bUseDiagnosticBuffer = true;
	// On NVIDIA, the NV intrinsics space is always needed for NVAPI shader extensions.
	// On non-NVIDIA, it's still needed when the platform compiles shaders with cluster ops support
	// (PLATFORM_SUPPORTS_RAYTRACING_CLUSTER_OPS=1), because hit group shaders will reference g_NvidiaExt behind a dynamic branch (UsingCLAS). 
	// The root signature must include this space even on AMD/Intel so the D3D12 runtime accepts the shader bytecode.
	QBSS.bNeedsNvIntrinsicsSpace = IsRHIDeviceNVIDIA() || FDataDrivenShaderPlatformInfo::GetSupportsRayTracingClusterOps(GMaxRHIShaderPlatform);
...
void LogExecuteCommandLists(uint32 NumCommandLists, ID3D12CommandList* const* ppCommandLists)
{
	for (uint32 i = 0; i < NumCommandLists; i++)
	{
		...
		UE_LOGF(LogD3D12RHI, Log, "*** [tid:%08x] EXECUTE (CmdList: %016llX) %u/%u ***", FPlatformTLS::GetCurrentThreadId(), pCurrentCommandList, i + 1, NumCommandLists);
	}
}
...
bool AssertResourceState(ID3D12CommandList* pCommandList, FD3D12Resource* pResource, const D3D12_RESOURCE_STATES& State, const FD3D12ViewSubset& ViewSubset)
{
	...
	if (!bWithD3DDebug)
	{
		UE_LOGF(LogD3D12RHI, Fatal, "*** AssertResourceState requires the debug layer ***");
		return false;
	}
...
//--------------------------------------------------------------------------------------------------
// BuildComputeShaderRootSignature — Build a serialized root signature from a UE shader blob
// Uses the same root signature construction logic as RHICreateComputeShader.
// Can be called without an initialized D3D12 RHI.
//--------------------------------------------------------------------------------------------------

bool BuildComputeShaderRootSignature(const TConstArrayView<uint8>& ShaderCode, D3D12_RESOURCE_BINDING_TIER ResourceBindingTier, TRefCountPtr<ID3DBlob>& OutRootSigBlob, int32& OutNativeBytecodeOffset)
{
	if (ShaderCode.Num() == 0)
	{
		return false;
	...
	// Parse the shader blob - same as InitStandardShader
	FShaderCodeReader CodeReader(ShaderCode);

	// Extract resource counts from optional data
	const FShaderCodePackedResourceCounts* PackedResourceCounts = CodeReader.FindOptionalData<FShaderCodePackedResourceCounts>();
	if (!PackedResourceCounts)
	{
		UE_LOGF(LogD3D12RHI, Warning, "BuildComputeShaderRootSignature: No resource counts found in shader blob");
		return false;
	}

	// Deserialize SRT to find where native bytecode starts
	FMemoryReaderView Ar(ShaderCode, true);
	FShaderResourceTable SRT;
	Ar << SRT;
	OutNativeBytecodeOffset = Ar.Tell();

	// Build quantized bound shader state - same as GetRootSignature(ComputeShader)
	FD3D12QuantizedBoundShaderState QBSS{};

	// Use a temporary FD3D12ShaderData to hold the extracted data
	// so we can call the existing quantization functions
	FD3D12ShaderData TempShaderData;
	TempShaderData.ResourceCounts = *PackedResourceCounts;

#if D3D12RHI_NEEDS_VENDOR_EXTENSIONS
	// Extract vendor extensions from optional data
	int32 VendorExtensionTableSize = 0;
	auto* VendorExtensionData = CodeReader.FindOptionalDataAndSize(FShaderCodeVendorExtension::Key, VendorExtensionTableSize);
	if (VendorExtensionData && VendorExtensionTableSize > 0)
	{
		FBufferReader VendorAr((void*)VendorExtensionData, VendorExtensionTableSize, false);
		VendorAr << TempShaderData.VendorExtensions;
	}
#endif

	// Root constants are always supported on D3D12 - force enable for tool context
	// where GRHISupportsShaderRootConstants is not initialized by device init
	GRHISupportsShaderRootConstants = true;

	// Quantize for compute: SV_All visibility, UAVs allowed
	QuantizeBoundShaderStateCommon(QBSS, &TempShaderData, ResourceBindingTier, SV_All, true /*bAllowUAVs*/);

	// Build root signature descriptor
	FD3D12RootSignatureDesc RootSigDesc(QBSS, ResourceBindingTier);
	...
	HRESULT hr = D3DX12SerializeVersionedRootSignature(
		...
		OutRootSigBlob.GetInitReference(),
		ErrorBlob.GetInitReference());

	if (FAILED(hr))
	{
		UE_LOGF(LogD3D12RHI, Error, "BuildComputeShaderRootSignature: Failed to serialize root signature: 0x%08X %ls",
			hr, ErrorBlob ? ANSI_TO_TCHAR((const char*)ErrorBlob->GetBufferPointer()) : TEXT(""));
		return false;
	}
...
//--------------------------------------------------------------------------------------------------
// BuildGraphicsShaderRootSignature -- Build a serialized root signature from multiple UE shader blobs.
// Merges resource counts across all active stages (VS, PS, GS, MS, AS).
// Can be called without an initialized D3D12 RHI.
//--------------------------------------------------------------------------------------------------

bool BuildGraphicsShaderRootSignature(const TConstArrayView<TConstArrayView<uint8>> ShaderBlobs,
	D3D12_RESOURCE_BINDING_TIER ResourceBindingTier, TRefCountPtr<ID3DBlob>& OutRootSigBlob)
{
	if (ShaderBlobs.IsEmpty())
	{
		return false;
	}

	// Merge FShaderCodePackedResourceCounts across all active shader stages.
	// The root signature must cover the union of all resources required by any stage.
	FShaderCodePackedResourceCounts MergedCounts = {};
	bool bFoundAny = false;

	for (const TConstArrayView<uint8>& ShaderCode : ShaderBlobs)
	{
		...
		const FShaderCodePackedResourceCounts* PackedCounts = CodeReader.FindOptionalData<FShaderCodePackedResourceCounts>();
		if (!PackedCounts)
		{
			// Not fatal -- some stages may not have resource counts (e.g. pass-through GS)
			continue;
		}

		// Merge by taking the max of each resource count across stages
		MergedCounts.NumSamplers = FMath::Max(MergedCounts.NumSamplers, PackedCounts->NumSamplers);
		MergedCounts.NumSRVs     = FMath::Max(MergedCounts.NumSRVs,     PackedCounts->NumSRVs);
		MergedCounts.NumCBs      = FMath::Max(MergedCounts.NumCBs,      PackedCounts->NumCBs);
		MergedCounts.NumUAVs     = FMath::Max(MergedCounts.NumUAVs,     PackedCounts->NumUAVs);
		MergedCounts.UsageFlags |= PackedCounts->UsageFlags;
		bFoundAny = true;
	}

	if (!bFoundAny)
	{
		UE_LOGF(LogD3D12RHI, Warning, "BuildGraphicsShaderRootSignature: No resource counts found in any shader stage.");
		return false;
	}

	// Build quantized bound shader state from merged resource counts -- same path as compute
	GRHISupportsShaderRootConstants = true;

	FD3D12ShaderData TempShaderData;
	TempShaderData.ResourceCounts = MergedCounts;
	// NOTE: VendorExtensions (AGS/NV intrinsics) are not yet extracted from graphics shader stages
	// and are therefore not forwarded to QuantizeBoundShaderStateCommon. Graphics PSOs that rely on
	// vendor intrinsics will get a root signature without the extra descriptor space those require.
	// This is acceptable for the current ASD tool scope -- support can be added when needed.

	FD3D12QuantizedBoundShaderState QBSS{};
	// Use SV_All visibility so the root signature covers all stages
	QuantizeBoundShaderStateCommon(QBSS, &TempShaderData, ResourceBindingTier, SV_All, true /*bAllowUAVs*/);
	...
	if (FAILED(hr))
	{
		UE_LOGF(LogD3D12RHI, Error, "BuildGraphicsShaderRootSignature: Failed to serialize root signature: 0x%08X %ls",
			hr, ErrorBlob ? ANSI_TO_TCHAR((const char*)ErrorBlob->GetBufferPointer()) : TEXT(""));
		return false;
	}
