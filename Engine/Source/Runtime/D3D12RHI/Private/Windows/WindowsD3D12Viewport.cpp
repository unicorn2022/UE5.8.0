...
void FD3D12Viewport::Init()
{
	...
	if (DXGISwapchainProvider)
	{
		...
		{
			UE_LOGF(LogD3D12RHI, Log, "Found a custom swapchain provider: '%ls'.", DXGISwapchainProvider->GetProviderName());
	...
	if (FD3D12DynamicRHI::GetD3DRHI()->IsQuadBufferStereoEnabled())
	{
		...
		{
			UE_LOGF(LogD3D12RHI, Log, "FD3D12Viewport::FD3D12Viewport was not able to create stereo SwapChain; Please enable stereo in driver settings.");
	...
	if (bNeedSwapChain)
	{
		...
		if (FAILED(hr))
		{
			UE_LOGF(LogD3D12RHI, Warning, "Failed to create swapchain with the following parameters:");
			UE_LOGF(LogD3D12RHI, Warning, "\tWidth: %d Height: %d DXGI format: %d", SwapChainDesc1.Width, SwapChainDesc1.Height, SwapChainDesc1.Format);
			UE_LOGF(LogD3D12RHI, Warning, "\tBack buffer count: %d", SwapChainDesc1.BufferCount);
			UE_LOGF(LogD3D12RHI, Warning, "\tWindows handle: 0x%llx (IsWindow: %ls)", WindowHandle, IsWindow(WindowHandle) ? TEXT("true") : TEXT("false"));
			UE_LOGF(LogD3D12RHI, Warning, "\tFullscreen: %ls", bIsFullscreen ? TEXT("true") : TEXT("false"));
			UE_LOGF(LogD3D12RHI, Warning, "\tSwapchain flags: 0x%08x", SwapChainDesc1.Flags);
			UE_LOGF(LogD3D12RHI, Warning, "\tCustom swapchain provider: %ls", DXGISwapchainProvider ? DXGISwapchainProvider->GetProviderName() : TEXT("none"));

			// DXGI does not support CreateSwapChainForHwnd on WS_EX_LAYERED windows; DWM owns the composition path for layered windows.
			// The call usually succeeds quietly on most driver/OS combos but DWM can reject it with E_ACCESSDENIED under stress (e.g. during a recursive FlushRenderingCommands triggered by another subsystem's teardown).
			// In that case gracefully degrade rather than fatal: leave SwapChain1 null and clear bNeedSwapChain so the rest of Init (and subsequent Resize / Present) falls through the existing null-swapchain code paths used by pixel streaming windowless mode (GetSwapChainSurface creates CPU-side back buffers when SwapChain is null, Present is guarded by if (SwapChain1)).
			const bool bIsLayeredWindow = IsWindow(WindowHandle) &&
				(GetWindowLongPtr(WindowHandle, GWL_EXSTYLE) & WS_EX_LAYERED) != 0;
			if (hr == E_ACCESSDENIED && bIsLayeredWindow)
			{
				UE_LOGF(LogD3D12RHI, Warning, "Window 0x%llx has WS_EX_LAYERED. DXGI does not support swapchains on layered windows; continuing without a swapchain for this viewport.", WindowHandle);
		...
		if (SwapChain1)
		{
			...
			FRHIFrameFlipStatTrackingRunnable::Create(SwapChain1);
#endif
...
void FD3D12Viewport::FinalDestroyInternal()
{
	...
	{
		FRHIFrameFlipStatTrackingRunnable::Release();
	}
#endif
}
...
#undef CASE_ERROR_NAME
					...
					UE_LOGF(LogD3D12RHI, Error, "IDXGISwapChain::SetFullscreenState returned 0x%08x, %ls.", Result, Name);
...
HRESULT FD3D12Viewport::PresentInternal(int32 SyncInterval)
{
	...
	if (SwapChain1)
	{
		...
		{
			// Assuming Next PresentID == Current PresentID + 1 , this might be not true in some cases
			GRHIPresentCounter = PresentID + 1;
		}
		else
		{
			GRHIPresentCounter++;
...
void FD3D12Viewport::ShutdownHDR()
{
	...
	{
		// Per DXGI spec, float-valued swapchains must contain linear data.
		// The swapchain color space must therefore be G10 instead of G22.
		EnsureColorSpace(DXGI_COLOR_SPACE_RGB_FULL_G10_NONE_P709);
	}
	else
	{
		// Ensure we have the correct color space set.
...
void FD3D12Viewport::EnsureColorSpace(DXGI_COLOR_SPACE_TYPE NewColorSpace)
{
	if (!SwapChain4.GetReference())
	{
		return;
	...
	if (ColorSpace != NewColorSpace)
	{
		...
		if (SUCCEEDED(hr) && ((ColorSpaceSupport & DXGI_SWAP_CHAIN_COLOR_SPACE_SUPPORT_FLAG_PRESENT) != 0))
		{
			...
			UE_LOGF(LogD3D12RHI, Verbose, "Setting color space on swap chain (%p): %ls", SwapChain4.GetReference(), *NewColorSpaceName);
			ColorSpace = NewColorSpace;
		}
		else
		{
			UE_LOGF(LogD3D12RHI, Error, "Warning: unable to set color space %ls to the swapchain: verify EDisplayOutputFormat / swapchain format", *NewColorSpaceName);
		}
	}
}
...
void FD3D12Viewport::EnsureColorSpace(EDisplayColorGamut DisplayGamut, EDisplayOutputFormat OutputDevice)
{
	...
	EnsureColorSpace(NewColorSpace);
}
