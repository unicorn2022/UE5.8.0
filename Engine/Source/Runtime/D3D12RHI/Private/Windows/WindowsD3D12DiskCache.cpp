...
void FDiskCacheInterface::Init(FString &filename, bool bEnable)
{
	...
	if (fileFound && mFileStart)
	{
		...
		{
			UE_LOGF(LogD3D12RHI, Warning, "Disk cache is stale. Disk Cache version: %d App version: %d", mHeader.mHeaderVersion, mCurrentHeaderVersion);
...
#endif
	UE_LOGF(LogD3D12RHI, Warning, "Deleted PSO Cache with result %d", result);
}
