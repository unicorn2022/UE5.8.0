...
void FD3D12PipelineStateCache::RebuildFromDiskCache()
{
	...
	UE_LOGF(LogD3D12RHI, Log, "Reading %u Graphics PSO(s) from the disk cache.", NumGraphicsPSOs);
	for (uint32 i = 0; i < NumGraphicsPSOs; i++)
	{
		...
		{
			UE_LOGF(LogD3D12RHI, Warning, "PSO Cache read error!");
			break;
		}
	}

	const uint32 NumComputePSOs = DiskCaches[PSO_CACHE_COMPUTE].GetNumPSOs();
	UE_LOGF(LogD3D12RHI, Log, "Reading %u Compute PSO(s) from the disk cache.", NumComputePSOs);
	...
	if (!DiskCache.IsInErrorState())
	{
		...
		if (PsoDesc.InputLayout.NumElements)
		{
			...
			for (uint32 i = 0; i < PsoDesc.InputLayout.NumElements; i++)
			{
				...
				uint32 stringLength = (uint32)FPlatformString::Strnlen(PsoDesc.InputLayout.pInputElementDescs[i].SemanticName, IL_MAX_SEMANTIC_NAME);
...
void FD3D12PipelineStateCache::Close()
{
	...
	if (UsePipelineLibrary() && bOverwriteExistingPipelineLibrary)
	{
		...
		if (LibrarySize)
		{
			...
			UE_LOGF(LogD3D12RHI, Log, "Serializing Pipeline Library to disk (%llu KiB).", LibrarySize / 1024ll);
			...
			UE_CLOGF(!bSuccess, LogD3D12RHI, Warning, "Failed to write Pipeline Library to disk.");
		}
	}
...
void FD3D12PipelineStateCache::Init(FString& GraphicsCacheFileName, FString& ComputeCacheFileName, FString& DriverBlobFileName)
{
	...
	UE_CLOGF(!bEnableGeneralPipelineStateDiskCaches, LogD3D12RHI, Display, "Not using pipeline state disk cache per r.D3D12.PSO.DiskCache=0");
	
	const bool bEnableDriverOptimizedPipelineStateDiskCaches = CVarDriverOptimizedPipelineStateDiskCache.GetValueOnAnyThread() != 0;
	UE_CLOGF(!bEnableDriverOptimizedPipelineStateDiskCaches, LogD3D12RHI, Display, "Not using driver-optimized pipeline state disk cache per r.D3D12.PSO.DriverOptimizedDiskCache=0");
	...
	if (bUseAPILibaries)
	{
		...
		if (pDevice1)
		{
			...
			if (pLibraryBlob)
			{
				UE_LOGF(LogD3D12RHI, Log, "Creating Pipeline Library from existing disk cache (%llu KiB).", LibrarySize / 1024ll);
			}
			else
			{
				UE_LOGF(LogD3D12RHI, Log, "Creating new Pipeline Library.");
			}
			...
			{
				UE_LOGF(LogD3D12RHI, Log, "The driver doesn't support Pipeline Libraries.");
			}
			else if (FAILED(HResult))
			{
				UE_LOGF(LogD3D12RHI, Log, "Create Pipeline Library failed. Perhaps the Library has stale PSOs for the current HW or driver. Clearing the disk cache and trying again...");
...
static void DumpShaderAsm(FString& String, const D3D12_SHADER_BYTECODE& Shader)
{
	...
	if (Shader.pShaderBytecode)
	{
		...
		if (D3DDisasmFunc)
		{
			...
			{
				String.Appendf(TEXT("%.*hs\n"), uint32(blob->GetBufferSize()), static_cast<const char*>(blob->GetBufferPointer()));
				blob->Release();
			}
		}
	}
...
static void DumpGraphicsPSO(const FD3D12_GRAPHICS_PIPELINE_STATE_DESC& Desc, const TCHAR* Name)
{
	...
	UE_LOGF(LogD3D12RHI, Warning, "Failed to create graphics PSO with combined hash 0x%ls:\n%ls", Name, *String);
}
...
static void DumpComputePSO(const FD3D12_COMPUTE_PIPELINE_STATE_DESC& Desc, const TCHAR* Name)
{
	...
	UE_LOGF(LogD3D12RHI, Warning, "Failed to create compute PSO with combined hash 0x%ls:\n%ls", Name, *String);
}
...
static HRESULT CreatePipelineStateFromStream(ID3D12PipelineState*& PSO, ID3D12Device2* Device, const D3D12_PIPELINE_STATE_STREAM_DESC* Desc, ID3D12PipelineLibrary1* Library, const TCHAR* Name)
{
	...
	else
	{
		...
		if (FAILED(hr))
		{
			UE_LOGF(LogD3D12RHI, Error, "Failed to create pipeline state with combined hash %ls, error %x.", Name, hr);
		}
	}
...
#else
	ShdExtensionDesc.registerSpace = UE_HLSL_SPACE_NV_SHADER_EXTN;
#endif
...
static void CreatePipelineStateWithExtensions(ID3D12PipelineState** PSO, FD3D12Adapter* Adapter, const TCreationArgs* CreationArgs, TArrayView<const FShaderCodeVendorExtension> VendorExtensions)
{
	...
	{
		// Optional vendor extensions may be present in the shader on GPUs from a different vendor, in which case we skip vendor specific setup.
		if (Extension.bIsOptional && Extension.VendorId != GetRHIDeviceVendorId())
		{
			continue;
		}
	...
	// All vendor extensions were optional and skipped (vendor mismatch). Create PSO normally.
	CreatePipelineStateWrapper(PSO, Adapter, CreationArgs);
}
#endif
