...
int32 GD3D12RequestExitOnGPUTimeout = 0;
static FAutoConsoleVariableRef CVarD3D12RequestExitOnGPUTimeout(
	TEXT("D3D12.RequestExitOnGPUTimeout"),
	GD3D12RequestExitOnGPUTimeout,
	TEXT("Request exit on GPU timeouts, useful for external crash handlers"));
...
FD3D12DynamicRHI::FD3D12DynamicRHI(TSharedPtr<FD3D12Adapter>& InChosenAdapter, bool bInPixEventEnabled)
	...
	, FlipEvent(INVALID_HANDLE_VALUE)
{
	...
	GRHIGlobals.ParallelRenderPasses.Supported = true;
...
#if PLATFORM_WINDOWS
	InitializeDeviceAgnosticVendorAPI();
#endif
}
...
{
	UE_LOGF(LogD3D12RHI, Log, "~FD3D12DynamicRHI");
...
void FD3D12DynamicRHI::ForEachQueue(TFunctionRef<void(FD3D12Queue&)> Callback)
{
	if (FD3D12Adapter* const Adapter = ChosenAdapter.Get())
	{
		for (FD3D12Device* const Device : Adapter->GetDevices())
		{
...
void FD3D12DynamicRHI::Shutdown()
{
	...
	if (RayTracingValidationCallbackHandle)
	{		
		const NvAPI_Status NvStatus = NvAPI_D3D12_UnregisterRaytracingValidationMessageCallback(GetAdapter().GetD3DDevice5(), RayTracingValidationCallbackHandle);
		check(NvStatus == NVAPI_OK);
		RayTracingValidationCallbackHandle = nullptr;
	}
#endif
...
void FD3D12DynamicRHI::RHIGetMemoryStats(FRHIMemoryStats& OutStats)
{
...
void FD3D12DynamicRHI::EnqueueEndOfPipeTask(TUniqueFunction<void()> TaskFunc, TUniqueFunction<void(FD3D12Payload&)> ModifyPayloadCallback)
{
	...
	ForEachQueue([&](FD3D12Queue& Queue)
	{
		...
		FD3D12SyncPointRef SyncPoint = FD3D12SyncPoint::Create(ED3D12SyncPointType::GPUAndCPU, GetD3DCommandQueueTypeFName(Queue.QueueType));
		...
		EnqueueEndOfPipeTask([Array = MoveTemp(Local)]()
		{
			...
			for (FD3D12DeferredDeleteObject const& ObjectToDelete : Array)
			{
				...
				case FD3D12DeferredDeleteObject::EType::DescriptorHeap:
					ObjectToDelete.DescriptorHeap->Release();
					break;
				case FD3D12DeferredDeleteObject::EType::D3DObject:
					ObjectToDelete.D3DObject->Release();
					break;
#if PLATFORM_SUPPORTS_BINDLESS_RENDERING
				case FD3D12DeferredDeleteObject::EType::DescriptorAllocation:
					ObjectToDelete.DescriptorAllocation.Device->GetBindlessDescriptorAllocator().FreeDescriptors(
						ObjectToDelete.DescriptorAllocation.Allocation,
						ObjectToDelete.DescriptorAllocation.TypeMask
					);
					break;
		...
		[this](FD3D12ContextArray const& Contexts)
	{
		if (FD3D12Adapter* const Adapter = ChosenAdapter.Get())
		{
			for (FD3D12Device* const Device : Adapter->GetDevices())
			{
				...
				FD3D12RayTracingCompactionRequestHandler* const RayTracingCompactionRequestHandler = Device->GetRayTracingCompactionRequestHandler();
				if (RayTracingCompactionRequestHandler)
				{
					RayTracingCompactionRequestHandler->Update(*Contexts[ERHIPipeline::Graphics]->GetSingleDeviceContext(Device->GetGPUIndex()));
				}
			#endif
			}
		}
...
#if WITH_NVAPI_RAYTRACING_VALIDATION
void FD3D12DynamicRHI::FlushRaytracingValidationMessages()
{
	if (RayTracingValidationCallbackHandle)
	{
		const NvAPI_Status NvStatus = NvAPI_D3D12_FlushRaytracingValidationMessages(GetAdapter().GetD3DDevice5());
		ensure(NvStatus == NVAPI_OK);
	}
}
#endif
...
void FD3D12DynamicRHI::RHIEndFrame(const FRHIEndFrameArgs& Args)
{
	...
	if (RayTracingValidationCallbackHandle)
	{
		EnqueueEndOfPipeTask([this]() { FlushRaytracingValidationMessages(); });
	}
#endif
...
ID3D12Device* FD3D12DynamicRHI::RHIGetDevice_NoMGPU() const
{
	ensureMsgf(GNumExplicitGPUsForRendering == 1,
		TEXT("Use of RHIGetDevice_NoMGPU() while multi-GPU is enabled. ")
		TEXT("The calling code may not be handling multi-GPU scenarios correctly. ")
		TEXT("Switch to using RHIGetDevice(N) where N is the GPU index if the calling code supports MGPU.")
	);
	return GetAdapter().GetDevice(0)->GetDevice();
}
...
FRHIViewport* FD3D12DynamicRHI::RHIGetSingletonViewport() const
{
	return ChosenAdapter->GetViewports().Num() == 1
		? ChosenAdapter->GetViewports()[0]
		: nullptr;
}
...
void FD3D12DynamicRHI::RHIFinishExternalComputeWork(FRHICommandListBase& ExecutingCmdList, uint32 InDeviceIndex, ID3D12GraphicsCommandList* InCommandList)
{
	...
	ED3D12SetDescriptorHeapsFlags SetHeapFlags = ED3D12SetDescriptorHeapsFlags::ForceChanged;
#if PLATFORM_SUPPORTS_BINDLESS_RENDERING
	// Make sure we restore the correct heaps
	if (Context.StateCache.GetDescriptorCache()->IsUsingBindlessHeap())
	{
		EnumAddFlags(SetHeapFlags, ED3D12SetDescriptorHeapsFlags::Bindless);
	}
#endif

	Context.StateCache.GetDescriptorCache()->SetDescriptorHeaps(SetHeapFlags);
}
...
{
	UE_LOGF(LogD3D12RHI, Error, "RHITransitionResource is no longer functional. Please use RHICmdList.Transition() instead.");
}
...
void FD3D12DynamicRHI::RHIWaitManualFence(FRHICommandList& RHICmdList, ID3D12Fence* Fence, uint64 Value, const TSharedPtr<std::atomic<bool>>& bHasSignalled)
{
	checkf(FRHIGPUMask::All() == FRHIGPUMask::GPU0(), TEXT("RHIWaitManualFence cannot be used by multi-GPU code"));
	const uint32 GPUIndex = 0;

	FD3D12CommandContext& Context = FD3D12CommandContext::Get(RHICmdList, GPUIndex);
	Context.WaitManualFence(Fence, Value, bHasSignalled);
}
...
void FD3D12DynamicRHI::RHIGetSupportedResolution(uint32& Width, uint32& Height)
{
...

	{
		...
		for (uint32 o = 0; o < 1; o++)
		{
			...
			{
				UE_LOGF(LogD3D12RHI, Fatal,
					"This application cannot be run over a remote desktop configuration"
					);
				return;
			}
...
{
	UE_LOGF(LogD3D12RHI, Warning, "GPU timeout: A payload (0x%p) on the [0x%p, %ls] queue has not completed after %f seconds."
		, Payload
		, &Payload->Queue
	...
	if (GD3D12RequestExitOnGPUTimeout)
	{
		GIsCriticalError = true;
		FPlatformMisc::RequestExit(true, TEXT("FD3D12DynamicRHI::HandleGpuTimeout"));
	}
}
