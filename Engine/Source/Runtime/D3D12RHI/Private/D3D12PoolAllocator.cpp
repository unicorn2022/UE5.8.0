...
void FD3D12MemoryPool::Init()
{
		...
		&& EnumHasAnyFlags(InitConfig.InitialD3D12Access, ED3D12Access::BVHRead | ED3D12Access::BVHWrite))
	{
		for (uint32 GPUIndex : GetGPUMask())
		{
			...
			const D3D12_RESIDENCY_PRIORITY HeapPriority = D3D12_RESIDENCY_PRIORITY_HIGH;
			Adapter->SetResidencyPriority(HeapResource, HeapPriority, GPUIndex, PoolSize);
		}
	}
...
FD3D12PoolAllocator::FD3D12PoolAllocator(FD3D12Device* ParentDevice, FRHIGPUMask VisibleNodes, const FD3D12ResourceInitConfig& InInitConfig, const FString& InName,
	EResourceAllocationStrategy InAllocationStrategy, uint64 InDefaultPoolSize, uint32 InPoolAlignment, uint32 InMaxAllocationSize, FRHIMemoryPool::EFreeListOrder InFreeListOrder, bool bInDefragEnabled, HeapId InTraceParentHeapId, ERHIPoolAllocationStrategy InPoolAllocationStrategy, uint16 InOverFlowPoolsCount) :
	FRHIPoolAllocator(InDefaultPoolSize, InPoolAlignment, InMaxAllocationSize, InFreeListOrder, bInDefragEnabled, InPoolAllocationStrategy, InOverFlowPoolsCount),
	FD3D12DeviceChild(ParentDevice), 
	FD3D12MultiNodeGPUObject(ParentDevice->GetGPUMask(), VisibleNodes), 
	InitConfig(InInitConfig), 
	Name(InName), 
	...
	FD3D12ResourceLocation& ResourceLocation)
{
		...
		&& ResourceLocation.GetAllocatorType() != FD3D12ResourceLocation::AT_Pool)
	{
		...
		Adapter->SetResidencyPriority(HeapResource, D3D12_RESIDENCY_PRIORITY_HIGH, GPUIndex, InSize);
	}
#endif // D3D12_RHI_RAYTRACING
}
...
void FD3D12PoolAllocator::TransferOwnership(FD3D12ResourceLocation& InSource, FD3D12ResourceLocation& InDest)
{
	...
	// Note: mirrors FD3D12ResourceLocation::TransferOwnership but is inside our CS.
	FMemory::Memmove(&InDest, &InSource, sizeof(FD3D12ResourceLocation));
