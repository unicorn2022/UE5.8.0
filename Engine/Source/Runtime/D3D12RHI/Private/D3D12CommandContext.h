...
#if PLATFORM_SUPPORTS_BINDLESS_RENDERING
	explicit FD3D12DeferredDeleteObject(FRHIDescriptorAllocation Allocation, ERHIDescriptorTypeMask TypeMask, FD3D12Device* Device)
		: Type(EType::DescriptorAllocation)
		, DescriptorAllocation({ Allocation, TypeMask, Device })
	{}
...
public:
	...
	void WaitManualFence(ID3D12Fence* Fence, uint64 Value, const TSharedPtr<std::atomic<bool>>& bHasSignalled);
	...
	FD3D12SyncPoint* GetContextSyncPoint()
	{
		if (!ContextSyncPoint)
		{
			ContextSyncPoint = FD3D12SyncPoint::Create(ED3D12SyncPointType::GPUAndCPU, UE_D3D12_STATIC_FNAME("ContextSyncPoint"));
...
public:
	...
	void SetPresent(FD3D12PresentArgs& PresentArgs);
...
public:
	...
	virtual void RHIDispatchComputeShaderBundle(
		...
		ERHIShaderBundleMethod Method) override;
	...
	virtual void RHIEndRenderPass()
	{
		...
		// Buffers used as vertex streams in this pass can be transitioned to other states after the pass ends. If such a buffer
		// was bound to a slot that's not used in a subsequent draw, the cache will apply the stale binding, resulting in validation
		// layer errors if the new state doesn't include D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER (despite the shader not actually
		// using the buffer - the validation layer can't know that). We clear the VB cache to prevent this case.
		StateCache.ClearVertexBuffers();
	}
...
#if WITH_NVAPI
	virtual void BuildAccelerationStructuresInternalEx(const TArrayView<NVAPI_D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_DESC_EX> ParamsArray);
#endif
#if WITH_MGPU
