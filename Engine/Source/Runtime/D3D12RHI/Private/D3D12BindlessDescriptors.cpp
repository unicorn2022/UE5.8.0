...
int32 GBindlessHeapUpdateUniqueUploadThreshold = 1024;
static FAutoConsoleVariableRef CVarBindlessHeapUpdateUniqueUploadThreshold(
	TEXT("D3D12.Bindless.HeapUpdateUniqueUploadThreshold"),
	GBindlessHeapUpdateUniqueUploadThreshold,
	TEXT("Threshold of upload index count before we make the list of update indices unique."),
	ECVF_Default
);
...
void FD3D12BindlessSamplerManager::InitializeDescriptor(FRHIDescriptorHandle DstHandle, const FD3D12SamplerState* SamplerState)
{
...
void FD3D12BindlessDescriptorAllocator::Init()
{
	...
	BindlessConfiguration = UE::RHICore::GetBindlessConfigurationOnStartup(GMaxRHIShaderPlatform);
	...
	if (BindlessConfiguration != ERHIBindlessConfiguration::Disabled)
	{
		...
		{
			UE_LOGF(LogD3D12RHI, Error, "D3D12.Bindless.SamplerDescriptorHeapSize was set to %d, which is higher than the D3D12 maximum of %d. Adjusting the value to prevent a crash.",
		...
		UE_LOGF(LogD3D12RHI, Log, "Bindless is enabled (Configuration=%ls). Initializing allocators with %d Resource and %d Sampler descriptors", GetBindlessConfigurationString(BindlessConfiguration), NumResourceDescriptors, NumSamplerDescriptors);
		...
		GRHIGlobals.DescriptorTypeGroups.Emplace(ResourceAllocator->GetTypeMask());
		GRHIGlobals.DescriptorTypeGroups.Emplace(SamplerAllocator->GetTypeMask());
	}
}
...
TOptional<FRHIDescriptorAllocation> FD3D12BindlessDescriptorAllocator::AllocateDescriptors(ERHIDescriptorTypeMask DescriptorTypeMask, uint32 DescriptorCount)
{
	if (SamplerAllocator && SamplerAllocator->HandlesAllocations(DescriptorTypeMask))
	{
		check(!EnumHasAnyFlags(DescriptorTypeMask, ERHIDescriptorTypeMask::Standard));
		return SamplerAllocator->Allocate(DescriptorCount);
	}

	if (ResourceAllocator && ResourceAllocator->HandlesAllocations(DescriptorTypeMask))
	{
		check(!EnumHasAnyFlags(DescriptorTypeMask, ERHIDescriptorTypeMask::Sampler));
...
TOptional<FRHIDescriptorAllocation> FD3D12BindlessDescriptorAllocator::ResizeGrowAndAllocate(uint32 NumAllocations)
{
	...
	{
		UE_LOGF(LogD3D12RHI, Fatal, "Hit D3D12 device limits on descriptors when attempting to allocate larger descriptor heap of size %d.", NewNumDescriptors);
	}
...
void FD3D12BindlessDescriptorAllocator::FreeDescriptors(FRHIDescriptorAllocation Allocation, ERHIDescriptorTypeMask TypeMask)
{
	if (SamplerAllocator && SamplerAllocator->HandlesAllocations(TypeMask))
	{
		SamplerAllocator->Free(Allocation);
	}
	else if (ResourceAllocator && ResourceAllocator->HandlesAllocations(TypeMask))
	{
		ResourceAllocator->Free(Allocation);
	}
	else
	{
		// bad configuration?
...
void FD3D12BindlessResourceManager::InitializeDescriptor(FRHIDescriptorHandle DstHandle, const FD3D12View* View)
{
	if (DstHandle.IsValid())
	{
		...
		if (ActiveGpuHeapIndex >= 0 && !bCPUHeapResized)
		{
			...
			ActiveGpuHeaps[ActiveGpuHeapIndex].UpdatedIndices.Add(DstHandle.GetIndex());
		}
...
void FD3D12BindlessResourceManager::UpdateDescriptor(FD3D12ContextArray const& Contexts, FRHIDescriptorHandle DstHandle, const FD3D12View* View)
{
	if (DstHandle.IsValid())
	{
...
void FD3D12BindlessResourceManager::CheckRequestNewActiveGPUHeap()
{
	...
	else
	{
		...
		for (int32 GpuHeapIndex = 0; GpuHeapIndex < NumGpuHeaps; ++GpuHeapIndex)
		{
			...
			{
				ActiveGpuHeaps[GpuHeapIndex].UpdatedIndices.Append(ActiveGpuHeaps[ActiveGpuHeapIndex].UpdatedIndices);
			}
		}
	...
	{
		// NOTE: For small update lists, copying duplicates is faster than deduplicating.
		//       For large lists (see GBindlessHeapUpdateUniqueUploadThreshold), we sort + unique
		//       to avoid excessive driver overhead from large CopyDescriptors calls.

		FGpuHeapData& GpuHeapData = ActiveGpuHeaps[NewActiveGpuHeapIndex];
		INC_DWORD_STAT_BY(STAT_D3D12BindlessResourceGPUDescriptorsCopied, GpuHeapData.UpdatedIndices.Num());

		// Deduplicate the updated indices when the list is large enough that sorting + unique
		// is cheaper than copying redundant descriptors. The first condition catches pathological
		// cases where accumulated updates exceed the heap size (guaranteed duplicates).
		if (GpuHeapData.UpdatedIndices.Num() > (int32)GpuHeapData.GpuHeap->GetNumDescriptors() ||
			GpuHeapData.UpdatedIndices.Num() > GBindlessHeapUpdateUniqueUploadThreshold)
		{
			Algo::Sort(GpuHeapData.UpdatedIndices);
			GpuHeapData.UpdatedIndices.SetNum(Algo::Unique(GpuHeapData.UpdatedIndices));
		}

		UE::D3D12Descriptors::CopyDescriptors(GetParentDevice(), GpuHeapData.GpuHeap, CpuHeap, GpuHeapData.UpdatedIndices);
		GpuHeapData.UpdatedIndices.Reset();
...
void FD3D12BindlessDescriptorManager::InitializeDescriptor(FRHIDescriptorHandle DstHandle, const FD3D12SamplerState* SamplerState)
{
	if (SamplerManager)
	{
...
void FD3D12BindlessDescriptorManager::InitializeDescriptor(FRHIDescriptorHandle DstHandle, const FD3D12View* View)
{
...
void FD3D12BindlessDescriptorManager::UpdateDescriptor(FD3D12ContextArray const& Contexts, FRHIDescriptorHandle DstHandle, const FD3D12View* View)
{
	if (ResourceManager)
	{
