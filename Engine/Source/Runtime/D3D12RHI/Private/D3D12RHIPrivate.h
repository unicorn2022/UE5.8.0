...
private:
	...
	void TerminateOnGPUCrash(TOptional<HRESULT> D3DResult);
	...
	void FlushRaytracingValidationMessages();
#endif
...
public:
	...
	void ProcessInterruptQueueOnGPUCrash(HRESULT D3DResult);
	...
	FD3D12DynamicRHI(TSharedPtr<FD3D12Adapter>& InChosenAdapter, bool bInPixEventEnabled);
	...
	template<typename TRHIType, typename TReturnType = typename TD3D12ResourceTraits<TRHIType>::TConcreteType>
	static FORCEINLINE_DEBUGGABLE const TReturnType* ResourceCast(const TRHIType* Resource, uint32 GPUIndex)
	{
		const TReturnType* Object = ResourceCast<const TRHIType, const TReturnType>(Resource);
		return Object ? static_cast<const TReturnType*>(Object->GetLinkedObject(GPUIndex)) : nullptr;
	}
	...
	virtual FPixelShaderRHIRef RHICreatePixelShader(const FRHICreateShaderDesc& CreateShaderDesc) final override;
	virtual FVertexShaderRHIRef RHICreateVertexShader(const FRHICreateShaderDesc& CreateShaderDesc) final override;
	virtual FMeshShaderRHIRef RHICreateMeshShader(const FRHICreateShaderDesc& CreateShaderDesc) final override;
	virtual FAmplificationShaderRHIRef RHICreateAmplificationShader(const FRHICreateShaderDesc& CreateShaderDesc) final override;
	virtual FGeometryShaderRHIRef RHICreateGeometryShader(const FRHICreateShaderDesc& CreateShaderDesc) final override;
	virtual FComputeShaderRHIRef RHICreateComputeShader(const FRHICreateShaderDesc& CreateShaderDesc) override;
	virtual FWorkGraphShaderRHIRef RHICreateWorkGraphShader(const FRHICreateShaderDesc& CreateShaderDesc, EShaderFrequency ShaderFrequency) override;
	...
	virtual void RHIWriteResourceGPUAddresses(FRHICommandListBase& RHICmdList, uint32 GPUIndex, TConstArrayView<FRHIResource*> Resources, uint64* AddressesPtr) final override;
	...
	virtual void RHIGetMemoryStats(FRHIMemoryStats& OutStats) final override;
	...
	virtual FViewportRHIRef RHICreateViewport(void* WindowHandle, uint32 SizeX, uint32 SizeY, bool bIsFullscreen, EPixelFormat PixelFormat) final override;
	virtual void RHIResizeViewport(FRHIViewport* ViewportRHI, uint32 SizeX, uint32 SizeY, bool bIsFullscreen, EPixelFormat PixelFormat) final override;
	virtual void RHIEndDrawingViewport(FRHICommandListImmediate& RHICmdList, FRHIViewport* Viewport, const FRHIPresentArgs& PresentArgs) final override;
#if PLATFORM_WINDOWS
	...
	virtual bool RHIGetAvailableResolutionsForDisplay(FScreenResolutionArray& Resolutions, bool bIgnoreRefreshRate, void* NativeMonitorHandle) final override;
	...
	virtual IRHIComputeContext* RHIGetCommandContext(ERHIPipeline Pipeline) final override;
	virtual FRHIFinalizeContextsResult RHIFinalizeContexts(FRHIFinalizeContextsArgs&& Args) final override;
	...
	virtual FDescriptorRangeRHIRef RHICreateDescriptorRange(FRHICommandListBase& RHICmdList, const FRHIDescriptorRangeDesc& Desc, TConstArrayView<FRHIDescriptorResource> InitialResources) override;
	virtual void RHIUpdateDescriptorRange(FRHICommandListBase& RHICmdList, FRHIDescriptorRange* Range, uint32 StartIndex, TConstArrayView<FRHIDescriptorResource> Resources) override;
#endif
	...
	virtual ID3D12Device* RHIGetDevice_NoMGPU() const final override;
	...
	virtual FRHIViewport* RHIGetSingletonViewport() const final override;
	...
	virtual void RHIWaitManualFence(FRHICommandList& RHICmdList, ID3D12Fence* Fence, uint64 Value, const TSharedPtr<std::atomic<bool>>& bHasSignalled) final override;
	...
	virtual FRayTracingShaderRHIRef RHICreateRayTracingShader(const FRHICreateShaderDesc& CreateShaderDesc, EShaderFrequency ShaderFrequency) final override;
...
public:
	...
	FD3D12Adapter& GetAdapter() { return *ChosenAdapter; }
	const FD3D12Adapter& GetAdapter() const { return *ChosenAdapter; }

	bool HasAdapter() const { return ChosenAdapter.IsValid(); }
...
protected:
	...
	TSharedPtr<FD3D12Adapter> ChosenAdapter;
...
#if WITH_NVAPI_RAYTRACING_VALIDATION
	void* RayTracingValidationCallbackHandle = nullptr;
#endif
	...
	TRefCountPtr<IDXGIFactory2> DXGIFactoryForResolutions;
#endif
...
#if PLATFORM_WINDOWS
	void InitializeDeviceAgnosticVendorAPI();
#endif
};
...
public:
	...
	~FD3D12DynamicRHIModule() = default;
