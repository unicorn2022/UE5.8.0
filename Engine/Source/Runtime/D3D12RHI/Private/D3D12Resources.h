...
class FD3D12Heap : public FRefCountedObject, public FD3D12DeviceChild, public FD3D12MultiNodeGPUObject
{
public:
...
class FD3D12Resource : public UE::Private::FQueryableRefCountedObject, public FD3D12DeviceChild, public FD3D12MultiNodeGPUObject
{
...
struct ID3D12UniformBufferUpdateListener
{
	...
	virtual void UniformBufferUpdated(const FD3D12ContextArray& Contexts, FD3D12UniformBuffer* InUpdatedUniformBuffer) = 0;
};
...
public:
	...
	void RemoveUpdateListener(ID3D12UniformBufferUpdateListener* InUpdateListener)
	{
		...
		// During dtor cleanup the Remove can be called on the same element multiple times
		UpdateListeners.Remove(InUpdateListener);
	}
	...
	void UniformBufferUpdated(const FD3D12ContextArray& Contexts)
	{
		FScopeLock Lock(&UpdateListenersCS);
		for (ID3D12UniformBufferUpdateListener* UpdateListener : UpdateListeners)
		{
			UpdateListener->UniformBufferUpdated(Contexts, this);
		}
	}
};
