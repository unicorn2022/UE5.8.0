...
void FPreAnimatedStateExtension::RemoveMetaData(const FPreAnimatedStateMetaData& MetaData)
{
	...
	if (MetaData.bWantsRestoreState)
	{
		if (--Aggregate->NumRestoreContributors == 0)
		{
			EPreAnimatedStorageRequirement NewRequirement = EPreAnimatedStorageRequirement::None;

			FRestoreStateParams Params = {Linker, MetaData.RootInstanceHandle};
			FPreAnimatedStorageIndex StorageIndex = MetaData.Entry.ValueHandle.StorageIndex;
			TSharedPtr<IPreAnimatedStorage> Storage = GetStorageChecked(MetaData.Entry.ValueHandle.TypeID);
			if (TotalNum == 0)
			{
				// We are completely done with the value (there are no more contributors). Restore the pre-animated 
				// value and discard it.
				EPreAnimatedStorageRequirement SourceRequirement = EPreAnimatedStorageRequirement::Transient;
				EPreAnimatedStorageRequirement TargetRequirement = EPreAnimatedStorageRequirement::None;
				NewRequirement = Storage->RestorePreAnimatedStateStorage(StorageIndex, SourceRequirement, TargetRequirement, Params);
			}
			else
			{
				// We still have contributors animating the value, but they're all set to KeepState.
				// At this point we can discard the pre-animated value, we don't need it anymore, unless we want to
				// later restore global state, in which case we keep that value in persistent storage.
				EPreAnimatedStorageRequirement SourceRequirement = IsCapturingGlobalState() ?
					EPreAnimatedStorageRequirement::Transient : EPreAnimatedStorageRequirement::Persistent;
				NewRequirement = Storage->DiscardPreAnimatedStateStorage(StorageIndex, SourceRequirement);
			}
