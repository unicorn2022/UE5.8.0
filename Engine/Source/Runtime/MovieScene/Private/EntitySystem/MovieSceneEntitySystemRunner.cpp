...
void FMovieSceneEntitySystemRunner::QueueUpdate(const FMovieSceneContext& Context, FInstanceHandle Instance, UE::MovieScene::ERunnerUpdateFlags Flags)
{
	...
	if (LogMovieSceneECS.GetVerbosity() >= ELogVerbosity::VeryVerbose)
	{
		...
		UE_LOGF(LogMovieSceneECS, VeryVerbose, "Queue update: Instance(%d,%d); Status='%ls'; Range=%ls; Sequence='%ls'",
...
bool FMovieSceneEntitySystemRunner::QueueFinalUpdateImpl(FInstanceHandle InInstanceHandle, FSimpleDelegate&& InOnLastFlushDelegate, bool bDestroyInstance)
{
	...
	if (LogMovieSceneECS.GetVerbosity() >= ELogVerbosity::VeryVerbose)
	{
		...
		UE_LOGF(LogMovieSceneECS, VeryVerbose, "Queue update: Instance(%d,%d); %ls; Sequence='%ls'",
...
void FMovieSceneEntitySystemRunner::FlushOutstanding(double BudgetMs, UE::MovieScene::ERunnerFlushState TargetState)
{
	...
	{
		UE_LOGF(LogMovieSceneECS, Warning, "Cannot flush this runner while it is already being flushed outside of a re-entrancy window");
		return;
	}
	...
	// Fire PreFlush before any phases run, allowing systems to perform
	// work that must happen outside the ECS pipeline (e.g. rebuilding
	// caches that require re-entrant evaluation).
	Linker->Events.PreFlush.Broadcast(Linker);
	...
	UE_LOGF(LogMovieSceneECS, VeryVerbose, "Flushing ECS runner state '0x%#08x' with budget %fms", EnumToUnderlyingType(FlushState), BudgetMs);
...
UE::MovieScene::ERunnerFlushResult FMovieSceneEntitySystemRunner::GameThread_UpdateSequenceInstances(UMovieSceneEntitySystemLinker* Linker)
{
	...
	{
		UE_LOGF(LogMovieSceneECS, VeryVerbose, "Executing %d dissected updates", DissectedUpdates.Num());
		...
		for (int32 Index = 0; Index < DissectedUpdates.Num(); ++Index)
		{
			...
			UE_LOGF(LogMovieSceneECS, VeryVerbose,
					"[%d] Order='%ls'; Instance(%d,%d); Status='%ls'; Range=%ls; OnFlushed='%ls'; Flags='%#03x', Sequence='%ls'",
					Index,
					...
					*LexToString(DissectedUpdate.Context.GetRange()),
					...
					EnumToUnderlyingType(DissectedUpdate.UpdateFlags),
					*GetNameSafe(Sequence));
		}
	}
#endif
	...
	{
		UE_LOGF(LogMovieSceneECS, VeryVerbose, "Running evaluation for %d current instances to update", CurrentInstances.Num());
		...
		for (int32 Index = 0; Index < CurrentInstances.Num(); ++Index)
		{
			...
			UE_LOGF(LogMovieSceneECS, VeryVerbose,
					"[%d] Instance(%d,%d); Status='%ls'; Range=%ls; Flags='%#03x', Sequence='%ls'",
					Index,
					...
					*LexToString(Instance.GetContext().GetRange()),
					EnumToUnderlyingType(UpdateParams.UpdateFlags),
					*GetNameSafe(Sequence));
		}
	}
#endif
