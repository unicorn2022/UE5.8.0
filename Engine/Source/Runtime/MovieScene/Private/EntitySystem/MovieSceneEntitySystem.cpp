...
struct FSystemDependencyGraph
{
	...
	{
		const bool bSubsequentEdgeAdded = ImplicitSubsequents.MakeEdge(UpstreamGraphID, DownstreamGraphID);
		const bool bPrerequisiteEdgeAdded = ImplicitPrerequisites.MakeEdge(DownstreamGraphID, UpstreamGraphID);
		if (bSubsequentEdgeAdded || bPrerequisiteEdgeAdded)
		{
			++SerialNumber;
		}
	}
...
void UMovieSceneEntitySystem::StaticInitialize()
{
	using namespace UE::MovieScene;

	// Pre-emptively call UpdateCache() on the global dependency graph to prevent doing it at a critical time.
	// In the event that a plugin/module that mutates this graph loads after this initialization, 
	// the cache will be invalidated and will be updated when the first sequence is played.
	GlobalDependencyGraph.UpdateCache();
}
...
void UMovieSceneEntitySystem::Run(FSystemTaskPrerequisites& InPrerequisites, FSystemSubsequentTasks& Subsequents)
{
	...
	UE_LOGF(LogMovieSceneECS, Verbose, "Running moviescene system for phase %d: %ls", (int32)Phase, *GetName());
