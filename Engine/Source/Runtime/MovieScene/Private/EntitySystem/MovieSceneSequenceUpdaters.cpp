...
void FSequenceUpdater_Hierarchical::Update(TSharedRef<const FSharedPlaybackState> SharedPlaybackState, const FMovieSceneContext& Context)
{
	...
	if (RootOverrideHierarchy)
	{
		...
		// Track sequences whose own condition has failed so that their descendants are also skipped.
		// The compiled tree is populated depth-first (parent AddRange -> recurse -> child AddRange),
		// so parent entries always appear before their children in the iteration.
		TSet<FMovieSceneSequenceID> ConditionFailedSequences;
		...
		for (const FMovieSceneSubSequenceTreeEntry& Entry : RootOverrideHierarchy->GetTree().GetAllData(SubSequenceIt.Node()))
		{
			...
			// If a parent sequence's condition failed, propagate that failure to this subsequence.
			if (const FMovieSceneSequenceHierarchyNode* SubNode = RootOverrideHierarchy->FindNode(Entry.SequenceID))
			{
				if (SubNode->ParentID != MovieSceneSequenceID::Root)
				{
					const FMovieSceneSequenceID ParentIDFromRoot = RootOverridePath.ResolveChildSequenceID(SubNode->ParentID);
					if (ConditionFailedSequences.Contains(ParentIDFromRoot))
					{
			...
			if (!bSubSequenceConditionFailed && Condition)
			{
				// The condition's director BP belongs to the parent sequence (the one
				// containing the sub-section), so resolve the parent's root-relative ID
				// for use in director instance lookup during condition evaluation.
				FMovieSceneSequenceID ConditionOwnerSequenceID = RootOverrideSequenceID;
				if (const FMovieSceneSequenceHierarchyNode* SubNode = RootOverrideHierarchy->FindNode(Entry.SequenceID))
				{
					if (SubNode->ParentID != MovieSceneSequenceID::Root)
					{
						ConditionOwnerSequenceID = RootOverridePath.ResolveChildSequenceID(SubNode->ParentID);
					}
				}
				...
				if (Condition->CanCacheResult(SharedPlaybackState))
				{
					if (bool* ConditionResult = CachedConditionResults.Find(Condition->ComputeCacheKey(FGuid(), ConditionOwnerSequenceID, SharedPlaybackState, FindObject<UMovieSceneSubSection>(CompiledDataManager->GetEntryRef(RootCompiledDataID).GetSequence(), *SubData->SectionPath.ToString()))))
					{
					...
					else if (!Condition->EvaluateCondition(FGuid(), ConditionOwnerSequenceID, SharedPlaybackState))
					{
						bSubSequenceConditionFailed = true;
					}
				}
			...
			{
				// Record this sequence so its descendants are also skipped
				ConditionFailedSequences.Add(SequenceIDFromRoot);
