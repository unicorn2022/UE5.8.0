...
void FSequenceInstance::InitializeLegacyEvaluator()
{
	...
	if (EnumHasAnyFlags(CompiledEntry.AccumulatedMask, EMovieSceneSequenceCompilerMask::EvaluationTemplate))
	{
		...
		if (!Player)
		{
			...
			{
				UE_LOGF(LogMovieScene, Log, "Initializing a legacy player for sequence '%ls'.", *GetNameSafe(SharedPlaybackState->GetRootSequence()));
		...
		if (!Player)
		{
			UE_LOGF(LogMovieScene, Log, "Initializing a default legacy player for sequence '%ls'.", *GetNameSafe(SharedPlaybackState->GetRootSequence()));
...
void FSequenceInstance::DestroyImmediately()
{
	...
	{
		UE_LOGF(LogMovieSceneECS, Verbose, "Instance being destroyed without first having been finished by calling Finish()");
