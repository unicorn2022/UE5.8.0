...
bool UMovieSceneEntitySystemLinker::StartEvaluation()
{
	...
	UE_LOGF(LogMovieSceneECS, Warning, "Can't start a new evaluation: the active runner is not in a re-entrancy window.");
	return false;
}
...
UMovieSceneEntitySystem* UMovieSceneEntitySystemLinker::LinkSystemImpl(TSubclassOf<UMovieSceneEntitySystem> InClassType)
{
	...
	if (Recycled)
	{
		...
		UE_LOGF(LogMovieSceneECS, Verbose, "Recycling system: ");
	}
	else
	{
