...
void ResolvePropertyRecursive(void* BasePointer, UStruct* InStruct, TArrayView<const FStringView> InPropertyNames, uint32 Index, FPropertyResolutionState& OutResolutionState)
{
	...
	if (PropertyStep.ArrayIndex != INDEX_NONE)
	{
		...
		if (FArrayProperty* ArrayProp = CastField<FArrayProperty>(PropertyStep.Property))
		{
			...
			{
				// The property path goes into an invalid index of the array.
				OutResolutionState.bIsValid = false;
			}
		}
		else
		{
			UE_LOGF(LogMovieScene, Error, "Mismatch in property evaluation. %ls is not of type: %ls", *PropertyStep.Property->GetName(), *FArrayProperty::StaticClass()->GetName());
			OutResolutionState.bIsValid = false;
		}
	}
...
void FTrackInstancePropertyBindings::CallFunctionForEnum(UObject& InRuntimeObject, int64 PropertyValue)
{
	...
	else if (Property)
	{
		...
		{
			UE_LOGF(LogMovieScene, Error, "Mismatch in property evaluation. %ls is not of type: %ls", *Property->GetName(), *FEnumProperty::StaticClass()->GetName());
		}
	}
}
...
template<> void FTrackInstancePropertyBindings::CallFunction<bool>(UObject& InRuntimeObject, TCallTraits<bool>::ParamType PropertyValue)
{
	...
	else if (Property)
	{
		...
		{
			UE_LOGF(LogMovieScene, Error, "Mismatch in property evaluation. %ls is not of type: %ls", *Property->GetName(), *FBoolProperty::StaticClass()->GetName());
		}
	}
}
...
template<> void FTrackInstancePropertyBindings::CallFunction<UObject*>(UObject& InRuntimeObject, UObject* PropertyValue)
{
	...
	else if (Property)
	{
		...
		{
			UE_LOGF(LogMovieScene, Error, "Mismatch in property evaluation. %ls is not of type: %ls", *Property->GetName(), *FObjectPropertyBase::StaticClass()->GetName());
		}
	}
}
