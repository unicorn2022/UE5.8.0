...
static TAutoConsoleVariable<bool> CVarSequencerAllowPausedClock(
	TEXT("Sequencer.AllowPausedClock"),
	true,
	TEXT("Allow game pause to pause the sequencer clock."),
	ECVF_Default);
...
{
	UE_LOGF(LogMovieScene, VeryVerbose, "TimeController PrepareToPlay: Sequence started preparing: frame %d, subframe %f. Frame rate: %f fps.", InStartTime.Time.FrameNumber.Value, InStartTime.Time.GetSubFrame(), InStartTime.Rate.AsDecimal());
...
{
	UE_LOGF(LogMovieScene, VeryVerbose, "TimeController Start: Sequence started: frame %d, subframe %f. Frame rate: %f fps.", InStartTime.Time.FrameNumber.Value, InStartTime.Time.GetSubFrame(), InStartTime.Rate.AsDecimal());
...
{
	UE_LOGF(LogMovieScene, VeryVerbose, "TimeController Start: Sequence stopped.");
...
FFrameTime FMovieSceneTimeController_ExternalClock::OnRequestCurrentTime(const FQualifiedFrameTime& InCurrentTime, float InPlayRate)
{
	...
	double PausedTime = 0;
	if (CVarSequencerAllowPausedClock->GetBool())
	{
		...
		{	
			...
			{
				PausedTime = Context.World()->RealTimeSeconds - Context.World()->AudioTimeSeconds;
				break;
			}
		}
	}

	double CurrentSequenceTimeSeconds = UndilatedDeltaTime + AccumulatedDilation - PausedTime;
...
#if !NO_LOGGING
	UE_LOGF(LogMovieScene, VeryVerbose, "TimeController Clock Start Time: %f, Clock Now: %f, Dilation Offset: %f, Sequence Start Time: frame %d, subframe %f, Sequence Offset Seconds: %f, Sequence Now: frame %d, subframe %f",
