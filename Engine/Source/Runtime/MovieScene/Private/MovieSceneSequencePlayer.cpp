...
void UMovieSceneSequencePlayer::PlayLooping(int32 NumLoops)
{
	...
	// PlayLooping should only override num loops temporarily, so cache the current settings here for restoring later when playback stops.
	if (!CachedPlaybackSettings.IsSet())
	{
		CachedPlaybackSettings = PlaybackSettings;
	}
...
void UMovieSceneSequencePlayer::PlayInternal()
{
	...
	if (!IsPlaying() && Sequence && CanPlay() && TimeControllerState == ETimeControllerState::ReadyToPlay)
	{
		...
		UE_LOGF(LogMovieScene, Verbose, "PlayInternal - %ls (current status: %ls)", *SequenceName, *UEnum::GetValueAsString(Status));
...
void UMovieSceneSequencePlayer::Pause()
{
	...
	const FString SequenceName = GetSequenceName(true);
	UE_LOGF(LogMovieScene, Verbose, "Pause - %ls (current status: %ls)", *SequenceName, *UEnum::GetValueAsString(Status));
	...
	if (IsPlaying())
	{
		...
		auto FinishPause = [this]
		{
			...
			UE_LOGF(LogMovieScene, Verbose, "Paused - %ls", *SequenceName);
...
void UMovieSceneSequencePlayer::StopInternal(FFrameTime TimeToResetTo)
{
	...
	const FString SequenceName = GetSequenceName(true);
	UE_LOGF(LogMovieScene, Verbose, "StopInternal - %ls (at: %ls, current status: %ls)", *SequenceName, *LexToString(TimeToResetTo), *UEnum::GetValueAsString(Status));
	...
	if (IsPlaying() || IsPaused())
	{
		...
		// Restore playback settings to cached state to clear any temporary overrides.
		if (CachedPlaybackSettings.IsSet())
		{
			PlaybackSettings = CachedPlaybackSettings.GetValue();
			CachedPlaybackSettings.Reset();
		}
		...
		auto OnFlushed = [this, TimeToResetTo]
		{
			...
			const FString SequenceName = this->GetSequenceName(true);
			UE_LOGF(LogMovieScene, Verbose, "Stopped - %ls", *SequenceName);
...
void UMovieSceneSequencePlayer::SetFrameRate(FFrameRate FrameRate)
{
	...
	{
		const FString SequenceName = GetSequenceName(true);
		UE_LOGF(LogMovieScene, Error, "Attempting to set sequence %ls with an invalid frame rate: %ls", *SequenceName, *FrameRate.ToPrettyText().ToString());
		return;
	}
	...
	if (MovieScene)
	{
		...
		{
			UE_LOGF(LogMovieScene, Warning, "Attempting to play back a sequence with tick resolution of %f ticks per second frame locked to %f fps, which is not a multiple of the resolution.", MovieScene->GetTickResolution().AsDecimal(), FrameRate.AsDecimal());
		}
	}
...
void UMovieSceneSequencePlayer::RestoreState()
{
	if (PlaybackSettings.FinishCompletionStateOverride != EMovieSceneCompletionModeOverride::ForceRestoreState)
	{
		UE_LOGF(LogMovieScene, Warning, "Attempting to restore pre-animated state for a player that was not set to capture pre-animated state. Please set PlaybackSettings.FinishCompletionStateOverride to ForceRestoreState");
	}
...
void UMovieSceneSequencePlayer::SetCompletionModeOverride(EMovieSceneCompletionModeOverride CompletionModeOverride)
{
	...
	{
		UE_LOGF(LogMovieScene, Warning, "Attempting to set completion mode override to force restore state while the sequence is already playing. Force restore state must be set before starting playback.");
	}
...
void UMovieSceneSequencePlayer::Initialize(UMovieSceneSequence* InSequence)
{
	...
	if (MovieScene)
	{
		...
		UE_LOGF(LogMovieScene, Verbose, "Initialize - MovieSceneSequence: %ls, TickResolution: %ls, DisplayRate: %ls", *InSequence->GetName(), *TickResolution.ToPrettyText().ToString(), *DisplayRate.ToPrettyText().ToString());

		if (!TickResolution.IsValid() || TickResolution.Numerator <= 0)
		{
			const FString SequenceName = GetSequenceName(true);
			const FFrameRate DefaultTickResolution(60000, 1);
			UE_LOGF(LogMovieScene, Error, "Attempting to set sequence %ls with an invalid tick resolution: %ls, defaulting to: %ls", *SequenceName, *TickResolution.ToPrettyText().ToString(), *DefaultTickResolution.ToPrettyText().ToString());
		...
		if (!DisplayRate.IsValid() || DisplayRate.Numerator <= 0)
		{
			...
			UE_LOGF(LogMovieScene, Error, "Attempting to set sequence %ls with an invalid display rate: %ls, defaulting to: %ls", *SequenceName, *DisplayRate.ToPrettyText().ToString(), *DefaultDisplayRate.ToPrettyText().ToString());
...
EMovieScenePlayerStatus::Type UpdateMethodToStatus(EUpdatePositionMethod Method, EMovieScenePlayerStatus::Type SequenceStatus)
{
	switch(Method)
	{
	case EUpdatePositionMethod::Scrub: return EMovieScenePlayerStatus::Scrubbing;
	case EUpdatePositionMethod::Jump:  return SequenceStatus;						// If jumping, the new status after the jump should be whatever the current status of the sequence is
...
{
	bHasJumpedOverride = bHasJumpedOverride || (Method == EUpdatePositionMethod::Jump);
	EMovieScenePlayerStatus::Type StatusOverride = UpdateMethodToStatus(Method, Status);
	...
	if (Duration == 0 && DurationSubFrames == 0.f)
	{
		if (bWarnZeroDuration)
		{
			...
			const FString SequenceName = GetSequenceName(true);
			UE_LOGF(LogMovieScene, Warning, "Attempting to play back sequence %ls with zero duration", *SequenceName);
		}
		return;
	}
	bWarnZeroDuration = true;
	...
	if (!PauseAtTime.IsSet() && Method == EUpdatePositionMethod::Play && ShouldStopOrLoop(NewPosition))
	{
		...
		if (bLoopIndefinitely || CurrentNumLoops + NumTimesLooped <= PlaybackSettings.LoopCount.Value)
		{
				...
				(!bReversePlayback && CurrentPosition < EndTimeWithReversed))
			{
				...
				UpdateMovieSceneInstance(Range, StatusOverride, bHasJumpedOverride);
			}
		...
		else
		{
			...
			{
				UE_LOGF(LogMovieScene, Verbose,
						"Finishing sequence %ls at frame %ls since we have authority.",
			...
			{
				UE_LOGF(LogMovieScene, Verbose, 
						"Keeping sequence %ls at frame %ls while waiting for playback finish from server.",
						*SequenceName, *LexToString(NewPosition));
			}
...
void UMovieSceneSequencePlayer::UpdateMovieSceneInstance(FMovieSceneEvaluationRange InRange, EMovieScenePlayerStatus::Type PlayerStatus, const FMovieSceneUpdateArgs& Args)
{
	if (Observer && !Observer->CanObserveSequence())
	{
		UE_LOGF(LogMovieScene, Error, "Refusing to update an unobservable sequence! Did it become unobservable during playback?");
		return;
	}
	...
	if (UE_LOG_ACTIVE(LogMovieScene, VeryVerbose))
	{
		...
		const FString SequenceName = GetSequenceName(true);
		UE_LOGF(LogMovieScene, VeryVerbose, "Evaluating sequence %ls at frame %d, subframe %f (%f fps).", *SequenceName, CurrentTime.Time.FrameNumber.Value, CurrentTime.Time.GetSubFrame(), CurrentTime.Rate.AsDecimal());
	}
#endif
...
void UMovieSceneSequencePlayer::RPC_ExplicitServerUpdateEvent_Implementation(EUpdatePositionMethod EventMethod, FFrameTime MarkerTime, int32 NewSerialNumber)
{
	...
	if (UE_LOG_ACTIVE(LogMovieScene, Verbose))
	{
		...
		const FString SequenceName = GetSequenceName(true);
		UE_LOGF(LogMovieScene, Verbose, "Explicit update event for sequence %ls %ls @ %ls. Server has moved to %ls @ %ls.",
			*SequenceName,
...
void UMovieSceneSequencePlayer::RPC_OnStopEvent_Implementation(FFrameTime StoppedTime, int32 NewSerialNumber)
{
	...
	if (UE_LOG_ACTIVE(LogMovieSceneRepl, Verbose))
	{
		const FFrameTime CurrentTime = PlayPosition.GetCurrentPosition();
		const FString SequenceName = GetSequenceName(true);
		UE_LOGF(LogMovieSceneRepl, Verbose, "Explicit Stop() event for sequence %ls %ls @ frame %d, subframe %f. Server has stopped at frame %d, subframe %f.",
	...
	{
		UE_LOGF(LogMovieSceneRepl, Verbose, "Catching up to explicit stop time %ls", *LexToString(StoppedTime));
...
void UMovieSceneSequencePlayer::RPC_OnFinishPlaybackEvent_Implementation(FFrameTime StoppedTime, int32 NewSerialNumber)
{
	...
	const FString SequenceName = GetSequenceName(true);
	UE_LOGF(LogMovieScene, Verbose, "Received RPC event to finish sequence %ls at frame %ls.", *SequenceName, *LexToString(StoppedTime));
...
void UMovieSceneSequencePlayer::PostNetReceive()
{
	...
	if (NetSyncProps.LastKnownSerialNumber < SerialNumber)
	{
		...
		const FFrameTime CurrentTime = PlayPosition.GetCurrentPosition();
		const FString SequenceName = GetSequenceName(true);
		UE_LOGF(LogMovieSceneRepl, Verbose, 
				"Ignoring network update with old serial (%d < %d) for sequence %ls %ls @ %ls. Server was %ls @ %ls.",
	...
	if (Observer && !Observer->CanObserveSequence())
	{
		...
		if (UE_LOG_ACTIVE(LogMovieSceneRepl, Verbose))
		{
			const FFrameTime CurrentTime = PlayPosition.GetCurrentPosition();
			const FString SequenceName = GetSequenceName(true);
			UE_LOGF(LogMovieSceneRepl, Verbose, "Ignoring network update for unobservable sequence %ls %ls @ %ls. Server is %ls @ %ls.",
				*SequenceName,
				*UEnum::GetValueAsString(Status.GetValue()), *LexToString(CurrentTime),
				*UEnum::GetValueAsString(NetSyncProps.LastKnownStatus.GetValue()), *LexToString(NetSyncProps.LastKnownPosition));
		}
	...
	if (UE_LOG_ACTIVE(LogMovieSceneRepl, VeryVerbose))
	{
		const FFrameTime CurrentTime = PlayPosition.GetCurrentPosition();
		const FString SequenceName = GetSequenceName(true);
		UE_LOGF(LogMovieSceneRepl, VeryVerbose, "Network sync for sequence %ls %ls @ %ls. Server is %ls @ %ls.",
			*SequenceName,
			*UEnum::GetValueAsString(Status.GetValue()), *LexToString(CurrentTime),
			*UEnum::GetValueAsString(NetSyncProps.LastKnownStatus.GetValue()), *LexToString(NetSyncProps.LastKnownPosition));
	}
	...
	if (Status == EMovieScenePlayerStatus::Playing)
	{
		...
		if (Difference > LagThreshold + PingLag)
		{
			...
			if (UE_LOG_ACTIVE(LogMovieSceneRepl, Log))
			{
				const FFrameTime CurrentTime = PlayPosition.GetCurrentPosition();
				const FString SequenceName = GetSequenceName(true);
				UE_LOGF(LogMovieSceneRepl, Log, "Correcting de-synced play position for sequence %ls %ls @ %ls. Server is %ls @ %ls, (smoothed: %ls). Client ping is %.2fms.",
					*SequenceName, 
					*UEnum::GetValueAsString(Status.GetValue()), *LexToString(CurrentTime),
					*UEnum::GetValueAsString(NetSyncProps.LastKnownStatus.GetValue()), *LexToString(NetSyncProps.LastKnownPosition),
