			...
			if (UMovieSceneSequence* CurrentSequence = TrackOuter->GetTypedOuter<UMovieSceneSequence>())
			{
				...
				if (CurrentSequence->IsSubSequenceCompatible(*SubSequence))
				{
					...
					for (UMovieSceneSubTrack* SubTrack : SubTracks)
					{
						...
						{
							UE_LOGF(LogMovieScene, Error, "Invalid level sequence %ls. It is already contained by: %ls.", *SubSequence->GetDisplayName().ToString(), *CurrentSequence->GetDisplayName().ToString());
				...
				{
					UE_LOGF(LogMovieScene, Error, "Level sequence %ls is not a compatible subsequence for %ls.", *SubSequence->GetDisplayName().ToString(), *CurrentSequence->GetDisplayName().ToString());
					SubSequence = PreviousSubSequence;
				}
			}
		}
