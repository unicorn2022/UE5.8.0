...
void UMovieScene::AddSpawnable(const FMovieSceneSpawnable& InNewSpawnable, const FMovieSceneBinding& InNewBinding)
{
	...
	for (UMovieSceneTrack* Track : NewBinding.GetTracks())
	{
		if (!Track->IsIn(this))
		{
			...
			Track->Rename(*NewName.ToString(), this, REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
		}
	}
...
bool UMovieScene::AddGivenTrack(UMovieSceneTrack* InTrack, const FGuid& ObjectGuid)
{
	...
	if (FMovieSceneBinding* Binding = FindBinding(ObjectGuid))
	{
		InTrack->Rename(nullptr, this, REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
...
bool UMovieScene::RemoveNullTracks()
{
	...
	{
		bNullTracksFound |= Binding.RemoveNullTracks();
		bNullTracksFound |= Binding.RemoveNulls();
	}
...
void UMovieScene::RemoveBinding(const FGuid& Guid)
{
	...
	if (Index != INDEX_NONE)
	{
		// Copy because OnRemovedFromMovieScene may cascade and mutate this binding's tracks.
		TArray<UMovieSceneTrack*> TracksToRemove(ObjectBindings[Index].GetTracks());
		for (UMovieSceneTrack* Track : TracksToRemove)
		{
			if (Track && ObjectBindings[Index].GetTracks().Contains(Track))
			{
				Track->OnRemovedFromMovieScene();
			}
		}
