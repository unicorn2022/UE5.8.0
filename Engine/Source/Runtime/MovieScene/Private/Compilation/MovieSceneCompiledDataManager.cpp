...
FString GMovieSceneCompilerVersion = "7D4B98092FAC4A6B964ECF72D8279EF8";
...
void UMovieSceneCompiledDataManager::Compile(FMovieSceneCompiledDataID DataID, UMovieSceneSequence* Sequence, EMovieSceneServerClientMask InNetworkMask)
{
...

	{
		if (ensure(MovieScene))
		{
			// Ensure we have no null tracks. This should really not be the case, but we have crash reporter cases of this happening
			ensure(!MovieScene->RemoveNullTracks());
	...
	if (bHasHierarchy)
	{
		...
		{
			UE_LOGF(LogMovieScene, Log, "Newly compiled sequence hierarchy:");
	...
	{
		UE_LOGF(LogMovieScene, Log, "No sequence hierarchy");
}
#endif
#endif
}
...
void UMovieSceneCompiledDataManager::GatherTrack(const FMovieSceneBinding* ObjectBinding, UMovieSceneTrack* Track, const FTrackGatherParameters& Params, const FMovieSceneEvaluationTemplate* TrackTemplate, FMovieSceneGatheredCompilerData* OutCompilerData) const
{
	...
	if (OutCompilerData->EntityField)
	{
		...
		for (UObject* Decoration : Track->GetActiveDecorations())
		{
			...
			// Handle decorations that provide sections for evaluation
			if (IMovieSceneSectionProviderDecoration* SectionProvider = Cast<IMovieSceneSectionProviderDecoration>(Decoration))
			{
				for (UMovieSceneSection* Section : SectionProvider->GetSections())
				{
					...
					TRange<FFrameNumber> EffectiveRange = TRange<FFrameNumber>::Intersection(Params.LocalClampRange, SectionRange);
					...
					// If the section is an entity provider, let it populate itself
					IMovieSceneEntityProvider* SectionEntityProvider = Cast<IMovieSceneEntityProvider>(Section);
					if (SectionEntityProvider)
					{
						if (!SectionEntityProvider->PopulateEvaluationField(EffectiveRange, MetaData, &FieldBuilder))
						{
							const int32 EntityIndex   = FieldBuilder.FindOrAddEntity(Section, 0);
...
#if WITH_EDITOR
			if (Entry.Section && Entry.Section->IsLocalEvalDisabled())
			{
				continue;
			}
#endif
...
#if WITH_EDITOR
				if (Section && Section->IsLocalEvalDisabled())
				{
					return true;
				}
#endif
