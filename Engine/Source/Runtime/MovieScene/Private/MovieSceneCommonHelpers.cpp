...
bool MovieSceneHelpers::IsSectionKeyable(const UMovieSceneSection* Section)
{
	...
	if (Track->IsEvalDisabled() || Track->IsRowEvalDisabled(Section->GetRowIndex()))
	{
		return false;
	}

#if WITH_EDITOR
	bool bHasAnyChannels = false;
	bool bHasAnyKeyableChannels = false;
	...
	{
		bHasAnyChannels = true;
		for (const FMovieSceneChannelMetaData& MetaData : ChannelEntry.GetMetaData())
		{
			if (MetaData.bEnabled && MetaData.bCanBeKeyed)
			{
				bHasAnyKeyableChannels = true;
				goto DoneCheckingChannels;
			}
		}
	}
DoneCheckingChannels:
...
void MovieSceneHelpers::DisableTrack(UMovieSceneTrack* Track)
{
	Track->Modify();
	...
	else
	{
		...
		{
			Track->SetRowEvalDisabled(true, RowIndex);
		}
	}
}
...
{
	// Guard against a stale template object of a different class occupying InName in the same outer.
	if (UObject* ExistingObject = StaticFindObjectFast(UObject::StaticClass(), InMovieScene, InName))
	{
		if (ExistingObject->GetClass() != InSourceObject.GetClass())
		{
			const FName ExpiredName = MakeUniqueObjectName(InMovieScene, ExistingObject->GetClass(), "ExpiredSpawnable");
			ExistingObject->Rename(*ExpiredName.ToString(), nullptr, REN_DoNotDirty | REN_DontCreateRedirectors | REN_NonTransactional);
	...
	static const FName SequencerActorTag("SequencerActor");
	static const FName SequencerPreviewActorTag("SequencerPreviewActor");
...
bool MovieSceneHelpers::ResetSectionOrder(UMovieSceneTrack* Track, const TArray<UMovieSceneSection*>& NewSections)
{
	if (Track)
	{
		const TArray<UMovieSceneSection*>& OldSections = Track->GetAllSections();
		//make sure the sections are equivalent but maybe not in same order
		if (NewSections.Num() != OldSections.Num())
		{
			return false;
		}
		//make sure new section have the right track parent and no nullptrs
		for (UMovieSceneSection* NewSection : NewSections)
		{
			if (NewSection == nullptr || NewSection->GetTypedOuter<UMovieSceneTrack>() != Track)
			{
				return false;
			}
		}
		//use set here to handle both duplicates and checking equivalency
		TSet<UMovieSceneSection*> NewSet(NewSections);
		TSet<UMovieSceneSection*> OldSet(OldSections);
		
		if (NewSet.Num() == OldSet.Num() && NewSet.Includes(OldSet))
		{
			//remove all existing
			int32 NumSections = OldSections.Num();
			for (int32 Index = NumSections - 1; Index >= 0; --Index)
			{
				Track->RemoveSectionAt(UMovieSceneTrack::FSectionIndexParameter(Index));
			}
			//add back new copy in order
			for (UMovieSceneSection* Section : NewSections)
			{
				Track->AddSection(UMovieSceneTrack::FSectionParameter(*Section));
			}
			return true;
		}
	}
