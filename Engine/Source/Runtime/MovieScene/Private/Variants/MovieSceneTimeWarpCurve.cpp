...
void UMovieSceneTimeWarpCurve::InitializeDefaults(UMovieSceneSection* Section)
{
	...
	if (MovieScene)
	{
		FFrameNumber StartFrame = DiscreteInclusiveLower(MovieScene->GetPlaybackRange());
		FFrameNumber EndFrame   = DiscreteExclusiveUpper(MovieScene->GetPlaybackRange());
		...
		if (Section)
		{
			if (Section->HasStartFrame())
			{
				...
				{
					EndFrame = Section->GetExclusiveEndFrame() - Section->GetInclusiveStartFrame();
				}
			}
			else if (Section->HasEndFrame())
			{
				EndFrame = Section->GetExclusiveEndFrame();
			}
		}
