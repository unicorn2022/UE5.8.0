...
static bool GAvoidOverlappingWeightedTangents = true;
static FAutoConsoleVariableRef CAvoidOverlappingWeightedTangents(
	TEXT("ControlRig.AvoidOverlappingWeightedTangents"),
	GAvoidOverlappingWeightedTangents,
	TEXT("(Default: true) If true weighted tangents won't overlap in time, legacy behavior was to have them overlap which can cause discontinuities"),
	ECVF_Default
);
	...
	bool bEndIsWeighted)
{
	...
	double StartCosAngle, StartSinAngle, StartAngle;
	// ---------------------------------------------------------------------------------
	// Initialize the start key parameters
	StartAngle = FMath::Atan(StartTangent * ToSeconds);
	FMath::SinCos(&StartSinAngle, &StartCosAngle, StartAngle);
	...
	double StartKeyTanX = StartCosAngle * StartWeight + Time1;
	StartKeyTanY              = StartSinAngle * StartWeight + StartValue;

	double EndCosAngle, EndSinAngle, EndAngle;
	// ---------------------------------------------------------------------------------
	// Initialize the end key parameters
	EndAngle = FMath::Atan(EndTangent * ToSeconds);
	FMath::SinCos(&EndSinAngle, &EndCosAngle, EndAngle);
	...
	double EndKeyTanX = -EndCosAngle * EndWeight + Time2;
	EndKeyTanY              = -EndSinAngle * EndWeight + EndValue;

	if (GAvoidOverlappingWeightedTangents) //if prevent overlapping tangents 
	{
		if (EndKeyTanX < StartKeyTanX)
		{
			//shrink tangents based upon proportional distance 
			const double Overlap = StartKeyTanX - EndKeyTanX;
			double FirstXDistance = StartKeyTanX - Time1;
			double SecondXDistance = Time2 - EndKeyTanX;
			const double Total = FirstXDistance + SecondXDistance;
			const double FirstProportion = FirstXDistance / Total;
			const double SecondProption = SecondXDistance / Total;
			const double MoveFirstBack = Overlap * FirstProportion;
			const double MoveSecondForward = Overlap * SecondProption;

			StartKeyTanX -= MoveFirstBack;
			FirstXDistance = StartKeyTanX - Time1;
			const double FirstYDistance = (FirstXDistance)*FMath::Tan(StartAngle);
			StartKeyTanY = FirstYDistance + StartValue;

			EndKeyTanX += MoveSecondForward;
			SecondXDistance = Time2 - EndKeyTanX;
			const double SecondYDistance = (SecondXDistance) * -FMath::Tan(EndAngle);
			EndKeyTanY = SecondYDistance + EndValue;
