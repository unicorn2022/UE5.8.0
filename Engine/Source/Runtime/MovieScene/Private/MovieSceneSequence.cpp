...
void UMovieSceneSequence::OptimizeForCook()
{
	...
	for (int32 TrackIndex = 0; TrackIndex < MovieScene->GetTracks().Num(); )
	{
		...
		{				
			...
			UE_LOGF(LogMovieScene, Display, "Removing muted track: %ls from: %ls", *Track->GetDisplayName().ToString(), *GetPathName());
			continue;
		}
		++TrackIndex;
	}
		...
		if (Track)
		{
			for (int32 SectionIndex = 0; SectionIndex < Track->GetAllSections().Num(); )
			{
				...
				if (Section && Section->GetCookOptimizationFlags() == ECookOptimizationFlags::RemoveSection)
				{
					...
					UE_LOGF(LogMovieScene, Display, "Removing muted section: %ls from: %ls", *Section->GetPathName(), *Track->GetDisplayName().ToString());
					continue;
				}
	...
	for (int32 ObjectBindingIndex = 0; ObjectBindingIndex < Bindings.Num(); )
	{
		...
		if (bRemoveObject)
		{
			...
			UE_LOGF(LogMovieScene, Display, "Removing muted object: %ls from: %ls", *ObjectDisplayName.ToString(), *GetPathName());
