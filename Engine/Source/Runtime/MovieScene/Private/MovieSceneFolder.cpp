	...
	if (OwningScene)
	{
		...
		for(int32 ChildTrackIndex = 0; ChildTrackIndex < ChildTracks.Num(); ChildTrackIndex++)
		{
			...
			if (!OwningScene->GetTracks().Contains(ChildTrack))
			{
				...
				UE_LOGF(LogMovieScene, Warning, "Folder (%ls) in Sequence (%ls) contained a reference to a Track (%ls) that no longer exists in the sequence, removing.", *GetFolderName().ToString(), *OwningScene->GetPathName(), *GetNameSafe(ChildTrack));
			}
		}
		...
		for (int32 ChildObjectBindingIndex = 0; ChildObjectBindingIndex < ChildObjectBindings.Num(); ChildObjectBindingIndex++)
		{
			...
			{
				UE_LOGF(LogMovieScene, Warning, "Folder (%ls) in Sequence (%ls) contained a reference to an Object Binding (%ls) that no longer exists in the sequence, removing.", *GetFolderName().ToString(), *OwningScene->GetPathName(), *ChildBinding.ToString());
		...
		for (int32 ChildFolderIndex = 0; ChildFolderIndex < ChildFolders.Num(); ChildFolderIndex++)
		{
			...
			{
				UE_LOGF(LogMovieScene, Warning, "Folder (%ls) in Sequence (%ls) contained a reference to an Folder (%ls) that exists in multiple places in the sequence, removing.", *GetFolderName().ToString(), *OwningScene->GetPathName(), *ChildFolders[ChildFolderIndex]->GetFolderName().ToString());
