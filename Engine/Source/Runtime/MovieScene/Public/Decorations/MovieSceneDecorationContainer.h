...
public:
	...
	*/
	...
	bool RemoveNulls()
	{
		return Decorations.Remove(nullptr) > 0;
	}
...
public:
	...
	*/
	...
	MOVIESCENE_API bool IsDecorationMuted(const UObject* Decoration) const;
	MOVIESCENE_API void SetDecorationMuted(UObject* Decoration, bool bMuted);

	MOVIESCENE_API bool IsDecorationDisabled(const UObject* Decoration) const;
	MOVIESCENE_API void SetDecorationDisabled(UObject* Decoration, bool bDisabled);

	// Returns true if the decoration is neither muted nor disabled
	MOVIESCENE_API bool IsDecorationActive(const UObject* Decoration) const;

	// Returns only decorations that are neither muted nor disabled
	MOVIESCENE_API TArray<UObject*> GetActiveDecorations() const;
...
protected:
	...
	UPROPERTY(Transient)
	TSet<TObjectPtr<UObject>> MutedDecorations;

	UPROPERTY()
	TSet<TObjectPtr<UObject>> DisabledDecorations;
};
