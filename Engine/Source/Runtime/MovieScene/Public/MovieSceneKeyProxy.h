...
#if WITH_EDITOR
		const FMovieSceneChannelMetaData* MetaData = InChannelHandle.GetMetaData();
		if (MetaData && Section)
		{
			if (UMovieScene* MovieScene = Section->GetTypedOuter<UMovieScene>())
			{
				TArray<FFrameNumber> Frames;
				Frames.Add(InOutTime);
				MovieScene->OnChannelChangedWithTime().Broadcast(Frames, MetaData, Section);
			}
		}
#endif
