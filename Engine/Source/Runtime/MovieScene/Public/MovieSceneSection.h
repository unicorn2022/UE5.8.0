...
public:
	...
	/**
	 * Expands this section's range to include the specified time if bAutoExpandToKeyframes is enabled for the track
	 */
	void ExpandToFrame(FFrameNumber InFrame)
	{
		...
		if (!Track->bAutoExpandToKeyframes)
		{
			return;
		}
	...
	virtual void SetRange(const TRange<FFrameNumber>& NewRange)
	{
		...
		if (!HasAnyFlags(RF_NeedInitialization))
		{
			...
			bCanSetRange = !IsReadOnly() && bHasChanged && TryModify();
		}
...
#if WITH_EDITOR
	/** Whether evaluation of this section is disabled due to local muting/soloing (not serialized). Virtual so subclasses (e.g. transitions) can add additional disable conditions. */
	MOVIESCENE_API virtual bool IsLocalEvalDisabled() const;
	void SetLocalEvalDisabled(bool bInEvalDisabled) { bIsLocalEvalDisabled = bInEvalDisabled; }
#endif
...
private:
	...
	/** Whether evaluation of this section is disabled due to local muting/soloing (transient, not serialized) */
	bool bIsLocalEvalDisabled = false;
#endif
