...
namespace Private
{
	...
	static int32 CountBits(T In)
	{
		if constexpr (sizeof(T) > 4)
		{
			return FMath::CountBits64(In);
		}
		else
		{
			return FMath::CountBits(In);
		}
	}
...
struct TFixedSparseBitSet
{
	...
	void CopyTo(TFixedSparseBitSet<OtherHashType, OtherStorageType>& Other) const
	{
		...
		// Copy the buckets
		const uint32 NumBuckets = Private::CountBits(Other.BucketHash);
	...
	TFixedSparseBitSet& operator|=(const TFixedSparseBitSet& Other)
	{
		...
		while (OtherBucketBitIndex < HashSize)
		{
			...
			const uint32 ThisBucketIndex = Private::CountBits(BucketHash & (HashBit-1));
	...
	{
		return Private::CountBits(this->BucketHash);
	}
	...
	uint32 CountSetBits() const
	{
		...
		{
			Total += Private::CountBits(Buckets.Get(Index));
		}
		return Total;
	}
	...
	bool IsBitSet(uint32 BitIndex) const
	{
		...
		{
			const uint32     BucketIndex  = Private::CountBits(BucketHash & (HashBit-1));
	...
	int32 GetSparseBucketIndex(uint32 BitIndex) const
	{
		...
		if (BucketHash & HashBit)
		{
			uint32 BucketIndex = Private::CountBits(BucketHash & (HashBit-1));
			...
			{
				// Compute the offset
				int32 SparseIndex = Private::CountBits(ThisBucket & (ThisBitMask-1));

				// Count all the preceding buckets to find the final sparse index
				while (BucketIndex > 0)
				{
					--BucketIndex;
					SparseIndex += Private::CountBits(Buckets.Get(BucketIndex));
				}
				return SparseIndex;
			}
		}
	...
	struct FIterator
	{
		...
		void operator++()
		{
			...
			else
			{
				...
				{
					const uint8 NextBucketIndex = Private::CountBits(BitSet->BucketHash & BitOffsetToLowBitMask<HashType>(BucketBitIndex));
...
private:
	...
	struct FBitOffsets
	{
		...
		FBitOffsets(HashType InBucketHash, uint32 BitIndex)
		{
			...
			BucketIndex = Private::CountBits(InBucketHash & (HashBit-1u));
