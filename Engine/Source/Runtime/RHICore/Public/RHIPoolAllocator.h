...
/**
@brief Type of allocation strategy
**/
enum class ERHIPoolAllocationStrategy
{
	Immediate,
	Async,
	Count
...
/**
@brief Status of the Async pool allocation
**/
enum class ERHIPoolAllocationStatus
{
	Idle,
	Allocating,
	ShutDown,
...
public:
	...
	static constexpr int16 InvalidPoolIndex = -1;
	...
	RHICORE_API void InitFreeBlocks();
	...
	// Index setter for async allocation
	void SetPoolIndex(int16 InPoolIndex) { PoolIndex = InPoolIndex; }
...
public:
	...
	// Constructor
	RHICORE_API FRHIPoolAllocator(uint64 InDefaultPoolSize, uint32 InPoolAlignment, uint32 InMaxAllocationSize, FRHIMemoryPool::EFreeListOrder InFreeListOrder, bool InDefragEnabled, ERHIPoolAllocationStrategy InPoolAllocationStrategy, uint16 InOverFlowPoolsCount);
	RHICORE_API virtual ~FRHIPoolAllocator();

	// Setup/Shutdown
	RHICORE_API void Initialize(ERHIPoolResourceTypes InAllocationResourceType);
...
protected:
	...
	RHICORE_API void PoolAsyncAllocate(uint32 InSizeInBytes, uint32 InAllocationAlignment, ERHIPoolResourceTypes InAllocationResourceType);
	...
	// Array of pre-allocated pools
	TArray<FRHIMemoryPool*> OverFlowPools;
