...
namespace UE::RHICore::Nvidia::Aftermath
{
	...
	RHICORE_API bool IsResourceTrackingEnabled() { return bEnabled && (Flags & GFSDK_Aftermath_FeatureFlags_EnableResourceTracking); }
	...
	void LoadAftermathDLL()
	{
		...
		UE_LOGF(LogNvidiaAftermath, Log, "Nvidia Aftermath is not compatible with Windows ASAN builds, so it is disabled.");
#else
		if (GDynamicRHI && GDynamicRHI->GetInterfaceType() == ERHIInterfaceType::D3D11)
		{
			UE_LOGF(LogNvidiaAftermath, Log, "Nvidia Aftermath is disabled in D3D11 due to instability issues.");
			return;
		}

		if (!AllowVendorDevice())
		{
			UE_LOGF(LogNvidiaAftermath, Log, "Vendor devices disallowed. Aftermath initialization skipped.");
			return;
		}

		if (!UE::RHI::ShouldEnableGPUCrashFeature(*CVarAftermath, TEXT("nvaftermath")))
		{
			UE_LOGF(LogNvidiaAftermath, Log, "Nvidia Aftermath is explicitly disabled. Aftermath initialization skipped.");
			return;
		}
		...
		{
			UE_LOGF(LogNvidiaAftermath, Warning, "Failed to load GFSDK_Aftermath_Lib.x64.dll");
		}
#endif
	}
	...
	RHICORE_API void InitializeBeforeDeviceCreation()
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(UE::RHICore::Nvidia::Aftermath::InitializeBeforeDeviceCreation);
		...
		GFSDK_Aftermath_GpuCrashDumpWatchedApiFlags WatchedAPIFlags = GFSDK_Aftermath_GpuCrashDumpWatchedApiFlags_DX;
		if (GDynamicRHI && GDynamicRHI->GetInterfaceType() == ERHIInterfaceType::Vulkan)
		{
			WatchedAPIFlags = GFSDK_Aftermath_GpuCrashDumpWatchedApiFlags_Vulkan;
		}
		...
		GFSDK_Aftermath_Result Result = GFSDK_Aftermath_EnableGpuCrashDumps(
			...
			nullptr, // ResolveMarkerCb - Markers are resolved after dump creation
			nullptr // user data
		);

		if (Result != GFSDK_Aftermath_Result_Success)
		{
			UE_LOGF(LogNvidiaAftermath, Log, "Aftermath crash dumping failed to initialize (%x).", Result);
			return;
		}

		UE_LOGF(LogNvidiaAftermath, Log, "Aftermath initialized");
		bEnabled = true;
	}
	...
	bool InitializeDevice(TFunctionRef<uint32(uint32)> InitCallback)
	{
		...
		if (!bEnabled)
		{
			UE_LOGF(LogNvidiaAftermath, Log, "Aftermath is not loaded.");
			return false;
		}

		if (!IsRHIDeviceNVIDIA())
		{
			UE_LOGF(LogNvidiaAftermath, Warning, "Skipping aftermath initialization on non-Nvidia device.");
			bEnabled = false;
			return false;
		}
		...
		if (Result != GFSDK_Aftermath_Result_Success)
		{
			UE_LOGF(LogNvidiaAftermath, Log, "Aftermath enabled but failed to initialize (%x).", Result);
			bEnabled = false;
			return false;
		}

		UE_LOGF(LogNvidiaAftermath, Log, "Aftermath enabled. Active feature flags: ");
		for (uint32 LowestBit, Bits = Flags; Bits; Bits ^= LowestBit)
		{
			...
			switch (LowestBit)
			{
			case GFSDK_Aftermath_FeatureFlags_EnableMarkers             : UE_LOGF(LogNvidiaAftermath, Log, " - Feature: EnableMarkers"); break;
			case GFSDK_Aftermath_FeatureFlags_EnableResourceTracking    : UE_LOGF(LogNvidiaAftermath, Log, " - Feature: EnableResourceTracking"); break;
			case GFSDK_Aftermath_FeatureFlags_CallStackCapturing        : UE_LOGF(LogNvidiaAftermath, Log, " - Feature: CallStackCapturing"); break;
			case GFSDK_Aftermath_FeatureFlags_GenerateShaderDebugInfo   : UE_LOGF(LogNvidiaAftermath, Log, " - Feature: GenerateShaderDebugInfo"); break;
			case GFSDK_Aftermath_FeatureFlags_EnableShaderErrorReporting: UE_LOGF(LogNvidiaAftermath, Log, " - Feature: EnableShaderErrorReporting"); break;

			default:
				UE_LOGF(LogNvidiaAftermath, Log, " - Feature: Unknown flag: 0x%08x", LowestBit);
				break;
			}
		}
	...
	// Called on NVIDIA driver thread
	static void GFSDK_AFTERMATH_CALL Callback_GpuCrashDump(const void* GPUCrashDumpData, const uint32_t GPUCrashDumpSize, void* UserData)
	{
		FScopeLock Lock(&CrashIntermediates.Lock);

		// Take a copy of the data provided by Aftermath. We process this on the interrupt thread after dumping is complete.
		TArray<uint8>& Data = CrashIntermediates.CrashDumps.Emplace_GetRef().Data;
		Data.SetNumUninitialized(GPUCrashDumpSize);
		FMemory::Memcpy(Data.GetData(), GPUCrashDumpData, GPUCrashDumpSize);
	}
	...
	void Callback_ShaderDebugInfo(const void* ShaderDebugInfo, const uint32 ShaderDebugInfoSize, void* UserData)
	{
		...
		if (Result == GFSDK_Aftermath_Result_Success)
		{
			...
			TUniquePtr<FArchive> Writer = TUniquePtr<FArchive>(IFileManager::Get().CreateFileWriter(*Filename));
			if (Writer)
			{
	...
	static const TCHAR* ResidencyStateToString(GFSDK_Aftermath_ResourceResidency State)
	{
		switch (State)
		{
		case GFSDK_Aftermath_ResourceResidency_FullyResident: return TEXT("Resident");
		case GFSDK_Aftermath_ResourceResidency_Evicted: return TEXT("Evicted");
		case GFSDK_Aftermath_ResourceResidency_MemoryFreed: return TEXT("Memory Freed");
		case GFSDK_Aftermath_ResourceResidency_MemoryUnbound: return TEXT("Memory Unbound");
	...
	static void ProcessCrashDump(TArray<FCrashResult>& OutResults, const void* GPUCrashDumpData, const uint32_t GPUCrashDumpSize)
	{
		FCrashResult& CrashResult = OutResults.Emplace_GetRef();
		...
		if (TUniquePtr<FArchive> Writer = TUniquePtr<FArchive>(IFileManager::Get().CreateFileWriter(*CrashResult.DumpPath.GetValue())))
		{
				...
				if (FaultInfo.resourceInfoCount > 0)
				{
					...
					for (uint32 ResIdx = 0; ResIdx < FaultInfo.resourceInfoCount; ++ResIdx)
					{
						...
						CrashResult.OutputLog += FString::Printf(
							...
							ResidencyStateToString(FaultResInfo.residency),
		...
		CrashResult.OutputLog += TEXT("\n\n\tJSON Data:");
		{
			auto ShaderDebugInfoLookup = [](const GFSDK_Aftermath_ShaderDebugInfoIdentifier* Identifier, PFN_GFSDK_Aftermath_SetData SetShaderDebugInfo, void* UserData)
			{
				TArray<uint8>* DebugData = CrashIntermediates.ShaderDebugLookup.Find(Identifier->id[0]);
				if (!DebugData)
				{
					UE_LOGF(LogRHICore, Error, "Shader debug data not found (%llu)", Identifier->id[0]);
					return;
				}
		...
		// Report the decoded results, this is to avoid cases where Aftermath fails to collect data post crash handling.
		UE_LOGF(LogNvidiaAftermath, Error, "%ls", *CrashResult.OutputLog);
	}
	...
	FShaderHash RegisterShaderBinary(const void* ShaderBinary, uint32 ByteSize, const FStringView& DebugName)
	{
		...
		{
			UE_LOGF(LogRHICore, Display, "Shader byte size too small, expected magic dword");
		...
		switch (*static_cast<const uint32*>(ShaderBinary))
		{
			default:
			{
				UE_LOGF(LogRHICore, Display, "Unknown shader format");
				return InvalidShaderHash;
			}
			case 'CBXD': // DXBC container header
			{
				...
				if (Result != GFSDK_Aftermath_Result_Success)
				{
					UE_LOGF(LogNvidiaAftermath, Display, "Failed to get aftermath shader hash (%u)", Result);
					return InvalidShaderHash;
				}
#else // __d3d12_h__
				UE_LOGF(LogNvidiaAftermath, Display, "DXBC container without D3D12 support");
				return InvalidShaderHash;
#endif // __d3d12_h__
...
#else // VULKAN_H_
				UE_LOGF(LogNvidiaAftermath, Display, "SPIRV container without Vulkan support");
				return InvalidShaderHash;
#endif // VULKAN_H_
			}
		}
	...
	bool OnGPUCrash(TArray<FCrashResult>& OutResults)
	{
		...
		while (true)
		{
			...
			if (Result = GFSDK_Aftermath_GetCrashDumpStatus(&Status); Result != GFSDK_Aftermath_Result_Success)
			{
				UE_LOGF(LogNvidiaAftermath, Warning, "GFSDK_Aftermath_GetCrashDumpStatus failed: 0x%08x. Skipping crash dump processing.", Result);
				return false;
			}
			...
			switch (Status)
			{
				...
				{
					UE_LOGF(LogNvidiaAftermath, Warning, "Unknown aftermath crash dump state. Skipping crash dump processing.");
					return false;
				}
				...
				case GFSDK_Aftermath_CrashDump_Status_NotStarted:
				{
					...
					{
						UE_LOGF(LogNvidiaAftermath, Warning, "Timed out while waiting for Aftermath to start the GPU crash dump.");
						return false;
					}
					else
					{
				...
				case GFSDK_Aftermath_CrashDump_Status_InvokingCallback:
				{
					...
					{
						UE_LOGF(LogNvidiaAftermath, Warning, "Timed out while waiting for Aftermath to complete GPU crash dump.");
						return false;
					}
					else
					{
				...
				{
					UE_LOGF(LogNvidiaAftermath, Warning, "Aftermath failed to collect GPU crash data.");
					return false;
				}
	...
	DumpReady:
		...
		for (auto& CrashDump : CrashIntermediates.CrashDumps)
		{
			// Use a crash dump editor to insert the resolved marker names
			GFSDK_Aftermath_GpuCrashDump_Editor Editor {};
			Result = GFSDK_Aftermath_GpuCrashDump_CreateEditor(
				GFSDK_Aftermath_Version_API,
				CrashDump.Data.GetData(),
				CrashDump.Data.Num(),
				&Editor);

			if (Result != GFSDK_Aftermath_Result_Success)
			{
				UE_LOGF(LogNvidiaAftermath, Warning, "GFSDK_Aftermath_GpuCrashDump_CreateEditor failed: 0x%08x. Skipping crash dump processing.", Result);
				return false;
			}

			ON_SCOPE_EXIT
			{
				GFSDK_Aftermath_GpuCrashDump_DestroyEditor(Editor);
			};

			auto ResolveMarker = [](const void* MarkerData, const uint32_t MarkerDataSize, void* UserData, PFN_GFSDK_Aftermath_ResolveMarker ResolveMarker)
			{
			#if WITH_RHI_BREADCRUMBS
				...
				{
					ResolveMarker(RootNodeName, sizeof(RootNodeName));
				}
				else
				{
					// Retrieve the breadcrumb formatted string
					FRHIBreadcrumb::FBuffer Buffer;
					TCHAR const* NameStr = Breadcrumb->GetTCHAR(Buffer);
					uint32 NameLen = FCString::Strlen(NameStr) + 1; // Include null terminator

					ResolveMarker(NameStr, NameLen * sizeof(TCHAR));
				}
			#else
				ResolveMarker(BreadcrumbsDisabledStr, sizeof(BreadcrumbsDisabledStr));
			#endif
			};

			if (Result = GFSDK_Aftermath_GpuCrashDumpEditor_ResolveEventMarkers(Editor, ResolveMarker, nullptr); Result != GFSDK_Aftermath_Result_Success)
			{
				UE_LOGF(LogNvidiaAftermath, Warning, "GFSDK_Aftermath_GpuCrashDumpEditor_ResolveEventMarkers failed: 0x%08x. Skipping crash dump processing.", Result);
				return false;
			}

			// Retrieve the edited crash dump data
			uint32 CrashDumpSize = 0;
			if (Result = GFSDK_Aftermath_GpuCrashDumpEditor_GetCrashDumpData(Editor, nullptr, 0, &CrashDumpSize); Result != GFSDK_Aftermath_Result_Success)
			{
				UE_LOGF(LogNvidiaAftermath, Warning, "GFSDK_Aftermath_GpuCrashDumpEditor_GetCrashDumpData failed: 0x%08x. Skipping crash dump processing.", Result);
				return false;
			}

			TArray<uint8> FinalDumpData;
			FinalDumpData.SetNumUninitialized(CrashDumpSize);
			if (Result = GFSDK_Aftermath_GpuCrashDumpEditor_GetCrashDumpData(Editor, FinalDumpData.GetData(), FinalDumpData.Num(), &CrashDumpSize); Result != GFSDK_Aftermath_Result_Success)
			{
				UE_LOGF(LogNvidiaAftermath, Warning, "GFSDK_Aftermath_GpuCrashDumpEditor_GetCrashDumpData failed: 0x%08x. Skipping crash dump processing.", Result);
				return false;
			}

			ProcessCrashDump(OutResults, FinalDumpData.GetData(), FinalDumpData.Num());
		}
