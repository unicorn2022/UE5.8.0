...
namespace UE::RHICore::Intel::GPUCrashDumps
{
	...
	RHICORE_API void InitializeBeforeDeviceCreation(uint32 DeviceId)
	{
		...
		{
			UE_LOGF(LogIntelBreadcrumbs, Log, "Intel Breadcrumbs is explicitly disabled. Intel Breadcrumbs initialization skipped...");
			return;
		}
		...
		{
			UE_LOGF( LogIntelBreadcrumbs, Log, "Failed to load Intel Extensions Library (Intel Breadcrumbs)" );
			return;
		}
		...
		if (hr != S_OK)
		{
			UE_LOGF(LogIntelBreadcrumbs, Log, "Intel Breadcrumbs enabling failed to enable GPU Crash Dumps [hr=0x%08x].", hr);
			if( hr == E_NOINTERFACE )
			{
				UE_LOGF( LogIntelBreadcrumbs, Log, "Intel Breadcrumbs not implemented in the Intel Extensions library");
			}
			return;
		}

		bEnabled = true;
		UE_LOGF(LogIntelBreadcrumbs, Log, "Intel Breadcrumbs Enabled!");
	}
	...
	static void WriteCrashDump( const INTC_CRASHDUMP_SECTION* Section )
	{
		...
		UE_LOGF( LogIntelBreadcrumbs, Display, "Writing Intel Breadcrumbs [%ls] to %ls...", Section->pSectionInfo, *DumpPath.GetValue() );
		if( FArchive* Writer = IFileManager::Get().CreateFileWriter( *DumpPath.GetValue() ) )
		{
			...
			if( Writer->Close() )
			{
				UE_LOGF( LogIntelBreadcrumbs, Display, "\tIntel Breadcrumbs GPU [%ls] file written!", Section->pSectionInfo )
			}
			else
			{
				UE_LOGF( LogIntelBreadcrumbs, Error, "\tFailed to create file: %ls", *DumpPath.GetValue() );
			}
		}
	}
	...
	RHICORE_API bool OnGPUCrash()
	{
		...
		while (true)
		{
			...
			{
				UE_LOGF( LogIntelBreadcrumbs, Error, "Intel Breadcrumbs GPU Crash Dump error: HRESULT = 0x%08x", hr );
				return false;
			}
			...
			if( Status == INTC_CRASHDUMP_STATUS_NOT_READY )
			{
				...
				if( NumTries == 1 )
				{
					UE_LOGF( LogIntelBreadcrumbs, Warning, "Intel Breadcrumbs GPU Crash Dump not ready..." );
				}

				// Crash dump is still in progress...
				if( FPlatformTime::Seconds() >= EndTime )
				{
					UE_LOGF( LogIntelBreadcrumbs, Warning, "Intel Breadcrumbs GPU Crash Dump processing - timeout!" );
					return false;
				}
			...
			UE_LOGF(LogIntelBreadcrumbs, Warning, "Intel Breadcrumbs GPU Crash Dump processed!" );
			break;
		}
		...
		{
			UE_LOGF( LogIntelBreadcrumbs, Error, "Intel Breadcrumbs GPU Crash Dump retreival error: HRESULT = 0x%08x", hr );
			return false;
		}
		...
		{
			UE_LOGF(LogIntelBreadcrumbs, Error, "Intel Breadcrumbs GPU Crash Dump has no sections!");
			return false;
		}
		...
		if( INTC_DecodeGpuCrashDump( nullptr, ReportSize ) == S_OK )
		{
			if( ReportSize == 0 )
			{
				UE_LOGF( LogIntelBreadcrumbs, Error, "Intel Breadcrumbs GPU Crash Dump is empty." );
				return true;
			}
			...
			{
				UE_LOGF( LogIntelBreadcrumbs, Display, "\n%ls", Report.GetData() );
			}
			else
			{
				UE_LOGF( LogIntelBreadcrumbs, Error, "Failed to decode Intel Breadcrumbs GPU Crash Dump." );
			}
		}
		else
		{
