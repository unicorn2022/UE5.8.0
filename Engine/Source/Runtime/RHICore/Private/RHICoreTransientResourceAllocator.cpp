...
void FRHITransientHeap::Flush(uint64 AllocatorCycle, FRHITransientMemoryStats& OutMemoryStats, FRHITransientAllocationStats* OutAllocationStats)
{
	...
	// Atomically update the high watermark. CommitSizeMax is read/reset by GarbageCollect on the RHI thread.
	uint64 CurrentMax = CommitSizeMax.load(std::memory_order_relaxed);
	while (CommitSize > CurrentMax && !CommitSizeMax.compare_exchange_weak(CurrentMax, CommitSize, std::memory_order_relaxed)) {}
...
void FRHITransientHeapCache::GarbageCollect()
{
	...
	for (FRHITransientHeap* Heap : LiveList)
	{
		TotalCommitSize += Initializer.bSupportsVirtualMapping ? Heap->CommitSizeMax.exchange(0, std::memory_order_relaxed) : Heap->GetCapacity();
	}
	...
	FRHITransientHeap::FCreateBufferFunction CreateBufferFunction)
{
	...
	if (!Buffer)
	{
		UE_LOGF(LogRHICore, Fatal,
			"Transient allocator failed to allocate Buffer %ls. Size: %u, Stride: %u. Allocation Size: %u, Allocation Alignment %u.",
...
void FRHITransientResourceHeapAllocator::SetCreateMode(ERHITransientResourceCreateMode InCreateMode)
{
	const bool bSupportsParallelResourceCreation = GRHITransientAllocatorParallelResourceCreation
		...
		&& !UE::RHICore::Nvidia::Aftermath::IsResourceTrackingEnabled()
#endif
		;
