...
#pragma pack(pop)
	...
	static void GetShaderBinaryDebugHashDXBC(const void* ShaderBinary, uint32 ByteSize, FShaderDebugData& OutData)
	{
		...
		{
			UE_LOGF(LogRHICore, Error, "Shader byte size too small");
			return;
		}
		...
		for (uint32 ChunkIndex = 0; ChunkIndex < Header.ChunkCount; ++ChunkIndex)
		{
			...
			switch (ChunkHeader.Type)
			{
				...
				case 'NDLI':
				{
					...
					{
						UE_LOGF(LogRHICore, Display, "DXIL name length not the expected hash");
						continue;
					}

					if (ChunkCtx.PendingBytes() < HashLength)
					{
						UE_LOGF(LogRHICore, Display, "ILDN block corrupt");
						continue;
					}
				...
				case '0VSP':
				{
					...
					if (RevisionByteSize >= sizeof(DXIL::FPSVRevision3Info))
					{
						...
						{
							UE_LOGF(LogRHICore, Error, "Pipeline state validation string table too small");
							continue;
						}
