...
namespace UE::RHICore
{
	...
	inline void UpdateUniformBufferConstants(void* DestinationData, const void* SourceData, const FRHIUniformBufferLayout& Layout, bool bAllowBindless = true)
	{
		...
		if (bAllowBindless)
		{
			...
			{
				if (Resource.MemberType == UBMT_DESCRIPTOR_RANGE)
				{
					if (FRHIDescriptorRange* DescriptorRange = Reader.Read<FRHIDescriptorRange*>(Resource))
					{
						FRHIDescriptorRangeShaderData ShaderData = DescriptorRange->GetShaderData();
						FMemory::Memcpy(reinterpret_cast<uint8*>(DestinationData) + Resource.MemberOffset, &ShaderData, sizeof(ShaderData));
					}
				}
				else
				{
