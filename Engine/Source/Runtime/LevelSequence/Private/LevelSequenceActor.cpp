...
#if WITH_EDITOR
	if (UDefaultLevelSequenceInstanceData* LevelSequenceInstanceData = Cast<UDefaultLevelSequenceInstanceData>(DefaultInstanceData))
	{
		LevelSequenceInstanceData->TransformOriginChangedEvent.AddUObject(this, &ALevelSequenceActor::TransformOriginChanged);
	}
#endif // WITH_EDITOR
...
#if WITH_EDITOR
bool ALevelSequenceActor::CanPreview(const UMovieSceneSequence& InSequence) const
{
	return GetSequence() == &InSequence;
}
#endif
...
void ALevelSequenceActor::BeginPlay()
{
	...
	if (GetSequencePlayer())
	{
		...
		GetSequencePlayer()->OnPlay.AddUniqueDynamic(this, &ALevelSequenceActor::ShowBurnin);
		GetSequencePlayer()->OnPlayReverse.AddUniqueDynamic(this, &ALevelSequenceActor::ShowBurnin);
		GetSequencePlayer()->OnStop.AddUniqueDynamic(this, &ALevelSequenceActor::HideBurnin);
	}
...
void ALevelSequenceActor::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	FName PropertyName = (PropertyChangedEvent.Property != nullptr) ? PropertyChangedEvent.Property->GetFName() : NAME_None;

	if (PropertyName == GET_MEMBER_NAME_CHECKED(ALevelSequenceActor, bOverrideInstanceData))
	{
...
void ALevelSequenceActor::TransformOriginChanged()
{
	if (LevelSequenceAsset)
	{
		if (UMovieScene* MovieScene = LevelSequenceAsset->GetMovieScene())
		{
