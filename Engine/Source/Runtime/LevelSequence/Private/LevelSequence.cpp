...
void PurgeLegacyBlueprints(UObject* InObject, UPackage* Package)
{
	if (UBlueprint* BP = Cast<UBlueprint>(InObject))
	{
...

		{
			...
			UE_LOGF(LogLevelSequence, Log, "Discarding blueprint '%ls' from package '%ls'.", *OldName, *Package->GetName());
		}
		...
		if (BP->GeneratedClass)
		{
			...
			if (SuperClass)
			{
				...
				UE_LOGF(LogLevelSequence, Log, "Discarding generated class '%ls' from package '%ls'. Replacing with redirector to '%ls'", *OldName.ToString(), *Package->GetName(), *SuperClass->GetName());
			}
			else
			{
				UE_LOGF(LogLevelSequence, Log, "Discarding generated class '%ls' from package '%ls'. Unable to create redirector due to no super class.", *OldName.ToString(), *Package->GetName());
			}
		}
	}
}
...
void ULevelSequence::PostLoad()
{
	...
	if (DirectorBlueprint)
	{
		...
		const int32 RenameFlags = REN_AllowPackageLinkerMismatch | REN_NonTransactional | REN_DoNotDirty | REN_DontCreateRedirectors;
		if (DirectorBlueprint->Rename(*GetDirectorBlueprintName(), nullptr, (RenameFlags | REN_Test)))
		{
			DirectorBlueprint->Rename(*GetDirectorBlueprintName(), nullptr, RenameFlags);
		}
	}
	...
	if (MovieScene)
	{
		...
		for (int32 Index = 0; Index < MovieScene->GetSpawnableCount(); ++Index)
		{
			...
			if (!Spawnable.GetObjectTemplate())
			{
				...
				UE_LOGF(LogLevelSequence, Warning, "Spawnable '%ls' with ID '%ls' does not have a valid object template", *Spawnable.GetName(), *Spawnable.GetGuid().ToString());
			}
		}
	}
	...
	if (GetLinkerCustomVersion(FSequencerObjectVersion::GUID) < FSequencerObjectVersion::PurgeSpawnableBlueprints)
	{
		// Remove any old generated classes from the package that will have been left behind from when we used blueprints for spawnables		
		{
			...
			GetObjectsWithOuter(Package, PackageSubobjects, EGetObjectsFlags::None);
...
bool ULevelSequence::ConvertOldSpawnables()
{
	...
	while (MovieScene->GetSpawnableCount() > 0)
	{
		...
		{
			UE_LOGF(LogLevelSequence, Warning, "Could not upgrade Spawnable '%ls' with ID '%ls'", *Spawnable.GetName(), *Spawnable.GetGuid().ToString());
			break;
		}
...
FGuid ULevelSequence::FindOrAddBinding(UObject* InObject)
{
	...
	{
		UE_LOGF(LogLevelSequence, Error, "Unable to possess object '%ls' because it's parent could not be bound.", *InObject->GetName());
		return FGuid();
	}
