...
void ALevelSequenceMediaController::Client_Play()
{
	...
	UE_LOGF(LogLevelSequence, Log, "Initiating playback of sequence '%ls' starting at time %fs", *Sequence->GetName(), StartTimeSeconds);
...
void ALevelSequenceMediaController::Client_ConditionallyForceTime(float DesyncThresholdSeconds)
{
	...
	if (Difference > DesyncThresholdSeconds)
	{
		...
		UE_LOGF(LogLevelSequence, Warning, "Forcibly synchronizing sequence '%ls' to time %f to server time (it is out by %fs).", *Sequence->GetName(), NewTimeSeconds, Difference);
	}
}
