...
UObject* FLevelSequenceActorSpawner::SpawnObject(FMovieSceneSpawnable& Spawnable, FMovieSceneSequenceIDRef TemplateID, TSharedRef<const UE::MovieScene::FSharedPlaybackState> SharedPlaybackState)
{
	...
	if (DesiredLevelName != NAME_None)
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogMovieScene, Warning, "Can't find sublevel '%ls' to spawn '%ls' into, defaulting to Persistent level", *DesiredLevelName.ToString(), *Spawnable.GetName());
			}
		}
	}
	...
	{
		UE_LOGF(LogMovieScene, Warning, "Can't find world to spawn '%ls' into, defaulting to Persistent level", *Spawnable.GetName());
