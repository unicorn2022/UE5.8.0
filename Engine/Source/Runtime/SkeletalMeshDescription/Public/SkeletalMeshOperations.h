...
public:
	...
	/**
	 * Prune the skin weights on the given profiles to match the bone weight settings.
	 * @param InOutMeshDescription The mesh description to modify.
	 * @param InBoneWeightSettings The bone weight settings to use for controlling the pruning. 
	 * @param InProfileNames The profile names to prune. If empty, will prune every profile. For only the default
	 *   profile, use FSkeletalMeshAttributes::DefaultSkinWeightProfileName
	 */
	static SKELETALMESHDESCRIPTION_API void PruneBoneWeights(
	...
	/**
	 * Splits the mesh into further polygon groups if the number of unique bones being bound to by all the triangles
	 * in that group exceeds the given maximum bone count. If a split happens, a new polygroup attribute is created 
	 * ("ParentPolygonGroup") that points to the polygon group ID that the split-off group got created from.
	 * @param InOutMeshDescription The mesh to partition.
	 * @param InMaxBoneCount The maximum number of unique bones in each group.
	 * @param InProfileNames The skin weight profiles to consider. If empty, will consider every profile. For only the 
	 *   default profile, use FSkeletalMeshAttributes::DefaultSkinWeightProfileName
	 */
	static SKELETALMESHDESCRIPTION_API void PartitionByMaximumUniqueBoneIndexCount(
		FMeshDescription& InOutMeshDescription,
	...
	/**
	 * Removes the specified bones, and its children, from the mesh on the given skin weight profiles. If the list of profiles
	 * is empty, then the bones are removed from all profiles. Any bones to remove that don't exist on the mesh will be ignored.
	 * All weights associated with removed bones are shifted to that bone's parent.
	 * If the mesh has no bone attributes, this operation is a no-op and the mesh is left unchanged.
	 * @note It is not allowed to remove the root bone.
	 * 
	 * @param InOutMeshDescription The skeletal mesh object to remove bones and bone bindings from.
	 * @param InBoneNames The list of bone names to remove.
	 * @param InBoneWeightSettings The settings to use when setting the new bone weights.
	 * @param InProfileNames The list of skin weight profiles to operate on. If empty, all skin weight profiles will be
	 *   processed.
	 */
	static SKELETALMESHDESCRIPTION_API void RemoveBones(
		FMeshDescription& InOutMeshDescription,
	...
	/**
	 * Removes any unused bones from the bone attribute list and adjust the parent indices accordingly. All profiles are checked
	 * for bones-in-use.
	 * 
	 * @param InOutMeshDescription The skeletal mesh object to remove unused bones from.
	 */
	static SKELETALMESHDESCRIPTION_API void RemoveUnusedBones(
		FMeshDescription& InOutMeshDescription
		);
};
