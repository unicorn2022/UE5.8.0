...
namespace MeshAttribute
{
	...
	namespace PolygonGroup
 	{
 		extern SKELETALMESHDESCRIPTION_API const FName ParentPolygonGroup;
 	}
...
public:
	...
	/**
	 * Returns the parent polygon group read-only attribute.
	 */
	TPolygonGroupAttributesConstRef<int32> GetPolygonGroupParents() const { return MeshDescriptionShared.PolygonGroupAttributes().GetAttributesRef<int32>(MeshAttribute::PolygonGroup::ParentPolygonGroup); }
...
public:
	...
	static bool IsReservedAttributeName(const FName InAttributeName)
	{
		return FStaticMeshAttributes::IsReservedAttributeName(InAttributeName) ||
			   ...
			   InAttributeName == MeshAttribute::Vertex::ImportPointIndex ||
			   InAttributeName == MeshAttribute::PolygonGroup::ParentPolygonGroup ||
	...
	/**
	 * Register an attribute on the polygon group that can be used for polygon groups that have been split to indicate which
	 * group they got split off from.
	 * @see FSkeletalMeshOperations::PartitionByMaximumUniqueBoneIndexCount
	 */
	SKELETALMESHDESCRIPTION_API bool RegisterPolygonGroupParentAttribute();

	/**
	 * Returns the parent polygon group attribute.
	 */
	TPolygonGroupAttributesRef<int32> GetPolygonGroupParents() { return MeshDescription.PolygonGroupAttributes().GetAttributesRef<int32>(MeshAttribute::PolygonGroup::ParentPolygonGroup); }
