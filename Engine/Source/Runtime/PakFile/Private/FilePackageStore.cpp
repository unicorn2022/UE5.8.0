...
bool FFilePackageStoreBackend::GetPackageRedirectInfo(FPackageId PackageId, FName& OutSourcePackageName, FPackageId& OutRedirectedToPackageId)
{
	...
	if (FindRedirect)
	{
		...
		UE_LOGF(LogFilePackageStore, Verbose, "Redirecting from %ls to 0x%ls", *OutSourcePackageName.ToString(), *LexToString(OutRedirectedToPackageId));
		return true;
	}
		...
		if (FindLocalizedPackageSourceName)
		{
			...
			if (!LocalizedPackageName.IsNone())
			{
				...
				if (PackageEntries.Find(LocalizedPackageId))
				{
					...
					UE_LOGF(LogFilePackageStore, Verbose, "Redirecting from localized package %ls to 0x%ls", *OutSourcePackageName.ToString(), *LexToString(OutRedirectedToPackageId));
					return true;
				}
			}
		}
...
TConstArrayView<uint32> FFilePackageStoreBackend::GetSoftReferences(FPackageId PackageId, TConstArrayView<FPackageId>& OutPackageIds)
{
	if (const FEntryHandle* Entry = PackageEntries.Find(PackageId); Entry != nullptr)
	{
		if (IsEmpty(*Entry))
		{
			OutPackageIds = TConstArrayView<FPackageId>();
		...
		if (Entry->HasSoftRefs)
		{
			...
			if (Container != nullptr)
			{
				...
				{
					UE_LOGF(LogFilePackageStore, Error, "Attempting to read softreference information for package %ls but container %ls never loaded softreference information", *LexToString(PackageId), *LexToString(Container->ContainerHeader->ContainerId));
				}
			...
			{
				UE_LOGF(LogFilePackageStore, Warning, "Failed to find container with soft references for package ID '%ls'", *LexToString(PackageId));
			}
		}
		else
		{
...
{
	LLM_SCOPE_BYNAME(TEXT("Filesystem/FilePackageStore"));
	...
	UE_LOGF(LogFilePackageStore, Log, "Mounting container: Id=%ls, Order=%u, NumPackages=%d",
...
FFilePackageStoreBackend::FEntryHandle FFilePackageStoreBackend::AddOldEntryData(FEntryHandle OldHandle, TConstArrayView<uint32> OldEntryData)
{
	...
	else
	{
		...
		uint32 NumShaderWords = OldHandle.HasShaderMaps ? 1 + GetNumWords<FShaderHash>(*It)  : 0;
...
TConstArrayView<FShaderHash> FFilePackageStoreBackend::GetShaderHashes(FEntryHandle Handle)
{
	...
	return MakeEntryDataSlice<FShaderHash>(Data + NumPackageIdWords, EntryData);
}
...
TConstArrayView<uint32> FFilePackageStoreBackend::GetSoftReferenceIndices(FEntryHandle Handle)
{
	...
	{
		NumPackageIdAndHashWords += (1 + GetNumWords<FShaderHash>(*(Data + NumPackageIdAndHashWords)));
	}
...
struct FFilePackageStoreEntryRef
{
	...
	friend uint32 GetTypeHash(FFilePackageStoreEntryRef Ref)
	{
		...
		for (const FShaderHash& ShaderMapHash : Ref.Entry->ShaderMapHashes)
		{
			Out = HashCombineFast(Out, static_cast<uint32>(ShaderMapHash.Hash));
		}
...
void FFilePackageStoreBackend::Update()
{
	...
	{	
		...
		for (FMountedContainer& MountedContainer : MountedContainers)
		{
			...
			{
				check(UniqueEntriesPerNewContainer.IsEmpty());
				...
				// Clear container for next use before clearing the referenced data to avoid use-after-free
		...
		UE_LOGF(LogFilePackageStore, Log, "Updated: NewPackages=%u, OldPackages=%u, TotalPackages=%u",
