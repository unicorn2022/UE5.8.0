...
)
{
	if (*RootNode != IntervalTreeInvalidIndex)
	{
		...
		//UE_LOGF(LogTemp, Warning, "Exploring Node %X [%d, %d] %d%d     interval %llX %llX    node interval %llX %llX   center %llX  ", *RootNode, CurrentShift, MaxShift, !!MinShifted, !!MaxShifted, MinInterval, MaxInterval, MinNode, MaxNode, Center);
		...
		if (!MinShifted)
		{
			if (CurrentShift == MaxShift)
			{
				//UE_LOGF(LogTemp, Warning, "LeftBottom %X [%d, %d] %d%d", *RootNode, CurrentShift, MaxShift, !!MinShifted, !!MaxShifted);
				ScanNodeListForRemovalFunc(&Root.LeftChildOrRootOfLeftList, Allocator, MinInterval, MaxInterval, Func);
			}
			else
			{
				//UE_LOGF(LogTemp, Warning, "LeftRecur %X [%d, %d] %d%d", *RootNode, CurrentShift, MaxShift, !!MinShifted, !!MaxShifted);
		...
		//UE_LOGF(LogTemp, Warning, "Center %X [%d, %d] %d%d", *RootNode, CurrentShift, MaxShift, !!MinShifted, !!MaxShifted);
		...
		if (MaxShifted)
		{
			if (CurrentShift == MaxShift)
			{
				//UE_LOGF(LogTemp, Warning, "RightBottom %X [%d, %d] %d%d", *RootNode, CurrentShift, MaxShift, !!MinShifted, !!MaxShifted);
				ScanNodeListForRemovalFunc(&Root.RightChildOrRootOfRightList, Allocator, MinInterval, MaxInterval, Func);
			}
			else
			{
				//UE_LOGF(LogTemp, Warning, "RightRecur %X [%d, %d] %d%d", *RootNode, CurrentShift, MaxShift, !!MinShifted, !!MaxShifted);
		...
		//UE_LOGF(LogTemp, Warning, "Done Exploring Node %X [%d, %d] %d%d     interval %llX %llX    node interval %llX %llX   center %llX  ", *RootNode, CurrentShift, MaxShift, !!MinShifted, !!MaxShifted, MinInterval, MaxInterval, MinNode, MaxNode, Center);
...
)
{
	while (Iter != IntervalTreeInvalidIndex)
	{
		...
		uint64 Offset = uint64(GetRequestOffset(Item.OffsetAndPakIndex));
		uint64 LastByte = Offset + uint64(Item.Size) - 1;
		//UE_LOGF(LogTemp, Warning, "Test Overlap %llu %llu %llu %llu", MinInterval, MaxInterval, Offset, LastByte);
		if (IntervalsIntersect(MinInterval, MaxInterval, Offset, LastByte))
		{
			//UE_LOGF(LogTemp, Warning, "Overlap %llu %llu %llu %llu", MinInterval, MaxInterval, Offset, LastByte);
			if (!Func(Iter))
			{
				return false;
			}
