...
static bool CheckIoStoreContainerBlockSignatures(const TCHAR* InContainerPath)
{
	...
	UE_LOGF(LogPakFile, Display, "Checking container file \"%ls\"...", InContainerPath);
	...
	if (!Status.IsOk())
	{
		UE_LOGF(LogPakFile, Error, "Failed reading toc file \"%ls\".", InContainerPath);
		return false;
	}

	if (TocResource.ChunkBlockSignatures.Num() != TocResource.CompressionBlocks.Num())
	{
		UE_LOGF(LogPakFile, Error, "Toc file \"%ls\" doesn't contain any chunk block signatures.", InContainerPath);
		return false;
	}
	...
	for (int32 BlockIndex = 0; BlockIndex < BlockCount; ++BlockIndex)
	{
		...
		if (PartitionIndex != LastPartitionIndex)
		{
			...
			{
				UE_LOGF(LogPakFile, Error, "Failed opening container file \"%ls\".", *UcasFilePath);
				return false;
			}
		...
		{
			UE_LOGF(LogPakFile, Warning, "Hash mismatch for block [%i/%i]! Expected %ls, Received %ls", BlockIndex, BlockCount, *TocResource.ChunkBlockSignatures[BlockIndex].ToString(), *BlockHash.ToString());
	...
	UE_LOGF(LogPakFile, Display, "Container file \"%ls\" checked in %.2fs", InContainerPath, ElapsedTime);
...
bool GPak_AllowMissingUToc = false;
static FAutoConsoleVariableRef CVar_AllowMissingUToc(
	TEXT("pak.AllowMissingUToc"),
	GPak_AllowMissingUToc,
	TEXT("Enables mounting a .pak file without the corresponding .utoc file even if the I/O dispatcher is enabled")
);
...
static bool MountFailsOnMissingUToc()
{
#if !UE_BUILD_SHIPPING
	static bool bAllowMissingUToc = FParse::Param(FCommandLine::Get(), TEXT("pak.AllowMissingUToc"));
	if (bAllowMissingUToc || GPak_AllowMissingUToc)
	{
		return false;
	}
#endif

	return FPakPlatformFile::bMountFailOnMissingUtoc;
}
...
public:
	...
	{
		bKeepIterating = bKeepIterating && Inner.Visit(*NormalizedFilename, bIsDir);
...
public:
		...
		bool bIsDir, FPakFile& PakFile) override
	{
		...
		bKeepIterating = bKeepIterating && Inner.Visit(*NormalizedFilename, StatData);
		return bKeepIterating;
	}
...
static FName GPakFakeCompression("PakFakeCompression");
	...
	struct FPakData
	{
			...
			, Signatures(nullptr)
		{
			...
			}
			{
				...
				//				UE_LOGF(LogTemp, Warning, "Test %d %llX %llX ", MaxShift, (uint64(PAK_CACHE_GRANULARITY) << (MaxShift + 1)), (uint64(PAK_CACHE_GRANULARITY) << MaxShift));
		...
		if (PakPrecacherSingleton)
		{
			...
			if (LocalPakPrecacherSingleton && LocalPakPrecacherSingleton == FPlatformAtomics::InterlockedCompareExchangePointer((void**)&PakPrecacherSingleton, nullptr, LocalPakPrecacherSingleton))
			{
				...
				while (!LocalPakPrecacherSingleton->IsProbablyIdle())
				{
					...
					{
						UE_LOGF(LogPakFile, Error, "FPakPrecacher was not idle after 10s, exiting anyway and leaking.");
						return;
					}
				}
...
//		GPakCache_MaxRequestsToLowerLevel = FMath::Max(FMath::Min(FPlatformMisc::NumberOfIOWorkerThreadsToSpawn(), GPakCache_MaxRequestsToLowerLevel), 1);
	...
	uint16* RegisterPakFile(FPakFile* InActualPakFile, FName File, int64 PakFileSize)
	{
		...
		if (!PakIndexPtr)
		{
			...
			UE_LOGF(LogPakFile, Log, "New pak file %ls added to pak precacher.", *PakFilename);
...
private: // below here we assume CachedFilesScopeLock until we get to the next section
	...
	{
		UE_LOGF(LogPakFile, VeryVerbose, "FPakReadRequest[%016llX, %016llX) ClearBlock", Block.OffsetAndPakIndex, Block.OffsetAndPakIndex + Block.Size);
			...
			while (BlockMemory > MemoryBudget)
			{
				...
				for (uint16 RealPakIndex = StartPakIndex; RealPakIndex < EndPakIndex; RealPakIndex++)
				{
					...
					if (CacheIndex < 0 || OffsetAndPakIndexOfSavedBlocked.Num() <= CacheIndex)
					{
						...
						UE_LOGF(LogPakFile, Error, "TrimCache1: Non-deleted Pak File %ls has invalid CacheIndex %d.", *CachedPakData[RealPakIndex].Name.ToString(), CacheIndex);
						continue;
					}
					...
					if (NumToRemove && OffsetAndPakIndexOfSavedBlocked[CacheIndex].Num())
					{
						...
						for (int32 Index = 0; Index < NumToRemove; Index++)
						{
								...
								[this](TIntervalTreeIndex BlockIndex) -> bool
							{
								...
								{
									UE_LOGF(LogPakFile, VeryVerbose, "FPakReadRequest[%016llX, %016llX) Discard Cached", Block.OffsetAndPakIndex, Block.OffsetAndPakIndex + Block.Size);
									ClearBlock(Block);
									return true;
								}
								return false;
			...
			if (GPakCache_TimeToTrim != 0.0f)
			{
				...
				for (uint16 RealPakIndex = StartPakIndex; RealPakIndex < EndPakIndex; RealPakIndex++)
				{
					...
					if (CacheIndex < 0 || OffsetAndPakIndexOfSavedBlocked.Num() <= CacheIndex)
					{
						...
						// Without the CacheIndex we will no longer be able to trim the cache. This isn't a problem if the PakFile
						// has actually been deleted, but that doesn't appear to be the case since Handle is still non-null.
						UE_LOGF(LogPakFile, Error, "TrimCache2: Non-deleted Pak File %ls has invalid CacheIndex %d.", *CachedPakData[RealPakIndex].Name.ToString(), CacheIndex);
						continue;
					}
					...
					if (OffsetAndPakIndexOfSavedBlocked[CacheIndex].Num())
					{
						for (int32 Index = 0; Index < OffsetAndPakIndexOfSavedBlocked[CacheIndex].Num(); Index++)
						{
								...
								[this, CurrentTime, &bRemovedAll](TIntervalTreeIndex BlockIndex) -> bool
							{
								...
								{
									UE_LOGF(LogPakFile, VeryVerbose, "FPakReadRequest[%016llX, %016llX) Discard Cached Based on Time", Block.OffsetAndPakIndex, Block.OffsetAndPakIndex + Block.Size);
									ClearBlock(Block);
									return true;
								}
								bRemovedAll = false;
		...
		else
		{
			...
			for (uint16 RealPakIndex = StartPakIndex; RealPakIndex < EndPakIndex; RealPakIndex++)
			{
				...
				if (CacheIndex < 0 || OffsetAndPakIndexOfSavedBlocked.Num() <= CacheIndex)
				{
					...
					// Without the CacheIndex we will no longer be able to trim the cache. This isn't a problem if the PakFile
					// has actually been deleted, but that doesn't appear to be the case since Handle is still non-null.
					UE_LOGF(LogPakFile, Error, "TrimCache3: Non-deleted Pak File %ls has invalid CacheIndex %d.", *CachedPakData[RealPakIndex].Name.ToString(), CacheIndex);
					continue;
				}
				int32 NumToKeep = bDiscardAll ? 0 : GPakCache_NumUnreferencedBlocksToCache;
				int32 NumToRemove = FMath::Max<int32>(0, OffsetAndPakIndexOfSavedBlocked[CacheIndex].Num() - NumToKeep);
	...
	void NotifyComplete(TIntervalTreeIndex RequestIndex)
	{
		...
		{
			UE_LOGF(LogPakFile, VeryVerbose, "FPakReadRequest[%016llX, %016llX) Notify complete", Request.OffsetAndPakIndex, Request.OffsetAndPakIndex + Request.Size);
...
#define CHECK_REDUNDANT_READS (0)
		...
		UE_LOGF(LogPakFile, VeryVerbose, "FPakReadRequest[%016llX, %016llX) StartBlockTask", Block.OffsetAndPakIndex, Block.OffsetAndPakIndex + Block.Size);
	...
	void CompleteRequest(bool bWasCanceled, uint8* Memory, TIntervalTreeIndex BlockIndex)
	{
		...
		if (Block.InRequestRefCount == 0 || bWasCanceled)
		{
			check(Block.Size > 0);
			FMemory::Free(Memory);
			UE_LOGF(LogPakFile, VeryVerbose, "FPakReadRequest[%016llX, %016llX) Cancelled", Block.OffsetAndPakIndex, Block.OffsetAndPakIndex + Block.Size);
			ClearBlock(Block);
		}
		else
		{
			...
			if (BlockMemory > BlockMemoryHighWater)
			{
				...
				if (BlockMemoryHighWater / 1024 / 1024 / 16 != LastPrint)
				{
					...
					UE_LOGF(LogPakFile, Log, "Precache HighWater %dMB\r\n", int32(LastPrint * 16));
				}
#endif
			}
...
public:
...
#endif
			UE_LOGF(LogPakFile, VeryVerbose, "FPakReadRequest[%016llX, %016llX) QueueRequest HOT", RequestOffsetAndPakIndex, RequestOffsetAndPakIndex + Request.Size);
		}
		else
		{
#if USE_PAK_PRECACHE && CSV_PROFILER_STATS
...
#endif
			UE_LOGF(LogPakFile, VeryVerbose, "FPakReadRequest[%016llX, %016llX) QueueRequest COLD", RequestOffsetAndPakIndex, RequestOffsetAndPakIndex + Request.Size);
		}
	...
	void Unmount(FName PakFile, FPakFile* UnmountedPak)
	{
		...
		for (TMap<FPakFile*, uint16>::TIterator It(CachedPaks); It; ++It)
		{
			if( It->Key->GetFilenameName() == PakFile )
			{
				...
				{
					UE_LOGF(LogPakFile, Log, "Pak file %ls removed from pak precacher.", *PakFile.ToString());
					if (Pak.ActualPakFile != UnmountedPak)
					{
						if (UnmountedPak)
						{
							UE_LOGF(LogPakFile, Warning, "FPakPrecacher::Unmount found multiple PakFiles with the name %ls. Unmounting all of them.", *PakFile.ToString());
						}
				...
				{
					UE_LOGF(LogPakFile, Log, "Pak file %ls was NOT removed from pak precacher because it had outstanding requests.", *PakFile.ToString());
				}
	...
	void DumpBlocks()
	{
		...
		if (!bDone)
		{
			UE_LOGF(LogPakFile, Log, "PakCache has outstanding requests with %llu total memory.", BlockMemory);
		}
		else
		{
			UE_LOGF(LogPakFile, Log, "PakCache has no outstanding requests with %llu total memory.", BlockMemory);
		}
	}
};
...
static void WaitPrecache(const TArray<FString>& Args)
{
	...
	UE_LOGF(LogPakFile, Log, "Loaded %4d blocks (align %4dKB) totalling %7.2fMB in %4.2fs   = %6.2fMB/s", Loads, PAK_CACHE_GRANULARITY / 1024, LoadSizeMB, TimeSpent, MBs);
}
...
public:
	...
	void PanicSyncRead(uint8* Buffer)
	{
		...
		UE_CLOGF(!Handle, LogPakFile, Fatal, "PanicSyncRead failed to open pak file %ls", *PanicPakFile.ToString());
		if (!Handle->Seek(Offset))
		{
			UE_LOGF(LogPakFile, Fatal, "PanicSyncRead failed to seek pak file %ls   %lld bytes at %lld ", *PanicPakFile.ToString(), BytesToRead, Offset);
		}

		if (!Handle->Read(Buffer, BytesToRead))
		{
			UE_LOGF(LogPakFile, Fatal, "PanicSyncRead failed to read pak file %ls   %lld bytes at %lld ", *PanicPakFile.ToString(), BytesToRead, Offset);
		}	
		delete Handle;
	}
};
...
public:
	...
	{
		UE_CLOGF(!bCompleteAndCallbackCalled, LogPakFile, Fatal, "IAsyncReadRequests must not be deleted until they are completed.");
	...
	for (int64 SignedChunkIndex = 0; SignedChunkIndex < NumSignaturesToCheck; ++SignedChunkIndex, ++SignatureIndex)
	{
...

		{
			...
			if (!bChunkHashesMatch)
			{
				...
				UE_LOGF(LogPakFile, Warning, "Pak chunk signing mismatch on chunk [%i/%i]! Expected %ls, Received %ls", SignatureIndex, PakData->Signatures->ChunkHashes.Num() - 1, *ChunkHashToString(PakData->Signatures->ChunkHashes[SignatureIndex]), *ChunkHashToString(ThisHash));
				...
				{
					UE_LOGF(LogPakFile, Warning, "Principal signature table has changed since initialization!");
				}
...
public:
		...
		, EncryptionKeyGuid(InPakFile->GetInfo().EncryptionKeyGuid)
	{
		...
		UE_LOGF(LogPakFile, VeryVerbose, "FPakPlatformFile::OpenAsyncRead[%016llX, %016llX) %ls", OffsetInPak, OffsetInPak + CompressedFileSize, Filename);
	...
	~FPakAsyncReadFileHandle()
	{
		...
		{
			UE_LOGF(LogPakFile, Fatal, "LiveRequests.Num or NumLiveRawReqeusts was > 0 in ~FPakAsyncReadFileHandle!");
		}
	...
	void DoProcessing(FCachedAsyncBlock* BlockPtr)
	{
		...
		if (Block.Raw)
		{
			...
			if (GPakCache_ForceDecompressionFails && FMath::FRand() < 0.001f)
			{
				...
				{
					UE_LOGF(LogPakFile, Error, "Forcing corruption of decompression source data (predecryption) to verify panic read recovery.  Offset = %d, Value = 0x%x", CorruptOffset, int32(CorruptValue));
			...
			{
				UE_LOGF(LogPakFile, Error, "The payload was corrupted, but this did not trigger a decompression failed.....pretending it failed anyway because otherwise it can crash later.");
				bFailed = true;
			}
#endif
			...
			{
				{
					...
					UE_LOGF(LogPakFile, Error, "Pak Decompression failed. PakFile:%ls, EntryOffset:%lld, EntrySize:%lld, Method:%ls, ProcessedSize:%d, RawSize:%d, Crc32:%u, BlockIndex:%d, Encrypt:%d, Delete:%d, Output:%p, Raw:%p, Processed:%p, Bytes:[%ls...]",
				...
				{
					FScopeLock ScopedLock(&CriticalSection);
					UE_CLOGF(!Block.RawRequest, LogPakFile, Fatal, "Cannot retry because Block.RawRequest is null.");
				...
				{
					UE_LOGF(LogPakFile, Warning, "Panic re-read (and decrypt if applicable) resulted in a different buffer.");
					...
					UE_CLOGF(Offset >= Block.DecompressionRawSize, LogPakFile, Fatal, "Buffers were different yet all bytes were the same????");

					UE_LOGF(LogPakFile, Warning, "Buffers differ at offset %d.", Offset);
					const FString HexBytes1 = BytesToHex(Block.Raw + Offset, FMath::Min(Block.DecompressionRawSize - Offset, 64));
					UE_LOGF(LogPakFile, Warning, "Original read (and decrypt) %ls", *HexBytes1);
					const FString HexBytes2 = BytesToHex(TempBuffer + Offset, FMath::Min(Block.DecompressionRawSize - Offset, 64));
					UE_LOGF(LogPakFile, Warning, "Panic reread  (and decrypt) %ls", *HexBytes2);
				}
				if (!FCompression::UncompressMemory(CompressionMethod, Output, Block.ProcessedSize, TempBuffer, Block.DecompressionRawSize))
				{
					UE_LOGF(LogPakFile, Fatal, "Retry was NOT sucessful.");
				}
				else
				{
					UE_LOGF(LogPakFile, Warning, "Retry was sucessful.");
				}
	...
	void RemoveRequest(FPakProcessedReadRequest* Req, int64 Offset, int64 BytesToRead, const bool& bAlreadyCancelled)
	{
		...
		for (int32 BlockIndex = FirstBlock; BlockIndex <= LastBlock; BlockIndex++)
		{
			...
			if (!--Block.RefCount)
			{
				...
				UE_CLOGF(Block.bInFlight && !Block.bCPUWorkIsComplete, LogPakFile, Fatal, "RemoveRequest called on Request that still has a block in processing.");
				if (Block.RawRequest)
				{
...
public:
		...
		, FileEntry(*InFileEntry)
	{
		...
		UE_LOGF(LogPakFile, VeryVerbose, "FPakPlatformFile::OpenAsyncRead (FBypassPakAsyncReadFileHandle)[%016llX, %016llX) %ls", OffsetInPak, OffsetInPak + CompressedFileSize, Filename);
		check(PakFileSize > 0 && OffsetInPak + CompressedFileSize <= PakFileSize && OffsetInPak >= 0);
...
{
	FString TestFile("../../../Engine/Config/BaseDeviceProfiles.ini");
	if (Args.Num() > 0)
	{
		TestFile = Args[0];
	}
...
bool FPakEntry::VerifyPakEntriesMatch(const FPakEntry& FileEntryA, const FPakEntry& FileEntryB)
{
	...
	{
		UE_LOGF(LogPakFile, Error, "Pak header file size mismatch, got: %lld, expected: %lld", FileEntryB.Size, FileEntryA.Size);
		bResult = false;
	}
	if (FileEntryA.UncompressedSize != FileEntryB.UncompressedSize)
	{
		UE_LOGF(LogPakFile, Error, "Pak header uncompressed file size mismatch, got: %lld, expected: %lld", FileEntryB.UncompressedSize, FileEntryA.UncompressedSize);
		bResult = false;
	}
	if (FileEntryA.CompressionMethodIndex != FileEntryB.CompressionMethodIndex)
	{
		UE_LOGF(LogPakFile, Error, "Pak header file compression method mismatch, got: %d, expected: %d", FileEntryB.CompressionMethodIndex, FileEntryA.CompressionMethodIndex);
		bResult = false;
	}
	if (FMemory::Memcmp(FileEntryA.Hash, FileEntryB.Hash, sizeof(FileEntryA.Hash)) != 0)
	{
		UE_LOGF(LogPakFile, Error, "Pak file hash does not match its index entry");
		bResult = false;
	}
	return bResult;
}
...
FPakPlatformFile::~FPakPlatformFile()
{
	UE_LOGF(LogPakFile, Log, "Destroying PakPlatformFile");
...
bool FPakPlatformFile::IsNonPakFilenameAllowed(const FString& InFilename)
{
...
#if !UE_BUILD_SHIPPING
	// Always test the ExcludedNonPakExtensions in shipping builds, because we always expect pak files in shipping builds
	// and we therefore are not allowed to skip the test.
	// But development might be running without pakfiles; we need to allow otherwise-excluded extensions in development
	// builds that are not using pakfiles.
	if (bProcessHasEverMountedAPakFile.load(std::memory_order_relaxed))
#endif // !UE_BUILD_SHIPPING
	{
		FName Ext = FName(*FPaths::GetExtension(InFilename));
		bAllowed = !ExcludedNonPakExtensions.Contains(Ext);
		UE_CLOGF(!bAllowed, LogPakFile, VeryVerbose, "Access to file '%ls' is limited to pak contents due to file extension being listed in ExcludedNonPakExtensions.", *InFilename)
	}
#endif // EXCLUDE_NONPAK_UE_EXTENSIONS
	...
	if (bIsIniFile && FParse::Value(FCommandLine::Get(), TEXT("-iniFile="), FileList, false))
	{
		...
		for (int32 Index = 0; Index < Files.Num(); Index++)
		{
			...
			{
				bAllowed = true;
				UE_LOGF(LogPakFile, Log, " Override -inifile: %ls", *InFilename);
				break;
			}
		}
	}
...
template<typename OpenOptionsType>
FOpenMappedResult TOpenMappedExImpl(FPakPlatformFile* pThis, const TCHAR* Filename, OpenOptionsType OpenOptions, int64 MaximumSize,
	FOpenMappedResult(IPlatformFile::*LowLevelOpenMappedEx)(const TCHAR*, OpenOptionsType, int64))
{
	// MSVC's static analysis doesn't see that Result is initialized
MSVC_PRAGMA(warning(push))
MSVC_PRAGMA(warning(disable : 6001))
...
#if !UE_BUILD_SHIPPING
	if (pThis->bLookLooseFirst && pThis->IsNonPakFilenameAllowed(Filename))
	{
		FOpenMappedResult Result = (pThis->LowerLevel->*LowLevelOpenMappedEx)(Filename, OpenOptions, MaximumSize);
		if (Result.HasValue())
		{
	...
	if (pThis->FindFileInPakFiles(Filename, &PakEntry, &FileEntry) && PakEntry.IsValid())
	{
		...
		{
			FOpenMappedResult Result = (pThis->LowerLevel->*LowLevelOpenMappedEx)(*PakEntry->GetFilename(), OpenOptions, MaximumSize);
			if (Result.HasError())
			{
	...
	if (pThis->IsNonPakFilenameAllowed(Filename))
	{
		return (pThis->LowerLevel->*LowLevelOpenMappedEx)(Filename, OpenOptions, MaximumSize);
	}
...
{
	return TOpenMappedExImpl(this, Filename, OpenOptions, MaximumSize, &IPlatformFile::OpenMappedEx);
}

FOpenMappedResult FPakPlatformFile::OpenMappedEx2(const TCHAR* Filename, EOpenMappedFlags OpenOptions, int64 MaximumSize)
{
	return TOpenMappedExImpl(this, Filename, OpenOptions, MaximumSize, &IPlatformFile::OpenMappedEx2);
}
...
TSharedPtr<const FPakSignatureFile, ESPMode::ThreadSafe> FPakPlatformFile::GetPakSignatureFile(const TCHAR* InFilename)
{
	...
	if (PublicKey != InvalidRSAKeyHandle)
	{
		...
		{
			UE_LOGF(LogPakFile, Warning, "Couldn't find pak signature file '%ls'", InFilename);
...
void FPakPlatformFile::GetPakEncryptionKey(FAES::FAESKey& OutKey, const FGuid& InEncryptionKeyGuid)
{
	...
	if (!UE::FEncryptionKeyManager::Get().TryGetKey(InEncryptionKeyGuid, OutKey))
	{
		...
		{
			UE_LOGF(LogPakFile, Fatal, "Failed to find requested encryption key %ls", *InEncryptionKeyGuid.ToString());
		}
	}
}
	...
	IPlatformFile::FDirectoryStatVisitor& Visitor, bool bRecursive)
{
	...
	if (!PakVisitor.bKeepIterating)
	{
		return false;
	}
	bool bExistsInPakFiles = Result;
	if (!LowerLevel->DirectoryExists(Directory))
	{
		return bExistsInPakFiles;
	}
	...
	// LowerLevel bDirectoryExists == true, so
	// 1) Our contract to return (bKeepIterating && bDirectoryExists) therefore reduces to return (bKeepIterating).
	// 2) LowerLevel IterateDirectoryStat return value also reduces to bKeepIterating.
	...
	IPlatformFile::FDirectoryVisitor& Visitor, bool bRecursive)
{
	...
	// LowerLevel bDirectoryExists == true, so
	// 1) Our contract to return (bKeepIterating && bDirectoryExists) therefore reduces to return (bKeepIterating).
	// 2) LowerLevel IterateDirectory return value also reduces to bKeepIterating.
	// So just return Result.
	return Result;
}
	...
	UE::PakFile::Private::FPakFileDirectoryVisitorBase& Visitor, bool bRecursive, TSet<FString>& FilesVisitedInPak)
{
	...
	bool bPathExistsInPak = false;
	// Iterate pak files first
	FString NormalizationBuffer;
	TSet<FString> FilesVisitedInThisPak;
	auto ShouldVisit = [&Visitor, &bPathExistsInPak](FStringView UnnormalizedPath)
	{
		// This calculation of bPathExistsInPak is currently correct but depends on bIncludeFiles=true and
		// bIncludeFolders=true both being set. We need to report whether the queried directory exists in the pak
		// file at all, but we only get a call to our ShouldVisit function here if it has files or folders in it.
		// For pak files it is equivalent to say that PathExists <=> ItHasFilesOrFoldersInIt, so our equivalency
		// that ShouldVisitIsCalled <=> bPathExistsInPak is valid.
		// But if we were to set bIncludeFiles=false or bIncludeFolders=false, we would skip being called for
		// in some cases even when PathExists, and that would break the equivalency.
		// We rely on this behavior rather than changing FindPrunedFilesAtPathInternal to return whether the path
		// exists because determining whether the path exists is expensive when FindPrunedFilesAtPathInternal is called
		// for other purposes that don't care about bPathExistsInPak and want to skip the enumeration of files
		// or folders.
		bPathExistsInPak = true;
	...
	for (int32 PakIndex = 0; PakIndex < Paks.Num(); PakIndex++)
	{
		...
		for (TSet<FString>::TConstIterator SetIt(FilesVisitedInThisPak); SetIt; ++SetIt)
		{
			...
			if (!FilesVisitedInPak.Contains(*NormalizedFilename))
			{
				...
				if (!Visitor.Visit(Filename, *NormalizedFilename, bIsDir, PakFile))
				{
					return false;
				}
			}
	...
	TRefCountPtr<FPakFile>* OutPakFile, FPakEntry* OutEntry)
{
	...
	for (int32 PakIndex = 0; PakIndex < Paks.Num(); PakIndex++)
	{
		...
		if (FindResult == FPakFile::EFindResult::Found)
		{
			...
			UE_CLOGF(!PaksWithDeleteRecord.IsEmpty(), LogPakFile, Verbose,
				"Delete Record: Ignored delete record for %ls - found it in %ls instead (asset was moved or duplicated between chunks)",
		...
		else if (FindResult == FPakFile::EFindResult::FoundDeleted)
		{
			...
			UE_LOGF(LogPakFile, Verbose, "Delete Record: Found a delete record for %ls in %ls",
				Filename, *PakFile->GetFilename());
		}
	}
	...
	{
		UE_CLOGF(bFoundOlderVersionOfDeleteRecordPak, LogPakFile, Verbose,
			"Delete Record: Accepted a delete record for %ls", Filename);
		UE_CLOGF(!bFoundOlderVersionOfDeleteRecordPak, LogPakFile, Warning,
			"Delete Record: No lower priority pak files looking for %ls. (maybe not downloaded?)", Filename);
	}
	return false;
}
...
bool FPakPlatformFile::ShouldBeUsed(IPlatformFile* Inner, const TCHAR* CmdLine) const
{
	...
	if (FParse::Param(CmdLine, TEXT("UsePaks")))
	{
		...
		{
			UE_LOGF(LogPakFile, Log, "No Pak files were found when checking to make Pak Environment");
		}
		return true;
	}
...
{
	UE_LOGF(LogPakFile, Log, "Initializing PakPlatformFile");
	...
	ExcludedNonPakExtensions.Add(TEXT("metalmap"));
#endif
	...
	if (FIoDispatcher::IsInitialized())
	{
		...
		if (bShouldMountGlobal || PakPlatformFile_IsForceUseIoStore(CmdLine))
		{
			...
			if (bShouldMountGlobal)
			{
				...
				{
					UE_LOGF(LogPakFile, Display, "Initialized I/O dispatcher file backend. Mounted the global container: %ls", *GlobalUTocPath);
					...
					FIoDispatcherInternal::MountContainerMeta(GlobalUTocPath);
				}
				else
				{
					UE_LOGF(LogPakFile, Error, "Initialized I/O dispatcher file backend. Failed to mount the global container: '%ls'", *IoDispatcherMountStatus.Status().ToString());
				}
			}
			else
			{
				UE_LOGF(LogPakFile, Display, "Initialized I/O dispatcher file backend. Running with -useiostore without the global container.");
			}
		}
	}
...
#endif
	{
		UE_CLOGF(FParse::Param(FCommandLine::Get(), TEXT("FileOpenLog")), LogPakFile, Display, "Disabled pak precacher to get an accurate load order. This should only be used to collect gameopenorder.log, as it is quite slow.");
		GPakCache_Enable = 0;
	}
#endif
}
...
void FPakPlatformFile::OptimizeMemoryUsageForMountedPaks()
{
	...
	{
		UE_LOGF(LogPakFile, Warning,
			"The UnloadPakEntryFilenamesIfPossible has been deprecated and is no longer sufficient to specify the unloading of pak files.\n"
			"The choice to not load pak files is now made earlier than Ini settings are available.\n"
			"To specify that filenames should be removed from the runtime PakFileIndex, use the new runtime delegate FPakPlatformFile::GetPakSetIndexSettingsDelegate().\n"
			"In a global variable constructor that executes before the process main function, bind this delegate to a function that sets the output bool bKeepFullDirectory to false.\n"
			"See FShooterPreMainCallbacks in Samples\\Games\\ShooterGame\\Source\\ShooterGame\\Private\\ShooterGameModule.cpp for an example binding.");
	}
#endif
	...
	UE_LOGF(LogPakFile, Log, "AllPaks IndexSizes: DirectoryHashSize=%llu, PathHashSize=%llu, EntriesSize=%llu, DirTreeIndexSize=%llu, TotalSize=%llu",
...
bool FPakPlatformFile::Mount(const FPakMountArgs& MountArgs, FIoStatus* OutIoMountStatus /*= nullptr*/, FPakListEntry* OutPakListEntry /*= nullptr*/)
{
	...
	if (LowerLevel->FileExists(InPakFilename))
	{
		...
		if (Pak.GetReference()->IsValid())
		{
			...
			{
				UE_LOGF(LogPakFile, Display, "Deferring mount of pak \"%ls\" until encryption key '%ls' becomes available", InPakFilename, *Pak->GetInfo().EncryptionKeyGuid.ToString());
				...
				Entry.MountOptions = MountArgs.MountOptions;
				Entry.bLoadIndex = bLoadIndex;
		...
		{
			UE_LOGF(LogPakFile, Warning, "Failed to mount pak \"%ls\", pak is invalid.", InPakFilename);
		...
		else
		{
			...
			else
			{
				...
				{
					FIoDispatcherInternal::MountContainerMeta(UtocPath);
					...
					if (MountResult.IsOk())
					{
						...
						if (FPlatformFileManager::Get().GetPlatformFile().FileExists(*OptionalSegmentUtocPath))
						{
							...
							if (MountResult.IsOk())
							{
								...
								UE_LOGF(LogPakFile, Display, "Mounted IoStore container \"%ls\"", *UtocPath);
								...
								UE_LOGF(LogPakFile, Display, "Mounted optional segment extension IoStore container \"%ls\"", *OptionalSegmentUtocPath);
							}
							else
							{
								UE_LOGF(LogPakFile, Warning, "Failed to mount optional segment extension IoStore container \"%ls\" [%ls]", *OptionalSegmentUtocPath, *MountResult.Status().ToString());
					...
					{
						UE_LOGF(LogPakFile, Warning, "Failed to mount IoStore container \"%ls\" [%ls]", *UtocPath, *MountResult.Status().ToString()); 
						FCoreDelegates::PakFileMountFailedDelegate.Broadcast(UtocPath);
					}
				}
				else
				{
					if (UE::PakFile::Private::MountFailsOnMissingUToc())
					{
						IoStoreSuccess = FIoStatus(EIoErrorCode::NotFound, TEXT("utoc not found"));
						UE_LOGF(LogPakFile, Warning, "IoStore container \"%ls\" not found", *UtocPath);
						FCoreDelegates::PakFileMountFailedDelegate.Broadcast(UtocPath);
					}
					else
					{
			...
			{
				{
					...
					PakFiles.Add(Entry);
#if !UE_BUILD_SHIPPING
					bProcessHasEverMountedAPakFile.store(true, std::memory_order_relaxed);
#endif
					PakFiles.StableSort();
				...
				UE_LOGF(LogPakFile, Display, "Mounted Pak file '%ls', mount point: '%ls'", InPakFilename, *Pak->GetMountPoint());
				UE_LOGF(LogPakFile, Verbose, "OnPakFileMounted2Time == %lf", OnPakFileMounted2Time);
					...
					if (!FPackageName::TryConvertFilenameToLongPackageName(MountPoint, OutPackageName))
					{
						...
						if (!bParentOfMountPoint)
						{
							UE_LOGF(LogPakFile, Display,
								"Mount point '%ls' is not mounted to a valid Root Path yet, " "assets in this pak file may not be accessible until a corresponding UFS Mount Point is added through FPackageName::RegisterMountPoint.",
								*MountPoint);
						}
					}
				}
	...
	{
		UE_LOGF(LogPakFile, Warning, "Failed to open pak \"%ls\"", InPakFilename);
...
int32 FPakPlatformFile::MountAllPakFiles(const TArray<FString>& PakFolders, const FString& WildCard)
{
	...
	if (bMountPaks)
	{
		...
		for (int32 PakFileIndex = 0; PakFileIndex < FoundPakFiles.Num(); PakFileIndex++)
		{
			...
			UE_LOGF(LogPakFile, Display, "Found Pak file %ls attempting to mount.", *PakFilename);
			...
			{
				UE_LOGF(LogPakFile, Display, "Pak file %ls already exists.", *PakFilename);
				continue;
			}

			uint32 PakOrder = GetPakOrderFromPakFilePath(PakFilename);

			UE_LOGF(LogPakFile, Display, "Mounting pak file %ls.", *PakFilename);
	...
	TArrayView<UE::FMountPaksExArgs> MountArgs)
{
	...
	for (UE::FMountPaksExArgs& Args : MountArgs)
	{
		...
		{
			UE_LOGF(LogPakFile, Warning, "Pak file %ls already exists, skipping mount.", Args.PakFilePath);
		...
		else
		{
			...
			UE::UnifiedError::FError Error = UE::IoStore::ConvertError(IoMountStatus);
...
void FPakPlatformFile::RegisterEncryptionKey(const FGuid& InGuid, const FAES::FAESKey& InKey)
{
	...
	{
		const TArray<FPakListDeferredEntry> LocalPendingEncryptedPakFiles = PendingEncryptedPakFiles;
		for (const FPakListDeferredEntry& Entry : LocalPendingEncryptedPakFiles)
		{
			if (Entry.EncryptionKeyGuid == InGuid)
			{
				FPakMountArgs MountArgs
				{
					.PakFilename = *Entry.Filename,
					.PakOrder = Entry.ReadOrder,
					.Path = Entry.Path.Len() == 0 ? nullptr : *Entry.Path,
					.MountOptions = Entry.MountOptions,
					.bLoadIndex = Entry.bLoadIndex
				};

				if (Mount(MountArgs))
				{
					UE_LOGF(LogPakFile, Log, "Successfully mounted deferred pak file '%ls'", *Entry.Filename);
					NumMounted++;
				...
				{
					UE_LOGF(LogPakFile, Warning, "Failed to mount deferred pak file '%ls'", *Entry.Filename);
				}
			}
		}
	}
	...
	if (NumMounted > 0)
	{
		...
		UE_LOGF(LogPakFile, Log, "Registered encryption key '%ls': %d pak files mounted, %d remain pending", *InGuid.ToString(), NumMounted, PendingEncryptedPakFiles.Num());
	}
}
...
static void AsyncFileTest(const TArray<FString>& Args)
{
	...
	if (Args.Num() == 0)
	{
		UE_LOGF(LogPakFile, Error, "pak.AsyncFileTest requires a filename argument: \"pak.AsyncFileTest <filename> <size> <offset>\"");
		return;
	}
	...
	if (Args.Num() > 1)
	{
		...
		if (Size <= 0)
		{
			UE_LOGF(LogPakFile, Error, "pak.AsyncFileTest size must be > 0: \"pak.AsyncFileTest <filename> <size> <offset>\"");
			return;
		}
	}
	...
	if (Args.Num() > 2)
	{
		...
		if (Size < 0)
		{
			UE_LOGF(LogPakFile, Error, "pak.AsyncFileTest offset must be >= 0: \"pak.AsyncFileTest <filename> <size> <offset>\"");
			return;
		}
	}
	...
	check(FileHandle);
	{
		...
		if (!SizeRequest)
		{
			UE_LOGF(LogPakFile, Error, "pak.AsyncFileTest: SizeRequest failed for %ls.", *TestFile);
			return;
		}
