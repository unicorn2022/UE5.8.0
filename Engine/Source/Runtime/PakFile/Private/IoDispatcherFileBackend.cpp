...
float GFileIoStoreUnmountTimeoutSeconds = 5.0;
static FAutoConsoleVariableRef CVar_FileIoStoreUnmountTimeoutSeconds(
	TEXT("iostore.UnmountTimeoutSeconds"),
	GFileIoStoreUnmountTimeoutSeconds,
	TEXT("Number of seconds (1-10) to wait for pending reads before unmounting containers."),
	ECVF_Default
);

bool GFileIoStoreAsyncUnmountEnabled = true;
static FAutoConsoleVariableRef CVar_FileIoStoreAsyncUnmountEnabled(
	TEXT("iostore.AsyncUnmountEnabled"),
	GFileIoStoreAsyncUnmountEnabled,
	TEXT("When enabled containers will be unloaded from memory asynchronously from unmount.")
);
...
{
public:
	FMappedFileProxy(IMappedFileHandle* InSharedMappedFileHandle, uint64 InSize, uint64 InPartitionSize)
	...
	{
		// The offset we get is the TocOffset for the container, and we need the offset into our partition.
		if (PartitionSize)
		{
...
{
	UE::TUniqueLock Lock(MappedFileHandleMutex);
	delete MappedFileHandle.load(std::memory_order_relaxed);
}

IMappedFileHandle* FFileIoStoreContainerFilePartition::BorrowMappedFileHandle()
{
	IMappedFileHandle* Handle = MappedFileHandle.load(std::memory_order_acquire);
	if (UNLIKELY(Handle == nullptr))
	{
		UE::TUniqueLock Lock(MappedFileHandleMutex);
		Handle = MappedFileHandle.load(std::memory_order_relaxed);
		if (UNLIKELY(Handle == nullptr))
		{
			IPlatformFile& Ipf = FPlatformFileManager::Get().GetPlatformFile();
			FOpenMappedResult Result = Ipf.OpenMappedEx2(*FilePath);
			if (!Result.HasError())
			{
				Handle = Result.StealValue().Release();
				MappedFileHandle.store(Handle, std::memory_order_release);
			}
			else
			{
				UE_LOGF(LogIoDispatcher, Error, "OpenMappedEx failed on: %ls", *FilePath);
			}
		}
	}
	return Handle;
...
{
	UE_TRACE_METADATA_SCOPE_ASSET_FNAME(FName(InTocFilePath), FName("FileIoStoreReader"), FName(InTocFilePath));
	...
	UE_LOGF(LogIoDispatcher, Verbose, "Reading toc: %ls", InTocFilePath);
	...
	{
		UE_LOGF(LogIoDispatcher, Warning, "Falling back to imperfect hashmap for container '%ls'", InTocFilePath);
	...
	UE_LOGF(LogIoDispatcher, Verbose, "Toc loaded : %ls, Id=%ls, Order=%d, EntryCount=%u, SignatureHash=%ls",
...
TUniquePtr<IMappedFileHandle> FFileIoStoreReader::GetMappedContainerFileHandle(uint64 TocOffset)
{
	check(!bClosed);
	...
	if (!EnumHasAnyFlags(ContainerFile.ContainerFlags, EIoContainerFlags::Encrypted))
	{
		MappedFileHandle = Partition.BorrowMappedFileHandle();
	}

	check(Partition.FileSize > 0);
	return TUniquePtr<IMappedFileHandle>(new FMappedFileProxy(MappedFileHandle, Partition.FileSize, ContainerFile.PartitionSize));
}
...
TIoStatusOr<FIoContainerHeader> FFileIoStoreReader::ReadContainerHeader(bool bReadSoftRefs) const
{
	...
	if (Ar.IsError())
	{
		UE_LOGF(LogIoDispatcher, Warning, "Invalid container header in file '%ls'", *ContainerFile.FilePath);
...
FFileIoStoreResolvedRequest::FFileIoStoreResolvedRequest(
	...
	, Priority(InPriority)
{
	ContainerFile->RefCount.fetch_add(1, std::memory_order_acq_rel);
}
...
FFileIoStoreReadersToUnload::~FFileIoStoreReadersToUnload()
{
	TRACE_CPUPROFILER_EVENT_SCOPE(WaitOnReferencedContainers);

	const double MaxWaitTime = FMath::Clamp(GFileIoStoreUnmountTimeoutSeconds, 1.0, 10.0);
	...
	for (;;)
	{
		int32 Pending = 0;
		{
			...
			for (TUniquePtr<FFileIoStoreReader>& Reader : Readers)
			{
				if (Reader->GetContainerFile()->RefCount.load(std::memory_order_acquire) > 0)
				{
					Pending++;
				}
			}
		...
		if (double WaitTime = FPlatformTime::Seconds() - StartTime; WaitTime > MaxWaitTime)
		{
			UE_LOGF(LogIoDispatcher, Error, "Shutting down file I/O dispatcher backend with %d referenced container(s).", Pending);
			break;
		}
...
void FFileIoStoreReadersToUnload::Add(TUniquePtr<FFileIoStoreReader>&& Reader)
{
	UE::TUniqueLock Lock(Mutex);
	Readers.Add(MoveTemp(Reader));
}

void FFileIoStoreReadersToUnload::TryUnload()
{
	TArray<TUniquePtr<FFileIoStoreReader>> ToUnload;
	{
		UE::TUniqueLock Lock(Mutex);
		for (auto It = Readers.CreateIterator(); It; ++It)
		{
			TUniquePtr<FFileIoStoreReader>& Reader	= *It;
			FFileIoStoreContainerFile& Container	= *Reader->GetContainerFile();
			const int32 RefCount					= Container.RefCount.load(std::memory_order_acquire);

			if (RefCount > 0)
			{
				UE_LOGF(LogIoDispatcher, Warning, "Waiting on %d pending I/O request before unloading container '%ls'",
					RefCount, *Container.FilePath);
			}
			else
			{
				check(RefCount == 0);
				ToUnload.Add(MoveTemp(Reader));
				It.RemoveCurrent();
			}
		}
	}

	for (TUniquePtr<FFileIoStoreReader>& Reader : ToUnload)
	{
		UE_LOGF(LogIoDispatcher, Display, "Unloading container '%ls'", *Reader->GetContainerFile()->FilePath);
	}
}
	...
	UE::IoStore::ETocMountOptions Options)
{
	...
	int32 InsertionIndex;
	{
		...
		const EIoContainerFlags ContainerFlags = Reader->GetContainerFile() != nullptr ? Reader->GetContainerFile()->ContainerFlags : EIoContainerFlags::None;
		IoStoreReaders.Insert(MoveTemp(Reader), InsertionIndex);
		UE_LOGF(LogIoDispatcher, Display, "Mounted container '%ls', Id='%ls', Order=%d, Slot=%d, Options=(%ls), Flags=(%ls), HasSoftRefs=%ls",
			InTocPath, *LexToString(ContainerHeader.ContainerId), Order, InsertionIndex, LexToString(Options), *LexToString(ContainerFlags),
			ContainerHeader.SoftPackageReferences.bContainsSoftPackageReferences ? TEXT("True") : TEXT("False"));
	}
...
bool FFileIoStore::Unmount(const TCHAR* InTocPath)
{
	...
	if (ReaderToUnmount)
	{
		UE_LOGF(LogIoDispatcher, Display, "Unmounting container '%ls'", InTocPath);

		int32 FailedRequestsCount = RequestQueue.HandleContainerUnmounted(*ReaderToUnmount->GetContainerFile());

		UE_CLOGF(FailedRequestsCount > 0, LogIoDispatcher, Warning, "Marking %d queued requests from unmounted container as failed", FailedRequestsCount);

		if (GFileIoStoreAsyncUnmountEnabled)
		{
			IoStoreReadersToUnload.Add(MoveTemp(ReaderToUnmount));
			BackendContext->WakeUpDispatcherThreadDelegate.Execute();
			return true;
		}
		...
		for (const FFileIoStoreContainerFilePartition& Partition : ReaderToUnmount->GetContainerFile()->Partitions)
		{
			...
			{
				UE_CLOGF(!bHasWarned, LogIoDispatcher, Warning, "Waiting for read requests to finish before unmounting container");
				bHasWarned = true;
		...
		// Perform a second reference count check that also includes pending scatter tasks
		FFileIoStoreContainerFile& Container = *ReaderToUnmount->GetContainerFile();
		int32 RefCount = Container.RefCount.load(std::memory_order_acquire);
		UE_CLOGF(RefCount > 0, LogIoDispatcher, Warning, "Waiting for %d request(s) before unmounting container '%ls'",
			RefCount, InTocPath);
		...
		while (RefCount > 0)
		{
			...
			{
				UE_LOGF(LogIoDispatcher, Warning, "Unmounting container '%ls' with %d remaining requests(s) after waiting for %.2f seconds(s).",
					InTocPath, RefCount, GFileIoStoreUnmountTimeoutSeconds);
				break;
			}
			FPlatformProcess::SleepNoStats(0.0);
			RefCount = Container.RefCount.load(std::memory_order_acquire);
		}
	...
	{
		UE_LOGF(LogIoDispatcher, Display, "Failed to unmount container '%ls'", InTocPath);
	}
	...
	if (!CompressedBlock->bFailed)
	{
		if (CompressedBlock->SignatureHash)
		{
			...
			if (*CompressedBlock->SignatureHash != BlockHash)
			{
				...
				UE_LOGF(LogIoDispatcher, Warning, "Signature error detected in container '%ls' at block index '%d'", *Error.ContainerName, Error.BlockIndex);
		...
		else
		{
			...
			if (bFailed)
			{
				UE_LOGF(LogIoDispatcher, Warning, "Failed decompressing block");
...
void FFileIoStore::CompleteDispatcherRequest(FFileIoStoreResolvedRequest* ResolvedRequest)
{
	...
	ResolvedRequest->ContainerFile->RefCount.fetch_sub(1, std::memory_order_acq_rel);
...
FIoRequestImpl* FFileIoStore::GetCompletedIoRequests()
{
	...
	IoStoreReadersToUnload.TryUnload();
...
TIoStatusOr<FIoMappedRegion> FFileIoStore::OpenMapped(const FIoChunkId& ChunkId, const FIoReadOptions& Options)
{
	...
	for (TUniquePtr<FFileIoStoreReader>& Reader : IoStoreReaders)
	{
		if (const FIoOffsetAndLength* OffsetAndLength = Reader->Resolve(ChunkId))
		{
			...
			TUniquePtr<IMappedFileHandle> MappedFileHandle = Reader->GetMappedContainerFileHandle(BlockOffset);
			const int64 MappingOffset = BlockOffset + Options.GetOffset();
			IMappedFileRegion* MappedFileRegion = MappedFileHandle->MapRegion(MappingOffset, ResolvedSize);
			if (MappedFileRegion != nullptr)
			{
				...
				return FIoMappedRegion{ MappedFileHandle.Release(), MappedFileRegion };
			}
			else
			{
				const FString& FilePath = Reader->GetContainerFile()->FilePath;
				UE_LOG(LogIoDispatcher, Error, TEXT("MapRegion failed on: %s (0x%016" INT64_x_FMT ", size: %" UINT64_FMT ")"), *FilePath, MappingOffset, ResolvedSize);
...
{
	UE_CLOGF(RequestTracker.GetLiveReadRequestsCount(), LogIoDispatcher, Warning, "Calling ReopenAllFileHandles with read requests in flight");
	FWriteScopeLock _(IoStoreReadersLock);
	for (const TUniquePtr<FFileIoStoreReader>& Reader : IoStoreReaders)
	{
