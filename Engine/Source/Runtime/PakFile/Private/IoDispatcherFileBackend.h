...
public:
	...
	TUniquePtr<IMappedFileHandle> GetMappedContainerFileHandle(uint64 TocOffset);
...
class FFileIoStoreReadersToUnload
{
public:
	FFileIoStoreReadersToUnload() = default;
	~FFileIoStoreReadersToUnload();
	void Add(TUniquePtr<FFileIoStoreReader>&& Reader);
	void TryUnload();

private:
	TArray<TUniquePtr<FFileIoStoreReader>> Readers;
	UE::FMutex Mutex;
};
...
private:
	...
	FFileIoStoreReadersToUnload IoStoreReadersToUnload;
