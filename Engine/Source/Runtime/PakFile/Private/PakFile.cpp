...
void FPakFile::Initialize(FArchive& Reader, bool bLoadIndex)
{
	...
	if (bShouldLoad)
	{
		UE_CLOGF(Info.Magic != FPakInfo::PakFile_Magic, LogPakFile, Fatal, "Trailing magic number (%ud) in '%ls' is different than the expected one. Verify your installation.", Info.Magic, *PakFilename);
	...
	if (Info.Version >= FPakInfo::PakFile_Version_PathHashIndex)
	{
			...
			bStoreDirectoryTMap, bStoreDirectoryTree))
		{
			...
			if (!LoadIndexInternal(Reader, DirectoryIndex, DirectoryTreeIndex, PrunedDirectoryIndex, PrunedDirectoryTreeIndex,
				bStoreDirectoryTMap, bStoreDirectoryTree))
			{
				UE_LOGF(LogPakFile, Fatal, "Corrupt pak index detected on pak file: %ls", *PakFilename);
			}
		}
	}
	else
	...
	FDirectoryTreeIndex& OutPrunedDirectoryTree, bool bStoreDirectoryTMap, bool bStoreDirectoryTree)
{
	...
	{
		UE_LOGF(LogPakFile, Fatal, "Corrupted index offset in pak file.");
		return false;
	}
	...
	FSHAHash ComputedHash;
	{
		...
		{
			UE_LOGF(LogPakFile, Log, "Corrupt pak PrimaryIndex detected!");
			UE_LOGF(LogPakFile, Log, " Filename: %ls", *PakFilename);
			UE_LOGF(LogPakFile, Log, " Encrypted: %d", Info.bEncryptedIndex);
			UE_LOGF(LogPakFile, Log, " Total Size: %lld", Reader.TotalSize());
			UE_LOGF(LogPakFile, Log, " Index Offset: %lld", Info.IndexOffset);
			UE_LOGF(LogPakFile, Log, " Index Size: %lld", Info.IndexSize);
			UE_LOGF(LogPakFile, Log, " Stored Index Hash: %ls", *Info.IndexHash.ToString());
			UE_LOGF(LogPakFile, Log, " Computed Index Hash: %ls", *ComputedHash.ToString());
			return false;
		}
	}
	...
	{
		UE_LOGF(LogPakFile, Error, "Corrupt pak index data: MountPoint path is longer than 65k");
		return false;
	}
	...
	if (NumEntries < 0)
	{
		UE_LOGF(LogPakFile, Error, "Corrupt pak index data: Negative entries count in pak file.");
		return false;
	}
	...
	if (FilesNum < 0)
	{
		...
		UE_LOGF(LogPakFile, Log, "Corrupt pak PrimaryIndex detected!");
		UE_LOGF(LogPakFile, Log, " FilesNum: %d", FilesNum);
		return false;
	}
	...
	else
	{
		...
		UE_LOGF(LogPakFile, Log, "Corrupt pak PrimaryIndex detected!");
		UE_LOGF(LogPakFile, Log, " bReaderHasPathHashIndex: false");
		UE_LOGF(LogPakFile, Log, " bReaderHasFullDirectoryIndex: false");
		return false;
	}
	...
	if (bWillUsePathHashIndex)
	{
			...
			IntFitsIn<int32>(PathHashIndexSize) == false)
		{
			...
			UE_LOGF(LogPakFile, Log, " Filename: %ls", *PakFilename);
			UE_LOGF(LogPakFile, Log, " Total Size: %lld", Reader.TotalSize());
			UE_LOGF(LogPakFile, Log, " PathHashIndexOffset : %lld", PathHashIndexOffset);
			UE_LOGF(LogPakFile, Log, " PathHashIndexSize: %lld", PathHashIndexSize);
			return false;
		}
...

		{
			...
			{
				UE_LOGF(LogPakFile, Log, "Corrupt pak PathHashIndex detected!");
				UE_LOGF(LogPakFile, Log, " Filename: %ls", *PakFilename);
				UE_LOGF(LogPakFile, Log, " Encrypted: %d", Info.bEncryptedIndex);
				UE_LOGF(LogPakFile, Log, " Total Size: %lld", Reader.TotalSize());
				UE_LOGF(LogPakFile, Log, " Index Offset: %lld", FullDirectoryIndexOffset);
				UE_LOGF(LogPakFile, Log, " Index Size: %lld", FullDirectoryIndexSize);
				UE_LOGF(LogPakFile, Log, " Stored Index Hash: %ls", *PathHashIndexHash.ToString());
				UE_LOGF(LogPakFile, Log, " Computed Index Hash: %ls", *ComputedHash.ToString());
				return false;
			}
		}
	...
	else
	{
			...
			IntFitsIn<int32>(FullDirectoryIndexSize) == false)
		{
			...
			UE_LOGF(LogPakFile, Log, " Filename: %ls", *PakFilename);
			UE_LOGF(LogPakFile, Log, " Total Size: %lld", Reader.TotalSize());
			UE_LOGF(LogPakFile, Log, " FullDirectoryIndexOffset : %lld", FullDirectoryIndexOffset );
			UE_LOGF(LogPakFile, Log, " FullDirectoryIndexSize: %lld", FullDirectoryIndexSize);
			return false;
		}
...

		{
			...
			{
				UE_LOGF(LogPakFile, Log, "Corrupt pak FullDirectoryIndex detected!");
				UE_LOGF(LogPakFile, Log, " Filename: %ls", *PakFilename);
				UE_LOGF(LogPakFile, Log, " Encrypted: %d", Info.bEncryptedIndex);
				UE_LOGF(LogPakFile, Log, " Total Size: %lld", Reader.TotalSize());
				UE_LOGF(LogPakFile, Log, " Index Offset: %lld", FullDirectoryIndexOffset);
				UE_LOGF(LogPakFile, Log, " Index Size: %lld", FullDirectoryIndexSize);
				UE_LOGF(LogPakFile, Log, " Stored Index Hash: %ls", *FullDirectoryIndexHash.ToString());
				UE_LOGF(LogPakFile, Log, " Computed Index Hash: %ls", *ComputedHash.ToString());
				return false;
			}
		}
...
bool FPakFile::LoadLegacyIndex(FArchive& Reader, FDirectoryIndex& OutDirectoryTMap)
{
	...
	if (Info.IndexSize < 0 ||
		...
		IntFitsIn<int32>(Info.IndexSize) == false)
	{
		UE_LOGF(LogPakFile, Fatal, "Corrupted index offset/size in pak file.");
		return false;
	}
	...
	{
		UE_LOGF(LogPakFile, Log, "Corrupt pak index detected!");
		UE_LOGF(LogPakFile, Log, " Filename: %ls", *PakFilename);
		UE_LOGF(LogPakFile, Log, " Encrypted: %d", Info.bEncryptedIndex);
		UE_LOGF(LogPakFile, Log, " Total Size: %lld", Reader.TotalSize());
		UE_LOGF(LogPakFile, Log, " Index Offset: %lld", Info.IndexOffset);
	...
	if (NumEntries < 0)
	{
		UE_LOGF(LogPakFile, Error, "Corrupt pak index data: NumEntries is negative");
		return false;
	}
	...
	const FDirectoryIndex& InDirectoryIndex, const FDirectoryTreeIndex& InTreeIndex) const
{
	...
	if (ShouldUseDirectoryTree())
	{
		...
		if (GPak_ValidateDirectoryTreeSearchConsistency)
		{
			...
			{
				UE_LOGF(LogPakFile, Fatal, "Mismatch between directoryindex and directorytreeindex search when searching for [%ls] in pak [%ls]",
...
bool FPakFile::Check()
{
	UE_LOGF(LogPakFile, Display, "Checking pak file \"%ls\". This may take a while...", *PakFilename);
	...
	else
	{
		...
		for (FPakFile::FPakEntryIterator It(*this, bIncludeDeleted); It; ++It, ++FileCount)
		{
			...
			{
				UE_LOGF(LogPakFile, Verbose, "%ls Deleted.", GetEntryName(It));
				continue;
			}
			...
			{
				UE_LOGF(LogPakFile, Error, "Index FPakEntry does not match Payload FPakEntry for %ls.", GetEntryName(It));
				ErrorCount++;
			}
			...
			{
				UE_LOGF(LogPakFile, Error, "Hash mismatch for %ls.", GetEntryName(It));
				ErrorCount++;
			}
			else
			{
				UE_LOGF(LogPakFile, Verbose, "%ls OK. [%ls]", GetEntryName(It), *Info.GetCompressionMethod(EntryFromIndex.CompressionMethodIndex).ToString());
			}
			FMemory::Free(FileContents);
		}
		if (ErrorCount == 0)
		{
			UE_LOGF(LogPakFile, Display, "Pak file \"%ls\" healthy, %d files checked.", *PakFilename, FileCount);
		}
		else
		{
			UE_LOGF(LogPakFile, Display, "Pak file \"%ls\" corrupted (%d errors out of %d files checked.).", *PakFilename, ErrorCount, FileCount);
		}
	}

	double EndTime = FPlatformTime::Seconds();
	double ElapsedTime = EndTime - StartTime;
	UE_LOGF(LogPakFile, Display, "Pak file \"%ls\" checked in %.2fs", *PakFilename, ElapsedTime);
...
void FPakFile::ValidateDirectorySearch(const TSet<FString>& FullFoundFiles, const TSet<FString>& PrunedFoundFiles, const TCHAR* InPath) const
{
	...
	UE_LOGF(LogPakFile, Error, "FindPrunedFilesAtPath('%ls') for PakFile '%ls' found a different list in the FullDirectory than in the PrunedDirectory. " "Change the calling code or add the files to Engine:[Pak]:WildcardsToKeepInPakStringIndex or Engine:[Pak]:IndexValidationIgnore.",
		InPath, *PakFilename);
	...
	{
		UE_LOGF(LogPakFile, Warning, "Callstack of FindPrunedFilesAtPath('%ls'):", InPath);
	...
	if (MissingFromPruned.Num() > 0)
	{
		...
		{
			UE_LOGF(LogPakFile, Warning, "MissingPrunedPakFile: %ls", *Missing);
		}
	}
	if (MissingFromFull.Num() > 0)
	{
		UE_LOGF(LogPakFile, Error, "Some files in the PrunedDirectory are missing from the FullDirectory.  This is a logic error in FPakFile since the PrunedDirectory should be a subset of the FullDirectory.");
		for (const FString& Missing : MissingFromFull)
		{
			UE_LOGF(LogPakFile, Warning, "MissingFullPakFile: %ls", *Missing);
		}
	}
}
#endif
...
bool FPakFile::RecreatePakReaders(IPlatformFile* LowerLevel)
{
	...
	{
		UE_LOGF(LogPakFile, Error, "Recreating pak readers while we have readers loaned out, this may be lead to crashes or decryption problems.");
	}
	...
	for (const FArchiveAndLastAccessTime& Reader : Readers)
	{
		...
		if (!PakReader)
		{
			UE_LOGF(LogPakFile, Warning, "Unable to re-create pak \"%ls\" handle", *GetFilename());
			return false;
		}
...
FSharedPakReader FPakFile::GetSharedReader(IPlatformFile* LowerLevel)
{
	...
	FArchive* PakReader = nullptr;
	{
		...
		else
		{
			...
			if (!PakReader)
			{
				UE_LOGF(LogPakFile, Warning, "Unable to create pak \"%ls\" handle", *GetFilename());
			}
		}
...
FPakFile::EFindResult FPakFile::Find(const FString& FullPath, FPakEntry* OutEntry) const
{
	...
	if (IsPakValidatePruning() && bHasPathHashIndex && bHasFullDirectoryIndex)
	{
		...
		if ((PathHashLocation != nullptr) != (DirectoryLocation != nullptr))
		{
			...
			UE_LOGF(LogPakFile, Error, "PathHashIndex does not match FullDirectoryIndex. Pakfile '%ls' has '%ls' in its %ls but not in its %ls.",
