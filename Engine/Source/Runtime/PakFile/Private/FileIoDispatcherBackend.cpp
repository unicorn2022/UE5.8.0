...
////////////////////////////////////////////////////////////////////////////////
class FMappedFileProxy final
	: public IMappedFileHandle
{
public:
	FMappedFileProxy(IMappedFileHandle* InSharedMappedFileHandle, uint64 InFileSize, int64 InBlockOffset)
		: IMappedFileHandle(InFileSize)
	...
	{
		return SharedMappedFileHandle != nullptr ? SharedMappedFileHandle->MapRegion(BlockOffset + Offset, BytesToMap, Flags) : nullptr;
	}
private:
	IMappedFileHandle* SharedMappedFileHandle;
	const int64 BlockOffset;
};
...
struct FContainer
{
	static TIoStatusOr<TUniquePtr<FContainer>>			Open(const TCHAR* Filename, int32 MountOrder, uint32 InstanceId, bool bReadsSoftRefs, FIoContainerHeader& OutContainerHeader);
	...
	FIoContainerHeader& OutContainerHeader)
{
	...
	if (OffsetAndLength != nullptr)
	{
		...
		if (bReadSoftRefs)
		{
			if (OutContainerHeader.SoftPackageReferencesSerialInfo.Offset < 0)
			{
				return FIoStatusBuilder(EIoErrorCode::ReadError)
					<< FString::Printf(TEXT("Invalid soft package reference offset '%" INT64_FMT "'"), OutContainerHeader.SoftPackageReferencesSerialInfo.Offset);
			}

			if ((OutContainerHeader.SoftPackageReferencesSerialInfo.Offset + OutContainerHeader.SoftPackageReferencesSerialInfo.Size) > Ar.TotalSize())
			{
				return FIoStatusBuilder(EIoErrorCode::ReadError)
					<< FString::Printf(TEXT("Soft package reference offset '%" INT64_FMT "' and size '%" INT64_FMT "' will seek past the end of archive size '%" INT64_FMT "'"),
						OutContainerHeader.SoftPackageReferencesSerialInfo.Offset,
						OutContainerHeader.SoftPackageReferencesSerialInfo.Size,
						Ar.TotalSize());
			}

			Ar.Seek(OutContainerHeader.SoftPackageReferencesSerialInfo.Offset);
			Ar << OutContainerHeader.SoftPackageReferences;
		}
	}
...
public:
	...
	TIoStatusOr<FIoContainerHeader>	Mount(const TCHAR* TocPath, int32 MountOrder, ETocMountOptions Options);
...
////////////////////////////////////////////////////////////////////////////////
TIoStatusOr<FIoContainerHeader>	FFileIoStore::Mount(const TCHAR* TocPath, int32 MountOrder, ETocMountOptions Options)
{
	const uint32		InstanceId = ContainerInstanceId.fetch_add(1, std::memory_order_relaxed);
	FIoContainerHeader	Hdr;

	const bool bReadSoftRefs = EnumHasAnyFlags(Options, ETocMountOptions::WithSoftReferences);
	TIoStatusOr<FUniqueContainer> Status = FContainer::Open(TocPath, MountOrder, InstanceId, bReadSoftRefs, Hdr);
	if (Status.IsOk() == false)
	{
	...
	const EIoContainerFlags ContainerFlags	= Container->ContainerFlags;
	...
	UE_LOGF(LogIoStore, Display, "Mounted container '%ls', Id='%ls', Order=%d, Slot=%d, Options=(%ls), Flags=(%ls), HasSoftRefs=%ls",
		TocPath, *LexToString(Hdr.ContainerId), MountOrder, MountIndex, LexToString(Options), *LexToString(ContainerFlags),
		Hdr.SoftPackageReferences.bContainsSoftPackageReferences ? TEXT("True") : TEXT("False"));
...
bool FFileIoStore::Unmount(const TCHAR* TocPath)
{
	...
	{
		UE_LOGF(LogIoStore, Warning, "Failed to unmount container '%ls', reason 'Not Found'", TocPath);
		return false;
	}
	...
	if (ContainerToRemove->ActiveReadCount.load(std::memory_order_seq_cst) > 0)
	{
		...
		{
			UE_LOGF(LogIoStore, Log, "Cancelling inflight read requests for file '%ls'", *Part.Filename);
			UE::FPlatformIoDispatcher::Get().CancelAllRequests((Part.FileHandle));
		}

		UE_LOGF(LogIoStore, Log, "Waiting for read request(s) to finish before unmounting '%ls.utoc'", TocPath);
		const double MaxWaitTimeSeconds	= Config::FileIoStoreUnmountTimeOutSeconds;
		...
		while (ContainerToRemove->ActiveReadCount.load(std::memory_order_seq_cst) > 0)
		{
			...
			{
				UE_LOGF(LogIoStore, Warning, "Stopped waiting for read request(s) after %.2lf seconds", MaxWaitTimeSeconds);
				break;
			}
		}
	}
	...
	UE_LOGF(LogIoStore, Log, "Unmounted container '%ls'", TocPath);
	return true;
}
...
void FFileIoStore::ReopenAllFileHandles()
{
	...
	{
		UE_CLOGF(Container->ActiveReadCount.load(std::memory_order_seq_cst) > 0, LogIoStore, Warning, "Calling ReopenAllFileHandles with read requests in flight");
		for (FContainerPartition& Part : Container->Partitions)
		{
			UE_LOGF(LogIoStore, Log, "Reopening container file '%ls'", *Part.Filename);
			UE::FPlatformIoDispatcher::Get().CloseFile((Part.FileHandle));
...
public:
	...
	virtual TIoStatusOr<FIoContainerHeader> Mount(
		...
		ETocMountOptions Options) override;
...
TIoStatusOr<FIoContainerHeader> FFileIoDispatcherBackend::Mount(
	...
	const FGuid& EncryptionKeyGuid, 
	const FAES::FAESKey& EncryptionKey, 
	ETocMountOptions Options)
{
	LLM_SCOPE_BYNAME(TEXT("FileSystem/FileIoStore"));
	return IoStore.Mount(TocPath, Order, Options);
}
...
void FFileIoDispatcherBackend::ResolveIoRequests(FIoRequestList Requests, FIoRequestList& OutUnresolved)
{
	...
	while (FIoRequestImpl* DispatcherRequest = ResolvedRequests.PopHead())
	{
		...
		{
			UE_LOGF(LogIoStore, Warning, "Failed to create platform read request, ChunkId='%ls' Filenname='%ls'",
...
TIoStatusOr<FIoMappedRegion> FFileIoDispatcherBackend::OpenMapped(const FIoChunkId& ChunkId, const FIoReadOptions& Options)
{
	...
	const int64 ResolvedOffset	= ChunkInfo.Offset();
	...
	{
		IPlatformFile& Ipf = FPlatformFileManager::Get().GetPlatformFile();
		if (FOpenMappedResult Result = Ipf.OpenMappedEx2(*Partition.Filename); !Result.HasError())
		{
			Partition.MappedFileHandle = Result.StealValue();
		}
		else
		{
			UE_LOGF(LogIoStore, Error, "Failed to memory map chunk, ChunkId='%ls', Filename='%ls', Reason='%ls'",
				*LexToString(ChunkId), *Partition.Filename, *Result.GetError().GetMessage());
		}
	}
	...
	if (IMappedFileRegion* MappedFileRegion = MappedFileHandle.MapRegion(BlockOffsetInPartition + Options.GetOffset(), ResolvedSize))
	{
		...
		{
			.MappedFileHandle	= new FMappedFileProxy(Partition.MappedFileHandle.Get(), MappedFileHandle.GetFileSize(), IntCastChecked<int64>(BlockOffsetInPartition)),
			.MappedFileRegion	= MappedFileRegion
		};
	}
	else
	{
		UE_LOGF(LogIoStore, Error, "Failed to memory map region, ChunkId='%ls', Filename='%ls', Region=(%llu, %lld)",
			*LexToString(ChunkId), *Partition.Filename, BlockOffsetInPartition + Options.GetOffset(), ResolvedSize);
	}
...
void FFileIoDispatcherBackend::HandleSignatureError(FIoRequestImpl& DispatcherRequest, uint32 FailedBlockIndex)
{
	FIoSignatureError SignatureError;
	{
		...
		UE_LOGF(LogIoStore, Warning, "Signature error detected, ChunkId='%ls', Filename='%ls', Offset=%llu",
