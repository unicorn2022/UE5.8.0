...
bool FChunkCacheWorker::CheckSignature(const FChunkRequest& ChunkInfo)
{
	...
	if (ensure(IsValid()))
	{
		...
		{
			UE_LOGF(LogPakFile, Warning, "Pak chunk signing mismatch on chunk [%i/%i]! Expected %ls, Received %ls", ChunkInfo.Index, Signatures->ChunkHashes.Num() - 1, *ChunkHashToString(Signatures->ChunkHashes[ChunkInfo.Index]), *ChunkHashToString(ChunkHash));
