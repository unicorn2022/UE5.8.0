...
// By default EXCLUDE_NONPAK_UE_EXTENSIONS is defined in runtime game and not defined in editor.
// Use .Build.cs file to define it to 0 if the game relies on accessing loose files.
// This functionality is not needed in Editor configurations using pak files, because Editor always
// includes the possibility of any type being uncooked on disk and not in pak.
#define EXCLUDE_NONPAK_UE_EXTENSIONS (!WITH_EDITOR)
#endif
...
struct FPakInfo
{
	...
	void Serialize(FArchive& Ar, int32 InVersion)
	{
		...
		if (Version >= PakFile_Version_FrozenIndex && Version < PakFile_Version_PathHashIndex)
		{
			...
			if (bIndexIsFrozen)
			{
				UE_LOGF(LogPakFile, Fatal, "PakFile was frozen with version FPakInfo::PakFile_Version_FrozenIndex, which is no longer supported. Regenerate Paks.");
			}
		}
...
struct FPakEntry
{
	...
	void Serialize(FArchive& Ar, int32 Version)
	{
		...
		if (Version < FPakInfo::PakFile_Version_FNameBasedCompressionMethod)
		{
			...
			{
				UE_LOGF(LogPakFile, Fatal, "Found an unknown compression type in pak file, will need to be supported for legacy files");
			}
		}
		else
		{
 ...
 * Pak file.
 */
class FPakFile : FNoncopyable, public FRefCountedObject, public IPakFile
{
public:
...
private:
	...
	friend FOpenMappedResult TOpenMappedExImpl(class FPakPlatformFile* pThis, const TCHAR* Filename, OpenOptionsType OpenOptions, int64 MaximumSize,
...
private:
	...
	friend class FRefCountedObject;
...
#if !UE_BUILD_SHIPPING
	// if true (via -lookloosefirst) then loose/non-ufs files will be looked for before looking in the .pak file
	// this respects IsNonPakFilenameAllowed()
	bool bLookLooseFirst = false;
	std::atomic<bool> bProcessHasEverMountedAPakFile{ false };
#endif
	...
	*/
	...
	virtual FDateTime GetTimeStamp(const TCHAR* Filename) override
	{
		...
		if (IsNonPakFilenameAllowed(Filename))
		{
			...
			UE_LOGF(LogPakFile, Verbose, "GetTimeStamp on disk (!!) for %ls took %6.2fms.", Filename, float(FPlatformTime::Seconds() - StartTime) * 1000.0f);
		}
		return Result;
	}
	...
	template<typename OpenOptionsType>
	friend FOpenMappedResult TOpenMappedExImpl(FPakPlatformFile* pThis, const TCHAR* Filename, OpenOptionsType OpenOptions, int64 MaximumSize,
		FOpenMappedResult(IPlatformFile::*LowLevelOpenMappedEx)(const TCHAR*, OpenOptionsType, int64));
...
struct FPakSignatureFile
{
	...
	bool DecryptSignatureAndValidate(const FRSAKeyHandle InKey, const FString& InFilename)
	{
		...
		{
			UE_LOGF(LogPakFile, Warning, "Pak signature file for '%ls' was invalid", *InFilename);
		}
		else
		{
			...
			if (BytesDecrypted > (int32)UE_ARRAY_COUNT(FSHAHash::Hash))
			{
				...
				{
					UE_LOGF(LogPakFile, Warning, "Pak signature table validation failed for '%ls'! Expected %ls, Received %ls", *InFilename, *DecryptedHash.ToString(), *CurrentHash.ToString());
				}
			}
			else
			{
				FString EncryptedHashString = BytesToHex(EncryptedHash.GetData(), EncryptedHash.Num());
				UE_LOGF(LogPakFile, Warning, "Pak signature table validation failed for '%ls'! Failed to decrypt signature. Signature EncryptedHash: (Bytes=%d, Data=%ls)", *InFilename, EncryptedHash.Num(), *EncryptedHashString);
			}
		}
