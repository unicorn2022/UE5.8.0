...
#define PLATFORM_IODISPATCHER_MODULE UE_STRINGIZE(UE_JOIN(PLATFORM_HEADER_NAME, PlatformIoDispatcher))
#endif
...
struct FFileIoStoreContainerFilePartition
{
	...
	~FFileIoStoreContainerFilePartition();
	...
	// This will lazily map the file, if needed. It is thread-safe to borrow
	// handles, but their lifetimes are tied to this partition.
	IMappedFileHandle* BorrowMappedFileHandle();
...
private:	
	std::atomic<IMappedFileHandle*> MappedFileHandle = nullptr;
	UE::FMutex MappedFileHandleMutex;
};

struct FFileIoStoreContainerFile
{
	FFileIoStoreContainerFile() = default;
	UE_NONCOPYABLE(FFileIoStoreContainerFile);
	...
	void Empty()
	{
		...
		CompressionBlocks = TConstArrayView<FIoStoreTocCompressedBlockEntry>();
		FilePath.Empty();
		EncryptionKeyGuid = FGuid();
		EncryptionKey = FAES::FAESKey();
		ContainerFlags = EIoContainerFlags::None;
		BlockSignatureTable = TConstArrayView<FSHAHash>();
		Partitions.Empty();
