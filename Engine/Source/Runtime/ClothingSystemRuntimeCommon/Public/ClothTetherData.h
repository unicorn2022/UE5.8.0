...
struct FClothTetherData
{
	...
	/**
	 * Particles whose MaxDistance / TetherEnds mask is strictly less than this are treated as
	 * kinematic for tether sorting. Authoritative source for the legacy "barely allowed to move"
	 * cutoff — referenced by the converter and consumed throughout the cloth simulation pipeline
	 * so the same threshold is applied to legacy assets at convert time and to runtime tether
	 * generation.
	 */
	...
	CLOTHINGSYSTEMRUNTIMECOMMON_API void GenerateTethers(
		...
		const TConstArrayView<uint32>& Indices,  // Triangle mesh
		const TConstArrayView<float>& MaxDistances,  // Mask for sorting the kinematic from the dynamic points
		const TConstArrayView<float>& TetherEnds,  // Separate mask for specifying tether ends (using same KinematicDistanceThreshold cutoff as MaxDistances). Actual tether end set is the intersection of tether ends and kinematic points.
		bool bUseGeodesicDistance);  // Whether to use geodesic (walking along the surface) or euclidean (beeline) distances to find the tethers.
