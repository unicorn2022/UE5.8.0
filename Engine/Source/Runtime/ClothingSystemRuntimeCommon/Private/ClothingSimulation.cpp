...
void FClothingSimulationCommon::FillContextAndPrepareTick(const USkeletalMeshComponent* InComponent, float InDeltaTime, IClothingSimulationContext* InOutContext, bool bIsInitialization, bool bForceTeleportResetOnly)
{
	...
	if (!IsValid(InComponent))
	{
		...
		UE_LOGF(LogSkeletalMesh, Warning, 
			"Attempting to fill a clothing simulation context for a PendingKill skeletal mesh component (Comp: %ls, Actor: %ls). "
	...
	if (Context->BoneTransforms.Num() == 0)
	{
		...
		UE_LOGF(LogSkeletalMesh, Warning, "Attempting to fill a clothing simulation context for a skeletal mesh component that has zero bones (Comp: %ls, Leader: %ls, Actor: %ls).", *InComponent->GetName(), Leader ? *Leader->GetName() : TEXT("None"), CompOwner ? *CompOwner->GetName() : TEXT("None"));
