...
namespace ClothingMeshUtils
{
		...
		TArray<FVector3f>& OutNormals)
	{
		...
		UE_CLOGF(MaxInfluences > 12, LogClothingMeshUtils, Warning, "The cloth physics mesh skinning code can't cope with more than 12 bone influences.");
	...
	namespace  // Helpers
	{
										 ...
										 FMeshToMeshVertData& OutSkinningData)
		{
			...
			if (TriNormal.SizeSquared() < SMALL_NUMBER)
			{
				...
				UE_LOGF(LogClothingMeshUtils, Warning, "%ls", *Error.ToString());
		...
		const TArray<FMeshToMeshFilterSet>& FilterSets)
	{
		...
		{
			UE_LOGF(LogClothingMeshUtils, Warning, "Failed to generate mesh to mesh skinning data. Invalid Target Mesh.");
			return;
		}
		...
		{
			UE_LOGF(LogClothingMeshUtils, Warning, "Failed to generate mesh to mesh skinning data. Invalid Source Mesh.");
			return;
		}
		...
		if (bUseMultipleInfluences)
		{
			...
			for (int32 VertIdx0 = 0; VertIdx0 < NumMesh0Verts; ++VertIdx0)
			{
				...
				if (!bOK)
				{
					UE_LOGF(LogClothingMeshUtils, Warning, "Error generating mesh-to-mesh skinning data");
					OutMeshToMeshVertData.Reset();
					return;
				}
