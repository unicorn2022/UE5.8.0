...
#if WITH_EDITOR
void ClothingAssetUtils::UnbindFromSkeletalMesh(const UClothingAssetBase& Asset, USkeletalMesh& SkeletalMesh, const int32 MeshLodIndex, const int32 LodSectionIndex)
{
	FSkeletalMeshModel* const Mesh = SkeletalMesh.GetImportedModel();
	if (!Mesh)
	{
		UE_LOGF(LogClothingAsset, Warning,
			"UnbindFromSkeletalMesh: Failed to unbind Clothing Data [%ls] from Skeletal Mesh [%ls], the Skeletal Mesh imported model doesn't exist.",
			*Asset.GetName(),
			*SkeletalMesh.GetName());
		return;
	}

	int32 StartMeshLodIndex = 0;
	int32 EndMeshLodIndex = -1;
	if (MeshLodIndex == INDEX_NONE)
	{
		EndMeshLodIndex += Mesh->LODModels.Num();
	}
	else if (Mesh->LODModels.IsValidIndex(MeshLodIndex))
	{
		StartMeshLodIndex = EndMeshLodIndex = MeshLodIndex;
	}
	else
	{
		UE_LOGF(LogClothingAsset, Warning,
			"UnbindFromSkeletalMesh: Failed to unbind Clothing Data [%ls] from Skeletal Mesh [%ls], LOD%d doesn't exist.",
			*Asset.GetName(),
			*SkeletalMesh.GetName(),
			MeshLodIndex);
		return;
	...
	ON_SCOPE_EXIT  // If the mesh changed we need to re-register any components that use it to reflect the changes
		{
			if (bChangedMesh)
			{
	...
	for (int32 LodIndex = StartMeshLodIndex; LodIndex <= EndMeshLodIndex; ++LodIndex)
	{
		FSkeletalMeshLODModel& LodModel = Mesh->LODModels[LodIndex];

		int32 StartLodSectionIndex = 0;
		int32 EndLodSectionIndex = -1;

		if (MeshLodIndex == INDEX_NONE || LodSectionIndex == INDEX_NONE)
		{
			EndLodSectionIndex += LodModel.Sections.Num();
		}
		else if (LodModel.Sections.IsValidIndex(LodSectionIndex))
		{
			StartLodSectionIndex = EndLodSectionIndex = LodSectionIndex;
		}
		else
		{
			UE_LOGF(LogClothingAsset, Warning,
				"UnbindFromSkeletalMesh: Failed to unbind Clothing Data [%ls] from Skeletal Mesh [%ls], LOD%d section %d doesn't exist.",
				*Asset.GetName(),
				*SkeletalMesh.GetName(),
				MeshLodIndex,
				LodSectionIndex);
		...
		for (int32 SectionIndex = EndLodSectionIndex; SectionIndex >= StartLodSectionIndex; --SectionIndex)
		{
			...
			if (Section.HasClothingData() && Section.ClothingData.AssetGuid == Asset.GetAssetGuid())
			{
				...
				if (IsInGameThread() && !bChangedMesh)
				{
					SkeletalMesh.PreEditChange(nullptr);
				}

				// Log the LOD bias mapping data that will be removed
				for (int32 BiasLodIndex = 0; BiasLodIndex < LodIndex; ++BiasLodIndex)
				{
					const int32 LodBias = LodIndex - BiasLodIndex;

					UE_CLOGF(Section.ClothMappingDataLODs.IsValidIndex(LodBias) && Section.ClothMappingDataLODs[LodBias].Num(),
						LogClothingAsset, Verbose,
						"Removed deformer data for [%ls/%ls], section %d LOD bias = %d, render mesh LOD = %d, sim mesh LOD = %d",
						*SkeletalMesh.GetName(),
						*Asset.GetName(),
						SectionIndex,
						LodBias,
				...
				ClothingAssetUtils::ClearLODBiasMappings(SkeletalMesh, LodIndex, SectionIndex);
...
void ClothingAssetUtils::UpdateLODBiasMappings(
	const UClothingAssetBase& Asset,
	const USkeletalMesh* SkeletalMesh,
	const int32 UpdatedLODIndex,
	const int32 SectionIndex,
	const bool bSmoothTransition,
	const bool bUseMultipleInfluences,
	const float SkinningKernelRadius)
{
	check(SkeletalMesh);
	...
	// Render mesh desc helper lambda
	TArray<FVector3f> RenderPositions;
	TArray<FVector3f> RenderNormals;
	TArray<FVector3f> RenderTangents;
	TArray<uint32> RenderIndices;

	auto PrepareDeformerTargetDescriptor = [&LODModels, &RenderPositions, &RenderNormals, &RenderTangents, &RenderIndices](
			int32 LODIndex,
			int32 SectionIndex) -> ClothingMeshUtils::ClothMeshDesc
		{
	...
	// Cloth sim mesh and max distance helper lambda
	TArray<FVector3f> SimPositions;
	TArray<uint32> SimIndices;
	FPointWeightMap SimMaxDistances;

	auto PrepareDeformerSourceDescriptor = [&SimPositions, &SimIndices, &SimMaxDistances](
			const UClothingAssetBase& ClothingAsset,
			const int32 LODIndex,
			FPointWeightMap*& OutMaxDistances) -> ClothingMeshUtils::ClothMeshDesc
		{
			TArray<float> TempMaxDistances;
			ClothingAsset.GetSimulationMesh(LODIndex, SimPositions, SimIndices, TempMaxDistances);
			if (TempMaxDistances.Num() == SimPositions.Num())
			{
				SimMaxDistances.Values = MoveTemp(TempMaxDistances);
				OutMaxDistances = &SimMaxDistances;
			}
			else
			{
				OutMaxDistances = nullptr;  // Null max distance will safely disable smoothing transition calculations
			}
			return ClothingMeshUtils::ClothMeshDesc(SimPositions, SimIndices);  // Calculate averaged normals
		};
	...
	{
		const int32 RayTracingMinLOD = SkeletalMesh->GetRayTracingMinLOD();
		if (bAddMappingsToMinLOD && LODModels.IsValidIndex(RayTracingMinLOD))
		{
			MinLOD = MaxLOD = RayTracingMinLOD;
		}
	}
	if (UpdatedLODIndex < MinLOD && MinLOD <= MaxLOD)
	{
		...
		const ClothingMeshUtils::ClothMeshDesc SourceMesh = PrepareDeformerSourceDescriptor(Asset, UpdatedLODIndex, MaxDistances);

		// Generate the mappings for this section at the various LODs using the current simulation mesh
		// so that higher LODs have something to map to when raytracing bias is active and a higher LOD is rendered
		for (int32 LODIndex = MinLOD; LODIndex <= MaxLOD; ++LODIndex)
		{
			...
			if (!Section.HasClothingData())  // Note it doesn't matter if this section has a different asset at this point, only that the shader will want to access the clothing data
			{
			...
			UE_LOGF(LogClothingAsset, Verbose, "Added deformer data for [%ls/%ls], section %d, LODBias = %d, render mesh LOD = %d, sim mesh LOD = %d",
				*SkeletalMesh->GetName(),
				*Asset.GetName(),
				SectionIndex,
				LODBias,
	...
	{
		FSkeletalMeshLODModel& LODModel = LODModels[UpdatedLODIndex];  // Note that the section LOD being updated here is the UpdatedLODIndex
		FSkelMeshSection& Section = LODModel.Sections[SectionIndex];   // not the biased (raytraced) section LOD which is done in the first loop above
		...
		for (int32 LODIndex = 0; LODIndex < UpdatedLODIndex; ++LODIndex)
		{
			...
			const UClothingAssetBase* const ClothingAsset = SkeletalMesh->GetClothingAsset(SourceSection.ClothingData.AssetGuid);
			if (!ClothingAsset)
			{
				UE_LOGF(LogClothingAsset, Warning, "Incomplete raytracing cloth LOD bias mappings generation on [%ls/%ls], section %d, LOD %d due to no clothing data set to LOD %d.",
					*SkeletalMesh->GetName(),
					*Asset.GetName(),
					SectionIndex,
					UpdatedLODIndex,
			...
			// Prepare the deformer source (simulation) mesh descriptor
			FPointWeightMap* MaxDistances;
			const ClothingMeshUtils::ClothMeshDesc SourceMesh = PrepareDeformerSourceDescriptor(*ClothingAsset, LODIndex, MaxDistances);
			...
			for (int32 LODIndex = UpdatedLodIndex + 1; LODIndex < SkeletalMeshModel->LODModels.Num(); ++LODIndex)
			{
				...
				if (BiasLodModel.Sections.IsValidIndex(SectionIndex))
				{
					...
					if (BiasSection.ClothMappingDataLODs.IsValidIndex(LODBias))
					{
						...
						UE_LOGF(LogClothingAsset, Verbose, "Removed deformer data for [%ls/%ls], section %d LODBias = %d, render mesh LOD = %d, sim mesh LOD = %d",
							*SkeletalMesh.GetName(),
	...
	const int32 InAssetLodIndex)
{
	...
	// Copy the new bone map to the section
	...
	if (InSkelMesh->GetSupportRayTracing())
	{
		...
		ClothingAssetUtils::UpdateLODBiasMappings(
			*this,
			InSkelMesh,
			InMeshLodIndex,
			InSectionIndex,
...
void UClothingAssetCommon::UpdateAllLODBiasMappings(USkeletalMesh* SkeletalMesh)
{
	...
	{
		if (LodMap.IsValidIndex(LODIndex) && LodData.IsValidIndex(LodMap[LODIndex]))
		{
			const FClothLODDataCommon& ClothLodData = LodData[LodMap[LODIndex]];
...
void UClothingAssetCommon::GetSimulationMesh(const int32 InMeshLodIndex, TArray<FVector3f>& OutPositions, TArray<uint32>& OutIndices, TArray<float>& OutMaxDistances)const
{
	OutPositions.Reset();
	OutIndices.Reset();
	OutMaxDistances.Reset();

	if (LodMap.IsValidIndex(InMeshLodIndex) && LodData.IsValidIndex(LodMap[InMeshLodIndex]))
	{
		const FClothPhysicalMeshData& PhysicalMeshData = LodData[LodMap[InMeshLodIndex]].PhysicalMeshData;

		OutPositions = PhysicalMeshData.Vertices;
		OutIndices = PhysicalMeshData.Indices;
		if (const FPointWeightMap* const MaxDistances = PhysicalMeshData.FindWeightMap(EWeightMapTargetCommon::MaxDistance);
			MaxDistances && MaxDistances->Num() == OutPositions.Num())
		{
			OutMaxDistances = MaxDistances->Values;
		}
	}
}

void UClothingAssetCommon::UnbindFromSkeletalMesh(USkeletalMesh* InSkelMesh, const int32 InMeshLodIndex, const int32 InSectionIndex)
{
	// Clear the LOD map entries to allow for rebinding a new asset
	if (InMeshLodIndex == INDEX_NONE)
	{
	...
	else if (LodMap.IsValidIndex(InMeshLodIndex))
	{
		LodMap[InMeshLodIndex] = INDEX_NONE;  // Note: This must be done prior calling UnbindFromSkeletalMesh(), so that the info is correct when rebuilding the mesh
	}
	// Unbind from the Skeletal Mesh
	if (InSkelMesh)
	{
		ClothingAssetUtils::UnbindFromSkeletalMesh(*this, *InSkelMesh, InMeshLodIndex, InSectionIndex);
	}
}
...
void UClothingAssetCommon::ApplyParameterMasks(bool bUpdateFixedVertData, bool bInvalidateDerivedDataCache)
{
	...
	{
		if (FSkeletalMeshModel* const Resource = SkeletalMesh->GetImportedModel())
		{
...
TArray<TPair<FText, FText>> UClothingAssetCommon::GetStats() const
{
	...
	for (int32 LodIndex = 0; LodIndex < LodData.Num(); ++LodIndex)
	{
		...
		const FClothPhysicalMeshData& PhysMeshData = LodData[LodIndex].PhysicalMeshData;
		Stats.Emplace(LOCTEXT("SimulVertexCount", "Simul Verts"), FText::AsNumber(PhysMeshData.Vertices.Num() - PhysMeshData.NumFixedVerts));
		Stats.Emplace(LOCTEXT("FixedVertexCount", "Fixed Verts"), FText::AsNumber(PhysMeshData.NumFixedVerts));
		Stats.Emplace(LOCTEXT("TriangleCount", "Sim Triangles"), FText::AsNumber(PhysMeshData.Indices.Num() / 3));
		Stats.Emplace(LOCTEXT("MaxBoneWeights", "Bone Weights"), FText::AsNumber(PhysMeshData.MaxBoneWeights));
	}
	return Stats;
}
#endif // WITH_EDITOR
...
void UClothingAssetCommon::PostLoad()
{
	...
	if (ClothSimConfig_DEPRECATED)
	{
		...
		ClothSimConfig_DEPRECATED->Rename(nullptr, nullptr, REN_DoNotDirty | REN_DontCreateRedirectors | REN_NonTransactional | REN_AllowPackageLinkerMismatch);  // Rename the config so that the name doesn't collide with the new config map name
	}
	if (ChaosClothSimConfig_DEPRECATED)
	{
		...
		ChaosClothSimConfig_DEPRECATED->Rename(nullptr, nullptr, REN_DoNotDirty | REN_DontCreateRedirectors | REN_NonTransactional | REN_AllowPackageLinkerMismatch);  // Rename the config so that the name doesn't collide with the new config map name
	}
	if (ClothSharedSimConfig_DEPRECATED)
	{
		...
		ClothSharedSimConfig_DEPRECATED->Rename(nullptr, nullptr, REN_DoNotDirty | REN_DontCreateRedirectors | REN_NonTransactional | REN_AllowPackageLinkerMismatch);  // Rename the config so that the name doesn't collide with the new config map name
	}
	...
	if (USkeletalMesh* const SkeletalMesh = Cast<USkeletalMesh>(GetOuter()))
	{
		...
		for (TPair<FName, TObjectPtr<UClothConfigBase>>& ClothSharedConfigItem : ClothConfigs)
		{
			if (UClothSharedConfigCommon* const ClothSharedConfig = Cast<UClothSharedConfigCommon>(ClothSharedConfigItem.Value))
			{
				...
				{
					ClothSharedConfig->Rename(nullptr, SkeletalMesh, REN_DontCreateRedirectors | REN_NonTransactional | REN_AllowPackageLinkerMismatch);
				}
...
void UClothingAssetCommon::RemoveLastLod()
{
	const int32 LodIndex = LodData.Num() - 1;
	if (LodIndex > 0)  // Can't remove LOD0
	{
		TOptional<FScopedSkeletalMeshPostEditChange> SkeletalMeshPostEditChange;
		USkeletalMesh* const SkeletalMesh = Cast<USkeletalMesh>(GetOuter());
		if (SkeletalMesh)
		{
			SkeletalMeshPostEditChange.Emplace(SkeletalMesh);
		}

		int32 MeshLodIndex;
		while (LodMap.Find(LodIndex, MeshLodIndex))
		{
			...
			{
				// Unbind from the Skeletal Mesh
				ClothingAssetUtils::UnbindFromSkeletalMesh(*this, *SkeletalMesh, MeshLodIndex, INDEX_NONE);
			}
		}
		LodData.RemoveAt(LodIndex, EAllowShrinking::Yes);
	}
}
