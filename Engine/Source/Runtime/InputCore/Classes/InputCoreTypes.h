...
/**
 * Identifies a specific touch contact on a specific input device.
 * Combines device identity (FInputDeviceId) with finger index (ETouchIndex) so that multiple touchscreens can each have independent finger-0, etc.
 */
...
struct FTouchId
{
	...
	explicit FTouchId(const FInputDeviceId InDeviceId, const ETouchIndex::Type InTouchIndex)
		: DeviceId(InDeviceId)
	...
	ETouchIndex::Type GetIndex() const
	{
		return TouchIndex;
	}

	bool operator==(const FTouchId& Other) const = default;
	...
	friend uint32 GetTypeHash(const FTouchId& Id)
	{
		return HashCombine(GetTypeHash(Id.DeviceId), GetTypeHash(static_cast<int32>(Id.TouchIndex)));
	}

private:

	/**
	 * The device being touched.
	 */
	UPROPERTY(BlueprintReadOnly, Category = "Input|Touch", meta = (AllowPrivateAccess = "true"))
	FInputDeviceId DeviceId = INPUTDEVICEID_NONE;

	/**
	 * The device-specific index that differentiates this touch from other simultaneous touch inputs on the same device.
	 */
	UPROPERTY(BlueprintReadOnly, Category = "Input|Touch", meta = (AllowPrivateAccess = "true"))
	TEnumAsByte<ETouchIndex::Type> TouchIndex = ETouchIndex::Touch1;
};
...
struct EKeys
{
	...
	static INPUTCORE_API const FKey Gesture_Pan;
	static INPUTCORE_API const FKey Gesture_Tap;
	static INPUTCORE_API const FKey Gesture_LongPress;
