...
struct FRecastBuildConfig : public rcConfig
{
	...
	/** if set, minimize base link pool size, else use previous larger base link pool. */
	uint32 bMinimizeLinkPoolSize : 1;
	...
	/** Average layers per tile */
	float AverageLayersPerTile;
...
struct FRecastRawGeometryElement
{
	...
	// Nav area for collision triangles. If set, replaces RC_WALKABLE_AREA during rasterization.
	...
	// Extracts AreaClass from nav modifier (UNavCollision creates single area modifier).
	NAVIGATIONSYSTEM_API static TSubclassOf<UNavAreaBase> GetCollisionAreaClass(const FCompositeNavModifier& InModifier);
};
...
typedef TArray<FGeneratedNavigationLink, TInlineAllocator<64>> TInlineGeneratedNavLinkArray;
...
protected:
	...
	NAVIGATIONSYSTEM_API ETimeSliceWorkResult RasterizeGeometryRecastTimeSliced(FNavMeshBuildContext& BuildContext, const TArray<FVector::FReal>& Coords, const TArray<int32>& Indices, const FBox& CoordsBounds, const rcRasterizationFlags RasterizationFlags, FTileRasterizationContext& RasterContext, TSubclassOf<UNavAreaBase> AreaClass = nullptr);
	NAVIGATIONSYSTEM_API void RasterizeGeometryRecast(FNavMeshBuildContext& BuildContext, const TArray<FVector::FReal>& Coords, const TArray<int32>& Indices, const FBox& CoordsBounds, const rcRasterizationFlags RasterizationFlags, FTileRasterizationContext& RasterContext, TSubclassOf<UNavAreaBase> AreaClass = nullptr);
	...
	NAVIGATIONSYSTEM_API void RasterizeGeometry(FNavMeshBuildContext& BuildContext, const TArray<FVector::FReal>& Coords, const TArray<int32>& Indices, const FBox& CoordsBounds, const FTransform& LocalToWorld, const rcRasterizationFlags RasterizationFlags, FTileRasterizationContext& RasterContext, TSubclassOf<UNavAreaBase> AreaClass = nullptr);
	...
	dtStatus BuildTileCacheLinks(FNavMeshBuildContext& BuildContext, struct dtTileCacheAlloc* alloc, const dtTileCacheLayer& layer,
		const struct dtTileCacheContourSet& lcset, const float DefaultSnapHeight, TInlineGeneratedNavLinkArray& OutGeneratedLinks) const;
	...
	NAVIGATIONSYSTEM_API void MarkDynamicAreas(FNavMeshBuildContext& BuildContext, dtTileCacheLayer& Layer);
	NAVIGATIONSYSTEM_API void MarkDynamicArea(FNavMeshBuildContext& BuildContext, const FAreaNavModifier& Modifier, const FTransform& LocalToWorld, dtTileCacheLayer& Layer);
	NAVIGATIONSYSTEM_API void MarkDynamicArea(FNavMeshBuildContext& BuildContext, const FAreaNavModifier& Modifier, const FTransform& LocalToWorld, dtTileCacheLayer& Layer, const int32 AreaId, const int32* ReplaceAreaIdPtr);
	...
	UE_DEPRECATED(5.8, "Please use the overload with the added BuildContext.")
...
protected:
	...
	/** Returns the number of tiles currently populated with navigation data. O(1) — reads the maintained counter directly. */
	int32 GetNumActiveTiles() const { return NumActiveTiles; }

	NAVIGATIONSYSTEM_API void RemoveLayers(const FIntPoint& Tile, TArray<FNavTileRef>& UpdatedTiles);
	NAVIGATIONSYSTEM_API void RemoveAllTiles(TArray<FNavTileRef>& UpdatedTiles);
