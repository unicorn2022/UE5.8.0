...
struct FSupportedAreaData
{
	...
	UPROPERTY(VisibleAnywhere, Category=Areas)
...
protected:
	...
	*  Setting bForceRebuildOnLoad to true can override this behavior. */
	...
	/** Set when this nav data was spawned at runtime because no serialized
	 *  instance existed in the loaded level. Gates that normally prevent
	 *  rebuilding Static navmesh in game worlds check this flag to allow
	 *  a one-time rebuild so NPCs can pathfind. Cleared once the rebuild
	 *  completes, after which the nav data resumes normal Static behavior. */
	uint32 bRequiresInitialRebuild : 1;
...
public:
	NAVIGATIONSYSTEM_API void DestroyActor();
	...
	NAVIGATIONSYSTEM_API virtual void OnUnregistered();
...
public:
	...
	bool RequiresInitialRebuild() const { return bRequiresInitialRebuild; }
	void MarkRequiresInitialRebuild() { bRequiresInitialRebuild = true; }
	void ClearRequiresInitialRebuild() { bRequiresInitialRebuild = false; }
	...
	bool NeedsRebuildOnLoad() const { return bForceRebuildOnLoad || bRequiresInitialRebuild; }
...
public:
	...
	/**	Batches FindMoveAlongSurface calls for efficiency.
	 *	@note Refer to FindMoveAlongSurface function documentation for more information
	 */
	virtual void BatchFindMoveAlongSurface(TArray<FNavigationFindMoveAlongSurfaceWork>& Workload, FSharedConstNavQueryFilter Filter = NULL, const UObject* Querier = NULL) const PURE_VIRTUAL(ANavigationData::BatchFindMoveAlongSurface, );
...
protected:
	...
	UPROPERTY(VisibleAnywhere, Category=Areas, meta=(TitleProperty="{AreaID}: {AreaClassName}"))
