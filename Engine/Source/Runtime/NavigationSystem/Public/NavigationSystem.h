...
{	
	...
	extern NAVIGATIONSYSTEM_API const FName DebugTimeSliceDefaultSectionName;
...
namespace ENavigationBuildLock
{
	enum Type
	{
		...
		Custom = 1 << 4,
		AsyncLoadLock = 1 << 5			// used in editor worlds to prevent automatic update when loading a map using async loading 
	};
}
...
protected:
	...
	UPROPERTY()
	uint32 bDirtyNavDataPackageOnNavigationGenerationFinished : 1;

#if WITH_EDITORONLY_DATA	
	/** In editor worlds, wait for async loading to finish before allowing to build navigation automatically (true by default). */
	UPROPERTY(config, EditAnywhere, Category=NavigationSystem)
	uint32 bWaitForAsyncLoadingBeforeBuildingNavigationAutomatically : 1 = true;
#endif	
...
#if WITH_EDITOR	
	void UpdateDelayedUnlockeRegistration(const TArray<FAssetCompileData>& AssetCompileData);
	void RegisterDelayedUnlock(float DelayInSeconds);
	void UnregisterDelayedUnlock();

	FDelegateHandle OnAssetPostCompileEventHandle;
	FTSTicker::FDelegateHandle DelayedUnlockHandle;
#endif // WITH_EDITOR
...
public:
	...
	bool SupportsNavigationGeneration() const { return bSupportRebuilding; }

	bool DirtyNavDataPackageOnNavigationGenerationFinished() const;
...
public:
	...
	NAVIGATIONSYSTEM_API void FlushPendingOperations();

	/** Update any pending FNavigationDirtyElement in the navigation system default navigation octree. */
	NAVIGATIONSYSTEM_API void ProcessPendingOctreeUpdates();
	
protected:
	NAVIGATIONSYSTEM_API void ResetDefaultDirtyAreasController();
...
protected:
	...
	TArray<TWeakObjectPtr<ANavigationData>> NavigationDataToRemove;
...
protected:
	...
	/** Returns true if any registered navigation data supports runtime generation.
	 *  Unlike GetRuntimeGenerationType which returns the raw config enum,
	 *  this also accounts for nav data that was spawned at runtime and needs rebuilding. */
	NAVIGATIONSYSTEM_API bool DoesAnyNavDataSupportRuntimeGeneration() const;

	/** Returns true if any registered navigation data requires geometry to be stored in the nav octree.
	 *  This is true for Dynamic nav data, or nav data that was spawned at runtime (which needs a full rebuild).
	 *  DynamicModifiersOnly does NOT require geometry storage. */
	NAVIGATIONSYSTEM_API bool DoesAnyNavDataRequireGeometryStorage() const;

	UE_DEPRECATED(5.8, "Use LogNavDataRegistrationResult receiving ANavigationData* instead.")
	NAVIGATIONSYSTEM_API void LogNavDataRegistrationResult(ERegistrationResult);
	
	NAVIGATIONSYSTEM_API void LogNavDataRegistrationResult(ERegistrationResult, const ANavigationData* NavigationData) const;
