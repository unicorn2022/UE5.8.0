...
void FNavigationPath::SetGoalActorObservation(const AActor& ActorToObserve, float TetherDistance)
{
	if (NavigationDataUsed.IsValid() == false)
	{
		...
		UE_LOGF(LogNavigation, Warning, "Updating navigation path on goal actor's location change is available only for navigation-generated paths. Called for %ls"
...
double UNavigationPath::GetLengthFromPosition(FVector Position, int32 NextPathPointIndex) const
{
	check((SharedPath.IsValid() && SharedPath->IsValid()) == !!bIsValid);
	...
	{
		UE_LOGF(LogNavigation, Warning, "Negative PathPointIndex passed in (%d).", NextPathPointIndex);
		return 0.;
	}

	return SharedPath->GetLengthFromPosition(Position, NextPathPointIndex);
}
