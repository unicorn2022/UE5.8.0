...
#if WITH_EDITOR
bool UNavAreaMeta_SwitchByAgent::bUpdateAgentConfigCallbackRegistered = false;
...
#if WITH_EDITOR
	if (HasAnyFlags(RF_ClassDefaultObject) && !bUpdateAgentConfigCallbackRegistered)
	{
		FCoreDelegates::OnAllModuleLoadingPhasesComplete.AddLambda([]() { GetMutableDefault<UNavAreaMeta_SwitchByAgent>()->UpdateAgentConfig(); });
		bUpdateAgentConfigCallbackRegistered = true;
	}
#endif //#if WITH_EDITOR
}
...
void UNavAreaMeta_SwitchByAgent::UpdateAgentConfig()
{
	...
	{
		UE_LOGF(LogNavigation, Error, "Navigation system supports %d agents, but only %d can be shown in %ls properties!",
