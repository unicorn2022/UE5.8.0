	...
	const ENotifyOnSuccess NotifyOnSuccess)
{
	...
	if (const TSharedPtr<FNavigationElement> ExistingElement = GetMutableNavigationElementForUObject(&NavRelevantObject))
	{
		...
		const FNavigationElementHandle Handle = ExistingElement->GetHandle();

		// Create a new FNavigationElement instance rather than overwriting the existing one in-place.
		// The old instance may still be referenced by the navigation octree or an in-flight tile generator
		// (via FNavigationRelevantData::SourceElement) and must remain valid until those holders release it.
		const TSharedRef<FNavigationElement> NewElement = MakeShared<FNavigationElement>(NavRelevantInterface);
		{
			UE_MT_SCOPED_WRITE_ACCESS(NavElementAccessDetector);
			NavRelevantElements.Emplace(Handle, NewElement);
		}
		...
		UE_LOGF(LogNavigation, VeryVerbose, "%s [already registered - updating] (%ls:%ls) Bounds: [%ls]->[%ls]", __FUNCTION__,
			*GetNameSafe(NavRelevantObject.GetOuter()), *GetNameSafe(&NavRelevantObject),
			*PreviousBounds.ToString(), *NewElement->GetBounds().ToString());
	...
	if (NavRelevantInterface.IsNavigationRelevant())
	{
		if (const TSharedPtr<const FNavigationElement> SharedElement = AddNavigationElement(FNavigationElement(NavRelevantInterface), NotifyOnSuccess))
		{
			...
			UE_LOGF(LogNavigation, VeryVerbose, "%s [registered] (%ls:%ls) Bounds: [%ls]", __FUNCTION__,
				*GetNameSafe(NavRelevantObject.GetOuter()), *GetNameSafe(&NavRelevantObject),
	...
	UE_LOGF(LogNavigation, VeryVerbose, "%s [skipped: not relevant] (%ls:%ls)", __FUNCTION__,
		*GetNameSafe(NavRelevantObject.GetOuter()), *GetNameSafe(&NavRelevantObject));
	return nullptr;
}
...
{
	UE_LOGF(LogNavigation, VeryVerbose, "%s (%ls:%ls)", __FUNCTION__,
