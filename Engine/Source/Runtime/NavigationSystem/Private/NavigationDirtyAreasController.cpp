...
void FNavigationDirtyAreasController::AddAreas(const TConstArrayView<FBox> NewAreas, const ENavigationDirtyFlag Flags, const TFunction<const TSharedPtr<const FNavigationElement>()>& ElementProviderFunc, const FNavigationDirtyElement* DirtyElement, const FName& DebugReason)
{
	...
	if (bUseWorldPartitionedDynamicMode)
	{
		...
		if (const bool bIsFromVisibilityChange = (DirtyElement && DirtyElement->bIsFromVisibilityChange) || (SourceElement && SourceElement->IsFromLevelVisibilityChange()))
		{
			...
			{
				UE_LOGF(LogNavigationDirtyArea, VeryVerbose, "Ignoring dirtyness (visibility changed and in base navmesh). (element: %ls from: %ls)",
	...
	for (const FBox& NewArea : NewAreas)
	{
		...
		{
			UE_LOGF(LogNavigationDirtyArea, Warning, "Adding an oversized dirty area: %ls | Threshold: %.2f", *DumpExtraInfo(), DirtyAreaWarningSizeThreshold);
		}
		else
		{
			UE_LOGF(LogNavigationDirtyArea, VeryVerbose, "Adding dirty area object: %ls", *DumpExtraInfo());
		}
#endif // !UE_BUILD_SHIPPING
	...
	UE_CLOGF(NumInvalidBounds > 0 || NumEmptyBounds > 0, LogNavigationDirtyArea, Warning,
		"Skipped some dirty area creation due to: %d invalid bounds, %d empty bounds (element: %ls, from: %ls)",
...
void FNavigationDirtyAreasController::Reset()
{
	...
	UE_LOGF(LogNavigationDirtyArea, VeryVerbose, "%s: Reseting All Dirty Areas. DirtyAreas.Num = [%d]",__FUNCTION__, DirtyAreas.Num());
