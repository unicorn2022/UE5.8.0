...
void ANavigationData::InstantiateAndRegisterRenderingComponent()
{
	...
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	{
		...
		if (RenderingComp)
		{
			...
			RenderingComp->Rename(NULL, NULL, REN_DontCreateRedirectors | REN_ForceGlobalUnique | REN_DoNotDirty | REN_NonTransactional | REN_AllowPackageLinkerMismatch);
		}
...
void ANavigationData::DestroyActor()
{
	UWorld* World = GetWorld();
	if (World && World->IsEditorWorld() && !World->IsPlayInEditor())
	{
		// Editor-aware destroy (notifies editor, handles actors)
		World->EditorDestroyActor(this, /*bShouldModifyLevel*/true);
	}
	else if (World)
	{
		// Runtime destroy
		Destroy(/*bNetForce*/false, /*bShouldModifyLevel*/false);
	}
}
...
void ANavigationData::RebuildAll()
{
	...
	if (UWorld* World = GetWorld())
	{
		...
		if (NavSys)
		{
			// This used to be in UNavigationSystemV1::RebuildAll but since we just loaded new assets it needs to be done here.
			NavSys->FlushPendingOperations();
		}
	}
	
	UE_LOGF(LogNavigationDataBuild, Display, "   %ls load time: %.2fs", ANSI_TO_TCHAR(__FUNCTION__), (FPlatformTime::Seconds() - LoadTime));
...
{
	UE_LOGF(LogNavigation, Log, "%s %ls", __FUNCTION__, *GetNameSafe(this));
...
void ANavigationData::OnNavAreaAdded(const UClass* NavAreaClass, int32 AgentIndex)
{
	...
	for (int32 i = 0; i < SupportedAreas.Num(); i++)
	{
		if (SupportedAreas[i].AreaClassName == AreaClassName)
		{
			...
			UE_VLOG_UELOG(this, LogNavigation, VeryVerbose, TEXT("%s: updated area %s with ID %d"), *GetFullName(), *AreaClassName, SupportedAreas[i].AreaID);
			return;
		}
	}
	...
	UE_VLOG_UELOG(this, LogNavigation, VeryVerbose, TEXT("  %s registered area %s with ID %d"), *GetPathName(), *AreaClassName, NewAgentData.AreaID);
}
