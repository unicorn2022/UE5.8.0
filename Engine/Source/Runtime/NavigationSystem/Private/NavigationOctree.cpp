...
void FNavigationOctree::DemandLazyDataGathering(FNavigationRelevantData& ElementData)
{
	...
	if (ElementData.IsPendingLazyModifiersGathering())
	{
		...
		if (bCanOutputDurationWarning)
		{
			...
			{
				UE_LOGF(LogNavigation, Warning, "The time (%f sec) for gathering navigation data on a navigation element exceeded the time limit (%f sec) | Element = %ls",
					DeltaTime,
...
void FNavigationOctree::AddNode(const FBox& Bounds, FNavigationOctreeElement& OctreeElement)
{
	...
	UE_LOGF(LogNavigation, VeryVerbose, "%s: '%ls' bounds: [%ls]", __FUNCTION__, *SourceElement.GetName(), *Bounds.ToString());
	...
	if (UE::NavigationOctree::Private::bValidateConsistencyWhenAddingNode)
	{
		if (const INavRelevantInterface* NavRelevantInterface = Cast<INavRelevantInterface>(SourceElement.GetWeakUObject().Get()))
		{
			...
			{
				UE_LOGF(LogNavigation, Warning, "%s: '%ls' bounds changed between element's creation and its addition to the octree: [%ls] --> [%ls]",
			...
			{
				UE_LOGF(LogNavigation, Warning, "%s: '%ls' parent changed between element's creation and its addition to the octree: [%ls] --> [%ls]",
	...
	{
		UE_LOGF(LogNavigation, Warning, "%s: %ls bounds, ignoring %ls.", __FUNCTION__, !Bounds.IsValid ? TEXT("Invalid") : TEXT("Empty"), *SourceElement.GetFullName());
		return;
	}
