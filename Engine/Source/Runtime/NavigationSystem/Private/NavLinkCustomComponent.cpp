...
#if WITH_EDITOR
	UE_CLOGF(NewUniqueId.IsLegacyId(), LogNavLink, Verbose, "%s Navlink using LinkUserId id %llu [%ls] is being updated with old style link ID. Please move to the new system. See FNavLinkId::GenerateUniqueId()", __FUNCTION__, CustomLinkId.GetId(), *GetFullName());
#endif
...
void UNavLinkCustomComponent::OnRegister()
{
	...
	if (CustomLinkId == FNavLinkId::Invalid || CustomLinkId.IsLegacyId() == false)
	{
		...
		UE_LOGF(LogNavLink, VeryVerbose, "%s navlink id generated %llu [%ls].", __FUNCTION__, CustomLinkId.GetId(), *GetFullName());

		if (OldCustomLinkId != CustomLinkId)
		{
			Modify(true);

			// Baked navmesh has off-mesh connections with the old ID — rebuild the tile
			// so they get regenerated with the new one.
			if (UNavigationSystemV1* NavSys = FNavigationSystem::GetCurrent<UNavigationSystemV1>(GetWorld()))
			{
				const FBox LinkBounds = UNavigationSystemV1::ComputeCustomLinkBounds(*this);
				if (LinkBounds.IsValid && !LinkBounds.GetSize().IsNearlyZero())
				{
					NavSys->AddDirtyArea(LinkBounds, ENavigationDirtyFlag::Geometry);
				}
			}
		}
	}
	...
	if (CustomLinkId == FNavLinkId::Invalid)
	{
		...
		UE_LOGF(LogNavLink, VeryVerbose, "%s incremental navlink id generated %llu [%ls].", __FUNCTION__, CustomLinkId.GetId(), *GetFullName());
	}
#endif // WITH_EDITOR
