...
static const FName NAVCOLLISION_FORMAT = "NavCollision_Chaos";
...
void UNavCollision::GetNavigationModifier(FCompositeNavModifier& Modifier, const FTransform& LocalToWorld)
{
	...
	// For non-dynamic obstacles with surface area, pass area class only without convex modifiers.
	if (!bIsDynamicObstacle && bUseSurfaceArea && AreaClass)
	{
		FAreaNavModifier AreaModifier;
		AreaModifier.SetAreaClass(UseAreaClass);
		Modifier.Add(AreaModifier);
...
bool UNavCollision::HasSurfaceAreaClass() const
{
	return bUseSurfaceArea && AreaClass != nullptr;
}
...
void UNavCollision::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);

	// Force a geometry rebuild when bUseSurfaceArea or AreaClass changes to prevent stale cached data.
	if (PropertyChangedEvent.GetPropertyName() == GET_MEMBER_NAME_CHECKED(UNavCollision, AreaClass) ||
		PropertyChangedEvent.GetPropertyName() == GET_MEMBER_NAME_CHECKED(UNavCollision, bUseSurfaceArea))
	{
		InvalidateCollision();
	}
}
...
void UNavCollision::Serialize(FArchive& Ar)
{
	...
	{
		UE_LOGF(LogNavigation, Fatal, "This platform requires cooked packages, and NavCollision data was not cooked into %ls.", *GetFullName());
	}
...
FByteBulkData* UNavCollision::GetCookedData(FName Format)
{
	...
	if (!bContainedData && CVarNavCollisionAvailable.GetValueOnAnyThread() != 0)
	{
		...
		{
			UE_LOGF(LogNavigation, Error, "Attempt to build nav collision data for %ls when we are unable to. This platform requires cooked packages.", *GetPathName());
			return nullptr;
		}
	...
	UE_CLOGF(!Result, LogNavigation, Error, "Failed to read CoockedDataFormat for %ls.", *GetPathName());
...
void UNavCollision::CopyUserSettings(const UNavCollision& OtherData)
{
	...
	bUseSurfaceArea = OtherData.bUseSurfaceArea;
