...
DEFINE_STAT(STAT_Navigation_DetourPERM_TILE_DYNLINK);
...
{	
	...
	const FName DebugTimeSliceDefaultSectionName("DefaultSection");
...
static FAutoConsoleCommandWithWorldArgsAndOutputDevice CmdNavDirtyAreaAroundPlayer(
	...
	TEXT("Dirty all tiles in a square area around the player using provided value as extent (in cm), using 10 meters if not specified."),
...
void FNavRegenTimeSliceManager::SetMinTimeSliceDuration(double NewMinTimeSliceDuration)
{
	...
	UE_LOGF(LogNavigationDataBuild, Verbose, "Navigation System: MinTimeSliceDuration = %f", MinTimeSliceDuration);
}
...
void FNavRegenTimeSliceManager::SetMaxTimeSliceDuration(double NewMaxTimeSliceDuration)
{
	...
	UE_LOGF(LogNavigationDataBuild, Verbose, "Navigation System: MaxTimeSliceDuration = %f", MaxTimeSliceDuration);
}
...
void FNavRegenTimeSliceManager::SetMaxDesiredTileRegenDuration(float NewMaxDesiredTileRegenDuration)
{
	...
	UE_LOGF(LogNavigationDataBuild, Verbose, "Navigation System: MaxDesiredTileRegenDuration = %f", MaxDesiredTileRegenDuration);
}
		...
		for (int32 NavDataIndex = 0; bLog && NavDataIndex < NavDataSet.Num(); ++NavDataIndex)
		{
			if (TileHistoryData.IsValidIndex(NavDataIndex))
			{
				...
				if (HistoryData.Num() > 0)
				{
					...
					UE_LOGF(LogNavigationHistory, Log, "%-35ls Median tile stats: regen time: %2.2f ms, regen frames %lld, wait time: %4.f ms (high regen time: %2.2f ms, high wait time: %4.f ms) regen count: %i, regen/s: %0.2f",
...
UNavigationSystemV1::UNavigationSystemV1(const FObjectInitializer& ObjectInitializer)
	...
	, bDirtyNavDataPackageOnNavigationGenerationFinished(false)
	...
	, CurrentlyDrawnNavDataIndex(0)
{
	...
	if (HasAnyFlags(RF_ClassDefaultObject) == false)
	{
		...
		{
			UE_LOGF(LogNavigation, Warning, "UNavigationObjectRepository is required for navigation system operations.");
		}
	}
...
void UNavigationSystemV1::DoInitialSetup()
{
	...
	UE_LOGF(LogNavigation, Verbose, "%s", __FUNCTION__);
...
#if WITH_EDITOR		
	if (bWaitForAsyncLoadingBeforeBuildingNavigationAutomatically && GetIsAutoUpdateEnabled() && OperationMode == FNavigationSystemRunMode::EditorMode)
	{
		UE_LOGF(LogNavigation, Verbose, "%s    Adding ENavigationBuildLock::AsyncLoadLock.", __FUNCTION__);
		AddNavigationBuildLock(ENavigationBuildLock::AsyncLoadLock);

		OnAssetPostCompileEventHandle = FAssetCompilingManager::Get().OnAssetPostCompileEvent().AddUObject(this, &UNavigationSystemV1::UpdateDelayedUnlockeRegistration);

		// Run it at least once to make sure we don't stay locked.
		UpdateDelayedUnlockeRegistration({});
	}
#endif // WITH_EDITOR	
...
void UNavigationSystemV1::PostInitProperties()
{
	...
	if (HasAnyFlags(RF_ClassDefaultObject) == false)
	{
		...
		if (GIsEditor && GEngine)
		{
			...
			FEditorDelegates::OnMapOpened.AddUObject(this, &UNavigationSystemV1::OnMapOpened);
		}
#endif // WITH_EDITOR
...
bool UNavigationSystemV1::ConditionalPopulateNavOctree()
{
	...
	UE_LOGF(LogNavigationDirtyArea, VeryVerbose, "%s: Reseting Dirty Areas added during octree construction. DirtyAreas.Num = [%d].", __FUNCTION__, DefaultDirtyAreasController.DirtyAreas.Num());
	...
	if (bSupportRebuilding)
	{
		...
		{
			const bool bStoreNavGeometry = DoesAnyNavDataRequireGeometryStorage();
...
{
	UE_LOGF(LogNavigation, Verbose, "%s, Mode: %ls", __FUNCTION__, *UEnum::GetValueAsString(Mode));
			...
			if (GetDefaultNavDataInstance(FNavigationSystem::DontCreate) != nullptr)
			{
				...
				for (TActorIterator<ANavigationData> It(World); It; ++It)
				{
					...
					if (NavData != nullptr)
					{
						...
						LogNavDataRegistrationResult(Result, NavData);
	...
	// If any nav data was spawned at runtime (not loaded from disk), trigger a rebuild.
	// Must run after ConditionalPopulateNavOctree so the geometry is available.
	if (GetWorld()->IsGameWorld())
	{
		const bool bRequiresInitialRebuild = NavDataSet.ContainsByPredicate(
			[](const ANavigationData* NavData) { return NavData && NavData->RequiresInitialRebuild(); });
		if (bRequiresInitialRebuild)
		{
...
void UNavigationSystemV1::RegisterNavigationDataInstances()
{
	...
	UE_LOGF(LogNavigation, Verbose, "%s World: %ls", __FUNCTION__, *GetNameSafe(World));
...
{
	UE_LOGF(LogNavigation, Verbose, "%s bNewEnable: %i", __FUNCTION__, bNewEnable);
...
void UNavigationSystemV1::PerformAsyncQueries(TArray<FAsyncPathFindingQuery> PathFindingQueries)
{
	...
	UE_LOGF(LogNavigation, Log, "Async pathfinding queries: %d completed, %d postponed to next frame", NumProcessed, NumPostponed);
}
...
const ANavigationData* UNavigationSystemV1::GetNavDataForProps(const FNavAgentProperties& AgentProperties) const
{
	...
	UE_CLOGF(!(AgentProperties.IsValid() || AgentProperties.PreferredNavData.IsValid()), LogNavigation, Warning, "Looking for NavData using invalid FNavAgentProperties.");
	...
	if (NavDataInstance == nullptr)
	{
		...
		for (TArray<FNavAgentProperties>::TConstIterator It(AgentPropertiesList); It; ++It)
		{
			...
			// Make sure to only allow NavData and Agent matches that have matching bCanFly.
			// This prevents ground-based Pawns from inadvertently getting associated with flying-based NavData when their PreferredNavData isn't set.

			if (NavIt.bCanFly != AgentProperties.bCanFly)
			{
				continue;
			}
...
ANavigationData* UNavigationSystemV1::GetDefaultNavDataInstance(FNavigationSystem::ECreateIfMissing CreateNewIfNoneFound)
{
	...
	if (!IsValid(MainNavData))
	{
		...
		if (MainNavData != nullptr && !MainNavData->IsRegistered())
		{
			const ERegistrationResult Result = RegisterNavData(MainNavData);
			LogNavDataRegistrationResult(Result, MainNavData);
		}
	}
...
FBox UNavigationSystemV1::GetWorldBounds() const
{
	...
	if (GetWorld() != nullptr)
	{
		...
		else
		{
			...
			for (FActorIterator It(GetWorld()); It; ++It)
			{
				...
				{
					UE_LOGF(LogNavigation, VeryVerbose, "%s %ls is NavRelevant with bounds %ls (%ls).",
						__FUNCTION__, *It->GetName(), *(*It)->GetComponentsBoundingBox().ToCompactString(), *It->GetFullName());
	...
	UE_LOGF(LogNavigation, Log, "%s NavigableWorldBounds: %ls.", __FUNCTION__, *NavigableWorldBounds.ToCompactString());
...
void UNavigationSystemV1::ApplyWorldOffset(const FVector& InOffset, bool bWorldShift)
{
	...
	// Only fully dynamic navmesh needs a complete geometry rebuild after a world shift.
...
void UNavigationSystemV1::RequestRegistrationDeferred(ANavigationData& NavData)
{
	...
	{
		UE_LOGF(LogNavigation, Verbose, "%s Adding to registration queue: %ls.", __FUNCTION__, *GetFullNameSafe(&NavData));
		NavDataRegistrationQueue.AddUnique(&NavData);
	}
	else
	{
		UE_LOGF(LogNavigation, Warning, "%s Navigation System: registration queue full! System: %ls NavData: %ls.", __FUNCTION__, *GetPathNameSafe(this), *GetPathNameSafe(&NavData));
	}
}
...
void UNavigationSystemV1::ProcessRegistrationCandidates()
{
	...
	for (int32 CandidateIndex = CandidatesCount - 1; CandidateIndex >= 0; --CandidateIndex)
	{
		...
		if (OwningLevel && OwningLevel->bIsVisible)
		{
			...
			LogNavDataRegistrationResult(Result, NavDataPtr);

			if (Result != RegistrationSuccessful && Result != RegistrationFailed_DataPendingKill)
			{
				UE_LOGF(LogNavigation, Warning, "%s %ls will be removed. This warning will disapear once the map is saved.", 
					__FUNCTION__, *GetNameSafe(NavDataPtr));
				
#if WITH_EDITOR				
				if (GEditor)
				{
					NavigationDataToRemove.Add(NavDataPtr);	
				}
				else
#endif // WITH_EDITOR					
				{
		...
		{
			UE_LOGF(LogNavigation, Verbose, "%s   Skipped %ls because owning level not visible.", __FUNCTION__, *GetFullNameSafe(NavDataPtr));
		}
	}	
...
{
	UE_LOGF(LogNavigation, Verbose, "%s %ls", __FUNCTION__, *GetFullNameSafe(NavData));
...
void UNavigationSystemV1::RegisterCustomLink(INavLinkCustomInterface& CustomLink)
{
	...
	if (CustomNavLinksMap.Contains(OldId))
	{
		if (OldId.IsLegacyId() == false)
		{
			...
			// This should be very unlikely to occur, if its causing issues we should add code to handle this being careful to account for the editor world being run as a commandlet to cook and build paths on separate runs.
			UE_CLOGF(!bGenerateNewId, LogNavLink, Warning, "%s navlink ID %llu is clashing with existing ID (Owner: %ls). "
		...
		if (NewId != OldId)
		{
			...
			UE_LOGF(LogNavLink, VeryVerbose, "%s new navlink ID %llu.", __FUNCTION__, CustomLink.GetId().GetId());
	...
	UE_CLOGF(bGenerateNewId && CustomNavLinksMap.Contains(CustomLink.GetId()), LogNavLink, Warning, "%s New navlink ID %llu is clashing with existing ID (Owner: %ls).",
...
void UNavigationSystemV1::RequestCustomLinkRegistering(INavLinkCustomInterface& CustomLink, UObject* Owner)
{
	...
	if (Owner != nullptr)
	{
		...
		{
			UE_LOGF(LogNavLink, Log, "%s 0x%p", __FUNCTION__, &CustomLink);
...
FBox UNavigationSystemV1::ComputeCustomLinkBounds(const INavLinkCustomInterface& CustomLink)
{
	...
	// If the owner is a navdata, the NavLink Custom Interface is shared for multiple link instances.
	// It does not have individual link data so act has if it had no owner actor.
	if (OwnerActor && !OwnerActor->IsA(ANavigationData::StaticClass()))
	{
...
void UNavigationSystemV1::DescribeFilterFlags(const TArray<FString>& FlagsDesc) const
{
	...
	const FString CustomNameMeta = "DisplayName";
...
void UNavigationSystemV1::RegisterComponentToNavOctree(UActorComponent* Comp)
{
	...
	{
		UE_LOGF(LogNavigation, VeryVerbose
			, "%s: %ls registration skipped (waiting for actor to register)", __FUNCTION__, *GetNameSafe(Comp));
		return;
	}
...
void UNavigationSystemV1::ResetDefaultDirtyAreasController()
{
...
void UNavigationSystemV1::ProcessPendingOctreeUpdates()
{
	FNavigationDataHandler NavHandler(DefaultOctreeController, DefaultDirtyAreasController);
	NavHandler.ProcessPendingOctreeUpdates();
}

void UNavigationSystemV1::FlushPendingOperations()
{
	// Make sure all changes are reflected in the octree
	ProcessPendingOctreeUpdates();
	...
	TFunctionRef<void(UNavigationSystemV1&, const TSharedRef<const FNavigationElement>&)> InCallback)
{
	...
	else
	{
		...
		UE_LOGF(LogNavigation, VeryVerbose,
			"%s: %ls Registering to the repository (NavigationSystem not available)", __FUNCTION__, *InObject.GetName());
...
{
	UE_LOGF(LogNavigation, Verbose, "%s Update request: %i", __FUNCTION__, UpdateRequest.UpdateRequest);
		...
		case FNavigationBoundsUpdateRequest::Removed:
			{
				...
				{
					UE_LOGF(LogNavigation, Log, "%s Remove bounds %ls", __FUNCTION__, *RegisteredNavBounds[ExistingElementId].AreaBox.ToCompactString());
		...
		case FNavigationBoundsUpdateRequest::Updated:
			{
				if (ExistingElementId.IsValidId())
				{
					UE_LOGF(LogNavigation, Log, "%s Update bounds %ls", __FUNCTION__, *RegisteredNavBounds[ExistingElementId].AreaBox.ToCompactString());
				...
				{
					UE_LOGF(LogNavigation, Log, "%s Add bounds %ls", __FUNCTION__, *Request.NavBounds.AreaBox.ToCompactString());
...
void UNavigationSystemV1::Build()
{
	...
	UE_LOGF(LogNavigationDataBuild, Display, "UNavigationSystemV1::Build started...");

	UWorld* World = GetWorld();
	if (!World)
	{
		UE_LOGF(LogNavigation, Error, "Unable to build navigation due to missing World pointer");
		return;
	}
	...
	{
		UE_LOGF(LogNavigation, Warning, "%s Navigation NOT building because navigation build is locked (flags: 0x%x).", __FUNCTION__, NavBuildingLockFlags);
		return;
	}
	...
	UE_LOGF(LogNavigationDataBuild, Display, "UNavigationSystemV1::Build total execution time: %.2fs", float(FPlatformTime::Seconds() - BuildStartTime));
	UE_LOGF(LogNavigation, Display, "UNavigationSystemV1::Build total execution time: %.5fs", float(FPlatformTime::Seconds() - BuildStartTime));
}
...
void UNavigationSystemV1::SpawnMissingNavigationDataInLevel(const TBitArray<>& InInstantiatedMask, ULevel* InLevel/*=nullptr*/)
{
	...
	UE_LOGF(LogNavigation, Verbose, "%s Level: %ls, OuterWorld: %ls", __FUNCTION__, *GetNameSafe(InLevel), *GetNameSafe(NavWorld));
	...
	for (int32 AgentIndex = 0; AgentIndex < AllSupportedAgentsCount; ++AgentIndex)
	{
		...
		{
			UE_LOGF(LogNavigation, Verbose, "%s   AgentIndex: %i is supported. NavConfig name: %ls.", __FUNCTION__, AgentIndex, *NavConfig.Name.ToString());
			...
			if (NavWorld->WorldType != EWorldType::Editor && NavDataCDO->GetRuntimeGenerationMode() == ERuntimeGenerationType::Static)
			{
				...
				{
					UE_LOGF(LogNavigation, Log, "Not spawning navigation data for %ls since indicated NavigationData type is not configured for dynamic generation",
						*NavConfig.Name.ToString());
					continue;
				}
			}

			// Check if it's already wating for registration
			const bool bIsWaitingForRegistration = NavDataRegistrationQueue.ContainsByPredicate([&NavConfig](const TObjectPtr<ANavigationData>& NavData)
			{
				return NavData->GetConfig().Name == NavConfig.Name;
			});
			
			if (bIsWaitingForRegistration)
			{
				UE_LOGF(LogNavigation, Verbose, "%s   Skipping %ls already waiting for registration.", __FUNCTION__, *NavConfig.Name.ToString());
				continue;
			}

			// Warn that we're about to spawn and build a navmesh that wasn't saved with this level.
			// This triggers an unexpected runtime rebuild which has a performance cost.
			// The level author should rebuild navigation in the editor and resave the level to prevent this.
			if (NavWorld->IsGameWorld() && NavDataCDO->GetRuntimeGenerationMode() == ERuntimeGenerationType::Static)
			{
				UE_LOGF(LogNavigation, Warning, "SpawnMissingNavigationData: No saved navigation data found for '%ls' in level '%ls'. "
					"Spawning and rebuilding at runtime to recover - this has a performance cost. "
					"Rebuild navigation and resave the level to fix this.",
					*NavConfig.Name.ToString(), *GetNameSafe(InLevel ? InLevel : NavWorld->PersistentLevel.Get()));
			}
			ANavigationData* Instance = CreateNavigationDataInstanceInLevel(NavConfig, InLevel);
			if (Instance)
			{
				// Only flag Static navmesh for an initial rebuild - Dynamic already has its own
				// runtime pipeline, and DynamicModifiersOnly must not trigger geometry storage.
				if (NavWorld->IsGameWorld() && Instance->GetRuntimeGenerationMode() == ERuntimeGenerationType::Static)
				{
					Instance->MarkRequiresInitialRebuild();
				}
				RequestRegistrationDeferred(*Instance);
			}
			else
			{
				UE_LOGF(LogNavigation, Warning, "%s   Was not able to create navigation data for SupportedAgent[%d]: %ls", __FUNCTION__, AgentIndex, *NavConfig.Name.ToString());
			}
		}
	}
}
...
ANavigationData* UNavigationSystemV1::CreateNavigationDataInstanceInLevel(const FNavDataConfig& NavConfig, ULevel* SpawnLevel)
{
	...
	{
		UE_LOGF(LogNavigation, Warning, "Unable to create NavigationData instance for config \'%ls\' as this agent is not supported by current NavigationSystem instance"
	...
	if (Instance != nullptr)
	{
		...
		if (NavConfig.Name != NAME_None)
		{
			...
			while (ExistingObject != nullptr)
			{
				...
				ExistingObject->Rename(nullptr, nullptr, REN_DontCreateRedirectors | REN_ForceGlobalUnique | REN_DoNotDirty | REN_NonTransactional | REN_AllowPackageLinkerMismatch);
			...
			Instance->Rename(*StrName, nullptr, REN_DoNotDirty | REN_AllowPackageLinkerMismatch);
#if WITH_EDITOR
...
void UNavigationSystemV1::AddNavigationBuildLock(uint8 Flags)
{
	...
	UE_LOGF(LogNavigation, Verbose, "UNavigationSystemV1::AddNavigationBuildLock WasLocked=%ls IsLocked=%ls (flags: 0x%x)",
		*LexToString(bWasLocked), *LexToString(bIsLocked), NavBuildingLockFlags);
...
void UNavigationSystemV1::RemoveNavigationBuildLock(uint8 Flags, const ELockRemovalRebuildAction RebuildAction /*= ELockRemovalRebuildAction::Rebuild*/)
{
	...
	const bool bIsLocked = IsNavigationBuildingLocked();
	UE_LOGF(LogNavigation, Verbose, "UNavigationSystemV1::RemoveNavigationBuildLock WasLocked=%ls IsLocked=%ls (flags: 0x%x)",
		*LexToString(bWasLocked), *LexToString(bIsLocked), NavBuildingLockFlags);
	if (bWasLocked && !bIsLocked)
	{
...
{
	UE_LOGF(LogNavigation, Verbose, "UNavigationSystemV1::SetNavigationOctreeLock IsLocked=%ls", *LexToString(bLock));
...
{
	UE_LOGF(LogNavigation, Verbose, "UNavigationSystemV1::RebuildAll");
	...
	for (int32 NavDataIndex = 0; NavDataIndex < NavDataSet.Num(); ++NavDataIndex)
	{
		...
		if (NavData)
		{
			const bool bShouldRebuildAll = (!bIsLoadTime || NavData->NeedsRebuildOnLoad()) && (!bIsInGame || NavData->SupportsRuntimeGeneration()) && (BuildBounds.IsValid == 0);
			if (bShouldRebuildAll)
			{
				UE_LOGF(LogNavigationDataBuild, Display, "   RebuildAll building NavData:  %ls.", *NavData->GetConfig().GetDescription());
				UE_LOGF(LogNavigationDataBuild, Verbose, "   RebuildAll bIsLoadTime=%ls, NavData->NeedsRebuildOnLoad()=%ls, bIsInGame=%ls, NavData->SupportsRuntimeGeneration()=%ls, BuildBounds.IsValid=%ls",
			...
			{
				UE_LOGF(LogNavigationDataBuild, Verbose, "   RebuildAll skipped.  bIsLoadTime=%ls, NavData->NeedsRebuildOnLoad()=%ls, bIsInGame=%ls, NavData->SupportsRuntimeGeneration()=%ls, BuildBounds.IsValid=%ls",
					*LexToString(bIsLoadTime), *LexToString(NavData->NeedsRebuildOnLoad()), *LexToString(bIsInGame), *LexToString(NavData->SupportsRuntimeGeneration()), *LexToString(BuildBounds.IsValid));
			}
		}
	}
...
void UNavigationSystemV1::OnNavigationGenerationFinished(ANavigationData& NavData)
{
	...
	{
		if (DirtyNavDataPackageOnNavigationGenerationFinished())
		{
			NavData.MarkPackageDirty();
		}
		
		UE_LOGF(LogNavigationDataBuild, Verbose, "Navigation data generation finished for %ls (%ls).", *NavData.GetActorLabel(), *NavData.GetFullName());
	}
...
void UNavigationSystemV1::OnLevelAddedToWorld(ULevel* InLevel, UWorld* InWorld)
{
	...
	UE_LOGF(LogNavigation, Verbose, "%s Level: %ls, World: %ls", __FUNCTION__, *GetNameSafe(InLevel), *GetNameSafe(InWorld));
	...
	if (OperationMode == FNavigationSystemRunMode::InvalidMode)
	{
		...
		UE_LOGF(LogNavigation, Log, "%s won't process navigation data registration candidates until OperationMode is set. Waiting for OnWorldInitDone.", __FUNCTION__);
	}
	else if (NavDataRegistrationQueue.Num() > 0)
	{
		UE_LOGF(LogNavigation, Log, "%s Calling ProcessRegistrationCandidates (level: %ls, world: %ls).", __FUNCTION__, *GetFullNameSafe(InLevel), *GetFullNameSafe(InWorld));
		ProcessRegistrationCandidates();
	}
}
...
void UNavigationSystemV1::OnPostLoadMap(UWorld* LoadedWorld)
{
	...
	UE_LOGF(LogNavigation, Verbose, "%s (Package: %ls)", __FUNCTION__, *GetNameSafe(LoadedWorld->GetOuter()));
		 ...
		 (GetRuntimeGenerationType() != ERuntimeGenerationType::Static) )	// Prevent creating a static default nav instance out of the editor (GetRuntimeGenerationType() is always dynamic in editor).
	{
		...
		UE_LOGF(LogNavigation, Verbose, "%s Created DefaultNavDataInstance %ls", __FUNCTION__, *GetNameSafe(NavData));
	}
}

#if WITH_EDITOR
void UNavigationSystemV1::OnMapOpened(const FString& Filename, bool bAsTemplate)
{
	// Now that the maps as loaded, remove unnecessary packages (and actors for OFPA).
	for (TWeakObjectPtr<ANavigationData> NavigationDataPtr : NavigationDataToRemove)
	{
		if (NavigationDataPtr.IsValid())
		{
			bool bIsUsingExternalActor = false;
			if (const UWorld* World = GetWorld())
			{
				const ULevel* Level = World->PersistentLevel;
				bIsUsingExternalActor = Level && Level->IsUsingExternalActors();
			}

			UPackage* Package = nullptr;
			FString PackageName;
			if (bIsUsingExternalActor)
			{
				Package = NavigationDataPtr->GetPackage();
				if (Package)
				{
				    PackageName = SourceControlHelpers::PackageFilename(Package);
				}	
			}
			
			UE_LOGF(LogNavigation, Verbose, "%s Destroying %ls because it failed to register.", __FUNCTION__, *NavigationDataPtr->GetFullName());
			NavigationDataPtr->DestroyActor();

			if (bIsUsingExternalActor && Package)
			{
				bool bDeleted = false;
				...
				{
					// Delete with source control
					bDeleted = PackageHelper.Delete(Package);
				}
				else
				{
					// Direct file delete
					...
					{
						bDeleted = FPlatformFileManager::Get().GetPlatformFile().DeleteFile(*PackageName);
					}
				}

				if (bDeleted)
				{
					UE_LOGF(LogNavigation, Verbose, "%s File deleted (%ls).", __FUNCTION__, *PackageName);
				}
			}
		}
	}
	NavigationDataToRemove.Reset();
}
...
void UNavigationSystemV1::CleanUp(FNavigationSystem::ECleanupMode Mode)
{
	...
	UE_LOGF(LogNavigation, Log, "UNavigationSystemV1::CleanUp");
	...
	UE_LOGF(LogNavigation, Verbose, "%s Removing functions from OnAssetPostCompileEvent delegate.", __FUNCTION__);
	FAssetCompilingManager::Get().OnAssetPostCompileEvent().Remove(OnAssetPostCompileEventHandle);
	OnAssetPostCompileEventHandle.Reset();
	UnregisterDelayedUnlock();
#endif // WITH_EDITOR
	...
	if (NavDataSet.Num())
	{
		UE_LOGF(LogNavigation, Error, "UNavigationSystemV1::CleanUp still has data in NavDataSet after unregister them all");
		NavDataSet.Reset();
	}

	if (AgentToNavDataMap.Num())
	{
		UE_LOGF(LogNavigation, Error, "UNavigationSystemV1::CleanUp still has agents mapped to navigation data after clean up");
	...
	if (MyWorld)
	{
		...
		{
			UE_LOGF(LogNavLink, VeryVerbose, "Reset navlink id on cleanup.");
			PRAGMA_DISABLE_DEPRECATION_WARNINGS
...
bool UNavigationSystemV1::DoesAnyNavDataSupportRuntimeGeneration() const
{
	UWorld* World = GetWorld();
	check(World);
...
bool UNavigationSystemV1::DoesAnyNavDataRequireGeometryStorage() const
{
	UWorld* World = GetWorld();
	check(World);
	...
	for (ANavigationData* NavData : NavDataSet)
	{
		if (NavData && (NavData->GetRuntimeGenerationMode() == ERuntimeGenerationType::Dynamic || NavData->RequiresInitialRebuild()))
		{
			return true;
		}
	}
...
{
	LogNavDataRegistrationResult(InResult, nullptr);
}

void UNavigationSystemV1::LogNavDataRegistrationResult(ERegistrationResult InResult, const ANavigationData* NavigationData) const
{
	switch (InResult)
	{
	case UNavigationSystemV1::RegistrationError:
		UE_VLOG_UELOG(this, LogNavigation, Warning,
			TEXT("NavData RegistrationError, could not be registered (%s)."), *GetNameSafe(NavigationData));
		break;
	case UNavigationSystemV1::RegistrationFailed_DataPendingKill:
		UE_VLOG_UELOG(this, LogNavigation, Warning,
			TEXT("NavData RegistrationFailed_DataPendingKill (%s)."), *GetNameSafe(NavigationData));
		break;
	case UNavigationSystemV1::RegistrationFailed_AgentAlreadySupported:
		UE_VLOG_UELOG(this, LogNavigation, Warning,
			TEXT("NavData RegistrationFailed_AgentAlreadySupported, specified agent type already has its navmesh implemented (%s)."), *GetNameSafe(NavigationData));
		break;
	case UNavigationSystemV1::RegistrationFailed_AgentNotValid:
		UE_VLOG_UELOG(this, LogNavigation, Warning,
			TEXT("NavData RegistrationFailed_AgentNotValid, NavData instance contains navmesh that doesn't support any of expected agent types (%s)."), *GetNameSafe(NavigationData));
		break;
	case UNavigationSystemV1::RegistrationFailed_NotSuitable:
		UE_VLOG_UELOG(this, LogNavigation, Warning,
			TEXT("NavData RegistrationFailed_NotSuitable (%s)."), *GetNameSafe(NavigationData));
		break;
	case UNavigationSystemV1::RegistrationSuccessful:
		UE_VLOG_UELOG(this, LogNavigation, Verbose,
			TEXT("NavData RegistrationSuccessful (%s)."), *GetNameSafe(NavigationData));
		break;
	default:
		UE_VLOG_UELOG(this, LogNavigation, Warning,
			TEXT("Registration not successful default warning (%s)."), *GetNameSafe(NavigationData));
		break;
	}
}
...
bool UNavigationSystemV1::IsAllowedToRebuild() const
{
	...
	return World && (!World->IsGameWorld() || DoesAnyNavDataSupportRuntimeGeneration());
}
...
{
	UE_LOGF(LogNavigation, Warning, "Logging NavigationSystem memory usage:");

	if (DefaultOctreeController.NavOctree.IsValid())
	{
		UE_LOGF(LogNavigation, Warning, "NavOctree memory: %llu", DefaultOctreeController.NavOctree->GetSizeBytes());
	}
...
bool UNavigationSystemV1::DirtyNavDataPackageOnNavigationGenerationFinished() const
{
	return bDirtyNavDataPackageOnNavigationGenerationFinished;
}
...
namespace UE::Navigation::Private
{
	void LogNavInvokerRegistration(const UNavigationSystemV1& NavSystem, const FNavigationInvoker& Data)
	{
		UE_SUPPRESS(LogNavInvokers, Log,
		{
			...
			UE_LOGF(LogNavInvokers, Log, "%ls", *RegisterText);
...
#if WITH_EDITOR
void UNavigationSystemV1::UpdateDelayedUnlockeRegistration(const TArray<FAssetCompileData>& AssetCompileData)
{
	UE_LOGF(LogNavigation, Verbose, "%s Assests remaining %i.", __FUNCTION__, FAssetCompilingManager::Get().GetNumRemainingAssets());
	
	// Register a delayed unlock navigation when all assets are finished compiling.
	if (FAssetCompilingManager::Get().GetNumRemainingAssets() == 0)
	{
		// Wait until we get some idle time to avoid unlocking too soon
		constexpr float TimeDelay = 2.f;
		RegisterDelayedUnlock(TimeDelay);
	}
	else
	{
		// We're not idle anymore, unregister unlocking until we get to 0 assets again
		UnregisterDelayedUnlock();
	}
}

void UNavigationSystemV1::RegisterDelayedUnlock(float DelayInSeconds)
{
	UE_LOGF(LogNavigation, VeryVerbose, "%hs", __FUNCTION__);
	
	// Unregister the previous one and register a new one.
	UnregisterDelayedUnlock();
	...
	DelayedUnlockHandle = FTSTicker::GetCoreTicker().AddTicker(
		TEXT("UNavigationSystemV1 async loading wait for unlock"),
		0.f,
		[this, FireInTickCount = 16, DelayInSeconds](float DeltaTime) mutable
		{
			UE_LOGF(LogNavigation, VeryVerbose, "%s    Time remaining %f.", __FUNCTION__, DelayInSeconds);
				
			DelayInSeconds -= DeltaTime;
			if (--FireInTickCount <= 0 && DelayInSeconds <= 0.f && FAssetCompilingManager::Get().GetNumRemainingAssets() == 0)
			{
				UE_LOGF(LogNavigation, Verbose, "%s    Removing functions from OnAssetPostCompileEvent delegate.", __FUNCTION__);
				FAssetCompilingManager::Get().OnAssetPostCompileEvent().RemoveAll(this);
				
				// Cleared initial async load dirty areas and remove AsyncLoadLock
				UE_LOGF(LogNavigation, Verbose, "%s    Done with assets. Removing ENavigationBuildLock::AsyncLoadLock.", __FUNCTION__);
				DefaultDirtyAreasController.Reset();
				RemoveNavigationBuildLock(ENavigationBuildLock::AsyncLoadLock, ELockRemovalRebuildAction::NoRebuild);
...
void UNavigationSystemV1::UnregisterDelayedUnlock()
{
	if (DelayedUnlockHandle.IsValid())
	{
		UE_LOGF(LogNavigation, Verbose, "%s", __FUNCTION__);
		
		FTSTicker::RemoveTicker(DelayedUnlockHandle);
		DelayedUnlockHandle.Reset();
	}
}
#endif // WITH_EDITOR
	...
	if (CurrentTime >= NextInvokersUpdateTime)
	{
		...
		if (Invokers.Num() > 0)
		{
			...
			for (auto ItemIterator = Invokers.CreateIterator(); ItemIterator; ++ItemIterator)
			{
				...
				{
					UE_LOGF(LogNavInvokers, Verbose, "Invoker %ls ignored because it's too far from any seed location. Closest seed at %.0f.",
			...
			for (const auto& InvokerData : InvokerLocations)
			{
				...
				UE_VLOG_CYLINDER(this, LogNavInvokers, Log, InvokerData.Location, InvokerData.Location + FVector(0, 0, 20), InvokerData.RadiusMin, FColorList::CadetBlue, TEXT("Priority %u"), uint32{EnumToUnderlyingType(InvokerData.Priority)});
			}
#endif // ENABLE_VISUAL_LOG
		}
...
void UNavigationSystemV1::VerifyNavigationRenderingComponents(const bool bShow)
{
	...
	if (NavData == nullptr)
	{
		UE_LOGF(LogNavigation, Warning, "No NavData found when calling UNavigationSystemV1::VerifyNavigationRenderingComponents()");
	}
}
...
{
	UE_CLOGF(bWorldInitDone, LogNavigation, Warning, "Trying to override NavigationSystem\'s SupportedAgents past the World\'s initialization");
...
void UNavigationSystemModuleConfig::UpdateWithNavSysCDO(const UNavigationSystemV1& NavSysCDO)
{
	...
	if (MyClass != nullptr && MyClass->IsChildOf(NavSysCDO.GetClass()))
	{
		...
		bDirtyNavDataPackageOnNavigationGenerationFinished = NavSysCDO.bDirtyNavDataPackageOnNavigationGenerationFinished;
	}
}
...
UNavigationSystemBase* UNavigationSystemModuleConfig::CreateAndConfigureNavigationSystem(UWorld& World) const
{
	...
	UE_CLOGF(NavSysInstance == nullptr && NewNavSys != nullptr, LogNavigation, Error
		, "Unable to spawn navigation system instance of class %ls - unable to cast to UNavigationSystemV1"
	...
	if (NavSysInstance)
	{
		...
		NavSysInstance->bDirtyNavDataPackageOnNavigationGenerationFinished = bDirtyNavDataPackageOnNavigationGenerationFinished;
