...
void FDetourTileLayout::InitFromSizeInfo(const FDetourTileSizeInfo& SizeInfo)
{
	...
	VertsSize = dtAlign(sizeof(dtTileVert) * SizeInfo.VertCount);
	...
	DetailVertsSize = dtAlign(sizeof(dtTileVert) * SizeInfo.DetailVertCount);
...
static FAutoConsoleCommandWithWorldArgsAndOutputDevice CmdDumpNavMeshMemory(
	TEXT("ai.debug.nav.DumpNavMeshMemory"),
	TEXT("Dumps detailed tile memory usage statistics for all active navmeshes."),
	FConsoleCommandWithWorldArgsAndOutputDeviceDelegate::CreateLambda(
		[](const TArray<FString>& Args, const UWorld* World, FOutputDevice& OutputDevice)
		{
			if (const UNavigationSystemV1* NavSys = FNavigationSystem::GetCurrent<UNavigationSystemV1>(World))
			{
				for (const ANavigationData* NavData : NavSys->NavDataSet)
				{
					...
					{
						RecastNavMesh->DumpDetailedMemoryStats();
					}
				}
			}
			else
			{
				OutputDevice.Log(ELogVerbosity::Warning, TEXT("Command failed: unable to find the navigation system."));
			}
		}
	));
#endif // WITH_RECAST && !UE_BUILD_SHIPPING
...
ARecastNavMesh::ARecastNavMesh(const FObjectInitializer& ObjectInitializer)
	...
	, bDrawOnScreenInformation(true)
	, bDistinctlyDrawTilesBeingBuilt(true)
	, DrawOffset(10.f)
	, TilePoolSize(1024)
	, AverageLayersPerTile(3)
...
{
	UE_LOGF(LogNavigation, Verbose, "%s %ls", __FUNCTION__, *GetFullNameSafe(this));
...
void ARecastNavMesh::CheckToDiscardSubLevelNavData(const UNavigationSystemBase& BaseNavSys)
{
	...
	if (OwningWorld && NavSys->ShouldDiscardSubLevelNavData(this))
	{
		...
		{
			UE_LOGF(LogNavigation, Verbose, "%ls Discarding %ls due to it not being part of PersistentLevel.", ANSI_TO_TCHAR(__FUNCTION__), *GetFullNameSafe(this));
...
void ARecastNavMesh::RegisterGeneratedLinksProxy(FNavLinkGenerationJumpConfig& InOutNavLinkJumpConfig)
{
	if (!InOutNavLinkJumpConfig.bLinkProxyRegistered && InOutNavLinkJumpConfig.LinkProxy)
	{
		if (UWorld* World = GetWorld())
		{
			...
			if (NavSys)
			{
				UE_LOGF(LogNavLink, Log, "RegisterLinkProxy id: %llu ptr: 0x%p", InOutNavLinkJumpConfig.LinkProxyId.GetId(), InOutNavLinkJumpConfig.LinkProxy.Get());

				NavSys->RegisterCustomLink(*InOutNavLinkJumpConfig.LinkProxy);
				InOutNavLinkJumpConfig.bLinkProxyRegistered = true;
			}
		}
	}
}

void ARecastNavMesh::UnregisterGeneratedLinksProxy(FNavLinkGenerationJumpConfig& InOutNavLinkJumpConfig)
{
	if (InOutNavLinkJumpConfig.bLinkProxyRegistered && InOutNavLinkJumpConfig.LinkProxy)
	{
		if (UWorld* World = GetWorld())
		{
			...
			if (NavSys)
			{
				UE_LOGF(LogNavLink, Log, "UnregisterLinkProxy id: %llu ptr: 0x%p", InOutNavLinkJumpConfig.LinkProxyId.GetId(), InOutNavLinkJumpConfig.LinkProxy.Get());

				NavSys->UnregisterCustomLink(*InOutNavLinkJumpConfig.LinkProxy);
				InOutNavLinkJumpConfig.bLinkProxyRegistered = false;
			}
		}
	}
}
...
void ARecastNavMesh::RegisterGeneratedLinksProxy()
{
	...
	if (!NavLinkJumpDownConfig.bLinkProxyRegistered && NavLinkJumpDownConfig.LinkProxy)
	{
		if (UWorld* World = GetWorld())
		{
			...
			if (NavSys)
			{
				UE_LOGF(LogNavLink, Log, "RegisterLinkProxy id: %llu ptr: 0x%p", NavLinkJumpDownConfig.LinkProxyId.GetId(), NavLinkJumpDownConfig.LinkProxy.Get());
...
void ARecastNavMesh::UnregisterGeneratedLinksProxy()
{
	...
	if (NavLinkJumpDownConfig.bLinkProxyRegistered && NavLinkJumpDownConfig.LinkProxy)
	{
		if (UWorld* World = GetWorld())
		{
			...
			if (NavSys)
			{
				UE_LOGF(LogNavLink, Log, "UnregisterLinkProxy id: %llu ptr: 0x%p", NavLinkJumpDownConfig.LinkProxyId.GetId(), NavLinkJumpDownConfig.LinkProxy.Get());
	...
	const FNavLinkId LinkProxyId /*= FNavLinkId::GenerateUniqueId()*/)
{
	...
	if (LinkProxy)
	{
		...
		RegisterGeneratedLinksProxy(InOutNavLinkJumpConfig);
	}
}
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	for (FNavLinkGenerationJumpConfig& NavLinkJumpConfig : NavLinkJumpConfigs)
	{
		UnregisterGeneratedLinksProxy(NavLinkJumpConfig);
...
void ARecastNavMesh::PostInitProperties()
{
	...
	if (IsVoxelCacheEnabled() && !HasAnyFlags(RF_ClassDefaultObject))
	{
		...
		{
			UE_LOGF(LogNavigation, Warning, "%ls param: TileSizeUU(%f) differs from config settings, forcing value %f so it can be used with voxel cache!",
		...
		for (int i = 0; i < (uint8)ENavigationDataResolution::MAX; ++i)
		{
			...
			{
				UE_LOGF(LogNavigation, Warning, "%ls param: CellSize(%f) differs from config settings, forcing value %f so it can be used with voxel cache!",
			...
			{
				UE_LOGF(LogNavigation, Warning, "%ls param: CellHeight(%f) differs from config settings, forcing value %f so it can be used with voxel cache!",
			...
			{
				UE_LOGF(LogNavigation, Warning, "%ls param: AgentMaxStepHeight(%f) differs from config settings, forcing value %f so it can be used with voxel cache!",
		...
		{
			UE_LOGF(LogNavigation, Warning, "%ls param: AgentMaxSlope(%f) differs from config settings, forcing value %f so it can be used with voxel cache!",
...
{
	UE_LOGF(LogNavigation, VeryVerbose, "%ls> %ls Tile (%d,%d:%d), %ls TileId: %d, Salt: %d, TileRef: 0x%llx (%ls)",
...
void ARecastNavMesh::OnRegistered()
{
	...
	// Off-mesh link deferred connection should have been resolved by OnAreaCostChanged
	// during ProcessNavAreas, which runs before OnRegistered.
	ensure(RecastNavMeshImpl->GetRecastMesh() == nullptr || !RecastNavMeshImpl->GetRecastMesh()->isDeferringOffMeshLinking());

	Super::OnRegistered();

	// Re-register any generated link proxies that couldn't be registered
	// when created (NavSys was null), or that need re-registration after
	// NavSys was destroyed and recreated (flags reset in OnUnregistered).
	for (FNavLinkGenerationJumpConfig& NavLinkJumpConfig : NavLinkJumpConfigs)
	{
		RegisterGeneratedLinksProxy(NavLinkJumpConfig);
	}
}

void ARecastNavMesh::OnUnregistered()
{
	Super::OnUnregistered();

	// Reset link proxy registration flags so they can be re-registered
	// with a new NavSys instance in OnRegistered. The proxies themselves
	// remain valid — only the registration state is cleared, since the
	// NavSys they were registered with is being torn down.
	for (FNavLinkGenerationJumpConfig& NavLinkJumpConfig : NavLinkJumpConfigs)
	{
...
void ARecastNavMesh::Serialize( FArchive& Ar )
{
	UE_LOGF(LogNavigation, Verbose, "%s %ls %ls, '%ls'", __FUNCTION__,
		Ar.IsLoading() ? TEXT("'loading'") :
			(Ar.IsSaving() ? TEXT("'saving'") :
				(Ar.IsTransacting() ? TEXT("'transacting'") : TEXT("'unknown'"))),
		*Ar.GetArchiveName(),
	...
	if (Ar.IsLoading())
	{
		...
		{
			UE_LOGF(LogNavigation, Display, "%ls: ARecastNavMesh: Navmesh version %d is compatible but not at latest (%d). Rebuild navmesh to use the latest features. \n", *GetFullName(), NavMeshVersion, NAVMESHVER_LATEST);
		}

		if (NavMeshVersion < NAVMESHVER_MIN_COMPATIBLE)
		{
			UE_LOGF(LogNavigation, Warning, "%ls: ARecastNavMesh: Navmesh version %d < Min compatible %d. Navmesh needs to be rebuilt. \n", *GetFullName(), NavMeshVersion, NAVMESHVER_MIN_COMPATIBLE);
			CleanUpBadVersion();
		}
		else if (NavMeshVersion > NAVMESHVER_LATEST)
		{
			UE_LOGF(LogNavigation, Warning, "%ls: ARecastNavMesh: Navmesh version %d > NAVMESHVER_LATEST %d. Newer navmesh should not be loaded by older code. At a minimum the nav mesh needs to be rebuilt. \n", *GetFullName(), NavMeshVersion, NAVMESHVER_LATEST);
			CleanUpBadVersion();
		}
	...
	if (HasValidNavmesh() && TileRef.IsValid())
	{
		...
		if (MaxPolys > 0)
		{
			...
			for (int32 i = 0; i < MaxPolys; i++, Poly++)
			{
				...
				{
					FVector::FReal Vertex[3];
					Tile->verts[Poly->verts[k]].toWorld(Tile->header->bmin, Vertex);
					PolyCenter += Recast2UnrealPoint(Vertex);
				}
	...
	if (Tile && Tile->header)
	{
		...
		if (LinkCount > 0)
		{
			...
			for (int32 LinkIndex = 0; LinkIndex < LinkCount; ++LinkIndex, ++Poly)
			{
				...
				FVector::FReal VertexA[3];
				FVector::FReal VertexB[3];
				Tile->verts[Poly->verts[0]].toWorld(Tile->header->bmin, VertexA);
				Tile->verts[Poly->verts[1]].toWorld(Tile->header->bmin, VertexB);
				OutPoly.Center = (Recast2UnrealPoint(VertexA) + Recast2UnrealPoint(VertexB)) / 2;
			}
		}
...
int32 ARecastNavMesh::GetNumActiveTiles() const
{
	const FRecastNavMeshGenerator* Generator = static_cast<const FRecastNavMeshGenerator*>(GetGenerator());
	return Generator ? Generator->GetNumActiveTiles() : 0;
}
...
void ARecastNavMesh::BatchFindMoveAlongSurface(TArray<FNavigationFindMoveAlongSurfaceWork>& Workload, FSharedConstNavQueryFilter Filter, const UObject* Querier) const
{
	if (RecastNavMeshImpl)
	{
		...
		{
			Work.bResult = RecastNavMeshImpl->FindMoveAlongSurface(Work.StartLocation, Work.DesiredEndLocation, Work.ResultNavLocation, FilterToUse, Querier);
		}
	}
}
...
void ARecastNavMesh::CreateSegmentLinkConnections(TSet<FNavTileRef>& TilesPendingSegmentLinks)
{
	...
	for (FNavTileRef TileRef : TilesPendingSegmentLinksArray)
	{
		...
		UE_LOGF(LogSegmentLink, Log, "1) %s and store skipped tiles", __FUNCTION__);
		...
		{
			UE_LOGF(LogNavigation, Warning, "The number of neighbor tiles skipped (%d) while connecting segment links exceeds the max allowed (%d). Consider increasing ExpectedMaxLayersPerTile in the NavMesh settings.", NumSkippedNeighborTiles, MaxSkippedNeighborTiles);
		...
		for (uint32 Index = 0; Index < NumSkippedNeighborTiles; ++Index)
		{
			...
			if (!ProcessedTiles.Contains(NeighborTileRef))
			{
				...
				UE_LOGF(LogSegmentLink, Log, "2) %s process skipped tileRef %llu", __FUNCTION__, uint64(NeighborTileRef));
	...
	// the system safer in case of future timing changes.
	{
		...
		for (int32 PathIndex = PathsCount - 1; PathIndex >= 0; --PathIndex)
		{
			...
			else
			{
				...
				const int32 PathLength = Path->PathCorridor.Num();
				const NavNodeRef* PathPoly = Path->PathCorridor.GetData();
				for (int32 NodeIndex = 0; NodeIndex < PathLength; ++NodeIndex, ++PathPoly)
				{
...
{
	// If this navmesh was spawned at runtime to fill a missing nav data gap,
	// the one-time rebuild is now complete. Clear the flag so it behaves
	// as a Static navmesh going forward.
	if (RequiresInitialRebuild())
	{
		ClearRequiresInitialRebuild();
	}
...
uint32 ARecastNavMesh::LogMemUsed() const
{
	...
	UE_LOGF(LogNavigation, Warning, "%ls: ARecastNavMesh: %u\n    self: %zu", *GetName(), MemUsed, sizeof(ARecastNavMesh));

	return MemUsed + SuperMemUsed;
}

void ARecastNavMesh::DumpDetailedMemoryStats() const
{
#if WITH_RECAST
	if (!RecastNavMeshImpl || !RecastNavMeshImpl->DetourNavMesh)
	{
		UE_LOGF(LogNavigation, Display, "%ls: No Detour navmesh data.", *GetName());
		return;
	}
	...
	// Aggregate counters
	int32 TotalActiveTiles = 0;
	uint64 TotalTileDataBytes = 0;

	// Per-section aggregates
	uint64 TotalHeaderBytes = 0;
	uint64 TotalVertBytes = 0;
	uint64 TotalPolyBytes = 0;
	uint64 TotalLinkBytes = 0;
	uint64 TotalDetailMeshBytes = 0;
	uint64 TotalDetailVertBytes = 0;
	uint64 TotalDetailTriBytes = 0;
	uint64 TotalBvTreeBytes = 0;
	uint64 TotalOffMeshConBytes = 0;
	uint64 TotalOffMeshSegBytes = 0;
	uint64 TotalClusterBytes = 0;
	uint64 TotalPolyClusterBytes = 0;

	// Count trackers
	int32 TilesWithBvTree = 0;
	int32 TilesWithoutBvTree = 0;
	int32 TotalPolyCount = 0;
	int32 TotalVertCount = 0;
	int32 TotalLinkCount = 0;
	int32 TotalBvNodeCount = 0;
	int32 TotalDetailVertCount = 0;
	int32 TotalDetailTriCount = 0;
	int32 TotalOffMeshConCount = 0;
	int32 MinPolysPerTile = MAX_int32;
	int32 MaxPolysPerTile = 0;

	// Link utilization tracking
	uint64 TotalBaseLinksAllocated = 0;
	uint64 TotalLinksUsed = 0;
	int32 MaxPreAllocatedLinksInAnyTile = 0;
	uint64 TotalLinksPerPolySum = 0;
	int32 TotalPolyCountForAvg = 0;

	// Link utilization distribution buckets (0-10%, 10-20%, ..., 90-100%)
	static constexpr int32 UtilBuckets = 10;
	int32 UtilBucketCount[UtilBuckets] = {0};

	// Dynamic allocation tracking
	int32 TilesWithDynamic = 0;
	uint64 TotalDynamicLinks = 0;
	int32 MaxDynamicInTile = 0;

	// Histogram: poly count per tile
	// Buckets: [1-2], [3-4], [5-8], [9-16], [17-32], [33-64], [65-128], [129-256], [257+]
	static constexpr int32 NumBuckets = 9;
	static constexpr int32 BucketMaxPolys[NumBuckets] = { 2, 4, 8, 16, 32, 64, 128, 256, MAX_int32 };
	static const char* BucketLabels[NumBuckets] = {
		"1-2", "3-4", "5-8", "9-16", "17-32", "33-64", "65-128", "129-256", "257+"
	};
	int32 BucketTileCount[NumBuckets] = {};
	uint64 BucketTotalBytes[NumBuckets] = {};
	uint64 BucketBvTreeBytes[NumBuckets] = {};
	uint64 BucketDetailBytes[NumBuckets] = {};
	...
	for (int32 TileIndex = 0; TileIndex < MaxTiles; ++TileIndex)
	{
		...
		TotalTileDataBytes += TileLayout.TileSize;
		TotalHeaderBytes += TileLayout.HeaderSize;
		TotalVertBytes += TileLayout.VertsSize;
		TotalPolyBytes += TileLayout.PolysSize;
		TotalLinkBytes += TileLayout.LinksSize;
		TotalDetailMeshBytes += TileLayout.DetailMeshesSize;
		TotalDetailVertBytes += TileLayout.DetailVertsSize;
		TotalDetailTriBytes += TileLayout.DetailTrisSize;
		TotalBvTreeBytes += TileLayout.BvTreeSize;
		TotalOffMeshConBytes += TileLayout.OffMeshConsSize;
		TotalOffMeshSegBytes += TileLayout.OffMeshSegsSize;
		TotalClusterBytes += TileLayout.ClustersSize;
		TotalPolyClusterBytes += TileLayout.PolyClustersSize;

		TotalPolyCount += Header->polyCount;
		TotalVertCount += Header->vertCount;
		TotalLinkCount += Header->maxLinkCount;
		TotalBvNodeCount += Header->bvNodeCount;
		TotalDetailVertCount += Header->detailVertCount;
		TotalDetailTriCount += Header->detailTriCount;
		TotalOffMeshConCount += Header->offMeshConCount;

		// Count actually used links by traversing polygon link chains
		int32 UsedBaseLinks = 0;
		int32 UsedDynamicLinks = 0;
		for (int32 PolyIdx = 0; PolyIdx < Header->polyCount; ++PolyIdx)
		{
			...
			{
				if (LinkIdx < (unsigned int)Header->maxLinkCount)
				{
					UsedBaseLinks++;
				}
				else
				...
				const dtLink& Link = NavMesh->getLink(Tile, LinkIdx);
				LinkIdx = Link.next;
			}
		}

		const int32 UsedLinksInTile = UsedBaseLinks + UsedDynamicLinks;
		TotalBaseLinksAllocated += Header->maxLinkCount;
		TotalLinksUsed += UsedLinksInTile;
		MaxPreAllocatedLinksInAnyTile = FMath::Max(MaxPreAllocatedLinksInAnyTile, (int32)Header->maxLinkCount);

		// Track dynamic allocations
		TotalDynamicLinks += UsedDynamicLinks;
		if (UsedDynamicLinks > 0)
		{
			TilesWithDynamic++;
			MaxDynamicInTile = FMath::Max(MaxDynamicInTile, UsedDynamicLinks);
		}

		// Links per polygon stats
		TotalLinksPerPolySum += UsedLinksInTile;
		TotalPolyCountForAvg += Header->polyCount;

		// Utilization bucket (base pool only)
		const float Utilization = Header->maxLinkCount > 0
			? (float)UsedBaseLinks / Header->maxLinkCount : 0.0f;
		const int32 BucketIdx = FMath::Clamp((int32)(Utilization * 10.0f), 0, UtilBuckets - 1);
		UtilBucketCount[BucketIdx]++;
		...
		MinPolysPerTile = FMath::Min(MinPolysPerTile, (int32)Header->polyCount);
		MaxPolysPerTile = FMath::Max(MaxPolysPerTile, (int32)Header->polyCount);

		// Bucket by poly count
		for (int32 Bucket = 0; Bucket < NumBuckets; ++Bucket)
		{
			if ((int32)Header->polyCount <= BucketMaxPolys[Bucket])
			{
				BucketTileCount[Bucket]++;
				BucketTotalBytes[Bucket] += TileLayout.TileSize;
				BucketBvTreeBytes[Bucket] += TileLayout.BvTreeSize;
				BucketDetailBytes[Bucket] += TileLayout.DetailMeshesSize
					+ TileLayout.DetailVertsSize + TileLayout.DetailTrisSize;
				break;
			}
		}
	}
	...
	{
		UE_LOGF(LogNavigation, Display, "%ls: No active tiles.", *GetName());
		return;
	}

	// Tile pool overhead
	const uint64 TilePoolBytes = (uint64)MaxTiles * sizeof(dtMeshTile);
	const uint64 LookupTableBytes =
		(uint64)FMath::Max(1u, dtNextPow2(static_cast<unsigned int>(MaxTiles) / 4)) * sizeof(dtMeshTile*);

	// Log everything
	constexpr float ToKB = 1.0f / 1024.0f;
	constexpr float ToMB = 1.0f / (1024.0f * 1024.0f);

	UE_LOGF(LogNavigation, Display, "=======================================================================================================");
	UE_LOGF(LogNavigation, Display, "  NavMesh Memory Report: %ls", *GetName());
	UE_LOGF(LogNavigation, Display, "=======================================================================================================");
	UE_LOGF(LogNavigation, Display, "  Max tiles (pool):    %d", MaxTiles);
	UE_LOGF(LogNavigation, Display, "  Active tiles:        %d", TotalActiveTiles);
	UE_LOGF(LogNavigation, Display, "  Pool utilization:    %.1f%%",
		100.0f * (float)TotalActiveTiles / FMath::Max(MaxTiles, 1.f));
	UE_LOGF(LogNavigation, Display, "");

	// Overall memory
	UE_LOGF(LogNavigation, Display, "--- Overall Memory ---");
	UE_LOGF(LogNavigation, Display, "  Tile data total:     %10llu bytes  (%.2f MB)", TotalTileDataBytes, (float)TotalTileDataBytes * ToMB);
	UE_LOGF(LogNavigation, Display, "  Tile pool structs:   %10llu bytes  (%.2f MB)", TilePoolBytes, (float)TilePoolBytes * ToMB);
	UE_LOGF(LogNavigation, Display, "  Lookup table:        %10llu bytes  (%.2f MB)", LookupTableBytes, (float)LookupTableBytes * ToMB);
	UE_LOGF(LogNavigation, Display, "  TOTAL:               %10llu bytes  (%.2f MB)",
		(TotalTileDataBytes + TilePoolBytes + LookupTableBytes),
		(float)(TotalTileDataBytes + TilePoolBytes + LookupTableBytes) * ToMB);
	UE_LOGF(LogNavigation, Display, "");

	// Per-section breakdown
	UE_LOGF(LogNavigation, Display, "--- Per-Section Breakdown (tile data only) ---");
	UE_LOGF(LogNavigation, Display, "  %-20s %9s %7s %6s    %s",
		"Section", "Bytes", "MB", "%", "Description");

	auto LogSection = [&](const char* Name, uint64 Bytes, const char* Desc)
	{
		const float Pct = TotalTileDataBytes > 0
			? 100.0f * (float)Bytes / (float)TotalTileDataBytes : 0.0f;

		UE_LOGF(LogNavigation, Display, "  %-20s %9llu %7.2f %6.1f%%   %s",
			Name, Bytes, (float)Bytes * ToMB, Pct, Desc);
	};

	LogSection("Header",        TotalHeaderBytes,      "");
	LogSection("Vertices",      TotalVertBytes,
		TCHAR_TO_ANSI(*FString::Printf(TEXT("%d verts, %d bytes/vert"), TotalVertCount, TotalVertCount > 0 ? (int32)(TotalVertBytes / TotalVertCount) : 0)));
	LogSection("Polygons",      TotalPolyBytes, TCHAR_TO_ANSI(*FString::Printf(TEXT("%d polys"), TotalPolyCount)));
	LogSection("Links",         TotalLinkBytes, TCHAR_TO_ANSI(*FString::Printf(TEXT("%d max links allocated"), TotalLinkCount)));
	LogSection("Detail Meshes", TotalDetailMeshBytes, "");
	LogSection("Detail Verts",  TotalDetailVertBytes, TCHAR_TO_ANSI(*FString::Printf(TEXT("%d detail verts"), TotalDetailVertCount)));
	LogSection("Detail Tris",   TotalDetailTriBytes, TCHAR_TO_ANSI(*FString::Printf(TEXT("%d detail tris"), TotalDetailTriCount)));
	LogSection("BV Tree",       TotalBvTreeBytes, TCHAR_TO_ANSI(*FString::Printf(TEXT("%d nodes, %d/%d tiles have BV tree"), TotalBvNodeCount, TilesWithBvTree, TotalActiveTiles)));
	LogSection("OffMesh Cons",  TotalOffMeshConBytes, TCHAR_TO_ANSI(*FString::Printf(TEXT("%d connections"), TotalOffMeshConCount)));
	LogSection("OffMesh Segs",  TotalOffMeshSegBytes, "");
	LogSection("Clusters",      TotalClusterBytes, "");
	LogSection("PolyClusters",  TotalPolyClusterBytes, "");
	UE_LOGF(LogNavigation, Display, "");

	// Per-tile statstistics
	UE_LOGF(LogNavigation, Display, "--- Per-Tile Statistics ---");
	const uint64 AvgTileBytes = TotalTileDataBytes / TotalActiveTiles;
	UE_LOGF(LogNavigation, Display, "  Avg tile data size:  %llu bytes (%.4f MB)", AvgTileBytes, (float)AvgTileBytes * ToMB);
	UE_LOGF(LogNavigation, Display, "  Min polys/tile:      %d", MinPolysPerTile);
	UE_LOGF(LogNavigation, Display, "  Max polys/tile:      %d", MaxPolysPerTile);
	UE_LOGF(LogNavigation, Display, "  Avg polys/tile:      %.1f", (float)TotalPolyCount / (float)TotalActiveTiles);
	UE_LOGF(LogNavigation, Display, "");

	// Poly count histogram
	UE_LOGF(LogNavigation, Display, "--- Tile Distribution by Poly Count ---");
	UE_LOGF(LogNavigation, Display, "  %-16s %6s %6s     %9s  %10s  %10s  %10s   %10s  %10s",
		"Polys", "Tiles", "%",
		"Total KB", "Total MB", "BvTree KB", "BvTree MB", "Detail KB", "Detail MB");

	for (int32 Bucket = 0; Bucket < NumBuckets; ++Bucket)
	{
		if (BucketTileCount[Bucket] == 0)
		{
			continue;
		}
		
		const float TilePercentage = 100.0f * (float)BucketTileCount[Bucket] / (float)TotalActiveTiles;
		UE_LOGF(LogNavigation, Display,
			"  %-16hs %6d %6.1f%%     %9.1f  %10.2f  %10.1f  %10.2f   %10.1f  %10.2f",
			BucketLabels[Bucket],
			BucketTileCount[Bucket],
			TilePercentage,
			(float)BucketTotalBytes[Bucket] * ToKB,
			(float)BucketTotalBytes[Bucket] * ToMB,
			(float)BucketBvTreeBytes[Bucket] * ToKB,
			(float)BucketBvTreeBytes[Bucket] * ToMB,
			(float)BucketDetailBytes[Bucket] * ToKB,
			(float)BucketDetailBytes[Bucket] * ToMB);
	}
	UE_LOGF(LogNavigation, Display, "");

	// Link utilization analysis
	const uint64 TotalAllocated = TotalBaseLinksAllocated + TotalDynamicLinks;
	UE_LOGF(LogNavigation, Display, "--- Link Utilization Analysis ---");
	UE_LOGF(LogNavigation, Display, "  Base pool allocated:   %llu links", TotalBaseLinksAllocated);
	UE_LOGF(LogNavigation, Display, "  Dynamic used:        %llu links", TotalDynamicLinks);
	UE_LOGF(LogNavigation, Display, "  Total allocated:       %llu links", TotalAllocated);
	UE_LOGF(LogNavigation, Display, "  Links actually used:   %llu (%.1f%%)", TotalLinksUsed, 100.0f * (float)TotalLinksUsed / (float)FMath::Max((uint64)1, TotalAllocated));

	const uint64 WastedLinks = TotalAllocated > TotalLinksUsed
		? TotalAllocated - TotalLinksUsed : 0;

	UE_LOGF(LogNavigation, Display, "  Wasted links:          %llu (%.2f MB)",
		WastedLinks,
		(float)WastedLinks * (float)sizeof(dtLink) * ToMB);
	UE_LOGF(LogNavigation, Display, "  Max preallocated links in any tile: %d", MaxPreAllocatedLinksInAnyTile);

	if (TotalPolyCountForAvg > 0)
	{
		UE_LOGF(LogNavigation, Display, "  Links per poly:        avg=%.2f",
			(float)TotalLinksPerPolySum / (float)TotalPolyCountForAvg);
	}

	UE_LOGF(LogNavigation, Display, "");
	UE_LOGF(LogNavigation, Display, "--- Tile Utilization Distribution ---");
	UE_LOGF(LogNavigation, Display, "  Utilization    Tiles     %%");
	for (int32 i = 0; i < UtilBuckets; ++i)
	{
		if (UtilBucketCount[i] > 0)
		{
			UE_LOGF(LogNavigation, Display, "  %3d%%-%3d%%     %6d   %5.1f%%",
				i * 10, (i + 1) * 10,
				UtilBucketCount[i],
				100.0f * (float)UtilBucketCount[i] / (float)TotalActiveTiles);
		}
	}
	UE_LOGF(LogNavigation, Display, "");

	// Dynamic allocation statistics
	UE_LOGF(LogNavigation, Display, "--- Dynamic Allocation Statistics ---");
	UE_LOGF(LogNavigation, Display, "  Tiles using dynamic: %d (%.1f%%)",
		TilesWithDynamic, 100.0f * (float)TilesWithDynamic / (float)FMath::Max(TotalActiveTiles, 1));
	UE_LOGF(LogNavigation, Display, "  Total dynamic links: %llu (%.1f%% of used links)",
		TotalDynamicLinks,
		TotalLinksUsed > 0 ? 100.0f * (float)TotalDynamicLinks / (float)TotalLinksUsed : 0.0f);
	UE_LOGF(LogNavigation, Display, "  Max dynamic in tile: %d links", MaxDynamicInTile);
	UE_LOGF(LogNavigation, Display, "  Dynamic memory:      %.2f MB",
		(float)TotalDynamicLinks * (float)sizeof(dtLink) * ToMB);

	UE_LOGF(LogNavigation, Display, "=======================================================================================================");
	UE_LOGF(LogNavigation, Display, "");

#else  // WITH_RECAST
	UE_LOGF(LogNavigation, Display, "%ls: Recast not available.", *GetName());
#endif // WITH_RECAST
}
#endif // WITH_RECAST && !UE_BUILD_SHIPPING
...
void ARecastNavMesh::FillNavigationDataChunkActor(const FBox& QueryBounds, ANavigationDataChunkActor& DataChunkActor, FBox& OutTilesBounds) const
{
	...
	UE_LOGF(LogNavigation, Verbose, "%ls Bounds pos: (%ls)  size: (%ls).", ANSI_TO_TCHAR(__FUNCTION__), *QueryBounds.GetCenter().ToString(), *QueryBounds.GetSize().ToString());
...
#else
				UE_LOGF(LogNavigation, Error, "Navmesh requires generation of clusters for hierarchical path. Set WITH_NAVMESH_CLUSTER_LINKS to 1 to generate them.");
				bPathExists = false;
#endif // WITH_NAVMESH_CLUSTER_LINKS
			}
			else
			{
				UE_LOGF(LogNavigation, Log, "Hierarchical path finding test failed: filter doesn't match!");
				bUseFallbackSearch = true;
			}
...
bool ARecastNavMesh::CanUpdateNavMeshInEditor() const
{
...
{
	static const FName NAME_Generation = FName("Generation");
	static const FName NAME_NavMeshResolutionParams = FName("NavMeshResolutionParams");
...
{
	static const FName NAME_Generation = FName("Generation");
	static const FName NAME_Display = FName("Display");
	static const FName NAME_RuntimeGeneration = FName("RuntimeGeneration");
	static const FName NAME_TileNumberHardLimit = GET_MEMBER_NAME_CHECKED(ARecastNavMesh, TileNumberHardLimit);
	static const FName NAME_Query = FName("Query");
	...
	if (PropertyChangedEvent.Property != NULL)
	{
		...
		if (CategoryName == NAME_Generation)
		{
			...
			if (PropName == GET_MEMBER_NAME_CHECKED(ARecastNavMesh, AgentRadius))
			{
				...
				UE_LOGF(LogNavigation, Warning, "Changing AgentRadius directly on RecastNavMesh instance is unsupported. Please use Project Settings > NavigationSystem > SupportedAgents to change AgentRadius");
			}
...
void ARecastNavMesh::PostTransacted(const FTransactionObjectEvent& TransactionEvent)
{
	Super::PostTransacted(TransactionEvent);

	if (TransactionEvent.GetEventType() == ETransactionObjectEventType::UndoRedo)
	{
		// (Note, that this doesn't handle property chains at all. It can also match identical property names in a category outside "Generation", which is undesirable.)

		if (CanUpdateNavMeshInEditor())
		{
			struct FNavMeshGenerationPropertyNames
			{
				FNavMeshGenerationPropertyNames()
				{
					static const FName NAME_Generation = FName("Generation");

					// Assume almost all matches in the "Generation" category will trigger a rebuild.

					for (TFieldIterator<FProperty> PropertyIt(ARecastNavMesh::StaticClass()); PropertyIt; ++PropertyIt)
					{
						const FProperty* Property = *PropertyIt;
						const FName CategoryName = FObjectEditorUtils::GetCategoryFName(Property);
						if (CategoryName != NAME_Generation)
							continue;

						const FName PropName = Property->GetFName();
						if (PropName != GET_MEMBER_NAME_CHECKED(ARecastNavMesh, MaxSimultaneousTileGenerationJobsCount))
							ValidProperties.Add(PropName);
					}

				}

				TArray<FName> ValidProperties;
			};
			static FNavMeshGenerationPropertyNames BuiltPropertyNames;

			const TArray<FName>& PropertiesChanged = TransactionEvent.GetChangedProperties();

			const bool bRebuildNavMesh = PropertiesChanged.ContainsByPredicate([](const FName Name)
				{
					return BuiltPropertyNames.ValidProperties.Contains(Name);
				});
...
void ARecastNavMesh::DirtyTilesInBounds(const FBox& Bounds)
{
	...
	if (OverlappingBounds.IsValid)
	{
		...
		UE_LOGF(LogNavigation, VeryVerbose, "RebuildTilesFromBounds %i tiles: (%i,%i) to (%i,%i)", (TileBox.XMax-TileBox.XMin)*(TileBox.YMax-TileBox.YMin), TileBox.XMin, TileBox.YMin, TileBox.XMax, TileBox.YMax);
