...
void URecastNavMeshDataChunk::Serialize(FArchive& Ar)
{
	...
	if (Ar.IsLoading())
	{
		...
		{
			UE_LOGF(LogNavigation, Warning, "%ls: URecastNavMeshDataChunk: Nav mesh version %d < Min compatible %d. Nav mesh needs to be rebuilt. \n", *GetFullName(), NavMeshVersion, NAVMESHVER_MIN_COMPATIBLE);

			CleanUpBadVersion();
		}
		else if (NavMeshVersion > NAVMESHVER_LATEST)
		{
			UE_LOGF(LogNavigation, Warning, "%ls: URecastNavMeshDataChunk: Nav mesh version %d > NAVMESHVER_LATEST %d. Newer nav mesh should not be loaded by older versioned code. At a minimum the nav mesh needs to be rebuilt. \n", *GetFullName(), NavMeshVersion, NAVMESHVER_LATEST);
...
{
	UE_LOGF(LogNavigation, Verbose, "%ls Attaching to NavMesh - %ls", ANSI_TO_TCHAR(__FUNCTION__), *NavigationDataName.ToString());
	...
	if (DetourNavMesh != nullptr)
	{
		...
		for (FRecastTileData& TileData : Tiles)
		{
			if (!TileData.bAttached && TileData.TileRawData.IsValid())
			{
				...
				{
					UE_LOGF(LogNavigation, Warning, "Null rawdata. This can be caused by the reuse of unloaded sublevels. 'LevelStreaming.ShouldReuseUnloadedButStillAroundLevels 0' can be used until this gets fixed.");
					continue;
				}
				...
				if (dtStatusFailed(status))
				{
					if (dtStatusDetail(status, DT_OUT_OF_MEMORY))
					{
						UE_LOGF(LogNavigation, Warning, 
							...
							*NavMesh.GetName(), Header->x, Header->y, Header->layer, DetourNavMesh->getMaxTiles(), __FUNCTION__);
					}
	...
	UE_LOGF(LogNavigation, Verbose, "Attached %d tiles to NavMesh - %ls", Result.Num(), *NavigationDataName.ToString());
	return Result;
}
...
{
	UE_LOGF(LogNavigation, Verbose, "%ls Detaching from %ls", ANSI_TO_TCHAR(__FUNCTION__), *NavigationDataName.ToString());
	...
	UE_LOGF(LogNavigation, Verbose, "Detached %d tiles from NavMesh - %ls", Result.Num(), *NavigationDataName.ToString());
	return Result;
}
#endif // WITH_RECAST
...
{
#if WITH_RECAST	
	UE_LOGF(LogNavigation, Verbose, "%ls Moving %i tiles on navmesh %ls.", ANSI_TO_TCHAR(__FUNCTION__), Tiles.Num(), *NavigationDataName.ToString());
	...
	if (NavMesh != nullptr)
	{
		for (FRecastTileData& TileData : Tiles)
		{
			if (TileData.TileCacheDataSize != 0)
			{
				UE_LOGF(LogNavigation, Error, "   TileCacheRawData is expected to be empty. No support for moving the cache data yet.");
				continue;
			}
			...
			if ((TileData.bAttached == false) && TileData.TileRawData.IsValid())
			{
				...
				UE_CLOGF(bSuccess, LogNavigation, Verbose, "   Moved tile from (%i,%i) to (%i,%i).", TileData.OriginalX, TileData.OriginalY, (TileData.OriginalX + OffsetWithRotX), (TileData.OriginalY + OffsetWithRotY));
			}
		}
	}

	UE_LOGF(LogNavigation, Verbose, "%ls Moving done.", ANSI_TO_TCHAR(__FUNCTION__));
#endif// WITH_RECAST
}
