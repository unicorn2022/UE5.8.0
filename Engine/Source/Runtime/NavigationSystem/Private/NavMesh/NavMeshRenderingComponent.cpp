...
namespace FNavMeshRenderingHelpers
{
	...
	int32 GetDetailFlags(const ARecastNavMesh* NavMesh)
	{
		return (NavMesh == nullptr) ? 0 : 0 |
			...
			(NavMesh->bDrawOnScreenInformation ? (1 << static_cast<int32>(ENavMeshDetailFlags::OnScreenInformation)) : 0);
	}
#endif // WITH_RECAST
...
void FNavMeshSceneProxyData::GatherData(const ARecastNavMesh* NavMesh, int32 InNavDetailFlags, const TArray<FNavTileRef>& TileSet)
{
	...
	if (NavMesh && NavDetailFlags)
	{
		...
		const bool bDisplayOnScreenInformation = FNavMeshRenderingHelpers::HasFlag(NavDetailFlags, ENavMeshDetailFlags::OnScreenInformation);
		...
		if (bDisplayOnScreenInformation)
		{
