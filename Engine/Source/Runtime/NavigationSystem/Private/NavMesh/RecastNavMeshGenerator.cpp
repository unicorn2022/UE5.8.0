...
namespace UE::NavMesh::Private
{
	...
	static FAutoConsoleVariableRef CVarUseAsymetricBorderSizes(TEXT("ai.nav.UseAsymetricBorderSizes"), bUseAsymetricBorderSizes, TEXT("Disabled by default. When generating links, use asymetric tile border sizes to improve generation speed."), ECVF_Default);

	/** Hard upper bound on heightfield width/height (in voxels) used as a last-resort guard in CreateHeightField.
	 * Sized so that sizeof(rcTempSpan)*(size+2)*(size+2) fits within int32 (the binding constraint,
	 * since rcAlloc takes int rather than size_t): floor(sqrt(INT32_MAX / sizeof(rcTempSpan))) - 2
	 * = floor(sqrt(INT32_MAX / 8)) - 2 = 16381. The spans allocation (sizeof(rcSpan*)*size*size)
	 * is less constraining. */
	static constexpr int32 MaxHeightfieldSize = 16381;
}
...
#define USE_COMPRESSION 0
	...
	if (FileAr != NULL)
	{
		...
		UE_LOGF(LogNavigation, Error, "UncompressedBuffer size:: %d ", AllDataChunks[0].UncompressedBuffer.Num() + AllDataChunks[1].UncompressedBuffer.Num() + AllDataChunks[2].UncompressedBuffer.Num());
		FileAr->Close();
	}
...
TSubclassOf<UNavAreaBase> FRecastRawGeometryElement::GetCollisionAreaClass(const FCompositeNavModifier& InModifier)
{
	const TArray<FAreaNavModifier>& Areas = InModifier.GetAreas();
	ensureMsgf(Areas.Num() <= 1, TEXT("GetCollisionAreaClass expects to find at most one area modifier"));
	return Areas.Num() > 0 ? Areas[0].GetAreaClass() : nullptr;
}
...
static void StoreCollisionCache(FRecastGeometryExport& GeomExport)
{
	...
	// allocate memory
	constexpr int32 HeaderSize = sizeof(FRecastGeometryCache);
	const int64 CoordsSize = sizeof(FVector::FReal) * 3 * NumVerts;
	const int64 IndicesSize = sizeof(int32) * 3 * NumFaces;
	const int64 CacheSize64 = HeaderSize + CoordsSize + IndicesSize;

	if (!IntFitsIn<int32>(CacheSize64))
	{
		UE_LOGF(LogNavigation, Warning, "%s Can't cache collision data (expected size would be %lld). "
			"See other warnings, like logs about collision geometry triangle count.",
			__FUNCTION__, CacheSize64);

		// Fill with a empty header
		HeaderInfo.Validation.DataSize = 0;
		HeaderInfo.NumFaces = 0;
		HeaderInfo.NumVerts = 0;
		HeaderInfo.SlopeOverride = GeomExport.SlopeOverride;

		// empty + add combo to allocate exact amount (without any overhead/slack)
		GeomExport.Data->CollisionData.Empty(HeaderSize);
		GeomExport.Data->CollisionData.AddUninitialized(HeaderSize);
	...
	const int32 CacheSize = (int32)CacheSize64;
	...
	, FBox& UnrealBounds)
{
	...
	if (Convex->VertexData.Num())
	{
		...
		{
			UE_LOGF(LogNavigation, Verbose, "Zero indices in convex.");
			return;
		}

		if(Convex->IndexData.Num() % 3 != 0)
		{
			UE_LOGF(LogNavigation, Verbose, "Invalid indices in convex.");
			return;
		}
	}
	...
	if (const UObject* Owner = GeomExport.Data->SourceElement->GetWeakUObject().Get())
	{
		if (const UNavigationSystemV1* NavSys = FNavigationSystem::GetCurrent<UNavigationSystemV1>(Owner->GetWorld()))
		{
				...
				&& (GeomExport.VertexBuffer.Num() / CoordinatePerTriangle) > NavSys->GeometryExportTriangleCountWarningThreshold)
			{
				...
				{
					UE_LOGF(LogNavigation,
						Warning,
						"Exporting collision geometry with too many triangles (%i). This might cause performance and memory issues."
						" Add a simple collision or change GeometryExportVertexCountWarningThreshold. See '%ls'.",
...
void FRecastGeometryExport::ExportCustomMesh(const FVector* InVertices, int32 NumVerts, const int32* InIndices, int32 NumIndices, const FTransform& LocalToWorld)
{
	if (NumIndices % 3 != 0)
	{
		UE_LOGF(LogNavigation, Warning, "%s: InIndices doesn't represent a list of triangles. Skipping it [Data Owner: %ls]", __FUNCTION__, *GetDataOwnerName());
		return;
	}
...
void FRecastGeometryExport::ConvertVertexBufferToRecast()
{
	...
	{
		UE_LOGF(LogNavigation, Warning, "%s: try to convert a vertex buffer that doesn't contain a list of vertex triplets. Skipping it [Data Owner: %ls]", __FUNCTION__, *GetDataOwnerName());
		return;
	}
...
struct FOffMeshData
{
	...
	void AddLink(const FNavigationLink& Link, const FTransform& LocalToWorld, float DefaultSnapHeight, TFunctionRef<void(dtOffMeshLinkCreateParams&)> AddAreaID)
	{
		...
		NewInfo.type = DT_OFFMESH_CON_POINT | 
			...
			(Link.Direction == ENavLinkDirection::RightToLeft ? DT_OFFMESH_CON_REVERSED : 0);
	...
	void AddLinks(const TArray<FNavigationLink>& Links, const FTransform& LocalToWorld, int32 AgentIndex, float DefaultSnapHeight)
	{
		for (int32 LinkIndex = 0; LinkIndex < Links.Num(); ++LinkIndex)
		{
			...
			AddLink(Link, LocalToWorld, DefaultSnapHeight, [&] (dtOffMeshLinkCreateParams& NewInfo)
			{
				...
				{
					UE_LOGF(LogNavigation, Warning, "FRecastTileGenerator: Trying to use undefined area class while defining Off-Mesh links! (%ls)", *GetNameSafe(AreaClass));
				}
			});
		}
	}
	
	void AddLinks(const TInlineGeneratedNavLinkArray& Links, const FTransform& LocalToWorld, int32 AgentIndex, float DefaultSnapHeight)
	{
		for (int32 LinkIndex = 0; LinkIndex < Links.Num(); ++LinkIndex)
		{
...
protected:
	...
	virtual void doLog(const rcLogCategory category, const char* Msg, const int32 /*len*/) 
	{
		...
		{
		case RC_LOG_ERROR:
			UE_LOGF(LogNavigation, Error, "Recast: %s",  Msg);
			break;
		case RC_LOG_WARNING:
			UE_LOGF(LogNavigation, Log, "Recast: %s",  Msg);
			break;
		default:
			UE_LOGF(LogNavigation, VeryVerbose, "Recast: %s", Msg);
			break;
		}
	}
...
void FRecastTileGenerator::GatherNavigationDataGeometry(const TSharedRef<FNavigationRelevantData, ESPMode::ThreadSafe>& ElementDataRef, UNavigationSystemV1& NavSys, const FNavDataConfig& OwnerNavDataConfig, const bool bGeometryChanged)
{
	...
	{
		UE_LOGF(LogNavigation, Log, "Gathering geometry for tile (%i,%i): %ls.\n"
										"                       Bounds: %ls\n"
										"                       Geometry: Has=%ls Pending=%ls Slice=%ls\n"
										"                       Modifier: Has=%ls Pending=%ls",
...
void FRecastTileGenerator::MarkDynamicAreas(dtTileCacheLayer& Layer)
{
	...
	return MarkDynamicAreas(BuildContext, Layer);
}
...
{
	FNavMeshBuildContext BuildContext(*this);
	return MarkDynamicArea(BuildContext, Modifier, LocalToWorld, Layer);
}

// Deprecated
void FRecastTileGenerator::MarkDynamicArea(const FAreaNavModifier& Modifier, const FTransform& LocalToWorld, dtTileCacheLayer& Layer, const int32 AreaId, const int32* ReplaceAreaIdPtr)
{
	FNavMeshBuildContext BuildContext(*this);
	return MarkDynamicArea(BuildContext, Modifier, LocalToWorld, Layer, AreaId, ReplaceAreaIdPtr);
}
...
void FRecastTileGenerator::PrepareVoxelCache(const TNavStatArray<uint8>& RawCollisionCache, const FBox& RecastBounds, const FCompositeNavModifier& InModifier, TNavStatArray<rcSpanCache>& SpanData)
{
	...
	// Apply surface area if specified
	const TSubclassOf<UNavAreaBase> AreaClass = FRecastRawGeometryElement::GetCollisionAreaClass(InModifier);
	if (AreaClass)
	{
		const int32* AreaIDPtr = AdditionalCachedData.AreaClassToIdMap.Find(AreaClass);
		if (AreaIDPtr)
		{
			rcApplyAreaToTriangles(TriAreas.GetData(), CachedCollisions.Header.NumFaces, IntCastChecked<uint8>(*AreaIDPtr));
		}
	}
...
void FRecastTileGenerator::AppendModifier(const FCompositeNavModifier& Modifier, const FNavDataPerInstanceTransformDelegate& InTransformsDelegate)
{
	...
	if (IsTileDebugActive())
	{
		...
		{
			StrBuilder.AppendChar(' ').Append(Transform.ToString());
		}
		UE_LOGF(LogNavigation, Log, "    Modifier transforms [%d]:%ls", ModifierElement.PerInstanceTransform.Num(), *StrBuilder);

		for (const FAreaNavModifier& AreaNavModifier : ModifierElement.Areas)
		{
			UE_LOGF(LogNavigation, Log, "    Adding area modifier:\n"
										"                           Type: %ls\n"
										"                           ApplyMode: %ls\n"
										"                           AreaClass: %ls\n"
										"                           Bounds: %ls",
...
{	
	...
	GeometryElement.AreaClass = FRecastRawGeometryElement::GetCollisionAreaClass(InModifier);
	...
	if (NumIndices > 0)
	{
		UE_LOGF(LogNavigationDataBuild, VeryVerbose, "%s adding %i vertices from %ls.",
...
{
	UE_LOGF(LogNavigation, Verbose, "Building tile (time sliced): (%i,%i)", TileX, TileY);
...
bool FRecastTileGenerator::GenerateTile()
{
	...
	UE_LOGF(LogNavigation, Verbose, "Building tile: (%i,%i)", TileX, TileY);
...
bool FRecastTileGenerator::CreateHeightField(FNavMeshBuildContext& BuildContext)
{
	...
	// Guard against pathological borderSize values that would cause int32 overflow in width*height,
	// or an unrecoverable OOM allocation. This is a last-resort check; ComputeConfigBorderSizes
	// should have already warned if LinkSpillDistance was extreme.
	// Use int64 for the addition to avoid signed overflow (UB) before the check executes.
	const int64 size64 = (int64)TileConfig.tileSize + TileConfig.borderSize.low + TileConfig.borderSize.high;
	if (size64 <= 0 || size64 > UE::NavMesh::Private::MaxHeightfieldSize)
	{
		BuildContext.log(RC_LOG_ERROR, "CreateHeightField: Heightfield size %lld is out of range [1, %d]. "
			"Skipping tile.", size64, UE::NavMesh::Private::MaxHeightfieldSize);
		return false;
	}
...
ETimeSliceWorkResult FRecastTileGenerator::RasterizeGeometryRecastTimeSliced(FNavMeshBuildContext& BuildContext, 
	const TArray<FVector::FReal>& Coords, const TArray<int32>& Indices, const FBox& CoordsBounds, 
	const rcRasterizationFlags RasterizationFlags, FTileRasterizationContext& RasterContext, TSubclassOf<UNavAreaBase> AreaClass)
{
	...
	case ERasterizeGeomRecastTimeSlicedState::MarkWalkableTriangles:
	{
		...
		if (AreaClass)
		{
			...
			if (AreaIDPtr)
			{
				rcApplyAreaToTriangles(RasterizeGeomRecastTriAreas.GetData(), NumFaces, IntCastChecked<uint8>(*AreaIDPtr));
			}
		}
...
void FRecastTileGenerator::RasterizeGeometryRecast(FNavMeshBuildContext& BuildContext, 
	const TArray<FVector::FReal>& Coords, const TArray<int32>& Indices, const FBox& CoordsBounds, 
	const rcRasterizationFlags RasterizationFlags, FTileRasterizationContext& RasterContext, TSubclassOf<UNavAreaBase> AreaClass)
{
	QUICK_SCOPE_CYCLE_COUNTER(STAT_Navigation_RasterizeGeometryRecast);
...
ETimeSliceWorkResult FRecastTileGenerator::RasterizeGeometryTimeSliced(
	FNavMeshBuildContext& BuildContext, 
	const TArray<FVector::FReal>& Coords, 
	const TArray<int32>& Indices, 
	const FBox& CoordsBounds, 
	const FTransform& LocalToWorld, 
	const rcRasterizationFlags RasterizationFlags, 
	...
	TSubclassOf<UNavAreaBase> AreaClass)
{
	...
	case ERasterizeGeomTimeSlicedState::RasterizeGeometryRecast:
	{
		...
		WorkResult = RasterizeGeometryRecastTimeSliced(BuildContext, RasterizeGeometryWorldRecastCoords, RasterizeGeometryIndices, 
			RasterizeGeometryWorldRecastCoordsBounds, RasterizationFlags, RasterContext, AreaClass);
...
void FRecastTileGenerator::RasterizeGeometry(
	FNavMeshBuildContext& BuildContext, 
	const TArray<FVector::FReal>& Coords, 
	const TArray<int32>& Indices, 
	const FBox& CoordsBounds, 
	...
	TSubclassOf<UNavAreaBase> AreaClass)
{
	...
	RasterizeGeometryTransformCoordsAndFlipIndices(Coords, Indices, CoordsBounds, LocalToWorld);
	const TArray<int32>& RasterizeGeometryIndices = bRasterizeGeometryUseFlippedIndices ? RasterizeGeometryFlippedIndices : Indices;
	RasterizeGeometryRecast(BuildContext, RasterizeGeometryWorldRecastCoords, RasterizeGeometryIndices, 
		RasterizeGeometryWorldRecastCoordsBounds, RasterizationFlags, RasterContext, AreaClass);
}
	...
	while (RasterizeTrianglesTimeSlicedRawGeomIdx < RawGeometry.Num())
	{
		...
		if (Element.PerInstanceTransform.Num() > 0)
		{
			while (RasterizeTrianglesTimeSlicedInstTransformIdx < Element.PerInstanceTransform.Num())
			{
				...
				const ETimeSliceWorkResult WorkResult = RasterizeGeometryTimeSliced(BuildContext, Element.GeomCoords, Element.GeomIndices, 
					Element.GeomCoordsBounds, InstanceTransform, Element.RasterizationFlags, RasterContext, Element.AreaClass);
		...
		{
			const ETimeSliceWorkResult WorkResult = RasterizeGeometryRecastTimeSliced(BuildContext, Element.GeomCoords, Element.GeomIndices, 
				Element.GeomCoordsBounds, Element.RasterizationFlags, RasterContext, Element.AreaClass);
...
void FRecastTileGenerator::RasterizeTriangles(FNavMeshBuildContext& BuildContext, FTileRasterizationContext& RasterContext)
{
	...
	for (int32 RawGeomIdx = 0; RawGeomIdx < RawGeometry.Num(); ++RawGeomIdx)
	{
		...
		if (Element.PerInstanceTransform.Num() > 0)
		{
			...
			{
				RasterizeGeometry(BuildContext, Element.GeomCoords, Element.GeomIndices, 
					Element.GeomCoordsBounds, InstanceTransform, Element.RasterizationFlags, RasterContext, Element.AreaClass);
			}
		}
		else
		{
			RasterizeGeometryRecast(BuildContext, Element.GeomCoords, Element.GeomIndices, 
				Element.GeomCoordsBounds, Element.RasterizationFlags, RasterContext, Element.AreaClass);
		}
	}
}
...
namespace UE::NavMesh::Private
{
	...
	void ComputeConfigBorderSizes(const bool bGeneratingLinks, FRecastBuildConfig& InOutConfig)
	{
		...
		if (bGeneratingLinks)
		{
			...
			// Warn if the border is large enough to cause a significant allocation overhead.
			// At this point we do not clamp -- a large JumpLength is a valid (if expensive) configuration.
			// CreateHeightField will abort the tile if size would overflow int32.
			if (BorderForLinksVx > InOutConfig.tileSize)
			{
				UE_LOGF(LogNavigationDataBuild, Warning,
					"ComputeConfigBorderSizes: BorderForLinks (%d vx) exceeds tileSize (%d vx). "
					"This will increase memory usage during nav build.",
					BorderForLinksVx, InOutConfig.tileSize);
			}
		}
...
static void AddGeneratedLinks(
	TInlineGeneratedNavLinkArray& OutGeneratedLinks,
	const dtLinkBuilderConfig& linkBuilderConfig,
	const dtNavLinkBuilder& linkBuilder,
	const dtReal* TileBoundsMin,
	const dtReal* TileBoundsMax)
{
	// Helper to check if a point is inside the tile's XZ bounds
	auto IsInsideTileXZ = [&TileBoundsMin, &TileBoundsMax](const dtReal* pos) -> bool
	{
		return pos[0] >= TileBoundsMin[0] && pos[0] <= TileBoundsMax[0]
			&& pos[2] >= TileBoundsMin[2] && pos[2] <= TileBoundsMax[2];
	};
	...
	auto AddBidirectionalLinkLambda = [&IsInsideTileXZ , &AddLinkSharedLambda](const dtNavLinkBuilderJumpConfig& config, const dtReal* posA, const dtReal* posB)
	{
		const FVector UnrealPosA = Recast2UnrealPoint(posA);
		const FVector UnrealPosB = Recast2UnrealPoint(posB);

		// Check if the endpoints are in the current tile and, if posA isn't inside this one, flip the direction.
		// Always keep the direction from owner position to far position.

		const bool bAInside = IsInsideTileXZ(posA);
		const bool bBInside = IsInsideTileXZ(posB);
		const bool bSwapForOwnership = !bAInside && bBInside;

		const FVector OwnerPos = bSwapForOwnership ? UnrealPosB : UnrealPosA;
		const FVector FarPos = bSwapForOwnership ? UnrealPosA : UnrealPosB;
		...
		NewLink.Left = OwnerPos;
		NewLink.Right = FarPos;
	};
	...
	auto AddDirectionalLinksLambda = [&IsInsideTileXZ, &AddLinkSharedLambda](const dtNavLinkBuilderJumpConfig& config, const dtReal* posA, const dtReal* posB)
	{
		const FVector UnrealPosA = Recast2UnrealPoint(posA);
		const FVector UnrealPosB = Recast2UnrealPoint(posB);
		...
		// If posA is not inside the tile but posB is, swap directions for ownership.
		const bool bSwap = !bAInside && bBInside;
		
		const bool bABDown = UnrealPosA.Z > UnrealPosB.Z;

		// If directions are swapped but posA is above posB, reverse the directionality of the links. This is to preserve the downDirArea going down, and the upDirArea going up.
		const bool bDownIsRightToLeft = (bSwap == bABDown);

		const FVector OwnerPos = bSwap ? UnrealPosB : UnrealPosA;
		const FVector FarPos = bSwap ? UnrealPosA : UnrealPosB;
		...
		if (config.downDirArea != RECAST_NULL_AREA)
		{
			...
			DownLink.Left = OwnerPos;
			DownLink.Right = FarPos;
			DownLink.Direction = bDownIsRightToLeft
...
dtStatus FRecastTileGenerator::BuildTileCacheLinks(FNavMeshBuildContext& BuildContext, dtTileCacheAlloc* alloc, const dtTileCacheLayer& layer,
                                                   const dtTileCacheContourSet& lcset, const float DefaultSnapHeight, TInlineGeneratedNavLinkArray& OutGeneratedLinks) const
{
	...
	const FBox RcTileBB = Unreal2RecastBox(TileBB);
	const FVector RcTileMin(RcTileBB.Min);
	const FVector RcTileMax(RcTileBB.Max);

	AddGeneratedLinks(OutGeneratedLinks, linkBuilderConfig, linkBuilder, &RcTileMin.X, &RcTileMax.X);
	...
	FTileCacheAllocator& GenNavAllocator, FTileGenerationContext& GenerationContext, const dtLinkBuilderData& InLinkBuilderData, int32 LayerIdx)
{
	...
	// Rasterize obstacles.
	MarkDynamicAreas(BuildContext, *GenerationContext.Layer);
	...
	// Build Links
	TInlineGeneratedNavLinkArray GeneratedLinks;
	if (InLinkBuilderData.generatingLinks)
	{
		...
		if (SolidHF && CompactHF)
		{
			status = BuildTileCacheLinks(BuildContext, &GenNavAllocator, *GenerationContext.Layer, *GenerationContext.ContourSet, DefaultSnapHeight, GeneratedLinks);
	...
	{
		ensureMsgf(GenerationContext.PolyMesh->npolys <= TileConfig.MaxPolysPerTile, TEXT("Polys per Tile limit exceeded!"));
		...
		// Skip BVtree for tiles having few polygons since linear scan is faster for low poly counts. 
		// This saves sizeof(dtBVNode) * polyCount * 2 bytes per tile.
		constexpr int32 PolyCountThreshold = 16;
		const bool bShouldBuildBvTree = TileConfig.bGenerateBVTree && (Params.polyCount > PolyCountThreshold);
		Params.buildBvTree = bShouldBuildBvTree;

		Params.bMinimizeLinkPoolSize = TileConfig.bMinimizeLinkPoolSize;
...
ETimeSliceWorkResult FRecastTileGenerator::GenerateNavigationDataTimeSliced(FNavMeshBuildContext& BuildContext, const dtLinkBuilderData& InLinkBuilderData)
{
	...
	case EGenerateNavDataTimeSlicedState::GenerateLayers:
	{
		for (; GenNavDataLayerTimeSlicedIdx < CompressedLayers.Num() && GenNavDataLayerTimeSlicedIdx < DirtyLayers.Num(); GenNavDataLayerTimeSlicedIdx++)
		{
			...
			FTileCacheCompressor TileCompressor;
			const bool bGenDataLayerSuccess = GenerateNavigationDataLayer(BuildContext, TileCompressor, *GenNavDataTimeSlicedAllocator,
...
bool FRecastTileGenerator::GenerateNavigationData(FNavMeshBuildContext& BuildContext, const dtLinkBuilderData& InLinkBuilderData)
{
	...
	FTileCacheCompressor TileCompressor;
	bool bGenDataLayerSuccess = true;
	...
	for (int32 LayerIdx = 0; LayerIdx < CompressedLayers.Num() && LayerIdx < DirtyLayers.Num(); LayerIdx++)
	{
		...
		bGenDataLayerSuccess = GenerateNavigationDataLayer(BuildContext, TileCompressor, GenNavAllocator, GenerationContext, InLinkBuilderData, LayerIdx);
		if (!bGenDataLayerSuccess)
		{
			break;
		}
...
void FRecastTileGenerator::ComputeRasterizationMasks(FNavMeshBuildContext& BuildContext, FTileRasterizationContext& RasterContext)
{
	...
	UE_LOGF(LogNavigationDataBuild, VeryVerbose, "      %ls", ANSI_TO_TCHAR(__FUNCTION__));
...
void FRecastTileGenerator::MarkDynamicAreas(FNavMeshBuildContext& BuildContext, dtTileCacheLayer& Layer)
{
				...
				for (int32 AreaIdx = Element.Areas.Num() - 1; AreaIdx >= 0; AreaIdx--)
				{
						...
						AreaMod.GetApplyMode() == ENavigationAreaMode::ReplaceInLowPass)
					{
						...
						if (AreaIDPtr != nullptr)
						{
							...
							{
								MarkDynamicArea(BuildContext, AreaMod, LocalToWorld, Layer, *AreaIDPtr, ReplaceAreaIDPtr);
							}

							if (Element.PerInstanceTransform.Num() == 0)
							{
								MarkDynamicArea(BuildContext, AreaMod, FTransform::Identity, Layer, *AreaIDPtr, ReplaceAreaIDPtr);
							}
						}
					}
				}
		...
		for (const FRecastAreaNavModifierElement& Element : Modifiers)
		{
			for (const FAreaNavModifier& Area : Element.Areas)
			{
				...
				if (AreaIDPtr)
				{
					for (const FTransform& LocalToWorld : Element.PerInstanceTransform)
					{
						MarkDynamicArea(BuildContext, Area, LocalToWorld, Layer, *AreaIDPtr, ReplaceIDPtr);
					}

					if (Element.PerInstanceTransform.Num() == 0)
					{
						MarkDynamicArea(BuildContext, Area, FTransform::Identity, Layer, *AreaIDPtr, ReplaceIDPtr);
					}
				}
			}
		}
...
void FRecastTileGenerator::MarkDynamicArea(FNavMeshBuildContext& BuildContext, const FAreaNavModifier& Modifier, const FTransform& LocalToWorld, dtTileCacheLayer& Layer)
{
	...
	if (AreaIDPtr)
	{
		MarkDynamicArea(BuildContext, Modifier, LocalToWorld, Layer, *AreaIDPtr, ReplaceIDPtr);
	}
}
...
void FRecastTileGenerator::MarkDynamicArea(FNavMeshBuildContext& BuildContext, const FAreaNavModifier& Modifier, const FTransform& LocalToWorld, dtTileCacheLayer& Layer, const int32 AreaID, const int32* ReplaceIDPtr)
{
...
#if RECAST_INTERNAL_DEBUG_DATA
	const bool bAddShapesToDebugDraw = IsTileDebugActive() && TileDebugSettings.bDynamicAreas;
	const ARecastNavMesh* RecastParentNavmesh = nullptr;
	if (bAddShapesToDebugDraw)
	{
		TSharedPtr<FNavDataGenerator, ESPMode::ThreadSafe> ParentGenerator = ParentGeneratorWeakPtr.Pin();
		if (ParentGenerator.IsValid())
		{
			TSharedPtr<FRecastNavMeshGenerator> RecastParentGenerator = StaticCastSharedPtr<FRecastNavMeshGenerator>(ParentGenerator);
			if (const FRecastNavMeshGenerator* RecastNavMeshGeneratorPtr = RecastParentGenerator.Get())
			{
				RecastParentNavmesh = RecastNavMeshGeneratorPtr->GetOwner();
			}
		}
	}
#endif // RECAST_INTERNAL_DEBUG_DATA
	...
	case ENavigationShapeType::Cylinder:
		{
			...
			const FVector RecastPos = Unreal2RecastPoint(CylinderData.Origin);
			...
			if (bAddShapesToDebugDraw)
			{
				const FColor AreaColor = RecastParentNavmesh ? RecastParentNavmesh->GetAreaIDColor(IntCastChecked<unsigned char>(AreaID)) : FColor::White;
				duDrawCylinderArea(&BuildContext.InternalDebugData, &(RecastPos.X), CylinderData.Radius, CylinderData.Height, AreaColor.ToPackedRGBA());
			}
#endif // RECAST_INTERNAL_DEBUG_DATA
		}
		break;
	...
	case ENavigationShapeType::Box:
		{
			...
			{
				const FColor AreaColor = RecastParentNavmesh ? RecastParentNavmesh->GetAreaIDColor(IntCastChecked<unsigned char>(AreaID)) : FColor::White;
				duDrawBoxArea(&BuildContext.InternalDebugData, &(RecastPos.X), &(RecastExtent.X), AreaColor.ToPackedRGBA());
			}
#endif // RECAST_INTERNAL_DEBUG_DATA
		}
		break;
	...
	case ENavigationShapeType::InstancedConvex:
		{
			...
			if (ConvexVerts.Num())
			{
				...
				{
					const FColor AreaColor = RecastParentNavmesh ? RecastParentNavmesh->GetAreaIDColor(IntCastChecked<unsigned char>(AreaID)) : FColor::White;
					duDrawConvexArea(&BuildContext.InternalDebugData,ConvexCoords.GetData(), ConvexVerts.Num(), ConvexData.MinZ, ConvexData.MaxZ, AreaColor.ToPackedRGBA());
				}
#endif // RECAST_INTERNAL_DEBUG_DATA
			}
		}
...
void CheckTileIndicesInValidRange(const TNavStatArray<FBox>& NavigableAreas, const ARecastNavMesh& NavMesh)
{
	auto CheckTileHelper = [&NavMesh](const FVector& Pos) {
		...
		UE_CLOGF(!bIndiciesFitInInt32, LogNavigation, Error, "Magnitude of Recast tile indicies are too large to fit in an int32 for NavigableAreas extent %ls for %ls",*Pos.ToString(), *GetFullNameSafe(&NavMesh));
	};
...
int32 CalculateMaxTilesCount(const TNavStatArray<FBox>& NavigableAreas, FVector::FReal TileSizeInWorldUnits, FVector::FReal AvgLayersPerGridCell, const uint32 NavMeshVersion)
{
	...
	for (int32 Index = 0; Index < NavigableAreas.Num(); ++Index)
	{
		...
		UE_LOGF(LogNavigation, Verbose, "%s AreaBounds: %ls", __FUNCTION__, *AreaBounds.ToCompactString());
	...
	UE_SUPPRESS(LogNavigationDataBuild, Warning,
	{
		if (!IntFitsIn<int32>(MaxTilesCount))
		{
			...
			UE_LOGF(LogNavigationDataBuild, Warning, "%ls", DebugInformation.ToString());
		}
	});
...
// Whether geometry rebuild is disabled for this navmesh.
// True for Static and DynamicModifiersOnly navmesh in game worlds (neither supports rebuild from geometry),
// unless the navmesh requires an initial rebuild to recover missing nav data.
static bool IsGeometryRebuildDisabled(ARecastNavMesh* InNavMesh)
{
	return InNavMesh->GetWorld()->IsGameWorld()
		&& InNavMesh->GetRuntimeGenerationMode() != ERuntimeGenerationType::Dynamic
		&& !InNavMesh->RequiresInitialRebuild();
}
...
{
	UE_CLOGF(RunningDirtyTiles.Num() > 0, LogNavigation, Log, "Discarding %d build tasks", RunningDirtyTiles.Num());
	CancelBuild();
...
void FRecastNavMeshGenerator::SetupTileConfig(const ENavigationDataResolution TileResolution, FRecastBuildConfig& OutConfig) const
{
	...
	UE_CLOGF(OutConfig.tileSize == 1, LogNavigation, Error, "RecastNavMesh TileSize of 1 is highly discouraged. This occurence indicates an issue with RecastNavMesh\'s generation properties (specifically TileSizeUU: %f, CellSize: %f). Please ensure their correctness."
...
void FRecastNavMeshGenerator::ConfigureBuildProperties(FRecastBuildConfig& OutConfig)
{
	...
	for (int32 Index = 0; Index < (uint8)ENavigationDataResolution::MAX; Index++)
	{
		...
		if (WalkableClimbVx < RequiredClimbVx)
		{
			...
			UE_LOGF(LogNavigationDataBuild, Log, "%ls: AgentMaxStepHeight (%f) for resolution %ls is not high enough in steep slopes (AgentMaxSlope is %f). "
	...
	OutConfig.AverageLayersPerTile = FMath::Max(1.f, DestNavMesh->AverageLayersPerTile);
	...
	OutConfig.bMinimizeLinkPoolSize = DestNavMesh->bMinimizeLinkPoolSize;
...
void FRecastNavMeshGenerator::Init()
{
	...
	UE_LOGF(LogNavigation, Log, "Using max of %d workers to build navigation.", MaxTileGeneratorTasks);
	NumActiveTiles = 0;
	...
	{
		const bool bGeometryRebuildDisabled = IsGeometryRebuildDisabled(DestNavMesh);
		const dtNavMeshParams* SavedNavParams = DestNavMesh->GetRecastNavMeshImpl()->DetourNavMesh->getParams();
		if (SavedNavParams)
		{
			if (bGeometryRebuildDisabled)
			{
			...
			else
			{
				...
				if (SavedNavParams->tileHeight == TileDim && SavedNavParams->tileWidth == TileDim)
				{
					...
					{
						UE_LOGF(LogNavigation, Warning, "Recreating dtNavMesh instance %ls due to saved navmesh origin (%ls, usually the RecastNavMesh location) not being aligned with tile size (%d uu) "
				...
				if (!bRecreateNavmesh)
				{
					...
					// Only check for a tile count mismatch if there is somewhere to build navigation.
					// With no bounds (and bWholeWorldNavigable false), MaxTiles == 0 is expected and correct.
					if (NavSys && NavSys->IsThereAnywhereToBuildNavigation() &&
						FMath::CeilToInt(FMath::Log2(static_cast<float>(MaxTiles))) != FMath::CeilToInt(FMath::Log2(static_cast<float>(SavedNavParams->maxTiles))))
					{
						bRecreateNavmesh = true;
						UE_LOGF(LogNavigation, Warning, "Recreating dtNavMesh instance (%ls) due mismatch in number of bytes required to store serialized maxTiles "
							"(serialized: %d, %d bits) vs calculated required (%d, %d bits)"
							, *DestNavMesh->GetName()
	...
	// Ensure MaxPolysPerTile is always populated before use.
	// CalcNavMeshProperties may not have been called above if bRecreateNavmesh was set
	// by a path other than the tile count mismatch check (e.g. origin misalignment).
	if (MaxPolysPerTile == 0)
	{
		CalcNavMeshProperties(MaxTiles, MaxPolysPerTile);
	}

	if (bRecreateNavmesh && NavSys && NavSys->IsThereAnywhereToBuildNavigation())
	{
...
{
	UE_LOGF(LogNavigation, Log, "%s", __FUNCTION__);
	...
	{		
		if (NavSys->ShouldGenerateNavigationEverywhere() == false)
		{
			...
			UE_LOGF(LogNavigation, Log, "  %s TotalNavBounds: %ls.", __FUNCTION__, *TotalNavBounds.ToCompactString());
		}
		else
		{
			InclusionBounds.Reset(1);
			TotalNavBounds = NavSys->GetWorldBounds();
			UE_LOGF(LogNavigation, Log, "  %s bWholeWorldNavigable==true, TotalNavBounds: %ls.", __FUNCTION__, *TotalNavBounds.ToCompactString());
...
bool FRecastNavMeshGenerator::ConstructTiledNavMesh() 
{
	...
	if (DetourMesh)
	{
		ensureMsgf(DestNavMesh, TEXT("ConstructTiledNavMesh doesn't have a valid DestNavMesh"));
		...
		{
			UE_LOGF(LogNavigation, Warning, "ConstructTiledNavMesh: Failed to create navmesh of size 0.");
			bSuccess = false;
		}
		else
		{
			...
			if (dtStatusFailed(status))
			{
				UE_LOGF(LogNavigation, Warning, "ConstructTiledNavMesh: Could not init navmesh.");
				bSuccess = false;
			}
			else
			{
				...
				if (!ensureMsgf(DestNavMesh->GetRecastNavMeshImpl(), TEXT("ConstructTiledNavMesh doesn't have a valid DestNavMesh->GetRecastNavMeshImpl")))
				{
					bSuccess = false;
				}
	...
	{
		UE_LOGF(LogNavigation, Warning, "ConstructTiledNavMesh: Could not allocate navmesh." );
		bSuccess = false;
	}
...
{
	return DestNavMesh && DestNavMesh->IsGeneratingLinks(); 
}
	...
	if (IsGeneratingLinks())
	{
		auto GetAreaIDAndPolyFlags = [this](const TSubclassOf<UNavAreaBase>& AreaClass, unsigned char& OutAreaID, ARecastNavMesh::FNavPolyFlags& OutPolyFlags)
		{
			if (AreaClass)
			{
				...
				{
					UE_LOGF(LogNavigation, Warning, "FRecastTileGenerator: Trying to use undefined area class while resolving generated links areas. (%ls)", *GetNameSafe(AreaClass));
				}
			}
		};
		...
		if (ensureMsgf(DestNavMesh->NavLinkJumpConfigs.Num() == OutConfig.JumpConfigs.Num(), TEXT("Link configuration sizes are expected to match.")))
		{
			for (int32 Index = 0; Index < DestNavMesh->NavLinkJumpConfigs.Num(); Index++)
			{
				...
				{
					UE_LOGF(LogNavigation, Warning, "FRecastTileGenerator: no area class has been defined for the jump down links. Disabling them");
...
void FRecastNavMeshGenerator::CalcNavMeshProperties(int32& MaxTiles, int32& MaxPolys)
{
	...
	{
		MaxRequestedTiles = UE::NavMesh::Private::CalculateMaxTilesCount(
			InclusionBounds, Config.GetTileSizeUU(), Config.AverageLayersPerTile, DestNavMesh->NavMeshVersion);
	}
	else
	{
	...
	{
		UE_LOGF(LogNavigation, Error, "Navmesh bounds are too large! Limiting requested tiles count (%d) to: (%d) for %ls. To resolve this, try using bigger tiles or increasing the TileNumberHardLimit in the NavMesh properties.",
...
bool FRecastNavMeshGenerator::RebuildAll()
{
	...
	const UNavigationSystemV1* NavSysForRebuild = FNavigationSystem::GetCurrent<UNavigationSystemV1>(GetWorld());
	// Only recreate and rebuild the navmesh if there is somewhere to build navigation.
	// This accounts for both registered bounds volumes and the bWholeWorldNavigable flag.
	// If there is nowhere to build, leave the existing mesh intact to avoid a broken null-mesh state.
	if (NavSysForRebuild && NavSysForRebuild->IsThereAnywhereToBuildNavigation())
	{
	...
	{
		// Nowhere to build — remove all existing tiles so the mesh is empty but structurally valid.
		// This prevents stale tile data from persisting after all NavMeshBoundsVolumes are removed.
		TArray<FNavTileRef> UpdatedTiles;
		RemoveAllTiles(UpdatedTiles);
		if (UpdatedTiles.Num() > 0)
		{
			DestNavMesh->OnNavMeshTilesUpdated(UpdatedTiles);
		}
...
void FRecastNavMeshGenerator::EnsureBuildCompletion()
{
	...
	{
#if !RECAST_ASYNC_REBUILDING
		// When time slicing is enabled, we need to setup the time slice before processing tiles.
		// Use a large duration since EnsureBuildCompletion wants to complete all work immediately.
		if (SyncTimeSlicedData.TimeSliceManager && SyncTimeSlicedData.bTimeSliceRegenActive)
		{
			SyncTimeSlicedData.TimeSliceManager->GetTimeSlicer().SetupTimeSlice(TNumericLimits<double>::Max());
		}
#endif // !RECAST_ASYNC_REBUILDING
			...
			UE_SUPPRESS(LogNavigation, VeryVerbose,
			{
				if (DestNavMesh->GetRecastNavMeshImpl())
				{
					if (const dtNavMesh* DetourMesh = DestNavMesh->GetRecastNavMeshImpl()->GetRecastMesh())
					{
						...
						UE_LOGF(LogNavigation, VeryVerbose, "%ls Adding to RecentlyBuiltTiles TileId: %d Salt: %d TileRef: 0x%llx", ANSI_TO_TCHAR(__FUNCTION__), TileIndex, Salt, (dtTileRef)TileRef);
					}
				}
			});
...
void FRecastNavMeshGenerator::OnNavigationBoundsChanged()
{
	...
	if (!IsGeometryRebuildDisabled(DestNavMesh) && DetourMesh)
	{
		const UNavigationSystemV1* NavSys = FNavigationSystem::GetCurrent<UNavigationSystemV1>(GetWorld());
		if (NavSys && !NavSys->IsThereAnywhereToBuildNavigation())
		{
			// Nowhere to build navigation — clear all existing tiles so the mesh is empty but structurally valid.
			// This accounts for both the case where all bounds volumes are removed and bWholeWorldNavigable is false.
			UE_LOGF(LogNavigation, Log, "%ls> No navigation bounds, clearing navmesh tiles.", *DestNavMesh->GetName());
			TArray<FNavTileRef> UpdatedTiles;
			RemoveAllTiles(UpdatedTiles);
			if (UpdatedTiles.Num() > 0)
			{
		...
		else if (DestNavMesh->IsResizable())
		{
			if (NavSys && !NavSys->IsNavigationBuildingLocked())
			{
				// Check whether Navmesh size needs to be changed
				const int32 MaxRequestedTiles = UE::NavMesh::Private::CalculateMaxTilesCount(
					InclusionBounds, Config.GetTileSizeUU(), Config.AverageLayersPerTile, DestNavMesh->NavMeshVersion);

				if (DetourMesh->getMaxTiles() != MaxRequestedTiles)
				{
					UE_LOGF(LogNavigation, Log, "%ls> Navigation bounds changed, rebuilding navmesh", *DestNavMesh->GetName());

					// Destroy and recreate the navmesh with the new tile count.
...
void FRecastNavMeshGenerator::RemoveTiles(const TArray<FIntPoint>& Tiles)
{
	...
	for (const FIntPoint& TileXY : Tiles)
	{
		...
		if (PendingDirtyTiles.Num() > 0)
		{
			...
			PendingDirtyTiles.RemoveSwap(DirtyTile);
		}
	...
	if (LayerDataIndex != INDEX_NONE)
	{
		...
		if (LayerData.IsValid())
		{
			...
			if (dtStatusFailed(status))
			{
				...
				{
					UE_LOGF(LogNavigation, Warning, 
						"%ls> Failed to add tile (%d,%d:%d), %d tile limit reached! (from %s). Increase AverageLayersPerTile or, if using FixedTilePoolSize, increase the TilePoolSize or use bigger tiles.",
						*DestNavMesh->GetName(), TileX, TileY, LayerIndex, DetourMesh->getMaxTiles(), __FUNCTION__);
				}
			}
			else
			{
	...
	const TMap<FPendingTileElement, TArray<FNavigationDirtyAreaPerTileDebugInformation>>& DirtyAreasDebuggingInformation) const
{
	...
	for (const auto& [DirtyArea, NewlyAddedDirtyTiles, TotalDirtyTiles] : DirtyAreaToDirtyTilesCount)
	{
		...
		UE_LOGF(LogNavigationDirtyArea, VeryVerbose,
			"(navmesh: %-30ls) Dirty area trying to dirty %2d tiles (out of which %2d are newly added/not pending) | Source = %ls | Bounds size = %ls)",
...
void FRecastNavMeshGenerator::MarkDirtyTiles(const TArray<FNavigationDirtyArea>& DirtyAreas)
{
	...
	if (!bRestrictBuildingToActiveTiles || !ActiveTileSet.IsEmpty())
	{
		for (const FNavigationDirtyArea& DirtyArea : DirtyAreas)
		{
			...
			// Navmeshes with geometry rebuild disabled accept only area modifier updates
			if (bGeometryRebuildDisabled && (!DirtyArea.HasFlag(ENavigationDirtyFlag::DynamicModifier) || DirtyArea.HasFlag(ENavigationDirtyFlag::NavigationBounds)))
			{
				continue;
			}
...

			{
				...
				for (int32 TileY = TileBox.YMin; TileY <= TileBox.YMax; ++TileY)
				{
					for (int32 TileX = TileBox.XMin; TileX <= TileBox.XMax; ++TileX)
					{
						...
						// Make sure to prevent bRebuildGeometry when geometry rebuild is disabled.
						// Navmeshes with geometry rebuild disabled accept only area modifier updates. Rebuilding geometry would bRegenerateCompressedLayers without having the geometry for them.
						Element.bRebuildGeometry = !bGeometryRebuildDisabled && (DirtyArea.HasFlag(ENavigationDirtyFlag::Geometry) || DirtyArea.HasFlag(ENavigationDirtyFlag::NavigationBounds));
						Element.CreationTime = CurrentTimeSeconds;
				...
				UE_SUPPRESS(LogNavigationDirtyArea, Warning,
				{
					if (PendingTilesMarked > 0)
					{
							...
							NavSys->GetDirtyAreaWarningSizeThreshold() > 0.f && AdjustedAreaBounds.GetSize().GetMax() > NavSys->GetDirtyAreaWarningSizeThreshold())
						{
							...
							UE_LOGF(LogNavigationDirtyArea, Warning,
								"(navmesh: %-30ls) Added an oversized dirty area | Tiles marked: %2u | Source Element = %ls | Bounds size = %ls | Threshold: %.0f",
	...
	else if (SortPendingTilesMethod == ENavigationSortPendingTilesMethod::SortByPriority)
	{
		...
		UE_SUPPRESS(LogNavigation, VeryVerbose,
		{
			for (int32 Index = 0; Index < PendingDirtyTiles.Num(); Index++)
			{
				...
				const FBox TileBox = FRecastTileGenerator::CalculateTileBounds(Element.Coord.X, Element.Coord.Y, FVector::ZeroVector, TotalNavBounds, TileSizeInWorldUnits);
				const ARecastNavMesh* const OwnerNav = GetOwner();
				UE_VLOG_BOX(OwnerNav, LogNavigation, VeryVerbose, TileBox, FColor::Silver, TEXT("Index: %i, Priority %u"), Index, uint32{EnumToUnderlyingType(Element.SortingPriority)});
			}
		});
	}
}
...
void FRecastNavMeshGenerator::RemoveAllTiles(TArray<FNavTileRef>& UpdatedTiles)
{
	const dtNavMesh* DetourMesh = DestNavMesh->GetRecastNavMeshImpl() ? DestNavMesh->GetRecastNavMeshImpl()->GetRecastMesh() : nullptr;
	if (!DetourMesh)
	{
		return;
	}

	// Remove all tiles that have active data.
	// RemoveLayers is safe to call multiple times on the same coordinate — it checks
	// the layer count before acting, so duplicate coords from multi-layer tiles are harmless.
	for (int32 i = 0; i < DetourMesh->getMaxTiles(); ++i)
	{
		const dtMeshTile* Tile = DetourMesh->getTile(i);
		if (Tile && Tile->header && Tile->dataSize > 0)
		{
			RemoveLayers(FIntPoint(Tile->header->x, Tile->header->y), UpdatedTiles);
		}
	}
}
	...
	for (int32 ElementIdx = PendingDirtyTiles.Num()-1; ElementIdx >= 0 && NumProcessedTasks < NumTasksToProcess; ElementIdx--)
	{
		...
		if (!RunningDirtyTiles.Contains(RunningElement))
		{
			...
			if (TileTask->GetTask().TileGenerator->HasDataToBuild())
			{
				...
				UE_LOGF(LogNavigationDataBuild, VeryVerbose, "   Tile generation task #%i)", Count);
				Count++;
				
				RunningDirtyTiles.Add(RunningElement);
			}
			else if (!bGeometryRebuildDisabled)
			{
...
TArray<FNavTileRef> FRecastNavMeshGenerator::ProcessTileTasksSyncTimeSlicedAndGetUpdatedTiles()
{
	...
	auto DebugLongTileRegenFunction = [this](FName DebugSectionName, double Duration)
	{
		...
		if (ensure(TileGenerator))
		{
			...
			UE_LOGF(LogNavigation, Verbose, "Nav mesh data: %ls, tile at %d, %d, coordinate %f, %f, %f, section %ls is taking %f secs to partially regenerate!", *DestNavMesh->GetName(), TileGenerator->GetTileX(), TileGenerator->GetTileY(), Pos.X, Pos.Y, Pos.Z, *DebugSectionName.ToString(), Duration);
...
TArray<FNavTileRef> FRecastNavMeshGenerator::ProcessTileTasksAndGetUpdatedTiles(const int32 NumTasksToProcess)
{
	...
	if (bHasTasksAtStart && !bHasTasksAtEnd)
	{
		...
		{
			UE_LOGF(LogNavigationDataBuild, Display, "   %s build time: %.2fs", __FUNCTION__, (FPlatformTime::Seconds() - RebuildAllStartTime));
...
{
	UE_LOGF(LogNavigation, Display, "    FRecastNavMeshGenerator: self %llu", sizeof(FRecastNavMeshGenerator));
	...
	UE_LOGF(LogNavigation, Display, "    FRecastNavMeshGenerator: Total Generator\'s size %u, count %d", GeneratorsMem, RunningDirtyTiles.Num());
...
void FRecastNavMeshGenerator::GrabDebugSnapshot(struct FVisualLogEntry* Snapshot, const FBox& BoundingBox, const FName& CategoryName, ELogVerbosity::Type LogVerbosity) const
{
	...
	if (NavOctree == NULL)
	{
		UE_LOGF(LogNavigation, Error, "Failed to vlog navigation data due to %ls being NULL", NavSys == NULL ? TEXT("NavigationSystem") : TEXT("NavOctree"));
		return;
	}
...
void FRecastNavMeshGenerator::ExportNavigationData(const FString& FileName) const
{
	...
	if (NavOctree == NULL)
	{
		UE_LOGF(LogNavigation, Error, "Failed to export navigation data due to %ls being NULL", NavSys == NULL ? TEXT("NavigationSystem") : TEXT("NavOctree"));
		return;
	}
	...
	UE_LOGF(LogNavigation, Log, "ExportNavigation time: %.3f sec .", FPlatformTime::Seconds() - StartExportTime);
}
#endif
...
protected:
	...
	virtual bool Exec_Dev( UWorld* InWorld, const TCHAR* Cmd, FOutputDevice& Ar ) override
	{
		...
		if (FParse::Command(&Cmd, TEXT("ExportNavigation")))
		{
			if (InWorld == nullptr)
			{
				UE_LOGF(LogNavigation, Error, "Failed to export navigation data due to missing UWorld");
			}
			else 
			{
				...
				if (NavSys)
				{
					for (ANavigationData* NavData : NavSys->NavDataSet)
					{
						...
						{
							UE_LOGF(LogNavigation, Error, "Failed to export navigation data %ls due to missing generator", *NavData->GetName());
						}
					}
				}
				else
				{
					UE_LOGF(LogNavigation, Error, "Failed to export navigation data due to missing navigation system");
				}
			}
		}
