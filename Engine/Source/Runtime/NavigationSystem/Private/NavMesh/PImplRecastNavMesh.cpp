...
static void* DetourMalloc(int Size, dtAllocHint Hint)
{
	...
	case DT_ALLOC_PERM_TILE_DYNLINK:
		INC_MEMORY_STAT_BY(STAT_Navigation_DetourPERM_TILE_DYNLINK, ActualSize);
		break;
...
static void DetourFree(void* Original, dtAllocHint Hint)
{
	...
	case DT_ALLOC_PERM_TILE_DYNLINK:
		DEC_MEMORY_STAT_BY(STAT_Navigation_DetourPERM_TILE_DYNLINK, Size);
		break;
...
void FRecastQueryFilter::SetAllAreaCosts(const float* CostArray, const int32 Count) 
{
	...
	{
		UE_LOGF(LogNavigation, Warning, "FRecastQueryFilter: Trying to set cost to more areas than allowed! Discarding redundant values.");
	}
...
void FPImplRecastNavMesh::Serialize( FArchive& Ar, int32 NavMeshVersion )
{
	...
	if (Ar.IsSaving() && !Ar.IsTransacting() && !UE::Transaction::DiffUtil::IsGeneratingDiffableObject(Ar)) // Do not save tiles during transactions (i.e. undo/redo)
	{
		...
		if (NavMeshOwner->bIsWorldPartitioned)
		{
			...
			UE_LOGF(LogNavigation, VeryVerbose, "%ls Ar.IsSaving() no tiles are being saved because bIsWorldPartitioned=true in %ls.", ANSI_TO_TCHAR(__FUNCTION__), *NavMeshOwner->GetFullName());
		}
		else
		{
		...
		UE_LOGF(LogNavigation, VeryVerbose, "%s Ar.IsSaving() %i tiles in %ls.", __FUNCTION__, NumTiles, *NavMeshOwner->GetFullName());
	}
	...
	UE_LOGF(LogNavigation, VeryVerbose, "%s maxTiles: %i", __FUNCTION__, Params.maxTiles);
	Ar << Params.maxPolys;
	...
	if (Ar.IsLoading())
	{
		...
		// Detect mismatch with quantization factor
		bool bQuantizationFactorMismatch = false;
		if (NavMeshOwner->NavMeshVersion >= NAVMESHVER_TILE_RESOLUTIONS)
		{
			for (uint8 Index = 0; Index < (uint8)ENavigationDataResolution::MAX; Index++)
			{
				const double expectedQuantFactor =  (NavMeshOwner->NavMeshResolutionParams[Index].CellSize != 0.f) ? (1.f / NavMeshOwner->NavMeshResolutionParams[Index].CellSize) : 0.f;
				if (!FMath::IsNearlyEqual(Params.resolutionParams[(uint8)Index].bvQuantFactor, expectedQuantFactor))
				{
					UE_LOGF(LogNavigation, Warning, "%s Navmesh %ls will be loaded empty because it has been built with different navmesh settings than the current settings. "
						"If possible, this will trigger a navmesh rebuild. "
						"The mismatch was detected on stored quantization factor %f (expected: %f) for CellSize %f (resolution: %i).",
						__FUNCTION__,
						*NavMeshOwner->GetName(),
						Params.resolutionParams[(uint8)Index].bvQuantFactor,
						expectedQuantFactor,
						NavMeshOwner->NavMeshResolutionParams[Index].CellSize,
						Index);
				
					bQuantizationFactorMismatch = true;
				}
			}
		}
		...
		const bool bTileSizeMismatch = ActorsTileSize != Params.tileWidth;
		if (bTileSizeMismatch)
		{
			UE_LOGF(LogNavigation, Warning, "%hs Navmesh %ls will be loaded empty because it has been built with different navmesh settings than the current settings. "
				"If possible, this will trigger a navmesh rebuild, else you should rebuild it manually."
				"The mismatch was detected on tile size %f (expected: %f) for CellSize %f, NavmeshOwnerTileSize %f.",
				__FUNCTION__,
				*NavMeshOwner->GetName(),
				Params.tileWidth,
				ActorsTileSize,
				DefaultCellSize,
				NavMeshOwner->TileSizeUU);
		}

		if (bTileSizeMismatch || bQuantizationFactorMismatch)
		{
		...
		else
		{
			...
			// Defer off-mesh point link snapping until after all tiles are loaded
			// and area cost order is applied. Without this, links using
			// bSnapToCheapestArea snap by distance instead of cost because
			// m_areaCostOrder hasn't been initialized yet.
			DetourNavMesh->setDeferOffMeshLinking(true);
				...
				if (TileData != NULL)
				{
					...
					if (dtStatusDetail(Status, DT_OUT_OF_MEMORY))
					{
						UE_LOGF(LogNavigation, Warning, 
							"%s Failed to add tile (%d,%d:%d), %d tile limit reached in %ls. Increase AverageLayersPerTile or, if using FixedTilePoolSize, increase the TilePoolSize or use bigger tiles.",
...
void FPImplRecastNavMesh::SerializeRecastMeshTile(FArchive& Ar, int32 NavMeshVersion, unsigned char*& TileData, int32& TileDataSize)
{
	...
	// In-memory tile layout always uses latest vertex format (dtTileVert).
	...
	if (Ar.IsLoading())
	{
		...
		if (!TileData)
		{
			UE_LOGF(LogNavigation, Error, "Failed to alloc navmesh tile");
		}
		else
		{
	...
	if (TileData != NULL)
	{
		...
		dtTileVert* NavVerts = (dtTileVert*)d; d += TileLayout.VertsSize;
		...
		dtTileVert* DetailVerts = (dtTileVert*)d; d += TileLayout.DetailVertsSize;
		...
		// mesh and offmesh connection vertices
		if (NavMeshVersion < NAVMESHVER_TILEVERT_ENCODING)
		{
			// Old format is in world-space (FVector::FReal * 3 per vertex).
			if (Ar.IsLoading())
			{
				// Load old format and convert to tile-local dtTileVert.
				for (int32 VertIdx = 0; VertIdx < SizeInfo.VertCount; ++VertIdx)
				{
					FVector::FReal wx, wy, wz;
					Ar << wx;
					Ar << wy;
					Ar << wz;
					const dtReal worldPos[3] = {(dtReal)wx, (dtReal)wy, (dtReal)wz};
					NavVerts[VertIdx].setFromWorld(Header->bmin, worldPos);
				}

				// Update navigation tile data version to latest so it becomes readable.
				Header->version = DT_NAVMESH_VERSION;
			}
			else
			{
				// Save in old format: convert from tile-local dtTileVert to world-space.
				for (int32 VertIdx = 0; VertIdx < SizeInfo.VertCount; ++VertIdx)
				{
					dtReal worldPos[3];
					NavVerts[VertIdx].toWorld(Header->bmin, worldPos);
					FVector::FReal wx = (FVector::FReal)worldPos[0];
					FVector::FReal wy = (FVector::FReal)worldPos[1];
					FVector::FReal wz = (FVector::FReal)worldPos[2];
					Ar << wx;
					Ar << wy;
					Ar << wz;
				}
		...
		else
		{
			...
			for (int32 VertIdx = 0; VertIdx < SizeInfo.VertCount; ++VertIdx)
			{
				Ar << NavVerts[VertIdx].x;
				Ar << NavVerts[VertIdx].y;
				Ar << NavVerts[VertIdx].z;
			}
		}
		...
		if (NavMeshVersion < NAVMESHVER_TILEVERT_ENCODING)
		{
			...
			if (Ar.IsLoading())
			{
				...
				for (int32 VertIdx = 0; VertIdx < SizeInfo.DetailVertCount; ++VertIdx)
				{
					...
					Ar << wz;
					const dtReal worldPos[3] = {(dtReal)wx, (dtReal)wy, (dtReal)wz};
					DetailVerts[VertIdx].setFromWorld(Header->bmin, worldPos);
				}
			}
			else
			{
				...
				{
					dtReal worldPos[3];
					DetailVerts[VertIdx].toWorld(Header->bmin, worldPos);
					FVector::FReal wx = (FVector::FReal)worldPos[0];
					FVector::FReal wy = (FVector::FReal)worldPos[1];
					FVector::FReal wz = (FVector::FReal)worldPos[2];
					Ar << wx;
		...
		else
		{
			...
			for (int32 VertIdx = 0; VertIdx < SizeInfo.DetailVertCount; ++VertIdx)
			{
				Ar << DetailVerts[VertIdx].x;
				Ar << DetailVerts[VertIdx].y;
				Ar << DetailVerts[VertIdx].z;
			}
		}
...
void FPImplRecastNavMesh::SerializeCompressedTileCacheData(FArchive& Ar, int32 NavMeshVersion, unsigned char*& CompressedData, int32& CompressedDataSize)
{
	...
	if (Ar.IsLoading())
	{
		// Initialize to 0 if we are loading as this is calculated and used during processing.
	...
	if (Ar.IsSaving())
	{
		...
		// as well as valid CompressedDataSize == 0, to make this function at least as robust as it was.
	...
	if (Ar.IsLoading())
	{
		...
		if (!CompressedData)
		{
			UE_LOGF(LogNavigation, Error, "Failed to alloc tile compressed data");
		}
		else
		{
...
static void StorePathfindingDebugData(const dtNavMeshQuery& NavQuery, const dtNavMesh* NavMesh, FRecastDebugPathfindingData& Data)
{
	...
	for (int32 Idx = 0; Idx < NodeCount; Idx++)
	{
		...
		if (Data.Flags & ERecastDebugPathfindingFlags::Vertices)
		{
			...
			{
				dtReal V[3];
				NavTile->verts[NavPoly->verts[VertIdx]].toWorld(NavTile->header->bmin, V);
				NodeInfo.Verts.Add((FVector3f)Recast2UnrealPoint(V));
			}
		}
...
bool FPImplRecastNavMesh::GetPolyCenter(NavNodeRef PolyID, FVector& OutCenter) const
{
	if (DetourNavMesh)
	{
		...
		if (dtStatusSucceed(Status))
		{
			...
			{
				FVector::FReal V[3];
				Tile->verts[Poly->verts[VertIdx]].toWorld(Tile->header->bmin, V);
				Center[0] += V[0];
				Center[1] += V[1];
				Center[2] += V[2];
			}
...
FVector::FReal FPImplRecastNavMesh::GetPolySurfaceArea(NavNodeRef PolyID) const
{
	if (DetourNavMesh)
	{
		...
		if (dtStatusSucceed(Status))
		{
			...
			{
				dtReal VA[3], VB[3], VC[3];
				Tile->verts[Poly->verts[0]].toWorld(Tile->header->bmin, VA);
				Tile->verts[Poly->verts[j - 1]].toWorld(Tile->header->bmin, VB);
				Tile->verts[Poly->verts[j]].toWorld(Tile->header->bmin, VC);
	...
	if (DetourNavMesh)
	{
		...
		if (dtStatusSucceed(Status))
		{
			...
			for (uint32 LinkIt = Poly->firstLink; LinkIt != DT_NULL_LINK;)
			{
				...
				{
					FNavigationPortalEdge NeiData;
					dtReal VLeft[3], VRight[3];
					Tile->verts[Poly->verts[LinkInfo.edge]].toWorld(Tile->header->bmin, VLeft);
					Tile->verts[Poly->verts[(LinkInfo.edge + 1) % Poly->vertCount]].toWorld(Tile->header->bmin, VRight);
					NeiData.Left = Recast2UnrealPoint(VLeft);
					NeiData.Right = bIsNavLink ? NeiData.Left : Recast2UnrealPoint(VRight);
	...
	if (Tile && ClusterIdx < (uint32)Tile->header->clusterCount)
	{
		for (int32 i = 0; i < Tile->header->offMeshBase; i++)
		{
			if (Tile->polyClusters[i] == ClusterIdx)
			{
				...
				{
					FVector::FReal V[3];
					Tile->verts[Poly->verts[iVert]].toWorld(Tile->header->bmin, V);
...
void FPImplRecastNavMesh::GetEdgesForPathCorridorImpl(const TArray<NavNodeRef>* PathCorridor, TArray<FNavigationPortalEdge>* PathCorridorEdges, const dtNavMeshQuery& NavQuery) const
{
	...
	PathCorridorEdges->Empty(FMath::Max(CorridorLength - 1, 0));
	for (int32 i = 0; i < CorridorLength - 1; ++i)
	{
...
bool FPImplRecastNavMesh::GetPolysInTile(int32 TileIndex, TArray<FNavPoly>& Polys) const
{
	...
	if (MaxPolys > 0)
	{
		...
		for (int32 i = 0; i < MaxPolys; i++, Poly++)
		{
			...
			{
				dtReal V[3];
				Tile->verts[Poly->verts[k]].toWorld(Tile->header->bmin, V);
				PolyCenter += Recast2UnrealPoint(V);
			}
		...
		for (int j = 0, nj = (int)Poly->vertCount; j < nj; ++j)
		{
			if (bIsPolyiExterior)
			{
						...
						if (Link.edge == j)
						{
							...
							if (dtStatusSucceed(Status) && Polyj && PolyjTile)
							{
								if (!IsPolyExterior(*Polyj, *PolyjTile, Link.ref, DetourNavMesh->getPolyRefBase(PolyjTile)))
								{
									...
									FVector::FReal V0[3], V1[3];
									Tile.verts[Poly->verts[Link.edge]].toWorld(Tile.header->bmin, V0);
									Tile.verts[Poly->verts[(Link.edge + 1) % nj]].toWorld(Tile.header->bmin, V1);
				...
				else
				{
					...
					if (!bIsPolyjExterior)
					{
						// Poly i is interior and neighbor j is exterior
						FVector::FReal V0[3], V1[3];
						Tile.verts[Poly->verts[j]].toWorld(Tile.header->bmin, V0);
						Tile.verts[Poly->verts[(j + 1) % nj]].toWorld(Tile.header->bmin, V1);
						OutEdges.Emplace(Recast2UnrVector(V0), Recast2UnrVector(V1));
					}
				}
			}
			else
			{
				...
				if (bIsPolyjOutsideNavmesh)
				{
					...
					FVector::FReal V0[3], V1[3];
					Tile.verts[Poly->verts[(j + 1) % nj]].toWorld(Tile.header->bmin, V0);
					Tile.verts[Poly->verts[j]].toWorld(Tile.header->bmin, V1);
					OutEdges.Emplace(Recast2UnrVector(V0), Recast2UnrVector(V1));
				}
			}
		}
		...
		for (int j = 0, nj = (int)Poly->vertCount; j < nj; ++j)
		{
			...
			for (int32 k = 0; k < pd->triCount; ++k)
			{
				...
				for (int32 m = 0; m < 3; ++m)
				{
					...
					{
						Tile.verts[Poly->verts[t[m]]].toWorld(Tile.header->bmin, tvBuf[m]);
						tv[m] = tvBuf[m];
					}
					else
					{
						Tile.detailVerts[pd->vertBase + (t[m] - Poly->vertCount)].toWorld(Tile.header->bmin, tvBuf[m]);
						tv[m] = tvBuf[m];
					}
				}
...
uint8 GetValidEnds(const dtNavMesh& NavMesh, const dtMeshTile& Tile, const dtPoly& Poly, const bool bIsReversed = false)
{
	if (Poly.getType() == DT_POLYTYPE_GROUND)
	{
		return false;
	...
	while (k != DT_NULL_LINK)
	{
		...
		if (link.edge == 0)
		{
			ValidEnds |= bIsReversed ? FRecastDebugGeometry::OMLE_Right : FRecastDebugGeometry::OMLE_Left;
		}
		if (link.edge == 1)
		{
			ValidEnds |= bIsReversed ? FRecastDebugGeometry::OMLE_Left : FRecastDebugGeometry::OMLE_Right;
		}
	}
...
int32 FPImplRecastNavMesh::GetTilesDebugGeometry(const FRecastNavMeshGenerator* Generator, const dtMeshTile& Tile, int32 VertBase, FRecastDebugGeometry& OutGeometry, int32 TileIdx, uint16 ForbiddenFlags) const
{
	...
	UE_SUPPRESS(LogNavigation, VeryVerbose,
	{
		if (bIsBeingBuilt)
		{
			...
			UE_LOGF(LogNavigation, VeryVerbose, "%s TileId: %d Salt: %d TileRef: 0x%llx bIsBeingBuilt",
	...
	{
		FVector::FReal V[3];
		Tile.verts[VertIdx].toWorld(Header->bmin, V);
		FVector const VertPos = Recast2UnrVector(V);
	...
	{
		FVector::FReal V[3];
		Tile.detailVerts[DetailVertIdx].toWorld(Header->bmin, V);
		FVector const VertPos = Recast2UnrVector(V);
		OutGeometry.MeshVerts.Add(VertPos);
	}
	...
	for (int32 i = 0; i < Header->offMeshConCount; ++i)
	{
		...
		if (OffMeshConnection != NULL)
		{
			...
			FVector::FReal va[3], vb[3];
			Tile.verts[LinkPoly->verts[0]].toWorld(Tile.header->bmin, va); //OffMeshConnection->pos;
			Tile.verts[LinkPoly->verts[1]].toWorld(Tile.header->bmin, vb); //OffMeshConnection->pos[3];
			...
			const FRecastDebugGeometry::FOffMeshLink Link = {
				...
				, GetValidEnds(*DetourNavMesh, Tile, *LinkPoly, OffMeshConnection->getIsReversed())
	...
	// Note: Segment Links don't currently support the notion of reversed links yet.
	...
	for (int32 i = 0; i < Header->offMeshSegConCount; ++i)
	{
		...
		if (OffMeshSeg != NULL)
		{
			...
			for (int32 j = 0; j < OffMeshSeg->npolys; j++)
			{
				...
				FVector::FReal vLS[3], vLE[3], vRS[3], vRE[3];
				Tile.verts[LinkPoly->verts[0]].toWorld(Tile.header->bmin, vLS);
				Tile.verts[LinkPoly->verts[1]].toWorld(Tile.header->bmin, vLE);
				Tile.verts[LinkPoly->verts[2]].toWorld(Tile.header->bmin, vRS);
				Tile.verts[LinkPoly->verts[3]].toWorld(Tile.header->bmin, vRE);
				Link.LeftStart = Recast2UnrealPoint(vLS);
				Link.LeftEnd = Recast2UnrealPoint(vLE);
				Link.RightStart = Recast2UnrealPoint(vRS);
				Link.RightEnd = Recast2UnrealPoint(vRE);
...
void FPImplRecastNavMesh::OnAreaCostChanged()
{
	...
	if (NavMeshOwner && DetourNavMesh)
	{
		...
		// If off-mesh linking was deferred during batch tile loading,
		// connect them now that the cost order is correct.
		DetourNavMesh->connectDeferredOffMeshLinks();
	}
}
