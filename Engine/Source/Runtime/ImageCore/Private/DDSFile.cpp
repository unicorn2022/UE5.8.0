...
EDDSError FDDSFile::Validate() const
{
	...
	if (!FormatInfo)
	{
		UE_LOGF(LogDDSFile, Warning, "Unsupported format %d (%ls)", int(DXGIFormat), DXGIFormatGetName(DXGIFormat));
	...
	case 1:
		if (Height != 1 || Depth != 1)
		{
			UE_LOGF(LogDDSFile, Warning, "1D textures must have height and depth of 1.");
	...
	case 2:
		if (Depth != 1)
		{
			UE_LOGF(LogDDSFile, Warning, "2D textures must have depth of 1.");
			return EDDSError::BadImageDimension;
		}
		break;
	...
	case 3:
		if (ArraySize != 1)
		{
			UE_LOGF(LogDDSFile, Warning, "3D textures must have array size of 1.");
			return EDDSError::BadImageDimension;
		}
		break;

	default:
		UE_LOGF(LogDDSFile, Warning, "DDS textures must be 1D, 2D or 3D.");
	...
	{
		UE_LOGF(LogDDSFile, Warning, "One or more image dimensions are zero.");
		return EDDSError::BadImageDimension;
	}
	...
	{
		UE_LOGF(LogDDSFile, Warning, "Image dimensions %ux%ux%u of DDS exceed maximum of %u.", Width, Height, Depth, MaxDimension);
		return EDDSError::BadImageDimension;
	}
	...
	{
		UE_LOGF(LogDDSFile, Warning, "Invalid mipmap count of %u.", MipCount);
	...
	{
		UE_LOGF(LogDDSFile, Warning, "ArraySize %ux of DDS exceed maximum of %u.", ArraySize, MaxDimension);
		return EDDSError::BadImageDimension;		
	}
	...
	{
		UE_LOGF(LogDDSFile, Warning, "Image dimensions %ux%ux%ux%u of DDS exceed maximum pixel count.", Width, Height, Depth, ArraySize);
		return EDDSError::BadImageDimension;
	}
	...
	if ((Width >> FinalMip) == 0 &&
		...
		(Depth >> FinalMip) == 0)
	{
		UE_LOGF(LogDDSFile, Warning, "Invalid mipmap count of %u for %ux%ux%u image.", MipCount, Width, Height, Depth);
		return EDDSError::BadMipmapCount;
	}
	...
	if (CreateFlags & CREATE_FLAG_CUBEMAP)
	{
		if (Dimension != 2 )
		{
			UE_LOGF(LogDDSFile, Warning, "Cubemap must be dimension 2!");
			return EDDSError::BadCubemap;
		}

		if (Width != Height || Depth != 1)
		{
			UE_LOGF(LogDDSFile, Warning, "Cubemap has non-square faces or non-1 depth!");
			return EDDSError::BadCubemap;
		}

		if (ArraySize < 6 || (ArraySize % 6) != 0)
		{
			UE_LOGF(LogDDSFile, Warning, "Cubemap or cubemap array doesn't have a multiple of 6 faces.");
			return EDDSError::BadCubemap;
		}
	}
...
static EDDSError ParseHeader(FDDSFile* InDDS, FDDSHeaderWithMagic const* InHeader, FDDSHeaderDX10 const* InDX10Header)
{
	...
	if ((ddpf.flags & DDPF_FOURCC) && ddpf.fourCC == DX10_MAGIC)
	{
		...
		{
			UE_LOGF(LogDDSFile, Warning, "D3D10 resource dimension in DDS is neither 1D, 2D, nor 3D.");
			return EDDSError::BadResourceDimension;
		}
...
static EDDSError ReadPayload(FDDSFile* InDDS, const uint8* InReadCursor, const uint8* InReadEnd)
{
	...
	for (auto& Mip : InDDS->Mips)
	{
		...
		{
			UE_LOGF(LogDDSFile, Warning, "Error reading texture data.");
...
/* static */ FDDSFile* FDDSFile::CreateFromDDSInMemory(const uint8* InDDS, int64 InDDSSize, EDDSError *OutError, EDDSReadMipMode InMipReadMode)
{
	...
	{
		//UE_LOGF(LogDDSFile, Display, "Too few bytes to read DDS header");
	...
	{
		//UE_LOGF(LogDDSFile, Display, "Not a DDS file.");
	...
	if ((ddpf.flags & DDPF_FOURCC) && ddpf.fourCC == DX10_MAGIC)
	{
		...
		{
			UE_LOGF(LogDDSFile, Display, "Failed to read DX10 DDS header");
			*OutError = EDDSError::IoError;
			return nullptr;
		}
...
EDDSError FDDSFile::WriteDDS(TArray64<uint8>& OutDDS, EDDSFormatVersion InFormatVersion)
{
	...
	if (InFormatVersion == EDDSFormatVersion::D3D9)
	{
		...
		{
			UE_LOGF(LogDDSFile, Warning, "Unsupported pixel format %ls for D3D9 DDS.", DXGIFormatGetName(EffectiveFormat));
			return EDDSError::BadPixelFormat;
		}

		if (ArraySize != 1)
		{
			UE_LOGF(LogDDSFile, Warning, "D3D9 .DDS does not support arrays.");
			return EDDSError::BadImageDimension;
		}
...
bool FDDSFile::GetMipImage(const FImageView & ToImage, int MipIndex) const
{
	...
	{	
		...
		if ( DXGIToRawFormat == ToImage.Format )
		{
			if ( ! bIsExactMatch )
			{
				UE_LOGF(LogDDSFile, Warning, "DDS BlitMip DXGIFormat isn't exact match fmt %d=(%ls) to (%ls)", \
		...
		else 
		{
			...
			UE_LOGF(LogDDSFile, Warning, "DDS BlitMip DXGIFormat incompatible but same size! may be junk!  fmt %d=(%ls) to (%ls)", \
						 int(DXGIFormat), DXGIFormatGetName(DXGIFormat),ERawImageFormat::GetName(ToImage.Format));
	...
	else
	{
		...
		UE_LOGF(LogDDSFile, Warning, "DDS BlitMip DXGIFormat cannot blit!  %d (%ls) to (%ls)", \
					 int(DXGIFormat), DXGIFormatGetName(DXGIFormat),ERawImageFormat::GetName(ToImage.Format));
