...
namespace Audio
{
	...
	void FWasapiCaptureRunnable::Stop()
	{
		...
		{
			UE_LOGF(LogAudioCaptureCore, Error, "WasapiCapture Error: InputStream->WaitOnBuffer() reported %d timeouts during capture", InputStreamTimeoutsDetected);
		}
		if (InputStreamDeviceErrorsDetected > 0)
		{
			UE_LOGF(LogAudioCaptureCore, Error, "WasapiCapture Error: InputStream->WaitOnBuffer() reported %d device during capture", InputStreamDeviceErrorsDetected);
		}
	}
