...
namespace UE::Interchange
{
	namespace Private
	{
		...
		int32 GetUsableMaxAssetPathLength()
		{
			...
			static bool RunOnce = [&]()
			{
				...
				{
					UE_LOGF(LogInterchangeCore, Error,
							"Interchange can encounter import issues due to a Content path too long, and an OS limitation on path length.\n" "Content path: '%ls'\n" "System max path length: %d.\n"
							, *FullPathProjectContentDir, FPlatformMisc::GetMaxPathLength());
				}

				if (GInterchangeMaxAssetPathLength > ProjectConstraint)
				{
					UE_LOGF(LogInterchangeCore, Warning,
							"The Interchange.MaxAssetPathLength value (%d) is too high for the current setup.\n" "Content path: '%ls'\n" "System max path length: %d.\n"
	...
	FString GetAssetNameWBudget(const FString& DesiredAssetName, int32 CharBudget, TCHAR CharReplacement)
	{
		...
		if (CharBudget <= 5)
		{
			UE_LOGF(LogInterchangeCore,
				   Warning,
				   "Char budget is too small for generating a new asset name, please adapt the name of the asset manually or try modifying the value of Interchange.MaxAssetPathLength"
			);
			return DesiredAssetName;
		}
