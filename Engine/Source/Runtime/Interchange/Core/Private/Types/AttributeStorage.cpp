...
namespace UE
{
	namespace Interchange
	{
		...
		template<>
		struct TAttributeTypeTraitsInternal<FInterchangeLODMeshData>
		{
			static constexpr EAttributeTypes GetType() { return EAttributeTypes::InterchangeLODMeshData; }
			static FString ToString(const FInterchangeLODMeshData& Value) { return Value.ToString(); };
		};

		template<>
		struct TAttributeTypeTraitsInternal<TArray<FInterchangeLODMeshData>>
		{
			static constexpr EAttributeTypes GetType() { return EAttributeTypes::InterchangeLODMeshDataArray; }
			static FString ToString(const TArray<FInterchangeLODMeshData>& Value) { return StringifyArrayAttribute(Value); };
		};
		...
		FString AttributeTypeToString(EAttributeTypes AttributeType)
		{
			...
			switch (AttributeType)
			{
				...
				case EAttributeTypes::InterchangeLODMeshData: { AttributeTypeString = TEXT("InterchangeLODMeshData"); } break;
				case EAttributeTypes::InterchangeLODMeshDataArray: { AttributeTypeString = TEXT("InterchangeLODMeshDataArray"); } break;
		...
		void LogAttributeStorageErrors(const EAttributeStorageResult Result, const FString OperationName, const FAttributeKey AttributeKey)
		{
			...
			{
				UE_LOGF(LogInterchangeCore, Error, "Attribute storage operation [%ls] Key[%ls]: Storage is corrupted.", *OperationName, *(AttributeKey.ToString()));
			}
			...
			{
				UE_LOGF(LogInterchangeCore, Warning, "Attribute storage operation [%ls] Key[%ls]: Cannot find attribute key.", *OperationName, *(AttributeKey.ToString()));
			}

			if (HasAttributeStorageResult(Result, EAttributeStorageResult::Operation_Error_CannotOverrideAttribute))
			{
				UE_LOGF(LogInterchangeCore, Warning, "Attribute storage operation [%ls] Key[%ls]: Cannot override attribute.", *OperationName, *(AttributeKey.ToString()));
			}

			if (HasAttributeStorageResult(Result, EAttributeStorageResult::Operation_Error_CannotRemoveAttribute))
			{
				UE_LOGF(LogInterchangeCore, Warning, "Attribute storage operation [%ls] Key[%ls]: Cannot remove an attribute.", *OperationName, *(AttributeKey.ToString()));
			}

			if (HasAttributeStorageResult(Result, EAttributeStorageResult::Operation_Error_WrongSize))
			{
				UE_LOGF(LogInterchangeCore, Warning, "Attribute storage operation [%ls] Key[%ls]: Stored attribute value size does not match parameter value size.", *OperationName, *(AttributeKey.ToString()));
			}

			if (HasAttributeStorageResult(Result, EAttributeStorageResult::Operation_Error_WrongType))
			{
				UE_LOGF(LogInterchangeCore, Warning, "Attribute storage operation [%ls] Key[%ls]: Stored attribute value type does not match parameter value type.", *OperationName, *(AttributeKey.ToString()));
			}

			if (HasAttributeStorageResult(Result, EAttributeStorageResult::Operation_Error_InvalidStorage))
			{
				UE_LOGF(LogInterchangeCore, Warning, "Attribute storage operation [%ls] Key[%ls]: The storage is invalid (NULL).", *OperationName, *(AttributeKey.ToString()));
			}

			if (HasAttributeStorageResult(Result, EAttributeStorageResult::Operation_Error_InvalidMultiSizeValueData))
			{
				UE_LOGF(LogInterchangeCore, Warning, "Attribute storage operation [%ls] Key[%ls]: Cannot retrieve the multisize value data pointer.", *OperationName, *(AttributeKey.ToString()));
			}
		}
		...
		EAttributeStorageResult FAttributeStorage::RegisterAttribute(const FAttributeKey& ElementAttributeKey, const T& DefaultValue, EAttributeProperty AttributeProperty)
		{
			...
			else
			{
				...
				else if constexpr (std::is_same_v<T, FInterchangeLODMeshData>)
				{
					constexpr int32 NumberOfAttributes = FInterchangeLODMeshData::NumberOfAttributes();
					static_assert(NumberOfAttributes == 4, "Unexpected number of attributes on FInterchangeLODMeshData. Please update attribute parsing in scope.");
					
					AttributeAllocationInfo = &AttributeAllocationTable.Add(ElementAttributeKey);
					AttributeAllocationInfo->Type = ValueType;
					AttributeAllocationInfo->Sizes.SetNumZeroed(NumberOfAttributes);
					AttributeAllocationInfo->Offsets.SetNumZeroed(NumberOfAttributes);
					AllocationCount += NumberOfAttributes;

					{
						const uint64 ElementValueSize = GetValueSizeInternal(DefaultValue.MeshUid);

						AttributeAllocationInfo->Sizes[0] = ElementValueSize;
						AttributeAllocationInfo->Offsets[0] = (uint64)AttributeStorage.AddZeroed(ElementValueSize);
					}
					{
						const uint64 ElementValueSize = GetValueSizeInternal(DefaultValue.SceneNodeName);

						AttributeAllocationInfo->Sizes[1] = ElementValueSize;
						AttributeAllocationInfo->Offsets[1] = (uint64)AttributeStorage.AddZeroed(ElementValueSize);
					}
					{
						const uint64 ElementValueSize = GetValueSizeInternal(DefaultValue.Transform);

						AttributeAllocationInfo->Sizes[2] = ElementValueSize;
						AttributeAllocationInfo->Offsets[2] = (uint64)AttributeStorage.AddZeroed(ElementValueSize);
					}
					{
						const uint64 ElementValueSize = GetValueSizeInternal(DefaultValue.GeometricTransform);

						AttributeAllocationInfo->Sizes[3] = ElementValueSize;
						AttributeAllocationInfo->Offsets[3] = (uint64)AttributeStorage.AddZeroed(ElementValueSize);
					}
				}
				else if constexpr (std::is_same_v<T, TArray<FInterchangeLODMeshData>>)
				{
					constexpr int32 NumberOfAttributes = FInterchangeLODMeshData::NumberOfAttributes();
					static_assert(NumberOfAttributes == 4, "Unexpected number of attributes on FInterchangeLODMeshData. Please update attribute parsing in scope.");

					const TArray<FInterchangeLODMeshData>& InterchangeLODMeshArray = DefaultValue;
					const int32 NumOfAllocations = InterchangeLODMeshArray.Num() * NumberOfAttributes;
					AttributeAllocationInfo = &AttributeAllocationTable.Add(ElementAttributeKey);
					AttributeAllocationInfo->Type = ValueType;
					AttributeAllocationInfo->Sizes.SetNumZeroed(NumOfAllocations);
					AttributeAllocationInfo->Offsets.SetNumZeroed(NumOfAllocations);
					AllocationCount += NumOfAllocations;

					for (int32 Index = 0; Index < InterchangeLODMeshArray.Num(); ++Index)
					{
						const FInterchangeLODMeshData& InterchangeLODMesh = InterchangeLODMeshArray[Index];
						int32 MeshUidIndex = Index * NumberOfAttributes;
						int32 SceneNodeNameIndex = MeshUidIndex + 1;
						int32 TransformIndex = SceneNodeNameIndex + 1;
						int32 GeometricTransformIndex = TransformIndex + 1;
						{
							const uint64 ElementValueSize = GetValueSizeInternal(InterchangeLODMesh.MeshUid);

							AttributeAllocationInfo->Sizes[MeshUidIndex] = ElementValueSize;
							AttributeAllocationInfo->Offsets[MeshUidIndex] = (uint64)AttributeStorage.AddZeroed(ElementValueSize);
						}
						{
							const uint64 ElementValueSize = GetValueSizeInternal(InterchangeLODMesh.SceneNodeName);

							AttributeAllocationInfo->Sizes[SceneNodeNameIndex] = ElementValueSize;
							AttributeAllocationInfo->Offsets[SceneNodeNameIndex] = (uint64)AttributeStorage.AddZeroed(ElementValueSize);
						}
						{
							const uint64 ElementValueSize = GetValueSizeInternal(InterchangeLODMesh.Transform);

							AttributeAllocationInfo->Sizes[TransformIndex] = ElementValueSize;
							AttributeAllocationInfo->Offsets[TransformIndex] = (uint64)AttributeStorage.AddZeroed(ElementValueSize);
						}
						{
							const uint64 ElementValueSize = GetValueSizeInternal(InterchangeLODMesh.GeometricTransform);

							AttributeAllocationInfo->Sizes[GeometricTransformIndex] = ElementValueSize;
							AttributeAllocationInfo->Offsets[GeometricTransformIndex] = (uint64)AttributeStorage.AddZeroed(ElementValueSize);
						}
					}
				}
		...
		INTERCHANGECORE_API EAttributeStorageResult FAttributeStorage::GetAttribute(const FAttributeKey& ElementAttributeKey, T& OutValue) const
		{
			...
			else // Non-array types
			{
				...
				if (AttributeAllocationInfo)
				{
					...
					if constexpr (!std::is_same_v<T, FInterchangeLODMeshData>)
					{
				...
				else if constexpr (std::is_same_v<T, FInterchangeLODMeshData>)
				{
					...
					if (!ensure(NumAllocations == NumberOfAttributes))
					{
						return EAttributeStorageResult::Operation_Error_WrongSize;
					}
					...
					FInterchangeLODMeshData& InterchangeLODMesh = OutValue;

					//FString
					{
						const FStringView ValueView = GetFStringViewAttributeFromStorage(StorageData, AttributeAllocationInfo, 0);
						InterchangeLODMesh.MeshUid = ValueView;
					}

					//FString
					{
						const FStringView ValueView = GetFStringViewAttributeFromStorage(StorageData, AttributeAllocationInfo, 1);
						InterchangeLODMesh.SceneNodeName = ValueView;
					}

					//FTransform
					{
						const uint8* SourceBytes = &StorageData[AttributeAllocationInfo->Offsets[2]];
						const uint64 AllocationSize = AttributeAllocationInfo->Sizes[2];
						FMemory::Memcpy(&InterchangeLODMesh.Transform, SourceBytes, AllocationSize);
					}

					//FTransform
					{
						const uint8* SourceBytes = &StorageData[AttributeAllocationInfo->Offsets[3]];
						const uint64 AllocationSize = AttributeAllocationInfo->Sizes[3];
						FMemory::Memcpy(&InterchangeLODMesh.GeometricTransform, SourceBytes, AllocationSize);
					}
				}
				else
				{
...
#define COMPARE_MATH_STRUCTS( MathType ) \
	...
	{ \
		...
		bool bGetVersionValue = GetAttributeValue(VersionStorage, VersionTypeValue); \
		if(!bGetVersionValue) \
		{ \
			return false; \
		} \
...
#define COMPARE_MATH_ARRAY( MathType ) \
	...
	{ \
		...
		bool bGetVersionValue = GetAttributeValue(VersionStorage, VersionArray); \
		if(!bGetVersionValue || BaseArray.Num() != VersionArray.Num()) \
		{ \
			return false; \
		} \
...
#define DECLARE_LOCAL_VARIABLES( ValueType ) \
	...
	bool bGetVersionValue = GetAttributeValue(VersionStorage, Version##ValueType); \
	if (!bGetVersionValue) \
	{ \
		return false; \
	}
...
#define DECLARE_LOCAL_ARRAY_VARIABLES( ValueType ) \
	...
	bool bGetVersionValue = GetAttributeValue(VersionStorage, Version##ValueType##s); \
	if (!bGetVersionValue || Base##ValueType##s.Num() != Version##ValueType##s.Num()) \
	{ \
		return false; \
	}
		...
		bool FAttributeStorage::AreAllocationInfosEqual(const FAttributeKey& BaseKey, const FAttributeAllocationInfo& BaseInfo, const FAttributeStorage& VersionStorage, const FAttributeAllocationInfo& VersionInfo) const
		{
			...
			switch (BaseInfo.Type)
			{
				...
				case EAttributeTypes::OrientedBox:
				{
					...
					bIsEqual &= BaseOrientedBox.Center.Equals(VersionOrientedBox.Center);
				...
				case EAttributeTypes::TwoVectors:
				{
					...
					bool bIsEqual = BaseTwoVectors.v1.Equals(VersionTwoVectors.v1);
					bIsEqual &= BaseTwoVectors.v2.Equals(VersionTwoVectors.v2);
				...
				case EAttributeTypes::Float:
				{
					...
					float VersionFloat;
					bool bVersionValue = GetAttributeValue(VersionStorage, VersionFloat);
				...
				case EAttributeTypes::FloatArray:
				{
					...
					bool bVersionValue = GetAttributeValue(VersionStorage, VersionFloats);

					if (!bVersionValue || BaseFloats.Num() != VersionFloats.Num())
					{
						return false;
					}
				...
				case EAttributeTypes::Double:
				{
					...
					bool bVersionValue = GetAttributeValue(VersionStorage, VersionDouble);
				...
				case EAttributeTypes::DoubleArray:
				{
					...
					bool bVersionValue = GetAttributeValue(VersionStorage, VersionDoubles);

					if (!bVersionValue || BaseDoubles.Num() != VersionDoubles.Num())
					{
						return false;
					}
		...
		)
		{
			...
			if constexpr (TIsStringType<MultiSizeType>)
			{
				...
				const static TSet<EAttributeTypes> AllowedTypes = {
					...
					EAttributeTypes::InterchangeLODMeshData,
					EAttributeTypes::InterchangeLODMeshDataArray
				};
			...
			//InterchangeLODMeshData and InterchangeLODMeshDataArray will call this method on its properties in which case the ValueType (of the property) won't match the AttributeAllocationInfo->Type which is expected
			else if (AttributeAllocationInfo->Type != ValueType && !(AttributeAllocationInfo->Type == EAttributeTypes::InterchangeLODMeshData || AttributeAllocationInfo->Type == EAttributeTypes::InterchangeLODMeshDataArray))
			{
				return EAttributeStorageResult::Operation_Error_WrongType;
			}
		...
		EAttributeStorageResult FAttributeStorage::GenericArrayGetAttribute(const FAttributeKey& ElementAttributeKey, ArrayType& OutValue) const
		{
			...
			else if constexpr (std::is_same_v<ArrayType, TArray<FInterchangeLODMeshData>>)
			{
				const uint64 NumAllocations = AttributeAllocationInfo->Sizes.Num();

				const constexpr int32 NumberOfAttributes = FInterchangeLODMeshData::NumberOfAttributes();
				static_assert(NumberOfAttributes == 4, "Unexpected number of attributes on FInterchangeLODMeshData. Please update attribute parsing in scope.");
				ensure(NumAllocations % NumberOfAttributes == 0);

				const uint64 NumElements = NumAllocations / NumberOfAttributes;
				OutValue.SetNum(NumElements);
				...
				for (int32 ElementIndex = 0; ElementIndex < NumElements; ++ElementIndex)
				{
					//FString
					{
						const FStringView ValueView = GetFStringViewAttributeFromStorage(StorageData, AttributeAllocationInfo, ElementIndex * NumberOfAttributes);
						OutValue[ElementIndex].MeshUid = ValueView;
					}

					//FString
					{
						const FStringView ValueView = GetFStringViewAttributeFromStorage(StorageData, AttributeAllocationInfo, ElementIndex * NumberOfAttributes + 1);
						OutValue[ElementIndex].SceneNodeName = ValueView;
					}

					//FTransform
					{
						const int32 AllocationIndex = (ElementIndex * NumberOfAttributes) + 2;
						const uint8* SourceBytes = &StorageData[AttributeAllocationInfo->Offsets[AllocationIndex]];
						const uint64 AllocationSize = AttributeAllocationInfo->Sizes[AllocationIndex];
						FMemory::Memcpy(&OutValue[ElementIndex].Transform, SourceBytes, AllocationSize);
					}

					//FTransform
					{
						const int32 AllocationIndex = (ElementIndex * NumberOfAttributes) + 3;
						const uint8* SourceBytes = &StorageData[AttributeAllocationInfo->Offsets[AllocationIndex]];
						const uint64 AllocationSize = AttributeAllocationInfo->Sizes[AllocationIndex];
						FMemory::Memcpy(&OutValue[ElementIndex].GeometricTransform, SourceBytes, AllocationSize);
					}
				}
			}
		...
		EAttributeStorageResult FAttributeStorage::SetAttribute(FAttributeAllocationInfo* AttributeAllocationInfo, const T& Value)
		{
			...
			{
				if constexpr (std::is_same_v<T, TArray<FInterchangeLODMeshData>>)
				{
					const EAttributeTypes ValueType = TAttributeTypeTraitsInternal<T>::GetType();
					...
					if (Result == EAttributeStorageResult::Operation_Success)
					{
						...
						const int32 NumOfAllocations = Value.Num() * NumberOfAttributes;
						AllocationCount -= AttributeAllocationInfo->Sizes.Num();
						AllocationCount += NumOfAllocations;

						// We only need as many allocations as Value has elements: Discard the extra ones
						for (int32 Index = AttributeAllocationInfo->Sizes.Num() - 1; Index >= NumOfAllocations; --Index)
						{
							FragmentedMemoryCost += AttributeAllocationInfo->Sizes[Index];
							bNeedsDefrag = true;
						}
						AttributeAllocationInfo->Sizes.SetNum(NumOfAllocations);
						AttributeAllocationInfo->Offsets.SetNum(NumOfAllocations);

						int32 Index = 0;
						for (const FInterchangeLODMeshData& InterchangeLODMesh : Value)
						{
							//FString
							Result = MultiSizeSetAttribute(AttributeAllocationInfo, Index++, InterchangeLODMesh.MeshUid, (uint8*)(*InterchangeLODMesh.MeshUid), bNeedsDefrag);

							//FString
							if (Result == EAttributeStorageResult::Operation_Success)
							{
								Result = MultiSizeSetAttribute(AttributeAllocationInfo, Index++, InterchangeLODMesh.SceneNodeName, (uint8*)(*InterchangeLODMesh.SceneNodeName), bNeedsDefrag);
							}

							//FTransform:
							if (Result == EAttributeStorageResult::Operation_Success)
							{
								Result = MultiSizeSetAttribute(AttributeAllocationInfo, Index++, InterchangeLODMesh.Transform, &InterchangeLODMesh.Transform, bNeedsDefrag);
							}

							//FTransform:
							if (Result == EAttributeStorageResult::Operation_Success)
							{
								Result = MultiSizeSetAttribute(AttributeAllocationInfo, Index++, InterchangeLODMesh.GeometricTransform, &InterchangeLODMesh.GeometricTransform, bNeedsDefrag);
							}
						}
					}
				}
			...
			else if constexpr (std::is_same_v<T, FInterchangeLODMeshData>)
			{
				...
				if (Result == EAttributeStorageResult::Operation_Success)
				{
					...
					// We only need as many allocations as Value has elements: Discard the extra ones
					for (int32 Index = AttributeAllocationInfo->Sizes.Num() - 1; Index >= NumberOfAttributes; --Index)
					{
						FragmentedMemoryCost += AttributeAllocationInfo->Sizes[Index];
						bNeedsDefrag = true;
					}
					AttributeAllocationInfo->Sizes.SetNum(NumberOfAttributes);
					AttributeAllocationInfo->Offsets.SetNum(NumberOfAttributes);

					//FString
					Result = MultiSizeSetAttribute(AttributeAllocationInfo, 0, Value.MeshUid, (uint8*)(*Value.MeshUid), bNeedsDefrag);
					if (Result == EAttributeStorageResult::Operation_Success)
					{
						Result = MultiSizeSetAttribute(AttributeAllocationInfo, 1, Value.SceneNodeName, (uint8*)(*Value.SceneNodeName), bNeedsDefrag);
					}

					//FTransform:
					if (Result == EAttributeStorageResult::Operation_Success)
					{
						Result = MultiSizeSetAttribute(AttributeAllocationInfo, 2, Value.Transform, &Value.Transform, bNeedsDefrag);
					}

					//FTransform:
					if (Result == EAttributeStorageResult::Operation_Success)
					{
						Result = MultiSizeSetAttribute(AttributeAllocationInfo, 3, Value.GeometricTransform, &Value.GeometricTransform, bNeedsDefrag);
					}
				}
			}
...
#define SPECIALIZE(X) \
		...
		SPECIALIZE(FInterchangeLODMeshData);
		SPECIALIZE(TArray<FInterchangeLODMeshData>);
