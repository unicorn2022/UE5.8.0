...
{	
	...
	FString CreateConfigSectionName(const FName PipelineStackName, UInterchangePipelineBase* PipelineAsset)
	{
		...
		{
			UE_LOGF(LogInterchangeCore, Warning, "Failed to resolve pipeline's original pipeline path. Using class name based config section name.");
...
bool UInterchangePipelineBase::CanAddPropertyToAnalytics(TPropertyValueIterator<FProperty>& PropertyValueIterator) const
{
	const FProperty* Property = PropertyValueIterator.Key();
	if (!ensure(Property))
	{
		return false;
	}
	const void* PropertyValue = PropertyValueIterator.Value();
	...
	UInterchangePipelineBase* SubPipeline = SubObject 
												? Cast<UInterchangePipelineBase>(SubObject->GetObjectPropertyValue(PropertyValue)) 
												: nullptr;
	if (!SubPipeline && Property->GetMetaData("Category").IsEmpty())
	{
...
void UInterchangePipelineBase::AddPropertyAnalytics(TPropertyValueIterator<FProperty>& PropertyValueIterator, TArray<FAnalyticsEventAttribute>& PipelineAttributes)
{
	const FProperty* Property = PropertyValueIterator.Key();
	const void* Value = PropertyValueIterator.Value();

	TArray<const FProperty*> PropertyChain;
	PropertyValueIterator.GetPropertyChain(PropertyChain);

	const FString PropertyName = [&PropertyChain]() -> FString {
		if (PropertyChain.Num() == 1)
		{
			return PropertyChain[0]->GetName();
		}

		TStringBuilder<256> StringBuilder;	
		for (int32 PropertyIndex = PropertyChain.Num() - 1; PropertyIndex >= 0; --PropertyIndex)
		{
			StringBuilder << PropertyChain[PropertyIndex]->GetName();
			if (PropertyIndex > 0)
			{
				StringBuilder << TEXT(".");
			}
	...
	bool bUseDefaultHandler = false;
	constexpr int32 PortFlags = 0;
	if (const FArrayProperty* Array = CastField<FArrayProperty>(Property))
	{
		FScriptArrayHelper ArrayHelper(Array, Value);
		for (int32 i = 0; i < ArrayHelper.Num(); i++)
		{
			FString	ValueString;
			Array->Inner->ExportTextItem_Direct(ValueString, ArrayHelper.GetRawPtr(i), ArrayHelper.GetRawPtr(i), this, PortFlags);

			PipelineAttributes.Add(FAnalyticsEventAttribute(FString::Printf(TEXT("Property%s.%s"), *GetCategoryNameForAnalytics(PropertyChain), *PropertyName), ValueString));
		}
	}
	else if(const FStructProperty* StructProperty = CastField<FStructProperty>(Property))
	{
		if (StructProperty->Struct == TBaseStructure<FVector>::Get()
			...
			|| StructProperty->Struct == TBaseStructure<FInt32Interval>::Get())
		{
			bUseDefaultHandler = true;
			PropertyValueIterator.SkipRecursiveProperty();
		}
	}
	else
	{
	...
	if (bUseDefaultHandler)
	{
		FString	ValueString;
		Property->ExportText_Direct(ValueString, Value, Value, this, PortFlags);
		PipelineAttributes.Add(FAnalyticsEventAttribute(FString::Printf(TEXT("Property%s.%s"), *GetCategoryNameForAnalytics(PropertyChain), *PropertyName), ValueString));
	}
}

FString UInterchangePipelineBase::GetCategoryNameForAnalytics(const TArray<const FProperty*>& PropertyChain) const
{
...
#if WITH_EDITORONLY_DATA
	if (const FProperty* Property = PropertyChain.Last())
	{
			...
			for (int32 i = 0; i < Property->ArrayDim; i++)
			{
				...
				if (bFoundValue)
				{
					if (Property->ImportText_Direct(*Value, Property->ContainerPtrToValuePtr<uint8>(this, i), this, PortFlags) == NULL)
					{
						...
						UE_LOGF(LogInterchangeCore, Error, "UInterchangePipeline (class:%ls) failed to load settings. Property: %ls Value: %ls", *this->GetClass()->GetName(), *Property->GetName(), *Value);
					}
				}
			}
		}
