...
static bool GUseNodeUidAsName = false;
static FAutoConsoleVariableRef CvarGUseInterchangeUidAsName(
	TEXT("Interchange.FeatureFlags.UseNodeUidAsName"),
	GUseNodeUidAsName,
	TEXT("Whether to use the InterchangeBaseNode <DisplayName | UniqueUid> as the UObject Name. Useful for debugging."),
	ECVF_Default);

#define INTERCHANGE_USE_NODE_UID_AS_NAME (WITH_EDITOR && !UE_BUILD_SHIPPING)
...
#if INTERCHANGE_USE_NODE_UID_AS_NAME
static void UpdateNameForDebugging(UInterchangeBaseNode* Node)
{
	if (!GUseNodeUidAsName)
	{
		return;
	}

	//For debugging purpose
	const FString Name = Node->GetDisplayLabel() + TEXT(" | ") + Node->GetUniqueID();

	if (Name.Len() < NAME_SIZE && Node->Rename(*Name, nullptr, REN_Test))
	{
		Node->Rename(*Name, nullptr, REN_DontCreateRedirectors | REN_NonTransactional);
	}
}
#endif
...
#if INTERCHANGE_USE_NODE_UID_AS_NAME
	UpdateNameForDebugging(this);
#endif
}

FString UInterchangeBaseNode::GetTypeName() const
{
	const FString TypeName = "BaseNode";
	return TypeName;
}
...
FString UInterchangeBaseNode::GetKeyDisplayName(const UE::Interchange::FAttributeKey& NodeAttributeKey) const
{
	...
	{
		KeyDisplayName = UE::Interchange::BuildDisplayNameForArrayKey(TEXT("Target Asset Index "), NodeAttributeKeyString);
	}
...
{
	FString CategoryName = "Attributes";
