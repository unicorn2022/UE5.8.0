...
bool UInterchangeBaseNodeContainer::RemoveFactoryNode(const FString& NodeUniqueID)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UInterchangeBaseNodeContainer::RemoveFactoryNode)
	if (UInterchangeFactoryBaseNode* OldNode = GetFactoryNode(NodeUniqueID))
	{
...
bool UInterchangeBaseNodeContainer::ReplaceFactoryNode(const FString& NodeUniqueID, UInterchangeFactoryBaseNode* NewNode)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UInterchangeBaseNodeContainer::ReplaceNode)
	if (RemoveFactoryNode(NodeUniqueID)) //Check existence and confirm it is FactoryNode
	{
		AddNode(NewNode);
		return true;
	}
...
void UInterchangeBaseNodeContainer::SerializeNodeContainerData(FArchive& Ar)
{
	...
	else if(Ar.IsLoading())
	{
		...
		GetDerivedClasses(UInterchangeBaseNode::StaticClass(), Classes);
		...
		// The base class is never returned by GetDerivedClasses but was present before, add it manually.
		ClassPerName.Add(UInterchangeBaseNode::StaticClass()->GetFullName(), UInterchangeBaseNode::StaticClass());
...
void UInterchangeBaseNodeContainer::SetupAndReplaceFactoryNode(UInterchangeFactoryBaseNode* Node, const FString& NodeUID,
	...
	const FString& ParentNodeUID)
{
	if (RemoveFactoryNode(OldNodeUID))
	{
		SetupNode(Node, NodeUID, DisplayLabel, ContainerType, ParentNodeUID);
	}
}
