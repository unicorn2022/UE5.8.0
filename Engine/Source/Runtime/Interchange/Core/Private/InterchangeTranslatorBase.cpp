			...
			for (int32 i = 0; i < Property->ArrayDim; i++)
			{
				...
				if (bFoundValue)
				{
					if (Property->ImportText_Direct(*Value, Property->ContainerPtrToValuePtr<uint8>(this, i), this, PortFlags) == NULL)
					{
						...
						UE_LOGF(LogInterchangeCore, Error, "UInterchangeTranslatorSettings (class:%ls) failed to load settings. Property: %ls Value: %ls", *this->GetClass()->GetName(), *Property->GetName(), *Value);
					}
				}
			}
		}
