...
public:
	...
	/** 
	* Gets, SkeletalMeshes requires the Scene Transforms to be aligned with the Skeleton. 
	*	If false then Skeleton and SkeletalMesh should be aligned on Asset level already.
	*	(prior to the Scene Composition)
	* For example:
	*		true:=FBX
	*		false:=glTF
	*/
	UFUNCTION(BlueprintCallable, Category = "Interchange | Node | Source")
	INTERCHANGECORE_API bool GetCustomAlignSkeletalMeshInScene(bool& AlignSkeletalMeshInScene) const;

	/** Sets, if SkeletalMeshes requires the Scene Transforms to be aligned with the Skeleton. */
	UFUNCTION(BlueprintCallable, Category = "Interchange | Node | Source")
	INTERCHANGECORE_API bool SetCustomAlignSkeletalMeshInScene(const bool& AlignSkeletalMeshInScene);
...
private:
	...
	IMPLEMENT_NODE_ATTRIBUTE_KEY(AlignSkeletalMeshInScene);
