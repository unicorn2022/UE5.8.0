...
public:
	...
	virtual void Serialize(FArchive& Ar) override
	{
		...
		{
			FactoryDependencies.RebuildCache();
		}
	}
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Interchange | Node")
	INTERCHANGECORE_API TSet<FString> GetFactoryDependencies() const;

	/**
	 * Retrieve one dependency for this object.
	 */
	UFUNCTION(BlueprintCallable, Category = "Interchange | Node")
	UE_DEPRECATED(5.8, "Use GetFactoryDependencies or HasFactoryDependency instead.")
	INTERCHANGECORE_API void GetFactoryDependency(const int32 Index, FString& OutDependency) const;

	/**
	 * Returns true if this factory node has a dependency on the factory which Uid is DependencyUid.
	 */
	UFUNCTION(BlueprintCallable, Category = "Interchange | Node")
	INTERCHANGECORE_API bool HasFactoryDependency(const FString& DependencyUid) const;
...
protected:
	...
	inline bool ApplyPropertyValueToObject(const PropertyType& Value, UObject* Object, const FName PropertyName) const;
	 ...
	 */
	UE::Interchange::TSetAttributeHelper<FString> FactoryDependencies;
...
inline bool UInterchangeFactoryBaseNode::ApplyAttributeToObject(const FString& NodeAttributeKey, UObject* Object, const FName PropertyName) const
{
	...
	{
		return ApplyPropertyValueToObject(ValueData, Object, PropertyName);
	}
...
template<typename PropertyType>
inline bool UInterchangeFactoryBaseNode::ApplyPropertyValueToObject(const PropertyType& Value, UObject* Object, const FName PropertyName) const
{
	if (!Object)
	{
		return false;
	...
	if (FProperty* Property = InterchangePrivateNodeBase::FindPropertyByPathChecked(Container, Object->GetClass(), PropertyName.ToString()))
	{
		PropertyType* PropertyValueAddress;
		if (Container.IsType<UObject*>())
		{
			PropertyValueAddress = Property->ContainerPtrToValuePtr<PropertyType>(Container.Get<UObject*>());
		}
		else
		{
			PropertyValueAddress = Property->ContainerPtrToValuePtr<PropertyType>(Container.Get<uint8*>());
		}

		*PropertyValueAddress = Value;
...
/**
 * Specialized version of ApplyPropertyValueToObject for strings.
 * If the target property is an FObjectPropertyBase, treat the string as an object path.
 */
template<>
inline bool UInterchangeFactoryBaseNode::ApplyPropertyValueToObject<FString>(const FString& Value, UObject* Object, const FName PropertyName) const
{
	if (!Object)
	{
		return false;
	...
	if (FProperty* Property = InterchangePrivateNodeBase::FindPropertyByPathChecked(Container, Object->GetClass(), PropertyName.ToString()))
	{
		...
		if (FObjectPropertyBase* ObjectProperty = CastField<FObjectPropertyBase>(Property))
		{
			ObjectProperty->SetObjectPropertyValue(PropertyValueAddress, FSoftObjectPath(Value).TryLoad());
		}
		else
		{
			*PropertyValueAddress = Value;
...
/**
 * Specialized version of ApplyPropertyValueToObject for bools.
 * If the target property is an FBoolProperty, treat the property as a bitfield.
 */
template<>
inline bool UInterchangeFactoryBaseNode::ApplyPropertyValueToObject<bool>(const bool& Value, UObject* Object, const FName PropertyName) const
{
	if (!Object)
	{
		return false;
	...
	if (FProperty* Property = InterchangePrivateNodeBase::FindPropertyByPathChecked(Container, Object->GetClass(), PropertyName.ToString()))
	{
		...
		if (FBoolProperty* BoolProperty = CastField<FBoolProperty>(Property))
		{
			BoolProperty->SetPropertyValue(PropertyValueAddress, Value);
		}
		else
		{
			*PropertyValueAddress = Value;
