...
#define UE_API INTERCHANGECORE_API
...
public:
	...
	UE_API FString ScriptedGetPipelineDisplayName() const;
	 ...
	 */
	UFUNCTION(BlueprintNativeEvent, BlueprintCallable, Category = "Interchange | Pipeline")
	UE_API void ScriptedExecutePipeline(UInterchangeBaseNodeContainer* BaseNodeContainer, const TArray<UInterchangeSourceData*>& SourceDatas, const FString& ContentBasePath);
	 ...
	 */
	UFUNCTION(BlueprintNativeEvent, BlueprintCallable, Category = "Interchange | Pipeline")
	UE_API void ScriptedExecutePostFactoryPipeline(const UInterchangeBaseNodeContainer* BaseNodeContainer, const FString& FactoryNodeKey, UObject* CreatedAsset, bool bIsAReimport);
	 ...
	 */
	UFUNCTION(BlueprintNativeEvent, BlueprintCallable, Category = "Interchange | Pipeline")
	UE_API void ScriptedExecutePostImportPipeline(const UInterchangeBaseNodeContainer* BaseNodeContainer, const FString& FactoryNodeKey, UObject* CreatedAsset, bool bIsAReimport);
	 ...
	 */
	UFUNCTION(BlueprintNativeEvent, BlueprintCallable, Category = "Interchange | Pipeline")
	UE_API void ScriptedExecutePostBroadcastPipeline(const UInterchangeBaseNodeContainer* BaseNodeContainer, const FString& FactoryNodeKey, UObject* CreatedAsset, bool bIsAReimport);
	...
	UE_API void ScriptedExecuteExportPipeline(UInterchangeBaseNodeContainer* BaseNodeContainer);
	 ...
	 */
	UFUNCTION(BlueprintNativeEvent, BlueprintCallable, Category = "Interchange | Pipeline")
	UE_API void ScriptedSetReimportSourceIndex(UClass* ReimportObjectClass, const int32 SourceFileIndex);
	...
	UE_API static FString GetDefaultConfigFileName();
	
	UE_API void LoadSettings(const FName PipelineStackName, bool bResetPreDialog = false);

	UE_API void SaveSettings(const FName PipelineStackName);
	 ...
	 */
	UE_API virtual void AdjustSettingsForContext(const FInterchangePipelineContextParams& ContextParams);
	UE_API virtual void AdjustSettingsFromCache();

	/** Transfer the source pipeline adjust settings to this pipeline. */
	UE_API void TransferAdjustSettings(const UInterchangePipelineBase* SourcePipeline);
	 ...
	 */
	UE_API const FInterchangePipelinePropertyStates* GetPropertyStates(const FName PropertyPath) const;

	/**
	 * Return a mutable property states pointer. Return nullptr if the property does not exist.
	 */
	UE_API FInterchangePipelinePropertyStates* GetMutablePropertyStates(const FName PropertyPath);

	/**
	 * Return true if the property has valid states, or false if no states were set for the property.
	 */
	UFUNCTION(BlueprintCallable, Category = "Interchange | Pipeline")
	UE_API bool DoesPropertyStatesExist(const FName PropertyPath) const;

	/**
	 * Return a mutable property states reference. Add the property states if it doesn't exist.
	 */
	UFUNCTION(BlueprintCallable, Category = "Interchange | Pipeline")
	UE_API FInterchangePipelinePropertyStates& FindOrAddPropertyStates(const FName PropertyPath);

	/**
	 * Return the property name of the properties states map.
	 */
	static UE_API FName GetPropertiesStatesPropertyName();

	static UE_API FName GetResultsPropertyName();
	...
	*/
	UE_API void UpdateWeakObjectPtrs();
	...
	*/
	...
	UE_API UInterchangePipelineBase* GetMostPipelineOuter() const;

	UE_DEPRECATED(5.8, "Use the version with PropertyValueIterator instead.")
	virtual bool CanAddPropertyToAnalytics(FProperty* Property) const
	{
		return false;
	}

	UE_API virtual bool CanAddPropertyToAnalytics(TPropertyValueIterator<FProperty>& PropertyValueIterator) const;

	UE_DEPRECATED(5.8, "Use the version with PropertyValueIterator instead.")
	virtual void AddPropertyAnalytics(FProperty* Property, TArray<FAnalyticsEventAttribute>& PipelineAttributes)
	{
	}

	UE_API virtual void AddPropertyAnalytics(TPropertyValueIterator<FProperty>& PropertyValueIterator, TArray<FAnalyticsEventAttribute>& PipelineAttributes);


protected:

	UE_API virtual FString GetCategoryNameForAnalytics(const TArray<const FProperty*>& PropertyChain) const;

	UE_API virtual FString GetPipelineDisplayName() const;
	...
	UE_API void LoadSettingsInternal(const FName PipelineStackName, const FString& ConfigFilename, TMap<FName, FInterchangePipelinePropertyStates>& ParentPropertiesStates, bool bResetPreDialog, bool& bOutRequiresSaving);

	UE_API void SaveSettingsInternal(const FName PipelineStackName, const FString& ConfigFilename);

#if WITH_EDITOR
	static UE_API void InternalToggleVisibilityPropertiesOfMetaDataValue(UInterchangePipelineBase* OuterMostPipeline, UInterchangePipelineBase* Pipeline, bool bDoTransientSubPipeline, const FString& MetaDataKey, const FString& MetaDataValue, const bool bVisibilityState);
	static UE_API void HidePropertiesOfCategory(UInterchangePipelineBase* OuterMostPipeline, UInterchangePipelineBase* Pipeline, const FString& HideCategoryName, bool bDoTransientSubPipeline = false);
	static UE_API void HidePropertiesOfSubCategory(UInterchangePipelineBase* OuterMostPipeline, UInterchangePipelineBase* Pipeline, const FString& HideSubCategoryName, bool bDoTransientSubPipeline = false);
	static UE_API void HideProperty(UInterchangePipelineBase* OuterMostPipeline, UInterchangePipelineBase* Pipeline, const FName& HidePropertyName);
#endif //WITH_EDITOR
