...
TFuture<bool> UInterchangeMeshUtilities::InternalImportCustomLod(TSharedPtr<TPromise<bool>> Promise, UObject* MeshObject, const int32 LodIndex, const UInterchangeSourceData* SourceData, bool bAsync)
{
	...
	if (bInvalidLodIndex)
	{
		UE_LOGF(LogInterchangeEngine, Warning, "FInterchangeMeshUtilities::InternalImportCustomLod: Invalid mesh LOD index %d, no prior LOD index exists.", LodIndex);
		Promise->SetValue(false);
		return Promise->GetFuture();
	}
	...
	{
		UE_LOGF(LogInterchangeEngine, Warning, "FInterchangeMeshUtilities::InternalImportCustomLod: Cannot import mesh LOD index %d, no interchange translator support this source file. [%ls]", LodIndex, *(SourceData->GetFilename()));
		Promise->SetValue(false);
		return Promise->GetFuture();
	}
...
TFuture<bool> UInterchangeMeshUtilities::ImportMorphTarget(USkeletalMesh* SkeletalMesh, const int32 LodIndex, const UInterchangeSourceData* SourceData, bool bAsync, const FString& MorphTargetName)
{
	...
	if (!SkeletalMesh)
	{
		UE_LOGF(LogInterchangeEngine, Warning, "FInterchangeMeshUtilities::ImportMorphTarget: Cannot import morph targets, invalid skeletal mesh.");
		return ExitNoImport();
	}
	...
	if (LodIndex > SkeletalMesh->GetLODNum())
	{
		UE_LOGF(LogInterchangeEngine, Warning, "FInterchangeMeshUtilities::ImportMorphTarget: Cannot import morph targets, invalid skeletal mesh LOD index %d.", LodIndex);
		return ExitNoImport();
	}
	...
	if (!bInterchangeCanImportSourceData)
	{
		UE_LOGF(LogInterchangeEngine, Warning, "FInterchangeMeshUtilities::ImportMorphTarget: Cannot import skeletal mesh morph targets, no interchange translator support this source file. [%ls]", *(SourceData->GetFilename()));
		return ExitNoImport();
	}
	...
	AssetImportResult->OnDone([Promise, SkeletalMesh, LodIndex, SourceDataFilename, ImportAssetPath, bAsync, MorphTargetName](UE::Interchange::FImportResult& ImportResult)
		{
			...
			if (SourceStaticMesh)
			{
				...
				if (SourceLodCount == 0)
				{
					UE_LOGF(LogInterchangeEngine, Warning, "FInterchangeMeshUtilities::ImportMorphTarget: Cannot import skeletal mesh morph targets, There was no geometry in the provided source file. [%ls]", *(SourceDataFilename));
					LambdaCleanup(false);
					return;
				}
				...
				{
					UE_LOGF(LogInterchangeEngine, Warning, "FInterchangeMeshUtilities::ImportMorphTarget: Cannot import skeletal mesh morph targets, There was no geometry for LOD index %d in the provided source file. [%ls]", LodIndex, *(SourceDataFilename));
					LambdaCleanup(false);
					return;
				}
				...
				{
					UE_LOGF(LogInterchangeEngine, Warning, "FInterchangeMeshUtilities::ImportMorphTarget: Cannot import morph targets, missing skeletal mesh geometry for LOD index %d.", LodIndex);
					LambdaCleanup(false);
					return;
				}
				if (SourceMeshDescription->Vertices().Num() != TargetMeshDescription->Vertices().Num())
				{
					UE_LOGF(LogInterchangeEngine, Warning, "FInterchangeMeshUtilities::ImportMorphTarget: Cannot import morph targets, the imported morph target geometry don't match target skeletal mesh geometry topology. (LOD index %d)", LodIndex);
					LambdaCleanup(false);
					return;
				}
				...
				if (!ensure(LodInfo))
				{
					UE_LOGF(LogInterchangeEngine, Warning, "FInterchangeMeshUtilities::ImportMorphTarget: Cannot import morph targets, the skeletal mesh LOD info do not exist. (LOD index %d)", LodIndex);
					LambdaCleanup(false);
					return;
				}
