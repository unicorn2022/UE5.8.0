...
namespace UE::Interchange
{
	namespace Private::ImportCommon
	{
		...
		void RecursivelyDuplicateFactoryNodeDependencies(UInterchangeBaseNodeContainer* SourceContainer, UInterchangeBaseNodeContainer* DestinationContainer, TSet<FString>& FactoryDependencies)
		{
			...
			for (const FString& DependencyUid : FactoryDependencies)
			{
				if (UInterchangeFactoryBaseNode* FactoryNode = Cast<UInterchangeFactoryBaseNode>(SourceContainer->GetFactoryNode(DependencyUid)))
				{
					...
					TSet<FString> ChildFactoryDependencies = FactoryNode->GetFactoryDependencies();
		...
		void EndSetupAssetData(FFactoryCommon::FUpdateImportAssetDataParameters& Parameters, UInterchangeAssetImportData* AssetImportData)
		{
			...
			if (UInterchangeFactoryBaseNode* FactoryNode = Cast<UInterchangeFactoryBaseNode>(Parameters.NodeContainer->GetFactoryNode(Parameters.NodeUniqueID)))
			{
				...
				TSet<FString> FactoryDependencies;
										...
										, UInterchangeFactoryBaseNode* PipelineAssetNode)
	{
		...
		switch (ReimportStrategyFlags)
		{
			...
			case EReimportStrategyFlags::ApplyEditorChangedProperties:
			{
				...
				{
					UE_LOGF(LogInterchangeEngine, Error, "Cannot apply the reimport strategy for asset [%ls], because there is no previous asset node in the import data.", *Asset->GetName());
					return;
				}
	...
	const UInterchangeFactoryBaseNode* FFactoryCommon::GetFactoryNode(UObject* ObjectToReimport, const FString& PackageName, const FString& AssetName, const FString& SubPathString)
	{
		...
		{
			return SceneImportAsset->GetFactoryNodeFromPath(PackageName, AssetName, SubPathString);
		}
#endif
	...
	UObject* FFactoryCommon::AsyncFindObject(UInterchangeFactoryBaseNode* FactoryNode, const UClass* FactoryClass, UObject* Parent, const FString& AssetName)
	{
		...
		{
			UE_LOGF(LogInterchangeEngine, Error, "The asset class %ls is not compatible with the factory class %ls. Cannot import asset %ls ."
		...
		if (!ExistingAsset)
		{
			UE_LOGF(LogInterchangeEngine, Error, "Cannot import the %ls asset [%ls] because it was not created on the game thread.", *AssetClass->GetName(), *AssetName);
			return nullptr;
		}

		if (!ExistingAsset->GetClass()->IsChildOf(AssetClass))
		{
			UE_LOGF(LogInterchangeEngine, Error, "Cannot import the %ls asset [%ls] because it will override an asset of a different class (%ls).", *AssetClass->GetName(), *AssetName, *ExistingAsset->GetClass()->GetName());
			return nullptr;
		}
