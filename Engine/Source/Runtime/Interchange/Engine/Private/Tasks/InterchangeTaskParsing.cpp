...
namespace UE::InterchangeTaskParsing::Private
{
	...
	struct FTaskData
	{
		...
		uint64 GraphEventRef = INTERCHANGE_INVALID_TASK_ID;
...
void UE::Interchange::FTaskParsing::Execute()
{
	...
	TMap<FString, int32> TaskUniqueIdToIndex;
	...
	if (!AsyncHelper->bCancel)
	{
		for (int32 SourceIndex = 0; SourceIndex < AsyncHelper->SourceDatas.Num(); ++SourceIndex)
		{
			...
			BaseNodeContainer->IterateNodesOfType<UInterchangeFactoryBaseNode>([&](const FString& NodeUID, UInterchangeFactoryBaseNode* FactoryNode)
			{
				...
				{
					if (const UClass* RegisteredFactoryClass = InterchangeManager->GetRegisteredFactoryClass(ObjectClass))
					{
						if (UInterchangeFactoryBase* FactoryCDO = Cast<UInterchangeFactoryBase>(RegisteredFactoryClass->GetDefaultObject()))
						{
							const bool bIsSceneNode = (FactoryCDO->GetFactoryAssetType() == EInterchangeFactoryAssetType::None);
							if (bIsSceneNode && !bCanImportSceneNode)
							{
								//nothing we can import from this element
								return;
							}

							FTaskData NodeTaskData;
							...
							NodeTaskData.Dependencies = FactoryNode->GetFactoryDependencies();
							NodeTaskData.FactoryClass = RegisteredFactoryClass;
							int32 TaskDataIndex = TaskDatas.Add(NodeTaskData);
							TaskUniqueIdToIndex.Add(NodeTaskData.UniqueID, TaskDataIndex);
						}
					}
				}
			});
	...
	auto GetTaskDependencies = [&TaskDatas, &TaskUniqueIdToIndex](int32 Index)
	{
		TArray<int32> TaskDependencies;
		TaskDependencies.Reserve(TaskDatas[Index].Dependencies.Num());
		for(const FString& DependencyTaskID : TaskDatas[Index].Dependencies)
		{
			if (int32* DependencyTaskIndex = TaskUniqueIdToIndex.Find(DependencyTaskID))
			{
				TaskDependencies.Add(*DependencyTaskIndex);
			}
		}
		return TaskDependencies;
	};

	TArray<int32> TasksIndexArray;
	TasksIndexArray.SetNumUninitialized(TaskDatas.Num());
	for (int32 i = 0; i < TaskDatas.Num(); i++)
	{
		TasksIndexArray[i] = i;
	}

	// Sort Tasks According to their dependency in order that their GraphEventRef are created in in the right order
	Algo::TopologicalSort(TasksIndexArray, GetTaskDependencies);

	TArray<uint64> CompletionPrerequistes;
	for (int32 TaskIndex : TasksIndexArray)
	{
		FTaskData& TaskData = TaskDatas[TaskIndex];
		for(const FString& DependencyTaskID : TaskData.Dependencies)
		{
			if (int32* DependencyTaskIndex = TaskUniqueIdToIndex.Find(DependencyTaskID))
			{
				ensure(TaskDatas[*DependencyTaskIndex].GraphEventRef != INTERCHANGE_INVALID_TASK_ID);				
				TaskData.Prerequisites.Add(TaskDatas[*DependencyTaskIndex].GraphEventRef);
			}
		}
