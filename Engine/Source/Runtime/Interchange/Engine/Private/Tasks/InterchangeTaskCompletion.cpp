...
void UE::Interchange::FTaskPreCompletion_GameThread::Execute()
{
	...
	auto IterationCallback = [&AsyncHelper, &Results, &bIsAsset](int32 SourceIndex, const TArray<FImportAsyncHelper::FImportedObjectInfo>& ImportedObjects)
	{
		...
		for (const FImportAsyncHelper::FImportedObjectInfo& ObjectInfo : ImportedObjects)
		{
			UObject* ImportedObject = ObjectInfo.GetImportedObject();
			...
			GetObjectsWithOuter(ImportedObject, ImportedSubobjects, EGetObjectsFlags::IncludeNestedObjects);
...
void UE::Interchange::FTaskCompletion_GameThread::Execute()
{
	...
	if (AsyncHelper->AnalyticsHandler.IsValid())
	{
		FInterchangeImportResultAnalyticsInfo ImportResultAnalyticsInfo = FImportAsyncHelper::CreateImportResultAnalyticsInfo(*AsyncHelper.Get());
		AsyncHelper->AnalyticsHandler->Send(ImportResultAnalyticsInfo);
	}
	...
	if (!AsyncHelper->bCancel)
	{
		...
		AsyncHelper->IterateImportedAssetsPerSourceIndex([AsyncHelper, &bImportedSomething](int32 SourceIndex, const TArray<FImportAsyncHelper::FImportedObjectInfo>& AssetInfos)
			{
				for (const FImportAsyncHelper::FImportedObjectInfo& AssetInfo : AssetInfos)
				{
					if (UObject* Asset = AssetInfo.GetImportedObject())
					{
				...
				UE_LOGF(LogInterchangeEngine, Display, "Interchange import completed [%ls]", *AsyncHelper->SourceDatas[SourceIndex]->ToDisplayString());
			});

		//Iterate the Scene Actors
		AsyncHelper->IterateImportedSceneObjectsPerSourceIndex([AsyncHelper, &bImportedSomething](int32 SourceIndex, const TArray<FImportAsyncHelper::FImportedObjectInfo>& AssetInfos)
			{
				for (const FImportAsyncHelper::FImportedObjectInfo& SceneObjectInfo : AssetInfos)
				{
					if (AActor* Actor = Cast<AActor>(SceneObjectInfo.GetImportedObject()))
					{
						bImportedSomething = true;
						for (UInterchangePipelineBase* PipelineBase : AsyncHelper->Pipelines)
						{
	...
	else
	{
		...
		AsyncHelper->IterateImportedAssetsPerSourceIndex([AsyncHelper](int32 SourceIndex, const TArray<FImportAsyncHelper::FImportedObjectInfo>& AssetInfos)
			{
				UE_LOGF(LogInterchangeEngine, Display, "Interchange import cancelled [%ls]", *AsyncHelper->SourceDatas[SourceIndex]->ToDisplayString());
				for (const FImportAsyncHelper::FImportedObjectInfo& AssetInfo : AssetInfos)
				{
					using namespace UE::Interchange::Private::ObjectDeletionUtils;

					//Make any created asset go away
					UObject* ObjectToPurge = AssetInfo.GetImportedObject();
	...
	if (!bImportedSomething && !AsyncHelper->bCancel)
	{
		for (TObjectPtr<UInterchangeTranslatorBase> Translator : AsyncHelper->Translators)
		{
			...
			ErrorResult->Text = NSLOCTEXT("InterchangeTaskCompletion", "NothingImported", "There was nothing to import from the provided source data using the chosen pipeline options.");
