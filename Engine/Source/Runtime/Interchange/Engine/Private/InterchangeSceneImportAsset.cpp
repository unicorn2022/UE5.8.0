...
const UInterchangeFactoryBaseNode* UInterchangeSceneImportAsset::GetFactoryNodeFromPath(const FString& PackageName, const FString& AssetName, const FString& SubPathString) const
{
#if WITH_EDITORONLY_DATA
	const FSoftObjectPath ObjectPath(FTopLevelAssetPath(FName(PackageName), FName(AssetName)), SubPathString);
...
const UInterchangeFactoryBaseNode* UInterchangeSceneImportAsset::GetFactoryNodeFromPath(const FSoftObjectPath& SoftObjectPath) const
{
#if WITH_EDITORONLY_DATA
	...
	for (const TPair<FSoftObjectPath, FString>& SceneObjectPair : SceneObjects)
	{
			...
			&& SceneObjectPath.GetAssetFName() == SoftObjectPath.GetAssetFName())
		{
			...
			if (SceneObjectSubPathString.Contains(SoftObjectPathSubPathString)
				&& FActorSpawnUtils::GetBaseName(*SceneObjectSubPathString) == ObjectSubPathBaseName)
			{
				return AssetImportData->GetStoredFactoryNode(SceneObjectPair.Value);
			}
		}
	}
#endif
...
const UInterchangeFactoryBaseNode* UInterchangeSceneImportAsset::GetFactoryNodeFromUniqueID(const FString& InNodeUniqueId) const
{
#if WITH_EDITORONLY_DATA
	if (!ensure(AssetImportData))
	{
	...
	return AssetImportData->GetStoredFactoryNode(InNodeUniqueId);
#else
	return nullptr;
#endif
}
