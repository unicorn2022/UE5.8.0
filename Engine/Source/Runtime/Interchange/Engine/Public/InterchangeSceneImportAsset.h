...
public:
	...
	UE_DEPRECATED(5.8, "This function is deprecated. Use the GetFactoryNodeFromPath() instead.")
	...
	{
		return GetFactoryNodeFromPath(PackageName, AssetName, SubPathString);
	}

	UE_DEPRECATED(5.8, "This function is deprecated. Use the GetFactoryNodeFromPath() instead.")
	const UInterchangeFactoryBaseNode* GetFactoryNode(const FSoftObjectPath& ObjectPath) const
	{
		return GetFactoryNodeFromPath(ObjectPath);
	}
	 ...
	 * @param SubPathString: Optional subobject name
	 */
	UE_API const UInterchangeFactoryBaseNode* GetFactoryNodeFromPath(const FString& PackageName, const FString& AssetName, const FString& SubPathString = FString()) const;
	
	UE_API const UInterchangeFactoryBaseNode* GetFactoryNodeFromPath(const FSoftObjectPath& ObjectPath) const;

	/**
	 * Returns the factory node associated with a Node UID.
	 * @param InNodeUniqueId: Unique ID of the Node
	 * @return Factory Node if available or nullptr. 
	 */
	UE_API const UInterchangeFactoryBaseNode* GetFactoryNodeFromUniqueID(const FString& InNodeUniqueId) const;
