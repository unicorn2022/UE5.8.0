...
public:
	...
	void SetErrorDiagnostic(FString InDiagnostic) { ErrorContext.SetErrorDiagnostic(MoveTemp(InDiagnostic)); }
	const FString& GetErrorDiagnostic() const { return ErrorContext.GetErrorDiagnostic(); }
	...
	/** Store the replication protocol id associated with an error. Used to dedupe diagnostic messages per protocol. */
	void SetErrorProtocolId(FReplicationProtocolIdentifier InProtocolId) { ErrorContext.SetErrorProtocolId(InProtocolId); }
	FReplicationProtocolIdentifier GetErrorProtocolId() const { return ErrorContext.GetErrorProtocolId(); }

	/** Direct access to the underlying error context for callers that need to forward both the diagnostic and the protocol id. */
	const FNetErrorContext& GetErrorContext() const { return ErrorContext; }
	 ...
	 */
	IRISCORE_API UObject* GetLocalConnectionUserData(uint32 ConnectionId) const;
