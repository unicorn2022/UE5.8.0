...
namespace UE::Net
{
	...
	enum class ENetConnectionTraits : uint32;
...
public:
	...
	struct FReplicationSystemParams
	{
		...
		/**
		 * If true, when using remote objects with UE_WITH_REMOTE_OBJECT_HANDLE, runs code that may dereference remote UObjects inside of a transaction.
		 * If the replication system is being used by a NetDriver, the NetDriver should set this flag if the NetDriver is also replicating transactionally.
		 */
		bool bReplicateTransactionally = false;
		...
		*/
		...
		/** If true, call RepNotifies functions associated with replicated properties. */
		bool bAllowExecuteRepNotifies = true;

		/** If true, call RPC functions associated with replicated objects. */
		bool bAllowExecuteRPC = true;

		/** If true, swap local and remote roles before replicating ENetRole properties to clients. */
		bool bAlwaysSwapRolesOnReplication = false;
		...
		/** 
		 * The initial index to use when assigning new NetRefHandle.
		 * Must be greater than 0, because 0 is reserved for an invalid handle.
		 */
		uint32 InitialNetRefHandleIndex = 1;

		/** Configure the replication system as a proxy. */
		UE::Net::EProxyType ProxyType = UE::Net::EProxyType::None;

		/** 
		 * Map a UObject pointer to the unique serial field in a NetRefHandle. 
		 *
		 * This delegate is called when assigning a NetRefHandle to an object being
		 * registered for replication.
		 *
		 * Assumes there's a one-to-one mapping between UObjects registered
		 * for replication and the serial field in NetRefHandle.
		 * 
		 * If left unset, Iris uses an internal incrementing counter to assign
		 * a value to the NetRefHandle serial field.
		 */
		UE::Net::FObjectToNetRefHandleSerial ObjectToNetRefHandleSerial;

		/** 
		 * Map a unique serial field from a NetRefHandle serial to a UObject pointer.
		 *
		 * Will always be passed a serial field that is greater than 0. 
		 *
		 * Assumes there's a one-to-one mapping between UObjects registered
		 * for replication and the serial field in NetRefHandle.
		 */
		UE::Net::FNetRefHandleSerialToObject NetRefHandleSerialToObject;
	};
	...
	bool AllowExecuteRPC() const { return bAllowExecuteRPC; }
	bool AllowExecuteRepNotify() const { return bAllowExecuteRepNotify; };
	bool AlwaysSwapRolesOnReplication() const { return bAlwaysSwapRolesOnReplication; };

	UE::Net::EProxyType GetProxyType() const { return ProxyType; };
	 ...
	 * @param NetConnectionTraits Traits of the connection being added.
	 */
	IRISCORE_API void AddConnection(uint32 ConnectionId, UE::Net::ENetConnectionTraits NetConnectionTraits = {});
	...
	/**
	 * Query whether there's a connection with any of the supplied traits. 
	 * @param NetConnectionTraits The sought after traits of a connection. If UE::Net::ENetConnectionTraits::None is specified it will match all connections.
	 * @return true if a connection exists with the sought after traits.
	 */
	IRISCORE_API bool HasConnectionWithAnyTraits(UE::Net::ENetConnectionTraits NetConnectionTraits) const;
	...
	/** Special group, used for async stop replication */
	IRISCORE_API FNetObjectGroupHandle GetAsyncStopReplicationNetObjectGroup() const;
	...
	/**
	 * Create a filter at runtime.
	 * 
	 * Definition.FilterName must be unique.
	 *
	 * Rather than explicitly creating filters at runtime, they can be specified in config files and instantiated automatically by the network
	 * system. See https://dev.epicgames.com/documentation/en-us/unreal-engine/iris-filtering-in-unreal-engine#dynamic-filter-configuration .
	 *
	 * If .ini configuration applies to the filter or its use, call UObjectReplicationBridge::ReloadConfig() after creating the filter to apply that
	 * config.
	 *
	 * The filter is owned by the replication system, but can be accessed via the returned weak pointer.
	 *
	 * The filter can be destroyed later via DestroyFilter().
	 *
	 * @param Definition Filter definition (class, config).
	 *
	 * @return Weak pointer to filter instance, or nullptr.
	 */
	IRISCORE_API TWeakObjectPtr<UNetObjectFilter> CreateFilter(const FNetObjectFilterDefinition& Definition);

	/**
	 * Destroy a filter.
	 *
	 * Should only be used on filters created at runtime by calls to CreateFilter().
	 *
	 * Any objects still assigned to the filter are unassigned.
	 */
	IRISCORE_API void DestroyFilter(const FName Name);
	...
	/** Set to true if a connection's remote endpoint is a proxy. */
	IRISCORE_API void SetProxyConnection(uint32 ConnectionId, bool bEnabled);
	...
	*/
	...
	/**
	  * Mark an object as a being a debugobject. 
	  * NOTE: Only allowed in non-shipping builds
	  */
	IRISCORE_API void SetIsDebugObject(FNetRefHandle Handle);
	...
	IRISCORE_API void SetConnectionRPCDoS(uint32 ConnectionId, FRPCDoSDetection* InRPCDoS);

	bool IsReplicatingTransactionally() const { return bReplicateTransactionally; }
	...
	IRISCORE_API void ReportProtocolMismatch(uint64 NetRefHandleId, uint32 ConnectionId, const TArray<uint8>& ClientCDOStateBytes = TArray<uint8>());

	/** Called when a connection reports a critical error with a netrefhandle object */
	IRISCORE_API void ReportErrorWithNetRefHandle(UE::Net::ENetRefHandleError ErrorType, uint64 NetRefHandleId, uint32 ConnectionId, const TArray<uint64>& ExtraNetRefHandles, const FString& DiagnosticMessage = FString());
	...
	IRISCORE_API void StartParallelPhase();
	IRISCORE_API void StopParallelPhase();
	IRISCORE_API bool IsInParallelPhase() const;

	// Used to provide a NetStatsContext for the FNetSerializationContext - thread safe
	IRISCORE_API UE::Net::Private::FNetStatsContext* AcquireStatsForSerializationContext();
	// Used to free any aquired NetStatsContext used by this SerializationContext  thread safe
	IRISCORE_API void ReleaseStatsForSerializationContext(UE::Net::Private::FNetStatsContext* NetStatsContext);

	/** Function to query if a particular replicated object is relevant to a connection or not. */
	UE_EXPERIMENTAL(5.7, "IsRelevantForConnection() is experimental and should not be used.")
	IRISCORE_API bool IsRelevantForConnection(FNetRefHandle NetRefHandle, uint32 ConnectionId) const;
...
private:
	...
	void InitializeNetRefHandleGenerator(UReplicationSystem::FReplicationSystemParams& Params);
...
private:
	...
	uint32 bAllowExecuteRPC : 1;
	uint32 bAllowExecuteRepNotify : 1;
	uint32 bAlwaysSwapRolesOnReplication : 1;
	uint32 bDoCollectGarbage : 1;
	uint32 bReplicateTransactionally : 1;

	UE::Net::EProxyType ProxyType = UE::Net::EProxyType::None;

	UPROPERTY()
	TObjectPtr<UNetRefHandleGenerator> NetRefHandleGenerator;
};
...
public:
	...
	static TSharedPtr<UE::Net::Private::FObjectReferenceCache> GetSharedObjectReferenceCache();

private:

	friend UReplicationSystem* GetReplicationSystem(uint32 Id);

	static TSharedPtr<UE::Net::Private::FObjectReferenceCache> SharedObjectReferenceCache;
...
/**
* A helper RAII class to allow thread-safe access to FNetStatsContext
* If created within a ParallelPhase, the constructor acquires a ChildNetStatsContext,
* otherwise it falls back to the ParentNetStatsContext. This means that we do not
* lock any mutexes for the duration of this object, only briefly inside AcquireChildNetStatsContext
*/
class FScopedNetStatsContext
{
public:
	
	IRISCORE_API explicit FScopedNetStatsContext(UReplicationSystem* const InReplicationSystem);

	IRISCORE_API ~FScopedNetStatsContext();

	IRISCORE_API Private::FNetStatsContext* GetRawPtr();

private:
	class Private::FNetStatsContext* StoredNetStatsContext = nullptr;
	UReplicationSystem* StoredReplicationSystem = nullptr;
	bool bWasAcquiredInParallelPhase = false;
};
