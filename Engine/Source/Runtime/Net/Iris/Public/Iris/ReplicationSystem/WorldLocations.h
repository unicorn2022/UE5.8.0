...
public:
	...
	bool HasInfoForObject(FInternalNetRefIndex ObjectIndex) const
	{
	...
	inline FVector GetWorldLocation(FInternalNetRefIndex ObjectIndex) const;

	/** Get the object's current cull distance. */
	inline float GetCullDistance(FInternalNetRefIndex ObjectIndex) const;
	...
	inline TOptional<FWorldLocations::FWorldInfo> GetWorldInfo(FInternalNetRefIndex ObjectIndex) const;

	/** Set the mandatory info of a replicated root object*/
	void SetObjectInfo(FInternalNetRefIndex ObjectIndex, const FVector& Location, float NetCullDistance);
	
	/** Assign a world information cache to the replicated object */
	void InitObjectInfoCache(FInternalNetRefIndex ObjectIndex);

	/** Remove the world information cache of the replicated object */
	void RemoveObjectInfoCache(FInternalNetRefIndex ObjectIndex);
	 ...
	 */
	void SetObjectRequiresFrequentWorldLocationUpdate(FInternalNetRefIndex ObjectIndex, bool bRequiresFrequentUpdate)
	{
	...
	bool GetObjectRequiresFrequentWorldLocationUpdate(FInternalNetRefIndex ObjectIndex) const
	{
	 ...
	 */
	bool SetCullDistanceOverride(FInternalNetRefIndex ObjectIndex, float CullDistance);
	 ...
	 */
	bool ClearCullDistanceOverride(FInternalNetRefIndex ObjectIndex);

	/** Returns true if the object was set a cull distance override and is using it instead of his default cull distance value */
	bool HasCullDistanceOverride(FInternalNetRefIndex ObjectIndex) const
	{
...
private:
	...
	const int32 GetStorageIndex(FInternalNetRefIndex ObjectIndex) const
	{
		return StorageIndexes[ObjectIndex];
	}

	const FObjectInfo& GetObjectInfo(FInternalNetRefIndex ObjectIndex) const
	{
		return StoredObjectInfo[GetStorageIndex(ObjectIndex)];
	}

	FObjectInfo& GetObjectInfo(FInternalNetRefIndex ObjectIndex)
	{
		return StoredObjectInfo[GetStorageIndex(ObjectIndex)];
	}
...
inline FVector FWorldLocations::GetWorldLocation(FInternalNetRefIndex ObjectIndex) const
{
	return ValidInfoIndexes.IsBitSet(ObjectIndex) ? GetObjectInfo(ObjectIndex).WorldLocation : FVector::Zero();
}

inline float FWorldLocations::GetCullDistance(FInternalNetRefIndex ObjectIndex) const
{
...
inline TOptional<FWorldLocations::FWorldInfo> FWorldLocations::GetWorldInfo(FInternalNetRefIndex ObjectIndex) const
{
	if (ValidInfoIndexes.IsBitSet(ObjectIndex))
	{
		const FObjectInfo& Info = GetObjectInfo(ObjectIndex);
