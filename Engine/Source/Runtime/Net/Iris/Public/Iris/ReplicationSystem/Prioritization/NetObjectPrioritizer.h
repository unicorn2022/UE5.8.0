...
struct FNetObjectPrioritizationParams
{
	 ...
	 */
	const UE::Net::FInternalNetRefIndex* ObjectIndices;

	/** The number of objects to prioritize. */
	uint32 ObjectCount;

	//TODO: This should be a TArrayView
...
public:
	...
	virtual bool AddObject(UE::Net::FInternalNetRefIndex ObjectIndex, FNetObjectPrioritizerAddObjectParams& Params) PURE_VIRTUAL(AddObject, return false;)

	/** An object do no longer want to use this prioritizer. */
	virtual void RemoveObject(UE::Net::FInternalNetRefIndex ObjectIndex, const FNetObjectPrioritizationInfo& Info) PURE_VIRTUAL(RemoveObject,)
