...
constexpr uint32 GetBitsNeeded(const T Value)
{
 ...
 */
template<typename T, typename TEnableIf<TIsSigned<T>::Value && sizeof(T) <= 4U, int32>::Type X = -1>
constexpr uint32 GetBitsNeededForRange(const T LowerBound, const T UpperBound)
{
...
constexpr T GetLeastSignificantBit(const T Value)
{
...
/** Return the largest unsigned value that can be stored in BitCount numbers of bits. */
constexpr inline uint64 LargestBitValueForBits(uint64 BitCount)
{
	if (BitCount == 0)
	{
		return 0ULL;
