...
{
	{
		UE_IRIS_PARALLEL_EXPR(UE::TRWScopeLock FNameReadLock(NameTokenStoreMutex, SLT_ReadOnly));
	...
	if (NewKey.IsValid())
	{
		UE_IRIS_PARALLEL_EXPR(UE::TRWScopeLock FNameWriteLock(NameTokenStoreMutex, SLT_Write));
...
FName FNameTokenStore::ResolveToken(FNetToken Token, const FNetTokenStoreState* NetTokenStoreState) const
{
	...
	if (Token.IsValid() && ensureMsgf(TokenStoreState, TEXT("FNameTokenStore::ResolveToken Needs valid remote NetTokenStoreState to resolve remote %s"), *Token.ToString()))
	{
		...
		{
			UE_LOGF(LogNetToken, Error, "FNameTokenStore::ResolveToken failed to resolve %ls in NetTokenDataStore: %ls", *Token.ToString(), *GetTokenStoreName().ToString());
		}
	}
