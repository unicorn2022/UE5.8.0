...
static const FName NAME_DefaultPrioritizer("DefaultPrioritizer");
...
public:
	...
	{
		TChunkedArrayWithChunkManagement<FInternalNetRefIndex, MaxObjectCountPerBatch*sizeof(uint32)> ObjectIndices;
	};
...
public:
	...
	struct FPerPrioritizerInfo
	{
		FInternalNetRefIndex* ObjectIndices;
		uint32 ObjectCount;
...
private:
	...
	TArray<UE::Net::FInternalNetRefIndex> ObjectIndicesStorage;
...
void FReplicationPrioritization::SetStaticPriority(FInternalNetRefIndex ObjectIndex, float NewPrio)
{
...
bool FReplicationPrioritization::SetPrioritizer(FInternalNetRefIndex ObjectIndex, FNetObjectPrioritizerHandle NewPrioritizer)
{
	...
	// Register object with new prioritizer
	{
		...
		UE_LOGF(LogIris, Verbose, "Prioritizer '%ls' does not support prioritizing object %u", ToCStr(PrioritizerInfo.Prioritizer->GetFName().GetPlainNameString()), ObjectIndex);
...
void FReplicationPrioritization::AddConnection(uint32 ConnectionId, ENetConnectionTraits NetConnectionTraits)
{
...
void FReplicationPrioritization::UpdatePrioritiesForNewAndDeletedObjects()
{
	...
	TArray<FInternalNetRefIndex> NewIndices;
	TFunction<void(FInternalNetRefIndex)> DoNothing = [](FInternalNetRefIndex ObjectIndex){};
	TFunction<void(FInternalNetRefIndex)> AddIndexAndClearFromNewPriority = [&NewIndices, this](FInternalNetRefIndex ObjectIndex)
	{
	...
	TFunction<void(FInternalNetRefIndex)> ForEachNewObject = (ConnectionCount > 0 ? AddIndexAndClearFromNewPriority : DoNothing);
	...
	if (HasNewObjectsWithStaticPriority)
	{
		...
		if (ConnectionCount > 0)
		{
			ObjectsWithNewStaticPriority.ForAllSetBits([this, &NewIndices](FInternalNetRefIndex ObjectIndex)
			{ 
				NewIndices.Add(ObjectIndex); 
			});
		}
	...
	{
		const TArrayView<FInternalNetRefIndex> ObjectIndices = MakeArrayView(NewIndices);
		for (FPerConnectionInfo& ConnectionInfo : ConnectionInfos)
		{
			if (!ConnectionInfo.IsValid)
			{
			...
			for (const FInternalNetRefIndex ObjectIndex : ObjectIndices)
			{
...
void FReplicationPrioritization::SetHighPriorityOnViewTargets(const TArrayView<float>& Priorities, const FReplicationView& ReplicationView)
{
	...
	for (FNetHandle NetHandle : ViewTargets)
	{
		...
		if (ViewTargetInternalIndex != InvalidInternalNetRefIndex)
		{
...
void FReplicationPrioritization::BatchNotifyPrioritizersOfDirtyObjects(FUpdateDirtyObjectsBatchHelper& BatchHelper, FInternalNetRefIndex* ObjectIndices, uint32 ObjectCount)
{
...
void FReplicationPrioritization::InitPrioritizers()
{
	...
	UE_CLOGF(PrioritizerInfos.Num() == 0, LogIris, Warning, "%ls", TEXT("No prioritizers have been registered. This may result in a bad gameplay experience because nearby actors will not have higher priority than actors far away."));
#endif
}
