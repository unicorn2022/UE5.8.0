...
static bool bEnableRPCSendWarnings = true;
static FAutoConsoleVariableRef CVarEnableRPCSendWarnings(TEXT("net.Iris.EnableRPCSendWarnings"), bEnableRPCSendWarnings, TEXT("Whether to warn about RPC send failures or not. Defaults to true"));
...
bool FNetBlobManager::QueueNetObjectAttachment(uint32 ConnectionId, const FNetObjectReference& TargetRef, const TRefCountPtr<FNetObjectAttachment>& Attachment, ENetObjectAttachmentSendPolicyFlags SendFlags)
{
	...
	{
		UE_LOGF(LogIris, Warning, "Dropping attachment to invalid connection %u.", ConnectionId);
		return true;
	}
	...
	if (!bCanSendRpc)
	{
		...
		{
			UE_LOGF(LogIris, Warning, "QueueNetObjectAttachment %ls Failed due to invalid Target. Unable to resolve target reference %ls.", *Attachment->GetNetObjectReference().ToString(), *TargetRef.ToString());
			return false;
		}
		...
		OwnerInfo.CallerRef = ObjectReferenceCache->GetReplicatedOuter(TargetRef, ReplicationSystem);
		...
		if (!bCanSendRpc)
		{
			UE_LOGF(LogIris, Warning, "QueueNetObjectAttachment %ls Failed due to invalid Outer. Unable to resolve outer reference %ls (index: %u) for target reference %ls (index: %u).", 
...
bool FNetBlobManager::SendMulticastRPC(const FSendRPCContext& Context, const void* Parameters, UE::Net::ENetObjectAttachmentSendPolicyFlags SendFlags)
{
	...
	if (!RPC.IsValid())
	{
		UE_LOGF(LogIris, Warning, "Unable to create RPC for function %ls.", ToCStr(Context.Function->GetName()));
		return true;
	}
...
bool FNetBlobManager::SendUnicastRPC(uint32 ConnectionId, const FSendRPCContext& Context, const void* Parameters, UE::Net::ENetObjectAttachmentSendPolicyFlags SendFlags)
{
	...
	if (!Connections->IsValidConnection(ConnectionId))
	{
		UE_LOGF(LogIris, Warning, "Trying to call RPC on non-existing connection %u.", ConnectionId);
		return true;
	}
...
bool FNetBlobManager::GetRPCOwner(FRPCOwner& OutOwnerInfo, const FSendRPCContext& Context) const
{
	...
	{
		OutOwnerInfo.TargetRef = ObjectReferenceCache->GetOrCreateObjectReference(Context.RootObject, ReplicationSystem);
		...
		if (!bCanSendRpc)
		{
			...
			UE_CLOGF(bEnableRPCSendWarnings && bLogRPCFailed, LogIris, Warning, "SendRPC %ls for %ls Failed. This root object is not yet replicated (RefHandle: %ls Index: %u).",
	...
	else
	{
		...
		{
			OutOwnerInfo.TargetRef = ObjectReferenceCache->GetOrCreateObjectReference(Context.SubObject, ReplicationSystem);
			OutOwnerInfo.CallerRef = OutOwnerInfo.TargetRef;
		...
		{
			OutOwnerInfo.TargetRef = ObjectReferenceCache->GetOrCreateObjectReference(Context.SubObject, ReplicationSystem);
			OutOwnerInfo.CallerRef = ObjectReferenceCache->GetOrCreateObjectReference(Context.RootObject, ReplicationSystem);
		...
		if (!bCanSendRpc)
		{
			...
			UE_CLOGF(bLogRPCFailed, LogIris, Warning, "SendRPC %ls for %ls::%ls Failed. The root object (RefHandle: %ls Index: %u) and subobject (RefHandle: %ls Index: %u) is not yet replicated.",
...
bool FNetBlobManager::GetRootObjectIndicesFromHandle(FNetRefHandle RootObjectRefHandle, FInternalNetRefIndex& OutRootObjectIndex) const
{
	...
	checkf(NetRefHandleManager->GetReplicatedObjectDataNoCheck(ObjectIndex).SubObjectRootIndex == InvalidInternalNetRefIndex, TEXT("Object %s (index:%u) (netref:%s) is not a rootobject"),
...
bool FNetBlobManager::GetRootObjectAndSubObjectIndicesFromSubObjectHandle(FNetRefHandle SubObjectRefHandle, FInternalNetRefIndex& OutRootObjectIndex, FInternalNetRefIndex& OutSubObjectIndex) const
{
	...
	checkf(ObjectData.SubObjectRootIndex != InvalidInternalNetRefIndex, TEXT("SubObject %s (index:%u) (netref:%s) does not have a rootobject"),
	...
	return OutRootObjectIndex != InvalidInternalNetRefIndex;
}
...
bool FNetBlobManager::GetRootObjectAndSubObjectIndicesFromAnyHandle(FNetRefHandle AnyRefHandle, FInternalNetRefIndex& OutRootObjectIndex, FInternalNetRefIndex& OutSubObjectIndex) const
{
	...
	{
		OutRootObjectIndex = ObjectIndex;
		OutSubObjectIndex = InvalidInternalNetRefIndex;
	}
...
void FNetBlobManager::FNetObjectAttachmentSendQueue::PrepareProcessQueue(FReplicationConnections* InConnections, const FNetRefHandleManager* InNetRefHandleManager)
{
	...
	if (Manager->AllowObjectReplication())
	{
		...
		{
			const uint32 TargetInternalObjectIndex = Entry.SubObjectIndex != InvalidInternalNetRefIndex ? Entry.SubObjectIndex : Entry.OwnerIndex;
...
void FNetBlobManager::FNetObjectAttachmentSendQueue::ProcessQueue(EProcessMode ProcessMode)
{
	...
	IndicesToProcess.ForAllSetBits([this, Entries, &PartialNetBlobs](uint32 Index)
	{
		...
		if (bMulticast)
		{
			...
			{
				UE_LOGF(LogIris, Warning, "Splitting multicast net object attachment %ls.", ReplicationStateDescriptor != nullptr && ReplicationStateDescriptor->DebugName ? ReplicationStateDescriptor->DebugName->Name : TEXT("Unknown"));
			}
