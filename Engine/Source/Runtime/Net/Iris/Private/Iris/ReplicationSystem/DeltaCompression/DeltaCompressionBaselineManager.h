...
private:
	...
	friend FScopedBaselineAccess;

	// For internal use by the FScopedBaselineAccess to lock DeltaCompressionBaselineManagerMutex
	void AcquireBaselineManager(const bool bWriteLock);
	// For internal use by the FScopedBaselineAccess to unlock DeltaCompressionBaselineManagerMutex
	void ReleaseBaselineManager(const bool bWriteLock);

	FTransactionallySafeRWLock DeltaCompressionBaselineManagerMutex;
...
/**
* A helper RAII class to allow thread-safe access to Baselines
* When making calls to the FDeltaCompressionBaselineManager in a parallel phase, first create FScopedBaselineAccess to
* ensure that the access is threadsafe during the lifetime of the FScopedBaselineAccess object.
*/
class FScopedBaselineAccess
{
public:
	
	explicit FScopedBaselineAccess(FDeltaCompressionBaselineManager* InBaselineManager, const bool bWriteLock);

	~FScopedBaselineAccess();

private:
	FDeltaCompressionBaselineManager* StoredBaselineManager = nullptr;
	bool bStoredWriteLockType = false;
};
...
{
	checkSlow(ObjectIndex != InvalidInternalNetRefIndex);
