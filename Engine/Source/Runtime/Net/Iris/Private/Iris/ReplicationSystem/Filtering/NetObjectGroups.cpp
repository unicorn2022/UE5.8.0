...
: CurrentEpoch((++NetObjectGroupsInternal::NextEpoch) & FNetObjectGroupHandle::EpochMask)
{
	if (CurrentEpoch == 0U)
	{
		UE_LOGF(LogIris, Warning, "FNetObjectGroups::Epoch wraparound detected.");
	}
}
...
FNetObjectGroupHandle FNetObjectGroups::CreateGroup(FName InGroupName)
{
	...
	if (!bCanCreateGroup)
	{
		UE_LOGF(LogIrisGroup, Warning, "Maximum allowed groups allocated: %u. Group: %ls cannot be created", MaxGroupCount, *InGroupName.ToString());
	...
	// Validate that the given name is unique
	{
		...
		{
			UE_LOGF(LogIrisGroup, Warning, "Tried to create a group named: %ls. But this name was already registered to groupHandle: %u", *InGroupName.ToString(), ExistingGroup.GetGroupIndex());
	...
	UE_LOGF(LogIrisGroup, Verbose, "Created GroupHandle named: %ls Index: %u Id: %u", *InGroupName.ToString(), Index, NewGroupId);
...
void FNetObjectGroups::DestroyGroup(FNetObjectGroupHandle GroupHandle)
{
	if (IsValidGroup(GroupHandle))
	{
		...
		UE_LOGF(LogIrisGroup, Verbose, "Destroyed GroupHandle named: %ls Index: %u Id: %u", *Group.GroupName.ToString(), GroupHandle.Index, Group.GroupId);
...
void FNetObjectGroups::AddToGroup(FNetObjectGroupHandle GroupHandle, FInternalNetRefIndex InternalIndex)
{
	...
	if (!Group)
	{
		UE_LOGF(LogIrisGroup, Warning, "FNetObjectGroups::AddToGroup received invalid group (Index: %u Id: %u). Cannot add %ls to it.", GroupHandle.Index, GroupHandle.UniqueId, *NetRefHandleManager->PrintObjectFromIndex(InternalIndex));
		return;
	}

	UE_LOGF(LogIrisGroup, Verbose, "FNetObjectGroups::AddToGroup Adding %ls to Group %ls (Index: %u Id: %u)",
	...
	if (AddGroupMembership(Membership, GroupHandle))
	{
		...
		{
			UE_LOGF(LogIrisGroup, Error, "FNetObjectGroups::AddGroupMembership Unexpected high num groupmemberships for group %ls (Index: %u Id: %u) NetObject %ls is member of %d groups.",
...
void FNetObjectGroups::RemoveFromGroup(FNetObjectGroupHandle GroupHandle, FInternalNetRefIndex InternalIndex)
{
	...
	if (!Group)
	{
		UE_LOGF(LogIrisGroup, Warning, "FNetObjectGroups::RemoveFromGroup received invalid group (Index: %u Id: %u). Cannot remove %ls from it.", GroupHandle.Index, GroupHandle.UniqueId, *NetRefHandleManager->PrintObjectFromIndex(InternalIndex));
		return;
	}

	UE_LOGF(LogIrisGroup, Verbose, "FNetObjectGroups::RemoveFromGroup Removing %ls from Group %ls (Index: %u Id: %u)",
		*NetRefHandleManager->PrintObjectFromIndex(InternalIndex), *GetGroupName(GroupHandle).ToString(), GroupHandle.Index, GroupHandle.UniqueId);
...
void FNetObjectGroups::AddExclusionFilterTrait(FNetObjectGroupHandle GroupHandle)
{
	if (FNetObjectGroup* Group = GetGroup(GroupHandle))
	{
		if (!IsFilterGroup(*Group))
		{
			UE_LOGF(LogIrisGroup, Verbose, "FNetObjectGroups::AddExclusionFilterTrait to Group %ls (GroupIndex: %u)", *GetGroupName(GroupHandle).ToString(), GroupHandle.GetGroupIndex());
...
void FNetObjectGroups::RemoveExclusionFilterTrait(FNetObjectGroupHandle GroupHandle)
{
	...
	UE_LOGF(LogIrisGroup, Verbose, "FNetObjectGroups::RemoveExclusionFilterTrait to Group %ls (GroupIndex: %u)", *GetGroupName(GroupHandle).ToString(), GroupHandle.GetGroupIndex());
