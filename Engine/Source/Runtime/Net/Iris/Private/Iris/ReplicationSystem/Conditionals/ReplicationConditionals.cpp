...
void FReplicationConditionals::OnMaxInternalNetRefIndexIncreased(FInternalNetRefIndex NewMaxInternalIndex)
{
	...
	{
		// Resize the ObjectConditionals list for valid connections
...
void FReplicationConditionals::MarkLifeTimeConditionalsDirtyForObjectsInGroup(FNetObjectGroupHandle GroupHandle)
{
	...
	{
		UE_LOGF(LogIris, Warning, "FReplicationConditionals::MarkLifeTimeConditionalsDirtyForObjectsInGroup - Marking reserved group dirty is not allowed. GroupIndex: %u which is not allowed.", GroupIndex);
		return;
	}
...
bool FReplicationConditionals::SetConditionConnectionFilter(FInternalNetRefIndex ObjectIndex, EReplicationCondition Condition, uint32 ConnectionId, bool bEnable)
{
	...
	{
		UE_LOGF(LogIris, Error, "Only EReplicationCondition::RoleAutonomous supports connection filtering, got '%u'.", uint32(Condition));
		return false;
	}
	...
	{
		UE_LOGF(LogIrisConditionals, Verbose, "SetConditionConnectionFilter %ls. AutonomousConnectionId: %u", *NetRefHandleManager->PrintObjectFromIndex(ObjectIndex), AutonomousConnectionId);
...
void FReplicationConditionals::SetOwningConnection(FInternalNetRefIndex ObjectIndex, uint32 OwningConnectionId)
{
	...
	{
		UE_LOGF(LogIrisConditionals, Verbose, "SetOwningConnection on object %u. Connection: %u", ObjectIndex, OwningConnectionId);
...
void FReplicationConditionals::AddConnection(uint32 ConnectionId, ENetConnectionTraits NetConnectionTraits)
{
	...
	ConnectionInfo.NetConnectionTraits = NetConnectionTraits;
}
...
void FReplicationConditionals::RemoveConnection(uint32 ConnectionId)
{
	...
	ConnectionInfo.NetConnectionTraits = ENetConnectionTraits::None;
}

void FReplicationConditionals::SetAutonomousAsSimulated(uint32 ConnectionId, bool bEnabled)
{
	if (ConnectionInfos.IsValidIndex(ConnectionId))
	{
		FPerConnectionInfo& ConnectionInfo = ConnectionInfos[ConnectionId];
		ConnectionInfo.bAutonomousAsSimulated = bEnabled;
	}
	else
	{
		UE_LOGF(LogIris, Warning, "Attempting to set AutonomousAsSimulated on an invalid connection id %d", ConnectionId);
	}
}
...
bool FReplicationConditionals::SetCondition(FInternalNetRefIndex ObjectIndex, EReplicationCondition Condition, bool bEnable)
{
	if (!ensure(Condition != EReplicationCondition::RoleAutonomous))
	{
		UE_LOGF(LogIris, Error, "%ls", TEXT("EReplicationCondition::RoleAutonomous requires a connection."));
		return false;
	}
	...
	if (Condition == EReplicationCondition::ReplicatePhysics)
	{
		...
		{
			UE_LOGF(LogIrisConditionals, Verbose, "SetCondition object %ls. EReplicationCondition::ReplicatePhysics: %u", *NetRefHandleManager->PrintObjectFromIndex(ObjectIndex), bEnable ? 1U : 0U);
...
bool FReplicationConditionals::SetPropertyCustomCondition(FInternalNetRefIndex ObjectIndex, const void* Owner, uint16 RepIndex, bool bIsActive)
{
	...
	if (Protocol->LifetimeConditionalsStateCount == 1U)
	{
		...
		{
			UE_LOGF(LogIris, Warning, "Trying to change non-existing custom conditional for RepIndex %u in protocol %ls", RepIndex, ToCStr(Protocol->DebugName));
			return false;
		}
...
bool FReplicationConditionals::SetPropertyDynamicCondition(FInternalNetRefIndex ObjectIndex, const void* Owner, uint16 RepIndex, ELifetimeCondition Condition)
{
	...
	if (Protocol->LifetimeConditionalsStateCount == 1U)
	{
		...
		if (RepIndex >= StateDescriptor->RepIndexCount)
		{
			UE_LOGF(LogIris, Warning, "Trying to change non-existing dynamic conditional for RepIndex %u in protocol %ls", RepIndex, ToCStr(Protocol->DebugName));
			return false;
		}
		...
		{
			UE_LOGF(LogIris, Warning, "Trying to change condition for member %ls with wrong condition in protocol %ls", ToCStr(StateDescriptor->MemberDebugDescriptors[MemberIndex].DebugName), ToCStr(Protocol->DebugName));
			return false;
		}
...
void FReplicationConditionals::GetChildSubObjectsToReplicate(uint32 ReplicatingConnectionId, const FConditionalsMask& LifetimeConditionals,  const FInternalNetRefIndex ParentObjectIndex, const FChildSubObjectsInfo& SubObjectsInfo, FSubObjectsToReplicateArray& OutSubObjectsToReplicate) const
{
	if (SubObjectsInfo.SubObjectLifeTimeConditions == nullptr)
	{
		for (uint32 ArrayIndex = 0; ArrayIndex < SubObjectsInfo.NumSubObjects; ++ArrayIndex)
		{
			...
			FChildSubObjectsInfo ChildSubObjectsInfo;
			if (NetRefHandleManager->GetChildSubObjects(SubObjectIndex, ChildSubObjectsInfo))
			{
				// If SubObject has ChildSubObjects we need to consider replication order.
				if (ChildSubObjectsInfo.bReplicateChildSubObjectsAfterParent)
				{
					OutSubObjectsToReplicate.Add(SubObjectIndex);
					GetChildSubObjectsToReplicate(ReplicatingConnectionId, LifetimeConditionals, SubObjectIndex, ChildSubObjectsInfo, OutSubObjectsToReplicate);
				}
				else
				{
					GetChildSubObjectsToReplicate(ReplicatingConnectionId, LifetimeConditionals, SubObjectIndex, ChildSubObjectsInfo, OutSubObjectsToReplicate);
	...
	else
	{
		...
		for (uint32 ArrayIndex = 0; ArrayIndex < SubObjectsInfo.NumSubObjects; ++ArrayIndex)
		{
			...
			if (LifeTimeCondition == COND_NetGroup)
			{
				...
				UE_CLOGF(!bShouldReplicateSubObject, LogIrisConditionals, VeryVerbose, "%ls Filtered out by COND_NetGroup", *NetRefHandleManager->PrintObjectFromIndex(SubObjectIndex));
			}
			...
			{
				UE_LOGF(LogIrisConditionals, VeryVerbose, "%ls Filtered out by %ls", *NetRefHandleManager->PrintObjectFromIndex(SubObjectIndex), *UEnum::GetValueAsString(LifeTimeCondition));
			}
		}
	}
}
...
void FReplicationConditionals::GetSubObjectsToReplicate(uint32 ReplicationConnectionId, FInternalNetRefIndex RootObjectIndex, FSubObjectsToReplicateArray& OutSubObjectsToReplicate)
{
	...
	if (NetRefHandleManager->GetChildSubObjects(RootObjectIndex, SubObjectsInfo))
	{
		GetChildSubObjectsToReplicate(ReplicationConnectionId, LifetimeConditionals, RootObjectIndex, SubObjectsInfo, OutSubObjectsToReplicate);
	}
}
		...
		if (Protocol->LifetimeConditionalsStateCount == 1U)
		{
			...
			for (uint32 MemberIt = 0U, MemberEndIt = StateDescriptor->MemberCount; MemberIt != MemberEndIt; ++MemberIt)
			{
				...
				if (LifetimeConditionals.IsConditionEnabled(Condition))
				{
					...
					{
						UE_LOGF(LogIrisConditionals, Verbose, "Dirtying member %ls %ls:%ls due to condition %ls", *NetRefHandleManager->PrintObjectFromIndex(ObjectIndex), ToCStr(StateDescriptor->DebugName), ToCStr(StateDescriptor->MemberDebugDescriptors[MemberIt].DebugName), *UEnum::GetValueAsString(Condition));
				...
				else
				{
					...
					{
						UE_LOGF(LogIrisConditionals, VeryVerbose, "Filtering out member %ls %ls:%ls due to condition %ls", *NetRefHandleManager->PrintObjectFromIndex(ObjectIndex), ToCStr(StateDescriptor->DebugName), ToCStr(StateDescriptor->MemberDebugDescriptors[MemberIt].DebugName), *UEnum::GetValueAsString(Condition));
...
FReplicationConditionals::FConditionalsMask FReplicationConditionals::GetLifetimeConditionals(uint32 ReplicatingConnectionId, FInternalNetRefIndex ParentObjectIndex, bool bIsInitialState) const
{
	...
	bool bRoleSimulated = ReplicatingConnectionId != ObjectInfo->AutonomousConnectionId;
	...
	const bool bIsReplay = EnumHasAnyFlags(ConnectionInfos[ReplicatingConnectionId].NetConnectionTraits, ENetConnectionTraits::ReplayPlayback | ENetConnectionTraits::ReplayRecording);
	if (ConnectionInfos.IsValidIndex(ObjectOwnerConnectionId))
	{
		const FPerConnectionInfo& ConnectionInfo = ConnectionInfos[ObjectOwnerConnectionId];
		if (bRoleAutonomous && ConnectionInfo.bAutonomousAsSimulated)
		{
			bRoleSimulated = true;
		}
	}
	...
	ConditionalsMask.SetConditionEnabled(COND_ReplayOrOwner, bIsReplicatingToOwner | bIsReplay);
	ConditionalsMask.SetConditionEnabled(COND_ReplayOnly, bIsReplay);
	ConditionalsMask.SetConditionEnabled(COND_SimulatedOnlyNoReplay, bRoleSimulated & !bIsReplay);
	ConditionalsMask.SetConditionEnabled(COND_SimulatedOrPhysicsNoReplay, (bRoleSimulated | bRepPhysics) & !bIsReplay);
	ConditionalsMask.SetConditionEnabled(COND_SkipReplay, !bIsReplay);
	ConditionalsMask.SetConditionEnabled(COND_Dynamic, true);
	//ConditionalsMask.SetConditionEnabled(COND_Never, false); Ctor sets its false by default
	//ConditionalsMask.SetConditionEnabled(COND_NetGroup, Not Known); NetGroup is evaluated in FReplicationConditionals::GetChildSubObjectsToReplicate
...
void FReplicationConditionals::MarkPropertyDirty(FInternalNetRefIndex ObjectIndex, uint16 RepIndex)
{
	...
	UE_LOGF(LogIris, Warning, "Trying to mark non-existing property with RepIndex %u in protocol %ls as dirty", RepIndex, ToCStr(Protocol->DebugName));
}
