...
static_assert(!UE::Net::ENetConnectionTraits::None, "UE::Net::ENetConnectionTraits::None must be zero to avoid header include in ReplicationSystem.h");
...
public:
	...
	FNetObjectGroupHandle AsyncStopReplicationNetObjectGroupHandle;
	...
	inline static const FName MetricNameTotalRootObjects = "TotalRootObjects";
	inline static const FName MetricNameTotalSubObjects = "TotalSubObjects";
	inline static const FName MetricNameAvgPendingObjectCount = "AvgPendingObjectCount";
	inline static const FName MetricNameMaxPendingObjectCount = "MaxPendingObjectCount";
	inline static const FName MetricNameAvgPendingDependentObjectCount= "AvgPendingDependentObjectCount";
	inline static const FName MetricNameMaxPendingDependentObjectCount = "MaxPendingDependentObjectCount";
	inline static const FName MetricNameAvgHugeObjectSendQueue = "AvgHugeObjectSendQueue";
	inline static const FName MetricNameMaxHugeObjectSendQueue = "MaxHugeObjectSendQueue";
	...
	, ReplicationSystemInternal(
		...
		{ 
			...
			.bUseSharedObjectReferenceCache = (Params.ProxyType == UE::Net::EProxyType::Backend)
		}))
	{
		const bool bIsProxyBackend = (Params.ProxyType == UE::Net::EProxyType::Backend);
		ensureMsgf(!bIsProxyBackend || (bIsProxyBackend && !InReplicationSystem->IsServer()), TEXT("A replication system configured as a proxy backend is expected to be a client connecting to a remote server."));
	}
	...
	{
		NotReplicatedNetObjectGroupHandle = ReplicationSystem->CreateGroup(FName("NotReplicated"));
		...
		AsyncStopReplicationNetObjectGroupHandle = ReplicationSystem->CreateGroup(FName("AsyncStopReplication"));
		check(AsyncStopReplicationNetObjectGroupHandle.IsAsyncStopReplicationNetObjectGroup());
		ReplicationSystem->AddExclusionFilterGroup(AsyncStopReplicationNetObjectGroupHandle);
	...
	void Init(const UReplicationSystem::FReplicationSystemParams& Params)
	{
		...
		FNetRefHandleManager& NetRefHandleManager = ReplicationSystemInternal.GetNetRefHandleManager();
		{
			...
			NetRefHandleManagerInitParams.InitialNetRefHandleIndex = Params.InitialNetRefHandleIndex;
		...
		{
			FObjectReferenceCacheParams InitParams;
			InitParams.bAuthority = ReplicationSystem->IsServer();

			const bool bIsUsingSharedObjectReferenceCache = &ReplicationSystemInternal.GetObjectReferenceCache() == FReplicationSystemFactory::GetSharedObjectReferenceCache().Get();
			if (!bIsUsingSharedObjectReferenceCache)
			{
				InitParams.ObjectToNetRefHandleSerial = Params.ObjectToNetRefHandleSerial;
				InitParams.NetRefHandleSerialToObject = Params.NetRefHandleSerialToObject;
			}
			else
			{
				const bool bDelegatesAreBound = Params.ObjectToNetRefHandleSerial.IsBound() || Params.NetRefHandleSerialToObject.IsBound();
				check(!bDelegatesAreBound);
				UE_CLOGF(bDelegatesAreBound, LogIris, Error, "Cannot bind FNetRefHandle mapping delegates when using a shared object reference cache.");
			}

			ReplicationSystemInternal.GetObjectReferenceCache().Init(InitParams);
		}
	...
	void OnInternalNetRefIndicesFreed(const TConstArrayView<FInternalNetRefIndex>& FreedIndices)
	{
		...
		ReplicationSystemInternal.GetObjectReferenceCache().OnInternalNetRefIndicesFreed(FreedIndices);
	}
	...
	void ExecuteQuantizeDirtyStateData(bool bTransact)
	{
#if UE_WITH_REMOTE_OBJECT_HANDLE && UE_AUTORTFM
		if (bTransact && !AutoRTFM::IsTransactional())
		{
			static FName TransactionalWorkName("Iris FReplicationSystemImpl::QuantizeDirtyStateData");
	...
	void ResetObjectStateDirtiness()
	{
		...
		// Reset the poll list.
		NetRefHandleManager.GetPolledObjectsInternalIndices().ClearAllBits();
	}
	...
	TWeakObjectPtr<UNetObjectFilter> CreateFilter(const FNetObjectFilterDefinition& Definition)
	{
		if (!ensure(!ReplicationSystemInternal.AreFilterChangesBlocked()))
		{
			UE_LOGF(
				LogIris,
				Error,
				"CreateFilter() called while filter changes are blocked; filter %ls not created",
				*Definition.FilterName.ToString());

			return nullptr;
		}

		return ReplicationSystemInternal.GetFiltering().CreateFilter(Definition);
	}

	void DestroyFilter(const FName Name)
	{
		if (ReplicationSystemInternal.AreFilterChangesBlocked())
		{
			UE_LOGF(LogIris, Warning, "DestroyFilter() called while filter changes are blocked; filter %ls not destroyed", *Name.ToString());
			return;
		}

		ReplicationSystemInternal.GetFiltering().DestroyFilter(Name);
	}

	void AddConnection(uint32 ConnectionId, UE::Net::ENetConnectionTraits NetConnectionTraits)
	{
		...
		{
			UE_LOGF(LogIris, Error, "UReplicationSystem::AddConnection called with a bad connection Id: %u. Max connection count: %u.", ConnectionId, Connections.GetMaxConnectionCount());
			return;
		}
		...
		{
			UE_LOGF(LogIris, Error, "UReplicationSystem::AddConnection called with already added connection Id: %u.", ConnectionId);
			return;
		}

		{
			Connections.AddConnection(ConnectionId, NetConnectionTraits);
			...
			// $IRIS TODO Replay connection specifics 
		...
		{
			FReplicationConditionals& ReplicationConditionals = ReplicationSystemInternal.GetConditionals();
			ReplicationConditionals.AddConnection(ConnectionId, NetConnectionTraits);
		}

		{
			FReplicationFiltering& ReplicationFiltering = ReplicationSystemInternal.GetFiltering();
			ReplicationFiltering.AddConnection(ConnectionId, NetConnectionTraits);
		}

		{
			FReplicationPrioritization& ReplicationPrioritization = ReplicationSystemInternal.GetPrioritization();
			ReplicationPrioritization.AddConnection(ConnectionId, NetConnectionTraits);
		}

		{
			FDeltaCompressionBaselineManager& DC = ReplicationSystemInternal.GetDeltaCompressionBaselineManager();
			DC.AddConnection(ConnectionId, NetConnectionTraits);
		}
	...
	void RemoveConnection(uint32 ConnectionId)
	{
		...
		if (!Connections.IsValidConnection(ConnectionId))
		{
			UE_LOGF(LogIris, Error, "UReplicationSystem::RemoveConnection called for connection ID that isn't added: %u.", ConnectionId);
		}
		...
		{
			FObjectReferenceCache& ObjectReferenceCache = ReplicationSystemInternal.GetObjectReferenceCache();
			ObjectReferenceCache.RemoveConnection(ReplicationSystem, ConnectionId);
		}
	...
	bool HasConnectionWithAnyTraits(ENetConnectionTraits NetConnectionTraits) const
	{
		const FReplicationConnections& Connections = ReplicationSystemInternal.GetConnections();
		return Connections.HasConnectionWithAnyTraits(NetConnectionTraits);
	}
	...
	void UpdateUnresolvableReferenceTracking()
	{
		...
		UE_NET_TRACE_FRAME_STATSCOUNTER(ReplicationSystem->GetId(), ObjectReferenceCache.GetNumObjectsWithTrackedSubObjectHandles, ReplicationSystemInternal.GetObjectReferenceCache().GetNumObjectsWithTrackedSubObjectHandles(), ENetTraceVerbosity::Verbose);	
...
void FReplicationSystemInternal::RemoveFromAllGroups(FInternalNetRefIndex ObjectInternalIndex)
{
	uint32 NumGroupMemberShips = 0;
	if (AreFilterChangesBlocked())
	{
		ensureMsgf(false, TEXT("Setting filter conditions is not yet supported during this operation. Filter condition on %s failed."), *NetRefHandleManager.PrintObjectFromIndex(ObjectInternalIndex));
		return;
	}
...
void UReplicationSystem::Init(uint32 InId, const FReplicationSystemParams& Params)
{
	...
	bAllowExecuteRPC = Params.bAllowExecuteRPC;
	bAllowExecuteRepNotify = Params.bAllowExecuteRepNotifies;
	bAlwaysSwapRolesOnReplication = Params.bAlwaysSwapRolesOnReplication;
	bReplicateTransactionally = Params.bReplicateTransactionally;
	ProxyType = Params.ProxyType;

	ReplicationBridge = Params.ReplicationBridge;

	FReplicationSystemParams LocalParams = Params;

	if (bIsServer)
	{
		InitializeNetRefHandleGenerator(LocalParams);
	}
	else
	{
		LocalParams.ObjectToNetRefHandleSerial.Unbind();
		LocalParams.NetRefHandleSerialToObject.Unbind();
	}

	Impl = MakePimpl<UE::Net::Private::FReplicationSystemImpl>(this, LocalParams);
	Impl->Init(LocalParams);
...
void UReplicationSystem::StartParallelPhase()
{ 
	GetReplicationSystemInternal()->StartParallelPhase();
}

void UReplicationSystem::StopParallelPhase()
{
	GetReplicationSystemInternal()->StopParallelPhase(); 
}

bool UReplicationSystem::IsInParallelPhase() const
{
	return GetReplicationSystemInternal()->IsInParallelPhase();
}

UE::Net::Private::FNetStatsContext* UReplicationSystem::AcquireStatsForSerializationContext()
{
	const bool bIsParallel = IsInParallelPhase();
	check(GetReplicationSystemInternal());
	UE::Net::Private::FNetTypeStats& NetTypeStats = GetReplicationSystemInternal()->GetNetTypeStats();
	UE::Net::Private::FNetStatsContext* NetStatsContext = bIsParallel ? NetTypeStats.AcquireChildNetStatsContext() : NetTypeStats.GetNetStatsContext();
	return NetStatsContext;
}

void UReplicationSystem::ReleaseStatsForSerializationContext(UE::Net::Private::FNetStatsContext* NetStatsContext)
{
	const bool bIsParallel = IsInParallelPhase();
	check(GetReplicationSystemInternal());
	UE::Net::Private::FNetTypeStats& NetTypeStats = GetReplicationSystemInternal()->GetNetTypeStats();
	if (bIsParallel && ensure(NetStatsContext))
	{
		NetTypeStats.ReleaseChildNetStatsContext(NetStatsContext);
	}
}
...
{
	Impl->ReplicationSystemInternal.GetReplicationBridge()->NotifyStreamingContainerUnload(Level);
}

void UReplicationSystem::PreSeamlessTravelGarbageCollect()
{
	UE_LOGF(LogIris, Verbose, "UReplicationSystem::PreSeamlessTravelGarbageCollect IsServer: %d", IsServer());
	Impl->ReplicationSystemInternal.PreSeamlessTravelGarbageCollect();
}

void UReplicationSystem::PostSeamlessTravelGarbageCollect()
{
	UE_LOGF(LogIris, Verbose, "UReplicationSystem::PostSeamlessTravelGarbageCollect IsServer: %d", IsServer());
	CollectGarbage();
	Impl->ReplicationSystemInternal.PostSeamlessTravelGarbageCollect();
}

void UReplicationSystem::InitializeNetRefHandleGenerator(UReplicationSystem::FReplicationSystemParams& Params)
{	
	using namespace UE::Net;

	if (Params.ProxyType != UE::Net::EProxyType::None)
	{
		return;
	}

	const UClass* GeneratorClass = UObjectReplicationBridgeConfig::GetConfig()->GetNetRefHandleGeneratorClass();
	if (!GeneratorClass)
	{
		return;
	}

	if (!GeneratorClass->IsChildOf<UNetRefHandleGenerator>())
	{
		return;
	}

	NetRefHandleGenerator = NewObject<UNetRefHandleGenerator>(this, GeneratorClass);
	if (!NetRefHandleGenerator)
	{
		UE_LOGF(LogIris, Error, "UReplicationSystem::InitializeNetRefHandleGenerator: Unable to instantiate class %ls", *GeneratorClass->GetName());
		return;
	}

	UE_LOGF(LogIris, Log, "UReplicationSystem::InitializeNetRefHandleGenerator: Using generator %ls", *GeneratorClass->GetName());

	UE::Net::FNetRefHandleGeneratorBindParams BindParams;
	BindParams.ObjectToNetRefHandleSerial = &Params.ObjectToNetRefHandleSerial;
	BindParams.NetRefHandleSerialToObject = &Params.NetRefHandleSerialToObject;

	UE_CLOGF(BindParams.ObjectToNetRefHandleSerial->IsBound(), LogIris, Warning, "UReplicationSystem::InitializeNetRefHandleGenerator: ObjectToNetRefHandleSerial is already set and will be overwritten.");
	UE_CLOGF(BindParams.NetRefHandleSerialToObject->IsBound(), LogIris, Warning, "UReplicationSystem::InitializeNetRefHandleGenerator: NetRefHandleSerialToObject is already set and will be overwritten.");

	NetRefHandleGenerator->Bind(BindParams);
}
...
UDataStream* UReplicationSystem::OpenDataStream(uint32 ConnectionId, FName Name)
{
	if (!IsServer())
	{
		UE_LOGF(LogIris, Error, "DataStream %ls can only be opened from server", *Name.ToString());
		return nullptr;
	}
...
bool UReplicationSystem::IsRelevantForConnection(FNetRefHandle Handle, uint32 ConnectionId) const
{
	using namespace UE::Net;
	using namespace UE::Net::Private;

	const FReplicationConnections& Connections = Impl->ReplicationSystemInternal.GetConnections();
	if (!ensureMsgf(Connections.IsValidConnection(ConnectionId), TEXT("Invalid ConnectionId %u passed to UReplicationSystem::IsRelevantForConnection."), ConnectionId))
	{
		return false;
	}
	...
	const FReplicationConnection* Connection = Connections.GetConnection(ConnectionId);

	return ObjectsInScope.GetNumBits() > ObjectInternalIndex ? (ObjectsInScope.IsBitSet(ObjectInternalIndex) && Connection->ReplicationWriter->IsInReplicatingState(ObjectInternalIndex)) : false;
}
...
bool UReplicationSystem::SendRPC(uint32 ConnectionId, const UObject* RootObject, const UObject* SubObject, const UFunction* Function, const void* Parameters)
{
	...
	FNetBlobManager::FSendRPCContext RPCContext = { .RootObject = RootObject, .SubObject = SubObject, .Function = Function };
	FNetBlobManager& NetBlobManager = Impl->ReplicationSystemInternal.GetNetBlobManager();
	return NetBlobManager.SendUnicastRPC(ConnectionId, RPCContext, Parameters, SendFlags);
}
...
bool UReplicationSystem::SetRPCSendPolicyFlags(const UFunction* Function, UE::Net::ENetObjectAttachmentSendPolicyFlags SendFlags)
{
	...
	UE_LOGF(LogIris, Verbose, "SetRPCSendPolicyFlags %ls::%ls => %ls ", *GetNameSafe(Function->GetOuterUClass()), *GetNameSafe(Function), LexToString(SendFlags));
...
void UReplicationSystem::SetConnectionRPCDoS(uint32 ConnectionId, FRPCDoSDetection* InRPCDoS)
{
	UE::Net::Private::FReplicationConnections& Connections = Impl->ReplicationSystemInternal.GetConnections();
	if (!ensureMsgf(Connections.IsValidConnection(ConnectionId), TEXT("Invalid ConnectionId %u passed to UReplicationSystem::SetConnectionRPCDoS."), ConnectionId))
	{
		return;
	}
	...
	if (UE::Net::Private::FReplicationConnection* Connection = Connections.GetConnection(ConnectionId))
	{
		if (UE::Net::Private::FReplicationReader* Reader = Connection->ReplicationReader)
		{
			Reader->SetRPCDoS(InRPCDoS);
		}
	}
}
...
TWeakObjectPtr<UNetObjectFilter> UReplicationSystem::CreateFilter(const FNetObjectFilterDefinition& Definition)
{
	return Impl->CreateFilter(Definition);
}

void UReplicationSystem::DestroyFilter(const FName Name)
{
	Impl->DestroyFilter(Name);
}
...
void UReplicationSystem::RemoveFromAllGroups(FNetRefHandle Handle)
{
	...
	Impl->ReplicationSystemInternal.RemoveFromAllGroups(ObjectInternalIndex);
}
...
UE::Net::FNetObjectGroupHandle UReplicationSystem::GetAsyncStopReplicationNetObjectGroup() const
{
	return Impl->AsyncStopReplicationNetObjectGroupHandle;
}
...
void UReplicationSystem::SetProxyConnection(uint32 ConnectionId, bool bEnabled)
{
	Impl->ReplicationSystemInternal.GetConditionals().SetAutonomousAsSimulated(ConnectionId, bEnabled);
}
...
void UReplicationSystem::SetIsDebugObject(FNetRefHandle Handle)
{
#if UE_NET_IRIS_ALLOW_DEBUG_DATA
	UE::Net::Private::FNetRefHandleManager& NetRefHandleManager = Impl->ReplicationSystemInternal.GetNetRefHandleManager();	
	NetRefHandleManager.SetIsDebugObject(Handle);
#else
	ensureMsgf(false, TEXT("SetIsDebugObject is only supported in non-shipping builds."));
#endif
}
...
void UReplicationSystem::TearOffNextUpdate(FNetRefHandle Handle)
{
	...
	{
		Impl->ReplicationSystemInternal.GetReplicationBridge()->AddPendingEndReplication(Handle, InternalObjectIndex, DestroyFlags);
	}
}
...
void UReplicationSystem::AddReferencedObjects(UObject* InThis, FReferenceCollector& Collector)
{
	...
	if (This->Impl.IsValid())
	{
		...
		This->Impl->ReplicationSystemInternal.GetObjectReferenceCache().AddReferencedObjects(Collector, This);
	}
...
void UReplicationSystem::SetCullDistanceOverride(FNetRefHandle Handle, float CullDistance)
{
	...
	UE_LOGF(LogIrisNetCull, Verbose, "UReplicationSystem::SetCullDistanceOverride: %ls will be overridden to %f. Previous cull distance: %f",
...
void UReplicationSystem::ClearCullDistanceOverride(FNetRefHandle Handle)
{
	...
	UE_CLOGF(bWasCullDistanceOverriden, LogIrisNetCull, Verbose, "UReplicationSystem::ClearCullDistanceOverride: %ls is no longer overridden. Now using cull distance: %f",
...
void UReplicationSystem::ReportProtocolMismatch(uint64 NetRefHandleId, uint32 ConnectionId, const TArray<uint8>& ClientCDOStateBytes)
{
	using namespace UE::Net::Private;
	const FNetRefHandle NetRefHandle = FNetRefHandleManager::MakeNetRefHandle(NetRefHandleId, GetId());

	Impl->ReplicationSystemInternal.GetReplicationBridge()->OnProtocolMismatchReported(NetRefHandle, ConnectionId, ClientCDOStateBytes);
}

void UReplicationSystem::ReportErrorWithNetRefHandle(UE::Net::ENetRefHandleError ErrorType, uint64 NetRefHandleId, uint32 ConnectionId, const TArray<uint64>& ExtraNetRefHandles, const FString& DiagnosticMessage)
{
	using namespace UE::Net::Private;
	const FNetRefHandle NetRefHandle = FNetRefHandleManager::MakeNetRefHandle(NetRefHandleId, GetId());
	...
	Impl->ReplicationSystemInternal.GetReplicationBridge()->OnErrorWithNetRefHandleReported(ErrorType, NetRefHandle, ConnectionId, ExtraHandles, DiagnosticMessage);
}
...
TSharedPtr<UE::Net::Private::FObjectReferenceCache> FReplicationSystemFactory::SharedObjectReferenceCache;
...
TSharedPtr<UE::Net::Private::FObjectReferenceCache> FReplicationSystemFactory::GetSharedObjectReferenceCache()
{
	if (!SharedObjectReferenceCache)
	{
		SharedObjectReferenceCache = MakeShared<UE::Net::Private::FObjectReferenceCache>();

		// It's assumed that the shared object reference cache will be used by client replication systems.
		UE::Net::Private::FObjectReferenceCacheParams InitParams;
		SharedObjectReferenceCache->Init(InitParams);
	}

	return SharedObjectReferenceCache;
}
...
UReplicationSystem* FReplicationSystemFactory::CreateReplicationSystem(const UReplicationSystem::FReplicationSystemParams& Params)
{
	...
	{
		UE_LOGF(LogIris, Error, "Cannot create ReplicationSystem without a ReplicationBridge");
		return nullptr;
	}
	...
	if (static_cast<uint32>(ReplicationSystems.Num()) < FNetRefHandle::MaxReplicationSystemCount)
	{
		...
		UE_LOGF(LogIris, Display, "Iris ReplicationSystem[%i]: %ls (0x%p) is created", ReplicationSystemId, *ReplicationSystem->GetName(), ReplicationSystem);
	...
	LowLevelFatalError(TEXT("Too many ReplicationSystems have already been created (%llu)"), FNetRefHandle::MaxReplicationSystemCount);
	return nullptr;
}
...
void FReplicationSystemFactory::DestroyReplicationSystem(UReplicationSystem* System)
{
	...
	UE_LOGF(LogIris, Display, "Iris ReplicationSystem[%i]: %ls (0x%p) is about to be destroyed", Id, *System->GetName(), System);
	...
	if (SharedObjectReferenceCache.IsValid())
	{
		SharedObjectReferenceCache->ReplicationSystemDestroyed(System);
	}
	...
	// If this is the last replication system, reset the shared object reference cache.
	if (ReplicationSystems.IsEmpty())
	{
		SharedObjectReferenceCache.Reset();
	}
}
...
FScopedNetStatsContext::FScopedNetStatsContext(UReplicationSystem* const InReplicationSystem)
{
	if (ensure(InReplicationSystem))
	{
		StoredNetStatsContext = InReplicationSystem->AcquireStatsForSerializationContext();
		StoredReplicationSystem = InReplicationSystem;
		bWasAcquiredInParallelPhase = InReplicationSystem->IsInParallelPhase();
	}
}

FScopedNetStatsContext::~FScopedNetStatsContext()
{
	if (ensure(StoredReplicationSystem))
	{
		ensureMsgf(bWasAcquiredInParallelPhase == StoredReplicationSystem->IsInParallelPhase(), TEXT("ScopedNetStatsContext lifetime mismatch - %s"), bWasAcquiredInParallelPhase ? TEXT("Expected to be destroyed in ParallelPhase") : TEXT("Expected to be destroyed outside of ParallelPhase"));
	
		StoredReplicationSystem->ReleaseStatsForSerializationContext(StoredNetStatsContext);
		StoredNetStatsContext = nullptr;
	}
}

Private::FNetStatsContext* FScopedNetStatsContext::GetRawPtr()
{
	return StoredNetStatsContext;
}
