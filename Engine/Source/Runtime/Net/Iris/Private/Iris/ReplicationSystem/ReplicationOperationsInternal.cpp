...
uint32 FReplicationInstanceOperationsInternal::QuantizeObjectStateData(FNetBitStreamWriter& ChangeMaskWriter, FChangeMaskCache& Cache, FNetRefHandleManager& NetRefHandleManager, FNetSerializationContext& SerializationContext, uint32 InternalIndex)
{
	if (NetRefHandleManager.IsScopableIndex(InternalIndex))
	{
		...
		if (Object.InstanceProtocol && Object.Protocol->InternalTotalSize > 0U)
		{
			...
			// Quantize dirty state
			{
				...
				//UE_LOGF(LogIris, Log, "Copying state data for ( InternalIndex: %u ) with NetRefHandle (Id=%u)", InternalIndex, Object.RefHandle.GetId());
			...
			if (const uint32 SubObjectOwnerIndex = Object.SubObjectRootIndex)
			{
				...
				if (bIsOwnerScopable)
				{
					...
					{
						//UE_LOGF(LogIris, Log, "Marking SubObjectOwner( InternalIndex: %u ) as dirty for ( InternalIndex: %u ) with NetRefHandle (Id=%u)", SubObjectOwnerIndex, InternalIndex, Object.RefHandle.GetId());
		...
		else if (const uint32 SubObjectOwnerIndex = Object.SubObjectRootIndex)
		{
			...
			{	
				...
				{
					//UE_LOGF(LogIris, Log, "Marking SubObjectOwner( InternalIndex: %u ) as dirty for ( InternalIndex: %u ) with NetRefHandle (Id=%u)", SubObjectOwnerIndex, InternalIndex, Object.Handle.GetIndex());
					Cache.AddSubObjectOwnerDirty(SubObjectOwnerIndex);
				}
			}			
		}
	...
	else
	{
		...
		UE_LOGF(LogIris, Warning, "CopyObjectStateData called on object ( InternalIndex: %u ) not in scope", InternalIndex);
	}
