...
void FPartialNetBlob::SplitPayload(const FPartialNetBlob::FPayloadSplitParams& SplitParams, TArray<TRefCountPtr<FNetBlob>>& OutPartialBlobs)
{
	...
	// We do not currently run SplitPayload in a closed AutoRTFM scope, but it is acceptable for the sequence number to
	// increase and never roll back, so this makes it safe to access this atomic in a closed scope in the future
	UE_AUTORTFM_OPEN
	{
		SequenceNumber = Private::PartialNetBlobGlobalSequenceNumber;
