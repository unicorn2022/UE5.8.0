...
FNetTokenDataStore::FNetTokenStoreKey FNetTokenDataStore::GetTokenKey(FNetToken Token, const FNetTokenStoreState& TokenStoreState) const
{
	...
	{
		UE_LOGF(LogNetToken, Error, "FNetTokenDataStore::GetTokenKey Invalid tokentype %ls StoreTypeId: %u", *Token.ToString(), GetTypeId());
	...
	if (const bool bIsValid = (TokenIndex != FNetToken::InvalidTokenIndex))
	{
		...
		if (!Context.HasErrorOrOverflow())
		{
			...
			if (!ReadToken.IsValid())
			{
				...
				UE_LOGF(LogNetToken, Error, "Expected a valid token but got an invalid one. TokenTypeId: %u TokenIndex: %u.", TokenTypeId, TokenIndex);
				ensure(false);
			}
		}
	}
...
void FNetTokenDataStore::WriteNetToken(UE::Net::FNetSerializationContext& Context, FNetToken Token) const
{
...
bool FNetTokenStore::RegisterDataStore(TUniquePtr<FNetTokenDataStore> DataStore, FName TokenStoreName)
{
	...
	{
		UE_LOGF(LogNetToken, Error, "No TypeID information found in [/Script/IrisCore.NetTokenTypeIdConfig] Engine.ini for %ls", *TokenStoreName.ToString());
		return false;
	}

	if (!ensure(TypeId != FNetToken::InvalidTokenTypeId && TypeId < FNetToken::MaxTypeIdCount))
	{
		UE_LOGF(LogNetToken, Error, "Invalid TypeID information found for %ls", *TokenStoreName.ToString());
		return false;
	}
	...
	{
		UE_LOGF(LogNetToken, Error, "An existing TokenDataStore (Name %ls) has been created with that TypeId when Trying to add New TokenDataStore with name: %ls",*ExistingName.ToString(),*TokenStoreName.ToString());
		return false;
	}
...
void FNetTokenStore::WriteTokenData(FNetSerializationContext& Context, const FNetToken NetToken) const
{
	if (NetToken.IsValid())
	{
		UE_IRIS_PARALLEL_EXPR(FScopedNetTokenStoreAccess ScopeAccess(this));
...
void FNetTokenStore::WriteTokenData(FArchive& Ar, const FNetToken NetToken, UPackageMap* Map /*= nullptr*/) const
{
	if (NetToken.IsValid())
	{
		...
		{
			UE_LOGF(LogNetToken, Error, "Trying to write data for unknown NetToken %ls", *NetToken.ToString());
			return;
		}
...
bool FNetTokenStore::ValidateAndStoreNetTokenData(FNetTokenDataStore& DataStore, FNetTokenStoreState& RemoteNetTokenStoreState, const FNetToken NetToken, const FNetTokenStoreKey StoreKey)
{
	...
	{
		UE_LOGF(LogNetToken, Verbose, "FNetTokenStore::ReadTokenData - Replaced local key %ls with %ls", *DataStore.StoredTokens[StoreKey.GetKeyIndex()].ToString(), *NetToken.ToString());	
...
void FNetTokenStore::ReadTokenData(FNetSerializationContext& Context, const FNetToken NetToken, FNetTokenStoreState& RemoteNetTokenStoreState)
{
	if (NetToken.IsValid())
	{
		...
		{
			Context.SetError(GNetError_InvalidValue);
			UE_LOGF(LogNetToken, Error, "Failed to read ReadTokenData for %ls.", *NetToken.ToString());
			ensure(false);
			return;
		}
...
FScopedNetTokenStoreAccess::FScopedNetTokenStoreAccess(const FNetTokenStore* InTokenStore)
{
#if UE_SUPPORT_PARALLEL_IRIS
	check(InTokenStore);
	InTokenStore->NetTokenStoreCS.Lock();
	TokenStore = InTokenStore;
#endif // UE_SUPPORT_PARALLEL_IRIS
}

FScopedNetTokenStoreAccess::~FScopedNetTokenStoreAccess()
{
#if UE_SUPPORT_PARALLEL_IRIS
	check(TokenStore);

	TokenStore->NetTokenStoreCS.Unlock();
#endif // UE_SUPPORT_PARALLEL_IRIS
}
...
bool UNetTokenTypeIdConfig::ReservedTypeIdsAppearValid() const
{
	...
	{
		UE_LOGF(LogNetToken, Error, "No TypeID information found in [/Script/IrisCore.NetTokenTypeIdConfig] Engine.ini");
		return false;
	}
	...
	for (const FNetTokenStoreTypeIdPair& TypeIdPair : ReservedTypeIds)
	{
		...
		{
			UE_LOGF(LogNetToken, Error, "TypeID %d found for %ls that is larger than FNetToken::MaxIdCount %d", TypeIdPair.TypeID, *TypeIdPair.StoreTypeName, UE::Net::FNetToken::MaxTypeIdCount);
			return false;
		}
		...
		{
			UE_LOGF(LogNetToken, Error, "Duplicate TypeID information found in [/Script/IrisCore.NetTokenTypeIdConfig] Engine.ini for%ls and %ls with TypeId: %u", *Found[TypeIdPair.TypeID], *TypeIdPair.StoreTypeName, TypeIdPair.TypeID);
			return false;
		}
