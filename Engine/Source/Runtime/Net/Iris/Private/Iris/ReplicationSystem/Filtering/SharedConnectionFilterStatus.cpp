...
bool FSharedConnectionFilterStatus::SetFilterStatus(FConnectionHandle ConnectionHandle, ENetFilterStatus FilterStatus)
{
	...
	{
		UE_LOGF(LogIrisFiltering, Error, "%ls", TEXT("Trying to set filter status for an invalid FConnectionHandle on an FConnectionHandleFilterGroup"));
		return false;
	}

	if (ParentConnectionId != InvalidConnectionId && ConnectionHandle.GetParentConnectionId() != ParentConnectionId)
	{
		UE_LOGF(LogIrisFiltering, Error, "FConnectionHandleFilterGroup ignoring SetFilterStatus call due to connection ID mismatch. Expected %u got %u.", ParentConnectionId, ConnectionHandle.GetParentConnectionId());
		return false;
	}
...
void FSharedConnectionFilterStatusCollection::SetFilterStatus(FConnectionHandle ConnectionHandle, ENetFilterStatus FilterStatus)
{
	if (!ConnectionHandle.IsValid())
	{
		UE_LOGF(LogIrisFiltering, Error, "%ls", TEXT("Trying to set filter status for an invalid FConnectionHandle on an FConnectionHandleFilterGroupCollection"));
		return;
	}
