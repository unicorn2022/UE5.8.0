...
public:
	 ...
	 * @see UReplicationSystem::CreateFilter().
	 */
	TWeakObjectPtr<UNetObjectFilter> CreateFilter(const FNetObjectFilterDefinition& Definition);

	/**
	* Destroy a filter.
	* @see UReplicationSystem::DestroyFilter().
	*/
	void DestroyFilter(const FName FilterName);
	...
	// Connection handling
	void AddConnection(uint32 ConnectionId, ENetConnectionTraits NetConnectionTraits);
...
private:
	struct FPerConnectionInfo
	{
		...
		// Creation dependents that have been filtered because a dependency has been filtered.
		FNetBitArray FilteredOutCreationDependents;
	...
	struct FFilterInfo
	{
		bool IsValid() const { return Filter != nullptr; };
...
private:
	...
	// Mark creation dependents as filtered out. This may only be called from within the creation dependency update.
	void FilterOutCreationDependents(uint32 ParentIndex, FPerConnectionInfo& ConnectionInfo, const FNetBitArrayView ObjectsWithCreationDependents) const;
	// Mark creation dependents as filtered in if none of their ancestors are filtered out. This may only be called from within the creation dependency update.
	void FilterInCreationDependents(uint32 ParentIndex, FPerConnectionInfo& ConnectionInfo, const FNetBitArrayView ObjectsWithCreationDependents) const;
	// Reevaluate the filter status of an object based on creation dependencies. This may only be called from within the creation dependency update.
	void RefreshCreationDependencyFilterStatus(uint32 ObjectIndex, FPerConnectionInfo& ConnectionInfo, const FNetBitArrayView ObjectsWithCreationDependents) const;

	void UpdateCreationDependentParent(uint32 ChildIndex, const FNetBitArrayView ObjectsWithCreationDependencies, FNetBitArrayView OutConnectionObjectsInScope, FNetBitArrayView ProcessedObjects, bool bIsRecursive) const;
	...
	ENetFilterStatus GetDependentObjectFilterStatus(uint32 ConnectionId, FInternalNetRefIndex ObjectIndex, TSet<FInternalNetRefIndex>& Visited);
	...
	bool IsValidDynamicFilter(uint32 FilterIndex) const;
	void RemoveFromDynamicFilter(UE::Net::FInternalNetRefIndex ObjectIndex, uint32 FilterIndex);

	void NotifyFiltersOfDirtyObjects();
	void BatchNotifyFiltersOfDirtyObjects(FUpdateDirtyObjectsBatchHelper& BatchHelper, const UE::Net::FInternalNetRefIndex* ObjectIndices, uint32 ObjectCount);
	...
	// Get InitParams for a filter.
	// @param ConfigClass Filter-specific config class. May be nullptr.
	// @return InitParams suitable for passing to UNetObjectFilter::Init().
	FNetObjectFilterInitParams GetFilterInitParams(UClass* ConfigClass) const;

	// Create & initialize an instance of the no-op filter.
	TStrongObjectPtr<UNetObjectFilter> MakeNoOpNetObjectFilter() const;
...
private:
	...
	// Scratch buffers for scope delta tracking (shared across connections, reused each iteration)
	FNetBitArray ScopeDeltaPrevObjects;
	FNetBitArray ScopeDeltaNewlyInObjects;
	FNetBitArray ScopeDeltaNewlyOutObjects;
	...
	// Dynamic Filter infos. May contain invalid/null filters after dynamic filters are destroyed. Invalid/null filters must be ignored.
	...
	uint32 LoggedCyclicDependencyCount = 0U;
};
