...
void FNetObjectFactoryRegistry::RegisterFactory(UClass* FactoryClass, FName FactoryName)
{
	...
	{
		UE_LOGF(LogIris, Error, "FNetObjectFactoryRegistry::RegisterFactory cannot register factory: %ls name: %ls because it was called while Iris replication was already started", *GetNameSafe(FactoryClass), *FactoryName.ToString());
	...
	{
		UE_LOGF(LogIris, Error, "FNetObjectFactoryRegistry::RegisterFactory already has %u factories registered. Cannot register factory: %ls with name: %ls", MaxFactories, *GetNameSafe(FactoryClass), *FactoryName.ToString());
		ensure(false);
		return;
	}
	...
	for (const FFactoryData& FactoryData : NetFactories)
	{
		...
		{
			UE_LOGF(LogIris, Error, "FNetObjectFactoryRegistry::RegisterFactory cannot register factory: %ls with name: %ls. This name is already used by factory: %ls id: %u", *GetNameSafe(FactoryClass), *FactoryName.ToString(), *GetNameSafe(FactoryData.NetFactoryClass.Get()), FactoryData.Id);
			ensure(false);
			return;
		}
	}
	...
	UE_LOGF(LogIris, Verbose, "FNetObjectFactoryRegistry::RegisterFactory has registered factory: %ls name: %ls id: %u", *GetNameSafe(NetFactories[Index].NetFactoryClass.Get()), *NetFactories[Index].Name.ToString(), NetFactories[Index].Id);
}
...
void FNetObjectFactoryRegistry::UnregisterFactory(FName FactoryName)
{
	...
	if (!bIsFactoryRegistrationAllowed)
	{
		UE_LOGF(LogIris, Error, "FNetObjectFactoryRegistry::UnregisterFactory cannot unregister factory name: %ls because it was called while Iris replication was already started", *FactoryName.ToString());
		check(bIsFactoryRegistrationAllowed);
		return;
	}
	...
	for (int32 Index=0; Index < NetFactories.Num(); ++Index)
	{
		...
		{
			UE_LOGF(LogIris, Verbose, "FNetObjectFactoryRegistry::UnregisterFactory is unregistering factory: %ls name: %ls id: %u", *GetNameSafe(Data.NetFactoryClass.Get()), *Data.Name.ToString(), Data.Id);
	...
	UE_LOGF(LogIris, Error, "FNetObjectFactoryRegistry::UnregisterFactory could not find any factories using name: %ls", *FactoryName.ToString());
	ensure(false);
}
