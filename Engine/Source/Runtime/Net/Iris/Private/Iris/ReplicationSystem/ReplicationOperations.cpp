...
static void OutputDescriptorMemberHashToString(FNetSerializationContext Context, uint32* HashBuffer, uint32 Indentation, FStringBuilderBase& StringBuilder, const uint8* RESTRICT SrcInternalBuffer, const FReplicationStateDescriptor* Descriptor)
{
	...
	{
		StringBuilder.Appendf(TEXT("%*sMember: %u "), 4*Indentation, TEXT(""), MemberIndex);
	};
...
/** Build structured member hashes from descriptors, mirroring the logic in OutputDescriptorMemberHashToString. */
static void BuildDescriptorMemberState(FNetSerializationContext Context, uint32* HashBuffer, const uint8* RESTRICT SrcInternalBuffer, const FReplicationStateDescriptor* Descriptor, TArray<Private::FDebugCDOMemberState>& OutMembers)
{
	const FReplicationStateMemberDescriptor* MemberDescriptors = Descriptor->MemberDescriptors;
	const FReplicationStateMemberSerializerDescriptor* MemberSerializerDescriptors = Descriptor->MemberSerializerDescriptors;
	const FReplicationStateMemberDebugDescriptor* MemberDebugDescriptors = Descriptor->MemberDebugDescriptors;
	...
	for (uint32 MemberIt = 0; MemberIt < MemberCount; ++MemberIt)
	{
		...
		Private::FDebugCDOMemberState& Member = OutMembers.AddDefaulted_GetRef();
		Member.Name = ToCStr(MemberDebugDescriptors[MemberIt].DebugName);

		if (IsUsingStructNetSerializer(MemberSerializerDescriptor))
		{
			const FStructNetSerializerConfig* StructConfig = static_cast<const FStructNetSerializerConfig*>(MemberSerializerDescriptor.SerializerConfig);
			const FReplicationStateDescriptor* StructDescriptor = StructConfig->StateDescriptor;
			BuildDescriptorMemberState(Context, HashBuffer, SrcInternalBuffer + MemberDescriptor.InternalMemberOffset, StructDescriptor, Member.SubMembers);
		}
		else
		{
			Member.SerializerName = MemberSerializerDescriptor.Serializer->Name;

			// Hash the serialized default state for this member (same as OutputDescriptorMemberHashToString)
			Writer.Seek(0U);
			...
			{
				UE_LOGF(LogIris, Warning, "BuildDescriptorMemberState: Writer overflow for member %ls, hash will be zero", *Member.Name);
				Member.Hash = 0;
			}
			else
			{
				...
				Member.Hash = CityHash64((const char*)HashBuffer, Writer.GetPosBytes());
			}
		}
	}
}

void FReplicationInstanceOperations::BuildInternalDefaultState(UReplicationSystem* ReplicationSystem, const FReplicationFragments& Fragments, Private::FDebugCDOState& OutState)
{
	if (!ReplicationSystem)
	{
		return;
	...
	for (int32 Index = 0; Index < Fragments.Num(); ++Index)
	{
		...
		Private::FDebugCDOFragmentState& Fragment = OutState.Fragments.AddDefaulted_GetRef();
		Fragment.Name = ToCStr(CurrentDescriptor->DebugName);
		Fragment.DescriptorId = CurrentDescriptor->DescriptorIdentifier.Value;
		Fragment.DefaultStateHash = CurrentDescriptor->DescriptorIdentifier.DefaultStateHash;

		if (CurrentDescriptor->DefaultStateBuffer)
		{
			BuildDescriptorMemberState(Context, TempBuffer.GetData(), CurrentDescriptor->DefaultStateBuffer, CurrentDescriptor, Fragment.Members);
		}

		for (uint32 FunctionIt = 0; FunctionIt < CurrentDescriptor->FunctionCount; ++FunctionIt)
		{
			const FReplicationStateMemberFunctionDescriptor& FunctionDescriptor = CurrentDescriptor->MemberFunctionDescriptors[FunctionIt];
			Private::FDebugCDOFunctionState& Function = Fragment.Functions.AddDefaulted_GetRef();
			Function.Name = GetNameSafe(FunctionDescriptor.Function);
			Function.DescriptorId = FunctionDescriptor.Descriptor ? FunctionDescriptor.Descriptor->DescriptorIdentifier.Value : 0;
		}
	}
}

TArray<uint8> FReplicationInstanceOperations::SerializeInternalDefaultState(Private::FDebugCDOState& State)
{
	TArray<uint8> Bytes;
...
bool FReplicationInstanceOperations::DeserializeInternalDefaultState(const TArray<uint8>& Bytes, Private::FDebugCDOState& OutState)
{
...
/** Diff two member lists recursively and append results to DiffBuilder. */
static void DiffDescriptorMembers(const TArray<Private::FDebugCDOMemberState>& ClientMembers, const TArray<Private::FDebugCDOMemberState>& ServerMembers, FStringBuilderBase& DiffBuilder, int32 Indent)
{
	TMap<FString, const Private::FDebugCDOMemberState*> ServerMembersByName;
	for (const Private::FDebugCDOMemberState& Member : ServerMembers)
	{
		ServerMembersByName.Add(Member.Name, &Member);
	}

	TSet<FString> MatchedServerMembers;
	for (const Private::FDebugCDOMemberState& ClientMember : ClientMembers)
	{
		const Private::FDebugCDOMemberState** ServerMemberPtr = ServerMembersByName.Find(ClientMember.Name);
		if (!ServerMemberPtr)
		{
			DiffBuilder.Appendf(TEXT("%*sClient only: %s"), Indent, TEXT(""), *ClientMember.Name);
			if (!ClientMember.SerializerName.IsEmpty())
			{
				DiffBuilder.Appendf(TEXT(" Serializer: %s Hash: 0x%" UINT64_x_FMT), *ClientMember.SerializerName, ClientMember.Hash);
			}
			DiffBuilder.AppendChar(TEXT('\n'));
		}
		else
		{
			MatchedServerMembers.Add(ClientMember.Name);
			const Private::FDebugCDOMemberState& ServerMember = **ServerMemberPtr;

			const bool bClientIsLeaf = !ClientMember.SerializerName.IsEmpty();
			const bool bServerIsLeaf = !ServerMember.SerializerName.IsEmpty();

			if (bClientIsLeaf && bServerIsLeaf)
			{
				// Leaf member — compare serializer and hash
				if (ClientMember.SerializerName != ServerMember.SerializerName || ClientMember.Hash != ServerMember.Hash)
				{
					DiffBuilder.Appendf(TEXT("%*sChanged: %s\n"), Indent, TEXT(""), *ClientMember.Name);
					DiffBuilder.Appendf(TEXT("%*s  Client: Serializer: %s Hash: 0x%" UINT64_x_FMT "\n"), Indent, TEXT(""), *ClientMember.SerializerName, ClientMember.Hash);
					DiffBuilder.Appendf(TEXT("%*s  Server: Serializer: %s Hash: 0x%" UINT64_x_FMT "\n"), Indent, TEXT(""), *ServerMember.SerializerName, ServerMember.Hash);
				}
			}
			else if (!bClientIsLeaf && !bServerIsLeaf)
			{
				// Struct member — recurse into sub-members
				DiffDescriptorMembers(ClientMember.SubMembers, ServerMember.SubMembers, DiffBuilder, Indent + 2);
			}
			else
			{
				// Shape mismatch: one side is a struct, the other isn't
				DiffBuilder.Appendf(TEXT("%*sChanged shape: %s\n"), Indent, TEXT(""), *ClientMember.Name);
				if (bClientIsLeaf)
				{
					DiffBuilder.Appendf(TEXT("%*s  Client: Serializer: %s Hash: 0x%" UINT64_x_FMT "\n"), Indent, TEXT(""), *ClientMember.SerializerName, ClientMember.Hash);
					DiffBuilder.Appendf(TEXT("%*s  Server: struct with %d sub-member(s)\n"), Indent, TEXT(""), ServerMember.SubMembers.Num());
				}
				else
				{
					DiffBuilder.Appendf(TEXT("%*s  Client: struct with %d sub-member(s)\n"), Indent, TEXT(""), ClientMember.SubMembers.Num());
					DiffBuilder.Appendf(TEXT("%*s  Server: Serializer: %s Hash: 0x%" UINT64_x_FMT "\n"), Indent, TEXT(""), *ServerMember.SerializerName, ServerMember.Hash);
				}
			}
		}
	...
	for (const Private::FDebugCDOMemberState& ServerMember : ServerMembers)
	{
		if (!MatchedServerMembers.Contains(ServerMember.Name))
		{
			DiffBuilder.Appendf(TEXT("%*sServer only: %s"), Indent, TEXT(""), *ServerMember.Name);
			if (!ServerMember.SerializerName.IsEmpty())
			{
				DiffBuilder.Appendf(TEXT(" Serializer: %s Hash: 0x%" UINT64_x_FMT), *ServerMember.SerializerName, ServerMember.Hash);
			}
			DiffBuilder.AppendChar(TEXT('\n'));
		}
	}
...
/** Diff two UFUNCTION lists by name */
static void DiffDescriptorFunctions(const TArray<Private::FDebugCDOFunctionState>& ClientFunctions, const TArray<Private::FDebugCDOFunctionState>& ServerFunctions, FStringBuilderBase& DiffBuilder, int32 Indent)
{
	TMap<FString, const Private::FDebugCDOFunctionState*> ServerFunctionsByName;
	for (const Private::FDebugCDOFunctionState& Function : ServerFunctions)
	{
		ServerFunctionsByName.Add(Function.Name, &Function);
	}

	TSet<FString> MatchedServerFunctions;
	for (const Private::FDebugCDOFunctionState& ClientFunction : ClientFunctions)
	{
		const Private::FDebugCDOFunctionState** ServerFunctionPtr = ServerFunctionsByName.Find(ClientFunction.Name);
		if (!ServerFunctionPtr)
		{
			DiffBuilder.Appendf(TEXT("%*sClient only function: %s DescriptorId: 0x%" UINT64_x_FMT "\n"), Indent, TEXT(""), *ClientFunction.Name, ClientFunction.DescriptorId);
		}
		else
		{
			MatchedServerFunctions.Add(ClientFunction.Name);
			const Private::FDebugCDOFunctionState& ServerFunction = **ServerFunctionPtr;
			if (ClientFunction.DescriptorId != ServerFunction.DescriptorId)
			{
				DiffBuilder.Appendf(TEXT("%*sChanged function signature: %s (client DescriptorId: 0x%" UINT64_x_FMT ", server DescriptorId: 0x%" UINT64_x_FMT ")\n"), Indent, TEXT(""), *ClientFunction.Name, ClientFunction.DescriptorId, ServerFunction.DescriptorId);
			}
		}
	}

	for (const Private::FDebugCDOFunctionState& ServerFunction : ServerFunctions)
	{
		if (!MatchedServerFunctions.Contains(ServerFunction.Name))
		{
			DiffBuilder.Appendf(TEXT("%*sServer only function: %s DescriptorId: 0x%" UINT64_x_FMT "\n"), Indent, TEXT(""), *ServerFunction.Name, ServerFunction.DescriptorId);
		}
	}
}

void FReplicationInstanceOperations::DiffAndLogInternalDefaultState(const Private::FDebugCDOState& ClientState, const Private::FDebugCDOState& ServerState)
{
	// Index server fragments by name
	TMap<FString, const Private::FDebugCDOFragmentState*> ServerFragmentsByName;
	for (const Private::FDebugCDOFragmentState& Fragment : ServerState.Fragments)
	{
		ServerFragmentsByName.Add(Fragment.Name, &Fragment);
	}

	TStringBuilder<4096> DiffBuilder;

	// Compare each client fragment against the server
	TSet<FString> ProcessedFragments;
	for (const Private::FDebugCDOFragmentState& ClientFrag : ClientState.Fragments)
	{
		ProcessedFragments.Add(ClientFrag.Name);
		const Private::FDebugCDOFragmentState** ServerFragPtr = ServerFragmentsByName.Find(ClientFrag.Name);

		if (!ServerFragPtr)
		{
			DiffBuilder.Appendf(TEXT("Fragment %s: client only (not present on server)\n"), *ClientFrag.Name);
			continue;
		}

		const Private::FDebugCDOFragmentState& ServerFrag = **ServerFragPtr;

		// Skip fragments that match on both identity (DescriptorId) and contents (DefaultStateHash).
		if (ClientFrag.DescriptorId == ServerFrag.DescriptorId && ClientFrag.DefaultStateHash == ServerFrag.DefaultStateHash)
		{
			continue;
		}

		DiffBuilder.Appendf(TEXT("Fragment %s: mismatch (client DescriptorId: 0x%" UINT64_x_FMT ", server DescriptorId: 0x%" UINT64_x_FMT " | client DefaultStateHash: 0x%" UINT64_x_FMT ", server DefaultStateHash: 0x%" UINT64_x_FMT ")\n"), *ClientFrag.Name, ClientFrag.DescriptorId, ServerFrag.DescriptorId, ClientFrag.DefaultStateHash, ServerFrag.DefaultStateHash);

		constexpr int32 IndentSize = 2;
		DiffDescriptorMembers(ClientFrag.Members, ServerFrag.Members, DiffBuilder, IndentSize);
		DiffDescriptorFunctions(ClientFrag.Functions, ServerFrag.Functions, DiffBuilder, IndentSize);
	}

	// Check for server-only fragments
	for (const Private::FDebugCDOFragmentState& ServerFrag : ServerState.Fragments)
	{
		if (!ProcessedFragments.Contains(ServerFrag.Name))
		{
			DiffBuilder.Appendf(TEXT("Fragment %s: server only (not present on client)\n"), *ServerFrag.Name);
		}
	}

	if (DiffBuilder.Len() == 0)
	{
		UE_LOGF(LogIris, Error, "CDO state diff: no differences found");
		return;
	}

	UE_LOGF(LogIris, Error, "CDO state differences between client and server:\n%ls", DiffBuilder.ToString());
}
