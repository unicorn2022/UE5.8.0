...
public:
	...
	void SetRPCDoS(FRPCDoSDetection* InRPCDoS) { RPCDoS = InRPCDoS; }
	...
	// Get the number of unresolved handles we are tracking
	uint32 GetNumTrackedUnresolvedHandleToDependents() const
	{
		return UnresolvedHandleToDependents.Num();
	}

	// Get the number of resolved dynamic handles we are tracking
	uint32 GetNumTrackedResolvedDynamicHandleToDependents() const
	{
		return ResolvedDynamicHandleToDependents.Num();
	}
...
private:
	...
	// Assemble and deserialize huge object if present for the given attachmenttype.
	void ProcessHugeObject(FNetSerializationContext& Context, ENetObjectAttachmentType AttachmentType);
	...
	void EndReplication(FInternalNetRefIndex InternalIndex, EInternalDetachReason DetachReason);
	...
	bool EnqueueEndReplication(FPendingBatchData* PendingBatchData, FNetRefHandle NetRefHandleToEndReplication, EInternalDetachReason DetachReason);
...
private:
	...
	// Used during receive to track objects read from hugeobject path
	TArray<FNetRefHandle> ObjectsReadFromHugeObjects;
