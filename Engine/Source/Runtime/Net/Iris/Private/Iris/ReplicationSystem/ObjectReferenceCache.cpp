...
static bool bIrisAllowAsyncReload = true;
FAutoConsoleVariableRef CVarIrisAllowAsyncReload(TEXT("net.iris.AllowAsyncReload"), bIrisAllowAsyncReload, TEXT("Flag to allow async reload of objects from packages that might be partially garbage collected. Note: Only relevant if async loading is enabled."), ECVF_Default);
...
static bool bIrisVerifyOuterForStaleDynamicReferences = true;
FAutoConsoleVariableRef CVarIrisVerifyOuterForStaleDynamicReferences(TEXT("net.iris.VerifyOuterForStaleDynamicReferences"), bIrisVerifyOuterForStaleDynamicReferences, TEXT("If set, we will verify outer for when creationg stably named references of dynamic objects. Default is true."), ECVF_Default);

static bool bIrisEvictTrackedReferencesWithDynamicOuterFromCache = true;
FAutoConsoleVariableRef CVarIrisEvictTrackedReferencesWithDynamicOuterFromCache(TEXT("net.iris.EvictTrackedReferencesWithDynamicOuterFromCache"), bIrisEvictTrackedReferencesWithDynamicOuterFromCache, TEXT("If set, relative references to dynamic outer will be evicted from cache once the outer is unregistered and no longer replicates. Default is true."), ECVF_Default);
...
void FObjectReferenceCache::Init(const FObjectReferenceCacheParams& Params)
{
	bIsAuthority = Params.bAuthority;
	ObjectToNetRefHandleSerial = Params.ObjectToNetRefHandleSerial;
	NetRefHandleSerialToObject = Params.NetRefHandleSerialToObject;
}
...
bool FObjectReferenceCache::ShouldIgnoreWhenMissing(FNetRefHandle RefHandle) const
{
	...
	UE_IRIS_PARALLEL_EXPR(UE::TScopeLock InternalLock(ObjectReferenceCacheInternalCS));
...
bool FObjectReferenceCache::RenamePathForPie(uint32 ConnectionId, FString& Str, bool bReading, UReplicationSystem* ReplicationSystem)
{
	if constexpr (WITH_EDITOR)
	{
		UObjectReplicationBridge* ReplicationBridge = ReplicationSystem->GetReplicationBridgeAs<UObjectReplicationBridge>();
		return ReplicationBridge->RemapPathForPIE(ConnectionId, Str, bReading);
	}
	else
	{
		return false;
...
FNetObjectReference FObjectReferenceCache::GetOrCreateObjectReferenceUsingPath(const UObject* Instance, UReplicationSystem* ReplicationSystem)
{
	if (Instance && Instance->IsFullNameStableForNetworking())
	{
		FStringTokenStore* LocalStringTokenStore = ReplicationSystem->GetNetTokenStore()->GetDataStore<FStringTokenStore>();

		FString ObjectPath = Instance->GetPathName();
		const FNetToken ObjectPathToken = LocalStringTokenStore->GetOrCreateToken(ObjectPath);

		return MakeNetObjectReference(FNetRefHandle::GetInvalid(), ObjectPathToken);
	}

	return GetOrCreateObjectReference(Instance, ReplicationSystem);
}

FNetRefHandle FObjectReferenceCache::CreateObjectReferenceHandle(const UObject* Object, UReplicationSystem* ReplicationSystem)
{
	FNetObjectReference Ref;
	CreateObjectReferenceInternal(Object, Ref, ReplicationSystem);

	return Ref.GetRefHandle();
}

FNetRefHandle FObjectReferenceCache::ReassignObjectRefHandle(const UObject* Object, UReplicationSystem* ReplicationSystem)
{
	// Clear the handle already assigned to the pointer.
	const FSetElementId ObjectToNetRefId = ObjectToNetReferenceHandle.FindId(Object);
	if (ObjectToNetRefId.IsValidId())
	{
		// TODO: Should we remove old NetRefHandle -> Object map entry ?
		//		 It depends if the RepSystem will want to access the object via the old NetRefHandle later.
		//		 But since the handle is supposed to be in a Destroyed/StopReplication state its already dangerous to access  the object.
		//		 We might want to mark the entry + netrefhandle as deprecated on StopReplication directly and only access the object via an Explicit call if it is actually neeeded.
		//const FNetRefHandle DeprecatedRefHandle = ObjectToNetReferenceHandle.Get(ObjectToNetRefId);
		//ReferenceHandleToCachedReference.Remove(DeprecatedRefHandle);

		// Only remove the object->handle map for now.
		ObjectToNetReferenceHandle.Remove(ObjectToNetRefId);
	}

	FNetObjectReference ObjectRef;
	bool bSuccess = CreateObjectReferenceInternal(Object, ObjectRef, ReplicationSystem);

	return bSuccess ? ObjectRef.GetRefHandle() : FNetRefHandle::GetInvalid();
}

FNetRefHandle FObjectReferenceCache::PreRegisterObjectReferenceHandle(const UObject* Object, UReplicationSystem* ReplicationSystem)
{
	if (IsAuthority())
	{
		FNetObjectReference Ref;
...
bool FObjectReferenceCache::IsCachedOuterChainValid(const UObject* Object, FNetRefHandle RefHandle, const FCachedNetObjectReference& CachedObject) const
{
	if (!(RefHandle.IsDynamic() && CachedObject.RelativePath.IsValid()))
	{
		return true;
	}

	const UObject* CurrentOuter = Object->GetOuter();
	FNetRefHandle CurrentCachedOuterHandle = CachedObject.OuterNetRefHandle;

	while (CurrentOuter != nullptr || CurrentCachedOuterHandle.IsValid())
	{
		const FCachedNetObjectReference* CachedOuterPtr = CurrentCachedOuterHandle.IsValid() ? ReferenceHandleToCachedReference.Find(CurrentCachedOuterHandle) : nullptr;
		if (CachedOuterPtr)
		{
			if (CachedOuterPtr->Object.Get() != CurrentOuter)
			{
				return false;
			}
			if (!(CurrentCachedOuterHandle.IsDynamic() && CachedOuterPtr->RelativePath.IsValid()))
			{
				break;
			}
			CurrentOuter = CurrentOuter ? CurrentOuter->GetOuter() : nullptr;
			CurrentCachedOuterHandle = CachedOuterPtr->OuterNetRefHandle;
		}
		else
		{
			return !CurrentCachedOuterHandle.IsValid();
		}
	}
	return true;
}

bool FObjectReferenceCache::CreateObjectReferenceInternal(const UObject* Object, FNetObjectReference& OutReference, UReplicationSystem* ReplicationSystem)
{
	if (!IsValid(Object))
	{
		return false;
	...
	if (FNetRefHandle* RefHandlePtr = ObjectToNetReferenceHandle.Find(Object))
	{
		...
		if (FCachedNetObjectReference* CachedObjectPtr = ReferenceHandleToCachedReference.Find(RefHandle))
		{
			...
			{
				const bool bOuterChainValid = !bIrisVerifyOuterForStaleDynamicReferences || IsCachedOuterChainValid(Object, RefHandle, CachedObject);
	...
	const bool bIsStableForNetworking = Object->IsNameStableForNetworking();
	...
	if (!IsAuthority())
	{
		...
		for (const UObject* Outer = Object; Outer != nullptr; Outer = Outer->GetOuter())
		{
			if (FNetRefHandle* ReplicatedOuter = ObjectToNetReferenceHandle.Find(Outer))
			{
				...
				RenamePathForPie(UE::Net::InvalidConnectionId, RelativeObjectPath, bReading, ReplicationSystem);
				OutReference = MakeNetObjectReference(*ReplicatedOuter, LocalStringTokenStore->GetOrCreateToken(RelativeObjectPath));
		...
		if (bIsStatic)
		{
			...
			constexpr bool bReading = false;
			// The connection ID isn't used unless reading.
			RenamePathForPie(UE::Net::InvalidConnectionId, ObjectPath, bReading, ReplicationSystem);
			OutReference = MakeNetObjectReference(FNetRefHandle::GetInvalid(), LocalStringTokenStore->GetOrCreateToken(ObjectPath));	
	...
	if (ReplicationSystem->GetProxyType() == UE::Net::EProxyType::Frontend)
	{
		TSharedPtr<FObjectReferenceCache> SharedObjectReferenceCache = FReplicationSystemFactory::GetSharedObjectReferenceCache();
		if (!ensureMsgf(SharedObjectReferenceCache, TEXT("Failed initializing and retrieving the shared object reference cache.")))
		{
			return false;
		}

		FNetRefHandle SharedNetRefHandle = SharedObjectReferenceCache->GetObjectReferenceHandleFromObject(Object); 
		if (SharedNetRefHandle.IsValid())
		{
			UE_LOGF(LogIris, Verbose, "ObjectReferenceCache::CreateObjectReferenceHandle Found a valid NetRefHandle %ls in the shared object reference cache for object %ls", *SharedNetRefHandle.ToString(), *Object->GetName());
			NetRefHandle = NetRefHandleManager.MakeNetRefHandle(SharedNetRefHandle.GetId(), ReplicationSystem->GetId());
		}
		else
		{
			UE_LOGF(LogIris, Warning, "ObjectReferenceCache::CreateObjectReferenceHandle Unable to find a NetRefHandle for object %ls in the shared object reference cache.", *Object->GetName());			
			return false;
		}
	}
	else
	{
		if (ObjectToNetRefHandleSerial.IsBound())
		{
			TObjectPtr<const UObject> ObjectPtr(Object);
			const uint64 Serial = ObjectToNetRefHandleSerial.Execute(ObjectPtr);
			NetRefHandle = NetRefHandleManager.AllocateNetRefHandleFromSerial(Serial, bIsStatic);
		}
		else
		{
			NetRefHandle = NetRefHandleManager.AllocateNetRefHandle(bIsStatic);
		}
	}
	...
	if (bIsFullNameStableForNetworking || Object->IsNameStableForNetworking())
	{
		...
		PathToken = LocalStringTokenStore->GetOrCreateToken(ObjectPath);
	}
	...
	CachedObject.RelativePath.AssignToken(PathToken, { .ReplicationSystemId = ReplicationSystem->GetId(), .ConnectionId = InvalidConnectionId });
	CachedObject.OuterNetRefHandle = CreateObjectReferenceHandle(Object->GetOuter(), ReplicationSystem);
	...
	if (NetRefHandle.IsValid())
	{
		...
		Builder.Appendf(TEXT("Ref_%s"), PathToken.IsValid() ? LocalStringTokenStore->ResolveToken(PathToken) : ToCStr(Object->GetName()));
	...
	const FNetRefHandle ReplicatedOuterHandle = GetFullyDynamicOuter(CachedObject);
...
void FObjectReferenceCache::AddPreRegisteredReference(FNetRefHandle RefHandle, const UObject* Object, UReplicationSystem* ReplicationSystem)
{
	if (!IsAuthority())
	{
...
void FObjectReferenceCache::RemoveReference(FNetRefHandle RefHandle, const UObject* Object, bool& bOutInvalidatedTrackedSubObjectHandles)
{
	bOutInvalidatedTrackedSubObjectHandles = false;
	...
	// Util to safely remove object validating that it is the same
	auto SafeRemoveObjectToRefHandleMapping = [this](const UObject* ObjectKeyToRemove, FNetRefHandle ExpectedRefHandle)
	{
		if (ObjectKeyToRemove)
		{
			// We create an iterator to avoid having to find the key twice
			TMap<const UObject*, FNetRefHandle>::TKeyIterator ObjectIt = ObjectToNetReferenceHandle.CreateKeyIterator(ObjectKeyToRemove);
			if (ObjectIt)
			{
				const FNetRefHandle ExistingRefHandle = ObjectIt.Value();
				if (ExistingRefHandle == ExpectedRefHandle)
				{
					UE_LOG_REFERENCECACHE(Verbose, TEXT("ObjectReferenceCache::RemoveReference: %s removed from ObjectToNetReferenceHandle Object: (0x%p)."), 
						*ExpectedRefHandle.ToString(), ObjectKeyToRemove);
				...
				else
				{
					...
					UE_LOG_REFERENCECACHE(Verbose, TEXT("ObjectReferenceCache::RemoveReference: ExistingReference %s != RefHandle being removed: %s, Object: %s (0x%p) "), 
						*(ExistingRefHandle.ToString()), *ExpectedRefHandle.ToString(), *GetNameSafe(ObjectKeyToRemove), ObjectKeyToRemove);
				}
			}
			else
			{
				UE_LOG_REFERENCECACHE(Verbose, TEXT("ObjectReferenceCache::RemoveReference: %s, Object: %s (0x%p). Skipped removing since ObjectToNetReferenceHandle had no entry"), 
					*ExpectedRefHandle.ToString(), *GetNameSafe(ObjectKeyToRemove), ObjectKeyToRemove);
			}
		}
	};
	...
	if (It)
	{
		...
		// Invalidate subobject references that referenced the removed dynamic root
		// We keep cache entries intact for relative references so remapping can work later
		// Note: We do NOT remove from HandleToDynamicOuter here.
		// This is because we don't know if the root is being destroyed or just filtered out, 
		// and we need the tracking to persist for UpdateUnresolvableReferenceTracking to be able to remap references if the object becomes relevant again
		TArray<FNetRefHandle, TInlineAllocator<16>> SubReferences;
		HandleToDynamicOuter.MultiFind(RefHandle, SubReferences);
		if (SubReferences.Num())
		{
			for (FNetRefHandle SubRefHandle : SubReferences)
			{
				if (FCachedNetObjectReference* SubObjectRef = ReferenceHandleToCachedReference.Find(SubRefHandle))
				{
					UE_LOG_REFERENCECACHE(Verbose, TEXT("ObjectReferenceCache::RemoveReference: %s invalidating subref %s. Object: %s Pointer: %p"),
						*RefHandle.ToString(), *SubRefHandle.ToString(),
						SubObjectRef->Object.Get() ? *SubObjectRef->Object->GetName() : TEXT("NULL"), SubObjectRef->ObjectKey);

					// Remove from ObjectToNetReferenceHandle so the object can be remapped later
					SafeRemoveObjectToRefHandleMapping(SubObjectRef->ObjectKey, SubRefHandle);

					// Invalidate object pointers but keep cache entry for remapping
					SubObjectRef->ObjectKey = nullptr;
					SubObjectRef->Object.Reset();
				}
			}
			bOutInvalidatedTrackedSubObjectHandles = true;
		}
	...
	SafeRemoveObjectToRefHandleMapping(Object, RefHandle);
}
...
UObject* FObjectReferenceCache::ResolveObjectReferenceHandleInternal(FNetRefHandle RefHandle, const FNetObjectResolveContext& ResolveContext, bool& bOutMustBeMapped)
{
	...
	if (Object != nullptr)
	{
...
#if UE_NET_VALIDATE_REFERENCECACHE
		{
			...
			//RenamePathForPie(ResolveContext.ConnectionId, ObjectPath, bReading, ResolveContext.ReplicationSystem);
	...
	if (CacheObjectPtr->OuterNetRefHandle.IsValid())
	{
		...
		if (OuterCacheObject == nullptr)
		{
			...
			{
				UE_LOGF(LogIris, Error, "GetObjectFromRefHandle: Static outer not registered. FullPath: %ls", ToCStr(FullPath(RefHandle, ResolveContext)))
		...
		{
			UE_LOGF(LogIris, Error, "GetObjectFromRefHandle:  Outer is broken. FullPath: %ls", ToCStr(FullPath(RefHandle, ResolveContext)));
			CacheObjectPtr->bIsBroken = 1;	// Set this to 1 so that we don't keep spamming
	...
	{
		UE_LOGF(LogIris, Error, "GetObjectFromRefHandle: Outer is null or package is not fully loaded. FullPath: %ls Outer: %ls", ToCStr(FullPath(RefHandle, ResolveContext)), *GetFullNameSafe(ObjOuter));
	}

	const TCHAR* ResolvedToken = CacheObjectPtr->RelativePath.ResolveToken();
	...
	if (Object == nullptr && !CacheObjectPtr->bNoLoad)
	{
		if (IsAuthority())
		{
			...
			UE_LOGF(LogIris, Error, "GetObjectFromRefHandle: Server re-loading object (might have been GC'd). FullPath: %ls", ToCStr(FullPath(RefHandle, ResolveContext)));
		}
		...
		if (bIsPackage)
		{
			...
			if (ShouldAsyncLoad() && !ResolveContext.bForceSyncLoad)
			{
				if (!PendingAsyncLoadRequests.Contains(ObjectPathName))
				{
					...
					StartAsyncLoadingPackage(*CacheObjectPtr, ObjectPathName, ResolveContext.AsyncLoadingPriority, RefHandle, false, ResolveContext.ReplicationSystem);
		...
		{
			// If we have a package, but for some reason didn't find the object we try to do a partial async load of the specific object or a blocking load as a last attempt
			...
			if (ShouldAsyncLoad() && !ResolveContext.bForceSyncLoad)
			{
				if (bIrisAllowAsyncReload)
				{
					// Construct full object path:
					const bool bUseSubObjectDelimiter = ObjOuter->GetClass() != UPackage::StaticClass() && ObjOuter->GetOuter()->GetClass() == UPackage::StaticClass();
					FSoftObjectPath SoftPath(FString::Printf(TEXT("%s%c%s"), *ObjOuter->GetPathName(), bUseSubObjectDelimiter ? SUBOBJECT_DELIMITER_CHAR : TEXT('.'), *ObjectPath));

					if (!PendingAsyncObjectLoadRequests.Contains(SoftPath))
					{
						CacheObjectPtr->bIsPending = true;
						StartAsyncLoadingObject(*CacheObjectPtr, SoftPath, ResolveContext.AsyncLoadingPriority, RefHandle, ResolveContext.ReplicationSystem);
						UE_LOG_REFERENCECACHE(VeryVerbose, TEXT("GetObjectFromRefHandle: Async loading object. Path: %s, %s"), *SoftPath.ToString(), *RefHandle.ToString());
					}
					else
					{
						// Merge into existing request
						PendingAsyncObjectLoadRequests[SoftPath].Merge(RefHandle);
						CacheObjectPtr->bIsPending = true;
						UE_LOG_REFERENCECACHE(VeryVerbose, TEXT("GetObjectFromRefHandle: Already async loading object. Path: %s, %s"), *SoftPath.ToString(), *RefHandle.ToString());
					}
					return nullptr;
				}
				else
				{
					UE_LOGF(LogIris, Error, "GetObjectFromRefHandle: Forced blocking load. Path: %ls, %ls", *ObjectPath, *RefHandle.ToString());
				}
			}
	...
	if (Object == nullptr)
	{
		...
		{
			CacheObjectPtr->bIsBroken = 1;	// Set this to 1 so that we don't keep spamming
			UE_LOGF(LogIris, Error, "GetObjectFromRefHandle: Failed to resolve. FullPath: %ls", ToCStr(FullPath(RefHandle, ResolveContext)));
		}
	...
	if (bIsPackage)
	{
		...
		if (Package == nullptr)
		{
			...
			UE_LOGF(LogIris, Error, "GetObjectFromRefHandle: Object is not a package but should be! Path: %ls, NetRefHandle: %ls", *ObjectPath, *RefHandle.ToString() );
			return nullptr;
		}
			...
			&& !Package->HasAnyPackageFlags(TreatAsLoadedFlags)) //TODO: dependencies of CompiledIn could still be loaded asynchronously. Are they necessary at this point??
		{
			if (ShouldAsyncLoad() && !ResolveContext.bForceSyncLoad && Package->HasAnyInternalFlags(EInternalObjectFlags_AsyncLoading))
			{
				...
				StartAsyncLoadingPackage(*CacheObjectPtr, ObjectPathName, ResolveContext.AsyncLoadingPriority, RefHandle, true, ResolveContext.ReplicationSystem);
				UE_LOGF(LogIrisReferences, Verbose, "GetObjectFromRefHandle: Listening to existing async load. Path: %ls, NetRefHandle: %ls", *ObjectPath, *RefHandle.ToString());
			}
	...
	UObjectReplicationBridge* ReplicationBridge = ResolveContext.ReplicationSystem->GetReplicationBridgeAs<UObjectReplicationBridge>();
	if (Object && !ReplicationBridge->ObjectContainerHasFinishedLoading(Object))
	{
		UE_LOG_REFERENCECACHE(Verbose, TEXT("GetObjectFromRefHandle: Forcing object to NULL since container is not loaded yet. Object: %s" ), *Object->GetFullName());
	...
	if (!Object)
	{
		...
		{
			CacheObjectPtr->bIsBroken = 1;	// Set this to 1 so that we don't keep spamming
			UE_LOGF(LogIris, Error, "GetObjectFromRefHandle: Failed to resolve. FullPath: %ls non existing", ToCStr(FullPath(RefHandle, ResolveContext)));
		}
		return nullptr;
	}
	...
	// Promote to full handle
	FNetRefHandle CompleteRefHandle = FNetRefHandleManager::MakeNetRefHandle(RefHandle.GetId(), ResolveContext.ReplicationSystem->GetId());
...
ENetObjectReferenceResolveResult FObjectReferenceCache::ResolveObjectReference(const FNetObjectReference& Reference, const FNetObjectResolveContext& ResolveContext, UObject*& OutResolvedObject)
{
	...
	{
		FStringTokenStore* LocalStringTokenStore = ResolveContext.ReplicationSystem->GetNetTokenStore()->GetDataStore<FStringTokenStore>();

		// This path is only used by Client to Server references
		const TCHAR* ResolvedToken = LocalStringTokenStore->ResolveRemoteToken(Reference.PathToken, *ResolveContext.RemoteNetTokenStoreState);
		if (ResolvedToken)
		{
			FString ObjectPath(ResolvedToken);
			constexpr bool bReading = true;
			...
			{
				UObject* ReplicatedOuter = nullptr;

				if (ReplicatedOuter == nullptr)
				{
					bool bLocalMustBeMapped = false;
					ReplicatedOuter = ResolveObjectReferenceHandleInternal(Reference.GetRefHandle(), ResolveContext, bLocalMustBeMapped);
				}
				
				if (ReplicatedOuter == nullptr && NetRefHandleSerialToObject.IsBound())
				{
					const uint64 Serial = Reference.GetRefHandle().GetSerial();
					ReplicatedOuter = NetRefHandleSerialToObject.Execute(Serial);
				}
	...
	else
	{
		...
		if (ResolvedObject == nullptr && Reference.GetRefHandle().IsValid() && NetRefHandleSerialToObject.IsBound())
		{
			const uint64 Serial = Reference.GetRefHandle().GetSerial();
			ResolvedObject = NetRefHandleSerialToObject.Execute(Reference.GetRefHandle().GetSerial());
		}
	}
...
FNetObjectReference FObjectReferenceCache::GetReplicatedOuter(const FNetObjectReference& Reference, UReplicationSystem* ReplicationSystem) const
{
...
FNetRefHandle FObjectReferenceCache::GetFullyDynamicOuter(const FCachedNetObjectReference& Reference) const
{
	if (!Reference.NetRefHandle.IsDynamic() || !Reference.RelativePath.IsValid())
	{
		return FNetRefHandle::GetInvalid();
	}

	UE_IRIS_PARALLEL_EXPR(UE::TScopeLock InternalLock(ObjectReferenceCacheInternalCS));
	const FCachedNetObjectReference* CachedObject = &Reference;
	while (CachedObject && CachedObject->NetRefHandle.IsDynamic())
	{
		if (CachedObject->RelativePath.IsValid())
		{
...
uint32 FObjectReferenceCache::GetNumObjectsWithTrackedSubObjectHandles() const
{
	return (uint32)HandleToDynamicOuter.Num();
}

void FObjectReferenceCache::GetTrackedSubObjectHandles(FNetRefHandle ReplicatedObjectHandle, TArray<FNetRefHandle>& OutSubObjectHandles) const
{
	UE_IRIS_PARALLEL_EXPR(UE::TScopeLock InternalLock(ObjectReferenceCacheInternalCS));
	HandleToDynamicOuter.MultiFind(ReplicatedObjectHandle, OutSubObjectHandles);
}

void FObjectReferenceCache::MarkTrackedSubObjectHandlesForEviction(FInternalNetRefIndex ObjectIndex, FNetRefHandle ReplicatedObjectHandle)
{
	if (!bIrisEvictTrackedReferencesWithDynamicOuterFromCache)
	{
		return;
	}

	UE_IRIS_PARALLEL_EXPR(UE::TScopeLock InternalLock(ObjectReferenceCacheInternalCS));
	DynamicHandlesToEvictFromCache.Add(ObjectIndex, ReplicatedObjectHandle);
}

void FObjectReferenceCache::OnInternalNetRefIndicesFreed(const TConstArrayView<FInternalNetRefIndex>& FreedIndices)
{
	if (!bIrisEvictTrackedReferencesWithDynamicOuterFromCache)
	{
		return;
	...
	{
		FNetRefHandle HandleWithDynamicReferences;
		if (DynamicHandlesToEvictFromCache.RemoveAndCopyValue(ObjectIndex, HandleWithDynamicReferences))
		{
			// Evict tracked dynamice relative references
			TArray<FNetRefHandle, TInlineAllocator<16>> SubReferences;
			HandleToDynamicOuter.MultiFind(HandleWithDynamicReferences, SubReferences);
			if (SubReferences.Num())
			{
				for (FNetRefHandle SubRefHandle : SubReferences)
				{
					ReferenceHandleToCachedReference.Remove(SubRefHandle);
					UE_LOG_REFERENCECACHE(Verbose, TEXT("ObjectReferenceCache::OnInternalNetRefIndicesFreed: Evicting invalidating subref %s.from cache"), *SubRefHandle.ToString());
				}
				HandleToDynamicOuter.Remove(HandleWithDynamicReferences);
			}
		}
	}
}

FNetObjectReference FObjectReferenceCache::GetOrCreateObjectReference(const UObject* Instance, UReplicationSystem* ReplicationSystem)
{
	FNetObjectReference ObjectReference;
	CreateObjectReferenceInternal(Instance, ObjectReference, ReplicationSystem);

	return ObjectReference;
}

FNetObjectReference FObjectReferenceCache::GetOrCreateObjectReference(const FString& ObjectPath, const UObject* Outer, UReplicationSystem* ReplicationSystem)
{
	...
	if (RefHandle.IsValid())
	{
		...
		CachedObject.ObjectKey = nullptr;
		CachedObject.NetRefHandle = RefHandle;
		CachedObject.RelativePath.AssignToken(LocalStringTokenStore->GetOrCreateToken(ObjectPath), { .ReplicationSystemId = ReplicationSystem->GetId(), .ConnectionId = InvalidConnectionId });
		CachedObject.OuterNetRefHandle = CreateObjectReferenceHandle(Outer, ReplicationSystem);
...
void FObjectReferenceCache::WriteFullReferenceInternal(FNetSerializationContext& Context, const FNetObjectReference& Ref) const
{
	...
	if (!CachedObject)
	{
		UE_CLOGF(RefHandle.IsValid(), LogIrisReferences, Log, "ObjectReferenceCache::WriteFullReference Trying to write Stale handle, %ls", *RefHandle.ToString());
	...
	{	
		...
		if (Writer->WriteBool(bHasPath))
		{
			...
			CachedObject->RelativePath.WriteNetToken(Context);
			CachedObject->RelativePath.ConditionalWriteNetTokenData(Context, ExportContext);
...
void FObjectReferenceCache::ReadFullReferenceInternal(FNetSerializationContext& Context, FNetObjectReference& OutObjectRef, uint32 RecursionCount)
{
	...
	if (bIsExported)
	{
		...
		if (Reader->ReadBool())
		{
			FStringTokenStore* LocalStringTokenStore = Context.GetInternalContext()->ReplicationSystem->GetNetTokenStore()->GetDataStore<FStringTokenStore>();
			RelativePath = LocalStringTokenStore->ReadNetToken(Context);
			FNetTokenStore* LocalNetTokenStore = Context.GetInternalContext()->ReplicationSystem->GetNetTokenStore();
			LocalNetTokenStore->ConditionalReadNetTokenData(Context, RelativePath);
	...
	{
		FNetTokenResolveContext ReceivedTokenContext = Context.GetInternalContext()->ReplicationSystem->GetNetTokenResolveContext(Context.GetLocalConnectionId());

		// Dynamic objects can be referenced by either NetRefHandle or parent NetRefHandle and path combo
		check(!bIsExported || ((CachedObject->NetRefHandle == NetRefHandle && CachedObject->RelativePath.IsEqualToToken(RelativePath, ReceivedTokenContext) && CachedObject->OuterNetRefHandle == OuterRef.GetRefHandle()) ||
	...
	if (!IsAuthority())
	{
		...
		FSharedStringNetToken::FTokenSource TokenSource =
		{
			.ReplicationSystemId = Context.GetInternalContext()->ReplicationSystem->GetId(),
			.ConnectionId = Context.GetLocalConnectionId()
		};

		CachedObject.ObjectKey = nullptr;
		CachedObject.NetRefHandle = NetRefHandle;
		CachedObject.RelativePath.AssignToken(RelativePath, TokenSource);
		...
		const FNetRefHandle DynamicOuterHandle = GetFullyDynamicOuter(CachedObject);
...
void FObjectReferenceCache::ReadFullReference(FNetSerializationContext& Context, FNetObjectReference& OutRef)
{
	...
	if (const bool bIsClientAssignedReference = Reader->ReadBool())
	{
		...
		const FNetRefHandle NetRefHandle = ReadNetRefHandle(Context);
		FNetToken RelativePath = LocalStringTokenStore->ReadNetToken(Context);
		LocalNetTokenStore->ConditionalReadNetTokenData(Context, RelativePath);
...
void FObjectReferenceCache::WriteFullReference(FNetSerializationContext& Context, FNetObjectReference Ref) const
{
	...
	if (Writer->WriteBool(bIsClientAssignedReference))
	{
		...
		// Note: As we use the StringTokenStore to write the token we also need to read it using the stringtokenstore.
		LocalStringTokenStore->WriteNetToken(Context, Ref.PathToken);
		LocalNetTokenStore->ConditionalWriteNetTokenData(Context, Context.GetExportContext(), Ref.PathToken);
		return;
	}
...
void FObjectReferenceCache::WriteReference(FNetSerializationContext& Context, FNetObjectReference Ref) const
{
	...
	if (!CachedObject)
	{
		UE_CLOGF(RefHandle.IsValid(), LogIrisReferences, Verbose, "ObjectReferenceCache::WriteReference Trying to write Stale handle, %ls", *RefHandle.ToString());
...
{	
	...
	// Force exports to be written
	{
		...
		TArrayView<const FNetToken> NetTokenExportsView = MakeArrayView(ExportContext->GetBatchExports().NetTokensPendingExportInCurrentBatch);
		{
			...
			FNetTokenStore* LocalNetTokenStore = Context.GetInternalContext()->ReplicationSystem->GetNetTokenStore();
			const bool bIsNetTokenAuthority = LocalNetTokenStore->IsAuthority();
			...
			for (const FNetToken& NetToken : NetTokenExportsView)
			{
				...
				if (!(NetToken.IsAssignedByAuthority() && !bIsNetTokenAuthority) && !ExportContext->IsExported(NetToken))
				{
					...
					LocalNetTokenStore->WriteNetToken(Context, NetToken);
					LocalNetTokenStore->WriteTokenData(Context, NetToken);
		...
		for (const FNetObjectReference& Reference : NetObjectReferenceExportsView)
		{
			...
			if ((bIsClientAssigned && !ExportContext->IsExported(Reference.GetPathToken())) || (IsAuthority() && Reference.CanBeExported() && !ExportContext->IsExported(Handle)))
			{
				...
				//UE_LOGF(LogIrisReferences, Verbose, "WritePendingExports for %ls : Export[%d] %ls", *NetRefHandleManager->PrintObjectFromIndex(ObjectIndex), CountExports, *NetRefHandleManager->PrintObjectFromNetRefHandle(Handle));
	...
	// Read NetToken exports
	{
		...
		// Force inlined exports
		{
			...
			while (bHasExportsToRead && !Context.HasErrorOrOverflow())
			{
				...
				FNetToken ImportedNetToken = LocalNetTokenStore->ReadNetToken(Context);
				LocalNetTokenStore->ReadTokenData(Context, ImportedNetToken, *ResolveContext.RemoteNetTokenStoreState);
...
bool FObjectReferenceCache::WriteMustBeMappedExports(FNetSerializationContext& Context, FInternalNetRefIndex ObjectIndex, TArrayView<const FNetObjectReference> ExportsView) const
{
	...
	if (IsAuthority() && ShouldAsyncLoad() && ExportContext)
	{
		...
		UReplicationSystem* ReplicationSystem = Context.GetInternalContext()->ReplicationSystem;
		FNetRefHandleManager& NetRefHandleManager = ReplicationSystem->GetReplicationSystemInternal()->GetNetRefHandleManager();
...
#if UE_NET_IRIS_CSV_STATS
				UE_LOGF(LogIrisReferences, VeryVerbose, "WriteMustBeMappedExports for %ls. MustBeMapped[%d] %ls (%ls)", *NetRefHandleManager.PrintObjectFromIndex(ObjectIndex), CountExports, *GetNameSafe(CachedObject->Object.Get()), *Handle.ToString());
#endif
...
#if UE_NET_IRIS_CSV_STATS
		UE_CLOGF(CountExports>0, LogIrisReferences, VeryVerbose, "WriteMustBeMappedExports for %ls. Wrote %d exports with priority: %ls", *NetRefHandleManager.PrintObjectFromIndex(ObjectIndex), CountExports, LexToString(IrisAsyncLoadingPriority));
#endif
...
void FObjectReferenceCache::ReadMustBeMappedExports(const FNetRefHandle& NetObjectHandle, FNetSerializationContext& Context, TArray<FNetRefHandle>* MustBeMappedExports, EIrisAsyncLoadingPriority& OutIrisAsyncLoadingPriority)
{
	...
	while (bHasExportsToRead && !Context.HasErrorOrOverflow())
	{
		...
		UE_LOGF(LogIrisReferences, VeryVerbose, "ReadMustBeMappedExports for %ls. MustBeMapped[%d] %ls", *NetObjectHandle.ToString(), MustBeMappedExports->Num(), *MustBeMappedHandle.ToString());

		MustBeMappedExports->Add(MustBeMappedHandle);
	}

	UE_CLOGF(MustBeMappedExports->Num() > 0, LogIrisReferences, VeryVerbose, "ReadMustBeMappedExports for %ls. Read %d references with priority: %ls", *NetObjectHandle.ToString(), MustBeMappedExports->Num(), LexToString(OutIrisAsyncLoadingPriority));
}
...
FString FObjectReferenceCache::DescribeObjectReference(const FNetObjectReference Ref, const FNetObjectResolveContext& ResolveContext)
{
	...
	if (Ref.PathToken.IsValid())
	{
		...
		// This path is only used by Client to Server references
		const TCHAR* ResolvedToken = LocalStringTokenStore->ResolveRemoteToken(Ref.PathToken, *ResolveContext.RemoteNetTokenStoreState);
		if (ResolvedToken)
		{
			if (!FullPath.IsEmpty())
			{
...
void FObjectReferenceCache::GenerateFullPath_r(FNetRefHandle RefHandle, const FNetObjectResolveContext& ResolveContext, FString& FullPath) const
{
	...
	else
	{
		...
		{
			const TCHAR* ResolvedPath = CacheObject->RelativePath.ResolveToken();
			FullPath += FString::Printf(TEXT("[%s](%s)"), *RefHandle.ToString(), ResolvedPath ? ResolvedPath : TEXT("EMPTY"));
		}
	}
}
...
void FObjectReferenceCache::StartAsyncLoadingPackage(FCachedNetObjectReference& CacheObject, FName PackagePath, TAsyncLoadPriority AsyncLoadingPriority, const FNetRefHandle RefHandle, const bool bWasAlreadyAsyncLoading, UReplicationSystem* ReplicationSystem)
{
	LLM_SCOPE_BYTAG(Iris);
	...
	UE_LOGF(LogIrisReferences, Verbose, "StartAsyncLoadingPackage for %ls: Package: %ls (Async loading priority: %d)", *NetRefHandleManager.PrintObjectFromNetRefHandle(RefHandle), *PackagePath.ToString(), AsyncLoadingPriority);
...
//#endif
	...
	if (ExistingRequest)
	{
		// Same package name but a possibly different NetRefHandle. Note down the NetRefHandle and wait for the async load completion callback
	...
	if (FPendingAsyncLoadRequest const* const PendingLoadRequest = PendingAsyncLoadRequests.Find(PackageName))
	{
		...
		for (FNetRefHandle RefHandleToProcess : PendingLoadRequest->NetRefHandles)
		{
			if (FCachedNetObjectReference* CacheObject = ReferenceHandleToCachedReference.Find(RefHandleToProcess))
			{
				...
				// $TODO: Jira: UE-370863: Investigate if it is safe to resolve now to update CacheObject and mapping, or if it is best to wait until it is done as part of dispatching data.
				// This does not really do anything since CachedObject->Object is not updated until the next resolve of the reference
			...
			{
				UE_LOG_REFERENCECACHE(Error, TEXT("AsyncPackageCallback: Could not find NetRefHandle. Path: %s, RefHandle: %s"), *PackageName.ToString(), *RefHandleToProcess.ToString());
			}
		}
...
void FObjectReferenceCache::StartAsyncLoadingObject(FCachedNetObjectReference& CacheObject, const FSoftObjectPath& ObjectPath, TAsyncLoadPriority AsyncLoadingPriority, const FNetRefHandle RefHandle, UReplicationSystem* ReplicationSystem)
{
	LLM_SCOPE_BYTAG(Iris);
	...
	if (!bIsPrioritySet)
	{
		ensureMsgf(false, TEXT("AsyncLoadingPriority wasn't set in the ResolveContext. Using default value as fallback"));
		AsyncLoadingPriority = ConvertAsyncLoadingPriority(EIrisAsyncLoadingPriority::Default);
	}

	FNetRefHandleManager& NetRefHandleManager = ReplicationSystem->GetReplicationSystemInternal()->GetNetRefHandleManager();

	UE_LOGF(LogIrisReferences, Verbose, "StartAsyncLoadingObject for %ls: Object: %ls (Async loading priority: %d)", *NetRefHandleManager.PrintObjectFromNetRefHandle(RefHandle), *ObjectPath.ToString(), AsyncLoadingPriority);

	FPendingAsyncObjectLoadRequest LoadRequest(RefHandle, ReplicationSystem->GetElapsedTime());

	CacheObject.bIsPending = true;

	FPendingAsyncObjectLoadRequest* ExistingRequest = PendingAsyncObjectLoadRequests.Find(ObjectPath);
	if (ExistingRequest)
	{
		// Same object path but a possibly different NetRefHandle. Note down the NetRefHandle and wait for the async load completion callback
		ExistingRequest->Merge(LoadRequest);
		return;
	}

	PendingAsyncObjectLoadRequests.Emplace(ObjectPath, MoveTemp(LoadRequest));

#if UE_NET_OBJECTREFERENCECACHE_ASYNCLOAD_DEBUG
	FName PackageName = ObjectPath.GetAssetPath().GetPackageName();
	if (bIrisAsyncLoadingFailAllLoads || bIrisAsyncLoadingFailNextLoad || (IrisAsyncLoadingFailPackageName.Len() > 0 && PackageName.ToString().Find(IrisAsyncLoadingFailPackageName) != INDEX_NONE))
	{
		bIrisAsyncLoadingFailNextLoad = false;
		// Not thread safe. We don't want to load the object so we call our callback immediately with nullptr.
		AsyncObjectCallback(ObjectPath, nullptr);
	}
	else
#endif
	{
		FLoadAssetAsyncOptionalParams OptionalParams;
		OptionalParams.PackagePriority = AsyncLoadingPriority;

		ObjectPath.LoadAsync(
			FLoadSoftObjectPathAsyncDelegate::CreateWeakLambda(ReplicationSystem,
				[this, ObjectPath](const FSoftObjectPath& InObjectPath, UObject* LoadedObject)
				{
					AsyncObjectCallback(InObjectPath, LoadedObject);
				}),
			OptionalParams
		);
	}
...
void FObjectReferenceCache::AsyncObjectCallback(const FSoftObjectPath& ObjectPath, UObject* LoadedObject)
{
	LLM_SCOPE_BYTAG(Iris);

	if (FPendingAsyncObjectLoadRequest const* const PendingLoadRequest = PendingAsyncObjectLoadRequests.Find(ObjectPath))
	{
		const bool bIsBroken = (LoadedObject == nullptr);
		...
		for (FNetRefHandle RefHandleToProcess : PendingLoadRequest->NetRefHandles)
		{
			if (FCachedNetObjectReference* CacheObject = ReferenceHandleToCachedReference.Find(RefHandleToProcess))
			{
				if (!CacheObject->bIsPending)
				{
					UE_LOG_REFERENCECACHE(Error, TEXT("AsyncObjectCallback: Object wasn't pending. Path: %s, RefHandle: %s"), *ObjectPath.ToString(), *RefHandleToProcess.ToString());
				}
				...
				{
					CacheObject->bIsBroken = true;
					UE_LOG_REFERENCECACHE(Error, TEXT("AsyncObjectCallback: Object FAILED to load. Path: %s, RefHandle: %s"), *ObjectPath.ToString(), *RefHandleToProcess.ToString());
				}

				// $TODO: Jira: UE-370863: Investigate if it is safe to resolve now to update CacheObject and mapping, or if it is best to wait until it is done as part of dispatching data.

				if (UObject* Object = LoadedObject)
				{
					// Something is loaded, we can now resolve queued data
					UpdateTrackedQueuedBatchObjectReference(RefHandleToProcess, Object);
				}
			...
			{
				UE_LOG_REFERENCECACHE(Error, TEXT("AsyncObjectCallback: Could not find net ref handle. Path: %s, RefHandle: %s"), *ObjectPath.ToString(), *RefHandleToProcess.ToString());
			}
		}

		PendingAsyncObjectLoadRequests.Remove(ObjectPath);
	}
	else
	{
		UE_LOG_REFERENCECACHE(Error, TEXT("AsyncObjectCallback: Could not find object. Path: %s"), *ObjectPath.ToString());
	}
}
...
void FObjectReferenceCache::AddReferencedObjects(FReferenceCollector& ReferenceCollector, UReplicationSystem* ReplicationSystem)
{
...
const TCHAR* FObjectReferenceCache::GetObjectRelativePath(FNetRefHandle NetRefHandle, UReplicationSystem* ReplicationSystem) const
{
	UE_IRIS_PARALLEL_EXPR(UE::TScopeLock InternalLock(ObjectReferenceCacheInternalCS));

	if (const FCachedNetObjectReference* CacheObject = ReferenceHandleToCachedReference.Find(NetRefHandle))
	{
		const TCHAR* ResolvedToken = CacheObject->RelativePath.ResolveToken();
...
void FObjectReferenceCache::ReplicationSystemDestroyed(UReplicationSystem* RepSystem)
{
	// Invalidate FNetTokens that came from the RepSystem in our FCachedNetObjectReferences.
	for (TPair<FNetRefHandle, FCachedNetObjectReference>& ReferencePair : ReferenceHandleToCachedReference)
	{
		FSharedStringNetToken& RelativePath = ReferencePair.Value.RelativePath;
		if (RelativePath.GetSourceReplicationSystemId() == RepSystem->GetId())
		{
			RelativePath.MarkStale();
		}
	}
}

void FObjectReferenceCache::RemoveConnection(UReplicationSystem* RepSystem, uint32 ConnectionId)
{
	// Invalidate FNetTokens that came from the RepSystem and connection in our FCachedNetObjectReferences.
	for (TPair<FNetRefHandle, FCachedNetObjectReference>& ReferencePair : ReferenceHandleToCachedReference)
	{
		FSharedStringNetToken& RelativePath = ReferencePair.Value.RelativePath;
		if (RelativePath.GetSourceReplicationSystemId() == RepSystem->GetId() &&
			RelativePath.GetSourceConnectionId() == ConnectionId)
		{
			RelativePath.MarkStale();
		}
	}
