...
public:
	...
	/** 
	 * When considering replication conditions that require an owning object's role to be ROLE_SimulatedProxy, and the
	 * owning object's role is ROLE_AutonomousProxy, treat its role as ROLE_SimulatedProxy if this option is set to true.
	 * 
	 * Example:
	 *	If an object has the role ROLE_AutonomousProxy but it has a property with the condition COND_SimulatedOnly, the
	 *	property would still be replicated to clients. Other properties that would have COND_AutonomousOnly will still
	 *	be replicated.
	 */
	void SetAutonomousAsSimulated(uint32 ConnectionId, bool bEnabled);
...
private:
	...
	struct FPerConnectionInfo
	{
		...
		ENetConnectionTraits NetConnectionTraits = {};
		bool bAutonomousAsSimulated = false;
	};
...
private:
	...
	void GetChildSubObjectsToReplicate(uint32 ReplicatingConnectionId, const FConditionalsMask& LifetimeConditionals, const FInternalNetRefIndex ParentObjectIndex, const FChildSubObjectsInfo& SubObjectsInfo, FSubObjectsToReplicateArray& OutSubObjectsToReplicate) const;
