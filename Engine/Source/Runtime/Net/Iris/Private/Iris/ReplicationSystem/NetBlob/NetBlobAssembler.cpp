...
void FNetBlobAssembler::AddPartialNetBlob(FNetSerializationContext& Context, FNetRefHandle InRefHandle, const TRefCountPtr<FPartialNetBlob>& PartialNetBlob)
{
	...
	{
		UE_LOGF(LogIris, Error, "NetBlobAssembler - Requires a PartialNetBlobHandlerConfig");
	...
	if (bIsFirstPart)
	{
		...
		if (PartCount == 0)
		{
			bIsBrokenSequence = true;
			Context.SetError(GNetError_InvalidValue);
			UE_LOGF(LogIris, Error, "NetBlobAssembler - Expected partCount with first part");
			ensure(false);
			return;
		}
		...
		if (PartCount > PartialNetBlobHandlerConfig->GetMaxPartCount())
		{
			bIsBrokenSequence = true;
			Context.SetError(GNetError_InvalidValue);
			UE_LOGF(LogIris, Error, "NetBlobAssembler - Received invalid PartCount %u configured MaxPartCount %u", PartCount, PartialNetBlobHandlerConfig->GetMaxPartCount());
			ensure(false);
			return;
		}
		...
		if (PayloadBitCount > PartialNetBlobHandlerConfig->GetMaxPartBitCount())
		{
			bIsBrokenSequence = true;
			Context.SetError(GNetError_InvalidValue);
			UE_LOGF(LogIris, Error, "NetBlobAssembler - Received invalid PayloadBitCount %u configured MaxPartBitCount %u", PayloadBitCount, PartialNetBlobHandlerConfig->GetMaxPartBitCount());
			ensure(false);
			return;
		}
	...
	else
	{
		...
		if (((PayloadBitCount != FirstPayloadBitCount) && (NextSequenceSumber != LastPartSequenceNumber)) || (PayloadBitCount > FirstPayloadBitCount))
		{
			bIsBrokenSequence = true;
			Context.SetError(GNetError_InvalidValue);
			UE_LOGF(LogIris, Error, "NetBlobAssembler - Received invalid PayloadBitCount %u", PayloadBitCount);
			ensure(false);
			return;
		}
