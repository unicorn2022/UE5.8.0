...
void FWorldLocations::OnMaxInternalNetRefIndexIncreased(FInternalNetRefIndex NewMaxInternalIndex)
{
...
void FWorldLocations::InitObjectInfoCache(FInternalNetRefIndex ObjectIndex)
{
...
void FWorldLocations::RemoveObjectInfoCache(FInternalNetRefIndex ObjectIndex)
{
	if (!ValidInfoIndexes.IsBitSet(ObjectIndex))
	{
...
void FWorldLocations::SetObjectInfo(FInternalNetRefIndex ObjectIndex, const FVector& Location, const float NetCullDistance)
{
	...
	{
		UE_LOGF(LogIrisNetCull, Verbose, "FWorldLocations::SetObjectInfo ReplicatedObject %ls cull distance %f is above the max %f. Consider making object always relevant instead", *NetRefHandleManager->PrintObjectFromIndex(ObjectIndex), NetCullDistance, MaxNetCullDistance);
...
bool FWorldLocations::SetCullDistanceOverride(FInternalNetRefIndex ObjectIndex, float CullDistance)
{
	ensure(!bLockdownDirtyList);
	...
	if (ValidInfoIndexes.IsBitSet(ObjectIndex))
	{
		...
		if (GetObjectInfo(ObjectIndex).CullDistanceOverride != CullDistance)
		{
			...
			{
				UE_LOGF(LogIrisNetCull, Verbose, "FWorldLocations::SetCullDistanceOverride ReplicatedObject %ls cull distance %f is above the max %f. Consider making object always relevant instead", *NetRefHandleManager->PrintObjectFromIndex(ObjectIndex), CullDistance, MaxNetCullDistance);	
...
bool FWorldLocations::ClearCullDistanceOverride(FInternalNetRefIndex ObjectIndex)
{
	ensure(!bLockdownDirtyList);
