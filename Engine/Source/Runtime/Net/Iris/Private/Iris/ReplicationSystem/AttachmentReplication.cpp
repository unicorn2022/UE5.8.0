...
public:
	...
	bool Enqueue(TArrayView<const TRefCountPtr<FNetBlob>> Attachments)
	{
		...
		{
			ensureMsgf(false, TEXT("%u reliable RPCs not enqueued as adding them would exceed the current MaxPreQueueCount (%u)"), AttachmentCount, MaxPreQueueCount);
			return false;
		}
	...
	void ProcessPacketDeliveryStatus(EPacketDeliveryStatus Status, const FReliableNetBlobQueue::FReplicationRecord& Record, bool& bOutMarkHasAttachmentsToSendDirty)
	{
		ReliableQueue.ProcessPacketDeliveryStatus(Status, Record);
		if (Status == EPacketDeliveryStatus::Delivered && Record.IsValid())
		{
			const uint32 PreUnsentBlobCount = ReliableQueue.GetUnsentBlobCount();
			PopulateQueueFromPreQueue();
			bOutMarkHasAttachmentsToSendDirty = ReliableQueue.GetUnsentBlobCount() > PreUnsentBlobCount;
		}
	}
...
SIZE_T FNetObjectAttachmentSendQueue::GetReliableCount() const
{
	if (ReliableQueue == nullptr)
	{
		return 0;
	...
	return ReliableQueue->GetUnsentBlobCount();
}
...
void FNetObjectAttachmentSendQueue::ProcessPacketDeliveryStatus(EPacketDeliveryStatus Status, const FNetObjectAttachmentSendQueue::FReliableReplicationRecord& Record, bool& bOutMarkHasAttachmentsToSendDirty)
{
	bOutMarkHasAttachmentsToSendDirty = false;
	if (ReliableQueue == nullptr)
	{
		return;
	}

	ReliableQueue->ProcessPacketDeliveryStatus(Status, Record, bOutMarkHasAttachmentsToSendDirty);
}
...
SIZE_T FNetObjectAttachmentsWriter::GetReliableAttachmentCount(ENetObjectAttachmentType Type, uint32 ObjectIndex) const
{
	const FNetObjectAttachmentSendQueue* Queue = GetQueue(Type, ObjectIndex);
	if (Queue == nullptr)
	{
...
void FNetObjectAttachmentsWriter::ProcessPacketDeliveryStatus(EPacketDeliveryStatus Status, ENetObjectAttachmentType Type, uint32 ObjectIndex, const FReliableReplicationRecord& Record, bool& bOutMarkHasAttachmentsToSendDirty)
{
	bOutMarkHasAttachmentsToSendDirty = false;
	FNetObjectAttachmentSendQueue* Queue = GetQueue(Type, ObjectIndex);
	if (Queue == nullptr)
	...
	Queue->ProcessPacketDeliveryStatus(Status, Record, bOutMarkHasAttachmentsToSendDirty);
}
...
{
	bool bDummyMarkHasAttachmentsToSendDirty = false;
	ProcessPacketDeliveryStatus(Status, Type, ObjectIndex, Record, bDummyMarkHasAttachmentsToSendDirty);
}
...
public:
	...
	void Enqueue(FNetSerializationContext& Context, FNetRefHandle RefHandle, const TRefCountPtr<FNetBlob>& NetBlob, bool bIsPartialNetBlob)
	{
		...
		{
			UE_LOGF(LogIris, Error, "Received reliable blob when only unreliable is supported.");
...
uint32 FNetObjectAttachmentReceiveQueue::DeserializeUnreliable(FNetSerializationContext& Context, FNetRefHandle RefHandle)
{
	...
	do
	{
		...
		{
			UE_LOGF(LogIris, Error, "Unreliable queue is full for %ls", *RefHandle.ToString());
		...
		if (Context.HasErrorOrOverflow())
		{
			UE_LOGF(LogIris, Error, "Failed to deserialize unreliable attachments for %ls", *RefHandle.ToString());
			break;
		}
