...
public:
	...
	static constexpr uint32 PreAllocatedScheduledObjectCount = 2048U;
...
public:
	...
	enum class EReplicatedObjectState : uint8
	{
		...
		DebugObject,					// Special state for object index used for sending parts of debug object payloads
	...
	static_assert(UE::Net::GetBitsNeeded(EReplicatedObjectState::Count) <= 5, "EReplicatedObjectState must fit in 5 bits. See FReplicationInfo::State and FReplicationRecord::FRecordInfo::ReplicatedObjectState members.");
	...
	struct FReplicationInfo
	{
		...
		union 
		{
			...
			struct 
			{
				...
				uint64 IsDebugObject : 1;								// If this flag is set, the object is a debugobject and is always sent using DebugObject attachments.
				uint64 IsFilteredOutSubObjectPendingCreate : 1;			// If this flag is set, the object is a subobject that was filtered out while sending pending create which requires some extra attention
				uint64 IsFilteredOutSubObjectTearOff : 1;				// If this flag is set, the object is a subobject confirmed as created that was filtered out while marked as tearoff which requires some extra attention
			};
		};
...
public:
	...
	/** Is the object in a state where we can send data about it. It will be false when the object is about to be destroyed or we are waiting for destroy confirmation. */
	bool IsInReplicatingState(FInternalNetRefIndex ObjectInternalIndex) const;
...
private:
	...
	using FScheduledObjectInfosArray = TArray<FScheduleObjectInfo, TInlineAllocator<PreAllocatedScheduledObjectCount>>;

	struct FExtraScheduleInfo
	{
		FExtraScheduleInfo(FInternalNetRefIndex InInternalIndex, bool bInAllowAppendCreationDependencies = true)
		: InternalIndex(InInternalIndex) 
		, bAppendCreationDependencies(bInAllowAppendCreationDependencies)
		{
		}

		FInternalNetRefIndex InternalIndex = 0U;
		bool bAppendCreationDependencies = false;
	};
	...
	struct FWriteContext
	{
		...
		// Objects we have written in this packet to avoid sending same object multiple times
		FNetBitArray ObjectsWrittenThisPacket;

		// Push a list of objects to the AdditionalObjectSchedulingStack. It will be added in reverse to maintain order.
		void PushAdditionalScheduledObjects(TArrayView<FExtraScheduleInfo> ObjectsToSchedule)
		{ 
			for (int32 It = ObjectsToSchedule.Num() - 1; It >= 0; It--)
			{
				AdditionalObjectSchedulingStack.Push(ObjectsToSchedule[It]);
			}
		}

		// Additional objects that we have scheduled to be written, it could be dependencies, dependent objects.
		TArray<FExtraScheduleInfo, TInlineAllocator<32>> AdditionalObjectSchedulingStack;

		// Objects skipped this packet since they did not fit, that we want to retry next packet
		TArray<FInternalNetRefIndex, TInlineAllocator<32>> ObjectsSkippedInThisPacket;

		// Scheduled objects
		FScheduledObjectInfosArray ScheduledObjectInfos;
		...
		// Whether there is at least one debug object to send or not
		uint32 bHasDebugObjectToSend : 1;
	...
	{
	public:
		FHugeObjectSendQueue(const TCHAR* QueueName, bool bInAllowMultipleBatchesPerRootObjectInTransit = false);
	...
	private:
		TMap<FInternalNetRefIndex, int32> RootObjectsInTransit;
		TDoubleLinkedList<FHugeObjectContext> SendContexts;
		bool bAllowMultipleBatchesPerRootObjectInTransit = false;
	};

	struct FWriteObjectTraits
	{
		uint32 bWriteState : 1U = 0U;
		uint32 bWriteAttachments : 1U = 0U;
		uint32 OOBWriteAttachmentType : 2U = 0U;
		uint32 bIsWritingHugeObjectBatch : 1U = 0U;
	};
...
private:
	...
	void ScheduleDependentObjects(uint32 Index, float ParentPriority, TArray<float>& LocalPriorities, FScheduledObjectInfosArray& ScheduledObjectIndices);

	uint32 ScheduleObjects(FScheduledObjectInfosArray& ScheduledObjectIndices);
	...
	EWriteObjectStatus WriteObjectAndSubObjects(FNetSerializationContext& Context, uint32 InternalIndex, FWriteObjectTraits WriteObjectTraits, FBatchInfo& OutBatchInfo);

	// Write objects recursive
	EWriteObjectStatus WriteObjectInBatch(FNetSerializationContext& Context, uint32 InternalIndex, FWriteObjectTraits WriteObjectTraits, FBatchInfo& OutBatchInfo);
	...
	enum class EWriteStatus : int32
	{
		...
		Skipped = 0,
		// Object was skipped due to not enough space but will be retried in next packet if we are allowed to write more data.
		SkippedRetryNextPacket = 1,
		// Object was successfully written
		Written = 2,
		// Object is rescheduled due to creation dependency
		RescheduledDueToCreationDependency = 3,
	};
	...
	// returns > 1 if Objects was written, 0 if the objects was skipped (failed dependency or waiting for creation confirmation) -1 if the stream is full and we should stop
	FWriteBatchResult WriteObjectBatch(FNetSerializationContext& Context, FInternalNetRefIndex InternalIndex, FWriteObjectTraits WriteObjectTraits);

	EWriteStatus PrepareAndSendHugeObjectPayload(FNetSerializationContext& Context, FInternalNetRefIndex InternalIndex, ENetObjectAttachmentType AttachmentType);
	...
	bool CanQueueDebugObject() const;

	void FreeHugeObjectSendQueues();
...
private:
	...
	FHugeObjectSendQueue DebugObjectSendQueue;
