...
void FObjectPollFrequencyLimiter::Init(FInternalNetRefIndex InMaxInternalIndex, const Private::FNetRefHandleManager* InNetRefHandleManager)
{
	NetRefHandleManager = InNetRefHandleManager;
	OnMaxInternalNetRefIndexIncreased(InMaxInternalIndex);
}
...
void FObjectPollFrequencyLimiter::OnMaxInternalNetRefIndexIncreased(FInternalNetRefIndex NewMaxInternalIndex)
{
	...
	Frequencies.SetNumZeroed(StorageObjectCount);
}
...
void FObjectPollFrequencyLimiter::Update(const FNetBitArrayView& RelevantObjects, const FNetBitArrayView& DirtyObjects, FNetBitArrayView& OutObjectsToPoll)
{
...
#if PLATFORM_ENABLE_VECTORINTRINSICS == 1 && !PLATFORM_ENABLE_VECTORINTRINSICS_NEON
	{
		...
		for (FInternalNetRefIndex ObjectIndex = 0, ObjectEndIndex = MaxInternalIndex;
...
void FObjectPollFrequencyLimiter::ReinitFramesBetweenUpdates()
{
	if (!NetRefHandleManager)
	{
		return;
	}

	auto UpdatePollFrequency = [this](uint32 RootObjectIndex)
	{
		const float PollFrequency = Frequencies[RootObjectIndex];
		SetPollFrequency(RootObjectIndex, PollFrequency);

		// Make sure the subobjects are polled the same frame as the root object.
		for (const FInternalNetRefIndex SubObjectIndex : NetRefHandleManager->GetSubObjects(RootObjectIndex))
		{
			SetPollWithObject(RootObjectIndex, SubObjectIndex);
		}
	};

	// Update poll frame periods for all root objects
	const FNetBitArrayView ReplicatedObjects = NetRefHandleManager->GetGlobalScopableInternalIndices();
	const FNetBitArrayView SubObjects = MakeNetBitArrayView(NetRefHandleManager->GetSubObjectInternalIndices());
