...
enum class ENetObjectAttachmentType : uint32
{
	...
	// For special attachments like OutOfBand, HugeObject and DebugObjects the ObjectIndex is assumed to be zero.
	OutOfBand,
	HugeObject,
	DebugObject,
...
static constexpr uint32 NetObjectAttachmentTypeBitCount = 2;

static_assert((uint32)ENetObjectAttachmentType::InternalCount == 1U << NetObjectAttachmentTypeBitCount, "ENetObjectAttachmentType must fit in NetObjectAttachementTypeBitCount bits");
...
private:
	...
	void ProcessPacketDeliveryStatus(EPacketDeliveryStatus Status, const FReliableReplicationRecord& Record, bool& bOutMarkHasAttachmentsToSendDirty);
...
public:
	...
	// Counts all Reliable attachments for the given type
	SIZE_T GetReliableAttachmentCount(ENetObjectAttachmentType Type, uint32 ObjectIndex) const;
	...
	void ProcessPacketDeliveryStatus(EPacketDeliveryStatus Status, ENetObjectAttachmentType Type, uint32 ObjectIndex, const FReliableReplicationRecord& Record, bool& bOutMarkHasAttachmentsToSendDirty);
