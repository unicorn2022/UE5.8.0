...
bool FNetBlobHandlerManager::RegisterHandler(UNetBlobHandler* Handler)
{
	...
	UE_LOGF(LogIris, Warning, "Handler of class %ls was not found in the NetBlobHandlerDefinitions", ToCStr(Handler->GetClass()->GetName()));
	return false;
}
...
TRefCountPtr<FNetBlob> FNetBlobHandlerManager::CreateNetBlob(const FNetBlobCreationInfo& CreationInfo) const
{
	...
	if (Handler == nullptr)
	{
		UE_LOGF(LogIris, Warning, "No handler registered for NetBlob type %u", CreationInfo.Type);
		return nullptr;
	}
