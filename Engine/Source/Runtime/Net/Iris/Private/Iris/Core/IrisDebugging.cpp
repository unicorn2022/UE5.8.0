...
namespace IrisDebugHelperInternal
{
	...
	static UE::Net::FInternalNetRefIndex GIrisDebugInternalIndex = InvalidInternalNetRefIndex;
...
bool BreakOnInternalNetRefIndex(UE::Net::FInternalNetRefIndex InternalIndex)
{
...
UE::Net::FInternalNetRefIndex GetDebugInternalNetRefIndex()
{
...
void SetIrisDebugInternalNetRefIndex(UE::Net::FInternalNetRefIndex InternalIndex)
{
...
void SetIrisDebugInternalNetRefIndexViaNetHandle(FNetRefHandle RefHandle)
{
	...
	if (!RefHandle.IsValid())
	{
		...
		IrisDebugHelperInternal::GIrisDebugInternalIndex = InvalidInternalNetRefIndex;
		return;
	}
...
FNetReplicatedObjectDebugInfo DebugNetRefHandleById(uint64 NetRefHandleId, uint32 ReplicationSystemId)
{
	...
	// See if we can find the instance in any replication system
	{
		...
		if (ReplicationSystem)
		{
			...
			if (InternalNetRefIndex != InvalidInternalNetRefIndex)
			{
	...
	ProtocolManager.ForEachProtocol(ProtocolId, [&StringBuilder, &NetRefHandleManager](const FReplicationProtocol* Protocol, const FObjectKey TemplateKey)
		{
			...
			NetRefHandleManager.GetAssignedInternalIndicesView().ForAllSetBits(FindMatchingProtocol);
		}
	);
}
