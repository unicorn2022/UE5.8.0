...
bool UDataStreamManager::HasDebugDataToSend() const
{
	return Impl->HasDebugDataToSend();
}
...
UDataStreamManager::EWriteResult UDataStreamManager::FImpl::WriteData(UE::Net::FNetSerializationContext& Context, FDataStreamRecord const*& OutRecord)
{
	...
	// Suppress clang analyzer false positive about Context retaining reference to ExportScope's member.
	// The FExportScope destructor restores the original ExportContext before this return.
	UE_CLANG_SUPPRESS
	return CombinedWriteResult;
}
...
bool UDataStreamManager::FImpl::HasDebugDataToSend() const
{
	for (TObjectPtr<const UDataStream> Stream : Streams)
	{
		if (Stream && Stream->HasDebugDataToSend())
		{
			return true;
		}
	}
