...
// Whether a string contains only ASCII codepoints.
static bool ContainsASCIIOnly(FStringView StringView)
{
	return Algo::AllOf(
		StringView,
		[](FStringView::ElementType Char) -> bool
		{
			return (Char & static_cast<FStringView::ElementType>(0x7FU)) == Char;
		}
	);
}
...
void WriteString(FNetBitStreamWriter* Writer, FStringView StringView)
{
	...
	{
		UE_LOGF(LogSerialization, Error, "String to write is too long.");
		Writer->WriteBool(false);
		WritePackedUint16(Writer, 0);
		return;
	}

	constexpr uint32 StackAllocatedBufferSize = 1024;
	TArray<uint8, TInlineAllocator<StackAllocatedBufferSize>> EncodingBuffer;
	const bool bIsEncoded = !ContainsASCIIOnly(StringView);
	if (Writer->WriteBool(bIsEncoded))
	{
		...
		{
			UE_LOGF(LogSerialization, Error, "String to write is invalid or too long.");
			Writer->WriteBits(0, 16);
			return;
		}

		WritePackedUint16(Writer, static_cast<uint16>(OutEncodedLength));
		Writer->WriteBitStream(reinterpret_cast<uint32*>(EncodingBuffer.GetData()), 0U, OutEncodedLength*8U);
	}
	else
	{
		WritePackedUint16(Writer, static_cast<uint16>(StringViewLength));
		if (Writer->IsOverflown())
		{
			return;
		}
		...
		{
			Writer->WriteBits(static_cast<uint32>(Char) & 0x7FU, 7);
		}
	}
}
...
void ReadString(FNetBitStreamReader* Reader, FString& OutString)
{
	...
	const uint32 Length = ReadPackedUint16(Reader);
	...
	if (bIsEncoded)
	{
		...
		{
			UE_LOGF(LogSerialization, Error, "Received invalid encoded string.");
			Reader->DoOverflow();
			return;
		}
	...
	{
		OutString.Reset(Length);
		for (uint32 Index = 0; Index < Length; ++Index)
		{
			OutString.AppendChar(static_cast<FString::ElementType>(Reader->ReadBits(7)));
		}

		if (Reader->IsOverflown())
		{
			UE_LOGF(LogSerialization, Error, "Received truncated ASCII string.");
			return;
		}
	}
}
