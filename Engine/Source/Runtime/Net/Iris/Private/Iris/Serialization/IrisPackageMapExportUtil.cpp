...
void FIrisPackageMapExportsUtil::Deserialize(FNetSerializationContext& Context, QuantizedType& Value)
{
	...
	// Read any object references
	{
		...
		if (bHasObjectReferences)
		{
			...
			{
				UE_LOGF(LogIris, Error, "FIrisPackageMapExportsUtil::Received too many object reference exports %u > max:%u", NumReferences, MaxExports);
	...
	// Read any exported names
	{
		...
		if (bHasNames)
		{
			...
			{
				UE_LOGF(LogIris, Error, "FIrisPackageMapExportsUtil::Received too many name exports %u > max:%u", NumNames, MaxExports);
				Context.SetError(GNetError_ArraySizeTooLarge);
				return;
			}
