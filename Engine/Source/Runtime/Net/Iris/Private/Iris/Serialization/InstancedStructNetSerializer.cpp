...
void FInstancedStructNetSerializer::Deserialize(FNetSerializationContext& Context, const FNetDeserializeArgs& Args)
{
	...
	if (Reader->ReadBool())
	{
		...
		{
			UE_LOGF(LogIris, Error, "Unable to find struct using FQuantizedObjectReference %ls when deserializing instanced struct.", ToCStr(StructType.ToString()));
...
void FInstancedStructNetSerializer::SerializeDelta(FNetSerializationContext& Context, const FNetSerializeDeltaArgs& Args)
{
	...
	// If the type has changed proceed with regular serialization.
	...
	// If the instance isn't set there's nothing to delta serialize
	if (Value.StructName.IsNone())
	{
		return;
	}
	...
	if (Writer->WriteBool(StructConfig.StateDescriptor.IsValid()))
	{
		// Delta serialize struct data
		FNetSerializeDeltaArgs DeltaSerializeArgs = Args;
		DeltaSerializeArgs.Source = NetSerializerValuePointer(Value.StructData.GetData());
		DeltaSerializeArgs.Prev = NetSerializerValuePointer(PrevValue.StructData.GetData());
		DeltaSerializeArgs.NetSerializerConfig = NetSerializerConfigParam(&StructConfig);
		StructNetSerializer->SerializeDelta(Context, DeltaSerializeArgs);
	}
}
...
{
	FNetBitStreamReader* Reader = Context.GetBitStreamReader();
	if (const bool bTypeDiffers = Reader->ReadBool())
	{
	...
	// If the instance isn't set there's nothing to deserialize
	if (PrevValue.StructName.IsNone())
	{
		Reset(Context, Config, Value);
		return;
	}

	// The sender may not have succeeded in serializing the data due to struct type no longer existing.
	if (!Reader->ReadBool())
	{
		Reset(Context, Config, Value);
		return;
	...
	// Delta deserialize
	FStructNetSerializerConfig StructConfig;
	StructConfig.StateDescriptor = Config->DescriptorCache.FindOrAddDescriptor(PrevValue.StructName);
	if (!StructConfig.StateDescriptor.IsValid())
	{
		ensureMsgf(StructConfig.StateDescriptor.IsValid(), TEXT("Unable to create ReplicationStateDescriptor for struct %s."), ToCStr(PrevValue.StructName.ToString()));
		Context.SetError(GNetError_InvalidValue);
		return;
	}

	// Fix up struct info and allocations if the target value is changing types.
	if (Value.StructName != PrevValue.StructName)
	{
		FreeStructInstance(Context, Config, Value);
		Value.StructData.AdjustSize(Context, StructConfig.StateDescriptor->InternalSize, StructConfig.StateDescriptor->InternalAlignment);
		Value.StructType = PrevValue.StructType;
		Value.StructDescriptorTraits = PrevValue.StructDescriptorTraits;
		Value.StructName = PrevValue.StructName;
	}

	// Delta deserialize struct data
	{
		FNetDeserializeDeltaArgs DeltaDeserializeArgs = Args;
		DeltaDeserializeArgs.Target = NetSerializerValuePointer(Value.StructData.GetData());
		DeltaDeserializeArgs.Prev = NetSerializerValuePointer(PrevValue.StructData.GetData());
		DeltaDeserializeArgs.NetSerializerConfig = NetSerializerConfigParam(&StructConfig);
		StructNetSerializer->DeserializeDelta(Context, DeltaDeserializeArgs);
	}
}
...
void FInstancedStructNetSerializer::Quantize(FNetSerializationContext& Context, const FNetQuantizeArgs& Args)
{
	...
	if (Source.IsValid() && !Context.IsInitializingDefaultState())
	{
		...
		if (ensureMsgf(DescriptorRef.IsValid(), TEXT("Unable to create descriptor for struct %s. Unexpected."), ToCStr(Struct->GetFullName())))
		{
			...
			// If the struct type is different from the previous instance we need adjust the memory allocation.
...
void FInstancedStructDescriptorCache::SetMaxCachedDescriptorCount(int32 MaxCount)
{
	...
	else
	{
		...
		UE_CLOGF(!DescriptorMap.IsEmpty(), LogIris, Warning, "Clearing DescriptorMap from FIstancedStructDescriptorCache %ls", ToCStr(DebugName));
		DescriptorMap.Empty();
