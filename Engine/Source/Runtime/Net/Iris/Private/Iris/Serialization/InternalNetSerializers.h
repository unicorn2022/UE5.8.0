...
USTRUCT()
struct FOptionalPropertyNetSerializerConfig : public FNetSerializerConfig
{
	GENERATED_BODY()

	IRISCORE_API FOptionalPropertyNetSerializerConfig();
	IRISCORE_API ~FOptionalPropertyNetSerializerConfig();

	FOptionalPropertyNetSerializerConfig(const FOptionalPropertyNetSerializerConfig&) = delete;
	FOptionalPropertyNetSerializerConfig(FOptionalPropertyNetSerializerConfig&&) = delete;

	FOptionalPropertyNetSerializerConfig& operator=(const FOptionalPropertyNetSerializerConfig&) = delete;
	FOptionalPropertyNetSerializerConfig& operator=(FOptionalPropertyNetSerializerConfig&&) = delete;

	UPROPERTY()
	TFieldPath<FOptionalProperty> Property;

	TRefCountPtr<const UE::Net::FReplicationStateDescriptor> StateDescriptor;
};

template<>
struct TStructOpsTypeTraits<FOptionalPropertyNetSerializerConfig> : public TStructOpsTypeTraitsBase2<FOptionalPropertyNetSerializerConfig>
{
	enum
	{
		WithCopy = false
...
UE_NET_DECLARE_SERIALIZER_INTERNAL(FOptionalPropertyNetSerializer, IRISCORE_API);
