...
void FNetRoleNetSerializer::Serialize(FNetSerializationContext& Context, const FNetSerializeArgs& Args)
{
	...
	if (Context.GetInternalContext()->bAlwaysSwapRolesOnReplication)
	{
		FNetSerializeArgs SwapArgs = Args;

		SourceType* OtherRolePtr = reinterpret_cast<SourceType*>(Args.Source + Config->RelativeInternalOffsetToOtherRole);
		const SourceType OtherRoleValue = *OtherRolePtr;

		if (Context.GetInternalContext()->bDowngradeAutonomousProxyRole & (OtherRoleValue == Config->AutonomousProxyValue))
		{
			*OtherRolePtr = Config->SimulatedProxyValue;
		}

		SwapArgs.Source = NetSerializerValuePointer(OtherRolePtr);
		Private::FIntRangeNetSerializerBase<SourceType, ConfigType>::Serialize(Context, SwapArgs);

		*OtherRolePtr = OtherRoleValue;
	}
	else if (Context.GetInternalContext()->bDowngradeAutonomousProxyRole & (Value == Config->AutonomousProxyValue))
	{
