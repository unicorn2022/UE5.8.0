		...
		for (uint32 MemberIt = 0; MemberIt < MemberCount; ++MemberIt)
		{
			...
			if (RepNotifyFunction && (bIsInitState || IsDirty(MemberIt)))
			{
				...
				if (UE_LOG_ACTIVE(LogIrisRepNotify, Verbose) && IrisDebugHelper::FilterDebuggedObject(Object))
				{
					...
					if (bShouldCallRepNotify)
					{
						UE_LOGF(LogIrisRepNotify, Verbose, "Calling RepNotify. Object: %ls, Function: %ls IsInit: %d IsRepAlways: %d", *Object->GetFullName(), ToCStr(RepNotifyFunction->GetName()), Params.bIsInit, bIsRepNotifyAlways);
					}
					else
					{
						UE_LOGF(LogIrisRepNotify, VeryVerbose, "Skipping RepNotify. Object: %ls, Function: %ls IsInit: %d IsRepAlways: %d", *Object->GetFullName(), ToCStr(RepNotifyFunction->GetName()), Params.bIsInit, bIsRepNotifyAlways);
					}					
				}
#endif
			}
