...
private:
	...
	TRefCountPtr<const FReplicationStateDescriptor> GetDescriptorForOptionalProperty(const FProperty* Property, FReplicationStateDescriptorRegistry* DescriptorRegistry) const;
...
TRefCountPtr<const FReplicationStateDescriptor> FPropertyReplicationStateDescriptorBuilder::GetDescriptorForOptionalProperty(const FProperty* Property, FReplicationStateDescriptorRegistry* DescriptorRegistry) const
{
	FReplicationStateDescriptorBuilder::FParameters Params;
	Params.DescriptorRegistry = DescriptorRegistry;
	...
	FString DescriptorName;
	GetPropertyPathName(OptionalProperty, DescriptorName);

	return CreateDescriptorForProperty(DescriptorName, ValueProperty, Params);
}
...
void FPropertyReplicationStateDescriptorBuilder::BuildMemberCache(FBuilderContext& Context, FReplicationStateDescriptorRegistry* DescriptorRegistry)
{
	...
	for (FMemberProperty& Member : Members)
	{
		...
		else if (IsOptionalPropertyNetSerializer(Serializer))
		{
			bSomeMembersAreProperties = true;

			// Build descriptor for inner property
			CurrentCacheEntry->Descriptor = GetDescriptorForOptionalProperty(Member.Property, DescriptorRegistry);
			...
			// We propagate this up so that a optional with a struct that is not fully replicated will also have this set
			if (!EnumHasAnyFlags(Descriptor->Traits, EReplicationStateTraits::AllMembersAreReplicated))
			{
				bAllMembersAreReplicated = false;
			}
...
void FPropertyReplicationStateDescriptorBuilder::BuildMemberSerializerConfigs(uint8* SerializerConfigBuffer, FReplicationStateDescriptor* Descriptor, const FMemberCache& MemberCache) const
{
	...
	for (const FMemberProperty& Member : Members)
	{
		...
		else
		{
			...
			else if (IsOptionalPropertyNetSerializer(MemberCacheEntry->Serializer))
			{
				FOptionalPropertyNetSerializerConfig* SerializerConfig = new (SerializerConfigBuffer) FOptionalPropertyNetSerializerConfig();
				SerializerConfig->StateDescriptor = MemberCacheEntry->Descriptor;
				SerializerConfig->Property = TFieldPath<FOptionalProperty>(const_cast<FOptionalProperty*>(CastField<FOptionalProperty>(Member.Property)));
				MemberSerializerDescriptor->SerializerConfig = SerializerConfig;				
			}
			else
			{
	...
	if (IsStructNetSerializer(NetSerializer))
	{
		if (const FStructProperty* StructProperty = CastField<FStructProperty>(InProperty))
		{
			...
			else if (Struct->IsChildOf(FFastArraySerializerItem::StaticStruct()))
			{
				...
				if (IsStructWithCustomSerializer(Struct))
				{
					UE_LOGF(LogIris, Error, "FPropertyReplicationStateDescriptorBuilder found unsupported custom NetSerializers for FastArrayItems %ls", *Struct->GetName());
...
TRefCountPtr<const FReplicationStateDescriptor> FReplicationStateDescriptorBuilder::CreateDescriptorForStruct(const UStruct* InStruct, const FParameters& Parameters)
{
	...
	else
	{
		...
		if (bIsStructWithCustomSerialization)
		{
			...
			{
				UE_LOGF(LogIris, Warning, "Generating descriptor for struct %ls that has custom serialization.", ToCStr(InStruct->GetName()));
			}
		}
		...
		if (bAllMembersAreReplicated && PropertyCount == 0U)
		{
			...
			UE_CLOGF(bWarnAboutStructPropertiesWithSuspectedNotReplicatedProperties, LogIris, Warning, "FReplicationStateDescriptorBuilder ::CreateDescriptorForStruct a replicated struct WITH no replicated properties must have a CustomNetSerializer. Struct: %ls.", ToCStr(InStruct->GetName()));
			bAllMembersAreReplicated = false;
		}
	}
...
SIZE_T FReplicationStateDescriptorBuilder::CreateDescriptorsForClass(FResult& CreatedDescriptors, UClass* InObjectClass, const FParameters& Parameters)
{
	...
	{
		UE_LOGF(LogIris, Error, "Trying to create descriptor for invalid class %ls", *GetNameSafe(InObjectClass));
		return 0U;
	}
		...
		if ((bIncludeSuper || Property->GetOwnerVariant() == InObjectClass))
		{
			if (FPropertyReplicationStateDescriptorBuilder::IsSupportedProperty(MemberProperty, Property, IsSupportedPropertyParams))
			{
				...
				else if (EnumHasAnyFlags(MemberProperty.Traits, EMemberPropertyTraits::IsFastArray))
				{
					...
					UE_LOGF(LogIris, Error, "FReplicationStateDescriptorBuilder::CreateDescriptorsForClass FFastArray property %ls not registered and won't be replicated.", *Property->GetFullName());
	...
	// Add special Actor properties
	{
		...
		if (InObjectClass->IsChildOf(ActorClass))
		{
			...
			{
				UE_LOGF(LogIris, Error, "FPropertyReplicationStateDescriptorBuilder: Failed to find expected property %ls.%ls", ToCStr(InObjectClass->GetName()), *FName(ReplicationStateDescriptorBuilder_NAME_NetCullDistanceSquared).ToString());
				ensure(false);
			}
		}
	}
