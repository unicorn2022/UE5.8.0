...
struct FDefaultOptionalPropertyNetSerializerInfo : public FPropertyNetSerializerInfo
{
	virtual const FFieldClass* GetPropertyTypeClass() const override { return FOptionalProperty::StaticClass(); }
	virtual bool IsSupported(const FProperty* Property) const override { return FPropertyNetSerializerInfoRegistry::FindSerializerInfo(CastFieldChecked<FOptionalProperty>(Property)->GetValueProperty()) != nullptr; }
	virtual const FNetSerializer* GetNetSerializer(const FProperty* Property) const override { return &UE_NET_GET_SERIALIZER(FOptionalPropertyNetSerializer); }
	virtual bool CanUseDefaultConfig(const FProperty* Property) const override { return false; }
	virtual FNetSerializerConfig* BuildNetSerializerConfig(void* NetSerializerConfigBuffer, const FProperty* Property) const override { return nullptr; }
};
UE_NET_IMPLEMENT_NETSERIALIZER_INFO(FDefaultOptionalPropertyNetSerializerInfo);

/** Last resort for properties that have no specialized serializer. An example is TextProperty. */
FLastResortPropertyNetSerializerInfo::FLastResortPropertyNetSerializerInfo(const FLastResortPropertyNetSerializerUserOptions& UserOptions)
: FPropertyNetSerializerInfo()
, CreateAndRegisterReplicationFragmentFunction(UserOptions.CreateAndRegisterReplicationFragmentFunction)
, MaxQuantizedSizeBits(UserOptions.MaxQuantizedSizeBits)
, bExcludeFromDefaultStateHash(UserOptions.bExcludeFromDefaultStateHash)
{
}
...
FNetSerializerConfig* FLastResortPropertyNetSerializerInfo::BuildNetSerializerConfig(void* NetSerializerConfigBuffer, const FProperty* Property) const
{
	...
	Config->MaxQuantizedSizeBits = MaxQuantizedSizeBits > 0U ? MaxQuantizedSizeBits : FLastResortPropertyNetSerializerConfig::DefaultMaxQuantizedSizeBits;
...
const FPropertyNetSerializerInfo* FPropertyNetSerializerInfoRegistry::FindSerializerInfo(const FProperty* Property)
{
	...
	// Special handling for optional properties required
	if (Property->IsA(FOptionalProperty::StaticClass()))
	{
		const FPropertyNetSerializerInfo& Info = GetPropertyNetSerializerInfo_FDefaultOptionalPropertyNetSerializerInfo();
		if (Info.IsSupported(Property))
		{
			return &Info;
		}
