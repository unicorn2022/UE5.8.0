...
bool FGuidReferences::UpdateUnmappedGUIDs(UPackageMap* InPackageMap, UObject* OriginalObject, const FProperty* Property, int32 AbsOffset)
{
	...
	for (auto UnmappedIt = UnmappedGUIDs.CreateIterator(); UnmappedIt; ++UnmappedIt)
	{
		...
		{
			UE_LOGF(LogNetCore, Warning, "FGuidReferences::UpdateUnmappedGUIDs: Broken GUID. NetGuid: %ls", *GUID.ToString());
		...
		if (Object != nullptr)
		{
			UE_LOGF(LogNetCore, VeryVerbose, "FGuidReferences::UpdateUnmappedGUIDs: REMOVED unmapped property: Offset: %i, Guid: %ls, PropName: %ls, ObjName: %ls", AbsOffset, *GUID.ToString(), *Property->GetName(), *Object->GetName());
