...
void FNetAnalyticsAggregator::InitConfig()
{
	...
	UE_CLOGF(AnalyticsDataMap.Num() > 0, LogNetCore, Warning,
			"Net Analytics hotfixed while already active. Analytics hotfix changes may not be applied correctly.");
	...
	if (CurConfig != nullptr)
	{
		for (const FNetAnalyticsDataConfig& CurEntry : CurConfig->NetAnalyticsData)
		{
			...
			UE_LOGF(LogNetCore, Log, "Adding NetAnalyticsData: %ls, bEnabled: %i", *CurEntry.DataName.ToString(),
					CurEntry.bEnabled);
		}
	}
}
																					...
																					FString InTypeName)
{
	...
	{
		UE_LOGF(LogNetCore, Error, "NetAnalyticsData type '%ls' must be added to NetAnalyticsAggregatorConfig, for NetDriverName: %ls.",
