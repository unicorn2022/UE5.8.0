...
UE_TRACE_CHANNEL_DEFINE(NetChannel, "Network connections, packets (sent/received/dropped), packet content, replicated object lifecycle, and per-connection stats. Viewable in the Network Insights profiler.")
...
// Trace a name
UE_TRACE_EVENT_BEGIN(NetTrace, NameEvent, Important | NoSync)
	UE_TRACE_EVENT_FIELD(uint32, NameId)
	UE_TRACE_EVENT_FIELD(UE::Trace::AnsiString, Name)
UE_TRACE_EVENT_END()

UE_TRACE_EVENT_BEGIN(NetTrace, ObjectCreatedEvent, NoSync)
...
// StatsCounterEvent
UE_TRACE_EVENT_BEGIN(NetTrace, PacketStatsCounterEvent, NoSync)
...
UE_TRACE_EVENT_BEGIN(NetTrace, FrameStatsCounterEvent, NoSync)
	UE_TRACE_EVENT_FIELD(uint64, Timestamp)
	UE_TRACE_EVENT_FIELD(uint32, StatsValue)
	UE_TRACE_EVENT_FIELD(uint32, NameId)
	UE_TRACE_EVENT_FIELD(uint8, GameInstanceId)
...
UE_TRACE_EVENT_BEGIN(NetTrace, PacketContentEvent, NoSync)
	UE_TRACE_EVENT_FIELD(uint16, ConnectionId)
	UE_TRACE_EVENT_FIELD(uint8, GameInstanceId)
...
{
	UE_TRACE_LOG(NetTrace, NameEvent, NetChannel, NameSize * sizeof(char))
