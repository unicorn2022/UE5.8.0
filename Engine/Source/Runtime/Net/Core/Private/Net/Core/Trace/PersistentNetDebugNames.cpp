...
FPersistentNetDebugNames& FPersistentNetDebugNames::Get()
{
	...
	if (!Singleton.bIsInitialized)
	{
		...
		Private::ReliabilityNetDebugNames[0] = UE::Net::CreatePersistentNetDebugName(TEXT("Unreliable"));
		Private::ReliabilityNetDebugNames[1] = UE::Net::CreatePersistentNetDebugName(TEXT("Reliable"));
#endif
	}
...
FNetDebugName const* GetReliabilityNetDebugName(bool bIsReliable)
{
	return Private::ReliabilityNetDebugNames[bIsReliable ? 1 : 0];
}
