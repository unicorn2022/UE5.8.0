...
void FDDoSDetection::InitConfig()
{
	...
	UE_LOGF(LogNetCore, Log, "DDoS detection status: detection enabled: %d analytics enabled: %d", bDDoSDetection, bDDoSAnalytics);
	...
	if (bDDoSDetection)
	{
		...
		for (const FString& CurCategory : SeverityCatagories)
		{
			...
			{
				UE_LOGF(LogNetCore, Warning, "DDoS detection could not find ini section: %ls", *CurSection);
			}
		}
		...
		{
			UE_LOGF(LogNetCore, Warning, "DDoS detection enabled, but no DetectionSeverity states specified! Disabling.");
...
void FDDoSDetection::UpdateSeverity(bool bEscalate)
{
	...
	if (NewState != ActiveState)
	{
		...
		UE_LOGF(LogNetCore, Warning, "Updated DDoS detection severity from '%ls' to '%ls'",
	...
	if (bDDoSDetection)
	{
		...
		if (HitchTimeQuotaMS > 0 && EndFrameRecvTimestamp != 0.0)
		{
			...
			if ((((int32)HitchTimeMS) - HitchTimeQuotaMS) > 0)
			{
				...
				UE_LOGF(LogNetCore, Verbose, "Detected '%i' successive hitches outside NetDriver Tick. Last Hitch: %fms (Max: %ims)",
		...
		{
			UE_CLOGF(DroppedPacketCounter > 0, LogNetCore, Warning,
				"DDoS Detection dropped '%i' packets during last second (bHitFrameNonConnLimit: %i, bHitFrameNetConnLimit: %i, " "DetectionSeverity: %ls).",
		...
		{
			UE_LOGF(LogNetCore, Warning, "Previous frame hit DDoS LogHitCounter limit - hit count: %i (Max: %i)", LogHitCounter,
					DDoSLogSpamLimit);
		}
