...
namespace UEPushModelPrivate
{
	...
	public:
		...
		bool ValidateObjectIdReassignment(FNetLegacyPushObjectId CurrentId, FNetLegacyPushObjectId NewId)
		{
			if (!PerObjectStates.IsValidIndex(CurrentId))
			{
				...
				UE_LOGF(LogPushModel, Log, "ValidateObjectIdReassignment:  CurrentId is invalid.");
				return true;
			}
			if (!PerObjectStates.IsValidIndex(NewId))
			{
				...
				UE_LOGF(LogPushModel, Fatal, "ValidateObjectIdReassignment: NewId is invalid.");
				return false;
			}
			...
			if (CurrentObjectState.GetObjectKey() != NewObjectState.GetObjectKey())
			{
				...
				UE_LOGF(LogPushModel, Log, "ValidateObjectIdReassignment: IDs point to different objects. CurrentObject = %ls, NewObject = %ls",
			...
			UE_LOGF(LogPushModel, Warning, "ValidObjectIdReassignment: IDs point to the same object. Object=%ls",
		...
		{
			UE_LOGF(LogPushModel, Display, "Starting print of %u objects currently assigned a PushModel handle", ObjectKeyToInternalId.Num());
			for (auto It=ObjectKeyToInternalId.CreateConstIterator(); It; ++It)
			{
				...
				UE_LOGF(LogPushModel, Display, "Object: %ls (%ls) | PushObjectId: 0x%x", *GetNameSafe(ObjPtr), IsValid(ObjPtr)?TEXT("Valid"):TEXT("Invalid"), Value);
			}
		}
	...
	{
		UE_LOGF(LogPushModel, Log, "PushModel HandleCreation is now %ls", bAllow ? TEXT("enabled") : TEXT("disabled"));
