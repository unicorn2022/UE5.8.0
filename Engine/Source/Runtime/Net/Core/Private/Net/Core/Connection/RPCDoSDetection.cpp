...
int32 GMaxBlockedRPCLogs = 16;

FAutoConsoleVariableRef CVarMaxBlockedRPCLogs(
	TEXT("net.MaxBlockedRPCLogs"),
	GMaxBlockedRPCLogs,
	TEXT("The maximum number of individual blocked RPC log messages per tick, before switching to aggregated per-tick logging."));
...
#if RPC_QUOTA_DEBUG
	UE_CLOGF(bReturnVal, LogNetCore, Log, "HasHitQuota_Count: Hit Frame Quota: RPCCounter: %i, Limit: %i",
...
#if RPC_QUOTA_DEBUG
		UE_CLOGF(bReturnVal, LogNetCore, Log, "HasHitQuota_Count: Hit Period Quota: RPCsPerPeriodCounter: %i, Limit: %i",
...
bool FRPCDoSState::HasHitQuota_Time(const FRPCDoSCounters(&PerPeriodHistory)[16], FRPCDoSCounters& InFrameCounter) const
{
	...
	UE_CLOGF(bReturnVal, LogNetCore, Log,
			"HasHitQuota_Time: Hit Frame Quota: AccumRPCTime: %f, DebugAccumRPCTime: %f, Limit: %f (%i ms)",
			InFrameCounter.AccumRPCTime, InFrameCounter.DebugAccumRPCTime, EscalateTimeQuotaSecsPerFrame,
			EscalateTimeQuotaMSPerFrame);

	UE_CLOGF(bReturnVal ^ bDebugReturnVal, LogNet, Warning,
			"HasHitQuota_Time: Approximate vs Debug Quota mismatch: AccumRPCTime: %f, DebugAccumRPCTime: %f, Limit: %f (%i ms)",
			InFrameCounter.AccumRPCTime, InFrameCounter.DebugAccumRPCTime, EscalateTimeQuotaSecsPerFrame,
			EscalateTimeQuotaMSPerFrame);
#endif
	...
	if (EscalateTimeQuotaMSPerPeriod > 0 && !bReturnVal)
	{
		...
		UE_CLOGF(bReturnVal, LogNetCore, Log,
				"HasHitQuota_Time: Hit Period Quota: AccumPerPeriodRPCTime: %f, DebugAccumPerPeriodRPCTime: %f, Limit: %f (%i ms)",
		...
		UE_CLOGF(bReturnVal ^ bDebugReturnVal, LogNetCore, Warning,
				"HasHitQuota_Time: Approximate vs Debug Period Quota mismatch: " "AccumPerPeriodRPCTime: %f, DebugAccumPerPeriodRPCTime: %f, Limit: %f (%i ms)",
				(PeriodCounter.AccumRPCTime + InFrameCounter.AccumRPCTime),
				(PeriodCounter.DebugAccumRPCTime + InFrameCounter.DebugAccumRPCTime),
				EscalateTimeQuotaSecsPerPeriod, EscalateTimeQuotaMSPerPeriod);
#endif
		...
		for (const FString& CurVar : StructVars)
		{
			...
			if (CurVar.Split(TEXT("="), &Var, &Value))
			{
				...
				if (CurProp != nullptr)
				{
					...
					{
						UE_LOGF(LogNetCore, Error, "FRPCDoSStateConfig: Ini property '%ls' is not a config property.", *Var);
					}
				}
				else
				{
					UE_LOGF(LogNetCore, Error, "FRPCDoSStateConfig: Ini property '%ls' not found.", *Var);
				}
			}
		}
...
void FRPCDoSStateConfig::ValidateConfig()
{
	auto ValidateTimePeriod = [](int8& Value, const TCHAR* PropertyName, bool bMustBeSet)
		{
			...
			{
				UE_LOGF(LogNetCore, Warning, "FRPCDoSStateConfig: %ls '%i' must be between %i and 16. Clamping.",
						PropertyName, Value, BaseValue);

				Value = static_cast<int8>(FMath::Clamp((int32)Value, BaseValue, 16));
			}
		};
	...
	{
		UE_LOGF(LogNetCore, Warning, "FRPCDoSStateConfig: AutoEscalateTime must be larger than CooloffTime.");
	}
}
...
void FRPCDoSDetection::Init(FName NetDriverName, TSharedPtr<FNetAnalyticsAggregator>& AnalyticsAggregator,
							TSharedPtr<FRPCDoSAnalyticsSender> AnalyticsSender, FGetRPCDoSAddress&& InAddressFunc,
							FGetRPCDoSPlayerUID&& InPlayerUIDFunc, FRPCDoSKickPlayer&& InKickPlayerFunc)
{
	...
	if (bRPCDoSDetection && bRPCDoSAnalytics && AnalyticsAggregator.IsValid())
	{
		...
		{
			RPCDoSAnalyticsData->SetAnalyticsSender(AnalyticsSender);
		}
	}
...
void FRPCDoSDetection::InitConfig(FName NetDriverName)
{
	...
	if (bRPCDoSDetection)
	{
		...
		if (CurConfigObj != nullptr)
		{
			...
			for (const FString& CurCategory : SeverityCategories)
			{
				...
				if (GConfig->DoesSectionExist(*CurSection, GEngineIni))
				{
					...
					{
						UE_LOGF(LogNetCore, Warning, "RPC DoS Detection failed to load ini section: %ls", *CurSection);
					}
				}
				else
				{
					UE_LOGF(LogNetCore, Warning, "RPC DoS detection could not find ini section: %ls", *CurSection);
				}
			}
		}
		...
		{
			UE_LOGF(LogNetCore, Warning, "RPC DoS detection enabled, but no DetectionSeverity states specified! Disabling.");
...
void FRPCDoSDetection::UpdateSeverity_Private(ERPCDoSSeverityUpdate Update, ERPCDoSEscalateReason Reason)
{
	...
	if (NewStateIdx != ActiveState)
	{
		...
		while (true)
		{
			...
			ActiveState = static_cast<int8>(NewStateIdx);
		...
		if (bLogEscalate)
		{
			UE_LOGF(LogNetCore, Warning, "Updated RPC DoS detection severity for '%ls' from '%ls' to '%ls' (Reason: %ls)",
		...
		if (bEscalate)
		{
			...
			if (ActiveState > WorstAnalyticsState && !NewState.bEscalationConfirmed)
			{
				...
				if (NewState.bEscalationConfirmed)
				{
					...
					if (bLogEscalate)
					{
						UE_LOGF(LogNetCore, Warning, "Updated analytics RPC DoS detection severity from '%ls' to '%ls'.",
						...
						if (UntimedRPCs.Num() > 0)
						{
							...
							UE_LOGF(LogNetCore, Log, "RPC DoS escalation Untimed RPC's: Time: %f, RPC's: %ls",
			...
			{
				UE_LOGF(LogNetCore, Warning, "RPC DoS detection kicking player IP '%ls', UID '%ls'.", *GetPlayerAddress(),
						*GetPlayerUID());
...
void FRPCDoSDetection::TrackBlockedRPC(FName RPCName)
{
	if (RPCName != CachedBlockedRPCName)
	{
		CachedBlockedRPCName = RPCName;
		CachedBlockedRPCCount = &BlockedRPCCounts.FindOrAdd(RPCName, 0);
	}

	(*CachedBlockedRPCCount)++;

	if (BlockedRPCLogCounter < GMaxBlockedRPCLogs)
	{
		BlockedRPCLogCounter++;

		UE_LOGF(LogNetCore, Log, "Blocked RPC: %ls", ToCStr(RPCName.ToString()));
	}
}

void FRPCDoSDetection::FlushBlockedRPCLogs()
{
	if (BlockedRPCLogCounter >= GMaxBlockedRPCLogs && BlockedRPCCounts.Num() > 0)
	{
		TStringBuilder<2048> AggregateStr;

		AggregateStr.Append(TEXT("Blocked RPCs: "));

		bool bFirst = true;

		for (TPair<FName, int32>& Pair : BlockedRPCCounts)
		{
			if (Pair.Value > 0)
			{
				if (!bFirst)
				{
					AggregateStr.Append(TEXT(", "));
				}

				AggregateStr.Append(ToCStr(Pair.Key.ToString()));
				AggregateStr.Appendf(TEXT(" (%i)"), Pair.Value);
		...
		UE_LOGF(LogNetCore, Log, "%ls", ToCStr(AggregateStr.ToString()));
	}

	BlockedRPCLogCounter = 0;
}

void FRPCDoSDetection::ResetBlockedRPCTracking()
{
	BlockedRPCCounts.Empty();
	CachedBlockedRPCName = NAME_None;
	CachedBlockedRPCCount = nullptr;
	BlockedRPCLogCounter = 0;
}
...
ERPCNotifyResult FRPCDoSDetection::CheckRPCTracking(const UFunction* Function, FName FunctionName)
{
...
void FRPCDoSDetection::EnableRPCTracking(double TimeSeconds)
{
	...
	if (SequentialRPCScopePrivate.IsActive())
	{
		...
		const UFunction* FirstFunc = LightweightRPCTracking.RPC[0].Function;
...
void FRPCDoSDetection::EnableForcedRPCTracking(const UFunction* Function, FName FunctionName, double TimeSeconds)
{
