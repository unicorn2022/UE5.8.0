...
struct FFastArraySerializerItem
{
	 ...
	 * NOTE: intentionally not virtual; invoked via templated code, @see FExampleItemEntry
	 */
	inline FString GetDebugString() { return FString(); }
};
...
struct FFastArraySerializer
{
	...
	*/
	...
	void SetDeltaSerializationEnabled(const bool bEnabled)
	{
		...
		{
			UE_LOGF(LogNetFastTArray, Log, "FFastArraySerializer::SetDeltaSerializationEnabled - Called after array has been serialized. Ignoring");
		}
	}
...
void FFastArraySerializer::TFastArraySerializeHelper<Type, SerializerType>::ConditionalRebuildItemMap()
{
	if ((Parms.bUpdateUnmappedObjects || Parms.Writer == NULL) && ArraySerializer.ItemMap.Num() != Items.Num())
	{
		...
		UE_LOGF(LogNetFastTArray, Log, "FastArrayDeltaSerialize: Recreating Items map. Struct: %ls, Items.Num: %d Map.Num: %d", *Struct->GetOwnerStruct()->GetName(), Items.Num(), ArraySerializer.ItemMap.Num());
		...
		for (int32 i = 0; i < Items.Num(); ++i)
		{
			if (Items[i].ReplicationID == INDEX_NONE)
			{
				if (Parms.Writer)
				{
					UE_LOGF(LogNetFastTArray, Warning, "FastArrayDeltaSerialize: Item with uninitialized ReplicationID. Struct: %ls, ItemIndex: %i", *Struct->GetOwnerStruct()->GetName(), i);
				}
				else
				{
...
void FFastArraySerializer::TFastArraySerializeHelper<Type, SerializerType>::ConditionalLogSerializerState(const TMap<int32, int32>* OldIDToKeyMap) const
{
	...
	if (UE_LOG_ACTIVE(LogNetFastTArray, Log))
	{
		...
		UE_LOGF(LogNetFastTArray, Log, "%ls", *CurrentState);
		...
		UE_LOGF(LogNetFastTArray, Log, "%ls", *ClientStateStr);
	}
}
...
bool FFastArraySerializer::TFastArraySerializeHelper<Type, SerializerType>::ConditionalCreateNewDeltaState(const TMap<int32, int32>& OldIDToKeyMap, const int32 BaseReplicationKey)
{
	if (ArraySerializer.ArrayReplicationKey == BaseReplicationKey)
	{
		...
		{
			UE_LOGF(LogNetFastTArray, Warning, "OldMap size (%d) does not match item count (%d) for struct (%ls), missing a MarkArrayDirty on element add/remove?", OldIDToKeyMap.Num(), ArraySerializer.CachedNumItemsToConsiderForWriting, *Struct->GetOwnerStruct()->GetName());
		}
	...
	TArray<FFastArraySerializer_FastArrayDeltaSerialize_FIdxIDPair, TInlineAllocator<8>>& ChangedElements) const
{
	...
	for (int32 i = 0; i < Items.Num(); ++i)
	{
		...
		UE_LOGF(LogNetFastTArray, Log, "    Array[%d] - ID %d. CL %d.", i, Item.ReplicationID, Item.ReplicationKey);
		if (!ArraySerializer.template ShouldWriteFastArrayItem<Type, SerializerType>(Item, Parms.bIsWritingOnClient))
		{
		...
		if (Item.ReplicationID == INDEX_NONE)
		{
			UE_LOGF(LogNetFastTArray, Log, "    FastArraySerializer::BuildChangedAndDeletedBuffersFromDefault Item with uninitialized ReplicationID detected in. Struct: %ls, ItemIndex: %i", *Parms.DebugName, i);
			continue;
		}

		NewIDToKeyMap.Add(Item.ReplicationID, Item.ReplicationKey);

		UE_LOGF(LogNetFastTArray, Log, "       New! Element ID: %d. %ls", Item.ReplicationID, *Item.GetDebugString());
		// New
	...
	TArray<int32, TInlineAllocator<8>>& DeletedElements)
{
	...
	UE_LOGF(LogNetFastTArray, Log, "NetSerializeItemDeltaFast: %ls. DeleteCount: %d", *Parms.DebugName, DeleteCount);
	...
	for (int32 i = 0; i < Items.Num(); ++i)
	{
		...
		if (OldValuePtr)
		{
			...
			{
				UE_LOGF(LogNetFastTArray, Log, "       Stayed The Same - Skipping");
			...
			{
				UE_LOGF(LogNetFastTArray, Log, "       Changed! Was: %d. Element ID: %d. %ls", *OldValuePtr, Item.ReplicationID, *Item.GetDebugString());
	...
	if (DeleteCount > 0 && OldIDToKeyMap)
	{
		for (auto It = OldIDToKeyMap->CreateConstIterator(); It; ++It)
		{
			...
			{
				UE_LOGF(LogNetFastTArray, Log, "   Deleting ID: %d", It.Key());
...
void FFastArraySerializer::TFastArraySerializeHelper<Type, SerializerType>::WriteDeltaHeader(FFastArraySerializerHeader& Header) const
{
	...
	UE_LOGF(LogNetFastTArray, Log, "   Writing Bunch. NumChange: %d. NumDel: %d [%d/%d]",
	...
	UE_CLOGF(NumDeletes > MaxNumDeleted, LogNetFastTArray, Warning, "NumDeletes > GetMaxNumberOfAllowedDeletionsPerUpdate: %d > %d. (Write)", NumDeletes, MaxNumDeleted);
	UE_CLOGF(Header.NumChanged > MaxNumChanged, LogNetFastTArray, Warning, "NumChanged > GetMaxNumberOfAllowedChangesPerUpdate: %d > %d. (Write)", Header.NumChanged, MaxNumChanged);
	...
	for (auto It = Header.DeletedIndices.CreateIterator(); It; ++It)
	{
		int32 ID = *It;
		Writer << ID;
		UE_LOGF(LogNetFastTArray, Log, "   Deleted ElementID: %d", ID);
	}
}
...
bool FFastArraySerializer::TFastArraySerializeHelper<Type, SerializerType>::ReadDeltaHeader(FFastArraySerializerHeader& Header) const
{
	...
	UE_LOGF(LogNetFastTArray, Log, "Received [%d/%d].", Header.ArrayReplicationKey, Header.BaseReplicationKey);

	const int32 MaxNumDeleted = FFastArraySerializer::GetMaxNumberOfAllowedDeletionsPerUpdate();
	if (NumDeletes > MaxNumDeleted)
	{
		UE_LOGF(LogNetFastTArray, Warning, "NumDeletes > GetMaxNumberOfAllowedDeletionsPerUpdate: %d > %d. (Read)", NumDeletes, MaxNumDeleted);
		Reader.SetError();
		return false;
	}
	...
	{
		UE_LOGF(LogNetFastTArray, Warning, "NumChanged > GetMaxNumberOfAllowedChangesPerUpdate: %d > %d. (Read)", Header.NumChanged, MaxNumChanged);
		Reader.SetError();
		return false;
	}

	UE_LOGF(LogNetFastTArray, Log, "Read NumChanged: %d NumDeletes: %d.", Header.NumChanged, NumDeletes);
	...
	if (NumDeletes > 0)
	{
		for (int32 i = 0; i < NumDeletes; ++i)
		{
			...
			if (ElementIndexPtr)
			{
				...
				UE_LOGF(LogNetFastTArray, Log, "   Adding ElementID: %d for deletion", ElementID);
			}
			else
			{
				UE_LOGF(LogNetFastTArray, Log, "   Couldn't find ElementID: %d for deletion!", ElementID);
			}
		}
	}
	...
	GuidMapType& GuidMap)
{
		...
		for (int32 idx = 0; idx < Items.Num(); ++idx)
		{
			...
			if (Item.MostRecentArrayReplicationKey < Header.ArrayReplicationKey && Item.MostRecentArrayReplicationKey > Header.BaseReplicationKey)
			{
				...
				if (!Header.DeletedIndices.Contains(idx))
				{
					...
					UE_LOGF(LogNetFastTArray, Log, "Adding implicit delete for ElementID: %d. MostRecentArrayReplicationKey: %d. Current Payload: [%d/%d]",
	...
	{
		UE_LOGF(LogNetFastTArray, Error, "Item size changed after PreReplicatedRemove! PremoveSize: %d  Item.Num: %d",
	...
	if (PreRemoveSize != Items.Num())
	{
		UE_LOGF(LogNetFastTArray, Error, "Item size changed after PostReplicatedAdd/PostReplicatedChange! PremoveSize: %d  Item.Num: %d",
			PreRemoveSize, Items.Num());
	}
	...
	if (Header.DeletedIndices.Num() > 0)
	{
		...
		for (int32 i = Header.DeletedIndices.Num() - 1; i >= 0; --i)
		{
			...
			if (Items.IsValidIndex(DeleteIndex))
			{
				...
				UE_LOGF(LogNetFastTArray, Log, "   Deleting: %d", DeleteIndex);
			}
		}
...
bool FFastArraySerializer::FastArrayDeltaSerialize(TArray<Type> &Items, FNetDeltaSerializeInfo& Parms, SerializerType& ArraySerializer)
{
	...
	UE_LOGF(LogNetFastTArray, Log, "FastArrayDeltaSerialize for %ls. %ls. %ls", *InnerStruct->GetName(), *InnerStruct->GetOwnerStruct()->GetName(), Parms.Reader ? TEXT("Reading") : TEXT("Writing"));
	...
	{
		UE_LOGF(LogNetFastTArray, Log, "FastArrayDeltaSerialize for %ls. %ls. GatherGuidReferences", *InnerStruct->GetName(), *InnerStruct->GetOwnerStruct()->GetName());
	...
	{
		UE_LOGF(LogNetFastTArray, Log, "FastArrayDeltaSerialize for %ls. %ls. MovedGuidToUnmapped", *InnerStruct->GetName(), *InnerStruct->GetOwnerStruct()->GetName());
	...
	{
		UE_LOGF(LogNetFastTArray, Log, "FastArrayDeltaSerialize for %ls. %ls. UpdateUnmappedObjects", *InnerStruct->GetName(), *InnerStruct->GetOwnerStruct()->GetName());
		...
		for ( auto It = ArraySerializer.GuidReferencesMap.CreateIterator(); It; ++It )
		{
			...
			for ( auto UnmappedIt = GuidReferences.UnmappedGUIDs.CreateIterator(); UnmappedIt; ++UnmappedIt )
			{
				...
				if ( Parms.Map->IsGUIDBroken( GUID, false ) )
				{
					...
					UE_LOGF( LogNetFastTArray, Warning, "FastArrayDeltaSerialize: Broken GUID. NetGuid: %ls", *GUID.ToString() );
	...
	if (Parms.Writer)
	{
		UE_LOGF(LogNetFastTArray, Log, "FastArrayDeltaSerialize for %ls. %ls. Writing", *InnerStruct->GetName(), *InnerStruct->GetOwnerStruct()->GetName());
		...
		*Parms.NewState = MakeShareable( NewState );
		...
		for (auto It = ChangedElements.CreateIterator(); It; ++It)
		{
			...
			UE_LOGF(LogNetFastTArray, Log, "   Changed ElementID: %d", ID);
	...
	{
		UE_LOGF(LogNetFastTArray, Log, "FastArrayDeltaSerialize for %ls. %ls. Reading", *InnerStruct->GetName(), *InnerStruct->GetOwnerStruct()->GetName());
		...
		for(int32 i = 0; i < Header.NumChanged; ++i)
		{
			...
			if (!ElementIndexPtr)
			{
				UE_LOGF(LogNetFastTArray, Log, "   New. ID: %d. New Element!", ElementID);
			...
			{
				UE_LOGF(LogNetFastTArray, Log, "   Changed. ID: %d -> Idx: %d", ElementID, *ElementIndexPtr);
			...
			if ( Reader.IsError() )
			{
				UE_LOGF( LogNetFastTArray, Warning, "Parms.NetSerializeCB->NetSerializeStruct: Reader.IsError() == true" );
				return false;
			}
		}
...
bool FFastArraySerializer::FastArrayDeltaSerialize_DeltaSerializeStructs(TArray<Type>& Items, FNetDeltaSerializeInfo& Parms, SerializerType& ArraySerializer)
{
	...
	if (Parms.GatherGuidReferences)
	{
		UE_LOGF(LogNetFastTArray, Log, "FastArrayDeltaSerialize_DeltaSerializeStruct for %ls. %ls. GatherGuidReferences", *InnerStruct->GetName(), *InnerStruct->GetOwnerStruct()->GetName());
		Parms.NetSerializeCB->GatherGuidReferencesForFastArray(DeltaSerializeParams);
		return true;
	}
	else if (Parms.MoveGuidToUnmapped)
	{
		UE_LOGF(LogNetFastTArray, Log, "FastArrayDeltaSerialize_DeltaSerializeStruct for %ls. %ls. MoveGuidToUnmapped", *InnerStruct->GetName(), *InnerStruct->GetOwnerStruct()->GetName());
		return Parms.NetSerializeCB->MoveGuidToUnmappedForFastArray(DeltaSerializeParams);
	}
	else if (Parms.bUpdateUnmappedObjects)
	{
		UE_LOGF(LogNetFastTArray, Log, "FastArrayDeltaSerialize_DeltaSerializeStruct for %ls. %ls. UpdateUnmappedObjects", *InnerStruct->GetName(), *InnerStruct->GetOwnerStruct()->GetName());
	...
	{
		UE_LOGF(LogNetFastTArray, Log, "FastArrayDeltaSerialize_DeltaSerializeStruct for %ls. %ls. Writing", *InnerStruct->GetName(), *InnerStruct->GetOwnerStruct()->GetName());
	...
	{
		UE_LOGF(LogNetFastTArray, Log, "FastArrayDeltaSerialize_DeltaSerializeStruct for %ls. %ls. Reading", *InnerStruct->GetName(), *InnerStruct->GetOwnerStruct()->GetName());
