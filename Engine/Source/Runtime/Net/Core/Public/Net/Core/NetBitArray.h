...
private:
	...
	template<typename T> static void ForAllSetBits(const StorageWordType* Storage, const uint32 WordCount, const uint32 BitCount, T&& Functor)
	{
		...
		for (uint32 WordIt = 0, CurrentBitIndex = 0; WordIt < WordCount; ++WordIt, CurrentBitIndex += WordBitCount)
		{
			...
			// Invoke functor for all set bits in the CurrentWord
			while (CurrentWord)
			{
				const uint32 BitIndex = CurrentBitIndex + FPlatformMath::CountTrailingZeros(CurrentWord);
				Functor(BitIndex);
	...
	template<typename T> static void ForAllSetBitsInRange(const StorageWordType* Storage, const uint32 TotalWordCount, const uint32 TotalBitCount, const uint32 StartBit, const uint32 InEndBit, T&& Functor)
	{
		const uint32 EndBit = FPlatformMath::Min(InEndBit, TotalBitCount - 1U);
		if ((TotalBitCount == 0U) || (StartBit >= TotalBitCount) || (EndBit < StartBit))
		{
			return;
		}
