 ...
 */
DECLARE_MULTICAST_DELEGATE_TwoParams(FModifyRPCDoSAnalytics, UObject* /*World*/, TArray<FAnalyticsEventAttribute>& /*InOutAttrs*/);


// Globals

/** Globally accessible multicast delegate, for modifying/adding-to the RPC DoS analytics event */
NETCORE_API extern FModifyRPCDoSAnalytics GModifyRPCDoSAnalytics;

/** Globally accessible multicast delegate, for modifying/adding-to the RPC DoS escalation analytics event */
NETCORE_API extern FModifyRPCDoSAnalytics GModifyRPCDoSEscalationAnalytics;
...
public:
	...
	/** Default constructor */
	NETCORE_API FRPCDoSAnalyticsVars();

	NETCORE_API bool operator == (const FRPCDoSAnalyticsVars& A) const;
...
/**
 * Class which is responsible for sending the actual analytics events (for implementation outside of NetCore, to avoid Analytics dependency)
 */
class FRPCDoSAnalyticsSender
{
public:
	virtual void SendAnalytics(FNetAnalyticsAggregator* Aggregator, FRPCDoSAnalyticsData* Data) {}
...
{
	/** Implementation responsible for the analytics send event */
	TSharedPtr<FRPCDoSAnalyticsSender> AnalyticsSender;
...
public:
	using FRPCDoSAnalyticsVars::operator ==;

	void SetAnalyticsSender(TSharedPtr<FRPCDoSAnalyticsSender> InSender);

	NETCORE_API FRPCDoSAnalyticsVars* GetVars();
