...
public:
	...
	~FNetTraceNullEventScope() = default;
...
struct FNetTrace
{
	...
	NETCORE_API static void SetupInstanceForNetTraceId(uint32 InNetTraceId);

	NETCORE_API static void FinishConnectionTick(const uint32 GameInstanceId);
...
public:
	...
	inline bool CanBeginPacketContentEvent() const { return CurrentNestingLevel < (MaxNestingLevel - 1); }
...
{
	if (Collector && Collector->CanBeginPacketContentEvent())
	{
...
#define UE_NET_TRACE_INTERNAL_BUNCH_SCOPE(Collector, Bunch, ...) FNetTraceBunchScope<decltype(Bunch)> UE_JOIN(NetTraceBunchScope, __LINE__)(Bunch, __VA_ARGS__, Collector)

#define UE_NET_TRACE_INTERNAL_SCOPE(Name, Stream, Collector, Verbosity) \
	auto UE_JOIN(NetTraceNameFunc_, __LINE__) = []() { static const UE::Net::FNetDebugName* Name = UE::Net::CreatePersistentNetDebugName(TEXT(#Name)); return FNetTrace::TraceName(Name); }; \
	FNetTrace::TChooseTraceEventScope<Verbosity, decltype(Stream)>::Type UE_JOIN(NetTraceScope, __LINE__)(UE_JOIN(NetTraceNameFunc_, __LINE__), Stream, Collector, Verbosity)

#define UE_NET_TRACE_INTERNAL_OBJECT_SCOPE(HandleOrNetGUID, Stream, Collector, Verbosity) \
	FNetTrace::TChooseTraceEventScope<Verbosity, decltype(Stream)>::Type UE_JOIN(NetTraceScope, __LINE__)(GetObjectIdForNetTrace(HandleOrNetGUID), Stream, Collector, Verbosity)

#define UE_NET_TRACE_INTERNAL_DYNAMIC_NAME_SCOPE(Name, Stream, Collector, Verbosity) \
	auto UE_JOIN(NetTraceDynamicNameFunc_, __LINE__) = [&]() { return FNetTrace::TraceName(Name); }; \
	FNetTrace::TChooseTraceEventScope<Verbosity, decltype(Stream)>::Type UE_JOIN(NetTraceScope, __LINE__)(UE_JOIN(NetTraceDynamicNameFunc_, __LINE__), Stream, Collector, Verbosity)

#define UE_NET_TRACE_INTERNAL_NAMED_SCOPE(ScopeName, EventName, Stream, Collector, Verbosity) \
	auto UE_JOIN(NetTraceNameFunc_, __LINE__) = []() { static const UE::Net::FNetDebugName* Name = UE::Net::CreatePersistentNetDebugName(TEXT(#EventName)); return FNetTrace::TraceName(Name); }; \
	FNetTrace::TChooseTraceEventScope<Verbosity, decltype(Stream)>::Type ScopeName(UE_JOIN(NetTraceNameFunc_, __LINE__), Stream, Collector, Verbosity)
...
#define UE_NET_TRACE_INTERNAL_NAMED_DYNAMIC_NAME_SCOPE(ScopeName, EventName, Stream, Collector, Verbosity) \
	auto UE_JOIN(NetTraceDynamicNameFunc_, __LINE__) = [&]() { UE::Net::FNetDebugNameId NameId = FNetTrace::TraceName(EventName); return NameId; }; \
	FNetTrace::TChooseTraceEventScope<Verbosity, decltype(Stream)>::Type ScopeName(UE_JOIN(NetTraceDynamicNameFunc_, __LINE__), Stream, Collector, Verbosity)
...
#define UE_NET_TRACE_INTERNAL_OFFSET_SCOPE(Offset, Collector) FNetTraceOffsetScope UE_JOIN(NetTraceOffsetScope, __LINE__)(Offset, Collector);
...
#define UE_NET_TRACE_INTERNAL(Name, Collector, StartPos, EndPos, Verbosity) \
	do { if (FNetTrace::GetCollectorAtVerbosity(Collector, Verbosity)) { \
			static const UE::Net::FNetDebugName* UE_JOIN(NetTraceNameId_, __LINE__) = UE::Net::CreatePersistentNetDebugName(TEXT(#Name)); \
			FNetTrace::TracePacketContentEvent(*Collector, FNetTrace::TraceName(UE_JOIN(NetTraceNameId_, __LINE__)), StartPos, EndPos, Verbosity); \
	} } while (0)
...
#define UE_NET_TRACE_INTERNAL_PACKET_STATSCOUNTER(GameInstanceId, ConnectionId, Name, StatValue, Verbosity) \
	do { if (FNetTrace::GetNetTraceVerbosityEnabled(Verbosity)) { \
			static const UE::Net::FNetDebugName* UE_JOIN(NetTraceNameId_, __LINE__) = UE::Net::CreatePersistentNetDebugName(TEXT(#Name)); \
			FNetTrace::TracePacketStatsCounter(GameInstanceId, ConnectionId, FNetTrace::TraceName(UE_JOIN(NetTraceNameId_, __LINE__)), StatValue); \
	} } while (0)

#define UE_NET_TRACE_INTERNAL_FRAME_STATSCOUNTER(GameInstanceId, Name, StatValue, Verbosity) \
	do { if (FNetTrace::GetNetTraceVerbosityEnabled(Verbosity)) { \
			static const UE::Net::FNetDebugName* UE_JOIN(NetTraceNameId_, __LINE__) = UE::Net::CreatePersistentNetDebugName(TEXT(#Name)); \
			FNetTrace::TraceFrameStatsCounter(GameInstanceId, FNetTrace::TraceName(UE_JOIN(NetTraceNameId_, __LINE__)), StatValue); \
	} } while (0)
...
#define UE_NET_TRACE_INTERNAL_QUANTIZE_OBJECT_SCOPE(Handle, Timer) FNetTraceObjectQuantizeTimer UE_JOIN(__NetTraceQuantizeTimer, __LINE__)(Timer, (GNetTraceRuntimeVerbosity >= ENetTraceVerbosity::VeryVerbose && Handle.IsValid()) ? Handle.GetReplicationSystemId() : -1, Handle.GetId());
#define UE_NET_TRACE_INTERNAL_WRITE_OBJECT_SCOPE(Handle, Timer) FNetTraceObjectWriteTimer UE_JOIN(__NetTraceWriteTimer, __LINE__)(Timer, (GNetTraceRuntimeVerbosity >= ENetTraceVerbosity::VeryVerbose && Handle.IsValid()) ? Handle.GetReplicationSystemId() : -1, Handle.GetId());	
