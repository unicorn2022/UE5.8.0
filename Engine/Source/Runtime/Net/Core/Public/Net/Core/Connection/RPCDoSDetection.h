...
namespace UE::Net
{
	...
	extern NETCORE_API int32 GRPCDoSScopeDebugging;
}
#endif
...
public:
	 ...
	 */
	NETCORE_API bool HasHitQuota_Count(const FRPCDoSCounters(&PerPeriodHistory)[16], FRPCDoSCounters& InFrameCounter) const;
...
public:
	NETCORE_API virtual void ApplyImpliedValues() override;
...
public:
	 ...
	 * @param AnalyticsSender		Reference to the Engine implementation of RPC DoS Analytics sending
	 ...
	 */
	NETCORE_API void Init(FName NetDriverName, TSharedPtr<FNetAnalyticsAggregator>& AnalyticsAggregator, TSharedPtr<FRPCDoSAnalyticsSender> AnalyticsSender,
	 ...
	 */
	NETCORE_API void PreTickDispatch(double TimeSeconds);
	 ...
	 */
	ERPCNotifyResult NotifyReceivedRPC(UObject* Object, const UFunction* Function, FName FunctionName)
	{
	 ...
	 * @param FunctionName		The cached name of the function
	 */
	void LightweightReceivedRPC(const UFunction* Function, FName FunctionName)
	{
#if RPC_DOS_DEV_STATS
		CSV_SCOPED_TIMING_STAT_EXCLUSIVE(RPCDoS_Checks);
#endif
	 ...
	 * @param TimeSeconds	Cached/approximate timestamp, to save grabbing a new timestamp
	 */
	NETCORE_API void PostReceivedRPCPacket(double TimeSeconds);

	/**
	 * Called after the packet-loop TickDispatch
	 */
	NETCORE_API void PostTickDispatch();

	/**
	 * Triggered upon NetConnection Close (for committing analytics)
	 */
	NETCORE_API void NotifyClose();


	/**
	 * Tracks a blocked RPC for aggregated logging. Below the log threshold, logs individually.
	 * Above the threshold, accumulates counts for per-tick aggregated logging.
	 *
	 * @param RPCName	The name of the RPC that was blocked
	 */
	NETCORE_API void TrackBlockedRPC(FName RPCName);
...
private:
	 ...
	 */
	NETCORE_API void UpdateSeverity_Private(ERPCDoSSeverityUpdate Update, ERPCDoSEscalateReason Reason);
	 ...
	 * @return					Whether or not to block the RPC
	 */
	NETCORE_API ERPCNotifyResult CheckRPCTracking(const UFunction* Function, FName FunctionName);
	 ...
	 * @param TimeSeconds	Cached/approximate timestamp, to save grabbing a new timestamp
	 */
	NETCORE_API void EnableForcedRPCTracking(const UFunction* Function, FName FunctionName, double TimeSeconds);
	 ...
	 */
	FRPCTrackingInfo& FindOrAddRPCTracking(const UFunction* InFunc, bool& bOutNewTracking)
	{
...
private:
	...
	public:
		...
		inline ERPCNotifyResult CheckRPCTracking(FRPCDoSDetection& This, const UFunction* Function, FName FunctionName)
		{
		...
		inline FRPCTrackingInfo& FindOrAddRPCTracking(FRPCDoSDetection& This, const UFunction* InFunc, bool& bOutNewTracking)
		{
...
private:
	...
	TMap<const void*, TSharedPtr<FRPCTrackingInfo>> RPCTracking;
	...
	/**
	 * Flushes aggregated blocked RPC log data. Called once per tick from PostTickDispatch.
	 * If the per-tick log threshold was reached, prints one aggregated log line and resets counts (not map keys).
	 */
	void FlushBlockedRPCLogs();

	/**
	 * Fully resets blocked RPC tracking state, including wiping the map. Called on cooldown/de-escalation.
	 */
	void ResetBlockedRPCTracking();


	/** Accumulated blocked RPC counts, keyed by RPC name. Counts reset per-tick on flush; map wiped on de-escalation. */
	TMap<FName, int32> BlockedRPCCounts;

	/** Cached RPC name for fast-path blocked RPC tracking (avoids map lookup when the same RPC is blocked repeatedly) */
	FName CachedBlockedRPCName;

	/** Cached pointer to the count in BlockedRPCCounts for CachedBlockedRPCName */
	int32* CachedBlockedRPCCount									= nullptr;

	/** Number of individual blocked RPC log messages emitted this tick */
	int32 BlockedRPCLogCounter										= 0;
};
