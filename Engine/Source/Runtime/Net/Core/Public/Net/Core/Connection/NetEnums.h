...
/** Traits of a UNetConnection that are of importance to propagate to replication systems not depending on Engine. */
enum class ENetConnectionTraits : uint32
{
	/** No traits. Must be zero. */
	None = 0U,
	/** The connection is for replay recording */
	ReplayRecording = 1U,
	/** The connection is for replay playback */
	ReplayPlayback = 2U,
};
ENUM_CLASS_FLAGS(ENetConnectionTraits);

enum class EProxyType : uint8
{
	None = 0,
	/** Replicate objects from different game servers into the world. */
	Backend,
	/** Replicate objects to clients that were replicated into the world by another replication system. */
	Frontend,
};
