...
bool FNetworkPlatformFile::Initialize(IPlatformFile* Inner, const TCHAR* CmdLine)
{
	...
	{
		UE_LOGF(LogNetworkPlatformFile, Warning, "No COTF server connection.");
		return false;
	}
	if (!InitializeInternal(Inner, *Connection->GetHost()))
	{
		UE_LOGF(LogNetworkPlatformFile, Warning, "Failed to initialize %ls.", *Connection->GetHost());
		return false;
	}
...
bool FNetworkPlatformFile::InitializeInternal(IPlatformFile* Inner, const TCHAR* HostIP)
{
	...
	if (HostIP == NULL)
	{
		UE_LOGF(LogNetworkPlatformFile, Error, "No Host IP specified in the commandline.");
		bIsUsable = false;
		return false;
	}
...
bool FNetworkPlatformFile::SendPayloadAndReceiveResponse(TArray<uint8>& In, TArray<uint8>& Out)
{
	...
	if (!Response.IsOk())
	{
		UE_LOGF(LogCookOnTheFly, Warning, "Failed to send 'NetworkPlatformFile' request");
		return false;
	}
	...
	{
		UE_LOGF(LogCookOnTheFly, Warning, "Failed to parse 'CookOnTheFlyResponse' because the payload was too large");
		return false;
	}
...
void FNetworkPlatformFile::OnCookOnTheFlyMessage(const UE::Cook::FCookOnTheFlyMessage& Message)
{
	...
	if (Message.GetHeader().MessageType == ECookOnTheFlyMessage::NetworkPlatformFile)
	{
		...
		{
			UE_LOGF(LogCookOnTheFly, Warning, "Failed to parse 'CookOnTheFlyMessage' because the payload was too large");
			return;
		}
...
void FNetworkPlatformFile::InitializeAfterSetActive()
{
	double NetworkFileStartupTime = 0.0;
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogNetworkPlatformFile, Error, "Could not delete file sync test file %ls.", *TestSyncFile);
			}
			...
			{
				UE_LOGF(LogNetworkPlatformFile, Fatal, "Could not sync test file %ls.", *TestSyncFile);
			}
		}
	}
...
void FNetworkPlatformFile::ProcessServerCachedFilesResponse(FArrayReader& Response, const FPackageFileVersion& ServerPackageVersion, const int32 ServerPackageLicenseeVersion)
{
	...
	UE_LOGF(LogNetworkPlatformFile, Display, "Received '%d' cached file(s) from server", ServerCachedFiles.Num());
		...
		if (FileHandle.IsValid())
		{
			...
			if (FileHandle->Read((uint8*)&StoredPackageCookedVersion, sizeof(FPackageFileVersion)) == true)
			{
				if (FileHandle->Read((uint8*)&StoredPackageCookedLicenseeVersion, sizeof(int32)) == true)
				{
					...
					{
						UE_LOGF(LogNetworkPlatformFile, Display,
							"Engine version mismatch: Server %d.%d, Stored %d.%d\n",
	...
	{
		UE_LOGF(LogNetworkPlatformFile, Display, "Cooked version file missing: %ls\n", *CookedVersionFile);
	}
	...
	UE_LOGF(LogNetworkPlatformFile, Display, "Found '%d' locally cached file(s)", ClientLocalFiles.Num());
	...
	if (bDeleteAllFiles)
	{
		UE_LOGF(LogNetworkPlatformFile, Display, "Deleting all '%d' cached file(s) due to missing cooked version file...", ClientLocalFiles.Num());
		...
		{
			UE_LOGF(LogNetworkPlatformFile, Warning, "Failed to write cooked version file '%ls'", *CookedVersionFile);
		}
	}
	...
	for (const auto& ClientFileTimePair : ClientLocalFiles)
	{
		...
		if (bDeleteFile)
		{
			UE_LOGF(LogNetworkPlatformFile, Display, "Deleting cached file '%ls'", *Filename);
		}
		else
		{
			if (const FDateTime* ServerTime = ServerCachedFiles.Find(Filename))
			{
				...
				if (bDeleteFile)
				{
					UE_LOGF(LogNetworkPlatformFile, Display, "Deleting outdated cached file '%ls' with time difference '%.3f's", *Filename, TimeDiffInSeconds);
				}
				else
				{
					UE_LOGF(LogNetworkPlatformFile, Display, "Keeping cached file '%ls' with time difference '%.3f's", *Filename, TimeDiffInSeconds);
				}
			}
			else
			{
				UE_LOGF(LogNetworkPlatformFile, Display, "Deleting cached file '%ls' missing on server", *Filename);
				bDeleteFile = true;
			}
		}
	...
	UE_LOGF(LogNetworkPlatformFile, Display, "Deleted '%d' of '%d' cached file(s)", NumDeleted, ClientLocalFiles.Num());
}
...
//	FScopeLock ScopeLock(&SynchronizationObject);
	...
	//UE_LOGF(LogNetworkPlatformFile, Display, "Open local file %6.2fms", ThisTime);
	return Result;
}
...
void FNetworkPlatformFile::ProcessServerInitialResponse(FArrayReader& InResponse, FPackageFileVersion& OutServerPackageVersion, int32& OutServerPackageLicenseeVersion)
{
	...
	UE_LOGF(LogNetworkPlatformFile, Display, "    Server EngineDir      = %ls", *ServerEngineDir);
	UE_LOGF(LogNetworkPlatformFile, Display, "     Local EngineDir      = %ls", *FPaths::EngineDir());
	UE_LOGF(LogNetworkPlatformFile, Display, "    Server ProjectDir     = %ls", *ServerProjectDir);
	UE_LOGF(LogNetworkPlatformFile, Display, "     Local ProjectDir     = %ls", *FPaths::ProjectDir());
	UE_LOGF(LogNetworkPlatformFile, Display, "    Server EnginePlatformExtDir = %ls", *ServerEnginePlatformExtensionsDir);
	UE_LOGF(LogNetworkPlatformFile, Display, "     Local EnginePlatformExtDir = %ls", *UnsafeEnginePlatformExtensionDir());
	UE_LOGF(LogNetworkPlatformFile, Display, "    Server ProjectPlatformExtDir = %ls", *ServerProjectPlatformExtensionsDir);
	UE_LOGF(LogNetworkPlatformFile, Display, "     Local ProjectPlatformExtDir = %ls", *UnsafeProjectPlatformExtensionDir());
		...
		if (ServerTimeStamp != FDateTime::MinValue())  // if the file didn't actually exist on the server, don't create a zero byte file
		{
			if (!bCopiedExternally)
			{
				...
				InnerPlatformFile.CreateDirectoryTree(*FPaths::GetPath(Filename));
				{
					...
					if (!FileHandle)
					{
						UE_LOGF(LogNetworkPlatformFile, Fatal, "Could not open file for writing '%ls'.", *TempFilename);
					}
					...
					{
						UE_LOGF(LogNetworkPlatformFile, Fatal, "Could not write '%ls'.", *TempFilename);
					}
					...
					{
						UE_LOGF(LogNetworkPlatformFile, Fatal, "Did not write '%ls'.", *TempFilename);
					}
				}
			    ...
			    {
				    UE_LOGF(LogNetworkPlatformFile, Fatal, "Could Not Set Timestamp '%ls'  %ls < %ls.", *Filename, *CheckTime.ToString(), *ServerTimeStamp.ToString());
			    }
			}
	...
	public:
		...
		void DoWork()
		{
			...
			for (int32 Index = 0; Index < NumUnsolictedFiles; Index++)
			{
				...
				{
					UE_LOGF(LogNetworkPlatformFile, Fatal, "Receive failure!");
					return;
				}
...
void FNetworkPlatformFile::EnsureFileIsLocal(const FString& Filename)
{
	...
	UE_LOGF(LogNetworkPlatformFile, Verbose, "Searching for %ls locally ", *Filename);

	{
		...
		if (FinishedAsyncWriteUnsolicitedFiles)
		{
			if (bIncrementedPackageWaits == false && FinishedAsyncWriteUnsolicitedFiles->Get() == 0)
			{
				++UnsolicitedPackageWaits;
			}
	...
	//UE_LOGF(LogNetworkPlatformFile, Display, "Lock and wait for old async writes %6.2fms", ThisTime);
	...
	UE_LOGF(LogNetworkPlatformFile, Verbose, "Attempting to get %ls from server", *Filename);
	...
	if (InnerPlatformFile->FileExists(*Filename))
	{
		...
		UE_LOGF(LogNetworkPlatformFile, Verbose, "File %ls exists locally but wasn't in cache", *Filename);
		return;
	}
	...
	ThisTime = 1000.0f * float(FPlatformTime::Seconds() - StartTime);
	//UE_LOGF(LogNetworkPlatformFile, Display, "Check for local file %6.2fms - %ls", ThisTime, *Filename);
	...
	if (!bIsCookable && (ServerFiles.FindFile(Filename) == NULL))
	{
		...
		UE_LOGF(LogNetworkPlatformFile, Verbose, "Didn't find %ls in server files list", *Filename);
	...
	UE_LOGF(LogNetworkPlatformFile, Log, "Requesting file %ls", *Filename);
	...
	//UE_LOGF(LogNetworkPlatformFile, Display, "Send and receive %6.2fms", ThisTime);
	...
	if (ServerTimeStamp != FDateTime::MinValue())  // if the file didn't actually exist on the server, don't create a zero byte file
	{
		UE_LOGF(LogNetworkPlatformFile, Verbose, "Succeeded in getting %ls from server", *Filename);
	}
	else
	{
		UE_LOGF(LogNetworkPlatformFile, Verbose, "File not found %ls from server", *Filename);
	}
	...
	//UE_LOGF(LogNetworkPlatformFile, Display, "Write file to local %6.2fms", ThisTime);
}

static FString NetworkPlatformFileEndChop("/");
...
void FNetworkPlatformFile::PerformHeartbeat()
{
	...
	for (int32 FileIndex = 0; FileIndex < UpdatedFiles.Num(); FileIndex++)
	{
		...
		UE_LOGF(LogNetworkPlatformFile, Log, "Server updated file '%ls', deleting local copy %ls", *UpdatedFiles[FileIndex], *LocalFileName);
		...
		{
			UE_LOGF(LogNetworkPlatformFile, Log, "Unable to convert filename to package name %ls", *LocalFileName);
		}
...
void FNetworkPlatformFile::OnFileUpdated(const FString& LocalFileName)
{
	...
	{
		UE_LOGF(LogNetworkPlatformFile, Error, "Failed to delete %ls, someone is probably accessing without FNetworkPlatformFile, or we need better thread protection", *LocalFileName);
	}
