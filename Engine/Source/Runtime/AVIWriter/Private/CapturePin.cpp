...
TOptional<HRESULT> FCapturePin::ProcessFrames()
{
	...
	for (int32 FrameIndex = 0; FrameIndex < PendingFrames.Num(); ++FrameIndex)
	{
		...
		if (FAILED(hr))
		{
			UE_LOGF(LogMovieCapture, Warning, "Failed to get delivery buffer: %08x; Stopping.", hr);
			return S_OK;
		}
		...
		{
			pSample->Release();
			UE_LOGF(LogMovieCapture, Warning, "Failed to fill sample buffer: %08x; Stopping.", hr);
			return S_OK;
		}
		...
		if(hr != S_OK)
		{
			UE_LOGF(LogMovieCapture, Warning, "Deliver() returned %08x; Stopping.", hr);
			return S_OK;
		}
