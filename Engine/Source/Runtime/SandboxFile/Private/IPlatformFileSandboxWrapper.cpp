...
bool FSandboxPlatformFile::Initialize(IPlatformFile* Inner, const TCHAR* CmdLine)
{
	...
	if (CommandLineDirectory.IsEmpty() && bEntireEngineWillUseThisSandbox)
	{
		...
		UE_LOGF(LogInit, Display, "No sandbox specified, assuming %ls", *CommandLineDirectory);
	...
	if ((bSandboxEnabled == true) && (SandboxDirectory.Len() > 0))
	{
		if (InjectedTargetDirectory.Len())
		{
			...
			if (SandboxPath.StartsWith(InjectedSourceDirectory))
			{
				...
				UE_LOGF(LogInit, Verbose, "Injected %ls -->  %ls", *SandboxPath, *NewPath);
...
const FString& FSandboxPlatformFile::GetAbsoluteGameDirectory()
{
	if (AbsoluteGameDirectory.IsEmpty())
	{
		...
		UE_CLOGF(AbsoluteGameDirectory.IsEmpty(), SandboxFile, Fatal, "SandboxFileWrapper tried to access project path before it was set.");
	...
	virtual bool Visit(const TCHAR* FilenameOrDirectory, bool bIsDirectory) override
	{
		...
		if (SandboxFile.InjectedTargetDirectory.Len())
		{
			...
			if (NormalizedFilename == SandboxFile.InjectedSourceDirectoryParent)
			{
				...
				bKeepIterating = bKeepIterating && Visitor.CallShouldVisitAndVisit(*SandboxFile.InjectedSourceDirectory, true);
				if (!bKeepIterating)
				{
					return false;
				}
		...
		if (CanVisit)
		{
			bKeepIterating = bKeepIterating && Visitor.CallShouldVisitAndVisit(*LocalFilename, bIsDirectory);
			if (!bKeepIterating)
			{
				return false;
			}
...
bool FSandboxPlatformFile::IterateDirectory(const TCHAR* Directory, IPlatformFile::FDirectoryVisitor& Visitor)
{
	...
	if (SandboxVisitor.bKeepIterating && (SandboxDir != Directory))
	{
		Result = LowerLevel->IterateDirectory(Directory, SandboxVisitor) || Result;
	}
	return Result && SandboxVisitor.bKeepIterating;
}
...
bool FSandboxPlatformFile::IterateDirectoryRecursively(const TCHAR* Directory, IPlatformFile::FDirectoryVisitor& Visitor)
{
	...
	if (SandboxVisitor.bKeepIterating && (SandboxDir != Directory))
	{
		Result = LowerLevel->IterateDirectoryRecursively(Directory, SandboxVisitor) || Result;
	}
	return Result && SandboxVisitor.bKeepIterating;
}
	...
	virtual bool Visit(const TCHAR* FilenameOrDirectory, const FFileStatData& StatData) override
	{
		...
		if (SandboxFile.InjectedTargetDirectory.Len())
		{
			...
			if (NormalizedFilename == SandboxFile.InjectedSourceDirectoryParent)
			{
				...
				bKeepIterating = bKeepIterating && Visitor.CallShouldVisitAndVisit(*SandboxFile.InjectedSourceDirectory, InjectedStat);
				if (!bKeepIterating)
				{
					return false;
				}
		...
		if (CanVisit)
		{
			bKeepIterating = bKeepIterating && Visitor.CallShouldVisitAndVisit(*LocalFilename, StatData);
			if (!bKeepIterating)
			{
				return false;
			}
...
bool		FSandboxPlatformFile::IterateDirectoryStat(const TCHAR* Directory, IPlatformFile::FDirectoryStatVisitor& Visitor)
{
	...
	if (SandboxVisitor.bKeepIterating && (SandboxDir != Directory))
	{
		Result = LowerLevel->IterateDirectoryStat(Directory, SandboxVisitor) || Result;
	}
	return Result && SandboxVisitor.bKeepIterating;
}
...
bool		FSandboxPlatformFile::IterateDirectoryStatRecursively(const TCHAR* Directory, IPlatformFile::FDirectoryStatVisitor& Visitor)
{
	...
	if (SandboxVisitor.bKeepIterating && (SandboxDir != Directory))
	{
		Result = LowerLevel->IterateDirectoryStatRecursively(Directory, SandboxVisitor) || Result;
	}
	return Result && SandboxVisitor.bKeepIterating;
}
...
FOpenMappedResult FSandboxPlatformFile::OpenMappedEx2(const TCHAR* Filename, EOpenMappedFlags OpenOptions, int64 MaximumSize)
{
	FString UserFilename(*ConvertToSandboxPath(Filename));
	if (!OkForInnerAccess(Filename) || LowerLevel->FileExists(*UserFilename))
	{
		return LowerLevel->OpenMappedEx2(*UserFilename, OpenOptions, MaximumSize);
	}
