...
FVector2D UStaticMeshDescription::GetVertexInstanceUV(FVertexInstanceID VertexInstanceID, int32 UVIndex) const
{
	...
	{
		UE_LOGF(LogMeshDescription, Warning, "GetVertexInstanceUV: VertexInstanceID %d doesn't exist.", VertexInstanceID.GetValue());
		return FVector2D::ZeroVector;
	}

	if (!GetMeshDescription().VertexInstanceAttributes().HasAttribute(MeshAttribute::VertexInstance::TextureCoordinate))
	{
		UE_LOGF(LogMeshDescription, Warning, "GetVertexInstanceUV: VertexInstanceAttribute TextureCoordinate doesn't exist.");
		return FVector2D::ZeroVector;
	}
...
void UStaticMeshDescription::SetVertexInstanceUV(FVertexInstanceID VertexInstanceID, FVector2D UV, int32 UVIndex)
{
	if (!GetMeshDescription().IsVertexInstanceValid(VertexInstanceID))
	{
		UE_LOGF(LogMeshDescription, Warning, "SetVertexInstanceUV: VertexInstanceID %d doesn't exist.", VertexInstanceID.GetValue());
		return;
	}

	if (!GetMeshDescription().VertexInstanceAttributes().HasAttribute(MeshAttribute::VertexInstance::TextureCoordinate))
	{
		UE_LOGF(LogMeshDescription, Warning, "SetVertexInstanceUV: VertexInstanceAttribute TextureCoordinate doesn't exist.");
		return;
	}
...
void UStaticMeshDescription::SetPolygonGroupMaterialSlotName(FPolygonGroupID PolygonGroupID, const FName& SlotName)
{
	...
	{
		UE_LOGF(LogMeshDescription, Warning, "SetPolygonGroupMaterialSlotName: PolygonGroupID %d doesn't exist.", PolygonGroupID.GetValue());
		return;
	}

	if (!GetMeshDescription().PolygonGroupAttributes().HasAttribute(MeshAttribute::PolygonGroup::ImportedMaterialSlotName))
	{
		UE_LOGF(LogMeshDescription, Warning, "SetPolygonGroupMaterialSlotName: PolygonGroupAttribute ImportedMaterialSlotName doesn't exist.");
		return;
	}
