...
void FStaticMeshOperations::ComputeTriangleTangentsAndNormals(FMeshDescription& MeshDescription, float ComparisonThreshold, const TCHAR* DebugName)
{
	...
	ParallelFor( TEXT("ComputeTriangleTangentsAndNormals.PF"), BatchCount,1,
		[BatchSize, ComparisonThreshold, NumTriangles, &Attributes, DebugName](int32 BatchIndex)
		{
			...
			if (Attributes.GetVertexInstanceUVs().IsValid() && Attributes.GetVertexInstanceUVs().GetNumChannels() > 0)
			{
			...
			for (; StartIndex < EndIndex; ++StartIndex, TriIndex += 3)
			{
				...
				if (!TriangleVertexIDs.IsValidIndex(TriIndex + 2))
				{
					UE_LOGF(LogStaticMeshOperations, Warning, "Static Mesh %ls has invalid TriangleVertexIDs length! TriIndex %d. Skipping",
						DebugName ? DebugName : TEXT("UnknownMesh"), TriIndex + 2);
				...
				if (!VertexPositions.IsValidIndex(TriangleVertexIDs[TriIndex]) ||
					!VertexPositions.IsValidIndex(TriangleVertexIDs[TriIndex + 1]) ||
					!VertexPositions.IsValidIndex(TriangleVertexIDs[TriIndex + 2])
					)
				{
					UE_LOGF(LogStaticMeshOperations, Warning, "Static Mesh %ls has invalid vertex ids! StartIndex %d. VertId0=%d, VertId1=%d, VertId2=%d. Skipping",
						DebugName ? DebugName : TEXT("UnknownMesh"), StartIndex, TriangleVertexIDs[TriIndex].GetValue(), TriangleVertexIDs[TriIndex + 1].GetValue(), TriangleVertexIDs[TriIndex + 2].GetValue());
					TriangleNormals[StartIndex] = FVector3f(1, 0, 0);
					TriangleTangents[StartIndex] = FVector3f(0, 1, 0);
					TriangleBinormals[StartIndex] = FVector3f(0, 0, 1);
					continue;
				...
				{
					UE_CLOGF(DebugName != nullptr, LogStaticMeshOperations, Warning, "Static Mesh %ls has NaNs in it's vertex positions! Triangle index %d -- using identity for tangent basis.", DebugName, StartIndex);
					TriangleNormals[StartIndex] = FVector3f(1, 0, 0);
					TriangleTangents[StartIndex] = FVector3f(0, 1, 0);
					TriangleBinormals[StartIndex] = FVector3f(0, 0, 1);
					continue;
				...
				{
					if (!TriangleVertexInstanceIDs.IsValidIndex(TriIndex + 2))
					{
						UE_LOGF(LogStaticMeshOperations, Warning, "Static Mesh %ls has invalid TriangleVertexInstanceIDs length! TriIndex %d. Skipping",
							DebugName ? DebugName : TEXT("UnknownMesh"), TriIndex + 2);
						TriangleNormals[StartIndex] = FVector3f(1, 0, 0);
						TriangleTangents[StartIndex] = FVector3f(0, 1, 0);
						TriangleBinormals[StartIndex] = FVector3f(0, 0, 1);
					...
					if (!VertexUVs.IsValidIndex(TriangleVertexInstanceIDs[TriIndex]) ||
						!VertexUVs.IsValidIndex(TriangleVertexInstanceIDs[TriIndex + 1]) ||
						!VertexUVs.IsValidIndex(TriangleVertexInstanceIDs[TriIndex + 2])
						)
					{
						UE_LOGF(LogStaticMeshOperations, Warning, "Static Mesh %ls has invalid vertex instance ids! Triangle index %d. InstanceId0=%d, InstanceId1=%d, InstanceId2=%d. Skipping",
							DebugName ? DebugName : TEXT("UnknownMesh"), StartIndex, TriangleVertexInstanceIDs[TriIndex].GetValue(), TriangleVertexInstanceIDs[TriIndex + 1].GetValue(), TriangleVertexInstanceIDs[TriIndex + 2].GetValue());
						TriangleNormals[StartIndex] = FVector3f(1, 0, 0);
						TriangleTangents[StartIndex] = FVector3f(0, 1, 0);
						TriangleBinormals[StartIndex] = FVector3f(0, 0, 1);
						continue;
					...
					{
						UE_CLOGF(DebugName != nullptr, LogStaticMeshOperations, Warning, "Static Mesh %ls has NaNs in it's vertex uvs! Triangle index %d -- using identity for tangent basis.", DebugName, StartIndex);
						TriangleNormals[StartIndex] = FVector3f(1, 0, 0);
						TriangleTangents[StartIndex] = FVector3f(0, 1, 0);
						TriangleBinormals[StartIndex] = FVector3f(0, 0, 1);
						continue;
...
bool FStaticMeshOperations::AddUVChannel(FMeshDescription& MeshDescription)
{
	...
	{
		UE_LOGF(LogStaticMeshOperations, Error, "AddUVChannel: Cannot add UV channel. Maximum number of UV channels reached (%d).", MAX_MESH_TEXTURE_COORDS);
		return false;
	}
...
bool FStaticMeshOperations::InsertUVChannel(FMeshDescription& MeshDescription, int32 UVChannelIndex)
{
	...
	{
		UE_LOGF(LogStaticMeshOperations, Error, "InsertUVChannel: Cannot insert UV channel. Given UV channel index %d is out of bounds.", UVChannelIndex);
		return false;
	}

	if (VertexInstanceUVs.GetNumChannels() >= MAX_MESH_TEXTURE_COORDS)
	{
		UE_LOGF(LogStaticMeshOperations, Error, "InsertUVChannel: Cannot insert UV channel. Maximum number of UV channels reached (%d).", MAX_MESH_TEXTURE_COORDS);
		return false;
	}
...
bool FStaticMeshOperations::RemoveUVChannel(FMeshDescription& MeshDescription, int32 UVChannelIndex)
{
	...
	{
		UE_LOGF(LogStaticMeshOperations, Error, "RemoveUVChannel: Cannot remove UV channel. There must be at least one channel.");
		return false;
	}

	if (UVChannelIndex < 0 || UVChannelIndex >= VertexInstanceUVs.GetNumChannels())
	{
		UE_LOGF(LogStaticMeshOperations, Error, "RemoveUVChannel: Cannot remove UV channel. Given UV channel index %d is out of bounds.", UVChannelIndex);
		return false;
	}
	...
	, TOptional<const FString> DestinationPolygonGroupCountDifferFromSource_Msg)
{
	...
	if (DestinationMeshDescription.PolygonGroups().Num() != SourceMeshDescription.PolygonGroups().Num())
	{
		...
		{
			UE_LOGF(LogStaticMeshOperations, Warning, "%ls", *DestinationPolygonGroupCountDifferFromSource_Msg.GetValue());
		}
		return;
	}
	...
	{
		UE_LOGF(LogStaticMeshOperations, Warning, "%ls", *DestinationUnmatchMaterialName_Msg.GetValue());
	}
...
bool FStaticMeshOperations::ValidateAndFixData(FMeshDescription& MeshDescription, const FString& DebugName)
{
	...
	if (!DebugName.IsEmpty())
	{
		...
		{
			UE_LOGF(LogStaticMeshOperations, Display, "Mesh %ls has NaNs in it's vertex positions! Offending positions are set to zero.", *DebugName);
		}
		if (bHasInvalidTangentSpaces)
		{
			UE_LOGF(LogStaticMeshOperations, Display, "Mesh %ls has NaNs in it's vertex instance tangent space! Offending tangents are set to zero.", *DebugName);
		}
		if (bHasInvalidUVs)
		{
			UE_LOGF(LogStaticMeshOperations, Display, "Mesh %ls has NaNs in it's vertex instance uvs! Offending uvs are set to zero.", *DebugName);
		}
		if (bHasInvalidVertexColors)
		{
			UE_LOGF(LogStaticMeshOperations, Display, "Mesh %ls has NaNs in it's vertex instance colors! Offending colors are set to white.", *DebugName);
		}
	}
