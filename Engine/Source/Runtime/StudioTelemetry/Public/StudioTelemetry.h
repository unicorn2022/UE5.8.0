...
private:
	...
	struct FConfig
	{
		...
		bool bSendSessionContext = true; // Send the session context data with every event
	};
...
#define STUDIO_TELEMETRY_SESSION_SCOPE FStudioTelemetry::FSessionScope UE_JOIN(FSessionScope, __LINE__);
#define STUDIO_TELEMETRY_SPAN_SCOPE(Name) FStudioTelemetry::FSpanScope UE_JOIN(FSpanScope, __LINE__)(TEXT(#Name));
