...
{
	const FString TelemetryProviderSection("StudioTelemetry.Provider");
	...
	{
		UE_LOGF(LogAnalytics, Display, "Selected Telemetry Provider %ls", *ProviderName);
	}
	...
	if (GConfig->GetSectionNames(GEngineIni, SectionNames))
	{
		for (const FString& SectionName : SectionNames)
		{
			if (SectionName.Find(TelemetryProviderSection) != INDEX_NONE)
			{
				...
				// Is this Provider enabled? A given application may choose to disable it 
				bool Enabled = true;
				Enabled = GConfig->GetBoolOrDefault(*ProviderSection, TEXT("Enabled"), true, GEngineIni);

				if (Enabled == false)
				{
					UE_LOGF(LogAnalytics, Display, "Provider %ls is disabled for this application", *ProviderSection);
					continue;
				}
				...
				else
				{
					...
					UE_LOGF(LogAnalytics, Warning, "There must be a valid UsageType specified for analytics provider %ls", *ProviderSection);
					continue;
				}
				...
				if (GConfig->GetString(*ProviderSection, TEXT("ProviderModule"), ProviderModuleName, GEngineIni))
				{
					...
					if ( Name.IsEmpty() )
					{ 
						UE_LOGF(LogAnalytics, Error, "There must be a valid Name specified for analytics provider %ls.", *ProviderSection);
						continue;
					}
					else if (Providers.Find(Name) )
					{
						UE_LOGF(LogAnalytics, Warning, "An analytics provider with name %ls already exists.", *Name);
						continue;
					}
					...
					{
						UE_LOGF(LogAnalytics, Display, "Created an analytics provider %ls from module %ls configuration %ls [%ls]", *Name, *ProviderModuleName, *GEngineIni, *ProviderSection);
						Providers.Add(Name, AnalyticsProvider);
					}
					else
					{
						UE_LOGF(LogAnalytics, Warning, "Unable to create an analytics provider %ls from module %ls configuration %ls [%ls]", *Name, *ProviderModuleName, *GEngineIni, *ProviderSection);
					}
				}
				else
				{
					UE_LOGF(LogAnalytics, Warning, "There must be a valid ProviderModule specified for analytics provider %ls", *ProviderSection);
				}
			}
		}
	}
	...
	if (ProviderName.Len() && Providers.IsEmpty())
	{
		...
		UE_LOGF(LogAnalytics, Warning, "Unable to find a named analytics provider %ls", *ProviderName);
	}
}
...
{
	return DefaultEventAttributes.IsValidIndex(AttributeIndex)? DefaultEventAttributes[AttributeIndex] : FAnalyticsEventAttribute();
}
...
static void CheckForDuplicateAttributes(const FString& EventName, const TArray<FAnalyticsEventAttribute>& Attributes)
{
	...
	for (int32 indexA = 0; indexA < Attributes.Num(); ++indexA)
	{
		...
		for (int32 indexB = indexA + 1; indexB < Attributes.Num(); ++indexB)
		{
			...
			{	
				UE_LOGF(LogAnalytics, Warning, "Duplicate Attributes %ls Found For Event %ls", *AttributeName, *EventName);
			}
