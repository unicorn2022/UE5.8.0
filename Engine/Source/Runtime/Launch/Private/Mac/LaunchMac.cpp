...
- (void)applicationDidFinishLaunching:(NSNotification *)Notification
{
	...
	NSOperatingSystemVersion CurrentSystemVersion = FMacPlatformMiscEx::GetNSOperatingSystemVersion();
	...
	if (FMacPlatformMiscEx::VersionCompare(MinimumSystemVersion, MinSupportedMacOSVersion) < 0)
	{
		MinimumSystemVersion = MinSupportedMacOSVersion;
		MinimumSystemVersionString = MinSupportedMacOSVersionString;
	}

	if (FMacPlatformMiscEx::VersionCompare(CurrentSystemVersion, MinimumSystemVersion) < 0)
	{
	...
	if (FMacPlatformMiscEx::VersionCompare(CurrentSystemVersion, {13, 0, 0}) >= 0 && FMacPlatformMiscEx::VersionCompare(CurrentSystemVersion, {13, 3, 0}) < 0)
	{
		CFDictionaryRef SessionDictionary = CGSessionCopyCurrentDictionary();
		const bool bIsWindowServerAvailable = SessionDictionary != nullptr;
