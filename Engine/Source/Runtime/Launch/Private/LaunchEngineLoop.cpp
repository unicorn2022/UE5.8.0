...
#if WITH_APPLICATION_CORE && (PLATFORM_IOS || PLATFORM_TVOS)
	...
	FConsoleCommandWithArgsDelegate::CreateLambda( [](const TArray<FString>& Args) 
		{
			if ( Args.Num() == 0 )
			{
				UE_LOGF(LogInit, Error, "Need to specify a commandlet name to run!");
				return;
			}
			...
			if (!CommandletClass)
			{
				UE_LOGF(LogInit, Error, "Commandlet class '%ls' not found!", *CommandletName);
				return;
			}
...
struct FPakValidator
{
	...
	{
		Handle = FCoreDelegates::PakFileMountFailedDelegate.AddLambda([this](const FString& InError) { PakMountErrors.Add(InError); });
	}
	...
	bool HasPakMountFailed() const { return !PakMountErrors.IsEmpty(); }

	void HandlePakMountError()
	{
		UE_LOGF(LogInit, Warning, "Corrupt or missing files detected: %ls", *FString::Join(PakMountErrors, TEXT(", ")));
		const FText DialogTitle = LOCTEXT("Error.CorruptInstall.Title", "Corrupt/missing files detected");

		if (ILauncherCheckModule::Get().WasRanFromLauncher())
		{
			bool bRunVerifyInstallation = FMessageDialog::Open(
				EAppMsgCategory::Error,
				EAppMsgType::YesNo,
				EAppReturnType::No,
				LOCTEXT("Error.CorruptInstall.VerifyLauncher", "Corrupt or missing files detected. \n\nDo you want to verify your installation?"),
				DialogTitle
			) == EAppReturnType::Yes;
		...
		else
		{
			...
			FMessageDialog::Open(
				EAppMsgCategory::Error,
				EAppMsgType::Ok,
				LOCTEXT("Error.CorruptInstall", "Corrupt or missing files detected. Please verify your installation."),
				DialogTitle
			);
			RequestEngineExit(TEXT("Corrupt or missing files detected; exiting"));
#else
			constexpr int MAX_PAK_ERRORS { 10 };
			const FString TruncatedMessage = PakMountErrors.Num() > MAX_PAK_ERRORS ? FString::Format(TEXT("(showing first {0} errors)"), { MAX_PAK_ERRORS }) : TEXT("");
			const TConstArrayView<FString> PakMountErrorsSubset = MakeArrayView(PakMountErrors).Slice(0, FMath::Min(MAX_PAK_ERRORS, PakMountErrors.Num()));
			const FString Message = FString::Format(TEXT("Corrupt or missing files detected {0}:\n{1}\n\nWould you like to continue loading anyway?"),
				{ TruncatedMessage, FString::Join(PakMountErrorsSubset, TEXT("\n")) } );

			bool bRequestExit = FMessageDialog::Open(
				EAppMsgCategory::Error,
				EAppMsgType::YesNo,
				EAppReturnType::No,
...
bool ParseGameProjectFromCommandLine(const TCHAR* InCmdLine, FString& OutProjectFilePath, FString& OutGameName)
{
	...
	if ( ( FirstCommandLineToken.Len() && !FirstCommandLineToken.StartsWith(TEXT("-"))) || ParseProjectOptionResult)
	{
		...
		else if (FPaths::IsRelative(ProjectFilePath) && FPlatformProperties::IsMonolithicBuild() == false && FPlatformProperties::IsMergedModulesBuild() == false)
		{
...
bool LaunchSetGameName(const TCHAR *InCmdLine, FString& OutGameProjectFilePathUnnormalized)
{
	if (GIsGameAgnosticExe)
	{
		...
		if (!bPrinted)
		{
			...
			{
				UE_LOGF(LogInit, Display, "Running engine for game: %ls", FApp::GetProjectName());
			}
			else
			{
				if (FPlatformProperties::RequiresCookedData())
				{
					UE_LOGF(LogInit, Fatal, "Non-agnostic games on cooked platforms require a uproject file be specified.");
				}
				else
				{
					UE_LOGF(LogInit, Display, "Running engine without a game");
				}
			}
		}
	}
	else
	{
		...
		if (ParseGameProjectFromCommandLine(InCmdLine, ProjFilePath, LocalGameName) == true)
		{
			if (FPlatformProperties::RequiresCookedData())
			{
				...
				{
					UE_LOGF(LogInit, Fatal, "Non-agnostic games cannot load projects on cooked platforms - expected [%ls], found [%ls]", FApp::GetProjectName(), *LocalGameName);
				}
			}
			// Only set the game name if this is NOT a program...
			if (FPlatformProperties::IsProgram() == false)
		...
		if (!FApp::HasProjectName())
		{
			UE_LOGF(LogInit, Fatal,"Could not set game name!");
		}
	}
	...
	if (FPaths::IsProjectFilePathSet())
	{
		...
		if (!bGameNameMatchesProjectCaseSensitive && (FApp::IsProjectNameEmpty() || GIsGameAgnosticExe || (GameName.Len() > 0 && GIsGameAgnosticExe)))
		{
			...
			else
			{
				...
				if (!GIsBuildMachine)
				{
					UE_LOGF(LogInit, Warning, "%ls", *Message.ToString());
...
bool LaunchCheckForFileOverride(const TCHAR* CmdLine, bool& OutFileOverrideFound)
{
	...
	// Report if Zen streaming is in use now that configuration is complete.
	CSV_METADATA(TEXT("ZenStreaming"), FPlatformFileManager::Get().FindPlatformFile(TEXT("StorageServer")) == nullptr ? TEXT("0") : TEXT("1"));
...
void LaunchCheckForCommandLineAliases(const FConfigFile& Config, TArray<FString>& PrevExpansions, bool& bChanged)
{
	...
	if (const FConfigSection* Section = Config.FindSection(TEXT("CommandLineAliases")))
	{
		...
		{
			FString Key = FString("-") + ConfigIt.Key().ToString();
...
void LaunchCheckForCmdLineFile(TArray<FString>& PrevExpansions, bool& bChanged)
{
	...
	auto TryProcessFile = [&RemoveExpansion, &bChanged](const FString& InFilePath)
	{
		...
		{
			UE_LOGF(LogInit, Log, "Inserting commandline from file: %ls, %ls", *InFilePath, *FileCmds);
	...
	while (FParse::Value(FCommandLine::Get(), TEXT("-CmdLineFile="), CmdLineFile))
	{
		...
		{
			UE_LOGF(LogInit, Warning, "Can only load commandline files ending with .txt, can't load: %ls", *CmdLineFile);
			RemoveExpansion();
			continue;
		}
		...
		if (!bFoundFile)
		{
			UE_LOGF(LogInit, Warning, "Failed to load commandline file '%ls'.", *CmdLineFile);
			RemoveExpansion();
			continue;
		}
	...
	if ( FFileHelper::LoadFileToString(RecentProjectFileContents, *AutoLoadProjectFileName) )
	{
		if ( RecentProjectFileContents.Len() )
		{
			...
			if ( FPlatformFileManager::Get().GetPlatformFile().FileExists(*AutoLoadInProgressFilename) )
			{
				...
				UE_LOGF(LogInit, Display, "There was a problem auto-loading %ls. Auto-load will be disabled until the editor successfully starts up with a project.", *RecentProjectFileContents);
			}
			else if ( FPlatformFileManager::Get().GetPlatformFile().FileExists(*RecentProjectFileContents) )
			{
				...
				UE_LOGF(LogInit, Display, "Loading recent project file: %ls", *RecentProjectFileContents);
...
private:
	...
	{
		//UE_LOGF(LogInit, Warning, "OnFileOpenedForRead %u: %ls - %ls", FPlatformTLS::GetCurrentThreadId(), PakFileName, FileName);
...
public:
	...
	void DumpHistory()
	{
		...
		{
			UE_LOGF(LogInit, Display, "%ls", *Line);
			FTCHARToUTF8 UTF8String(*(MoveTemp(Line) + LINE_TERMINATOR));
			Writer->Serialize((UTF8CHAR*)UTF8String.Get(), UTF8String.Length());
		};

		UE_LOGF(LogInit, Display, "Dumping History of files read from Paks to %ls", *SavePath);
		UE_LOGF(LogInit, Display, "Begin History of files read from Paks");
		UE_LOGF(LogInit, Display, "------------------------------------------------------");
		...
		UE_LOGF(LogInit, Display, "------------------------------------------------------");
		UE_LOGF(LogInit, Display, "End History of files read from Paks");
...
bool IsServerDelegateForOSS(FName WorldContextHandle)
{
...
#if WITH_EDITOR
	if (GEngine && WorldContextHandle != NAME_None)
	{
	...
	if (!World)
	{
		...
		else
		{
			...
			UE_CLOGF(World == nullptr, LogInit, Error, "Failed to determine if OSS is server in PIE, OSS requests will fail");
#endif
		}
	}
...
namespace AppLifetimeEventCapture
{
	...
	{
		UE_LOGF( LogCore, Display, "AppLifetime: Application will deactivate" );
		CSV_EVENT_GLOBAL(TEXT("App_WillDeactivate"));
	}

	static void AppHasReactivated()
	{
		UE_LOGF( LogCore, Display, "AppLifetime: Application has reactivated" );
		CSV_EVENT_GLOBAL(TEXT("App_HasReactivated"));
	}

	static void AppWillEnterBackground()
	{
		UE_LOGF( LogCore, Display, "AppLifetime: Application will enter background" );
		CSV_EVENT_GLOBAL(TEXT("App_WillEnterBackground"));
	}

	static void AppHasEnteredForeground()
	{
		UE_LOGF( LogCore, Display, "AppLifetime: Application has entered foreground" );
...
int32 FEngineLoop::PreInitPreStartupScreen(const TCHAR* CmdLine)
{
...
#if WITH_ENGINE && CSV_PROFILER
	// PreInit the CSV profiler as early as possible in case we're boot profiling with -csvCaptureFrames
	FCsvProfiler::PreInit(CmdLine);
#endif
...
#if IS_PROGRAM
	// Non-program targets have the backlog enabled by default. Enable it as early as possible here for
	// cases where it is disabled by default. It will be disabled when module loading is complete.
	// Do not disable the backlog if the program enabled it explicitly before calling PreInit().
	const bool bDisableBacklog = !GLog->EnableBacklog(true);
	ON_SCOPE_EXIT
	{
		if (bDisableBacklog)
		{
	...
	if (Env.Len())
	{
		UE_LOGF(LogInit, Log, "Inserting commandline from UE-CmdLineArgs: %ls", *Env);
	...
	if (FParse::Param(CmdLine, TEXT("UTF8Output")))
	{
		...
		UE_LOGF(LogInit, Display, "Output is UTF-8 by default. Use of -UTF8Output on the command is deprecated and can be removed.");
	}
...
// Continue to protect shipping build from emitdrawevents (if needed in shipping, ALLOW_PROFILEGPU_IN_SHIPPING should be used instead to enable WITH_PROFILEGPU)
	...
	if (FPlatformProperties::IsProgram() == false)
	{
		...
		if (FPaths::IsProjectFilePathSet())
		{
			...
			if (FPaths::FileExists(ProjPath) == false)
			{
				...
				UE_LOGF(LogInit, Display, "Project file not found: %ls", *ProjPath);
				UE_LOGF(LogInit, Display, "\tAttempting to find via project info helper.");
				...
				{
					UE_LOGF(LogInit, Display, "\tFound project file %ls.", *GameProjectFile);
...
#if PLATFORM_MAC && WITH_APPLICATION_CORE
		if (!FMacPlatformApplicationMisc::IsMainThread())
		{
#endif
...
#if !UE_BUILD_SHIPPING
	{
		...
		// Refresh local CmdLine: the alias/file-expansion loop above and OnOutputDevicesInit
		// may have called FCommandLine::Set/Append internally.
		CmdLine = FCommandLine::Get();
	}
#endif
	...
	{
		UE_LOGF(LogInit, Error, "Failed to initialize I/O dispatcher: '%ls'", *Status.ToString());
		return 1;
	}
#endif
	...
	bool bErrorExitAfterSettingUpLogging = false;
	{
		SCOPED_BOOT_TIMING("LaunchCheckForFileOverride");
		if (LaunchCheckForFileOverride(CmdLine, bFileOverrideFound) == false)
		{
			// We will crash at some point. Try to reach log initialization to record the buffered logs and then exit.
			bErrorExitAfterSettingUpLogging = true;
		}
	}
		...
		(FStringView CommandletName)
	{
		...
		if (CommandletName == TEXTVIEW("cookcommandlet") || FParse::Param(FCommandLine::Get(), TEXT("RunAsCookCommandlet")))
		{
			...
			PRIVATE_GIsRunningDLCCookCommandlet = (FCString::Strifind(FCommandLine::Get(), TEXT("-dlcname=")) != nullptr);
		}
#endif
#if WITH_ENGINE
	...
	// Initialize random number generator.
	{
		...
		UE_LOGF(LogInit, Verbose, "RandInit(%d) SRandInit(%d).", Seed1, Seed2);
	}
		...
		)
	{
		...
		if (!IProjectManager::Get().LoadProjectFile(FPaths::GetProjectFilePath()))
		{
			...
			UE_LOGF(LogInit, Warning, "Could not find a valid project file, the engine will exit now.");
			// We will crash at some point. Try to reach log initialization to record the buffered logs and then exit.
			bErrorExitAfterSettingUpLogging = true;
		}
...
#if WITH_EDITOR
	{
		...
		{
			UE_LOGF(LogInit, Fatal, "Failed to create Intermediate directory '%ls'.", *ProjectIntermediateDir);
		}
		...
		{
			UE_LOGF(LogInit, Fatal, "Failed to create Intermediate/ShaderAutogen/ directory '%ls'. Make sure Intermediate exists.", *AutogenAbsolutePath);
		}
	...
	if (FPlatformProcess::SupportsMultithreading() && bCreateTaskGraphAndThreadPools)
	{
		...
		int32 StackSize = FPlatformProcess::GetDefaultPoolThreadStackSize();
	...
	// Load Core modules required for everything else to work (needs to be loaded before InitializeRenderingCVarsCaching)
	{
		...
		if (!LoadCoreModules())
		{
			UE_LOGF(LogInit, Error, "Failed to load Core modules.");
			// We will crash at some point. Try to reach log initialization to record the buffered logs and then exit.
			bErrorExitAfterSettingUpLogging = true;
		}
	}
	...
	{
		UE_LOGF(LogInit, Verbose, "Begin recording CVar changes for config patching.");
	...
	// init Oodle here ; note Decompressions are okay to run before this point
	FOodleDataCompression::StartupPreInit();
	
	// install named compression format modular features
	// must be done after FCommandLine and DLL dirs are set up
	FCompression::Init();
	...
	// Start the application
	{
		...
		if (bErrorExitAfterSettingUpLogging)
		{
			AppInitLoggingOnly();
			return 1;
		}
...
#if WITH_VERSE_VM
		AutoRTFM::InitializeForUE(Verse::RollbackAutoRTFMWrite);
#else
		AutoRTFM::InitializeForUE(nullptr);
#endif
	}
...
#if UE_CLIENT || UE_EDITOR
	FModuleManager::Get().LoadModule("BandwidthDebugDelegates");
	FModuleManager::Get().LoadModule("BandwidthMeasurementTool");
	FModuleManager::Get().LoadModule("DownlinkBandwidthManager");
#endif // UE_CLIENT || UE_EDITOR
	...
	SlowTask.EnterProgressFrame(5, LOCTEXT("InitializingRHI", "Initializing Render Hardware Interface..."));
	{
...
int32 FEngineLoop::PreInitPostStartupScreen(const TCHAR* CmdLine)
{
	...
	*/
...
#if WITH_ENGINE
	{
		...
		if (!IsRunningDedicatedServer() && !IsRunningCommandlet())
		{
			...
			if (IsMoviePlayerEnabled() && GetMoviePlayer()->HasEarlyStartupMovie())
			{
				...
				FString FileList = "All files accessed before init\n";
	...
	{
		// Flushing async loading before closing the DisregardForGC window to avoid having packages that start async loading before the window is closed
		// contain assets that are created after the window is closed.
		FlushAsyncLoading();
	...
	if (!bHasEditorToken && !IsRunningDedicatedServer())
	{
		...
		if (!bIsRegularClient)
		{
			...
			if (!CommandletClass)
			{
				...
				UE_LOGF(LogInit, Error, "%ls looked like a commandlet, but we could not find the class.", *Token);
			...
			UE_LOGF(LogInit, Log,  "Executing %ls", *CommandletClass->GetFullName() );
			...
			if (Default->IsEditor)
			{
				UE_LOGF(LogInit, Error, "Cannot run editor commandlet %ls with game executable.", *CommandletClass->GetFullName());
			...
			if ( GEngine == nullptr )
			{
				...
				if ( GIsEditor )
				{
					...
					{
						UE_LOGF(LogInit, Fatal, "Failed to load Editor Engine class '%ls'.", *EditorEngineClassName);
					}
					...
					UE_LOGF(LogInit, Log, "Initializing Editor Engine...");
					GEditor->InitEditor(this);
					UE_LOGF(LogInit, Log, "Initializing Editor Engine Completed");
				}
				else
#endif
				{
					...
					{
						UE_LOGF(LogInit, Fatal, "Failed to load Engine class '%ls'.", *GameEngineClassName);
					}
					...
					UE_LOGF(LogInit, Log, "Initializing Game Engine...");
					GEngine->Init(this);
					UE_LOGF(LogInit, Log, "Initializing Game Engine Completed");
				}
			}
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
				FCoreDelegates::GetOnPostEngineInit().Broadcast();
			}
			...
			if( Commandlet->ShowErrorCount )
			{
				...
				if (AllErrors.Num() || AllWarnings.Num())
				{
					...
					UE_LOGF(LogInit, Display, "");
					UE_LOGF(LogInit, Display, "Warning/Error Summary (Unique only)");
					UE_LOGF(LogInit, Display, "-----------------------------------");
					...
					for (const FString& ErrorMessage : AllErrors)
					{
						...
						if (!bAlreadyShown)
						{
							...
							UE_LOGF(LogInit, Display, "%ls", *ErrorMessage);
						}
					}
					...
					for (const FString& WarningMessage : AllWarnings)
					{
						...
						if (!bAlreadyShown)
						{
							...
							UE_LOGF(LogInit, Display, "%ls", *WarningMessage);
						}
					}
				}
				...
				{
					SET_WARN_COLOR(COLOR_RED);
					UE_LOGF(LogInit, Display, "Commandlet->Main return this error code: %d", ErrorLevel );
					UE_LOGF(LogInit, Display, "With %d error(s), %d warning(s)", AllErrors.Num(), AllWarnings.Num() );
				}
				else if( ( AllErrors.Num() == 0 ) )
				{
					SET_WARN_COLOR(AllWarnings.Num() ? COLOR_YELLOW : COLOR_GREEN);
					UE_LOGF(LogInit, Display, "Success - %d error(s), %d warning(s)", AllErrors.Num(), AllWarnings.Num() );
				}
				else
				{
					SET_WARN_COLOR(COLOR_RED);
					UE_LOGF(LogInit, Display, "Failure - %d error(s), %d warning(s)", AllErrors.Num(), AllWarnings.Num() );
				}
				CLEAR_WARN_COLOR();
			}
			else
			{
				UE_LOGF(LogInit, Display, "Finished.");
			}
			...
			uint64 PeakPhysMemoryMB = static_cast<uint64>(Stats.PeakUsedPhysical) / (1024 * 1024);
			uint64 PeakVirtMemoryMB = static_cast<uint64>(Stats.PeakUsedVirtual) / (1024 * 1024);
			if (CommandletExecutionTime <= 60)
			{
				UE_LOG(LogInit, Display, LINE_TERMINATOR TEXT("Execution of commandlet took:  %.2f seconds, PeakPhysMemoryMB=%llu MB, PeakVirtMemoryMB=%llu MB"), 
					CommandletExecutionTime, PeakPhysMemoryMB, PeakVirtMemoryMB);
			}
			else
			{
				...
				if (Hours)
				{
					UE_LOG(LogInit, Display, LINE_TERMINATOR TEXT("Execution of commandlet took:  %dh %dm %ds (%.2f seconds), PeakPhysMemoryMB=%llu MB, PeakVirtMemoryMB=%llu MB"), Hours, Minutes, Seconds, CommandletExecutionTime,
						PeakPhysMemoryMB, PeakVirtMemoryMB);
				}
				else
				{
					UE_LOG(LogInit, Display, LINE_TERMINATOR TEXT("Execution of commandlet took:  %dm %ds (%.2f seconds), PeakPhysMemoryMB=%llu MB, PeakVirtMemoryMB=%llu MB"), Minutes, Seconds, CommandletExecutionTime,
						PeakPhysMemoryMB, PeakVirtMemoryMB);
				}
			}
		...
		else
		{
			...
			if (bIsPossiblyUnrecognizedCommandlet)
			{
				...
				if (TempCommandletClass)
				{
					UE_LOGF(LogInit, Fatal, "You probably meant to call a commandlet. Please use the full name %ls.", *(Token+TEXT("Commandlet")));
				}
			}
		}
	}
...
int32 FEngineLoop::Init()
{
	...
	if( !GIsEditor )
	{
		...
		if (EngineClass == nullptr)
		{
			UE_LOGF(LogInit, Fatal, "Failed to load Game Engine class '%ls'.", *GameEngineClassName);
		}
		GEngine = NewObject<UEngine>(GetTransientPackage(), EngineClass);
	}
	else
	{
		...
		if (EngineClass == nullptr)
		{
			UE_LOGF(LogInit, Fatal, "Failed to load UnrealEd Engine class '%ls'.", *UnrealEdEngineClassName);
		}
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	UE_LOGF(LogInit, Display, "Engine is initialized. Leaving FEngineLoop::Init()");
	return 0;
}
...
void FEngineLoop::Exit()
{
	...
	// Reset all ticker elements. Registered lambda ticker functions can hold on to resources in their captures,
	// so we want to make sure those are released before shutting down.
...
void FEngineLoop::Tick()
{
	...
	{
		UE_LOGF(LogExit, Display, "**** TestExit: %ls ****", *GScopedTestExit->RequestExitPhrase());
...
#if WITH_ENGINE && CSV_PROFILER
			FCsvProfiler::Get()->UpdateFrameTime();
#endif
		}
...
#if WITH_RHI_BREADCRUMBS
			// Uses _NOMARKER to avoid intersecting "BeginFrame" scope with the "Frame" breadcrumb from BeginFrameRenderThread().
...

	{
		...
		if (bValidateReplicatedProperties)
		{
			...
			{
				UE_LOGF(LogInit, Log, "Replicated properties changed during Slate tick!");
			}
		}
		...
		// tick automation worker
		{
			...
			static const FName AutomationWorkerModuleName = "AutomationWorker";
	...
	FMacPlatformApplicationMisc::ProcessAsyncTasks();
#endif
}
...
bool DoesCurrentTargetRequireUpgrade(const FString& EditorTargetFileName, FString& InTargetName)
{
	...
	if (Receipt.BuildSettingsVersion.IsEmpty() || Receipt.TargetBuildEnvironment.IsEmpty())
	{
		UE_LOGF(
			LogInit,
			Log,
			"Unable to discern build settings version & build environment used for target - target may require upgrade. Target file: %ls",
	...
	const bool bIsUpToDate = (Receipt.BuildSettingsVersion == TEXT("V7"));
	...
	if (!bIsUpToDate && bIsShared)
	{
		UE_LOGF(
			LogInit,
			Warning,
			"Current target requires upgrade to V7, as it is not a unique build environment. File: %ls, BuildSettingsVersion: %ls, TargetBuildEnvironment: %ls",
			*EditorTargetFileName,
...
bool LaunchCorrectEditorExecutable(const FString& EditorTargetFileName)
{
	...
	UE_LOGF(LogInit, Display, "Running incorrect executable for target (%ls). Launching %ls instead...", *CurrentExecutableName, *LaunchExecutableName);
...
void FEngineLoop::AppInitUpToSetupOutputDevices()
{
	{
...
void FEngineLoop::AppInitLoggingOnly()
{
	AppInitUpToSetupOutputDevices();
}
...
bool FEngineLoop::AppInit()
{
	...
	if(!GIsBuildMachine && !FApp::IsUnattended() && FPaths::IsProjectFilePathSet() && !PluginManager.GetPreloadBinaries())
	{
		...
		if(!bNeedCompile)
		{
			...
			if (IncompatibleFiles.Num() > 0)
			{
				...
				{
					UE_LOGF(LogInit, Warning, "Incompatible or missing module: %ls", *IncompatibleFiles[Idx]);
				}

				// Build the error message for the dialog box
				FString ModulesList = "The following modules are missing or built with a different engine version:\n\n";
		...
		if(bNeedCompile)
		{
			...
			if(!bCompileResult || StillIncompatibleFiles.Num() > 0)
			{
				...
				{
					UE_LOGF(LogInit, Warning, "Still incompatible or missing module: %ls", *StillIncompatibleFiles[Idx]);
				}
				if (!FApp::IsUnattended())
				{
	...
	static_assert(sizeof(ANSICHAR) == 1);
...
void FEngineLoop::AppPreExit( )
{
	...
	UE_LOGF(LogExit, Log, "Preparing to exit." );
