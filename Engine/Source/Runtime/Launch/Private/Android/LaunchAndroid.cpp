...
int32 GAndroidTerminateOnExit = 0;
static FAutoConsoleVariableRef CVarTerminateOnExit(
	TEXT("Android.TerminateOnExit"),
	GAndroidTerminateOnExit,
	TEXT("Whether process is terminated on exit instead of normal shutdown on Android.\n")
	TEXT(" 0: disabled (default)\n")
	TEXT(" 1: enabled"),
	ECVF_Default);
...
extern bool GDebugConsoleOpen; // from AndroidPlatformMisc.cpp
#endif
...
static void SetSustainedPerformanceMode()
{
	...
	if (bSustainedPerformanceMode != bIncomingSustainedPerformanceMode)
	{
		...
		UE_LOGF(LogAndroid, Log, "Setting sustained performance mode: %d", (int32)bSustainedPerformanceMode);
...
void FPlatformMisc::UnlockAndroidWindow()
{
	...
	{
		UE_LOGF(LogAndroid, Log, "Unlocking android HW window during preinit. bAppIsActive_EventThread=%d", bAppIsActive_EventThread);
...
void EngineExit(void)
{
	...
	GEngineLoop.Exit();
	UE_LOGF(LogAndroid, Log, "Exiting is over");
	FPlatformMisc::RequestExit(1);
}

static void InitCommandLine()
{
	static constexpr int32 CMD_LINE_MAX = 16384;
...
#endif
	{
		...
		AAsset* asset = AAssetManager_open(AssetMgr, "UECommandLine.txt", AASSET_MODE_BUFFER);
		if (nullptr != asset)
		{
			const void* FileContents = AAsset_getBuffer(asset);
			int32 CommandLineLength = int32(FMath::Min(AAsset_getLength(asset), off_t(CMD_LINE_MAX - 1)));

			UTF8CHAR CommandLine[CMD_LINE_MAX];
			memcpy(CommandLine, FileContents, CommandLineLength);

			AAsset_close(asset);

			// chop off trailing spaces
			CommandLineLength = FUtf8StringView{CommandLine, CommandLineLength}.TrimEnd().Len();
			CommandLine[CommandLineLength] = '\0';

			FCommandLine::Append(StringCast<TCHAR>(CommandLine, CommandLineLength + 1).Get());
		...
		if (CommandLineFile)
		{
			...
			UTF8CHAR CommandLine[CMD_LINE_MAX];
			if (fgets((char*)CommandLine, CMD_LINE_MAX, CommandLineFile))
			{
				// chop off trailing spaces
				int32 CommandLineLength = FUtf8StringView{CommandLine}.TrimEnd().Len();
				CommandLine[CommandLineLength] = '\0';
			...
			if (Key.StartsWith(AndroidCompat))
			{
				...
				if (AllowedCompatCVars.Contains(CVarName))
				{
					...
					if (CVar)
					{
						...
						UE_LOGF(LogAndroid, Log, "Compat Setting %ls = %ls", *CVarName, *Value);
					}
				}
			}
		}
...
static bool ShouldUseLoggingThread()
{
	bool bAllowLoggingThread = false;
	GConfig->GetBool(FPlatformProperties::GetRuntimeSettingsClassName(), TEXT("bAllowLoggingThread"), bAllowLoggingThread, GEngineIni);

	const FString* ConfigRulesOverride = FPlatformMisc::GetConfigRulesVariable(TEXT("AllowLoggingThread"));
	if (ConfigRulesOverride)
	{
		bAllowLoggingThread = ConfigRulesOverride->Equals("true", ESearchCase::IgnoreCase);
	}

	return bAllowLoggingThread;
}
...
int32 AndroidMain(struct android_app* state)
{
	...
	if(!FAndroidMisc::UseNewWindowBehavior())
	{
		...
		UE_LOGF(LogAndroid, Log, "PreInit android HW window lock. bAppIsActive_EventThread=%d", bAppIsActive_EventThread);
		GAndroidWindowLock.Lock();
	}
	else
	{
		UE_LOGF(LogAndroid, Log, "PreInit android using new window behavior. bAppIsActive_EventThread=%d", bAppIsActive_EventThread);
	}
	...
	UE_LOGF(LogAndroid, Display, "Passed PreInit()");

	if (!ShouldUseLoggingThread())
	{
	...
	UE_LOGF(LogAndroid, Log, "Passed GEngineLoop.Init()");
	double EngineInitializationTime = FPlatformTime::Seconds() - GStartTime;
	UE_LOGF(LogLoad, Log, "(Engine Initialization) Total time: %.2f seconds", EngineInitializationTime);
...
// 			}
	...
	UE_LOGF(LogAndroid, Log, "Exiting");

	if (GAndroidTerminateOnExit && !FParse::Param(FCommandLine::Get(), TEXT("ProperShutdown")))
	{
		UE_LOGF(LogAndroid, Log, "Terminating process (skip shutdown)");
...
struct FChoreographer
{
	...
	void SetupChoreographer()
	{
		...
		if (!AChoreographer_getInstance_)
		{
			...
			{
				UE_LOGF(LogAndroid, Warning, "Failed to set up Choreographer");
			...
			{
				SetCallback(0);
				UE_LOGF(LogAndroid, Display, "Choreographer set up.");
			}
		}
	}
	...
	void DoCallback(long frameTimeNanos)
	{
		...
		//UE_LOGF(LogAndroid, Warning, "Choreographer %lld   delta %lld", frameTimeNanos, frameTimeNanos - LastFrameTimeNanos);
...
void FChoreographer::SetCallback(int64 Delay)
{
	...
	UE_CLOGF(!choreographer, LogAndroid, Fatal, "Choreographer was null (wrong thread?).");
...
static void* AndroidEventThreadWorker( void* param )
{
	...
	if(!FAndroidMisc::UseNewWindowBehavior())
	{
		...
		UE_LOGF(LogAndroid, Log, "AndroidEventThreadWorker, Initial HW window lock.");
		GAndroidWindowLock.Lock();
	}
	...
	UE_LOGF(LogAndroid, Log, "AndroidEventThreadWorker->Exiting");
	...
	if (EventType == AINPUT_EVENT_TYPE_MOTION)
	{
		...
		else
		{
			...
			case AMOTION_EVENT_ACTION_HOVER_EXIT:
				return 0;
			default:
				UE_LOGF(LogAndroid, Verbose, "Unknown AMOTION_EVENT %d ignored", actionType);
				return 0;
			}
			...
			else if ((AndroidThunkCpp_IsWebViewShown() || AndroidThunkCpp_HasExtraIgnoreView()) && (type == TouchBegan || type == TouchMoved || type == TouchEnded))
			{
				...
				{
					UE_LOGF(LogAndroid, Verbose, "Received touch event %d - Ignored", type);
					return 0;
				}
				UE_LOGF(LogAndroid, Verbose, "Received touch event %d", type);
			}
			if (isActionTargeted)
			{
				...
				UE_LOGF(LogAndroid, Verbose, "Received targeted motion event from pointer %zu (id %d) action %d: (%.2f, %.2f)", actionPointer, pointerId, action, x, y);
			...
			else
			{
				for (size_t i = 0; i < pointerCount; ++i)
				{
					...
					UE_LOGF(LogAndroid, Verbose, "Received motion event from index %zu (id %d) action %d: (%.2f, %.2f)", i, pointerId, action, x, y);
...
FAppEventData::FAppEventData(ANativeWindow* WindowIn)
{
	if (WindowIn == nullptr)
	{
		UE_LOGF(LogAndroid, Log, "FAppEventData was passed a NULL WindowsIn pointer.");
		return;
	}
	...
	if (WindowWidth <= 0)
	{
		UE_LOGF(LogAndroid, Log, "FAppEventData was passed a window with ZERO Height.");
		WindowWidth = 1;
	}
	if (WindowHeight <= 0)
	{
...
static void ActivateApp_EventThread()
{
	...
	if (!FAndroidMisc::UseNewWindowBehavior())
	{
		...
		UE_LOGF(LogAndroid, Log, "event thread, activate app, unlocking HW window");
		GAndroidWindowLock.Unlock();
	}
	...
	{
		UE_LOGF(LogAndroid, Log, "performing app foregrounding callback.");
...
{
	// Unlock window when we're ready.
	UE_LOGF(LogAndroid, Log, "event thread, new window");
...
static void SuspendApp_EventThread()
{
	...
	if (!bAppIsActive_EventThread)
	{
		UE_LOGF(LogAndroid, Warning, "SuspendApp_EventThread -> event thread, suspending app, bAppIsActive_EventThread is false so early return!");
	...
	if (!FAndroidMisc::UseNewWindowBehavior())
	{
		UE_LOGF(LogAndroid, Log, "SuspendApp_EventThread -> event thread, suspending app, acquiring HW window lock.");
		GAndroidWindowLock.Lock();
	}
	...
	if (bReadyToProcessEvents == false)
	{
		...
		UE_LOGF(LogAndroid, Warning, "SuspendApp_EventThread -> UNEXPECTED: event thread, app not yet ready.");
		return;
	}
	...
	{
		UE_LOGF(LogAndroid, Log, "SuspendApp_EventThread -> performing app backgrounding callback. %p", EMDoneTrigger.Get());
	...
	if (!FAndroidMisc::UseNewWindowBehavior())
	{
		...
		UE_LOGF(LogAndroid, Log, "SuspendApp_EventThread -> , waiting for event manager to process. tid: %d", FPlatformTLS::GetCurrentThreadId());
		...
		UE_CLOGF(!bSuccess, LogAndroid, Log, "SuspendApp_EventThread -> ERROR: backgrounding callback, not responded in timely manner. EMDoneTrigger->Wait, waited '%f' ms", (float)ElapsedTimeInMs_EMDoneTrigger_Wait);
		UE_LOGF(LogAndroid, Log, "SuspendApp_EventThread -> EMDoneTrigger->Wait, waited '%f' ms", (float)ElapsedTimeInMs_EMDoneTrigger_Wait);
	...
	UE_LOGF(LogAndroid, Log, "SuspendApp_EventThread(EOF)");
}
...
static void OnAppCommandCB(struct android_app* app, int32_t cmd)
{
	...
	case APP_CMD_SAVE_STATE:
		...
		*/
		...
		UE_LOGF(LogAndroid, Log, "Case APP_CMD_SAVE_STATE");
	...
	case APP_CMD_INIT_WINDOW:
		...
		UE_LOGF(LogAndroid, Log, "Case APP_CMD_INIT_WINDOW");
		...
		bHasWindow = true;
#endif
		UE_LOGF(LogAndroid, Log, "Case APP_CMD_INIT_WINDOW: bHasWindow=%d, bHasFocus=%d, bIsResumed=%d", bHasWindow, bHasFocus, bIsResumed);
		if (!FAndroidMisc::UseNewWindowBehavior())
		{
	...
	case APP_CMD_TERM_WINDOW:
		...
		UE_LOGF(LogAndroid, Log, "Case APP_CMD_TERM_WINDOW");
	...
	case APP_CMD_LOST_FOCUS:
		...
		UE_LOGF(LogAndroid, Log, "Case APP_CMD_LOST_FOCUS");
	...
	case APP_CMD_GAINED_FOCUS:
		...
		UE_LOGF(LogAndroid, Log, "Case APP_CMD_GAINED_FOCUS");
	...
	case APP_CMD_INPUT_CHANGED:
		UE_LOGF(LogAndroid, Log, "Case APP_CMD_INPUT_CHANGED");
		break;
	case APP_CMD_WINDOW_RESIZED:
		...
		UE_LOGF(LogAndroid, Log, "Case APP_CMD_WINDOW_RESIZED");
	...
	case APP_CMD_WINDOW_REDRAW_NEEDED:
		...
		UE_LOGF(LogAndroid, Log, "Case APP_CMD_WINDOW_REDRAW_NEEDED");
	...
	*/
	case APP_CMD_LOW_MEMORY:
		...
		UE_LOGF(LogAndroid, Log, "Case APP_CMD_LOW_MEMORY");
		break;
	case APP_CMD_START:
		/**
		...
		UE_LOGF(LogAndroid, Log, "Case APP_CMD_START");
	...
	case APP_CMD_RESUME:
		...
		UE_LOGF(LogAndroid, Log, "Case APP_CMD_RESUME");
	...
	case APP_CMD_PAUSE:
	{
		...
		UE_LOGF(LogAndroid, Log, "Case APP_CMD_PAUSE");
		...
		if (!bHasWindow && FAndroidMisc::GetDeviceMake() == FString("Oculus"))
		{
			...
			UE_LOGF(LogAndroid, Log, "Oculus: Ignoring APP_CMD_PAUSE command before APP_CMD_INIT_WINDOW");
			break;
		}
...
// 			}
		...
		*/
		...
		{
			UE_LOGF(LogAndroid, Log, "MoviePlayer force completion");
	...
	case APP_CMD_STOP:
		...
		UE_LOGF(LogAndroid, Log, "Case APP_CMD_STOP");
	...
	case APP_CMD_DESTROY:
		...
		*/
		...
		UE_LOGF(LogAndroid, Log, "Case APP_CMD_DESTROY");
...
{
#if !UE_BUILD_SHIPPING
	// TODO this is a very weak implementation, console popup can also be dismissed by pausing the app, improve it in the future
...
void JNICALL UE::Jni::FNativeCalls::HandleCustomTouchEvent(JNIEnv* jenv, jclass clazz, jint deviceId, jint pointerId, jint action, jint soucre, jfloat x, jfloat y)
{
	...
	UE_LOGF(LogAndroid, Verbose, "Handle custom touch event %d (%d) x=%f y=%f", TouchMessage.Type, action, x, y);
	FAndroidInputInterface::QueueTouchInput(TouchesArray);
#endif
}
...
void JNICALL UE::Jni::Asis::FAsisGameActivity::nativeMain(JNIEnv* jenv, jclass clazz, jstring projectModule)
{
	...
	FCoreDelegates::GetOnPostEngineInit().AddStatic(PostEngineInitEvent);
