...
// NOTE: We don't want WebRTC logic to always run. 
// Plugins that wish to use WebRTC should provide an implementation of this method.
extern "C" void InitWebRTCOnAndroid(JavaVM*) __attribute__((weak));
...
void FJavaWrapper::FindClassesAndMethods(JNIEnv* Env)
{
	...
	// this is optional - only inserted if OpenXR plugin enabled
	AndroidThunkJava_IsOpenXRImmersiveHMDApplication = FindMethod(Env, GameActivityClassID, "AndroidThunkJava_IsOpenXRImmersiveHMDApplication", "()Z", true);

	// this is optional - only inserted if bPackageForMetaQuest
	...
	// audio focus
	AndroidThunkJava_OnAudioFocusChangedListenersUpdated = FindMethod(Env, GameActivityClassID, "AndroidThunkJava_OnAudioFocusChangedListenersUpdated", "(Z)V", bIsOptional);
...
void FJavaWrapper::FindGooglePlayBillingMethods(JNIEnv* Env)
{
	...
	AndroidThunkJava_IapSetupService = FindMethod(Env, GoogleServicesClassID, "AndroidThunkJava_IapSetupService", "(Ljava/lang/String;Z)V", bIsStoreOptional);
	...
	AndroidThunkJava_IapGetBillingCountryCode = FindMethod(Env, GoogleServicesClassID, "AndroidThunkJava_IapGetBillingCountryCode", "()Ljava/lang/String;", bIsStoreOptional);
	AndroidThunkJava_IapGetIsAlternativeBillingOnlyAvailable = FindMethod(Env, GoogleServicesClassID, "AndroidThunkJava_IapGetIsAlternativeBillingOnlyAvailable", "()Z", bIsStoreOptional);
	AndroidThunkJava_IapShowAlternativeBillingOnlyInformationDialog = FindMethod(Env, GoogleServicesClassID, "AndroidThunkJava_IapShowAlternativeBillingOnlyInformationDialog", "()Z", bIsStoreOptional);
}
...
jmethodID FJavaWrapper::AndroidThunkJava_IsOpenXRImmersiveHMDApplication;
...
jmethodID FJavaWrapper::AndroidThunkJava_IapGetBillingCountryCode;
jmethodID FJavaWrapper::AndroidThunkJava_IapGetIsAlternativeBillingOnlyAvailable;
jmethodID FJavaWrapper::AndroidThunkJava_IapShowAlternativeBillingOnlyInformationDialog;
...
jmethodID FJavaWrapper::AndroidThunkJava_OnAudioFocusChangedListenersUpdated;
...
// call out to JNI to see if the application was packaged for OpenXR Immersive
bool AndroidThunkCpp_IsOpenXRImmersiveHMDApplication()
{
	static int32 IsOpenXRImmersiveHMDApplication = -1;

	if (IsOpenXRImmersiveHMDApplication == -1)
	{
		IsOpenXRImmersiveHMDApplication = 0;
		if (FJavaWrapper::AndroidThunkJava_IsOpenXRImmersiveHMDApplication)
		{
			if (JNIEnv* Env = FAndroidApplication::GetJavaEnv())
			{
				IsOpenXRImmersiveHMDApplication = FJavaWrapper::CallBooleanMethod(Env, FJavaWrapper::GameActivityThis, FJavaWrapper::AndroidThunkJava_IsOpenXRImmersiveHMDApplication) ? 1 : 0;
			}
		}
	}
	return IsOpenXRImmersiveHMDApplication == 1;
}

bool AndroidThunkCpp_IsStandaloneApplication()
{
	return USE_ANDROID_STANDALONE != 0;
}
...
bool AndroidThunkCpp_IsXRImmersiveHMDApplication()
{
	return AndroidThunkCpp_IsOpenXRImmersiveHMDApplication() || AndroidThunkCpp_IsOculusMobileApplication();
}
...
void AndroidThunkCpp_OnAudioFocusChangedListenersUpdated(bool bInHasListeners)
{
	if (JNIEnv* Env = FAndroidApplication::GetJavaEnv())
	{
		FJavaWrapper::CallVoidMethod(Env, FJavaWrapper::GameActivityThis, FJavaWrapper::AndroidThunkJava_OnAudioFocusChangedListenersUpdated, bInHasListeners);
	}
}
...
void AndroidThunkCpp_Iap_SetupIapService(const FString& InProductKey, bool InEnableAlternativeBillingOnly)
{
	if (JNIEnv* Env = FAndroidApplication::GetJavaEnv())
	{
		auto ProductKey = FJavaHelper::ToJavaString(Env, InProductKey);
		jboolean EnableAlternativeBillingOnly = InEnableAlternativeBillingOnly ? JNI_TRUE : JNI_FALSE;
		FJavaWrapper::CallVoidMethod(Env, FJavaWrapper::GoogleServicesThis, FJavaWrapper::AndroidThunkJava_IapSetupService, *ProductKey, EnableAlternativeBillingOnly);
	}
}
...
FString AndroidThunkJava_IapGetBillingCountryCode()
{
	FString Result = FString();
	if (JNIEnv* Env = FAndroidApplication::GetJavaEnv())
	{
		Result = FJavaHelper::FStringFromLocalRef(Env, (jstring)FJavaWrapper::CallObjectMethod(Env, FJavaWrapper::GameActivityThis, FJavaWrapper::AndroidThunkJava_IapGetBillingCountryCode));
	}

	return Result;
}

bool AndroidThunkJava_IapGetIsAlternativeBillingOnlyAvailable()
{
	bool Result = false;
	if (JNIEnv* Env = FAndroidApplication::GetJavaEnv())
	{
		CHECK_JNI_METHOD(FJavaWrapper::AndroidThunkJava_IapGetIsAlternativeBillingOnlyAvailable)

		Result = FJavaWrapper::CallBooleanMethod(Env, FJavaWrapper::GameActivityThis, FJavaWrapper::AndroidThunkJava_IapGetIsAlternativeBillingOnlyAvailable);
	}

	return Result;
}

bool AndroidThunkJava_IapShowAlternativeBillingOnlyInformationDialog()
{
	bool Result = false;
	if (JNIEnv* Env = FAndroidApplication::GetJavaEnv())
	{
		CHECK_JNI_METHOD(FJavaWrapper::AndroidThunkJava_IapShowAlternativeBillingOnlyInformationDialog)

		Result = FJavaWrapper::CallBooleanMethod(Env, FJavaWrapper::GameActivityThis, FJavaWrapper::AndroidThunkJava_IapShowAlternativeBillingOnlyInformationDialog);
	}
...
JNIEXPORT jint JNI_OnLoad(JavaVM* InJavaVM, void* InReserved)
{
	...
	if (InitWebRTCOnAndroid)
	{
		InitWebRTCOnAndroid(GJavaVM);
	}
...
JNI_METHOD void Java_com_epicgames_unreal_GameActivity_nativeOnSafetyNetAttestationFailed(JNIEnv* jenv, jobject thiz, jint jwsValue)
{
	...
	if (FTaskGraphInterface::IsRunning())
	{
		...
		{
			FJavaWrapper::OnSafetyNetAttestationResultDelegate.Broadcast(false, FString(), jwsValue);
		}, TStatId(), NULL, ENamedThreads::GameThread);
...
void AndroidThunkCpp_OnNativeToEmbeddedReply(FString ID, const FEmbeddedCommunicationMap& InReturnValues, FString InError, FString RoutingFunction)
{
	if (JNIEnv* Env = FAndroidApplication::GetJavaEnv())
	{
		// marshall some data
#if !UE_BUILD_SHIPPING
		UE_LOGF(LogInit, Display, "Java call Id: %ls, Routing Function: %ls, Error %ls, Params:", *ID, *RoutingFunction, *InError);
#endif
...
#if !UE_BUILD_SHIPPING
			UE_LOGF(LogInit, Display, "  %ls : %ls", *It.Key, *It.Value);
#endif
		}
...
void FJavaWrapper::SetupEmbeddedCommunication(JNIEnv* Env)
{
	...
	{
		UE_LOGF(LogAndroid, Display, "Calling out from embedded to native for 'webview' subsystem:");
	...
	{
		UE_LOGF(LogAndroid, Display, "Calling out from embedded to native for 'native' subsystem:");
