...
int64 UTextureImportSettings::GetAutoLimitPixelCount() const
{
	...
	if ( IsImportAutoVTEnabled() )
	{
		if ( AutoVTSize != AutoLimitDimension )
		{
			...
			UE_CALL_ONCE( [&](){	
				UE_LOGF(LogCore, Warning, "VT is enabled with AutoVTSize (%d) not equal AutoLimitDimension (%d); they must be equal or zero, fix config!  Ignoring AutoLimitDimension and using AutoVTSize.", 
	...
	TEXTUREUTILITIESCOMMON_API void ApplyDefaultsForNewlyImportedTextures(UTexture * Texture, bool bIsReimport)
	{
		...
		if ( bIsReimport )
		{
				...
				Texture->LODGroup != TEXTUREGROUP_UI )
			{
				...
				if ( bIsPowerOfTwo )
				{
					UE_LOGF(LogCore, Display, "Texture [%ls] settings changed by reimport; was NoMips UI, is now pow2 Default", 
						*Texture->GetName() );	
