...
namespace UE
{
	namespace TextureUtilitiesCommon
	{
		...
		{
			FString PreUDIMNameToMatch;
			FString PostUDIMName;
			TMap<int32, FString> UDIMsAndSourcesFile;

			if (ParseUDIMName(FPaths::GetBaseFilename(File), UdimRegexPattern, PreUDIMNameToMatch, PostUDIMName) != INDEX_NONE)
			{ 
				const FString Path = FPaths::GetPath(File);
				const FString UDIMFilter = (Path / PreUDIMNameToMatch) + TEXT("*") + PostUDIMName + FPaths::GetExtension(File, true);

				TArray<FString> UDIMFiles;
				IFileManager::Get().FindFiles(UDIMFiles, *UDIMFilter, true, false);
				
				FString PreUDIMNameFound;
				for (FString& UDIMFile : UDIMFiles)
				{
					const int32 UDIMIndex = ParseUDIMName(FPaths::GetBaseFilename(UDIMFile), UdimRegexPattern, PreUDIMNameFound, PostUDIMName);
					
					if (UDIMIndex == INDEX_NONE || !PreUDIMNameFound.Equals(PreUDIMNameToMatch))
					{
						continue;
					}

					const FString UDIMName = PreUDIMNameFound + PostUDIMName;
				...
				{
					*OutFilenameWithoutUdimPatternAndExtension = PreUDIMNameToMatch + PostUDIMName;
				}
			}
