	...
	namespace TextureUtilitiesCommon
	{
		...
		public:
			...
			void ProcessData(bool bDoOnComplexAlphaNotJustBinaryTransparency)
			{
				...
				for (int64 Y = 0; Y < TextureHeight; ++Y)
				{
					...
					for(int64 X = 0; X < TextureWidth; ++X)
					{
						...
						else
						{
							...
							{
								UE_LOGF(LogCore, Log,  "PNG has complex alpha channel, will not fill RGB in transparent background, due to setting PNGInfill == OnlyOnBinaryTransparency");
				...
				if ( HasComplexAlpha )
				{
					UE_LOGF(LogCore, Log,  "PNG has alpha channel, doing fill of RGB in transparent background, due to setting PNGInfill == Always");
				}
				else
				{
					UE_LOGF(LogCore, Log,  "PNG has binary transparency, doing fill of RGB in transparent background, due to setting PNGInfill != Never");
				}
