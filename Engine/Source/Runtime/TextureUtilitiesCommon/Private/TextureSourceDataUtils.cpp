...
namespace Private
{
	...
	static bool ResizeTexture2D(UTexture* Texture, int32 MaxSize, const ITargetPlatform* TargetPlatform)
	{
		...
		{
			UE_LOGF(LogTexture,Error,"ResizeTexture2D: Texture GetMipImage failed [%ls]",
				*Texture->GetFullName());
			return false;
		}
		...
		else
		{
			...
			{
				UE_LOGF(LogTexture,Error,"ResizeTexture2D: Texture DownsizeImageUsingTextureSettings failed [%ls]",
					*Texture->GetFullName());
				return false;
			}
			if ( ! MadeChanges )
	...
	static bool ResizeTexture2DBlocked(UTexture* Texture, int32 TotalMaxSize, const ITargetPlatform* TargetPlatform)
	{
		...
		for (int32 BlockIndex = 0; BlockIndex < Texture->Source.GetNumBlocks(); ++BlockIndex)
		{
			...
			{
				UE_LOGF(LogTexture,Error,"ResizeTexture2DBlocked: Texture GetMipImage failed [%ls]",
					*Texture->GetFullName());
				return false;
			}
			...
			{
				// critical error
				UE_LOGF(LogTexture,Error,"ResizeTexture2DBlocked: Texture DownsizeImageUsingTextureSettings failed [%ls]",
					*Texture->GetFullName());
				return false;
			}
...
static bool IsTextureResizeSupported(UTexture * Texture)
{
	...
	if ( Texture->Source.GetNumMips() > 1 && Texture->MipGenSettings == TMGS_LeaveExistingMips )
	{
		...
		UE_LOGF(LogTexture,Warning,"DownsizeTextureSourceData: Texture has LeaveExistingMips ; they will be discarded! [%ls]",
			*Texture->GetFullName());
	}
...
TEXTUREUTILITIESCOMMON_API bool DownsizeTextureSourceDataNearRenderingSize(UTexture* Texture, const ITargetPlatform* TargetPlatform, int32 AdditionalSourceSizeLimit)
{
	...
	if (DownsizeTextureSourceData(Texture, TargetSourceSize, TargetPlatform))
	{
		...
		{
			UE_LOGF(LogTexture,Warning,"DownsizeTextureSourceDataNearRenderingSize failed to preserve built size; was: %dx%d now: %dx%d on [%ls]",
...
TEXTUREUTILITIESCOMMON_API bool ChangeTextureSourceFormat(UTexture* Texture, ETextureSourceFormat NewFormat)
{
	...
	if ( Texture->Source.GetNumBlocks() == 1 && Texture->Source.GetNumMips() == 1 )
	{
		...
		{
			UE_LOGF(LogTexture,Error,"ChangeTextureSourceFormat: Texture GetMipImage failed [%ls]",
				*Texture->GetFullName());
			return false;
		}
...
TEXTUREUTILITIESCOMMON_API bool CompressTextureSourceWithJPEG(UTexture* Texture,int32 Quality)
{
	...
	{
		UE_LOGF(LogTexture,Error,"CompressTextureSourceWithJPEG: Texture GetMipImage failed [%ls]",
			*Texture->GetFullName());
		return false;
	}
	...
	{
		UE_LOGF(LogTexture,Error,"CompressTextureSourceWithJPEG: Texture CompressImage failed [%ls]",
			*Texture->GetFullName());
		return false;
	}
