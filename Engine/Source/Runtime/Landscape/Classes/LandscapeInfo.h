...
public:
	...
	FTargetLayerBlendGroup() = default;
	FTargetLayerBlendGroup(const FName& InName, ELandscapeTargetLayerBlendMethod InBlendMethod, const TSet<ULandscapeLayerInfoObject*>& InTargetLayers)
		: Name(InName)
		, BlendMethod(InBlendMethod)
		, TargetLayers(ObjectPtrWrap(InTargetLayers))
	{
	}
	...
	const TSet<TObjectPtr<ULandscapeLayerInfoObject>>& GetTargetLayers() const
	{
		return TargetLayers;
	}
...
private:
	...
	UPROPERTY()
	ELandscapeTargetLayerBlendMethod BlendMethod = ELandscapeTargetLayerBlendMethod::Count;

	UPROPERTY()
	TSet<TObjectPtr<ULandscapeLayerInfoObject>> TargetLayers;
};
...
#if WITH_EDITOR
	LANDSCAPE_API TArray<FTargetLayerBlendGroup> GetTargetLayerBlendGroups() const;
	LANDSCAPE_API TOptional<FTargetLayerBlendGroup> GetBlendGroupForTargetLayer(ULandscapeLayerInfoObject* InLayerInfo) const;
