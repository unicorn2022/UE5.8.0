...
struct FLandscapeComponentGrassData
{
	...
	TMap<FName, int32> WeightOffsets;
	...
	void FixupFallbackGrassNames(const TMap<FName, TObjectPtr<ULandscapeGrassType>>& InNamedGrassTypes);
#endif // WITH_EDITOR
	void InitializeFrom(IBuffer2DView<uint16>* HeightData, TMap<FName, IBuffer2DView<uint8>*>& WeightData, bool bStripEmptyWeights);

	bool HasWeightData() const;
	TArrayView<uint8> GetWeightData(const FName& GrassName);
	bool Contains(const FName& GrassName) const;
	TArrayView<uint16> GetHeightData();

	SIZE_T GetAllocatedSize() const;

	// Check whether we can discard any data not needed with current scalability settings
	void ConditionalDiscardDataOnLoad(const TMap<FName, TObjectPtr<ULandscapeGrassType>>& InNamedGrassTypes);
...
/**
	ELandscapeLayerUpdateMode flags relate to types of updates to landscape layer data.  Stored on the ALandscape and on
	ULandscapeComponent to indicate pending changes.  Stored on FLandscapeEditLayerComponentReadbackResult to indicate
	what type of change triggered the creation of the new data.
 
	Update_All_Components on ALandscape or passed to RequestLayersContentUpdate will cause all components to be enqueued
	for layer merging, rather than only those components carrying relevant flags of their own.

 	See IsUpdateFlagEnabledForModes, where these flags are translated to ELandscapeComponentUpdateFlag, that indicate
	specific tasks.
**/
...
private:
...
#if WITH_EDITORONLY_DATA
	/** A CRC of everything that can lead NamedGrassTypes to be changed (e.g. the material changing, or grass type overrides changing) */
	uint32 GrassTypesCRC = 0;

	UPROPERTY()
	TArray<TObjectPtr<ULandscapeGrassType>> GrassTypes_DEPRECATED;
#endif // WITH_EDITORONLY_DATA
	...
	* Call UpdateGrassTypes() to ensure this array is up to date */
	UPROPERTY()
	TMap<FName, TObjectPtr<ULandscapeGrassType>> NamedGrassTypes;
...
public:
	...
	LANDSCAPE_API const TArray<UTexture2D*>& GetCurrentRuntimeWeightmapTextures() const;
	...
	/** Collect all the PSO precache data used by the landscape component */
	LANDSCAPE_API virtual void CollectPSOPrecacheData(const FPSOPrecacheParams& BasePrecachePSOParams, FMaterialInterfacePSOPrecacheParamsList& OutParams) override;
	...
	UE_DEPRECATED(5.8, "Use GetNamedGrassTypes")
	TArray<TObjectPtr<ULandscapeGrassType>> GetGrassTypes() const;

	const TMap<FName, TObjectPtr<ULandscapeGrassType>>& GetNamedGrassTypes() const { return NamedGrassTypes; }
	...
	void SetNamedGrassTypes(const TMap<FName, TObjectPtr<ULandscapeGrassType>>& InNamedGrassTypes)
	{
		NamedGrassTypes = InNamedGrassTypes;
		InvalidateGrassTypeSummary();
	}
	
	bool MaterialHasGrass() const { return !GetNamedGrassTypes().IsEmpty(); }
	...
	/** @return a user-friendly name to easily identify this component when dumping debug info, for example */
	LANDSCAPE_API FString GetDebugName() const;
	...
	*/
	...
	/**
	 * Retrieve the list of all materials that might be used by this landscape component (LandscapeMaterial, LandscapeHoleMaterial, etc.)
	...
	* @param TextureDataInfo - FLandscapeTextureDataInfo pointer, to notify of the mip data region updated.
	 ...
	 */
	LANDSCAPE_API TArray<UTexture*> ReallocateWeightmaps(FLandscapeEditDataInterface* DataInterface, const FGuid& InEditLayerGuid, bool bInSaveToTransactionBuffer, bool bInForceReallocate, ALandscapeProxy* InTargetProxy, const TSet<ULandscapeComponent*>* InRestrictSharingToComponents);
