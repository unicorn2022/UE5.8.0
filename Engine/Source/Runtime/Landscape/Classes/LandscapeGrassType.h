...
class ULandscapeGrassType : public UObject
{
	...
	*/
	...
	/** 
	 * Return a FName that corresponds to this grass type. It used to be that grass types were unique for spawning grass on a given component. Nowadays, 
	 *  we use named "slots" for each grass type (the GrassName being the slot's name in the landscape material). This function here returns a fallback
	 *  name for when serializing old data that was grass type-based only. 
	*/
	inline FName GetGrassNameFallback() const
	{
		// Simply use the UObject's name as the fallback since it's unique on a given landscape component (because grass types were unique on a given component already)
		return GetFName();
	}
};
