...
private:
	...
	TArray<FBox> DeferredSnapFoliageInstanceBoxes;
...
public:
	...
	struct FHeightfieldGeometryRef : public UE::Private::FQueryableRefCountedObject
	{
		FGuid Guid;
	...
	mutable FRWLock CachedHeightFieldSamplesLock;
...
public:
	...
	LANDSCAPE_API void MarkAreaForSnapFoliageInstances(const FBox& InBox);
#endif
...
#if WITH_EDITORONLY_DATA
	// Foliage snapping functions : called from editor code to manage foliage instances on landscape.
	UE_DEPRECATED(5.8, "Use the method taking a ESnapFoliageInstancesFlags parameter")
	LANDSCAPE_API void SnapFoliageInstances();

	enum class ESnapFoliageInstancesFlags
	{
		ForceAll = 1 << 0, // Force-snap foliage instances across the entire component
		MarkedAreasOnly = 1 << 1, // Only update the areas that have already been marked for snapping via MarkAreaForSnapFoliageInstances
	};
	FRIEND_ENUM_CLASS_FLAGS(ESnapFoliageInstancesFlags);
	/** Snap foliage instances overlapping with the component */
	LANDSCAPE_API void SnapFoliageInstances(ESnapFoliageInstancesFlags InFlags);
	/** Snap foliage instances overlapping with the specified area only */
