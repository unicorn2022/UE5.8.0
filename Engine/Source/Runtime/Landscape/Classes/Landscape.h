...
public:
	...
	bool GetGrassEnabled() const { return bGrassEnabled; }

	UFUNCTION(BlueprintCallable, Category = "Landscape")
	LANDSCAPE_API void SetGrassEnabled(bool bEnabled, bool bFlushGrassMaps = true);
	...
	*/
	...
	/**
	* @return the list of edit layers on this landscape (editor-only : returns nothing at runtime)
	*/
	UFUNCTION(BlueprintCallable, DisplayName = GetEditLayers, Category = "Landscape|Editor", meta = (DevelopmentOnly))
	const TArray<ULandscapeEditLayerBase*> GetEditLayersBP() const;

	/**
	* @return the edit layer whose name matches the parameter on this landscape (editor-only : returns nothing at runtime)
	*/
	UFUNCTION(BlueprintCallable, DisplayName = GetEditLayer, Category = "Landscape|Editor", meta = (DevelopmentOnly))
	const ULandscapeEditLayerBase* GetEditLayerByNameBP(const FName& InLayerName) const;
...
private:
	...
	void ReallocateLayersWeightmaps(FUpdateLayersContentContext& InUpdateLayersContentContext, 
...
private:
	...
	UE_DEPRECATED(5.8, "WasCompilingShaders property is no longer used.")
	UPROPERTY(Transient)
	bool WasCompilingShaders_DEPRECATED = false;
	...
	bool bWarnedNumComponentsPerBatchExceeded = false;
