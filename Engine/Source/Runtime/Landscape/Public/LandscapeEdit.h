...
struct FLandscapeTextureDataInfo
{
	...
	void AddMipUpdateRegion(int32 InMipIndex, int32 InX1, int32 InY1, int32 InX2, int32 InY2);
	
	void* GetMipData(int32 InMipIndex);

	int32 GetMipSizeX(int32 InMipIndex) const
	{
		return FMath::Max(Texture->Source.GetSizeX() >> InMipIndex, 1);
	}

	int32 GetMipSizeY(int32 InMipIndex) const
	{
		return FMath::Max(Texture->Source.GetSizeY() >> InMipIndex, 1);
	}
...
private:
	...
	FORCEINLINE const FColor& GetHeightMapColor(const ULandscapeComponent* Component, int32 TexU, int32 TexV, FColor* TextureData);
...
void FLandscapeEditDataInterface::ShrinkData(TArray<T>& Data, int32 OldMinX, int32 OldMinY, int32 OldMaxX, int32 OldMaxY, int32 NewMinX, int32 NewMinY, int32 NewMaxX, int32 NewMaxY)
{
		...
		NewMaxX != OldMaxX || NewMaxY != OldMaxY)
	{
		...
		if (NewMinX != OldMinX || NewMinY != OldMinY || NewMaxX != OldMaxX)
		{
			...
			{
//				UE_LOGF(LogLandscape, Warning, "Dest: %d, %d = %d Src: %d, %d = %d Width = %d", 0, DestY, DestY * (1 + NewMaxX - NewMinX), NewMinX - OldMinX, SrcY, SrcY * (1 + OldMaxX - OldMinX) + NewMinX - OldMinX, (1 + NewMaxX - NewMinX));
...
struct FHeightmapAccessor
{
	...
	FHeightmapAccessor(ULandscapeInfo* InLandscapeInfo, ULandscapeLayerInfoObject* InLayerInfo = nullptr)
	...
	FHeightmapAccessor(FHeightmapAccessor&& InOther) = default;
	FHeightmapAccessor& operator = (FHeightmapAccessor&& InOther) = default;
	FHeightmapAccessor(const FHeightmapAccessor& InOther) = delete;
	FHeightmapAccessor& operator = (const FHeightmapAccessor& InOther) = delete;
	...
	void SetData(int32 X1, int32 Y1, int32 X2, int32 Y2, const uint16* Data, ELandscapeLayerPaintingRestriction PaintingRestriction = ELandscapeLayerPaintingRestriction::None)
	{
		...
		if (LandscapeInfo && LandscapeEdit.GetComponentsInRegion(X1, Y1, X2, Y2, &Components))
		{
			...
			if (GetDefault<ULandscapeSettings>()->GetFoliageLandscapeSnappingMode() == EFoliageLandscapeSnappingMode::SnapUnderBrushOnly)
			{
				bool bHasAttachedFoliage = false;
				for (ULandscapeComponent* Component : Components)
				{
					if (ULandscapeHeightfieldCollisionComponent* CollisionComponent = Component->GetCollisionComponent();
						CollisionComponent && AInstancedFoliageActor::HasFoliageAttached(CollisionComponent))
					{
						bHasAttachedFoliage = true;
						break;
					}
				}

				if (bHasAttachedFoliage)
				{
					TArray<ULandscapeHeightfieldCollisionComponent*> CollisionComponents;
					TArray<FBox> PreUpdateLocalBoxes;
					CollisionComponents.Reserve(Components.Num());
					PreUpdateLocalBoxes.Reserve(Components.Num());
					...
					for (ULandscapeComponent* Component : Components)
					{
						if (ULandscapeHeightfieldCollisionComponent* CollisionComponent = Component->GetCollisionComponent())
						{
							...
							PreUpdateLocalBoxes.Add(FBox(
								FVector(static_cast<float>(X1), static_cast<float>(Y1), Component->CachedLocalBox.Min.Z),
								FVector(static_cast<float>(X2), static_cast<float>(Y2), Component->CachedLocalBox.Max.Z)));
						}
					}

					const FMatrix44d LandscapeToWorld = LandscapeInfo->GetLandscapeProxy()->LandscapeActorToWorld().ToMatrixWithScale();

					for (int32 Index = 0; Index < CollisionComponents.Num(); ++Index)
					{
						CollisionComponents[Index]->MarkAreaForSnapFoliageInstances(
							PreUpdateLocalBoxes[Index].TransformBy(LandscapeToWorld).ExpandBy(1.0f));
					}
				}
			}
...
struct TAlphamapAccessor
{
	...
	TAlphamapAccessor(TAlphamapAccessor&& InOther) = default;
	TAlphamapAccessor& operator = (TAlphamapAccessor&& InOther) = default;
	TAlphamapAccessor(const TAlphamapAccessor& InOther) = delete;
	TAlphamapAccessor& operator = (const TAlphamapAccessor& InOther) = delete;
