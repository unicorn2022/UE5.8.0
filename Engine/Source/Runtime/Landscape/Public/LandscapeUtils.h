...
LANDSCAPE_API bool SupportsRuntimeGrassMapGeneration();
...
public:
	...
	public:
		template <size_t N>
		FScopedBreadcrumbEvent(FRDGBuilderRecorder& InRecorder, TCHAR const(&StaticName)[N], FRDGEventName&& EventName)
			: Recorder(&InRecorder)
...
/**
 * Analyzes the material's material expressions and return the landscape target layer names declared in them
 * @param InMaterial the material to analyze
 * @param bIncludeVisibilityLayer when false, removes the landscape visibility layer from the list of names returned by the material
 * 
 * @return the list of unique target layer names declared in that material
 */
LANDSCAPE_API TArray<FName> RetrieveTargetLayerNamesFromMaterial(UMaterialInterface* InMaterial, bool bIncludeVisibilityLayer = true);
...
LANDSCAPE_API TArray<TPair<FName, TObjectPtr<ULandscapeGrassType>>> ConvertToNamedGrassTypesList(const TMap<FName, TObjectPtr<ULandscapeGrassType>>& InNamedGrassTypes);
/** From the list of named grass types, only retain those with a valid name and grass type. */
LANDSCAPE_API TMap<FName, TObjectPtr<ULandscapeGrassType>> FilterValidNamedGrassTypes(const TMap<FName, TObjectPtr<ULandscapeGrassType>>& InNamedGrassTypes);
