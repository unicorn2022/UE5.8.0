...
UENUM()
enum class EFoliageLandscapeSnappingMode : uint8
{
	/** Foliage will not ever be snapped to the landscape */
	DontSnap,
	/** Foliage will only be snapped on areas where the landscape sculpt brush tool was used. This doesn't include procedural changes (e.g. water brush) or global updates (e.g. deactivating a layer) */
	SnapUnderBrushOnly,
	/** Foliage will be snapped onto the entire landscape upon update. This includes procedural changes (e.g. water brush) or global updates (e.g. deactivating a layer) */
	SnapEverywhere
};
...
public:
	...
	EFoliageLandscapeSnappingMode GetFoliageLandscapeSnappingMode() const { return FoliageLandscapeSnappingMode; }
...
protected:
	...
	UPROPERTY(EditAnywhere, config, Category = "Edit Layers")
	EFoliageLandscapeSnappingMode FoliageLandscapeSnappingMode = EFoliageLandscapeSnappingMode::DontSnap;

	UPROPERTY(transient)
	bool bRestrictiveMode = false;

	/** Offset in Z for the landscape spline point icon in world-space. */
	...
	/** Size of the landscape spline control point icon in the viewport. */
