...
void FLandscapeTextureStreamingManager::UnrequestTextureFullyStreamedIn(UTexture* Texture)
{
	...
	if (State)
	{
		...
		{
			UE_LOGF(LogLandscape, Warning, "Texture Streaming Manager received more Unrequests than Requests to stream texture %ls", *Texture->GetName());
		}
	}
}
...
FLandscapeTextureStreamingManager::~FLandscapeTextureStreamingManager()
{
	...
	{
		UE_LOGF(LogLandscape, Display, "At destruction, the Landscape Texture Streaming Manager still has streaming requests for %d Textures, this may indicate failure to clean them up.", RemainingRequests);
	}
}
