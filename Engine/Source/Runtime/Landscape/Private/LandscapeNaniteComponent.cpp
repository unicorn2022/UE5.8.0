	...
	bool Nanite::FAsyncBuildData::CheckForStallAndWarn()
	{
		...
		if (bStalled)
		{
			...
			if (!bWarnedStall)
			{
				...
				UE_LOGF(LogLandscape, Warning, "Nanite Build Task for '%ls' is taking a long time: Req:%f Exp:%f-%f MB:%f-%f BB:%f PMB:%f LU:%f-%f Complete:%f Cancelled:%f Now:%f bResult:%d bCancel:%d bNeedsPMB:%d  Changing landscape.Nanite.StallDetectionTimeout controls how long until this message appears.",
					*LandscapeName,
					TimeStamp_Requested,
...
void ULandscapeNaniteComponent::CollectPSOPrecacheData(const FPSOPrecacheParams& BasePrecachePSOParams, FMaterialInterfacePSOPrecacheParamsList& OutParams)
{
	...
	for (FMaterialInterfacePSOPrecacheParams& Params : OutParams)
	{
		...
		// Ignore masked because don't want this to be the only reason for pixel programmable
		Params.PSOPrecacheParams.bIgnoreMaskMaterial = true;
	}
}
...
{
	UE_LOGF(LogLandscape, VeryVerbose, "InitializeForLandscapeAsync actor: '%ls' package:'%ls'", *Landscape->GetActorNameOrLabel(), *Landscape->GetPackage()->GetName());

	check(bVisibleInRayTracing == false);

	// Make sure we don't allow Nanite landscape to be built in standalone editor (-game) mode (see GetNaniteContentId())
	check(GIsEditor);
	...
	FGraphEventRef ExportMeshEvent = FFunctionGraphTask::CreateAndDispatchWhenReady(
		...
		{			
			...
			UE_LOGF(LogLandscape,  VeryVerbose, "Exporting actor '%ls' package:'%ls'", *Name, *AsyncBuildData->LandscapeWeakRef->GetPackage()->GetName());
			...
			FString ExportDDCKey;
			{
				...
				static const char* MeshExportVersion = "b395d787-a7f0-1be9-244a-0c8689c8b7a6";
				...
				// Ensure the key also changes when the FMeshDescription serialization format changes.
				FTCHARToUTF8 MeshSerializationVersionId(*FMeshDescriptionBulkData::GetSerializationVersionIdString());
				Hasher.Update(reinterpret_cast<const uint8*>(MeshSerializationVersionId.Get()), MeshSerializationVersionId.Length() * sizeof(UTF8CHAR));
			...
			if (!bSuccess)
			{
				UE_LOGF(LogLandscape, Log, "Failed export of raw static mesh for Nanite landscape (%i components) for actor %ls : (DDC: %d, DDC read: %lld bytes, DDC write: %lld bytes, key: %ls, export: %f seconds)", AsyncBuildData->InputComponents.Num(), *Name, bUseNaniteExportCache, DDCReadBytes, DDCWriteBytes, *ExportDDCKey, ExportSeconds);
				if (AsyncBuildData->TimeStamp_Cancelled < 0.0)
				{
					AsyncBuildData->TimeStamp_Cancelled = FPlatformTime::Seconds();
				}
			...
			UE_LOGF(LogLandscape, Log, "Successful export of raw static mesh for Nanite landscape (%i components) for actor %ls : (DDC: %d, DDC read: %lld bytes, DDC write: %lld bytes, key: %ls, export: %f seconds, commit: %f seconds)", AsyncBuildData->InputComponents.Num(), *Name, bUseNaniteExportCache, DDCReadBytes, DDCWriteBytes, *ExportDDCKey, ExportSeconds, DurationSeconds - ExportSeconds);
	...
	FGraphEventRef BatchBuildEvent = FFunctionGraphTask::CreateAndDispatchWhenReady([AsyncBuildData, Component = this, NewProxyContentId, Name = Landscape->GetActorNameOrLabel(), StaticMeshBuildCompleteEvent, InNaniteComponentIndex]()
		{
			...
			if (AsyncBuildData->bCancelled || !AsyncBuildData->LandscapeWeakRef.IsValid())
			{
				...
				UE_LOGF(LogLandscape, Verbose, "CANCELLED Build Static Mesh '%ls'", *Name);
				OnFinishTask();
				if (AsyncBuildData->TimeStamp_Cancelled < 0.0)
				{
					AsyncBuildData->TimeStamp_Cancelled = FPlatformTime::Seconds();
			...
			UE_LOGF(LogLandscape, VeryVerbose, "Build Static Mesh '%ls' package:'%ls'", *Name, *AsyncBuildData->LandscapeWeakRef->GetPackage()->GetName());
			auto CompleteStaticMesh = [AsyncBuildData, Component, NewProxyContentId, Name, InNaniteComponentIndex, OnFinishTask](UStaticMesh* InStaticMesh)
			{
				...
				if (AsyncBuildData->bStaticMeshNeedsToCallPostMeshBuild)
				{
					...
					UE_LOGF(LogLandscape, Verbose, "Called CompleteStaticMesh from PostMeshBuild for %ls", *Name);
				...
				UE_LOGF(LogLandscape, VeryVerbose, "Complete Static Mesh '%ls' package:'%ls'", *Name, *AsyncBuildData->LandscapeWeakRef->GetPackage()->GetName());
				AsyncBuildData->TimeStamp_LandscapeUpdateEnd = FPlatformTime::Seconds();
			...
			UE_LOGF(LogLandscape, VeryVerbose, "Attaching to PostMeshBuild for %ls", *Name);
...
{
	UE_LOGF(LogLandscape, Display, "SetSourceLandscapeComponents for '%ls' package:'%ls'", *GetLandscapeProxy()->GetActorNameOrLabel(), *GetLandscapeProxy()->GetPackage()->GetName());
...
bool ULandscapeNaniteComponent::InitializePlatformForLandscape(ALandscapeProxy* Landscape, const ITargetPlatform* TargetPlatform)
{
	...
	UE_LOGF(LogLandscape, Verbose, "InitializePlatformForLandscape '%ls' package:'%ls'", *Landscape->GetActorNameOrLabel(), *Landscape->GetPackage()->GetName());
	...
	if (TargetPlatform)
	{
		UE_LOGF(LogLandscape, Verbose, "InitializePlatformForLandscape '%ls' platform:'%ls'", *Landscape->GetActorNameOrLabel(), *TargetPlatform->DisplayName().ToString());
		if (UStaticMesh* NaniteStaticMesh = GetStaticMesh())
		{
			UE_LOGF(LogLandscape, Verbose, "InitializePlatformForLandscape '%ls' mesh:'%p'", *Landscape->GetActorNameOrLabel(), NaniteStaticMesh);
			...
			while (!NaniteStaticMesh->IsCachedCookedPlatformDataLoaded(TargetPlatform))
			{
				...
				{
					UE_LOGF(LogLandscape, Error, "ULandscapeNaniteComponent::InitializePlatformForLandscape waited more than %f seconds for IsCachedCookedPlatformDataLoaded to return true", MaxWaitSeconds);
					return false;
				}
			}
			UE_LOGF(LogLandscape, Verbose, "InitializePlatformForLandscape '%ls' Finished in %f", *Landscape->GetActorNameOrLabel(), FPlatformTime::Seconds() - StartTime);
		}	
	}
