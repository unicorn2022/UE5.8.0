...
#if WITH_EDITOR
UE_DEPRECATED(5.8, "The CVar has been fully deprecated.")
TAutoConsoleVariableDeprecated_NoReplace<int32> CVarForceLayersUpdate_Deprecated(
	/*Name = */TEXT("landscape.ForceLayersUpdate"),
	/*DefaultValue = */0, 
	/*DeprecatedAtVersion = */TEXT("5.8"), 
	/*UsageBehavior = */EShadowCVarBehavior::Warn, 
	/*DeprecationMessage = */TEXT("landscape.ForceLayersUpdate is deprecated. Use landscape.ForceLayersFullUpdate to trigger a single global edit layers update."));
...
TAutoConsoleVariableDeprecated_NoReplace<int32> CVarOutputLayersRTContent_Deprecated(
	/*Name = */TEXT("landscape.OutputLayersRTContent"),
	/*DefaultValue = */0,
	/*DeprecatedAtVersion = */TEXT("5.7"),
	/*UsageBehavior = */EShadowCVarBehavior::Warn,
	/*DeprecationMessage = */TEXT("landscape.OutputLayersRTContent is deprecated. This was only used in Global merge. Batched merge (landscape.EditLayersLocalMerge.Enable 2) is the only merge technique available now."));

UE_DEPRECATED(5.7, "The CVar has been fully deprecated.")
TAutoConsoleVariableDeprecated_NoReplace<int32> CVarOutputLayersWeightmapsRTContent_Deprecated(
	/*Name = */TEXT("landscape.OutputLayersWeightmapsRTContent"),
	/*DefaultValue = */0,
	/*DeprecatedAtVersion = */TEXT("5.7"),
	/*UsageBehavior = */EShadowCVarBehavior::Warn,
	/*DeprecationMessage = */TEXT("landscape.OutputLayersWeightmapsRTContent is deprecated. This was only used in Global merge. Batched merge (landscape.EditLayersLocalMerge.Enable 2) is the only merge technique available now."));
...
UE_DEPRECATED(5.7, "The CVar has been fully deprecated.")
TAutoConsoleVariableDeprecated_NoReplace CVarLandscapeEditLayersLocalMerge_Deprecated(
	/*Name = */TEXT("landscape.EditLayersLocalMerge.Enable"), 
	/*DefaultValue = */0,
	/*DeprecatedAtVersion = */TEXT("5.7"), 
	/*UsageBehavior = */EShadowCVarBehavior::Warn, 
	/*DeprecationMessage = */TEXT("landscape.EditLayersLocalMerge.Enable is deprecated. Batched merge (landscape.EditLayersLocalMerge.Enable 2) is the only merge technique available now."));

UE_DEPRECATED(5.7, "The CVar has been fully deprecated.")
TAutoConsoleVariableDeprecated_NoReplace CVarLandscapeEditLayersMaxComponentsPerHeightmapResolveBatch_Deprecated(
	/*Name = */TEXT("landscape.EditLayersLocalMerge.MaxComponentsPerHeightmapResolveBatch"),
	/*DefaultValue = */0,
	/*DeprecatedAtVersion = */TEXT("5.7"),
	/*UsageBehavior = */EShadowCVarBehavior::Warn,
	/*DeprecationMessage = */TEXT("landscape.EditLayersLocalMerge.MaxComponentsPerHeightmapResolveBatch is deprecated. This was only used in Local merge. Batched merge (landscape.EditLayersLocalMerge.Enable 2) is the only merge technique available now."));

UE_DEPRECATED(5.7, "The CVar has been fully deprecated.")
TAutoConsoleVariableDeprecated_NoReplace CVarLandscapeEditLayersMaxComponentsPerWeightmapResolveBatch_Deprecated(
	/*Name = */TEXT("landscape.EditLayersLocalMerge.MaxComponentsPerWeightmapResolveBatch"),
	/*DefaultValue = */0,
	/*DeprecatedAtVersion = */TEXT("5.7"),
	/*UsageBehavior = */EShadowCVarBehavior::Warn,
	/*DeprecationMessage = */TEXT("landscape.EditLayersLocalMerge.MaxComponentsPerWeightmapResolveBatch is deprecated. This was only used in Local merge. Batched merge (landscape.EditLayersLocalMerge.Enable 2) is the only merge technique available now."));
...
private:
	...
	void ClearDirty()
	{
		...
		UE_LOGF(LogLandscape, Verbose, "Landscape.Dirty: %ls", bCleared ? TEXT("Cleared") : TEXT("Landscape.Dirty: Nothing to clear"));
	}
};
...
void ALandscape::ConvertNonEditLayerLandscape()
{
	...
	{
		UE_LOGF(LogLandscape, Error, "Attempting to make illegal call to ConvertNonEditLayerLandscape during UpdateLayersContent.");
		return;
	}
	...
	{
		UE_LOGF(LogLandscape, Warning, "Attempting to ConvertNonEditLayerLandscape on a landscape with edit-layer data.");
		return;
	}
...
void ALandscape::CopyDataToEditLayer(ALandscapeProxy* InProxy, const FGuid& InSourceEditLayerGuid, const FGuid& InDestEditLayerGuid, bool bInUseObsoleteLayerData)
{
	...
	{
		UE_LOGF(LogLandscape, Error, "CopyDataToEditLayer: Failed to copy data to destination layer for proxy %ls. Destination edit layer is not found in ALandscape or does not support persistent edit layer data", *InProxy->GetName());
		return;
	}
	...
	for (ULandscapeComponent* Component : InProxy->LandscapeComponents)
	{
		...
		if (DestLayerComponentData == nullptr || !DestLayerComponentData->IsInitialized())
		{
			...
			UE_LOGF(LogLandscape, Warning, "CopyDataToEditLayer: Defensively creating edit layer before data copy. It's expected this layer should have already been created via FixupSharedData. Other shared properties may not have upgraded properly!");
		}
...
TArray<UE::Landscape::EditLayers::FEditLayerRendererState> ALandscape::GetEditLayerRendererStatesEnableOverride(const UE::Landscape::EditLayers::FMergeContext* InMergeContext, const TBitArray<>& InLayerEnableOverride)
{
	...
	for (FLandscapeLayer& Layer : LandscapeEditLayers)
	{
		...
		{
			ELandscapeToolTargetTypeFlags MergeTypeMask = InMergeContext->IsHeightmapMerge() ? ELandscapeToolTargetTypeFlags::Heightmap : (ELandscapeToolTargetTypeFlags::Weightmap | ELandscapeToolTargetTypeFlags::Visibility);
			bool bEnabled = InLayerEnableOverride[LayerIdx];
			Algo::ForEach(LayerRendererStates, [MergeTypeMask, bEnabled](FEditLayerRendererState& State)
				{
					// Don't try to enable/disable a state if it's not supported :
					if (EnumHasAnyFlags(State.GetSupportedTargetTypeState().GetTargetTypeMask(), MergeTypeMask))
					{
						if (bEnabled)
						{
							State.EnableTargetTypeMask(MergeTypeMask);
						}
						else
						{
							State.DisableTargetTypeMask(MergeTypeMask);
						}
					}
				});
		}
...
TArray<UE::Landscape::EditLayers::FEditLayerRenderItem> ULandscapeEditLayerPersistent::GetRenderItems(const UE::Landscape::EditLayers::FMergeContext* InMergeContext) const
{
	...
	// Each component has an input local area of (-1, -1) -> (0, 0) in order to make sure that components on the left, top-left and top get included in the batch and rendered.
	//  This is to resolve potential non-determinism in case source data differs on the common edge between neighboring components : in such a case, components on the left and top 
	//  have authority (other edges are ignored), so we sort and render components in this order in order to enforce that behavior. 
	//  See FMergeRenderContext::RenderComponentIds for details
	const FIntRect InputArea(FIntPoint(-1, -1), FIntPoint::ZeroValue);
	...
	{
		// The layer needs the component itself. Plus its 3 neighbors (see comment about InputArea definition), to render in a deterministic way
		FInputWorldArea InputWorldArea(FInputWorldArea::CreateLocalComponent(/*InLocalArea = */InputArea));
	...
	// Weightmaps : add one render item per component in order to be able to indicate exactly which weightmap is needed for each of them. This avoids pre-allocating weightmaps on the merged result where we know we won't write a weightmap : 
	{
		...
		for (ULandscapeComponent* Component : AllLandscapeComponents)
		{
			...
			if (FLandscapeLayerComponentData* LayerData = Component->GetLayerData(GetGuid()))
			{
				...
				{
					// The layer needs the component itself. Plus its 3 neighbors (see comment about InputArea definition), to render in a deterministic way
					FInputWorldArea InputWorldArea(FInputWorldArea::CreateSpecificComponent(ComponentKey, /*InLocalArea = */InputArea));
					// The layer only writes into the component itself (i.e. it renders to the area that it's currently being asked to render to):
...
namespace UE::Landscape::EditLayers::Private
{
	...
	TArray<FRenderBatchInfo> DivideIntoBatches(const TBitArray<>& InFinalComponentsToRenderInfoBitIndices, const TArray<FComponentToRenderInfo>& InAllComponentsToRenderInfos, bool &bInOutWarnedResolution, bool &bInOutWarnedMergeDimensionsExceeded)
	{
		...
		if (MinBatchResolution.X > GRHIGlobals.MaxTextureDimensions || MinBatchResolution.Y > GRHIGlobals.MaxTextureDimensions)
		{
			...
			{
				UE_LOGF(LogLandscape, Error, "Cannot render landscape edit layers because the current device does not support render targets of the required size (required resolution: %ix%i, "
...
UE::Landscape::EditLayers::FMergeRenderContext ALandscape::PrepareEditLayersMergeRenderContext(const UE::Landscape::EditLayers::FMergeContext& InMergeContext, const UE::Landscape::EditLayers::FMergeRenderParams& InMergeRenderParams)
{
	...
	if (!InMergeContext.IsHeightmapMerge())
	{
		for (FName TargetLayerName : InMergeRenderParams.WeightmapLayerNames)
		{
			...
			{
				UE_LOGF(LogLandscape, Warning, "Target layer \"%ls\" was requested by the merge but is invalid (missing its layer info object asset). It will be ignored", *TargetLayerName.ToString());
			}
		}
	}
	...
	// Prepare a dummy copy for early-outs :
	const FMergeRenderContext DummyMergeRenderContext(MergeRenderContext);
	check(!DummyMergeRenderContext.IsValid());
	...
	else
	{
		...
		{
			return DummyMergeRenderContext;
		}
	...
	if (!VisualLogShowComponentDependenciesFilter.IsEmpty())
	{
		...
		if (ComponentKey.InitFromString(VisualLogShowComponentDependenciesFilter))
		{
			...
			{
				UE_LOGF(LogLandscape, Warning, "Component key \"%ls\" specified for dependencies filter does not correspond to a valid component. Ignoring show component dependencies filter", *VisualLogShowComponentDependenciesFilter);
			}
		}
		else
		{
			UE_LOGF(LogLandscape, Warning, "Cannot parse string \"%ls\". Ignoring show component dependencies filter", *VisualLogShowComponentDependenciesFilter);
		}
	}
...

	{
		...
		// Last chance to bail out from the merge if we are beyond the capabilities for rendering batches
		if ((static_cast<int64>(MergeRenderContext.MaxNumComponents.X) * static_cast<int64>(MergeRenderContext.MaxNumComponents.Y)) >= static_cast<int64>(FMergeRenderContext::MaxComponentsPerBatch))
		{
			if (!bWarnedNumComponentsPerBatchExceeded)
			{
				UE_LOGF(LogLandscape, Error, "Cannot render landscape edit layers because one of the batches requires more than %u components, (component count : %ix%i). "
					"Please reduce the loaded landscape size, or make sure that all edit layer renderers participating to this landscape don't require an input area with "
					"this many components.",
					FMergeRenderContext::MaxComponentsPerBatch, MergeRenderContext.MaxNumComponents.X, MergeRenderContext.MaxNumComponents.Y);
				bWarnedNumComponentsPerBatchExceeded = true;
			}
			return DummyMergeRenderContext;
		}
...
int32 ALandscape::RegenerateLayersHeightmaps(const FUpdateLayersContentContext& InUpdateLayersContentContext)
{
	...
	if ((HeightmapUpdateModes == 0) || Info == nullptr)
	{
		return 0;
	}
...
void ALandscape::UpdateForChangedHeightmaps(const TArrayView<FLandscapeEditLayerComponentReadbackResult>& InComponentReadbackResults)
{
	...
	for (const FLandscapeEditLayerComponentReadbackResult& ComponentReadbackResult : InComponentReadbackResults)
	{
		...
		const uint32 HeightUpdateMode = ComponentReadbackResult.UpdateModes & (ELandscapeLayerUpdateMode::Update_Heightmap_All_Modes | ELandscapeLayerUpdateMode::Update_Heightmap_Editing | ELandscapeLayerUpdateMode::Update_Heightmap_Editing_NoCollision);
	...
	TArray<FLandscapeEditLayerComponentReadbackResult>& InOutComponentReadbackResults)
{
	...
	for (UTexture2D* Heightmap : HeightmapsToResolve)
	{
		...
		{
			// Only process the readback object if it has active tasks.
			if (!(*CPUReadback)->HasActiveTasks())
			{
				continue;
			}
					...
					&& ((1 << AllocInfo.WeightmapTextureChannel) & ChangedChannelsMask) != 0)
				{
					...
					if (bDumpDiff)
					{
						...
						FString FilePattern = FString::Format(TEXT("{0}/LandscapeLayers/{1}/{2}/{3}/Weightmaps/{4}/{5}-{6}-{7}[mip{8}]"), 
							{ FPaths::ProjectSavedDir(), CurrentTime.ToString(), WorldName, ParentLandscapeActorName, AllocInfo.GetLayerName().ToString(), ActorName, Component->GetDebugName(), InWeightmap->GetName(), InMipLevel});
	...
	if (Components != nullptr)
	{
		for (ULandscapeComponent* Component : *Components)
		{
			...
			if (bDumpDiff)
			{
				...
				check(Proxy);
				FString ActorName = Proxy->GetActorLabel();
				FString FilePattern = FString::Format(TEXT("{0}/LandscapeLayers/{1}/{2}/{3}/Heightmaps/{4}-{5}-{6}[mip{7}]"), 
					{ FPaths::ProjectSavedDir(), CurrentTime.ToString(), WorldName, ParentLandscapeActorName, ActorName, Component->GetDebugName(), InHeightmap->GetName(), InMipLevel });
				...
				{
					FFileHelper::CreateBitmap(*(FilePattern + "_a(pre).bmp"), SizeU, SizeV, InOldData, &SubRegion, &IFileManager::Get(), /*OutFilename = */nullptr, /*bInWriteAlpha = */true);
					FFileHelper::CreateBitmap(*(FilePattern + "_b(post).bmp"), SizeU, SizeV, InNewData, &SubRegion, &IFileManager::Get(), /*OutFilename = */nullptr, /*bInWriteAlpha = */true);
...
void ALandscape::ReallocateLayersWeightmaps(FUpdateLayersContentContext& InUpdateLayersContentContext, 
	const TMap<ULandscapeComponent*, TArray<ULandscapeLayerInfoObject*>>& InPerComponentAllocations, 
	const TSet<ULandscapeComponent*>& InRestrictTextureSharingToComponents)
{
	...
	InUpdateLayersContentContext.LandscapeComponentsWeightmapsToResolve.RemoveAllSwap([&InPerComponentAllocations, &ComponentsWithChangedAllocs, &NumNoAllocs, &NumUndoFlagged](ULandscapeComponent* Component) -> bool
		{
			const TArray<ULandscapeLayerInfoObject*>* ComponentLayerAlloc = InPerComponentAllocations.Find(Component);
	...
	for (auto& ItPair : InPerComponentAllocations)
	{
		ULandscapeComponent* Component = ItPair.Key;
		const TArray<ULandscapeLayerInfoObject*>& ComponentLayerAlloc = ItPair.Value;
	...
	UE_LOGF(LogLandscape, VeryVerbose, "ReallocateLayersWeightmaps - Components ToResolve: %d, Invalidated: %d - Added: %d, Removed: %d, NoAlloc: %d, UndoFlagged: %d",
...
int32 ALandscape::PerformLayersWeightmapsBatchedMerge(FUpdateLayersContentContext& InUpdateLayersContentContext, const FEditLayersWeightmapMergeParams& InMergeParams)
{
...

	{
		...
		TMap<ULandscapeComponent*, TArray<ULandscapeLayerInfoObject*>> PerComponentAllocations;
		{
			...
			PerComponentAllocations.Reserve(InUpdateLayersContentContext.LandscapeComponentsWeightmapsToResolve.Num());
			for (ULandscapeComponent* ComponentToResolve : InUpdateLayersContentContext.LandscapeComponentsWeightmapsToResolve)
			{
				const TBitArray<>* ComponentTargetLayerBitIndices = MergeRenderContext.ComponentToTargetLayerBitIndices.Find(ComponentToResolve);
				check(ComponentTargetLayerBitIndices != nullptr);
				PerComponentAllocations.Add(ComponentToResolve, MergeRenderContext.ConvertTargetLayerBitIndicesToLayerInfos(*ComponentTargetLayerBitIndices));
			}
		}
		...
		ReallocateLayersWeightmaps(InUpdateLayersContentContext, PerComponentAllocations, RestrictTextureSharingToComponents);
				...
				(FRHICommandListImmediate& InRHICmdList)
			{
				...
				for (auto It : WeightmapResolveInfosForBatch)
				{
...

					{
						...
						for (int32 MipLevel = 0; MipLevel < NumMipsToCopy; ++MipLevel)
						{
...
void ALandscape::UpdateForChangedWeightmaps(const TArrayView<FLandscapeEditLayerComponentReadbackResult>& InComponentReadbackResults)
{
	...
	for (const FLandscapeEditLayerComponentReadbackResult& ComponentReadbackResult : InComponentReadbackResults)
	{
		...
		const int32 WeightUpdateMode = ComponentReadbackResult.UpdateModes & (ELandscapeLayerUpdateMode::Update_Weightmap_All_Modes | ELandscapeLayerUpdateMode::Update_Weightmap_Editing | ELandscapeLayerUpdateMode::Update_Weightmap_Editing_NoCollision);
...
void ALandscape::RequestLayersContentUpdateForceAll(ELandscapeLayerUpdateMode InModeMask, bool bInUserTriggered)
{
	...
	const bool bUpdateWeightmap = (InModeMask & (ELandscapeLayerUpdateMode::Update_Weightmap_All_Modes | ELandscapeLayerUpdateMode::Update_Weightmap_Editing | ELandscapeLayerUpdateMode::Update_Weightmap_Editing_NoCollision)) != 0;
	const bool bUpdateHeightmap = (InModeMask & (ELandscapeLayerUpdateMode::Update_Heightmap_All_Modes | ELandscapeLayerUpdateMode::Update_Heightmap_Editing | ELandscapeLayerUpdateMode::Update_Heightmap_Editing_NoCollision)) != 0;
	const bool bUpdateWeightCollision = (InModeMask & (ELandscapeLayerUpdateMode::Update_Weightmap_All_Modes | ELandscapeLayerUpdateMode::Update_Weightmap_Editing)) != 0;
	const bool bUpdateHeightCollision = (InModeMask & (ELandscapeLayerUpdateMode::Update_Heightmap_All_Modes | ELandscapeLayerUpdateMode::Update_Heightmap_Editing)) != 0;
	const bool bUpdateAllHeightmapModes = (InModeMask & ELandscapeLayerUpdateMode::Update_Heightmap_All_Modes) != 0;
	const bool bUpdateAllWeightmapModes = (InModeMask & ELandscapeLayerUpdateMode::Update_Weightmap_All_Modes) != 0;
	const bool bUpdateClientUpdateEditing = (InModeMask & ELandscapeLayerUpdateMode::Update_Client_Editing) != 0;
	if (ULandscapeInfo* LandscapeInfo = GetLandscapeInfo())
	{
		LandscapeInfo->ForEachLandscapeProxy([bUpdateHeightmap, bUpdateWeightmap, bUpdateAllHeightmapModes, bUpdateAllWeightmapModes, bUpdateHeightCollision, bUpdateWeightCollision, bUpdateClientUpdateEditing, bInUserTriggered](ALandscapeProxy* Proxy)
		{
			if (Proxy)
			{
				for (ULandscapeComponent* Component : Proxy->LandscapeComponents)
				{
					if (bUpdateHeightmap)
					{
						Component->RequestHeightmapUpdate(bUpdateAllHeightmapModes, bUpdateHeightCollision, bInUserTriggered);
					}

					if (bUpdateWeightmap)
					{
						Component->RequestWeightmapUpdate(bUpdateAllWeightmapModes, bUpdateWeightCollision, bInUserTriggered);
					}

					if (bUpdateClientUpdateEditing)
					{
...
{
	if (InUpdateModes & ELandscapeLayerUpdateMode::Update_Heightmap_All_Modes)
	{
	...
	if (InUpdateModes & ELandscapeLayerUpdateMode::Update_Weightmap_All_Modes)
	{
...
{
	bUserTriggeredChangeRequested = bInUserTriggered;
	uint32 NewLayerUpdateFlags = 0;
	if (bUpdateAll || bUpdateCollision)
	{
		NewLayerUpdateFlags |= ELandscapeLayerUpdateMode::Update_Heightmap_Editing;
		NewLayerUpdateFlags |= ELandscapeLayerUpdateMode::Update_Heightmap_All_Modes;
	}
	else
	{
		NewLayerUpdateFlags |= ELandscapeLayerUpdateMode::Update_Heightmap_Editing_NoCollision;
	}

	LayerUpdateFlagPerMode |= NewLayerUpdateFlags;

	// The typical codepath for sculpt or paint will have bUpdateCollision=true and bUpdateAll=false.  Only pass through
	// the _All_Modes flag in the case of bUpdateAll, not just from bUpdateCollision.
	uint32 ActorFlags = NewLayerUpdateFlags;
	if (!bUpdateAll)
	{
		ActorFlags &= ~ELandscapeLayerUpdateMode::Update_Heightmap_All_Modes;  // Remove _All_Modes flag
	}

	if (ALandscape* LandscapeActor = GetLandscapeActor())
	{
		LandscapeActor->RequestLayersContentUpdate((ELandscapeLayerUpdateMode)ActorFlags);
	}
	else
	{
		// This condition is only valid with streaming proxies, since the landscape actor will always be reachable in the non-WP Case
		ALandscapeStreamingProxy* LandscapeStreamingProxy = CastChecked<ALandscapeStreamingProxy>(GetLandscapeProxy());
		LandscapeStreamingProxy->RequestDeferredLayerContentUpdateModes((ELandscapeLayerUpdateMode)ActorFlags);
	}
}
...
void ULandscapeComponent::RequestWeightmapUpdate(bool bUpdateAll, bool bUpdateCollision, bool bInUserTriggered)
{
	...
	if (bUpdateAll || bUpdateCollision)
	{
		NewLayerUpdateFlags |= ELandscapeLayerUpdateMode::Update_Weightmap_Editing;
		NewLayerUpdateFlags |= ELandscapeLayerUpdateMode::Update_Weightmap_All_Modes;
	}
	else
	{
		NewLayerUpdateFlags |= ELandscapeLayerUpdateMode::Update_Weightmap_Editing_NoCollision;
	}
	...
	if (!bUpdateAll)
	{
		ActorFlags &= ~ELandscapeLayerUpdateMode::Update_Weightmap_All_Modes;  // Remove _All_Modes flag
	}
...
void ALandscape::MonitorLandscapeEdModeChanges()
{
	...
	{
		// Update_Client_Editing does not trigger readback changes. As a result, UpdateCollisionAndClients will not be run
		// Use Update_All_Editing to force an update, making sure the contribution mode visualization state is updated with the read back results
		RequestLayersContentUpdateForceAll(ELandscapeLayerUpdateMode::Update_All_Editing);
	}
}
...
bool ALandscape::SelectiveRenderEditLayersHeightmaps(const FLandscapeEditLayerRenderHeightParams& InSelectiveRenderParams)
{
	...
	if (AffectedComponents.Num() == 0)
	{
		...
		UE_LOGF(LogLandscape, Display, "SelectiveRenderLayers with no affected components");
		return true;
	}
...
void ALandscape::UpdateLayersContent(bool bInWaitForStreaming, bool bInSkipMonitorLandscapeEdModeChanges, bool bFlushRender)
{
	...
	if (LayerUpdateCount > 0 && UE::Landscape::Private::InBPCallstack())
	{
		UE_LOGF(LogLandscapeBP, Error, "Attempting to make illegal re-entrant call to UpdateLayersContent.");
		return;
	}
	...
	const bool bProcessReadbacks = FLandscapeEditLayerReadback::HasAnyGlobalWork();
	...
	else
	{
		...
		if (LayerContentUpdateModes != ELandscapeLayerUpdateMode::Update_None)
		{
			...
			{
				UE_LOGF(LogLandscape, Warning, "Cannot update landscape with a feature level less than SM5");
	...
	bool bUpdateAllComponents = LayerContentUpdateModes & Update_All_Components;
	bool bPartialUpdate = !bUpdateAllComponents && CVarLandscapeLayerOptim.GetValueOnAnyThread() == 1;
	...
	ProcessedModes |= (LayerContentUpdateModes & ELandscapeLayerUpdateMode::Update_All_Components);
		...
		if (bDoUpdateClient && IsUpdateFlagEnabledForModes(ELandscapeComponentUpdateFlag::Component_Update_Client, ComponentReadbackResult.UpdateModes))
		{
			if (!GUndo)
			{
				if (ULandscapeHeightfieldCollisionComponent* CollisionComp = LandscapeComponent->GetCollisionComponent())
				{
					...
					const EFoliageLandscapeSnappingMode SnappingMode = GetDefault<ULandscapeSettings>()->GetFoliageLandscapeSnappingMode();
					if (SnappingMode != EFoliageLandscapeSnappingMode::DontSnap)
					{
						const ULandscapeHeightfieldCollisionComponent::ESnapFoliageInstancesFlags Flags = (SnappingMode == EFoliageLandscapeSnappingMode::SnapUnderBrushOnly)
							? ULandscapeHeightfieldCollisionComponent::ESnapFoliageInstancesFlags::MarkedAreasOnly
							: ULandscapeHeightfieldCollisionComponent::ESnapFoliageInstancesFlags::ForceAll;
						CollisionComp->SnapFoliageInstances(Flags);
					}
				}
			}
			else
...
bool ALandscapeProxy::RemoveObsoleteEditLayers(bool bShowMapCheckWarning)
{
	...
	{
		UE_LOGF(LogLandscape, Error, "RemoveObsoleteEditLayers: Cannot remove obsolete edit layer data until proxy is registered with parent ALandscape.");
		return bModified;
	}
...
TMap<FGuid, FName> ALandscapeProxy::GetObsoleteEditLayersInComponents()
{
	...
	{
		UE_LOGF(LogLandscape, Error, "GetObsoleteEditLayersInComponents: Cannot get obsolete edit layer data until proxy is registered with parent ALandscape.");
...
bool ALandscape::IsUpToDate() const
{
	...
	{
		return LayerContentUpdateModes == 0 && !FLandscapeEditLayerReadback::HasAnyGlobalWork();
	}
#endif
...
bool ALandscape::DeleteLayer(int32 InLayerIndex)
{
	...
	if (LayerUpdateCount > 0 && UE::Landscape::Private::InBPCallstack())
	{
		UE_LOGF(LogLandscapeBP, Error, "Attempting to make illegal call to DeleteLayer during UpdateLayersContent.");
		return false;
	}
	check(LayerUpdateCount == 0);
...
uint32 ULandscapeComponent::ComputeLayerHash(bool InReturnEditingHash) const
{
	...
	// Sort allocations infos by LayerInfo Path so the Weightmaps hashes get ordered properly
...
FLandscapeLayer* ALandscape::DuplicateLayerAndMoveBrushes(const FLandscapeLayer& InOtherLayer)
{
	...
	{
		UE_LOGF(LogLandscape, Warning, "Cannot duplicate layer : %ls as the max number of layers (%i) has been reached", *InOtherLayer.EditLayer->GetName().ToString(), GetDefault<ULandscapeSettings>()->MaxNumberOfLayers);
		return nullptr;
	}
...
int32 ALandscape::CreateLayer(FName InName, const TSubclassOf<ULandscapeEditLayerBase>& InEditLayerClass, bool bInIgnoreLayerCountLimit)
{
	...
	if (LayerUpdateCount > 0 && UE::Landscape::Private::InBPCallstack())
	{
		UE_LOGF(LogLandscapeBP, Error, "Attempting to make illegal call to CreateLayer during UpdateLayersContent.");
		return INDEX_NONE;
	}
	check(LayerUpdateCount == 0);
...
void LandscapeDumpSelectiveLayerRender(const TArray<FString>& Args)
{
	...
	for (int32 i = 0; i < Args.Num(); ++i)
	{
		...
		{
			UE_LOGF(LogLandscape, Error, "LandscapeDumpSelectiveLayerRender: Unknown parameter %ls", *Arg);
		}
	}
	...
	if (!Landscape)
	{
		UE_LOGF(LogLandscape, Error, "LandscapeDumpSelectiveLayerRender: landscape not found");
		return;
	}

	if (!bRenderHeight && !bRenderWeight)
	{
		UE_LOGF(LogLandscape, Error, "LandscapeDumpSelectiveLayerRender: must specify either -h or -w");
		return;
	}
	...
	if (!bHasRegion)
	{
		UE_LOGF(LogLandscape, Warning, "LandscapeDumpSelectiveLayerRender: couldn't determine landscape extent.  Nothing loaded?");
		return;
	}
	...
	if (bRenderWeight)
	{
			...
			WeightmapNames.SetNum(Algo::RemoveIf(WeightmapNames, [&AllNames](FName TestName)
				{
					...
					{
						UE_LOGF(LogLandscape, Warning, "LandscapeDumpSelectiveLayerRender: weightmap named %ls not found.", *TestName.ToString());
						return true;
					}
					return false;
				}));
		...
		if (WeightmapNames.IsEmpty())
		{
			UE_LOGF(LogLandscape, Warning, "LandscapeDumpSelectiveLayerRender: no weightmaps to render.");
			bRenderWeight = false;
		}
	}
	...
	if (bRenderHeight)
	{
		UE_LOGF(LogLandscape, Display, "LandscapeDumpSelectiveLayerRender: SelectiveRenderLayers heightmap, landscape=%ls region=(%d,%d)-(%d,%d) layers=%x",
		...
		if (!bRenderRet)
		{
			UE_LOGF(LogLandscape, Error, "LandscapeDumpSelectiveLayerRender: SelectiveRenderEditLayersHeightmaps failed");
			return;
		}
		...
		if (bUseRenderTarget)
		{
			...
			{
				UE_LOGF(LogLandscape, Error, "LandscapeDumpSelectiveLayerRender: ReadPixels failed");
				return;
			}
		...
		{
			UE_LOGF(LogLandscape, Display, "LandscapeDumpSelectiveLayerRender: wrote heightmap data to %ls", *Filename);
		}
		else
		{
			UE_LOGF(LogLandscape, Error, "LandscapeDumpSelectiveLayerRender: failed to write heightmap data to %ls", *Filename);
		}
	}
	...
	if (bRenderWeight)
	{
		UE_LOGF(LogLandscape, Display, "LandscapeDumpSelectiveLayerRender: SelectiveRenderLayers weightmaps, landscape=%ls region=(%d,%d)-(%d,%d) layers=%x",
			*Landscape->GetActorNameOrLabel(), Region.Min.X, Region.Min.Y, Region.Max.X, Region.Max.Y, LayerMask);
		...
		for (int32 NameIdx = 0; NameIdx < WeightmapNames.Num(); NameIdx++)
		{
			...
			if (FImageUtils::SaveImageByExtension(*Filename, ImageView))
			{
				UE_LOGF(LogLandscape, Display, "LandscapeDumpSelectiveLayerRender: wrote weightmap data to %ls", *Filename);
			}
			else
			{
				UE_LOGF(LogLandscape, Error, "LandscapeDumpSelectiveLayerRender: failed to write weightmap data to %ls", *Filename);
			}
		}
...
{
	TArray<FString> StringFilters(Args);
	...
	TSet<ALandscape*> UpdatedLandscapes;
	auto UpdateLandscapeLayers = [&UpdatedLandscapes](ALandscape* InLandscape, const FString& InSuffix)
		{
			if (!UpdatedLandscapes.Contains(InLandscape))
			{
				UE_LOGF(LogLandscape, Display, "LandscapeForceLayersFullUpdate : %ls updated%ls", *InLandscape->GetActorNameOrLabel(), *InSuffix);
				InLandscape->ForceLayersFullUpdate();
				UpdatedLandscapes.Add(InLandscape);
			}
		};

	TArray<FString> RemovedFilters;
	if (StringFilters.IsEmpty())
	{
		for (ALandscape* Landscape : TActorRange<ALandscape>(World))
		{
			UpdateLandscapeLayers(Landscape, TEXT(""));
		}
	}
	else
	{
	...
	for (ALandscape* Landscape : TActorRange<ALandscape>(World))
	{
		const FString ActorName = Landscape->GetActorNameOrLabel();
			// If we've found a landscape whose name perfectly matches, update layers and remove the name from the filters so that we don't run the command
			//  on other landscapes with a partial name match
			if (int32 PerfectMatchFilterIndex = StringFilters.IndexOfByKey(ActorName); PerfectMatchFilterIndex != INDEX_NONE)
		{
				UpdateLandscapeLayers(Landscape, FString::Format(TEXT(" (perfect name match with: \"{0}\")"), { StringFilters[PerfectMatchFilterIndex] }));
				// Note : don't use RemoveAt here in order to remove all potential duplicate filters
				StringFilters.Remove(ActorName);
				RemovedFilters.Add(ActorName);
		}
	}

		// Run again for the landscapes whose name may only partially match one of the filters
		for (ALandscape* Landscape : TActorRange<ALandscape>(World))
		{
			if (!UpdatedLandscapes.Contains(Landscape))
			{
				const FString ActorName = Landscape->GetActorNameOrLabel();
				if (int32 PartialMatchFilterIndex = StringFilters.IndexOfByPredicate([ActorName](const FString& InStringFilter) { return ActorName.Find(InStringFilter) != INDEX_NONE; }); 
					PartialMatchFilterIndex != INDEX_NONE)
				{
					UpdateLandscapeLayers(Landscape, FString::Format(TEXT(" (partial name match with: \"{0}\")"), { StringFilters[PartialMatchFilterIndex] }));
				}
				else if (int32 PartialMatchWithRemovedFilterIndex = RemovedFilters.IndexOfByPredicate([ActorName](const FString& InStringFilter) { return ActorName.Find(InStringFilter) != INDEX_NONE; });
					PartialMatchWithRemovedFilterIndex != INDEX_NONE)
				{
					UE_LOGF(LogLandscape, Display, "LandscapeForceLayersFullUpdate : %ls skipped  (partial name match with: \"%ls\" but another landscape with that exact name was found)", 
						*ActorName, *RemovedFilters[PartialMatchWithRemovedFilterIndex]);
				}
			}
		}
	}

	if (UpdatedLandscapes.IsEmpty())
	{
		UE_LOGF(LogLandscape, Display, "LandscapeForceLayersFullUpdate : no landscape updated");
	}
}
