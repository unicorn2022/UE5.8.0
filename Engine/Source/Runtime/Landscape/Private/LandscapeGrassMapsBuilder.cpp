...
#define GRASS_DEBUG_TRANSITION(StateRef, StageBefore, StageAfter) \
...
uint32 ComputeGrassInstanceGenerationHash(uint32 GrassMapGenerationHash, const TMap<FName, TObjectPtr<ULandscapeGrassType>>& NamedGrassTypes)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(ComputeGrassInstanceGenerationHash);

	// grass maps are input to grass instances
	return HashCombine(GrassMapGenerationHash, UE::Landscape::Grass::Private::ComputeNamedGrassTypesHash(NamedGrassTypes));
}

// Deprecated
bool CanRenderGrassMap(ULandscapeComponent *Component)
{
	return Component->CanRenderGrassMap();
}
...
{
	return UE::Landscape::SupportsRuntimeGrassMapGeneration();
}
...
FLandscapeGrassMapsBuilder::~FLandscapeGrassMapsBuilder()
{
	...
	if (ComponentStates.Num() != 0)
	{
		...
		{
			FComponentState* State = It.Value();
			UE_LOGF(LogGrass, Warning, "Failed to clear grass data state after %d iterations (stage:%d ticks:%d), forcing deletion of the state.", Iterations, EnumToUnderlyingType(State->Stage), State->TickCount);
			...
			if (State->Stage == EComponentStage::Rendering)
			{
				...
				{
					UE_LOGF(LogGrass, Warning, "  %ls", *State->ActiveRender->GameThreadAsyncReadbackPtr->ToString());
				}
			}
			else if (State->Stage == EComponentStage::AsyncFetch)
			{
				...
				{
					UE_LOGF(LogGrass, Warning, "  AsyncFetchTask: %p", State->AsyncFetchTask.Get());
				}
			}
		}
	...
	: Component(Component)
{
	...
	if (Material)
	{
		GrassMapGenerationHash = UE::Landscape::Grass::ComputeGrassMapGenerationHash(Component, Material);
		GrassInstanceGenerationHash = UE::Landscape::Grass::ComputeGrassInstanceGenerationHash(GrassMapGenerationHash, Material->GetCachedExpressionData().NamedGrassTypes);
	}
#endif // WITH_EDITOR
}
...
void FLandscapeGrassMapsBuilder::FPendingComponent::UpdatePriorityDistance(const TArray<FVector>&Cameras, float MustHaveDistanceScale)
{
	...
	const double MinSqrDistanceToComponent = UE::Landscape::Grass::Private::CalculateMinDistanceToCamerasSquared(Cameras, WorldBounds);
	...
	for (auto It = ComponentStates.CreateIterator(); It; ++It, ++ComponentStateIndex)
	{
		...
		else
		{
			...
			case EComponentStage::NotReady:
				// check to see if the conditions changed.
				if (AmortizedUpdate.ShouldUpdate(ComponentStateIndex))
				{
					if (Component->GetLandscapeProxy()->ShouldGenerateGrass() && Component->CanRenderGrassMap())
					{
			...
			case EComponentStage::TextureStreaming:
				if (!Component->GetLandscapeProxy()->ShouldGenerateGrass())
				{
					break; // cancel and evict to restart build process
				}
			...
			case EComponentStage::GrassMapsPopulated:
				...
				if (AmortizedUpdate.ShouldUpdate(ComponentStateIndex))
				{
					if (!Component->GetLandscapeProxy()->ShouldGenerateGrass()) // TODO put above the ShouldUpdate()?
					{
						break; // cancel and evict to restart build process
					}
					...
					if (!bIsGameWorld)
					{
						...
						const uint32 CurGrassMapGenerationHash = UE::Landscape::Grass::ComputeGrassMapGenerationHash(Component, Material);
						...
						const uint32 CurGrassInstanceGenerationHash = UE::Landscape::Grass::ComputeGrassInstanceGenerationHash(CurGrassMapGenerationHash, Component->GetNamedGrassTypes());
	...
	if (StreamingStatesToProcess.Num() > 0)
	{
		...
		for (int32 i = 0; (RenderingCount < LocalMaxRendering) && (i < StreamingStatesToProcess.Num()); i++)
		{
			...
			if (State->AreTexturesStreamedIn())
			{
				...
				if (!State->Component->CanRenderGrassMap())
				{
...
bool FLandscapeGrassMapsBuilder::CanCurrentlyRender() const
{
	if (FApp::CanEverRender())
	{
		...
		return UseGPUScene(ShaderPlatform);
	}
	return false;
}
...
void FLandscapeGrassMapsBuilder::AmortizedUpdateGrassMaps(
	...
	bool bAnyLandscapeActorHasGrassEnabled)
{
#if !WITH_EDITOR
	...
	if (!bAnyLandscapeActorHasGrassEnabled && !StreamingCount && !RenderingCount && !AsyncFetchCount && !PopulatedCount)
	{
		return;
	}
		...
		if (bRenderThisFrame)
		{
			...
			for (auto It = ComponentStates.CreateIterator(); It; ++It)
			{
				...
				if (ULandscapeComponent* Component = State->Component)
				{
					...
					if (bDebugThisComponent)
					{
						UE_LOGF(LogGrass, Display, "Issuing debug render for the grassmap on component'%ls'", *State->Component->GetName());
				...
				case EComponentStage::NotReady:
					{
						...
						if (!FailedComponents.Contains(Component) && !Component->CanRenderGrassMap())
						{
	#if WITH_EDITOR
							// in editor, try to force compilation to complete
							UE::Landscape::Grass::Private::CompileGrassMapShader(State->Component);
	...
	TArrayView<TObjectPtr<ULandscapeComponent>> LandscapeComponents, FScopedSlowTask* SlowTask, bool bMarkDirty)
{
	...
	while (UpToDateCount + FailedComponents.Num() != LandscapeComponents.Num())
	{
		...
		{
			UE_LOGF(LogGrass, Error, "ERROR: BuildGrassMapsNowForComponents() took too long, grass maps are not up to date");
			break;
		}
		...
		for (ULandscapeComponent* Component : LandscapeComponents)
		{
			...
			if (State->Stage == EComponentStage::GrassMapsPopulated)
			{
				...
				{
					UE_LOGF(LogGrass, Warning, "Unexpected content change while generating grass maps synchronously (Component:%p %p %ls Ticks:%d WorldType:%d OldHash:%x NewHash:%x StateHash:%x NumElems:%d)",
					...
					UE_LOGF(LogGrass, Warning, "  GrassBuilder State: Pend:%d (Heap:%d) Strm:%d Rend:%d Fetch:%d Pop:%d NR:%d Total:%d",
			...
			if (State->Stage == EComponentStage::NotReady)
			{
				if (!Component->GetLandscapeActor()->ShouldGenerateGrass())
				{
					// nothing to do for this component
					++UpToDateCount;
				}
				else if (!FailedComponents.Contains(Component))
				{
#if WITH_EDITOR
					// if it's not ready because the shaders are not ready
					if (!Component->CanRenderGrassMap())
					{
						// try to force the shaders to compile
						UE::Landscape::Grass::Private::CompileGrassMapShader(State->Component);
						if (Component->CanRenderGrassMap())
						{
							// succeeded, let it try to complete the grass (the next UpdateTrackedComponents should pull it out of NotReady state if it can)
						}
						else
						{
							// failed to compile shaders, it's definitely failed for real
							FailedComponents.Add(Component);
						}
					}
					else
#endif // WITH_EDITOR
					{
						// failed for other unspecified reasons
						FailedComponents.Add(Component);
					}
				}
			}
		...
		if (bRenderCommandsQueuedByLastUpdate || (RenderingCount > 0 && (CurTime - LastFlush > (1.0 / 60.0))))
		{
			...
			UE::Landscape::Private::SubmitGPUCommands(/* bBlockUntilRTComplete =  */ true, /* bBlockRTUntilGPUComplete =  */ false);
			LastFlush = CurTime;
		}
	...
	UE_LOGF(LogGrass, Verbose, "BuildGrassMapsNowForComponents() updated %d/%d components in %f seconds", UpToDateCount, LandscapeComponents.Num(), FPlatformTime::Seconds() - StartTime);
	...
	{
		UE_LOGF(LogGrass, Warning, "Failed to build grass maps for %d/%d landscape components, check if you are using a render preview mode, or a non-SM5 capable render device.  (%d pending, %d streaming, %d rendering, %d fetching, %d built)",
	...
	case EComponentStage::NotReady:
		{
			...
			NotReadyCount--;
			PendingCount++;
			GRASS_DEBUG_TRANSITION(State, NotReady, Pending);
		}
		break;
	case EComponentStage::TextureStreaming:
		{
			...
			StreamingCount--;
			PendingCount++;
			GRASS_DEBUG_TRANSITION(State, TextureStreaming, Pending);
		}
		break;
	case EComponentStage::Rendering:
		{
			...
			if (State.ActiveRender != nullptr)
			{
				...
				else
				{
					...
					const bool bForceFinish = (State.TickCount > 4);
					if (!State.ActiveRender->CheckAndUpdateAsyncReadback(bNewRenderCommands, bForceFinish))
					{
			...
			RenderingCount--;
			PendingCount++;
			GRASS_DEBUG_TRANSITION(State, Rendering, Pending);
		}
		break;
	case EComponentStage::AsyncFetch:
		{
			...
			AsyncFetchCount--;
			PendingCount++;
			GRASS_DEBUG_TRANSITION(State, AsyncFetch, Pending);
		}
		break;
	case EComponentStage::GrassMapsPopulated:
		{
			...
			PopulatedCount--;
			PendingCount++;
			GRASS_DEBUG_TRANSITION(State, Populated, Pending);
		}
		break;
	default:
		check(false);	// unreachable
...
bool FLandscapeGrassMapsBuilder::TryFastpathsFromPending(FComponentState& State, bool bRecalculateHashes)
{
	...
	if (!Component->GetLandscapeProxy()->ShouldGenerateGrass())
	{
		GRASS_DEBUG_LOG(TEXT("Grass generation disabled for %s"), *Component->GetName());
	...
	else
	{
		...
		if (bRecalculateHashes)
		{
			...
			{
				State.GrassMapGenerationHash = UE::Landscape::Grass::ComputeGrassMapGenerationHash(Component, Material);
				State.GrassInstanceGenerationHash = UE::Landscape::Grass::ComputeGrassInstanceGenerationHash(State.GrassMapGenerationHash, Material->GetCachedExpressionData().NamedGrassTypes);
			}
		}
...
bool FLandscapeGrassMapsBuilder::StartGrassMapGeneration(FComponentState& State, bool bForceCompileShaders, bool bTryFastpaths)
{
	...
	if (!Component->CanRenderGrassMap())
	{
		...
		{
			UE::Landscape::Grass::Private::CompileGrassMapShader(Component);
		}
...
void FLandscapeGrassMapsBuilder::StreamingToNotReady(FComponentState& State)
{
	...
	GRASS_DEBUG_TRANSITION(State, Streaming, NotReady);
	State.TickCount = 0;
}
...
void FLandscapeGrassMapsBuilder::PendingToNotReady(FComponentState& State)
{
	...
	GRASS_DEBUG_TRANSITION(State, Pending, NotReady);
	State.TickCount = 0;
}
...
void FLandscapeGrassMapsBuilder::PendingToPopulatedFastPathAlreadyHasData(FComponentState& State)
{
	...
	GRASS_DEBUG_TRANSITION(State, Pending, Populated_Existing);
	State.TickCount = 0;
}
...
void FLandscapeGrassMapsBuilder::PendingToPopulatedFastPathNoGrass(FComponentState& State)
{
	...
	GRASS_DEBUG_TRANSITION(State, Pending, Populated_Empty);
	State.TickCount = 0;
}
...
void FLandscapeGrassMapsBuilder::PendingToStreaming(FComponentState& State)
{
	...
	GRASS_DEBUG_TRANSITION(State, Pending, Streaming);
	State.TickCount = 0;
}
...
void FLandscapeGrassMapsBuilder::KickOffRenderAndReadback(FComponentState& State)
{
	...
	State.ActiveRender.Reset(new FLandscapeGrassWeightExporter(Component->GetLandscapeActor(), { Component }, UE::Landscape::Grass::EGrassWeightExporterFlags::Default, /*InRequestedGrassNames = */{}, MakeConstArrayView(HeightMips)));
	check(State.ActiveRender != nullptr);

	State.Stage = EComponentStage::Rendering;
	RenderingCount++;

	GRASS_DEBUG_TRANSITION(State, Streaming, Rendering);
	State.TickCount = 0;
}
...
void FLandscapeGrassMapsBuilder::LaunchAsyncFetchTask(FComponentState& State)
{
	...
	GRASS_DEBUG_TRANSITION(State, Rendering, AsyncFetch);
	State.TickCount = 0;
}
...
void FLandscapeGrassMapsBuilder::PopulateGrassDataFromAsyncFetchTask(FComponentState& State)
{
	...
	GRASS_DEBUG_TRANSITION(State, AsyncFetch, Populated);
	State.TickCount = 0;
}
...
void FLandscapeGrassMapsBuilder::PopulateGrassDataFromReadback(FComponentState& State)
{
	...
	GRASS_DEBUG_TRANSITION(State, Rendering, Populated);
	State.TickCount = 0;
}
...
bool FLandscapeGrassMapsBuilder::FComponentState::IsBeyondEvictionRange(const TArray<FVector>& Cameras) const
{
	...
	const float MinSqrDistanceToComponent = UE::Landscape::Grass::Private::CalculateMinDistanceToCamerasSquared(Cameras, WorldBounds);
...
void FLandscapeGrassMapsBuilder::Build(UE::Landscape::EBuildFlags InBuildFlags)
{
	if (World)
	{
		...
		UE_LOGF(LogGrass, Verbose, "FLandscapeGrassMapsBuilder::Build() building %d component grass maps (%d / %d valid)",
