...
static FAutoConsoleCommand ConsoleCommandDumpLandscapeGrassData(
	...
	FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& Args)
	{
		...
		for (const FString& Arg : Args)
		{
			...
			if (FParse::Param(*Arg, TEXT("bygrassname")))
			{
				bByGrassName = true;
			}
		...
		if (!bDetailed && !bByProxy && !bByComponent && !bByGrassName && !bByGrassType && !bFull)
		{
		...
		{
			FSortDataEntry(const UWorld* InWorld, const ALandscape* InActor, const ALandscapeProxy* InProxy, const ULandscapeComponent* InComponent, const FName& InGrassName, const ULandscapeGrassType* InGrassType, int32 InOffset, bool bInIsHeightData)
				: World(InWorld)
				, Actor(InActor)
				, Proxy(InProxy)
				, Component(InComponent)
				, GrassName(InGrassName)
				, GrassType(InGrassType)
				, Offset(InOffset)
				, bIsHeightData(bInIsHeightData)
			{}
			const UWorld* World = nullptr;
			const ALandscape* Actor = nullptr;
			const ALandscapeProxy* Proxy = nullptr;
			const ULandscapeComponent* Component = nullptr;
			FName GrassName; // Can be NAME_None for height data 
			const ULandscapeGrassType* GrassType = nullptr; // Can be nullptr if this entry corresponds to height data or if we have some grass data in the grass map but no grass type associated with it
			int32 Offset = 0;
			int32 Size = 0;
			bool bIsHeightData = false;
		};
		TArray<FSortDataEntry> SortData;
		TSet<const ULandscapeGrassType*> UniqueGrassTypes;
		TSet<FName> UniqueGrassNames;
				...
				if (const UWorld* World = Component->GetWorld())
				{
					if (const ALandscapeProxy* Proxy = Component->GetLandscapeProxy())
					{
						if (const ALandscape* Actor = Proxy->GetLandscapeActor())
						{
							...
							{
								const FName& GrassName = ItPair.Key;
								const ULandscapeGrassType* GrassType = nullptr;
								check(!GrassName.IsNone());
								if (const TObjectPtr<ULandscapeGrassType>* FoundGrassType = Component->GetNamedGrassTypes().Find(GrassName))
								{
									GrassType = *FoundGrassType;
									int32 GrassTypeOffset = ItPair.Value;
									SortData.Add(FSortDataEntry(World, Actor, Proxy, Component, GrassName, GrassType, GrassTypeOffset, /*bInIsHeightData = */false));
									MinWeightOffset = FMath::Min(GrassTypeOffset, MinWeightOffset);

									UniqueGrassNames.Add(GrassName);
							...
							SortData.Add(FSortDataEntry(World, Actor, Proxy, Component, /*InGrassName = */NAME_None, /* InGrassType = */nullptr, HeightOffset, /*bInIsHeightData = */true));
						}
					}
				}
			}
		...
		// Sort by World, then Actor, then Proxy, then Component, then grass name : 
		SortData.Sort([](const FSortDataEntry& InLHS, const FSortDataEntry& InRHS)
		{
			if (InLHS.World == InRHS.World)
			{
				if (InLHS.Actor == InRHS.Actor)
				{
					if (InLHS.Proxy == InRHS.Proxy)
					{
						if (InLHS.Component == InRHS.Component)
						{
							return InLHS.GrassName.LexicalLess(InRHS.GrassName);
						}
						else
						{
							return InLHS.Component < InRHS.Component;
		...
		auto GetFormattedGrassName = [](const FName& InGrassName) -> FString { check(!InGrassName.IsNone()); return InGrassName.ToString(); };
		auto GetFormattedGrassTypeName = [bUseFullNames](const ULandscapeGrassType* InGrassType) -> FString { return (InGrassType != nullptr) ? (bUseFullNames ? InGrassType->GetFullName() : InGrassType->GetName()) : FString(TEXT("(null)")); };
		...
		if (bFull || bDetailed)
		{
			...
			if (bCSV)
			{
				GLog->Logf(TEXT("World,Landscape Actor,Landscape Proxy,Landscape Component,Landscape Grass Name,Landscape Grass Type,Size (KB)"));
				for (const FSortDataEntry& Entry : SortData)
				{
					if (Entry.bIsHeightData)
					{
						FString Line = FString::Printf(TEXT("%s,%s,%s,%s,(HeightData),(HeightData),%.2f"),
					...
					else if ((Entry.GrassType != nullptr) || (bIncludeEmpty || (Entry.Size > 0)))
					{
						FString Line = FString::Printf(TEXT("%s,%s,%s,%s,%s,%s,%.2f"),
							*GetFormattedObjectName(Entry.World),
							*GetFormattedActorName(Entry.Actor),
							*GetFormattedActorName(Entry.Proxy),
							*Entry.Component->GetName(),
							*GetFormattedGrassName(Entry.GrassName),
			...
			else
			{
				...
				GLog->Logf(TEXT("Num grass names used: %i"), UniqueGrassNames.Num());
		...
		if (bFull || bByProxy)
		{
			...
			auto LogCSVHeader = [bCSV]() { if (bCSV) { GLog->Logf(TEXT("Landscape Proxy,Num Grass Names,Num Grass Types,Total Height Data Size (KB), Total Weight Data Size (KB), Total Data Size (KB)")); } };
			auto LogLine = [bCSV, AddLineToSubReport](const TCHAR* InProxyName, int32 InNumGrassNames, int32 InNumGrassTypes, int32 InTotalHeightDataSizeBytes, int32 InTotalWeightDataSizeBytes)
			{
				if (bCSV)
				{
					AddLineToSubReport(FString(InProxyName), FString::Printf(TEXT("%s,%i,%i,%.2f,%.2f,%.2f"),
						InProxyName,
						InNumGrassNames,
						InNumGrassTypes,
				...
				{
					AddLineToSubReport(FString(InProxyName), FString::Printf(TEXT("%s: Num grass names = %i, Num grass types = %i, total height data size = %.2f KB, total weight data size = %.2f KB, total data size = %.2f KB"),
						InProxyName,
						InNumGrassNames,
						InNumGrassTypes,
						InTotalHeightDataSizeBytes / 1024.0f,
			...
			TSet<FName> UniqueGrassNamesForCurrentProxy;
			...
			for (int32 Index = 0; Index < NumEntries; ++Index)
			{
				...
				if (Entry.Proxy != CurrentProxy)
				{
					...
					LogLine(*GetFormattedActorName(CurrentProxy), UniqueGrassNamesForCurrentProxy.Num(), UniqueGrassTypesForCurrentProxy.Num(), HeightDataSizeForCurrentProxy, WeightDataSizeForCurrentProxy);

					// Start gathering stats for the next proxy :
					CurrentProxy = Entry.Proxy;
					HeightDataSizeForCurrentProxy = 0;
					WeightDataSizeForCurrentProxy = 0;
					UniqueGrassNamesForCurrentProxy.Empty();
				...
				if (Entry.Size > 0)
				{
					...
					else
					{
						...
						UniqueGrassNamesForCurrentProxy.Add(Entry.GrassName);
		...
		if (bFull || bByComponent)
		{
			...
			auto LogCSVHeader = [bCSV]() { if (bCSV) { GLog->Logf(TEXT("Landscape Proxy,Landscape Component,Num Grass Names,Num Grass Types,Total Height Data Size (KB), Total Weight Data Size (KB), Total Data Size (KB)")); } };
			auto LogLine = [bCSV, AddLineToSubReport](const TCHAR* InProxyName, const TCHAR* InComponentName, int32 InNumGrassNames, int32 InNumGrassTypes, int32 InTotalHeightDataSizeBytes, int32 InTotalWeightDataSizeBytes)
			{
				if (bCSV)
				{
					AddLineToSubReport(FString(InComponentName), FString::Printf(TEXT("%s,%s,%i,%i,%.2f,%.2f,%.2f"),
						InProxyName,
						InComponentName,
						InNumGrassNames,
						InNumGrassTypes,
				...
				{
					AddLineToSubReport(FString(InComponentName), FString::Printf(TEXT("%s (%s): Num grass names = %i, Num grass types = %i, total height data size = %.2f KB, total weight data size = %.2f KB, total data size = %.2f KB"),
						InProxyName,
						InComponentName,
						InNumGrassNames,
						InNumGrassTypes,
			...
			TSet<FName> UniqueGrassNamesForCurrentComponent;
			TSet<const ULandscapeGrassType*> UniqueGrassTypesForCurrentComponent;
			for (int32 Index = 0; Index < NumEntries; ++Index)
			{
				const FSortDataEntry& Entry = SortData[Index];
				if (Entry.Component != CurrentComponent)
				{
					...
					LogLine(*GetFormattedActorName(CurrentComponent->GetLandscapeProxy()), *CurrentComponent->GetName(), UniqueGrassNamesForCurrentComponent.Num(), UniqueGrassTypesForCurrentComponent.Num(), HeightDataSizeForCurrentComponent, WeightDataSizeForCurrentComponent);

					// Start gathering stats for the next component :
					CurrentComponent = Entry.Component;
					HeightDataSizeForCurrentComponent = 0;
					WeightDataSizeForCurrentComponent = 0;
					UniqueGrassNamesForCurrentComponent.Empty();
					UniqueGrassTypesForCurrentComponent.Empty();
				}
				...
				if (Entry.Size > 0)
				{
					...
					else
					{
						...
						UniqueGrassNamesForCurrentComponent.Add(Entry.GrassName);
						if (Entry.GrassType != nullptr)
						{
							UniqueGrassTypesForCurrentComponent.Add(Entry.GrassType);
						}
					}
				}
			}
		...
		if (bFull || bByGrassName)
		{
			// Sort by Grass Name, then Component: 
			SortData.Sort([](const FSortDataEntry& InLHS, const FSortDataEntry& InRHS)
				{
					if (InLHS.bIsHeightData == InRHS.bIsHeightData)
					{
						if (InLHS.GrassName == InRHS.GrassName)
						{
							return (InLHS.Component < InRHS.Component);
						}
						return InLHS.GrassName.LexicalLess(InRHS.GrassName);
					}
					// Put the height data entries first :
					return InLHS.bIsHeightData;
				});

			// Start a new sub-report :
			GLog->Logf(TEXT("By grass name report:"));
			ON_SCOPE_EXIT{ GLog->Logf(TEXT("--------")); };

			auto LogCSVHeader = [bCSV]() { if (bCSV) { GLog->Logf(TEXT("Grass Name,Num Components,Total Data Size (KB)")); } };
			auto LogLine = [bCSV, AddLineToSubReport](const TCHAR* InGrassName, int32 InNumComponents, int32 InTotalDataSizeBytes)
				{
					if (bCSV)
					{
						AddLineToSubReport(FString(InGrassName), FString::Printf(TEXT("%s,%i,%.2f"),
							InGrassName,
							InNumComponents,
					...
					{
						AddLineToSubReport(FString(InGrassName), FString::Printf(TEXT("%s: Num components = %i, total data size = %.2f KB"),
							InGrassName,
							InNumComponents,
							InTotalDataSizeBytes / 1024.0f));
					}
			...
			int32 Index = 0;
			int32 HeightDataSize = 0;
			int32 NumComponentsWithHeightData = 0;
			// Display all the height data first : 
			for (; (Index < NumEntries) && SortData[Index].bIsHeightData; ++Index)
			{
				const FSortDataEntry& Entry = SortData[Index];
				check(Entry.bIsHeightData);
				HeightDataSize += Entry.Size;
				++NumComponentsWithHeightData;
			}
			if (NumComponentsWithHeightData > 0)
			{
				// Dump info for height :
				LogLine(TEXT("(HeightData)"), NumComponentsWithHeightData, HeightDataSize);
			}

			FName CurrentGrassName = (Index < NumEntries) ? SortData[Index].GrassName : NAME_None;
			int32 DataSizeForCurrentGrassName = 0;
			int32 NumComponentsForCurrentGrassName = 0;
			for (; Index < NumEntries; ++Index)
			{
				const FSortDataEntry& Entry = SortData[Index];
				check(!Entry.bIsHeightData);
				if (Entry.GrassName != CurrentGrassName)
				{
					// The info from CurrentGrassName is now complete, we can dump it :
					LogLine(*GetFormattedGrassName(CurrentGrassName), NumComponentsForCurrentGrassName, DataSizeForCurrentGrassName);

					// Start gathering stats for the next grass type :
					CurrentGrassName = Entry.GrassName;
					DataSizeForCurrentGrassName = 0;
					NumComponentsForCurrentGrassName = 0;
				}

				// Accumulate data about this grass type :
				if (Entry.Size > 0)
				{
					DataSizeForCurrentGrassName += Entry.Size;
					++NumComponentsForCurrentGrassName;
				}
			}

			// And dump one more time, for the last grass type, which hasn't been reported yet :
			if (!CurrentGrassName.IsNone())
			{
				LogLine(*GetFormattedGrassName(CurrentGrassName), NumComponentsForCurrentGrassName, DataSizeForCurrentGrassName);
			}
			FinalizeSubReport();
		...
		{
			// Sort by HeightData first, then Grass Type, then Component: 
			SortData.Sort([](const FSortDataEntry& InLHS, const FSortDataEntry& InRHS)
			{
				if (InLHS.bIsHeightData == InRHS.bIsHeightData)
				{
			...
			const ULandscapeGrassType* CurrentGrassType = (Index < NumEntries) ? SortData[Index].GrassType : nullptr;
...
bool ULandscapeComponent::UpdateGrassTypes(bool bForceUpdate)
{
	...
	auto BuildNamedGrassTypes = [](UMaterialInterface* InMaterial, ALandscapeProxy* InProxy) 
		{
			// Build the final list of grass types for this component : start by using the material's list (we won't ever be able to support anything that is not declared in that list anyway) : 
			TMap<FName, TObjectPtr<ULandscapeGrassType>> LocalNamedGrassTypes = InMaterial->GetMaterial()->GetCachedExpressionData().NamedGrassTypes;
			const TMap<FLandscapeGrassName, TObjectPtr<ULandscapeGrassType>>& GrassTypesOverrides = InProxy->GetGrassTypesOverrides();

			// And apply eventual overrides onto it : 
			for (TPair<FName, TObjectPtr<ULandscapeGrassType>>& ItPair : LocalNamedGrassTypes)
			{
				if (const TObjectPtr<ULandscapeGrassType>* GrassTypeOverride = GrassTypesOverrides.Find(FLandscapeGrassName(ItPair.Key)))
				{
					ItPair.Value = *GrassTypeOverride;
				}
			}

			return LocalNamedGrassTypes;
		};

#if WITH_EDITOR
	if (UMaterialInterface* Material = GetLandscapeMaterial())
	{
		TMap<FName, TObjectPtr<ULandscapeGrassType>> LocalNamedGrassTypes = BuildNamedGrassTypes(Material, GetLandscapeProxy());
		uint32 NewGrassTypesCRC = HashCombine(Material->ComputeAllStateCRC(), UE::Landscape::Grass::Private::ComputeNamedGrassTypesHash(LocalNamedGrassTypes));
		if (bForceUpdate || (GrassTypesCRC != NewGrassTypesCRC))
		{
			GrassTypesCRC = NewGrassTypesCRC;

			SetNamedGrassTypes(LocalNamedGrassTypes);
			bChanged = true;
		}
	}
#else
...
bool ULandscapeComponent::CanRenderGrassMap() const
{
	...
	if (!CanEverRender() || !World || !SceneProxy)
	{
		GRASS_DEBUG_LOG(TEXT("GrassMap No SceneProxy for %s"), *GetName());
		return false;
	}

	// Check we can render the material
	const int32 MatInstCount = GetCurrentRuntimeMaterialInstanceCount();
	if (MatInstCount <= 0)
	{
		GRASS_DEBUG_LOG(TEXT("GrassMap MaterialInstanceCount %d <= 0 for %s (%d dyn:%d inst:%d mob:%d)"),
			MatInstCount,
			*GetName(),
			GetLandscapeProxy()->bUseDynamicMaterialInstance ? 1 : 0,
			MaterialInstancesDynamic.Num(),
			MaterialInstances.Num(),
			MobileMaterialInterfaces.Num());
		return false;
	}

	UMaterialInterface* MaterialInterface = GetCurrentRuntimeMaterialInterface(0);
	if (MaterialInterface == nullptr)
	{
		GRASS_DEBUG_LOG(TEXT("GrassMap MaterialInterface NULL for %s"), *GetName());
		return false;
	}

	const FMaterialResource* MaterialResource = MaterialInterface->GetMaterialResource(GetFeatureLevelShaderPlatform_Checked(World->GetFeatureLevel()));
	if (MaterialResource == nullptr)
	{
		GRASS_DEBUG_LOG(TEXT("GrassMap MaterialResource NULL for %s (%s feature level %d)"), *GetName(), *MaterialInterface->GetName(), World->GetFeatureLevel());
		return false;
	}

	// We only need the GrassWeight shaders on the fixed grid vertex factory to render grass maps : 
	FMaterialShaderTypes ShaderTypes;
	AddGrassWeightShaderTypes(ShaderTypes);
	...
	{
		GRASS_DEBUG_LOG(TEXT("GrassMap MaterialResource does not have FixedGridVF for %s"), *GetName());
		return false;
	}
	return true;
}
...
{
	return UE::Landscape::Grass::ComputeGrassMapGenerationHash(this, GetLandscapeMaterial());
}
...
TArray<uint16> ULandscapeComponent::RenderWPOHeightmap(int32 LOD)
{
	...
	if (ensure(SceneProxy))
	{
		if (!CanRenderGrassMap())
		{
			...
			if (!CanRenderGrassMap())
			{
				UE_LOGF(LogGrass, Verbose, "Failed to calculate Landscape WPO height for static lighting. Grass map generation shader could not be compiled.");
				return Results;
			}
		}
		...
		if (LOD == 0)
		{
			EGrassWeightExporterFlags LandscapeGrassWeightExporterFlags = EGrassWeightExporterFlags::NeedsHeightmap | EGrassWeightExporterFlags::RenderImmediately | EGrassWeightExporterFlags::ReadbackToCPU;
			FLandscapeGrassWeightExporter Exporter(Landscape, /*InRequestedComponents = */{ this }, LandscapeGrassWeightExporterFlags);

			while (!Exporter.IsAsyncReadbackComplete())
			{
				UE::Landscape::Private::SubmitGPUCommands(/* bBlockUntilRTComplete =  */ true, /* bBlockRTUntilGPUComplete =  */ true);
				bool bOutRenderCommandsQueued;
				Exporter.CheckAndUpdateAsyncReadback(bOutRenderCommandsQueued, /*bInForceFinish = */true);
			}
		...
		{
			EGrassWeightExporterFlags LandscapeGrassWeightExporterFlags = EGrassWeightExporterFlags::RenderImmediately | EGrassWeightExporterFlags::ReadbackToCPU;
			TArray<int32> HeightMips;
			HeightMips.Add(LOD);
			FLandscapeGrassWeightExporter Exporter(Landscape, /*InRequestedComponents = */{ this }, LandscapeGrassWeightExporterFlags, /*InRequestedGrassTypes = */{}, MakeConstArrayView(HeightMips));
...
void ALandscapeProxy::UpdateGrassTypeSummary()
{
	...
	for (ULandscapeComponent* Component : LandscapeComponents)
	{
		...
		if (Component->GrassTypeSummary.bInvalid)
		{
			...
			TMap<FName, TObjectPtr<ULandscapeGrassType>>  ValidNamedGrassTypes = UE::Landscape::Grass::FilterValidNamedGrassTypes(Component->GetNamedGrassTypes());
			for (const TPair<FName, TObjectPtr<ULandscapeGrassType>>& ItPair : ValidNamedGrassTypes)
			{
				check(!ItPair.Key.IsNone() && (ItPair.Value != nullptr));

				for (FGrassVariety& GrassVariety : ItPair.Value->GrassVarieties)
				{
...
bool ALandscapeProxy::ShouldGenerateGrass() const
{
	if (!GGrassEnable)
	{
		return false;
	...
	// HACK [jonathan.bard] : for hotfix : don't let grass be rendered until all components of the proxy have been registered. That prevents grass maps to be rendered on those components, which 
	//  allows LandscapeInfo to be valid by the time the grass results are fetched
	...
	{
		return LandscapeActor->GetGrassEnabled();
	}

	// No landscape actor available, don't generate grass
	return false;
}
	...
	if (GIsEditor)
	{
		for (TObjectIterator<ALandscapeProxy> It(/*AdditionalExclusionFlags = */RF_ClassDefaultObject, /*bIncludeDerivedClasses = */true, /*InInternalExclusionFlags = */EInternalObjectFlags::Garbage); It; ++It)
		{
			...
			if (Proxy->GetWorld() && !Proxy->GetWorld()->IsPlayInEditor())
			{
				...
				{
					if (const FName* FoundGrassNameForType = Component->GetNamedGrassTypes().FindKey(this))
					{
...
TArrayView<uint8> FLandscapeComponentGrassData::GetWeightData(const FName& GrassName)
{
	if (!HeightWeightData.IsEmpty())
	{
		if (int32* OffsetPtr = WeightOffsets.Find(GrassName))
		{
...
bool FLandscapeComponentGrassData::Contains(const FName& GrassName) const
{
	return WeightOffsets.Contains(GrassName);
}
...
#if WITH_EDITOR
// Initialize function meant for deserializing from old data : the new format doesn't take ULandscapeGrassType* but FName :
void FLandscapeComponentGrassData::InitializeFrom(const TArray<uint16>& HeightData, const TMap<ULandscapeGrassType*, TArray<uint8>>& WeightData)
{
	...
	{
		// At the time this data was serialized, weights were stored per ULandscapeGrassType. Nowadays, we use FNames instead, but for old data, we 
		//  use the ULandscapeGrassType's name as its grass name : 
		ULandscapeGrassType* GrassType = Pair.Key;
		FName GrassName = GrassType->GetGrassNameFallback();
		WeightOffsets.Add(GrassName, CopyOffset);
...
void FLandscapeComponentGrassData::FixupFallbackGrassNames(const TMap<FName, TObjectPtr<ULandscapeGrassType>>& InNamedGrassTypes)
{
	TMap<FName, int32> FixedUpWeightOffsets;
	FixedUpWeightOffsets.Reserve(WeightOffsets.Num());
	for (const TPair<FName, int32>& ItPair : WeightOffsets)
	{
		const FName& GrassNameToFixup = ItPair.Key;
		const int32 Offset = ItPair.Value;
		// If the name is already included in the official list of available grass names, keep it as is (failsafe, in case the function is run on already fixed-up data) :
		if (InNamedGrassTypes.Contains(GrassNameToFixup))
		{
			check(!FixedUpWeightOffsets.Contains(GrassNameToFixup));
			FixedUpWeightOffsets.Add(GrassNameToFixup, Offset);
		}
		else
		{
			if (const TPair<FName, TObjectPtr<ULandscapeGrassType>>* FoundPair = Algo::FindByPredicate(InNamedGrassTypes, [GrassNameToFixup](const TPair<FName, TObjectPtr<ULandscapeGrassType>>& InPair)
				{
					const ULandscapeGrassType* GrassType = InPair.Value;
					return (GrassType != nullptr) && (GrassType->GetGrassNameFallback() == GrassNameToFixup);
				}))
			{ 
				// We've found the new, proper, grass name corresponding to the grass type's fallback name, let's use this
				FixedUpWeightOffsets.Add(FoundPair->Key, Offset);
			}
			else
			{
				UE_LOGF(LogLandscape, Log, "Couldn't fixup grass name %ls : this grass type will be unavailable for spawning grass until generating new grass data", *GrassNameToFixup.ToString());
			}
		}
	}

	// Update the weight offsets with the new names : 
	Swap(FixedUpWeightOffsets, WeightOffsets);
}
#endif // WITH_EDITOR

// Note that this is a relatively expensive operation (in the runtime sense), as it's making a copy of the buffers
void FLandscapeComponentGrassData::InitializeFrom(IBuffer2DView<uint16>* HeightData, TMap<FName, IBuffer2DView<uint8>*>& WeightData, bool bStripEmptyWeights)
{
	...
	for (const TPair<FName, IBuffer2DView<uint8>*>& Pair : WeightData)
	{
		check(Pair.Value->Num() == NumElements);
		check((CopyOffset + CopySize) <= HeightWeightData.Num());
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	else if ((Ar.CustomVer(FFortniteMainBranchObjectVersion::GUID) < FFortniteMainBranchObjectVersion::OverridableLandscapeGrassTypes) && Ar.IsLoading())
	{
		Ar << Data.NumElements;

		TMap<ULandscapeGrassType*, int32> DeprecatedWeightOffsets;
		Ar << DeprecatedWeightOffsets;

		// Convert from ULandscapeGrassType* to fallback FNames for the new format (this will eventually get fixed up afterwards to proper grass names, extracted from the material, in ULandscapeComponent::PostLoad)
		for (const auto& ItPair : DeprecatedWeightOffsets)
		{
			if (ItPair.Key != nullptr)
			{
				Data.WeightOffsets.FindOrAdd(ItPair.Key->GetGrassNameFallback(), ItPair.Value);
			}
		}
...
void FLandscapeComponentGrassData::ConditionalDiscardDataOnLoad(const TMap<FName, TObjectPtr<ULandscapeGrassType>>& InNamedGrassTypes)
{
	//check(HasValidData());
	if (!GIsEditor && GGrassDiscardDataOnLoad)
	{
		...
		for (auto It = WeightOffsets.CreateIterator(); It; ++It)
		{
			const TObjectPtr<ULandscapeGrassType>* FoundGrassType = InNamedGrassTypes.Find(It->Key);
			if ((FoundGrassType == nullptr) || (*FoundGrassType == nullptr) || (*FoundGrassType)->bEnableDensityScaling)
			{
				It.RemoveCurrent();
				bRemoved = true;
			}
		...
		{
			TMap<FName, int32> PreviousOffsets(ObjectPtrDecay(MoveTemp(WeightOffsets)));
			...
			for (const TPair<FName, int32>& Pair : PreviousOffsets)
			{
...
{
	FLandscapeComponentGrassAccess(const ULandscapeComponent* InComponent, const FName& InGrassName)
	: GrassData(InComponent->GrassData)
	, HeightData(InComponent->GrassData->GetHeightData())
	, WeightData(InComponent->GrassData->GetWeightData(InGrassName))
...
struct FAsyncGrassBuilder : public FGrassBuilderBase
{
	...
	FAsyncGrassBuilder(ALandscapeProxy* Landscape, ULandscapeComponent* Component, const FName& GrassName, const ULandscapeGrassType* GrassType, const FGrassVariety& GrassVariety, 
		ERHIFeatureLevel::Type FeatureLevel, UGrassInstancedStaticMeshComponent* GrassInstancedStaticMeshComponent, int32 SqrtSubsections, int32 SubX, int32 SubY, uint32 InHaltonBaseIndex, TArray<FBox>& InExcludedBoxes)
	...
	if (ShouldGenerateGrass())
	{
		...
		if (World)
		{
			...
			for (const SortedLandscapeElement& SortedLandscapeComponent : SortedLandscapeComponents)
			{
				...
				const TMap<FName, TObjectPtr<ULandscapeGrassType>> ValidNamedGrassTypes = UE::Landscape::Grass::FilterValidNamedGrassTypes(Component->GetNamedGrassTypes());
				for (const TPair<FName, TObjectPtr<ULandscapeGrassType>>& ItPair : ValidNamedGrassTypes)
				{
					const FName GrassName = ItPair.Key;
					TObjectPtr<ULandscapeGrassType> GrassType = ItPair.Value;
					check(!GrassName.IsNone() && (GrassType != nullptr));

					if (World->IsGameWorld() && !Component->GrassData->Contains(GrassName))		// TODO [chris.tchou] why do we only do this in game worlds?
					{
						continue;
					}
						...
						if (GrassVariety.GrassMesh && GrassVariety.GetDensity() > 0.0f && EndCullDistance > 0)
						{
							...
							for (int32 SubX = 0; SubX < SqrtSubsections; SubX++)
							{
								for (int32 SubY = 0; SubY < SqrtSubsections; SubY++)
								{
									...
									NewComp.Key.GrassName = GrassName;
									...
									int32 FolSeed = FCrc::StrCrc32(StringCast<ANSICHAR>(*FString::Printf(TEXT("%s_%s_%d_%d_%d_%d_%d"),
										*GrassName.ToString().ToLower(), *GrassType->GetName().ToLower(), Component->GetSectionBase().X, Component->GetSectionBase().Y, SubX, SubY, GrassVarietyIndex)).Get());
									if (FolSeed == 0)
									{
										FolSeed++;
									}
...

									{
										...
										Builder = new FAsyncGrassBuilder(this, Component, GrassName, GrassType, GrassVariety, FeatureLevel, GrassInstancedStaticMeshComponent, SqrtSubsections, SubX, SubY, HaltonIndexForSub, NewComp.ExcludedBoxes);
									}
...
void ALandscapeProxy::ProcessAsyncGrassInstanceTasks(bool bWaitAsyncTasks, bool bForceSync, const TSet<UHierarchicalInstancedStaticMeshComponent*>& StillUsed)
{
	...
	for (int32 Index = 0; Index < AsyncFoliageTasks.Num(); Index++)
	{
		...
		if (Task->IsDone())
		{
			...
			//UE_LOGF(LogCore, Display, "%d instances in %4.0fms     %6.0f instances / sec", NumBuiltRenderInstances, 1000.0f * float(Inner.Builder->BuildTime), float(NumBuiltRenderInstances) / float(Inner.Builder->BuildTime));

			FCachedLandscapeFoliage::FGrassCompKey& InnerKey = Inner.Key;

			UE_LOGF(LogGrass, Verbose, "ASYNC GRASS INSTANCES COMPLETE %ls %ls %d %d (%d)",
...
static void DumpExclusionBoxes(const TArray<FString>& Args)
{
	for (const TPair<FWeakObjectPtr, FBox>& Pair : GGrassExclusionBoxes)
	{
		...
		UE_LOGF(LogCore, Warning, "%f %f %f   %f %f %f   %ls",
			Pair.Value.Min.X,
			Pair.Value.Min.Y,
			Pair.Value.Min.Z,
			Pair.Value.Max.X,
