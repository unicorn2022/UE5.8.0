...
static FAutoConsoleVariable CVarVisualLogShowFoliageSnapping(
		TEXT("landscape.VisualLog.ShowFoliageSnapping"),
		false,
		TEXT("When visual log is active, allows to leave a visual log of the areas where foliage snapping occurs."));
...
void ULandscapeHeightfieldCollisionComponent::OnUnregister()
{
	...
	{
		FWriteScopeLock Lock(CachedHeightFieldSamplesLock);
	...
	if (!BodyInstance.IsValidBodyInstance())
	{
		...
		if (IsValidRef(HeightfieldRef))
		{
			auto CreateShapesAndActors = [this]()
			{
				...
				Builder.SetOwnerID(GetOwner()->GetUniqueID());
				...
				// If we don't create separate collision for simple, this heightfield is used for both simple and complex.
				Builder.SetFlags(Chaos::EFilterFlags::SimpleCollision, !bCreateSimpleCollision);
				Builder.SetFlags(Chaos::EFilterFlags::ComplexCollision, true);
				Builder.SetFlags(Chaos::EFilterFlags::CCD, true);
				Builder.SetFlags(Chaos::EFilterFlags::StaticShape, true);

				const Chaos::Filter::FShapeFilterData ShapeFilterData = Builder.BuildShapeFilterData();
				const Chaos::Filter::FInstanceData FilterInstanceData = Builder.BuildInstanceData();

				NewShape->SetFilterInstanceData(FilterInstanceData);
				NewShape->SetShapeFilterData(ShapeFilterData);
				...
				if (bCreateSimpleCollision)
				{
					...
					Builder.SetFlags(Chaos::EFilterFlags::ComplexCollision, false);
					Builder.SetFlags(Chaos::EFilterFlags::SimpleCollision, true);
					const Chaos::Filter::FShapeFilterData ShapeFilterDataSimple = Builder.BuildShapeFilterData();

					NewSimpleShape->SetFilterInstanceData(FilterInstanceData);
					NewSimpleShape->SetShapeFilterData(Builder.BuildShapeFilterData());
				...
				if (!GetWorld()->IsGameWorld() && !GetOutermost()->bIsCookedForEditor)
				{
					...
					FPhysicsFilterBuilder EdBuilder;
					EdBuilder.SetCollisionChannelIndex(ECollisionChannel::ECC_Visibility);
					EdBuilder.SetResponses(CollisionResponse);
					EdBuilder.SetFlags(Chaos::EFilterFlags::SimpleCollision | Chaos::EFilterFlags::ComplexCollision, true);
					EdBuilder.SetFlags(Chaos::EFilterFlags::CCD, true);
					EdBuilder.SetFlags(Chaos::EFilterFlags::StaticShape, true);
					const Chaos::Filter::FShapeFilterData ShapeFilterDataEd = EdBuilder.BuildShapeFilterData();

					NewEditorShape->SetShapeFilterData(ShapeFilterDataEd);
					NewEditorShape->SetFilterInstanceData(FilterInstanceData);
...
FPrimitiveSceneProxy* ULandscapeHeightfieldCollisionComponent::CreateSceneProxy()
{
	...
	if (HeightfieldRef.IsValid() && IsValidRef(HeightfieldRef))
	{
		...
		default:
			UE_LOGF(LogLandscape, Warning, "Invalid Value for CVar landscape.CollisionMesh.Show");
		}
			...
			if (CookedCollisionData.IsEmpty())
			{
				...
				else
				{
					if (bCookedCollisionDataWasDeleted)
					{
						...
						UE_LOGF(LogLandscape, Warning, "Tried to create heightfield collision for component '%ls', but the collision data was deleted!", *GetName());
					}
					return;
				}
...
ULandscapeHeightfieldCollisionComponent::FWriteRuntimeDataParams ULandscapeHeightfieldCollisionComponent::MakeWriteRuntimeDataParams(bool bUseDefaultMaterialOnly) const
{
	...
	checkf(CollisionHeightData.GetElementCount() == Info.NumSamples + Info.NumSimpleSamples, 
		TEXT("Invalid collision height data element count : %lld found while there should be (%d samples + %d simple samples)"), CollisionHeightData.GetElementCount(), Info.NumSamples, Info.NumSimpleSamples);
	...
	if (DominantLayerData.GetElementCount())
	{
		...
		checkf(DominantLayerData.GetElementCount() == Info.NumSamples + Info.NumSimpleSamples,
			TEXT("Invalid dominant layer data element count : %lld found while there should be (%d samples + %d simple samples)"), DominantLayerData.GetElementCount(), Info.NumSamples, Info.NumSimpleSamples);
	...
	if (PhysicalMaterialRenderData.GetElementCount())
	{
		...
		checkf(PhysicalMaterialRenderData.GetElementCount() == Info.NumSamples + Info.NumSimpleSamples,
			TEXT("Invalid physical material render data element count : %lld found while there should be (%d samples + %d simple samples)"), PhysicalMaterialRenderData.GetElementCount(), Info.NumSamples, Info.NumSimpleSamples);
	...
	if (IsValidRef(HeightfieldRef))
	{
		...
		FPhysicsCommand::ExecuteWrite(PhysActorHandle, [&](const FPhysicsActorHandle& Actor)
		{
			...
			checkf(CollisionHeightData.GetElementCount() == FMath::Square(CollisionSizeVerts) + FMath::Square(SimpleCollisionSizeVerts),
				TEXT("Invalid collision height data element count : %lld found while there should be (%d samples + %d simple samples)"), CollisionHeightData.GetElementCount(), FMath::Square(CollisionSizeVerts), FMath::Square(SimpleCollisionSizeVerts));
...
#if WITH_EDITORONLY_DATA
void ULandscapeHeightfieldCollisionComponent::MarkAreaForSnapFoliageInstances(const FBox& InBox)
{
	UE_CVLOG_BOX(CVarVisualLogShowFoliageSnapping->GetBool(), GetOwner(), LogLandscape, Log, InBox, FColor::Blue, TEXT("MarkAreaForSnapFoliageInstances"));

	for (FBox& ExistingBox : DeferredSnapFoliageInstanceBoxes)
	{
		// If the new box is fully included in the other box, then it's not needed at all
		if (InBox.IsInsideOrOn(ExistingBox))
		{
			return;
		}
		// And if the existing box is fully included within the new one, then we can replace the existing one with the new one
		if (ExistingBox.IsInsideOrOn(InBox))
		{
			ExistingBox = InBox;
			return;
		}
	}
	DeferredSnapFoliageInstanceBoxes.Add(InBox);
}

ENUM_CLASS_FLAGS(ULandscapeHeightfieldCollisionComponent::ESnapFoliageInstancesFlags);

// Deprecated
void ULandscapeHeightfieldCollisionComponent::SnapFoliageInstances()
{
	SnapFoliageInstances(ESnapFoliageInstancesFlags::ForceAll);
}

void ULandscapeHeightfieldCollisionComponent::SnapFoliageInstances(ESnapFoliageInstancesFlags InFlags)
{
	check(EnumHasAnyOneFlag(InFlags, ESnapFoliageInstancesFlags::MarkedAreasOnly | ESnapFoliageInstancesFlags::ForceAll));
	if (EnumHasAnyFlags(InFlags, ESnapFoliageInstancesFlags::MarkedAreasOnly))
	{
		for (const FBox& Box : DeferredSnapFoliageInstanceBoxes)
		{
			SnapFoliageInstances(Box);
		}
		DeferredSnapFoliageInstanceBoxes.Empty();
	}
	else if (EnumHasAnyFlags(InFlags, ESnapFoliageInstancesFlags::ForceAll))
	{
...
void ULandscapeHeightfieldCollisionComponent::SnapFoliageInstances(const FBox& InInstanceBox)
{
	...
	bool bAtLeastOneInstanceWithinBox = false;
	...
	for (TActorIterator<AInstancedFoliageActor> It(ComponentWorld); It; ++It)
	{
		...
		IFA->ForEachFoliageInfo([this, IFA, BaseId, &InInstanceBox, &bAtLeastOneInstanceWithinBox](UFoliageType* Settings, FFoliageInfo& MeshInfo)
		{
			...
			if (InstanceSet)
			{
				...
				for (int32 InstanceIndex : *InstanceSet)
				{
					...
					{
						bAtLeastOneInstanceWithinBox = true;
						...
						UE_CVLOG_SEGMENT(CVarVisualLogShowFoliageSnapping->GetBool(), GetOwner(), LogLandscape, Log, Start, End, bFoundHit ? FColor::Red : FColor::Green, TEXT("SnapFoliageInstancesRay"));
						if (bFoundHit)
						{
							UE_CVLOG_LOCATION(CVarVisualLogShowFoliageSnapping->GetBool(), GetOwner(), LogLandscape, Log, HitLocation, 5.0f, FColor::Red, TEXT("SnapFoliageInstancesRayHitLocation"));
						}
						else
						{
	...
	UE_CVLOG_BOX(CVarVisualLogShowFoliageSnapping->GetBool(), GetOwner(), LogLandscape, Log, InInstanceBox, bAtLeastOneInstanceWithinBox ? FColor::Red : FColor::Green, TEXT("SnapFoliageInstances"));
}
...
void ULandscapeHeightfieldCollisionComponent::Serialize(FArchive& Ar)
{
	...
	else
	{
		...
		{
			UE_LOGF(LogPhysics, Fatal, "This platform requires cooked packages, and physics data was not cooked into %ls.", *GetFullName());
		}
...
{
	FReadScopeLock Lock(CachedHeightFieldSamplesLock);
...
void ULandscapeHeightfieldCollisionComponent::PostLoad()
{
	...
	if (!HasAnyFlags(RF_ClassDefaultObject) && IsValidChecked(this))
	{
		...
		if (ensure(LandscapeProxy) && GIsEditor)
		{
			...
			{
				UE_LOGF(LogLandscape, Warning, "ULandscapeHeightfieldCollisionComponent RelativeLocation disagrees with its section base, attempted automated fix: '%ls', %f,%f vs %f,%f.",
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(ULandscapeHeightfieldCollisionComponent::ImportCustomProperties);
	...
	if (FParse::Command(&SourceText, TEXT("CollisionHeightData")))
	{
		...
		// Helper to convert digits to uint (assume FChar::IsDigit(InTChar) == true)
		auto TCharToDigit = [](TCHAR InTChar) { return static_cast<uint32>(InTChar - TEXT('0')); };
		...
		while (FChar::IsDigit(*SourceText))
		{
			...
			{
				uint32 Value = 0;
				// Don't use FCString::Atoi as it can be pathologically slow on some platforms because of the StringCast conversion to convert from TCHAR to char string
				while (FChar::IsDigit(*SourceText))
				{
					Value = Value * 10 + TCharToDigit(*SourceText++);
				}
				Heights[Index++] = static_cast<uint16>(Value);
			}
			else
			{
				// If there are more tokens despite the Index having reached NumHeights, then the input is malformed. We still want to advance the parsing in such a case to avoid looping infintely
				while (FChar::IsDigit(*SourceText))
				{
					++SourceText;
				}
		...
		if (NumTokensParsed != NumHeights)
		{
			Warn->Log(*FText::Format(NSLOCTEXT("Core", "Importing_HeightfieldCollision_SyntaxError", "Syntax Error : num tokens: {0} vs. num height: {1}"), { NumTokensParsed, NumHeights }).ToString());
		}
	}
