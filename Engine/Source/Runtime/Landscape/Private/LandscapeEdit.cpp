...
extern FAutoConsoleVariable CVarLandscapeSilenceInvalidPropertyOverridesMapCheckWarnings;
...
bool ULandscapeComponent::ValidateCombinationMaterial(UMaterialInstanceConstant* InCombinationMaterial) const
{
	...
	{
		UE_LOGF(LogLandscape, Display, "Material instance %ls in landscape component %ls doesn't match the expected shader combination: different number of allocations (expected: %i, found: %i)",
	...
	for (const FWeightmapLayerAllocationInfo& Allocation : ComponentWeightmapAllocations)
	{
		...
		{
			UE_LOGF(LogLandscape, Display, "Material instance %ls in landscape component %ls doesn't match the expected shader combination: invalid layer info",
				*InCombinationMaterial->GetName(), *GetPathName());
		...
		{
			UE_LOGF(LogLandscape, Display, "Material instance %ls in landscape component %ls doesn't match the expected shader combination: missing layer %ls",
...
UMaterialInstanceConstant* ULandscapeComponent::GetCombinationMaterial(FMaterialUpdateContext* InMaterialUpdateContext, const TArray<FWeightmapLayerAllocationInfo>& Allocations, int8 InLODIndex, bool bMobile /*= false*/) const
{
	...
	if (ensure(MaterialToUse != nullptr))
	{
		...
		if (CombinationMaterialInstance == nullptr || CombinationMaterialInstance->Parent != MaterialToUse || GetOuter() != CombinationMaterialInstance->GetOuter())
		{
			...
			UE_LOGF(LogLandscape, Verbose, "Looking for key %ls, making new combination %ls", *LayerKey, *CombinationMaterialInstance->GetName());
...
void ULandscapeComponent::UpdateMaterialInstances_Internal(FMaterialUpdateContext& Context)
{
	...
	for (auto It = MaterialPerLOD.CreateConstIterator(); It; ++It)
	{
		...
		if (CombinationMaterialInstance != nullptr)
		{
			...
			if (BaseHeightmap)
			{
				MaterialInstance->SetTextureParameterValueEditorOnly(FName("Heightmap"), BaseHeightmap);
			}
...
TArray<UTexture*> ULandscapeComponent::RepairInvalidTextures()
{
	...
	for (UTexture* Texture : AllTextures)
	{
		...
		{
			UE_LOGF(LogLandscape, Error, "Invalid data found in texture %ls from landscape component %ls : clearing data.", *Texture->GetName(), *GetPathName());
...
void ULandscapeComponent::FixupWeightmaps(const FGuid& InEditLayerGuid)
{
	if (GIsEditor && !HasAnyFlags(RF_ClassDefaultObject))
	{
		...
		if (Info)
		{
			...
			{
				TArray<FString> LayerStrings;
				Algo::Transform(LayersToDelete, LayerStrings, [](ULandscapeLayerInfoObject* LayerInfo) { return LayerInfo ? LayerInfo->GetLayerName().ToString() : FString("null"); });
...
void ULandscapeComponent::UpdatePhysicalMaterialTasks()
{
	...
	if (PhysicalMaterialTask.GetHash() != Hash)
	{
		...
		bool bWeightMapsReady = false;

		// WP levels may have a deleted ALandscape actor resulting in a non-editable landscape (Invalid LandscapeInfo). UpdateLayersContent will not run leaving bForceMiplevelsToBeResident unset for the remaining landscape components
		const ULandscapeInfo* LandscapeInfo = GetLandscapeInfo();
		if (LandscapeInfo && LandscapeInfo->LandscapeActor.IsValid())
		{
			bWeightMapsReady = Algo::AllOf(WeightmapTextures, [](UTexture2D* Weightmap)
				{
...
{
	if (GIsEditor && LandscapeActor.IsValid())
	{
...
{
	return UE::Landscape::RetrieveTargetLayerNamesFromMaterial(MaterialInterface);
}

// Deprecated
const TArray<FName> ALandscapeProxy::GetLayersFromMaterial() const
{
	return RetrieveTargetLayerNamesFromMaterials();
}

TArray<FName> ALandscapeProxy::RetrieveTargetLayerNamesFromMaterials(bool bIncludeVisibilityLayer) const
{
	TSet<UMaterialInterface*> LandscapeMaterials;
	RetrieveAllLandscapeMaterials(LandscapeMaterials);
	...
	for (UMaterialInterface* Material : LandscapeMaterials)
	{
		LayerNames.Append(UE::Landscape::RetrieveTargetLayerNamesFromMaterial(Material, bIncludeVisibilityLayer));
	}
...
#define MAX_HEIGHTMAP_TEXTURE_SIZE 512
	...
	for (int32 ComponentY = 0; ComponentY < NumComponentsY; ComponentY++)
	{
		for (int32 ComponentX = 0; ComponentX < NumComponentsX; ComponentX++)
		{
			...
			UE_LOGF(LogLandscape, VeryVerbose, "%ls needs %d alphamaps", *LandscapeComponent->GetName(), EditingAlphaLayerData.Num());
	...
	for (int32 ComponentY = 0; ComponentY < NumComponentsY; ComponentY++)
	{
		...
		for (int32 ComponentX = 0; ComponentX < NumComponentsX; ComponentX++)
		{
			...
			UE_LOGF(LogLandscape, VeryVerbose, "%ls needs %d weightmap channels", *LandscapeComponent->GetName(), WeightValues.Num());
			...
			while (LayerIndex < WeightValues.Num())
			{
				...
				if (BestAllocationIndex != -1)
				{
					...
					UE_LOGF(LogLandscape, VeryVerbose, "  ==> Storing %d channels starting at %ls[%d]", RemainingLayers, *Allocation.Texture->GetName(), Allocation.ChannelsInUse);
				...
				else
				{
					...
					UE_LOGF(LogLandscape, VeryVerbose, "  ==> Storing %d channels in new texture %ls", ThisAllocationLayers, *WeightmapTexture->GetName());
...
TSharedRef<UE::Landscape::Nanite::FAsyncBuildData> ALandscapeProxy::MakeAsyncNaniteBuildData(int32 InLODToExport, const TArray<ULandscapeComponent*>& InComponentsToExport) const
{
	...
	if (UWorld* World = GetWorld())
	{
		...
		{
			UE_LOGF(LogLandscape, Verbose, "%ls : no Nanite mesh to export", *GetActorNameOrLabel());
		}

		if (AsyncBuildData->InputMaterials.Num() > NANITE_MAX_CLUSTER_MATERIALS)
		{
			UE_LOGF(LogLandscape, Warning, "%ls : Nanite landscape mesh would have more than %i materials, which is currently not supported. Please reduce the number of components in this landscape actor to enable Nanite.", *GetActorNameOrLabel(), NANITE_MAX_CLUSTER_MATERIALS)
		}
...
{
	FProperty* NanitePositionPrecisionProperty = StaticClass()->FindPropertyByName(GET_MEMBER_NAME_CHECKED(ALandscapeProxy, NanitePositionPrecision));
	check(NanitePositionPrecisionProperty);

	// PostEditChangeProperty propagates to proxies and invalidates nanite/render state
	if (NanitePositionPrecision != InPrecision)
	{
		Modify(bInShouldDirtyPackage);
		NanitePositionPrecision = InPrecision;

		FPropertyChangedEvent PropertyChangedEvent(NanitePositionPrecisionProperty);
...
bool ALandscapeProxy::ExportToRawMeshDataCopy(const FRawMeshExportParams& InExportParams, FMeshDescription& OutRawMesh, const UE::Landscape::Nanite::FAsyncBuildData& AsyncData) const
{
	...
	ON_SCOPE_EXIT
	{
		...
		UE_LOGF(LogLandscape, Verbose, "ExportToRawMeshDataCopy took %0.4f seconds for %i vertices, %i polygons", StartupDuration, Mesh.Vertices().GetArraySize(), Mesh.Polygons().GetArraySize());
	};
...
void ALandscape::UnregisterAllComponents(const bool bForReregister)
{
	// Unregister from edit layer data changed events
	for (FLandscapeLayer& Layer : LandscapeEditLayers)
	{
		Layer.EditLayer->OnLayerDataChanged().RemoveAll(this);
	}
...
void ALandscape::PostRegisterAllComponents()
{
	...
	// Register to data change events on the edit layers so that we can update the landscape accordingly : 
	for (FLandscapeLayer& Layer : LandscapeEditLayers)
	{
...
ALandscapeProxy* ULandscapeInfo::MoveComponentsToProxy(const TArray<ULandscapeComponent*>& InComponents, ALandscapeProxy* LandscapeProxy, bool bSetPositionAndOffset, ULevel* TargetLevel)
{
	...
	ALandscape* Landscape = LandscapeActor.Get();
	check(Landscape != nullptr && LandscapeProxy != nullptr);
	...
	if(bSetPositionAndOffset)
	{
		...
		// update the landscape info's proxy transform after changing the location and section base
		FixupProxiesTransform(/*bDirty =*/true);
	}
...
{
	ensureAlwaysMsgf(!GetLevel() || (GetLevel() && !GetLevel()->HasAnyFlags(EObjectFlags::RF_NeedPostLoad)), TEXT("Executing ALandscapeProxy::PostEditChangeProperty before the landscape actor has finished loading with the world can result in registration events occuring in PostLoad, which could leave the landscape proxy in an temporary invalid state"));
	...
	if (MemberPropertyName == FName("RelativeScale3D"))
	{
		...
		if (RootComponent)
		{
			...
			else
			{
				...
				if (!FMath::IsNearlyEqual(FMath::Abs(OriginalScale.X), FMath::Abs(OriginalScale.Y)))
				{
					...
					ModifiedScale.Y = FMath::Abs(OriginalScale.X) * FMath::Sign(OriginalScale.Y);
					UE_LOGF(LogLandscape, Warning, "Non-uniform XY scale for landscape (%f, %f) : scale will be forced to (%f, %f)", OriginalScale.X, OriginalScale.Y, ModifiedScale.X, ModifiedScale.Y);
				}
			}
	...
	else if (MemberPropertyName == GET_MEMBER_NAME_CHECKED(ALandscapeProxy, GrassTypesOverrides))
	{
		for (ULandscapeComponent* Component : LandscapeComponents)
		{
			if (Component != nullptr)
...
void ALandscapeStreamingProxy::CheckOverriddenSharedProperties()
{
	ALandscape* ParentLandscape = GetLandscapeActor();
	if (ParentLandscape == nullptr)
	{
		// Cannot validate anything without the parent 
		return;
	}

	TArray<FName> PropertiesToUnOverride;
	auto AddSharedPropertyOverrideMapCheckMessage = [this, &PropertiesToUnOverride](const FName& InOverriddenProperty)
		{
			FFormatNamedArguments Arguments;
			Arguments.Add(TEXT("PropertyName"), FText::FromString(InOverriddenProperty.ToString()));
			TWeakObjectPtr<ALandscapeStreamingProxy> LandscapeStreamingProxyWeakPtr = this;

			FMessageLog MessageLog("MapCheck");
			TSharedRef<FTokenizedMessage> TokenizedMessage = CVarLandscapeSilenceInvalidPropertyOverridesMapCheckWarnings->GetBool() ? MessageLog.Info() : MessageLog.Warning();
			TokenizedMessage->AddToken(FUObjectToken::Create(this, FText::FromString(GetActorNameOrLabel())))
				->AddToken(FTextToken::Create(FText::Format(LOCTEXT("MapCheck_Message_InvalidLandscapePropertyOverride", "The Landscape property {PropertyName} has been overridden but points to an invalid asset. It's most likely a mistake. The engine default material will be used. "), Arguments)))
				->AddToken(FMapErrorToken::Create(FMapErrors::LandscapeProxyInvalidPropertyOverride_Warning))
				->AddToken(FActionToken::Create(LOCTEXT("MapCheck_UnOverrideProperty", "Un-override property"), FText::Format(LOCTEXT("MapCheck_UnOverrideProperty_Desc", "Un-override {PropertyName}."), Arguments),
					FOnActionTokenExecuted::CreateLambda([LandscapeStreamingProxyWeakPtr, PropertyName = InOverriddenProperty]()
						{
							if (LandscapeStreamingProxyWeakPtr.IsValid())
							{
								LandscapeStreamingProxyWeakPtr->SetSharedPropertyOverride(PropertyName, false);
							}
						}),
					/*bInSingleUse = */true));
		};

	// If the user has set a material override but left the material unset, this is most likely a mistake : 
	if ((ParentLandscape->LandscapeMaterial != nullptr) && (LandscapeMaterial == nullptr) && IsSharedPropertyOverridden(GET_MEMBER_NAME_CHECKED(ALandscapeProxy, LandscapeMaterial)))
	{
		AddSharedPropertyOverrideMapCheckMessage(GET_MEMBER_NAME_CHECKED(ALandscapeProxy, LandscapeMaterial));
	}

	// If the user has set a material override but left the material unset, this is most likely a mistake : 
	if ((ParentLandscape->LandscapeHoleMaterial != nullptr) && (LandscapeHoleMaterial == nullptr) && IsSharedPropertyOverridden(GET_MEMBER_NAME_CHECKED(ALandscapeProxy, LandscapeHoleMaterial)))
	{
		AddSharedPropertyOverrideMapCheckMessage(GET_MEMBER_NAME_CHECKED(ALandscapeProxy, LandscapeHoleMaterial));
	}
}

void ALandscapeStreamingProxy::CheckForErrors()
{
...
bool ALandscapeStreamingProxy::CanEditChange(const FProperty* InProperty) const
{
	...
	else if (InProperty && IsPropertyOverridable(InProperty))
	{
		return IsSharedPropertyOverridden(InProperty->GetFName());
	}
...
bool ALandscape::CanDeleteSelectedActor(FText& OutReason) const
{
	if (!IsUserManaged())
	{
		...
		ULandscapeInfo* Info = GetLandscapeInfo();
		return Info ? Info->CanDeleteLandscape(OutReason) : false;
	}
				...
				if (LandscapeActorDesc->GridGuid == LandscapeGuid)
				{
					...
					if (LandscapeProxy != LandscapeActor)
					{
						...
						else if (bIsLoaded && !bIsRegistered)
						{
							...
							UE_LOGF(LogLandscape, Display, "The Landscape Proxy '%p' is loaded and valid, but not registered. This may cause incorrect display in the outliner view. You can reload the level to correct this.",
								LandscapeProxy);
...
void ALandscape::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	else if (MemberPropertyName == FName("RelativeScale3D") ||
			 MemberPropertyName == FName("RelativeLocation") ||
			 MemberPropertyName == FName("RelativeRotation"))
	{
		if (Info != nullptr)
		{
	...
	else if (MemberPropertyName == GET_MEMBER_NAME_CHECKED(ALandscape, bGrassEnabled))
	{
		if (!bGrassEnabled)
		{
			// This should not technically be required but is done to expedite the flush and mirror the default logic in ALandscape::SetGrassEnabled().
			// This may leave the grass builder in a non-sync'd state as it thinks it is populated but the grass maps are gone, but the builder does detect that
			// and will fix itself.
			FlushGrassComponents(/* OnlyForComponents = */nullptr, /* bFlushGrassMaps = */true);
		}
	}
	...
	if (Info != nullptr)
	{
		if (bPropagateToProxies)
		{
			...
			Info->ForEachLandscapeProxy([this, &PropertyChangedEvent](ALandscapeProxy* Proxy)
			{
				...
				if (Proxy != this)
				{
					Proxy->SynchronizeSharedProperties(this, /*bInExecutePostEditChangeProperty =*/true);
				}
				return true;
			});
		}
...
void ULandscapeComponent::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if ((MemberPropertyName == GET_MEMBER_NAME_CHECKED(ULandscapeComponent, OverrideMaterial))
		...
		|| (SubPropertyName == FName("MaterialPerLOD_Key")))
	{
...
TArray<UTexture*> ULandscapeComponent::ReallocateWeightmaps(FLandscapeEditDataInterface* DataInterface, const FGuid& InEditLayerGuid, bool bInSaveToTransactionBuffer, bool bInForceReallocate, 
	ALandscapeProxy* InTargetProxy, const TSet<ULandscapeComponent*>* InRestrictSharingToComponents)
{
	...
	if (!bInForceReallocate)
	{
		// UE_LOGF(LogLandscape, Log, "----------------------");
		// UE_LOGF(LogLandscape, Log, "Component %ls needs %d layers (%d new)", *GetName(), WeightmapLayerAllocations.Num(), NeededNewChannels);
		...
		if (ExistingTexAvailableChannels >= NeededNewChannels)
		{
			// UE_LOGF(LogLandscape, Log, "Existing texture has available channels");
	...
	// UE_LOGF(LogLandscape, Log, "Reallocating.");
	...
	{
		// UE_LOGF(LogLandscape, Log, "Still need %d channels", TotalNeededChannels);
		...
		{
			// UE_LOGF(LogLandscape, Log, "Looking for nearest");
		...
		if (CurrentWeightmapTexture == nullptr)
		{
			...
			// UE_LOGF(LogLandscape, Log, "Making a new texture %ls", *CurrentWeightmapTexture->GetName());
		}
		else
		{
		...
		{
			// UE_LOGF(LogLandscape, Log, "Finding allocation for layer %d", CurrentLayer);
			if (CurrentWeightmapUsage->ChannelUsage[ChanIdx] == nullptr)
			{
				...
				if (!AllocInfo.IsAllocated())
				{
					...
					if (DataInterface)
					{
						...
						// UE_LOGF(LogLandscape, Log, "Zeroing out channel %ls.%d", *CurrentWeightmapTexture->GetName(), ChanIdx);
					}
				}
				else
				{
					...
					if (ensure(DataInterface != nullptr)) // it's not safe to skip the copy
					{
						...
						// UE_LOGF(LogLandscape, Log, "Copying old channel (%ls).%d to new channel (%ls).%d", *OldWeightmapTexture->GetName(), AllocInfo.WeightmapTextureChannel, *CurrentWeightmapTexture->GetName(), ChanIdx);
					}
	...
	if (Landscape)
	{
		if (!Landscape->HasAnyFlags(RF_BeginDestroyed))
		{
				...
				|| (!LandscapeActorRef && LandscapeGuid.IsValid() && (LandscapeGuid == Landscape->GetLandscapeGuid())))
			{
				...
				if (!bCompatibleSize)
				{
					UE_LOGF(LogLandscape, Warning, "Landscape streaming proxy %ls's setup (Num Quads = %i, Num Subsections = %i, Num Quads per Subsection = %i) is not compatible with landscape actor %ls (Num Quads = %i, Num Subsections = %i, Num Quads per Subsection = %i)", 
...
bool ALandscapeProxy::LandscapeImportHeightmapFromRenderTarget(UTextureRenderTarget2D* InRenderTarget, bool InImportHeightFromRGChannel, int32 InEditLayerIndex)
{
	...
	UE_LOGF(LogLandscapeBP, Display, "Took %f seconds to import heightmap from render target.", SecondsTaken);
...
bool ALandscapeProxy::LandscapeExportHeightmapToRenderTarget(UTextureRenderTarget2D* InRenderTarget, bool bInExportHeightIntoRGChannel, bool InExportLandscapeProxies)
{
	...
	double SecondsTaken = FPlatformTime::ToSeconds64(FPlatformTime::Cycles64() - StartCycle);
	UE_LOGF(LogLandscapeBP, Display, "Took %f seconds to export heightmap to render target.", SecondsTaken);
#endif
	return true;
}
...
const TArray<ULandscapeEditLayerBase*> ALandscape::GetEditLayersBP() const
{
#if WITH_EDITOR
		return GetEditLayers();
#else // WITH_EDITOR
		return {};
#endif // !WITH_EDITOR
}

const ULandscapeEditLayerBase* ALandscape::GetEditLayerByNameBP(const FName& InLayerName) const
{
#if WITH_EDITOR
	return ObjectPtrWrap(GetEditLayer(InLayerName));
#else // WITH_EDITOR
	return nullptr;
#endif // !WITH_EDITOR
}
	...
	if (ALandscape* Landscape = GetLandscapeActor())
	{
		...
		if (LandscapeInfo && LandscapeInfo->GetLandscapeExtent(MinX, MinY, MaxX, MaxY))
		{
			...
			if (RTWidth >= LandscapeWidth && RTHeight >= LandscapeHeight)
			{
				...
				UE_LOGF(LogLandscape, Verbose, "Took %f seconds to import heightmap from render target", FPlatformTime::ToSeconds64(CycleEnd));
