	...
	ENQUEUE_RENDER_COMMAND(FLandscapeAsyncTextureReadback_CancelAndSelfDestruct)(
		[Readback](FRHICommandListImmediate& RHICmdList)
		{
			...
			else
			{
				...
				{
					UE_LOGF(LogLandscape, Warning, "In FLandscapeAsyncTextureReadback::CancelAndSelfDestruct(), readback not started, but AsyncReadback structure is unexpectedly allocated (%d, %d, %d, %d, %p, %d, %d, %d).  Attempting to clean it up.",
