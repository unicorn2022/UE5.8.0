...
{
	if (InTargetType == ELandscapeToolTargetType::Invalid)
	{
		return ELandscapeToolTargetTypeFlags::None;
	}
...
{
	check(FMath::CountBits64(static_cast<uint64>(InSingleFlag)) == 1);
...
bool SupportsRuntimeGrassMapGeneration()
{
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(CreateToolLandscapeMaterialInstanceConstant);
...
ULandscapeMaterialInstanceConstant* CreateLandscapeLayerThumbnailMIC(FMaterialUpdateContext& MaterialUpdateContext, UMaterialInterface* LandscapeMaterial, FName LayerName)
{
	...
	MaterialInstance->SetTextureParameterValueEditorOnly(FName("Weightmap0"), ThumbnailWeightmap);
	MaterialInstance->SetTextureParameterValueEditorOnly(FName("Heightmap"), ThumbnailHeightmap);
...
TArray<FName> RetrieveTargetLayerNamesFromMaterial(UMaterialInterface* InMaterial, bool bIncludeVisibilityLayer)
{
	TArray<FName> LayerNames;
	if (InMaterial)
	{
		const FMaterialCachedExpressionData& CachedExpressionData = InMaterial->GetCachedExpressionData();
		if (CachedExpressionData.EditorOnlyData)
		{
			LayerNames = CachedExpressionData.EditorOnlyData->LandscapeLayerNames;
			...
			LayerNames.RemoveAll([bIncludeVisibilityLayer, &VisibilityLayerName](const FName& Name)
				{
					return Name.IsNone() || (!bIncludeVisibilityLayer && Name == VisibilityLayerName);
				});
		}
	}
	return LayerNames;
...
TArray<TPair<FName, TObjectPtr<ULandscapeGrassType>>> ConvertToNamedGrassTypesList(const TMap<FName, TObjectPtr<ULandscapeGrassType>>& InNamedGrassTypes)
{
	TArray<TPair<FName, TObjectPtr<ULandscapeGrassType>>> LocalNamedGrassTypes;
	LocalNamedGrassTypes.Reserve(InNamedGrassTypes.Num());
	Algo::Transform(InNamedGrassTypes, LocalNamedGrassTypes, [](const TPair<FName, TObjectPtr<ULandscapeGrassType>>& ItPair) { return ItPair; });
	return LocalNamedGrassTypes;
}

TMap<FName, TObjectPtr<ULandscapeGrassType>> FilterValidNamedGrassTypes(const TMap<FName, TObjectPtr<ULandscapeGrassType>>& InNamedGrassTypes)
{
	TMap<FName, TObjectPtr<ULandscapeGrassType>> ValidNamedGrassTypes;
	ValidNamedGrassTypes.Reserve(InNamedGrassTypes.Num());
	for (const TPair<FName, TObjectPtr<ULandscapeGrassType>>& ItPair : InNamedGrassTypes)
	{
		if (!ItPair.Key.IsNone() && (ItPair.Value != nullptr))
		{
			ValidNamedGrassTypes.Add(ItPair);
		}
	}
	return ValidNamedGrassTypes;
}
