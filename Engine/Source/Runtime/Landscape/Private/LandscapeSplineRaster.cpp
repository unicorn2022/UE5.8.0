...
private:
	...
	bool Initialize(ULandscapeLayerInfoObject* InLayerInfo, int32 InLandscapeMinX, int32 InLandscapeMinY)
	{
		...
		{
			UE_LOGF(LogLandscape, Error, "Invalid source data for spline falloff modulation texture (%ls) of layer %ls. Alpha modulation will be disabled.",
		...
		{
			UE_LOGF(LogLandscape, Error, "Invalid source data for spline falloff modulation texture (%ls) of layer %ls: incorrect texture size (%lli x %lli). Alpha modulation will be disabled.",
		...
		{
			UE_LOGF(LogLandscape, Error, "Invalid color mask (%ls) for spline falloff modulation texture (%ls) of layer %ls: the texture only has %i channel(s) available. Alpha modulation will be disabled.",
		...
		{
			UE_LOGF(LogLandscape, Warning, "Invalid tiling for spline falloff modulation texture of layer %ls. Alpha modulation will be disabled.",
				*InLayerInfo->GetLayerName().ToString());
			return false;
		}
...
bool ULandscapeInfo::ApplySplinesInternal(TScriptInterface<ILandscapeSplineInterface> SplineOwner, TSet<TObjectPtr<ULandscapeComponent>>* OutModifiedComponents, bool bMarkPackageDirty, int32 LandscapeMinX, int32 LandscapeMinY, int32 LandscapeMaxX, int32 LandscapeMaxY, TFunctionRef<TSharedPtr<FModulateAlpha>(ULandscapeLayerInfoObject*)> GetOrCreateModulate)
{
	...
	for (const ULandscapeSplineControlPoint* ControlPoint : SplineComponent->ControlPoints)
	{
		...
		int32 MinX = FMath::FloorToInt32(ControlPointBounds.Min.X);
		int32 MinY = FMath::FloorToInt32(ControlPointBounds.Min.Y);
		int32 MaxX = FMath::CeilToInt32(ControlPointBounds.Max.X);
		int32 MaxY = FMath::CeilToInt32(ControlPointBounds.Max.Y);
	...
	for (const ULandscapeSplineSegment* Segment : SplineComponent->Segments)
	{
		...
		int32 MinX = FMath::FloorToInt32(SegmentBounds.Min.X);
		int32 MinY = FMath::FloorToInt32(SegmentBounds.Min.Y);
		int32 MaxX = FMath::CeilToInt32(SegmentBounds.Max.X);
		int32 MaxY = FMath::CeilToInt32(SegmentBounds.Max.Y);
