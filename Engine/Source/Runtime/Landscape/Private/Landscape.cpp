...
static void PrintNumLandscapeShadows()
{
	...
	UE_LOGF(LogLandscape, Log, "%d/%d landscape components cast shadows", NumShadowCasters, NumComponents);
}
...
FAutoConsoleVariable CVarLandscapeSilenceInvalidPropertyOverridesMapCheckWarnings(
	TEXT("landscape.SilenceInvalidPropertyOverridesMapCheckWarnings"),
	false,
	TEXT("Issue MapCheck Info messages instead of warnings if some landscape property overrides are incorrectly set (e.g. overriding the LandscapeMaterial but leaving it empty)."));
...
int32 ULandscapeComponent::GetCurrentRuntimeMaterialInstanceCount() const
{
	...
	const bool bIsMobile = (World != nullptr) && (World->GetFeatureLevel() == ERHIFeatureLevel::ES3_1);
	if (bIsMobile)
	{
...
UMaterialInterface* ULandscapeComponent::GetCurrentRuntimeMaterialInterface(int32 InIndex) const
{
	ALandscapeProxy* Proxy = GetLandscapeProxy();
	UWorld* World = GetWorld();
	...
	if (bIsMobile)
	{
		check(MobileMaterialInterfaces.IsValidIndex(InIndex));
...
bool ALandscapeProxy::IsNaniteMeshUpToDate() const
{
	...
	{
		// Consider that the Nanite mesh is up-to-date in the standalone editor (!GIsEditor) because the nanite content Id cannot be relied up in that mode and we still want to display Nanite. 
		//  This means the Nanite mesh might not be accurate, though. We could rebuild it then, just as we do on cook, but that would impact the standalone editor performance, and we don't want that. 
		if (GIsEditor)
		{
...
FGraphEventRef ALandscapeProxy::UpdateNaniteRepresentationAsync(const ITargetPlatform* InTargetPlatform)
{
	...
	if (IsNaniteEnabled() && !HasAnyFlags(RF_ClassDefaultObject) && LandscapeComponents.Num() > 0)
	{
		...
		const bool bNaniteContentDirty = !IsNaniteMeshUpToDate();
		if (bNaniteContentDirty && IsRunningCookCommandlet())
		{
			UE_LOGF(LogLandscape, Display, "Landscape Nanite out of date. Map requires resaving. Actor: '%ls' Package: '%ls'", *GetActorNameOrLabel(), *GetPackage()->GetName());
		}
		...
		for (int32 i = 0; i < NumNewNaniteComponents; ++i)
		{
			...
			if (bNaniteContentDirty)
			{
				FGraphEventRef ComponentProcessTask = NaniteComponents[i]->InitializeForLandscapeAsync(this, GetNaniteContentId(), NaniteComponents[i]->GetSourceLandscapeComponents(), i);
			...
			FGraphEventRef FinalizeEvent = FFunctionGraphTask::CreateAndDispatchWhenReady([WeakComponent, WeakProxy, Name = GetActorNameOrLabel(), InTargetPlatform]() {
				...
				{
					UE_LOGF(LogLandscape, Log, "UpdateNaniteRepresentationAsync Component on: '%ls' Is Invalid", *Name);
					return;
				}
...
void ALandscapeProxy::UpdateNaniteRepresentation(const ITargetPlatform* InTargetPlatform)
{
	...
	// If Nanite was disabled, we want to remove Nanite components no matter what 
	if (!IsNaniteEnabled())
	{
		RemoveNaniteComponents();
		return;
...
void ALandscapeProxy::InvalidateNaniteRepresentation(bool bInCheckContentId)
{
	...
	{
		// If Nanite was disabled, we want to remove Nanite components no matter what
		if (!IsNaniteEnabled() || !bInCheckContentId || !IsNaniteMeshUpToDate())
		{
			RemoveNaniteComponents();
		}
	}
...
FGuid ALandscapeProxy::GetNaniteContentId() const
{
	...
	// This function isn't meant to be called outside the actual editor (i.e. not in the -game editor), as it relies on ULandscapeTextureHash, which is editor-only
	// The only purpose of querying this is to detect whether Nanite landscape is out of date and regenerate it, which only doable in the editor anyway. 
	check(GIsEditor); 
...
void ALandscapeProxy::CheckForErrors()
{
	Super::CheckForErrors();

	ValidateGrassTypeOverrides();
}

void ALandscapeProxy::ValidateGrassTypeOverrides() 
{
	if (GrassTypesOverrides.IsEmpty())
	{
		return;
	}

 	TSet<UMaterialInterface*> LandscapeMaterials;
	RetrieveAllLandscapeMaterials(LandscapeMaterials);

	TArray<FString> MissingGrassOverrides;
	for (const auto& ItPair : GrassTypesOverrides)
	{
		FName GrassName = ItPair.Key.Name;
		if (!GrassName.IsNone())
		{
			// If that grass name is unknown to all of our landscape materials, report : 
			if (Algo::NoneOf(LandscapeMaterials, [&GrassName](UMaterialInterface* InMaterial) 
				{ 
					return InMaterial->GetCachedExpressionData().NamedGrassTypes.Contains(GrassName);
				}))
			{ 
				MissingGrassOverrides.AddUnique(GrassName.ToString());
			}
		}
	}

	if (!MissingGrassOverrides.IsEmpty())
	{
		TArray<FString> LandscapeMaterialStrings;
		LandscapeMaterialStrings.Reserve(LandscapeMaterials.Num());
		Algo::Transform(LandscapeMaterials, LandscapeMaterialStrings, [](UMaterialInterface* InMaterial) { return InMaterial->GetName(); });

		FFormatNamedArguments Arguments;
		Arguments.Add(TEXT("NumMissingGrassOverrides"), MissingGrassOverrides.Num());
		Arguments.Add(TEXT("MissingGrassOverrides"), FText::FromString(FString::Join(MissingGrassOverrides, TEXT(","))));
		Arguments.Add(TEXT("NumLandscapeMaterials"), LandscapeMaterialStrings.Num());
		Arguments.Add(TEXT("LandscapeMaterials"), FText::FromString(FString::Join(LandscapeMaterialStrings, TEXT(","))));

		FMessageLog MessageLog("MapCheck");
		MessageLog.Warning()->AddToken(FUObjectToken::Create(this, FText::FromString(GetActorNameOrLabel())))
			->AddToken(FTextToken::Create(FText::Format(LOCTEXT("MapCheck_Message_InvalidLandscapeGrassTypesOverride", 
				"None of the following grass type overrides '{MissingGrassOverrides}' can be found in the proxy landscape "
				"{NumLandscapeMaterials}|plural(one=material,other=materials) '{LandscapeMaterials}'. {NumMissingGrassOverrides}|plural(one=This override,other=These overrides) will be ineffective."), Arguments)))
			->AddToken(FMapErrorToken::Create(FMapErrors::LandscapeProxyInvalidGrassTypesOverride_Warning));
	}
}
...
void ULandscapeComponent::Serialize(FArchive& Ar)
{
	...
	if (Ar.IsLoadingFromCookedPackage() && (CachedLocalBox.GetVolume() <= 0))
	{
		...
		UE_LOGF(LogLandscape, Error, "The component %ls has an invalid CachedLocalBox. It has been set to a conservative bounds, that may result in reduced visibility culling performance", *GetName());
	}
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	...
	if (Ar.IsLoading())
	{
		...
		{
			check(NamedGrassTypes.IsEmpty()); 
			for (TObjectPtr<ULandscapeGrassType> GrassType : GrassTypes_DEPRECATED)
			{
				if (GrassType != nullptr)
				{
					// Just use the grass type's fallback name for backwards compatibility since we do the same in FMaterialCachedExpressionData::PostSerialize, 
					//  this will be updated by UpdateGrassTypes to support the actual remapping feature that comes with NamedGrassTypes
					check(!NamedGrassTypes.Contains(GrassType->GetGrassNameFallback())); // Should be unique as the old GrassTypes were forced unique by the UX code
					NamedGrassTypes.Emplace(GrassType->GetGrassNameFallback(), GrassType);
				}
			}
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	{
		UE_LOGF(LogLandscape, Fatal, "This platform requires cooked packages, and this landscape does not contain cooked data %ls.", *GetName());
	}
...
void ULandscapeComponent::RetrieveAllLandscapeMaterials(TSet<UMaterialInterface*>& OutMaterials) const
{
	...
	if (UMaterialInterface* Material = GetLandscapeHoleMaterial();
		Material && (Material != UMaterial::GetDefaultMaterial(MD_Surface)))
	{
		OutMaterials.Add(Material);
	}
...
void ULandscapeComponent::PostLoad()
{
	...
	if (ensure(LandscapeProxy))
	{
		...
		{
			UE_LOGF(LogLandscape, Warning, "LandscapeComponent SectionBaseX disagrees with its location, attempted automated fix: '%ls', %d,%d vs %d,%d.",
	...
	if (GIsEditor && !HasAnyFlags(RF_ClassDefaultObject) && ensure(LandscapeProxy))
	{
		...
		{
			UE_LOGF(LogLandscape, Warning, "LandscapeComponent RelativeLocation disagrees with its section base, attempted automated fix: '%ls', %f,%f vs %f,%f.",
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	auto ReparentObject = [this](UObject* Object)
	{
		...
		{
			Object->Rename(nullptr, GetOuter(), REN_AllowPackageLinkerMismatch);
			return true;
		}
		return false;
	};
	...
	if (GIsEditor && !HasAnyFlags(RF_ClassDefaultObject))
	{
		...
		if (ULevel* Level = GetLevel())
		{
			...
			for (auto* Obj : ObjectsToMoveFromLevelToPackage)
			{
				...
				{
					Obj->Rename(nullptr, MyPackage, REN_DoNotDirty | REN_DontCreateRedirectors | REN_NonTransactional | REN_AllowPackageLinkerMismatch);
				}
			}
		}
	}
	...
	if (MobileCombinationMaterialInstances.Num() == 0)
	{
		...
		{
			UE_LOGF(LogLandscape, Error, "Landscape component (%d, %d) Does not have a valid mobile combination material. To correct this issue, open the map in the editor and resave the map.", SectionBaseX, SectionBaseY);
		}
	}
#endif // UE_BUILD_SHIPPING
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	if (GIsEditor && GetLinkerCustomVersion(FFortniteMainBranchObjectVersion::GUID) < FFortniteMainBranchObjectVersion::OverridableLandscapeGrassTypes)
	{
		// Make sure grass types are up-to-date (i.e. the grass names are updated based on the material). We need to make sure the material has performed its post-load :
		if (UMaterialInterface* Material = GetLandscapeMaterial())
		{
			Material->ConditionalPostLoad();
		}
		UpdateGrassTypes();
		// Prior to this version, GrassData contained ULandscapeGrassType* entries. These have turned into GrassName (FName), as declared by the landscape material. We can do the fixup here
		//  since we have access to the material-declared grass types here (while FLandscapeComponentGrassData's operator<< cannot) :
		GrassData->FixupFallbackGrassNames(GetNamedGrassTypes());
	}

#endif // WITH_EDITOR

	GrassData->ConditionalDiscardDataOnLoad(GetNamedGrassTypes());
}

void ULandscapeComponent::CollectPSOPrecacheData(const FPSOPrecacheParams& BasePrecachePSOParams, FMaterialInterfacePSOPrecacheParamsList& OutParams)
{
	FPSOPrecacheParams PrecachePSOParams = BasePrecachePSOParams;
	PrecachePSOParams.bIgnoreMaskMaterial = !UE::Landscape::UseLandscapeVertexMask();
	...
	if (UE::Landscape::NeedsFixedGridVertexFactory(GMaxRHIShaderPlatform))
	{
		VertexFactoryDataList.Add(FPSOPrecacheVertexFactoryData(&FLandscapeFixedGridVertexFactory::StaticType));
	}

	if (UE::Landscape::Culling::UseCulling(GMaxRHIShaderPlatform))
	{
		VertexFactoryDataList.Add(FPSOPrecacheVertexFactoryData(UE::Landscape::Culling::GetTileVertexFactoryType()));
	}

	ILandscapeModule& LandscapeModule = FModuleManager::GetModuleChecked<ILandscapeModule>("Landscape");
	const UE::Landscape::FSetupLandscapePSOPrecacheVertexFactoryDataListDelegate& SetupLandscapePSOPrecacheVertexFactoryDataListDelegate = LandscapeModule.GetSetupLandscapePSOPrecacheVertexFactoryDataListDelegate();
	if (SetupLandscapePSOPrecacheVertexFactoryDataListDelegate.IsBound())
	{
		SetupLandscapePSOPrecacheVertexFactoryDataListDelegate.Execute(this, VertexFactoryDataList);
	}
	...
	for (UMaterialInterface* MaterialInterface : Materials)
	{
		if (MaterialInterface)
		{
			...
			ComponentParams.VertexFactoryDataList = VertexFactoryDataList;
...
void ALandscapeProxy::PostInitProperties()
{
	...
	if (UWorld* World = GetWorld())
	{
		if (!IsInAsyncLoadingThread() && !HasAnyFlags(RF_ArchetypeObject | RF_ClassDefaultObject))
		{
			...
			// This needs to be called on the game thread. In the case of async loading, it will get called in PostLoad()
...
void ALandscape::SetGrassEnabled(bool bEnabled, bool bFlushGrassMaps)
{
	if (bEnabled != bGrassEnabled)
	{
		bGrassEnabled = bEnabled;

		if (!bGrassEnabled)
		{
			FlushGrassComponents(/* OnlyForComponents = */nullptr, /* bFlushGrassMaps = */bFlushGrassMaps);
		}
	}
}
...
{
	if (ALandscapeProxy* LandscapeProxy = GetLandscapeProxy())
	{
		// When the proxy is a LandscapeStreamingProxy, the landscape actor may be invalid whenever ALandscape is not registered to the current LandscapeInfo
		// This may happen during operations like Undo that trigger UnregisterActor events
		return LandscapeProxy->GetLandscapeActor();
	}
	return nullptr;
}
...
FString ULandscapeComponent::GetDebugName() const
{
	const FIntPoint ComponentKey = GetComponentKey();
	const FIntPoint SectionBase = GetSectionBase();
	return FString::Format(TEXT("Component_{0}_{1}[{2}_{3}]"), { ComponentKey.X, ComponentKey.Y, SectionBase.X, SectionBase.Y });
}
...
TArray<ULandscapeGrassType*> ULandscapeComponent::GetGrassTypesBP() const
{
	TArray<ULandscapeGrassType*> GrassTypes;
	GrassTypes.Reserve(NamedGrassTypes.Num());
	for (const TPair<FName, TObjectPtr<ULandscapeGrassType>>& ItPair : NamedGrassTypes)
	{
		if (ItPair.Value != nullptr)
		{
			GrassTypes.Add(ItPair.Value);
		}
	}
	return GrassTypes;
}

TArray<TObjectPtr<ULandscapeGrassType>> ULandscapeComponent::GetGrassTypes() const
{
	return ObjectPtrWrap(GetGrassTypesBP());
}
...
{
	if (CheckPSOPrecachingAndBoostPriority(EPSOPrecachePriority::Highest) && UE::Landscape::GetLandscapePSOPrecacheProxyCreationStrategy() == EPSOPrecacheProxyCreationStrategy::DelayUntilPSOPrecached)
	{
		UE_LOGF(LogLandscape, Verbose, "Skipping CreateSceneProxy for ULandscapeComponent %ls (Its PSOs are still compiling)", *GetFullName());
		return nullptr;
	}
...
ULandscapeHeightmapTextureEdgeFixup* ULandscapeComponent::InstallOrUpdateTextureUserDatas(bool bUseEdgeFixup, bool bUseCompression, bool bUpdateSnapshotNow, int32 HeightmapCompressionMipThreshold)
{
	...
	{
		UE_LOGF(LogLandscape, Warning, "Tried to install EdgeFixup on component %ls (proxy %ls), but it had NO heightmap", *GetPathName(), *GetLandscapeProxy()->GetPathName());
		return nullptr;
	}
...
#endif // !WITH_EDITOR
	{
		...
		{
			StorageFactory = ULandscapeTextureStorageProviderFactory::ApplyTo(HeightmapTexture, LandscapeGridScale, HeightmapCompressionMipThreshold, NumSubsections, SubsectionSizeQuads);
		}
		else
		{
			// since different platforms may change compression settings / thresholds, we should update the compression each time
			StorageFactory->UpdateCompressedDataFromSource(HeightmapTexture, LandscapeGridScale, HeightmapCompressionMipThreshold, NumSubsections, SubsectionSizeQuads);
		}
#endif // WITH_EDITOR
...
const TArray<UTexture2D*>& ULandscapeComponent::GetCurrentRuntimeWeightmapTextures() const
{
	UWorld* World = GetWorld();
	bool bIsMobile = (World != nullptr) && (World->GetFeatureLevel() == ERHIFeatureLevel::ES3_1);
	return bIsMobile ? MobileWeightmapTextures : WeightmapTextures;
}
...
void ULandscapeComponent::CopyComponentDataToEditLayer(const FGuid& InSourceEditLayerGuid, const FGuid& InDestEditLayerGuid, bool bInUseObsoleteLayerData, TSet<UTexture2D*>& OutProcessedHeightmaps, TSet<UTexture2D*>& OutProcessedWeightmaps)
{
	...
	{
		UE_LOGF(LogLandscape, Error, "CopyComponentDataToEditLayer: Failed to copy data to component: %ls. Destination edit layer guid is invalid or Proxy is not yet registered with ALandscape", *GetName());
		return;
	}

	if (DestLayerComponentData == nullptr || !DestLayerComponentData->IsInitialized())
	{
		UE_LOGF(LogLandscape, Error, "CopyComponentDataToEditLayer: Failed to copy data to component: %ls. Destination edit layer data does not exist or is not initialized on component", *GetName());
		return;
	}
	...
	{
		UE_LOGF(LogLandscape, Error, "CopyComponentDataToEditLayer: Failed to copy data to destination layer for %ls : Component: %ls. Source layer GUID is invalid or data is not initialized properly",
...
void ULandscapeComponent::AddDefaultLayerData(const FGuid& InLayerGuid, const TArray<ULandscapeComponent*>& InComponentsUsingHeightmap, TMap<UTexture2D*, UTexture2D*>& InOutCreatedHeightmapTextures)
{
	...
	if (LayerData == nullptr || !LayerData->IsInitialized())
	{
		...
		const FColor DefaultHeightColor = LandscapeDataAccess::GetDefaultPackedHeightColor();
					...
					for (int32 SubsectionX = 0; SubsectionX < NumSubsections; SubsectionX++)
					{
						for (int32 SubY = 0; SubY <= SubsectionSizeQuads; SubY++)
						{
							for (int32 SubX = 0; SubX <= SubsectionSizeQuads; SubX++)
							{
								...
								// Copy default height and normal data. Normals will be calculated later
								Mip0Data[HeightTexDataIdx] = DefaultHeightColor;
							}
						}
					}
				}
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
		...
		{
			UE_LOGF(LogLandscape, Error, "Couldn't load edit layer object associated with layer at index %i for landscape %ls. This may happen when the edit layer class cannot be found "
...
bool ALandscape::RenderMergedTextureInternal(const FTransform& InRenderAreaWorldTransform, const FBox2D& InRenderAreaExtents, const TArray<FName>& InWeightmapLayerNames, UTextureRenderTarget* OutRenderTarget)
{
	...
	{
		UE_LOGF(LogLandscape, Log, "RenderMergedTexture : Cannot render anything on landscape (%ls) : the app doesn't have a valid rendering device ", *GetFullName());
		return false;
	}
	...
	if (Info == nullptr)
	{
		UE_LOGF(LogLandscape, Error, "RenderMergedTexture : Cannot render anything if there's no associated landscape info with this landscape (%ls)", *GetFullName());
		return false;
	}
	...
	{
		UE_LOGF(LogLandscape, Error, "RenderMergedTexture : Missing render target");
		return false;
	}
	...
	if (bIsHeightmap)
	{
		...
		{
			UE_LOGF(LogLandscape, Error, "RenderMergedTexture : Heightmap capture requires a UTextureRenderTarget2D");
			return false;
		}

		if (!IsValidRenderTargetFormatHeightmap(RenderTargetFormat, bCompressHeight))
	{
			UE_LOGF(LogLandscape, Warning, "RenderMergedTexture : invalid render target format for rendering heightmap (%ls)", GetPixelFormatString(RenderTargetFormat));
			return false;
		}
	}
	else
	{
		...
		{
			UE_LOGF(LogLandscape, Warning, "RenderMergedTexture : invalid render target format for rendering weightmap (%ls)", GetPixelFormatString(RenderTargetFormat));
			return false;
		}
		...
		if (InWeightmapLayerNames.Num() > 1)
		{
			...
			{
				UE_LOGF(LogLandscape, Warning, "RenderMergedTexture : Not enough channels available (%i) in render target to accomodate for all requested weightmaps (%i)", NumChannels, InWeightmapLayerNames.Num());
				return false;
			}
			else if ((RenderTarget2DArray != nullptr) && ((NumChannels * RenderTarget2DArray->Slices) < InWeightmapLayerNames.Num()))
			{
				UE_LOGF(LogLandscape, Warning, "RenderMergedTexture : Not enough channels available (%i) in render target array to accomodate for all requested weightmaps (%i)", NumChannels * RenderTarget2DArray->Slices, InWeightmapLayerNames.Num());
				return false;
			}
		}
	}
	...
	{
		UE_LOGF(LogLandscape, Log, "RenderMergedTexture : nothing to render");
		return true;
	}
	...
	{
		const FName TargetLayerName = bIsHeightmap ? FName("Heightmap") : InWeightmapLayerNames[TargetLayerIndex];
		...
		for (auto It : OverlappedComponents)
		{
			...
			{
				const TArray<UTexture2D*>& WeightmapTextures = Component->GetCurrentRuntimeWeightmapTextures();
				const TArray<FWeightmapLayerAllocationInfo>& AllocInfos = Component->GetCurrentRuntimeWeightmapLayerAllocations();
...
{
	FProperty* bNaniteSkirtEnabledProperty = StaticClass()->FindPropertyByName(GET_MEMBER_NAME_CHECKED(ALandscapeProxy, bNaniteSkirtEnabled));
	FProperty* NaniteSkirtDepthProperty = StaticClass()->FindPropertyByName(GET_MEMBER_NAME_CHECKED(ALandscapeProxy, NaniteSkirtDepth));
	check(bNaniteSkirtEnabledProperty && NaniteSkirtDepthProperty);

	// PostEditChangeProperty propagates to proxies and invalidates nanite/render state
	if (bNaniteSkirtEnabled != bInEnable)
	{
		Modify(bInShouldDirtyPackage);
		bNaniteSkirtEnabled = bInEnable;

		FPropertyChangedEvent PropertyChangedEvent(bNaniteSkirtEnabledProperty);
		PostEditChangeProperty(PropertyChangedEvent);
	}

	if (NaniteSkirtDepth != InSkirtDepth)
	{
		Modify(bInShouldDirtyPackage);
		NaniteSkirtDepth = InSkirtDepth;

		FPropertyChangedEvent PropertyChangedEvent(NaniteSkirtDepthProperty);
		PostEditChangeProperty(PropertyChangedEvent);
	}
}
...
void ALandscapeProxy::PreSave(FObjectPreSaveContext ObjectSaveContext)
{
	...
	if (!ObjectSaveContext.IsProceduralSave()) // only finalize grass in a true editor save (when a GPU is available).
	{
		...
		// The grass maps will get stripped later in BeginCacheForCookedPlatformData for cooked builds that don't need them.
		{
			...
			UE_LOGF(LogGrass, Verbose, "PRESAVE: landscape %ls has %d / %d valid grass components (UseRuntimeGeneration %d Disable %d)", *GetName(), ValidGrassCount, LandscapeComponents.Num(), GGrassMapUseRuntimeGeneration, bDisableRuntimeGrassMapGeneration);
		}
	}
	...
	for (ULandscapeComponent* LandscapeComponent : LandscapeComponents)
	{
		...
		{
			UE_LOGF(LogLandscape, Display, "Completing landscape physical material before saving.  %ls", *LandscapeComponent->GetFullName());
			...
			{
				UE_LOGF(LogLandscape, Error, "Landscape physical material failed to complete before saving. That could be due to the landscape material failing to compile on this component (%ls)", *LandscapeComponent->GetFullName());
			}
		}
		...
		if (LandscapeComponent->UpdateCachedBounds(/* bInApproximateBounds= */ false))
		{
			...
			{
				UE_LOGF(LogLandscape, Display, "The component %ls had non-optimal bounds.  The bounds have been recalculated (old CachedLocalBox: %ls, new CachedLocalBox: %ls)", *LandscapeComponent->GetPathName(), *OldCachedLocalBox.ToString(), *LandscapeComponent->CachedLocalBox.ToString());
			}
			else
			{
				UE_LOGF(LogLandscape, Display, "The component %ls had incorrect bounds.  The bounds have been recalculated (old CachedLocalBox: %ls, new CachedLocalBox: %ls)", *LandscapeComponent->GetPathName(), *OldCachedLocalBox.ToString(), *LandscapeComponent->CachedLocalBox.ToString());
			}
...
void ALandscapeProxy::PostLoad()
{
	...
	for (ULandscapeComponent* Comp : LandscapeComponents)
	{
		...
		UE_LOGF(LogGrass, Verbose, "POSTLOAD: component %ls on landscape %ls UseRuntimeGeneration %d Disable: %d data: %d",
			*Comp->GetName(),
			*GetName(),
		...
		if (GGrassMapUseRuntimeGeneration && !bDisableRuntimeGrassMapGeneration)
		{
			...
			{
				UE_LOGF(LogGrass, Warning, "grass.GrassMap.UseRuntimeGeneration is enabled, but component %ls on landscape %ls has unnecessary grass data saved.  Ensure grass.GrassMap.UseRuntimeGeneration is enabled at cook time to reduce cooked data size.",
					*Comp->GetName(),
					*GetName());
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	// Silently add a visibility layer to each proxy. The cost is minimal, since components do not store weightmap info until data is written to the layer
	// Target layers are no longer auto generated during material compilation so this guarantees the visibility layer is always found
	const FName VisibilityLayerName = (ALandscapeProxy::VisibilityLayer != nullptr) ? ALandscapeProxy::VisibilityLayer->GetLayerName() : FName();
	if (!IsTemplate() && !VisibilityLayerName.IsNone() && !HasTargetLayer(VisibilityLayerName))
	{
		AddTargetLayer(VisibilityLayerName, FLandscapeTargetLayerSettings(), /*bPostEditChange =*/false);
	}
#endif // WITH_EDITOR
}
...
{
	static bool CopyProperty(FProperty* InProperty, UObject* InSourceObject, UObject* InDestinationObject, bool bInExecutePostEditChangeProperty)
	{
		// Do not execute the post edit change if the containing level has not finished post loading.  
		// This will trigger Registration of the components, before the owning level of the world has been determined.
		// This avoids registring landscape proxies in the incorrect landscape info object.
		if(bInExecutePostEditChangeProperty)
		{
			ULevel* Level = nullptr;
			if (ALandscapeProxy* LandscapeProxy = Cast<ALandscapeProxy>(InDestinationObject))
			{
				Level = LandscapeProxy->GetLevel();
			}
			else if (USceneComponent* SceneComponent = Cast<USceneComponent>(InDestinationObject))
			{
				Level = SceneComponent->GetOwner()->GetLevel();
			}

			bInExecutePostEditChangeProperty = Level == nullptr || !Level->HasAnyFlags(EObjectFlags::RF_NeedPostLoad);
			ensureAlwaysMsgf(bInExecutePostEditChangeProperty, TEXT("Executing CopyProperty with bInExecutePostEditChangeProperty set to true before the level has post loaded would trigger registration in the world before it's owning world has been determined."));
		}
		...
		if (bInExecutePostEditChangeProperty)
		{
			...
			InDestinationObject->PostEditChangeProperty(PropertyChangedEvent);
			InDestinationObject->Modify();
		}

		return true;
	}

	static bool CopyPropertyByName(const TWeakObjectPtr<ALandscapeProxy>& InLandscapeProxy, const TWeakObjectPtr<ALandscape>& InParentLandscape, const FName& InPropertyName, bool bInExecutePostEditChangeProperty)
	{
		...
		CopyProperty(PropertyToCopy, InParentLandscape.Get(), InLandscapeProxy.Get(), bInExecutePostEditChangeProperty);
		return true;
	}
...
void ALandscapeProxy::CopySharedProperties(ALandscapeProxy* InLandscape, bool bInExecutePostEditChangeProperty)
{
	...
	for (TFieldIterator<FProperty> PropertyIterator(GetClass()); PropertyIterator; ++PropertyIterator)
	{
		...
		{
			UE::Landscape::Private::CopyProperty(Property, InLandscape, this, bInExecutePostEditChangeProperty);
		}
	}
}

TArray<FName> ALandscapeProxy::SynchronizeSharedProperties(ALandscapeProxy* InLandscape, bool bInExecutePostEditChangeProperty)
{
	...
	for (TFieldIterator<FProperty> PropertyIterator(GetClass()); PropertyIterator; ++PropertyIterator)
	{
			...
			!Property->Identical_InContainer(this, InLandscape))
		{
			...
			// Caller may need a PostEditChangeProperty to trigger other landscape updates (Eg. Dirty Render state, nanite, materials)
			// PostEditChangeProperty updates are unnecessary when syncing properties during new landscape creation
			UE::Landscape::Private::CopyProperty(Property, InLandscape, this, bInExecutePostEditChangeProperty);
		}
	}
...
UMaterialInterface* ALandscapeStreamingProxy::GetLandscapeMaterial(int8 InLODIndex) const
{
	...
	// Note : we don't fallback to the parent ALandscape anymore, since this is only reliable in the editor. The LandscapeMaterial property is now a LandscapeOverridable one,
	//  which means that overriding the material is now done explicitly via OverriddenSharedProperties rather than by the LandscapeMaterial property value being not null. It means it's now a
	//  "valid" thing to override the material property and set it to "None" (probably a user mistake, though, hence the MapCheck warning if data is setup this way). 
	//  In order to make the error plain visible, we therefore return an invalid material now, so that the editor doesn't show the parent ALandscape's material, while cooking
	//  potentially returns an invalid material depending on whether ALandscape is loaded. 
...
UMaterialInterface* ALandscapeStreamingProxy::GetLandscapeHoleMaterial() const
{
	...
	// Note : we don't fallback to the parent ALandscape anymore, since this is only reliable in the editor. The LandscapeHoleMaterial property is now a LandscapeOverridable one,
	//  which means that overriding the material is now done explicitly via OverriddenSharedProperties rather than by the LandscapeHoleMaterial property value being not null. It means it's now a
	//  "valid" thing to override the material property and set it to "None" (probably a user mistake, though, hence the MapCheck warning if data is setup this way). 
	//  In order to make the error plain visible, we therefore return a null material now, so that the editor doesn't show the parent ALandscape's hole material, while cooking
	//  potentially returns an invalid material depending on whether ALandscape is loaded. 

	return nullptr;
}

void ALandscapeProxy::RetrieveAllLandscapeMaterials(TSet<UMaterialInterface*>& OutMaterials) const
{
	// Retrieve the materials specified in the proxy or its parent landscape (for a streaming proxy)
	if (UMaterialInterface* Material = GetLandscapeMaterial();
		Material && (Material != UMaterial::GetDefaultMaterial(MD_Surface)))
	{
		OutMaterials.Add(Material);
	...
	// Also retrieve the materials from the components, as they may override the proxy's materials : 
	for (ULandscapeComponent* Component : LandscapeComponents)
	{
		if (Component != nullptr)
		{
			Component->RetrieveAllLandscapeMaterials(OutMaterials);
		}
	}
}
...
void ALandscapeStreamingProxy::SetSharedPropertyOverride(const FName& InPropertyName, const bool bIsOverridden, const bool bInExecutePostEditChangeProperty)
{
	...
	else
	{
		...
		{
			UE_LOGF(LogLandscape, Warning, "Unable to retrieve the parent landscape's shared property value (ALandscapeStreamingProxy: %ls, Property: %ls). The proper value will be fixedup when reloading this proxy.",
				   *GetFullName(), *InPropertyName.ToString());
		}
		else
		{
			UE::Landscape::Private::CopyPropertyByName(LandscapeProxy, ParentLandscape, InPropertyName, bInExecutePostEditChangeProperty);
		}
...
void ALandscapeStreamingProxy::RequestDeferredLayerContentUpdateModes(ELandscapeLayerUpdateMode InLayerContentUpdateModes)
{
	DeferredLayerContentUpdateModes |= InLayerContentUpdateModes;
}

void ALandscapeStreamingProxy::PerformDeferredLayerContentUpdateModesRequest()
{
	if (DeferredLayerContentUpdateModes != 0)
	{
		check(LandscapeActorRef != nullptr);
		LandscapeActorRef->RequestLayersContentUpdate((ELandscapeLayerUpdateMode)DeferredLayerContentUpdateModes);

		// Reset deferred update modes
		DeferredLayerContentUpdateModes = 0;
	}
}

void ALandscapeProxy::UpgradeSharedProperties(ALandscape* InParentLandscape, const bool bInExecutePostEditChangeProperty)
{
	...
	for (TFieldIterator<FProperty> PropertyIterator(GetClass()); PropertyIterator; ++PropertyIterator)
	{
		...
		{
			SynchronizedProperties.Emplace(Property->GetFName());
			UE::Landscape::Private::CopyProperty(Property, InParentLandscape, this, bInExecutePostEditChangeProperty);
		}
		else if (IsPropertyOverridable(Property) && !IsSharedPropertyOverridden(Property->GetFName()) && !Property->Identical_InContainer(this, InParentLandscape))
		{
			...
			{
				SetSharedPropertyOverride(Property->GetFName(), true, bInExecutePostEditChangeProperty);
			}
			else
			{
				FFormatNamedArguments Arguments;
				...
				FMessageLog("MapCheck").Warning()
					...
					->AddToken(FActionToken::Create(LOCTEXT("MapCheck_OverrideProperty", "Override property"), LOCTEXT("MapCheck_OverrideProperty_Desc", "Keeping the current value and marking the property as overriding the parent landscape's value."),
						FOnActionTokenExecuted::CreateLambda([LandscapeProxy, PropertyName, bInExecutePostEditChangeProperty]()
							{
								if (LandscapeProxy.IsValid())
								{
									LandscapeProxy->SetSharedPropertyOverride(PropertyName, true, bInExecutePostEditChangeProperty);
								}
							}),
						/*bInSingleUse = */true))
					...
					->AddToken(FActionToken::Create(LOCTEXT("MapCheck_InheritProperty", "Inherit from parent landscape"), LOCTEXT("MapCheck_InheritProperty_Desc", "Copying the parent landscape's value for this property."),
							FOnActionTokenExecuted::CreateLambda([LandscapeProxy, ParentLandscape, PropertyName, bInExecutePostEditChangeProperty]()
								{
									UE::Landscape::Private::CopyPropertyByName(LandscapeProxy, ParentLandscape, PropertyName, bInExecutePostEditChangeProperty);
								}),
							/*bInSingleUse = */true))
...
void ALandscapeProxy::FixupSharedData(ALandscape* Landscape, const bool bMapCheck, const bool bInExecutePostEditChangeProperty)
{
	if (Landscape == nullptr)
	{
		UE_LOGF(LogLandscape, Error, "FixupSharedData: Invalid parent ALandscape. Proxy failed to synchronize parent properties.");
		return;
	}
	...
	{
		UpgradeSharedProperties(Landscape, bInExecutePostEditChangeProperty);
		bUpgradeSharedPropertiesPerformed = true;
	}
	else
	{
		SynchronizedProperties = SynchronizeSharedProperties(Landscape, bInExecutePostEditChangeProperty);
	...
	{
		UE_LOGF(LogLandscape, Warning, "FixupSharedData: Removed invalid target layer. Batched merge requires all target layers to have a valid FName. Actor: '%ls' Package: '%ls'", *GetActorNameOrLabel(), *GetPackage()->GetName());
	...
	// Validate that property overrides are properly setup
	CheckOverriddenSharedProperties();
...
void ULandscapeInfo::FixupProxiesTransform(bool bDirty)
{
	...
	for (TWeakObjectPtr<ALandscapeStreamingProxy> ProxyPtr : SortedStreamingProxies)
	{
		...
		if (LandscapeSectionOffsetRem.X != 0 || LandscapeSectionOffsetRem.Y != 0)
		{
			...
			UE_LOGF(LogLandscape, Warning, "Landscape section base is not multiple of component size, attempted automated fix: '%ls', %d,%d vs %d,%d.",
...
#if WITH_EDITOR
	if (!Landscape->IsNaniteEnabled() || !GIsEditor)
	{
		return;
	}
...
void ULandscapeInfo::RegisterLandscapeActorWithProxyInternal(ALandscapeProxy* Proxy, bool bMapCheck)
{
...
#if WITH_EDITOR
		// Proxy may have a deferred update request. This occurs if a component requests an update when the LandscapeActorRef is invalid.
		// This can happen on Undo, when the component requests an update while ALandscape is unregistered 
		StreamingProxy->PerformDeferredLayerContentUpdateModesRequest();

		StreamingProxy->FixupSharedData(Landscape, bMapCheck, true /*bInExecutePostEditChangeProperty*/);
#endif // WITH_EDITOR
	...
	if (bool bLayerInfoMapChanged = UpdateLayerInfoMap(Proxy))
	{
		...
		{
			Proxy->FixupSharedData(Landscape, bMapCheck, true /*bInExecutePostEditChangeProperty*/);
			return true;
		});
	}
		...
		if (Proxy->RegistrationCount == 0)
		{
			...
			*/
			...
			if (!LandscapeActor->IsTemplate() && bProxyRequiresNonEditLayerConversion)
			{
				...
				{
					UE_LOGF(LogLandscape, Log, "RegisterLandscapeActorWithProxyInternal: Automatically enabling edit layers on ALandscape: %ls", *Proxy->GetFullName());
				...
				{
					UE_LOGF(LogLandscape, Log, "RegisterLandscapeActorWithProxyInternal: Automatically enabling edit layers on Proxy: %ls", *Proxy->GetFullName());
...
void ULandscapeInfo::RegisterActor(ALandscapeProxy* Proxy, bool bMapCheck, bool bUpdateAllAddCollisions)
{
	...
	if (ALandscape* Landscape = Cast<ALandscape>(Proxy))
	{
		...
		{
			UE_LOGF(LogLandscape, Warning, "Multiple landscape actors with the same GUID detected: %ls vs %ls", *LandscapeActor->GetPathName(), *Landscape->GetPathName());
		}
#if WITH_EDITORONLY_DATA
		Landscape->bIsRegisteredWithLandscapeInfo = true;
#endif // WITH_EDITORONLY_DATA
...
#if WITH_EDITOR
TArray<FTargetLayerBlendGroup> ULandscapeInfo::GetTargetLayerBlendGroups() const
{
	TArray<FTargetLayerBlendGroup> BlendGroups;
	for (const FLandscapeInfoLayerSettings& LayerSettings : Layers)
	{
		if (LayerSettings.LayerInfoObj != nullptr)
		{
			if (LayerSettings.LayerInfoObj->GetBlendMethod() != ELandscapeTargetLayerBlendMethod::None)
			{
				FTargetLayerBlendGroup* BlendGroup = BlendGroups.FindByPredicate([&LayerSettings](const FTargetLayerBlendGroup& InGroup)
					{
						return (InGroup.GetBlendMethod() == LayerSettings.LayerInfoObj->GetBlendMethod())
							&& (InGroup.GetName() == LayerSettings.LayerInfoObj->GetBlendGroup());
					});

				if (BlendGroup == nullptr)
				{
					BlendGroup = &BlendGroups.Add_GetRef(FTargetLayerBlendGroup(LayerSettings.LayerInfoObj->GetBlendGroup(), LayerSettings.LayerInfoObj->GetBlendMethod(), { LayerSettings.LayerInfoObj }));
				}

				BlendGroup->TargetLayers.Add(LayerSettings.LayerInfoObj);
			}
...
TOptional<FTargetLayerBlendGroup> ULandscapeInfo::GetBlendGroupForTargetLayer(ULandscapeLayerInfoObject* InLayerInfo) const
{
	TOptional<FTargetLayerBlendGroup> BlendGroup;

	TArray<FTargetLayerBlendGroup> BlendGroups = GetTargetLayerBlendGroups();
	if (FTargetLayerBlendGroup* FoundBlendGroup = BlendGroups.FindByPredicate([InLayerInfo](const FTargetLayerBlendGroup& InGroup)
		{
			return InGroup.GetTargetLayers().Contains(InLayerInfo);
		}))
	{
		BlendGroup = *FoundBlendGroup;
	}
...
void ALandscapeProxy::SetSourceComponentsForNaniteComponents(bool bSetEmptyComponentsOnly) const
{
	...
	for (int32 NaniteComponentIndex = 0; NaniteComponentIndex < NaniteComponents.Num(); ++NaniteComponentIndex)
	{
		...
		{
			UE_LOGF(LogLandscape, Warning, "Landscape Nanite failed to set source components for NaniteComponents[%d]. Landscape Nanite requires rebuild. Actor: '%ls' Package: '%ls'", NaniteComponentIndex, *GetActorNameOrLabel(), *GetPackage()->GetName());
			continue;
		}
...
void ALandscapeProxy::OnLayerInfoObjectDataChanged(const FOnLandscapeLayerInfoDataChangedParams& InParams)
{
	...
	if ((InParams.PropertyChangedEvent.ChangeType != EPropertyChangeType::Interactive) 
		&& ((MemberPropertyName == ULandscapeLayerInfoObject::GetBlendMethodMemberName())
			|| (MemberPropertyName == ULandscapeLayerInfoObject::GetBlendGroupMemberName())
			|| bUpdateAll))
	{
		// There's no real hook to attach to when a custom thumbnail is generated (or cleared) for a given asset and we need to regenerate the thumbnails when that happens.
		//  PostEditChange is called on the layer info object when this happens, though, so we can ask for regenerating thumbnails when that happens. Not the most efficient
		//  thing but landscape layer changes are pretty rare anyway
		ILandscapeModule& LandscapeModule = FModuleManager::GetModuleChecked<ILandscapeModule>("Landscape");
...
void ALandscapeProxy::UpdateRenderingMethod()
{
	...
	if (bNaniteActive)
	{
		bNaniteActive = IsNaniteMeshUpToDate();
	}
#endif //WITH_EDITOR
