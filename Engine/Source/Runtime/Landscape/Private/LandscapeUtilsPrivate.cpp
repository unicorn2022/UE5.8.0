...
uint32 ComputeNamedGrassTypesHash(const TMap<FName, TObjectPtr<ULandscapeGrassType>>& InNamedGrassTypes)
{
	uint32 Hash = 0;

	// Only keep the valid entries
	TMap<FName, TObjectPtr<ULandscapeGrassType>> ValidNamedGrassTypes = FilterValidNamedGrassTypes(InNamedGrassTypes);
	// Turn into an array  
	const TArray<TPair<FName, TObjectPtr<ULandscapeGrassType>>> SortedNamedGrassTypes = ConvertToNamedGrassTypesList(ValidNamedGrassTypes);
	// Then sort by grass name for hash stability
	Algo::SortBy(SortedNamedGrassTypes, &TPair<FName, TObjectPtr<ULandscapeGrassType>>::Key, FNameLexicalLess());

	// If anything changes in the grass types, that affects the grass instances
	for (const TPair<FName, TObjectPtr<ULandscapeGrassType>>& ItPair : SortedNamedGrassTypes)
	{
		check(!ItPair.Key.IsNone() && (ItPair.Value != nullptr));

		Hash = FCrc::TypeCrc32(GetTypeHash(ItPair.Key.ToString().ToLower()), Hash);
		Hash = FCrc::TypeCrc32(ItPair.Value->StateHash, Hash);
	}
