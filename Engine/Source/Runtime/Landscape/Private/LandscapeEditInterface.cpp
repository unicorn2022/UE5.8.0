											   ...
											   bool InUpdateBounds, bool InUpdateCollision, bool InGenerateMips)
{
	...
	for (int32 ComponentIndexY = ComponentIndexY1; ComponentIndexY <= ComponentIndexY2; ComponentIndexY++)
	{
		for (int32 ComponentIndexX = ComponentIndexX1; ComponentIndexX <= ComponentIndexX2; ComponentIndexX++)
		{
			...
			FColor* HeightmapTextureData = (FColor*)TexDataInfo->GetMipData(/*InMipIndex = */0);
...
void FLandscapeEditDataInterface::GetHeightDataTemplFast(const int32 X1, const int32 Y1, const int32 X2, const int32 Y2, TStoreData& StoreData, UTexture2D* InHeightmap, TStoreData* NormalData /*= nullptr*/)
{
	...
	for( int32 ComponentIndexY=ComponentIndexY1;ComponentIndexY<=ComponentIndexY2;ComponentIndexY++ )
	{
		...
		{		
			...
			FLandscapeTextureDataInfo* TexDataInfo = nullptr;
			FColor* HeightmapTextureData = nullptr;
			TexDataInfo = GetTextureDataInfo(Heightmap);
			HeightmapTextureData = (FColor*)TexDataInfo->GetMipData(/*InMipIndex = */0);
...
const FColor& FLandscapeEditDataInterface::GetHeightMapColor(const ULandscapeComponent* Component, int32 TexU, int32 TexV, FColor* TextureData)
{
	...
	{
		FLandscapeTextureDataInfo* TexDataInfo = GetTextureDataInfo(Heightmap);
		TextureData = (FColor*)TexDataInfo->GetMipData(/*InMipIndex = */0);	
	}
	...
	const FColor& TexData = TextureData[ TexX + TexY * SizeU ];
	return TexData;
}

uint16 FLandscapeEditDataInterface::GetHeightMapData(const ULandscapeComponent* Component, int32 TexU, int32 TexV, FColor* TextureData /*= nullptr*/)
{
	const FColor& TexData = GetHeightMapColor(Component, TexU, TexV, TextureData);
	return ((((uint16)TexData.R) << 8) | TexData.G);
}

uint16 FLandscapeEditDataInterface::GetHeightMapAlphaBlendData(const ULandscapeComponent* Component, int32 TexU, int32 TexV, FColor* TextureData /*= nullptr*/)
{
	const FColor& TexData = GetHeightMapColor(Component, TexU, TexV, TextureData);
	return ((((uint16)TexData.B) << 8) | TexData.A) & 0xFFFC;
}

uint8 FLandscapeEditDataInterface::GetHeightMapFlagsData(const ULandscapeComponent* Component, int32 TexU, int32 TexV, FColor* TextureData /*= nullptr*/)
{
	const FColor& TexData = GetHeightMapColor(Component, TexU, TexV, TextureData);
...
void FLandscapeEditDataInterface::GetHeightDataInternal(int32& ValidX1, int32& ValidY1, int32& ValidX2, int32& ValidY2, TDataAccess& StoreData, TGetHeightMapDataFunction GetHeightMapDataFunction)
{
	...
	for( int32 ComponentIndexY=ComponentIndexY1;ComponentIndexY<=ComponentIndexY2;ComponentIndexY++ )
	{
		NoBorderX1 = false;
		NoBorderX2 = false;
		BorderComponent[0] = BorderComponent[1] = nullptr;
		for( int32 ComponentIndexX=ComponentIndexX1;ComponentIndexX<=ComponentIndexX2;ComponentIndexX++ )
		{
			BorderComponent[2] = BorderComponent[3] = nullptr;
			int32 ComponentIndexXY = ComponentSizeX*(ComponentIndexY-ComponentIndexY1) + ComponentIndexX-ComponentIndexX1;
			int32 ComponentIndexXX = ComponentIndexX - ComponentIndexX1;
			int32 ComponentIndexYY = ComponentIndexY - ComponentIndexY1;
			...
			else
			{
				...
				if ( bShouldSearchX || (!NoBorderX1 && !BorderComponent[0]))
				{
					...
					for (int32 X = ComponentIndexX-1; X >= ComponentIndexX1; X--)
					{
						...
						if (BorderComponent[0])
						{
							...
							NeighborHeightmapTextureData[0] = (FColor*)NeighborTexDataInfo[0]->GetMipData(/*InMipIndex = */0);
							break;
						}
					}
				}
				...
				if ( bShouldSearchX || (!NoBorderX2 && !BorderComponent[1]))
				{
					...
					for (int32 X = ComponentIndexX+1; X <= ComponentIndexX2; X++)
					{
						...
						if (BorderComponent[1])
						{
							...
							NeighborHeightmapTextureData[1] = (FColor*)NeighborTexDataInfo[1]->GetMipData(/*InMipIndex = */0);
							break;
						}
					}
				}
				...
				if ( bShouldSearchY || (!NoBorderY1[ComponentIndexXX] && !BorderComponentY1[ComponentIndexXX]))
				{
					...
					for (int32 Y = ComponentIndexY-1; Y >= ComponentIndexY1; Y--)
					{
						...
						if (BorderComponent[2])
						{
							...
							NeighborHeightmapTextureData[2] = (FColor*)NeighborTexDataInfo[2]->GetMipData(/*InMipIndex = */0);
							break;
						}
					}
				}
				...
				if ( bShouldSearchY || (!NoBorderY2[ComponentIndexXX] && !BorderComponentY2[ComponentIndexXX]))
				{
					...
					for (int32 Y = ComponentIndexY+1; Y <= ComponentIndexY2; Y++)
					{
						...
						if (BorderComponent[3])
						{
							...
							NeighborHeightmapTextureData[3] = (FColor*)NeighborTexDataInfo[3]->GetMipData(/*InMipIndex = */0);
							break;
						}
					}
				}
				...
				CornerComponent[0] = ComponentIndexX >= ComponentIndexX1 && ComponentIndexY >= ComponentIndexY1 ?
					LandscapeInfo->XYtoComponentMap.FindRef(FIntPoint((ComponentIndexX-1),(ComponentIndexY-1))) : nullptr;
...
void FLandscapeEditDataInterface::GetHeightDataFast(const int32 X1, const int32 Y1, const int32 X2, const int32 Y2, uint16* Data, int32 Stride, uint16* NormalData /*= nullptr*/, UTexture2D* InHeightmap)
{
	if( Stride==0 )
	{
		Stride = (1+X2-X1);
...
void FLandscapeEditDataInterface::GetHeightDataFast(const int32 X1, const int32 Y1, const int32 X2, const int32 Y2, TMap<FIntPoint, uint16>& SparseData, TMap<FIntPoint, uint16>* NormalData /*= nullptr*/, UTexture2D* InHeightmap)
{
	TSparseDataAccess<uint16> SparseStoreData(SparseData);
	if (NormalData)
	{
...
void ULandscapeComponent::FillLayer(ULandscapeLayerInfoObject* LayerInfo, FLandscapeEditDataInterface& LandscapeEdit)
{
	...
	// fill the layer
	{
		...
		uint8* MaterialLayerData = (uint8*)LandscapeEdit.GetTextureDataInfo(ComponentWeightmapTextures[FillLayerAllocation.WeightmapTextureIndex])->GetMipData(/*InMipIndex = */0);
...
void ULandscapeComponent::ReplaceLayerInternal(ULandscapeLayerInfoObject* FromLayerInfo, ULandscapeLayerInfoObject* ToLayerInfo, FLandscapeEditDataInterface& LandscapeEdit, const FGuid& InEditLayerGUID)
{
	...
	if (bMerging)
	{
		...
		uint8* const SrcTextureData = (uint8*)FromTexDataInfo->GetMipData(/*InMipIndex = */0) + ChannelOffsets[FromLayerAllocation.WeightmapTextureChannel];
		uint8* const DestTextureData = (uint8*)ToTexDataInfo->GetMipData(/*InMipIndex = */0) + ChannelOffsets[ToLayerAllocation.WeightmapTextureChannel];
...
void FLandscapeEditDataInterface::SetAlphaData(ULandscapeLayerInfoObject* const LayerInfo, const int32 X1, const int32 Y1, const int32 X2, const int32 Y2, const uint8* Data, int32 Stride, ELandscapeLayerPaintingRestriction PaintingRestriction /*= None*/)
{
	...
	for (int32 ComponentIndexY = ComponentIndexY1; ComponentIndexY <= ComponentIndexY2; ComponentIndexY++)
	{
		for (int32 ComponentIndexX = ComponentIndexX1; ComponentIndexX <= ComponentIndexX2; ComponentIndexX++)
		{
			...
			// if nullptr, there is no component at this location
			if (Component == nullptr)
			{
				continue;
			}
			...
			for (int32 LayerIdx = 0; LayerIdx < ComponentWeightmapLayerAllocations.Num(); LayerIdx++)
			{
				...
				{
					LayerDataPtrs[LayerIdx] = (uint8*)TexDataInfos[Allocation.WeightmapTextureIndex]->GetMipData(/*InMipIndex = */0) + ChannelOffsets[Allocation.WeightmapTextureChannel];
			...
			for (int32 SubIndexY = SubIndexY1; SubIndexY <= SubIndexY2; SubIndexY++)
			{
				for (int32 SubIndexX = SubIndexX1; SubIndexX <= SubIndexX2; SubIndexX++)
				{
					...
					for (int32 WeightmapIdx = 0; WeightmapIdx < ComponentWeightmapTextures.Num(); WeightmapIdx++)
					{
						if (TexDataInfos[WeightmapIdx] != nullptr)
						{
	...
	for (int32 ComponentIndexY = ComponentIndexY1; ComponentIndexY <= ComponentIndexY2; ComponentIndexY++)
	{
		for (int32 ComponentIndexX = ComponentIndexX1; ComponentIndexX <= ComponentIndexX2; ComponentIndexX++)
		{
			...
			for (int32 LayerIdx = 0; LayerIdx < ComponentWeightmapLayerAllocations.Num(); LayerIdx++)
			{
				...
				LayerDataInfos[LayerIdx].TexDataPtr = (uint8*)TexDataInfos[Allocation.WeightmapTextureIndex]->GetMipData(/*InMipIndex = */0) + ChannelOffsets[Allocation.WeightmapTextureChannel];
				LayerEditDataAllZero[LayerIdx] = true;
			}
					...
					for (int32 SubY = SubY1; SubY <= SubY2; SubY++)
					{
						for (int32 SubX = SubX1; SubX <= SubX2; SubX++)
						{
							...
							for (int32 LayerIdx = 0; LayerIdx < ComponentWeightmapLayerAllocations.Num(); LayerIdx++)
							{
								...
								if (LayerDataInfos[LayerIdx].TexDataPtr != nullptr)
								{
...
void FLandscapeEditDataInterface::GetWeightDataTemplFast(ULandscapeLayerInfoObject* LayerInfo, const int32 X1, const int32 Y1, const int32 X2, const int32 Y2, TStoreData& StoreData)
{
	...
	for( int32 ComponentIndexY=ComponentIndexY1;ComponentIndexY<=ComponentIndexY2;ComponentIndexY++ )
	{
		...
		{		
			...
			UTexture2D* WeightmapTexture = nullptr;
			FLandscapeTextureDataInfo* TexDataInfo = nullptr;
			uint8* WeightmapTextureData = nullptr;
			...
			if (LayerInfo != nullptr)
			{
				for( int32 LayerIdx=0;LayerIdx<ComponentWeightmapLayerAllocations.Num();LayerIdx++ )
				{
					if(ComponentWeightmapLayerAllocations[LayerIdx].LayerInfo == LayerInfo )
					{
						...
						WeightmapTextureData = (uint8*)TexDataInfo->GetMipData(/*InMipIndex = */0);
						...
						for( int32 SubX=SubX1;SubX<=SubX2;SubX++ )
						{
							...
							else // Whole weight map case...
							{
								...
								for( int32 LayerIdx=0;LayerIdx<ComponentWeightmapLayerAllocations.Num();LayerIdx++ )
								{
									...
									uint8* ComponentWeightmapTextureData = (uint8*)TexDataInfos[Idx]->GetMipData(/*InMipIndex = */0);
...
uint8 FLandscapeEditDataInterface::GetWeightMapData(const ULandscapeComponent* Component, ULandscapeLayerInfoObject* LayerInfo, int32 TexU, int32 TexV, uint8 Offset /*= 0*/, UTexture2D* Texture /*= nullptr*/, uint8* TextureData /*= nullptr*/)
{
	check(Component);
		...
		if (LayerInfo != nullptr)
		{
			...
			for( int32 LayerIdx=0;LayerIdx<ComponentWeightmapLayerAllocations.Num();LayerIdx++ )
			{
				if(ComponentWeightmapLayerAllocations[LayerIdx].LayerInfo == LayerInfo )
				{
					...
					TextureData = (uint8*)TexDataInfo->GetMipData(/*InMipIndex = */0);
							...
							if (LayerInfo != nullptr)
							{
								...
								for( int32 LayerIdx=0;LayerIdx<BorderWeightmapLayerAllocations.Num();LayerIdx++ )
								{
									if(BorderWeightmapLayerAllocations[LayerIdx].LayerInfo == LayerInfo )
									{
										...
										NeighborWeightmapTextureData[0] = (uint8*)NeighborTexDataInfo[0]->GetMipData(/*InMipIndex = */0);
							...
							if (LayerInfo != nullptr)
							{
								...
								for( int32 LayerIdx=0;LayerIdx<BorderWeightmapLayerAllocations.Num();LayerIdx++ )
								{
									if(BorderWeightmapLayerAllocations[LayerIdx].LayerInfo == LayerInfo )
									{
										...
										NeighborWeightmapTextureData[1] = (uint8*)NeighborTexDataInfo[1]->GetMipData(/*InMipIndex = */0);
							...
							if (LayerInfo != nullptr)
							{
								...
								for( int32 LayerIdx=0;LayerIdx<BorderWeightmapLayerAllocations.Num();LayerIdx++ )
								{
									if(BorderWeightmapLayerAllocations[LayerIdx].LayerInfo == LayerInfo )
									{
										...
										NeighborWeightmapTextureData[2] = (uint8*)NeighborTexDataInfo[2]->GetMipData(/*InMipIndex = */0);
							...
							if (LayerInfo != nullptr)
							{
								...
								for( int32 LayerIdx=0;LayerIdx<BorderWeightmapLayerAllocations.Num();LayerIdx++ )
								{
									if(BorderWeightmapLayerAllocations[LayerIdx].LayerInfo == LayerInfo )
									{
										...
										NeighborWeightmapTextureData[3] = (uint8*)NeighborTexDataInfo[3]->GetMipData(/*InMipIndex = */0);
...
void FLandscapeTextureDataInterface::CopyTextureChannel(UTexture2D* Dest, int32 DestChannel, UTexture2D* Src, int32 SrcChannel)
{
	...
	if (SrcMips != NumMips)
	{
		...
		if (SrcMips < NumMips)
		{
			UE_LOGF(LogLandscape, Warning, "Unexpected mip count mismatch when copying landscape texture channels from '%ls' (%d mips) to '%ls' (%d mips) -- mip data may be lost, or left uninitialized",
...
bool FLandscapeTextureDataInterface::EqualTextureValueTempl(UTexture2D* Src, TData Value)
{
	...
	TData* DestTextureData = (TData*)DestDataInfo->GetMipData(/*InMipIndex = */0);
	...
	for (int32 ComponentIndexY = ComponentIndexY1; ComponentIndexY <= ComponentIndexY2; ComponentIndexY++)
	{
		for (int32 ComponentIndexX = ComponentIndexX1; ComponentIndexX <= ComponentIndexX2; ComponentIndexX++)
		{
			...
			if (EditToolTexture)
			{
				...
				SelectTextureData = (uint8*)TexDataInfo->GetMipData(/*InMipIndex = */0);
			}
...
void FLandscapeEditDataInterface::SetEditToolTextureData(int32 X1, int32 Y1, int32 X2, int32 Y2, const uint8* Data, int32 Stride, TFunctionRef<UTexture2D*&(ULandscapeComponent*)> GetComponentTexture, TextureGroup InTextureGroup)
{
	...
	for( int32 ComponentIndexY=ComponentIndexY1;ComponentIndexY<=ComponentIndexY2;ComponentIndexY++ )
	{
		...
		{	
			...
			uint8* SelectTextureData = (uint8*)TexDataInfo->GetMipData(/*InMipIndex = */0);
...
// Deprecated
FVector2D FLandscapeEditDataInterface::GetXYOffsetmapData(const ULandscapeComponent* Component, int32 TexU, int32 TexV, FColor* TextureData/* = nullptr*/)
{
	return FVector2D();
}
...
{
	MipInfo.AddDefaulted(Texture->Source.GetNumMips());
	Texture->SetFlags(RF_Transactional);
	Texture->TemporarilyDisableStreaming();

	// Call BlockOnAnyAsyncBuild here to make sure that Modify cannot cancel the potentially on-going async cache
	//  The better fix would be not to call Modify when we don't intend to actually modify the texture (e.g. when FLandscapeTextureDataInfo is used for reading the data only)
	//  but in practice, that would require rebuilding the entire TLandscapeEditCache system, that is used both as a read-only and/ read-write facility
	Texture->BlockOnAnyAsyncBuild();
	Texture->Modify(bShouldDirtyPackage);
	check(!Texture->IsDefaultTexture()); // If we're the default texture, then Modify likely has cancelled the async cache, which would be a disaster if nothing requests a new cache afterwards
}

void FLandscapeTextureDataInfo::AddMipUpdateRegion(int32 InMipIndex, int32 InX1, int32 InY1, int32 InX2, int32 InY2)
{
	if (MipInfo[InMipIndex].bFull)
	{
		return;
	}

	check(InMipIndex < MipInfo.Num());
	...
	if (Width == GetMipSizeX(InMipIndex) && Height == GetMipSizeY(InMipIndex))
	{
		MipInfo[InMipIndex].bFull = true;
		MipInfo[InMipIndex].MipUpdateRegions.Reset();
		// Push a full region for UpdateTextureData() to process later
		MipInfo[InMipIndex].MipUpdateRegions.Emplace(0, 0, 0, 0, Width, Height);
		return;
	}

	MipInfo[InMipIndex].MipUpdateRegions.Emplace(InX1, InY1, InX1, InY1, Width, Height);
}

void* FLandscapeTextureDataInfo::GetMipData(int32 InMipIndex)
{
	check(InMipIndex < MipInfo.Num());

	if (!MipInfo[InMipIndex].MipData)
	{
		// will Unlock in destructor
		MipInfo[InMipIndex].MipData = Texture->Source.LockMip(InMipIndex);
		// probably should check that we got the lock :
		//check( MipInfo[InMipIndex].MipData != nullptr );
	}
	return MipInfo[InMipIndex].MipData;
}
...
bool FLandscapeTextureDataInfo::UpdateTextureData()
{
	...
	for (int32 MipIndex = 0; MipIndex < MipInfo.Num(); MipIndex++)
	{
		if (MipInfo[MipIndex].MipData && MipInfo[MipIndex].MipUpdateRegions.Num() > 0)
		{
			if (bCompressed)
			{
				bNeedToWaitForUpdate = true;
			...
			const uint32 SrcSizeX = (Texture->Source.GetSizeX()) >> MipIndex;
			const uint32 SrcSizeY = (Texture->Source.GetSizeY()) >> MipIndex;
			...
			CopyMipInfo->MipUpdateRegions = MipInfo[MipIndex].MipUpdateRegions;
			CopyMipInfo->MipData = FMemory::Malloc(BufferSize);
			FMemory::Memcpy(CopyMipInfo->MipData, MipInfo[MipIndex].MipData, BufferSize);

			Texture->UpdateTextureRegions(MipIndex, CopyMipInfo->MipUpdateRegions.Num(), &CopyMipInfo->MipUpdateRegions[0], SrcPitch, DataSize, (uint8*)CopyMipInfo->MipData,
...
FLandscapeTextureDataInfo::~FLandscapeTextureDataInfo()
{
	...
	for( int32 MipIndex = 0; MipIndex < MipInfo.Num(); MipIndex++ )
	{
		if(MipInfo[MipIndex].MipData != nullptr)
		{
			Texture->Source.UnlockMip(MipIndex);
			MipInfo[MipIndex].MipData = nullptr;
		}
	}
