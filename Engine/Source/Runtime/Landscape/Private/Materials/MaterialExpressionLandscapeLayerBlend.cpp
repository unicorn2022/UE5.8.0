...
int32 UMaterialExpressionLandscapeLayerBlend::Compile(class FMaterialCompiler* Compiler, int32 OutputIndex)
{
	...
	{
		// Ensure all MA properties have proper default values compiled
		if (IsResultMaterialAttributes(OutputIndex) && Property != MP_MAX)
		{
			OutputCode = FMaterialAttributeDefinitionMap::CompileDefaultExpression(Compiler, Compiler->GetMaterialAttribute());
		}
		else
		{
...
{
	OutCaptions.Add(FString("Landscape Layer Blend"));
}
