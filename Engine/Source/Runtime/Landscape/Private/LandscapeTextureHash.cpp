...
uint64 ULandscapeTextureHash::CalculateTextureHash64(UTexture2D* LandscapeTexture, ELandscapeTextureType TextureType, bool& bOutHashIsValid)
{
	...
	{
		UE_LOGF(LogLandscape, Warning, "Could not calculate the texture hash for landscape texture '%ls' because the texture could not be locked (it may be corrupt or being modified incorrectly)", *LandscapeTexture->GetName());
		Hash = 0;
...
uint64 ULandscapeTextureHash::CalculateTextureHash64(const FColor* Mip0Data, int32 PixelCount, ELandscapeTextureType TextureType)
{
	...
	switch (TextureType)
	{
		...
		case ELandscapeTextureType::Weightmap:
			...
			break;
		default:
			checkf(false, TEXT("TextureType is invalid: %d"), EnumToUnderlyingType(TextureType));
			break;
	}
