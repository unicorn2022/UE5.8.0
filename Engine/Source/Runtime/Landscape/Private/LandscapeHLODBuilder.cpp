...
bool ULandscapeHLODBuilder::ComputeHLODHash(FHLODHashBuilder& HashBuilder, const UActorComponent* InSourceComponent) const
{
	...
	// Base landscape HLOD key, changing this will force a rebuild of all landscape HLODs
	FString HLODBaseKey = "51EDA4343E26409DA6ABEACE14C138D1";
	HashBuilder.HashField(HLODBaseKey, TEXT("LandscapeHLODBuilderBaseKey"));

	const ALandscapeProxy* LSProxy = LSComponent->GetLandscapeProxy();
	...
	// LS Materials
	{
		if (LSProxy->HLODMaterialOverride)
		{
			HashBuilder << LSProxy->HLODMaterialOverride;
		}
		else
		{
		...
		HashBuilder << LSComponent->OverrideHoleMaterial;  //Hole material matters regardless of which base material is used.
	}
...
static UMaterialInterface* BakeLandscapeMaterial(const FHLODBuildContext& InHLODBuildContext, const FMeshDescription& InMeshDescription, const ALandscapeProxy* InLandscapeProxy, int32 InTextureSize)
{
	...
	{
		UE_LOGF(LogHLODBuilder, Error, "Landscape flatten material %ls should use world space normals rather than tangent space normals.", *LandscapeMaterial->GetName());
	}
	...
	Algo::ForEach(GeneratedAssets, [&InHLODBuildContext](UObject* Asset) 
	{
		// We don't want any of the generate HLOD assets to be public, unless required by HLODBuildContext
		Asset->ClearFlags(RF_Public | RF_Standalone);
		Asset->SetFlags(InHLODBuildContext.AssetsObjectFlags);
...
TArray<UActorComponent*> ULandscapeHLODBuilder::Build(const FHLODBuildContext& InHLODBuildContext, const TArray<UActorComponent*>& InSourceComponents) const
{
	...
	for (ALandscapeProxy* LandscapeProxy : LandscapeProxies)
	{
		...
		// Mesh
		{
			...
			{
				UE_LOGF(LogHLODBuilder, Display, "Skipping HLOD builder for landscape proxy '%ls' as it failed to export a mesh!", *LandscapeProxy->GetFullName());
				continue;
			}
