...
/** Data for an extruded tapered capsule primitive. */
USTRUCT()
struct FClothCollisionPrim_ExtrudedTaperedCapsule
{
	GENERATED_BODY()

	explicit FClothCollisionPrim_ExtrudedTaperedCapsule(
		const FVector& InLocalPosition = FVector::ZeroVector,
		const FQuat& InLocalRotation = FQuat::Identity,
		float InLength = 0.f,
		float InWidth = 0.f,
	...
	bool operator==(const FClothCollisionPrim_ExtrudedTaperedCapsule& Other) const
	{
		return
			BoneIndex == Other.BoneIndex &&
			LocalPosition.Equals(Other.LocalPosition) &&
