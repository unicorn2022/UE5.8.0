...
void FClothCollisionData::Append(const FClothCollisionData& InOther)
{
	...
	ExtrudedTaperedCapsules.Append(InOther.ExtrudedTaperedCapsules);
}
...
void FClothCollisionData::AppendTransformed(const FClothCollisionData& InOther, const TArray<FTransform>& BoneTransforms)
{
	...
	const int32 NumExtrudedCapsulesBefore = ExtrudedTaperedCapsules.Num();
	...
	for (int32 Index = NumConvexesBefore; Index < Convexes.Num(); ++Index)
	{
		...
		Convexes[Index].LocalPosition = Transform.TransformPosition(Convexes[Index].LocalPosition);
		Convexes[Index].LocalRotation = Transform.TransformRotation(Convexes[Index].LocalRotation);
	}
	...
	for (int32 Index = NumExtrudedCapsulesBefore; Index < ExtrudedTaperedCapsules.Num(); ++Index)
	{
		const FTransform& Transform = BoneTransforms[ExtrudedTaperedCapsules[Index].BoneIndex];
		ExtrudedTaperedCapsules[Index].LocalPosition = Transform.TransformPosition(ExtrudedTaperedCapsules[Index].LocalPosition);
		ExtrudedTaperedCapsules[Index].LocalRotation = Transform.TransformRotation(ExtrudedTaperedCapsules[Index].LocalRotation);
	}
}
...
void FClothCollisionData::AppendUnique(const FClothCollisionData& InOther)
{
	...
	ExtrudedTaperedCapsules.Reserve(ExtrudedTaperedCapsules.Num() + InOther.ExtrudedTaperedCapsules.Num());
	for (const FClothCollisionPrim_ExtrudedTaperedCapsule& Capsule : InOther.ExtrudedTaperedCapsules)
	{
		ExtrudedTaperedCapsules.AddUnique(Capsule);
	}
}
