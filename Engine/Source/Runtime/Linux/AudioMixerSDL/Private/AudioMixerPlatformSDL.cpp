...
namespace Audio
{
	...
	bool FMixerPlatformSDL::InitializeHardware()
	{
		if (bInitialized)
		{
			UE_LOGF(LogAudioMixerSDL, Error, "SDL Audio already initialized.");
			return false;
		}
		...
		{
			UE_LOGF(LogAudioMixerSDL, Log, "Hinting SDL to use 'dummy' audio driver.");
		...
		if (Result < 0)
		{
			UE_LOGF(LogAudioMixerSDL, Error, "SDL_InitSubSystem create failed: %d", Result);
			return false;
		}

		const char* DriverName = SDL_GetCurrentAudioDriver();
		UE_LOGF(LogAudioMixerSDL, Display, "Initialized SDL using %ls platform API backend.", ANSI_TO_TCHAR(DriverName));
	...
	bool FMixerPlatformSDL::GetNumOutputDevices(uint32& OutNumOutputDevices)
	{
		if (!bInitialized)
		{
			UE_LOGF(LogAudioMixerSDL, Error, "SDL3 Audio is not initialized.");
			return false;
		}
	...
	bool FMixerPlatformSDL::OpenAudioStream(const FAudioMixerOpenStreamParams& Params)
	{
		...
		{		
			...
			{
				UE_LOGF(LogAudioMixerSDL, Log, "Opening %ls audio device (device index %d)", DeviceName ? ANSI_TO_TCHAR(DeviceName) : TEXT("default"), OpenStreamParams.OutputDeviceIndex);
			}
			else
			{
				UE_LOGF(LogAudioMixerSDL, Log, "Opening overridden '%ls' audio device (device index %d)", *CurrentDeviceNameString, OpenStreamParams.OutputDeviceIndex);
			}
			...
			if (!AudioStream)
			{
				...
				UE_LOGF(LogAudioMixerSDL, Error, "%ls", ANSI_TO_TCHAR(ErrorText));
				return false;
			}
	...
	{
		static FString DefaultName("Default SDL Audio Device.");
		return DefaultName;
	}
	...
	void FMixerPlatformSDL::ResumeContext()
	{
		if (bSuspended)
		{
			...
			{
				SDL_ResumeAudioStreamDevice(AudioStream);
				UE_LOGF(LogAudioMixerSDL, Display, "Resuming Audio");
				bSuspended = false;
	...
	void FMixerPlatformSDL::SuspendContext()
	{
		if (!bSuspended)
		{
			...
			{
				SDL_PauseAudioStreamDevice(AudioStream);
				UE_LOGF(LogAudioMixerSDL, Display, "Suspending Audio");
				bSuspended = true;
			}
		}
	}
