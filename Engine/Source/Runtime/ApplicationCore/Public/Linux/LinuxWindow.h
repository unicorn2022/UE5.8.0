...
private:
	...
	/**
	 * Converts screen-absolute coordinates to parent-relative coordinates for popup menu
	 * and tooltip windows. Walks the parent chain to accumulate offsets. For windows that
	 * are not parent-relative, X and Y are left unmodified.
	 *
	 * @param ThisWindow	The window whose position is being converted.
	 * @param X				In/out X coordinate to convert.
	 * @param Y				In/out Y coordinate to convert.
	 */
	static void ConvertToParentRelativePosition(SDL_Window* ThisWindow, int32& X, int32& Y);

	/**
	 * Gets the offset from screen-absolute to parent-relative coordinates during window
	 * creation. If Props indicates the window is not parent-relative, X and Y are set to 0.
	 * Use ConvertToParentRelativePosition() if the window already exists.
	 *
	 * @param Props			SDL properties for the window being created.
	 * @param StartParent	The parent window for the window being created.
	 * @param X				Out: X offset to apply.
	 * @param Y				Out: Y offset to apply.
	 */
	static void GetParentRelativeOffset(SDL_PropertiesID Props, SDL_Window* StartParent, int32& X, int32& Y);
