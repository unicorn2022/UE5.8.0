...
public:
	...
	APPLICATIONCORE_API virtual bool RequiresPerWindowTooltips() const override;
...
private:
	...
	uint32 SDLDoubleClickTime;
	uint32 SDLDoubleClickRadius;

	const uint32 GetSDLDoubleClickTime();
	const uint32 GetSDLDoubleClickRadius();
	...
	/** Used to detect a double click on the title bar area of the window */
	float LastTitleBarClickX;
	float LastTitleBarClickY;
	uint64 LastTitleBarClickTimestampNS;
	TSharedPtr< FLinuxWindow > LastTitleBarClickWindow;

	/**
	 * Timestamp (ns) of the last SDL_EVENT_WINDOW_HIT_TEST. Used to coalesce
	 * the duplicate HIT_TEST events that SDL3 X11 XInput2 emits for every
	 * Button1 press
	 */
	uint64 LastHitTestTimestampNS;
	...
	struct FCachedDisplay
	{
		SDL_DisplayID	Id;
		SDL_Rect	UsableBounds;
	};

	// SDL caches full display bounds in-memory but re-fetches _NET_WORKAREA per call for usable bounds.
	mutable TArray<FCachedDisplay>	CachedDisplays;
