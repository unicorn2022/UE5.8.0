...
public:
	...
	void DismissAllMenusOnGameThread();
	...
	void StartScopedModalEvent(const FCoreDelegates::FModalWindowContext& Context);

	void EndScopedModalEvent(const FCoreDelegates::FModalWindowContext& Context);
#endif
...
public:
	...
	/** Restore mouse input state after macOS state transitions (sleep, minimize, app switch)
	 *  that may silently reset CGAssociate or invalidate GCMouse handlers. */
	void ReassociateMouseInput();
...
private:
	...
	TSharedPtr<FAppleMouseController> MouseController;
	
	void HandleMouseControllerMovement(FIntVector2 Delta);
	...
	std::atomic<bool> bMouseInputNeedsReassociation;
