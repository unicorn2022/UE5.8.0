...
public:
	...
	*/
	...
	bool IsInModalSizeLoop() const { return bInModalSizeLoop; }
	void EnterModalSizeLoop() { bInModalSizeLoop = true; }
	void ExitModalSizeLoop() { bInModalSizeLoop = false; }
...
public:
	...
	APPLICATIONCORE_API virtual bool SupportsDynamicCursorLock() const override;
...
private:
	...
	/**
	 * Creates an HRGN for the window's current region.  Remember to delete the returned handle when you're done with it using ::DeleteObject, unless
     * you're passing it to SetWindowRgn(), which will absorb the reference itself.
	 *
	 * @param bForQuery Bypasses situational behavior compensation, so that the same compensation does not also need to be applied to the queried
	 *     location.
	 */
	APPLICATIONCORE_API HRGN MakeWindowRegionObject(bool bForQuery) const;

	APPLICATIONCORE_API void DisableTouchFeedback();

	int32 GetWin32BorderSize() const;
...
private:
	...
	/** when true the window in the middle of a windows modal size loop */
