...
enum class EAudioFeature : uint8
{
	...
	AllowHapticsAndSystemSoundsDuringRecording, // allow haptic feedback and system sounds when Record or PlayAndRecord category is active
...
// Shared lifecycle handlers. UIApplicationDelegate (legacy) and UIWindowSceneDelegate (TN3187)
// both forward into these so the engine sees identical Suspend/Resume/Foreground/Background flow
// regardless of which lifecycle is selected at compile time.
