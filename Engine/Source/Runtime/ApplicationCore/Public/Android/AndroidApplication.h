...
public:	
	...
	/** Game thread only; the touch loop brackets each OnTouch* call with set / clear. */
	void SetCurrentlyTouchedWindow(TSharedPtr<FGenericWindow> InWindow);

	/** Returns the primary (first-created) Android window, or nullptr if none has been added yet. */
	TSharedPtr<FAndroidWindow> GetPrimaryWindow() const;
...
private:
	...
	/** See GetWindowUnderCursor. Game thread only. */
	TSharedPtr<FGenericWindow> CurrentlyTouchedWindow;
