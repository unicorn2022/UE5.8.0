...
struct TouchInput
{
	/** This is the Android kernel-provided "Device ID", NOT the Unreal "Device ID". */
	int32 DeviceId;
	int32 Handle;
	FVector2D LastPosition;
	FVector2D Position;
	/**
	 * When valid, the touch loop uses this directly instead of resolving DeviceId through InputDeviceInfoMap.
	 * See FAndroidInputInterface::RegisterSurfaceForTouch.
	 */
...
{
	FAndroidGamepadDeviceMapping(const int32 InVendorId, const int32 InProductId, const FName InDeviceName)
	{
		Init(InVendorId, InProductId, InDeviceName);
	}

	void Init(const int32 InVendorId, const int32 InProductId, const FName InDeviceName);
	...
	GameControllerMapping::DeviceConfig_WPtr DeviceConfig;
...
typedef GameControllerMapping::ControllerClassType ControllerClassType;
/**
...
public:
	...
	/**
	 * Register an Android Surface so that touches associated with it can be processed properly by QueueTouchInput.
	 * This is necessary when allocating any Surfaces other than the primary activity one that the engine owns, because Android reports touch device
	 * ID based on the physical hardware, but tracks finger indices uniquely per Surface.  Unreal expects finger indices to be unique per device.
	 *
	 * Caller is responsible for mapping the returned device to an FPlatformUserId, and for forwarding touch events to QueueTouchInput.
	 */
	static APPLICATIONCORE_API FInputDeviceId RegisterSurfaceForTouch(const TSharedRef<FAndroidWindow>& Window);

	/** Release a virtual touchscreen previously returned by RegisterSurfaceForTouch. */
	static APPLICATIONCORE_API void UnregisterSurface(FInputDeviceId DeviceId);
...
private:
	...
	/** Windows registered via RegisterSurfaceForTouch, looked up during touch dispatch. Guarded by TouchInputCriticalSection. */
	static TMap<FInputDeviceId, TWeakPtr<FGenericWindow>> InputMappedSurfaces;
	/**
	 * FInputDeviceIds previously returned by RegisterSurfaceForTouch and freed via UnregisterSurface, available for reuse.
	 * Bounds the FInputDeviceId allocation counter across repeated attach/detach cycles.  Guarded by TouchInputCriticalSection.
	 */
	static TArray<FInputDeviceId> FreeSurfaceInputDeviceIds;
