...
{
	virtual void Init() = 0;
	virtual ~IAndroidFramePacer() = default;
	virtual bool SwapBuffers(struct FPlatformOpenGLSurfaceContext* SurfaceContext, bool bLockToVsync) { return true; }
...
struct FAndroidOpenGLFramePacer : public IAndroidFramePacer
{
	...
	virtual bool SwapBuffers(struct FPlatformOpenGLSurfaceContext* SurfaceContext, bool bLockToVsync) override;
...
{
	virtual void Init() override;
	virtual ~FAndroidVulkanFramePacer() = default;
	virtual bool SupportsFramePace(int32 QueryFramePace) override;
private:
	bool SupportsFramePaceInternal(int32 QueryFramePace, int32& OutRefreshRate, int32& OutSyncInterval);
...
struct FAndroidPlatformRHIFramePacer : public FGenericPlatformRHIFramePacer
{
	...
	static void SwapBuffers(struct FPlatformOpenGLSurfaceContext* SurfaceContext, bool bLockToVsync) { if (ensure(FramePacer)) { FramePacer->SwapBuffers(SurfaceContext, bLockToVsync); } }
