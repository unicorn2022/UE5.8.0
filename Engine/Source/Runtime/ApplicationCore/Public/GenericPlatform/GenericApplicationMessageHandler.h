...
namespace EWindowZone
{
	 ...
	 * 
	 * Must remain synchronized with platform enum maps in FWindowsApplication::ProcessMessage and FLinuxWindow::HitTest.
	 */
	enum Type
	{
		NotInWindow			= 0,
		...
		// IsBorder below assumes these are contiguous in this order
		TopLeftBorder		= 2,
		TopBorder			= 3,
		TopRightBorder		= 4,
		LeftBorder			= 5,
	...
	inline bool IsBorder(Type Zone)
	{
		return Zone >= TopLeftBorder && Zone <= BottomRightBorder;
	}
}
...
class UE_DEPRECATED(5.8, "Use FInputDeviceRegistry::Get() instead") FInputDeviceScope;
 ...
 *
 * @deprecated FInputDeviceScope is being superseded by FInputDeviceRegistry, which provides the same
 * hardware metadata in a thread-safe manner keyed on FInputDeviceId. All call sites should migrate to
 * FInputDeviceRegistry::FindDescriptor(DeviceId) and FInputDeviceScope will be removed once the
...
public:
	...
	virtual bool OnNativeGestureBegin(EGestureEvent GestureType, const FVector2D& Delta, float Value, bool bIsDirectionInvertedFromDevice, FPlatformUserId PlatformUserId, FInputDeviceId DeviceId)
	{
		return false;
	}

	virtual bool OnNativeGestureUpdate(EGestureEvent GestureType, const FVector2D& Delta, float Value, bool bIsDirectionInvertedFromDevice, FPlatformUserId PlatformUserId, FInputDeviceId DeviceId)
	{
		return false;
	}

	virtual bool OnNativeGestureEnd(EGestureEvent GestureType, const FVector2D& Delta, float Value, bool bIsDirectionInvertedFromDevice, FPlatformUserId PlatformUserId, FInputDeviceId DeviceId)
	{
		return false;
	}
	...
	virtual void OnRegularWindowMovedOrResized(const TSharedRef< FGenericWindow >& Window)
	{
