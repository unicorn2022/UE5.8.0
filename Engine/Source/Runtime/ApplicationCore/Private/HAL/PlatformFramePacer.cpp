	...
	FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& Args)
{
	if (Args.Num() > 0)
	{
		...
		UE_LOGF(LogFramePacer, Display, "r.FramePace : requesting %d, set as %d", RequestFramePace, ResultFramePace);
	}
	else
	{
		UE_LOGF(LogFramePacer, Display, "r.FramePace : current frame pace is %d", FPlatformRHIFramePacer::GetFramePace());
	}
}),
ECVF_Default);