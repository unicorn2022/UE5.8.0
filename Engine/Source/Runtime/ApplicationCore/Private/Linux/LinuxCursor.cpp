	...
	,	bPositionCacheIsValid(false)
{
	if (!FApp::CanEverRender())
	{
		...
		UE_LOGF(LogInit, Log, "Not creating cursor resources due to headless application.");
		return;
	}

	if (!FLinuxPlatformApplicationMisc::InitSDL()) //	will not initialize more than once
	{
		UE_LOGF(LogInit, Fatal, "FLinuxCursor::FLinuxCursor() : InitSDL() failed, cannot construct cursor.");
		// unreachable
		return;
	}
