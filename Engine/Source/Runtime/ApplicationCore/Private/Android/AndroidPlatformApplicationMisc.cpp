...
EScreenPhysicalAccuracy FAndroidApplicationMisc::ComputePhysicalScreenDimensions(FVector2f& OutScreenDimensions, float& OutScreenSize)
{
	...
	{
		UE_LOGF(LogAndroid, Warning, "ComputePhysicalScreenDimensions : Incorrect ScreenDensity or Unknown ScreenPhysicalAccuracy. ScreenDensity: %d, Accuracy: %d",
			ScreenDensity,
			EnumToUnderlyingType(Accuracy));
	...
	UE_LOGF(LogAndroid, Display, "ComputePhysicalScreenDimensions : NativeScreenResolution: %ls, ScreenDensity: %d, OutScreenDimensions (inches): %ls, OutScreenSize (inches): %f",
...
#if USE_ANDROID_JNI
	extern FString AndroidThunkCpp_GetMetaDataString(const FString& Key);
	FString DisplayString = AndroidThunkCpp_GetMetaDataString(FString("unreal.display"));
