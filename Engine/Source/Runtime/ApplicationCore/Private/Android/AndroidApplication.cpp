...
TSharedPtr<FGenericWindow> FAndroidApplication::GetWindowUnderCursor()
{
	return CurrentlyTouchedWindow;
}

void FAndroidApplication::SetCurrentlyTouchedWindow(TSharedPtr<FGenericWindow> InWindow)
{
	CurrentlyTouchedWindow = MoveTemp(InWindow);
}

TSharedPtr<FAndroidWindow> FAndroidApplication::GetPrimaryWindow() const
{
	return Windows.Num() > 0 ? TSharedPtr<FAndroidWindow>(Windows[0]) : TSharedPtr<FAndroidWindow>();
}
