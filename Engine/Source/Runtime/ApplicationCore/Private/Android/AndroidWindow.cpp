...
TSharedRef<FAndroidWindow> FAndroidWindow::Make()
{
	...
	if( Num == 1)  // special case for native activity, the first window gets any pre-existing ANative window. if there is not yet a pending window then the event thread will set it later.
	{
		...
		MyANativeAccessor.SetANativeWindow(PendingAccessor.GetPreviousANativeWindow()); // first set previous handle, this is retained when the current handle is set next
...
#endif
	UE_LOGF(LogAndroid, Log, "nativeSetSafezoneInfo bIsPortrait=%d, left=%f, top=%f, right=%f, bottom=%f", bIsPortrait ? 1 : 0, left, top, right, bottom);
}
#endif
...
void FAndroidWindow::AcquireWindowRef(ANativeWindow* InWindow)
{
	...
	if (InWindow == nullptr)
	{
		UE_LOGF(LogAndroid, Log, "FAndroidWindow::AcquireWindowRef skipped because InWindow is null.");
		return;
	}
...
void CacheRect(bool bUseEventThreadWindow, const FAndroidCachedWindowRectParams& NewValues)
{
	...
	UE_LOGF(LogAndroid, Log, "***** Cached WindowRect %d, %d (%d)", NewValues.WindowWidth, NewValues.WindowHeight, (int32)bUseEventThreadWindow);
}
...
FPlatformRect FAndroidWindow::GetScreenRect(bool bUseEventThreadWindow)
{
	...
	UE_LOGF(LogAndroid, Fatal, "FAndroidWindow::CalculateSurfaceSize currently expects non-JNI platforms to override resolution");
	return FPlatformRect();
#else

	static const bool bIsXRImmersiveHMDApp = AndroidThunkCpp_IsXRImmersiveHMDApplication();
	...
	// If the app is for XR Mobile then always use 0 as ScaleFactor (to match window size).
	float RequestedContentScaleFactor = bIsXRImmersiveHMDApp ? 0.0f : CVarScale->GetFloat();
	...
	int32 RequestedResX = bIsXRImmersiveHMDApp ? 0 : CVarResX->GetInt();
	int32 RequestedResY = bIsXRImmersiveHMDApp ? 0 : CVarResY->GetInt();
	...
	CurrentParams.MobileResY = RequestedResY;
	...
	if (bComputeRect)
	{
		...
		UE_LOGF(LogAndroid, Log, "Mobile HDR: %ls", bMobileHDR ? TEXT("YES") : TEXT("no"));

		if (!bIsXRImmersiveHMDApp && (GAndroidWindowOverride == nullptr))
		{
...
void FAndroidWindow::CalculateSurfaceSize(int32_t& SurfaceWidth, int32_t& SurfaceHeight, bool bUseEventThreadWindow)
{
	...
	static const bool bIsXRImmersiveHMDApp = AndroidThunkCpp_IsXRImmersiveHMDApplication();
	const int DividableBy = bIsXRImmersiveHMDApp ? 1 : 8;
#endif
...
bool FAndroidWindow::OnWindowOrientationChanged(EDeviceScreenOrientation DeviceScreenOrientation)
{
	if (GDeviceScreenOrientation != DeviceScreenOrientation)
	{
		...
		UE_LOGF(LogAndroid, Log, "Window orientation changed: %ls, GDeviceScreenOrientation=%d", bIsPortrait ? TEXT("Portrait") : TEXT("Landscape"), EnumToUnderlyingType(GDeviceScreenOrientation));
...
static FAndroidDisplayInfo GetAndroidDisplayInfoFromDPITargets(int32 TargetDPI, int32 SceneMaxDesiredPixelCount, int32 LowerLimit3DDPI)
{
	...
	FString JNIMetrics = AndroidThunkCpp_GetMetaDataString(FString("unreal.displaymetrics.metrics"));
	FString JNIDisplay = AndroidThunkCpp_GetMetaDataString(FString("unreal.display"));

	TMap<FString, FString> metricsParams = StringToMap(JNIMetrics);
	TMap<FString, FString> displayParams = StringToMap(JNIDisplay);
	static const FString DensityDpiKey("densityDpi");
	static const FString xDpiKey("xdpi");
	static const FString yDpiKey("ydpi");
	static const FString WidthPixelsKey("realWidth");
	static const FString HeightPixelsKey("realHeight");
	...
	UE_LOGF(LogAndroid, Display, "AndroidDisplayInfoFromDPITargets : DPI info: DensityDPI %d, dpi %d, xdpi %d, ydpi %d", DensityDPI, avgdpi, xdpi, ydpi);
	...
	UE_LOGF(LogAndroid, Display, "AndroidDisplayInfoFromDPITargets : Native screen dpi %d, res %d x %d", NativeScreenDensityDPI, NativeScreenPixelDims.X, NativeScreenPixelDims.Y);
	...
	UE_CLOGF(TargetDPI > NativeScreenDensityDPI, LogAndroid, Display, "AndroidDisplayInfoFromDPITargets : TargetDPI too high, using native screen DPI %d, window dims %d, %d", NativeScreenDensityDPI, Info.WindowDims.X, Info.WindowDims.Y);
	...
	if (SceneMaxDesiredPixelCount && DesiredPixelCount > SceneMaxDesiredPixelCount)
	{
		...
		UE_LOGF(LogAndroid, Warning, "AndroidDisplayInfoFromDPITargets : DPI %d has a %d pixels, this exceeds the pixels limit of %d by %d%%. 3d scene target is reduced to %d x %d",
			TargetDPI,
			DesiredPixelCount, 
	...
	if (LowerLimit3DDPI && (int32)((float)TargetDPI * Info.SceneScaleFactor) < LowerLimit3DDPI)
	{
		...
		UE_LOGF(LogAndroid, Warning, "AndroidDisplayInfoFromDPITargets : 3d scene target of %d DPI is lower than specified limit of %d DPI, increasing scene target dims %dx%d -> %dx%d",
	...
	if (SceneMaxDesiredPixelCount)
	{
		...
		UE_LOGF(LogAndroid, Display, "AndroidDisplayInfoFromDPITargets : SceneTarget Pixel count %d%% of limit.", (uint32)(((float)FinalSceneTargetPixelCount / (float)SceneMaxDesiredPixelCount) * 100));
	}
