...
#endif
{
	...
	if (TSharedPtr<FAppleKeyboardController> KeyboardController = InputInterface->GetKeyboardController().Pin())
	{
		KeyboardController->OnAvailabilityChanged().AddRaw(this, &FIOSApplication::HandlePhysicalKeyboardConnectionChanged);
		
		if (KeyboardController->IsAnyKeyboardConnected())
		{
			HandlePhysicalKeyboardConnectionChanged();
		}
	}
	
	if (TSharedPtr<FAppleMouseController> MouseController = InputInterface->GetMouseController().Pin())
	{
		MouseController->OnAvailabilityChanged().AddRaw(this, &FIOSApplication::HandlePhysicalMouseConnectionChanged);
		MouseController->OnMovementDispatched().AddRaw(this, &FIOSApplication::HandleMouseMovement);
		
		if (MouseController->IsAnyMouseConnected())
		{
			HandlePhysicalMouseConnectionChanged();
		}
	}
}
...
FIOSApplication::~FIOSApplication()
{
	...
	if (TSharedPtr<FAppleKeyboardController> KeyboardController = InputInterface->GetKeyboardController().Pin())
	{
		KeyboardController->OnAvailabilityChanged().RemoveAll(this);
	}
	
	if (TSharedPtr<FAppleMouseController> MouseController = InputInterface->GetMouseController().Pin())
	{
		MouseController->OnAvailabilityChanged().RemoveAll(this);
		MouseController->OnMovementDispatched().RemoveAll(this);
	}
}

void FIOSApplication::HandlePhysicalKeyboardConnectionChanged()
{
	IOSViewController* ViewController = [[IOSAppDelegate GetDelegate] IOSController];
	
	if (!ensure(ViewController))
	{
		return;
	}

	if (InputInterface->IsAnyPhysicalKeyboardConnected())
	{
		ViewController.controllerUserInteractionEnabled = false;
		[ViewController becomeFirstResponder];
		UE_LOGF(LogIOS, Log, "[%s] External keyboard connected.", __func__);
	}
	else
	{
		...
		UE_LOGF(LogIOS, Log, "[%s] All external keyboards disconnected", __func__);
	}
}

void FIOSApplication::HandlePhysicalMouseConnectionChanged()
{
	bool bWasAttached = bIsMouseAttached;
	TSharedPtr<FAppleMouseController> MouseController = InputInterface->GetMouseController().Pin();
	if (MouseController && MouseController->IsAnyMouseConnected())
	{	
		bIsMouseAttached = true;
		UE_LOGF(LogIOS, Log, "[%s] External Mouse connected.", __func__);
	}
	else
	{
		bIsMouseAttached = false;
		UE_LOGF(LogIOS, Log, "[%s] All external mice disconnected", __func__);
	}
	
	if (bWasAttached != bIsMouseAttached)
	{
		Cursor->Show(bIsMouseAttached);
		MessageHandler->OnCursorSet();
	}
}

void FIOSApplication::HandleMouseMovement(FIntVector2 MovementDelta)
{
	const FVector2D TargetPos = Cursor->GetPosition() + FVector2D(MovementDelta);
	Cursor->SetPosition(FMath::TruncToInt(TargetPos.X), FMath::TruncToInt(TargetPos.Y));
}

FModifierKeysState FIOSApplication::GetModifierKeys() const
{ 
	if (TSharedPtr<FAppleKeyboardController> KeyboardController = InputInterface->GetKeyboardController().Pin())
	{
		return KeyboardController->GetModifierKeys();
	}

	return FModifierKeysState(); 
}

bool FIOSApplication::IsMouseAttached() const
{
