...
#include <AVFAudio/AVAudioApplication.h>
...
static void SignalHandler(int32 Signal, struct __siginfo* Info, void* Context)
{
	...
	if (FPlatformAtomics::InterlockedCompareExchange(&bHasEntered, 1, 0) == 0)
	{
		...
		UE_LOGF(LogIOS, Error, "%ls", ANSI_TO_TCHAR(StackTrace));
...
void LexFromString(EAudioFeature& OutFeature, const TCHAR* String)
{
	...
	else if (FCString::Stricmp(String, TEXT("AllowHapticsAndSystemSoundsDuringRecording")) == 0)
	{
		OutFeature = EAudioFeature::AllowHapticsAndSystemSoundsDuringRecording;
	}
}
...
FString LexToString(EAudioFeature Feature)
{
	...
	case EAudioFeature::AllowHapticsAndSystemSoundsDuringRecording: return TEXT("AllowHapticsAndSystemSoundsDuringRecording");
...
+ (IOSAppDelegate*)GetDelegate
{
	...
	{
		UE_LOGF(LogIOS, Fatal, "Currently, a native app embedding Unreal must have the AppDelegate subclass from IOSAppDelegate.");
...
/*	dispatch_async(dispatch_get_main_queue(), ^{
	...
	dispatch_async(dispatch_get_main_queue(), ^{
		[[IOSAppDelegate GetDelegate].IOSController setNeedsUpdateOfScreenEdgesDeferringSystemGestures];
	});
#endif
    ...
    while( !IsEngineExitRequested() )
	{
        ...
        else
        {
			...
			if (static_cast<float>(OutputVolume) != self.LastVolume || self.bForceEmitVolume)
			{
				FGraphEventRef AudioMuteTask = FFunctionGraphTask::CreateAndDispatchWhenReady([bMuted, OutputVolume]()
					{
				...
				self.LastVolume = static_cast<float>(OutputVolume);
...
- (void)InitializeAudioSession
{
	[[NSNotificationCenter defaultCenter] addObserverForName:AVAudioSessionInterruptionNotification object:nil queue:nil usingBlock:^(NSNotification *notification)
	{
		...
		UE_CLOGF(InterruptionType == AVAudioSessionInterruptionTypeBegan, LogIOSAudioSession, Log, "AudioSession interruption notification: Began");
		UE_CLOGF(InterruptionType == AVAudioSessionInterruptionTypeEnded, LogIOSAudioSession, Log, "AudioSession interruption notification: Ended");
	...
	{
		UE_LOGF(LogIOSAudioSession, Log, "AVAudioSessionRouteChange: UserInfo: [%ls] CurrentRoute: [%ls]", *FString([[notification userInfo]description]), *FString([[[AVAudioSession sharedInstance] currentRoute] description]));
...
- (void)ToggleAudioSession:(bool)bActive
{
	...
	if (self.bAudioActive || [self IsBackgroundAudioPlaying] || [self IsFeatureActive:EAudioFeature::BackgroundAudio])
	{
		...
		if ([self IsFeatureActive:EAudioFeature::Playback] && bRecordActive)
		{
			...
			{
				// iOS sdk 26 deprecated AVAudioSessionCategoryOptionAllowBluetooth in favor of AVAudioSessionCategoryOptionAllowBluetoothHFP
				// but AVAudioSessionCategoryOptionAllowBluetoothHFP is not present in sdk 16 despite what the docs/header say
#if (defined(__IPHONE_26_0) && __IPHONE_OS_VERSION_MAX_ALLOWED >= __IPHONE_26_0) || PLATFORM_VISIONOS
	...
	{
		UE_LOGF(LogIOSAudioSession, Log, "Setting AVAudioSession category to Category:%ls Mode:%ls Options:%llx", *FString(Category), *FString(Mode), Options);

		[[AVAudioSession sharedInstance] setCategory:Category mode:Mode options:Options error:&ActiveError];
		if (ActiveError)
		{
			UE_LOGF(LogIOSAudioSession, Error, "Failed to set AVAudioSession category to Category:%ls (current %ls) Mode:%ls (current %ls) Options:%llx (current %llx)! [Error = %ls].", 
	...
	const bool bAllowHapticsDuringRecording = [self IsFeatureActive:EAudioFeature::AllowHapticsAndSystemSoundsDuringRecording];
	[[AVAudioSession sharedInstance] setAllowHapticsAndSystemSoundsDuringRecording:bAllowHapticsDuringRecording error:nil];
	...
	if (bActive)
	{
		...
		if (ActiveError)
		{
			UE_LOGF(LogIOSAudioSession, Error, "Failed to set audio session to active = %d [Error = %ls]", bActive, *FString([ActiveError description]));
		}
	}
}
...
-(void)RequestRecordPermission:(TFunction<void(bool)>)OnComplete
{
#if PLATFORM_TVOS
	// TVOS does not have sound recording capabilities.
	return;
#elif (defined(__IPHONE_17_0) && __IPHONE_OS_VERSION_MAX_ALLOWED >= __IPHONE_17_0)
	[AVAudioApplication requestRecordPermissionWithCompletionHandler:^(BOOL bGranted)
	{
		OnComplete(bGranted);
	}];
#else
	[[AVAudioSession sharedInstance] requestRecordPermission:^(BOOL bGranted)
	{
		OnComplete(bGranted);
	}];
#endif
...
-(FIOSPlatformMisc::EIOSAuthMicrophonePermissionStatus)GetMicrophonePermissionStatus
{
#if PLATFORM_TVOS
	// TVOS does not have sound recording capabilities.
	return FIOSPlatformMisc::EIOSAuthMicrophonePermissionStatus::Denied;
#elif (defined(__IPHONE_17_0) && __IPHONE_OS_VERSION_MAX_ALLOWED >= __IPHONE_17_0)
	switch ([[AVAudioApplication sharedInstance] recordPermission])
	{
	case AVAudioApplicationRecordPermissionUndetermined:
		return FIOSPlatformMisc::EIOSAuthMicrophonePermissionStatus::NotDetermined;
	case AVAudioApplicationRecordPermissionDenied:
		return FIOSPlatformMisc::EIOSAuthMicrophonePermissionStatus::Denied;
	case AVAudioApplicationRecordPermissionGranted:
		return FIOSPlatformMisc::EIOSAuthMicrophonePermissionStatus::Granted;
	default:
		return FIOSPlatformMisc::EIOSAuthMicrophonePermissionStatus::Unknown;
	}
#else
...
- (void)SetFeature:(EAudioFeature)Feature Active:(bool)bIsActive
{
	...
	else
	{
		...
		{
			UE_LOGF(LogIOSAudioSession, Warning, "Attempted to disable audio feature that is already disabled");
		}
		else
		{
			--GEnabledAudioFeatures[static_cast<uint8>(Feature)];
...
//        InstallSignalHandlers();
    ...
    // Legacy UIApplicationDelegate path: the app delegate owns the window and starts the game
    // thread here. In scene mode, IOSSceneDelegate.scene:willConnectToSession:options: takes
    // over this work because the UIWindow must be initialized against a UIWindowScene.
...
{
	return [self handleContinueUserActivity:userActivity];
}
#endif
...
{
	return [self handleOpenURL:url sourceApplication:sourceApplication annotation:annotation];
}
#endif
...
- (void)handleWillResignActive
{
    ...
    if (self.bEngineInit)
    {
		...
		while((FPlatformTime::Seconds() - startTime) < [self GetBackgroundingMainThreadBlockTime])
		{
			...
			{
				UE_LOGF(LogTemp, Display, "Task was completed before time.");
				break;
			}
		}
		UE_LOGF(LogTemp, Display, "Done with entering background tasks time.");
    }
...
// fix for freeze on tvOS, moving to applicationDidEnterBackground. Not making the changes for iOS platforms as the bug does not happen and could bring some side effets.
	...
	BackgroundTaskId = [[UIApplication sharedApplication] beginBackgroundTaskWithExpirationHandler:^{
		UE_LOGF(LogIOS, Warning, "Background task expired before completing FIOSCoreDelegates::OnWillResignActive broadcast");
...
- (void)handleDidEnterBackground
{
	...
	__block UIBackgroundTaskIdentifier BackgroundTaskId = UIBackgroundTaskInvalid;	
	BackgroundTaskId = [[UIApplication sharedApplication] beginBackgroundTaskWithExpirationHandler:^{
		UE_LOGF(LogIOS, Warning, "Background task expired before completing FCoreDelegates::ApplicationWillEnterBackgroundDelegate broadcast");
        [[UIApplication sharedApplication] endBackgroundTask: BackgroundTaskId];
    }];
...
{
	UE_LOGF(LogTemp, Display, "Received RemoteNotification");
...
- (void)application:(UIApplication *)application didRegisterForRemoteNotificationsWithDeviceToken:(NSData *)deviceToken
{
	...
	UE_LOGF(LogTemp, Display, "Device Token: %ls", *FString(token));
...
-(void)application:(UIApplication *)application didFailToRegisterForRemoteNotificationsWithError:(NSError *)error
{
	...
	UE_LOGF(LogTemp, Warning, "FAILED to register for remote notificaiton: %ls", *errorDescription);
...
+(EDeviceScreenOrientation) ConvertFromUIInterfaceOrientation:(UIInterfaceOrientation)Orientation
{
	...
	{
		default:
		case UIInterfaceOrientationUnknown : return EDeviceScreenOrientation::Unknown;
		case UIInterfaceOrientationPortrait : return EDeviceScreenOrientation::Portrait;
		case UIInterfaceOrientationPortraitUpsideDown : return EDeviceScreenOrientation::PortraitUpsideDown;
		case UIInterfaceOrientationLandscapeLeft : return EDeviceScreenOrientation::LandscapeLeft;
		case UIInterfaceOrientationLandscapeRight : return EDeviceScreenOrientation::LandscapeRight;
	}
}
...
+(UIInterfaceOrientation) GetUIInterfaceOrientationFromUIDeviceOrientation:(UIDeviceOrientation)Orientation
{
	...
	{
		default:
		case UIDeviceOrientationUnknown : return UIInterfaceOrientationUnknown;
...
+(UIInterfaceOrientationMask) GetMaskFromEDeviceScreenOrientation:(EDeviceScreenOrientation)Orientation
{
	...
	{
		default:
		case EDeviceScreenOrientation::Unknown : return UIInterfaceOrientationMaskAll;
		case EDeviceScreenOrientation::Portrait : return UIInterfaceOrientationMaskPortrait;
		case EDeviceScreenOrientation::PortraitUpsideDown : return UIInterfaceOrientationMaskPortraitUpsideDown;
		case EDeviceScreenOrientation::LandscapeLeft : return UIInterfaceOrientationMaskLandscapeLeft;
		case EDeviceScreenOrientation::LandscapeRight : return UIInterfaceOrientationMaskLandscapeRight;
	}
}
...
static bool HandleReceivedNotification(UNNotification* notification)
{
	...
	if (IsReadyToHandle)
	{
		...
		if(NotificationType != nullptr)
		{
			...
			if(LocalOrRemote == FString("Local"))
			{
				IsLocal = true;
			}
		}
...
-(void)batteryChanged:(NSNotification*)notification
{
	...
	[FAppleAsyncTask CreateTaskWithBlock : ^ bool(void)
	{
		UE_LOGF(LogIOS, Display, "Battery Level Changed: %d", self.BatteryLevel);
		return true;
	}];
#endif
}
...
-(void)batteryStateChanged:(NSNotification*)notification
{
	...
	[FAppleAsyncTask CreateTaskWithBlock : ^ bool(void)
	{
		UE_LOGF(LogIOS, Display, "Battery State Changed: %d", self.bBatteryState);
		return true;
	}];
#endif
}
...
-(void)temperatureChanged:(NSNotification *)notification
{
    ...
    [FAppleAsyncTask CreateTaskWithBlock : ^ bool(void)
     {
        UE_LOGF(LogIOS, Display, "Temperature Changed: %ls", *Level);
    ...
    [FAppleAsyncTask CreateTaskWithBlock : ^ bool(void)
     {
        ...
        UE_LOGF(LogIOS, Display, "Low Power Mode Changed: %d", bInLowPowerMode);
...
-(void)handle
{
	...
	{
		UE_LOGF(LogIOS, Warning, "Failed to deliver notification after retrying for 10 seconds.");
