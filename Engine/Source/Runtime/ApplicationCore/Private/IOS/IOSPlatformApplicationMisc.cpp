...
{
	UE_LOGF(LogIOS, Warning, "Restting gamepad assignments is not allowed in IOS")
}
...
EScreenPhysicalAccuracy FIOSPlatformApplicationMisc::ComputePhysicalScreenDimensions(FVector2f& OutScreenDimensions, float& OutScreenSize)
{
	...
	{
		UE_LOGF(LogIOS, Warning, "ComputePhysicalScreenDimensions : Approximating ScreenDensity due to CVarPhysicalScreenDensity not set or unknown (%d).", ScreenDensity);

		const double NativeScale = [[UIScreen mainScreen] scale];
		ScreenDensity = FMath::TruncToInt(163 * NativeScale);
		if (ScreenDensity <= 0)
		{
			UE_LOGF(LogIOS, Warning, "ComputePhysicalScreenDimensions : Incorrect ScreenDensity resulting in Unknown ScreenPhysicalAccuracy. ScreenDensity: %d", ScreenDensity);
			return EScreenPhysicalAccuracy::Unknown;
		}
	...
	UE_LOGF(LogIOS, Display, "ComputePhysicalScreenDimensions - NativeResolution: %ls, ScreenDensity: %d, OutScreenDimensions (inches): %ls, OutScreenSize (inches): %f",
...
{
	{
		...
		if (ensure(InputInterface))
		{
			return !InputInterface->IsAnyPhysicalKeyboardConnected();
		}
	}
	
	// Falback to showing the virtual keyboard
	return true;
}
