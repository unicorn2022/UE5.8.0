...
bool FMacPlatformSurvey::GetSurveyResults( FHardwareSurveyResults& OutResults, bool bWait )
{
	...
	OutResults.MemoryMB = uint32(float(static_cast<double>(TotalPhys)/1024.0/1024.0)+ .1f);
	...
	if (OutResults.CPUCount > 0)
	{
		...
		{
			OutResults.CPUClockGHz = (float)(0.000000001 * static_cast<double>(CPUSpeed));
		}
		else
		{
	...
	{
		UE_LOGF(LogMac, Warning, "FMacPlatformSurvey::TickSurveyHardware() failed to get root-folder drive size" );
		OutResults.ErrorCount++;
	...
	WriteFStringToResults(OutResults.OSVersion, FString("Mac OS X ") + OSXVersion);
	...
	{
		UE_LOGF(LogMac, Warning, "FMacPlatformSurvey::TickSurveyHardware() failed to get system power sources info. Assuming desktop Mac." );
		OutResults.ErrorCount++;
