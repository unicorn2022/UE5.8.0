...
bool FMacWindow::IsPointInWindow( int32 X, int32 Y ) const
{
	...
	if (!WindowHandle->bMinimized)
	{
		...
		if (WindowHandle->bIsOnActiveSpace)
		{
			...
			NSPoint CursorPoint = NSMakePoint(static_cast<float>(X) / DPIScaleFactor, WindowFrame.size.height - (static_cast<float>(Y) / DPIScaleFactor + 1));
...
void FMacWindow::ApplySizeAndModeChanges(int32 X, int32 Y, int32 Width, int32 Height, EWindowMode::Type WindowMode)
{
	...
	if (WindowMode == EWindowMode::Windowed || WindowMode == EWindowMode::WindowedFullscreen)
	{
		...
		if(WindowHandle)
		{
			...
			const float DPIScaleFactor = FPlatformApplicationMisc::GetDPIScaleFactorAtPoint(static_cast<float>(X), static_cast<float>(Y));
			Width = FMath::CeilToInt(static_cast<float>(Width) / DPIScaleFactor);
			Height = FMath::CeilToInt(static_cast<float>(Height) / DPIScaleFactor);
	...
	else
	{
		...
		if (WindowedModeSavedState.CapturedDisplayID != kCGNullDirectDisplay)
		{
			CGDisplayModeRef DisplayMode = FMacPlatformMiscEx::GetSupportedDisplayMode(WindowedModeSavedState.CapturedDisplayID, Width, Height);
			MainThreadCall(^{
