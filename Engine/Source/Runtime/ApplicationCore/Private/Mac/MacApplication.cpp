...
namespace MacApplicationCVars
{
	static bool bUseNewMouseControllerForMovement = true;
	static FAutoConsoleVariableRef CVarUseNewMouseControllerMovement(
		TEXT("Slate.MacUseNewMouseControllerMovement"),
		bUseNewMouseControllerForMovement,
		TEXT("false: AppKit handles mouse movement in all modes (legacy). true: FAppleMouseController handles movement during high-precision mode; AppKit handles normal mode."),
		ECVF_ReadOnly);
}
...
public:
	...
	// IInputInterface forwarders. The legacy HIDInputInterface has no haptics or lightbar, so these are
	// only meaningful when the GameController-based path is selected
	void SetForceFeedbackChannelValue(int32 ControllerId, FForceFeedbackChannelType ChannelType, float Value)
	{
		if (AppleControllerInterface.IsValid()) AppleControllerInterface->SetForceFeedbackChannelValue(ControllerId, ChannelType, Value);
	}
	void SetForceFeedbackChannelValues(int32 ControllerId, const FForceFeedbackValues& Values)
	{
		if (AppleControllerInterface.IsValid()) AppleControllerInterface->SetForceFeedbackChannelValues(ControllerId, Values);
	}
	void SetLightColor(int32 ControllerId, FColor Color)
	{
		if (AppleControllerInterface.IsValid()) AppleControllerInterface->SetLightColor(ControllerId, Color);
	}
	void ResetLightColor(int32 ControllerId)
	{
		if (AppleControllerInterface.IsValid()) AppleControllerInterface->ResetLightColor(ControllerId);
	}
	void SetDeviceProperty(int32 ControllerId, const FInputDeviceProperty* Property)
	{
		if (AppleControllerInterface.IsValid()) AppleControllerInterface->SetDeviceProperty(ControllerId, Property);
	}
...
,	bMouseInputNeedsReassociation(false)
...
,	bDisplayReconfiguring(false)
{
	...
	MainThreadCall(^{
		...
		ScreensDidWakeObserver = [[[NSWorkspace sharedWorkspace] notificationCenter] addObserverForName:NSWorkspaceScreensDidWakeNotification
																								object:[NSWorkspace sharedWorkspace]
																								 queue:[NSOperationQueue mainQueue]
																							usingBlock:^(NSNotification* Notification){ ReassociateMouseInput(); }];

		SystemDidWakeObserver = [[[NSWorkspace sharedWorkspace] notificationCenter] addObserverForName:NSWorkspaceDidWakeNotification
																							   object:[NSWorkspace sharedWorkspace]
																								queue:[NSOperationQueue mainQueue]
																						   usingBlock:^(NSNotification* Notification){ ReassociateMouseInput(); }];
	...
	FCoreDelegates::PreSlateModalWithContext.AddRaw(this, &FMacApplication::StartScopedModalEvent);
    FCoreDelegates::PostSlateModalWithContext.AddRaw(this, &FMacApplication::EndScopedModalEvent);
#endif
	...
	if (MacApplicationCVars::bUseNewMouseControllerForMovement)
	{
		// Only use the movement capabilities from the controller. For button input handling we need the context about on which window the event is happening
		// but that is not supported by GC Mouse yet. In this setup we use Appkit to handle Mouse button and non precision movement modes, and the new Mouse controller
		// (GC Mouse based) for high precision movement
		MouseController = MakeShared<FAppleMouseController>(MessageHandler, EMouseControllerCapabilities::Movement);
		MouseController->OnMovementDispatched().AddRaw(this, &FMacApplication::HandleMouseControllerMovement);
	}
}
...
FMacApplication::~FMacApplication()
{
	...
	MainThreadCall(^{
		...
		if (ScreensDidWakeObserver)
		{
			[[[NSWorkspace sharedWorkspace] notificationCenter] removeObserver:ScreensDidWakeObserver];
		}
		if (SystemDidWakeObserver)
		{
			[[[NSWorkspace sharedWorkspace] notificationCenter] removeObserver:SystemDidWakeObserver];
		}
	...
	if (MouseController)
	{
		MouseController->OnMovementDispatched().RemoveAll(this);
...
void FMacApplication::DismissAllMenusOnGameThread()
{
	GameThreadCall(^{
		MessageHandler->DismissAllMenus();
	}, /*bWait=*/false, @[ NSDefaultRunLoopMode, UnrealResizeEventMode ]);
}
...
{
	UE_LOGF(LogAppleController, Verbose, "[%s] HighPrecisionMouseMode %ls", __func__, Enable ? TEXT("Enabled") : TEXT("Disabled"));

	bUsingHighPrecisionMouseInput = Enable;
	((FMacCursor*)Cursor.Get())->SetHighPrecisionMouseMode(Enable);

	if (MouseController && MacApplicationCVars::bUseNewMouseControllerForMovement)
	{
		if (Enable && bMouseInputNeedsReassociation.load(std::memory_order_acquire))
		{
			MouseController->RebindMouseHandlers();
		}
		MouseController->SetEnabled(Enable);
	}
}

void FMacApplication::ReassociateMouseInput()
{
	CGAssociateMouseAndMouseCursorPosition(true);

	if (MouseController && MacApplicationCVars::bUseNewMouseControllerForMovement)
	{
		MouseController->SuspendMovementHandler();
	}

	bMouseInputNeedsReassociation.store(true, std::memory_order_release);
}
...
{
	static_assert(((uint32)EGestureEvent::Count) == 9, "If the number of gestures changes you need to add more entries below!");
	...
	GestureAttributes.Add(FAnalyticsEventAttribute(FString("Pan"),	GestureUsage[(int32)EGestureEvent::Pan]));
	GestureAttributes.Add(FAnalyticsEventAttribute(FString("Tap"),	GestureUsage[(int32)EGestureEvent::Tap]));
	GestureAttributes.Add(FAnalyticsEventAttribute(FString("LongPress"),	GestureUsage[(int32)EGestureEvent::LongPress]));
	GestureAttributes.Add(FAnalyticsEventAttribute(FString("Flick"),	GestureUsage[(int32)EGestureEvent::Flick]));
...
void FMacApplication::StartScopedModalEvent(const FCoreDelegates::FModalWindowContext& Context)
{
    FPlatformApplicationMisc::bMacApplicationModalMode = true;
    FPlatformApplicationMisc::bChachedMacMenuStateNeedsUpdate = true;
}

void FMacApplication::EndScopedModalEvent(const FCoreDelegates::FModalWindowContext& Context)
{
	...
	else if (Object && [Object isKindOfClass:[NSNotification class]])
	{
		...
		if ([[Notification object] isKindOfClass:[FCocoaWindow class]])
		{
			...
			if (DeferredEvent.NotificationName == NSWindowDidResizeNotification)
			{
				...
				{
					const bool bWaitForThread = false;
					GameThreadCall(^{
					...
					}, bWaitForThread, @[ NSDefaultRunLoopMode, UnrealResizeEventMode, UnrealShowEventMode, UnrealFullscreenEventMode ]);
				}
				return;
			}
		}
...
void FMacApplication::HandleMouseControllerMovement(FIntVector2 Delta)
{
	if (Delta.X != 0 || Delta.Y != 0)
	{
		bMouseInputNeedsReassociation.store(false, std::memory_order_release);
	}

	// GCMouse reports raw hardware deltas — cursor warps (CGWarpMouseCursorPosition) do not
	// generate GCMouse events, so no warp compensation is needed (unlike the NSEvent path).
	if (bUsingHighPrecisionMouseInput)
	{
		...
		if (ActiveWindow.IsValid() && ActiveWindow->GetWindowMode() != EWindowMode::Windowed)
		{
			VisibleFrame.origin.y += 5;
			VisibleFrame.size.height -= 10;
		}

		const int32 ClampedPosX = FMath::Clamp(HighPrecisionMousePos.X, static_cast<int32>(VisibleFrame.origin.x), static_cast<int32>(VisibleFrame.origin.x + VisibleFrame.size.width) - 1);
		const int32 ClampedPosY = FMath::Clamp(HighPrecisionMousePos.Y, static_cast<int32>(VisibleFrame.origin.y), static_cast<int32>(VisibleFrame.origin.y + VisibleFrame.size.height) - 1);
...
void FMacApplication::ProcessMouseMovedEvent(const FDeferredMacEvent& Event, TSharedPtr<FMacWindow> EventWindow)
{
	...
	if (bUsingHighPrecisionMouseInput)
	{
		if (!MacApplicationCVars::bUseNewMouseControllerForMovement || bMouseInputNeedsReassociation.load(std::memory_order_acquire))
		{
	...
	if (WindowToActivate.IsValid())
	{
		...
		if (WindowType == EWindowType::Menu && bActivateAllowMainWindow)
		{
			...
			if (NumMenus<=0)
			{
				...
				}, bWaitForThread, @[ NSDefaultRunLoopMode, UnrealResizeEventMode, UnrealShowEventMode, UnrealFullscreenEventMode, UnrealCloseEventMode ]);
			}
		}
	}
...
void FMacApplication::SetForceFeedbackChannelValue(int32 ControllerId, FForceFeedbackChannelType ChannelType, float Value)
{
	...
	// Route to the GameController-backed implementation when it owns this controller id.
	HIDInput->SetForceFeedbackChannelValue(ControllerId, ChannelType, Value);
}
...
void FMacApplication::SetForceFeedbackChannelValues(int32 ControllerId, const FForceFeedbackValues &Values)
{
	...
	HIDInput->SetForceFeedbackChannelValues(ControllerId, Values);
}

void FMacApplication::SetLightColor(int32 ControllerId, FColor Color)
{
	HIDInput->SetLightColor(ControllerId, Color);
}

void FMacApplication::ResetLightColor(int32 ControllerId)
{
	HIDInput->ResetLightColor(ControllerId);
}

void FMacApplication::SetDeviceProperty(int32 ControllerId, const FInputDeviceProperty* Property)
{
	HIDInput->SetDeviceProperty(ControllerId, Property);
}
...
{
	return Zone == EWindowZone::NotInWindow || EWindowZone::IsBorder(Zone);
}
