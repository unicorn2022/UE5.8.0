...
void FMacPlatformApplicationMisc::ClipboardCopy(const TCHAR* Str)
{
	...
	if (UE::Private::GMalloc != GCrashMalloc)
	{
		...
		{
			UE_LOGF(LogMac, Warning, "Failed to create NSString to copy text to the pasteboard.");
		}
	}
}
...
void* FMacPlatformApplicationMisc::GetActiveWindowPtr()
{
	return [NSApp keyWindow];
}

void FMacPlatformApplicationMisc::ProcessAsyncTasks()
{
	[FAppleAsyncTask ProcessAsyncTasks];
}
bool FMacPlatformApplicationMisc::IsMainThread()
{
