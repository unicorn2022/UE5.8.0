...
void FMacPlatformSplash::Show()
{
	if( !GSplashWindow && FParse::Param(FCommandLine::Get(),TEXT("NOSPLASH")) != true )
	{
		...
		NSRect ContentRect;
		{
			...
			float ImageWidth = static_cast<float>([BitmapRep pixelsWide]);
			float ImageHeight = static_cast<float>([BitmapRep pixelsHigh]);
			...
			GSplashScreenTextRects[ SplashTextType::GameName ].size.width = ImageWidth - static_cast<float>(2 * OriginX);
			...
			GSplashScreenTextRects[ SplashTextType::StartupProgress ].size.width = ImageWidth - static_cast<float>(2 * OriginX);
