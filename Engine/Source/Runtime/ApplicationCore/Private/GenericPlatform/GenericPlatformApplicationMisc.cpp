...
TAutoConsoleVariable<bool> FGenericPlatformApplicationMisc::CVarEnableNativeGestureRecognition(
	TEXT("Input.EnableNativeGestureRecognition"),
	false,
	TEXT("Use native platform gesture recognizers instead of the software gesture detection. Enabled via platform ini on supported platforms."),
	ECVF_Default
);

bool FGenericPlatformApplicationMisc::AreNativeGesturesEnabled()
{
	return CVarEnableNativeGestureRecognition.GetValueOnAnyThread();
}
...
{
	UE_LOGF(LogHAL, Fatal, "FGenericPlatformProcess::IsThisApplicationForeground not implemented on this platform");
	return false;
}
