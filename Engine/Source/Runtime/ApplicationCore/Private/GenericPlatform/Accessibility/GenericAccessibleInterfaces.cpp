...
void FGenericAccessibleMessageHandler::SetActive(bool bActive)
{
	...
	if (bActive != bIsActive)
	{
		...
		if (bIsActive)
		{
			UE_LOGF(LogAccessibility, Verbose, "Enabling Accessibility");
			OnActivate();
		}
		else
		{
			OnDeactivate();
			UE_LOGF(LogAccessibility, Verbose, "Accessibility Disabled");
		}
	}
}
...
bool FGenericAccessibleUserRegistry::RegisterUser(const TSharedRef<FGenericAccessibleUser>& User)
{
	...
	UE_LOGF(LogAccessibility, Verbose, "Registering accessible user %d failed. Another accessible user with accessible index %d already registered.", UserIndex, UserIndex);
	return false;
}
...
bool FGenericAccessibleUserRegistry::UnregisterUser(const FAccessibleUserIndex UserIndex)
{
	...
	UE_LOGF(LogAccessibility, Verbose, "Failed to unregister accessible user with accessible index %d. No accessible user with accessible index %d currently registered.", UserIndex, UserIndex);
	return false;
}
