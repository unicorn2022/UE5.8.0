...
IPlatformInputDeviceMapper::FOnInputDeviceHardwareInfoAvailable IPlatformInputDeviceMapper::OnInputDeviceHardwareInfoAvailable;
...
bool IPlatformInputDeviceMapper::Internal_SetInputDeviceConnectionState(FInputDeviceId DeviceId, EInputDeviceConnectionState NewState)
{
	if (!DeviceId.IsValid())
	{
		UE_LOGF(LogInputDeviceMapper, Error, "IPlatformInputDeviceMapper::Internal_SetInputDeviceConnectionState was called with an invalid DeviceId of '%d'", DeviceId.GetId());
		return false;
	}
...
bool IPlatformInputDeviceMapper::Internal_MapInputDeviceToUser(FInputDeviceId DeviceId, FPlatformUserId UserId, EInputDeviceConnectionState ConnectionState)
{
	if (!DeviceId.IsValid())
	{
		UE_LOGF(LogInputDeviceMapper, Error, "IPlatformInputDeviceMapper::Internal_MapInputDeviceToUser was called with an invalid DeviceId of '%d'", DeviceId.GetId());
		return false;
	}
...
#if 0	// todo_bh: Not sure if we actually need this here since we have updated all platforms already
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	// Forward compatibility for FInputDeviceScope with the newer FInputDeviceRegistry system. 
	// If we have an input device scope, and there is not yet a device descriptor set for this device,
	// then populate it with the FInputDeviceScope data if possible.
	if (IsInGameThread())
	{
		if(!FInputDeviceRegistry::FindDescriptor(DeviceId).IsSet())
		{
			if (const FInputDeviceScope* Scope = FInputDeviceScope::GetCurrent())
			{
				UE_LOGF(LogInputDeviceMapper, Log, "Registering device based on the FInputDeviceScope %d", DeviceId.GetId());
			
				FInputDeviceRegistry::RegisterDevice(
					FInputDeviceDescriptor
					{
						.HardwareDeviceHandle     = DeviceId,
						.InputDeviceName          = Scope->InputDeviceName,
						.HardwareDeviceIdentifier = FName(*Scope->HardwareDeviceIdentifier),
					});
			}
		}	
	}
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	// If there is a valid FInputDeviceScope at the time of connection, or the device registry is aware of this device,
	// then the hardware info is available.
	if (FInputDeviceRegistry::FindDescriptor(DeviceId).IsSet())
	{
		OnInputDeviceHardwareInfoAvailable.Broadcast(ConnectionState, UserId, DeviceId);	
	}
...
bool IPlatformInputDeviceMapper::Internal_ChangeInputDeviceUserMapping(FInputDeviceId DeviceId, FPlatformUserId NewUserId, FPlatformUserId OldUserId)
{
	if (!DeviceId.IsValid())
	{
		UE_LOGF(LogInputDeviceMapper, Error, "IPlatformInputDeviceMapper::Internal_ChangeInputDeviceUserMapping was called with an invalid DeviceId of '%d'", DeviceId.GetId());
		return false;
	}

	if (NewUserId == OldUserId)
	{
		UE_LOGF(LogInputDeviceMapper, Log, "[%s] DeviceId of '%d' is already mapped to platform user '%d'.", __func__, DeviceId.GetId(), OldUserId.GetInternalId());
		return false;
	}
	...
	{
		UE_LOGF(LogInputDeviceMapper, Error, "IPlatformInputDeviceMapper::Internal_ChangeInputDeviceUserMapping: DeviceID '%d' is not mapped! Call Internal_MapInputDeviceToUser to map it to a user first!", DeviceId.GetId());
		return false;
	}
...
void IPlatformInputDeviceMapper::HandleInputDevicePolicyChanged(const EInputDeviceMappingPolicy NewPolicy)
{
	...
	{
		UE_LOGF(LogInputDeviceMapper, Log, "[%s] Device Mapping Policy is already set to '%ls'...",
			__func__,
	...
	UE_LOGF(LogInputDeviceMapper, Log, "[%s] Changing device mapping policy from '%ls' to '%ls'...",
		__func__,
		*LexToString(UE::Input::CachedDevicePolicy),
...
FPlatformUserId FGenericPlatformInputDeviceMapper::AllocateNewUserId()
{
	...
	UE_LOGF(LogInputDeviceMapper, Log, "[%s] Allocating a new PlatformUserId %d", __func__, LastPlatformUserId.GetInternalId());
