...
//	{
//		UE_LOGF(LogAccessibility, VeryVerbose, "UIA Property Changed: %i", PropertyId);
			...
			if (SupportsInterface(patternId))
			{
				...
				case UIA_SelectionItemPatternId:
					*pRetVal = static_cast<ISelectionItemProvider*>(ControlProvider);
					break;
				default:
					UE_LOGF(LogAccessibility, Error, "FWindowsUIAWidgetProvider::SupportsInterface() returned true, but was unhandled in GetPatternProvider(). PatternId = %i", patternId);
					*pRetVal = nullptr;
