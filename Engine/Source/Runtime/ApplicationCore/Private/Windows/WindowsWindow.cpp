...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FWindowsWindow::Initialize);
	...
	if ( RegisterTouchWindow( HWnd, 0 ) == false )
	{
		...
		UE_LOGF(LogWindows, Warning, "Register touch input failed!");
	}
...
HRGN FWindowsWindow::MakeWindowRegionObject(bool bForQuery) const
{
	int X1, Y1, X2, Y2;
	if ( RegionWidth != INDEX_NONE && RegionHeight != INDEX_NONE )
	{
		const bool bIsBorderless = !GetDefinition().HasOSWindowBorder;
		if (IsMaximized())
		{
			// Maximized windows are forcibly oversized by cxWindowBorders, so the rect we pass to SetWindowRgn will be interpreted as relative to
			// -cxWindowBorders pixels from the screen's corner. We don't want anything being drawn on adjacent monitors, so we need to offset the rgn
			// by this much (unless it is being used for a query against a cursor position that we have already remapped to client-relative).
			int32 BorderSize = bForQuery ? 0 : GetWin32BorderSize();
			X1 = BorderSize;
			Y1 = BorderSize;
			X2 = BorderSize + RegionWidth;
			Y2 = BorderSize + RegionHeight;
		}
		else
		{
			const bool bUseCornerRadius = WindowMode == EWindowMode::Windowed && !bIsBorderless &&
#if ALPHA_BLENDED_WINDOWS
			...
			if( bUseCornerRadius )
			{
				...
				HRGN Region = CreateRoundRectRgn( 0, 0, RegionWidth+1, RegionHeight+1, Definition->CornerRadius, Definition->CornerRadius );
				if (!Region)
				{
					UE_LOGF(LogWindows, Error, "CreateRoundRectRgn for 0,0,%d+1,%d+1,%d,%d failed with error %d",
						RegionWidth, RegionHeight, Definition->CornerRadius, Definition->CornerRadius, GetLastError());
				}
	...
	else
	{
		...
		X1 = 0;
		Y1 = 0;
		X2 = rcWnd.right - rcWnd.left;
		Y2 = rcWnd.bottom - rcWnd.top;
	}

	HRGN Region = CreateRectRgn(X1, Y1, X2, Y2);
	if (!Region)
	{
		UE_LOGF(LogWindows, Error, "CreateRectRgn failed for %d,%d,%d,%d with error %d", X1, Y1, X2, Y2, GetLastError());
	}
	return Region;
}
...
void FWindowsWindow::AdjustWindowRegion( int32 Width, int32 Height )
{
	...
	const bool bForQuery = false;
	HRGN Region = MakeWindowRegionObject(bForQuery);
	if (!Region)
	{
		return;
	}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FWindowsWindow::ReshapeWindow);
...
{
	if (!GetDefinition().HasOSWindowBorder)
	{
		// Our borderless windows technically have a thick border, but we draw over them to simulate a borderless window.
		return 0;
	}

	return GetWin32BorderSize();
}

int32 FWindowsWindow::GetWin32BorderSize() const
{
	WINDOWINFO WindowInfo;
	FMemory::Memzero( WindowInfo );
	WindowInfo.cbSize = sizeof( WindowInfo );
...
bool FWindowsWindow::SupportsDynamicCursorLock() const
{
	static const TConsoleVariableData<bool>* const CVarDynamicLockToggleData = IConsoleManager::Get().FindTConsoleVariableDataBool(TEXT("WindowsCursor.DynamicLockToggle"));
	return CVarDynamicLockToggleData->GetValueOnGameThread();
}

bool FWindowsWindow::IsFullscreenSupported() const
{
#if defined(UE_WINDOWS_FORCE_NONEXCLUSIVE_FULLSCREEN) && UE_WINDOWS_FORCE_NONEXCLUSIVE_FULLSCREEN
	return false;
#else
