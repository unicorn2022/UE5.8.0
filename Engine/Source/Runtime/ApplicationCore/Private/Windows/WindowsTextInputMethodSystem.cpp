...
bool FWindowsTextInputMethodSystem::Initialize()
{
	...
	if(Result)
	{
		// Log all available input methods, to catch issues where crashes are caused by having an IME installed that's never actually activated
		{
			...
			UE_LOGF(LogWindowsTextInputMethodSystem, Log, "Available input methods:");
			for (const FString& AvailableInputMethod : AvailableInputMethods)
			{
				UE_LOGF(LogWindowsTextInputMethodSystem, Log, "  - %ls.", *AvailableInputMethod);
			}
		}
...
void FWindowsTextInputMethodSystem::LogActiveInputMethod()
{
	...
	UE_LOGF(LogWindowsTextInputMethodSystem, Log, "Activated input method: %ls.", InputMethodDescription.IsEmpty() ? TEXT("Unknown") : *InputMethodDescription);
}

bool FWindowsTextInputMethodSystem::InitializeIMM()
{
	UE_LOGF(LogWindowsTextInputMethodSystem, Verbose, "Initializing IMM...");
	...
	if (!IMMContextId)
	{
		UE_CLOGF(!GIsBuildMachine, LogWindowsTextInputMethodSystem, Warning, "Initialization failed while creating the IMM context.");
		return false;
	}

	UE_LOGF(LogWindowsTextInputMethodSystem, Verbose, "Initialized IMM!");
...
void FWindowsTextInputMethodSystem::CancelIMMComposition()
{
	...
	UE_LOGF(LogWindowsTextInputMethodSystem, Verbose, "WM_IME_COMPOSITION Composition Canceled");
...
{
	UE_LOGF(LogWindowsTextInputMethodSystem, Verbose, "Initializing TSF...");
	...
	// Input Processors
	{
		...
		if(FAILED(Result))
		{
			...
			UE_LOGF(LogWindowsTextInputMethodSystem, Error, "Initialization failed while creating the TSF input processor profiles. %ls (0x%08x)", ErrorMsg, Result);
			return false;
		}
		...
		if(FAILED(Result))
		{
			TCHAR ErrorMsg[1024];
			FPlatformMisc::GetSystemErrorMessage(ErrorMsg, 1024, Result);
			UE_LOGF(LogWindowsTextInputMethodSystem, Error, "Initialization failed while acquiring the TSF input processor profile manager. %ls (0x%08x)", ErrorMsg, Result);
	...
	// Thread Manager
	{
		...
		if(FAILED(Result))
		{
			if (!GIsBuildMachine)
			{
				TCHAR ErrorMsg[1024];
				FPlatformMisc::GetSystemErrorMessage(ErrorMsg, 1024, Result);
				UE_LOGF(LogWindowsTextInputMethodSystem, Warning, "Initialization failed while creating the TSF thread manager. %ls (0x%08x)", ErrorMsg, Result);
			}
			TSFInputProcessorProfiles.Reset();
		...
		if(FAILED(Result))
		{
			TCHAR ErrorMsg[1024];
			FPlatformMisc::GetSystemErrorMessage(ErrorMsg, 1024, Result);
			UE_LOGF(LogWindowsTextInputMethodSystem, Error, "Initialization failed while activating the TSF thread manager. %ls (0x%08x)", ErrorMsg, Result);
			TSFInputProcessorProfiles.Reset();
			TSFInputProcessorProfileManager.Reset();
		...
		if(FAILED(Result))
		{
			TCHAR ErrorMsg[1024];
			FPlatformMisc::GetSystemErrorMessage(ErrorMsg, 1024, Result);
			UE_LOGF(LogWindowsTextInputMethodSystem, Error, "Initialization failed while acquiring the TSF source from TSF thread manager. %ls (0x%08x)", ErrorMsg, Result);
			TSFInputProcessorProfiles.Reset();
			TSFInputProcessorProfileManager.Reset();
			TSFThreadManager.Reset();
			return false;
...
#pragma warning(pop)
		...
		if(FAILED(Result))
		{
			TCHAR ErrorMsg[1024];
			FPlatformMisc::GetSystemErrorMessage(ErrorMsg, 1024, Result);
			UE_LOGF(LogWindowsTextInputMethodSystem, Error, "Initialization failed while advising the profile notification sink to the TSF source. %ls (0x%08x)", ErrorMsg, Result);
			TSFInputProcessorProfiles.Reset();
			TSFInputProcessorProfileManager.Reset();
			TSFThreadManager.Reset();
	...
	if(FAILED(Result))
	{
		TCHAR ErrorMsg[1024];
		FPlatformMisc::GetSystemErrorMessage(ErrorMsg, 1024, Result);
		UE_LOGF(LogWindowsTextInputMethodSystem, Warning, "Initialization failed while creating the TSF document manager. %ls (0x%08x)", ErrorMsg, Result);
		TSFInputProcessorProfiles.Reset();
		TSFInputProcessorProfileManager.Reset();
		TSFThreadManager.Reset();
		TSFActivationProxy.Reset();
	...
	if(FAILED(Result))
	{
		TCHAR ErrorMsg[1024];
		FPlatformMisc::GetSystemErrorMessage(ErrorMsg, 1024, Result);
		UE_LOGF(LogWindowsTextInputMethodSystem, Error, "Initialization failed while activating the TSF document manager. %ls (0x%08x)", ErrorMsg, Result);
		TSFInputProcessorProfiles.Reset();
		TSFInputProcessorProfileManager.Reset();
		TSFThreadManager.Reset();
		TSFActivationProxy.Reset();
	...
	UE_LOGF(LogWindowsTextInputMethodSystem, Verbose, "Initialized TSF!");
...
void FWindowsTextInputMethodSystem::Terminate()
{
	...
	if(FAILED(Result) || !TSFSource)
	{
		TCHAR ErrorMsg[1024];
		FPlatformMisc::GetSystemErrorMessage(ErrorMsg, 1024, Result);
		UE_LOGF(LogWindowsTextInputMethodSystem, Error, "Terminating failed while acquiring the TSF source from the TSF thread manager. %ls (0x%08x)", ErrorMsg, Result);
	}
	...
	if(TSFSource && TSFActivationProxy)
	{
		...
		if(TSFActivationProxy->TSFLanguageCookie != TF_INVALID_COOKIE)
		{
			...
			if(FAILED(Result))
			{
				TCHAR ErrorMsg[1024];
				FPlatformMisc::GetSystemErrorMessage(ErrorMsg, 1024, Result);
				UE_LOGF(LogWindowsTextInputMethodSystem, Error, "Terminating failed while unadvising the language notification sink from the TSF source. %ls (0x%08x)", ErrorMsg, Result);
			}
		}
		...
		if(TSFActivationProxy->TSFProfileCookie != TF_INVALID_COOKIE)
		{
			...
			if(FAILED(Result))
			{
				TCHAR ErrorMsg[1024];
				FPlatformMisc::GetSystemErrorMessage(ErrorMsg, 1024, Result);
				UE_LOGF(LogWindowsTextInputMethodSystem, Error, "Terminating failed while unadvising the profile notification sink from the TSF source. %ls (0x%08x)", ErrorMsg, Result);
			}
		}
	}
	TSFActivationProxy.Reset();
	...
	if(FAILED(Result))
	{
		TCHAR ErrorMsg[1024];
		FPlatformMisc::GetSystemErrorMessage(ErrorMsg, 1024, Result);
		UE_LOGF(LogWindowsTextInputMethodSystem, Error, "Terminating failed while deactivating the TSF thread manager. %ls (0x%08x)", ErrorMsg, Result);
	}
...
{
	UE_LOGF(LogWindowsTextInputMethodSystem, Verbose, "Apply Defaults...");
	...
	if(ActiveContext.IsValid())
	{
		...
		if (FAILED(Result))
		{
			...
			TCHAR ErrorMsg[1024];
			FPlatformMisc::GetSystemErrorMessage(ErrorMsg, 1024, Result);
			UE_LOGF(LogWindowsTextInputMethodSystem, Error, "Getting the active TSF document manager failed, so will fallback to using the disabled document manager. %ls (0x%08x)", ErrorMsg, Result);
		}
	...
	if (FAILED(Result))
	{
		TCHAR ErrorMsg[1024];
		FPlatformMisc::GetSystemErrorMessage(ErrorMsg, 1024, Result);
		UE_LOGF(LogWindowsTextInputMethodSystem, Error, "Couldn't associate focus between window and TSF document manager, %ls (0x%08x)", ErrorMsg, Result);
	}
...
TSharedPtr<ITextInputMethodChangeNotifier> FWindowsTextInputMethodSystem::RegisterContext(const TSharedRef<ITextInputMethodContext>& Context)
{
	...
	UE_LOGF(LogWindowsTextInputMethodSystem, Verbose, "Registering context %p...", &(Context.Get()));
	...
	if(FAILED(Result) || !TextStore->TSFDocumentManager)
	{
		TCHAR ErrorMsg[1024];
		FPlatformMisc::GetSystemErrorMessage(ErrorMsg, 1024, Result);
		UE_LOGF(LogWindowsTextInputMethodSystem, Error, "Registering a context failed while creating the TSF document manager. %ls (0x%08x)", ErrorMsg, Result);
		TextStore.Reset();
		return nullptr;
	}
	...
	if(FAILED(Result) || !TextStore->TSFContext)
	{
		TCHAR ErrorMsg[1024];
		FPlatformMisc::GetSystemErrorMessage(ErrorMsg, 1024, Result);
		UE_LOGF(LogWindowsTextInputMethodSystem, Error, "Registering a context failed while creating the TSF context. %ls (0x%08x)", ErrorMsg, Result);
		TextStore.Reset();
		return nullptr;
	}
	...
	if(FAILED(Result))
	{
		TCHAR ErrorMsg[1024];
		FPlatformMisc::GetSystemErrorMessage(ErrorMsg, 1024, Result);
		UE_LOGF(LogWindowsTextInputMethodSystem, Error, "Registering a context failed while pushing a TSF context onto the TSF document manager. %ls (0x%08x)", ErrorMsg, Result);
		TextStore.Reset();
		return nullptr;
	}
	...
	if(FAILED(Result) || !TextStore->TSFContextOwnerCompositionServices)
	{
		TCHAR ErrorMsg[1024];
		FPlatformMisc::GetSystemErrorMessage(ErrorMsg, 1024, Result);
		UE_LOGF(LogWindowsTextInputMethodSystem, Error, "Registering a context failed while getting the TSF context owner composition services. %ls (0x%08x)", ErrorMsg, Result);
		Result = TextStore->TSFDocumentManager->Pop(TF_POPF_ALL);
		if(FAILED(Result))
		{
			FPlatformMisc::GetSystemErrorMessage(ErrorMsg, 1024, Result);
			UE_LOGF(LogWindowsTextInputMethodSystem, Error, "Failed to pop a TSF context off from TSF document manager while recovering from failing. %ls (0x%08x)", ErrorMsg, Result);
		}
		TextStore.Reset();
		return nullptr;
	}

	UE_LOGF(LogWindowsTextInputMethodSystem, Verbose, "Registered context %p!", &(Context.Get()));
...
void FWindowsTextInputMethodSystem::UnregisterContext(const TSharedRef<ITextInputMethodContext>& Context)
{
	...
	UE_LOGF(LogWindowsTextInputMethodSystem, Verbose, "Unregistering context %p...", &(Context.Get()));
	...
	if(FAILED(Result))
	{
		TCHAR ErrorMsg[1024];
		FPlatformMisc::GetSystemErrorMessage(ErrorMsg, 1024, Result);
		UE_LOGF(LogWindowsTextInputMethodSystem, Error, "Unregistering a context failed while popping a TSF context off from the TSF document manager. %ls (0x%08x)", ErrorMsg, Result);
	}

	ContextToInternalContextMap.Remove(Context);
	UE_LOGF(LogWindowsTextInputMethodSystem, Verbose, "Unregistered context %p!", &(Context.Get()));
}
...
void FWindowsTextInputMethodSystem::ActivateContext(const TSharedRef<ITextInputMethodContext>& Context)
{
	...
	UE_LOGF(LogWindowsTextInputMethodSystem, Verbose, "Activating context %p...", &(Context.Get()));
	HRESULT Result;
	...
	if (InternalContext.WindowHandle && ::IsWindowEnabled(InternalContext.WindowHandle))
	{
		...
		if (FAILED(Result))
		{
			TCHAR ErrorMsg[1024];
			FPlatformMisc::GetSystemErrorMessage(ErrorMsg, 1024, Result);
			UE_LOGF(LogWindowsTextInputMethodSystem, Error, "Activating a context failed while setting focus on a TSF document manager. %ls (0x%08x)", ErrorMsg, Result);
		}
		ITfDocumentMgr* Unused;
		...
		if (FAILED(Result))
		{
			TCHAR ErrorMsg[1024];
			FPlatformMisc::GetSystemErrorMessage(ErrorMsg, 1024, Result);
			UE_LOGF(LogWindowsTextInputMethodSystem, Error, "Activating a context failed while associating focus on a TSF document manager. %ls (0x%08x)", ErrorMsg, Result);
		}
		UE_LOGF(LogWindowsTextInputMethodSystem, Verbose, "Activated context %p!", &(Context.Get()));
	}
}
...
void FWindowsTextInputMethodSystem::DeactivateContext(const TSharedRef<ITextInputMethodContext>& Context)
{
	...
	{
		UE_LOGF(LogWindowsTextInputMethodSystem, Verbose, "Deactivating context %p...", &(Context.Get()));
		...
		// TSF Implementation
		HRESULT Result = TSFThreadManager->SetFocus(TSFDisabledDocumentManager);
		if (FAILED(Result))
		{
			TCHAR ErrorMsg[1024];
			FPlatformMisc::GetSystemErrorMessage(ErrorMsg, 1024, Result);
			UE_LOGF(LogWindowsTextInputMethodSystem, Error, "Deactivating a context failed while setting focus on the disabled TSF document manager. %ls (0x%08x)", ErrorMsg, Result);
		}
		...
		UE_LOGF(LogWindowsTextInputMethodSystem, Verbose, "Deactivated context %p!", &(Context.Get()));
	}
	...
	case WM_IME_SETCONTEXT:
		{
			if(ActiveContext.IsValid())
			{
				...
				UE_LOGF(LogWindowsTextInputMethodSystem, Verbose, "Setting IMM context.");
			}
	...
	case WM_IME_STARTCOMPOSITION:
		{
			if(ActiveContext.IsValid())
			{
				...
				UE_LOGF(LogWindowsTextInputMethodSystem, Verbose, "Beginning IMM composition.");
			}
	...
	case WM_IME_COMPOSITION:
		{
			if(ActiveContext.IsValid())
			{
				...
				if(bHasResultStringFlag)
				{
					...
					UE_LOGF(LogWindowsTextInputMethodSystem, Verbose, "WM_IME_COMPOSITION Result String: %ls", *ResultString);
				...
				if(bHasCompositionStringFlag)
				{
					...
					UE_LOGF(LogWindowsTextInputMethodSystem, Verbose, "WM_IME_COMPOSITION Composition String: %ls", *CompositionString);
				...
				if(!bHasNoMoveCaretFlag && bHasCursorPosFlag)
				{
					...
					// Update Cursor
					UE_LOGF(LogWindowsTextInputMethodSystem, Verbose, "WM_IME_COMPOSITION Cursor Position: %d", CursorPosition);
				...
				UE_LOGF(LogWindowsTextInputMethodSystem, Verbose, "Updating IMM composition.");
			}
	...
	case WM_IME_ENDCOMPOSITION:
		{
			...
			if(ActiveContext.IsValid())
			{
				...
				UE_LOGF(LogWindowsTextInputMethodSystem, Verbose, "Ending IMM composition.");
			}
	...
	case WM_IME_CHAR:
		{
			...
			UE_LOGF(LogWindowsTextInputMethodSystem, Verbose, "Ignoring WM_IME_CHAR message.");
			return 0;
		}
	default:
		{
			UE_LOGF(LogWindowsTextInputMethodSystem, Warning, "Unexpected windows message received for processing.");
