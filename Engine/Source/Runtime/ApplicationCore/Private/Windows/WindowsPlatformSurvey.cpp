...
bool FWindowsPlatformSurvey::GetSurveyResults( FHardwareSurveyResults& OutResults, bool bWait )
{
	...
	else
	{
		...
		UE_LOGF(LogWindows, Warning, "FWindowsPlatformSurvey::TickSurveyHardware() failed to get Unreal Engine root-folder drive size from Win32" );
		OutResults.ErrorCount++;
	...
	{
		uint32 ErrorCode = FPlatformMisc::GetLastError();
		UE_LOGF(LogWindows, Warning, "FWindowsPlatformSurvey::TickSurveyHardware() failed to get locale info from Win32" );
		OutResults.ErrorCount++;
	...
	if (FAILED(COMResult))
	{
		UE_LOGF(LogWindows, Warning, "FWindowsPlatformSurvey::TickSurveyHardware() failed to get query interface from WinSAT API" );
		OutResults.ErrorCount++;
	...
	else
	{
		...
		if (FAILED(COMResult))
		{
			UE_LOGF(LogWindows, Error, "FWindowsPlatformSurvey::TickSurveyHardware() failed to get assessment results from WinSAT API" );
			OutResults.ErrorCount++;
		...
		else
		{
			...
			if (FAILED(COMResult))
			{
				UE_LOGF(LogWindows, Error, "FWindowsPlatformSurvey::TickSurveyHardware() failed to get assessment state from WinSAT API" );
				OutResults.ErrorCount++;
			...
			else
			{
				...
				case WINSAT_ASSESSMENT_STATE_INCOHERENT_WITH_HARDWARE:
					UE_LOGF(LogWindows, Log, "FWindowsPlatformSurvey::TickSurveyHardware() WinSAT assessment state is out-of-date. Unable to examine some hardware metrics. Run the Windows Experience Index Assessment." );
					OutResults.ErrorCount++;
				...
				case WINSAT_ASSESSMENT_STATE_NOT_AVAILABLE:
					UE_LOGF(LogWindows, Log, "FWindowsPlatformSurvey::TickSurveyHardware() WinSAT assessment unavailable. Unable to examine some hardware metrics. Run the Windows Experience Index Assessment." );
					OutResults.ErrorCount++;
				...
				default:
					UE_LOGF(LogWindows, Warning, "FWindowsPlatformSurvey::TickSurveyHardware() WinSAT assessment data was invalid." );
					OutResults.ErrorCount++;
				...
				if (bAssessmentAvailable)
				{
					...
					{
						UE_LOGF(LogWindows, Error, "FWindowsPlatformSurvey::TickSurveyHardware() failed to get MEMORY score from WinSAT API." );
					}

					if (!GetSubComponentIndex(WinSATResults, OutResults, WINSAT_ASSESSMENT_CPU, OutResults.CPUPerformanceIndex))
					{
						UE_LOGF(LogWindows, Error, "FWindowsPlatformSurvey::TickSurveyHardware() failed to get CPU score from WinSAT API." );
					}

					float GPU3DScoreIndex = 0.0f;
					if (!GetSubComponentIndex(WinSATResults, OutResults, WINSAT_ASSESSMENT_D3D, GPU3DScoreIndex))
					{
						UE_LOGF(LogWindows, Error, "FWindowsPlatformSurvey::TickSurveyHardware() failed to get D3D score from WinSAT API." );
					}
					...
					{
						UE_LOGF(LogWindows, Error, "FWindowsPlatformSurvey::TickSurveyHardware() failed to get GRAPHICS score from WinSAT API." );
					}
	...
	{
		UE_LOGF(LogWindows, Warning, "FWindowsPlatformSurvey::TickSurveyHardware() failed to get system power capabilities. Assuming desktop PC." );
		OutResults.ErrorCount++;
...
bool FWindowsPlatformSurvey::GetSubComponentIndex( IProvideWinSATResultsInfo* WinSATResults, FHardwareSurveyResults& OutSurveyResults, int32 SubComponent, float& OutSubComponentIndex ) 
{
	...
	if (FAILED(COMResult))
	{
		UE_LOGF(LogWindows, Log, "FWindowsPlatformSurvey::GetSubComponentIndex() failed to get assessment info for a sub-component from WinSAT API." );
	...
	else
	{
		...
		if (FAILED(COMResult))
		{
			UE_LOGF(LogWindows, Log, "FWindowsPlatformSurvey::GetSubComponentIndex() failed to get sub-component score from WinSAT API." );
			OutSurveyResults.ErrorCount++;
...
#pragma warning(pop)
		...
		UE_LOGF( LogWindows, Warning, "FWindowsPlatformSurvey::GetOSVersionLabel() unknown Windows version info from GetVersionEx()" );
		OutResults.ErrorCount++;
	...
	if( ErrorCode & FWindowsOSVersionHelper::ERROR_GETVERSIONEX_FAILED )
	{
		...
		UE_LOGF( LogWindows, Warning, "FWindowsPlatformSurvey::GetOSVersionLabel() failed to get Windows version info from GetVersionEx()" );
		OutResults.ErrorCount++;
