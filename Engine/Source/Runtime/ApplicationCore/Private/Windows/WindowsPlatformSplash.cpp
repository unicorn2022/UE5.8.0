...
static TAutoConsoleVariable<bool> CVarWindowsApplicationShowSplash(
	TEXT("WindowsApplication.ShowSplash"),
	true,
	TEXT("Whether to show the splash screen on Windows (default = true).")
);
...
static FString GSplashScreenFileNames[ SplashImageType::NumImageTypes ];
static HBITMAP GSplashScreenBitmaps[ SplashImageType::NumImageTypes ];
static FIntPoint GSplashScreenPos[ SplashImageType::NumImageTypes ];
...
static ESplashTextSizeType GSplashScreenTextSizes[ SplashTextType::NumTextTypes ];
static ESplashTextBrightnessType GSplashScreenTextBrightness[ SplashTextType::NumTextTypes ];
static ESplashTextFlags GSplashScreenTextFlags[ SplashTextType::NumTextTypes ];
		...
		case WM_PAINT:
			{
				...
				if (!GSplashScreenBitmaps[SplashImageType::Background])
				{
					return 0;
				}
				...
				{
					// Take a critical section since another thread may be trying to set a splash text or image
					FScopeLock ScopeLock( &GSplashScreenSynchronizationObject );

					// Draw splash bitmap
					for (int32 i = 0; i < SplashImageType::NumImageTypes; ++i)
					{
						if (HBITMAP BitmapHandle = GSplashScreenBitmaps[i])
						{
							const FIntPoint Pos = GSplashScreenPos[i];
							DrawState(hdc, DSS_NORMAL, NULL, (LPARAM)BitmapHandle, 0, Pos.X, Pos.Y, 0, 0, DST_BITMAP);
						}
					}
					...
					for( int32 CurTypeIndex = 0; CurTypeIndex < SplashTextType::NumTextTypes; ++CurTypeIndex )
					{
						...
						{
							if ( GSplashScreenTextSizes[CurTypeIndex] == ESplashTextSizeType::Normal )
							{
								SelectObject( hdc, GSplashScreenNormalTextFontHandle );
							}
							else if ( GSplashScreenTextSizes[CurTypeIndex] == ESplashTextSizeType::Title )
							{
							...
							FString String = SplashText.ToString();
							const ESplashTextFlags TextFlags = GSplashScreenTextFlags[CurTypeIndex];
							...
							for( int32 CurBGPass = 0; CurBGPass < NumBGPasses; ++CurBGPass )
							{
								...
								RECT OffsetRect = RECT(
									...
									TextRect.right + BGXOffset,
									TextRect.bottom + BGYOffset
								);

								if (EnumHasAllFlags(TextFlags, ESplashTextFlags::WordWrap))
								{
									DrawTextW(hdc, *String, String.Len(), &OffsetRect, DT_WORDBREAK | DT_LEFT | DT_TOP | DT_NOPREFIX);
								}
								else
								{
									TextOut(hdc, OffsetRect.left, OffsetRect.top, *String, String.Len());
								}
							}
#endif // UE_WINDOWS_SPLASH_USE_TEXT_OUTLINE
							
							// Draw foreground text pass
							if( GSplashScreenTextBrightness[CurTypeIndex] == ESplashTextBrightnessType::Low )
							{
								SetTextColor( hdc, RGB(160, 160, 160) );
							}
							else if( GSplashScreenTextBrightness[CurTypeIndex] == ESplashTextBrightnessType::Medium )
							{
							...
							if (EnumHasAllFlags(TextFlags, ESplashTextFlags::WordWrap))
							{
								...
								DrawTextW(hdc, *String, String.Len(), &DrawRect, DT_WORDBREAK | DT_LEFT | DT_TOP);
							}
							else
							{
								TextOut(hdc, TextRect.left, TextRect.top, *String, String.Len());
							}
						}
					}
				}
 ...
 */
HBITMAP LoadSplashBitmap(SplashImageType::Type Type)
{
	if (GSplashScreenFileNames[Type].IsEmpty())
	{
		return nullptr;
	}
	...
	hr = Factory->CreateDecoderFromFilename(
		(LPCTSTR)*GSplashScreenFileNames[Type], // Image to be decoded
...
uint32 WINAPI StartSplashScreenThread( LPVOID unused )
{
	...
	const bool bAllowFading = false;
	{
		...
		for (int32 i = 0; i < SplashImageType::NumImageTypes; ++i)
		{
			GSplashScreenBitmaps[i] = LoadSplashBitmap(static_cast<SplashImageType::Type>(i));
		}

		if (GSplashScreenBitmaps[SplashImageType::Background])
		{
			GetObjectW(GSplashScreenBitmaps[SplashImageType::Background], sizeof(bm), &bm);
			...
			// Enable W11 window style (drop shadow, thin border, slightly rounded)
			constexpr DWM_WINDOW_CORNER_PREFERENCE Corners = DWMWCP_ROUND;
			DwmSetWindowAttribute(GSplashScreenWnd, DWMWA_WINDOW_CORNER_PREFERENCE, &Corners, sizeof(Corners));
#endif
		}
	}
	...
	{
		check(GSplashScreenBitmaps[SplashImageType::Background]);
		if (bAllowFading)
		{
		...
		GSplashScreenTextSizes[ SplashTextType::GameName ] = ESplashTextSizeType::Title;
		GSplashScreenTextBrightness[ SplashTextType::GameName ] = ESplashTextBrightnessType::High;
		...
		GSplashScreenTextSizes[ SplashTextType::VersionInfo1 ] = ESplashTextSizeType::Normal;
		GSplashScreenTextBrightness[ SplashTextType::VersionInfo1 ] = ESplashTextBrightnessType::Low;
		...
		GSplashScreenTextSizes[ SplashTextType::CopyrightInfo ] = ESplashTextSizeType::Small;
		GSplashScreenTextBrightness[ SplashTextType::CopyrightInfo ] = ESplashTextBrightnessType::Low;
		...
		GSplashScreenTextSizes[ SplashTextType::StartupProgress ] = ESplashTextSizeType::Normal;
		GSplashScreenTextBrightness[ SplashTextType::StartupProgress ] = ESplashTextBrightnessType::Low;
		...
		for (int32 i = 0; i < SplashImageType::NumImageTypes; ++i)
		{
			if (GSplashScreenBitmaps[i])
			{
				ensure(DeleteObject(GSplashScreenBitmaps[i]));
				GSplashScreenBitmaps[i] = NULL;
			}
		}
	}
...
{
	if(!GSplashScreenThread && FParse::Param(FCommandLine::Get(),TEXT("NOSPLASH")) != true && CVarWindowsApplicationShowSplash->GetBool())
	{
		...
		if ( GetSplashPath(SplashImage, SplashPath, IsCustom ) == true )
		{
			...
			GSplashScreenFileNames[SplashImageType::Background] = MoveTemp(SplashPath);
			DWORD ThreadID = 0;
...
void FWindowsPlatformSplash::RegisterUserSplashText(const SplashTextType::Type InType, const FPlatformRect& InRect,
	const ESplashTextSizeType InSize, const ESplashTextBrightnessType InBrightness, const ESplashTextFlags InFlags)
{
	check(InType >= SplashTextType::Type::User1);
	check(!GSplashScreenThread);

	GSplashScreenTextRects[InType] = RECT(InRect.Left, InRect.Top, InRect.Right, InRect.Bottom);
	GSplashScreenTextSizes[InType] = InSize;
	GSplashScreenTextBrightness[InType] = InBrightness;
	GSplashScreenTextFlags[InType] = InFlags;
}

void FWindowsPlatformSplash::RegisterUserSplashImage(const SplashImageType::Type InType, const FIntPoint& InPos,
	const TCHAR* InFilename)
{
	check(InType >= SplashImageType::User1);
	check(!GSplashScreenThread);

	bool bCustom;
	GetSplashPath(InFilename, GSplashScreenFileNames[InType], bCustom, false);
	GSplashScreenPos[InType] = InPos;
}
...
void FWindowsPlatformSplash::SetSplashImage(const SplashImageType::Type InType, const TCHAR* InFilename)
{
	if (GSplashScreenThread)
	{
		const HBITMAP ImageHandle = LoadSplashBitmap(InType);
		if (!ImageHandle)
		{
			return;
		}

		// Take a critical section since the splash thread may already be repainting using this image
		FScopeLock ScopeLock(&GSplashScreenSynchronizationObject);

        if (GSplashScreenBitmaps[InType])
        {
            ensure(DeleteObject(GSplashScreenBitmaps[InType]));
        }

		GSplashScreenBitmaps[InType] = ImageHandle;

		SIZE Size;
		if (GetBitmapDimensionEx(ImageHandle, &Size))
		{
			// Repaint the window
			RECT Rect;
			Rect.left = GSplashScreenPos[InType].X;
			Rect.top = GSplashScreenPos[InType].Y;
			Rect.right = GSplashScreenPos[InType].X + Size.cx;
			Rect.bottom = GSplashScreenPos[InType].Y + Size.cy;

			constexpr BOOL bErase = false;
			InvalidateRect(GSplashScreenWnd, &Rect, bErase);
		}
	}
	else // No drawing is running yet so we can just update the path here
	{
		bool bCustom;
		GetSplashPath(InFilename, GSplashScreenFileNames[InType], bCustom, false);
	}
