...
public:
	...
	void CreateColors()
	{
		...
		{
			FString Filename = "DebugConsoleColors.ini";
	...
	LRESULT MainWinProc(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_COMMAND:
			...
			else if (LOWORD(wParam) == ID_COMMANDBUTTON)
			{
				...
				// Note: This will execute during the CoreTicker part of the EngineLoop (after Engine Tick is complete).
				// This decision was made to avoid the previous Async implementation which ticks during Engine/World Tick (TG_PrePhysics).
				// The reasoning is that the new MultiServer Proxy does not execute TickGroups (UE_SUPPORT_FOR_ACTOR_TICK_DISABLE).
				ExecuteOnGameThread(UE_SOURCE_LOCATION, [Cmd = FString(Command)]()
					{
						TArray<IConsoleCommandExecutor*> CmdExecutors = IModularFeatures::Get().GetModularFeatureImplementations<IConsoleCommandExecutor>(IConsoleCommandExecutor::ModularFeatureName());
						if (IConsoleCommandExecutor** CmdExecutor = CmdExecutors.FindByPredicate([](IConsoleCommandExecutor* CmdExecutor) { return CmdExecutor->GetName() == NAME_Cmd; }))
						{
							UE_LOGF(LogExec, Log, "Executing console command: %ls", *Cmd);
							(*CmdExecutor)->Exec(*Cmd);
						}
						else
						{
							UE_LOGF(LogExec, Log, "Failed to execute console command: %ls", *Cmd);
						}
					});
			}
			...
			else if (LOWORD(wParam) == ID_ADDCHECKPOINTBUTTON)
			{
				...
				AddLogEntry(TempString, ELogVerbosity::Display, FName("LogCheckpoint"), 0, FOREGROUND_BLUE | FOREGROUND_GREEN | FOREGROUND_RED);
...
FName AppStatusTrackerName("AppStatusTracker");
