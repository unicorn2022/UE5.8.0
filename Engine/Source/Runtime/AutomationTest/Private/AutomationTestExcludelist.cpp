...
namespace
{
	const FString FunctionalTestsPreFix = "Project.Functional Tests.";
	...
	const FString TicketTrackerURLHashtagPropertyName = "URLHashtag";
	const FString TicketTrackerURLBasePropertyName = "URLBase";
} // anonymous namespace
...
{
	static const FString DefaultTaskTrackerTagSuffix = "unknown";
	...
	if (ISourceControlModule::Get().IsEnabled())
	{
		...
		if (SourceControlState.IsValid())
		{
			if (SourceControlState->IsSourceControlled())
			{
				...
				{
					UE_LOGF(LogAutomationTestExcludelist, Error, "The configuration file is marked for deletion.");
				}
				else if (SourceControlState->CanCheckout() || SourceControlState->IsCheckedOutOther() || FPlatformFileManager::Get().GetPlatformFile().IsReadOnly(*InFileToCheckOut))
				{
					...
					{
						UE_LOGF(LogAutomationTestExcludelist, Error, "Failed to check out the configuration file.");
					}
					else if (CommandResult == ECommandResult::Cancelled)
					{
						UE_LOGF(LogAutomationTestExcludelist, Warning, "Checkout was cancelled.");
					}
					else
					{
				...
				else if (SourceControlState->CanAdd() || SourceControlState->IsUnknown())
				{
					...
					if (CommandResult == ECommandResult::Failed)
					{
						UE_LOGF(LogAutomationTestExcludelist, Error, "Failed to mark for add the configuration file.");
					}
					else if (CommandResult == ECommandResult::Cancelled)
					{
						UE_LOGF(LogAutomationTestExcludelist, Warning, "Mark for add was cancelled.");
					}
					else
					{
						bSuccessfullyCheckedOutOrAddedFile = true;
...
void UAutomationTestExcludelistConfig::SaveConfig()
{
	...
	if (bIsFileExists && !bIsWritable)
	{
		...
		if (!bIsWritable)
		{
			UE_LOGF(LogAutomationTestExcludelist, Warning, "Config file '%ls' is readonly and could not be checked out. File will be marked writable.", *ConfigFilename);
	...
	if (bIsFileExists && !bIsWritable)
	{
		UE_LOGF(LogAutomationTestExcludelist, Error, "Failed to make the configuration file '%ls' writable.", *ConfigFilename);
	}
	else
#endif
	{
