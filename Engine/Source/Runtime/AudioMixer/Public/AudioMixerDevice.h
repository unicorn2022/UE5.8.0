...
namespace Audio
{
	AUDIOMIXER_API extern int32 GRemoveSubmixesFromRoot;
	...
	public:
		...
		AUDIOMIXER_API virtual void FlushAudioRenderingCommands(bool bPumpSynchronously = false, bool bRenderThreadStopped = false) override;
		...
		void UpdateSubmixDebugVolumes();
#endif // ENABLE_AUDIO_DEBUG

		bool UseRenderScheduler() const
		{
			return bUseRenderScheduler;
		}
		...
		AUDIOMIXER_API TUniquePtr<IAudioMixerGeneratorSource> CreateGeneratorSource(ISoundGeneratorRef InGenerator);
	...
	private:
		...
		void InitializeMainSubmixTruePeakEnvelopeFollower(int32 InSampleRate, int32 InNumChannels, int32 InNumSamples);
		void InitializeMainSubmixLKFSAnalyzer(int32 InSampleRate, int32 InNumChannels);

		void SendMainSubmixLoadedMessage();
		void SendMainSubmixAlivePingMessage();

		void UpdateAndSendMainSubmixTruePeakMaxValueMessage(const FAlignedFloatBuffer& InMainSubmixAudioBuffer);
		void UpdateAndSendMainSubmixLKFSValuesMessage(const FAlignedFloatBuffer& InMainSubmixAudioBuffer);
	
		void SendSubmixesAlivePing();
		void UpdateSubmixesAlivePingTime();
		void SendSubmixesSendLevelTraceMessages();
		...
		void StartSubmixEnvelopeFollowing(const FMixerSubmixPtr& InSubmix);
		void StopSubmixEnvelopeFollowing(const FMixerSubmixPtr& InSubmix);

		void AutomaticallyStartSubmixEnvelopeFollowers();

		void RegisterResetMainSubmixAnalyzersCmd();
		void UnregisterResetMainSubmixAnalyzersCmd();

		void OnResetMainSubmixAnalyzersCmdInvoked();

		void ResetMainSubmixAnalyzers();
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
		...
		/** Submix Buffer Listeners which are trying to listen to submixes which have not been registered yet */
		TMap<FString, TMap<FString, TSharedRef<ISubmixBufferListener, ESPMode::ThreadSafe>>> PendingSubmixBufferListeners;
		...
		TSpscQueue<FMixerSourceVoice*> SourceVoices;
		...
		TMpscQueue<TFunction<void()>> CommandQueue;
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
		TSet<uint32> SubmixesWithAutoEnvelopeFollowers;
		TSet<FString> SubmixesWithCVarEnvelopeFollowers;

		IConsoleVariable* StartSubmixEnvelopeFollowerCVar = nullptr;
		IConsoleVariable* StopSubmixEnvelopeFollowerCVar  = nullptr;
		IConsoleObject*   ResetMainSubmixAnalyzersCmd     = nullptr;

		TUniquePtr<FEnvelopeFollower> MainSubmixTruePeakEnvelopeFollower;
		FAlignedFloatBuffer MainSubmixTruePeakOutputEnvelope;

		TUniquePtr<FLKFSAnalyzer> MainSubmixLKFSAnalyzer;
	...
	public:
		...
		UE_DEPRECATED(5.8, "Synchronized audio task queues are deprecated.")
