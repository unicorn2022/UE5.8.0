...
namespace Audio
{
	...
	// Consolidated filter settings for a single bus/submix send.
	// Passed between the game thread and render thread as a simple value type.
	struct FSendFilterSettings
	{
		uint8 bEnableLPF : 1 = 0;
		uint8 bEnableHPF : 1 = 0;
		float LpfCutoff = 20000.0f;
		float HpfCutoff = 20.0f;

		FSendFilterSettings() = default;
		FSendFilterSettings(bool bInEnableLPF, bool bInEnableHPF, float InLpfCutoff, float InHpfCutoff)
			: LpfCutoff(InLpfCutoff), HpfCutoff(InHpfCutoff)
		{
			bEnableLPF = bInEnableLPF;
			bEnableHPF = bInEnableHPF;
		}

		bool IsFiltering() const { return bEnableLPF || bEnableHPF; }

		// Returns true if any filter parameter differs significantly from Other.
		// Use to determine whether a render-thread update is needed.
		bool HasChanged(const FSendFilterSettings& Other) const
		{
			return bEnableLPF != Other.bEnableLPF
				|| bEnableHPF != Other.bEnableHPF
				|| (bEnableLPF && !FMath::IsNearlyEqual(LpfCutoff, Other.LpfCutoff))
				|| (bEnableHPF && !FMath::IsNearlyEqual(HpfCutoff, Other.HpfCutoff));
		}
	};
	...
	private:
		...
		void AddInstanceId(const int32 InSourceInstanceId, const uint32 WaveInstancePlayOrder, int32 InNumOutputChannels);

		// Removes the source id from this bus. Returns true if there are no more instances or sends.
		bool RemoveInstanceId(const int32 InSourceId, const uint32 WaveInstancePlayOrder);
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
		bool StartEnvelopeFollower(const float InAttackTime, const float InReleaseTime, const float InSampleRate);
		bool StopEnvelopeFollower();
		...
		// Scratch buffer used for intermediate filter output in MixBuffer(). Kept as a member so its
		// heap allocation persists across render callbacks and avoids per-frame reallocation.
		...
		int32 EnvelopeFollowerRequestsRefCount = 0;
