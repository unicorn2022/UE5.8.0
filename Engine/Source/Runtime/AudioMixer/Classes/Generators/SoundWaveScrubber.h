...
public:
	...
	UE_API void Init(TSharedRef<const FSoundWaveProxy> InSoundWaveProxyRef, float InSampleRate, int32 InNumChannels, float InPlayheadTimeSeconds = 0.0f);

	UE_DEPRECATED(5.8, "Please use the TSharedRef<const FSoundWaveProxy> overload instead")
...
private:
	...
	// Sound wave data to safely query and use the referenced sound wave asset
	TSharedPtr<const FSoundWaveData> SoundWaveDataPtr;
	// Sound wave proxy player is a decoder we can use to decode audio into chunks
