...
namespace Audio
{
	...
	struct FMixerSourceSubmixSend
	{
		...
		// Cached submix ID — avoids atomic Pin() calls in the render-thread hot path
		uint32 SubmixId = TNumericLimits<uint32>::Max();
	...
	struct FMixerSourceVoiceInitParams
	{
		...
		bool bHasAttenuationSubmixSends = false;
	...
	public:
		...
		void MixOutputBuffers(const int32 SourceId, int32 InNumOutputChannels, const float InSendLevel, EMixerSourceSubmixSendStage InSubmixSendStage, uint32 InSubmixId, FAlignedFloatBuffer& ScratchBuffer, FAlignedFloatBuffer& OutWetBuffer);
		...
		void SetBusSendInfo(const int32 SourceId, EBusSendType InAudioBusSendType, uint32 AudiobusId, float BusSendLevel, const FString& InBusName, const FSendFilterSettings& InFilter = FSendFilterSettings{});
		...
		const float* GetPostDistanceAttenuationBuffer(const int32 SourceId) const;
		...
		void FlushCommandQueue(bool bPumpCommandQueue = false, bool bSkipWaitForRenderThread = false);
		...
		void AddPendingAudioBusConnection(FAudioBusKey AudioBusKey, int32 NumChannels, bool bIsAutomatic, FPatchInput PatchInput, uint32 WaveInstancePlayOrder = INDEX_NONE, const uint64 InstanceID = 0u)
		{
			PendingAudioBusConnections.Enqueue(FPendingAudioBusConnection{ FPendingAudioBusConnection::FPatchVariant(TInPlaceType<FPatchInput>()
												, MoveTemp(PatchInput))
												, MoveTemp(AudioBusKey)
												, NumChannels
												, bIsAutomatic
												, WaveInstancePlayOrder
#if	UE_AUDIO_PROFILERTRACE_ENABLED
												, InstanceID
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
			});
		...
		void AddPendingAudioBusConnection(FAudioBusKey AudioBusKey, int32 NumChannels, bool bIsAutomatic, FPatchOutputStrongPtr PatchOutputStrongPtr, uint32 WaveInstancePlayOrder = INDEX_NONE, const uint64 InstanceID = 0u)
		{
			PendingAudioBusConnections.Enqueue(FPendingAudioBusConnection{ FPendingAudioBusConnection::FPatchVariant(TInPlaceType<FPatchOutputStrongPtr>()
												, MoveTemp(PatchOutputStrongPtr))
												, MoveTemp(AudioBusKey)
												, NumChannels
												, bIsAutomatic
												, WaveInstancePlayOrder
		...
		bool UseRenderScheduler() const
		{
			return MixerDevice->UseRenderScheduler();
		}
		...
		const bool CheckAndResetHardwareCallbackStallFlag()
		{
			bool bShouldResetTemp = bShouldResetHardwareCallback;
			bShouldResetHardwareCallback = false;
			return bShouldResetTemp; 
		}

		int32 AllocateGeneratorSource(ISoundGeneratorRef InGenerator);
		bool IsGeneratorSourceDone(const int32 SourceId);
		void FreeGeneratorSource(const int32 SourceId);
		void PlayGeneratorSource(const int32 SourceId);
		void StopGeneratorSource(const int32 SourceId);
...
#define INVALID_AUDIO_RENDER_THREAD_ID static_cast<uint32>(-1)
		...
		void SetSourceDone(const int32 SourceId);
		...
		void ComputeBuses();
		void UpdateBuses();
		void AddBusPatchDependency(const uint32 WaveInstancePlayOrder, const FAudioRenderStepId FirstStep, const FAudioRenderStepId SecondStep);
		void RemoveBusPatchDependencies(const uint32 WaveInstancePlayOrder);
		...
		bool bShouldResetHardwareCallback = false;
		...
		struct FSourceInfo : public IAudioMixerRenderStep
		{
			...
			Audio::FAlignedFloatBuffer PostDistanceAttenuationBuffer;
			...
			uint8 bHasAttenuationSubmixSends : 1;
			uint8 bIsVorbis:1;
		...
		void AutomaticallyStartAudioBusEnvelopeFollower(const FAudioBusKey& InAudioBusKey, const TSharedPtr<FMixerAudioBus>& InAudioBus);
		void StopAutoAudioBusEnvelopeFollower(const uint32 InAudioBusId, FMixerAudioBus& InAudioBus);

		void TraceSourceToSubmixSendMessage(const FSourceInfo& SourceInfo) const;
		void TraceSourceToBusSendMessage(const uint32 SourceId, const FSourceInfo& SourceInfo) const;

		uint32 ResolvePlayOrder(const FPendingAudioBusConnection& Connection) const;

		IConsoleVariable* StartAudioBusEnvelopeFollowerCVar = nullptr;
		IConsoleVariable* StopAudioBusEnvelopeFollowerCVar = nullptr;

		TSet<uint32> AudioBusesWithAutoEnvelopeFollowers;

		double BusIsAlivePingTimeSec = 0.0;
		static constexpr double BusIsAlivePingWaitTimeSec = 1.0;

		// Audio render thread only
		TMultiMap<uint32, uint32> ActiveBusPatchWriters;  // WaveInstancePlayOrder -> BusObjectId (source->bus, writer)
		TMultiMap<uint32, uint32> ActiveBusPatchReaders;  // WaveInstancePlayOrder -> BusObjectId (bus->source, reader)
		TMap<uint64, uint32> InstanceIdToPlayOrderMap;    // MixerSourceBuffer InstanceID -> WaveInstancePlayOrder
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
		...
		int32 NumLegacySources;
		int32 NumGeneratorSources;
		int32 NumOutputFrames;
		int32 NumOutputSamples;

		struct FGeneratorSourceState
		{
			// Generator used for rendering, owned by render thread.
			ISoundGeneratorPtr Generator;

			// Set on game thread, cleared from render thread when rendering is complete
			std::atomic<uint8> bSourceBusy = false;

			// Is this voice allocated on the game thread?
			uint8 bAllocated:1 = false;
		};

		TArray<FGeneratorSourceState> GeneratorSourceState;
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
			uint64 InstanceID = 0u; // MixerSourceBuffer instance ID, used for PlayOrder resolution in trace profiling
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
		};
