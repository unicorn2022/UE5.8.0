...
void USynthComponent::Initialize(int32 SampleRateOverride)
{
	...
	if (SampleRate != INDEX_NONE)
	{
		...
		{
			UE_LOGF(LogAudioMixer, Error, "Synthesis component '%ls' has set an invalid channel count '%d'.", *GetName(), NumChannels);
		}
...
void USynthComponent::SynthCommand(TFunction<void()> Command)
{
	...
	{
		UE_LOGF(LogAudioMixer, Error, "Synthesis component '%ls' has implemented a sound generator interface. Do not call SynthCommand on the USynthComponent).", *GetName());
	}
	else
	{
