...
namespace Audio
{
	...
	FMixerBuffer::~FMixerBuffer()
	{
		...
		{
			UE_LOGF(LogAudioMixer, Fatal, "Can't free resource '%ls' as it was allocated in permanent pool.", *ResourceName);
		}
	...
	bool FMixerBuffer::ReadCompressedInfo(USoundWave* SoundWave)
	{
		if (!DecompressionState)
		{
			UE_LOGF(LogAudioMixer, Warning, "Attempting to read compressed info without a compression state instance for resource '%ls'", *ResourceName);
			return false;
		}
		...
		{
			UE_LOGF(LogAudioMixer, Warning, "Failed to read compressed info of '%ls' because there was no resource data or invalid resource size.", *ResourceName);
			return false;
		}
		...
		{
			UE_LOGF(LogAudioMixer, Warning, "Failed to read compressed info of '%ls'.", *ResourceName);
		}
		return false;
	}
	...
	FMixerBuffer* FMixerBuffer::CreateStreamingBuffer(FAudioDevice* AudioDevice, USoundWave* InWave)
	{
		...
		if (InWave->HasError())
		{
			UE_LOGF(LogAudioMixer, VeryVerbose, "FMixerBuffer::CreateStreamingBuffer, ignoring '%ls' as it contains previously seen errors", *InWave->GetName() );
			return nullptr;
		}
		...
		Buffer->DecompressionState = IAudioInfoFactoryRegistry::Get().Create(InWave->GetRuntimeFormat(), InWave->GetFName());
		...
		else
		{
			...
			if (InWave && !InWave->IsSeekable())
			{
				UE_LOGF(LogAudioMixer, Warning,
					"FMixerBuffer::CreateStreamingBuffer failed to StreamCompressedInfo on SoundWave '%ls'.  Invalidating wave resource data (asset now requires re-cook).",
					*InWave->GetName());
