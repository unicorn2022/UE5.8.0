...
static int32 SubmixBufferListenerForceRegistrationCVar = 1;
FAutoConsoleVariableRef CVarSubmixBufferListenerForceRegistration(
	TEXT("au.SubmixBufferListenerForceRegistration"),
	SubmixBufferListenerForceRegistrationCVar,
	TEXT("If true, will force register the referenced submix when Registering a Submix Buffer Listener."),
	ECVF_Default);

static int32 AudioMixerGeneratorSourcesCVar = 0;
FAutoConsoleVariableRef CVarAudioMixerGeneratorSources(
	TEXT("au.NumGeneratorSources"),
	AudioMixerGeneratorSourcesCVar,
	TEXT("Sets number of generator sources available in the AudioMixer."),
	ECVF_Default);

#if UE_AUDIO_PROFILERTRACE_ENABLED

UE_TRACE_EVENT_BEGIN(Audio, MainSubmixLoaded)
	UE_TRACE_EVENT_FIELD(uint32, DeviceId)
	UE_TRACE_EVENT_FIELD(uint32, SubmixId)
	UE_TRACE_EVENT_FIELD(uint64, Timestamp)
UE_TRACE_EVENT_END()

UE_TRACE_EVENT_BEGIN(Audio, MainSubmixAlivePing)
	UE_TRACE_EVENT_FIELD(uint32, DeviceId)
	UE_TRACE_EVENT_FIELD(uint32, SubmixId)
	UE_TRACE_EVENT_FIELD(uint64, Timestamp)
UE_TRACE_EVENT_END()

UE_TRACE_EVENT_BEGIN(Audio, MainSubmixTruePeakMaxValue)
	UE_TRACE_EVENT_FIELD(uint32, DeviceId)
	UE_TRACE_EVENT_FIELD(uint32, SubmixId)
	UE_TRACE_EVENT_FIELD(uint64, Timestamp)
	UE_TRACE_EVENT_FIELD(float, TruePeakMaxValueDb)
UE_TRACE_EVENT_END()

UE_TRACE_EVENT_BEGIN(Audio, MainSubmixLKFSValues)
	UE_TRACE_EVENT_FIELD(uint32, DeviceId)
	UE_TRACE_EVENT_FIELD(uint32, SubmixId)
	UE_TRACE_EVENT_FIELD(uint64, Timestamp)
	UE_TRACE_EVENT_FIELD(float, LongTermLoudness)
	UE_TRACE_EVENT_FIELD(float, ShortTermLoudness)
	UE_TRACE_EVENT_FIELD(float, MomentaryLoudness)
	UE_TRACE_EVENT_FIELD(float, LoudnessRangeLowerBound)
	UE_TRACE_EVENT_FIELD(float, LoudnessRangeUpperBound)
UE_TRACE_EVENT_END()
...
UE_TRACE_EVENT_BEGIN(Audio, SubmixToSubmixSend)
	UE_TRACE_EVENT_FIELD(uint32, DeviceId)
	UE_TRACE_EVENT_FIELD(uint64, Timestamp)
	UE_TRACE_EVENT_FIELD(uint32, SenderID)
	UE_TRACE_EVENT_FIELD(uint32, ReceiverID)
	UE_TRACE_EVENT_FIELD(float, SendLevel)
	UE_TRACE_EVENT_FIELD(bool, IsFinalStage)
UE_TRACE_EVENT_END()
...
namespace Audio
{
	int32 GRemoveSubmixesFromRoot = 1;
	FAutoConsoleVariableRef CVarRemoveSubmixesFromRoot(
		TEXT("au.RemoveSubmixesFromRoot"),
		GRemoveSubmixesFromRoot,
		TEXT("Submixes that are registered will use a ref counter. When unregistering, the ref counter is decremented. When no longer referenced, the submixes are removed from root.\n")
		TEXT("1: Submixes are removed from root when no longer referenced. 0: Submixes remain rooted."),
		ECVF_Default);
	...
	static FAutoConsoleCommand DumpSubmixCmd(
		...
		TEXT("Draws the submix hierarchy for this world to the debug output"),
	...
	static void DrawSubmixHierarchy(USoundSubmixBase* InSubmix, const TSharedPtr<Audio::FMixerSubmix, ESPMode::ThreadSafe> InInstance, const FMixerDevice* InDevice, int32 InIdent, FOutputDevice& Ar, const TCHAR* GroupingText)
	{
		if (!InSubmix)
		{
			return;
		...
		FString Indent = FCString::Spc(InIdent*3);
		FString FxChain;
		
		// Show what FX the mixer is actually using: the live FMixerSubmix's current chain
		// This reflects any active override 
		// Fall back to the static USoundSubmix asset chain only when no runtime instance exists yet.
		if (InInstance.IsValid())
		{
			const int32 NumEffects = InInstance->GetNumChainEffects();
			for (int32 EffectIdx = 0; EffectIdx < NumEffects; ++EffectIdx)
			{
				FSoundEffectSubmixPtr EffectPtr = InInstance->GetSubmixEffect(EffectIdx);
				const USoundEffectPreset* Preset = EffectPtr.IsValid() ? EffectPtr->GetPreset() : nullptr;
				FxChain += FString::Printf(TEXT("[%s]"), *GetNameSafe(Preset));
			}
		}
		else if (USoundSubmix* Submix = Cast<USoundSubmix>(InSubmix))
		{	
		...
		//Get Mute/Solo state
		FString MuteSoloStateString = FCString::Spc(3);

#if ENABLE_AUDIO_DEBUG
		if (FAudioDeviceManager* Manager = FAudioDeviceManager::Get())
		{
			FAudioDebugger& Debugger = Manager->GetDebugger();
			bool bIsSoloed = false;
			bool bIsMuted = false;
			FString SoloMuteStateReason;
			Debugger.QuerySoloMuteSubmix(InSubmix->GetName(), bIsSoloed, bIsMuted, SoloMuteStateReason);

			if  (bIsSoloed)
			{
				MuteSoloStateString = "[S]";
			}
			else if (bIsMuted)
			{
				MuteSoloStateString = "[M]";
			}
		}
#endif
		
		Ar.Logf(TEXT("%s%sName=%s,Instance=0x%p,Id=%u,Fx=%s,[%s]"), *MuteSoloStateString, *Indent, *InSubmix->GetName(), InInstance.Get(), InInstance ? InInstance->GetId() : 0, *FxChain, GroupingText);
		for (const auto& i : InSubmix->ChildSubmixes)
		{
			DrawSubmixHierarchy(i, InDevice->GetSubmixInstance(i).Pin(), InDevice, InIdent+1, Ar, TEXT("Static"));
		}
		...
		{
			DrawSubmixHierarchy(i, InDevice->GetSubmixInstance(i).Pin(), InDevice, InIdent+1, Ar, TEXT("Dynamic"));
		}
	}
	...
	void FMixerDevice::DrawSubmixes(FOutputDevice& InOutput, const TArray<FString>& InArgs) const
	{
		...
		{
			InOutput.Logf(TEXT("SlotName=[%s]"), ToCStr(LexToString(static_cast<ERequiredSubmixes>(i))));
			DrawSubmixHierarchy(RequiredSubmixes[i], RequiredSubmixInstances[i], this, 1, InOutput, TEXT("In Slot"));
		}
	}

#if ENABLE_AUDIO_DEBUG
	void FMixerDevice::UpdateSubmixDebugVolumes()
	{
		Submixes.Iterate([](const FSubmixMap::FPair& SubmixPair)
		{
			const FMixerSubmixPtr& Submix = SubmixPair.Value;
			if (Submix.IsValid())
			{
				Submix->UpdateDebugVolume();
			}
		});
		...
		for (const FMixerSubmixPtr& RequiredSubmix : RequiredSubmixInstances)
		{
			...
			{
				RequiredSubmix->UpdateDebugVolume();
			}
		}
	}

	static FAutoConsoleCommand SubmixSoloCmd(
		TEXT("au.submix.solo"),
		TEXT("Toggle solo on a submix by name. Soloed submix is audible, all others are silenced. Usage: au.submix.solo <SubmixName>"),
		FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& InArgs)
		{
			...
			if (InArgs.Num() < 1 || InArgs[0].IsEmpty())
			{
				Debugger.ClearSubmixMuteSoloState();
				return;
			}

			Debugger.ToggleSoloSubmixChain(FName(InArgs[0]));
		}));

	static FAutoConsoleCommand SubmixMuteCmd(
		TEXT("au.submix.mute"),
		TEXT("Toggle mute on a submix by name. Usage: au.submix.mute <SubmixName>"),
		FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& InArgs)
		{
			FAudioDeviceManager* AudioDeviceManager = FAudioDeviceManager::Get();
			if (AudioDeviceManager == nullptr)
			...
			Debugger.ToggleMuteSubmix(FName(InArgs[0]));
		}));
		...
		, bSubmixRegistrationDisabled(true)
	{
		...
		bUseRenderScheduler = AudioMixerCVars::UseRenderScheduler != 0;
		UE_LOGFMT(LogAudioMixer, Display, "bUseRenderScheduler set to {UseRenderScheduler}.", ("UseRenderScheduler", bUseRenderScheduler));
		...
		FTraceAuxiliary::OnTraceStopped.AddRaw(this, &FMixerDevice::OnTraceStopped);
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
	}
	...
	void FMixerDevice::OnTraceStarted(FTraceAuxiliary::EConnectionType TraceType, const FString& TraceDestination)
	{
		...
		SendMainSubmixAlivePingMessage();
	}

	void FMixerDevice::OnTraceStopped(FTraceAuxiliary::EConnectionType TraceType, const FString& TraceDestination)
	{
		// Auto-Disable output metering
		Submixes.Iterate([this](const FSubmixMap::FPair& SubmixPair)
		{
			const FMixerSubmixPtr& Submix = SubmixPair.Value;
			if (Submix.IsValid() && Submix->bIsEnvelopeFollowing && SubmixesWithAutoEnvelopeFollowers.Contains(Submix->GetId()))
			{
				FAudioThread::RunCommandOnAudioThread([this, Submix]()
				{
					StopSubmixEnvelopeFollowing(Submix);
				});
			}
		});
		...
		for (const FMixerSubmixPtr& RequiredSubmix : RequiredSubmixInstances)
		{
			if (RequiredSubmix.IsValid() && RequiredSubmix->bIsEnvelopeFollowing && SubmixesWithAutoEnvelopeFollowers.Contains(RequiredSubmix->GetId()))
			{
				FAudioThread::RunCommandOnAudioThread([this, RequiredSubmix]()
				{
					StopSubmixEnvelopeFollowing(RequiredSubmix);
				});
			}
		}

		SubmixesWithAutoEnvelopeFollowers.Reset();
	}

	void FMixerDevice::InitializeMainSubmixTruePeakEnvelopeFollower(int32 InSampleRate, int32 InNumChannels, int32 InNumSamples)
	{
		MainSubmixTruePeakEnvelopeFollower = MakeUnique<FEnvelopeFollower>(
			FEnvelopeFollowerInitParams{
				.SampleRate = static_cast<float>(InSampleRate),
				.NumChannels = InNumChannels,
				.AttackTimeMsec = 0.0f,
				.ReleaseTimeMsec = 0.0f,
				.Mode = EPeakMode::TruePeak,
				.bIsAnalog = false,
			});
		
		MainSubmixTruePeakOutputEnvelope.SetNumUninitialized(InNumSamples);
	}

	void FMixerDevice::InitializeMainSubmixLKFSAnalyzer(int32 InSampleRate, int32 InNumChannels)
	{
		FLKFSAnalyzerSettings LKFSAnalyzerSettings;
		LKFSAnalyzerSettings.IntegratedLoudnessDuration = 1800.0f; // 30 minutes
		LKFSAnalyzerSettings.bCalculateOverallLoudnessRange = true;

		MainSubmixLKFSAnalyzer = MakeUnique<FLKFSAnalyzer>(InNumChannels, static_cast<float>(InSampleRate), LKFSAnalyzerSettings);
	}

	void FMixerDevice::SendMainSubmixLoadedMessage()
	{
		...
		const FMixerSubmixPtr MainSubmixPtr = GetMainSubmix().Pin();
		if (!MainSubmixPtr.IsValid())
		{
			return;
		}

		UE_TRACE_LOG(Audio, MainSubmixLoaded, AudioMixerChannel)
			<< MainSubmixLoaded.DeviceId(DeviceID)
			<< MainSubmixLoaded.SubmixId(GetTypeHash(MainSubmixPtr->GetPathName()))
			<< MainSubmixLoaded.Timestamp(FPlatformTime::Cycles64());
	}

	void FMixerDevice::SendMainSubmixAlivePingMessage()
	{
		const bool bChannelEnabled = UE_TRACE_CHANNELEXPR_IS_ENABLED(AudioMixerChannel);
		if (!bChannelEnabled)
		{
		...
		UE_TRACE_LOG(Audio, MainSubmixAlivePing, AudioMixerChannel)
			<< MainSubmixAlivePing.DeviceId(DeviceID)
			<< MainSubmixAlivePing.SubmixId(GetTypeHash(MainSubmixPtr->GetPathName()))
			<< MainSubmixAlivePing.Timestamp(FPlatformTime::Cycles64());
	}

	void FMixerDevice::UpdateAndSendMainSubmixTruePeakMaxValueMessage(const FAlignedFloatBuffer& InMainSubmixAudioBuffer)
	{
		const bool bChannelEnabled = UE_TRACE_CHANNELEXPR_IS_ENABLED(AudioMixerChannel);
		if (!bChannelEnabled)
		{
		...
		if (!MainSubmixTruePeakEnvelopeFollower.IsValid())
		{
			return;
		}

		MainSubmixTruePeakEnvelopeFollower->ProcessAudio(InMainSubmixAudioBuffer.GetData(), PlatformSettings.CallbackBufferFrameSize, MainSubmixTruePeakOutputEnvelope.GetData());
		
		const float* MainSubmixTruePeakMaxValuePtr = Algo::MaxElement(MainSubmixTruePeakOutputEnvelope);
		const float MainSubmixTruePeakMaxValueDb   = MainSubmixTruePeakMaxValuePtr ? ConvertToDecibels(*MainSubmixTruePeakMaxValuePtr) : MIN_VOLUME_DECIBELS;

		UE_TRACE_LOG(Audio, MainSubmixTruePeakMaxValue, AudioMixerChannel)
			<< MainSubmixTruePeakMaxValue.DeviceId(DeviceID)
			<< MainSubmixTruePeakMaxValue.SubmixId(GetTypeHash(MainSubmixPtr->GetPathName()))
			<< MainSubmixTruePeakMaxValue.Timestamp(FPlatformTime::Cycles64())
			<< MainSubmixTruePeakMaxValue.TruePeakMaxValueDb(MainSubmixTruePeakMaxValueDb);
	}

	void FMixerDevice::UpdateAndSendMainSubmixLKFSValuesMessage(const FAlignedFloatBuffer& InMainSubmixAudioBuffer)
	{
		const bool bChannelEnabled = UE_TRACE_CHANNELEXPR_IS_ENABLED(AudioMixerChannel);
		if (!bChannelEnabled)
		{
		...
		if (!MainSubmixLKFSAnalyzer.IsValid())
		{
			return;
		}

		FLKFSAnalyzerResults MainSubmixLKFSAnalyzerResults;
		MainSubmixLKFSAnalyzer->Analyze(InMainSubmixAudioBuffer, MainSubmixLKFSAnalyzerResults);

		const TArray<FLKFSResult>& OverallLKFSArray = MainSubmixLKFSAnalyzerResults.GetLoudnessResults();
		if (OverallLKFSArray.Num() > 0)
		{
			const FLKFSResult& LatestLKFSResult = OverallLKFSArray[OverallLKFSArray.Num() - 1];

			float LoudnessRangeLowerBound = 0.0f;
			float LoudnessRangeUpperBound = 0.0f;

			if (LatestLKFSResult.LoudnessRange.IsSet())
			{
				LoudnessRangeLowerBound = LatestLKFSResult.LoudnessRange.GetValue().Min;
				LoudnessRangeUpperBound = LatestLKFSResult.LoudnessRange.GetValue().Max;
			}

			UE_TRACE_LOG(Audio, MainSubmixLKFSValues, AudioMixerChannel)
				<< MainSubmixLKFSValues.DeviceId(DeviceID)
				<< MainSubmixLKFSValues.SubmixId(GetTypeHash(MainSubmixPtr->GetPathName()))
				<< MainSubmixLKFSValues.Timestamp(FPlatformTime::Cycles64())
				<< MainSubmixLKFSValues.LongTermLoudness(LatestLKFSResult.GatedLoudness)
				<< MainSubmixLKFSValues.ShortTermLoudness(LatestLKFSResult.ShortTermLoudness)
				<< MainSubmixLKFSValues.MomentaryLoudness(LatestLKFSResult.Loudness)
				<< MainSubmixLKFSValues.LoudnessRangeLowerBound(LoudnessRangeLowerBound)
				<< MainSubmixLKFSValues.LoudnessRangeUpperBound(LoudnessRangeUpperBound);
		}
	}
	...
	void FMixerDevice::SendSubmixesAlivePing()
	{
		...
		AutomaticallyStartSubmixEnvelopeFollowers();
	}
	...
	void FMixerDevice::SendSubmixesSendLevelTraceMessages()
	{
		const bool bChannelEnabled = UE_TRACE_CHANNELEXPR_IS_ENABLED(AudioMixerChannel);
		if (bChannelEnabled)
		{
			auto TraceSubmixToSubmixSendMessage = [this](const FMixerSubmixPtr& InSubmix)
			{
				if (!InSubmix.IsValid())
				{
					return;
				...
				const Audio::FMixerSubmixPtr OutputSubmix = InSubmix->GetParentSubmix().Pin();
				
				// If this submix has no parent, send the Audio Device as the receiver ID and mark as a final stage of the chain
				const bool bIsFinalStage = !OutputSubmix.IsValid();
				const uint32 ReceiverID = bIsFinalStage ? DeviceID : GetTypeHash(OutputSubmix->GetPathName());
				
				UE_TRACE_LOG(Audio, SubmixToSubmixSend, AudioMixerChannel)
					<< SubmixToSubmixSend.DeviceId(DeviceID)
					<< SubmixToSubmixSend.Timestamp(FPlatformTime::Cycles64())
					<< SubmixToSubmixSend.SenderID(GetTypeHash(InSubmix->GetPathName()))
					<< SubmixToSubmixSend.ReceiverID(ReceiverID)
					<< SubmixToSubmixSend.SendLevel(InSubmix->CurrentOutputVolume)
					<< SubmixToSubmixSend.IsFinalStage(bIsFinalStage);
			};

			Submixes.Iterate([&TraceSubmixToSubmixSendMessage](const FSubmixMap::FPair& SubmixPair)
			{
				TraceSubmixToSubmixSendMessage(SubmixPair.Value);
			});

			for (const FMixerSubmixPtr& RequiredSubmixInstance : RequiredSubmixInstances)
			{
				TraceSubmixToSubmixSendMessage(RequiredSubmixInstance);
			}
		}
	}
	...
	void FMixerDevice::UnregisterSubmixEnvelopeFollowerCVars()
	{
		...
		StartSubmixEnvelopeFollowerCVar	= nullptr;
		StopSubmixEnvelopeFollowerCVar	= nullptr;
	}
	...
	void FMixerDevice::OnStartSubmixEnvelopeFollowerCVarChanged(IConsoleVariable* InStartSubmixEnvelopeFollowerCVar)
	{
		...
		{
			const FString SubmixName = InStartSubmixEnvelopeFollowerCVar->GetString();
			if (!SubmixesWithCVarEnvelopeFollowers.Contains(SubmixName))
			{
				Submixes.Iterate([this, &SubmixName](const FSubmixMap::FPair& SubmixPair)
				{
					const FMixerSubmixPtr& Submix = SubmixPair.Value;

					if (Submix.IsValid() && Submix->GetName() == SubmixName)
					{
						FAudioThread::RunCommandOnAudioThread([this, Submix]()
						{
							StartSubmixEnvelopeFollowing(Submix);
						});

						SubmixesWithCVarEnvelopeFollowers.Add(SubmixName);
					}
				});

				for (const FMixerSubmixPtr& RequiredSubmix : RequiredSubmixInstances)
				{
					if (RequiredSubmix.IsValid() && RequiredSubmix->GetName() == SubmixName)
					{
						FAudioThread::RunCommandOnAudioThread([this, RequiredSubmix]()
						{
							StartSubmixEnvelopeFollowing(RequiredSubmix);
						});
	...
	void FMixerDevice::OnStopSubmixEnvelopeFollowerCVarChanged(IConsoleVariable* InStopSubmixEnvelopeFollowerCVar)
	{
		...
		{
			const FString SubmixName = InStopSubmixEnvelopeFollowerCVar->GetString();
			if (SubmixesWithCVarEnvelopeFollowers.Contains(SubmixName))
			{
				Submixes.Iterate([this, &SubmixName](const FSubmixMap::FPair& SubmixPair)
				{
					...
					if (Submix.IsValid() && Submix->GetName() == SubmixName)
					{
						...
						SubmixesWithCVarEnvelopeFollowers.Remove(SubmixName);
					}
				});
	...
	void FMixerDevice::StartSubmixEnvelopeFollowing(const FMixerSubmixPtr& InSubmix)
	{
		ensure(IsInAudioThread());

		if (!InSubmix || !InSubmix->IsOwnerValid())
		{
			return;
		}
		...
		InSubmix->StartEnvelopeFollowing(AttackTime, ReleaseTime);
	}

	void FMixerDevice::StopSubmixEnvelopeFollowing(const FMixerSubmixPtr& InSubmix)
	{
		ensure(IsInAudioThread());

		if (!InSubmix || !InSubmix->IsOwnerValid() || !InSubmix->bIsEnvelopeFollowing)
		{
			return;
		}

		InSubmix->StopEnvelopeFollowing();
	}

	void FMixerDevice::AutomaticallyStartSubmixEnvelopeFollowers()
	{
		const bool bAudioMixerChannelEnabled = UE_TRACE_CHANNELEXPR_IS_ENABLED(AudioMixerChannel);
		if (bAudioMixerChannelEnabled)
		{
			// Auto-Enable output metering
			Submixes.Iterate([this](const FSubmixMap::FPair& SubmixPair)
			{
				const FMixerSubmixPtr& Submix = SubmixPair.Value;
				if (Submix.IsValid())
				{
					const uint32 SubmixID = Submix->GetId();
					if (!SubmixesWithAutoEnvelopeFollowers.Contains(SubmixID))
					{
						SubmixesWithAutoEnvelopeFollowers.Add(SubmixID);
			...
			for (const FMixerSubmixPtr& RequiredSubmix : RequiredSubmixInstances)
			{
				if (RequiredSubmix.IsValid())
				{
					const uint32 SubmixID = RequiredSubmix->GetId();
					if (!SubmixesWithAutoEnvelopeFollowers.Contains(SubmixID))
					{
						SubmixesWithAutoEnvelopeFollowers.Add(SubmixID);
						FAudioThread::RunCommandOnAudioThread([this, RequiredSubmix]()
	...
	void FMixerDevice::RegisterResetMainSubmixAnalyzersCmd()
	{
		if (ResetMainSubmixAnalyzersCmd == nullptr)
		{
			ResetMainSubmixAnalyzersCmd = IConsoleManager::Get().RegisterConsoleCommand(
				*FString::Printf(TEXT("au.ResetMainSubmixAnalyzers_AD%d"), DeviceID),
				TEXT("Resets the main submix LKFS and TruePeak analyzers"),
				FConsoleCommandDelegate::CreateRaw(this, &FMixerDevice::OnResetMainSubmixAnalyzersCmdInvoked),
				ECVF_Default
			);
		}
	}

	void FMixerDevice::UnregisterResetMainSubmixAnalyzersCmd()
	{
		IConsoleManager::Get().UnregisterConsoleObject(ResetMainSubmixAnalyzersCmd);
		ResetMainSubmixAnalyzersCmd = nullptr;
	}

	void FMixerDevice::OnResetMainSubmixAnalyzersCmdInvoked()
	{
		ensure(IsInGameThread());

		FAudioThread::RunCommandOnAudioThread([this]()
		{
			ResetMainSubmixAnalyzers();
		});
	}

	void FMixerDevice::ResetMainSubmixAnalyzers()
	{
		ensure(IsInAudioThread());

		const int32 NumSamples = PlatformInfo.NumChannels * PlatformSettings.CallbackBufferFrameSize;

		MainSubmixLKFSAnalyzer.Reset();
		InitializeMainSubmixLKFSAnalyzer(PlatformSettings.SampleRate, PlatformInfo.NumChannels);

		MainSubmixTruePeakEnvelopeFollower.Reset();
		InitializeMainSubmixTruePeakEnvelopeFollower(PlatformSettings.SampleRate, PlatformInfo.NumChannels, NumSamples);
	}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
	...
	bool FMixerDevice::InitializeHardware()
	{
		...
		{
			UE_LOGF(LogAudioMixer, Display, "Initializing audio mixer using platform API: '%ls'", *AudioMixerPlatform->GetPlatformApi());

			// Issue a log if the audio engine will be muted.
#if !UE_BUILD_SHIPPING
			if (bMuteAudioCVar || FApp::IsAudioMuted())
			{
				UE_LOGF(LogAudioMixer, Display, "Audio output will be muted. If this not intentional, check for au.MuteAudio cvar or -muteaudio command line.");
			}
			...
			if (AudioMixerPlatform->OpenAudioStream(OpenStreamParams))
			{
				...
				UE_LOGF(LogAudioMixer, Display, "Using Audio Hardware Device %ls", *PlatformInfo.Name);
				...
				SourceManagerInitParams.NumGeneratorSources = AudioMixerGeneratorSourcesCVar;
				...
				RegisterResetMainSubmixAnalyzersCmd();
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
		...
		{
			UE_LOGF(LogAudioMixer, Warning, "Failed to initialize audio mixer for platform API: '%ls'", *AudioMixerPlatform->GetPlatformApi());
		}
	...
	void FMixerDevice::TeardownHardware()
	{
		...
		if (IsInitialized())
		{
			...
			UnregisterResetMainSubmixAnalyzersCmd();
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
	...
	void FMixerDevice::UpdateHardware()
	{
		...
		if (SourceManager->CheckAndResetHardwareCallbackStallFlag())
		{
			// This is a foreground thread check if we are not receiving hardware callbacks. 
			AudioMixerPlatform->RequestDeviceSwap(TEXT(""), /* force */true, TEXT("FMixerDevice::UpdateHardware(). Force device swap because hardware has stalled."));
		}

		AudioMixerPlatform->OnHardwareUpdate();

		// Device swaps span multiple calls beginning with CheckAudioDeviceChange() and ending with
		// ResumePlaybackOnNewDevice(). We protect the associated resources by locking and unlocking around
		// the entire call chain.
		{
			FScopedDeviceSwapLock Lock(AudioMixerPlatform);
			...
			if (AudioMixerPlatform->CheckAudioDeviceChange())
			{
				...
				// The render thread is stopped at this point, so skip waiting on it
				// to avoid a ~1s timeout per Wait() call in FlushCommandQueue.
				FlushAudioRenderingCommands(true /* bPumpSynchronously */, true /* bRenderThreadStopped */);
				...
				UE_CLOGF(CurrentAudioPlatformThreadId != AudioPlatformThreadId, LogAudioMixer, Error, "Platform audio thread id changed while flushing render commands. Expected %d, found %d. May result in corrupt internal audio source state.", CurrentAudioPlatformThreadId.load(), AudioPlatformThreadId.load());
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
		UpdateAndSendMainSubmixTruePeakMaxValueMessage(Output);
		UpdateAndSendMainSubmixLKFSValuesMessage(Output);

		UpdateSubmixesAlivePingTime();
		SendSubmixesSendLevelTraceMessages();
#endif //UE_AUDIO_PROFILERTRACE_ENABLED
	...
	void FMixerDevice::LoadRequiredSubmix(ERequiredSubmixes InType, const FString& InDefaultName, bool bInDefaultMuteWhenBackgrounded, FSoftObjectPath& InObjectPath) 
	{
		...
		if (!NewSubmix)
		{
			static const FString EngineSubmixDir = "/Engine/EngineSounds/Submixes";
			...
			UE_LOGF(LogAudioMixer, Display, "Submix unset or invalid in 'AudioSettings': Using engine asset '%ls'",
		...
		if (!NewSubmix)
		{
			UE_LOGF(LogAudioMixer, Error, "Failed to load submix from engine asset path '%ls'. Creating '%ls' as a stub.",
				*InObjectPath.GetAssetPathString(),
				*InDefaultName);
		...
		{
			UE_LOGF(LogAudioMixer, Display, "Creating Master Submix '%ls'", *NewSubmix->GetName());
		...
		{
			UE_LOGF(LogAudioMixer, Display, "Submix '%ls' has been promoted to master array.", *NewSubmix->GetName());
		}
	...
	void FMixerDevice::LoadPluginSoundSubmixes()
	{
		...
		if (IsReverbPluginEnabled() && ReverbPluginInterface)
		{
			...
			// This AddToRoot is redundant since we AddToRoot in the LoadSoundSubmix stack
			if (!GRemoveSubmixesFromRoot)
			{
			...
			{
				UE_LOGF(LogAudioMixer, Error, "Reverb plugin failed to provide valid effect submix.  Plugin audio processing disabled.");
			}
		}
	}

	static inline FString GetRequiredSubmixName(const ERequiredSubmixes InRequiredSubmix)
	{
		switch (InRequiredSubmix) 
		{
			case ERequiredSubmixes::Main: { return TEXT("MasterSubmixDefault"); }
			case ERequiredSubmixes::BaseDefault: { return TEXT("BaseDefault"); }
			case ERequiredSubmixes::Reverb: { return TEXT("MasterReverbSubmixDefault"); }
			case ERequiredSubmixes::EQ: { return TEXT("MasterEQSubmixDefault"); }
			case ERequiredSubmixes::Count: break;
		}

		UE_LOGF(LogAudioMixer, Warning, "Unsupported submix name for provided ERequiredSubmix");
		return "";
	}
	...
	void FMixerDevice::InitSoundSubmixes()
	{
		if (IsInGameThread())
		{
			...
			{
				UE_LOGF(LogAudioMixer, Display, "Re-initializing Sound Submixes...");
			}
			else
			{
				UE_LOGF(LogAudioMixer, Display, "Initializing Sound Submixes...");
			}

			// 1. Load or reload all sound submixes/instances
			LoadRequiredSubmix(ERequiredSubmixes::Main, GetRequiredSubmixName(ERequiredSubmixes::Main), false /* DefaultMuteWhenBackgrounded */, AudioSettings->MasterSubmix);

#if UE_AUDIO_PROFILERTRACE_ENABLED
			SendMainSubmixLoadedMessage();
#endif // UE_AUDIO_PROFILERTRACE_ENABLED

			// BaseDefaultSubmix is an optional master submix type set by project settings
			if (AudioSettings->BaseDefaultSubmix.IsValid())
			{
				LoadRequiredSubmix(ERequiredSubmixes::BaseDefault, GetRequiredSubmixName(ERequiredSubmixes::BaseDefault), false /* DefaultMuteWhenBackgrounded */, AudioSettings->BaseDefaultSubmix);
			}

			LoadRequiredSubmix(ERequiredSubmixes::Reverb, GetRequiredSubmixName(ERequiredSubmixes::Reverb), true /* DefaultMuteWhenBackgrounded */, AudioSettings->ReverbSubmix);

			if (!DisableSubmixEffectEQCvar)
			{
				LoadRequiredSubmix(ERequiredSubmixes::EQ, GetRequiredSubmixName(ERequiredSubmixes::EQ), false /* DefaultMuteWhenBackgrounded */, AudioSettings->EQSubmix);
			}
	...
	void FMixerDevice::RebuildSubmixLinks(const USoundSubmixBase& SoundSubmix, FMixerSubmixPtr& SubmixInstance)
	{
 	...
 	FAudioPlatformSettings FMixerDevice::GetPlatformSettings() const
 	{
		...
		if (AudioMixerPlatform)
		{
			...
			UE_LOGF(LogAudioMixer, Display, "Audio Mixer Platform Settings:");
			UE_LOGF(LogAudioMixer, Display, "	Sample Rate:						  %d", Settings.SampleRate);
			UE_LOGF(LogAudioMixer, Display, "	Callback Buffer Frame Size Requested: %d", Settings.CallbackBufferFrameSize);
			UE_LOGF(LogAudioMixer, Display, "	Callback Buffer Frame Size To Use:	  %d", AudioMixerPlatform->GetNumFrames(Settings.CallbackBufferFrameSize));
			UE_LOGF(LogAudioMixer, Display, "	Number of buffers to queue:			  %d", Settings.NumBuffers);
			UE_LOGF(LogAudioMixer, Display, "	Max Channels (voices):				  %d", (Settings.MaxChannels > 0) ? Settings.MaxChannels : DefaultMaxChannels);
			UE_LOGF(LogAudioMixer, Display, "	Number of Async Source Workers:		  %d", Settings.NumSourceWorkers);
		}
	...
	int32 FMixerDevice::AddSubmixEffect(USoundSubmix* InSoundSubmix, FSoundEffectSubmixPtr SoundEffect)
	{
		...
		{
			UE_LOGF(LogAudio, Warning, "Submix instance %ls not found.", *InSoundSubmix->GetName());
		}
		return 0;
	}
	...
	void FMixerDevice::FlushAudioRenderingCommands(bool bPumpSynchronously, bool bRenderThreadStopped)
	{
		if (IsInitialized() && (FPlatformProcess::SupportsMultithreading() && !AudioMixerPlatform->IsNonRealtime()))
		{
			SourceManager->FlushCommandQueue(bPumpSynchronously, bRenderThreadStopped);
		}
	...
	void FMixerDevice::RegisterSoundSubmix(USoundSubmixBase* InSoundSubmix, bool bInit)
	{
		...
		{
			UE_LOGF(LogAudioMixer, Warning, "Attempted register Submix %ls before the submix graph was initialized.", *InSoundSubmix->GetFullName());
			return;
		}
		...
		UE_LOGF(LogAudioMixer, Display, "Registering submix %ls.", *InSoundSubmix->GetFullName());
		...
		{
			UE_LOGF(LogAudioMixer, Display, "Submix %ls was already registered as one of the master submixes.", *InSoundSubmix->GetFullName());
		}
		...
		// Now that the submix is loaded, we can register the submix buffer listeners that were waiting for it
		if (PendingSubmixBufferListeners.Contains(InSoundSubmix->GetPathName()))
		{
			UE_LOGF(LogAudioMixer, Display, "Submix %ls had pending Submix Buffer Listeners, registering them now...", *InSoundSubmix->GetName());
			
			TMap<FString, TSharedRef<ISubmixBufferListener, ESPMode::ThreadSafe>>& PendingListeners = PendingSubmixBufferListeners[InSoundSubmix->GetPathName()];

			if (USoundSubmix* Submix = Cast<USoundSubmix>(InSoundSubmix))
			{
				for (TPair<FString, TSharedRef<ISubmixBufferListener, ESPMode::ThreadSafe>>& Listener : PendingListeners)
				{
					RegisterSubmixBufferListener(Listener.Value, *Submix);
				}
				PendingSubmixBufferListeners.Remove(InSoundSubmix->GetPathName());
			}
		}
	}
	...
	void FMixerDevice::LoadSoundSubmix(USoundSubmixBase& InSoundSubmix)
	{
		...
		if (!MixerSubmix.IsValid())
		{
			if (GRemoveSubmixesFromRoot)
			{
				InSoundSubmix.IncrementLoadRefCount();
			}
			else
			{
	...
	void FMixerDevice::UnloadSoundSubmix(const USoundSubmixBase& InSoundSubmix, const bool bReparentChildren)
	{
		...
		if (MixerSubmix.IsValid())
		{
			FAudioThread::RunCommandOnGameThread([this, SubmixID = MixerSubmix->GetId(), SubmixName = MixerSubmix->GetName()]()
			{
				SubmixesWithAutoEnvelopeFollowers.Remove(SubmixID);
				SubmixesWithCVarEnvelopeFollowers.Remove(SubmixName);
			});
		}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED

		const int32 RemoveCount = Submixes.Remove(InSoundSubmix.GetUniqueID());
		
		if (GRemoveSubmixesFromRoot && RemoveCount > 0)
		{
			USoundSubmixBase& MutableSubmix = const_cast<USoundSubmixBase&>(InSoundSubmix);
			MutableSubmix.DecrementLoadRefCount();
		}
	}
	...
	FMixerSubmixPtr FMixerDevice::FindSubmixInstanceByObjectId(uint32 InObjectId)
	{
		for (int32 i = 0; i < RequiredSubmixes.Num(); i++)
		{
			...
			else
			{
				...
				{
					UE_LOGF(LogAudioMixer, Warning, "Failed to query EQ Submix when it was expected to be loaded.");
				}
			}
		}
	...
	void FMixerDevice::RegisterSubmixBufferListener(TSharedRef<ISubmixBufferListener, ESPMode::ThreadSafe> InSubmixBufferListener, USoundSubmix& InSubmix)
	{
		...
		auto RegisterLambda = [this, InSubmixBufferListener, SubmixPtr]()
		{
			...
			// Submix is not currently registered
			if (!FoundSubmix.IsValid() && SubmixPtr.IsValid())
			{
				const FString SubmixName = SubmixPtr->GetPathName();

				if (SubmixBufferListenerForceRegistrationCVar)
				{
					RegisterSoundSubmix(SubmixPtr.Get(), true /* bInit */);
					FoundSubmix = GetSubmixInstance(SubmixPtr.Get()).Pin();
					
					UE_LOGF(LogAudioMixer, Warning, "Submix '%ls' force-enabled by Submix Buffer Listener '%ls' due to CVar 'au.SubmixBufferListenerForceRegistration'."
						"Please disable the CVar and manually ensure the submix is registered before registering the submix buffer listener.", *SubmixPtr->GetName(), *ListenerName);
				}
				else
				{
					UE_LOGF(LogAudioMixer, Display, "Submix '%ls' not currently loaded, Submix buffer listener '%ls' added to pending list.", *SubmixName, *ListenerName);
					if (!PendingSubmixBufferListeners.Contains(SubmixName))
					{
						PendingSubmixBufferListeners.Add(SubmixName, TMap<FString, TSharedRef<ISubmixBufferListener, ESPMode::ThreadSafe>>());
					}

					TMap<FString, TSharedRef<ISubmixBufferListener, ESPMode::ThreadSafe>>& PendingMap = PendingSubmixBufferListeners[SubmixName];
					PendingMap.Add(ListenerName, InSubmixBufferListener);
				}
			}
			...
			if (FoundSubmix.IsValid())
			{
				...
				UE_LOGF(LogAudioMixer, Display, "Submix buffer listener '%ls' registered with submix '%ls'", *ListenerName, *FoundSubmix->SubmixName);
			}
		};

		UE_LOGF(LogAudioMixer, Display, "Sending SubmixBufferListener '%ls' register command...", *InSubmixBufferListener->GetListenerName());
	...
	void FMixerDevice::UnregisterSubmixBufferListener(TSharedRef<ISubmixBufferListener, ESPMode::ThreadSafe> InSubmixBufferListener, USoundSubmix& InSubmix)
	{
		...
		auto UnregisterLambda = [this, SubmixPtr, InSubmixBufferListener, ListenerName]()
		{
			CSV_SCOPED_TIMING_STAT(Audio, UnregisterSubmixBufferListener);

			if (SubmixPtr.IsValid())
			{
				if (PendingSubmixBufferListeners.Contains(SubmixPtr->GetPathName()))
				{
					PendingSubmixBufferListeners[SubmixPtr->GetPathName()].Remove(InSubmixBufferListener->GetListenerName());
				}
			}
			...
			if (FoundSubmix.IsValid())
			{
				UE_LOGF(LogAudioMixer, Display, "Unregistering submix buffer listener '%ls' from submix '%ls'", *ListenerName, *FoundSubmix->SubmixName);
			...
			{
				UE_LOGF(LogAudioMixer, Display, "Submix buffer listener '%ls' not unregistered. Submix not loaded.", *ListenerName);
			}
		};
	...
	{
		// No-op: procedural sources now render directly on the audio render thread.
	}
	...
	TUniquePtr<IAudioMixerGeneratorSource> FMixerDevice::CreateGeneratorSource(ISoundGeneratorRef InGenerator)
	{
		return MakeUnique<FAudioMixerGeneratorSource>(*this, InGenerator);
	}
