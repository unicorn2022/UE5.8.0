...
namespace Audio
{
	...
	double FQuartzMetronome::GetFramesUntilBoundary(FQuartzQuantizationBoundary InQuantizationBoundary) const
	{
		...
		{
			UE_LOGF(LogAudioQuartz, Warning, "Quantization Boundary being clamped to 1.0 (from %f)", InQuantizationBoundary.Multiplier);
	...
	void FQuartzMetronome::RecalculateDurations()
	{
		...
		if (CurrentTimeSignature.OptionalPulseOverride.Num() != 0)
		{
			...
			{
				UE_LOGF(LogAudioQuartz, Warning
					, "Pulse override array on Time Signature reperesents more than a bar. The provided list will be trunctaed to 1 Bar in length");
