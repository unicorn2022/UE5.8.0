...
UQuartzClockHandle* UQuartzSubsystem::CreateNewClock(const UObject* WorldContextObject, FName ClockName, FQuartzClockSettings InSettings, bool bOverrideSettingsIfClockExists, bool bUseAudioEngineClockManager)
{
	...
	{
		UE_LOGF(LogAudioQuartz, Warning, "Clock: (%ls) is attempting to set a time signature with a Numerator < 1.  Clamping to 1 beat per bar", *ClockName.ToString());
...
void UQuartzSubsystem::AddProxyForExternalClock(const Audio::FQuartzClockProxy& InProxy)
{
	...
	{
		UE_LOGF(LogAudioQuartz, Warning, "Received request to add external Clock: (%ls) when a clock of that name already exists (Ignoring Request)", *InProxy.GetClockName().ToString());
		return;
	}
}
