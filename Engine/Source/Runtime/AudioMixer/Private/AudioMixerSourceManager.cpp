...
static float FloatCompareMinScaleCVar = 0.001f;
...
static float FloatCompareMaxScaleCVar = 0.1f;
...
static FAutoConsoleCommand GModulationSetMaxPitchRange(
	...
	FConsoleCommandWithArgsDelegate::CreateStatic(
		[](const TArray<FString>& Args)
		{
			if (Args.Num() < 1)
			{
				UE_LOGF(LogAudioMixer, Error, "Failed to set max modulation pitch range: Range not provided");
				return;
			}
...
#define AUDIO_MIXER_DEBUG_LOG(SourceId, Format, ...)																							\
	...
	{																																			\
		...
		UE_LOGF(LogAudioMixer, Log, "%ls", *LogMessage);																								\
	}
...
UE_TRACE_EVENT_BEGIN(Audio, SoundToSubmixSend)
	UE_TRACE_EVENT_FIELD(uint32, DeviceId)
	UE_TRACE_EVENT_FIELD(uint64, Timestamp)
	UE_TRACE_EVENT_FIELD(uint32, SenderID)
	UE_TRACE_EVENT_FIELD(uint32[], ReceiverIDs)
	UE_TRACE_EVENT_FIELD(float[], SendLevels)
UE_TRACE_EVENT_END()

UE_TRACE_EVENT_BEGIN(Audio, SoundToBusSend)
	UE_TRACE_EVENT_FIELD(uint32, DeviceId)
	UE_TRACE_EVENT_FIELD(uint64, Timestamp)
	UE_TRACE_EVENT_FIELD(uint32, SenderID)
	UE_TRACE_EVENT_FIELD(uint32[], ReceiverIDs)
...
UE_TRACE_EVENT_BEGIN(Audio, BusPatchWriterConnected)
	UE_TRACE_EVENT_FIELD(uint32, DeviceId)
	UE_TRACE_EVENT_FIELD(uint64, Timestamp)
	UE_TRACE_EVENT_FIELD(uint32, SenderID)
	UE_TRACE_EVENT_FIELD(uint32[], ReceiverIDs)
...
UE_TRACE_EVENT_BEGIN(Audio, BusPatchReaderConnected)
	UE_TRACE_EVENT_FIELD(uint32, DeviceId)
	UE_TRACE_EVENT_FIELD(uint64, Timestamp)
	UE_TRACE_EVENT_FIELD(uint32, SenderID)
	UE_TRACE_EVENT_FIELD(uint32[], ReceiverIDs)
...
UE_TRACE_EVENT_BEGIN(Audio, AudioBusIsAlivePing)
	UE_TRACE_EVENT_FIELD(uint32, DeviceId)
	UE_TRACE_EVENT_FIELD(uint32, AudioBusId)
	UE_TRACE_EVENT_FIELD(uint64, Timestamp)
	UE_TRACE_EVENT_FIELD(UE::Trace::WideString, Name)
...
namespace Audio
{
	...
	void TraceAudioBusIsAlivePingMessage(const uint32 InDeviceId, const uint32 InObjectId, const FString& InAudioBusName, const int32 InNumChannels)
	{
		const bool bChannelEnabled = UE_TRACE_CHANNELEXPR_IS_ENABLED(AudioChannel);
		if (bChannelEnabled)
		{
			UE_TRACE_LOG(Audio, AudioBusIsAlivePing, AudioChannel)
				<< AudioBusIsAlivePing.DeviceId(InDeviceId)
				<< AudioBusIsAlivePing.AudioBusId(InObjectId)
				<< AudioBusIsAlivePing.Timestamp(FPlatformTime::Cycles64())
				<< AudioBusIsAlivePing.Name(InAudioBusName.IsEmpty() ? *FString::Printf(TEXT("%d"), InObjectId) : *InAudioBusName)
				<< AudioBusIsAlivePing.NumChannels(InNumChannels);
		}
	}
	...
	void TraceBusPatchWriterConnectedMessage(const uint32 InDeviceId, const uint32 InPlayOrder, const uint32 InBusObjectId)
	{
		const bool bChannelEnabled = UE_TRACE_CHANNELEXPR_IS_ENABLED(AudioChannel);
		if (!bChannelEnabled)
		{
		...
		UE_TRACE_LOG(Audio, BusPatchWriterConnected, AudioChannel)
			<< BusPatchWriterConnected.DeviceId(InDeviceId)
			<< BusPatchWriterConnected.Timestamp(FPlatformTime::Cycles64())
			<< BusPatchWriterConnected.SenderID(InPlayOrder)
			<< BusPatchWriterConnected.ReceiverIDs(&InBusObjectId, 1)
			<< BusPatchWriterConnected.SendLevels(&SendLevel, 1);
	}

	void TraceBusPatchReaderConnectedMessage(const uint32 InDeviceId, const uint32 InBusObjectId, const uint32 InPlayOrder)
	{
		const bool bChannelEnabled = UE_TRACE_CHANNELEXPR_IS_ENABLED(AudioChannel);
		if (!bChannelEnabled)
		{
		...
		constexpr float SendLevel = 1.0f;
		UE_TRACE_LOG(Audio, BusPatchReaderConnected, AudioChannel)
			<< BusPatchReaderConnected.DeviceId(InDeviceId)
			<< BusPatchReaderConnected.Timestamp(FPlatformTime::Cycles64())
			<< BusPatchReaderConnected.SenderID(InBusObjectId)
			<< BusPatchReaderConnected.ReceiverIDs(&InPlayOrder, 1)
			<< BusPatchReaderConnected.SendLevels(&SendLevel, 1);
	}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
	...
	**************************************************************************/
	...
	void FMixerSourceManager::OnTraceStopped(FTraceAuxiliary::EConnectionType TraceType, const FString& TraceDestination)
	{
		FAudioThread::RunCommandOnAudioThread([this]()
		{
			AudioMixerThreadCommand([this]()
			{
				const bool bChannelEnabled = UE_TRACE_CHANNELEXPR_IS_ENABLED(AudioMixerChannel);

				for (auto& [AudioBusKey, AudioBus] : AudioBuses)
				{
					if (AudioBus.IsValid() && AudioBus->bIsEnvelopeFollowing && AudioBusesWithAutoEnvelopeFollowers.Contains(AudioBusKey.ObjectId))
					{
						const bool bStopped = AudioBus->StopEnvelopeFollower();
						if (bChannelEnabled && bStopped)
						{
							UE_TRACE_LOG(Audio, AudioBusEnvelopeFollowerEnabled, AudioMixerChannel)
				...
				AudioBusesWithAutoEnvelopeFollowers.Reset();
			}, AUDIO_MIXER_THREAD_COMMAND_STRING("OnTraceStopped()"));
		});
	}
	...
	void FMixerSourceManager::OnStartAudioBusEnvelopeFollowerCVarChanged(IConsoleVariable* InStartAudioBusEnvelopeFollowerCVar)
	{
		...
		if (InStartAudioBusEnvelopeFollowerCVar)
		{
			...
			{
				AudioMixerThreadCommand([this, AudioBusId]()
				{
					StartAudioBusEnvelopeFollowerFromCVar(AudioBusId);
				}, AUDIO_MIXER_THREAD_COMMAND_STRING("OnStartAudioBusEnvelopeFollowerCVarChanged"));
			});
		}
	}
	...
	void FMixerSourceManager::OnStopAudioBusEnvelopeFollowerCVarChanged(IConsoleVariable* InStopAudioBusEnvelopeFollowerCVar)
	{
		...
		if (InStopAudioBusEnvelopeFollowerCVar)
		{
			...
			FAudioThread::RunCommandOnAudioThread([this, AudioBusId]()
			{
				...
				}, AUDIO_MIXER_THREAD_COMMAND_STRING("OnStopAudioBusEnvelopeFollowerCVarChanged"));
			});
		}
	}
	...
	void FMixerSourceManager::StartAudioBusEnvelopeFollowerFromCVar(const uint32 InAudioBusId)
	{
		...
		for (auto& [AudioBusKey, AudioBus] : AudioBuses)
		{
			...
			{
				const bool bStarted = AudioBus->StartEnvelopeFollower(AttackTime, ReleaseTime, MixerDevice->GetSampleRate());

				if (bChannelEnabled && bStarted)
				{
					UE_TRACE_LOG(Audio, AudioBusEnvelopeFollowerEnabled, AudioMixerChannel)
						<< AudioBusEnvelopeFollowerEnabled.DeviceId(MixerDevice->DeviceID)
						<< AudioBusEnvelopeFollowerEnabled.AudioBusId(AudioBusKey.ObjectId)
	...
	void FMixerSourceManager::AutomaticallyStartAudioBusEnvelopeFollower(const FAudioBusKey& InAudioBusKey, const TSharedPtr<FMixerAudioBus>& InAudioBus)
	{
		if (!AudioBusesWithAutoEnvelopeFollowers.Contains(InAudioBusKey.ObjectId))
		{
			constexpr float AttackTime = 10.0f;
			constexpr float ReleaseTime = 500.0f;

			AudioBusesWithAutoEnvelopeFollowers.Add(InAudioBusKey.ObjectId);

			const bool bStarted = InAudioBus->StartEnvelopeFollower(AttackTime, ReleaseTime, MixerDevice->GetSampleRate());
			if (bStarted)
			{
				UE_TRACE_LOG(Audio, AudioBusEnvelopeFollowerEnabled, AudioMixerChannel)
					<< AudioBusEnvelopeFollowerEnabled.DeviceId(MixerDevice->DeviceID)
					<< AudioBusEnvelopeFollowerEnabled.AudioBusId(InAudioBusKey.ObjectId)
					<< AudioBusEnvelopeFollowerEnabled.Timestamp(FPlatformTime::Cycles64())
					<< AudioBusEnvelopeFollowerEnabled.EnvelopeFollowerEnabled(true);
			}
		}
	...
	void FMixerSourceManager::StopAutoAudioBusEnvelopeFollower(const uint32 InAudioBusId, FMixerAudioBus& InAudioBus)
	{
		if (AudioBusesWithAutoEnvelopeFollowers.Remove(InAudioBusId) && InAudioBus.bIsEnvelopeFollowing)
		{
			const bool bStopped = InAudioBus.StopEnvelopeFollower();
			const bool bChannelEnabled = UE_TRACE_CHANNELEXPR_IS_ENABLED(AudioMixerChannel);
			if (bStopped && bChannelEnabled)
			{
				UE_TRACE_LOG(Audio, AudioBusEnvelopeFollowerEnabled, AudioMixerChannel)
					<< AudioBusEnvelopeFollowerEnabled.DeviceId(MixerDevice->DeviceID)
					<< AudioBusEnvelopeFollowerEnabled.AudioBusId(InAudioBusId)
					<< AudioBusEnvelopeFollowerEnabled.Timestamp(FPlatformTime::Cycles64())
					<< AudioBusEnvelopeFollowerEnabled.EnvelopeFollowerEnabled(false);
			}
		}
	...
	void FMixerSourceManager::TraceSourceToSubmixSendMessage(const FSourceInfo& SourceInfo) const
	{
		const bool bChannelEnabled = UE_TRACE_CHANNELEXPR_IS_ENABLED(AudioMixerChannel);

		if (!bChannelEnabled || SourceInfo.SubmixSends.IsEmpty())
		{
			return;
		}

		TArray<uint32, TInlineAllocator<32>> SendReceiverIDs;
		TArray<float, TInlineAllocator<32>> SendLevels;

		SendReceiverIDs.Reserve(SourceInfo.SubmixSends.Num());
		SendLevels.Reserve(SourceInfo.SubmixSends.Num());

		for (const Audio::FMixerSourceSubmixSend& SubmixSend : SourceInfo.SubmixSends)
		{
			const FMixerSubmixPtr SubmixPtr = SubmixSend.Submix.Pin();
			if (SubmixPtr.IsValid())
			{
				SendReceiverIDs.Add(GetTypeHash(SubmixPtr->GetPathName()));
				SendLevels.Add(SubmixSend.SendLevel);
			}
		}

		if (!SendReceiverIDs.IsEmpty() && ensure(SendReceiverIDs.Num() == SendLevels.Num()))
		{
			UE_TRACE_LOG(Audio, SoundToSubmixSend, AudioMixerChannel)
				<< SoundToSubmixSend.DeviceId(MixerDevice->DeviceID)
				<< SoundToSubmixSend.Timestamp(FPlatformTime::Cycles64())
				<< SoundToSubmixSend.SenderID(SourceInfo.PlayOrder)
				<< SoundToSubmixSend.ReceiverIDs(SendReceiverIDs.GetData(), SendReceiverIDs.Num())
				<< SoundToSubmixSend.SendLevels(SendLevels.GetData(), SendLevels.Num());
		}
	}

	void FMixerSourceManager::TraceSourceToBusSendMessage(const uint32 SourceId, const FSourceInfo& SourceInfo) const
	{
		const bool bChannelEnabled = UE_TRACE_CHANNELEXPR_IS_ENABLED(AudioMixerChannel);
		if (!bChannelEnabled)
		{
		...
		TArray<uint32, TInlineAllocator<32>> BusReceiverIDs;
		TArray<float, TInlineAllocator<32>> BusSendLevels;
		...
		for (int32 AudioBusSendType = 0; AudioBusSendType < (int32)EBusSendType::Count; ++AudioBusSendType)
		{
			for (uint32 AudioBusId : SourceInfo.AudioBusSends[AudioBusSendType])
			{
				...
				if (AudioBusPtr && AudioBusPtr->IsValid())
				{
					FAudioBusSend* SourceToBusSendInfo = (*AudioBusPtr)->AudioBusSends[AudioBusSendType].FindByPredicate([SourceId](const FAudioBusSend& BusSendInfo)
					{
						return BusSendInfo.SourceId == SourceId;
					});

					if (SourceToBusSendInfo)
					{
						BusReceiverIDs.Add(AudioBusId);
						BusSendLevels.Add(SourceToBusSendInfo->SendLevel);
					}
				}
			}
		}

		if (!BusReceiverIDs.IsEmpty() && ensure(BusReceiverIDs.Num() == BusSendLevels.Num()))
		{
			UE_TRACE_LOG(Audio, SoundToBusSend, AudioMixerChannel)
				<< SoundToBusSend.DeviceId(MixerDevice->DeviceID)
				<< SoundToBusSend.Timestamp(FPlatformTime::Cycles64())
				<< SoundToBusSend.SenderID(SourceInfo.PlayOrder)
				<< SoundToBusSend.ReceiverIDs(BusReceiverIDs.GetData(), BusReceiverIDs.Num())
				<< SoundToBusSend.SendLevels(BusSendLevels.GetData(), BusSendLevels.Num());
		}
	}

	uint32 FMixerSourceManager::ResolvePlayOrder(const FPendingAudioBusConnection& Connection) const
	{
		uint32 PlayOrder = Connection.WaveInstancePlayOrder;
		if (PlayOrder == INDEX_NONE)
		{
			const uint32* FoundPlayOrder = InstanceIdToPlayOrderMap.Find(Connection.InstanceID);
			if (FoundPlayOrder)
			{
				PlayOrder = *FoundPlayOrder;
			}
		}
	...
	void FMixerSourceManager::Init(const FSourceManagerInitParams& InitParams)
	{
		...
		NumLegacySources = InitParams.NumSources;
		NumGeneratorSources = InitParams.NumGeneratorSources;
		NumTotalSources = NumLegacySources + NumGeneratorSources;
		...
		GeneratorSourceState.AddDefaulted(NumGeneratorSources);

		MixerSources.Init(nullptr, NumLegacySources);

		// Populate output sources array with default data
		SourceSubmixOutputBuffers.Reset(NumTotalSources);
		...
		for (int32 i = 0; i < NumTotalSources; ++i)
		{
			...
			SourceInfo.AudioBusSends[(int32)EBusSendType::PostAttenuation].Reset();
			...
			SourceInfo.bHasAttenuationSubmixSends = false;
		...
		GameThreadInfo.bIsBusy.AddDefaulted(NumLegacySources);
		GameThreadInfo.bNeedsSpeakerMap.AddDefaulted(NumLegacySources);
		GameThreadInfo.bIsDebugMode.AddDefaulted(NumLegacySources);
		GameThreadInfo.bIsUsingHRTFSpatializer.AddDefaulted(NumLegacySources);
#if ENABLE_AUDIO_DEBUG
		GameThreadInfo.CPUCoreUtilization.AddZeroed(NumLegacySources);
#endif // if ENABLE_AUDIO_DEBUG

		GameThreadInfo.RelativeRenderCost.Reset(NumLegacySources);
		GameThreadInfo.FreeSourceIndices.Reset(NumLegacySources);
		GameThreadInfo.ModulationVolume.Reset(NumLegacySources);
		for (int32 i = NumLegacySources - 1; i >= 0; --i)
		{
		...
		FTraceAuxiliary::OnTraceStopped.AddRaw(this, &FMixerSourceManager::OnTraceStopped);
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
	}
	...
	void FMixerSourceManager::Update(bool bTimedOut)
	{
		...
		for (int32 i = 0; i < NumLegacySources; ++i)
		{
		...
		if (FPlatformProcess::SupportsMultithreading())
		{
			...
			if (CommandsProcessedEvent->Wait(0))
			{
				...
				{
					UE_LOGF(LogAudioMixer, Warning, "Audio render callback stopped. Flushing %d commands.", NextCommandBuffer.SourceCommandQueue.Num());
		...
		else
		{
			...
			if (CurrentRenderIndex == CurrentGameIndex)
			{
				...
				{
					UE_LOGF(LogAudioMixer, Warning, "Source command queue not empty: %d", CommandBuffers[NextIndex].SourceCommandQueue.Num());
				}
				bPumpQueue = true;
			}
		}
	...
	void FMixerSourceManager::ReleaseSource(const int32 SourceId)
	{
		...
		if (SourceId < NumLegacySources && MixerSources[SourceId] == nullptr)
		{
			UE_LOGF(LogAudioMixer, Warning, "Ignoring double release of SourceId: %i", SourceId);
			return;
		}
		...
		{
			check(SourceId >= NumLegacySources || SourceInfo.MixerSourceBuffer.IsValid());
			GetRenderScheduler().RemoveStep(FAudioRenderStepId::FromWaveInstancePlayOrder(SourceInfo.PlayOrder));
		}
		...
		if (SourceInfo.AudioBusId != INDEX_NONE)
		{
			...
			if (AudioBusPtr.IsValid())
			{
				...
				if (AudioBusPtr->RemoveInstanceId(SourceId, SourceInfo.PlayOrder))
				{
#if UE_AUDIO_PROFILERTRACE_ENABLED
					TraceAudioBusStopMessage(MixerDevice->DeviceID, SourceInfo.AudioBusId);
					StopAutoAudioBusEnvelopeFollower(SourceInfo.AudioBusId, *AudioBusPtr);
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
			...
			for (uint32 AudioBusId : SourceInfo.AudioBusSends[AudioBusSendType])
			{
				...
				if (AudioBusPtr.IsValid())
				{
					if (AudioBusPtr->RemoveSend((EBusSendType)AudioBusSendType, SourceId))
					{
						...
						StopAutoAudioBusEnvelopeFollower(AudioBusId, *AudioBusPtr);
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
		...
		{
#if UE_AUDIO_PROFILERTRACE_ENABLED
			InstanceIdToPlayOrderMap.Remove(SourceInfo.MixerSourceBuffer->GetInstanceID());
			ActiveBusPatchWriters.Remove(SourceInfo.PlayOrder);
			ActiveBusPatchReaders.Remove(SourceInfo.PlayOrder);
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
		...
		for (FMixerSourceSubmixSend& SubmixSendItem : SourceInfo.SubmixSends)
		{
			...
			if (SubmixPtr.IsValid())
			{
				SubmixPtr->RemoveSourceById(SourceId);
			}
		}
		SourceInfo.SubmixSends.Reset();
		...
		SourceInfo.PostDistanceAttenuationBuffer.Empty();
	...
	{
		AUDIO_MIXER_CHECK(SourceId < NumLegacySources);
		...
		VolumeMod.SetModulators(InitParams.ModulationSettings.VolumeModulationDestination.Modulators);

		FModulationDestination PitchMod;
		PitchMod.Init(MixerDevice->DeviceID, FName("Pitch"), false /* bInIsBuffered */);
		PitchMod.SetModulators(InitParams.ModulationSettings.PitchModulationDestination.Modulators);

		FModulationDestination HighpassMod;
		HighpassMod.Init(MixerDevice->DeviceID, FName("HPFCutoffFrequency"), false /* bInIsBuffered */);
		HighpassMod.SetModulators(InitParams.ModulationSettings.HighpassModulationDestination.Modulators);

		FModulationDestination LowpassMod;
		LowpassMod.Init(MixerDevice->DeviceID, FName("LPFCutoffFrequency"), false /* bInIsBuffered */);
		LowpassMod.SetModulators(InitParams.ModulationSettings.LowpassModulationDestination.Modulators);
		...
		]() mutable
		{
			...
			if (SourceInfo.MixerSourceBuffer.IsValid())
			{
				InstanceIdToPlayOrderMap.Add(SourceInfo.MixerSourceBuffer->GetInstanceID(), SourceInfo.PlayOrder);
			}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
			...
			SourceInfo.PostDistanceAttenuationBuffer.Reset();
			SourceInfo.PostDistanceAttenuationBuffer.AddZeroed(BufferSize);
			...
			if (QuantData.QuantizedCommandPtr)
			{
				...
				{
					UE_LOGF(LogAudioMixer, Warning, "Quantization Clock: '%ls' Does not exist.", *QuantData.ClockName.ToString());
			...
			SourceInfo.bHasAttenuationSubmixSends = InitParams.bHasAttenuationSubmixSends;
			...
			if (InitParams.NumInputChannels <= InitParams.SourceEffectChainMaxSupportedChannels)
			{
				...
				if (InitParams.AudioBusId != INDEX_NONE)
				{
					...
					AudioBusPtr->AddInstanceId(SourceId, SourceInfo.PlayOrder, InitParams.NumInputChannels);
			...
			for (int32 BusSendType = 0; BusSendType < (int32)EBusSendType::Count; ++BusSendType)
			{
				for (const FInitAudioBusSend& AudioBusSend : InitParams.AudioBusSends[BusSendType])
				{
					...
					NewAudioBusSend.WaveInstancePlayOrder = SourceInfo.PlayOrder;
					...
					{
						NewAudioBusSend.Filter = AudioBusSend.Filter;
						const int32 NumSourceChannels = FMath::Max(SourceInfo.NumInputChannels, 1);
						NewAudioBusSend.Lpf.Init(MixerDevice->SampleRate, NumSourceChannels);
						NewAudioBusSend.Hpf.Init(MixerDevice->SampleRate, NumSourceChannels);
					}
			...
			for (int32 i = 0; i < InitParams.SubmixSends.Num(); ++i)
			{
				...
				if (SubmixPtr.IsValid())
				{
					FMixerSourceSubmixSend& AddedSend = SourceInfo.SubmixSends.Add_GetRef(MixerSubmixSend);
					AddedSend.SubmixId = SubmixPtr->GetId();

					if (AddedSend.Filter.IsFiltering())
					{
						// Use device output channel count: the filtered buffer is the source device-format output
						// buffer, not the destination submix output channel count.
						const int32 NumDeviceChannels = FMath::Max(MixerDevice->GetNumDeviceChannels(), 1);
						AddedSend.Lpf.Init(MixerDevice->SampleRate, NumDeviceChannels);
						AddedSend.Hpf.Init(MixerDevice->SampleRate, NumDeviceChannels);
						
						// Snap coefficients to the initial cutoffs immediately. bIsFirstFrequencyChange (reset by
						// Init) forces CurrInterpLength=0 in StartFrequencyInterpolation, so coefficients land on
						// the target without interpolation rather than ramping from the default.
						if (AddedSend.Filter.bEnableLPF)
						{
							AddedSend.Lpf.StartFrequencyInterpolation(AddedSend.Filter.LpfCutoff, 1);
						}
						
						if (AddedSend.Filter.bEnableHPF)
						{
							AddedSend.Hpf.StartFrequencyInterpolation(AddedSend.Filter.HpfCutoff, 1);
						}
					}
					...
					SubmixPtr->AddOrSetSourceById(SourceId, MixerSubmixSend.SendLevel, MixerSubmixSend.SubmixSendStage);
			...
			if (UseRenderScheduler())
			{
				GetRenderScheduler().AddStep(FAudioRenderStepId::FromWaveInstancePlayOrder(SourceInfo.PlayOrder), &SourceInfo);
			}
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
			if (AudioBusPtr->BusName.IsEmpty())
			{
	...
	void FMixerSourceManager::StopAudioBus(FAudioBusKey InAudioBusKey)
	{
		...
		AudioMixerThreadCommand([this, InAudioBusKey]()
		{
			...
			if (AudioBusPtr)
			{
				...
				{
#if UE_AUDIO_PROFILERTRACE_ENABLED
					StopAutoAudioBusEnvelopeFollower(InAudioBusKey.ObjectId, **AudioBusPtr);
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
	...
	void FMixerSourceManager::Play(const int32 SourceId)
	{
		...
		AudioMixerThreadCommand([this, SourceId]()
		{
			...
			UE_CLOGF(Audio::MatchesLogFilter(*SourceInfo.GetDebugName()), LogAudioTiming, Verbose,
				"FMixerSourceManager::Play (render thread), Wave=%ls, SourceId=%d", *SourceInfo.GetDebugName(), SourceId);
	...
	void FMixerSourceManager::StopInternal(const int32 SourceId)
	{
		...
		{
			UE_LOGF(LogAudioMixer, Display, "StopInternal() cancelling command [%ls]", *SourceInfo.QuantizedCommandHandle.CommandPtr->GetCommandName().ToString());
	...
	void FMixerSourceManager::SetModulationRouting(const int32 SourceId, FSoundModulationDefaultSettings& ModulationSettings)
	{
		...
		FModulationDestination VolumeMod;
		VolumeMod.Init(MixerDevice->DeviceID, FName("Volume"), false /* bInIsBuffered */, true /* bInValueLinear */);
		VolumeMod.SetModulators(ModulationSettings.VolumeModulationDestination.Modulators);

		FModulationDestination PitchMod;
		PitchMod.Init(MixerDevice->DeviceID, FName("Pitch"), false /* bInIsBuffered */);
		PitchMod.SetModulators(ModulationSettings.PitchModulationDestination.Modulators);

		FModulationDestination HighpassMod;
		HighpassMod.Init(MixerDevice->DeviceID, FName("HPFCutoffFrequency"), false /* bInIsBuffered */);
		HighpassMod.SetModulators(ModulationSettings.HighpassModulationDestination.Modulators);

		FModulationDestination LowpassMod;
		LowpassMod.Init(MixerDevice->DeviceID, FName("LPFCutoffFrequency"), false /* bInIsBuffered */);
		LowpassMod.SetModulators(ModulationSettings.LowpassModulationDestination.Modulators);
	...
	void FMixerSourceManager::SetSubmixSendInfo(const int32 SourceId, const FMixerSourceSubmixSend& InSubmixSend)
	{
		...
		AudioMixerThreadCommand([this, SourceId, InSubmixSend]()
		{
			...
			if (InSubmixPtr.IsValid())
			{
				...
				for (FMixerSourceSubmixSend& SubmixSend : SourceInfo.SubmixSends)
				{
					...
					if (SubmixPtr.IsValid())
					{
						if (SubmixPtr->GetId() == InSubmixPtr->GetId())
						{
							...
							const bool bWasFilteringEnabled = SubmixSend.Filter.IsFiltering();
							SubmixSend.Filter.bEnableLPF = InSubmixSend.Filter.bEnableLPF;
							SubmixSend.Filter.bEnableHPF = InSubmixSend.Filter.bEnableHPF;
							if (SubmixSend.Filter.IsFiltering())
							{
								if (!bWasFilteringEnabled)
								{
									const int32 NumDeviceChannels = FMath::Max(MixerDevice->GetNumDeviceChannels(), 1);
									SubmixSend.Lpf.Init(MixerDevice->SampleRate, NumDeviceChannels);
									SubmixSend.Hpf.Init(MixerDevice->SampleRate, NumDeviceChannels);
								}
								
								if (SubmixSend.Filter.bEnableLPF)
								{
									SubmixSend.Filter.LpfCutoff = InSubmixSend.Filter.LpfCutoff;
									SubmixSend.Lpf.StartFrequencyInterpolation(SubmixSend.Filter.LpfCutoff, 1);
								}
								
								if (SubmixSend.Filter.bEnableHPF)
								{
									SubmixSend.Filter.HpfCutoff = InSubmixSend.Filter.HpfCutoff;
									SubmixSend.Hpf.StartFrequencyInterpolation(SubmixSend.Filter.HpfCutoff, 1);
								}
							}
				...
				if (bIsNew)
				{
					FMixerSourceSubmixSend NewSend = InSubmixSend;
					NewSend.SubmixId = InSubmixPtr->GetId();
					if (NewSend.Filter.IsFiltering())
					{
						const int32 NumDeviceChannels = FMath::Max(MixerDevice->GetNumDeviceChannels(), 1);
						NewSend.Lpf.Init(MixerDevice->SampleRate, NumDeviceChannels);
						NewSend.Hpf.Init(MixerDevice->SampleRate, NumDeviceChannels);
						// Snap coefficients to the initial cutoffs immediately. bIsFirstFrequencyChange (reset by
						// Init) forces CurrInterpLength=0 in StartFrequencyInterpolation, so coefficients land on
						// the target without interpolation rather than ramping from the default.
						if (NewSend.Filter.bEnableLPF)
						{
							NewSend.Lpf.StartFrequencyInterpolation(NewSend.Filter.LpfCutoff, 1);
						}
						
						if (NewSend.Filter.bEnableHPF)
						{
							NewSend.Hpf.StartFrequencyInterpolation(NewSend.Filter.HpfCutoff, 1);
						}
					}
					
					SourceInfo.SubmixSends.Add(MoveTemp(NewSend));
				}
				...
				InSubmixPtr->AddOrSetSourceById(SourceId, InSubmixSend.SendLevel, InSubmixSend.SubmixSendStage);
			}
	...
	void FMixerSourceManager::ClearSubmixSendInfo(const int32 SourceId, const FMixerSourceSubmixSend& InSubmixSend)
	{
		...
		AudioMixerThreadCommand([this, SourceId, InSubmixSend]()
		{
			...
			if (InSubmixPtr.IsValid())
			{
				...
				InSubmixPtr->RemoveSourceById(SourceId);
			}
		}, AUDIO_MIXER_THREAD_COMMAND_STRING("ClearSubmixSendInfo()"));
	}

	void FMixerSourceManager::SetBusSendInfo(const int32 SourceId, EBusSendType InAudioBusSendType, uint32 AudioBusId, float BusSendLevel, const FString& InBusName, const FSendFilterSettings& InFilter)
	{
		AUDIO_MIXER_CHECK(SourceId < NumLegacySources);
		AUDIO_MIXER_CHECK(GameThreadInfo.bIsBusy[SourceId]);
		AUDIO_MIXER_CHECK_GAME_THREAD(MixerDevice);

		AudioMixerThreadCommand([this, SourceId, InAudioBusSendType, AudioBusId, BusSendLevel, InBusName, InFilter]()
		{
			FSourceInfo& SourceInfo = SourceInfos[SourceId];
			...
			BusSend.WaveInstancePlayOrder = SourceInfo.PlayOrder;
			...
			BusSend.Filter = InFilter;
			if (InFilter.IsFiltering())
			{
				const int32 NumSourceChannels = FMath::Max(SourceInfo.NumInputChannels, 1);
				BusSend.Lpf.Init(MixerDevice->SampleRate, NumSourceChannels);
				BusSend.Hpf.Init(MixerDevice->SampleRate, NumSourceChannels);
			}
	...
	void FMixerSourceManager::ComputeSourceBuffer(const bool bGenerateBuses, const int32 SourceId)
	{
		...
		UE_CLOGF(Audio::MatchesLogFilter(*SourceInfo.GetDebugName()), LogAudioTiming, Verbose,
			"ComputeSourceBuffer Name=%ls, NumOutputFrames=%d, FramesPlayed=%lld, CurrentFrameIndex=%d ",
		...
		else
		{
			...
			const bool bIsGeneratorSource = SourceId >= NumLegacySources;
			if (bIsGeneratorSource)
			{
				SourceInfo.bHasStarted = true;

				const int32 GeneratorSourceIndex = SourceId - NumLegacySources;
				const ISoundGeneratorPtr& SourceGenerator = GeneratorSourceState[GeneratorSourceIndex].Generator;
				check(SourceGenerator.IsValid());
				
				TArrayView<float> WriteBuffer(PreDistanceAttenBufferPtr, NumSamples);
				while (WriteBuffer.Num() > 0)
				{
					const int32 SamplesGenerated = SourceGenerator->OnGenerateAudio(WriteBuffer.GetData(), WriteBuffer.Num());
					WriteBuffer = WriteBuffer.RightChop(SamplesGenerated);
					if (SourceGenerator->IsFinished())
					{
						SourceInfo.bIsLastBuffer = true;
						break;
					}
					else if (!ensureMsgf(SamplesGenerated > 0, TEXT("Source generator returned no samples but is not finished!")))
					{
						break;
					}
				}
				
				const int32 TotalSamplesGenerated = NumSamples - WriteBuffer.Num();
				SourceInfo.CurrentFrameIndex += TotalSamplesGenerated / SourceInfo.NumInputChannels;
				SourceInfo.NumFramesPlayed += TotalSamplesGenerated / SourceInfo.NumInputChannels;
			}
			else
			{
	...
	void FMixerSourceManager::ConnectBusPatches()
	{
		while (TOptional<FPendingAudioBusConnection> PendingAudioBusConnection = PendingAudioBusConnections.Dequeue())
		{
			...
			if (bAlreadyExisted)
			{
				if (!ensure(AudioBusPtr->GetNumChannels() == NumChannels))
				{
					// Refuse the connection if the patch's channel count disagrees with the bus. The patch carries interleaved samples sized for NumChannels, so attaching it
					//  to a bus with a different channel count would corrupt the bus output.
					UE_LOGF(LogAudioMixer, Error, "Audio bus patch channel count mismatch for bus id %u: bus has %d channels, patch has %d. Refusing connection to avoid distorted audio.",
						AudioBusKey.ObjectId, AudioBusPtr->GetNumChannels(), NumChannels);
					continue;
				}
			...
			case FPendingAudioBusConnection::FPatchVariant::IndexOfType<FPatchInput>():
				if (UseRenderScheduler())
				{
					const uint32 PlayOrder = PendingAudioBusConnection->WaveInstancePlayOrder;
					if (PlayOrder != INDEX_NONE)
					{
						FAudioRenderStepId FirstStep = FAudioRenderStepId::FromWaveInstancePlayOrder(PlayOrder);
						FAudioRenderStepId SecondStep = FAudioRenderStepId::FromAudioBusKey(AudioBusKey);
						AddBusPatchDependency(PlayOrder, FirstStep, SecondStep);
					}
				}
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
				{
					const uint32 PlayOrder = ResolvePlayOrder(*PendingAudioBusConnection);
					if (PlayOrder != INDEX_NONE)
					{
						TraceBusPatchWriterConnectedMessage(MixerDevice->DeviceID, PlayOrder, AudioBusKey.ObjectId);
						ActiveBusPatchWriters.AddUnique(PlayOrder, AudioBusKey.ObjectId);
					}
				}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
			...
			case FPendingAudioBusConnection::FPatchVariant::IndexOfType<FPatchOutputStrongPtr>():
				if (UseRenderScheduler())
				{
					...
					if (PlayOrder != INDEX_NONE)
					{
						...
						FAudioRenderStepId SecondStep = FAudioRenderStepId::FromWaveInstancePlayOrder(PlayOrder);
						AddBusPatchDependency(PlayOrder, FirstStep, SecondStep);
					}
				}
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
				{
					...
					if (PlayOrder != INDEX_NONE)
					{
						TraceBusPatchReaderConnectedMessage(MixerDevice->DeviceID, AudioBusKey.ObjectId, PlayOrder);
						ActiveBusPatchReaders.AddUnique(PlayOrder, AudioBusKey.ObjectId);
					}
				}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
	...
	void FMixerSourceManager::UpdateBuses()
	{
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
		const bool bAudioChannelEnabled = UE_TRACE_CHANNELEXPR_IS_ENABLED(AudioChannel);
		const bool bAudioMixerChannelEnabled = UE_TRACE_CHANNELEXPR_IS_ENABLED(AudioMixerChannel);
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
		...
		for (auto& [AudioBusKey, AudioBus] : AudioBuses)
		{
			...
			if (bAudioChannelEnabled)
			{
				...
				if (BusIsAlivePingTimeSec >= BusIsAlivePingWaitTimeSec)
				{
					TraceAudioBusIsAlivePingMessage(MixerDevice->DeviceID, AudioBusKey.ObjectId, AudioBus->BusName, AudioBus->NumChannels);
				}
			}

			if (bAudioMixerChannelEnabled)
			{
				if (BusIsAlivePingTimeSec >= BusIsAlivePingWaitTimeSec)
				{
					AutomaticallyStartAudioBusEnvelopeFollower(AudioBusKey, AudioBus);
				}

				if (AudioBus->bIsEnvelopeFollowing)
				{
					UE_TRACE_LOG(Audio, AudioBusEnvelopeValues, AudioMixerChannel)
			...
			else if (AudioBus->bIsEnvelopeFollowing)
			{
				StopAutoAudioBusEnvelopeFollower(AudioBusKey.ObjectId, *AudioBus);
			}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
		}
		...
		if (bAudioChannelEnabled)
		{
			if (BusIsAlivePingTimeSec >= BusIsAlivePingWaitTimeSec)
			{
				for (const auto& [PlayOrder, BusObjectId] : ActiveBusPatchWriters)
				{
					TraceBusPatchWriterConnectedMessage(MixerDevice->DeviceID, PlayOrder, BusObjectId);
				}

				for (const auto& [PlayOrder, BusObjectId] : ActiveBusPatchReaders)
				{
					TraceBusPatchReaderConnectedMessage(MixerDevice->DeviceID, BusObjectId, PlayOrder);
				}

				BusIsAlivePingTimeSec = 0.0;
			}
			else
			{
				BusIsAlivePingTimeSec += MixerDevice->GetAudioClockDelta();
			}
		}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
	}

	void FMixerSourceManager::AddBusPatchDependency(const uint32 WaveInstancePlayOrder, const FAudioRenderStepId FirstStep, const FAudioRenderStepId SecondStep)
	{
		check(UseRenderScheduler());
		if (FirstStep.IsValid() && SecondStep.IsValid())
		{
			GetRenderScheduler().AddDependency(FirstStep, SecondStep);
			BusPatchDependencies.Add(WaveInstancePlayOrder, { FirstStep, SecondStep });
		}
	}

	void FMixerSourceManager::RemoveBusPatchDependencies(const uint32 WaveInstancePlayOrder)
	{
		check(UseRenderScheduler());
		TArray<TPair<FAudioRenderStepId, FAudioRenderStepId>> Dependencies;
		BusPatchDependencies.MultiFind(WaveInstancePlayOrder, Dependencies, false);
		if (!Dependencies.IsEmpty())
		{
			FAudioRenderScheduler& Scheduler = GetRenderScheduler();
			for (TPair<FAudioRenderStepId, FAudioRenderStepId>& Dependency : Dependencies)
			{
				Scheduler.RemoveDependency(Dependency.Get<0>(), Dependency.Get<1>());
			}
			BusPatchDependencies.Remove(WaveInstancePlayOrder);
		}
	}
	...
	void FMixerSourceManager::ComputePostSourceEffectBuffer(const bool bGenerateBuses, const int32 SourceId)
	{
		...
		{
			SourceInfo.PreEffectBuffer.Reset();
			SourceInfo.PreEffectBuffer.AddZeroed(SourceInfo.PreDistanceAttenuationBuffer.Num());
		...
		// Update volume fade information if we're stopping
		{
			...
			if (bChannelEnabled)
			{
				...
				TraceSourceToSubmixSendMessage(SourceInfo);
				TraceSourceToBusSendMessage(SourceId, SourceInfo);
			}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
		}
		...
		if (SourceInfo.SourceListener && !bWasEffectTailsDone && SourceInfo.bEffectTailsDone)
		{
		...
		if (SourceInfo.IsRenderingToSubmixes() || SpatialInterfaceInfo.bSpatializationIsExternalSend || SourceInfo.AudioLink.IsValid())
		{
			...
			// Copy output buffer to Post-Attenuation Bus Send
			SourceInfo.PostDistanceAttenuationBuffer = SourceInfo.SourceBuffer;
	...
	void FMixerSourceManager::MixOutputBuffers(const int32 SourceId, int32 InNumOutputChannels, const float InSendLevel, EMixerSourceSubmixSendStage InSubmixSendStage, uint32 InSubmixId, FAlignedFloatBuffer& ScratchBuffer, FAlignedFloatBuffer& OutWetBuffer)
	{
		if (InSendLevel > 0.0f)
		{
			FSourceInfo& SourceInfo = SourceInfos[SourceId];
			...
			if (!SourceInfo.bIsPaused && !SourceInfo.bIsPausedForQuantization && !SourceInfo.bIsDone && SourceInfo.bIsPlaying)
			{
				...
				// Find the send matching this submix to check for per-send filter state.
				// SubmixId is cached on the send to avoid atomic Pin() calls on every buffer tick.
				FMixerSourceSubmixSend* MatchingSend = nullptr;
				for (FMixerSourceSubmixSend& Send : SourceInfo.SubmixSends)
				{
					if (Send.SubmixId == InSubmixId)
					{
						MatchingSend = &Send;
						break;
					}
				...
				if (MatchingSend && MatchingSend->Filter.IsFiltering())
				{
					SourceSubmixOutputBuffer.MixOutput(InSendLevel, InSubmixSendStage, MatchingSend->Filter, MatchingSend->Lpf, MatchingSend->Hpf, ScratchBuffer, OutWetBuffer);
				}
				else
				{
	...
	const float* FMixerSourceManager::GetPostDistanceAttenuationBuffer(const int32 SourceId) const
	{
		const FSourceInfo& SourceInfo = SourceInfos[SourceId];
		...
		return SourceInfo.PostDistanceAttenuationBuffer.GetData();
	}
	...
	void FMixerSourceManager::SetSourceDone(const int32 SourceId)
	{
		FSourceInfo& SourceInfo = SourceInfos[SourceId];
		...
		if (SourceId >= NumLegacySources)
		{
			FGeneratorSourceState& State = GeneratorSourceState[SourceId - NumLegacySources];
			if (State.Generator.IsValid())
			{
				State.Generator->OnEndGenerate();
				State.Generator.Reset();
			}
			State.bSourceBusy = false;
		}
	}
	...
	void FMixerSourceManager::AudioMixerThreadCommand(TFunction<void()>&& InFunction, const char* InDebugString, bool bInDeferExecution /*= false*/)
	{
		...
		int32 AudioThreadCommandIndex = -1;
		{
			...
			if (LogCmdQueueWhenNumberReachedCVar > 0 && LogCmdQueueWhenNumberReachedCVar == Commands.SourceCommandQueue.Num())
			{
				...
				{
					UE_LOGF(LogAudioMixer, Display, "Commands %ls:%d", StringCast<TCHAR>(i.Key).Get(), i.Value);
				}
			}
		...
		if (OldMax != NewMax)
		{
			...
			// if so, set a flag that the AudioMixerDevice can check and try swapping to safe hardware
			...
			if (CurrentBufferSizeInBytes >= MaxBufferSizeInBytes)
			{
				...
				bShouldResetHardwareCallback = true;
			}
		}
	...
	void FMixerSourceManager::FlushCommandQueue(bool bPumpInCommand, bool bSkipWaitForRenderThread)
	{
		check(CommandsProcessedEvent != nullptr);
		...
		{
			UE_LOGF(LogAudioMixer, Verbose, "No commands were queued while flushing the source manager.");
			return;
		}

		bool bTimedOut = false;

		if (bSkipWaitForRenderThread)
		{
			// The render thread is known to be stopped (e.g. during device swap).
			// Skip waiting on CommandsProcessedEvent since no thread will signal it.
			// We'll pump commands inline below.
			CommandsProcessedEvent->Trigger();
			bTimedOut = true;
		}
		// Make sure render thread is still executing
		else if (!CommandsProcessedEvent->Wait(CommandBufferFlushWaitTimeMsCvar))
		{
			CommandsProcessedEvent->Trigger();
			bTimedOut = true;
			UE_LOGF(LogAudioMixer, Warning, "Timed out waiting to flush the source manager command queue (1).");
		}
		else
		{
			UE_LOGF(LogAudioMixer, Verbose, "Flush succeeded in the source manager command queue (1).");
		}
		...
		if (bSkipWaitForRenderThread)
		{
			// Already pumped inline, no second wait needed.
			CommandsProcessedEvent->Trigger();
		}
		// wait one more time for the double pump
		else if (!CommandsProcessedEvent->Wait(1000))
		{
			CommandsProcessedEvent->Trigger();
			UE_LOGF(LogAudioMixer, Warning, "Timed out waiting to flush the source manager command queue (2).");
		}
		else
		{
			UE_LOGF(LogAudioMixer, Verbose, "Flush succeeded the source manager command queue (2).");
		}
	}
	...
	{
		return bEnableBaseSubmix || bEnableSubmixSends || bHasAttenuationSubmixSends;
	}
	...
	void FMixerSourceManager::LogRenderThreadStall()
	{
		...
		{
			FReadScopeLock Lock(CurrentlyExecutingCmdLock);
			UE_LOGF(LogAudioMixer, Warning, "Stall in Cmd Queue: Cmd='%ls', Executing For: %2.5f secs, AudioRenderThread='%ls'",
		...
		{
			UE_LOGF(LogAudioMixer, Warning, "Stall in AudioRenderThread Phase: '%ls'", ToCStr(LexToString(RenderThreadPhase)));
		}
	}
	...
	void FMixerSourceManager::LogInflightAsyncTasks()
	{
		...
		{
			UE_LOGF(LogAudioMixer, Warning, "Inflight Task: %ls, %2.2f secs, Procedural=%d",
	...
	void FMixerSourceManager::LogCallstack(uint32 InThreadId)
	{
		if (InThreadId != INVALID_AUDIO_RENDER_THREAD_ID)
		{
			...
			UE_LOGF(LogAudioMixer, Warning, "***** ThreadStackWalkAndDump for ThreadId(%lu) ******\n%ls", InThreadId, ANSI_TO_TCHAR(StackTrace));
		}
	}

	int32 FMixerSourceManager::AllocateGeneratorSource(ISoundGeneratorRef InGenerator)
	{
		auto IsFree = [this](const FGeneratorSourceState& State)
			{
				return !State.bAllocated && !State.bSourceBusy;
			};
		const int32 GeneratorSourceIndex = GeneratorSourceState.IndexOfByPredicate(IsFree);
		if (GeneratorSourceIndex != INDEX_NONE)
		{
			GeneratorSourceState[GeneratorSourceIndex].bAllocated = true;
			GeneratorSourceState[GeneratorSourceIndex].bSourceBusy = true;
			const int32 SourceId = GeneratorSourceIndex + NumLegacySources;
			check(SourceId >= NumLegacySources && SourceId < NumTotalSources);
			AudioMixerThreadMPSCCommand([this, SourceId, InGenerator]
				{
					static int32 PlayOrder = 1 << 31;

					FSourceInfo& SourceInfo = SourceInfos[SourceId];
					SourceInfo.bIsBusy = true;
					SourceInfo.NumInputChannels = InGenerator->GetNumChannels();
					SourceInfo.PlayOrder = PlayOrder++;

					check(SourceInfo.NumInputChannels > 0);
					GeneratorSourceState[SourceId - NumLegacySources].Generator = InGenerator;
					InGenerator->OnBeginGenerate();

					// Initialize the submix output source for this source id
					FMixerSourceSubmixOutputBufferSettings SourceSubmixOutputResetSettings = {};
					SourceSubmixOutputResetSettings.NumOutputChannels = MixerDevice->GetDeviceOutputChannels();
					SourceSubmixOutputResetSettings.NumSourceChannels = SourceInfo.NumInputChannels;
					SourceSubmixOutputBuffers[SourceId].Reset(SourceSubmixOutputResetSettings);

					MixerDevice->Get2DChannelMap(SourceInfo.bIsVorbis, SourceSubmixOutputResetSettings.NumSourceChannels, SourceSubmixOutputResetSettings.NumOutputChannels, SourceInfo.bIsCenterChannelOnly, SourceInfo.ScratchChannelMap);
					SourceSubmixOutputBuffers[SourceId].SetChannelMap(SourceInfo.ScratchChannelMap, SourceInfo.bIsCenterChannelOnly);

					FMixerSubmixPtr SubmixPtr = MixerDevice->GetMasterSubmix().Pin();
					if (SubmixPtr.IsValid())
					{
						SubmixPtr->AddOrSetSourceById(SourceId, 1.0f, EMixerSourceSubmixSendStage::PostDistanceAttenuation);
					}
	...
	void FMixerSourceManager::FreeGeneratorSource(const int32 SourceId)
	{
		const int32 GeneratorSourceIndex = SourceId - NumLegacySources;
		check(SourceId >= NumLegacySources && SourceId < NumTotalSources && GeneratorSourceState[GeneratorSourceIndex].bAllocated);
		GeneratorSourceState[GeneratorSourceIndex].bAllocated = false;
		AudioMixerThreadMPSCCommand([this, SourceId, GeneratorSourceIndex]
			{
				FMixerSubmixPtr SubmixPtr = MixerDevice->GetMasterSubmix().Pin();
				if (SubmixPtr.IsValid())
				{
				...
				ISoundGeneratorPtr& Generator = GeneratorSourceState[GeneratorSourceIndex].Generator;
				if (GeneratorSourceState[GeneratorSourceIndex].bSourceBusy && ensure(Generator.IsValid()))
				{
					Generator->OnEndGenerate();
					Generator.Reset();
				}
				ReleaseSource(SourceId);
				GeneratorSourceState[GeneratorSourceIndex].bSourceBusy = false;
			}
		);
	}

	void FMixerSourceManager::PlayGeneratorSource(const int32 SourceId)
	{
		check(SourceId >= NumLegacySources && SourceId < NumTotalSources && GeneratorSourceState[SourceId - NumLegacySources].bAllocated);
		auto Execute = [this, SourceId]()
			{
				FSourceInfo& SourceInfo = SourceInfos[SourceId];
				SourceInfo.bIsDone = false;
				SourceInfo.bIsPlaying = true;
				SourceInfo.bIsPaused = false;
				SourceInfo.bIsActive = true;
				SourceInfo.bEnableBaseSubmix = true;
			};
		...
		{
			AudioMixerThreadMPSCCommand(MoveTemp(Execute));
		}
	}

	void FMixerSourceManager::StopGeneratorSource(const int32 SourceId)
	{
		check(SourceId >= NumLegacySources && SourceId < NumTotalSources && GeneratorSourceState[SourceId - NumLegacySources].bAllocated);
		auto Execute = [this, SourceId]()
			{
	...
	bool FMixerSourceManager::IsGeneratorSourceDone(const int32 SourceId)
	{
		check(SourceId >= NumLegacySources && SourceId < NumTotalSources && GeneratorSourceState[SourceId - NumLegacySources].bAllocated);
		return !GeneratorSourceState[SourceId - NumLegacySources].bSourceBusy;
	}
