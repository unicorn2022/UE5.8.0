...
#define AUDIO_RENDER_SCHEDULER_CHECK_THREAD() ensure(MixerDevice == nullptr || MixerDevice->IsAudioRenderingThread())

namespace Audio
{
	FAudioRenderStepId FAudioRenderStepId::FromWaveInstancePlayOrder(const uint32 PlayOrder)
	{
		return PlayOrder | (uint64(1) << 63);
	}
	...
	void FAudioRenderScheduler::AddStep(const FAudioRenderStepId Id, IAudioMixerRenderStep* Step)
	{
		...
		{
			UE_LOGFMT(LogAudioRenderScheduler, Verbose, "Adding render step '{Name}' with id {Id}",
				("Name", Step->GetRenderStepName()),
				("Id", Id.GetRawValue()));
		...
		{
			UE_LOGFMT(LogAudioRenderScheduler, Error, "Tried to add duplicate render step '{Name}' with id {Id}",
				("Name", Step->GetRenderStepName()),
				("Id", Id.GetRawValue()));
	...
	void FAudioRenderScheduler::RemoveStep(const FAudioRenderStepId Id)
	{
		...
		if (Entry.StepInterface != nullptr)
		{
			UE_LOGFMT(LogAudioRenderScheduler, Verbose, "Removing render step '{Name}' with id {Id}",
				("Name", Entry.StepInterface->GetRenderStepName()),
				("Id", Id.GetRawValue()));
		...
		{
			UE_LOGFMT(LogAudioRenderScheduler, Error, "Tried to remove unknown render step with id {Id}",
				("Id", Id.GetRawValue()));
#if ENABLE_AUDIO_DEBUG
			DumpOnError();
#endif
	...
	void FAudioRenderScheduler::RemoveDependency(const FAudioRenderStepId FirstStep, const FAudioRenderStepId SecondStep)
	{
		...
		if (Steps[SecondIndex].Prerequisites.RemoveSingleSwap(FirstIndex))
		{
		...
		{
			UE_LOGFMT(LogAudioRenderScheduler, Error, "Unknown dependency removed from audio render scheduler: {Id1} -> {Id2}",
				("Id1", FirstStep.GetRawValue()),
				("Id2", SecondStep.GetRawValue()));
#if ENABLE_AUDIO_DEBUG
			DumpOnError();
#endif
		}
	...
	void FAudioRenderScheduler::BreakCycle(const int FirstPendingStepIndex)
	{
		...
		while (true)
		{
			...
			if (VisitedIndices.Contains(PrerequisiteStep))
			{
				...
				UE_LOGFMT(LogAudioRenderScheduler, Display, "Breaking cycle between step {Id1} and step {Id2}",
					("Id1", Steps[CurrentStep].StepId.GetRawValue()),
					("Id2", Steps[PrerequisiteStep].StepId.GetRawValue()));
...
#if ENABLE_AUDIO_DEBUG
	void FAudioRenderScheduler::DumpSteps()
	{
		bShouldDumpSteps = true;
	}

	void FAudioRenderScheduler::DumpStepsInternal()
	{
		for (const FStepEntry& Step : Steps)
		{
			if (Step.ReferenceCount == 0)
			{
				continue;
			}

			UE_LOGFMT(LogAudioRenderScheduler, Display, "Index: {Index} Id: {Id} Name: '{Name}' Prerequisites: {Prerequisites}",
				("Index", int(&Step - &Steps[0])),
				("Id", Step.StepId.GetRawValue()),
				("Name", Step.StepInterface ? Step.StepInterface->GetRenderStepName() : TEXT("(Empty)")),
				("Prerequisites", Step.Prerequisites));
		}

		UE_LOGFMT(LogAudioRenderScheduler, Display, "Step ordering: {Steps}", ("Steps", ActiveSteps));
	}

	void FAudioRenderScheduler::DumpOnError()
	{
		// Only do this once so we don't spam in a persistent error state.
		if (!bDidDumpOnError)
		{
			bDidDumpOnError = true;
			bShouldDumpSteps = true;
		}
	}
#endif
