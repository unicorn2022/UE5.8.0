...
namespace Audio::SoundFileUtils
{
	...
	bool AUDIOMIXER_API ConvertAudioToWav(const TArray<uint8>& InAudioData, TArray<uint8>& OutWaveData)
	{
		...
		while (InputSamplesRead == ProcessBuffer.Num())
		{
			...
			{
				UE_LOGF(LogAudioMixer, Error, "Error encountered while converting audio to wav, file too large... skipping");
	...
	void CopyOptionalWavChunks(TSharedPtr<ISoundFileReader>& InSoundDataReader, const int32 InInputFormat, TSharedPtr<ISoundFileWriter>& InSoundFileWriter, const int32 InOutputFormat, const TSet<uint32>& ChunkIdsToSkip)
	{
		...
		if ((InInputFormat & ESoundFileFormat::WAV) && (InOutputFormat & ESoundFileFormat::WAV))
		{
			...
			if (Error != ESoundFileError::Type::NONE)
			{
				UE_LOGF(LogAudioMixer, Error, "Error encountered while reading optional chunk data...skipping");
			}
			else
			{
				...
				if (Error != ESoundFileError::Type::NONE)
				{
					UE_LOGF(LogAudioMixer, Error, "Error encountered while writing optional chunk data...skipping");
				}
			}
		}
	}
	...
	bool AUDIOMIXER_API CreateCueAndSampleChunks(const TArray<uint8>& InAudioData, TArray<uint8>& OutWaveData, const TArray<FWaveCue>& InWaveCues, const TArray<FWaveSampleLoop>& InSampleLoops)
	{
		...
		if (Error != ESoundFileError::Type::NONE)
		{
			UE_LOGF(LogAudioMixer, Error, "Error encountered while initializing InputSoundDataReader... returning false");
		...
		if (Error != ESoundFileError::Type::NONE)
		{
			UE_LOGF(LogAudioMixer, Error, "Error encountered while initializing SoundFileWriter... returning false");
		...
		if (!InWaveCues.IsEmpty())
		{
			...
			if (Error != ESoundFileError::Type::NONE)
			{
				UE_LOGF(LogAudioMixer, Error, "Error encountered while writing cue data... returning false");
			...
			if (Error != ESoundFileError::Type::NONE)
			{
				UE_LOGF(LogAudioMixer, Error, "Error encountered while writing list chunk data... returning false");
		...
		if (!InSampleLoops.IsEmpty())
		{
			...
			if (NewSoundFileDescription.SampleRate <= 0)
			{
				UE_LOGF(LogAudioMixer, Error, "Invalid sample rate (%d) in audio data; cannot write smpl chunk", NewSoundFileDescription.SampleRate);
				return false;
			}
			...
			if (Error != ESoundFileError::Type::NONE)
			{
				UE_LOGF(LogAudioMixer, Error, "Error encountered while writing sample chunk data... returning false");
		...
		if (Error != ESoundFileError::Type::NONE)
		{
			UE_LOGF(LogAudioMixer, Error, "Error encountered while ReadingSamples... returning false");
