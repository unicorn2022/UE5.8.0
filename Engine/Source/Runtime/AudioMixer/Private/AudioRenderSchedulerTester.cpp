	...
	class FAudioRenderSchedulerTesterImpl : public IAudioRenderSchedulerTester
	{
		...
		virtual void AddStep(int Id) override
		{
			if (!Steps.Contains(Id))
			{
				...
				Scheduler.AddStep(FAudioRenderStepId::FromWaveInstancePlayOrder(Id), Steps[Id].Get());
			}
		}

		virtual void AddDependency(int FirstStep, int SecondStep) override
		{
			Scheduler.AddDependency(FAudioRenderStepId::FromWaveInstancePlayOrder(FirstStep), FAudioRenderStepId::FromWaveInstancePlayOrder(SecondStep));
		}

		virtual void RemoveDependency(int FirstStep, int SecondStep) override
		{
			Scheduler.RemoveDependency(FAudioRenderStepId::FromWaveInstancePlayOrder(FirstStep), FAudioRenderStepId::FromWaveInstancePlayOrder(SecondStep));
		}
