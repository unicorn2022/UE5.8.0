...
namespace Audio
{
	...
	bool FSoundSourceDecoder::InitDecodingSourceInternal(const FSourceDecodeInit& InitData)
	{
		...
		if (DecodingSoundWaveDataPtr->PreInit(SampleRate))
		{
			...
			EnqueueDecoderCommand([this, InitData, DecodingSoundWaveDataPtr]()
			{
				...
				UE_LOGF(LogAudioMixer, Verbose, "Decoding SoundWave '%ls' (Num Decoding: %d)",
		...
		UE_LOGF(LogAudioMixer, Warning, "Failed to initialize sound wave %ls.", InitData.SoundWave ? *InitData.SoundWave->GetName() : TEXT("Unset"));
		return false;
	}
	...
	bool FSoundSourceDecoder::InitDecodingSource(const FSourceDecodeInit& InitData)
	{
		...
		{
			UE_LOGF(LogAudioMixer, Error, "Cannot Decode NULL SoundWave");
			return false;
		}

		if (InitData.SoundWave->NumChannels == 0)
		{
			UE_LOGF(LogAudioMixer, Error, "Cannot Decode invalid or corrupt sound wave %ls. NumChannels = 0", *InitData.SoundWave->GetName());
			return false;
		}

		if (InitData.SoundWave->NumChannels <= 0 || InitData.SoundWave->NumChannels > 2)
		{
			UE_LOGF(LogAudioMixer, Error, "Only supporting 1 or 2 channel decodes in sound source decoder.");
			return false;
		}
		...
		{
			UE_LOGF(LogAudioMixer, Warning, "Sound wave decoder does not support buses or procedural sounds.");
			return false;
		}
		...
		else
		{
			...
			{
				AudioDevice->Precache(InitData.SoundWave, true, false, false);
			}
