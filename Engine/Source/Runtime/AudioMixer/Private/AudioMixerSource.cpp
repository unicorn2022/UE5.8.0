...
UE_TRACE_EVENT_BEGIN(Audio, SoundWaveStart)
	...
	UE_TRACE_EVENT_FIELD(int32, SourceBusID)
	UE_TRACE_EVENT_FIELD(UE::Trace::WideString, AudioComponentName)
UE_TRACE_EVENT_END()
...
UE_TRACE_EVENT_BEGIN(Audio, SoundWaveStop)
	UE_TRACE_EVENT_FIELD(uint32, DeviceId)
	UE_TRACE_EVENT_FIELD(uint64, Timestamp)
	UE_TRACE_EVENT_FIELD(uint32, PlayOrder)
	UE_TRACE_EVENT_FIELD(uint32, ActiveSoundPlayOrder)
...
static float RelativeRenderCostLogThresholdCVar = -1.f;
FAutoConsoleVariableRef CVarRelativeRenderCostLogThreshold(
	TEXT("au.RelativeRenderCostLogThresholdCVar"),
	RelativeRenderCostLogThresholdCVar,
	TEXT("Log (verbose) if the Relative Render Cost of a playing wave exceeds this threshold. " \
		"Default: -1.f (disabled), Enabled for values >= 0"),
	ECVF_Default);
...
namespace Audio
{
	namespace MixerSourcePrivate
	{
		...
		int32 GetSourceBusID(const TObjectPtr<USoundWave> WaveData)
		{
			if (WaveData && WaveData->bIsSourceBus)
			{
				const TObjectPtr<USoundSourceBus> SoundSourceBus = CastChecked<USoundSourceBus>(WaveData);
				return SoundSourceBus->AudioBus ? SoundSourceBus->AudioBus->GetUniqueID() : WaveData->GetUniqueID();
			}
	...
	bool FMixerSource::Init(FWaveInstance* InWaveInstance)
	{
		...
		{
			UE_LOGF(LogAudioMixer, Warning, "Soundwave %ls has invalid compressed data.", *(WaveData->GetName()));
			FreeResources();
			return false;
		}
		...
		if (SoundBuffer->NumChannels > 0)
		{
			...
			{
				UE_LOGF(LogAudioMixer, Warning, "Sound wave %ls was flagged as being ambisonics but had a channel count of %d. Currently the audio engine only supports FOA sources that have four channels.", *InWaveInstance->GetName(), WaveData->NumChannels);
			}
			if (ActiveSound)
			{
			...
			InitParams.bHasAttenuationSubmixSends = WaveInstance->AttenuationSubmixSends.Num() > 0;
			...
			else
			{
				...
				if (FMixerSubmixPtr SubmixPtr = SubmixWeakPtr.Pin())
				{
					...
					{
						UE_LOGF(LogAudioMixer, Warning, "Ignoring soundfield Base Submix destination being set on SoundWave (%ls) because spatialization method is set to Binaural."
			...
			for (FSoundSubmixSendInfo& SendInfo : WaveInstance->SoundSubmixSends)
			{
				if (SendInfo.SoundSubmix != nullptr)
				{
					...
					SubmixSend.SoundfieldFactory = MixerDevice->GetFactoryForSubmixInstance(SubmixSend.Submix);
					SubmixSend.Filter = FSendFilterSettings(SendInfo.bEnableLPFCutoff != 0, SendInfo.bEnableHPFCutoff != 0,
						SendInfo.LPFCutoff, SendInfo.HPFCutoff);
					InitParams.SubmixSends.Add(SubmixSend);
				}
			}
			...
			UE_CLOGF(Audio::MatchesLogFilter(*WaveInstance->GetName()), LogAudioTiming, Verbose,
				"FMixerSource::Init Name=%ls,BufferType=%d,CachedRealtimeFirstBuffer=0x%p",*WaveInstance->GetName(),
			...
			{
				InitializationState = EMixerSourceInitializationState::NotInitialized;
				UE_LOGF(LogAudioMixer, Warning, "Failed to initialize mixer source voice '%ls'.", *InWaveInstance->GetName());
			}
		}
		else
		{
			UE_LOGF(LogAudioMixer, Warning, "Num channels was 0 for sound buffer '%ls'.", *InWaveInstance->GetName());
		}
		...
		for (int32 BusSendType = 0; BusSendType < (int32)EBusSendType::Count; ++BusSendType)
		{
			...
			for (FSoundSourceBusSendInfo& SendInfo : WaveInstance->BusSends[BusSendType])
			{
				...
				auto SetupBusSend = [this](TArray<FInitAudioBusSend>* AudioBusSends, const FSoundSourceBusSendInfo& InSendInfo, int32 InBusSendType, uint32 InBusId, bool bEnableBusSends, int32 InBusChannels, const FString& InBusName)
				{
					...
					BusSend.Filter = FSendFilterSettings(InSendInfo.bEnableLPFCutoff != 0, InSendInfo.bEnableHPFCutoff != 0,
						InSendInfo.LPFCutoff, InSendInfo.HPFCutoff);
					...
					NewDynamicBusSendInfo.Filter = FSendFilterSettings(InSendInfo.bEnableLPFCutoff != 0, InSendInfo.bEnableHPFCutoff != 0,
						InSendInfo.LPFCutoff, InSendInfo.HPFCutoff);
					...
					for (FDynamicBusSendInfo& BusSendInfo : DynamicBusSendInfos)
					{
						...
						{
							// Detect filter param changes before overwriting so the dirty-check can dispatch them.
							const bool bFilteringChanged = BusSendInfo.Filter.HasChanged(NewDynamicBusSendInfo.Filter);
							BusSendInfo = NewDynamicBusSendInfo;
							BusSendInfo.bFilterStateChanged |= bFilteringChanged;
	...
	void FMixerSource::Update()
	{
		...
		UE_CLOGF(Audio::MatchesLogFilter(*WaveInstance->GetName()), LogAudioTiming, Verbose,
			"FMixerSource::Update, Name=%ls, StartTime=%.2f bProcedural=%d, bIsSourceBus=%d, CachedRealTimeFirstBuffer=0x%p, MixerSourceBufferValid=%ls, TickCount=%d",
	...
	bool FMixerSource::PrepareForInitialization(FWaveInstance* InWaveInstance)
	{
		...
		UE_CLOGF(Audio::MatchesLogFilter(*WaveInstance->GetName()), LogAudioTiming, Verbose,
			"FMixerSource::PrepareForInitialization, Name=%ls, StartTime=%.2f bProcedural=%d, bIsSourceBus=%d, CachedRealTimeFirstBuffer=0x%p, MixerSourceBufferValid=%ls",
			*WaveInstance->GetName(), WaveInstance->StartTime, WaveInstance->WaveData->bProcedural, WaveInstance->WaveData->bIsSourceBus,
	...
	bool FMixerSource::IsPreparedToInit()
	{
		...
		if (MixerBuffer && MixerBuffer->IsRealTimeSourceReady())
		{
			...
			if (MixerSourceBuffer->IsAsyncTaskInProgress())
			{
				...
				UE_CLOGF(Audio::MatchesLogFilter(*WaveInstance->GetName()), LogAudioTiming, Verbose,
					"FMixerSource::IsPreparedToInit (not ready), Name=%ls, StartTime=%.2f bProcedural=%d, bIsSourceBus=%d, CachedRealTimeFirstBuffer=0x%p, IsAyncTaskDone=%ls, (IsAsyncTaskInProgress)",
			...
			else if (WaveInstance)
			{
				...
				else
				{
					...
					if ((BufferType == EBufferType::PCMRealTime || BufferType == EBufferType::Streaming) && WaveInstance->WaveData && !WaveInstance->WaveData->bProcedural)
					{
						// If any of these conditions meet, we need to do an initial async decode before we're ready to start playing the sound
						if (WaveInstance->StartTime > 0.0f || WaveInstance->WaveData->bIsSourceBus || !WaveInstance->WaveData->CachedRealtimeFirstBuffer)
						{
							...
							UE_CLOGF(Audio::MatchesLogFilter(*WaveInstance->GetName()), LogAudioTiming, Verbose,
								"FMixerSource::IsPreparedToInit (not ready), Name=%ls, StartTime=%.2f bProcecural=%d, bIsSourceBus=%d, CachedRealTimeFirstBuffer=0x%p, (Kicking Off Initial Async Decode)",
		...
		UE_CLOGF(Audio::MatchesLogFilter(*WaveInstance->GetName()), LogAudioTiming, Verbose,
			"FMixerSource::IsPreparedToInit (not ready), Name=%ls, StartTime=%.2f bProcedural=%d, bIsSourceBus=%d, CachedRealTimeFirstBuffer=0x%p, MixerBuffer=0x%p, IsRealTimeSourceReady=%ls (realtime source not ready, or no mixer buffer)",
			*WaveInstance->GetName(), WaveInstance->StartTime, WaveInstance->WaveData->bProcedural, WaveInstance->WaveData->bIsSourceBus, WaveInstance->WaveData->CachedRealtimeFirstBuffer,
	...
	void FMixerSource::Play()
	{
		...
		if (bIsStopping)
		{
			UE_LOGF(LogAudioMixer, Warning, "Restarting a source which was stopping. Stopping now.");
			return;
		}

		AUDIO_MIXER_TRACE_CPUPROFILER_EVENT_SCOPE(AudioMixerSource::Play);

		UE_CLOGF(WaveInstance && Audio::MatchesLogFilter(*WaveInstance->GetName()), LogAudioTiming, Verbose, 
			"FMixerSource::Play, Name=%ls, StartTime=%.2f bProcedural=%d, bIsSourceBus=%d, CachedRealTimeFirstBuffer=0x%p, MixerSourceBufferValid=%ls, TickCount=%d, InitState=%ls",
			*WaveInstance->GetName(), WaveInstance->StartTime, WaveInstance->WaveData->bProcedural, WaveInstance->WaveData->bIsSourceBus,
		...
		{
			UE_CLOGF(WaveInstance && Audio::MatchesLogFilter(*WaveInstance->GetName()), LogAudioTiming, Verbose,
				"FMixerSourceVoice::Play, Name=%ls, StartTime=%.2f bProcedural=%d, bIsSourceBus=%d, CachedRealTimeFirstBuffer=0x%p, MixerSourceBufferValid=%ls, TickCount=%d, InitState=%ls",
				*WaveInstance->GetName(), WaveInstance->StartTime, WaveInstance->WaveData->bProcedural, WaveInstance->WaveData->bIsSourceBus,
				WaveInstance->WaveData->CachedRealtimeFirstBuffer,ToCStr(LexToString(MixerSourceBuffer.IsValid())), TickCount, ToCStr(LexToString(InitializationState)));
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
		SendSoundWaveStartTraceMessage();
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
	...
	float FMixerSource::GetInheritedSubmixVolumeModulation() const
	{
		...
		if (SpatializationInfo.bReturnsToSubmixGraph)
		{
			if (MixerDevice && MixerDevice->ReverbPluginInterface)
			{
				...
				if (ReturnSubmix)
				{
					...
					while (CurrReturnSubmixPtr && CurrReturnSubmixPtr->IsOwnerValid())
					{
		...
		while (CurrSubmixPtr && CurrSubmixPtr->IsOwnerValid())
		{
			...
			SubmixModVolume *= CurrSubmixPtr->GetOutputVolumeModulationValueThreadSafe();
			SubmixModVolume *= CurrSubmixPtr->GetWetVolumeModulationValueThreadSafe();
	...
	void FMixerSource::UpdateSubmixSendLevels(const FSoundSubmixSendInfoBase& InSendInfo, const EMixerSourceSubmixSendStage InSendStage, TSet<FMixerSubmixWeakPtr>& OutTouchedSubmixes)
	{
		if (InSendInfo.SoundSubmix != nullptr)
		{
			...
			if (InSendInfo.SendLevelControlMethod == ESendLevelControlMethod::Manual)
			{
			...
			MixerSourceVoice->SetSubmixSendInfo(SubmixInstance, SendLevel, InSendStage,
				FSendFilterSettings(InSendInfo.bEnableLPFCutoff != 0, InSendInfo.bEnableHPFCutoff != 0,
					InSendInfo.LPFCutoff, InSendInfo.HPFCutoff));
		}
	}
	...
	void FMixerSource::UpdateEffects()
	{
		...
		// Gated independently by FSoundAttenuationSettings::bEnableSubmixSends upstream in
		// FActiveSound::ParseAttenuation, so we do NOT gate them on the SoundBase's flag here.
		...
		// Sound submix Sends. (these come from SoundBase derived assets) — gated by SoundBase->bEnableSubmixSends.
		// When the gate is closed the loop is skipped; the cleanup loop below will call ClearSubmixSendInfo
		// for any submix that was previously sent to but is no longer in TouchedSubmixes.
		if (WaveInstance->bEnableSubmixSends)
		{
			for (FSoundSubmixSendInfo& SendInfo : WaveInstance->SoundSubmixSends)
			{
	...
	void FMixerSource::UpdateSourceBusSends()
	{
		...
		if (bHasNewBusSends)
		{
			...
			for (TTuple<EBusSendType, FSoundSourceBusSendInfo>& NewSend : NewBusSends)
			{
				if (NewSend.Value.SoundSourceBus)
				{
					...
					// Match SetupBusData: prefer the underlying AudioBus ID when present (explicit bus),
					// otherwise fall back to the SoundSourceBus own ID (automatic source bus).
					const uint32 BusId = NewSend.Value.SoundSourceBus->AudioBus
						? NewSend.Value.SoundSourceBus->AudioBus->GetUniqueID()
						: NewSend.Value.SoundSourceBus->GetUniqueID();
					MixerSourceVoice->SetAudioBusSendInfo(NewSend.Key, BusId, NewSend.Value.SendLevel, BusName,
						FSendFilterSettings(NewSend.Value.bEnableLPFCutoff != 0, NewSend.Value.bEnableHPFCutoff != 0,
							NewSend.Value.LPFCutoff, NewSend.Value.HPFCutoff));
					bSendingAudioToBuses = true;
					// Sync DynamicBusSendInfos so the per-tick dirty-check does not double-dispatch this send.
					for (FDynamicBusSendInfo& DynamicInfo : DynamicBusSendInfos)
					{
						if (DynamicInfo.BusId == BusId)
						{
							DynamicInfo.SendLevel = NewSend.Value.SendLevel;
							DynamicInfo.Filter = FSendFilterSettings(NewSend.Value.bEnableLPFCutoff != 0, NewSend.Value.bEnableHPFCutoff != 0,
								NewSend.Value.LPFCutoff, NewSend.Value.HPFCutoff);
							DynamicInfo.bFilterStateChanged = false;
						}
					}
				}
				...
				if (NewSend.Value.AudioBus)
				{
					...
					const uint32 AudioBusId = NewSend.Value.AudioBus->GetUniqueID();
					MixerSourceVoice->SetAudioBusSendInfo(NewSend.Key, AudioBusId, NewSend.Value.SendLevel, BusName,
						FSendFilterSettings(NewSend.Value.bEnableLPFCutoff != 0, NewSend.Value.bEnableHPFCutoff != 0,
							NewSend.Value.LPFCutoff, NewSend.Value.HPFCutoff));
					bSendingAudioToBuses = true;
					// Sync DynamicBusSendInfos so the per-tick dirty-check does not double-dispatch this send.
					for (FDynamicBusSendInfo& DynamicInfo : DynamicBusSendInfos)
					{
						if (DynamicInfo.BusId == AudioBusId)
						{
							DynamicInfo.SendLevel = NewSend.Value.SendLevel;
							DynamicInfo.Filter = FSendFilterSettings(NewSend.Value.bEnableLPFCutoff != 0, NewSend.Value.bEnableHPFCutoff != 0,
								NewSend.Value.LPFCutoff, NewSend.Value.HPFCutoff);
		...
		for (FDynamicBusSendInfo& DynamicBusSendInfo : DynamicBusSendInfos)
		{
			...
			const bool bFilterStateChanged = DynamicBusSendInfo.bFilterStateChanged;

			if (bSendLevelChanged || bBusEnablementChanged || bFilterStateChanged)
			{
				DynamicBusSendInfo.SendLevel = SendLevel;
				DynamicBusSendInfo.bIsInit = false;
				DynamicBusSendInfo.bFilterStateChanged = false;
	...
	void FMixerSource::UpdateRelativeRenderCost()
	{
		if (MixerSourceVoice)
		{
			...
			if (DebugInfo.IsValid())
			{
				...
				// au.RelativeRenderCostLogThresholdCVar logging
				static constexpr float RelativeRenderCostLogTolerance = 0.1f;
				if (RelativeRenderCostLogThresholdCVar >= 0 &&
					RelativeRenderCost > RelativeRenderCostLogThresholdCVar &&
					RelativeRenderCost - DebugInfo->MaxLoggedRelativeRenderCost > RelativeRenderCostLogTolerance)
				{
					UE_LOGF(LogAudioMixer, Verbose, "New peak Relative Render Cost: %f (exceeds log threshold %f) for wave %ls.",
						RelativeRenderCost,
						RelativeRenderCostLogThresholdCVar,
						*WaveInstance->GetName());
					DebugInfo->MaxLoggedRelativeRenderCost = RelativeRenderCost;
				}
			}
#endif // if ENABLE_AUDIO_DEBUG
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
	void FMixerSource::SendSoundWaveStartTraceMessage() const
	{
		using namespace MixerSourcePrivate;

		const bool bChannelEnabled = UE_TRACE_CHANNELEXPR_IS_ENABLED(AudioChannel);
		if (bChannelEnabled && WaveInstance && WaveInstance->WaveData && WaveInstance->ActiveSound && WaveInstance->SoundClass)
		{
			...
			if (SoundWaveObjectPtr)
			{
				...
				const FString OwnerActorLabel = Audio::Trace::Util::GetOwnerActorLabel(*WaveInstance->ActiveSound);
				const int32 SourceBusID = GetSourceBusID(WaveInstance->WaveData);

				UE_TRACE_LOG(Audio, SoundWaveStart, AudioChannel)
					...
					<< SoundWaveStart.ActiveSoundPlayOrder(WaveInstance->ActiveSound->GetPlayOrder())
					...
					<< SoundWaveStart.ActorIconName(*Audio::Trace::Util::GetOwnerActorIconName(*WaveInstance->ActiveSound).ToString())
					<< SoundWaveStart.SourceBusID(SourceBusID)
					<< SoundWaveStart.AudioComponentName(*Audio::Trace::Util::GetAudioComponentName(*WaveInstance->ActiveSound));
			}
		}
	}

	void FMixerSource::SendSoundWaveStopTraceMessage() const
	{
		const bool bChannelEnabled = UE_TRACE_CHANNELEXPR_IS_ENABLED(AudioChannel);
		if (bChannelEnabled && MixerDevice && WaveInstance && WaveInstance->ActiveSound)
		{
			UE_TRACE_LOG(Audio, SoundWaveStop, AudioChannel)
				<< SoundWaveStop.DeviceId(MixerDevice->DeviceID)
				<< SoundWaveStop.Timestamp(FPlatformTime::Cycles64())
				<< SoundWaveStop.PlayOrder(WaveInstance->GetPlayOrder())
				<< SoundWaveStop.ActiveSoundPlayOrder(WaveInstance->ActiveSound->GetPlayOrder());
		}
	}

	void FMixerSource::UpdateIsAlivePingTraceMessage() const
	{
		using namespace MixerSourcePrivate;

		const bool bChannelEnabled = UE_TRACE_CHANNELEXPR_IS_ENABLED(AudioChannel);

		if (bChannelEnabled && WaveInstance && WaveInstance->IsPlaying() && WaveInstance->WaveData && WaveInstance->ActiveSound && WaveInstance->SoundClass)
		{
			...
			if (FMath::Floor(WaveInstance->ActiveSound->PlaybackTimeUnscaled + AudioDevice->GetDeviceDeltaTime()) - FMath::Floor(WaveInstance->ActiveSound->PlaybackTimeUnscaled) > 0.0f)
			{
				const TObjectPtr<UObject> SoundWaveObjectPtr = FSoftObjectPath(WaveInstance->WaveData->GetPathName()).ResolveObject();
				if (SoundWaveObjectPtr)
				{
					...
					UE_TRACE_LOG(Audio, SoundWaveIsAlivePing, AudioChannel)
						...
						<< SoundWaveIsAlivePing.ActiveSoundPlayOrder(WaveInstance->ActiveSound->GetPlayOrder())
						...
						<< SoundWaveIsAlivePing.ActorIconName(*Audio::Trace::Util::GetOwnerActorIconName(*WaveInstance->ActiveSound).ToString())
						<< SoundWaveIsAlivePing.SourceBusID(SourceBusID)
						<< SoundWaveIsAlivePing.AudioComponentName(*Audio::Trace::Util::GetAudioComponentName(*WaveInstance->ActiveSound));
				}
			}
		}
	}
	...
	bool FMixerSource::ComputeMonoChannelMap(Audio::FAlignedFloatBuffer& OutChannelMap)
	{
		if (IsUsingObjectBasedSpatialization())
		{
			if (WaveInstance->SpatializationMethod != ESoundSpatializationAlgorithm::SPATIALIZATION_HRTF && !bEditorWarnedChangedSpatialization)
			{
				...
				UE_LOGF(LogAudioMixer, Warning, "Changing the spatialization method on a playing sound is not supported (WaveInstance: %ls)", *WaveInstance->WaveData->GetFullName());
			}
