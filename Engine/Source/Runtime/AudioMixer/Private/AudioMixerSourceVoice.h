...
namespace Audio
{
	...
	public:
		...
		// Sets the send level and optional filter for a single submix send
		void SetSubmixSendInfo(FMixerSubmixWeakPtr Submix, const float SendLevel, const EMixerSourceSubmixSendStage SendStage = EMixerSourceSubmixSendStage::PostDistanceAttenuation, const FSendFilterSettings& InFilter = FSendFilterSettings{});
		...
		void SetAudioBusSendInfo(EBusSendType InBusSendType, uint32 AudioBusId, float BusSendLevel, const FString& InBusName, const FSendFilterSettings& InFilter = FSendFilterSettings{});
	...
	private:
		...
		uint16 bHasAttenuationSubmixSends : 1;
