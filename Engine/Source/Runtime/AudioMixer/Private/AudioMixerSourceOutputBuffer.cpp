...
namespace Audio
{
	...
	void FMixerSourceSubmixOutputBuffer::MixOutput(float InSendLevel, EMixerSourceSubmixSendStage InSubmixSendStage, const FSendFilterSettings& InFilter, FInterpolatedLPF& InLpf, FInterpolatedHPF& InHpf, FAlignedFloatBuffer& ScratchBuffer, FAlignedFloatBuffer& OutMixedBuffer)
	{
		const FAlignedFloatBuffer& SrcBuffer = (InSubmixSendStage == EMixerSourceSubmixSendStage::PostDistanceAttenuation) ? PostAttenuationOutputBuffer : PreAttenuationOutputBuffer;

		Audio::ArrayFilterAndMixIn(SrcBuffer, OutMixedBuffer, InSendLevel, NumOutputChannels, InFilter.bEnableLPF, InFilter.LpfCutoff, InFilter.bEnableHPF, InFilter.HpfCutoff, InLpf, InHpf, ScratchBuffer);
	}
