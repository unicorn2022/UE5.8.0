...
FAudioFileReader::FAudioFileReader(const FString& InPath)
{
	...
	if (FileHandle.IsValid())
	{
		...
		else
		{
			...
			UE_LOGF(LogTemp, Error, "Invalid file extension!");
		}
	}
	else
	{
		QualityInfo.NumChannels = 0;
		UE_LOGF(LogTemp, Error, "Invalid file %ls!", *InPath);
	}
}
...
{	
	...
	UE_LOGF(LogTemp, Error, "Unable to determin/create decompressor for '%ls'", *InPath);
	return nullptr;
}
