...
static bool bNeverDisableSubmixes = false;
static FAutoConsoleVariableRef CVarNeverDisableSubmixes(
	TEXT("au.NeverDisableSubmixes"),
	bNeverDisableSubmixes,
	TEXT("Forces submixes to stay enabled and ignore the bAutoDisable value for situations where rendering silence is required instead of no audio rendering\n")
	TEXT("0: Submixes use auto disable as normal\n")
	TEXT("1: Submixes always render silence instead of disabling\n"),
	ECVF_Default
);

namespace MixerSubmixCVars
{
	bool bRemoveModulatorsOnDisabledSubmixes = false;
	FAutoConsoleVariableRef CVarRemoveModulatorsOnDisabledSubmixes(
		TEXT("au.RemoveModulatorsOnDisabledSubmixes"),
		bRemoveModulatorsOnDisabledSubmixes,
		TEXT("When true, modulators are released on a submix while it is auto-disabled and restored from the USoundSubmix asset on re-enable. Note: this discards any ")
		TEXT("runtime modulator overrides applied via UpdateSubmixModulationSettings. Enable only if needed as a workaround for legacy behavior."),
		ECVF_Default);
}
...
namespace Audio
{
	...
	FMixerSubmix::~FMixerSubmix()
	{
		...
		{
			FString InterruptedFileName = "InterruptedRecording.wav";
			UE_LOGF(LogAudioMixer, Warning, "Recording of Submix %ls was interrupted. Saving interrupted recording as %ls.", *(OwningSubmixObject->GetName()), *InterruptedFileName);
	...
	void FMixerSubmix::InitInternal()
	{
		...
		else if (const USoundfieldEndpointSubmix* SoundfieldEndpointSubmix = Cast<const USoundfieldEndpointSubmix>(OwningSubmixObject))
		{
			...
			{
				UE_LOGF(LogAudio, Display, "Wasn't able to set up soundfield format for submix %ls. Sending to default output.", *OwningSubmixObject->GetName());
				return;
			}
	...
	{
		AddOrSetSourceById(InSourceVoice->GetSourceId(), InSendLevel, InSubmixSendStage);
	}

	void FMixerSubmix::AddOrSetSourceById(const int32 InSourceId, const float InSendLevel, EMixerSourceSubmixSendStage InSubmixSendStage)
	{
		AUDIO_MIXER_CHECK_AUDIO_PLAT_THREAD(MixerDevice);
		...
		MixerSourceIds.Add(InSourceId, NewVoiceData);
	}
	...
	FPatchOutputStrongPtr FMixerSubmix::AddPatch(float InGain)
	{
		if (IsSoundfieldSubmix())
		{
			UE_LOGF(LogAudioMixer, Warning, "Patch listening to SoundfieldSubmixes is not supported.");
			return nullptr;
		}
	...
	{
		RemoveSourceById(InSourceVoice->GetSourceId());
	}

	void FMixerSubmix::RemoveSourceById(const int32 InSourceId)
	{
		AUDIO_MIXER_CHECK_AUDIO_PLAT_THREAD(MixerDevice);
		
		// If the source has a corresponding ambisonics encoder, close it out.
		uint32 SourceEncoderID = INDEX_NONE;
		const FSubmixVoiceData* MixerSourceVoiceData = MixerSourceIds.Find(InSourceId);

		// If we did find a valid corresponding FSubmixVoiceData, remove it from the map.
		if (MixerSourceVoiceData)
		{
			int32 NumRemoved = MixerSourceIds.Remove(InSourceId);
	...
	void FMixerSubmix::MixInChildSubmix(FChildSubmixInfo& Child, ISoundfieldAudioPacket& PacketToSumTo)
	{
		...
		if (ChildSubmixSharedPtr.IsValid())
		{
			if (!ChildSubmixSharedPtr->IsSoundfieldSubmix())
			{
				...
				// If this is true, the Soundfield Factory explicitly requested that a separate encoder stream was set up for every
	...
	bool FMixerSubmix::IsOwnerValid() const
	{
	...
	bool FMixerSubmix::IsRenderingAudio() const
	{
		...
		if (MixerSourceIds.Num() == 0 && SilenceTimeStartSeconds >= 0.0)
		{
	...
	void FMixerSubmix::UpdateDebugVolume()
	{
		check(IsInAudioThread());

#if ENABLE_AUDIO_DEBUG
		FAudioDeviceManager* const DeviceManager = FAudioDeviceManager::Get();
		if (DeviceManager)
		{
			const Audio::FAudioDebugger& Debugger = DeviceManager->GetDebugger();

			if (!Debugger.IsSubmixMuteOrSoloActive())
			{
				DebugVolume.store(1.0f, std::memory_order_relaxed);
				return;
			}
			...
			Debugger.QuerySoloMuteSubmix(SubmixName, bIsSoloed, bIsMuted, Reason);

			DebugVolume.store(bIsMuted ? 0.0f : 1.0f, std::memory_order_relaxed);
			return;
		}
	...
	void FMixerSubmix::ProcessAudio(FAlignedFloatBuffer& OutAudioBuffer)
	{
		...
		if (bAutoDisable && !IsRenderingAudio() && !bNeverDisableSubmixes)
		{
			if (!bIsCurrentlyDisabled)
			{
				bIsCurrentlyDisabled = true;

				if (MixerSubmixCVars::bRemoveModulatorsOnDisabledSubmixes)
				{
				...
				if (bChannelEnabled && OwningSubmixObject.IsValid())
				{
					...
					UE_TRACE_LOG(Audio, SubmixEnvelopeValues, AudioMixerChannel)
				...
				UE_CLOGF(LogSubmixEnablementCVar == 1, LogAudioMixer, Verbose,
					"Submix Disabled. Num Sources: %d, Time Silent: %.2f, Disablement Threshold: %.2f, Submix Name: %ls", 
					MixerSourceIds.Num(),
		...
		if (bIsCurrentlyDisabled)
		{
			...
			bIsCurrentlyDisabled = false;
			UE_CLOGF(LogSubmixEnablementCVar == 1, LogAudioMixer, Verbose, "Submix Re-Enabled: %ls", *SubmixName);
		}
...

		{
			...
			CONDITIONAL_SCOPE_CYCLE_COUNTER(STAT_AudioMixerSubmixSource, (MixerSourceIds.Num() > 0));

			// Pre-size the scratch buffer to match InputBuffer so it never needs to grow mid-loop.
			// FilterScratchBuffer is a member, so its backing allocation persists across MixBuffer()
			// calls; SetNumUninitialized only reallocates if the current capacity is insufficient.
			FilterScratchBuffer.SetNumUninitialized(InputBuffer.Num());
...

		{
			...
			{		
				...
				float TotalStartFadeVolume = 0.0f;
				float TotalEndFadeVolume = 0.0f;
				...
				for (int32 EffectChainIndex = EffectChains.Num() - 1; EffectChainIndex >= 0; --EffectChainIndex)
				{
					...
					TotalStartFadeVolume += StartFadeVolume;
					TotalEndFadeVolume += EndFadeVolume;
				...
				{
					// When the active effect-chain fade volumes don't sum to 1.0 (e.g. an override is fading in/out
					// with no base chain to absorb the gap), mix the unprocessed InputBuffer back in at the deficit
					// so the dry signal crossfades against the effect chains instead of cutting out. UE-378714.
					const float StartDryGain = FMath::Clamp(1.0f - TotalStartFadeVolume, 0.0f, 1.0f);
					const float EndDryGain = FMath::Clamp(1.0f - TotalEndFadeVolume, 0.0f, 1.0f);
					if (StartDryGain > 0.0f || EndDryGain > 0.0f)
					{
						ArrayMixIn(InputBuffer, SubmixChainMixBuffer, StartDryGain, EndDryGain);
					}
				...
				if (MixerDevice->IsModulationPluginEnabled() && MixerDevice->ModulationInterface.IsValid())
				{
					...
					CachedWetLevelModValue = TargetWetLevel;
				}
				else
				{
					TargetWetLevel = Audio::ConvertToLinear(WetModBaseDb);
		...
		if (MixerDevice->IsModulationPluginEnabled() && MixerDevice->ModulationInterface.IsValid())
		{
			...
			CachedVolumeModValue = TargetOutputVolume;
		}
		else
		{
			TargetOutputVolume = Audio::ConvertToLinear(VolumeModBaseDb);
		...
		TargetOutputVolume *= GetDebugVolume();
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
		// Trace envelope values scaled by the final output volume so Audio Insights reflects the audible signal (and not raw pre-volume envelope).
		if (bIsEnvelopeFollowing)
		{
			TraceSubmixEnvelopeValues(NumChannels, TargetOutputVolume);
		}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
		...
		if (bAutoDisable && !bNeverDisableSubmixes)
		{
	...
	void FMixerSubmix::ProcessAudio(ISoundfieldAudioPacket& OutputAudio)
	{
		...
		FMixerSourceManager* SourceManager = MixerDevice->GetSourceManager();
		...
		// Mix all source sends into OutputAudio.
		{
			...
			CONDITIONAL_SCOPE_CYCLE_COUNTER(STAT_AudioMixerSubmixSoundfieldSources, (MixerSourceIds.Num() > 0));
			...
			for (const auto& MixerSourceVoiceIter : MixerSourceIds)
			{
				...
				const ISoundfieldAudioPacket* Packet = SourceManager->GetEncodedOutput(SourceId, GetKeyForSubmixEncoding());
				UpdateListenerRotation(SourceManager->GetListenerRotation(SourceId));
	...
	void FMixerSubmix::PauseRecordingOutput()
	{
		...
		{
			UE_LOGF(LogAudioMixer, Warning, "Cannot pause recording output as no recording is in progress.");
			return;
		}
	...
	void FMixerSubmix::ResumeRecordingOutput()
	{
		if (!RecordingData.Num())
		{
			UE_LOGF(LogAudioMixer, Warning, "Cannot resume recording output as no recording is in progress.");
			return;
		}
		bIsRecording = true;
	}
	...
	{
		EnvelopeFollowerRequestsRefCount.Increment();
		...
		if (!bIsEnvelopeFollowing)
		{
			...
			if (bChannelEnabled && OwningSubmixObject.IsValid())
			{
				UE_TRACE_LOG(Audio, SubmixEnvelopeFollowerEnabled, AudioMixerChannel)
	...
	void FMixerSubmix::StopEnvelopeFollowing()
	{
		...
		EnvelopeFollowerRequestsRefCount.Decrement();
		if (EnvelopeFollowerRequestsRefCount.GetValue() <= 0)
		{
			EnvelopeFollowerRequestsRefCount.Reset();
	...
	void FMixerSubmix::GetMagnitudeForFrequencies(const TArray<float>& InFrequencies, TArray<float>& OutMagnitudes)
	{
		...
		{
			UE_LOGF(LogAudioMixer, Warning, "Call StartSpectrumAnalysis before calling GetMagnitudeForFrequencies.");
		}
	}
	...
	void FMixerSubmix::GetPhaseForFrequencies(const TArray<float>& InFrequencies, TArray<float>& OutPhases)
	{
		...
		{
			UE_LOGF(LogAudioMixer, Warning, "Call StartSpectrumAnalysis before calling GetPhaseForFrequencies.");
		}
	}
	...
	{
		VolumeMod.SetModulators(InOutputModulators);
		WetLevelMod.SetModulators(InWetLevelModulators);
		DryLevelMod.SetModulators(InDryLevelModulators);
	}
	...
	float FMixerSubmix::GetOutputVolumeModulationValueThreadSafe() const
	{
		return CachedVolumeModValue;
	}

	float FMixerSubmix::GetWetVolumeModulationValueThreadSafe() const
	{
		return CachedWetLevelModValue;
	}
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
	void FMixerSubmix::TraceSubmixEnvelopeValues(int32 InNumChannels, float InOutputVolume)
	{
		if (!UE_TRACE_CHANNELEXPR_IS_ENABLED(AudioMixerChannel) || !OwningSubmixObject.IsValid() || InNumChannels <= 0)
		{
			return;
		}

		check(InNumChannels <= AUDIO_MIXER_MAX_OUTPUT_CHANNELS);

		float ScaledEnvelopeValues[AUDIO_MIXER_MAX_OUTPUT_CHANNELS];
		FMemory::Memcpy(ScaledEnvelopeValues, EnvelopeValues, sizeof(float) * InNumChannels);

		Audio::ArrayMultiplyByConstantInPlace(MakeArrayView(ScaledEnvelopeValues, InNumChannels), InOutputVolume);
		Audio::ArrayClampInPlace(MakeArrayView(ScaledEnvelopeValues, InNumChannels), 0.0f, 1.0f);

		UE_TRACE_LOG(Audio, SubmixEnvelopeValues, AudioMixerChannel)
			<< SubmixEnvelopeValues.DeviceId(MixerDevice->DeviceID)
			<< SubmixEnvelopeValues.SubmixId(GetTypeHash(OwningSubmixObject->GetPathName()))
			<< SubmixEnvelopeValues.Timestamp(FPlatformTime::Cycles64())
			<< SubmixEnvelopeValues.EnvelopeValues(ScaledEnvelopeValues, InNumChannels);
	}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
}
