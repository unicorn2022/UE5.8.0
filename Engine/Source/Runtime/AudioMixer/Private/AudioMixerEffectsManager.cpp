...
namespace Audio
{
	...
	void FAudioMixerEffectsManager::SetReverbEffectParameters(const FAudioEffectParameters& InEffectParameters)
	{
		...
		if (MasterReverbSubmixPtr.IsValid())
		{
			...
			{
				UE_LOGF(LogAudioMixer, Error, "Failed to update reverb parameters on Default Reverb Submix. Ensure first submix effect is supported type");
	...
	void FAudioMixerEffectsManager::SetEQEffectParameters(const FAudioEffectParameters& InEffectParameters)
	{
		...
		if (MasterEQSubmixPtr.IsValid())
		{
			...
			{
				UE_LOGF(LogAudioMixer, Error, "Failed to update EQ parameters on legacy Default EQ Submix. Ensure first submix effect is supported type");
