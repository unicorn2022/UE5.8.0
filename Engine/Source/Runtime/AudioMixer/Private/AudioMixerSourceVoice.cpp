...
namespace Audio
{
	...
	*/
	...
	void FMixerSourceVoice::Reset(FMixerDevice* InMixerDevice)
	{
		...
		bHasAttenuationSubmixSends = false;
		bStopFadedOut = false;
	...
	bool FMixerSourceVoice::Init(const FMixerSourceVoiceInitParams& InitParams)
	{
		...
		if (SourceManager->GetFreeSourceId(SourceId))
		{
			...
			bHasAttenuationSubmixSends = InitParams.bHasAttenuationSubmixSends;
	...
	void FMixerSourceVoice::SetSubmixSendInfo(FMixerSubmixWeakPtr Submix, const float SendLevel, const EMixerSourceSubmixSendStage SendStage/* = EMixerSourceSubmixSendStage::PostDistanceAttenuation*/, const FSendFilterSettings& InFilter/* = FSendFilterSettings{}*/)
	{
		AUDIO_MIXER_CHECK_GAME_THREAD(MixerDevice);
		...
		if (SubmixPtr.IsValid())
		{
			...
			if (!SubmixSend)
			{
				...
				NewSubmixSend.Filter = InFilter;

				SubmixSends.Add(SubmixPtr->GetId(), NewSubmixSend);
				SourceManager->SetSubmixSendInfo(SourceId, NewSubmixSend);
			}
			else if (!FMath::IsNearlyEqual(SubmixSend->SendLevel, SendLevel, SourceManager->GetFloatCompareTolerance())
				|| SubmixSend->SubmixSendStage != SendStage
				|| SubmixSend->Filter.HasChanged(InFilter))
			{
				SubmixSend->SendLevel = SendLevel;
				SubmixSend->SubmixSendStage = SendStage;
				SubmixSend->Filter = InFilter;
	...
	void FMixerSourceVoice::SetAudioBusSendInfo(EBusSendType InBusSendType, uint32 AudioBusId, float BusSendLevel, const FString& InBusName, const FSendFilterSettings& InFilter)
	{
		AUDIO_MIXER_CHECK_GAME_THREAD(MixerDevice);
		...
		SourceManager->SetBusSendInfo(SourceId, InBusSendType, AudioBusId, BusSendLevel, InBusName, InFilter);
	}
