...
namespace Audio
{
	// Data needed for a procedural audio task (rendering on the audio render thread)
	...
	TSharedPtr<FMixerSourceBuffer, ESPMode::ThreadSafe> FMixerSourceBuffer::Create(FMixerSourceBufferInitArgs& InArgs, TArray<FAudioParameter>&& InDefaultParams)
	{
		...
		check(InArgs.SoundWave && InArgs.Buffer);

		// Fail if the Wave has been flagged to contain an error
		if (InArgs.SoundWave->HasError())
		{
			UE_LOGF(LogAudioMixer, VeryVerbose, "FMixerSourceBuffer::Create failed as '%ls' is flagged as containing errors", *InArgs.SoundWave->GetName());
			return {};
		}
		...
		, bHasError(false)
	{
		...
		// Only allocate one buffer to render into for procedural sources
		const int32 NumBuffers = bProcedural ? 1 : Audio::MAX_BUFFERS_QUEUED;
	...
	void FMixerSourceBuffer::OnBufferEnd()
	{
		...
		const bool bBufferCompleted = (NumBuffersQueued == 0 && bBufferFinished);
	...
	TSharedPtr<FAlignedFloatBuffer, ESPMode::ThreadSafe> FMixerSourceBuffer::GetNextBuffer()
	{
		...
		if (BufferQueue.Dequeue(NewBufferPtr))
		{
			--NumBuffersQueued;
		}
		return NewBufferPtr;
	...
	{
		// When doing procedural rendering, should only ever use buffer 0
		check(!bProcedural || CurrentBuffer == 0);
	...
	void FMixerSourceBuffer::ProcessRealTimeSource()
	{
		...
		if (AsyncRealtimeAudioTask)
		{
			...
			check(AsyncRealtimeAudioTask->GetType() == EAudioTaskType::Decode);
			{
		...
		if (!AsyncRealtimeAudioTask && !bProcedural)
		{
	...
	{
		return bProcedural ? bBufferFinished : GetNumBuffersQueued() == 0;
	}
