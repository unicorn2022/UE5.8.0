...
namespace UE::Mass
{
	...
	namespace ObserverManager
	{
		...
		struct FBufferedNotification
		{
			...
			FMassElementBitSet CompositionChange;
			using FEntitiesContainer = TVariant<FEntityCollection, FMassEntityHandle>;
			FEntitiesContainer AffectedEntities;

			FBufferedNotification(const EMassObservedOperation InType, FMassElementBitSet&& Composition, FEntitiesContainer&& Entities)
				: Type(InType)
				, CompositionChange(Forward<FMassElementBitSet>(Composition))
				, AffectedEntities(MoveTemp(Entities))
			{
			}

			template<typename TEntities>
			FBufferedNotification(const EMassObservedOperation InType, FMassElementBitSet&& Composition, TEntities&& Entities)
				: Type(InType)
			...
			FBufferedNotification(const EMassObservedOperation InType, FMassElementBitSet&& Composition, const FMassArchetypeEntityCollection& Entities)
				: Type(InType)
				, CompositionChange(MoveTemp(Composition))
			...
			template<typename TEntities>
			FBufferedNotification(const EMassObservedOperation InType, TEntities&& Entities)
				: Type(InType)
				, AffectedEntities(TInPlaceType<typename TDecay<TEntities>::Type>(), Forward<TEntities>(Entities))
			{
			}

			FBufferedNotification(const EMassObservedOperation InType, const FMassArchetypeEntityCollection& Entities)
				: Type(InType)
				, AffectedEntities(TInPlaceType<typename TDecay<FEntityCollection>::Type>(), Entities)
			{
			}
			...
			static bool AreCompositionsEqual(const FMassElementBitSet& A, const FMassElementBitSet& B)
			{
		...
		struct FObserverLock
		{
			...
			FTransactionallySafeCriticalSection& GetNotificationLock()
			{
		...
		private:
			...
			{
				UE::TScopeLock ScopeLock(NotificationLock);

				if (CreationNotificationIndex == INDEX_NONE)
				{
					CreationNotificationIndex = BufferedNotifications.Num();
					BufferedNotifications.Emplace(EMassObservedOperation::CreateEntity, UE::Mass::FEntityCollection());
				}
			...
			int32 AddCreatedEntity(FMassEntityHandle CreatedEntity)
			{
				...
				{
					CreationNotificationIndex = BufferedNotifications.Num();
					BufferedNotifications.Emplace(EMassObservedOperation::CreateEntity, CreatedEntity);
				}
				else
				{
			...
			int32 AddCreatedEntities(const TConstArrayView<FMassEntityHandle> InCreatedEntities)
			{
				...
				{
					CreationNotificationIndex = BufferedNotifications.Num();
					BufferedNotifications.Emplace(EMassObservedOperation::CreateEntity, UE::Mass::FEntityCollection(InCreatedEntities));
				}
				else
				{
			...
			int32 AddCreatedEntitiesCollection(T&& InEntityCollection)
			{
				...
				{
					CreationNotificationIndex = BufferedNotifications.Num();
					BufferedNotifications.Emplace(EMassObservedOperation::CreateEntity, UE::Mass::FEntityCollection(Forward<T>(InEntityCollection)));
				}
				else
				{
			...
			template<typename TEntities>
			void AddNotification(const EMassObservedOperation OperationType, TEntities&& Entities, FMassElementBitSet&& OverlapBitSet)
			{
				UE::TScopeLock ScopeLock(NotificationLock);

				if (BufferedNotifications.Num()
					&& BufferedNotifications.Last().Type == OperationType
					&& FBufferedNotification::AreCompositionsEqual(BufferedNotifications.Last().CompositionChange, OverlapBitSet))
				{
					BufferedNotifications.Last().AppendEntities(Forward<TEntities>(Entities));
				}
				else
				{
					BufferedNotifications.Emplace(OperationType, MoveTemp(OverlapBitSet), Forward<TEntities>(Entities));
				}
			}
