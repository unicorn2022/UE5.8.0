...
struct FMassExecutionRequirements
{
	...
	TMassExecutionAccess<FMassFragmentBitSet> IndirectFragments;
	...
	/** Sparse elements - treated like indirect fragments (archetype-agnostic access) */
	TMassExecutionAccess<FMassElementBitSet> SparseElements;
...
struct FMassProcessorDependencySolver
{
	struct FNode
	{
		...
		/**
		 * The highest execution priority across this node and all nodes in its logical dependency chain.
		 * Nodes that are only dependencies due to resource contention (i.e. blocking shared resources)
		 * are excluded from this calculation.
		 * Note: we use int32 rather than UMassProcessor.ExecutionPriority's int16 to avoid overflow
		 * when propagating priorities in UpdateExecutionPriority.
		 */
...
private:
	...
	private:
		...
		TMassExecutionAccess<FResourceAccess> IndirectFragmentsAccess;
		...
		TMassExecutionAccess<FResourceAccess> SparseElementsAccess;
		TConstArrayView<FNode> AllNodesView;

		template<typename TBitSet>
		void HandleElementType(TMassExecutionAccess<FResourceAccess>& ElementAccess
			, const TMassExecutionAccess<TBitSet>& TestedRequirements, FMassProcessorDependencySolver::FNode& InOutNode, const int32 NodeIndex, bool bSkipArchetypeOverlapTest = false);
...
protected:
	 ...
	 */
	const bool bSingleThreadTarget = !MASS_DO_PARALLEL;
