...
namespace UE::Mass
{
	...
	struct FTypeManager : TSharedFromThis<FTypeManager>
	{
		...
		/**
		 * Move-only, ref-counted handle for a static type registration. The static registry tracks
		 * how many handles reference each type — the entry is only removed when the last handle
		 * for that type is destroyed. Already-initialized FTypeManager instances are unaffected.
		 */
		struct FStaticTypeRegistrationHandle
		{
			FStaticTypeRegistrationHandle() = default;
			UE_API ~FStaticTypeRegistrationHandle();

			UE_API FStaticTypeRegistrationHandle(FStaticTypeRegistrationHandle&& InOther);
			UE_API FStaticTypeRegistrationHandle& operator=(FStaticTypeRegistrationHandle&& InOther);

			FStaticTypeRegistrationHandle(const FStaticTypeRegistrationHandle&) = delete;
			FStaticTypeRegistrationHandle& operator=(const FStaticTypeRegistrationHandle&) = delete;

			bool IsValid() const;

		private:
			friend struct FTypeManager;
			TObjectKey<const UStruct> RegisteredType;
		};
		...
		// Types registered here are copied into every FTypeManager instance
		// during RegisterBuiltInTypes(). Use for types whose lifetime spans
		// multiple entity managers (engine subsystems, game instance subsystems).
		//---------------------------------------------------------------------

		/**
		 * Register a type statically with an RAII handle. Destroying the returned handle removes
		 * the type from the static registry (future TypeManager instances won't see it).
		 */
		UE_API static FStaticTypeRegistrationHandle RegisterStaticType(TNotNull<const UStruct*> InType, FSubsystemTypeTraits&& TypeTraits);
		UE_API static FStaticTypeRegistrationHandle RegisterStaticType(TNotNull<const UStruct*> InType, FSharedFragmentTypeTraits&& TypeTraits);
	...
	inline bool FTypeManager::FStaticTypeRegistrationHandle::IsValid() const
	{
		return RegisteredType != TObjectKey<const UStruct>();
	}
	...
	//---------------------------------------------------------------------
	template<CSubsystem T>
	FTypeManager::FStaticTypeRegistrationHandle FTypeManager::RegisterStaticType()
	{
		return RegisterStaticType(T::StaticClass(), FSubsystemTypeTraits::Make<T>());
	}

	template<CSharedFragment T>
	FTypeManager::FStaticTypeRegistrationHandle FTypeManager::RegisterStaticType()
	{
		return RegisterStaticType(T::StaticStruct(), FSharedFragmentTypeTraits::Make<T>());
	}
