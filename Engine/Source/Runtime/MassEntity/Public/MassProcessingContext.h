...
namespace UE::Mass
{
	struct FProcessingContext
	{
		...
		UE_DEPRECATED_FORGAME(5.6, "Direct access to FProcessingContext.DeltaSeconds has been deprecated. Set it via the constructor instead, read via the getter.")
		 ...
		 */
		UE_DEPRECATED_FORGAME(5.6, "Direct access to FProcessingContext.bFlushCommandBuffer has been deprecated. Set it via the constructor instead, read via the GetWillFlushCommands getter.")
	...
	protected:
		alignas(FMassExecutionContext) uint8 ExecutionContextBuffer[sizeof(FMassExecutionContext)];
	...
	// INLINES
	//----------------------------------------------------------------------//
	// @todo remove the deprecation disabling once CommandBuffer and EntityManager are moved to `protected` and un-deprecated (around UE5.8)
