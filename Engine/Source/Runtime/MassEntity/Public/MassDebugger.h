...
namespace UE::Mass::Debug
{
	struct FArchetypeStats
	{
		...
		/**
		 * Memory used by per-chunk sparse element bookkeeping (FChunkSparseElements). Reported as a subset
		 * of AllocatedSize so it can be displayed separately. Will be zero for archetypes whose chunks
		 * carry no sparse elements.
		 */
		SIZE_T SparseElementsBookkeepingSize = 0;
	};
...
namespace UE::Mass::Debug
{
	...
	/**
	 * Fetches the right world to draw the debug drawing in, depending on the state of mass.debug.ServerPIEDebugDrawMode.
	 * Makes sense only in PIE, nop for other cases. Will return nullptr if drawing shouldn't be attempted,
	 * for example when mass.debug.ServerPIEDebugDrawMode = 1 and the call is on client.
	 */
	MASSENTITY_API extern UWorld* GetDebugDrawWorld(const FMassExecutionContext& Context);
	...
	{
		static constexpr EMessageSeverity::Type ConversionMap[static_cast<int32>(EMassDebugMessageSeverity::MAX)] =
		{
		...
		return MassSeverity == EMassDebugMessageSeverity::Default 
			? OriginalSeverity
			: ConversionMap[static_cast<int32>(MassSeverity)];
	}
} // namespace UE::Mass::Debug
...
struct FMassDebugger
{
	...
	static MASSENTITY_API FMassArchetypeCompositionDescriptor GetArchetypeComposition(const FMassArchetypeHandle& ArchetypeHandle);
	static MASSENTITY_API FMassElementBitSet GetArchetypeSparseElementsBitset(const FMassArchetypeHandle& ArchetypeHandle);
	...
	static MASSENTITY_API FString GetArchetypeRequirementCompatibilityDescription(const FMassFragmentRequirements& Requirements, const FMassElementBitSet& ArchetypeCompositionBitSet);
	...
	{
		return GetArchetypeRequirementCompatibilityDescription(Requirements, ArchetypeComposition.GetElementsBitSet());
	}
...
struct FMassDebugger
{
	...
	{
		return TEXT("[no debug information]");
	}
