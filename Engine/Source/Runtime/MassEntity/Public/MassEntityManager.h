...
namespace UE::Mass
{
	...
	template<typename T>
	concept CStructType = TIsDerivedFrom<Clean<T>, FInstancedStruct>::Value
		|| TIsDerivedFrom<Clean<T>, FStructView>::Value
		|| TIsDerivedFrom<Clean<T>, FConstStructView>::Value;
} // namespace UE::Mass
...
// Setting REQUESTED_MASS_CONCURRENT_RESERVE=0 is no longer supported and will be ignored in a future release.
...
struct FMassEntityManager : public TSharedFromThis<FMassEntityManager>, public FGCObject
{
	...
	friend FMassExecutionContext; // Execution context needs direct access to unsafe api.
...
public:
	...
	/**
	 * CreateArchetype from a mass element bitset and initial values
	 *
	 * @param ElementBitSet Bitset of fragment, tag and chunk fragment types
	 * @param CreationParams Parameters used during archetype construction
	 * @return a handle of a new archetype
	 */
	UE_API FMassArchetypeHandle CreateArchetype(const FMassElementBitSet& ElementBitSet, const FMassArchetypeCreationParams& CreationParams = FMassArchetypeCreationParams());
	...
	/** 
	 * Method to iterate on all the element (fragments, tags, etc.) types of an archetype.
	 * Note that sparse elements are not archetype characteristics and therefore not included in this call.
	 */
	static UE_API void ForEachArchetypeElementType(const FMassArchetypeHandle& ArchetypeHandle, TFunctionRef< bool(const UScriptStruct* /*ElementType*/)> Function);
	...
	/** List all entities in an archetype per chunk. */
	static UE_API void ForEachArchetypeChunkEntityList(const FMassArchetypeHandle& ArchetypeHandle, TFunctionRef<void(TConstArrayView<FMassEntityHandle> /*Entities*/)> Function);
	...
	/**
	 * Adds sparse element indicated by ElementType to the entity indicated by EntityHandle.
	 * Works with tags too, but returns an empty struct view
	 * For more information on Sparse Elements see FSparseElementsStorage
	 */
	...
	void RemoveSparseElementFromEntity(FMassEntityHandle EntityHandle)
	{
		RemoveSparseElementFromEntity(EntityHandle, T::StaticStruct());
	}

	/**
	 * Adds sparse elements indicated by ElementsBitset to the entity indicated by EntityHandle.
	 * For more information on Sparse Elements see FSparseElementsStorage
	 */
	...
	bool RemoveConstSharedFragmentFromEntity(const FMassEntityHandle EntityHandle, const UScriptStruct& ConstSharedFragmentType)
	{
		return RemoveConstSharedFragmentFromEntity(EntityHandle, &ConstSharedFragmentType);
	}
	...
	UE_DEPRECATED(5.8, "Use overload taking TNotNull")
	bool RemoveSharedFragmentFromEntity(const FMassEntityHandle EntityHandle, const UScriptStruct& SharedFragmentType)
	{
		return RemoveSharedFragmentFromEntity(EntityHandle, &SharedFragmentType);
	}
	...
	void BatchChangeFragmentCompositionForEntities(TConstArrayView<FMassArchetypeEntityCollection> EntityCollections, const FMassFragmentBitSet& FragmentsToAdd, const FMassFragmentBitSet& FragmentsToRemove)
	{
		...
		BatchChangeCompositionForEntities(EntityCollections, FragmentsToAdd, FragmentsToRemove);
	}
	 ...
	 */
	UE_API void BatchAddFragmentInstancesForEntities(TConstArrayView<FMassArchetypeEntityCollectionWithPayload> EntityCollections, const FMassFragmentBitSet& FragmentsAffected, const FMassTagBitSet& TagsToAdd);

	/**
	 * Adds fragment types with per-entity values, tags, and shared fragment values — all in a single entity move.
	 * Tags are included in composition but excluded from value payload. Shared fragment values are set per-chunk.
	 */
	UE_API void BatchAddFragmentInstancesForEntities(TConstArrayView<FMassArchetypeEntityCollectionWithPayload> EntityCollections, const FMassFragmentBitSet& FragmentsAffected, const FMassTagBitSet& TagsToAdd, const FMassArchetypeSharedFragmentValues& SharedFragmentValuesToAdd);

	/**
	 * Adds fragment types, sets their per-entity values, and adds shared fragment values — all in a single entity move.
	 * Combines the functionality of BatchAddFragmentInstancesForEntities and BatchAddSharedFragmentsForEntities.
	 */
	UE_API void BatchAddFragmentInstancesForEntities(TConstArrayView<FMassArchetypeEntityCollectionWithPayload> EntityCollections, const FMassFragmentBitSet& FragmentsAffected, const FMassArchetypeSharedFragmentValues& SharedFragmentValuesToAdd);
	...
	UE_API void BatchAddSparseElementToEntities(TConstArrayView<FMassArchetypeEntityCollection> EntityCollections, TNotNull<const UScriptStruct*> ElementType);
	// @todo add a version with FMassArchetypeEntityCollectionWithPayload to add pre-configured fragment instances
	UE_API void BatchRemoveSparseElementFromEntities(TConstArrayView<FMassArchetypeEntityCollection> EntityCollections, TNotNull<const UScriptStruct*> ElementType);

	/**
	 * Removes the specified shared fragment types from all entities in the provided collections.
	 * Entities are moved to new archetypes without the removed shared fragment types.
	 * @param EntityCollections the entity collections to process
	 * @param ToRemove bitset of shared fragment types to remove
	 */
	UE_API void BatchRemoveSharedFragmentFromEntities(TConstArrayView<FMassArchetypeEntityCollection> EntityCollections, const FMassSharedFragmentBitSet& ToRemove);

	/**
	 * Removes the specified const shared fragment types from all entities in the provided collections.
	 * Entities are moved to new archetypes without the removed shared fragment types.
	 * @param EntityCollections the entity collections to process
	 * @param ToRemove bitset of const shared fragment types to remove
	 */
	UE_API void BatchRemoveConstSharedFragmentFromEntities(TConstArrayView<FMassArchetypeEntityCollection> EntityCollections, const FMassConstSharedFragmentBitSet& ToRemove);
	 ...
	 *  @param EntityHandle indicates the target entity
	 */
	void SetEntityFragmentValues(FMassEntityHandle EntityHandle, TArrayView<const FInstancedStruct> FragmentInstanceList);

	template<UE::Mass::CStructType TStruct>
	void SetEntityFragmentValues(FMassEntityHandle EntityHandle, TConstArrayView<TStruct> FragmentInstanceList);
	...
	void BatchSetEntityFragmentValues(const FMassArchetypeEntityCollection& SparseEntities, TArrayView<const FInstancedStruct> FragmentInstanceList);

	template<UE::Mass::CStructType TStruct>
	void BatchSetEntityFragmentValues(const FMassArchetypeEntityCollection& SparseEntities, TConstArrayView<TStruct> FragmentInstanceList);

	void BatchSetEntityFragmentValues(TConstArrayView<FMassArchetypeEntityCollection> EntityCollections, TArrayView<const FInstancedStruct> FragmentInstanceList);

	template<UE::Mass::CStructType TStruct>
	void BatchSetEntityFragmentValues(TConstArrayView<FMassArchetypeEntityCollection> EntityCollections, TConstArrayView<TStruct> FragmentInstanceList);
	...
	{
		MASS_STATIC_CHECK_FRAGMENT(FragmentType);
	...
	/** Returns a FStructView for the given fragment on the specified entity. The view points
	 *  directly into archetype chunk storage.
	 *  WARNING: For initializing fragments on newly created entities, prefer the
	 *  CreateEntity(TConstArrayView<FInstancedStruct>,...) overload which sets initial values
	 *  atomically. The two-step pattern (CreateEntity + GetFragmentDataStruct + mutate) is only
	 *  safe when no processors are running (e.g., during BeginPlay or subsystem Initialize).
	 *  @return an empty FStructView if the entity does not have the fragment. */
	...
	UE_API FStructView GetMutableSparseElementDataForEntity(TNotNull<const UScriptStruct*> ElementType, const FMassEntityHandle EntityHandle);
	UE_API FConstStructView GetSparseElementDataForEntity(TNotNull<const UScriptStruct*> ElementType, const FMassEntityHandle EntityHandle) const;
...
private:
	...
	/**
	 * Creates fully built entity ready to be used by the subsystem
	 * Does not check for synchronous execution and should only be used by processors through the execution context
	 * This should only ever be used by FMassExecutionContext which will validate data access safety.
	 * @param ArchetypeHandle the archetype you want this entity to be
	 * @param SharedFragmentValues to be associated with the entity
	 * @return FMassEntityHandle id of the newly created entity
	 */
	UE_API FMassEntityHandle CreateEntityAsyncUnsafe(const FMassArchetypeHandle& ArchetypeHandle, const FMassArchetypeSharedFragmentValues& SharedFragmentValues = {});
	
	/**
	 * Fully buils an entity with a reserved handle ready to be used by the subsystem
	 * Does not check for synchronous execution and should only be used by processors through the execution context
	 * This should only ever be used by FMassExecutionContext which will validate data access safety.
	 * @param ArchetypeHandle the archetype you want this entity to be
	 * @param ReservedEntityHandle the already reserved handle to use for the new enetity
	 * @param SharedFragmentValues to be associated with the entity
	 */
	UE_API void BuildEntityAsyncUnsafe(const FMassArchetypeHandle& ArchetypeHandle, const FMassEntityHandle& ReservedEntityHandle, const FMassArchetypeSharedFragmentValues& SharedFragmentValues = {});

	/**
	 * A version of CreateEntity that's creating a number of entities (Count) in one go
	 * Unsafe version only for use in processors via ExecutionContext
	 * Note: this does not create or return a FEntityCreationContext because it's assumed that observers are already locked by the processing phase.
	 * @param ArchetypeHandle you want this entity to be
	 * @param SharedFragmentValues to be associated with the entities
	 * @param Count number of entities to create
	 * @param InOutEntities the newly created entities are appended to given array, i.e. the pre-existing content of OutEntities won't be affected by the call
	 */
	UE_API void BatchCreateEntitiesAsyncUnsafe(const FMassArchetypeHandle& ArchetypeHandle, const FMassArchetypeSharedFragmentValues& SharedFragmentValues, const int32 Count, TArray<FMassEntityHandle>& InOutEntities);
	void BatchCreateEntitiesAsyncUnsafe(const FMassArchetypeHandle& ArchetypeHandle, const int32 Count, TArray<FMassEntityHandle>& InOutEntities)
	{
		BatchCreateEntitiesAsyncUnsafe(ArchetypeHandle, FMassArchetypeSharedFragmentValues(), Count, InOutEntities);
	}
...
public:
	...
	/**
	 * Creates UE::Mass::FSparseElementIterator instance that can be used to iterate over all instances of a given sparse element fragment
	 * Note: works only for sparse fragments, as tags contain no data.
	 */
	UE::Mass::FSparseElementIterator CreateSparseElementsIterator(TNotNull<const UScriptStruct*> ElementType)
	{
		return UE::Mass::IsA<FMassSparseFragment>(ElementType)
			? SparseElementsStorage.CreateElementIterator(ElementType)
			: UE::Mass::FSparseElementIterator();
	}
	...
	{
		return SparseElementsStorage.CreateElementIterator(T::StaticStruct());
	}
	...
	// Type management
	//-----------------------------------------------------------------------------
	UE_API void OnNewTypeRegistered(UE::Mass::FTypeHandle RegisteredTypeHandle);
...
protected:
	 ...
	 */
	UE_API FMassArchetypeHandle InternalCreateSimilarArchetype(const TSharedPtr<FMassArchetypeData>& SourceArchetype, FMassElementBitSet&& NewCompositionBitSet);
...
private:
	void InternalBatchRemoveSharedFragmentsFromEntities(
		TConstArrayView<FMassArchetypeEntityCollection> EntityCollections,
	 ...
	 */
	FMassElementBitSet InternalAddFragmentListToEntityChecked(FMassEntityHandle EntityHandle, const FMassElementBitSet& FragmentList);
	 ...
	 */
	void InternalAddFragmentListToEntity(FMassEntityHandle EntityHandle, const FMassElementBitSet& FragmentList);
	...
	TSharedPtr<FMassEntityManager::FEntityCreationContext> InternalBatchCreateReservedEntities(const FMassArchetypeHandle& ArchetypeHandle
		, const FMassArchetypeSharedFragmentValues& SharedFragmentValues, TConstArrayView<FMassEntityHandle> ReservedEntities
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	/** Stores sparse elements instances. Not accessible from outside on purpose. */
	UE::Mass::FSparseElementsStorage SparseElementsStorage;
...
inline void FMassEntityManager::SetEntityFragmentValues(FMassEntityHandle EntityHandle, TArrayView<const FInstancedStruct> FragmentInstanceList)
{
	SetEntityFragmentValues<FInstancedStruct>(EntityHandle, FragmentInstanceList);
}

inline void FMassEntityManager::BatchSetEntityFragmentValues(const FMassArchetypeEntityCollection& SparseEntities, TArrayView<const FInstancedStruct> FragmentInstanceList)
{
	BatchSetEntityFragmentValues<FInstancedStruct>(SparseEntities, FragmentInstanceList);
}

inline void FMassEntityManager::BatchSetEntityFragmentValues(TConstArrayView<FMassArchetypeEntityCollection> EntityCollections, TArrayView<const FInstancedStruct> FragmentInstanceList)
{
	BatchSetEntityFragmentValues<FInstancedStruct>(EntityCollections, FragmentInstanceList);
}

extern template UE_API void FMassEntityManager::SetEntityFragmentValues<FInstancedStruct>(FMassEntityHandle EntityHandle, TConstArrayView<FInstancedStruct> FragmentInstanceList);
extern template UE_API void FMassEntityManager::SetEntityFragmentValues<FStructView>(FMassEntityHandle EntityHandle, TConstArrayView<FStructView> FragmentInstanceList);
extern template UE_API void FMassEntityManager::SetEntityFragmentValues<FConstStructView>(FMassEntityHandle EntityHandle, TConstArrayView<FConstStructView> FragmentInstanceList);
extern template UE_API void FMassEntityManager::BatchSetEntityFragmentValues<FInstancedStruct>(const FMassArchetypeEntityCollection& SparseEntities, TConstArrayView<FInstancedStruct> FragmentInstanceList);
extern template UE_API void FMassEntityManager::BatchSetEntityFragmentValues<FStructView>(const FMassArchetypeEntityCollection& SparseEntities, TConstArrayView<FStructView> FragmentInstanceList);
extern template UE_API void FMassEntityManager::BatchSetEntityFragmentValues<FConstStructView>(const FMassArchetypeEntityCollection& SparseEntities, TConstArrayView<FConstStructView> FragmentInstanceList);
extern template UE_API void FMassEntityManager::BatchSetEntityFragmentValues<FInstancedStruct>(TConstArrayView<FMassArchetypeEntityCollection> EntityCollections, TConstArrayView<FInstancedStruct> FragmentInstanceList);
extern template UE_API void FMassEntityManager::BatchSetEntityFragmentValues<FStructView>(TConstArrayView<FMassArchetypeEntityCollection> EntityCollections, TConstArrayView<FStructView> FragmentInstanceList);
extern template UE_API void FMassEntityManager::BatchSetEntityFragmentValues<FConstStructView>(TConstArrayView<FMassArchetypeEntityCollection> EntityCollections, TConstArrayView<FConstStructView> FragmentInstanceList);
