...
public:
	...
	/** Set the current world elapsed time (recording time) for subsequent trace events.
	 *  Should be called once per frame/phase from code that has UWorld access.
	 *  This allows Mass trace events to be aligned with the RewindDebugger's
	 *  ElapsedTime-based scrub time, which pauses when the simulation pauses. */
	MASSENTITY_API static void SetRecordingTime(double InRecordingTime);

	MASSENTITY_API static uint64 OutputBeginPhaseWithID(const TCHAR* PhaseName);
...
#define UE_TRACE_MASS_ENTITIES_MOVED(EntityHandles, NewArchetype) \
	for (const FMassEntityHandle& Entity : EntityHandles) \
	{ \
		FMassTrace::EntityMoved(Entity, NewArchetype); \
	}
...
#define UE_TRACE_MASS_SET_RECORDING_TIME(RecordingTime) \
	FMassTrace::SetRecordingTime(RecordingTime);
