...
protected:
	...
	UE::Mass::FProcessingQueue ProcessingQueue;
...
private:
	...
	TSharedPtr<FMassObserverManager::FObserverLock> ObserversLockPtr;
...
public:
	...
	{
		return ProcessingPhases[static_cast<uint8>(Phase)].OnPhaseStart;
	}
	...
	{
		return ProcessingPhases[static_cast<uint8>(Phase)].OnPhaseEnd;
	}
	...
	FTickFunction& GetProcessingPhaseTickFunction(EMassProcessingPhase Phase)
	{
		check(Phase < EMassProcessingPhase::MAX);
		check(static_cast<int32>(Phase) >= 0);
		return ProcessingPhases[static_cast<int32>(Phase)];
	}

	const FTickFunction& GetProcessingPhaseTickFunction(EMassProcessingPhase Phase) const
	{
		check(Phase < EMassProcessingPhase::MAX);
		check(static_cast<int32>(Phase) >= 0);
		return ProcessingPhases[static_cast<int32>(Phase)];
...
protected:
	FMassProcessingPhase ProcessingPhases[static_cast<uint8>(EMassProcessingPhase::MAX)];
	FPhaseGraphBuildState ProcessingGraphBuildStates[static_cast<uint8>(EMassProcessingPhase::MAX)];
	...
	TMpscQueue<FDynamicProcessorOperation> PendingDynamicProcessors[static_cast<uint8>(EMassProcessingPhase::MAX)];
