...
namespace UE::Mass
{
	...
	struct FRelationData
	{
		...
		/** Returns true if RoleEntity has at least one valid participant in the queried role. No allocation. */
		bool HasParticipants(const FMassEntityManager& EntityManager, FMassEntityHandle RoleEntity, ERelationRole QueriedRole) const;

		/** Callback-based enumeration of participants. Same iteration as GetParticipants but invokes a callback per valid entity instead of building a TArray. */
		void ForEachParticipant(const FMassEntityManager& EntityManager, FMassEntityHandle RoleEntity,
			ERelationRole QueriedRole, TFunctionRef<void(FMassEntityHandle)> Callback) const;
	};
	...
	struct FRelationManager
	{
		...
		/** Callback-based enumeration of subjects for a given object in a relation. Zero allocation. */
		UE_API void ForEachRelationSubject(const FTypeHandle RelationTypeHandle, FMassEntityHandle ObjectEntity,
			TFunctionRef<void(FMassEntityHandle)> Callback) const;

		/** Callback-based enumeration of objects for a given subject in a relation. Zero allocation. */
		UE_API void ForEachRelationObject(const FTypeHandle RelationTypeHandle, FMassEntityHandle SubjectEntity,
			TFunctionRef<void(FMassEntityHandle)> Callback) const;

		/** Returns true if ObjectEntity has at least one subject in this relation type. No allocation. */
		UE_API bool HasRelationSubjects(const FTypeHandle RelationTypeHandle, FMassEntityHandle ObjectEntity) const;

		/** Returns true if SubjectEntity has at least one object in this relation type. No allocation. */
		UE_API bool HasRelationObjects(const FTypeHandle RelationTypeHandle, FMassEntityHandle SubjectEntity) const;
