...
public:
	...
	bool IsEmpty() const
	{
		return FMassFragmentRequirements::IsEmpty() && FMassSubsystemRequirements::IsEmpty();
	}
	...
	{
		ChunkCondition.Reset();
	}

	bool HasChunkFilter() const
	{
		return bool(ChunkCondition);
	}
	...
	{
		bAllowParallelCommands = bInAllowParallelCommands;
	}
	...
	{
		EntityCreationRequirements.AddCreatedArchetype(InComposition);
	}
	...
	{
		return EntityCreationRequirements;
	}
...
private:
	...
	/**
	 * Internal function to hash any value for all archetypes associated with this query.
	 */
	uint64 CalculateArchetypeHash_Internal(TFunctionRef<uint32(const FMassArchetypeData*)> HashValueGetter);
	...
	FMassEntityCreationRequirements EntityCreationRequirements;
