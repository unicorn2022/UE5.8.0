...
public:
	...
	const FMassEntityCreationRequirements& GetProcessorEntityCreationRequirements() const;
	...
	/**
	 * Indicates whether or not this processor has declared in its requirements that it creates entities.
	 */
	UE_API bool HasEntityCreationRequirements();

	/**
	 * Export the cached archetypes (by handle) that this processor can create
	 */
...
private:
	...
	/** Cache presence of entity creation requirements in owned queries. */
	uint8 bQueryHasEntityCreationRequirements : 1 = false;

	/** Tracks if created archetypes handles are resolved*/
	uint8 bCreatedArchetypeHandlesResolved : 1 = false;
...
protected:
	...
	friend UE::Mass::FQueuedProcessor; // needs access to OwnedQueries
...
inline const FMassEntityCreationRequirements& UMassProcessor::GetProcessorEntityCreationRequirements() const
{
