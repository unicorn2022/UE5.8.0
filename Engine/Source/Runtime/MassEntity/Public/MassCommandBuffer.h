...
struct FMassCommandBuffer
{
	...
	{
		TUniquePtr<TCommand> CommandInstance = MakeUnique<TCommand>(Forward<ArgsType>(Args)...);
		return static_cast<TCommand&>(PushUniqueCommand(MoveTemp(CommandInstance)));
	}
	...
	{
		MASS_STATIC_CHECK_FRAGMENT(T);
		PushCommand<FMassCommandRemoveElements<T>>(Entity);
	}

	template<typename T>
	UE_DEPRECATED(5.8, "Use RemoveFragment instead")
	...
	{
		static_assert(UE::Mass::CTag<T>, "Given struct type is not a valid tag type.");
		PushCommand<FMassCommandAddElements<T>>(Entities);
	}

	template<typename T>
	UE_DEPRECATED(5.8, "Use AddTag instead")
	...
	{
		static_assert(UE::Mass::CTag<T>, "Given struct type is not a valid tag type.");
		PushCommand<FMassCommandRemoveElements<T>>(Entities);
	}

	template<typename T>
	UE_DEPRECATED(5.8, "Use RemoveTag instead")
	...
	template<typename TOld, typename TNew>
	UE_DEPRECATED(5.8, "Use SwapTags instead")
	...
	void AddElements(FMassEntityHandle Entity)
	{
		PushCommand<FMassCommandAddElements<TTypes...>>(Entity);
	}

	/** Adds any mix of non-shared element types (fragments, tags, sparse) to entities. Batches via PushCommand. */
	template<typename... TTypes>
	void AddElements(TConstArrayView<FMassEntityHandle> Entities)
	{
		PushCommand<FMassCommandAddElements<TTypes...>>(Entities);
	}
	...
	void AddElements(TConstArrayView<FMassEntityHandle> Entities, TConstArrayView<const UScriptStruct*> ElementTypes)
	{
		PushUniqueCommand<FMassCommandAddElementList>(ElementTypes).Add(Entities);
	}

	/** Removes any mix of element types (fragments, tags, shared, const shared, sparse) from an entity. Batches via PushCommand. */
	template<typename... TTypes>
	void RemoveElements(FMassEntityHandle Entity)
	{
		PushCommand<FMassCommandRemoveElements<TTypes...>>(Entity);
	}

	/** Removes any mix of element types (fragments, tags, shared, const shared, sparse) from entities. Batches via PushCommand. */
	template<typename... TTypes>
	void RemoveElements(TConstArrayView<FMassEntityHandle> Entities)
	{
		PushCommand<FMassCommandRemoveElements<TTypes...>>(Entities);
	}

	/** Removes any mix of element types from an entity. Types provided at runtime. Uses PushUniqueCommand. */
	void RemoveElements(FMassEntityHandle Entity, TConstArrayView<const UScriptStruct*> ElementTypes)
	{
		PushUniqueCommand<FMassCommandRemoveElementList>(ElementTypes).Add(Entity);
	}
	...
	{
		PushUniqueCommand<FMassCommandRemoveElementList>(ElementTypes).Add(Entities);
	}
