...
public:
	 ...
	 */
	int32 GetTypeIndex(TNotNull<const TUStructType*> InStructType) const
	{
	 ...
	 * @return The index of the struct type.
	 */
	static int32 GetTypeIndex(const FStructTracker& InStructTracker, TNotNull<const TUStructType*> InStructType)
	{
		if constexpr (bTestInheritanceAtRuntime)
		{
			// Ensure the struct type derives from the base struct
	...
	UE_DEPRECATED(5.8, "Use overload taking TNotNull")
	int32 GetTypeIndex(const FStructTracker& InStructTracker, const TUStructType& InStructType) const
	{
		return GetTypeIndex(InStructTracker, &InStructType);
	}
