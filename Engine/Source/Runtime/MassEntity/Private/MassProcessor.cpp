...
DECLARE_CYCLE_STAT(TEXT("Mass Processor Execute"), STAT_Mass_ExecuteProcessor, STATGROUP_Mass);
...
void UMassProcessor::CallInitialize(const TNotNull<UObject*> Owner, const TSharedRef<FMassEntityManager>& EntityManager)
{
		...
		&& GetClass()->HasAnyClassFlags(CLASS_Abstract) == false))
	{
		...
		ResolveRequestedArchetypes(EntityManager.Get());
		...
		UE_CLOGF(bRequiresGameThreadExecution != bNeedsGameThread, LogMass, Verbose
			, "%ls is marked bRequiresGameThreadExecution = %ls, while the registered queries' or processor requirements indicate the opposite"
...
void UMassProcessor::ResolveRequestedArchetypes(const FMassEntityManager& EntityManager)
{
	if (bCreatedArchetypeHandlesResolved)
	{
		return;
	}

	ProcessorEntityCreationRequirements.ResolveArchetypes(EntityManager);

	for (FMassEntityQuery* QueryPtr : OwnedQueries)
	{
		CA_ASSUME(QueryPtr);

		FMassEntityCreationRequirements& QueryEntityCreationRequirements = QueryPtr->GetEntityCreationRequirements();

		QueryEntityCreationRequirements.ResolveArchetypes(EntityManager);

		bQueryHasEntityCreationRequirements = bQueryHasEntityCreationRequirements || !QueryEntityCreationRequirements.IsEmpty();
	}
	 
	bCreatedArchetypeHandlesResolved = true;
}

void UMassProcessor::ClearArchetypesCreatedHandles()
{
	bCreatedArchetypeHandlesResolved = false;
	ProcessorEntityCreationRequirements.ClearCreatedArchetypeHandles();
	...
	for (FMassEntityQuery* QueryPtr : OwnedQueries)
	{
		...
		QueryEntityCreationRequirements.ClearCreatedArchetypeHandles();
	}
}
...
{
	UE_CLOGF(OwnedQueries.Num() == 0, LogMass, Warning, "%ls has no registered queries while being asked for matching archetypes", *GetName());
...
{
	// Capture constructor-set hard deps before Super::PostInitProperties calls LoadConfig
	TArray<FName> HardcodedExecuteAfter = ExecutionOrder.ExecuteAfter;
	TArray<FName> HardcodedExecuteBefore = ExecutionOrder.ExecuteBefore;

	Super::PostInitProperties();

	// Re-merge hardcoded deps
	for (const FName& Name : HardcodedExecuteAfter)
	{
		ExecutionOrder.ExecuteAfter.AddUnique(Name);
	}
	for (const FName& Name : HardcodedExecuteBefore)
	{
		ExecutionOrder.ExecuteBefore.AddUnique(Name);
	}
...
{
	SCOPE_CYCLE_COUNTER(STAT_Mass_ExecuteProcessor);
	if (LIKELY(ensureMsgf(IsActive(), TEXT("Trying to CallExecute for an inactive processor %s"), *GetProcessorName())))
	{
		...
		bool bAllRequirementsMet = false;
		{
			// Setup output device to collect errors that can be reported by CacheSubsystemRequirements
			FStringOutputDevice ErrorOutput;
			UE::Mass::ErrorReporting::FScopedOutputDevice ScopedOutput(ErrorOutput);
			...
			bAllRequirementsMet = Context.CacheSubsystemRequirements(ProcessorRequirements);
			if (!bAllRequirementsMet)
			{
				UE_VLOG_UELOG(this, LogMass, Log, TEXT("%s Skipping Execute due to subsystem requirements not being met: %s"), *GetProcessorName(), *ErrorOutput);
			}
		}

		if (bAllRequirementsMet)
		{
			Context.SetActiveArchetypeRequirements(&ProcessorEntityCreationRequirements);
			Execute(EntityManager, Context);
		}

		Context.SetActiveArchetypeRequirements(nullptr);
...
void UMassProcessor::ExportRequirements(FMassExecutionRequirements& OutRequirements) const
{
	...
	ProcessorEntityCreationRequirements.ExportRequirements(OutRequirements);
}

bool UMassProcessor::HasEntityCreationRequirements()
{
	return bQueryHasEntityCreationRequirements || !ProcessorEntityCreationRequirements.IsEmpty();
}

void UMassProcessor::ExportArchetypesCreated(TArray<FMassArchetypeHandle>& OutArchetypes)
{
	OutArchetypes.Append(ProcessorEntityCreationRequirements.GetCreatedArchetypes());
	for (FMassEntityQuery* Query : OwnedQueries)
	{
		CA_ASSUME(Query);
		OutArchetypes.Append(Query->GetEntityCreationRequirements().GetCreatedArchetypes());
	}
}

void UMassProcessor::RegisterQuery(FMassEntityQuery& Query)
{
	const uintptr_t ThisStart = reinterpret_cast<uintptr_t>(this);
	const uintptr_t ThisEnd = ThisStart + GetClass()->GetStructureSize();
	const uintptr_t QueryStart = reinterpret_cast<uintptr_t>(&Query);
...
void UMassCompositeProcessor::BuildFlatProcessingGraph(TConstArrayView<FMassProcessorOrderInfo> SortedProcessors)
{
	...
	UE_LOGF(LogMass, Warning
		, "MassCompositeProcessor::BuildFlatProcessingGraph is not expected to run in a single-threaded Mass setup. The flat graph will not be used at runtime.");
...
#if WITH_MASSENTITY_DEBUG
	FScopedCategoryAndVerbosityOverride LogOverride(TEXT("LogMass"), ELogVerbosity::Log);
	UE_LOGF(LogMass, Log, "%ls flat processing graph:", *GroupName.ToString());
	...
	for (const FDependencyNode& ProcessingNode : FlatProcessingGraph)
	{
		...
		{
			UE_LOGF(LogMass, Log, "[%2d]%*ls%ls%ls%ls", Index, ProcessingNode.SequenceIndex * 2, TEXT(""), *ProcessingNode.Processor->GetProcessorName()
				, DependenciesDesc.Len() > 0 ? TEXT(" depends on ") : TEXT(""), *DependenciesDesc);
		}
		++Index;
	}
...
void UMassCompositeProcessor::UpdateProcessorsCollection(TArrayView<FMassProcessorOrderInfo> InOutOrderedProcessors, EProcessorExecutionFlags InWorldExecutionFlags)
{
	...
	const UMassEntitySettings* Settings = GetDefault<UMassEntitySettings>();
	check(Settings);
	...
	const FMassProcessingPhaseConfig& PhaseConfig = Settings->GetProcessingPhaseConfig(ProcessingPhase);
