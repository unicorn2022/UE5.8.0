...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	SIZE_T FConcurrentEntityStorage::GetAllocatedSize() const
	{
		...
		const uint32 MaxPageCount = (MaxEntities / MaxEntitiesPerPage);
		const SIZE_T PagePointerArraySizeBytes = MaxPageCount * sizeof(FEntityData**);
	...
	bool FConcurrentEntityStorage::DebugAssumptionsSelfTest()
	{
		...
		{
			UE_LOGF(LogMass, Error, "%s assumption about default FEntityData values is no longer true.", __FUNCTION__);
			return false;
		}
