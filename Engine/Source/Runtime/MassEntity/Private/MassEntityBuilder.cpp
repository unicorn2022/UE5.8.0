...
namespace UE::Mass
{
	...
	namespace Tweakable
	{
		bool bForceDeferredEntityBuilderCommit = false;
		namespace
		{
			FAutoConsoleVariableRef AnonymousCVars[] = {
				{TEXT("mass.EntityBuilder.ForceDeferredCommit"), bForceDeferredEntityBuilderCommit
					, TEXT("When set will result in each EntityBuilder commit its work using commands, regardless of EntityManager.IsProcessing value.")}
			};
		}
	}
	...
	namespace Private
	{
		struct FEntityBuilderHelper
		{
			...
			{
				auto SourceContainer = ArchetypeComposition.GetContainer<T>();
			...
			, TConstArrayView<FInstancedStruct> InitialFragmentValues, TConstArrayView<FConstSharedStruct> ConstSharedFragments, TConstArrayView<FSharedStruct> SharedFragments)
		{
			...
			bool bIssuesFound = CheckStructContainer<FMassFragment>(InitialFragmentValues, Composition.GetElementsBitSet(), LogOwner);
			bIssuesFound |= CheckStructContainer<FMassConstSharedFragment>(ConstSharedFragments, Composition.GetElementsBitSet(), LogOwner);
			bIssuesFound |= CheckStructContainer<FMassSharedFragment>(SharedFragments, Composition.GetElementsBitSet(), LogOwner);
...
FEntityBuilder::FEntityBuilder(FMassEntityManager& InEntityManager)
	...
	, bForceDeferred(UE::Mass::Tweakable::bForceDeferredEntityBuilderCommit)
{	
}
...
FEntityBuilder& FEntityBuilder::operator=(const FEntityBuilder& Other)
{
	if (testableEnsureMsgf(Other.IsValid(), TEXT("Copying invalid entity builder instances is not supported")))
	{
		...
		bForceDeferred = Other.bForceDeferred;
	}
...
FMassEntityHandle FEntityBuilder::Commit()
{
	...
	if (bForceDeferred || EntityManager->IsProcessing())
	{
		...
		EntityManager->Defer().PushCommand<FMassDeferredCreateCommand>(
					...
					, RelationsParams = RelationsParams
					, CompositionBitSet = Composition.GetElementsBitSet()](FMassEntityManager& Manager)
					{
						FEntityBuilder::CreateEntityImpl(Manager, ReservedEntityHandle, ArchetypeHandle
							, SharedFragmentValues, FragmentsCopy, RelationsParams, CompositionBitSet);
					});
	}
	else
	{
		// directly create the entity, right now
		CreateEntityImpl(*EntityManager, EntityHandle, CachedArchetypeHandle, CachedSharedFragmentValues
			, Fragments, RelationsParams, Composition.GetElementsBitSet());
	}
	...
	, const FMassElementBitSet& CompositionBitSet)
{
	...
	FMassElementBitSet SparseElements = CompositionBitSet & FMassElementBitSet::GetAllSparseElements();
	if (SparseElements.IsEmpty() == false)
	{
		EntityManager.AddSparseElementsToEntity(EntityHandle, SparseElements);
	}
...
bool FEntityBuilder::AppendDataFromEntity(const FMassEntityHandle SourceEntityHandle, EIncludeSparseElements IncludeSparseElements)
{
	...
	if (State == EState::Empty)
	{
		...
		return CopyDataFromEntity(SourceEntityHandle, IncludeSparseElements);
	}

	InvalidateCachedData();

	// handling sparse elements first in case we need to look for existing sparse fragment instances in Fragments,
	// before we add more stuff to it
	if (IncludeSparseElements == EIncludeSparseElements::Yes)
	{
		const FMassElementBitSet SparseElements = EntityManager->GetSparseElementsBitSetForEntity(SourceEntityHandle);
		const FMassElementBitSet SparseFragments = SparseElements & FMassElementBitSet::GetTypeBitArray(UE::Mass::EElementType::Fragment);
		for (FMassElementBitSet::FIndexIterator It = SparseFragments.GetIndexIterator(); It; ++It)
		{
			const UScriptStruct* SparseElementType = FMassElementBitSet::GetTypeAtIndex(*It);
			CA_ASSUME(SparseElementType);
			const FConstStructView FragmentInstance = EntityManager->GetSparseElementDataForEntity(SparseElementType, SourceEntityHandle);

			if (Composition.GetElementsBitSet().IsBitSet(*It))
			{
				// it's possible there's an instance in Fragments already
				FInstancedStruct* ExistingStruct = Fragments.FindByPredicate([&SparseElementType](const FInstancedStruct& Element)
					{
						return Element.GetScriptStruct() == SparseElementType;
					});
				if (ExistingStruct)
				{
					*ExistingStruct = FragmentInstance;
					continue;
				}
			}
			
			Fragments.Emplace(FragmentInstance);
		}
		Composition.GetElementsBitSet() += SparseElements;
	}
...
bool FEntityBuilder::CopyDataFromEntity(const FMassEntityHandle SourceEntityHandle, EIncludeSparseElements IncludeSparseElements)
{
	if (!testableEnsureMsgf(EntityManager->IsEntityActive(SourceEntityHandle)
		, TEXT("%hs expecting a valid, built entity as input"), __FUNCTION__))
	{
	...
	if (IncludeSparseElements == EIncludeSparseElements::Yes)
	{
		...
		for (FMassElementBitSet::FIndexIterator It = SparseFragments.GetIndexIterator(); It; ++It)
		{
			...
			CA_ASSUME(SparseElementType);
			const FConstStructView FragmentInstance = EntityManager->GetSparseElementDataForEntity(SparseElementType, SourceEntityHandle);
			// we can add elements without checking if instances of the given type has already been added
			// Due to the nature of how sparse fragments are stored it's guaranteed the Fragments container doesn't contain any
			Fragments.Emplace(FragmentInstance);
		}
		Composition.GetElementsBitSet() += SparseElements;
	}
...
bool FEntityBuilder::HandleTypeInstance(const UScriptStruct* Type, TArray<FInstancedStruct>*& OutTargetContainer, bool& bOutAlreadyInComposition)
{
	...
	{
		auto& BitSet = Composition.GetElementsBitSet();
...
{
	if (UE::Mass::IsA<FMassElement>(ElementType))
	{
		Composition.GetElementsBitSet().Add(ElementType);
		State = EState::ReadyToCommit;
...
bool FEntityBuilder::GetDefaultValueOfForceDeferredCommit()
{
	return Tweakable::bForceDeferredEntityBuilderCommit;
}
