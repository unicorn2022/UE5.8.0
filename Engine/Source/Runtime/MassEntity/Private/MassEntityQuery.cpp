...
struct FScopedEntityQueryContext
{
	...
	{
		// callback might pause execution until resources are available, must be made first
		ExecutionContext.PreQueryCallback(&InQuery);
		...
		if (!bSubsystemRequirementsCached)
		{
#if WITH_MASSENTITY_DEBUG
			UE_VLOG_UELOG(ExecutionContext.GetSharedEntityManager().Get().GetOwner(), LogMass, Log
				, TEXT("Mass query '%s' can't be successfully set up due to subsystem requirements not being met: %s")
				, *ExecutionContext.DebugGetExecutionDesc()
				, *ErrorOutput);
#else
			UE_VLOG_UELOG(ExecutionContext.GetSharedEntityManager().Get().GetOwner(), LogMass, Log
				, TEXT("Mass query can't be successfully set up due to subsystem requirements not being met: %s"), *ErrorOutput);
#endif
		}
	}
	...
	~FScopedEntityQueryContext()
	{
		// The query has discovered all the chunks for processing.
		CachedExecutionContext.PostQueryCallback(&Query);
...
void FMassEntityQuery::ForEachEntityChunk(FMassArchetypeEntityCollection&& EntityCollection, FMassExecutionContext& ExecutionContext
	, const FMassExecuteFunction& ExecuteFunction)
{
	ExecutionContext.SetEntityCollection(Forward<FMassArchetypeEntityCollection>(EntityCollection));
	ForEachEntityChunk(ExecutionContext, ExecuteFunction);
	ExecutionContext.ClearEntityCollection();
}
...
void FMassEntityQuery::ForEachEntityChunk(FMassExecutionContext& ExecutionContext, const FMassExecuteFunction& ExecuteFunction)
{
	...
	if (FMassFragmentRequirements::IsEmpty())
	{
		if (ensureMsgf(ExecutionContext.GetEntityCollection().IsEmpty() == false, TEXT("Using empty queries is only supported in combination with Entity Collections that explicitly indicate entities to process")))
		{
			...
			if (ExecutionContext.HasChunkQueueFunc())
			{
				for (const FMassArchetypeEntityCollection::FArchetypeEntityRange& Range : ExecutionContext.GetEntityCollection().GetRanges())
				{
					ExecutionContext.QueueChunkChecked({
						.SourceQuery = this,
						.Archetype = &ArchetypeData,
						.EntityRange = Range,
						.ExecuteFunction = &ExecuteFunction,
						.ChunkCondition = &ChunkCondition,
						.RequirementMapping = nullptr,
	...
	else
	{
			...
			if (ExecutionContext.HasChunkQueueFunc())
			{
				for (const FMassArchetypeEntityCollection::FArchetypeEntityRange& Range : ExecutionContext.GetEntityCollection().GetRanges())
				{
					ExecutionContext.QueueChunkChecked({
						...
						.ExecuteFunction = &ExecuteFunction,
						.ChunkCondition = &ChunkCondition,
						.RequirementMapping = &GetRequirementsMappingForArchetype(ArchetypeHandle),
						.bParallelExecution = false });
				}
			}
			else
		...
		else
		{
			...
			for (const int32 ArchetypeIndex : OrderedArchetypeIndices)
			{
				...
				if (ExecutionContext.HasChunkQueueFunc())
				{
					for (int32 ChunkIndex = 0; ChunkIndex < ArchetypeData.GetChunkCount(); ChunkIndex++)
					{
						ExecutionContext.QueueChunkChecked({
							.SourceQuery = this,
							.Archetype = &ArchetypeData,
							.EntityRange = FMassArchetypeEntityCollection::FArchetypeEntityRange(ChunkIndex),
							.ExecuteFunction = &ExecuteFunction,
							.ChunkCondition = &ChunkCondition,
							.RequirementMapping = &ArchetypeFragmentMapping[ArchetypeIndex] });
					}
				}
				else
				{
...
void FMassEntityQuery::ForEachEntityChunk(FMassExecutionContext& ExecutionContext, UE::Mass::FExecutionLimiter& Limiter, const FMassExecuteFunction& ExecuteFunction)
{
	...
	do
	{
		...
		if (ExecutionContext.HasChunkQueueFunc())
		{
			const int32 MaxChunkIndex = FMath::Min(ArchetypeData.GetChunkCount(), Limiter.MaxChunkIndex);
			Limiter.ChunkIndex = Limiter.ChunkIndex >= MaxChunkIndex ? 0 : FMath::Max(0, Limiter.ChunkIndex);
			for (; Limiter.ChunkIndex < MaxChunkIndex && Limiter.EntityCountRemaining > 0; Limiter.ChunkIndex++)
			{
				const FMassArchetypeEntityCollection::FArchetypeEntityRange Range = FMassArchetypeEntityCollection::FArchetypeEntityRange(Limiter.ChunkIndex);
				Limiter.EntityCountRemaining -= ArchetypeData.CalculateRangeLength(Range);
				ExecutionContext.QueueChunkChecked({
					.SourceQuery = this,
					.Archetype = &ArchetypeData,
					.EntityRange = Range,
	...
	, const FMassExecuteFunction& ExecuteFunction, const EParallelExecutionFlags Flags)
{
	...
	if (Jobs.Num())
	{
		...
		if (ExecutionContext.HasChunkQueueFunc())
		{
			for (FChunkJob& Job : Jobs)
			{
				const FMassQueryRequirementIndicesMapping* RequirementMapping = Job.ArchetypeIndex != INDEX_NONE ? &ArchetypeFragmentMapping[Job.ArchetypeIndex] : nullptr;
				ExecutionContext.QueueChunkChecked({
					.SourceQuery = this,
					.Archetype = &Job.Archetype,
					.EntityRange = Job.EntityRange,
					.ExecuteFunction = &ExecuteFunction,
					.ChunkCondition = &ChunkCondition,
		...
		else if (bAllowParallelCommands)
		{
			struct FTaskContext
			{
...
uint64 FMassEntityQuery::CalculateArchetypeEntityOrderHash()
{
	return CalculateArchetypeHash_Internal([](const FMassArchetypeData* ArchetypeData)
		{
			return ArchetypeData->GetEntityOrderVersion();
		});
}

uint64 FMassEntityQuery::CalculateArchetypeVersionHash()
{
	return CalculateArchetypeHash_Internal([](const FMassArchetypeData* ArchetypeData)
		{
			return ArchetypeData->GetArchetypeVersion();
		});
}
...
uint64 FMassEntityQuery::CalculateArchetypeHash_Internal(TFunctionRef<uint32(const FMassArchetypeData*)> HashValueGetter)
{
	CacheArchetypes();

	// The hash is created as a cumulative hash to make sure that the order of archetypes doesn't matter.
	uint64 Hash = 0;
	for (const FMassArchetypeHandle& ArchetypeHandle : ValidArchetypes)
	{
		if (const FMassArchetypeData* Archetype = FMassArchetypeHelper::ArchetypeDataFromHandle(ArchetypeHandle))
		{
			int32 HashValue = HashValueGetter(Archetype);
			// Accept the possible overflow as a desired side effect.
			Hash += FXxHash64::HashBuffer(&HashValue, sizeof(int32)).Hash;
		}
	}
	// Further reduce the risk of a hash collision between two different sized archetype lists.
	int32 ArchetypeCount = ValidArchetypes.Num();
	Hash += FXxHash64::HashBuffer(&ArchetypeCount, sizeof(int32)).Hash;
	return Hash;
}
...
void FMassEntityQuery::ExportRequirements(FMassExecutionRequirements& OutRequirements) const
{
	...
	EntityCreationRequirements.ExportRequirements(OutRequirements);
}
