...
{
	checkf(!ObservedTypes.IsEmpty(), TEXT("ObservedTypes must be populated before ConfigureQueries is called"));
	const UScriptStruct* PrimaryObservedType = GetSingleObservedTypeChecked();
	check(PrimaryObservedType);
	checkf(UE::Mass::IsA<FMassFragment>(PrimaryObservedType) || UE::Mass::IsA<FMassTag>(PrimaryObservedType)
		, TEXT("Only tags and fragments are valid observed types for RelationObservers. Received %s")
		, *PrimaryObservedType->GetName());

	if (UE::Mass::IsA<FMassTag>(PrimaryObservedType))
	{
		EntityQuery.AddTagRequirement(*PrimaryObservedType, EMassFragmentPresence::All);
	}
	else // UE::Mass::IsA<FMassFragment>(PrimaryObservedType)
	{
		EntityQuery.AddRequirement(PrimaryObservedType, ObservedTypeAccess);
	}
	...
	if (bAutoAddRelationFragmentRequirement || bAutoAddRelationTagRequirement)
	{
		...
		if (bAutoAddRelationTagRequirement)
		{
			...
			if (RelationTag != PrimaryObservedType)
			{
		...
		if (bAutoAddRelationFragmentRequirement)
		{
			...
			if (UE::Mass::IsA<FMassFragment>(RelationFragmentType) && RelationFragmentType != PrimaryObservedType)
			{
	...
	DebugDescription = FString::Printf(TEXT("RelationType: %s ObservedType: %s Operation: %s")
		, *RelationTypeHandle.ToString(), *PrimaryObservedType->GetName()
...
{
	ObservedTypes.Reset();
	ObservedTypes.Add(Traits.RelationFragmentType);
...
void UMassRelationEntityCreation::Execute(FMassEntityManager& EntityManager, FMassExecutionContext& Context)
{
	...
	EntityQuery.ForEachEntityChunk(Context, [&RelationInstances, ObservedType = GetSingleObservedTypeChecked()](FMassExecutionContext& ExecutionContext)
	{
...
void UMassRelationEntityDestruction::Execute(FMassEntityManager& EntityManager, FMassExecutionContext& Context)
{
	...
	EntityQuery.ForEachEntityChunk(Context, [&EntitiesToClearOut, ObservedType = GetSingleObservedTypeChecked()](FMassExecutionContext& ExecutionContext)
	{
		TConstArrayView<FMassRelationFragment> RelationFragments = ExecutionContext.GetFragmentView<FMassRelationFragment>(ObservedType);
		for (FMassExecutionContext::FEntityIterator EntityIt = ExecutionContext.CreateEntityIterator(); EntityIt; ++EntityIt)
		{
...
bool UMassRelationRoleDestruction::ConfigureRelationObserver(UE::Mass::FTypeHandle InRegisteredTypeHandle, const UE::Mass::FRelationTypeTraits& Traits)
{
	...
	const int32 OppositeRoleIndex = static_cast<int32>((RelationRole == UE::Mass::ERelationRole::Object) ? UE::Mass::ERelationRole::Subject : UE::Mass::ERelationRole::Object);
	...
	{
		ObservedTypes.Add(Traits.RoleTraits[RoleAsIndex].Element);
	}
	else
	{
		TNotNull<const UScriptStruct*> RelationTypeTag = RelationTypeHandle.GetScriptStruct();
		ObservedTypes.Add(RelationTypeTag);
	}
	...
	switch (Traits.RoleTraits[RoleAsIndex].DestructionPolicy)
	{
		...
		case UE::Mass::ERemovalPolicy::Splice:
			// use my Object as my Subject's Object, and vice versa
			ExecuteFunction = [InRegisteredTypeHandle](FMassExecutionContext& ExecutionContext)
			{
			...
			bExecutionFunctionAssigned = true;
			break;
		case UE::Mass::ERemovalPolicy::Custom: // fall through on purpose
		default: ;
	}
...
{
	static_assert(static_cast<int32>(UE::Mass::ERelationRole::MAX) == 2, "This implementation is tailored to there being only two roles to any relation");

	// if both roles have the same policy and the same element, we can get away with creating only one observer
