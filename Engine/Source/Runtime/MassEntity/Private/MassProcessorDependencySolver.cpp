...
namespace UE::Mass::Private
{
	...
	void LogCycle(TArray<FMassProcessorDependencySolver::FNode> AllNodes, TConstArrayView<int32> CycleIndices, TArray<uint32>& InOutReportedCycleHashes)
	{
		...
		// Finding the cycle length by finding the other occurrence of cycle-starting node
		...
		if (InOutReportedCycleHashes.Find(CycleHash) == INDEX_NONE)
		{
			...
			UE_LOGF(LogMassDependencies, Error, "Detected processing dependency cycle:");

			for (const int32 CycleNodeIndex : CycleView)
			{
				if (const UMassProcessor* Processor = AllNodes[CycleNodeIndex].Processor)
				{
					UE_LOGF(LogMassDependencies, Warning, "\t%ls, group: %ls, before: %ls, after %ls"
						, *Processor->GetName()
				...
				{
					// group
					UE_LOGF(LogMassDependencies, Warning, "\tGroup %ls", *AllNodes[CycleNodeIndex].Name.ToString());
				}
			}
		}
	}
...
void FMassExecutionRequirements::Append(const FMassExecutionRequirements& Other)
{
	for (int i = 0; i < EMassAccessOperation::MAX; ++i)
	{
		...
		IndirectFragments[i] += Other.IndirectFragments[i];
		...
		SparseElements[i] += Other.SparseElements[i];
	}
...
void FMassExecutionRequirements::CountResourcesUsed()
{
	...
	for (int i = 0; i < EMassAccessOperation::MAX; ++i)
	{
		...
		ResourcesUsedCount += IndirectFragments[i].CountStoredTypes();
		...
		ResourcesUsedCount += SparseElements[i].CountStoredTypes();
	}
}
...
{
	return Fragments.IsEmpty() && IndirectFragments.IsEmpty() && ChunkFragments.IsEmpty()
		&& SharedFragments.IsEmpty() && ConstSharedFragments.IsEmpty() && RequiredSubsystems.IsEmpty()
		&& SparseElements.IsEmpty()
...
{
	return FMassArchetypeCompositionDescriptor(Fragments.Read + Fragments.Write + IndirectFragments.Read + IndirectFragments.Write
	...
	: AllNodesView(InAllNodes)
{
	for (int i = 0; i < EMassAccessOperation::MAX; ++i)
	{
		...
		IndirectFragmentsAccess[i].Access.AddZeroed(FMassFragmentBitSet::GetMaxNum());
		...
		SparseElementsAccess[i].Access.AddZeroed(FMassElementBitSet::GetMaxNum());
	}
}
...
void FMassProcessorDependencySolver::FResourceUsage::HandleElementType(TMassExecutionAccess<FResourceAccess>& ElementAccess
	, const TMassExecutionAccess<TBitSet>& TestedRequirements, FMassProcessorDependencySolver::FNode& InOutNode, const int32 NodeIndex, bool bSkipArchetypeOverlapTest)
{
	...
	bSkipArchetypeOverlapTest = bSkipArchetypeOverlapTest || bSubsystems;
	...
	for (auto It = TestedRequirements.Read.GetIndexIterator(); It; ++It)
	{
		...
		{
			if (UserIndex != NodeIndex && (bSkipArchetypeOverlapTest || DoArchetypeContainersOverlap(AllNodesView[UserIndex].ValidArchetypes, InOutNode.ValidArchetypes)))
			{
...
bool FMassProcessorDependencySolver::FResourceUsage::CanAccessRequirements(
	const FMassExecutionRequirements& Tested, const TArray<FMassArchetypeHandle>& InArchetypes) const
{
	// Direct fragments: allow archetype fallback.
	const bool bDirectOK =
		(CanAccess<FMassFragmentBitSet>(Requirements.Fragments, Tested.Fragments) ||
			!HasArchetypeConflict(FragmentsAccess, InArchetypes));

	// Indirect fragments: archetype-agnostic -> no fallback permitted.
	const bool bIndirectOK =
		CanAccess<FMassFragmentBitSet>(Requirements.IndirectFragments, Tested.IndirectFragments);

	// Cross-type contention: whenever either side is indirect, treat as global (no archetype fallback).
	const bool bIndirectVsDirectOK =
		!(Tested.IndirectFragments.Write.HasAny(Requirements.Fragments.Write)
			|| Tested.IndirectFragments.Write.HasAny(Requirements.Fragments.Read)
			|| Tested.IndirectFragments.Read.HasAny(Requirements.Fragments.Write));

	const bool bDirectVsIndirectOK =
		!(Tested.Fragments.Write.HasAny(Requirements.IndirectFragments.Write)
			|| Tested.Fragments.Write.HasAny(Requirements.IndirectFragments.Read)
			|| Tested.Fragments.Read.HasAny(Requirements.IndirectFragments.Write));

	const bool bChunkOK =
		(CanAccess<FMassChunkFragmentBitSet>(Requirements.ChunkFragments, Tested.ChunkFragments) ||
			!HasArchetypeConflict(ChunkFragmentsAccess, InArchetypes));

	const bool bSharedOK =
		(CanAccess<FMassSharedFragmentBitSet>(Requirements.SharedFragments, Tested.SharedFragments) ||
			!HasArchetypeConflict(SharedFragmentsAccess, InArchetypes));

	const bool bSubsystemsOK =
		CanAccess<FMassExternalSubsystemBitSet>(Requirements.RequiredSubsystems, Tested.RequiredSubsystems);

	const bool bSparseElementsOK =
		CanAccess<FMassElementBitSet>(Requirements.SparseElements, Tested.SparseElements);

	return bDirectOK && bIndirectOK && bIndirectVsDirectOK && bDirectVsIndirectOK
		&& bChunkOK && bSharedOK && bSubsystemsOK && bSparseElementsOK;
}


void FMassProcessorDependencySolver::FResourceUsage::SubmitNode(const int32 NodeIndex, FNode& InOutNode)
{
	// Populate Node data access dependencies:
	// Fragments Direct-Direct:
	HandleElementType<FMassFragmentBitSet>(FragmentsAccess, InOutNode.Requirements.Fragments, InOutNode, NodeIndex);
	// Fragments Indirect-Indirect:
	HandleElementType<FMassFragmentBitSet>(IndirectFragmentsAccess, InOutNode.Requirements.IndirectFragments, InOutNode, NodeIndex, /*bSkipArchetypeOverlapTest*/ true);
	// Fragments Indirect-Direct:
	HandleElementType<FMassFragmentBitSet>(IndirectFragmentsAccess, InOutNode.Requirements.Fragments, InOutNode, NodeIndex, /*bSkipArchetypeOverlapTest*/ true);
	// Fragments Direct-Indirect:
	HandleElementType<FMassFragmentBitSet>(FragmentsAccess, InOutNode.Requirements.IndirectFragments, InOutNode, NodeIndex, /*bSkipArchetypeOverlapTest*/ true);
	// Chunk Fragments:
	...
	// Sparse Elements (archetype-agnostic like indirect fragments):
	HandleElementType<FMassElementBitSet>(SparseElementsAccess, InOutNode.Requirements.SparseElements, InOutNode, NodeIndex, /*bSkipArchetypeOverlapTest*/ true);
...
bool FMassProcessorDependencySolver::PerformSolverStep(FResourceUsage& ResourceUsage, TArray<int32>& InOutIndicesRemaining, TArray<int32>& OutNodeIndices)
{
	...
	if (AcceptedNodeIndex != INDEX_NONE || FallbackAcceptedNodeIndex != INDEX_NONE)
	{
		...
		UE_CLOGF(AcceptedNodeIndex == INDEX_NONE, LogMassDependencies, Verbose, "No dependency-free node can be picked, due to resource requirements. Picking %ls as the next node."
			, *Node.Name.ToString());
...
int32 FMassProcessorDependencySolver::CreateNodes(UMassProcessor& Processor)
{
	...
	if (const int32* NodeIndexPtr = NodeIndexMap.Find(ProcName))
	{
		...
		{
			UE_LOGF(LogMassDependencies, Warning, "%s Processor %ls, name %ls, already registered. This processor class does suport duplicates, but individual instances need to have a unique name."
				, __FUNCTION__, *Processor.GetFullName(), *ProcName.ToString());
		}
		else
		{
			UE_LOGF(LogMassDependencies, Warning, "%s Processor %ls already registered. Duplicates are not supported by this processor class."
	...
	for (int32 NodeIndex = 0; NodeIndex < AllNodes.Num(); ++NodeIndex)
	{
		for (int i = 0; i < AllNodes[NodeIndex].ExecuteBefore.Num(); ++i)
		{
			...
			if (DependentNodeIndexPtr == nullptr)
			{
				...
				UE_LOGF(LogMassDependencies, Log, "Unable to find dependency \"%ls\" declared by %ls. Creating a dummy dependency node."
...
void FMassProcessorDependencySolver::LogNode(const FNode& Node, int Indent)
{
	...
	if (Node.IsGroup())
	{
		UE_LOGF(LogMassDependencies, Log, "%*ls%ls before:%ls after:%ls", Indent, TEXT(""), *Node.Name.ToString()
...
void FMassProcessorDependencySolver::Solve(TArray<FMassProcessorOrderInfo>& OutResult)
{
	...
	while (IndicesRemaining.Num())
	{
		...
		{
			UE_LOGF(LogMassDependencies, Error, "Encountered processing dependency cycle - cutting the chain at an arbitrary location.");
...
void FMassProcessorDependencySolver::ResolveDependencies(TArray<FMassProcessorOrderInfo>& OutResult, TSharedPtr<FMassEntityManager> EntityManager, FMassProcessorDependencySolver::FResult* InOutOptionalResult)
{
	...
	UE_LOGF(LogMassDependencies, Log, "Gathering dependencies data:");
	...
	for (UMassProcessor* Processor : Processors)
	{
		...
		{
			UE_LOGF(LogMassDependencies, Warning, "%ls nullptr found in Processors collection being processed", ANSI_TO_TCHAR(__FUNCTION__));
			continue;
		}
		...
		if (bCreateVirtualArchetypes)
		{
			...
			if (AllNodes[ProcessorNodeIndex].Processor->HasEntityCreationRequirements())
			{
				AllNodes[ProcessorNodeIndex].Processor->ClearArchetypesCreatedHandles();

				// ResolveRequestedArchetypes creates the actual archetypes
				AllNodes[ProcessorNodeIndex].Processor->ResolveRequestedArchetypes(*EntityManager.Get());
			}
		}
	}

	UE_LOGF(LogMassDependencies, Verbose, "Pruning processors...");
	...
	for (FNode& Node : AllNodes)
	{
		if (Node.IsGroup() == false)
		{
			...
			if (bDoQueryBasedPruning || Node.Processor->GetOwnedQueriesNum() || Node.Processor->HasEntityCreationRequirements())
			{
				...
				Node.Processor->ExportArchetypesCreated(Node.ValidArchetypes);
			}

			if (bCreateVirtualArchetypes)
			{
				// reset the archetype handles if they're virtual
				Node.Processor->ClearArchetypesCreatedHandles();
			}

			// prune the archetype-less processors
			if (Node.ValidArchetypes.Num() == 0 && bDoQueryBasedPruning)
			{
				UE_LOGF(LogMassDependencies, Verbose, "\t%ls", *Node.Processor->GetName());
	...
	UE_LOGF(LogMassDependencies, Verbose, "Number of processors pruned: %d", PrunedProcessorsCount);
	...
	UE_LOGF(LogMassDependencies, Verbose, "Dependency order:");
	for (const FMassProcessorOrderInfo& Info : OutResult)
	{
		UE_LOGF(LogMassDependencies, Verbose, "\t%ls", *Info.Name.ToString());
	}
	...
	UE_LOGF(LogMassDependencies, Verbose, "Max sequence length: %d", MaxSequenceLength);
...
{
	// Any archetype version change invalidates the cached result. New archetypes can introduce
	// fragment-access overlap between processors whose ValidArchetypes lists were disjoint at
	// solve time (see FResourceUsage::HandleElementType - Read<->Write dependency edges are
	// gated on DoArchetypeContainersOverlap). A previously edge-free pair becomes a data race
	// if a single new archetype matches both processors' queries, so a pure version check is
	// the correct invalidation signal - this also subsumes pruned-processor revival, since any
	// revival requires a new matching archetype, which always bumps the version.
	return EntityManager && (InResult.ArchetypeDataVersion == EntityManager->GetArchetypeDataVersion());
}
