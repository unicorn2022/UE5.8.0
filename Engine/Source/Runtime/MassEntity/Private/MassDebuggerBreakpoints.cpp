...
namespace UE::Mass::Debug
{
	...
	bool FBreakpoint::CheckFragmentRemoveBreakpoints(TConstArrayView<FMassEntityHandle> Entities, const UScriptStruct* FragmentType)
	{
		if (LIKELY(!bHasBreakpoint))
		{
			return false;
		...
		for (const FMassEntityHandle& Handle : Entities)
		{
			if (CheckFragmentRemoveBreakpoints(Handle, FragmentType))
			{
				return true;
			}
		}
