...
bool FMassCommandBuffer::Flush(FMassEntityManager& EntityManager)
{
	...
	struct FBatchedCommandsSortedIndex
	{
		...
		bool IsValid() const
		{
			return GroupOrder < MAX_int32;
		}
		bool operator<(const FBatchedCommandsSortedIndex& Other) const
		{
	...
	for (int32 i = 0; i < OwnedCommandsCount; ++i)
	{
		...
		CommandsOrder.Add(FBatchedCommandsSortedIndex(i, (Command && Command->HasWork())? CommandTypeOrder[static_cast<int32>(Command->GetOperationType())] : MAX_int32));
	}
	for (int32 i = 0; i < AppendedCommandInstances.Num(); ++i)
	{
		const TUniquePtr<FMassBatchedCommand>& Command = AppendedCommandInstances[i];
		CommandsOrder.Add(FBatchedCommandsSortedIndex(i + OwnedCommandsCount, (Command && Command->HasWork()) ? CommandTypeOrder[static_cast<int32>(Command->GetOperationType())] : MAX_int32));
	}
