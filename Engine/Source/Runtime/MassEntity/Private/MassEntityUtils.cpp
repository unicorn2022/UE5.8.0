	...
	, const FMassArchetypeEntityCollection::EDuplicatesHandling DuplicatesHandling, TArray<FMassArchetypeEntityCollection>& OutEntityCollections)
{
	...
	int32 EntityIndex = 0;
	for (; EntityIndex < Entities.Num(); ++EntityIndex)
	{
	...
	FMassArchetypeHandle Archetype = EntityManager.GetArchetypeForEntityUnsafe(Entities[EntityIndex]);
	if (Entities.Num() - EntityIndex == 1)
	{
		// common, trivial case, so we can handle it directly
		OutEntityCollections.Add(FMassArchetypeEntityCollection(Archetype, Entities[EntityIndex]));
		return;
	}

	TMap<const FMassArchetypeHandle, TArray<FMassEntityHandle>> ArchetypeToEntities;

	TArray<FMassEntityHandle>* PerArchetypeEntities = &ArchetypeToEntities.Add(Archetype);
	PerArchetypeEntities->Add(Entities[EntityIndex++]);
	...
	for (; EntityIndex < Entities.Num(); ++EntityIndex)
	{
		...
		{
			FMassArchetypeHandle LocalArchetype = EntityManager.GetArchetypeForEntityUnsafe(Entity);
			if (Archetype != LocalArchetype)
			{
				Archetype = LocalArchetype;
				PerArchetypeEntities = &ArchetypeToEntities.FindOrAdd(Archetype);
			}
