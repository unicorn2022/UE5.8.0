...
void FMassProcessingPhase::ExecuteTick(float DeltaTime, ELevelTick TickType, ENamedThreads::Type CurrentThread, const FGraphEventRef& MyCompletionGraphEvent)
{
	...
	if (UE::Mass::Tweakables::bUseProcessingQueue && PhaseManager->IsPaused() == false)
	{
		...
		auto OnProcessingDone = [this, DeltaTime]()
			{
...
void FMassProcessingPhaseManager::Initialize(UObject& InOwner, TConstArrayView<FMassProcessingPhaseConfig> InProcessingPhasesConfig, const FString& DependencyGraphFileName)
{
	...
	for (int32 PhaseAsInt = 0; PhaseAsInt < static_cast<int32>(EMassProcessingPhase::MAX); ++PhaseAsInt)
	{		
		const EMassProcessingPhase Phase = static_cast<EMassProcessingPhase>(PhaseAsInt);
	...
	UE_LOGF(LogMass, Log, "FMassProcessingPhaseManager::Initialize complete. bUseProcessingQueue=%ls", UE::Mass::Tweakables::bUseProcessingQueue ? TEXT("true") : TEXT("false"));
}
...
void FMassProcessingPhaseManager::OnPhaseStart(FMassProcessingPhase& Phase)
{
	...
	const int32 PhaseAsInt = static_cast<int32>(Phase.Phase);
		...
		&& ensure(ProcessingPhasesConfig.IsValidIndex(PhaseAsInt)))
	{
			...
			|| FMassProcessorDependencySolver::IsResultUpToDate(GraphBuildState.LastResult, EntityManager) == false)
		{
			...
			if (UE::Mass::Tweakables::bUseProcessingQueue)
			{
				UE::Mass::FProcessingQueue& PhaseQueue = ProcessingPhases[PhaseAsInt].ProcessingQueue;
				TConstArrayView<UMassProcessor*> Processors = PhaseProcessor->GetChildProcessorsView();
				UE_LOGF(LogMass, Log, "FMassProcessingPhaseManager::OnPhaseStart SetProcessors: Phase=%d ProcessorCount=%d", PhaseAsInt, Processors.Num());
				for (int32 i = 0; i < Processors.Num(); ++i)
				{
					UE_LOGF(LogMass, Log, "  [%d] %ls", i, Processors[i] ? *Processors[i]->GetName() : TEXT("null"));
				}
				PhaseQueue.SetProcessors(Processors);
			}
...
void FMassProcessingPhaseManager::UnregisterDynamicProcessor(UMassProcessor& Processor)
{
	if (ensureMsgf(Processor.GetProcessingPhase() != EMassProcessingPhase::MAX
		, TEXT("%hs, Misconfigured processor %s, marked as ProcessingPhase == MAX"), __FUNCTION__, *Processor.GetName()))
	{
		PendingDynamicProcessors[static_cast<int32>(Processor.GetProcessingPhase())].Enqueue(&Processor, EDynamicProcessorOperationType::Remove);
	}
}
...
void FMassProcessingPhaseManager::UnregisterDynamicProcessorInternal(TNotNull<UMassProcessor*> Processor)
{
	...
	if (DynamicProcessors.Find(Processor, Index))
	{
		...
		ProcessingGraphBuildStates[static_cast<int32>(Processor->GetProcessingPhase())].bProcessorsNeedRebuild = true;
