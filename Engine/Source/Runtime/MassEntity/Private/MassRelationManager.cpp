...
namespace UE::Mass
{
	...
	TArray<FMassEntityHandle> FRelationData::GetParticipants(const FMassEntityManager& EntityManager, const FMassEntityHandle RoleEntity, const ERelationRole QueriedRole) const
	{
		...
		{
			ReturnSubjects.Reserve((*InstanceData)[QueriedRole].Num());
		}
		ForEachParticipant(EntityManager, RoleEntity, QueriedRole, [&ReturnSubjects](FMassEntityHandle Handle)
		{
			ReturnSubjects.Add(Handle);
		});
		return ReturnSubjects;
	}

	void FRelationData::ForEachParticipant(const FMassEntityManager& EntityManager, const FMassEntityHandle RoleEntity,
		const ERelationRole QueriedRole, TFunctionRef<void(FMassEntityHandle)> Callback) const
	{
		...
		ensureMsgf(Traits.RoleTraits[QueriedRoleIndex].RequiresExternalMapping == EExternalMappingRequired::Yes,
			TEXT("Fetching relation participants for role %s, while role traits explicitly prevent generic mapping"),
		...
		if (const FRelationInstanceData* InstanceData = RoleMap.Find(RoleEntity))
		{
			for (const FMassRelationRoleInstanceHandle RelationInstanceHandle : (*InstanceData)[QueriedRoleIndex])
			{
				...
				{
					Callback(RoleEntityHandle);
				}
			}
		}
	}

	bool FRelationData::HasParticipants(const FMassEntityManager& EntityManager, const FMassEntityHandle RoleEntity,
		const ERelationRole QueriedRole) const
	{
		const int32 QueriedRoleIndex = static_cast<int32>(QueriedRole);
		if (const FRelationInstanceData* InstanceData = RoleMap.Find(RoleEntity))
		{
			for (const FMassRelationRoleInstanceHandle RelationInstanceHandle : (*InstanceData)[QueriedRoleIndex])
			{
				if (RelationInstanceHandle.GetRoleEntityHandle(EntityManager).IsValid())
				{
					return true;
				}
			}
	...
	TArray<FMassEntityHandle> FRelationManager::CreateRelationInstances(const FTypeHandle RelationTypeHandle, TArrayView<FMassEntityHandle> Subjects, TArrayView<FMassEntityHandle> Objects)
	{
		...
		{
			EntityManager.BatchAddElementToEntities(Objects, ObjectElement);
			EntityManager.BatchAddElementToEntities(Subjects, SubjectElement);
		}
	...
	void FRelationManager::ForEachRelationSubject(const FTypeHandle RelationTypeHandle, const FMassEntityHandle ObjectEntity,
		TFunctionRef<void(FMassEntityHandle)> Callback) const
	{
		const FRelationData& RelationData = GetRelationDataChecked(RelationTypeHandle);
		RelationData.ForEachParticipant(EntityManager, ObjectEntity, ERelationRole::Subject, Callback);
	}

	void FRelationManager::ForEachRelationObject(const FTypeHandle RelationTypeHandle, const FMassEntityHandle SubjectEntity,
		TFunctionRef<void(FMassEntityHandle)> Callback) const
	{
		const FRelationData& RelationData = GetRelationDataChecked(RelationTypeHandle);
		RelationData.ForEachParticipant(EntityManager, SubjectEntity, ERelationRole::Object, Callback);
	}

	bool FRelationManager::HasRelationSubjects(const FTypeHandle RelationTypeHandle, const FMassEntityHandle ObjectEntity) const
	{
		const FRelationData& RelationData = GetRelationDataChecked(RelationTypeHandle);
		return RelationData.HasParticipants(EntityManager, ObjectEntity, ERelationRole::Subject);
	}

	bool FRelationManager::HasRelationObjects(const FTypeHandle RelationTypeHandle, const FMassEntityHandle SubjectEntity) const
	{
		const FRelationData& RelationData = GetRelationDataChecked(RelationTypeHandle);
		return RelationData.HasParticipants(EntityManager, SubjectEntity, ERelationRole::Object);
	}
