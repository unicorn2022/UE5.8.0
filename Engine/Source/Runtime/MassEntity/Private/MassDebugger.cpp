...
{
	enum class EServerPIEDebugDrawMode : uint8
	{
		Default = 0,			// Attempt to draw server in server world, overal debug drawing unaffected
		ReplaceClient = 1,		// Draw server in client world, replace client debug drawing
		AppendToClient = 2,		// Draw server in client world, add to client debug drawing
	};
	...
	int32 ServerPIEDebugDrawMode = static_cast<int32>(EServerPIEDebugDrawMode::Default);
	...
	FAutoConsoleVariableRef CVars[] =
	{
		...
		, {	TEXT("mass.debug.TestSelectedEntityAgainstProcessorQueries"), bTestSelectedEntityAgainstProcessorQueries
			...
			, ECVF_Cheat }
		, {	TEXT("mass.debug.ServerPIEDebugDrawMode"), ServerPIEDebugDrawMode
			, TEXT("If enabled will (where supported) draw server's debug drawing in the PIE's client view\n"
			" 0 - default, debug drawing unaffected\n"
			" 1 - will draw only server information\n"
			" 2 - will draw both client and server\n"
			"Non-networked modes are not affected")
			, ECVF_Cheat }
	};
	...
	FString DebugGetFragmentAccessString(EMassFragmentAccess Access)
	{
		...
		default:
			ensureMsgf(false, TEXT("Missing string conversion for EMassFragmentAccess=%d"), EnumToUnderlyingType(Access));
			break;
		}
		...
		FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& Args)
			{
				if (Args.Num() != 2)
				{
					UE_LOGF(LogConsoleResponse, Display, "Error: Expecting 2 parameters");
					return;
				}
				...
				{
					UE_LOGF(LogConsoleResponse, Display, "Error: first parameter must be an integer");
					return;
				}
			
				if (!LexTryParseString<int32>(LastID, *Args[1]))
				{
					UE_LOGF(LogConsoleResponse, Display, "Error: second parameter must be an integer");
					return;
				}
	...
	UWorld* GetDebugDrawWorld(const FMassExecutionContext& Context)
	{
		UWorld* World = Context.GetWorld();
#if WITH_EDITOR
		if (GIsEditor && World)
		{
			//                    | Default  | ReplaceClient | AppendToClient |
			// NM_Standalone      | Allow    | Allow            | Allow           |
			// NM_DedicatedServer | Suppress | Redirect         | Redirect        |
			// NM_ListenServer    | Allow    | Redirect         | Redirect        |
			// NM_Client          | Allow    | Suppress         | Allow           |

			enum class EDrawStatus
			{
				Allow,
				Suppress,
				Redirect,
			...
			EServerPIEDebugDrawMode DrawMode = static_cast<EServerPIEDebugDrawMode>(FMath::Clamp(ServerPIEDebugDrawMode, 0, 2));

			EDrawStatus DrawStatus;
			switch (NetMode)
			{
				default: // falling though on purpose
				case NM_Standalone:
					DrawStatus = EDrawStatus::Allow;
					break;

				case NM_DedicatedServer:
					DrawStatus = (DrawMode == EServerPIEDebugDrawMode::Default)
						? EDrawStatus::Suppress
						: EDrawStatus::Redirect;
					break;
				...
				case NM_Client:
					DrawStatus = (DrawMode == EServerPIEDebugDrawMode::ReplaceClient)
						? EDrawStatus::Suppress
						: EDrawStatus::Allow;
					break;
			}
			...
			switch (DrawStatus)
			{
				case EDrawStatus::Allow:
					break;

				case EDrawStatus::Suppress:
					World = nullptr;
					break;

				case EDrawStatus::Redirect:
					for (const FWorldContext& WorldContext : GEngine->GetWorldContexts())
					{
						UWorld* LocalWorld = WorldContext.World();
						if (LocalWorld && LocalWorld->GetNetMode() == NM_Client)
						{
							World = LocalWorld;
							break;
						}
		...
		FConsoleCommandWithWorldAndArgsDelegate::CreateLambda([](const TArray<FString>& Args, UWorld* World)
	{
		if (Args.Num() != 1)
		{
			UE_LOGF(LogConsoleResponse, Display, "Error: Expecting 1 parameter");
			return;
		}
		...
		{
			UE_LOGF(LogConsoleResponse, Display, "Error: parameter must be an integer");
			return;
		}

		if (!World)
		{
			UE_LOGF(LogConsoleResponse, Display, "Error: invalid world");
			return;
		}
		...
		{
			UE_LOGF(LogConsoleResponse, Display, "Error: cannot find entity for this index");
			return;
		}
	...
	static FAutoConsoleCommandWithWorldAndArgs SetDebugEntity(
		...
		FConsoleCommandWithWorldAndArgsDelegate::CreateLambda([](const TArray<FString>& Args, UWorld* World)
		{
			...
			{
				UE_LOGF(LogConsoleResponse, Display, "Cannot find entity for this index, clearing current selection");
				return;
			}
	...
	static FAutoConsoleCommandWithWorldAndArgs SetFragmentBreakpoint(
		...
		FConsoleCommandWithWorldAndArgsDelegate::CreateLambda([](const TArray<FString>& Args, UWorld* World)
		{
			...
			if (Args.Num() == 0)
			{
				UE_LOGF(LogConsoleResponse, Display, "No fragment types indicated");
			}
			else
			{
				...
				if (SelectedEntity.IsValid())
				{
					for (const FString& PartialFragmentName : Args)
					{
						...
						{
							UE_LOGF(LogConsoleResponse, Display, "Warning: Unable to find element type %ls", *PartialFragmentName);
						}
					}
				}
				else
				{
					UE_LOGF(LogConsoleResponse, Display, "Warning: No entity selected, no break points set");
				}
			}
		}
	));
...
FMassArchetypeCompositionDescriptor FMassDebugger::GetArchetypeComposition(const FMassArchetypeHandle& ArchetypeHandle)
{
	const FMassArchetypeData& ArchetypeData = FMassArchetypeHelper::ArchetypeDataFromHandleChecked(ArchetypeHandle);
	return FMassArchetypeCompositionDescriptor(ArchetypeData.GetCompositionBitSet());
}

FMassElementBitSet FMassDebugger::GetArchetypeSparseElementsBitset(const FMassArchetypeHandle& ArchetypeHandle)
{
	const FMassArchetypeData& ArchetypeData = FMassArchetypeHelper::ArchetypeDataFromHandleChecked(ArchetypeHandle);
	return ArchetypeData.GetSparseElementsBitSet();
}
...
void FMassDebugger::GetArchetypeEntityStats(const FMassArchetypeHandle& ArchetypeHandle, UE::Mass::Debug::FArchetypeStats& OutStats)
{
	...
	OutStats.SparseElementsBookkeepingSize = ArchetypeData.GetSparseElementsBookkeepingAllocatedSize();
...
FString FMassDebugger::GetArchetypeRequirementCompatibilityDescription(const FMassFragmentRequirements& Requirements, const FMassArchetypeHandle& ArchetypeHandle)
{
	...
	const FMassArchetypeData& ArchetypeData = FMassArchetypeHelper::ArchetypeDataFromHandleChecked(ArchetypeHandle);
	return FMassDebugger::GetArchetypeRequirementCompatibilityDescription(Requirements, ArchetypeData.GetCompositionBitSet());
}
	
FString FMassDebugger::GetArchetypeRequirementCompatibilityDescription(const FMassFragmentRequirements& Requirements, const FMassElementBitSet& ArchetypeCompositionBitSet)
{
	FStringOutputDevice OutDescription;

	FMassFragmentBitSet FragmentBitSet = ArchetypeCompositionBitSet.Get<FMassFragmentBitSet>();
	FMassTagBitSet TagBitSet = ArchetypeCompositionBitSet.Get<FMassTagBitSet>();
	FMassChunkFragmentBitSet ChunkFragmentBitSet = ArchetypeCompositionBitSet.Get<FMassChunkFragmentBitSet>();
	FMassSharedFragmentBitSet SharedFragmentBitSet = ArchetypeCompositionBitSet.Get<FMassSharedFragmentBitSet>();
	FMassConstSharedFragmentBitSet ConstSharedFragmentBitSet = ArchetypeCompositionBitSet.Get<FMassConstSharedFragmentBitSet>();

	if (Requirements.HasNegativeRequirements())
	{
		if (FragmentBitSet.HasNone(Requirements.RequiredNoneFragments) == false)
		{
			// has some of the fragments required absent
			OutDescription += TEXT("\nHas fragments required absent: ");
			(Requirements.RequiredNoneFragments & FragmentBitSet).DebugGetStringDesc(OutDescription);
		}

		if (TagBitSet.HasNone(Requirements.RequiredNoneTags) == false)
		{
			// has some of the tags required absent
			OutDescription += TEXT("\nHas tags required absent: ");
			(Requirements.RequiredNoneTags & TagBitSet).DebugGetStringDesc(OutDescription);
		}

		if (ChunkFragmentBitSet.HasNone(Requirements.RequiredNoneChunkFragments) == false)
		{
			// has some of the chunk fragments required absent
			OutDescription += TEXT("\nHas chunk fragments required absent: ");
			(Requirements.RequiredNoneChunkFragments & ChunkFragmentBitSet).DebugGetStringDesc(OutDescription);
		}

		if (SharedFragmentBitSet.HasNone(Requirements.RequiredNoneSharedFragments) == false)
		{
			// has some of the chunk fragments required absent
			OutDescription += TEXT("\nHas shared fragments required absent: ");
			(Requirements.RequiredNoneSharedFragments & SharedFragmentBitSet).DebugGetStringDesc(OutDescription);
		}

		if (ConstSharedFragmentBitSet.HasNone(Requirements.RequiredNoneConstSharedFragments) == false)
		{
			// has some of the chunk fragments required absent
			OutDescription += TEXT("\nHas const shared fragments required absent: ");
			(Requirements.RequiredNoneConstSharedFragments & ConstSharedFragmentBitSet).DebugGetStringDesc(OutDescription);
		}
	}
	...
	{
		if (FragmentBitSet.HasAll(Requirements.RequiredAllFragments) == false)
		{
			// missing one of the strictly required fragments
			OutDescription += TEXT("\nMissing required fragments: ");
			(Requirements.RequiredAllFragments - FragmentBitSet).DebugGetStringDesc(OutDescription);
		}

		if (Requirements.RequiredAnyFragments.IsEmpty() == false && FragmentBitSet.HasAny(Requirements.RequiredAnyFragments) == false)
		{
		...
		if (TagBitSet.HasAll(Requirements.RequiredAllTags) == false)
		{
			// missing one of the strictly required tags
			OutDescription += TEXT("\nMissing required tags: ");
			(Requirements.RequiredAllTags - TagBitSet).DebugGetStringDesc(OutDescription);
		}

		if (Requirements.RequiredAnyTags.IsEmpty() == false && TagBitSet.HasAny(Requirements.RequiredAnyTags) == false)
		{
		...
		if (ChunkFragmentBitSet.HasAll(Requirements.RequiredAllChunkFragments) == false)
		{
			// missing one of the strictly required chunk fragments
			OutDescription += TEXT("\nMissing required chunk fragments: ");
			(Requirements.RequiredAllChunkFragments - ChunkFragmentBitSet).DebugGetStringDesc(OutDescription);
		}

		if (SharedFragmentBitSet.HasAll(Requirements.RequiredAllSharedFragments) == false)
		{
			// missing one of the strictly required Shared fragments
			OutDescription += TEXT("\nMissing required Shared fragments: ");
			(Requirements.RequiredAllSharedFragments - SharedFragmentBitSet).DebugGetStringDesc(OutDescription);
		}

		if (ConstSharedFragmentBitSet.HasAll(Requirements.RequiredAllConstSharedFragments) == false)
		{
			// missing one of the strictly required Shared fragments
			OutDescription += TEXT("\nMissing required Const Shared fragments: ");
			(Requirements.RequiredAllConstSharedFragments - ConstSharedFragmentBitSet).DebugGetStringDesc(OutDescription);
		}
	}
	// else we check if there are any optionals and if so test them
	else if (Requirements.HasOptionalRequirements() && (Requirements.DoesMatchAnyOptionals(ArchetypeCompositionBitSet) == false))
	{
...
const UScriptStruct* FMassDebugger::GetFragmentTypeFromName(FName FragmentName)
{
	...
	for (FEnvironment& Environment : ActiveEnvironments)
	{
		...
		for (FMassArchetypeHandle& ArchetypeHandle : ArchetypeHandles)
		{
			...
			FMassFragmentBitSet::FIndexIterator It = Composition.DebugGetFragments().GetIndexIterator();
			while (It)
			{
				FName StructName = Composition.DebugGetFragments().DebugGetStructTypeName(*It);
				const UScriptStruct* StructType = Composition.DebugGetFragments().GetTypeAtIndex(*It);
				
				FragmentsByName.Add(StructName, StructType);
				++It;
			}

			FMassChunkFragmentBitSet::FIndexIterator ChunkIt = Composition.DebugGetChunkFragments().GetIndexIterator();
			while (ChunkIt)
			{
				FName StructName = Composition.DebugGetChunkFragments().DebugGetStructTypeName(*ChunkIt);
				const UScriptStruct* StructType = Composition.DebugGetChunkFragments().GetTypeAtIndex(*ChunkIt);

				FragmentsByName.Add(StructName, StructType);
				++ChunkIt;
			}

			FMassSharedFragmentBitSet::FIndexIterator SharedFragIt = Composition.DebugGetSharedFragments().GetIndexIterator();
			while (SharedFragIt)
			{
				FName StructName = Composition.DebugGetSharedFragments().DebugGetStructTypeName(*SharedFragIt);
				const UScriptStruct* StructType = Composition.DebugGetSharedFragments().GetTypeAtIndex(*SharedFragIt);

				FragmentsByName.Add(StructName, StructType);
				++SharedFragIt;
			}

			FMassConstSharedFragmentBitSet::FIndexIterator ConstSharedFragIt = Composition.DebugGetConstSharedFragments().GetIndexIterator();
			while (ConstSharedFragIt)
			{
				FName StructName = Composition.DebugGetConstSharedFragments().DebugGetStructTypeName(*ConstSharedFragIt);
				const UScriptStruct* StructType = Composition.DebugGetConstSharedFragments().GetTypeAtIndex(*ConstSharedFragIt);
...
bool FMassDebugger::GetFragmentData(const FMassEntityManager& EntityManager, const UScriptStruct* FragmentType, FMassEntityHandle Entity, TSharedPtr<FStructOnScope>& OutStructData)
{
	...
	{
		const void* FragmentData = Archetype->GetFragmentDataForEntity(FragmentType, Entity);
		if (FragmentData)
		{
