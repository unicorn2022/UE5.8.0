...
{
	// Attempt to load the CUDA library, if library loading was successful:
	// When the plugin loading happens in the engine initialization, wire up our post-init delegate.
	// If the plugin loading is on demand, calls InitCuda directly.
	if (LoadCuda() == true)
	{
		if (!GEngine || !GEngine->IsInitialized())
		{
			PostEngineInitHandle = FCoreDelegates::GetOnPostEngineInit().AddRaw(this, &FCUDAModule::InitCuda);

			UE_LOGF(LogCUDA, Display, "CUDA module ready pending PostEngineInit.");
		}
		else
		{
			FCUDAModule::InitCuda();
			UE_LOGF(LogCUDA, Display, "CUDA module loading directly as PostEngineInit has passed.");
		}
	}
}
...
#	pragma warning(disable:6398) // The address-of a field cannot be null in well-defined code
	...
	{
		UE_LOGF(LogCUDA, Warning, "Failed to check IsRHISelectedDevice at %ls:%u", ANSI_TO_TCHAR(__FILE__), __LINE__);
		return false;
	}
#ifdef _MSC_VER
#	pragma warning(pop)
	...
	{
		UE_LOGF(LogCUDA, Warning, "Could not get CUDA device at device %d.", DeviceIndex);
		return false;
	}
...
bool FCUDAModule::IsRHISelectedDevice(CUdevice cuDevice)
{
	...
	if (RHIType == ERHIInterfaceType::Vulkan)
	{
		...
		{
			UE_LOGF(LogCUDA, Warning, "Could not get CUDA device UUID at device %d.", cuDevice);
			return false;
		}
		...
		if (RHIType == ERHIInterfaceType::D3D11)
		{
			...
			{
				UE_LOGF(LogCUDA, Warning, "Could not get CUDA device LUID at device %d.", cuDevice);
				return false;
			}

			ID3D11Device* NativeD3D11Device = GetID3D11DynamicRHI()->RHIGetDevice();
			...
			if (FAILED(Result))
			{
				UE_LOGF(LogCUDA, Error, "Failed to get DXGIDevice when trying to resolve to CUDA device at %ls:%u \n with error %u", ANSI_TO_TCHAR(__FILE__), __LINE__, Result);
				return false;
			}
		...
		else if (RHIType == ERHIInterfaceType::D3D12)
		{
			...
			ID3D12Device* NativeD3D12Device = GetID3D12DynamicRHI()->RHIGetDevice_NoMGPU();
		...
		{
			UE_LOGF(LogCUDA, Warning, "CUDA is unsupported on OpenGL.");
			return false;
		}
		else
		...
		{
			UE_LOGF(LogCUDA, Warning, "Unsupported RHI or Platform for CUDA.");
		}
...
void FCUDAModule::InitCuda()
{
	...
	{
		UE_LOGF(LogCUDA, Display, "RHI device not NVIDIA unloading CUDA support");
		UnloadCuda();
		return;
	}
	...
	// Initialise CUDA API
	{
		UE_LOGF(LogCUDA, Display, "Initialising CUDA API...");

		CUresult err = DriverApiPtrs.cuInit(0);
		if (err == CUDA_SUCCESS)
		{
			UE_LOGF(LogCUDA, Display, "CUDA API initialised successfully.");
		}
		else
		{
			UE_LOGF(LogCUDA, Error, "CUDA API failed to initialise.");
			UnloadCuda();
			return;
		}
	}
	...
	{
		UE_LOGF(LogCUDA, Display, "Found %d CUDA capable devices.", device_count);
	}
	else
	{
		UE_LOGF(LogCUDA, Error, "Could not count how many graphics devices there are using CUDA.");
	}

	if (device_count == 0)
	{
		UE_LOGF(LogCUDA, Error, "There are no available device(s) that support CUDA. If that is untrue check CUDA is installed.");
		return;
	}
	...
	for (int current_device = 0; current_device < device_count; ++current_device)
	{
		...
		if (getDeviceErr != CUDA_SUCCESS)
		{
			UE_LOGF(LogCUDA, Warning, "Could not get CUDA device at device %d.", current_device);
			continue;
		}
		...
		if (getNameErr == CUDA_SUCCESS)
		{
			...
			UE_LOGF(LogCUDA, Display, "Found device %d called \"%ls\".", current_device, *deviceName);
		}
		else
		{
			UE_LOGF(LogCUDA, Warning, "Could not get name of CUDA device %d.", current_device);
		}

		// Compare the CUDA device with current RHI selected device.
		if (this->IsRHISelectedDevice(cuDevice))
		{
			UE_LOGF(LogCUDA, Display, "Attempting to create CUDA context on GPU Device %d...", current_device);

			CUresult createCtxErr = DriverApiPtrs.cuCtxCreate(&cudaContext, 0, current_device);
			if (createCtxErr == CUDA_SUCCESS)
			{
				UE_LOGF(LogCUDA, Display, "Created CUDA context on device \"%ls\".", *deviceName);
			...
			{
				UE_LOGF(LogCUDA, Error, "CUDA module could not create CUDA context on RHI device \"%ls\".", *deviceName);
				return;
			}
		}
		else
		{
			UE_LOGF(LogCUDA, Verbose, "CUDA device %d \"%ls\" does not match RHI device.", current_device, *deviceName);
		}
	}

	if (rhiDeviceIndex == -1)
	{
		UE_LOGF(LogCUDA, Error, "CUDA module failed to create a CUDA context on the RHI selected device.");
		return;
	}
	else
	{
	...
	UE_LOGF(LogCUDA, Error, "CUDA attemped to be initialized on unsupported Platform (Currently supported platforms are Win32, Win64, Linux and LinuxAArch64).");
...
CUcontext FCUDAModule::GetCudaContext()
{
	...
	{
		UE_LOGF(LogCUDA, Error, "You have requested a CUDA context when the CUDA Driver API is not loaded.");
		return nullptr;
	}

	if (rhiDeviceIndex == -1)
	{
		UE_LOGF(LogCUDA, Error, "You have requested a CUDA context when the RHI selected device does not have a CUDA context, did initialisation fail?");
		return nullptr;
	}
...
CUcontext FCUDAModule::GetCudaContextForDevice(int DeviceIndex)
{
	if (IsAvailable() == false)
	{
		UE_LOGF(LogCUDA, Fatal, "You have requested a CUDA context when the CUDA Driver API is not loaded.");
	}
	...
	if (!contextMap.Contains(DeviceIndex))
	{
		...
		if (createCtxErr == CUDA_SUCCESS)
		{
			UE_LOGF(LogCUDA, Display, "Created a new CUDA context on device %d on request.", DeviceIndex);
			contextMap.Add(DeviceIndex, cudaContext);
		}
		else
		{
			UE_LOGF(LogCUDA, Error, "Failed to create a CUDA context on device %d on request.", DeviceIndex);
			return cudaContext;
		}
	}
