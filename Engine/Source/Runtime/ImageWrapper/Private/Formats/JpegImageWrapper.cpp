...
void FJpegImageWrapper::Uncompress(const ERGBFormat InFormat, int32 InBitDepth)
{
	...
	{
		UE_LOGF(LogImageWrapper, Error, "JPEG Decompress Error");
		RawData.Empty();
	}
#endif
}
...
bool FJpegImageWrapper::SetCompressedTurbo(const void* InCompressedData, int64 InCompressedSize)
{
	...
	{
		UE_LOGF(LogImageWrapper, Error, "Unsupported CMYK colorspace for JPEG, only Grayscale and RGB are supported");
		return false;
	}
...
void FJpegImageWrapper::UncompressTurbo(const ERGBFormat InFormat, int32 InBitDepth)
{
	...
	{
		UE_LOGF(LogImageWrapper, Error, "JPEG Decompress Error - %ls", ANSI_TO_TCHAR(tj3GetErrorStr(Decompressor)));
