...
bool FBmpImageWrapper::UncompressBMPData(const ERGBFormat InFormat, const int32 InBitDepth)
{
	...
	{
		UE_LOGF(LogImageWrapper, Error, "BitmapHeaderVersion invalid");
		return false;
	}

	/*
	UE_LOGF(LogImageWrapper, Log, "BMP compression = (%i) BitCount = (%i)", bmhdr->biCompression,bmhdr->biBitCount)
	UE_LOGF(LogImageWrapper, Log, "BMP BitsOffset = (%i)", BitsOffset)
	UE_LOGF(LogImageWrapper, Log, "BMP biSize = (%i)", bmhdr->biSize)
	*/

	if (bmhdr->biCompression != BCBI_RGB && bmhdr->biCompression != BCBI_BITFIELDS && bmhdr->biCompression != BCBI_ALPHABITFIELDS)
	{
		UE_LOGF(LogImageWrapper, Error, "RLE compression of BMP images not supported");
		return false;
	}
	
	if (bmhdr->biPlanes != 1 )
	{
		UE_LOGF(LogImageWrapper, Error, "BMP uses an unsupported biPlanes != 1");
		return false;
	}
	
	if ( bmhdr->biBitCount != 8 && bmhdr->biBitCount != 16 && bmhdr->biBitCount != 24 && bmhdr->biBitCount != 32 )
	{
		UE_LOGF(LogImageWrapper, Error, "BMP uses an unsupported biBitCount (%i)", bmhdr->biBitCount);
		return false;
	}
	...
	{
		UE_LOGF(LogImageWrapper, Error, "Bmp read would overrun buffer");
		return false;
	}
	...
	{
		UE_LOGF(LogImageWrapper, Error, "Bmp dimensions invalid");
		return false;
	}
	...
	if ( bmhdr->biBitCount==8)
	{
		...
		{
			UE_LOGF(LogImageWrapper, Error, "Bmp paletteCount over 256");
			return false;
		}
	...
	else if ( ( bmhdr->biBitCount==16 || bmhdr->biBitCount==32 ) && ( bmhdr->biCompression == BCBI_BITFIELDS || bmhdr->biCompression == BCBI_ALPHABITFIELDS) )
	{
		...
		if (HeaderVersion >= EBitmapHeaderVersion::BHV_BITMAPV4HEADER)
		{
			...
			{
				UE_LOGF(LogImageWrapper, Warning, "BMP uses an unsupported custom color space definition, sRGB color space will be used instead.");
			}
		}
		...
		if ( bmhdr->biSize == 56 && RGBAMask[3] != 0 )
		{
			...
			UE_LOGF(LogImageWrapper, Display, "Adobe 56-byte header might have alpha but we ignore it %08X", RGBAMask[3]);				
		}
	...
	{
		UE_LOGF(LogImageWrapper, Error, "BMP uses an unsupported format (planes=%i, bitcount=%i, compression=%i)", 
...
bool FBmpImageWrapper::LoadBMPHeader()
{
	...
	UE_LOGF(LogImageWrapper, Error, "Bmp header invalid");
	return false;
}
...
bool FBmpImageWrapper::LoadBMPInfoHeader(int64 HeaderOffset)
{
	...
	{	
		UE_LOGF(LogImageWrapper, Error, "BMP uses an unsupported format (planes = %i, bitcount = %i)", bmhdr->biPlanes, bmhdr->biBitCount);
		return false;
	}
}
