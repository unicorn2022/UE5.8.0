...
bool FExrImageWrapper::SetCompressed(const void* InCompressedData, int64 InCompressedSize)
{
	...
	catch (const std::exception& Exception)
	{
		...
		UE_LOGF(LogImageWrapper, Error, "Cannot parse EXR image header: %ls", Convertor.Get());
...
void FExrImageWrapper::Compress(int32 Quality)
{
	...
	UE_LOGF(LogImageWrapper, Verbose, "Compressed image in %.3f seconds", DeltaTime);
}
...
void FExrImageWrapper::Uncompress(const ERGBFormat InFormat, const int32 InBitDepth)
{
	...
	{
		UE_LOGF(LogImageWrapper, Error, "Cannot decompress EXR image: %ls.", *ErrorMessage);
	...
	else
	{
		...
		UE_LOGF(LogImageWrapper, Verbose, "Reading EXR with Channel Names: %ls %ls %ls %ls", 
	...
	{
		TStringConversion<TStringConvert<char, TCHAR>> Convertor(Exception.what());
		UE_LOGF(LogImageWrapper, Error, "Cannot decompress EXR image: %ls", Convertor.Get());
		SetError(Convertor.Get());
		return;
	}
...
void FExrImageWrapper::Compress(int32 Quality)
{
	...
	const double DeltaTime = FPlatformTime::Seconds() - StartTime;
	UE_LOGF(LogImageWrapper, Verbose, "written image in %.3f seconds", DeltaTime);
}

void FExrImageWrapper::Uncompress(const ERGBFormat InFormat, const int32 InBitDepth)
{
	ensure(false);
	UE_LOGF(LogImageWrapper, Error, "FExrImageWrapper::Uncompress is not supported");
}
