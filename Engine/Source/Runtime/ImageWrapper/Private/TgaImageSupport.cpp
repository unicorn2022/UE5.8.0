...
bool DecompressTGA_helper( const FTGAFileHeader* TgaHeader, const int64 TGABufferLength, uint32* TextureData, const int64 TextureDataSize )
{
	...
	if ( TgaHeader->ImageTypeCode == 10 ) // 10 = RLE compressed 
	{
		...
		{
			UE_LOGF( LogImageWrapper, Error, "TgaHeader uses an unsupported rle-compressed bit-depth: %u", TgaHeader->BitsPerPixel );
			return false;
		}
	}
	else if(TgaHeader->ImageTypeCode == 2) // 2 = Uncompressed RGB
	{
		...
		{
			UE_LOGF( LogImageWrapper, Error, "TgaHeader uses an unsupported bit-depth: %u", TgaHeader->BitsPerPixel );
			return false;
		}
	}
	...
	{
		UE_LOGF( LogImageWrapper, Error, "TgaHeader is an unsupported type: %u", TgaHeader->ImageTypeCode );
		return false;
	}

	if (!bSuccess)
	{
		UE_LOGF(LogImageWrapper, Error, "The TGA file is invalid or corrupted");
		return false;
	}
