	...
	if (OpusDecoderWrapper)
	{
		...
		while(OutputSizeToGo > 0)
		{
			...
			else if (NumDecodedFrames == OPUS_BUFFER_TOO_SMALL)
			{
				...
				{
					UE_LOGF(LogOpusAudioDecoder, Error, "opus_multistream_decode() returned %d while decoding into temporary buffer", NumDecodedFrames);
			...
			else
			{
				...
				UE_LOGF(LogOpusAudioDecoder, Error, "opus_multistream_decode() returned %d", NumDecodedFrames);
				return FDecodeResult();
			}
		}
		Result.NumCompressedBytesConsumed = InputDataPtr - CompressedData;
...
{
	if (GetStreamingSoundWaveData().IsValid())
	{
