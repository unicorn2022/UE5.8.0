...
void FLODUtilities::ProcessImportMeshInfluences(const int32 VertexCount, TArray<SkeletalMeshImportData::FRawBoneInfluence>& Influences, const FString& MeshName)
{
	...
	{
		UE_LOGF(LogLODUtilities, Display, "Skeletal mesh (%ls) influence count of %d exceeds max count of %d. Influence truncation will occur. Maximum Ignored Weight %f", *MeshName, MaxVertexInfluence, MAX_TOTAL_INFLUENCES, MaxIgnoredWeight);
	}
	...
	if (Influences.Num() == 0)
	{
		...
		UE_LOGF(LogLODUtilities, Display, "Warning skeletal mesh (%ls) has no vertex influences", *MeshName);
...
void FLODUtilities::RemoveLOD(FSkeletalMeshUpdateContext& UpdateContext, int32 DesiredLOD )
{
	...
	if(SkelMeshModel->LODModels.Num() <= 1)
	{
		...
		UE_LOGF(LogLODUtilities, Warning, "Cannot remove LOD %d, there must be at least one LOD after the removal.", DesiredLOD);
		return;
	}
...
void FLODUtilities::RemoveLODs(FSkeletalMeshUpdateContext& UpdateContext, const TArray<int32>& DesiredLODs)
{
	...
	auto NoLODToRemoveDialog = []()
	{
		...
		UE_LOGF(LogLODUtilities, Warning, "No LOD to remove or there must be at least one LOD after we remove this one.");
	...
	for (int32 DesiredLODIndex = 0; DesiredLODIndex < DesiredLODs.Num(); ++DesiredLODIndex)
	{
		...
		{
			UE_LOGF(LogLODUtilities, Warning, "Cannot remove LOD %d", DesiredLOD);
		}
	}
...
bool FLODUtilities::SetCustomLOD(USkeletalMesh* DestinationSkeletalMesh, USkeletalMesh* SourceSkeletalMesh, const int32 LodIndex, const FString& SourceDataFilename)
{
		...
		if (ExistingSkinWeightProfileInfos.Num() > 0)
		{
			...
			for (const FSkinWeightProfileInfo& ProfileInfo : SkinProfiles)
			{
				...
				for(int32 AllLodIndex = 0; AllLodIndex < LodCount; ++AllLodIndex)
				{
					...
					{
						UE_LOGF(LogLODUtilities, Error, "Alternate skinning mesh vertex number is different from the mesh LOD, we cannot apply the existing alternate skinning [%ls] on the re-import skeletal mesh LOD %d [%ls]", *ProfileNameStr, LodIndex, *DestinationSkeletalMesh->GetName());
						continue;
					}
	...
	{
		UE_LOGF(LogLODUtilities, Error, "SkeletalMesh [%ls] FLODUtilities::SetCustomLOD: Root bone in LOD is '%ls' instead of '%ls'.\nImport failed.."
	...
	for (int32 i = 0; i < SourceSkeletalMesh->GetRefSkeleton().GetRawBoneNum(); i++)
	{
		...
		{
			UE_LOGF(LogLODUtilities, Error, "FLODUtilities::SetCustomLOD: Bone '%ls' not found in destination SkeletalMesh '%ls'.\nImport failed."
		...
		if (i > 0)
		{
			...
			{
				UE_LOGF(LogLODUtilities, Error, "SkeletalMesh [%ls] FLODUtilities::SetCustomLOD: Bone '%ls' in LOD has parent '%ls' instead of '%ls'"
					, *DestinationSkeletalMesh->GetName()
					, *LODBoneName.ToString()
	...
	if (LodIndex > 0)
	{
		...
		for (int32 i = 0; i < Sockets.Num(); i++)
		{
			...
			{
				UE_LOGF(LogLODUtilities, Error, "FLODUtilities::SetCustomLOD: This LOD is missing bone '%ls' used by socket '%ls'.\nAborting import."
...
FVector3f GetBaryCentric(const FVector3f& Point, const FVector3f& A, const FVector3f& B, const FVector3f& C, const TCHAR* DebugContext)
{
	...
	{
		UE_LOGF(LogUnrealMath, Warning, "SkeletalMesh [%ls]: Small triangle detected in FMath::ComputeBaryCentric2D(); can't compute valid barycentric coordinate.", DebugContext ? DebugContext : TEXT("Unknown Source"));
	}
						 ...
						 TArray<FTargetMatch>& TargetMatchData, const TArray<FSkelMeshSection>& TargetSections, const TArray<int32>& TargetSectionMatchBaseIndex, const TCHAR* DebugContext)
{
	...
	for (int32 SectionIndex = 0; SectionIndex < TargetSections.Num(); ++SectionIndex)
	{
		...
		if (BaseMeshUVBound.GetExtent().IsNearlyZero())
		{
			...
			{
				UE_LOGF(LogLODUtilities, Display, "SkeletalMesh [%ls] Remap morph target: Cannot remap morph target because source UVs are missings.", DebugContext ? DebugContext : TEXT("Unknown Source"));
			...
			if (QuadTreeTriangleResults.Num() > 0)
			{
				...
				if(!FindTriangleUVMatch(TargetUV, Triangles, QuadTreeTriangleResults, MatchTriangleIndexes))
				{
					if (!FailSafeUnmatchVertex(FoundIndexMatch))
					{
						...
						if (!bNoMatchMsgDone)
						{
							UE_LOGF(LogLODUtilities, Warning, "Reduce LOD, remap morph target: Cannot find a triangle from the base LOD that contain a vertex UV in the target LOD. Remap morph target quality will be lower.");
	...
	)
{
		...
		&& !SkeletalMesh->GetImportedModel()->InlineReductionCacheDatas.IsValidIndex(0))
	{
		...
		{
			UE_LOGF(LogLODUtilities, Warning, "%ls", *(Message.ToString()));
		}
		else
		{
...
bool ValidateAlternateSkeleton(const FSkeletalMeshImportData& ImportDataSrc, const FSkeletalMeshImportData& ImportDataDest, const FString& SkeletalMeshDestName, const int32 LODIndexDest)
{
	...
	if (SrcBonesNotUsedByDest.Num() > 0)
	{
		...
		{
			UE_LOGF(LogLODUtilities, Error, "Alternate skinning import: Not all the alternate mesh bones are used by the mesh.");
			return false;
		}
	}
	else if (DestBonesNotUsedBySrc.Num() > 0) //Do a else here since the DestBonesNotUsedBySrc is less prone to give a bad alternate influence result.
	{
		...
		{
			UE_LOGF(LogLODUtilities, Display, "Alternate skinning import: Not all the mesh bones are used by the alternate mesh.");
			return false;
		}
	}
		...
		if (OcTreeTriangleResults.Num() > 0)
		{
			...
			if (!FindTrianglePositionMatch((FVector)PositionSrc, TrianglesDest, OcTreeTriangleResults, MatchTriangleIndexes))
			{
				...
				if (!FailSafeUnmatchVertex(FoundIndexMatch))
				{
					...
					if (!bNoMatchMsgDone)
					{
						UE_LOGF(LogLODUtilities, Warning, "Alternate skinning import: Cannot find a triangle from the destination LOD that contain a vertex UV in the imported alternate skinning LOD mesh. Alternate skinning quality will be lower.");
						bNoMatchMsgDone = true;
					}
					continue;
				}
...
bool FLODUtilities::UpdateAlternateSkinWeights(USkeletalMesh* SkeletalMeshDest, const FName& ProfileNameDest, int32 LODIndexDest, const IMeshUtilities::MeshBuildOptions& Options)
{
	...
	{
		UE_LOGF(LogLODUtilities, Error, "Failed to import Skin Weight Profile as the target skeletal mesh (%ls) requires reimporting first.", *SkeletalMeshDest->GetName());
	...
	const IMeshUtilities::MeshBuildOptions& Options)
{
	...
	if (!SkeletalMeshDest->HasMeshDescription(LODIndexDest))
	{
		UE_LOGF(LogLODUtilities, Error, "Failed to import Skin Weight Profile as the target skeletal mesh (%ls) requires reimporting first.", SkeletalMeshDest ? *SkeletalMeshDest->GetName() : TEXT("NULL"));
		//Very old asset will not have this data, we cannot add alternate until the asset is reimported
		return false;
	}
	FSkeletalMeshImportData ImportDataDest;
	...
	{
		UE_LOGF(LogLODUtilities, Error, "Asset %ls failed to import Skin Weight Profile as the incomming alternate influence model vertex number is different. LOD model vertex count: %d Alternate model vertex count: %d", *SkeletalMeshDestName, PointNumberDest, ImportDataSrc.Points.Num());
		return false;
	}
...
void FLODUtilities::UnbindClothingAndBackup(USkeletalMesh* SkeletalMesh, TArray<ClothingAssetUtils::FClothingAssetMeshBinding>& ClothingBindings, const int32 LODIndex)
{
	...
	for (ClothingAssetUtils::FClothingAssetMeshBinding& Binding : ClothingBindings)
	{
		if (Binding.LODIndex == LODIndex)
		{
			...
			if (Binding.Asset)
			{
				Binding.Asset->UnbindFromSkeletalMesh(SkeletalMesh, Binding.LODIndex, INDEX_NONE);
...
void FLODUtilities::MatchImportedMaterials(FLODUtilities::FSkeletalMeshMatchImportedMaterialsParameters& Parameters)
{
	...
	{
		UE_LOGF(LogLODUtilities, Warning, "FLODUtilities::MatchImportedMaterials: Bad parameters, SkeletalMesh is null");
		return;
	}
...
bool FLODUtilities::StripLODGeometry(USkeletalMesh* SkeletalMesh, const int32 LODIndex, UTexture2D* TextureMask, const float Threshold)
{
	...
	{
		UE_LOGF(LogLODUtilities, Warning, "Cannot strip triangle for skeletalmesh %ls LOD %d.", *SkeletalMesh->GetPathName(), LODIndex);
		return false;
	}
	...
	if (bIsReductionActive && LODInfo.ReductionSettings.BaseLOD < LODIndex)
	{
		...
		UE_LOGF(LogLODUtilities, Warning, "Cannot strip triangle for skeletalmesh %ls LOD %d. Because this LOD is generated, strip the source instead.", *SkeletalMesh->GetPathName(), LODIndex);
		return false;
	}
	...
	if (FormatDataSize <= 0)
	{
		UE_LOGF(LogLODUtilities, Warning, "Cannot strip triangle for skeletalmesh %ls LOD %d. Because the texture format size is 0.", *SkeletalMesh->GetPathName(), LODIndex);
		return false;
	}
	ETextureSourceFormat SourceFormat = InitialSource.GetFormat();
	if (SourceFormat <= TSF_Invalid || SourceFormat >= TSF_MAX)
	{
		UE_LOGF(LogLODUtilities, Warning, "Cannot strip triangle for skeletalmesh %ls LOD %d. Because the texture format is invalid.", *SkeletalMesh->GetPathName(), LODIndex);
		return false;
	}
	TArray64<uint8> Ref2DData;
	if (!InitialSource.GetMipData(Ref2DData, 0, nullptr))
	{
		UE_LOGF(LogLODUtilities, Warning, "Cannot strip triangle for skeletalmesh %ls LOD %d. Because the texture data cannot be extracted.", *SkeletalMesh->GetPathName(), LODIndex);
		return false;
	}
