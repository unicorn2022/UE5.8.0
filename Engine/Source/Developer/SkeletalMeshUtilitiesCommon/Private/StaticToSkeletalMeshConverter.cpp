	...
	)
{
	...
	{
		UE_LOGF(LogStaticToSkeletalMeshConverter, Error, "Skeleton '%ls' is not empty", *InSkeleton->GetPathName());
		return false;
	}
	...
	const bool bCacheOptimize = true)
{
	...
	if (!InMeshUtilities.BuildSkeletalMesh(SkeletalMeshModel, InSkeletalMesh->GetPathName(), InSkeletalMesh->GetRefSkeleton(), LODInfluences, LODWedges, LODFaces, LODPoints, LODPointToRawMap, BuildOptions, &WarningMessages, nullptr))
	{
		...
		{
			UE_LOGF(LogStaticToSkeletalMeshConverter, Warning, "%ls", *Message.ToString());
		}
		return false;
	}
...
)
{
	...
	{
		UE_LOGF(LogStaticToSkeletalMeshConverter, Error, "Skeletal mesh '%ls' is not empty", *InSkeletalMesh->GetPathName());
		return false;
	}
	...
	if (!InBindBone.IsNone())
	{
		...
		{
			UE_LOGF(LogStaticToSkeletalMeshConverter, Error, "Bone '%ls' not found in skeleton.", *InBindBone.ToString());
			return false;
		}
	}
	...
	)
{
	...
	if (Profiles.IsEmpty())
	{
		UE_LOGF(LogStaticToSkeletalMeshConverter, Error, "Mesh description doesn't have a skin weight attribute.");
		return false;
	}
	...
	for (const FVertexID VertexID: InMeshDescription.Vertices().GetElementIDs())
	{
		for (FBoneWeight BoneWeight: VertexSkinWeights.Get(VertexID))
		{
			...
			{
				UE_LOGF(LogStaticToSkeletalMeshConverter, Error, "Mesh description's skin weight refers to a non-existent bone (%d of %d).", BoneWeight.GetBoneIndex(), BoneIndexMax);
				return false;
			}
		}
	}
...
{
	FInitializationParams Parameters;
	Parameters.Materials = InMaterials;
	Parameters.bRecomputeNormals = bInRecomputeNormals;
	Parameters.bRecomputeTangents = bInRecomputeTangents;
	Parameters.bCacheOptimize = bCacheOptimize;
	
	return InitializeSkeletalMeshFromMeshDescriptions(InSkeletalMesh, InMeshDescriptions, InReferenceSkeleton, Parameters);
}
...
)	
{
	...
	{
		UE_LOGF(LogStaticToSkeletalMeshConverter, Error, "No mesh descriptions given");
		return false;
	}
	...
	InSkeletalMesh->SetMaterials(TArray<FSkeletalMaterial>{InParams.Materials});

	TSet<FName> ValidMaterialSlotNames;
	for (int32 Index = 0; Index < InParams.Materials.Num(); Index++)
	{
		const FSkeletalMaterial& Material = InParams.Materials[Index];
	...
	// This ensures that the render data gets built before we return, by calling PostEditChange when we fall out of scope.
	{
		...
		for (const FMeshDescription* MeshDescription: InMeshDescriptions)
		{
			...
			SkeletalLODInfo.BuildSettings.bRecomputeNormals = InParams.bRecomputeNormals;
			SkeletalLODInfo.BuildSettings.bRecomputeTangents = InParams.bRecomputeTangents;
			...
			for (FPolygonGroupID PolygonGroupID: ClonedDescription.PolygonGroups().GetElementIDs())
			{
				if (!ValidMaterialSlotNames.Contains(MaterialSlotNamesAttribute.Get(PolygonGroupID)))
				{
					...
					MaterialIndex = FMath::Clamp(MaterialIndex, 0, InParams.Materials.Num() - 1);
					if (InParams.Materials.IsValidIndex(MaterialIndex))
					{
						MaterialSlotNamesAttribute.Set(PolygonGroupID, InParams.Materials[MaterialIndex].MaterialSlotName);
					}
				}
			}
			
			// pre-add the vertex attributes to enable for render so that the render mesh generation takes them into account during creation. 
			if (!InParams.VertexAttributesForRender.IsEmpty())
			{
				const TAttributesSet<FVertexID>& VertexAttributes = MeshDescription->VertexAttributes();
				VertexAttributes.ForEachByType<float>([&InParams, &SkeletalLODInfo](const FName AttributeName, TVertexAttributesConstRef<float>)
				{
					if (!FSkeletalMeshAttributes::IsReservedAttributeName(AttributeName))
					{
						if (InParams.VertexAttributesForRender.Contains(AttributeName))
						{
							FSkeletalMeshVertexAttributeInfo& NewAttribute = SkeletalLODInfo.VertexAttributes.AddDefaulted_GetRef();
							NewAttribute.Name = AttributeName;
							NewAttribute.EnabledForRender = true;
						}
					}
				});
			}
			
			if (!AddLODFromMeshDescription(MoveTemp(ClonedDescription), InSkeletalMesh, MeshUtilities, InParams.bCacheOptimize))
			{
				// If we didn't get a model for LOD index 0, we don't have a mesh. Bail out.
				if (bFirstSourceModel)
				{
