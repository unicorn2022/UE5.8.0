...
FAsyncDirectoryReader::EProgressResult FAsyncDirectoryReader::Tick(const FTimeLimit& TimeLimit)
{
	...
	for (int32 Index = 0; Index < PendingDirectories.Num(); ++Index)
	{
		...
		{
			UE_LOGF(LogFileCache, Log, "Skipping file cache scan due to exit.");
			return EProgressResult::Finished;
		}
	}
	...
	UE_LOGF(LogFileCache, Log, "Scanning file cache for directory '%ls' took %.2fs", *RootPath, GetAge());
	return EProgressResult::Finished;
}
	...
	, LastFileHashGetTime(0)
{
	...
	if (IDirectoryWatcher* DirectoryWatcher = Module.Get())
	{
		...
		{
			UE_LOGF(LogFileCache, Display, "Failed registering directory watcher for folder '%ls'", *Config.Directory);
		}
	}
}
...
void FFileCache::DiffDirtyFiles(TMap<FImmutableString, FFileData>& InDirtyFiles, TArray<FUpdateCacheTransaction>& OutTransactions, const FDirectoryState* InFileSystemState) const
{
	...
	for (const auto& Pair : InDirtyFiles)
	{
		...
		//Retrieve the timestamp directly instead of checking the existence of the file beforehand to save time
		//If the file doesn't exist the function will return FDateTime::MinValue to indicate this
		const FDateTime TimeStamp = InFileSystemState ? FDateTime::MinValue() : FileManager.GetTimeStamp(*AbsoluteFilename);
		const bool bFileExists = InFileSystemState ? InFileSystemState->Files.Find(File) != nullptr : TimeStamp != FDateTime::MinValue();
		if (bFileExists)
		{
			FFileData FileData;
			...
			else
			{
				...
				FileData = FFileData(TimeStamp, Pair.Value.FileHash);
			}
...
void FFileCache::Tick()
{
	...
	else if (AsyncFileHasher.IsValid())
	{
		...
		if (Now - LastFileHashGetTime > 5.f)
		{
			...
			{
				UE_LOGF(LogFileCache, Log, "Retrieving MD5 hashes for directory '%ls' took %.2fs", *Config.Directory, AsyncFileHasher->GetAge());
				AsyncFileHasher = nullptr;
			}
		}
	}
