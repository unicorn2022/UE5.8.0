...
bool FDirectoryWatchRequestWindows::Init(const FString& InDirectory)
{
	...
	if ( DirectoryHandle == INVALID_HANDLE_VALUE )
	{
		...
		UE_LOGF(LogDirectoryWatcher, Display, "CreateFile failed for '%ls'. GetLastError code [%d]", *FullPath, ErrorCode);
		return false;
	}
	...
	{
		const DWORD ErrorCode = ::GetLastError();
		UE_LOGF(LogDirectoryWatcher, Display, "Initial ReadDirectoryChangesW failed for '%ls'. GetLastError code [%d]", *Directory, ErrorCode);
		::CloseHandle(DirectoryHandle);
		DirectoryHandle = INVALID_HANDLE_VALUE;
		return false;
	}
...
void FDirectoryWatchRequestWindows::ProcessChange(uint32 Error, uint32 NumBytes)
{
	...
	if (Error == ERROR_OPERATION_ABORTED) 
	{
		...
		bPendingDelete = true;
		UE_CLOGF(!IsEngineExitRequested(), LogDirectoryWatcher, Log, "A directory notification for '%ls' was aborted.", *Directory);
		return;
	}
	...
	else if (Error == ERROR_INVALID_PARAMETER)
	{
		...
		{
			UE_LOGF(LogDirectoryWatcher, Display, "A directory notification failed for '%ls' with ERROR_INVALID_PARAMETER. Attempting another request...", *Directory);
		}
	}
	else if (Error == ERROR_ACCESS_DENIED)
	{
		CloseHandleAndMarkForDelete();
		UE_LOGF(LogDirectoryWatcher, Display, "A directory notification failed for '%ls' because it could not be accessed. Aborting watch request...", *Directory);
		return;
	}
	...
	{
		UE_LOGF(LogDirectoryWatcher, Display, "A directory notification failed for '%ls' with error code [%d]. Attemping another request...", *Directory, Error);
	}
	else
	{
		UE_LOGF(LogDirectoryWatcher, Display, "A directory notification failed for '%ls' due to buffer overflow. Attemping another request...", *Directory);
	}
	...
	{
		const DWORD ErrorCode = ::GetLastError();
		UE_LOGF(LogDirectoryWatcher, Display, "Refresh of ReadDirectoryChangesW failed. GetLastError code [%d] Handle [%p], Path [%ls]. Aborting watch request...",
