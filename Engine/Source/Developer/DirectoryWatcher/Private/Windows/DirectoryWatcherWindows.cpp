...
bool FDirectoryWatcherWindows::RegisterDirectoryChangedCallback_Handle( const FString& Directory, const FDirectoryChanged& InDelegate, FDelegateHandle& Handle, uint32 Flags )
{
	...
	else
	{
		...
		if ( !Request->Init(DirectoryFullPath) )
		{
			...
			UE_LOGF(LogDirectoryWatcher, Warning, "Failed to begin reading directory changes for %ls. Error: %ls (0x%08x)", *DirectoryFullPath, ErrorMsg, Error);
...
void FDirectoryWatcherWindows::Tick( float DeltaSeconds )
{
	...
	TArray<FDirectoryWatchRequestWindows*> WatchRequestsToProcessThisFrame;
		...
		RequestIt; ++RequestIt)
	{
		...
		{
			WatchRequestsToProcessThisFrame.Add(RequestIt.Value());
		}
	}

	for (FDirectoryWatchRequestWindows* WatchRequest : WatchRequestsToProcessThisFrame)
	{
		if (!bHasPolled)
		{
