...
struct FDirectoryWatcherTestPayload
{
	...
	void OnDirectoryChanged(const TArray<FFileChangeData>& InFileChanges)
	{
		for (const auto& Change : InFileChanges)
		{
			...
			UE_LOGF(LogDirectoryWatcherTests, Log, "File '%ls'. Code: %d.", *Change.Filename, (uint8)Change.Action);
...
bool FDirectoryWatcherSimpleCreateTest::RunTest(const FString& Parameters)
{
	...
	AddCommand(new FDelayedFunctionLatentCommand([this, WorkingDir, Test]{
		...
		AddCommand(new FDelayedFunctionLatentCommand([this, Test]{
			...
			{
				UE_LOGF(LogDirectoryWatcherTests, Error, "New file '%ls' was not correctly reported as being added.", Filename);
			}
	...
	AddCommand(new FDelayedFunctionLatentCommand([this, WorkingDir]{
		...
		AddCommand(new FDelayedFunctionLatentCommand([this, WorkingDir, Test]{
			...
			AddCommand(new FDelayedFunctionLatentCommand([this, Test]{
				...
				{
					UE_LOGF(LogDirectoryWatcherTests, Error, "File '%ls' was not correctly reported as being modified.", Filename);
				}
...
bool FDirectoryWatcherSimpleDeleteTest::RunTest(const FString& Parameters)
{
	...
	AddCommand(new FDelayedFunctionLatentCommand([this, WorkingDir, Test]{
		...
		AddCommand(new FDelayedFunctionLatentCommand([this, Test]{
			...
			{
				UE_LOGF(LogDirectoryWatcherTests, Error, "File '%ls' was not correctly reported as being deleted.", Filename);
			}
	...
	AddCommand(new FDelayedFunctionLatentCommand([this, WorkingDir]{
		...
		AddCommand(new FDelayedFunctionLatentCommand([this, WorkingDir, Test]{
			...
			AddCommand(new FDelayedFunctionLatentCommand([this, Test]{
				...
				if (!Action || *Action != FFileChangeData::FCA_Added)
				{
					UE_LOGF(LogDirectoryWatcherTests, Error, "File '%ls' was not correctly reported as being added.", CreatedFilename);
				}

				Action = Test->ReportedChanges.Find(ModifiedFilename);
				if (!Action || *Action != FFileChangeData::FCA_Modified)
				{
					UE_LOGF(LogDirectoryWatcherTests, Error, "File '%ls' was not correctly reported as being modified.", ModifiedFilename);
				}

				Action = Test->ReportedChanges.Find(RemovedFilename);
				if (!Action || *Action != FFileChangeData::FCA_Removed)
				{
					UE_LOGF(LogDirectoryWatcherTests, Error, "File '%ls' was not correctly reported as being deleted.", RemovedFilename);
				}
	...
	AddCommand(new FDelayedFunctionLatentCommand([this, WorkingDir] {
		...
		AddCommand(new FDelayedFunctionLatentCommand([this, WorkingDir, Test] {
			...
			AddCommand(new FDelayedFunctionLatentCommand([this, Test] {
				...
				if (!Action || *Action != FFileChangeData::FCA_Added)
				{
					UE_LOGF(LogDirectoryWatcherTests, Error, "Folder '%ls' was not correctly reported as being added.", CreatedDirectory);
				}

				Action = Test->ReportedChanges.Find(RemovedDirectory);
				if (!Action || *Action != FFileChangeData::FCA_Removed)
				{
					UE_LOGF(LogDirectoryWatcherTests, Error, "Folder '%ls' was not correctly reported as being removed.", RemovedDirectory);
				}
	...
	AddCommand(new FDelayedFunctionLatentCommand([this, WorkingDir]{
		...
		AddCommand(new FDelayedFunctionLatentCommand([this, WorkingDir, Test]{
			...
			AddCommand(new FDelayedFunctionLatentCommand([this, Test]{
				...
				if (!Action || *Action != FFileChangeData::FCA_Added)
				{
					UE_LOGF(LogDirectoryWatcherTests, Error, "Folder '%ls' was not correctly reported as being added.", ChildDirectory);
				}

				Action = Test->ReportedChanges.Find(GrandchildDirectory);
				if (Action)
				{
					UE_LOGF(LogDirectoryWatcherTests, Error, "Changes to folder '%ls' (creation of subfolder '%ls') was reported in spite of us setting the mode 'ignore changes in subtree'.",
...
bool FDirectoryWatcherCreateDualWatchersTest::RunTest(const FString& Parameters)
{
	...
	AddCommand(new FDelayedFunctionLatentCommand([this, FileFullPath, WatchRelativePath, WatchFullPath] {
		...
		AddCommand(new FDelayedFunctionLatentCommand([this, WatchRelativePath, WatchFullPath] {
			...
			if (!Action || *Action != FFileChangeData::FCA_Added)
			{
				UE_LOGF(LogDirectoryWatcherTests, Error, "New file '%ls' was not correctly reported as being added by relative path watcher.", Filename);
			}
			...
			if (!Action || *Action != FFileChangeData::FCA_Added)
			{
				UE_LOGF(LogDirectoryWatcherTests, Error, "New file '%ls' was not correctly reported as being added by full path watcher.", Filename);
			}
