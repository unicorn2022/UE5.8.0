...
void FDirectoryWatchRequestLinux::Shutdown()
{
	...
	for (auto MapIt = GWatchDescriptorsToWatchInfo.CreateIterator(); MapIt; ++MapIt)
	{
		...
		if (&MapIt->Value.WatchRequest == this)
		{
			...
			{
				UE_LOGF(LogDirectoryWatcher, VeryVerbose, "- inotify_rm_watch(%d)", WatchDescriptor);
...
bool FDirectoryWatchRequestLinux::Init(const FString& InDirectory, uint32 Flags)
{
	...
	UE_LOGF(LogDirectoryWatcher, Verbose, "Adding watch for directory tree '%ls'", *WatchDirectory);
	...
	if (GFileDescriptor == -1)
	{
		...
		if (GFileDescriptor == -1)
		{
			...
			{
				UE_LOGF(LogDirectoryWatcher, Error, "Failed to init inotify (errno=%d, %ls)", errno, UTF8_TO_TCHAR(strerror(errno)));
			}
			return false;
		}
	}
...
void FDirectoryWatchRequestLinux::ProcessNotifications(TMap<FString, FDirectoryWatchRequestLinux*>& RequestMap)
{
	...
	// Create a list of the WatchRequests to process this tick. Do not iterate over RequestMap when calling ProcessPendingNotifications because
	// calls to Register or Unregister requests while processing delegates would modify it while we are iterating over it
	TArray<FDirectoryWatchRequestLinux*> WatchRequestsToProcessThisFrame;
	WatchRequestsToProcessThisFrame.Reserve(RequestMap.Num());
	for (auto MapIt = RequestMap.CreateConstIterator(); MapIt; ++MapIt)
	{
		WatchRequestsToProcessThisFrame.Add(MapIt.Value());
	}
	
	// Trigger any file change notification delegates
	for (FDirectoryWatchRequestLinux* WatchRequest : WatchRequestsToProcessThisFrame)
	{
...
void FDirectoryWatchRequestLinux::WatchDirectoryTree(const FString & RootAbsolutePath, TArray<TPair<FFileChangeData, bool>>* FileChangesPtr)
{
	...
	UE_LOGF(LogDirectoryWatcher, VeryVerbose, "Watching tree '%ls'", *RootAbsolutePath);
	...
	for (const FString& FolderName: AllFiles)
	{
		...
		if (!PathNameHashSet.Contains(PathNameHash))
		{
			...
			if (WatchDescriptor == -1)
			{
				...
				{
					UE_LOGF(LogDirectoryWatcher, Warning, "inotify_add_watch cannot watch folder %ls (errno = %d, %ls)",
							*FolderName, errno, UTF8_TO_TCHAR(strerror(errno)));
				}
			}
			else
			{
				UE_LOGF(LogDirectoryWatcher, VeryVerbose, "+ Added WatchDescriptor %d for '%ls'", WatchDescriptor, *FolderName);
...
void FDirectoryWatchRequestLinux::UnwatchDirectoryTree(const FString& RootAbsolutePath)
{
	...
	UE_LOGF(LogDirectoryWatcher, VeryVerbose, "Unwatching tree '%ls'", *RootAbsolutePath);
	...
	for (auto MapIt = GWatchDescriptorsToWatchInfo.CreateIterator(); MapIt; ++MapIt)
	{
		...
		{
			UE_LOGF(LogDirectoryWatcher, VeryVerbose, "- Removing WatchDescriptor %d for '%ls'", WatchDescriptor, *WatchInfo.FolderName);
			...
			if (!GWatchDescriptorsToWatchInfo.Contains(WatchDescriptor))
			{
				...
				UE_LOGF(LogDirectoryWatcher, VeryVerbose, "- inotify_rm_watch(%d): %d", WatchDescriptor, RetVal ? errno : 0);
				...
				{
					UE_LOGF(LogDirectoryWatcher, Warning, "inotify_rm_watch cannot remove descriptor %d for folder '%ls' (errno = %d, %ls)",
...
void FDirectoryWatchRequestLinux::ProcessNotifyChanges(const FString& FolderName, const struct inotify_event* Event)
{
	...
	UE_LOGF(LogDirectoryWatcher, VeryVerbose, "Event: WatchDescriptor %d, mask 0x%08x, EventPath: '%ls' Event Name: '%ls' Len: %u %ls",
...
void FDirectoryWatchRequestLinux::ProcessAllINotifyChanges()
{
	...
	for (;;)
	{
		...
		{
			UE_LOGF(LogDirectoryWatcher, Error, "FDirectoryWatchRequestLinux::ProcessAllINotifyChanges() read() error (errno = %d, %ls)",
...
#endif
		{
			UE_LOGF(LogDirectoryWatcher, Warning, "  %ls (pid %d) watches:%u instances:%u", *ExeName, Pid, INotifyCount, INotifyInstances);
		}
...
static void INotifyDumpProcessStats()
{
	...
	UE_LOGF(LogDirectoryWatcher, Warning, "Total inotify Watches:%u Instances:%u", INotifyCountTotal, INotifyInstancesTotal);
}
...
void FDirectoryWatchRequestLinux::DumpINotifyErrorDetails(TMap<FString, FDirectoryWatchRequestLinux*>& RequestMap)
{
	if (!GDumpStats)
	{
		...
		UE_LOGF(LogDirectoryWatcher, Warning, "inotify error: %ls", *GINotifyErrorMsg);
	}
	GDumpStats = false;
	...
	UE_LOGF(LogDirectoryWatcher, Warning, "inotify limits");
	UE_LOGF(LogDirectoryWatcher, Warning, "  max_queued_events: %u", MaxQueuedEvents);
	UE_LOGF(LogDirectoryWatcher, Warning, "  max_user_instances: %u", MaxUserInstances);
	UE_LOGF(LogDirectoryWatcher, Warning, "  max_user_watches: %u", MaxUserWatches);
#endif

	UE_LOGF(LogDirectoryWatcher, Warning, "inotify per-process stats");
	INotifyDumpProcessStats();

	uint32 Count = 0;
	UE_LOGF(LogDirectoryWatcher, Warning, "Current watch requests");
	...
	for (auto MapIt = RequestMap.CreateConstIterator(); MapIt; ++MapIt)
	{
		...
		{
			UE_LOGF(LogDirectoryWatcher, Warning, "  %ls: %u watches (dir does not exist)",
		...
		else
		{
			...
			UE_LOGF(LogDirectoryWatcher, Warning, "  %ls: %u watches (%u total dirs)",
	...
	UE_LOGF(LogDirectoryWatcher, Warning, "Total UE inotify Watches:%u WatchDescriptors:%u Instances:%u ",
...
static bool INotifyCommandHandler(UWorld* InWorld, const TCHAR* Cmd, FOutputDevice& Ar)
{
	...
	{
		UE_LOGF(LogDirectoryWatcher, Warning, "Dumping inotify stats");
		GDumpStats = true;
		return true;
	}
