...
bool FDirectoryWatcherMac::RegisterDirectoryChangedCallback_Handle( const FString& Directory, const FDirectoryChanged& InDelegate, FDelegateHandle& OutHandle, uint32 Flags )
{
	...
	else
	{
		...
		{
			UE_LOGF(LogDirectoryWatcher, Warning, "Failed to begin reading directory changes for %ls.", *Directory);
			delete Request;
			NumRequests--;
			return false;
		}
...
void FDirectoryWatcherMac::Tick( float DeltaSeconds )
{
	...
	TArray<FDirectoryWatchRequestMac*> WatchRequestsToProcessThisFrame;
	...
	for (FDirectoryWatchRequestMac* WatchRequest : WatchRequestsToProcessThisFrame)
	{
