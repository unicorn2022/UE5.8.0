	...
	int32       InMode)
{
	...
	UE::Geometry::FAdaptiveTessellatorOptions Options;
	...
	else if (Mode == AdaptiveTessellationMode::Validation)
	{
		...
		if (bEqual)
		{
			UE_LOGF(LogStaticMesh, Display, "Nanite original and GeometryCore adaptive tessellation results are equal.");
		}
		else
		{
			UE_LOGF(LogStaticMesh, Error, "Nanite original and GeometryCore adaptive tessellation results are different.");
		}
	}
