...
void FAndroidTargetPlatformControls::GetPlatformSpecificProjectAnalytics(TArray<FAnalyticsEventAttribute>& AnalyticsParamArray) const
{
	...
	AppendAnalyticsEventConfigBool(AnalyticsParamArray, TEXT("/Script/AndroidRuntimeSettings.AndroidRuntimeSettings"), TEXT("bPackageForOpenXRImmersive"), GEngineIni);
}
...
void FAndroid_ASTCTargetPlatformControls::GetTextureFormats(const UTexture* Texture, TArray< TArray<FName> >& OutFormats) const
{
	...
	for (FName& TextureFormatName : LayerFormats)
	{
		...
		if (Texture && Texture->GetTextureClass() == ETextureClass::Volume)
		{
			// Fallback to ETC for 3D textures, as ASTC for volume textures is unsupported on certain devices like the Pixel 10
			const static FName VolumeTexRemap[][2] =
			{
				{ AndroidTexFormat::NameAutoASTC,	AndroidTexFormat::NameAutoETC2	},
				{ FName(TEXT("ASTC_RGBA")),			AndroidTexFormat::NameETC2_RGBA	},
				{ FName(TEXT("ASTC_RGB")),			AndroidTexFormat::NameETC2_RGB	},
				{ FName(TEXT("ASTC_RGBA_HQ")),		AndroidTexFormat::NameETC2_RGBA	}
			};

			for (int32 RemapIndex = 0; RemapIndex < UE_ARRAY_COUNT(VolumeTexRemap); ++RemapIndex)
			{
				if (TextureFormatName == VolumeTexRemap[RemapIndex][0])
				{
					TextureFormatName = VolumeTexRemap[RemapIndex][1];
					break;
				}
			}
		}
