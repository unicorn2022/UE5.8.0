...
public:
	...
	virtual uint32 Run()
	{
		...
		for (int32 LoopCount = 10;;)
		{
			...
			if (LoopCount++ >= GAndroidDeviceDetectionPollInterval || bForceCheck.load(std::memory_order_relaxed))
			{
	...
	void ExtractGPUInfo(FString& outGLVersion, FString& outGPUFamily, const FString& SurfaceFlingerOutput)
	{
		...
		if (LineIndex != INDEX_NONE)
		{
			...
			if (StartIndex != INDEX_NONE)
			{
				...
				// ANGLE-based devices report the GL version as "ANGLE (<GPU Name>)"
				{
					FRegexPattern AnglePattern(TEXT("ANGLE\\s*\\((.+)\\)"));
					FRegexMatcher AngleMatcher(AnglePattern, outGLVersion);
					if (AngleMatcher.FindNext())
					{
						outGPUFamily = AngleMatcher.GetCaptureGroup(1);
					}
				}
			}
		}
...
public:
		...
		if (AndroidHomeDirectory.IsEmpty() || AndroidUserHomeDirectory.IsEmpty() || AndroidEmulatorHomeDirectory.IsEmpty() || AndroidAvdHomeDirectory.IsEmpty())
		{
			...
			if (!FileReader)
			{
				...
				if (!FileReader)
				{
					...
					UE_LOGF(LogCore, Warning, ".zshrc is missing. Falling back to .bash_profile.");
				}
				else
				{
					UE_LOGF(LogCore, Warning, ".zshrc is missing. Falling back to .bashrc.");
				}
			}
#endif
			if (FileReader)
	...
	virtual void ExportDeviceProfile(const FString& OutPath, const FString& DeviceName) override
	{
		...
		bool bOpenGL3x = false;
		{
			...
			DeviceSpecs.AndroidProperties.Version = ANDROID_DEVICE_PROPERTIES_VERSION;
			...
			if (!bOpenGL3x)
			{
				UE_LOGF(LogCore, Warning, "Cannot export device info, a minimum of OpenGL ES 3 is required.");
				return;
			}
