...
DECLARE_MULTICAST_DELEGATE_OneParam(FOnProviderReady, FSubmitToolServiceProvider& /*ServiceProvider*/)

class FSubmitToolServiceProvider final
{

public:
	FSubmitToolServiceProvider()
	{
		OnProviderReady.Broadcast(*this);
	}

	inline static FOnProviderReady OnProviderReady;
	...
	{
		return Services.Contains(TNameOf<T>::GetName());
	}
