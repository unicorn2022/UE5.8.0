...
struct FGeneralParameters 
{
	...
	UPROPERTY()
	FString NewChangelistMessage = TEXT("Submit Tool generated changelist from default");

	UPROPERTY()
	TArray<FString> ForbiddenDescriptions = 
	{
		TEXT("Submit Tool generated changelist from default"),
	...
	UPROPERTY()
	FString OutOfHoursSubmitMessage = TEXT("**** It's $(DateTime) local time, be mindful of submitting late and going away, please remain alert and available for a reasonable period of time (2-3 hours) in case your changes cause issues ****");

	UPROPERTY()
	TArray<FString> GroupsToExclude;
	
	UPROPERTY()
	TMap<FString, FString> PathOverrides;
	
	UPROPERTY()
	TMap<FString, FString> PathOverridesInSourceBuild;
	
	UPROPERTY()
	TArray<FString> AutomaticChangelistTags;

	UPROPERTY()
	TArray<FString> SyncOnStartupPaths;

	UPROPERTY()
	FString DescriptionGenProcess;

	UPROPERTY()
	FString DescriptionGenTooltip;

	UPROPERTY()
	FString DescriptionGenArgs;

	UPROPERTY()
	FString DescriptionGenParsing;

	UPROPERTY()
	FString DescriptionGenDocumentationUrl;
};
...
struct FP4LockdownParameters
{
	...
	UPROPERTY()
	TMap<FString, FString> DynamicConfigPaths;	
};
