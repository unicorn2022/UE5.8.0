...
bool FCookMetadataState::Serialize(FArchive& Ar)
{
	...
	if (Ar.IsLoading())
	{
		...
		{
			UE_LOGF(LogCookedMetadata, Error, "Cook metadata version too old: found %d, we can load %d, latest is %d", EnumToUnderlyingType(Version), EnumToUnderlyingType(ECookMetadataStateVersion::AddedPluginEntryType), EnumToUnderlyingType(ECookMetadataStateVersion::LatestVersion));
			return false; // invalid version - current we don't support backcompat
		}
		if (Version > ECookMetadataStateVersion::LatestVersion)
		{
			UE_LOGF(LogCookedMetadata, Error, "Cook metadata version too new: found %d, latest is %d", EnumToUnderlyingType(Version), EnumToUnderlyingType(ECookMetadataStateVersion::LatestVersion));
			return false;
		}
...
bool FCookMetadataState::ReadFromFile(const FString& FilePath)
{
	...
	if (FileReader)
	{
		...
		{
			UE_LOGF(LogCookedMetadata, Error, "Failed to serialize cook metadata from file (%ls)", *FilePath);
		}
	}
	else
	{
		UE_LOGF(LogCookedMetadata, Error, "Failed to make file reader from (%ls)", *FilePath);
	}
	return false;
}
...
bool FCookMetadataState::SaveToFile(const FString& FilePath)
{
	...
	{
		UE_LOGF(LogCookedMetadata, Error, "Failed to write cook metadata file (%ls)", *FilePath);
		return false;
	}
}
