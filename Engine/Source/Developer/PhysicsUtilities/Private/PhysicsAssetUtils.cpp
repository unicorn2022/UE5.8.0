...
// This updates the collision in BodySetup associated with BoneIndex from SkelMesh. Often this is
// being done as part of a "bulk" operation, so the caller can pass in a physics asset and a list of
// bones that are going to be made physical - these can be used to help identify which bones count
// as being tied to BoneIndex, and thus which vertices etc from the skin to use when initialising
// the collision shapes.
bool CreateCollisionFromBoneInternal(
	UBodySetup* BodySetup, USkeletalMesh* SkelMesh, int32 BoneIndex, const FPhysAssetCreateParams& Params,
	const FBoneVertInfo& Info, const FSkinnedBoneTriangleCache& TriangleCache, const UPhysicsAsset* PhysicsAsset,
	TConstArrayView<int32> BoneIndicesInCreation);
...
static float GetMaximalMinSizeBelow(int32 BoneIndex, USkeletalMesh* SkelMesh, const TArray<FBoneVertInfo>& Infos)
{
	...
	for(int32 i=BoneIndex; i<SkelMesh->GetRefSkeleton().GetRawBoneNum(); i++)
	{
		if( SkelMesh->GetRefSkeleton().BoneIsChildOf(i, BoneIndex) )
		{
			...
			UE_LOGF(LogPhysics, Log,  "Parent: %ls Bone: %ls Size: %f", *SkelMesh->GetRefSkeleton().GetBoneName(BoneIndex).ToString(), *SkelMesh->GetRefSkeleton().GetBoneName(i).ToString(), MinBoneDim );
...
bool CreateFromSkeletalMeshInternal(UPhysicsAsset* PhysicsAsset, USkeletalMesh* SkelMesh, const FPhysAssetCreateParams& Params, const FSkinnedBoneTriangleCache& TriangleCache, bool bShowProgress)
{
	...
	// When creating bodies, sometimes we will need to find other non-physical (helper) bones to
	// include. For this, we need to know which physical bones to exclude.
	TArray<int32> PhysicalBoneIndices;
	for (int32 BoneIndex = 0; BoneIndex < NumBones; BoneIndex++)
	{
		// Determine if we should create a physics body for this bone
		if (ShouldMakeBone(BoneIndex))
		{
			PhysicalBoneIndices.Emplace(BoneIndex);
		}
	}
	...
	for (int32 BoneIndex = 0; BoneIndex < NumBones; BoneIndex++)
	{
		...
		if (bMakeBone)
		{
			...
			if (bShowProgress && IsInGameThread())
			{
				SlowTask.EnterProgressFrame(1.0f, FText::Format(NSLOCTEXT(
					"PhysicsAssetEditor", "ResetCollsionStepInfo", "Generating collision for {0}"), FText::FromName(BoneName)));
			}
			...
			else
			{
				...
				const bool bSuccess = CreateCollisionFromBoneInternal(
					NewBodySetup, SkelMesh, BoneIndex, Params, Info, TriangleCache, PhysicsAsset, PhysicalBoneIndices);
				if (bSuccess)
				{
...
bool CreateFromSkeletalMesh(UPhysicsAsset* PhysicsAsset, USkeletalMesh* SkelMesh, const FPhysAssetCreateParams& Params, FText& OutErrorMessage, bool bSetToMesh /*= true*/, bool bShowProgress /*= true */)
{
	...
	if (Params.bAlwaysUseVertices || Params.GeomType == EFG_SingleConvexHull ||
		Params.GeomType == EFG_MultiConvexHull || Params.GeomType == EFG_LevelSet ||
		Params.GeomType == EFG_SkinnedLevelSet)
	{
...
FMatrix ComputeCovarianceMatrix(const TArray<FVector3f>& Verts)
{
	return ComputeViewCovarianceMatrix(MakeArrayView(Verts));
}

FVector ComputeEigenVector(const FMatrix& A)
{
	// Using the power method: this is ok because we only need the dominant eigenvector and speed is
	// not critical: http://en.wikipedia.org/wiki/Power_iteration
...
/**
 * Collects BoneIndex plus all descendant bone indices in the skeleton hierarchy, stopping at any 
 * bone that has its own physics body in the PhysicsAsset.
 * 
 * Note that this may return an empty array (OutBoneIndices) if the skeletal mesh, physics asset, 
 * reference skeleton etc turn out to be invalid/problematic.
 */
static void GetRelevantBoneIndices(
	...
	TConstArrayView<int32> BoneIndicesInCreation)
{
	if (!SkeletalMesh || !PhysicsAsset || BoneIndex == INDEX_NONE)
	{
		return;
	}
	...
	// Build a set of bone indices that have their own physics body in the asset.
	// We look up by bone name since that's the reliable link between skeleton and physics asset.
	TSet<int32> PhysicalBoneIndices;
	for (const USkeletalBodySetup* BodySetup : PhysicsAsset->SkeletalBodySetups)
	{
		if (BodySetup)
		{
			int32 BodyBoneIdx = RefSkeleton.FindBoneIndex(BodySetup->BoneName);
			if (BodyBoneIdx != INDEX_NONE)
			{
				PhysicalBoneIndices.Add(BodyBoneIdx);
			}
		}
	}

	// Duplicates are automatically ignored
	PhysicalBoneIndices.Append(BoneIndicesInCreation);

	// Iterative traversal down the hierarchy. Always include the starting bone (it's the one we're
	// generating the hull for). For children, include them only if they are NOT physical, then
	// continue recursing.
	OutBoneIndices.Reset();
	TArray<int32> Stack;
	Stack.Push(BoneIndex);

	while (Stack.Num() > 0)
	{
		const int32 Current = Stack.Pop(EAllowShrinking::No);
		OutBoneIndices.Add(Current);

		// Find all direct children of Current in the ref skeleton.
		for (int32 ChildIdx = 0; ChildIdx < NumBones; ++ChildIdx)
		{
			if (RefSkeleton.GetParentIndex(ChildIdx) == Current)
			{
				// Stop at (don't include) bones that have their own physics body,
				// UNLESS it's the starting bone itself (which is physical by definition).
				if (!PhysicalBoneIndices.Contains(ChildIdx))
				{
					Stack.Push(ChildIdx);
				}
			}
	...
	TConstArrayView<int32> BoneIndicesInCreation)
{
	...
	const TArray<FVector3f>* VertsToUse = &Info.Positions;
	TArray<FVector3f> TriangleCacheVerts;
	TArray<uint32> TriangleCacheIndices;

	if (TriangleCache.IsBuilt())
	{
		TArray<int32> BoneIndices;
		if (Params.bIncludeChildBones)
		{
			GetRelevantBoneIndices(SkelMesh, PhysicsAsset, BoneIndex, BoneIndices, BoneIndicesInCreation);
		}
		else
		{
			BoneIndices.Add(BoneIndex);
		...
		TArray<uint32> Unused;
		TriangleCache.GetVerticesAndIndicesForBones(BoneIndex, BoneIndices, TriangleCacheVerts, TriangleCacheIndices, Unused);

		if (TriangleCacheVerts.IsEmpty())
		{
			if (BodySetup->AggGeom.GetElementCount() > 0)
			{
				// If we're supposed to be using the mesh but have failed to find any verts, then
				// just leave any existing collision as it is.
				FMessageLog EditorErrors("EditorErrors");
				FFormatNamedArguments Arguments;
				Arguments.Add(TEXT("BoneName"), FText::FromName(SkelMesh->GetRefSkeleton().GetBoneName(BoneIndex)));
				EditorErrors.Warning(FText::Format(NSLOCTEXT("PhysicsAssetUtils", "NoVertsUpdateHull2",
					"Unable to update hull for {BoneName} as there are no vertices associated with the bone."), Arguments));
				EditorErrors.Open();
				return false;
			}
		}
		...
		{
			VertsToUse = &TriangleCacheVerts;
		}
	}

	// Convex hulls aren't user-editable after creation, so don't set them unless we know the result
	// will be good. Other shapes can be tweaked by the user, so even if we don't have vertices,
	// create/change to them as the user can fix them up.
	if (Params.GeomType == EFG_SingleConvexHull || Params.GeomType == EFG_MultiConvexHull)
	{
		if (VertsToUse->IsEmpty())
		{
			// If there are no verts, then either leave the shape as is, or create something "default"
			FMessageLog EditorErrors("EditorErrors");
			FFormatNamedArguments Arguments;
			Arguments.Add(TEXT("BoneName"), FText::FromName(SkelMesh->GetRefSkeleton().GetBoneName(BoneIndex)));
			if (BodySetup->AggGeom.GetElementCount() > 0)
			{
				EditorErrors.Warning(FText::Format(NSLOCTEXT("PhysicsAssetUtils", "NoVertsUpdateHull1",
					"Unable to update hull for {BoneName} as there are no vertices associated with the bone."), Arguments));
				EditorErrors.Open();
				return false;
			}
			EditorErrors.Warning(FText::Format(NSLOCTEXT("PhysicsAssetUtils", "NoVertsCreateShape",
				"Unable to create a good shape for the {BoneName} as there are no vertices associated with it."), Arguments));
			EditorErrors.Open();
		}
	}
	...
	{
		const FMatrix CovarianceMatrix = ComputeCovarianceMatrix(*VertsToUse);
	...
	// Convert to ElemTM to FTransform as Matrix inverse doesn't handle well when DET == 0, so 
	FTransform ElementTransform(ElemTM);
	// Get the (Unreal scale) bounding box for this bone using the rotation.
	FBox BoneBox(ForceInit);
	for (const FVector3f& Vert : *VertsToUse)
	{
		BoneBox += ElementTransform.InverseTransformPosition(FVector(Vert));
	}

	// Verts are already in the bone's space (including scale)
	FVector BoxCenter(0, 0, 0), BoxExtent(0, 0, 0);
	if (BoneBox.IsValid)
	{
		BoneBox.GetCenterAndExtents(BoxCenter, BoxExtent);
	}
	BoxExtent = FVector::Max(BoxExtent, FVector(MinPrimSize, MinPrimSize, MinPrimSize));
	...
	{
		// We need to pick up vertices from this bone and any bone below it in the hierarchy,
		// stopping the search when we move on to another bone with physics.
		if (!TriangleCacheIndices.IsEmpty())
		{
			const int32 HullCount = Params.GeomType == EFG_SingleConvexHull ? 1 : Params.HullCount;
			DecomposeMeshToHulls(BodySetup, *VertsToUse, TriangleCacheIndices, HullCount, Params.MaxHullVerts);
		}
		else
		{
			FMessageLog EditorErrors("EditorErrors");
			FFormatNamedArguments Arguments;
			Arguments.Add(TEXT("BoneName"), FText::FromName(SkelMesh->GetRefSkeleton().GetBoneName(BoneIndex)));
			EditorErrors.Warning(FText::Format(NSLOCTEXT("PhysicsAssetUtils", "ConvexNoPositions",
				"Unable to create a convex hull for {BoneName} as there are no vertices associated with the bone."), Arguments));
			EditorErrors.Open();
			return false;
		}
	}
	...
	else if (Params.GeomType == EFG_TaperedCapsule)
	{
		...
		BodySetup->RemoveSimpleCollision();
		BodySetup->AggGeom.TaperedCapsuleElems.Add(TaperedCapsuleElem);
		BodySetup->InvalidatePhysicsData();
	}
	else if (Params.GeomType == EFG_LevelSet)
	{
		if (!VertsToUse->IsEmpty() && !TriangleCacheIndices.IsEmpty())
		{
			// This clears out existing collision (or, if it fails, it leaves it)
			const bool bOK = LevelSetHelpers::CreateLevelSetForBone(BodySetup, *VertsToUse, TriangleCacheIndices, Params.LevelSetResolution);
			if (!bOK)
			{
				FMessageLog EditorErrors("EditorErrors");
...
bool CreateCollisionFromBone(
	UBodySetup* BodySetup, USkeletalMesh* SkelMesh, int32 BoneIndex, const FPhysAssetCreateParams& Params, 
	const FBoneVertInfo& Info, const UPhysicsAsset* PhysicsAsset, TConstArrayView<int32> BoneIndicesInCreation)
{
	check(SkelMesh);
	...
	return CreateCollisionFromBoneInternal(
		BodySetup, SkelMesh, BoneIndex, Params, Info, TriangleCache, PhysicsAsset, BoneIndicesInCreation);
}

bool CreateCollisionFromBones(
	UBodySetup* BodySetup, USkeletalMesh* SkelMesh, const TArray<int32>& BoneIndices, const FPhysAssetCreateParams& Params, 
	const FBoneVertInfo& Info, const UPhysicsAsset* PhysicsAsset)
{
	check(SkelMesh);
	...
	{
		if ( !CreateCollisionFromBoneInternal(
			BodySetup, SkelMesh, BoneIndices[Index], Params, Info, TriangleCache, PhysicsAsset, BoneIndices) )
		{
...
bool CreateCollisionsFromBones(
	UPhysicsAsset* PhysicsAsset, USkeletalMesh* SkelMesh, const TArray<int32>& BodyIndices, 
	const FPhysAssetCreateParams& Params, const TArray<FBoneVertInfo>& Info, TArray<int32>& OutSuccessfulBodyIndices)
{
	check(SkelMesh);
	check(PhysicsAsset);
	...
	for (const FName& BoneName : BodySetupBoneNames)
	{
		...
		{
			TArray<int32> BoneIndicesInCreation;
			BoneIndicesInCreation.Reserve(BodyIndices.Num());
			for (int32 Index = 0; Index < BodySetupBoneNames.Num(); ++Index)
			{
				const int32 BoneIndexInCreation = SkelMesh->GetRefSkeleton().FindBoneIndex(BodySetupBoneNames[Index]);
				BoneIndicesInCreation.Add(BoneIndexInCreation);
			}

			if (CreateCollisionFromBoneInternal(
				BodySetup, SkelMesh, BoneIndex, Params, Info[BoneIndex], TriangleCache, PhysicsAsset, BoneIndicesInCreation))
			{
			...
			else
			{
				...
				// If collision generation failed, then there are two possibilities:
				// 1. It was a fresh generation, in which case we have a body with no shapes, which is Bad and must go.
				// 2. It was a regeneration, in which case we have the unchanged shapes from before - keep them.
				if (BodySetup->AggGeom.GetElementCount() == 0)
				{
					DestroyBody(PhysicsAsset, BodyIndex);
				}
	...
	for (const FName& BoneName : SubBoneNames)
	{
		...
		else
		{
			...
			if (CreateCollisionFromBoneInternal(
				NewBodySetup, SkelMesh, BoneIndex, Params, Info[BoneIndex], TriangleCache, PhysicsAsset, BoneIndicesInCreation))
			{
				SuccessfulBodySetupNames.AddUnique(BoneName);
			}
			else
