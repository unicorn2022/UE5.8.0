...
struct FPhysAssetCreateParams
{
	...
	FPhysAssetCreateParams()
	{
		...
		bAlwaysUseVertices = true;
		bIncludeChildBones = true;
	...
	/** 
	 * If true, then mesh vertices are always used to calculate the collision shape, which can be slow. If false then 
	 * the simpler shapes will not use vertices, which can result in faster processing.
	 */
	UPROPERTY(EditAnywhere, Category = "Body Creation")
	bool								bAlwaysUseVertices;

	/**
	 * If true, then when mesh vertices are used, non-physical bones of the current bone will be included when
	 * calculating the collision shapes.
	 */
	UPROPERTY(EditAnywhere, Category = "Body Creation")
...
namespace FPhysicsAssetUtils
{
	 ...
	 *
	 * @param	BodySetup              BodySetup to create the collision for
	 ...
	 * @param   PhysicsAsset           If set, then this can/will be used to recursively find relevant child bones for collision generation
	 * @param   BoneIndicesInCreation  If set, then this can/will be used to identify bones that will be physical (for collision generation) 
	 *                                 and should not be included in another bone's collision
	 * @return  Returns true if successfully created collision from bone
	 */
	PHYSICSUTILITIES_API bool CreateCollisionFromBone(
		...
		TConstArrayView<int32> BoneIndicesInCreation = TConstArrayView<int32>());
	 ...
	 */
	PHYSICSUTILITIES_API bool CreateCollisionFromBones( UBodySetup* BodySetup, USkeletalMesh* SkelMesh, const TArray<int32>& BoneIndices, FPhysAssetCreateParams& Params, const FBoneVertInfo& Info, const UPhysicsAsset* PhysicsAsset );
	...
	*/
	...
	UE_DEPRECATED(5.8, "SanitizeRestrictedContent is no longer used.") PHYSICSUTILITIES_API void SanitizeRestrictedContent(UPhysicsAsset* PhysAsset);
	...
	PHYSICSUTILITIES_API FMatrix ComputeCovarianceMatrix(const TArray<FVector3f>& Verts);
