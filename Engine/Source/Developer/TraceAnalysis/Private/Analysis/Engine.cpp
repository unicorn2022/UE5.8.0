...
struct FDispatch
{
	...
	uint16			Uid					= 0;
	uint8			FieldCount			= 0;
	uint8			Flags				= 0;	// IAnalyzer::EEventFlags
	uint16			EventSize			= 0;
	uint16			LoggerNameOffset	= 0;	// From FDispatch ptr
	uint16			EventNameOffset		= 0;	// From FDispatch ptr
	FField			Fields[];

	inline bool IsImportant() const  { return (Flags & (uint8)IAnalyzer::EEventFlags::Important) != 0; }
	inline bool IsNoSync() const     { return (Flags & (uint8)IAnalyzer::EEventFlags::NoSync) != 0; }
	inline bool IsSync() const       { return (Flags & (uint8)IAnalyzer::EEventFlags::NoSync) == 0; }
	inline bool IsDefinition() const { return (Flags & (uint8)IAnalyzer::EEventFlags::Definition) != 0; }
	inline bool MaybeHasAux() const  { return (Flags & (uint8)IAnalyzer::EEventFlags::MaybeHasAux) != 0; }
};
...
public:
	...
	void				SetFlags(IAnalyzer::EEventFlags Flags);
...
////////////////////////////////////////////////////////////////////////////////
void FDispatchBuilder::SetFlags(IAnalyzer::EEventFlags Flags)
{
	auto* Dispatch = (FDispatch*)(Buffer.GetData());
	Dispatch->Flags = (uint8)Flags;
}
...
////////////////////////////////////////////////////////////////////////////////
IAnalyzer::EEventFlags IAnalyzer::FEventTypeInfo::GetFlags() const
{
	const auto* Inner = (const FDispatch*)this;
	return static_cast<IAnalyzer::EEventFlags>(Inner->Flags);
}
...
////////////////////////////////////////////////////////////////////////////////
uint8 IAnalyzer::FEventFieldInfo::GetTypeSize() const
{
	const auto* Inner = (const FDispatch::FField*)this;
	...
	if (Inner->SizeAndType > 0)
	{
		return uint8(Inner->SizeAndType);
	}
...
////////////////////////////////////////////////////////////////////////////////
uint32 IAnalyzer::FEventFieldInfo::GetRefUid() const
{
	const auto* Inner = (const FDispatch::FField*)this;
	return Inner->RefUid;
}
...
const void* IAnalyzer::FArrayReader::GetImpl(uint32 Index, int8& SizeAndType) const
{
	...
	return (SizeAndType < 0) ?
		Inner->Data + (Index * (-SizeAndType)) :
		Inner->Data + (Index * SizeAndType);
}
...
uint32 IAnalyzer::FEventData::GetTotalSize(IAnalyzer::EStyle Style, const IAnalyzer::FOnEventContext& Context, uint32 ProtocolVersion) const
{
	...
	else if (Info->Dispatch.IsImportant())
	{
		Size += 4; // sizeof(UE::Trace::Protocol5::FImportantEventHeader);
	}
	else if (Info->Dispatch.IsNoSync())
	{
...
bool IAnalyzer::FEventData::IsDefinitionImpl(uint32& OutTypeId) const
{
	...
	return Info->Dispatch.IsDefinition();
}
...
const FTypeRegistry::FTypeInfo* FTypeRegistry::AddVersion4(const void* TraceData)
{
	...
	// NewEvent.Flags contains Protocol1::EEventFlags,
	// so it can be used in place for IAnalyzer::EEventFlags.
	static_assert((uint8)UE::Trace::Protocol1::EEventFlags::Important == (uint8)IAnalyzer::EEventFlags::Important, "");
	static_assert((uint8)UE::Trace::Protocol1::EEventFlags::MaybeHasAux == (uint8)IAnalyzer::EEventFlags::MaybeHasAux, "");
	static_assert((uint8)UE::Trace::Protocol1::EEventFlags::NoSync == (uint8)IAnalyzer::EEventFlags::NoSync, "");
	Builder.SetFlags(static_cast<IAnalyzer::EEventFlags>(NewEvent.Flags));
...
const FTypeRegistry::FTypeInfo* FTypeRegistry::AddVersion6(const void* TraceData)
{
	...
	// NewEvent.Flags contains Protocol6::EEventFlags,
	// so it can be used in place for IAnalyzer::EEventFlags.
	static_assert((uint8)UE::Trace::Protocol6::EEventFlags::None == (uint8)IAnalyzer::EEventFlags::None, "");
	static_assert((uint8)UE::Trace::Protocol6::EEventFlags::Important == (uint8)IAnalyzer::EEventFlags::Important, "");
	static_assert((uint8)UE::Trace::Protocol6::EEventFlags::MaybeHasAux == (uint8)IAnalyzer::EEventFlags::MaybeHasAux, "");
	static_assert((uint8)UE::Trace::Protocol6::EEventFlags::NoSync == (uint8)IAnalyzer::EEventFlags::NoSync, "");
	static_assert((uint8)UE::Trace::Protocol6::EEventFlags::Definition == (uint8)IAnalyzer::EEventFlags::Definition, "");
	Builder.SetFlags(static_cast<IAnalyzer::EEventFlags>(NewEvent.Flags));
...
void FAnalyzerHub::OnNewType(const FTypeRegistry::FTypeInfo* TypeInfo)
{
	...
	UE_TRACE_ANALYSIS_DEBUG_LOG("NewEvent: Uid=%u (%s.%s) Flags=%s%s%s%s",
		...
		EventTypeInfo.IsDefinition() ? "Definition|" : "",
		EventTypeInfo.IsImportant() ? "Important|" : "",
		EventTypeInfo.MaybeHasAux() ? "MaybeHasAux|" : "",
		EventTypeInfo.IsNoSync() ? "NoSync" : "Sync");
#endif // UE_TRACE_ANALYSIS_DEBUG
...
#if UE_TRACE_ANALYSIS_DEBUG
////////////////////////////////////////////////////////////////////////////////
bool FAnalysisBridge::ShouldWriteDebugLog()
{
	// Example for limiting debug logs to first 100K events...
	//return (State.TotalEventCount < 100000);
	return UE::Trace::Analysis::GDebugLevel >= 2;
}
...
void FAnalysisBridge::DebugLogEvent(const FTypeRegistry::FTypeInfo* TypeInfo, uint32 FixedSize, uint32 AuxSize, uint32 Serial)
{
	...
	const bool bDebugLog = ShouldWriteDebugLog();
	if (bDebugLog)
	{
	...
	case Protocol3::EProtocol::Id:
		{
			...
			Dispatch.SetLoggerName("$Trace");
			Dispatch.SetEventName("NewEvent");
			Dispatch.SetFlags(IAnalyzer::EEventFlags::NoSync);
			TypeRegistry.Add(Dispatch.Finalize());
		}
		break;
	}
			...
			case Protocol1::EProtocol::Id:
				{
					...
					{
						UE_TRACE_ANALYSIS_DEBUG_LOG_EX(2, "Tid=%u --> EventSerial=%u", Bridge.GetActiveThreadId(), HeaderV1->Serial);
			...
			case Protocol3::EProtocol::Id:
				{
					...
					{
						UE_TRACE_ANALYSIS_DEBUG_LOG_EX(2, "Tid=%u --> EventSerial=%u", Bridge.GetActiveThreadId(), EventSerial);
						return EventSerial;
					}
...
protected:
	...
	EStatus					OnDataPseudoImportant(const FMachineContext& Context);
	...
	const FMachineContext& Context)
{
	...
	do
	{
		...
		// Pseudo Important events, events that should be treated
		// as important but written like regular event (e.g. during OnConnect).
		Ret = OnDataPseudoImportant(Context);
		if (Ret == EStatus::Error)
		{
			return Ret;
		}
...
FProtocol5Stage::EStatus FProtocol5Stage::OnDataNewEvents(const FMachineContext& Context)
{
	...
	FStreamReader* ThreadReader = Transport.GetKnownStream(FTidPacketTransport::EThreadStreamIndex::Events);
...
FProtocol5Stage::EStatus FProtocol5Stage::OnDataImportant(const FMachineContext& Context)
{
	...
	FStreamReader* ThreadReader = Transport.GetKnownStream(FTidPacketTransport::EThreadStreamIndex::Importants);
	if (ThreadReader->IsEmpty())
	{
		return EStatus::EndOfStream;
	}
...
////////////////////////////////////////////////////////////////////////////////
FProtocol5Stage::EStatus FProtocol5Stage::OnDataPseudoImportant(const FMachineContext& Context)
{
	EventDescs.Reset();

	FStreamReader* ThreadReader = Transport.GetKnownStream(FTidPacketTransport::EThreadStreamIndex::PseudoImportants);
	if (ThreadReader->IsEmpty())
	{
		return EStatus::EndOfStream;
	}
	...
	{
		UE_TRACE_ANALYSIS_DEBUG_LOG("Error: Failed to parse on connect events");
		return EStatus::Error;
	}
	...
	// Make sure serial events on the OnConnect thread is processed as 
	// no-sync events.
	for (auto& Event : EventDescs)
	{
		if (Event.Serial != ESerial::Ignored)
		{
			// TraceLog emits serial events on the OnConnect thread
			// with serial value '0'.
			if (Event.Serial != 0)
			{
				UE_TRACE_ANALYSIS_DEBUG_LOG("Warning: An event in PseudoImportant thread contained a non-zero serial.");
			}
			Event.Serial = ESerial::Ignored;
		}
	}
	...
	Context.Bridge.SetActiveThread(ETransportTid::PseudoImportants);
	if (DispatchEvents(Context, EventDescs.GetData(), EventDescs.Num() - 1) < 0)
	{
		UE_TRACE_ANALYSIS_DEBUG_LOG("Error: Failed to dispatch pseudo important events");
		return EStatus::Error;
	}
	...
	while (true)
	{
		...
		if (TypeInfo->MaybeHasAux())
		{
			...
			while (Cursor <= End)
			{
				...
				check(Cursor[0] == uint8(EKnownUids::AuxData));
...
FProtocol5Stage::EStatus FProtocol5Stage::OnDataNormal(const FMachineContext& Context)
{
	...
	for (uint32 i = (uint32)FTidPacketTransport::EThreadStreamIndex::RealThreadsStart, n = Transport.GetThreadCount(); i < n; ++i)
	{
		uint32 NumEventDescs = EventDescs.Num();

		UE_TRACE_ANALYSIS_DEBUG_LOG_EX(3, "Thread:%u Id:%u", i, Transport.GetThreadId(i));
		...
		UE_TRACE_ANALYSIS_DEBUG_LOG_EX(3, "Thread:%u bNotEnoughData:%d", i, bNotEnoughData);
	}
	...
	// Try to dispatch the parsed events.
	{
		...
		UE_TRACE_ANALYSIS_DEBUG_LOG_EX(2, "Dispatched %d normal events (%d --> %d)", NumDispatchedEvents, NumAvailableEvents, NumAvailableEvents - NumDispatchedEvents);
		...
		NextSerialWaitCount > MaxNextSerialWaitCount)
	{
		...
		if (!bSkipSerialError)
		{
			bSkipSerialError = true;
#if UE_TRACE_ANALYSIS_DEBUG
			if (UE::Trace::Analysis::GDebugLevel >= 2)
			{
		...
		(bSkipSerialError && NumAvailableEvents > MaxAvailableEventsLowLimit))
	{
		do
		{
			...
			UE_TRACE_ANALYSIS_DEBUG_LOG_EX(2, "Skipped serials and dispatched %d normal events (%d --> %d)", NumDispatchedEvents, NumAvailableEvents, NumAvailableEvents - NumDispatchedEvents);
			NumAvailableEvents -= NumDispatchedEvents;
			check(NumAvailableEvents >= 0);
		}
...
{
	UE_TRACE_ANALYSIS_DEBUG_LOG_EX(2, "Queued event descs: %d", EventDescs.Num());
...
int32 FProtocol5Stage::DispatchEvents(const FMachineContext& Context, TArray<FEventDescStream>& EventDescHeap)
{
	...
	do
	{
		...
		else
		{
			...
			{
				UE_TRACE_ANALYSIS_DEBUG_LOG_EX(2, "Lowest serial %d (Tid=%u Uid=%u) is not low enough (NextSerial=%u; %d event descs)", EndDesc->Serial, Stream.ThreadId, uint32(EndDesc->Uid), NextSerial, EventDescs.Num());
			}
...
int32 FProtocol5Stage::ParseEvents(FStreamReader& Reader, EventDescArray& OutEventDescs, const FMachineContext& Context)
{
	while (!Reader.IsEmpty())
	{
...
#if UE_TRACE_ANALYSIS_DEBUG
		if (UE::Trace::Analysis::GDebugLevel >= 3)
		{
		...
		if (EventDesc.bHasAux)
		{
			...
			if (Ok == 0)
			{
				UE_TRACE_ANALYSIS_DEBUG_LOG_EX(2, "Warning: Incomplete aux stack (Uid=%u)! Rewind %d parsed events.", EventDesc.Uid, OutEventDescs.Num() - RewindDescsNum);
...
int32 FProtocol5Stage::ParseEventsWithAux(FStreamReader& Reader, EventDescArray& OutEventDescs, const FMachineContext& Context)
{
	...
	if (bUnsorted)
	{
		...
		UE_TRACE_ANALYSIS_DEBUG_LOG_EX(3, "Sorting %u event descs", NumDescs);
...
int32 FProtocol5Stage::ParseEvent(FStreamReader& Reader, FEventDesc& EventDesc, const FMachineContext& Context)
{
	...
	else
	{
		...
		EventDesc.bHasAux = TypeInfo->MaybeHasAux();
...
int32 FProtocol5Stage::DispatchEvents(const FMachineContext& Context, const FEventDesc* EventDesc, uint32 Count)
{
	...
	UE_TRACE_ANALYSIS_DEBUG_LOG_EX(3, "Dispatch run of %u consecutive events (Tid=%u)", Count, Context.Bridge.GetActiveThreadId());

	for (const FEventDesc* Cursor = EventDesc, *End = EventDesc + Count; Cursor < End; ++Cursor)
	{
		uint32 Uid = uint32(Cursor->Uid);

		UE_TRACE_ANALYSIS_DEBUG_LOG_EX(4, "Dispatch event (Uid=%u)...", Uid);
