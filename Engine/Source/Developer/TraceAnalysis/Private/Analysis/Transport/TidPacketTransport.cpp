...
FTidPacketTransport::EReadPacketResult FTidPacketTransport::ReadPacket()
{
	...
	if (PacketBase->ThreadId & FTidPacketBase::EncodedMarker)
	{
...
#if UE_TRACE_ANALYSIS_DEBUG
		UE_TRACE_ANALYSIS_DEBUG_LOG_EX(3, "[PACKET %u] Tid=%u, Size: %u + %u bytes, DecodedSize: %u bytes (%.0f%%)%s",
			NumPackets,
			ThreadId,
		...
		//UE_LOGF(LogCore, Log, "Decompress packet (tid=%u): %u --> %u bytes", ThreadId, DataSize, uint32(DecodedSize));
		...
		if (DataSize != 0)
		{
			...
			{
				UE_LOGF(LogCore, Error, "Unable to decompress packet; tid=%u, expected %u bytes, but decoded %d bytes!", ThreadId, uint32(DecodedSize), ResultSize);
				return EReadPacketResult::ReadError;
			}

#if UE_TRACE_ANALYSIS_DEBUG
			if (UE::Trace::Analysis::GDebugLevel >= 4)
			{
		...
		{
			UE_LOGF(LogCore, Warning, "Unable to decompress packet; tid=%u, expected to decompress %u bytes, but the packet is empty!", ThreadId, uint32(DecodedSize));
		}
	}
	else // not encoded
	{
#if UE_TRACE_ANALYSIS_DEBUG
		UE_TRACE_ANALYSIS_DEBUG_LOG_EX(3, "[PACKET %u] Tid=%u, Size: %u + %u bytes",
			NumPackets,
			ThreadId,
		...
		// for debugging purposes only
		//UE_LOGF(LogCore, Log, "Uncompressed packet (tid=%u): %u bytes", ThreadId, DataSize);
	...
	if (bHasPacketSerial)
	{
		...
		{
			UE_LOGF(LogCore, Error, "Found packet with index '%llu' when '%llu' was expected.", PacketSerial, LastPacketSerial);
			return EReadPacketResult::ReadError;
		}
...
FTidPacketTransport::ETransportResult FTidPacketTransport::Update()
{
	...
	{
		return Thread.ThreadId != ETransportTid::Events &&
				Thread.ThreadId != ETransportTid::Importants &&
				Thread.ThreadId != ETransportTid::PseudoImportants &&
				Thread.Buffer.IsEmpty();
	});
...
////////////////////////////////////////////////////////////////////////////////
FStreamReader* FTidPacketTransport::GetKnownStream(EThreadStreamIndex KnownThread)
{
	check(KnownThread < EThreadStreamIndex::RealThreadsStart);
	return GetThreadStream((uint32)KnownThread);
}
