...
public:
	...
	struct FTraceInfo
	{
		...
		UE_API FUtf8StringView				GetMetadataUri() const;
	};
	...
	UE_DEPRECATED(5.8, "Connect has been deprecated. Please use Create which returns a TUniquePtr instead.")
	static UE_API FStoreClient*				Connect(const TCHAR* Host, uint32 Port = 0);
	static UE_API TUniquePtr<FStoreClient>  Create(const TCHAR* Host, uint32 Port = 0);
