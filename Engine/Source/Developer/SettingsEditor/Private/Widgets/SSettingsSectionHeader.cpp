...
void SSettingsSectionHeader::ShowNotification(const FText& InText, SNotificationItem::ECompletionState InCompletionState, FString InNavigateTo)
{
	FNotificationInfo Notification(InText);
	Notification.ExpireDuration = 10.f;
	Notification.bFireAndForget = true;
	Notification.bUseSuccessFailIcons = false;

	if (!InNavigateTo.IsEmpty())
	{
		Notification.Hyperlink = FSimpleDelegate::CreateLambda([InNavigateTo]()
			{
				IPlatformFile& PlatformFile = FPlatformFileManager::Get().GetPlatformFile();
				if (PlatformFile.FileExists(*InNavigateTo) || PlatformFile.DirectoryExists(*InNavigateTo))
				{
					// Navigate in explorer
					FPlatformProcess::ExploreFolder(*InNavigateTo);
				}
			}
		);
		Notification.HyperlinkText = LOCTEXT("NavigateToFile", "Navigate to file");
	}

	FSlateNotificationManager::Get().AddNotification(Notification)->SetCompletionState(InCompletionState);
}
...
FReply SSettingsSectionHeader::HandleExportButtonClicked()
{
	if(SettingsSection.IsValid())
	{
		...
		if(FDesktopPlatformModule::Get()->SaveFileDialog(ParentWindowHandle, LOCTEXT("ExportSettingsDialogTitle", "Export settings...").ToString(), LastExportDir, DefaultFileName, TEXT("Config files (*.ini)|*.ini"), EFileDialogFlags::None, OutFiles))
		{
			...
			{
				ShowNotification(LOCTEXT("ExportSettingsSuccess", "Export settings succeeded"), SNotificationItem::CS_Success, OutFiles[0]);
			}
			else
			{
...
{
	return !SelectedSectionObject.IsValid() || (!InRootObjectSet.RootObjects.IsEmpty() && SelectedSectionObject == InRootObjectSet.RootObjects[0]);
}
