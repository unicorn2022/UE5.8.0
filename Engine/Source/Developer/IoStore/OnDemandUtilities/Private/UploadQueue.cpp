...
void FUploadQueue::ThreadEntry()
{
	...
	for (;;)
	{
		...
		if (Response.IsOk())
		{
			UE_LOGF(LogIoStoreOnDemand, Log, "Uploaded chunk '%ls/%ls/%ls'", *Client.GetConfig().ServiceUrl, *Bucket, *Entry.Key);
		}
		else
		{
			...
			UE_LOGF(LogIoStoreOnDemand, Warning, "Failed to upload chunk '%ls/%ls/%ls' (%ls)", *Client.GetConfig().ServiceUrl, *Bucket, *Entry.Key, ErrorResponse.ToString());
			ErrorCount++;
		}
		ConcurrentUploads--;
