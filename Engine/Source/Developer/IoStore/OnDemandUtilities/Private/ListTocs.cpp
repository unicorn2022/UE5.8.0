 ...
 *
 * Note: All values must be surrounded with "".
 */

////////////////////////////////////////////////////////////////////////////////

enum class EOnDemandTocJsonOptions : uint32
{
	Header			= (1 << 0),
	TocEntries		= (1 << 1),
	BlockSizes		= (1 << 2),
	...
	All				= Header | TocEntries | BlockSizes | BlockHashes | ContainerHeader
};
ENUM_CLASS_FLAGS(EOnDemandTocJsonOptions);

EOnDemandTocJsonOptions LexFromContext(const FContext& Context)
{
	if (Context.Get<bool>(TEXT("-AllFilters"), false))
	{
		return EOnDemandTocJsonOptions::All;
	}

	EOnDemandTocJsonOptions JsonOptions = EOnDemandTocJsonOptions::Header;

	if (Context.Get<bool>(TEXT("-TocEntries"), false))
	{
		JsonOptions |= EOnDemandTocJsonOptions::TocEntries;
	}

	if (Context.Get<bool>(TEXT("-BlockSizes"), false))
	{
	...
	if (Context.Get<bool>(TEXT("-ContainerHeader"), false))
	{
		JsonOptions |= EOnDemandTocJsonOptions::ContainerHeader;
	}
...
static FListTocsParams BuildListTocsParams(const FContext& Context)
{
	...
	Ret.JsonOptions = LexFromContext(Context);
...
static void ToJson(FJsonWriter JsonWriter, const FOnDemandToc& Toc, EOnDemandTocJsonOptions Options)
{
	JsonWriter->WriteObjectStart(TEXT("Header"));
	{
		...
		JsonWriter->WriteValue(TEXT("CompressionFormat"), Toc.Header.CompressionFormat);
		JsonWriter->WriteValue(TEXT("ChunksDirectory"), Toc.Header.ChunksDirectory);
		JsonWriter->WriteValue(TEXT("HostGroupName"), Toc.Header.HostGroupName);
	}
	...
	JsonWriter->WriteArrayStart(TEXT("Containers"));
	{
		for (const FOnDemandTocContainerEntry& Container : Toc.Containers)
		{
			...
			JsonWriter->WriteValue(TEXT("UTocHash"), LexToString(Container.UTocHash));

			if (EnumHasAnyFlags(Options, EOnDemandTocJsonOptions::ContainerHeader))
			{
				JsonWriter->WriteObjectStart(TEXT("Header"));
				{
					FIoContainerHeader ContainerHeader;
					FMemoryReaderView Ar(Container.Header);
					Ar << ContainerHeader;

					JsonWriter->WriteValue(TEXT("ContainerId"), LexToString(ContainerHeader.ContainerId));
					
					JsonWriter->WriteArrayStart("PackageIds");
					for (const FPackageId& PackageId : ContainerHeader.PackageIds)
					{
						JsonWriter->WriteValue(LexToString(PackageId));
					}
					JsonWriter->WriteArrayEnd();

					JsonWriter->WriteArrayStart("OptionalSegmentPackageIds");
					for (const FPackageId& PackageId : ContainerHeader.OptionalSegmentPackageIds)
					{
						JsonWriter->WriteValue(LexToString(PackageId));
					}
					JsonWriter->WriteArrayEnd();
...
FIoStatus ListTocs(const FListTocsParams& Params)
{
	...
	{
		UE_LOGF(LogIas, Display, "Loading credentials file '%ls'", *Params.CredentialsFile);
		...
		{
			UE_LOGF(LogIas, Display, "Found credentials for '%ls'", *Params.CredentialsFileKeyName);
		}
	}
	else
	{
	...
	{
		UE_LOGF(LogIas, Display, "Loading TOC from file '%ls'", *Params.TocPath);
	...
	else if (Params.TocUrl.IsEmpty())
	{
		...
		UE_LOGF(LogIas, Display, "Fetching TOC's from '%ls/%ls/%ls'", *Client.GetConfig().ServiceUrl, *Params.Bucket, Path.ToString());
		...
		{
			UE_LOGF(LogIas, Display, "Not TOC's found at '%ls/%ls/%ls' (%ls)", *Client.GetConfig().ServiceUrl, *Params.Bucket, Path.ToString(), *Response.GetErrorStatus());
			return FIoStatus(EIoErrorCode::NotFound);
		}
		...
		ParallelFor(Response.Objects.Num(), [&Params, &Client, &Response, &Tocs, &TocMutex](int32 Index)
		{
			...
			UE_LOGF(LogIas, Display, "Fetching TOC '%ls/%ls/%ls'", *Client.GetConfig().ServiceUrl, *Params.Bucket, *Obj.Key);
			...
			{
				UE_LOGF(LogIas, Warning, "Failed to fetch TOC '%ls/%ls/%ls'", *Client.GetConfig().ServiceUrl, *Params.Bucket, *Obj.Key);
				return;
			}
			...
			{
				UE_LOGF(LogIas, Warning, "Failed to serialize TOC '%ls/%ls/%ls'. Header version/magic mimsatch", *Client.GetConfig().ServiceUrl, *Params.Bucket, *Obj.Key);
				return;
			}
	...
	{
		UE_LOGF(LogIas, Display, "Fetching TOC '%ls/%ls'", *Client.GetConfig().ServiceUrl, *Params.TocKey);
		FS3GetObjectResponse TocResponse = Client.GetObject(FS3GetObjectRequest
		{
			.Key = Params.TocKey
		});
		...
		if (TocResponse.IsOk())
		{
			...
			{
				UE_LOGF(LogIas, Warning, "Failed to serialize TOC '%ls/%ls'. Header magic mimsatch", *Client.GetConfig().ServiceUrl, *Params.TocKey);
			}
		}
	}
	...
	if (Params.OutFile.IsEmpty())
	{
		...
		for (const FTocDescription& Desc : Tocs)
		{
			...
			UE_LOGF(LogIas, Display, "%d) %ls", Counter++, *Desc.Key);
			UE_LOGF(LogIas, Display, "%-20ls: %ls",TEXT("Date"), *Desc.DateTime.ToString());
			UE_LOGF(LogIas, Display, "%-20ls: %ls", TEXT("BuildVersion"), *Desc.Toc.Meta.BuildVersion);
			UE_LOGF(LogIas, Display, "%-20ls: %ls", TEXT("TargetPlatform"), *Desc.Toc.Meta.TargetPlatform);
			UE_LOGF(LogIas, Display, "%-20ls: %.2lf KiB", TEXT("TocSize"), double(Desc.Size) / 1024.0);
			UE_LOGF(LogIas, Display, "%-20ls: %llu", TEXT("ChunkCount"), Desc.ChunkCount);
			UE_LOGF(LogIas, Display, "%-20ls: %.2lf MiB", TEXT("TotalChunkSize"), double(Desc.TotalChunkSize) / 1024.0 / 1024.0);
			if (Params.TocPath.IsEmpty())
			{
				UE_LOGF(LogIas, Display, "%-20ls: %ls", TEXT("Url"), Url.ToString());
			}
			else
			{
				UE_LOGF(LogIas, Display, "%-20ls: %ls", TEXT("FilePath"), *Params.TocPath);
			}
		}
		UE_LOGF(LogIas, Display, "");
		...
		if (bFilteredQuery == false)
		{
			...
			UE_LOGF(LogIas, Display, "%-80ls %15ls %15ls", TEXT("BuildVersion"), TEXT("Chunk(s)"), TEXT("MiB"));
			UE_LOGF(LogIas, Display, "-------------------------------------------------------------------------------------------------------------------");
			for (const FString& Key : Keys)
			{
				if (Key != Params.Bucket)
				{
					...
					UE_LOGF(LogIas, Display, "%-80ls %15d %15.2f", *Key, ChunkStats.Chunks.Num(), double(ChunkStats.TotalChunkSize) / 1024.0 / 1024.0);
				}
			}
			UE_LOGF(LogIas, Display, "-------------------------------------------------------------------------------------------------------------------");
			FChunkStats& BucketStats = Stats.FindOrAdd(Params.Bucket);
			UE_LOGF(LogIas, Display, "%-80ls %15d %15.2f", *Params.Bucket, BucketStats.Chunks.Num(), double(BucketStats.TotalChunkSize) / 1024.0 / 1024.0);
			UE_LOGF(LogIas, Display, "");
		}
	}
	else
	{
		...
		for (const FTocDescription& Desc : Tocs)
		{
			...
			ToJson(JsonWriter, Desc.Toc, Params.JsonOptions);
			JsonWriter->WriteObjectEnd();
		}
		JsonWriter->WriteArrayEnd();
		...
		UE_LOGF(LogIas, Display, "Saving file '%ls'", *Params.OutFile);
	...
	if (Params.ChunkKeys.IsEmpty() == false)
	{
		...
		UE_LOGF(LogIas, Display, "Saving chunk key(s) '%ls'", *Params.ChunkKeys);
...
static FCommand ListTocsCommand(
	...
	TEXT("Enumerates and output information for available TOCs"),
	{
		...
		TArgument<bool>(TEXT("-AllFilters"),			TEXT("Enabled all filters in JSON output")),
		TArgument<bool>(TEXT("-ContainerHeader"),		TEXT("Add the container header info in JSON")),
		TArgument<bool>(TEXT("-TocEntries"),			TEXT("Include TOC entries in JSON output")),
		TArgument<bool>(TEXT("-BlockSizes"),			TEXT("Write block sizes in JSON output")),
		TArgument<bool>(TEXT("-BlockHashes"),			TEXT("Add block hash values in JSON output")),
