...
// Add the globals as member in the VulkanShaderRecordLooseParameterData structure in the shader record
static bool ConvertGlobalsToShaderRecord(const FShaderCompilerInput& Input, const FShaderParameterParser& ShaderParameterParser, const TMap<FStringView, FStringView>& ReplacedGlobals, FString& PreprocessedShaderSource, FShaderCompilerOutput& Output)
{
...
struct FVulkanShaderParameterParserPlatformConfiguration : public FShaderParameterParser::FPlatformConfiguration
{
	...
	virtual FString ReplaceGlobal(FStringView FullDeclString, FStringView ParamName) const final
	{
		...
		if (!FullDeclString.Contains(TEXT("ConstantBuffer")))
		{
...
void CompileVulkanShader(const FShaderCompilerInput& Input, const FShaderPreprocessOutput& InPreprocessOutput, FShaderCompilerOutput& Output)
{
	...
	const bool bSDCEMinified = UE::ShaderMinifier::SDCE::MinifyInPlace(Input, InPreprocessOutput, PreprocessedSource);
	...
	if (InternalState.bUseBindlessUniformBuffer)
	{
		...
		InternalState.bHasShaderRecordGlobals = ConvertGlobalsToShaderRecord(Input, ShaderParameterParser, ReplacedGlobals, PreprocessedSource, Output);
	}

	if (ShaderParameterParser.DidModifyShader() || bSDCEMinified || InternalState.AllBindlessUBs.Num() || InternalState.bHasShaderRecordGlobals || InternalState.PushConstantUBs.Num())
	{
