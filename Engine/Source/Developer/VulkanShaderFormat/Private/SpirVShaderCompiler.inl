...
public:
	...
	virtual ~FSpirvShaderCompilerInternalState() = default;
	...
	bool bHasShaderRecordGlobals = false;
...
)
{
	...
	Header.CodeHash = FShaderHash::HashBuffer(SerializedOutput.Spirv.Data.GetData(), SerializedOutput.Spirv.GetByteSize());
	...
	if (Frequency == SF_Vertex)
	{
		...
		for (const SpvReflectInterfaceVariable* Attribute : SpirvReflectBindings.InputAttributes)
		{
			...
			const FString InputAttrName(Attribute->semantic);
	...
	if (Frequency == SF_Pixel)
	{
		...
		for (const SpvReflectInterfaceVariable* Attribute : SpirvReflectBindings.OutputAttributes)
		{
			...
			else
			{
				...
				const FString OutputAttrName(Attribute->semantic);
	...
	// Build the SRT for this shader.
	{
		...
		{
			ShaderOutput.AddErrorf(TEXT("Internal error on BuildResourceTableMapping."));
			return;
		}
	...
	if (InternalState.bSupportsBindless)
	{
		...
		auto MoveBindlessHeaps = [&](TArray<SpvReflectDescriptorBinding*>& BindingArray, const TCHAR* HeapPrefix, uint32 BinldessDescSetNo, int32& InOutNumBindlessEntries)
		{
			...
			for (int32 Index = BindingArray.Num() - 1; Index >= 0; --Index)
			{
				...
				const FString BindingName(pBinding->name);
		...
		MoveBindlessHeaps(OutBindings.UniformBuffers, FShaderParameterParser::kBindlessCBVArrayPrefix, VulkanBindless::BindlessUniformBufferSet, OutBindings.NumBindlessResources);
		...
		// Move uniform buffers to the correct set
		{
			...
			checkf(OutBindings.UniformBuffers.Num() <= VulkanBindless::MaxUniformBuffersPerStage, TEXT("Too many UB bindings in shader."));
		}
	}
}
...
)
{
	...
	spv_reflect::ShaderModule Reflection(SpirvDataSize, SerializedOutput.Spirv.GetByteData(), SPV_REFLECT_MODULE_FLAG_SAMPLER_IMAGE_USAGE);
	...
	if (InternalState.bHasShaderRecordGlobals)
	{
		// Scan the early sections of the SPIRV to find the full definition of the VulkanShaderRecordLooseParameterData in the FVulkanHitGroupSystemParameters
		// so that we can properly reflect the offset/size of each of its members.  The compiler doesn't decorate unused members, so anything we find requires reflection.
		TArray<FSpirvStructureMemberData> Members = ParseSpirvStructureMembers(SerializedOutput.Spirv, "VulkanShaderRecordLooseParameterData");
		for (const FSpirvStructureMemberData& MemberData : Members)
		{
			if (MemberData.Name.Len() > 0)
			{
				checkf(MemberData.Offset >= 0, TEXT("Shader Record Global was named but never decorated with an Offset."));
				checkf(MemberData.Size > 0, TEXT("Shader Record Global was named but the structure never assigned types."));

				HandleReflectedGlobalConstantBufferMember(
					InternalState.Input,
					*InternalState.ParameterParser,
					MemberData.Name,
					0 /*ConstantBufferIndex*/,
					MemberData.Offset /*ReflectionOffset*/,
					MemberData.Size /*ReflectionSize*/,
					Output
				);
			}
		}
	...
	const FString UBOGlobalsNameSpv(CrossCompiler::FShaderConductorContext::GetIdentifierTable().GlobalsUniformBuffer);
	...
	// Final descriptor binding numbers for all other resource types
	{
		...
		const int32 StageOffset = InternalState.bSupportsBindless ? VulkanBindless::GetOffsetForFrequency(InternalState.GetShaderFrequency()) : 0;
		...
		auto AddReflectionInfos = [&](TArray<SpvReflectDescriptorBinding*>& BindingArray, const VkDescriptorType DescriptorType, int32 BindingTypeCount, bool bIsPackedUniformBuffer=false)
		{
			for (const SpvReflectDescriptorBinding* Binding : BindingArray)
			{
				...
				const FString ResourceName(Binding->name);
				...
				const int32 BindingIndex = (StageOffset < 0) ? VulkanBindless::MaxUniformBuffersTotal - BindingSlot - 1 : StageOffset + BindingSlot;
				...
				case VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER:
					{
						if (bIsPackedUniformBuffer)
						{
							...
							if (InternalState.UseRootParametersStructure())
							{
								...
								HandleReflectedUniformBuffer(InternalState.Input, ResourceName, ReflectionSlot, Output);
							}
		...
		checkf(UBOBindings <= VulkanBindless::MaxUniformBuffersPerStage, TEXT("Too many UB bindings in shader."));
	...
	// After we've copied the SPIRV module's copy of the SPIRV, use it to evaluate specialization constants
	SerializedOutput.Header.SpecializationConstantData = UE::ShaderCompilerCommon::ProcessSpirvSpecializationConstants(InternalState.Input, CompilerContext, Reflection, Bindings, Output);
	...
	if (InternalState.ShouldStripReflect())
	{
		...
		{
			Output.AddErrorf(TEXT("Failed to strip debug instructions from SPIR-V module"));
			return false;
		}
	}
	...
	if (InternalState.IsAndroid())
	{
		...
		if (!CompilerContext.OptimizeSpirv(SerializedOutput.Spirv.Data, OptArgs, UE_ARRAY_COUNT(OptArgs)))
		{
			Output.AddErrorf(TEXT("Failed to apply driver patches for Android"));
			return false;
		}
	}
...
static void ModifyCompilerInput(FSpirvShaderCompilerInternalState& InternalState, FShaderCompilerInput& Input)
{
	...
	else if (InternalState.IsSM5())
	{
		...
		// Only use relaxed precision on mobile platforms
		if (InternalState.IsAndroid())
		{
			Input.Environment.SetDefine(TEXT("PLATFORM_SUPPORTS_RELAXED_PRECISION"), 1);
		}
	...
	Input.Environment.SetDefine(TEXT("BINDLESS_CBV_ARRAY_PREFIX"), FShaderParameterParser::kBindlessCBVArrayPrefix);
	...
	TArrayView<FString> UBNames = {})
{
	...
	// Patch ConstantBuffers
	{
		const FString UniformBufferDeclIdentifier = TEXT("ConstantBuffer");
		...
		{
			const TCHAR* TemplateBeginPtr = FCString::Strstr(&PreprocessedShaderSource[SearchIndex + UniformBufferDeclIdentifier.Len()], TEXT("<"));
			const TCHAR* TemplateEndPtr = FCString::Strstr(TemplateBeginPtr + 1, TEXT(">"));

			FString TemplateName;
			const TCHAR* TemplateNameEndPtr = ParseHLSLSymbolName(TemplateBeginPtr + 1, TemplateName);
			check(TemplateName.Len() > 0);

			FString CBName;
			const TCHAR* CBNameEndPtr = ParseHLSLSymbolName(TemplateEndPtr + 1, CBName);
			check(CBName.Len() > 0);


			int32 CBIndex = INDEX_NONE;

			// Don't patch the VulkanHitGroupSystemParameters system buffer
			if (CBName != TEXT("VulkanHitGroupSystemParameters"))
			{
				if (UBNames.IsEmpty())
				{
					CBIndex = BindlessUBs.Add(CBName);
					check(CBIndex < 16);
				}
				else
				{
					CBIndex = UBNames.Find(*CBName);
					if (CBIndex != INDEX_NONE)
					{
						BindlessUBs[CBIndex] = CBName;
					}
				}
			}

			const TCHAR* EndDeclPtr = FCString::Strstr(CBNameEndPtr, TEXT(";"));
			const int32 EndDeclIndex = EndDeclPtr - (*PreprocessedShaderSource);

			int32 NextStartIndex = EndDeclIndex;
			if (CBIndex != INDEX_NONE)
			{
				FString NewDecl;

				// Start with the bindless heap
				const FString BindlessCBHeapName = FString::Printf(TEXT("%s%d_%s%s"),
					*SpirvShaderCompiler::kBindlessCBPrefix, CBIndex, *CBName, *SpirvShaderCompiler::kBindlessHeapSuffix);
				NewDecl += FString::Printf(TEXT("ConstantBuffer<%s> %s[]; "), *TemplateName, *BindlessCBHeapName);
				...
				const TCHAR* BindlessIndexName =
					...
					TEXT("VulkanHitGroupSystemParameters.BindlessUniformBuffers") :
					TEXT("VulkanPushConstants.StaticUniformBufferBindings");
				NewDecl += FString::Printf(TEXT("static const %s %s = %s[%s[%d]];"),
					*TemplateName, *CBName, *BindlessCBHeapName, BindlessIndexName, CBIndex);

				const int32 OldDeclLen = EndDeclIndex - SearchIndex + 1;
...
)
{
	...
	// Compile each one of the shader modules separately and create one big blob for the engine
