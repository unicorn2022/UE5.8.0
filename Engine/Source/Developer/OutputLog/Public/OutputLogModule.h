...
public:
	 ...
	 *
	 * @note The widget becomes invalid if the OutputLog module is unloaded at runtime
	 *
	 * @param InCreationParams Input box creation parameters
	 * @return Result structure that contains the requested widget and additional data
	 */
	virtual UE::OutputLog::FConsoleInputBoxCreationResult MakeConsoleInputBox(const UE::OutputLog::FConsoleInputBoxCreationParams& InCreationParams) const;
	...
	/** Returns list used to filter which command executors are allowed */
	virtual const TUniquePtr<FNamePermissionList>& GetConsoleCommandExecutorPermissionList() const;
...
private:
	...
	/** List used to filter which command executors are allowed */
	TUniquePtr<FNamePermissionList> ConsoleCommandExecutorPermissionList;
};
