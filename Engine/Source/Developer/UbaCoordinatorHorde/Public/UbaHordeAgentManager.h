...
public:
	...
	// Requests a UBA cache server from Horde. Returns true if a cache server was assigned.
	// OutCacheEndpoint will be in "ip:port" format. OutCacheSessionKey is the AES encryption key.
	UBACOORDINATORHORDE_API bool RequestCacheServer(FString& OutCacheEndpoint, FString& OutCacheSessionKey, bool& bOutWriteAccess);
