...
void FUbaHordeConfigParser::ParseConfigFromIniFile()
{
	...
	{
		UE_LOGF(LogUbaHorde, Warning, "Configuration '[UbaController]:Enabled' is deprecated since UE 5.6; Add an enabled Horde provider to '[UbaController]:+Providers' instead");
	}
	...
	GetUbaConfigInt(*Provider.Value, TEXT("MaxWorkers"), MaxWorkers);
...
void FUbaHordeConfigParser::ParseConfigFromCommandline(const TCHAR* CmdLine)
{
	...
	ParseCmdLineInt(CmdLine, TEXT("UBAHordeMaxWorkers="), MaxWorkers);
...
void FUbaHordeConfig::Initialize()
{
	...
	for (const FString& ConfigFilePath : GetBuildConfigFilePaths())
	{
		...
		{
			UE_LOGF(LogUbaHorde, Verbose, "Read UBA/Horde settings from %ls", *ConfigFilePath);
		}
	}
	...
	ConfigParser.GetConfig(ConfigParser.MaxWorkers, HordeMaxWorkers);
	...
	if (bIsProviderEnabled)
	{
		...
		UE_LOGF(
			...
			, HordeMaxWorkers, GetConfigSource(ConfigParser.MaxWorkers.Source)
	...
	{
		UE_LOGF(LogUbaHorde, Display, "UBA/Horde Configuration [%ls]: Not Enabled", *Provider);
	}
}
