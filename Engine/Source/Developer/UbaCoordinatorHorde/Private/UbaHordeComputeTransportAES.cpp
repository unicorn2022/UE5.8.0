...
struct FUbaHordeComputeTransportAES::FOpenSSLContext
{
	...
	FOpenSSLContext(const uint8 (&InKey)[HORDE_KEY_SIZE])
	{
		...
		{
			UE_LOGF(LogUbaHorde, Warning, "Unhandled OpenSSL error (%lu) left in queue: %ls", ErrorCode, ANSI_TO_TCHAR(ERR_error_string(ErrorCode, nullptr)));
		}
	}
	...
	bool VerifyOpenSSLStatus(int ReturnCode, const TCHAR* CommandName)
	{
		if (ReturnCode != 1)
		{
			...
			UE_LOGF(LogUbaHorde, Warning, "%ls() failed with return code (%d) and error code (%lu): %ls", CommandName, ReturnCode, ErrorCode, ANSI_TO_TCHAR(ERR_error_string(ErrorCode, nullptr)));
			bHasErrors = true;
			return false;
		}
		return true;
...
size_t FUbaHordeComputeTransportAES::Recv(void* Data, size_t Size)
{
	...
	{
		UE_LOGF(LogUbaHorde, VeryVerbose, "Failed to receive AES packet (Expected size = %d)", (int32)Size);
		return 0;
	}
	...
	{
		UE_LOGF(LogUbaHorde, VeryVerbose, "Failed to receive AES packet (Expected size = %d, Size specified in header = %d)", (int32)Size, Header.DataLength);
		return 0;
	}
