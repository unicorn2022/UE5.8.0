...
bool FUbaHordeAgent::BeginCommunication()
{
	...
	UE_LOGF(LogUbaHordeAgent, Verbose, "Got a response from the server: Type=0x%08X", (int)Type);
	...
	auto FindOrAddBlobFile = [&BlobFiles, &BundleDirectory](const AgentMessage::FBlobRequest& BlobRequest) -> FArchive*
		{
			...
			if (Locator.IsEmpty())
			{
				UE_LOGF(LogUbaHordeAgent, Error, "Cannot upload binaries to Horde agent with empty locator");
				return nullptr;
			}
			...
			else
			{
				...
				{
					UE_LOGF(LogUbaHordeAgent, Error, "Cannot read blob file for Horde agent upload: %ls", *Path);
					return nullptr;
				}
			}
		};
	...
	for (;;)
	{
		...
		if (Type != EAgentMessageType::ReadBlob)
		{
			if (bTimedOut)
			{
				...
				{
					UE_LOGF(LogUbaHordeAgent, Log, "Uploading UBA agent binaries has been canceled");
					return false;
				}
				continue;
			}
		...
		UE_LOGF(LogUbaHordeAgent, Verbose, "Response [ReadBlob]: Locator=%ls, Offset=%d, Length=%d", UTF8_TO_TCHAR(LocatorData.GetData()), BlobRequest.Offset, BlobRequest.Length);
	...
	while ((Type = ChildChannel->ReadResponse(ReadresponseTimeoutMS)) != EAgentMessageType::None)
	{
		...
		case EAgentMessageType::ExecuteOutput:
		{
			if (LogReports && ChildChannel->GetResponseSize() > 0)
			{
				...
				{
					UE_LOGF(LogUbaHordeAgent, Log, "Response [ExecuteOutput]: %ls", ResponseString.Get());
				}
		...
		case EAgentMessageType::ExecuteResult:
		{
			if (ChildChannel->GetResponseSize() == sizeof(int32))
			{
				if (LogReports)
				{
					...
					UE_LOGF(LogUbaHorde, Log, "Response [ExecuteResult]: ExitCode=%d", ExecuteExitCode);
				}
				bIsValid = false;
			}
		}
		...
		case EAgentMessageType::Exception:
		{
			...
			UE_LOGF(LogUbaHorde, Log, "EXCEPTION Message: %ls", *FString(Ex.Message));
			UE_LOGF(LogUbaHorde, Log, "EXCEPTION Description: %ls", *FString(Ex.Description));
		}
		break;
