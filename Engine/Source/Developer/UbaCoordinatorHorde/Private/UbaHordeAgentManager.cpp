...
void FUbaHordeAgentManager::SetTargetCoreCount(uint32 Count)
{
	...
	while (EstimatedCoreCount < TargetCoreCount)
	{
		...
		//UE_LOGF(LogUbaHorde, Display, "Requested new agent. Estimated core count: %u, Target core count: %u", EstimatedCoreCount.Load(), TargetCoreCount.Load());
		RequestAgent();
	}
...
bool FUbaHordeAgentManager::RequestCacheServer(FString& OutCacheEndpoint, FString& OutCacheSessionKey, bool& bOutWriteAccess)
{
	...
	if (!HordeMetaClient)
	{
		...
		{
			UE_LOGF(LogUbaHorde, Warning, "Failed to create HttpClient for cache server request");
			HordeMetaClient.Reset();
			return false;
		}
	}
	ScopeLock.Unlock();
	...
	// Resolve cluster ID if set to auto
	FHordeClusterInfo ClusterInfo;
	if (Config.HordeCluster == FUbaHordeConfig::ClusterAuto)
	{
		const FString HordeRequestJsonBody = FUbaHordeMetaClient::BuildHordeRequestJsonBody(
			...
			*Config.HordeCondition,
			false, // not exclusive for cache lookup
			Config.bHordeAllowWine
		);
		TSharedPtr<FUbaHordeMetaClient::HordeClusterPromise, ESPMode::ThreadSafe> ClusterPromise = HordeMetaClient->RequestClusterId(HordeRequestJsonBody);
		if (!ClusterPromise)
		{
			UE_LOGF(LogUbaHorde, Warning, "Failed to resolve Horde cluster ID for cache server request");
			return false;
		}
		TFuture<TTuple<FHttpResponsePtr, FHordeClusterInfo>> ClusterFuture = ClusterPromise->GetFuture();
		ClusterFuture.Wait();
		ClusterInfo = ClusterFuture.Get().Value;
	}
	else
	{
	...
	TSharedPtr<FUbaHordeMetaClient::HordeCacheServerPromise, ESPMode::ThreadSafe> Promise = HordeMetaClient->RequestCacheServer(*ClusterInfo.ClusterId);
	if (!Promise)
	{
		UE_LOGF(LogUbaHorde, Warning, "Failed to request Horde cache server from cluster: %ls", *ClusterInfo.ClusterId);
		return false;
	}

	TFuture<TTuple<FHttpResponsePtr, FHordeCacheServerInfo>> Future = Promise->GetFuture();
	Future.Wait();
	FHordeCacheServerInfo CacheInfo = Future.Get().Value;

	if (CacheInfo.CacheEndpoint.IsEmpty())
	{
		UE_LOGF(LogUbaHorde, Display, "No cache server available from Horde cluster: %ls", *ClusterInfo.ClusterId);
		return false;
	}

	OutCacheEndpoint = CacheInfo.CacheEndpoint;
	OutCacheSessionKey = CacheInfo.CacheSessionKey;
	bOutWriteAccess = CacheInfo.bWriteAccess;
	return true;
}
...
void FUbaHordeAgentManager::ThreadAgent(FHordeAgentWrapper& Wrapper)
{
...

	{
		...
		if (BundleRefPaths.IsEmpty())
		{
			...
			{
#if PLATFORM_WINDOWS
				{ TEXT("UbaAgent.exe"), TEXT("UbaAgent.Bundle.ref"), false },
				{ TEXT("UbaWine.dll.so"), TEXT("UbaWine.Bundle.ref"), true },
#elif PLATFORM_LINUX
				{ TEXT("UbaAgent"), TEXT("UbaAgent.Bundle.ref"), false },
				{ TEXT("UbaAgent.debug"), TEXT("UbaAgent.debug.Bundle.ref"), true },
//				{ TEXT("libclang_rt.tsan.so"), TEXT("Tsan.Bundle.ref") }, // for debugging
			...
			for (const FBundleRec& Rec : BundleRecs)
			{
				...
				if (!FPaths::FileExists(*FilePath))
				{
					if (Rec.Optional)
					{
						UE_LOGF(LogUbaHorde, Warning, "Failed to find optional %ls for Horde bundle", *FilePath);
						continue;
					}
					else
					{
						UE_LOGF(LogUbaHorde, Error, "Failed to create Horde bundle for: %ls", *FilePath);
						bAskForAgents = false;
				...
				UE_LOGF(LogUbaHorde, Display, "Created Horde bundle for: %ls", *FilePath);
		...
		if (!HordeMetaClient)
		{
			...
			if (!HordeMetaClient->RefreshHttpClient())
			{
				UE_LOGF(LogUbaHorde, Error, "Failed to create HttpClient for UbaAgent");
				bAskForAgents = false;
		...
		if (Config.HordeCluster == FUbaHordeConfig::ClusterAuto)
		{
			...
			if (!Promise)
			{
				UE_LOGF(LogUbaHorde, Log, "Failed to resolve Horde cluster ID with HTTP/Json request: %ls", *HordeRequestJsonBody);
				return;
			}
			TFuture<TTuple<FHttpResponsePtr, FHordeClusterInfo>> Future = Promise->GetFuture();
			Future.Wait();
			ClusterInfo = Future.Get().Value;
			UE_CLOGF(!ClusterInfo.ClusterId.IsEmpty(), LogUbaHorde, Verbose, "Received resolved cluster ID from Horde: %ls", *ClusterInfo.ClusterId);
		}
		else
		{
			// Otherwise, just adopt specified cluster identifier
			ClusterInfo.ClusterId = Config.HordeCluster;
		}
		...
		if (!Promise)
		{
			UE_LOGF(LogUbaHorde, Log, "Failed to request Horde machine from cluster: %ls", *ClusterInfo.ClusterId);
			return;
		}
		...
		{
			UE_CLOGF(!LastRequestFailTime, LogUbaHorde, Verbose, "No resources available in Horde. Will keep retrying until %u cores are used (Currently have %u)", TargetCoreCount.Load(), ActiveCoreCount.Load());
		...
		for (const FString& Bundle : BundleRefPaths)
		{
			...
			{
				UE_LOGF(LogUbaHorde, Error, "Cannot launch Horde processes for UBA controller because bundle path could not be found: %ls", *Bundle);
				return;
			}
			Locator.Add('\0');
		...
		UbaAgentArgs.Add(FAnsiString::Printf("-MaxWorkers=%d", Config.HordeMaxWorkers));
		...
		UE_LOGF(LogUbaHorde, Log, "Remote execution on Horde machine [%ls:%u]: %ls", *UbaAgentIp, UbaPort.Port, *UbaAgentCmdArgs);
	...
	while (Agent->IsValid() && !ShouldExit.Wait(100))
	{
		...
		{
			UE_LOGF(LogUbaHorde, Display, "Server_AddClient(%ls:%u) failed", *UbaAgentIp, UbaPort.Port);
			return;
		}
