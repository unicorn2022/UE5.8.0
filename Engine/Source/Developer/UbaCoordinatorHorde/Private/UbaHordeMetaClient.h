...
struct FHordeCacheServerInfo
{
	FString CacheEndpoint;    // "ip:port" format, empty if unavailable
	FString CacheSessionKey;  // AES encryption key for cache communication
...
public:
	...
	using HordeCacheServerPromise = TPromise<TTuple<FHttpResponsePtr, FHordeCacheServerInfo>>;
	...
	TSharedPtr<HordeCacheServerPromise, ESPMode::ThreadSafe> RequestCacheServer(const TCHAR* ClusterId = nullptr);
