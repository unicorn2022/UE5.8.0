...
bool FUbaHordeMetaClient::RefreshHttpClient()
{
	...
	{
		UE_LOGF(LogUbaHorde, Display, "Getting Horde server URL succeeded [URL: %ls, Source: %ls]", *ServerUrl, *ServerUrlConfigSource);
	}
	else
	{
		UE_LOGF(LogUbaHorde, Warning, "Getting Horde server URL failed [Source: %ls]", *ServerUrlConfigSource);
		return false;
	}
	...
	{
		UE_LOGF(LogUbaHorde, Warning, "Login to Horde server [URL: %ls, Source: %ls] failed", *ServerUrl, *ServerUrlConfigSource);
		return false;
	}
...
TSharedPtr<FUbaHordeMetaClient::HordeClusterPromise, ESPMode::ThreadSafe> FUbaHordeMetaClient::RequestClusterId(const FString& HordeRequestJsonBody)
{
	...
	Request->OnProcessRequestComplete().BindLambda(
		[this, Promise](FHttpRequestPtr /*Request*/, FHttpResponsePtr HttpResponse, bool bConnectedSuccessfully)
		{
			...
			{
				UE_LOGF(LogUbaHorde, Display, "No response from Horde");
				return;
			}
			...
			{
				UE_LOGF(LogUbaHorde, Verbose, "Horde agent request returned with HTTP/%d: %ls", (int32)ResponseCode, *ResponseStr);
				return;
			}

			if (ResponseCode == EHttpServerResponseCodes::Denied)
			{
				UE_LOGF(LogUbaHorde, Display, "Token expired, refreshing");
			...
			if (!FJsonSerializer::Deserialize(Reader, OutJson, FJsonSerializer::EFlags::None))
			{
				...
				UE_LOGF(LogUbaHorde, Display, "Invalid response body for cluster Id resolution (HTTP/%d): %ls", (int32)ResponseCode, *ResponseStr);
				return;
			}
			...
			{
				// Report invalid response body with Display verbosity only, since this should not fail a CIS job
				UE_LOGF(LogUbaHorde, Display, "Missing \"clusterId\" entry in response body (HTTP/%d): %ls", (int32)ResponseCode, *ResponseStr);
				return;
			}
...
TSharedPtr<FUbaHordeMetaClient::HordeMachinePromise, ESPMode::ThreadSafe> FUbaHordeMetaClient::RequestMachine(const FString& HordeRequestJsonBody, const TCHAR* ClusterId)
{
	...
	Request->OnProcessRequestComplete().BindLambda(
		[this, Promise, ClusterId = FString(ClusterId)](FHttpRequestPtr /*Request*/, FHttpResponsePtr HttpResponse, bool bConnectedSuccessfully)
		{
			...
			if (ResponseCode == EHttpServerResponseCodes::NotFound ||
				ResponseCode == EHttpServerResponseCodes::ServiceUnavail ||
				ResponseCode == EHttpServerResponseCodes::TooManyRequests)
			{
				UE_LOGF(LogUbaHorde, Verbose, "Horde agent request returned with HTTP/%d: %ls", EnumToUnderlyingType(ResponseCode), *ResponseStr);
				return;
			}
			...
			{
				// Report invalid response body with Display verbosity only, since this should not fail a CIS job
				UE_LOGF(LogUbaHorde, Display, "Invalid response body for remote helper request (HTTP/%d): %ls", (int32)ResponseCode, *ResponseStr);
				return;
			}
			...
			{
				// Report invalid response body with Display verbosity only, since this should not fail a CIS job
				UE_LOGF(LogUbaHorde, Display, "Missing \"nonce\", \"ip\", or \"port\" entry in response body (HTTP/%d): %ls", (int32)ResponseCode, *ResponseStr);
				return;
			}
			...
			if (TSharedPtr<FJsonValue> PortsValue = OutJson->AsObject()->TryGetField(TEXT("ports")))
			{
				...
				{
					FHordeRemoteMachineInfo::FPortInfo& PortInfo = Info.Ports.Add(FString(Iter.Key()));
			...
			UE_LOGF(LogUbaHorde, Display, "%ls", HordeMachineSummary.ToString());
		});
...
TSharedPtr<FUbaHordeMetaClient::HordeCacheServerPromise, ESPMode::ThreadSafe> FUbaHordeMetaClient::RequestCacheServer(const TCHAR* ClusterId)
{
	TSharedPtr<HordeCacheServerPromise> Promise = MakeShared<HordeCacheServerPromise, ESPMode::ThreadSafe>();
	...
	const FString ResourcePath = FString::Format(TEXT("api/v2/compute/{0}/uba-cache"), { ClusterId != nullptr && *ClusterId != TEXT('\0') ? ClusterId : TEXT("default") });
	FHttpRequestRef Request = HttpClient->CreateRequest(TEXT("POST"), *ResourcePath);
	...
	Request->OnProcessRequestComplete().BindLambda(
		[Promise](FHttpRequestPtr /*Request*/, FHttpResponsePtr HttpResponse, bool bConnectedSuccessfully)
		{
			FHordeCacheServerInfo Info;
			...
			if (!bConnectedSuccessfully || !HttpResponse.IsValid())
			{
				UE_LOGF(LogUbaHorde, Display, "No response from Horde for cache server request");
				return;
			}
			...
			if (ResponseCode == EHttpServerResponseCodes::ServiceUnavail || ResponseCode == EHttpServerResponseCodes::TooManyRequests)
			{
				UE_LOGF(LogUbaHorde, Verbose, "Horde cache server request returned with HTTP/%d: %ls", (int32)ResponseCode, *ResponseStr);
				return;
			}

			if (ResponseCode == EHttpServerResponseCodes::Denied)
			{
				UE_LOGF(LogUbaHorde, Display, "Token expired during cache server request");
				return;
			}
			...
			if (!FJsonSerializer::Deserialize(Reader, OutJson, FJsonSerializer::EFlags::None))
			{
				UE_LOGF(LogUbaHorde, Display, "Invalid response body for cache server request (HTTP/%d): %ls", (int32)ResponseCode, *ResponseStr);
				return;
			}

			TSharedPtr<FJsonValue> CacheEndpointValue = OutJson->AsObject()->TryGetField(TEXT("cacheEndpoint"));
			if (!CacheEndpointValue.Get())
			{
				UE_LOGF(LogUbaHorde, Display, "Missing \"cacheEndpoint\" in cache server response (HTTP/%d): %ls", (int32)ResponseCode, *ResponseStr);
				return;
			}

			Info.CacheEndpoint = CacheEndpointValue->AsString();

			if (TSharedPtr<FJsonValue> KeyValue = OutJson->AsObject()->TryGetField(TEXT("cacheSessionKey")))
			{
				Info.CacheSessionKey = KeyValue->AsString();
			}

			if (TSharedPtr<FJsonValue> WriteAccessValue = OutJson->AsObject()->TryGetField(TEXT("writeAccess")))
			{
				Info.bWriteAccess = WriteAccessValue->AsBool();
			}

			UE_LOGF(LogUbaHorde, Display, "Horde cache server assigned: %ls (WriteAccess: %ls)", *Info.CacheEndpoint, Info.bWriteAccess ? TEXT("true") : TEXT("false"));
		});
