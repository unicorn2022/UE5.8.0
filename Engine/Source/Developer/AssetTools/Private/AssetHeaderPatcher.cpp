	...
	, StringMountReplacements(InMountPointReplacements)
{
	...
	else
	{
		...
		for (const FString& SrcFile : *InSrcFilePathsToPatch)
		{
			...
			if (!DstFile)
			{
				UE_LOGF(LogAssetHeaderPatcher, Warning, "No remapping for file %ls was provided when patching. This file will not be patched.", *SrcFile);
			}
			else
			{
	...
	for (const TTuple<FString, FString>& Package : PackagePathRenameMap)
	{
		...
		if (FPackageName::DoesPackageExist(SrcPackageName, &SrcFilename))
		{
			...
			if (InFilePathsToPatch)
			{
				...
				{
					UE_LOGF(LogAssetHeaderPatcher, Warning, "No remapping for package %ls was provided. This package will not be patched.", *SrcPackageName);
				}
				else
				{
		...
		{
			UE_LOGF(LogAssetHeaderPatcher, Warning, "{%ls} package does not exist, and will not be patched.", *SrcPackageName);
		}
	}
}
...
void FAssetHeaderPatcher::FContext::GatherDependentPackages()
{
	...
	for (const TPair<FString, FString>& SrcDstPair : PackagePathRenameMap)
	{
		...
		if (SrcPackageRoot != DstPackageRoot)
		{
			...
			if (RemappedRoot)
			{
				UE_CLOGF(DstPackageRoot != *RemappedRoot, LogAssetHeaderPatcher, Warning,
					"Found conflicting mountpoint remapping: /%ls/ -> /%ls/ and /%ls/ -> /%ls/. The second mapping will be used to overwrite the first.",
...
void FAssetHeaderPatcher::FContext::GenerateAdditionalRemappings()
{
	...
	for (auto& Redirect : Redirects)
	{
		...
		for (FString& VerseMount : VerseMountPoints)
		{
			...
			VerseStringReplacements.Add(SrcNameBuilder.ToString(), DstNameBuilder.ToString());
		}
	}
...
#define UE_POPULATE_OFFSET_INFO(NAME)					\
			...
			std::is_same_v<decltype(((FPackageFileSummary*)0)->NAME), int64>
				...
				{ UE_POPULATE_OFFSET_INFO(ImportTypeHierarchiesOffset) },
...
public:
	...
	FAssetHeaderPatcherInner(const FString& InSrcAsset, const FString& InDstAsset, const TMap<FString, FString>& InVerseStringReplacements, const TMap<FString, FString>& InStringMountPointReplacements, FArchive* InDstArchive = nullptr)
		: SrcAsset(InSrcAsset)
		, DstAsset(InDstAsset)
		, VerseStringReplacements(InVerseStringReplacements)
	...
	bool DoPatch(UE::Serialization::Private::FTypeResource& InOutTypeResource);
	...
	const TMap<FString, FString>& VerseStringReplacements;
	...
	struct FHeaderInformation
	{
		...
		int64 ImportTypeHierarchiesMapSize = -1;
	...
	TMap<FPackageIndex, UE::Serialization::Private::FImportTypeHierarchy> ImportTypeHierarchyMap;
...
{
	FAssetHeaderPatcherInner Inner(InSrcAsset, InDstAsset, InContext.VerseStringReplacements, InContext.StringMountReplacements);
	if (!FFileHelper::LoadFileToArray(Inner.SrcBuffer, *Inner.SrcAsset))
	{
		UE_LOGF(LogAssetHeaderPatcher, Error, "Failed to load %ls", *Inner.SrcAsset);
...
FAssetHeaderPatcher::EResult FAssetHeaderPatcherInner::PatchHeader_Deserialize()
{
	...
	if (Summary.PackageName.IsEmpty() || Summary.PackageName.Equals(TEXT("None")))
	{
		...
		if (Pos <= 0)
		{
			UE_LOGF(LogAssetHeaderPatcher, Error, "Cannot patch '%ls': Package header is missing a 'PackageName' string, nor could a PackageName be deduced.", *SrcAsset);
	...
	// patching paths for other objects in the package
	{
		...
		// so we determine which package we are and cache the map name in case we need it
		{
			...
			{
				UE_LOGF(LogAssetHeaderPatcher, Error, "Cannot patch '%ls': PackageName is malformed.", *SrcAsset);
	...
	if (Summary.DataResourceOffset > 0)
	{
		...
		UE_LOGF(LogAssetHeaderPatcher, Error, "Asset %ls has an unexpected DataResourceOffset", *SrcAsset);
		return EResult::ErrorUnexpectedSectionOrder;
	}
	if (Summary.CellExportCount > 0 || Summary.CellImportCount > 0)
	{
		UE_LOGF(LogAssetHeaderPatcher, Error, "Asset %ls contains unexpected VCells", *SrcAsset);
		return EResult::ErrorUnexpectedSectionOrder;
	}
	...
	{
		UE_LOGF(LogAssetHeaderPatcher, Error, "Asset '%ls' is too old to be used with AssetHeaderPatching. Please resave the file before trying to patch again.", *SrcAsset);
...
#define UE_CHECK_AND_SET_ERROR_AND_RETURN(EXP)	\
	...
	{											\
		if (EXP)								\
		{										\
			UE_LOGF(LogAssetHeaderPatcher, Log, "Asset %ls fails %ls", *SrcAsset, TEXT(#EXP));	\
...
#undef UE_CHECK_AND_SET_ERROR_AND_RETURN
	...
	if (Summary.ImportTypeHierarchiesOffset)
	{
		MemAr.Seek(Summary.ImportTypeHierarchiesOffset);

		MemAr << ImportTypeHierarchyMap;

		HeaderInformation.ImportTypeHierarchiesMapSize = MemAr.Tell() - Summary.ImportTypeHierarchiesOffset;
	}
	...
	if (Summary.AssetRegistryDataOffset)
	{
		...
		{
			UE_LOGF(LogAssetHeaderPatcher, Error, "Failed to deserialize asset registry data for %ls", *SrcAsset);
		...
		{
			UE_LOGF(LogAssetHeaderPatcher, Error,
				"Failed to deserialize asset registry data for %ls. ReadPackageDataDependencies internal error: (%d != %d || %d != %d).",
...
{
	auto RedirectString = [this](const FStringView InMaybePath, FString& OutRedirectedString)
		{
			// Strings we will redirect can be made of Package, Outer and Object FNames so we can't handle anything larger than 3 x NAME_SIZE
			if (InMaybePath.Len() >= (NAME_SIZE*3))
			{
				return false;
			}

			FCoreRedirectObjectName SrcName = FString(InMaybePath);
			FCoreRedirectObjectName DstName = FCoreRedirects::GetRedirectedName(ECoreRedirectFlags::Type_AllMask, SrcName, ECoreRedirectMatchFlags::DisallowPartialLHSMatch);
			if (SrcName != DstName)
			{
				OutRedirectedString = DstName.ToString();
				return true;
			}
			else
			{
				FStringView MaybeReplacement = Find(VerseStringReplacements, InMaybePath);
				if (!MaybeReplacement.IsEmpty())
				{
					OutRedirectedString = MaybeReplacement;
					return true;
				}
			}
			return false;
	...
	if (RedirectString(InOutString, InOutString))
	{
		return true;
	}
	...
	TStringBuilder<NAME_SIZE> DstStringBuilder;
	{
		...
		while (PathView.FindChar(SUBOBJECT_DELIMITER_CHAR, ColonPos))
		{
			...
			FStringView MaybePath(PathView.GetData() + LastPathDelimiterPos, ColonPos - LastPathDelimiterPos);
			FString MaybeReplacement;
			if (RedirectString(MaybePath, MaybeReplacement))
			{
			...
			else if (ShouldReplaceMountPoint(MaybePath, SrcMountPoint, DstMountPoint))
			{
				FStringView LeftPart(*InOutString, int32(PathView.GetData() - *InOutString) + LastPathDelimiterPos);
...

	{
		...
		auto PatchQuotedPath = [this, &DstStringBuilder, &RedirectString](FString& StringToPatch, FStringView Quote)
			{
				...
				while ((FirstQuotePos = PathView.Find(Quote, 0, ESearchCase::CaseSensitive)) != INDEX_NONE)
				{
					...
					FStringView SrcMountPoint;
					FStringView DstMountPoint;
					FStringView MaybePath = FStringView(PathView.GetData() + FirstQuotePos + 1, SecondQuotePos - FirstQuotePos - 1); // +1 and -1 are to skip the quotes
					FString MaybeReplacement;
					if (RedirectString(MaybePath, MaybeReplacement))
					{
...
bool FAssetHeaderPatcherInner::DoPatch(UE::Serialization::Private::FTypeResource& InOutTypeResource)
{
	auto PatchObjectName = [](FName& Name, const FCoreRedirectObjectName& Src, const FCoreRedirectObjectName& Dst, FAssetHeaderPatcherInner& Patcher) -> bool
	{
		if (Patcher.RemapFName(Src.ObjectName, Dst.ObjectName))
		{
			Name = Dst.ObjectName;
			return true;
		}
	...
	auto PatchPackageName = [](FName& Name, const FCoreRedirectObjectName& Src, const FCoreRedirectObjectName& Dst, FAssetHeaderPatcherInner& Patcher) -> bool
	{
		if (Patcher.RemapFName(Src.PackageName, Dst.PackageName))
		{
			Name = Dst.PackageName;
			return true;
		}
		else
		{
			Name = Src.PackageName;
			return false;
		}
	};
	...
	// Note that even though we are patching the super types stored in the type hierarchy, the *actual* type hierarchy
	// might be different (because e.g., types have moved). Note however, that the type hierarchy itself is a snapshot
	// of what the import types looked liked at the point the asset was saved. For this reason, we don't attempt to
	// create the type hierarchy again here. This will be done the next type the patched asset is saved.

	const FCoreRedirectObjectName SrcClass(InOutTypeResource.ClassName, NAME_None, InOutTypeResource.ClassPackageName);
	const FCoreRedirectObjectName DstClass = FCoreRedirects::GetRedirectedName(ECoreRedirectFlags::Type_Class | ECoreRedirectFlags::Type_Package, SrcClass, ECoreRedirectMatchFlags::DisallowPartialLHSMatch);

	bDidPatch |= PatchObjectName(InOutTypeResource.ClassName, SrcClass, DstClass, *this);
	bDidPatch |= PatchPackageName(InOutTypeResource.ClassPackageName, SrcClass, DstClass, *this);

	const FCoreRedirectObjectName SrcObj(InOutTypeResource.TypeName, NAME_None, InOutTypeResource.PackageName);
	const FCoreRedirectObjectName DstObj = FCoreRedirects::GetRedirectedName(ECoreRedirectFlags::Type_Object | ECoreRedirectFlags::Type_Package, SrcObj, ECoreRedirectMatchFlags::DisallowPartialLHSMatch);

	bDidPatch |= PatchObjectName(InOutTypeResource.TypeName, SrcObj, DstObj, *this);
	bDidPatch |= PatchPackageName(InOutTypeResource.PackageName, SrcObj, DstObj, *this);
...
FAssetHeaderPatcher::EResult FAssetHeaderPatcherInner::PatchHeader_PatchSections()
{
	...
	// ImportTypeHierarchy Map
	for (auto& Pair : ImportTypeHierarchyMap)
	{
		UE::Serialization::Private::FImportTypeHierarchy& Entry = Pair.Value;

		const TArray<UE::Serialization::Private::FTypeResource>& SuperTypes = Entry.GetSuperTypes();

		for (const UE::Serialization::Private::FTypeResource& SuperType : SuperTypes)
		{
			UE::Serialization::Private::FTypeResource& WritableSuperType = const_cast<UE::Serialization::Private::FTypeResource&>(SuperType);

			DoPatch(WritableSuperType);
		}
	}
...
FAssetHeaderPatcher::EResult FAssetHeaderPatcherInner::PatchHeader_WriteDestinationFile()
{
	...
	const FSectionData SourceSections[] = {
		...
		{ EPatchedSection::ImportTypeHierarchyMap,		Summary.ImportTypeHierarchiesOffset,	HeaderInformation.ImportTypeHierarchiesMapSize,		false	},
	...
	for (int32 SectionIdx = 0; SectionIdx < UE_ARRAY_COUNT(SourceSections); ++SectionIdx)
	{
		...
		{
			UE_LOGF(LogAssetHeaderPatcher, Error, "Unexpected missing required section for %ls: %d is required but has zero size.",
				*SrcAsset, SectionIdx);
	...
	if (!DstArchive)
	{
		...
		if (!FileWriter)
		{
			UE_LOGF(LogAssetHeaderPatcher, Error, "Failed to open %ls for write", *DstAsset);
			return EResult::ErrorFailedToOpenDestinationFile;
		}
	...
	if (Writer.GetCustomVersions().GetVersion(FFortniteMainBranchObjectVersion::GUID) == nullptr)
	{
		const_cast<FCustomVersionContainer&>(Writer.GetCustomVersions()).SetVersion(FFortniteMainBranchObjectVersion::GUID, -1, "");
	}
	...
	for (int32 SectionIdx = 0; SectionIdx < UE_ARRAY_COUNT(SourceSections); ++SectionIdx)
	{
		...
		case EPatchedSection::ImportTypeHierarchyMap:
		{
			const int64 MapStartOffset = Writer.Tell();
			Writer << ImportTypeHierarchyMap;

			checkf(!Writer.IsCriticalError(), TEXT("Issue writing %s"), *Writer.GetErrorMessage());

			const int64 MapSize = Writer.Tell() - MapStartOffset;
			const int64 Delta = MapSize - SourceSection.Size;
			checkf(Delta == 0, TEXT("Delta should be Zero. is %d"), (int)Delta);
			check(Summary.ImportTypeHierarchiesCount == ImportTypeHierarchyMap.Num());
			check(Summary.ImportTypeHierarchiesOffset == MapStartOffset);
		...
		case EPatchedSection::ThumbnailTable:
		{
			...
			Summary.TotalHeaderSize += (int32)Delta;
			checkf(ThumbnailTableStartOffset == Summary.ThumbnailTableOffset, TEXT("%" INT64_FMT " == %d"),
		...
		case EPatchedSection::AssetRegistryData:
		{
			...
			checkf(AssetRegistryDataStartOffset == Summary.AssetRegistryDataOffset, TEXT("%" INT64_FMT " == %d"),
		...
		case EPatchedSection::AssetRegistryDependencyData:
		{
			...
			if (Writer.IsError())
			{
				UE_LOGF(LogAssetHeaderPatcher, Error, "Failed to write to %ls", *DstAsset);
		...
		default:
			UE_LOGF(LogAssetHeaderPatcher, Error, "Unexpected section for %ls", *SrcAsset);
			return EResult::ErrorUnkownSection;
		}
...
void FAssetHeaderPatcherInner::DumpState(FStringView OutputDirectory)
{
	...
	Builder.Append(TEXT("\t\"ImportTypeHierarchyMap\":[ "));
	for (TPair<FPackageIndex, UE::Serialization::Private::FImportTypeHierarchy> Pair : ImportTypeHierarchyMap)
	{
		Builder.Append(TEXT("\n\t\t{\n"));
		Builder.Append(TEXT("\t\t\t\"Super types\": ["));

		const TArray<UE::Serialization::Private::FTypeResource>& SuperTypes = Pair.Value.GetSuperTypes();
		...
		for (const UE::Serialization::Private::FTypeResource& SuperType : SuperTypes)
		{
			...
			Builder.Append(TEXT("\n\t\t\t\t\t\"TypeName\": \""));
			Builder.Append(GetDebugFNameString(SuperType.TypeName));
			Builder.Append(TEXT("\","));

			Builder.Append(TEXT("\n\t\t\t\t\t\"PackageName\": \""));
			Builder.Append(GetDebugFNameString(SuperType.PackageName));
			Builder.Append(TEXT("\","));

			Builder.Append(TEXT("\n\t\t\t\t\t\"ClassName\": \""));
			Builder.Append(GetDebugFNameString(SuperType.ClassName));
			Builder.Append(TEXT("\","));

			Builder.Append(TEXT("\n\t\t\t\t\t\"ClassPackageName\": \""));
			Builder.Append(GetDebugFNameString(SuperType.ClassPackageName));
			Builder.Append(TEXT("\","));
...
#define CHECK_NOT_EQUALS(What, X, Y) if(!FAutomationTestFramework::Get().GetCurrentTest()->TestNotEqual(What, X, Y)) { UE_DEBUG_BREAK(); };
	...
	struct FTestPatcherContext : FAssetHeaderPatcher::FContext
	{
		...
		const TMap<FString, FString>& GetVerseStringReplacements()
		{
			return VerseStringReplacements;
		}
	...
	const TMap<FString, FString>& VerseStringReplacements = Context.GetVerseStringReplacements();
	CHECK(VerseStringReplacements.Num() > PackageRenameMap.Num()); // Ensure we generated more mappings off of the PackageRenameMap
	CHECK(FCoreRedirects::AddRedirectList(Context.GetRedirects(), TEXT("Asset Header Patcher Tests")));

	// Add Global CoreRedirects (scoped within the test context)
	{
		FCoreRedirect TagClassRedirect(ECoreRedirectFlags::Type_Class, TEXT("/Script/VerseGameplayTags.VerseTagMarkupComponent"), TEXT("/Script/VerseTags.VerseTagMarkupComponent"));
		FCoreRedirects::AddRedirectList({ TagClassRedirect }, TEXT("Asset Header Patcher Tests - Additional Redirects"));
	}

	FAssetHeaderPatcherInner Patcher(DummySrcDstAsset, DummySrcDstAsset, VerseStringReplacements, MountPointReplacementMap);
	...
	SECTION("DoPatch(FString)")
	{
		...
		SECTION("Quoted match")
		{
			...
			SECTION("Substring match")
			{
				...
				// Relies on the TagClassRedirect defined above
				{
					FString Actual(TEXT(R"(CacheVersion=1,AllowedClassReferences=("/Script/VerseGameplayTags.VerseTagMarkupComponent"))"));
					FString Expected(TEXT(R"(CacheVersion=1,AllowedClassReferences=("/Script/VerseTags.VerseTagMarkupComponent"))"));
					CHECK(Patcher.DoPatch(Actual));
					CHECK_EQUALS(TEXT("Patch substring with quoted package, object and sub-object paths. No non-fully-qualified ObjectNames are patched."), Actual, Expected);
	...
	SECTION("DoPatch(FTypeResource)")
	{
		ResetPatcher();
		TArray<FCoreRedirect> ImportTypeHierarchyRedirects =
		{
		...
		CHECK(FCoreRedirects::AddRedirectList(ImportTypeHierarchyRedirects, TEXT("Asset Header Patcher Tests - FTypeResource Patching")));

		UE::Serialization::Private::FTypeResource Actual;
		Actual.TypeName = SrcVerseImportObjectName;
		Actual.PackageName = SrcVerseAssetName;
		Actual.ClassName = SrcImportClassName;
		Actual.ClassPackageName = SrcImportClassPackage;

		UE::Serialization::Private::FTypeResource Expected;
		Expected.TypeName = DstVerseImportObjectName;
		Expected.PackageName = DstVerseAssetName;
		Expected.ClassName = DstImportClassName;
		Expected.ClassPackageName = DstImportClassPackage;

		CHECK(Patcher.DoPatch(Actual));
		CHECK_EQUALS(TEXT("FTypeResource patching doesn't update TypeName"), Actual.TypeName, Expected.TypeName);
		CHECK_EQUALS(TEXT("FTypeResource patching doesn't update PackageName"), Actual.PackageName, Expected.PackageName);
		CHECK_EQUALS(TEXT("FTypeResource patching doesn't update ClassName"), Actual.ClassName, Expected.ClassName);
		CHECK_EQUALS(TEXT("FTypeResource patching doesn't update ClassPackageName"), Actual.ClassPackageName, Expected.ClassPackageName);
	}
