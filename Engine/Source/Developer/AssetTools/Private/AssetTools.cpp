...
namespace UE::AssetTools::Private
{
	...
	FAutoConsoleCommand LogFolderPermissionsCommand = FAutoConsoleCommand(
		...
		FConsoleCommandWithArgsDelegate::CreateLambda(
			...
			{
				UE_LOGF(LogAssetTools, Log, "Folder Read Permissions:\n%ls", *IAssetTools::Get().GetFolderPermissionList()->ToString());
				UE_LOGF(LogAssetTools, Log, "Folder Write Permissions:\n%ls", *IAssetTools::Get().GetWritableFolderPermissionList()->ToString());
			}));
	...
	static bool bEnableEpicInternalAssetFeature = false;
	static FAutoConsoleVariableRef CVarEnableEpicInternalAssetFeature(
		...
		TEXT("Enables the Epic Internal Asset Feature UI (False: disabled, True: enabled")
	);
	...
	public:
		...
		static FString GetMountPointRootPath(const FString& DestinationFolder)
		{
			...
			else
			{
				...
				{
					UE_LOGF(LogAssetTools
						, Error
						, "{%ls} does not appear to be a game Content folder. Migrated content only work if placed in a Content folder. Aborting the Migration."
						, *DestinationFolder
					);
				}
			}
		...
		static void CleanInstancedPackages(const TArray<FName>& PackagesToClean, FPackageMigrationContext& PackageMigrationContext)
		{
			...
			for (UPackage* Package : ExistingPackageToClean)
			{
				const ERenameFlags PkgRenameFlags = REN_AllowPackageLinkerMismatch | REN_DoNotDirty | REN_DontCreateRedirectors | REN_NonTransactional | REN_SkipGeneratedClasses;
				verify(Package->Rename(*MakeUniqueObjectName(nullptr, UPackage::StaticClass(), *FString::Printf(TEXT("%s_DEADFROMMIGRATION"), *Package->GetName())).ToString(), nullptr, PkgRenameFlags));
				(*PurgeObject)(Package);
	...
	for (int32 i = 0; i < (int32)EAssetClassAction::AllAssetActions; ++i)
	{
		...
		for (FString& Type : SupportedTypesArray)
		{
			if (FPackageName::IsShortPackageName(Type))
			{
				...
				if (TypePath.IsNull())
				{
					UE_LOGF(LogAssetTools, Warning, "Failed to convert short type name \"%ls\" to path name. Please update SupportedAssetTypes entries in [AssetTools] ini section", *Type);
				}
				else
				{
	...
	if (CachedVersion == 0 || CachedVersion != Version)
	{
		...
		for (UAssetDefinition* AssetDefinition : AssetDefinitions)
		{
			if (!AssetDefinition->IsA<UAssetDefinition_AssetTypeActionsProxy>())
			{
				...
				// If an asset definition is already registered check that the new one doesn't take precedence over the old one
				if (ActionDefinitionAlreadyProxied)
				{
					if (const int* Index = AssetTypeActionsLookup.Find(AssetClass.Get()))
					{
						TWeakPtr<IAssetTypeActions> TypeActions = AssetTypeActionsList[*Index];
						if (const TSharedPtr<IAssetTypeActions>& ActionPtr = TypeActions.Pin())
						{
							if (ActionPtr->IsAssetDefinitionInDisguise())
							{
								const TSharedPtr<FAssetDefinitionProxy>& ProxyAction = StaticCastSharedPtr<FAssetDefinitionProxy>(ActionPtr);
								if (const UAssetDefinition* ProxyAssetDefinition = ProxyAction->GetAssetDefinition())
								{
									// If the old one can't be registered statically, but the new one can, we should remove the old in favor of the new one
									if (!ProxyAssetDefinition->CanRegisterStatically() && AssetDefinition->CanRegisterStatically())
									{
										// Unregister the old one in favor of the one that can be registered statically
										UnregisterAssetTypeActions(ProxyAction.ToSharedRef());
...
EAssetTypeCategories::Type UAssetToolsImpl::RegisterAdvancedAssetCategory(FName CategoryKey, FText CategoryDisplayName)
{
	...
	if (Result == EAssetTypeCategories::Misc)
	{
		if (NextUserCategoryBit != 0)
		{
			...
			UE_LOGF(LogAssetTools, Verbose, "RegisterAssetTypeCategory(\"%ls\", \"%ls\") used up bit 0x%x, Offset: %d", *CategoryKey.ToString(), *CategoryDisplayName.ToString(), NextUserCategoryBit, FMath::FloorLog2(Result));
		...
		{
			UE_LOGF(LogAssetTools, Warning, "RegisterAssetTypeCategory(\"%ls\", \"%ls\") failed as all user bits have been exhausted (placing into the Misc category instead)", *CategoryKey.ToString(), *CategoryDisplayName.ToString());
		}
	}
...
UObject* UAssetToolsImpl::CreateAsset(const FString& AssetName, const FString& PackagePath, UClass* AssetClass, UFactory* Factory, FName CallingContext, bool bOverwriteExisting)
{
	...
	if ( !CanCreateAsset(AssetName, PackageName, LOCTEXT("CreateANewObject", "Create a new object"), bOverwriteExisting) )
	{
		return nullptr;
	}
...
bool UAssetToolsImpl::CreateAssetAsync(const FString& AssetName, const FString& PackagePath, UClass* AssetClass, UFactory* Factory, FAssetCreateComplete OnComplete, FAssetCreateCancelled OnCancelled, FName CallingContext, bool bOverwriteExisting)
{
	// Verify the factory class
	if (!ensure(AssetClass && Factory))
	{
	...
	return Factory->ConfigurePropertiesAsync(
		FOnFactoryConfigurePropertiesAsyncComplete::CreateUObject(this, &UAssetToolsImpl::CreateAssetAsync_OnComplete, DontGCPackage, ClassToUse, 
			AssetName, Flags, CallingContext, PackageName, OnComplete, OnCancelled),
		FOnFactoryConfigurePropertiesAsyncCancelled()
	);
}
	...
	if ( SourceObjects.Num() == 1 )
	{
		if (UObject* SourceObject = SourceObjects[0])
		{
			if (UFactory* Factory = FactoryConstructor(SourceObject))
			{
				...
				TOptional<FString> DefaultAssetName = GetDefaultAssetNameForClass(CreateAssetType, Factory, SourceObject);

				const FString BasePackageName = DefaultAssetName.Get(SourceObject->GetName());
...
UObject* UAssetToolsImpl::CreateAssetWithDialog(UClass* AssetClass, UFactory* Factory, FName CallingContext)
{
	if (Factory != nullptr)
	{
		...
		// Determine an appropriate name
		TOptional<FString> DefaultAssetName = GetDefaultAssetNameForClass(AssetClass, Factory, nullptr);
		const FString BaseAssetName = DefaultAssetName.Get(Factory->GetDefaultNewAssetName());
	...
	FAssetCreateCancelled OnCancelled, FName CallingContext)
{
	...
	// Determine an appropriate name
	const TOptional<FString> DefaultAssetName = GetDefaultAssetNameForClass(AssetClass, Factory, nullptr);
	const FString BaseAssetName = DefaultAssetName.Get(Factory->GetDefaultNewAssetName());
	...
	FAssetCreateComplete OnComplete, FAssetCreateCancelled OnCancelled, FName CallingContext)
{
	...
	return Factory->ConfigurePropertiesAsync(
		FOnFactoryConfigurePropertiesAsyncComplete::CreateUObject(this, &UAssetToolsImpl::CreateAssetWithDialogAsync_OnComplete,
...
UObject* UAssetToolsImpl::PerformDuplicateAsset(const FString& AssetName, const FString& PackagePath, UObject* OriginalObject, bool bWithDialog)
{
	...
	// Make sure we can create the asset without conflicts
	if ( !CanCreateAsset(AssetName, PackageName, LOCTEXT("DuplicateAnObject", "Duplicate an object"), false) )
	{
		return nullptr;
	}
...
void UAssetToolsImpl::ImportAssetTasks(const TArray<UAssetImportTask*>& ImportTasks)
{
	...
	for (UAssetImportTask* ImportTask : ImportTasks)
	{
		if (!ImportTask)
		{
			UE_LOGF(LogAssetTools, Warning, "ImportAssetTasks() supplied an empty task");
			continue;
		}
...
void UAssetToolsImpl::ExportAssets(const TArray<FString>& AssetsToExport, const FString& ExportPath)
{
	...
	for (const FString& AssetStr : AssetsToExport)
	{
		...
		{
			UE_LOGF(LogAssetTools, Error, "Could not load asset '%ls' to export it", *AssetStr);
		}
	}
...
void UAssetToolsImpl::DiffAssets(UObject* OldAsset, UObject* NewAsset, const struct FRevisionInfo& OldRevision, const struct FRevisionInfo& NewRevision) const
{
	...
	{
		UE_LOGF(LogAssetTools, Warning, "DiffAssets: Both of the supplied assets were nullptr.");
		return;
	}
	...
	{
		UE_LOGF(LogAssetTools, Warning, "DiffAssets: Classes were not the same.");
	}
}
...
FString UAssetToolsImpl::DumpAssetToTempFile(UObject* Asset) const
{
	...
	{
		//UE_LOGF(LogAssetTools, Warning, "DiffAssets: Could not write %ls", *AbsoluteTempFileName);
		return TEXT("");
	}
	else
	{
...
{
	MigratePackages(PackageNamesToMigrate, DestinationPath, TSet<FName>(), TSet<FName>(), Options);
}

void UAssetToolsImpl::MigratePackages(const TArray<FName>& PackageNamesToMigrate, const FString& DestinationPath, TSet<FName> AllDependenciesToMigrate, TSet<FName> ExcludedDependencies, const struct FMigrationOptions& Options) const
{
	...
	{
		auto RunMigratePackages = [this, PackageNamesToMigrate, DestinationPath, Options, AllDependenciesToMigrate = MoveTemp(AllDependenciesToMigrate), ExcludedDependencies = MoveTemp(ExcludedDependencies)]() mutable
			{
				bool bSanitizeInputs = true;
				if (AllDependenciesToMigrate.IsEmpty() && ExcludedDependencies.IsEmpty())
				{
					BuildMigrateDependencies(PackageNamesToMigrate, AllDependenciesToMigrate, ExcludedDependencies);
					bSanitizeInputs = false;
				}
				PerformMigratePackages(PackageNamesToMigrate, DestinationPath, Options, AllDependenciesToMigrate, ExcludedDependencies, bSanitizeInputs);
			};
		...
		if (AssetRegistryModule.Get().IsLoadingAssets())
		{
			...
			SDiscoveringAssetsDialog::OpenDiscoveringAssetsDialog(
				SDiscoveringAssetsDialog::FOnAssetsDiscovered::CreateLambda(RunMigratePackages)
			);
		}
		else
		{
...
TArray<UObject*> UAssetToolsImpl::ImportAssetsInternal(const TArray<FString>& Files, const FString& RootDestinationPath, TArray<TPair<FString, FString>> *FilesAndDestinationsPtr, const FAssetImportParams& Params) const
{
	...
	for(int32 FileIdx = 0; FileIdx < FilesAndDestinations.Num() && !bImportWasCancelled; ++FileIdx)
	{
		...
		if (!FPackageName::TryConvertFilenameToLongPackageName(ObjectTools::SanitizeObjectPath(FilesAndDestinations[FileIdx].Value), DestinationPath, &ErrorMsg))
		{
			...
			UE_LOGF(LogAssetTools, Warning, "%ls", *ErrorMsg);
			UE_LOGF(LogAssetTools, Warning, "%ls", *Message.ToString());
				...
				TFunction<void(UE::Interchange::FImportResult&)> AppendAndBroadcastImportResultIfNeeded =
					...
					[bSceneImport = Params.bSceneImport, ImportStatus, AppendImportResult, bSyncToBrowser](UE::Interchange::FImportResult& Result)
					{
						...
						if (--ImportStatus->ImportCount == 0)
						{
							...
							// Content Browser sync is editor-only behavior and must stay disabled in commandlets.
                            // In non-commandlet contexts, the CVar overrides the passed value when available.
                            const bool bResolvedSyncToBrowser = IsRunningCommandlet() ? false : (Var ? Var->GetBool() : bSyncToBrowser);
		...
		else if(FactoriesPtr)
		{
			...
			if(Factories.Num() > 0)
			{
				...
				for(auto FactoryIt = Factories.CreateConstIterator(); FactoryIt; ++FactoryIt)
				{
					...
					if(TestFactory->FactoryCanImport(Filename))
					{
						if ( Factory != nullptr )
						{
							...
							{
								UE_LOGF(LogAssetTools, Warning, "Two factories registered with same priority : %ls and %ls", *Factory->GetName(), *TestFactory->GetName() );
							}
							break;
						}
				...
				if ( Factory == nullptr )
				{
					...
					UE_LOGF(LogAssetTools, Warning, "Factory did not pass FactoryCanImport, trying anyway : %ls", *Factory->GetName());
				}
			}
		}
		else
		...
		if (bImportWasCancelled)
		{
			UE_LOGF(LogAssetTools, Log, "The import task was canceled.");
		}
	}
...
void UAssetToolsImpl::ExportAssetsInternal(const TArray<UObject*>& ObjectsToExport, bool bPromptIndividualFilenames, bool bSaveAsCleanFilename, const FString& ExportPath) const
{
	...
	for (int32 Index = 0; Index < ObjectsToExport.Num(); Index++)
	{
		...
		else
		{
			...
			UE_LOGF(LogAssetTools, Log, "Exporting \"%ls\" to \"%ls\"", *ObjectToExport->GetPathName(), *SaveFileName);
		}
...
bool UAssetToolsImpl::CanCreateAsset(const FString& AssetName, const FString& PackageName, const FText& OperationText, bool bOverwriteExisting) const
{
	...
	if( ExistingObject != nullptr )
	{
		...
		if (!CanShowDialogs() && !bOverwriteExisting)
		{
			UE_LOGF(LogAssetTools, Error, "The asset '%ls' already exists in package '%ls'. CanCreateAsset cannot ask the user as the application is running unattended and will return false", *AssetName, *PackageName);
			return false;
		}
		
		bool bWantReplace = bOverwriteExisting;
		if (!bWantReplace)
		{
			...
			bWantReplace = ConfirmationResult == 0;
		}
...
void UAssetToolsImpl::BuildMigrateDependencies(const TArray<FName>& PackageNamesToMigrate, TSet<FName>& OutAllDependenciesToMigrate, TSet<FName>& OutExcludedDependencies) const
{
	// Form a full list of packages to move by including the dependencies of the supplied packages
	OutAllDependenciesToMigrate.Reset();
	TSet<FString> ExternalObjectsPaths;
	OutExcludedDependencies.Reset();
	{
		...
		for ( auto PackageIt = PackageNamesToMigrate.CreateConstIterator(); PackageIt; ++PackageIt )
		{
			...
			if (!OutAllDependenciesToMigrate.Contains(*PackageIt) )
			{
				OutAllDependenciesToMigrate.Add(*PackageIt);
				RecursiveGetDependencies(*PackageIt, OutAllDependenciesToMigrate, ExternalObjectsPaths, OutExcludedDependencies, ShouldExcludePackage);
			}
		}
	}
}

void UAssetToolsImpl::PerformMigratePackages(const TArray<FName>& PackageNamesToMigrate, const FString& DestinationPath, const FMigrationOptions& Options, TSet<FName> AllDependenciesToMigrate, TSet<FName> ExcludedDependencies, bool bSanitizeInputs) const
{
	if (bSanitizeInputs)
	{
		TSet<FName> VerifiedAllPackageNamesToMove;
		TSet<FName> VerifiedExcludedDependencies;
		BuildMigrateDependencies(PackageNamesToMigrate, VerifiedAllPackageNamesToMove, VerifiedExcludedDependencies);
		
		// Don't allow migration of packages that are not in the allowed list of verified packages 
		for (auto It = AllDependenciesToMigrate.CreateIterator(); It; ++It)
		{
			if (!VerifiedAllPackageNamesToMove.Contains(*It))
			{
				It.RemoveCurrent();
			}
		}
		
		// Don't allow migration of dependencies that have been excluded in verified list
		for (const FName& Ex : VerifiedExcludedDependencies)
		{
			if (!ExcludedDependencies.Contains(Ex))
			{
				ExcludedDependencies.Add(Ex);
			}
		}
	}
	...
	TMap<FName, FName> ExternalObjectPackageToParentPackage;
	{
		FAssetRegistryModule& AssetRegistryModule = FModuleManager::Get().LoadModuleChecked<FAssetRegistryModule>(TEXT("AssetRegistry"));
		...
		for (const FName& PackageName : AllDependenciesToMigrate)
		{
			...
			if (PackageNameStr.Contains(FPackagePath::GetExternalActorsFolderName()) || PackageNameStr.Contains(FPackagePath::GetExternalObjectsFolderName()))
			{
				...
				if (AssetRegistryModule.Get().GetAssetsByPackageName(PackageName, Assets, bOnlyIncludeOnDiskAssets))
				{
					for (const FAssetData& AssetData : Assets)
					{
						if (!AssetData.GetOptionalOuterPathName().IsNone())
						{
							const FName OuterPackageName = FSoftObjectPath(AssetData.GetOptionalOuterPathName().ToString()).GetLongPackageFName();
							PackageToExternalObjectPackages.FindOrAdd(OuterPackageName).Add(PackageName);
							
							if (bSanitizeInputs)
							{
								ExternalObjectPackageToParentPackage.Add(PackageName, OuterPackageName);
							}
						}
					}
				}
		...
		AllDependenciesToMigrate = FilteredPackageNamesToMove;
	}

	// Confirm that there is at least one package to move 
	if (AllDependenciesToMigrate.Num() == 0)
	{
		if (CanShowDialogs())
		{
	...
	TSet<FName> SelectedParents;
	TSharedPtr<TArray<ReportPackageData>> ReportPackages = MakeShareable(new TArray<ReportPackageData>);
	for (auto PackageIt = AllDependenciesToMigrate.CreateConstIterator(); PackageIt; ++PackageIt)
	{
		const FName PackageName = *PackageIt;
		bool bShouldMigratePackage = ShouldMigratePackage.Find(PackageName) != nullptr;
		
		if (bSanitizeInputs && bShouldMigratePackage)
		{
			// For each external object/actor if their parent is not being migrated, then they shouldn't either
			if (const FName* ParentPackageName = ExternalObjectPackageToParentPackage.Find(PackageName))
			{
				if (!ShouldMigratePackage.Contains(*ParentPackageName))
				{
					bShouldMigratePackage = false;
				}
			}
		...
		ReportPackages.Get()->Add({ PackageName.ToString(), bShouldMigratePackage });
	}
	
	if (bSanitizeInputs)
	{
		// Clean PackageToExternalObjectPackages to keep only the selected Package parents
		for (auto It = PackageToExternalObjectPackages.CreateIterator(); It; ++It)
		{
			if (!ShouldMigratePackage.Contains(It.Key()))
			{
				It.RemoveCurrent();
			}
		}
...
void UAssetToolsImpl::MigratePackages_ReportConfirmed(TSharedPtr<TArray<ReportPackageData>> PackageDataToMigrate, const FString DestinationPath, TSet<FName> ExcludedDependencies, TMap<FName, TSet<FName>> PackageToExternalObjectPackages, const FMigrationOptions Options) const
{
	...
	if (!CanShowDialogs() || !DestinationPath.IsEmpty())
	{
		...
		{
			UE_LOGF(LogAssetTools, Error, "Migration Destination path cannot be empty.");
			return;
		}
		...
		if (!MountPointRootPath.IsEmpty())
		{
			...
			if (WritableFolderPermissionList->HasFiltering())
			{
				if (FPathViews::IsParentPathOf(FPaths::ProjectDir(), DestinationFolder))
				{
					...
					{
						UE_LOGF(LogAssetTools, Error, "Migration Destination path is not a writable folder.");
						return;
					}
				}
				else
	...
	else
	{
		...
		if (!LostPackages.IsEmpty())
		{
			...
			if (IFileManager::Get().DirectoryExists(*SrcDiskFolderFilename))
			{
				...
				{
					UE_LOGF(LogAssetTools, Error, "%ls", *Message.ToString());
				}
				return;
			}
...

		{
			...
			for ( const FName& PackageNameToMove : AllPackageNamesToMove )
			{
				...
				if (!FPackageName::DoesPackageExist(PackageName, &SrcFilename))
				{
					...
					UE_LOGF(LogAssetTools, Warning, "%ls", *ErrorMessage.ToString());
				...
				else
				{
					...
					if ( bFileOKToCopy )
					{
						...
						{
							UE_LOGF(LogAssetTools, Warning, "Failed to copy %ls to %ls while migrating assets", *SrcFilename, *DestFilename);
		...
		if (!LostPackages.IsEmpty())
		{
			...
			{
				UE_LOGF(LogAssetTools, Warning, "Failed to delete temporary directory %ls while migrating assets", *SrcDiskFolderFilename);
...
bool UAssetToolsImpl::IsAssetClassSupported(const UClass* AssetClass) const
{
	...
	{
		const TSharedRef<FPathPermissionList>& CreateAssetPermissionList = GetAssetClassPathPermissionList(EAssetClassAction::CreateAsset);
		if (CreateAssetPermissionList->HasFiltering())
		{
			// Use the Real AssetClass passed from the Factory and not the one from AssetDefinition.
			// This way we avoid issue where the real AssetClass falls back to use the AssetDefinition of its parent.
			FNameBuilder AssetClassPath;
			AssetClass->GetClassPathName().AppendString(AssetClassPath);
			return CreateAssetPermissionList->PassesFilter(AssetClassPath.ToView());	
		}
...
TOptional<FString> UAssetToolsImpl::GetDefaultAssetNameForClass(const UClass* Class, const UFactory* Factory, const UObject* SourceObject)
{
	...
	if (const FAssetNamingConventionEntry* NamingConvention = GetDefault<UAssetToolsEditorSettings>()->AssetNamingConventionOverrides.Find(Class))
	{
		DefaultAssetName = FString::Format(TEXT("{0}{1}{2}"),
			{
				NamingConvention->Prefix.Get(TEXT("")),
				DefaultAssetName.IsEmpty() ? NamingConvention->AssetName.Get(Factory ? Factory->GetDefaultNewAssetName() : TEXT("")) : DefaultAssetName,
				NamingConvention->Suffix.Get(TEXT(""))
			});
	}
	...
	DefaultAssetName = DefaultAssetName.Replace(TEXT("{source}"), SourceObject ? *SourceObject->GetName() : TEXT(""), ESearchCase::IgnoreCase);

	SanitizeName(DefaultAssetName);

	FString SanitizedDefaultName = ObjectTools::SanitizeInvalidChars(DefaultAssetName, INVALID_LONGPACKAGE_CHARACTERS INVALID_OBJECTNAME_CHARACTERS);

	FText ErrorText;
	if (!FFileHelper::IsFilenameValidForSaving(SanitizedDefaultName, ErrorText))
	{
		UE_LOGF(LogAssetTools, Warning, "The default name '%ls' for class '%ls' is invalid: %ls", *SanitizedDefaultName, *Class->GetName(), *ErrorText.ToString());
		return TOptional<FString>();
	}

	return SanitizedDefaultName;
}
...
bool UAssetToolsImpl::CanAssetBePublic(FStringView AssetPath, FText* DisallowReason) const
{
	for (const TPair<FName, UE::AssetTools::FCanAssetBePublic>& Pair : CanAssetBePublicDelegates)
	{
		if (Pair.Value.IsBound())
		{
			if (!Pair.Value.Execute(AssetPath, DisallowReason))
			{
				return false;
			}
		}
