...
{
	FDeleteFolderParameters Options;
	Options.bShowConfirmationBeforeLoadingAssets = true;
	Options.PathsToDelete = PathsToDelete;
	return DeleteFolders(Options);
}

bool AssetViewUtils::DeleteFolders(const FDeleteFolderParameters& InOptions)
{
	// Get a list of assets in the paths to delete
	TArray<FAssetData> AssetDataList;
	GetAssetsInPaths(InOptions.PathsToDelete, AssetDataList);
	...
	{
		// There were no assets found, allow the empty folders deletion without any confirmation
		ObjectTools::FDeleteItemsParameters Options;
		Options.FoldersToDelete = InOptions.PathsToDelete;
		Options.bShowConfirmation = false;
		bSuccess = ObjectTools::DeleteItems(Options).FoldersDeleted == InOptions.PathsToDelete.Num();
	}
	else
	{
		if (InOptions.bShowConfirmationBeforeLoadingAssets)
		{
			FText Prompt;
			if (InOptions.PathsToDelete.Num() == 1)
			{
				Prompt = FText::Format(LOCTEXT("FolderDeleteConfirm_Single", "Delete folder '{0}'?"), FText::FromString(InOptions.PathsToDelete[0]));
			}
			else
			{
				Prompt = FText::Format(LOCTEXT("FolderDeleteConfirm_Multiple", "Delete {0} folders?"), FText::AsNumber(InOptions.PathsToDelete.Num()));
			}

			// If we had any folders selected, ask the user whether they want to delete them 
			// as it can be slow to build the deletion dialog on an accidental click (due to assets being loaded)
			if (FMessageDialog::Open(EAppMsgType::YesNo, Prompt, LOCTEXT("FolderDeleteConfirmTitle", "Delete")) != EAppReturnType::Type::Yes)
			{
				return false;
			}
		}

		// Load all the assets in the folder and attempt to delete them. If it was successful, allow the folder delete.
		...
		if (LoadAssetsIfNeeded(AssetDataList, LoadedAssets, Settings) == ELoadAssetsResult::Success)
		{
			// Make sure loaded assets match with pending deletion assets
			bool bProcessDelete = LoadedAssets.Num() == NumAssetsInPaths;

			if (!bProcessDelete)
			{
				// Add any missing object contained within the same package from AssetDataList like blueprint generated class
				TSet<UObject*> LoadedAssetsSet(LoadedAssets);
				for (const FAssetData& Asset : AssetDataList)
				{
					if (UObject* Object = Asset.FastGetAsset(/** Load */false))
					{
						LoadedAssetsSet.Add(Object);
					}
				}

				LoadedAssets = LoadedAssetsSet.Array();
				bProcessDelete = LoadedAssets.Num() == NumAssetsInPaths;
			}

			if (bProcessDelete)
			{
				ObjectTools::FDeleteItemsParameters Options;
				Options.ObjectsToDelete = LoadedAssets;
				Options.FoldersToDelete = InOptions.PathsToDelete;
				ObjectTools::AddExtraObjectsToDelete(Options.ObjectsToDelete);
				ObjectTools::FDeleteItemsResult Result = ObjectTools::DeleteItems(Options);
				bSuccess = Result.FoldersDeleted == Options.FoldersToDelete.Num()
					&& Result.ObjectsDeleted == Options.ObjectsToDelete.Num();
			}
			else
			{
				// Not all the assets in the selected paths were loaded
				UE_LOGF(LogAssetViewTools, Warning, "Cannot load some assets for deletion operation (%i vs %i) cannot proceed...", LoadedAssets.Num(), NumAssetsInPaths);
			}
		}
		else
		{
...
bool AssetViewUtils::RenameFolder(const FString& DestPath, const FString& SourcePath)
{
	...
	FOnPreDestructiveAssetActionArgs PreDestructiveArgs(ObjectsInFolder, EDestructiveAssetActions::AssetRename);
	FEditorDelegates::OnPreDestructiveAssetActionWithArgs.Broadcast(PreDestructiveArgs);
	if (!PreDestructiveArgs.Result.bSuccess)
	{
		UE_LOGF(LogAssetViewTools, Warning, "%ls", *PreDestructiveArgs.Result.ErrorMessage);
		return false;
	}
...
bool AssetViewUtils::MoveFolders(const TArray<FString>& InSourcePathNames, const FString& DestPath)
{
	...
	TArray<FTopLevelAssetPath> Destinations;
	for (auto PathIt = SourcePathToLoadedAssets.CreateConstIterator(); PathIt; ++PathIt)
	{
		AssetsToMove.Append(PathIt.Value());

		const FString& SourcePath = PathIt.Key();
		const FStringView SourcePathPrefix = FPathViews::GetPath(SourcePath);

		for (UObject* Obj : PathIt.Value())
		{
			FSoftObjectPath SourceObjectPath(Obj);
			FNameBuilder SourcePackagePath(SourceObjectPath.GetLongPackageFName());
			FStringView RelativePath = SourcePackagePath.ToView().RightChop(SourcePathPrefix.Len() + 1);

			FNameBuilder DestPackagePath;
			DestPackagePath.Append(DestPath);
			FPathViews::AppendPath(DestPackagePath, RelativePath);

			FTopLevelAssetPath Dest(FName(DestPackagePath.ToView()), SourceObjectPath.GetAssetFName());
			Destinations.Add(Dest);
		}
	}

	FOnPreDestructiveAssetActionArgs PreDestructiveArgs(AssetsToMove, EDestructiveAssetActions::AssetMove, Destinations);
	FEditorDelegates::OnPreDestructiveAssetActionWithArgs.Broadcast(PreDestructiveArgs);
	if (!PreDestructiveArgs.Result.bSuccess)
	{
		UE_LOGF(LogAssetViewTools, Warning, "%ls", *PreDestructiveArgs.Result.ErrorMessage);
...
bool AssetViewUtils::IsValidObjectPathForCreate(const FString& ObjectPath, const UClass* ObjectClass, FText& OutErrorMessage, bool bAllowExistingAsset)
{
	...
	{
		OutErrorMessage = FText::Format(LOCTEXT("AssetNameTooLong", "The name for the asset is too long, the maximum is '{0}'. \nPlease choose a shorter name for the asset or create it in a shallower folder structure."),
			FText::AsNumber(NAME_SIZE - 1));
		// Return false to indicate that the user should enter a new name
		return false;
	}
...
bool AssetViewUtils::IsValidFolderPathForCreate(const FString& InFolderPath, const FString& NewFolderName, FText& OutErrorMessage, bool bAllowExistingFolder)
{
	...
	// Make sure we are not creating an FName that is too large
	if (NewFolderPath.Len() >= NAME_SIZE)
	{
		OutErrorMessage = FText::Format(LOCTEXT("PackagePathTooLong", "The path for the folder is too deep, the maximum is '{0}'. \nPlease choose a shorter name for the folder or create it in a shallower folder structure."),
			FText::AsNumber(NAME_SIZE - 1));
		// Return false to indicate that the user should enter a new name
		return false;
	}
