...
namespace UE::AssetTools
{
	...
	DECLARE_DELEGATE_RetVal_TwoParams(bool, FCanAssetBePublic, FStringView /*AssetPath*/, FText* /*DisallowReason*/);
...
public:
	...
	*/
	 ...
	 * @param bOverwriteExisting if set to true, this will overwrite an existing object without prompting
	 * @return the new asset or NULL if it fails
	 */
	UFUNCTION(BlueprintCallable, Category = "Editor Scripting | Asset Tools")
	virtual UObject* CreateAsset(const FString& AssetName, const FString& PackagePath, UClass* AssetClass, UFactory* Factory, FName CallingContext = NAME_None, bool bOverwriteExisting = false) = 0;
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Editor Scripting | Asset Tools")
	virtual bool CreateAssetAsync(const FString& AssetName, const FString& PackagePath, UClass* AssetClass, UFactory* Factory, FAssetCreateCompleteDynamic OnComplete, FAssetCreateCancelledDynamic OnCancelled, FName CallingContext = NAME_None, bool bOverwriteExisting = false)
	{
	...
	virtual bool CreateAssetAsync(const FString& AssetName, const FString& PackagePath, UClass* AssetClass, UFactory* Factory, FAssetCreateComplete OnComplete = FAssetCreateComplete(), FAssetCreateCancelled OnCancelled = FAssetCreateCancelled(), FName CallingContext = NAME_None, bool bOverwriteExisting = false) = 0;
	...
	*/
	...
	/* Migrate packages and dependencies to another folder */
	virtual void MigratePackages(const TArray<FName>& PackageNamesToMigrate, const FString& DestinationPath, TSet<FName> AllDependenciesToMigrate, TSet<FName> ExcludedDependencies, const struct FMigrationOptions& Options = FMigrationOptions()) const = 0;

	/* Build dependency packages and excluded dependencies from a list of packages to migrate */
	virtual void BuildMigrateDependencies(const TArray<FName>& PackageNamesToMigrate, TSet<FName>& OutAllDependenciesToMigrate, TSet<FName>& OutExcludedDependencies) const = 0;
	...
	*/
	...
	virtual TOptional<FString> GetDefaultAssetNameForClass(const UClass* Class, const UFactory* Factory = nullptr, const UObject* SourceObject = nullptr) = 0;

	/** Get the default name for the supplied class (if one is registered) to use when creating a new asset of that type from the input source object */
	UE_DEPRECATED(5.8, "Use GetDefaultAssetNameForClass and pass a source object")
	virtual TOptional<FString> GetDefaultAssetNameForClassFromSource(const UClass* Class, const UObject* SourceObject, const UFactory* Factory = nullptr) final
	{
		return GetDefaultAssetNameForClass(Class, Factory, SourceObject);
	}
	...
	virtual bool CanAssetBePublic(FStringView AssetPath, FText* DisallowReason = nullptr) const = 0;
