...
static FVector4f ComputeAlphaCoverage(const FVector4f Thresholds, const FVector4f Scales, const FImageView2D& SourceImageData)
{
	...
	if ( Thresholds[0] == 0.f && Thresholds[1] == 0.f && Thresholds[2] == 0.f )
	{
		...
		UE_LOGF(LogTextureCompressor, VeryVerbose, "Thresholds = 000 %f Coverage = 000 %f",  \
	...
	else
	{
		...
		UE_LOGF(LogTextureCompressor, VeryVerbose, "Thresholds = %f %f %f %f Coverage = %f %f %f %f",  \
...
static FVector4f ComputeAlphaScale(const FVector4f Coverages, const FVector4f AlphaThresholds, const FImageView2D& SourceImageData)
{
	...
	for (int32 i = 0; i < 8; ++i)
	{
		...
		UE_LOGF(LogTextureCompressor, VeryVerbose, "Tried AlphaScale = %f ComputedCoverage = %f Goal = %f", AlphaScales[3], ComputedCoverages[3], Coverages[3] );
	...
	UE_LOGF(LogTextureCompressor, VeryVerbose, "Final AlphaScales = %f %f %f %f", AlphaScales[0], AlphaScales[1], AlphaScales[2], AlphaScales[3] );
	...
	*/
	...
	if ( Settings.bUseNewMipFilter )
	{
		...
		else
		{
			...
			UE_LOGF(LogTextureCompressor, Verbose, "Not ideal: Heterogeneous XY Wrap/Clamp will generate mips with Wrap on both dimensions" );
		}
	}
...
static void GenerateTopMip(const FImage& SrcImage, FImage& DestImage, const FTextureBuildSettings& Settings)
{
	...
	{	
		...
		UE_LOGF(LogTextureCompressor, Warning, "GenerateTopMip used with non-Gaussian blur filter will cause half pixel shift");
...
static void DownscaleImage(const FImage& SrcImage, FImage& DstImage, const FTextureDownscaleSettings& Settings)
{
	...
	if ( Settings.UseNewMipFilter )
	{
		...
		default:
			UE_LOGF(LogTextureCompressor, Warning, "Downscale option not recognized : %d",(int)Settings.DownscaleOptions);
			Filter = FImageCore::EResizeImageFilter::CubicMitchell;
			break;
		}
...
static void LinearizeToWorkingColorSpace(const FImage& SrcImage, FImage& DstImage, const FTextureBuildSettings& BuildSettings)
{
	...
	if (SourceEncodingOverride != UE::Color::EEncoding::None || BuildSettings.bHasColorSpaceDefinition)
	{
		...
		if (!bSuccess)
		{
			UE_LOGF(LogTextureCompressor, Error, "Failed to linearize the texture to the working color space.");
		}
	}
}
...
void ITextureCompressorModule::GenerateAngularFilteredMips(TArray<FImage>& InOutMipChain, int32 NumMips, uint32 DiffuseConvolveMipLevel)
{
	...
	for (int32 i = 0; i < NumMips; ++i)
	{
		...
		UE_LOGF(LogTextureCompressor, Verbose, "GenerateAngularFilteredMips  %f %f %f %f %f", NormalizedMipLevel, AdjustedMipLevel, NormalizedWidth, TexelSize, ConeAngle * 180 / PI);
...
static bool ApplyCompositeTexture(FImage& DestRoughness, const FImage& SourceNormals, uint8 CompositeTextureMode, float CompositePower)
{
	...
	switch((ECompositeTextureMode)CompositeTextureMode)
	{
		...
		case CTM_NormalRoughnessToAlpha:
			...
			break;
		default:
			UE_LOGF(LogTextureCompressor, Error, "Invalid CompositeTextureMode");
			return false;
	}
	...
	else
	{
		...
		{
			UE_LOGF(LogTextureCompressor, Warning, "CompositeTexture XY sizes don't match, stretch not supported because slice count is not 1 : %d vs %d",
	...
	uint32& OutExtData)
{
	...
	UE_LOGF(LogTextureCompressor,Verbose,"Compressed %dx%dx%d %ls in %fms",
		MipChain[0].SizeX,
		MipChain[0].SizeY,
	...
	int32& OutMip0SizeX, int32& OutMip0SizeY, int32& OutMip0NumSlices, int32& OutMipCount) const
{
	...
	if (this->bLongLatSource == false)
	{
		...
		if (this->Downscale > 1.0f)
		{
			...
			if (this->bVolume)
			{
				UE_LOGF(LogTextureCompressor, Error, "Downscaling volumes not yet supported - should have been handled in GetTextureBuildSettings!");
				return false;
			}
...
public:
	...
	)
	{
		...
		{
			UE_LOGF(LogTextureCompressor, Warning,
				"Failed to find compressor for texture format '%ls'. [%.*ls]",
		...
		if (AssociatedNormalSourceMips.Num())
		{
			...
			if (!BuildTextureMips(AssociatedNormalSourceMips, DefaultSettings, true, bDoDetailedAlphaLogging ? &AlphaLogs : nullptr, IntermediateAssociatedNormalSourceMipChain, DebugTexturePathName))
			{
				...
				UE_LOGF(LogTextureCompressor, Warning, "Failed to generate texture mips for composite texture [%.*ls]",
					DebugTexturePathName.Len(),DebugTexturePathName.GetData());
			...
			if (!ApplyCompositeTextureToMips(IntermediateMipChain, IntermediateAssociatedNormalSourceMipChain, BuildSettings.CompositeTextureMode, BuildSettings.CompositePower, BuildSettings.LODBias))
			{
				...
				UE_LOGF(LogTextureCompressor, Display, "ApplyCompositeTextureToMips failed [%.*ls]",
					DebugTexturePathName.Len(),DebugTexturePathName.GetData());
		...
		if (!bKnowAlphaTransparency)
		{
			...
			if (FormatWithAlpha != FormatWithoutAlpha)
			{
				...
				UE_LOGF(LogTextureCompressor, Warning, "Scanning intermediate mips for alpha: %.*ls", DebugTexturePathName.Len(), DebugTexturePathName.GetData());
		...
		if (bDoDetailedAlphaLogging)
		{
				...
				BuildSettings.GetTextureExpectsAlphaInPixelFormat(bIntermediateMipsAlphaDetected) != bSourceMipsExpectAlpha)
			{
				...
				UE_LOGF(LogTextureCompressor, Warning, "%ls", *AlphaLogs);
			}
			else
			{
				UE_LOGF(LogTextureCompressor, Display, "%ls", *AlphaLogs);
			}
		}
		...
		if ( bCompressSucceeded )
		{
			...
			for(int32 MipIndex=0;MipIndex < OutTextureMips.Num();MipIndex++)
			{
				...
				UE_LOGF(LogTextureCompressor, Verbose, "Built texture: [%.*ls] Compressed Mip %d PF=%d=%ls : %dx%dx%d : CompressedDataSize=%lld",
					DebugTexturePathName.Len(),DebugTexturePathName.GetData(),
...
private:
		...
		FStringView DebugTexturePathName)
	{
		...
		if ( PowerOfTwoMode != ETexturePowerOfTwoSetting::None )
		{
			...
			if (bPadOrStretchTexture)
			{
				if (BuildSettings.MipGenSettings == TMGS_LeaveExistingMips)
				{
					...
					UE_LOGF(LogTextureCompressor, Error,	"Texture padding or resizing is not allowed when leaving existing mips.");
					return false;
				}
		...
		if ( ! bLongLatCubemap )
		{
			...
			if ( StartMip == NumSourceMips )
			{
				...
				UE_LOGF(LogTextureCompressor, Verbose,
					"Source image %dx%d too large for compressors max dimension (%d). Resizing.",
					BaseImage.SizeX,
					BaseImage.SizeY,
		...
		{
			UE_LOGF(LogTextureCompressor, Error, "Texture %.*ls generated unexpected mip chain: GetMipCountForBuildSettings expected %d mips, %dx%dx%d, got %d mips, %dx%dx%d!", 
				DebugTexturePathName.Len(), DebugTexturePathName.GetData(), 
	...
	bool ApplyCompositeTextureToMips(TArray<FImage>& DestRoughnessMips, const TArray<FImage>& NormalSourceMips, uint8 CompositeTextureMode, float CompositePower, int32 DestLODBias)
	{
		...
		for(int32 DestLevel = 0; DestLevel < DestRoughnessMips.Num(); ++DestLevel)
		{
			...
			{
				UE_LOGF(LogTextureCompressor, Display, 
					"ApplyCompositeTexture: Couldn't find matching mip size, will stretch.  (dest: %dx%d with %d mips, source: %dx%d with %d mips).  current: (dest: %dx%d at %d, source: %dx%d at %d)",
