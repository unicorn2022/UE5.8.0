...
static void PlatformInitPixelFormatForDevice(HDC DeviceContext)
{
	...
	{
		UE_LOGF(LogOpenGLShaderCompiler, Fatal,"Failed to set pixel format for device context.");
	}
}
...
PRAGMA_DISABLE_CAST_FUNCTION_TYPE_MISMATCH_WARNINGS
		...
		if (glCreateShader == NULL)
		{
			...
			#define CHECK_GL_ENTRYPOINTS(Type,Func) if (Func == NULL) { bFoundAllEntryPoints = false; UE_LOGF(LogOpenGLShaderCompiler, Warning, "Failed to find entry point for %ls", TEXT(#Func)); }
...
static void PlatformInitOpenGL(void*& ContextPtr, void*& PrevContextPtr, int InMajorVersion, int InMinorVersion)
{
	...
	if (!bInitialized)
	{
		...
		{
			UE_LOGF(LogOpenGLShaderCompiler, Fatal, "Unable to dynamically load libGL: %ls", ANSI_TO_TCHAR(SDL_GetError()));
		}
		...
		if (glCreateShader == nullptr)
		{
			...
			// Check that all of the entry points have been initialized.
			bool bFoundAllEntryPoints = true;
			#define CHECK_GL_ENTRYPOINTS(Type,Func) if (Func == nullptr) { bFoundAllEntryPoints = false; UE_LOGF(LogOpenGLShaderCompiler, Warning, "Failed to find entry point for %ls", TEXT(#Func)); }
			ENUM_GL_ENTRYPOINTS(CHECK_GL_ENTRYPOINTS);
			checkf(bFoundAllEntryPoints, TEXT("Failed to find all OpenGL entry points."));
		}
		...
		{
			UE_LOGF(LogOpenGLShaderCompiler, Fatal, "Failed to set GL major version: %ls", ANSI_TO_TCHAR(SDL_GetError()));
		}

		if	(SDL_GL_SetAttribute( SDL_GL_CONTEXT_MINOR_VERSION, InMinorVersion))
		{
			UE_LOGF(LogOpenGLShaderCompiler, Fatal, "Failed to set GL minor version: %ls", ANSI_TO_TCHAR(SDL_GetError()));
		}

		if	(SDL_GL_SetAttribute( SDL_GL_CONTEXT_FLAGS, SDL_GL_CONTEXT_FORWARD_COMPATIBLE_FLAG))
		{
			UE_LOGF(LogOpenGLShaderCompiler, Fatal, "Failed to set GL flags: %ls", ANSI_TO_TCHAR(SDL_GetError()));
		}

		if	(SDL_GL_SetAttribute( SDL_GL_CONTEXT_PROFILE_MASK, SDL_GL_CONTEXT_PROFILE_CORE))
		{
			UE_LOGF(LogOpenGLShaderCompiler, Fatal, "Failed to set GL mask/profile: %ls", ANSI_TO_TCHAR(SDL_GetError()));
		}
		...
		{
			UE_LOGF(LogOpenGLShaderCompiler, Fatal, "OpenGL %d.%d not supported by driver", InMajorVersion, InMinorVersion);
			return;
		}
	...
	GLSLVersion Version)
{
	...
	{
		UE_LOGF(LogOpenGLShaderCompiler, Error, "Bad hlslcc header found");
	}

	if (*USFSource != '#')
	{
		UE_LOGF(LogOpenGLShaderCompiler, Error, "Bad hlslcc header found! Missing '#'!");
	}
	...
	case SF_Compute:
		...
		break;
	default:
		UE_LOGF(LogOpenGLShaderCompiler, Fatal, "Invalid shader frequency: %d", (int32)Frequency);
	}
	...
	for (auto& PackedUB : CCHeader.PackedUBs)
	{
		...
		HandleReflectedUniformBuffer(ShaderInput, PackedUB.Attribute.Name, PackedUB.Attribute.Index, ShaderOutput);
		Header.Bindings.NumUniformBuffers++;
 ...
 */
static void PrecompileShaderInternal(FShaderCompilerOutput& ShaderOutput, const FShaderCompilerInput& ShaderInput, const FShaderParameterParser& ShaderParameterParser, const ANSICHAR* ShaderSource, GLSLVersion Version)
{
							...
							spv_reflect::ShaderModule& Reflection,	const ANSICHAR* SPIRV_DummySamplerName, const EShaderFrequency Frequency, bool bEmulatedUBs)
{
	...
	// The GLSL backend shares texture slots between SRV and UAV, so we need more than 32 slots.
	// Exceeding the SRV limit of 32 samplers should not be handled by this function, so use 64 bit.
	uint64 TextureIndices = 0xffffffffffffffffllu;
	...
	for (auto const& Binding : ReflectionBindings.TBufferSRVs)
	{
		...
		uint32 Index = static_cast<uint32>(FPlatformMath::CountTrailingZeros64(TextureIndices));
...
static void ConvertToEmulatedUBs(std::string& GlslSource, const ReflectionData& ReflectData, const EShaderFrequency Frequency)
{
	...
	for (const auto& PackedUBNamePair : ReflectData.PackedUBNames)
	{
		...
		if (std::regex_search(GlslSource, RegexMatch, std::regex(UBSearchString)) && !RegexMatch.empty())
		{
			UBSearchString = RegexMatch[0];
		}
...
bool GenerateGlslShader(std::string& OutString, GLSLCompileParameters& GLSLCompileParams, ReflectionData& ReflectData, bool bWriteToCCHeader, bool bIsDeferred, bool bEmulatedUBs)
{
	...
	// Fixup packed globals
	{
		...
		std::smatch RegexMatch;
		if (std::regex_search(OutString, RegexMatch, std::regex(GlobalsSearchString)) && !RegexMatch.empty())
		{
			GlobalsSearchString = RegexMatch[0];
		}
	...
	char*&						OutGlslShaderSource)
{
	...
	if(!bCompilationFailed && !Options.bDisableOptimizations && (Version == GLSL_ES3_1_ANDROID || Version == GLSL_150_ES3_1))
	{
		...
		{
			UE_LOGF(LogOpenGLShaderCompiler, Error, "Failed to apply reduce-const-array-to-struct for Android");
			return false;
		} 
	}
	...
	if (!bCompilationFailed && (Version == GLSL_ES3_1_ANDROID || Version == GLSL_150_ES3_1))
	{
		...
		if (!CompilerContext.OptimizeSpirv(SpirvData, OptArgs, UE_ARRAY_COUNT(OptArgs)))
		{
			UE_LOGF(LogOpenGLShaderCompiler, Error, "Failed to apply interface-variable-scalar-replacement for Android");
			return false;
		}
	}
...
inline bool OpenGLSupportsShaderFrequency(EShaderFrequency InFrequency)
{
	return InFrequency == SF_Vertex
		|| InFrequency == SF_Pixel
		|| InFrequency == SF_Geometry
		|| InFrequency == SF_Compute;
}
 ...
 */
void CompileOpenGLShader(const FShaderCompilerInput& Input, const FShaderPreprocessOutput& InPreprocessOutput, FShaderCompilerOutput& Output, GLSLVersion Version)
{
	...
	if (!OpenGLSupportsShaderFrequency(Frequency))
	{
	...
	if (bCompilationSucceeded)
	{
		...
		PrecompileShaderInternal(Output, Input, ShaderParameterParser, GlslShaderSource, Version);
#else // VALIDATE_GLSL_WITH_DRIVER
		int32 SourceLen = FCStringAnsi::Strlen(GlslShaderSource);
		Output.Target = Input.Target;
		BuildShaderOutputInternal(Output, Input, ShaderParameterParser, GlslShaderSource, SourceLen, Version);
