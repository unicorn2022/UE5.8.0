...
void FIOSTargetSettingsCustomization::CustomizeDetails(IDetailLayoutBuilder& DetailLayout)
{
	...
	UE::ShaderPlatformConfigEditor::AddShaderPlatformConfigsFromSettingsModule(DetailLayout, "IOSTargetPlatformSettings");
}
...
static void OnOutput(FString Message)
{
	...
	OutputMessage += "\n";
	UE_LOGF(LogTemp, Log, "%ls", *Message);
}
...
/*	ProvisionCategory.AddCustomRow(TEXT("Certificate Request"), false)
    ...
    ProvisionCategory.AddCustomRow(LOCTEXT("ModernXcodeSigningLabel", "XcodeSigning"), false)
        ...
        [
            SNew( STextBlock )
            .Text( LOCTEXT( "ModernXcodeSigningText", "Code signing is handled by \"Xcode Projects\" section") )
            .AutoWrapText( true )
        ];
...
#define SETUP_STATUS_PROP(PropName, Category) \
	...
	FSimpleDelegate OnEnableMetalMobileSM5 = FSimpleDelegate::CreateSP(this, &FIOSTargetSettingsCustomization::UpdateMetalMobileWarning);
	FSimpleDelegate OnEnableMetalMobileSM6 = FSimpleDelegate::CreateSP(this, &FIOSTargetSettingsCustomization::UpdateMetalMobileWarning);
	...
	MRTPropertyHandle = DetailLayout.GetProperty(GET_MEMBER_NAME_CHECKED(UIOSRuntimeSettings, bSupportsMetalMobileSM5));
	MRTPropertyHandle->SetOnPropertyValueChanged(OnEnableMetalMobileSM5);
	RenderCategory.AddProperty(MRTPropertyHandle);

	SM6PropertyHandle = DetailLayout.GetProperty(GET_MEMBER_NAME_CHECKED(UIOSRuntimeSettings, bSupportsMetalMobileSM6));
	SM6PropertyHandle->SetOnPropertyValueChanged(OnEnableMetalMobileSM6);
	RenderCategory.AddProperty(SM6PropertyHandle);
...
void FIOSTargetSettingsCustomization::FindRequiredFiles()
{
	...
	FString IPPPath = FPaths::ConvertRelativePathToFull(FPaths::EngineDir() / TEXT("Binaries/DotNET/IOS/IPhonePackagerCLI.dll"));
...
FReply FIOSTargetSettingsCustomization::OnInstallProvisionClicked()
{
	...
	if ( bOpened )
	{
		...
		FString Path;
#if PLATFORM_MAC
		Path = FString::Printf(TEXT("%s/Library/%s/Provisioning Profiles"), *FMacPlatformMisc::GetEnvironmentVariable(TEXT("HOME")), FPlatformMisc::XCodeVersionCompare(16, 0, 0) < 0 ? TEXT("MobileDevice") : TEXT("Developer/Xcode/UserData"));
		FString Destination = FString::Printf(TEXT("\"%s/%s.mobileprovision\""), *Path, *DestName);
		FString Destination2 = FString::Printf(TEXT("\"%s/%s.mobileprovision\""), *Path, FApp::GetProjectName());
#else
...
FReply FIOSTargetSettingsCustomization::OnGenerateSSHKey(bool IsPrimary)
{
	...
	{
		UE_LOGF(LogIOSTargetSettings, Error, "cwRsync is not installed correctly");
	}
...
void FIOSTargetSettingsCustomization::UpdateOSVersionWarning()
{
	...
	{
		bool bSM5Enabled = false;
		bool bSM6Enabled = false;
		MRTPropertyHandle->GetValue(bSM5Enabled);
		if (SM6PropertyHandle.IsValid())
		{
			SM6PropertyHandle->GetValue(bSM6Enabled);
		}

		if (bSM5Enabled || bSM6Enabled)
		{
...
void FIOSTargetSettingsCustomization::UpdateMetalMobileWarning()
{
	if (MRTPropertyHandle.IsValid() && ShaderVersionPropertyHandle.IsValid() && MinOSPropertyHandle.IsValid())
	{
		bool bSM5Enabled = false;
