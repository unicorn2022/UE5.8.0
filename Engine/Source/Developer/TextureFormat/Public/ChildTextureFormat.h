...
public:
	...
	) const
	{
		...
		{
			UE_LOGF(LogTemp, Error, "Failed to compress with base compressor [format %ls]", *BaseSettings.TextureFormatName.ToString());
			return false;
		}
		return true;
	}
	...
	) const
	{
		...
		{
			UE_LOGF(LogTemp, Error, "Failed to compress with base tiled compressor [format %ls]", *BaseSettings.TextureFormatName.ToString());
			return false;
		}
		return true;
	}
