...
public:
	...
	void UpdateTextureFormatList()
	{
		...
		if ( bForceCacheUpdate )
		{
			...
			if (!Modules.Num())
			{
				UE_LOGF(LogTextureFormatManager, Error, "No texture formats found!");
			}
			...
			for (int32 Index = 0; Index < BaseModules.Num(); Index++)
			{
				...
				if (Format != nullptr)
				{
					...
					UE_LOGF(LogTextureFormatManager, VerboseIfNotEditor,"Loaded Base TextureFormat: %ls",*BaseModules[Index].ModuleName.ToString());
			...
			for (int32 Index = 0; Index < ChildModules.Num(); Index++)
			{
				...
				if (Format != nullptr)
				{
					UE_LOGF(LogTextureFormatManager,VerboseIfNotEditor,"Loaded Child TextureFormat: %ls",*ChildModules[Index].ModuleName.ToString());
	...
	virtual TArray<const class ITextureFormat*> GetAllTextureFormats() override
	{
		// note : does not call UpdateTextureFormatList , similar to FindTextureFormat
		//	assumes list is kept up to date

		TArray<const class ITextureFormat*> Ret;
		
		// copy member variable to local with lock held
		{
		FScopeLock Lock(&ModuleMutex);
		Ret = TextureFormats;
		}
