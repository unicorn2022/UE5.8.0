...
struct FQueryTest_LinkedFragmentAccess : FEntityTestBase
{
	virtual bool InstantTest() override
	{
		CA_ASSUME(EntityManager);
		...
		FMassExecutionContext ExecContext(*EntityManager.Get());
		Query.ForEachEntityChunk(ExecContext, [this, &bAnyMatch, &bFoundShared](FMassExecutionContext& Context)
			{
		...
		AITEST_TRUE("linked fragment should be discoverable", bFoundShared);
		AITEST_FALSE("owned fragment should be independent from linked fragment", bAnyMatch);
		return true;
	}
};
IMPLEMENT_AI_INSTANT_TEST(FQueryTest_LinkedFragmentAccess, "System.Mass.Query.LinkedFragmentAccess");

struct FQueryTest_IndirectFragmentAccess : FEntityTestBase
{
	virtual bool InstantTest() override
	{
		CA_ASSUME(EntityManager);
		...
		FMassExecutionContext ExecContext(*EntityManager.Get());
		Query.ForEachEntityChunk(ExecContext, [this, &bAllMatch, &bFoundIndirect](FMassExecutionContext& Context)
			{
		...
		AITEST_TRUE("Indirect fragment should be discoverable", bFoundIndirect);
		AITEST_TRUE("Indirect fragment should have correct value", bAllMatch);
		return true;
	}
};
IMPLEMENT_AI_INSTANT_TEST(FQueryTest_IndirectFragmentAccess, "System.Mass.Query.IndirectFragmentAccess");
