...
struct FSharedFragmentValues_Hash : FExecutionTestBase
{
	virtual bool InstantTest() override
	{
		...
		MASS_SCOPED_ENSURE_TEST("Expecting the containers to be sorted", 1);
...
struct FConstSharedFragment_AddToEntity : FEntityTestBase
{
	virtual bool InstantTest() override
	{
		...
		{
			MASS_SCOPED_ENSURE_TEST("shared struct requires to be registered first", 1);
			const FConstSharedStruct UnregisteredSharedFragmentInstance = FSharedStruct::Make(FragmentInstance);
			const bool bSuccessfullyAddedConstSharedFragment =  EntityManager->AddConstSharedFragmentToEntity(EntityHandle, UnregisteredSharedFragmentInstance);
			AITEST_FALSE("Adding unregistered const shared fragment struct should fail", bSuccessfullyAddedConstSharedFragment);
		}

		{
			...
			AITEST_TRUE("Adding registered const shared fragment struct should succeed", bSuccessfullyAddedConstSharedFragment);
		}
		...
		const FConstSharedStruct DifferentConstSharedFragmentInstance = EntityManager->GetOrCreateConstSharedFragment(DifferentFragmentInstance);

		MASS_SCOPED_ENSURE_TEST("Changing shared fragment value of entities is not supported", 1);
...
struct FSharedFragment_AddToEntity : FEntityTestBase
{
	virtual bool InstantTest() override
	{
		...
		const FSharedStruct DifferentSharedFragmentInstance = EntityManager->GetOrCreateSharedFragment(DifferentFragmentInstance);
...
// Tests that an entity's shared fragment values are preserved (not dropped) when
// RemoveFragmentListFromEntity triggers an archetype change.
struct FRemoveFragmentList_PreservesSharedFragmentValues : FEntityTestBase
{
	virtual bool InstantTest() override
	{
		constexpr int32 TestIntValue = 42;
		const FTestSharedFragment_Int FragmentInstance(TestIntValue);
		const FSharedStruct SharedFragmentInstance = EntityManager->GetOrCreateSharedFragment(FragmentInstance);

		// Start with an entity that has a float fragment (FloatsArchetype)
		const FMassEntityHandle EntityHandle = EntityManager->CreateEntity(FloatsArchetype);

		// Add a shared fragment — entity now lives in a new archetype: {float, SharedInt}
		AITEST_TRUE("Adding shared fragment should succeed",
			EntityManager->AddSharedFragmentToEntity(EntityHandle, SharedFragmentInstance));

		const FTestSharedFragment_Int* EntitySharedFragment = EntityManager->GetSharedFragmentDataPtr<FTestSharedFragment_Int>(EntityHandle);
		AITEST_NOT_NULL("Entity should have the shared fragment before the remove", EntitySharedFragment);
		AITEST_EQUAL("Shared fragment should hold the configured value", EntitySharedFragment->Value, TestIntValue);

		// Remove the regular float fragment via RemoveFragmentListFromEntity — triggers an archetype change to {SharedInt}
		const UScriptStruct* FragmentsToRemove[] = { FTestFragment_Float::StaticStruct() };
		EntityManager->RemoveFragmentListFromEntity(EntityHandle, MakeArrayView(FragmentsToRemove));

		// Shared fragment must survive the archetype move
		EntitySharedFragment = EntityManager->GetSharedFragmentDataPtr<FTestSharedFragment_Int>(EntityHandle);
		AITEST_NOT_NULL("Entity should still have the shared fragment after removing the regular fragment", EntitySharedFragment);
		AITEST_EQUAL("Shared fragment value should be unchanged after the archetype move", EntitySharedFragment->Value, TestIntValue);

		return true;
	}
};
IMPLEMENT_AI_INSTANT_TEST(FRemoveFragmentList_PreservesSharedFragmentValues, "System.Mass.SharedFragments.RemoveFragmentList.PreservesSharedFragmentValues");

// Tests the typed template overload RemoveConstSharedFragmentFromEntity<T>().
struct FConstSharedFragment_RemoveFromEntity_Typed : FEntityTestBase
{
	virtual bool InstantTest() override
	{
		constexpr int32 TestIntValue = 77;
		const FTestConstSharedFragment_Int FragmentInstance(TestIntValue);
		const FConstSharedStruct SharedFragmentInstance = EntityManager->GetOrCreateConstSharedFragment(FragmentInstance);

		const FMassEntityHandle EntityHandle = EntityManager->CreateEntity(FloatsArchetype);

		AITEST_TRUE("Adding const shared fragment should succeed",
			EntityManager->AddConstSharedFragmentToEntity(EntityHandle, SharedFragmentInstance));

		const FTestConstSharedFragment_Int* EntitySharedFragment = EntityManager->GetConstSharedFragmentDataPtr<FTestConstSharedFragment_Int>(EntityHandle);
		AITEST_NOT_NULL("Entity should have the const shared fragment", EntitySharedFragment);
		AITEST_EQUAL("Const shared fragment should hold the configured value", EntitySharedFragment->Value, TestIntValue);

		// Use the typed template overload (the new API being tested)
		AITEST_TRUE("Typed RemoveConstSharedFragmentFromEntity<T>() should succeed",
			EntityManager->RemoveConstSharedFragmentFromEntity<FTestConstSharedFragment_Int>(EntityHandle));

		EntitySharedFragment = EntityManager->GetConstSharedFragmentDataPtr<FTestConstSharedFragment_Int>(EntityHandle);
		AITEST_NULL("Entity should no longer have the const shared fragment after typed removal", EntitySharedFragment);

		AITEST_EQUAL("Entity's archetype should be back to the initial one", EntityManager->GetArchetypeForEntity(EntityHandle), FloatsArchetype);

		return true;
	}
};
IMPLEMENT_AI_INSTANT_TEST(FConstSharedFragment_RemoveFromEntity_Typed, "System.Mass.ConstSharedFragments.RemoveFromEntity.Typed");
...
struct FSharedFragment_BatchSetAttempt : FEntityTestBase
{
	virtual bool InstantTest() override
	{
		...
		{	
			MASS_SCOPED_ENSURE_TEST("Setting shared fragment values without archetype change is not supported", 1);
			MASS_SCOPED_ENSURE_TEST("Trying to set shared fragment values, without adding new shared fragments", 1);
...
struct FSharedFragment_BatchAddToEmpty : FEntityTestBase
{
	virtual bool InstantTest() override
	{
		...
		{
			MASS_SCOPED_ENSURE_TEST("Adding shared fragments to archetype-less entities is not supported", 1);
			EntityManager->BatchAddSharedFragmentsForEntities(MakeArrayView(&EntityCollection, 1), SharedIntValues);
		}
		return true;
	}
...
struct FSharedFragmentValues_TypeEquivalency : FEntityTestBase
{
	virtual bool InstantTest() override
	{
		...
		AITEST_TRUE("Empty shared values match type with empty bitset", HasExactSharedFragmentTypesMatch(Values, EmptySharedFragmentBitSet));
		AITEST_TRUE("Empty const shared values match type with empty const bitset", HasExactConstSharedFragmentTypesMatch(Values, EmptyConstSharedFragmentBitSet));
		...
		AITEST_TRUE("Single shared value type matches expected bitset", HasExactSharedFragmentTypesMatch(Values, IntSharedFragmentBitSet));
		AITEST_FALSE("Single shared value type doesn't match two-type bitset", HasExactSharedFragmentTypesMatch(Values, IntFloatSharedFragmentBitSet));
		AITEST_FALSE("Single shared value type doesn't match empty", HasExactSharedFragmentTypesMatch(Values, EmptySharedFragmentBitSet));
		...
		AITEST_TRUE("Single const shared value type matches expected bitset", HasExactConstSharedFragmentTypesMatch(Values, IntConstSharedFragmentBitSet));
		AITEST_FALSE("Single const shared value type doesn't match two-type bitset", HasExactConstSharedFragmentTypesMatch(Values, IntFloatConstSharedFragmentBitSet));
		AITEST_FALSE("Single const shared value type doesn't match empty", HasExactConstSharedFragmentTypesMatch(Values, EmptyConstSharedFragmentBitSet));

		Values.Remove(IntSharedFragmentBitSet);
		AITEST_TRUE("Emptied shared values match type with empty bitset", HasExactSharedFragmentTypesMatch(Values, EmptySharedFragmentBitSet));
		Values.Remove(IntConstSharedFragmentBitSet);
		AITEST_TRUE("Emptied const shared values match type with empty const bitset", HasExactConstSharedFragmentTypesMatch(Values, EmptyConstSharedFragmentBitSet));
