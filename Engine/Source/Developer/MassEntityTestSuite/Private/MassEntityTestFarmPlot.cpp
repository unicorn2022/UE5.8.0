...
void AMassEntityTestFarmPlot::BeginPlay()
{
	...
	{
		UE_LOGF(LogMass, Error, "%ls is misconfigured. Make sure TestDataCropIndicies, TestDataFlowerIndicies and VisualDataTable are not empty", *GetName());
		return;
	}
	...
	EntityManager.Initialize(FMassEntityManagerStorageInitParams(InitParams));
#else
