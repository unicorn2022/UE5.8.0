...
namespace UE::Mass::Test::Relations
{
	...
	struct FHierarchyTrivialCycle : FRelationTestBase
	{
		virtual bool InstantTest() override
		{
			...
			{
				MASS_SCOPED_ENSURE_TEST("subject and object to be different", 1);
	...
	struct FHierarchyCycle : FRelationTestBase
	{
		virtual bool InstantTest() override
		{
			...
			RelationshipManagerPtr->CreateRelationInstance(RelationTypeHandle, Entities[2], Entities[1]);
			{
				...
				MASS_SCOPED_ENSURE_TEST("result in a cycle", 1);
	...
	struct FInvalidRelations : FRelationTestBase
	{
		virtual bool InstantTest() override
		{
			...
			{
				MASS_SCOPED_ENSURE_TEST("Relation Objects count need match Subjects", 1);
			...
			}
			{
				...
				MASS_SCOPED_ENSURE_TEST("Relation needs a valid Subject entity", 1);
				Subjects.Add(FMassEntityHandle());
				Objects.Add(Entities[0]);

				MASS_SCOPED_ENSURE_TEST("Relation needs a valid Object entity", 1);
				Subjects.Add(Entities[0]);
				Objects.Add(FMassEntityHandle());
				
				MASS_SCOPED_ENSURE_TEST("Hierarchical relation requires the subject and object to be different", 1);
				Subjects.Add(Entities[0]);
				Objects.Add(Entities[0]);
				...
				MASS_SCOPED_ENSURE_TEST("Relation between the two entities already exists", 1);
				Subjects.Add(Entities[0]);
	...
	struct FReRegisteringRelationType : FRelationTestBase
	{
		virtual bool InstantTest() override
		{
			...
			{
				MASS_SCOPED_ENSURE_TEST("Modifying relationship after registration done is not supported", 1);
				FRelationTypeTraits LocalRelationTraits = CreateRelationTypeDescription("TestFragmentA");
				// make it different
	...
	struct FHasRelationSubjectsAndObjects : FRelationTestBase
	{
		virtual bool InstantTest() override
		{
			const FMassArchetypeHandle OriginalArchetype = IntsArchetype;
			...
			TArray<FMassEntityHandle> Entities;
			EntityManager->BatchCreateEntities(OriginalArchetype, 3, Entities);

			const FMassEntityHandle ChildA = Entities[0];
			const FMassEntityHandle ChildB = Entities[1];
			const FMassEntityHandle Parent = Entities[2];

			// Before any relations: nothing should exist
			AITEST_FALSE("Parent has no subjects before relations", RelationshipManagerPtr->HasRelationSubjects(RelationTypeHandle, Parent));
			AITEST_FALSE("ChildA has no objects before relations", RelationshipManagerPtr->HasRelationObjects(RelationTypeHandle, ChildA));
			AITEST_FALSE("ChildA has no subjects before relations", RelationshipManagerPtr->HasRelationSubjects(RelationTypeHandle, ChildA));

			// Create ChildA -> Parent
			RelationshipManagerPtr->CreateRelationInstance(RelationTypeHandle, ChildA, Parent);
			EntityManager->FlushCommands();

			AITEST_TRUE("Parent has subjects after first relation", RelationshipManagerPtr->HasRelationSubjects(RelationTypeHandle, Parent));
			AITEST_TRUE("ChildA has objects after first relation", RelationshipManagerPtr->HasRelationObjects(RelationTypeHandle, ChildA));
			AITEST_FALSE("ChildA has no subjects (it is a leaf)", RelationshipManagerPtr->HasRelationSubjects(RelationTypeHandle, ChildA));
			AITEST_FALSE("Parent has no objects (it is a root)", RelationshipManagerPtr->HasRelationObjects(RelationTypeHandle, Parent));

			// Create ChildB -> Parent (second child)
			RelationshipManagerPtr->CreateRelationInstance(RelationTypeHandle, ChildB, Parent);
			EntityManager->FlushCommands();

			AITEST_TRUE("Parent still has subjects after second relation", RelationshipManagerPtr->HasRelationSubjects(RelationTypeHandle, Parent));
			AITEST_TRUE("ChildB has objects after relation", RelationshipManagerPtr->HasRelationObjects(RelationTypeHandle, ChildB));

			// Destroy ChildA -> Parent
			RelationshipManagerPtr->DestroyRelationInstance(RelationTypeHandle, ChildA, Parent);
			EntityManager->FlushCommands();

			AITEST_TRUE("Parent still has subjects (ChildB remains)", RelationshipManagerPtr->HasRelationSubjects(RelationTypeHandle, Parent));
			AITEST_FALSE("ChildA has no objects after relation destroyed", RelationshipManagerPtr->HasRelationObjects(RelationTypeHandle, ChildA));

			// Destroy ChildB -> Parent
			RelationshipManagerPtr->DestroyRelationInstance(RelationTypeHandle, ChildB, Parent);
			EntityManager->FlushCommands();

			AITEST_FALSE("Parent has no subjects after all relations destroyed", RelationshipManagerPtr->HasRelationSubjects(RelationTypeHandle, Parent));
			AITEST_FALSE("ChildB has no objects after relation destroyed", RelationshipManagerPtr->HasRelationObjects(RelationTypeHandle, ChildB));

			return true;
		}
	};
	IMPLEMENT_AI_INSTANT_TEST(FHasRelationSubjectsAndObjects, "System.Mass.Relations.HasRelationSubjectsAndObjects");

	struct FForEachRelationSubjectAndObject : FRelationTestBase
	{
		virtual bool InstantTest() override
		{
			const FMassArchetypeHandle OriginalArchetype = IntsArchetype;
			...
			TArray<FMassEntityHandle> Entities;
			EntityManager->BatchCreateEntities(OriginalArchetype, 4, Entities);

			const FMassEntityHandle ChildA = Entities[0];
			const FMassEntityHandle ChildB = Entities[1];
			const FMassEntityHandle ChildC = Entities[2];
			const FMassEntityHandle Parent = Entities[3];

			RelationshipManagerPtr->CreateRelationInstance(RelationTypeHandle, ChildA, Parent);
			RelationshipManagerPtr->CreateRelationInstance(RelationTypeHandle, ChildB, Parent);
			RelationshipManagerPtr->CreateRelationInstance(RelationTypeHandle, ChildC, Parent);
			EntityManager->FlushCommands();

			// ForEachRelationSubject: enumerate children of Parent
			{
				TArray<FMassEntityHandle> CollectedSubjects;
				RelationshipManagerPtr->ForEachRelationSubject(RelationTypeHandle, Parent,
					[&CollectedSubjects](FMassEntityHandle Entity)
					{
						CollectedSubjects.Add(Entity);
					});

				TArray<FMassEntityHandle> ExpectedSubjects = RelationshipManagerPtr->GetRelationSubjects(RelationTypeHandle, Parent);
				AITEST_EQUAL("ForEachRelationSubject count matches GetRelationSubjects", CollectedSubjects.Num(), ExpectedSubjects.Num());

				// Verify same set of entities (order may differ)
				for (const FMassEntityHandle& Expected : ExpectedSubjects)
				{
					AITEST_TRUE("ForEachRelationSubject contains expected entity", CollectedSubjects.Contains(Expected));
				}
			}

			// ForEachRelationObject: enumerate parents of ChildA
			{
				TArray<FMassEntityHandle> CollectedObjects;
				RelationshipManagerPtr->ForEachRelationObject(RelationTypeHandle, ChildA,
					[&CollectedObjects](FMassEntityHandle Entity)
					{
						CollectedObjects.Add(Entity);
					});

				TArray<FMassEntityHandle> ExpectedObjects = RelationshipManagerPtr->GetRelationObjects(RelationTypeHandle, ChildA);
				AITEST_EQUAL("ForEachRelationObject count matches GetRelationObjects", CollectedObjects.Num(), ExpectedObjects.Num());

				for (const FMassEntityHandle& Expected : ExpectedObjects)
				{
					AITEST_TRUE("ForEachRelationObject contains expected entity", CollectedObjects.Contains(Expected));
				}
			}

			// ForEach on entity with no relations: callback should not fire
			{
				int32 CallCount = 0;
				RelationshipManagerPtr->ForEachRelationSubject(RelationTypeHandle, ChildA,
					[&CallCount](FMassEntityHandle)
					{
						++CallCount;
					});
				AITEST_EQUAL("ForEachRelationSubject on leaf fires zero callbacks", CallCount, 0);
			}
			{
				int32 CallCount = 0;
				RelationshipManagerPtr->ForEachRelationObject(RelationTypeHandle, Parent,
					[&CallCount](FMassEntityHandle)
					{
						++CallCount;
					});
				AITEST_EQUAL("ForEachRelationObject on root fires zero callbacks", CallCount, 0);
			}

			return true;
		}
	};
	IMPLEMENT_AI_INSTANT_TEST(FForEachRelationSubjectAndObject, "System.Mass.Relations.ForEachRelationSubjectAndObject");
