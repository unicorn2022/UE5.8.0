...
/** Works with checks too, but didn't want to make the name too lon */
#define MASS_SCOPED_ENSURE_TEST(PartialTextToExpect, NumOccurrences) \
	GetTestRunner().AddExpectedErrorPlain(PartialTextToExpect, EAutomationExpectedErrorFlags::Contains, NumOccurrences); \
	ON_SCOPE_EXIT \
	{ \
		--UE::Mass::Test::TestsInProgress; \
		FAutomationTestExecutionInfo ExecutionInfo; \
		GetTestRunner().GetExecutionInfo(ExecutionInfo); \
		if (ExecutionInfo.GetWarningTotal() + ExecutionInfo.GetErrorTotal() > 0) \
		{ \
...
struct FTestFragment_EntityHandle : public FMassFragment
{
	...
	FTestFragment_EntityHandle(const FMassEntityHandle InValue = FMassEntityHandle()) : Value(InValue) {}
};
...
public:
	...
	{
		return ProcessorEntityCreationRequirements;
	}
...
/**
 * Minimal concrete multi-type observer processor for unit tests.
 * Counts invocations of Execute() to verify deduplication guarantees.
 * bAutoRegisterWithObserverRegistry is set to false in the constructor so that the CDO
 * produced during module init does not attempt to contact UMassObserverRegistry.
 */
UCLASS(MinimalAPI)
...
public:
	...
	/** Sets the observed types list directly for test configuration. */
	...
	/** Sets the observed operation flags directly for test configuration. */
