		...
		if (bSaved)
		{
			if (SaveFilenames.Num() > 0 && SaveFilenames[0].IsEmpty() == false)
			{
				...
				if (FrameCache.Num())
				{
					...
					{
						UE_LOGF(LogVisualLogger, Error, "Failed to create file \"%ls\"", *SaveFilenames[0]);
					}
				}
			}
		}
...
void SVisualLogger::OnFiltersChanged()
{
	...
	UE_LOGF(LogVisualLogger, Display, "SVisualLogger::OnFiltersChanged: %5.2fms", FPlatformTime::ToMilliseconds(BlockingCycles));
}
...
void SVisualLogger::OnFiltersSearchChanged(const FText& Filter)
{
	...
	const int32 BlockingCycles = int32(FPlatformTime::Cycles() - StartCycles);
	UE_LOGF(LogVisualLogger, Display, "SVisualLogger::OnFiltersSearchChanged: %5.2fms", FPlatformTime::ToMilliseconds(BlockingCycles));
}
