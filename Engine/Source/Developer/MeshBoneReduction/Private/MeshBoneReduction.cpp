...
public:
	...
	// Iterate through all LODs and add all of the sections' bones to the set.
	// OutBoneIndices will contain all bones that have vertices weighted against them.
	void FindBonesUsedBySkinWeights(const USkeletalMesh* SkeletalMesh, TSet<int32>& OutBoneIndices)
	{
		if (!SkeletalMesh)
		{
			return;
		...
		// Iterate through all LODs and add all of the sections' bones to the set.
		const int32 NumLODs = ImportedModel->LODModels.Num();
		for (int32 LODIndex = 0; LODIndex < NumLODs; ++LODIndex)
		{
			const FSkeletalMeshLODModel& LODModel = ImportedModel->LODModels[LODIndex];
			for (const FSkelMeshSection& Section : LODModel.Sections)
			{
				for (const int32 BoneIndex : Section.BoneMap)
				{
	...
	void AddAllParentBonesToRoot(const USkeletalMesh* SkeletalMesh, TSet<int32>& InOutBoneIndices)
	{
		if (!SkeletalMesh)
		{
			return;
		...
		const TSet<int32> Copy = InOutBoneIndices;
		for (int32 BoneIndex : Copy)
		{
			...
			{
				InOutBoneIndices.Add(ParentBoneIndex);
				ParentBoneIndex = ReferenceSkeleton.GetParentIndex(ParentBoneIndex);
			}
		}
	}

	void AddBonesToKeepByName(const USkeletalMesh* SkeletalMesh, const TArray<FName>& ForceKeepBones, TSet<int32>& InOutKeepBones)
	{
		if (!SkeletalMesh || ForceKeepBones.Num() == 0)
		{
			return;
		}
		...
		for (const FName BoneName : ForceKeepBones)
		{
			const int32 BoneIndex = ReferenceSkeleton.FindBoneIndex(BoneName);
			if (BoneIndex != INDEX_NONE)
			{
				InOutKeepBones.Add(BoneIndex);
			}
		}
	}
	
	void BuildBonesToBeRemoved(const USkeletalMesh* SkeletalMesh, const TSet<int32>& KeepBoneIndices, TArray<FName>& OutBonesToRemove)
	{
		if (!SkeletalMesh)
		{
			return;
		...
		OutBonesToRemove.Reset();
		OutBonesToRemove.Reserve(NumBones);

		for (int32 BoneIndex = 0; BoneIndex < NumBones; ++BoneIndex)
		{
			if (!KeepBoneIndices.Contains(BoneIndex))
			{
				OutBonesToRemove.Add(ReferenceSkeleton.GetBoneName(BoneIndex));
			}
		}
	}
	
	void BuildBonesToBeRemovedUsedBySkinWeights(const USkeletalMesh* SkeletalMesh, const TArray<FName>& ForceKeepBones, TArray<FName>& OutBonesToRemove)
	{
		// Build the list of bones that we need to keep.
		TSet<int32> KeepBoneIndices;
		FindBonesUsedBySkinWeights(SkeletalMesh, KeepBoneIndices);
		AddBonesToKeepByName(SkeletalMesh, ForceKeepBones, KeepBoneIndices);
		AddAllParentBonesToRoot(SkeletalMesh, KeepBoneIndices);

		// Build the list of bones that we want to remove.
		BuildBonesToBeRemoved(SkeletalMesh, KeepBoneIndices, OutBonesToRemove);
	}
	
	void ReduceBoneCounts(USkeletalMesh* SkeletalMesh, const TArray<FName>& ForceKeepBones, const TArray<FName> BonesToRemove, int32 LODIndex, bool bIncludeBelowLODs)
	{
		if (BonesToRemove.Num() == 0)
		{
			return;
		}
		...
		const int32 TotalRefBones = RefSkel.GetNum();
		int32 RequiredBonesBefore = 0;
		{
			if (const FSkeletalMeshModel* Model = SkeletalMesh->GetImportedModel())
			{
				if (Model->LODModels.IsValidIndex(LODIndex))
				{
					RequiredBonesBefore = Model->LODModels[LODIndex].RequiredBones.Num();
				}
			}
		}

		UE_LOGF(LogTemp, Display,
			"[ReduceBoneCounts] %ls: RefBones: %d | ForceKeep: %d | Removing: %d",
		...
		{
			SkeletalMesh->AddBoneToReductionSetting(LODIndex, BoneName);
		}
		...
		if (bIncludeBelowLODs)
		{
			...
			for (int32 Index = LODIndex + 1; Index < TotalLOD; ++Index)
			{
				...
				ReduceBoneCounts(SkeletalMesh, Index, nullptr, bCallPostEditChange);
			}
		}

		ReduceBoneCounts(SkeletalMesh, LODIndex, nullptr, bCallPostEditChange);
		
		
		// 3. Capture state after removal.
		int32 RequiredBonesAfter = 0;
		TSet<int32> RequiredBoneSetAfter;
		...
		if (const FSkeletalMeshModel* Model = SkeletalMesh->GetImportedModel())
		{
			if (Model->LODModels.IsValidIndex(LODIndex))
			{
				...
				RequiredBonesAfter = LODModel.RequiredBones.Num();

				for (int32 BoneIndex : LODModel.RequiredBones)
				{
					RequiredBoneSetAfter.Add(BoneIndex);
				}
			}
		}

		// 4. Verification.
		for (const FName& RemovedBoneName : BonesToRemove)
		{
			const int32 RemovedIndex = RefSkel.FindBoneIndex(RemovedBoneName);
			if (RemovedIndex != INDEX_NONE && RequiredBoneSetAfter.Contains(RemovedIndex))
			{
				UE_LOGF(LogTemp, Warning,
					"[ReduceBoneCounts] %ls: Bone '%ls' could not be removed.",
					*SkeletalMesh->GetName(),
					*RemovedBoneName.ToString());
			}
		}

		const int32 RemovedCount = FMath::Max(0, RequiredBonesBefore - RequiredBonesAfter);

		UE_LOGF(LogTemp, Display, "[ReduceBoneCounts] %ls: Removed %d bones (RequiredBones: %d -> %d)",
			*SkeletalMesh->GetName(),
			RemovedCount,
			RequiredBonesBefore,
			RequiredBonesAfter);
	...
	bool ReduceBoneCounts(USkeletalMesh* SkeletalMesh, int32 DesiredLOD, const TArray<FName>* BoneNamesToRemove, bool bCallPostEditChange /*= true*/) override
	{
		if (!SkeletalMesh)
		{
			UE_LOGF(LogMeshBoneReduction, Error, "Failed to remove Skeletal Mesh LOD %i bones, as the Skeletal Mesh is invalid.", DesiredLOD);
			return false;
		}

		if(!SkeletalMesh->GetSkeleton())
		{
			UE_LOGF(LogMeshBoneReduction, Error, "Failed to remove bones from LOD %i in %ls, as its Skeleton is invalid.", DesiredLOD, *SkeletalMesh->GetPathName());
			return false;
		}
