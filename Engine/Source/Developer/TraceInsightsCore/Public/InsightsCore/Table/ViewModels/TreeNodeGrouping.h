...
public:
	...
	virtual ~FTreeNodeGrouping() = default;
...
public:
	...
	virtual ~FTreeNodeGroupingFlat() = default;
...
public:
	...
	virtual ~FTreeNodeGroupingByUniqueValue() = default;
...
protected:
	TSharedRef<FCustomTableTreeNode> CreateGroupNode(TWeakPtr<FTable> InParentTable, const FText& InGroupName) const;
	TSharedRef<FCustomTableTreeNode> CreateUnsetGroupNode(TWeakPtr<FTable> InParentTable) const;
...
public:
	...
	virtual ~TTreeNodeGroupingByUniqueValue() = default;
...
void TTreeNodeGroupingByUniqueValue<Type>::GroupNodes(const TArray<FTableTreeNodePtr>& Nodes, FTableTreeNode& ParentGroup, TWeakPtr<FTable> InParentTable, IAsyncOperationProgress& InAsyncOperationProgress) const
{
	...
	for (FTableTreeNodePtr NodePtr : Nodes)
	{
		...
		if (CellValue.IsSet())
		{
			...
			if (!GroupPtrPtr)
			{
				const FText GroupName = Column.GetValueAsGroupingText(*NodePtr);
				GroupPtr = CreateGroupNode(InParentTable, GroupName);
		...
		else
		{
			if (!UnsetGroupPtr)
			{
				UnsetGroupPtr = CreateUnsetGroupNode(InParentTable);
...
public:
	...
	virtual ~FTreeNodeGroupingByUniqueValueCString() = default;
...
public:
	...
	virtual ~FTreeNodeGroupingByNameFirstLetter() = default;
...
public:
	...
	virtual ~FTreeNodeGroupingByType() = default;
...
public:
	...
	virtual ~FTreeNodeGroupingByPathBreakdown() = default;
