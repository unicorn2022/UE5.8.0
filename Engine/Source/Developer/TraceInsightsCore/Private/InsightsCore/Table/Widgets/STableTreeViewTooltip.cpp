...
TSharedPtr<SToolTip> STableTreeViewTooltip::GetRowTooltip(const TSharedPtr<FTableTreeNode> TreeNodePtr)
{
			...
			[
				...
				[
					...
					[
						...
						.Text(TreeNodePtr->GetDisplayName())
						.TextStyle(GetTableTreeViewTooltipNormalTextStyle())
					]
				]
	...
	if (Table.IsValid())
	{
		...
		for (const TSharedRef<FTableColumn>& Column : Table->GetColumns())
		{
			...
			{
				const FText Value = Column->GetValueAsTooltipText(*TreeNodePtr);
				if (!Value.IsEmpty())
				{
					const FText Name = FText::Format(LOCTEXT("TooltipValueFormat", "{0}:"), Column->GetTitleName());
					AddGridRow(GridPanel, Row, Name, Value);
				}
			}
		}
	}
