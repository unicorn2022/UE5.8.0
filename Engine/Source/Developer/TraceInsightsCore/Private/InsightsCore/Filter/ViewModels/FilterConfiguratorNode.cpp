...
FFilterConfiguratorNode::FFilterConfiguratorNode(bool bInIsGroup)
	: FBaseTreeNode(bInIsGroup)
{
	if (bInIsGroup)
	{
...
{
	TSharedRef<FFilterConfiguratorNode> NodeCopyPtr = MakeShared<FFilterConfiguratorNode>(InSourceNode.IsGroup());
...
bool FFilterConfiguratorNode::ApplyFilters(const FBaseFilterContext& Context) const
{
	if (IsGroup())
	{
	...
	{
		return FilterState->ApplyFilter(Context);
	}
}
