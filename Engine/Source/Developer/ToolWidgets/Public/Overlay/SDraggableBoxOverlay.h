...
public:
	...
	SLATE_BEGIN_ARGS(SDraggableBoxOverlay)
		...
		, _MinimumBoxWidth(TOptional<float>(20.f))
		, _MinimumBoxHeight(TOptional<float>(20.f))
	{}
	...
	SLATE_ATTRIBUTE_DEPRECATED(bool, IsDraggable, 5.8, "Use Draggable instead.")
	/** Whether this widget should be allowed to be dragged. */
	...
	/** Which edges the user is allowed to resize by dragging. Default: None (resizing disabled). */
	...
	/** Minimum allowed box width when the user resizes. */
	SLATE_ATTRIBUTE(TOptional<float>, MinimumBoxWidth)
	/** Minimum allowed box height when the user resizes. */
	SLATE_ATTRIBUTE(TOptional<float>, MinimumBoxHeight)
	/** Maximum allowed box width when the user resizes. */
	SLATE_ATTRIBUTE(TOptional<float>, MaximumBoxWidth)
	/** Maximum allowed box height when the user resizes. */
	SLATE_ATTRIBUTE(TOptional<float>, MaximumBoxHeight)
	/** Explicit width override. Unset (default) means the box sizes itself to its content width. */
	SLATE_ARGUMENT(TOptional<float>, WidthOverride)
	/** Explicit height override. Unset (default) means the box sizes itself to its content height. */
	SLATE_ARGUMENT(TOptional<float>, HeightOverride)
	/** Invoked when the user finishes a resize operation. */
	...
	/** Sets the content's position. This is useful for loading a widget position saved in a config file.*/
	UE_API void SetDragBoxPosition(const FToolWidget_DragBoxPosition& InPosition);
	/** Restores the content position. This is useful for loading a widget position saved in a config file. */
	UE_DEPRECATED(5.8, "Use SetDragBoxPosition instead.")
	UE_API void RestoreFromDragBoxPosition(const FToolWidget_DragBoxPosition& InWidgetPosition);

	/** Returns the explicit width override set by a user resize, or unset if the box is content-sized on this axis. */
	UE_API TOptional<float> GetWidthOverride() const;
	/** Sets an explicit width override. Pass an unset optional to revert to content sizing on this axis. */
	UE_API void SetWidthOverride(const TOptional<float>& NewWidth);

	/** Returns the explicit height override set by a user resize, or unset if the box is content-sized on this axis. */
	UE_API TOptional<float> GetHeightOverride() const;
	/** Sets an explicit height override. Pass an unset optional to revert to content sizing on this axis. */
	UE_API void SetHeightOverride(const TOptional<float>& NewHeight);
	
	UE_API TOptional<float> GetMinimumBoxWidth() const;
	UE_API TOptional<float> GetMaximumBoxWidth() const;
	UE_API TOptional<float> GetMinimumBoxHeight() const;
	UE_API TOptional<float> GetMaximumBoxHeight() const;
...
protected:
	...
	/** Width override set by a user resize operation. Unset means the box sizes itself to its content on this axis. */
	TOptional<float> WidthOverride;
	/** Height override set by a user resize operation. Unset means the box sizes itself to its content on this axis. */
	TOptional<float> HeightOverride;
	/** Minimum allowed box width when the user resizes. */
	TAttribute<TOptional<float>> MinimumBoxWidth;
	/** Minimum allowed box height when the user resizes. */
	TAttribute<TOptional<float>> MinimumBoxHeight;
	/** Maximum allowed box width when the user resizes. */
	TAttribute<TOptional<float>> MaximumBoxWidth;
	/** Maximum allowed box height when the user resizes. */
	TAttribute<TOptional<float>> MaximumBoxHeight;
