...
void SSimpleTimeSlider::Construct( const SSimpleTimeSlider::FArguments& InArgs )
{
	...
	OnFormatTickLabel = InArgs._OnFormatTickLabel;
	MinPixelsPerDisplayTick = InArgs._MinPixelsPerDisplayTick.Get(ScrubConstants::MinPixelsPerDisplayTick);
...
{
	const float Spacing = DetermineOptimalSpacing( RangeToScreen.PixelsPerInput, MinPixelsPerDisplayTick, ScrubConstants::MinDisplayTickSpacing );
	...
	while( (Seconds = static_cast<float>(OffsetNum)*Spacing) < RangeToScreen.ViewInput.GetUpperBoundValue() )
	{
		...
		if ( AbsOffsetNum % Divider == 0 )
		{
			...
			{
				FString FrameString = OnFormatTickLabel.IsBound()
					? OnFormatTickLabel.Execute(Seconds)
					: (Spacing == ScrubConstants::MinDisplayTickSpacing ? FString::Printf(TEXT("%.3f"), Seconds) : FString::Printf(TEXT("%.2f"), Seconds));
