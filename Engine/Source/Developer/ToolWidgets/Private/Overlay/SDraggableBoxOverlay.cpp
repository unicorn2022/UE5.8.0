...
void SDraggableBoxOverlay::Construct(const FArguments& InArgs)
{
	...
	MinimumBoxWidth = InArgs._MinimumBoxWidth;
	MinimumBoxHeight = InArgs._MinimumBoxHeight;
	MaximumBoxWidth = InArgs._MaximumBoxWidth;
	MaximumBoxHeight = InArgs._MaximumBoxHeight;
	...
	[
		...
		[
			...
			.Draggable(InArgs._Draggable)
			.DragOverridesCenterAlignment(InArgs._DragOverridesCenterAlignment)
			.OnUserDraggedToNewPosition(InArgs._OnUserDraggedToNewPosition)
			.Resizable(InArgs._Resizable)
			.ResizeHandleThickness(InArgs._ResizeHandleThickness)
			.OnUserResized(InArgs._OnUserResized)
...
void SDraggableBoxOverlay::SetBoxAlignmentOffset(const FVector2f& InOffset, bool bInRecomputeAnchorPoints)
{
	...
	case EHorizontalAlignment::HAlign_Fill:
		// Clear out any padding from prior alignments (Left/Right).
		Padding.Left = 0.f;
		Padding.Right = 0.f;
		break;
...
void SDraggableBoxOverlay::SetDragBoxPosition(const FToolWidget_DragBoxPosition& InWidgetPosition)
{
...
{
	SetDragBoxPosition(InWidgetPosition);
}

TOptional<float> SDraggableBoxOverlay::GetWidthOverride() const
{
	return WidthOverride;
}

void SDraggableBoxOverlay::SetWidthOverride(const TOptional<float>& NewWidth)
{
	WidthOverride = NewWidth;
	if (WidthOverride.IsSet())
	{
		TOptional<float> MinBoxWidth = MinimumBoxWidth.Get();
		if (MinBoxWidth.IsSet())
		{
			WidthOverride = FMath::Max(WidthOverride.GetValue(), MinBoxWidth.GetValue());
		}
		TOptional<float> MaxBoxWidth = MaximumBoxWidth.Get();
		if (MaxBoxWidth.IsSet())
		{
			WidthOverride = FMath::Min(WidthOverride.GetValue(), MaxBoxWidth.GetValue());
		}
	}
}

TOptional<float> SDraggableBoxOverlay::GetHeightOverride() const
{
	return HeightOverride;
}

void SDraggableBoxOverlay::SetHeightOverride(const TOptional<float>& NewHeight)
{
	HeightOverride = NewHeight;
	if (HeightOverride.IsSet())
	{
		TOptional<float> MinBoxHeight = MinimumBoxHeight.Get();
		if (MinBoxHeight.IsSet())
		{
			HeightOverride = FMath::Max(HeightOverride.GetValue(), MinBoxHeight.GetValue());
		}
		TOptional<float> MaxBoxHeight = MaximumBoxHeight.Get();
		if (MaxBoxHeight.IsSet())
		{
			HeightOverride = FMath::Min(HeightOverride.GetValue(), MaxBoxHeight.GetValue());
		}
	}
}
	
TOptional<float> SDraggableBoxOverlay::GetMinimumBoxWidth() const
{
	return MinimumBoxWidth.Get();
}

TOptional<float> SDraggableBoxOverlay::GetMaximumBoxWidth() const
{
	return MaximumBoxWidth.Get();
}
	
TOptional<float> SDraggableBoxOverlay::GetMinimumBoxHeight() const
{
	return MinimumBoxHeight.Get();
}

TOptional<float> SDraggableBoxOverlay::GetMaximumBoxHeight() const
{
	return MaximumBoxHeight.Get();
}
}
