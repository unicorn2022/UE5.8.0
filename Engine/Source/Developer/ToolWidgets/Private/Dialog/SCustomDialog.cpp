...
int32 SCustomDialog::ShowModal(const int32 InDefaultReturnValue)
{
	int32 Result = InDefaultReturnValue;
	if (!FApp::IsUnattended() && !GIsRunningUnattendedScript && FSlateApplication::IsInitialized())
	{
		...
		if (LastPressedButton != INDEX_NONE)
		{
			Result = LastPressedButton;
		}
	}
	else
	{
		...
		if (GWarn)
		{
			GWarn->Logf(TEXT("Message dialog was triggered in unattended script mode, title: %s"), *Title.Get().ToString());
		}
	}

	if (GWarn)
	{
		GWarn->Logf(TEXT("Message dialog closed, result: %d, title: %s"), Result, *Title.Get().ToString());
	}
...
void SCustomDialog::Show()
{
	if (!FApp::IsUnattended() && !GIsRunningUnattendedScript && FSlateApplication::IsInitialized())
	{
		...
		if (OnClosed.IsBound())
		{
			Window->GetOnWindowClosedEvent().AddSPLambda(this, [this](const TSharedRef<SWindow>&) { OnClosed.Execute(); });
		}
	}
	else
	{
	...
	if (GWarn)
	{
		GWarn->Logf(TEXT("Message dialog closed, title: %s"), *Title.Get().ToString());
	}
}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(SCustomDialogShowModalTest, "System.ToolWidgets.CustomDialog.ShowModalInvokesOnClosedWhenUnattended",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::CommandletContext | EAutomationTestFlags::EngineFilter)
 
bool SCustomDialogShowModalTest::RunTest(const FString& Parameters)
{
	bool bClosed = false;
	const TSharedRef<SCustomDialog> Dialog = SNew(SCustomDialog)
		.Title(FText::FromString("Test dialog."))
		.OnClosed_Lambda([&bClosed] {	bClosed = true;	});
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(SCustomDialogShowTest, "System.ToolWidgets.CustomDialog.ShowInvokesOnClosedWhenUnattended",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::CommandletContext | EAutomationTestFlags::EngineFilter)
 
bool SCustomDialogShowTest::RunTest(const FString& Parameters)
{
	bool bClosed = false;
	const TSharedRef<SCustomDialog> Dialog = SNew(SCustomDialog)
		.Title(FText::FromString("Test dialog."))
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(SCustomDialogShowModalReturnTest, "System.ToolWidgets.CustomDialog.ShowModalReturnsDefaultValueInUnattendedMode",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::CommandletContext | EAutomationTestFlags::EngineFilter)
 
bool SCustomDialogShowModalReturnTest::RunTest(const FString& Parameters)
{
	const TSharedRef<SCustomDialog> Dialog = SNew(SCustomDialog)
		.Title(FText::FromString("Test dialog."));
	
	const int32 DefaultValue = 5;
	return Dialog->ShowModal(DefaultValue) == DefaultValue;
}
