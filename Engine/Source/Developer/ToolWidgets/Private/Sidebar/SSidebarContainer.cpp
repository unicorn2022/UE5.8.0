...
void SSidebarContainer::OpenDrawer_Internal(const TSharedRef<FSidebarDrawer>& InDrawer, const bool bInAnimate)
{
	...
	// Skip creating a new widget if one is already animating closed
	if (!InDrawer->DrawerWidget.IsValid() || !ClosingDrawerWidgets.Contains(InDrawer->DrawerWidget))
	{
	...
	BroadcastDrawerOpenClose(InDrawer, /*bOpen=*/true);
}
	...
	, const bool bInSummonPinnedTabIfNothingOpened)
{
	...
	BroadcastDrawerOpenClose(InDrawer, /*bOpen=*/false);
}
...
void SSidebarContainer::BroadcastDrawerOpenClose(const TSharedRef<FSidebarDrawer>& InDrawer, const bool bOpen)
{
	InDrawer->OnDrawerOpenCloseDelegate.ExecuteIfBound(InDrawer->GetUniqueId(), bOpen);

	TArray<TSharedRef<ISidebarDrawerContent>> ContentSections;
	InDrawer->ContentSections.GenerateValueArray(ContentSections);

	for (const TSharedRef<ISidebarDrawerContent>& DrawerSection : ContentSections)
	{
		DrawerSection->OnDrawerOpenClose(bOpen);
	}
}
