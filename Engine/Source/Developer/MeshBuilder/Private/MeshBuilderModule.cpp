	...
	FStaticMeshSectionArray& Sections)
{
	...
	if ( Mesh==nullptr )
	{
		UE_LOGF(LogMeshBuilderModule, Warning, "NULL Mesh UObject passed to BuildMeshVertexPositions");
	}	
	else if (StaticMesh == nullptr)
	{
		UE_LOGF(LogMeshBuilderModule, Warning, "Mesh UObject %ls (%ls) passed to BuildMeshVertexPositions could not be converted to a UStaticMesh", *Mesh->GetFName().ToString(), *Mesh->GetPathName());
	}
