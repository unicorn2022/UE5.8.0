	...
	FStaticMeshNaniteBuildContext* ParentContext = nullptr)
{
	...
	{
		UE_LOGF(LogStaticMeshBuilder, Error, "Invalid mesh description during Nanite build [%ls].", *StaticMesh->GetFullName());
		return false;
	}
	if (LOD0MeshDescription->IsEmpty())
	{
		UE_LOGF(LogStaticMeshBuilder, Error, "Empty mesh description during Nanite build [%ls].", *StaticMesh->GetFullName());
		return false;
	}
	...
	if (bTargetSupportsNanite && bHasHiResSourceModel)
	{
		if (const FMeshDescription* HiResMeshDescription = HiResSourceModel.GetOrCacheMeshDescription())
		{
			...
			{
				UE_LOGF(LogStaticMeshBuilder, Display,
					"Invalid hi-res mesh description during Nanite build [%ls]. The hi-res mesh is empty. " "This is not supported and LOD 0 will be used as a fallback to build Nanite data.",
			...
			else
			{
				...
				{
					UE_LOGF(LogStaticMeshBuilder, Display,
						"Invalid hi-res mesh description during Nanite build [%ls]. " "The number of sections from the hires mesh is higher than LOD 0 section count. " "This is not supported and LOD 0 will be used as a fallback to build Nanite data.",
						*StaticMesh->GetFullName());
				}
				else
				{
					...
					{
						UE_LOGF(LogStaticMeshBuilder, Display,
							"Nanite hi-res mesh description for [%ls] has fewer sections than lod 0. " "Verify you have the proper material id result when Nanite is turned on.",
							*StaticMesh->GetFullName());
					}
	...
	{
		UE_LOGF(LogStaticMeshBuilder, Warning,
			"Static mesh %ls has Shape Preservation set to \"Voxelize\", but voxels aren't enabled for the project. " "Voxelization will be disabled for the mesh. Enable Nanite Foliage in the project settings or configure " "r.Nanite.AllowVoxels=1 to enable support.",
			*StaticMesh->GetFullName());
	...
	FBoxSphereBounds& OutVertexBounds)
{
	...
	{
		UE_LOGF(LogStaticMeshBuilder, Error,
			"Failed to clone mesh description during Nanite build [%ls].",
	...
	{
		UE_LOGF(LogStaticMeshBuilder, Error,
			"Cannot build an empty mesh description during Nanite build [%ls].",
			*Context.StaticMesh->GetFullName());
		return false;
	}
...
)
{
	...
	for (int32 InVertIndex = 0; InVertIndex < NumVertices; ++InVertIndex)
	{
		...
		// No CurveRadius for static meshes
		check(InMeshData.Vertices.CurveRadius.Num() == 0);
...
{
	TArray<uint32, TInlineAllocator<2>> MaxNumOfTriangles;
	...
	FRayTracingFallbackBuildContext& OutBuildContext)
{
	...
	if (StaticMesh->bSupportRayTracing && TargetPlatform->UsesRayTracing())
	{
		...
		static const TConsoleVariableData<int32>* CVarRayTracingProxiesLOD1MaxNumOfTriangles = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.StaticMesh.RayTracingProxies.LOD1MaxNumOfTriangles"));
		...
		{
			const uint32 LOD1MaxNumOfTriangles = CVarRayTracingProxiesLOD1MaxNumOfTriangles->GetValueOnAnyThread();

			// LOD0 is generated by Nanite build and is already simplified so no need to reduce
			OutBuildContext.MaxNumOfTriangles.Add(MAX_uint32);
			OutBuildContext.MaxNumOfTriangles.Add(LOD1MaxNumOfTriangles);

			OutBuildContext.MeshDescriptions.SetNum(OutBuildContext.MaxNumOfTriangles.Num());
...
bool FStaticMeshBuilder::Build(FStaticMeshRenderData& StaticMeshRenderData, const FStaticMeshBuildParameters& BuildParameters)
{
	...
	{
		UE_LOGF(LogStaticMeshBuilder, Error, "Provided FStaticMeshBuildParameters must have a valid TargetPlatform.");
		return false;
	}
	...
	if (!StaticMesh->IsMeshDescriptionValid(0))
	{
		...
		UE_LOGF(LogStaticMeshBuilder, Error, "Cannot find a valid mesh description to build the asset.");
		return false;
	}
	...
	if (StaticMeshRenderData.LODResources.Num() > 0)
	{
		...
		UE_LOGF(LogStaticMeshBuilder, Error, "Cannot build static mesh render data twice [%ls].", *StaticMesh->GetFullName());
	...
	TFunction<void(const FMeshDescription&, const FMeshDescription&)> CheckReduction = [&](const FMeshDescription& InitMesh, const FMeshDescription& ReducedMesh)
	{
		...
		{
			UE_LOGF(LogStaticMeshBuilder, Warning, "The generation of LOD could have generated spikes on the mesh for %ls", *StaticMesh->GetName());
		}
	};
	...
	if (bBuildRayTracingFallback)
	{
		...
		for (int32 LodIndex = 0; LodIndex < NumRayTracingLODs; ++LodIndex)
		{
			...
			ReductionSettings.MaxNumOfTriangles = RayTracingFallbackBuildContext.MaxNumOfTriangles[LodIndex];
			...
			const bool bUseReduction = ReductionSettings.PercentTriangles < 1.f || ReductionSettings.MaxNumOfTriangles < (uint32)RayTracingMeshDescriptions[BaseReduceLodIndex].Triangles().Num();
	...
	FStaticMeshSectionArray& Sections)
{
	...
	if (MeshDescription.IsEmpty())
	{
		UE_LOGF(LogStaticMeshBuilder, Error, "Cannot build the asset from an empty mesh description.");
		return false;
	}
...
bool AreVerticesEqual(const FPendingVertex& Vertex, const FMeshBuildVertexData& VertexData, int32 CompareVertex, float ComparisonThreshold)
{
	...
	if (VertexData.CurveRadius.Num() > 0)
	{
		if (Vertex.CurveRadius != VertexData.CurveRadius[CompareVertex])
		{
			return false;
		}
	}
...
)
{
	...
	// No curve radius for meshes
	const bool bValidCurveRadius = false;
	...
	for (const FTriangleID TriangleID : MeshDescription.Triangles().GetElementIDs())
	{
		...
		const int32* FoundSectionIndex = PolygonGroupToSectionIndex.Find(PolygonGroupID);
		const int32 SectionIndex = 
			ensureMsgf(FoundSectionIndex, TEXT("Static mesh builder found mesh description triangle with invalid polygon group ID not in the mesh description polygon groups list: %d"), PolygonGroupID.GetValue()) 
			? *FoundSectionIndex : 0;
...
void BuildAllBufferOptimizations(FStaticMeshLODResources& StaticMeshLOD, const FMeshBuildSettings& LODBuildSettings, TArray< uint32 >& IndexBuffer, bool bNeeds32BitIndices, const FConstMeshBuildVertexView& BuildVertices)
{
	...
	static const TConsoleVariableData<int32>* CVarDepthOnlyIndexBuffers = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.SupportDepthOnlyIndexBuffers"));
	static const TConsoleVariableData<int32>* CVarReversedIndexBuffers = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.SupportReversedIndexBuffers"));

	const bool bDepthOnlyIndexBuffers = !!CVarDepthOnlyIndexBuffers->GetValueOnAnyThread();
	const bool bReversedIndexBuffers = !!CVarReversedIndexBuffers->GetValueOnAnyThread();
	...
	if (LODBuildSettings.bBuildReversedIndexBuffer && bReversedIndexBuffers)
	{
	...
	if (LODBuildSettings.bBuildReversedIndexBuffer && bReversedIndexBuffers && bDepthOnlyIndexBuffers)
	{
