...
public:
	...
	*/
	...
	public:
		...
		* Virtual dtor
		*/
		virtual ~FMeshEdgeBuilder() = default;
	...
	)
	{
		...
		OriginalVertices.CurveRadius = Vertices.CurveRadius;
		...
		for (int32 SectionIndex = 0; SectionIndex < PerSectionIndices.Num(); SectionIndex++)
		{
			...
			if (Indices.Num())
			{
				...
				for (int32 Index = 0; Index < Indices.Num(); Index++)
				{
					...
					if (Vertices.CurveRadius.Num() > 0)
					{
						Vertices.CurveRadius[Indices[Index]] = OriginalVertices.CurveRadius[OriginalIndices[Index]];
					}
