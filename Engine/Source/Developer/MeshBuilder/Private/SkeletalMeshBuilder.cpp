...
static void BuildNaniteFallbackMeshDescription(
	...
	const FSkeletalMeshUnifiedBoneIndices& UnifiedBoneIndices
)
{
	...
	for (int32 InVertIndex = 0; InVertIndex < NumVertices; ++InVertIndex)
	{
		...
		for (uint32 InfluenceIndex = 0; InfluenceIndex < NumBoneInfluences; ++InfluenceIndex)
		{
			...
			uint16 BoneIndex = BoneIndices[InVertIndex];

			if (UnifiedBoneIndices.IsValid())
			{
				// Input vertices are in unified bone space, but the fallback path expects bones
				// in skeleton space which it will then remap to unified in its subsequent build step.
				BoneIndex = UnifiedBoneIndices.GetUnifiedToSkeletonIndex(BoneIndex);
			}

			InfluenceBones[InfluenceIndex] = BoneIndex;
...
public:
	...
	FContext(
		...
		TConstArrayView<uint16> UnifiedBoneMap = {}
	)
		...
		, UnifiedBoneIndices(UnifiedBoneMap)
	{
	...
	FContext(const FContext& ParentContext, USkeletalMesh& AssemblyPartMesh)
		...
		, UnifiedBoneIndices(ParentContext.UnifiedBoneIndices)
	{
	...
	bool BuildNaniteAssemblyData()
	{
		...
		{
			UE_LOGF(LogSkeletalMesh, Warning,
				"Failed to build Nanite Assembly skeletal mesh %ls. Nanite assemblies are not enabled for the project. " "Enable Nanite Foliage in the project settings or configure r.Nanite.AllowAssemblies=1 to enable support.",
				*SkeletalMesh->GetPathName());
			return false;
		}
		...
		{
			UE_LOGF(LogSkeletalMesh, Warning,
				"Failed to build Nanite Assembly skeletal mesh %ls. The referenced skeletal meshes were not cached before build.",
				*SkeletalMesh->GetPathName());
			return false;
		}
		...
		for (int32 PartIndex = 0; PartIndex < InAssemblyData.Parts.Num(); ++PartIndex)
		{
			...
			{
				UE_LOGF(LogSkeletalMesh, Warning, 
					"Failed to build Nanite Assembly part from skeletal mesh (%ls). See previous line(s) for details.",
	...
	bool BuildNanite(FSkeletalMeshRenderData& OutRenderData)
	{
		...
		if (bGenerateFallback)
		{
			...
			BuildNaniteFallbackMeshDescription(SkeletalMesh, FallbackMeshData, *FallbackMeshDescription, UnifiedBoneIndices);
...
private:
	void Init(const FContext* ParentContext = nullptr)
	{
		...
		if (IsNaniteBuildEnabled())
		{
			...
			{
				UE_LOGF(LogSkeletalMesh, Warning,
					"Skeletal mesh %ls has Shape Preservation set to \"Voxelize\", but voxels aren't enabled for the project. " "Voxelization will be disabled for the mesh. Enable Nanite Foliage in the project settings or configure " "r.Nanite.AllowVoxels=1 to enable support.",
		...
		if (IsNaniteAssemblyBuild())
		{
			...
			// When the skeletal mesh uses unified bone indices, give the Nanite builder a remap
			// table so it can convert assembly bone influences from skeleton space to unified
			// space. Without this, vertex bone indices produced from assembly data end up in a
			// different index space than vertex bone indices encoded by InitNaniteBuildInput,
			// and the fallback decode path mis-interprets them.
			if (UnifiedBoneIndices.IsValid())
			{
				NaniteAssemblyData.SkeletonToUnifiedBoneIndex.SetNumUninitialized(NumBones);
				for (uint32 i = 0; i < NumBones; ++i)
				{
					NaniteAssemblyData.SkeletonToUnifiedBoneIndex[i] = UnifiedBoneIndices.GetSkeletonToUnifiedIndex(i);
				}
			}
		}
	}
	...
	void InitNaniteBuildInput(Nanite::IBuilderModule::FInputMeshData& InputMeshData)
	{
		...
		for (int32 SectionIndex = 0; SectionIndex < LODModel.Sections.Num(); ++SectionIndex)
		{
			...
			for (int32 VertIndex = 0; VertIndex < Section.SoftVertices.Num(); ++VertIndex)
			{
				...
				{
					FBoneIndexType BoneIndex = Section.BoneMap[SoftVertex.InfluenceBones[Influence]];

					if (UnifiedBoneIndices.IsValid())
					{
						const FBoneIndexType PreRemapBoneIndex = BoneIndex;
						const FBoneIndexType PreRemapLocalIndex = SoftVertex.InfluenceBones[Influence];
						const uint16 InfluenceWeight = SoftVertex.InfluenceWeights[Influence];

						// Remap the skeleton index into unified bone index space.
						BoneIndex = UnifiedBoneIndices.GetSkeletonToUnifiedIndex(PreRemapBoneIndex);

						if (BoneIndex == INVALID_BONE_INDEX && InfluenceWeight != 0)
						{
							if (!bLogOnce)
							{
								UE_LOG(LogSkeletalMesh, Error,
									TEXT("Nanite ISKM encode: skeleton bone %u (from Section.BoneMap[%u]) has no unified mapping "
										 "but vertex has non-zero weight %u. Nanite vertex would reference an invalid transform slot. "
										 "Falling back to bone 0 (root) with zero weight for this influence."),
									(uint32)PreRemapBoneIndex,
									(uint32)PreRemapLocalIndex,
									(uint32)InfluenceWeight);
								bLogOnce = true;
							}
							BoneIndex = 0;
							InputMeshData.Vertices.BoneIndices[Influence][Section.BaseVertexIndex + VertIndex] = 0;
							InputMeshData.Vertices.BoneWeights[Influence][Section.BaseVertexIndex + VertIndex] = 0;
							continue;
						}
					}

					InputMeshData.Vertices.BoneIndices[Influence][Section.BaseVertexIndex + VertIndex] = BoneIndex;
...
static bool BuildNanite(FScopedSlowTask& SlowTask, SkeletalMeshBuilderPrivate::FContext& Context, FSkeletalMeshRenderData& OutRenderData)
{
	...
	if (!bBuildSuccess)
	{
		UE_LOGF(LogSkeletalMesh, Error, "Failed to build Nanite for skeletal mesh. See previous line(s) for details.");
	}
...
bool FSkeletalMeshBuilder::Build(FSkeletalMeshRenderData& OutRenderData, const FSkeletalMeshBuildParameters& SkeletalMeshBuildParameters)
{
	...
	{
		UE_LOGF(LogSkeletalMesh, Error, "Failed to build skeletal mesh. Invalid Mesh Description at LODIndex %d", SkeletalMeshBuildParameters.LODIndex);
		return false;
	}
	...
	if (bBuildNaniteFallback)
	{
		...
		Nanite::FMeshDataSectionArray NaniteFallbackMeshSections;
		{
			SkeletalMeshBuilderPrivate::FContext NaniteContext(
				...
				{},
				OutRenderData.GetUnifiedBoneMap()
			);
