...
OODEFFUNC typedef void (OOEXPLINK t_fp_OodleTex_Plugins_SetCancelRequested)(t_fp_OodleTex_Plugin_CancelRequested * fp_CancelRequested);
...
struct FOodleTextureVTable
{
	...
	t_fp_OodleTex_Plugins_SetCancelRequested * fp_OodleTex_Plugins_SetCancelRequested = nullptr;
	...
	bool TryLoad()
	{
		...
		UE_LOGF(LogTextureFormatOodle,VerboseIfNotEditor,"Oodle Texture loading DLL: %ls", *DynamicLibName);

		void * DynamicLib = FPlatformProcess::GetDllHandle(*DynamicLibName);
		if ( DynamicLib == nullptr )
		{
			UE_LOGF(LogTextureFormatOodle, Warning, "Oodle Texture %ls requested but could not be loaded", *DynamicLibName);
		...
		if ( OodleErr != OodleTex_Err_OK )
		{
			...
			UE_LOGF(LogTextureFormatOodle, Warning, "Oodle Texture %ls loaded but failed in LogVersion with error %d=%ls", *DynamicLibName,
		...
		fp_OodleTex_Plugins_SetCancelRequested = (t_fp_OodleTex_Plugins_SetCancelRequested *) FPlatformProcess::GetDllExport( DynamicLib, TEXT("OodleTex_Plugins_SetCancelRequested") );
		// no check for non-nullptr, we support versions that don't have this
...
//	change : 1-20-2026 : ALSO register unprefixed names
//	note this means if the legacy NVTT/ISPC is also registered then we get two registrations for the unprefixed names
//	 and then the order of insertion of the TextureFormat in TFManager matters
#define TEXTURE_FORMAT_PREFIX	"TFO_"
#define DECL_FORMAT_NAME(FormatName) \
	static FName GPrefixedTextureFormatName##FormatName = FName(TEXT(TEXTURE_FORMAT_PREFIX #FormatName)); \
	static FName GUnprefixedTextureFormatName##FormatName = FName(TEXT(#FormatName));

ENUM_SUPPORTED_FORMATS(DECL_FORMAT_NAME);
#undef DECL_FORMAT_NAME

#define DECL_FORMAT_NAME_ENTRY(FormatName) \
	GPrefixedTextureFormatName##FormatName,\
	GUnprefixedTextureFormatName##FormatName,
...
static FName RemoveTFOPrefix(const FName & InName)
{
	#define REMOVEPREFIX_FORMAT_NAME(FormatName) \
		if ( InName == GPrefixedTextureFormatName##FormatName ) \
			return GUnprefixedTextureFormatName##FormatName;

	ENUM_SUPPORTED_FORMATS(REMOVEPREFIX_FORMAT_NAME);
	#undef REMOVEPREFIX_FORMAT_NAME
...
public:
	...
	void ImportFromConfigCache()
	{
		...
		{
			UE_LOGF(LogTextureFormatOodle, Display, "Enabling debug dump from command line: -OodleDebugDumpFilter=%ls", *CmdLineString);
			LocalDebugConfig.DebugDumpFilter = CmdLineString;
		}

		if (FParse::Param(FCommandLine::Get(), TEXT("OodleDebugColor")))
		{
			UE_LOGF(LogTextureFormatOodle, Display, "Enabling debug color encoding from command line (-OodleDebugColor)");
			bDebugColor = true;
		}
		...
		UE_LOGF(LogTextureFormatOodle,VerboseIfNotEditor,
			"Oodle Texture TFO init; latest sdk version = %ls",TEXT(OodleTextureVersion)
			);
		#ifdef DO_FORCE_UNIQUE_DDC_KEY_PER_BUILD
		UE_LOGF(LogTextureFormatOodle, Display, "Oodle Texture DO_FORCE_UNIQUE_DDC_KEY_PER_BUILD");
		#endif
	}
	...
	void GetOodleCompressParameters(EPixelFormat * OutCompressedPixelFormat,int * OutRDOLambda, OodleTex_EncodeEffortLevel * OutEffortLevel, bool * bOutDebugColor, OodleTex_RDO_UniversalTiling* OutRDOUniversalTiling, OodleTex_BCNFlags* OutBCNFlags, const struct FTextureBuildSettings& InBuildSettings, bool bHasAlpha) const
	{
		...
		FName TextureFormatName = RemoveTFOPrefix( InBuildSettings.BaseTextureFormatName );

		EPixelFormat CompressedPixelFormat = PF_Unknown;
		if (TextureFormatName == GUnprefixedTextureFormatNameDXT1)
		{
			CompressedPixelFormat = PF_DXT1;
		}
		else if (TextureFormatName == GUnprefixedTextureFormatNameDXT3)
		{
			CompressedPixelFormat = PF_DXT3;
		}
		else if (TextureFormatName == GUnprefixedTextureFormatNameDXT5)
		{
			CompressedPixelFormat = PF_DXT5;
		}
		else if (TextureFormatName == GUnprefixedTextureFormatNameAutoDXT)
		{
		...
		else if (TextureFormatName == GUnprefixedTextureFormatNameDXT5n)
		{
		...
		else if (TextureFormatName == GUnprefixedTextureFormatNameBC4)
		{
			CompressedPixelFormat = PF_BC4;
		}
		else if (TextureFormatName == GUnprefixedTextureFormatNameBC5)
		{
			CompressedPixelFormat = PF_BC5;
		}
		else if (TextureFormatName == GUnprefixedTextureFormatNameBC6H)
		{
			CompressedPixelFormat = PF_BC6H;
		}
		else if (TextureFormatName == GUnprefixedTextureFormatNameBC7)
		{
			CompressedPixelFormat = PF_BC7;
		}
		else
		{
			UE_LOGF(LogTextureFormatOodle,Fatal,
				"Unsupported TextureFormatName for compression: %ls",
		...
		*OutCompressedPixelFormat = CompressedPixelFormat;
		...
		{
			UE_LOGF(LogTextureFormatOodle, Warning, "Invalid effort level passed to texture format oodle: %d is invalid, using default", (uint32)EffortLevel);
		...
		{
			UE_LOGF(LogTextureFormatOodle, Warning, "Invalid universal tiling value passed to texture format oodle: %d is invalid, disabling", (uint32)UniversalTiling);
...
public:
	...
	bool Init()
	{
		...
		// verify the latest DLL can be found :
		if ( GetOodleTextureVTable(OodleTextureVersionLatest) == nullptr )
		{
			UE_LOGF(LogTextureFormatOodle,Warning,
				"Required Oodle Texture versions not available : (%ls), will be disabled.",
	...
	FName ValidateOodleTextureSdkVersion(FName DesiredOodleTextureSdkVersion) const
	{
		...
		if (VTable == nullptr)
		{
			...
			{
				// ?? Warning ??
				UE_LOGF(LogTextureFormatOodle,Display,
					"Unsupported OodleTextureSdkVersion: %ls ; instead using: %ls",
			...
			const void * PixelData, size_t PixelDataSize)
	{
		if (DebugFormat != UE::DDS::EDXGIFormat::UNKNOWN)
		{
			...
			{
				UE_LOGF(LogTextureFormatOodle, Warning, "DebugDump mip sizes don't match %dx%d: %lld != %lld", SizeX,SizeY, DDS->Mips[0].DataSize, PixelDataSize);
			}
			...
			{
				UE_LOGF(LogTextureFormatOodle, Error, "Failed to open DDS debug file: %ls", *FileName);
			}
	...
	virtual bool DecodeImage(int32 InSizeX, int32 InSizeY, int32 InNumSlices, EPixelFormat InPixelFormat, bool bInSRGB, const FName& InTextureFormatName, FSharedBuffer InEncodedData, FImage& OutImage, FStringView InTextureName) const
	{
		...
		for (int32 Slice = 0; Slice < InNumSlices; Slice++)
		{
			...
			{
				UE_LOGF(LogTextureFormatOodle, Error, "Failed to decode %s", (VTable->fp_OodleTex_Err_GetName)(Result));
				return false;
			}
		}
		return true;
		...
		int32 InMip0NumSlicesNoDepth, int32 InMipIndex, int32 InMipCount, FStringView DebugTexturePathName, const bool bInHasAlpha, FCompressedImage2D& OutImage) const override
	{
		...
		{
			UE_LOGF(LogTextureFormatOodle,Error,
				"Image larger than OODLETEX_MAX_SURFACE_DIMENSION : %dx%d > %d",
		...
		{
			UE_LOGF(LogTextureFormatOodle,Fatal,
				"Unsupported CompressedPixelFormat for compression: %d",
		...
		if ( InBuildSettings.OodleTextureSdkVersion.IsNone() )
		{
			...
			// use default:
			CompressOodleTextureVersion = ValidateOodleTextureSdkVersion(OodleTextureSdkVersionToUseIfNone);
		}
		else
		{
		...
		{
			UE_LOGF(LogTextureFormatOodle,Error,
				"Unsupported OodleTextureSdkVersion: %ls",
		...
		FName TextureFormatName = RemoveTFOPrefix( InBuildSettings.TextureFormatName );
		...
		{
			UE_LOGF(LogTextureFormatOodle, Display, "Oodle%ls %ls encode %i x %i x %i to format %ls%ls (%ls) lambda=%i effort=%i ",
				*CompressOodleTextureVersion.ToString(),
			...
			Gamma != EGammaSpace::Linear )
		{
			...
			UE_LOGF(LogTextureFormatOodle, Display, "Image format %ls (Oodle %ls) encoded with non-Linear Gamma", \
		...
		bool bNeedsImageCopy = ImageFormat != InImage.Format ||
			...
			(CompressedPixelFormat == PF_DXT5 && TextureFormatName == GUnprefixedTextureFormatNameDXT5n) ||
		...
		if ( CompressedPixelFormat == PF_DXT5 &&
			TextureFormatName == GUnprefixedTextureFormatNameDXT5n)
		{
		...
		/*
		UE_LOGF(LogTextureFormatOodle, Display, "bHasAlpha=%d OodlePF=%d=%ls OodleBCN=%d=%ls",
			(int)bHasAlpha,
			(int)OodlePF,
		...
		ParallelFor(
			...
			[&](int32 Slice)
			{
...

				{
					...
					if (OodleErr != OodleTex_Err_OK)
					{
						// First to fail logs the error
						if (bCompressionSucceeded.exchange(false) == true)
						{
							if (OodleErr == OodleTex_Err_Canceled)
							{
								// Cancelations are not real errors and don't belong in the "Warning" category.
								UE_LOGF(LogTextureFormatOodle, Display, "Oodle Texture encode cancelled by user request.");
							}
							else
							{
								const char * OodleErrStr = (VTable->fp_OodleTex_Err_GetName)(OodleErr);
								UE_LOGF(LogTextureFormatOodle, Warning, "Oodle Texture encode failed!? %d=%ls", (int)OodleErr, ANSI_TO_TCHAR(OodleErrStr) );
							}
						}
						return;
					}
...
static OO_U64 OODLE_CALLBACK TFO_RunJob(t_fp_Oodle_Job* JobFunction, void* JobData, OO_U64* Dependencies, int NumDependencies, void* UserPtr)
{
	...
	// We want our tasks to inherit our cancellation token
	FCancellationToken* CancellationToken = UE::Tasks::FCancellationTokenScope::GetCurrentCancellationToken();

	FTask* Task = new FTask;
	Task->Launch(
		TEXT("TFOodle.EncodeBCN_Task"),
		[JobFunction, JobData, CancellationToken]
		{
			TRACE_CPUPROFILER_EVENT_SCOPE(TFOodle.EncodeBCN_Task);
			UE::Tasks::FCancellationTokenScope CancelScope(CancellationToken);
			JobFunction(JobData);
		},
...
static OO_BOOL OODLE_CALLBACK TFO_OodleCancelRequested(void* UserPtr)
{
	// DebugInfo to inspect:
	FOodleJobDebugInfo* DebugInfo = (FOodleJobDebugInfo*) UserPtr;
...
static void TFO_Plugins_Init()
{
	...
	if (OodleJobifyUseExampleJobify)
	{
		UE_LOGF(LogTextureFormatOodle, Display, "Using Oodle Example Jobify");
...
static void TFO_Plugins_Install(const FOodleTextureVTable * VTable)
{
	...
	else
	{
		...
		// Cancellation support hangs of task graph cancellation tokens, not supported in
		// ExampleJobify mode
		if (VTable->fp_OodleTex_Plugins_SetCancelRequested)
		{
			(VTable->fp_OodleTex_Plugins_SetCancelRequested)(TFO_OodleCancelRequested);
		}
	}
...
public:
	...
	virtual ITextureFormat* GetTextureFormat()
	{
		...
		auto MakeTextureFormat = [&]()
		{
			...
			{
				UE_LOGF(LogTextureFormatOodle,Warning,"Oodle Texture Init failed, not installed");
				delete ptr;
		}
		};
