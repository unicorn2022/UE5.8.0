...
TEST_CASE("Shaders::ShaderPreprocessor::ConditionalExpression", "[EditorContext][EngineFilter]")
{
	int Result;
...
FPreprocessTestResult ExecutePreprocessTest(const FShaderSource& Source)
{
	...
	FShaderSource::FViewType SourceView = Source.GetView();
	Result.Source = preprocess_file("preprocessor_test", &SourceView, nullptr, 0u, &Result.Diagnostics, &Result.NumDiagnostics, nullptr, 0u);
	return Result;
}

TEST_CASE("Shaders::ShaderPreprocessor::ConditionalExpressionErrorReporting", "[EditorContext][EngineFilter]")
{
	...
	FShaderSource Source(ANSITEXTVIEW(
		"#" "if 0xFFFFFFFF+1 == 2\n"
		"#" "error \"Unreachable\n\""
		"#" "endif\n"));
...
TEST_CASE("Shaders::ShaderPreprocessor::InvalidCharErrors", "[EditorContext][EngineFilter]")
{
	{
		// EOF after #define
		FShaderSource Source(ANSITEXTVIEW(
...
TEST_CASE("Shaders::ShaderPreprocessor::BuiltinMacros", "[EditorContext][EngineFilter]")
{
	FShaderSource Source(ANSITEXTVIEW(
		"int A = __" "LINE__;\n"
		"int B = __" "COUNTER__;\n"
...
TEST_CASE("Shaders::ShaderPreprocessor::EmitLineDirective", "[EditorContext][EngineFilter]")
{
