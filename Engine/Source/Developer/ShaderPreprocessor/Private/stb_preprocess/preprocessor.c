...
static int copy_to_action_point(parse_state* cs)
{
	...
	for (;;)
	{
		...
		cs->dest = out;  // arrsetcap may realloc 'out' to a new address; keep cs->dest in sync so copy_and_filter_macro uses a valid pointer
		q = out + out_len;
...
static void maybe_expand_macro(parse_state* cs, struct macro_definition* pending, int in_macro_expansion, char* in_macro, struct macro_definition** final_function_macro)
{
	...
	arrsetlen(cs->dest, arrlen(cs->dest)-1);
		...
		for (i = 0; i < md->num_parameters; ++i)
		{
			...
			{
				char* e;
				// if this is a multi-line argument maintain the whitespace by re-inserting the leading ws (this assumes multiline macro arguments are not token pasted)
				if (arg_newlines > 0)
				{
				...
				arrput(copy, 0); // add null terminator to working array

				p = copy;
				e = copy + arrlen(copy) - 1;  // get address of null terminator
				ptrdiff_t len = e - p;			  // get length excluding null terminator
				...
				if (arg_newlines == 0)
				{
					...
					*e = 0;
					len = e - p; // reset new length after stripping trailing ws
				}
				...
				arrsetlen(copy, len); // set argument string len to exclude null terminator
...
void init_preprocessor_scanner(void)
{
	...
	// Disable V557 due to false positive and the use of STATE_DEFAULT
	//-V:transition:557
