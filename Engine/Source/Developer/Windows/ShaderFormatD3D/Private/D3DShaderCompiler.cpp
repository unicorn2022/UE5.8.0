	...
	)
{
	...
	switch (CompileFlags & SHADER_OPTIMIZATION_LEVEL_MASK)
	{
		...
		default:
			Output.AddErrorf(TEXT("Unknown D3DCOMPILE optimization level"));
			break;
	}
...
private:
	FxcCompilerFunctions()
	{
		...
		if (!CompilerDLL)
		{
			UE_LOGF(LogD3DShaderCompiler, Fatal, "Cannot find the compiler DLL '%ls'", *CompilerPath);
		}
	...
	TArray<FShaderCompilerError>& OutErrors)
{
	...
	if (!UE::HlslParser::FindEntryPointParameters(OriginalShaderSource, OriginalEntryPoint, StageVariablesStorageClass, {}, Variables, ParsingErrors))
	{
		...
		{
			OutErrors.Emplace(MoveTemp(Error));
		}
		return false;
	}
	...
	FShaderCompilerOutput& Output)
{
	...
	if (D3DCompileFunc)
	{
		...
		const bool bFxcCompilerFailed = Result == HRESULT_FROM_WIN32(ERROR_ARITHMETIC_OVERFLOW) || Result == E_OUTOFMEMORY || Result == E_FAIL || (Result != S_OK && CompileErrorsContainInternalError(Errors.GetReference()));
		if (bPrecompileWithDXC || bFxcCompilerFailed)
		{
			// If we're cross-compiling HLSL to simplified HLSL due to an FXC error, force diagnostics output and dump debug information.
			// This is meant to help track down potential issues that only occur at PSO creation time.
			if (bFxcCompilerFailed)
			{
				Output.bForceDumpDebugInfo = true;
				Output.bForceLogDiagnosticMessages = true;
			}
			...
			{
				Output.AddErrorf(TEXT("Validation error in FXC encountered: Compiling intermediate HLSL a second time with simplified control flow"));
				...
				if (Result == E_FAIL && D3DCompileErrorContainsValidationErrors(Errors.GetReference()))
				{
					Output.AddErrorf(TEXT("Validation error in FXC encountered: Compiling intermediate HLSL a third time without optimization (D3DCOMPILE_SKIP_OPTIMIZATION)"));
			...
			if (bFxcCompilerFailed && SUCCEEDED(Result))
			{
				...
				Output.AddErrorf(TEXT("Cross-compiled shader to intermediate HLSL after first attempt crashed FXC"));
	...
	{
		Output.AddErrorf(TEXT("Couldn't find D3D shader compiler DLL"));
		Result = E_FAIL;
	}
	...
	if (ErrorBuffer)
	{
		...
		for (int32 ErrorIndex = 0; ErrorIndex < FilteredErrors.Num(); ErrorIndex++)
		{
			...
			Output.Errors.Emplace(MoveTemp(NewError));
		}
	}
		...
		if (GD3DCheckForDoubles || bCheckForTypedUAVs || bDumpDebugInfo)
		{
			...
			if (SUCCEEDED(D3DDisassembleFunc(Shader->GetBufferPointer(), Shader->GetBufferSize(), 0, "", Disassembly.GetInitReference())))
			{
				...
				if (GD3DCheckForDoubles)
				{
					...
					{
						Output.AddErrorf(TEXT("Shader uses double precision floats, which are not supported on all D3D11 hardware!"));
						return false;
					}
				}
				...
				if (bCheckForTypedUAVs)
				{
					...
					{
						Output.AddErrorf(TEXT("Shader uses UAV loads from additional typed formats, which are not supported on all D3D11 hardware! Set r.D3D.CheckedForTypedUAVs=0 if you want to allow typed UAV loads for your project, or individual shaders can opt-in by specifying CFLAG_AllowTypedUAVLoads."));
						return false;
					}
				}
			}
	...
	if (SUCCEEDED(Result))
	{
		...
		if (D3DReflectFunc)
		{
			...
			if (FAILED(Result))
			{
				UE_LOGF(LogD3DShaderCompiler, Fatal, "D3DReflect failed: Result=%08x", Result);
			}
		...
		{
			Output.AddErrorf(TEXT("Couldn't find shader reflection function in D3D Compiler DLL"));
			Result = E_FAIL;
		...
		{
			Output.AddErrorf(TEXT("Number of UAVs exceeded limit: %d slots used, but limit is %d due to maximum feature level 11.0"), CompileData.NumUAVs, GD3DMaximumNumUAVs);
			Result = E_FAIL;
			Output.bSucceeded = false;
		}
		...
		if (Output.bSucceeded)
		{
			...
			if ((bGenerateSymbols || bGenerateSymbolsInfo) && D3DGetBlobPartFunc && D3DGetDebugInfoFunc)
			{
				...
				if (SUCCEEDED(Result))
				{
					...
					if (bGenerateSymbols)
					{
						...
						{
							Output.AddErrorf(TEXT("Symbol generation was requested, but no PDB blob exists in the compiler output."));
							Result = E_FAIL;
							Output.bSucceeded = false;
						}
					}
				...
				{
					Output.AddErrorf(TEXT("Symbol or symbols info generation was requested, but no debug name blob exists in the compiler output."));
					Output.bSucceeded = false;
				}
			}
			...
			if (D3DStripShaderFunc)
			{
				...
				if (FAILED(Result))
				{
					UE_LOGF(LogD3DShaderCompiler, Fatal, "D3DStripShader failed: Result=%08x", Result);
				}
			}
			else
			{
...
void CompileD3DShader(const FShaderCompilerInput& Input, const FShaderPreprocessOutput& InPreprocessOutput, FShaderCompilerOutput& Output, ED3DShaderModel ShaderModel)
{
	...
	if (!ShaderProfile)
	{
		Output.AddErrorf(TEXT("Unrecognized shader frequency %s"), GetShaderFrequencyString(Input.Target.GetFrequency()));
		return;
	}
	...
	if (Input.Environment.CompilerFlags.Contains(CFLAG_ForceRemoveUnusedInterpolators) && Input.Target.Frequency == SF_Vertex && Input.bCompilingForShaderPipeline)
	{
		...
		(void)UE::ShaderCompilerCommon::RemoveUnusedOutputs(Input, Output, PreprocessedSource, EntryPointName, UsedOutputs, Exceptions, ScopedDeclarations);
	}
	...
	{
		Output.AddErrorf(TEXT("Compile failed without errors!"));
	}
