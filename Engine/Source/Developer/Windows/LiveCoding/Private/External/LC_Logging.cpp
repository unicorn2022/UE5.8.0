...
namespace
{
	...
	static void DefaultOutputHandler(Logging::Channel::Enum channel, Logging::Type::Enum type, const wchar_t* const Message)
	{
		...
		case Logging::Type::LOG_WARNING:
			UE_LOGF(LogLiveCoding, Warning, "%ls", *MessageWithoutNewline);
			break;
		case Logging::Type::LOG_ERROR:
			UE_LOGF(LogLiveCoding, Error, "%ls", *MessageWithoutNewline);
			break;
		default:
			UE_LOGF(LogLiveCoding, Display, "%ls", *MessageWithoutNewline);
			break;
		}
	}
