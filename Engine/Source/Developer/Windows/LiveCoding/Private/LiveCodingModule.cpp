...
public:
	...
	virtual bool GetEnableReinstancing(bool bHasChanged) const
	{
		if (bHasChanged && !bEnabledMessage)
		{
			...
			UE_LOGF(LogLiveCoding, Error, "%ls", Message);
#if WITH_ENGINE
			if (GEngine)
			{
...
void FLiveCodingModule::WaitForStartup()
{
	...
	{
		UE_LOGF(LogLiveCoding, Display, "Waiting for console to start.");
...
void FLiveCodingModule::HideConsole()
{
	if (HasStarted())
	{
		UE_LOGF(LogLiveCoding, Display, "Console will be hidden but remain running in the background. Restart to disable completely.");
		LppSetActive(false);
		LppSetVisible(false);
	}
}
		...
		if (!GIsCompileActive)
		{
			...
			case commands::PostCompileResult::Success:
				...
				if (bHasReinstancingOccurred)
				{
					...
					{
						UE_LOGF(LogLiveCoding, Warning, "%ls, %ls", *Success, TEXT("data type changes with re-instancing disabled is not supported and will likely lead to a crash"));
					}
					else
					{
#if WITH_EDITOR
						UE_LOGF(LogLiveCoding, Warning, "%ls, %ls", *Success, TEXT("data type changes may cause packaging to fail if assets reference the new or updated data types"));
#else
						UE_LOGF(LogLiveCoding, Warning, "%ls, %ls", *Success, TEXT("data type changes may cause unexpected failures"));
#endif
					}
				}
				else
				{
					UE_LOGF(LogLiveCoding, Display, "%ls", *Success);
				}
				break;
			case commands::PostCompileResult::NoChanges:
				LastResults = ELiveCodingCompileResult::NoChanges;
				UE_LOGF(LogLiveCoding, Display, "%ls, %ls", *Success, TEXT("no code changes detected"));
				break;
			case commands::PostCompileResult::Cancelled:
				LastResults = ELiveCodingCompileResult::Cancelled;
				UE_LOGF(LogLiveCoding, Error, "Live coding canceled");
				break;
			case commands::PostCompileResult::Failure:
				LastResults = ELiveCodingCompileResult::Failure;
				UE_LOGF(LogLiveCoding, Error, "Live coding failed, please see Live console for more information");
				break;
			default:
				LastResults = ELiveCodingCompileResult::Failure;
				check(false);
...
bool FLiveCodingModule::SetupConsolePath()
{
	...
	if (!FPaths::FileExists(GLiveCodingConsolePath))
	{
		...
		if (!FPaths::FileExists(CodingConsolePathFromExecutable))
		{
			...
			UE_LOGF(LogLiveCoding, Error, "Unable to start live coding session. Missing executable '%ls'. Use the LiveCoding.ConsolePath console variable to modify.", *GLiveCodingConsolePath);
			return false;
		}
...
bool FLiveCodingModule::StartLiveCoding(ELiveCodingStartupMode StartupMode)
{
	...
	if (!HasStarted())
	{
		...
		if (!CanEnableForSession())
		{
			...
			UE_LOGF(LogLiveCoding, Error, "Unable to start live coding session. Some modules have already been hot reloaded.");
			State = EState::NotRunning;
			return false;
		}
		...
		if (SourceProject.Len() > 0 && !FPaths::FileExists(SourceProject))
		{
			...
			EnableErrorText = FText::Format(FormatString, Args);
			UE_LOGF(LogLiveCoding, Error, "Unable to start live coding session. Unable to find source project file '%ls'.", *SourceProject);
			State = EState::NotRunning;
			return false;
		}
		...
		UE_LOGF(LogLiveCoding, Display, "Starting LiveCoding");
		...
		{
			FFunctionGraphTask::CreateAndDispatchWhenReady(MoveTemp(FinalizeStartup), TStatId(), nullptr, ENamedThreads::GameThread);
		}
	}
	return true;
}
...
void FLiveCodingModule::SetBuildArguments()
{
	...
	if (KnownTargetName != LastKnownTargetName)
	{
		...
		UE_LOGF(LogLiveCoding, Display, "LiveCodingConsole Arguments: %ls", *Arguments);
...
{
	UE_LOGF(LogLiveCoding, Display, "Starting Live Coding compile.");
	GIsCompileActive = true;
	if (GLiveCodingModule != nullptr)
	{
