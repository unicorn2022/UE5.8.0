...
{
	UE_LOGF(LogStandaloneRenderer, Warning, "%ls Result: %ls [%X]", *Description, *GetReadableResult(Hr), Hr);

	if (Hr == DXGI_ERROR_DEVICE_REMOVED && IsValidRef(GD3DDevice))
	{
		HRESULT ReasonHr = GD3DDevice->GetDeviceRemovedReason();
		UE_LOGF(LogStandaloneRenderer, Warning, "%ls Reason: %ls [0x%08X]", *Description, *GetReadableResult(ReasonHr), ReasonHr);
	}
}
...
void FSlateD3DRenderer::RegisterCurrentScene_Impl(FSceneInterface* Scene)
{
	// This is a no-op
}
