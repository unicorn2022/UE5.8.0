...
bool FSlateD3DTextureManager::LoadTexture( const FSlateBrush& InBrush, uint32& OutWidth, uint32& OutHeight, TArray<uint8>& OutDecodedImage )
{
	...
	if( FFileHelper::LoadFileToArray( RawFileData, *ResourcePath, FILEREAD_Silent) )
	{
		...
		if ( ImageWrapper.IsValid() && ImageWrapper->SetCompressed( RawFileData.GetData(), RawFileData.Num() ) )
		{
			...
			{
				UE_LOGF(LogSlateD3D, Warning, "Invalid texture format %d for Slate resource only RGBA and RGB pngs are supported: %ls", int(ImageFormat), *InBrush.GetResourceName().ToString() );
			}
		}
		else
		{
			UE_LOGF(LogSlateD3D, Warning, "Only pngs are supported in Slate. [%ls] '%ls'", *InBrush.GetResourceName().ToString(), *ResourcePath);
		}
	}
	else
	{
		UE_CLOGF(!IsRunningCommandlet(), LogSlateD3D, Warning, "Could not load file for Slate resource: [%ls] '%ls'", *InBrush.GetResourceName().ToString(), *ResourcePath);
	}
