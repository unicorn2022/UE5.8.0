...
void FFindBestDevice::LoadSettings()
{
	...
	UE_LOGF(LogStandaloneRenderer, Log, "D3D11 adapters settings:");
	if (ExplicitAdapterValue >= 0)
	{
		UE_LOGF(LogStandaloneRenderer, Log, "ExplicitAdapterValue=%d", ExplicitAdapterValue);
	}
	if (PreferedAdapterVendor >= 0)
	{
		UE_LOGF(LogStandaloneRenderer, Log, "PreferedAdapterVendor=%d", PreferedAdapterVendor);
	}
	if (bAllowSoftwareRendering)
	{
		UE_LOGF(LogStandaloneRenderer, Log, "bAllowSoftwareRendering=%ls", (bAllowSoftwareRendering ? TEXT("true") : TEXT("false")));
	}
	if (bPreferedMinimalPower)
	{
		UE_LOGF(LogStandaloneRenderer, Log, "bPreferedMinimalPower=%ls", (bPreferedMinimalPower ? TEXT("true") : TEXT("false")));
	}
	else if (bPreferedHighPerformance)
	{
		UE_LOGF(LogStandaloneRenderer, Log, "bPreferedHighPerformance=%ls", (bPreferedHighPerformance ? TEXT("true") : TEXT("false")));
	}
	if (bDebug)
	{
		UE_LOGF(LogStandaloneRenderer, Log, "bPreferedHighPerformance=%ls", (bPreferedHighPerformance ? TEXT("true") : TEXT("false")));
...
bool FFindBestDevice::CreateTesingDevice(IDXGIAdapter* Adapter, D3D_FEATURE_LEVEL& ResultFeatureLevel)
{
	...
	__try
	{
		...
		{
			UE_LOGF(LogStandaloneRenderer, Log, "Failed to create device [0x%08X]", CreateDeviceResult);
		}
	}
...
void FFindBestDevice::SelectAdapter()
{
	...
	UE_LOGF(LogStandaloneRenderer, Log, "D3D11 adapters:");
	...
	for (uint32 AdapterIndex = 0; EnumerateAdapters(AdapterIndex, TestingAdapter.GetInitReference()) != DXGI_ERROR_NOT_FOUND; ++AdapterIndex)
	{
		...
		{
			UE_LOGF(LogStandaloneRenderer, Warning, "Failed to get description for adapter %u.", AdapterIndex);
		}
		else
		{
			UE_LOGF(LogStandaloneRenderer, Log, "Testing D3D11 adapter: %u. Description: '%ls'. VendorId: %04x. DeviceId: %04x."
		...
		{
			UE_LOGF(LogStandaloneRenderer, Log, "  Failed to create test device.");
			continue;
		}

		UE_LOGF(LogStandaloneRenderer, Log, "  %u. '%ls'. Feature level: %ls", AdapterIndex, AdapterDesc.Description, GetFeatureLevelString(FeatureLevel));
		...
		if (bSkipAdapter)
		{
			UE_LOGF(LogStandaloneRenderer, Log, "  Skip adapter.");
			continue;
		}
	...
	if (ChosenAdapter.IsValid())
	{
		...
		{
			UE_LOGF(LogStandaloneRenderer, Warning, "Failed to get description for selected adapter.");
		}
		else
		{
			UE_LOGF(LogStandaloneRenderer, Log, "Selected D3D11 Description: '%ls'. VendorId: %04x. DeviceId: %04x."
