...
void FSlateOpenGLContext::Initialize( void* InWindow, const FSlateOpenGLContext* SharedContext )
{
	...
	if (SDL_GL_LoadLibrary(nullptr) == false)
	{
		...
		UE_LOGF(LogInit, Fatal, "FSlateOpenGLContext::Initialize - Unable to dynamically load libGL: %ls\n.", *SdlError);
	}
	...
	UE_LOGF(LogInit, Log, "FSlateOpenGLContext::Initialize - creating OpenGL %d.%d context", OpenGLMajorVersionToUse, OpenGLMinorVersionToUse);
	...
	if (Context == nullptr)
	{
		...
		UE_LOGF(LogInit, Fatal, "FSlateOpenGLContext::Initialize - Could not create OpenGL %d.%d context, SDL error: '%ls'", 
	...
	if (PlatformOpenGLDebugCtx())
	{
		...
		#endif
				UE_LOGF(LogStandaloneRenderer,Verbose,"disabling reporting back of debug groups and markers to the OpenGL debug output callback");
			}
