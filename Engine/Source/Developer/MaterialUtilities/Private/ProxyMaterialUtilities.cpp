...
static FString GetMatchingParamName(EFlattenMaterialProperties InProperty, UMaterialInterface* InBaseMaterial)
{
	...
	{
		UE_LOGF(LogProxyMaterialUtilities, Fatal, "Invalid base material, should have been rejected by IsValidFlattenMaterial()");
	}
	return ParamName;
}
...
bool FMaterialUtilities::IsValidFlattenMaterial(const UMaterialInterface* InBaseMaterial)
{
	if (InBaseMaterial != nullptr)
	{
		...
		if (MissingParameters.Num() > 0)
		{
			...
			UE_LOGF(LogProxyMaterialUtilities, Warning, "Material %ls is missing required Material Parameters %ls", *InBaseMaterial->GetName(), *MissingNamesString);
