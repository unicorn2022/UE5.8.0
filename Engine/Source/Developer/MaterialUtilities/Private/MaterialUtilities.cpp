...
class FExportMaterialProxy : public FEditorMaterialResourceBase, public FMaterialRenderProxy
{
public:
		...
		, PropertyToCompile(InPropertyToCompile)
	{
		...
		ReferencedTextures = InMaterialInterface->GetDefaultTextures();
	...
	*/
	...
	int32 CompilePropertyAndSetMaterialPropertyWithoutCast(EMaterialProperty Property, FMaterialCompiler* Compiler) const
	{
		...
		{
			// Return the proper default value for each property so the translator doesn't
			// falsely detect usage of features like WPO, anisotropy, etc.
	...
	FFlattenMaterial& OutFlattenMaterial)
{
	...
	for (int32 PropertyIndex = 0; PropertyIndex < (int32)EFlattenMaterialProperties::NumFlattenMaterialProperties; ++PropertyIndex)
	{
		...
		if (OutFlattenMaterial.ShouldGenerateDataForProperty(Property))
		{
			...
			{
				UE_LOGF(LogMaterialUtilities, Error, "RenderSceneToTextures - Ignoring unsupported property");
			}
		}
	}
}
...
bool FMaterialUtilities::ExportLandscapeMaterial(const ALandscapeProxy* InLandscape, FFlattenMaterial& OutFlattenMaterial)
{
	...
	if (!bExportSuccess)
	{
		UE_LOGF(LogMaterialUtilities, Warning, "ExportLandscapeMaterial: Failed to export material for the provided ALandcapeProxy (%ls)", InLandscape ? *InLandscape->GetName() : TEXT("<null>"));
	}
...
UMaterial* FMaterialUtilities::CreateMaterial(const FFlattenMaterial& InFlattenMaterial, UPackage* InOuter, const FString& BaseName, EObjectFlags Flags, const struct FMaterialProxySettings& MaterialProxySettings, TArray<UObject*>& OutGeneratedAssets, const TextureGroup& InTextureGroup /*= TEXTUREGROUP_World*/)
{
	...
	{
		UE_LOGF(LogMaterialUtilities, Warning, "Invalid long package name: '%ls'.", *BaseName);
		return nullptr;
	}
...
FFlattenMaterial FMaterialUtilities::CreateFlattenMaterialWithSettings(const FMaterialProxySettings& InMaterialLODSettings)
{
	...
	{
		UE_LOGF(LogMaterialUtilities, Error, "Unsupported TextureSizingType value. You should resolve the material texture size first with ResolveTextureSize()");
	}
	...
	for (TMap<FError, TArray<FInstance> >::TIterator It(ErrorInstances);; ++It)
	{
		...
		if (!It || CurrentMaterial != It->Value[0].Material)
		{
			...
			if (CurrentMaterial)
			{
				...
				{
					UE_LOGF(TextureStreamingBuild, Verbose, "Incomplete texcoord scale analysis for %ls%ls: %ls", *CurrentMaterial->GetName(), *SimilarCount, *TextureErrors);
				}
				else
				{
					UE_LOGF(TextureStreamingBuild, Verbose, "Incomplete texcoord scale analysis for %ls, UMaterial=%ls%ls: %ls", *CurrentMaterial->GetName(), *CurrentMaterial->GetMaterial()->GetName(), *SimilarCount, *TextureErrors);
				}
			}
...
int32 FMaterialUtilities::GetTextureSizeFromTargetTexelDensity(double InMesh3DArea, double InMeshUVArea, double InTargetTexelDensity)
{
	...
	{
		UE_LOGF(LogMaterialUtilities, Warning, "Mesh would require %d x %d textures, clamping down to maximum (%d x %d)", TextureSize, TextureSize, MaxTextureSize, MaxTextureSize);
	}
