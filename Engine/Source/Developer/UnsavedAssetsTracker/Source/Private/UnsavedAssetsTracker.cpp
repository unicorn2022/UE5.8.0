...
namespace
{
static bool bSyncUnsavedAssetInfoToTEDS = true;
FAutoConsoleVariableRef CVarSyncUnsavedInformation(
	TEXT("TEDS.RevisionControl.SyncUnsavedInformation"),
	bSyncUnsavedAssetInfoToTEDS,
	TEXT("Automatically sync information about unsaved assets to TEDS")
);
...
void FUnsavedAssetsTracker::OnPackageDeleted(UPackage* Package)
{
	...
	RemoveTedsPackage(Package->GetPathName());
}
...
void FUnsavedAssetsTracker::SyncWithDirtyPackageList()
{
	...
	// The list of dirty packages tracked by the engine (considered source of truth)
...
void FUnsavedAssetsTracker::StartTrackingDirtyPackage(const UPackage* Package)
{
	...
	UpdateTedsPackage(PackagePathname, true);

	FUnsavedAssetsTrackerModule::Get().OnUnsavedAssetAdded.Broadcast(AbsPackageFilePathname);

	UE_LOGF(LogUnsavedAssetsTracker, Verbose, "Added file to the unsaved asset list: %ls (%ls)", *GetHumanFriendlyAssetName(Package), *PackagePathname);
}
...
void FUnsavedAssetsTracker::StopTrackingDirtyPackage(const FString& PackagePathname)
{
	...
	if (FStatus* Status = UnsavedPackages.Find(PackagePathname))
	{
		...
		UpdateTedsPackage(PackagePathname, false);
		...
		UE_LOGF(LogUnsavedAssetsTracker, Verbose, "Removed file from the unsaved asset list: %ls", *PackagePathname);
	}
}

void FUnsavedAssetsTracker::UpdateTedsPackage(const FString& PackagePathname, const bool bIsDirty)
{
	using namespace UE::Editor::DataStorage;

	if (bSyncUnsavedAssetInfoToTEDS)
	{
		if (ICoreProvider* Storage = GetMutableDataStorageFeature<ICoreProvider>(StorageFeatureName))
		{
			if (FString PackagePath; FPackageName::DoesPackageExist(PackagePathname, &PackagePath))
			{
				FPaths::NormalizeFilename(PackagePath);
				FString FullPackageFilename = FPaths::ConvertRelativePathToFull(PackagePath);
				if (const RowHandle PackageRow = Storage->LookupMappedRow(UE::Editor::RevisionControl::MappingDomain, FMapKey(FullPackageFilename)); Storage->IsRowAvailable(PackageRow))
				{
					TArray<RowHandle> ChildrenRowHandles; // Rows that remain in the package, but need to sync IsDirty
					TArray<RowHandle> RemapChildrenRowHandles; // Rows that are no longer in the package and need to be resynced in the processor
					auto GetChildRows = [&ChildrenRowHandles, &RemapChildrenRowHandles, PackagePathname](const ICoreProvider& Context, RowHandle, RowHandle TargetRow)
					{
						if (const FTypedElementUObjectColumn* ObjectColumn = Context.GetColumn<FTypedElementUObjectColumn>(TargetRow))
						{
							if (const UObject* ObjectInstance = ObjectColumn->Object.Get(); ObjectInstance != nullptr)
							{
								// If the package of the object is null or isn't the same, we need to resync this object row to its current package
								if (const UPackage* Target = ObjectInstance->GetPackage())
								{
									FString Path;
									Target->GetPathName(nullptr, Path);
									if (!Path.Equals(PackagePathname))
									{
										RemapChildrenRowHandles.Add(TargetRow);
										return;
									}
								}
								else
						...
						ChildrenRowHandles.Add(TargetRow);
					};
					Storage->WalkDepthFirst(Storage->FindHierarchyByName(PackageHierarchyName), PackageRow, GetChildRows);

					// Remove package columns from any rows that don't belong to this package anymore so they can resync
					Storage->BatchAddRemoveColumns(
						TConstArrayView<RowHandle>(RemapChildrenRowHandles),
						{},
						{FTypedElementPackageReference::StaticStruct(), FTedsPackageDirtyTag::StaticStruct(), FTedsPrimaryPackageObjectTag::StaticStruct()});
					

					// Any rows still in the package should update to the correct IsDirty state
					for (const RowHandle ChildRow : ChildrenRowHandles)
					{
						if (bIsDirty)
						{
							Storage->AddColumns<FTedsPackageDirtyTag>(ChildRow);
						}
						else
						{
							Storage->RemoveColumn<FTedsPackageDirtyTag>(ChildRow);
						}
					}
				}
			}
...
void FUnsavedAssetsTracker::RemoveTedsPackage(const FString& PackagePathname)
{
	using namespace UE::Editor::DataStorage;
	...
	if (bSyncUnsavedAssetInfoToTEDS)
	{
		if (ICoreProvider* Storage = GetMutableDataStorageFeature<ICoreProvider>(StorageFeatureName))
		{
			if (FString PackagePath; FPackageName::DoesPackageExist(PackagePathname, &PackagePath))
			{
				...
				if (const RowHandle PackageRow = Storage->LookupMappedRow(UE::Editor::RevisionControl::MappingDomain, FMapKey(FullPackageFilename)); Storage->IsRowAvailable(PackageRow))
				{
					TArray<RowHandle> ChildRowHandles;
					auto GetChildRows = [&ChildRowHandles](const ICoreProvider&, RowHandle, RowHandle TargetRow)
						{
							ChildRowHandles.Add(TargetRow);
						};
					Storage->WalkDepthFirst(Storage->FindHierarchyByName(PackageHierarchyName), PackageRow, GetChildRows);

					Storage->BatchAddRemoveColumns(
						TConstArrayView<RowHandle>(ChildRowHandles),
						{},
						{FTypedElementPackageReference::StaticStruct(), FTedsPackageDirtyTag::StaticStruct(), FTedsPrimaryPackageObjectTag::StaticStruct()});
					
					Storage->RemoveRow(PackageRow);
				}
			}
		}
	}
