...
private:
	...
	/** Adds the Dirty tag to the Package and its contained object rows */
	static void UpdateTedsPackage(const FString& PackagePathname, const bool bIsDirty);
	
	/** Removes the Package row from TEDS */
	static void RemoveTedsPackage(const FString& PackagePathname);
