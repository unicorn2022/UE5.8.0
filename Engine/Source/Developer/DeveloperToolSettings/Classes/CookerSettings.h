...
public:
	...
	/** Enable IncrementalCooks for launch on */
	UPROPERTY(GlobalConfig, EditAnywhere, Category = Cooker, meta = (DisplayName = "Use Incremental Cook for builds launched from the editor (launch on)"))
	bool bIterativeCookingForLaunchOn;

	/** Enable IncrementalCooks when triggering from Platforms dropdown menu */
	UPROPERTY(GlobalConfig, EditAnywhere, Category = Cooker, meta = (DisplayName = "Use Incremental Cook for the Platforms->Cook Content menu item"))
	...
	/** Whether to allow ASTC HDR profile */
	UPROPERTY(GlobalConfig, EditAnywhere, Category = Textures, meta = (
		...
		ToolTip = "Whether to allow ASTC HDR profile, the HDR format is only supported on some devices, e.g. Apple A13, Mali-G72, Adreno (TM) 660",
