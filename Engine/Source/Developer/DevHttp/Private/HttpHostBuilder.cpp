...
void FHttpHostBuilder::AddFromEndpoint(const FAnsiStringView HostUrl, const FAnsiStringView AccessToken)
{
	...
	DefaultClientParams.ConnectTimeoutMs = 5000;
	...
	{
		UE_LOGFMT(LogHttpHostBuilder, Display, "Unsuccessful attempt to fetch hosts by endpoint from host: '{Host}'. Status code was: {Code}", Uri, Response->GetStatusCode());
		return;
	}
...
bool FHttpHostBuilder::BenchmarkHostList(TConstArrayView<FString> InHostCandidates, double WarningTimeoutSeconds, double TimeoutSeconds, FAnsiStringBuilderBase& OutHost, double& OutLatency)
{
	...
	const uint32 MaxTotalConnections = InHostCandidates.Num();
	FHttpConnectionPoolParams ConnectionPoolParams;
	...
	std::atomic<int32> FailedBenchmarkAttempts = 0;
	bool bAllHostsFailed = false;
	const int32 CountOfBenchmarksToRun = InHostCandidates.Num();
	for (const FString& HostCandidate : InHostCandidates)
	{
		...
		TUniquePtr<FHttpBenchmarkOperation> Operation = MakeUnique<FHttpBenchmarkOperation>(HostCandidate, LocalClient->TryCreateRequest(RequestParams), [&SortedHostList, &Mutex, &HostBenchmarkDone, &FailedBenchmarkAttempts, &bAllHostsFailed, &CountOfBenchmarksToRun](const IHttpResponse& Response, FString& Host)
		{
			if (Response.GetStatusCode() != 200)
			{
				if (FailedBenchmarkAttempts.fetch_add(1, std::memory_order_relaxed) + 1 == CountOfBenchmarksToRun)
				{
	...
	if (bAllHostsFailed)
	{
		UE_LOGF(LogHttpHostBuilder, Display, "No valid host found as all benchmark attempts had errors");
...

	{
		...
		if (!bValidHostFound)
		{
			...
			UE_LOGF(LogHttpHostBuilder, Display, "HTTP Benchmarking is slow, continuing to wait to determine ideal host...");

			bValidHostFound = HostBenchmarkDone.WaitFor(FMonotonicTimeSpan::FromSeconds(TimeoutSeconds));
			if (!bValidHostFound)
			{
				UE_LOGF(LogHttpHostBuilder, Display, "No valid host found while benchmarking after timeout was reached");
		...
		
		{
			...
			else
			{
				...
				UE_LOGF(LogHttpHostBuilder, Display, "Failed to determine fastest host option because we had no valid options");
			}
		}
	}
	...
	UE_LOGF(LogHttpHostBuilder, Display, "Resolved to using host '%s' based on HTTP benchmark with a estimated latency of '%.0fms'. Spent %.0fms doing benchmarking.", *OutHost, OutLatency * 1000, BenchmarkingDuration * 1000);
	return true;
}
