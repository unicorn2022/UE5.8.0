...
FCurlHttpManager::FCurlHttpManager()
{
	...
	TUtf8StringBuilder<256> UserAgentBuilder;
	const FEngineVersion& Version = FEngineVersion::Current();
	UserAgentBuilder << ANSITEXTVIEW("User-Agent: UnrealEngine/")
	...
	if (IsRunningCommandlet())
	{
		...
		UserAgentBuilder << '[' << CommandletName << ']';
	}

	if (FApp::HasProjectName() && FApp::GetName() != FApp::GetProjectName())
	{
		UserAgentBuilder << ANSITEXTVIEW(" (") << FApp::GetProjectName() << ')';
	}
	...
	, bThreadStopping(!Params.bAllowAsync || !FPlatformProcess::SupportsMultithreading())
{
	...
	{
		UE_LOGF(LogHttp, Warning, "FCurlHttpConnectionPool could not initialize libcurl (result=%d).", (int32)InitResult);
	}
...
void FCurlHttpClient::SetDefaultOptions(CURL* Curl, FCurlHttpHeaders& Headers) const
{
	...
	curl_easy_setopt(Curl, CURLOPT_CONNECTTIMEOUT_MS, long(Params.ConnectTimeoutMs));
	curl_easy_setopt(Curl, CURLOPT_TIMEOUT_MS, long(Params.TotalTimeoutMs));
	...
	: Client(InClient)
{
	...
	if (InitResult != CURLE_OK)
	{
		UE_LOGF(LogHttp, Warning, "FCurlHttpRequest could not initialize libcurl (result=%d).", (int32)InitResult);
	}
	Curl = curl_easy_init();
	SetDefaultOptions();
}
...
{
	//CheckIdle(TEXT("~FCurlHttpRequest"));
	if (Response.load(std::memory_order_relaxed))
	{
		UE_LOGFMT(LogHttp, Warning, "{Method} {Uri}: {FunctionName} requires the request to be idle.",
			LexToString(Method), Uri, TEXT("~FCurlHttpRequest"));
...
void FCurlHttpRequest::CheckIdle(const TCHAR* FunctionName)
{
	checkf(!Response.load(std::memory_order_relaxed),
		...
		LexToString(Method).Len(), LexToString(Method).GetData(), *Uri, FunctionName);
}
...
void FCurlHttpResponse::SetComplete(CURLcode Code)
{
	...
	if (UE_LOG_ACTIVE(LogHttp, Verbose))
	{
		...
		UE_LOGF(LogHttp, Verbose, "%ls (%ls)", *WriteToString<256>(*this), *StatsText);
	}
