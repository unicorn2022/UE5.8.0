...
DEVHTTP_API FAnsiStringView LexToString(EHttpErrorCode ErrorCode);
DEVHTTP_API bool TryLexFromString(EHttpErrorCode& OutErrorCode, FAnsiStringView View);
...
struct FHttpClientParams final
{
	...
	uint32 ConnectTimeoutMs = 0;

	/** Total timeout in milliseconds. Use 0 for no timeout. */
	uint32 TotalTimeoutMs = 0;
