...
bool FDatasmithFacadeDirectLink::Init(bool bUseDatasmithExporterUI, const TCHAR* RemoteEngineDirPath)
{
	...
	{
		UE_LOGF(LogDatasmithFacade, Error, "Fail to initialize FDatasmithExporterManager");
		return false;
	}

	if (int32 ErrorCode = FDatasmithDirectLink::ValidateCommunicationSetup())
	{
		UE_LOGF(LogDatasmithFacade, Error, "Communication setup issue: ErrorCode=%d", ErrorCode);
		return false;
	}

	UE_LOGF(LogDatasmithFacade, Display, "FDatasmithFacadeDirectLink Init OK");
	return true;
}
