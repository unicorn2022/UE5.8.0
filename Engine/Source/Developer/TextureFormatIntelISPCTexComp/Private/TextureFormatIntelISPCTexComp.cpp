...
static EPixelFormat GetASTCQualityFormat(int& BlockWidth, int& BlockHeight, const FTextureBuildSettings& BuildSettings)
{
	...
	switch (OverrideSizeValue >= 0 ? OverrideSizeValue : GetDefaultCompressionBySizeValue(InFormatConfigOverride))
	{
		...
		default: UE_LOGF(LogTemp, Fatal, "Max quality higher than expected");
	}
...
public:
	...
	void LoadDLLOnce()
	{
		...
		if (DLLPath.Len() > 0)
		{
			...
			UE_CLOGF(mDllHandle == nullptr, LogTextureFormatIntelISPCTexComp, Error, "Unable to load %ls", *DLLPath);
		}
		else
		{
			UE_LOGF(LogTextureFormatIntelISPCTexComp, Error, "Platform does not have an ispc_texcomp DLL/library");
		}
	}
