	...
	void RecvThreadProc()
	{
		...
		while (Transport->RecvMessage(&Header, sizeof(Header)))
		{
			if (Header.Size >= 0)
			{
				...
				{
					UE_LOGF(LogHorde, Log, "Failed to read frame header (Channel %d, Size %d)", Header.Channel, Header.Size);
					return;
				}
			}
			...
			{
				UE_LOGF(LogHorde, Warning, "Invalid frame header size received (%d)", Header.Size);
				return;
			}
		}
	}
