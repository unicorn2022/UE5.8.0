...
void FInsightsAutomationController::RunTests(const FString& InCmd)
{
	...
	{
		UE_LOGF(InsightsAutomationController, Warning, "[FInsightsAutomationController] Command %ls does not start with Automation RunTests. Command will be ignored.", *InCmd);
		return;
	}
