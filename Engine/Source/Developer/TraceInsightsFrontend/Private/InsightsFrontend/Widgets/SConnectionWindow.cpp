...
FReply SConnectionWindow::StartUnrealInsightsDirectTrace_OnClicked()
{
	...
	UE_LOGF(LogInsightsFrontend, Log, "[TraceStore] Start analysis (in separate process) for a direct trace (listen on port %u)", Port);
...
FReply SConnectionWindow::Connect_OnClicked()
{
	...
	FGraphEventRef PreConnectTask = FFunctionGraphTask::CreateAndDispatchWhenReady(
		[this, TraceRecorderAddressStr, RunningInstanceAddressStr, ChannelsExpandedStr]
		{
			...
			UE_LOGF(LogInsightsFrontend, Log, "[Connection] Try connecting to \"%ls\"...", *RunningInstanceAddressStr);
			...
			{
				UE_LOGF(LogInsightsFrontend, Log, "[Connection] SendSendTo(\"%ls\")...", *TraceRecorderAddressStr);
				ControlClient.SendSendTo(*TraceRecorderAddressStr);
				UE_LOGF(LogInsightsFrontend, Log, "[Connection] ToggleChannel(\"%ls\")...", *ChannelsExpandedStr);
	...
	ConnectTask = FFunctionGraphTask::CreateAndDispatchWhenReady(
		[this, RunningInstanceAddressStr]
		{
			...
			{
				UE_LOGF(LogInsightsFrontend, Log, "[Connection] Successfully connected.");
			...
			{
				UE_LOGF(LogInsightsFrontend, Warning, "[Connection] Failed to connect to \"%ls\"!", *RunningInstanceAddressStr);
