...
TSharedRef<SWidget> STraceStoreWindow::ConstructFiltersToolbar()
{
	...
	ToolbarBuilder.BeginSection("Filters");
	{
		...
		ToolbarBuilder.AddComboButton(
			FUIAction(),
			FOnGetContent::CreateSP(this, &STraceStoreWindow::MakeBuildVersionFilterMenu),
			LOCTEXT("FilterByBuildVersionText", "Build Version"),
			LOCTEXT("FilterByBuildVersionToolTip", "Filters the list of trace sessions by Build Version."),
			FSlateIcon(FInsightsFrontendStyle::GetStyleSetName(), "Icons.Filter.ToolBar"),
			false);
...
#if 0
		// Filter by Changelist
		ToolbarBuilder.AddComboButton(
			FUIAction(),
			FOnGetContent::CreateSP(this, &STraceStoreWindow::MakeChangelistFilterMenu),
			LOCTEXT("FilterByChangelistText", "Changelist"),
			LOCTEXT("FilterByChangelistToolTip", "Filters the list of trace sessions by changelist."),
			FSlateIcon(FInsightsFrontendStyle::GetStyleSetName(), "Icons.Filter.ToolBar"),
			false);
#endif
		...
		ToolbarBuilder.AddComboButton(
			FUIAction(),
			FOnGetContent::CreateSP(this, &STraceStoreWindow::MakeEngineVersionFilterMenu),
			LOCTEXT("FilterByEngineVersionText", "Engine Version"),
			LOCTEXT("FilterByEngineVersionToolTip", "Filters the list of trace sessions by Engine Version."),
			FSlateIcon(FInsightsFrontendStyle::GetStyleSetName(), "Icons.Filter.ToolBar"),
			false);
...
TSharedRef<SWidget> STraceStoreWindow::ConstructSessionsPanel()
{
	TSharedRef<SWidget> Widget = SAssignNew(TraceListView, SListView<TSharedPtr<FTraceViewModel>>)
		...
		(
			...
			.OnSort(this, &STraceStoreWindow::OnSortModeChanged)
			.OnGetMenuContent(this, &STraceStoreWindow::MakeBuildVersionColumnHeaderMenu)
			[
				SNew(SBox)
				.VAlign(VAlign_Center)
				[
					...
					.ColorAndOpacity_Lambda([this] { return FilterByBuildVersion->IsEmpty() ? FLinearColor(0.5f, 0.5f, 0.5f, 1.0f) : FLinearColor(0.3f, 0.75f, 1.0f, 1.0f); })
				]
			]
...
#if 0
			+ SHeaderRow::Column(FTraceListColumns::Changelist)
			.FillWidth(0.1f)
			.InitialSortMode(EColumnSortMode::Ascending)
			.SortMode(this, &STraceStoreWindow::GetSortModeForColumn, FTraceListColumns::Changelist)
			.OnSort(this, &STraceStoreWindow::OnSortModeChanged)
			.OnGetMenuContent(this, &STraceStoreWindow::MakeChangelistColumnHeaderMenu)
			[
				SNew(SBox)
				.VAlign(VAlign_Center)
				[
					SNew(STextBlock)
					.Text(LOCTEXT("ChangelistColumn", "CL"))
					.ColorAndOpacity_Lambda([this] { return FilterByChangelist->IsEmpty() ? FLinearColor(0.5f, 0.5f, 0.5f, 1.0f) : FLinearColor(0.3f, 0.75f, 1.0f, 1.0f); })
				]
			]
#endif

			+ SHeaderRow::Column(FTraceListColumns::EngineVersion)
			.FillWidth(0.1f)
			.InitialSortMode(EColumnSortMode::Ascending)
			.SortMode(this, &STraceStoreWindow::GetSortModeForColumn, FTraceListColumns::EngineVersion)
			.OnSort(this, &STraceStoreWindow::OnSortModeChanged)
			.OnGetMenuContent(this, &STraceStoreWindow::MakeEngineVersionColumnHeaderMenu)
			[
				SNew(SBox)
				.VAlign(VAlign_Center)
				[
					SNew(STextBlock)
					.Text(LOCTEXT("EngineVersionColumn", "UE"))
					.ColorAndOpacity_Lambda([this] { return FilterByEngineVersion->IsEmpty() ? FLinearColor(0.5f, 0.5f, 0.5f, 1.0f) : FLinearColor(0.3f, 0.75f, 1.0f, 1.0f); })
				]
			]
...
bool STraceStoreWindow::CanRenameSelectedTrace() const
{
	...
	return SelectedTrace.IsValid()
		&& SelectedTrace->HasValidId()
		&& !SelectedTrace->IsLive();
}
...
void STraceStoreWindow::RenameSelectedTrace()
{
	...
	TSharedPtr<FTraceViewModel> SelectedTrace = GetSingleSelectedTrace();
	SelectedTrace->StartRenaming();

	TSharedPtr<SEditableTextBox> RenameTextBox = SelectedTrace->GetRenameTextBox();
...
bool STraceStoreWindow::CanDeleteSelectedTraces() const
{
	...
	{
		if (SelectedTrace->HasValidId() && !SelectedTrace->IsLive())
		{
			return true;
		}
	}
...
void STraceStoreWindow::DeleteSelectedTraces()
{
	...
	TracesToDelete.RemoveAll([](const TSharedPtr<FTraceViewModel>& InTrace) { return !InTrace->HasValidId() || InTrace->IsLive(); });
	...
	if (bIsDeleteTraceConfirmWindowVisible)
	{
		...
		{
			TraceFilesToDelete.Append(TracesToDelete[TraceIndex]->GetUri());
	...
	for (int32 TraceIndex = 0; TraceIndex < TracesToDelete.Num(); ++TraceIndex)
	{
		...
		if (DeleteTrace(TraceToDelete))
		{
			...
			TraceViewModelMap.Remove(TraceToDelete->GetId());
		}
	}
...
{
	FString TraceName = TraceToDelete->GetName();
	UE_LOGF(LogInsightsFrontend, Log, "[TraceStore] Deleting \"%ls\"...", *TraceName);

	if (TraceToDelete->IsLive())
	{
	...
	FString TraceFile = TraceToDelete->GetUri();
	if (!FPaths::FileExists(TraceFile) || !IFileManager::Get().Delete(*TraceFile))
	{
		UE_LOGF(LogInsightsFrontend, Warning, "[TraceStore] Failed to delete trace file (\"%ls\")!", *TraceFile);
	...
	UE_LOGF(LogInsightsFrontend, Verbose, "[TraceStore] Deleted utrace file (\"%ls\").", *TraceFile);
	...
	if (FPaths::FileExists(CacheFile))
	{
		...
		{
			UE_LOGF(LogInsightsFrontend, Verbose, "[TraceStore] Deleted ucache file (\"%ls\").", *CacheFile);
		}
	}

	// Also delete the metadata ini file (<trace>.utrace.ini) if present.
	const FString SidecarFile = TraceFile + TEXT(".ini");
	if (FPaths::FileExists(SidecarFile))
	{
		if (IFileManager::Get().Delete(*SidecarFile))
		{
			UE_LOGF(LogInsightsFrontend, Verbose, "[TraceStore] Deleted sidecar ini file (\"%ls\").", *SidecarFile);
		}
	}
...
{
	TSharedPtr<FTraceViewModel> SelectedTrace = GetSingleSelectedTrace();
	return SelectedTrace.IsValid() && SelectedTrace->HasValidId();
}
...
void STraceStoreWindow::CopyTraceId()
{
	...
	{
		TSharedPtr<FTraceViewModel> SelectedTrace = GetSingleSelectedTrace();
		FString ClipboardText = FString::Printf(TEXT("0x%X"), SelectedTrace->GetId());
...
void STraceStoreWindow::CopyFullPath()
{
	...
	{
		TSharedPtr<FTraceViewModel> SelectedTrace = GetSingleSelectedTrace();
		FPlatformApplicationMisc::ClipboardCopy(*SelectedTrace->GetUri());
	}
}
...
void STraceStoreWindow::OpenContainingFolder()
{
	...
	{
		TSharedPtr<FTraceViewModel> SelectedTrace = GetSingleSelectedTrace();
		FPlatformProcess::ExploreFolder(*SelectedTrace->GetUri());
	}
}
...
{
	return TraceViewModels.FindByPredicate([](const TSharedPtr<FTraceViewModel>& Trace) { return Trace->IsLive(); }) != nullptr;
}
...
void STraceStoreWindow::RefreshTraceList()
{
...

	{
		...
		if (NewTracesChangeSerial != TracesChangeSerial || bSettingsChanged)
		{
			...
			//UE_LOGF(LogInsightsFrontend, Log, "[TraceStore] Syncing the trace list with StoreBrowser...");
			...
			// Check for removed traces.
			{
				...
				for (int32 TraceIndex = 0; TraceIndex < TraceViewModelCount; ++TraceIndex)
				{
					...
					const TSharedPtr<FStoreBrowserTraceInfo>* InTracePtrPtr = InTraceMap.Find(Trace.GetId());
					if (!InTracePtrPtr)
					{
						// This trace was removed.
						RemovedTraces++;
						TraceViewModels.RemoveAtSwap(TraceIndex);
						TraceViewModelMap.Remove(Trace.GetId());
						TraceIndex--;
						TraceViewModelCount--;
					}
				}
			...
			for (const TSharedPtr<FStoreBrowserTraceInfo>& InTracePtr : InTraces)
			{
				...
				if (TracePtrPtr)
				{
					...
					if (Trace.GetChangeSerial() != SourceTrace.ChangeSerial || bSettingsChanged)
					{
				...
				else
				{
					...
					TracePtr->SetId(SourceTrace.TraceId);
					UpdateTrace(*TracePtr, SourceTrace);
					TraceViewModels.Add(TracePtr);
					TraceViewModelMap.Add(TracePtr->GetId(), TracePtr);
				}
			}
		}
	...
	{
		UE_LOGF(LogInsightsFrontend, Log, "[TraceStore] The trace list refreshed in %.0f ms (%d traces : %d updated, %d added, %d removed).",
...
{
	check(InOutTrace.GetId() == InSourceTrace.TraceId);

	InOutTrace.SetChangeSerial(InSourceTrace.ChangeSerial);

	InOutTrace.SetName(InSourceTrace.Name);
	InOutTrace.SetUri(InSourceTrace.Uri);
	InOutTrace.SetDirectoryColor(GetColorByPath(InSourceTrace.Uri));

	InOutTrace.SetTimestamp(InSourceTrace.Timestamp);
	InOutTrace.SetSize(InSourceTrace.Size);

	InOutTrace.SetIsLive(InSourceTrace.bIsLive);
	InOutTrace.SetIpAddress(InSourceTrace.IpAddress);

	// Is metadata updated?
	if (!InOutTrace.IsMetadataUpdated() && InSourceTrace.MetadataUpdateCount == 0)
	{
		InOutTrace.SetIsMetadataUpdated(true);
		InOutTrace.SetPlatform(InSourceTrace.Platform);
		if (!InSourceTrace.ProjectName.IsEmpty())
		{
			InOutTrace.SetAppName(InSourceTrace.ProjectName);
		}
		else
		{
			InOutTrace.SetAppName(InSourceTrace.AppName);
		}
		InOutTrace.SetBuildConfiguration(InSourceTrace.ConfigurationType);
		InOutTrace.SetBuildTarget(InSourceTrace.TargetType);
		InOutTrace.SetBuildVersion(InSourceTrace.BuildVersion);
		InOutTrace.SetBuildBranch(InSourceTrace.Branch);
		InOutTrace.SetChangelist(InSourceTrace.Changelist);
		InOutTrace.SetEngineVersion(InSourceTrace.EngineVersion);
		InOutTrace.SetCommandLine(InSourceTrace.CommandLine);
		InOutTrace.SetVFSPaths(InSourceTrace.VFSPaths);
	}

	const FInsightsFrontendSettings& Settings = GetSettings();

	// Auto start analysis for a live trace session.
	if (InOutTrace.IsLive() &&
		InOutTrace.IsMetadataUpdated() &&
		Settings.IsAutoStartAnalysisEnabled() && // is auto start enabled?
		!AutoStartedSessions.Contains(InOutTrace.GetId())) // is not already auto-started?
	{
		...
		// matches filter?
		if ((AutoStartPlatformFilterStr.IsEmpty() || FCString::Strcmp(*AutoStartPlatformFilterStr, *InOutTrace.GetPlatform()) == 0) &&
			(AutoStartAppNameFilterStr.IsEmpty() || FCString::Strcmp(*AutoStartAppNameFilterStr, *InOutTrace.GetAppName()) == 0) &&
			(AutoStartConfigurationTypeFilter == EBuildConfiguration::Unknown || AutoStartConfigurationTypeFilter == InOutTrace.GetBuildConfiguration()) &&
			(AutoStartTargetTypeFilter == EBuildTargetType::Unknown || AutoStartTargetTypeFilter == InOutTrace.GetBuildTarget()))
		{
			UE_LOGF(LogInsightsFrontend, Log, "[TraceStore] Auto starting analysis for trace with id 0x%08X...", InOutTrace.GetId());
			AutoStartedSessions.Add(InOutTrace.GetId());
			OpenTraceSession(InOutTrace.GetId());
		}
	}
}
...
void STraceStoreWindow::UpdateTraceListView()
{
	...
	if (bIsUserSelectedTrace)
	{
		...
		for (const TSharedPtr<FTraceViewModel>& SelectedTrace : SelectedTraces)
		{
			TSharedPtr<FTraceViewModel>* FoundNewTrace = TraceViewModelMap.Find(SelectedTrace->GetId());
			if (!FoundNewTrace)
			{
				FoundNewTrace = TraceViewModels.FindByPredicate([SelectedTrace](const TSharedPtr<FTraceViewModel>& Trace) { return Trace->HasSameUri(*SelectedTrace); });
			}
			if (!FoundNewTrace)
			{
				FoundNewTrace = TraceViewModels.FindByPredicate([SelectedTrace](const TSharedPtr<FTraceViewModel>& Trace) { return Trace->HasSameName(*SelectedTrace); });
			}
			if (FoundNewTrace)
			{
...
void STraceStoreWindow::EnableAutoConnect()
{
	...
	{
		UE_LOGF(LogInsightsFrontend, Warning, "[TraceStore] Failed to create AutoConnect event.");
	}
	...
	{
		UE_LOGF(LogInsightsFrontend, Warning, "[TraceStore] Failed to create AutoConnect semaphore: %d", errno);
	}
#endif
}
...
void STraceStoreWindow::DisableAutoConnect()
{
	...
	if (AutoConnectEvent != SEM_FAILED)
	{
		...
		{
			UE_LOGF(LogInsightsFrontend, Warning, "[TraceStore] Failed to remove AutoConnect semaphore: %d", errno);
		}
	}
#endif
}
...
{
	UE_LOGF(LogInsightsFrontend, Log, "[TraceStore] Start analysis (in separate process) for trace file: \"%ls\"", *InTraceFile);
...
{
	if (InTraceSession.IsValid() && InTraceSession->HasValidId())
	{
		OpenTraceSession(InTraceSession->GetId());
	}
}
...
void STraceStoreWindow::OpenTraceSession(uint32 InTraceId)
{
	...
	UE_LOGF(LogInsightsFrontend, Log, "[TraceStore] Start analysis (in separate process) for trace id: 0x%08X", InTraceId);
	...
	if (TraceSessionPtrPtr)
	{
		...
		SplashScreenOverlayTraceFile = FPaths::GetBaseFilename(TraceSession.GetUri());
	}
	ShowSplashScreenOverlay();
}
...
TSharedRef<SWidget> STraceStoreWindow::MakeTraceListMenu()
{
	...
	MenuBuilder.BeginSection("AvailableTraces", LOCTEXT("TraceListMenu_Section_AvailableTraces", "Top Most Recently Created Traces"));
	{
		...
		Algo::SortBy(SortedTraces, [](const TSharedPtr<FTraceViewModel>& Trace) { return Trace->GetTimestamp(); });
		...
		for (int32 TraceIndex = SortedTraces.Num() - 1; TraceIndex >= 0 && TraceCountLimit > 0; --TraceIndex, --TraceCountLimit)
		{
			...
			MenuBuilder.AddMenuEntry(
				...
				FSlateIcon(),
				FUIAction(FExecuteAction::CreateSP(this, &STraceStoreWindow::OpenTraceSession, Trace.GetId())),
				NAME_None,
				EUserInterfaceActionType::Button);
		}
	}
...
TSharedRef<SWidget> STraceStoreWindow::MakeBuildVersionColumnHeaderMenu()
{
	FSlateApplication::Get().CloseToolTip();

	FMenuBuilder MenuBuilder(/*bInShouldCloseWindowAfterMenuSelection=*/true, nullptr);

	MenuBuilder.BeginSection("Filter", LOCTEXT("ColumnHeaderMenuSection_BuildVersionFilter", "Build Version Filter"));
	BuildBuildVersionFilterSubMenu(MenuBuilder);
	MenuBuilder.EndSection();
...
TSharedRef<SWidget> STraceStoreWindow::MakeBuildVersionFilterMenu()
{
	FSlateApplication::Get().CloseToolTip();

	FMenuBuilder MenuBuilder(/*bInShouldCloseWindowAfterMenuSelection=*/true, nullptr);

	MenuBuilder.BeginSection("Filter", LOCTEXT("MenuSection_BuildVersionFilter", "Build Version Filter"));
	BuildBuildVersionFilterSubMenu(MenuBuilder);
	MenuBuilder.EndSection();
...
void STraceStoreWindow::BuildBuildVersionFilterSubMenu(FMenuBuilder& InMenuBuilder)
{
	FilterByBuildVersion->BuildMenu(InMenuBuilder, *this);
}
...
TSharedRef<SWidget> STraceStoreWindow::MakeChangelistColumnHeaderMenu()
{
	FSlateApplication::Get().CloseToolTip();

	FMenuBuilder MenuBuilder(/*bInShouldCloseWindowAfterMenuSelection=*/true, nullptr);

	MenuBuilder.BeginSection("Filter", LOCTEXT("ColumnHeaderMenuSection_ChangelistFilter", "Changelist Filter"));
	BuildChangelistFilterSubMenu(MenuBuilder);
	MenuBuilder.EndSection();
...
TSharedRef<SWidget> STraceStoreWindow::MakeChangelistFilterMenu()
{
	FSlateApplication::Get().CloseToolTip();

	FMenuBuilder MenuBuilder(/*bInShouldCloseWindowAfterMenuSelection=*/true, nullptr);

	MenuBuilder.BeginSection("Filter", LOCTEXT("MenuSection_ChangelistFilter", "Changelist Filter"));
	BuildChangelistFilterSubMenu(MenuBuilder);
	MenuBuilder.EndSection();
...
void STraceStoreWindow::BuildChangelistFilterSubMenu(FMenuBuilder& InMenuBuilder)
{
	FilterByChangelist->BuildMenu(InMenuBuilder, *this);
}

////////////////////////////////////////////////////////////////////////////////////////////////////

TSharedRef<SWidget> STraceStoreWindow::MakeEngineVersionColumnHeaderMenu()
{
	FSlateApplication::Get().CloseToolTip();

	FMenuBuilder MenuBuilder(/*bInShouldCloseWindowAfterMenuSelection=*/true, nullptr);

	MenuBuilder.BeginSection("Filter", LOCTEXT("ColumnHeaderMenuSection_EngineVersionFilter", "Engine Version Filter"));
	BuildEngineVersionFilterSubMenu(MenuBuilder);
	MenuBuilder.EndSection();
...
TSharedRef<SWidget> STraceStoreWindow::MakeEngineVersionFilterMenu()
{
	FSlateApplication::Get().CloseToolTip();

	FMenuBuilder MenuBuilder(/*bInShouldCloseWindowAfterMenuSelection=*/true, nullptr);

	MenuBuilder.BeginSection("Filter", LOCTEXT("MenuSection_EngineVersionFilter", "Engine Version Filter"));
	BuildEngineVersionFilterSubMenu(MenuBuilder);
	MenuBuilder.EndSection();
...
void STraceStoreWindow::BuildEngineVersionFilterSubMenu(FMenuBuilder& InMenuBuilder)
{
	FilterByEngineVersion->BuildMenu(InMenuBuilder, *this);
}
...
FReply STraceStoreWindow::AddWatchDir_Clicked()
{
	...
	if (IDesktopPlatform* DesktopPlatform = FDesktopPlatformModule::Get())
	{
		...
		if (bHasSelected && !FPathViews::Equals(SelectedDirectory, CurrentStoreDirectory))
		{
			...
			UE_LOGF(LogInsightsFrontend, Log, "[TraceStore] Adding monitored directory: \"%ls\"...", *SelectedDirectory);
...
void STraceStoreWindow::CreateFilters()
{
	...
	FilterByBuildVersion = MakeShared<FTraceFilterByBuildVersion>();
	Filters->Add(FilterByBuildVersion);

	FilterByBranch = MakeShared<FTraceFilterByBranch>();
	Filters->Add(FilterByBranch);

	FilterByChangelist = MakeShared<FTraceFilterByChangelist>();
	Filters->Add(FilterByChangelist);

	FilterByEngineVersion = MakeShared<FTraceFilterByEngineVersion>();
	Filters->Add(FilterByEngineVersion);
...
void STraceStoreWindow::HandleItemToStringArray(const FTraceViewModel& InTrace, TArray<FString>& OutSearchStrings) const
{
	...
	{
		OutSearchStrings.Add(InTrace.GetCommandLine());
	}
	else
	{
		OutSearchStrings.Add(InTrace.GetName());
	}
}
...
void STraceStoreWindow::UpdateFiltering()
{
	...
	if (FilterByName->GetRawFilterText().IsEmpty() &&
		...
		FilterByBuildVersion->IsEmpty() &&
		FilterByBranch->IsEmpty() &&
		FilterByChangelist->IsEmpty() &&
		FilterByEngineVersion->IsEmpty() &&
...
void STraceStoreWindow::UpdateFilterStatsText()
{
	...
	{
		FilterdTotalSize += Trace->GetSize();
		if (Trace->IsLive())
		{
...
void STraceStoreWindow::UpdateSorting()
{
	if (SortColumn == FTraceListColumns::Date)
	{
		...
		{
			FilteredTraceViewModels.Sort(
				[](const TSharedPtr<FTraceViewModel>& A, const TSharedPtr<FTraceViewModel>& B)
				{
	...
	else if (SortColumn == FTraceListColumns::Name)
	{
		if (SortMode == EColumnSortMode::Ascending)
		{
			FilteredTraceViewModels.Sort(
				[](const TSharedPtr<FTraceViewModel>& A, const TSharedPtr<FTraceViewModel>& B)
				{
					return A->GetNameText().CompareTo(B->GetNameText()) < 0;
				});
		}
		else
		{
			FilteredTraceViewModels.Sort(
				[](const TSharedPtr<FTraceViewModel>& A, const TSharedPtr<FTraceViewModel>& B)
				{
					return B->GetNameText().CompareTo(A->GetNameText()) < 0;
				});
		}
	}
	else if (SortColumn == FTraceListColumns::Uri)
	{
		if (SortMode == EColumnSortMode::Ascending)
		{
			FilteredTraceViewModels.Sort(
				[](const TSharedPtr<FTraceViewModel>& A, const TSharedPtr<FTraceViewModel>& B)
				{
					return A->GetUriText().CompareTo(B->GetUriText()) < 0;
				});
		}
		else
		{
			FilteredTraceViewModels.Sort(
				[](const TSharedPtr<FTraceViewModel>& A, const TSharedPtr<FTraceViewModel>& B)
				{
					return B->GetUriText().CompareTo(A->GetUriText()) < 0;
				});
		}
	}
	else if (SortColumn == FTraceListColumns::Platform)
	{
		if (SortMode == EColumnSortMode::Ascending)
		{
			FilteredTraceViewModels.Sort(
				[](const TSharedPtr<FTraceViewModel>& A, const TSharedPtr<FTraceViewModel>& B)
				{
					int32 CompareResult = A->GetPlatformText().CompareTo(B->GetPlatformText());
					return CompareResult == 0 ? A->GetTimestamp() < B->GetTimestamp() : CompareResult < 0;
				});
		}
		else
		{
			FilteredTraceViewModels.Sort(
				[](const TSharedPtr<FTraceViewModel>& A, const TSharedPtr<FTraceViewModel>& B)
				{
					int32 CompareResult = B->GetPlatformText().CompareTo(A->GetPlatformText());
					return CompareResult == 0 ? A->GetTimestamp() < B->GetTimestamp() : CompareResult < 0;
				});
		}
	}
	else if (SortColumn == FTraceListColumns::AppName)
	{
		if (SortMode == EColumnSortMode::Ascending)
		{
			FilteredTraceViewModels.Sort(
				[](const TSharedPtr<FTraceViewModel>& A, const TSharedPtr<FTraceViewModel>& B)
				{
					int32 CompareResult = A->GetAppNameText().CompareTo(B->GetAppNameText());
					return CompareResult == 0 ? A->GetTimestamp() < B->GetTimestamp() : CompareResult < 0;
				});
		}
		else
		{
			FilteredTraceViewModels.Sort(
				[](const TSharedPtr<FTraceViewModel>& A, const TSharedPtr<FTraceViewModel>& B)
				{
					int32 CompareResult = B->GetAppNameText().CompareTo(A->GetAppNameText());
					return CompareResult == 0 ? A->GetTimestamp() < B->GetTimestamp() : CompareResult < 0;
				});
		}
	}
	else if (SortColumn == FTraceListColumns::BuildConfig)
	{
		if (SortMode == EColumnSortMode::Ascending)
		{
			FilteredTraceViewModels.Sort(
				[](const TSharedPtr<FTraceViewModel>& A, const TSharedPtr<FTraceViewModel>& B)
				{
					return A->GetBuildConfiguration() == B->GetBuildConfiguration() ?
						A->GetTimestamp() < B->GetTimestamp() :
						A->GetBuildConfiguration() < B->GetBuildConfiguration();
				});
		}
		else
		{
	...
	else if (SortColumn == FTraceListColumns::BuildTarget)
	{
		if (SortMode == EColumnSortMode::Ascending)
		{
			FilteredTraceViewModels.Sort(
				[](const TSharedPtr<FTraceViewModel>& A, const TSharedPtr<FTraceViewModel>& B)
				{
					return A->GetBuildTarget() == B->GetBuildTarget() ?
						A->GetTimestamp() < B->GetTimestamp() :
						A->GetBuildTarget() < B->GetBuildTarget();
				});
		}
		else
		{
	...
	else if (SortColumn == FTraceListColumns::BuildVersion)
	{
		if (SortMode == EColumnSortMode::Ascending)
		{
			FilteredTraceViewModels.Sort(
				[](const TSharedPtr<FTraceViewModel>& A, const TSharedPtr<FTraceViewModel>& B)
				{
					return A->GetBuildVersionText().CompareTo(B->GetBuildVersionText()) < 0;
				});
		}
		else
		{
			FilteredTraceViewModels.Sort(
				[](const TSharedPtr<FTraceViewModel>& A, const TSharedPtr<FTraceViewModel>& B)
				{
					return B->GetBuildVersionText().CompareTo(A->GetBuildVersionText()) < 0;
				});
		}
	}
	else if (SortColumn == FTraceListColumns::BuildBranch)
	{
		if (SortMode == EColumnSortMode::Ascending)
		{
			FilteredTraceViewModels.Sort(
				[](const TSharedPtr<FTraceViewModel>& A, const TSharedPtr<FTraceViewModel>& B)
				{
					return A->GetBuildBranchText().CompareTo(B->GetBuildBranchText()) < 0;
				});
		}
		else
		{
			FilteredTraceViewModels.Sort(
				[](const TSharedPtr<FTraceViewModel>& A, const TSharedPtr<FTraceViewModel>& B)
				{
					return B->GetBuildBranchText().CompareTo(A->GetBuildBranchText()) < 0;
				});
		}
	}
	else if (SortColumn == FTraceListColumns::Changelist)
	{
		if (SortMode == EColumnSortMode::Ascending)
		{
			FilteredTraceViewModels.Sort(
				[](const TSharedPtr<FTraceViewModel>& A, const TSharedPtr<FTraceViewModel>& B)
				{
					return A->GetChangelist() < B->GetChangelist();
				});
		}
		else
		{
			FilteredTraceViewModels.Sort(
				[](const TSharedPtr<FTraceViewModel>& A, const TSharedPtr<FTraceViewModel>& B)
				{
					return B->GetChangelist() < A->GetChangelist();
				});
		}
	}
	else if (SortColumn == FTraceListColumns::EngineVersion)
	{
		if (SortMode == EColumnSortMode::Ascending)
		{
			FilteredTraceViewModels.Sort(
				[](const TSharedPtr<FTraceViewModel>& A, const TSharedPtr<FTraceViewModel>& B)
				{
					return A->GetEngineVersionNumber() < B->GetEngineVersionNumber();
				});
		}
		else
		{
			FilteredTraceViewModels.Sort(
				[](const TSharedPtr<FTraceViewModel>& A, const TSharedPtr<FTraceViewModel>& B)
				{
					return B->GetEngineVersionNumber() < A->GetEngineVersionNumber();
				});
		}
	}
	...
	{
		Algo::SortBy(FilteredTraceViewModels,
			[](const TSharedPtr<FTraceViewModel>& Trace)
			{
