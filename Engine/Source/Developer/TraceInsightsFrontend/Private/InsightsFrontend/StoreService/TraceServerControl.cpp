...
void FTraceServerControl::MakeMenu(FMenuBuilder& Builder)
{
	...
	if (!Client)
	{
		Client = (UE::Trace::FStoreClient::Create(*Host, Port));
		if (Client)
		{
...
bool FTraceServerControl::ChangeState(EState Expected, EState ChangeTo, uint32 Attempts)
{
	while (Attempts-- > 0)
	{
		...
		if (bChanged)
		{
			UE_LOGF(LogInsights, VeryVerbose, "Changing state from '%ls' -> '%ls'", LexState(Expected), LexState(ChangeTo))
			return true;
		}
		UE_LOGF(LogInsights, VeryVerbose, "Busy wait for '%ls'...", LexState(Expected));
		...
		if (ChangeState(EState::NotConnected, EState::Command, GStateChangeRetries))
		{
			...
			if (!Client)
			{
				for (uint32 Attempts = 0; Attempts < GStartConnectAttempts; ++Attempts)
				{
					...
					Client = UE::Trace::FStoreClient::Create(TEXT("127.0.0.1"));
					if (Client)
					{
						break;
					}
			...
			else
			{
				...
				UE_LOGF(LogInsights, Warning, "Failed to connect to store.");
			}
		}
		else
		{
			UE_LOGF(LogInsights, Warning, "Failed to start server.");
		}
	});
}
...
void FTraceServerControl::OnStop_Clicked()
{
	FGraphEventRef CommandTask = FFunctionGraphTask::CreateAndDispatchWhenReady([this]
	{
		...
		{
			UE_LOGF(LogInsights, Warning, "Failed to stop server.");
		}
	});
}
...
void FTraceServerControl::OnSponsored_Changed()
{
	FGraphEventRef CommandTask = FFunctionGraphTask::CreateAndDispatchWhenReady([this]
	{
		...
		{
			UE_LOGF(LogInsights, Warning, "Failed to set sponsored mode.");
		}
	});
}
