...
public:
	TTraceSetFilter();
	virtual ~TTraceSetFilter() = default;
...
protected:
	...
	{
		return (uint8)InTrace.GetBuildConfiguration();
	}
...
protected:
	virtual uint8 GetFilterValueForTrace(const FTraceViewModel& InTrace) const override
	{
		return (uint8)InTrace.GetBuildTarget();
	}
...
class FTraceFilterByBuildVersion : public FTraceFilterByStringSet
{
public:
	FTraceFilterByBuildVersion();

protected:
	virtual FString GetFilterValueForTrace(const FTraceViewModel& InTrace) const override
	{
		return InTrace.GetBuildVersion();
	}
};
...
protected:
	virtual FString GetFilterValueForTrace(const FTraceViewModel& InTrace) const override
	{
		return InTrace.GetBuildBranch();
	}
};

////////////////////////////////////////////////////////////////////////////////////////////////////

class FTraceFilterByChangelist : public TTraceSetFilter<uint8>
{
public:
	enum class EChangelistCategory : uint8
	{
		Zero,  // CL 0
		NotZero,
...
protected:
	...
	virtual uint8 GetFilterValueForTrace(const FTraceViewModel& InTrace) const override
	{
		if (InTrace.GetChangelist() == 0)
		{
			return (uint8)EChangelistCategory::Zero;
		}
		else
		{
			return (uint8)EChangelistCategory::NotZero;
		}
	}
...
class FTraceFilterByEngineVersion : public FTraceFilterByStringSet
{
public:
	FTraceFilterByEngineVersion();

protected:
	virtual FString GetFilterValueForTrace(const FTraceViewModel& InTrace) const override
	{
		return InTrace.GetEngineVersion();
	}
};
...
protected:
	...
	virtual uint8 GetFilterValueForTrace(const FTraceViewModel& InTrace) const override
	{
		const uint64 TraceSize = InTrace.GetSize();
		if (TraceSize == 0)
		{
			return (uint8)ESizeCategory::Empty;
		}
		else if (TraceSize < 1024ull * 1024ull)
		{
			return (uint8)ESizeCategory::Small;
		}
		else if (TraceSize < 1024ull * 1024ull * 1024ull)
		{
