...
static bool GetResolvedBuildSettings(const FCbObject& Settings, FTextureBuildSettings* OutBuildSettings, const ITextureFormat** OutTextureFormat)
{
	*OutBuildSettings = ReadBuildSettingsFromCompactBinary(Settings["Build"].AsObjectView());
	...
	{
		UE_LOGF(LogTextureBuildFunction, Error, "TextureFormatManager not found!");
		return false;
	}

	if (!TextureFormat)
	{
		UE_LOGF(LogTextureBuildFunction, Error, "Texture format %ls not found", *WriteToString<128>(OutBuildSettings->TextureFormatName));
		return false;
	}
	...
	{
		UE_LOGF(LogTextureBuildFunction, Error, "%ls has version %hu when version %hu is required.",
												...
												bool bVolume, TArray<FImage>& OutMips)
{
	...
	if (!InputBuffer)
	{
		UE_LOGF(LogTextureBuildFunction, Error, "Missing input '%ls'.", *WriteToString<64>(Source.GetName()));
		return false;
	}
	if ( InputBuffer.GetSize() == 0 )
	{
		UE_LOGF(LogTextureBuildFunction, Error, "Input size zero '%ls'.", *WriteToString<64>(Source.GetName()));
		return false;
	}
...
void FTextureBuildFunction::Build(UE::DerivedData::FBuildContext& Context) const
{
	...
	if (!Settings)
	{
		UE_LOGF(LogTextureBuildFunction, Error, "Settings are not available.");
		return;
	}
	...
	UE_LOGF(LogTextureBuildFunction, Display, "Compressing [%ls] from %dx%d (%d slices, %d mips) to %ls...%ls%ls%ls%ls%ls RequiredMemory=%.3f MB", 
		*Context.GetName(), 
	...
	if ( FParse::Param(FCommandLine::Get(), TEXT("tbfmemcheck")) )
	{
		...
		{
			UE_LOGF(LogTextureBuildFunction, Display, "NOTE: Memory use report may be inaccurate; use -ansimalloc.");
		}
	}
#endif
	...
	// log built info :
	{
		...
		UE_LOGF(LogTextureBuildFunction, Display, "Built texture: %d Mips PF=%d=%ls : %dx%dx%d : CompressedDataSize=%lld , MemAllocated = %.3f MB",
			...
			FParse::Param(FCommandLine::Get(), TEXT("tbfcsv")) )
		{
			...
			{
				UE_LOGF(LogTextureBuildFunction, Display, "Failed to save CSV file %ls", *CSVFilename);
			}
			else
			{
	...
	if ( DoMemoryCheck )
	{
		...
		{
			UE_LOGF(LogTextureBuildFunction, Warning, "BuildMemAllocated (%lld) > RequiredMemPadded (%lld)",
	...
	FChildBuildData OutputData;
	{
		...
		{
			UE_LOGF(LogTextureBuildFunction, Error, "Engine parameters are not available.");
			return;
		}
...
void GenericTextureTilingBuildFunction(UE::DerivedData::FBuildContext& Context, const ITextureTiler* Tiler, const UE::FUtf8SharedString& BuildFunctionName)
{
	...
	UE_LOGF(LogTextureBuildFunction, Display, "Tiling %ls with %ls -> %d source mip(s) with a tail of %d...", 
...
void GenericTextureDecodeBuildFunction(UE::DerivedData::FBuildContext& Context, const UE::FUtf8SharedString& BuildFunctionName)
{
	...
	UE_LOGF(LogTextureBuildFunction, Display, "Decoding %ls with %ls...", *Context.GetName(), StringCast<TCHAR>(*BuildFunctionName).Get());
...
void GenericTextureDetileBuildFunction(UE::DerivedData::FBuildContext& Context, const ITextureTiler* Tiler, const UE::FUtf8SharedString& BuildFunctionName)
{
	...
	UE_LOGF(LogTextureBuildFunction, Display, "De-Tiling %ls with %ls -> %d source mip(s) with a tail of %d...", 
