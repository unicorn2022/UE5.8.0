...
void SToggleTraceButton::PrivateRegisterAttributes(FSlateAttributeInitializer& AttributeInitializer)
{
	...
	SLATE_ADD_MEMBER_ATTRIBUTE_DEFINITION_WITH_NAME(AttributeInitializer, "LabelText", LabelTextAttribute, EInvalidateWidgetReason::Paint);
}

SToggleTraceButton::SToggleTraceButton()
	: IsTraceRunningAttribute(*this)
	, DynamicToolTipTextAttribute(*this)
	, LabelTextAttribute(*this)
{
}
...
void SToggleTraceButton::Construct(const FArguments& InArgs)
{
	...
	LabelTextAttribute.Assign(*this, InArgs._LabelText);
	ButtonSize = InArgs._ButtonSize;
	bShowStopButton = InArgs._ShowStopButton;
	...
	[
		...
		[
			...
			[
				SNew(STextBlock)
				.Text(this, &SToggleTraceButton::GetRecordingButtonLabelText)
			]
		]
	];
}
...
{
	if (bShowStopButton && bIsTraceRecordButtonHovered && IsTraceRunningAttribute.Get())
	{
		return EVisibility::Visible;
	}
...
FText SToggleTraceButton::GetRecordingButtonLabelText() const
{
	return LabelTextAttribute.Get();
}
