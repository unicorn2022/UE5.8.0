...
namespace CrossCompiler
{
	...
	EParseResult ParseColonSpecifier(FHlslScanner& Scanner, FLinearAllocator* Allocator, int32 EDFFlags, AST::FSemanticSpecifier** OutSpecifier)
	{
		...
		else if ((EDFFlags & EDF_SEMANTIC) == EDF_SEMANTIC || ((EDFFlags & EDF_BITPACK) == EDF_BITPACK))
		{
			bool bMatched = false;
			...
			else if (((EDFFlags & EDF_BITPACK) == EDF_BITPACK) && Scanner.MatchToken(EHlslToken::Literal))
			{
				bMatched = true;
			}
		...
		else
		{
			...
			if (Result == EParseResult::Matched)
			{
				...
				do
				{
					...
					if ((DeclarationFlags & (EDF_REGISTER | EDF_PACKOFFSET | EDF_SEMANTIC | EDF_BITPACK)) != 0)
					{
						if (Parser.Scanner.MatchToken(EHlslToken::Colon))
						{
							if (ParseColonSpecifier(Parser.Scanner, Allocator, (DeclarationFlags & (EDF_SEMANTIC | EDF_REGISTER | EDF_PACKOFFSET | EDF_BITPACK)), &Declaration->Semantic) != EParseResult::Matched)
							{
	...
	static EParseResult ParseStructBody(FHlslParser& Parser, FSymbolScope* SymbolScope, FLinearAllocator* Allocator, AST::FStructSpecifier* Struct)
	{
		...
		while (Parser.Scanner.HasMoreTokens())
		{
			...
			AST::FDeclaratorList* Declaration = nullptr;
			EParseResult Result = ParseGeneralDeclarationNoSemicolon(Parser, SymbolScope, 0, EDF_CONST_ROW_MAJOR | EDF_SEMANTIC | EDF_MULTIPLE | EDF_TEXTURE_SAMPLER_OR_BUFFER | EDF_INTERPOLATION | EDF_BITPACK, Allocator, &Declaration);
		...
		while (Parser.Scanner.HasMoreTokens())
		{
			...
			while (Parser.Scanner.HasMoreTokens())
			{
				...
				else
				{
					...
					// Ignore null statements, e.g. from a double semicolon `;;`
