...
bool FCookOnTheFlyClientConnectionBase::Initialize()
{
	...
	{
		UE_LOGF(LogCookOnTheFlyNetworkServer, Warning, "Failed to receive handshake payload");
		return false;
	}
	...
	if (!TargetPlatformNames.IsEmpty())
	{
		...
		{
			UE_LOGF(LogCookOnTheFlyNetworkServer, Verbose, "    Possible Target Platform from client: %ls", *TargetPlatformNames[TPIndex]);

			// look for a matching target platform
			for (int32 ActiveTPIndex = 0; ActiveTPIndex < Owner.ActiveTargetPlatforms.Num(); ActiveTPIndex++)
			{
				UE_LOGF(LogCookOnTheFlyNetworkServer, Verbose, "   Checking against: %ls", *Owner.ActiveTargetPlatforms[ActiveTPIndex]->PlatformName());
		...
		if (!TargetPlatform)
		{
			...
			UE_LOGF(LogCookOnTheFlyNetworkServer, Warning, "Unable to find target platform for client, terminating client connection!");

			for (int32 TPIndex = 0; TPIndex < TargetPlatformNames.Num(); TPIndex++)
			{
				UE_LOGF(LogCookOnTheFlyNetworkServer, Warning, "    Target platforms from client: %ls", *TargetPlatformNames[TPIndex]);
			}
			for (int32 ActiveTPIndex = 0; ActiveTPIndex < Owner.ActiveTargetPlatforms.Num(); ActiveTPIndex++)
			{
				UE_LOGF(LogCookOnTheFlyNetworkServer, Warning, "    Active target platforms on server: %ls", *Owner.ActiveTargetPlatforms[ActiveTPIndex]->PlatformName());
			}
			bIsOk = false;
		}
	}
...
uint32 FCookOnTheFlyClientConnectionBase::Run()
{
	while (!bStopRequested)
	{
		...
		{
			UE_LOGF(LogCookOnTheFlyNetworkServer, Warning, "ReceivePayload failed");
			break;
		}
