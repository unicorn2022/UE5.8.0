...
int32 FNavigationSimulationOverlay::PaintSnapshotNode(const TArray<FNavigationSimulationWidgetNodePtr>& NodeToPaint, const FGeometry& AllottedGeometry, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FVector2D& RootDrawOffset)
{
	...
	{
		auto GetGeometry = [&GeometryMap, &AllottedGeometry, RootDrawOffset](const FNavigationSimulationWidgetInfo& Info) -> TOptional<FPaintGeometry>
		{
			if (Info.bHasGeometry)
			{
				if (const FPaintGeometry* PaintGeometry = GeometryMap.GetGeometry(Info, AllottedGeometry, RootDrawOffset))
				{
					return TOptional<FPaintGeometry>(*PaintGeometry);
				}
				else
				{
					UE_LOGF(LogSlate, Warning, "Slate Navigation Simulation missing paint geometry for snapshot widget %ls", *(Info.WidgetTypeAndShortName.ToString()));
				}
			}
...
int32 FNavigationSimulationOverlay::PaintLiveNode(const TArray<FNavigationSimulationWidgetNodePtr>& NodeToPaint, const FGeometry& AllottedGeometry, FSlateWindowElementList& OutDrawElements, int32 LayerId)
{
	...
	for (const FNavigationSimulationWidgetNodePtr& NodePtr : NodeToPaint)
	{
		auto GetGeometry = [&GeometryMap](const FNavigationSimulationWidgetInfo& Info) -> TOptional<FPaintGeometry>
		{
			...
			{
				if (const FPaintGeometry* PaintGeometry = GeometryMap.GetGeometry(WidgetLive.ToSharedRef()))
				{
					return TOptional<FPaintGeometry>(*PaintGeometry);
				}
				else
				{
					UE_LOGF(LogSlate, Warning, "Slate Navigation Simulation missing paint geometry for live widget %ls", *(Info.WidgetTypeAndShortName.ToString()));
				}
			}
			return TOptional<FPaintGeometry>();
		};
