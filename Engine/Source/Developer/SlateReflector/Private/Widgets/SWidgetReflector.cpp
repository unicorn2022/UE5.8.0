...
namespace WidgetReflectorTabID
{
	...
	static const FName VisualElements = "WidgetReflector.VisualElements";
	static const FName WidgetSearch = "WidgetReflector.WidgetSearch";
}
...
namespace VisualCaptureEnabled
{
	static const FName WidgetPickingMode = "WidgetPickingMode";
}
...
public:
	...
	virtual void UpdateColumnWidth(TSharedRef<FWidgetReflectorNodeBase> InItem, FName InColumn) override;
...
private:
	...
	TSharedRef<SDockTab> SpawnVisualElementsTab(const FSpawnTabArgs& Args);
	TSharedRef<SDockTab> SpawnWidgetSearchTab(const FSpawnTabArgs& Args);

	float GetColumnWidth(FName InColumn, float InDefault) const;
...
public:
	...
	bool HandleSourceAccessDelegate(const FString& FileName, int32 LineNumber, int32 ColumnNumber) const
	{
		if (SourceAccessDelegate.IsBound())
		{
			return SourceAccessDelegate.Execute(FileName, LineNumber, ColumnNumber);
		}
		return false;
	}
	...
	{
		AssetAccessDelegate = InDelegate;
	}

	bool HandleAssetAccessDelegate(UObject* Asset) const
	{
		if (AssetAccessDelegate.IsBound())
		{
			return AssetAccessDelegate.Execute(Asset);
		}
		return false;
	}
	
	virtual void SetDebugObjectAccessDelegate(FAccessDebugObject InDelegate)override
	{
		DebugObjectAccessDelegate = InDelegate;
	}
	...
	/** Clear previous selection and select the given node in the reflector tree by expanding its path and scrolling it into view. */
	void SelectReflectorNode(TSharedRef<FWidgetReflectorNodeBase> InNode);

	/** Get all selected live widgets. */
	TArray<TSharedPtr<const SWidget>> GetSelectedLiveWidgets() const;

	/** Returns true when Drawable picking mode is active. */
	bool IsDrawablePickingActive() const { return PickingMode == EWidgetPickingMode::Drawable; }
	...
	void SetPickingMode(EWidgetPickingMode InMode)
	{
		...
		{
			// If leaving Drawable mode, remove its entry from the enablers.
			if (PickingMode == EWidgetPickingMode::Drawable)
			{
				DisableVisualCapture(VisualCaptureEnabled::WidgetPickingMode);
			}

			// Enable the picking mode.
			PickingMode = InMode;

			// If we're entering Drawable mode, add its entry to the enablers.
			if (PickingMode == EWidgetPickingMode::Drawable)
			{
				EnableVisualCapture(VisualCaptureEnabled::WidgetPickingMode);
			}
			else if (PickingMode != EWidgetPickingMode::None && !VisualCapture.IsEnabled())
			{
				VisualCapture.Reset();
			}
		}
	...
	/** Enable the VisualCapture. */
	void EnableVisualCapture(FName Enabler);

	/** Disable the VisualCapture if there are other enablers. */
	void DisableVisualCapture(FName Enabler);
...
#endif
	TSharedPtr<SVisualElementsTab> VisualElementsTabWidget;
	TWeakPtr<UE::SlateReflector::SWidgetSearchPanel> SearchPanel;

	FVisualTreeCapture VisualCapture;
	/** Controls whether VisualCapture is active. Each entry is a named reason for keeping it enabled. */
	TArray<FName> VisualCaptureEnablers;
...
void SWidgetReflector::Construct( const FArguments& InArgs )
{
		...
		(
			...
			(
				...
				(
					...
					->AddTab(WidgetReflectorTabID::WidgetSearch, ETabState::ClosedTab)
#if WITH_SLATE_DEBUGGING
					...
					->AddTab(WidgetReflectorTabID::VisualElements, ETabState::ClosedTab)
				)
			)
#if WITH_EDITOR
			->Split
	...
	RegisterTrackedTabSpawner(WidgetReflectorTabID::WidgetSearch, FOnSpawnTab::CreateSP(this, &SWidgetReflector::SpawnWidgetSearchTab))
		.SetDisplayName(LOCTEXT("WidgetSearchTab", "Widget Search"));

	RegisterTrackedTabSpawner(WidgetReflectorTabID::VisualElements, FOnSpawnTab::CreateSP(this, &SWidgetReflector::SpawnVisualElementsTab))
		.SetDisplayName(LOCTEXT("VisualElementsTab", "Elements"));
...
float SWidgetReflector::GetColumnWidth(FName InColumn, float InDefault) const
{
	if (const float* Width = ColumnWidths.Find(InColumn))
	{
		return FMath::Max(*Width, InDefault);
	}

	return InDefault;
}

void SWidgetReflector::UpdateColumnWidth(TSharedRef<FWidgetReflectorNodeBase> InItem, FName InColumn)
{
	if (ReflectorTree.IsValid())
	{
		if (TSharedPtr<SReflectorTreeWidgetItem> Row = StaticCastSharedPtr<SReflectorTreeWidgetItem>(ReflectorTree->WidgetFromItem(InItem)))
		{
			if (const TSharedRef<SWidget>* WidgetPtr = Row->GetWidgetFromColumnId(InColumn))
			{
				const float WidgetWidth = (*WidgetPtr)->GetDesiredSize().X;
				float& Width = ColumnWidths.FindOrAdd(InColumn);
				...
				if (WidgetWidth > Width)
				{
					...
					if (ReflectorTree.IsValid())
					{
						ReflectorTree->GetHeaderRow()->SetColumnWidth(InColumn, WidgetWidth);
						ReflectorTree->GetHeaderRow()->OnColumnsChanged();
					}
				}
			}
		}
					...
					[
						...
						[
							...
							[
								...
								.ScrollBarStyle(&FWidgetReflectorStyle::Get().GetWidgetStyle<FScrollBarStyle>("ScrollBar.Hidden"))
								.HeaderRow
								(
									SNew(SHeaderRow)
									...
									.ManualWidth(this, &SWidgetReflector::GetColumnWidth, SReflectorTreeWidgetItem::NAME_WidgetName, 100.f)
									...
									.ManualWidth(this, &SWidgetReflector::GetColumnWidth, SReflectorTreeWidgetItem::NAME_LayerId, 100.f)

									+ SHeaderRow::Column(SReflectorTreeWidgetItem::NAME_ActualSize)
									.DefaultLabel(LOCTEXT("ActualSize", "Size"))
									.ManualWidth(this, &SWidgetReflector::GetColumnWidth, SReflectorTreeWidgetItem::NAME_ActualSize, 100.f)

									+SHeaderRow::Column(SReflectorTreeWidgetItem::NAME_WidgetInfo)
									.DefaultLabel(LOCTEXT("Source", "Source" ))
									.ManualWidth(this, &SWidgetReflector::GetColumnWidth, SReflectorTreeWidgetItem::NAME_WidgetInfo, 200.f)
...
TSharedRef<SDockTab> SWidgetReflector::SpawnWidgetList(const FSpawnTabArgs& Args)
{
	return SNew(SDockTab)
		...
		[
			SNew(SWidgetList)
			.OnAccessSource(FAccessSourceCode::CreateSP(this, &SWidgetReflector::HandleSourceAccessDelegate))
			.OnAccessAsset(FAccessAsset::CreateSP(this, &SWidgetReflector::HandleAssetAccessDelegate))
		];
}

TSharedRef<SDockTab> SWidgetReflector::SpawnVisualElementsTab(const FSpawnTabArgs& Args)
{
	return SNew(SDockTab)
		.Label(LOCTEXT("DrawVisualElementsTab", "Draw Elements"))
		[
			SAssignNew(VisualElementsTabWidget, SVisualElementsTab, &VisualCapture)
			.OnWidgetSelected(this, &SWidgetReflector::SelectLiveWidget)
			.FilterWidgets(this, &SWidgetReflector::GetSelectedLiveWidgets)
			.IsPickingActive(this, &SWidgetReflector::IsDrawablePickingActive)
		];
}

TSharedRef<SDockTab> SWidgetReflector::SpawnWidgetSearchTab(const FSpawnTabArgs& Args)
{
	TSharedRef<SDockTab> SpawnedTab = SNew(SDockTab)
		.Label(LOCTEXT("WidgetSearchTab", "Widget Search"))
		[
			SNew(UE::SlateReflector::SWidgetSearchPanel)
			.OnSelectNode(this, &SWidgetReflector::SelectReflectorNode)
			.OnSelectLiveWidget(this, &SWidgetReflector::SelectLiveWidget)
			.OnGetRootNodes_Lambda([this]() -> TArray<TSharedRef<FWidgetReflectorNodeBase>>
			{
				return ReflectorTreeRoot;
			})
			.OnAccessSourceCode(FAccessSourceCode::CreateSP(this, &SWidgetReflector::HandleSourceAccessDelegate))
			.OnAccessAsset(FAccessAsset::CreateSP(this, &SWidgetReflector::HandleAssetAccessDelegate))
		];
...
void SWidgetReflector::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
	...
	//if (VisualCapture.IsEnabled() && VisualElementsTabWidget.IsValid())
	//{
	//	VisualElementsTabWidget->SetDrawablePickingActive(PickingMode == EWidgetPickingMode::Drawable);
	//	VisualElementsTabWidget->Refresh();
	//}
}
...
int32 SWidgetReflector::Visualize( const FWidgetPath& InWidgetsToVisualize, FSlateWindowElementList& OutDrawElements, int32 LayerId )
{
	...
	if (PickingMode == EWidgetPickingMode::Drawable)
	{
		TSharedPtr<const FVisualTreeSnapshot> Tree = VisualCapture.GetVisualTreeForWindow(OutDrawElements.GetPaintWindow());
		if (Tree)
		{
...
void SWidgetReflector::SelectLiveWidget(TSharedPtr<const SWidget> InWidget)
{
	...
	if (bFound && TabManager.IsValid())
	{
		TabManager->TryInvokeTab(WidgetReflectorTabID::WidgetHierarchy);
	}
}

void SWidgetReflector::SelectReflectorNode(TSharedRef<FWidgetReflectorNodeBase> InNode)
{
	if (!ReflectorTree.IsValid())
	{
		return;
	...
	// Walk up the parent chain and expand each ancestor so the node is visible in the tree.
	TArray<TSharedRef<FWidgetReflectorNodeBase>> PathToExpand;
	TSharedPtr<FWidgetReflectorNodeBase> Parent = InNode->GetParentNode();
	while (Parent.IsValid())
	{
		PathToExpand.Add(Parent.ToSharedRef());
		Parent = Parent->GetParentNode();
	}

	for (int32 Index = PathToExpand.Num() - 1; Index >= 0; --Index)
	{
		const TSharedRef<FWidgetReflectorNodeBase>& PathNode = PathToExpand[Index];
		ReflectorTree->SetItemExpansion(PathNode, true);
	}

	ReflectorTree->SetSelection(InNode);
	ReflectorTree->RequestScrollIntoView(InNode);
}

TArray<TSharedPtr<const SWidget>> SWidgetReflector::GetSelectedLiveWidgets() const
{
	TArray<TSharedPtr<const SWidget>> Result;
	if (ReflectorTree)
	{
		TArray<TSharedRef<FWidgetReflectorNodeBase>> SelectedItems;
		ReflectorTree->GetSelectedItems(SelectedItems);
		for (const TSharedRef<FWidgetReflectorNodeBase>& Item : SelectedItems)
		{
			if (TSharedPtr<const SWidget> Widget = Item->GetLiveWidget())
			{
				Result.Add(Widget);
			}
		}
...
void SWidgetReflector::EnableVisualCapture(FName Enabler)
{
	VisualCaptureEnablers.AddUnique(Enabler);
	if (!VisualCapture.IsEnabled())
	{
		VisualCapture.Enable();
	}
}

void SWidgetReflector::DisableVisualCapture(FName Enabler)
{
	VisualCaptureEnablers.RemoveSingleSwap(Enabler);
	if (VisualCaptureEnablers.Num() == 0 && VisualCapture.IsEnabled())
	{
...
TSharedRef<ITableRow> SWidgetReflector::HandleReflectorTreeGenerateRow( TSharedRef<FWidgetReflectorNodeBase> InReflectorNode, const TSharedRef<STableViewBase>& OwnerTable )
{
	return SNew(SReflectorTreeWidgetItem, OwnerTable)
		.WidgetReflector(SharedThis(this))
		.WidgetInfoToVisualize(InReflectorNode)
		.ToolTip(GenerateToolTipForReflectorNode(InReflectorNode))
		.SourceCodeAccessor(FAccessSourceCode::CreateSP(this, &SWidgetReflector::HandleSourceAccessDelegate))
		.AssetAccessor(AssetAccessDelegate)
		.DebugObjectAccessor(DebugObjectAccessDelegate);
}
...
void DumpFontAtlasesCommand(const TArray<FString>& Args)
{
	...
	UE_LOGF(LogSlate, Display, "Dumping all current font atlases to directory '%ls'. All textures will be saved with a user provided file prefix of '%ls'", *SaveDirectoryName, *SaveFilePrefix);
	...
	for (int32 Index = 0; Index < NumFontAtlases; ++Index)
	{
		...
		if (!FontTexture)
		{
			UE_LOGF(LogSlate, Warning, "Font texture page %d is not a valid font texture. This page will not be saved.", Index);
			continue;
		}
		...
		{
			UE_LOGF(LogSlate, Log, "Texture page %d has no texture data. The texture will not be saved.", Index);
			continue;
		}
		...
		if (!ShaderResource)
		{
			UE_LOGF(LogSlate, Log, "Unable to retrieve the shader resource for texture page %d. Skipping saving this texture.", Index);
			continue;
		}
		...
		if (bSavedImage)
		{
			...
			UE_LOGF(LogSlate, Display, "Saving font texture '%ls'", *AbsoluteSaveFileName);
			if (!FFileHelper::SaveArrayToFile(SaveImageBuffer, *AbsoluteSaveFileName))
			{
				UE_LOGF(LogSlate, Warning, "Failed to save font texture '%ls'", *AbsoluteSaveFileName);
			}
		}
		else
		{
			UE_LOGF(LogSlate, Warning, "Failed to convert texture page %d to a buffer to be saved.", Index);
		}
	}
}
} // namespace WidgetReflectorImpl
