...
void FWidgetSnapshotData::LoadSnapshotFromJson(const TSharedRef<FJsonObject>& InRootJsonObject)
{
...

	{
		...
		{
			UE_LOGF(LogSlate, Error, "The version of the snapshot (%f) is older than the current version (%f). New fields will be initialized to their defaulted value.", VersionNumber, SnapshotJsonVersion);
		}
	}
