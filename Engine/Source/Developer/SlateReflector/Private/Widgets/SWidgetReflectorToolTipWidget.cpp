...
FText SReflectorToolTipWidget::GetWidgetsSourceName() const
{
#if WITH_EDITOR
	if (UUserWidget* SourceObject = Cast<UUserWidget>(WidgetInfo->GetWidgetSourceObject()))
	{
		...
		return FText::Format(
			LOCTEXT("WidgetTypeInAsset", "{0} in {1}"),
			FText::FromString(SourceClass->ClassGeneratedBy ? SourceClass->ClassGeneratedBy->GetName() : SourceClass->GetName()),
			WidgetInfo->GetWidgetReadableLocation()
		);
	}
#endif
