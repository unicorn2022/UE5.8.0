...
TSharedPtr<const SWidget> FVisualTreeSnapshot::Pick(FVector2f Point) const
{
	for (int32 Index = Entries.Num() - 1; Index >= 0; Index--)
	{
		const FVisualEntry& Entry = Entries[Index];
		if (Entry.ClippingIndex != INDEX_NONE)
		{
...
TArray<int32> FVisualTreeSnapshot::PickAll(FVector2f Point) const
{
	TArray<int32> Result;
...
TSharedPtr<const FVisualTreeSnapshot> FVisualTreeCapture::GetVisualTreeForWindow(SWindow* InWindow) const
{
