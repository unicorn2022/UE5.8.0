...
void AScreenshotFunctionalTestBase::PrepareForScreenshot()
{
	...
	if (bApplyScreenshotSettings)
	{
		...
		if (!FPlatformProperties::HasFixedResolution())
		{
			...
			if (ScreenshotOptions.bAllowEmulatingSplitscreen)
			{
				const int32 EmulateSplitscreenMode = CVarEmulateSplitscreenMode->GetInt();

				if (EmulateSplitscreenMode >= 1 && EmulateSplitscreenMode <= 3)
				{
					FIntPoint QuantizedScreenshotViewportSize;
					QuantizeSceneBufferSize(ScreenshotViewportSize, QuantizedScreenshotViewportSize);

					if (ScreenshotViewportSize != QuantizedScreenshotViewportSize)
					{
						UE_LOGF(LogScreenshotFunctionalTest, Warning, "Screenshot size %ls does not match quantized screenshot size %ls, emulated splitscreen rendering may behave incorrectly.",
							*ScreenshotViewportSize.ToString(), *QuantizedScreenshotViewportSize.ToString());
					}
				}
				...
				switch (EmulateSplitscreenMode)
				{
					...
					case 3:	// Diagonal
						...
						break;
					default:
						ensureMsgf(false, TEXT("Selected r.ScreenshotTest.EmulateSplitscreenMode of %d is invalid. Defaulting to 0 (off)."), EmulateSplitscreenMode);
						break;
				}
			}
...
void AScreenshotFunctionalTestBase::OnScreenShotCaptured(int32 InSizeX, int32 InSizeY, const TArray<FColor>& InImageData)
{
	...
	UE_LOGF(LogScreenshotFunctionalTest, Log, "Screenshot captured as %ls", *Data.ScreenshotPath);
#endif
}
...
void AScreenshotFunctionalTestBase::RequestScreenshot()
{
	...
	if (GDumpGPUDumpOnScreenshotTest.GetValueOnGameThread() != 0)
	{
		FRDGBuilder::BeginResourceDump(TEXT(""), TEXT("ScreenshotTest"));
	}
#endif
}
