...
namespace
{
	...
	bool PerformComparison(const T& lhs, const T& rhs, EComparisonMethod comparison)
	{
		...
		UE_LOGF(LogFunctionalTest, Error, "Invalid comparison method");
		return false;
	}
...
void AFunctionalTest::FinishTest(EFunctionalTestResult TestResult, const FString& Message)
{
	...
	//	UE_LOGF(LogFunctionalTest, Log, "%ls", *AdditionalDetails);
	//}
...
{
	UE_LOGF(LogFunctionalTest, Log, "%ls", *Message);
...
void AFunctionalTest::LogStep(ELogVerbosity::Type Verbosity, const FString& Message)
{
	...
	{
		UE_LOGF(LogFunctionalTest, Warning, "FunctionalTest '%ls' ran test '%ls' when no functional test was active. This result will not be tracked.", *TestLabel, *Message);
	}
	...
	*/
	...
	case ELogVerbosity::Log:
		...
		else
		{
			...
			UE_LOGF(LogFunctionalTest, Log, "%ls", *FullMessage);
		}
		break;
	...
	case ELogVerbosity::Display:
		...
		else
		{
			...
			UE_LOGF(LogFunctionalTest, Display, "%ls", *FullMessage);
		}
		break;
	...
	case ELogVerbosity::Warning:
		...
		else
		{
			...
			UE_LOGF(LogFunctionalTest, Warning, "%ls", *FullMessage);
		}
		break;
	...
	case ELogVerbosity::Error:
		...
		else
		{
			...
			UE_LOGF(LogFunctionalTest, Error, "%ls", *FullMessage);
		}
		break;
	}
}
...
void UAutomationPerformaceHelper::EndRecording()
{
	...
	{
		UE_LOGF(LogFunctionalTest, Log, "Finished Perf Stats Record:\n%ls", *Record->GetReportString());
	}
	bRecordingBasicStats = false;
}
...
void UAutomationPerformaceHelper::Tick(float DeltaSeconds)
{
	...
	if (bGPUTraceIfBelowBudget)
	{
		if (!IsCurrentRecordWithinGPUBudget())
		{
			...
			UE_LOGF(LogFunctionalTest, Log, "Functional Test has fallen below GPU budget. Performing GPU trace.");
...
void UAutomationPerformaceHelper::WriteLogFile(const FString& CaptureDir, const FString& CaptureExtension)
{
	...
	UE_LOGF(LogTemp, Display, "Finished test, wrote file to %ls", *FilenameFull);
...
void UAutomationPerformaceHelper::StartCPUProfiling()
{
	...
	UE_LOGF(LogFunctionalTest, Log, "START PROFILING...");
...
{
#if UE_EXTERNAL_PROFILING_ENABLED
	UE_LOGF(LogFunctionalTest, Log, "STOP PROFILING...");
