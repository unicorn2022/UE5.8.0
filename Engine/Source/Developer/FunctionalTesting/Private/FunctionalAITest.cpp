...
void AFunctionalAITestBase::BeginPlay()
{
	...
	RemoveSpawnSetIfPredicate([&](FAITestSpawnSetBase& SpawnSet) {
		...
		{
			UE_LOGF(LogFunctionalTest, Log, "Removing disabled spawn set \'%ls\'.", *SpawnSet.Name.ToString());
			return true;
		}
		return false;
	});
...
void AFunctionalAITestBase::OnTimeout()
{
	...
	if (NavSys)
	{
		...
		UE_LOGF(LogFunctionalTest, Log, "Test timed out, log details for: %ls", *GetNameSafe(Navmesh));
		UE_LOGF(LogFunctionalTest, Log, "> dirty areas? %ls", NavSys->HasDirtyAreasQueued() ? TEXT("YES") : TEXT("no"));
		...
		NavigationOctree->FindElementsWithBoundsTest(DebugBounds, [&AreaFilter, &Navmesh](const FNavigationOctreeElement& Element)
		{
			if (Element.IsMatchingFilter(AreaFilter))
			{
				...
				UE_LOGF(LogFunctionalTest, Log, "> modifier, owner:%ls areas:%ls", *Element.GetSourceElement()->GetName(), *DebugAreaNames);
			}
		});
	}
...
void AFunctionalAITestBase::StartSpawning()
{
	...
	SpawnSet->ForEachSpawnInfo([&](FAITestSpawnInfoBase& SpawnInfo) {
		...
		else
		{
			...
			UE_LOGF(LogFunctionalTest, Warning, "%ls", *SpawnFailureMessage);
...
bool FAITestSpawnInfo::Spawn(AFunctionalAITestBase* AITest) const
{
	...
	if (SpawnedPawn == NULL)
	{
		...
		UE_LOGF(LogFunctionalTest, Warning, "%ls", *FailureMessage);
	}
