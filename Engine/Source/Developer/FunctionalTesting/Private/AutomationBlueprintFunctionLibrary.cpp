...
public:
	...
	void DeleteSelfNextFrame()
	{
		if (!bDeleteQueued)
		{
			FTSTicker::GetCoreTicker().AddTicker(TEXT("ScreenshotCleanup"), 0.1f, [this](float) {
				delete this;
				return false;
				});
			bDeleteQueued = true;
	...
	void GrabScreenShot(int32 InSizeX, int32 InSizeY, const TArray<FColor>& InImageData)
	{
		...
		if (World.IsValid())
		{
			...
			UE_LOGF(AutomationFunctionLibrary, Log, "Screenshot captured as %ls", *Data.ScreenshotPath);
	...
	{
		UE_LOGF(AutomationFunctionLibrary, Log, "Screenshot processed, but not compared.");
		...
		if (FModuleManager::Get().IsModuleLoaded("LevelEditor"))
		{
			...
			if (LevelViewport)
			{
				if (NeedGameViewToggle)
				{
					...
					{
						UE_LOGF(AutomationFunctionLibrary, Verbose, "Expected to be able to toggle off the Game View mode after the screenshot was taken, but the Viewport was already no longer in that mode or it is not a Perspective.");
					}
				}
				if (NeedCameraChange)
				{
...
void UAutomationBlueprintFunctionLibrary::TakeAutomationScreenshot(UObject* WorldContextObject, FLatentActionInfo LatentInfo, const FString& InScreenShotName, const FString& Notes, const FAutomationScreenshotOptions& Options)
{
	if ( GIsAutomationTesting )
	{
		...
		if ( ScreenShotName.IsEmpty() )
		{
			...
			UE_LOGF(AutomationFunctionLibrary, Warning, "Screenshot name is empty. Default name will be used.");
		}
	...
	{
		UE_LOGF(AutomationFunctionLibrary, Log, "Screenshot not captured - screenshots are only taken during automation tests");
	}
}
...
float HelperGetStat(FName StatName)
{
	...
	UE_LOGF(AutomationFunctionLibrary, Warning, "%ls", *WarningOut.ToString());
#endif
...
UAutomationEditorTask* UAutomationBlueprintFunctionLibrary::TakeHighResScreenshot(int32 ResX, int32 ResY, FString Filename, ACameraActor* Camera, bool bMaskEnabled, bool bCaptureHDR, EComparisonTolerance ComparisonTolerance, FString ComparisonNotes, float Delay, bool bForceGameView)
{
	...
	if (FModuleManager::Get().IsModuleLoaded("LevelEditor"))
	{
		...
		UE_LOGF(AutomationFunctionLibrary, Error, "Screenshot size exceeds the maximum allowed texture size (%d x %d)", GetMax2DTextureDimension(), GetMax2DTextureDimension());
	}
#endif
	return Task;
}
...
bool UAutomationBlueprintFunctionLibrary::CompareImageAgainstReference(FString InImagePath, FString ComparisonName, EComparisonTolerance InTolerance, FString InNotes, UObject* WorldContextObject)
{
	...
	if (GIsAutomationTesting)
	{
		...
		{
			UE_LOGF(AutomationFunctionLibrary, Error, "Unable to locate image processor for %ls file format", *ImageExtension);
			return false;
		}
		...
		if (!OpenSuccess)
		{
			UE_LOGF(AutomationFunctionLibrary, Error, "Unable to read image %ls", *InImagePath);
			return false;
		}

		if (!ImageReader->SetCompressed(ImageData.GetData(), ImageData.Num()))
		{
			UE_LOGF(AutomationFunctionLibrary, Error, "Unable to parse image %ls", *InImagePath);
			return false;
		}

		if (ImageReader->GetBitDepth() != 8)
		{
			UE_LOGF(AutomationFunctionLibrary, Error, "Automation can only compare 8bit depth channel. %ls has %dbit per channel.", *InImagePath, ImageReader->GetBitDepth());
			return false;
		}
		...
		{
			UE_LOGF(AutomationFunctionLibrary, Error, "Unable to decompress image %ls", *InImagePath);
			return false;
		}
...
#endif
	UE_LOGF(AutomationFunctionLibrary, Warning, "Can compare image only during test automation.");
	return false;
}
