...
bool UFunctionalTestingManager::RunAllFunctionalTests(UObject* WorldContextObject, bool bNewLog /*= true*/, bool bRunLooped /*= true*/, FString ReproString /*= TEXT("")*/)
{
	...
	{
		UE_LOGF(LogFunctionalTest, Log, "Functional tests are already running.");
		return true;
	}
	...
	{
		UE_LOGF(LogFunctionalTest, Log, "Running tests indicated by Repro String: %ls", *ReproString);
	...
	{
		UE_LOGF(LogFunctionalTest, Warning, "No tests defined on map or . DONE.");
		return false;
	}
...
void UFunctionalTestingManager::TriggerFirstValidTest()
{
	...
	if(bInEditorWorld)
	{
		UE_LOGF(LogTemp, Log, "Running %ls in Editor World", TestReproStrings.Num() > 0 ? *TestReproStrings[0] : TEXT("unknown test"));
	}
#endif
...
void UFunctionalTestingManager::AllTestsDone()
{
	if (bLooped == true)
	{
		...
		UE_LOGF(LogFunctionalTest, Log, "----- Starting iteration %d -----", CurrentIteration);
		bIsRunning = RunFirstValidTest();
		if (bIsRunning == false)
		{
			UE_LOGF(LogFunctionalTest, Warning, "Failed to start another iteration.");
		}
	}
	else
	{
...
bool UFunctionalTestingManager::RunFirstValidTest()
{
	...
	if (TestReproStrings.Num() > 0)
	{
		...
		if (World == nullptr)
		{
			UE_LOGF(LogFunctionalTest, Warning, "Unable to find testing world!");
		...
		while (TestReproStrings.Num() > 0)
		{
			...
			if (TestParams.Num() == 0)
			{
				UE_LOGF(LogFunctionalTest, Warning, "Unable to parse \'%ls\'", *SingleTestReproString);
				continue;
			}
			...
			if (TestToRun)
			{
				...
				{
					UE_LOGF(LogFunctionalTest, Warning, "Test \'%ls\' failed to start", *TestToRun->GetName());
				}
			}
			else
			{
				UE_LOGF(LogFunctionalTest, Warning, "Unable to find test \'%ls\' in world %ls, the available tests are...", *TestName, *World->GetFullName());

				// Find Actors by type (needs a UWorld object)
				for (TActorIterator<AFunctionalTest> It(World); It; ++It)
				{
					UE_LOGF(LogFunctionalTest, Warning, "\'%ls\'.", *It->GetName());
				}
			}
		}
	}
	...
	if (bTestSuccessfullyTriggered == false)
	{
		for (int32 Index = TestsLeft.Num()-1; Index >= 0; --Index)
		{
			...
			if (TestsLeft[Index] != NULL)
			{
				...
				{
					UE_LOGF(LogFunctionalTest, Warning, "Test: %ls failed to start", *TestsLeft[Index]->GetName());
					bRemove = true;
				}
			}
