...
public:
	...
	/**
	 * Creates a new analysis session instance using a secure connection to the specified host.
	 ...
	 */
	void ConnectToSecureTrace(FStringView Host, uint16 Port, TConstArrayView<uint8> AuthorizationToken, FStringView CertificateFile = FStringView(), TConstArrayView<uint8> CertificateAuthority = TConstArrayView<uint8>(), FAnsiStringView Identity = FAnsiStringView());
