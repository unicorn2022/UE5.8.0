...
void FLogFilteringAsyncTask::DoWork()
{
	...
	{
		UE_LOGF(TraceInsights, Log, "[LogView] FLogFilteringAsyncTask::DoWork [%d to %d] by Text (\"%ls\")", StartIndex, EndIndex, *Filter.GetFilterText().ToString());
	}
	else
	{
		UE_LOGF(TraceInsights, Log, "[LogView] FLogFilteringAsyncTask::DoWork [%d to %d]", StartIndex, EndIndex);
	}
#endif // !WITH_EDITOR
	...
	if (bCanceled)
	{
		FilteredMessages.Reset();
#if !WITH_EDITOR
		UE_LOGF(TraceInsights, Log, "[LogView] FLogFilteringAsyncTask::DoWork CANCELED");
#endif // !WITH_EDITOR
	}
	else
	{
#if !WITH_EDITOR
		UE_LOGF(TraceInsights, Log, "[LogView] FLogFilteringAsyncTask::DoWork DONE (%d filtered messages)", FilteredMessages.Num());
#endif // !WITH_EDITOR
	}
}
