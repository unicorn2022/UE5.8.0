...
uint32 FTimingEvent::ComputeEventColor(const TCHAR* Str, TConstArrayView<uint8> Metadata)
{
	uint32 Color = 0;
	// Let the special event names (ex. "<unknown>", "<noname>", "<invalid>", etc.) to be colored in black.
	if (Str != nullptr && *Str != TEXT('<'))
	...
	for (int Index = 0; Index < Metadata.Num(); ++Index)
	{
		Color = (Color + Metadata[Index]) * 0x2c2c57ed;
	}
