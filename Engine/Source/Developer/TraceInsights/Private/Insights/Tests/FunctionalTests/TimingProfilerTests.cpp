...
bool FEnumerateTest::RunTest(const FString& Parameters)
{
	...
	TestEqual(TEXT("SumTimerIndex"), (uint64)CheckValues.SumTimerIndex, 4137608268ull);
...
bool FEnumeratePerformanceTest::RunTest(const FString& Parameters)
{
	...
	TestEqual(TEXT("SumTimerIndex"), (uint64)CheckValues.SumTimerIndex, 3636619455ull);
...
bool FEnumerateFastTest::RunTest(const FString& Parameters)
{
	...
	TestEqual(TEXT("SumTimerIndex"), (uint64)CheckValues.SumTimerIndex, 1241561258ull);
		...
		if (TimingProfilerProvider)
		{
			...
			TimingProfilerProvider->ReadTimeline(TimelineIndex,
				...
				(const TraceServices::ITimingProfilerProvider::Timeline& Timeline)
				{
					Timeline.EnumerateEvents(-1.0, Session->GetDurationSeconds() + 1.0,
						[&ArchiveWriter, &TimerReader]
						(bool bStart, double Time, const TraceServices::FTimingProfilerEvent& Event)
						{
							const TraceServices::FTimingProfilerTimer* Timer = TimerReader.GetTimer(Event.TimerIndex);
...
{
	UE_LOGF(TimingProfilerTests, Log, "RUNNING BENCHMARK...");
	...
	if (Session.IsValid())
	{
		...
		if (TimingProfilerProvider)
		{
			...
			TimingProfilerProvider->ReadTimeline(TimelineIndex,
				[&OutCheckValues, &InParams, TimeIncrement]
				(const TraceServices::ITimingProfilerProvider::Timeline& Timeline)
				{
					double Time = 0.0;
	...
	UE_LOGF(TimingProfilerTests, Log, "BENCHMARK RESULT: %f seconds", OutCheckValues.EnumerationDuration);
	UE_LOGF(TimingProfilerTests, Log, "SessionTime: %f seconds", SessionTime);
	UE_LOGF(TimingProfilerTests, Log, "TimelineIndex: %u", TimelineIndex);
	UE_LOGF(TimingProfilerTests, Log, "Check Values: %f %llu %u %u", OutCheckValues.TotalEventDuration, OutCheckValues.EventCount, OutCheckValues.SumDepth, OutCheckValues.SumTimerIndex);
}

void FTimingProfilerTests::RunEnumerateAsyncBenchmark(const FEnumerateTestParams& InParams, FCheckValues& OutCheckValues)
{
	UE_LOGF(TimingProfilerTests, Log, "RUNNING ASYNC ENUMERATE BENCHMARK...");
	...
	if (Session.IsValid())
	{
		...
		if (TimingProfilerProvider)
		{
			...
			TimingProfilerProvider->ReadTimeline(TimelineIndex,
				[SessionTime, &InParams, TimeIncrement, &TaskCheckValues]
				(const TraceServices::ITimingProfilerProvider::Timeline& Timeline)
				{
					double Time = 0.0;
					for (int32 Index = 0; Index < InParams.NumEnumerations; ++Index)
					{
						...
						Params.SetupCallback =
							[&TaskCheckValues](uint32 NumTasks)
							{
								TaskCheckValues.AddDefaulted(NumTasks);
							};
						Params.EventRangeCallback =
							[&TaskCheckValues, SessionTime]
							(double EventStartTime, double EventEndTime, uint32 EventDepth, const TraceServices::FTimingProfilerEvent& Event, uint32 TaskIndex)
							{
	...
	Stopwatch.Stop();
	OutCheckValues.EnumerationDuration = Stopwatch.GetAccumulatedTime();
	UE_LOGF(TimingProfilerTests, Log, "ASYNC ENUMERATE BENCHMARK RESULT: %f seconds", OutCheckValues.EnumerationDuration);
	UE_LOGF(TimingProfilerTests, Log, "SessionTime: %f seconds", SessionTime);
	UE_LOGF(TimingProfilerTests, Log, "TimelineIndex: %u", TimelineIndex);
	UE_LOGF(TimingProfilerTests, Log, "Check Values: %f %llu %u %u", OutCheckValues.TotalEventDuration, OutCheckValues.EventCount, OutCheckValues.SumDepth, OutCheckValues.SumTimerIndex);
}
...
{
	UE_LOGF(TimingProfilerTests, Log, "RUNNING ENUMERATE ALL TRACKS BENCHMARK...");
	...
	if (Session.IsValid())
	{
		...
		if (TimingProfilerProvider)
		{
			...
			ThreadProvider.EnumerateThreads(
				[&TimelineIndex, TimingProfilerProvider, &OutCheckValues, &InParams, TimeIncrement]
	...
	Stopwatch.Stop();
	OutCheckValues.EnumerationDuration = Stopwatch.GetAccumulatedTime();
	UE_LOGF(TimingProfilerTests, Log, "ENUMERATE ALL TRACKS BENCHMARK RESULT: %f seconds", OutCheckValues.EnumerationDuration);
	UE_LOGF(TimingProfilerTests, Log, "SessionTime: %f seconds", SessionTime);
	UE_LOGF(TimingProfilerTests, Log, "TimelineIndex: %u", TimelineIndex);
	UE_LOGF(TimingProfilerTests, Log, "Check Values: %f %llu %u %u", OutCheckValues.TotalEventDuration, OutCheckValues.EventCount, OutCheckValues.SumDepth, OutCheckValues.SumTimerIndex);
}
...
{
	UE_LOGF(TimingProfilerTests, Log, "RUNNING ASYNC ENUMERATE ALL TRACKS BENCHMARK...");
	...
	if (Session.IsValid())
	{
		...
		if (TimingProfilerProvider)
		{
			...
			const TraceServices::IThreadProvider& ThreadProvider = TraceServices::ReadThreadProvider(*Session.Get());
			ThreadProvider.EnumerateThreads(
				[&TimelineIndex, TimingProfilerProvider, &OutCheckValues, &InParams, TimeIncrement, &TaskCheckValues]
				(const TraceServices::FThreadInfo& ThreadInfo)
				{
					TimingProfilerProvider->GetCpuThreadTimelineIndex(ThreadInfo.Id, TimelineIndex);

					TimingProfilerProvider->ReadTimeline(TimelineIndex,
						[&OutCheckValues, &InParams, TimeIncrement, &TaskCheckValues]
						(const TraceServices::ITimingProfilerProvider::Timeline& Timeline)
						{
							double Time = 0.0;
							for (int32 Index = 0; Index < InParams.NumEnumerations; ++Index)
							{
								...
								Params.EventRangeCallback =
									[&OutCheckValues, &TaskCheckValues]
									(double EventStartTime, double EventEndTime, uint32 EventDepth, const TraceServices::FTimingProfilerEvent& Event, uint32 TaskIndex)
									{
										EventEndTime = FMath::Min(EventEndTime, OutCheckValues.SessionDuration);
										TaskCheckValues[TaskIndex].TotalEventDuration += EventEndTime - EventStartTime;
	...
	Stopwatch.Stop();
	OutCheckValues.EnumerationDuration = Stopwatch.GetAccumulatedTime();
	UE_LOGF(TimingProfilerTests, Log, "ASYNC ENUMERATE ALL TRACKS BENCHMARK RESULT: %f seconds", OutCheckValues.EnumerationDuration);
	UE_LOGF(TimingProfilerTests, Log, "SessionTime: %f seconds", SessionTime);
	UE_LOGF(TimingProfilerTests, Log, "TimelineIndex: %u", TimelineIndex);
	UE_LOGF(TimingProfilerTests, Log, "Check Values: %f %llu %u %u", OutCheckValues.TotalEventDuration, OutCheckValues.EventCount, OutCheckValues.SumDepth, OutCheckValues.SumTimerIndex);
}
...
uint32 FTimingProfilerTests::GetTimelineIndex(const TCHAR* InName)
{
	...
	// Assumes this is called under FAnalysisSessionReadScope.
	TSharedPtr<const TraceServices::IAnalysisSession> Session = UE::Insights::FInsightsManager::Get()->GetSession();
	...
	if (TimingProfilerProvider)
	{
		const TraceServices::IThreadProvider& ThreadProvider = TraceServices::ReadThreadProvider(*Session.Get());
		ThreadProvider.EnumerateThreads(
			[&TimelineIndex, TimingProfilerProvider, InName]
			(const TraceServices::FThreadInfo& ThreadInfo)
			{
