...
{
public:
	FScopedLog(const TCHAR* InName, double InMinDuration = 0.001)
		: Name(InName), MinDuration(InMinDuration)
	{
		Stopwatch.Start();
	}
	...
	~FScopedLog()
	{
		...
		{
			UE_LOGF(LogTimingProfiler, Log, "%ls took %.3f seconds.", Name, Duration);
		}
	}
...
#define INSIGHTS_SCOPED_LOG(Name) \
	FScopedLog ScopedLog(TEXT(#Name));
...
STimingView::STimingView()
	...
	, QuickFindTabId(TEXT("InsightsQuickFind"), TimingViewId++)
{
...
TSharedPtr<FFilterConfigurator> STimingView::MakeCustomFilterConfigurator(uint32 TimerId, double IntervalStart, double IntervalEnd)
{
	...
	FFilterConfiguratorNodePtr StartTimeNode = MakeShared<FFilterConfiguratorNode>(false);
	...
	FFilterConfiguratorNodePtr EndTimeNode = MakeShared<FFilterConfiguratorNode>(false);
	...
	FFilterConfiguratorNodePtr TimerIdNode = MakeShared<FFilterConfiguratorNode>(false);
...
void STimingView::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	...
	constexpr float HorizontalScrollBarHeight = 12.0f;

	Viewport.SetPosY(ToolbarHeight);
	Viewport.UpdateSize(FMath::RoundToFloat(ViewWidth), FMath::RoundToFloat(ViewHeight) - Viewport.GetPosY() - HorizontalScrollBarHeight);
	...
	if (Session)
	{
			...
			SessionTime != std::numeric_limits<double>::infinity())
		{
			...
			//UE_LOGF(LogTimingProfiler, Log, "Session Duration: %g", DT);
	...
	if (bAutoScroll)
	{
		...
		if (static_cast<double>(CurrentTime - LastAutoScrollTime) * FPlatformTime::GetSecondsPerCycle64() > AutoScrollMinDelay)
		{
			...
			// By default, align the current session time with the right side of the viewport + offset.
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	VerticalScrollBar->SetPadding(FMargin(0.0f, Viewport.GetPosY() + TopOffset + 2.0f, 0.0f, BottomOffset + HorizontalScrollBarHeight));

	////////////////////////////////////////////////////////////////////////////////////////////////////

	TSharedPtr<FBaseTimingTrack> PinnedTrack;
	float InitialPinnedTrackPosY = 0.0f;
	if (SelectedTrack.IsValid() && SelectedTrack->IsVisible() && DelayedVisibleTrackCounter <= 0)
	{
		if (ScrollableTracks.Contains(SelectedTrack))
		{
			PinnedTrack = SelectedTrack;
			InitialPinnedTrackPosY = PinnedTrack->GetPosY();
		}
	}
	...
	if (PinnedTrack.IsValid())
	{
		...
		const float AdjustmentDY = LastScrollPosY - InitialScrollPosY + InitialPinnedTrackPosY - PinnedTrack->GetPosY();
		const float NewScrollPosY = InitialScrollPosY - AdjustmentDY;
		if (!FMath::IsNearlyZero(AdjustmentDY, 0.5f))
		{
			ViewportScrollPosYOnButtonDown -= AdjustmentDY;
			ScrollAtPosY(NewScrollPosY);
			UpdatePositionForScrollableTracks();
		}
	}

	// Force the DelayedVisibleTrack to be visible.
	if (DelayedVisibleTrackCounter > 0)
	{
		if (DelayedVisibleTrack.IsValid() &&
			DelayedVisibleTrack->IsVisible() &&
			DelayedVisibleTrack->GetLocation() == ETimingTrackLocation::Scrollable)
		{
			float PreviousScrollPosY = Viewport.GetScrollPosY();

			// Try to bring into view the entire track (with 10% top/bottom margins).
			const float TrackHeight = DelayedVisibleTrack->GetHeight();
			const float TopScrollY = DelayedVisibleTrack->GetPosY() + Viewport.GetScrollPosY() - Viewport.GetTopOffset() - Viewport.GetPosY();
			const float ScrollableAreaHeight = Viewport.GetScrollableAreaHeight();
			BringIntoViewY(TopScrollY - ScrollableAreaHeight * 0.1f, TopScrollY + TrackHeight + ScrollableAreaHeight * 0.2f);

			// Try to bring into view the entire track.
			BringIntoViewY(TopScrollY, TopScrollY + TrackHeight);

			if (DelayedVisibleTrackLaneDepth >= 0)
			{
				// Try to bring into view only the lane at specified depth (with 10% top/bottom margins).
				const float LaneHeight = Viewport.GetLayout().EventH + Viewport.GetLayout().EventDY;
				const float ScrollY = TopScrollY + float(DelayedVisibleTrackLaneDepth) * LaneHeight;
				BringIntoViewY(ScrollY - ScrollableAreaHeight * 0.1f, ScrollY + LaneHeight + ScrollableAreaHeight * 0.2f);
			}

			UpdatePositionForScrollableTracks();

			if (FMath::IsNearlyZero(Viewport.GetScrollPosY() - PreviousScrollPosY, 0.5f) &&
				FMath::IsNearlyZero(Viewport.GetScrollHeight() - DelayedVisibleTrackLastScrollHeight, 0.5f))
			{
				if (--DelayedVisibleTrackCounter == 0)
				{
					DelayedVisibleTrack.Reset();
				}
			}
			else
			{
				DelayedVisibleTrackCounter = 3;
				DelayedVisibleTrackLastScrollHeight = Viewport.GetScrollHeight();
			}
		}
		else
		{
			DelayedVisibleTrack.Reset();
			DelayedVisibleTrackCounter = 0;
		}
	}

	// Elastic snap to vertical scroll limits.
	if (!bIsPanning && DelayedVisibleTrackCounter <= 0)
	{
...
int32 STimingView::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	...
	const FLinearColor BackgroundColor = FSlateColor(EStyleColor::Panel).GetSpecifiedColor();

	// Fill the background of the toolbar.
	DrawContext.DrawBox(0.0f, 0.0f, ViewWidth, Viewport.GetPosY(), WhiteBrush, BackgroundColor);
	...
	// Fill the background of the vertical scrollbar.
	{
		...
		if (ScrollBarHeight > 0)
		{
			...
			const     float ScrollBarPosX = ViewWidth - ScrollBarWidth;
			const     float ScrollBarPosY = Viewport.GetPosY() + Viewport.GetTopOffset();
			DrawContext.DrawBox(ScrollBarPosX, ScrollBarPosY, ScrollBarWidth, ScrollBarHeight, WhiteBrush, BackgroundColor);
		}
	}
	// Fill the background of the horizontal scrollbar.
	{
		const     float ScrollBarWidth = ViewWidth;
		constexpr float ScrollBarHeight = 12.0f;
		constexpr float ScrollBarPosX = 0.0f;
		const     float ScrollBarPosY = ViewHeight - ScrollBarHeight;
		DrawContext.DrawBox(ScrollBarPosX, ScrollBarPosY, ScrollBarWidth, ScrollBarHeight, WhiteBrush, BackgroundColor);
	}
...
void STimingView::AddTrack(TSharedPtr<FBaseTimingTrack> Track, ETimingTrackLocation Location)
{
	...
	if (TrackList.Num() >= MaxNumTracks)
	{
		...
		UE_LOGF(LogTimingProfiler, Warning, "Too many tracks already created (%d tracks)! Ignoring %ls track : %ls (\"%ls\")",
			TrackList.Num(),
			LocationName,
...
#if 0
	UE_LOGF(LogTimingProfiler, Log, "New %ls Track (%d) : %ls (\"%ls\")",
		LocationName,
		TrackList.Num() + 1,
		*Track->GetTypeName().ToString(),
		*Track->GetName());
...
#if 0
		UE_LOGF(LogTimingProfiler, Log, "Removed %ls Track (%d) : %ls (\"%ls\")",
			LocationName,
			TrackList.Num(),
			*Track->GetTypeName().ToString(),
			*Track->GetName());
	...
	if (!CursorDelta.IsZero())
	{
		...
		else if (TimeRulerTrack->IsScrubbing())
		{
			if (HasMouseCapture())
			{
				...
				// Snap to user annotations.
				{
					FUserAnnotationsTimingViewExtender* AnnotationExt = FindAnnotationExtender();
					if (AnnotationExt)
					{
						const double SnapTolerance = 5.0 / Viewport.GetScaleX();
						Time = AnnotationExt->Snap(*this, Time, SnapTolerance);
					}
				}
...
void STimingView::ShowContextMenu(const FPointerEvent& MouseEvent)
{
	...
	MenuBuilder.BeginSection(TEXT("Misc"));
	{
		...
		MenuBuilder.AddMenuEntry(Commands.GoToFrame,
			FName("GoToFrame"),
			TAttribute<FText>(),
			TAttribute<FText>(),
			FSlateIcon(FInsightsStyle::GetStyleSetName(), "Icons.FramesTrack"));

		MenuBuilder.AddMenuEntry(Commands.GoToTime,
			FName("GoToTime"),
			TAttribute<FText>(),
			TAttribute<FText>(),
		...
		MenuBuilder.AddMenuEntry(Commands.TrimAndSaveAs,
			FName("TrimAndSaveAs"),
			TAttribute<FText>(),
			TAttribute<FText>(),
			FSlateIcon(FInsightsStyle::GetStyleSetName(), "Icons.Save"));
...
void STimingView::BindCommands()
{
	...
	CommandList->MapAction(
		Commands.GoToFrame,
		FExecuteAction::CreateSP(this, &STimingView::GoToFrame_Execute),
		FCanExecuteAction::CreateSP(this, &STimingView::GoToFrame_CanExecute));

	CommandList->MapAction(
		Commands.GoToTime,
		FExecuteAction::CreateSP(this, &STimingView::GoToTime_Execute),
		FCanExecuteAction::CreateSP(this, &STimingView::GoToTime_CanExecute));

	CommandList->MapAction(
		Commands.TrimAndSaveAs,
		FExecuteAction::CreateSP(this, &STimingView::TrimAndSaveAs_Execute),
		FCanExecuteAction::CreateSP(this, &STimingView::TrimAndSaveAs_CanExecute));

	// User Annotation shortcuts
	CommandList->MapAction(
		Commands.AddPointAnnotation,
		FExecuteAction::CreateSP(this, &STimingView::AddPointAnnotation_Execute),
		FCanExecuteAction::CreateSP(this, &STimingView::AddPointAnnotation_CanExecute));

	CommandList->MapAction(
		Commands.AddRangeAnnotation,
		FExecuteAction::CreateSP(this, &STimingView::AddRangeAnnotation_Execute),
		FCanExecuteAction::CreateSP(this, &STimingView::AddRangeAnnotation_CanExecute));

	CommandList->MapAction(
		Commands.AddEventAnnotation,
		FExecuteAction::CreateSP(this, &STimingView::AddEventAnnotation_Execute),
		FCanExecuteAction::CreateSP(this, &STimingView::AddEventAnnotation_CanExecute));

	CommandList->MapAction(
		Commands.ToggleAllAnnotationVisibility,
		FExecuteAction::CreateSP(this, &STimingView::ToggleAllAnnotationVisibility_Execute),
		FCanExecuteAction(),
		FIsActionChecked::CreateSP(this, &STimingView::ToggleAllAnnotationVisibility_IsChecked));

	CommandList->MapAction(
		Commands.NextAnnotation,
		FExecuteAction::CreateSP(this, &STimingView::NextAnnotation_Execute),
		FCanExecuteAction::CreateSP(this, &STimingView::HasAnyAnnotations));

	CommandList->MapAction(
		Commands.PrevAnnotation,
		FExecuteAction::CreateSP(this, &STimingView::PrevAnnotation_Execute),
		FCanExecuteAction::CreateSP(this, &STimingView::HasAnyAnnotations));

	CommandList->MapAction(
		Commands.ToggleFloatingAnnotations,
		FExecuteAction::CreateSP(this, &STimingView::ToggleFloatingAnnotations_Execute),
		FCanExecuteAction(),
		FIsActionChecked::CreateSP(this, &STimingView::ToggleFloatingAnnotations_IsChecked));

	CommandList->MapAction(Commands.FindPreviousInstance,
		FExecuteAction::CreateSP(this, &STimingView::FindEventInstance_Execute, ESelectEventType::Previous),
		FCanExecuteAction::CreateSP(this, &STimingView::FindEventInstance_CanExecute));
	CommandList->MapAction(Commands.FindNextInstance,
		FExecuteAction::CreateSP(this, &STimingView::FindEventInstance_Execute, ESelectEventType::Next),
		FCanExecuteAction::CreateSP(this, &STimingView::FindEventInstance_CanExecute));
	CommandList->MapAction(Commands.FindMaxInstance,
		FExecuteAction::CreateSP(this, &STimingView::FindEventInstance_Execute, ESelectEventType::Max),
		FCanExecuteAction::CreateSP(this, &STimingView::FindEventInstance_CanExecute));
	CommandList->MapAction(Commands.FindPreviousInstanceInSelection,
		FExecuteAction::CreateSP(this, &STimingView::FindEventInstanceInSelection_Execute, ESelectEventType::Previous),
		FCanExecuteAction::CreateSP(this, &STimingView::FindEventInstanceInSelection_CanExecute));
	CommandList->MapAction(Commands.FindNextInstanceInSelection,
		FExecuteAction::CreateSP(this, &STimingView::FindEventInstanceInSelection_Execute, ESelectEventType::Next),
		FCanExecuteAction::CreateSP(this, &STimingView::FindEventInstanceInSelection_CanExecute));
	CommandList->MapAction(Commands.FindMaxInstanceInSelection,
		FExecuteAction::CreateSP(this, &STimingView::FindEventInstanceInSelection_Execute, ESelectEventType::Max),
		FCanExecuteAction::CreateSP(this, &STimingView::FindEventInstanceInSelection_CanExecute));
...
void STimingView::BringScrollableTrackIntoViewDelayed(TSharedPtr<FBaseTimingTrack> Track, int32 LaneDepth)
{
	DelayedVisibleTrack = Track;
	DelayedVisibleTrackLaneDepth = LaneDepth;
	DelayedVisibleTrackCounter = 3;
	DelayedVisibleTrackLastScrollHeight = 0.0f;
}
...
void STimingView::SetTimeMarkersVisible(bool bIsMarkersTrackVisible)
{
	if (MarkersTrack->IsVisible() != bIsMarkersTrackVisible)
	{
		...
		if (MarkersTrack->IsVisible())
		{
			...
			{
				UE_LOGF(LogTimingProfiler, Log, "SetTimeMarkersVisible!!!");
...
void STimingView::SetDrawOnlyBookmarks(bool bIsBookmarksTrack)
{
	if (MarkersTrack->IsBookmarksTrack() != bIsBookmarksTrack)
	{
		...
		if (MarkersTrack->IsVisible())
		{
			if (Viewport.GetScrollPosY() != 0.0f)
			{
				UE_LOGF(LogTimingProfiler, Log, "SetDrawOnlyBookmarks!!!");
...
void STimingView::UpdateHoveredTimingEvent(float InMousePosX, float InMousePosY)
{
	...
	if (HoveredTrack.IsValid())
	{
		...
		if (DT > 0.001)
		{
			UE_LOGF(LogTimingProfiler, Log, "HoveredTrack [%g, %g] GetEvent: %.1f ms", InMousePosX, InMousePosY, DT * 1000.0);
		}
	}
	...
	if (NewHoveredEvent.IsValid())
	{
		if (!HoveredEvent.IsValid() || !NewHoveredEvent->Equals(*HoveredEvent))
		{
			...
			if (T3 > 0.001)
			{
				UE_LOGF(LogTimingProfiler, Log, "HoveredTrack [%g, %g] Tooltip: %.1f ms (%.1f + %.1f + %.1f)",
...
{
	SelectTimingEvent(HoveredEvent, false, false);
}
...
TSharedRef<SWidget> STimingView::MakeCompactAutoScrollOptionsMenu()
{
	...
	MenuBuilder.BeginSection("AutoScrollOptions", LOCTEXT("CompactAutoScrollOptionsMenu_Section", "Auto-Scroll Options"));
	{
		CreateCompactMenuLine(MenuBuilder,
			...
			.ToolTip(LOCTEXT("AutoScrollAlignWithRenderingFrames_Tooltip", "Aligns the viewport's center position with the start time of a Rendering frame (when auto-scrolling)."))
		);
...
{
	MenuBuilder.BeginSection("TrackDepthLimits", LOCTEXT("ContextMenu_Section_TrackDepthLimits", "Track Depth Limits"));
	{
#if 0 // TODO
		MenuBuilder.AddMenuEntry(
			FUIAction(),
			...
			.VAlign(VAlign_Center)
			.AutoWidth()
			.Padding(FMargin(-32.0f, 0.0f, 0.0f, 0.0f))
			[
				SNew(STextBlock)
				.MinDesiredWidth(34.0f)
				.Text(LOCTEXT("TrackDepthLimitMin_Text", "Min"))
			]
			+ SHorizontalBox::Slot()
			.HAlign(HAlign_Left)
			.AutoWidth()
			...
			[
				...
				.Value_Lambda([this]
				{
					return FTimingProfilerManager::Get()->GetEventDepthLimitMin();
				})
				.OnValueChanged_Lambda([this](uint32 InValue)
				{
					SetEventDepthLimitMin(InValue);
				})
				+ SSegmentedControl<uint32>::Slot(0u)
				.Text(FText::FromString("0"))
				+ SSegmentedControl<uint32>::Slot(1u)
				.Text(FText::FromString("1"))
				+ SSegmentedControl<uint32>::Slot(2u)
				.Text(FText::FromString("2"))
				+ SSegmentedControl<uint32>::Slot(4u)
				.Text(FText::FromString("4"))
				+ SSegmentedControl<uint32>::Slot(8u)
				.Text(FText::FromString("8"))
				+ SSegmentedControl<uint32>::Slot(16u)
				.Text(FText::FromString("16"))
			]
			+ SHorizontalBox::Slot()
			.HAlign(HAlign_Left)
			.AutoWidth()
			...
			[
				...
				.MinValue(TOptional<uint32>(0))
				.MaxValue(TOptional<uint32>(100))
				.MinSliderValue(TOptional<uint32>(0))
				.MaxSliderValue(TOptional<uint32>(16))
				.Delta(1)
				.MinDesiredWidth(60.0f)
				.Value_Lambda([this]() -> uint32
				{
					return FTimingProfilerManager::Get()->GetEventDepthLimitMin();
				})
				.OnValueChanged_Lambda([this](uint32 InValue)
			...
			NAME_None,
			LOCTEXT("TrackDepthLimitMin_Tooltip", "The minimum depth (number of lanes) for a Timing Events track (like a CPU Thread track).\nValid values: [0 .. 100]\nThis option is persistent to the UnrealInsightsSettings.ini file."),
		...
		MenuBuilder.AddMenuEntry(
			...
			[
				SNew(STextBlock)
				.MinDesiredWidth(34.0f)
				.Text(LOCTEXT("TrackDepthLimitMax_Text", "Max"))
			]
			+ SHorizontalBox::Slot()
			.HAlign(HAlign_Left)
			.AutoWidth()
			...
			[
				...
				.Value_Lambda([this]
				{
					return FTimingProfilerManager::Get()->GetEventDepthLimitMax();
				})
				.OnValueChanged_Lambda([this](uint32 InValue)
				{
					SetEventDepthLimitMax(InValue);
				})
				+ SSegmentedControl<uint32>::Slot(1000u)
				[
					SNew(STextBlock)
					.Margin(FMargin(0.0f, -1.0f, 0.0f, 0.0f))
					.Text(LOCTEXT("DepthLimitMax_Unlimited", "\u221E"))
				]
				+ SSegmentedControl<uint32>::Slot(1u)
				.Text(FText::FromString("1"))
				+ SSegmentedControl<uint32>::Slot(2u)
			...
			[
				SNew(SSpinBox<uint32>)
				.MinValue(TOptional<uint32>(1))
				.MaxValue(TOptional<uint32>(1000))
				.MinSliderValue(TOptional<uint32>(1))
				.MaxSliderValue(TOptional<uint32>(16))
				.Delta(1)
				.MinDesiredWidth(60.0f)
				.Value_Lambda([this]() -> uint32
			...
			[
				...
				.Text(LOCTEXT("TrackDepthLimitMax_Keybinding", "X"))
			],
			NAME_None,
			LOCTEXT("TrackDepthLimitMax_Tooltip", "The maximum depth (number of lanes) for a Timing Events track (like a CPU Thread track).\nValid values: [1 .. 1000]\nThis option is persistent to the UnrealInsightsSettings.ini file."),
			EUserInterfaceActionType::None
		);
	}
	MenuBuilder.EndSection();
...
FText STimingView::GetEventDepthLimitKeybindingText(uint32 InDepthLimitMax) const
{
	uint32 CurrentDepthLimit = FTimingProfilerManager::Get()->GetEventDepthLimitMax();
	uint32 NextDepthLimit = GetNextEventDepthLimitMax(CurrentDepthLimit);
	return InDepthLimitMax == NextDepthLimit ? LOCTEXT("DepthLimitKeybinding", "X") : FText::GetEmpty();
}

////////////////////////////////////////////////////////////////////////////////////////////////////

bool STimingView::CheckEventDepthLimitMax(uint32 InDepthLimitMax) const
{
	return InDepthLimitMax == FTimingProfilerManager::Get()->GetEventDepthLimitMax();
}

////////////////////////////////////////////////////////////////////////////////////////////////////

uint32 STimingView::GetNextEventDepthLimitMax(uint32 DepthLimit) const
{
	// Cycles between: 1, 2, 4, 8, 16, Unlimited
	if (DepthLimit == FTimingProfilerManager::UnlimitedEventDepth)
	{
		return 1;
	}
	DepthLimit = FMath::RoundUpToPowerOfTwo(DepthLimit * 2);
	return (DepthLimit <= 16) ? DepthLimit : FTimingProfilerManager::UnlimitedEventDepth;
}

////////////////////////////////////////////////////////////////////////////////////////////////////

void STimingView::ChooseNextEventDepthLimitMax()
{
	const uint32 CurrentDepthLimit = FTimingProfilerManager::Get()->GetEventDepthLimitMax();
	const uint32 NextDepthLimit = GetNextEventDepthLimitMax(CurrentDepthLimit);
	FTimingProfilerManager::Get()->SetEventDepthLimitMax(NextDepthLimit);
	Viewport.AddDirtyFlags(ETimingTrackViewportDirtyFlags::HInvalidated);
}

////////////////////////////////////////////////////////////////////////////////////////////////////

void STimingView::SetEventDepthLimitMax(uint32 InDepthLimitMax)
{
	FTimingProfilerManager::Get()->SetEventDepthLimitMax(InDepthLimitMax);
	Viewport.AddDirtyFlags(ETimingTrackViewportDirtyFlags::HInvalidated);
}

////////////////////////////////////////////////////////////////////////////////////////////////////

void STimingView::SetEventDepthLimitMin(uint32 InDepthLimitMin)
{
	FTimingProfilerManager::Get()->SetEventDepthLimitMin(InDepthLimitMin);
	Viewport.AddDirtyFlags(ETimingTrackViewportDirtyFlags::HInvalidated);
}
...
void STimingView::CreateCpuThreadTrackColoringModeMenu(FMenuBuilder& MenuBuilder)
{
	MenuBuilder.BeginSection("CpuThreadTrackColoringMode", LOCTEXT("ContextMenu_Section_CpuThreadTrackColoringMode", "Coloring Mode (CPU Thread Tracks)"));
	{
		...
		MenuBuilder.AddMenuEntry(
			LOCTEXT("CpuThreadTrackColoringMode_ByTimerNameAndMetadata", "By Timer Name And Metadata"),
			LOCTEXT("CpuThreadTrackColoringMode_ByTimerNameAndMetadata_Desc", "Assign a color to CPU/GPU timing events based on their combined timer name and metadata values."),
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateSP(this, &STimingView::SetCpuThreadTrackColoringMode, ETimingEventsColoringMode::ByTimerNameAndMetadata),
				FCanExecuteAction(),
				FIsActionChecked::CreateSP(this, &STimingView::CheckCpuThreadTrackColoringMode, ETimingEventsColoringMode::ByTimerNameAndMetadata)),
			NAME_None,
			EUserInterfaceActionType::RadioButton
		);
		MenuBuilder.AddMenuEntry(
...
bool STimingView::GoToFrame_CanExecute() const
{
	return true;
}
...
void STimingView::GoToFrame_Execute()
{
	using namespace UE::Insights::TimingProfiler;

	// Spawn container window
	TSharedRef<SWindow> Window = SNew(SWindow)
		.Title(LOCTEXT("GoToFrameTitle", "Go to Frame"))
		.SizingRule(ESizingRule::Autosized)
		.SupportsMaximize(false)
		.SupportsMinimize(false);

	// Retrieve saved preferences
	const FInsightsSettings& Settings = FInsightsManager::GetSettings();

	// Spawn window contents
	TSharedRef<SGoToFrameDialog> Dialog = SNew(SGoToFrameDialog)
		.ParentWindow(Window)
		.InitialFrameType(Settings.GetGoToFrameType())
		.InitialFrameNumber(Settings.GetGoToFrameFrameNumber())
		.InitialSelectTimeRange(Settings.IsGoToFrameSelectTimeRangeEnabled())
		.InitialZoomTimingView(Settings.IsGoToFrameZoomTimingViewEnabled())
		.OnNavigate_Lambda([this](double StartTime, double EndTime, uint64 FrameNumber, bool bSelectTimeRange, bool bZoomTimingView, ETraceFrameType FrameType)
		{
			// Whenever navigate is called with valid parameters, save the values to disk
			FInsightsSettings& MutableSettings = FInsightsManager::GetSettings();
			MutableSettings.SetGoToFrameType(FrameType);
			MutableSettings.SetGoToFrameFrameNumber((int64)FrameNumber);
			MutableSettings.SetGoToFrameSelectTimeRange(bSelectTimeRange);
			MutableSettings.SetGoToFrameZoomTimingView(bZoomTimingView);
			MutableSettings.SaveToConfig();

			// When navigating to a Game Frame, ensure Game Frames track is visible. Same for Render Frames.
			if (TSharedPtr<FFrameSharedState> FrameState = GetFrameSharedState())
			{
				if (TSharedPtr<FFrameTimingTrack> Track = FrameState->GetFrameTrack((uint32)FrameType))
				{
					if (!Track->IsVisible())
					{
						Track->Show();
			...
			// Zoom or center based on navigate parameters
			const double Duration = EndTime - StartTime;
			if (bZoomTimingView)
			{
				ZoomOnTimeInterval(StartTime - Duration * 0.1, Duration * 1.2);
			}
			else
			{
				CenterOnTimeInterval(StartTime, Duration);
			}

			if (bSelectTimeRange)
			{
				SelectTimeInterval(StartTime, Duration);
			}
		});

	// Spawn the modal and focus the frame number input field. The modal will keep focus while opened.
	Window->SetContent(Dialog);
	Window->SetWidgetToFocusOnActivate(Dialog->GetFrameNumberBox());
	FSlateApplication::Get().AddModalWindow(Window, SharedThis(this));
}

////////////////////////////////////////////////////////////////////////////////////////////////////

bool STimingView::GoToTime_CanExecute() const
{
	return true;
}
...
void STimingView::GoToTime_Execute()
{
	using namespace UE::Insights::TimingProfiler;

	// Spawn container window
	TSharedRef<SWindow> Window = SNew(SWindow)
		.Title(LOCTEXT("GoToTimeTitle", "Go to Time"))
		.SizingRule(ESizingRule::Autosized)
		.SupportsMaximize(false)
		.SupportsMinimize(false);
	...
	// Spawn window contents
	TSharedRef<SGoToTimeDialog> Dialog = SNew(SGoToTimeDialog)
		.ParentWindow(Window)
		.InitialTime(Settings.GetGoToTimeTime())
		.InitialSelectTimeRange(Settings.IsGoToTimeSelectTimeRangeEnabled())
		.InitialZoomTimingView(Settings.IsGoToTimeZoomTimingViewEnabled())
		.InitialTimeRangeDuration(Settings.GetGoToTimeTimeRangeDuration())
		.OnNavigate_Lambda([this](double TargetTime, bool bSelectTimeRange, bool bZoomTimingView, double TimeRangeDuration)
		{
			// Whenever navigate is called with valid parameters, save the values to disk
			FInsightsSettings& MutableSettings = FInsightsManager::GetSettings();
			MutableSettings.SetGoToTimeTime(TargetTime);
			MutableSettings.SetGoToTimeSelectTimeRange(bSelectTimeRange);
			MutableSettings.SetGoToTimeZoomTimingView(bZoomTimingView);
			MutableSettings.SetGoToTimeTimeRangeDuration(TimeRangeDuration);
			MutableSettings.SaveToConfig();

			// TargetTime is the start of the interval
			if (bSelectTimeRange)
			{
				SelectTimeInterval(TargetTime, TimeRangeDuration);
			}

			if (bZoomTimingView)
			{
				ZoomOnTimeInterval(TargetTime - TimeRangeDuration * 0.1, TimeRangeDuration * 1.2);
			}
			else
			{
				CenterOnTimeInterval(TargetTime, 0.0);
			}
		});

	// Spawn the modal and focus the timestamp input field. The modal will keep focus while opened.
	Window->SetContent(Dialog);
	Window->SetWidgetToFocusOnActivate(Dialog->GetTimeBox());
	FSlateApplication::Get().AddModalWindow(Window, SharedThis(this));
}
...
bool STimingView::TrimAndSaveAs_CanExecute() const
{
	if (SelectionStartTime >= SelectionEndTime)
	{
		return false;
	...
	ETraceStreamType TraceStreamType = InsightsManager->GetTraceStreamType();
...
void STimingView::TrimAndSaveAs_Execute()
{
	if (SelectionStartTime >= SelectionEndTime)
	{
		return;
	...
	TrimParams.StartTime = SelectionStartTime;
	TrimParams.EndTime = SelectionEndTime;
	...
	if (!Session)
	{
		UE_LOGF(LogTimingProfiler, Error, "[TraceTrimmer] Invalid session!");
		return;
	}
	...
	TUniquePtr<UE::Trace::IInDataStream> InputDataStream;
	FString InputTraceName;

	ETraceStreamType TraceStreamType = InsightsManager->GetTraceStreamType();
	if (TraceStreamType == ETraceStreamType::TraceFile)
	{
		InputTraceName = Session->GetName();
		FPaths::NormalizeFilename(InputTraceName);

		InputDataStream = MakeUnique<UE::Trace::FFileDataStream>();
		UE::Trace::FFileDataStream* FileDataStream = (UE::Trace::FFileDataStream*)InputDataStream.Get();
		if (!FileDataStream->Open(*InputTraceName))
		{
			UE_LOGF(LogTimingProfiler, Error, "[TraceTrimmer] Cannot open the input trace file ('%ls')!", *InputTraceName);
			return;
		}
	}
	else if (TraceStreamType == ETraceStreamType::TraceStore)
	{
		InputTraceName = Session->GetName();
		FPaths::NormalizeFilename(InputTraceName);

		const uint32 TraceId = Session->GetTraceId();

		UE::Trace::FStoreClient* StoreClient = InsightsManager->GetStoreClient();
		if (StoreClient)
		{
			InputDataStream = StoreClient->ReadTrace(TraceId);
		}

		if (!InputDataStream)
		{
			UE_LOGF(LogTimingProfiler, Error, "[TraceTrimmer] Cannot open the input trace stream ('%ls')!", *InputTraceName);
			return;
		}
	}
	else
	{
		UE_LOGF(LogTimingProfiler, Error, "[TraceTrimmer] Unsupported input trace stream ('%ls')!", *InsightsManager->GetTraceName());
		return;
	}

	//////////////////////////////////////////////////
	// Choose the output trace file
	...
	if (IDesktopPlatform* DesktopPlatform = FDesktopPlatformModule::Get())
	{
		...
		const FString DefaultFile = FPaths::GetBaseFilename(InputTraceName) + TEXT("_trim.utrace");

		bDialogResult = DesktopPlatform->SaveFileDialog(
			FSlateApplication::Get().FindBestParentWindowHandleForDialogs(nullptr),
			LOCTEXT("SaveTrimmedTraceAs", "Save Trimmed Trace As").ToString(),
			DefaultPath,
			DefaultFile,
			TEXT("Trace Files (*.utrace)|*.utrace|All Files (*.*)|*.*"),
	...
	const FString& OutputTraceFile = SaveFilenames[0];

	//////////////////////////////////////////////////
	// OutputDataStream

	TUniquePtr<UE::Trace::FFileOutDataStream> OutputDataStream = MakeUnique<UE::Trace::FFileOutDataStream>();
	if (!OutputDataStream->Open(*OutputTraceFile))
	{
		UE_LOGF(LogTimingProfiler, Error, "[TraceTrimmer] Cannot write the output trace file ('%ls')!", *OutputTraceFile);
		return;
	}
	...
	int32 Result = TraceServices::Trim(TrimParams, *InputDataStream, *OutputDataStream);
	if (Result == 0)
	{
		UE_LOGF(LogTimingProfiler, Log, "[TraceTrimmer] Success!");
	}
	else
	{
		UE_LOGF(LogTimingProfiler, Error, "[TraceTrimmer] Failed!");
	}
}
...
{
	INSIGHTS_SCOPED_LOG(FindFirstEvent);
...
{
	INSIGHTS_SCOPED_LOG(FindPrevEvent);
...
{
	INSIGHTS_SCOPED_LOG(FindNextEvent);
...
{
	INSIGHTS_SCOPED_LOG(FindLastEvent);
...
{
	INSIGHTS_SCOPED_LOG(FindMaxEvent);
	...
	const double MaxValidTime = Viewport.GetMaxValidTime();
	TSharedPtr<const ITimingEvent> BestMatchEvent;
	double CurrentDuration = -1.0;

	TSharedPtr<FFilterConfigurator> Filter = QuickFindVm->GetFilterConfigurator();

	auto EventFilter = [&CurrentDuration, MaxValidTime](double EventStartTime, double EventEndTime, uint32 EventDepth)
	{
		const double Duration = EventEndTime == std::numeric_limits<double>::infinity() ?
			MaxValidTime - EventStartTime : EventEndTime - EventStartTime;
		return Duration > CurrentDuration;
	};

	auto EventMatched = [&CurrentDuration, MaxValidTime](double EventStartTime, double EventEndTime, uint32 EventDepth)
	{
		const double Duration = EventEndTime == std::numeric_limits<double>::infinity() ?
			MaxValidTime - EventStartTime : EventEndTime - EventStartTime;
...
{
	INSIGHTS_SCOPED_LOG(FindMinEvent);
...
void STimingView::PostFindEvent(TSharedPtr<const ITimingEvent> BestMatchEvent)
{
	if (BestMatchEvent)
	{
		...
		bool bZoomOnFoundEvent = true;
		if (bZoomOnFoundEvent)
		{
			const double Duration = Viewport.RestrictDuration(SelectedEvent->GetStartTime(), SelectedEvent->GetEndTime());
			if (Duration > 0.0)
			{
				ZoomOnTimeInterval(SelectedEvent->GetStartTime() - Duration * 0.1, Duration * 1.2);
			}
		}
		BringIntoView(SelectedEvent->GetStartTime(), SelectedEvent->GetEndTime());
		TSharedPtr<FBaseTimingTrack> Track = ConstCastSharedPtr<FBaseTimingTrack>(SelectedEvent->GetTrack().ToSharedPtr());
		BringScrollableTrackIntoViewDelayed(Track, int32(SelectedEvent->GetDepth()));
		SelectTimingTrack(Track, true);
		OnSelectedTimingEventChanged();
	}
	else
	{
...
class FBaseTimingTrackFilterContext : public FBaseFilterContext
{
public:
	static constexpr int32 TrackNameKey = static_cast<int32>(UE::Insights::EFilterField::TrackName);

public:
	FBaseTimingTrackFilterContext() = default;
	virtual ~FBaseTimingTrackFilterContext() = default;

	virtual bool GetDefaultReturnValue() const override
	{
		return true;
	}
	...
	{
		return Key == TrackNameKey;
	}

	virtual EFilterDataType GetDataType(int32 Key) const override
	{
		return Key == TrackNameKey ? EFilterDataType::String : EFilterDataType::Void;
	}

	virtual FStringView GetDataAsStringView(int32 Key) const override
	{
		return Track && Key == TrackNameKey ? Track->GetName() : FStringView();
	}
	virtual const TCHAR* GetDataAsCString(int32 Key) const override
	{
		return Track && Key == TrackNameKey ? *Track->GetName() : nullptr;
	}
	virtual const FString& GetDataAsString(int32 Key) const override
	{
		return Track && Key == TrackNameKey ? Track->GetName() : EmptyString;
	}

	void SetTrack(const FBaseTimingTrack* InTrack)
	{
		Track = InTrack;
	}

private:
	const FBaseTimingTrack* Track = nullptr;
};
...
{
	FBaseTimingTrackFilterContext FilterContext;
	...
	if (PriorityTrack.IsValid())
	{
		...
		{
			FilterContext.SetTrack(PriorityTrack.Get());
	...
	for (auto& Entry : AllTracks)
	{
		...
		{
			FilterContext.SetTrack(Entry.Value.Get());
			if (InFilterConfigurator->ApplyFilters(FilterContext))
			{
				Callback(Entry.Value);
			}
...
ETraceFrameType STimingView::GetFrameTypeToSnapTo()
{
	...
	{
		ETimerAggregationMode AggregationMode = TimersView->GetAggregationMode();
	...
	return ETraceFrameType::TraceFrameType_Count; // no frame snap
}

////////////////////////////////////////////////////////////////////////////////////////////////////

bool STimingView::FindEventInstance_CanExecute() const
{
	return SelectedEvent.IsValid() && SelectedEvent->Is<FThreadTrackEvent>();
}

////////////////////////////////////////////////////////////////////////////////////////////////////

void STimingView::FindEventInstance_Execute(ESelectEventType EventType)
{
	if (!FindEventInstance_CanExecute())
	{
		return;
	}
	const FThreadTrackEvent ThreadTrackEvent = SelectedEvent->As<FThreadTrackEvent>();
	SelectEventInstance(ThreadTrackEvent.GetTimerId(), EventType, false);
}

////////////////////////////////////////////////////////////////////////////////////////////////////

bool STimingView::FindEventInstanceInSelection_CanExecute() const
{
	return FindEventInstance_CanExecute() && SelectionEndTime > SelectionStartTime;
}

////////////////////////////////////////////////////////////////////////////////////////////////////

void STimingView::FindEventInstanceInSelection_Execute(ESelectEventType EventType)
{
	if (!FindEventInstanceInSelection_CanExecute())
	{
		return;
	}

	const FThreadTrackEvent ThreadTrackEvent = SelectedEvent->As<FThreadTrackEvent>();
	SelectEventInstance(ThreadTrackEvent.GetTimerId(), EventType, true);
}
...
void STimingView::SelectEventInstance(uint32 TimerId, ESelectEventType Type, bool bUseSelection)
{
	...
	case ESelectEventType::Min:
		...
		TimingEvent = ThreadTimingSharedState->FindMinEventInstance(TimerId, IntervalStart, IntervalEnd, MaxValidTime);
		break;
	case ESelectEventType::Max:
		SelectTimingEvent(nullptr, false, false);
		TimingEvent = ThreadTimingSharedState->FindMaxEventInstance(TimerId, IntervalStart, IntervalEnd, MaxValidTime);
		break;
	}
...
////////////////////////////////////////////////////////////////////////////////////////////////////
// User Annotation shortcut handlers
////////////////////////////////////////////////////////////////////////////////////////////////////

FUserAnnotationsTimingViewExtender* STimingView::FindAnnotationExtender() const
{
	// Cache the result — module is process-lifetime and the extender lives as a member of the
	// module, so the pointer is stable. Hot path (CanExecute predicates fire many times per frame).
	if (CachedAnnotationExtender)
	{
		return CachedAnnotationExtender;
	}
	...
	if (Module)
	{
		CachedAnnotationExtender = &Module->GetUserAnnotationsExtender();
	}
	return CachedAnnotationExtender;
}
...
void STimingView::AddPointAnnotation_Execute()
{
	FUserAnnotationsTimingViewExtender* AnnotationExt = FindAnnotationExtender();
	if (!AnnotationExt)
	{
	...
	TSharedPtr<FUserAnnotationStore> Store = AnnotationExt->GetAnnotationStoreForCurrentSession();
	if (!Store.IsValid())
	{
		return;
	}
	...
	FUserAnnotationsTimingViewExtender::FAnnotationContext Ctx =
		FUserAnnotationsTimingViewExtender::BuildPointContext(*this, Time);
	AnnotationExt->ShowAnnotationDialog(Store, Ctx);
}

////////////////////////////////////////////////////////////////////////////////////////////////////

bool STimingView::AddPointAnnotation_CanExecute() const
{
	if (SelectionEndTime > SelectionStartTime) // Only when no range selection active
	{
		return false;
	}
	FUserAnnotationsTimingViewExtender* AnnotationExt = FindAnnotationExtender();
	return AnnotationExt
		&& AnnotationExt->IsAnnotationsTrackVisible(*this)
		&& AnnotationExt->CanPersistAnnotations(*this);
}

////////////////////////////////////////////////////////////////////////////////////////////////////

void STimingView::AddRangeAnnotation_Execute()
{
	FUserAnnotationsTimingViewExtender* AnnotationExt = FindAnnotationExtender();
	if (!AnnotationExt)
	{
	...
	FUserAnnotationsTimingViewExtender::FAnnotationContext Ctx =
		FUserAnnotationsTimingViewExtender::BuildRangeContext(*this, SelectionStartTime, SelectionEndTime);
	AnnotationExt->ShowAnnotationDialog(Store, Ctx);
}

bool STimingView::AddRangeAnnotation_CanExecute() const
{
	if (SelectionEndTime <= SelectionStartTime)
	{
		return false;
...
void STimingView::AddEventAnnotation_Execute()
{
	FUserAnnotationsTimingViewExtender* AnnotationExt = FindAnnotationExtender();
	if (!AnnotationExt)
	{
	...
	if (!HoveredEvent.IsValid() || !HoveredEvent->Is<FThreadTrackEvent>())
	{
		return;
	}

	FUserAnnotationsTimingViewExtender::FAnnotationContext Ctx =
		FUserAnnotationsTimingViewExtender::BuildEventContext(*this, *HoveredEvent);
	AnnotationExt->ShowAnnotationDialog(Store, Ctx);
}

bool STimingView::AddEventAnnotation_CanExecute() const
{
	if (!HoveredEvent.IsValid() || !HoveredEvent->Is<FThreadTrackEvent>())
	{
		return false;
...
void STimingView::ToggleAllAnnotationVisibility_Execute()
{
	FUserAnnotationsTimingViewExtender* AnnotationExt = FindAnnotationExtender();
	if (AnnotationExt)
	{
		AnnotationExt->ToggleAllAnnotationVisibility();
	}
}

bool STimingView::ToggleAllAnnotationVisibility_IsChecked() const
{
	FUserAnnotationsTimingViewExtender* AnnotationExt = FindAnnotationExtender();
	if (!AnnotationExt)
	{
	...
	// Returns true if any annotations are currently visible
...
void STimingView::NextAnnotation_Execute()
{
	FUserAnnotationsTimingViewExtender* AnnotationExt = FindAnnotationExtender();
	if (AnnotationExt)
	{
		AnnotationExt->NavigateToNextAnnotation(*this, /*bForward=*/true);
	}
}

void STimingView::PrevAnnotation_Execute()
{
	FUserAnnotationsTimingViewExtender* AnnotationExt = FindAnnotationExtender();
	if (AnnotationExt)
	{
		AnnotationExt->NavigateToNextAnnotation(*this, /*bForward=*/false);
	}
}

bool STimingView::HasAnyAnnotations() const
{
	FUserAnnotationsTimingViewExtender* AnnotationExt = FindAnnotationExtender();
	if (!AnnotationExt)
	{
	...
	TSharedPtr<FUserAnnotationStore> Store = AnnotationExt->GetAnnotationStoreForCurrentSession();
	return Store.IsValid() && Store->GetAllAnnotations().Num() > 0;
}

////////////////////////////////////////////////////////////////////////////////////////////////////

void STimingView::ToggleFloatingAnnotations_Execute()
{
	FUserAnnotationsTimingViewExtender* AnnotationExt = FindAnnotationExtender();
	if (AnnotationExt)
	{
		AnnotationExt->ToggleFloatingAnnotations();
	}
}

bool STimingView::ToggleFloatingAnnotations_IsChecked() const
{
	FUserAnnotationsTimingViewExtender* AnnotationExt = FindAnnotationExtender();
	return AnnotationExt ? AnnotationExt->GetShowFloatingAnnotations() : true;
}
