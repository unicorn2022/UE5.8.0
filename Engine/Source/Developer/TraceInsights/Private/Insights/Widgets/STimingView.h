...
public:
	...
	bool GoToFrame_CanExecute() const;
	void GoToFrame_Execute();

	bool GoToTime_CanExecute() const;
	void GoToTime_Execute();

	bool TrimAndSaveAs_CanExecute() const;
	void TrimAndSaveAs_Execute();
	...
	virtual const TArray<TUniquePtr<ITimingEventRelation>>& GetCurrentRelations() const override { return CurrentRelations; }
	virtual TArray<TUniquePtr<ITimingEventRelation>>& EditCurrentRelations() override { return CurrentRelations; }
	virtual void AddRelation(TUniquePtr<ITimingEventRelation>& Relation) override { CurrentRelations.Add(MoveTemp(Relation)); }
	virtual void ClearRelations() override;
	...
	void BringScrollableTrackIntoViewDelayed(TSharedPtr<FBaseTimingTrack> Track, int32 LaneDepth = -1);
...
private:
	// TODO: STimingView here reaches into a specific extender by type, and does so through
	// FTraceInsightsModule — both of which run against the ITimingViewExtender contract. When
	// STimingView is eventually split into a trace-agnostic base class (TraceInsightsCore) and a
	// TimingProfiler-specific derived class, annotations belong in the base as a generic feature.
	// The intended direction is to register the annotations extender as an IModularFeatures
	// implementation alongside other plugin extenders (see GetExtenders) and drive it through
	// generic ITimingViewExtender hooks or a dedicated interface, removing both the type-specific
	// access and the module dependency. The current direct access is scoped to these private
	// handlers and the TraceInsights module, so it can be migrated without touching external
	// callers.
	UE::Insights::TimingProfiler::FUserAnnotationsTimingViewExtender* FindAnnotationExtender() const;

	/** Cached extender pointer — avoids per-call FModuleManager::GetModulePtr (slow) from CanExecute predicates.
	 *  Safe because FTraceInsightsModule is process-lifetime: loaded once at IMPLEMENT_MODULE and never
	 *  unloaded in standalone UnrealInsights or the editor. The extender lives as a value member of the
	 *  module, so the address is stable for the widget's lifetime. If the module ever becomes unload/
	 *  reload-capable at runtime, this needs FModuleManager::OnModulesChanged invalidation. */
	mutable UE::Insights::TimingProfiler::FUserAnnotationsTimingViewExtender* CachedAnnotationExtender = nullptr;

public:
	void AddPointAnnotation_Execute();
	bool AddPointAnnotation_CanExecute() const;

	void AddRangeAnnotation_Execute();
	bool AddRangeAnnotation_CanExecute() const;

	void AddEventAnnotation_Execute();
	bool AddEventAnnotation_CanExecute() const;

	void ToggleAllAnnotationVisibility_Execute();
	bool ToggleAllAnnotationVisibility_IsChecked() const;

	void NextAnnotation_Execute();
	void PrevAnnotation_Execute();
	bool HasAnyAnnotations() const;

	void ToggleFloatingAnnotations_Execute();
	bool ToggleFloatingAnnotations_IsChecked() const;
	...
	bool FindEventInstance_CanExecute() const;
	void FindEventInstance_Execute(ESelectEventType EventType);
	bool FindEventInstanceInSelection_CanExecute() const;
	void FindEventInstanceInSelection_Execute(ESelectEventType EventType);
...
protected:
	...
	FText GetEventDepthLimitKeybindingText(uint32 InDepthLimitMax) const;
	uint32 GetNextEventDepthLimitMax(uint32 InDepthLimitMax) const;
	bool CheckEventDepthLimitMax(uint32 InDepthLimitMax) const;
	void ChooseNextEventDepthLimitMax();
	void SetEventDepthLimitMax(uint32 InDepthLimitMax);
	void SetEventDepthLimitMin(uint32 InDepthLimitMin);
...
protected:
	...
	TSharedPtr<FBaseTimingTrack> DelayedVisibleTrack;
	int32 DelayedVisibleTrackLaneDepth = -1;
	float DelayedVisibleTrackLastScrollHeight = 0.0f;
	int32 DelayedVisibleTrackCounter = 0;
	...
	typedef TFixedCircularBuffer<uint64, 32> FHistoryBuffer;
	FHistoryBuffer PreUpdateTracksDurationHistory;
	FHistoryBuffer UpdateTracksDurationHistory;
	FHistoryBuffer PostUpdateTracksDurationHistory;
	FHistoryBuffer TickDurationHistory;
	mutable FHistoryBuffer PreDrawTracksDurationHistory;
	mutable FHistoryBuffer DrawTracksDurationHistory;
	mutable FHistoryBuffer PostDrawTracksDurationHistory;
	mutable FHistoryBuffer TotalDrawDurationHistory;
	mutable FHistoryBuffer OnPaintDeltaTimeHistory;
