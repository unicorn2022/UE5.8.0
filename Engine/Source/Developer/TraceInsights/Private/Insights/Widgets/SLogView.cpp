...
void SLogView::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	...
	if (LogProviderNumCategories != TotalNumCategories)
	{
		...
		UE_LOGF(TraceInsights, Log, "[LogView] Total Log Categories: %d", TotalNumCategories);
		...
		{
			UE_LOGF(TraceInsights, Warning, "[LogView] Duplicated Log Categories: %d (+%dx)", DuplicatedCategories.Num(), TotalNumCategories - NumAvailableLogCategories);
			for (const auto& KV : DuplicatedCategories)
			{
				UE_LOGF(TraceInsights, Log, "[LogView]    \"%ls\" (+%dx)", *KV.Key.GetPlainNameString(), KV.Value);
			}
		}
		UE_LOGF(TraceInsights, Log, "[LogView] Unique Log Categories: %d", NumAvailableLogCategories);
	...
	if (LogProviderNumMessages != TotalNumMessages)
	{
		...
		if (LogProviderNumMessages > TotalNumMessages)
		{
...
#if !WITH_EDITOR
					UE_LOGF(TraceInsights, Log, "[LogView] Start async task for filtering by%ls%ls%ls (\"%ls\") (%d to %d)",
			...
			else // no filtering
			{
				...
				{
					UE_LOGF(TraceInsights, Log, "[LogView] Updated (no filter; %d added / %d total messages) in %llu ms.",
...
#if !WITH_EDITOR
			UE_LOGF(TraceInsights, Log, "[LogView] RESET");
#endif // !WITH_EDITOR
			Cache.Reset();
			FilteredMessages.Reset();
			TotalNumMessages = 0;
		...
		FilteringAsyncTask->IsDone())
	{
			...
			FilteringChangeNumber == Filter.GetChangeNumber())
		{
			...
			if (DurationMs > 10) // avoids spams
			{
				...
				UE_LOGF(TraceInsights, Log, "[LogView] Updated (%d added / %d async filtered / %d total messages) in %llu ms (%.2f messages/second).",
...
#if !WITH_EDITOR
	UE_LOGF(TraceInsights, Log, "[LogView] OnFilterChanged: \"%ls\"", *FilterText);
#endif // !WITH_EDITOR
	Cache.Reset();
	FilteredMessages.Reset();
	TotalNumMessages = 0;
...
void SLogView::CopyRange() const
{
	...
	if (Session.IsValid())
	{
		...
		{
			LogProvider.ReadMessage(Message->GetIndex(),
				[this, SelectionStartTime, SelectionEndTime, &StringBuilder, &NumMessagesInSelectedRange]
				(const TraceServices::FLogMessageInfo& MessageInfo)
				{
	...
	{
		FPlatformApplicationMisc::ClipboardCopy(StringBuilder.ToString());
		UE_LOGF(TraceInsights, Log, "Copied %d logs to clipboard.", NumMessagesInSelectedRange);
	}
}
...
void SLogView::CopyAll() const
{
	...
	FPlatformApplicationMisc::ClipboardCopy(StringBuilder.ToString());
	UE_LOGF(TraceInsights, Log, "Copied %d logs to clipboard.", FilteredMessages.Num());
}
...
void SLogView::SaveLogsToFile(bool bSaveLogsInSelectedRangeOnly) const
{
	...
	TUniquePtr<IFileHandle> ExportFileHandle{ FPlatformFileManager::Get().GetPlatformFile().OpenWrite(*Filename) };
	if (!ExportFileHandle)
	{
	...
	const TCHAR Separator = bIsCSV ? TEXT(',') : TEXT('\t');
	FUtf8FileWriter Writer(ExportFileHandle.Get(), bIsCSV);

	// Write header.
	{
		TStringBuilder<1024> Line;
		AppendFormatMessageDelimitedHeader(Line, Separator);
		Writer.GetStringBuilder().Append(Line.ToView());
	...
	if (Session.IsValid())
	{
		...
		{
			LogProvider.ReadMessage(Message->GetIndex(),
				[this, bSaveLogsInSelectedRangeOnly, SelectionStartTime, SelectionEndTime, Separator, &Writer, &NumMessagesInSelectedRange]
				(const TraceServices::FLogMessageInfo& MessageInfo)
				{
					if (!bSaveLogsInSelectedRangeOnly || (MessageInfo.Time >= SelectionStartTime && MessageInfo.Time <= SelectionEndTime))
					{
						FLogMessageRecord Log(MessageInfo);
						TStringBuilder<1024> Line;
						AppendFormatMessageDelimited(Log, Line, Separator);
						Writer.GetStringBuilder().Append(Line.ToView());
						Writer.AppendLineEnd();
						++NumMessagesInSelectedRange;
					}
	...
	UE_LOGF(TraceInsights, Log, "Saved %d logs to file in %.3fs.", NumMessagesInSelectedRange, TotalTime);
}
