									 ...
									 bool bMultiLine) const
{
	InVerticalBox->AddSlot()
		...
		[
			...
			[
				SNew(SBox)
				.Padding(FMargin(0.0f, 4.0f, 16.0f, 0.0f))
				.HAlign(HAlign_Right)
...
TSharedRef<SDockTab> SSessionInfoWindow::SpawnTab_SessionInfo(const FSpawnTabArgs& Args)
{
	...
	AddInfoLine(VerticalBox, LOCTEXT("EngineVersion_HeaderText",	"Engine Version"),		&SSessionInfoWindow::GetEngineVersionText,	&SSessionInfoWindow::IsVisibleEngineVersionText);
	...
	AddInfoLine(VerticalBox, LOCTEXT("VFSPaths_HeaderText",			"VFS Paths"),			&SSessionInfoWindow::GetVFSPathsText,		&SSessionInfoWindow::IsVisibleVFSPathsText, true);
...
void SSessionInfoWindow::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	...
	if (Session != AnalysisSession)
	{
		...
		StreamTypeText = FText::GetEmpty();
		SessionNameText = FText::GetEmpty();
		UriText = FText::GetEmpty();
		...
		EngineVersionText = FText::GetEmpty();
		CommandLineText = FText::GetEmpty();
		VFSPathsText = FText::GetEmpty();
	...
	{
		if (StreamTypeText.IsEmpty())
		{
			UpdateStreamTypeAndSessionName(*Session.Get());
		}

		bIsSessionInfoSet = TryUpdateDiagnosticsInfo(*Session.Get());
	}
}

////////////////////////////////////////////////////////////////////////////////////////////////////

void SSessionInfoWindow::UpdateStreamTypeAndSessionName(const TraceServices::IAnalysisSession& Session)
{
	...
	case ETraceStreamType::DirectTraceSecure:
...
bool SSessionInfoWindow::TryUpdateDiagnosticsInfo(const TraceServices::IAnalysisSession& Session)
{
	TraceServices::FAnalysisSessionReadScope SessionReadScope(Session);

	const TraceServices::IDiagnosticsProvider* DiagnosticsProvider = TraceServices::ReadDiagnosticsProvider(Session);
	...
	EngineVersionText = FText::FromString(SessionInfo.EngineVersion);
	if (!SessionInfo.EngineVersion.IsEmpty() &&
		!SessionInfo.BuildVersion.IsEmpty())
	{
		// Trim the BuildVersion from the EngineVersion, if it is included and it is the same.
		int32 Index = SessionInfo.EngineVersion.Find(SessionInfo.BuildVersion, ESearchCase::CaseSensitive);
		if (Index > 0 && SessionInfo.EngineVersion[Index - 1] == TEXT('-'))
		{
			EngineVersionText = FText::FromString(SessionInfo.EngineVersion.Left(Index - 1));
		}
	}

	CommandLineText = FText::FromString(SessionInfo.CommandLine);

	TArray<FString> VFSPaths;
	SessionInfo.VFSPaths.ParseIntoArray(VFSPaths, TEXT(";"), true);
	int32 NumVFSPaths = 0;
	TStringBuilder<1024> VFSPathsBuilder;
	for (FString& Path : VFSPaths)
	{
	...
	Session.EnumerateMetadata(
		[&OtherMetadata]
		(const TraceServices::FTraceSessionMetadata& Metadata)
		{
...
FText SSessionInfoWindow::GetModulesText() const
{
	...
	Modules.Sort([](const TraceServices::FModuleInfoEx& LHS, const TraceServices::FModuleInfoEx& RHS)
		{
			return FCString::Strcmp(LHS.Info.DisplayName, RHS.Info.DisplayName) < 0;
		});
