...
////////////////////////////////////////////////////////////////////////////////////////////////////
// FCpuCoreTimingTrackFilterContext
////////////////////////////////////////////////////////////////////////////////////////////////////

class FCpuCoreTimingTrackFilterContext : public FBaseFilterContext
{
public:
	FCpuCoreTimingTrackFilterContext(const FCpuCoreTimingTrack& InTrack)
		: Track(InTrack)
	{
	}
	virtual ~FCpuCoreTimingTrackFilterContext() = default;
	...
	virtual bool HasData(int32 Key) const override
	{
		...
		case static_cast<int32>(EFilterField::CoreEventName):
			return true;
		default:
			return false;
		}
	...
	virtual EFilterDataType GetDataType(int32 Key) const override
	{
		...
		case static_cast<int32>(EFilterField::CoreEventName): // exposed as SystemThreadId
	...
	{
		if (Key == static_cast<int32>(EFilterField::CoreEventName))
		{
			return int64(SystemThreadId);
		}
		return 0;
	}
	...
	void SetEvent(const TraceServices::FCpuCoreEvent& Event)
	{
		EventStartTime = Event.Start;
		EventEndTime = Event.End;
		SystemThreadId = Event.SystemThreadId;
	}

private:
	const FCpuCoreTimingTrack& Track;
...
void FCpuCoreTimingTrack::BuildFilteredDrawState(ITimingEventsTrackDrawStateBuilder& Builder, const ITimingTrackUpdateContext& Context)
{
	...
	if (FilterConfigurator.IsValid() && !FilterConfigurator->IsEmpty()) // HasCustomFilter()
	{
		if (const TraceServices::IContextSwitchesProvider* ContextSwitchesProvider = TraceServices::ReadContextSwitchesProvider(*Session.Get()))
		{
			FCpuCoreTimingTrackFilterContext FilterContext(*this);
			const FTimingTrackViewport& Viewport = Context.GetViewport();
			bool bShowNonTargetProcessEvents = SharedState.AreNonTargetProcessEventsVisible();
			...
			ContextSwitchesProvider->EnumerateCpuCoreEvents(CoreNumber, Viewport.GetStartTime(), Viewport.GetEndTime(),
				...
				(const TraceServices::FCpuCoreEvent& CpuCoreEvent)
				{
					FilterContext.SetEvent(CpuCoreEvent);
...
const TSharedPtr<const ITimingEvent> FCpuCoreTimingTrack::SearchEvent(const FTimingEventSearchParameters& InSearchParameters) const
{
	...
	FindCpuCoreTimingEvent(InSearchParameters,
		[this, &FoundEvent]
...
bool FCpuCoreTimingTrack::FindCpuCoreTimingEvent(
	const FTimingEventSearchParameters& InParameters,
	TFunctionRef<void(double, double, uint32, const TraceServices::FCpuCoreEvent&)> InFoundPredicate) const
{
	TSharedPtr<const TraceServices::IAnalysisSession> Session = FInsightsManager::Get()->GetSession();
	if (!Session.IsValid())
	{
	...
	return TTimingEventSearch<TraceServices::FCpuCoreEvent>::Search(
		...
		(TTimingEventSearch<TraceServices::FCpuCoreEvent>::FContext& InContext)
		{
			...
			if (Session.IsValid())
			{
				if (const TraceServices::IContextSwitchesProvider* ContextSwitchesProvider = TraceServices::ReadContextSwitchesProvider(*Session.Get()))
				{
					...
					{
						ContextSwitchesProvider->EnumerateCpuCoreEvents(CoreNumber, InParameters.StartTime, InParameters.EndTime, Callback);
					}
					else
					{
						ContextSwitchesProvider->EnumerateCpuCoreEventsBackwards(CoreNumber, InParameters.EndTime, InParameters.StartTime, Callback);
					}
				}
			}
		},
