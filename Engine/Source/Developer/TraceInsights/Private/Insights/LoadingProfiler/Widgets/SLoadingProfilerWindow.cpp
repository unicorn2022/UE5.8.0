...
#include "Insights/TimingProfiler/Widgets/SUserAnnotationsPanel.h"
...
const FName FLoadingProfilerTabs::UserAnnotationsID(TEXT("UserAnnotations"));
...
void SLoadingProfilerWindow::UpdateEventAggregationTreeView()
{
	...
	if (EventAggregationTreeView)
	{
		TSharedPtr<IUntypedTable> SourceTable;

		TSharedPtr<const IAnalysisSession> Session = FInsightsManager::Get()->GetSession();
		if (Session.IsValid())
		{
			const ILoadTimeProfilerProvider* LoadTimeProfilerProvider = ReadLoadTimeProfilerProvider(*Session.Get());
			if (LoadTimeProfilerProvider)
			{
				FAnalysisSessionReadScope SessionReadScope(*Session.Get());
				ITable<FLoadTimeProfilerAggregatedStats>* EventAggregationTable =
					LoadTimeProfilerProvider->CreateEventAggregation(SelectionStartTime, SelectionEndTime);
				SourceTable = MakeShareable(EventAggregationTable);
			}
		}

		EventAggregationTreeView->UpdateSourceTable(SourceTable);
	}
}
...
void SLoadingProfilerWindow::UpdateObjectTypeAggregationTreeView()
{
	...
	if (ObjectTypeAggregationTreeView)
	{
		...
		if (Session.IsValid())
		{
			...
			{
				FAnalysisSessionReadScope SessionReadScope(*Session.Get());
				ITable<FLoadTimeProfilerAggregatedStats>* ObjectTypeAggregationTable =
					LoadTimeProfilerProvider->CreateObjectTypeAggregation(SelectionStartTime, SelectionEndTime);
				SourceTable = MakeShareable(ObjectTypeAggregationTable);
			}
		}

		ObjectTypeAggregationTreeView->UpdateSourceTable(SourceTable);
	}
}
...
void SLoadingProfilerWindow::UpdatePackageDetailsTreeView()
{
	...
	if (PackageDetailsTreeView)
	{
		...
		if (Session.IsValid())
		{
			...
			{
				FAnalysisSessionReadScope SessionReadScope(*Session.Get());
				ITable<FPackagesTableRow>* PackageDetailsTable =
					LoadTimeProfilerProvider->CreatePackageDetailsTable(SelectionStartTime, SelectionEndTime);
				SourceTable = MakeShareable(PackageDetailsTable);
			}
		}

		PackageDetailsTreeView->UpdateSourceTable(SourceTable);
	}
}
...
void SLoadingProfilerWindow::UpdateExportDetailsTreeView()
{
	...
	if (ExportDetailsTreeView)
	{
		...
		if (Session.IsValid())
		{
			...
			{
				FAnalysisSessionReadScope SessionReadScope(*Session.Get());
				ITable<FExportsTableRow>* ExportDetailsTable =
					LoadTimeProfilerProvider->CreateExportDetailsTable(SelectionStartTime, SelectionEndTime);
				SourceTable = MakeShareable(ExportDetailsTable);
			}
		}

		ExportDetailsTreeView->UpdateSourceTable(SourceTable);
	}
}
...
void SLoadingProfilerWindow::UpdateRequestsTreeView()
{
	...
	if (RequestsTreeView)
	{
		...
		if (Session.IsValid())
		{
			...
			{
				FAnalysisSessionReadScope SessionReadScope(*Session.Get());
				ITable<FRequestsTableRow>* RequestsTable =
					LoadTimeProfilerProvider->CreateRequestsTable(SelectionStartTime, SelectionEndTime);
				SourceTable = MakeShareable(RequestsTable);
			}
		}

		RequestsTreeView->UpdateSourceTable(SourceTable);
	}
}
...
BEGIN_SLATE_FUNCTION_BUILD_OPTIMIZATION
TSharedRef<SDockTab> SLoadingProfilerWindow::SpawnTab_UserAnnotations(const FSpawnTabArgs& Args)
{
	const TSharedRef<SDockTab> DockTab = SNew(SDockTab)
		.ShouldAutosize(false)
		.TabRole(ETabRole::PanelTab)
		[
			SAssignNew(UserAnnotationsPanel, TimingProfiler::SUserAnnotationsPanel)
			.HostTimingView(TimingView)
		];

	DockTab->SetOnTabClosed(SDockTab::FOnTabClosedCallback::CreateRaw(this, &SLoadingProfilerWindow::OnUserAnnotationsTabClosed));
	AddOpenTab(DockTab);
...
void SLoadingProfilerWindow::OnUserAnnotationsTabClosed(TSharedRef<SDockTab> TabBeingClosed)
{
	UserAnnotationsPanel = nullptr;
...
void SLoadingProfilerWindow::RegisterTabSpawners()
{
	...
	if (Config.ShouldRegisterMinorTab(FLoadingProfilerTabs::UserAnnotationsID))
	{
		TabManagerPtr->RegisterTabSpawner(FLoadingProfilerTabs::UserAnnotationsID, FOnSpawnTab::CreateRaw(this, &SLoadingProfilerWindow::SpawnTab_UserAnnotations))
			.SetDisplayName(LOCTEXT("UserAnnotationsTabTitle", "Annotations"))
			.SetIcon(FSlateIcon(FInsightsStyle::GetStyleSetName(), "Icons.Annotation"))
			.SetGroup(Group);
	}
}
...
{
	return FTabManager::NewLayout("InsightsLoadingProfilerLayout_v1.3")
		->AddArea
		(
			...
			(
				...
				->AddTab(FLoadingProfilerTabs::UserAnnotationsID, ETabState::OpenedTab)
