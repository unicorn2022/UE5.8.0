...
private:
	void RegisterDefaultToggleOptions()
	{
		...
		RegisterToggleOptions(Option_ShowGpuFencesTracks, false);
...
public:
	...
	int32 GetTimingTrackDepthLimitMin() const { return TimingTrackDepthLimitMin; }
	void SetTimingTrackDepthLimitMin(int32 DepthLimit) { TimingTrackDepthLimitMin = DepthLimit; }
	void SetAndSaveTimingTrackDepthLimitMin(int32 DepthLimit) { SET_AND_SAVE(TimingTrackDepthLimitMin, DepthLimit); }

	int32 GetTimingTrackDepthLimitMax() const { return TimingTrackDepthLimitMax; }
	void SetTimingTrackDepthLimitMax(int32 DepthLimit) { TimingTrackDepthLimitMax = DepthLimit; }
	void SetAndSaveTimingTrackDepthLimitMax(int32 DepthLimit) { SET_AND_SAVE(TimingTrackDepthLimitMax, DepthLimit); }
	...
	bool GetShowFormatInTimerNames() const { return bShowFormatInTimerNames; }
	void SetShowFormatInTimerNames(bool Value) { bShowFormatInTimerNames = Value; }
	void SetAndSaveShowFormatInTimerNames(bool Value) { SET_AND_SAVE(bShowFormatInTimerNames, Value); }
	...
	int32 GetTimersViewAggregationMode() const { return TimersViewAggregationMode; }
	void SetTimersViewAggregationMode(int32 InMode) { TimersViewAggregationMode = InMode; }
	void SetAndSaveTimersViewAggregationMode(int32 InMode) { SET_AND_SAVE(TimersViewAggregationMode, InMode); }
	...
	ETraceFrameType GetGoToFrameType() const { return GoToFrameType; }
	void SetGoToFrameType(ETraceFrameType InType) { GoToFrameType = InType; }
	void SetAndSaveGoToFrameType(ETraceFrameType InType) { SET_AND_SAVE(GoToFrameType, InType); }

	int64 GetGoToFrameFrameNumber() const { return GoToFrameFrameNumber; }
	void SetGoToFrameFrameNumber(int64 InFrameNumber) { GoToFrameFrameNumber = InFrameNumber; }
	void SetAndSaveGoToFrameFrameNumber(int64 InFrameNumber) { SET_AND_SAVE(GoToFrameFrameNumber, InFrameNumber); }

	bool IsGoToFrameZoomTimingViewEnabled() const { return bGoToFrameZoomTimingView; }
	void SetGoToFrameZoomTimingView(bool bOnOff) { bGoToFrameZoomTimingView = bOnOff; }
	void SetAndSaveGoToFrameZoom(bool bOnOff) { SET_AND_SAVE(bGoToFrameZoomTimingView, bOnOff); }

	bool IsGoToFrameSelectTimeRangeEnabled() const { return bGoToFrameSelectTimeRange; }
	void SetGoToFrameSelectTimeRange(bool bOnOff) { bGoToFrameSelectTimeRange = bOnOff; }
	void SetAndSaveGoToFrameSelectTimeRange(bool bOnOff) { SET_AND_SAVE(bGoToFrameSelectTimeRange, bOnOff); }

	//////////////////////////////////////////////////
	// [Insights.GoToTime]

	double GetGoToTimeTime() const { return GoToTimeTime; }
	void SetGoToTimeTime(double InTime) { GoToTimeTime = InTime; }
	void SetAndSaveGoToTimeTime(double InTime) { SET_AND_SAVE(GoToTimeTime, InTime); }

	double GetGoToTimeTimeRangeDuration() const { return GoToTimeTimeRangeDuration; }
	void SetGoToTimeTimeRangeDuration(double InDuration) { GoToTimeTimeRangeDuration = InDuration; }
	void SetAndSaveGoToTimeTimeRangeDuration(double InDuration) { SET_AND_SAVE(GoToTimeTimeRangeDuration, InDuration); }

	bool IsGoToTimeZoomTimingViewEnabled() const { return bGoToTimeZoomTimingView; }
	void SetGoToTimeZoomTimingView(bool bOnOff) { bGoToTimeZoomTimingView = bOnOff; }
	void SetAndSaveGoToTimeZoom(bool bOnOff) { SET_AND_SAVE(bGoToTimeZoomTimingView, bOnOff); }

	bool IsGoToTimeSelectTimeRangeEnabled() const { return bGoToTimeSelectTimeRange; }
	void SetGoToTimeSelectTimeRange(bool bOnOff) { bGoToTimeSelectTimeRange = bOnOff; }
	void SetAndSaveGoToTimeSelectTimeRange(bool bOnOff) { SET_AND_SAVE(bGoToTimeSelectTimeRange, bOnOff); }
...
private:
	...
	/** The minimum number of lanes in a timing events track. */
	int32 TimingTrackDepthLimitMin;

	/** The maximum number of lanes in a timing events track. */
	int32 TimingTrackDepthLimitMax;
	...
	/** When enabled, the format will be appended to the timers static name for events with metadata specs. */
	bool bShowFormatInTimerNames = true;
	 ...
	 * See ETimerAggregationMode in TimingProfiler\ViewModels\TimerAggregator.h.
	 */
	int32 TimersViewAggregationMode = 0; // 0=Instance, 1=GameFrame, 2=RenderingFrame
	...
	/** Last selected frame type in the Go to Frame dialog. */
	ETraceFrameType GoToFrameType = TraceFrameType_Game;

	/** Last frame number entered in the Go to Frame dialog. */
	int64 GoToFrameFrameNumber = 0;

	/** If true, zooms in on the target frame; if false, centers the view on it. */
	bool bGoToFrameZoomTimingView = true;

	/** If true, selects the frame's time range when navigating. */
	bool bGoToFrameSelectTimeRange = true;

	//////////////////////////////////////////////////
	// [Insights.GoToTime]

	/** Last time (in seconds) entered in the Go to Time dialog. Negative if none saved. */
	double GoToTimeTime = -1.0;

	/** Time range duration in seconds used for selection and/or zoom when navigating to a time. */
	double GoToTimeTimeRangeDuration = 1.0;

	/** If true, zooms in on the target time; if false, centers the view on it. */
	bool bGoToTimeZoomTimingView = true;

	/** If true, selects the time range starting at the target time. */
	bool bGoToTimeSelectTimeRange = true;
