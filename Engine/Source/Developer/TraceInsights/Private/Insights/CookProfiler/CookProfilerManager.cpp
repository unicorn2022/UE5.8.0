...
{
	// Check if session has Cook events (to spawn the Packages tab), but not too often.
	if (!bIsAvailable && AvailabilityCheck.Tick())
	{
		...
		if (Session.IsValid())
		{
			...
			if (CookProvider)
			{
				...
				{
					bIsAvailable = true;
					TabManagerShared->TryInvokeTab(FCookProfilerTabs::PackageTableTreeViewTabID, true);
				}
