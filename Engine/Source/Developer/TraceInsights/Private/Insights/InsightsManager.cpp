...
{
	ActionManager.Map_LoadTraceFile();
	ActionManager.Map_ToggleDebugInfo();
	ActionManager.Map_OpenSettings();
}
...
void FInsightsManager::UpdateSessionDuration()
{
	...
	if (Session.IsValid())
	{
		...
		if (LocalSessionDuration != SessionDuration)
		{
			...
			if (bIsAnalysisComplete)
			{
				UE_LOGF(TraceInsights, Warning, "The session duration was updated (%ls) after the analysis has been completed.",
		...
		if (bLocalIsAnalysisComplete && !bIsAnalysisComplete)
		{
			...
			UE_LOGF(TraceInsights, Log, "Analysis has completed in %ls (%.1fX speed; session duration: %ls).",
...
void FInsightsManager::ActivateTimingInsightsTab()
{
	...
	TimingProfiler::FTimingProfilerManager::Get()->ActivateMajorTab();
}
...
void FInsightsManager::ConnectToSecureTrace(FStringView Host, uint16 Port, TConstArrayView<uint8> AuthorizationToken, FStringView CertificateFile, TConstArrayView<uint8> CertificateAuthority, FAnsiStringView Identity)
{
#if UE_TRACE_ANALYSIS_SSS_ENABLED
	ResetSession();

	TUniquePtr<UE::Trace::FSecureSocketStream> TraceData = MakeUnique<UE::Trace::FSecureSocketStream>();
	bool bConnected = false;
	if (!CertificateFile.IsEmpty())
	{
		bConnected = TraceData->ConnectWithCertificate(Host, Port, AuthorizationToken, CertificateFile);
	}
	else if (!CertificateAuthority.IsEmpty())
	{
		bConnected = TraceData->ConnectWithSelfSigned(Host, Port, AuthorizationToken, CertificateAuthority);
	}
	else
	{
		bConnected = TraceData->ConnectWithPreSharedKey(Host, Port, AuthorizationToken, Identity);
	}
	...
	if (bConnected)
	{
		...
		constexpr uint32 TraceId = 0;
		TStringBuilder<64> TraceName;
		TraceName << TEXT("Secure Trace Stream from ") << Host;

		Session = AnalysisService->StartAnalysis(TraceId, TraceName.ToString(), MoveTemp(TraceData));

		if (Session)
		{
			CurrentTraceStreamType = ETraceStreamType::DirectTraceSecure;
			CurrentTraceName = TraceName;
			CurrentTraceId = 0;
			CurrentDirectTracePort = Port;
			bIsSessionInfoSet = false;
	...
	{
		UE_LOGF(LogCore, Error, "Failed to connect to secure trace session.");
	}

	if (!bSuccess && bAutoQuit)
	{
		RequestEngineExit(AutoQuitMsgOnFail);
	}
