...
#include "Insights/SpatialProfiler/Widgets/SSpatialInsightsWindow.h"
...
void FTraceInsightsModule::ShutdownModule()
{
	...
	// Unregister User Annotations extender
...
#define INSIGHTS_CHECK_SHARED_REFERENCES 1
	...
	{
		UE_LOGF(TraceInsights, Warning, "The analysis Session is still referenced! Force delete!");
...
FString FTraceInsightsModule::GetDefaultStoreDir()
{
	...
	if (!StoreClient)
	{
		UE_LOGF(TraceInsights, Error, "Failed to connect to the store client.");
		return FString("");
	}
	...
	if (!Status)
	{
		UE_LOGF(TraceInsights, Error, "Failed to get the status of the store client.");
		return FString("");
	}
...
void FTraceInsightsModule::CreateSessionViewer(bool bAllowDebugTools)
{
	...
	TSharedRef<FTabManager::FLayout> DefaultLayout = FTabManager::NewLayout("UnrealInsightsLayout_v1.2");
...
void FTraceInsightsModule::AddAreaForSessionViewer(TSharedRef<FTabManager::FLayout> Layout)
{
	...
	Stack->AddTab(UE::Insights::ObjectProfiler::ObjectProfilerTabId, ETabState::ClosedTab);
	Stack->AddTab(UE::Insights::SpatialProfiler::SpatialProfilerTabId, ETabState::ClosedTab);
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void FTraceInsightsModule::StartAnalysisWithSecureTrace(FStringView Host, uint16 Port, TConstArrayView<uint8> AuthorizationToken, FStringView CertificateFile, TConstArrayView<uint8> CertificateAuthority, FAnsiStringView Identity)
{
	FInsightsManager::Get()->ConnectToSecureTrace(Host, Port, AuthorizationToken, CertificateFile, CertificateAuthority, Identity);
	UpdateAppTitle();
}
...
void FTraceInsightsModule::SetAssetInfoProvider(
	TSharedPtr<UE::Insights::ObjectProfiler::IAssetInfoProvider> InProvider)
{
	AssetInfoProvider = InProvider;

	// Apply immediately if the window is already open.
	if (TSharedPtr<UE::Insights::ObjectProfiler::FObjectProfilerManager> Manager =
		UE::Insights::ObjectProfiler::FObjectProfilerManager::Get())
	{
		if (TSharedPtr<UE::Insights::ObjectProfiler::SObjectProfilerWindow> Window =
			Manager->GetProfilerWindow())
		{
			Window->SetAssetInfoProvider(InProvider);
		}
	}
}
...
TSharedPtr<UE::Insights::ObjectProfiler::IAssetInfoProvider>
FTraceInsightsModule::GetAssetInfoProvider() const
{
	return AssetInfoProvider;
}

void FTraceInsightsModule::OpenMemoryAllocationsView(double InTimeA, const FText& InTabDisplayName, const TSet<FString>& InPackageFilter)
{
	using namespace UE::Insights::MemoryProfiler;

	check(IsInGameThread());

	TSharedPtr<FMemoryProfilerManager> Manager = FMemoryProfilerManager::Get();
	if (!Manager.IsValid())
	{
		UE_LOGF(TraceInsights, Warning, "Cannot open Memory allocations view: Memory Profiler is not loaded.");
		return;
	}

	TSharedPtr<SMemoryProfilerWindow> Window = Manager->GetProfilerWindow();
	if (!Window.IsValid())
	{
		UE_LOGF(TraceInsights, Warning, "Cannot open Memory allocations view: Memory Profiler window is not open.");
		return;
	}

	FMemorySharedState& SharedState = Window->GetSharedState();

	TSharedPtr<FMemoryRuleSpec> ActiveAllocsRule;
	for (const TSharedPtr<FMemoryRuleSpec>& Rule : SharedState.GetMemoryRules())
	{
		if (Rule.IsValid() && Rule->GetValue() == TraceServices::IAllocationsProvider::EQueryRule::aAf)
		{
			ActiveAllocsRule = Rule;
			break;
		}
	}

	if (!ActiveAllocsRule.IsValid())
	{
		UE_LOGF(TraceInsights, Warning, "Cannot open Memory allocations view: 'aAf' rule not registered.");
		return;
	}

	TSharedPtr<SMemAllocTableTreeView> MemAllocTableTreeView = Window->OpenNewMemAllocTableTreeViewTab(InTabDisplayName);
	if (!MemAllocTableTreeView.IsValid())
	{
		UE_LOGF(TraceInsights, Warning, "Cannot open Memory allocations view: failed to create allocations tab.");
		return;
	}

	SMemAllocTableTreeView::FQueryParams QueryParams;
	QueryParams.Rule = ActiveAllocsRule;
	QueryParams.TimeMarkers[0] = InTimeA;
	QueryParams.PackageFilter = InPackageFilter;
