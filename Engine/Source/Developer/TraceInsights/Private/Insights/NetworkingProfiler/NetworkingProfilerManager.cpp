...
FNetworkingProfilerManager::FNetworkingProfilerManager(TSharedRef<FUICommandList> InCommandList)
	...
	, ActionManager(this)
	, MajorTabId(FInsightsManagerTabs::NetworkingProfilerTabId)
	, LogListingName("NetworkingInsights")
{
}
...
void FNetworkingProfilerManager::Initialize(IUnrealInsightsModule& InsightsModule)
{
	...
	UE_LOGF(LogNetworkingProfiler, Log, "Initialize");
...
void FNetworkingProfilerManager::Shutdown()
{
	...
	UE_LOGF(LogNetworkingProfiler, Log, "Shutdown");
}
	...
	if (!bIsTabRegistered)
	{
		...
		if (Config.bIsAvailable)
		{
			...
			FTabSpawnerEntry& TabSpawnerEntry = FGlobalTabmanager::Get()->RegisterNomadTabSpawner(MajorTabId,
					...
					FCanSpawnTab::CreateRaw(this, &FNetworkingProfilerManager::CanSpawnTab))
...
TSharedRef<SDockTab> FNetworkingProfilerManager::SpawnTab(const FSpawnTabArgs& Args)
{
	...
	// Register OnTabClosed to handle Networking profiler manager shutdown.
...
bool FNetworkingProfilerManager::Tick(float DeltaTime)
{
	...
	if (!bIsAvailable && AvailabilityCheck.Tick())
	{
		...
		if (NetTraceVersion > 0)
		{
			...
			FMessageLogModule& MessageLogModule = FModuleManager::LoadModuleChecked<FMessageLogModule>("MessageLog");
			MessageLogModule.RegisterLogListing(GetLogListingName(), LOCTEXT("NetworkingInsights", "Networking Insights"));
			MessageLogModule.EnableMessageLogDisplay(true);

			if (FGlobalTabmanager::Get()->HasTabSpawner(MajorTabId))
			{
				// Spawn 2 tabs.
				UE_LOGF(LogNetworkingProfiler, Log, "Opening the \"Networking Insights\" tabs...");
...
{
	UE_LOGF(LogNetworkingProfiler, Log, "OnSessionChanged");
