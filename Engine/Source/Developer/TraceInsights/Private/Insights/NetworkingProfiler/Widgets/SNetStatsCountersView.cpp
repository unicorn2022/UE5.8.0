...
void SNetStatsCountersView::UpdateTree()
{
	...
	if (TotalTime > 0.1)
	{
		UE_LOGF(LogNetworkingProfiler, Log, "[NetStats] Tree view updated in %.3fs (%d events) --> G:%.3fs + S:%.3fs + F:%.3fs",
...
void SNetStatsCountersView::RebuildTree(bool bResync)
{
	...
	if (TotalTime > 0.01)
	{
		...
		UE_LOGF(LogNetworkingProfiler, Log, "[NetStatsCounters] Tree view rebuilt in %.4fs (%.4fs + %.4fs) --> %d net stats counters (%d added)",
...
void SNetStatsCountersView::UpdateStatsInternal()
{
	...
	UE_LOGF(LogNetworkingProfiler, Log, "[NetStats] Aggregated stats updated in %.4fs (%.4fs + %.4fs)",
