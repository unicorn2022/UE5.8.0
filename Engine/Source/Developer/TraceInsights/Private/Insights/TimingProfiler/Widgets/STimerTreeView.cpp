...
void STimerTreeView::InitCommandList()
{
	...
	CommandList->MapAction(FTimerTreeViewCommands::Get().Command_FindMaxInstance, FExecuteAction::CreateSP(this, &STimerTreeView::ContextMenu_FindInstance_Execute, ESelectEventType::Max), FCanExecuteAction::CreateSP(this, &STimerTreeView::ContextMenu_FindInstance_CanExecute));
	CommandList->MapAction(FTimerTreeViewCommands::Get().Command_FindMinInstance, FExecuteAction::CreateSP(this, &STimerTreeView::ContextMenu_FindInstance_Execute, ESelectEventType::Min), FCanExecuteAction::CreateSP(this, &STimerTreeView::ContextMenu_FindInstance_CanExecute));
	CommandList->MapAction(FTimerTreeViewCommands::Get().Command_FindMaxInstanceInSelection, FExecuteAction::CreateSP(this, &STimerTreeView::ContextMenu_FindInstanceInSelection_Execute, ESelectEventType::Max), FCanExecuteAction::CreateSP(this, &STimerTreeView::ContextMenu_FindInstanceInSelection_CanExecute));
	CommandList->MapAction(FTimerTreeViewCommands::Get().Command_FindMinInstanceInSelection, FExecuteAction::CreateSP(this, &STimerTreeView::ContextMenu_FindInstanceInSelection_Execute, ESelectEventType::Min), FCanExecuteAction::CreateSP(this, &STimerTreeView::ContextMenu_FindInstanceInSelection_CanExecute));
}

////////////////////////////////////////////////////////////////////////////////////////////////////

BEGIN_SLATE_FUNCTION_BUILD_OPTIMIZATION
void STimerTreeView::Construct(const FArguments& InArgs, const FText& InViewName, ETreeType InTreeType)
{
	ViewName = InViewName;
	TreeType = InTreeType;
...
TSharedPtr<SWidget> STimerTreeView::TreeView_GetMenuContent()
{
	...
	FTimerNodePtr SingleSelectedNode = nullptr;
	...
	MenuBuilder.BeginSection("TimerOptions", LOCTEXT("ContextMenu_Section_TimerOptions", "Timer Options"));
	{
		...
		// Highlight event
		{
			...
			Action_ToggleHighlight.ExecuteAction = FExecuteAction::CreateSP(this, &STimerTreeView::ToggleTimingViewEventFilter, SingleSelectedNode);
...
void STimerTreeView::InitializeAndShowHeaderColumns()
{
	...
	if (TreeType == ETreeType::Callers)
	{
		FTimersViewColumnFactory::CreateCallersViewColumns(Columns);
	}
	else if (TreeType == ETreeType::Callees)
	{
		FTimersViewColumnFactory::CreateCalleesViewColumns(Columns);
	}
	else
	{
...
void STimerTreeView::ShowColumn(const FName ColumnId)
{
	...
	ColumnArgs
		...
		.ToolTip(STimersViewTooltip::GetColumnTooltipForMode(Column, ETimerAggregationMode::Instance))
...
{
	TreeNodes.Reset();
	FTimerNodePtr RootTimerNodePtr;
	...
	if (Session.IsValid())
	{
		...
		{
			RootTimerNodePtr = CreateTimerNodeRec(Root, *TimingProfilerProvider);
			if (RootTimerNodePtr)
			{
...
FTimerNodePtr STimerTreeView::CreateTimerNodeRec(const TraceServices::FTimingProfilerButterflyNode& Node, const TraceServices::ITimingProfilerProvider& TimingProfilerProvider)
{
	...
	if (FInsightsManager::Get()->GetSettings().GetShowFormatInTimerNames())
	{
		...
		if (Session)
		{
			...
			TimingProfilerProvider.MakeTimerDisplayName(*Node.Timer, StringBuilder);
		}
	}
	else
	{
		StringBuilder.Append(Node.Timer->Name);
	}

	FTimerNodePtr TimerNodePtr = MakeShared<FTimerNode>(Node.Timer->Id, *StringBuilder, Type, true);
	...
	for (const TraceServices::FTimingProfilerButterflyNode* ChildNodePtr : Node.Children)
	{
		...
		{
			FTimerNodePtr ChildTimerNodePtr = CreateTimerNodeRec(*ChildNodePtr, TimingProfilerProvider);
...
void STimerTreeView::ContextMenu_FindInstance_Execute(ESelectEventType EventType) const
{
	FTimerNodePtr SelectedNode = GetSingleSelectedTimerNode();
...
void STimerTreeView::ContextMenu_FindInstanceInSelection_Execute(ESelectEventType EventType) const
{
	FTimerNodePtr SelectedNode = GetSingleSelectedTimerNode();
	if (!SelectedNode.IsValid() || SelectedNode->GetType() == ETimerNodeType::Group)
	{
...
TSharedPtr<STimingProfilerWindow> STimerTreeView::GetProfilerWindow() const
{
