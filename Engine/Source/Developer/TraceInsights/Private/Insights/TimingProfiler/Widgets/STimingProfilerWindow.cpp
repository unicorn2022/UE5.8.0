...
const FName FTimingProfilerTabs::UserAnnotationsID(TEXT("UserAnnotations"));
...
STimingProfilerWindow::~STimingProfilerWindow()
{
	...
	check(UserAnnotationsPanel == nullptr);
...
void STimingProfilerWindow::Reset()
{
	...
	// UserAnnotationsPanel does not need a Reset call; it auto-refreshes via Tick.
...
TSharedRef<SDockTab> STimingProfilerWindow::SpawnTab_TimingView(const FSpawnTabArgs& Args)
{
	...
	// Listen to TimingView keyboard shortcuts from anywhere in this window. The command list is kept internally as 
	// a weak reference, so when the TimingView is closed and destroyed, its command list will be ignored and cleaned
	// up on next mutation of this window's command list. Initial motivation to add this was for keyboard shortcuts 
	// for Go to Frame and Go to Time, which only make sense when TimingView is shown, but are nice to capture from any panel.
	GetCommandList()->Append(TimingView->GetCommandList().ToSharedRef());
...
TSharedRef<SDockTab> STimingProfilerWindow::SpawnTab_Callers(const FSpawnTabArgs& Args)
{
	...
	const TSharedRef<SDockTab> DockTab = SNew(SDockTab)
		...
		.TabRole(ETabRole::PanelTab)
		[
			SAssignNew(CallersTreeView, STimerTreeView, LOCTEXT("CallersTreeViewName", "Callers"), STimerTreeView::ETreeType::Callers)
		];
...
TSharedRef<SDockTab> STimingProfilerWindow::SpawnTab_Callees(const FSpawnTabArgs& Args)
{
	...
	const TSharedRef<SDockTab> DockTab = SNew(SDockTab)
		...
		.TabRole(ETabRole::PanelTab)
		[
			SAssignNew(CalleesTreeView, STimerTreeView, LOCTEXT("CalleesTreeViewName", "Callees"), STimerTreeView::ETreeType::Callees)
		];
...
BEGIN_SLATE_FUNCTION_BUILD_OPTIMIZATION
TSharedRef<SDockTab> STimingProfilerWindow::SpawnTab_UserAnnotations(const FSpawnTabArgs& Args)
{
	FTimingProfilerManager::Get()->SetUserAnnotationsViewVisible(true);

	const TSharedRef<SDockTab> DockTab = SNew(SDockTab)
		.ShouldAutosize(false)
		.TabRole(ETabRole::PanelTab)
		[
			SAssignNew(UserAnnotationsPanel, SUserAnnotationsPanel)
		.HostTimingView(TimingView)
		];

	DockTab->SetOnTabClosed(SDockTab::FOnTabClosedCallback::CreateRaw(this, &STimingProfilerWindow::OnUserAnnotationsTabClosed));
	AddOpenTab(DockTab);
...
void STimingProfilerWindow::OnUserAnnotationsTabClosed(TSharedRef<SDockTab> TabBeingClosed)
{
	FTimingProfilerManager::Get()->SetUserAnnotationsViewVisible(false);
...
void STimingProfilerWindow::RegisterTabSpawners()
{
	...
	{
		TabManagerPtr->RegisterTabSpawner(FTimingProfilerTabs::UserAnnotationsID, FOnSpawnTab::CreateRaw(this, &STimingProfilerWindow::SpawnTab_UserAnnotations))
...
{
	return FTabManager::NewLayout("InsightsTimingProfilerLayout_v1.3")
		->AddArea
		(
			...
			(
				...
				(
					...
					->AddTab(FTimingProfilerTabs::UserAnnotationsID, ETabState::OpenedTab)
