...
void STimersView::Construct(const FArguments& InArgs)
{
			...
			[
				...
				[
					...
					[
						...
						.MinDesiredWidth(128.0f)
						[
							SAssignNew(ModeComboBox, SComboBox<TSharedPtr<ETimerAggregationMode>>)
	...
	Aggregator->SetAggregationMode(AggregationMode);
...
{
	int32 NumSelectedNodes = TreeView->GetNumItemsSelected();
	FTimerNodePtr SingleSelectedNode;
	bool bIsValidNode = false;
	...
	if (NumSelectedNodes == 1)
	{
		...
		{
			bIsValidNode = (SingleSelectedNode->GetType() != ETimerNodeType::Group);
		}
		else
		{
			NumSelectedNodes = 0;
	...
	{
		check(SingleSelectedNode.IsValid());
		FString ItemName = SingleSelectedNode->GetDisplayName().ToString();
	...
	{
		auto CanExecute = [this, bIsValidNode]()
		{
			return bIsValidNode && GetTimingView().IsValid();
		};
		...
		// Highlight event
		{
			...
			Action_ToggleHighlight.ExecuteAction = FExecuteAction::CreateSP(this, &STimersView::ToggleTimingViewEventFilter, SingleSelectedNode);

			TSharedPtr<STimingView> TimingView = GetTimingView();

			if (bIsValidNode &&
				TimingView.IsValid() &&
				TimingView->IsFilterByEventType(SingleSelectedNode->GetTimerId()))
			{
				MenuBuilder.AddMenuEntry
				(
		...
		// Open Source in IDE
		{
			...
			FString File;
			uint32 Line = 0;
			bool bIsValidSource = bIsValidNode && SingleSelectedNode->GetSourceFileAndLine(File, Line);
			FString UsablePath;
...
void STimersView::TreeView_BuildPlotTimerMenu(FMenuBuilder& MenuBuilder)
{
	...
	const bool bIsValidNode = SelectedNode.IsValid() && SelectedNode->GetType() != ETimerNodeType::Group;

	auto CanExecuteAddToGraphTrack = [this, bIsValidNode]()
	{
		return bIsValidNode && GetTimingView().IsValid();
	};

	auto CanExecuteAddToFramesTrack = [this, bIsValidNode]()
	{
		return bIsValidNode && GetFrameTrack().IsValid();
	};
...
void STimersView::TreeView_BuildSettingsMenu(FMenuBuilder& MenuBuilder)
{
	...
	MenuBuilder.BeginSection("Settings");
	{
		...
		{
			FUIAction Action_ShowFormatInTimerNames
			(
				FExecuteAction::CreateSP(this, &STimersView::ToggleShowFormatInTimerNames_Execute),
				FCanExecuteAction::CreateLambda([]() { return true; }),
				FIsActionChecked::CreateLambda([]() { return FInsightsManager::Get()->GetSettings().GetShowFormatInTimerNames(); })
			);

			MenuBuilder.AddMenuEntry
			(
				LOCTEXT("SetShowFormatInTimerNames_Fmt", "Show Format In Timer Names"),
				LOCTEXT("SetShowFormatInTimerNamesTooltip_Fmt", "Append the format to timers names for events with metadata."),
				FSlateIcon(),
				Action_ShowFormatInTimerNames,
				NAME_None,
				EUserInterfaceActionType::ToggleButton
			);
		}
...
void STimersView::ApplyFiltering()
{
	...
	for (int32 GroupIndex = 0; GroupIndex < NumGroups; ++GroupIndex)
	{
		...
		for (const FBaseTreeNodePtr& ChildPtr : GroupChildren)
		{
			...
			const bool bIsChildVisible =
				(!bFilterOutZeroCountTimers || NodePtr->GetAggregatedStats().InstanceCount > 0) &&
	...
	if (TotalTime > 0.1)
	{
		UE_LOGF(LogTimingProfiler, Log, "[Timers] Tree view filtered in %.3fs (%d timers)",
...
{
	OutSearchStrings.Add(FTimerNodePtr->GetDisplayName().ToString());
}
...
void STimersView::FilterByTimerType_OnCheckStateChanged(ECheckBoxState NewRadioState, const ETimerNodeType InNodeType)
{
	bTimerTypeIsVisible[static_cast<int>(InNodeType)] = (NewRadioState == ECheckBoxState::Checked);
	ApplyFiltering();
}

////////////////////////////////////////////////////////////////////////////////////////////////////

ECheckBoxState STimersView::FilterByTimerType_IsChecked(const ETimerNodeType InNodeType) const
{
	return bTimerTypeIsVisible[static_cast<int>(InNodeType)] ? ECheckBoxState::Checked : ECheckBoxState::Unchecked;
}
...
EHorizontalAlignment STimersView::TableRow_GetColumnOutlineHAlignment(const FName ColumnId) const
{
	...
	// First column
	if (Columns.Num() == 0 ||
		Columns[0].ColumnId == ColumnId)
	{
		return HAlign_Left;
	}
	// Last column
	else if (Columns[Columns.Num() - 1].ColumnId == ColumnId)
	{
		return HAlign_Right;
	}
	// Middle columns
...
void STimersView::CreateGroups()
{
	...
	if (GroupingMode == ETimerGroupingMode::Flat)
	{
		...
		FTimerNodePtr GroupPtr = MakeShared<FTimerNode>(LOCTEXT("GroupName_All", "All"));
	...
	else if (GroupingMode == ETimerGroupingMode::ByMetaGroupName)
	{
		...
		for (const FTimerNodePtr& NodePtr : TimerNodes)
		{
			...
			if (!GroupPtr)
			{
				const FText DisplayName = FText::FromName(GroupName);
				GroupPtr = GroupNodeSet.Add(GroupName, MakeShared<FTimerNode>(DisplayName));
			}
			GroupPtr->AddChildAndSetParent(NodePtr);
			TreeView->SetItemExpansion(GroupPtr, true);
		}
	...
	else if (GroupingMode == ETimerGroupingMode::ByType)
	{
		...
		for (const FTimerNodePtr& NodePtr : TimerNodes)
		{
			...
			if (!GroupPtr)
			{
				const FText GroupName = TimerNodeTypeHelper::ToText(NodeType);
	...
	else if (GroupingMode == ETimerGroupingMode::ByName)
	{
		...
		for (const FTimerNodePtr& NodePtr : TimerNodes)
		{
			FString FirstLetterStr(NodePtr->GetDisplayName().ToString().Left(1).ToUpper());
			const TCHAR FirstLetter = FirstLetterStr.Len() > 0 ? FirstLetterStr[0] : TEXT('\0');
			FTimerNodePtr GroupPtr = GroupNodeSet.FindRef(FirstLetter);
			if (!GroupPtr)
			{
				const FText GroupName = FText::FromString(FirstLetterStr);
	...
	else if (GroupingMode == ETimerGroupingMode::ByInstanceCount)
	{
		...
		for (const FTimerNodePtr& NodePtr : TimerNodes)
		{
			...
			if (!GroupPtr)
			{
				const FText GroupName =
					(Order == 0) ?
						LOCTEXT("GroupName_ZeroCount", "Count == 0") :
					(Order < MaxOrder) ?
						FText::Format(LOCTEXT("GroupName_CountFmt1", "Count: [{0} .. {1})"), FText::FromString(Orders[Order - 1]), FText::FromString(Orders[Order])) :
						FText::Format(LOCTEXT("GroupName_CountFmt2", "Count >= {0}"), FText::FromString(Orders[MaxOrder - 1]));
	...
	if (TotalTime > 0.1)
	{
		UE_LOGF(LogTimingProfiler, Log, "[Timers] Tree view grouping updated in %.3fs (%d timers)",
			TotalTime, TimerNodes.Num());
	}
}
...
{
	if (SelectInfo != ESelectInfo::Direct &&
		NewGroupingMode.IsValid() &&
		*NewGroupingMode != GroupingMode)
	{
...
{
	ModeOptionsSource.Reset(uint32(ETimerAggregationMode::Count));
	for (uint32 Mode = 0; Mode < uint32(ETimerAggregationMode::Count); ++Mode)
	{
		ModeOptionsSource.Add(MakeShared<ETimerAggregationMode>(ETimerAggregationMode(Mode)));
	}

	for (TSharedPtr<ETimerAggregationMode> Option : ModeOptionsSource)
	{
		if (*Option == AggregationMode)
		{
...
void STimersView::Mode_OnSelectionChanged(TSharedPtr<ETimerAggregationMode> InAggregationMode, ESelectInfo::Type SelectInfo)
{
	if (!InAggregationMode.IsValid() || *InAggregationMode == AggregationMode)
	{
		return;
	}
...
TSharedRef<SWidget> STimersView::Mode_OnGenerateWidget(TSharedPtr<ETimerAggregationMode> InAggregationMode) const
{
	check(InAggregationMode.IsValid());
	return SNew(STextBlock)
		.Text(Mode_GetText(*InAggregationMode))
		.ToolTipText(Mode_GetTooltipText(*InAggregationMode));
}
...
{
	return Mode_GetText(AggregationMode);
}

////////////////////////////////////////////////////////////////////////////////////////////////////

FText STimersView::Mode_GetText(ETimerAggregationMode InAggregationMode) const
{
...
{
	return Mode_GetTooltipText(AggregationMode);
}

////////////////////////////////////////////////////////////////////////////////////////////////////

FText STimersView::Mode_GetTooltipText(ETimerAggregationMode InAggregationMode) const
{
	switch (InAggregationMode)
	{
	case ETimerAggregationMode::Instance:
...
void STimersView::SortTreeNodes()
{
	...
	if (TotalTime > 0.1)
	{
		UE_LOGF(LogTimingProfiler, Log, "[Timers] Tree view sorted (%ls, %c) in %.3fs (%d timers)",
...
void STimersView::RebuildTree(bool bResync)
{
	...
	if (Session.IsValid())
	{
		...
		if (TimingProfilerProvider)
		{
			...
			if (TimerCount != PreviousNodeCount)
			{
				...
				{
					const TraceServices::FTimingProfilerTimer* TimerPtr = TimerReader.GetTimer(TimerIndex);
					check(TimerPtr != nullptr);
					const TraceServices::FTimingProfilerTimer& Timer = *TimerPtr;
					...
					if (FInsightsManager::Get()->GetSettings().GetShowFormatInTimerNames())
					{
						TimingProfilerProvider->MakeTimerDisplayName(Timer, StringBuilder);
					}
					else
					{
					...
					TimerNodePtr = MakeShared<FTimerNode>(Timer.Id, *StringBuilder, Type, false);
					...
					TSharedPtr<STimingProfilerWindow> ProfilerWindow = GetProfilerWindow();
	...
	if (TotalTime > 0.01)
	{
		...
		UE_LOGF(LogTimingProfiler, Log, "[Timers] Tree view rebuilt in %.4fs (sync: %.4fs + update: %.4fs) --> %d timers (%d added)",
			TotalTime, SyncTime, TotalTime - SyncTime, TimerNodes.Num(), AddedNodeCount);
	}
}
...
{
	if (Aggregator->GetIntervalStartTime() != StartTime ||
		Aggregator->GetIntervalEndTime() != EndTime)
	{
		Aggregator->Cancel();
...
void STimersView::SelectTimerNode(uint32 TimerId)
{
	...
	if (NodePtr)
	{
		FBaseTreeNodePtr ParentNodePtr = NodePtr->GetParent();
		while (ParentNodePtr.IsValid())
		{
			check(ParentNodePtr->Is<FTimerNode>());
			TreeView->SetItemExpansion(StaticCastSharedPtr<FTimerNode>(ParentNodePtr), true);
			ParentNodePtr = ParentNodePtr->GetParent();
		}
...
void STimersView::ToggleTimingViewMainGraphEventSeries(FTimerNodePtr TimerNode) const
{
	...
	case ETimerAggregationMode::Instance:
		...
		break;
	case ETimerAggregationMode::GameFrame:
		ToggleTimingViewMainGraphEventFrameStatsSeries(TimerNode, ETraceFrameType::TraceFrameType_Game);
		break;
	case ETimerAggregationMode::RenderingFrame:
		ToggleTimingViewMainGraphEventFrameStatsSeries(TimerNode, ETraceFrameType::TraceFrameType_Rendering);
		break;
	default:
		ensure(0);
	}
...
void STimersView::ToggleGraphInstanceSeries(TSharedRef<FTimingGraphTrack> GraphTrack, FTimerNodeRef NodePtr) const
{
	...
	else
	{
		...
		Series->SetName(NodePtr->GetDisplayName());
		GraphTrack->SetDirtyFlag();
	}
...
void STimersView::ContextMenu_Export_Execute()
{
	...
	TArray<FTimerNodePtr> SelectedTimerNodes = TreeView->GetSelectedItems();
	...
	for (FTimerNodePtr Node : SelectedTimerNodes)
	{
	...
	// Write header.
	{
		...
		for (const TSharedRef<FTableColumn>& ColumnRef : VisibleColumns)
		{
			...
			FString ColumnName = ColumnRef->GetShortName().ToString();
			Writer.AppendValue(ColumnName);
		}
	...
	for (int32 Index = 0; Index < NodeCount; Index++)
	{
		...
		for (const TSharedRef<FTableColumn>& ColumnRef : VisibleColumns)
		{
			...
			FString Value = ColumnRef->GetValueAsSerializableString(*Node);
	...
	Stopwatch.Stop();
	const double TotalTime = Stopwatch.GetAccumulatedTime();
	UE_LOGF(TraceInsights, Log, "Exported aggregated timer stats to file in %.3fs (\"%ls\").", TotalTime, *Filename);
}
	...
	if (TimingView.IsValid())
	{
		...
		if (TimingProfilerProvider)
		{
			...
			TimingProfilerProvider->EnumerateGpuQueues(
				[&ExcludedThreads](const TraceServices::FGpuQueueInfo& QueueInfo)
				{
		...
		TimingView->EnumerateAllTracks(
			[&IncludedThreads, &ExcludedThreads]
			(TSharedPtr<FBaseTimingTrack>& Track) -> bool
			{
				if (Track->IsVisible())
				{
...
void STimersView::ContextMenu_ExportTimers_Execute() const
{
	...
	Params.bShowFormatInTimerNames = FInsightsManager::Get()->GetSettings().GetShowFormatInTimerNames();
...
void STimersView::SetRandomColorToNode(FTimerNode& TimerNode) const
{
	...
	SetColorToNode(TimerNode, CustomColor);
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////
// Find First/Previous/Next/Last/Min/Max Instance
////////////////////////////////////////////////////////////////////////////////////////////////////
...
void STimersView::SaveSettings()
{
	...
	Settings.SetTimersViewAggregationMode(static_cast<int32>(AggregationMode));
...
void STimersView::LoadSettings()
{
	...
	AggregationMode = static_cast<ETimerAggregationMode>(Settings.GetTimersViewAggregationMode());
...
TSharedPtr<STimingProfilerWindow> STimersView::GetProfilerWindow() const
{
	return FTimingProfilerManager::Get()->GetProfilerWindow();
}
...
void STimersView::ToggleShowFormatInTimerNames_Execute()
{
	FInsightsSettings& Settings = FInsightsManager::Get()->GetSettings();
	Settings.SetAndSaveShowFormatInTimerNames(!Settings.GetShowFormatInTimerNames());

	RebuildTree(true);

	FTimingProfilerManager::Get()->UpdateCallersAndCallees();
}
