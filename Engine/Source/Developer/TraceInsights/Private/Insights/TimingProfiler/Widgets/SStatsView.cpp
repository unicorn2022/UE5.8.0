...
TSharedPtr<SWidget> SStatsView::TreeView_GetMenuContent()
{
	...
	FStatsNodePtr SingleSelectedNode;
	bool bIsValidNode = false;

	if (NumSelectedNodes == 1)
	{
		TArray<FStatsNodePtr> SelectedNodes = TreeView->GetSelectedItems();
		check(SelectedNodes.Num() == 1);
		SingleSelectedNode = SelectedNodes[0];
		if (SingleSelectedNode.IsValid())
		{
			bIsValidNode = (SingleSelectedNode->GetType() != EStatsNodeType::Group);
		}
		else
		{
			NumSelectedNodes = 0;
	...
	MenuBuilder.BeginSection("CounterOptions", LOCTEXT("ContextMenu_Section_CounterOptions", "Counter Options"));
	{
		...
		// Add/remove series to/from graph track
		{
			...
			Action_ToggleCounterInGraphTrack.ExecuteAction = FExecuteAction::CreateSP(this, &SStatsView::ToggleTimingViewMainGraphEventSeries, SingleSelectedNode);

			if (bIsValidNode &&
				IsSeriesInTimingViewMainGraph(SingleSelectedNode))
			{
				MenuBuilder.AddMenuEntry
				(
...
void SStatsView::ApplyFiltering()
{
	...
	for (int32 GroupIndex = 0; GroupIndex < NumGroups; ++GroupIndex)
	{
		...
		{
			check(ChildPtr->Is<FStatsNode>());
			const FStatsNodePtr& NodePtr = StaticCastSharedPtr<FStatsNode>(ChildPtr);

			const bool bIsChildVisible =
				(!bFilterOutZeroCountStats || NodePtr->GetAggregatedStats().Count > 0) &&
	...
	if (TotalTime > 0.1)
	{
		UE_LOGF(LogTimingProfiler, Log, "[Counters] Tree view filtered in %.3fs (%d counters)",
...
{
	OutSearchStrings.Add(FStatsNodePtr->GetDisplayName().ToString());
}
...
void SStatsView::TreeView_OnSelectionChanged(FStatsNodePtr SelectedItem, ESelectInfo::Type SelectInfo)
{
	...
	{
		//FStatsNodePtr SelectedNode = GetSingleSelectedStatsNode();
		//if (SelectedNode.IsValid() && SelectedNode->GetType() != EStatsNodeType::Group)
		//{
		//	FTimingProfilerManager::Get()->SetSelectedCounter(SelectedNode->GetCounterId());
		//}
	}
}
...
FStatsNodePtr SStatsView::GetSingleSelectedStatsNode() const
{
...
void SStatsView::TreeView_OnGetChildren(FStatsNodePtr InParent, TArray<FStatsNodePtr>& OutChildren)
{
	...
	{
		check(Child->Is<FStatsNode>());
		OutChildren.Add(StaticCastSharedPtr<FStatsNode>(Child));
	}
}
...
void SStatsView::CreateGroups()
{
	...
	if (GroupingMode == EStatsGroupingMode::Flat)
	{
		...
		FStatsNodePtr GroupPtr = MakeShared<FStatsNode>(LOCTEXT("GroupName_All", "All"));
	...
	else if (GroupingMode == EStatsGroupingMode::ByMetaGroupName)
	{
		...
		for (const FStatsNodePtr& NodePtr : StatsNodes)
		{
			...
			if (!GroupPtr)
			{
				...
				GroupPtr = GroupNodeSet.Add(GroupName, MakeShared<FStatsNode>(DisplayName));
			}
			GroupPtr->AddChildAndSetParent(NodePtr);
			TreeView->SetItemExpansion(GroupPtr, true);
		}
	...
	else if (GroupingMode == EStatsGroupingMode::ByType)
	{
		...
		for (const FStatsNodePtr& NodePtr : StatsNodes)
		{
			...
			if (!GroupPtr)
			{
				const FText GroupName = StatsNodeTypeHelper::ToText(NodeType);
	...
	else if (GroupingMode == EStatsGroupingMode::ByDataType)
	{
		...
		for (const FStatsNodePtr& NodePtr : StatsNodes)
		{
			...
			if (!GroupPtr)
			{
				const FText GroupName = StatsNodeDataTypeHelper::ToText(DataType);
	...
	if (TotalTime > 0.1)
	{
		UE_LOGF(LogTimingProfiler, Log, "[Counters] Tree view grouping updated in %.3fs (%d counters)",
			TotalTime, StatsNodes.Num());
	}
}
...
void SStatsView::SortTreeNodes()
{
	...
	if (TotalTime > 0.1)
	{
		UE_LOGF(LogTimingProfiler, Log, "[Counters] Tree view sorted (%ls, %c) in %.3fs (%d counters)",
...
void SStatsView::RebuildTree(bool bResync)
{
	...
	if (Session.IsValid())
	{
		...
		if (CounterCount != PreviousNodeCount)
		{
			...
			CountersProvider.EnumerateCounters(
				[this, &CounterIndex, &AddedNodeCount]
				(uint32 CounterId, const TraceServices::ICounter& Counter)
				{
					auto DeduceGroup = [](const TraceServices::ICounter& Counter)
					{
						static const FName MemoryGroup(TEXT("Memory"));
						static const FName BandwidthGroup(TEXT("Bandwidth"));
						static const FName PercentGroup(TEXT("Percent"));
						static const FName MiscFloatGroup(TEXT("Misc_double"));
						static const FName MiscInt64Group(TEXT("Misc_int64"));
						...
						return Counter.IsFloatingPoint() ? MiscFloatGroup : MiscInt64Group;
					};
					...
					{
						++CounterIndex;
						const FText DisplayName = FText::FromName(Counter.GetName());
						const FName Group = Counter.GetGroup() ? Counter.GetGroup() : DeduceGroup(Counter);
						...
						NodePtr = MakeShared<FStatsNode>(CounterId, DisplayName, Group, Type, DataType);
						UpdateNode(NodePtr);
			...
			check(AddedNodeCount == CounterIndex - PreviousNodeCount);
	...
	if (TotalTime > 0.01)
	{
		...
		UE_LOGF(LogTimingProfiler, Log, "[Counters] Tree view rebuilt in %.4fs (sync: %.4fs + update: %.4fs) --> %d counters (%d added)",
			TotalTime, SyncTime, TotalTime - SyncTime, StatsNodes.Num(), AddedNodeCount);
	}
}
...
void SStatsView::ContextMenu_Export_Execute()
{
	...
	TArray<FStatsNodePtr> SelectedStatsNodes = TreeView->GetSelectedItems();
	if (SelectedStatsNodes.Num() == 0)
	{
		return;
	}

	TArray<FBaseTreeNodePtr> SelectedNodes;
	for (FStatsNodePtr Node : SelectedStatsNodes)
	{
	...
	Stopwatch.Stop();
	const double TotalTime = Stopwatch.GetAccumulatedTime();
	UE_LOGF(TraceInsights, Log, "Exported aggregated counter stats to file in %.3fs (\"%ls\").", TotalTime, *Filename);
}
...
{
	FStatsNodePtr SelectedNode = GetSingleSelectedStatsNode();
	return SelectedNode.IsValid() && !SelectedNode->IsGroup();
}
...
void SStatsView::ContextMenu_ExportValues_Execute() const
{
	...
	FStatsNodePtr SelectedNode = GetSingleSelectedStatsNode();
	if (!SelectedNode.IsValid() || SelectedNode->IsGroup())
	{
		return;
	}

	const uint32 CounterId = SelectedNode->GetCounterId();
	FString CounterName = SelectedNode->GetDisplayName().ToString();
...
void SStatsView::SetColorToNode(FStatsNode& StatsNode, FLinearColor CustomColor) const
{
	...
	if (GraphTrack.IsValid())
	{
		TSharedPtr<FTimingGraphSeries> CounterSeries = GraphTrack->GetStatsCounterSeries(CounterId);
...
void SStatsView::SetRandomColorToNode(FStatsNode& StatsNode) const
{
	...
	SetColorToNode(StatsNode, CustomColor);
}
...
TSharedPtr<STimingProfilerWindow> SStatsView::GetProfilerWindow() const
{
...
TSharedPtr<STimingView> SStatsView::GetTimingView() const
{
