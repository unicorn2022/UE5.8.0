...
TSharedPtr<SToolTip> STimersViewTooltip::GetColumnTooltip(const FTableColumn& Column)
{
	...
	FText InstanceDescription = TimersColumn.GetDescription(ETimerAggregationMode::Instance);
	FText GameFrameDescription = TimersColumn.GetDescription(ETimerAggregationMode::GameFrame);
	FText RenderingDescription = TimersColumn.GetDescription(ETimerAggregationMode::RenderingFrame);

	auto GetDescriptionLambda = [InstanceDescription, GameFrameDescription, RenderingDescription]()
	{
		...
		if (Window.IsValid())
		{
			...
			{
				AggregationMode = TimersView->GetAggregationMode();
			}
		}
	...
	TSharedPtr<SToolTip> ColumnTooltip =
		...
		[
			...
			[
				SNew(STextBlock)
				.Text(TAttribute<FText>::CreateLambda(GetDescriptionLambda))
				.TextStyle(FInsightsStyle::Get(), TEXT("TreeTable.Tooltip"))
			]
		];
...
TSharedPtr<SToolTip> STimersViewTooltip::GetColumnTooltipForMode(const FTableColumn& Column, ETimerAggregationMode InAggregationMode)
{
	const FTimersTableColumn& TimersColumn = static_cast<const FTimersTableColumn&>(Column);
			...
			[
				...
				[
					...
					[
						...
						.Text(TimerNodePtr->GetDisplayName())
						.TextStyle(FInsightsStyle::Get(), TEXT("TreeTable.Tooltip"))
					]
