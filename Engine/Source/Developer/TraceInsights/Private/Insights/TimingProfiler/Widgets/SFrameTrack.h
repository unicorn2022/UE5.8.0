...
protected:
	...
	void ContextMenu_ShowFrameSeries_Execute(ETraceFrameType FrameType);
	bool ContextMenu_ShowFrameSeries_CanExecute(ETraceFrameType FrameType) const;
	bool ContextMenu_ShowFrameSeries_IsChecked(ETraceFrameType FrameType) const;

	void ContextMenu_ShowFrameStats_Execute(ETraceFrameType FrameType, uint32 TimerId);
	bool ContextMenu_ShowFrameStats_CanExecute(ETraceFrameType FrameType, uint32 TimerId) const;
	bool ContextMenu_ShowFrameStats_IsChecked(ETraceFrameType FrameType, uint32 TimerId) const;

	void ContextMenu_AutoZoom_Execute();
	bool ContextMenu_AutoZoom_CanExecute() const;
	bool ContextMenu_AutoZoom_IsChecked() const;
	void AutoZoom();

	void ContextMenu_ZoomTimingViewOnFrameSelection_Execute();
	bool ContextMenu_ZoomTimingViewOnFrameSelection_CanExecute() const;
	bool ContextMenu_ZoomTimingViewOnFrameSelection_IsChecked() const;

	void ContextMenu_ScrollLogView_Execute();
	bool ContextMenu_ScrollLogView_CanExecute() const;
...
protected:
	...
	/** Weak reference to the user annotation store, for drawing indicators. */
	TWeakPtr<FUserAnnotationStore> AnnotationStoreWeak;

	/** Handle for the annotations-extender "store available" subscription. Unsubscribed in the
	 *  destructor so we don't fire into a dead widget if the module outlives the tab. */
	FDelegateHandle OnAnnotationStoreChangedHandle;

	/** Refreshes AnnotationStoreWeak from the current session's store. Invoked on subscribe
	 *  (catches the already-active session) and on the extender's broadcast (catches later
	 *  session begin or store-sharing swaps). */
	void RefreshAnnotationStore();
