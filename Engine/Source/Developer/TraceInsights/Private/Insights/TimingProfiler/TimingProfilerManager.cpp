...
FTimingProfilerManager::FTimingProfilerManager(TSharedRef<FUICommandList> InCommandList)
	...
	, MajorTabId(FInsightsManagerTabs::TimingProfilerTabId)
	, LogListingName("TimingInsights")
{
}
...
void FTimingProfilerManager::Initialize(IUnrealInsightsModule& InsightsModule)
{
	...
	UE_LOGF(LogTimingProfiler, Log, "Initialize");
	...
	EventDepthLimitMin = Settings.GetTimingTrackDepthLimitMin();
	EventDepthLimitMax = Settings.GetTimingTrackDepthLimitMax();
...
void FTimingProfilerManager::Shutdown()
{
	...
	UE_LOGF(LogTimingProfiler, Log, "Shutdown");
}
...
{
	ActionManager.Map_ToggleFramesTrackVisibility();
	ActionManager.Map_ToggleTimingViewVisibility();
	ActionManager.Map_ToggleTimersViewVisibility();
	ActionManager.Map_ToggleCallersTreeViewVisibility();
	ActionManager.Map_ToggleCalleesTreeViewVisibility();
	ActionManager.Map_ToggleStatsCountersViewVisibility();
	ActionManager.Map_ToggleLogViewVisibility();
	ActionManager.Map_ToggleModulesViewVisibility();
	ActionManager.Map_ToggleUserAnnotationsViewVisibility();
}
	...
	if (!bIsTabRegistered && bShouldRegister)
	{
		...
		if (Config.bIsAvailable)
		{
			...
			FTabSpawnerEntry& TabSpawnerEntry = FGlobalTabmanager::Get()->RegisterNomadTabSpawner(MajorTabId,
					...
					FCanSpawnTab::CreateRaw(this, &FTimingProfilerManager::CanSpawnTab))
...
bool FTimingProfilerManager::Tick(float DeltaTime)
{
	...
	if (Session.IsValid())
	{
		...
		if (!bIsAvailable && AvailabilityCheck.Tick())
		{
			...
			{
				UE_LOGF(LogTimingProfiler, Log, "Opening the \"Timing Insights\" tab...");
...
void FTimingProfilerManager::ActivateMajorTab()
{
	// Ensure Timing Insights / Timing View is the active tab / view.
	TSharedPtr<SDockTab> MajorTab = FGlobalTabmanager::Get()->FindExistingLiveTab(MajorTabId);
	if (MajorTab.IsValid())
	{
		MajorTab->ActivateInParent(ETabActivationCause::SetDirectly);
		ActivateView();
	}
}
...
void FTimingProfilerManager::ActivateView()
{
	// Ensure Timing View is the active view.
	TSharedPtr<STimingProfilerWindow> Wnd = GetProfilerWindow();
	if (Wnd)
	{
		TSharedPtr<FTabManager> TabManager = Wnd->GetTabManager();
		TSharedPtr<SDockTab> MinorTab = TabManager->FindExistingLiveTab(FTimingProfilerTabs::TimingViewID);
		if (MinorTab.IsValid())
		{
			MinorTab->ActivateInParent(ETabActivationCause::SetDirectly);
			FSlateApplication::Get().SetKeyboardFocus(MinorTab->GetContent());
		}
	}
}
...
{
	UE_LOGF(LogTimingProfiler, Log, "OnSessionChanged");
...
void FTimingProfilerManager::ShowHideTimingView(const bool bIsVisible)
{
	...
	if (Wnd)
	{
		Wnd->ShowHideTab(FTimingProfilerTabs::TimingViewID, bIsVisible);
	}
}
...
void FTimingProfilerManager::ShowHideTimersView(const bool bIsVisible)
{
	...
	if (Wnd)
	{
		Wnd->ShowHideTab(FTimingProfilerTabs::TimersID, bIsVisible);
...
void FTimingProfilerManager::ShowHideCallersTreeView(const bool bIsVisible)
{
	...
	if (Wnd)
	{
		Wnd->ShowHideTab(FTimingProfilerTabs::CallersID, bIsVisible);
...
void FTimingProfilerManager::ShowHideCalleesTreeView(const bool bIsVisible)
{
	...
	if (Wnd)
	{
		Wnd->ShowHideTab(FTimingProfilerTabs::CalleesID, bIsVisible);
...
void FTimingProfilerManager::ShowHideStatsCountersView(const bool bIsVisible)
{
	...
	if (Wnd)
	{
		Wnd->ShowHideTab(FTimingProfilerTabs::StatsCountersID, bIsVisible);
...
void FTimingProfilerManager::ShowHideLogView(const bool bIsVisible)
{
	...
	if (Wnd)
	{
		Wnd->ShowHideTab(FTimingProfilerTabs::LogViewID, bIsVisible);
	}
}
...
void FTimingProfilerManager::ShowHideModulesView(const bool bIsVisible)
{
	...
	if (Wnd)
	{
		Wnd->ShowHideTab(FTimingProfilerTabs::ModulesViewID, bIsVisible);
	}
}

////////////////////////////////////////////////////////////////////////////////////////////////////

void FTimingProfilerManager::ShowHideUserAnnotationsView(const bool bIsVisible)
{
	bIsUserAnnotationsViewVisible = bIsVisible;

	TSharedPtr<STimingProfilerWindow> Wnd = GetProfilerWindow();
	if (Wnd)
	{
		Wnd->ShowHideTab(FTimingProfilerTabs::UserAnnotationsID, bIsVisible);
	}
}
	...
	if (FParse::Command(&Cmd, TEXT("TimingInsights.ExportTimers")))
	{
		...
		while (Cmd && Cmd[0] != TEXT('\0'))
		{
			...
			if (FParse::Token(Cmd, Token, bUseEscape))
			{
				...
				static constexpr TCHAR ShowFormatInTimerNamesToken[] = TEXT("-ShowFormatInTimerNames=");
				if (Token.StartsWith(ShowFormatInTimerNamesToken))
				{
					Token.RightChopInline(UE_ARRAY_COUNT(ShowFormatInTimerNamesToken) - 1);
					Token.TrimQuotesInline();
					Params.bShowFormatInTimerNames = Token.ToBool();
				}
			}
		}
	...
	if (FParse::Command(&Cmd, TEXT("TimingInsights.ExportTimerCallees")))
	{
		...
		while (Cmd && Cmd[0] != TEXT('\0'))
		{
			...
			if (FParse::Token(Cmd, Token, bUseEscape))
			{
				...
				else if (Token.StartsWith(ShowFormatInTimerNamesToken))
				{
					// Default: -ShowFormatInTimerNames=
					// Example: -ShowFormatInTimerNames="False"
					Token.RightChopInline(UE_ARRAY_COUNT(ShowFormatInTimerNamesToken) - 1);
					Token.TrimQuotesInline();
					Params.bShowFormatInTimerNames = Token.ToBool();
				}
...
void FTimingProfilerManager::SetEventDepthLimitMin(uint32 InEventDepthLimitMin)
{
	EventDepthLimitMin = FMath::Min<uint32>(InEventDepthLimitMin, MaxEventDepthLimitMin);

	// Persistent option. Save it to the config file.
	FInsightsSettings& Settings = FInsightsManager::Get()->GetSettings();
	Settings.SetAndSaveTimingTrackDepthLimitMin(static_cast<int32>(EventDepthLimitMin));
}

////////////////////////////////////////////////////////////////////////////////////////////////////

void FTimingProfilerManager::SetEventDepthLimitMax(uint32 InEventDepthLimitMax)
{
	EventDepthLimitMax = FMath::Clamp<uint32>(InEventDepthLimitMax, 1u, UnlimitedEventDepth);

	// Persistent option. Save it to the config file.
	FInsightsSettings& Settings = FInsightsManager::Get()->GetSettings();
	Settings.SetAndSaveTimingTrackDepthLimitMax(static_cast<int32>(EventDepthLimitMax));
}

////////////////////////////////////////////////////////////////////////////////////////////////////

FUserAnnotationsTimingViewExtender* FTimingProfilerManager::GetUserAnnotationsExtender()
{
	if (!CachedUserAnnotationsExtender)
	{
		...
		if (Module)
		{
			CachedUserAnnotationsExtender = &Module->GetUserAnnotationsExtender();
		}
	}
	return CachedUserAnnotationsExtender;
}
