...
const FName FTimersViewColumns::ParentPercentageInclusiveTimeColumnID(TEXT("ParentPercentageInclTime"));
const FName FTimersViewColumns::RootPercentageInclusiveTimeColumnID(TEXT("RootPercentageInclTime"));
...
const FName FTimersViewColumns::PercentageExclusiveInclusiveTimeColumnID(TEXT("PercentageExclInclTime"));
...
void FTimersViewColumnFactory::CreateTimersViewColumns(TArray<TSharedRef<FTableColumn>>& Columns)
{
	...
	Columns.Add(CreatePercentageExclusiveInclusiveTimeColumn());
...
void FTimersViewColumnFactory::CreateCallersViewColumns(TArray<TSharedRef<FTableColumn>>& Columns)
{
	Columns.Reset();
	...
	Columns.Add(CreateTotalInclusiveTimeColumn());
	Columns.Add(CreateParentPercentageInclusiveTimeColumn());
	Columns.Add(CreateRootPercentageInclusiveTimeColumn());
	Columns.Add(CreateAverageInclusiveTimeColumn());
	Columns.Add(CreateTotalExclusiveTimeColumn());
	Columns.Add(CreatePercentageExclusiveInclusiveTimeColumn());
	Columns.Add(CreateAverageExclusiveTimeColumn());
...
void FTimersViewColumnFactory::CreateCalleesViewColumns(TArray<TSharedRef<FTableColumn>>& Columns)
{
	Columns.Reset();
...
TSharedRef<FTableColumn> FTimersViewColumnFactory::CreateNameColumn()
{
	...
	Column.SetDescription(ETimerAggregationMode::Instance, Description);
	Column.SetDescription(ETimerAggregationMode::GameFrame, Description);
	Column.SetDescription(ETimerAggregationMode::RenderingFrame, Description);
...
TSharedRef<FTableColumn> FTimersViewColumnFactory::CreateAverageInstanceCountColumn()
{
	...
	Column.SetDescription(ETimerAggregationMode::Instance,
	...
	Column.SetDescription(ETimerAggregationMode::GameFrame,
	...
	Column.SetDescription(ETimerAggregationMode::RenderingFrame,
...
TSharedRef<FTableColumn> FTimersViewColumnFactory::CreateParentPercentageInclusiveTimeColumn()
{
	TSharedRef<FTimersTableColumn> ColumnRef = MakeShared<FTimersTableColumn>(FTimersViewColumns::ParentPercentageInclusiveTimeColumnID);
	FTimersTableColumn& Column = *ColumnRef;

	Column.SetShortName(LOCTEXT("ParentPercentageInclusiveTime_ColumnName", "% Parent"));
	Column.SetTitleName(LOCTEXT("ParentPercentageInclusiveTime_ColumnTitle", "Total Inclusive Time / Parent's Total Inclusive Time (%)"));

	FText Description = LOCTEXT("ParentPercentageInclusiveTime_ColumnDesc", "Percentage of total inclusive duration (the selected timer relative to the parent timer)");
	Column.SetDescription(ETimerAggregationMode::Instance, Description);
	Column.SetDescription(ETimerAggregationMode::GameFrame, Description);
	Column.SetDescription(ETimerAggregationMode::RenderingFrame, Description);
	...
	Column.SetHorizontalAlignment(HAlign_Right);
	Column.SetInitialWidth(PercentageColumnInitialWidth);

	Column.SetDataType(ETableCellDataType::Double);
	Column.SetAggregation(ETableColumnAggregation::Sum);

	class FParentPercentageInclusiveTimeValueGetter : public FTableCellValueGetter
	{
	public:
		virtual const TOptional<FTableCellValue> GetValue(const FTableColumn& Column, const FBaseTreeNode& Node) const
		{
			ensure(Column.GetId() == FTimersViewColumns::ParentPercentageInclusiveTimeColumnID);
			if (Node.Is<FTimerNode>())
			{
				const FTimerNode& TimerNode = Node.As<FTimerNode>();
				FBaseTreeNodePtr ParentNode = TimerNode.GetParent();
				if (ParentNode.IsValid() && ParentNode->Is<FTimerNode>())
				{
					const double ParentTime = ParentNode->As<FTimerNode>().GetAggregatedStats().TotalInclusiveTime;
					if (ParentTime > 0.0)
					{
						const double Time = TimerNode.GetAggregatedStats().TotalInclusiveTime;
						return TOptional<FTableCellValue>(FTableCellValue(Time / ParentTime));
					}
				}
			}
			return TOptional<FTableCellValue>();
	...
	TSharedRef<ITableCellValueGetter> Getter = MakeShared<FParentPercentageInclusiveTimeValueGetter>();
	Column.SetValueGetter(Getter);

	TSharedRef<ITableCellValueFormatter> Formatter = MakeShared<FDoubleValueFormatterAsPercent>();
	Column.SetValueFormatter(Formatter);
...
TSharedRef<FTableColumn> FTimersViewColumnFactory::CreateRootPercentageInclusiveTimeColumn()
{
	TSharedRef<FTimersTableColumn> ColumnRef = MakeShared<FTimersTableColumn>(FTimersViewColumns::RootPercentageInclusiveTimeColumnID);
	FTimersTableColumn& Column = *ColumnRef;

	Column.SetShortName(LOCTEXT("RootPercentageInclusiveTime_ColumnName", "% Root"));
	Column.SetTitleName(LOCTEXT("RootPercentageInclusiveTime_ColumnTitle", "Total Inclusive Time / Root's Total Inclusive Time (%)"));

	FText Description = LOCTEXT("RootPercentageInclusiveTime_ColumnDesc", "Percentage of total inclusive duration (the selected timer relative to the root timer)");
	Column.SetDescription(ETimerAggregationMode::Instance, Description);
	Column.SetDescription(ETimerAggregationMode::GameFrame, Description);
	Column.SetDescription(ETimerAggregationMode::RenderingFrame, Description);
	...
	class FRootPercentageInclusiveTimeValueGetter : public FTableCellValueGetter
	{
	public:
		virtual const TOptional<FTableCellValue> GetValue(const FTableColumn& Column, const FBaseTreeNode& Node) const
		{
			ensure(Column.GetId() == FTimersViewColumns::RootPercentageInclusiveTimeColumnID);
			if (Node.Is<FTimerNode>())
			{
				const FTimerNode& TimerNode = Node.As<FTimerNode>();
				FBaseTreeNodePtr RootNode = TimerNode.GetParent();
				while (RootNode.IsValid() && RootNode->GetParent().IsValid())
				{
					RootNode = RootNode->GetParent();
				}
				if (RootNode.IsValid() && RootNode->Is<FTimerNode>())
				{
					const double RootTime = RootNode->As<FTimerNode>().GetAggregatedStats().TotalInclusiveTime;
					if (RootTime > 0.0)
					{
						const double Time = TimerNode.GetAggregatedStats().TotalInclusiveTime;
						return TOptional<FTableCellValue>(FTableCellValue(Time / RootTime));
					}
				}
			}
			return TOptional<FTableCellValue>();
	...
	TSharedRef<ITableCellValueGetter> Getter = MakeShared<FRootPercentageInclusiveTimeValueGetter>();
	Column.SetValueGetter(Getter);
...
TSharedRef<FTableColumn> FTimersViewColumnFactory::CreatePercentageExclusiveInclusiveTimeColumn()
{
	TSharedRef<FTimersTableColumn> ColumnRef = MakeShared<FTimersTableColumn>(FTimersViewColumns::PercentageExclusiveInclusiveTimeColumnID);
	FTimersTableColumn& Column = *ColumnRef;

	Column.SetShortName(LOCTEXT("PercentageExclusiveInclusiveTime_ColumnName", "Excl/Incl"));
	Column.SetTitleName(LOCTEXT("PercentageExclusiveInclusiveTime_ColumnTitle", "Total Exclusive Time / Total Inclusive Time (%)"));

	FText Description = LOCTEXT("PercentageExclusiveInclusiveTime_ColumnDesc", "Percentage of total exclusive duration from the total inclusive duration");
	Column.SetDescription(ETimerAggregationMode::Instance, Description);
	Column.SetDescription(ETimerAggregationMode::GameFrame, Description);
	Column.SetDescription(ETimerAggregationMode::RenderingFrame, Description);
	...
	class FPercentageExclusiveInclusiveTimeValueGetter : public FTableCellValueGetter
	{
	public:
		virtual const TOptional<FTableCellValue> GetValue(const FTableColumn& Column, const FBaseTreeNode& Node) const
		{
			ensure(Column.GetId() == FTimersViewColumns::PercentageExclusiveInclusiveTimeColumnID);
			if (Node.Is<FTimerNode>())
			{
				const FTimerNode& TimerNode = Node.As<FTimerNode>();
				const double InclTime = TimerNode.GetAggregatedStats().TotalInclusiveTime;
				if (InclTime > 0.0)
				{
					const double ExclTime = TimerNode.GetAggregatedStats().TotalExclusiveTime;
					return TOptional<FTableCellValue>(FTableCellValue(ExclTime / InclTime));
				}
			}
			return TOptional<FTableCellValue>();
		}
	...
	TSharedRef<ITableCellValueGetter> Getter = MakeShared<FPercentageExclusiveInclusiveTimeValueGetter>();
	Column.SetValueGetter(Getter);
