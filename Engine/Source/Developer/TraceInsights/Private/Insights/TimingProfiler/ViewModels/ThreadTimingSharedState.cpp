...
void FThreadTimingSharedState::Tick(Timing::ITimingViewSession& InSession, const TraceServices::IAnalysisSession& InAnalysisSession)
{
	...
	if (TimingProfilerProvider)
	{
			...
			CurrentLoadTimeProfilerTimelineCount != LoadTimeProfilerTimelineCount)
		{
			...
			TimingProfilerProvider->EnumerateGpuQueues(
				[this, bShowAllGpuTracks , &InSession, &bTracksOrderChanged, &GpuTrackOrder]
	...
	const TraceServices::IStackSamplesProvider* StackSamplesProvider = TraceServices::ReadStackSamplesProvider(InAnalysisSession);
...
TSharedPtr<const ITimingEvent> FThreadTimingSharedState::FindMaxEventInstance(uint32 TimerId, double StartTime, double EndTime, double MaxValidTime)
{
	auto CompareAndAssignEvent =
		[](TSharedPtr<const ITimingEvent>& TimingEvent, TSharedPtr<const ITimingEvent>& TrackEvent)
		{
	...
	for (const auto& KV : CpuTracks)
	{
		...
		if (Track.IsVisible())
		{
			TrackEvent = Track.FindMaxEventInstance(TimerId, StartTime, EndTime, MaxValidTime);
	...
	if (OldGpu1Track.IsValid() && OldGpu1Track->IsVisible())
	{
		TrackEvent = OldGpu1Track->FindMaxEventInstance(TimerId, StartTime, EndTime, MaxValidTime);
		CompareAndAssignEvent(TimingEvent, TrackEvent);
	}

	if (OldGpu2Track.IsValid() && OldGpu2Track->IsVisible())
	{
		TrackEvent = OldGpu2Track->FindMaxEventInstance(TimerId, StartTime, EndTime, MaxValidTime);
		CompareAndAssignEvent(TimingEvent, TrackEvent);
	}
...
TSharedPtr<const ITimingEvent> FThreadTimingSharedState::FindMinEventInstance(uint32 TimerId, double StartTime, double EndTime, double MaxValidTime)
{
	auto CompareAndAssignEvent =
		[](TSharedPtr<const ITimingEvent>& TimingEvent, TSharedPtr<const ITimingEvent>& TrackEvent)
		{
	...
	for (const auto& KV : CpuTracks)
	{
		...
		if (Track.IsVisible())
		{
			TrackEvent = Track.FindMinEventInstance(TimerId, StartTime, EndTime, MaxValidTime);
			CompareAndAssignEvent(TimingEvent, TrackEvent);
		}
	}
	...
	if (OldGpu1Track.IsValid() && OldGpu1Track->IsVisible())
	{
		TrackEvent = OldGpu1Track->FindMinEventInstance(TimerId, StartTime, EndTime, MaxValidTime);
		CompareAndAssignEvent(TimingEvent, TrackEvent);
	}

	if (OldGpu2Track.IsValid() && OldGpu2Track->IsVisible())
	{
		TrackEvent = OldGpu2Track->FindMinEventInstance(TimerId, StartTime, EndTime, MaxValidTime);
		CompareAndAssignEvent(TimingEvent, TrackEvent);
	}
...
void FThreadTimingSharedState::AddGpuWorkChildTracks()
{
	...
	if (TimingProfilerProvider)
	{
		...
		TimingProfilerProvider->EnumerateGpuQueues(
			[this](const TraceServices::FGpuQueueInfo& QueueInfo)
			{
				// Check if there is an available GPU track for this queue.
				TSharedPtr<FGpuQueueTimingTrack>* TrackPtrPtr = GpuTracks.Find(QueueInfo.Id);
				if (TrackPtrPtr != nullptr)
...
void FThreadTimingSharedState::Command_ShowGpuFencesRelations_Execute()
{
	...
	{
		TimingView->EditCurrentRelations().RemoveAll(
			[](TUniquePtr<ITimingEventRelation>& Item)
			{
...
void FThreadTimingSharedState::OnTimingEventSelected(TSharedPtr<const ITimingEvent> InSelectedEvent)
{
	...
	auto AddFenceRelation =
		[this, TimingProfilerProvider]
		(const FGpuSignalFence& SignalFence, const FGpuWaitFence& WaitFence, uint32 WaitFenceQueueId)
		{
			uint32 Index = 0;
			...
			int32 SourceDepth = 0;
			TimingProfilerProvider->ReadTimeline(Index,
				[SignalFence, &SourceDepth]
	...
	TimingProfilerProvider->EnumerateResolvedGpuFences(QueueId, InSelectedEvent->GetStartTime(), InSelectedEvent->GetEndTime(),
		[AddFenceRelation, TimingProfilerProvider, QueueId]
		(uint32 SignalFenceQueueId, const FGpuSignalFence& SignalFence, uint32 WaitFenceQueueId, const FGpuWaitFence& WaitFence)
		{
