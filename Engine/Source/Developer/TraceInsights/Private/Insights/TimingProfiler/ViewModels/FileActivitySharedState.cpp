...
void FFileActivitySharedState::Tick(Timing::ITimingViewSession& InSession, const TraceServices::IAnalysisSession& InAnalysisSession)
{
	...
	if (bForceIoEventsUpdate)
	{
		...
		Stopwatch.Stop();
		UE_LOGF(LogTimingProfiler, Log, "[IO] Enumerated %ls events (%ls file activities) in %ls.",
		...
		Stopwatch.Stop();
		UE_LOGF(LogTimingProfiler, Log, "[IO] Sorted file activities and events in %ls.", *FormatTimeAuto(Stopwatch.GetAccumulatedTime()));
		...
		if (FileActivities.Num() > 0)
		{
			...
			Stopwatch.Stop();
			UE_LOGF(LogTimingProfiler, Log, "[IO] Computed layout for file activities in %ls.", *FormatTimeAuto(Stopwatch.GetAccumulatedTime()));
			...
			Stopwatch.Stop();
			UE_LOGF(LogTimingProfiler, Log, "[IO] Updated depth for events in %ls.", *FormatTimeAuto(Stopwatch.GetAccumulatedTime()));
		}
	}
}
