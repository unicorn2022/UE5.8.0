...
void FStatsAggregator::Start()
{
	if (IntervalStartTime < IntervalEndTime)
	{
		...
		UE_LOGF(LogTimingProfiler, Log, "[%ls] Request async aggregation (op %d) [%fs to %fs] (%fs)...",
...
void FStatsAggregator::Cancel()
{
	if (AsyncTask)
	{
		UE_LOGF(LogTimingProfiler, Log, "[%ls] Cancel requested for async aggregation (op %d)...", *LogName, OperationCount);
		CancellationToken->Cancel();
	}
}
							...
							TFunctionRef<void()> OnFinishedCallback)
{
	if (AsyncTask && AsyncTask->IsDone())
	{
		...
		UE_LOGF(LogTimingProfiler, Log, "[%ls] Aggregated stats computed in %.4fs (%.4fs + %.4fs)%ls%ls - total: %.4fs (%d ops)",
			*LogName,
	...
	if (bIsStartRequested)
	{
		if (AsyncTask)
		{
			if (!CancellationToken->ShouldCancel())
			{
				...
				UE_LOGF(LogTimingProfiler, Log, "[%ls] Cancel previous async aggregation (op %d)...", *LogName, OperationCount);
				CancellationToken->Cancel();
			}
		}
		else
		{
			...
			UE_LOGF(LogTimingProfiler, Log, "[%ls] Start async aggregation (op %d)...", *LogName, OperationCount);
