...
const FName FMemoryProfilerTabs::UserAnnotationsID(TEXT("UserAnnotations"));
...
BEGIN_SLATE_FUNCTION_BUILD_OPTIMIZATION
TSharedPtr<SMemAllocTableTreeView> SMemoryProfilerWindow::OpenNewMemAllocTableTreeViewTab(const FText& InTabDisplayName)
{
	if (!GetTabManager().IsValid())
	{
		return nullptr;
	...
	TabManagerPtr->RegisterTabSpawner(TabId, FOnSpawnTab::CreateRaw(this, &SMemoryProfilerWindow::SpawnTab_MemAllocTableTreeView, LastMemAllocTableTreeViewIndex))
		.SetDisplayName(InTabDisplayName)
		.SetIcon(FSlateIcon(FInsightsStyle::GetStyleSetName(), "Icons.MemAllocTableTreeView"))
		.SetGroup(Group);
	...
	return StaticCastSharedRef<SMemAllocTableTreeView>(Tab->GetContent());
}
END_SLATE_FUNCTION_BUILD_OPTIMIZATION
...
BEGIN_SLATE_FUNCTION_BUILD_OPTIMIZATION
TSharedRef<SDockTab> SMemoryProfilerWindow::SpawnTab_UserAnnotations(const FSpawnTabArgs& Args)
{
	const TSharedRef<SDockTab> DockTab = SNew(SDockTab)
		.ShouldAutosize(false)
		.TabRole(ETabRole::PanelTab)
	...
	DockTab->SetOnTabClosed(SDockTab::FOnTabClosedCallback::CreateRaw(this, &SMemoryProfilerWindow::OnUserAnnotationsTabClosed));
	AddOpenTab(DockTab);
...
void SMemoryProfilerWindow::OnUserAnnotationsTabClosed(TSharedRef<SDockTab> TabBeingClosed)
{
...
void SMemoryProfilerWindow::RegisterTabSpawners()
{
	...
	if (Config.ShouldRegisterMinorTab(FMemoryProfilerTabs::UserAnnotationsID))
	{
		TabManagerPtr->RegisterTabSpawner(FMemoryProfilerTabs::UserAnnotationsID, FOnSpawnTab::CreateRaw(this, &SMemoryProfilerWindow::SpawnTab_UserAnnotations))
...
{
	return FTabManager::NewLayout("InsightsMemoryProfilerLayout_v1.4")
		->AddArea
		(
			...
			(
				...
				->AddTab(FMemoryProfilerTabs::UserAnnotationsID, ETabState::OpenedTab)
