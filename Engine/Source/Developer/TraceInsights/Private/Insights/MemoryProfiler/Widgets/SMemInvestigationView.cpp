...
FReply SMemInvestigationView::RunQuery()
{
	...
	if (!ProfilerWindow.IsValid())
	{
		UE_LOGF(LogMemoryProfiler, Error, "[MemQuery] Invalid Profiler Window!");
	...
	if (!Rule)
	{
		UE_LOGF(LogMemoryProfiler, Error, "[MemQuery] Invalid Rule!");
		return FReply::Handled();
	}
	...
	{
		UE_LOGF(LogMemoryProfiler, Error, "[MemQuery] Only %d time markers available. Current rule (%ls) requires %u time markers!",
	...
	UE_LOGF(LogMemoryProfiler, Log, "[MemQuery] Run Query %ls%ls...", *Rule->GetShortName().ToString(), Builder.ToString());
#endif
