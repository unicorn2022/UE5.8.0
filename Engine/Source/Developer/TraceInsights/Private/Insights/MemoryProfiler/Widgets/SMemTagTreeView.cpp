...
void SMemTagTreeView::RebuildTree(bool bResync)
{
	...
	if (TotalTime > 0.01)
	{
		...
		UE_LOGF(LogMemoryProfiler, Log, "[MemTags] Tree view rebuilt in %.4fs (sync: %.4fs + update: %.4fs) --> %d tags (%d added)",
...
void SMemTagTreeView::UpdateStats()
{
	...
	if (TotalTime > 0.01)
	{
		UE_LOGF(LogMemoryProfiler, Log, "[MemTags] Aggregated stats updated in %.4fs", TotalTime);
	}
}
		...
		if (Section != nullptr)
		{
			...
			for (FConfigSection::TConstKeyIterator It(*Section, KeyName); It; ++It)
			{
				...
				if (FPaths::FileExists(FullPath))
				{
					...
					UE_LOGF(LogMemoryProfiler, Log, "[MemTags] Budget file found (Label=\"%ls\", Path=\"%ls\")", *Label, *FullPath);
				}
				else
				{
					UE_LOGF(LogMemoryProfiler, Warning, "[MemTags] Budget file not found (Label=\"%ls\", Path=\"%ls\")", *Label, *FullPath);
				}
			}
		}
		else
		{
			UE_LOGF(LogMemoryProfiler, Log, "[MemTags] Couldn't find Insights.MemoryProfiler config section");
		}
	}
	else
	{
		UE_LOGF(LogMemoryProfiler, Log, "[MemTags] Couldn't find UnrealInsightsSettings config");
	}
...
void SMemTagTreeView::ApplyBudgetToNodes()
{
	...
	if (TotalTime > 0.01)
	{
		UE_LOGF(LogMemoryProfiler, Log, "[MemTags] Budgets updated in %.4fs", TotalTime);
	}
}
