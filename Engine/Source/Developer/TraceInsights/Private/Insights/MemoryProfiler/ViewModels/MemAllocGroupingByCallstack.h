...
public:
	...
	explicit FCallstackFrameGroupNode(TWeakPtr<FTable> InParentTable, const TraceServices::FStackFrame* InStackFrame);

	virtual ~FCallstackFrameGroupNode() = default;
...
private:
	struct FCallstackGroup
	{
		...
		TMap<uint64, FCallstackGroup*> GroupMap; // Callstack Frame Address or Group Name Id -> FCallstackGroup*
	};
...
public:
	...
	static FText GetDisplayName(const TraceServices::FStackFrame* Frame);
...
private:
	uint64 GetGroupNameId(const TraceServices::FStackFrame* Frame) const;

	FCallstackGroup* CreateGroup(TArray<FCallstackGroup*>& InOutAllCallstackGroup, FCallstackGroup* InParentGroup, uint64 InGroupNameId, TWeakPtr<FTable> InParentTable, const TraceServices::FStackFrame* InFrame) const;
