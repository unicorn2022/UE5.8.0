...
public:
	...
	virtual ~FTraceInsightsModule() = default;
	...
	virtual void StartAnalysisWithSecureTrace(FStringView Host, uint16 Port, TConstArrayView<uint8> AuthorizationToken, FStringView CertificateFile = FStringView(), TConstArrayView<uint8> CertificateAuthority = TConstArrayView<uint8>(), FAnsiStringView Identity = FAnsiStringView()) override;
	...
	virtual void SetAssetInfoProvider(TSharedPtr<UE::Insights::ObjectProfiler::IAssetInfoProvider> InProvider) override;
	virtual TSharedPtr<UE::Insights::ObjectProfiler::IAssetInfoProvider> GetAssetInfoProvider() const override;

	virtual void OpenMemoryAllocationsView(double InTimeA, const FText& InTabDisplayName, const TSet<FString>& InPackageFilter) override;

	/** Returns the user annotations extender (for accessing annotation stores). */
	UE::Insights::TimingProfiler::FUserAnnotationsTimingViewExtender& GetUserAnnotationsExtender() { return UserAnnotationsExtender; }
...
private:
	...
	TSharedPtr<UE::Insights::ObjectProfiler::IAssetInfoProvider> AssetInfoProvider;

	TSharedRef<FVersionWidget> VersionWidgetVM;

	/** User Annotations extender registered as a modular feature. */
	UE::Insights::TimingProfiler::FUserAnnotationsTimingViewExtender UserAnnotationsExtender;
};
