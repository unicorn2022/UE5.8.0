	...
	if (bResync || NewQueryStartTime != QueryStartTime || NewQueryEndTime != QueryEndTime)
	{
		...
		if (QueryStartTime < QueryEndTime && Session.IsValid())
		{
			...
			if (TasksProvider)
			{
				...
				TasksProvider->EnumerateTasks(QueryStartTime, QueryEndTime, SelectedTasksSelectionOption,
					[&Tasks, &TaskTable, Nodes]
					(const TraceServices::FTaskInfo& TaskInfo)
					{
						Tasks.Emplace(TaskInfo);
						int32 Index = Tasks.Num() - 1;
						FTaskNodePtr NodePtr = MakeShared<FTaskNode>(TaskTable, Index);
						Nodes->Add(NodePtr);
