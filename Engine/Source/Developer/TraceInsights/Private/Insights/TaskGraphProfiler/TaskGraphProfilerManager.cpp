	...
	if (!bIsAvailable && AvailabilityCheck.Tick())
	{
		...
		if (Session.IsValid())
		{
			...
			if (TasksProvider && TasksProvider->GetNumTasks() > 0 && TabManagerShared.IsValid())
			{
				...
				TabManagerShared->TryInvokeTab(FTaskGraphProfilerTabs::TaskTableTreeViewTabID, true);
			}
	...
	if (Session.IsValid())
	{
		...
		if (TimingProfilerProvider)
		{
			...
			TimingProfilerProvider->ReadTimeline(Track->GetTimelineIndex(),
				[TaskStartedTime, TaskFinishedTime, &Depth]
				(const TraceServices::ITimingProfilerProvider::Timeline& Timeline)
				{
					Timeline.EnumerateEventsDownSampled(TaskStartedTime, TaskFinishedTime, 0,
						[TaskStartedTime, &Depth]
						(bool IsEnter, double Time, const TraceServices::FTimingProfilerEvent& Event)
						{
