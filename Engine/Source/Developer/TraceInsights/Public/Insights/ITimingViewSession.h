...
public:
	...
	/** Get the active relations for this view. */
	virtual const TArray<TUniquePtr<ITimingEventRelation>>& GetCurrentRelations() const = 0;

	/** Get an editable array of relations for this view. */
	virtual TArray<TUniquePtr<ITimingEventRelation>>& EditCurrentRelations() = 0;

	/** Add a new relation to this view. */
	virtual void AddRelation(TUniquePtr<ITimingEventRelation>& Relation) = 0;

	/** Remove all relations from this view. */
	virtual void ClearRelations() = 0;
};
