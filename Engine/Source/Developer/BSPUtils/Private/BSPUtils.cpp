...
//	UE_LOGF(LogEditorBsp, Log,  "Found %i coplanar sets in %i", n, Model->Polys->Element.Num() );
	Mark.Pop();
...
// 	UE_LOGF(LogEditorBsp, Log,  "BspMergeCoplanars reduced %i->%i", OriginalNum, Model->Polys->Element.Num() );
	Mark2.Pop();
}
...
void CleanupNodes( UModel *Model, int32 iNode, int32 iParent )
{
	...
	if( Node->NumVertices==0 && Node->iPlane!=INDEX_NONE )
	{
		...
		else
		{
			...
			else UE_LOGF(LogBspUtils, Fatal, "CleanupNodes: Parent and child are unlinked");
		}
	}
...
)
{
	...
	else if( CoplanarInfo.ProcessingBack )
	{
		...
		{
			UE_LOGF(LogBspUtils, Fatal, "FilterLeaf: Bad Locs");
			return;
		}
...
)
{
	...
	else if( SplitResult == SP_Coplanar )
	{
		if( CoplanarInfo.iOriginalNode != INDEX_NONE )
		{
			...
			FBSPOps::GErrors++;
// 			UE_LOGF(LogEditorBsp, Warning, "FilterEdPoly: Encountered out-of-place coplanar" );
			goto Front;
		}
...
//		UE_LOGF(LogEditorBsp, Warning, "Node side limit reached" );
		return;
	}
	...
	if( Dist > -THRESH_OPTGEOM_COPLANAR && Dist < THRESH_OPTGEOM_COPLANAR )
	{
		...
		for( ; iNode!=INDEX_NONE; iNode=Model->Nodes[iNode].iPlane )
		{
			...
			for( i=0; i<Model->Nodes[iNode].NumVertices; i++ )
			{
				...
				{
					FBSPOps::GErrors++;
					//UE_LOGF(LogEditorBsp, Warning, "Found tiny side" );
			...
			else if( SkippedColinear )
			{
				...
				FBSPOps::GErrors++;
				//UE_LOGF(LogEditorBsp, Warning, "Skipped colinear" );
				//Poly.PolyFlags |= PF_Selected;
			}
			else if( SkippedInside )
			{
				...
				FBSPOps::GErrors++;
				//UE_LOGF(LogEditorBsp, Warning, "Skipped interior" );
				//Poly.PolyFlags |= PF_Selected;
			}
		}
	}
...
void MergeNearPoints( UModel *Model, float Dist )
{
	...
	// Success.
// 	UE_LOGF(LogEditorBsp, Log,  "MergeNearPoints merged %i/%i, collapsed %i", Merged, Model->Points.Num(), Collapsed );
	Mark.Pop();
}
...
void FBSPUtils::bspOptGeom( UModel *Model )
{
	...
	//UE_LOGF(LogEditorBsp, Log,  "BspOptGeom begin" );
	...
	//UE_LOGF(LogEditorBsp, Log,  "BspOptGeom building sidelinks" );
	...
	for( int32 iNode=0; iNode < Model->Nodes.Num(); iNode++ )
	{
		...
		for( uint8 ThisVert=0; ThisVert < Node.NumVertices; ThisVert++ )
		{
			if( Model->Verts[Node.iVertPool+ThisVert].iSide == INDEX_NONE )
			{
				...
				FBSPOps::GErrors++;
				//UE_LOGF(LogEditorBsp, Warning, "Failed to link side" );
			}
	...
	// Done.
	//UE_LOGF(LogEditorBsp, Log,  "BspOptGeom end" );
// 	UE_LOGF(LogEditorBsp, Log,  "Processed %i T-points, linked: %i/%i sides", TeesFound, j, i );
