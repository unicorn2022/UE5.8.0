...
void FProxyGenerationProcessor::OnModuleConfigurationCompleted()
{
	...
	if (!MeshMerging)
	{
		UE_LOGF(LogMeshMerging, Log, "No automatic mesh merging module available");
	}
	else
	{
	...
	{
		UE_LOGF(LogMeshMerging, Log, "No distributed automatic mesh merging module available");
	}
	else
	{
...
void FProxyGenerationProcessor::ProxyGenerationFailed(const FGuid OutJobGUID, const FString& ErrorMessage)
{
	...
	if (FindData && *FindData)
	{
		UE_LOGF(LogMeshMerging, Log, "Failed to generate proxy mesh for cluster %ls, %ls", *(*FindData)->ProxyBasePackageName, *ErrorMessage);
