...
struct FMaterialDataUsage
{
	bool bUsesVertexData = false;
	bool bUsesWorldPosition = false;
};

static FMaterialDataUsage DetermineMaterialDataUsage(UMaterialInterface* Material, const UMaterialOptions* MaterialOptions)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(DetermineMaterialDataUsage);

	FMaterialDataUsage Usage;
	...
	for (const FPropertyEntry& Entry : MaterialOptions->Properties)
	{
		...
		if (!Entry.bUseConstantValue && Entry.Property != MP_MAX)
		{
			...
			Material->AnalyzeMaterialPropertyEx(Entry.Property, Result);

			int32 NumTextureCoordinates = Result.TextureCoordinates.FindLast(true) + 1;

			Usage.bUsesVertexData |= Result.bRequiresVertexData || NumTextureCoordinates > 1;
			Usage.bUsesWorldPosition |= Result.bRequiresWorldPosition;

			if (Usage.bUsesVertexData && Usage.bUsesWorldPosition)
			{
				break;
			}
		}
...
void FMeshMergeUtilities::PopulatePropertyEntry(const FMaterialProxySettings& MaterialSettings, EMaterialProperty MaterialProperty, FPropertyEntry& InOutPropertyEntry) const
{
	...
	{	
		...
		default:
			UE_LOGF(LogMeshMerging, Error, "Unsupported TextureSizingType value. You should resolve the material texture size first with ResolveTextureSize()");
...
static TArray<FVector2D> GetCustomTextureCoordinates(const FMeshDescription& InMeshDescription, const UStaticMesh* InStaticMesh, const FMeshProxySettings& InMeshProxySettings)
{
	...
	else
	{
		...
		if (!bSuccess)
		{
			UE_LOGF(LogMeshMerging, Warning, "GenerateUV: Failed to pack UVs for static mesh \"%ls\" (num triangles = %d, texture resolution = %d).", *InStaticMesh->GetName(), InMeshDescription.Triangles().Num(), InMeshProxySettings.MaterialSettings.TextureSize.GetMax());
...
void FMeshMergeUtilities::CreateProxyMesh(const TArray<AActor*>& InActors, const struct FMeshProxySettings& InMeshProxySettings, UMaterialInterface* InBaseMaterial, UPackage* InOuter, const FString& InProxyBasePackageName, const FGuid InGuid, const FCreateProxyDelegate& InProxyCreatedDelegate, const bool bAllowAsync /*= false*/, const float ScreenSize /*= 1.0f*/) const
{
	...
	if (InActors.Num() == 0)
	{
		UE_LOGF(LogMeshMerging, Log, "No actors specified to generate a proxy mesh for");
		return;
	}
	...
	UPackage* InOuter, const FString& InProxyBasePackageName, const FGuid InGuid, const FCreateProxyDelegate& InProxyCreatedDelegate, const bool bAllowAsync, const float ScreenSize) const
{
	...
	{
		UE_LOGF(LogMeshMerging, Error, "No mesh reduction module available. You must enable a plugin that provides that functionality (ex: ProxyLODPlugin)");
		return;
	}
	...
	{
		UE_LOGF(LogMeshMerging, Warning, "Invalid (unbound) delegate for returning generated proxy mesh");
		return;
	}
	...
	{
		UE_LOGF(LogMeshMerging, Log, "No static mesh specified to generate a proxy mesh for");
	...
	{
		UE_LOGF(LogMeshMerging, Warning, "Invalid long package name: '%ls'.", *InProxyBasePackageName);
		return;
	}
...
void FMeshMergeUtilities::MergeComponentsToStaticMesh(const TArray<UPrimitiveComponent*>& ComponentsToMerge, UWorld* World, const FMeshMergingSettings& InSettings, UMaterialInterface* InBaseMaterial, UPackage* InOuter, const FString& InBasePackageName, TArray<UObject*>& OutAssetsToSync, FVector& OutMergedActorLocation, const float ScreenSize, bool bSilent /*= false*/) const
{
	...
	for (int32 MeshId = 0; MeshId < ComponentsToMerge.Num(); ++MeshId)
	{
		...
		if (MeshComponent && MeshComponent->GetStaticMesh())
		{
			...
			if (bFirstMesh)
			{
				...
				switch (InSettings.PivotType)
				{
				case EMeshMergePivotType::WorldOrigin:
					MergedAssetPivot = FVector::ZeroVector;
					break;

				case EMeshMergePivotType::Automatic:
					MergedAssetPivot = MeshComponent->GetComponentTransform().GetLocation();
					break;

				case EMeshMergePivotType::Custom:
					MergedAssetPivot = InSettings.CustomPivotLocation;
					break;
	...
	const int32 LightMapUVChannel = [&]()
	{
		if (InSettings.bGenerateLightMapUV)
		{
			...
			if (TempChannel == INDEX_NONE)
			{
				...
				UE_LOGF(LogMeshMerging, Warning, "Failed to find an available channel for Lightmap UVs. Lightmap UVs will not be generated.");
			}
			return TempChannel;
		}
	...
	if (bContainsMergedMeshes || bContainsImposters)
	{
		...
		// Check that an asset of a different class does not already exist
		{
			...
			if(ExistingObject && !ExistingObject->GetClass()->IsChildOf(UStaticMesh::StaticClass()))
			{
				...
				UE_LOGF(LogMeshMerging, Warning,
					"Cannot create %ls %ls.%ls\n" "An object with the same fully qualified name but a different class already exists.\n" "\tExisting Object: %ls\n" "The merged mesh will be named %ls.%ls",
		...
		for (UMaterialInterface* Material : UniqueMaterials)
		{
			if (Material && (!Material->IsAsset() && InOuter != GetTransientPackage()))
			{
				...
				{
					UE_LOGF(LogMeshMerging, Error, "Merged mesh is referencing a non-asset material (\"%ls\"). This material can't be referenced outside it's package as it is not public. Clearing out material entry in the final mesh.", *Material->GetPathName());
	...
	{
		UE_LOGF(LogMeshMerging, Display, "MergeComponentsToStaticMesh: Skipped creation of a static mesh asset as no input meshes were provided");
	}
}
...
void FMeshMergeUtilities::CreateMergedMaterial(FMeshMergeDataTracker& InDataTracker, const FMeshMergingSettings& InSettings, const TArray<UStaticMeshComponent*>& InStaticMeshComponentsToMerge, TArray<FStaticMeshComponentAdapter>& InAdapters, const TArray<UMaterialInterface*>& InUniqueMaterials, const TMap<UMaterialInterface*, UMaterialInterface*>& InCollapsedMaterialMap, TMultiMap<FMeshLODKey, MaterialRemapPair>& InOutputMaterialsMap, bool bInMergeAllLODs, bool bInMergeMaterialData, const FVector& InMergedAssetPivot, FFlattenMaterial& OutFlattenMaterial) const
{
	...
	// Check each material to see if the shader actually uses vertex data / world position.
	// Deferred and cached per material, as AnalyzeMaterialPropertyEx() is a costly call.
	TArray<TOptional<FMaterialDataUsage>> CachedMaterialDataUsage;
	CachedMaterialDataUsage.SetNum(InUniqueMaterials.Num());

	auto GetMaterialDataUsage = [&](const int32 InMaterialIndex)
	{
		if (!CachedMaterialDataUsage[InMaterialIndex].IsSet())
		{
			CachedMaterialDataUsage[InMaterialIndex] = DetermineMaterialDataUsage(InUniqueMaterials[InMaterialIndex], MaterialOptions);
		}

		return CachedMaterialDataUsage[InMaterialIndex].GetValue();
	};

	auto DoesMaterialUsesVertexData = [&](const int32 InMaterialIndex)
	{
		return GetMaterialDataUsage(InMaterialIndex).bUsesVertexData;
	};

	auto DoesMaterialUseWorldPosition = [&](const int32 InMaterialIndex)
	{
		return GetMaterialDataUsage(InMaterialIndex).bUsesWorldPosition;
	};
	...
	for (TConstRawMeshIterator RawMeshIterator = InDataTracker.GetConstRawMeshIterator(); RawMeshIterator; ++RawMeshIterator)
	{
		...
		for (TPair<UMaterialInterface*, TArray<int32>>& MaterialSectionIndexPair : MaterialAndSectionIndices)
		{
			...
			// When the material reads world position, instances of the same mesh at different world positions must not share a bake entry, as they produce different results.
			const bool bMaterialNeedsUniqueInstances = DoesMaterialUseWorldPosition(MaterialIndex);

			auto CompareMeshData = [&ComparePrimitiveData, bMaterialNeedsUniqueInstances](const FMeshData& LHS, const FMeshData& RHS)
			{
				return (LHS.Mesh == RHS.Mesh) && (!bMaterialNeedsUniqueInstances || LHS.MeshDescription == RHS.MeshDescription) && (LHS.MaterialIndices == RHS.MaterialIndices) && (LHS.bMirrored == RHS.bMirrored) && (LHS.VertexColorHash == RHS.VertexColorHash) && ComparePrimitiveData(LHS.PrimitiveData, RHS.PrimitiveData);
			};
	...
	if (bGloballyRemapUVs)
	{
		...
		// Only merge identical triangles if no material reads world position,
		// as instances of the same mesh at different world positions would otherwise share
		// UV space in the atlas and produce identical baked results.
		bool bAnyMaterialUsesWorldPosition = false;
		for (int32 MaterialIndex = 0; MaterialIndex < InUniqueMaterials.Num(); ++MaterialIndex)
		{
			if (DoesMaterialUseWorldPosition(MaterialIndex))
			{
				bAnyMaterialUsesWorldPosition = true;
				break;
			}
		}
		...
		FStaticMeshOperations::FGenerateUVOptions GenerateUVOptions;
		GenerateUVOptions.TextureResolution = MaterialOptions->TextureSize.GetMax();
		GenerateUVOptions.bMergeTrianglesWithIdenticalAttributes = !bAnyMaterialUsesWorldPosition;
		GenerateUVOptions.UVMethod = GetUVGenerationMethodToUse();
		...
		{
			UE_LOGF(LogMeshMerging, Warning, "GenerateUV: Failed to pack UVs for static mesh");
		}
	}
