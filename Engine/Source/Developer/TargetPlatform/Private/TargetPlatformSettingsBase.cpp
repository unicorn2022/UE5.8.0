...
ERayTracingRuntimeMode FTargetPlatformSettingsBase::ParseRuntimeRayTracingMode(const TCHAR* SectionName) const
{
	FString ModeString;
	GetConfigSystem()->GetString(SectionName, TEXT("RayTracingMode"), ModeString, GEngineIni);
	if (ModeString.Equals(TEXT("Inline"), ESearchCase::IgnoreCase))
	{
		return ERayTracingRuntimeMode::Inline;
	}
	else if (ModeString.Equals(TEXT("Full"), ESearchCase::IgnoreCase))
	{
		return ERayTracingRuntimeMode::Full;
	}
	return ERayTracingRuntimeMode::Disabled;
}
...
bool FTargetPlatformSettingsBase::UsesASTCHDR() const
{
	...
	return (CVarASTCHDR ? (CVarASTCHDR->GetInt() != 0) : false);
}
