...
static const TCHAR* SkipUBTSDKSetupEnvVar = TEXT("UE_SKIP_UBT_SDK_SETUP");
static FProcHandle SDKSetupUBTProc;
...
{
#if AUTOSDKS_ENABLED
	FString SDKPath = FPlatformMisc::GetEnvironmentVariable(UE::AutoSDK::SDKRootEnvVar);
...
static FString GetSDKInfo(const TCHAR* Platform, const TCHAR* Key)
{
	...
	for (int Pass = 0; Pass < 2; Pass++)
	{
		...
		do
		{
			...
			{
				UE_LOGF(LogTargetPlatformManager, Error, "Failed to parse SDK json file '%ls'", *InfoFile);
			}
...
{
	int32 SkipUBTSDKSetupEnvVarValue = FCString::Atoi(*FPlatformMisc::GetEnvironmentVariable(SkipUBTSDKSetupEnvVar));
	if (SkipUBTSDKSetupEnvVarValue == 1)
	{
		return false;
	}
	...
	{
		// UBT is usually always invoked when AutoSDK is disabled to check platform status, as there is no other way to check sdk validity. 
		// This causes a startup overhead and unexpected console log output because we have to launch an UBT process and read the output in order to check platform validity.
		// Returning false here for program target type solves the problem by skipping calling UBT, we assume all platforms are valid.
#if IS_PROGRAM
		return false;
#else
		return true;
	...
	{
		UE_LOGF(LogTargetPlatformManager, Log, "Running UBT for AutoSDK init because the value of %ls changed", UE::AutoSDK::SDKRootEnvVar);
		bNeedsToRun = true;
	}
	...
	for (const TPair<FName, FDataDrivenPlatformInfo>& Pair : FDataDrivenPlatformInfoRegistry::GetAllPlatformInfos())
	{
		...
		{
			UE_LOGF(LogTargetPlatformManager, Log, "Running UBT for AutoSDK init because AutoSDK version for %ls changed", *PlatformName);
			bNeedsToRun = true;
		}
		...
		{
			UE_LOGF(LogTargetPlatformManager, Log, "Running UBT for AutoSDK init because the existence of %ls changed", *SetupScript);
			bNeedsToRun = true;
		}
		...
		if (bSetupScriptExists)
		{
			...
			if (FFileHelper::LoadFileToStringArray(FileLines, *(TargetSDKRoot / UE::AutoSDK::SDKInstallManifestFileName)) &&
				FileLines.Num() > 1)
			{
				...
				{
					UE_LOGF(LogTargetPlatformManager, Log, "Running UBT for AutoSDK init because the installed version of %ls doesn't match the required version", *PlatformName);
					bNeedsToRun = true;
				}
			}
			else
			{
				UE_LOGF(LogTargetPlatformManager, Log, "Running UBT for AutoSDK init because the the file %ls is invalid", *(TargetSDKRoot / UE::AutoSDK::SDKInstallManifestFileName));
				bNeedsToRun = true;
			}
			...
			if (!bIsManualInstall)
			{
				...
				if (!FFileHelper::LoadFileToString(RequiredScriptVersion, *(TargetSDKRoot / AutoSDKVersion / UE::AutoSDK::SDKRequiredScriptVersionFileName)))
				{
					RequiredScriptVersion = TEXT("UnspecifiedScriptVersion");
				}
				if (!FFileHelper::LoadFileToString(InstalledScriptVersion, *(TargetSDKRoot / UE::AutoSDK::SDKLastScriptRunVersionFileName)) ||
					RequiredScriptVersion.TrimStartAndEnd() != InstalledScriptVersion.TrimStartAndEnd())
				{
					UE_LOGF(LogTargetPlatformManager, Log, "Running UBT for AutoSDK init because the last run script version in %ls doesn't match required (%ls !- %ls)", *(TargetSDKRoot / UE::AutoSDK::SDKLastScriptRunVersionFileName), *InstalledScriptVersion, *RequiredScriptVersion);
					bNeedsToRun = true;
				}
			}

			// autosdk writes out the OUtputEnvvars.txt file that should exists if autosdk ran
			if (!bIsManualInstall && !FPaths::FileExists(*(TargetSDKRoot / UE::AutoSDK::SDKEnvironmentVarsFile)))
			{
				UE_LOGF(LogTargetPlatformManager, Log, "Running UBT for AutoSDK init because the file %ls is missing", *(TargetSDKRoot / UE::AutoSDK::SDKEnvironmentVarsFile));
				bNeedsToRun = true;
			}
		}
	}
...
void LaunchUBTValidatePlatformsProc(FName PlatformName = NAME_None)
{
	FString UBTParams(TEXT("-Mode=ValidatePlatforms -OutputSDKs "));
	if (PlatformName == NAME_None)
	{
		UBTParams += TEXT(" -AllPlatforms ");
	}
	else
	{
		UBTParams += FString::Printf(TEXT("  -Platforms=%s "), *(PlatformName.ToString()));
	}
	...
	if (FParse::Param(FCommandLine::Get(), TEXT("Multiprocess")) == false && NeedsToRunSetupPlatformsUBT())
	{
		...
		SDKSetupUBTProc = FDesktopPlatformModule::Get()->InvokeUnrealBuildToolAsync(UBTParams, *GLog, ReadPipe, WritePipe, true);
		if (!SDKSetupUBTProc.IsValid())
		{
			UE_LOGF(LogTargetPlatformManager, Warning, "AutoSDK is enabled (UE_SDKS_ROOT is set), but failed to run UBT to check SDK status! Check your installation.");
		}
	}
}

// kick off a call to UBT nice and early so that it's results are hopefully ready when needed
FDelayedAutoRegisterHelper GSDKInit(EDelayedRegisterRunPhase::FileSystemReady, []
	{
		SCOPED_BOOT_TIMING("AutoSDKInit - DelayedAutoRegisterHelper");
		LaunchUBTValidatePlatformsProc();
	}
);
...
{
	void SDKSetup(FName InPlatformName = NAME_None)
	{
		if (SDKSetupUBTProc.IsValid())
		{
			SCOPED_BOOT_TIMING("AutoSDKInit - WaitForUBTProc");
			// wait for UBT to finish
			FPlatformProcess::WaitForProc(SDKSetupUBTProc);
			...
			bool bIsAutoSDKEnabled = IsAutoSDKsEnabled();
			if (FFileHelper::LoadFileToStringArray(Lines, *AbsLogFile) && !bIsAutoSDKEnabled)
			{
				TMap<FName, FName> UBTNameToIniPlatformName;
				for (auto Pair : FDataDrivenPlatformInfoRegistry::GetAllPlatformInfos())
				{
					FName PlatformName = Pair.Key;
					if (InPlatformName == PlatformName || InPlatformName == NAME_None)
					{
						const FDataDrivenPlatformInfo& Info = Pair.Value;
						UBTNameToIniPlatformName.Add(Info.UBTPlatformName, PlatformName);
					}
				}

				for (int Index = 0; Index < Lines.Num(); ++Index)
				{
					FString Item = Lines[Index];
					FString ValidateTag = TEXT("##PlatformValidate: ");
					int32 StartIndex = Item.Find(ValidateTag, ESearchCase::IgnoreCase);
					if (StartIndex != INDEX_NONE)
					{
						StartIndex += ValidateTag.Len();
						int32 EndIndex = Item.Find(TEXT(" "), ESearchCase::IgnoreCase, ESearchDir::FromStart, StartIndex);
						bool bIsSDKValid = !Item.Contains(TEXT("INVALID"));
						if (EndIndex != INDEX_NONE)
						{
							FName UBTName = FName(Item.Mid(StartIndex, EndIndex - StartIndex));
							FName* IniPlatformName = UBTNameToIniPlatformName.Find(UBTName);
							if (IniPlatformName != nullptr)
							{
								PlatformsSetup.Add(*IniPlatformName, bIsSDKValid);
							}
						}
					}
				}
			...
			FPlatformProcess::GetProcReturnCode(SDKSetupUBTProc, &ReturnCode);
			FPlatformProcess::CloseProc(SDKSetupUBTProc);
			SDKSetupUBTProc.Reset();
			if (IsAutoSDKsEnabled() && ReturnCode == 0)
			{
				// now save out the cached info so next run we can skip UBT
				GetCachedInfoToSaveAfterUBT().Write(*(FPaths::ProjectIntermediateDir() / TEXT("CachedAutoSdkInfo.ini")));
			}
		...
		SCOPED_BOOT_TIMING("AutoSDKInit - SetupAutoSDK");
		if (IsAutoSDKsEnabled())
		{
			for (auto Pair : FDataDrivenPlatformInfoRegistry::GetAllPlatformInfos())
			{
				...
				if (InPlatformName == PlatformName || InPlatformName == NAME_None)
				{
					...
					{
						SetupAutoSDK(Pair.Value.AutoSDKPath);
					}
				}
			}
		}
		else if (!NeedsToRunSetupPlatformsUBT())
		{
			// If AutoSDK is disabled and we didn't run UBT, assume all platforms are valid as there is no way to check.
			for (auto Pair : FDataDrivenPlatformInfoRegistry::GetAllPlatformInfos())
			{
				FName PlatformName = Pair.Key;
				PlatformsSetup.Add(PlatformName, true);
			}
		}
...
public:
	...
	virtual void SetupEnvironmentVariables(TArray<FString>& EnvVarNames, const TArray<FString>& EnvVarValues) override
	{
		checkf(EnvVarNames.Num() == EnvVarValues.Num(), TEXT("EnvVarNames and EnvVarValues are expected to have the same number of elements."));
		for (int i = 0; i < EnvVarNames.Num(); ++i)
		{
			const FString& EnvVarName = EnvVarNames[i];
			const FString& EnvVarValue = EnvVarValues[i];
			UE_LOGF(LogTargetPlatformManager, Verbose, "Setting variable '%ls' to '%ls'.", *EnvVarName, *EnvVarValue);
...
protected:
	...
	bool InitializeSinglePlatform(FName PlatformName, const FString& AutoSDKPath)
	{
		...
		if(ModuleSettings != nullptr)
		{
			...
			{
				// one running platform
				if(Platform->IsRunningPlatform())
				{
					checkf(!bIsRunningPlatform, TEXT("Cannot have more than one Running Platform"));
					bIsRunningPlatform = true;
				}
		...
		if (Module == nullptr)
		{
			if (ModuleManager.ModuleExists(*PlatformModuleName.ToString()))
			{
				...
				UE_CLOGFMT(!Module, LogTargetPlatformManager, Warning,
					"Failed to load module '{PlatformModuleName}' for platform {PlatformName} (Reason={Reason}).",
					PlatformModuleName, PlatformName.ToString(), LexToString(Result));
			}
			else
			{
				UE_LOGFMT(LogTargetPlatformManager, Log,
					"Failed to load module '{PlatformModuleName}' for platform {PlatformName} (Reason={Reason}).",
					PlatformModuleName, PlatformName.ToString(), LexToString(EModuleLoadResult::FileNotFound));
			}
		}
		...
		if (Module)
		{
			...
			// If AutoSDK is enabled, then we skip the check if AutoSDKPath of this platform is Empty.
			// If AutoSDK is disabled for whatever reason, then we do not skip and base the platform validity on UBT.
			// If the platform is not in the DataDrivenPlatformInfoRegistry, it's a custom platform module
			// that doesn't participate in the standard SDK management system, so skip SDK validation
			bool bSkipSDKCheck = !FDataDrivenPlatformInfoRegistry::GetAllPlatformInfos().Contains(PlatformName);
			if (bSkipSDKCheck)
			{
				UE_LOGF(LogTargetPlatformManager, Log, "Platform '%ls' not in DataDrivenPlatformInfoRegistry, skipping SDK validation", *PlatformName.ToString());
			}
			FName PlatformSDKNameToCheck = PlatformName;
		...
		RETRY_SETUPANDVALIDATE:
			// This is done here in case autosdk setup failed because shared files are in use by another child cooker
			if (IsAutoSDKsEnabled())
			{
				bSkipSDKCheck = bSkipSDKCheck || (AutoSDKPath == TEXT(""));
				PlatformSDKNameToCheck = FName(*AutoSDKPath);
				if (!bSkipSDKCheck)
				{
					SetupAutoSDK(AutoSDKPath);
				}
			}

			if (bSkipSDKCheck || bIsRunningPlatform || ValidateSDK(PlatformSDKNameToCheck))
			{
				...
				{
					UE_LOGF(LogTargetPlatformManager, Display, "Loaded TargetPlatform '%ls'", *Platform->PlatformName());
			...
			else
			{
				...
				UE_LOGF(LogTargetPlatformManager, Display, "Failed to SetupSDK for platform '%ls'", *PlatformName.ToString());
			}
		}
	...
	void DiscoverAvailablePlatforms()
	{
		...
		if (!Platforms.Num())
		{
			UE_LOGF(LogTargetPlatformManager, Error, "No target platforms found!");
		}
	}
	...
	bool SetupEnvironmentFromAutoSDK(const FString& AutoSDKPath)
	{
		...
		FString TargetSDKRoot = GetAutoSDKPlatformDir(*AutoSDKPath);
		...
		if (FFileHelper::LoadFileToStringArray(FileLines, *SDKInstallManifestFilePath))
		{
			...
			{
				UE_LOGF(LogTargetPlatformManager, Warning, "Malformed install manifest file for Platform %ls", *AutoSDKPath);
				return false;
			}
			...
			{
				UE_LOGF(LogTargetPlatformManager, Verbose, "Platform %ls has manual sdk install", *AutoSDKPath);
				return true;
			}
		}
		else
		{
			UE_LOGF(LogTargetPlatformManager, Log, "Install manifest file for Platform %ls not found.  Platform not set up.", *AutoSDKPath);
			return false;			
		}
		...
		{
			UE_LOGF(LogTargetPlatformManager, Warning, "OutputEnvVars.txt not found for platform: '%ls'", *AutoSDKPath);
			return false;
		}

		UE_LOGF(LogTargetPlatformManager, Verbose, "Platform %ls has auto sdk install", *AutoSDKPath);
		return true;
	}

	void SetupAutoSDK(const FString& AutoSDKPath)
	{
		...
		{		
			...
			bool* bPreviousSetupSuccessful = PlatformsSetup.Find(AutoSDKName);
			if (!bPreviousSetupSuccessful)
			{
				bValidSDK = SetupEnvironmentFromAutoSDK(AutoSDKPath);
				PlatformsSetup.Add(AutoSDKName, bValidSDK);
			}
		}
	}

	bool ValidateSDK(FName PlatformName)
	{
		bool bValidSDK = false;
		bool* SetupSuccessful = PlatformsSetup.Find(PlatformName);
		if (SetupSuccessful)
		{
			bValidSDK = *SetupSuccessful;
		}
		return bValidSDK;
	}
	...
	bool SetupEnvironmentFromManualSDK(const FString& EnvVarFileName)
	{
		...
		UE_LOGF(LogTargetPlatformManager, Verbose, "Reading the manifest for auto-selected manual sdks" );
		...
		if (EnvVarFile)
		{
			...
			for (int32 PathRemoveIndex = 0; PathRemoveIndex < PathRemoves.Num(); ++PathRemoveIndex)
			{
				...
				for (int32 PathVarIndex = 0; PathVarIndex < PathVars.Num(); ++PathVarIndex)
				{
					...
					{
						UE_LOGF(LogTargetPlatformManager, Verbose, "Removing Path: '%ls'", *PathVar);
			...
			for (int32 PathAddIndex = 0; PathAddIndex < PathAdds.Num(); ++PathAddIndex)
			{
				...
				{
					UE_LOGF(LogTargetPlatformManager, Verbose, "Adding Path: '%ls'", *PathAdd);					
	...
	bool UpdateAfterSDKInstall(FName PlatformName)
	{
		...
		FName AutoSDKName = NAME_None;
		FString AutoSDKPath;
		if(IsAutoSDKsEnabled())
		{
			AutoSDKPath = Info.AutoSDKPath;
			AutoSDKName = *AutoSDKPath;
		...
		{
			// Must call UBT again to update platform status if AutoSDK is off.
			LaunchUBTValidatePlatformsProc(PlatformName);
		}
		
		SDKSetup(PlatformName);
...
private:
	...
	void ModulesChangesCallback(FName ModuleName, EModuleChangeReason ReasonForChange)
	{
		...
		{
			bIsInDelegateCall = true;
			Invalidate();
			bIsInDelegateCall = false;
		}
	...
	bool InitializeActiveTargetPlatforms(TArray<ITargetPlatform*>& OutResults)
	{
		...
		if (FParse::Value(FCommandLine::Get(), TEXT("TARGETPLATFORM="), PlatformStr))
		{
			...
			else
			{
				...
				if (OutResults.Num() == 0)
				{
					...
					UE_LOGF(LogTargetPlatformManager, Error, "Invalid target platform specified (%ls). Available = { %ls } ", *PlatformStr, *AvailablePlatforms);
					return false;
				}
			}
		}
		...
		if (!OutResults.Num())
		{
			UE_LOGF(LogTargetPlatformManager, Display, "Not building assets for any platform.");
		}
		else
		{
			for (int32 Index = 0; Index < OutResults.Num(); Index++)
			{
				UE_LOGF(LogTargetPlatformManager, Display, "Building Assets For %ls", *OutResults[Index]->PlatformName());
			}
		}
	...
	void InitializeFormatsWithHints(TArray<const FormatType*>& OutResults)
	{
		...
		for (FName Format : RequiredFormats)
		{
			...
			{
				UE_LOGF(LogTargetPlatformManager, Log, "Unable to find %ls format %ls from hinted modules, loading all potential format modules to find it", HelperType::GetFormatDesc(), *Format.ToString());
		...
		{
#if IS_PROGRAM
			// Only log this information to file, but don't report this as an error for standalone programs.
			//   UnrealPak for instance loads TPM to request shader formats only, but doesn't care about audio formats.
			//   The only format UnrealPak is built against is MetalShaderFormat to check whether native shader libs are supported.
			UE_LOGF(LogTargetPlatformManager, Log, "No target %ls formats found!", HelperType::GetFormatDesc());
#else
			UE_LOGF(LogTargetPlatformManager, Error, "No target %ls formats found!", HelperType::GetFormatDesc());
#endif
		}
		...
		for (int32 Index = 0; Index < Modules.Num(); Index++)
		{
			...
			{
				FormatType* Format = HelperType::GetFormatFromModule(Module);
				UE_LOGF(LogTargetPlatformManager, Log, "Loaded format module %ls", *Modules[Index].ToString());
				if (Format != nullptr)
				{
					TArray<FName> Formats;
					...
					{
						UE_LOGF(LogTargetPlatformManager, Log, "  %ls", *Name.ToString());
					}
	...
	uint32 FindShaderFormatVersion(const TMap<FName, uint32>& FormatVersionCache, FName Name) const
	{
		...
		if (!Result)
		{
			UE_LOGF(LogTargetPlatformManager, Fatal, "ShaderFormat not found for %ls!  Dynamically loaded shader formats require invalidation of FormatVersionCache.", *Name.ToString());
			return INDEX_NONE;
		}
		return *Result;
	}
	...
	// Flag to avoid redundant reloads
	bool bIgnoreFirstDelegateCall;
	
	// Flag to avoid recursive reloads
	bool bIsInDelegateCall = false;
	...
	// When AutoSDK is enabled this holds the AutoSDKName because multiple platform can use the same AutoSDKName
