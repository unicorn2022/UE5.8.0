...
bool FPakOrderMap::ProcessOrderFile(const TCHAR* ResponseFile, bool bSecondaryOrderFile, bool bMergeOrder, TOptional<uint64> InOffset)
{
	...
	FString Text;
	UE_LOGF(LogPakFile, Display, "Loading pak order file %ls...", ResponseFile);
	if (FFileHelper::LoadFileToString(Text, ResponseFile))
	{
		...
		for (int32 EntryIndex = 0; EntryIndex < Lines.Num(); EntryIndex++)
		{
			...
			{
				UE_LOGF(LogPakFile, Error, "Invalid entry in the response file %ls.", *Lines[EntryIndex]);
				return false;
			}
			...
			if(Lines[EntryIndex].FindLastChar('"', OpenOrderNumber))
			{
				...
				{
					UE_LOGF(LogPakFile, Error, "Invalid entry in the response file %ls, couldn't parse an order number after the path.", *Lines[EntryIndex]);
					return false;
				}
			}
			else
		...
		UE_LOGF(LogPakFile, Display, "Finished loading pak order file %ls.", ResponseFile);
		return true;
	}
	else
	{
		UE_LOGF(LogPakFile, Error, "Unable to load pak order file %ls.", ResponseFile);
		return false;
	}
}
...
bool FCompressedFileBuffer::ReadSource(const FPakInputPair& InputInfo, FCookedPackageStore* PackageStore)
{
	...
	else
	{
		...
		if (System.TryRehydratePackages(PackagePath, FAES::AESBlockSize, Errors, RehydratedPackage, &RehydrationInfo) == ERehydrationResult::Success)
		{
			...
			if (RehydrationCount)
			{
				UE_LOGF(LogPakFile, Verbose, "Rehydrated (+%lld bytes) %ls", RehydrationBytes, *PackagePath[0]);
			}
...
void ProcessCommonCommandLine(const TCHAR* CmdLine, FPakCommandLineParameters& CmdLineParameters)
{
	...
	if (FParse::Param(CmdLine, TEXT("ForceCompress")))
	{
		...
		UE_LOGF(LogPakFile, Warning, "-ForceCompress is deprecated.  It will be removed in a future release.");
	}
	...
	if (FParse::Value(CmdLine, TEXT("-compressionformats="), DesiredCompressionFormats) || FParse::Value(CmdLine, TEXT("-compressionformat="), DesiredCompressionFormats))
	{
		...
		for (FString& Format : Formats)
		{
			...
			{
				UE_LOGF(LogPakFile, Warning, "Compression format %ls is not recognized", *Format);
			}
		}
	}
	...
	{
		UE_LOGF(LogPakFile, Display, "Disabling ZLib as a compression option.");
	}
	else
	{
...
void ProcessPakFileSpecificCommandLine(const TCHAR* CmdLine, const TArray<FString>& NonOptionArguments, TArray<FPakInputPair>& Entries, FPakCommandLineParameters& CmdLineParameters)
{
	...
	if (FParse::Value(CmdLine, TEXT("-create="), ResponseFile))
	{
		...
		if ( bCompress )
		{
			...
			UE_LOGF(LogPakFile, Warning, "-compress is deprecated, use -compressed with UAT instead");
		}
		...
		if (ResponseFile.EndsWith(TEXT(".pak"), ESearchCase::IgnoreCase) && CmdLineParameters.GeneratePatch)
		{
			...
			{
				UE_LOGF(LogPakFile, Error, "-extractedpaktemp= not specified.  Required when specifying pak file as the response file.");
			}
		...
		else
		{
			...
			else
			{
				...
				FString Text;
				UE_LOGF(LogPakFile, Display, "Loading response file %ls", *ResponseFile);
				if (FFileHelper::LoadFileToString(Text, *ResponseFile))
				{
				...
				{
					UE_LOGF(LogPakFile, Error, "Failed to load %ls", *ResponseFile);
				}
			}
			...
			for (int32 EntryIndex = 0; EntryIndex < Lines.Num(); EntryIndex++)
			{
				...
				UE_LOGF(LogPakFile, Verbose, "Added file Source: %ls Dest: %ls", *Input.Source, *Input.Dest);
					...
					&& !Input.bNeedEncryption) // if it is encrypted, we will compress it anyway since it won't be mapped at runtime
				{
					...
					UE_LOGF(LogPakFile, Verbose, "Stripped compression from %ls for memory mapping.", *Input.Dest);
				}
			}
		}

		UE_LOGF(LogPakFile, Display, "Added %d entries to add to pak file.", Entries.Num());
	}
	else
	{
...
bool InitializeVirtualizationSystem()
{
	...
	UE_LOGF(LogPakFile, Display, "Initializing the virtualization system...");

	if (FModuleManager::Get().LoadModule(TEXT("Virtualization"), ELoadModuleFlags::LogFailures) == nullptr)
	{
		UE_LOGF(LogPakFile, Error, "Failed to load the virtualization module");
		return false;
	}
	...
	{
		UE_LOGF(LogPakFile, Error, "Failed to find 'PerforceSourceControl' plugin due to: %ls", *ErrorMsg.ToString());
		return false;
	}
	...
	{
		UE_LOGF(LogPakFile, Error, "The 'PerforceSourceControl' plugin is disabled.");
		return false;
	}
	...
	{
		UE_LOGF(LogPakFile, Error, "Failed to load config files for the project '%ls", *ProjectPath);
		return false;
	}
	...
	UE_LOGF(LogPakFile, Display, "Virtualization system initialized");
...
void CollectFilesToAdd(TArray<FPakInputPair>& OutFilesToAdd, const TArray<FPakInputPair>& InEntries, const FPakOrderMap& OrderMap, const FPakCommandLineParameters& CmdLineParameters)
{
	...
	UE_LOGF(LogPakFile, Display, "Collecting files to add to pak file...");
	...
	UE_LOGF(LogPakFile, Display, "Collected %d files in %.2lfs.", OutFilesToAdd.Num(), FPlatformTime::Seconds() - StartTime);
}
...
void LoadKeyChain(const TCHAR* CmdLine, FKeyChain& OutCryptoSettings)
{
	...
	{
		UE_LOGF(LogPakFile, Display, "Parsing crypto keys from a crypto key cache file");
	...
	else if (FParse::Param(CmdLine, TEXT("encryptionini")))
	{
		...
		{
			UE_LOGF(LogPakFile, Warning, "A legacy command line syntax is being used for crypto config. Please update to using the -cryptokey parameter as soon as possible as this mode is deprecated");
			...
			if (ConfigFile.Num())
			{
				UE_LOGF(LogPakFile, Display, "Using new format crypto.ini files for crypto configuration");
				...
				if (bSignPak)
				{
					...
					UE_LOGF(LogPakFile, Display, "Parsed signature keys from config files.");
				}
				...
				if (bEncryptPak)
				{
					...
					if (EncryptionKeyString.Len() > 0)
					{
						...
						UE_LOGF(LogPakFile, Display, "Parsed AES encryption key from config files.");
					}
				}
			}
			else
			{
				...
				UE_LOGF(LogPakFile, Display, "Using old format encryption.ini files for crypto configuration");
	...
	{
		UE_LOGF(LogPakFile, Display, "Using command line for crypto configuration");
		...
		if (EncryptionKeyString.Len() > 0)
		{
			...
			{
				UE_LOGF(LogPakFile, Fatal, "AES encryption key must be %d characters long", RequiredKeyLength);
			}

			if (EncryptionKeyString.Len() > RequiredKeyLength)
			{
				UE_LOGF(LogPakFile, Warning, "AES encryption key is more than %d characters long, so will be truncated!", RequiredKeyLength);
				EncryptionKeyString.LeftInline(RequiredKeyLength);
			}

			if (!FCString::IsPureAnsi(*EncryptionKeyString))
			{
				UE_LOGF(LogPakFile, Fatal, "AES encryption key must be a pure ANSI string!");
			}
			...
			OutCryptoSettings.GetEncryptionKeys().Add(NewKey.Guid, NewKey);
			UE_LOGF(LogPakFile, Display, "Parsed AES encryption key from command line.");
		}
	}
...
FArchive* CreatePakWriter(const TCHAR* Filename, const FKeyChain& InKeyChain, bool bSign)
{
	...
	{
		UE_LOGF(LogPakFile, Error, "Failed to create pak at %ls. File already exists at that location.", Filename);
		return nullptr;
	}
	...
	if (Writer)
	{
		if (bSign)
		{
			UE_LOGF(LogPakFile, Display, "Creating signed pak %ls.", Filename);
			Writer = new FSignedArchiveWriter(*Writer, Filename, InKeyChain.GetSigningKey());
		}
		else
		{
			UE_LOGF(LogPakFile, Display, "Creating pak %ls.", Filename);
		}
	}
...
bool FPakWriterContext::Initialize(const FPakCommandLineParameters& InCmdLineParameters)
{
...

	{
		...
		UE_LOGF(LogPakFile, Display, "%ls", *FormatLogLine);
	}
	...
	if (!CmdLineParameters.ProjectStoreFilename.IsEmpty())
	{
		...
		{
			UE_LOGF(LogPakFile, Fatal, "Failed loading project store '%ls'", *CmdLineParameters.ProjectStoreFilename);
		}
	}
	...
	if ( bDoUseOodleDespiteNoPluginCompression && CompressionFormatsAndNone[0] == NAME_Oodle )
	{
		...
		UE_LOGF(LogPakFile, Display, "Oodle enabled on 'NoPluginCompression' files");
	}
	else
	{
...
bool FPakWriterContext::AddPakFile(const TCHAR* Filename, const TArray<FPakInputPair>& FilesToAdd, const FKeyChain& InKeyChain)
{
	...
	{
		UE_LOGF(LogPakFile, Error, "Unable to create pak file \"%ls\".", Filename);
		return false;
	}
	...
	if (CmdLineParameters.bFileRegions)
	{
		...
		{
			UE_LOGF(LogPakFile, Error, "Unable to create pak regions file \"%ls\".", *RegionsFilename);
			return false;
		}
	}
	...
	{
		UE_LOGF(LogPakFile, Display, "Using encryption key '%ls' [%ls]", *InKeyChain.GetPrincipalEncryptionKey()->Name, *InKeyChain.GetPrincipalEncryptionKey()->Guid.ToString());
	}
	...
	for (;;)
	{
		...
		if (!bDeleted)
		{
				...
				(NewEntryOffset / CmdLineParameters.FileSystemBlockSize) != ((NewEntryOffset + RealFileSize - 1) / CmdLineParameters.FileSystemBlockSize)) // File crosses a block boundary
			{
				...
				{
					UE_LOGF(LogPakFile, Verbose, "%14llu - %14llu : %14llu padding.", PakFileHandle->Tell(), PakFileHandle->Tell() + PaddingRequired, PaddingRequired);
			...
			if (OutputEntry->bIsMappedBulk && CmdLineParameters.AlignForMemoryMapping > 0 && OriginalFileSize != INDEX_NONE && !bDeleted)
			{
				if (!IsAligned(NewEntryOffset + NewEntry.Info.GetSerializedSize(FPakInfo::PakFile_Version_Latest), CmdLineParameters.AlignForMemoryMapping))
				{
					...
					{
						UE_LOGF(LogPakFile, Verbose, "%14llu - %14llu : %14llu bulk padding.", PakFileHandle->Tell(), PakFileHandle->Tell() + PaddingRequired, PaddingRequired);
		...
		if (OutputEntry->bCopiedToPak)
		{
				...
				)
			{
				...
				if (!OutputEntry->bIsUAssetUExpPairUExp) // never patch-pad the uexp of a uasset/uexp pair
				{
						...
						bCrossesBoundary || bPairProbablyCrossesBoundary)
					{
						...
						if (PaddingSize)
						{
							UE_LOGF(LogPakFile, Verbose, "%14llu - %14llu : %14llu patch padding.", PakFileHandle->Tell(), PakFileHandle->Tell() + PaddingSize, PaddingSize);
			...
			// Write to file
			{
				...
				UE_LOGF(LogPakFile, Verbose, "%14llu [header] - %14llu - %14llu : %14llu header+file %ls.", Offset, PayloadOffset, EndOffset, EndOffset - Offset, *NewEntry.Filename);
			}
	...
	{
		{
			...
			for (;;)
			{
				...
				UE_LOGF(LogPakFile, Display, "Writing entries (%llu/%llu)...", RetiredEntriesCount.Load(), TotalEntriesCount);
			}
		}
		...
		if (bPakFileIsEmpty)
		{
			UE_LOGF(LogPakFile, Display, "Created empty pak file: %ls", *OutputPakFile->Filename);
		}
		else
		{
			UE_LOGF(LogPakFile, Display, "Created pak file: %ls", *OutputPakFile->Filename);
		}
		...
		for (FOutputPakFileEntry& OutputEntry : OutputPakFile->Entries)
		{
			...
			{
				UE_LOGF(LogPakFile, Verbose, "File \"%ls\" did not get small enough from compression, or compression failed.", *OutputEntry.InputPair.Source);
			}
			if (OutputEntry.bCopiedToPak)
			{
				const TCHAR* EncryptedString = TEXT("");
				...
				else
				{
					if (OutputEntry.InputPair.SuggestedOrder < MAX_uint64)
					{
						UE_LOGF(LogPakFile, Verbose, "Added %lsfile \"%ls\", %lld bytes (order %llu).", EncryptedString, *OutputEntry.Entry.Filename, OutputEntry.Entry.Info.Size, OutputEntry.InputPair.SuggestedOrder);
					}
					else
					{
						UE_LOGF(LogPakFile, Verbose, "Added %lsfile \"%ls\", %lld bytes (no order given).", EncryptedString, *OutputEntry.Entry.Filename, OutputEntry.Entry.Info.Size);
					}
				}
			}
			else
			{
				...
				{
					UE_LOGF(LogPakFile, Verbose, "Created delete record for file \"%ls\".", *OutputEntry.InputPair.Source);
				}
				else
				{
					UE_LOGF(LogPakFile, Warning, "Missing file \"%ls\" will not be added to PAK file.", *OutputEntry.InputPair.Source);
				}
			}
		}
		...
		if (bPakFileIsEmpty == false)
		{
			...
			for (int32 MethodIndex = 0; MethodIndex < CompressionFormatsAndNone.Num(); MethodIndex++)
			{
				...
				{
					FName CompressionMethod = CompressionFormatsAndNone[MethodIndex];
					UE_LOGF(LogPakFile, Display, "CompressionFormat %d [%ls] : %d files, %lld -> %lld bytes", MethodIndex, *(CompressionMethod.ToString()),
			...
			UE_LOGF(LogPakFile, Display, "Added %d files, %lld bytes total", OutputPakFile->Index.Num(), TotalPakFileSize);
			...
			{
				UE_LOGF(LogPakFile, Display, "Asset Rehydration");
			...
			{
				UE_LOGF(LogPakFile, Display, "Encryption - ENABLED");
				...
				{
					UE_LOGF(LogPakFile, Display, "  Index: Unencrypted");
				}
			...
			{
				UE_LOGF(LogPakFile, Display, "Encryption - DISABLED");
			}
			...
			UE_LOGF(LogPakFile, Display, "");
		}
	}

	WriterThread.Wait();
	CompressionThread.Wait();
	UE_LOGF(LogPakFile, Display, "Writer and Compression Threads exited.");
...
#if DETAILED_UNREALPAK_TIMING
	UE_LOGF(LogPakFile, Display, "Detailed timing stats");
	UE_LOGF(LogPakFile, Display, "Compression time: %lf", ((double)GCompressionTime) * FPlatformTime::GetSecondsPerCycle());
	UE_LOGF(LogPakFile, Display, "DDC Hits: %d", GDDCHits);
	UE_LOGF(LogPakFile, Display, "DDC Misses: %d", GDDCMisses);
	UE_LOGF(LogPakFile, Display, "DDC Sync Read Time: %lf", ((double)GDDCSyncReadTime) * FPlatformTime::GetSecondsPerCycle());
	UE_LOGF(LogPakFile, Display, "DDC Sync Write Time: %lf", ((double)GDDCSyncWriteTime) * FPlatformTime::GetSecondsPerCycle());
#endif
...
{	
	...
	{
		UE_LOGF(LogPakFile, Error, "Unable to open pak file \"%ls\".", Filename);
		return false;
	}
}
...
bool ListFilesInPak(const TCHAR * InPakFilename, int64 SizeFilter, bool bIncludeDeleted, const FString& CSVFilename, bool bExtractToMountPoint, const FKeyChain& InKeyChain, bool bAppendFile, bool bIncludeFooter)
{
	...
	if (PakFile.IsValid())
	{
		UE_LOGF(LogPakFile, Display, "Listing %ls with mount point \"%ls\"",
		...
		int32 EntryIndex = 0;
		{
			TRACE_CPUPROFILER_EVENT_SCOPE(ReadHashFromPayload);

			FSharedPakReader SharedReader = PakFile.GetSharedReader(nullptr);

			for (const TPair<TOptional<FString>, FPakEntry>& It : Records)
			{
				SharedReader->Seek(It.Value.Offset);

				FPakEntry SerializedPakEntry;
				SerializedPakEntry.Serialize(SharedReader.GetArchive(), PakFile.GetInfo().Version);
				FMemory::Memcpy(Hashes + (EntryIndex++) * sizeof(FPakEntry::Hash), &SerializedPakEntry.Hash, sizeof(SerializedPakEntry.Hash));
			}
		}
		...
		if (CSVFilename.Len() > 0)
		{
			...
			{
				UE_LOGF(LogPakFile, Display, "Failed to save CSV file %ls", *CSVFilename);
				return false;
			}
			...
			OutputArchive->Flush();
			UE_LOGF(LogPakFile, Display, "Saved CSV file to %ls", *CSVFilename);
			return true;
		}
		...
		if (InspectChunkString.TrimStartAndEnd().ParseIntoArray(InspectChunkRanges, TEXT(",")))
		{
			for (const FString& InspectChunkRangeString : InspectChunkRanges)
			{
				...
				if (InspectChunkRangeString.TrimStartAndEnd().ParseIntoArray(RangeLimits, TEXT("-")))
				{
					...
					{
						UE_LOGF(LogPakFile, Error, "Error parsing inspect chunk range '%ls'", *InspectChunkRangeString);
					}
				}
			}
		}
		...
		UE_LOGF(LogPakFile, Display, "%d files (%lld bytes), (%lld filtered bytes).", FileCount, FileSize, FilteredSize);
	...
	else
	{
		...
		{
			UE_LOGF(LogPakFile, Fatal, "Missing encryption key %ls for pak file \"%ls\".", *PakFile.GetInfo().EncryptionKeyGuid.ToString(), InPakFilename);
		}
		...
		{
			UE_LOGF(LogPakFile, Fatal, "Unable to open pak file \"%ls\".", InPakFilename);
		}
...
bool AuditPakFiles( const FString& InputPath, bool bOnlyDeleted, const FString& CSVFilename, const FPakOrderMap& OrderMap, bool bSortByOrdering )
{
	...
	{
		UE_LOGF(LogPakFile, Error, "No pak files found searching \"%ls\".", *InputPath);
		return false;
	}
	...
	for (int32 PakFileIndex = 0; PakFileIndex < PakFileList.Num(); PakFileIndex++)
	{
		...
		if (PakFile.IsValid())
		{
			...
			{
				UE_LOGF(LogPakFile, Error, "Pakfiles were loaded without Filenames, cannot audit.");
				return false;
			}
		...
		{
			UE_LOGF(LogPakFile, Error, "Unable to open pak file \"%ls\".", *PakFilename);
			return false;
		}
	}
	...
	if( !CSVFilename.IsEmpty() )
	{
		...
		{
			UE_LOGF(LogPakFile, Error, "Unable to open csv file \"%ls\".", *CSVFilename);
			return false;
		}
	}
	...
	auto WriteCSVLine = [CSVFileWriter]( const FString& Text )
	{
		...
		{
			UE_LOGF(LogPakFile, Display, "%ls", *Text );
		}
	};
	...
	if( bHasOpenOrder )
	{
		UE_LOGF(LogPakFile, Display, "Checking open order data" );
	...
	{
		UE_LOGF(LogPakFile, Display, "Sorting pak audit data by open order" );
	...
	{
		UE_LOGF(LogPakFile, Display, "Sorting pak audit data by name" );
		FileRevisions.KeySort([]( const FString& A, const FString& B )
		{
	...
	UE_CLOGF((CSVFileWriter!=nullptr),LogPakFile, Display, "Writing pak audit CSV file %ls...", *CSVFilename );
	...
	{
		UE_LOGF( LogPakFile, Display, "%d guaranteed seeks out of %d files read (%.2f%%) with the given open order", NumSeeks, NumReads, (float)(NumSeeks*100.0) / (float)NumReads );
	}
...
bool ListFilesAtOffset( const TCHAR* InPakFileName, const TArray<int64>& InOffsets )
{
	if( InOffsets.Num() == 0 )
	{
		UE_LOGF(LogPakFile, Error, "No offsets specified" );
		return false;
	}
	...
	if (!PakFile.IsValid())
	{
		UE_LOGF(LogPakFile, Error, "Failed to open %ls", InPakFileName );
		return false;
	}

	UE_LOGF( LogPakFile, Display, "%-12ls%-12ls%-12ls%ls", TEXT("Offset"), TEXT("File Offset"), TEXT("File Size"), TEXT("File Name") );
...
bool ShowCompressionBlockCRCs( const TCHAR* InPakFileName, TArray<int64>& InOffsets, const FKeyChain& InKeyChain )
{
	...
	for( int64 Offset : InOffsets )
	{
		...
		if (Entry == nullptr)
		{
			UE_LOGF(LogPakFile, Error, "Offset: %lld - no entry found.", Offset );
			continue;
		}
		...
		{
			UE_LOGF(LogPakFile, Error, "    Entry isn't compressed (not supported)" );
			continue;
		}
		if (Entry->IsDeleteRecord())
		{
			UE_LOGF(LogPakFile, Error, "    Entry is deleted" );
			continue;
		}
	...
	UE_LOGF(LogPakFile, Display, "done" );
	return true;
}
...
bool GeneratePIXMappingFile(const TArray<FString> InPakFileList, const FString& OutputPath)
{
	...
	{
		UE_LOGF(LogPakFile, Error, "Pak file list can not be empty.");
		return false;
	}

	if (!FPaths::DirectoryExists(OutputPath))
	{
		UE_LOGF(LogPakFile, Error, "Output path doesn't exist.  Create %ls.", *OutputPath);
	...
	for (const FString& PakFileName : InPakFileList)
	{
		...
		if (!OutputMappingFilename.IsEmpty())
		{
			...
			if (CSVFileWriter == nullptr)
			{
				UE_LOGF(LogPakFile, Error, "Unable to open csv file \"%ls\".", *OutputMappingFilename);
				return false;
			}
		}
		...
		if (!PakFile.IsValid())
		{
			UE_LOGF(LogPakFile, Error, "Failed to open %ls", *PakFileName);
			return false;
		}
		...
		if (!PakFile.HasFilenames())
		{
			UE_LOGF(LogPakFile, Error, "PakFiles were loaded without filenames, cannot generate PIX mapping file.");
			return false;
		}
	...
	TArray<FGuid>* OutUsedEncryptionKeys, bool* OutAnyPakSigned)
{
	...
	if (OutOrderMap)
	{
		...
		{
			UE_LOGF(LogPakFile, Error,
				"ExtractFilesFromPak: Collecting the order of the files in a pak is not implemented when multiple packs are specified by wildcard. "
				"ExtractFilesFromPak was called on '%ls', which matched %d pakfiles.",
				InPakFilename, PakFileList.Num());
			return false;
		}
		OutOrderMap->Empty();
	}
	...
	const FString DestPath = FPaths::ConvertRelativePathToFull(InDestPath);
	for (int32 PakFileIndex = 0; PakFileIndex < PakFileList.Num(); PakFileIndex++)
	{
		FString PakFilename = PakFileDirectory + "\\" + PakFileList[PakFileIndex];
		int32 PakPriority = GetPakPriorityFromFilename(PakFilename);
		...
		if (PakFile.IsValid())
		{
			...
			if (!PakFile.HasFilenames())
			{
				UE_LOGF(LogPakFile, Error, "PakFiles were loaded without filenames, cannot extract.");
				return false;
			}
			...
			uint8* PersistentCompressionBuffer = NULL;
			int32 ErrorCount = 0;
			int32 FileCount = 0;
			...
			for (FPakFile::FPakEntryIterator It(PakFile,bIncludeDeleted); It; ++It, ++FileCount)
			{
				...
				{
					FString DestFilename = FPaths::ConvertRelativePathToFull(DestPath / PakMountPoint / EntryFileName);

					const FPakEntry& Entry = It.Info();
					if (Entry.IsDeleteRecord())
					{
						checkSlow(OutDeletedEntries);
						UE_LOGF(LogPakFile, Display, "Found delete record for \"%ls\".", *EntryFileName);
					...
					if (!FPaths::IsUnderDirectory(DestFilename, DestPath))
					{
						...
						FString SafeDestFilename = FPaths::ConvertRelativePathToFull(DestPath / HashDir / FPaths::GetCleanFilename(EntryFileName));
						UE_LOGF(LogPakFile, Display, "Fixup extract path for \"%ls\": from \"%ls\" to \"%ls\"",
					...
					if (EntryInfo.IndexDataEquals(Entry))
					{
						...
						if (FileHandle)
						{
							...
							{
								UncompressCopyFile(*FileHandle, PakReader.GetArchive(), Entry, PersistentCompressionBuffer, CompressionBufferSize, InKeyChain, PakFile);
							}
							UE_LOGF(LogPakFile, Display, "Extracted \"%ls\" to \"%ls\" Offset %lld.", *EntryFileName, *DestFilename, Entry.Offset);
							ExtractedCount++;
						...
						{
							UE_LOGF(LogPakFile, Error, "Unable to create file \"%ls\".", *DestFilename);
							ErrorCount++;
						}
					}
					else
					{
						UE_LOGF(LogPakFile, Error, "PakEntry mismatch for \"%ls\".", *EntryFileName);
						ErrorCount++;
					}
				}
			}
			FMemory::Free(Buffer);
			FMemory::Free(PersistentCompressionBuffer);

			UE_LOGF(LogPakFile, Log, "Finished extracting %d (including %d errors).", ExtractedCount, ErrorCount);
		}
		else
		{
			...
			if (OutEntries)
			{
				UE_LOGF(LogPakFile, Warning, "Generating response files from IoStore containers is not supported.");
			}
...
bool DumpPakInfo(const FString& InPakFilename, const FKeyChain& InKeyChain)
{
	...
	UE_LOGF(LogPakFile, Display, "Pak File: %ls", *InPakFilename);
	UE_LOGF(LogPakFile, Display, "    Version: %d", Info.Version);
	...
	UE_LOGF(LogPakFile, Display, "    IndexHash: %ls", *Info.IndexHash.ToString());
	UE_LOGF(LogPakFile, Display, "    bEncryptedIndex: %d", Info.bEncryptedIndex);
	UE_LOGF(LogPakFile, Display, "    EncryptionKeyGuid: %ls", *Info.EncryptionKeyGuid.ToString());
	UE_LOGF(LogPakFile, Display, "    CompressionMethods:");
...
bool DiffFilesInPaks(const FString& InPakFilename1, const FString& InPakFilename2, const bool bLogUniques1, const bool bLogUniques2, const FKeyChain& InKeyChain1, const FKeyChain& InKeyChain2)
{
	...
	{		
		UE_LOGF(LogPakFile, Log, "FileEventType, FileName, Size1, Size2");

		if (!PakFile1.HasFilenames() || !PakFile2.HasFilenames())
		{
			UE_LOGF(LogPakFile, Error, "Pakfiles were loaded without Filenames, cannot diff.");
			return false;
		}
		...
		for (FPakFile::FPakEntryIterator It(PakFile1); It; ++It, ++FileCount)
		{
			...
			{
				UE_LOGF(LogPakFile, Log, "PakEntry1Invalid, %ls, 0, 0", *PAK1FileName);
				continue;
			}
			...
			{
				UE_LOGF(LogPakFile, Log, "PakEntry2Invalid, %ls, 0, 0", *PAK1FileName);
				continue;;
			}
			...
			//serialize and memcompare the two entries
			{
				...
				{
					UncompressCopyFile(PAKWriter1, PakReader1.GetArchive(), Entry1, PersistentCompressionBuffer, CompressionBufferSize, InKeyChain1, PakFile1);
				}
				...
				{
					UncompressCopyFile(PAKWriter2, PakReader2.GetArchive(), Entry2, PersistentCompressionBuffer, CompressionBufferSize, InKeyChain2, PakFile2);
				}
		...
		UE_LOGF(LogPakFile, Display, "Unique to first pak: %i, Unique to second pak: %i, Num Different: %i, NumEqual: %i", NumUniquePAK1, NumUniquePAK2, NumDifferentContents, NumEqualContents);
		return true;
	}
...
bool GenerateHashesFromPak(const TCHAR* InPakFilename, const TCHAR* InDestPakFilename, TMap<FString, FFileInfo>& FileHashes, bool bUseMountPoint, const FKeyChain& InKeyChain, int32& OutLowestSourcePakVersion, TArray<FGuid>* OutUsedEncryptionKeys = nullptr)
{
	...
	for (int32 PakFileIndex = 0; PakFileIndex < PakFileList.Num(); PakFileIndex++)
	{
		...
		if (PakFile.IsValid())
		{
			...
			if (!PakFile.HasFilenames())
			{
				UE_LOGF(LogPakFile, Error, "Pakfiles were loaded without Filenames, cannot GenerateHashesFromPak.");
				return false;
			}
				...
				else
				{
					...
					if (EntryInfo.IndexDataEquals(Entry))
					{
						...
						// if (FileHandle.IsValid())
						{
							...
							UE_LOGF(LogPakFile, Display, "Generated hash for \"%ls\"", *FullFilename);
					...
					{
						UE_LOGF(LogPakFile, Error, "Serialized hash mismatch for \"%ls\".", **It.TryGetFilename());
						ErrorCount++;
					}
				}
			...
			UE_LOGF(LogPakFile, Log, "Finished extracting %d files (including %d errors).", FileCount, ErrorCount);
		}
		else
		{
			UE_LOGF(LogPakFile, Error, "Unable to open pak file \"%ls\".", *PakFilename);
...
bool FileIsIdentical(FString SourceFile, FString DestFilename, const FFileInfo* Hash, int64* DestSizeOut = nullptr)
{
	...
	if (!Hash)
	{
		...
		{
			// file size doesn't match 
			UE_LOGF(LogPakFile, Display, "Source file size %ls doesn't exist will be included in build", *SourceFile);
			return false;;
		}
		else
		{
			UE_LOGF(LogPakFile, Warning, "Generated hash for file %ls but it should have been in the FileHashes array", *SourceFile);
		}
	}
	else
	{
	...
	if (GenerateHashForFile(DestFilename, DestFileHash) == false)
	{
		...
		UE_LOGF(LogPakFile, Display, "File was removed from destination cooked content %ls not included in patch", *DestFilename);
		return false;
	}
	...
	if (Diff != 0)
	{
		UE_LOGF(LogPakFile, Display, "Source file hash for %ls doesn't match dest file hash %ls and will be included in patch", *SourceFile, *DestFilename);
		return false;
	}
	...
	for (int i = 0; i < FilesToPak.Num(); i++)
	{
		...
		if (CounterpartFileIndex != -1)
		{
			...
			if (IncludeFilesMask[i] != IncludeFilesMask[CounterpartFileIndex])
			{
				if (!IncludeFilesMask[i])
				{
					//UE_LOGF(LogPakFile, Display, "Added %ls because %ls is already included", *FilesToPak[i].Source, *FilesToPak[CounterpartFileIndex].Source);
					IncludeFilesMask[i] = true;
					OutSizeIncrease += FileSizes[i];
				}
				else
				{
					//UE_LOGF(LogPakFile, Display, "Added %ls because %ls is already included", *FilesToPak[CounterpartFileIndex].Source, *FilesToPak[i].Source);
...
{
	UE_CLOGF(SeekOptParams.MaxGapSize == 0, LogPakFile, Fatal, "ApplyGapFilling requires MaxGapSize > 0");
	...
	{
		UE_CLOGF(SeekOptParams.MaxInflationPercent == 0.0f, LogPakFile, Fatal, "ESeekOptMode::Incremental* requires MaxInflationPercent > 0.0");
	...
	{
		UE_LOGF(LogPakFile, SEEK_OPT_VERBOSITY, "Patch seek optimization did not modify patch pak size (no additional files added)");
	}
	else
	{
		UE_LOGF(LogPakFile, SEEK_OPT_VERBOSITY, "Patch seek optimization increased estimated patch pak size from %.2fMB to %.2fMB (+%.1f%%)", OriginalSizeMB, TotalSizeMB, SizeIncreasePercent);
		UE_LOGF(LogPakFile, SEEK_OPT_VERBOSITY, "Total files added : %d (from %d)", NewPatchFileCount - OriginalPatchFileCount, OriginalPatchFileCount);
	}
	...
	UE_LOGF(LogPakFile, SEEK_OPT_VERBOSITY, "Fragmentation pre-optimization: %.2f%%", FragmentationPercentageOriginal);
	UE_LOGF(LogPakFile, SEEK_OPT_VERBOSITY, "Fragmentation final: %.2f%%", FragmentationPercentageNew);
	UE_LOGF(LogPakFile, SEEK_OPT_VERBOSITY, "Fragmentation pre-optimization (primary files): %.2f%%", FragmentationPercentageOriginalPrimary);
	UE_LOGF(LogPakFile, SEEK_OPT_VERBOSITY, "Fragmentation final (primary files): %.2f%%", FragmentationPercentageNewPrimary);
}
	...
	ParallelFor(FilesToPak.Num(), [SourceDirectory, ChangedFilesArchive, &FilesToPak, &FileHashes, &FileSizes, &IncludeFilesMask, &ChangedFilesArchiveArray](int32 Idx)
		{
			...
			if (!FoundFileHash)
			{
				UE_LOGF(LogPakFile, Display, "Didn't find hash for %ls No mount %ls", *SourceFilename, *SourceFileNoMountPoint);
			}
			...
			{
				UE_LOGF(LogPakFile, Display, "Source file %ls matches dest file %ls and will not be included in patch", *SourceFilename, *DestFilename);
	...
	ParallelFor(FilesToPak.Num(), [&FilesToPak, &UAssetToUexpMapping, &IncludeFilesMask](int32 Idx)
		{
			...
			if (CounterpartFileIndex != -1)
			{
				...
				{
					UE_LOGF(LogPakFile, Display, "One of %ls and %ls is different from source, so both will be included in patch", *FilesToPak[Idx].Source, *FilesToPak[CounterpartFileIndex].Source);
...
void ProcessLegacyFileMoves( TArray<FPakInputPair>& InDeleteRecords, TMap<FString, FFileInfo>& InExistingPackagedFileHashes, const FString& InInputPath, const TArray<FPakInputPair>& InFilesToPak, int32 CurrentPatchChunkIndex )
{
	...
	if( PakFileList.Num() == 0 )
	{
		UE_LOGF( LogPakFile, Error, "No pak files searching \"%ls\"", *InInputPath );
		return;
	}
	...
	for (int32 PakFileIndex = 0; PakFileIndex < PakFileList.Num(); PakFileIndex++)
	{
		...
		UE_LOGF(LogPakFile, Display, "Checking old pak file \"%ls\" Pri:%d Chunk:%d.", *PakFilename, PakPriority, PakChunkIndex );
		...
		if (PakFile.IsValid())
		{
			...
			if (!PakFile.HasFilenames())
			{
				UE_LOGF(LogPakFile, Error, "Pakfiles were loaded without Filenames, cannot ProcessLegacyFileMoves.");
				return;
			}
			for (FPakFile::FPakEntryIterator It(PakFile,bIncludeDeleted); It; ++It)
			{
	...
	{
		UE_LOGF(LogPakFile, Display, "checking deleted revision %ls chunk %d vs %d   pak version %d vs %d", *Itr.Key, Itr.Value.PakChunkIndex, CurrentPatchChunkIndex, Itr.Value.PakVersion, FPakInfo::PakFile_Version_DeleteRecords );
		...
		if( Itr.Value.PakChunkIndex != CurrentPatchChunkIndex )
		{
			...
			if( NumDeleted > 0 )
			{
				UE_LOGF( LogPakFile, Display, "Ignoring delete record for %ls - it was moved to %ls before delete records were created", *Itr.Key, *FPaths::GetCleanFilename(Itr.Value.PakFilename) );
			}
		}
	}
	...
	for (auto Itr : RequiredFileRevisions)
	{
		if (Itr.Value.PakVersion < FPakInfo::PakFile_Version_DeleteRecords && Itr.Value.PakChunkIndex != CurrentPatchChunkIndex )
		{
			...
			{
				UE_LOGF( LogPakFile, Display, "Ensuring %ls is included in the pak file - it was moved to %ls before delete records were created", *Itr.Key, *FPaths::GetCleanFilename(Itr.Value.PakFilename) );
				InExistingPackagedFileHashes[Itr.Key].bForceInclude = true;
			}
		}
	}

	UE_LOGF(LogPakFile, Display, "...took %.2fs to manage legacy patch pak files", FPlatformTime::Seconds() - StartTime );
}
...
TArray<FPakInputPair> GetNewDeleteRecords( const TArray<FPakInputPair>& InFilesToPak, const TMap<FString, FFileInfo>& InExistingPackagedFileHashes)
{
	...
	for (const TTuple<FString, FFileInfo>& Pair : InExistingPackagedFileHashes)
	{
		...
		if (bFound == false)
		{
			...
			UE_LOGF(LogPakFile, Display, "Existing pak entry %ls not found in new pak asset list, so a delete record will be created in the patch pak.", *SourceFileName);
		}
 	}


	UE_LOGF(LogPakFile, Display, "Took %.2fS for delete records", FPlatformTime::Seconds()-StartTime );
...
FString GetPakPath(const TCHAR* SpecifiedPath, bool bIsForCreation)
{
	...
	if (!bIsForCreation && !AtLeastOnePakFileExists(PakFilename))
	{
		...
		{
			UE_LOGF(LogPakFile, Fatal, "Existing pak file %ls could not be found (checked against binary and launch directories)", SpecifiedPath);
			return TEXT("");
		}
	}
...
bool Repack(const FString& InputPakFile, const FString& OutputPakFile, const FPakCommandLineParameters& CmdLineParameters, const FKeyChain& InKeyChain, bool bIncludeDeleted)
{
	...
	if (ExtractFilesFromPak(*InputPakFile, Hashes, *TempDir, false, InKeyChain, nullptr, &Entries, &DeletedEntries, &OrderMap, &EncryptionKeys, &bAnySigned))
	{
		...
		{
			UE_LOGF(LogPakFile, Display, "%ls has %d delete records - these will not be included in the repackage. Specify -IncludeDeleted to include them", *InputPakFile, DeletedEntries.Num() );
		}
		...
		UE_LOGF(LogPakFile, Display, "Creating %ls...", *OutputPakFile);
...
void CheckAndReallocThreadPool()
{
	if (FPlatformProcess::SupportsMultithreading())
	{
		...
		{
			UE_LOGF(LogPakFile, Log, "Engine only spawned %d worker threads, bumping up to %d!", ThreadsSpawned, DesiredThreadCount);
			GThreadPool->Destroy();
		...
		{
			UE_LOGF(LogPakFile, Log, "Continuing with %d spawned worker threads.", ThreadsSpawned);
		}
	}
}
...
bool MakeBinaryConfig(const TCHAR* CmdLine)
{
	...
	{
		UE_LOGF(LogMakeBinaryConfig, Error, "OutputFile= parameter required");
		return false;
	}
	...
	{
		UE_LOGF(LogMakeBinaryConfig, Error, "StagedPluginsFile= parameter required");
		return false;
	}
	...
	{
		UE_LOGF(LogMakeBinaryConfig, Error, "ProjectFile= parameter required (path to .uproject file)");
		return false;
	}
	...
	{
		UE_LOGF(LogMakeBinaryConfig, Error, "Platform= parameter required (Ini platform name, not targetplatform name - Windows, not Win64 or WindowsClient)");
		return false;
	}
	...
	{
		UE_LOGF(LogMakeBinaryConfig, Fatal, "Failed to load required file %ls", *StagedPluginsFile);
	}
	...
	FString ScalabilityPlatformOverride;
	FParse::Value(FCommandLine::Get(), TEXT("ScalabilityIniPlatformOverride="), ScalabilityPlatformOverride);
	if (!ScalabilityPlatformOverride.IsEmpty())
	{
		FString EngineConfigPlatformOverride = FString::Format(TEXT("{0}{1}/{1}Scalability.ini"), { *Context.EngineConfigDir, *ScalabilityPlatformOverride });
		FString ProjectConfigPlatformOverride = FString::Format(TEXT("{0}{1}/{1}Scalability.ini"), { *Context.ProjectConfigDir, *ScalabilityPlatformOverride });
		if (Config.StagedGlobalConfigCache == nullptr)
		{
			Config.StagedGlobalConfigCache = new TSet<FString>();
		}
		Config.StagedGlobalConfigCache->Add(EngineConfigPlatformOverride);
		Config.StagedGlobalConfigCache->Add(ProjectConfigPlatformOverride);
	}
	...
	{
		FConfigBranch* Branch = Config.FindBranch(*Filename, Filename);
		Branch->RunOnEachFile([&SectionsDenyList, &KeysDenyList](FConfigFile& File, const FString& Name)
			{
		...
		Branch->RunOnEachCommandStream([&SectionsDenyList, &KeysDenyList](FConfigCommandStream& Stream, const FString& Name)
			{
				for (const FString& Section : SectionsDenyList)
				{
					Stream.Remove(Section);
				}

				// now go over any remaining sections and remove keys
				for (TPair<FString, FConfigCommandStreamSection>& SectionPair : Stream)
				{
					for (FName Key : KeysDenyList)
					{
						SectionPair.Value.Remove(Key);
					}
				}
			});
	}
	...
	{
		UE_LOGF(LogMakeBinaryConfig, Error, "Failed to create binary config file '%ls'", *OutputFile);
		return false;
	}
...
{
	UE_LOGF(LogPakFile, Display, "ProjectDir: %ls", *FPaths::ProjectDir());
	{
		FString IoStoreArg;
		...
		if (FParse::Value(CmdLine, TEXT("-ListContainerMetadataFile="), IoStoreArg))
		{
			return ListIoStoreContainerMetadataFile(CmdLine);
		}
	...
	if (FParse::Value(CmdLine, TEXT("-Project="), ProjectArg))
	{
		...
		{
			UE_LOGF(LogPakFile, Error, "Project file does not exist: %ls", *ProjectArg);
		}

		UE_LOGF(LogPakFile, Error, "Project should be specified as a first unnamed argument. E.g. 'UnrealPak %ls -arg1 -arg2", *ProjectArg);
		return false;
	}
	...
	if (FParse::Param(CmdLine, TEXT("listformats")))
	{
		...
		UE_LOGF(LogPakFile, Display, "Supported Pak Formats:");
		for (auto& Format : Formats)
		{
			UE_LOGF(LogPakFile, Display, "\t%ls %d", *Format->GetCompressionFormatName().ToString(), Format->GetVersion());
		}
		return true;
	}
	...
	if (FParse::Value(CmdLine, TEXT("-Batch="), BatchFileName))
	{
		...
		{
			UE_LOGF(LogPakFile, Error, "Unable to read '%ls'", *BatchFileName);
			return false;
		}

		UE_LOGF(LogPakFile, Display, "Running UnrealPak in batch mode with commands:");
		for (int i = 0; i < Commands.Num(); i++)
		{
			UE_LOGF(LogPakFile, Display, "[%d] : %ls", i, *Commands[i]);
		}
	...
	if (IsTestCommand || IsVerifyCommand)
	{
		...
		{
			UE_LOGF(LogPakFile, Error, "Incorrect arguments. Expected: -Test <PakFile>");
			return false;
		}
	...
	if (FParse::Param(CmdLine, TEXT("List")))
	{
		...
		if (NonOptionArguments.Num() != 1)
		{
			UE_LOGF(LogPakFile, Error, "Incorrect arguments. Expected: -List <PakFile> [-SizeFilter=N]");
			return false;
		}
		...
		TArray<bool> bSucceeded;
		bSucceeded.SetNum(FilesToList.Num());

		ParallelFor(FilesToList.Num(), [&FilesToList, 
										&CSVFilename, 
										&bOutputCSV, 
										&bAppendCSV, 
										&bExtractToMountPoint, 
										...
										&SizeFilter,
										&bExcludeDeleted,
										&bSucceeded](int32 Idx)
		{
			const FString& PakFilename = FilesToList[Idx];
			...
			bSucceeded[Idx] = ListFilesInPak(*PakFilename, SizeFilter, !bExcludeDeleted, *ThisPaksCSV, bExtractToMountPoint, KeyChain, bAppendCSV, bIncludeFooter);
		});

		for (const bool &bDidSucceed : bSucceeded)
		{
			bAllSucceeded &= bDidSucceed;
		}
	...
	if (FParse::Param(CmdLine, TEXT("Info")))
	{
		if (NonOptionArguments.Num() != 1)
		{
			UE_LOGF(LogPakFile, Error, "Incorrect arguments. Expected: -Info <PakFile>");
			return false;
		}
	...
	if (FParse::Param(CmdLine, TEXT("Diff")))
	{
		...
		{
			UE_LOGF(LogPakFile,Error,"Incorrect arguments. Expected: -Diff <PakFile1|PakPath1> <PakFile2|PakPath2> [-NoUniques] [-NoUniquesFile1] [-NoUniquesFile2] -CryptoKeys2=<cryptofile>");
			return false;
		}
		...
		UE_LOGF(LogPakFile, Display, "Diff Source Pak Folder: %ls - %d pak file(s)",
			*SourceFolderName, SourcePakFiles.Num());
		UE_LOGF(LogPakFile, Display, "Diff Target Pak Folder: %ls - %d pak file(s)",
		...
		for (int32 I = SourcePakFiles.Num() - 1; I >= 0; --I)
		{
			if (!TargetPakFileSet.Contains(SourcePakFiles[I]))
			{
				if (bLogUniques1)
				{
					UE_LOGF(LogPakFile, Display, "Source PakFile '%ls' is missing in target folder",
						*SourcePakFiles[I]);
				}
		...
		if (bLogUniques2)
		{
			for (int32 I = TargetPakFiles.Num() - 1; I >= 0; --I)
			{
				...
				{
					UE_LOGF(LogPakFile, Display, "Target PakFile '%ls' is missing in source folder",
						*TargetPakFiles[I]);
				}
			}
		}
		...
		for (const FString& PakFile : SourcePakFiles)
		{
			...
			UE_LOGF(LogPakFile, Display, "Diffing PakFile '%ls'", *PakFile);
	...
	if (FParse::Param(CmdLine, TEXT("Extract")))
	{
		...
		if (NonOptionArguments.Num() != 2)
		{
			UE_LOGF(LogPakFile, Error, "Incorrect arguments. Expected: -Extract <PakFile> <OutputPath> [-responsefile=<outputresponsefilename> -order=<outputordermap>]");
			return false;
		}
	...
	if (FParse::Param(CmdLine, TEXT("AuditFiles")))
	{
		if (NonOptionArguments.Num() != 1)
		{
			UE_LOGF(LogPakFile, Error, "Incorrect arguments. Expected: -AuditFiles <PakFolder> -CSV=<OutputPath> [-OnlyDeleted] [-Order=<OrderingFile>] [-SortByOrdering]");
			return false;
		}
	...
	if (FParse::Param(CmdLine, TEXT("WhatsAtOffset")))
	{
		if (NonOptionArguments.Num() < 2)
		{
			UE_LOGF(LogPakFile, Error, "Incorrect arguments. Expected: -WhatsAtOffset <PakFile> [Offset...]");
			return false;
		}
	...
	if (FParse::Param(CmdLine, TEXT("CalcCompressionBlockCRCs")))
	{
		if (NonOptionArguments.Num() < 2)
		{
			UE_LOGF(LogPakFile, Error, "Incorrect arguments. Expected -CalcCompressionBlockCRCs <PakFile> [Offset...] " );
			return false;
		}
	...
	if (FParse::Param(CmdLine, TEXT("GeneratePIXMappingFile")))
	{
		if (NonOptionArguments.Num() != 1)
		{
			UE_LOGF(LogPakFile, Error, "Incorrect arguments. Expected: -GeneratePIXMappingFile <PakFile> [-OutputPath=<OutputPath>]");
			return false;
		}
	...
	if (FParse::Param(CmdLine, TEXT("Repack")))
	{
		if (NonOptionArguments.Num() != 1)
		{
			UE_LOGF(LogPakFile, Error, "Incorrect arguments. Expected: -Repack <PakFile> [-Output=<PakFile>]");
			return false;
		}
		...
		{
			UE_LOGF(LogPakFile, Error, "No files found matching '%ls'", *NonOptionArguments[0]);
			return false;
		}
		...
		for (int Idx = 0; Idx < InputPakFiles.Num(); Idx++)
		{
			UE_LOGF(LogPakFile, Display, "Repacking %ls into %ls", *InputPakFiles[Idx], *OutputPakFiles[Idx]);
	...
	TArray<FString> CreatePakCommandsList;
	{
		...
		if (FParse::Value(CmdLine, TEXT("-CreateMultiple="), CreateMultipleCommandListPath))
		{
			...
			{
				UE_LOGF(LogPakFile, Error, "Failed to read command list file '%ls'.", *CreateMultipleCommandListPath);
				return false;
			}
		}
	}
	if(NonOptionArguments.Num() > 0 || !CreatePakCommandsList.IsEmpty())
	{
		...
		for (const FString& CreatePakCommand : CreatePakCommandsList)
		{
			...
			if (CmdLineParameters.GeneratePatch)
			{
				...
				UE_LOGF(LogPakFile, Display, "Generating patch from %ls.", *CmdLineParameters.SourcePatchPakFilename);
				...
				{
					UE_LOGF(LogPakFile, Warning, "Unable to generate hashes from pak file %ls", *CmdLineParameters.SourcePatchPakFilename);
					...
					{
						UE_LOGF(LogPakFile, Warning, "Unable to extract files from source pak file for patch");
					}
					else
					{
				...
				if (UsedEncryptionKeys.Num() == 1)
				{
					...
					{
						UE_LOGF(LogPakFile, Display, "Found encryption key %ls in pak file %ls will use to encrypt patch", *((UsedEncryptionKeys[0]).ToString()), *CmdLineParameters.SourcePatchPakFilename);
			...
			if (CmdLineParameters.GeneratePatch)
			{
				...
				if (LowestSourcePakVersion < FPakInfo::PakFile_Version_DeleteRecords)
				{
					...
					UE_LOGF(LogPakFile, Display, "Some patch source paks were generated with an earlier version of UnrealPak that didn't support delete records. checking for historic assets that have moved between chunks to avoid creating invalid delete records");
	...
	UE_LOGF(LogPakFile, Error, "No pak file name specified. Usage:");
	UE_LOGF(LogPakFile, Error, "  UnrealPak <PakFilename> -Test");
	UE_LOGF(LogPakFile, Error, "  UnrealPak <PakFilename> -Verify");
	UE_LOGF(LogPakFile, Error, "  UnrealPak <PakFilename> -Info");
	UE_LOGF(LogPakFile, Error, "  UnrealPak <PakFilename> -List [-ExcludeDeleted]");
	UE_LOGF(LogPakFile, Error, "  UnrealPak <PakFilename> <GameUProjectName> <GameFolderName> -ExportDependencies=<OutputFileBase> -NoAssetRegistryCache -ForceDependsGathering");
	UE_LOGF(LogPakFile, Error, "  UnrealPak <PakFilename> -Extract <ExtractDir> [-Filter=<filename>]");
	UE_LOGF(LogPakFile, Error, "  UnrealPak <PakFilename> -Create=<ResponseFile> [Options]");
	UE_LOGF(LogPakFile, Error, "  UnrealPak <PakFilename> -Dest=<MountPoint>");
	UE_LOGF(LogPakFile, Error, "  UnrealPak <PakFilename> -Repack [-Output=Path] [-ExcludeDeleted] [Options]");
	UE_LOGF(LogPakFile, Error, "  UnrealPak <PakFilename1> <PakFilename2> -diff");
	UE_LOGF(LogPakFile, Error, "  UnrealPak <PakFolder> -AuditFiles [-OnlyDeleted] [-CSV=<filename>] [-order=<OrderingFile>] [-SortByOrdering]");
	UE_LOGF(LogPakFile, Error, "  UnrealPak <PakFilename> -WhatsAtOffset [offset1] [offset2] [offset3] [...]");
	UE_LOGF(LogPakFile, Error, "  UnrealPak <PakFolder> -GeneratePIXMappingFile -OutputPath=<Path>");
	UE_LOGF(LogPakFile, Error, "  Options:");
	UE_LOGF(LogPakFile, Error, "    -blocksize=<BlockSize>");
	UE_LOGF(LogPakFile, Error, "    -bitwindow=<BitWindow>");
	UE_LOGF(LogPakFile, Error, "    -compress");
	UE_LOGF(LogPakFile, Error, "    -encrypt");
	UE_LOGF(LogPakFile, Error, "    -order=<OrderingFile>");
	UE_LOGF(LogPakFile, Error, "    -diff (requires 2 filenames first)");
	UE_LOGF(LogPakFile, Error, "    -enginedir (specify engine dir for when using ini encryption configs)");
	UE_LOGF(LogPakFile, Error, "    -projectdir (specify project dir for when using ini encryption configs)");
	UE_LOGF(LogPakFile, Error, "    -cryptokeys=<cryptofile> (For example Crypto.json in cooked data)");
	UE_LOGF(LogPakFile, Error, "    -encryptionini (specify ini base name to gather encryption settings from)");
	UE_LOGF(LogPakFile, Error, "    -extracttomountpoint (Extract to mount point path of pak file)");
	UE_LOGF(LogPakFile, Error, "    -encryptindex (encrypt the pak file index, making it unusable in unrealpak without supplying the key)");
	UE_LOGF(LogPakFile, Error, "    -compressionformat[s]=<Format[,format2,...]> (set the format(s) to compress with, falling back on failures)");
	UE_LOGF(LogPakFile, Error, "    -encryptionkeyoverrideguid (override the encryption key guid used for encrypting data in this pak file)");
	UE_LOGF(LogPakFile, Error, "    -sign (generate a signature (.sig) file alongside the pak)");
	UE_LOGF(LogPakFile, Error, "    -fallbackOrderForNonUassetFiles (if order is not specified for ubulk/uexp files, figure out implicit order based on the uasset order. Generally applies only to the cooker order)");
