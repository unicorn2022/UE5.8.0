...
void FTargetDeviceServiceManager::LoadSettings()
{
	...
	for (FConfigSection::TConstIterator It(*OwnedDevices); It; ++It)
	{
		...
		{
			UE_LOGF(TargetDeviceServicesLog, Warning, "[TargetDeviceServices] failed to parse DeviceName in configuration setting: StartupServices=%ls", *ServiceString);
		...
		{
			UE_LOGF(TargetDeviceServicesLog, Warning, "[TargetDeviceServices] duplicate entry for : StartupServices=%ls", *ServiceString);
		...
		{
			UE_LOGF(TargetDeviceServicesLog, Warning, "[TargetDeviceServices] failed to create service for: StartupServices=%ls", *ServiceString);
		}
	}
}
