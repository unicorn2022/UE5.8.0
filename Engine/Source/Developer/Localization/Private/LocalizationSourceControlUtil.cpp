...
FLocalizationSCC::FLocalizationSCC(const EInitSCC InitSCC)
{
	checkf(IsInGameThread(), TEXT("FLocalizationSCC must be created on the game-thread"));

	if (InitSCC == EInitSCC::Always || (InitSCC == EInitSCC::Commandlet && IsRunningCommandlet()))
	{
...
FLocalizationSCC::~FLocalizationSCC()
{
	...
	{
		UE_LOGF(LogLocalizationSourceControl, Log, "Revision Control wrapper shutting down with checked out files.");
	}
...
TSharedPtr<FLocalizationSCC> FLocalizationSCC::CreateIfSCCEnabled(const EInitSCC InitSCC)
{
	checkf(IsInGameThread(), TEXT("FLocalizationSCC::CreateIfSCCEnabled must be called on the game-thread"));

	if (ISourceControlModule::Get().IsEnabled())
	{
		return MakeShared<FLocalizationSCC>(InitSCC);
	}
...
bool FLocalizationSCC::EndParallelTasks(FText& OutError)
{
	...
	{
		for (FString& DeferredCheckedOutFile : DeferredCheckedOutFilesArray)
		{
			CheckedOutFiles.Add(MoveTemp(DeferredCheckedOutFile));
		}
	}
