...
const FString FJsonInternationalizationManifestSerializer::TAG_DEVNOTES = TEXT("DevNotes");
...
bool FJsonInternationalizationManifestSerializer::DeserializeManifestFromString( const FString& InStr, TSharedRef< FInternationalizationManifest > Manifest, const FName PlatformName )
{
	...
	{
		UE_LOGF(LogInternationalizationManifestSerializer, Error, "Failed to parse manifest. %ls.", *Document.GetError().CreateMessage(InStr));
		return false;
	}
	...
	if (!RootObject.IsSet())
	{
		UE_LOGF(LogInternationalizationManifestSerializer, Error, "Failed to parse manifest. No root object.");
		return false;
	}
...
bool FJsonInternationalizationManifestSerializer::DeserializeManifestFromFile( const FString& InJsonFile, TSharedRef< FInternationalizationManifest > Manifest, const FName PlatformName )
{
	...
	{
		UE_LOGF(LogInternationalizationManifestSerializer, Error, "Failed to load manifest '%ls'.", *InJsonFile);
		return false;
	}
	...
	if (Document.HasError())
	{
		...
		UE_LOGF(LogInternationalizationManifestSerializer, Error, "Failed to parse manifest '%ls'. %ls.", *InJsonFile, *Document.GetError().CreateMessage(FileContents));
		return false;
	}
	...
	if (!RootObject.IsSet())
	{
		UE_LOGF(LogInternationalizationManifestSerializer, Error, "Failed to parse manifest '%ls'. No root object.", *InJsonFile);
		return false;
	}
...
bool FJsonInternationalizationManifestSerializer::SerializeManifestToFile( TSharedRef< const FInternationalizationManifest > Manifest, const FString& InJsonFile )
{
	...
	{
		UE_LOGF(LogInternationalizationManifestSerializer, Error, "Failed to serialize manifest '%ls'.", *InJsonFile);
		return false;
	}
	...
	{
		UE_LOGF(LogInternationalizationManifestSerializer, Error, "Failed to save manifest '%ls'.", *InJsonFile);
		return false;
	}
			...
			if (TOptional<UE::Json::FConstArray> ContextArray = UE::Json::GetArrayField(ChildJSONObject, *TAG_KEYCOLLECTION))
			{
				for (const UE::Json::FValue& ContextEntry : *ContextArray)
				{
					...
					if (TOptional<FStringView> Key = UE::Json::GetStringField(ContextJSONObject, *TAG_KEY))
					{
						...
						CommandContext.DevNotes = UE::Json::GetStringField(ContextJSONObject, *TAG_DEVNOTES).Get(TEXTVIEW(""));
						...
						if (!bAddSuccessful)
						{
							UE_LOGF(LogInternationalizationManifestSerializer, Warning, "Could not add JSON entry to the Internationalization manifest: Namespace:%ls SourceText:%ls SourceData:%ls",
...
void FJsonInternationalizationManifestSerializer::StructuredDataToJsonObj( TSharedPtr< const FStructuredEntry > InElement, UE::Json::FObject JsonObj, UE::Json::FAllocator& Allocator )
{
	...
	for (const TSharedPtr<FManifestEntry>& Entry : InElement->ManifestEntries)
	{
		...
		for (const FManifestContext& AContext : Entry->Contexts)
		{
			...
			if (!AContext.DevNotes.IsEmpty())
			{
				KeyJSONObject.AddMember(UE::Json::MakeStringRef(TAG_DEVNOTES), UE::Json::MakeStringValue(AContext.DevNotes, Allocator), Allocator);
			}
