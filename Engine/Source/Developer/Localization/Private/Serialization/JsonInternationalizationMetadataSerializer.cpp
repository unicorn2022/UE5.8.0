...
TSharedPtr<FLocMetadataValue> JsonValueToLocMetaDataValue(const UE::Json::FValue& JsonValue)
{
	switch (JsonValue.GetType())
	{
		...
		{
			return MakeShared<FLocMetadataValueString>(FString(UE::Json::ValueAsStringView(JsonValue)));
		}
		...
		case rapidjson::kObjectType:
		{
			...
			for (const UE::Json::FMember& ValueIter : JsonObject)
			{
				...
				{
					MetadataSubObject->SetField(FString(UE::Json::ValueAsStringView(ValueIter.name)), MetadataValue);
				}
			}
