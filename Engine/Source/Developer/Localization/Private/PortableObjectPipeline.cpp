...
namespace
{
			...
			for (const FManifestContext& Context : InManifestEntry->Contexts)
			{
				...
				if (ExistingEntry.IsValid())
				{
					...
					else
					{
						...
						UE_LOGF(LogPortableObjectPipeline, Warning, "%ls", *Message);
				...
				else
				{
					...
					{
						UE_LOGF(LogPortableObjectPipeline, Error, "Could not process localized string: %ls [%ls] %ls=\"%ls\" %ls.",
							*Context.SourceLocation,
							*CollapsedNamespace.GetString(),
							*Context.Key.GetString(),
							*InManifestEntry->Source.Text,
	...
	TMap<FPortableObjectEntryKey, FLocComments> ExtractPreservedPOComments(const FPortableObjectFormatDOM& InPortableObject)
	{
		...
		for (auto EntryPairIterator = InPortableObject.GetEntriesIterator(); EntryPairIterator; ++EntryPairIterator)
		{
			...
			{
				return !ExtractedComment.StartsWith(TEXT("Key:"), ESearchCase::CaseSensitive) && !ExtractedComment.StartsWith(TEXT("SourceLocation:"), ESearchCase::CaseSensitive) && !ExtractedComment.StartsWith(TEXT("InfoMetaData:"), ESearchCase::CaseSensitive) && !ExtractedComment.StartsWith(TEXT("DevNotes:"), ESearchCase::CaseSensitive);
			});
	...
	bool LoadPOFile(const FString& POFilePath, FPortableObjectFormatDOM& OutPortableObject)
	{
		...
		{
			UE_LOGF(LogPortableObjectPipeline, Log, "Could not find file %ls", *POFilePath);
			return false;
		}
		...
		{
			UE_LOGF(LogPortableObjectPipeline, Error, "Failed to load file %ls.", *POFilePath);
			return false;
		}
		...
		{
			UE_LOGF(LogPortableObjectPipeline, Error, "Failed to parse Portable Object file %ls: %ls", *POFilePath, *POErrorMsg.ToString());
			return false;
		}
	...
	bool ImportPortableObject(FLocTextHelper& InLocTextHelper, const FString& InCulture, const FString& InPOFilePath, const ELocalizedTextCollapseMode InTextCollapseMode, const EPortableObjectFormat InPOFormat, const FCollapsedData& InCollapsedData)
	{
		...
		{
			UE_LOGF(LogPortableObjectPipeline, Log, "Could not find file %ls", *InPOFilePath);
		...
		bool bModifiedArchive = false;
		{
			for (auto EntryPairIter = PortableObject.GetEntriesIterator(); EntryPairIter; ++EntryPairIter)
			{
				...
				{
					UE_LOGF(LogPortableObjectPipeline, Warning, "Portable Object entry has plural form we did not process.  File: %ls  MsgCtxt: %ls  MsgId: %ls", *InPOFilePath, *POEntry->MsgCtxt, *POEntry->MsgId);
				}
				...
				{
					UE_LOGF(LogPortableObjectPipeline, Log, "Could not import PO entry as it did not map to any known entries in the collapsed manifest data.  File: %ls  MsgCtxt: %ls  MsgId: %ls", *InPOFilePath, *POEntry->MsgCtxt, *POEntry->MsgId);
					continue;
				}
		...
		UE_LOGF(LogPortableObjectPipeline, Log, "Imported file %ls", *InPOFilePath);
		...
		if (bModifiedArchive)
		{
			...
			{
				UE_LOGF(LogPortableObjectPipeline, Error, "%ls", *SaveError.ToString());
				return false;
			}
		}
	...
	bool ExportPortableObject(FLocTextHelper& InLocTextHelper, const FString& InCulture, const FString& InPOFilePath, const ELocalizedTextCollapseMode InTextCollapseMode, const EPortableObjectFormat InPOFormat, TSharedRef<FInternationalizationManifest> InCollapsedManifest, const FCollapsedData& InCollapsedData, const bool bShouldPersistComments)
	{
		...
		{
			UE_LOGF(LogPortableObjectPipeline, Error, "Skipping export of culture %ls because it is not recognized PO language.", *InCulture);
			return false;
		}
		...
		for (FManifestEntryByStringContainer::TConstIterator ManifestIterator = InCollapsedManifest->GetEntriesBySourceTextIterator(); ManifestIterator; ++ManifestIterator)
		{
			...
			for (const FManifestContext& Context : ManifestEntry->Contexts)
			{
				...
				if (!Context.DevNotes.IsEmpty())
				{
					PoEntry->AddExtractedComment(GetConditionedDevNotesForExtractedComment(Context.DevNotes)); // "Notes from Programmer" in the form of the notes entered by the developer.
					}
#endif
		...
		const bool bPOFileSaved = UE::Localization::FileUtil::WriteFileIfModified(InPOFilePath,
			[&NewPortableObject](const FString& Filename)
			{
				//@TODO We force UTF8 at the moment but we want this to be based on the format found in the header info.
				return FFileHelper::SaveStringToFile(NewPortableObject.ToString(), *Filename, FFileHelper::EEncodingOptions::ForceUTF8);
			}, &UE::Localization::FileUtil::PortableObjectHash, InLocTextHelper.GetLocFileNotifies());

		if (!bPOFileSaved)
		{
			UE_LOGF(LogPortableObjectPipeline, Error, "Could not write file %ls", *InPOFilePath);
			return false;
		}

		UE_LOGF(LogPortableObjectPipeline, Log, "Exported file %ls", *InPOFilePath);
...
bool PortableObjectPipeline::Import(FLocTextHelper& InLocTextHelper, const FString& InCulture, const FString& InPOFilePath, const ELocalizedTextCollapseMode InTextCollapseMode, const EPortableObjectFormat InPOFormat)
{
	...
	{
		UE_LOGF(LogPortableObjectPipeline, Error, "PortableObjectPipeline::Import may only be used when not splitting platform data.");
		return false;
	}
...
bool PortableObjectPipeline::ImportAll(FLocTextHelper& InLocTextHelper, const FString& InPOCultureRootPath, const FString& InPOFilename, const ELocalizedTextCollapseMode InTextCollapseMode, const EPortableObjectFormat InPOFormat, const bool bUseCultureDirectory)
{
	...
	{
		UE_LOGF(LogPortableObjectPipeline, Error, "bUseCultureDirectory may only be used with a single culture when not splitting platform data.");
		return false;
	}
...
bool PortableObjectPipeline::Export(FLocTextHelper& InLocTextHelper, const FString& InCulture, const FString& InPOFilePath, const ELocalizedTextCollapseMode InTextCollapseMode, const EPortableObjectFormat InPOFormat, const bool bShouldPersistComments)
{
	...
	if (InLocTextHelper.ShouldSplitPlatformData())
	{
		UE_LOGF(LogPortableObjectPipeline, Error, "PortableObjectPipeline::Export may only be used when not splitting platform data.");
		return false;
	}
...
bool PortableObjectPipeline::ExportAll(FLocTextHelper& InLocTextHelper, const FString& InPOCultureRootPath, const FString& InPOFilename, const ELocalizedTextCollapseMode InTextCollapseMode, const EPortableObjectFormat InPOFormat, const bool bShouldPersistComments, const bool bUseCultureDirectory)
{
	...
	{
		UE_LOGF(LogPortableObjectPipeline, Error, "The export mode 'ELocalizedTextCollapseMode::IdenticalPackageIdTextIdAndSource' is no longer supported (it was deprecated in 4.15 and removed in 4.17). Please use 'ELocalizedTextCollapseMode::IdenticalTextIdAndSource' instead.");
		return false;
	}
...
FString PortableObjectPipeline::GetConditionedDevNotesForExtractedComment(const FString& DevNotes)
{
	return FString::Printf(TEXT("DevNotes:\t%s"), *ConditionArchiveStrForPO(DevNotes));
}
