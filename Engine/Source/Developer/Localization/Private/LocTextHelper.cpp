...
FLocTextHelper::FLocTextHelper(TSharedPtr<FInternationalizationManifest> InLoadedManifest, FString InTargetName, FString InTargetPath, FString InNativeCulture, TArray<FString> InForeignCultures, TSharedPtr<ILocFileNotifies> InLocFileNotifies, const ELocTextPlatformSplitMode InPlatformSplitMode)
	: FLocTextHelper(MoveTemp(InTargetPath),
		InTargetName + TEXT(".manifest"),
		InTargetName + TEXT(".archive"),
		MoveTemp(InNativeCulture),
		MoveTemp(InForeignCultures),
		MoveTemp(InLocFileNotifies),
		InPlatformSplitMode)
{
	TargetName = MoveTemp(InTargetName);
	Manifest = MoveTemp(InLoadedManifest);
}
...
bool FLocTextHelper::SaveConflictReport(const FString& InReportFilePath, EConflictReportFormat InConflictReportFormat, FText* OutError) const
{
	...
	const bool bSaved = UE::Localization::FileUtil::WriteFileIfModified(InReportFilePath,
		[&ConflictReport](const FString& Filename)
		{
			return FFileHelper::SaveStringToFile(ConflictReport, *Filename);
		}, &UE::Localization::FileUtil::TextHash, LocFileNotifies);
...
bool FLocTextHelper::SaveWordCountReport(const FDateTime& InTimestamp, const FString& InReportFilePath, FText* OutError) const
{
	...
	const bool bSaved = UE::Localization::FileUtil::WriteFileIfModified(InReportFilePath,
		[&WordCounts](const FString& Filename)
		{
			return FFileHelper::SaveStringToFile(WordCounts.ToCSV(), *Filename);
		}, &UE::Localization::FileUtil::TextHash, LocFileNotifies);
...
bool FLocTextHelper::SaveManifestImpl(const TSharedRef<const FInternationalizationManifest>& InManifest, const FString& InManifestFilePath, FText* OutError) const
{
	...
	{
		const bool bSaved = UE::Localization::FileUtil::WriteFileIfModified(InManifestFilePathToSave,
			[&InManifestToSave](const FString& Filename)
			{
				return FJsonInternationalizationManifestSerializer::SerializeManifestToFile(InManifestToSave, Filename);
			}, &UE::Localization::FileUtil::TextHash, LocFileNotifies);
...
bool FLocTextHelper::SaveArchiveImpl(const TSharedRef<const FInternationalizationArchive>& InArchive, const FString& InArchiveFilePath, FText* OutError) const
{
	...
	{
		const bool bSaved = UE::Localization::FileUtil::WriteFileIfModified(InArchiveFilePathToSave,
			[&InArchiveToSave](const FString& Filename)
			{
				return FJsonInternationalizationArchiveSerializer::SerializeArchiveToFile(InArchiveToSave, Filename);
			}, &UE::Localization::FileUtil::TextHash, LocFileNotifies);
