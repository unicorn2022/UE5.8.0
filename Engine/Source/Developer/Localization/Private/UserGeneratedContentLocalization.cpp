...
bool SetLocalizationTargetName(const TSharedRef<IPlugin>& Plugin, const FString& LocalizationTargetName, bool bUseSourceControl, FText& OutFailReason)
{
	...
	FLocFileSCCNotifies LocFileNotifies = CreateLocFileNotifies(bUseSourceControl);

	LocFileNotifies.PreFileWrite(Plugin->GetDescriptorFileName());
	const bool bResult = Plugin->UpdateDescriptor(PluginDescriptor, OutFailReason);
	LocFileNotifies.PostFileWrite(Plugin->GetDescriptorFileName());
	return bResult;
}
		...
		if (FPaths::FileExists(Filename))
		{
			if (USourceControlHelpers::CheckOutOrAddFile(Filename, /*bSilent*/true))
			{
					...
					PlatformFile.IsReadOnly(*Filename))
				{
					...
					{
						UE_LOGF(LogUGCLocalization, Error, "Failed to make file '%ls' writable", *Filename);
					}
				}
			}
			else
			{
				UE_LOGF(LogUGCLocalization, Error, "Failed to check out or add file '%ls'. %ls", *Filename, *USourceControlHelpers::LastErrorMsg().ToString());
			}
		}
	}
}
...
FLocFileSCCNotifies CreateLocFileNotifies(const bool bUseSourceControl)
{
	TSharedPtr<FLocalizationSCC> SourceControlInfo;
	if (bUseSourceControl)
	{
		SourceControlInfo = FLocalizationSCC::CreateIfSCCEnabled(FLocalizationSCC::EInitSCC::Never);
	}
	return FLocFileSCCNotifies{ SourceControlInfo };
}

void GetLocalizationFileHashes(const FString& BaseDirectory, TMap<FString, uint64>& OutFileHashes)
{
	// Get the list of localization files under the given directory
	TArray<TTuple<FString, uint64>> FilesToHash;
	IFileManager::Get().IterateDirectoryRecursively(*BaseDirectory, [&FilesToHash](const TCHAR* FilenameOrDirectory, bool bIsDirectory)
	{
		if (!bIsDirectory)
		{
			...
			{
				FilesToHash.Add(MakeTuple(FilenameOrDirectory, 0));
			}
		}
		return true;
	});
	...
	{
		TTuple<FString, uint64>& FileToHashPair = FilesToHash[Index];

		FileToHashPair.Value = FPathViews::GetExtension(FileToHashPair.Key) == TEXTVIEW("po")
			? UE::Localization::FileUtil::PortableObjectHash(FileToHashPair.Key)
			: UE::Localization::FileUtil::TextHash(FileToHashPair.Key);
	});

	// Append the content hashes to the result
	OutFileHashes.Reserve(OutFileHashes.Num() + FilesToHash.Num());
	for (TTuple<FString, uint64>& FileToHashPair : FilesToHash)
	{
...
bool ExportLocalization(TArrayView<const TSharedRef<IPlugin>> Plugins, const FExportLocalizationOptions& ExportOptions, TFunctionRef<int32(const FString&, FString&)> CommandletExecutor)
{
	...
	{
		UE_LOGF(LogUGCLocalization, Error, "Localization export options did not have a 'NativeCulture' set");
		return false;
	}
	...
	FileManager.MakeDirectory(*RootLocalizationScratchDirectory, /*bTree*/true);
	ON_SCOPE_EXIT
	{
		if (ExportOptions.bAutoCleanup)
		{
			// Delete the entire scratch directory
			FileManager.DeleteDirectory(*RootLocalizationScratchDirectory, /*bRequireExists*/false, /*bTree*/true);
		}
	};
	...
	TMap<FString, TMap<FString, uint64>> PerTargetLocalizationFileHashes;
	for (const TSharedRef<IPlugin>& Plugin : Plugins)
	{
		...
		TMap<FString, uint64>& LocalizationFileHashes = PerTargetLocalizationFileHashes.FindOrAdd(PluginLocalizationTargetName);
		...
		for (const TTuple<FString, uint64>& LocalizationFileHashPair : LocalizationFileHashes)
		{
			...
			{
				if (FileManager.Copy(*DestinationFilename, *SourceFilename) == COPY_OK)
				{
					// Preserve the source timestamp, as MergeLocalization relies on this
					FileManager.SetTimeStamp(*DestinationFilename, FileManager.GetTimeStamp(*SourceFilename));
					UE_LOGF(LogUGCLocalization, Display, "Imported existing file for '%ls': %ls", *PluginLocalizationTargetName, *SourceFilename);
				}
				else
				{
					UE_LOGF(LogUGCLocalization, Error, "Failed to import existing file for '%ls': %ls", *PluginLocalizationTargetName, *SourceFilename);
					return false;
				}
			}
		}
		if (!ExportOptions.LocalizationTargetRootDirectoryOverride.IsEmpty() && ExportOptions.MergeProjectDataWithRootDirectoryOverrideData.IsSet())
		{
			...
			if (!MergeLocalization(PluginLocalizationTargetName, PluginLocalizationSourceDirectory, PluginLocalizationTargetName, PluginLocalizationScratchDirectory, ExportOptions.MergeProjectDataWithRootDirectoryOverrideData.GetValue(), /*bOnlyIfSourceFileNewer*/true, /*bUseSourceControl*/false))
			{
				return false;
			}
		}
		...
		// Build the gather config
		{
			...
			// Write config
			{
				...
				{
					UE_LOGF(LogUGCLocalization, Error, "Failed to write gather config for '%ls': %ls", *PluginLocalizationTargetName, *GatherConfigFilename);
					return false;
				}
			}
		}
	...
	if (GatherConfigFilenames.Num() > 0)
	{
		...
		else
		{
			...
			{
				bGatherFailed = false;
				UE_LOGF(LogUGCLocalization, Warning, "Localization commandlet finished with a non-zero exit code, but GatherText finished with a zero exit code. Considering the export a success, but there may be errors or omissions in the exported data.");
			}
		}

		// Log the output and result of the commandlet
		{
			UE_LOGF(LogUGCLocalization, Display, "Localization commandlet finished with exit code %d", ReturnCode);
			...
			if (bGatherFailed || UserGeneratedContentLocalization::bAlwaysExportFullGatherLog)
			{
				...
				{
					UE_LOGF(LogUGCLocalization, Display, "    %ls", *CommandletOutputLine);
				}
			}
		}
	...
	FLocFileSCCNotifies LocFileNotifies = CreateLocFileNotifies(ExportOptions.LocalizationTargetRootDirectoryOverride.IsEmpty());

	LocFileNotifies.BeginParallelTasks(); // Batch the below SCC operations
	ON_SCOPE_EXIT
	{
		LocFileNotifies.EndParallelTasks(); // End the SCC batch
	};
	...
	for (const TSharedRef<IPlugin>& Plugin : Plugins)
	{
		...
		{
			UE_LOGF(LogUGCLocalization, Warning, "Failed to write updated .ugcloc file for '%ls': %ls", *PluginLocalizationTargetName, *UGCLocFilename);
		}
		...
		TMap<FString, uint64> ScratchLocalizationFileHashes;
		GetLocalizationFileHashes(PluginLocalizationScratchDirectory, ScratchLocalizationFileHashes);

		const TMap<FString, uint64>& SourceLocalizationFileHashes = PerTargetLocalizationFileHashes.FindChecked(PluginLocalizationTargetName);
		for (const TTuple<FString, uint64>& LocalizationFileHashPair : ScratchLocalizationFileHashes)
		{
			const FString& SourceFilename = LocalizationFileHashPair.Key;
			FString DestinationFilename = SourceFilename;
			if (DestinationFilename.ReplaceInline(*PluginLocalizationScratchDirectory, *PluginLocalizationTargetDirectory) > 0)
			{
				...
				if (const uint64* SourceFileHash = SourceLocalizationFileHashes.Find(DestinationFilename);
					!SourceFileHash || *SourceFileHash != LocalizationFileHashPair.Value)
				{
					LocFileNotifies.PreFileWrite(DestinationFilename);
					if (FileManager.Copy(*DestinationFilename, *SourceFilename) == COPY_OK)
					{
						LocFileNotifies.PostFileWrite(DestinationFilename);
						UE_LOGF(LogUGCLocalization, Display, "Updated file for '%ls': %ls", *PluginLocalizationTargetName, *DestinationFilename);
					}
					else
					{
						UE_LOGF(LogUGCLocalization, Warning, "Failed to update file for '%ls': %ls", *PluginLocalizationTargetName, *DestinationFilename);
					}
				}
			}
		}
		...
		if (ExportOptions.bUpdatePluginDescriptor && ExportOptions.LocalizationTargetRootDirectoryOverride.IsEmpty())
		{
			...
			if (!PluginDescriptor.LocalizationTargets.ContainsByPredicate([&PluginLocalizationTargetName](const FLocalizationTargetDescriptor& LocalizationTargetDescriptor) { return LocalizationTargetDescriptor.Name == PluginLocalizationTargetName; }))
			{
				...
				{
					LocFileNotifies.PostFileWrite(Plugin->GetDescriptorFileName());
					UE_LOGF(LogUGCLocalization, Display, "Updated .uplugin file for '%ls'", *PluginLocalizationTargetName);
				}
				else
				{
					UE_LOGF(LogUGCLocalization, Warning, "Failed to update .uplugin file for '%ls': %ls", *PluginLocalizationTargetName, *DescriptorUpdateFailureReason.ToString());
				}
			}
		}
...
bool CompileLocalizationTarget(const FString& LocalizationTargetDirectory, const FLocTextHelper& LocTextHelper)
{
	...
	// Generate the LocMeta file
	{
		...
		if (FTextLocalizationResourceGenerator::GenerateLocMeta(LocTextHelper, LocResName, LocMeta))
		{
			...
			{
				UE_LOGF(LogUGCLocalization, Error, "Failed to save LocMeta file for '%ls'", *LocTextHelper.GetTargetName());
				return false;
			}
		}
		else
		{
			UE_LOGF(LogUGCLocalization, Error, "Failed to generate LocMeta file for '%ls'", *LocTextHelper.GetTargetName());
			return false;
		}
	}

	{
		std::atomic<bool> bAnySaveFailed{false};
		
		const TArray<FString>& AllCultures = LocTextHelper.GetAllCultures();
		const int32 NumCultures = AllCultures.Num();
		ParallelFor (NumCultures, [&bAnySaveFailed, &AllCultures, &LocTextHelper, &LocalizationTargetDirectory, &LocResName](const int32 Index)
			{
				if (bAnySaveFailed.load(std::memory_order_relaxed))
				{
					return;
				}
				
				const FString& CultureToGenerate = AllCultures[Index];
				...
				if (FTextLocalizationResourceGenerator::GenerateLocRes(LocTextHelper, CultureToGenerate, EGenerateLocResFlags::None, LocalizationTargetDirectory / CultureToGenerate / LocResName, LocRes, PerPlatformLocRes))
				{
					...
					{
						UE_LOGF(LogUGCLocalization, Error, "Failed to save LocRes file for '%ls' (culture '%ls')", *LocTextHelper.GetTargetName(), *CultureToGenerate);
						bAnySaveFailed.store(true, std::memory_order_relaxed);
						return;
					}
				}
				else
				{
					UE_LOGF(LogUGCLocalization, Error, "Failed to generate LocRes file for '%ls' (culture '%ls')", *LocTextHelper.GetTargetName(), *CultureToGenerate);
					bAnySaveFailed.store(true, std::memory_order_relaxed);
					return;
				}
			});
...
ELoadLocalizationResult LoadLocalization(const FString& LocalizationTargetName, const FString& LocalizationTargetDirectory, TSharedPtr<FLocTextHelper>& OutLocTextHelper, const FUserGeneratedContentLocalizationDescriptor* DefaultDescriptor)
{
	...
	{
		UE_LOGF(LogUGCLocalization, Error, "Failed to load localization descriptor for '%ls'", *LocalizationTargetName);
	...
	const TArray<FString>& AllCultures = OutLocTextHelper->GetAllCultures();
	{
		bool bHasPOFiles = false;
		for (const FString& CultureToGenerate : AllCultures)
		{
	...
	const FString NativeCulture = OutLocTextHelper->GetNativeCulture();
	
	// If the native PO file is missing then we effectively have no localization data, but consider that case a warning
	if (const FString POFilename = GetLocalizationTargetPOFile(OutLocTextHelper->GetTargetName(), LocalizationTargetDirectory, NativeCulture);
		!FPaths::FileExists(POFilename))
	{
		UE_LOGF(LogUGCLocalization, Warning, "Missing PO file for '%ls' (culture '%ls')", *OutLocTextHelper->GetTargetName(), *NativeCulture);
		return ELoadLocalizationResult::NoData;
	}
	...
	{
		UE_LOGF(LogUGCLocalization, Error, "Failed to load PO file for '%ls' (culture '%ls')", *OutLocTextHelper->GetTargetName(), *NativeCulture);
		return ELoadLocalizationResult::Failed;
	}
	
	{
		std::atomic<bool> bAnyImportFailed{false};
		const FString& OutLocTargetName = OutLocTextHelper->GetTargetName();
		ParallelFor(AllCultures.Num(), [&bAnyImportFailed, &AllCultures, &NativeCulture, &OutLocTargetName, &OutLocTextHelper, &LocalizationTargetDirectory, &UGCLocDescriptor](const int32 Index)
			{	
				// this is a simple early-out flag so 'relaxed' is sufficient
				if (bAnyImportFailed.load(std::memory_order_relaxed))
				{
					return;
				}
				
				const FString& Culture = AllCultures[Index];
				if (Culture == NativeCulture)
				{
					return;
				}
				
				// Quick existence check (fast)
				const FString POFilename = GetLocalizationTargetPOFile(OutLocTargetName, LocalizationTargetDirectory, Culture);
				if (!FPaths::FileExists(POFilename))
				{
					UE_LOGF(LogUGCLocalization, Warning, "Missing PO file for '%ls' (culture '%ls')", *OutLocTargetName, *Culture);
					return;
				}
				
				if (!ImportPortableObject(LocalizationTargetDirectory, Culture, UGCLocDescriptor.PoFormat, *OutLocTextHelper))
				{
					UE_LOGF(LogUGCLocalization, Error, "Failed to load PO file for '%ls' (culture '%ls')", *OutLocTargetName, *Culture);
					bAnyImportFailed.store(true, std::memory_order_relaxed);
				}
			});
...
bool MergeLocalization(const FString& SourceLocalizationTargetName, const FString& SourceLocalizationTargetDirectory, const FString& DestLocalizationTargetName, const FString& DestLocalizationTargetDirectory, const EMergeLocalizationMode MergeMode, const bool bOnlyIfSourceFileNewer, const bool bUseSourceControl)
{
	TMap<FString, uint64> SourceLocalizationFileHashes;
	...
	TMap<FString, uint64> DestLocalizationFileHashes;
	...
	auto LoadPOFile = [](const FString& POFilename, FPortableObjectFormatDOM& OutPOFile)
	{
		...
		{
			UE_LOGF(LogUGCLocalization, Error, "Merge failed to load '%ls'", *POFilename);
			return false;
		}
		return true;
	};

	auto SavePOFile = [&LocFileNotifies](const FString& POFilename, FPortableObjectFormatDOM& POFile)
	{
		FString POFileData;
		POFile.ToString(POFileData);

		LocFileNotifies.PreFileWrite(POFilename);
		if (FFileHelper::SaveStringToFile(POFileData, *POFilename, FFileHelper::EEncodingOptions::ForceUTF8))
		{
			LocFileNotifies.PostFileWrite(POFilename);
			return true;
		}
		else
		{
			UE_LOGF(LogUGCLocalization, Error, "Merge failed to save '%ls'", *POFilename);
			return false;
		}
	};
	...
	for (const TTuple<FString, uint64>& SourceLocalizationFileHashPair : SourceLocalizationFileHashes)
	{
		...
		if (const uint64* DestLocalizationFileHash = DestLocalizationFileHashes.Find(DestFilename))
		{
			if (SourceLocalizationFileHashPair.Value == *DestLocalizationFileHash)
			{
				// File hash is identical; nothing to merge
				UE_LOGF(LogUGCLocalization, Log, "Merge skipped '%ls' as it is identical to '%ls'", *SourceFilename, *DestFilename);
				continue;
			}
			
			if (bOnlyIfSourceFileNewer)
			{
				const FDateTime SourceTimestamp = FileManager.GetTimeStamp(*SourceFilename);
				const FDateTime DestTimestamp = FileManager.GetTimeStamp(*DestFilename);
				if (SourceTimestamp <= DestTimestamp)
				{
					// Source file is older; nothing to merge
					UE_LOGF(LogUGCLocalization, Log, "Merge skipped '%ls' as it is older than '%ls'", *SourceFilename, *DestFilename);
					continue;
				}
			}
			
			if (FPathViews::GetExtension(SourceFilename) == TEXTVIEW("po"))
			{
		...
		else
		{
			...
			LocFileNotifies.PreFileWrite(DestFilename);
			if (FileManager.Copy(*DestFilename, *SourceFilename) == COPY_OK)
			{
				LocFileNotifies.PostFileWrite(DestFilename);
				UE_LOGF(LogUGCLocalization, Log, "Merge copied '%ls' to '%ls'", *SourceFilename, *DestFilename);
			}
			else
			{
				UE_LOGF(LogUGCLocalization, Error, "Merge failed to copy '%ls' to '%ls'", *SourceFilename, *DestFilename);
				return false;
			}
		}
	}

	ParallelFor(TEXT("UserGeneratedContentLocalization.MergeLocalization"), POFilesToMerge.Num(), 1, [&POFilesToMerge, &LoadPOFile, &SavePOFile, MergeMode](int32 Index)
	{
		FPOFileToMerge& POFileToMerge = POFilesToMerge[Index];

		FPortableObjectFormatDOM SourcePOFile;
		FPortableObjectFormatDOM DestPOFile;
	...
	for (FPOFileToMerge& POFileToMerge : POFilesToMerge)
	{
		...
		UE_LOGF(LogUGCLocalization, Log, "Merge applied '%ls' to '%ls'", *POFileToMerge.SourceFilename, *POFileToMerge.DestFilename);
	}
...
void CleanupLocalization(TArrayView<const TSharedRef<IPlugin>> PluginsToClean, TArrayView<const TSharedRef<IPlugin>> PluginsToRemove, const FUserGeneratedContentLocalizationDescriptor& DefaultDescriptor, const bool bSilent)
{
	...
	if (LocalizationFilesToCleanup.Num() > 0)
	{
		...
		if (bSilent || FMessageDialog::Open(EAppMsgType::YesNo, GetCleanupLocalizationMessage(), LOCTEXT("CleanupLocalization.Title", "Cleanup localization data?")) == EAppReturnType::Yes)
		{
			...
			{
				FLocFileSCCNotifies LocFileNotifies = CreateLocFileNotifies();
