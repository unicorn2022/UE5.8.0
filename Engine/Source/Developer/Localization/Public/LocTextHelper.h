...
public:
	...
	/**
	 * Initialize helper with already loaded manifest.
	 * @note Suitable for dealing with in-memory manifest (but not limited to).
	 *
	 * @param InLoadedManifest		A pre-loaded manifest for intrusive LocTextHelper initialization.
	 * @param InTargetName			The name of the loc target. Used to infer ManifestName and ArchiveName.
	 * @param InTargetPath			Path to the localization target the pre-loaded manifest refers or should be saved to.
	 * @param InNativeCulture		Culture code of the native culture (eg, en), or an empty string if the native culture is unknown.
	 * @param InForeignCultures		Array of culture codes for the foreign cultures (the native culture will be removed from this array if present).
	 * @param InLocFileNotifies		Interface for allowing source control integration (may be null).
	 * @param InPlatformSplitMode	Should we split localization data per-platform?
	 */
	UE_API FLocTextHelper(TSharedPtr<FInternationalizationManifest> InLoadedManifest, FString InTargetName, FString InTargetPath, FString InNativeCulture, TArray<FString> InForeignCultures, TSharedPtr<ILocFileNotifies> InLocFileNotifies, const ELocTextPlatformSplitMode InPlatformSplitMode = ELocTextPlatformSplitMode::None);
