 ...
 */
UE_DEPRECATED(5.8, "PreWriteFileWithSCC is deprecated in favor of CreateLocFileNotifies, as the latter can batch SCC requests via BeginParallelTasks/EndParallelTasks")
 ...
 */
UE_DEPRECATED(5.8, "PostWriteFileWithSCC is deprecated in favor of CreateLocFileNotifies, as the latter can batch SCC requests via BeginParallelTasks/EndParallelTasks")
LOCALIZATION_API void PostWriteFileWithSCC(const FString& Filename, const bool bUseSourceControl = true);

/**
 * Create a loc file notifies helper that will use source control if currently enabled.
 */
LOCALIZATION_API FLocFileSCCNotifies CreateLocFileNotifies(const bool bUseSourceControl = true);
 ...
 * @param bOnlyIfSourceFileNewer			True if we should only merge from source -> dest if the source file is newer than the dest file.
 ...
 */
LOCALIZATION_API bool MergeLocalization(const FString& SourceLocalizationTargetName, const FString& SourceLocalizationTargetDirectory, const FString& DestLocalizationTargetName, const FString& DestLocalizationTargetDirectory, const EMergeLocalizationMode MergeMode, const bool bOnlyIfSourceFileNewer, const bool bUseSourceControl = true);
