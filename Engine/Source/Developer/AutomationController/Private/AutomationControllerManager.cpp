...
void FAutomationResultEmitter::EmitTestStarted(const FString& Name, const FString& Path)
{
	UE_LOG(LogAutomationController, Display, AutomationTestStarting, *Name, *Path);
}

void FAutomationResultEmitter::EmitTestCompleted(const FString& State, const FString& Name, const FString& Path)
{
	UE_LOG(LogAutomationController, Display, AutomationStateFormat, *State, *Name, *Path);
}

void FAutomationResultEmitter::EmitBeginEvents(const FString& Path)
{
	UE_LOG(LogAutomationController, Log, BeginEventsFormat, *Path);
}

void FAutomationResultEmitter::EmitEndEvents(const FString& Path)
{
	UE_LOG(LogAutomationController, Log, EndEventsFormat, *Path);
}

void FAutomationResultEmitter::EmitEntry(const FAutomationExecutionEntry& Entry)
{
...
FAutomationControllerManager::FAutomationControllerManager()
{
	...
	{
		UE_LOGF(LogAutomationController, Warning, "Argument -ReportOutputPath= is now -ReportExportPath=. Please update your command line!");
	}

	if (ReportURLPath.Len())
	{
		UE_LOGF(LogAutomationController, Warning, "Argument -DisplayReportOutputPath= is now -ReportURL=. Please update your command line!");
	}

	if (DeveloperPath.Len())
	{
		UE_LOGF(LogAutomationController, Warning, "Argument -DeveloperReportOutputPath= is now -DeveloperReport (no value). Please update your command line!");
	}
...
void FAutomationControllerManager::RequestTests()
{
	...
	for ( int32 ClusterIndex = 0; ClusterIndex < DeviceClusterManager.GetNumClusters(); ++ClusterIndex )
	{
		...
		if ( DevicesInCluster > 0 )
		{
			...
			UE_LOGF(LogAutomationController, Log, "Requesting test list from %ls", *MessageAddress.ToString());
...
void FAutomationControllerManager::RunTests(const bool bInIsLocalSession)
{
	...
	bRequiresReportGeneration = (TestReportGeneratedDelegate.IsBound() || !ReportExportPath.IsEmpty());
	if (bRequiresReportGeneration)
	{
		...
		if (bResumeRunTest)
		{
			if (!ReportExportPath.IsEmpty())
			{
				...
				{
					UE_LOGF(LogAutomationController, Warning, "Cannot load previous test results. ReportFileName '%ls' was not found.", *ReportFileName);
				}
			}
			else
			{
				UE_LOGF(LogAutomationController, Warning, "Cannot load previous test results. Report export path not provided.");
			}
		}
	...
	for ( int32 ClusterIndex = 0; ClusterIndex < DeviceClusterManager.GetNumClusters(); ++ClusterIndex )
	{
		...
		for ( int32 DeviceIndex = 0; DeviceIndex < DeviceClusterManager.GetNumDevicesInCluster(ClusterIndex); ++DeviceIndex )
		{
			...
			UE_LOGF(LogAutomationController, Log, "Sending StartTestSession to %ls", *MessageAddress.ToString());
	...
	UE_LOGF(LogAutomationController, Display, "Clearing %ls and %ls", *FPaths::AutomationLogDir(), *FPaths::AutomationTransientDir());
	...
	if ( DeviceClusterManager.HasActiveDevice() )
	{
		for (int32 ClusterIndex = 0; ClusterIndex < DeviceClusterManager.GetNumClusters(); ++ClusterIndex)
		{
			...
			for (int32 DeviceIndex = 0; DeviceIndex < DeviceClusterManager.GetNumDevicesInCluster(ClusterIndex); ++DeviceIndex)
			{
				...
				UE_LOGF(LogAutomationController, Log, "Sending StopTests to %ls", *MessageAddress.ToString());
				SendMessage(
...
void FAutomationControllerManager::ReportImageComparisonResult(const FAutomationWorkerImageComparisonResults& Result)
{
	...
	{
		UE_LOGF(LogAutomationController, Error, "Cannot generate screenshot report for screenshot %ls as report is missing", *Result.IncomingFilePath);
	}
}
	...
	if ( ComparisonQueue.Peek(Entry) )
	{
		if ( Entry->PendingComparison.IsReady() )
		{
			...
			// Send the message back to the automation worker letting it know the results of the comparison test.
			{
				...
				UE_LOGF(LogAutomationController, Log, "Sending ImageComparisonResult to %ls (IsNew=%d, AreSimilar=%d)",
...
{
	UE_LOGF(LogAutomationController, Log, "Test Pass Results:");
	for ( int32 i = 0; i < JsonTestPassResults.Tests.Num(); i++ )
	{
		UE_LOGF(LogAutomationController, Log, "%ls: %ls", *JsonTestPassResults.Tests[i].TestDisplayName, *AutomationStateToString(JsonTestPassResults.Tests[i].State));
	}
}
...
void FAutomationControllerManager::CollectTestResults(TSharedPtr<IAutomationReport> Report, const FAutomationTestResults& Results)
{
	if (JsonTestPassResults.IsRequired)
	{
		...
		// Only save the artifacts if there is a valid ReportExportPath
		if (!ReportExportPath.IsEmpty())
		{
...
bool FAutomationControllerManager::GenerateJsonTestPassSummary(FAutomatedTestPassResults& SerializedPassResults, FString& OutTestReportJson)
{
	UE_LOGF(LogAutomationController, Display, "Converting results to json object...");
	SerializedPassResults.ReportCreatedOn = FDateTime::UtcNow();

	if (FJsonObjectConverter::UStructToJsonObjectString(SerializedPassResults, OutTestReportJson))
	{
		return true;
	}
	else
	{
		UE_LOGF(LogAutomationController, Error, "Failed to convert test results to json object - No report will be generated.");
	}
		
	return false;
}

bool FAutomationControllerManager::SaveJsonTestPassSummary(const FString& TestReportJson)
{
	if (!TestReportJson.IsEmpty())
	{
		FString ReportFileName = FString::Printf(TEXT("%s/index.json"), *ReportExportPath);
		...
		{
			if (FFileHelper::SaveStringToFile(TestReportJson, *ReportFileName, FFileHelper::EEncodingOptions::ForceUTF8))
			{
				UE_LOGF(LogAutomationController, Display, "Successfully wrote json results file!");
				return true;
			}
		...
		UE_LOGF(LogAutomationController, Warning, "Failed to write test report json to '%ls' after 3 attempts - No report will be generated.", *ReportFileName);
	}
	else
	{
		UE_LOGF(LogAutomationController, Error, "Json is empty. No test results report has been generated.");
	}

	return false;
}

void FAutomationControllerManager::BroadcastJsonTestPassSummary(const FString& TestReportJson)
{
	if (!TestReportGeneratedDelegate.IsBound())
	{
		return;
	}
	...
	if (!TestReportJson.IsEmpty())
	{
		TestReportGeneratedDelegate.Broadcast(TestReportJson);
	}
	else
	{
		UE_LOGF(LogAutomationController, Error, "Json is empty. No test results report has been broadcast.");
	}
}

bool FAutomationControllerManager::GenerateTestPassHtmlIndex()
{
	UE_LOGF(LogAutomationController, Display, "Loading results html template...");
	...
	{		
		...
		for (int32 Attempt = 1; Attempt <= WriteAttempts; ++Attempt)
		{
			...
			{
				UE_LOGF(LogAutomationController, Display, "Successfully wrote html results file!");
				return true;
			}
		...
		UE_LOGF(LogAutomationController, Warning, "Failed to write test report html to '%ls' after 3 attempts - No report will be generated.", *ReportFileName);
	}
	else
	{
		UE_LOGF(LogAutomationController, Error, "Failed to load test report html template - No report will be generated.");
	}
...
bool FAutomationControllerManager::LoadJsonTestPassSummary(FString& ReportFilePath, TArray<IAutomationReportPtr> TestReports)
{
	...
	{
		UE_LOGF(LogAutomationController, Error, "Failed to read json results file '%ls'.", *ReportFilePath);
		return false;
	}
	if (!FJsonObjectConverter::JsonObjectStringToUStruct(Json, &JsonTestPassResults))
	{
		UE_LOGF(LogAutomationController, Error, "Failed to load json results file '%ls'.", *ReportFilePath);
		return false;
	}
	UE_LOGF(LogAutomationController, Display, "Successfully loaded json results file.");
	...
	for (IAutomationReportPtr& TestReport : TestReports)
	{
		...
		{
			UE_LOGF(LogAutomationController, Warning, "No match to test '%ls' in json results.", *TestReport->GetFullTestPath());
		}
	}
				...
				if (TestState == EAutomationState::NotRun)
				{
					...
					if (DeviceAddresses.Num() == NextTest->GetNumParticipantsRequired())
					{
						...
						for (int32 AddressIndex = 0; AddressIndex < DeviceAddresses.Num(); ++AddressIndex)
						{
							...
							UE_LOGF(LogAutomationController, Log, "Sending RunTest %ls to %ls", *NextTest->GetDisplayName(), *DeviceAddress.ToString());
						...
						if (JsonTestPassResults.IsRequired)
						{
							...
							if (!ReportExportPath.IsEmpty() && bResumeRunTest)
							{
								...
								FString Json;
								if (GenerateJsonTestPassSummary(JsonTestPassResults, Json))
								{
									SaveJsonTestPassSummary(Json);
								}
							}
						}
...
void FAutomationControllerManager::RemoveCallbacks()
{
	...
	TestReportGeneratedDelegate.Clear();
}
...
void FAutomationControllerManager::ProcessResults()
{
	...
	if ( JsonTestPassResults.IsRequired )
	{
		UE_LOGF(LogAutomationController, Display, "Exporting Automation Report.");
		...
		if (bRequiresReportGeneration)
		{
			// Generate Json
			FString Json;
			if (GenerateJsonTestPassSummary(SerializedPassResults, Json))
			{
				if (!ReportExportPath.IsEmpty())
				{
					FDateTime StepTime = FDateTime::Now();
					UE_LOGF(LogAutomationController, Log, "Writing reports to %ls...", *ReportExportPath);

					SaveJsonTestPassSummary(Json);
					UE_LOGF(LogAutomationController, Log, "Exported report to '%ls' in %.02f Seconds", *ReportExportPath, (FDateTime::Now() - StepTime).GetTotalSeconds());
				}

				if (TestReportGeneratedDelegate.IsBound())
				{
					BroadcastJsonTestPassSummary(Json);
				}
			}
			else
			{
				UE_LOGF(LogAutomationController, Error, "Failed to serialize test results into json. No test results report can be generated.");
			}
		}

		if (!ReportURLPath.IsEmpty())
		{
			UE_LOGF(LogAutomationController, Display, "Report can be viewed in a browser at '%ls'", DeveloperReportUrl.IsEmpty() ? *ReportURLPath : *DeveloperReportUrl);

			if (!DeveloperReportUrl.IsEmpty())
			{
				UE_LOGF(LogAutomationController, Display, "Launching Report URL %ls.", *DeveloperReportUrl);

				FPlatformProcess::LaunchURL(*DeveloperReportUrl, nullptr, nullptr);
			}
		}

		UE_LOGF(LogAutomationController, Display, "Report can be opened in the editor at '%ls'", ReportExportPath.IsEmpty() ? *FPaths::ConvertRelativePathToFull(FPaths::AutomationReportsDir()) : *ReportExportPath);
	}
...
void FAutomationControllerManager::UpdateTests()
{
		...
		if (CheckTestTimer > CheckTestIntervalSeconds)
		{
			for ( int32 Index = 0; Index < TestRunningArray.Num(); Index++ )
			{
				...
				if (TestRunningArray[Index].LastPingTime > GameInstanceLostTimerSeconds)
				{
					...
					UE_LOGF(LogAutomationController, Error, "Removing test and marking failure after timeout waiting for device %ls LastPingTime was greater than %.1f", *DeviceName, GameInstanceLostTimerSeconds);
					...
					if ( DeviceClusterManager.HasActiveDevice() == false )
					{
						...
						UE_LOGF(LogAutomationController, Display, "Module disabled");
						SetControllerStatus(EAutomationControllerModuleState::Disabled);
						ClusterDistributionMask = 0;
					}
					else
					{
						UE_LOGF(LogAutomationController, Display, "Module not disabled. Keep looking.");
	...
	{
		UE_LOGF(LogAutomationController, Log, "Ignoring very large delta of %.02f seconds in calls to FAutomationControllerManager::Tick() and not penalizing unresponsive tests", TickDelta);
	}
}
...
void FAutomationControllerManager::StopStartedTestSessions()
{
	...
	for (const auto& WorkerInstanceId : StartedTestSessionWorkerInstanceIdSet)
	{
		if (DeviceClusterManager.FindDevice(WorkerInstanceId, ClusterIndex, DeviceIndex))
		{
			...
			UE_LOGF(LogAutomationController, Log, "Sending StopTestSession to %ls", *MessageAddress.ToString());
			SendMessage(
...
{
	UE_LOGF(LogAutomationController, Log, "Received FindWorkersResponseMessage from %ls", *Context->GetSender().ToString());
	...
	{
		UE_LOGF(LogAutomationController, Verbose, "Already running test. Don't request tests from %ls", *Context->GetSender().ToString());
		return;
	}
...
{
	UE_LOGF(LogAutomationController, Log, "ReceivedScreenShot for %ls (%dx%d) from %ls", *Message.Metadata.TestName, Message.Metadata.Width, Message.Metadata.Height, *Context->GetSender().ToString());
	DeviceClusterManager.UpdateDeviceFromMessage(Context->GetSender(), Message);
	...
	{
		UE_LOGF(LogAutomationController, Error, "Failed to create directory %ls for incoming screenshot", *DirectoryPath);
		return;
	}
	...
	if (!FFileHelper::SaveArrayToFile(Message.ScreenImage, *IncomingFileName))
	{
		...
		UE_LOGF(LogAutomationController, Warning, "Fail to save screenshot to %ls. WriteError: %u (%ls)", *IncomingFileName, WriteErrorCode, WriteErrorBuffer);
		return;
	}
	...
	if (Message.FrameTrace.Num() > 0)
	{
		if (!FFileHelper::SaveArrayToFile(Message.FrameTrace, *TraceFileName))
		{
			...
			TCHAR WriteErrorBuffer[2048];
			FPlatformMisc::GetSystemErrorMessage(WriteErrorBuffer, 2048, WriteErrorCode);
			UE_LOGF(LogAutomationController, Warning, "Faild to save frame trace to %ls. WriteError: %u (%ls)", *TraceFileName, WriteErrorCode, WriteErrorBuffer);
		}
	}
...
{
	UE_LOGF(LogAutomationController, Log, "Received TestDataRequest from %ls", *Context->GetSender().ToString());
	DeviceClusterManager.UpdateDeviceFromMessage(Context->GetSender(), Message);
...
void FAutomationControllerManager::HandlePerformanceDataRequest(const FAutomationWorkerPerformanceDataRequest& Message, const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& Context)
{
	...
	UE_LOGF(LogAutomationController, Log, "Received PerformanceDataRequest from %ls", *Context->GetSender().ToString());
...
{
	UE_LOGF(LogAutomationController, Log, "Received RequestNextNetworkCommandMessage from %ls", *Context->GetSender().ToString());
	DeviceClusterManager.UpdateDeviceFromMessage(Context->GetSender(), Message);
...
{
	UE_LOGF(LogAutomationController, Log, "Received RequestTestsReplyCompleteMessage from %ls", *Context->GetSender().ToString());
	DeviceClusterManager.UpdateDeviceFromMessage(Context->GetSender(), Message);
	...
	UE_LOGF(LogAutomationController, Log, "%d tests available on %ls", TestInfo.Num(), *Context->GetSender().ToString());
...
void FAutomationControllerManager::ReportAutomationResult(const TSharedPtr<IAutomationReport> InReport, int32 ClusterIndex, int32 PassIndex)
{
	...
	{
		UE_LOGF(LogAutomationController, Error, "%ls", *ResultString);
	}
	else
	{
		UE_LOGF(LogAutomationController, Display, "%ls", *ResultString);
	}
	...
	for (const FAutomationExecutionEntry& Entry : Results.GetEntries())
	{
		...
		case EAutomationEventType::Info:
			UE_LOGF(LogAutomationController, Log, "%ls", *Entry.ToString());
#if WITH_EDITOR
			AutomationEditorLog.Info()->AddToken(TextToken);
#endif
			break;
		case EAutomationEventType::Warning:
			UE_LOGF(LogAutomationController, Warning, "%ls", *Entry.ToString());
#if WITH_EDITOR
			AutomationEditorLog.Warning()->AddToken(TextToken);
#endif
			break;
		case EAutomationEventType::Error:
			UE_LOGF(LogAutomationController, Error, "%ls", *Entry.ToString());
#if WITH_EDITOR
