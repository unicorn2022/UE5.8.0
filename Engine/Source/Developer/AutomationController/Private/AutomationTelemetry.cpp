...
void FAutomationTelemetry::HandleAddTelemetry(const FAutomationWorkerTelemetryData& Data)
{
	...
	UE_LOGF(LogAutomationTelemetry, Verbose, "Received request for %ls", *Data.TestName);

	FString StorageFilePath = GetStorageFilePath(Data.Storage);
	if (!InitiateStorage(StorageFilePath))
	{
		UE_LOGF(LogAutomationTelemetry, Error, "Could not create Telemetry file '%ls'", *StorageFilePath);
		return;
	}
	...
	{
		UE_LOGF(LogAutomationTelemetry, Error, "Could not write in Telemetry file '%ls'", *StorageFilePath);
	}
}
