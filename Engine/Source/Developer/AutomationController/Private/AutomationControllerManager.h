...
protected:
	...
	*/
	...
	/**
	 * Generates a serialized json string that contains all of our test report data
	 */
	bool GenerateJsonTestPassSummary(FAutomatedTestPassResults& SerializedPassResults, FString& OutTestReportJson);

	/**
	 * Create a json file that contains all of our test report data at /saved/automation/logs/AutomationReport-{CL}-{DateTime}.json
	 */
	bool SaveJsonTestPassSummary(const FString& TestReportJson);

	/**
	 * Braodcasts a serialized json string that contains all of our test report data
	 */
	void BroadcastJsonTestPassSummary(const FString& TestReportJson);
...
private:
	...
	/** If the test requires any sort of reporting to be handled (delegate or writing to file) */
	bool bRequiresReportGeneration = false;
