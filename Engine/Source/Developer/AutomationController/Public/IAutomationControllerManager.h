...
/** Delegate for receiving a test report upon test pass completion. */
DECLARE_MULTICAST_DELEGATE_OneParam(FOnAutomationControllerReportGenerated, const FString&);
...
public:
	...
	*/
	...
	/**
	* Gets a delegate that is invoked when the controller generates a report.
	*
	* @return The delegate.
	*/
	virtual FOnAutomationControllerReportGenerated& OnTestReportGenerated()
	{
		return TestReportGeneratedDelegate;
	}

public:

	/** Virtual destructor.*/
	virtual ~IAutomationControllerManager() = default;

protected:
	/** Holds a delegate that is invoked when the tests have generated a report. */
	FOnAutomationControllerReportGenerated TestReportGeneratedDelegate;
};
