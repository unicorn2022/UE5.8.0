...
TSharedPtr<SWidget> FCollisionAnalyzer::SummonUI() 
{
	...
	UE_LOGF(LogCollisionAnalyzer, Log, "Opening CollisionAnalyzer...");
	...
	{
		UE_LOGF(LogCollisionAnalyzer, Warning, "FCollisionAnalyzer::DisplayUI: Not in game thread.");
	}
...
void FCollisionAnalyzer::SaveCollisionProfileData(FString ProfileFileName)
{
	...
	if(FileWriter != nullptr)
	{
		...
		UE_LOGF(LogCollisionAnalyzer, Log, "Saved collision analyzer data to file '%ls'.", *ProfileFileName);
	}
	else
	{
		UE_LOGF(LogCollisionAnalyzer, Warning, "Unable to save collision analyzer data to file '%ls'.", *ProfileFileName);
	}
}
...
void FCollisionAnalyzer::LoadCollisionProfileData(FString ProfileFileName)
{
	...
	if (FileReader != nullptr)
	{
		...
		if(Magic == COLLISION_ANALYZER_MAGIC)
		{
			...
			if(Version == COLLISION_ANALYZER_VERSION)
			{
				...
				UE_LOGF(LogCollisionAnalyzer, Log, "Loaded collision analyzer data from file '%ls'.", *ProfileFileName);
	...
	if(!bSuccess)
	{
		UE_LOGF(LogCollisionAnalyzer, Warning, "Unable to load collision analyzer data from file '%ls'.", *ProfileFileName);
	}
}
