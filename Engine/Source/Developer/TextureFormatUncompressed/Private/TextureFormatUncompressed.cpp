...
class FTextureFormatUncompressed : public ITextureFormat
{
	...
	virtual EPixelFormat GetEncodedPixelFormat(const FTextureBuildSettings& BuildSettings, bool bImageHasAlphaChannel) const override
	{
		...
		UE_LOGF(LogTextureFormatUncompressed, Fatal, "Unhandled texture format '%ls' given to FTextureFormatUncompressed::GetEncodedPixelFormat()", *BuildSettings.TextureFormatName.ToString());
		return PF_Unknown;
	}
		...
		) const override
	{
		...
		{
			UE_LOGF(LogTextureFormatUncompressed, Warning,
				"Cannot convert uncompressed image to format '%ls'.",
