		...
		if (ITestRunner* TestRunner = ITestRunner::Get())
		{
			while (true)
			{
				if (CurrentTestLocal != CurrentTest)
				{
					...
					UE_CLOGF(CurrentTestLocal == CurrentTest, LogLowLevelTests, Error,
						"Timeout detected: Test case \"%s\" has been running for more than %d minute(s)", CurrentTest.c_str(), TestRunner->GetTimeoutMinutes());
...
public:
	...
	void testCaseEnded(const Catch::TestCaseStats& TestCaseStats) final
	{
		if (ITestRunner* TestRunner = ITestRunner::Get())
		{
			...
			UE_CLOGF(TestRunner->IsDebugMode(), LogLowLevelTests, Display,
				"Test took %ls",
				*DurationText);
