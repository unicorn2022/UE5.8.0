				...
				if (component->offset == INDEX_NONE)
				{
					for (int32 j = 0; j < temp_registers.Num(); ++j)
					{
						if (temp_registers[j] < op_idx || temp_registers[j] == INDEX_NONE)
						{
							...
							UE_LOGF(LogVVMBackend, Log, "OP:%d | Comp:%p allocated Reg: %d | Last Read: %d |", op_idx, component, j, component->last_read);
#endif
							break;
						}
					}
...
#if VM_VERBOSE_LOGGING
						UE_LOGF(LogVVMBackend, Log, "OP:%d | Comp:%p allocated Reg: %d | Last Read: %d |", op_idx, component, temp_registers.Num() - 1, component->last_read);
#endif
					}
...
#else
		Args.Add(FString::Printf(TEXT("__OP__%d"), EnumToUnderlyingType(op_code)));
#endif
