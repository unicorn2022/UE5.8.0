	...
	bool bSkipBackendOptimizations)
{
	...
	if (ErrorLog)
	{
		...
		UE_LOGF(LogVectorVMShaderCompiler, Warning, "Warnings while processing %ls", *Input.DebugGroupName);
		UE_LOGF(LogVectorVMShaderCompiler, Warning, "%ls", *VMCompilationOutput.Errors);
...
bool CompileVectorVMShader(const FShaderCompilerInput& Input, const FShaderPreprocessOutput& PreprocessOutput, FShaderCompilerOutput& Output)
{
	FVectorVMCompilationOutput CompilationOutput;
	bool bResult = CompileVectorVMShader(Input, PreprocessOutput, CompilationOutput, Input.Environment.CompilerFlags.Contains(CFLAG_SkipOptimizations));
	...
	bool bSkipBackendOptimizations)
{
	...
	return CompileVectorVMShader(Input, PreprocessOutput, VMCompilationOutput, bSkipBackendOptimizations);
}
