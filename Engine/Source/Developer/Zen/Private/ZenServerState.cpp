...
bool ZenServerState::IsProcessRunning(uint32 Pid)
{
	...
	if (!Handle)
	{
		...
		UE_LOGF(LogZenServiceState, Warning, "Failed to open running process %d: %d, assuming it is not running", Pid, Error);
		return false;
	}
	...
	{
		DWORD Error = GetLastError();
		UE_LOGF(LogZenServiceState, Warning, "Failed to get running process exit code %d: %d, assuming it is still running", Pid, Error);
		return true;
	}
	...
	if (Error == EPERM)
	{
		UE_LOGF(LogZenServiceState, Warning, "No permission to signal running process %d: %d, assuming it is running", Pid, Error);
		return true;
	}
	else if (Error == ESRCH)
	{
	...
	UE_LOGF(LogZenServiceState, Warning, "Failed to signal running process %d: %d, assuming it is running", Pid, Error);
	return true;
#endif
}
...
bool ZenServerState::Terminate(uint32 Pid)
{
	...
	if (Handle == NULL)
	{
		...
		if (Error != ERROR_INVALID_PARAMETER)
		{
			UE_LOGF(LogZenServiceState, Warning, "Failed to open running process for terminate %d: %d", Pid, Error);
			return false;
		}
		return true;
	}
	...
	{
		DWORD Error = GetLastError();
		UE_LOGF(LogZenServiceState, Warning, "Failed to terminate running process %d: %d", Pid, Error);
		return false;
	}
	...
	if (!bSuccess)
	{
		...
		{
			DWORD Error = GetLastError();
			UE_LOGF(LogZenServiceState, Warning, "Failed to wait for terminated process %d: %d", Pid, Error);
		}
		return false;
	}
#elif PLATFORM_UNIX || PLATFORM_MAC
	...
	if (Res != 0)
	{
		...
		if (LastError != ESRCH)
		{
			UE_LOGF(LogZenServiceState, Warning, "Failed to terminate running process %d: %d", Pid, LastError);
			return false;
		}
	}
#endif
...
bool ZenSharedEvent::Create()
{
	...
	if (m_EventHandle == NULL)
	{
		...
		UE_LOGF(LogZenServiceState, Warning, "Failed creating named event '%ls' (err: %d)", *FullEventName, LastError);
		return false;
	}
	return true;
#elif PLATFORM_UNIX || PLATFORM_MAC
	...
	{
		int LastError = errno;
		UE_LOGF(LogZenServiceState, Warning, "Failed to create named event '%s' (err: %d)", *EventPath, LastError);
		return false;
	}
	fchmod(m_Fd, 0666);
	...
	if (IpcKey < 0)
	{
		...
		close(m_Fd);
		m_Fd = -1;
		UE_LOGF(LogZenServiceState, Warning, "Failed to create an SysV IPC key for named event '%s' (err: %d)", *EventPath, LastError);
		return false;
	}
	...
	if (m_Semaphore < 0)
	{
		...
		close(m_Fd);
		m_Fd = -1;
		UE_LOGF(LogZenServiceState, Warning, "Failed creating an SysV semaphore for named event '%s' (err: %d)", *EventPath, LastError);
		return false;
	}
...
bool ZenSharedEvent::Exists() const
{
	...
	if (EventHandle == NULL)
	{
		...
		{
			UE_LOGF(LogZenServiceState, Warning, "Failed checking existance of named event '%ls' (err: %d)", *FullEventName, LastError);
		}
		return false;
	}
	else
	...
	if (Semaphore < 0)
	{
		...
		if (LastError != ENOENT)
		{
			UE_LOGF(LogZenServiceState, Warning, "Failed checking named event '%s' (err: %d)", *EventPath, LastError);
		}
		return false;
	}
	return true;
...
bool ZenSharedEvent::Open()
{
	...
	if (m_EventHandle == NULL)
	{
		...
		if (LastError != ERROR_FILE_NOT_FOUND)
		{
			UE_LOGF(LogZenServiceState, Warning, "Failed opening named event '%ls' (err: %d)", *FullEventName, LastError);
		}
		return false;
	}
	return true;
	...
	if (m_Semaphore < 0)
	{
		...
		if (LastError != ENOENT)
		{
			UE_LOGF(LogZenServiceState, Warning, "Failed opening named event '%s' (err: %d)", *EventPath, LastError);
		}
		return false;
	}
	return true;
...
bool ZenSharedEvent::Wait(int TimeoutMs)
{
	...
	{
		DWORD LastError = GetLastError();
		UE_LOGF(LogZenServiceState, Warning, "Failed waiting for named event '%ls' (err: %d)", *m_EventName, LastError);
		return false;
	}
...
bool ZenSharedEvent::Set()
{
	...
	{
		DWORD LastError = GetLastError();
		UE_LOGF(LogZenServiceState, Warning, "Failed signalling named event '%ls' (err: %d)", *m_EventName, LastError);
		return false;
	}
#elif PLATFORM_UNIX || PLATFORM_MAC
	check(m_Semaphore != -1);
...
ZenLockFileData::IsLockFileLocked(const TCHAR* FileName, bool bAttemptCleanUp)
{
	...
	if (bAttemptCleanUp)
	{
		...
		if (Fd < 0)
		{
			...
			UE_LOGF(LogZenServiceState, Warning, "Failed opening lock file '%s' (err: %d)", *LockFilePath, LastError);
			return true;
		}
		...
		if (LockRet < 0)
		{
			...
			{
				UE_LOGF(LogZenServiceState, Warning, "Failed locking lock file '%s' (err: %d)", *LockFilePath, LastError);
			}
			return true;
		}
	...
	UE_LOGF(LogZenServiceState, Warning, "Failed checking stat of '%s' (err: %d)", *LockFilePath, LastError);
	return true;
#endif
}
