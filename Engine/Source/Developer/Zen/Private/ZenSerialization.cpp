...
bool TryLoadCbPackage(FCbPackage& Package, FArchive& Ar, FCbBufferAllocator Allocator)
{
	...
	for (;;)
	{
		...
		if (!ValueField)
		{
			UE_LOGF(LogZenSerialization, Warning, "Package is malformed, can't read compact binary data");
			Ar.SetError();
			return false;
		}
		...
		else if (ValueField.IsBinary())
		{
			...
			if (View.GetSize() > 0)
			{
				...
				{
					UE_LOGF(LogZenSerialization, Warning, "Package attachment has malformed/invalid attachment hash field");
					Ar.SetError();
					return false;
				}
		...
		else
		{
			...
			{
				UE_LOGF(LogZenSerialization, Warning, "Package attachment has malformed/invalid object field");
				Ar.SetError();
				return false;
			}
			...
			if (Object)
			{
				...
				{
					UE_LOGF(LogZenSerialization, Warning, "Package attachment has malformed/invalid object hash field");
					Ar.SetError();
					return false;
				}
	...
	for (const CbAttachmentEntry& Entry : AttachmentEntries)
	{
		...
		if (Entry.Flags & CbAttachmentEntry::kIsLocalRef)
		{
			...
			if (!(Entry.Flags & CbAttachmentEntry::IsCompressed))
			{
				...
				UE_LOGF(LogZenSerialization, Error, "Attachment data is not compressed");
				Ar.SetError();
				return false;
			}
			...
			else
			{
				...
				if (!ChunkReader)
				{
					...
					UE_LOGF(LogZenSerialization, Warning, "Unable to read local file '%ls', treating it as a missing attachment", *Path);
					++Index;
					continue;
				}
			...
			if (!CompBuf)
			{
				...
				UE_LOGF(LogZenSerialization, Error, "Local file '%ls' format is not compressed", *Path);
				Ar.SetError();
				return false;
			}
