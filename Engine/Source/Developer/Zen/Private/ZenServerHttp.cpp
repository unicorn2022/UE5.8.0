...
#define UE_ZENDDC_HTTP_DEBUG					0
	...
	FZenHttpRequest::FZenHttpRequest(const FZenServiceEndpoint& InEndpoint, bool bInLogErrors, uint32 InConnectTimeoutMS, uint32 InTimeoutMS)
		: bLogErrors(bInLogErrors)
		, ConnectTimeoutMS(InConnectTimeoutMS)
	...
	void FZenHttpRequest::Reset()
	{
		...
		curl_easy_setopt(Curl, CURLOPT_CONNECTTIMEOUT_MS, ConnectTimeoutMS > 0 ? ConnectTimeoutMS : 5000L);
		if (TimeoutMS > 0)
		{
		...
		curl_easy_setopt(Curl, CURLOPT_UPLOAD_BUFFERSIZE, 2 * 1024L * 1024L); // Max buffer size is 2MB. 
		// Response functions
	...
	void FZenHttpRequest::LogResult(long InResult, const TCHAR* Uri, RequestVerb Verb) const
	{
		...
		if (Result == CURLE_OK)
		{
			...
			if (bSuccess)
			{
				UE_LOGF(
					LogZenHttp,
					Verbose,
					"Finished %ls zen data (response %ld) from %ls. %ls",
					VerbStr,
					ResponseCode,
					Uri,
					*AdditionalInfo
			...
			else if (bLogErrors)
			{
				...
				UE_LOGF(
					LogZenHttp,
					Error,
					"Failed %ls zen data (response %ld) from %ls. Response: %ls",
					VerbStr,
					ResponseCode,
					Uri,
					*Response
		...
		else if (bLogErrors)
		{
			UE_LOGF(
				LogZenHttp,
				Error,
				"Error while connecting to %ls: %ls",
				*Domain,
	...
	FString FZenHttpRequest::GetDetailedResultString(FStringView Uri) const
	{
		TStringBuilder<256> SB;
		if (CurlResult != CURLE_OK)
		{
			SB << TEXTVIEW("Failed: ") << GetError();
			SB << TEXTVIEW(" (") << CurlResult << TEXTVIEW(") ");
		}
		SB << TEXTVIEW("Url: ") << Uri;

		if (ResponseCode != 0)
		{
			SB << TEXTVIEW(" Response: ") << LexResponseCodeToString(ResponseCode) << TEXTVIEW(" (") << ResponseCode << TEXTVIEW(")");
		}

		FString ResponseString = GetResponseAsString();
		if (!ResponseString.IsEmpty())
		{
			SB << TEXTVIEW(" \"") << ResponseString << TEXTVIEW("\"");
		}
		else if (int64 PayloadSize = ResponseBuffer.Num(); PayloadSize > 0)
		{
			SB << TEXTVIEW(" ") << PayloadSize << TEXTVIEW(" bytes");
		}
		return SB.ToString();
	}
			...
			case CURLINFO_TEXT:
				{
					...
					UE_LOGF(LogZenHttp, VeryVerbose, "CURL %p: '%ls'", Request, *DebugText);
				}
				break;

			case CURLINFO_HEADER_IN:
				UE_LOGF(LogZenHttp, VeryVerbose, "CURL %p: Received header (%zd bytes)", Request, DebugInfoSize);
				UE_LOGF(LogZenHttp, VeryVerbose, "CURL HEADER <<< %*s", uint32(DebugInfoSize), DebugInfo);
				break;

			case CURLINFO_HEADER_OUT:
				UE_LOGF(LogZenHttp, VeryVerbose, "CURL %p: Send header (%zd bytes)", Request, DebugInfoSize);
				UE_LOGF(LogZenHttp, VeryVerbose, "CURL HEADER >>> %*s", uint32(DebugInfoSize), DebugInfo);
				break;

			case CURLINFO_DATA_IN:
				UE_LOGF(LogZenHttp, VeryVerbose, "CURL %p: Received data (%zd bytes)", Request, DebugInfoSize);
				break;

			case CURLINFO_DATA_OUT:
				UE_LOGF(LogZenHttp, VeryVerbose, "CURL %p: Sent data (%zd bytes)", Request, DebugInfoSize);
				break;

			case CURLINFO_SSL_DATA_IN:
				UE_LOGF(LogZenHttp, VeryVerbose, "CURL %p: Received SSL data (%zd bytes)", Request, DebugInfoSize);
				break;

			case CURLINFO_SSL_DATA_OUT:
				UE_LOGF(LogZenHttp, VeryVerbose, "CURL %p: Sent SSL data (%zd bytes)", Request, DebugInfoSize);
				break;
		}
