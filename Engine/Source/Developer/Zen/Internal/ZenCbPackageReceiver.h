...
public:
	...
	ZEN_API bool ShouldRetry(FZenServiceInstance& ZenServiceInstance, IHttpResponse& LocalResponse, bool UseSoftRecovery) const;

	ZEN_API static FString GetPayloadAsString(EHttpMediaType ContentType, FMemoryView BodyView);

private:
	ZEN_API IHttpReceiver* OnCreate(IHttpResponse& Response) final;
	ZEN_API IHttpReceiver* OnComplete(IHttpResponse& Response) final;

	bool TryRecover(Zen::FZenServiceInstance& ZenServiceInstance, bool UseSoftRecovery) const;
