...
public:
	...
	void AddSubMenuEntryToMenuBuilder()
	{
		...
		else if (Block.SubMenuData.ConstructMenu.NewToolMenuWidget.IsBound() || Block.SubMenuData.ConstructMenu.OnGetContent.IsBound())
		{
			...
			FOnGetContent OnGetContent = UToolMenus::Get()->ConvertWidgetChoice(Block.SubMenuData.ConstructMenu, MenuData->Context, Block);
		...
		else if (BlockNameOverride == NAME_None)
		{
			...
			{
				UE_LOGF(LogToolMenus, Warning, "Submenu that has no name is missing required delegate: %ls", *MenuData->MenuName.ToString());
			}
		}
		else
		{
	...
	void AddStandardEntryToMenuBuilder()
	{
		...
		if (Block.Command.IsValid())
		{
			...
			{
				UE_LOGF(LogToolMenus, Error, "UI command not found for menu entry: %ls[%ls], menu: %ls",
	...
	void Populate()
	{
		...
		{
			UE_LOGF(LogToolMenus, Warning, "Menu '%ls', item '%ls', Menus do not support: %ls", *MenuData->MenuName.ToString(), *BlockNameOverride.ToString(), *UEnum::GetValueAsString(Block.Type));
		}
	};
...
TArray<UToolMenu*> UToolMenus::CollectHierarchy(const FName InName, const TMap<FName, FName>& UnregisteredParentNames)
{
	...
	while (CurrentMenuName != NAME_None)
	{
		...
		if (!Current)
		{
			UE_LOGF(LogToolMenus, Warning, "Failed to find menu: %ls for %ls", *AdjustedMenuName.ToString(), *InName.ToString());
			return TArray<UToolMenu*>();
		}

		if (Result.Contains(Current))
		{
			UE_LOGF(LogToolMenus, Warning, "Infinite loop detected in tool menu: %ls", *InName.ToString());
			return TArray<UToolMenu*>();
		}
...
void UToolMenus::AssembleMenuSection(UToolMenu* GeneratedMenu, const UToolMenu* Other, FToolMenuSection* DestSection, const FToolMenuSection& OtherSection)
{
	if (!DestSection)
	{
		UE_LOGF(LogToolMenus, Warning, "Trying to add to invalid section for menu: %ls, section: %ls. Default section info will be used instead.", *OtherSection.Owner.TryGetName().ToString(), *OtherSection.Name.ToString());
	}
		...
		while (GeneratedEntries.Num() > 0)
		{
			...
			if (GeneratedEntry.IsNonLegacyDynamicConstruct())
			{
				if (NumIterations++ > 5000)
				{
					...
					UE_LOGF(LogToolMenus, Warning, "Possible infinite loop for menu: %ls, section: %ls, block: %ls", *MenuName.ToString(), *OtherSection.Name.ToString(), *Block.Name.ToString());
					break;
				}
	...
	if (DestSection)
	{
		...
		while (RemainingBlocks.Num() > 0)
		{
			...
			if (NumHandled == 0)
			{
				...
				{
					UE_LOGF(LogToolMenus, Warning, "Menu item not found: '%ls' for insert: '%ls'", *Block.InsertPosition.Name.ToString(), *Block.Name.ToString());
				}
...
void UToolMenus::AssembleMenu(UToolMenu* GeneratedMenu, const UToolMenu* Other)
{
	...
	for (const FToolMenuSection& OtherSection : Other->Sections)
	{
		...
		while (GeneratedSections.Num() > 0)
		{
			if (GeneratedSections[0].IsNonLegacyDynamic())
			{
				if (NumIterations++ > 5000)
				{
					UE_LOGF(LogToolMenus, Warning, "Possible infinite loop for menu: %ls, section: %ls", *Other->MenuName.ToString(), *OtherSection.Name.ToString());
					break;
				}
	...
	while (RemainingSections.Num() > 0)
	{
		...
		if (NumHandled == 0)
		{
			...
			{
				UE_LOGF(LogToolMenus, Warning, "Menu section not found: '%ls' for insert: '%ls'", *RemainingSection.InsertPosition.Name.ToString(), *RemainingSection.Name.ToString());
			}
			break;
		}
	}
...
void UToolMenus::ApplyCustomizationAndProfiles(UToolMenu* GeneratedMenu)
{
	...
	if(ProfileContext)
	{
		for(const FName& ActiveProfile : ProfileContext->ActiveProfiles)
		{
			...
			{
				UE_LOGF(LogToolMenus, Verbose, "Menu Profile %ls for menu %ls not found!", *ActiveProfile.ToString(), *GeneratedMenu->GetMenuName().ToString());
...
void UToolMenus::ExtractChildBlocksFromSubMenu(UToolMenu* ParentMenu, FToolMenuEntry& InBlock, TArray<UE::ToolMenus::FSubBlockReference>& SubMenuBlocks)
{
	const bool bBuiltViaDelegate =
		ConvertWidgetChoice(InBlock.ToolBarData.ComboButtonContextMenuGenerator, ParentMenu->Context, InBlock).IsBound();
...
void UToolMenus::PopulateToolBarBuilderWithTopLevelChildren(FToolBarBuilder& ToolBarBuilder, UToolMenu* ParentMenu, FToolMenuEntry& InBlock, bool bAddSpaceAfterRaisedChildren)
{
	...
	while (SubMenuBlocks.Num() > 0)
	{
		...
		if (++NumIterations > 5000)
		{
			UE_LOGF(LogToolMenus, Warning,
				"Possible infinite loop for menu with section menu. parent menu: %ls, menu: %ls, block: %ls",
...
)
{
	...
	if (Block.Type == EMultiBlockType::ToolBarButton || (Block.Type == EMultiBlockType::MenuEntry && !Block.IsSubMenu()))
	{
		if (Block.Command.IsValid() && !Block.IsCommandKeybindOnly())
		{
			...
			{
				UE_LOGF(LogToolMenus, Verbose, "UI command not found for toolbar entry: %ls, toolbar: %ls",
		...
		if (Block.ToolBarData.OptionsDropdownData.IsValid())
		{
			FOnGetContent OnGetContent = ConvertWidgetChoice(
				Block.ToolBarData.OptionsDropdownData->MenuContentGenerator, MenuData->Context, Block);
	...
	else if (Block.Type == EMultiBlockType::ToolBarComboButton || (Block.Type == EMultiBlockType::MenuEntry && Block.IsSubMenu()))
	{
		...
		FOnGetContent OnGetContent = ConvertWidgetChoice(Block.ToolBarData.ComboButtonContextMenuGenerator, MenuData->Context, Block);
		...
		{
			OnGetContent = ConvertWidgetChoice(Block.SubMenuData.ConstructMenu, MenuData->Context, Block);
		}
		...
		TSharedPtr<FToolBarComboButtonBlock> ComboButtonBlock = ToolBarBuilder.AddComboButton(
			UIAction,
			OnGetContent,
			Block.Label,
			Block.ToolTip,
		...
		if (ComboButtonBlock.IsValid() && Block.MakeCustomWidget.IsBound())
		{
			TSharedRef<FMultiBox> MultiBox = ToolBarBuilder.GetMultiBox();
			...
			TSharedPtr<SWidget> CustomWidget = Block.MakeCustomWidget.Execute(MenuData->Context, EntryWidgetContext);
			if (CustomWidget.IsValid())
			{
				ComboButtonBlock->SetCustomWidget(CustomWidget);
			}
		}
	...
	{
		UE_LOGF(LogToolMenus, Warning, "Toolbar '%ls', item '%ls', Toolbars do not support: %ls",
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UToolMenus::PopulateToolBarBuilder);
...
FOnGetContent UToolMenus::ConvertWidgetChoice(const FNewToolMenuChoice& Choice, const FToolMenuContext& Context, const FToolMenuEntry& Block) const
{
	...
	{
		return FOnGetContent::CreateLambda([ToCall = Choice.NewMenuLegacy, Context, bShouldCloseWindowAfterMenuSelection = Block.bShouldCloseWindowAfterMenuSelection]()
		{
			if (ToCall.IsBound())
			{
				FMenuBuilder MenuBuilder(bShouldCloseWindowAfterMenuSelection, Context.CommandList, Context.GetAllExtenders());
...
void UToolMenus::ExecuteStringCommand(const FToolMenuStringCommand StringCommand, FName MenuName, FToolMenuContext Context)
{
	if (StringCommand.IsBound())
	{
		...
		{
			UE_LOGF(LogToolMenus, Warning, "Unknown string command handler type: '%ls'", *TypeName.ToString());
		}
...
UToolMenu* UToolMenus::RegisterMenu(const FName InName, const FName InParent, EMultiBoxType InType, bool bWarnIfAlreadyRegistered)
{
	if (UToolMenu* Found = FindMenu(InName))
	{
		...
		{
			UE_LOGF(LogToolMenus, Warning, "Menu already registered : %ls", *InName.ToString());
		}
...
FDelegateHandle UToolMenus::RegisterStartupCallback(const FSimpleMulticastDelegate::FDelegate& InDelegate)
{
	...
	else
	{
		...
		{
			InternalStartupCallbackHandle = FCoreDelegates::GetOnPostEngineInit().Add(FSimpleMulticastDelegate::FDelegate::CreateStatic(&UToolMenus::PrivateStartupCallback));
		}
...
void UToolMenus::UnregisterPrivateStartupCallback()
{
	if (InternalStartupCallbackHandle.IsSet())
	{
		...
		if (Handle.IsValid())
		{
			FCoreDelegates::GetOnPostEngineInit().Remove(Handle);
			Handle.Reset();
		}
	}
}
