...
TUniquePtr<FMapTestSpawner> FMapTestSpawner::CreateFromTempLevel(FTestCommandBuilder& InCommandBuilder)
{
	...
	{
		UE_LOGF(LogMapTest, Verbose, "Active PIE session '%ls' needs to be shutdown before a creation of a new level can occur.", *GUnrealEd->PlayWorld->GetMapName());
...
void FMapTestSpawner::AddWaitUntilLoadedCommand(FAutomationTestBase* TestRunner, TOptional<FTimespan> Timeout)
{
	...
	{
		UE_LOGF(LogMapTest, Warning, "CVar '%ls' was not found. Defaulting to %f seconds.", CQTestConsoleVariables::MapTestTimeoutName, CQTestConsoleVariables::MapTestTimeout);
...
void FMapTestSpawner::OnEndPlayMap()
{
	...
	{
		UE_LOGF(LogMapTest, Verbose, "Play session has ended.");
		GameWorld = nullptr;
		PieWorld = nullptr;
