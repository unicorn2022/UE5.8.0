...
bool FindAssets(const FARFilter& Filter, const FString& Name, TArray<FAssetData>& OutAssets)
{
	...
	{
		UE_LOGF(LogCqTestAssets, Error, "Error encountered while searching for asset.");
		return false;
	}
...
TOptional<FAssetData> FindAsset(const FARFilter& Filter, const FString& Name) 
{
	...
	{
		UE_LOGF(LogCqTestAssets, Warning, "Asset name '%ls' not found.", *Name);
	}
	else if (FoundAssets.Num() > 1)
	{
		UE_LOGF(LogCqTestAssets, Warning, "Duplicate assets were found. May use the wrong one.");
	}
...
namespace CQTestAssetHelper 
{
	...
	UClass* GetBlueprintClass(const FARFilter& Filter, const FString& Name)
	{
		...
		if (Asset.IsSet())
		{
			...
			UE_LOGF(LogCqTestAssets, Error, "Failed to load blueprint class for %ls", *Asset->AssetName.ToString());
		}
