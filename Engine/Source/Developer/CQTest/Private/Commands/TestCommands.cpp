...
bool CQTEST_API FWaitUntil::Update()
{
	if (!bHasTimerStarted)
	{
		...
		{
			UE_LOGF(LogCqTest, Log, "Starting %ls", Description);
		}
	}
	...
	if (Query())
	{
		if (Description)
		{
			UE_LOGF(LogCqTest, Log, "Finished %ls after %.0f milliseconds", Description, Elapsed.GetTotalMilliseconds());
		}
		return true;
	}
...
FTimespan FWaitUntil::MakeTimeout(TOptional<FTimespan> InTimeout)
{
	...
	{
		UE_LOGF(LogCqTest, Warning, "CVar '%ls' was not found. Defaulting to %f seconds.", CQTestConsoleVariables::CommandTimeoutName, CQTestConsoleVariables::CommandTimeout);
...
bool CQTEST_API FWaitDelay::Update()
{
	...
	if (FDateTime::UtcNow() >= EndTime)
	{
		if (Description)
		{
			UE_LOGF(LogCqTest, Log, "Finished %ls", Description);
		}
		return true;
	}
	return false;
...
bool CQTEST_API FExecute::Update()
{
	...
	if (Description)
	{
		UE_LOGF(LogCqTest, Log, "%ls %ls", bHasToRun? TEXT("Running") : TEXT("Skipping"), Description);
	}
	if (bHasToRun)
	{
		Func();
