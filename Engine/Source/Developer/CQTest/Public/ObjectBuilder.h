...
public:
	...
	TObjectBuilder<TUObject>& SetParam(const FName& InPropertyName, T InValue)
	{
		...
		if (!Object)
		{
			UE_LOGF(LogObjectBuilder, Error, "Tried to SetParam on property %ls after Spawning. ObjectBuilder can only be used to paramaterise pre-spawn", *InPropertyName.ToString());
			return *this;
		}
		...
		auto TrySet = [&](auto Property) {
			if (!Property)
			{
				UE_LOGF(LogObjectBuilder, Error, "Failed to find %ls property %ls on object %ls, check type and Property name are correct", *GetTypeName<Raw>(), *InPropertyName.ToString(), *Object->GetName());
				return;
			}
			if (!IsCompatibleWith<Raw>(*Property))
			{
				UE_LOGF(LogObjectBuilder, Error, "Type mismatch: Tried to set %ls property %ls on object %ls to a %ls", *Property->GetCPPType(nullptr, 0), *InPropertyName.ToString(), *Object->GetName(), *GetTypeName<Raw>());
				return;
			}
			...
			{
				UE_LOGF(LogObjectBuilder, Error, "Container type mismatch: Could not cast from %ls to %ls", *Property->GetCPPType(nullptr, 0), *GetTypeName<Raw>());
			}
		};
		...
		else
		{
			if (auto Property = FindFProperty<FProperty>(Object->GetClass(), InPropertyName))
			{
				...
				{
					UE_LOGF(LogObjectBuilder, Error, "Type mismatch: Tried to set %ls property %ls on object %ls to a %ls", *Property->GetCPPType(nullptr, 0), *InPropertyName.ToString(), *Object->GetName(), *this->template GetTypeName<Raw>());
				}
			}
			else
			{
	...
	T& Spawn(FTransform InTransform = FTransform::Identity) 
	{
		if (!Object)
		{
			UE_LOGF(LogObjectBuilder, Error, "Tried to spawn from builder multiple times.  Builder can only spawn a single object");
	...
	TObjectBuilder<TUObject>& AddComponentTo(TComponent* InComponentToAdd = nullptr) 
	{
		...
		if (!Object)
		{
			UE_LOGF(LogObjectBuilder, Error, "Tried to AddComponentTo actor after Spawning. ObjectBuilder can only be used to paramaterise pre-spawn");
			return *this;
		}
	...
	TObjectBuilder<TUObject>& AddChildActorComponentTo()
	{
		...
		if (!Object)
		{
			UE_LOGF(LogObjectBuilder, Error, "Tried to AddChildActorComponentTo actor after Spawning. ObjectBuilder can only be used to paramaterise pre-spawn");
			return *this;
		}
