...
{
	using PrintStream = TStringBuilder<64>;
}
...
namespace CQTestConvert
{
	...
	inline FString ToString(const T& Input)
	{
		...
		{
			// Fallback for pointer type
			return FString::Printf(TEXT("0x%p"), (void*)Input);
		}
		else
		{
	...
	inline FString ToString(const TOptional<TInnerType>& Input)
	{
		if (Input.IsSet())
		{
			return ToString(Input.GetValue());
		}
		else
		{
			return TEXT("[NotSet]");
