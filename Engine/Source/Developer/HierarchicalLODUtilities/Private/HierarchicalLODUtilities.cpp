...
struct FHLODBuildParams
{
		...
		, LODSetup(InLODSetup)
	{
		...
		{
			UE_LOGF(LogHierarchicalLODUtilities, Error, "Unsupported simplification method provided");
		}
	}
};
...
FHLODBuildResults GenerateHLODMesh_Approximate(const FHLODBuildParams& InBuildParams)
{
	...
	auto RenameNewAsset = [&PackagePath, &NewAssetNamePrefix, &InBuildParams](UObject* NewAsset)
	{
		...
		NewAsset->Rename(*AssetName, InBuildParams.AssetsOuter, REN_NonTransactional | REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
	};
...
bool FHierarchicalLODUtilities::BuildStaticMeshForLODActor(ALODActor* LODActor, UHLODProxy* Proxy, const FHierarchicalSimplification& LODSetup, UMaterialInterface* InBaseMaterial)
{
	...
	UE_LOGF(LogHierarchicalLODUtilities, Log, "Building Proxy Mesh for Cluster %ls", *LODActor->GetName());
	...
	if (AllComponents.Num() > 0)
	{
		...
		if (!HLODBuildResults.bDeferredResults)
		{
			...
			{
				UE_LOGF(LogHierarchicalLODUtilities, Warning, "No HLOD mesh generated");
				return false;
			}
			...
			bDirtyPackage |= (LODActor->GetStaticMeshComponent()->bRayTracingFarField != true);
			LODActor->GetStaticMeshComponent()->bRayTracingFarField = true;
