...
struct FNVErrorHandler : public nvtt::ErrorHandler
{
	...
	{
		UE_LOGF(LogTextureFormatDXT, Warning, "nvtt::compress() failed with error '%ls'", ANSI_TO_TCHAR(nvtt::errorString(e)));
		bSuccess = false;
	}
...
public:
		...
		: OutputHandler(OutBuffer, BufferSize)
	{
		...
		{
			UE_LOGF(LogTextureFormatDXT,Fatal,
				"Unsupported EPixelFormat for compression: %u",
				(uint32)PixelFormat
				);
		}
...
public:
	...
	virtual EPixelFormat GetEncodedPixelFormat(const FTextureBuildSettings& BuildSettings, bool bImageHasAlphaChannel) const override
	{
		...
		UE_LOGF(LogTextureFormatDXT, Fatal, "Unhandled texture format '%ls' given to FTextureFormatDXT::GetEncodedPixelFormat()", *BuildSettings.TextureFormatName.ToString());
		return PF_Unknown;
	}
