...
public:
	...
	virtual uint32 Run() override
	{
		while(!IsEngineExitRequested() && ServerSocket.IsValid())
		{
			...
			UE_LOGF( LogTurnkeyIO, Verbose, "Turnkey IO waiting for connection...");
			ClientSocket = TSharedPtr<FSocket>( ServerSocket->Accept(TEXT("TurnkeyIOClient")) );
			if (!IsClientConnected())
			{
				UE_LOGF( LogTurnkeyIO, Verbose, "Client not connected");
				continue;
			}
			UE_LOGF( LogTurnkeyIO, Verbose, "Client connected");
			...
			UE_LOGF( LogTurnkeyIO, Verbose, "Transaction complete. Disconnecting" );
			if (ClientSocket)
			{
				ClientSocket->Close();
				ClientSocket.Reset();
...
private:
	...
	void OnActionFinish( TSharedPtr<FJsonObject> Result )
	{
		...
		UE_LOGF( LogTurnkeyIO, Verbose, "Turnkey action completed. Sending reply.");
		SendMessage(Result);

		UE_LOGF( LogTurnkeyIO, Verbose, "Turnkey awaiting ack");
	...
	void HandleActionMessage( TSharedPtr<FJsonObject> Action )
	{
		...
		{
			UE_LOGF( LogTurnkeyIO, Error, "Invalid action message" );
			return;
		}
		...
		{
			UE_LOGF(LogTurnkeyIO, Error, "Unknown action type %ls", *Type );
			return;
		}

		// wait for the action to finish
		UE_LOGF( LogTurnkeyIO, Verbose, "Turnkey action in progress. Waiting for completion" );
	...
	{
		UE_LOGF(LogTurnkeyIO, Verbose, "Waiting for message from Turnkey client" );
		...
		do
		{
			...
			if (ClientSocket->Wait(ESocketWaitConditions::WaitForRead, FTimespan::FromMilliseconds(100)))
			{
				...
				else
				{
					...
					{
						UE_LOGF(LogTurnkeyIO, Error, "Client not responding");
		...
		{
			UE_LOGF(LogTurnkeyIO, Error, "Failed to parse message from Turnkey client %ls", *Reader->GetErrorMessage());
			return TSharedPtr<FJsonObject>();
		}
		return JsonObject;
	}
			...
			if (NewSocket.IsValid())
			{
				...
				if (NewSocket->Bind( *LocalHostAddr ))
				{
					if (NewSocket->Listen(1))
					{
						...
						UE_LOGF(LogTurnkeyIO, Verbose, "using port %d", Port );
					}
				}
			}
		}
