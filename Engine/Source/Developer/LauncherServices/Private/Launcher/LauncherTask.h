...
public:
	 ...
	 * @param InDesc - The description of the task.
	 */
	FLauncherTask(FString InName, FString InDesc)
		: Name{MoveTemp(InName)}
		, Desc{MoveTemp(InDesc)}
	{ }
	
	virtual ~FLauncherTask() override
	{
		if (Thread)
		{
...
public:
	...
	void Execute(const FLauncherTaskChainState* ChainState)
	{
		...
		Thread = TUniquePtr<FRunnableThread>{FRunnableThread::Create(this, *TaskName)};
	}
...
protected:
	 ...
	 */
	virtual bool PerformTask(const FLauncherTaskChainState& ChainState) = 0;
...
private:
	...
	const FLauncherTaskChainState* LocalChainState = nullptr;
