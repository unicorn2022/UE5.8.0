...
public:
	...
	UE_DEPRECATED(5.8, "Use OnPostProcessUATCommandLine")
	...
	virtual FOnLauncherProfilePostProcessUATCommandLine& OnPostProcessUATCommandLine( ) override
	{
		return PostProcessUATCommandLine;
	}

	virtual void RemoveDeviceGroup( const ILauncherDeviceGroupRef& DeviceGroup ) override;

	virtual void SaveDeviceGroups( bool bTrimUnused = false ) override;
...
private:
	...
	// Holds the config file name to save to
...
private:
	...
	// Holds a delegate to be involked to modify the command line for an extra UAT command before launching
	FOnLauncherProfilePostProcessUATCommandLine PostProcessUATCommandLine;
};
