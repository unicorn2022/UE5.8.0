 ...
 *
 * The first parameter is the profile that is being launched, the second parameter is the build target type (or 'unknown' for global arguments), and the third parameter is the command line to modify.
 */
DECLARE_MULTICAST_DELEGATE_ThreeParams(FOnLauncherProfilePostProcessCommandLine, const ILauncherProfileRef&, EBuildTargetType, FString& );
...
/**
 * Declares a delegate to be invoked to post-process a command line before launching a UAT command
 */
DECLARE_MULTICAST_DELEGATE_OneParam(FOnLauncherProfilePostProcessUATCommandLine, FUATCommandPostProcessParameters& );
...
public:
	/** Virtual destructor. */
	virtual ~ILauncherProfileWizard( ) = default;
};
...
public:
	...
	virtual void SaveDeviceGroups( bool bTrimUnused = false ) = 0;
...
public:
	 ...
	 */
	UE_DEPRECATED(5.8, "Please use LoadJSONProfile")
	...
	*/
	 ...
	 * @see LoadProfile
	 */
	UE_DEPRECATED(5.8, "Please use SaveJSONProfile")
...
public:
	...
	/**
	 * Returns a delegate that is invoked to post-process the command line before a UAT command is launched
	 * 
	 * @return The delegate.
	 */
	virtual FOnLauncherProfilePostProcessUATCommandLine& OnPostProcessUATCommandLine( ) = 0;

public:

	/** Virtual destructor. */
	virtual ~ILauncherProfileManager( ) = default;
};
