...
public:
	...
	virtual FText GetText() const override
	{
		...
		TArray<FText> WidgetsText;

		while(Stack.Num() > 0)
		{
			const TSharedRef<SWidget> Widget = Stack.Pop();
			FText WidgetText = FText::GetEmpty();

			if (Widget->GetType() == STextBlockType)
			{
				WidgetText = StaticCastSharedRef<STextBlock, SWidget>(Widget)->GetText();
			}
			else if (Widget->GetType() == SEditableTextType)
			{
				WidgetText = StaticCastSharedRef<SEditableText, SWidget>(Widget)->GetText();
			}
			else if (Widget->GetType() == SEditableTextBoxType)
			{
				WidgetText = StaticCastSharedRef<SEditableTextBox, SWidget>(Widget)->GetText();
			}
			else if (Widget->GetType() == SRichTextBlockType)
			{
				WidgetText = StaticCastSharedRef<SRichTextBlock, SWidget>(Widget)->GetText();
			}
			else if (Widget->GetType() == SMultiLineEditableTextType)
			{
				WidgetText = StaticCastSharedRef<SMultiLineEditableText, SWidget>(Widget)->GetText();
			}
			else if (Widget->GetType() == SMultiLineEditableTextBoxType)
			{
				WidgetText = StaticCastSharedRef<SMultiLineEditableTextBox, SWidget>(Widget)->GetText();
			}
			else
			{
				FChildren* ChildWidgets = Widget->GetChildren();
			...
			{
				WidgetsText.Add(WidgetText);
			}
		}

		return FText::Join(FText::FromString(LINE_TERMINATOR), WidgetsText);
	}
