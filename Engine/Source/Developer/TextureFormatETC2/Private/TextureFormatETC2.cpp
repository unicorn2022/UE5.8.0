	...
	TArray64<uint8>& OutCompressedData)
{
	...
	case PF_ETC2_RG11_EAC:
		...
		break;
	default:
		UE_LOGF(LogTextureFormatETC2, Fatal, "Unsupported EPixelFormat for compression: %u", (uint32)PixelFormat);
		return false;
	}
...
public:
	...
	virtual EPixelFormat GetEncodedPixelFormat(const FTextureBuildSettings& BuildSettings, bool bImageHasAlphaChannel) const override
	{
		...
		UE_LOGF(LogTextureFormatETC2, Fatal, "Unhandled texture format '%ls' given to FTextureFormatAndroid::GetEncodedPixelFormat()", *BuildSettings.TextureFormatName.ToString());
		return PF_Unknown;
	}
		...
		for (uint64 Slice = 0; Slice < NumSlices; Slice++)
		{
			for (uint64 BlockY = 0; BlockY < NumBlocksY; BlockY++)
			{
				for (uint64 BlockX = 0; BlockX < NumBlocksX; BlockX++)
				{
					...
					{
						UE_LOGF(LogTextureFormatETC2, Error, "Invalid block offset calculated during DecodeImage: %llu + %d, have %llu bytes available. Texture %.*ls", BlockOffset, BytesPerBlock, InEncodedData.GetSize(), InTextureName.Len(), InTextureName.GetData());
						UE_LOGF(LogTextureFormatETC2, Error, "....Slice %llu BlockX %llu BlockY %llu NumBlocksX %llu NumBlocksY %llu BytesPerSlice %llu", Slice, BlockX, BlockY, NumBlocksX, NumBlocksY, BytesPerSlice);
						return false;
					}
