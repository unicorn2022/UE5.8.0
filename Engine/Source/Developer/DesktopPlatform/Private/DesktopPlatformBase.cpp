...
FProcHandle FDesktopPlatformBase::InvokeUnrealBuildToolAsync(const FString& InCmdLineParams, FOutputDevice &Ar, void*& OutReadPipe, void*& OutWritePipe, bool bSkipBuildUBT)
{
	...
	if (!bSkipBuild)
	{
		...
		if (!bSuccessfullyBuiltUBTOnce)
		{
			...
			else
			{
				...
				UE_LOGF(LogDesktopPlatform, Warning, "Failed to compile UnrealBuildTool (project file is '%ls', exe path is '%ls')", *GetUnrealBuildToolProjectFileName(FPaths::RootDir()), *ExecutableFileName);
				return FProcHandle();
			}
		}
	}
	...
	if (!ProcHandle.IsValid())
	{
		UE_LOGF(LogDesktopPlatform, Warning, "Failed to launch UnrealBuildTool (exe path is '%ls')", *ExecutableFileName);
...
bool FDesktopPlatformBase::GetOidcTokenStatus(const FString& RootDir, const FString& ProjectFileName, const FString& ProviderIdentifier, FFeedbackContext* Warn, int& OutStatus)
{
	...
	if (!bRes)
	{
		UE_LOGF(LogDesktopPlatform, Warning, "Failed to run OidcToken to determine token status (project file is '%ls', exe path is '%ls')", *ProjectFileName, *GetOidcTokenExecutableFilename(RootDir));
		return false;
	}
...
bool FDesktopPlatformBase::GetOidcAccessTokenInternal(const FString& RootDir, const FString& BaseArguments, EDesktopLoginInteractionLevel InteractionLevel, FFeedbackContext* Warn, FString& OutToken, FDateTime& OutTokenExpiresAt, bool& bOutWasInteractiveLogin)
{
	...
	FString ResultFilePath = FPaths::CreateTempFilename(FPlatformProcess::UserTempDir(), TEXT("oidcToken.json"));
	#else
	...
	if (ExitCode == 10)
	{
		...
		if (!bRes)
		{
			...
			{
				UE_LOGF(LogDesktopPlatform, Warning, "Unable to allocate an access token. Unable to complete interactive login. Make sure you start the editor and login once or log in using UGS or using the UGS cli command 'login'. (Ran '%ls%ls')", *GetOidcTokenExecutableFilename(RootDir), *Arguments);
			}
			else
			{
				UE_LOGF(LogDesktopPlatform, Error, "Unable to allocate an access token. Interactive login failed, make sure you are assigned access and are able to login in the created browser window. (Ran '%ls%ls')", *GetOidcTokenExecutableFilename(RootDir), *Arguments);
			}
			return false;
		}
	}
	...
	if (!bRes)
	{
		UE_LOGF(LogDesktopPlatform, Warning, "Failed to run OidcToken ('%ls%ls'). ExitCode: %i", *GetOidcTokenExecutableFilename(RootDir), *Arguments, ExitCode);
		return false;
	}

	if (ExitCode == 11)
	{
		UE_LOGF(LogDesktopPlatform, Display, "Auth is disabled for connection");
	...
	UE_LOGF(LogDesktopPlatform, Warning, "Failed to run OidcToken ('%ls%ls'). No result file found at '%ls', closed with exit code: %d", *GetOidcTokenExecutableFilename(RootDir), *Arguments, *ResultFilePath, ExitCode);
...
bool FDesktopPlatformBase::GetOidcAccessTokenInteractive(const FString& RootDir, const FString& Arguments, EDesktopLoginInteractionLevel InteractionLevel, FFeedbackContext* Warn, int32& OutReturnCode)
{
	...
	while (!bIsFinished)
	{
		...
		if (DurationPhase == EWaitDurationPhase::Initial)
		{
			...
			UE_LOGF(LogDesktopPlatform, Display, "Waiting for OidcToken to finish login...");
...
FProcHandle FDesktopPlatformBase::InvokeOidcTokenToolAsync(const FString& InArguments, void*& OutReadPipe, void*& OutWritePipe)
{
	...
	UE_LOGF(LogDesktopPlatform, Display, "Launching OidcToken... [%ls %ls]", *ExecutableFileName, *CmdLineParams);
	...
	if (!ProcHandle.IsValid())
	{
		UE_LOGF(LogDesktopPlatform, Warning, "Failed to launch OidcToken (exe path is '%ls')", *ExecutableFileName);
	}
...
struct FTargetFileVisitor : IPlatformFile::FDirectoryStatVisitor
{
	...
	virtual bool Visit(const TCHAR* InFileNameOrDirectory, const FFileStatData& StatData) override
	{
		...
		if (FStringView WithoutExtension = UE::String::RemoveFromEnd(FileNameOrDirectory, TEXTVIEW(".Target.cs"), ESearchCase::IgnoreCase);
			WithoutExtension.Len() != FileNameOrDirectory.Len())
		{
			...
			{	
				UE_LOGF(LogDesktopPlatform, Log, "Found target file %ls newer than cache", InFileNameOrDirectory);
				bCacheInvalid = true;
			}
			// Any target file found on disk not in the TargetInfo.json is assumed to not be 
			// relevant for the current build since anything new on disk hasn't been built at all. 
			if (RemainingTargetNames.Remove(TargetName) != 1)
			{
				UE_LOGF(LogDesktopPlatform, Log, "Found target file %ls not present in cache. Ignoring.", InFileNameOrDirectory);
				return true;
			}
...
bool IsTargetInfoValid(const TArray<FTargetInfo>& Targets, TArray<FString>& DirectoryNames, const FDateTime& LastModifiedTime)
{
	...
	{
		// UE_LOGF(LogDesktopPlatform, Verbose, "Checking %ls for target files", *DirectoryNames[Idx]);
	...
	{
		UE_LOGF(LogDesktopPlatform, Log, "Failed to find target file for %ls, cache out of date.", *Target);
	}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE_STR("FDesktopPlatformBase::GetTargetsForProject");
	...
	Arguments += TEXT(" -DontIncludeProgramTargets");
	...
	if(!ReadTargetInfo(InfoFileName, NewTargets))
	{
		UE_LOGF(LogDesktopPlatform, Warning, "Unable to read target info for %ls", (ProjectFile.Len() == 0)? TEXT("engine") : *ProjectFile);
	}
...
bool FDesktopPlatformBase::EnumerateProjectsKnownByEngine(const FString &Identifier, bool bIncludeNativeProjects, TArray<FString> &OutProjectFileNames)
{
	...
	UE_LOGF(LogDesktopPlatform, Log, "Enumerating Projects From Engine Ver: %ls", *Identifier);

	UE_LOGF(LogDesktopPlatform, Log, "Looking for directories to scan from : %ls", *GameAgnosticConfigDir);
	...
	if (!FConfigCacheIni::LoadExternalIniFile(GameAgnosticConfig, TEXT("EditorSettings"), NULL, *GameAgnosticConfigDir, false))
	{
		...
		UE_LOGF(LogDesktopPlatform, Log, "%ls not found, looking for directories in %ls", *PreviousConfigDir , *GameAgnosticConfigDir);
	...
	{
		UE_LOGF(LogDesktopPlatform, Log, "Config not found");
	}

	if(Section != NULL)
	{
		UE_LOGF(LogDesktopPlatform, Log, "Searching for previously created project directories...");
		...
		for(int Idx = 0; Idx < AdditionalDirectories.Num(); Idx++)
		{
			...
			UE_LOGF(LogDesktopPlatform, Log, "Found directory: \"%ls\"", *AdditionalDirectories[Idx]);
		...
		for(int Idx = 0; Idx < RecentlyOpenedFiles.Num(); Idx++)
		{
			...
			UE_LOGF(LogDesktopPlatform, Log, "Found project \"%ls\" in recently opened files", *RecentlyOpenedFiles[Idx]);
	...
	for(int Idx = 0; Idx < SearchDirectories.Num(); Idx++)
	{
		...
		for(int32 FolderIdx = 0; FolderIdx < ProjectFolders.Num(); FolderIdx++)
		{
			...
			for(int32 FileIdx = 0; FileIdx < ProjectFiles.Num(); FileIdx++)
			{
				...
				UE_LOGF(LogDesktopPlatform, Log, "Found project \"%ls\" in previously created project directory", *ProjName);
	...
	UE_LOGF(LogDesktopPlatform, Log, "Searching for projects in .uprojectdirs");
	...
	if(bIncludeNativeProjects)
	{
		...
		for(int Idx = 0; Idx < NativeProjectPaths.Num(); Idx++)
		{
			...
			{
				UE_LOGF(LogDesktopPlatform, Log, "Found project \"%ls\" in .uprojectdirs", *NativeProjectPaths[Idx]);
...
bool FDesktopPlatformBase::BuildUnrealBuildTool(const FString& RootDir, FOutputDevice& Ar)
{
	...
	{
		FString ScriptPath = FPaths::ConvertRelativePathToFull(RootDir / TEXT("Engine/Build/BatchFiles/BuildUBT.bat"));
		CompilerExecutableFilename = TEXT("cmd.exe");
		CmdLineParams = FString::Printf(TEXT("/c \"%s\""), *ScriptPath);
	}
	else if constexpr (PLATFORM_MAC)
	{
		FString ScriptPath = FPaths::ConvertRelativePathToFull(RootDir / TEXT("Engine/Build/BatchFiles/BuildUBT.sh"));
		CompilerExecutableFilename = TEXT("/bin/sh");
		CmdLineParams = FString::Printf(TEXT("\"%s\""), *ScriptPath);
	}
	...
	return true;
#else // #if PLATFORM_WINDOWS || PLATFORM_MAC || PLATFORM_LINUX
	Ar.Log(TEXT("Unsupported platform, unable to build UnrealBuildTool."));
	return false;
#endif // #else // #if PLATFORM_WINDOWS || PLATFORM_MAC || PLATFORM_LINUX
}
