...
public:
	...
	virtual bool Cook(FName InFormat, const TArray<uint8>& InSrcBuffer, FSoundQualityInfo& InQualityInfo, TArray<uint8>& OutCompressedDataStore) const override
	{
		...
		{
			UE_LOGF(LogAudioFormatRad, Error, "Header/library mismatch with Rad Audio! Header: %u, Library: %u - verify sync is correct", RADA_BUILD_VERSION, RadAGetBuildVersion());
			return false;
		}
		#endif
		...
		if (RadCompressError)
		{
			UE_LOGF(LogAudioFormatRad, Warning, "Failed to encode RAD Audio: %s",  RadAErrorString(RadCompressError));
			if (RadCompressError == RADA_COMPRESS_ERROR_RATE)
			{
				UE_LOGF(LogAudioFormatRad, Warning, "Sample rate provided: %u - please reimport at a valid sample rate.", InQualityInfo.SampleRate);
			}
			else if (RadCompressError == RADA_COMPRESS_ERROR_CHANS)
			{
				UE_LOGF(LogAudioFormatRad, Warning, "Channels provided: %u, max channels allowed: %d", InQualityInfo.NumChannels, RADA_MAX_CHANS);
			}
			return false;
		}
	...
	virtual int32 GetMinimumSizeForInitialChunk(FName Format, const TArray<uint8>& SrcBuffer) const override
	{
		...
		if (FileHeader == 0)
		{
			UE_LOGF(LogAudioFormatRad, Error, "Invalid buffer passed to GetMinimumSizeForInitialChunk (size=%d)", SrcBuffer.Num());
			return 0;
		}
	...
	virtual bool SplitDataForStreaming(const TArray<uint8>& InSrcBuffer, TArray<TArray<uint8>>& OutBuffers, const int32 InMaxInitialChunkSize, const int32 InMaxChunkSize) const override
	{
		...
		if (FileHeader == 0)
		{
			UE_LOGF(LogAudioFormatRad, Error, "Buffer provided to SplitDataForStreaming is not a RADA file!");
			return false;
		}
		...
		if (RadAGetMemoryNeededToOpen(Source, SourceLen, &ContainerMemoryRequried) != 0)
		{
			...
			UE_LOGF(LogAudioFormatRad, Error, "Couldn't figure memory required to open Rada decoder - invalid file.");
			return false;
		}
		...
		{
			UE_LOGF(LogAudioFormatRad, Error, "Couldn't open Rada decoder - invalid file.");
			return false;
		}
		...
		for (;;)
		{
			...
			{
				UE_LOGF(LogAudioFormatRad, Error, "Couldn't parse rada block in file, offset %d", (uint32_t)(SourceEnd - Current));
				return false;
			}
	...
	virtual bool ExtractSeekTableForStreaming(TArray<uint8>& InOutBuffer, IAudioFormat::FSeekTable& OutSeekTable) const override
	{
		...
		{
			UE_LOGF(LogAudioFormatRad, Error, "Failed to decode seek table for streaming: result = %d", EnumToUnderlyingType(DecodeResult));
			return false;
		}
...
public:
	...
	virtual void StartupModule() override
	{
		...
		{
			UE_LOGF(LogAudioFormatRad, Error, "Header/library mismatch with Rad Audio! Header: %u, Library: %u - verify sync is correct", RADA_BUILD_VERSION, LibraryBuildVersion);
		}
		#endif
