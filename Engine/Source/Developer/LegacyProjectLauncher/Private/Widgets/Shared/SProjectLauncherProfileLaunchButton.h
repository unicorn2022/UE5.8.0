...
public:
	...
	virtual TSharedPtr<IToolTip> GetToolTip() override
	{
		...
		else if (!IsLegacyProfile())
		{
			ErrorToolTipWidget->SetContentWidget(
				SNew(STextBlock)
				.Text(LOCTEXT("ProjectLauncherNotLegacyProfile", "This profile can only be used with the new Project Launcher")));
...
private:
	...
	{
		ILauncherProfilePtr LaunchProfile = LaunchProfileAttr.Get();
		return LaunchProfile.IsValid() && LaunchProfile->SupportsLegacyProjectLauncher();
	};

	// Callback to see if the launch button should be enabled
	bool ButtonEnabled() const
	{
		return !HasError() && IsLegacyProfile();
	}
