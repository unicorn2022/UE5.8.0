...
public:
	...
	void Construct( const FArguments& InArgs)
	{
		...
		VertBox->AddSlot().AutoHeight()
			[
				MakeValidationMessage(TEXT("Icons.Error"), ELauncherProfileValidationErrors::AutomatedTestRequiredBuildCookRun)
			];

		static_assert(ELauncherProfileValidationErrors::Count == 31, "SProjectLauncherValidation::Construct() needs to be updated to account for modified enum values.");
