...
public:
		...
		, bEmissiveHDR(InMaterialBakingModule.bEmissiveHDR)
	{
		...
		UE_LOGF(LogMaterialBaking, Verbose, "Performing material baking for %d materials", MaterialSettings.Num());
		for (int32 i = 0; i < MaterialSettings.Num(); i++)
		{
			if (MaterialSettings[i]->Material && MeshSettings[i]->MeshDescription)
			{
				UE_LOGF(LogMaterialBaking, Verbose, "    [%5d] Material: %-50ls Vertices: %8d    Triangles: %8d", i, *MaterialSettings[i]->Material->GetName(), MeshSettings[i]->MeshDescription->Vertices().Num(), MeshSettings[i]->MeshDescription->Triangles().Num());
			}
		}
