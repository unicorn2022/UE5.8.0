...
private:
	virtual FCompressedBuffer RequestPayload(const FIoHash& Identifier) override
	{
		...
		if (Data == nullptr)
		{
			UE_LOGF(LogVirtualization, Error, "FWorkspaceDomainPayloadProvider was unable to find a payload with the identifier '%ls'", 
		...
		{
			UE_LOGF(LogVirtualization, Error, "FWorkspaceDomainPayloadProvider was unable to open the package '%ls' for reading", 
				*Data->PackageName);
		...
		{
			UE_LOGF(LogVirtualization, Error, "FWorkspaceDomainPayloadProvider failed to load the package trailer from the package '%ls'", 
				*Data->PackageName);
		...
		if (!Payload)
		{
			UE_LOGF(LogVirtualization, Error, "FWorkspaceDomainPayloadProvider was uanble to load the payload '%ls' from the package '%ls'",
				*LexToString(Identifier),
				*Data->PackageName);
		...
		{
			UE_LOGF(LogVirtualization, Error, "FWorkspaceDomainPayloadProvider loaded an incorrect payload from the package '%ls'. Expected '%ls' Loaded  '%ls'", 
				*Data->PackageName,
				*LexToString(Identifier),
...
void VirtualizePackages(TConstArrayView<FString> PackagePaths, EVirtualizationOptions Options, FVirtualizationResult& OutResultInfo)
{
	...
	ON_SCOPE_EXIT
	{
		...
		UE_LOGF(LogVirtualization, Display, "Virtualization process complete");
		UE_LOGF(LogVirtualization, Verbose, "Virtualization process took %.3f(s)", OutResultInfo.TimeTaken);
	};
	...
	UE_LOGF(LogVirtualization, Display, "Considering %d file(s) for virtualization", PackagePaths.Num());
	...
	for (const FString& AbsoluteFilePath : PackagePaths)
	{
		...
		if (bIsDuplicate)
		{
			UE_LOGF(LogVirtualization, Verbose, "Skipping duplicate package entry '%ls'", *AbsoluteFilePath);
	...
	UE_CLOGF(NumSkippedPackages > 0, LogVirtualization, Warning, "Discarded %d duplicate package paths", NumSkippedPackages);
	...
	{
		UE_LOGF(LogVirtualization, Display, "Pushing payload(s) to EStorageType::Cache storage...");

		if (!System.PushData(PayloadsToSubmit, EStorageType::Cache))
		{
			// Caching is not critical to the process so we only warn if it fails
			UE_LOGF(LogVirtualization, Warning, "Failed to push to EStorageType::Cache storage");
		}
	...
	{
		UE_LOGF(LogVirtualization, Display, "Pushing payload(s) to cached storage is disbled, skipping");
	}
	...
	// Push payloads to persistent storage
	{
		...
		UE_LOGF(LogVirtualization, Display, "Pushing payload(s) to EStorageType::Persistent storage...");
	...
	{
		UE_LOGF(LogVirtualization, Display, "No packages need to be updated on disk");
		return;
	}

	UE_LOGF(LogVirtualization, Display, "%d package(s) had their trailer container modified and need to be updated", PackagesToReplace.Num());

	{
		...
		UE_LOGF(LogVirtualization, Display, "Detaching loaded packages from disk...");
		...
		for (const FPackageReplacement& PackageInfo : PackagesToReplace)
		{
			...
			{
				UE_LOGF(LogVirtualization, Verbose, "Detaching '%ls'", *PackageInfo.Path.GetDebugName());
		...
		UE_LOGF(LogVirtualization, Display, "Reset the loaders of %d package(s)", NumPackagesReset);
	}
	...
	if (EnumHasAnyFlags(Options, EVirtualizationOptions::Checkout))
	{
		...
		UE_LOGF(LogVirtualization, Display, "Checking out packages from revision control...");
		...
		UE_LOGF(LogVirtualization, Display, "Checked out %d package(s)", OutResultInfo.CheckedOutPackages.Num());
	}

	{
		TRACE_CPUPROFILER_EVENT_SCOPE(UE::Virtualization::VirtualizePackages::EnsureWritePermissions);

		UE_CLOGF(!PackagesToReplace.IsEmpty(), LogVirtualization, Display, "Checking packages for write access permission...");
		...
		for (int32 Index = 0; Index < PackagesToReplace.Num(); ++Index)
		{
			...
			if (!CanWriteToFile(PackageInfo.Path.GetLocalFullPath()))
			{
				...
				UE_LOGF(LogVirtualization, Warning, "%ls", *Message.ToString());
		...
		UE_CLOGF(NumSkipped > 0, LogVirtualization, Warning, "Skipped %d package(s)", NumSkipped);
		UE_CLOGF(NumSkipped == 0, LogVirtualization, Display, "All packages have write permission");
	}
	
	{
		...
		UE_LOGF(LogVirtualization, Display, "Replacing old packages with the virtualized version...");
		...
		for (const FPackageReplacement& PackageInfo : PackagesToReplace)
		{
			...
			if (IFileManager::Get().Move(*OriginalPackagePath, *NewPackagePath))
			{
				...
				UE_LOGF(LogVirtualization, Verbose, "Reducing %ls: %ls -> %ls",
		...
		{
			UE_LOGF(LogVirtualization, Display, "Total Reduction %ls (%ls -> %ls)",
		...
		UE_LOGF(LogVirtualization, Display, "Replaced %d package(s)", OutResultInfo.VirtualizedPackages.Num());
	}
}
