...
bool FFileSystemBackend::Initialize(const FString& ConfigEntry)
{
	...
	if (FParse::Value(*ConfigEntry, TEXT("EnvPathOverride="), EnvOverrideName))
	{
		...
		{
			UE_LOGF(LogVirtualization, Log, "[%ls] Overriding path with envvar '%ls'", *GetDebugName(), *EnvOverrideName);
	...
	{
		UE_LOGF(LogVirtualization, Error, "[%ls] Config file entry 'Path=' was empty", *GetDebugName());
		return false;
	}
	...
	UE_LOGF(LogVirtualization, Log, "[%ls] Using path: '%ls'", *GetDebugName(), *FPaths::ConvertRelativePathToFull(RootDirectory));
	UE_LOGF(LogVirtualization, Log, "[%ls] Will retry failed read attempts %d times with a gap of %dms between them", *GetDebugName(), RetryCount, RetryWaitTimeMS);
...
bool FFileSystemBackend::PushData(TArrayView<FPushRequest> Requests, EPushFlags Flags)
{
	...
	for (FPushRequest& Request : Requests)
	{
		...
		{
			UE_LOGF(LogVirtualization, Verbose, "[%ls] Already has a copy of the payload '%ls'.", *GetDebugName(), *LexToString(PayloadId));
		...
		if (FileAr == nullptr)
		{
			...
			UE_LOGF(LogVirtualization, Error, "[%ls] Failed to write payload '%ls' to '%ls' due to system error: %ls",
				*GetDebugName(),
		...
		else
		{
			...
			{
				UE_LOGF(LogVirtualization, Error, "[%ls] Failed to move payload '%ls' to it's final location '%ls' due to system error: %ls",
					*GetDebugName(),
					*LexToString(PayloadId),
					*FilePath,
					SystemErrorMsg.ToString());
...
bool FFileSystemBackend::PullData(TArrayView<FPullRequest> Requests, EPullFlags Flags, FText& OutErrors)
{
	...
	for (FPullRequest& Request : Requests)
	{
		...
		{
			UE_LOGF(LogVirtualization, VeryVerbose, "[%ls] Does not contain the payload '%ls'", 
				*GetDebugName(), 
		...
		if (FileAr == nullptr)
		{
			...
			UE_LOGF(LogVirtualization, Error, "[%ls] Failed to load payload '%ls' from file '%ls' due to system error: %ls",
				*GetDebugName(),
				*LexToString(Request.GetIdentifier()),
				FilePath.ToString(),
				SystemErrorMsg.ToString());
		...
		if (FileAr->IsError())
		{
			UE_LOGF(LogVirtualization, Error, "[%ls] Failed to serialize payload '%ls' from file '%ls'",
				*GetDebugName(),
				*LexToString(Request.GetIdentifier()),
				FilePath.ToString());
...
TUniquePtr<FArchive> FFileSystemBackend::OpenFileForReading(const TCHAR* FilePath)
{
	...
	while (Retries < RetryCount)
	{
		...
		{
			UE_LOGF(LogVirtualization, Warning, "[%ls] Failed to open '%ls' for reading attempt retrying (%d/%d) in %dms...", *GetDebugName(), FilePath, Retries, RetryCount, RetryWaitTimeMS);
