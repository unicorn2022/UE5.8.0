...
bool FDDCBackend::Initialize(const FString& ConfigEntry)
{
	...
	UE_LOGF(LogVirtualization, Log, "[%ls] Bucket set to '%ls", *GetDebugName(), *BucketName);
	UE_LOGF(LogVirtualization, Log, "[%ls] Use of local storage set to '%ls", *GetDebugName(), bAllowLocal ? TEXT("true") : TEXT("false"));
	UE_LOGF(LogVirtualization, Log, "[%ls] Use of remote storage set to '%ls", *GetDebugName(), bAllowRemote ? TEXT("true") : TEXT("false"));

	if (!bAllowLocal && !bAllowRemote)
	{
		UE_LOGF(LogVirtualization, Error, "[%ls] LocalStorage and RemoteStorage cannot both be disabled", *GetDebugName());
		return false;
	}
