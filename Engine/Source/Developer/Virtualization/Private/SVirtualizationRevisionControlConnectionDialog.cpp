...
SRevisionControlConnectionDialog::FResult SRevisionControlConnectionDialog::RunDialog(FStringView RevisionControlName, FStringView ConfigSectionName,  FStringView CurrentPort, FStringView CurrentUsername, const FText& ErrorMessage)
{
	...
	{
		UE_LOGF(LogVirtualization, Warning, "Skipping attempt to show SRevisionControlConnectionDialog as the application is unattended");
		return FResult();
	}

	if (!IsInGameThread())
	{
		UE_LOGF(LogVirtualization, Warning, "Attempting to show SRevisionControlConnectionDialog on a worker thread!");
		return FResult();
	}

	if (!FSlateApplication::IsInitialized() || FSlateApplication::Get().GetRenderer() == nullptr)
	{
		UE_LOGF(LogVirtualization, Warning, "Attempting to show SRevisionControlConnectionDialog before slate is initialized");
		return FResult();
	}

	UE_LOGF(LogVirtualization, Display, "Creating dialog");
	...
	UE_LOGF(LogVirtualization, Display, "Connection to revision control for virtualized assets failed. Offering the user the choice to retry or continue anyway");
...
{
	UE_LOGF(LogVirtualization, Display, "User opted to clear the ini file settings and connect using the revision control defaults");
...
{
	UE_LOGF(LogVirtualization, Display, "User opted to retry connecting to revision control");
...
{
	UE_LOGF(LogVirtualization, Warning, "User opted not to connect to revision control. Virtualized data may not be accessible!");
