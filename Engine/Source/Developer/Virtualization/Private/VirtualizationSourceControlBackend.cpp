...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FSourceControlBackend::OnConnect);
	...
	if (!P4Plugin)
	{
		UE_LOGF(LogVirtualization, Error, "[%ls] Failed to find the 'PerforceSourceControl' plugin", *GetDebugName());
		return IVirtualizationBackend::EConnectionStatus::Error;
	}

	if (!P4Plugin->IsEnabled())
	{
		UE_LOGF(LogVirtualization, Error, "[%ls] The 'PerforceSourceControl' plugin is disabled for this target", *GetDebugName());
		return IVirtualizationBackend::EConnectionStatus::Error;
	}
	...
	while (true)
	{
		...
		// Record each attempt and keep track of the total time we've spent connecting across all attempts.
		Attempts++;
		...
		if (ConnectionResult == IVirtualizationBackend::EConnectionStatus::Connected)
		{
...

			{
				...
				Attributes.Add({ TEXT("Connected"), true }); // Hard coded for now, but this will make more sense if we combine this event with the failures below
...
IVirtualizationBackend::EConnectionStatus FSourceControlBackend::OnConnectInternal(FString& InOutPort, FString& InOutUsername, bool bSaveConnectionSettings, FText& OutErrorMessage)
{
	...
	{
		UE_LOGF(LogVirtualization, Error, "[%ls] Failed to create a perforce revision control provider", *GetDebugName());
		return IVirtualizationBackend::EConnectionStatus::Error;
	}

	const ISourceControlProvider::EInitFlags InitOptions =	ISourceControlProvider::EInitFlags::AttemptConnection |
															ISourceControlProvider::EInitFlags::SuppressErrorLogging;
...
bool FSourceControlBackend::PullData(TArrayView<FPullRequest> Requests, EPullFlags Flags, FText& OutErrors)
{
	...
	if (RetryCount <= 0)
	{
		UE_LOGF(LogVirtualization, Warning, "[%ls] Retry count is set to 0, no attempts can be made!", *GetDebugName());
		return false;
	}
	...
	while (true)
	{
		...
		if (Retries != 0)
		{
			UE_LOGF(LogVirtualization, Warning, "[%ls] Failed to pull payloads! Attempt (%d/%d) retrying in %d(ms)...", *GetDebugName(), Retries, RetryCount, RetryWaitTimeMS);
		...
		{
			UE_LOGF(LogVirtualization, Warning, "[%ls] Failed to pull payloads! Attempt (%d/%d)", *GetDebugName(), Retries, RetryCount);
...
bool FSourceControlBackend::PushData(TArrayView<FPushRequest> Requests, EPushFlags Flags)
{
...

	{
		...
		UE_LOGF(LogVirtualization, Log, "[%ls] Found %d unique payload(s)", *GetDebugName(), UniqueRequests.Num());
	}
...

	{
		...
		for (int32 BatchIndex = 0; BatchIndex < NumBatches; ++BatchIndex)
		{
			...
			{
				UE_LOGF(LogVirtualization, Error, "[%ls] Failed to find the current file state for payloads", *GetDebugName());
				return false;
			}
	...
	UE_LOGF(LogVirtualization, Log, "[%ls] Determined that %d payload(s) require submission to revision control", *GetDebugName(), RequestsToPush.Num());
	...
	UE_LOGF(LogVirtualization, Log, "[%ls] Started payload submission session '%ls' for '%d' payload(s)", *GetDebugName(), *LexToString(SessionGuid), RequestsToPush.Num());
	...
	{
		UE_LOGF(LogVirtualization, Error, "[%ls] Failed to setup the directory to submit payloads from", *GetDebugName());
		return false;
	}

	UE_LOGF(LogVirtualization, Log, "[%ls] Created directory '%ls' to submit payloads from", *GetDebugName(), SessionDirectory.ToString());
	...
	// Create a temp workspace so that we can submit the payload from
	{
		...
		CreateWorkspaceCommand->SetDescription(TEXT("This workspace was auto generated when submitting virtualized payloads to revision control"));

		if (SCCProvider->Execute(CreateWorkspaceCommand) != ECommandResult::Succeeded)
		{
			UE_LOGF(LogVirtualization, Error, "[%ls] Failed to create temp workspace '%ls' to use for payload submission",
				*GetDebugName(),
	...
	ON_SCOPE_EXIT
	{
		...
		{
			UE_LOGF(LogVirtualization, Warning, "[%ls] Failed to remove temp workspace '%ls' please delete manually", *GetDebugName(), WorkspaceName.ToString());
		}
	};
	...
	FString OriginalWorkspace;
	{
		...
		{
			UE_LOGF(LogVirtualization, Error, "[%ls] Failed to switch to temp workspace '%ls' when trying to submit payloads",
				*GetDebugName(),
				WorkspaceName.ToString());
	...
	ON_SCOPE_EXIT
	{
		...
		{
			UE_LOGF(LogVirtualization, Error, "[%ls] Failed to restore the original workspace '%ls'", *GetDebugName(), *OriginalWorkspace);
		}
	};

	const int32 NumBatches = FMath::DivideAndRoundUp<int32>(RequestsToPush.Num(), MaxBatchCount);

	UE_LOGF(LogVirtualization, Log, "[%ls] Splitting the submission into '%d' batches", *GetDebugName(), NumBatches);

	for (int32 BatchIndex = 0; BatchIndex < NumBatches; ++BatchIndex)
	{
		UE_LOGF(LogVirtualization, Log, "[%ls] Processing batch %d/%d...", *GetDebugName(), BatchIndex + 1, NumBatches);
		...
		// be on disk)
		{
			...
			for (const FPushRequest* Request : RequestBatch)
			{
				...
				UE_LOGF(LogVirtualization, Verbose, "[%ls] Writing payload to '%ls' for submission", *GetDebugName(), PayloadFilePath.ToString());

				FCompressedBuffer Payload = Request->GetPayload();
				if (Payload.IsNull())
				{
					UE_LOGF(LogVirtualization, Error, "[%ls] Failed to acquire payload '%ls' contents to '%ls' for writing",
						*GetDebugName(),
				...
				if (!FileAr)
				{
					...
					UE_LOGF(LogVirtualization, Error, "[%ls] Failed to write payload '%ls' contents to '%ls' due to system error: %ls",
						*GetDebugName(),
						*LexToString(Request->GetIdentifier()),
						PayloadFilePath.ToString(),
...

		{
			...
			{
				UE_LOGF(LogVirtualization, Error, "[%ls] Failed to mark the payload file for Add in revision control", *GetDebugName());
				return false;
			}
		}
		...
		// Now submit the payload
		{
			...
			{
				UE_LOGF(LogVirtualization, Error, "[%ls] Failed to submit the payload file(s) to revision control", *GetDebugName());
				return false;
			}
		}
...
bool FSourceControlBackend::DoPayloadsExist(TArrayView<const FIoHash> PayloadIds, TArray<bool>& OutResults)
{
	...
	{
		UE_LOGF(LogVirtualization, Error, "[%ls] Failed to query the state of files in the revision control depot", *GetDebugName());
		return false;
	}
...
bool FSourceControlBackend::TryApplySettingsFromConfigFiles(const FString& ConfigEntry)
{
	// If the depot root is within a perforce stream then we must specify which stream. This may be a virtual stream with a custom view.
	{
		...
		if (!ClientStream.IsEmpty())
		{
			UE_LOGF(LogVirtualization, Log, "[%ls] Using client stream: '%ls'", *GetDebugName(), *ClientStream);
		}
		else
		{
			UE_LOGF(LogVirtualization, Log, "[%ls] Not using client stream", *GetDebugName());
		}
	}
	...
	// We require that a valid depot root has been provided
	{
		...
		{
			UE_LOGF(LogVirtualization, Warning, "[%ls] Entry 'DepotRoot' is deprecated, replace with 'DepotPath'", *GetDebugName());
		}
		else
		{
		...
		if (DepotPathRoot.IsEmpty())
		{
			UE_LOGF(LogVirtualization, Error, "[%ls] 'DepotPath' was not found in the config file!", *GetDebugName());
			return false;
		}
...

	{
		...
		{
			UE_LOGF(LogVirtualization, Log, "[%ls] Using server address: '%ls'", *GetDebugName(), *ServerAddress);
		}
		else
		{
			UE_LOGF(LogVirtualization, Log, "[%ls] Will connect to the default server address", *GetDebugName());
		}
	}
	...
	// Check to see if we should use partitioned clients or not. This is a perforce specific optimization to make the workspace churn cheaper on the server
	{
		...
		UE_LOGF(LogVirtualization, Log, "[%ls] Using partitioned clients: '%ls'", *GetDebugName(), bUsePartitionedClient ? TEXT("true") : TEXT("false"));
	}
	...
	// Allow the source control backend to retry failed pulls
	{
		...
		UE_LOGF(LogVirtualization, Log, "[%ls] Will retry failed downloads attempts %d time(s) with a gap of %dms between them", *GetDebugName(), RetryCount, RetryWaitTimeMS);
	}
	...
	// Allow the number of concurrent connections to be limited
	{
		...
		if (MaxLimit != INDEX_NONE)
		{
			...
			UE_LOGF(LogVirtualization, Log, "[%ls] Limited to %d concurrent revision control connections", *GetDebugName(), MaxLimit);
		}
		else
		{
			UE_LOGF(LogVirtualization, Log, "[%ls] Has no limit to it's concurrent revision control connections", *GetDebugName());
		}
	}
	...
	// Check for the optional BatchCount parameter
	{
		...
		UE_LOGF(LogVirtualization, Log, "[%ls] Will push payloads in batches of up to %d payloads(s) at a time", *GetDebugName(), MaxBatchCount);
	}
	...
	// Check to see if connection error notification pop ups should be shown or not
	{
		...
		{
			UE_LOGF(LogVirtualization, Log, "[%ls] Connection pop up warnings are suppressed", *GetDebugName());
		}
		else
		{
			UE_LOGF(LogVirtualization, Log, "[%ls] Connection pop up warnings will be shown", *GetDebugName());
		}
	}

	{
		FParse::Bool(*ConfigEntry, TEXT("UseLocalIniFileSettings="), bUseLocalIniFileSettings);
		UE_LOGF(LogVirtualization, Log, "[%ls] Reading settings from local SourceControlSettings.ini is %ls", *GetDebugName(), bUseLocalIniFileSettings ? TEXT("enabled") : TEXT("disabled"));	
	}

	{
		...
		UE_LOGF(LogVirtualization, Log, "[%ls] Showing a reconnect dialog on initial failure %ls", *GetDebugName(), bUseRetryConnectionDialog ? TEXT("enabled") : TEXT("disabled"));
	}

	{
		...
		if (FParse::Value(*ConfigEntry, TEXT("IgnoreFile="), IgnoreFileName))
		{
			...
			{
				UE_LOGF(LogVirtualization, Error, "[%ls] Invalid IgnoreFile entry! This value should just be the filename and not contain any path info", *GetDebugName());
				return false;
			}
#endif
		}

		UE_LOGF(LogVirtualization, Log, "[%ls] Using '%ls' as the p4 ignore file name", *GetDebugName(), *IgnoreFileName);
	}
...
bool FSourceControlBackend::FindSubmissionWorkingDir(const FString& ConfigEntry)
{
	...
	if (FParse::Value(*ConfigEntry, TEXT("WorkingDir="), WorkingDirFromIniFile) && !WorkingDirFromIniFile.IsEmpty())
	{
		...
		{
			UE_LOGF(LogVirtualization, Log, "[%ls] Failed to correctly expand 'WorkingDir=%ls'", *GetDebugName(), *WorkingDirFromIniFile);
		}
	}
	...
	if (SubmissionRootDir.IsEmpty())
	{
		...
		if (!SubmissionRootDir.IsEmpty())
		{
			UE_LOGF(LogVirtualization, Log, "[%ls] Found Environment Variable: UE-VirtualizationWorkingDir", *GetDebugName());
		}
	}
	...
	if (SubmissionRootDir.IsEmpty())
	{
		...
		{
			UE_LOGF(LogVirtualization, Warning, "[%ls] Found legacy ini file setting 'SubmitFromTempDir' use '-WorkingDir=$(Temp)/UnrealEngine/VASubmission' instead!", *GetDebugName());
		}
	...
	{
		UE_LOGF(LogVirtualization, Log, "[%ls] Setting '%ls' as the working directory", *GetDebugName(), *SubmissionRootDir);
		return true;
	}
	else
	{
		...
		UE_LOGF(LogVirtualization, Error, "[%ls] Failed to set the  working directory to '%ls' due to %ls", *GetDebugName(), *SubmissionRootDir, SystemErrorMsg.ToString());
...
void FSourceControlBackend::OnConnectionError(FText ErrorMessage)
{
	...
	{
		UE_LOGF(LogVirtualization, Warning, "%ls", *ErrorMessage.ToString());
		UE_LOGF(LogVirtualization, Warning, "%ls", *UserMessage.ToString());
	...
	else
	{
		...
		UE_LOGF(LogVirtualization, Error, "%ls", *ErrorMessage.ToString());
		UE_LOGF(LogVirtualization, Error, "%ls", *UserMessage.ToString());
