...
void FUncontrolledChangelistsDiscoverAssetsTask::DoWork()
{
	...
	UE_LOGF(LogSourceControl, Log, "Uncontrolled asset discovery started...");
	...
	UE_LOGF(LogSourceControl, Log, "Uncontrolled asset discovery finished in %ls seconds (Found %d uncontrolled assets)", *FString::SanitizeFloat(FPlatformTime::Seconds() - StartTime), AddedAssetsCache.Num());;
}
...
bool FUncontrolledChangelistsModule::OnReconcileAssets()
{
	...
	{
		while (!DiscoverAssetsTask->WaitCompletionWithTimeout(0.016f))
		{
...
static bool ExecuteRevertOperation(const TArray<FString>& InFilenames)
{
	...
	{
		UE_LOGF(LogSourceControl, Error, "Failed to update the revision control files states for %ls.", *BuildFileString(InFilenames));
		return false;
	}
	...
	if (!FilesToRevert.IsEmpty())
	{
		...
		{
			UE_LOGF(LogSourceControl, Error, "Failed to sync the following files to a previous version: %ls.", *BuildFileString(FilesToRevert));
			return false;
		}
	}
	...
	for (const FString& FileToDelete : FilesToDelete)
	{
		...
		{
			UE_LOGF(LogSourceControl, Error, "Failed to delete %ls.", *FileToDelete);
			bSuccess = false;
		}
	}
...
TOptional<FUncontrolledChangelist> FUncontrolledChangelistsModule::CreateUncontrolledChangelist(const FText& InDescription, const TOptional<FUncontrolledChangelist>& InUncontrolledChangelist)
{
	...
	if (InUncontrolledChangelist)
	{
		...
		{
			UE_LOGF(LogSourceControl, Error, "Cannot use CreateUncontrolledChangelist with the Default Uncontrolled Changelist.");
			return TOptional<FUncontrolledChangelist>();
		}
...
void FUncontrolledChangelistsModule::EditUncontrolledChangelist(const FUncontrolledChangelist& InUncontrolledChangelist, const FText& InNewDescription)
{
	...
	if (InUncontrolledChangelist.IsDefault())
	{
		UE_LOGF(LogSourceControl, Error, "Cannot edit Default Uncontrolled Changelist.");
		return;
	}
	...
	{
		UE_LOGF(LogSourceControl, Error, "Cannot find Uncontrolled Changelist %ls in cache.", *InUncontrolledChangelist.ToString());
		return;
	}
...
void FUncontrolledChangelistsModule::DeleteUncontrolledChangelist(const FUncontrolledChangelist& InUncontrolledChangelist)
{
	...
	if (InUncontrolledChangelist.IsDefault())
	{
		UE_LOGF(LogSourceControl, Error, "Cannot delete Default Uncontrolled Changelist.");
		return;
	}
	...
	{
		UE_LOGF(LogSourceControl, Error, "Cannot delete Uncontrolled Changelist %ls while it contains files.", *InUncontrolledChangelist.ToString());
		return;
	}
...
void FUncontrolledChangelistsModule::StartAssetDiscovery()
{
	...
	if (HasCustomProjectFilter())
	{
		...
		{
			UE_LOGF(LogSourceControl, Log, "Uncontrolled asset discovery was skipped due to having no custom project content paths");
			return;
		}
	}
...
void FUncontrolledChangelistsModule::StopAssetDiscovery()
{
	if (DiscoverAssetsTask.IsValid())
	{
		...
		{
			UE_LOGF(LogSourceControl, Log, "Uncontrolled asset discovery was cancelled by request");
		}
		else if (!DiscoverAssetsTask->IsDone())
		{
			...
			UE_LOGF(LogSourceControl, Log, "Waiting on uncontrolled asset discovery to stop...");
			DiscoverAssetsTask->EnsureCompletion();
			UE_LOGF(LogSourceControl, Log, "Uncontrolled asset discovery stopped after stalling for %.1f(s)", FPlatformTime::Seconds() - StartTime);
...
void FUncontrolledChangelistsModule::SaveState()
{
	...
	auto SaveStateImpl = [this](const FString& PersistentFilePath, const TFunction<bool(const FString&)>& FilenameFilter)
	{
		...
		{
			UE_LOGF(LogSourceControl, Display, "Uncontrolled Changelist persistency file saved %ls", *PersistentFilePath);
		}
	};
...
void FUncontrolledChangelistsModule::LoadState()
{
	auto LoadStateImpl = [this](const FString& PersistentFilePath)
	{
		...
		{
			UE_LOGF(LogSourceControl, Error, "Cannot deserialize RootObject.");
			return;
		}
		...
		{
			UE_LOGF(LogSourceControl, Error, "Cannot get field %ls.", VERSION_NAME);
			return;
		}

		if (VersionNumber > VERSION_NUMBER)
		{
			UE_LOGF(LogSourceControl, Error, "Version number is invalid (file: %u, current: %u).", VersionNumber, VERSION_NUMBER);
			return;
		}
		...
		{
			UE_LOGF(LogSourceControl, Error, "Cannot get field %ls.", CHANGELISTS_NAME);
			return;
		}
		...
		for (const TSharedPtr<FJsonValue>& JsonValue : *UncontrolledChangelistsArray)
		{
			...
			{
				UE_LOGF(LogSourceControl, Error, "Cannot deserialize FUncontrolledChangelist.");
				continue;
			}
		...
		UE_LOGF(LogSourceControl, Display, "Uncontrolled Changelist persistency file loaded %ls", *PersistentFilePath);
	};
