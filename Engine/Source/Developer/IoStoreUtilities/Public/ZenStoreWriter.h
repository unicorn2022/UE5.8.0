...
public:
	IOSTOREUTILITIES_API FZenStoreWriter(	const FString& OutputPath, 
											...
											TSharedRef<UE::Cook::ICookArtifactReader> CookArtifactReader);

	IOSTOREUTILITIES_API ~FZenStoreWriter();

	static IOSTOREUTILITIES_API bool CanUse();
	...
	IOSTOREUTILITIES_API void InitializeConnection();
	...
	IOSTOREUTILITIES_API virtual void PopulateOplog(const FAssetRegistryState& PreviousState,
		int32& OutNumPackagesInOplog) override;
	IOSTOREUTILITIES_API virtual void UpdateLastReferenceDateAndPruneStaleOps(
		UE::Cook::Artifact::FUpdateOplogPackagesContext& OplogContext) override;
	IOSTOREUTILITIES_API virtual TArray<FName> GetOplogPackageNames() override;
	...
	IOSTOREUTILITIES_API virtual void AppendOp(FName KeyName, FCbObject Payload) override;
...
private:
	...
	{
		UE::Tasks::TTask<FCompressedBuffer> CompressedPayload;
		FBulkDataInfo Info;
		FCbObjectId ChunkId;
	...
	FString GetProjectStorePath() const;
	void WriteProjectStoreFile();
	...
	UE::FMutex				ZenFileSystemManifestMutex;
	...
	std::atomic<int32>					NumCommitPending;
	UE::FEventCount						CommitPendingEvent;
