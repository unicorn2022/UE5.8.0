...
public:
	...
	virtual void SetReferenceChunkDatabase(TSharedPtr<IIoStoreWriterReferenceChunkDatabase> ReferenceChunkDatabase, TSharedPtr<IIoStoreWriterReferenceChunkDatabase> PatchInPlaceReferenceChunkDatabase) override
	{ }
						...
						if (const FIoBuffer* SourceBuffer = PendingWrite->WriteRequest->GetSourceBuffer())
						{
							...
							if (IFileManager::Get().MakeDirectory(*FileDirectory, true))
							{
								if (TUniquePtr<FArchive> Ar(IFileManager::Get().CreateFileWriter(*FilePath)); Ar.IsValid())
								{
									...
									UE_CLOGF((CurrentCount % 128 == 0), LogIoStore, Display,
										"Writing loose file chunk #%u '%ls' -> '%ls' (%llu Bytes)",
	...
	virtual TIoStatusOr<FIoStoreWriterResult> GetResult() override
	{
		...
		for (TUniquePtr<FPendingWrite>& PendingWrite : PendingWrites)
		{
			...
			{
				UE_LOGF(LogIoStore, Display, "Failed to write on demand chunk '%ls', reason '%ls'", *LexToString(PendingWrite->ChunkId), *PendingWrite->ErrorText);
		...
		UE_LOGF(LogIoStore, Display, "Saving loose file JSON manifest '%ls'", *Settings.TocFilePath);
