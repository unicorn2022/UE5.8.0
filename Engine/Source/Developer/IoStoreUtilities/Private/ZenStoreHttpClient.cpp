	...
	FStringView ProjectFilePath)
{
	...
	UE_LOGF(LogZenStore, Display, "Establishing oplog '%ls/%ls'", *FString(InProjectId), *FString(InOplogId));
...

	{
		...
		{
			UE_LOGF(LogZenStore, Display, "Zen project '%ls' already exists", *FString(InProjectId));
		...
		else
		{
			...
			{
				UE_LOGF(LogZenStore, Warning, "Zen project '%ls' creation FAILED: %ls", *FString(InProjectId), *Request->GetDetailedResultString(ProjectUri));
			...
			{
				const int ResponseCode = Request->GetResponseCode();
				if (ResponseCode == 201)
				{
					UE_LOGF(LogZenStore, Display, "Zen project '%ls' created", *FString(InProjectId));
					bConnectionSucceeded = true;
				}
				else if (200 <= ResponseCode && ResponseCode < 300)
				{
					UE_LOGF(LogZenStore, Warning, "Zen project '%ls' creation returned success but unexpected result: %ls", *FString(InProjectId), *Request->GetDetailedResultString(ProjectUri));
					bConnectionSucceeded = true;
				}
				else
				{
...
bool FZenStoreHttpClient::TryDeleteOplog(FStringView InProjectId, FStringView InOplogId)
{
	...
	UE_LOGF(LogZenStore, Display, "Deleting oplog '%ls/%ls' if it exists", *FString(InProjectId), *FString(InOplogId));
	bool Success = Delete(*Request, OplogPath);
	if (!Success && Request->GetResponseCode() != 404)
	{
		UE_LOGF(LogZenStore, Warning, "Deleting oplog FAILED: %ls", *Request->GetDetailedResultString(OplogPath));
	}
	return Success;
}
...
bool FZenStoreHttpClient::TryCreateOplog(FStringView InProjectId, FStringView InOplogId, FStringView InOplogLifetimeMarkerPath)
{
	...
	{
		UE_LOGF(LogZenStore, Display, "Zen oplog '%ls/%ls' already exists", *FString(InProjectId), *FString(InOplogId));
	...
	else
	{
		...
		{
			UE_LOGF(LogZenStore, Warning, "Zen oplog '%ls/%ls' creation FAILED: %ls", *FString(InProjectId), *FString(InOplogId), *Request->GetDetailedResultString(OplogPath));
		...
		else
		{
			...
			if (ResponseCode == 201)
			{
				UE_LOGF(LogZenStore, Display, "Zen oplog '%ls/%ls' created", *FString(InProjectId), *FString(InOplogId));
			}
			else if (200 <= ResponseCode && ResponseCode < 300)
			{
				UE_LOGF(LogZenStore, Warning, "Zen oplog '%ls/%ls' creation returned success but unexpected result: %ls", *FString(InProjectId), *FString(InOplogId), *Request->GetDetailedResultString(OplogPath));
			}
			else
			{
				UE_LOGF(LogZenStore, Warning, "Zen oplog '%ls/%ls' creation FAILED: %ls", *FString(InProjectId), *FString(InOplogId), *Request->GetDetailedResultString(OplogPath));
		...
		{
			UE_LOGF(LogZenStore, Warning, "Zen oplog '%ls/%ls' get FAILED: %ls", *FString(InProjectId), *FString(InOplogId), *Request->GetDetailedResultString(OplogPath));
			bConnectionSucceeded = false;
			return false;
		}
	}
...
void FZenStoreHttpClient::InitializeReadOnly(FStringView InProjectId, FStringView InOplogId)
{
	...
	UE_LOGF(LogZenStore, Display, "Opening oplog %ls / %ls", *FString(InProjectId), *FString(InOplogId));
	...
	ProjectPath = WriteToString<128>("/prj/", InProjectId);
	{
		...
		{
			UE_LOGF(LogZenStore, Fatal, "Zen project '%ls' get FAILED: %ls", *FString(InProjectId), *Request->GetDetailedResultString(ProjectPath));
			bConnectionSucceeded = false;
		}
		else
		{
...

	{
		...
		if (!Download(*Request, OplogPath, &GetBuffer, Zen::EContentType::CbObject) || Request->GetResponseCode() != 200)
		{
			UE_LOGF(LogZenStore, Fatal, "Zen oplog '%ls/%ls' get FAILED: %ls", *FString(InProjectId), *FString(InOplogId), *Request->GetDetailedResultString(OplogPath));
			bConnectionSucceeded = false;
		}
	}
...
void FZenStoreHttpClient::Rpc(FAnsiStringView Uri, FCbObject RpcRequestObject, FCbPackage& OutResponse)
{
	...
	do
	{
		...
		bRetry = Receiver.ShouldRetry(ZenService.GetInstance(), *HttpResponse, /*UseSoftRecovery*/ false) && (++Attempt < MaxAttempts);
	}
	while (bRetry);
}
...
bool FZenStoreHttpClient::ShouldRetry(Zen::FZenHttpRequest& Request)
{
	const int ConnectError = 7;
	const int SSLConnectError = 35;
	const int TimeoutError = 28;
	const int SendError = 55;
	const int ReceiveError = 56;

	if ((Request.GetErrorCode() == ConnectError) ||
		(Request.GetErrorCode() == SSLConnectError) ||
		(Request.GetErrorCode() == TimeoutError) ||
		(Request.GetErrorCode() == SendError) ||
		(Request.GetErrorCode() == ReceiveError))
	{
		return TryRecover();
	}
	else if (Request.GetResponseCode() == 404 && Request.GetResponseAsString().Contains("No suitable route found"))
	{
...
bool FZenStoreHttpClient::TryRecover()
{
	Zen::FZenServiceInstance& ZenInstance = ZenService.GetInstance();
	if (ZenInstance.IsServiceRunningLocally())
	{
		return ZenInstance.TryRecovery(Zen::FZenServiceInstance::ERecoveryMode::Hard);
	}
	return true;
}
...
TIoStatusOr<uint64> FZenStoreHttpClient::AppendOp(FCbPackage OpEntry)
{
...

	{
		...
		{
			TSet<FIoHash> AttachmentHashes;
			for (const FCbAttachment& Attachment : Attachments)
			{
				AttachmentHashes.Add(Attachment.GetHash());
			}
			TStringBuilder<256> ErrorBuffer;
			OpEntry.GetObject().IterateAttachments([&AttachmentHashes, &ErrorBuffer](FCbFieldView Field)
				{
					const FIoHash Hash = Field.AsAttachment();
					if (!AttachmentHashes.Contains(Hash))
					{
						if (ErrorBuffer.Len() > 0)
						{
							ErrorBuffer << ", ";
						}
						ErrorBuffer << LexToString(Hash);
					}
				});

			if (ErrorBuffer.Len() > 0)
			{
				return TIoStatusOr<uint64>((FIoStatus)(FIoStatusBuilder(EIoErrorCode::Unknown) << TEXT("Oplog body references attachments that are not included in the package: ") << ErrorBuffer.ToString()));
			}
		}
		...
		ON_SCOPE_EXIT
		{
			...
			{
				(void)PlatformFile.DeleteFile(*TempFile);
			}
		};
		...
		else
		{
			...
			if (!Attachments.IsEmpty())
			{
				...
				if (Post(*Request, OplogPrepNewEntryPath, Prep.AsObjectView()) && Request->GetResponseCode() == 200)
				{
					FCbObjectView NeedObject = FCbObjectView(Request->GetResponseBuffer().GetData());
				...
				{
					UE_LOGF(LogZenStore, Warning, "Oplog prep FAILED: %ls", *Request->GetDetailedResultString(OplogPrepNewEntryPath));
					UE_LOGF(LogZenStore, Warning, "Unable to check for needed chunks, assuming all are needed");
					for (const FCbAttachment& Attachment : Attachments)
					{
			...
			for (const FCbAttachment& Attachment : Attachments)
			{
				...
				if (NeedChunks.Contains(AttachmentHash))
				{
					...
					if (bCanUseLocalTempFile && AttachmentData.GetSize() >= StandaloneThresholdBytes)
					{
						...
						{
							UE_LOGF(LogZenStore, Warning, "A temp file named '%ls' already exists, taking slow path for large attachment", *TempFilePath);
						}
						else
						{
							if (IFileHandle* FileHandle = PlatformFile.OpenWrite(*TempFilePath))
							{
								...
								else
								{
									...
									UE_LOGF(LogZenStore, Warning, "Could not write to file '%ls', taking slow path for large attachment", *TempFilePath);
								}
							}
							else
							{
								// Take the slow path if we can't open the payload file in the large attachment directory
								UE_LOGF(LogZenStore, Warning, "Could not create file '%ls', taking slow path for large attachment", *TempFilePath);
							}
						}
					}
		...
		if (Post(*Request, NewOpPostUri, SerializedPackage.GetView()) && (Request->GetResponseCode() == 200 || Request->GetResponseCode() == 201))
		{
			return TIoStatusOr<uint64>(SerializedPackage.TotalSize());
		}
		else
		{
			return TIoStatusOr<uint64>((FIoStatus)(FIoStatusBuilder(EIoErrorCode::Unknown) << TEXT("Append OpLog failed, NewOpLogPath='") << OplogNewEntryPath << TEXT("' ") << Request->GetDetailedResultString(OplogPrepNewEntryPath)));
		}
	}
}
...
TIoStatusOr<uint64> FZenStoreHttpClient::GetChunkSize(const FIoChunkId& Id)
{
	...
	if (int ResponseCode = Request->GetResponseCode(); ResponseCode != 404)
	{
		UE_LOGF(LogZenStore, Verbose, "Get chunk request FAILED: %ls", *Request->GetDetailedResultString(ChunkUri));
	}
...
TIoStatusOr<FIoBuffer> FZenStoreHttpClient::ReadOpLogUri(FStringBuilderBase& ChunkUri, uint64 Offset, uint64 Size)
{
	...
	else
	{
		...
		UE_LOGF(LogZenStore, Log, "Get chunk request FAILED: %ls", *Request->GetDetailedResultString(ChunkUri));
		if (ResponseCode == 404)
		{
			return FIoStatus(EIoErrorCode::NotFound);
		}
...
FZenStoreHttpClient::EndBuildPass(FCbPackage OpEntry)
{
	...
	UE_LOGF(LogZenStore, Verbose, "Package size: %lld", SerializedPackage.TotalSize());

	UE::Zen::FZenScopedRequestPtr Request(RequestPool.Get(), false /* bLogErrors */);

	FMemoryView Payload { SerializedPackage.GetData(), (uint64)SerializedPackage.TotalSize()};
	if (Post(*Request, OplogNewEntryPath, Payload) && (Request->GetResponseCode() == 200 || Request->GetResponseCode() == 201))
	{
		return static_cast<uint64>(Payload.GetSize());
	}
	else
	{
		UE_LOGF(LogZenStore, Warning, "Oplog end FAILED: %ls", *Request->GetDetailedResultString(OplogNewEntryPath));
