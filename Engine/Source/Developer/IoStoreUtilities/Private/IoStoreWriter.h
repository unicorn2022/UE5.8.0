...
struct FIoStoreWriterSettings
{
	...
	uint64 PatchInPlaceMaximumTotalGrowth = 0;        // fail build if total overhead goes over this value and bUsePatchInPlaceLayout is enabled, 0 to ignore
	uint64 PatchInPlaceMaximumPerContainerGrowth = 0; // defragment container if it's overhead goes above this value by moving some entries, 0 to ignore
	uint64 PatchInPlaceParetoFrontCoarseSamples = 0;  // amount of coarse samples to use for pareto front probing during defrag
	uint64 PatchInPlaceParetoFrontFineSamples = 0;    // amount of fine samples to fine tune pareto front during defrag
	uint64 PatchInPlaceReadGranularity = 0;           // group inserted entries with-in read granularity bins based on offsets from file open order, 0 to ignore
	uint64 PatchInPlaceMaxChainSize = 0;              // largest possible insertion chain size, 0 to ignore
	int32 PatchInPlaceMaxChainLength = 0;             // largest possible insertion chain length, 0 to ignore
	uint8 PatchInPlaceParetoFrontHeuristic = 0;       // heuristic to choose for pareto front, see FPatchInPlaceLayout::BuildUnpinCandidates
	TMap<FString, uint64> MaxPartitionSizeOverride;
	bool bEnableFileRegions = false;
	bool bContainerMetaEnabled = false;
	bool bPerContainerMeta = false;
...
struct FIoStoreWriterResult
{
	...
	uint64 DeduplicatedSavedBytes = 0;
	FName CompressionMethod = NAME_None;
...
struct FIoWriteOptions
{
	...
	// If valid, this chunk is byte-identical to specified chunk.
	FIoChunkId DuplicateOf = {};
...
public:
	struct FProgress
	{
		...
		uint64 ScheduledCompressionMemoryBytes = 0;
...
public:
	...
	*/
	...
	/*
	* Returns true if ContainerId was found and OutContainerFilePath is set.
	*/
	virtual bool GetContainerFilePath(const FIoContainerId& InContainerId, FString& OutContainerFilePath) = 0;
};
...
public:
	...
	*/
	virtual void SetReferenceChunkDatabase(TSharedPtr<IIoStoreWriterReferenceChunkDatabase> ReferenceChunkDatabase, TSharedPtr<IIoStoreWriterReferenceChunkDatabase> PatchInPlaceReferenceChunkDatabase) = 0;
