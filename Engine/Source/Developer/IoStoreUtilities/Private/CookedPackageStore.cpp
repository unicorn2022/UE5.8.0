...
FIoStatus FCookedPackageStore::LoadManifest(const TCHAR* ManifestFilename)
{
	...
	UE_LOGF(LogIoStore, Display, "Fetched %lld oplog items from %ls in %.2lf seconds",
	...
	if (FJsonSerializer::Deserialize(Reader, ProjectStoreObject) && ProjectStoreObject.IsValid())
	{
		...
		if (ProjectStoreObject->TryGetObjectField(TEXT("zenserver"), ZenServerObjectPtr) && (ZenServerObjectPtr != nullptr))
		{
				...
				ZenServerObject->TryGetStringField(TEXT("oplogid"), OplogId) && !OplogId.IsEmpty())
			{
				...
				UE_LOGF(LogIoStore, Display, "Fetched %lld oplog items from Zen in %.2lf seconds",
					OplogObject["entries"].AsArrayView().Num(),
					FPlatformTime::Seconds() - StartTime);
...
void FCookedPackageStore::ParseOplog(FCbObject& OplogObject)
{
					...
					else if (ChunkObj["clientpath"])
					{
						...
						if (ClientPathView.StartsWith("/{project}/"))
						{
							if (!FPaths::IsProjectFilePathSet())
							{
								...
								UE_LOGF(LogIoStore, Warning, "Project relative path could not be remapped because project file path is unset (possibly due to not specifying uproject path as first argument). %ls", RelativeFilename.ToString());
							}
							else
							{
	...
	UE_LOGF(LogIoStore, Display, "Parsed %lld oplog items %.2lf seconds, %d chunks",
		EntriesArray.Num(),
		FPlatformTime::Seconds() - StartTime,
		ChunkInfoMap.Num());
}
...
FIoStatus FCookedPackageStore::LoadChunkHashes()
{
	...
	if (HasZenStoreClient())
	{
		...
		for (FCbField& ChunkEntry : ChunksObj["chunkinfos"])
		{
			...
			{
				UE_LOGF(LogIoStore, Warning, "Received invalid chunk id, skipping.");
				continue;
			}
	...
	UE_LOGF(LogIoStore, Display, "Loaded %u chunk hashes from %ls in %.2lf seconds, %d hashes updated in %.2lf seconds",
		LoadedHashCount,
