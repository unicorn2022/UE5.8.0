...
public:
	...
	/**
	 * Find the cooked path for the given source-domain path from the CookSandbox, and if found, return the
	 * cookedpath and the (Client,LocalRoot) pair the cookedpath is under.
	 */
	bool TryGetCookedPathOfSourceDomainPath(const FString& SourceDomainPath, FString& OutCookedPath,
	...
	/** Return the relative path from LocalRoot to LocalFilePath joined onto ClientRoot. */
	static FString ConvertLocalPathToClientPath(const FString& ClientRoot, const FString& LocalRoot, FStringView LocalFilePath);
...
#if WITH_EDITOR
void FZenFileSystemManifest::ConstructReferencedSetClientPath(const FString& MetadataDirectoryPath)
{
	using namespace UE::ZenFileSystemManifest;
	FString ReferencedSetLocalPath = FPaths::Combine(MetadataDirectoryPath, UE::Cook::GetReferencedSetFilename());
	FString ProjectClientRoot = TEXT("/{project}");
	FString ProjectLocalRoot = FPaths::Combine(CookDirectory, FApp::GetProjectName());
	ReferencedSetClientPath = FManifestGenerator::ConvertLocalPathToClientPath(ProjectClientRoot,
		ProjectLocalRoot, ReferencedSetLocalPath);
	ReferencedSetClientPath.ReplaceInline(TEXT("\\"), TEXT("/"));
}
void FZenFileSystemManifest::SetCookSandbox(UE::Cook::ICookSandbox* InCookSandbox)
{
	CookSandbox = InCookSandbox;
}
#endif
	...
	const TOptional<ICookedPackageWriter::FReferencedPluginsInfo>& ReferencedPlugins)
{
	...
	for (TSharedRef<IPlugin>& Plugin : Generator.GetApplicablePlugins())
	{
		...
		UE_LOGF(LogZenFileSystemManifest, Verbose, "Plugin '%ls': BaseDir: '%ls'", *PluginName, *BaseDir);

		FString ClientRoot;
		FString LocalRoot;
		FString CookedDir;
		FString CookedClientRoot;
		FString CookedLocalRoot;
		...
		if (Generator.TryGetCookedPathOfSourceDomainPath(LocalizationDir, CookedDir, CookedClientRoot, CookedLocalRoot))
		{
			Generator.AddFilesFromDirectory(CookedClientRoot, CookedLocalRoot, CookedDir, true, &LocalizationFilter);
		}
		...
		for (const FString& ExtensionBaseDir : Generator.GetApplicablePluginExtensionBaseDirs(Plugin))
		{
			...
			UE_LOGF(LogZenFileSystemManifest, Verbose, "Plugin '%ls': ExtensionBaseDir: '%ls'", *PluginName, *ExtensionBaseDir);
			...
			if (Generator.TryGetCookedPathOfSourceDomainPath(ExtensionLocalizationDir, CookedDir, CookedClientRoot, CookedLocalRoot))
			{
				Generator.AddFilesFromDirectory(CookedClientRoot, CookedLocalRoot, CookedDir, true, &LocalizationFilter);
			}
	...
	if (Generator.IsIncludeEngine())
	{
		...
		// Source the ICU data folder name from the cook target's DataDrivenPlatformInfo.ini 
		const FDataDrivenPlatformInfo& TargetDDPI = FDataDrivenPlatformInfoRegistry::GetPlatformInfo(TargetPlatform.IniPlatformName());
		const FString ICUDataVersion = !TargetDDPI.ICUDataVersion.IsEmpty()
			? TargetDDPI.ICUDataVersion
			: FString(TEXT("icudt64l")); // Fallback during the migration period for platforms that have not yet declared ICUDataVersion.
...
#if WITH_EDITOR
	if (!ReferencedSetClientPath.IsEmpty() && Entry.ClientPath == ReferencedSetClientPath)
	{
...
FString FManifestGenerator::ConvertLocalPathToClientPath(const FString& ClientRoot, const FString& LocalRoot, FStringView LocalFilePath)
{
...
void FManifestGenerator::AddSingleFile(const FString& ClientRoot, const FString& LocalRoot, FStringView LocalFilePath)
{
	...
	FString ClientPath(ConvertLocalPathToClientPath(ClientRoot, LocalRoot, LocalFilePath));

	FString ServerRelativePath(LocalFilePath);

	// MakePathRelative returns false if Target and Source are on different drives. If that's the case 
	// we use the full path. Otherwise, we make the path relative to the root dir. 
	// TODO: Absolute paths are not valid for use in paths on the zenserver , because they are supposed to be 
	// transferable. We will need to add an ExternalProjectRoot token to ZenServer paths, which will require 
	// some changes in zenserver to support.
	if (FPaths::MakePathRelativeTo(ServerRelativePath, *(UnrealEngineRootDir / TEXT(""))))
	{
	...
	bool bRecursive, FFileFilter* AdditionalFilter)
{
	...
	{
		UE_LOGF(LogZenFileSystemManifest, Warning,
			"Ignoring additional folder to stage that is not relative to the engine or project directory: %ls",
...
void FManifestGenerator::AddFilesFromExtensionDirectories(const TCHAR* ExtensionSubDir, FFileFilter* AdditionalFilter)
{
	...
	if (IsIncludeEngine())
	{
		...
		for (const FString& LocalDir : ExtensionDirs)
		{
			...
			{
				UE_LOGF(LogZenFileSystemManifest, Warning,
					"GetExtensionDirs on root directory '%ls' unexpectedly returned folder '%ls' that is not a subdirectory of the root directory.",
					*EngineDir, *LocalDir);
				continue;
			}
	...
	DiscoveredPlugins.RemoveAll([this](TSharedRef<IPlugin>& Plugin)
		{
			...
			{
				UE_LOGF(LogZenFileSystemManifest, Verbose, "Skipping plugin '%ls' because it is disabled.", *PluginName);
				return true;
			}
			const FPluginDescriptor& Descriptor = Plugin->GetDescriptor();
			if (!Descriptor.SupportsTargetPlatform(UBTPlatformString))
			{
				UE_LOGF(LogZenFileSystemManifest, Verbose, "Skipping plugin '%ls' because it is not supported on platform '%ls'.",
				...
				{
					UE_LOGF(LogZenFileSystemManifest, Verbose, "       '%ls' supports platform '%ls'",
			...
			if (ReferencedPlugins.IsSet())
			{
				...
				if (!Plugin->CanContainContent())
				{
					...
					{
						UE_LOGF(LogZenFileSystemManifest, Verbose,
							"Skipping plugin '%ls' because it is a code-only Engine plugin, and we did not cook /Engine.",
							*PluginName);
						return true;
					}
					...
					{
						UE_LOGF(LogZenFileSystemManifest, Verbose,
							"Skipping plugin '%ls' because it is a code-only Project plugin, and we did not cook /Game.",
							*PluginName);
						return true;
					}
				}
				else
				{
					...
					{
						UE_LOGF(LogZenFileSystemManifest, Verbose,
							"Skipping plugin '%ls' because we did not cook any content from it.",
							*PluginName);
						return true;
					}
				}
...
bool FManifestGenerator::TryGetCookedPathOfSourceDomainPath(const FString& SourceDomainPath, FString& OutCookedPath, 
	FString& OutClientRoot, FString& OutLocalRoot)
{
#if !WITH_EDITOR
	return false;
#else
	UE::Cook::ICookSandbox* CookSandbox = Manifest.CookSandbox;
	if (!CookSandbox)
	{
		return false;
	}
	const ITargetPlatform& TargetPlatform = Manifest.TargetPlatform;
	
	FString FullPath = FPaths::ConvertRelativePathToFull(SourceDomainPath);
	FString CookedPath = CookSandbox->ConvertToFullPlatformSandboxPath(FullPath, true /* bForWrite */, TargetPlatform.PlatformName());
	if (FullPath.Equals(CookedPath, ESearchCase::IgnoreCase))
	{
		return false;
	}

	FString CookedEngineRoot = GetCookedEngineDir();
	if (FPathViews::IsParentPathOf(CookedEngineRoot, CookedPath))
	{
		OutCookedPath = MoveTemp(CookedPath);
		OutClientRoot = TEXT("/{engine}");
		OutLocalRoot = MoveTemp(CookedEngineRoot);
		return true;
	}
	FString CookedProjectRoot = GetCookedProjectDir();
	if (FPathViews::IsParentPathOf(CookedProjectRoot, CookedPath))
	{
		OutCookedPath = MoveTemp(CookedPath);
		OutClientRoot = TEXT("/{project}");
		OutLocalRoot = MoveTemp(CookedProjectRoot);
		return true;
	}
...
FString FManifestGenerator::GetInternationalizationPresetPath()
{
	...
	{
		UE_LOGF(LogZenFileSystemManifest, Warning, "Failed reading internationalization preset setting, defaulting to English");
