...
FPackageStorePackage* FPackageStoreOptimizer::CreatePackageFromCookedHeader(const FName& Name, const FIoBuffer& CookedHeaderBuffer) const
{
	...
	const int32 ImportCount = CookedHeaderData.ObjectImports.Num();
	const int32 ExportCount = CookedHeaderData.ObjectExports.Num();
	const int32 CellImportCount = CookedHeaderData.CellImports.Num();	
	const int32 CellExportCount = CookedHeaderData.CellExports.Num();
	...
	ProcessImports(CookedHeaderData, Package, UnresolvedImports, UnresolvedExports);
	ProcessExports(CookedHeaderData, Package, UnresolvedImports, UnresolvedExports);
...
FPackageStoreOptimizer::FCookedHeaderData FPackageStoreOptimizer::LoadCookedHeader(const FIoBuffer& CookedHeaderBuffer) const
{
	...
	public:
		...
		FArchive& operator<<(FName& Name)
		{
			...
			{
				UE_LOGF(LogPackageStoreOptimizer, Fatal, "Bad name index %i/%i", NameIndex, NameMap.Num());
			}
...
void FPackageStoreOptimizer::ResolveImport(FPackageStorePackage::FUnresolvedImport* Imports, FPackageStorePackage::FUnresolvedExport* Exports, const FObjectImport* ObjectImports, const FObjectExport* ObjectExports, const FName& PackageName, int32 LocalImportIndex) const
{
	...
	if (Import->FullName.Len() == 0)
	{
		...
		{
			FName ImportPackageName = ObjectImport->ObjectName;
			ImportPackageName.AppendString(Import->FullName);
			Import->FullName.ToLowerInline();
			Import->FullPath = Import->FullName;
			Import->FromPackageName = ImportPackageName;
		...
		else if (ObjectImport->OuterIndex.IsImport())
		{
			const int32 OuterImportIndex = ObjectImport->OuterIndex.ToImport();
			ResolveImport(Imports, Exports, ObjectImports, ObjectExports, PackageName, OuterImportIndex);
			FPackageStorePackage::FUnresolvedImport* OuterImport = Imports + OuterImportIndex;
			...
			Import->FullName.ToLowerInline();
			if (ObjectImport->HasPackageName())
			{
				ObjectImport->GetPackageName().AppendString(Import->FullPath);
				Import->FullPath.AppendChar(TEXT('/'));
				ObjectImport->ObjectName.AppendString(Import->FullPath);
				Import->FullPath.ToLowerInline();
				Import->FromPackageName = ObjectImport->GetPackageName();
				Import->FromPackageNameLen = ObjectImport->GetPackageName().ToString().Len();
			}
			else
			{
				Import->FullPath.Append(OuterImport->FullPath);
				Import->FullPath.AppendChar(TEXT('/'));
				ObjectImport->ObjectName.AppendString(Import->FullPath);
				Import->FullPath.ToLowerInline();
		...
		{
			check(ObjectImport->HasPackageName());
			int32 OuterExportIndex = ObjectImport->OuterIndex.ToExport();
			ResolveExport(Imports, Exports, ObjectImports, ObjectExports, PackageName, OuterExportIndex);
			FPackageStorePackage::FUnresolvedExport* OuterExport = Exports + OuterExportIndex;
			Import->FullName.Append(OuterExport->FullName);
			Import->FullName.AppendChar(TEXT('/'));
			ObjectImport->ObjectName.AppendString(Import->FullName);
			Import->FullName.ToLowerInline();
			ObjectImport->GetPackageName().AppendString(Import->FullPath);
...
void FPackageStoreOptimizer::ProcessImports(const FCookedHeaderData& CookedHeaderData, FPackageStorePackage* Package, TArray<FPackageStorePackage::FUnresolvedImport>& UnresolvedImports, TArray<FPackageStorePackage::FUnresolvedExport>& UnresolvedExports) const
{
	const int32 ImportCount = CookedHeaderData.ObjectImports.Num();
	const int32 CellImportCount = CookedHeaderData.CellImports.Num();
	...
	{
		ResolveImport(UnresolvedImports.GetData(), UnresolvedExports.GetData(), CookedHeaderData.ObjectImports.GetData(), CookedHeaderData.ObjectExports.GetData(), Package->Name, ImportIndex);
	...
	for (int32 CellImportIndex = 0; CellImportIndex < CellImportCount; ++CellImportIndex)
	{
		...
		ResolveImport(UnresolvedImports.GetData(), UnresolvedExports.GetData(), CookedHeaderData.ObjectImports.GetData(), CookedHeaderData.ObjectExports.GetData(), Package->Name, ImportPackageIndex);
	...
	for (int32 ImportIndex = 0; ImportIndex < ImportCount; ++ImportIndex)
	{
		...
		if (UnresolvedImport.bIsScriptImport)
		{
			...
			{
				UE_LOGF(LogPackageStoreOptimizer, Warning, "Package '%ls' is referencing missing script import '%ls'", *Package->Name.ToString(), *UnresolvedImport.FullName);
			}
		...
		if (bImportFromPackage)
		{
			...
			for (uint32 PackageIndex = 0, PackageCount = static_cast<uint32>(Package->ImportedPackages.Num()); PackageIndex < PackageCount; ++PackageIndex)
			{
				...
				{
					FStringView PackageRelativeName = FStringView(UnresolvedImport.FullPath).RightChop(UnresolvedImport.FromPackageNameLen);
	...
	for (int32 CellImportIndex = 0; CellImportIndex < CellImportCount; ++CellImportIndex)
	{
		...
		if (PackageImport.bIsScriptImport)
		{
			...
			if (!ScriptCellsMap.Contains(ScriptCellIndex))
			{
				...
				UE_LOGF(LogPackageStoreOptimizer, Warning, "Package '%ls' is referencing missing script import '%ls'", *Package->Name.ToString(), *VersePath);
			}
...
void FPackageStoreOptimizer::ResolveExport(FPackageStorePackage::FUnresolvedImport* Imports, FPackageStorePackage::FUnresolvedExport* Exports, const FObjectImport* ObjectImports, const FObjectExport* ObjectExports, const FName& PackageName, int32 LocalExportIndex) const
{
	...
	if (Export->FullName.Len() == 0)
	{
		...
		if (ObjectExport->OuterIndex.IsNull())
		{
			...
			Export->FullPath = Export->FullName;
		}
		else
		{
			...
			if (ObjectExport->OuterIndex.IsExport())
			{
				...
				Export->FullPath.Append(*Exports[OuterExportIndex].FullPath);
				Export->FullPath.AppendChar(TEXT('/'));
				ObjectExport->ObjectName.AppendString(Export->FullPath);
				Export->FullPath.ToLowerInline();
			}
			else
			{
				...
				Export->FullPath.Append(PackageName.ToString());
				if (OuterName->StartsWith(PackageName.ToString()))
				{
					FStringView PackageRelativeName = FStringView(*OuterName).RightChop(PackageName.ToString().Len());
					Export->FullPath += PackageRelativeName;
				}
				Export->FullPath.AppendChar(TEXT('/'));
				ObjectExport->ObjectName.AppendString(Export->FullPath);
				Export->FullPath.ToLowerInline();
...
void FPackageStoreOptimizer::ProcessExports(const FCookedHeaderData& CookedHeaderData, FPackageStorePackage* Package, TArray<FPackageStorePackage::FUnresolvedImport>& UnresolvedImports, TArray<FPackageStorePackage::FUnresolvedExport>& UnresolvedExports) const
{
	const int32 ExportCount = CookedHeaderData.ObjectExports.Num();
	const int32 CellExportCount = CookedHeaderData.CellExports.Num();
	...
	for (int32 ExportIndex = 0; ExportIndex < ExportCount; ++ExportIndex)
	{
		...
		Export.bIsPublic = (Export.ObjectFlags & (RF_Public | RF_GeneratePublicHash)) > 0 || ObjectExport.bGeneratePublicHash;
		ResolveExport(UnresolvedImports.GetData(), UnresolvedExports.GetData(), CookedHeaderData.ObjectImports.GetData(), CookedHeaderData.ObjectExports.GetData(), Package->Name, ExportIndex);
		if (Export.bIsPublic)
		{
			check(UnresolvedExport.FullPath.Len() > 0);
			check(UnresolvedExport.FullPath.StartsWith(PackageNameStr));
			FStringView PackageRelativeName = FStringView(UnresolvedExport.FullPath).RightChop(PackageNameStr.Len());
			check(PackageRelativeName.Len());
			...
			{
				UE_LOGF(LogPackageStoreOptimizer, Fatal, "Export hash collision in package \"%ls\": \"%ls\" and \"%ls", *PackageNameStr, PackageRelativeName.GetData(), *FindCollidingExport->FullPath);
			}
	...
	for (int32 CellExportIndex = 0; CellExportIndex < CellExportCount; ++CellExportIndex)
	{
		...
		if (!VersePathUtf8.IsEmpty())
		{
			...
			{
				UE_LOGF(LogPackageStoreOptimizer, Fatal, "Export hash collision in package \"%ls\": \"%ls\" and \"%ls\"", *PackageNameStr, *UnresolvedExport.FullName, *FindCollidingExport->FullName);
			}
			SeenPublicExportHashes.Add(Export.PublicExportHash, &UnresolvedExport);
		}
...
void FPackageStoreOptimizer::FinalizePackageHeader(FPackageStorePackage* Package) const
{
	...
	// NOTE: operator<<(FStructuredArchive::FSlot Slot, FObjectImport& I) added unversioned cooked serialization of I.PackageName, i.e. two int32 fields for every import entry.
#if WITH_EDITORONLY_DATA
	constexpr size_t ExpectedPackageFileSummarySize = 360;
#else
	constexpr size_t ExpectedPackageFileSummarySize = 344;
#endif
	static_assert(sizeof(FPackageFileSummary) == ExpectedPackageFileSummarySize, "Any change to FPackageFileSummary serialization requires adjusting the offset below, transient changes can just update the size in this assert.");
	constexpr uint32 CookedHeaderSizeForImportTypeHierarchies = 8; // sizeof(ImportTypeHierarchiesCount) + sizeof(ImportTypeHierarchiesOffset) = 8
	const uint32 CookedHeaderSizeForImportPackageNames = 8 * Package->Imports.Num(); // sizeof(NameIndex) + sizeof(NameNumber) = 8
	const uint32 CookedHeaderSizeOffsetToMinimizePatchSize = CookedHeaderSizeForImportTypeHierarchies + CookedHeaderSizeForImportPackageNames;
	...
	TMap<FPackageObjectIndex, FScriptObjectData>& OutScriptObjectsMap, FPackageObjectIndex OuterIndex, bool bOuterIsVerseVNI, UObject* Object)
{
	...
	{
		UE_LOGF(LogPackageStoreOptimizer, Verbose, "Skipping script object: %ls (!RF_Public)", *Object->GetFullName());
		return;
	}
	...
	if (ScriptImport)
	{
		UE_LOGF(LogPackageStoreOptimizer, Fatal, "Import name hash collision \"%ls\" and \"%ls", *TempFullName, *ScriptImport->FullName);
	}
	...
	GetObjectsWithOuter(Object, InnerObjects, EGetObjectsFlags::None);
...
void FPackageStoreOptimizer::FindScriptObjects()
{
	...
	if (Verse::VPackage* BuiltInPackage = Verse::GlobalProgram->LookupPackage("$BuiltIn"))
	{
		for (int32 Index = 0; Index < BuiltInPackage->NumDefinitions(); ++Index)
		{
			...
			if (ScriptImport)
			{
				UE_LOGF(LogPackageStoreOptimizer, Fatal, "Import name hash collision \"%ls\" and \"%ls\"", *VersePath.AsString(), *FString(ScriptImport->VersePath));
			}
...
FPackageStoreEntryResource FPackageStoreOptimizer::CreatePackageStoreEntry(const FPackageStorePackage* Package, const FPackageStorePackage* OptionalSegmentPackage) const
{
	...
	for (const FName& SoftRefName : Package->SoftPackageReferences)
	{
		...
		if (!FPackageName::IsScriptPackage(SoftRef))
		{
			...
			{
				UE_LOGF(LogPackageStoreOptimizer, Display, "Skipping invalid soft package reference name '%ls'", *SoftRefName.ToString());
				continue;
			}
