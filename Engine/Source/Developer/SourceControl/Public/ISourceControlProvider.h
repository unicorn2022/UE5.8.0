...
public:
	...
	enum class EInitFlags : uint32
	{
		...
		SuppressErrorLogging = 1 << 1,
		SupressErrorLogging UE_DEPRECATED(5.8, "Spelling typo, please use ISourceControlProvider::EInitFlags::SuppressErrorLogging instead") = 1 << 1
	};
...
public:
	...
	*/
	...
	/**
	 * Whether the provider uses soft revert before delete
	 */
	virtual bool UsesSoftRevertOnDelete() const = 0;
	 ...
	 */
	virtual TOptional<bool> HasChangesToSync() const = 0;

	/**
	 * Whether there is anything to check-in
	 */
	virtual TOptional<bool> HasChangesToCheckIn() const = 0;

	UE_DEPRECATED(5.8, "This function is no longer used.")
	virtual TOptional<bool> IsAtLatestRevision() const final { return TOptional<bool>(); }

	UE_DEPRECATED(5.8, "This function is no longer used.")
	virtual TOptional<int> GetNumLocalChanges() const final { return TOptional<int>(); }
