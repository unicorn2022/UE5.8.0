...
static bool CanQuerySourceControl(const FString& AbsPathname)
{
#if WITH_EDITORONLY_DATA
	FString PackageName;
	if (FPackageName::TryConvertFilenameToLongPackageName(AbsPathname, PackageName))
	{
		if (UPackage* Package = FindPackage(nullptr, *PackageName))
		{
			// There's nothing to query if the package didn't come from disk or hasn't yet been saved to disk.
			return Package->GetHasBeenEndLoaded();
		}
	}
#endif // WITH_EDITORONLY_DATA
...
, RemainingPeriodPolicy(FTimespan::FromSeconds(5))
...
bool FSourceControlFileStatusMonitor::Tick(float DeltaTime)
{
	...
	if (NewAddedFileCount == 0 && NowSecs - OldestFileStatusTimeSecs < RefreshPeriodPolicy.GetTotalSeconds() && LastRemainingFileTimeSecs == 0.f)
	{
		return true; // Nothing to do this time around.
	}

	// Throttle the checks when remaining files are left.
	if (NewAddedFileCount == 0 && NowSecs - LastRemainingFileTimeSecs < RemainingPeriodPolicy.GetTotalSeconds() && LastRemainingFileTimeSecs != 0.f)
	{
		return true; // Delay the retry.
	}
	...
	TRACE_CPUPROFILER_EVENT_SCOPE(FSourceControlFileStatusMonitor::Tick);
	...
	uint32 NewFilesRemaining = NewFiles.Num();
	uint32 RefreshedFilesRemaining = RefreshedFiles.Num();
	...
	while (RequestedStatusFiles.Num() < MaxFileNumPerRequestPolicy)
	{
		if (NewFiles.Num() > 0)
		{
			const FString& AbsPathname = NewFiles.Last()->Key;
			if (CanQuerySourceControl(AbsPathname))
			{
				RequestedStatusFiles.Emplace(AbsPathname);
				NewFilesRemaining--;
			}
		...
		{
			const FString& AbsPathname = RefreshedFiles.Last()->Key;
			if (CanQuerySourceControl(AbsPathname))
			{
				RequestedStatusFiles.Emplace(AbsPathname);
				RefreshedFilesRemaining--;
			}
	...
	LastRemainingFileTimeSecs = NewFilesRemaining > 0 ? FPlatformTime::Seconds() : 0.f;
