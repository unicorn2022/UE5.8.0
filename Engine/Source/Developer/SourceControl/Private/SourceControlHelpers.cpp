...
void RunQueryFileStates(TConstArrayView<FString> InFiles, EConcurrency::Type InConcurrencyType, bool bSilent, bool bUseSourceControlStateCache, TFunction<void(FSourceControlState)> FileStateCallback)
{
	...
	auto SetStatusAndRunCallback = [](const TArray<FString>& InFilesToUpdate, const ISourceControlProvider* Provider, const TFunction<void(FSourceControlState)>& InCallbackFn)
	{
		TArray<FSourceControlStateRef> SCStates;
		const_cast<ISourceControlProvider*>(Provider)->GetState(InFilesToUpdate, SCStates, EStateCacheUsage::Use);
	...
	if (!bUseSourceControlStateCache)
	{
		...
		ISourceControlModule::Get().GetProvider().Execute(
			...
			SCFilesToUpdate,
			InConcurrencyType,
			FSourceControlOperationComplete::CreateLambda([FileStateCallback, Provider, SCFilesToUpdate, SetStatusAndRunCallback](const FSourceControlOperationRef& InOperation, ECommandResult::Type InResult)
			{
				SetStatusAndRunCallback(SCFilesToUpdate, Provider, FileStateCallback);
			}));
	}
	else
	{
...
bool USourceControlHelpers::ApplyOperationAndReloadPackages(const TArray<FString>& InFilenames,	const TFunctionRef<bool(const TArray<FString>&)>& InOperation, bool bReloadWorld, bool bInteractive)
{
	...
	double TimeTakenFlush = 0;
	double TimeTakenReload = 0;
	double TimeTakenOperation = 0;
	double TimeTakenUnload = 0;
	...
	if (!FilteredPackages.IsEmpty())
	{
		...
		const FString Packages = *Builder;

		UE_LOGF(LogSourceControl, Warning, "This operation could not complete on the following map or external packages, please unload them before retrying : %ls", *Packages);
		return false;
	}
	...
	{
		const double TimeStartedFlush = FPlatformTime::Seconds();
		...
		TimeTakenFlush = FPlatformTime::Seconds() - TimeStartedFlush;
	}
	...
	// Apply Operation
	{
		const double TimeStartedOperation = FPlatformTime::Seconds();

		bSuccess = InOperation(PackageFilenames);

		TimeTakenOperation = FPlatformTime::Seconds() - TimeStartedOperation;
	}
	...
	{
		const double TimeStartedReload = FPlatformTime::Seconds();

		FBookmarkScoped BookmarkScoped;
		FScopedSlowTask SlowTaskScoped(1.f, NSLOCTEXT("SourceControl", "Reload", "Reloading..."));
		SlowTaskScoped.MakeDialog();
		SlowTaskScoped.EnterProgressFrame(1.f);
		SlowTaskScoped.ForceRefresh();
		...
		if (LoadedNonWorldPackages.Num() > 0)
		{
			...
			{
				UE_LOGF(LogSourceControl, Warning, "%ls", *OutReloadErrorMsg.ToString());
			}
		}
		...
		TimeTakenReload = FPlatformTime::Seconds() - TimeStartedReload;
	}
	...
	{
		const double TimeStartedUnload = FPlatformTime::Seconds();

		if (ObjectTools::DeleteObjectsUnchecked(ObjectsToDelete) != ObjectsToDelete.Num())
		{
			UE_LOGF(LogSourceControl, Warning, "Failed to unload some assets.");
		}

		TimeTakenUnload = FPlatformTime::Seconds() - TimeStartedUnload;
	}


	// Re-cache the SCC state...
	ISourceControlModule::Get().GetProvider().Execute(ISourceControlOperation::Create<FUpdateStatus>(), PackageFilenames, EConcurrency::Asynchronous);

	// Log timings...
	const double TimeTaken = FPlatformTime::Seconds() - TimeStarted;
	if (TimeTaken > 1.f)
	{
		TStringBuilder<512> TimingMessage;
		TimingMessage.Appendf(TEXT("ApplyOperationAndReloadPackages on %d file(s) took %.3fs"), InFilenames.Num(), TimeTaken);
		if (TimeTakenOperation > 0)
		{
			TimingMessage.Appendf(TEXT(" - operation: %.3fs"), TimeTakenOperation);
		}
		if (TimeTakenReload > 0)
		{
			TimingMessage.Appendf(TEXT(" - reload: %.3fs"), TimeTakenReload);
		}
		if (TimeTakenFlush > 0)
		{
			TimingMessage.Appendf(TEXT(" - flush: %.3fs"), TimeTakenFlush);
		}
		if (TimeTakenUnload > 0)
		{
			TimingMessage.Appendf(TEXT(" - unload: %.3fs"), TimeTakenUnload);
		}
		if (bReloadWorld)
		{
			TimingMessage.Appendf(TEXT(" - ReloadWorld"));
		}
		UE_LOGF(LogSourceControl, Log, "%ls", *TimingMessage);

		if (FEngineAnalytics::IsAvailable())
		{
			TArray<FAnalyticsEventAttribute> Attributes;
			Attributes.Emplace(TEXT("Duration.Seconds"), TimeTaken);
			Attributes.Emplace(TEXT("Duration.Seconds.Operation"), TimeTakenOperation);
			Attributes.Emplace(TEXT("Duration.Seconds.Reload"), TimeTakenReload);
			Attributes.Emplace(TEXT("Duration.Seconds.Flush"), TimeTakenFlush);
			Attributes.Emplace(TEXT("Duration.Seconds.Unload"), TimeTakenUnload);

			FEngineAnalytics::GetProvider().RecordEvent(TEXT("Editor.Usage.SourceControl.ApplyOperationAndReloadPackages"), Attributes);
		}
	}
	...
	TArray<FSourceControlStateRef> PendingDeleteItems = SourceControlProvider.GetCachedStateByPredicate(
		[&PackageNames, &FileNamesToPackageNames](const FSourceControlStateRef& State)
		{
			...
			if (FPackageName::IsPackageFilename(Filename))
			{
				...
				{
					UE_LOGF(LogSourceControl, Warning, "%ls", *FailureReason);
					return false;
				}
...
TArray<FSourceControlState> USourceControlHelpers::QueryFileStates(const TArray<FString> InFiles, bool bSilent, bool bUseSourceControlStateCache)
{
	TArray<FSourceControlState> States;

	SourceControlHelpersInternal::RunQueryFileStates(InFiles, EConcurrency::Synchronous, bSilent, bUseSourceControlStateCache,
...
FSourceControlState USourceControlHelpers::QueryFileState(const FString& InFile, bool bSilent, bool bUseSourceControlStateCache)
{
	FSourceControlState State;
	State.Filename = InFile;
	SourceControlHelpersInternal::RunQueryFileStates(TArray<FString> {InFile}, EConcurrency::Synchronous, bSilent, bUseSourceControlStateCache,
...
{
	bool constexpr bUseCachedSourceControlState = false;
	SourceControlHelpersInternal::RunQueryFileStates(InFiles, EConcurrency::Asynchronous, bSilent, bUseCachedSourceControlState,
