...
void FSourceControlModule::Tick()
{
	if (CurrentSourceControlProvider != nullptr)
	{
		...
		{
			// determine delay before processing pending requests
			const int32 Delay = SourceControlCVars::CVarSourceControlIntervalBatchStatusUpdate.GetValueOnGameThread();

			// check for any pending dispatches
			const FDateTime Now = FDateTime::UtcNow();
			if ((PendingStatusUpdateFiles.Num() > 0) && (PendingStatusUpdateQueuedTime + FTimespan::FromSeconds(Delay) < Now))
			{
				// get batch size
	...
	if (IsEnabled())
	{
		...
		if (SourceControlState.IsValid())
		{
			...
			if (TimeSinceLastUpdate > SourceControlConstants::StateRefreshInterval)
			{
				...
				PendingStatusUpdateQueuedTime = FDateTime::UtcNow();
			}
		}
	}
}
...
void FSourceControlModule::SetProvider( const FName& InName )
{
	...
	UE_LOGF(LogSourceControl, Fatal, "Tried to set unknown revision control provider: %ls", *InName.ToString());
}
		...
		for (const FSourceControlStateRef& SCCState : PackageSCCStates)
		{
			if (SCCState->IsSourceControlled())
			{
				if (!SCCState->CanDelete())
				{
					...
					UE_LOGF(LogSourceControl, Warning, "Asset(s) can't be deleted because at least one is currently checked out by another user and/or not at latest.");
					return;
				}
			}
		}
...
void FSourceControlModule::RegisterPreSubmitChangelistValidationEx(const FSourceControlPreSubmitChangelistValidationExDelegate& PreSubmitChangelistValidationDelegate)
{
...
{
	OnSourceControlPreSubmitChangelistValidation = FSourceControlPreSubmitChangelistValidationExDelegate();
}

FSourceControlPreSubmitChangelistValidationExDelegate FSourceControlModule::GetRegisteredPreSubmitChangelistValidationEx()
{
	return OnSourceControlPreSubmitChangelistValidation;
}

void FSourceControlModule::RegisterPreSubmitFilesValidationEx(const FSourceControlPreSubmitFilesValidationExDelegate& PreSubmitFilesValidationDelegate)
{
...
{
	OnSourceControlPreSubmitFilesValidation = FSourceControlPreSubmitFilesValidationExDelegate();
}

FSourceControlPreSubmitFilesValidationExDelegate FSourceControlModule::GetRegisteredPreSubmitFilesValidationEx()
{
