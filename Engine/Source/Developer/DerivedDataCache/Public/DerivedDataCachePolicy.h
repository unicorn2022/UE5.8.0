 ...
 *
 * Put(Default | SkipData): Store to every cache, do not overwrite existing valid records or values, and skip fetching
 *                          values from conflicting existing records for the record returned in the response.
 * Put(Default): Store to every cache, and do not overwrite existing valid records or values, and fetch values
 *               from conflicting existing records for the record returned in the response.
...
enum class ECachePolicy : uint32
{
	...
	/**
	 * Values associated with this policy are known to be non-deterministic.
	 *
	 * This is meant to be applied to values that are expected to differ between builds.
	 * Disables reporting of non-determinism in put requests and disables verification of
	 * determinism for associated values when running in verify mode.
	 *
	 * DO NOT APPLY THIS FLAG TO VALUES THAT ARE WRITTEN TO COOKED PACKAGES.
	 */
	NonDeterministic = 1 << 8,
};
...
struct FCacheValuePolicy
{
	...
	static constexpr ECachePolicy PolicyMask = ECachePolicy::Default | ECachePolicy::SkipData | ECachePolicy::NonDeterministic;
};
...
/** Apply the policy to the value by removing data if SkipData is present. */
UE_API FValue ApplySkipPolicy(const FValue& Value, ECachePolicy Policy);
/** Apply the policy to the value by removing data if SkipData is present. */
UE_API FValueWithId ApplySkipPolicy(const FValueWithId& Value, ECachePolicy Policy);
/** Apply the policy to the record by removing data if SkipData is present and metadata if SkipMeta is present. */
UE_API FCacheRecord ApplySkipPolicy(const FCacheRecord& Record, const FCacheRecordPolicy& Policy);
