...
public:
	...
	**/
	...
	UE_DEPRECATED(5.8, "The cache no longer uses this internally.")
	void NotifyBootComplete() {}

	UE_DEPRECATED(5.8, "The cache tracks its async operations internally. External modifications are no longer supported.")
	...
	UE_DEPRECATED(5.8, "Querying cache directories is no longer supported.")
	void GetDirectories(TArray<FString>& OutResults) {}

	UE_DEPRECATED(5.8, "Querying whether a shared cache is being used is no longer supported.")
	...
	/**
	 * Retrieve stats about the integrity of DDC data vs generated data
	 */
	UE_INTERNAL virtual void GatherVerificationStats(FDerivedDataCacheVerificationStats& OutStats) const = 0;
	...
	enum UE_DEPRECATED(5.8, "Notifications from the cache are no longer supported.") EDDCNotification
	{
		// DDC performance notification generated when not using a shared cache
		SharedDDCPerformanceNotification
	};

	using FOnDDCNotification UE_DEPRECATED(5.8, "Notifications from the cache are no longer supported.") = TMulticastDelegate<void (EDDCNotification)>;

	UE_DEPRECATED(5.8, "Notifications from the cache are no longer supported.")
