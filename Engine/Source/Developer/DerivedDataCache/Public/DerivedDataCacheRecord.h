...
public:
	...
	UE_API explicit FCacheRecord(const FCacheKey& Key);
	...
	/** Append the cache record to an existing package and writer, as an object, for e.g. a batch of records. */
	...
	UE_INTERNAL static FCacheRecord CreateByMove(const FCacheKey& Key, FCbObject&& Meta, TArray<FValueWithId>&& Values);
