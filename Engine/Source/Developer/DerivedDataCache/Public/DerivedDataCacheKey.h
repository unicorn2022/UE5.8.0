...
template <typename CharType>
inline bool TryLexFromString(FCacheBucket& OutBucket, TStringView<CharType> String)
{
	if (FCacheBucket::IsValidName(String))
	{
		OutBucket = FCacheBucket(String);
		return true;
	}
	return false;
}
...
UE_API bool TryLexFromString(FCacheKey& OutKey, FAnsiStringView String);
UE_API bool TryLexFromString(FCacheKey& OutKey, FWideStringView String);
 ...
 *
 * Keys converted with this function are the same keys used internally by the legacy cache API.
 *
...
/**
 * Builds a cache key from the same components as the legacy BuildCacheKey API.
 *
 * Equivalent to ConvertLegacyCacheKey(FDerivedDataCacheInterface::BuildCacheKey(PluginName, VersionString, PluginSpecificCacheKeySuffix)).
 */
UE_API FCacheKey BuildLegacyCacheKey(FStringView PluginName, FStringView VersionString, FStringView PluginSpecificCacheKeySuffix);
