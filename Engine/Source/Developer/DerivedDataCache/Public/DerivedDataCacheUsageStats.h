...
// DDC-Verify collects data about a subset of newly generated derived data, and whether it matches existing cached values
// This object is used to track statistics about verification results and is collected per DDC bucket
struct FDerivedDataCacheVerificationStat
{
	FString BucketName;						// name of the bucket
	uint32 RecordsTested = 0;				// num records that were tested against the cache
	uint32 RecordsMatchedCache = 0;			// num records that match existing cache data
	uint32 RecordsNotInCache = 0;			// records that have no existing data in the cache 
	uint32 ValuesTested = 0;				// multiple values per record
	uint64 ValuesTestedBytes = 0;			// total size of values tested
	uint32 ValuesMatchedCache = 0;			// num values that match existing cache data
	uint64 ValuesMatchedCacheBytes = 0;		// total size of matching data
	uint32 ValuesNotInCache = 0;			// num values that have no existing data in the cache
	uint64 ValuesNotInCacheBytes = 0;		// num bytes of new values written to the cache
	uint32 ValuesChanged = 0;				// num values existing in the cache that don't match the new data
	uint64 ValuesChangedBytes = 0;			// num bytes of changed values detected (note this tracks the entire value, not the individual changes)
	uint32 ValuesNotInRequest = 0;			// num values that were in the cache before, but not in the new data
	uint64 ValuesNotInRequestBytes = 0;		// num bytes of data we are no longer writing in values

	const FDerivedDataCacheVerificationStat& operator+=(const FDerivedDataCacheVerificationStat& OtherStat)
	{
		RecordsTested += OtherStat.RecordsTested;
		RecordsMatchedCache += OtherStat.RecordsMatchedCache;
		RecordsNotInCache += OtherStat.RecordsNotInCache;
		ValuesTested += OtherStat.ValuesTested;
		ValuesTestedBytes += OtherStat.ValuesTestedBytes;
		ValuesMatchedCache += OtherStat.ValuesMatchedCache;
		ValuesMatchedCacheBytes += OtherStat.ValuesMatchedCacheBytes;
		ValuesNotInCache += OtherStat.ValuesNotInCache;
		ValuesNotInCacheBytes += OtherStat.ValuesNotInCacheBytes;
		ValuesChanged += OtherStat.ValuesChanged;
		ValuesChangedBytes += OtherStat.ValuesChangedBytes;
		ValuesNotInRequest += OtherStat.ValuesNotInRequest;
		ValuesNotInRequestBytes += OtherStat.ValuesNotInRequestBytes;
		return *this;
	}
};

struct FDerivedDataCacheVerificationStats
{
	TArray<FDerivedDataCacheVerificationStat> Entries;
};
