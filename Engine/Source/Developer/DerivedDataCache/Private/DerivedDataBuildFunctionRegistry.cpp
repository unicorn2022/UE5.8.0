...
void FBuildFunctionRegistry::AddFunctionNoLock(IBuildFunctionFactory* Factory)
{
	...
	UE_CLOGF(!Function.GetVersion().IsValid(), LogDerivedDataBuild, Error,
		"Version of zero is not allowed in build function with the name %ls.", *WriteToString<32>(FunctionName));
	UE_CLOGF(Functions.FindByHash(FunctionHash, FunctionName), LogDerivedDataBuild, Error,
		"More than one build function has been registered with the name %ls.", *WriteToString<32>(FunctionName));
