...
struct FS3CacheStore::FRootManifest
{
	...
	bool Load(const FString& InRootManifestPath)
	{
		...
		{
			UE_LOGF(LogDerivedDataCache, Warning, "Unable to read manifest from %ls", *InRootManifestPath);
			return false;
		}
		...
		{
			UE_LOGF(LogDerivedDataCache, Warning, "Unable to parse manifest from %ls", *InRootManifestPath);
			return false;
		}
		...
		{
			UE_LOGF(LogDerivedDataCache, Warning, "Root manifest %ls does not specify AccessKey", *InRootManifestPath);
			return false;
		}
		if (!RootManifestObject->TryGetStringField(TEXT("SecretKey"), SecretKey))
		{
			UE_LOGF(LogDerivedDataCache, Warning, "Root manifest %ls does not specify SecretKey", *InRootManifestPath);
			return false;
		}
		...
		{
			UE_LOGF(LogDerivedDataCache, Warning, "Root manifest from %ls is missing entries array", *InRootManifestPath);
			return false;
		}
	...
	, bEnabled(false)
{
	...
	if (RootManifest.Load(InRootManifestPath))
	{
		...
		if (LocalManifestPath.IsEmpty())
		{
			if (GIsBuildMachine)
			{
				UE_LOGF(LogDerivedDataCache, Log, "S3DerivedDataBackend: Disabling on build machine");
				bCanaryValid = false;
			}
			else if (CanaryObjectKey.Len() > 0)
			{
				...
				{
					UE_LOGF(LogDerivedDataCache, Log, "S3DerivedDataBackend: Unable to download canary file. Disabling.");
					bCanaryValid = false;
				}
			}
		}
		...
		{
			UE_LOGF(LogDerivedDataCache, Log, "S3DerivedDataBackend: Disabling due to config setting");
			bCanaryValid = false;
		}
		...
		if (bCanaryValid)
		{
			UE_LOGF(LogDerivedDataCache, Log, "Using %ls S3 backend at %ls", *Region, *BaseUrl);
...
void FS3CacheStore::ReadBundle(FBundle& Bundle)
{
	...
	{
		UE_LOGF(LogDerivedDataCache, Warning, "Unable to open bundle %ls for reading. Ignoring.", *Bundle.LocalFile);
		return;
	}
	...
	{
		UE_LOGF(LogDerivedDataCache, Warning, "Unable to read bundle with signature %08x", Header.Signature);
		return;
	}
		...
		RequestStats.Op = ERequestOp::GetChunk;
		{
			...
			if (bHasValue)
			{
				...
				UE_CLOGFMT(!bRawHashMatches, LogDerivedDataCache, Verbose,
					...
					*GetName(), Request.Id, Value.GetRawHash(), Request.RawHash, Request.Key, Request.Name);
			}
		}
		RequestStats.Status = Status;
		StoreStats->AddRequest(RequestStats);
...
ILegacyCacheStore* CreateS3CacheStore(const TCHAR* Name, const TCHAR* Config, ICacheStoreOwner& Owner)
{
	...
	{
		UE_LOGF(LogDerivedDataCache, Error, "%ls: Missing required parameter 'Manifest'", Name);
		return nullptr;
	}
	...
	{
		UE_LOGF(LogDerivedDataCache, Error, "%ls: Missing required parameter 'BaseUrl'", Name);
		return nullptr;
	}
	...
	{
		UE_LOGF(LogDerivedDataCache, Error, "%ls: Missing required parameter 'Region'", Name);
		return nullptr;
	}
	...
	{
		UE_LOGF(LogDerivedDataCache, Log, "%ls: Disabled because the path is configured to 'None'", Name);
		return nullptr;
	}
...
#else
	UE_LOGF(LogDerivedDataCache, Log, "S3 backend is not supported on the current platform.");
	return nullptr;
#endif
}
