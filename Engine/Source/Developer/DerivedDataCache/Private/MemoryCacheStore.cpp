	...
	, bCanBeDisabled(FParse::Param(InConfig, TEXT("Boot")))
{
	...
	UE_CLOG(SignedMaxCacheSize >= 0, LogDerivedDataCache, Display, TEXT("%s: Max Cache Size: %" INT64_FMT " MB"), *Name, SignedMaxCacheSize);
	...
	StoreOwner.Add(Name, this, Flags);
	...
	FOnCachePutComplete&& OnComplete)
{
	...
	{
		FCacheRecord ResponseRecord = Request.Record;
		EStatus Status = EStatus::Error;
		ON_SCOPE_EXIT
		{
			PRAGMA_DISABLE_DEPRECATION_WARNINGS;
			OnComplete({Request.Name, Request.Record.GetKey(), ApplySkipPolicy(ResponseRecord, Request.Policy), Request.UserData, Status});
		...
		FCacheRecordComponents& Components = [this, &Key, &bHasExisting]() -> FCacheRecordComponents&
		{
			if (FCacheRecordComponents* Existing = CacheRecords.Find(Key))
			{
				bHasExisting = true;
				return *Existing;
			}
			return CacheRecords.Add(Key);
		}();
		Status = EStatus::Ok;

		ON_SCOPE_EXIT
		{
			ResponseRecord = FCacheRecord::CreateByMove(Key, CopyTemp(Components.Meta), CopyTemp(Components.Values));
			if (!bHasExisting && Status == EStatus::Error)
			{
	...
	FOnCacheGetComplete&& OnComplete)
{
	...
	for (const FCacheGetRequest& Request : Requests)
	{
		...
		if (FReadScopeLock ScopeLock(SynchronizationObject); const FCacheRecordComponents* FoundComponents = CacheRecords.Find(Key))
		{
	...
	FOnCachePutValueComplete&& OnComplete)
{
	...
	{
		FValue ResponseValue = Request.Value;
		EStatus Status = EStatus::Error;
		ON_SCOPE_EXIT
		{
			OnComplete({Request.Name, Request.Key, ApplySkipPolicy(ResponseValue, Request.Policy), Request.UserData, Status});
		};
		...
		{
			ResponseValue = *ExistingValue;
			Status = EStatus::Ok;
			continue;
		}

		const int64 ExistingValueSize = ExistingValue ? ExistingValue->GetData().GetCompressedSize() : 0;
		const int64 RequiredSize = Value.HasData() ? Value.GetData().GetCompressedSize() - ExistingValueSize : 0;
		...
		if (!Value.HasData())
		{
			UE_LOGF(LogDerivedDataCache, Verbose, "%ls: Cache put missing data for put of %ls from '%ls'",
	...
	FOnCacheGetChunkComplete&& OnComplete)
{
	...
	for (const FCacheGetChunkRequest& Request : Requests)
	{
		...
		if (bHasValue && (ValueKey == Request.Key) && (ValueId == Request.Id) && (bExistsOnly || Reader.HasSource()))
		{
		...
		if (bProcessHit && (Request.RawHash.IsZero() || Request.RawHash == Value.GetRawHash()))
		{
...
ILegacyCacheStore* CreateMemoryCacheStore(const TCHAR* Name, const TCHAR* Config, ICacheStoreOwner& Owner)
{
	...
	CreateMemoryCacheStore(Cache, Name, Config, Owner);
	return Cache;
}
