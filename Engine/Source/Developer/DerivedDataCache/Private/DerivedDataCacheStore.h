...
enum class ECacheStoreFlags : uint32
{
	...
	/** Async operation is natively provided by this cache store for requests of non-blocking priority. */
	NativeAsync     = 1 << 6,
};
...
{
public:
	virtual ~ICacheStoreOwner() = default;

	virtual void Add(const FSharedString& Name, ILegacyCacheStore* CacheStore, ECacheStoreFlags Flags) = 0;
	...
	virtual void AddMaintainer(ICacheStoreMaintainer* Maintainer) = 0;
	virtual void RemoveMaintainer(ICacheStoreMaintainer* Maintainer) = 0;

	virtual int32 AddToAsyncTaskCounter(int32 Value) = 0;
...
struct TCacheStoreFunctionFor;

template <>
struct TCacheStoreFunctionFor<FCachePutRequest>
{
	constexpr static void (ICacheStore::*Function)(TConstArrayView<FCachePutRequest>, IRequestOwner&, TCacheOnCompleteFor<FCachePutRequest>&&) = &ICacheStore::Put;
};

template <>
struct TCacheStoreFunctionFor<FCacheGetRequest>
{
	constexpr static void (ICacheStore::*Function)(TConstArrayView<FCacheGetRequest>, IRequestOwner&, TCacheOnCompleteFor<FCacheGetRequest>&&) = &ICacheStore::Get;
};

template <>
struct TCacheStoreFunctionFor<FCachePutValueRequest>
{
	constexpr static void (ICacheStore::*Function)(TConstArrayView<FCachePutValueRequest>, IRequestOwner&, TCacheOnCompleteFor<FCachePutValueRequest>&&) = &ICacheStore::PutValue;
};

template <>
struct TCacheStoreFunctionFor<FCacheGetValueRequest>
{
	constexpr static void (ICacheStore::*Function)(TConstArrayView<FCacheGetValueRequest>, IRequestOwner&, TCacheOnCompleteFor<FCacheGetValueRequest>&&) = &ICacheStore::GetValue;
};

template <>
struct TCacheStoreFunctionFor<FCacheGetChunkRequest>
{
	constexpr static void (ICacheStore::*Function)(TConstArrayView<FCacheGetChunkRequest>, IRequestOwner&, TCacheOnCompleteFor<FCacheGetChunkRequest>&&) = &ICacheStore::GetChunks;
};

} // Private

/** Alias for the cache store function corresponding to the request type. */
template <typename RequestType> inline constexpr auto CacheStoreFunctionFor = Private::TCacheStoreFunctionFor<RequestType>::Function;
