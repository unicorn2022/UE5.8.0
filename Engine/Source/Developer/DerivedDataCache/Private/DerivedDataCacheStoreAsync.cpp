...
{
public:
	FCacheStoreAsync(ICacheStoreOwner& StoreOwner, ILegacyCacheStore* InnerCache, IMemoryCacheStore* MemoryCache, bool bDeleteInnerCache);
		...
		FOnCachePutComplete&& OnComplete) override
	{
		if (MemoryCache)
		{
			...
			Execute(COOK_STAT(&FDerivedDataCacheUsageStats::TimePut,) Requests, Owner,
				...
				{
					MemoryCache->Delete(Response.Record.GetKey(), Response.Name);
...
FCacheStoreAsync::FCacheStoreAsync(ICacheStoreOwner& InStoreOwner, ILegacyCacheStore* InInnerCache, IMemoryCacheStore* InMemoryCache, bool bInDeleteInnerCache)
	...
	OnExecuteType&& OnExecute)
{
	auto ExecuteWithStats = [this, COOK_STAT(OnAddStats,) OnExecute](TConstArrayView<RequestType> Requests, IRequestOwner& Owner, OnCompleteType&& OnComplete) mutable
	{
		Invoke(OnExecute, *InnerCache, Requests, Owner, [this, COOK_STAT(OnAddStats,) OnComplete = MoveTemp(OnComplete)](auto&& Response)
		{
			...
			OnComplete(MoveTemp(Response));
			StoreOwner.AddToAsyncTaskCounter(-1);
		});
	};

	StoreOwner.AddToAsyncTaskCounter(Requests.Num());
...
{
	return new FCacheStoreAsync(Owner, InnerCache, MemoryCache, bDeleteInnerCache);
}
