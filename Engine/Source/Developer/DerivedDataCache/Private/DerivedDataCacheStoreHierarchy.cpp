...
ILegacyCacheStore* CreateCacheStoreAsync(ICacheStoreOwner& Owner, ILegacyCacheStore* InnerCache, IMemoryCacheStore* MemoryCache, bool bDeleteInnerCache);

///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

class FCacheStoreHierarchy final : public ILegacyCacheStore, public ICacheStoreOwner, public ICacheStoreMaintainer
{
public:
	explicit FCacheStoreHierarchy(
		ICacheStoreOwner*& OutOwner,
		ICacheStoreMaintainer*& OutMaintainer,
		TFunctionRef<void (IMemoryCacheStore*&)> MemoryCacheCreator);
	~FCacheStoreHierarchy() final;

	// ICacheStoreOwner Interface

	void Add(const FSharedString& Name, ILegacyCacheStore* CacheStore, ECacheStoreFlags Flags) final;
	...
	void AddMaintainer(ICacheStoreMaintainer* Maintainer) final;
	void RemoveMaintainer(ICacheStoreMaintainer* Maintainer) final;

	int32 AddToAsyncTaskCounter(int32 Value) final;
	...
	void SetShuttingDown() final { bShuttingDown.store(true, std::memory_order_relaxed); }
	bool IsShuttingDown() const final { return bShuttingDown.load(std::memory_order_relaxed); }
...
private:
	...
	struct FCacheStoreNode
	{
		...
		ILegacyCacheStore* AsyncCache{};
		TArray<FCacheStoreStats*> CacheStats;
	};

	mutable FSharedRecursiveMutex NodesMutex;
	...
	std::atomic<int32> AsyncTaskCounter = 0;
	std::atomic<bool> bShuttingDown = false;

	TArray<ICacheStoreMaintainer*> CacheStoreMaintainers;
	mutable FSharedMutex CacheStoreMaintainerMutex;
};
...
FCacheStoreHierarchy::~FCacheStoreHierarchy()
{
	...
	while (!Nodes.IsEmpty())
	{
		...
		if (Node.AsyncCache != Node.Cache)
		{
			delete Node.AsyncCache;
			// Deleting Node.Cache below can access Node.AsyncCache if it calls RemoveNotSafe.
			// Assign it here as if this was a native-async node.
			Node.AsyncCache = Node.Cache;
		}
		delete Node.Cache;
...
void FCacheStoreHierarchy::Add(const FSharedString& Name, ILegacyCacheStore* CacheStore, ECacheStoreFlags Flags)
{
	TUniqueLock Lock(NodesMutex);
	...
	ILegacyCacheStore* AsyncCacheStore = EnumHasAnyFlags(Flags, ECacheStoreFlags::NativeAsync) ? CacheStore
		: CreateCacheStoreAsync(*this, CacheStore, /*MemoryCache*/ nullptr, /*bDeleteInnerCache*/ false);
	Nodes.Emplace(Name, CacheStore, Flags, ECacheStoreNodeFlags::None, AsyncCacheStore, TArray<FCacheStoreStats*>{});
	UpdateNodeFlags();
}
...
void FCacheStoreHierarchy::AddMaintainer(ICacheStoreMaintainer* Maintainer)
{
	TUniqueLock Lock(CacheStoreMaintainerMutex);
	CacheStoreMaintainers.Add(Maintainer);
}

void FCacheStoreHierarchy::RemoveMaintainer(ICacheStoreMaintainer* Maintainer)
{
	TUniqueLock Lock(CacheStoreMaintainerMutex);
	CacheStoreMaintainers.Remove(Maintainer);
}

bool FCacheStoreHierarchy::IsIdle() const
{
	TSharedLock Lock(CacheStoreMaintainerMutex);
	return Algo::AllOf(CacheStoreMaintainers, &ICacheStoreMaintainer::IsIdle);
}

void FCacheStoreHierarchy::BoostPriority()
{
	TSharedLock Lock(CacheStoreMaintainerMutex);
...
int32 FCacheStoreHierarchy::AddToAsyncTaskCounter(int32 Value)
{
	return AsyncTaskCounter.fetch_add(Value);
}
...
void FCacheStoreHierarchy::WaitForIdle() const
{
	const double StartTime = FPlatformTime::Seconds();
	double LastPrint = StartTime;
	while (const int32 Counter = AsyncTaskCounter.load(std::memory_order_relaxed))
	{
		check(Counter > 0);
		FPlatformProcess::Sleep(0.1f);
		const double CurrentTime = FPlatformTime::Seconds();
		if (CurrentTime - LastPrint > 5.0)
		{
			UE_LOGFMT(LogDerivedDataCache, Log, "Waited {WaitTime}s for Derived Data Dache to finish {TaskCount} tasks...",
				int32(CurrentTime - StartTime), Counter);
...
{
	TSharedLock Lock(Hierarchy.NodesMutex);
...
{
	Hierarchy.RecordStats(GetCacheKey(Response).Bucket, GetBatchType(), ERequestOp::Put, Response.Status, 0, MeasureLogicalSize(Request));
...
{
	const bool bCreateEmptyRecord =
		Response.Status != EStatus::Ok &&
		!EnumHasAnyFlags(Request.Policy.GetRecordPolicy(), ECachePolicy::PartialRecord) &&
		(Response.Record.GetMeta() || !Response.Record.GetValues().IsEmpty());
	FCacheRecord Record(bCreateEmptyRecord ? FCacheRecord(Response.Record.GetKey()) : ApplySkipPolicy(Response.Record, Request.Policy));
	return {Response.Name, MoveTemp(Record), Response.UserData, Response.Status};
}
...
ILegacyCacheStore* CreateCacheStoreHierarchy(
	ICacheStoreOwner*& OutOwner,
	ICacheStoreMaintainer*& OutMaintainer,
	TFunctionRef<void (IMemoryCacheStore*&)> MemoryCacheCreator)
{
	return new FCacheStoreHierarchy(OutOwner, OutMaintainer, MemoryCacheCreator);
}
