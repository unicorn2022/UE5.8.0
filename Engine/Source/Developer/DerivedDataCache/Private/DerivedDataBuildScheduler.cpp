...
void FBuildSchedulerTypeQueue::AddQueueNoLock(IBuildSchedulerTypeQueue* Queue)
{
	...
	if (TypeName.IsEmpty())
	{
		UE_LOGF(LogDerivedDataBuild, Error,
			"An empty type name is not allowed in a build scheduler type queue.");
	}
	else if (Queues.FindByHash(TypeNameHash, TypeName))
	{
		UE_LOGF(LogDerivedDataBuild, Error,
			"More than one build scheduler type queue has been registered with the type name %ls.",
...
void FBuildSchedulerMemoryQueue::AddQueueNoLock(IBuildSchedulerMemoryQueue* Queue)
{
	if (GlobalQueue)
	{
		UE_LOGF(LogDerivedDataBuild, Error, "More than one build scheduler memory queue has been registered.");
	}
	else
	{
		GlobalQueue = Queue;
