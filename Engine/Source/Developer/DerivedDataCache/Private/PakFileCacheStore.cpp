	...
	, StoreOwner(InOwner)
{
	...
	if (bWriting)
	{
		...
		if (!FileHandle)
		{
			UE_LOGF(LogDerivedDataCache, Fatal, "%ls: Failed to open pak cache for writing.", *CachePath);
			bClosed = true;
		}
		else
		{
			UE_LOGF(LogDerivedDataCache, Display, "%ls: Opened pak cache for writing.", *CachePath);
		}
	}
	else
	{
		...
		if (!FileHandle)
		{
			UE_LOGF(LogDerivedDataCache, Warning, "%ls: Failed to open pak cache for reading.", *CachePath);
		}
	...
	if (StoreOwner)
	{
		...
		StoreOwner->Add({InName}, this, Flags);
...
void FPakFileCacheStore::Close()
{
	...
	// Capture and clear the owner before calling RemoveNotSafe because RemoveNotSafe can delete
	// this cache store. The local variable ensures re-entrancy safety from the destructor.
	if (ICacheStoreOwner* Owner = StoreOwner)
	{
		StoreOwner = nullptr;
		Owner->DestroyStats(StoreStats);
		StoreStats = nullptr;
		// RemoveNotSafe can delete this cache store.
		Owner->RemoveNotSafe(this);
...
bool FPakFileCacheStore::GetCachedData(const TCHAR* CacheKey, TArray<uint8>& OutData)
{
	...
	if (FCacheValue* Item = CacheItems.Find(FString(CacheKey)))
	{
		...
		{
			UE_LOGF(LogDerivedDataCache, Warning, "%ls: Pak file, %ls exceeds 2 GiB limit.", *CachePath, CacheKey);
		}
		else if (!FileHandle->Seek(Item->Offset))
		{
			UE_LOGF(LogDerivedDataCache, Warning, "%ls: Pak file, bad seek.", *CachePath);
		}
		else
		{
			check(Item->Size);
			...
			{
				UE_LOGF(LogDerivedDataCache, Warning, "%ls: Pak file, bad read.", *CachePath);
			}
			else if (uint32 TestCrc = FCrc::MemCrc_DEPRECATED(OutData.GetData(), int32(Item->Size)); TestCrc != Item->Crc)
			{
				UE_LOGF(LogDerivedDataCache, Warning, "%ls: Pak file, bad crc.", *CachePath);
			}
			else
			{
				UE_LOGF(LogDerivedDataCache, Verbose, "%ls: Cache hit on %ls", *CachePath, CacheKey);
				check(OutData.Num());
	...
	{
		UE_LOGF(LogDerivedDataCache, Verbose, "%ls: Cache miss on %ls", *CachePath, CacheKey);
	}
	OutData.Empty();
	return false;
}
...
void FPakFileCacheStore::PutCachedData(const TCHAR* CacheKey, TArrayView<const uint8> InData, bool bPutEvenIfExists)
{
	...
	if (bWriting && !bClosed)
	{
		...
		if (Offset < 0)
		{
			CacheItems.Empty();
			FileHandle.Reset();
			UE_LOGF(LogDerivedDataCache, Fatal, "%ls: Could not write pak file... out of disk space?", *CachePath);
		}
		else
		{
			COOK_STAT(Timer.AddHit(InData.Num()));
			...
			FileHandle->Write(InData.GetData(), InData.Num());
			UE_LOGF(LogDerivedDataCache, Verbose, "%ls: Put %ls", *CachePath, CacheKey);
...
bool FPakFileCacheStore::LoadCache(const TCHAR* InFilename)
{
	...
	{
		UE_LOGF(LogDerivedDataCache, Error, "%ls: Pak cache was corrupted (short).", InFilename);
		return false;
	}
	int64 IndexOffset = -1;
	int64 Trailer = -1;
	{
		...
		if (Trailer != SeekPos)
		{
			UE_LOGF(LogDerivedDataCache, Error, "%ls: Pak cache was corrupted (bad seek).", InFilename);
			return false;
		}
		...
		{
			UE_LOGF(LogDerivedDataCache, Error, "%ls: Pak cache was corrupted (bad footer).", InFilename);
			return false;
		}
	}
	uint32 IndexCrc = 0;
	...
	uint32 SizeIndex = 0;
	{
		...
		{
			UE_LOGF(LogDerivedDataCache, Error, "%ls: Pak cache was corrupted (bad seek index).", InFilename);
			return false;
		}
		...
		{
			UE_LOGF(LogDerivedDataCache, Error, "%ls: Pak cache was corrupted (bad index header).", InFilename);
			return false;
		}
		if (IndexOffset + sizeof(uint32) * 4 + SizeIndex != Trailer) 
		{
			UE_LOGF(LogDerivedDataCache, Error, "%ls: Pak cache was corrupted (bad index size).", InFilename);
			return false;
		}
	}
	{
		...
		while (Loader.Tell() < SizeIndex)
		{
			...
			{
				UE_LOGF(LogDerivedDataCache, Error, "%ls: Pak cache was corrupted (bad index entry).", InFilename);
				return false;
			}
		...
		{
			UE_LOGF(LogDerivedDataCache, Error, "%ls: Pak cache was corrupted (bad index count).", InFilename);
			return false;
		}
	}
	return true;
...
void FPakFileCacheStore::MergeCache(IPakFileCacheStore* OtherPakInterface)
{
	...
	UE_LOGF(LogDerivedDataCache, Display, "Merging %d entries (%d skipped).", CopyKeyNames.Num(), KeyNames.Num() - CopyKeyNames.Num());
	...
	FRequestStats& Stats)
{
	...
	if (Buffer.IsNull())
	{
		UE_LOGF(LogDerivedDataCache, Verbose, "%ls: Cache miss with missing record for %ls from '%.*ls'",
			*CachePath, *WriteToString<96>(Key), Name.Len(), Name.GetData());
		return FOptionalCacheRecord();
	}
	...
	FOptionalCacheRecord Record;
	{
		...
		{
			UE_LOGF(LogDerivedDataCache, Display, "%ls: Cache miss with package load failure for %ls from '%.*ls'",
				*CachePath, *WriteToString<96>(Key), Name.Len(), Name.GetData());
			return Record;
		}
	...
	FRequestStats& Stats)
{
	...
	if (!bValueExists)
	{
		...
		if (!Value.HasData())
		{
			...
			{
				UE_LOGF(LogDerivedDataCache, Verbose, "%ls: Failed due to missing data for put of %ls from '%.*ls'",
					*CachePath, *WriteToString<96>(Key), Name.Len(), Name.GetData());
				return false;
			}
		}
	...
	FRequestStats& Stats)
{
	...
	if (Buffer.IsNull())
	{
		UE_LOGF(LogDerivedDataCache, Verbose, "%ls: Cache miss with missing value for %ls from '%.*ls'",
			*CachePath, *WriteToString<96>(Key), Name.Len(), Name.GetData());
		return false;
	}
...
FSharedBuffer FPakFileCacheStore::LoadFile(const FStringView Path, const FStringView DebugName, FRequestStats& Stats)
{
	...
	if (const FCacheValue* Item = CacheItems.FindByHash(GetTypeHash(Path), Path))
	{
		...
		if (FileHandle->Seek(Item->Offset))
		{
			...
			{
				UE_LOGF(LogDerivedDataCache, Display,
					"%ls: File %.*ls from '%.*ls' is corrupted and has CRC 0x%08x when 0x%08x is expected.",
...
{
	FString NodeType;
	FParse::Value(Config, TEXT("Type="), NodeType);
	const bool bWriting = (NodeType == TEXT("WritePak"));

	FString Filename;
	if (!FParse::Value(Config, TEXT("Filename="), Filename))
	{
		UE_LOG(LogDerivedDataCache, Log, TEXT("%s: Pak cache Filename not found, will not use a pak cache."), Name);
		return nullptr;
	}

	bool bCompressed = false;
	FParse::Bool(Config, TEXT("Compressed="), bCompressed);

	if (bWriting)
	{
		FGuid TempGuid = FGuid::NewGuid();
		FString WriteFilename = Filename + TEXT(".") + TempGuid.ToString();
		return CreatePakFileCacheStore(Name, *WriteFilename, /*bWriting*/ true, bCompressed, Owner);
	}
	else
	{
		if (!FPlatformFileManager::Get().GetPlatformFile().FileExists(*Filename))
		{
			UE_LOG(LogDerivedDataCache, Log, TEXT("%s: Pak cache file %s not found, will not use a pak cache."), Name, *Filename);
			return nullptr;
		}
		return CreatePakFileCacheStore(Name, *Filename, /*bWriting*/ false, bCompressed, Owner);
	}
}
