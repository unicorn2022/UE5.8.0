	...
	static void AddCookStats(FCookStatsManager::AddStatFuncRef AddStat)
	{
		...
		// If the DDC graph ever contains more than one local or remote filesystem, this will only find one of them.
		{
			...
			AddStat(TEXT("DDC.Summary"), FCookStatsManager::CreateKeyValueArray(
				TEXT("BackEnd"), FDerivedDataBackend::GetStatic().GetGraphName(),
...
{
	PRAGMA_DISABLE_DEPRECATION_WARNINGS;
	return {Name, Record.GetKey(), ApplySkipPolicy(Record, Policy), UserData, Status};
	PRAGMA_ENABLE_DEPRECATION_WARNINGS;
}

FCacheGetResponse FCacheGetRequest::MakeResponse(const EStatus Status) const
{
	return {Name, FCacheRecord(Key), UserData, Status};
}

FCachePutValueResponse FCachePutValueRequest::MakeResponse(const EStatus Status) const
{
	PRAGMA_DISABLE_DEPRECATION_WARNINGS;
	return {Name, Key, ApplySkipPolicy(Value, Policy), UserData, Status};
	PRAGMA_ENABLE_DEPRECATION_WARNINGS;
}
...
private:
	...
	void BeginGet()
	{
		...
		STAT(double ThisTime = 0);
		{
			...
			Request.Key = FLegacyCacheKey(CacheKey);
	...
	void EndBuild(EBuildMode BuildMode, TArray64<uint8>&& BuildData)
	{
		if (Status == EStatus::Ok && BuildMode == EBuildMode::Verify)
		{
			...
			if (!bMatchesInSize || bDifferentMemory)
			{
				...
				UE_LOGF(LogDerivedDataCache, Error, "%ls", *ErrMsg);
			}
		}
...
**/
class FDerivedDataCache final : public FDerivedDataCacheInterface, public IDDCCleanup
{
public:
	...
	{
		Backend = FDerivedDataBackend::CreateStatic();

		GVerifyDDC = FParse::Param(FCommandLine::Get(), TEXT("VerifyDDC"));

		UE_CLOGF(GVerifyDDC, LogDerivedDataCache, Display, "Items retrieved from the DDC will be verified (-VerifyDDC)");
	}
	...
	virtual bool GetSynchronous(FDerivedDataPluginInterface* DataDeriver, TArray<uint8>& OutData, bool* bDataWasBuilt) override
	{
		...
		UE_LOGF(LogDerivedDataCache, VeryVerbose, "GetSynchronous %ls from '%ls'", *CacheKey, *DataDeriver->GetDebugContextString());
	...
	virtual uint32 GetAsynchronous(FDerivedDataPluginInterface* DataDeriver) override
	{
		...
		FString CacheKey = FDerivedDataCache::BuildCacheKey(DataDeriver);
		UE_LOGF(LogDerivedDataCache, VeryVerbose, "GetAsynchronous %ls from '%ls', Handle %d", *CacheKey, *DataDeriver->GetDebugContextString(), Handle);
	...
	virtual void WaitAsynchronousCompletion(uint32 Handle) override
	{
		...
		STAT(double ThisTime = 0);
		{
			...
			check(AsyncTask);
			AsyncTask->WaitAsync();
			UE_LOGF(LogDerivedDataCache, Verbose, "WaitAsynchronousCompletion, Handle %d", Handle);
		}
	...
	bool GetAsynchronousResultsByHandle(uint32 Handle, DataType& OutData, bool* bOutDataWasBuilt)
	{
		...
		{
			UE_LOGF(LogDerivedDataCache, Verbose, "GetAsynchronousResults, bDataWasBuilt: %d, Handle %d, FAILED", (int32)bDataWasBuilt, Handle);
			return false;
		}

		UE_LOGF(LogDerivedDataCache, Verbose, "GetAsynchronousResults, bDataWasBuilt: %d, Handle %d, SUCCESS", (int32)bDataWasBuilt, Handle);
	...
	bool GetSynchronousByKey(const TCHAR* CacheKey, DataType& OutData, FStringView DebugContext)
	{
		...
		UE_LOGF(LogDerivedDataCache, VeryVerbose, "GetSynchronous %ls from '%.*ls'", CacheKey, DebugContext.Len(), DebugContext.GetData());
	...
	virtual uint32 GetAsynchronous(const TCHAR* CacheKey, FStringView DebugContext) override
	{
		...
		const uint32 Handle = NextHandle();
		UE_LOGF(LogDerivedDataCache, VeryVerbose, "GetAsynchronous %ls from '%.*ls', Handle %d", CacheKey, DebugContext.Len(), DebugContext.GetData(), Handle);
	...
	virtual void Put(const TCHAR* CacheKey, TArrayView64<const uint8> Data, FStringView DebugContext, bool bPutEvenIfExists = false) override
	{
		...
		UE_LOGF(LogDerivedDataCache, VeryVerbose, "Put %ls from '%.*ls'", CacheKey, DebugContext.Len(), DebugContext.GetData());
		STAT(double ThisTime = 0);
		{
			SCOPE_SECONDS_COUNTER(ThisTime);
			FLegacyCachePutRequest LegacyRequest;
			LegacyRequest.Name = DebugContext;
			LegacyRequest.Key = FLegacyCacheKey(CacheKey);
	...
	virtual bool TryToPrefetch(TConstArrayView<FString> CacheKeys, FStringView DebugContext) override
	{
		if (!CacheKeys.IsEmpty())
		{
			...
			UE_LOGF(LogDerivedDataCache, VeryVerbose, "TryToPrefetch %d keys including %ls from '%.*ls'",
	...
	{
		return Backend->GetMaintainer().IsIdle();
	}
	...
	virtual void WaitBetweenDeletes(bool bWait) override
	{
		if (!bWait)
		{
			Backend->GetMaintainer().BoostPriority();
		}
	}
	...
	void GatherVerificationStats(FDerivedDataCacheVerificationStats& OutStats) const override
	{
		Backend->GatherVerificationStats(OutStats);
	}
