...
public:
	 ...
	 *
	 * - Config = "-DDC-All-MissTypes=StaticMesh@50 -DDC-All-MissKeys=Texture/0123456789abcdef0123456789abcdef01234567"
	 * - BucketPrefix = "-DDC-All-MissTypes="
	 * - KeyPrefix = "-DDC-All-MissKeys="
	 * - Filter matches StaticMesh at 50%, the single Texture key specified, and everything else at a rate of 0%.
	 *
	 * Multiple buckets or keys in one argument are delimited by '+' or ','.
	 *
	 * @param Config         The configuration in the form of command line parameters.
	 * @param BucketPrefix   The prefix of the parameter to parse the bucket filter from. Disabled if null.
	 * @param KeyPrefix      The prefix of the parameter to parse the key filter from. Disabled if null.
	 * @param DefaultRate    The default rate at which to apply the filter. [0, 100]
	 */
	UE_API static FCacheKeyFilter Parse(const TCHAR* Config, const TCHAR* BucketPrefix, const TCHAR* KeyPrefix, float DefaultRate = 0.0f);
	...
	UE_API bool RequiresSalt() const;
