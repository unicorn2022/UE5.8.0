...
TSharedPtr<ICollectionContainer> FCollectionManager::AddCollectionContainer(const TSharedRef<ICollectionSource>& CollectionSource)
{
	...
	for (const TSharedPtr<FCollectionContainer>& CollectionContainer : CollectionContainers)
	{
		...
		{
			UE_LOGF(LogCollectionManager, Warning, "Collection source '%ls' already added.", *CollectionSource->GetName().ToString());
			return nullptr;
		}
		...
		{
			UE_LOGF(LogCollectionManager, Warning, "Collection source shares the same name with existing collection container '%ls'.", *CollectionSource->GetName().ToString());
			return nullptr;
		}
	}
