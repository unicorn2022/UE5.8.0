...
bool FCollection::Save(const TArray<FText>& AdditionalChangelistText, FText& OutError, bool bForceCommitToRevisionControl)
{
	...
	if ( bUseSCC )
	{
		...
		{
			UE_LOGF(LogCollectionManager, Error, "Failed to check out a collection file: %ls", *CollectionName.ToString());
			GWarn->EndSlowTask();
			return false;
		}
	}
	...
	if ( ensure(FileOutput.Len()) )
	{
		...
		else
		{
			...
			UE_LOGF(LogCollectionManager, Error, "%ls", *OutError.ToString());
		}
	}
	else
	{
	...
	if ( bSaveSuccessful )
	{
		if ( bUseSCC && (bForceCommitToRevisionControl || GetDefault<UCollectionSettings>()->bAutoCommitOnSave))
		{
			...
			if ( bSaveSuccessful )
			{
				...
				{
					UE_LOGF(LogCollectionManager, Error, "Failed to check in a collection successfully saving: %ls", *CollectionName.ToString());
			...
			if ( !bSaveSuccessful )
			{
				...
				if ( !RevertCollection(Unused) )
				{
					...
					UE_LOGF(LogCollectionManager, Warning, "Failed to revert a checked out collection after failing to save or checkin: %ls", *CollectionName.ToString());
				}
			}
		}
	}
	...
	UE_LOGF(LogCollectionManager, Verbose, "Saved collection %ls in %0.6f seconds", *CollectionName.ToString(), FPlatformTime::Seconds() - SaveStartTime);
...
void FCollection::PrintCollection() const
{
	if (StorageMode == ECollectionStorageMode::Static)
	{
		UE_LOGF(LogCollectionManager, Log, "    Printing static elements of collection %ls", *CollectionName.ToString());
		UE_LOGF(LogCollectionManager, Log, "    =============================");
		...
		for (const FSoftObjectPath& ObjectName : ObjectList)
		{
			UE_LOGF(LogCollectionManager, Log, "        %ls", *ObjectName.ToString());
		}
	}
	else
	{
		UE_LOGF(LogCollectionManager, Log, "    Printing dynamic query of collection %ls", *CollectionName.ToString());
		UE_LOGF(LogCollectionManager, Log, "    =============================");
		UE_LOGF(LogCollectionManager, Log, "        %ls", *DynamicQueryText);
	}
}
...
void FCollection::SaveHeaderPairs(TMap<FString,FString>& OutHeaderPairs) const
{
	...
	OutHeaderPairs.Add(TEXT("IsAutoGenerated"), bIsAutoGenerated ? TEXT("true") : TEXT("false"));
	if (CollectionColor)
	{
...
bool FCollection::LoadHeaderPairs(const TMap<FString,FString>& InHeaderPairs)
{
	...
	if (const FString* IsAutoGeneratedString = InHeaderPairs.Find(TEXT("IsAutoGenerated")))
	{
		bIsAutoGenerated = IsAutoGeneratedString->ToBool();
	}
		...
		else if(SourceControlState->CanCheckout())
		{
			if ( SourceControlProvider.Execute(ISourceControlOperation::Create<FDelete>(), AbsoluteFilename) == ECommandResult::Succeeded )
			{
				...
				else
				{
					...
					if ( !RevertCollection(Unused) )
					{
						UE_LOGF(LogCollectionManager, Warning, "Failed to revert collection '%ls' after failing to check in the file that was marked for delete.", *CollectionName.ToString());
					}
