...
public:
	...
	void Create(const TArray<FString>& Args)
	{
		if ( Args.Num() < 2 )
		{
			UE_LOGF(LogCollectionManager, Log, "Usage: CollectionManager.Create [CollectionContainer] CollectionName CollectionType [CollectionStorageMode]");
			return;
		}
		...
		if (Args.Num() >= 3)
		{
			...
			{
				UE_LOGF(LogCollectionManager, Warning, "Invalid collection storage mode: %ls", *StorageModeStr);
				return;
			}
		}
		...
		if (Args.Num() >= 4 || (Args.Num() == 3 && StorageMode == InvalidStorageMode))
		{
			...
			{
				UE_LOGF(LogCollectionManager, Warning, "Invalid collection container: %ls", *CollectionSourceName);
				return;
			}
		}
		else
		...
		{
			UE_LOGF(LogCollectionManager, Warning, "Invalid collection share type: %ls", *ShareStr);
			return;
		}
		...
		{
			UE_LOGF(LogCollectionManager, Log, "Collection created: %ls", *CollectionName.ToString());
		}
		else
		{
			UE_LOGF(LogCollectionManager, Warning, "Failed to create collection: %ls", *CollectionName.ToString());
		}
	}
	...
	void Destroy(const TArray<FString>& Args)
	{
		if ( Args.Num() < 2 )
		{
			UE_LOGF(LogCollectionManager, Log, "Usage: CollectionManager.Destroy [CollectionContainer] CollectionName CollectionType");
			return;
		}
		...
		{
			UE_LOGF(LogCollectionManager, Log, "Collection destroyed: %ls", *CollectionName.ToString());
		}
		else
		{
			UE_LOGF(LogCollectionManager, Warning, "Failed to destroyed collection: %ls", *CollectionName.ToString());
		}
	}
	...
	void Add(const TArray<FString>& Args)
	{
		if ( Args.Num() < 3 )
		{
			UE_LOGF(LogCollectionManager, Log, "Usage: CollectionManager.Add [CollectionContainer] CollectionName CollectionType ObjectPath");
			return;
		}
		...
		{
			UE_LOGF(LogCollectionManager, Log, "%ls added to collection %ls", *ObjectPath.ToString(), *CollectionName.ToString());
		}
		else
		{
			UE_LOGF(LogCollectionManager, Warning, "Failed to add %ls to collection %ls", *ObjectPath.ToString(), *CollectionName.ToString());
		}
	}
	...
	void Remove(const TArray<FString>& Args)
	{
		if ( Args.Num() < 3 )
		{
			UE_LOGF(LogCollectionManager, Log, "Usage: CollectionManager.Remove [CollectionContainer] CollectionName CollectionType ObjectPath");
			return;
		}
		...
		{
			UE_LOGF(LogCollectionManager, Log, "%ls removed from collection %ls", *ObjectPath.ToString(), *CollectionName.ToString());
		}
		else
		{
			UE_LOGF(LogCollectionManager, Warning, "Failed to remove %ls from collection %ls", *ObjectPath.ToString(), *CollectionName.ToString());
		}
	}
};
