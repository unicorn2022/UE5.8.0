...
public:
	...
	static void EnumerateEventsDownSampled(FAsyncEnumerateTaskData<InEventType, SettingsType>& Data, InitialStackCallback InitCallback, typename ITimeline<InEventType>::EventCallback Callback)
	{
		...
		if ((Data.SortOrder == EEventSortOrder::ByEndTime) ||
			(Data.TaskIndex == 0 && Data.SortOrder == EEventSortOrder::ByStartTime))
		{
			InitCallback(0);
...
private:
	...
	{
		if (Data.SortOrder == EEventSortOrder::ByStartTime &&
			Data.TaskIndex > 0 &&
			CurrentStackDepth > 0 &&
			CurrentStackDepth <= InitialPage->InitialStackCount)
		{
...
public:
	...
	void EnumerateOrderedByStartTime()
	{
		...
		EnumerateAsyncAlgoritm<InEventType, SettingsType>::EnumerateEventsDownSampled(Data,
			[&CurrentDepth, &MinimumDepth]
			(int32 InInitialDepth)
			{
				MinimumDepth = InInitialDepth;
				CurrentDepth = InInitialDepth;
			},
			[this, &EventStack, &OutputEvents, &CurrentDepth, &MinimumDepth]
				...
				else
				{
					...
					if (CurrentDepth <= MinimumDepth)
					{
						...
						for (FOutputEvent& OutputEvent : OutputEvents)
						{
							...
							{
								if (Data.EventRangeCallback(OutputEvent.StartTime, OutputEvent.EndTime, OutputEvent.Depth, OutputEvent.Event, Data.TaskIndex) == EEventEnumerate::Stop)
								{
		...
		EnumerateAsyncAlgoritm<InEventType, SettingsType>::EnumerateEventsDownSampled(Data,
			...
			(bool IsEnter, double Time, const EventType& Event)
			{
				...
				else
				{
					...
					{
						if (Data.EventRangeCallback(StackEntry.StartTime, Time, CurrentDepth, Event, Data.TaskIndex) == EEventEnumerate::Stop)
						{
							return EEventEnumerate::Stop;
						}
					}
