...
void FAllocationsQuery::Run()
{
	...
	UE_LOGF(LogTraceServices, Log, "[MemAlloc] Processing %d live allocs...", AllocationsProvider.GetNumLiveAllocs());
	...
	if (NumLiveAllocs != 0)
	{
		UE_LOGF(LogTraceServices, Log, "[MemAlloc] Enqueue %u live allocs...", NumLiveAllocs);
	...
	UE_LOGF(LogTraceServices, Log, "[MemAlloc] Detecting cells...");
	...
	UE_LOGF(LogTraceServices, Log, "[MemAlloc] %d cells to process", Cells.Num());
	uint32 CellIndex = 0;
	...
	for (const FSbTreeCell* Cell : Cells)
	{
		...
		++CellIndex;
		UE_LOGF(LogTraceServices, Log, "[MemAlloc] Processing cell %u (%u allocs)...", CellIndex, Cell->GetAllocCount());
		...
		if (NumAllocs != 0)
		{
			UE_LOGF(LogTraceServices, Log, "[MemAlloc] Enqueue %u allocs...", NumAllocs);
	...
	UE_LOGF(LogTraceServices, Log, "[MemAlloc] Allocations query completed in %.3fs (%u cells, %u allocations)",
