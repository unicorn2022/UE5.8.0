...
void FRegionProvider::AppendRegionBegin(const TCHAR* Name, double Time, const TCHAR* CategoryName)
{
	...
	if (FoundOpenRegions)
	{
		...
		{
			UE_LOGF(LogTraceServices, Warning, "[Regions] A region begin event (BeginTime=%f, Name=\"%ls\", Category=\"%ls\") was encountered while a region with same name is already open.",
...
void FRegionProvider::AppendRegionBeginWithId(const TCHAR* Name, uint64 Id, double Time, const TCHAR* CategoryName)
{
	...
	if (FoundOpenRegions)
	{
		...
		if (NumWarnings <= MaxWarningMessages)
		{
			UE_LOGF(LogTraceServices, Warning, "[Regions] A region begin event (BeginTime=%f, Id=%llu, Name=\"%ls\", Category=\"%ls\") was encountered while a region with same name is already open.",
...
void FRegionProvider::AppendRegionEnd(const TCHAR* Name, double Time)
{
	...
	if (!OpenRegionsPos)
	{
		...
		if (NumWarnings <= MaxWarningMessages)
		{
			UE_LOGF(LogTraceServices, Warning, "[Regions] A region end event (EndTime=%f, Name=\"%ls\") was encountered without having seen a matching region begin event first.", Time, Name)
		}
...
void FRegionProvider::AppendRegionEndWithId(uint64 Id, double Time)
{
	...
	if (!Id)
	{
		...
		if (NumWarnings <= MaxWarningMessages)
		{
			UE_LOGF(LogTraceServices, Warning, "[Regions] A region end event with id 0 was encountered, ignoring (EndTime=%f.", Time)
		}
		return;
	}
	...
	if (!OpenRegionsPos)
	{
		...
		if (NumWarnings <= MaxWarningMessages)
		{
			UE_LOGF(LogTraceServices, Warning, "[Regions] A region end event (EndTime=%f, Id=%llu) was encountered without having seen a matching region begin event first.", Time, Id)
		}
...
void FRegionProvider::OnAnalysisSessionEnded()
{
	...
	auto PrintOpenRegionMessage = [this](const auto& KV)
	{
		...
		if (NumWarnings <= MaxWarningMessages)
		{
			...
			UE_LOGF(LogTraceServices, Warning, "[Regions] A region (BeginTime=%f, Id=%llu, Name=\"%ls\", Category=\"%ls\") was never closed.",
				Region.BeginTime,
				Region.Id,
				Region.Timer->Name,
	...
	if (NumWarnings > 0)
	{
		UE_LOGF(LogTraceServices, Warning, "[Regions] %u warnings", NumWarnings);
	}
	if (NumErrors > 0)
	{
		UE_LOGF(LogTraceServices, Error, "[Regions] %u errors", NumErrors);
	}

	const uint64 TotalRegionCount = GetRegionCount();
	UE_LOGF(LogTraceServices, Log, "[Regions] Analysis completed (%llu timers, %llu categories, %llu timing regions, %d lanes).",
