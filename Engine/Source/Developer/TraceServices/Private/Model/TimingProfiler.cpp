...
#define TIMING_PROFILER_LOG_INVALID_METADATA_1 //{ UE_LOGF(LogTraceServices, Warning, "[TimingProfiler][1] MetadataTimerId=%u >= 0", MetadataTimerId); UE_DEBUG_BREAK(); }
#define TIMING_PROFILER_LOG_INVALID_METADATA_2 //{ UE_LOGF(LogTraceServices, Warning, "[TimingProfiler][2] MetadataTimerId=%u >= Num=%u", MetadataTimerId, uint32(Metadatas.Num())); UE_DEBUG_BREAK(); }
#define TIMING_PROFILER_LOG_INVALID_METADATA_SPEC //{ UE_LOGF(LogTraceServices, Warning, "[TimingProfiler][3] MetadataSpecId=%u >= Num=%u", MetadataSpecId, uint32(MetadataSpecs.Num())); UE_DEBUG_BREAK(); }
...
void FTimingProfilerProvider::SetTimerName(uint32 TimerId, FStringView Name)
{
	...
	if (ensure(TimerId < uint32(Timers.Num())))
	{
		...
		{
			UE_LOGF(LogTraceServices, Warning, "[TimingProfiler] Name changed for timer %u: from \"%ls\" to \"%ls\" !!!",
...
void FTimingProfilerProvider::SetTimerNameAndLocation(uint32 TimerId, FStringView Name, const TCHAR* File, uint32 Line)
{
	...
	if (ensure(TimerId < uint32(Timers.Num())))
	{
		...
		{
			UE_LOGF(LogTraceServices, Warning, "[TimingProfiler] Location changed for timer %u (%ls): from \"%ls\" (%u) to \"%ls\" (%u) !!!",
...
void FTimingProfilerProvider::SetMetadataSpec(uint32 TimerId, uint32 MetadataSpecId)
{
	...
	if (ensure(TimerId < uint32(Timers.Num())))
	{
		...
		{
			UE_LOGF(LogTraceServices, Warning, "Metadata changed for timer %u (%ls): from \"%ls\" (%d fields) to \"%ls\" (%d fields) !!!",
				TimerId, Timer.Name,
...
void FTimingProfilerProvider::MakeTimerDisplayName(const FTimingProfilerTimer& InTimer, TStringBuilder<1024>& InOutDisplayName) const
{
	Session.ReadAccessCheck();

	if (InTimer.Name)
	{
		InOutDisplayName.Append(InTimer.Name);
	}

	if (InTimer.HasValidMetadataSpecId())
	{
		const TraceServices::FMetadataSpec* Spec = GetMetadataSpec(InTimer.MetadataSpecId);
		if (Spec && Spec->Format)
		{
			TStringView Format = Spec->Format;
			Format.TrimStartInline();
			if (!Format.StartsWith(TEXT("- ")) && !Format.StartsWith(TEXT("= ")))
			{
				InOutDisplayName.Append(TEXT(" - "));
			}
			InOutDisplayName.Append(Format);
		}
	}
}
...
ITable<FTimingProfilerAggregatedStats>* FTimingProfilerProvider::CreateAggregationInternal(TArray<const ITimingProfilerTimeline*>& IncludedTimelines, const FCreateAggregationParams& Params) const
{
	...
	FTimelineStatistics::TAggregationResult<const FTimingProfilerTimer*, FAggregatedTimingStats> Aggregation;
	...
	if (Params.SortBy == FCreateAggregationParams::ESortBy::TotalInclusiveTime)
	{
		...
		Aggregation.GetMap().ValueStableSort(SortLambda);
	}

	int EntryLimit = Params.TableEntryLimit;
	for (const auto& KV : Aggregation.GetMap())
	{
