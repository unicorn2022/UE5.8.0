...
#define INSIGHTS_LOGF(Format, ...) { UE_LOGF(LogTraceServices, Log, "[MemAlloc]" Format, __VA_ARGS__); }
//#define INSIGHTS_LOGF(Format, ...) { FPlatformMisc::LowLevelOutputDebugStringf(TEXT("[MemAlloc]") TEXT(Format) TEXT("\n"), __VA_ARGS__); }

#if 0 || INSIGHTS_DEBUG_WATCH
#define INSIGHTS_API_LOGF(ThreadId, Time, Address, Format, ...) \
	INSIGHTS_LOGF("[API][%3u][%f] " Format, ThreadId, Time, __VA_ARGS__);
#else
#define INSIGHTS_API_LOGF(ThreadId, Time, Address, Format, ...)
#endif
...
#define INSIGHTS_INDIRECT_API_LOGF(ApiName, ThreadId, Time, Address) \
	INSIGHTS_LOGF("[API] --> " ApiName " 0x%llX", Address);
#else
#define INSIGHTS_INDIRECT_API_LOGF(ApiName, ThreadId, Time, Address)
#endif
...
void FTagTracker::AddTagSpec(TagIdType InTag, TagIdType InParentTag, const TCHAR* InDisplay)
{
	if (InTag == InvalidTagId)
	{
		...
		{
			UE_LOGF(LogTraceServices, Error, "[MemAlloc] Cannot add Tag spec ('%ls') with invalid id!", InDisplay);
		}
		return;
	}
	...
	if (!InDisplay || *InDisplay == TEXT('\0'))
	{
		...
		if (NumErrors <= MaxLogMessagesPerErrorType)
		{
			UE_LOGF(LogTraceServices, Error, "[MemAlloc] Tag with id %u has invalid display name (ParentTag=%u)!", InTag, InParentTag);
		}
	...
	if (!TagEntry)
	{
		...
		UE_LOGF(LogTraceServices, Verbose, "[MemAlloc] Added Tag '%ls' ('%ls') with id %u (ParentTag=%u).", Entry.Display, Entry.FullPath, InTag, InParentTag);
	}
	else
	{
		FStringView DisplayName(InDisplay);
		...
		if (InParentTag == TagEntry->ParentTag && DisplayName.Equals(TagEntry->Display))
		{
			...
			{
				UE_LOGF(LogTraceServices, Warning, "[MemAlloc] Tag with id %u (ParentTag=%u, Display='%ls') was already added!", InTag, InParentTag, InDisplay);
			}
		}
		else
		{
			...
			if (NumErrors <= MaxLogMessagesPerErrorType)
			{
				UE_LOGF(LogTraceServices, Error, "[MemAlloc] Tag with id %u (ParentTag=%u, Display='%ls') was already added (ParentTag=%u, Display='%ls')!", InTag, InParentTag, InDisplay, TagEntry->ParentTag, TagEntry->Display);
			}
		}
	}
}
...
void FTagTracker::PushTag(uint32 InThreadId, uint8 InTracker, TagIdType InTag)
{
	...
	if (InTag == InvalidTagId)
	{
		...
		if (NumErrors <= MaxLogMessagesPerErrorType)
		{
			UE_LOGF(LogTraceServices, Error, "[MemAlloc] Invalid Tag on Thread %u (Tracker=%u)!", InThreadId, uint32(InTracker));
		}
	}
...
void FTagTracker::PopTag(uint32 InThreadId, uint8 InTracker)
{
	...
	else
	{
		...
		if (NumErrors <= MaxLogMessagesPerErrorType)
		{
			UE_LOGF(LogTraceServices, Error, "[MemAlloc] Tag stack on Thread %u (Tracker=%u) is already empty!", InThreadId, uint32(InTracker));
		}
	}
}
...
void FAllocationsProvider::EditInit(double InTime, uint8 InMinAlignment, uint64 InPlatformPageSize)
{
	...
	INSIGHTS_API_LOGF(0u, InTime, 0ull, "Init : MinAlignment=%u)", uint32(InMinAlignment));
...
void FAllocationsProvider::AllocInternal(uint32 ThreadId, double Time, uint32 CallstackId, uint64 Address, uint64 Size, uint32 Alignment, HeapId RootHeapId)
{
	INSIGHTS_WATCH_API_LOGF(ThreadId, Time, Address, "Alloc 0x%llX : Size=%llu Alignment=%u RootHeap=%u CallstackId=%u", Address, Size, Alignment, RootHeapId, CallstackId);
	...
	if (Address == 0 && RootHeapId == EMemoryTraceRootHeap::SystemMemory)
	{
		...
		{
			UE_LOGF(LogTraceServices, Warning, "[MemAlloc] Alloc at address 0 : Size=%llu, RootHeap=%u, Time=%f, CallstackId=%u", Size, RootHeapId, Time, CallstackId);
		}
		return;
	}
	...
	if (!IsValidRootHeap(RootHeapId))
	{
		...
		if (AllocWarnings <= MaxLogMessagesPerWarningType)
		{
			UE_LOGF(LogTraceServices, Warning, "[MemAlloc] Alloc with invalid root heap id (%u).", RootHeapId);
		}
		return;
	}
	...
	if (ExistingAllocationPtr)
	{
		...
		{
			UE_LOGF(LogTraceServices, Error, "[MemAlloc] Invalid ALLOC event (Address=0x%llX, Size=%llu, Tag=%u, RootHeap=%u, Time=%f, CallstackId=%u)! The previous alloc will be freed.", Address, Size, Tag, RootHeapId, Time, CallstackId);
		}

		// Free the previous allocation.
		INSIGHTS_WATCH_INDIRECT_API_LOGF("Free", ThreadId, Time, Address);
		...
		if (AllocErrors <= MaxLogMessagesPerErrorType)
		{
			UE_LOGF(LogTraceServices, Error, "[MemAlloc] Invalid ALLOC event (Address=0x%llX, Size=%llu, Tag=%u, RootHeap=%u, Time=%f, CallstackId=%u)!", Address, Size, Tag, RootHeapId, Time, CallstackId);
		}
	...
	FAllocationItem* Alloc = RootHeap.LiveAllocs->AddNew(Address);
	FAllocationItem& Allocation = *Alloc;
	...
	Allocation.SetAllocThreadId(ThreadId);
	...
	Allocation.Tag = Tag;
	Allocation.SetHeapId(RootHeapId);
	Allocation.SetFlags(EMemoryTraceHeapAllocationFlags::None);

	RootHeap.UpdateHistogramByAllocSize(Size);

	// Update the timeline stats (Alloc added to LiveAllocs as a new normal "alloc").
	...
	{
		UE_LOGF(LogTraceServices, Error, "[MemAlloc] Too many events!");
...
void FAllocationsProvider::FreeInternal(uint32 ThreadId, double Time, uint32 CallstackId, uint64 Address, HeapId RootHeapId)
{
	INSIGHTS_WATCH_API_LOGF(ThreadId, Time, Address, "Free 0x%llX : RootHeap=%u CallstackId=%u", Address, RootHeapId, CallstackId);
	INSIGHTS_FILTER_EVENT(ThreadId, Time, Address, RootHeapId, CallstackId);
	...
	if (Address == 0 && RootHeapId == EMemoryTraceRootHeap::SystemMemory)
	{
		...
		{
			UE_LOGF(LogTraceServices, Warning, "[MemAlloc] Free for address 0 : RootHeap=%u, Time=%f, CallstackId=%u", RootHeapId, Time, CallstackId);
		}
#endif // INSIGHTS_WARNINGS_FOR_NULLPTR_ALLOCS
		return;
	}
	...
	if (!IsValidRootHeap(RootHeapId))
	{
		...
		if (FreeWarnings <= MaxLogMessagesPerWarningType)
		{
			UE_LOGF(LogTraceServices, Warning, "[MemAlloc] Free with invalid root heap id (%u).", RootHeapId);
		}
		return;
	}
	...
	if (!Alloc)
	{
		...
		Alloc = RootHeap.LiveAllocs->FindHeapRef(Address);
		if (Alloc)
		{
			INSIGHTS_WATCH_INDIRECT_API_LOGF("UnmarkAllocationAsHeap", ThreadId, Time, Address);
			EditUnmarkAllocationAsHeap(ThreadId, Time, CallstackId, Address, Alloc->Heap);
			RootHeap.SbTree->SetTimeForEvent(RootHeap.EventIndex, Time); // for the case where the next event is first event in a new SbTree column after changing the heap alloc
	...
	if (!Alloc)
	{
		...
		{
			UE_LOGF(LogTraceServices, Error, "[MemAlloc] Invalid FREE event (Address=0x%llX, RootHeap=%u, Time=%f, CallstackId=%u)! A fake alloc will be created with size 0.", Address, RootHeapId, Time, CallstackId);
		}

		// Fake the missing alloc.
		constexpr uint64 FakeAllocSize = 0;
		constexpr uint32 FakeAllocAlignment = 0;
		INSIGHTS_WATCH_INDIRECT_API_LOGF("Alloc", ThreadId, Time, Address);
	...
	if (Alloc)
	{
		...
		Alloc->SetEndEventIndex(RootHeap.EventIndex);
		Alloc->SetEndTime(Time);
		Alloc->SetFreeThreadId(ThreadId);
		Alloc->FreeCallstackId = CallstackId;

		const uint64 AllocSize = Alloc->GetSize();
		const uint32 EventDistance = Alloc->GetEventDistance();

		RootHeap.SbTree->AddAlloc(Alloc); // SbTree takes ownership of Alloc
		Alloc = nullptr;

		RootHeap.UpdateHistogramByEventDistance(EventDistance);

		// Update stats for the current timeline sample (Alloc removed from LiveAllocs).
		TotalAllocatedMemory -= AllocSize;
		--TotalLiveAllocations;
	...
	else
	{
		...
		if (FreeErrors <= MaxLogMessagesPerErrorType)
		{
			UE_LOGF(LogTraceServices, Error, "[MemAlloc] Invalid FREE event (Address=0x%llX, RootHeap=%u, Time=%f, CallstackId=%u)!", Address, RootHeapId, Time, CallstackId);
		}
	}
...
void FAllocationsProvider::EditUpdateAlloc(uint32 ThreadId, double Time, uint32 CallstackId, uint64 Address, HeapId RootHeapId)
{
	...
	INSIGHTS_WATCH_API_LOGF(ThreadId, Time, Address, "UpdateAlloc 0x%llX : RootHeap=%u CallstackId=%u", Address, RootHeapId, CallstackId);
	INSIGHTS_FILTER_EVENT(ThreadId, Time, Address, RootHeapId, CallstackId);

	if (!IsValidRootHeap(RootHeapId))
	{
		++MiscWarnings;
		if (MiscWarnings <= MaxLogMessagesPerWarningType)
		{
			UE_LOGF(LogTraceServices, Warning, "[MemAlloc] Update alloc with invalid root heap id (%u).", RootHeapId);
		}
		return;
	}
	...
	// Try to update a regular alloc.
	FAllocationItem* Alloc = RootHeap.LiveAllocs->FindRef(Address); // alloc owned by LiveAllocs
	if (Alloc)
	{
		const uint32 AllocCallstackId = Alloc->AllocCallstackId;
		const uint64 AllocSize = Alloc->GetSize();
		const uint32 AllocAlignment = Alloc->GetAlignment();
		FreeInternal(ThreadId, Time, CallstackId, Address, RootHeapId);
		AllocInternal(ThreadId, Time, AllocCallstackId, Address, AllocSize, AllocAlignment, RootHeapId);
		return;
	}

	// Try to update a "heap" alloc.
	Alloc = RootHeap.LiveAllocs->FindHeapRef(Address); // alloc owned by LiveAllocs
	if (Alloc)
	{
		const uint32 AllocCallstackId = Alloc->AllocCallstackId;
		const uint64 AllocSize = Alloc->GetSize();
		const uint32 AllocAlignment = Alloc->GetAlignment();
		const HeapId AllocHeapId = Alloc->GetHeapId();
		const EMemoryTraceHeapAllocationFlags AllocHeapFlags = Alloc->GetFlags();
		FreeInternal(ThreadId, Time, CallstackId, Address, RootHeapId);
		AllocInternal(ThreadId, Time, AllocCallstackId, Address, AllocSize, AllocAlignment, RootHeapId);
		EditMarkAllocationAsHeap(ThreadId, Time, AllocCallstackId, Address, AllocHeapId, AllocHeapFlags);
		return;
	}
	...
	{
		UE_LOGF(LogTraceServices, Error, "[MemAlloc] UpdateAlloc: Address 0x%llX not found (RootHeap=%u, Time=%f, CallstackId=%u)!", Address, RootHeapId, Time, CallstackId);
	}
}
...
void FAllocationsProvider::EditSwapOp(uint32 ThreadId, double Time, uint64 UnmaskedPageAddress, EMemoryTraceSwapOperation SwapOp, uint64 CompressedPageSize, uint32 CallstackId)
{
	...
	// Emit fake "page in" if there was a previous "page out" with the same address.
	if (SwapOp == EMemoryTraceSwapOperation::PageOut &&
		RootHeap.LiveAllocs->FindSwapRef(MaskedPagedAddress) != nullptr)
	{
		++SwapErrors;
		if (SwapErrors <= MaxLogMessagesPerErrorType)
		{
			UE_LOGF(LogTraceServices, Error, "[MemAlloc] EditSwapOp: Trying to swap out page 0x%llX that is currently already in swap (Time=%f, CallstackId=%u). A fake swap page in will be created.", MaskedPagedAddress, Time, CallstackId);
		}
	...
	if (MaskedPagedAddress != 0)
	{
		...
		case EMemoryTraceSwapOperation::PageOut:
		{
			...
			FAllocationItem* Alloc = RootHeap.LiveAllocs->AddNewSwap(MaskedPagedAddress);
			FAllocationItem& Allocation = *Alloc;

			INSIGHTS_SLOW_CHECK(Allocation.Address == Address);
			Allocation.SetSize(CompressedPageSize);
			Allocation.SetAlignment(0);
			Allocation.StartEventIndex = RootHeap.EventIndex;
			Allocation.EndEventIndex = (uint32)-1;
			Allocation.StartTime = Time;
			Allocation.EndTime = std::numeric_limits<double>::infinity();
			...
			Allocation.Tag = 0;
			Allocation.SetHeapId(RootHeapId);
			Allocation.SetFlags(EMemoryTraceHeapAllocationFlags::Swap);

			RootHeap.UpdateHistogramByAllocSize(CompressedPageSize);
			break;
		}
		case EMemoryTraceSwapOperation::PageIn:
		case EMemoryTraceSwapOperation::FreeInSwap:
		{
			...
			FAllocationItem* Alloc = RootHeap.LiveAllocs->RemoveSwap(MaskedPagedAddress); // we take ownership of Alloc
			...
			else
			{
				...
				if (SwapErrors <= MaxLogMessagesPerErrorType)
				{
					UE_LOGF(LogTraceServices, Error, "[MemAlloc] EditSwapOp: Trying to page in swap page 0x%llX that is currently not in swap (Time=%f, CallstackId=%u). Ignoring page in. Swap total compressed memory value will be invalid.", MaskedPagedAddress, Time, CallstackId);
				}
			}
...
void FAllocationsProvider::EditHeapSpec(HeapId Id, HeapId ParentId, const FStringView& Name, EMemoryTraceHeapFlags Flags)
{
	...
	INSIGHTS_API_LOGF(0u, 0.0, 0ull, "HeapSpec : Id=%u ParentId=%u Name=\"%.*ls\" Flags=0x%X", Id, ParentId, Name.Len(), Name.GetData(), uint32(Flags));
...
void FAllocationsProvider::AddHeapSpec(HeapId Id, HeapId ParentId, const FStringView& Name, EMemoryTraceHeapFlags Flags)
{
	...
	if (Id < MaxRootHeaps)
	{
		...
		if (RootHeapPtr != nullptr)
		{
			...
			if (Id != 0)
			{
				...
				{
					UE_LOGF(LogTraceServices, Warning, "[MemAlloc] Root heap %u has changed from (\"%ls\", flags=%u) to (\"%ls\", flags=%u).",
						Id,
						HeapSpec->Name,
						int(HeapSpec->Flags),
						HeapName,
	...
	else
	{
		...
		if (ParentHeapSpec.Name == GDefaultHeapName)
		{
			...
			if (HeapWarnings <= MaxLogMessagesPerWarningType)
			{
				UE_LOGF(LogTraceServices, Warning, "[MemAlloc] Heap %u (\"%ls\") is used before its parent heap %u was announced.", Id, HeapName, ParentId);
			}
		}
		...
		if (IsValidHeap(Id))
		{
			...
			if (PreviousHeap.Name != GDefaultHeapName)
			{
				...
				if (HeapWarnings <= MaxLogMessagesPerWarningType)
				{
					UE_LOGF(LogTraceServices, Warning, "[MemAlloc] Heap %u has changed from (\"%ls\", parent=\"%ls\", flags=%u) to (\"%ls\", parent=\"%ls\", flags=%u).",
						Id,
						PreviousHeap.Name,
...
void FAllocationsProvider::EditMarkAllocationAsHeap(uint32 ThreadId, double Time, uint32 CallstackId, uint64 Address, HeapId Heap, EMemoryTraceHeapAllocationFlags Flags)
{
	...
	INSIGHTS_WATCH_API_LOGF(ThreadId, Time, Address, "MarkAllocAsHeap 0x%llX : Heap=%u Flags=0x%X CallstackId=%u", Address, Heap, uint32(Flags), CallstackId);
	...
	if (!IsValidHeap(Heap))
	{
		...
		{
			UE_LOGF(LogTraceServices, Error, "[MemAlloc] HeapMarkAlloc: Heap %u is not valid (Address=0x%llX, Time=%f, CallstackId=%u)!", Heap, Address, Time, CallstackId);
		}
		return;
	}
	...
	if (Heap == RootHeap.HeapSpec->Id)
	{
		...
		if (HeapWarnings <= MaxLogMessagesPerWarningType)
		{
			UE_LOGF(LogTraceServices, Warning, "[MemAlloc] HeapMarkAlloc: Heap %u is a root heap (Address=0x%llX, Time=%f, CallstackId=%u)!", Heap, Address, Time, CallstackId);
		}
	}
#endif
	...
	if (!Alloc)
	{
		...
		if (HeapErrors <= MaxLogMessagesPerErrorType)
		{
			UE_LOGF(LogTraceServices, Error, "[MemAlloc] HeapMarkAlloc: Could not find alloc with address 0x%llX (Heap=%u \"%ls\", Flags=%u, Time=%f, CallstackId=%u)!", Address, Heap, HeapSpec.Name, uint32(Flags), Time, CallstackId);
			UE_LOGF(LogTraceServices, Error, "[MemAlloc] HeapMarkAlloc: A fake alloc will be created with size 0.");
		}
	...
	if (Alloc)
	{
		...
		// Update the timeline stats (Alloc removed from LiveAllocs).
...
void FAllocationsProvider::EditUnmarkAllocationAsHeap(uint32 ThreadId, double Time, uint32 CallstackId, uint64 Address, HeapId Heap)
{
	...
	INSIGHTS_WATCH_API_LOGF(ThreadId, Time, Address, "UnmarkAllocAsHeap 0x%llX : Heap=%u CallstackId=%u", Address, Heap, CallstackId);
	...
	if (!IsValidHeap(Heap))
	{
		...
		if (HeapErrors <= MaxLogMessagesPerErrorType)
		{
			UE_LOGF(LogTraceServices, Error, "[MemAlloc] HeapUnmarkAlloc: Heap %u is not valid (Address=0x%llX, Time=%f, CallstackId=%u)!", Heap, Address, Time, CallstackId);
		}
		return;
	}
	...
	if (Heap == RootHeap.HeapSpec->Id)
	{
		...
		if (HeapWarnings <= MaxLogMessagesPerWarningType)
		{
			UE_LOGF(LogTraceServices, Warning, "[MemAlloc] HeapUnmarkAlloc: Heap %u is a root heap (Address=0x%llX, Time=%f, CallstackId=%u)!", Heap, Address, Time, CallstackId);
		}
	}
#endif
	...
	if (!Alloc)
	{
		...
		if (HeapErrors <= MaxLogMessagesPerErrorType)
		{
			UE_LOGF(LogTraceServices, Error, "[MemAlloc] HeapUnmarkAlloc: Could not find heap with address 0x%llX (Heap=%u \"%ls\", Time=%f, CallstackId=%u)!", Address, Heap, HeapSpec.Name, Time, CallstackId);
		}
		...
		if (!Alloc)
		{
			if (HeapErrors <= MaxLogMessagesPerErrorType)
			{
				UE_LOGF(LogTraceServices, Error, "[MemAlloc] HeapUnmarkAlloc: A fake heap alloc will be created with size 0.");
			}
		...
		else
		{
			if (HeapErrors <= MaxLogMessagesPerErrorType)
			{
				UE_LOGF(LogTraceServices, Error, "[MemAlloc] HeapUnmarkAlloc: An alloc (Size=%llu, CallstackId=%u) with this address exists. It will be temporary marked as heap.", Alloc->GetSize(), Alloc->AllocCallstackId);
			}
		}
	...
	if (Alloc)
	{
		...
		if (bSearchLiveAllocs)
		{
			...
			RootHeap.LiveAllocs->Enumerate(Address, EndAddress,
				[&AllocatedSizeInHeap, &AllocsInHeap]
				(const FAllocationItem& ChildAlloc)
				{
			...
			if (AllocsInHeap.Num() > 0)
			{
				if (!EnumHasAnyFlags(HeapSpec.Flags, EMemoryTraceHeapFlags::NeverFrees))
				{
					...
					if (HeapWarnings <= MaxLogMessagesPerWarningType)
					{
						...
						UE_LOGF(LogTraceServices, Warning, "[MemAlloc] HeapUnmarkAlloc: %d memory leaks (%llu bytes) detected for heap %u (\"%ls\", Address=0x%llX, Size=%llu, Time=%f, CallstackId=%u)",
							AllocsInHeap.Num(), AllocatedSizeInHeap, Heap, HeapSpec.Name, Address, HeapSize, Time, CallstackId);
						UE_LOGF(LogTraceServices, Warning, "[MemAlloc] Top memory leaks:");
						...
						{
							UE_LOGF(LogTraceServices, Warning, "[MemAlloc]     alloc 0x%llX (%llu bytes)", AllocInHeap.Address, AllocInHeap.Size);
							if (--NumAllocsInTop <= 0)
							{
								break;
							}
				...
				{
					UE_LOGF(LogTraceServices, Log, "[MemAlloc] HeapUnmarkAlloc: %d memory allocs (%llu bytes) freed for heap %u (\"%ls\", Address=0x%llX, Size=%llu, Time=%f, CallstackId=%u)",
						AllocsInHeap.Num(), AllocatedSizeInHeap, Heap, HeapSpec.Name, Address, HeapSize, Time, CallstackId);
				}
#endif
				...
				for (const FAllocInHeap& AllocInHeap : AllocsInHeap)
				{
					INSIGHTS_WATCH_INDIRECT_API_LOGF("Free", ThreadId, Time, AllocInHeap.Address);
		...
		const uint32 AllocAlignment = Alloc->GetAlignment();
		const uint32 AllocCallstackId = Alloc->AllocCallstackId;
		const TagIdType AllocTag = Alloc->Tag;

		// Re-add this allocation to the Live allocs.
		ensure(Alloc->IsHeap());
		Alloc->RemoveHeapFlag();
		RootHeap.LiveAllocs->Add(Alloc); // the Live allocs takes ownership of Alloc
		Alloc = nullptr;

		// Update the timeline stats (Alloc re-added to LiveAllocs as a normal "alloc").
		TotalAllocatedMemory += AllocSize;
		++TotalLiveAllocations;
		SetMaxSampleU64(ETimelineU64::MaxTotalAllocatedMemory, TotalAllocatedMemory);
		SetMaxSampleU32(ETimelineU32::MaxLiveAllocations, TotalLiveAllocations);
		...
		TagTracker.PushTagFromPtr(ThreadId, Tracker, AllocTag);
		INSIGHTS_WATCH_INDIRECT_API_LOGF("Free", ThreadId, Time, Address);
		FreeInternal(ThreadId, Time, CallstackId, Address, RootHeap.HeapSpec->Id);
		INSIGHTS_WATCH_INDIRECT_API_LOGF("Alloc", ThreadId, Time, Address);
		AllocInternal(ThreadId, Time, AllocCallstackId, Address, AllocSize, AllocAlignment, RootHeap.HeapSpec->Id);
...
void FAllocationsProvider::FRootHeap::UpdateHistogramByAllocSize(uint64 Size)
{
	...
	if (Size != 0)
	{
		Index = 65 - static_cast<uint32>(FMath::CountLeadingZeros64(Size - 1));
		check(Index < NumAllocSizeHistogramPow2);
	}
	++AllocSizeHistogramPow2[Index];
}
...
void FAllocationsProvider::FRootHeap::UpdateHistogramByEventDistance(uint32 EventDistance)
{
	...
	else if (EventDistance != uint32(-1))
	{
		--EventDistance;
		Index = 1;
		while (EventDistance > 0)
		{
			++Index;
			EventDistance /= 10;
		}
		check(Index < NumEventDistanceHistogramPow10 - 1);
	}
	else // EventDistance == uint32(-1)
	{
		Index = NumEventDistanceHistogramPow10 - 1;
	}
	++EventDistanceHistogramPow10[Index];
}

////////////////////////////////////////////////////////////////////////////////////////////////////

void FAllocationsProvider::FRootHeap::LogStats()
{
	if (EventIndex == 0)
	{
		return;
	...
	UE_LOGF(LogTraceServices, Log, "[MemAlloc] RootHeap %u (%ls) : %u events, %u live allocs, max alloc size = %llu bytes, max event distance = %u",
		uint32(HeapSpec->Id), HeapSpec->Name, EventIndex, LiveAllocs->Num(), MaxAllocSize, MaxEventDistance);

	LogAllocSizeHistogram();
	LogEventDistanceHistogram();
}

////////////////////////////////////////////////////////////////////////////////////////////////////

static FUtf8String Pow2ToString(uint32 Pow2, const UTF8CHAR* UnitName = UTF8TEXT(""), bool bAddSpaceBeforeUnit = false)
{
	const ANSICHAR* UnitPrefixes[] = { "", "Ki", "Mi", "Gi", "Ti", "Pi", "Ei" };
	constexpr int32 NumPrefixes = UE_ARRAY_COUNT(UnitPrefixes);

	uint32 Pow = 10;
	for (int32 Index = 0; Index < NumPrefixes; ++Index)
	{
		if (Pow2 < Pow)
		{
			if (bAddSpaceBeforeUnit)
			{
				if (UnitPrefixes[Index][0] == 0 && UnitName[0] == 0)
				{
					return FUtf8String::Printf("%u", 1u << (Pow2 + 10 - Pow));
				}
				else
				{
					return FUtf8String::Printf("%u %s%s", 1u << (Pow2 + 10 - Pow), UnitPrefixes[Index], UnitName);
				}
			}
			else
			{
...
static FUtf8String Pow2BytesToString(uint32 Pow2)
{
	return Pow2ToString(Pow2, UTF8TEXT("B"), true);
}

////////////////////////////////////////////////////////////////////////////////////////////////////

static FUtf8String Pow10ToString(uint32 Pow10, const UTF8CHAR* UnitName = UTF8TEXT(""), bool bAddSpaceBeforeUnit = false)
{
	const ANSICHAR* UnitPrefixes[] = { "", "K", "M", "G", "T", "P", "E" };
	constexpr int32 NumPrefixes = UE_ARRAY_COUNT(UnitPrefixes);
	...
	for (int32 Index = 0; Index < NumPrefixes; ++Index)
	{
		...
		{
			uint64 Value = 1ull;
			const uint32 ShiftCount = Pow10 + 3 - Pow;
			for (uint32 Shift = 0; Shift < ShiftCount; ++Shift)
			{
				Value *= 10;
			}
			if (bAddSpaceBeforeUnit)
			{
				if (UnitPrefixes[Index][0] == 0 && UnitName[0] == 0)
				{
					return FUtf8String::Printf("%llu", Value);
				}
				else
				{
					return FUtf8String::Printf("%llu %s%s", Value, UnitPrefixes[Index], UnitName);
				}
			}
			else
			{
...
static FUtf8String Pow10EventsToString(uint32 Pow10)
{
	return Pow10ToString(Pow10, UTF8TEXT(""), false);
}

////////////////////////////////////////////////////////////////////////////////////////////////////

static FUtf8String CountToString(uint64 Count)
{
	FUtf8String Out = FUtf8String::Printf("%llu", Count);
	for (int32 Index = Out.Len() - 3; Index > 0; Index -= 3)
	{
		Out.InsertAt(Index, ',');
	}
	return Out;
...
void FAllocationsProvider::FRootHeap::LogAllocSizeHistogram()
{
	uint32 StartIndex = 0;
	while (StartIndex < NumAllocSizeHistogramPow2 && AllocSizeHistogramPow2[StartIndex] == 0)
	{
		++StartIndex;
	}

	uint32 EndIndex = NumAllocSizeHistogramPow2;
	while (EndIndex > 0 && AllocSizeHistogramPow2[EndIndex - 1] == 0)
	{
		--EndIndex;
	}
	...
	check(HeapSpec != nullptr);
	UE_LOGF(LogTraceServices, Log, "[MemAlloc] Alloc Size Histogram (%ls):", HeapSpec->Name);
	...
	{
		Sum += AllocSizeHistogramPow2[Index];
	}
	uint64 PartialSum = 0;
	double PercentFactor = 100.0 / (double)Sum;

	UE_LOGF(LogTraceServices, Log, "[MemAlloc] -----------------------------------------------------------");
	for (uint32 Index = StartIndex; Index < EndIndex; ++Index)
	{
		const uint64 Count = AllocSizeHistogramPow2[Index];
		if (Count == 0)
		{
			continue;
		}
		PartialSum += Count;
		const double Percent = (double)Count * PercentFactor;
		const double CumulativePercent = (double)PartialSum * PercentFactor;
		if (Index == 65)
		{
			// 65 : (2^63 .. 2^64)
			UE_LOGF(LogTraceServices, Log, "[MemAlloc] (%8s .. %-8s) %11s %4.01f%% %11s %5.01f%%",
				*Pow2BytesToString(Index - 2), *Pow2BytesToString(Index - 1), *CountToString(Count), Percent, *CountToString(PartialSum), CumulativePercent);
		}
		else if (Index > 2)
		{
			// i : (2^(i-2) .. 2^(i-1)], i = 3 .. 64
		...
		{
			UE_LOGF(LogTraceServices, Log, "[MemAlloc] [     1 byte         ] %11s %4.01f%% %11s %5.01f%%",
				*CountToString(Count), Percent, *CountToString(PartialSum), CumulativePercent);
		}
		else // if (Index == 0 || Index == 2)
		{
			UE_LOGF(LogTraceServices, Log, "[MemAlloc] [     %u bytes        ] %11s %4.01f%% %11s %5.01f%%",
				Index, *CountToString(Count), Percent, *CountToString(PartialSum), CumulativePercent);
		}
	}
	UE_LOGF(LogTraceServices, Log, "[MemAlloc] -----------------------------------------------------------");
}
...
void FAllocationsProvider::FRootHeap::LogEventDistanceHistogram()
{
	uint32 StartIndex = 0;
	while (StartIndex < NumEventDistanceHistogramPow10 && EventDistanceHistogramPow10[StartIndex] == 0)
	{
		++StartIndex;
	}

	uint32 EndIndex = NumEventDistanceHistogramPow10;
	while (EndIndex > 0 && EventDistanceHistogramPow10[EndIndex - 1] == 0)
	{
		--EndIndex;
	}
	...
	check(HeapSpec != nullptr);
	UE_LOGF(LogTraceServices, Log, "[MemAlloc] Event Distance Histogram (%ls):", HeapSpec->Name);
	...
	//  3 : (10 .. 100]
	// ...
	//  i : (10^(i-2) .. 10^(i-1)], i = 2 .. 11
	// ...
	// 11 : (10^9 .. 10^10]
	// 12 : [MAX_uint32]

	uint64 Sum = LiveAllocs->Num();
	for (uint32 Index = StartIndex; Index < EndIndex; ++Index)
	{
		Sum += EventDistanceHistogramPow10[Index];
	}
	uint64 PartialSum = 0;
	double PercentFactor = 100.0 / (double)Sum;
	...
	for (uint32 Index = StartIndex; Index < EndIndex; ++Index)
	{
		const uint64 Count = EventDistanceHistogramPow10[Index];
		if (Count == 0)
		{
			continue;
		}
		...
		const double Percent = (double)Count * PercentFactor;
		const double CumulativePercent = (double)PartialSum * PercentFactor;
		if (Index == NumEventDistanceHistogramPow10 - 1)
		{
			UE_LOGF(LogTraceServices, Log, "[MemAlloc] [     MAX_uint32     ] %11s %4.01f%% %11s %5.01f%%",
				*CountToString(Count), Percent, *CountToString(PartialSum), CumulativePercent);
		}
		else if (Index > 1)
		{
			// i : (10^(i-2) .. 10^(i-1)], i = 2 .. 11
			UE_LOGF(LogTraceServices, Log, "[MemAlloc] (%8s .. %-8s] %11s %4.01f%% %11s %5.01f%%",
				*Pow10EventsToString(Index - 2), *Pow10EventsToString(Index - 1), *CountToString(Count), Percent, *CountToString(PartialSum), CumulativePercent);
		}
		else if (Index == 1)
		{
			UE_LOGF(LogTraceServices, Log, "[MemAlloc] [       1 event      ] %11s %4.01f%% %11s %5.01f%%",
				*CountToString(Count), Percent, *CountToString(PartialSum), CumulativePercent);
		}
		else if (Index == 0)
		{
			UE_LOGF(LogTraceServices, Log, "[MemAlloc] [       0 events     ] %11s %4.01f%% %11s %5.01f%%",
				*CountToString(Count), Percent, *CountToString(PartialSum), CumulativePercent);
		}
	}
	if (LiveAllocs->Num() > 0)
	{
		const uint64 Count = LiveAllocs->Num();
		PartialSum += Count;
		const double Percent = (double)Count * PercentFactor;
		const double CumulativePercent = (double)PartialSum * PercentFactor;
		UE_LOGF(LogTraceServices, Log, "[MemAlloc] (   \u221E (live allocs)  ) %11s %4.01f%% %11s %5.01f%%",
			*CountToString(Count), Percent, *CountToString(PartialSum), CumulativePercent);
	}
	UE_LOGF(LogTraceServices, Log, "[MemAlloc] -----------------------------------------------------------");
}
...
void FAllocationsProvider::EditPushTagFromPtr(uint32 ThreadId, uint8 Tracker, uint64 Ptr, HeapId RootHeapId)
{
	...
	else
	{
		...
		if (MiscErrors <= MaxLogMessagesPerErrorType)
		{
			UE_LOGF(LogTraceServices, Error, "[MemAlloc] Invalid root heap (%u) for MemoryScopePtr or ReallocFree event!", RootHeapId);
		}
	}
	...
	if (!Alloc)
	{
		...
		if (MiscErrors <= MaxLogMessagesPerErrorType)
		{
			UE_LOGF(LogTraceServices, Error, "[MemAlloc] Invalid address (0x%llX) for MemoryScopePtr or ReallocFree event!", Ptr);
		}
	}
}
...
void FAllocationsProvider::EditOnAnalysisCompleted(double Time)
{
	...
	if (!bInitialized)
	{
		...
		{
			UE_LOGF(LogTraceServices, Error, "[MemAlloc] TagTracker errors: %u", TagTracker.GetNumErrors());
		}
		return;
	}
	...
	{
		UE_LOGF(LogTraceServices, Warning, "[MemAlloc] %llu warnings (%llu ALLOC + %llu FREE + %llu HEAP + %llu SWAP + %llu other)",
			AllocWarnings + FreeWarnings + HeapWarnings + SwapWarnings + MiscWarnings,
			AllocWarnings, FreeWarnings, HeapWarnings, SwapWarnings, MiscWarnings);
	}
	if (TagTracker.GetNumWarnings() > 0)
	{
		UE_LOGF(LogTraceServices, Warning, "[MemAlloc] TagTracker warnings: %u", TagTracker.GetNumWarnings());
	}

	if (AllocErrors > 0)
	{
		UE_LOGF(LogTraceServices, Error, "[MemAlloc] ALLOC event errors: %llu", AllocErrors);
	}
	if (FreeErrors > 0)
	{
		UE_LOGF(LogTraceServices, Error, "[MemAlloc] FREE event errors: %llu", FreeErrors);
	}
	if (HeapErrors > 0)
	{
		UE_LOGF(LogTraceServices, Error, "[MemAlloc] HEAP event errors: %llu", HeapErrors);
	}
	if (SwapErrors > 0)
	{
		UE_LOGF(LogTraceServices, Error, "[MemAlloc] SWAP event errors: %llu", SwapErrors);
	}
	if (MiscErrors > 0)
	{
		UE_LOGF(LogTraceServices, Error, "[MemAlloc] Other errors: %llu", MiscErrors);
	}
	if (TagTracker.GetNumErrors() > 0)
	{
		UE_LOGF(LogTraceServices, Error, "[MemAlloc] TagTracker errors: %u", TagTracker.GetNumErrors());
	...
	UE_LOGF(LogTraceServices, Log, "[MemAlloc] Total Indexed: %llu events", TotalEventCount);
	UE_LOGF(LogTraceServices, Log, "[MemAlloc] Traced: %llu alloc events, %llu free events, %llu heap events, %llu swap op events",
		AllocEvents, FreeEvents, HeapEvents, SwapOpEvents);
	UE_LOGF(LogTraceServices, Log, "[MemAlloc] Analysis completed (%llu allocs, %llu frees, %llu heaps, %u heap specs).",
