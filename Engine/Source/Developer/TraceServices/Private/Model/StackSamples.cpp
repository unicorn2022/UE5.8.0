...
void FStackSamplesProvider::AddStackSample(uint32 SystemThreadId, double Time, uint32 Count, const uint64* Addresses)
{
	...
	{
		UE_LOGF(LogTraceServices, Warning, "[StackSampling] Time should increase monotonically (SysThreadId=0x%X, Time=%f < %f)!", SystemThreadId, Time, Thread.LastSampleTime);
...
bool FStackSamplesProvider::UpdateTimerName(FStackSampleFrame& StackFrame, bool bFirstTime)
{
	...
	{
		TimerName = FString::Printf(TEXT("0x%llX"), StackFrame.Address);
	}
	else
	{
		switch (StackFrame.Symbol->GetResult())
		{
			case TraceServices::ESymbolQueryResult::Pending:
				if (bFirstTime)
				{
					TimerName = FString::Printf(TEXT("0x%llX pending..."), StackFrame.Address);
				}
				bIsPending = true;
				break;

			case TraceServices::ESymbolQueryResult::OK:
				TimerName = FString::Printf(TEXT("%s (%s / 0x%llX)"),
			...
			case TraceServices::ESymbolQueryResult::NotLoaded:
				TimerName = FString::Printf(TEXT("%s / 0x%llX NotLoaded"),
					StackFrame.Symbol->Module ? StackFrame.Symbol->Module : TEXT("N/A"),
					StackFrame.Address);
				break;

			case TraceServices::ESymbolQueryResult::Mismatch:
				TimerName = FString::Printf(TEXT("%s / 0x%llX Mismatch"),
					StackFrame.Symbol->Module ? StackFrame.Symbol->Module : TEXT("N/A"),
					StackFrame.Address);
				break;

			case TraceServices::ESymbolQueryResult::NotFound:
				TimerName = FString::Printf(TEXT("%s / 0x%llX NotFound"),
					StackFrame.Symbol->Module ? StackFrame.Symbol->Module : TEXT("N/A"),
					StackFrame.Address);
				break;

			default:
				TimerName = FString::Printf(TEXT("0x%llX Invalid !!!"), StackFrame.Address);
				break;
		}
	}
...
double FStackSamplesProvider::OnAnalysisEnd()
{
	...
	UE_LOGF(LogTraceServices, Log, "[StackSampling] max stack size: %d frames", MaxStackSize);
	UE_LOGF(LogTraceServices, Log, "[StackSampling] Analysis completed (%llu timers, %d timelines, %llu threads, %llu stack samples, %llu stack frames).",
