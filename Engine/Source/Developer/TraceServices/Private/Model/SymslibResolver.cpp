...
namespace
{
	...
	static bool LoadBinary(const TCHAR* ModuleFullName, SYMS_Arena* Arena, SYMS_ParseBundle& Bundle, TArray<FAutoMappedFile>& Files, const FString& SearchPath, const FString& Platform, const FString& AppName)
	{
		...
		if (!bFileFound)
		{
			UE_LOGF(LogSymslib, Verbose, "Binary file '%ls' not found", *FilePath);
			return false;
		}
		...
		{
			UE_LOGF(LogSymslib, Warning, "Failed to load binary file '%ls'", *BinaryPath);
			return false;
		}
		...
		{
			UE_LOGF(LogSymslib, Warning, "Cannot parse binary file '%ls'", *BinaryPath);
			return false;
		}
	...
	static bool LoadDebug(SYMS_Arena* Arena, SYMS_ParseBundle& Bundle, TArray<FAutoMappedFile>& Files, const FString& SearchPath)
	{
		if (syms_bin_is_dbg(Bundle.bin))
		{
			...
			UE_LOGF(LogSymslib, Verbose, "Binary file '%ls' has debug info built-in", *Files[0].FilePath);
			return true;
		}
		...
		if (!List.first)
		{
			UE_LOGF(LogSymslib, Warning, "Binary file '%ls' built without debug info", *Files[0].FilePath);
			return false;
		}
		...
		if (DebugPath.IsEmpty())
		{
			UE_LOGF(LogSymslib, Verbose, "Debug symbols file '%ls' not found", *FilePath);
			return false;
		}
		...
		{
			UE_LOGF(LogSymslib, Warning, "Failed to load debug symbols file '%ls'", *DebugPath);
			return false;
		}
		...
		{
			// syms_dbg_accel_from_file only handles PDB. For ELF .debug files (separate DWARF),
			// we need to go through the bin path: parse as binary, then extract debug info from it.
			if (syms_file_is_bin(Accel))
			{
				SYMS_BinAccel* DbgBin = syms_bin_accel_from_file(Arena, File.GetData(), Accel);
				if (syms_accel_is_good(DbgBin) && syms_bin_is_dbg(DbgBin))
				{
					DbgAccel = syms_dbg_accel_from_bin(Arena, File.GetData(), DbgBin);
				}
			}

			if (!syms_accel_is_good(DbgAccel))
			{
				UE_LOGF(LogSymslib, Warning, "Cannot parse debug symbols file '%ls'", *DebugPath);
				return false;
			}
		}
	...
	FFunctionGraphTask::CreateAndDispatchWhenReady([this, Entry, OverrideSearchPath, ResolveOnSuccess]
	{
		...
		if (Entry->Module->Status.load() == EModuleStatus::Loaded)
		{
			...
			// resolve them immediately.
			{
				...
				UE_LOGF(LogSymslib, VeryVerbose, "String allocator: %.02f KiB used (+ %.02f KiB wasted) in %d blocks",
...
void FSymslibResolver::OnAnalysisComplete()
{
	...
	CleanupTask = FFunctionGraphTask::CreateAndDispatchWhenReady([this]
	{
		...
		UE_LOGF(LogSymslib, Display, "Allocated %.02f MiB of strings (%.02f MiB wasted).",
...
void FSymslibResolver::LoadModuleTracked(FModuleEntry* Entry, FStringView OverrideSearchPath)
{
	...
	for (const FString& SearchPath : SearchPaths)
	{
		...
		UE_LOGF(LogSymslib, Display, "%ls", StatusMessage.ToString());
...
void FSymslibResolver::ResolveSymbolTracked(uint64 Address, FResolvedSymbol& Target, FSymbolStringAllocator& StringAllocator)
{
	...
	if (!Entry)
	{
		UE_LOGF(LogSymslib, Warning, "No module mapped to address 0x%llX.", Address);
