...
void FVerseAnalyzer::OnAnalysisEnd()
{
	...
	UE_LOGF(LogTraceServices, Log, "[Verse] Analysis completed (%u timers, %u samples; max stack size = %d).", NumVerseTimers, NumSamples, MaxStackSize);
}
...
bool FVerseAnalyzer::OnEvent(uint16 RouteId, EStyle Style, const FOnEventContext& Context)
{
	...
	case RouteId_DeclareString:
	{
		...
		UE_LOGF(LogTraceServices, VeryVerbose, "[Verse] DeclareString Id=0x%X Name=\"%ls\"", Id, *Name);
		...
		if (FoundTimerId)
		{
			...
			UE_LOGF(LogTraceServices, Warning, "[Verse] Duplicated timer Id=0x%X Name=\"%ls\" --> TimerId=%u", Id, *Name, TimerId);
		}
		else
		{
			{
	...
	case RouteId_NativeSample:
	{
		...
		if (RouteId == RouteId_BytecodeSample)
		{
			...
			{
				UE_LOGF(LogTraceServices, Warning, "[Verse] BytecodeSample Time=%f [%d callstack frames] BytecodeOffset=%d Line=%d", Time, NewSampleStackSize, Offset, Line);
			}
			else
			{
				UE_LOGF(LogTraceServices, VeryVerbose, "[Verse] BytecodeSample Time=%f [%d callstack frames] BytecodeOffset=%d Line=%d", Time, NewSampleStackSize, Offset, Line);
			}
		}
		else
		{
			if (bStackSizeWarning)
			{
				UE_LOGF(LogTraceServices, Warning, "[Verse] NativeSample Time=%f [%d callstack frames]", Time, NewSampleStackSize);
			}
			else
			{
				UE_LOGF(LogTraceServices, VeryVerbose, "[Verse] NativeSample Time=%f [%d callstack frames]", Time, NewSampleStackSize);
			}
		}
		...
		{
			UE_LOGF(LogTraceServices, Warning, "[Verse] Time should increase monotonically (Time=%f < %f)!", Time, LastSampleTime);
			Time = LastSampleTime;
		}
