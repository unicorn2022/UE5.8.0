...
bool FNetTraceAnalyzer::OnEvent(uint16 RouteId, EStyle Style, const FOnEventContext& Context)
{
	...
	// so we need to discard those. NameEvents are part of the ImportantEvents, so are also expected to arrive before InitEvent
	if ((NetTraceVersion == 0) && (RouteId != RouteId_InitEvent) && (RouteId != RouteId_NameEvent))
	{
		return true;
	}
...
void FNetTraceAnalyzer::HandlePacketContentEvent(const FOnEventContext& Context, const FEventData& EventData)
{
	...
	//UE_LOGF(LogNetTrace, Display, "FNetTraceAnalyzer::HandlePacketContentEvent: GameInstanceId: %u, ConnectionId: %u, %ls", (uint32)GameInstanceId, (uint32)ConnectionId, PacketType ? TEXT("Incoming") : TEXT("Outgoing"));
		...
		switch (DecodedEventType)
		{
			...
			case EContentEventType::NameId:
			{
				...
				else if (DecodedEventType == EContentEventType::NameId)
				{
					...
					{
						UE_LOGF(LogNetTrace, Warning, "PacketContentEvent GameInstanceId: %u, ConnectionId: %u %ls, Missing NameIndex: %llu", (uint32)GameInstanceId, (uint32)ConnectionId, ConnectionMode ? TEXT("Incoming") : TEXT("Outgoing"), DecodedNameOrObjectId);
					}
				}
...
void FNetTraceAnalyzer::HandlePacketEvent(const FOnEventContext& Context, const FEventData& EventData)
{
	...
	//UE_LOGF(LogNetTrace, Log, "PacketEvent GameInstanceId: %u, ConnectionId: %u, %ls, Seq: %u PacketBits: %u", (uint32)GameInstanceId, (uint32)ConnectionId, ConnectionMode ? TEXT("Incoming") : TEXT("Outgoing"), SequenceNumber, Packet.ContentSizeInBits);
}
...
void FNetTraceAnalyzer::HandleConnectionCreatedEvent(const FOnEventContext& Context, const FEventData& EventData)
{
	...
	//Sometimes it's possible for CreateConnection to be called multiple times for a ConnectionId if the capture was made using the TailBuffer (can cause multiple OnConnection calls)
	if (!GameInstanceState->ActiveConnections.Contains(ConnectionId))
	{
...
void FNetTraceAnalyzer::HandleConnectionUpdatedEvent(const FOnEventContext& Context, const FEventData& EventData)
{
	...
	else
	{
		...
		UE_LOGF(LogNetTrace, Warning, "Connection %d is missing", ConnectionId);
	}
