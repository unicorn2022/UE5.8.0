...
void FOldGpuProfilerAnalyzer::OnAnalysisEnd()
{
	...
	{
		UE_LOGF(LogTraceServices, Error, "[GpuProfiler] Frames with errors: %u", NumFramesWithErrors);
	}
	
	if (NumFrames > 0 || EventTypeMap.Num() > 0)
	{
		UE_LOGF(LogTraceServices, Log, "[GpuProfiler] Analysis completed (%u frames, %d timers).", NumFrames, EventTypeMap.Num());
	}
}
...
void FOldGpuProfilerAnalyzer::OnFrame(const UE::Trace::IAnalyzer::FOnEventContext& Context, uint32 GpuIndex)
{
	...
	{
		UE_LOGF(LogTraceServices, Error, "[GpuProfiler] The rendering frame %u has invalid timestamps!", RenderingFrameNumber);
	}
