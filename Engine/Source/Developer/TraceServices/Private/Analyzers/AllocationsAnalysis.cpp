...
bool FAllocationsAnalyzer::OnEvent(uint16 RouteId, EStyle Style, const FOnEventContext& Context)
{
	...
	switch (RouteId)
	{
		case RouteId_Init:
		{
			...
			{
				UE_LOGF(LogTraceServices, Error, "[MemAlloc] Version %u for Memory trace events is not supported by the current analyzer. Supported versions: [%u .. %u]", Version, MinSupportedVersion, MaxSupportedVersion);
				break;
			}
