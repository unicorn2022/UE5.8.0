...
#define CPUPROFILER_LOG_API_L1(Format, ...) UE_LOGF(LogTraceServices, Log, Format, ##__VA_ARGS__)
#define CPUPROFILER_LOG_API_L2(Format, ...) //UE_LOGF(LogTraceServices, Verbose, Format, ##__VA_ARGS__)
#define CPUPROFILER_LOG_API_L3(Format, ...) //UE_LOGF(LogTraceServices, VeryVerbose, Format, ##__VA_ARGS__)

// debug functionality; adds detection and extra logging for timer name changes
#define CPUPROFILER_DETECT_TIMER_NAME_CHANGE 0

// debug functionality; filters the trace events by ThreadId
#define CPUPROFILER_FILTER_BY_THREAD(ThreadId, ReturnOrBreak) //{ if (ThreadId != 2) { ReturnOrBreak; } }
...
void FCpuProfilerAnalyzer::OnAnalysisEnd()
{
	...
	for (auto& KV : ScopeNameToTimerIdMap)
	{
		...
		UE_LOGF(LogTraceServices, Warning, "[CpuProfiler] Timer defined %u times! (id=%d name=\"%ls\")", KV.Value.Count, KV.Value.Id, (const TCHAR*)KV.Key);
		bPossibleOutputEventTypeIssue = true;
	}
	if (bPossibleOutputEventTypeIssue)
	{
		UE_LOGF(LogTraceServices, Warning, "[CpuProfiler] Possible incorrect use of FCpuProfilerTrace::OutputEventType()!");
	}

	if (NumTimerWarnings > 0)
	{
		UE_LOGF(LogTraceServices, Warning, "[CpuProfiler] Number of timer/metadata warnings: %u", NumTimerWarnings);
	}

	UE_LOGF(LogTraceServices, Log, "[CpuProfiler] Analysis completed (%d threads, %d timers, %d spec ids, %llu scopes, %llu bytes, %.1f bytes/scope).",
		ThreadStatesMap.Num(),
...
bool FCpuProfilerAnalyzer::OnEvent(uint16 RouteId, EStyle Style, const FOnEventContext& Context)
{
	...
	case RouteId_EndThread:
	{
		...
		CPUPROFILER_FILTER_BY_THREAD(ThreadId, break);
	...
	bool bIsBeginEvent)
{
	...
	for (; RemainingPending > 0; RemainingPending--, PendingCursor++)
	{
		...
		if (PendingCycle < LastCycle)
		{
			...
			PendingCursor += RemainingPending;
			RemainingPending = 0;
			UE_LOGF(LogTraceServices, Error, "[CpuProfiler] Detected non-monotonically increasing timestamp. Further CPU timing events with metadata are ignored on thread %u.", ThreadState.ThreadId);
			break;
		}
...
void FCpuProfilerAnalyzer::OnCpuScopeEnter(const FOnEventContext& Context)
{
	...
	CPUPROFILER_FILTER_BY_THREAD(ThreadId, return);
	FThreadState& ThreadState = GetOrAddThreadState(ThreadId);
	...
	if (ensure(Cycle >= ThreadState.LastCycle))
	{
	...
	else
	{
		...
		// The issue can occur in late connect trace sessions with trace protocol <= 6 (i.e. the scoped events have relative timestamps).
		ThreadState.bShouldIgnorePendingEvents = true;
		UE_LOGF(LogTraceServices, Error, "[CpuProfiler] Detected scope enter event non-monotonically increasing timestamp. Further CPU timing events with metadata are ignored on thread %u.", ThreadState.ThreadId);
	}
}
...
void FCpuProfilerAnalyzer::OnCpuScopeLeave(const FOnEventContext& Context)
{
	...
	else
	{
		...
		// The issue can occur in late connect trace sessions with trace protocol <= 6 (i.e. the scoped events have relative timestamps).
		ThreadState.bShouldIgnorePendingEvents = true;
		UE_LOGF(LogTraceServices, Error, "[CpuProfiler] Detected scope leave event with non-monotonically increasing timestamp. Further CPU timing events with metadata are ignored on thread %u.", ThreadState.ThreadId);
	}
}
...
void FCpuProfilerAnalyzer::OnEventSpec(const FOnEventContext& Context)
{
	...
	CPUPROFILER_LOG_API_L2(TEXT("[CpuProfiler] EventSpec Id=%u Name=\"%ls\" File=\"%ls\" Line=%u"),
...
void FCpuProfilerAnalyzer::OnMetadataSpec(const FOnEventContext& Context)
{
	...
	CPUPROFILER_LOG_API_L2(TEXT("[CpuProfiler] MetadataSpec Id=%u Name=\"%ls\" NameFormat=\"%ls\" Fields[]={%u bytes}"),
		...
		if (TimingProfilerProvider)
		{
			...
			if (PreviousTimer && PreviousTimer->HasValidMetadataSpecId())
			{
				...
				if (++NumTimerWarnings < NumMaxWarnings)
				{
					...
					UE_LOGF(LogTraceServices, Warning, "[CpuProfiler] MetadataSpec -- de-duplicated timer for SpecId %u -- old: %u (%ls) metadata=\"%ls\" (%d fields) --> new: %u (%ls) metadata=\"%ls\" (%d fields) !!!",
						SpecId,
...
uint32 FCpuProfilerAnalyzer::AddCpuTimer(uint32 SpecId, const TCHAR* TimerName, const TCHAR* File, uint32 Line)
{
	...
	CPUPROFILER_LOG_API_L3(TEXT("[CpuProfiler] --> AddCpuTimer(SpecId=%u, Name=\"%ls\") --> TimerId=%u"), SpecId, TimerName, TimerId);
...
void FCpuProfilerAnalyzer::SetTimerName(uint32 SpecId, uint32 TimerId, const TCHAR* TimerName)
{
	...
	{
		UE_LOGF(LogTraceServices, Warning, "[CpuProfiler] --> SetTimerName(SpecId=%u, TimerId=%u, Name=\"%ls\") : name was \"%ls\" !!!", SpecId, TimerId, TimerName, PrevTimerName);
	}
	else
	{
		CPUPROFILER_LOG_API_L2(TEXT("[CpuProfiler] --> SetTimerName(SpecId=%u, TimerId=%u, Name=\"%ls\""), SpecId, TimerId, TimerName);
	}
#endif
}
...
#undef CPUPROFILER_FILTER_BY_THREAD
