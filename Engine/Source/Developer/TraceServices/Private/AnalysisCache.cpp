	...
	: CacheFilePath(FilePath)
{
	...
	{
		UE_LOGF(LogAnalysisCache, Display, "Putting cache in transient mode.");
		bTransientMode = true;
		return;
	}
	...
	if (const int64 FileSize = PlatformFile.FileSize(*CacheFilePath); FileSize > 0)
	{
		if (const TUniquePtr<IFileHandle> File(PlatformFile.OpenRead(*CacheFilePath)); File.IsValid())
		{
			...
			{
				UE_LOGF(LogAnalysisCache, Error, "Failed to open cache file table of contents.");
				bTransientMode = true;
			...
			{
				UE_LOGF(LogAnalysisCache, Error, "Cache file has written several blocks but table of contents contains no blocks. This is likely caused by abnormal program termination. Please delete \"%ls\". Putting cache in transient mode.", *CacheFilePath);
				IndexEntries.Empty();
				bTransientMode = true;
				return;
			}

			UE_LOGF(LogAnalysisCache, VeryVerbose, "Cache contains %d blocks:", Blocks.Num());
			UE_LOGF(LogAnalysisCache, VeryVerbose, "   %10ls   %10ls   %13ls   %13ls   %13ls", TEXT("Cache index"), TEXT("Block index"), TEXT("Offset"), TEXT("Uncompressed"), TEXT("Compressed"));
		...
		else
		{
			...
			UE_LOGF(LogAnalysisCache, Warning,
			       "Unable to read the cache file %ls, possibly already open in another session. Putting cache in transient mode.",
			       *CacheFilePath);
			bTransientMode = true;
		}
	}
...
FAnalysisCache::FFileContents::~FFileContents()
{
	...
	if (!Blocks.IsEmpty())
	{
		...
		{
			UE_LOGF(LogAnalysisCache, Error, "Failed to update cache files table of contents.");
		}
	}
}
...
bool FAnalysisCache::FFileContents::Load()
{
	...
	{
		UE_LOGF(LogAnalysisCache, Warning, "Cache file of unknown version (%u), cannot load.", Version);
		return false;
	}

	UE_LOGF(LogAnalysisCache, Display, "Loading cache file (version %u).", Version);
...
uint64 FAnalysisCache::FFileContents::LoadBlock(FMutableMemoryView Block, BlockKeyType BlockKey)
{
	...
	if (EntryIndex == INDEX_NONE)
	{
		UE_LOGF(LogAnalysisCache, Error, "Trying to load unknown block 0x%x.", BlockKey);
		return 0;
	}
...
IFileHandle* FAnalysisCache::FFileContents::GetFileHandleForWrite()
{
	...
	if (!CacheFileWrite.IsValid())
	{
		...
		UE_LOGF(LogAnalysisCache, Warning,
			   "Unable to write to the cache file %ls, possibly already open in another session. Putting cache in transient mode.",
			   *CacheFilePath);
		bTransientMode = true;
		return nullptr;
	}
...
FAnalysisCache::~FAnalysisCache()
{
	...
	UE_LOGF(LogAnalysisCache, Display, "Closing analysis cache, %0.2f MiB read, %0.2f MiB written.",
...
FSharedBuffer FAnalysisCache::GetBlocks(FCacheId CacheId, uint32 BlockIndexStart, uint32 BlockCount)
{
	...
	{
		UE_LOGF(LogAnalysisCache, Error, "Block range %u to %u is invalid for cache id %u.", BlockIndexStart,
