...
void FAnalysisSession::Start()
{
	...
	Context.SetOnCompletedDelegate(UE::Trace::FOnCompletedDelegate::CreateRaw(this, &FAnalysisSession::OnAnalysisComplete));
	Processor = Context.Process(*DataStream);
}

void FAnalysisSession::Stop(bool bAndWait) const
{
	if (!bDataStreamClosed.exchange(true))
	{
		DataStream->Close();
	}
	Processor.Stop();
...
void FAnalysisSession::OnAnalysisComplete()
{
	if (!bDataStreamClosed.exchange(true))
	{
		DataStream->Close();
