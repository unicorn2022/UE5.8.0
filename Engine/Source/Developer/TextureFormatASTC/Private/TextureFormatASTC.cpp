...
static EPixelFormat GetQualityFormat(const FTextureBuildSettings& BuildSettings)
{
	...
	if (bHDRFormat)
	{
		switch (OverrideSizeValue)
		{
			...
			default: UE_LOGF(LogTextureFormatASTC, Fatal, "Max quality higher than expected");
		}
	}
	else
	{
	...
	FCompressedImage2D& OutCompressedImage)
{
	...
	switch (GetDefaultCompressionBySpeedValue(BuildSettings.FormatConfigOverride))
	{
		...
		default: UE_LOGF(LogTextureFormatASTC, Fatal, "ASTC speed quality higher than expected");
	}
	...
	if (ThunkError)
	{
		UE_LOGF(LogTextureFormatASTC, Error, "Failed to create astcenc thunk: %ls", ANSI_TO_TCHAR(ThunkError));
	...
	for (uint32 TaskIndex = 0; TaskIndex < CreateParams.TaskCount; TaskIndex++)
	{
		...
		{
			UE_LOGF(LogTextureFormatASTC, Error, "Astcenc Thunk DoWork has failed: %ls", ANSI_TO_TCHAR(Results[TaskIndex]));
			bSucceeded = false;
			break;
		}
	}
...
public:
	FTextureFormatASTC()
	{
		...
		if (!DecodeThunk)
		{
			UE_LOGF(LogTextureFormatASTC, Fatal, "Couldn't find latest ASTC enc version lib: %ls", AstcEncVersionStrings[SupportedAstcEncVersionCount-1]);
		}
	}
		...
		static bool bIsInitialized = [this]() mutable
		{
			...
			for (const ITargetPlatformSettings* TargetPlatform : TargetPlatforms)
			{
				...
				if (!RDOEnabledByPlatform.Contains(TargetPlatform->IniPlatformName()))
				{
					...
					if (bRDOEnabled)
					{
						UE_LOGF(LogTextureFormatASTC, Display, "ArmASTC RDO: %ls from section %ls on platform %ls", 
	...
	virtual bool DecodeImage(int32 InSizeX, int32 InSizeY, int32 InNumSlices, EPixelFormat InPixelFormat, bool bInSRGB, const FName& InTextureFormatName, FSharedBuffer InEncodedData, FImage& OutImage, FStringView InTextureName) const
	{
		...
		if (Error)
		{
			UE_LOGF(LogTextureFormatASTC, Error, "Failed to decode astc image: %ls - texture %.*ls", ANSI_TO_TCHAR(Error), InTextureName.Len(), InTextureName.GetData());
			return false;
		}
		...
		) const override
	{
		...
		UE_CALL_ONCE( [&](){
			UE_LOGF(LogTextureFormatASTC, Display, "TextureFormatASTC using astcenc")
		} );
		...
		for (size_t i = 0; i < SupportedAstcEncVersionCount; i++)
		{
			if (SupportedAstcEncVersions[i] == Version)
			{
				...
				if (!ThunkVersion->LibHandle)
				{
					...
					{
						UE_LOGF(LogTextureFormatASTC, Warning, "ASTCEnc version %ls requested but not found.", AstcEncVersionStrings[i]);
					}
					else
					{
						UE_LOGF(LogTextureFormatASTC, Fatal, "Latest ASTCEnc version %ls required but not found.", AstcEncVersionStrings[i]);
					}
					return nullptr;
				}
				...
				if (!ThunkVersion->Create ||
					...
					!SetAllocators)
				{
					UE_LOGF(LogTextureFormatASTC, Fatal, "ASTCEnc version %ls library loaded but has missing exports", AstcEncVersionStrings[i]);
					return nullptr;
				}

				UE_LOGF(LogTextureFormatASTC, Display, "ASTCEnc version %ls library loaded", AstcEncVersionStrings[i]);
