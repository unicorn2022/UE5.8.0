	...
	if(!PackageName.IsEmpty() && !MeshName.IsEmpty())
	{
		...
		if (bValidData)
		{
			...
			for (int32 RawMeshLODIndex = 0; RawMeshLODIndex < RawMeshes.Num(); RawMeshLODIndex++)
			{
				...
				{
					if (RawMesh.MaterialIndexToImportIndex.IsValidIndex(UniqueMaterialIndex))
					{
						StaticMesh->GetSectionInfoMap().Set(RawMeshLODIndex, SectionIndex, FMeshSectionInfo(RawMesh.MaterialIndexToImportIndex[UniqueMaterialIndex]));
					}
					else
					{
...
)
{
	...
	for (int32 SectionIndex = 0; SectionIndex < LODModel.Sections.Num(); SectionIndex++)
	{
		...
		UE_LOGF(LogSkeletalMesh, Verbose, "Section %u: %u vertices, %u active bones",
			SectionIndex,
...
public:
	...
	bool GatherSourceMeshesPerLOD(IMeshReduction* MeshReduction)
	{
		...
		for (int32 LODIndex = 0; LODIndex < NumSourceModels; ++LODIndex)
		{
			...
			if (!SrcModel.IsRawMeshEmpty())
			{
				...
				if (!RawMesh.IsValidOrFixable())
				{
					UE_LOGF(LogMeshUtilities, Error, "Raw mesh is corrupt for LOD%d.", LODIndex);
					return false;
				}
		...
		if (LODMeshes.Num() == 0)
		{
			UE_LOGF(LogMeshUtilities, Error, "Raw Mesh data contains no mesh data to build a mesh that can be rendered.");
			return false;
		}
		else if (LODMeshes[0].WedgeIndices.Num() == 0)
		{
			UE_LOGF(LogMeshUtilities, Error, "Raw Mesh data contains no wedge index data to build a mesh that can be rendered.");
			return false;
		}
	...
	bool ReduceLODs(const FStaticMeshLODGroup& LODGroup, IMeshReduction* MeshReduction, TArray<bool>& OutWasReduced)
	{
		...
		{
			UE_LOGF(LogMeshUtilities, Error, "Mesh contains zero source models.");
			return false;
		}
		...
		for (int32 LODIndex = 0; LODIndex < NumSourceModels; ++LODIndex)
		{
			...
			if (MeshReduction && (ReductionSettings.PercentTriangles < 1.0f || ReductionSettings.MaxDeviation > 0.0f))
			{
				...
				{
					UE_LOGF(LogMeshUtilities, Error, "Mesh reduction produced a corrupt mesh for LOD%d", LODIndex);
					return false;
				}
		...
		if (NumValidLODs < 1)
		{
			UE_LOGF(LogMeshUtilities, Error, "Mesh reduction produced zero LODs.");
			return false;
		}
		Stage = EStage::Reduce;
		return true;
			...
			auto VerifyTangentSpace = [&bIsZeroLengthNormalErrorMessageDisplayed, &BuildData, &SkeletalMeshName](FVector3f& NormalizedVector)
			{
				if (NormalizedVector.IsNearlyZero() || NormalizedVector.ContainsNaN())
				{
					...
					if (!bIsZeroLengthNormalErrorMessageDisplayed)
					{
						...
						{
							UE_LOGF(LogSkeletalMesh, Display, "%ls", *(TextMessage.ToString()));
						}
					}
				}
			};
	...
	bool GenerateSkeletalRenderMesh(SkeletalMeshBuildData& BuildData)
	{
		...
		for (int32 FaceIndex = 0; FaceIndex < BuildData.Faces.Num(); FaceIndex++)
		{
			...
			for (int32 VertexIndex = 0; VertexIndex < 3; VertexIndex++)
			{
				...
				const FVector3f& V = Vertex.Position;
				IAndZ.Z = 0.30f * V.X + 0.33f * V.Y + 0.37f * V.Z;
		...
		SkeletalMeshTools::ChunkSkinnedVertices(BuildData.Chunks, AlternateBoneIDs, FGPUBaseSkinVertexFactory::GHardwareMaxGPUSkinBones);
...
)
{
	...
	if (IsInGameThread())
	{
		...
		for (int32 SectionIndex = 0; SectionIndex < BuildData.LODModel.Sections.Num(); SectionIndex++)
		{
			...
			UE_LOGF(LogSkeletalMesh, Log, "Section %u: Material=%u, %u triangles",
				SectionIndex,
	...
	{
		UE_LOGF(LogSkeletalMesh, Fatal, "Cannot call FMeshUtilities::BuildSkeletalMesh on a console!");
	}
	return false;
#endif
}
...
bool FMeshUtilities::GetBoneReductionData(const USkeletalMesh* SkeletalMesh, int32 LODIndex, TMap<uint16, uint16>& OutBonesToReplace) const
{
	IMeshBoneReductionModule& MeshBoneReductionModule = FModuleManager::Get().LoadModuleChecked<IMeshBoneReductionModule>("MeshBoneReduction");
	IMeshBoneReduction* MeshBoneReductionInterface = MeshBoneReductionModule.GetMeshBoneReductionInterface();

	return MeshBoneReductionInterface ? MeshBoneReductionInterface->GetBoneReductionData(SkeletalMesh, LODIndex, OutBonesToReplace) : false;
}
...
void FMeshUtilities::GenerateRuntimeSkinWeightData(
	...
	FRuntimeSkinWeightProfileData& InOutSkinWeightOverrideData,
	const FSkeletalMeshUnifiedBoneIndices* UnifiedBoneIndices) const
{
	...
	if (InRawSkinWeights.Num() == TargetLODModel.NumVertices)
	{
		...
		TargetLODModel.GetVertices(TargetVertices, UnifiedBoneIndices);
		...
		const bool b16BitBoneIndices = (UnifiedBoneIndices && UnifiedBoneIndices->GetNumUnifiedBones() > 256) || TargetLODModel.DoSectionsUse16BitBoneIndex();
		...
		for (int32 VertexIndex = 0; VertexIndex < TargetVertices.Num(); ++VertexIndex)
		{
			...
			// Weights are generally sorted from high to low, but they are not guaranteed to be sorted by bone index.
			// Since we want to know if two list of weights are the same, we have to sort them again by both index and weight
			{
				struct FInfluence
				{
					uint16 BoneIndex = 0;
					...
					bool operator < (const FInfluence& Other) const
					{
					...
					bool operator == (const FInfluence& Other) const = default;
				};


				TArray<FInfluence, TInlineAllocator<MAX_TOTAL_INFLUENCES>> SourceWeightArray;
				TArray<FInfluence, TInlineAllocator<MAX_TOTAL_INFLUENCES>> TargetWeightArray;
			
				for (int32 InfluenceIndex = 0; InfluenceIndex < NumInfluences; ++InfluenceIndex)
				{
					SourceWeightArray.Add({SourceSkinWeight.InfluenceBones[InfluenceIndex], SourceSkinWeight.InfluenceWeights[InfluenceIndex]});
					TargetWeightArray.Add({TargetVertex.InfluenceBones[InfluenceIndex], TargetVertex.InfluenceWeights[InfluenceIndex]});
				}

				Algo::Sort(SourceWeightArray);
				Algo::Sort(TargetWeightArray);
			
			
				for (int32 InfluenceIndex = 0; InfluenceIndex < NumInfluences; ++InfluenceIndex)
				{
					if (SourceWeightArray[InfluenceIndex] != TargetWeightArray[InfluenceIndex])
					{
						bIsDifferent = true;
						break;
					}
