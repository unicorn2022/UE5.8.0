	...
	FMeshCardsBuildData& MeshCardsBuildData)
{
	...
	if (ClusteringParams.bDebug)
	{
		UE_LOGF(LogMeshUtilities, Log, "CardGen Mesh:%ls Surfels:%d Clusters:%d WeightedSurfaceCoverage:%f ClusterArea:%f",
			*Context.MeshName,
	...
	FCardRepresentationData& OutData)
{
	...
	if (MaxLumenMeshCards > 0)
	{
		...
		if (TimeElapsed > 1.0f)
		{
			UE_LOGF(LogMeshUtilities, Log, "Finished mesh card build in %.1fs %ls tris:%d surfels:%d",
				TimeElapsed,
				*MeshName,
	...
	return true;
#else
	UE_LOGF(LogMeshUtilities, Warning, "Platform did not set USE_EMBREE, GenerateCardRepresentationData failed.");
	return false;
#endif
}
