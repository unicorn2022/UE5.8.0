...
void EmbreeErrorFunc(void* userPtr, RTCError code, const char* str)
{
	...
	UE_LOGF(LogMeshUtilities, Error, "Embree error: %ls Code=%u", *ErrorString, (uint32)code);
}
...
void MeshRepresentation::SetupEmbreeScene(FString MeshName, bool bGenerateAsIfTwoSided, FEmbreeScene& OutEmbreeScene)
{
	...
	{
		UE_LOGF(LogMeshUtilities, Warning, "Failed to created Embree device for %ls (OUT_OF_MEMORY).", *MeshName);
		FPlatformMemory::OnOutOfMemory(0, 16);
	}
	if (ReturnErrorNewDevice != RTC_ERROR_NONE)
	{
		UE_LOGF(LogMeshUtilities, Warning, "Failed to created Embree device for %ls. Code: %d", *MeshName, (int32)ReturnErrorNewDevice);
		return;
	}
	...
	{
		UE_LOGF(LogMeshUtilities, Warning, "Failed to created Embree scene for %ls (OUT_OF_MEMORY).", *MeshName);
		FPlatformMemory::OnOutOfMemory(0, 16);
	}
	if (ReturnErrorNewScene != RTC_ERROR_NONE)
	{
		UE_LOGF(LogMeshUtilities, Warning, "Failed to created Embree scene for %ls. Code: %d", *MeshName, (int32)ReturnErrorNewScene);
...
bool MeshRepresentation::AddMeshDataToEmbreeScene(FEmbreeScene& EmbreeScene, const FMeshDataForDerivedDataTask& MeshData, bool bIncludeTranslucentTriangles)
{
	...
	{
		UE_LOGF(LogMeshUtilities, Warning, "Provided MeshData for %ls doesn't contain any data.", *EmbreeScene.MeshName);
		return false;
	}
	...
	for (const FNaniteAssemblyNode& Node : MeshData.Nodes)
	{
		...
		{
			UE_LOGF(LogStaticMesh, Warning, "Nanite Assembly node ignored because corresponding part doesn't have a valid representation. See previous line(s) for details.");
		}
	}
	...
	bool bInstantiable)
{
		...
		if (!bDegenerateTriangle)
		{
			...
			for (int32 SectionIndex = 0; SectionIndex < Sections.Num(); SectionIndex++)
			{
				...
				if ((uint32)(TriangleIndex * 3) >= Section.FirstIndex && (uint32)(TriangleIndex * 3) < Section.FirstIndex + Section.NumTriangles * 3)
				{
					...
					{
						UE_LOGF(LogMeshUtilities, Warning, "Missing section data for %ls, section = %d.", *MeshName, SectionIndex);
	...
	if (bInstantiable)
	{
		...
		{
			UE_LOGF(LogMeshUtilities, Warning, "Failed to create instantiable Embree geometry for %ls (OUT_OF_MEMORY).", *MeshName);
			FPlatformMemory::OnOutOfMemory(0, 16);
			//return nullptr; // unreachable code
		}
		if (ReturnError != RTC_ERROR_NONE)
		{
			UE_LOGF(LogMeshUtilities, Warning, "Failed to create instantiable Embree geometry for %ls. Code: %d", *MeshName, (int32)ReturnError);
			return nullptr;
		}
	}
...
const FEmbreeGeometry* FEmbreeScene::AddGeometry(const FEmbreeGeometryAsset* GeometryAsset)
{
	...
	if (ReturnError == RTC_ERROR_OUT_OF_MEMORY)
	{
		UE_LOGF(LogMeshUtilities, Warning, "Failed to add geometry to Embree scene for %ls (OUT_OF_MEMORY).", *MeshName);
		FPlatformMemory::OnOutOfMemory(0, 16);
		//return nullptr; // unreachable code
	}
	if (ReturnError != RTC_ERROR_NONE)
	{
		UE_LOGF(LogMeshUtilities, Warning, "Failed to add geometry to Embree scene for %ls. Code: %d", *MeshName, (int32)ReturnError);
		return nullptr;
	}
...
const FEmbreeGeometry* FEmbreeScene::AddGeometryInstance(const FEmbreeGeometryAsset* GeometryAsset, const FMatrix44f& Transform)
{
	...
	if (ReturnError == RTC_ERROR_OUT_OF_MEMORY)
	{
		UE_LOGF(LogMeshUtilities, Warning, "Failed to add geometry instance to Embree scene for %ls (OUT_OF_MEMORY).", *MeshName);
		FPlatformMemory::OnOutOfMemory(0, 16);
		//return nullptr; // unreachable code
	}
	if (ReturnError != RTC_ERROR_NONE)
	{
		UE_LOGF(LogMeshUtilities, Warning, "Failed to add geometry instance to Embree scene for %ls. Code: %d", *MeshName, (int32)ReturnError);
		return nullptr;
	}
...
void FEmbreeScene::Commit()
{
	...
	if (ReturnError == RTC_ERROR_OUT_OF_MEMORY)
	{
		UE_LOGF(LogMeshUtilities, Warning, "Failed to commit Embree scene for %ls (OUT_OF_MEMORY).", *MeshName);
		FPlatformMemory::OnOutOfMemory(0, 16);
	}
	if (ReturnError != RTC_ERROR_NONE)
	{
		UE_LOGF(LogMeshUtilities, Warning, "Failed to commit Embree scene for %ls. Code: %d", *MeshName, (int32)ReturnError);
		return;
	}
#endif
}