...
public:
	...
	*/
	...
	/**
	 * Predict which bones bone reduction will remove for a given LOD and compute the reparent map.
	 * This is a read-only operation — it does not modify the mesh. Used by the unified bone map
	 * builder to determine reparent targets that need unified bone slots.
	 *
	 * @param SkeletalMesh		Mesh to inspect
	 * @param LODIndex			LOD to query
	 * @param OutBonesToReplace	Map of [removed bone index -> reparent target bone index]
	 *
	 * @return true if any bones would be removed
	 */
	virtual bool GetBoneReductionData(const USkeletalMesh* SkeletalMesh, int32 LODIndex, TMap<uint16, uint16>& OutBonesToReplace) const = 0;
	...
	virtual void GenerateRuntimeSkinWeightData(
		...
		const FSkeletalMeshUnifiedBoneIndices* UnifiedBoneIndices = nullptr) const = 0;
