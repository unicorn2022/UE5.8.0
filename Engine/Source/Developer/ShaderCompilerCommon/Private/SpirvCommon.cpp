...
// Scan the early sections of the SPIRV to find the used members of the specified structure
SHADERCOMPILERCOMMON_API TArray<FSpirvStructureMemberData> ParseSpirvStructureMembers(const FSpirv& Spirv, FAnsiStringView StructureName)
{
	uint32_t StructureId = MAX_uint32;

	TArray<FSpirvStructureMemberData> Members;

	TMap<uint32, uint32> TypeSizes;

	bool bReachedBody = false;
	for (FSpirvConstIterator Iter = Spirv.begin(); Iter != Spirv.end() && !bReachedBody; ++Iter)
	{
		switch (Iter.Opcode())
		{

		// Use OpName to find the TypeId of the two structs we're interested in
		case SpvOpName:
		{
			// Operands:
			// 1 - Result Id
			...
			if (StructureId == MAX_uint32)
			{
				FAnsiStringView NameStr = FAnsiStringView(Iter.OperandAsString(2));
				if (NameStr.EndsWith(StructureName))
				{
					StructureId = Iter.Operand(1);
				}
			}
		...
		// Grab the OpMemberName for each member of the loose globals
		case SpvOpMemberName:
		{
			...
			if (Iter.Operand(1) == StructureId)
			{
				const uint32_t Index = Iter.Operand(2);
				if (Index >= (uint32)Members.Num())
				{
					Members.SetNum(Index + 1);
				}
				Members[Index].Name = FString(Iter.OperandAsString(3));
			}
			break;
		}

		// Take down the offset of each member (unused members aren't decorated)
		case SpvOpMemberDecorate:
		{
			// Operands:
			// 1 - Type Id
			...
			if (Iter.Operand(1) == StructureId)
			{
				if (Iter.Operand(3) == SpvDecorationOffset)
				{
					const uint32_t Index = Iter.Operand(2);
					Members[Index].Offset = Iter.Operand(4);
				}
			}
			break;
		}
		...
		case SpvOpTypeFloat:
		{
			...
			// 1 - Type Id
			// 2 - Bit width
			// 3 - Signedness (for ints)

			TypeSizes.Add(Iter.Operand(1), Iter.Operand(2) / 8);
			break;
		}
		...
		case SpvOpTypeVector:
		{
			...
			if (TypeSizes.Contains(Iter.Operand(1)))
			{
				const uint32 ComponentSize = TypeSizes[Iter.Operand(2)];
				TypeSizes.Add(Iter.Operand(1), Iter.Operand(3) * ComponentSize);
			}
			break;
		}
		...
		case SpvOpTypeStruct:
		{
			// Operands:
			// 1 - Type Id
			// 2,3,n - Member 0,1,n Type

			if (Iter.Operand(1) == StructureId)
			{
				const uint32 OperandStartIndex = 2;
				for (uint32 OperandIndex = OperandStartIndex; OperandIndex < Iter.WordCount(); ++OperandIndex)
				{
					Members[OperandIndex - OperandStartIndex].Size = TypeSizes[Iter.Operand(OperandIndex)];
				}
			}
			break;
		}

		// When the body starts, that means there's no more useful information for us
		case SpvOpFunction:
			bReachedBody = true;
			break;
		}
