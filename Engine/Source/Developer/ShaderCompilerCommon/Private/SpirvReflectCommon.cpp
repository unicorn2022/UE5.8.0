...
void FSpirvReflectBindings::GatherPushConstants(const spv_reflect::ShaderModule& SpirvReflection)
{
	// Enumerate all interface variables before filtering out push constants.
	uint32 NumVariables = 0;
	SpvReflectResult SpvResult = SpirvReflection.EnumerateEntryPointInterfaceVariables(SpirvReflection.GetEntryPointName(), &NumVariables, nullptr);
	check(SpvResult == SPV_REFLECT_RESULT_SUCCESS);

	if (NumVariables > 0)
	{
		TArray<SpvReflectInterfaceVariable*> AllVariables;
		AllVariables.SetNum(NumVariables);

		SpvResult = SpirvReflection.EnumerateEntryPointInterfaceVariables(SpirvReflection.GetEntryPointName(), &NumVariables, AllVariables.GetData());
		check(SpvResult == SPV_REFLECT_RESULT_SUCCESS);

		// Extract only the variables with the Push Constant storage class.
		for (SpvReflectInterfaceVariable* Variable : AllVariables)
		{
			if (Variable && Variable->storage_class == SpvStorageClassPushConstant)
			{
				PushConstants.Emplace(Variable);
			}
		}
	}
}
