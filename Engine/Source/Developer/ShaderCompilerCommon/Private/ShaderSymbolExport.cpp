...
void FShaderSymbolExport::Initialize()
{
	...
	if (bExportShaderSymbols)
	{
		UE_LOGF(LogShaderSymbolExport, Display, "Shader symbol export enabled. Output directory: \"%ls\"", *ExportPath);
		if (ZipWriter)
		{
...
void FShaderSymbolExport::WriteSymbolData(const FString& Filename, const FString& DebugData, TConstArrayView<uint8> Contents)
{
	...
	// Read phase: early-out for shaders already exported, without acquiring the write lock.
	{
		FRWScopeLock ReadLock(SymbolWriteLock, SLT_ReadOnly);
		if (ExportedShaders.Contains(Filename))
		{
			if (!bDeterministicSymbols)
			{
				// DuplicateSymbols is only incremented in the read phase on early return, because the write phase will also increment.
				// Remember that this block may fail to detect a duplicate if two threads race to insert.
			...
			const FSymbolFileInfo* PrevExportedSymbolInfo = ExportedSymbolInfo.Find(Filename);
			...
			// Hash is lower; fall through to write phase to overwrite
		}
	}

	// Write phase: first insertion, or deterministic overwrite with a lower hash.
	// Re-check after acquiring the write lock to handle races on first insertion.
	bool bShouldWriteSymbols = ShouldWriteShaderSymbols(ShaderFormat) && Contents.Num();
	bool bShouldWriteToZip = ZipWriter != nullptr;
	{
		FRWScopeLock WriteLock(SymbolWriteLock, SLT_Write);

		bool bAlreadyInSet = false; // Two threads may be trying to insert concurrently
		...
		{
			UE_LOGF(LogShaderSymbolExport, Display, "Shader symbols export size: %.2f MB, count: %llu",
	...
	if (bShouldWriteSymbols && !bShouldWriteToZip)
	{
		// In deterministic mode, files may be overwritten.
		// If a thread wants to overwrite a file while the previous is still being written,
		// we need to wait for the first one to finish.
		WriteFile(Filename, Contents, bDeterministicSymbols);
	}
}

void FShaderSymbolExport::WriteFile(const FString& Filename, const TConstArrayView<uint8>& Contents, bool bAllowRetry)
{
	...
	{
		FFileOpenResult FileResult = PlatformFile.OpenWrite(*OutputPath, IPlatformFile::EOpenWriteFlags::None);

		const int MaxRetries = 10;
		int Retries = 0;
		while (FileResult.HasError() && bAllowRetry && (Retries++ < MaxRetries))
		{
			FPlatformProcess::Sleep(0.0f);
			FRWScopeLock WriteLock(SymbolWriteLock, SLT_Write);
			FileResult = PlatformFile.OpenWrite(*OutputPath, IPlatformFile::EOpenWriteFlags::None);
		}

		if (FileResult.HasError())
		{
			FFileSystemError& Error = FileResult.GetError();
			FString ErrorMessage = Error.GetMessage();
			UE_LOGF(LogShaderSymbolExport, Error, "Failed to open shader symbols file \"%ls\":", *OutputPath);
			UE_LOGF(LogShaderSymbolExport, Error, "%ls", *ErrorMessage);
			return;
		}

		TUniquePtr<IFileHandle> File = FileResult.StealValue();
		if (!File->Write(Contents.GetData(), Contents.Num()))
		{
			UE_LOGF(LogShaderSymbolExport, Error, "Failed to write shader symbols \"%ls\".", *OutputPath);
		}
	}
}
	...
	if (ShaderInfos.Num())
	{
		if (InfoFilePath.Len())
		{
			...
			if (Writer.IsValid())
			{
				...
				UE_LOGF(LogShaderSymbolExport, Display, "%d total shaders, %d shaders after platform deduplication. %d duplicates (%4.1f%%).",
...
#else
		UE_CLOGF(!ZipsToMergeIn.IsEmpty(), LogShaderSymbolExport, Error,
			"Cannot merge zips from multiprocess instances in %ls; merging is only available in editor.", *ExportPath);
#endif
	}
	ZipWriter.Reset();
}
