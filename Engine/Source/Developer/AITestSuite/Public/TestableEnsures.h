...
#define aiTestableEnsureMsgf(InExpression, InFormat, ... ) \
	(LIKELY(!!(InExpression)) || ([&]() \
		{ \
...
#define aiTestableCheckf(InExpression, InFormat, ... ) \
	if (UNLIKELY(UE::AITestSuite::TestsInProgress > 0)) \
	{ \
		if (!(InExpression))\
		{ \
...
#define aiTestableCheckfReturn(InExpression, ReturnExpression, InFormat, ... ) \
	if (UNLIKELY(UE::AITestSuite::TestsInProgress > 0)) \
	{ \
		if (!(InExpression)) \
		{ \
