...
// Enable to start logging debug output from curl
#define UE_ENABLE_S3_DEBUGGING 0
...
#if UE_ENABLE_S3_DEBUGGING

int DebugCallback(CURL* Handle, curl_infotype Type, char* Data, size_t Size, void* UserData)
{
	FString Message(Size, Data);
	Message.TrimEndInline();

	switch (Type)
	{
		case CURLINFO_TEXT:
			UE_LOGF(LogS3Client, Display, "Info: %ls", *Message);
			break;
		case CURLINFO_HEADER_OUT:
			UE_LOGF(LogS3Client, Display, "Send Header: %ls", *Message);
			break;
		case CURLINFO_HEADER_IN:
			UE_LOGF(LogS3Client, Display, "Recv Header: %ls", *Message);
			break;
		default:
			break;
	}
...
FS3Response FS3Client::FS3Request::Perform(EMethod Method, const ANSICHAR* Url, FSharedBuffer Body)
{
...
#if UE_ENABLE_S3_DEBUGGING
	curl_easy_setopt(Curl, CURLOPT_DEBUGFUNCTION, S3Client::DebugCallback);
	...
	if (CurlResult != CURLE_OK)
	{
		UE_LOGF(LogS3Client, Error, "%s", ErrorBuffer);
	}
