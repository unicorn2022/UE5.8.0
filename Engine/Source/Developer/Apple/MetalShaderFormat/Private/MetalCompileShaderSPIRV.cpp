	...
	const FString& MinOSVersion)
{
...
#if PLATFORM_MAC || PLATFORM_WINDOWS
	{
		...
		if (CompilerContext.CompileHlslToSpirv(Options, SpirvData))
		{
			...
			TArray<SpvExecutionMode> ExecutionModes;
			...
			
			{
				...
				{
					const SpvReflectEntryPoint* EntryPoint = spvReflectGetEntryPoint(&Reflection.GetShaderModule(), Reflection.GetEntryPointName());
					if (EntryPoint)
					{
						check(EntryPoint->shader_stage == SPV_REFLECT_SHADER_STAGE_COMPUTE_BIT);
						CCHeaderWriter.WriteNumThreads(EntryPoint->local_size.x, EntryPoint->local_size.y, EntryPoint->local_size.z);
					}
				}
			}
			...
			if (Count > 0)
			{
				...
				{
					const FString BindingName(Binding->name);

					if (Binding->resource_type != SPV_REFLECT_RESOURCE_FLAG_CBV && ResourceTable.Contains(BindingName))
					{
						ResourceBindings.Add(Binding);
						
						FMetalResourceTableEntry Entry = ResourceTable.FindRef(BindingName);
					...
					if (Binding->resource_type == SPV_REFLECT_RESOURCE_FLAG_CBV && Binding->accessed && TableNames.Contains(BindingName))
					{
				...
				if (IABTier == 1)
				{
					...
					{
						FMetalResourceTableEntry* Entry = ResourceTable.Find(FString(Binding->name));
		...
		if (Result)
		{
			...
			switch (VersionEnum)
			{
				...
				default:
				{
					UE_LOGF(LogShaders, Warning, "Metal Shader Version Unsupported, switching to default 2.2"); //EMacMetalShaderStandard::MacMetalSLStandard_Minimum
					TargetDesc.Version = 20200;
					break;
				}
#else
				...
				default:
				{
					UE_LOGF(LogShaders, Warning, "Metal Shader Version Unsupported, switching to default 2.4"); //EIOSMetalShaderStandard::IOSMetalSLStandard_Minimum
					TargetDesc.Version = 20400;
					break;
				}
#endif
		...
		else
		{
			if (Options.bEnableFMAPass)
			{
				...
				size_t IncludePos = ResultsTargetDataAsString.find(IncludeString);
				if (IncludePos != std::string::npos)
					ResultsTargetDataAsString.replace(IncludePos, IncludeString.length(), FMADefine);
			}

			// We calculate the CRC of the metal source code before adding comments
			CRCLen = ResultsTargetDataAsString.length();
			CRC = FCrc::MemCrc_DEPRECATED(ResultsTargetDataAsString.c_str(), CRCLen);
