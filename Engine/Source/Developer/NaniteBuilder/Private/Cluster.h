...
struct FVertexFormat
{
	...
	FORCEINLINE bool Matches( const FVertexFormat& Other ) const
	{
		return
			...
			bHasCurveRadius		== Other.bHasCurveRadius &&
			bHasColors			== Other.bHasColors;
	}

	FORCEINLINE uint8 GetCurveRadiusOffset() const
	{
	...
	{
		return GetCurveRadiusOffset() + ( bHasCurveRadius ? 4 : 0 );
	}
...
public:
	...
	uint32	FindOrAddHash( const float* Vert, uint32 VertIndex, FHashTable& HashTable ) const;
	void	Sanitize();
	...
	FORCEINLINE float&				GetCurveRadius(		uint32 VertIndex )			{ return *Fetch< float			>( VertIndex, CurveRadiusOffset); }
	...
	FORCEINLINE const float&		GetCurveRadius(		uint32 VertIndex ) const	{ return *Fetch< float			>( VertIndex, CurveRadiusOffset); }
...
{
	CurveRadiusOffset	= Format.GetCurveRadiusOffset();
...
public:
	...
	float		SimplifyTriangles( const FClusterDAG& DAG, uint32 TargetNumTris, float TargetError = 0.0f, uint32 LimitNumTris = 0, const FRayTracingFallbackBuildSettings* RayTracingFallbackBuildSettings = nullptr );
	float		SimplifyCurves( const FClusterDAG& DAG, uint32 TargetNumCurves, uint32 TargetNumSegments, float TargetError = 0.0f, uint32 LimitNumCurves = 0, const FRayTracingFallbackBuildSettings* RayTracingFallbackBuildSettings = nullptr );
	...
	FORCEINLINE bool IsVoxel() const { return NumTris == 0 && NumCurves == 0; }
	FORCEINLINE bool IsCurve() const { return NumTris == 0 && NumCurves > 0; }
...
private:
	...
	void		VoxelsToBricks( TSparseMap< FIntVector3, uint32 >& VoxelMap );

public:
	friend FArchive& operator<<(FArchive& Ar, FCluster& Cluster);

	static const uint32	ClusterSize = 128;
	static const uint32 MaxNumSegmentPerCluster = NANITE_MAX_CLUSTER_VERTICES;
	...
	uint32				NumCurves = 0;
	TArray<FCurveIndex> CurveIndexes;
	uint32				CurveFadingIndex = 0;

#if RAY_TRACE_VOXELS
	TArray< FIntVector3 >	ExtraVoxels;
#endif

	TArray< uint8 >		ExternalEdges;
	...
	float		ParentLODError = 0.f;
	float		ParentSphereBoundRadius = 0.f;
