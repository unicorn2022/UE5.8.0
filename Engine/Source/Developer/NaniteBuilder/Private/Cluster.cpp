...
template< bool bHasTangents, bool bHasCurveRadius, bool bHasColors >
void CorrectAttributes( float* Attributes )
{
	...
	if ( bHasCurveRadius )
	{
		float& CurveRadius = *reinterpret_cast< float* >( AttributesPtr );
		AttributesPtr += 1;
	}
...
static CorrectAttributesFunction* CorrectAttributesFunctions[ 2 ][ 2 ][ 2 ] =	// [ bHasTangents ][ bHasCurveRadius ][ bHasColors ]
{
	{
		{ CorrectAttributes<false, false, false>,	CorrectAttributes<false, false, true>	},
		{ CorrectAttributes<false, true,  false>,	CorrectAttributes<false, true,  true>	}
	},
	{
		{ CorrectAttributes<true, false, false>,	CorrectAttributes<true, false, true>	},
		{ CorrectAttributes<true, true,  false>,	CorrectAttributes<true, true,  true>	}
	}
};

static void CopyVertices(
	const FConstMeshBuildVertexView& InVerts,
	FVertexArray& OutVerts,
	int32 NewIndex, 
	int32 OldIndex)
{
	OutVerts.GetPosition( NewIndex ) = InVerts.Position[OldIndex];
	OutVerts.GetNormal( NewIndex ) = InVerts.TangentZ[OldIndex];
	
	if( OutVerts.Format.bHasTangents )
	{
		const float TangentYSign = ((InVerts.TangentZ[OldIndex] ^ InVerts.TangentX[OldIndex]) | InVerts.TangentY[OldIndex]);
		OutVerts.GetTangentX( NewIndex ) = InVerts.TangentX[OldIndex];
		OutVerts.GetTangentYSign( NewIndex ) = TangentYSign < 0.0f ? -1.0f : 1.0f;
	}
	
	if( OutVerts.Format.bHasCurveRadius )
	{
		OutVerts.GetCurveRadius( NewIndex ) = InVerts.CurveRadius[OldIndex];
	}

	if( OutVerts.Format.bHasColors )
	{
		OutVerts.GetColor( NewIndex ) = InVerts.Color[OldIndex].ReinterpretAsLinear();
	}

	if( OutVerts.Format.NumTexCoords > 0 )
	{
		FVector2f* UVs = OutVerts.GetUVs( NewIndex );
		for( uint32 UVIndex = 0; UVIndex < OutVerts.Format.NumTexCoords; UVIndex++ )
		{
			UVs[UVIndex] = InVerts.UVs[UVIndex][OldIndex];
		}
	}

	if( OutVerts.Format.NumBoneInfluences > 0 )
	{
		FVector2f* BoneInfluences = OutVerts.GetBoneInfluences(NewIndex);
		for (uint32 Influence = 0; Influence < OutVerts.Format.NumBoneInfluences; Influence++)
		{
...
static void SanitizeAndCorrectAttributes(FVertexArray& OutVerts)
{
	OutVerts.Sanitize();

	for( uint32 VertIndex = 0; VertIndex < OutVerts.Num(); VertIndex++ )
	{
		// Make sure this vertex is valid from the start
		CorrectAttributesFunctions[ OutVerts.Format.bHasTangents ][ OutVerts.Format.bHasCurveRadius ][ OutVerts.Format.bHasColors ]( OutVerts.GetAttributes( VertIndex ) );
	}
}
	...
	TArrayView< const uint32 > SortedCurveIndexes)
{
	...
	Verts.Reserve( FCluster::MaxNumSegmentPerCluster );
	CurveIndexes.Reserve( NumCurves );
	MaterialIndexes.Reserve( NumCurves );
	ExternalEdges.Empty();
	NumExternalEdges = 0;

	check(InMaterialIndexes.Num() == InCurveIndexes.Num());

	TMap< uint32, uint32 > OldToNewIndex;
	OldToNewIndex.Reserve( NumCurves );

	// Sanity check
	// All curves within a should have the same amount of segments
	const FClusterCurveStats Stats = GetClusterCurveStats(InCurveIndexes, SortedCurveIndexes, Begin, End);
	check(Stats.MinPointPerCurve == Stats.MaxPointPerCurve);
	check(Stats.MinPointPerCurve >= 2);

	for( uint32 i = Begin; i < End; i++ )
	{
		const uint32 SortedCurveIndex = SortedCurveIndexes[i];
		const FCurveIndex OldCurveIndex = InCurveIndexes[SortedCurveIndex];

		uint32 NewCurveOffset = 0;
		for (uint32 VertIt=0;VertIt<OldCurveIndex.Count; ++VertIt)
		{
			const uint32 OldIndex = OldCurveIndex.Offset + VertIt;
			const uint32 NewIndex = Verts.AddUninitialized();
			if (VertIt == 0)
			{
				NewCurveOffset = NewIndex;
			}

			CopyVertices(InVerts, Verts, NewIndex, OldIndex);
		}
		FCurveIndex NewCurveIndex; 
		NewCurveIndex.Offset = NewCurveOffset; 
		NewCurveIndex.Count = OldCurveIndex.Count;

		CurveIndexes.Add(NewCurveIndex);
		MaterialIndexes.Add(InMaterialIndexes[SortedCurveIndex]);
	}

	SanitizeAndCorrectAttributes(Verts);
	Bound();

	check(Verts.Num() <= FCluster::MaxNumSegmentPerCluster)
}
	...
	const FAdjacency& Adjacency )
{
	...
	for( uint32 i = Begin; i < End; i++ )
	{
		...
		for( uint32 k = 0; k < 3; k++ )
		{
			...
			ExternalEdges.Add( (uint8)FMath::Min<int32>(AdjCount, MAX_uint8) );
	...
	: MipLevel( SrcCluster.MipLevel )
{
	...
	// Curves
	else if (SrcCluster.NumCurves)
	{
		check(SortedIndexes.Num() > 0);
		check(Adjacency.Direct.Num() == 0);
		check(SortedTo.Num() == 0);
		NumTris = 0;
		NumCurves = NumElements;
	
		Verts.Reserve( NumElements * 32u );
		CurveIndexes.Reserve( NumElements );
		MaterialIndexes.Reserve( NumElements );
		Indexes.Empty();
		ExternalEdges.Empty();
		NumExternalEdges = 0;
		...
		for( uint32 i = Begin; i < End; i++ )
		{
			const uint32 SortedOldCurveIndex = SortedIndexes[i];
			const FCurveIndex OldCurveIndex = SrcCluster.CurveIndexes[SortedOldCurveIndex];

			uint32 NewCurveOffset = 0;
			for (uint32 VertIt = 0; VertIt < OldCurveIndex.Count; ++VertIt)
			{
				const uint32 OldIndex = OldCurveIndex.Offset + VertIt;
				const uint32 NewIndex = Verts.Add( &SrcCluster.Verts.GetPosition( OldIndex ) );
				if (VertIt == 0)
				{
					NewCurveOffset = NewIndex;
				}
			...
			MaterialIndexes.Add( SrcCluster.MaterialIndexes[ SortedOldCurveIndex ] );
		}

		// Sanity check for curve clusters
		const FClusterCurveStats Stats = GetClusterCurveStats(CurveIndexes);
		check(Stats.MinPointPerCurve == Stats.MaxPointPerCurve);
		check(Stats.MinPointPerCurve >= 2);
	}
	// Voxels
	else
	{
		...
		for( uint32 i = Begin; i < End; i++ )
		{
			...
			uint32 NumVoxels = FMath::CountBits64( Brick.VoxelMask );
...
FCluster::FCluster( const FClusterDAG& DAG, TArrayView< const FClusterRef > Children )
{
	...
	for( FClusterRef ChildRef : Children )
	{
		...
		Verts.Format.bHasCurveRadius	|= Child.Verts.Format.bHasCurveRadius;
	...
	Indexes.Reserve( 3 * NumTris );
	MaterialIndexes.Reserve( FMath::Max(NumTris, NumCurves) );
	ExternalEdges.Reserve( 3 * NumTris );
	...
	for( FClusterRef ChildRef : Children )
	{
		...
		if( ChildRef.IsInstance() )
		{
			...
			auto TransformVert = [&]( uint32 VertIndex )
			{
				...
				{
					float& CurveRadius = Verts.GetCurveRadius(VertIndex);
					CurveRadius *= Transform.GetMaximumAxisScale();
				}
				...
				CorrectAttributesFunctions[ Verts.Format.bHasTangents ][ Verts.Format.bHasCurveRadius ][ Verts.Format.bHasColors ]( Verts.GetAttributes( VertIndex ) );
			};

			if( bAllowVoxels && Child.IsVoxel() )
			{
			...
			else
			{
				...
				// Curves
				for( FCurveIndex CurveIt : Child.CurveIndexes)
				{
					FCurveIndex& NewCurveIt = CurveIndexes.AddDefaulted_GetRef();
					NewCurveIt.Count = CurveIt.Count;
					for (uint32 PointIt = 0; PointIt < CurveIt.Count; PointIt++)
					{
						uint32 NewIndex = AddVertMismatched( CurveIt.Offset + PointIt, Child, VertHashTable, TransformVert );
						if (PointIt == 0)
						{
							NewCurveIt.Offset = NewIndex;
						}
					}
				}
			}
		...
		else
		{
			...
			else if( Verts.Format.Matches( Child.Verts.Format ) )
			{
				...
				// Curves
				for(const FCurveIndex& CurveIt : Child.CurveIndexes)
				{
					// Compare only the curve's root to check if the curve already exists
					const float* ChildVert0 = (float*)&Child.Verts.GetPosition(CurveIt.Offset + 0/*PointIt*/);
					uint32 NewIndex = Verts.FindOrAddHash(ChildVert0, Verts.Num(), VertHashTable);
					if (NewIndex == Verts.Num())
					{
						Verts.Add(ChildVert0);

						FCurveIndex& NewCurveIt = CurveIndexes.AddDefaulted_GetRef();
						NewCurveIt.Count = CurveIt.Count;
						NewCurveIt.Offset = NewIndex;
						for (uint32 PointIt = 1; PointIt < CurveIt.Count; PointIt++)
						{
							const float* ChildVert = (float*)&Child.Verts.GetPosition(CurveIt.Offset + PointIt);
							NewIndex = Verts.FindOrAddHash(ChildVert, Verts.Num(), VertHashTable);
							check(NewIndex == Verts.Num());
					...
					{
						check(false); // We should't reusing vertices it should all be new curves
					}
				}
			}
			else
			{
				...
				for( FCurveIndex CurveIt : Child.CurveIndexes)
				{
					...
					for (uint32 PointIt = 0; PointIt < CurveIt.Count; PointIt++)
					{
						uint32 NewIndex = AddVertMismatched( CurveIt.Offset + PointIt, Child, VertHashTable, [&]( uint32 NewIndex ){} );
						if (PointIt == 0)
						{
							NewCurveIt.Offset = NewIndex;
						}
		...
		if( Child.NumTris > 0 || Child.NumCurves > 0)
		{
	...
	for( int32 EdgeIndex = 0; EdgeIndex < ExternalEdges.Num(); EdgeIndex++ )
	{
		...
		ExternalEdges[ EdgeIndex ] = (uint8)FMath::Min<int32>(AdjCount, MAX_uint8);
		NumExternalEdges += AdjCount != 0 ? 1 : 0;
	}

	ensure( NumTris == Indexes.Num() / 3 );
	ensure( NumCurves == CurveIndexes.Num());
	check( MaterialIndexes.Num() > 0 );
...
float FCluster::SimplifyCurves( const FClusterDAG& DAG, uint32 TargetNumCurves, uint32 TargetNumSegments, float TargetError, uint32 LimitNumCurves, const FRayTracingFallbackBuildSettings* RayTracingFallbackBuildSettings )
{
	if( ( TargetNumCurves >= NumCurves && TargetNumSegments >= Verts.Num() && TargetError == 0.0f) || LimitNumCurves >= NumCurves)
	{
		return 0.0f;
	}

	TArray<FVector3f> Roots;
	Roots.Reserve(NumCurves);
	for (const FCurveIndex& CurveIt : CurveIndexes)
	{
		Roots.Add(Verts.GetPosition(CurveIt.Offset));
	}

	// Compute all distances between curves' roots
	struct FDistancePair
	{
		float Dist2;
		uint32 i;
		uint32 j;
	};
	auto ComputeDistancePairs = [Roots](const TBitArray<>& InActiveCurves, TArray<FDistancePair>& Out)
	{
		const uint32 NumRoots = Roots.Num();
		Out.Reserve(NumRoots * NumRoots / 2);
		...
		for (uint32 i = 0; i < NumRoots; i++)
		{
			if (InActiveCurves[i])
			{
				for (uint32 j = 0; j < i; j++)
				{
					if (InActiveCurves[j])
					{
						FDistancePair& Pair = Out.AddDefaulted_GetRef();
						Pair.Dist2 = FVector3f::DistSquared(Roots[i], Roots[j]);
						Pair.i = i;
						Pair.j = j;
					}
				}
		...
		Out.Sort([](const FDistancePair& A, const FDistancePair& B)
		{
	...
	// Simplifier
	// TODO_CURVE: Improve - For now very simple scheme: Trim curves to match target curve count using greedy decimation based on root distance
	TBitArray<> ActiveCurves;
	ActiveCurves.Init(true, NumCurves);
	uint32 NumActiveCurves = NumCurves;
	uint32 NumActiveSegments = Verts.Num();

	// Trim curves in batch of 32
	float MaxDist2 = 0;
	while (NumActiveCurves > TargetNumCurves || NumActiveSegments > TargetNumSegments)
	{
		TArray<FDistancePair> DistancePairs;
		ComputeDistancePairs(ActiveCurves, DistancePairs);

		uint32 TrimCount = 0;
		for (const FDistancePair& D : DistancePairs)
		{
			if (ActiveCurves[D.i] && ActiveCurves[D.j])
			{
				MaxDist2 = FMath::Max(MaxDist2, D.Dist2);
				ActiveCurves[D.j] = false;

				NumActiveSegments = NumActiveSegments - CurveIndexes[D.j].Count;
				++TrimCount;
				--NumActiveCurves;
			}

			if (TrimCount == 32 || (NumActiveCurves <= TargetNumCurves && NumActiveSegments <= TargetNumSegments))
			{
				break;
			}
		}
	...
	// Extract active curves
	uint32 RemainingNumVerts = 0;
	TArray<FCurveIndex> RemainingSrcCurves;
	RemainingSrcCurves.Reserve(TargetNumCurves);
	TArray< int32 > RemainingMaterialIndexes;
	RemainingMaterialIndexes.Reserve(TargetNumCurves);
	for (uint32 CurveIt = 0; CurveIt < NumCurves; ++CurveIt)
	{
		if (ActiveCurves[CurveIt])
		{			
			RemainingSrcCurves.Add(CurveIndexes[CurveIt]);
			RemainingMaterialIndexes.Add(MaterialIndexes[CurveIt]);
			RemainingNumVerts += CurveIndexes[CurveIt].Count;
		}
	}
	check(uint32(RemainingSrcCurves.Num()) <= TargetNumCurves);
	check(RemainingNumVerts <= TargetNumSegments);

	// TODO_CURVE: temp metric. Use the maximum roots' distance among the removed curves. Try different metric like average coverage with unscaled curves' radius.
	float MaxErrorSqr = MaxDist2;

	// Copy actives vertices
	FVertexArray RemainingVerts(Verts.Format);
	RemainingVerts.AddUninitialized(RemainingNumVerts);
	uint32 DstVertIndex = 0;
	CurveIndexes.Empty(RemainingSrcCurves.Num());
	for (const FCurveIndex& CurveIt : RemainingSrcCurves)
	{
		FCurveIndex& DstCurveIndex = CurveIndexes.AddDefaulted_GetRef();
		DstCurveIndex.Offset = DstVertIndex;
		DstCurveIndex.Count  = CurveIt.Count; 
		for (uint32 PointIt = 0; PointIt < CurveIt.Count; ++PointIt)
		{
			const uint32 SrcVertIndex = CurveIt.Offset + PointIt;
			RemainingVerts.GetPosition(DstVertIndex) = Verts.GetPosition(SrcVertIndex);
			RemainingVerts.CopyAttributes(DstVertIndex, SrcVertIndex, Verts);
			++DstVertIndex;
		}		
	}

	Verts = RemainingVerts;
	MaterialIndexes = RemainingMaterialIndexes;
	NumCurves = CurveIndexes.Num();
...
float FCluster::SimplifyTriangles( const FClusterDAG& DAG, uint32 TargetNumTris, float TargetError, uint32 LimitNumTris, const FRayTracingFallbackBuildSettings* RayTracingFallbackBuildSettings )
{
	...
	*WeightsPtr++ = 1.0f;
	...
	// CurveRadius is only supported on curve primitive
	check(!Verts.Format.bHasCurveRadius);
	...
	TMap< TTuple< FVector3f, FVector3f >, uint8 > LockedEdges;
	...
	Simplifier.SetCorrectAttributes( CorrectAttributesFunctions[ Verts.Format.bHasTangents ][ Verts.Format.bHasCurveRadius ][ Verts.Format.bHasColors ] );
	...
	for( int32 EdgeIndex = 0; EdgeIndex < ExternalEdges.Num(); EdgeIndex++ )
	{
		...
		uint8* AdjCount = LockedEdges.Find( Edge );
		if( AdjCount )
		{
...
uint32 FVertexArray::FindOrAddHash( const float* Vert, uint32 VertIndex, FHashTable& HashTable ) const
{
	...
	const FVertexArray& SrcVerts )
{
	...
	if ( Format.bHasCurveRadius )
	{
		GetCurveRadius(DstVertIndex) = SrcVerts.Format.bHasCurveRadius ? SrcVerts.GetCurveRadius(SrcVertIndex) : 0.f;
	}
	...
	const FVector3f& Barycentrics )
{
	...
	else
	{
		...
		if( Format.bHasCurveRadius )
		{
			if( SrcVerts.Format.bHasCurveRadius )
			{
				// If Cluster has bHasCurveRadius, it has curves topology. So barycentrics are only interpolated between two adjacent points
				GetCurveRadius( DstVertIndex ) =
					SrcVerts.GetCurveRadius( SrcVertIndexes[0] ) * Barycentrics[0] +
					SrcVerts.GetCurveRadius( SrcVertIndexes[1] ) * Barycentrics[1];
			}
			else
			{
				GetCurveRadius( DstVertIndex ) = 0.f;
			}
		}
		if( Format.bHasColors )
		{
...
void FCluster::Bound()
{
	...
	{
		if (CurveIndex.Count >= 2)
		{
			for (uint32 PointIt = 0; PointIt < CurveIndex.Count-1u; ++PointIt)
			{
				const FVector3f P0 = Verts.GetPosition(CurveIndex.Offset + PointIt + 0);
				const FVector3f P1 = Verts.GetPosition(CurveIndex.Offset + PointIt + 1);
				const float R0 = Verts.GetCurveRadius(CurveIndex.Offset + PointIt + 0);
				const float R1 = Verts.GetCurveRadius(CurveIndex.Offset + PointIt + 1);

				const FVector3f Edge = P1 - P0;
				const float EdgeRadius = (R0 + R1) * 0.5f;

				MaxEdgeLength2 = FMath::Max( MaxEdgeLength2, Edge.SizeSquared() );

				const float SegmentArea = Edge.Length() * EdgeRadius;
...
void FCluster::Voxelize( FClusterDAG& DAG, TArrayView< const FClusterRef > Children, float VoxelSize )
{
	...
	Verts.Format.bHasCurveRadius	|= DAG.bHasCurveRadius;
	Verts.Format.bHasColors			|= DAG.bHasColors;
#endif

	Verts.InitFormat();

	TSparseMap< FIntVector3, uint32 > VoxelMap;
	...
	for( FClusterRef ChildRef : Children )
	{
		...
		else
		{
			...
			for( int32 BrickIndex = 0; BrickIndex < Child.Bricks.Num(); BrickIndex++ )
			{
				...
				uint32 NumVoxels = FMath::CountBits64( Child.Bricks[ BrickIndex ].VoxelMask );
	...
	for( auto& Voxel : VoxelMap )
	{
		...
		if( SampledVoxel.InstanceIndex != NANITE_MAX_ASSEMBLY_TRANSFORMS )
		{
			...
			if( Verts.Format.bHasCurveRadius )
			{
				float& CurveRadius = Verts.GetCurveRadius( NewIndex );
				CurveRadius *= InstanceData.Transform.GetMaximumAxisScale();
			}
...
void FCluster::VoxelsToBricks( TSparseMap< FIntVector3, uint32 >& VoxelMap )
{
...
void FCluster::BuildMaterialRanges()
{
	...
	check( NumCurves == CurveIndexes.Num() );
	...
	{
		TArray<FCurveIndex>	NewCurveIndexes;
		TArray< int32 >		NewMaterialIndexes;
	
		NewCurveIndexes.AddUninitialized( CurveIndexes.Num() );
		NewMaterialIndexes.AddUninitialized( MaterialIndexes.Num() );
	
		for( uint32 NewIndex = 0; NewIndex < NumCurves; NewIndex++ )
		{
			uint32 OldIndex = MaterialElements[ NewIndex ];
			NewCurveIndexes[ NewIndex] = CurveIndexes[ OldIndex];
			NewMaterialIndexes[ NewIndex ] = MaterialIndexes[ OldIndex ];
		}
		Swap( CurveIndexes,		NewCurveIndexes );
		Swap( MaterialIndexes,	NewMaterialIndexes );
	}
	else
	{
		...
		for( int32 NewIndex = 0; NewIndex < MaterialElements.Num(); NewIndex++ )
		{
			...
			uint32 NumVoxels = FMath::CountBits64( OldBrick.VoxelMask );
...
void FVertexArray::Sanitize()
{
	...
	for( uint32 VertexIndex = 0; VertexIndex < NumVerts; VertexIndex++ )
	{
		...
		if( Format.bHasCurveRadius )
		{
			float& CurveRadius = GetCurveRadius( VertexIndex );
			SanitizeFloat( CurveRadius, 0.0f, 10000.0f, 0.0f );
		}
