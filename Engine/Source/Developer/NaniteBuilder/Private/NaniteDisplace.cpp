	...
	TArrayView< FDisplacementMap > DisplacementMaps )
{
	if( DisplacementMaps.IsValidIndex( MaterialIndex ) )
	{
		...
		for( int k = 0; k < 3; k++ )
		{
			...
			UVs[k].X *= (float)DisplacementMaps[ MaterialIndex ].GetSizeX();
			UVs[k].Y *= (float)DisplacementMaps[ MaterialIndex ].GetSizeY();
		}
	...
	const FMeshNaniteSettings& Settings )
{
	...
	// CurveRadius not supported on displaced triangle mesh
