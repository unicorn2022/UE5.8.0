	...
	)
{
	...
	// Now all clusters' pages should be assigned, add instanced clusters pages to their page ranges
	{
		...
		for (uint32 GroupIndex : GroupsWithInstances)
		{
			...
			if (RangeStartIndex > NANITE_PAGE_RANGE_KEY_MAX_INDEX || RangeCount > NANITE_PAGE_RANGE_KEY_MAX_COUNT)
			{
				// we have exceeded the addressable space in the page range lookup or the max representable number of
				// pages/ranges, we must fail to prevent assertion failures
				return false;
			}
