...
static void PackCluster(FPackedCluster& OutCluster, const FCluster& InCluster, const FEncodingInfo& EncodingInfo, bool bHasTangents, bool bHasCurveRadius, uint32 NumTexCoords)
{
	...
	OutCluster.SetNumClusterBoneInfluences(InCluster.IsVoxel() ? EncodingInfo.BoneInfluence.VoxelBoneInfluences.Num() :
	...
	// 7
	// Optionally write curve data as some curve's data are aliased with triangle/voxel data
	if (InCluster.IsCurve())
	{
		const uint32 NumPointsPerCurve = InCluster.NumCurves ? InCluster.Verts.Num() / InCluster.NumCurves : 0u;
		OutCluster.SetNumCurves(InCluster.NumCurves);
		OutCluster.SetNumPointsPerCurve(NumPointsPerCurve);
		OutCluster.SetHasCurveRadius(bHasCurveRadius);
		OutCluster.SetCurveRadiusBits(EncodingInfo.CurveRadiusBits);
		OutCluster.SetCurveRadiusScale(EncodingInfo.CurveRadiusScale);
		OutCluster.SetCurveFadingIndex(InCluster.CurveFadingIndex);
	}
	OutCluster.SetParentLODError(InCluster.ParentLODError);
	OutCluster.SetParentSphereBoundRadius(InCluster.ParentSphereBoundRadius);
}
...
static int32 CalculateQuantizedPositionsUniformGrid(TArray< FCluster >& Clusters, const FMeshNaniteSettings& Settings)
{
	...
	if (PositionPrecision == MIN_int32)
	{
		...
		{
			if (Cluster.MipLevel == 0 && (Cluster.NumTris != 0 || Cluster.NumCurves != 0))
			{
	...
	for (const FCluster& Cluster : Clusters)
	{
		if (Cluster.NumTris == 0 && Cluster.NumCurves == 0)
		{
			continue;
		}
	...
	uint32* OutTotalGPUSize)
{
	...
	ParallelFor(TEXT("NaniteEncode.BuildPages.PF"), NumPages, 1, [&](int32 PageIndex)
	{
		...
		if (bHasSkinning)
		{
			PackedBoneInfluenceHeaders.SetNum(Page.NumClusters);
		}
		...
		ProcessPageClusters(Page, Parts, [&](uint32 LocalClusterIndex, uint32 ClusterIndex)
		{
			...
			const bool bSkinnedCluster = bHasSkinning && Cluster.Verts.Format.NumBoneInfluences > 0;

			FPackedCluster& PackedCluster = PackedClusters[LocalClusterIndex];
			PackCluster(PackedCluster, Cluster, EncodingInfos[ClusterIndex], Cluster.Verts.Format.bHasTangents, Cluster.Verts.Format.bHasCurveRadius, Cluster.Verts.Format.NumTexCoords);
			...
			PackedCluster.SetHasSkinning(bSkinnedCluster);
	...
	UE_LOGF(LogStaticMesh, Log, "WritePages: %u", NumPages);
	UE_LOGF(LogStaticMesh, Log, "  Root: GPU size: %d bytes. %d Pages. %.3f bytes per page (%.3f%% utilization).", TotalRootGPUSize, NumRootPages, (float)TotalRootGPUSize / (float)NumRootPages, (float)TotalRootGPUSize / (float(NumRootPages * NANITE_ROOT_PAGE_GPU_SIZE)) * 100.0f);
	if(NumStreamingPages > 0)
	{
		UE_LOGF(LogStaticMesh, Log, "  Streaming: GPU size: %d bytes. %d Pages (%d with relative encoding). %.3f bytes per page (%.3f%% utilization).", TotalStreamingGPUSize, NumStreamingPages, NumRelativeEncodingPages, (float)TotalStreamingGPUSize / float(NumStreamingPages), (float)TotalStreamingGPUSize / (float(NumStreamingPages * NANITE_STREAMING_PAGE_GPU_SIZE)) * 100.0f);
	}
	else
	{
		UE_LOGF(LogStaticMesh, Log, "  Streaming: 0 bytes.");
	}
	UE_LOGF(LogStaticMesh, Log, "  Page data disk size: %d bytes. Fixup data size: %d bytes.", TotalPageDiskSize, TotalFixupSize);
	UE_LOGF(LogStaticMesh, Log, "  Total GPU size: %d bytes, Total disk size: %d bytes.", TotalPageGPUSize, TotalPageDiskSize + TotalFixupSize);
...
struct FCachedPos
{
	FVector3f Position;
	uint32 ClusterIndex;
	bool operator==(const FCachedPos& In) const { return In.Position == Position; }
};

FORCEINLINE uint32 GetTypeHash(const FCachedPos& In)
{
	return HashCombine(HashCombine(::GetTypeHash(In.Position.X), ::GetTypeHash(In.Position.Y)), ::GetTypeHash(In.Position.Z));
}

struct FPositionCache
{
	TSet<FCachedPos> Positions;
	//TSet<FVector3f> Positions;
	void Reserve(uint32 Num) { Positions.Reserve(Num); }
	void Add(const FVector3f& InPos, uint32 InCIdx) { Positions.Add({InPos, InCIdx}); }
	int32 Exists(FVector3f In) const
	{ 
		int32 Out = INDEX_NONE; 
		if (const FCachedPos* Pos = Positions.Find({In, 0}))
		{ 
			Out = Pos->ClusterIndex;
		} 
		return Out;
	}
};

void BuildCurveHierarchyData(FClusterDAG& ClusterDAG)
{
	auto IsCurve = [&ClusterDAG](const FClusterGroup& InGroup)
	{
		return InGroup.Children.Num() > 0 && InGroup.Children[0].GetCluster(ClusterDAG).IsCurve();
	};

	// Build Curve visibility
	// When a cluster transition to its parent, curves which are not part of the parent, are progressively faded out.
	{
		struct FData
		{
			struct FMipLevelClusters
			{
				int32 MipLevel = -1;
				FPositionCache PositionCache;
				TArray<FClusterRef> ClusterRefs;
			};

			struct FMesh
			{
				TArray<FMipLevelClusters> MipLevelClusters;
			};
			TArray<FMesh> Meshes;
		};

		// Build data structure to build MipLevel position cache and process cluster in parallel
		FData Data;
		Data.Meshes.SetNum(ClusterDAG.MeshInput.Num());
		for (const FClusterGroup& Group : ClusterDAG.Groups)
		{
			if (Group.bRoot && IsCurve(Group))
			{
				FData::FMesh& Mesh = Data.Meshes[Group.MeshIndex];
				Mesh.MipLevelClusters.SetNum(Group.MipLevel + 1);
				for (int32 MipIt = 0; MipIt <= Group.MipLevel; ++MipIt)
				{
					const uint32 RoughEstimateNumClusterGroupPerMip = FMath::DivideAndRoundUp(uint32(ClusterDAG.Clusters.Num()), 1u << MipIt);
					Mesh.MipLevelClusters[MipIt].ClusterRefs.Reserve(RoughEstimateNumClusterGroupPerMip);
					Mesh.MipLevelClusters[MipIt].MipLevel = MipIt;
				}
			}
		}
		for (FClusterGroup& Group : ClusterDAG.Groups)
		{
			if (IsCurve(Group))
			{
				Data.Meshes[Group.MeshIndex].MipLevelClusters[Group.MipLevel].ClusterRefs.Append(Group.Children);
			}
		}

		#define CURVE_USE_GROUP_ERROR 1
		// Each mesh and each group are independent and running parallel should be safe and deterministic.
		const EParallelForFlags ParallelFlags = EParallelForFlags::None;
		for (FData::FMesh& Mesh : Data.Meshes)
		{
			// Fill position cache
			ParallelFor(TEXT("NaniteEncode.CurvePositionCache.PF"), Mesh.MipLevelClusters.Num(), 1, [&](int32 MipLevel)
			{
				FData::FMipLevelClusters& Group = Mesh.MipLevelClusters[MipLevel];
				const uint32 RootCount = Group.ClusterRefs.Num() * 64u; // 64u curves per cluster in average
				Group.PositionCache.Reserve(RootCount);

				for (const FClusterRef& ClusterRef : Group.ClusterRefs)
				{
					const FCluster& Cluster = ClusterRef.GetCluster(ClusterDAG);
					for (const FCurveIndex& CurveIndex : Cluster.CurveIndexes)
					{
						const FVector3f& P = Cluster.Verts.GetPosition(CurveIndex.Offset);
						Group.PositionCache.Add(P, ClusterRef.ClusterIndex);
					}
				} 
			}, ParallelFlags);

			// Fill visibility
			for (FData::FMipLevelClusters& Group : Mesh.MipLevelClusters)
			{
				const bool bIsRoot = Group.MipLevel + 1 == Mesh.MipLevelClusters.Num();
				if (bIsRoot)
				{
					for (const FClusterRef& ClusterRef : Group.ClusterRefs)
					{
						FCluster& Cluster = ClusterRef.GetCluster(ClusterDAG);
						Cluster.CurveFadingIndex = Cluster.CurveIndexes.Num();
						#if !CURVE_USE_GROUP_ERROR
						Cluster.ParentLODError = Cluster.LODError;
						Cluster.ParentSphereBoundRadius = Cluster.SphereBounds.W;
						#endif
					}
				}
				else
				{
					ParallelFor( TEXT("NaniteEncode.CurveVisibleCurve.PF"), Group.ClusterRefs.Num(), 1, [&]( int32 ClusterRefIndex )
					{
						const FPositionCache& Cache = Mesh.MipLevelClusters[Group.MipLevel+1].PositionCache;
						const FClusterRef& ClusterRef = Group.ClusterRefs[ClusterRefIndex];
						
						FCluster& Cluster = ClusterRef.GetCluster(ClusterDAG);
						const uint32 NumCurves = Cluster.CurveIndexes.Num();

						// Split curves in two groups: being present or not in its parents
						TArray<uint32> CurveToShuffle, CurveToCopy;
						CurveToShuffle.Reserve(NumCurves);
						CurveToCopy.Reserve(NumCurves);

						#if !CURVE_USE_GROUP_ERROR
						bool bIsParentDataInitialized = false;
						float ParentLODError = 0;
						float ParentSphereBoundRadius = 0;
						#endif
						for (uint32 CurveIt = 0; CurveIt < NumCurves; ++CurveIt)
						{
							const FCurveIndex& SrcCurveIndex = Cluster.CurveIndexes[CurveIt];
							const FVector3f& P = Cluster.Verts.GetPosition(SrcCurveIndex.Offset);

							const int32 ParentClusterIndex = Cache.Exists(P);
							if (ParentClusterIndex != INDEX_NONE)
							{
								CurveToCopy.Add(CurveIt);

								#if !CURVE_USE_GROUP_ERROR
								// Extract parent cluster's LOD error and sphere bound radius
								const FCluster& ParentCluster = ClusterDAG.Clusters[ParentClusterIndex];
								ParentLODError = FMath::Max(ParentLODError, ParentCluster.LODError);
								ParentSphereBoundRadius = FMath::Max(ParentSphereBoundRadius, ParentCluster.SphereBounds.W);
								bIsParentDataInitialized = true;
								#endif
							}
							else
							{
								CurveToShuffle.Add(CurveIt);
							}
						}

						// * Insert persistent curves at the begining 
						// * Insert faded curves at the end
						FVertexArray DstVerts(Cluster.Verts.Format);
						DstVerts.AddUninitialized(Cluster.Verts.Num());

						TArray<int32> DstMaterialIndexes;
						DstMaterialIndexes.Reserve(NumCurves);

						TArray<FCurveIndex> DstCurveIndexes;
						DstCurveIndexes.Reserve(NumCurves);

						uint32 DstVertIndex = 0;
						auto CopyCurves = [&DstVertIndex, &DstVerts, &DstCurveIndexes, &DstMaterialIndexes, &Cluster](const TArray<uint32>& InSrcCurveIndexes)
						{
							for (uint32 SrcCurveIt : InSrcCurveIndexes)
							{
								const FCurveIndex& SrcCurveIndex = Cluster.CurveIndexes[SrcCurveIt];
								FCurveIndex& DstCurveIndex = DstCurveIndexes.AddDefaulted_GetRef();
								DstCurveIndex.Offset = DstVertIndex;
								DstCurveIndex.Count = SrcCurveIndex.Count;
								DstMaterialIndexes.Add(Cluster.MaterialIndexes[SrcCurveIt]);
								for (uint32 PointIt = 0; PointIt < SrcCurveIndex.Count; ++PointIt)
								{
									const uint32 SrcVertIndex = SrcCurveIndex.Offset + PointIt;
									DstVerts.GetPosition(DstVertIndex) = Cluster.Verts.GetPosition(SrcVertIndex);
									DstVerts.CopyAttributes(DstVertIndex, SrcVertIndex, Cluster.Verts);
									++DstVertIndex;
								}
							}
						};
						...
						// Persistent curves
						CopyCurves(CurveToCopy);

						// Faded curves
						FRandomStream RandomStream;
						RandomStream.Initialize(0);
						Algo::RandomShuffle(CurveToShuffle, RandomStream);
						CopyCurves(CurveToShuffle);

						Cluster.Verts = DstVerts;
						Cluster.CurveIndexes = DstCurveIndexes;
						Cluster.CurveFadingIndex = CurveToCopy.Num();
						Cluster.MaterialIndexes = DstMaterialIndexes;
						#if !CURVE_USE_GROUP_ERROR
						Cluster.ParentLODError = ParentLODError;
						Cluster.ParentSphereBoundRadius = ParentSphereBoundRadius;
						check(bIsParentDataInitialized);
						#endif
					}, ParallelFlags);
				}
			}
	...
	// Parent's error and Parent's bound radius
	#if CURVE_USE_GROUP_ERROR
	for (FClusterGroup& Group : ClusterDAG.Groups)
	{
		if (IsCurve(Group))
		{
			for (FClusterRef& ClusterRef : Group.Children)
			{
				FCluster& Cluster = ClusterRef.GetCluster(ClusterDAG);
				check(Cluster.LODError <= Group.ParentLODError);
				Cluster.ParentLODError = Group.ParentLODError;
				Cluster.ParentSphereBoundRadius = Group.Bounds.W;
			}
		}
	}
	#endif
...
)
{
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(Nanite::Build::BuildCurveHierarchyData);
...

	{
		...
		BuildHierarchies(Resources, ClusterDAG, Pages, GroupParts, EncodingInfos, NumMeshes);
	}
