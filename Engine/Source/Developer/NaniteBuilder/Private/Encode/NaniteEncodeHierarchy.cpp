...
struct FIntermediateNode
{
	...
	bool IsAssemblyRef() const { return NewAssemblyInstanceIndex != MAX_uint32; }
};
...
struct FHierarchyNode
{
	...
	TArray<uint32> BoneIndices[NANITE_MAX_BVH_NODE_FANOUT];
};

static uint32 PackNumBones_BoneIndicesOffset(uint32 NumBones, uint32 BoneIndicesOffset)
{
	check(NumBones <= (1u << NANITE_HIERARCHY_NUM_BONES_BITS) - 1u);
	check(BoneIndicesOffset <= (1u << NANITE_HIERARCHY_BONE_INDICES_OFFSET_BITS) - 1u);
	return BoneIndicesOffset | (NumBones << NANITE_HIERARCHY_BONE_INDICES_OFFSET_BITS);
}

static void UnpackNumBones_BoneIndicesOffset(uint32 Packed, uint32& OutNumBones, uint32& OutBoneIndicesOffset)
{
	OutBoneIndicesOffset = Packed & ((1u << NANITE_HIERARCHY_BONE_INDICES_OFFSET_BITS) - 1u);
	OutNumBones = Packed >> NANITE_HIERARCHY_BONE_INDICES_OFFSET_BITS;
}

static TArray<uint32> MergeLists(const TArray<uint32>& List0, const TArray<uint32>& List1)
{
	TArray<uint32> MergedList;
	MergedList.Reserve(List0.Num() + List1.Num());

	int32 Index0 = 0;
	int32 Index1 = 0;

	while (Index0 < List0.Num() && Index1 < List1.Num())
	{
		const uint32 Value0 = List0[Index0];
		const uint32 Value1 = List1[Index1];
		const uint32 MinValue = FMath::Min(Value0, Value1);

		Index0 += (Value0 == MinValue) ? 1 : 0;
		Index1 += (Value1 == MinValue) ? 1 : 0;

		MergedList.Add(MinValue);
	}

	while (Index0 < List0.Num())
	{
		MergedList.Add(List0[Index0++]);
	}

	while (Index1 < List1.Num())
	{
...
static void AppendToBoneList(TArray<uint32>& List, const TArray<uint32>& ToAppend, bool& bOverflow)
{
	if (bOverflow)
		return;
	...
	List = MergeLists(List, ToAppend);
	if (List.Num() > NANITE_MAX_HIERARCHY_BONE_INFLUENCES)
	{
		List.Empty();
		bOverflow = true;
	}
...
)
{
	...
	for( uint32 ChildIndex = 0; ChildIndex < NumChildren; ChildIndex++ )
	{
		...
		if( ChildNode->IsLeaf() )
		{
			...
			HNode.BoneIndices[ChildIndex] = ChildNode->BoneIndices;
		...
		else
		{
			...
			TArray<uint32> BoneIndices;
			bool bBoneIndicesOverflow = false;
			...
			for (uint32 GrandChildIndex = 0; GrandChildIndex < NANITE_MAX_BVH_NODE_FANOUT && ChildHNode.NumChildren[GrandChildIndex] != 0; GrandChildIndex++)
			{
				...
				AppendToBoneList(BoneIndices, ChildHNode.BoneIndices[GrandChildIndex], bBoneIndicesOverflow);
			}
			...
			HNode.AssemblyTransformIndex[ChildIndex] = ChildNewAssemblyInstanceIndex;
			HNode.BoneIndices[ChildIndex] = BoneIndices;
		}
	}
...
static void PackHierarchyNode(FResources& Resources, FPackedHierarchyNode& OutNode, const FHierarchyNode& InNode, const TArray<FClusterGroup>& Groups, const TArray<FClusterGroupPart>& GroupParts, const uint32 NumResourceRootPages)
{
	static_assert(NANITE_MAX_RESOURCE_PAGES_BITS + NANITE_MAX_CLUSTERS_PER_GROUP_BITS + NANITE_MAX_GROUP_PARTS_BITS <= 32);
	static_assert(NANITE_HIERARCHY_NUM_BONES_BITS + NANITE_HIERARCHY_BONE_INDICES_OFFSET_BITS <= 32);
	static_assert(NANITE_MAX_HIERARCHY_BONE_INFLUENCES <= (1 << NANITE_HIERARCHY_NUM_BONES_BITS) - 1);
	for (uint32 i = 0; i < NANITE_MAX_BVH_NODE_FANOUT; i++)
	{
		...
		const uint32 NumBones = InNode.BoneIndices[i].Num();
		if (NumBones > 0)
		{
			const uint32 BoneIndicesOffset = Resources.BoneIndices.Num();	//TODO: We could potentially deduplicate bone sets
			check(NumBones <= NANITE_MAX_HIERARCHY_BONE_INFLUENCES);
			OutNode.Misc2[i].NumBones_BoneIndicesOffset = PackNumBones_BoneIndicesOffset(NumBones, BoneIndicesOffset);

			// Pack two 16-bit bone indices per dword
			const uint32 NumDwords = (NumBones + 1) / 2;
			for (uint32 j = 0; j < NumDwords; j++)
			{
				const uint32 Idx0 = j * 2;
				const uint32 Idx1 = j * 2 + 1;
				check(InNode.BoneIndices[i][Idx0] < (1u << NANITE_MAX_BONE_INDEX_BITS));
				uint32 Packed = InNode.BoneIndices[i][Idx0];
				if (Idx1 < NumBones)
				{
					check(InNode.BoneIndices[i][Idx1] < (1u << NANITE_MAX_BONE_INDEX_BITS));
					Packed |= InNode.BoneIndices[i][Idx1] << NANITE_MAX_BONE_INDEX_BITS;
				}
				Resources.BoneIndices.Add(Packed);
			}
		}
		else
		{
			OutNode.Misc2[i].NumBones_BoneIndicesOffset = 0;
		}
	}
}

// Collect the unique set of bone indices influencing a cluster group part, sorted by bone index.
// Sort order also provides GPU cache locality when loading bone transforms (which are laid out by index).
static TArray<uint32> GatherPartBoneIndices(const FClusterGroupPart& Part, const TArray<FEncodingInfo>& EncodingInfos)
{
	TArray<uint32> BoneIndices;

	for (const uint32 ClusterIndex : Part.Clusters)
	{
		const TArray<FClusterBoneInfluence>& ClusterBones = EncodingInfos[ClusterIndex].BoneInfluence.ClusterBoneInfluences;

		if (ClusterBones.Num() == 0)
		{
			return TArray<uint32>();
		}

		for (const FClusterBoneInfluence& BoneInfluence : ClusterBones)
		{
			BoneIndices.AddUnique(BoneInfluence.BoneIndex);
			if (BoneIndices.Num() > NANITE_MAX_HIERARCHY_BONE_INFLUENCES)
			{
				return TArray<uint32>();
			}
		}
	...
	bool bAssemblyPart)
{
	...
	NodesByMip.SetNum(MaxMipLevel + 1);
	{
		...
		for (uint32 i = 0; i < NumGroupParts; i++)
		{
			...
			if (!bAssemblyPart)
			{
				Node.BoneIndices = GatherPartBoneIndices(Part, EncodingInfos);
			}
	...
	else
	{
		...
		for (int32 MipLevel = 0; MipLevel <= MaxMipLevel; MipLevel++)
		{
			if (NodesByMip[MipLevel].Num() > 0)
			{
				...
				if (Node.IsLeaf() || Node.IsAssemblyRef() || Node.Children.Num() == NANITE_MAX_BVH_NODE_FANOUT)
				{
					// Leaf, assembly reference or filled node. Just add it.
	...
	uint32 NumMeshes)
{
	...
	for (uint32 AssemblyPartIndex = 0; AssemblyPartIndex < NumAssemblyParts; AssemblyPartIndex++)
	{
		...
		if (GroupParts.Num() > 0)
		{
			RootIndex = BuildHierarchy(Nodes, ClusterDAG, Parts, AssemblyRoots, GroupParts, EncodingInfos, true);
		}
	...
	{
		const uint32 RootIndex = BuildHierarchy(Nodes, ClusterDAG, Parts, AssemblyRoots, RootMeshGroupParts[MeshIndex], EncodingInfos, false);
		...
		{
			PackHierarchyNode(Resources, Resources.HierarchyNodes[PackedBaseIndex + i], HierarchyNodes[i], ClusterDAG.Groups, Parts, Resources.NumRootPages);
		}
	}
	
	// Relocate Indices offset to after HierarchyNodes
	for (FPackedHierarchyNode& HierarchyNode : Resources.HierarchyNodes)
	{
		for (uint32 i = 0; i < NANITE_MAX_BVH_NODE_FANOUT; i++)
		{
			uint32& Packed = HierarchyNode.Misc2[i].NumBones_BoneIndicesOffset;
			uint32 NumBones, Offset;
			UnpackNumBones_BoneIndicesOffset(Packed, NumBones, Offset);
			if (NumBones)
			{
				Offset += Resources.HierarchyNodes.Num() * NANITE_HIERARCHY_NODE_SLICE_SIZE_DWORDS;
				Packed = PackNumBones_BoneIndicesOffset(NumBones, Offset);
			}
		}
