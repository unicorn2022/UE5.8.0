...
static int32 ShortestWrap(int32 Value, uint32 NumBits)
{
	...
	const int32 NumValues = (1u << NumBits);
...
static void CalculateEncodingInfo(FEncodingInfo& Info, const FCluster& Cluster, int32 NormalPrecision, int32 TangentPrecision, int32 BoneWeightPrecision)
{
	...
	const uint32 NumClusterCurves	= Cluster.NumCurves;
	...
	const uint32 NumPositions = (Cluster.NumTris != 0 || Cluster.NumCurves != 0) ? NumClusterVerts : 0;
	...
	Info.CurveRadiusBits = 0;
	Info.CurveRadiusScale = 1;
	...
	Info.CurveRadiusBits = 0;
	Info.CurveRadiusScale = 1;
	if (Cluster.Verts.Format.bHasCurveRadius)
	{
		float CurveRadiusMax = 0;
		for (uint32 i = 0; i < NumClusterVerts; i++)
		{
			CurveRadiusMax = FMath::Max(CurveRadiusMax, Cluster.Verts.GetCurveRadius(i));
		}
		const float CurveRadiusScale	= CurveRadiusMax > 0 ? CurveRadiusMax : 1.f;
		const int32 CurveRadiusBits	= NANITE_MAX_CURVE_RADIUS_QUANTIZATION_BITS ;

		Info.BitsPerAttribute 	+= CurveRadiusBits;
		Info.CurveRadiusBits	= CurveRadiusBits;
		Info.CurveRadiusScale  	= CurveRadiusScale;
	}
							...
							TMap<FVariableVertex, uint32>& UniqueVertices, uint32& NumCodedVertices, FPageStreams& Streams)
{
	...
	for (uint32 VertexIndex = 0; VertexIndex < NumClusterVerts; VertexIndex++)
	{
		...
		if (Cluster.Verts.Format.bHasCurveRadius)
		{
			uint32 CurveRadiusDW = *(uint32*)&Cluster.Verts.GetCurveRadius(VertexIndex);
			BitWriter_Attribute.PutBits(CurveRadiusDW, 32);
		}
	...
	const uint32 BytesPerCurveRadiusComponent = (EncodingInfo.CurveRadiusBits + 7) / 8;
	...
	if (Cluster.Verts.Format.bHasCurveRadius)
	{
		uint32 PrevCurveRadiusValue = 0u;
		for (uint32 LocalVertexIndex = 0; LocalVertexIndex < NumUniqueToVertices; LocalVertexIndex++)
		{
			uint32 VertexIndex = LocalVertexIndex;
			if( bUseVertexRefs )
			...
			const uint32 CurveRadius8bits = FMath::Clamp(uint32(Cluster.Verts.GetCurveRadius(VertexIndex) / EncodingInfo.CurveRadiusScale * 255.f), 0u, 0xFFu);
			const uint32 CurveRadiusDelta = ShortestWrap(CurveRadius8bits - PrevCurveRadiusValue, EncodingInfo.CurveRadiusBits);
			WriteZigZagDelta(CurveRadiusDelta, BytesPerCurveRadiusComponent);
			
			PrevCurveRadiusValue = CurveRadius8bits;
		}
	}
