...
void PrintMaterialRangeStats( const TArray<FCluster>& Clusters )
{
	...
	UE_LOGF( LogStaticMesh, Log, "Material Stats - Unique Materials: %d, Fast Path Clusters: %d, Slow Path Clusters: %d, 1 Material: %d, 2 Materials: %d, 3 Materials: %d, At Least 4 Materials: %d",
	...
	for( uint32 MaterialIndex = 0; MaterialIndex < MAX_CLUSTER_MATERIALS; ++MaterialIndex )
	{
		...
		{
			UE_LOGF( LogStaticMesh, Log, "  Material Index: %d", MaterialIndex );
		}
	}
#endif
}
