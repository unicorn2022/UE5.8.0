...
public:
	...
	~FTimeLogScope()
	{
		...
		UE_LOGF(LogStaticMesh, Log, "%ls [%.2fs]", Label, FPlatformTime::ToMilliseconds(EndTime - StartTime) / 1000.0f);
	}
...
const FString& FBuilderModule::GetVersionString() const
{
	...
	if (VersionString.IsEmpty())
	{
		...
		const bool bCurvesSupported = NaniteCurvesSupported();
		...
		VersionString = FString::Printf(TEXT("%s_CONSTRAINED%s%s%s%s"), *FDevSystemGuids::GetSystemGuid(FDevSystemGuids::Get().NANITE_DERIVEDDATA_VER).ToString(EGuidFormats::DigitsWithHyphens),
										...
										bVoxelsSupported ? TEXT("_VOXEL2") : TEXT(""),
										bCurvesSupported ? TEXT("_CURVE") : TEXT(""),
										bAssembliesSupported ? TEXT("_ASSEMBLIES3") : TEXT(""));
...
static void EvaluateCoarseRepresentation(const FRayTracingScene& Source, const FRayTracingScene& Coarse, FBounds3f Bounds, const FCluster* SourceCluster)
{
	...
	UE_LOGF(LogStaticMesh, Log, "CoarseRepresentation accuracy = %.1f%% (%d / %d)", float(MatchCount) / float(RayCount) * 100.0f, MatchCount, RayCount);
	UE_LOGF(LogStaticMesh, Log, "Hit accuracy = %.1f%% (%d / %d)", float(HitMatchCount) / float(NumHitsSource) * 100.0f, HitMatchCount, NumHitsSource);
	UE_LOGF(LogStaticMesh, Log, "Miss accuracy = %.1f%% (%d / %d)", float(MissMatchCount) / float(NumMissesSource) * 100.0f, MissMatchCount, NumMissesSource);
	UE_LOGF(LogStaticMesh, Log, "Occlusion = %.1f%% / Source Occlusion = %.1f%%", float(NumHitsCoarse) / float(RayCount) * 100.0f, float(NumHitsSource) / float(RayCount) * 100.0f);
}
	...
	const FRayTracingFallbackBuildSettings* RayTracingFallbackBuildSettings )
{
	...
	const float OutError = CoarseRepresentation.SimplifyTriangles( ClusterDAG, TargetNumTris, TargetError, FMath::Min( TargetNumTris, 256u ), RayTracingFallbackBuildSettings );
	...
	for( uint32 Iter = 0, Num = CoarseRepresentation.Verts.Num(); Iter < Num; ++Iter )
	{
		...
		if( CoarseRepresentation.Verts.Format.bHasCurveRadius )
		{
			Verts.CurveRadius.Emplace( CoarseRepresentation.Verts.GetCurveRadius(Iter) );
		}
	...
	const FMeshNaniteSettings& Settings )
{
	if( Settings.DisplacementMaps.Num() && InputMeshData.TriangleCounts.Num() == 1 && Settings.TrimRelativeError != 0.0f )
	{
		...
		UE_LOGF( LogStaticMesh, Log, "Adaptive tessellate [%.2fs], tris: %i", FPlatformTime::ToMilliseconds( Time1 - Time0 ) / 1000.0f, InputMeshData.TriangleCounts[0] );
	}
}
	...
	bool bAssemblyPart )
{
	...
	if (InputMeshData.TriangleIndices.Num() == 0 && InputMeshData.CurveIndices.Num()==0 && !bIsAssembly)
	{
		UE_LOGF(LogStaticMesh, Warning, "Failed to build Nanite mesh. Input has 0 triangles 0 curves.");
		return false;
	}
	...
	Resources.NumInputCurves	= InputMeshData.CurveIndices.Num();
	Resources.ResourceFlags = 0x0;

	// Sanity check
	check(InputMeshData.TriangleCounts.Num() == 0 || InputMeshData.CurveCounts.Num() == 0);
	if (InputMeshData.CurveCounts.Num() > 0)
	{
		// For now, no voxel, only curves
		check(Settings.VoxelLevel == 0);
		check(Settings.ShapePreservation == ENaniteShapePreservation::None);
	}
	...
	VertexFormat.bHasCurveRadius	= InputMeshData.Vertices.CurveRadius.Num() == InputMeshData.Vertices.Position.Num() && InputMeshData.CurveIndices.Num() > 0;
	...
	
	{
		...
		uint32 BaseCurve = 0;
		for (uint32 NumCurves : InputMeshData.CurveCounts)
		{
			if (NumCurves)
			{
				Resources.ClusterDAG.AddCurves(
					VertexView,
					TArrayView< const FCurveIndex >( &InputMeshData.CurveIndices[BaseCurve], NumCurves ),
					TArrayView< const int32 >( &InputMeshData.MaterialIndices[BaseCurve], NumCurves ),
					InputMeshData.VertexBounds, VertexFormat );
			}
			else
			{
	...
	if( bCanFreeInputMeshData )
	{
		...
		InputMeshData.CurveIndices.Empty();
	}
...

	{
		...
		for ( int32 CurvesIndex = 0; CurvesIndex < InputMeshData.CurveCounts.Num(); CurvesIndex++ )
		{
			Resources.ClusterDAG.ReduceMesh( CurvesIndex );
		}
	}
	...
	if( Settings.KeepPercentTriangles < 1.0f || Settings.TrimRelativeError > 0.0f )
	{
		...
		UE_LOGF( LogStaticMesh, Log, "Trimmed to %u tris", NumTris );
	}
	...
	const FRayTracingFallbackBuildSettings* RayTracingFallbackBuildSettings = nullptr)
{
	...
	const float FallbackTargetError = FallbackRelativeError * 0.01f * FMath::Sqrt( FMath::Min( 2.0f * Intermediate.ClusterDAG.SurfaceArea, Intermediate.ClusterDAG.TotalBounds.GetSurfaceArea() ) );
	...
	UE_LOGF(LogStaticMesh, Log, "Fallback [%.2fs], num tris: %d", FPlatformTime::ToMilliseconds(FallbackEndTime - FallbackStartTime) / 1000.0f, OutFallbackMeshData.TriangleIndices.Num() / 3);
}
	...
	FInputAssemblyData* InputAssemblyData)
{
	...
	uint32 TotalGPUSize;
	{
		...
		const uint32 NumMeshes = InputMeshData.TriangleCounts.Num() ? InputMeshData.TriangleCounts.Num() : InputMeshData.CurveCounts.Num();
		check(InputMeshData.TriangleCounts.Num()==0 || InputMeshData.CurveCounts.Num()==0);
		check(NumMeshes);
		...
		if (!bSuccess)
		{
			UE_LOGF(LogStaticMesh, Warning, "Failed to build Nanite mesh. An internal limitation was exceeded while encoding.");
			return false;
		}
	}
	...
	UE_LOGF(LogStaticMesh, Log, "Compressed size: %.2fMB -> %.2fMB", UncompressedSize / 1048576.0f, CompressedSize / 1048576.0f);

	{
		...
		UE_LOGF(LogStaticMesh, Log, "Total: %d Meshes, GPU: %.2fMB, Uncompressed: %.2fMB, Compressed: %.2fMB", TotalMeshes, TotalMeshGPUSize / 1048576.0f, TotalMeshUncompressedSize / 1048576.0f, TotalMeshCompressedSize / 1048576.0f);
	}
#endif
...
bool FBuilderModule::BuildMaterialIndices(const FMeshDataSectionArray& SectionArray, const uint32 TriangleCount, TArray<int32>& OutMaterialIndices)
{
	...
	{
		UE_LOGF(LogStaticMesh, Log, "Unable to build Nanite data. Unsupported number of sections: %d.", SectionArray.Num());
		return false;
	}
