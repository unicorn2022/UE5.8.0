	...
	const FVertexFormat& VertexFormat )
{
	...
	bHasCurveRadius		|= VertexFormat.bHasCurveRadius;
	...
	UE_LOGF( LogStaticMesh, Log,
		"Adjacency [%.2fs], tris: %i, UVs %i%ls%ls%ls",
		...
		VertexFormat.bHasCurveRadius ? TEXT(", CurveRadius") : TEXT(""),
	...
	UE_LOGF( LogStaticMesh, Log, "Clustering [%.2fs]. Ratio: %f", FPlatformTime::ToMilliseconds( ClusterTime - BoundaryTime ) / 1000.0f, (float)Partitioner.Ranges.Num() / (float)OptimalNumClusters );
	...
	UE_LOGF( LogStaticMesh, Log, "Leaves [%.2fs]", FPlatformTime::ToMilliseconds( LeavesTime - ClusterTime ) / 1000.0f );
}

FClusterCurveStats GetClusterCurveStats(const TArrayView<const FCurveIndex>& CurveIndexes)
{
	FClusterCurveStats Out;
	Out.MinPointPerCurve = ~0u;
	for (const FCurveIndex& CurveIndex : CurveIndexes)
	{
		Out.TotalPointCount += CurveIndex.Count;
		Out.MinPointPerCurve = FMath::Min(Out.MinPointPerCurve, CurveIndex.Count);
		Out.MaxPointPerCurve = FMath::Max(Out.MaxPointPerCurve, CurveIndex.Count);
	}
	Out.AvgPointPerCurve = FMath::Max(1u, FMath::DivideAndRoundDown(Out.TotalPointCount, uint32(CurveIndexes.Num())));
	return Out;
}

FClusterCurveStats GetClusterCurveStats(const TArray<int32>& CurveIndices, const TArrayView<const FCurveIndex>& CurvePointRanges)
{
	FClusterCurveStats Out;
	Out.MinPointPerCurve = ~0u;
	for (const int32& CurveIndex : CurveIndices)
	{
		const uint32 Count = CurvePointRanges[CurveIndex].Count;
		Out.TotalPointCount += Count;
		Out.MinPointPerCurve = FMath::Min(Out.MinPointPerCurve, Count);
		Out.MaxPointPerCurve = FMath::Max(Out.MaxPointPerCurve, Count);
	}
	Out.AvgPointPerCurve = FMath::Max(1u, FMath::DivideAndRoundDown(Out.TotalPointCount, uint32(CurveIndices.Num())));
	return Out;
}

FClusterCurveStats GetClusterCurveStats(const TArrayView<const FCurveIndex>& InCurveIndexes, const TArrayView<const uint32>& SortedCurveIndexes, uint32 Begin, uint32 End)
{
	FClusterCurveStats Out;
	Out.MinPointPerCurve = ~0u;
	for (uint32 i = Begin; i < End; i++)
	{
		const FCurveIndex CurveIndex = InCurveIndexes[SortedCurveIndexes[i]];
		Out.TotalPointCount += CurveIndex.Count;
		Out.MinPointPerCurve = FMath::Min(Out.MinPointPerCurve, CurveIndex.Count);
		Out.MaxPointPerCurve = FMath::Max(Out.MaxPointPerCurve, CurveIndex.Count);
	}
	Out.AvgPointPerCurve = FMath::Max(1u, FMath::DivideAndRoundDown(Out.TotalPointCount, uint32(End - Begin)));
	return Out;
}

static void InternalGetClusterCurveTargetMetric(const FClusterCurveStats& InStats, const uint32 InNumPoints, const uint32 InNumCurves, uint32 InNumParents, uint32 InTargetClusterSize, uint32& OutTargetNumCurves, uint32& OutTargetNumSegments)
{
	OutTargetNumCurves   = InNumParents * (InStats.MaxPointPerCurve > 0 ? (uint32)FMath::Max(1, (int32)FMath::FloorToInt(float(InTargetClusterSize) / float(InStats.MaxPointPerCurve))) : 0u);
	OutTargetNumSegments = InNumParents * InTargetClusterSize;
}

static void GetClusterCurveTargetMetric(const FCluster& In, uint32 InNumParents, uint32 InTargetClusterSize, uint32& OutTargetNumCurves, uint32& OutTargetNumSegments)
{
	InternalGetClusterCurveTargetMetric(GetClusterCurveStats(In.CurveIndexes), In.Verts.Num(), In.CurveIndexes.Num(), InNumParents, InTargetClusterSize, OutTargetNumCurves, OutTargetNumSegments);
}

static void GetClusterCurveTargetMetric(const TArrayView<const FCurveIndex>& InCurveIndexes, uint32 InNumParents, uint32 InTargetClusterSize, uint32& OutTargetNumCurves, uint32& OutTargetNumSegments)
{
	const FClusterCurveStats Stats = GetClusterCurveStats(InCurveIndexes);
	InternalGetClusterCurveTargetMetric(Stats, Stats.TotalPointCount, InCurveIndexes.Num(), InNumParents, InTargetClusterSize, OutTargetNumCurves, OutTargetNumSegments);
}

static uint32 InternalClusterizeCurves(
	const TArray<FVector3f>& RootPoints,
	TArrayView< const FCurveIndex > CurveIndexes,
	TArray<uint32>& OutClusteredCurveIndices,
	TArray<FRange>& OutClusteredCurveRanges,
	uint32 MaxNumClusters)
{
	uint32 NumCurvePerCluster, NumSegmentsPerCluster;
	GetClusterCurveTargetMetric(CurveIndexes, 1u, FCluster::MaxNumSegmentPerCluster, NumCurvePerCluster, NumSegmentsPerCluster);
	
	// Either use 'MaxNumClusters' as optimal cluster count and derive it
	uint32 OptimalNumClusters = 0;
	if (MaxNumClusters > 0)
	{
		OptimalNumClusters = MaxNumClusters;
		const uint32 TotalNumSegments = GetClusterCurveStats(CurveIndexes).TotalPointCount;
		check(TotalNumSegments <= FCluster::MaxNumSegmentPerCluster * MaxNumClusters);
	}
	else
	{
		OptimalNumClusters = FMath::DivideAndRoundUp((uint32)CurveIndexes.Num(), NumCurvePerCluster );
	}

	OutClusteredCurveIndices.Empty();
	OutClusteredCurveRanges.Empty();

	// Special case: 1 cluster
	if (OptimalNumClusters == 1)
	{
		uint32 Idx = 0;
		OutClusteredCurveIndices.Reserve(CurveIndexes.Num());
		uint32 OutClusterNumSegments = 0;
		for (const FCurveIndex& CurveIndex : CurveIndexes)
		{
			OutClusterNumSegments += CurveIndex.Count;
			OutClusteredCurveIndices.Add(Idx++);
		}
		check(OutClusterNumSegments < FCluster::MaxNumSegmentPerCluster);
		FRange& Range = OutClusteredCurveRanges.AddDefaulted_GetRef();
		Range.Begin = 0;
		Range.End = Idx;
		return OutClusterNumSegments;
	}

	// Split curves into clusters of FCluster::MaxNumSegmentPerCluster segments
	{
		// 1. Kmeans for extracting clusters
		TArray<TArray<int32>> ClusterIndices;
		{
			UE::Geometry::FClusterKMeans Cluster;
			int32 NumInitialPoints = RootPoints.Num();
			int32 NumClusters = OptimalNumClusters;
		...
		// 2. Split large clusters
		// The output of Kmeans does not ensure balanced clusters, nor enforce the max-curves per cluster, nor max-points per cluster
		// Due to this, we manually split clusters not meeting the criteria
		// TODO_CURVE: replace this with a capacited flow graph algo
		{
			// 2.a Queue all clusters which needs to be split
			TArray<TArray<int32>> ClustersToSplit;
			for (int32 ClusterIt = ClusterIndices.Num()-1; ClusterIt >= 0; --ClusterIt)
			{
				const TArray<int32>& Curves = ClusterIndices[ClusterIt];
				if (uint32(Curves.Num()) > NumCurvePerCluster || GetClusterCurveStats(Curves, CurveIndexes).TotalPointCount > FCluster::MaxNumSegmentPerCluster)
				{
					ClustersToSplit.Add(Curves);				
					ClusterIndices.RemoveAtSwap(ClusterIt);
				}
			}

			// 2.b Manually split large clusters
			for (const TArray<int32>& ClusterToSplit : ClustersToSplit)
			{
				int32 NumClusters = FMath::DivideAndRoundUp((uint32)ClusterToSplit.Num(), NumCurvePerCluster);
				TArray<int32> CurrentCluster;
				CurrentCluster.Reserve(NumCurvePerCluster);
				for (int32 CurveIt : ClusterToSplit)
				{
					CurrentCluster.Add(CurveIt);
					if (CurrentCluster.Num() == NumCurvePerCluster)
					{
						const uint32 ClusterNumSegments = GetClusterCurveStats(CurrentCluster, CurveIndexes).TotalPointCount;
						check(ClusterNumSegments <= FCluster::MaxNumSegmentPerCluster);
		...
		// 3. Manually merge small clusters
		// Manual splitting might have create more clusters than requested (i.e., MaxNumClusters). 
		// Redistribution small cluster's curves among existing clusters which have room for it.
		// TODO_CURVE: This breaks spatial coherence of cluster, capacited flow graph should solve this
		if (MaxNumClusters > 0 && uint32(ClusterIndices.Num()) > MaxNumClusters)
		{
			struct FSortedCluster
			{
				uint32 ClusterIndex;
				uint32 NumCurve;
				uint32 NumSegments;
			};
			TArray<FSortedCluster> SortedClusters;
			SortedClusters.Reserve(ClusterIndices.Num());
			uint32 GlobalIndex = 0;
			for (const TArray<int32>& Cluster : ClusterIndices)
			{
				FSortedCluster& C = SortedClusters.AddDefaulted_GetRef();
				C.ClusterIndex = GlobalIndex++;
				C.NumCurve = Cluster.Num();
				C.NumSegments = GetClusterCurveStats(Cluster, CurveIndexes).TotalPointCount;
			}
			SortedClusters.Sort([](const FSortedCluster& A, const FSortedCluster& B) { return A.NumCurve < B.NumCurve; });

			int32 i = 0;
			int32 j = SortedClusters.Num()-1;
			while (i < j)
			{
				const uint32 Si = SortedClusters[i].ClusterIndex;
				const uint32 Sj = SortedClusters[j].ClusterIndex;

				uint32 SiNumSegmentLastCurve = FCluster::MaxNumSegmentPerCluster;
				if (ClusterIndices[Si].Num() > 0)
				{
					const uint32 SiLastCurveIndex = ClusterIndices[Si].Last();
					SiNumSegmentLastCurve = CurveIndexes[SiLastCurveIndex].Count;
				}

				// All curves of the current cluster are merged
				if (SortedClusters[i].NumSegments + SortedClusters[j].NumSegments <= FCluster::MaxNumSegmentPerCluster)
				{
					ClusterIndices[Sj].Append(ClusterIndices[Si]);
					SortedClusters[j].NumCurve    += SortedClusters[i].NumCurve;
					SortedClusters[j].NumSegments += SortedClusters[i].NumSegments;
					SortedClusters[i].NumCurve     = 0;
					SortedClusters[i].NumSegments  = 0;
					ClusterIndices[Si].Empty();
					i++;
				}
				// 1 curve of the current cluster is merged
				else if (SiNumSegmentLastCurve + SortedClusters[j].NumSegments <= FCluster::MaxNumSegmentPerCluster)
				{
					
					ClusterIndices[Sj].Add(ClusterIndices[Si].Last());
					SortedClusters[j].NumCurve += 1;
					SortedClusters[j].NumSegments += SiNumSegmentLastCurve;
					SortedClusters[i].NumCurve -= 1;
					SortedClusters[i].NumSegments -= SiNumSegmentLastCurve;
					ClusterIndices[Si].RemoveAtSwap(ClusterIndices[Si].Num()-1);
					if (ClusterIndices[Si].IsEmpty())
					{
						i++;
					}
				}
			...
			for (int32 ClusterIt=ClusterIndices.Num()-1;ClusterIt>=0;--ClusterIt)
			{
				if (ClusterIndices[ClusterIt].IsEmpty())
				{
					ClusterIndices.RemoveAtSwap(ClusterIt);
				}
			}

			check(uint32(ClusterIndices.Num()) <= MaxNumClusters);
		}

		// Build clusters' ranges
		uint32 OutClusterMaxNumSegments = 0;
		OutClusteredCurveIndices.Reserve(CurveIndexes.Num());
		OutClusteredCurveRanges.Reserve(ClusterIndices.Num());
		for (const TArray<int32>& Curves : ClusterIndices)
		{
			FRange& ClusterRange = OutClusteredCurveRanges.AddDefaulted_GetRef();
			ClusterRange.Begin = OutClusteredCurveIndices.Num();
			uint32 ClusterNumSegments = 0;
			for (int32 CurveIndex : Curves)
			{
				check(CurveIndex != INDEX_NONE);
				OutClusteredCurveIndices.Add(CurveIndex);
				ClusterNumSegments += CurveIndexes[CurveIndex].Count;
			}
			ClusterRange.End = OutClusteredCurveIndices.Num();
			OutClusterMaxNumSegments = FMath::Max(OutClusterMaxNumSegments, ClusterNumSegments);
		}

		check(OutClusterMaxNumSegments <= FCluster::MaxNumSegmentPerCluster);
		return OutClusterMaxNumSegments;
	}
}

static void ClusterizeCurves(
	const FConstMeshBuildVertexView& Verts,
	TArrayView<const FCurveIndex> CurveIndexes,
	TArray<uint32>& OutClusteredCurveIndices,
	TArray<FRange>& OutClusteredCurveRanges,
	uint32 MaxNumClusters = 0)
{
	// Extract curves' root position
	TArray<FVector3f> RootPoints;
	RootPoints.Reserve(CurveIndexes.Num());
	for (const FCurveIndex& CurveIndex : CurveIndexes)
	{
		RootPoints.Add(Verts.Position[CurveIndex.Offset]);
	}
	InternalClusterizeCurves(RootPoints, CurveIndexes, OutClusteredCurveIndices, OutClusteredCurveRanges, MaxNumClusters);
}

static void ClusterizeCurves(
	const FVertexArray& Verts,
	TArrayView<const FCurveIndex> CurveIndexes,
	TArray<uint32>& OutClusteredCurveIndices,
	TArray<FRange>& OutClusteredCurveRanges,
	uint32 MaxNumClusters = 0)
{
	...
	for (const FCurveIndex& CurveIndex : CurveIndexes)
	{
		RootPoints.Add(Verts.GetPosition(CurveIndex.Offset));
	}
	InternalClusterizeCurves(RootPoints, CurveIndexes, OutClusteredCurveIndices, OutClusteredCurveRanges, MaxNumClusters);
}

void FClusterDAG::AddCurves(
	const FConstMeshBuildVertexView& Verts,
	TArrayView< const FCurveIndex > CurveIndexes,
	TArrayView< const int32 > MaterialIndexes,
	const FBounds3f& VertexBounds,
	const FVertexFormat& VertexFormat )
{
	...
	// Partition curves into clusters
	TArray<uint32> ClusteredCurveIndices;
	TArray<FRange> ClusteredCurveRanges;
	ClusterizeCurves(Verts, CurveIndexes, ClusteredCurveIndices, ClusteredCurveRanges);

	const uint32 NumClusters = ClusteredCurveRanges.Num();
	const uint32 BaseCluster = Clusters.Num();
	Clusters.AddDefaulted( NumClusters );

	const uint32 MeshIndex = MeshInput.AddDefaulted();
	MeshInput[ MeshIndex ].AddUninitialized( NumClusters );

	{
		TRACE_CPUPROFILER_EVENT_SCOPE(Nanite::Build::BuildClusters);
		ParallelFor( TEXT("Nanite.BuildClusters.PF"), NumClusters, 1024,
		[&]( int32 Index )
		{
			uint32 ClusterIndex = BaseCluster + Index;
			...
			Clusters[ ClusterIndex ] = FCluster(
				...
				MaterialIndexes,
				VertexFormat,
				ClusteredCurveRanges[Index].Begin, ClusteredCurveRanges[Index].End,
				ClusteredCurveIndices /*Sorted indices*/);
	...
	// Add surface area only for the newly added clusters
	for (uint32 ClusterIndex = BaseCluster, TotalClusterCount = BaseCluster+NumClusters; ClusterIndex < TotalClusterCount; ++ClusterIndex)
	{		
		SurfaceArea += Clusters[ClusterIndex].SurfaceArea;
	}
		...
		[&]( uint32 ClusterRefIndex )
		{
			...
			for( int32 EdgeIndex = 0; EdgeIndex < Cluster.ExternalEdges.Num(); EdgeIndex++ )
			{
				...
				if( Cluster.ExternalEdges[ EdgeIndex ] )
				{
					...
					// Update external edges based on current connectivity.
					// This is important to prevent 'leaking' of external edges, resulting in reduced quality.
					if (AdjCount == 0)
					{
						Cluster.NumExternalEdges--;
					}
					Cluster.ExternalEdges[EdgeIndex] = (int8)FMath::Min(AdjCount, 127u);
				}
			}
...
uint32 GetMaxParents( const FClusterGroup& Group, FClusterDAG& DAG, uint32 MaxClusterSize )
{
	...
	{
		const FCluster& Cluster = Child.GetCluster(DAG);
		if (Cluster.IsCurve())
		{
			const uint32 NumPointPerCurve = Cluster.CurveIndexes.Num() > 0 ? FMath::DivideAndRoundUp(Cluster.Verts.Num(), uint32(Cluster.CurveIndexes.Num())) : 0u;
			const uint32 NumCurvePerCluster = NumPointPerCurve > 0 ? FMath::Max(1, (int32)FMath::FloorToInt(float(MaxClusterSize) / float(NumPointPerCurve))) : 0u;
			NumGroupElements += NumCurvePerCluster * NumPointPerCurve;
		}
		else
		{
...
void FClusterDAG::GroupCurveClusters(TArrayView<const FClusterRef> LevelClusters)
{
	if( LevelClusters.IsEmpty() )
		return;
	...
	// Split using kmeans - Closest clusters are regrouped together
	TArray<FVector3f> ClusterCenters;
	ClusterCenters.Reserve(LevelClusters.Num());
	for (const FClusterRef& LevelCluster : LevelClusters)
	{
		ClusterCenters.Add(LevelCluster.GetCluster(*this).SphereBounds.Center);
	}
	
	const uint32 TargetNumGroup = FMath::DivideAndRoundUp((uint32)LevelClusters.Num(), MinGroupSize);
	UE::Geometry::FClusterKMeans KMeans;
	TArray<FVector3f> Centers;
	KMeans.GetUniformSpacedInitialCenters<FVector3f>(ClusterCenters, TargetNumGroup, Centers);
	int32 FoundClusters = KMeans.ComputeClusters<FVector3f, false/*bParallel*/>(ClusterCenters, TargetNumGroup, Centers);

	// Verify cluster counts
	TArray<TArray<int32>> GroupOfClusterIndices;
	KMeans.GetClusters(GroupOfClusterIndices);
	check(GroupOfClusterIndices.Num() == FoundClusters);

	// Add the new groups
	Groups.Reserve(Groups.Num() + GroupOfClusterIndices.Num());
	for(const TArray<int32>& ClusterIndices : GroupOfClusterIndices )
	{
		check(ClusterIndices.Num() <= MaxGroupSize);

		FClusterGroup& Group = Groups.AddDefaulted_GetRef();
		for (int32 ClusterIndex : ClusterIndices)
		{
			Group.Children.Add(LevelClusters[ClusterIndex]);
		}
	}
}
...
void FClusterDAG::ReduceMesh( uint32 MeshIndex )
{
	...
	while( true )
	{
		...
		UE_LOGF( LogStaticMesh, Verbose, "Num clusters %i. Error %.4f, %.4f, %.4f", LevelClusters.Num(), MinError, AvgError, MaxError );
		...
		if( LevelClusters.Num() == 1 )
		{
			...
			if( Cluster.NumTris )
				break;
			else if (Cluster.IsCurve())
			{
				MaxClusterSize = FCluster::MaxNumSegmentPerCluster;
				break;
			}
		...
		int32 NumCurveClusters = Algo::Partition( LevelClusters,
			[ this ]( FClusterRef ClusterRef )
			{
				return ClusterRef.GetCluster( *this ).NumCurves > 0;
			} );

		// Sanity check - Triangles and curves are mutually exclusive for now
		check(NumTriClusters == 0 || NumCurveClusters == 0);

		TArrayView< FClusterRef > TriClusters	= LevelClusters.Slice( 0, NumTriClusters );
		TArrayView< FClusterRef > CurveClusters	= LevelClusters.Slice( NumTriClusters, NumCurveClusters );
		TArrayView< FClusterRef > VoxelClusters	= LevelClusters.Slice( NumTriClusters + NumCurveClusters, LevelClusters.Num() - (NumTriClusters + NumCurveClusters) );
		...
		if( CurveClusters.Num() == 1 )
		{
			CurveClusters = TArrayView< FClusterRef >();
			VoxelClusters = LevelClusters;
		}
		...
		GroupCurveClusters(CurveClusters);
		...
		// Force a deterministic order of the generated parent clusters
		{
			...
			//UE_LOGF(LogStaticMesh, Log, "SortTime Adjacency [%.2fs]", FPlatformTime::ToMilliseconds(FPlatformTime::Cycles() - StartTime) / 1000.0f);
		}
	}
...
{
	const bool bIsSingleCluster = Merged.IsCurve() ? Merged.Verts.Num() <= MaxClusterSize : Merged.MaterialIndexes.Num() <= (int32)MaxClusterSize;
	if (bIsSingleCluster)
	{
	...
	else if( NumParents > 1 )
	{
		...
		if (Merged.NumCurves)
		{
			TArray<uint32> ClusteredCurveIndices;
			TArray<FRange> ClusteredCurveRanges;
			TArrayView<const FCurveIndex> CurveIndexesView(Merged.CurveIndexes.GetData(), Merged.NumCurves);
			ClusterizeCurves(Merged.Verts, CurveIndexesView, ClusteredCurveIndices, ClusteredCurveRanges, NumParents);

			if (ClusteredCurveRanges.Num() <= (int32)NumParents)
			{
				NumParents = ClusteredCurveRanges.Num();
				ParentEnd = ( NumClusters += NumParents );
				ParentStart = ParentEnd - NumParents;

				const FAdjacency DummyAdjacency(0u);
				const TArrayView<const uint32> DummySortedTo;

				int32 Parent = ParentStart;
				for( auto& Range : ClusteredCurveRanges )
				{
					Clusters[ Parent ] = FCluster( Merged, Range.Begin, Range.End, ClusteredCurveIndices, DummySortedTo, DummyAdjacency);
					Parent++;
				}
...
void FClusterDAG::ReduceGroup( std::atomic< int32 >& NumClusters, uint32 MaxClusterSize, uint32 NumParents, int32 GroupIndex, uint32 MeshIndex )
{
	...
	bool bAnyCurves = false;
	bool bAllCurves = true;

	uint32 GroupNumVerts = 0;
	uint32 GroupNumCurves = 0;
	...
	for( FClusterRef Child : Group.Children )
	{
		...
		GroupNumCurves+= Cluster.CurveIndexes.Num();

		bAnyTriangles = bAnyTriangles || Cluster.NumTris > 0;
		bAllTriangles = bAllTriangles && Cluster.NumTris > 0;

		bAnyCurves = bAnyCurves || Cluster.NumCurves > 0;
		bAllCurves = bAllCurves && Cluster.NumCurves > 0;
	...
	bool bVoxels = false;
	bVoxels = (!bAllTriangles && !bAllCurves) || Settings.ShapePreservation == ENaniteShapePreservation::Voxelize;

	// Sanity check - If a cluster has curves, it can't have triangle nor voxels atm
	check (!bAnyTriangles || !bAnyCurves);
	check (!bAnyCurves || (bAnyCurves && !bVoxels));
	...
	if( bAllTriangles )
	{
			...
			Settings.VoxelLevel > Group.MipLevel + 1 )
		{
			...
			SimplifyError = Merged.SimplifyTriangles( *this, TargetNumTris );
		}
	}
	...
	{
		Merged = FCluster( *this, Group.Children );
		uint32 TargetNumCurves,TargetNumSegments;
		GetClusterCurveTargetMetric(Merged, NumParents, TargetClusterSize, TargetNumCurves, TargetNumSegments);
		SimplifyError = Merged.SimplifyCurves( *this, TargetNumCurves, TargetNumSegments);
	}
	...
	if( !bVoxels)
	{
		check(bAllTriangles || bAllCurves);
