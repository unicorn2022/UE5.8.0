...
public:
	...
	bool TryReadFile(FStringView FileName, TArray<uint8>& OutData, FOutputDevice* ErrorHandler, UE::FileUtilities::FZipFileMetaData* OutMetadata) const;
};
...
bool FZipArchiveReader::TryReadFile(
	FStringView FileName, TArray<uint8>& OutData, FOutputDevice* ErrorHandler, UE::FileUtilities::FZipFileMetaData* OutMetadata
	) const
{
	return Impl->TryReadFile(FileName, OutData, ErrorHandler, OutMetadata);
}
...
bool FZipArchiveReader::FImpl::TryReadFile(
	FStringView FileName, TArray<uint8>& OutData, FOutputDevice* ErrorHandler, UE::FileUtilities::FZipFileMetaData* OutMetadata
	) const
{
	OutData.Reset();
	...
	if (OutMetadata && ZipFileStat.valid & ZIP_STAT_MTIME)
	{
		OutMetadata->Timestamp = FDateTime::FromUnixTimestamp(ZipFileStat.mtime);
	}
