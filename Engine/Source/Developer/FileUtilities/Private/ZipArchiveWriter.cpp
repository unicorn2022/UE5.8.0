...
{
	UE_LOGF(LogZipArchive, Display, "Closing zip file with %d entries.", Files.Num());
...
void FZipArchiveWriter::Flush()
{
	if (Buffer.Num())
	{
		...
		{
			UE_LOGF(LogZipArchive, Error, "Failed to write to zip file. Zip file writing aborted.");
			delete File;
			File = nullptr;
		}
