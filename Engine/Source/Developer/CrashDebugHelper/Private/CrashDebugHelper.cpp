...
bool ICrashDebugHelper::Init()
{
	...
	UE_LOGF( LogCrashDebugHelper, Log, "DepotName: %ls", *CrashInfo.DepotName );
	UE_LOGF( LogCrashDebugHelper, Log, "BuiltFromCL: %i", CrashInfo.BuiltFromCL );
	UE_LOGF( LogCrashDebugHelper, Log, "EngineVersion: %ls", *CrashInfo.EngineVersion );
	UE_LOGF( LogCrashDebugHelper, Log, "BuildVersion: %ls", *CrashInfo.BuildVersion );
...
bool ICrashDebugHelper::ReadSourceFile( TArray<FString>& OutStrings )
{
	...
	if (FFileHelper::LoadFileToString( Line, *FilePath ))
	{
		...
		UE_LOGF( LogCrashDebugHelper, Log, "Reading a single source file: %ls", *FilePath );
		return true;
	}
	else
	{
		UE_LOGF( LogCrashDebugHelper, Warning, "Failed to open source file: %ls", *FilePath );
		return false;
	}
}
...
{
	UE_LOGF( LogCrashDebugHelper, Warning, "%ls", *Line );
...
void FCrashInfo::GenerateReport( const FString& DiagnosticsPath )
{
	...
	if( ReportFile != NULL )
	{
		...
		for( int32 ModuleIndex = 0; ModuleIndex < Modules.Num(); ModuleIndex++ )
		{
			...
			ModuleDetail += FString::Printf( TEXT( " 0x%016llx 0x%08x" ), Module.BaseOfImage, Module.SizeOfImage );
