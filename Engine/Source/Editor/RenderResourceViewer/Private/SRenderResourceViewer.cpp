...
void SRenderResourceViewerWidget::InitCommandList()
{
	...
	CommandList->MapAction(FindInContentBrowserCommand,
...
bool SRenderResourceViewerWidget::ContextMenu_FindInContentBrowser_CanExecute() const
{
	...
	{
		for (const TSharedPtr<FRHIResourceStats>& SelectedNode : SelectedNodes)
		{
			if (SelectedNode && !SelectedNode->OwnerName.IsNone())
			{
				return true;
			}
		}
