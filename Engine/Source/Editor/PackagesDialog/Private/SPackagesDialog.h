...
public:
		...
		, IconToolTip(MoveTemp(InIconToolTip))
	{
		...
		if (const UObject* AssetObject = GetPackageObject())
		{
			AssetIcon = FSlateIconFinder::FindIconForClass(AssetObject->GetClass());
		}

		// Falling back on default icon
		if (!AssetIcon.IsSet())
		{
			AssetIcon = FSlateIconFinder::FindIconForClass(UObject::StaticClass());
		}
	}

	/**
	 * @return FSlateIcon the icon for the underlying asset object class or default object icon
	 */
	const FSlateIcon& GetAssetIcon() const { return AssetIcon; }
...
private:
	...
	/** Icon for the asset class */
	FSlateIcon AssetIcon;
};
