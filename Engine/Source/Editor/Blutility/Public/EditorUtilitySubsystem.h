...
public:
	...
	/** Given an editor utility widget blueprint, get the TabID for it. This will return None if the widget is not currently in a tab.*/
	UFUNCTION(BlueprintCallable, Category = "Development|Editor")
	UE_API FName GetTabIDFromBlueprint(class UEditorUtilityWidgetBlueprint* InBlueprint);
...
private:
	FName GenerateNewTabID(const UObject* WidgetObject, const FName& OptionalPostfix);
	
	void ExtendTabContextMenu(FMenuBuilder& InMenuBuilder, FName InTabId);
	void OnBrowseAssetInContentBrowser(FName InTabId);
	bool OnCanBrowseAssetInContentBrowser(FName InTabId);
