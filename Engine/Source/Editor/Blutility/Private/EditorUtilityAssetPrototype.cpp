...
namespace AssetActionUtilityConsoleVariables
{
	bool bHideAmbiguousScriptedAssetActions = true;
	FAutoConsoleVariableRef CVarEnableMutatorsSubsystem(
		TEXT("AssetActionUtility.HideAmbiguousScriptedAssetActions"),
		bHideAmbiguousScriptedAssetActions,
		TEXT("If enabled, Scripted Asset Actions will only show up if they explicitly define Supported Class(es) via their Class Defaults.\n0: Disable, 1: Enable (default)"),
		ECVF_Default);
...
TArray<TSoftClassPtr<UObject>> FAssetActionUtilityPrototype::GetSupportedClasses() const
{
	...
	{
		// If this cvar is set, we DON'T assume an empty array means we support UObject and derived classes
		// This cleans up the "Scripted Asset Actions" list and ensures only actions that explicitly support the asset class being requested show up
		if (!AssetActionUtilityConsoleVariables::bHideAmbiguousScriptedAssetActions)
		{
