...
public:
	...
	/** Moves the scrub cursor to Time (seconds since the recording started), clamping to the recording range and disabling auto-scroll-with-recording. */
	virtual void SetScrubTime(double Time) = 0;

	/** Re-centers the visible view range on Time, preserving the current view width. Independent of the scrub cursor. */
	virtual void CenterViewOnTime(double Time) = 0;
	...
	/**
	 * Returns shared pointer to the selected debug session (PIE or remote).
	 * @return the currently selected debug session
	 */
	virtual TSharedPtr<UE::TraceBasedDebuggers::FSessionInfo> GetSelectedDebugSessionInfo() const = 0;

	/**
	 * Returns pointer to the current analysis sessions for immediate access.
	 * If the goal is to store in a variable for later access then GetAnalysisSessionAsShared should be used instead.
	 * @return the current analysis session
	 * @see GetAnalysisSessionAsShared
	 */
	virtual const TraceServices::IAnalysisSession* GetAnalysisSession() const = 0;

	/**
	 * Returns shared pointer to the current analysis sessions.
	 * @return the current analysis session
	 */
	virtual TSharedPtr<const TraceServices::IAnalysisSession> GetAnalysisSessionAsShared() const = 0;
	...
	UE_DEPRECATED(5.8, "Method is no longer used and will be removed.")
	virtual bool IsTraceFileLoaded() const final
	{
		return false;
	}
