...
public:
	...
	void AddAction(const FString& Category, TSharedPtr<FEdGraphSchemaAction> Action)
	{
		...
		SortedActionsByCategoryRoot.Add(RootCategory, Action);
	}
	...
	void GetAllActions(FGraphActionListBuilderBase& OutActions)
	{
		...
		for (const TPair<int32, TSharedPtr<FEdGraphSchemaAction>>& Iter : SortedActions)
		{
	...
	bool HasCategory(const FString& Category, const NodeSectionID::Type SectionID) const
	{
		const int32 RootCategoryDelim = Category.Find(TEXT("|"));
		const FName RootCategory = RootCategoryDelim == INDEX_NONE ? *Category : *Category.Left(RootCategoryDelim);
		for (TMultiMap<FName, TSharedPtr<FEdGraphSchemaAction>>::TConstKeyIterator It = SortedActionsByCategoryRoot.CreateConstKeyIterator(RootCategory); It; ++It)
		{
			if (It.Value()->GetSectionID() == SectionID)
			{
				//It contains the root but might not contain the full path.
				if (RootCategoryDelim == INDEX_NONE || It.Value()->GetCategory().ToString().StartsWith(Category))
				{
					return true;
				}
			}
...
private:
	...
	TMultiMap<FName, TSharedPtr<FEdGraphSchemaAction>> SortedActionsByCategoryRoot;
};
...
void SMyBlueprint::CollectAllActions(FGraphActionListBuilderBase& OutAllActions)
{
	...
	// Inject a placeholder for each pinned category that is missing.
	FBlueprintEditorModule& BlueprintEditorModule = FModuleManager::LoadModuleChecked<FBlueprintEditorModule>("Kismet");
	for (const UE::BlueprintEditor::FAdditionalCategory& Category : BlueprintEditorModule.GetAdditionalBlueprintCategories(BlueprintObj))
	{
		if (Category.Placeholders.IsEmpty())
		{
			continue;
		}

		for (const UE::BlueprintEditor::FAdditionalCategory::FPlaceholder& Placeholder : Category.Placeholders)
		{
			const FString CategoryDisplayName = FEditorCategoryUtils::GetCategoryDisplayString(Category.Category.ToString());
			if (!SortList.HasCategory(CategoryDisplayName, Placeholder.SectionID))
			{
				TSharedPtr<FEdGraphSchemaAction_Dummy> Action = MakeShareable(new FEdGraphSchemaAction_Dummy(
					Category.Category,
					Placeholder.MenuDescription,
					Placeholder.Tooltip,
					0,
					FText::GetEmpty(),
					static_cast<int32>(Placeholder.SectionID)));
				SortList.AddAction(CategoryDisplayName, Action);
			}
		}
	}
