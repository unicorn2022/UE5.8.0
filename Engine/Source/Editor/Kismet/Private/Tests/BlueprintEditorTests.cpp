...
namespace BlueprintEditorPromotionUtils
{
	...
	*/
	static void CompileBlueprint(UBlueprint* InBlueprint)
	{
		...
		{
			UE_LOGF(LogBlueprintEditorPromotionTests, Display, "Blueprint compiled successfully (%ls)", *InBlueprint->GetName());
		}
		else if (InBlueprint->Status == EBlueprintStatus::BS_UpToDateWithWarnings)
		{
			UE_LOGF(LogBlueprintEditorPromotionTests, Display, "Blueprint compiled successfully with warnings(%ls)", *InBlueprint->GetName());
		}
		else if (InBlueprint->Status == EBlueprintStatus::BS_Error)
		{
			UE_LOGF(LogBlueprintEditorPromotionTests, Display, "Blueprint failed to compile (%ls)", *InBlueprint->GetName());
		}
		else
		{
			UE_LOGF(LogBlueprintEditorPromotionTests, Error, "Blueprint is in an unexpected state after compiling (%ls)", *InBlueprint->GetName());
		}
	}
	...
	*/
	static void ConnectGraphNodes(UEdGraphNode* NodeA, const FName& PinAName, UEdGraphNode* NodeB, const FName& PinBName)
	{
		...
		{
			UE_LOGF(LogBlueprintEditorPromotionTests, Error, "Could not connect pins %ls and %ls ", *PinAName.ToString(), *PinBName.ToString());
		}
	}
