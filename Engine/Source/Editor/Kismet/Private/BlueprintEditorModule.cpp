...
FBlueprintEditorModule::FAdditionalCategoryHandle FBlueprintEditorModule::RegisterAdditionalBlueprintCategory(UE::BlueprintEditor::FAdditionalCategory InCategory)
{
	const uint32 RawHandle = NextAdditionalCategoryHandle++;
	const FAdditionalCategoryHandle Handle(RawHandle);
	RegisteredAdditionalCategories.Emplace(Handle, MoveTemp(InCategory));
	return Handle;
}

void FBlueprintEditorModule::UnregisterAdditionalBlueprintCategory(FAdditionalCategoryHandle Handle)
{
	if (Handle.IsValid())
	{
		RegisteredAdditionalCategories.RemoveAllSwap(
			[Handle](const TTuple<FAdditionalCategoryHandle, UE::BlueprintEditor::FAdditionalCategory>& Category)
			{
				return Category.Get<0>() == Handle;
			});
	}
}

TArray<UE::BlueprintEditor::FAdditionalCategory> FBlueprintEditorModule::GetAdditionalBlueprintCategories() const
{
	TArray<UE::BlueprintEditor::FAdditionalCategory> Result;

	for (const TTuple<FAdditionalCategoryHandle, UE::BlueprintEditor::FAdditionalCategory>& Category : RegisteredAdditionalCategories)
	{
		Result.Add(Category.Get<1>());
	}

	// Merge entries from UBlueprintEditorSettings
	for (const FAdditionalBlueprintCategory& Settings : GetDefault<UBlueprintEditorSettings>()->AdditionalBlueprintCategories)
	{
		if (!Settings.Name.IsEmpty())
		{
			UE::BlueprintEditor::FAdditionalCategory Converted;
			// Empty array is for all classes.
			if (!Settings.ClassFilter.IsNull())
			{
				Converted.ClassFilters.Add(Settings.ClassFilter);
			}
			Converted.Category = Settings.Name;
			Result.Add(MoveTemp(Converted));
		}
	}
	return Result;
}

TArray<UE::BlueprintEditor::FAdditionalCategory> FBlueprintEditorModule::GetAdditionalBlueprintCategories(TNotNull<const UBlueprint*> Blueprint) const
{
	TArray<UE::BlueprintEditor::FAdditionalCategory> Result;

	auto Filter = [Blueprint](const FSoftClassPath& Filter)
		{
			// If null, then the class is not loaded.
			//The Blueprint can't be loaded if the parent is not loaded. Then no need to load the class to test IsChildOf.
			const UClass* FilterClass = Filter.ResolveClass();
			return Blueprint->ParentClass && FilterClass && Blueprint->ParentClass->IsChildOf(FilterClass);
		};

	for (const TTuple<FAdditionalCategoryHandle, UE::BlueprintEditor::FAdditionalCategory>& Category : RegisteredAdditionalCategories)
	{
		const bool bPassesFilter = Category.Get<1>().ClassFilters.IsEmpty() || Category.Get<1>().ClassFilters.ContainsByPredicate(Filter);
		if (bPassesFilter)
		{
			Result.Add(Category.Get<1>());
		}
	...
	for (const FAdditionalBlueprintCategory& Settings : GetDefault<UBlueprintEditorSettings>()->AdditionalBlueprintCategories)
	{
		if (!Settings.Name.IsEmpty())
		{
			if (Settings.ClassFilter.IsNull() || Filter(Settings.ClassFilter))
			{
				UE::BlueprintEditor::FAdditionalCategory Converted;
				Converted.Category = Settings.Name;
				Converted.ClassFilters.Add(Settings.ClassFilter);
