...
void FBlueprintVarActionDetails::PopulateCategories(SMyBlueprint* MyBlueprint, TArray<TSharedPtr<FText>>& CategorySource)
{
	...
	{
		UE_LOGF(LogBlueprint, Error, "Blueprint %ls has NULL SkeletonGeneratedClass in FBlueprintVarActionDetails::PopulateCategories().  Cannot Populate Categories.", *GetNameSafe(Blueprint));
		return;
	}
	...
	for (const UE::BlueprintEditor::FAdditionalCategory& Category : BlueprintEditorModule.GetAdditionalBlueprintCategories(Blueprint))
	{
		if (IsNewCategorySource(Category.Category))
		{
			CategorySource.Add(MakeShared<FText>(Category.Category));
		}
	}
...
bool FBaseBlueprintGraphActionDetails::OnPinRenamed(UK2Node_EditablePinBase* TargetNode, const FName OldName, const FString& NewName)
{
	...
	if (TargetNode)
	{
		...
		if (bHasFunctionEntryNode)
		{
			if (!Graph)
			{
				// Function graph could be in the middle of being deleted
				return false;
			}
			...
			[
				...
				[
					...
					.OnVerifyTextChanged_Lambda([&](const FText& InNewText, FText& OutErrorMessage) -> bool
					{
						...
						if (!MyBlueprint)
						{
							// Probably during shutdown, just ignore the change
							return false;
						}
						if (InNewText.IsEmpty())
						{
						...
						if (InNewText.EqualTo(FText::FromString(MyBlueprint->GetName())))
						{
							OutErrorMessage = LOCTEXT("CategoryEqualsBlueprintName", "Cannot add a category with the same name as the blueprint.");
							return false;
						}
