...
void FImaginaryFiBData::ParseAllChildData_Internal(ESearchableValueStatus InSearchabilityOverride/* = ESearchableValueStatus::Searchable*/)
{
	if (UnparsedJsonObject.IsValid())
	{
		if (InSearchabilityOverride & ESearchableValueStatus::Searchable)
		{
			...
			for (const TPair<FJsonObject::FStringType, TSharedPtr<FJsonValue>>& MapValues : UnparsedJsonObject->Values)
			{
		...
		for (const TPair<FJsonObject::FStringType, TSharedPtr<FJsonValue>>& MapValues : UnparsedJsonObject->Values)
		{
			...
			if (!TrySpecialHandleJsonValue(KeyText, JsonValue))
			{
				...
				// Parse the value making it searchable
				ParseJsonValue(KeyText, KeyText, JsonValue, ParsedValues, false, InSearchabilityOverride);

				// For some property types like TMaps we want KeyText to be a search candidate since it's entered by the user.
				// In most other cases KeyText represents property or category names which we don't want to return as search results.
				if (AreKeysSearchable())
				{
					FSearchableValueInfo& ParsedValue = ParsedValues.AddDefaulted_GetRef();
					ParsedValue = FSearchableValueInfo(FText::FromString("Key"), KeyText, InSearchabilityOverride);
					ParsedTagsAndValues.Add(FindInBlueprintsHelpers::FSimpleFTextKeyStorage(KeyText), ParsedValue);
				}
...
void FImaginaryFiBData::ParseJsonValue(FText InKey, FText InDisplayKey, TSharedPtr< FJsonValue > InJsonValue, TArray<FSearchableValueInfo>& OutParsedValues, bool bIsInArray/*=false*/, ESearchableValueStatus InSearchabilityOverride/* = ESearchableValueStatus::Searchable*/)
{
	...
	{
		TSharedPtr< FCategorySectionHelper, ESPMode::ThreadSafe > ArrayCategory = MakeShareable(new FCategorySectionHelper(AsShared(), LookupTablePtr, InKey, /*bIsTagAndValueCategory=*/true));
	...
	else if (InJsonValue->Type == EJson::Object)
	{
		...
		// - The following category represents a map entry, return this (the map)
		...
		const bool bIsDefaultValueKey = InDisplayKey.EqualTo(FFindInBlueprintSearchTags::FiB_DefaultValue);
		const bool bIsTagAndValueCategory = bIsInArray || IsTagAndValueCategory() || bIsDefaultValueKey;

		// Evaluate now whether this child's keys should be searchable. Currently this requires peeking at the value syntax 
		// because we don't store metadata to detect that we're processing a TMap. It's detectable from value though.
		TSharedPtr<FJsonObject> JsonValueObject = InJsonValue->AsObject();
		const bool bAreChildKeysSearchable = bIsDefaultValueKey && ShouldSearchChildKeys(JsonValueObject);

		ECategorySectionHelperFlags CategoryFlags = ECategorySectionHelperFlags::None;
		if (bIsTagAndValueCategory)
		{
			CategoryFlags |= ECategorySectionHelperFlags::IsTagAndValue;
		}
		if (bAreChildKeysSearchable)
		{
			CategoryFlags |= ECategorySectionHelperFlags::AreKeysSearchable;
		}

		TSharedPtr< FCategorySectionHelper, ESPMode::ThreadSafe > SubObjectCategory = MakeShareable(new FCategorySectionHelper(AsShared(), JsonValueObject, LookupTablePtr, InDisplayKey, CategoryFlags));
...
FCategorySectionHelper::FCategorySectionHelper(FImaginaryFiBDataWeakPtr InOuter, TSharedPtr< FJsonObject > InUnparsedJsonObject, TMap<int32, FText>* InLookupTablePtr, FText InCategoryName, ECategorySectionHelperFlags Flags)
	: FImaginaryFiBData(InOuter, InUnparsedJsonObject, InLookupTablePtr)
	, CategoryName(InCategoryName)
	, bIsTagAndValue((Flags & ECategorySectionHelperFlags::IsTagAndValue) != ECategorySectionHelperFlags::None)
	, bAreKeysSearchable((Flags & ECategorySectionHelperFlags::AreKeysSearchable) != ECategorySectionHelperFlags::None)
{
}
	...
	, BlueprintPath(InBlueprintPath)
{
	...
	TSharedPtr< FCategorySectionHelper, ESPMode::ThreadSafe > InterfaceCategory = MakeShareable(new FCategorySectionHelper(nullptr, &LookupTable, FFindInBlueprintSearchTags::FiB_Interfaces, /*bIsTagAndValueCategory=*/true));
...
bool FImaginaryBlueprint::TrySpecialHandleJsonValue(FText InKey, TSharedPtr< FJsonValue > InJsonValue)
{
	...
	else if(!InKey.CompareTo(FFindInBlueprintSearchTags::FiB_Components))
	{
		...
		ComponentsWrapperObject->SetField(FFindInBlueprintSearchTags::FiB_Components.ToString(), InJsonValue);
		ParsedChildData.Add(MakeShareable(new FCategorySectionHelper(AsShared(), ComponentsWrapperObject, &LookupTable, FFindInBlueprintSearchTags::FiB_Components, /*bIsTagAndValueCategory=*/false, FCategorySectionHelper::FCategorySectionHelperCallback::CreateRaw(this, &FImaginaryBlueprint::ParseComponents))));
		bResult = true;
	}
...
void FImaginaryProperty::ParseAllChildData_Internal(ESearchableValueStatus InSearchabilityOverride)
{
	// Detect and cache whether PinCategory == PC_Struct. If so, it can represent:
	// a) a struct, or
	// b) a TMap whose key type is a struct
	// The purpose for caching this is to detect TMap with struct keys, which must be searchable.
	// We don't explicitly save whether some search metadata represents a TMap default value.
	for (auto MapValues : UnparsedJsonObject->Values)
	{
		FText KeyText = FindInBlueprintsHelpers::AsFText(FCString::Atoi(*MapValues.Key), *LookupTablePtr);
		if (KeyText.EqualTo(FFindInBlueprintSearchTags::FiB_PinCategory))
		{
			const FText PinCategoryText = FindInBlueprintsHelpers::AsFText(FCString::Atoi(*MapValues.Value->AsString()), *LookupTablePtr);
			static FText PinCategoryStructValue = FText::FromName(UEdGraphSchema_K2::PC_Struct);
			bIsStructProperty = PinCategoryText.EqualTo(PinCategoryStructValue);
		}
	}
...
bool FImaginaryProperty::ShouldSearchChildKeys(TSharedPtr<FJsonObject> ChildNode) const
{
	// There are two scenarios where we want to search child keys:
	// - This is a TMap with a struct key type
	// - This is a TMap with a plain-old-data (POD) key type
	// In both cases ChildNode represents a key-value pair where we want to search the keys.
	if (bIsStructProperty)
	{
		// Entering here implies this FProperty is either a struct or a TMap with a struct key. Then:
		// - For structs, child nodes represent member properties where the key is the member name like 'MyMemberProperty'.
		// - For TMaps, child nodes represent key-value pairs where the keys store the struct's FStructProperty::ExportText output like '(MyMemberProperty=111)'.
		// Detect the latter case by peaking at the first element's syntax. This is not great, but implementing
		// TMap key searchability isn't worth changing the metadata format for.
		return !ChildNode->Values.IsEmpty() && FindInBlueprintsHelpers::AsFText(FCString::Atoi(*ChildNode->Values.Array()[0].Key), *LookupTablePtr).ToString().StartsWith("(");
	}

	// Entering here implies this is a TMap with a POD key type, because actual 
	// POD FProperties don't have FJsonObject child nodes.
	return true;
}
