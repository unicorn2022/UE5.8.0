...
void FBlueprintEditor::CreateEditorModeManager()
{
...
void FBlueprintEditor::OnClose()
{
	...
	const bool bHasSelectionSet = SelectionSet != nullptr;
	if (bHasSelectionSet)
	{
...
void FBlueprintEditor::AddViewportOverlayWidget(TSharedRef<SWidget> InViewportOverlayWidget, int32 ZOrder)
{
	if (SubobjectViewport.IsValid())
	{
		SubobjectViewport->AddOverlayWidget(InViewportOverlayWidget, ZOrder);
	}
}

void FBlueprintEditor::RemoveViewportOverlayWidget(TSharedRef<SWidget> InViewportOverlayWidget)
{
	if (SubobjectViewport.IsValid())
	{
		SubobjectViewport->RemoveOverlayWidget(InViewportOverlayWidget);
	}
}
...
void FBlueprintEditor::OnSelectionUpdated(const TArray<TSharedPtr<FSubobjectEditorTreeNode>>& SelectedNodes)
{
	...
	if (Inspector.IsValid())
	{
		...
		const bool bHasSceneComponent = InspectorObjects.ContainsByPredicate([](const UObject* Obj) { return Obj && Obj->IsA<USceneComponent>(); });
		GetEditorModeManager().SetSelectionHasSceneComponent(bHasSceneComponent);
	...
	// Notify each registered implementation (including ModeTools SelectionSet)
...
struct FLoadObjectsFromAssetRegistryHelper
{
	...
	static void Load(TSet<TWeakObjectPtr<TObjectType>>& Collection)
	{
		...
		UE_LOGF(LogBlueprint, Log, "Loading all assets of type: %ls took %.2f seconds", *TObjectType::StaticClass()->GetName(), static_cast<float>(FinishTime - CompileStartTime));
	}
};
...
void FBlueprintEditor::CommonInitialization(const TArray<UBlueprint*>& InitBlueprints, bool bShouldOpenInDefaultsMode)
{
	...
	// Only register our component common actions customization in Standalone mode.
	if (!IsWorldCentricAssetEditor())
	{
		const TSharedPtr<IToolkitHost> PinnedToolkitHost = ToolkitHost.Pin();
		if (ensure(PinnedToolkitHost.IsValid()))
		{
			TUniquePtr<FComponentElementSubobjectEditorCommonActionsCustomization> ComponentCustomization = MakeUnique<FComponentElementSubobjectEditorCommonActionsCustomization>();
			ComponentCustomization->SetSubobjectEditor(GetSubobjectEditor());

			PinnedToolkitHost->GetCommonActions()->RegisterInterfaceCustomizationByTypeName(NAME_Components, MoveTemp(ComponentCustomization));
		}
	}
	...
	if (UBlueprint* BP = GetBlueprintObj())
	{
		...
		if (BP->BlueprintType != BPTYPE_Interface)
		{
			...
			if (ExpensiveObjects.Num() > 0)
			{
				...
				UE_LOGF(LogBlueprintEditor, Log, "Perf: %.1f total seconds to load all %d blueprint libraries in project. Avoid references to content in blueprint libraries to shorten this time.", TotalSeconds, NumLibariesLoaded);
				...
				for (int32 i=0; i < ExpensiveObjects.Num() && i < MaxExpensiveObjectsToList; ++i)
				{
					...
					UE_LOGF(LogBlueprintEditor, Log, "Perf: %.1f seconds loading: %ls", ExpensiveObjectRecord.Seconds, *ExpensiveObjectRecord.Path.ToString());
				}
			}
		}
	}
	...
	TSharedPtr<ISCSEditorUICustomization> InSCSEditorUICustomization)
{
	...
	GetEditorModeManager().SetDefaultMode(UBlueprintEditorEdMode::Id);
	...
	if (const TSharedPtr<SSCSEditorViewport> Viewport = GetSubobjectViewport())
	{
		...
		if (const TSharedPtr<FEditorViewportClient>& ViewportClient = Viewport->GetViewportClient())
		{
			...
			{
				// Opt int to new ITF gizmos and viewport inputs
...
void FBlueprintEditor::PostRegenerateMenusAndToolbars()
{
	...
	{
		const FText NativeClassNameText = GetParentClassNativeNameText();
		...
		TSharedRef<SHorizontalBox> MenuOverlayBox = SNew(SHorizontalBox)
			...
			[
				...
				.ToolTipText(FText::Format(LOCTEXT("GoToCode_ToolTip", "Click to open the source file for the underlying class ({0}) in {1}"), 
					NativeClassNameText, FSourceCodeNavigation::GetSelectedSourceCodeIDE()))
			]
			+ SHorizontalBox::Slot()
			.AutoWidth()
			.VAlign(VAlign_Center)
...
FText FBlueprintEditor::GetParentClassNativeNameText() const
{
	UClass* ParentClass = (GetBlueprintObj() != nullptr) ? *GetBlueprintObj()->ParentClass : nullptr;
	return (ParentClass != nullptr) ? FText::FromName(ParentClass->GetFName()) : LOCTEXT("BlueprintEditor_NoParentClass", "None");
}
...
void FBlueprintEditor::OnGenerateSearchIndexForDebugging()
{
	...
	if (Blueprint)
	{
		...
		if (DumpFile)
		{
			...
			UE_LOGF(LogFindInBlueprint, Log, "Wrote search index to %ls", *FullPath);
		}
	}
}
...
void FBlueprintEditor::OnDumpCachedIndexDataForBlueprint()
{
	...
	if (Blueprint)
	{
		...
		if (DumpFile)
		{
			...
			UE_LOGF(LogFindInBlueprint, Log, "Wrote cached index data to %ls", *FullPath);
		}
	}
}
...
void FBlueprintEditor::ReparentBlueprint_NewParentChosen(UClass* ChosenClass)
{
	...
	if ((BlueprintObj != nullptr) && (ChosenClass != nullptr) && (ChosenClass != BlueprintObj->ParentClass))
	{
		...
		if ( bReparent )
		{
			...
			UE_LOGF(LogBlueprint, Warning, "Reparenting blueprint %ls from %ls to %ls...", *BlueprintObj->GetFullName(), BlueprintObj->ParentClass ? *BlueprintObj->ParentClass->GetName() : TEXT("[None]"), *ChosenClass->GetName());
...
private:
		...
		for(UEdGraphPin* OldPin : OldNode->Pins)
		{
			if(OldPin && (OldPin->PinName != UEdGraphSchema_K2::PN_Self))
			{
				...
				if (NewPin)
				{
					...
					{
						UE_LOGF(LogBlueprint, Error, "FUpdatePastedNodes: Cannot connect pin '%ls' node '%ls'",
				...
				{
					UE_LOGF(LogBlueprint, Error, "FUpdatePastedNodes: Cannot find pin '%ls'", *OldPin->PinName.ToString());
					bResult = false;
				}
			}
		}
	...
	bool Replace(UK2Node_Literal* OldTarget, UK2Node_CallFunctionOnMember* OldCall)
	{
		...
		if (NewTarget)
		{
			...
			{
				UE_LOGF(LogBlueprint, Error, "FUpdatePastedNodes: Cannot connect new self.");
				bResult = false;
			}
		}
...
AActor* FBlueprintEditor::GetPreviewActor() const
{
	if (!PreviewScene.IsInitialized())
	{
		...
		{
			UE_LOGF(LogBlueprint, Warning, "Expected preview actor '%ls' to be garbage collected, but it was not:", *Actor->GetName());
