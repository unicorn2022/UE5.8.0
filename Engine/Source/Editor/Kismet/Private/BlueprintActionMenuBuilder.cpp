...
TSharedPtr<FBlueprintDragDropMenuItem> FBlueprintActionMenuItemFactory::MakeDragDropMenuItem(UBlueprintNodeSpawner const* SampleAction)
{
	...
	{
		UE_LOGF(LogBlueprintActionMenuItemFactory, Warning, "Unhandled (or invalid) spawner: '%ls'", *SampleAction->GetName());
	}
...
void FBlueprintActionMenuBuilder::RebuildActionList()
{
	...
	{
		UE_LOGF(LogBlueprintActionMenuItemFactory, Log, "=== UNFILTERED ACTIONS: %d", TotalActionCount);
		if (MenuItemListAddHelper->GetNumPendingActions() > 0)
		{
			UE_LOGF(LogBlueprintActionMenuItemFactory, Log, "==  PENDING ACTION LIST MEMSIZE: %0.02f MB", MenuItemListAddHelper->GetAllocatedSize() / 1024.0f / 1024.0f);
		}
		...
		for (int32 SectionIdx = 0; SectionIdx < MenuSections.Num(); ++SectionIdx)
		{
			...
			UE_LOGF(LogBlueprintActionMenuItemFactory, Log, "=== FILTER TEST PROFILE: %ls ===", *DisplayName);

			for (const FString& ProfileEntry : MenuSection->Filter.GetFilterTestProfile())
			{
				UE_LOGF(LogBlueprintActionMenuItemFactory, Log, "%ls", *ProfileEntry);
			}
		}
	}
#endif	// ENABLE_BLUEPRINT_ACTION_FILTER_PROFILING
...
void FBlueprintActionMenuBuilder::BuildCompleted()
{
	...
	if (CVarBPEnableActionMenuDumpToFile.GetValueOnGameThread())
	{
		...
		UE_LOGF(LogBlueprintActionMenuItemFactory, Log, "Dumping menu builder action list to \"%ls\"...", *DumpFilePath);
		
		const int32 NumActions = GetNumActions();

		TArray<FString> Actions;
		TStringBuilder<512> CurrentLine;
		for (int32 I = 0; I < NumActions; ++I)
		{
			const TSharedPtr<FEdGraphSchemaAction>& Action = GetSchemaAction(I);

			CurrentLine = TEXT("");
			for (const FString& Category : Action->GetCategoryChain())
			{
				CurrentLine.Append(FString::Printf(TEXT("%s|"), *Category));
			}
			CurrentLine.Append(Action->GetMenuDescription().ToString());
			Actions.Emplace(CurrentLine);
		}
		Actions.Sort(
			[](const FString& A, const FString& B)
			{
				return FTextComparison::CompareTo(A, B) < 0;
			}
		);
		for( const FString& Line : Actions )
		{
			DumpFile.Log(Line);
		}
