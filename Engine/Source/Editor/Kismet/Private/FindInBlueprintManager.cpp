...
FSearchDataVersionInfo FSearchDataVersionInfo::Current()
{
	return
	{
		.FiBDataVersion = EFiBVersion::FIB_VER_LATEST,
...
uint32 FStreamSearch::Run()
{
	...
	UE_LOGF(LogFindInBlueprint, Log, "Search completed in %0.2f seconds.", FPlatformTime::Seconds() - StartTime);
...
namespace FiBSerializationHelpers
{
	...
	*/
	...
	{
		return InVersionInfo.FiBDataVersion != EFiBVersion::FIB_VER_NONE && InVersionInfo.CustomVersions.GetAllVersions().Num() > 0;
	}
	...
	bool ValidateSearchDataVersionInfo(const FString& InAssetPath, const FString& InFiBData, FSearchDataVersionInfo& InOutVersionInfo)
	{
		...
		if (InOutVersionInfo.CustomVersions.GetAllVersions().Num() == 0)
		{
			// Determine the custom versions that the asset package was last serialized with
			...
			if (ensureMsgf(FPackageName::DoesPackageExist(PackageName, &PackageFilename), TEXT("FiB: Failed to map package to filename.")))
			{
				...
				if (ensureMsgf(PackageFile != nullptr, TEXT("FiB: Unable to open package to read file summary.")))
				{
					...
					*PackageFile << PackageFileSummary;
					...
					// Record all custom versions from the package file summary
					InOutVersionInfo.CustomVersions = PackageFileSummary.GetCustomVersionContainer();
				}
			}
		}
...
namespace BlueprintSearchMetaDataHelpers
{
	...
	protected:
		...
		virtual void WriteTextValue(const FText& Text) override
		{
			...
			if (int32* TableLookupValuePtr = ReverseLookupTable.Find(Text))
			{
			...
			else
			{
				...
				int32 TableLookupValue = LookupTable.Num();
				{
					...
					ReverseLookupTable.Add(Text, TableLookupValue);
				}
	...
	private:
		...
		struct FTextKeyMapFuncs : BaseKeyFuncs<int32, FText, /*bInAllowDuplicateKeys*/false>
		{
			static const FString& GetSourceString(const FText& InText)
			{
				const FString* SourceString = FTextInspector::GetSourceString(InText);
				check(SourceString);
				return *SourceString;
			}
			static const FText& GetSetKey(const TPair<FText, int32>& Element)
			{
				return Element.Key;
			}
			static bool Matches(const FText& A, const FText& B)
			{
				return GetSourceString(A).Equals(GetSourceString(B), ESearchCase::CaseSensitive);
			}
			static uint32 GetKeyHash(const FText& Key)
			{
				return FLocKey::ProduceHash(GetSourceString(Key));
			}
		};
		TMap< FText, int32, FDefaultSetAllocator, FTextKeyMapFuncs > ReverseLookupTable;
	};
	 ...
	 */
	bool CheckIfJsonValueIsSearchable( TSharedPtr< FJsonValue > InJsonValue, const FBPVariableDescription& InVariableDescription)
	{
		...
		else if(InJsonValue->Type == EJson::Object)
		{
			...
			for(TMap<FJsonObject::FStringType, TSharedPtr<FJsonValue>>::TIterator Iter = JsonObject->Values.CreateIterator(); Iter; ++Iter)
			{
				// Map variables should always be searchable. (Iter->Key, Iter->Value) represents a user entered key-value pair.
				// Even if the value is uninteresting (empty string, 0, etc) then the key could be interesting.
				const bool bIsMapVariable = InVariableDescription.VarType.ContainerType == EPinContainerType::Map;

				// If not a map variable, check per entry if the value is also 'interesting' (see comment at the top of this function).
				if (!bIsMapVariable && !CheckIfJsonValueIsSearchable(Iter->Value, InVariableDescription))
				{
	...
	void SaveVariableDescriptionToJson(const TSharedRef<TFindInBlueprintJsonStringWriter<PrintPolicy>>& InWriter, const UBlueprint* InBlueprint, const FBPVariableDescription& InVariableDescription)
	{
		...
		if (VariableProperty)
		{
			...
			if (BlueprintSearchMetaDataHelpers::CheckIfJsonValueIsSearchable(JsonValue, InVariableDescription))
			{
...
void FFindInBlueprintSearchManager::Initialize()
{
	...
	if (!UE::IsSavingPackage() || (UE::IsSavingPackage() && FModuleManager::Get().IsModuleLoaded(TEXT("AssetRegistry"))))
	{
	...
	else
	{
		...
		{
			UE_LOGF(LogBlueprint, Warning, "Find-in-Blueprints could not pre-cache all unloaded Blueprints due to the Asset Registry module being unable to initialize because a package is currently being saved. Pre-cache will not be reattempted!");
		}
	}
	...
	if(!UE::IsSavingPackage() && AssetRegistryModule && (!bDisableImmediateAssetDiscovery || !AssetRegistryModule->GetRegistry().IsLoadingAssets()))
	{
...
bool FFindInBlueprintSearchManager::ProcessEncodedValueForUnloadedBlueprint(FSearchData& SearchData)
{
	...
	if (!SearchData.AssetKeyForValue.IsNone() && SearchData.Value.IsEmpty())
	{
		...
		if (TempEncodedString.Len() == 0)
		{
			...
			UE_LOGF(LogFindInBlueprint, Warning, "%ls AssetData loaded with %ls has an invalid FiB tag!", *AssetData.GetSoftObjectPath().ToString(), *AssetPath);
...
void FFindInBlueprintSearchManager::OnAssetLoaded(UObject* InAsset)
{
	...
	if (BlueprintObject)
	{
		...
		{
			// That index should never have a valid Blueprint already, but if it does, it should be the same Blueprint!
			ensureMsgf(SearchData.IsMarkedForDeletion() || !SearchData.Blueprint.IsValid() || SearchData.Blueprint == BlueprintObject,
				TEXT("Blueprint in database has path %s and is being stomped by %s"), *(SearchData.AssetPath.ToString()), *AssetPath.ToString());
			ensureMsgf(SearchData.IsMarkedForDeletion() || !SearchData.Blueprint.IsValid() || SearchData.AssetPath == AssetPath,
				TEXT("Blueprint in database has path %s and is being stomped by %s"), *(SearchData.AssetPath.ToString()), *AssetPath.ToString());
		...
		while (SearchQuery->DeferredAssetPaths.Dequeue(AssetPath))
		{
			...
			if (SearchData.IsValid() && !SearchData.IsMarkedForDeletion())
			{
				...
				if (!SearchData.IsIndexingCompleted())
				{
					...
					{
						UE_LOGF(LogFindInBlueprint, Warning, "%ls has been unexpectedly excluded from indexing and will not be included in the search.", *AssetPath.ToString());
					}
...
void FFindInBlueprintSearchManager::BeginSearchQuery(const FStreamSearch* InSearchOriginator)
{
	...
	{
		UE_LOGF(LogBlueprint, Warning, "Find-in-Blueprints was not fully initialized, possibly due to problems being initialized while saving a package. Please explicitly initialize earlier!");
	}
...
FSearchData FFindInBlueprintSearchManager::QuerySingleBlueprint(UBlueprint* InBlueprint, bool bInRebuildSearchData)
{
	...
	if (!bIsDiffingBlueprint)
	{
		...
		else if(bInRebuildSearchData)
		{
			...
			UE_LOGF(LogBlueprint, Warning, "Attempted to query a Blueprint (%ls) that was not re-indexed even after rebuilding. No results can be returned.", *InBlueprint->GetPathName());
		}
	}
	else
	{
		// Also warn here as we do not index diff-only packages.
		UE_LOGF(LogBlueprint, Warning, "Attempted to query an old Blueprint package opened for diffing!");
	}
	return FSearchData();
}
...
void FFindInBlueprintSearchManager::BuildCache()
{
	...
	ClassFilter.bIncludeOnlyOnDiskAssets = true;
...
void FFindInBlueprintSearchManager::ExportOutdatedAssetList()
{
	...
	if (FArchive* Ar = IFileManager::Get().CreateFileWriter(*FullPath))
	{
		...
		UE_LOGF(LogFindInBlueprint, Log, "%ls", *ExportConfirmationText.ToString());
	...
	else
	{
		...
		UE_LOGF(LogFindInBlueprint, Log, "%ls", *ExportFailureText.ToString());
...
TSharedPtr< FJsonObject > FFindInBlueprintSearchManager::ConvertJsonStringToObject(FSearchDataVersionInfo InVersionInfo, const FString& InJsonString, TMap<int32, FText>& OutFTextLookupTable)
{
	...
	// Configure the JSON stream with the proper custom versions for serialization when reading the LUT
	ReaderStream.SetCustomVersions(InVersionInfo.CustomVersions);
	...
	virtual bool Exec_Editor(class UWorld* InWorld, const TCHAR* Cmd, FOutputDevice& Ar)
	{
		if (FParse::Command(&Cmd, TEXT("DUMPFIBINDEXCACHE")))
		{
			...
			if (DumpFile)
			{
				...
				UE_LOGF(LogFindInBlueprint, Log, "Wrote full index cache to %ls", *FullPath);
			}
			return true;
		}
		return false;
