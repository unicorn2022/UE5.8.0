...
TSharedRef<FBlueprintDebugObjectInstance> SBlueprintEditorSelectedDebugObjectWidget::NoDebugInstance = MakeShared<FBlueprintDebugObjectInstance>(nullptr, GetNoDebugString());

SBlueprintEditorSelectedDebugObjectWidget::~SBlueprintEditorSelectedDebugObjectWidget()
{
	if (UObjectInitialized() && GEditor)
	{
		if (UAssetEditorSubsystem* AssetEditorSubsystem = GEditor->GetEditorSubsystem<UAssetEditorSubsystem>())
		{
			AssetEditorSubsystem->GetDebugObjectDelegateForClass(UObject::StaticClass())->RemoveAll(this);
		}
	}
}
...
void SBlueprintEditorSelectedDebugObjectWidget::Construct(const FArguments& InArgs, TSharedPtr<FBlueprintEditor> InBlueprintEditor)
{
	...
	NoDebugInstance->bPinned = true;

	TSharedPtr<FBlueprintDebugObjectInstance> CurrentDebugInstance = GetDebugObjectInstance();
	...
	DebugObjectsComboBox = SNew(SComboBox<TSharedPtr<FBlueprintDebugObjectInstance>>)
		...
		.InitiallySelectedItem(CurrentDebugInstance)
	...
	if (UAssetEditorSubsystem* AssetEditorSubsystem = GEditor->GetEditorSubsystem<UAssetEditorSubsystem>())
	{
		AssetEditorSubsystem->GetDebugObjectDelegateForClass(UObject::StaticClass())
			->AddSP(this, &SBlueprintEditorSelectedDebugObjectWidget::HandleOnDebugObjectAccess);
	}

	// If we construct this widget with an already debugged object, the object should be pinned as it was opened via debug object access.
	if (CurrentDebugInstance.IsValid())
	{
		CurrentDebugInstance->bPinned = true;
	}
}
...
const FString& SBlueprintEditorSelectedDebugObjectWidget::GetNoDebugString()
{
...
void SBlueprintEditorSelectedDebugObjectWidget::GenerateDebugObjectInstances(bool bRestoreSelection)
{
	...
	TArray<TWeakObjectPtr<UObject>> LastPinnedObjects;
	LastPinnedObjects.Reserve(DebugObjects.Num());

	for (const TSharedPtr<FBlueprintDebugObjectInstance>& DebugObject : DebugObjects)
	{
		if (DebugObject.IsValid() && DebugObject->bPinned && DebugObject->ObjectPtr.IsValid())
		{
			LastPinnedObjects.Add(DebugObject->ObjectPtr);
		}
	}

	// Empty the lists of actors and regenerate them
	DebugObjects.Empty();
	DebugObjects.Add(NoDebugInstance);
	...
	for (const TSharedPtr<FBlueprintDebugObjectInstance>& DebugObject : DebugObjects)
	{
		if (DebugObject == NoDebugInstance)
		{
			continue;
		}

		if (DebugObject.IsValid() && DebugObject->ObjectPtr.IsValid() && LastPinnedObjects.Contains(DebugObject->ObjectPtr))
		{
			DebugObject->bPinned = true;
		}
	}
...
TSharedRef<SWidget> SBlueprintEditorSelectedDebugObjectWidget::CreateDebugObjectItemWidget(TSharedPtr<FBlueprintDebugObjectInstance> InItem)
{
	...
	if (!InItem->ObjectPtr.IsExplicitlyNull())
	{
		Container->AddSlot()
			.AutoWidth()
			.VAlign(VAlign_Center)
			[
				SNew(SImage)
				.Image(this, &SBlueprintEditorSelectedDebugObjectWidget::GetDebugObjectItemWidgetPinBrush, InItem.ToWeakPtr())
				.Visibility(this, &SBlueprintEditorSelectedDebugObjectWidget::GetDebugObjectItemWidgetPinVisibility, InItem.ToWeakPtr(), StaticCastWeakPtr<SWidget>(Container.ToWeakPtr()))
				.DesiredSizeOverride(FVector2D(16.f))
				.OnMouseButtonDown(this, &SBlueprintEditorSelectedDebugObjectWidget::OnDebugObjectItemWidgetPinMouseDown, InItem.ToWeakPtr())
			];
	}
...
void SBlueprintEditorSelectedDebugObjectWidget::HandleOnDebugObjectAccess(UObject* InDebugObject)
{
	for (const TSharedPtr<FBlueprintDebugObjectInstance>& DebugObject : DebugObjects)
	{
		if (DebugObject.IsValid() && DebugObject->ObjectPtr.Get() == InDebugObject)
		{
			DebugObject->bPinned = true;
			LastObjectObserved = InDebugObject;
			break;
		}
	}
}

void SBlueprintEditorSelectedDebugObjectWidget::SortDebugObjects()
{	
	DebugObjects.StableSort(
		[](const TSharedPtr<FBlueprintDebugObjectInstance>& A, const TSharedPtr<FBlueprintDebugObjectInstance>& B)
		{
			if (A == NoDebugInstance)
			{
				return true;
			}

			if (B == NoDebugInstance)
			{
				return false;
			}
...
const FSlateBrush* SBlueprintEditorSelectedDebugObjectWidget::GetDebugObjectItemWidgetPinBrush(TWeakPtr<FBlueprintDebugObjectInstance> InInstanceWeak) const
{
	if (TSharedPtr<FBlueprintDebugObjectInstance> Instance = InInstanceWeak.Pin())
	{
		if (Instance->bPinned)
		{
...
EVisibility SBlueprintEditorSelectedDebugObjectWidget::GetDebugObjectItemWidgetPinVisibility(TWeakPtr<FBlueprintDebugObjectInstance> InInstanceWeak, TWeakPtr<SWidget> InRowWidgetWeak) const
{
	if (TSharedPtr<FBlueprintDebugObjectInstance> Instance = InInstanceWeak.Pin())
	{
		if (Instance->bPinned)
		...
		if (TSharedPtr<SWidget> RowWidget = InRowWidgetWeak.Pin())
		{
			if (RowWidget->GetParentWidget().IsValid())
			{
				if (RowWidget->GetParentWidget()->IsHovered())
				{
					return EVisibility::Visible;
				}
			}
...
FReply SBlueprintEditorSelectedDebugObjectWidget::OnDebugObjectItemWidgetPinMouseDown(const FGeometry& InGeometry, const FPointerEvent& InPointerEvent, TWeakPtr<FBlueprintDebugObjectInstance> InInstanceWeak)
{
	if (TSharedPtr<FBlueprintDebugObjectInstance> Instance = InInstanceWeak.Pin())
	{
		Instance->bPinned = !Instance->bPinned;
	}
