...
namespace UE::Kismet::BlueprintCompilationManager::Private
{
	namespace ConsoleVariables
	{
		...
		static FAutoConsoleVariableRef CVarAllowFastReinstancing(
			...
			TEXT("False by default, this flag enables a fast path in the logic that updates class \
...
namespace UE::Private
{
	TUniquePtr<IClassReinstancer> CreateBulkNewClassReinstancer(TArray<UClass*>& InOutClasses)
	{
		return MakeUnique<FBulkNewClassReinstancer>(InOutClasses);
	}
}
...
FBlueprintCompilationManagerImpl::FBlueprintCompilationManagerImpl()
{
	...
	UE::Private::GCreateClassReinstancerFPtr = &UE::Private::CreateBulkNewClassReinstancer;
}
...
{ 
	...
	UE::Private::GCreateClassReinstancerFPtr = nullptr;
}
...
void FBlueprintCompilationManagerImpl::CompileSynchronouslyImpl(const FBPCompileRequest& Request)
{
	...
	ScopeLock.Unlock();
#endif
	// We need to save outside the compilation lock because saving can require some additional loading.
...
{
	FArchetypeInfo(UObject* InArchetype, FReinstancingJob* InReinstancingJob = nullptr)
		: Archetype(InArchetype)
		, ArchetypeTemplate(InArchetype ? InArchetype->GetArchetype() : nullptr)
		, ReinstancingJob(InReinstancingJob)
	...
	UObject* ArchetypeTemplate = nullptr;
	FReinstancingJob* ReinstancingJob = nullptr;
};
...
void FBlueprintCompilationManagerImpl::FlushCompilationQueueImpl(bool bSuppressBroadcastCompiled, TArray<UBlueprint*>* BlueprintsCompiled, TArray<UBlueprint*>* BlueprintsCompiledOrSkeletonCompiled, FUObjectSerializeContext* InLoadContext, TMap<UClass*, TMap<UObject*, UObject*>>* OldToNewTemplates /* = nullptr*/)
{
	...
	{ // begin TimeCompiling scope 
		...
		*/
		...
		// Compute each blueprint's intended hierarchy depth using ParentClass (not the
		// stale GeneratedClass->SuperStruct). When the parent is itself in the compile
		// batch we recurse so the full chain is resolved correctly.
		TMap<const UBlueprint*, int32> IntendedDepths;
		const auto ComputeIntendedDepth = [&](const UBlueprint* BP, auto& Self) -> int32
		{
			if (const int32* Cached = IntendedDepths.Find(BP))
			{
				return *Cached;
			}
			IntendedDepths.Add(BP, 0); // cycle guard against malformed hierarchies
			int32 Depth = 0;
			for (UClass* Iter = BP->ParentClass; Iter; Iter = Iter->GetSuperClass())
			{
				++Depth;
				if (const UBlueprint* ParentBP = Cast<UBlueprint>(Iter->ClassGeneratedBy))
				{
					Depth += Self(ParentBP, Self);
					break;
				}
			}
			IntendedDepths.Add(BP, Depth);
			return Depth;
		};
		for (FCompilerData& Data : CurrentlyCompilingBPs)
		{
			ComputeIntendedDepth(Data.BP, ComputeIntendedDepth);
		}

		auto HierarchyDepthSortFn = [&IntendedDepths](const FCompilerData& CompilerDataA, const FCompilerData& CompilerDataB)
		{
			...
			const int32 DepthA = IntendedDepths.FindRef(&A);
			const int32 DepthB = IntendedDepths.FindRef(&B);
			if (DepthA != DepthB)
			{
...
void FBlueprintCompilationManagerImpl::BuildDSOMap(UObject* OldObject, UObject* NewObject, const TMap<UClass*, UClass*>& InOldToNewClassMap, TMap<UObject*, UObject*>& OutOldToNewDSO)
{
	...
	}, EGetObjectsFlags::None, /*ExclusionFlags*/ RF_MirroredGarbage);
...
void FBlueprintCompilationManagerImpl::ReinstanceBatch(TArray<FReinstancingJob>& Reinstancers, TMap< UClass*, UClass* >& InOutOldToNewClassMap, FUObjectSerializeContext* InLoadContext, TMap<UClass*, TMap<UObject*, UObject*>>* OldToNewTemplates /* = nullptr*/)
{
	...
	for (const FReinstancingJob* ReinstancingJobPtr : ReinstancersPtr)
	{
		...
		if (OldClass)
		{
			...
			if (OldCDO && ReinstancingJob.Reinstancer.IsValid())
			{
				...
				NewCDO->SetFlags(OldCDO->GetFlags() & RF_Reinst);
	...
	TSet<UObject*> OldArchetypeObjectsSet;
	...
	for (FReinstancingJob* ReinstancingJobPtr : ReinstancersPtr)
	{
		...
		if (OldClass)
		{
			...
			{
				OldArchetypeObjects.Emplace(Archetype, ReinstancingJobPtr);
				OldArchetypeObjectsSet.Add(Archetype);
	...
	// This happens when the class containing the archetype has derived classes or embed subobject of other classes.
	Algo::TopologicalSort(OldArchetypeObjects, [&OldArchetypeObjectsSet](const FArchetypeInfo& OldArchetypeInfo)
	{
		TArray<UObject*> Dependencies;
		if (OldArchetypeObjectsSet.Find(OldArchetypeInfo.ArchetypeTemplate))
		{
			Dependencies.Add(OldArchetypeInfo.ArchetypeTemplate);
		}

		auto GetArchetypeSubObjestDependencies = [&OldArchetypeObjectsSet, &Dependencies](UObject* OldArchetype, auto& Recurse) -> void
		{
			TArray<UObject*> OldArchetypeContainedObjects;
			GetObjectsWithOuter(OldArchetype, OldArchetypeContainedObjects, EGetObjectsFlags::None);
			for (UObject* OldArchetypeSubObject : OldArchetypeContainedObjects)
			{
				if (OldArchetypeObjectsSet.Find(OldArchetypeSubObject))
				{
					Dependencies.Add(OldArchetypeSubObject);
				}
				Recurse(OldArchetypeSubObject, Recurse);
			}
		};
		GetArchetypeSubObjestDependencies(OldArchetypeInfo.Archetype, GetArchetypeSubObjestDependencies);
	...
	{
		SCOPED_LOADTIMER_ASSET_TEXT(TEXT("FinishReinstancing Archetypes"));

		TMap<UObject*, FObjectInstancingGraph> GraphCache;

		for (const FArchetypeInfo& OldArchetypeInfo : OldArchetypeObjects)
		{
			checkf(OldArchetypeInfo.ReinstancingJob, TEXT("Expecting the job ptr to be set."));
			FReinstancingJob& ReinstancingJob = *OldArchetypeInfo.ReinstancingJob;
			UClass* OldClass = ReinstancingJob.OldToNew.Key;
			if (OldClass)
			{
				UClass* NewClass = ReinstancingJob.OldToNew.Value;
				...
				TMap<UObject*, UObject*> OldToNewInstanceMapTemp;
				TMap<UObject*, UObject*>* OldToNewTemplatesForClass = OldToNewTemplates ? &OldToNewTemplates->FindOrAdd(OldClass) : &OldToNewInstanceMapTemp;
				...
				TArray<UClass*> ClassesOfArchetypeOuterChain;
				FObjectInstancingGraph PrePopulatedInstancingGraph;
				const bool bUsePrePopulatedInstancingGraph = FBlueprintCompileReinstancer::PrePopulateInstancingGraphForArchetype(GraphCache, PrePopulatedInstancingGraph, OldInstance, Template, &OldArchetypeToNewArchetype);
				if (bUsePrePopulatedInstancingGraph)
				{
					// Make sure the old archetype has all it references patched up before using copying
					FArchiveReplaceObjectRef<UObject> ReplaceInOldInstanceAr(OldInstance, *OldToNewTemplatesForClass);

					// Make sure the template object has all it references patched up before using it to this archetype
					FArchiveReplaceObjectRef<UObject> ReplaceInCDOAr(Template, *OldToNewTemplatesForClass);

					// This needs to be done before we move the archetype to transient package is done, otherwise the outerchain is lost
					UObject* ArchetypeOuter = OldInstance->GetOuter();
					while (ArchetypeOuter)
					{
						if (InOutOldToNewClassMap.Find(ArchetypeOuter->GetClass()))
						{
							ClassesOfArchetypeOuterChain.Add(ArchetypeOuter->GetClass());
						}
						ArchetypeOuter = ArchetypeOuter->GetOuter();
					}
				}
				...
				UObject* NewArchetype = NewObject<UObject>(OriginalOuter, NewClass, OriginalName, OriginalFlags & RF_Reinst, Template, false, bUsePrePopulatedInstancingGraph ? &PrePopulatedInstancingGraph : nullptr);
				...
				if(NewArchetype)
				{
					...
					if (TMap<UObject*, UObject*>* OldToNewTemplateMapForSuperClass = OldToNewTemplates ? OldToNewTemplates->Find(OldClass->GetSuperClass()) : nullptr)
					{
						OldToNewTemplatesForClass->Append(*OldToNewTemplateMapForSuperClass);
					}

					TMap<UObject*, UObject*> CreatedInstanceMap;
					TArray< TTuple<UObject*, UObject*>> OrderedListOfObjectToCopy;
					FBlueprintCompileReinstancer::PreCreateSubObjectsForReinstantiation(InOutOldToNewClassMap, OldInstance, NewArchetype, CreatedInstanceMap, OldToNewTemplatesForClass, &OrderedListOfObjectToCopy);

					// We only need to copy properties of the pre-created instances, the rest of the default sub object is done inside the UEditorEngine::CopyPropertiesForUnrelatedObjects
					OldToNewTemplatesForClass->Append(CreatedInstanceMap);
					for (const TTuple<UObject*, UObject*>& Pair : OrderedListOfObjectToCopy)
					{
						FBlueprintCompileReinstancer::CopyPropertiesForUnrelatedObjects(Pair.Key, Pair.Value, /*bClearExternalReferences*/false, bUseDeltaSerialization, /*bOnlyHandleDirectSubObjects*/true, OldToNewTemplatesForClass, &InOutOldToNewClassMap);
					}
					FOverridableManager::Get().RestoreOverrideState(OldInstance, NewArchetype);

					// Append to any outer archetype classes, in case they are also re-instantiated during this pass.
					if (OldToNewTemplates)
					{
						for (UClass* OuterArchetypeClasses : ClassesOfArchetypeOuterChain)
						{
							OldToNewTemplates->FindOrAdd(OuterArchetypeClasses).Append(*OldToNewTemplatesForClass);
						}
					}
					...
					for (const TPair<UObject*, UObject*>& Pair : *OldToNewTemplatesForClass)
					{
	...
	for (const FReinstancingJob* ReinstancingJobPtr : ReinstancersPtr)
	{
		...
		if(UBlueprint* BP = Cast<UBlueprint>(ReinstancingJob.OldToNew.Value->ClassGeneratedBy))
		{
			...
			// Previously, this was CachedDependencies, but it doesn't make much sense to search for ourselves in the 
			// classes that we depend on. This has been modified to now search into the ones that depends on the BP instead.
			for(const TWeakObjectPtr<UBlueprint>& Dependent : BP->CachedDependents)
			{
				if (UBlueprint* DependentBP = Dependent.Get())
				{
					ArchetypeReferencers.Add(DependentBP);
				}
			}
		}
	}

	// Make sure all new archetypes get their cross references fixed up, most of them are already in the set so avoid modifying if possible
	for (const TPair<UObject*, UObject*>& Pair : OldArchetypeToNewArchetype)
	{
		if (!ArchetypeReferencers.Contains(Pair.Value))
		{
			ArchetypeReferencers.Add(Pair.Value);
		}
	}
	...
	const bool bIsInActualLoadingThread = IsInActualLoadingThread();
	for (TObjectIterator<UObjectRedirector> Itr; Itr; ++Itr)
	{
		...
		{
			// When we're in the loading thread, we limit the replacements to UObjectRedirector that the loader thread owns since
			// it would not be possible for an already loaded redirector to point to something still in the loader.
			if (bIsInActualLoadingThread && !Itr->HasAnyInternalFlags(EInternalObjectFlags::AsyncLoadingPhase1))
			{
				continue;
			}
...
UClass* FBlueprintCompilationManagerImpl::FastGenerateSkeletonClass(UBlueprint* BP, FKismetCompilerContext& CompilerContext, bool bIsSkeletonOnly, TArray<FSkeletonFixupData>& OutSkeletonFixupData)
{
	...
	FBlueprintEditorUtils::RemoveInvalidStructureMemberVariableFromBlueprint(BP);
		...
		for( const UEdGraph* Graph : Graphs )
		{
			...
			if(EntryNodes.Num() > 0)
			{
				...
				if(NewFunction)
				{
					...
					{
						FKismetCompilerContext::CreateUserDefinedLocalVariableForFunction(BPVD, NewFunction, Ret, PropertyListBuilder, Schema, MessageLog);
					}
...
{	
	...
	if(!Gather->bHasBeenRegenerated && Gather->GetLinker())
	{
		return;
	}
...
void FBlueprintCompilationManager::ReinstanceStruct(UUserDefinedStruct* Struct)
{
	FUserDefinedStructureCompilerUtils::ReplaceStructWithTempDuplicateSafe(Struct);
}
