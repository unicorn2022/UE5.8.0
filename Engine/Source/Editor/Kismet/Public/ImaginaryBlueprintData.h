...
public:
	...
	/** Returns TRUE if the child item's keys should be searchable. */
	virtual bool ShouldSearchChildKeys(TSharedPtr<FJsonObject> ChildNode) const { return false; }

	/** Returns TRUE if this item's keys should be searchable. For example if they don't represent property names, but values. Introduced for TMap keys. */
	virtual bool AreKeysSearchable() const { return false; }
...
enum class ECategorySectionHelperFlags : uint8
{
	None = 0,
	IsTagAndValue		= 1 << 0,	// See bIsTagAndValue
	AreKeysSearchable	= 1 << 1,	// See bAreKeysSearchable
};

ENUM_CLASS_FLAGS(ECategorySectionHelperFlags);
...
public:
	...
	FCategorySectionHelper(FImaginaryFiBDataWeakPtr InOuter, TSharedPtr< FJsonObject > InUnparsedJsonObject, TMap<int32, FText>* InLookupTablePtr, FText InCategoryName, ECategorySectionHelperFlags Flags);

	/** FImaginaryFiBData Interface */
	virtual bool IsCategory() const override { return true; }
	virtual bool AreKeysSearchable() const { return bAreKeysSearchable; }
...
protected:
	...
	uint8 bIsTagAndValue:1;

	/** TRUE if this category's keys should be searchable */
	uint8 bAreKeysSearchable:1;
};
...
public:
	...
	/** FImaginaryFiBData Interface */
	virtual bool IsCompatibleWithFilter(ESearchQueryFilter InSearchQueryFilter) const override;
	virtual void ParseAllChildData_Internal(ESearchableValueStatus InSearchabilityOverride) override;
	virtual bool ShouldSearchChildKeys(TSharedPtr<FJsonObject> ChildNode) const override;
	/** End FImaginaryFiBData Interface */
...
private:
	/** TRUE if this property's metadata's PinCategory was PC_Struct. If so, it can represent a struct, or a TMap whose key type is a struct. */
	uint8 bIsStructProperty:1;
};
