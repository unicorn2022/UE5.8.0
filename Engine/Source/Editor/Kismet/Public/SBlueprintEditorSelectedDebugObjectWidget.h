...
public:
	...
	UE_API virtual ~SBlueprintEditorSelectedDebugObjectWidget();
...
private:
	...
	UE_API static const FString& GetNoDebugString();
	...
	/** Handle selection of debug object through debug object access. */
	UE_API void HandleOnDebugObjectAccess(UObject* InDebugObject);

	/** Sorts the debug objects based on pinned status. */
	UE_API void SortDebugObjects();

	/** Returns either a pinned or unpinned image based on object pin status. */
	const FSlateBrush* GetDebugObjectItemWidgetPinBrush(TWeakPtr<FBlueprintDebugObjectInstance> InInstanceWeak) const;

	/** Returns Visible if the item is pinned or hovered. */
	EVisibility GetDebugObjectItemWidgetPinVisibility(TWeakPtr<FBlueprintDebugObjectInstance> InInstanceWeak, TWeakPtr<SWidget> InRowWidgetWeak) const;

	/** Pins or unpins the debug object. */
	FReply OnDebugObjectItemWidgetPinMouseDown(const FGeometry& InGeometry, const FPointerEvent& InPointerEvent, TWeakPtr<FBlueprintDebugObjectInstance> InInstanceWeak);

private:
	/** Pointer back to the blueprint editor tool that owns us */
	TWeakPtr<FBlueprintEditor> BlueprintEditor;

	static TSharedRef<FBlueprintDebugObjectInstance> NoDebugInstance;
