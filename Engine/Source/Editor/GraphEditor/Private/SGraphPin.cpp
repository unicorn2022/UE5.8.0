...
SGraphPin::SGraphPin()
	: PinImageHighDetailSlot(nullptr)
	, GraphPinObj(nullptr)
	...
	SBorder::Construct(SBorder::FArguments()
		...
		[
			...
			[
				...
				.OnGetActiveDetailSlotContent(this, &SGraphPin::OnGetActiveDetailSlotContent)
				.HighDetail()
				[
...
{
	IsEditable = MoveTemp(InIsEditable);
}
...
TSharedRef<SWidget> SGraphPin::OnGetActiveDetailSlotContent(bool bChangedToHighDetail) const
{
	if (PinImage)
	{
		// Here, we need to make sure we detach PinImage before setting it into the other slot
		PinImage->ConditionallyDetatchParentWidget(PinImage->GetParentWidget().Get());

		if (bChangedToHighDetail)
		{
			if (const TSharedPtr<SHorizontalBox> RowWidget = FullPinHorizontalRowWidget.Pin())
			{
				for (int32 i = 0; i < RowWidget->GetChildren()->NumSlot(); ++i)
				{
					if (&RowWidget->GetSlot(i) == PinImageHighDetailSlot)
					{
						RowWidget->GetSlot(i).AttachWidget(PinImage.ToSharedRef());
						break;
					}
				}
