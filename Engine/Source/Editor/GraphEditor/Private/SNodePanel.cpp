...
SLATE_IMPLEMENT_WIDGET(SNodePanel)
void SNodePanel::PrivateRegisterAttributes(FSlateAttributeInitializer& AttributeInitializer)
{
	SLATE_ADD_MEMBER_ATTRIBUTE_DEFINITION(AttributeInitializer, IsEditableAttribute, EInvalidateWidgetReason::Paint);
	SLATE_ADD_MEMBER_ATTRIBUTE_DEFINITION(AttributeInitializer, ViewOffsetAttribute, EInvalidateWidgetReason::Layout);
}


// equivalent of SLATE_IMPLEMENT_WIDGET(SNodePanel::SNode)
FSlateWidgetClassRegistration ClassRegistration__SNodePanel_SNode = SNodePanel::SNode::StaticWidgetClass();

void SNodePanel::SNode::PrivateRegisterAttributes(FSlateAttributeInitializer& AttributeInitializer)
{
	FSlateWidgetSlotAttributeInitializer Initializer = SLATE_ADD_PANELCHILDREN_DEFINITION(AttributeInitializer, Children);
	FNodeSlot::RegisterAttributes(Initializer);

	SLATE_ADD_MEMBER_ATTRIBUTE_DEFINITION(AttributeInitializer, DesiredSizeScaleAttribute, EInvalidateWidgetReason::Prepass);
}

void SNodePanel::SNode::FNodeSlot::RegisterAttributes(FSlateWidgetSlotAttributeInitializer& AttributeInitializer)
{
	TBasicLayoutWidgetSlot::RegisterAttributes(AttributeInitializer);

	SLATE_ADD_SLOT_ATTRIBUTE_DEFINITION_WITH_NAME(FNodeSlot, AttributeInitializer, "Slot.AllowScale", AllowScale, EInvalidateWidgetReason::Prepass | EInvalidateWidgetReason::Paint);
	SLATE_ADD_SLOT_ATTRIBUTE_DEFINITION_WITH_NAME(FNodeSlot, AttributeInitializer, "Slot.Size", Size, EInvalidateWidgetReason::Layout);
	SLATE_ADD_SLOT_ATTRIBUTE_DEFINITION_WITH_NAME(FNodeSlot, AttributeInitializer, "Slot.Offset", Offset, EInvalidateWidgetReason::Layout)
		.UpdatePrerequisite("Slot.Size");
}

void SNodePanel::SNode::FNodeSlot::Construct(const FChildren& SlotOwner, FSlotArguments&& InArgs)
{
	TBasicLayoutWidgetSlot<FNodeSlot>::Construct(SlotOwner, MoveTemp(InArgs));
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	{
		Offset.Assign(*this, MoveTemp(InArgs._SlotOffset2f));
	}
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	{
		SetSlotSize(MoveTemp(InArgs._SlotSize2f));
	}
	if (InArgs._AllowScaling.IsSet())
	{
		AllowScale.Assign(*this, MoveTemp(InArgs._AllowScaling));
	}
}
...
SNodePanel::SNodePanel()
	...
	, IsEditableAttribute(*this, true)
	, ViewOffsetAttribute(*this, FVector2f::ZeroVector)
{
...
{
	FChildren* ChildrenToArrange = ArrangedChildren.Accepts(EVisibility::Hidden) ? (FChildren*)&Children : (FChildren*)&VisibleChildren;

	// First pass nodes
	ChildrenToArrange->ForEachWidget([&, this](SWidget& ChildWidget)
	{
		SNode& SomeChild = static_cast<SNode&>(ChildWidget);
		if (!SomeChild.RequiresSecondPassLayout())
		{
			ArrangedChildren.AddWidget(AllottedGeometry.MakeChild(SomeChild.AsShared(), SomeChild.GetPosition2f() - GetViewOffset(), SomeChild.GetDesiredSize(), GetZoomAmount()));
		}
	});
...
FChildren* SNodePanel::GetVisibleChildren()
{
	return &VisibleChildren;
}

FChildren* SNodePanel::GetManagedChildren()
{
	return &Children;
}
...
{
	return ViewOffsetAttribute.Get();
}
...
void SNodePanel::Construct()
{
	...
	SetViewOffset(FVector2f::ZeroVector);
	TotalMouseDelta = 0;
	TotalMouseDeltaXY = 0;
	...
	SelectionManager.OnSelectionChanged.BindRaw(this, &SNodePanel::OnSelectionChanged);
}
...
void SNodePanel::UpdateViewOffset(const FGeometry& MyGeometry, const FVector2f& TargetPosition)
{
	...
	SetViewOffset(GetViewOffset() + PanAmount);
}
...
{
	// don't run this timer if we don't allow pan or zoom
	if (!bAllowPanning || !bAllowZoom)
	{
	...
	if (bDoneZooming && bDoneScrolling)
	{
		...
		SetViewOffset(DesiredViewCenter - ( 0.5f * CachedGeometry.GetLocalSize() / GetZoomAmount() ));
...
void SNodePanel::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
	...
	if (CurrentBookmarkGuid.IsValid() && (OldViewOffset != GetViewOffset() || OldZoomAmount != CurZoomAmount))
	{
	...
	if (!bCanMoveToTargetObjectThisFrame)
	{
...
FReply SNodePanel::OnTouchGesture( const FGeometry& MyGeometry, const FPointerEvent& GestureEvent )
{
	...
	else if (GestureType == EGestureEvent::Scroll)
	{
		...
		this->bIsPanning = true;
		SetViewOffset(GetViewOffset() - (bUseDirectionInvertedFromDevice == GestureEvent.IsDirectionInvertedFromDevice() ? GestureDelta : -GestureDelta) / GetZoomAmount());
...
void SNodePanel::PaintBackgroundAsLines(const FSlateBrush* BackgroundImage, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32& DrawLayerId) const
{
	...
	const float GraphSpaceGridX0 = FancyMod(GetViewOffset().X, Inflation * NominalGridSize * static_cast<float>(RulePeriod));
	const float GraphSpaceGridY0 = FancyMod(GetViewOffset().Y, Inflation * NominalGridSize * static_cast<float>(RulePeriod));
...
bool SNodePanel::ScrollToLocation(const FGeometry& MyGeometry, FVector2f DesiredCenterPosition, const float InDeltaTime)
{
	...
	FVector2f CurrentPosition = GetViewOffset() + HalfOFScreenInGraphSpace;

	FVector2f NewPosition = FMath::Vector2DInterpTo(CurrentPosition, DesiredCenterPosition, InDeltaTime, 10.f);
	SetViewOffset(NewPosition - HalfOFScreenInGraphSpace);
...
void SNodePanel::ChangeZoomLevel(int32 ZoomLevelDelta, const FVector2f& WidgetSpaceZoomOrigin, bool bOverrideZoomLimiting)
{
	...
	this->SetViewOffset(PointToMaintainGraphSpace - WidgetSpaceZoomOrigin / GetZoomAmount());
}

void SNodePanel::SetZoomLevel(int32 InZoomLevel)
{
	if (!ZoomLevels)
	{
		return;
	}
	if (InZoomLevel >= 0 && InZoomLevel < ZoomLevels->GetNumZoomLevels())
	{
...
void SNodePanel::OnSelectionChanged(const TSet<UObject*>& Objects)
{
	Invalidate(EInvalidateWidgetReason::Paint);
}
...
{
	return (!FMath::IsNearlyEqual(GetZoomAmount(), OldZoomAmount) || !GetViewOffset().Equals(OldViewOffset, SMALL_NUMBER));
}
