...
template <>
struct TWidgetTypeTraits<class SGraphPanel>
{
...
{
	SLATE_DECLARE_WIDGET_API(SGraphPanel, SNodePanel, UE_API)
public:
	...
	SLATE_BEGIN_ARGS( SGraphPanel )
		...
		, _ShouldDrawSurroundingShadow(true)
		, _OnUpdateGraphPanel()
		, _AllowZoom(true)
		, _AllowPanning(true)
		{
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
		...
		SLATE_ARGUMENT(bool, AllowPanning)
	SLATE_END_ARGS()
...
public:
	...
	bool IsGraphEditable() const { return GetIsEditable(); }
...
protected:
	...
	UE_API virtual void OnSelectionChanged(const TSet<UObject*>& Objects) override;
...
protected:
	void SetShowGraphStateOverlay(TAttribute<bool> InShowGraphStateOverlayAttribute) { ShowGraphStateOverlayAttribute.Assign(*this, MoveTemp(InShowGraphStateOverlayAttribute)); }
	bool GetShowGraphStateOverlay() const { return ShowGraphStateOverlayAttribute.Get(); }
	TSlateAttributeRef<bool> GetShowGraphStateOverlayAttribute() const { return TSlateAttributeRef<bool>{SharedThis(this), ShowGraphStateOverlayAttribute}; }

	void SetShouldDrawBackground(TAttribute<bool> InShouldDrawBackgroundAttribute) { ShouldDrawBackgroundAttribute.Assign(*this, MoveTemp(InShouldDrawBackgroundAttribute)); }
	bool GetShouldDrawBackground() const { return ShouldDrawBackgroundAttribute.Get(); }
	TSlateAttributeRef<bool> GetShouldDrawBackgroundAttribute() const { return TSlateAttributeRef<bool>{SharedThis(this), ShouldDrawBackgroundAttribute}; }

	void SetShouldDrawSurroundingShadow(TAttribute<bool> InShouldDrawSurroundingShadowAttribute) { ShouldDrawSurroundingShadowAttribute.Assign(*this, MoveTemp(InShouldDrawSurroundingShadowAttribute)); }
	bool GetShouldDrawSurroundingShadow() const { return ShouldDrawSurroundingShadowAttribute.Get(); }
	TSlateAttributeRef<bool> GetShouldDrawSurroundingShadowAttribute() const { return TSlateAttributeRef<bool>{SharedThis(this), ShouldDrawSurroundingShadowAttribute}; }

	void SetDisplayAsReadOnly(TAttribute<bool> InDisplayAsReadOnlyAttribute) { DisplayAsReadOnlyAttribute.Assign(*this, MoveTemp(InDisplayAsReadOnlyAttribute)); }
	bool GetDisplayAsReadOnly() const { return DisplayAsReadOnlyAttribute.Get(); }
	TSlateAttributeRef<bool> GetDisplayAsReadOnlyAttribute() const { return TSlateAttributeRef<bool>{SharedThis(this), DisplayAsReadOnlyAttribute}; }
...
protected:
	...
	/** Called when the graph selection changed */
	SGraphEditor::FOnSelectionChanged OnSelectionChangedDelegate;
	...
	UE_DEPRECATED(5.8, "Use SetShowGraphStateOverlay / GetShowGraphStateOverlay / GetShowGraphStateOverlayAttribute")
	TSlateDeprecatedTAttribute<bool> ShowGraphStateOverlay;

	/** Whether to allow slicing of connections in this graph panel */
	TAttribute<bool> AllowConnectionSlicing;

	/** Whether this panel should be drawing a background */
	UE_DEPRECATED(5.8, "Use SetShouldDrawBackground / GetShouldDrawBackground / GetShouldDrawBackgroundAttribute")
	TSlateDeprecatedTAttribute<bool> ShouldDrawBackground;

	/** Whether this panel should be drawing a sunken shadow on its borders */
	UE_DEPRECATED(5.8, "Use SetShouldDrawSurroundingShadow / GetShouldDrawSurroundingShadow / GetShouldDrawSurroundingShadowAttribute")
	TSlateDeprecatedTAttribute<bool> ShouldDrawSurroundingShadow;

	/**
	 * The last created policy which is cached to be used on MouseMove to invalidate when hovering connections.
	 * This must not be used for drawing outside of Paint
	 */
	mutable TUniquePtr<FConnectionDrawingPolicy> ConnectionDrawingPolicy;
...
private:
	...
	/** Whether to draw decorations for graph state (PIE / ReadOnly etc.) */
	TSlateAttribute<bool> ShowGraphStateOverlayAttribute;

	/** Whether this panel should be drawing a background */
	TSlateAttribute<bool> ShouldDrawBackgroundAttribute;

	/** Whether this panel should be drawing a sunken shadow on its borders */
	TSlateAttribute<bool> ShouldDrawSurroundingShadowAttribute;
	...
	TSlateAttribute<bool> DisplayAsReadOnlyAttribute;
