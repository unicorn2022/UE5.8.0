...
protected:
	...
	UE_DEPRECATED(5.8, "Use AbsoluteMousePosition instead, no other change of code needed. The name of this variable was wrong as SetMousePosition was passed the result of 'AllottedGeometry.LocalToAbsolute'.")
	FDeprecateSlateVector2D LocalMousePosition;
	FVector2f AbsoluteMousePosition;
...
public:
	...
	UE_DEPRECATED(5.8, "Use SetAbsoluteMousePosition instead, no other change of code needed.")
	UE_API void SetMousePosition(const UE::Slate::FDeprecateVector2DParameter& InMousePos);
	UE_API void SetAbsoluteMousePosition(FVector2f InAbsoluteMousePos);
	...
	/**
	 * Used in MouseMove to check if the connections have graphically changed and need the GraphPanel to invalidate.
	 * The default implementation checks for a difference in HoveredPins, or in difference in connection WireColor or WireThickness.
	 */
	UE_API virtual bool HaveConnectionsGraphicallyChanged(const SGraphPanel& InGraphPanel, const FVector2f& AbsoluteCursorPosition) const;

	/**
	 * Used in MouseMove to check if the connections have graphically changed and need the GraphPanel to invalidate.
	 * The default implementation checks for a difference in HoveredPins, or in difference in connection WireColor or WireThickness.
	 */
	UE_API virtual bool HaveConnectionsGraphicallyChanged(const SGraphPanel& InGraphPanel, const TSet<FEdGraphPinReference>& InNewlyHoveredPins) const;
...
protected:
	...
	/**
	 * Checks if the given spline overlaps with the Cursor.
	 * @param AbsoluteCursorPosition Absolute position of the cursor
	 * @param Start Start point of the Spline
	 * @param End End point of the Spline
	 * @param Params Parameters of the Spline
	 * @param InOutOverlapResults Will hold the overlap result. Pass in previous results used to find the closest spline to the cursor
	 * @param bOutSliceLineIntersectsSpline If not null, will return true if the SliceLine intersects this spline
	 */
	UE_API void CheckSplineConnectionOverlapWithCursor(
		const FVector2f& AbsoluteCursorPosition,
		const FVector2f& Start,
		const FVector2f& End,
		const FConnectionParams& Params,
		/*out*/ FGraphSplineOverlapResult& InOutOverlapResults,
		/*out*/ bool* bOutSliceLineIntersectsSpline = nullptr
		) const;

	struct FSimpleConnectionData
	{
	...
	/** Keeps track of the last connections drawn to check if they would have changed on MouseMove */
	TArray<FSimpleConnectionData> LastConnections;
private:
