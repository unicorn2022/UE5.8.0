...
#include "ViewportInteractions/PhysicsAssetSimInteraction.h"
...
void FPhysicsAssetEditorEditMode::Enter()
{
	...
	if (UModeManagerInteractiveToolsContext* Context = GetModeManager()->GetInteractiveToolsContext())
	{
		...
		{
			UPhysicsAssetSimInteraction* NewSimInteraction = Source->AddInteraction<UPhysicsAssetSimInteraction>(true);
			NewSimInteraction->SetEditMode(this);
		});
	}
}

void FPhysicsAssetEditorEditMode::Exit()
{
	if (UModeManagerInteractiveToolsContext* Context = GetModeManager()->GetInteractiveToolsContext())
	{
...
bool FPhysicsAssetEditorEditMode::InputAxis(FEditorViewportClient* InViewportClient, FViewport* InViewport, int32 ControllerId, FKey Key, float Delta, float DeltaTime)
{
	...
	if (SharedData->IsManipulating())
	{
		// Simulation mouse movement is handled by UPhysicsAssetSimInteraction::OnDragDelta
		// when viewport interactions are active (Gizmos.Version > 0). Fall back to the
		// legacy path when ITF is not available.
		if (SharedData->bRunningSimulation)
		{
			const UModeManagerInteractiveToolsContext* Context = GetModeManager()->GetInteractiveToolsContext();
			if (!Context || !Context->UsesViewportInteractions())
			{
				if (Key == EKeys::MouseX)
				{
...
bool FPhysicsAssetEditorEditMode::SimMousePress(FEditorViewportClient* InViewportClient, FKey Key)
{
	...
	if (bHit)
	{
		...
		UE_LOGF(LogPhysics, Log, "Physics Asset Editor Click Hit Bone (%ls)", *BoneName.ToString());
		...
		if(bCtrlDown || bShiftDown)
		{
			...
			if (Key == EKeys::RightMouseButton)
			{
				...
				FMatrix	InvViewMatrix = View->ViewMatrices.GetViewToWorld();
...
void FPhysicsAssetEditorEditMode::SimMouseMove(FEditorViewportClient* InViewportClient, float DeltaX, float DeltaY)
{
	...
	FVector LocalOffset = View->ViewMatrices.GetWorldToView().TransformPosition(SimGrabLocation + SimGrabZ * SimGrabPush);
...
void FPhysicsAssetEditorEditMode::HitNothing(FEditorViewportClient* InViewportClient, const bool bGroupSelect)
{
	...
	InViewportClient->Invalidate();
	PhysicsAssetEditorPtr.Pin()->RefreshHierarchyTree();
}
