...
{
	TAutoConsoleVariable<bool> CVarUseITFViewportInteractions(
		TEXT("PhysicsAssetEditor.UseITFViewportInteractions"),
		false,
		TEXT("If true, enables ITF based drag interactions, camera controls, and gizmos in the physics asset editor."));
...
FPhysicsAssetEditor::~FPhysicsAssetEditor()
{
	...
	PhysicsAssetEditor::CVarUseITFViewportInteractions->OnChangedDelegate().RemoveAll(this);
...
static void FillCreateBodiesConstraintsMenu(FMenuBuilder& InSubMenuBuilder)
{
	...
	// Primitive Type Section
	{
		...
		InSubMenuBuilder.AddMenuEntry( PhysicsAssetEditorCommands.CreateBodyWithSingleConvex );
		InSubMenuBuilder.AddMenuEntry( PhysicsAssetEditorCommands.CreateBodyWithMultiConvex );
...
void FPhysicsAssetEditor::InitPhysicsAssetEditor(const EToolkitMode::Type Mode, const TSharedPtr< class IToolkitHost >& InitToolkitHost, UPhysicsAsset* ObjectToEdit)
{
	...
	SharedData->HierarchyChangedEvent.AddRaw(this, &FPhysicsAssetEditor::RefreshHierarchyTree);
	...
	PhysicsAssetEditor::CVarUseITFViewportInteractions->OnChangedDelegate().AddSPLambda(this, [WeakModeTools = TWeakPtr<FEditorModeTools>(GetEditorModeManager().AsShared())](IConsoleVariable*)
	{
		if (TSharedPtr<FEditorModeTools> StrongModeTools = WeakModeTools.Pin())
		{
			const bool bEnabled = PhysicsAssetEditor::CVarUseITFViewportInteractions->GetBool();
			if (UModeManagerInteractiveToolsContext* ToolsContext = StrongModeTools->GetInteractiveToolsContext())
			{
				ToolsContext->SetViewportInteractionsEnabled(bEnabled);
			}
			StrongModeTools->SetSupportsViewportITF(bEnabled);
		}
	});

	{
		const bool bEnabled = PhysicsAssetEditor::CVarUseITFViewportInteractions->GetBool();
		if (UModeManagerInteractiveToolsContext* ToolsContext = GetEditorModeManager().GetInteractiveToolsContext())
		{
			ToolsContext->SetViewportInteractionsEnabled(bEnabled);
		}
		GetEditorModeManager().SetSupportsViewportITF(bEnabled);
	}
...
void FPhysicsAssetEditor::HandleViewportCreated(const TSharedRef<IPersonaViewport>& InViewport)
{
	Viewport = InViewport;
}

void FPhysicsAssetEditor::AddViewportOverlayWidget(TSharedRef<SWidget> InOverlaidWidget, int32 ZOrder)
{
	if (Viewport.IsValid())
	{
...
void FPhysicsAssetEditor::RemoveViewportOverlayWidget(TSharedRef<SWidget> InOverlaidWidget)
{
	if (Viewport.IsValid())
	{
...
void FPhysicsAssetEditor::RefreshHierarchyTree()
{
	if(SkeletonTree.IsValid())
	{
...
void FPhysicsAssetEditor::BindCommands()
{
	...
	ToolkitCommands->MapAction(
		...
		FExecuteAction::CreateSPLambda(this, [this]() { this->CreateBodiesAndConstraintsForSelectedBones(EPhysAssetFitGeomType::EFG_Box, this->ShouldCreateConstraintsWhenCreatingBodies()); }),
		FCanExecuteAction::CreateSP(this, &FPhysicsAssetEditor::IsNotSimulation));

	ToolkitCommands->MapAction(
		Commands.CreateBodyWithSphere,
		FExecuteAction::CreateSPLambda(this, [this]() { this->CreateBodiesAndConstraintsForSelectedBones(EPhysAssetFitGeomType::EFG_Sphere, this->ShouldCreateConstraintsWhenCreatingBodies()); }),
		FCanExecuteAction::CreateSP(this, &FPhysicsAssetEditor::IsNotSimulation));

	ToolkitCommands->MapAction(
		Commands.CreateBodyWithSphyl,
		FExecuteAction::CreateSPLambda(this, [this]() { this->CreateBodiesAndConstraintsForSelectedBones(EPhysAssetFitGeomType::EFG_Sphyl, this->ShouldCreateConstraintsWhenCreatingBodies()); }),
		FCanExecuteAction::CreateSP(this, &FPhysicsAssetEditor::IsNotSimulation));

	ToolkitCommands->MapAction(
		Commands.CreateBodyWithSingleConvex,
		FExecuteAction::CreateSPLambda(this, [this]() { this->CreateBodiesAndConstraintsForSelectedBones(EPhysAssetFitGeomType::EFG_SingleConvexHull, this->ShouldCreateConstraintsWhenCreatingBodies()); }),
		FCanExecuteAction::CreateSP(this, &FPhysicsAssetEditor::IsNotSimulation));

	ToolkitCommands->MapAction(
		Commands.CreateBodyWithMultiConvex,
		FExecuteAction::CreateSPLambda(this, [this]() { this->CreateBodiesAndConstraintsForSelectedBones(EPhysAssetFitGeomType::EFG_MultiConvexHull, this->ShouldCreateConstraintsWhenCreatingBodies()); }),
		FCanExecuteAction::CreateSP(this, &FPhysicsAssetEditor::IsNotSimulation));

	ToolkitCommands->MapAction(
		Commands.CreateBodyWithTaperedCapsule,
		FExecuteAction::CreateSPLambda(this, [this]() { this->CreateBodiesAndConstraintsForSelectedBones(EPhysAssetFitGeomType::EFG_TaperedCapsule, this->ShouldCreateConstraintsWhenCreatingBodies()); }),
		FCanExecuteAction::CreateSP(this, &FPhysicsAssetEditor::IsNotSimulation));
...
void FPhysicsAssetEditor::CreateBodiesAndConstraintsForSelectedBones(const EPhysAssetFitGeomType InGeomType, const bool bInShouldCreateConstraints)
{
	FPhysAssetCreateParams NewBodyData = GetDefault<UPhysicsAssetGenerationSettings>()->CreateParams;

	NewBodyData.GeomType = InGeomType;
...
void FPhysicsAssetEditor::CreateBodiesAndConstraintsForSelectedBones(const FPhysAssetCreateParams& NewBodyData)
{
	...
	if(!SharedData->UniqueSelectionReferencingBodies().IsEmpty())
	{
		...
		// SetSelectedBodies doesn't seem to broadcast/update correctly if BodyIndices is empty (it
		// looks selected, but actually nothing is selected, so if we get called again we can end up
		// regenerating everything!), so clear it to be sure.
	...
	else
	{
		...
		if(BoneItems.Num() > 0)
		{
			...
			BoneIndices.Reserve(BoneItems.Num());
			BoneNames.Reserve(BoneItems.Num());
			...
			for (TSharedPtr<ISkeletonTreeItem> BoneItem : BoneItems)
			{
				...
				if (BoneIndex != INDEX_NONE)
				{
					...
					BoneNames.Add(BoneItem->GetRowItemName());
				}
			}

			for (int32 Index = 0 ; Index != BoneIndices.Num() ; ++Index)
			{
				SlowTask.EnterProgressFrame(1.0f, FText::Format(LOCTEXT(
					"ResetCollsionStepInfo", "Generating collision for {0}"), FText::FromName(BoneNames[Index])));
				SharedData->MakeNewBody(NewBodyData, BoneIndices[Index], true, BoneIndices);
			}
		}
		else
		{
...
void FPhysicsAssetEditor::OnAssetSelectedFromStaticMeshAssetPicker( const FAssetData& AssetData )
{
	...
	if (BodyIndicesToUpdate.Num() > 0)
	{
		...
		{
			UE_LOGF(LogPhysics, Warning, "Failed to import body from static mesh %ls. Mesh probably has no collision setup.", *AssetData.AssetName.ToString());
		}
	}
}
				...
				{					
					...
					if (MLLevelSetModelInferenceInfo)
					{
						...
						if (RowModelInferenceInfo) 
						{
							...
							{
								UE_LOGF(LogChaos, Error, "(Parent) Bone Index index with name %ls is not found for the MLLevelSet.", *ParentBoneName.ToString());
								bCanLoadRow = false;
							...
							{								
								...
								if (ParentBodyIndex == INDEX_NONE)
								{
									UE_LOGF(LogChaos, Error, "Controlling Body with index %d is not found or cannot be created for MLLevelSet.", ParentBodyIndex);
									bCanLoadRow = false;
								...
								{
									const FReferenceSkeleton& RefSkeleton = EditorSkelMesh->GetRefSkeleton();
									UE_LOGF(LogChaos, Display, "MLLevelSet is imported and attached to ParentBone =%ls with index %d", *ParentBoneName.ToString(), ParentBoneIndex);
...
void FPhysicsAssetEditor::HandleSelectionChanged(const TArrayView<TSharedPtr<ISkeletonTreeItem>>& InSelectedItems, ESelectInfo::Type InSelectInfo)
{
	if (!bSelecting)
	{
		...
		// Update SharedData selection unconditionally so that
		// FPhysicsAssetEditorEditMode::ShouldDrawWidget() returns true for all
		// selection sources (tree, viewport, programmatic).  Without this the
		// gizmo won't appear on the first tree selection because SharedData has
		// no selected body/constraint when the selection comes from ESelectInfo::Direct.
		{
			// Block selection broadcast until we have selected all, as this can be an expensive operation
			FScopedBulkSelection BulkSelection(SharedData);
			...
			// Route selection to mode manager unconditionally so the gizmo framework
			// sees the current selection for all selection sources (tree, viewport, programmatic).
			RouteTreeSelectionToModeManager(InSelectedItems);

			// Only a user selection should update secondary views (graph, etc.)
			if (InSelectInfo != ESelectInfo::Direct && PhysicsAssetGraph.IsValid())
			{
				TSet<USkeletalBodySetup*> Bodies;
				TSet<UPhysicsConstraintTemplate*> Constraints;
...
TArray<int32> FPhysicsAssetEditor::GetSelectedBodyIndices() const
{
	TArray<int32> Result;
	const FPhysicsAssetEditorSharedData::SelectionUniqueRange SelectionRange = SharedData->UniqueSelectionReferencingBodies();
	Result.Reserve(SelectionRange.Num());
	Algo::Transform(SelectionRange, Result, [](const FPhysicsAssetEditorSharedData::FSelection& Selection) { return Selection.Index; });
	return Result;
}

TArray<int32> FPhysicsAssetEditor::GetSelectedConstraintIndices() const
{
	TArray<int32> Result;
	const FPhysicsAssetEditorSharedData::SelectionFilterRange SelectionRange = SharedData->SelectedConstraints();
	Result.Reserve(SelectionRange.Num());
	Algo::Transform(SelectionRange, Result, [](const FPhysicsAssetEditorSharedData::FSelection& Selection) { return Selection.Index; });
	return Result;
}

namespace PhysicsAssetEditorSelectionPrivate
{
	/** Map a public EPhysicsAssetEditorSelection primitive flag to an EAggCollisionShape value.
	 *  Returns EAggCollisionShape::Unknown if the flag is not a single primitive type. */
	EAggCollisionShape::Type SelectionTypeToShapeType(const EPhysicsAssetEditorSelection InType)
	{
		switch (InType)
		{
		case EPhysicsAssetEditorSelection::PrimitiveSphere:          return EAggCollisionShape::Sphere;
		case EPhysicsAssetEditorSelection::PrimitiveBox:             return EAggCollisionShape::Box;
		case EPhysicsAssetEditorSelection::PrimitiveCapsule:         return EAggCollisionShape::Sphyl;
		case EPhysicsAssetEditorSelection::PrimitiveConvex:          return EAggCollisionShape::Convex;
		case EPhysicsAssetEditorSelection::PrimitiveTaperedCapsule:  return EAggCollisionShape::TaperedCapsule;
		case EPhysicsAssetEditorSelection::PrimitiveLevelSet:        return EAggCollisionShape::LevelSet;
		case EPhysicsAssetEditorSelection::PrimitiveSkinnedLevelSet: return EAggCollisionShape::SkinnedLevelSet;
		default: return EAggCollisionShape::Unknown;
		}
	}

	/** Check whether a given EPhysicsAssetEditorSelection type flag includes the given shape type. */
	bool SelectionTypeIncludesShape(const int32 InSelectionFlags, const EAggCollisionShape::Type InShapeType)
	{
		switch (InShapeType)
		{
		case EAggCollisionShape::Sphere:          return (InSelectionFlags & UnderlyingType(EPhysicsAssetEditorSelection::PrimitiveSphere)) != 0;
		case EAggCollisionShape::Box:             return (InSelectionFlags & UnderlyingType(EPhysicsAssetEditorSelection::PrimitiveBox)) != 0;
		case EAggCollisionShape::Sphyl:           return (InSelectionFlags & UnderlyingType(EPhysicsAssetEditorSelection::PrimitiveCapsule)) != 0;
		case EAggCollisionShape::Convex:          return (InSelectionFlags & UnderlyingType(EPhysicsAssetEditorSelection::PrimitiveConvex)) != 0;
		case EAggCollisionShape::TaperedCapsule:  return (InSelectionFlags & UnderlyingType(EPhysicsAssetEditorSelection::PrimitiveTaperedCapsule)) != 0;
		case EAggCollisionShape::LevelSet:        return (InSelectionFlags & UnderlyingType(EPhysicsAssetEditorSelection::PrimitiveLevelSet)) != 0;
		case EAggCollisionShape::SkinnedLevelSet: return (InSelectionFlags & UnderlyingType(EPhysicsAssetEditorSelection::PrimitiveSkinnedLevelSet)) != 0;
		default: return false;
		}
	}
}

void FPhysicsAssetEditor::ModifySelection(EPhysicsAssetEditorSelection ElementType,
	const TArray<int32>& Indices, bool bAddToSelection)
{
	if (Indices.Num() == 0)
	{
		return;
	...
	if (ElementType == EPhysicsAssetEditorSelection::Body)
	{
		SharedData->SetSelectedBodiesAllPrimitive(Indices, bAddToSelection);
	}
	else if (ElementType == EPhysicsAssetEditorSelection::Constraint)
	{
		SharedData->ModifySelectedConstraints(Indices, bAddToSelection);
	}
	else if (ElementType == EPhysicsAssetEditorSelection::CenterOfMass)
	{
		TArray<FPhysicsAssetEditorSelectedElement> CoMSelections;
		CoMSelections.Reserve(Indices.Num());
		for (const int32 BodyIndex : Indices)
		{
			CoMSelections.Add(MakeCoMSelection(BodyIndex));
		}
		SharedData->ModifySelectedCoMs(CoMSelections, bAddToSelection);
	}
	else if (ElementType == EPhysicsAssetEditorSelection::Bone)
	{
		...
		TArray<FName> BoneNames;
		BoneNames.Reserve(Indices.Num());
		for (const int32 BoneIndex : Indices)
		{
		...
		if (BoneNames.Num() > 0)
		{
			if (bAddToSelection)
			{
				// Merge new names with existing selected bones
				UDebugSkelMeshComponent* MeshComp = GetPersonaToolkit()->GetPreviewScene()->GetPreviewMeshComponent();
				if (MeshComp)
				{
					for (const int32 ExistingBoneIdx : MeshComp->BonesOfInterest)
					{
						if(RefSkeleton.IsValidIndex(ExistingBoneIdx))
						{
							FName ExistingName = RefSkeleton.GetBoneName(ExistingBoneIdx);
							BoneNames.AddUnique(ExistingName);
						}
					}
				}
				GetPersonaToolkit()->GetPreviewScene()->SetSelectedBones(BoneNames, ESelectInfo::Direct);
			}
			else
			{
				// Remove specified bones from current selection
				UDebugSkelMeshComponent* MeshComp = GetPersonaToolkit()->GetPreviewScene()->GetPreviewMeshComponent();
				if (MeshComp)
				{
					TSet<int32> BoneIndicesToRemove(Indices);
					TArray<FName> RemainingBones;
					for (const int32 ExistingBoneIdx : MeshComp->BonesOfInterest)
					{
						if (!BoneIndicesToRemove.Contains(ExistingBoneIdx) && RefSkeleton.IsValidIndex(ExistingBoneIdx))
						{
							RemainingBones.Add(RefSkeleton.GetBoneName(ExistingBoneIdx));
						}
					}
					if (RemainingBones.Num() > 0)
					{
						GetPersonaToolkit()->GetPreviewScene()->SetSelectedBones(RemainingBones, ESelectInfo::Direct);
					}
					else
					{
						GetPersonaToolkit()->GetPreviewScene()->ClearSelectedBone();
	...
	else if ((ElementType == EPhysicsAssetEditorSelection::Primitive) || (ElementType == EPhysicsAssetEditorSelection::AllPrimitive))
	{
		// Select all primitives in the given bodies
		SharedData->SetSelectedBodiesAllPrimitive(Indices, bAddToSelection);
	}
	else if (UnderlyingType(ElementType) & UnderlyingType(EPhysicsAssetEditorSelection::AllPrimitive))
	{
		// Select only primitives of the matching shape type(s)
		const int32 SelectionFlags = UnderlyingType(ElementType);
		UPhysicsAsset* PhysicsAsset = SharedData->PhysicsAsset;
		TArray<FPhysicsAssetEditorSelectedElement> PrimSelections;
		...
		for (const int32 BodyIndex : Indices)
		{
			...
			auto AddPrimitivesOfType = [&PrimSelections, SelectionFlags, BodyIndex](const auto& ShapeElems)
			{
				for (int32 PrimIdx = 0; PrimIdx < ShapeElems.Num(); ++PrimIdx)
				{
					if (PhysicsAssetEditorSelectionPrivate::SelectionTypeIncludesShape(SelectionFlags, ShapeElems[PrimIdx].GetShapeType()))
					{
						PrimSelections.Add(MakePrimitiveSelection(BodyIndex, ShapeElems[PrimIdx].GetShapeType(), PrimIdx));
					}
				}
			};
			...
			AddPrimitivesOfType(AggGeom.SphereElems);
			AddPrimitivesOfType(AggGeom.BoxElems);
			AddPrimitivesOfType(AggGeom.SphylElems);
			AddPrimitivesOfType(AggGeom.ConvexElems);
			AddPrimitivesOfType(AggGeom.TaperedCapsuleElems);
			AddPrimitivesOfType(AggGeom.LevelSetElems);
			AddPrimitivesOfType(AggGeom.SkinnedLevelSetElems);
		}

		if (PrimSelections.Num() > 0)
		{
			SharedData->ModifySelectedPrimitives(PrimSelections, bAddToSelection);
		}	
	}
}

void FPhysicsAssetEditor::ClearSelection()
{
	SharedData->ClearSelected();
	GetPersonaToolkit()->GetPreviewScene()->ClearSelectedBone();
}
