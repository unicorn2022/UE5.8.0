...
FReply FDataLayerMode::OnKeyDown(const FKeyEvent& InKeyEvent)
{
	...
	// Rename key: Rename selected data layer (not rebindable, because it doesn't make much sense to bind.)
	...
	if (!ToolMenus->IsMenuRegistered(DefaultContextBaseMenuName))
	{
		...
		Menu->AddDynamicSection("DataLayerDynamicSection", FNewToolMenuDelegate::CreateLambda([](UToolMenu* InMenu)
		{
...

			{
				...
				if (OwningWorldAWorldDataLayers)
				{
					...
					Section.AddSubMenu("CreateNewDataLayerWithAsset", LOCTEXT("CreateNewDataLayerWithAssetSubMenu", "Create New Data Layer With Asset"), LOCTEXT("CreateNewDataLayerWithAssetSubMenu_ToolTip", "Create New Data Layer With Asset"),
						...
						}), false, FSlateIcon(FCoreStyle::Get().GetStyleSetName(), "Icons.Import"));
				}
				else
				{
					Section.AddMenuEntry("CreateNewDataLayer", LOCTEXT("CreateNewDataLayer", "Create New Data Layer"), FText(), FSlateIcon(FCoreStyle::Get().GetStyleSetName(), "Icons.AddCircle"),
						FUIAction(FExecuteAction::CreateLambda([CreateNewEmptyDataLayer]() { CreateNewEmptyDataLayer(); })));
				}

				Section.AddMenuEntry("CreateNewDataLayerPrivate", LOCTEXT("CreateNewDataLayerPrivate", "Create New Private Data Layer"), LOCTEXT("CreateNewDataLayerPrivateToolTip", "Creates an Editor Only Data Layer that cannot be used by other Worlds."), FSlateIcon(FCoreStyle::Get().GetStyleSetName(), "Icons.AddCircle"),
					FUIAction(FExecuteAction::CreateLambda([CreateNewEmptyDataLayer]() { CreateNewEmptyDataLayer(true); } )));

				bool bCanRename = false;
				if (SelectedDataLayers.Num() == 1)
				{
					FSceneOutlinerTreeItemPtr ItemToRename = SceneOutliner->GetSelection().SelectedItems[0].Pin();
				...
				Section.AddMenuEntry("RenameSelectedDataLayer", LOCTEXT("RenameSelectedDataLayer", "Rename Selected Data Layer"), (bCanRename) ? FText() : LOCTEXT("RenameSelectedDataLayerTooltip", "Only Private data layers can be renamed, as Asset data layers take their name from the asset. Also a single data layer can be renamed at a time"), FSlateIcon(FCoreStyle::Get().GetStyleSetName(), "GenericCommands.Rename"),
					FUIAction(
						FExecuteAction::CreateLambda([Mode, SelectedDataLayers, SceneOutliner]() 
						{
							if (SelectedDataLayers.Num() == 1)
							{
								FSceneOutlinerTreeItemPtr ItemToRename = SceneOutliner->GetSelection().SelectedItems[0].Pin();
						...
						FCanExecuteAction::CreateLambda([bCanRename]
						{
							return bCanRename;
						})
					), EUserInterfaceActionType::Button, NAME_None, LOCTEXT("F2", "F2"));
				...
				Section.AddMenuEntry("DeleteSelectedDataLayers", LOCTEXT("DeleteSelectedDataLayers", "Delete Selected Data Layer(s)"), FText(), FSlateIcon(FCoreStyle::Get().GetStyleSetName(), "GenericCommands.Delete"),
					FUIAction(
					...
					), EUserInterfaceActionType::Button, NAME_None, LOCTEXT("Delete", "Delete"));
					...
					Section.AddSubMenu("MoveSelectedDataLayersTo", LOCTEXT("MoveSelectedDataLayersTo", "Move Selected Data Layer(s) To"), FText(),
						FNewToolMenuDelegate::CreateLambda([Mode, SelectedDataLayers](UToolMenu* InSubMenu)
						{
							...
							CreateDataLayerPicker(InSubMenu,
								...
								bShowRoot);
						}), false, FSlateIcon(FCoreStyle::Get().GetStyleSetName(), "Icons.Reimport"));
				}

				Section.AddMenuEntry("Copy Selected Data Layer Instances(s) File Path", LOCTEXT("CopySelectedDataLayerInstancessFilePath", "Copy Selected Data Layer Instance(s) File Path"), FText(), FSlateIcon(FCoreStyle::Get().GetStyleSetName(), "GenericCommands.Copy"),
					FUIAction(
				...
				Section.AddMenuEntry("AddSelectedActorsToSelectedDataLayers", LOCTEXT("AddSelectedActorsToSelectedDataLayers", "Add Selected Actor(s) to Selected Data Layer(s)"), AddActorToolTip, FSlateIcon(FCoreStyle::Get().GetStyleSetName(), "DataLayerBrowser.AddSelection"),
					FUIAction(
						FExecuteAction::CreateLambda([SelectedDataLayers]()
						{
							check(!SelectedDataLayers.IsEmpty());
				...
				if (!Mode->SelectedDataLayerActors.IsEmpty())
				{
					Section.AddSubMenu("AddSelectedActorsTo", LOCTEXT("AddSelectedActorsTo", "Add Selected Actor(s) To"), FText(),
						...
						}), false, FSlateIcon(FCoreStyle::Get().GetStyleSetName(), "DataLayerBrowser.AddSelection"));
				}
				...
				Section.AddMenuEntry("RemoveSelectedActorsFromSelectedDataLayers", LOCTEXT("RemoveSelectedActorsFromSelectedDataLayersMenu", "Remove Selected Actor(s) from Selected Data Layer(s)"), RemoveActorToolTip, FSlateIcon(FCoreStyle::Get().GetStyleSetName(), "DataLayerBrowser.RemoveSelection"),
					FUIAction(
						FExecuteAction::CreateLambda([SelectedDataLayers]() 
						{
							check(!SelectedDataLayers.IsEmpty());
...

			{
				...
				Section.AddMenuEntry("SelectActorsInDataLayers", LOCTEXT("SelectActorsInDataLayers", "Select Actor(s) in Data Layer(s)"), FText(), FSlateIcon(FCoreStyle::Get().GetStyleSetName(), "GenericCommands.SelectAll"),
					FUIAction(
						FExecuteAction::CreateLambda([SelectedDataLayers]() {
						check(!SelectedDataLayers.IsEmpty());
						{
				...
				Section.AddMenuEntry("AddActorsToSelection", LOCTEXT("AddActorsToSelection", "Add Actors in Data Layer(s) to Selection"), FText(), FSlateIcon(FCoreStyle::Get().GetStyleSetName(), "Icons.Merge"),
					FUIAction(
						FExecuteAction::CreateLambda([SelectedDataLayers]() {
						check(!SelectedDataLayers.IsEmpty());
						{
				...
				Section.AddMenuEntry("DeselectActors", LOCTEXT("DeselectActors", "Deselect Actor(s) in Data Layer(s)"), FText(), FSlateIcon(FCoreStyle::Get().GetStyleSetName(), "Icons.XCircle"),
					FUIAction(
						FExecuteAction::CreateLambda([SelectedDataLayers]() {
						check(!SelectedDataLayers.IsEmpty());
						{
...

			{
				...
				Section.AddMenuEntry("ForceLoadActorsInDataLayers", LOCTEXT("ForceLoadActorsInDataLayers", "Force Load Actor(s) in Data Layer(s)"), FText(), FSlateIcon(FCoreStyle::Get().GetStyleSetName(), "Icons.Pinned"),
					FUIAction(
						FExecuteAction::CreateLambda([SelectedDataLayers]() 
						{
							check(!SelectedDataLayers.IsEmpty());
				...
				Section.AddMenuEntry("ReleaseForceLoadActorsInDataLayers", LOCTEXT("ReleaseForceLoadActorsInDataLayers", "Release Force Load Actor(s) in Data Layer(s)"), FText(), FSlateIcon(FCoreStyle::Get().GetStyleSetName(), "Icons.Unpinned"),
					FUIAction(
						FExecuteAction::CreateLambda([SelectedDataLayers]()
							{
								check(!SelectedDataLayers.IsEmpty());
...

			{
				...
				Section.AddMenuEntry("CollapseAllDataLayers", LOCTEXT("CollapseAllDataLayers", "Collapse All Data Layers"), FText(), FSlateIcon(FCoreStyle::Get().GetStyleSetName(), "Icons.ChevronUp"),
					FUIAction(
				...
				Section.AddMenuEntry("ExpandAllDataLayers", LOCTEXT("ExpandAllDataLayers", "Expand All Data Layers"), FText(), FSlateIcon(FCoreStyle::Get().GetStyleSetName(), "Icons.ChevronDown"),
					FUIAction(
						FExecuteAction::CreateLambda([SceneOutliner,SelectedDataLayers]() {
						check(!SelectedDataLayers.IsEmpty());
						{
...

			{
				...
				Section.AddMenuEntry("MakeCurrentDataLayers", LOCTEXT("MakeCurrentDataLayers", "Make Current Data Layer(s)"), FText(), FSlateIcon(FCoreStyle::Get().GetStyleSetName(), "Icons.Check"),
					FUIAction(
				...
				Section.AddMenuEntry("RemoveCurrentDataLayers", LOCTEXT("RemoveCurrentDataLayers", "Remove Current Data Layer(s)"), FText(), FSlateIcon(FCoreStyle::Get().GetStyleSetName(), "Icons.X"),
					FUIAction(
						FExecuteAction::CreateLambda([Mode, SceneOutliner, SelectedDataLayers]()
						{
							const FScopedTransaction Transaction(LOCTEXT("RemoveCurrentDataLayers", "Remove Current Data Layer(s)"));
				...
				Section.AddMenuEntry("ClearCurrentDataLayers", LOCTEXT("ClearCurrentDataLayers", "Clear Current Data Layer(s)"), FText(), FSlateIcon(FCoreStyle::Get().GetStyleSetName(), "Icons.Delete"),
					FUIAction(
...

			{
				...
				Section.AddMenuEntry("MakeAllDataLayersVisible", LOCTEXT("MakeAllDataLayersVisible", "Make All Data Layers Visible"), FText(), FSlateIcon(FCoreStyle::Get().GetStyleSetName(), "Icons.Visible"),
					FUIAction(
						FExecuteAction::CreateLambda([AllDataLayers]()
						{
							check(!AllDataLayers.IsEmpty());
