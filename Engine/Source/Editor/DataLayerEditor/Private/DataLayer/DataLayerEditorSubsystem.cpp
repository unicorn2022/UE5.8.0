...
const UExternalDataLayerInstance* UDataLayerEditorSubsystem::GetActorSpawningExternalDataLayerInstance(AActor* InActor) const
{
	...
	// When the object shares a mount point with the owning world, the actor's
	// default placement (the map package) is already in the right plugin and we shouldn't
	// auto-promote it into an EDL.
	const UPackage* WorldPackage = OwningWorld ? OwningWorld->GetPackage() : nullptr;
	auto IsInDifferentPluginThanWorld = [WorldPackage](const UObject* Object) -> bool
	{
		const UPackage* ObjectPackage = Object ? Object->GetPackage() : nullptr;
		return ObjectPackage && WorldPackage
			&& FPackageName::GetPackageMountPoint(ObjectPackage->GetName()) != FPackageName::GetPackageMountPoint(WorldPackage->GetName());
	};
	...
	if (!ResolvedExternalDataLayerAsset && IsInDifferentPluginThanWorld(OverrideSpawningLevelMountPointObject))
	{
	...
	if (!ResolvedExternalDataLayerAsset && IsInDifferentPluginThanWorld(InActor->GetClass()))
	{
...
void UDataLayerEditorSubsystem::MoveActorToExternalDataLayer(AActor* InActor, const UExternalDataLayerInstance* InExternalDataLayerInstance, bool bInNotifyFailureReason)
{
	...
	if (!FExternalDataLayerHelper::MoveActorsToExternalDataLayer({ InActor }, InExternalDataLayerInstance, &FailureReason))
	{
		...
		{
			UE_LOGF(LogWorldPartition, Warning, "%ls", *FailureReason.ToString());
...
void UDataLayerEditorSubsystem::OnEditorActorReplaced(AActor* InOldActor, AActor* InNewActor)
{
	...
	if (ShouldHandleActor(InNewActor))
	{
		...
		{
			// Only move actors which are actually stored in an EDL. Otherwise, if the owning world
			// shared a mount point with an EDL, the actor would erroneously be moved into that EDL.
			if (FExternalDataLayerHelper::IsExternalDataLayerPath(ActorPackage->GetPathName(), nullptr))
			{
...
{
	UE_CLOGF(!InWorld, LogDataLayerEditorSubsystem, Error, "%ls - Failed because world in null.", ANSI_TO_TCHAR(__FUNCTION__));
...
void UDataLayerEditorSubsystem::SetActorsPinStateInDataLayers(const TArray<UDataLayerInstance*>& DataLayerInstances, const bool bPinned)
{
	...
	if (UWorldPartition* WorldPartition = World ? World->GetWorldPartition() : nullptr)
	{
		...
		Algo::TransformIf(DataLayerInstances, DataLayerInstanceNames, [](UDataLayerInstance* DataLayerInstance) { return !!DataLayerInstance; }, [](UDataLayerInstance* DataLayerInstance) { return DataLayerInstance->GetFName(); });
...
{
	UE_CLOGF(!GetWorld(), LogDataLayerEditorSubsystem, Error, "%ls - Failed because world in null.", ANSI_TO_TCHAR(__FUNCTION__));
	if (UDataLayerManager* DataLayerManager = UDataLayerManager::GetDataLayerManager(GetWorld()))
	{
...
UDataLayerInstance* UDataLayerEditorSubsystem::CreateDataLayerInstance(const FDataLayerCreationParameters& Parameters)
{
	...
	{
		if (Parameters.bIsPrivate || !Parameters.DataLayerAsset)
		{
			ensureMsgf(Parameters.bIsPrivate, TEXT("When a DataLayerAsset is not provided, bIsPrivate should be set to true for clarity"));
		...
		else
		{
			if (UExternalDataLayerAsset* ExternalDataLayerAsset = Cast<UExternalDataLayerAsset>(Parameters.DataLayerAsset))
			{
				...
				if (UExternalDataLayerManager* ExternalDataLayerManager = UExternalDataLayerManager::GetExternalDataLayerManager(OuterWorld))
				{
						...
						if (AWorldDataLayers* EDLWorldDataLayers = ExternalDataLayerManager->GetWorldDataLayers(ExternalDataLayerAsset, bAllowCreate))
						{
							...
							if (ensure(!ExternalDataLayerInstance))
							{
								...
								if (ensure(ExternalDataLayerInstance))
								{
									...
									{
										UE_LOGF(LogDataLayerEditorSubsystem, Warning, "[EDL %ls] Failed to inject External Data Layer.", *ExternalDataLayerAsset->GetName());
									}
								}
							}
						}
					...
					{
						UE_LOGF(LogDataLayerEditorSubsystem, Warning, "%ls", *FailureReason.ToString());
						LastWarningNotification = FailureReason;
					}
				}
				UE_CLOGF(!NewDataLayerInstance, LogDataLayerEditorSubsystem, Error, "[EDL %ls] Failed to create External Data Layer Instance.", *ExternalDataLayerAsset->GetName());
			}
			else
			{
	...
	if (const UExternalDataLayerInstance* RootExternalDataLayerInstance = NewDataLayerInstance ? NewDataLayerInstance->GetDirectOuterWorldDataLayers()->GetRootExternalDataLayerInstance() : nullptr)
	{
		...
		if ((NewDataLayerInstance != Parent) && !NewDataLayerInstance->SetParent(Parent))
		{
			...
			{
				UE_LOGF(LogDataLayerEditorSubsystem, Warning, "Can't create Data Layer Instance %ls under %ls : %ls", *NewDataLayerInstance->GetDataLayerShortName(), *Parent->GetDataLayerShortName(), *Reason.ToString());
			}
...
bool UDataLayerEditorSubsystem::MoveActorToDataLayers(AActor* InActor, const TArray<UDataLayerInstance*>& InDataLayerInstances)
{
	...
	for (UDataLayerInstance* DataLayerInstance : InDataLayerInstances)
	{
		...
		{
			UE_LOGF(LogWorldPartition, Warning, "Can't move actor %ls to Data Layers: Invalid Data Layer %ls", *InActor->GetActorNameOrLabel(), *DataLayerInstance->GetDataLayerShortName());
			return false;
		}
	}
	...
	else
	{
		...
		if ((OldExternalDataLayerAsset != NewExternalDataLayerAsset))
		{
			...
			{
				UE_LOGF(LogWorldPartition, Warning, "Can't move actor %ls to External Data Layer. %ls", *InActor->GetName(), *FailureReason.ToString());
				return false;
			}
		}

		// Move to new Data Layers:
		// First, remove all non-EDL layers
		...
		// Second, move to new External Data Layer if any or different. Subsequent valid data layers should be in that EDL, and the EDL must be added first for package paths to allow adding the DLs.
		if (OldExternalDataLayerAsset != NewExternalDataLayerAsset)
		{
			verify(FExternalDataLayerHelper::MoveActorsToExternalDataLayer({ InActor }, Params));
		}
		// Finally, apply the rest of the layers
		for (UDataLayerInstance* DataLayerInstance : InDataLayerInstances)
		{
