...
namespace UE::Editor
{
	//~ It would be nice if this were a nested class inside USubobjectDataSubsystem, but
	//~  we need it in SubobjectDataAdapterBase.h, we can't forward declare nested classes,
	//~  and it's a pain to forward declare everything from that file here instead.
	/**
	 * Struct holding information about the context of a subobject editor
	 */
	struct FSubobjectEditorContext
	{
		// If editor operates on a single object, the object to use. I.e. the object
		//  typically returned by GetObjectContext inside a details panel. This might
		//  eventually be phased out in favor of always using SelectionSetName or
		//  some other way to identify the target objects (since there may be multiple)
		UObject* ObjectContext = nullptr;
		
		TWeakObjectPtr<UTypedElementSelectionSet> SelectionSet = nullptr;
	};
}
...
public:
	...
	* 
	* @param ObjectContext	Object to gather subobjects for
	* @param OutArray		Array to populate (will be emptied first)
	*/
	UE_API void GatherSubobjectData(UObject* ObjectContext, TArray<FSubobjectDataHandle>& OutArray);

	/**
	* Gather all subobjects that the given context has. Populates an array of
	* handles that will have the relevant subobjects (taken from selection, or from
	* an object, as appropriate).
	*
	* @param OutArray		Array to populate (will be emptied first)
	*/
	UE_EXPERIMENTAL(5.8, "Use GatherSubobjectData(UObject*, ...) for now.  This API may change.")
	UE_API void GatherSubobjectData(const UE::Editor::FSubobjectEditorContext& Context, TArray<FSubobjectDataHandle>& OutArray);
	...
	UE_API FSubobjectDataSubsystemAdapterHandle RegisterSubsystemAdapter(TUniquePtr<FSubobjectDataSubsystemAdapterBase> InAdapter);
	UE_EXPERIMENTAL(5.7, "Experimental adapter to adjust behavior of SubobjectDataSubsystem")
	...
	*/
	...
	UE_API bool CanDuplicateSubobjects(const FSubobjectDataHandle& Context, const TArray<FSubobjectDataHandle>& SubobjectsToDup);
