...
void USubobjectDataSubsystem::GatherSubobjectData(UObject* ObjectContext, TArray<FSubobjectDataHandle>& OutArray)
{
	UE::Editor::FSubobjectEditorContext Context;
	Context.ObjectContext = ObjectContext;
	GatherSubobjectData(Context, OutArray);
}

void USubobjectDataSubsystem::GatherSubobjectData(const UE::Editor::FSubobjectEditorContext& Context, TArray<FSubobjectDataHandle>& OutArray)
{
	OutArray.Reset();	

	for (const auto& Entry : SubsystemAdapters)
	{
		if (Entry.Adapter->CanIntercept_GatherSubobjectData(Context))
		{
			return Entry.Adapter->GatherSubobjectData(
				Context,
				OutArray,
	...
	{
		UE_LOGF(LogSubobjectSubsystem, Warning, "Could not gather subobject data, there was no context given!");
		return;
	}

	const AActor* ActorContext = Cast<AActor>(Context.ObjectContext);
	if(!ActorContext)
	{
		return;
	}

	FSubobjectDataHandle RootActorHandle = CreateSubobjectData(Context.ObjectContext);
...
UClass* USubobjectDataSubsystem::CreateNewCPPComponent(TSubclassOf<UActorComponent> ComponentClass, const FString& NewClassPath, const FString& NewClassName)
{
	...
	if (ComponentClass && !NewClassName.IsEmpty() && !NewClassPath.IsEmpty())
	{
		...
		{
			UE_LOGF(LogSubobjectSubsystem, Error, "Failed to create a new CPP component: %ls", *FailReason.ToString())
		}
	}
...
UClass* USubobjectDataSubsystem::CreateNewBPComponent(TSubclassOf<UActorComponent> ComponentClass, const FString& NewClassPath, const FString& NewClassName)
{
	...
	if(ComponentClass)
	{
		...
		{
			UE_LOGF(LogSubobjectSubsystem, Warning, "Failed to Create new BP Component: '%ls' is not a blueprintable class!", *ComponentClass->GetDisplayNameText().ToString());
		}
		else if (NewClassName.IsEmpty())
		{
			UE_LOGF(LogSubobjectSubsystem, Warning, "Failed to Create new BP Component: An NewClassName was given!");
		}
		else if (NewClassPath.IsEmpty())
		{
			UE_LOGF(LogSubobjectSubsystem, Warning, "Failed to Create new BP Component: An invalid class path was given!");
		}
		else
		{
			...
			{
				UE_LOGF(LogSubobjectSubsystem, Warning, "Failed to Create new BP Component: A class with a name '%ls' already exists!", *PackagePath);
				return nullptr;
			}
	...
	{
		UE_LOGF(LogSubobjectSubsystem, Warning, "Failed to Create new BP Component: ComponentClass is required!");
	}
...
static void ConformTransformRelativeToParent(USceneComponent* SceneComponentTemplate, const USceneComponent* ParentSceneComponent)
{
	...
	if (!SceneComponentTemplate->IsUsingAbsoluteRotation())
	{
		...
		// Identity quaternion can convert to a zero rotator with a -0.0 Roll value, correct that
		if (RelativeRotation == FRotator{ 0.0, 0.0, -0.0 })
		{
			RelativeRotation = FRotator{ 0.0 };
		}
		
		SceneComponentTemplate->SetRelativeRotation_Direct(RelativeRotation);
	}
...
FSubobjectDataHandle USubobjectDataSubsystem::FindSceneRootForSubobject(const FSubobjectDataHandle& InHandle) const
{
	...
	for (const auto& Entry : SubsystemAdapters)
	{
		if (Entry.Adapter->CanIntercept_FindSceneRootForSubobject(InHandle))
		{
			return Entry.Adapter->FindSceneRootForSubobject(InHandle);
		}
	}
...
FSubobjectDataHandle USubobjectDataSubsystem::AddNewSubobject(const FAddNewSubobjectParams& Params, FText& FailReason)
{
	...
	const FSubobjectDataHandle& ParentActorRootHandle = GetActorRootHandle(ParentObjHandle);
	if (!Blueprint)
	{
...
int32 USubobjectDataSubsystem::DeleteSubobjects(const FSubobjectDataHandle& ContextHandle, const TArray<FSubobjectDataHandle>& SubobjectsToDelete, FSubobjectDataHandle& OutComponentToSelect, UBlueprint* BPContext/* = nullptr*/, bool bForce /* = false */)
{
	...
	for (const auto& Entry : SubsystemAdapters)
	{
		if (Entry.Adapter->CanIntercept_DeleteSubobjects(ContextHandle, SubobjectsToDelete, BPContext, bForce))
		{
			NumDeletedSubobjects = Entry.Adapter->DeleteSubobjects(
				ContextHandle,
				SubobjectsToDelete);
			return NumDeletedSubobjects;
		}
		...
		for (const FSubobjectDataHandle& Handle : SubobjectsToDelete)
		{
			if (Handle.IsValid())
			{
				if (const FSubobjectData* Data = Handle.GetData())
				{
					...
					{
						UE_LOGF(LogSubobjectSubsystem, Warning, "Cannot delete subobject '%ls' from '%ls'!", *Data->GetDisplayString(), *BPContext->GetFullName());
						continue;
					}
					...
					if (SCS_Node)
					{
						...
						{
							UE_LOGF(LogSubobjectSubsystem, Warning, "Cannot remove subobject '%ls' because it is the default scene root!", *Data->GetDisplayString());
						}
		...
		for(const FSubobjectDataHandle& Handle : SubobjectsToDelete)
		{
			if(Handle.IsValid())
			{
				if(const FSubobjectData* Data = Handle.GetData())
				{
					if (!Data->CanDelete())
					{
						UE_LOGF(LogSubobjectSubsystem, Warning, "Cannot delete subobject '%ls' from '%ls'!", *Data->GetDisplayString(), *ContextObj->GetFullName());
						continue;
					}
...
bool USubobjectDataSubsystem::MakeNewSceneRoot(const FSubobjectDataHandle& Context, const FSubobjectDataHandle& DroppedNewSceneRootHandle, UBlueprint* Blueprint)
{
	...
	{
		UE_LOGF(LogSubobjectSubsystem, Warning, "Failed to make new scene root: Invalid context or scene root handle!");
		return false;
	}
...
bool USubobjectDataSubsystem::ReparentSubobjects(const FReparentSubobjectParams& Params, const TArray<FSubobjectDataHandle>& HandlesToMove)
{
	...
	{
		UE_LOGF(LogSubobjectSubsystem, Warning, "Failed to reparent: Invalid parent handle when reparenting!");
		return false;
	}
	...
	if (Params.BlueprintContext)
	{
		...
		{
			UE_LOGF(LogSubobjectSubsystem, Warning, "Failed to reparent: In a blueprint context there must be an actor preview!");
			return false;
		}
...
bool USubobjectDataSubsystem::CanCopySubobjects(const TArray<FSubobjectDataHandle>& Handles) const
{
	for (const auto& Entry : SubsystemAdapters)
	{
		if (Entry.Adapter->CanIntercept_CanCopySubobjects(Handles))
		{
			return Entry.Adapter->CanCopySubobjects(Handles);
		}
	}
...
void USubobjectDataSubsystem::CopySubobjects(const TArray<FSubobjectDataHandle>& Handles, UBlueprint* BpContext)
{
	for (const auto& Entry : SubsystemAdapters)
	{
		if (Entry.Adapter->CanIntercept_CopySubobjects(Handles, BpContext))
		{
			Entry.Adapter->CopySubobjects(Handles, BpContext);
			return;
		}
	}
...
bool USubobjectDataSubsystem::CanPasteSubobjects(const FSubobjectDataHandle& RootHandle, UBlueprint* BPContext) const
{
	for (const auto& Entry : SubsystemAdapters)
	{
		if (Entry.Adapter->CanIntercept_CanPasteSubobjects(RootHandle, BPContext))
		{
			return Entry.Adapter->CanPasteSubobjects(RootHandle, BPContext);
		}
	}
...
void USubobjectDataSubsystem::PasteSubobjects(const FSubobjectDataHandle& PasteToContext, const TArray<FSubobjectDataHandle>& NewParentHandles, UBlueprint* Blueprint, TArray<FSubobjectDataHandle>& OutPastedHandles)
{
	...
	for (const auto& Entry : SubsystemAdapters)
	{
		if (Entry.Adapter->CanIntercept_PasteSubobjects(PasteToContext, NewParentHandles, Blueprint))
		{
			Entry.Adapter->PasteSubobjects(PasteToContext, NewParentHandles, Blueprint, OutPastedHandles);
			return;
		}
	}
...
bool USubobjectDataSubsystem::CanDuplicateSubobjects(const FSubobjectDataHandle& Context, const TArray<FSubobjectDataHandle>& SubobjectsToDup)
{
	for (const auto& Entry : SubsystemAdapters)
	{
		if (Entry.Adapter->CanIntercept_CanDuplicateSubobjects(Context, SubobjectsToDup))
		{
			return Entry.Adapter->CanDuplicateSubobjects(Context, SubobjectsToDup);
		}
	}

	// Default behaviour inherited from SSubobjectEditor
	return CanCopySubobjects(SubobjectsToDup);
}
...
void USubobjectDataSubsystem::DuplicateSubobjects(const FSubobjectDataHandle& Context, const TArray<FSubobjectDataHandle>& SubobjectsToDup, UBlueprint* BpContext, TArray<FSubobjectDataHandle>& OutNewSubobjects)
{
	...
	for (const auto& Entry : SubsystemAdapters)
	{
		if (Entry.Adapter->CanIntercept_DuplicateSubobjects(Context, SubobjectsToDup, BpContext))
		{
			return Entry.Adapter->DuplicateSubobjects(Context, SubobjectsToDup, BpContext, OutNewSubobjects);
		}
	}
	...
	for(const FSubobjectDataHandle& OriginalHandle : SubobjectsToDup)
	{
		...
		{
			UE_LOGF(LogSubobjectSubsystem, Warning, "Could not duplicate one or more subobjects, an invalid SubobjectToDup was given!");
			continue;
		}
