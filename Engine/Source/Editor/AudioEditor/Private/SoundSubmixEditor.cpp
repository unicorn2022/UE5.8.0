...
void FSoundSubmixEditor::AddEditableSubmixChildren(USoundSubmixBase* ParentSubmix)
{
	if (!ParentSubmix)
	{
		return;
	...
	ParentSubmix->SetFlags(RF_Transactional);
	for (USoundSubmixBase* Child : ParentSubmix->ChildSubmixes)
	{
		if (Child)
		{
			// Only update the child submix if it's different
			// this avoids unnecessarily dirtying submixes if they aren't actually changed
			if (Child->SoundSubmixGraph != ParentSubmix->SoundSubmixGraph)
			{
				// Preserve child pacakge state for editor-only bookkeeping
				UPackage* ChildPkg = Child->GetOutermost();
				const bool bWasDirty = ChildPkg ? ChildPkg->IsDirty() : false;

				Child->Modify(false);
				Child->SoundSubmixGraph = ParentSubmix->SoundSubmixGraph;
				Child->PostEditChange();

				if (ChildPkg)
				{
					ChildPkg->SetDirtyFlag(bWasDirty);
				}
			}
...
void FSoundSubmixEditor::NotifyPreChange(FProperty* PropertyAboutToChange)
{
	// When the property editor is in a transaction, make sure edited objects get recorded.
	if (GUndo)
	{
		TArray<TWeakObjectPtr<UObject>> Edited;
		Edited = DetailsView->GetSelectedObjects();

		for (TWeakObjectPtr<UObject> Obj : Edited)
		{
			if (Obj.IsValid())
			{
				Obj->Modify();
...
void FSoundSubmixEditor::SaveAsset_Execute()
{
	...
	// Only attempt to save/checkout dirty ones.
	const bool bCheckDirty = true;
	const bool bPromptToSave = false;
	FEditorFileUtils::PromptForCheckoutAndSave(PackagesToSave, bCheckDirty, bPromptToSave);
}

void FSoundSubmixEditor::RefreshDetails()
{
