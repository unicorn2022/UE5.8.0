...
#include "AssetTypeActions/AssetDefinition_SoundEffectPreset.h"
...
public:
	...
	virtual void StartupModule() override
	{
		...
		{
			UE_LOGF(LogAudioEditor, Display, "LibSoundFile failed to load. Importing audio will not work correctly.");
		}
		...
		for (TObjectIterator<UClass> It; It; ++It)
		{
			...
			if (ParentClass && (ParentClass->IsChildOf(USoundEffectSourcePreset::StaticClass()) || ParentClass->IsChildOf(USoundEffectSubmixPreset::StaticClass())))
			{
				...
				if (!RegisteredActions.Contains(EffectPreset) && EffectPreset->HasAssetActions())
				{
					...
					UAssetDefinition_SoundEffectPresetDynamic* Definition = NewObject<UAssetDefinition_SoundEffectPresetDynamic>(
						GetTransientPackage(),
						MakeUniqueObjectName(GetTransientPackage(), UAssetDefinition_SoundEffectPresetDynamic::StaticClass())
					);
					Definition->Initialize(TSubclassOf<USoundEffectPreset>(ChildClass));
