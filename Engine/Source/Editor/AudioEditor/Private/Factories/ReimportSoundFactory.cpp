...
EReimportResult::Type UReimportSoundFactory::Reimport(UObject* Obj)
{
	...
	if (!Filename.Len())
	{
		...
		//UE_LOGF(LogEditorFactories, Warning, "-- cannot reimport: sound node wave resource does not have path stored.");
		return EReimportResult::Failed;
	}

	UE_LOGF(LogAudioEditor, Log, "Performing atomic reimport of [%ls]", *Filename);

	// Ensure that the file provided by the path exists
	if (IFileManager::Get().FileSize(*Filename) == INDEX_NONE)
	{
		UE_LOGF(LogAudioEditor, Warning, "-- cannot reimport: source file cannot be found.");
		return EReimportResult::Failed;
	}
	...
	if (!ImportObject(SoundWave->GetClass(), SoundWave->GetOuter(), *SoundWave->GetName(), RF_Public | RF_Standalone, Filename, nullptr, OutCanceled))
	{
		if (OutCanceled)
		{
			UE_LOGF(LogAudioEditor, Warning, "-- import canceled");
			return EReimportResult::Cancelled;
		}

		UE_LOGF(LogAudioEditor, Warning, "-- import failed");
		return EReimportResult::Failed;
	}

	UE_LOGF(LogAudioEditor, Log, "-- imported successfully");
