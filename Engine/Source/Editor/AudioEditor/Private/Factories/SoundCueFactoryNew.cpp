...
UObject* USoundCueFactoryNew::FactoryCreateNew(UClass* Class, UObject* InParent, FName Name, EObjectFlags Flags, UObject* Context, FFeedbackContext* Warn)
{
	...
	if (SoundObjects.Num() > 0)
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogAudio, Warning, "RandomNode is limited to %d input Sound Sources. Excess Sound Sources in SoundCue %ls will be left unconnected.", RandomNode->GetMaxChildNodes(), *Name.ToString());
			}
