...
void USoundClassGraphSchema::GetContextMenuActions(UToolMenu* Menu, UGraphNodeContextMenuContext* Context) const
{
	...
	FToolMenuSection& AssetSection = Menu->AddSection("AssetGraphSchemaNodeActions", LOCTEXT("AssetActionsMenuHeader", "Asset Actions"));
	AssetSection.AddMenuEntry(FGlobalEditorCommonCommands::Get().FindInContentBrowser);
	AssetSection.AddMenuEntry(FSoundClassEditorCommands::Get().ViewReferences);
...
void USoundClassGraphSchema::GetAssetsGraphHoverMessage(const TArray<FAssetData>& Assets, const UEdGraph* HoverGraph, FString& OutTooltipText, bool& OutOkIcon) const
{
	OutTooltipText.Reset();
	OutOkIcon = false;

	const USoundClassGraph* SoundClassGraph = CastChecked<const USoundClassGraph>(HoverGraph);
	for (int32 AssetIdx = 0; AssetIdx < Assets.Num(); ++AssetIdx)
	{
		USoundClass* SoundClass = Cast<USoundClass>(Assets[AssetIdx].GetAsset());
		if (SoundClass)
		{
			if (!SoundClassGraph->IsClassDisplayed(SoundClass))
			{
				OutTooltipText = FText::FormatOrdered(LOCTEXT("DropSoundClass", "Add {0} to Graph."), USoundClass::StaticClass()->GetDisplayNameText()).ToString();
				OutOkIcon = true;
				break;
			}
			else if (OutTooltipText.IsEmpty())
			{
				OutTooltipText = FText::FormatOrdered(LOCTEXT("SoundClassInGraph", "{0} already in graph."), USoundClass::StaticClass()->GetDisplayNameText()).ToString();
			}
		}
	}
}
