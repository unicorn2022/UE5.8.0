...
bool FInstancedStructFilter::IsStructAllowed(const FStructViewerInitializationOptions& InInitOptions, const UScriptStruct* InStruct, TSharedRef<FStructViewerFilterFuncs> InFilterFuncs)
{
	...
	if (!AllowedStructs.IsEmpty())
	{
		...
		{
			if (bAllowChildStructs ? InStruct->IsChildOf(AllowedStruct.Get()) : InStruct == AllowedStruct)
			{
				bStructAllowed = true;
				break;
			}
	...
	if (!DisallowedStructs.IsEmpty())
	{
		...
		{
			if (bAllowChildStructs ? InStruct->IsChildOf(DisallowedStruct.Get()) : DisallowedStruct == InStruct)
			{
				bStructAllowed = false;
				break;
			}
...
TSharedRef<SWidget> SInstancedStructPicker::GenerateStructPicker()
{
	...
	static const FName NAME_ExcludeChildStructs = "ExcludeChildStructs";
	...
	const bool bExcludeChildStructs = StructProperty->HasMetaData(NAME_ExcludeChildStructs);
	...
	StructFilter->bAllowChildStructs = !bExcludeChildStructs;
	...
	if (GEditor && StructProperty)
	{
		...
		DECLARE_DELEGATE_RetVal(TArray<TSoftObjectPtr<UScriptStruct>>, FGetClasses);

		auto GatherStructs = [this, &OwningObjects](const FName MetaName, TArray<TSoftObjectPtr<const UScriptStruct>>& OutStructs)
		{
			const FString& FunctionName = StructProperty->GetMetaData(MetaName);
			if (!FunctionName.IsEmpty())
			{
				...
				{
					const UFunction* Function = FindObject<UFunction>(nullptr, *FunctionName, EFindObjectFlags::ExactClass);
					if (ensureMsgf(Function && Function->HasAnyFunctionFlags(EFunctionFlags::FUNC_Static), TEXT("Invalid %s: %s"), *MetaName.ToString(), *FunctionName))
					{
						UObject* FunctionCDO = Function->GetOwnerClass()->GetDefaultObject();
						OutStructs.Append(FGetClasses::CreateUFunction(FunctionCDO, Function->GetFName()).Execute());
					}
				}
				else
				{
					...
					{
						const UFunction* Function = OwningObject ? OwningObject->FindFunction(*FunctionName) : nullptr;
						if (Function != nullptr)
						{
							OutStructs.Append(FGetClasses::CreateUFunction(OwningObject, Function->GetFName()).Execute());
						}
					}
				}
			}
		...
		GatherStructs("GetAllowedClasses", StructFilter->AllowedStructs);
		GatherStructs("GetDisallowedClasses", StructFilter->DisallowedStructs);
	}
	else
	{
