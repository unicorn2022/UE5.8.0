...
void FInstancedStructDataDetails::OnStructLayoutChanges()
{
	...
	bStructChangeDetected = true;
}

FInstancedStructDataWithHeaderDetails::FInstancedStructDataWithHeaderDetails(TSharedPtr<IPropertyHandle> InStructProperty)
	: FInstancedStructDataDetails(InStructProperty)
{
}

void FInstancedStructDataWithHeaderDetails::GenerateHeaderRowContent(FDetailWidgetRow& NodeRow)
{
	FInstancedStructDataDetails::GenerateHeaderRowContent(NodeRow);

	// Use EnumerateConstRawData instead of GetValueData to support multi-object editing.
	// GetValueData requires byte-identical data across all objects, but we only need the struct type for the display name.
	const UScriptStruct* CommonStructType = nullptr;
	bool bAllSameType = true;
	StructProperty->EnumerateConstRawData([&CommonStructType, &bAllSameType](const void* RawData, const int32 /*DataIndex*/, const int32 /*NumDatas*/)
	{
		if (const FInstancedStruct* StructData = static_cast<const FInstancedStruct*>(RawData))
		{
			if (const UScriptStruct* StructType = StructData->GetScriptStruct())
			{
				if (CommonStructType == nullptr)
				{
					CommonStructType = StructType;
				}
				else if (CommonStructType != StructType)
				{
					bAllSameType = false;
					return false;
				}
	...
	if (CommonStructType && bAllSameType)
	{
		NodeRow.WholeRowWidget.Widget = SNew(SBox)
		.Padding(4.f, 0.f, 0.f, 0.f)
		.VAlign(VAlign_Center)
		[
			SNew(STextBlock)
			.Text(CommonStructType->GetDisplayNameText())
...
FInstancedStructArrayBuilder::FInstancedStructArrayBuilder(TSharedRef<IPropertyHandle> InArrayHandle)
	: FDetailArrayBuilder(InArrayHandle, false, false, false)
{
}

void FInstancedStructArrayBuilder::GenerateHeaderRowContent(FDetailWidgetRow& NodeRow)
{
	FDetailArrayBuilder::GenerateHeaderRowContent(NodeRow);
}

void FInstancedStructArrayBuilder::GenerateChildContent(IDetailChildrenBuilder& ChildrenBuilder)
{
...
void FInstancedStructDataDetails::AddChildRows(IDetailChildrenBuilder& ChildBuilder, const TArray<TSharedPtr<IPropertyHandle>>& ChildProperties)
{
	...
	{
		IDetailGroup** PropertyGroupPtr = PropertyToGroup.Find(ChildHandle);
		IDetailGroup* PropertyGroup = PropertyGroupPtr ? *PropertyGroupPtr : nullptr;
		AddChildRow(ChildBuilder, ChildHandle, PropertyGroup);
	}
}

void FInstancedStructDataDetails::AddChildRow(IDetailChildrenBuilder& ChildBuilder, TSharedPtr<IPropertyHandle> ChildHandle, IDetailGroup* PropertyGroup)
{
...
void FInstancedStructDataDetails::Tick(float DeltaTime)
{
	...
	TArray<TWeakObjectPtr<const UStruct>> InstanceTypes = GetInstanceTypes();
	if (bStructChangeDetected || InstanceTypes != CachedInstanceTypes)
	{
		bStructChangeDetected = false;
		OnRegenerateChildren.ExecuteIfBound();
	}
...
void FInstancedStructDetails::CustomizeHeader(TSharedRef<class IPropertyHandle> StructPropertyHandle, class FDetailWidgetRow& HeaderRow, IPropertyTypeCustomizationUtils& StructCustomizationUtils)
{
	...
	if (!StructProperty->HasMetaData(UE::StructUtils::Metadata::HideStructTypeName) || StructProperty->GetMetaData(UE::StructUtils::Metadata::HideStructTypeName) == TEXT("false"))
	{
		HeaderRow
...
TSharedRef<IPropertyTypeCustomization> FInstancedStructArrayDetails::MakeInstance()
{
	return MakeShared<FInstancedStructArrayDetails>();
}

void FInstancedStructArrayDetails::CustomizeHeader(TSharedRef<IPropertyHandle> PropertyHandle, FDetailWidgetRow& HeaderRow,	IPropertyTypeCustomizationUtils& CustomizationUtils)
{
	// Leaving empty so no header is displayed
}

void FInstancedStructArrayDetails::CustomizeChildren(TSharedRef<IPropertyHandle> PropertyHandle, IDetailChildrenBuilder& ChildBuilder, IPropertyTypeCustomizationUtils& CustomizationUtils)
{	
	TSharedPtr<IPropertyHandle> ValuesHandle = PropertyHandle->GetChildHandle(GET_MEMBER_NAME_CHECKED(FInstancedStructArray, Values));
	
	TSharedPtr<IPropertyHandleArray> ArrayHandle = ValuesHandle->AsArray();
	...
	TSharedRef<FInstancedStructArrayBuilder> ArrayBuilder = MakeShared<FInstancedStructArrayBuilder>(ValuesHandle.ToSharedRef());
	ArrayBuilder->OnGenerateArrayElementWidget(FOnGenerateArrayElementWidget::CreateSP(this, &FInstancedStructArrayDetails::OnGenerateArrayElementWidget));
	ChildBuilder.AddCustomBuilder(ArrayBuilder);
}

void FInstancedStructArrayDetails::OnGenerateArrayElementWidget(TSharedRef<IPropertyHandle> PropertyHandle, int32 Index, IDetailChildrenBuilder& DetailChildrenBuilder)
{
	DetailChildrenBuilder.AddCustomBuilder(MakeShared<FInstancedStructDataWithHeaderDetails>(PropertyHandle));
}
