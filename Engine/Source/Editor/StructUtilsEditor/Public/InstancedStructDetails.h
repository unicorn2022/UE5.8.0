...
protected:
	...
	/** Add all the rows for a child properties. */
	...
	/** Add all the rows for a child's property. */
	UE_API virtual void AddChildRow(IDetailChildrenBuilder& ChildBuilder, TSharedPtr<IPropertyHandle> ChildHandle, IDetailGroup* PropertyGroup);
...
private:
	...
	/** Mark the struct has changed, it requires a rebuild on the next iteration. */
	bool bStructChangeDetected = false;
...
class FInstancedStructDataWithHeaderDetails : public FInstancedStructDataDetails
{
public:
	FInstancedStructDataWithHeaderDetails(TSharedPtr<IPropertyHandle> InStructProperty);
	virtual ~FInstancedStructDataWithHeaderDetails() override = default;
	
	virtual void GenerateHeaderRowContent(FDetailWidgetRow& NodeRow) override;
};

class FInstancedStructArrayBuilder : public FDetailArrayBuilder, public TSharedFromThis<FInstancedStructArrayBuilder>
{
public:
	FInstancedStructArrayBuilder(TSharedRef<IPropertyHandle> InArrayHandle);
	virtual ~FInstancedStructArrayBuilder() override = default;
...
class FInstancedStructArrayDetails : public IPropertyTypeCustomization
{
public:
	FInstancedStructArrayDetails() = default;
	virtual ~FInstancedStructArrayDetails() override = default;
...
private:
	void OnGenerateArrayElementWidget(TSharedRef<IPropertyHandle> PropertyHandle, int32 Index, IDetailChildrenBuilder& DetailChildrenBuilder);
};
...
public:
	...
	virtual ~FInstancedStructProvider() override = default; 
