...
void SObjectNameEditableTextBox::Construct( const FArguments& InArgs )
{
	...
	const bool bShouldUseMiddleEllipsis = true; // GetDefault<UEditorStyleSettings>()->bEnableMiddleEllipsis;

	TSharedPtr<STextBlock> SuffixTextBlock = SNew(STextBlock)
		.Text(this, &SObjectNameEditableTextBox::GetObjectDisplayNameSuffix)
		.Visibility(this, &SObjectNameEditableTextBox::GetNameSuffixVisibility)
		.IsEnabled(false);
	...
	[	
		SNew(SHorizontalBox)
			...
			+ SHorizontalBox::Slot()
			.FillContentWidth(1000.0f)
			.VAlign(VAlign_Center)
			[
				SuffixTextBlock.ToSharedRef()
			]
	];
}
...
EVisibility SObjectNameEditableTextBox::GetNameSuffixVisibility() const
{
	if (const TSharedPtr<const UE::EditorWidgets::FObjectNameEditSinkRegistry> RegistryPtr = ObjectNameEditSinkRegistry.Pin())
	{
		if (Objects.Num() == 1)
		{
			if (const UObject* const ObjectPtr = Objects[0].Get())
			{
				const TSharedPtr<const UE::EditorWidgets::IObjectNameEditSink> ObjectNameEditPtr = RegistryPtr->GetObjectNameEditSinkForClass(ObjectPtr->GetClass());
				if (ObjectNameEditPtr)
				{
					return ObjectNameEditPtr->ObjectDisplayNameHasSuffix(ObjectPtr) ? EVisibility::Visible : EVisibility::Collapsed;
				}
			}
		}
	}
...
FText SObjectNameEditableTextBox::GetObjectDisplayNameSuffix() const
{
	if (const TSharedPtr<const UE::EditorWidgets::FObjectNameEditSinkRegistry> RegistryPtr = ObjectNameEditSinkRegistry.Pin())
	{
		if (Objects.Num() == 1)
		{
			if (const UObject* const ObjectPtr = Objects[0].Get())
			{
				...
				if (ObjectNameEditPtr)
				{
					return ObjectNameEditPtr->GetObjectDisplayNameSuffix(ObjectPtr);
				}
			}
		}
	}
