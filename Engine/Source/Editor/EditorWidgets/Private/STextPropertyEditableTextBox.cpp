...
FReply STextPropertyEditableStringTableReference::OnUnlinkClicked()
{
	...
	for (int32 TextIndex = 0; TextIndex < NumTexts; ++TextIndex)
	{
		...
		if (CurrentText.IsFromStringTable())
		{
			...
			NewKey = TextNamespaceUtil::GenerateRandomTextKey();
#endif	// USE_STABLE_LOCALIZATION_KEYS
			
			EditableTextProperty->SetText(TextIndex, FText::ChangeKey(NewNamespace, NewKey, CurrentText, CurrentText.GetDevNotes()));
		}
	}
				...
				[
					...
					[
						...
						[
							...
							.Text(this, &STextPropertyEditableTextBox::GetDevNotesValue)
							.Font(PropertyNormalFont)
							.ClearKeyboardFocusOnCommit(false)
							.SelectAllTextOnCommit(true)
							.ModiferKeyForNewLine(EModifierKey::Shift)
							.OnTextCommitted(this, &STextPropertyEditableTextBox::OnDevNotesCommitted)
							.IsReadOnly(this, &STextPropertyEditableTextBox::IsIdentityReadOnly)
							.AutoWrapText(true)
						]
					]
...
void STextPropertyEditableTextBox::OnTextCommitted(const FText& NewText, ETextCommit::Type CommitInfo)
{
	...
	if (NumTexts > 0 && (NumTexts == 1 || !NewText.ToString().Equals(MultipleValuesText.ToString(), ESearchCase::CaseSensitive)))
	{
		...
		for (int32 TextIndex = 0; TextIndex < NumTexts; ++TextIndex)
		{
			...
			EditableTextProperty->SetText(TextIndex, FText::ChangeKey(NewNamespace, NewKey, NewText, PropertyValue.GetDevNotes()));
		}
	}
}
...
void STextPropertyEditableTextBox::OnNamespaceCommitted(const FText& NewText, ETextCommit::Type CommitInfo)
{
	...
	if (NumTexts > 0 && (NumTexts == 1 || NewText.ToString() != MultipleValuesText.ToString()))
	{
		...
		for (int32 TextIndex = 0; TextIndex < NumTexts; ++TextIndex)
		{
			...
			EditableTextProperty->SetText(TextIndex, FText::ChangeKey(NewNamespace, NewKey, PropertyValue, PropertyValue.GetDevNotes()));
		}
	}
}
...
FText STextPropertyEditableTextBox::GetDevNotesValue() const
{
	FText DevNotes;
	...
	if (NumTexts == 1)
	{
		DevNotes = FText::FromString(EditableTextProperty->GetText(0).GetDevNotes());
	}
	else if (NumTexts > 1)
	{
		DevNotes = MultipleValuesText;
	}

	return DevNotes;
}

void STextPropertyEditableTextBox::OnDevNotesCommitted(const FText& NewText, ETextCommit::Type CommitInfo)
{
	const int32 NumTexts = EditableTextProperty->GetNumTexts();
	...
	if (NumTexts > 0 && (NumTexts == 1 || NewText.ToString() != MultipleValuesText.ToString()))
	{
		...
		{
			const FText PropertyValue = EditableTextProperty->GetText(TextIndex);
			const FString Namespace = FTextInspector::GetNamespace(PropertyValue).Get(FString());
			const FString Key = FTextInspector::GetKey(PropertyValue).Get(FString());

			EditableTextProperty->SetText(TextIndex, FText::ChangeKey(Namespace, Key, PropertyValue, NewText.ToString()));
		}
	}
}
