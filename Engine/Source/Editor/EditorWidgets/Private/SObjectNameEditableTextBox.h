...
private:
	...
	/** Helper function to access name registry and determine if the name has a suffix */
	EVisibility GetNameSuffixVisibility() const;

	/** Access name registry and retrieve the name suffix, if there is one */
	FText GetObjectDisplayNameSuffix() const;
