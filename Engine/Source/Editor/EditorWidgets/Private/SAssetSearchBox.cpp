...
void SAssetSearchBox::Construct( const FArguments& InArgs )
{
	...
	ChildSlot
		[
			...
			(
				...
				[
					SNew(SBox)
					.WidthOverride(this, &SAssetSearchBox::GetSuggestionListWidth)
					.HeightOverride(InArgs._SuggestionListHeight) // avoids flickering, ideally this would be adaptive to the content without flickering
					[
	...
	{
		InputText->SetToolTipText(InArgs._ToolTipText);
	}
	else if (InArgs._ToolTip.IsSet())
	{
		InputText->SetToolTip(InArgs._ToolTip);
	}
	else
	{
		InputText->SetToolTip(IDocumentation::Get()->CreateToolTip(NSLOCTEXT("AssetSearchBox", "FilterSearchHint", "Type here to search"), /*OverrideContent = */nullptr, "Shared/ContentBrowser", "AdvancedSearchSyntax"));
	}
}
...
FText SAssetSearchBox::GetText() const
{
...
FOptionalSize SAssetSearchBox::GetSuggestionListWidth() const
{
	// to enforce some minimum width, ideally we define the minimum, not a fixed width
	constexpr float MinWidth = 175.f;
	...
	return FMath::Max(InputText->GetCachedGeometry().GetLocalSize().X, MinWidth);
}
