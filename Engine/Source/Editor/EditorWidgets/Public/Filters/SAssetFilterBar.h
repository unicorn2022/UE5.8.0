...
public:
	...
	SLATE_BEGIN_ARGS( SAssetFilterBar<FilterType> )
		...
		SLATE_EVENT(SBasicFilterBar<FilterType>::FOnFilterBarLayoutChanging, OnFilterBarLayoutChanging)
		...
		/** The command list provided to the filter context menu */
	...
	void Construct( const FArguments& InArgs )
	{
		...
		CommandListWeak = InArgs._CommandList;
	...
	virtual void SaveSettings()
	{
		...
		if(!FilterBarConfig)
		{
			UE_LOGF(LogSlate, Error, "SFilterBar Requires that you specify a FilterBarIdentifier to save settings");
			return;
		}
...
protected:
	...
	/** Save all the custom filters (created programmatically) into the specified config */
	void SaveFilters(FFilterBarSettings* FilterBarConfig)
	{
		 ...
		 * ones to save their state, since they will be added to the filter bar programmatically every time
		 */
	...
	/** Load all the custom filters (created programmatically) from the specified config */
	void LoadFilters(const FFilterBarSettings* FilterBarConfig)
	{
		// Load all the custom filters (i.e FrontendFilters)
		TMap<FString, TSharedRef<FFilterBase<FilterType>>> CustomFilters;
		CustomFilters.Reserve(FilterBarConfig->CustomFilters.Num());
		for (const TSharedRef<FFilterBase<FilterType>>& FrontendFilter : this->AllFrontendFilters)
		{
			if (!this->IsFrontendFilterInUse(FrontendFilter))
			{
				...
				if (FilterBarConfig->CustomFilters.Contains(FilterName))
				{
					CustomFilters.Add(FilterName, FrontendFilter);
				}
			}
		}

		// We add them here to preserve the saving config order
		for (const TPair<FString, bool>& CustomFilter : FilterBarConfig->CustomFilters)
		{
			if (const TSharedRef<FFilterBase<FilterType>>* Filter = CustomFilters.Find(CustomFilter.Key))
			{
				TSharedRef<SFilter> NewFilter = this->AddFilterToBar(*Filter);
				NewFilter->SetEnabled(CustomFilter.Value, false);
				this->SetFrontendFilterActive(*Filter, NewFilter->IsEnabled());
			}
		}
		...
		TMap<FString, TSharedRef<FCustomClassFilterData>> AssetClassFilters;
		AssetClassFilters.Reserve(FilterBarConfig->TypeFilters.Num());
		for(const TSharedRef<FCustomClassFilterData> &CustomClassFilter : this->CustomClassFilters)
		{
			...
			{
				// Try to find this type filter in our list of saved filters
				if (FilterBarConfig->TypeFilters.Contains(FilterName))
				{
					AssetClassFilters.Add(FilterName, CustomClassFilter);
		...
		// We add them here to preserve the saving config order
		for (const TPair<FString, bool>& TypeFilter : FilterBarConfig->TypeFilters)
		{
			if (const TSharedRef<FCustomClassFilterData>* Filter = AssetClassFilters.Find(TypeFilter.Key))
			{
				TSharedRef<SFilter> NewFilter = this->AddAssetFilterToBar(*Filter);
				NewFilter->SetEnabled(TypeFilter.Value, false);
			}
		}
	}
...
public:
	...
	/** Set the active asset filters for this filter list */
	void SetActiveFilters(TConstArrayView<FTopLevelAssetPath> InFilters)
	{
		RemoveAllFilters();
		for (const FTopLevelAssetPath& FilterAssetPath : InFilters)
		{
			SetAssetTypeFilterCheckState(FilterAssetPath, ECheckBoxState::Checked, /** Enabled */true);
		}
	}
...
protected:
	...
	virtual TSharedRef<SWidget> MakeAddFilterMenu() override final
	{
		...
		const FToolMenuContext ToolMenuContext(CommandListWeak.Pin(), TSharedPtr<FExtender>(), CreateAssetFilterBarContext());
