...
static void AuditDuplicatedCoreStyles(const ISlateStyle& EditorStyle)
{
	...
	{
		UE_LOGF(LogSlate, Log, "%ls", *Name.ToString());
	}
}
...
void FStarshipEditorStyle::FStyle::SetupGeneralStyles()
{
	...
	// EULA RichText
	{
		...
		Set("EULA.Highlight", FTextBlockStyle(NormalText)
			.SetFont(DEFAULT_FONT("Bold", FStarshipCoreStyle::RegularTextSize))
			.SetColorAndOpacity(FLinearColor(1.0f, 1.0f, 1.0f)));
	...
	// Common icons
	{
		...
		Set("Icons.OpenFile", new IMAGE_BRUSH_SVG("Starship/Common/OpenFile", Icon16x16));
		...
		Set("Icons.EditHierarchy", new IMAGE_BRUSH_SVG("Starship/Common/EditHierarchy_16", Icon16x16));
	...
	Set( "SoftwareCursor_Hand", new IMAGE_BRUSH_SVG( "Icons/HandOpen_24", Icon24x24 ) );
	Set( "SoftwareCursor_Grab", new IMAGE_BRUSH_SVG( "Icons/HandGrab_24", Icon24x24 ) );
	...
	// Editor Diagnostics
	{
		...
		Set("EditorPerformance.Report.Minor", new CORE_IMAGE_BRUSH_SVG("Starship/Common/alert-triangle", Icon16x16, EStyleColor::Warning));
		Set("EditorPerformance.Report.Major", new CORE_IMAGE_BRUSH_SVG("Starship/Common/alert-triangle", Icon16x16, EStyleColor::AccentOrange));
		Set("EditorPerformance.Report.Critical", new CORE_IMAGE_BRUSH_SVG("Starship/Common/alert-triangle", Icon16x16, EStyleColor::Error));
		...
		Set("EditorDiagnostics.SetShowCrashes", new IMAGE_BRUSH_SVG("Starship/Common/Debug", Icon20x20));

		Set("EditorDiagnostics.StatusBarButton", GetWidgetStyle<FButtonStyle>("StatusBar.StatusBarButton"));

		FButtonStyle StatusBarButtonAlert = GetWidgetStyle<FButtonStyle>("StatusBar.StatusBarButton");
		StatusBarButtonAlert
					   .SetNormal(FSlateColorBrush(FStyleColors::Error))
					   .SetNormalForeground(FStyleColors::ForegroundInverted)
					   .SetHovered(FSlateColorBrush(FStyleColors::Error))
					   .SetHoveredForeground(FStyleColors::ForegroundHover)
					   .SetPressed(FSlateColorBrush(FStyleColors::Error))
					   .SetPressedForeground(FStyleColors::ForegroundHover);

		Set("EditorDiagnostics.StatusBarButtonAlert", StatusBarButtonAlert);
	}
	...
	// Editor preferences
	{
		...
		Set("SystemWideCommands.OpenEditorPreferences", new CORE_IMAGE_BRUSH_SVG("Starship/Common/Preferences", Icon16x16));
		Set("SystemWideCommands.OpenKeyboardShortcuts", new IMAGE_BRUSH_SVG("Starship/Common/ViewportControls", Icon16x16));
	}

	// Project settings
	{
		Set("ProjectSettings.TabIcon", new IMAGE_BRUSH_SVG("Starship/Common/ProjectSettings", Icon16x16));
		Set("SystemWideCommands.OpenProjectSettings", new IMAGE_BRUSH_SVG("Starship/Common/ProjectSettings", Icon16x16));
	}
	...
	// SettingsEditor
	{
		Set( "SettingsEditor.ListBorder", new FSlateRoundedBoxBrush(FStyleColors::Recessed, 4.0f) );

		constexpr float PaddingAmount = 2.0f;
		constexpr float ExtraTopPadding = 12.f;
		constexpr float ExtraBottomPadding = 8.f;
		constexpr float AllPluginsExtraTopPadding = 9.f;
		constexpr float AllPluginsExtraBottomPadding = 7.f;
		...
		Set( "CategoryTreeItem.Root.AllBackgroundPadding", FMargin(PaddingAmount, PaddingAmount + AllPluginsExtraTopPadding, PaddingAmount, PaddingAmount + AllPluginsExtraBottomPadding) );

		const FTextBlockStyle ButtonStyleText = FAppStyle::Get().GetWidgetStyle<FTextBlockStyle>("PrimaryButtonText");
		FTextBlockStyle Text = FTextBlockStyle(ButtonStyleText);
		...
		const FTextBlockStyle NormalStyleText = FAppStyle::Get().GetWidgetStyle<FTextBlockStyle>("NormalText");
		FTextBlockStyle CategoryText = FTextBlockStyle(NormalStyleText);
...
void FStarshipEditorStyle::FStyle::SetupSequencerStyles()
{
	...
	if (IncludeEditorSpecificStyles())
	{
		...
		Set("Sequencer.LinkedFilterToggleButton", BaseCheckBoxStyle
			.SetUncheckedImage(FSlateRoundedBoxBrush(FStyleColors::Transparent, 4.0f))
			...
			.SetCheckedPressedImage(FSlateRoundedBoxBrush(FStyleColors::PrimaryHover, 4.0f))
			.SetPadding(FMargin(2, 2))
		);
		Set("Sequencer.LinkedFiltering.LinkedIcon", new IMAGE_BRUSH_SVG("Starship/Sequencer/FilteringLinked_16", Icon16x16));
		Set("Sequencer.LinkedFiltering.InstancedIcon", new IMAGE_BRUSH_SVG("Starship/Sequencer/FilteringInstanced_16", Icon16x16));
		...
		Set("Sequencer.Bake", new IMAGE_BRUSH_SVG(TEXT("Sequencer/Bake_14"), Icon14x14));
		Set("Sequencer.Weight", new IMAGE_BRUSH_SVG(TEXT("Sequencer/Weight_14"), Icon14x14));
		...
		constexpr float CommonPadding = 3;
		Set("Sequencer.FilterAreaMargin", FMargin(2 * CommonPadding, CommonPadding));

		Set("Sequencer.KeyTransform.TranslateLeft", new IMAGE_BRUSH_SVG("Sequencer/MoveKeysLeft_20", Icon20x20));
		Set("Sequencer.KeyTransform.TranslateRight", new IMAGE_BRUSH_SVG("Sequencer/MoveKeysRight_20", Icon20x20));
		Set("Sequencer.KeyTransform.ScaleMultiply", new IMAGE_BRUSH_SVG("Sequencer/MultiplySolid_20", Icon20x20));
		Set("Sequencer.KeyTransform.ScaleDivide", new IMAGE_BRUSH_SVG("Sequencer/DivideSolid_20", Icon20x20));

		Set("Sequencer.SimpleView.ToggleToSimpleView", new IMAGE_BRUSH_SVG("Sequencer/PanelTopOpen", Icon20x20));
		Set("Sequencer.SimpleView.ToggleToFullView", new IMAGE_BRUSH_SVG("Sequencer/PanelBottomOpen", Icon20x20));

		Set("Sequencer.SimpleView.KeyDisplay.SelectedAndPinned", new IMAGE_BRUSH_SVG("Sequencer/SelectedAndPinned", Icon20x20));
		Set("Sequencer.SimpleView.KeyDisplay.Selected", new IMAGE_BRUSH("Icons/GeneralTools/Select_40x", Icon20x20));
		{
			FSlateBrush KeyDisplayAllBrush = *GetBrush("GenericCommands.SelectAll");
			KeyDisplayAllBrush.SetImageSize(Icon20x20);
			Set("Sequencer.SimpleView.KeyDisplay.All", new FSlateBrush(KeyDisplayAllBrush));
		}

		Set("Sequencer.SimpleView.DragHandleLeft", new IMAGE_BRUSH_SVG("Sequencer/DragHandleLeft_12", Icon12x12));
		Set("Sequencer.SimpleView.DragHandleRight", new IMAGE_BRUSH_SVG("Sequencer/DragHandleRight_12", Icon12x12));
		Set("Sequencer.SimpleView.DragHandleBoth", new IMAGE_BRUSH_SVG("Sequencer/DragHandleBoth_16", Icon16x16));

		Set("Sequencer.SimpleView.FrameAllKeys", new IMAGE_BRUSH_SVG("Sequencer/FrameAllKeys_16", Icon16x16));
		Set("Sequencer.SimpleView.ResetViewRange", new IMAGE_BRUSH_SVG("Sequencer/ResetViewRange_16", Icon16x16));
		Set("Sequencer.SimpleView.GoToFrame", new IMAGE_BRUSH_SVG("Sequencer/GoToFrame_16", Icon16x16));
	}
...
{
	SetupNewViewportToolbarStyles();
	SetupOldViewportToolbarStyles();
	SetupLegacyViewportToolbarStyles();
...
void FStarshipEditorStyle::FStyle::SetupNewViewportToolbarStyles()
{
	...
	// Variant with no background
	{
		FToolBarStyle ViewportToolbarNoBackground(ViewportToolbarStyle);
		ViewportToolbarNoBackground.SetBackground(FSlateNoResource());
		ViewportToolbarNoBackground.BackgroundPadding = FMargin(0.f);
		Set("NoBackgroundViewportToolbar", ViewportToolbarNoBackground);
		Set("NoBackgroundViewportToolbar.Button.Start", ButtonStyleStart);
        Set("NoBackgroundViewportToolbar.Button.Middle", ButtonStyleMiddle);
        Set("NoBackgroundViewportToolbar.Button.End", ButtonStyleEnd);
        
        Set("NoBackgroundViewportToolbar.ToggleButton.Start", ToggleButtonStyleStart);
        Set("NoBackgroundViewportToolbar.ToggleButton.Middle", ToggleButtonStyleMiddle);
        Set("NoBackgroundViewportToolbar.ToggleButton.End", ToggleButtonStyleEnd);
        
        // Ensure split buttons don't have rounded corners on the inside
        Set("NoBackgroundViewportToolbar.SplitToggleButton", ToggleButtonStyleStart);
        FComboButtonStyle SplitComboButton = ViewportToolbarStyle.ComboButtonStyle;
        SplitComboButton.ButtonStyle = ButtonStyleEnd;
        Set("NoBackgroundViewportToolbar.SplitComboButton", SplitComboButton);
	}
	...
	// Special style for raised transform buttons
	{
		...
		// Reuse the same styling as the base ViewportToolbar for Raised.SplitToggleButton
		Set("ViewportToolbar.TransformTools.Raised.SplitToggleButton", GetWidgetStyle<FCheckBoxStyle>("ViewportToolbar.Raised.SplitToggleButton"));
	}
	...
	// Viewport Toolbar Icons
	{
		...
		Set("ViewportToolbar.CameraSpeedReset", new IMAGE_BRUSH_SVG("Starship/Common/CameraSpeedReset_16", Icon16x16));
...
void FStarshipEditorStyle::FStyle::SetupOldViewportToolbarStyles()
{
...
void FStarshipEditorStyle::FStyle::SetupLegacyViewportToolbarStyles()
{
...
void FStarshipEditorStyle::FStyle::SetupGeneralIcons()
{
	...
	Set("Minus", new IMAGE_BRUSH("Icons/MinusSymbol_12x", Icon12x12));
...
void FStarshipEditorStyle::FStyle::SetupLevelEditorStyle()
...
#if WITH_EDITOR
	{
		...
		Set( "EditorViewport.VisualizeMegaLightsMode", new IMAGE_BRUSH_SVG("Starship/Common/MegaLights_16", Icon16x16 ) );
		...
		Set( "EditorViewport.MeshEdges",              new IMAGE_BRUSH_SVG("Starship/Common/MeshEdges_16", Icon16x16 ) );

		Set( "EditorViewport.AllChannelsMask",   new IMAGE_BRUSH_SVG("Starship/Common/ColorChannelRGB", Icon16x16 ) );
		Set( "EditorViewport.RedChannelMask",   new IMAGE_BRUSH_SVG("Starship/Common/ColorChannelRed", Icon16x16 ) );
		Set( "EditorViewport.GreenChannelMask", new IMAGE_BRUSH_SVG("Starship/Common/ColorChannelGreen", Icon16x16 ) );
		Set( "EditorViewport.BlueChannelMask",  new IMAGE_BRUSH_SVG("Starship/Common/ColorChannelBlue", Icon16x16 ) );
		Set( "EditorViewport.AlphaChannelMask", new IMAGE_BRUSH_SVG("Starship/Common/ColorChannelAlpha", Icon16x16 ) );
		...
		Set("LevelViewport.ToggleFlipConstrainedAspectRatioInPreview", new IMAGE_BRUSH_SVG("Starship/Common/RotationScreenspace_16", Icon16x16));
...
#if WITH_EDITOR || (IS_PROGRAM && WITH_UNREAL_DEVELOPER_TOOLS)
		{
			...
			Set( "LevelEditor.Tabs.Media",                  new IMAGE_BRUSH_SVG("Starship/AssetIcons/MediaPlayer_16", Icon16x16) );
	...
	// Show flags menus
	{
		...
		Set("ShowFlagsMenu.SubMenu.MegaLights", new IMAGE_BRUSH_SVG("Starship/Common/LightingFeatures", Icon16x16));
...
void FStarshipEditorStyle::FStyle::SetupClassIconsAndThumbnails()
{
	...
	// Actor Classes Outliner
	{
		...
		const FClassIconInfo AssetTypesSVG[] = {
			...
			{TEXT("LiveLinkPreset")},
			{TEXT("Material")},
			{TEXT("MaterialCacheVirtualTextureTag")},
			...
			{TEXT("TextureRenderTarget2DArray")},
		...
		// SVG Asset icons
		{
			for (int32 TypeIndex = 0; TypeIndex < UE_ARRAY_COUNT(AssetTypesSVG); ++TypeIndex)
			{
				...
				{
					UE_LOGF(LogSlate, Log, "%ls already found", *ClassIconName);
				}
...
void FStarshipEditorStyle::FStyle::SetupLandscapeEditorStyle()
{
	...
	// Landscape Editor
	{
		...
		Set("LandscapeEditor.Brush.Action_Regular", new IMAGE_BRUSH_SVG("Icons/Landscape/Landscape_Brush_Action_Regular_16x", Icon16x16));
		Set("LandscapeEditor.Brush.Action_Inverse", new IMAGE_BRUSH_SVG("Icons/Landscape/Landscape_Brush_Action_Inverse_16x", Icon16x16));
...
void FStarshipEditorStyle::FStyle::SetupToolkitStyles()
{
...
#if WITH_EDITOR
	{
		...
		Set( "MaterialEditor.SetShaderBallPreview", new IMAGE_BRUSH_SVG("Icons/AssetIcons/ModelingShaderBall_16", Icon16x16));
	...
	// New Curve Editor 
	{
		...
		Set("GenericCurveEditor.SmartSnapKeys", new IMAGE_BRUSH_SVG("Starship/GenericCurveEditor/SmartSnapKeys_16", Icon16x16));
	}
	...
	// PhysicsAssetEditor
	{
		...
		Set( "PhysicsAssetEditor.CreateBodyWithSingleConvex", new IMAGE_BRUSH("PhysicsAssetEditor/icon_PhAT_Convex_40x", Icon40x40));
		Set( "PhysicsAssetEditor.CreateBodyWithMultiConvex", new IMAGE_BRUSH("PhysicsAssetEditor/icon_PhAT_MultiConvex_40x", Icon40x40));
...
void FStarshipEditorStyle::FStyle::SetupHomeScreenStyles()
{
	...
	Set("HomeScreen.CreatorRules", new IMAGE_BRUSH_SVG("Starship/HomeScreen/ParentChild", Icon20x20));
	Set("HomeScreen.Support", new IMAGE_BRUSH_SVG("Starship/HomeScreen/HeadphonesMic", Icon20x20));
	...
	FSlateFontInfo RecentProjectFont = DEFAULT_FONT("Bold", 18);
	...
	FButtonStyle SeeAllProjectsButton = FButtonStyle()
		.SetNormal(FSlateRoundedBoxBrush(FStyleColors::Transparent, 8.0f, COLOR("565659FF"), 1.f))
		.SetHovered(FSlateRoundedBoxBrush(COLOR("404044FF"), 8.0f, COLOR("FFFFFFFF"), 1.f))
		.SetPressed(FSlateRoundedBoxBrush(COLOR("404044FF"), 8.0f, COLOR("FFFFFFFF"), 1.f))
		.SetHoveredForeground(FStyleColors::White)
		.SetNormalForeground(COLOR("EDEDEDFF"))
		.SetPressedForeground(FStyleColors::White)
		.SetNormalPadding(FMargin(0.f, 4.0f))
		.SetPressedPadding(FMargin(0.f, 4.0f));
