				...
				for (TSharedPtr<FTemplateItem> OneTemplate : EachTemplate.Value)
				{
					...
					for (int i = 0; i < 2; ++i)
					{
						...
						if (!GameProjectUtils::CreateProject(ProjectInfo, FailReason, FailLog, &CreatedFiles))
						{
							...
							UE_LOGF(LogGameProjectGenerationTests, Error, "Failed to create %ls project %ls based on %ls. Reason: %ls\nProject Creation Failure Log:\n%ls", *InCategory.ToString(), *DesiredProjectFilename, *Item->GetDisplayName().ToString(), *FailReason.ToString(), *FailLog.ToString());
						}
						else
						{
							// Created ok
...
bool FBuildPromotionNewProjectCleanupTest::RunTest(const FString& Parameters)
{
	...
	{
		UE_LOGF(LogGameProjectGenerationTests, Display, "Found an old project file at %ls", *DesiredProjectFilename);
		if (FPaths::IsProjectFilePathSet())
		{
			FString CurrentProjectPath = FPaths::GetProjectFilePath();
			if (CurrentProjectPath == DesiredProjectFilename)
			{
				UE_LOGF(LogGameProjectGenerationTests, Warning, "Can not clean up the target project location because it is the current active project.");
			}
			else
			{
				UE_LOGF(LogGameProjectGenerationTests, Display, "Removing files from old project path: %ls", *FPaths::GetPath(DesiredProjectFilename));
	...
	{
		UE_LOGF(LogGameProjectGenerationTests, Display, "Target project location is clear");
	}
...
bool FBuildPromotionNewProjectCreateTest::RunTest(const FString& Parameters)
{
	...
	if (FPaths::FileExists(DesiredProjectFilename))
	{
		UE_LOGF(LogGameProjectGenerationTests, Warning, "A project already exists at the target location: %ls", *DesiredProjectFilename);
		...
		for (uint32 i = 2;; ++i)
		{
			...
			if (!FPaths::FileExists(PossibleProjectFilename))
			{
				...
				UE_LOGF(LogGameProjectGenerationTests, Warning, "Changing the target project name to: %ls", *FPaths::GetBaseFilename(DesiredProjectFilename));
				break;
			}
		}
	}
	...
	{
		UE_LOGF(LogGameProjectGenerationTests, Display, "Generated a new project: %ls", *DesiredProjectFilename);
		UE_LOGF(LogGameProjectGenerationTests, Display, "Test successful!");
		UE_LOGF(LogGameProjectGenerationTests, Display, "\nPlease switch to the new project and continue to Step 2.");
	}
	else
	{
		UE_LOGF(LogGameProjectGenerationTests, Error, "Could not generate new project: %ls - %ls", *FailReason.ToString(), *FailLog.ToString());
		UE_LOGF(LogGameProjectGenerationTests, Display, "Test failed!");
	}
...
bool FBuildPromotionNewProjectMapTest::RunTest(const FString& Parameters)
{
	...
	if (!CurrentWorld)
	{
		UE_LOGF(LogGameProjectGenerationTests, Error, "Failed to create an empty level");
		return false;
	}

	UE_LOGF(LogGameProjectGenerationTests, Display, "Adding Level Geometry");
	...
	if (PlayerStart)
	{
		UE_LOGF(LogGameProjectGenerationTests, Display, "Added a player start");
	}
	else
	{
		UE_LOGF(LogGameProjectGenerationTests, Error, "Failed to add a player start");
	}
	...
	UE_LOGF(LogGameProjectGenerationTests, Display, "Saved map");
