	...
	void Construct(const FArguments& InArgs, TSharedPtr<FTemplateCategory> Item)
	{
		...
		[
			...
			[
				...
				{
					if (const TSharedPtr<FTemplateCategory> TemplateCategory = WeakItem.Pin())
					{
						return TemplateCategory->GetDisplayName();
					}
					...
					[
						...
						[
							...
							[
								...
								.Text_Lambda([this]()
								{
									if (const TSharedPtr<FTemplateItem> ItemPtr = Item.Pin())
									{
										return ItemPtr->GetDisplayName();
									}
									return FText::GetEmpty();
								})
			...
			[
				...
				[
					...
					[
						...
						.Text_Lambda([this]()
						{
							if (const TSharedPtr<FTemplateItem> SelectedItem = GetSelectedTemplateItem())
							{
...
TSharedRef<SWidget> SProjectDialog::MakePluginInformationWidget()
{
	...
	const FText PluginName = FText::FromString(Plugin->GetFriendlyName());
	const FText PluginAuthor = FText::FromString(PluginDescriptor.CreatedBy);
			...
			[
				...
				[
					...
					[
						...
						.VAlign(VAlign_Bottom)
						.HAlign(HAlign_Left)
						.Padding(0.0f, PaddingAmount, 0.0f, 0.0f)
						[
							SNew(SHorizontalBox)
							// version number
							+ SHorizontalBox::Slot()
							...
							[
								SNew(STextBlock)
								.Text(FText::Format(LOCTEXT("PluginVersionLabel", "Version {0}"), FText::FromString(PluginDescriptor.VersionName)))
								.TextStyle(FAppStyle::Get(), "DialogButtonText")
							]
						]
					]
...
TSharedRef<SWidget> SProjectDialog::MakeRecentProjectsTile()
{
	...
	RecentProjectsCategory->DisplayNameOverride = LOCTEXT("RecentProjects", "Recent Projects");
	RecentProjectsCategory->DescriptionOverride = FText::GetEmpty();
...
TMap<FName, TArray<TSharedPtr<FTemplateItem>> > SProjectDialog::FindTemplateProjects()
{
	...
	for (const auto& [Folder, Plugin] : AllTemplateFolders)
	{
		...
		Template->LocalizedDisplayNames = TemplateDefs->LocalizedDisplayNames;
		Template->LocalizedDescriptions = TemplateDefs->LocalizedDescriptions;
	...
	for (const FName& CategoryKey : CategoryKeys)
	{
		...
		if (!bCategoryExists)
		{
			UE_LOGF(LogGameProjectGeneration, Warning, "Failed to find category definition named '%ls', it is not defined in any TemplateCategories.ini.", *CategoryKey.ToString());
		}
	}
	...
	// Add blank template to empty categories
	{
		...
		BlankTemplate->DisplayNameOverride = LOCTEXT("BlankProjectName", "Blank");
		BlankTemplate->DescriptionOverride = LOCTEXT("BlankProjectDescription", "A clean empty project with no code and default settings.");
...
void SProjectDialog::SetDefaultProjectLocation()
{
	...
	if (!GameProjectUtils::IsValidProjectFileForCreation(DefaultProjectFilePath / ProjectName / ProjectName + TEXT(".") + FProjectDescriptor::GetExtension(), FailReason))
	{
		...
		if (Suffix >= MaxSuffix)
		{
			UE_LOGF(LogGameProjectGeneration, Warning, "Failed to find a suffix for the default project name");
			ProjectName = TEXT("");
		}
	}
...
void SProjectDialog::DisplayError(const FText& ErrorText)
{
	...
	UE_LOGF(LogGameProjectGeneration, Log, "%ls", *ErrorString);
...
void SProjectDialog::DisplayWarning(const FText& WarningText)
{
	...
	UE_LOGF(LogGameProjectGeneration, Log, "%ls", *WarningString);
...
TArray<TSharedPtr<FTemplateCategory>> SProjectDialog::GetAllTemplateCategories()
{
	...
	if (AllTemplateCategories.Num() == 0)
	{
		...
		DefaultCategory->DisplayNameOverride = LOCTEXT("ProjectDialog_DefaultCategoryName", "Blank Project");
		DefaultCategory->DescriptionOverride = LOCTEXT("ProjectDialog_DefaultCategoryDescription", "Create a new blank Unreal project.");
