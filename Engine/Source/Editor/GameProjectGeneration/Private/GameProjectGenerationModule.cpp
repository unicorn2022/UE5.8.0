...
TSharedRef<class SWidget> FGameProjectGenerationModule::CreateEmbeddedProjectBrowser()
{
	return SNew(SProjectBrowser).IsEmbedded(true);
}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FGameProjectGenerationModule::LoadTemplateCategories);
	...
	for (const FString& Root : TemplateRootFolders)
	{
		...
		if (CategoryDefs != nullptr)
		{
			for (const FTemplateCategoryDef& Category : CategoryDefs->Categories)
			{
				...
				TemplateCategory->LocalizedDisplayNames = Category.LocalizedDisplayNames;
				TemplateCategory->LocalizedDescriptions = Category.LocalizedDescriptions;
