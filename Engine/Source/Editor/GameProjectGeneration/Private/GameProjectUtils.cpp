...
bool GameProjectUtils::OpenProject(const FString& ProjectFile, FText& OutFailReason)
{
	...
	{
		FEngineAnalytics::GetProvider().RecordEvent(TEXT("Editor.OpenProject"));
	}
	...
	if (FPaths::IsProjectFilePathSet() && !IProjectManager::Get().IsSuppressingProjectFileWrite())
	{
		...
		if (IProjectManager::Get().QueryStatusForCurrentProject(ProjectStatus))
		{
			if ( ProjectStatus.bRequiresUpdate )
			{
				...
				UE_LOGF(LogGameProjectGeneration, Log, "Project %ls requires update. Determined from QueryStatusForCurrentProject", *ProjectStatus.Name);
			}
		}
		...
		for(FPluginReferenceDescriptor& Reference: NewPluginReferences)
		{
			if(Reference.bEnabled)
			{
				...
				if(Plugin.IsValid())
				{
					...
					{
						UE_LOGF(LogGameProjectGeneration, Log, "Project %ls requires update. Plugin %ls MarketplaceURL value in project descriptor (%ls) differs from value in plugin descriptor (%ls) ",
					...
					if(Reference.SupportedTargetPlatforms != Descriptor.SupportedTargetPlatforms)
					{
						...
						UE_LOGF(LogGameProjectGeneration, Log, "Project %ls requires update. Plugin %ls SupportedTargetPlatforms value in project descriptor (%ls) differs from value in plugin descriptor (%ls) ",
...
UTemplateProjectDefs* GameProjectUtils::LoadTemplateDefs(const FString& ProjectDirectory)
{
	...
	if ( FPlatformFileManager::Get().GetPlatformFile().FileExists(*TemplateDefsIniFilename) )
	{
		...
		if (bFoundValue && ClassName.Len() > 0)
		{
			...
			{
				UE_LOGF(LogGameProjectGeneration, Error, "Failed to find template project defs class '%ls', using default.", *ClassName);
			}
		}
		...
		{
			UE_LOGF(LogGameProjectGeneration, Warning, "Template '%ls' contains 'All' in HiddenSettings in addition to other entries. This is a mistake, and means that all settings will be hidden.", *ProjectDirectory);
		}
		...
		if (TemplateDefs->Variants.Num() > 0) // We make sure the Variants are valid
		{
			...
			for (TArray<FTemplateVariant>::TIterator It = TemplateDefs->Variants.CreateIterator(); It; ++It)
			{
				...
				{
					UE_LOGF(LogGameProjectGeneration, Warning, "Template '%ls' contains multiple Variants with the name '%ls', only the first one will be kept.", *ProjectDirectory, *Variant.Name.ToString());
				}
				else if (Variant.Name.IsNone()) // NAME_None is used to represent no Variant selected
				{
					UE_LOGF(LogGameProjectGeneration, Error, "Template '%ls' contains a Variant with the name '%ls' which is not allowed, it will be discarded.", *ProjectDirectory, *Variant.Name.ToString());
				}
				else
				{
...
TOptional<FGuid> GameProjectUtils::GenerateProjectFromScratch(const FProjectInformation& InProjectInfo, FText& OutFailReason, FText& OutFailLog)
{
	...
	UE_LOGF(LogGameProjectGeneration, Log, "Created new project with %d files (plus project files)", CreatedFiles.Num());
	return ProjectID;
}
	...
	for (const FTemplateConfigValue& ConfigValue : ConfigValues)
	{
		...
		if (FFileHelper::LoadANSITextFileToStrings(*IniFilename, &IFileManager::Get(), FileLines))
		{
			...
			if (!FFileHelper::SaveStringToFile(FileOutput, *IniFilename))
			{
				...
				OutFailReason = FText::Format(LOCTEXT("FailedToFixUpConfigFile", "Failed to process file {ConfigFile}."), Args);
				return false;
			}
		}
	}
	...
	SlowTask.EnterProgressFrame();
	{
		...
		for ( const FString& SrcFilename : FilesToCopy )
		{
			...
			if ( const FString* IgnoredFolder = TemplateDefs->FoldersToIgnore.FindByPredicate([&SrcFileSubpath](const FString& Ignore){ return SrcFileSubpath.StartsWith(Ignore + TEXT("/")); }) )
			{
				...
				UE_LOGF(LogGameProjectGeneration, Verbose, "'%ls': Skipping as it is in an ignored folder '%ls'", *SrcFilename, **IgnoredFolder);
				continue;
			}
			...
			for ( const FTemplateFolderRename& FolderRename : TemplateDefs->FolderRenames )
			{
				if ( SrcFileSubpath.StartsWith(FolderRename.From + TEXT("/")) )
				{
					...
					UE_LOGF(LogGameProjectGeneration, Verbose, "'%ls': Moving to '%ls' as it matched folder rename ('%ls'->'%ls')", *SrcFilename, *DestFileSubpathWithoutFilename, *FolderRename.From, *FolderRename.To);
				}
			}
			...
			for ( const FTemplateReplacement& Replacement : TemplateDefs->FilenameReplacements )
			{
				if ( Replacement.Extensions.Contains( FileExtension ) )
				{
					...
					{
						UE_LOGF(LogGameProjectGeneration, Verbose, "'%ls': Renaming to '%ls/%ls' as it matched file rename ('%ls'->'%ls')", *SrcFilename, *DestFileSubpathWithoutFilename, *DestBaseFilename, *Replacement.From, *Replacement.To);
					}
				}
			}
...
{
	return TEXT("BuildSettingsVersion.V7");
}
