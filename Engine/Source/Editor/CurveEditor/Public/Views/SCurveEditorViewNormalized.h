 ...
 */
class SCurveEditorViewNormalized : public SInteractiveCurveEditorView, public FSelfRegisteringEditorUndoClient
{
public:
	...
	virtual void OnCurveListChanged() override;
...
private:
	/** Recomputes a single curve's ViewToCurveTransform from its current value range. */
	void RecomputeViewToCurveTransform(const TSharedRef<FCurveEditor>& CurveEditor, FCurveModelID CurveID, FCurveInfo& CurveInfo);

	/** Curve IDs whose ViewToCurveTransform has been computed at least once. */
	TSet<FCurveModelID> InitializedCurves;
};
