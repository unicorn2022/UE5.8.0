...
namespace UE::CurveEditor { struct FCurveModelIdCache; }
namespace UE::CurveEditor { class FResolveCurveModelPasskey; }
...
public:
	...
	/** @return Metadata cache for reusing FCurveModelIDs when re-adding curves. For now, not API exported as it's for internal CurveEditor use only. */
	UE::CurveEditor::FCurveModelIdCache* GetCurveMetaDataCache() const { return CurveModelIdCache.Get(); }
...
public:
	...
	*/
	...
	UE_DEPRECATED(5.8, "SetSelectionRangeStart is no longer specialized for the Curve Editor")
	UE_API void SetSelectionRangeStart() {}
	UE_DEPRECATED(5.8, "SetSelectionRangeEnd is no longer specialized for the Curve Editor")
	UE_API void SetSelectionRangeEnd() {}
	UE_DEPRECATED(5.8, "ClearSelectionRange is no longer specialized for the Curve Editor")
	...
	UE_API void ExpandAllNodes();

	/** Collapse all nodes in the tree */
	UE_API void CollapseAllNodes();

	/** Sort all nodes in the tree */
	UE_API void SortAllNodes();
...
public:
	...
	/**
	 * Reset selected keys to their curve's default value, for curves that support it.
	 */
	UE_API void ResetSelectedKeysToDefaultValues();

	/**
	 * Returns true if any selected key belongs to a curve that has a default value.
	 */
	UE_API bool CanResetSelectedKeysToDefaultValues() const;
...
protected:
	...
	*/
	...
	/**
	 * Tries to re-use FCurveModelIDs when the conceptually same FCurveModel is added back to FCurveEditor using AddCurve.
	 * @see FCurveModelIdCache.
	 */
	TPimplPtr<UE::CurveEditor::FCurveModelIdCache> CurveModelIdCache;

private:

	friend class UE::CurveEditor::FResolveCurveModelPasskey;
	
	/**
	 * Internally used for editing FCurveModels after they've been removed FCurveEditor::CurveData (e.g. once they're no longer visible).
	 * Important for applying undo / redo commands on curves that are not visible when the user issues the undo / redo operation.
	 */
	UE::CurveEditor::FResolveCurveModelDelegate ResolveCurveModelDelegate;
	
	/** Tries to re-create a FCurveModel that was in this FCurveEditor previously but has since been removed. */
	TUniquePtr<FCurveModel> ResolveCurve(const FCurveModelID& InCurveID) const;
