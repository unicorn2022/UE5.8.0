...
public:
	...
	/**
	 * Returns whether this curve has a default value that selected keys can be reset to.
	 */
	virtual bool HasDefaultValue() const { return false; }

	/**
	 * Returns the default value for this curve. Only meaningful when HasDefaultValue() returns true.
	 */
	virtual double GetDefaultValue() const { return 0.0; }
	...
	*/
	...
	/**
	 * Get the owning object of this curve as the specified type, or search its outer chain if it
	 * is not the specified type. Subclasses can override the UClass-taking virtual below to inject
	 * objects (e.g. a directly-stored UMovieSceneSection on time warp curves whose owning getter
	 * is sequence-outered rather than section-outered).
	 */
	UE_API virtual UObject* GetOwningObjectOrOuter(UClass* Class) const;

	template<typename T>
	T* GetOwningObjectOrOuter() const
	{
		return Cast<T>(GetOwningObjectOrOuter(T::StaticClass()));
	}
