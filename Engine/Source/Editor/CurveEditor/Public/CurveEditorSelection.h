...
struct FKeyHandleSet
{
	...
	/**
	 * Replace this set with exactly these keys and assign them this point type
	 *
	 * @param InKeys The array of key handles to reset the set with.
	 * @param InPointType The point type to associate with the keys.
	 */
	UE_API void Replace(const TArrayView<const FKeyHandle>& InKeys, const ECurvePointType InPointType);
...
class FScopedSelectionChangedEventSuppressor
{
public:
	UE_API explicit FScopedSelectionChangedEventSuppressor(FCurveEditorSelection& InSelection);
	UE_API ~FScopedSelectionChangedEventSuppressor();
...
struct FCurveEditorSelection
{
	...
	friend FScopedSelectionChangedEventSuppressor;
	/**
	 * Default constructor
	 */
...
public:
	...
	/**
	 * Replaces the current key selection in one batched update using prebuilt per-curve key-handle sets.
	 * Curves that are invalid or read-only are skipped using the same validation rules as Add(...).
	 *
	 * @param InCurveToSelectedKeys Map of curve IDs to the exact selected keys to apply for each curve.
	 */
	UE_API void Replace(const TMap<FCurveModelID, FKeyHandleSet>& InCurveToSelectedKeys);

	/**
	 * Replaces the current key selection in one batched update using raw per-curve key-handle arrays.
	 * Curves that are invalid or read-only are skipped using the same validation rules as Add(...).
	 * All keys supplied for a given curve are associated with InPointType.
	 *
	 * @param InCurveToSelectedKeys Map of curve IDs to the selected key handles to apply for each curve.
	 * @param InPointType Point type to assign to every supplied key handle.
	 */
	UE_API void Replace(const TMap<FCurveModelID, TArray<FKeyHandle>>& InCurveToSelectedKeys, const ECurvePointType InPointType);
