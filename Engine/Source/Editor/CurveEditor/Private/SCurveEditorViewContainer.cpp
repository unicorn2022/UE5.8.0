...
FReply SCurveEditorViewContainer::OnKeyDown(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent)
{
	...
	{
		UE::CurveEditor::InputState::SetKeyPressed(InKeyEvent.GetKey(), true);
...
FReply SCurveEditorViewContainer::OnKeyUp(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent)
{
	...
	{
		UE::CurveEditor::InputState::SetKeyPressed(InKeyEvent.GetKey(), false);
...
FReply SCurveEditorViewContainer::OnPreviewMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	bIsScrubbingTime = UE::CurveEditor::InputState::IsScrubTimeCommandPressed(FCurveEditorCommands::Get().ScrubTime);

	if (bIsScrubbingTime && MouseEvent.GetEffectingButton() == EKeys::LeftMouseButton)
	{
...
FReply SCurveEditorViewContainer::OnMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	if (MouseEvent.GetEffectingButton() == EKeys::LeftMouseButton)
	{
		if (bIsScrubbingTime)
		{
			UE::CurveEditor::InputState::SetCommandPressed(FCurveEditorCommands::Get().ScrubTime, MouseEvent.GetModifierKeys(), true);
	...
	else if (MouseEvent.IsAltDown() && MouseEvent.GetEffectingButton() == EKeys::RightMouseButton)
	{
		DragOperation = FCurveEditorDelayedDrag(MousePixel, MouseEvent.GetEffectingButton());
...
FReply SCurveEditorViewContainer::OnMouseButtonUp(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	if (!Reply.IsEventHandled() && bIsScrubbingTime && MouseEvent.GetEffectingButton() == EKeys::LeftMouseButton)
	{
...
FReply SCurveEditorViewContainer::OnMouseWheel(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	if (TimeSliderController)
	{
		const FReply Reply = TimeSliderController->OnTimeSliderMouseWheel(*this, MyGeometry, MouseEvent);
...
{
	const bool bIsScrubCommand = UE::CurveEditor::InputState::IsScrubTimeCommandPressed(FCurveEditorCommands::Get().ScrubTime);
	if (bIsScrubCommand)
	{
