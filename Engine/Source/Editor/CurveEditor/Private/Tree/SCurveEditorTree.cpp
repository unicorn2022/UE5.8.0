...
void SCurveEditorTree::Construct(const FArguments& InArgs, TSharedPtr<FCurveEditor> InCurveEditor)
{
	...
	CurveEditor->GetTree()->GetOnExpandAllNodes().AddSP(this, &SCurveEditorTree::ExpandAllNodes);
	CurveEditor->GetTree()->GetOnCollapseAllNodes().AddSP(this, &SCurveEditorTree::CollapseAllNodes);
}
...
{	
	if(bUpdatingCurveEditorTreeSelection == false)
	{
		...
		for (const TPair<FCurveEditorTreeItemID, ECurveEditorTreeSelectionState>& CurveEditorTreeSelectionEntry : CurrentCurveEditorTreeSelection)
		{
			...
			{
				const FCurveEditorTreeItemID VisibleSelectionItem = STreeView<FCurveEditorTreeItemID>::IsItemVisible(CurveEditorTreeSelectionEntry.Key)
					? CurveEditorTreeSelectionEntry.Key
					: FindVisibleSelectionProxy(CurveEditorTreeSelectionEntry.Key);

				if (VisibleSelectionItem.IsValid())
				{
					NewTreeWidgetSelection.AddUnique(VisibleSelectionItem);
					CurrentTreeWidgetSelection.RemoveSwap(VisibleSelectionItem);
				}
			}
		}
...
void SCurveEditorTree::ExpandAllNodes()
{
	for (const FCurveEditorTreeItemID& RootItemID : CurveEditor->GetRootTreeItems())
	{
		SetItemExpansionRecursive(RootItemID, true);
	}
}

void SCurveEditorTree::CollapseAllNodes()
{
	for (const FCurveEditorTreeItemID& RootItemID : CurveEditor->GetRootTreeItems())
	{
		SetItemExpansionRecursive(RootItemID, false);
	}
}
...
FCurveEditorTreeItemID SCurveEditorTree::FindVisibleSelectionProxy(const FCurveEditorTreeItemID InItemID) const
{
	const FCurveEditorTree& CurveEditorTree = *CurveEditor->GetTree();

	const FCurveEditorTreeItem* TreeItem = CurveEditorTree.FindItem(InItemID);
	while (TreeItem)
	{
		if (IsItemVisible(TreeItem->GetID()))
		{
			return TreeItem->GetID();
		}

		TreeItem = CurveEditorTree.FindItem(TreeItem->GetParentID());
	}
