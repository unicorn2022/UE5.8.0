...
{
	const FCurveEditorTreeItemID NewItemID = NextTreeItemID;
	{
		const FScopedCurveEditorTreeEventGuard EventGuard(this);
	...
	// ~FScopedCurveEditorTreeEventGuard executes FCurveEditorTree::Compact, so NewItem may have been reallocated.
	FCurveEditorTreeItem* Result = Items.Find(NewItemID);
	ensure(Result);
	return Result;
}
...
void FCurveEditorTree::ExpandAllNodes()
{
	ExpandAllNodesDelegate.Broadcast();
}

void FCurveEditorTree::CollapseAllNodes()
{
	CollapseAllNodesDelegate.Broadcast();
}
...
{
	SortPredicate = FCurveTreeItemSortPredicate::CreateLambda(
		[InSortPredicate = MoveTemp(InSortPredicate)](const FCurveEditorTreeItem& ItemA, const FCurveEditorTreeItem& ItemB)
		{
			return InSortPredicate.Execute(ItemA.GetItem().Get(), ItemB.GetItem().Get());
		});
}
PRAGMA_ENABLE_DEPRECATION_WARNINGS

void FCurveEditorTree::SetSortPredicate(FCurveTreeItemSortPredicate InSortPredicate)
{
	SortPredicate = MoveTemp(InSortPredicate);
}
...
void FCurveEditorTree::SortTreeItems(FSortedCurveEditorTreeItems& TreeItemIDsToSort)
{
	...
	{
		TreeItemsToSort.Sort([this](const FCurveEditorTreeItem& ItemA, const FCurveEditorTreeItem& ItemB)
		{
			return SortPredicate.Execute(ItemA, ItemB);
		});
