...
UCurveEditorSettings::UCurveEditorSettings()
{
	...
	bCubicAddKeyPrefersUserTangent = false;
	bMiddleMouseButtonAddsKey = true;

	SelectionColor = FLinearColor::White;
	ParentSpaceCustomColor = FLinearColor(.93f, .31f, .19f); //pastel orange
	WorldSpaceCustomColor = FLinearColor(.198f, .610f, .558f); //pastel teal
...
bool UCurveEditorSettings::AllowMousePan(const FPointerEvent& MouseEvent) const
{
	...
	{
		return MouseEvent.IsMouseButtonDown(EKeys::RightMouseButton);
	}
...
bool UCurveEditorSettings::GetShowTransportControls() const
{
	return bShowTransportControls;
}

void UCurveEditorSettings::SetShowTransportControls(const bool bInShow)
{
	if (bShowTransportControls != bInShow)
	{
		bShowTransportControls = bInShow;
		SaveConfig();
	}
}
...
bool UCurveEditorSettings::GetCubicAddKeyPrefersUserTangent() const
{
	return bCubicAddKeyPrefersUserTangent;
}

void UCurveEditorSettings::SetCubicAddKeyPrefersUserTangent(bool bInCubicAddKeyPrefersUserTangent)
{
	if (bCubicAddKeyPrefersUserTangent != bInCubicAddKeyPrefersUserTangent)
	{
		bCubicAddKeyPrefersUserTangent = bInCubicAddKeyPrefersUserTangent;
		SaveConfig();
	}
}
...
bool UCurveEditorSettings::GetMiddleMouseButtonAddsKey() const
{
	return bMiddleMouseButtonAddsKey;
}

void UCurveEditorSettings::SetMiddleMouseButtonAddsKey(bool bInValue)
{
	if (bMiddleMouseButtonAddsKey != bInValue)
	{
		bMiddleMouseButtonAddsKey = bInValue;
		SaveConfig();
	}
}
...
FLinearColor UCurveEditorSettings::GetNextRandomColor()
{
	...
	{
		IndexedColor.Add(FLinearColor(.904f, .323f, .539f)); //pastel red
		IndexedColor.Add(FLinearColor(.552f, .737f, .328f)); //pastel green
		IndexedColor.Add(FLinearColor(.947f, .418f, .219f)); //pastel orange
		IndexedColor.Add(FLinearColor(.156f, .624f, .921f)); //pastel blue
		IndexedColor.Add(FLinearColor(.921f, .314f, .337f)); //pastel red 2
		IndexedColor.Add(FLinearColor(.361f, .651f, .332f)); //pastel green 2
		IndexedColor.Add(FLinearColor(.982f, .565f, .254f)); //pastel orange 2
		IndexedColor.Add(FLinearColor(.246f, .223f, .514f)); //pastel purple
		IndexedColor.Add(FLinearColor(.208f, .386f, .687f)); //pastel blue2
		IndexedColor.Add(FLinearColor(.223f, .590f, .337f)); //pastel green 3
		IndexedColor.Add(FLinearColor(.230f, .291f, .591f)); //pastel blue 3
	}
