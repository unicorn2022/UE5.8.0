	...
	const bool bNeedToTestExisting, FKeyHandleSet& OutHandleSet)
{
	...
	// bOnlyReduceStaticChannels: skip curves whose value range is not flat
	if (InParams.bOnlyReduceStaticChannels)
	{
		double MinValue, MaxValue;
		Curve->GetValueRange(MinValue, MaxValue);
		if (!FMath::IsNearlyEqual(MinValue, MaxValue, UE_KINDA_SMALL_NUMBER))
		{
			return;
		}
	}
	...
	// bDeleteStaticChannels: if exactly 2 keys remain with the same value, remove them
	// and let RemoveKeys set the channel default via InCurrentTime
	if (InParams.bDeleteStaticChannels)
	{
		TArray<FKeyHandle> AllRemainingKeys;
		Curve->GetAllKeys(AllRemainingKeys);

		if (AllRemainingKeys.Num() == 2)
		{
			TArray<FKeyPosition> KeyPositions;
			KeyPositions.SetNum(2);
			Curve->GetKeyPositions(AllRemainingKeys, KeyPositions);

			if (FMath::IsNearlyEqual(
				KeyPositions[0].OutputValue,
				KeyPositions[1].OutputValue,
				UE_KINDA_SMALL_NUMBER))
			{
				// Remove both keys; InCurrentTime = first key time so the default
				// is evaluated to the constant value and stored on the channel
				Curve->RemoveKeys(AllRemainingKeys, KeyPositions[0].InputValue);
				for (const FKeyHandle& RemainingHandle : AllRemainingKeys)
				{
					OutHandleSet.Remove(RemainingHandle, ECurvePointType::Key);
				}
			}
		}
	}
