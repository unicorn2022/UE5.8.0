...
bool UCurveEditorEulerFilter::CanApplyFilter_Impl(TSharedRef<FCurveEditor> InCurveEditor)
{
	...
	// No selection — allow if any visible curve is a rotation curve
	return Algo::AnyOf(InCurveEditor->GetCurves(),
		[](const TTuple<FCurveModelID, TUniquePtr<FCurveModel>>& CurvePair)
		{
			return CurvePair.Value && CurvePair.Value->GetIntentionName().Contains("Rotation");
		});
}
