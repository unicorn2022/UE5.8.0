...
FReply SCurveEditorFilterPanel::OnApplyClicked()
{
	...
	if (CurveEditor && DetailsView->GetSelectedObjects().Num() > 0)
	{
		...
		{
			TransactionText = NSLOCTEXT("CurveEditorFilterApply", "Filter All Visible Curves", "Filtered All Visible Curves");
		}
...
FText SCurveEditorFilterPanel::GetCurrentFilterText() const
{
	...
	{
		const int32 SelectedKeysCount = CurveEditor->GetSelection().Count();
		...
		else if (CurveEditor->GetCurves().Num() > 0)
		{
			const FText FormatString = NSLOCTEXT("SCurveEditorFilterPanel", "CurrentFilterAllCurvesFormat", "Current Filter: {0} (All Visible Curves)");
