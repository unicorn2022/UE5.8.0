...
void SInteractiveCurveEditorView::UpdatedKeysWithValueIndicatorLines(const FCurveEditor& InCurveEditor) const
{
	...
	else if (Selection.Num() == 1)
	{
		...
		for (const TPair<FCurveModelID, FKeyHandleSet>& Pair : Selection)
		{
			...
			FKeyHandle Min = FKeyHandle::Invalid();
			FKeyHandle Max = FKeyHandle::Invalid();
...
void SInteractiveCurveEditorView::UpdateCurveProximities(FVector2D MousePixel)
{
	...
	if (UE::CurveEditor::InputState::IsScrubTimeCommandPressed(FCurveEditorCommands::Get().ScrubTime))
	{
		CurveProximities.Reset();
		CachedToolTipData.Reset();

		if (PreviouslyHovered.IsSet())
		{
			RefreshRetainer();
		}
		return;
...
void SInteractiveCurveEditorView::HandleDirectKeySelectionByMouse(TSharedPtr<FCurveEditor> CurveEditor, const FPointerEvent& MouseEvent, TOptional<FCurvePointHandle> MouseDownPoint)
{
	...
	else
	{
		...
		if (bKeySelected || bLeaveTangentSelected || bArriveTangentSelected)
		{
			...
			else if (MouseDownPoint->PointType == ECurvePointType::Key && !bKeySelected)
			{
				TArray<FCurvePointHandle> CurvePointHandles;
				for (const TTuple<FCurveModelID, FKeyHandleSet>& Pair : CurveEditor->GetSelection().GetAll())
				{
...
FReply SInteractiveCurveEditorView::OnMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	if (CurveEditor->GetSettings()->AllowMouseEdit(MouseEvent))
	{
		...
		if (TOptional<FCurveModelID> HoveredCurve = GetHoveredCurve())
		{
			...
			if (CurveToAddTo && !CurveToAddTo->IsReadOnly() && !bDraggingKeys && !bKeyAlreadyExists && CurveEditor->GetSettings()->GetMiddleMouseButtonAddsKey())
			{
	...
	// Alt RMB always Zoom Timeline
	else if (MouseEvent.IsAltDown() && MouseEvent.IsMouseButtonDown(EKeys::RightMouseButton))
	{
		DragOperation = FCurveEditorDelayedDrag(MousePixel, MouseEvent.GetEffectingButton());
...
FReply SInteractiveCurveEditorView::OnMouseButtonUp(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	if (!bWasDragging && MouseEvent.GetEffectingButton() == EKeys::LeftMouseButton)
	{
		...
		if (!HitPoint(MousePosition).IsSet() && HitCurve.IsSet())
		{
			...
			// Only open a transaction if this click would actually change the selection.
			const FCurveEditorSelection& CurrentSelection = CurveEditor->GetSelection();
			const FKeyHandleSet* CurveKeys = CurrentSelection.FindForCurve(HitCurve.GetValue());
			const int32 NumCurveKeysSelected = CurveKeys ? CurveKeys->Num() : 0;

			bool bWouldChangeSelection;
			if (bIsShiftDown)
			{
				bWouldChangeSelection = NumCurveKeysSelected < KeyHandles.Num();
			}
			else if (bIsAltDown)
			{
				bWouldChangeSelection = NumCurveKeysSelected > 0;
			}
			else if (bIsControlDown)
			{
				bWouldChangeSelection = KeyHandles.Num() > 0;
			}
			else
			{
				bWouldChangeSelection = CurrentSelection.Count() != KeyHandles.Num() || NumCurveKeysSelected != KeyHandles.Num();
			}

			if (bWouldChangeSelection)
			{
...
void SInteractiveCurveEditorView::RebindContextualActions(FVector2D InMousePosition)
{
	...
	CommandList->UnmapAction(FCurveEditorCommands::Get().AddKeyToSelectedCurves);
	CommandList->UnmapAction(FCurveEditorCommands::Get().ResetKeysToDefaultValues);
	...
	CommandList->MapAction(FCurveEditorCommands::Get().AddKeyToAllCurves, FExecuteAction::CreateSP(this, &SInteractiveCurveEditorView::AddKeyAtScrubTime, false));

	CommandList->MapAction(
		FCurveEditorCommands::Get().AddKeyToSelectedCurves,
		FExecuteAction::CreateSP(this, &SInteractiveCurveEditorView::AddKeyAtScrubTime, true /*use selection*/));

	CommandList->MapAction(
		FCurveEditorCommands::Get().ResetKeysToDefaultValues,
		FExecuteAction::CreateSP(this, &SInteractiveCurveEditorView::ResetKeysToDefaultValues, true /*use selection*/));
...
FKeyAttributes SInteractiveCurveEditorView::GetDefaultKeyAttributesForCurveTime(const FCurveEditor& CurveEditor, const FCurveModel& CurveModel, double EvalTime) const
{
    ...
    TPair<ERichCurveInterpMode, ERichCurveTangentMode> Modes = CurveModel.GetInterpolationMode(
    	EvalTime,
	...
	if (const UCurveEditorSettings* Settings = GetDefault<UCurveEditorSettings>())
	{
		if (Settings->GetCubicAddKeyPrefersUserTangent())
		{
			if (Modes.Key == RCIM_Cubic)
			{
				Modes.Value = RCTM_User;
			}
		}
	}
...
void SInteractiveCurveEditorView::ResetKeysToDefaultValues(bool bUseSelection)
{
	using namespace UE::CurveEditor;
	...
	TSet<FCurveModelID> CurvesToReset;
	if (bUseSelection == false)
	{
		CurvesToReset = CurveEditor->GetEditedCurves();
	}
	else
	{
		for (const auto& Pair : CurveEditor->GetSelection().GetAll())
		{
			CurvesToReset.Add(Pair.Key);
		}
	}

	FScopedTransaction Transaction(LOCTEXT("ResetKeysToDefaultValues", "Set Default Value")); 
	FScopedSelectionChange SelectionChange(WeakCurveEditor);
	FScopedCurveChange KeyChange(
		FCurvesSnapshotBuilder(WeakCurveEditor, CurvesToReset, ECurveChangeFlags::MoveKeys)
	);

	FCurveEditorSelection& Selection = CurveEditor->GetSelection();
	TArray<FKeyHandle> KeyHandles;
	TArray<FKeyPosition> SelectedKeyPositions;
	for (FCurveModelID CurveID : CurvesToReset)
	{
		if (FCurveModel* CurveModel = CurveEditor->FindCurve(CurveID))
		{
			if (CurveModel->HasDefaultValue())
			{
				double DefaultValue = CurveModel->GetDefaultValue();
				if (const FKeyHandleSet* SelectionSet = Selection.FindForCurve(CurveID))
				{
					if (SelectionSet->Num() > 0)
					{
						KeyHandles = SelectionSet->AsArray();
						SelectedKeyPositions.SetNum(KeyHandles.Num());
						CurveModel->GetKeyPositions(KeyHandles, SelectedKeyPositions);
						for (FKeyPosition &Key : SelectedKeyPositions)
						{
							Key.OutputValue = DefaultValue;
						}
						CurveModel->SetKeyPositions(KeyHandles, SelectedKeyPositions);
					}
				}
			}
		}
...
void SInteractiveCurveEditorView::AddKeyAtScrubTime(bool bUseSelection)
{
	TSharedPtr<FCurveEditor> CurveEditor = WeakCurveEditor.Pin();
	if (!CurveEditor.IsValid())
	{
