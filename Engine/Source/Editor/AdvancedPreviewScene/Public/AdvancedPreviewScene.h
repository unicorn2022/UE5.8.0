...
public:
	...
	UE_DEPRECATED(5.8, "bShowGrid is deprecated, this function does nothing")
	UE_API void HandleToggleGrid();
	
	/** Whether the grid is enabled. */
	UE_DEPRECATED(5.8, "bShowGrid is deprecated, this function returns false")
	...
	/** Sets the mesh for the floor */
	UE_API void SetFloorMesh(UStaticMesh* InFloorMesh);
	
	/** Sets the floor material on a specific slot, or -1 for all slots */
	UE_API void SetFloorMaterial(UMaterialInterface* InFloorMaterial, int32 InIndex = -1);

	/** Sets the floor mesh rotation */
	UE_API void SetFloorMeshRotation(const FRotator& InRotation);

	/** Sets the floor mesh scale */
	UE_API void SetFloorMeshScale(const FVector& InScale);

	/** Register sky rotation and light rotation viewport interactions on the given behavior source. */
	UE_API static void AddPreviewSceneInteractions(class UViewportInteractionsBehaviorSource* InBehaviorSource);
...
protected:
	...
	UE_API bool UsesViewportInteractions(const FEditorViewportClient* InViewportClient) const;

	/** Navigate to the next or previous profile */
	enum class ENavigationDirection : uint8
	{
		Forward,
		Backward,
	};
	UE_API void NavigateProfile(ENavigationDirection InDirection);
