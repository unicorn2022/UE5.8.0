...
UENUM()
enum class EProfileConfigVersion : uint8
{
	PreVersioning = 0,
	/** New Material M_Grid applied on shared engine profiles */
	ApplyNewGridMaterial,
	VersionPlusOne,
	LatestVersion = VersionPlusOne - 1
};

PRAGMA_DISABLE_DEPRECATION_WARNINGS // For bShowGrid
/**
...
struct FPreviewSceneProfile
{
	...
	{		
		...
		EnvironmentFloorMesh = nullptr;
		EnvironmentFloorMeshPath = TEXT("/Engine/EditorMeshes/AssetViewer/Floor_Mesh.Floor_Mesh");
		EnvironmentFloorMaterial = nullptr;
		EnvironmentFloorMaterialPath = TEXT("/Engine/EngineMaterials/M_Grid.M_Grid");
		EnvironmentFloorScale = FVector(4.0f, 4.0f, 1.0f);
		EnvironmentFloorRotation = FRotator();
		bPostProcessingEnabled = true;
		DirectionalLightRotation = FRotator(-40.f, -67.5f, 0.f);
		bEnableToneMapping = true;
		bShowMeshEdges = false;
		// This will stop scene from becoming bright due to exposure
		PostProcessingSettings.bOverride_AutoExposureMinBrightness = true;
		PostProcessingSettings.AutoExposureMinBrightness = -1;
		PostProcessingSettings.bOverride_AutoExposureMaxBrightness = true;
		PostProcessingSettings.AutoExposureMaxBrightness = 1;
	}
	...
	UPROPERTY(EditAnywhere, config, Category = Environment, meta=(DisplayAfter="EnvironmentCubeMap"))
	bool bShowFloor;

	/** Toggle visibility of floor grid on/off */
	UE_DEPRECATED(5.8, "Removed from preview scene profile since it's not relevant to every viewport")
	UPROPERTY(config)
	...
	UPROPERTY(EditAnywhere, transient, Category = Environment, meta=(EditCondition="bShowEnvironment", AssetViewInitialViewType=1, AssetViewInitialThumbnailSize=4, AssetViewSettingsName="", ForceShowPluginContent, ForceShowEngineContent))
	...
	/** Sets the environment floor mesh */
	UPROPERTY(EditAnywhere, transient, Category = Environment, meta=(EditCondition="bShowFloor", AssetViewInitialViewType=1, AssetViewInitialThumbnailSize=4, AssetViewSettingsName="", ForceShowPluginContent, ForceShowEngineContent))
	TSoftObjectPtr<UStaticMesh> EnvironmentFloorMesh;

	/** Storing path to the environment floor mesh to prevent it from getting cooked */
	UPROPERTY(config)
	FString EnvironmentFloorMeshPath;

	/** Sets the rotation of the floor mesh */
	UPROPERTY(EditAnywhere, config, Category = Environment, meta=(EditCondition="bShowFloor"))
	FRotator EnvironmentFloorRotation;

	/** Sets the scale of the floor mesh */
	UPROPERTY(EditAnywhere, config, Category = Environment, meta=(EditCondition="bShowFloor"))
	FVector EnvironmentFloorScale;

	/** Sets the environment floor material */
	UPROPERTY(EditAnywhere, transient, Category = Environment, meta=(EditCondition="bShowFloor", AssetViewInitialViewType=1, AssetViewInitialThumbnailSize=4, AssetViewSettingsName="", ForceShowPluginContent, ForceShowEngineContent))
	TSoftObjectPtr<UMaterialInterface> EnvironmentFloorMaterial;

	/** Storing path to the environment floor material to prevent it from getting cooked */
	UPROPERTY(config)
	FString EnvironmentFloorMaterialPath;
	...
	/** Loads and returns the environment floor mesh */
	UE_API UStaticMesh* GetEnvironmentFloorMesh() const;
	
	/** Loads and returns the environment floor material */
	UE_API UMaterialInterface* GetEnvironmentFloorMaterial() const;
	
	/** Loads required objects for this profile */
	UE_API void LoadProfileObjects();
	
	/** Called when a profile property has been changed */
	void PostEditChangeProperty(FPropertyChangedEvent& InEvent);
	
	bool operator==(const FPreviewSceneProfile& InOther) const
	{
		return ProfileName.Equals(InOther.ProfileName);
	}
	...
	friend uint32 GetTypeHash(const FPreviewSceneProfile& InItem)
	{
		return GetTypeHash(InItem.ProfileName);
	}
};

PRAGMA_ENABLE_DEPRECATION_WARNINGS // For bShowGrid
...
class USharedProfiles : public UObject
{
	GENERATED_BODY()
public:
	EProfileConfigVersion GetVersion() const;
...
private:
	/** Used for auto migrations when new changes are introduced */
	UPROPERTY(Config)
	EProfileConfigVersion Version = EProfileConfigVersion::PreVersioning;
};
...
public:
	...
	UE_API virtual ~UAssetViewerSettings() override;
	...
	UE_API virtual void PostEditChangeChainProperty(struct FPropertyChangedChainEvent& InPropertyChangedEvent) override;
