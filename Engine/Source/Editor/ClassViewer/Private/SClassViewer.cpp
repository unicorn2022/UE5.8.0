...
public:
	...
	void Construct( const FArguments& InArgs, const TSharedRef<STableViewBase>& InOwnerTableView )
	{
		...
		UE_LOGF(LogEditorClassViewer, VeryVerbose, "CLASS [%ls]", **ClassName);
...
void FClassHierarchy::PopulateClassHierarchy()
{
	...
	// Retrieve all blueprint classes 
	{
		...
		for (const FAssetData& AssetData : Assets)
		{
			...
			{
				UE_LOGF(LogEditorClassViewer, Warning, "AssetRegistry Blueprint %ls is missing tag value for %ls. Blueprint will not be available to ClassViewer when unloaded.",
	...
	for (TPair<FTopLevelAssetPath, TSharedPtr<FClassViewerNode>>& KVPair : ClassPathToNode)
	{
		...
		if (!ParentNodePtr)
		{
			UE_LOGF(LogEditorClassViewer, Warning, "Class %ls has parent %ls, but this parent is not found. The Class will not be shown in ClassViewer.",
