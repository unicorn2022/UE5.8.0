...
private:
	...
	virtual void CustomizeDetails(const TSharedPtr<IDetailLayoutBuilder>& InDetailLayoutBuilder) override;
	void OnWorldPartitionChanged(UWorld* InWorld);
	void ForceRefreshLayout();

	TWeakPtr<IDetailLayoutBuilder> DetailLayoutBuilderWeakPtr;
};