	...
	[
		...
		[
			...
			[
				...
				.HAlign(EHorizontalAlignment::HAlign_Center)
				.VAlign(EVerticalAlignment::VAlign_Center)
				.Text(LOCTEXT("ForceBuildHLODButtonText", "Force Build HLOD"))
				.OnClicked(this, &FWorldPartitionHLODDetailsCustomization::OnForceBuildHLOD)
				.IsEnabled(this, &FWorldPartitionHLODDetailsCustomization::CanBuildHLOD)
			]
		]
		+ SVerticalBox::Slot()
		...
		[
			...
			.HAlign(EHorizontalAlignment::HAlign_Center)
			.VAlign(EVerticalAlignment::VAlign_Center)
			.Text(LOCTEXT("CompareHLODButtonText", "Compare Against Source"))
			.ToolTipText(LOCTEXT("CompareHLODButtonTooltip", "Open a wipe comparison of the HLOD against its source actors"))
			.OnClicked(this, &FWorldPartitionHLODDetailsCustomization::OnCompareHLOD)
			.IsEnabled(this, &FWorldPartitionHLODDetailsCustomization::CanCompareHLOD)
		]
	];
}
...
{
	return OnBuildHLODInternal(/*bForceBuild=*/false);
}

FReply FWorldPartitionHLODDetailsCustomization::OnForceBuildHLOD()
{
	return OnBuildHLODInternal(/*bForceBuild=*/true);
}

FReply FWorldPartitionHLODDetailsCustomization::OnBuildHLODInternal(bool bForceBuild)
{
	TArray<AWorldPartitionHLOD*> SelectedHLODActors = GetSelectedHLODActors();
	...
	for (int32 Index = 0; Index < SelectedHLODActors.Num(); ++Index)
	{
		...
		bool bUpdated = HLODActor->BuildHLOD(bForceBuild);
		NumHLODActorsUpdated += bUpdated ? 1 : 0;
	};

	// Toast
	FNotificationInfo Notification(LOCTEXT("HLODBuildCompleted", "HLOD Build Completed"));
	Notification.Image = FAppStyle::GetBrush(TEXT("Icons.Info"));
	Notification.bFireAndForget = true;
	Notification.ExpireDuration = 3.0f;
	Notification.bUseThrobber = true;

	FText NumHLODActorsUpdatedText = NumHLODActorsUpdated > 0 ? FText::AsNumber(NumHLODActorsUpdated) : LOCTEXT("NoHLODActorUpdated", "No");
	Notification.SubText = FText::Format(LOCTEXT("NumHLODActorsUpdated", "{0} HLOD {1}|plural(one=actor,other=actors) updated"), NumHLODActorsUpdatedText, NumHLODActorsUpdated);
...
bool FWorldPartitionHLODDetailsCustomization::CanExportHLODAssets(const AWorldPartitionHLOD* HLODActor)
{
	...
	if (HLODActor->GetHLODResourcesPackage() == nullptr)
	{
		return false;
	}

	bool bHasPrivateStaticMesh = false;
	ForEachObjectWithPackage(HLODActor->GetHLODResourcesPackage(), [&bHasPrivateStaticMesh](UObject* InObject)
	{
...
static EAppReturnType::Type EditExportHLODAssetsParams(FExportHLODAssetsParams& InOutParams, const TArray<AWorldPartitionHLOD*>& InHLODActorsToExport)
{
	...
	TSharedRef<FStructOnScope> StructData = MakeShared<FStructOnScope>(FExportHLODAssetsParams::StaticStruct());
	FExportHLODAssetsParams& WorkingCopy = *reinterpret_cast<FExportHLODAssetsParams*>(StructData->GetStructMemory());
	FExportHLODAssetsParams::StaticStruct()->CopyScriptStruct(&WorkingCopy, &InOutParams);
...
bool FWorldPartitionHLODDetailsCustomization::CanCompareHLOD() const
{
	TArray<AWorldPartitionHLOD*> SelectedHLODActors = GetSelectedHLODActors();
	if (SelectedHLODActors.IsEmpty())
	{
	...
	AWorldPartitionHLOD* HLODActor = SelectedHLODActors[0];
	if (!HLODActor || HLODActor->IsTemplate())
	{
		return false;
	}
	...
	if (HLODActor->GetInstanceComponents().Num() == 0)
	{
		return false;
	}
...
FReply FWorldPartitionHLODDetailsCustomization::OnCompareHLOD()
{
	TArray<AWorldPartitionHLOD*> SelectedHLODActors = GetSelectedHLODActors();
	if (SelectedHLODActors.IsEmpty())
	{
	...
	TArray<TWeakObjectPtr<AWorldPartitionHLOD>> WeakHLODActors;
	for (AWorldPartitionHLOD* Actor : SelectedHLODActors)
	{
		if (Actor)
		{
			WeakHLODActors.Add(Actor);
		}
	}

	if (WeakHLODActors.IsEmpty())
	{
		return FReply::Handled();
	}

	FText WindowTitle = (WeakHLODActors.Num() == 1)
		? FText::Format(LOCTEXT("CompareWindowTitleSingle", "Compare HLOD: {0}"), FText::FromString(WeakHLODActors[0]->GetActorLabel()))
		: FText::Format(LOCTEXT("CompareWindowTitleMulti", "Compare HLOD ({0} actors)"), FText::AsNumber(WeakHLODActors.Num()));
