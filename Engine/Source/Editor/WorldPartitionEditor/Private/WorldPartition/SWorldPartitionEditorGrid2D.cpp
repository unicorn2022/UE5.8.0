...
static bool HitTestZFromLocation(UWorld* World, const FVector2D& WorldLocation, FHitResult& OutResult)
{
	...
	return World->LineTraceSingleByChannel(OutResult, TraceStart, TraceEnd, ECC_Visibility, TraceParams);
}
...
void SWorldPartitionEditorGrid2D::FEditorCommands::RegisterCommands()
{
	...
	UI_COMMAND(BuildHLODsForSelection, "Build HLODs For Selection", "Build HLODs in selected area.", EUserInterfaceActionType::Button, FInputChord());
	UI_COMMAND(BuildHLODsForSelectedRegions, "Build HLODs For Selected Region(s)", "Build HLODs for the selected region(s).", EUserInterfaceActionType::Button, FInputChord());
...
TSharedRef<SWidget> SWorldPartitionEditorGrid2D::SToolBar::GenerateBuildMenu() const
{
	...
	Section.AddMenuEntry(LevelEditorCommands.BuildHLODs_SelectedActors);
...
void SWorldPartitionEditorGrid2D::BindCommands()
{
	...
	auto CanBuildHLODsForSelectedRegions = [this]()
	{
		for (const TWeakInterfacePtr<IWorldPartitionActorLoaderInterface>& SelectedLoaderAdapter : SelectedLoaderInterfaces)
		{
			if (IWorldPartitionActorLoaderInterface* LoaderInterface = SelectedLoaderAdapter.Get())
			{
				if (LoaderInterface->GetLoaderAdapter()->GetBoundingBox().IsSet())
				{
					return true;
				}
			}
	...
	CommandList->MapAction(Commands.BuildHLODsForSelection, FExecuteAction::CreateSP(this, &SWorldPartitionEditorGrid2D::BuildHLODsForSelection), FCanExecuteAction::CreateLambda(CanCreateRegionFromSelection));
	CommandList->MapAction(Commands.BuildHLODsForSelectedRegions, FExecuteAction::CreateSP(this, &SWorldPartitionEditorGrid2D::BuildHLODsForSelectedRegions), FCanExecuteAction::CreateLambda(CanBuildHLODsForSelectedRegions));
...
void SWorldPartitionEditorGrid2D::BuildHLODsForSelection()
{
	TArray<FBox> FilterVolumes = { FBox(FVector(SelectBoxGridSnapped.Min.X, SelectBoxGridSnapped.Min.Y, -HALF_WORLD_MAX), FVector(SelectBoxGridSnapped.Max.X, SelectBoxGridSnapped.Max.Y, HALF_WORLD_MAX)) };
...
void SWorldPartitionEditorGrid2D::BuildHLODsForSelectedRegions()
{
	...
	for (const TWeakInterfacePtr<IWorldPartitionActorLoaderInterface>& SelectedLoaderAdapter : SelectedLoaderInterfaces)
	{
		if (IWorldPartitionActorLoaderInterface* LoaderInterface = SelectedLoaderAdapter.Get())
		{
			TOptional<FBox> BoundingBox = LoaderInterface->GetLoaderAdapter()->GetBoundingBox();
			if (BoundingBox.IsSet())
			{
				FilterVolumes.Add(BoundingBox.GetValue());
			}
		}
	}
...
void SWorldPartitionEditorGrid2D::BugItHere()
{
	...
	if (bHitResultValid && GCurrentLevelEditingViewportClient)
	{
		...
		UE_LOGF(LogWorldPartition, Log, "%ls", *GoString);
...
TSharedRef<SWidget> SWorldPartitionEditorGrid2D::GenerateContextualMenu() const
{
	...
	const FEditorCommands& Commands = FEditorCommands::Get();
	UToolMenu* ContextualMenu = UToolMenus::Get()->RegisterMenu(MenuName);

	if (GetDefault<UWorldPartitionEditorSettings>()->GetEnableLoadingInEditor())
	{
		static const FName SectionSelectionName(TEXT("ContextMenu.Selection"));
		FToolMenuSection& SectionSelection = ContextualMenu->AddSection(SectionSelectionName, LOCTEXT("WorldPartitionSelectionHeader", "Selection"));
		...
		FToolMenuSection& SectionRegions = ContextualMenu->AddSection(SectionRegionsName, LOCTEXT("WorldPartitionRegionsHeader", "Region(s)"));
		SectionRegions.AddMenuEntry(Commands.LoadSelectedRegions);
		SectionRegions.AddMenuEntry(Commands.UnloadSelectedRegions);
		SectionRegions.AddSeparator(TEXT("RegionsSeparator"));
		SectionRegions.AddMenuEntry(Commands.ConvertSelectedRegionsToActors);
	}

	if (GetWorldPartition() && GetWorldPartition()->IsStreamingEnabled())
	{
		static const FName SectionHLODName(TEXT("ContextMenu.HLOD"));
		FToolMenuSection& SectionHLOD = ContextualMenu->AddSection(SectionHLODName, LOCTEXT("WorldPartitionHLODHeader", "HLOD"));
		SectionHLOD.AddMenuEntry(Commands.BuildHLODsForSelection);
		if (GetDefault<UWorldPartitionEditorSettings>()->GetEnableLoadingInEditor())
		{
			SectionHLOD.AddMenuEntry(Commands.BuildHLODsForSelectedRegions);
		}
	}

	static const FName SectionMiscName(TEXT("ContextMenu.Misc"));
	FToolMenuSection& SectionMisc = ContextualMenu->AddSection(SectionMiscName, LOCTEXT("WorldPartitionMiscHeader", "Misc"));
