...
static void CreateLevelViewportContextMenuEntries(FMenuBuilder& MenuBuilder, TArray<TWeakObjectPtr<AActor>> Volumes, FBox SelectionBox, TSharedPtr<FHLODCreationVolumeFilter> VolumeFilter, TArray<TWeakObjectPtr<AWorldPartitionHLOD>> CompareHLODActors)
{
	...
	if (GCurrentLevelEditingViewportClient && VolumeFilter->IsActive())
	{
		TWeakObjectPtr<UWorld> World = GCurrentLevelEditingViewportClient->GetWorld();

		FUIAction BuildHLODsFromSelection(
			FExecuteAction::CreateLambda([World, VolumeFilter]()
			{
				if (World.IsValid())
				{
					UWorldPartitionHLODsBuilder::BuildWithFilters(World.Get(), { VolumeFilter });
				}
			}),
		...
		MenuBuilder.AddMenuEntry(LOCTEXT("BuildHLODsFromSelection", "Build HLODs From Selection"), LOCTEXT("BuildHLODSFromSelection_Tooltip", "Build HLODs from selected actor(s) bounds"), FSlateIcon(), BuildHLODsFromSelection);
	}

	// Compare HLOD against source actors (only when all selected actors are HLOD actors)
	if (!CompareHLODActors.IsEmpty())
	{
		FUIAction CompareHLODAction(
			FExecuteAction::CreateLambda([CompareHLODActors]()
			{
				FText WindowTitle = (CompareHLODActors.Num() == 1 && CompareHLODActors[0].IsValid())
					? FText::Format(LOCTEXT("CompareWindowTitleSingle", "Compare HLOD: {0}"), FText::FromString(CompareHLODActors[0]->GetActorLabel()))
					: FText::Format(LOCTEXT("CompareWindowTitleMulti", "Compare HLOD ({0} HLOD actors)"), FText::AsNumber(CompareHLODActors.Num()));
				...
				Window->SetContent(
					SNew(SHLODCompareWindow)
					.HLODActors(CompareHLODActors)
				);
		...
		MenuBuilder.AddMenuEntry(LOCTEXT("CompareHLODAgainstSource", "Compare HLOD against source"), LOCTEXT("CompareHLODAgainstSource_Tooltip", "Open a wipe comparison of the selected HLODs against their source actors"), FSlateIcon(), CompareHLODAction);
	}
...
static TSharedRef<FExtender> OnExtendLevelEditorMenu(const TSharedRef<FUICommandList> CommandList, TArray<AActor*> SelectedActors)
{
	...
	TSharedPtr<FHLODCreationVolumeFilter> VolumeFilter = MakeShared<FHLODCreationVolumeFilter>(MoveTemp(FilterVolumes));

	// Collect HLOD actors for the compare entry (only if ALL selected actors are HLOD actors)
	TArray<TWeakObjectPtr<AWorldPartitionHLOD>> CompareHLODActors;
	bool bAllAreHLOD = !SelectedActors.IsEmpty();
	for (AActor* Actor : SelectedActors)
	{
		if (AWorldPartitionHLOD* HLODActor = Cast<AWorldPartitionHLOD>(Actor))
		{
			CompareHLODActors.Add(HLODActor);
		}
		else
		{
			bAllAreHLOD = false;
	...
	if (!bAllAreHLOD)
	{
		CompareHLODActors.Empty();
	}

	if (!Volumes.IsEmpty() || BoundsBuilder.IsValid() || VolumeFilter->IsActive() || !CompareHLODActors.IsEmpty())
	{
		Extender->AddMenuExtension(
			"ActorTypeTools",
			EExtensionHook::After,
			nullptr,
			FMenuExtensionDelegate::CreateStatic(&CreateLevelViewportContextMenuEntries, Volumes, FBoxSphereBounds(BoundsBuilder).GetBox(), VolumeFilter, CompareHLODActors));
	}
...
void FWorldPartitionEditorModule::StartupModule()
{
	...
	SceneOutlinerModule.OnCreateActorBrowserColumns().AddRaw(this, &FWorldPartitionEditorModule::OnCreateActorBrowserColumns);
...
void FWorldPartitionEditorModule::ShutdownModule()
{
	...
	{
		SceneOutlinerModule->OnCreateActorBrowserColumns().RemoveAll(this);
	}
...
void FWorldPartitionEditorModule::OnCreateActorBrowserColumns(FSceneOutlinerInitializationOptions& InitOptions, UWorld* World)
{
...
{
	return UWorldPartitionHLODEditorSubsystem::WriteHLODStats(Params);
}
...
static void LoadMap(const FString& MapToLoad)
{
	...
	{
		UE_LOGF(LogWorldPartitionEditor, Error, "Failed to reopen world.");
	}
}
...
void FWorldPartitionEditorModule::RunCommandletAsExternalProcess(const FString& InCommandletArgs, const FText& InOperationDescription, int32& OutResult, bool& bOutCancelled)
{
	...
	UE_LOGF(LogWorldPartitionEditor, Display, "Running commandlet: %ls %ls", *CurrentExecutableName, *Arguments);
	...
	if (OutResult == 0)
	{
		UE_LOGF(LogWorldPartitionEditor, Display, "Commandlet executed successfully.");
		UE_LOGF(LogWorldPartitionEditor, Display, "Detailed output can be found in %ls", *AbsLogFilePath);
	}
	else
	{
		UE_LOGF(LogWorldPartitionEditor, Error, "#### Commandlet Failed ####");
		UE_LOGF(LogWorldPartitionEditor, Error, "%ls %ls", *CurrentExecutableName, *Arguments);
		UE_LOGF(LogWorldPartitionEditor, Error, "Return Code: %i", OutResult);

		UE_LOGF(LogWorldPartitionEditor, Error, "#### BEGIN COMMANDLET OUTPUT (from %ls) ####", *AbsLogFilePath);
		...
		for (const FString& OutputLine : OutputLines)
		{
			...
			if (Regex.FindNext())
			{
				...
				switch (Verbosity)
				{
				case ELogVerbosity::Display: UE_LOGF(LogWorldPartitionEditor, Display, "#### COMMANDLET OUTPUT >> %ls", *OutputLine); break;
				case ELogVerbosity::Warning: UE_LOGF(LogWorldPartitionEditor, Warning, "#### COMMANDLET OUTPUT >> %ls", *OutputLine); break;
				case ELogVerbosity::Error:	 UE_LOGF(LogWorldPartitionEditor, Error, "  #### COMMANDLET OUTPUT >> %ls", *OutputLine); break;
				case ELogVerbosity::Fatal:	 UE_LOGF(LogWorldPartitionEditor, Error, "  #### COMMANDLET OUTPUT >> %ls", *OutputLine); break; // Do not output as FATAL as it would crash the editor
				default: break;	// Ignore the non displayable log lines, they can be found in the log file
				}
			}
		}		

		UE_LOGF(LogWorldPartitionEditor, Error, "#### END COMMANDLET OUTPUT ####");
	}
}
