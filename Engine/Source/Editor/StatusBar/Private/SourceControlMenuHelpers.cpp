...
void FSourceControlCommands::RegisterCommands()
{
	...
	UI_COMMAND(ViewSnapshotHistory, "Open Branch History", "See all the changes that have been made to this branch over time.", EUserInterfaceActionType::Button, FInputChord());
	UI_COMMAND(ViewBranchExplorer, "Open Branch Explorer (Beta)", "See all the branches which this project uses.", EUserInterfaceActionType::Button, FInputChord(EModifierKey::Control | EModifierKey::Alt, EKeys::B));
	...
	ActionList->MapAction(
		ViewBranchExplorer,
		FExecuteAction::CreateStatic(&FSourceControlCommands::ViewBranchExplorer_Clicked),
		FCanExecuteAction::CreateStatic(&FSourceControlCommands::ViewBranchExplorer_CanExecute),
		FIsActionChecked::CreateLambda([]() { return false; }),
		FIsActionButtonVisible::CreateStatic(&FSourceControlCommands::ViewBranchExplorer_IsVisible)
	);

	// Add to global command list to allow key shortcut anywhere in Editor.
...
bool FSourceControlCommands::ViewBranchExplorer_CanExecute()
{
	return ISourceControlWindowsModule::Get().CanShowBranchExplorerTab();
}

bool FSourceControlCommands::ViewBranchExplorer_IsVisible()
{
	if (IConsoleManager::Get().FindConsoleObject(TEXT("UnrealRevisionControl.FocusBranchExplorer")) == nullptr)
	{
		return false;
	}
...
void FSourceControlCommands::ViewBranchExplorer_Clicked()
{
	ISourceControlWindowsModule::Get().ShowBranchExplorerTab();
}
...
bool FSourceControlCommands::CheckOutModifiedFiles_CanExecute()
{
	...
	{
		return FEditorFileUtils::HasDirtyPackages();
	}
...
void FSourceControlCommands::CheckOutModifiedFiles_Clicked()
{
	...
	FEditorFileUtils::GetDirtyPackages(PackagesToSave);
...
{
	FText Message = LOCTEXT("RevertAllModifiedFiles", "Are you sure you want to revert all local changes? By proceeding, your local changes will be discarded, and the state of your last synced revision will be restored.");
...
TSharedRef<SWidget> FSourceControlMenuHelpers::GenerateSourceControlMenuContent()
{
	...
	Section.AddMenuEntry(
		FSourceControlCommands::Get().ViewBranchExplorer,
		TAttribute<FText>(),
		TAttribute<FText>(),
		FSlateIcon(FRevisionControlStyleManager::GetStyleSetName(), "RevisionControl.BranchExplorerTab")
	);
