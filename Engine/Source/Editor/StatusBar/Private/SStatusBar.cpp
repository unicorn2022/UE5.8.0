...
FStatusBarMessage::FStatusBarMessage(const TAttribute<FText>& InMessageText, const TAttribute<FText>& InHintText, FStatusBarMessageHandle InHandle)
	: MessageText(InMessageText)
	, HintText(InHintText)
	, IconBrush(FAppStyle::Get().GetBrush("StatusBar.HelpIcon"))
	, IconColor(FSlateColor::UseForeground())
	, Handle(InHandle)
{}

FStatusBarMessage::FStatusBarMessage(const TAttribute<FText>& InMessageText, const TAttribute<FText>& InHintText, FStatusBarMessageHandle InHandle,
	const TAttribute<const FSlateBrush*>& InIconBrush, const TAttribute<FSlateColor>& InIconColor)
	: MessageText(InMessageText)
	, HintText(InHintText)
...
void SStatusBar::PushMessage(FStatusBarMessageHandle InHandle, const TAttribute<FText>& InMessage, const TAttribute<FText>& InHintText,
	const TAttribute<const FSlateBrush*>& InIconBrush, const TAttribute<FSlateColor>& InIconColor)
{
	MessageStack.Emplace(InMessage, InHintText, InHandle, InIconBrush, InIconColor);
}
...
EVisibility SStatusBar::GetIconVisibility() const
{
...
TSharedRef<SWidget> SStatusBar::MakeStatusMessageWidget()
{
	return 
		...
		[
			SNew(SImage)
			.ColorAndOpacity(this, &SStatusBar::GetIconColorAndOpacity)
			.Image(this, &SStatusBar::GetIconBrush)
			.Visibility(this, &SStatusBar::GetIconVisibility)
		]
		+ SHorizontalBox::Slot()
		.VAlign(VAlign_Center)
		.Padding(5.0f, 0.0f)
...
FSlateColor SStatusBar::GetIconColorAndOpacity() const
{
	return MessageStack.Num() > 0 ? MessageStack.Top().IconColor.Get() : FSlateColor::UseForeground();
}

const FSlateBrush* SStatusBar::GetIconBrush() const
{
	return MessageStack.Num() > 0 ? MessageStack.Top().IconBrush.Get() : FAppStyle::Get().GetBrush("StatusBar.HelpIcon");
}
