...
public:
	...
	*/
	...
	/**
	 * Pushes a new status bar message with a custom icon brush and color.
	 * Useful for contextual indicators (e.g. a red record icon during recording).
	 *
	 * @param StatusBarName The name of the status bar to push messages to
	 * @param InMessage The message to display
	 * @param InHintText Optional hint text. Highlighted to stand out
	 * @param InIconBrush Custom icon brush. Pass nullptr to use the default help icon
	 * @param InIconColor Color/opacity for the icon, supports dynamic attributes for pulsing effects
	 * @return A handle to the message for clearing it later
	 */
	UE_API FStatusBarMessageHandle PushStatusBarMessage(FName StatusBarName, const TAttribute<FText>& InMessage, const TAttribute<FText>& InHintText,
