...
public:
	...
	DECLARE_TS_MULTICAST_DELEGATE_OneParam(FOnPreCompileBlueprint, UBlueprint*);
...
private:
	/**
	* Set to true when there is an input event the graph and we want to automatically set up input component binding.
	*/
	bool bAutomaticallyRegisterInputEvents;

public:

	UE_API void SetAutomaticallyRegisterInputEvents(const bool bVal);
	UE_API bool GetAutomaticallyRegisterInputEvents() const;
	...
	static UE_API FOnPreCompileBlueprint OnPreCompileBlueprint;
...
protected:
	...
	virtual void PreCompile() { OnPreCompile.Broadcast(); OnPreCompileBlueprint.Broadcast(Blueprint); }
