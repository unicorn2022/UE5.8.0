...
{
	KISMETCOMPILER_API bool (*GCanSaveFunctionsForPackage)(const UPackage*) = nullptr;
}
...
FKismetCompilerContext::FOnPreCompileBlueprint FKismetCompilerContext::OnPreCompileBlueprint;
...
namespace UE::KismetCompiler::Private
{
	...
	static void RestoreFloatPinsToNode(UK2Node_EditablePinBase* Node, const UFunction* FunctionSignature, bool bProcessOutputParamsOnly = false)
	{
		...
		if (Node->UserDefinedPins.Num() > 0)
		{
			...
			{
				UE_LOGF(LogK2Compiler, Warning, "User pin '%ls' ('%ls') was not found in the pins list!",
		...
		auto IsFloatProperty = [Node](const FProperty* Property)
		{
			...
			else if (const FMapProperty* MapProperty = CastField<FMapProperty>(Property))
			{
				...
				{
					UE_LOGF(LogK2Compiler, Warning, "A map with float entries was found in '%ls'. Delegate signature may be inaccurate.",
						*Node->GetFullName());
				}
			...
			if (IsValidParameter(CurrentProperty))
			{
				if (UserDefinedPinCursor < Node->UserDefinedPins.Num())
				{
					if (IsFloatProperty(CurrentProperty))
					{
						...
						{
							UE_LOGF(LogK2Compiler, Warning, "Expected a 'real' type for user pin '%ls' ('%ls'), but found type '%ls' instead!",
						...
						if (PinCursor < Node->Pins.Num())
						{
							...
							{
								UE_LOGF(LogK2Compiler, Warning, "Expected a 'real' type for pin '%ls' ('%ls'), but found type '%ls' instead!",
						...
						{
							UE_LOGF(LogK2Compiler, Warning, "PinCursor ('%ls') has an invalid index: %d!",
								*Node->GetFullName(),
								PinCursor);
						}
					}
		...
		for (UK2Node_FunctionTerminator* FunctionTerminatorNode : TerminatorNodes)
		{
			...
			if (!Function)
			{
				...
				if (DelegateNodePtr)
				{
					...
					{
						UE_LOGF(LogK2Compiler, Warning, "Unable to determine delegate signature for node '%ls'!",
...
FKismetCompilerContext::FKismetCompilerContext(UBlueprint* SourceSketch, FCompilerResultsLog& InMessageLog, const FKismetCompilerOptions& InCompilerOptions)
	...
	, bAutomaticallyRegisterInputEvents(false)
{
	MacroRowMaxHeight = 0;
...
void FKismetCompilerContext::CleanAndSanitizeClass(UBlueprintGeneratedClass* ClassToClean, UObject*& InOldCDO)
{
	...
	GetObjectsWithOuter(ClassToClean, ClassSubObjects, EGetObjectsFlags::None);
...
void FKismetCompilerContext::ValidateVariableNames()
{
	...
	if (ParentClass != nullptr)
	{
		...
		for (FBPVariableDescription& VarDesc : Blueprint->NewVariables)
		{
			...
			if (VarDesc.VarType.PinCategory == UEdGraphSchema_K2::PC_Struct)
			{
				if(!IsValid(VarDesc.VarType.PinSubCategoryObject.Get()))
				{
					VarDesc.VarType.PinSubCategoryObject = GetFallbackStruct();
				}
			}
			...
			else if (ParentClass->IsNative()) // the above case handles when the parent is a blueprint
			{
				...
				if (ExistingField)
				{
					UE_LOGF(LogK2Compiler, Warning, "ValidateVariableNames name %ls (used in %ls) is already taken by %ls"
...
void FKismetCompilerContext::PrecompileFunction(FKismetFunctionContext& Context, EInternalCompilerFlags InternalFlags)
{
	...
	if (EntryPoints.Num())
	{
		...
		if( UE::Private::GCanSaveFunctionsForPackage && 
			!(*UE::Private::GCanSaveFunctionsForPackage)(Blueprint->GetPackage()))
		{
...
void FKismetCompilerContext::CompileFunction(FKismetFunctionContext& Context)
{
	...
	if (Context.ImplicitCastMap.Num() > 0)
	{
		...
		for (const auto& It : Context.ImplicitCastMap)
		{
			...
			{
				UE_LOGF(LogK2Compiler, Verbose, "Skipping implicit cast check for pin '%ls' in function '%ls'. Owning node '%ls' is not in the execution list.",
		...
		{
			UE_LOGF(LogK2Compiler, Warning, "Unhandled implicit casts found during compilation of function '%ls'!",
				*Context.Function->GetFullName());

			for (UEdGraphPin* Pin : UnhandledPins)
			{
				UE_LOGF(LogK2Compiler, Warning, "\tPin '%ls' was not handled by node '%ls'!",
					*Pin->PinName.ToString(),
...
void FKismetCompilerContext::ProcessOneFunctionGraph(UEdGraph* SourceGraph, bool bInternalFunction)
{
	...
	// Preserve the original graph name, which helps with debugging intermediate products
	{
		...
		ERenameFlags RenameFlags =
			(REN_NonTransactional | REN_DoNotDirty | REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
...
void FKismetCompilerContext::CompileClassLayout(EInternalCompilerFlags InternalFlags)
{
	...
	{
		UE_LOGF(LogK2Compiler, Warning, "%d convertible delegates were not processed during class layout compilation. Listing delegates in log below.", ConvertibleDelegates.Num());
		for (auto DelegateIt = ConvertibleDelegates.CreateConstIterator(); DelegateIt; ++DelegateIt)
		{
			UE_LOGF(LogK2Compiler, Display, "  Node:%ls Function:%ls Variable:%ls", *GetPathNameSafe(DelegateIt.Key()), *DelegateIt.Value().ProxyFunctionName.ToString(), *DelegateIt.Value().CapturedVariableName.ToString());
		}
	}
		...
		if (!Blueprint->bIsRegeneratingOnLoad && bIsFullCompile && DisplayLayout && NewClass && !IsRunningCommandlet())
		{
			...
			for (UFunction* LocFunction : TFieldRange<UFunction>(NewClass, EFieldIteratorFlags::ExcludeSuper))
			{
				...
				for (FProperty* Prop : TFieldRange<FProperty>(LocFunction))
				{
					...
					UE_LOGF(LogK2Compiler, Log, "%5d:\t%-64ls\t%ls %ls%ls"
...
void FKismetCompilerContext::SetAutomaticallyRegisterInputEvents(const bool bVal)
{
	bAutomaticallyRegisterInputEvents = bVal;
}

bool FKismetCompilerContext::GetAutomaticallyRegisterInputEvents() const
{
	return bAutomaticallyRegisterInputEvents;
}
...
void FKismetCompilerContext::SetCanEverTick() const
{
	...
	if (TickFunction->bCanEverTick != bOldFlag)
	{
		...
		UE_LOGF(LogK2Compiler, Verbose, "Overridden flag for class '%ls': CanEverTick %ls ", *NewClass->GetName(),
