...
public:
	...
	FArchive& operator<<(FObjectPtr& Res) override
	{
		...
		ScriptPointerType D = (ScriptPointerType)Res.GetHandle().Payload;
#else
...
public:
		...
		if (Term->bIsLiteral)
		{
			...
			else if (FLiteralTypeHelper::IsText(&Term->Type, CoerceProperty))
			{
				...
				else
				{
					...
					if (!TextId.IsEmpty())
					{
						...
						const FString& DevNotes = Term->TextLiteral.GetDevNotes();
						const bool bEmitDevNotes = !Writer.IsFilterEditorOnly() && !DevNotes.IsEmpty();
#else
						constexpr bool bEmitDevNotes = false;
#endif

						Writer << (bEmitDevNotes ? EBlueprintTextLiteralType::LocalizedTextWithNotes : EBlueprintTextLiteralType::LocalizedText);
			...
			else if (FLiteralTypeHelper::IsStruct(&Term->Type, CoerceProperty))
			{
				...
				else
				{
					...
					{
						UE_LOGF(LogK2Compiler, Error, "Unsupported static array. Property: %ls, Struct: %ls", *GetPathNameSafe(StructProperty), *Struct->GetName());
					}
	...
	void GenerateCodeForStatement(FKismetCompilerContext& CompilerContext, FKismetFunctionContext& FunctionContext, FBlueprintCompiledStatement& Statement, UEdGraphNode* SourceNode)
	{
		...
		case KCST_FloatToDoubleCast:
			...
			break;
		default:
			UE_LOGF(LogK2Compiler, Warning, "VM backend encountered unsupported statement type %d", (int32)Statement.Type);
		}
	}
