...
FConversion GetFloatingPointConversion(const UEdGraphPin& SourcePin, const UEdGraphPin& DestinationPin)
{
	...
	else if ((SourcePin.PinType.PinCategory == UEdGraphSchema_K2::PC_Struct) && (DestinationPin.PinType.PinCategory == UEdGraphSchema_K2::PC_Struct))
	{
		...
		{
			UE_CLOGF(!SourceStruct, LogK2Compiler, Warning, "Source pin '%ls' had null struct object (%ls)", *SourcePin.GetName(), *SourcePin.GetOwningNode()->GetFullName());
			UE_CLOGF(!DestinationStruct, LogK2Compiler, Warning, "Destination pin '%ls' had null struct object (%ls)", *DestinationPin.GetName(), *DestinationPin.GetOwningNode()->GetFullName());
		}
	}
