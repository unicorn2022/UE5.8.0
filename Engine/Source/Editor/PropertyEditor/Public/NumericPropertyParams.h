...
public:
	...
	TNumericPropertyParams(const FProperty* Property, FMetaDataGetter MetaDataGetter)	
	{
		...
		{
			UE_LOGF(LogTemp, Warning, "Clamp Min (%ls) >= Clamp Max (%ls) for Ranged Numeric property %ls", *ClampMinString, *ClampMaxString, Property ? *Property->GetPathName() : TEXT(""));
		}
		...
		{
			UE_LOGF(LogTemp, Warning, "UI Min (%ls) >= UI Max (%ls) for Ranged Numeric property %ls", *ActualUIMinString, *ActualUIMaxString, Property ? *Property->GetPathName() : TEXT(""));
		}
