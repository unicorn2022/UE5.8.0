...
struct FDetailsViewArgs
{
	...
	/** Enum to set expansion persistence. Used to support saving expansion per details view. */
	enum class EExpansionPersistenceMethod : uint8
	{
		/** Expansion will use default persistence (Global shared per struct type) */
		Default = 0,
		/** 
		 * Expansion will be stored transiently per details view identifier and persist in a single editor session.
		 * The Expansion will be written to EditorPerProjectUserSettings.ini, but will be cleared without read on editor startup.
		 */
		TransientPerViewIdentifier = 1,
		/** Expansion will be stored per details view identifier and persist on disk. */
		PersistentPerViewIdentifier = 2,
	};
	...
	/** Persistence method for expansion in this details view */
	EExpansionPersistenceMethod ExpansionPersistenceMethod;
	...
	DECLARE_DELEGATE_RetVal_OneParam(bool, FShouldForceHideProperty, const TSharedRef<IPropertyHandle>&);
	...
	/** Initial categories to be shown enabled in the filter */
	TSet<FName> InitialCategories;
...
public:
	...
	FDetailsViewArgs()
		...
		, ExpansionPersistenceMethod(EExpansionPersistenceMethod::Default)
		, NotifyHook(nullptr)
