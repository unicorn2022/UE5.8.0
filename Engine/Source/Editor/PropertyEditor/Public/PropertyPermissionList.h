...
DECLARE_DELEGATE_RetVal_OneParam(bool, FIsEnabledForPackage, const UPackage*);
 ...
 * class B : public class A - (AllowListAllProperties)					PermissionList = "MyProp","PropA1","PropA2"
...
public:
	...
	bool IsEnabled() const { return EnablePermissionList.IsBound() && EnablePermissionList.Execute(); }
	/** 
	 ...
	 * 
	 * Under the hood, it will be a delegate that will always return bEnabled. If a delegate is passed in instead, that
	 * delegate is called to return dynamically whether the filter is enabled or not.
	 */
	UE_API void SetEnabled(bool bEnable, bool bAnnounce = true);
	UE_API void SetEnabled(const FDelegateEnablePathFilter& bEnable, bool bAnnounce = true);
	...
	/** Gets a read-only reference to the cached permission list for the given struct, or null if the filtering is disabled. A non-null value will remain valid to read, but the info in it may become stale if this struct is re-cached because its raw rules change. */
	UE_API TSharedPtr<const FNamePermissionList> GetCachedPermissionListForStructIfEnabled(const UStruct* Struct) const;
	...
	const FDelegateEnablePathFilter& DelegateEnablePathFilter() { return EnablePermissionList; }
...
private:
	...
	void OnObjectPostCDOCompiled(UObject* CDO, const FObjectPostCDOCompiledContext& Context);

	/** Reset the cache and queue a broadcast. You must hold a write lock on PermissionListRW before calling this function! */
	void ClearCacheAndQueueBroadcast_NoLock(TSoftObjectPtr<const UStruct> ObjectStruct = nullptr, FName OwnerName = NAME_None);

	/** Clear the given class (and any derived classes) from the cache. You must hold a write lock on PermissionListRW before calling this function! */
	void ClearCacheForClass_NoLock(const UClass* Class);

	/** Delegate to return whether DoesPropertyPassFilter should perform its PermissionList check or always return true */
	FDelegateEnablePathFilter EnablePermissionList;
	...
	struct FCachedPermissionList
	{
		FCachedPermissionList(FNamePermissionList&& InPermissionList, EPropertyPermissionListRules InRules)
			: PermissionList(MakeShared<FNamePermissionList>(MoveTemp(InPermissionList)))
			, Rules(InRules)
		{
		}

		TSharedRef<const FNamePermissionList> PermissionList;
		EPropertyPermissionListRules Rules = EPropertyPermissionListRules::UseExistingPermissionList;
	};
	/** Lazily-constructed combined cache of both the flattened class PermissionList and struct PermissionList */
	mutable TMap<TWeakObjectPtr<const UStruct>, TSharedRef<const FCachedPermissionList>> CachedPropertyPermissionList;

	/** Protects RawPropertyPermissionList, CachedPropertyPermissionList, and PendingUpdates */
	mutable FRWLock PermissionListRW;

	/** Get or create the cached PermissionList for a specific UStruct */
	TSharedRef<const FCachedPermissionList> GetCachedPermissionListEntryForStruct(const UStruct* Struct) const;
};
...
public:
	...
	/** Set the delegate that is called in IsEnabledForPackage */
	void SetIsEnabledForPackageDelegate(const FIsEnabledForPackage& InIsEnabledForPackage ) { IsEnabledForPackageDelegate = InIsEnabledForPackage; }
	/** A special call to disable the filter for a specific blueprint, if the IsEnabledForPackageDelegate is set and returns false */
	UE_API bool IsEnabledForPackage(const UPackage* Package) const;

private:
	FPropertyEditorPermissionList() = default;
	~FPropertyEditorPermissionList() = default;

	FIsEnabledForPackage IsEnabledForPackageDelegate;
