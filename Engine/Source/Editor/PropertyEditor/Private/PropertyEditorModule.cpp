...
TAutoConsoleVariable<bool> FPropertyEditorModule::CVarAllowPropertyMatrixForNonActors = TAutoConsoleVariable<bool>(
	TEXT("PropertyEditor.AllowPropertyMatrixForNonActors"),
	false,
	TEXT("Specifies whether the property matrix can be launched when selected items are not actors or actor components.")
);
...
void FPropertyEditorModule::StartupModule()
{
	...
	FCoreDelegates::GetOnPostEngineInit().Add(FSimpleMulticastDelegate::FDelegate::CreateStatic(SDetailsViewBase::ResetTransientPerViewDetailExpansion));
}
...
void FPropertyEditorModule::ShutdownModule()
{
	...
	FPropertyEditorCommands::Unregister();
...
void FPropertyEditorModule::RequestRefreshOfAllDetailsPanels()
{
	if (FSlateApplication::IsInitialized())
	{
		for (int32 ViewIndex = 0; ViewIndex < AllDetailViews.Num(); ++ViewIndex)
		{
			...
			{
				TSharedPtr<SDetailsView> DetailViewPin = AllDetailViews[ViewIndex].Pin();
				DetailViewPin->RequestForceRefresh();
			}
		}
	}
}
...
TSharedPtr<FAssetThumbnailPool> FPropertyEditorModule::GetThumbnailPool()
{
	if (!GlobalThumbnailPool.IsValid())
	{
		...
		GlobalThumbnailPool = MakeShareable(new FAssetThumbnailPool(50));
	}
...
FStructProperty* FPropertyEditorModule::RegisterStructProperty(const UStruct* StructClass)
{
	...
	if(!StructProperty)
	{
		...
		StructProperty = new FStructProperty(StructOnScopePropertyOwner, *MakeUniqueObjectName(StructOnScopePropertyOwner, UField::StaticClass(), InnerStruct->GetFName()).ToString());
