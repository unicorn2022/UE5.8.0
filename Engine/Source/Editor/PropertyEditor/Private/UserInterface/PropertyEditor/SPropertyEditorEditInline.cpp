...
TSharedRef<SWidget> SPropertyEditorEditInline::GenerateClassPicker(TSharedRef<IPropertyHandle> PropertyHandle, FOnClassPicked OnPicked, TSharedPtr<IClassViewerFilter> AdditionalClassFilter) // static
{
	...
	if (ClassFilter->InterfaceThatMustBeImplemented != nullptr)
	{
		...
		{
			UE_LOGF(LogPropertyNode, Warning, "Property (%ls) specifies a MustImplement class which isn't an interface (%ls), clearing filter.", *Property->GetFullName(), *ClassFilter->InterfaceThatMustBeImplemented->GetPathName());
