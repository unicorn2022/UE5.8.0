...
void SPropertyEditorAsset::Construct(const FArguments& InArgs, const TSharedPtr<FPropertyEditor>& InPropertyEditor)
{
	...
	bDisplayTransient = InArgs._DisplayTransient;
	...
	AssetComboButton = SNew(SComboButton)
		...
		[
			...
			[
                ...
                .OverflowPolicy(
                    GetDefault<UEditorStyleSettings>()->bEnableMiddleEllipsis
                        ? TOptional(ETextOverflowPolicy::MiddleEllipsis)
                        : NullOpt
                )
			]
		];
	...
	if (ShouldDisplayThumbnail(InArgs, ObjectClass))
	{
		...
		{
			//Only override the color if we have a meaningful type specified.
			//=> When override is done, it will internally prevent to switch the color strip to the current asset type color, when updated the asset.
			if (ObjectClass != nullptr && ObjectClass != UObject::StaticClass())
			{
				FAssetToolsModule& AssetToolsModule = FModuleManager::LoadModuleChecked<FAssetToolsModule>(TEXT("AssetTools"));
				TSharedPtr<IAssetTypeActions> AssetTypeActions = AssetToolsModule.Get().GetAssetTypeActionsForClass(ObjectClass).Pin();
	...
	if( InArgs._DisplayBrowse )
	{
		...
		// Only the default SPropertyEditorAsset::OnBrowse delegate supports alt+click to directly open the asset by default, so we only modify the icon if there is no browse override
		TAttribute<const FSlateBrush*> IconDelegate = InArgs._OnBrowseIconOverride.IsSet() ? InArgs._OnBrowseIconOverride
			: !InArgs._OnBrowseOverride.IsBound() ? TAttribute<const FSlateBrush*>::CreateSP(this, &SPropertyEditorAsset::GetOnBrowseIcon)
			: TAttribute<const FSlateBrush*>();

		TAttribute<FText> OnBrowseToolTip = InArgs._OnBrowseToolTipOverride.IsSet() ? InArgs._OnBrowseToolTipOverride : TAttribute<FText>(this, &SPropertyEditorAsset::GetOnBrowseToolTip);
	...
	if (InArgs._CustomButtonSlot.Widget != SNullWidget::NullWidget)
	{
		ButtonBox->AddSlot()
			.VAlign(VAlign_Center)
			.Padding(FMargin(0.0f, 2.0f))
			[
				InArgs._CustomButtonSlot.Widget
			];
	}
...
{
	if( AssetThumbnail.IsValid() && !UE::IsSavingPackage() && !IsGarbageCollecting() )
	{
...
TSharedRef<SWidget> SPropertyEditorAsset::OnGetMenuContent()
{
	...
	if (bIsActor)
	{
		...
		if (bDisplayTransient)
		{
			...
			if (Property)
			{
				// Transient objects need to be keyable to ensure they can be handle properly through their lifetime.
				bShowTransient = Property->HasAnyPropertyFlags(EPropertyFlags::CPF_Interp);
			}
		}
...
FText SPropertyEditorAsset::OnGetAssetName() const
{
	...
	if( Result == FPropertyAccess::Success )
	{
		if(Value.Object != nullptr)
		{
			...
			else if (UDataAsset* DataAsset = Cast<UDataAsset>(Value.Object))
			{
				Name = DataAsset->GetDisplayNameText();
			}
			else
			{
				Name = FText::AsCultureInvariant(Value.Object->GetName());
...
FPropertyAccess::Result SPropertyEditorAsset::GetValue( FObjectOrAssetData& OutValue, FObjectOrAssetData::EAssetDataOptions AssetDataOptions ) const
{
	...
	if( PropertyEditor.IsValid() && PropertyEditor->GetPropertyHandle()->IsValidHandle() )
	{
		...
		if (Object && !Object->IsValidLowLevel())
		{
			...
			UE_LOGF(LogPropertyNode, Fatal, "Property \"%ls\" (%ls) contains invalid data.", *Property->GetName(), *Property->GetCPPType());
		}
#endif
...
FText SPropertyEditorAsset::GetOnBrowseToolTip() const
{
	...
	if (bIsActor)
	{
		return FText::Format(LOCTEXT("SelectActorInViewport", "Select Actor in the viewport"), Args);
	}
	else
	{
...
const FSlateBrush* SPropertyEditorAsset::GetOnBrowseIcon() const
{
	...
	if (!bIsActor && IsHovered() && FSlateApplication::Get().GetModifierKeys().IsAltDown())
	{
...
void SPropertyEditorAsset::OnCopy()
{
	...
	{
		// Copy "None" instead of ""
		FPlatformApplicationMisc::ClipboardCopy(*FName().ToString());
	}
}
...
UClass* SPropertyEditorAsset::GetObjectPropertyClass(const FProperty* Property)
{
	...
	if (CastField<const FObjectPropertyBase>(Property) != nullptr)
	{
		...
		if (Class == nullptr)
		{
			UE_LOGF(LogPropertyNode, Warning, "Object Property (%ls) has a null class, falling back to UObject", *Property->GetFullName());
	...
	else if (CastField<const FInterfaceProperty>(Property) != nullptr)
	{
		...
		if (Class == nullptr)
		{
			UE_LOGF(LogPropertyNode, Warning, "Interface Property (%ls) has a null class, falling back to UObject", *Property->GetFullName());
			Class = UObject::StaticClass();
		}
	}
	else
