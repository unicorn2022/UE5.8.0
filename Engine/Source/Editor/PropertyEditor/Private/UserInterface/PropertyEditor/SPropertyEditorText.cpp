		...
		if (!PropertyValidatorFunctionName.IsEmpty() && OuterBaseClass)
		{
			...
			if (PropertyValidatorFunction)
			{
				...
				else
				{
					...
					UE_CLOGF(!LoggedWarnings.Contains(WarningId), LogPropertyNode, Warning, "PropertyValidator ufunction '%ls' on %ls%ls::%ls must be a static function.",
			...
			else
			{
				...
				UE_CLOGF(!LoggedWarnings.Contains(WarningId), LogPropertyNode, Warning, "PropertyValidator ufunction '%ls' on %ls%ls::%ls not found.",
					*PropertyValidatorFunctionName, OuterBaseClass->GetPrefixCPP(), *OuterBaseClass->GetName(), *PropertyHandle->GetProperty()->GetName());
				LoggedWarnings.Add(WarningId);
			}
		}
