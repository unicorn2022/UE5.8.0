...
void SPropertyMenuAssetPicker::Construct( const FArguments& InArgs )
{
	...
	EAssetViewType::Type AssetViewType = EAssetViewType::List;
	static const FName AssetViewTypeMetadata = TEXT("AssetViewInitialViewType");
	if (PropertyHandle && PropertyHandle->HasMetaData(AssetViewTypeMetadata))
	{
		const int32 Value = PropertyHandle->GetIntMetaData(AssetViewTypeMetadata);
		if (Value >= 0 && Value <= (EAssetViewType::Type::MAX - 1))
		{
			AssetViewType = static_cast<EAssetViewType::Type>(Value);
		}
	}

	// Get initial thumbnail size
	EThumbnailSize AssetThumbnailSize = EThumbnailSize::Medium;
	static const FName AssetThumbnailSizeMetadata = TEXT("AssetViewInitialThumbnailSize");
	if (PropertyHandle && PropertyHandle->HasMetaData(AssetThumbnailSizeMetadata))
	{
		const int32 Value = PropertyHandle->GetIntMetaData(AssetThumbnailSizeMetadata);
		if (Value >= 0 && Value <= (static_cast<int32>(EThumbnailSize::MAX) - 1))
		{
			AssetThumbnailSize = static_cast<EThumbnailSize>(Value);
		}
	}

	// Get initial thumbnail scale
	TOptional<float> AssetThumbnailScale;
	static const FName AssetThumbnailScaleMetadata = TEXT("AssetViewInitialThumbnailScale");
	if (PropertyHandle && PropertyHandle->HasMetaData(AssetThumbnailScaleMetadata))
	{
		const int32 Value = PropertyHandle->GetFloatMetaData(AssetThumbnailScaleMetadata);
		if (Value >= 0 && Value <= 1)
		{
			AssetThumbnailScale = Value;
		}
	}

	// Get settings to load/save, use empty string to not use any settings
	static const FName AssetViewSettingsNameMetadata = TEXT("AssetViewSettingsName");
	const FString SaveSettingsName = PropertyHandle && PropertyHandle->HasMetaData(AssetViewSettingsNameMetadata) ? PropertyHandle->GetMetaData(AssetViewSettingsNameMetadata) : TEXT("AssetPropertyPicker");
	...
	MenuBuilder.BeginSection(NAME_None, LOCTEXT("BrowseHeader", "Browse"));
	{
		...
		if (UE::Editor::ContentBrowser::IsNewStyleEnabled())
		{
			...
			AssetPickerConfig.InitialThumbnailSize = AssetThumbnailSize;
		}
		// Set scale for thumbnail
		if (AssetThumbnailScale.IsSet())
		{
			AssetPickerConfig.ThumbnailScale = AssetThumbnailScale.GetValue();
		}
		...
		AssetPickerConfig.SaveSettingsName = SaveSettingsName;
