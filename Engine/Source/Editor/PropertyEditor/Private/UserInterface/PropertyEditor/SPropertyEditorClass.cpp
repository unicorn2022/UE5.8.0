...
public:
	...
	{
		if (bRequiresVerseConcrete && !FVerseClassProperty::StaticIsClassConcrete(InClass))
		{
			return false;
		}
		if (bRequiresVerseCastable && !FVerseClassProperty::StaticIsClassCastable(InClass))
		{
			return false;
		}
...
void SPropertyEditorClass::Construct(const FArguments& InArgs, const TSharedPtr< FPropertyEditor >& InPropertyEditor)
{
	...
	if (PropertyEditor.IsValid())
	{
		...
		if (RequiredInterface != nullptr)
		{
			...
			{
				UE_LOGF(LogPropertyNode, Warning, "Property (%ls) specifies a MustImplement class which isn't an interface (%ls), clearing filter.", *Property->GetFullName(), *RequiredInterface->GetPathName());
				RequiredInterface = nullptr;
			}
			else if (RequiredInterface == UInterface::StaticClass())
			{
				UE_LOGF(LogPropertyNode, Warning, "Property (%ls) specifies a MustImplement class which isn't valid (UInterface), clearing filter.", *Property->GetFullName());
				RequiredInterface = nullptr;
			}
		}
