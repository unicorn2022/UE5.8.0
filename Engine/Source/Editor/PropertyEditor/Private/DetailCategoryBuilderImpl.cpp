...
FString FDetailCategoryImpl::DetailCategories = TEXT("DetailCategories");
FString FDetailCategoryImpl::TransientPerViewDetailCategories = TEXT("TransientPerViewDetailCategories");
FString FDetailCategoryImpl::PersistentPerViewDetailCategories = TEXT("PersistentPerViewDetailCategories");
...
TOptional<FLinearColor> FDetailCategoryImpl::GetColor() const
{
	return Color;
}

void FDetailCategoryImpl::SetColor(FLinearColor InColor)
{
	Color = InColor;
}

void FDetailCategoryImpl::ClearColor()
{
	Color = TOptional<FLinearColor>();
}
...
TSharedRef<ITableRow> FDetailCategoryImpl::GenerateWidgetForTableView(const TSharedRef<STableViewBase>& OwnerTable, bool bAllowFavoriteSystem)
{
	...
	TSharedRef<SDetailCategoryTableRow> RowWidget =
		...
		.ShowHeader(ParentLayout.IsValid() ? ParentLayout->GetCurrentFilter().bShowHeaders : true);
...
void FDetailCategoryImpl::OnItemExpansionChanged(bool bIsExpanded, bool bShouldSaveState)
{
	if (bRestoreExpansionState && bShouldSaveState)
	{
		...
		GConfig->SetBool(*GetDetailCategoriesExpansionSection(), *GetCategoryPathConfigKey(CategoryPathName), bIsExpanded, GEditorPerProjectIni);
	}

	OnExpansionChangedDelegate.ExecuteIfBound(bIsExpanded);
}

const FString& FDetailCategoryImpl::GetDetailCategoriesExpansionSection() const
{
	if (const TSharedPtr<IDetailsViewPrivate> DetailsView = GetDetailsViewSharedPtr())
	{
		if (DetailsView->GetExpansionPersistenceMethod() == FDetailsViewArgs::EExpansionPersistenceMethod::TransientPerViewIdentifier)
		{
			return TransientPerViewDetailCategories;
		}
		else if (DetailsView->GetExpansionPersistenceMethod() == FDetailsViewArgs::EExpansionPersistenceMethod::PersistentPerViewIdentifier)
		{
			return PersistentPerViewDetailCategories;
		}
	}
...
FString FDetailCategoryImpl::GetCategoryPathConfigKey(const FString& InPath) const
{
	if (const TSharedPtr<IDetailsViewPrivate> DetailsView = GetDetailsViewSharedPtr())
	{
		if (DetailsView->GetExpansionPersistenceMethod() == FDetailsViewArgs::EExpansionPersistenceMethod::TransientPerViewIdentifier
			|| DetailsView->GetExpansionPersistenceMethod() == FDetailsViewArgs::EExpansionPersistenceMethod::PersistentPerViewIdentifier)
		{
			if (ensure(!DetailsView->GetViewIdentifier().IsNone()))
			{
				TStringBuilder<128> ViewIdentifierString;
				ViewIdentifierString.Append(DetailsView->GetViewIdentifier().ToString());
				ViewIdentifierString.Append(".");
				ViewIdentifierString.Append(InPath);
...
bool FDetailCategoryImpl::ShouldBeExpanded() const
{
	...
	else if (bRestoreExpansionState)
	{
		...
		// Save the collapsed state of this section
		GConfig->GetBool(*GetDetailCategoriesExpansionSection(), *GetCategoryPathConfigKey(CategoryPathName), bShouldBeExpanded, GEditorPerProjectIni);
...
void FDetailCategoryImpl::GenerateChildrenForSingleLayout(const FDetailLayout& Layout, const TArray<FDetailLayoutCustomization>& Customizations, FDetailNodeList& OutChildren)
{
	...
	// If this is the favorite category, create a group and put all properties based on their original category instead of showing them all flat
	const FName InstanceName = Layout.GetInstanceName();
	if (bFavoriteCategory || LayoutMap.ShouldShowGroup(InstanceName))
	{
...
void FDetailCategoryImpl::GenerateChildrenForLayouts()
{
	if (bFavoriteCategory)
	{
		// Merge advanced and simple layout customizations into simple nodes
		// Favorite category should not have advanced nodes
		for (const FDetailLayout& Layout : LayoutMap)
		{
			TArray<FDetailLayoutCustomization> Customizations;
			Customizations.Append(Layout.GetSimpleLayouts());
			Customizations.Append(Layout.GetAdvancedLayouts());
			GenerateChildrenForSingleLayout(Layout, Customizations, SimpleChildNodes);
		}
		return;
	}
