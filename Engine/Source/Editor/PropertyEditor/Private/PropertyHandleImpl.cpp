...
void LogAmbiguousArrayIndexWarning(const FPropertyNode* PropertyNode, const FString& CompetingName)
{
	...
	UE_LOGF(LogPropertyNode, Warning, "%ls", *LogMessage);
}
...
static int32 GetArrayPropertyLastValidIndex( FObjectPropertyNode* InObjectNode, const FString& InArrayName )
{
	...
	if (FoundProperty && (FoundProperty->ArrayDim == 1))
	{
		...
		{
			UE_LOGF(LogPropertyNode, Warning, "The property (%ls) passed for array clamping use is not an array.  Clamp will only ensure greater than zero.", *InArrayName);
		}
	}
	else
	{
		UE_LOGF(LogPropertyNode, Warning, "The property (%ls) passed for array clamping was not found.  Clamp will only ensure greater than zero.", *InArrayName);
	}
...
static Type ClampIntegerValueFromMetaData(Type InValue, FPropertyHandleBase& InPropertyHandle, FPropertyNode& InPropertyNode)
{
	...
	if (ArrayClampString.Len())
	{
		...
		{
			UE_LOGF(LogPropertyNode, Warning, "Array Clamping isn't supported in multi-select (Param Name: %ls)", *InPropertyHandle.GetProperty()->GetName());
		}
	}
...
void FPropertyValueImpl::ResetToDefault()
{
	...
	{
		EPropertyValueSetFlags::Type PropertyValueSetFlags = EPropertyValueSetFlags::ResetToDefault;
	
		// Ensure that we only instance the default value when required.
		const FProperty* Property = PropertyNodePin->GetProperty();
		if ((Property && Property->HasAnyPropertyFlags(CPF_InstancedReference))
			|| (PropertyNodePin->HasNodeFlags(EPropertyNodeFlags::EditInlineNew)))
		{
			PropertyValueSetFlags |= EPropertyValueSetFlags::InstanceObjects;
		}
		...
		if(KeyNode.IsValid())
		{
			FPropertyValueImpl(KeyNode, NotifyHook, PropertyUtilities.Pin())
				.ImportText(KeyNode->GetDefaultValueAsString(bUseDisplayName), PropertyValueSetFlags);
		}
		ImportText(PropertyNodePin->GetDefaultValueAsString(bUseDisplayName), PropertyValueSetFlags);
...
void FPropertyHandleBase::ClearResetToDefaultCustomized()
{
	if (TSharedPtr<FPropertyNode> PropertyNode = Implementation->GetPropertyNode(); PropertyNode.IsValid())
	{
		PropertyNode->SetNodeFlags(EPropertyNodeFlags::HasCustomResetToDefault, false);
	}
}
...
bool FPropertyHandleInt::Supports( TSharedRef<FPropertyNode> PropertyNode )
{
	...
	if (!bIsSupported)
	{
		...
		{
			// We only support an enum if it's underlying type is NOT uint8, which is instead handled by FPropertyHandleByte
			const FProperty* UnderlyingProperty = EnumProperty->GetUnderlyingProperty();
			bIsSupported = UnderlyingProperty && !UnderlyingProperty->IsA<FByteProperty>();
		}
	}

	// The value is an integer (or enum that isn't uint8-based)
	return bIsSupported;
}
...
{
	if (Implementation.IsPropertyTypeOf(PropertyClass::StaticClass()))
	{
		return Implementation.GetPropertyValue<PropertyClass>(PropValue);
	}

	const FNumericProperty* NumericProperty = nullptr;
	if (Implementation.IsPropertyTypeOf(FEnumProperty::StaticClass()))
	{
		const FEnumProperty* EnumProperty = CastFieldChecked<FEnumProperty>(Implementation.GetPropertyNode()->GetProperty());
		NumericProperty = EnumProperty->GetUnderlyingProperty();
	}
	else if (Implementation.IsPropertyTypeOf(FNumericProperty::StaticClass()))
	{
		NumericProperty = CastFieldChecked<const FNumericProperty>(Implementation.GetPropertyNode()->GetProperty());
	}

	if (NumericProperty)
	{
		const bool bIsSignedInteger = NumericProperty->CanHoldValue(-1);
		if (bIsSignedInteger)
		{
			return static_cast<ValueType>(NumericProperty->GetSignedIntPropertyValue(PropValue));
		}
		return static_cast<ValueType>(NumericProperty->GetUnsignedIntPropertyValue(PropValue));
	}
...
FPropertyAccess::Result FPropertyHandleInt::GetValue(uint8& OutValue) const
{
	void* PropValue = nullptr;
	FPropertyAccess::Result Res = Implementation->GetValueData(PropValue);
	...
	if (Res == FPropertyAccess::Success)
	{
		OutValue = GetIntegerValue<FByteProperty, uint8>(PropValue, *Implementation);
	}
...
FPropertyAccess::Result FPropertyHandleInt::SetValue(const int8& NewValue, EPropertyValueSetFlags::Type Flags)
{
	...
	FString ValueStr;
	if (const FEnumProperty* EnumProp = CastField<FEnumProperty>(GetProperty()))
	{
		ValueStr = EnumProp->GetEnum()->GetNameStringByValue(FinalValue);
	}
	else
	{
		ValueStr = LexToString(FinalValue);
	}
...
FPropertyAccess::Result FPropertyHandleInt::SetValue(const int16& NewValue, EPropertyValueSetFlags::Type Flags)
{
	...
	FString ValueStr;
	if (const FEnumProperty* EnumProperty = CastField<FEnumProperty>(GetProperty()))
	{
		ValueStr = EnumProperty->GetEnum()->GetNameStringByValue(FinalValue);
	}
	else
	{
		ValueStr = LexToString(FinalValue);
...
FPropertyAccess::Result FPropertyHandleInt::SetValue(const uint8& NewValue, EPropertyValueSetFlags::Type Flags)
{
	FPropertyAccess::Result Res;
	// Clamp the value from any meta data ranges stored on the property value
	uint8 FinalValue = ClampIntegerValueFromMetaData<uint8>(NewValue, *this, *Implementation->GetPropertyNode());
...
FPropertyAccess::Result FPropertyHandleOptional::SetOptionalValue(FProperty* NewProperty, const UClass* NewObjectClass)
{
	...
	{
		void* Addr = ReadAddresses.GetAddress(i);
		void* ValuePtr = OptionalProperty->MarkSetAndGetInitializedValuePointerToReplace(Addr);
		...
		static const FName DefaultSomeMetaDataName = "DefaultSome";
		const FString* DefaultSomeString = OptionalProperty->FindMetaData(DefaultSomeMetaDataName);
		if (ValueProperty && DefaultSomeString)
		{
			ValueProperty->ImportText_Direct(**DefaultSomeString, ValuePtr, nullptr, 0);
		}
		...
		if (ValueProperty && NewObjectClass)
		{
			if (ValueProperty->IsA(FClassProperty::StaticClass()) || ValueProperty->IsA(FSoftClassProperty::StaticClass()))
			{
				...
				{
					void* ClassPropertyValuePtr = ClassProperty->ContainerPtrToValuePtr<void>(ValuePtr);
