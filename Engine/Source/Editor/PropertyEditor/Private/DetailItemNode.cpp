...
void FDetailItemNode::InitPropertyEditor()
{
	...
	else if (CastField<FStructProperty>(NodeProperty))
	{
		const bool bSaveState = false;

		// Prefer the property node's own Expanded flag, which is restored per-element
		// (with array indices) by RestoreAllExpandedItems() and avoids key collisions.
		bool bExpanded = Customization.GetPropertyNode()->HasNodeFlags(EPropertyNodeFlags::Expanded);

		// For non-array-element struct properties, fall back to ExpandedDetailNodes if the
		// property node flag is not set. This is needed because some editor contexts (e.g. the
		// Workspace Editor) may not have fully restored property node flags at this point.
		// We skip this fallback for array elements because ExpandedDetailNodes uses
		// Property->GetFName() as key, which is shared across all elements of a struct array,
		// causing a key collision that corrupts individual element expansion states.
		if (!bExpanded && Customization.GetPropertyNode()->GetArrayIndex() == INDEX_NONE)
		{
			bExpanded = ParentCategory.Pin()->GetSavedExpansionState(*this);
		}

		SetExpansionState(bExpanded, bSaveState);
	}
}
...
void FDetailItemNode::OnItemExpansionChanged( bool bInIsExpanded, bool bShouldSaveState )
{
	...
	if (ParentCategory.IsValid() && bShouldSaveState &&
			...
			|| (Customization.HasPropertyNode() && CastField<FStructProperty>(Customization.GetPropertyNode()->GetProperty()))))
	{
...
void FDetailItemNode::FilterNode(const FDetailFilter& InFilter)
{
	...
	if (TSharedPtr<FPropertyNode> PropertyNodePin = Customization.GetPropertyNode())
	{
		...
		if (!bForceHidden && InFilter.ShouldForceHideProperty.IsBound())
		{
			...
			{
				TSharedPtr<IPropertyHandle> PropertyHandle = Customization.PropertyRow->GetPropertyHandle();
				if (PropertyHandle.IsValid() && InFilter.ShouldForceHideProperty.Execute(PropertyHandle.ToSharedRef()))
				{
					bForceHidden = true;
				}
			}
		...
		if (!bForceHidden)
		{
			if (FProperty* Property = PropertyNodePin->GetProperty())
			{
				if (UPropertyVisibilityOverrideSubsystem* PropertyVisibilityOverrideSubsystem = UPropertyVisibilityOverrideSubsystem::Get())
				{
					if (Customization.PropertyRow.IsValid())
					{
						if (TSharedPtr<IPropertyHandle> PropertyHandle = Customization.PropertyRow->GetPropertyHandle())
						{
							bForceHidden = PropertyVisibilityOverrideSubsystem->ShouldHidePropertyForDetailsPanel(*PropertyHandle);
						}
					}
