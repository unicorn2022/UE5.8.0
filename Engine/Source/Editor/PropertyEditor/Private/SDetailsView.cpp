...
void SDetailsView::Construct(const FArguments& InArgs, const FDetailsViewArgs& InDetailsViewArgs)
{
	...
	CurrentFilter.bShowHeaders = InDetailsViewArgs.bShowHeaders;
	...
	if (DetailsViewArgs.InitialCategories.Num() > 0)
	{
		CurrentFilter.VisibleCategories = DetailsViewArgs.InitialCategories;
	}
	...
	FMenuBuilder DetailViewOptions(true, InternalCommands, InDetailsViewArgs.OptionsExtender);
	...
	DetailViewOptions.AddMenuEntry(
		LOCTEXT("ShowHelperSections", "Show Helper Sections"),
		LOCTEXT("ShowHelperSectionsToolTip", "Specifies whether helper sections like Favorites, Modified are shown in the filter bar"),
		FSlateIcon(),
		FUIAction( 
			FExecuteAction::CreateSP(this, &SDetailsView::OnShowHelperSectionsClicked),
			FCanExecuteAction(),
			FIsActionChecked::CreateSP(this, &SDetailsView::IsShowHelperSectionsChecked)
		),
		NAME_None,
		EUserInterfaceActionType::Check
	);
	...
	// Commands have been registered earlier
	DetailViewOptions.AddMenuEntry(FPropertyEditorCommands::Get().CollapseAll);
	DetailViewOptions.AddMenuEntry(FPropertyEditorCommands::Get().ExpandAll);
	...
	if (DetailsViewArgs.bAllowFavoriteSystem)
	{
		FilterRowHBox->AddSlot()
			...
			[
				...
				.AddMetaData<FTagMetaData>(TEXT("Details.ToggleFavorites"))
				[
					SNew(SImage)
	...
	FilterRowVBox->AddSlot()
		...
		[
			...
			.AddMetaData<FTagMetaData>(FTagMetaData(TEXT("SectionView")))
		];
...
{
	auto SelectionContainsNonActors = [this]() -> bool
	{
		for (const TWeakObjectPtr<UObject> Object : SelectedObjects)
		{
			if (Object.IsValid() && !Object->IsA<AActor>() && !Object->IsA<UActorComponent>())
			{
				return true;
			}
		}
	...
	const bool bShowPropertyMatrixOverride = PropertyModule.GetCanUsePropertyMatrix();
	return (DetailsViewArgs.bShowPropertyMatrixButton && bShowPropertyMatrixOverride 
		&& (PropertyModule.CVarAllowPropertyMatrixForNonActors.GetValueOnAnyThread() || !SelectionContainsNonActors())) ? EVisibility::Visible : EVisibility::Collapsed;
}
...
bool SDetailsView::IsShowHelperSectionsChecked() const
{
	const FDetailsViewConfig* ViewConfig = GetConstViewConfig();
	return ViewConfig && ViewConfig->bShowHelperSections;
}

void SDetailsView::OnShowHelperSectionsClicked()
{
	if (FDetailsViewConfig* ViewConfig = GetMutableViewConfig())
	{
		ViewConfig->bShowHelperSections = !ViewConfig->bShowHelperSections;
...
void SDetailsView::RebuildSectionSelector()
{
	...
	const bool bAddHelperSections = IsShowHelperSectionsChecked();

	if (bAddHelperSections
		&& DetailsViewArgs.bShowOptions 
		&& DetailsViewArgs.bAllowFavoriteSystem)
	{
		SectionSelectorBox->AddSlot()
		[
			CreateSection(FName("Favorites"), NSLOCTEXT("UObjectSection", "Favorites", "Favorites"))
		];
	}

	if (bAddHelperSections
		&& DetailsViewArgs.bShowOptions 
		&& DetailsViewArgs.bShowModifiedPropertiesOption)
	{
		SectionSelectorBox->AddSlot()
		[
			CreateSection(FName("Modified"), NSLOCTEXT("UObjectSection", "Modified", "Modified"))
		];
	}
...
TMap<FName, SDetailsView::FSection> SDetailsView::GetAllSections() const
{
	...
	// Only include a category if it has children in the tree.
	// Permission-list filtering removes items from the tree entirely (not just visually), so a category
	// with no allowed properties will have no children. We use bInIgnoreVisibility=true so that
	// search/section filtering (which hides but doesn't remove items) doesn't affect this check.
	TArray<FName> LayoutCategories;
	auto AddCategoryIfNotEmpty = [&LayoutCategories](const TSharedRef<IDetailTreeNode>& CategoryNode)
	{
		TArray<TSharedRef<IDetailTreeNode>> Children;
		const bool bInIgnoreVisibility = true;
		CategoryNode->GetChildren(Children, bInIgnoreVisibility);
		if (!Children.IsEmpty())
		{
			LayoutCategories.Add(CategoryNode->GetNodeName());
		}
	};
	...
	for (const FDetailLayoutData& Layout : DetailLayouts)
	{
		for (const TSharedRef<FDetailTreeNode>& TreeNode : Layout.DetailLayout->GetAllRootTreeNodes())
		{
			...
			{
				AddCategoryIfNotEmpty(TreeNode);
			}
			else if (TreeNode->GetNodeType() == EDetailNodeType::Object)
			{
				...
				for (const TSharedRef<IDetailTreeNode>& RootObjectChild : RootObjectChildren)
				{
					...
					{
						AddCategoryIfNotEmpty(RootObjectChild);
					}
				}
			}
		}
