...
void SPropertyNameWidget::Construct( const FArguments& InArgs, TSharedPtr<FPropertyEditor> InPropertyEditor )
{
	...
	// Note : ToolTipText has to be captured immediately because FPropertyHandleBase::CreatePropertyNameWidget has a ToolTipOverride that cannot be deferred since 
	//  it's restored to FText::Empty right after SPropertyNameWidget::Construct :
	...
	*/
	...
	const UClass* FindOrLoadClass(const FString& ClassPath)
	{
		if (const UObject* Object = StaticLoadObject(nullptr, nullptr, ClassPath))
		{
			...
			{
				UE_LOGF(LogPropertyNode, Error, "Found object '%ls' instead of a class.", *Object->GetFullName());
			}
		}
		else
		{
			UE_LOGF(LogPropertyNode, Error, "Unable to load class '%ls'.", *ClassPath);
		}

		return nullptr;
	}

	FText GetNumElementsText(const FText& InContainerText, const FPropertyEditor& InPropertyEditor)
	{
		void* DummyValueAddress;
		if (InPropertyEditor.GetPropertyHandle()->GetValueData(DummyValueAddress) == FPropertyAccess::MultipleValues)
		{
			return PropertyEditorConstants::DefaultUndeterminedText;
		}

		return FText::Format(LOCTEXT("NumItemsFmt", "{0} {1} {0}|plural(one=element,other=elements)"), FText::AsNumber(InPropertyEditor.GetPropertyNode()->GetNumChildNodes()), InContainerText);
	}
