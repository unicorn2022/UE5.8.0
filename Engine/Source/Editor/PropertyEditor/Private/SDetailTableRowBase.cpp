...
{
	const TSharedPtr<FDetailTreeNode> DetailTreeNode = OwnerTreeNode.Pin();
	...
	{
		FDetailNodeList VisibleChildren;
		DetailTreeNode->GetChildren( VisibleChildren );
	...
	TSharedPtr<IDetailsViewPrivate> DetailsPanel = DetailTreeNode->GetDetailsViewSharedPtr();
	if (DetailsPanel && DetailsPanel->IsFavoritingEnabled())
	{
		FToolMenuSection& FavoritesSection = ToolMenu->FindOrAddSection(TEXT("Favorites"));

		bool bAddFavorites = true;
		FavoritesSection.AddMenuEntry(
			TEXT("FavoriteAll"),
			NSLOCTEXT("PropertyView", "FavoriteRow", "Add To Favorites"),
			NSLOCTEXT("PropertyView", "FavoriteRow_ToolTip", "Add this row to favorites"),
			FSlateIcon(FAppStyle::Get().GetStyleSetName(), "DetailsView.PropertyIsFavorite"),
			FUIAction(
				FExecuteAction::CreateSP(this, &SDetailTableRowBase::OnFavoriteRow, bAddFavorites),
				FCanExecuteAction(),
				FIsActionChecked(),
				FIsActionButtonVisible::CreateSP(this, &SDetailTableRowBase::CanFavoriteRow, bAddFavorites)
			)
		);
		...
		FavoritesSection.AddMenuEntry(
			TEXT("UnFavoriteAll"),
			NSLOCTEXT("PropertyView", "UnFavoriteRow", "Remove From Favorites"),
			NSLOCTEXT("PropertyView", "UnFavoriteRow_ToolTip", "Remove this row from favorites"),
			FSlateIcon(FAppStyle::Get().GetStyleSetName(), "DetailsView.PropertyIsNotFavorite"),
			FUIAction(
				FExecuteAction::CreateSP(this, &SDetailTableRowBase::OnFavoriteRow, bAddFavorites),
				FCanExecuteAction(),
				FIsActionChecked(),
...
void SDetailTableRowBase::OnFavoriteRow(bool bInAddToFavorite)
{
	
}

bool SDetailTableRowBase::CanFavoriteRow(bool bInAddToFavorite)
{
	return false;
}
