...
void FPropertyEditor::OnAddItem()
{
	...
	FPropertyNode* ParentNode = &PropertyNode.Get();
	while (ParentNode)
	{
		// We expand all parents. Normally, they'd all be expanded (otherwise the property would not be visible and the
		// user would not be able to add a new item), but with customizations it's possible to have something like:
		//  > [Some Label Widget]    | 3 Array elements (+)
		// where the row belongs to a struct property, while the label widget and the array are (for example) two child
		// properties of said struct. In this case, without expanding parents, adding an array item would try to expand
		// the property node which does not have its own row, while the row itself would remain collapsed.
		ParentNode->SetNodeFlags(EPropertyNodeFlags::Expanded, true);
...
void FPropertyEditor::ToggleEditConditionState()
{
	...
	const FPropertyNode* Cur = &PropertyNode.Get();
	const FComplexPropertyNode* Found = nullptr;

	TArray<TMap<FString, int32>> ArrayIndicesPerObject;
	while( InstanceObjects.Num() == 0 )
	{
		Found = Cur->AsComplexNode();
		if( Found )
		{
			ArrayIndicesPerObject.AddDefaulted(Found->GetInstancesNum());
			for (int32 ObjectIndex = 0; ObjectIndex < Found->GetInstancesNum(); ++ObjectIndex)
			{
				FPropertyValueImpl::GenerateArrayIndexMapToObjectNode(ArrayIndicesPerObject[ObjectIndex], &PropertyNode.Get());
				if (TWeakObjectPtr<UObject> Object = Found->GetInstanceAsUObject(ObjectIndex); Object.IsValid())
				{
					InstanceObjects.Add(Object.Get());
				}
			}
		}
	...
	FPropertyChangedEvent ChangeEvent(PropertyNode->GetProperty(), EPropertyChangeType::ToggleEditable, InstanceObjects);
	ChangeEvent.SetArrayIndexPerObject(ArrayIndicesPerObject);
