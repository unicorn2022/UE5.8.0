...
void SDetailSingleItemRow::Construct( const FArguments& InArgs, FDetailLayoutCustomization* InCustomization, bool bHasMultipleColumns, TSharedRef<FDetailTreeNode> InOwnerTreeNode, const TSharedRef<STableViewBase>& InOwnerTableView )
{
	...
	{
		TSharedRef<SWidget> ContentBorder = SNew(SBorder)
		...
		if (Category.IsValid() && Category->GetColor().IsSet())
		{
			this->ChildSlot
			[
				SNew(SHorizontalBox)
...
void SDetailSingleItemRow::SetPropertyColorationTarget(bool bSelectActors /*=false*/)
{
	...
	if (PropertyNode.IsValid() && PropertyHandle.IsValid())
	{
		...
		if (Handle->GetValueAsFormattedString(Value, PPF_Copy) == FPropertyAccess::Success)
		{
			...
			GEditor->SetPropertyColorationTarget(GWorld, Value, Property, TopProperty->GetOwnerClass(), &PropertyChain, bSelectActors);
		}
	}
}
		...
		!GroupProperties.IsEmpty())
	{
		...
		if (!PropertiesNotCopied.IsEmpty())
		{
			UE_LOGF(
				LogPropertyNode,
				Warning,
				"One or more of the properties in group \"%ls\" was not copied:\n%ls",
				*GetRowNameText(),
			...
			!GroupProperties.IsEmpty())
		{
			...
			if (!PropertiesNotPasted.IsEmpty())
			{
				UE_LOGF(
					LogPropertyNode,
					Warning,
					"One or more of the properties in group \"%ls\" was not pasted:\n%ls",
					*GetRowNameText(),
	...
	FToolMenuSection& EditSection = ToolMenu->FindOrAddSection(TEXT("Edit"));
	{
		if (CopyAction.IsBound() && PasteAction.IsBound())
		{
			...
			// Copy
			{
				...
				// Used to trigger animation after a copy
				FUIAction WrappedCopyAction(CopyAction);
				WrappedCopyAction.ExecuteAction = FExecuteAction::CreateSPLambda(this, 
					[this]()
					{
						if (CopyAction.Execute())
						{
							PulseAnimation.Play(SharedThis(this));
						}
					}
			...
			// Paste
			{
				...
				if (OwningDetailsView && OwningDetailsView->IsPropertyEditingEnabled() && WidgetRow.EditConditionValue.Get(true /*DefaultValue*/))
				{
					...
					// Used to trigger animation after a paste
					FUIAction WrappedPasteAction(PasteAction);
					WrappedPasteAction.ExecuteAction = FExecuteAction::CreateSPLambda(this, 
						[this]()
						{
							if (PasteAction.Execute())
							{
								PulseAnimation.Play(SharedThis(this));
							}
						}
		...
		// Select By Property
		{
			FUIAction SelectByPropertyAction = FExecuteAction::CreateSP(this, &SDetailSingleItemRow::SetPropertyColorationTarget, true);
			SelectByPropertyAction.IsActionVisibleDelegate = FIsActionButtonVisible::CreateSP(this, &SDetailSingleItemRow::CanSelectByProperty);

			static const FTextFormat TooltipFormat = NSLOCTEXT("PropertyView_Single", "SelectActorsByProperty_ToolTip", "Select all actors with the same value for the property named:\n{0}");

			EditSection.AddMenuEntry(
				TEXT("SelectByProperty"),
				NSLOCTEXT("PropertyView", "SelectActorsByProperty", "Select Actors by Property"),
				FText::Format(TooltipFormat, GetPropertyDisplayName()),
				FSlateIcon(FCoreStyle::Get().GetStyleSetName(), "GenericCommands.SelectAll"),
				SelectByPropertyAction);
		}
...
bool SDetailSingleItemRow::CanFavoriteRow(bool bInAddToFavorite)
{
	if (bInAddToFavorite == IsFavorite())
	{
		return false;		
	}
...
void SDetailSingleItemRow::OnFavoriteRow(bool bInAddToFavorite)
{
	const TSharedPtr<FDetailTreeNode> OwnerTreeNodePinned = OwnerTreeNode.Pin();
	if (!OwnerTreeNodePinned.IsValid())
	{
		return;
	}

	const TSharedPtr<IDetailsViewPrivate> DetailsView = OwnerTreeNodePinned->GetDetailsViewSharedPtr();
	if (!DetailsView)
	{
		return;
	}
	...
	if (Customization->HasPropertyNode())
	{
		const TSharedPtr<FPropertyNode> PropertyNode = Customization->GetPropertyNode();
	...
	else if (Customization->HasCustomBuilder())
	{
		...
		if (PropertyHandle.IsValid())
		{
			StaticCastSharedPtr<FPropertyHandleBase>(PropertyHandle)->GetPropertyNode()->SetFavorite(bInAddToFavorite); 
		}
		else if (!OriginalPath.IsEmpty())
		{
			DetailsView->SetCustomBuilderFavorite(OriginalPath, bInAddToFavorite);
		}
	}
	...
	DetailsView->MoveScrollOffset(bInAddToFavorite ? ExpandSize : -ExpandSize);
...
bool SDetailSingleItemRow::CanSelectByProperty()
{
	return GEditor && GEditor->IsPropertyColorationModeActive();
}
...
void SDetailSingleItemRow::OnToggleAllowList() const
{
	...
	if (Owner)
	{
		...
		if (IsAllowListChecked())
		{
			...
			UE_LOGF(LogPropertyEditorPermissionList, Log, "Removing %ls from AllowList", *GetRowNameText());
		}
		else
		{
			FPropertyEditorPermissionList::Get().AddToAllowList(ExactStruct, Owner->GetNodeName(), OwnerName);
			UE_LOGF(LogPropertyEditorPermissionList, Log, "Adding %ls to AllowList", *GetRowNameText());
		}
	}
}
...
void SDetailSingleItemRow::OnToggleDenyList() const
{
	...
	if (Owner)
	{
		...
		if (IsDenyListChecked())
		{
			...
			UE_LOGF(LogPropertyEditorPermissionList, Log, "Removing %ls from DenyList", *GetRowNameText());
		}
		else
		{
			FPropertyEditorPermissionList::Get().AddToDenyList(ExactStruct, Owner->GetNodeName(), OwnerName);
			UE_LOGF(LogPropertyEditorPermissionList, Log, "Adding %ls to DenyList", *GetRowNameText());
		}
	}
}
