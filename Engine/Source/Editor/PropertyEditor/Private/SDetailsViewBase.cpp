...
static FString DetailPropertyExpansion = TEXT("DetailPropertyExpansion");
static FString TransientPerViewDetailPropertyExpansion = TEXT("TransientPerViewDetailPropertyExpansion");
static FString PersistentPerViewDetailPropertyExpansion = TEXT("PersistentPerViewDetailPropertyExpansion");

static FString DetailCustomWidgetExpansion = TEXT("DetailCustomWidgetExpansion");
static FString TransientPerViewDetailCustomWidgetExpansion = TEXT("TransientPerViewDetailCustomWidgetExpansion");
static FString PersistentPerViewDetailCustomWidgetExpansion = TEXT("PersistentPerViewDetailCustomWidgetExpansion");

void SDetailsViewBase::ResetTransientPerViewDetailExpansion()
{
	GConfig->RemoveSectionFromBranch(*TransientPerViewDetailPropertyExpansion, *GEditorPerProjectIni);
	GConfig->RemoveSectionFromBranch(*TransientPerViewDetailCustomWidgetExpansion, *GEditorPerProjectIni);
	GConfig->RemoveSectionFromBranch(*FDetailCategoryImpl::TransientPerViewDetailCategories, *GEditorPerProjectIni);
}

void SDetailsViewBase::ResetPersistentPerViewDetailExpansion()
{
	GConfig->RemoveSectionFromBranch(*PersistentPerViewDetailPropertyExpansion, *GEditorPerProjectIni);
	GConfig->RemoveSectionFromBranch(*PersistentPerViewDetailCustomWidgetExpansion, *GEditorPerProjectIni);
	GConfig->RemoveSectionFromBranch(*FDetailCategoryImpl::PersistentPerViewDetailCategories, *GEditorPerProjectIni);
}

static FAutoConsoleCommand ResetTransientPerViewDetailExpansionCommand(
	TEXT("PropertyEditor.ResetTransientPerViewDetailExpansion"),
	TEXT("Removes all transient per view detail expansions. These or automatically purged without read on editor startup to avoid incremental file bloat."),
	FConsoleCommandDelegate::CreateStatic(&SDetailsViewBase::ResetTransientPerViewDetailExpansion)
);

static FAutoConsoleCommand ResetPersistentPerViewDetailExpansionCommand(
	TEXT("PropertyEditor.ResetPersistentPerViewDetailExpansion"),
	TEXT("Removes all persistent per view detail expansions."),
	FConsoleCommandDelegate::CreateStatic(&SDetailsViewBase::ResetPersistentPerViewDetailExpansion)
);
...
void SDetailsViewBase::ClearSearch()
{
...
void SDetailsViewBase::SetSearchText(const FText& InSearch)
{
...
FDetailFilter SDetailsViewBase::CreateDetailFilter()
{
	FDetailFilter Filter = CurrentFilter;
	
	// When "Favorites" section is active, show favorited items
	Filter.bShowFavoritesCategory |= CurrentFilter.VisibleSections.Contains(TEXT("Favorites"));

	// When "Modified" section is active, show all items with modified properties
	if (CurrentFilter.VisibleSections.Contains(TEXT("Modified")))
	{
		Filter.VisibleSections.Empty();
		Filter.bShowOnlyModified |= true;
	}

	return Filter;
}

void SDetailsViewBase::BindCommands()
{
	if (InternalCommands.IsValid())
	{
		return;
	}

	// Register commands here otherwise they are destroyed...
	FPropertyEditorCommands::Register();
	const FPropertyEditorCommands& PropertyEditorCommands = FPropertyEditorCommands::Get();

	InternalCommands = MakeShared<FUICommandList>();
	InternalCommands->MapAction(
		PropertyEditorCommands.CollapseAll, 
		FExecuteAction::CreateSP(this, &SDetailsViewBase::SetRootExpansionStates, /** Expand */false, /** Recurse */false)
	);

	InternalCommands->MapAction(
		PropertyEditorCommands.ExpandAll, 
		FExecuteAction::CreateSP(this, &SDetailsViewBase::SetRootExpansionStates, /** Expand */true, /** Recurse */false)
	);
}
...
void SDetailsViewBase::SetNodeExpansionState(TSharedRef<FDetailTreeNode> InTreeNode, bool bExpand, bool bRecursive)
{
	...
	constexpr bool bShouldSaveState = true;
	InTreeNode->OnItemExpansionChanged(bExpand, bShouldSaveState);

	if (bRecursive && !Children.IsEmpty())
	{
...
void SDetailsViewBase::GetExpansionState(UStruct* BaseStruct, FStringPrefixTree& OutPrefixTree) const
{
	...
	{
		GConfig->GetSingleLineArray(*GetDetailPropertyExpansionSection(), *GetStructExpansionConfigKey(BaseStruct), DetailPropertyExpansionStrings, GEditorPerProjectIni);
	}
...
FName SDetailsViewBase::GetViewIdentifier() const
{
	return DetailsViewArgs.ViewIdentifier;
}

FDetailsViewArgs::EExpansionPersistenceMethod SDetailsViewBase::GetExpansionPersistenceMethod() const
{
	return DetailsViewArgs.ExpansionPersistenceMethod;
}
...
FReply SDetailsViewBase::OnKeyDown(const FGeometry& InGeometry, const FKeyEvent& InKeyEvent)
{
	if (InternalCommands.IsValid() && InternalCommands->ProcessCommandBindings(InKeyEvent))
	{
		return FReply::Handled();
	}

	return IDetailsViewPrivate::OnKeyDown(InGeometry, InKeyEvent);
}
...
const FString& SDetailsViewBase::GetDetailPropertyExpansionSection() const
{
	if (DetailsViewArgs.ExpansionPersistenceMethod == FDetailsViewArgs::EExpansionPersistenceMethod::TransientPerViewIdentifier)
	{
		return TransientPerViewDetailPropertyExpansion;
	}
	else if (DetailsViewArgs.ExpansionPersistenceMethod == FDetailsViewArgs::EExpansionPersistenceMethod::PersistentPerViewIdentifier)
	{
		return PersistentPerViewDetailPropertyExpansion;
	}

	return DetailPropertyExpansion;
}

const FString& SDetailsViewBase::GetDetailCustomWidgetExpansionSection() const
{
	if (DetailsViewArgs.ExpansionPersistenceMethod == FDetailsViewArgs::EExpansionPersistenceMethod::TransientPerViewIdentifier)
	{
		return TransientPerViewDetailCustomWidgetExpansion;
	}
	else if (DetailsViewArgs.ExpansionPersistenceMethod == FDetailsViewArgs::EExpansionPersistenceMethod::PersistentPerViewIdentifier)
	{
		return PersistentPerViewDetailCustomWidgetExpansion;
	}

	return DetailCustomWidgetExpansion;
}

FString SDetailsViewBase::GetStructExpansionConfigKey(UStruct* InStruct) const
{
	if (DetailsViewArgs.ExpansionPersistenceMethod == FDetailsViewArgs::EExpansionPersistenceMethod::TransientPerViewIdentifier
		|| DetailsViewArgs.ExpansionPersistenceMethod == FDetailsViewArgs::EExpansionPersistenceMethod::PersistentPerViewIdentifier)
	{
		if (ensure(!DetailsViewArgs.ViewIdentifier.IsNone()))
		{
			TStringBuilder<128> ViewIdentifierString;
			ViewIdentifierString.Append(DetailsViewArgs.ViewIdentifier.ToString());
			ViewIdentifierString.Append(".");
			ViewIdentifierString.Append(InStruct->GetName());
			return ViewIdentifierString.ToString();
		}
	}

	return InStruct->GetName();
}
...
void SDetailsViewBase::SaveExpandedItems(TSharedRef<FPropertyNode> StartNode)
{
	...
	for (UStruct* Struct = BestBaseStruct; Struct && ((BestBaseStruct == Struct) || (Struct != AActor::StaticClass())); Struct = Struct->GetSuperStruct())
	{
		if (StartNode->GetNumChildNodes() > 0)
		{
			...
			if (!bShouldSave)
			{
				...
				GConfig->GetSingleLineArray(*GetDetailPropertyExpansionSection(), *GetStructExpansionConfigKey(Struct), DummyExpandedPropertyItems, GEditorPerProjectIni);
				bShouldSave = DummyExpandedPropertyItems.Num() > 0;
			}

			if (bShouldSave)
			{
				GConfig->SetSingleLineArray(*GetDetailPropertyExpansionSection(), *GetStructExpansionConfigKey(Struct), ExpandedPropertyItems, GEditorPerProjectIni);
			}
		}
	}
	...
	if (DetailLayouts.Num() > 0 && BestBaseStruct)
	{
		...
		if (!bShouldSave)
		{
			...
			GConfig->GetString(*GetDetailCustomWidgetExpansionSection(), *GetStructExpansionConfigKey(BestBaseStruct), DummyExpandedCustomItemsString, GEditorPerProjectIni);
			bShouldSave = !DummyExpandedCustomItemsString.IsEmpty();
		}

		if (bShouldSave)
		{
			GConfig->SetString(*GetDetailCustomWidgetExpansionSection(), *GetStructExpansionConfigKey(BestBaseStruct), *ExpandedCustomItemsBuilder, GEditorPerProjectIni);
		}
	}
}
...
void SDetailsViewBase::RestoreExpandedItems(TSharedRef<FPropertyNode> StartNode, TMap<UStruct*, FStringPrefixTree>* OptionalExpansionStates)
{
	...
	else
	{
		...
		GetExpansionState(BestBaseStruct, PrefixTree);
		SetExpandedItems(StartNode, PrefixTree, false);
	}
	...
	if (BestBaseStruct)
	{
		...
		GConfig->GetString(*GetDetailCustomWidgetExpansionSection(), *GetStructExpansionConfigKey(BestBaseStruct), ExpandedCustomItems, GEditorPerProjectIni);
...
void SDetailsViewBase::UpdateFilteredDetails()
{
	...
	const FDetailFilter Filter = CreateDetailFilter();
	...
	for (int32 RootNodeIndex = 0; RootNodeIndex < RootPropertyNodes.Num(); ++RootNodeIndex)
	{
		if (RootPropertyNodes[RootNodeIndex].IsValid())
		{
			...
			if (DetailLayout.IsValid())
			{
				...
				DetailLayout->FilterDetailLayout(Filter);
