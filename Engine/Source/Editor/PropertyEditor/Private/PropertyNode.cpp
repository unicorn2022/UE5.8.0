...
{
	using FPropertyNodeInlineArray = TArray<TSharedPtr<const FPropertyNode>, TInlineAllocator<2>>;
	auto GetChildStructureNodes = [Self = this](FPropertyNodeInlineArray& OutChildStructurePropertyNodes)
	{
		for (const TSharedPtr<FPropertyNode>& ChildNode : Self->ChildNodes)
		{
			if (const FComplexPropertyNode* ComplexNode = ChildNode->AsComplexNode())
			{
				if (ComplexNode->AsStructureNode())
				{
					OutChildStructurePropertyNodes.Emplace(ChildNode);
				}
			}
		}
	};
	...
	if (GetParentNode() == nullptr || HasNodeFlags(EPropertyNodeFlags::RequiresValidation))
	{
		...
		if (Property.IsValid())
		{
			...
			// Cached Child Structure nodes that are manually attached to this property node
			FPropertyNodeInlineArray CachedChildStructureNodes;
			GetChildStructureNodes(CachedChildStructureNodes);
			...
			int32 NumContainerChildren = -1;
			// Assume all arrays have the same length
			bool bContainersHaveEqualNum = true;
			// Assume all arrays match the number of property window children
			bool bContainersMatchChildNum = true;

			bool bContainerHasNewItem = false;
			...
			for (TSharedPtr<const FPropertyNode>& ChildStructureNode : CachedChildStructureNodes)
			{
				bValidateChildren |= DoesChildPropertyRequireValidation(ChildStructureNode->Property.Get());
			}
			...
			if (!bSuccess)
			{
				UE_LOGF( LogPropertyNode, Verbose, "Object is invalid %ls", *Property->GetName() );
				return EPropertyDataValidationResult::ObjectInvalid;
			}
			...
			for (int32 Scan = 0; Scan < ReadAddresses.Num(); ++Scan)
			{
				...
				if( ArrayProperty && !bIgnoreAllMismatch)
				{
					...
					// If first child
					if (NumContainerChildren == -1)
					{
						NumContainerChildren = ArrayNum;
					}
					bContainerHasNewItem = GetNumChildNodes() < ArrayNum;
					// Make sure multiple arrays match
					bContainersHaveEqualNum = bContainersHaveEqualNum && (NumContainerChildren == ArrayNum);
					// Make sure the array matches the number of property node children
					bContainersMatchChildNum = bContainersMatchChildNum && (GetNumChildNodes() == ArrayNum);
				}
				...
				if (SetProperty && !bIgnoreAllMismatch)
				{
					...
					if (NumContainerChildren == -1)
					{
						NumContainerChildren = SetNum;
					}

					bContainerHasNewItem = GetNumChildNodes() < SetNum;
					bContainersHaveEqualNum = bContainersHaveEqualNum && (NumContainerChildren == SetNum);
					bContainersMatchChildNum = bContainersMatchChildNum && (GetNumChildNodes() == SetNum);
				}
				...
				if (MapProperty && !bIgnoreAllMismatch)
				{
					...
					if (NumContainerChildren == -1)
					{
						NumContainerChildren = MapNum;
					}

					bContainerHasNewItem = GetNumChildNodes() < MapNum;
					bContainersHaveEqualNum = bContainersHaveEqualNum && (NumContainerChildren == MapNum);
					bContainersMatchChildNum = bContainersMatchChildNum && (GetNumChildNodes() == MapNum);
				}
				...
				if (OptionalProperty && !bIgnoreAllMismatch)
				{
					...
					bContainerHasNewItem = bIsSet != GetNumChildNodes();
				}
			}

			// If all arrays match each other but they do NOT match the property structure, cause a rebuild
			// Don't trigger a rebuild if we have any structure child nodes, because we can't retrieve that information from the actual memory
			// todo: Find a way to support a mix of container elements and manually added structure as child nodes
			if (CachedChildStructureNodes.IsEmpty() && bContainersHaveEqualNum && !bContainersMatchChildNum)
			{
				RebuildChildren();

				if( bContainerHasNewItem && ChildNodes.Num() )
				{
			...
			// Don't trigger a rebuild if we have any structure child nodes, because we can't retrieve that information from the actual memory
			if (CachedChildStructureNodes.IsEmpty() && bShowInnerObjectPropertiesObjectChanged)
			{
				RebuildChildren();
	...
	if (EditConditionExpression.IsValid() && !Property->HasMetaData(Name_HideEditConditionToggle))
	{
		...
		if (ConditionalProperty != nullptr)
		{
			...
			if (bIsInlineEditCondition)
			{
				if (!bIsEditable)
				{
					UE_LOGF(LogPropertyNode, Warning, "Property being used as inline edit condition is not editable, but has InlineEditConditionToggle flag. Field \"%ls\" in class \"%ls\".", *ConditionalProperty->GetNameCPP(), *Property->GetOwnerStruct()->GetName());
				}
				return true;
			}
		}
	...
	if( bUpdateDiffersFromDefault || UpdateDiffersFromDefaultEpoch != PropertyEditorPolicy::Get().GetPolicyEpoch())
	{
		...
		if (const FStructurePropertyNode* StructNode = FindStructureItemParent())
		{
			...
			for (int32 Index = 0; !bDiffersFromDefault && Index < Structs.Num(); Index++)
			{
				...
				const uint8* PropertyDefaultAddress;
				const uint8* PropertyDefaultBaseAddress;
				...
				if (TopLevelObject)
				{
					UObject* ObjectArchetype = FPropertyNode::GetArchetype(TopLevelObject);
					check(ObjectArchetype);

					PropertyDefaultAddress = GetValueAddress(StructNode->GetValueAddressFromObject(ObjectArchetype), bIsSparse, /*bIsStruct=*/true);
					PropertyDefaultBaseAddress = BaseNode->GetValueBaseAddress(StructNode->GetValueBaseAddressFromObject(ObjectArchetype), bIsSparse, /*bIsStruct=*/true);
				}
				else
				{
					...
					PropertyDefaultAddress = GetValueAddress(DefaultStruct.GetStructMemory(), bIsSparse, /*bIsStruct=*/true);
					PropertyDefaultBaseAddress = BaseNode->GetValueBaseAddress(DefaultStruct.GetStructMemory(), bIsSparse, /*bIsStruct=*/true);
				}
...
FString FPropertyNode::GetDefaultValueAsString(bool bUseDisplayName)
{
	...
	if (const FStructurePropertyNode* StructNode = FindStructureItemParent())
	{
		...
		const uint8* LastPropertyDefaultAddress = nullptr;
		const FObjectPropertyNode* TopLevelObjectNode = StructNode->FindObjectItemParent();
		for (int32 StructIndex = 0; StructIndex < Structs.Num(); ++StructIndex)
		{
			...
			if (NodeDefaultValue.IsEmpty()
				|| PropertyDefaultAddress != LastPropertyDefaultAddress)
			{
				NodeDefaultValue = GetDefaultValueAsString(PropertyDefaultAddress, Prop, Mode, TopLevelObject);
				LastPropertyDefaultAddress = PropertyDefaultAddress;
			}
...
void FPropertyNode::NotifyPostChange( FPropertyChangedEvent& InPropertyChangedEvent, class FNotifyHook* InNotifyHook )
{
	...
	if( ObjectNode )
	{
		...
		while ( true )
		{
			...
			{
				UE_LOGF(LogPropertyNode, Error, "The FPropertyNode was destroy while processing the PostEditChangeProperty or PostEditChangeChainProperty.");
				// Redraw viewports
			...
			{
				ObjectNode = nullptr;
				UE_LOGF(LogPropertyNode, Error, "Object for property '%ls, was valid before the PostEditChange callback and now it's invalid", *Property->GetName());
				break;
			}
		...
		for (auto PropertyChainNode = PropertyChain->GetActiveMemberNode(); PropertyChainNode && CurrentPropertyNode.IsValid() ; PropertyChainNode = PropertyChainNode->GetNextNode())
		{
			if (const FProperty* CurrentProperty = PropertyChainNode->GetValue())
			{
				...
				if (!ForceRebuildPropertyName.IsEmpty())
				{
					...
					{
						UE_LOGF(LogPropertyNode, Error, "Could not find named property '%ls' referenced from %ls ForceRebuildProperty", *ForceRebuildPropertyName, *CurrentPropertyNode->GetDisplayName().ToString());
					}
				}
				else if (CurrentProperty->HasMetaData(NAME_ForceRebuildProperty))
				{
					CurrentPropertyNode->RequestRebuildChildren();
				}
