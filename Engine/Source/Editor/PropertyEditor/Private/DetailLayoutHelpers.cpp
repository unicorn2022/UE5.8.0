				...
				if (bVisibleByDefault && !bPushOutStructProps)
				{
					...
					if (InUpdateArgs.bEnableFavoriteSystem)
					{
						if (ChildNodePtr->IsFavorite())
						{
							...
							// Add the property to the favorite by preserving its category, this will duplicate the row
							FavoritesCategory.AddPropertyNode(ChildNodePtr.ToSharedRef(), CurCategory);
						}
