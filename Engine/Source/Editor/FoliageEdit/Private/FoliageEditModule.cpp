...
public:
	...
	virtual void StartupModule() override
	{
		...
		IFoliageEditModuleBase::SetFoliageEditModulePtr(this);
	}
	...
	{
		IFoliageEditModuleBase::SetFoliageEditModulePtr(nullptr);
	...
	virtual void RegisterComponentBaseIDClassToIgnore(TSubclassOf<UActorComponent> InClassToIgnore) override
	{
		ComponentBaseIDClassesToIgnore.Add(InClassToIgnore.Get());
	}

	virtual void UnregisterComponentBaseIDClassToIgnore(TSubclassOf<UActorComponent> InClassToUnregister) override
	{
		ComponentBaseIDClassesToIgnore.Remove(InClassToUnregister.Get());
	}

	virtual bool ShouldIgnoreComponentForBaseID(UActorComponent* InComponent) const
	{
		for (TSoftObjectPtr<UClass> ClassToCheckPtr : ComponentBaseIDClassesToIgnore)
		{
			if (UClass* ClassToCheck = ClassToCheckPtr.Get())
			{
				if (InComponent->IsA(ClassToCheck))
				{
					return true;
				}
			}
	...
	TSet<TSoftObjectPtr<UClass>> ComponentBaseIDClassesToIgnore;
#endif
};
