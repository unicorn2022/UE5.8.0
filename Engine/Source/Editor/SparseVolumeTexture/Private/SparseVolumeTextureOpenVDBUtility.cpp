...
bool GetOpenVDBGridInfo(TArray64<uint8>& SourceFile, bool bCreateStrings, TArray<FOpenVDBGridInfo>* OutGridInfo)
{
	...
	{
		UE_LOGF(LogSparseVolumeTextureOpenVDBUtility, Error, "Failed to read file due to exception: %ls", *FString(Exception.what()));
		return false;
	}
	...
	bool Initialize(TArray64<uint8>& SourceFile, const FOpenVDBImportOptions& ImportOptions, const FIntVector3& InVolumeBoundsMin)
	{
		...
		for (int32 AttributesIdx = 0; AttributesIdx < NumAttributesDescs; ++AttributesIdx)
		{
			...
			if (bHasValidSourceGrids[AttributesIdx])
			{
				...
				{
					UE_LOGF(LogSparseVolumeTextureOpenVDBUtility, Warning, "SparseVolumeTexture is set to use an unsupported format: %i", (int32)Attributes[AttributesIdx].Format);
					return false;
				}
			}
		}
		...
		{
			UE_LOGF(LogSparseVolumeTextureOpenVDBUtility, Warning, "SparseVolumeTexture has all components set to <None>, so there is nothing to import.");
			return false;
		}
		...
		for (const FOpenVDBSparseVolumeAttributesDesc& AttributesDesc : Attributes)
		{
			for (const FOpenVDBSparseVolumeComponentMapping& Mapping : AttributesDesc.Mappings)
			{
				...
				{
					UE_LOGF(LogSparseVolumeTextureOpenVDBUtility, Warning, "SparseVolumeTexture has invalid index into the array of grids in the source file: %i", SourceGridIndex);
					return false;
				}
			}
		}
		...
		TOptional<FTransform> PreviousGridTransform;
		...
		for (int32 AttributesIdx = 0; AttributesIdx < NumAttributesDescs; ++AttributesIdx)
		{
			for (int32 CompIdx = 0; CompIdx < 4; ++CompIdx)
			{
				...
				if (PreviousGridTransform && !PreviousGridTransform->Equals(GridInfo.Transform))
				{
					UE_LOGF(LogSparseVolumeTextureOpenVDBUtility, Warning, "Frame has multiple grids with different transforms in the same frame! Data will likely not be imported/displayed correctly!");
				}
				PreviousGridTransform = GridInfo.Transform;
		...
		FrameTransform = PreviousGridTransform ? *PreviousGridTransform : FTransform::Identity;
