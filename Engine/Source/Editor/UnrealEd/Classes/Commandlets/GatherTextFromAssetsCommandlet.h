...
// Filtering of asset registry elements
	...
	bool DiscoverExternalPackages(TArray<FAssetData>& InOutAssetDataArray);
	void RemoveExistingExternalPackages(TArray<FAssetData>& InOutAssetDataArray, const TSet<FName>* WorldPackageFilter, TSet<FName>& OutExternalActorsWorldPackageNames, TSet<FName>& OutGameFeatureDataPackageNames) const;

	void ExtractPackageNamesToGather(const TArray<FAssetData>& InAssetDataArray, TSet<FName>& OutPackageNames, TMap<FName, FName>& OutExternalToOuterPackages) const;
	void PopulatePackagesPendingGather(TSet<FName> PackageNamesToGather);
	void ProcessAndRemoveCachedPackages(const TMap<FName, FName>& ExternalToOuterPackages, TMap<FName, TSet<FGuid>>& OutExternalActorsWithStaleOrMissingCaches, TMap<FName, TSet<FName>>& OutExternalPackagesWithStaleOrMissingCaches);
	void MergeInExternalPackagesWithStaleOrMissingCaches(TMap<FName, TSet<FGuid>>& ExternalActorsWithStaleOrMissingCaches, TMap<FName, TSet<FName>>& ExternalPackagesWithStaleOrMissingCaches);
	...
	EPackageLocCacheState CalculatePackageLocCacheState(const FPackageFileSummary& PackageFileSummary, const FName PackageName, bool bIsExternalActorPackage, bool bRequiresMetaDataGather) const;
	...
	struct FPackagePendingGather
	{
		...
		/** The set of external packages to process */
		TSet<FName> ExternalPackages;

		/** The localization ID of this package, if any */
		FString PackageLocalizationId;

		/** True if this package contains a map */
		bool bIsMapPackage = false;

		/** True if this package must also gather metadata (this will also process it locally rather than on a worker) */
		bool bRequiresMetaDataGather = false;
	...
	/** Helper for gathering text from field metadata (Blueprint/UserDefinedStruct/UserDefinedEnum). Non-null only when InputKeys is configured. */
