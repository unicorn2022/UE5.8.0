...
struct FPreviewPlatformInfo
{
	...
	,	bPreviewShaderPlatformActive(false)
	...
	FPreviewPlatformInfo(ERHIFeatureLevel::Type InFeatureLevel, 
		...
		bool InbPreviewShaderPlatformActive = false, 
	...
	,	bPreviewShaderPlatformActive(InbPreviewShaderPlatformActive)
	...
	/** Is shader platform preview currently active */
	bool bPreviewShaderPlatformActive;

	/** Preview Shader Platform Name */
	FName PreviewShaderPlatformName;

	/** Checks if two FPreviewPlatformInfos are for the same preview platform. Note, this does NOT compare the bPreviewShaderPlatformActive flag */
	...
	{
		return bPreviewShaderPlatformActive ? PreviewPlatformName : NAME_None;
	}
...
public:
	...
	UE_DEPRECATED(5.8, "This is replaced by the (protected) bCheckForMovementLockedItems.")
	bool bCheckForLockActors;

	UE_DEPRECATED(5.8, "This is replaced by the (private) bHasSelectedMovementLockedItems.")
	bool bHasLockedActors;

protected:
	/** Whether or not to recheck the current selection for movement locked items the next time HasSelectedMovementLockedItems is called */
	bool bCheckForMovementLockedItems = true;
private:
	/** Cached state of whether or not we have locked items in the selection*/
	bool bHasSelectedMovementLockedItems = false;

	// Don't use these handles directly. Prefer the getters in case the queries are moved
	//  out to a shared location.
	mutable UE::Editor::DataStorage::QueryHandle SelectedMovementLockedItemsQuery = UE::Editor::DataStorage::InvalidQueryHandle;
	mutable UE::Editor::DataStorage::QueryHandle SelectedItemsQuery = UE::Editor::DataStorage::InvalidQueryHandle;
	
	/** Reads FTypedElementWorldColumn for items that have FTypedElementSelectionColumn and FTypedElementLockTransformTag */
	UE::Editor::DataStorage::QueryHandle GetSelectedMovementLockedItemsQuery() const;
	
	/** Reads FTypedElementWorldColumn column for items that have FTypedElementSelectionColumn */
	UE::Editor::DataStorage::QueryHandle GetSelectedItemsQuery() const;

	void UnregisterTEDSQueries();
...
public:
	...
	UE_DEPRECATED(5.8, "FPreviewFeatureLevelChanged is deprecated, use FPreviewShaderPlatformChanged instead")
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	UE_DEPRECATED(5.8, "FPreviewPlatformChanged is deprecated, use FPreviewShaderPlatformChanged instead")
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	/** A delegate that is called when the preview platform changes. */
	DECLARE_MULTICAST_DELEGATE_OneParam(FPreviewShaderPlatformChanged, EShaderPlatform);
	FPreviewShaderPlatformChanged PreviewShaderPlatformChanged;
	...
	DECLARE_MULTICAST_DELEGATE_OneParam(FOnSoundBasePreview, TObjectPtr<USoundBase>)
	FOnSoundBasePreview OnSoundBasePreview;
...
public:
	...
	*/
	 ...
	 * @param	bSelectActors		Whether to also select the actors that match the PropertyValue or not
	 */
	UNREALED_API virtual void SetPropertyColorationTarget(UWorld* InWorld, const FString& PropertyValue, class FProperty* Property, class UClass* CommonBaseClass, TSharedRef<FEditPropertyChain>* PropertyChain, bool bSelectActors = false);

	/**
	 * Checks if the active viewport mode is property coloration mode.
	 *
	 * @return	true if the active viewport mode is property coloration mode.
	 */
	UNREALED_API bool IsPropertyColorationModeActive();
	 ...
	 */
	UE_DEPRECATED(5.8, "Use ToggleSelectedItemMovementLock instead.")
	UNREALED_API void ToggleSelectedActorMovementLock();

	/**
	 * @return true if there are selected locked actors
	 */
	UE_DEPRECATED(5.8, "Use HasSelectedMovementLockedItems instead.")
	UNREALED_API bool HasLockedActors();

	/**
	 * Toggles the movement lock on selected items via TEDS.
	 */
	UE_EXPERIMENTAL(5.8, "This API is subject to change.")
	UNREALED_API void ToggleSelectedItemMovementLock();

	/**
	 * @return true if any selected item is marked as movement-locked in TEDS.
	 */
	UE_EXPERIMENTAL(5.8, "This API is subject to change.")
	UNREALED_API bool HasSelectedMovementLockedItems();
	...
	/** An async editor wrapper for UPackage::SaveAsync. Auto-adds files to source control when necessary
 	 *
 	 *  @note It is very important that any optional arguments passed via SaveArgs (i.e. ArchiveCookData, SavePackageContext) 
 	 *        stay valid for the entire duration of the task.
	 */
	UNREALED_API UE::Tasks::TTask<FSavePackageResultStruct> SaveAsync(UPackage* InOuter, UObject* InAsset, const TCHAR* Filename,
		const FSavePackageArgs& SaveArgs);
	...
	/**
	 * Toggle on/off the Editor Flip of the Constrained Aspect Ratio in the preview.
	 */
	UNREALED_API void ToggleFlipConstrainedAspectRatioInPreview();

	/**
	 * @return  returns if the Editor Flips the Constrained Aspect Ratio in the preview
	 */
	UNREALED_API bool GetFlipConstrainedAspectRatioInPreview();
...
private:
	/**  Helper struct to cache the world package dirty state and reset the state for any packages dirtied during the scope. */
	struct FWorldScopedPackageDirtyGuard;
...
private:
	...
	/** True if we Flip the Constrained Aspect Ratio in the Preview */
	bool bFlipConstrainedAspectRatioInPreview;
	...
	TArray<class FEditorUndoClient*> InflightUndoClients;
...
protected:
	...
	// Handle requests from slate application to open debug objects.
	UNREALED_API bool HandleOpenDebugObject(UObject* DebugObject);
...
private:
	...
	/** Logic that needs to run on change of effective shader platform (can happen via both SetPreviewPlatform and TogglePreviewPlatform). */
...
public:
	...
	UNREALED_API void SetPreviewPlatform(const FPreviewPlatformInfo& NewPreviewPlatform, bool bSaveSettings, bool bCompileShaders = true);

	/** Toggle the preview platform */
	UNREALED_API void TogglePreviewPlatform();

	/** Return whether the preview platform is enabled for use via the user accessing Settings->Preview Rendering Level. */
	UNREALED_API bool IsPreviewPlatformEnabled() const;

	/** Return whether the preview platform enabled via Settings->Preview Rendering Level is currently active/displaying. */
	UNREALED_API bool IsPreviewPlatformActive() const;

	UE_DEPRECATED(5.8, "Use TogglePreviewPlatform instead")
	void ToggleFeatureLevelPreview() { TogglePreviewPlatform(); }
	UE_DEPRECATED(5.8, "Use IsPreviewPlatformEnabled instead")
	bool IsFeatureLevelPreviewEnabled() const { return IsPreviewPlatformEnabled(); }
	UE_DEPRECATED(5.8, "Use IsPreviewPlatformActive instead")
	bool IsFeatureLevelPreviewActive() const { return IsPreviewPlatformActive(); }
...
/** Return the delegate that is called when the preview feature level changes */
	UE_DEPRECATED(5.8, "OnPreviewFeatureLevelChanged is deprecated, use OnPreviewShaderPlatformChanged instead")
	...
	UE_DEPRECATED(5.8, "OnPreviewPlatformChanged is deprecated, use OnPreviewShaderPlatformChanged instead")
	FPreviewPlatformChanged& OnPreviewPlatformChanged() { return PreviewPlatformChanged; }
PRAGMA_ENABLE_DEPRECATION_WARNINGS

	/** Return the delegate that is called when the preview shader platform changes */
	FPreviewShaderPlatformChanged& OnPreviewShaderPlatformChanged() { return PreviewShaderPlatformChanged; }
...
private:
	...
	void BeginEditorSave(struct FEditorSaveContext& Context, struct FSlowTask* SlowTask);
	void EndEditorSave(struct FEditorSaveContext& Context, struct FSlowTask* SlowTask);
