...
UENUM()
enum class EGlobalTabSwitchingBehavior : uint8
{
...
class UEditorPerProjectUserSettings : public UObject
{
	...
	/** Defines global tab switching behavior for the editor */
	UPROPERTY(EditAnywhere, Config, Category = Behavior)
	EGlobalTabSwitchingBehavior TabSwitchingBehavior = EGlobalTabSwitchingBehavior::BrowserStyle;

	/** Whether to show or hide tab switching dialog when tab command is executed */
	UPROPERTY(EditAnywhere, Config, Category = Behavior)
	bool bShowTabSwitchingDialog = true;
	...
	/** If enabled, failed ensures will be displayed in the message log */
	UPROPERTY(EditAnywhere, config, Category = Ensures)
	uint32 bShowEnsuresInMessageLog : 1;
...
public:
	...
	/** The shader platforms friendly Name, or Empty Text if there is no preview platform or no Frienly Name */
	UPROPERTY(config)
	...
	/** Deprecated: renamed to bPreviewShaderPlatformActive */
	UPROPERTY(config, meta=(DeprecatedProperty, DeprecationMessage="bPreviewFeatureLevelActive has been renamed to bPreviewShaderPlatformActive."))
	bool bPreviewFeatureLevelActive = false;
