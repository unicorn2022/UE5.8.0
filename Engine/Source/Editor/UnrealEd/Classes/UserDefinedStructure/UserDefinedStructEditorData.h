...
struct FStructVariableDescription
{
	...
	/** Return true if it generates the same property. */
	UNREALED_API bool HasSameType(const FStructVariableDescription& Other) const;
...
private:
	...
	//Whether the MetaData was loaded from the package and saved in UUserDefinedStructEditorData.
	UPROPERTY()
	bool bMetaDataConverted = false;
