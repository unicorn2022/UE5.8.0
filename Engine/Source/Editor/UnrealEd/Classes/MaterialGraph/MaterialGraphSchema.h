...
class UMaterialGraphSchema : public UEdGraphSchema
{
	...
	/** Gathers Named Reroute Declaration actions from the given graph into the action list */
	UNREALED_API static void GetNamedRerouteActionsForGraph(FGraphActionListBuilderBase& OutAllActions, const UEdGraph* Graph);
	...
	/** Get the color for a connection based on its resolved shader value type (from material insights) */
	UNREALED_API static FLinearColor GetColorForConnectionType(UE::Shader::EValueType ConnectionType);
