...
private:
	 ...
	 */
	struct FPollable : public FRefCountedObject
	{
	public:
		enum EManualTrigger {};
	...
	bool bSaveGCDataForEvaluation = false;
...
public:
	...
	// ICookInfo interface
	UNREALED_API virtual UE::Cook::FInstigator GetInstigator(FName PackageName) const override;
	UNREALED_API virtual TArray<UE::Cook::FInstigator> GetInstigatorChain(FName PackageName) const override;
	UNREALED_API virtual UE::Cook::ECookType GetCookType() const override;
	UNREALED_API virtual UE::Cook::ECookingDLC GetCookingDLC() const override;
	UNREALED_API virtual FString GetDLCName() const override;
	UNREALED_API virtual FString GetBasedOnReleaseVersion() const override;
	UNREALED_API virtual FString GetCreateReleaseVersion() const override;
	UNREALED_API virtual UE::Cook::EProcessType GetProcessType() const override;
	UNREALED_API virtual UE::Cook::ECookValidationOptions GetCookValidationOptions() const override;
	UNREALED_API virtual bool IsIncremental() const override;
	UNREALED_API virtual TArray<const ITargetPlatform*> GetSessionPlatforms() const override;
	UNREALED_API virtual FString GetCookOutputFolder(const ITargetPlatform* TargetPlatform) const override;
	UNREALED_API virtual FString GetCookMetadataOutputFolder(const ITargetPlatform* TargetPlatform) const override;	
	UNREALED_API virtual const TSet<IPlugin*>* GetEnabledPlugins(const ITargetPlatform* TargetPlatform) const override;
	...
	UNREALED_API virtual UE::Cook::ICookSandbox* GetCookSandbox() override;
	...
	*/
	...
	/** Is the databuild using IoStore (cooked packages will be stored in iostore containers rather than pak files)? */
	UNREALED_API bool IsUsingIoStore() const;
	...
	UNREALED_API int32 NumLoadedPackages() const;
	...
	*/
	...
	/** Is the local CookOnTheFlyServer ONLY cooking a DLC plugin, ignoring all references outside the plugin path? */
	UNREALED_API bool IsCookingDLCPluginOnly() const;
...
private:
	...
	UE::Cook::FInstigator GetInstigator(FName PackageName, UE::Cook::EReachability Reachability) const;
	...
	void RegisterValidationCookArtifact();
	 ...
	 * incremental cook)
	 */
	void FinishPopulateShaderLibrary(const ITargetPlatform* TargetPlatform, FString const& Name, bool bInitializeReferenceTracking);
	/**
	 * Prune the named ShaderLibrary of any data no longer referenced by assets in the current cook or
	 * IncrementalCook oplog.
	 */
	void PruneShaderLibrary(const ITargetPlatform* TargetPlatform, const FString& Name);
	...
	void DumpShaderTypeStats(const ITargetPlatform* TargetPlatform, const FString& PlatformNameString);
	...
	bool IsForceNoFilterAssetsFromAssetRegistry();
	...
	*/
	...
	FString GetCookMetadataOutputFolderInternal(const ITargetPlatform* TargetPlatform) const;

	// Return the filename to use for the cook metadata file, adjusted for DLC, sandbox, and platform.
	FString GetCookedCookMetadataFilename(const ITargetPlatform* TargetPlatform);
	...
	UE::Cook::ICookArtifactReader& FindOrCreateCookArtifactReader(const ITargetPlatform* TargetPlatform);
	const UE::Cook::ICookArtifactReader* FindCookArtifactReader(const ITargetPlatform* TargetPlatform) const;
	...
	/** Clear temporary data that is allocated during GarbageCollect for use by AddReferencedObjects and PostGC evaluation. */
	void ClearGCData();
	...
	UE::Cook::FStatHistoryInt NumPackagesHistory;
	...
	bool bLegacyBuildDependenciesDueToNoZenStore = false;
	/**
	 * Automated DependencyTracking is only used in CookIncremental; legacyiterative does not use its data.
	 * Additionally, it can be turned off to improve performance in CookIncremental, at the cost of not being able to
	 * cook incrementally afterwards.
	 */
	bool bDependencyTrackingEnabled = true;
	...
	UE::Cook::FValidationCookArtifact* ValidationArtifact = nullptr;
...
/**
 * Public function for systems that track dependencies for the cooker and need to disable the tracking as early as
 * possible for performance when commandline or config settings disable CookIncremental dependency tracking. If this
 * function returns false, systems can turn off dependency tracking or reporting to the cooker.
 */
UNREALED_API bool IsDependencyTrackingEnabled();
