...
struct FLevelEditorViewportInstanceSettings
{
	...
	FLevelEditorViewportInstanceSettings()
		...
		, ColorChannelMask(INDEX_NONE)
		, bInvertAlphaChannelMask(true)
	...
	/** The MegaLights visualization mode for the viewport. */
	UPROPERTY(config)
	FName MegaLightsVisualizationMode;
	...
	/** The color channel to display for this viewport */
	UPROPERTY(config)
	int32 ColorChannelMask;

	/** Whether to invert the alpha channel when displaying it */
	UPROPERTY(config)
	bool bInvertAlphaChannelMask;

	/** Whether to display an alpha-blended checkerboard beneath the viewport to help visualize alpha in the viewport */
	UPROPERTY(config)
...
public:
	...
	/** If disabled, any and all snapping is disabled */
	UPROPERTY(EditAnywhere, config, AdvancedDisplay, Category=GridSnapping)
	bool bSnappingActive = true;

	/** If enabled will use power of 2 grid settings (e.g, 1,2,4,8,16,...,1024) instead of decimal grid sizes */
	UPROPERTY(EditAnywhere, config, Category=GridSnapping, meta=(DisplayName = "Use Power of Two Snap Size", EditCondition="bSnappingActive"))
	bool bUsePowerOf2SnapSize;

	/** Decimal grid sizes (for translation snapping and grid rendering) */
	UPROPERTY(EditAnywhere, config, AdvancedDisplay, Category=GridSnapping, meta=(EditCondition="bSnappingActive"))
	...
	UPROPERTY(EditAnywhere, config, Category=GridSnapping, meta=(DisplayName = "Enable Grid Snapping", EditCondition="bSnappingActive"))
	uint32 GridEnabled:1;
	
	/** If enabled, actor rotations will snap to the grid. */
	UPROPERTY(EditAnywhere, config, Category=GridSnapping, meta=(DisplayName = "Enable Rotation Snapping", EditCondition="bSnappingActive"))
	uint32 RotGridEnabled:1;

	/** If enabled, actor sizes will snap to the grid. */
	UPROPERTY(EditAnywhere, config, Category=GridSnapping, meta=(DisplayName = "Enable Scale Snapping", EditCondition="bSnappingActive"))
...
public:
	...
	UPROPERTY(EditAnywhere, config, Category=GridSnapping, meta=(DisplayName = "Enable 2D Layer Snapping", EditCondition="bSnappingActive"))
	...
	UPROPERTY(config, Category=GridSnapping, VisibleDefaultsOnly,AdvancedDisplay, meta=(EditCondition="bSnappingActive"))
...
public:
	...
	/** When enabled, the channel mask menu will show up in the editor toolbar next to the View Modes button */
	UPROPERTY(config)
	uint32 bShowChannelsInToolbar:1;
