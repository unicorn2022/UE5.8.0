...
public:
	...
	/** If enabled, the copy properties option will be enabled by default when using the replace actors context menu. */
	UPROPERTY(EditAnywhere, config, Category=Editing, meta=( DisplayName = "Copy Properties on Replace" ))
	uint32 bReplaceCopyPropertiesDefault:1;
	
	/** If enabled, replacing actors allows copying properties when related by the native parent of the replaced class. */
	UPROPERTY(EditAnywhere, config, Category=Editing, meta=( DisplayName = "Copy Properties when Replacing with Different Blueprint" ))
	uint32 bReplaceCopiesPropertiesWhenRelatedByNativeParent:1;
