...
namespace ActorPositioningLocals
{
	...
	static void AddIgnoredActors(FCollisionQueryParams& OutParams, const TArray<AActor*>* IgnoreActors)
	{
		if (!IgnoreActors)
		{
			return;
		...
		for (AActor* IgnoreActor : *IgnoreActors)
		{
			...
			FTypedElementHandle Element = UEngineElementsLibrary::AcquireEditorActorElementHandle(IgnoreActor);
			TTypedElement<ITypedElementWorldInterface> WorldElement = Registry->GetElement<ITypedElementWorldInterface>(Element);
			if (WorldElement)
			{
...
FActorPositionTraceResult FActorPositioning::TraceWorldForPositionWithDefault(const FViewportCursorLocation& Cursor, const FSceneView& View, const TArray<AActor*>* IgnoreActors)
{
	...
	ActorPositioningLocals::AddIgnoredActors(Param, IgnoreActors);
...
FTransform FActorPositioning::GetSurfaceAlignedTransform(const FPositioningData& Data)
{
	...
	{
		// Try to find a default alignment if we don't have a factory.
		RotatorQuat = FindActorAlignmentRotation(RotatorQuat, FVector::UpVector, Data.SurfaceNormal);
	}
