...
FEditorModeTools::FEditorModeTools()
	...
	, DefaultModeIDs({ FAssetEdModes::EM_AssetDefault })
	...
	, bIsTracking(false)
{
	...
	if( GEditor )
	{
		...
		GEditor->OnViewportClientListChanged().AddRaw(this, &FEditorModeTools::OnViewportListChanged);
	}
...
FEditorModeTools::~FEditorModeTools()
{
	SetDefaultMode( FAssetEdModes::EM_AssetDefault );
	DeactivateAllModes();
...
void FEditorModeTools::LoadConfig(void)
{
	...
	if (CoordinateSystem == COORD_Parent || CoordinateSystem == COORD_Explicit)
	{
		...
		if (!(bViewportITF && UEditorInteractiveGizmoManager::UsesNewTRSGizmos()))
		{
...
void FEditorModeTools::SetToolkitHost(TSharedRef<class IToolkitHost> InHost)
{
	...
	// Remove the existing context, if it exists
	InteractiveToolsContext->ContextObjectStore->RemoveContextObjectsOfType<UAssetEditorContextObject>();
...
EEditAction::Type FEditorModeTools::GetActionDragDuplicate()
{
	EEditAction::Type ReturnedAction = EEditAction::Skip;
	ForEachEdMode([&ReturnedAction](UEdMode* Mode)
		{
			const EEditAction::Type EditAction = Mode->GetActionDragDuplicate();
			if (EditAction == EEditAction::Process || EditAction == EEditAction::Halt)
			{
				ReturnedAction = EditAction;
				return false;
...
void FEditorModeTools::ExitAllModesPendingDeactivate()
{
	...
	// Loop so that exiting one mode can queue the exit of another and have that be processed in the same frame.
	constexpr int32 MaxCycles = 32; 
	int32 RemainingCycles = MaxCycles;
	while (PendingDeactivateModes.Num() && RemainingCycles > 0)
	{
		...
		{
			ExitMode(Pair.Value);
			PendingDeactivateModes.Remove(Pair.Key);
		}
	...
	if (PendingDeactivateModes.Num() > 0)
	{
		UE_LOGF(LogEditorModes, Error, "Flushing PendingDeactivateModes had %i items in the list after %i cycles. There may be a loop where modes remove each other.", PendingDeactivateModes.Num(), MaxCycles);
		for (const auto& Pair : PendingDeactivateModes)
		{
			FString ObjectName;
			if (Pair.Value)
			{
				Pair.Value->GetName(ObjectName);
			}
			UE_LOGF(LogEditorModes, Log, "  Mode Id: %ls; Mode: %ls", *Pair.Key.ToString(), *ObjectName);
		}
	}
}
...
void FEditorModeTools::OnViewportListChanged()
{
	bool bFoundHovered = false;
	...
	if (GEditor)
	{
		...
		{
			if (Client == HoveredViewportClient)
			{
				bFoundHovered = true;
			}
			if (Client == FocusedViewportClient)
			{
				bFoundFocused = true;
			}
		}
	...
	// If the hovered or focused clients are not found in the list, they have been removed.
	// Their pointers are now pointing at garbage and need to be reset.
	if (!bFoundHovered)
	{
...
void FEditorModeTools::RemoveAllDelegateHandlers()
{
	if (GEditor)
	{
		...
		GEditor->OnViewportClientListChanged().RemoveAll(this);
	}
...
void FEditorModeTools::DestroyMode( FEditorModeID InID )
{
	...
	{
		AddDefaultMode( FAssetEdModes::EM_AssetDefault );
	}
...
void FEditorModeTools::ActivateMode(FEditorModeID InID, bool bToggle)
{
	...
	if (!ScriptableMode)
	{
		UE_LOGF(LogEditorModes, Log, "FEditorModeTools::ActivateMode : Couldn't find mode '%ls'.", *InID.ToString());
...
void FEditorModeTools::PropagateInvalidations( FEditorViewportClient* ViewportClient )
{
	if (InteractiveToolsContext)
	{
		InteractiveToolsContext->PropagateInvalidations(ViewportClient);
	}
}
...
bool FEditorModeTools::RequiresLegacyViewportInteractions() const
{
	bool bRequiresGizmos = false;
	ForEachEdMode([&bRequiresGizmos, this](UEdMode* EdMode)
		{
			if (ILegacyEdModeViewportInterface* Interface = Cast<ILegacyEdModeViewportInterface>(EdMode))
			{
				if (Interface->RequiresLegacyViewportInteractions())
				{
					if (bViewportITF)
					{
						UE_LOGF(LogEditorModes, Warning, "ITF Gizmos and Controls have been disabled. The \"%ls\" Editor Mode requires the legacy systems.", *EdMode->GetModeInfo().Name.ToString())
					}
					
					bRequiresGizmos = true;
				}
			}
			return true;
		});
	return bRequiresGizmos;	
}
...
{
	return GetCursorVisibilityOverride(bWantsOverride, bHardwareCursorVisible, bSoftwareCursorVisible);
}

bool FEditorModeTools::GetCursorVisibilityOverride(bool& bOutWantsOverride, bool& bOutHardwareCursorVisible, bool& bOutSoftwareCursorVisible) const
{
	if (CVarEnableITFCursorOverrideSupport.GetValueOnGameThread() > 0)
	{
		if (InteractiveToolsContext->GetCursorVisibilityOverride(bOutHardwareCursorVisible, bOutSoftwareCursorVisible))
		{
			bOutWantsOverride = true;
			return true;
		}
	}
	...
	for (const UEdMode* Mode : ActiveScriptableModes)
	{
		bHandled |= Mode->GetOverrideCursorVisibility(bOutWantsOverride, bOutHardwareCursorVisible, bOutSoftwareCursorVisible);
	}
	return bHandled;
}
...
bool FEditorModeTools::UsesViewportITF() const
{
	return bViewportITF && InteractiveToolsContext && InteractiveToolsContext->UsesViewportInteractions() && UEditorInteractiveGizmoManager::UsesNewTRSGizmos();
}

void FEditorModeTools::SetSupportsViewportITF(bool bIsSupported)
{
	bViewportITF = bIsSupported;
}

bool FEditorModeTools::SupportsViewportITF() const
{
	return bViewportITF;
}
...
bool FEditorModeTools::OnTransformCanceled(const FGizmoState& InState) const
{
	bool bHandled = false;
	ForEachEdMode<IGizmoEdModeInterface>([&bHandled, &InState](IGizmoEdModeInterface* GizmoInterface)
	{
		bHandled |= GizmoInterface->OnTransformCanceled(InState);
		return true;
	});
	...
	if (!bHandled && FocusedViewportClient)
	{
		bHandled = FocusedViewportClient->OnTransformCanceled(InState);
	}
