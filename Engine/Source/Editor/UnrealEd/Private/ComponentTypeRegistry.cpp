...
void FComponentTypeRegistryData::ForceRefreshComponentList()
{
	...
	for (TObjectIterator<UClass> It; It; ++It)
	{
		...
		if (Class->IsChildOf(UActorComponent::StaticClass()))
		{
			...
			const bool bPackageForDiffing = Class->GetPackage()->HasAnyPackageFlags(PKG_ForDiffing);

			if (!bOutOfDateClass &&	!bBlueprintSkeletonClass && !bPackageForDiffing)
			{
	...
	// Add unloaded component classes
	{
		...
		for (FAssetData& BPAsset : Assets)
		{
			...
			if (!ClassPath.IsNull())
			{
				if (!InMemoryClassPaths.Contains(ClassPath)
					...
					&& !BPAsset.HasAnyPackageFlags(PKG_ForDiffing))
				{
			...
			{
				UE_LOGF(LogBlueprint, Warning, "Blueprint %ls is missing %ls asset tag", *BPAsset.PackageName.ToString(), *FBlueprintTags::GeneratedClassPath.ToString());
			}
		...
		for (FAssetData& BPGCAsset : Assets)
		{
			...
			if (ActorComponentDerivedClassNames.Contains(BPGCAssetClassPathName) && !InMemoryClassPaths.Contains(BPGCAssetClassPathName)
				&& !BPGCAsset.HasAnyPackageFlags(PKG_ForDiffing))
			{
