	...
	for (ULevel* LevelIter : Levels)
	{
		...
		if (LevelIter->Actors.Num() > 0)
		{
			...
			if (NumLODActors)
			{
				...
				if(NumLODLevels > 0)
				{
					UE_LOGF(LogLODGenerator, Log, "Building HLOD meshes for %ls", *LevelIter->GetOutermost()->GetName());
				}
				...
				if (bUseCustomTransformForHLODBaking)
				{
					...
					for (int32 LODIndex = 0; LODIndex < NumLODLevels; ++LODIndex)
					{
						...
						for (ALODActor* LODActor : LODLevelActors[LODIndex])
						{
							...
							if (LODLevelSettings.SimplificationMethod != EHierarchicalSimplificationMethod::Merge)
							{
							...
							{
								switch (LODLevelSettings.MergeSetting.PivotType)
								{
								...
								case EMeshMergePivotType::Custom:
									LODActor->SetActorTransform(FTransform(LODLevelSettings.MergeSetting.CustomPivotLocation));
									FStaticMeshOperations::ApplyTransform(*SMDesc, TransformInv);
									break;
		...
		// merge clusters first
		{
			...
			while (!bStable)
			{
				...
				for (int32 ClusterId = 0; ClusterId < NumClusters; ++ClusterId)
				{
					...
					UE_LOGF(LogLODGenerator, Verbose, "%d. %0.2f {%ls}", ClusterId + 1, Cluster.GetCost(), *Cluster.ToString());
					...
					if (Cluster.IsValid())
					{
						...
						UE_LOGF(LogLODGenerator, Verbose, "Processed(%ls): %0.2f {%ls}", Cluster.IsValid() ? TEXT("Valid") : TEXT("Invalid"), Cluster.GetCost(), *Cluster.ToString());
					}
				}
