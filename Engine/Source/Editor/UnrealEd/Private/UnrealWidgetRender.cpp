...
struct FSpaceDescriptor
{
	...
	*/
	FSpaceDescriptor(const FSceneView* View, const FEditorViewportClient* Viewport, const FVector& InLocation, const float ExternalScale) :
		bIsPerspective(View->ViewMatrices.GetViewToClip().M[3][3] < 1.0f),
		bIsLocalSpace(Viewport->GetWidgetCoordSystemSpace() == COORD_Local),
		bIsOrthoXY(!bIsPerspective && FMath::Abs(View->ViewMatrices.GetWorldToView().M[2][2]) > 0.0f),
		bIsOrthoXZ(!bIsPerspective && FMath::Abs(View->ViewMatrices.GetWorldToView().M[1][2]) > 0.0f),
		bIsOrthoYZ(!bIsPerspective && FMath::Abs(View->ViewMatrices.GetWorldToView().M[0][2]) > 0.0f),
		UniformScale(ExternalScale * View->WorldToScreen(InLocation).W * (4.0f / View->UnscaledViewRect.Width() / View->ViewMatrices.GetViewToClip().M[0][0])),
		Scale(CreateScale())
	{
	}
...
void FWidget::Render( const FSceneView* View,FPrimitiveDrawInterface* PDI, FEditorViewportClient* ViewportClient )
{
	...
	if (!bUseLegacyWidget)
	{
		bUseLegacyWidget = !UEditorInteractiveGizmoManager::AreEditorGizmosAllowed(ViewportClient->GetModeTools());
	}
...
void FWidget::Render_Axis(const FSceneView* View, FPrimitiveDrawInterface* PDI, EAxisList::Type InAxis, FMatrix& InMatrix, UMaterialInterface* InMaterial, const FLinearColor& InColor, FVector2D& OutAxisDir, const FVector& InScale, bool bDrawWidget, bool bCubeHead, float AxisLengthOffset)
{
	...
	if( bDrawWidget )
	{
		const bool bDisabled = EditorModeTools ? (EditorModeTools->IsDefaultModeActive() && GEditor->HasSelectedMovementLockedItems() ) : false;
...
void FWidget::Render_Translate( const FSceneView* View, FPrimitiveDrawInterface* PDI, FEditorViewportClient* ViewportClient, const FVector& InLocation, bool bDrawWidget )
{
	...
	if( bDrawWidget && ( DrawAxis & EAxisList::Screen ) && Space.bIsPerspective )
	{
		...
		const FVector CameraXAxis = View->ViewMatrices.GetWorldToView().GetColumn(0);
		const FVector CameraYAxis = View->ViewMatrices.GetWorldToView().GetColumn(1);
		const FVector CameraZAxis = View->ViewMatrices.GetWorldToView().GetColumn(2);
...
{
	float Scale = View->WorldToScreen(InLocation).W * (4.0f / View->UnscaledViewRect.Width() / View->ViewMatrices.GetViewToClip().M[0][0]);
	const float ExternalScale = EditorModeTools ? EditorModeTools->GetWidgetScale() : 1.0f;
	Scale *= ExternalScale;
	//get the axes 
...
void FWidget::Render_TranslateRotateZ( const FSceneView* View, FPrimitiveDrawInterface* PDI, FEditorViewportClient* ViewportClient, const FVector& InLocation, bool bDrawWidget )
{
	...
	{
		// Draw the axis lines with arrow heads
		if( DrawAxis&EAxisList::X && (Space.bIsPerspective || Space.bIsLocalSpace || View->ViewMatrices.GetWorldToView().M[0][2] != -1.f) )
		{
			Render_Axis( View, PDI, EAxisList::X, AxisMatrix, XMaterial, XColor, XAxisDir, Space.Scale, bDrawWidget );
		}

		if( DrawAxis&EAxisList::Y && (Space.bIsPerspective || Space.bIsLocalSpace || View->ViewMatrices.GetWorldToView().M[1][2] != -1.f) )
		{
			Render_Axis( View, PDI, EAxisList::Y, AxisMatrix, YMaterial, YColor, YAxisDir, Space.Scale, bDrawWidget );
		}

		if( DrawAxis&EAxisList::Z && (Space.bIsPerspective || Space.bIsLocalSpace || View->ViewMatrices.GetWorldToView().M[0][1] != 1.f) )
		{
		...
		//ZRotation
		if( DrawAxis&EAxisList::ZRotation && (Space.bIsPerspective || Space.bIsLocalSpace || View->ViewMatrices.GetWorldToView().M[0][2] != -1.f) )
		{
		...
		//XY Plane
		if( Space.bIsPerspective || Space.bIsLocalSpace || View->ViewMatrices.GetWorldToView().M[0][1] != 1.f )
		{
