...
TSharedRef<SToolTip> FEditorClassUtils::GetTooltip(const UClass* Class)
{
	...
	if (Class)
	{
		const bool bShowShortTooltips = GetDefault<UBlueprintEditorSettings>()->bShowShortTooltips;
		if (const UVerseClass* VerseClass = Cast<const UVerseClass>(Class))
		{
			...
			if ((bShowShortTooltips && !VerseClass->GetMetaData(NAME_ShortTooltip).IsEmpty()) || !VerseClass->GetMetaData(NAME_Tooltip).IsEmpty())
			{
				ToolTipText = Class->GetToolTipText(bShowShortTooltips);
			}
			else
			{
				// If the tooltip is empty, GetToolTipText will use the class FName, but that is the mangled package relative path, so instead use the full Verse path for Verse classes.
				ToolTipText = VerseClass->GetVersePath().AsText();
			}
		}
		else
		{
	...
	return GetTooltip(Class, ToolTipText);
}
...
TSharedRef<SWidget> FEditorClassUtils::GetSourceLink(const UClass* Class, const FSourceLinkParams& Params)
{
	...
	else if (Class)
	{
		...
		const FText ClassDisplayNameText = Class->GetDisplayNameText();
		const FText ClassNameText = FText::FromName(Class->GetFName());

		FText FormattedText;
		if (Params.CodeFormat)
		{
			FormattedText = FText::Format(*Params.CodeFormat, ClassDisplayNameText);
		}
		else if (Params.bUseDefaultFormat)
		{
			const FText DefaultCodeFormat = NSLOCTEXT("SourceHyperlink", "GoToCode", "Open {0}");
			FormattedText = FText::Format(DefaultCodeFormat, ClassDisplayNameText);
		}
		else
		{
			FormattedText = ClassDisplayNameText;
		}
		...
		{
			NoLinkWidget = SNew(STextBlock)
				.Text(Params.bUseFormatIfNoLink ? FormattedText : ClassDisplayNameText)
				.ColorAndOpacity(FSlateColor::UseSubduedForeground())
				.Visibility_Lambda([CanNavigateToClassCode]() { return CanNavigateToClassCode() ? EVisibility::Collapsed : EVisibility::Visible; });
		}
		...
		Link = SNew(SHorizontalBox)
			...
			[
				...
				.Text(FormattedText)
				.ToolTipText(FText::Format(NSLOCTEXT("SourceHyperlink", "GoToCode_ToolTip", "Click to open the source file for the underlying class ({0}) in {1}"), 
					ClassNameText, FSourceCodeNavigation::GetSelectedSourceCodeIDE()))
