...
FDelegateHandle FEditorViewportSelectability::RegisterObjectSelectablePredicate(const FOnIsObjectSelectableInViewport& InPredicate)
{
	...
	const FDelegateHandle NewHandle(FDelegateHandle::GenerateNewHandle);

	GetAdditionalObjectSelectablePredicates().Add(NewHandle, InPredicate);

	return NewHandle;
}

void FEditorViewportSelectability::UnregisterObjectSelectablePredicate(const FDelegateHandle InHandle)
{
	if (!InHandle.IsValid())
	{
		return;
	...
	GetAdditionalObjectSelectablePredicates().Remove(InHandle);
}

bool FEditorViewportSelectability::PassesAdditionalObjectSelectablePredicates(UObject* const InObject)
{
	for (const TPair<FDelegateHandle, FOnIsObjectSelectableInViewport>& Pair : GetAdditionalObjectSelectablePredicates())
	{
		if (Pair.Value.IsBound() && !Pair.Value.Execute(InObject))
		{
			return false;
		}
	}
...
bool FEditorViewportSelectability::PassesAdditionalObjectSelectablePredicates(HHitProxy* const InHitProxy)
{
	const HActor* const ActorHitProxy = HitProxyCast<HActor>(InHitProxy);
	if (!ActorHitProxy)
	{
		return true;
	}
	...
	{
		return PassesAdditionalObjectSelectablePredicates(ActorHitProxy->Actor);
	}

	if (ActorHitProxy->PrimComponent)
	{
		return PassesAdditionalObjectSelectablePredicates(const_cast<UPrimitiveComponent*>(ActorHitProxy->PrimComponent.Get()));
	}

	return true;
}

TMap<FDelegateHandle, FOnIsObjectSelectableInViewport>& FEditorViewportSelectability::GetAdditionalObjectSelectablePredicates()
{
	static TMap<FDelegateHandle, FOnIsObjectSelectableInViewport> AdditionalPredicates;
...
bool FEditorViewportSelectability::IsObjectSelectableInViewport(UObject* const InObject) const
{
	if (OnIsObjectSelectableInViewportDelegate.IsBound()
		&& !OnIsObjectSelectableInViewportDelegate.Execute(InObject))
	{
		return false;
	}

	return PassesAdditionalObjectSelectablePredicates(InObject);
}
...
void FEditorViewportSelectability::UpdateHoverFromHitProxy(HHitProxy* const InHitProxy)
{
	...
	bool bIsSelectableActorHit = false;
	...
	if (InHitProxy)
	{
		...
		else if (InHitProxy->IsA(HActor::StaticGetType()))
		{
			...
			{
				ActorHit = ActorHitProxy->Actor;

				if (bSelectionLimited)
				{
					bIsSelectableActorHit = IsObjectSelectableInViewport(ActorHitProxy->Actor);
				}
			}
		}
	}

	UpdateHoveredActorPrimitives(ActorHit);
...
bool FEditorViewportSelectability::IsTypedElementSelectable(const FTypedElementHandle& InElementHandle) const
{
	...
	TTypedElement<ITypedElementObjectInterface> const ObjectInterface = TypedElementRegistry->GetElement<ITypedElementObjectInterface>(InElementHandle);
	if (!ObjectInterface)
	{
		return false;
	}

	UPrimitiveComponent* const PrimitiveComponent = ObjectInterface.GetObjectAs<UPrimitiveComponent>();
	if (!IsValid(PrimitiveComponent))
	{
		return false;
	}
