...
#include "Misc/ITransaction.h" // GUndo
...
namespace UE::EditorActor::Private
{
	using namespace UE::Editor::DataStorage;
	using namespace UE::Editor::DataStorage::Queries;

	/** 
	 * Takes a query that provides readonly world and visibility columns, and sets the visibility on the returned columns 
	 *  if the world matches. Handles undo if a transaction is open.
	 */ 
	void RunSetVisibilityQuery(ICoreProvider& DataStorage, QueryHandle Query, UWorld* WorldIn, bool bVisible)
	{
		// We'll gather the rows to change in one pass and actually change them in another, to
		//  avoid trying to add a tag (FVisibilityChangedTag) from within a callback 
		FRowHandleArray RowsToChange;
		DataStorage.RunQuery(Query, CreateDirectQueryCallbackBinding(
			[WorldIn, bVisible, &RowsToChange](RowHandle Row, const FTypedElementWorldColumn& WorldColumn, const FVisibleInEditorColumn& VisibilityColumn)
		{
		...
		// Apply the change
		for (RowHandle Row : RowsToChange.GetRows())
		{
			FVisibleInEditorColumn* VisibilityColumn = DataStorage.GetColumn<FVisibleInEditorColumn>(Row);
			if (ensure(VisibilityColumn))
			{
		...
		// Emit an undo change if a transaction is open
		if (GUndo && WorldIn)
		{
			GUndo->StoreUndo(WorldIn, MakeUnique<FVisibilityColumnChange>(MoveTemp(RowsToChange), bVisible));
		}
	}

	/**
	 * Walks across editor object hierarchies for all selected items and calls VisitFn
	 */
	void WalkSelectedHierarchies(ICoreProvider& DataStorage, UWorld* WorldIn, 
		TFunction<void(const ICoreProvider& Context, RowHandle Owner, RowHandle Target)> VisitFn)
	{
		// Gather selected rows with matching world
		QueryHandle Query = DataStorage.RegisterQuery(
			...
			.Compile());
		ON_SCOPE_EXIT{ DataStorage.UnregisterQuery(Query); };

		TArray<RowHandle> SelectedRows;
		DataStorage.RunQuery(Query, CreateDirectQueryCallbackBinding(
			[WorldIn, &DataStorage, &SelectedRows](RowHandle Row, const FTypedElementWorldColumn& WorldColumn)
			{
				if (WorldColumn.World.Get() != WorldIn)
				{
					return;
		...
		// TODO: This should match UTedsEditorHierarchyFactory::EditorObjectHierarchyName when that is not experimental
		FName EditorObjectHierarchyName("EditorObjectHierarchy");
		FHierarchyHandle Hierarchy = DataStorage.FindHierarchyByName(EditorObjectHierarchyName);
		if (!ensure(DataStorage.IsValidHierarchyHandle(Hierarchy)))
		{
			return;
		}

		// Walk the hierarchies for the selected rows
		for (RowHandle Row : SelectedRows)
		{
			DataStorage.WalkDepthFirst(Hierarchy, Row, VisitFn);
		}
	}

	void SetSelectedNonActorVisibility(UWorld* WorldIn, bool bVisible, bool bApplyToHierarchy)
	{
		...
		{
			// This is the simpler case- just change the selected items.

			// Query gets world and visibility columns for all selected non-actors
			QueryHandle Query = DataStorage->RegisterQuery(
				Select()
			...
			RunSetVisibilityQuery(*DataStorage, Query, WorldIn, bVisible);
			return;
		}

		// If we got here, we're processing the hierarchy.

		FRowHandleArray RowsToChange;
		WalkSelectedHierarchies(*DataStorage, WorldIn, [&RowsToChange, bVisible](const ICoreProvider& Context, RowHandle Owner, RowHandle Target)
		{
			if (const FVisibleInEditorColumn* VisibilityColumn = Context.GetColumn<FVisibleInEditorColumn>(Target))
			{
				if (VisibilityColumn->bIsVisibleInEditor != bVisible)
				{
					RowsToChange.Add(Target);
				}
			}
		});
	...
	void SetAllNonActorVisiblity(UWorld* WorldIn, bool bVisible)
	{
		ICoreProvider* DataStorage = GetMutableDataStorageFeature<ICoreProvider>(StorageFeatureName);
		if (!DataStorage)
		{
	...
	void SetUnselectedNonActorVisibility(UWorld* WorldIn, bool bVisible)
	{
		ICoreProvider* DataStorage = GetMutableDataStorageFeature<ICoreProvider>(StorageFeatureName);
		if (!DataStorage)
		{
		...
		// Note that it is insufficient to just set the visibility of unselected items, even
		//  if the selected ones are already visible, because in some cases the visibility handling
		//  needs to propagate the visibility to some children that might otherwise remain invisible,
		//  as they are not directly selected. For this reason, we go with this two step approach.

		SetAllNonActorVisiblity(WorldIn, false);
		SetSelectedNonActorVisibility(WorldIn, true, 
			// This seems to be the current behavior for actors, so we match it
			/*bApplyToHierarchy*/ false);
	}

	bool HaveAnySelectedVisibleNonActor(UWorld* WorldIn, bool bIncludeHierarchy)
	{
		...
		if (!bIncludeHierarchy)
		{
			...
			DataStorage->RunQuery(Query, CreateDirectQueryCallbackBinding(
				[WorldIn, &bHaveVisible](RowHandle Row, const FTypedElementWorldColumn& WorldColumn, const FVisibleInEditorColumn& VisibilityColumn)
			{
				if (WorldColumn.World.Get() != WorldIn)
				{
					return;
				...
				bHaveVisible = bHaveVisible || VisibilityColumn.bIsVisibleInEditor;
			}));
		}
		else
		{
			WalkSelectedHierarchies(*DataStorage, WorldIn, [&bHaveVisible](const ICoreProvider& Context, RowHandle Owner, RowHandle Target)
			{
				if (const FVisibleInEditorColumn* VisibilityColumn = Context.GetColumn<FVisibleInEditorColumn>(Target))
				{
					bHaveVisible = bHaveVisible || VisibilityColumn->bIsVisibleInEditor;
...
void UUnrealEdEngine::PasteActors(TArray<AActor*>& OutPastedActors, UWorld* InWorld, const FVector& LocationOffset, bool bDuplicate, bool bWarnIfHidden, const FString* SourceData)
{
	...
	Factory->LevelFactoryCreateText(ULevel::StaticClass(), InWorld->GetCurrentLevel(), InWorld->GetCurrentLevel()->GetFName(), RF_Transactional, NULL, bDuplicate ? TEXT("move") : TEXT("paste"), Paste, Paste + FCString::Strlen(Paste), GWarn, &OutPastedActors);
	...
	// FactoryCreateText sets the selection to the new actors, so restore the original selection
...
public:
	...
	void DuplicateActorsToLevel(TArray<AActor*>& OutNewActors, ULevel* DestLevel, const FVector& LocationOffset)
	{
		...
		{
			UE_LOGF(LogEditorActor, Warning, "DuplicateActorsToLevel: The requested operation could not be completed because the level is locked.");
			return;
		}
...
bool UUnrealEdEngine::CanDeleteSelectedActors( const UWorld* InWorld, const bool bStopAtFirst, const bool bLogUndeletable, TArray<AActor*>* OutDeletableActors ) const
{
	...
	for ( FSelectionIterator It( GetSelectedActorIterator() ) ; It ; ++It )
	{
		...
		else if ( bLogUndeletable )
		{
			...
			UE_LOGF(LogEditorActor, Log, "%ls", *LogText.ToString());
		}
	}
...
bool UUnrealEdEngine::DeleteComponents(const TArray<UActorComponent*>& InComponentsToDelete, UTypedElementSelectionSet* InSelectionSet, const bool bVerifyDeletionCanHappen)
{
	...
	for (UActorComponent* ComponentToDelete : InComponentsToDelete)
	{
		...
		{
			UE_LOGF(LogEditorActor, Log, "Cannot delete component %ls: %ls", *ComponentToDelete->GetFullName(), *ErrorMsg.ToString());
			continue;
		}
	...
	UE_LOGF(LogEditorActor, Log, "Deleted %d Components (%3.3f secs)", NumDeletedComponents, FPlatformTime::Seconds() - StartSeconds);
...
bool UUnrealEdEngine::DeleteActors(const TArray<AActor*>& InActorsToDelete, UWorld* InWorld, UTypedElementSelectionSet* InSelectionSet, const bool bVerifyDeletionCanHappen, const bool bWarnAboutReferences, const bool bWarnAboutSoftReferences)
{
	...
	for (AActor* ActorToDelete : InActorsToDelete)
	{
		...
		if (!CanDeleteActor(ActorToDelete, &ErrorMsg))
		{
			...
			{
				UE_LOGF(LogEditorActor, Log, "Cannot delete actor %ls: %ls", *ActorToDelete->GetFullName(), *ErrorMsg.ToString());
			}
			continue;
		}
	...
	for (AActor* Actor : ActorsToDelete)
	{
			...
			for (AActor* ReferencingActor : *ReferencingActors)
			{
				...
				else
				{
					...
					if (ParentComponent == nullptr || ParentComponent->GetOwner() != Actor)
					{
						...
						UE_LOGF(LogEditorActor, Log, "%ls", *ActorReferencedMessage.ToString());
					}
				}
			}
		}
			...
			if (bReferencedByActor || bReferencedByLODActor || bReferencedByGroupActor)
			{
				...
				for (AActor* ReferencingActor : *ReferencingActors)
				{
					...
					if (LODActor)
					{
						...
						UE_LOGF(LogEditorActor, Log, "%ls", *SubActorRemovedMessage.ToString());
					}
				}
			}
		}
		...
		UE_LOGF(LogEditorActor, Log, "Deleted Actor: %ls", *Actor->GetClass()->GetName());
	...
	UE_LOGF(LogEditorActor, Log, "Deleted %d Actors (%3.3f secs)", DeleteCount, (FPlatformTime::Seconds() - StartSeconds) - DialogWaitingSeconds);
	return true;
}
...
void UUnrealEdEngine::Internal_HideSelected( UWorld* InWorld, bool bApplyToChildHierarchy )
{
	...
	// Hide selected non-actor elements via TEDS
	UE::EditorActor::Private::SetSelectedNonActorVisibility(InWorld, false, bApplyToChildHierarchy);
...
void UUnrealEdEngine::edactHideUnselected( UWorld* InWorld )
{
	...
	// Hide unselected non-actor elements via TEDS
	UE::EditorActor::Private::SetUnselectedNonActorVisibility(InWorld, false);
...
void UUnrealEdEngine::Internal_ToggleSelectedVisibility(UWorld* InWorld, bool bApplyToChildHierarchy)
{
	...
	// Check non-actor element visibility to include in the toggle decision
	if (bTargetVisibility)
	{
		bTargetVisibility = !UE::EditorActor::Private::HaveAnySelectedVisibleNonActor(InWorld, bApplyToChildHierarchy);
	}
	...
	// Apply the target visibility to non-actor elements
	UE::EditorActor::Private::SetSelectedNonActorVisibility(InWorld, bTargetVisibility, bApplyToChildHierarchy);
...
void UUnrealEdEngine::edactUnHideAll( UWorld* InWorld )
{
	...
	// Unhide all non-actor elements via TEDS
	UE::EditorActor::Private::SetAllNonActorVisiblity(InWorld, true);
...
void UUnrealEdEngine::Internal_UnhideSelected(UWorld* InWorld, bool bApplyToChildHierarchy)
{
	...
	// Show selected non-actor elements via TEDS
	UE::EditorActor::Private::SetSelectedNonActorVisibility(InWorld, true, bApplyToChildHierarchy);
...
void UUnrealEdEngine::edactSelectMatchingStaticMesh( bool bAllClasses )
{
	...
	{
		UE_LOGF(LogEditorActor, Log, "No worlds found in edactSelectMatchingStaticMesh" );
		return;
	}
...
void UUnrealEdEngine::edactSelectMatchingSkeletalMesh(bool bAllClasses)
{
	...
	if( SelectedWorlds.Num() == 0 )
	{
		UE_LOGF(LogEditorActor, Log, "No worlds found in edactSelectMatchingSkeletalMesh" );
		return;
	}
	// Make sure we have only 1 valid world 
	check( SelectedWorlds.Num() == 1 );
...
void UUnrealEdEngine::edactSelectMatchingMaterial()
{
	...
	if( SelectedWorlds.Num() == 0 )
	{
		UE_LOGF(LogEditorActor, Log, "No worlds found in edactSelectMatchingMaterial" );
		return;
	}
	// Make sure we have only 1 valid world 
	check( SelectedWorlds.Num() == 1 );
...
void UUnrealEdEngine::edactSelectMatchingEmitter()
{
	...
	if( SelectedWorlds.Num() == 0 )
	{
		UE_LOGF(LogEditorActor, Log, "No worlds found in edactSelectMatchingEmitter" );
		return;
	}
	// Make sure we have only 1 valid world 
	check( SelectedWorlds.Num() == 1 );
...
void UUnrealEdEngine::edactSelectRelevantLights( UWorld* InWorld )
{
	...
	UE_LOGF(LogEditorActor, Log, "Found %d relevant lights!", RelevantLightList.Num());
	for (int32 LightIdx = 0; LightIdx < RelevantLightList.Num(); LightIdx++)
	{
		...
		if (Light)
		{
			...
			UE_LOGF(LogEditorActor, Log, "\t%ls", *(Light->GetPathName()));
		}
	}
