...
void FAssetEditorToolkit::InitAssetEditor( const EToolkitMode::Type Mode, const TSharedPtr< class IToolkitHost >& InitToolkitHost, const FName AppIdentifier, const TSharedRef<FTabManager::FLayout>& StandaloneDefaultLayout, const bool bCreateDefaultStandaloneMenu, const bool bCreateDefaultToolbar, const TArray<UObject*>& ObjectsToEdit, const bool bInIsToolbarFocusable, const bool bInUseSmallToolbarIcons, const TOptional<EAssetOpenMethod>& InOpenMethod )
{
	...
	else if( ensure( ToolkitMode == EToolkitMode::Standalone ) )
	{
		...
		NewMajorTab = SNew(SDockTab)
			.OnExtendContextMenu(SDockTab::FExtendContextMenu::CreateSP(this, &FAssetEditorToolkit::OnExtendTabContextMenu))
			.ContentPadding(0.0f)
		...
		NewMajorTab->SetTabIcon(TabIcon);
		{
			...
			if (FName OverrideTabId = GetOverrideTabId(); !OverrideTabId.IsNone())
			{
				PlaceholderId = OverrideTabId;
			}
	...
	if (EditorModeManager)
	{
		...
		if (UModeManagerInteractiveToolsContext* ToolsContext = EditorModeManager->GetInteractiveToolsContext())
		{
			ToolsContext->OnBuildViewportInteractions().AddRaw(this, &FAssetEditorToolkit::OnBuildViewportInteractions);
			ToolsContext->PostBuildViewportInteractions().AddRaw(this, &FAssetEditorToolkit::PostBuildViewportInteractions);
		}
	}
...
FAssetEditorToolkit::~FAssetEditorToolkit()
{
	...
	if (EditorModeManager)
	{
		if (UModeManagerInteractiveToolsContext* ToolsContext = EditorModeManager->GetInteractiveToolsContext())
		{
			ToolsContext->OnBuildViewportInteractions().RemoveAll(this);
			ToolsContext->PostBuildViewportInteractions().RemoveAll(this);
		}
	}
...
void FAssetEditorToolkit::GetSaveableObjects(TArray<UObject*>& OutObjects) const
{
	for (const TObjectPtr<UObject>& Object : EditingObjects)
	{
		...
		for (UObject* TestObject = Object; TestObject != nullptr; TestObject = TestObject->GetOuter())
		{
			...
			{
				OutObjects.AddUnique(TestObject);
				break;
			}
		}
	}
...
void FAssetEditorToolkit::SaveAsset_Execute()
{
	...
	for (UObject* Object : ObjectsToSave)
	{
		if ((Object == nullptr) || !Object->IsAsset())
		{
			...
			UE_LOGF(LogAssetEditorToolkit, Log, "Invalid object to save: %ls", (Object != nullptr) ? *Object->GetFullName() : TEXT("Null Object"));
		}
		else
		{
...
void FAssetEditorToolkit::OnBuildViewportInteractions(UViewportInteractionsBehaviorSource* Source)
{
	UE::Editor::ViewportInteractions::AddDefaultCameraMovementInteractions(Source);
	
	Source->AddInteraction<UViewportLegacyClickInteraction>();
}
...
void FAssetEditorToolkit::OnExtendTabContextMenu(FMenuBuilder& InMenuBuilder)
{
	InMenuBuilder.BeginSection("DockingTabAssetOptions", LOCTEXT("AssetTabSection", "Assets"));
	{
		InMenuBuilder.AddMenuEntry(
			...
			FUIAction(
				FExecuteAction::CreateSP(this, &FAssetEditorToolkit::OnBrowseAssetInContentBrowser),
				FCanExecuteAction::CreateSP(this, &FAssetEditorToolkit::OnCanBrowseAssetInContentBrowser)
			)
		);
	}
...
bool FAssetEditorToolkit::OnCanBrowseAssetInContentBrowser() const
{
	for (const TObjectPtr<UObject>& EditingObject : EditingObjects)
	{
