...
void FGlobalEditorCommonCommands::RegisterCommands()
{
	...
	UI_COMMAND(FindInContentBrowser, "Browse to Asset(s)", "Browses and selects the associated asset(s) in the most recently used Content Browser (summoning one if necessary)", EUserInterfaceActionType::Button, FInputChord(EModifierKey::Control, EKeys::B));
...
#if PLATFORM_MAC
	EModifierKey::Type ModifierKey = EModifierKey::Command;
#else
	EModifierKey::Type ModifierKey = EModifierKey::Control;
#endif

	UI_COMMAND(OpenEditorPreferences, "Editor Preferences...", "Configure the behavior and features of the Unreal Editor.", EUserInterfaceActionType::Button, FInputChord(EModifierKey::Alt | ModifierKey, EKeys::E));
	UI_COMMAND(OpenKeyboardShortcuts, "Keyboard Shortcuts...", "Configure the keyboard shortcuts of the Unreal Editor.", EUserInterfaceActionType::Button, FInputChord(EModifierKey::Alt | ModifierKey, EKeys::K));
	UI_COMMAND(OpenProjectSettings, "Project Settings...", "Change the settings of the currently loaded project.", EUserInterfaceActionType::Button, FInputChord(EModifierKey::Alt | ModifierKey, EKeys::P));
}
...
void FGlobalEditorCommonCommands::MapActions(TSharedRef<FUICommandList>& ToolkitCommands)
{
	...
	FGlobalEditorCommonCommands& GlobalEditorCommands = Get();

	ToolkitCommands->MapAction(
		GlobalEditorCommands.SummonControlTabNavigation,
		FExecuteAction::CreateStatic(&FGlobalEditorCommonCommands::OnPressedCtrlTab, GlobalEditorCommands.SummonControlTabNavigation));

	ToolkitCommands->MapAction(
		GlobalEditorCommands.SummonControlTabNavigationAlternate,
		FExecuteAction::CreateStatic(&FGlobalEditorCommonCommands::OnPressedCtrlTab, GlobalEditorCommands.SummonControlTabNavigationAlternate));

	ToolkitCommands->MapAction(
		GlobalEditorCommands.SummonControlTabNavigationBackwards,
		FExecuteAction::CreateStatic(&FGlobalEditorCommonCommands::OnPressedCtrlTab, GlobalEditorCommands.SummonControlTabNavigationBackwards));

	ToolkitCommands->MapAction(
		GlobalEditorCommands.SummonControlTabNavigationBackwardsAlternate,
		FExecuteAction::CreateStatic(&FGlobalEditorCommonCommands::OnPressedCtrlTab, GlobalEditorCommands.SummonControlTabNavigationBackwardsAlternate));

	ToolkitCommands->MapAction(
		GlobalEditorCommands.SummonOpenAssetDialog,
		FExecuteAction::CreateStatic(&FGlobalEditorCommonCommands::OnSummonedAssetPicker));

	ToolkitCommands->MapAction(
		GlobalEditorCommands.SummonOpenAssetDialogAlternate,
		FExecuteAction::CreateStatic(&FGlobalEditorCommonCommands::OnSummonedAssetPicker));

	ToolkitCommands->MapAction(
		GlobalEditorCommands.OpenConsoleCommandBox,
		FExecuteAction::CreateStatic(&FGlobalEditorCommonCommands::OnSummonedConsoleCommandBox));

	ToolkitCommands->MapAction(
		GlobalEditorCommands.OpenContentBrowserDrawer,
		FExecuteAction::CreateStatic(&FGlobalEditorCommonCommands::OnOpenContentBrowserDrawer));

	ToolkitCommands->MapAction(
		GlobalEditorCommands.OpenOutputLogDrawer,
		FExecuteAction::CreateStatic(&FGlobalEditorCommonCommands::OnOpenOutputLogDrawer));

	ToolkitCommands->MapAction(
		GlobalEditorCommands.OpenLevel,
	...
	ToolkitCommands->MapAction(
		GlobalEditorCommands.OpenFindInAllBlueprints,
		FExecuteAction::CreateStatic(&FGlobalEditorCommonCommands::OnOpenFindInAllBlueprints));
	
	// Get editor default category and section
	FName CategoryName = NAME_None; // "All settings"
	FName SectionName = NAME_None; // "All settings"
	if (IMainFrameModule* MainFrameModule = FModuleManager::GetModulePtr<IMainFrameModule>("MainFrame"))
	{
		MainFrameModule->GetEditorSettingsDefaultSelectionOverride(CategoryName, SectionName);
	}

	ToolkitCommands->MapAction(
		GlobalEditorCommands.OpenEditorPreferences,
		FExecuteAction::CreateStatic(&FGlobalEditorCommonCommands::OpenSettings, FName("Editor"), CategoryName, SectionName));

	ToolkitCommands->MapAction(
		GlobalEditorCommands.OpenKeyboardShortcuts,
		FExecuteAction::CreateStatic(&FGlobalEditorCommonCommands::OpenSettings, FName("InputBinding"), FName("Shortcuts"), FName("InputBindings")));

	CategoryName = NAME_None; // "All settings"
	SectionName = NAME_None; // "All settings"
	ToolkitCommands->MapAction(
		GlobalEditorCommands.OpenProjectSettings,
		FExecuteAction::CreateStatic(&FGlobalEditorCommonCommands::OpenSettings, FName("Project"), CategoryName, SectionName));
}
...
void FGlobalEditorCommonCommands::OpenSettings(FName InContainerName, FName InCategoryName, FName InSectionName)
{
	FModuleManager::LoadModuleChecked<ISettingsModule>("Settings").ShowViewer(InContainerName, InCategoryName, InSectionName);
}
