...
void SStandaloneAssetEditorToolkitHost::RestoreFromLayout( const TSharedRef<FTabManager::FLayout>& NewLayout )
{
	...
	TSharedPtr<SDockTab> HostTab = HostTabPtr.Pin();
	if (!HostTab.IsValid())
	{
		return;
	}
	HostTab->SetOnTabClosed(SDockTab::FOnTabClosedCallback::CreateSP(this, &SStandaloneAssetEditorToolkitHost::OnTabClosed));
	HostTab->SetOnTabRelocated(FSimpleDelegate::CreateSP(this, &SStandaloneAssetEditorToolkitHost::OnTabRelocated));
	...
	TSharedPtr<SWindow> ParentWindow = FSlateApplication::Get().FindWidgetWindow( HostTab.ToSharedRef() );
	...
	{
		StatusBarWidget = StatusBarSubsystem->MakeStatusBarWidget(StatusBarName, HostTab.ToSharedRef(), StatusBarCreationOptions);
	}
...
UWorld* SStandaloneAssetEditorToolkitHost::GetWorld() const 
{
	...
	UE_LOGF(LogInit, Warning, "IToolkitHost::GetWorld() doesn't make sense in SStandaloneAssetEditorToolkitHost currently");
	return NULL;
}
...
void SStandaloneAssetEditorToolkitHost::OnTabClosed(TSharedRef<SDockTab> TabClosed)
{
...
void SStandaloneAssetEditorToolkitHost::OnTabRelocated()
{
	SaveTabLocation();
}

void SStandaloneAssetEditorToolkitHost::SaveTabLocation() const
{
	if (!HostedAssetEditorToolkit.IsValid())
	{
		return;
	...
	const bool IsDockedAssetEditor = HostTab->HasSiblingTab(FName("DockedToolkit"), false/*TreatIndexNoneAsWildcard*/);
	...
	// Only write this section to disk to avoid loosing last tab position on crash
	if (const FConfigFile* ConfigFile = GConfig->FindConfigFile(GEditorPerProjectIni))
	{
		const FConfigSection* Section = ConfigFile->FindSection(TEXT("AssetEditorToolkitTabLocation"));
		if (Section && ConfigFile->Branch)
		{
			FString IniPath = ConfigFile->Branch->IniPath;
			FConfigSection SectionCopy = *Section;

			// Avoid editor freeze since config flush happens on main thread...
			Async(EAsyncExecution::ThreadPool, [IniPath, SectionCopy = MoveTemp(SectionCopy)]()
			{
				FConfigFile DiskFile;
				DiskFile.Read(IniPath);
				FConfigSection* MutableSection = const_cast<FConfigSection*>(DiskFile.FindOrAddConfigSection(TEXT("AssetEditorToolkitTabLocation")));
				*MutableSection = SectionCopy;
				DiskFile.Write(IniPath);
			});
		}
	}
}
