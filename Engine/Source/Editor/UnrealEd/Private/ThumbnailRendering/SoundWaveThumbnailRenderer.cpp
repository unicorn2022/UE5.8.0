	...
	for (uint32 ChannelWaveFileIndex = 0; ChannelWaveFileIndex < NumPackedWaveFiles; ++ChannelWaveFileIndex)
	{
		...
		if (WaveInfo.ReadWaveHeader(RawWaveData + CurrentRawWaveByteIndex, RawChannelFileByteSize, 0))
		{
			...
			if (SoundWave->NumChannels > 2)
			{
				...
				if (!bLooksSane)
				{
					UE_LOGF(LogAudio, Warning, "USoundWaveThumbnailRenderer::Draw: Not rendering thumbnail for Wave '%ls', as its supposed to be mono and has '%d' channels.", 
			...
			else
			{
				...
				if (!bLooksSane)
				{
					UE_LOGF(LogAudio, Warning, "USoundWaveThumbnailRenderer::Draw: Not rendering thumbnail for Wave '%ls', as pChannels('%d') != SoundWave->NumChannels('%d')",
