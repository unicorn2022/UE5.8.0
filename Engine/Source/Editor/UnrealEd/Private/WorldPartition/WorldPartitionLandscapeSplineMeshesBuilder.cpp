...
ALandscapeSplineMeshesActor* UWorldPartitionLandscapeSplineMeshesBuilder::GetOrCreatePartitionActorForComponent(UWorld* InWorld, const UStaticMeshComponent* InMeshComponent, const FGuid& InLandscapeGuid)
{
	...
	if (const USplineMeshComponent* SplineMeshComponent = Cast<USplineMeshComponent>(InMeshComponent))
	{
		HLODLayer = SplineMeshComponent->GetHLODLayerForSplinePartitioning();
	}
	else if (const UControlPointMeshComponent* ControlPointMeshComponent = Cast<UControlPointMeshComponent>(InMeshComponent))
	{
		HLODLayer = ControlPointMeshComponent->GetHLODLayerForSplinePartitioning();
	}
	else
	{
		checkNoEntry();
...
{			
	...
	if (!WorldPartition)
	{
		UE_LOGF(LogWorldPartitionLandscapeSplineMeshBuilder, Error, "World is not partitioned.");
		return false;
	}
	...
	{
		UE_LOGF(LogWorldPartitionLandscapeSplineMeshBuilder, Display, "Changing Landscape Spline Meshes Grid Size from %d to %d", InWorld->GetWorldSettings()->LandscapeSplineMeshesGridSize, NewGridSize);
	...
	int32 Progress = 0;
	UE_LOGF(LogWorldPartitionLandscapeSplineMeshBuilder, Display, "Processing Landscape Splines for world %ls", *InWorld->GetName());
	for (ALandscapeSplineMeshesActor* Actor : ActorsToProcess)
	{
		UE_LOGF(LogWorldPartitionLandscapeSplineMeshBuilder, Display, "Processing Splines for %ls [%d/%d]", *Actor->GetName(), ++Progress, ActorsToProcess.Num());
...
bool UWorldPartitionLandscapeSplineMeshesBuilder::RunOnInitializedWorld(UWorld* InWorld)
{
	...
	{
		UE_LOGF(LogWorldPartitionLandscapeSplineMeshBuilder, Error, "World must be initialized.");
		return false;
	}
	...
	if (!Builder)
	{
		UE_LOGF(LogWorldPartitionLandscapeSplineMeshBuilder, Error, "Failed to create WorldPartitionLandscapeSplineMeshesBuilder.");
		return false;
	}
