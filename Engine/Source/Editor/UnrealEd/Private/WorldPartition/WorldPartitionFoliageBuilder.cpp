...
{			
	...
	if (!WorldPartition)
	{
		UE_LOGF(LogWorldPartitionFoliageBuilder, Error, "Failed to retrieve WorldPartition.");
		return false;
	}
	...
	if (bRepair)
	{
		if (NewGridSize)
		{
			UE_LOGF(LogWorldPartitionFoliageBuilder, Error, "Cannot repair foliage actors while specifying NewGridSize.");
			return false;
		}
	}
	else if (World->GetWorldSettings()->InstancedFoliageGridSize == NewGridSize)
	{
		UE_LOGF(LogWorldPartitionFoliageBuilder, Display, "Foliage Grid Size already is %d.", NewGridSize);
		return true;
	}
	else if (NewGridSize == 0)
	{
		UE_LOGF(LogWorldPartitionFoliageBuilder, Error, "Foliage Grid Size is invalid %d. Please specify NewGridSize parameter.", NewGridSize);
		return false;
	}
	...
	if (!bRepair)
	{
		...
		{ 
			...
			UE_LOGF(LogWorldPartitionFoliageBuilder, Error, "FoliageBuilder Commandlet is unsupported on WorldPartition using more than 1 ExternalActor Folder.");
			return false;
		}

		UE_LOGF(LogWorldPartitionFoliageBuilder, Display, "Changing Foliage Grid Size from %d to %d", World->GetWorldSettings()->InstancedFoliageGridSize, NewGridSize);
	}
	...
	FWorldPartitionHelpers::ForEachActorWithLoading(WorldPartition, [this, World, WorldPartition, ActorPartitionSubsystem, &SCCHelper, &NewActors, &NumInstances, &NumInstancesProcessed](const FWorldPartitionActorDescInstance* ActorDescInstance)
	{
		...
		if (!IFA)
		{
			UE_LOGF(LogWorldPartitionFoliageBuilder, Error, "Foliage actor failed to load: %ls (%ls)", *PartitionActorDesc->GetActorNameString(), *PartitionActorDesc->GetActorPackage().ToString());
			return false;
		}
		...
		if (bRepair)
		{
			...
			if (PartitionActorDesc->GetActorName() != ExpectedActorName)
			{
				...
				UE_LOGF(LogWorldPartitionFoliageBuilder, Display, "Repairing invalid foliage actor: %ls->%ls ", *PartitionActorDesc->GetActorNameString(), *ExpectedActorName);
				...
				if (const FWorldPartitionActorDescInstance* DupActorDescInstance = WorldPartition->GetActorDescInstanceByPath(ExpectedActorPath))
				{
					...
					if (!DupIFA)
					{
						UE_LOGF(LogWorldPartitionFoliageBuilder, Error, "Foliage actor failed to load: %ls (%ls)", *DupActorDescInstance->GetActorNameString(), *DupActorDescInstance->GetActorPackage().ToString());
						return false;
					}
		...
		{	
			...
			UE_LOGF(LogWorldPartitionFoliageBuilder, Display, "Processing existing foliage actor: %ls (%ls)", *PartitionActorDesc->GetActorNameString(), *PartitionActorDesc->GetActorPackage().ToString());
			...
			IFA->ForEachFoliageInfo([IFA, &FoliageToAdd, &InstanceBounds, &NumInstances](UFoliageType* FoliageType, FFoliageInfo& FoliageInfo)
			{
				...
				{
					UE_LOGF(LogWorldPartitionFoliageBuilder, Display, "    FoliageType: %ls Count: %d", *FoliageType->GetName(), FoliageInfo.Instances.Num());
				}
			...
			for (auto Pair : IntersectingActors)
			{
				...
				{
					UE_LOGF(LogWorldPartitionFoliageBuilder, Error, "Error saving package %ls.", *Package->GetName());
					return false;
				}
				...
				if (!NewActors.Contains(Pair.Key))
				{
					if (!SCCHelper.AddToSourceControl(Package))
					{
						...
						UE_LOGF(LogWorldPartitionFoliageBuilder, Error, "Error adding package to revision control %ls.", *Package->GetName());
						return false;
					}
				...
				UE_LOGF(LogWorldPartitionFoliageBuilder, Display, "Saved foliage actor %ls (%ls).", *IntersectingActor->GetName(), *Package->GetName());
			}
		}
	...
	if (!bRepair)
	{
		...
		{
			UE_LOGF(LogWorldPartitionFoliageBuilder, Error, "Error checking out package %ls.", *WorldSettingsPackage->GetName());
			return false;
		}
		...
		// Save World 
		{
			...
			{
				UE_LOGF(LogWorldPartitionFoliageBuilder, Error, "Error saving package %ls.", *WorldSettingsPackage->GetName());
				return false;
			}
		}
		...
		for (const FGuid& ActorGuid : ExistingActors)
		{
			...
			{
				UE_LOGF(LogWorldPartitionFoliageBuilder, Error, "Error deleting package %ls.", *PackageToDelete);
				return false;
			}
		}
	}
