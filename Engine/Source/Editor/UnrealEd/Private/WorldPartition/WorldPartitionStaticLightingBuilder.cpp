...
bool UWorldPartitionStaticLightingBuilder::DeleteIntermediates()
{
	...
	for (const FString& File : Files)
	{
		...
		if (!bDeleteResult)
		{
			UE_LOGF(LogWorldPartitionStaticLightingBuilder, Warning, "Could not delete intermediate file %ls", *FileName);
		}
	...
	for (FActorDescContainerInstanceCollection::TIterator<AWorldPartitionHLOD> HLODIterator(WorldPartition); HLODIterator; ++HLODIterator)
	{
		...
		if (AActor* HLODActor = HLODActorReference.GetActor())
		{
			...
			if (FLightingActorDesc* HLODActorDesc = Descriptors.ActorGuidsToDesc.Find(HLODActor->GetActorInstanceGuid()))
			{
				...
				{
					UE_LOGF(LogWorldPartitionStaticLightingBuilder, Warning, "Could not locate owning cell descriptors (CellPackage %ls) for HLOD actor %ls", *HLODActorDesc->CellLevelPackage.ToString(), *HLODIterator->GetActorLabelOrName().ToString());
				}
			}
		}
	}
...
bool UWorldPartitionStaticLightingBuilder::DeleteStaticLightingData(UWorld* World, FPackageSourceControlHelper& PackageHelper)
{
	...
	if (UActorFolder* Folder = World->PersistentLevel->GetActorFolder("MapBuildData"))
	{
		...
		{
			UE_LOGF(LogWorldPartitionStaticLightingBuilder, Log, "Deleting and saving ActorFolder %ls", *Folder->GetFullName());
	...
	if (bSaveWorld)
	{
		UE_LOGF(LogWorldPartitionStaticLightingBuilder, Log, "Saving World %ls to unreference MapBuildData", *World->GetFullName());
	...
	UE_LOGF(LogWorldPartitionStaticLightingBuilder, Log, "Deleting AMapBuildData Actors");
	for (FStaticLightingDescriptors::FActorPackage& PackageToDelete : PackagesToDelete)
	{
		UE_LOGF(LogWorldPartitionStaticLightingBuilder, Log, "   => Deleting %ls", *PackageToDelete.PackageName.ToString());
...
bool UWorldPartitionStaticLightingBuilder::Run(UWorld* World, const FCellInfo& InCellInfo, FPackageSourceControlHelper& PackageHelper)
{
	...
	UE_LOGF(LogWorldPartitionStaticLightingBuilder, Verbose, "Building Volumetric Lightmaps for %ls", *World->GetName());
	...
	auto BuildFailedDelegate = [&bLightingBuildFailed, &World]() {		
		UE_LOGF(LogWorldPartitionStaticLightingBuilder, Error, "[REPORT] Failed building lighting for %ls", *World->GetName());
	...
	{		
		...
		UE_LOGF(LogWorldPartitionStaticLightingBuilder, Verbose, "Iterating Actor %ls, ActorInstance Guid %ls", *Actor->GetActorNameOrLabel() , *ActorGuid.ToString());
...
{		
	...
	{			
		UE_LOGF(LogWorldPartitionStaticLightingBuilder, Log, "Deleting AMapBuildData Actor %ls, Associated CellPackage: %ls", *ActorPackage.PackageName.ToString(), *ActorPackage.AssociatedLevelPackage.ToString());
		bool bDeleted = DeletePackage(ActorPackage, PackageHelper);
		if (!bDeleted)
		{
			UE_LOGF(LogWorldPartitionStaticLightingBuilder, Log, "Failed to delete AMapBuildData Actor %ls", *ActorPackage.PackageName.ToString());
			bRet = false;
		}
	}
	...
	for (FStreamingDescriptor::FStreamingGrid& Grid : StreamingDesc.StreamingGrids)
	{
		for (FStreamingDescriptor::FStreamingCell& Cell : Grid.StreamingCells)
		{
			...
			for (FStreamingDescriptor::FStreamingActor& Actor : Cell.Actors)
			{
				...
				UE_LOGF(LogWorldPartitionStaticLightingBuilder, Verbose, "Discovered ActorInstance Guid %ls, Cell %ls / Grid %ls", *Actor.ActorGuid.ToString(), *CellPackageNameStr, *Grid.Name.ToString());
...
bool FStaticLightingDescriptors::CreateAndUpdateActors()
{
	...
	{	
		...
		if (MapBuildData)
		{
			...
			UE_LOGF(LogWorldPartitionStaticLightingBuilder, Log, "Updated/Created AMapBuildData Actor %ls", *DataActor->GetName());
			for (FGuid ActorInstanceGuid : CellDesc.ActorInstanceGuids)
			{
				UE_LOGF(LogWorldPartitionStaticLightingBuilder, Verbose, "  => ActorInstanceGuid %ls",  *ActorInstanceGuid.ToString());
			}
	...
	FConsoleCommandDelegate::CreateLambda([]()
	{
		for (UStaticMeshComponent* StaticMeshComponent : TObjectRange<UStaticMeshComponent>())
		{
			...
			for (FStaticMeshComponentLODInfo& LODInfo : StaticMeshComponent->LODData)
			{
				...
				{
					UE_LOGF(LogEngine, Log, "Marking component %ls's package dirty", *StaticMeshComponent->GetFullName());
			...
			{
				UE_LOGF(LogEngine, Log, "Component %ls has no LOD Data", *StaticMeshComponent->GetFullName());
			}
		}		
