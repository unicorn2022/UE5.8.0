...
bool UWorldPartitionMiniMapBuilder::PreRun(UWorld* World, FPackageSourceControlHelper& PackageHelper)
{
	...
	if (!WorldMiniMap)
	{
		UE_LOGF(LogWorldPartitionMiniMapBuilder, Error, "Failed to create Minimap. WorldPartitionMiniMap actor not found in the persistent level.");
		return false;
	}
	...
	//DirectionalLightComponent will return a Bound of size WORLD_MAX which result in a huge non-spatial Bound. The non-spatial bound is returned by GetMiniMapWorldBounds under the following conditions:
	// 1. The WP level has streaming disabled
	// 2. There is no AWorldPartitionMiniMapVolume in the level
	// 3. The level contains an Actor with a DirectionalLightComponent as its only component and root. 
	const FVector WorldSize = IterativeWorldBounds.GetSize();
	if (WorldSize.X >= (WORLD_MAX - 1) || WorldSize.Y >= (WORLD_MAX - 1) || WorldSize.Z >= (WORLD_MAX - 1))
	{
		UE_LOG(LogWorldPartitionMiniMapBuilder, Warning, TEXT("The World BoundingBox is huge %s. Consider adding a AWorldPartitionMiniMapVolume to the level and set it to cover the surface expected to be part of the minimap."), *IterativeWorldBounds.ToString());
	}
	...
	IncludedDataLayers = WorldMiniMap->IncludedDataLayers;
...
bool UWorldPartitionMiniMapBuilder::PostRun(UWorld* World, FPackageSourceControlHelper& PackageHelper, const bool bInRunSuccess)
{
	...
	{
		UE_LOGF(LogWorldPartitionMiniMapBuilder, Error, "Error checking out package %ls.", *WorldMiniMapExternalPackage->GetName());
		return false;
	}
	...
	{
		UE_LOGF(LogWorldPartitionMiniMapBuilder, Error, "Error saving package %ls.", *WorldMiniMapExternalPackage->GetName());
		return false;
	}

	if (!PackageHelper.AddToSourceControl(WorldMiniMapExternalPackage))
	{
		UE_LOGF(LogWorldPartitionMiniMapBuilder, Error, "Error adding package %ls to revision control.", *WorldMiniMapExternalPackage->GetName());
		return false;
	}
