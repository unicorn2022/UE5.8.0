...
bool UWorldPartitionResaveActorsBuilder::PreRun(UWorld* World, FPackageSourceControlHelper& PackageHelper)
{
	...
	if (bSwitchActorPackagingSchemeToReduced)
	{
		...
		{
			UE_LOGF(LogWorldPartitionResaveActorsBuilder, Error, "SwitchActorPackagingSchemeToReduced is not compatible with ActorClassName");
			return false;
		}
		else if (!ActorClassesFromFile.IsEmpty())
		{
			UE_LOGF(LogWorldPartitionResaveActorsBuilder, Error, "SwitchActorPackagingSchemeToReduced is not compatible with ActorClassesFromFile");
			return false;
		}
		else if (!ActorTags.IsEmpty())
		{
			UE_LOGF(LogWorldPartitionResaveActorsBuilder, Error, "SwitchActorPackagingSchemeToReduced is not compatible with ActorTags");
			return false;
		}
		else if (!ActorProperties.IsEmpty())
		{
			UE_LOGF(LogWorldPartitionResaveActorsBuilder, Error, "SwitchActorPackagingSchemeToReduced is not compatible with ActorProperties");
			return false;
		}
		else if (bResaveDirtyActorDescsOnly)
		{
			UE_LOGF(LogWorldPartitionResaveActorsBuilder, Error, "SwitchActorPackagingSchemeToReduced is not compatible with ResaveDirtyActorDescsOnly");
			return false;
		}
	}
	...
	if (bEnableActorFolders)
	{
		...
		{
			UE_LOGF(LogWorldPartitionResaveActorsBuilder, Error, "EnableActorFolders is not compatible with ResaveDirtyActorDescsOnly");
			return false;
		}
	}
...
TArray<TSubclassOf<AActor>> UWorldPartitionResaveActorsBuilder::GetActorClassesFilter()
{
	...
	if (!ActorClassName.IsEmpty())
	{
		...
		{
			UE_LOGF(LogWorldPartitionResaveActorsBuilder, Error, "Failed to find Actor Class: %ls.", *ActorClassName);
		}
	}
	else if (!ActorClassesFromFile.IsEmpty())
	{
		...
		if (bReadFile)
		{
			for (const FString& ClassName : ClassesNames)
			{
				...
				{
					UE_LOGF(LogWorldPartitionResaveActorsBuilder, Warning, "Failed to find Actor Class: %ls.", *ClassName);
				}
			}

			if (ActorClasses.IsEmpty())
			{
				UE_LOGF(LogWorldPartitionResaveActorsBuilder, Error, "Failed to find any valid actor classes in file: %ls.", *ActorClassesFromFile);
			}			
		}
		else
		{
			UE_LOGF(LogWorldPartitionResaveActorsBuilder, Error, "Failed to open/process actor classes in file: %ls.", *ActorClassesFromFile);
		}
	}
	else 
	{
...
bool UWorldPartitionResaveActorsBuilder::RunInternal(UWorld* World, const FCellInfo& InCellInfo, FPackageSourceControlHelper& PackageHelper)
{
	...
	if (!WorldPartition)
	{
		...
		{
			UE_LOGF(LogWorldPartitionResaveActorsBuilder, Error, "Cannot work on maps not using one file per actor.");
			return false;
		}

		if (bSwitchActorPackagingSchemeToReduced)
		{
			UE_LOGF(LogWorldPartitionResaveActorsBuilder, Error, "Cannot switch actor packaging scheme on non-partitioned worlds.");
			return false;
		}

		if (bEnableActorFolders)
		{
			UE_LOGF(LogWorldPartitionResaveActorsBuilder, Error, "Cannot enable actor folders non-partitioned worlds.");
			return false;
		}
			...
			for (FActorDescList::TIterator<> ActorDescIterator(LevelContainer); ActorDescIterator; ++ActorDescIterator)
			{
				if (AActor* Actor = FindObject<AActor>(World->PersistentLevel, *ActorDescIterator->GetActorName().ToString()))
				{
					...
					if (bResaveDirtyActorDescsOnly)
					{
						...
						UE_LOGF(LogWorldPartitionResaveActorsBuilder, Log, "Package %ls needs to be resaved.", *Package->GetName());
					}
	...
	else
	{
		if (bSwitchActorPackagingSchemeToReduced)
		{
			...
			{
				UE_LOGF(LogWorldPartitionResaveActorsBuilder, Error, "World is already using the reduced actor packaging scheme.");
				return false;
			}
		...
		if (bEnableActorFolders)
		{
			...
			{
				UE_LOGF(LogWorldPartitionResaveActorsBuilder, Error, "World is already using actor folder objects.");
				return false;
			}
				...
				for (FWorldPartitionReference& ActorReference : ActorReferences)
				{
					...
					else
					{
						...
						if (!bReportOnly)
						{
							...
							{
								DependantObject->Rename(nullptr, WorldPackage, REN_NonTransactional | REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
							}
							...
							for (UObject* DependantObject : DependantObjects)
							{
								DependantObject->Rename(nullptr, NewActorPackage, REN_NonTransactional | REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
							}
			...
			if (bResaveBlueprints)
			{
				...
				while (!ClassesToResave.IsEmpty())
				{
					for (const FTopLevelAssetPath& ClassToResave : ClassesToResave)
					{
						...
						if (!ClassPackage)
						{
							UE_LOGF(LogWorldPartitionResaveActorsBuilder, Error, "Failed to load package for class '%ls'", *ClassToResave.ToString());
							return false;
						}
			...
			FWorldPartitionHelpers::ForEachActorWithLoading(WorldPartition, [this, &PackagesToDelete, &PackagesToSave, &DirtyActorDescsOld, &DirtyActorDescsNew](const FWorldPartitionActorDescInstance* ActorDescInstance)
			{
				...
				else
				{
					if (bEnableActorFolders)
					{
						...
						{
							UE_LOGF(LogWorldPartitionResaveActorsBuilder, Error, "Failed to create actor folder for actor %ls.", *Actor->GetName());
							return true;
						}
					}
	...
	if (bDiffDirtyActorDescs && DirtyActorDescsOld.Num())
	{
		...
		{
			UE_LOGF(LogWorldPartitionResaveActorsBuilder, Error, "Failed to write actors desc diff files.");
		}
	}
