...
void UEditorPerformanceSettings::PostInitProperties()
{
	...
	if (FProperty* AllowSlateThrottlingProperty = GetClass()->FindPropertyByName(GET_MEMBER_NAME_CHECKED(UEditorPerformanceSettings, bAllowSlateThrottling)))
	{
		ExportValuesToConsoleVariables(AllowSlateThrottlingProperty);
	}
