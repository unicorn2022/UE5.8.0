	...
	for (int32 Index = 0; Index < AddedFiles.Num(); ++Index)
	{
		...
		if (bUseInterchangeFramework)
		{
			...
			if (InterchangeManager.CanTranslateSourceData(ScopedSourceData.GetSourceData()))
			{
				...
				// Priority is given to asset import. Try it first 
				bImportHandledThroughInterchange = InterchangeManager.ImportAsset(FPaths::GetPath(PackagePath), ScopedSourceData.GetSourceData(), ImportAssetParameters);
				if (!bImportHandledThroughInterchange)
				{
					// Try a level import then
					bImportHandledThroughInterchange = InterchangeManager.ImportScene(FPaths::GetPath(PackagePath), ScopedSourceData.GetSourceData(), ImportAssetParameters);
					if (!bImportHandledThroughInterchange)
					{
						// Add the message to the message log letting the user know that the import didn't work.
						Context.GetMessageLog().Message(EMessageSeverity::Info, FText::Format(LOCTEXT("Info_InterchangeFailedToImportAsset", "Interchange: Failed to import file {0}."), FText::FromString(FullFilename)));
					}
				}
			}
		}
	...
	for (int32 Index = 0; Index < ModifiedFiles.Num(); ++Index)
	{
		...
		if (Change.Action == DirectoryWatcher::EFileAction::Moved)
		{
			...
			if (Assets.Num() == 1)
			{
				...
				if (Asset && Utils::ExtractSourceFilePaths(Asset).Num() == 1 && !MountedContentPath.IsEmpty())
				{
