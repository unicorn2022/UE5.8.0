...
public:
	...
	/** Add a directory to watch at runtime. Directories added this way are not saved in the settings. Returns a handle that can be passed to RemoveRuntimeMonitoredDirectory.*/
	int32 AddRuntimeMonitoredDirectory(const FAutoReimportDirectoryConfig& Directory);

	/** Remove runtime directory with the given handle. */
	void RemoveRuntimeMonitoredDirectory(const int32 Handle);
...
private:
	...
	/** Schedule monitors to be updated after a delay */
	void SetMonitorsDirty();
...
private:
	...
	/** Monitored directories added at runtime */
	TMap<int32, FAutoReimportDirectoryConfig> RuntimeMonitoredDirectories;

	int32 NextRuntimeMonitoredDirectoryHandle = 0;
private:
...
void FAutoReimportManager::SetMonitorsDirty()
{
...
int32 FAutoReimportManager::AddRuntimeMonitoredDirectory(const FAutoReimportDirectoryConfig& Directory)
{
	const int32 NewHandle = NextRuntimeMonitoredDirectoryHandle++;
	RuntimeMonitoredDirectories.Add(NewHandle, Directory);
	SetMonitorsDirty();
	return NewHandle;
}

void FAutoReimportManager::RemoveRuntimeMonitoredDirectory(const int32 Handle)
{
	if (RuntimeMonitoredDirectories.Remove(Handle))
	{
		SetMonitorsDirty();
	}
}
...
void FAutoReimportManager::SetUpDirectoryMonitors()
{
	...
	const FString SupportedExtensions = GetAllFactoryExtensions();
	auto TryAddMapping = [&FinalArray, &SupportedExtensions](const FAutoReimportDirectoryConfig& Setting, bool bAllowEmptyMountPoint)
		{
			...
			{
				// Allow the parse failure if it's just because the mount point is empty.
				if (!(bAllowEmptyMountPoint && FPaths::DirectoryExists(NewMapping.SourceDirectory) && NewMapping.MountPoint.IsEmpty()))
				{
					return;
				}
			}
	...
	{
		TryAddMapping(Setting, false);
	}

	for (const TPair<int32, FAutoReimportDirectoryConfig>& Setting : RuntimeMonitoredDirectories)
	{
		TryAddMapping(Setting.Value, true);
	}
	...
	for (int32 Index = 0; Index < FinalArray.Num(); ++Index)
	{
		...
		for (int32 OtherIndex = Index + 1; OtherIndex < FinalArray.Num(); ++OtherIndex)
		{
			...
			{
				UE_LOGF(LogAutoReimportManager, Warning, "Unable to watch directory %ls as it will conflict with another watching %ls.", *FinalArray[Index].SourceDirectory, *FinalArray[OtherIndex].SourceDirectory);
				goto next;
			}
		}
...
int32 UAutoReimportManager::AddRuntimeMonitoredDirectory(const FAutoReimportDirectoryConfig& Directory)
{
	return Implementation->AddRuntimeMonitoredDirectory(Directory);
}

void UAutoReimportManager::RemoveRuntimeMonitoredDirectory(const int32 Handle)
{
	Implementation->RemoveRuntimeMonitoredDirectory(Handle);
}
