...
void FPendingPrivateAsset::DoPrivatize()
{
	if (OwningPackage->SetAssetAccessSpecifier(TargetAssetAccessSpecifier))
	{
...
bool FAssetPrivatizeModel::DoPrivatize()
{
	...
	for (const TSharedPtr<FPendingPrivateAsset>& PendingPrivateAsset : PendingPrivateAssets)
	{
		PendingPrivateAsset->DoPrivatize();
		ObjectsPrivatized++;
	}
...
bool FAssetPrivatizeModel::DoForcePrivatize()
{
	...
	ObjectsToPrivatize.Reserve(PendingPrivateAssets.Num());
