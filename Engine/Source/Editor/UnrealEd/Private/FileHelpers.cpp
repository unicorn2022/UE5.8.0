...
public:
	...
	~FScopedSaveOutput()
	{
		...
		if (!WritablePackages.IsEmpty() && !FUncontrolledChangelistsModule::Get().IsEnabled())
		{
			...
			UE_LOGF(LogFileHelpers, Warning, "%ls", *WritableFileWarning.ToString());
			if (bUseDialog)
			{
...
void FEditorFileUtils::RegisterLevelFilename(UObject* Object, const FString& NewLevelFilename)
{
	...
	//UE_LOGF(LogFileHelpers, Log, "RegisterLevelFilename: package %ls to name %ls", *PackageName, *NewLevelFilename );
...
FString FEditorFileUtils::GetFilename(const FName& PackageName)
{
	...
	if ( !Result )
	{
		//UE_LOGF(LogFileHelpers, Log, "GetFilename with package %ls, returning EMPTY", *PackageName );
		return FString(TEXT(""));
	}
	...
	//UE_LOGF(LogFileHelpers, Log, "GetFilename with package %ls, returning %ls", *PackageName, **Result );
	return *Result;
}
		...
		if ( bRenamePackageToFile )
		{
			if (bPackageNeedsRename)
			{
				...
				if (!DuplicatedWorld)
				{
					...
					Package->Rename(*NewPackageName, NULL, REN_NonTransactional | REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
					...
					if (bWorldNeedsRename)
					{
						...
						World->Rename(*NewWorldAssetName, NULL, REN_NonTransactional | REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
					}
...
static bool SaveAsImplementation( UWorld* InWorld, const FString& DefaultFilename, const bool bAllowStreamingLevelRename, FString* OutSavedFilename )
{
	...
	FPackageAutoSaverScopeGuard AutoSaveScopeGuard;
...
bool FEditorFileUtils::SaveMap(UWorld* InWorld, const FString& Filename )
{
	...
	UE_LOGF(LogFileHelpers, Log, "Saving map '%ls' took %.3f", *FPaths::GetBaseFilename(Filename), FPlatformTime::Seconds() - SaveStartTime );
	...
	if ( WorldsArray.Num() > 0 )
	{
		...
		for ( int32 WorldIndex = 0 ; WorldIndex < WorldsArray.Num(); ++WorldIndex )
		{
			...
			if ( Package->IsDirty() && (bForceIfNotInList || DirtyPackagesForAutoSave.Contains(Package)) )
			{
				...
				//UE_LOGF(LogFileHelpers, Log,  "Autosaving '%ls'", *AutosaveFilename );
				...
				{
					UE_LOGF(LogFileHelpers, Log, "Editor autosave (incl. sublevels) failed for file '%ls' which belongs to world '%ls'. Aborting autosave.", *FinalFilename, *EditorContext.World()->GetOutermost()->GetName() );
		...
		UE_LOGF(LogFileHelpers, Log, "Editor autosave (incl. external actors) for '%ls' took %.3f", *EditorContext.World()->GetOutermost()->GetName(), ThisTime  );
	}
	if( bResult == true )
	{
		UE_LOGF(LogFileHelpers, Log, "Editor autosave (incl. sublevels & external actors) for all levels took %.3f", TotalSaveTime );
	}
...
EAutosaveContentPackagesResult::Type FEditorFileUtils::AutosaveContentPackagesEx(const FString& AbsoluteAutosaveDir, const int32 AutosaveIndex, const bool bForceIfNotInList, const TSet< TWeakObjectPtr<UPackage>, TWeakObjectPtrSetKeyFuncs<TWeakObjectPtr<UPackage>> >& DirtyPackagesForAutoSave)
{
	...
	for ( TObjectIterator<UPackage> PackageIter; PackageIter; ++PackageIter )
	{
		...
		if ( CurPackage && ( CurPackage != TransientPackage ) && CurPackage->IsDirty() && (bForceIfNotInList || DirtyPackagesForAutoSave.Contains(CurPackage)) )
		{
			...
			GetObjectsWithPackage(CurPackage, ObjectsInPackage, EGetObjectsFlags::None);
	...
	if ( bSavedPkgs )
	{	
		UE_LOGF(LogFileHelpers, Log, "Auto-saving content packages took %.3f", FPlatformTime::Seconds() - SaveStartTime );
	}
...
static bool InternalSavePackagesFast(const TArray<UPackage*>& PackagesToSave, bool bUseDialog, IScopedSaveOutput& SaveOutput)
{
	...
	for (UPackage* Package : FinalPackagesToSave)
	{
		...
		if (SaveStatus == InternalSavePackageResult::Cancel)
		{
			...
			UE_LOGF(LogFileHelpers, Warning, "Cancelled saving package %ls", *Package->GetName());
		}
		...
		for ( TArray<UPackage*>::TIterator PkgIter(PackagesToSave); PkgIter; ++PkgIter )
		{
			...
			if ( !InOutParams.bCheckDirty || (InOutParams.bCheckDirty && CurPackage->IsDirty() ) )
			{
				...
				if ( CurPackage != GetTransientPackage() )
				{
					...
					{
						UE_LOGF(LogFileHelpers, Warning, "PromptForCheckoutAndSave attempted to open the save dialog with a compiled in package: %ls", *CurPackage->GetName());
					}
				}
				else
				{
					UE_LOGF(LogFileHelpers, Warning, "PromptForCheckoutAndSave attempted to open the save dialog with the transient package");
				}
			}
			else
			{
		...
		for ( TArray<UPackage*>::TIterator PkgIter(PackagesToSave); PkgIter; ++PkgIter )
		{
			...
			if ( !InOutParams.bCheckDirty || CurPackage->IsDirty() )
			{
				...
				if ( CurPackage != GetTransientPackage() )
				{
					...
					{
						PkgIter.RemoveCurrent();
						UE_LOGF(LogFileHelpers, Warning, "PromptForCheckoutAndSave attempted to save a compiled in package: %ls", *CurPackage->GetName());
					}
				}
				else
				{
					PkgIter.RemoveCurrent();
					UE_LOGF(LogFileHelpers, Warning, "PromptForCheckoutAndSave attempted to save the transient package");
				}
			}
			else
			{
...
bool FEditorFileUtils::AutomaticCheckoutOrPromptToRevertPackages(const TArray<UPackage*>& PackagesToCheckOut, TArray<UPackage*>* OutPackagesCheckedOutOrMadeWritable, TArray<UPackage*>* OutPackagesNotNeedingCheckout, TArray<UPackage*>* OutPackagesToRevert)
{
	...
	if (Filenames.Num() > 0)
	{
		...
		// Update states - this blocks the thread.
	...
	else if (bSourceControlEnabled)
	{
		...
		FText OfflineTitle = NSLOCTEXT("FileHelper", "OfflineDialogTitle", "Warning: Changes While Offline");
		FText OfflineMessage = NSLOCTEXT("FileHelper", "OfflineDialogMessage",
			"Changes you make while offline may conflict with your teammates' work when you reconnect.\r\n\r\n"
			"To minimize conflicts, reconnect as soon as possible.");
...
void FEditorFileUtils::FindAllSubmittablePackageFiles(TMap<FString, FSourceControlStatePtr>& OutPackages, const bool bIncludeMaps)
{
	...
	for (TArray<FString>::TConstIterator PackageIter(Packages); PackageIter; ++PackageIter)
	{
		...
		{
			UE_LOGF(LogFileHelpers, Warning, "%ls", *FailureReason);
			continue;
		}
...
bool FEditorFileUtils::HasDirtyWorldPackages(const FShouldIgnorePackageFunctionRef& ShouldIgnorePackageFunction)
{
	bool bHasDirtyWorldPackage = false;

	TFunction<bool(UPackage*)> Callback = [&bHasDirtyWorldPackage, &ShouldIgnorePackageFunction](UPackage* DirtyPackage)
	{
		if (!ShouldIgnorePackageFunction(DirtyPackage))
		{
			bHasDirtyWorldPackage = true;
			return false;
		}
		return true;
	};

	EnumerateDirtyWorldPackages(Callback);

	return bHasDirtyWorldPackage;
}
...
{
	TFunction<bool(UPackage*)> Callback = [&OutDirtyPackages, &ShouldIgnorePackageFunction](UPackage* DirtyPackage)
	{
		if (!ShouldIgnorePackageFunction(DirtyPackage))
		{
			OutDirtyPackages.Add(DirtyPackage);
		}
		return true;
	};
...
void FEditorFileUtils::EnumerateDirtyWorldPackages(const FEnumeratePackageFunctionRef& Callback)
{
	FAssetToolsModule& AssetToolsModule = FModuleManager::LoadModuleChecked<FAssetToolsModule>("AssetTools");
			...
			if (WorldIt->PersistentLevel && WorldIt->PersistentLevel->MapBuildData)
			{
				...
				if (BuiltDataPackage != WorldPackage)
				{
					if (WorldPackage->IsDirty() && !BuiltDataPackage->IsDirty())
					{
						...
						if (!FPackageName::IsValidLongPackageName(FNameBuilder(BuiltDataPackage->GetFName()), /*bIncludeReadOnlyRoots= */ false))
						{
			...
			if (WorldIt->PersistentLevel)
			{
				for (UPackage* ExternalPackage : WorldIt->PersistentLevel->GetLoadedExternalObjectPackages())
				{
					if (ExternalPackage->IsDirty())
					{
						...
						if (!FPackageName::IsValidLongPackageName(FNameBuilder(ExternalPackage->GetFName())))
						{
							continue;
						}
						...
						if (ExternalPackage->HasAnyPackageFlags(PKG_NewlyCreated) && UPackage::IsEmptyPackage(ExternalPackage))
						{
							...
							if (!AdditionalSaveCandidates->Contains(ExternalPackage))
							{
								continue;
							}
						}

						if (!Callback(ExternalPackage))
						{
							return;
						}
					}
...
bool FEditorFileUtils::HasDirtyContentPackages(const FShouldIgnorePackageFunctionRef& ShouldIgnorePackageFunction)
{
	bool bHasDirtyContentPackage = false;

	TFunction<bool(UPackage*)> Callback = [&bHasDirtyContentPackage, &ShouldIgnorePackageFunction](UPackage* DirtyPackage)
	{
		if (!ShouldIgnorePackageFunction(DirtyPackage))
		{
			bHasDirtyContentPackage = true;
			return false;
		}
		return true;
	};

	EnumerateDirtyContentPackages(Callback);

	return bHasDirtyContentPackage;
}
...
void FEditorFileUtils::EnumerateDirtyContentPackages(const FEnumeratePackageFunctionRef& Callback)
{
	FAssetToolsModule& AssetToolsModule = FModuleManager::LoadModuleChecked<FAssetToolsModule>("AssetTools");
	const TSharedRef<FPathPermissionList>& WritableFolderFilter = AssetToolsModule.Get().GetWritableFolderPermissionList();
	const bool bHasWritableFolderFilter = WritableFolderFilter->HasFiltering();
	...
	for (TObjectIterator<UPackage> It; It; ++It)
	{
		...
		if (Package->HasAnyPackageFlags(PKG_CompiledIn | PKG_PlayInEditor | PKG_ContainsMapData))
		{
			continue;
		}
		// Ignore packages that haven't been modified.
		...
		const UObject* const Asset = Package->FindAssetInPackage();
		if (Asset != nullptr)
		{
			// Ignore map packages, they are caught above.
			if (Cast<const UWorld>(Asset) != nullptr)
			{
				continue;
			}

			// Ignore external actors, they are caught alongside maps
			if (Asset->GetTypedOuter<UWorld>() != nullptr)
			{
				continue;
			}
		}

		const FNameBuilder PackageNameBuilder(Package->GetFName());
		const FStringView PackageName = PackageNameBuilder.ToView();

		// Ignore packages with long, invalid names. This culls out packages with paths in read-only roots such as /Temp.
		if (!FPackageName::IsShortPackageName(PackageName) && !FPackageName::IsValidLongPackageName(PackageName, /*bIncludeReadOnlyRoots=*/false))
		{
			continue;
		}

		// Ignore packages that cannot be saved due to a custom filter
		if (bHasWritableFolderFilter && !WritableFolderFilter->PassesStartsWithFilter(PackageName))
		{
			continue;
		}
...
bool FEditorFileUtils::HasDirtyPackages(const FEditorFileUtils::FShouldIgnorePackageFunctionRef& ShouldIgnorePackageFunction)
{
	return HasDirtyWorldPackages(ShouldIgnorePackageFunction) || HasDirtyContentPackages(ShouldIgnorePackageFunction);
}
...
static bool InternalCheckoutAndSavePackages(const TArray<UPackage*>& PackagesToSave)
{
	...
	{
		UE_LOGF(LogFileHelpers, Log, "%ls", *LOCTEXT("NoAssetsToSave", "All files are already saved.").ToString());
	}
