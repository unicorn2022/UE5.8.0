...
FWidget::FWidget()
{
	...
	ScreenAxisColor  = FLinearColor(0.76f, 0.72f, 0.14f);
	...
	FRotator& OutRotation)
{
	...
	else if (CurrentAxis == EAxisList::XYZ) //arcball rotate
	{
		...
		if (!FMath::IsNearlyZero(Length)) //degenerate check
		{
			...
			{
				const float ScaleInScreen = InView->WorldToScreen(InViewportClient->GetWidgetLocation()).W * (4.0f / InView->UnscaledViewRect.Width() / InView->ViewMatrices.GetViewToClip().M[0][0]);
			...
			else if (Distance > CircleRadius && Distance < MaxDiff) //do 
			{
				const float Scale = ScreenLocation.W *
					(4.0f / InView->UnscaledViewRect.Width() / InView->ViewMatrices.GetViewToClip().M[0][0]);
                                                   ...
                                                   FAbsoluteMovementParams& InOutParams, FVector& OutDrag)
{
	...
	case EAxisList::Screen:
		InOutParams.XAxis = InView->ViewMatrices.GetWorldToView().GetColumn(0);
		InOutParams.YAxis = InView->ViewMatrices.GetWorldToView().GetColumn(1);
		InOutParams.ZAxis = InView->ViewMatrices.GetWorldToView().GetColumn(2);
		GetPlaneNormalAndMask(InOutParams.ZAxis, InOutParams.PlaneNormal, InOutParams.NormalToRemove);
...
{
	return EditorModeTools ? (EditorModeTools->IsDefaultModeActive() && GEditor->HasSelectedMovementLockedItems()) : false;
}
