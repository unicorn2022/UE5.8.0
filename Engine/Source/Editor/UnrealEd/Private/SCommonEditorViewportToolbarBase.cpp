...
TSharedRef<SWidget> SCommonEditorViewportToolbarBase::GenerateViewModeOptionsMenu() const
{
	...
	TSharedRef<SEditorViewport> ViewportRef = GetInfoProvider().GetViewportWidget();
	TSharedRef<FEditorViewportClient> ViewClient = GetInfoProvider().GetViewportWidget()->GetViewportClient().ToSharedRef();
	const UWorld* World = ViewClient->GetWorld();
	
	FMenuBuilder MenuBuilder( true, ViewportRef->GetCommandList() );
	const UE::UnrealEd::FViewModeOptionsArgs ViewModeArgs
	{
		.ViewModeIndex = ViewClient->GetViewMode(),
		.FeatureLevel = World ? World->GetFeatureLevel() : GMaxRHIFeatureLevel,
		.ParamNameMap = ViewClient->GetViewModeParamNameMap(),
		.ViewportClient = ViewClient
	};
