...
void SCurveEditor::LogAndToastCurveTimeWarning(FRealCurve* Curve)
{
	...
	UE_LOGF(LogCurveEditor, Warning, "%ls", *Error.ToString());
}
...
{	
	FSelectedCurveKey SelectedKey(NULL,FKeyHandle::Invalid());
...
void SCurveEditor::OnBakeCurveSampleRateCommitted(const FText& InText, ETextCommit::Type CommitInfo)
{
	...
	if (CommitInfo == ETextCommit::OnEnter)
	{
		...
		{
			UE_LOGF(LogCurveEditor, Error, "Invalid Bake Sample Rate");
			return;
		}
		...
		if (CurveRangeMap.Num())
		{
			for (auto CurveToBake : CurveRangeMap)
			{
				...
				{
					UE_LOGF(LogCurveEditor, Warning, "Unable to bake single-point curve. Check if you don't have a single key selected before Baking.");
				}
			}
		}
		else
