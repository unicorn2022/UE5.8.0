...
void UAutomatedAssetImportData::Initialize(TSharedPtr<FJsonObject> InImportGroupJsonData)
{
		...
		if(!FactoryName.IsEmpty())
		{
			...
			if(FactoryClass)
			{
				...
				if(NewFactory->bEditorImport)
				{
					...
					{
						UE_LOGF(LogAutomatedImport, Error, "Factory %ls could not import one or more of the source files", *FactoryName);
						for(const FString& InvalidFiles : InvalidFilesForFactory)
						{
							UE_LOGF(LogAutomatedImport, Error, "    %ls", *InvalidFiles);
						}
					}
					else
					{
			...
			{
				UE_LOGF(LogAutomatedImport, Error, "Factory %ls could not be found", *FactoryName);
			}
		}
		else
		{
			UE_LOGF(LogAutomatedImport, Log, "Factory was not specified, will be set automatically");
		}
	}
	...
	if(!DestinationPath.IsEmpty() && FPackageName::GetPackageMountPoint(DestinationPath) == NAME_None)
	{
		...
		UE_LOGF(LogAutomatedImport, Warning, "DestinationPath has no valid mount point.  Assuming /Game is the mount point");
	}
	...
	{
		UE_LOGF(LogAutomatedImport, Error, "Invalid Destination Path (%ls): %ls", *DestinationPath, *FailureReason);
