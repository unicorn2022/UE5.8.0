...
#include "Elements/Interfaces/TypedElementEditorVisualizerInterface.h"
...
void UUnrealEdEngine::OnEditorElementSelectionChanged(const UTypedElementSelectionSet* SelectionSet)
{
	...
	// Try getting visualizers to use from typed elements. This could be used as the sole path instead
	//  of doing the above if we choose to implement the visualization interface for actors and components. 
	//  For now, however, it is used for non-actors that implement their visualizations via bridge/hack actors.
	LevelEditorSelectionSet->ForEachSelectedElement<ITypedElementEditorVisualizerInterface>(
		[this, SelectionSet](const TTypedElement<ITypedElementEditorVisualizerInterface>& Element)
	{
		TArray<ITypedElementEditorVisualizerInterface::FComponentVisualizerForSelection> Visualizers;
		if (Element.GetComponentVisualizersToUse(SelectionSet, Visualizers))
		{
			for (ITypedElementEditorVisualizerInterface::FComponentVisualizerForSelection& Visualizer : Visualizers)
			{
				// We'll avoid adding visualizers that target the same component since we don't currently remove
				//  any conflicting ones from regular component/actor selection
				bool bComponentAlreadyTargeted = VisualizersForSelection.ContainsByPredicate([&Visualizer](const FComponentVisualizerForSelection& Existing)
					{ return Existing.ComponentVisualizer.ComponentPropertyPath == Visualizer.ComponentVisualizer.ComponentPropertyPath; });

				if (!bComponentAlreadyTargeted)
				{
					VisualizersForSelection.Add({ Visualizer.ComponentVisualizer, Visualizer.IsEnabledDelegate });
				}
			}//end for each returned visualizer
		}
