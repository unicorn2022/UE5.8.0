...
static FName GetTextureClassIconName( ETextureClass TC )
{
	...
	static const FName NameTexture2dArray("ClassIcon.Texture2DArray");
	static const FName NameTextureCube("ClassIcon.TextureCube");
	static const FName NameTextureCubeArray("ClassIcon.TextureCubeArray");
	static const FName NameVolume("ClassIcon.VolumeTexture");
	static const FName NameRenderTarget("ClassIcon.TextureRenderTarget2D");
	static const FName NameDefault("ClassIcon.Default");

	switch(TC)
	{
	case ETextureClass::TwoD: return NameTexture2d;
	case ETextureClass::Cube: return NameTextureCube;
	case ETextureClass::Array: return NameTexture2dArray;
	case ETextureClass::CubeArray: return NameTextureCubeArray;
	case ETextureClass::Volume: return NameVolume;
	case ETextureClass::TwoDDynamic: return NameTexture2d;
	case ETextureClass::RenderTarget: return NameRenderTarget;
	case ETextureClass::Other2DNoSource: return NameTexture2d;
										...
										const FString & DescriptionString)
{
	...
	FSlateIcon ClassIcon = FSlateIconFinder::FindIconForClass(Texture->GetClass());
	if (!ClassIcon.IsSet())
	{
		FName TextureClassIconName = GetTextureClassIconName(Texture->GetTextureClass());
		ClassIcon = FSlateIcon(FAppStyle::GetAppStyleSetName(), TextureClassIconName);
	}
...
bool ResizeTextureSourceToPowerOfTwo(UTexture * Texture)
{
	...
	{
		UE_LOGF(LogTexture, Display, "Texture (%ls) did not resize.", *Texture->GetName());
	...
	UE_LOGF(LogTexture, Display, "Texture (%ls) did resize Before Size=%dx%d After Size=%dx%d", *Texture->GetName(),
...
bool ResizeTextureSource(UTexture * Texture,int TargetSize)
{
	...
	const FIntPoint BeforeSourceSize = Texture->Source.GetLogicalSize();
	UE_LOGF(LogTexture, Display, "Texture (%ls) Resizing to <= %d Source Size=%dx%d Built Size=%dx%d", *Texture->GetName() , TargetSize,
	...
	UE_LOGF(LogTexture, Display, "Texture (%ls) did resize Source Size=%dx%d Built Size=%dx%d", *Texture->GetName(),
	...
	if ( BeforeSizeX != AfterSizeX || BeforeSizeY != AfterSizeY )
	{
		...
		UE_LOGF(LogTexture,Display,"DoResizeTextureSource failed to preserve built size; was: %dx%d now: %dx%d on [%ls]",
...
bool ConvertTo8bitTextureSource(UTexture * Texture,bool NormalMapsKeep16bits)
{
	...
	UE_LOGF(LogTexture, Display, "Texture (%ls) changing format from %ls to %ls, TC = %ls", 
		*Texture->GetName() ,
...
bool CompressTextureSourceWithJPEG(UTexture * Texture)
{
	...
	UE_LOGF(LogTexture, Display, "Texture (%ls) %ls", 
		*Texture->GetName() ,
...
void STextureAssetList::DoAction()
{
	...
	for (TextureListEntry & Entry : TextureList)
	{
		...
		UE_LOGF(LogTexture, Display, "Texture (%ls) Enabled=%d", *Texture->GetName() , (int)Entry.IsEnabled);
	...
	for (TextureListEntry & Entry : TextureList)
	{
		...
		case ETextureAction::Resize:
			UE::TextureAssetActionsInternal::ResizeTextureSource(Texture,ThresholdValue);
			break;
		case ETextureAction::ResizePow2:
			UE::TextureAssetActionsInternal::ResizeTextureSourceToPowerOfTwo(Texture);
			break;
		case ETextureAction::ConvertTo8bit:
			UE::TextureAssetActionsInternal::ConvertTo8bitTextureSource(Texture,bNormalMapsKeep16bits);
			break;
		case ETextureAction::JPEG:
			UE::TextureAssetActionsInternal::CompressTextureSourceWithJPEG(Texture);
			break;
		default:
			check(0);
			break;
...
static EAssetActionStatus GetTextureFilterStatus(UTexture * Texture, ETextureAction Action, int ThresholdValue,
		...
		FString & DescriptionString )
{
	...
	else
	{
		...
		else if ( Action == ETextureAction::ConvertTo8bit )
		{
			...
			{
				// @@ NOTE : ThresholdValue not checked!
		...
		else if ( Action == ETextureAction::JPEG )
		{
			...
			{
				// hard-coded requirement of at least 16 in both dimensions for JPEG
				Status = EAssetActionStatus::UnderSized;
			}
...
bool GetTextureFilterEnabled( UTexture * Texture, ETextureAction Action, int ThresholdValue,
		bool bNormalMapsKeep16bits , bool * bOutExcludedBecauseAlreadySatisfied)
{
	FString EnabledActionString;
	FString DescriptionString;

	EAssetActionStatus Status = GetTextureFilterStatus( Texture, Action, ThresholdValue, bNormalMapsKeep16bits,
		EnabledActionString, DescriptionString );

	// Special returns for the exclusions that are due to the target already being in the desired state
	//  return another bool or trinary enum to indicate these

	bool bDummy;
	if ( bOutExcludedBecauseAlreadySatisfied == nullptr )
	{
		bOutExcludedBecauseAlreadySatisfied = &bDummy;
	}

	if ( Action == ETextureAction::Resize && Status == EAssetActionStatus::UnderSized )
	{
		*bOutExcludedBecauseAlreadySatisfied = true;
		return false;
	}
	if ( Action == ETextureAction::ResizePow2 && Status == EAssetActionStatus::AlreadyPow2 )
	{
		*bOutExcludedBecauseAlreadySatisfied = true;
		return false;
	}
	if ( Action == ETextureAction::ConvertTo8bit && Status == EAssetActionStatus::Already8bit )
	{
		*bOutExcludedBecauseAlreadySatisfied = true;
		return false;
	}
	if ( Action == ETextureAction::JPEG && Status == EAssetActionStatus::DontChangeJPEG )
	{
		*bOutExcludedBecauseAlreadySatisfied = true;
		return false;
	}
	
	*bOutExcludedBecauseAlreadySatisfied = false;

	return ( Status == EAssetActionStatus::Enabled );
}
...
{
	UE::TextureAssetActionsInternal::STextureActionDlg Dlg(InTextures,UE::TextureAssetActionsInternal::ETextureAction::Resize);
	Dlg.ShowModal();
}

void UE::TextureAssetActions::TextureSource_ConvertTo8bit_WithDialog(const TArray<UTexture*> & InTextures)
{
	UE::TextureAssetActionsInternal::STextureActionDlg Dlg(InTextures,UE::TextureAssetActionsInternal::ETextureAction::ConvertTo8bit);
	Dlg.ShowModal();
}

void UE::TextureAssetActions::TextureSource_JPEG_WithDialog(const TArray<UTexture*> & InTextures)
{
	UE::TextureAssetActionsInternal::STextureActionDlg Dlg(InTextures,UE::TextureAssetActionsInternal::ETextureAction::JPEG);
	Dlg.ShowModal();
}

void UE::TextureAssetActions::TextureSource_ResizeToPowerOfTwo_WithDialog(const TArray<UTexture*> & InTextures)
{
	UE::TextureAssetActionsInternal::STextureActionDlg Dlg(InTextures,UE::TextureAssetActionsInternal::ETextureAction::ResizePow2);
	Dlg.ShowModal();
}
