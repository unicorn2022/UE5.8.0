...
void FStaticLightingManager::FailLightingBuild( FText ErrorText)
{
	...
	if (!bForceNoPrecomputedLighting)
	{
		...
		UE_LOGF(LogStaticLightingSystem, Warning, "Failed to build lighting!!! %ls",*ErrorText.ToString());
...
void FStaticLightingManager::FinishLightingBuild()
{
	...
	for (UStaticMeshComponent* StaticMeshComponent : TObjectRange<UStaticMeshComponent>())
	{
		...
		for (FStaticMeshComponentLODInfo& LODInfo : StaticMeshComponent->LODData)
		{
			...
			{
				UE_LOGF(LogEngine, Warning, "MapBuildDataID for %ls was updated during the static lighting build, marking package dirty (%ls)", *StaticMeshComponent->GetOwner()->GetActorNameOrLabel(), *StaticMeshComponent->GetFullName());
...
bool FStaticLightingSystem::BeginLightmassProcess()
{
...

	{
		...
		{
			UE_LOGF(LogStaticLightingSystem, Warning, "WorldSettings.bForceNoPrecomputedLighting is true, Skipping Lighting Build!");
		}
...

	{
		...
		for (int32 CheckMapIdx = 0; CheckMapIdx < Mappings.Num(); CheckMapIdx++)
		{
			if (Mappings[CheckMapIdx]->bProcessMapping)
			{
				...
				{
					UE_LOGF(LogStaticLightingSystem, Warning, "Lightmass: Error in deterministic lighting for %ls:%ls",
...
void FStaticLightingSystem::GatherStaticLightingInfo(bool bRebuildDirtyGeometryForLighting, bool bForceNoPrecomputedLighting)
{
	...
	for (int32 LevelIndex = 0; LevelIndex < LightingContext.World->GetNumLevels(); LevelIndex++)
	{
		...
		{
			UE_LOGF(LogStaticLightingSystem, Warning, "WARNING: Lighting build detected that geometry needs to be rebuilt to avoid incorrect lighting (due to modifying a lighting property).");
			if (bRebuildDirtyGeometryForLighting)
			{
				// This will go ahead and clean up lighting on all dirty levels (not just this one)
				UE_LOGF(LogStaticLightingSystem, Warning, "WARNING: Lighting build automatically rebuilding geometry." );
		...
		for (int32 LevelIndex = 0; LevelIndex < LightingContext.World->GetNumLevels(); LevelIndex++)
		{
...

			{
				...
				if (NumMeshes > 1)
				{
					...
					UE_LOGF(LogStaticLightingSystem, Log, "%ls", *StatsString);
				}
			}
...
void FStaticLightingSystem::ReportStatistics()
{
	...
	if ( GLightmassStatsMode )
	{
		...
		UE_LOGF(LogStaticLightingSystem, Log,
			"Illumination: %ls total\n" "   %3.1f%%\t%8.1fs    Untracked time\n"
			, *FPlatformTime::PrettyTime(LightmassStatistics.TotalTime)
			, UntrackedTime / LightmassStatistics.TotalTime * 100.0
			, UntrackedTime
		);
		UE_LOGF(LogStaticLightingSystem, Log,
			"Breakdown of Illumination time\n" "   %3.1f%%\t%8.1fs \tStarting up\n" "   %3.1f%%\t%8.1fs \tCollecting\n" "   %3.1f%%\t%8.1fs \t--> Preparing lights\n" "   %3.1f%%\t%8.1fs \t--> Gathering lighting info\n" "   %3.1f%%\t%8.1fs \tProcessing\n" "   %3.1f%%\t%8.1fs \tImporting\n" "   %3.1f%%\t%8.1fs \tApplying\n" "   %3.1f%%\t%8.1fs \tEncoding\n" "   %3.1f%%\t%8.1fs \tInvalidating\n" "   %3.1f%%\t%8.1fs \tFinishing\n"
		...
		UE_LOGF(LogStaticLightingSystem, Log,
			"Breakdown of Processing time\n" "   %3.1f%%\t%8.1fs \tCollecting Lightmass scene\n" "   %3.1f%%\t%8.1fs \tExporting\n" "   %3.1f%%\t%8.1fs \tLightmass\n" "   %3.1f%%\t%8.1fs \tSwarm startup\n" "   %3.1f%%\t%8.1fs \tSwarm callback\n" "   %3.1f%%\t%8.1fs \tSwarm job open\n" "   %3.1f%%\t%8.1fs \tSwarm job close\n" "   %3.1f%%\t%8.1fs \tImporting\n" "   %3.1f%%\t%8.1fs \tApplying\n"
		...
		UE_LOGF(LogStaticLightingSystem, Log,
			"Breakdown of Export Times\n" "   %8.1fs\tVisibility Data\n" "   %8.1fs\tVolumetricLightmap Data\n" "   %8.1fs\tLights\n" "   %8.1fs\tModels\n" "   %8.1fs\tStatic Meshes\n" "   %8.1fs\tMaterials\n" "   %8.1fs\tMesh Instances\n" "   %8.1fs\tLandscape Instances\n" "   %8.1fs\tMappings\n"
		...
		UE_LOGF(LogStaticLightingSystem, Log, "Lightmap textures: %.1f M texels (%.1f%% mapped, %.1f%% unmapped, %.1f%% wasted by packing, %.1f M non-pow2 texels)"
		...
		UE_LOGF(LogStaticLightingSystem, Log, "Shadowmap textures: %.1f M texels (%.1f%% mapped, %.1f%% unmapped, %.1f%% wasted by packing)"
		...
		{
			ULevel* Level = LightingContext.World->GetLevel(LevelIndex);
			UE_LOGF(LogStaticLightingSystem, Log,  "Level %2d - Lightmaps: %.1f MB. Shadowmaps: %.1f MB.", LevelIndex, Level->LightmapTotalSize/1024.0f, Level->ShadowmapTotalSize/1024.0f );
		}
	}
	else	//if ( GLightmassStatsMode)
	{
		UE_LOGF(LogStaticLightingSystem, Log, "Illumination: %ls (%ls encoding lightmaps, %ls encoding shadowmaps)", *FPlatformTime::PrettyTime(LightmassStatistics.TotalTime), *FPlatformTime::PrettyTime(LightmassStatistics.EncodingLightmapsTime), *FPlatformTime::PrettyTime(LightmassStatistics.EncodingShadowMapsTime));
	}
	UE_LOGF(LogStaticLightingSystem, Log, "Lightmap texture memory:  %.1f MB (%.1f MB streaming, %.1f MB non-streaming), %d textures",
	...
	UE_LOGF(LogStaticLightingSystem, Log, "Shadowmap texture memory: %.1f MB (%.1f MB streaming, %.1f MB non-streaming), %d textures",
...
void FStaticLightingSystem::CompleteDeterministicMappings(class FLightmassProcessor* InLightmassProcessor)
{
	...
	for (int32 MappingIndex = 0; MappingIndex < Mappings.Num(); MappingIndex++)
	{
		...
		if (TextureMapping)
		{
			//UE_LOGF(LogStaticLightingSystem, Log, "%32ls Completed - %ls", *(TextureMapping->GetDescription()), *(TextureMapping->GetLightingGuid().ToString()));
...
void FStaticLightingSystem::AddPrimitiveStaticLightingInfo(FStaticLightingPrimitiveInfo& PrimitiveInfo, bool bBuildActorLighting, bool bDeferMapping)
{
	...
	for(int32 MappingIndex = 0;MappingIndex < PrimitiveInfo.Mappings.Num();MappingIndex++)
	{
		...
		if (GbLogAddingMappings)
		{
			...
			if (SLMesh)
			{
				//UE_LOGF(LogStaticLightingSystem, Log, "Adding %32ls: 0x%08p - %ls", *(CurrentMapping->GetDescription()), (PTRINT)(SLMesh->Component), *(SLMesh->Guid.ToString()));
			}
			else
			{
				//UE_LOGF(LogStaticLightingSystem, Log, "Adding %32ls: 0x%08x - %ls", *(CurrentMapping->GetDescription()), 0, TEXT("NO MESH????"));
			}
		}
...
bool FStaticLightingSystem::CreateLightmassProcessor()
{
	...
	{
		UE_LOGF(LogStaticLightingSystem, Warning, "Failed to initialize Swarm.");
	...
	{
		UE_LOGF(LogStaticLightingSystem, Warning, "Failed to connect to Swarm.");
		FMessageDialog::Open( EAppMsgType::Ok, 
#if USE_LOCAL_SWARM_INTERFACE
...
void FStaticLightingSystem::GatherScene()
{
	...
	if (LightmassExporter->GetImportanceVolumes().Num() == 0)
	{
		...
		else
		{
			...
			UE_LOGF(LogStaticLightingSystem, Warning, "No importance volume found, so the scene bounding box was used.  You can optimize your scene's quality and lighting build times by adding importance volumes.");
...
bool FStaticLightingSystem::InitiateLightmassProcessor()
{
	...
	if ( !GEditor->GetMapBuildCancelled() )
	{
		UE_LOGF(LogStaticLightingSystem, Log, "Running Lightmass w/ ImmediateImport mode %ls", GLightmassDebugOptions.bUseImmediateImport ? TEXT("ENABLED") : TEXT("DISABLED"));
		LightmassProcessor->SetImportCompletedMappingsImmediately(GLightmassDebugOptions.bUseImmediateImport);
		UE_LOGF(LogStaticLightingSystem, Log, "Running Lightmass w/ ImmediateProcess mode %ls", GLightmassDebugOptions.bImmediateProcessMappings ? TEXT("ENABLED") : TEXT("DISABLED"));
		UE_LOGF(LogStaticLightingSystem, Log, "Running Lightmass w/ Sorting mode %ls", GLightmassDebugOptions.bSortMappings ? TEXT("ENABLED") : TEXT("DISABLED"));
		UE_LOGF(LogStaticLightingSystem, Log, "Running Lightmass w/ Mapping paddings %ls", GLightmassDebugOptions.bPadMappings ? TEXT("ENABLED") : TEXT("DISABLED"));
		UE_LOGF(LogStaticLightingSystem, Log, "Running Lightmass w/ Mapping debug paddings %ls", GLightmassDebugOptions.bDebugPaddings ? TEXT("ENABLED") : TEXT("DISABLED"));
