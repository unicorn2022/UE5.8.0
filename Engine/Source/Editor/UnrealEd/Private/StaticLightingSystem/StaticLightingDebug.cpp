...
static void WriteTexel(UTexture2D* Texture, int32 X, int32 Y, FColor NewColor)
{
	if (X >= 0 && X < Texture->GetSizeX() && Y >= 0 && Y < Texture->GetSizeY())
	{
		...
		{
			UE_LOGF(LogStaticLightingSystem, Log, "Texel selection coloring failed because the lightmap is not PF_B8G8R8A8!");
		}
	}
}
	...
	int32 LightmapSizeX, int32 LightmapSizeY)
{
	...
	else
	{
		...
		{
			UE_LOGF(LogStaticLightingSystem, Log, "Texel selection failed because the lightmap is an invalid reference!");
			return false;
		}
	}
}
			...
			if (bHasStaticLighting)
			{
				...
				if (bUseTextureMap)
				{
					...
					if (ClosestPlaneTriangleIndex != -1)
					{
						...
						{
							UE_LOGF(LogStaticLightingSystem, Log, "Texel selection failed because the lightmap UV's wrap!");
						}
						else
						{
				...
				if (!bFoundLightmapSample && Indices.Num() > 0)
				{
					...
					UE_LOGF(LogStaticLightingSystem, Log, "Failed to intersect any triangles, picking random texel");
