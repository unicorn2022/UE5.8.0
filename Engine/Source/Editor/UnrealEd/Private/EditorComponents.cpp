...
void FGridWidget::DrawNewGrid(const FSceneView* View, FPrimitiveDrawInterface* PDI)
{
	...
	bool bIsPerspective = ( View->ViewMatrices.GetViewToClip().M[3][3] < 1.0f );
	...
	if(!bIsPerspective)
	{
		...
		float ScaleX = static_cast<float>(View->ViewMatrices.GetViewToClip().M[0][0] * View->UnscaledViewRect.Width());
		float ScaleY = static_cast<float>(View->ViewMatrices.GetViewToClip().M[1][1] * View->UnscaledViewRect.Height());
	...
	if(!bIsPerspective)
	{
		...
		if(View->ViewMatrices.GetWorldToView().M[1][1] == -1.f )		// Top
		{
			ObjectToWorld.SetOrigin(FVector(CameraPos.X, CameraPos.Y, -FarZ));
		}
		if(View->ViewMatrices.GetWorldToView().M[1][2] == -1.f )		// Front
		{
		...
		else if(View->ViewMatrices.GetWorldToView().M[1][0] == 1.f )		// Side
		{
	...
	{
		double ScaleX = View->ViewMatrices.GetViewToClip().M[0][0];
		double ScaleY = View->ViewMatrices.GetViewToClip().M[1][1];
...
void FEditorCommonDrawHelper::DrawOldGrid(const FSceneView* View,FPrimitiveDrawInterface* PDI)
{
	...
	{
		const bool bIsOrthoXY = ( FMath::Abs(View->ViewMatrices.GetWorldToView().M[2][2]) > 0 );
		const bool bIsOrthoXZ = ( FMath::Abs(View->ViewMatrices.GetWorldToView().M[1][2]) > 0 );
		const bool bIsOrthoYZ = ( FMath::Abs(View->ViewMatrices.GetWorldToView().M[0][2]) > 0 );
		...
		if (bIsOrthoXY)
		{
			const bool bNegative = View->ViewMatrices.GetWorldToView().M[2][2] > 0;
		...
		{
			const bool bNegative = View->ViewMatrices.GetWorldToView().M[1][2] > 0;
		...
		{
			const bool bNegative = View->ViewMatrices.GetWorldToView().M[0][2] < 0;
...
{
	if (FMath::IsNearlyZero(ViewportGridY))
	{
	...
	const double Zoom = (1.0 / View->ViewMatrices.GetViewToClip().M[0][0]) * 2.0 / SizeX;
	const double Dist = SizeX * Zoom / ViewportGridY;

	// when the grid fades
	constexpr double Tweak = 6.0;
	const int32 Exponent = GEditor->IsGridSizePowerOfTwo() ? 8 : 10;
	const double IncValue = FMath::LogX(Exponent, Dist / (double)(SizeX / Tweak));
	int32 IncScale = 1;
	...
	const FMatrix InvViewProjMatrix = View->ViewMatrices.GetClipToWorld();
	...
	// Check if we're in a mode that is not using the view background (uses lighting)
	const FLinearColor Background = View->BackgroundColor;
	bool bIsLitMode = Background.GetLuminance() > 0.5;
	if (View->Family 
		&& View->Family->EngineShowFlags.Lighting > 0 
		&& View->Family->EngineShowFlags.LightingOnlyOverride == 0)
	{
		bIsLitMode = true;
	}

	// Reduce grid density when further away by removing minor lines
	const double Fract = IncValue - FMath::FloorToDouble(IncValue);
	const float MinorLineAlpha = Fract < 0.5 || Zoom < 1.f ? 0.1 : 0;
	constexpr float MajorLineAlpha = 0.25f;

	// Use contrasting color based on lit/unlit mode
	const FLinearColor LineColor = bIsLitMode ? FLinearColor::Black : FLinearColor::White;
	...
	for (int32 LineIndex = FirstLineClamped; LineIndex <= LastLineClamped; ++LineIndex)
	{
		...
		*BX = FPlatformMath::TruncToDouble(LineIndex * ViewportGridY) * IncScale;
		...
		// Only minor lines fade out with ortho zoom distance
		...
		if (bIsMajorLine)
		{
			Color = FMath::Lerp(FLinearColor::Transparent, LineColor, MajorLineAlpha);
		}
		else
		{
			Color = FMath::Lerp(FLinearColor::Transparent, LineColor, MinorLineAlpha);
		}

		PDI->DrawTranslucentLine(*A, *B, Color, SDPG_World);
	}
}
...
{
	const FMatrix CameraToWorld = View->ViewMatrices.GetViewToWorld();

	const FVector PivLoc = GLevelEditorModeTools().SnappedLocation;

	const FVector::FReal ZoomFactor = FMath::Min<FVector::FReal>(View->ViewMatrices.GetViewToClip().M[0][0], View->ViewMatrices.GetViewToClip().M[1][1]);
	const FVector::FReal WidgetRadius = View->ViewMatrices.GetWorldToClip().TransformPosition(PivLoc).W * (PivotSize / ZoomFactor);
