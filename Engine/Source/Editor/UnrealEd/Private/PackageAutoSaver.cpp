...
void FPackageAutoSaver::ResumeAutoSave()
{
	--AutoSaveSuspendedCounter;
	if (!ensureMsgf(AutoSaveSuspendedCounter >= 0, TEXT("FPackageAutoSaver::ResumeAutoSave() called more times than FPackageAutoSaver::SuspendAutoSave(), those calls should come in pair.")))
	{
		AutoSaveSuspendedCounter = 0;
	}
}
...
bool FPackageAutoSaver::AttemptAutoSave(const bool bForceAutoSave)
{
	...
	const bool bTimeToAutosave = bForceAutoSave || (!IsAutoSaveSuspended() && LoadingSavingSettings->bAutoSaveEnable && AutoSaveCount >= LoadingSavingSettings->AutoSaveTimeMinutes * 60.0f);
							...
							if (Provider.IsEnabled())
							{
								...
								if (Provider.GetState(SavedPackageFilenames, FileStates, EStateCacheUsage::ForceUpdate) == ECommandResult::Succeeded)
								{
									...
									if (FilesToRevertAndDelete.Num() > 0)
									{
										...
										RevertOperation->SetSoftRevert(Provider.UsesSoftRevertOnDelete());
										Provider.Execute(RevertOperation, FilesToRevertAndDelete);

										TSharedRef<FDelete, ESPMode::ThreadSafe> DeleteOperation = ISourceControlOperation::Create<FDelete>();
										Provider.Execute(DeleteOperation, FilesToRevertAndDelete);
			...
			UncontrolledPackages.Num() > 0)
		{
			auto FindAndDirtyPackage = [this](const FName PackageName)
			{
					...
					IsValid(Pkg) && !Pkg->IsDirty())
				{
					...
					UE_LOGF(LogPackageAutoSaver, Log, "Package '%ls' was marked dirty by the auto-save uncontrolled changelist", FNameBuilder(Pkg->GetFName()).ToString());
				}
			};
...
FPackageAutoSaverScopeGuard::FPackageAutoSaverScopeGuard()
{
	if (GUnrealEd)
	{
...
FPackageAutoSaverScopeGuard::FPackageAutoSaverScopeGuard(FPackageAutoSaverScopeGuard&& Other)
	: bSuspended(Other.bSuspended)
{
	Other.bSuspended = false;
}

FPackageAutoSaverScopeGuard::~FPackageAutoSaverScopeGuard()
{
	if (bSuspended && GUnrealEd)
	{
		GUnrealEd->GetPackageAutoSaver().ResumeAutoSave();
	}
}

FPackageAutoSaverScopeGuard& FPackageAutoSaverScopeGuard::operator=(FPackageAutoSaverScopeGuard&& Other)
{
	bSuspended = Other.bSuspended;
	Other.bSuspended = false;
	return *this;
