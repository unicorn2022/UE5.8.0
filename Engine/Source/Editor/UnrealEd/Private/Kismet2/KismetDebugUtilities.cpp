...
void FKismetDebugUtilities::AttemptToBreakExecution(UBlueprint* BlueprintObj, const UObject* ActiveObject, const FFrame& StackFrame, const FBlueprintExceptionInfo& Info, UEdGraphNode* NodeStoppedAt, int32 DebugOpcodeOffset)
{
	...
	if (NodeStoppedAt != NULL)
	{
		...
		if (Info.GetType() == EBlueprintExceptionType::Breakpoint)
		{
			...
			if (Breakpoint != NULL)
			{
				...
				UE_LOGF(LogBlueprintDebug, Warning, "Hit breakpoint on node '%ls', from offset %d", *(NodeStoppedAt->GetDescriptiveCompiledName()), DebugOpcodeOffset);
				UE_LOGF(LogBlueprintDebug, Log, "\n%ls", *StackFrame.GetStackTrace());
			}
			else
			{
				UE_LOGF(LogBlueprintDebug, Warning, "Unknown breakpoint hit at node %ls in object %ls:%04X", *NodeStoppedAt->GetDescriptiveCompiledName(), *StackFrame.Node->GetFullName(), DebugOpcodeOffset);
			}
		}
	...
	else if(UEdGraphNode* PreviousNode = FKismetDebugUtilities::GetCurrentInstruction())
	{
		if (UK2Node_Message* MessageNode = Cast<UK2Node_Message>(PreviousNode))
		{
			...
			UE_LOGF(LogBlueprintDebug, Warning, "Can't break execution on function '%ls'. Possibly interface '%ls' in class '%ls' was not fully implemented.",
		...
		{
			UE_LOGF(LogBlueprintDebug, Warning, "Can't break execution on function '%ls'. Possibly it was not implemented in class '%ls'.",
				*(PreviousNode->GetDocumentationExcerptName()),					  //Function name
				*(ActiveObject->GetClass()->GetName()));						  //Current object class name
		}
	}
	...
	{
		UE_LOGF(LogBlueprintDebug, Warning, "Tried to break execution in an unknown spot at object %ls:%04llX", *StackFrame.Node->GetFullName(), StackFrame.Code - StackFrame.Node->Script.GetData());
	}
...
void FKismetDebugUtilities::CleanupBreakpoints(const UBlueprint* Blueprint)
{
	RemoveBreakpointsByPredicate(
		...
		[Blueprint](const FBlueprintBreakpoint& Breakpoint)
		{
			...
			{
				UE_LOGF(LogBlueprintDebug, Display, "Encountered a blueprint breakpoint in %ls without an associated node. The blueprint breakpoint has been removed", *Blueprint->GetPathName());
				return true;
			}
			return false;
		}
