...
UUserDefinedStruct* FStructureEditorUtils::CreateUserDefinedStruct(UObject* InParent, FName Name, EObjectFlags Flags)
{
	...
	if (UserDefinedStructEnabled())
	{
		...
		UUserDefinedStructEditorData* EditorData = NewObject<UUserDefinedStructEditorData>(Struct, NAME_None, RF_Transactional);
...
bool FStructureEditorUtils::AddVariable(UUserDefinedStruct* Struct, const FEdGraphPinType& VarType)
{
	if (Struct)
	{
		...
		{
			UE_LOGF(LogBlueprint, Warning, "%ls", *ErrorMessage);
			return false;
		}
...
bool FStructureEditorUtils::RemoveVariable(UUserDefinedStruct* Struct, FGuid VarGuid)
{
	if(Struct)
	{
		...
		{
			UE_LOGF(LogBlueprint, Log, "Member variable cannot be removed. User Defined Structure cannot be empty");
		}
	}
	return false;
}
...
{
	FBlueprintEditorUtils::RemoveInvalidStructureMemberVariableFromBlueprint(Blueprint);
}
