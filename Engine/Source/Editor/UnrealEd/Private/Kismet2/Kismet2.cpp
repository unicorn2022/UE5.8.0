...
namespace UE::Private
{
	...
	static bool ExternalDataLayerAssetCopyPropertyFilter(FProperty& Property, UObject& Object)
	{
		static FName ExternalDataLayerAssetPropertyName(AActor::GetExternalDataLayerAssetPropertyName());
		return !Object.IsA<AActor>() || (Property.GetFName() != ExternalDataLayerAssetPropertyName);
	};
}
...
void FKismetEditorUtilities::CompileBlueprint(UBlueprint* BlueprintObj, EBlueprintCompileOptions CompileFlags, FCompilerResultsLog* pResults)
{
		...
		&& !EnumHasAnyFlags(CompileFlags, EBlueprintCompileOptions::IsRegeneratingOnLoad))
	{
		...
		UE_LOGF(LogBlueprint, Log, "Compiling Blueprint '%ls'", *GetPathNameSafe(BlueprintObj));
	}
	...
	if (Actor != nullptr)
	{
		...
		if (NewBlueprint != nullptr)
		{
			...
			if (NewBlueprint->GeneratedClass != nullptr)
			{
				...
				const EditorUtilities::ECopyOptions::Type CopyOptionFlags = (EditorUtilities::ECopyOptions::Type)(EditorUtilities::ECopyOptions::OnlyCopyEditOrInterpProperties | EditorUtilities::ECopyOptions::PropagateChangesToArchetypeInstances | EditorUtilities::ECopyOptions::SkipInstanceOnlyProperties);
				const EditorUtilities::FCopyOptions CopyOptions(CopyOptionFlags, UE::Private::ExternalDataLayerAssetCopyPropertyFilter); // Don't allow to copy ExternalDataLayerAsset property to NewBlueprint
...
void CreateChildActorComponentsForActors(const FBlueprintAssemblyProps& AssemblyProps)
{
	...
	{
		CAC->Rename(nullptr, nullptr, REN_DoNotDirty | REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
	}
}
	...
	if (Actor != nullptr && Blueprint != nullptr)
	{
		...
		FRestoreSelectedInstanceComponent RestoreSelectedInstanceComponent;
		{
			...
			// Perform the actual copy
			{
				...
				{
					const EditorUtilities::ECopyOptions::Type CopyOptionFlags = (EditorUtilities::ECopyOptions::Type)(EditorUtilities::ECopyOptions::OnlyCopyEditOrInterpProperties | EditorUtilities::ECopyOptions::PropagateChangesToArchetypeInstances | EditorUtilities::ECopyOptions::SkipInstanceOnlyProperties);
					const EditorUtilities::FCopyOptions CopyOptions(CopyOptionFlags, UE::Private::ExternalDataLayerAssetCopyPropertyFilter); // Don't allow to copy ExternalDataLayerAsset property to BlueprintCDO
...
void FKismetEditorUtilities::UpgradeCosmeticallyStaleBlueprint(UBlueprint* Blueprint)
{
	...
	if (FBlueprintEditorUtils::DoesSupportEventGraphs(Blueprint))
	{
		...
		if ((OldStateGraph != NULL) && (CollidingObject == NULL))
		{
			...
			OldStateGraph->Rename(*(UEdGraphSchema_K2::GN_EventGraph.ToString()), OldStateGraph->GetOuter(), REN_DoNotDirty | REN_AllowPackageLinkerMismatch);
			Blueprint->Status = BS_Dirty;
		}
	}
}

void FKismetEditorUtilities::CreateNewBoundEventForActor(AActor* Actor, FName EventName, bool bShouldJumpToNode)
{
		...
		if (FMulticastDelegateProperty* DelegateProperty = FindFProperty<FMulticastDelegateProperty>(Actor->GetClass(), EventName))
		{
			...
			if (ULevelScriptBlueprint* LSB = Actor->GetLevel()->GetLevelScriptBlueprint())
			{
				if (UEdGraph* TargetGraph = LSB->GetLastEditedUberGraph())
				{
					...
					if (EventNode && bShouldJumpToNode)
					{
...
void FKismetEditorUtilities::CreateNewBoundEventForComponent(UObject* Component, FName EventName, UBlueprint* Blueprint, FObjectProperty* ComponentProperty, bool bShouldJumpToNode)
{
	if ( Component != nullptr )
	{
		CreateNewBoundEventForClass(Component->GetClass(), EventName, Blueprint, ComponentProperty, bShouldJumpToNode);
	}
}

void FKismetEditorUtilities::CreateNewBoundEventForClass(UClass* Class, FName EventName, UBlueprint* Blueprint, FObjectProperty* ComponentProperty, bool bShouldJumpToNode)
{
	if ( ( Class != nullptr ) && ( EventName != NAME_None ) && ( Blueprint != nullptr ) && ( ComponentProperty != nullptr ) )
	{
		...
		if ( DelegateProperty != nullptr )
		{
			...
			if ( TargetGraph != nullptr )
			{
				...
				// Finally, bring up kismet and jump to the new node
				if ( EventNode != nullptr && bShouldJumpToNode)
				{
					BringKismetToFocusAttentionOnObject(EventNode);
				}
			}
