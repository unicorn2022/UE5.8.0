...
void FEnumEditorUtils::UpdateAfterPathChanged(UEnum* Enum)
{
	...
	Enum->SetEnums(NewEnumeratorsNames, UEnum::ECppForm::Namespaced, UEnum::EUnderlyingType::uint8, EEnumFlags::None, UEnum::EAddMaxKeyIfMissing::Yes);
}
...
void FEnumEditorUtils::AddNewEnumeratorForUserDefinedEnum(UUserDefinedEnum* Enum)
{
	...
	const UEnum::EUnderlyingType UnderlyingType = Enum->GetUnderlyingType();
	Enum->SetEnums(Names, EnumType, UnderlyingType, EEnumFlags::None, UEnum::EAddMaxKeyIfMissing::Yes);
...
void FEnumEditorUtils::UpgradeDisplayNamesFromMetaData(UUserDefinedEnum* Enum)
{
	if (Enum)
	{
		...
		{
			UE_LOGF(LogClass, Warning, "Enum '%ls' was upgraded to use FText to store its display name data. Please re-save this asset to avoid issues with localization and determinstic cooking.", *Enum->GetPathName());
		}
#endif // USE_STABLE_LOCALIZATION_KEYS
	}
}
