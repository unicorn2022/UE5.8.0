...
{
	static void ForEachObjectReference(const UObject* RootObject, TFunctionRef<void(const UObject*)> Function);
}

void UE::Private::ForEachObjectReference(const UObject* RootObject, TFunctionRef<void(const UObject*)> Function)
{
	TSet<const UObject*> VisitedObjects;
	TArray<const UObject*> Queue;
	Queue.Add(RootObject);
	VisitedObjects.Add(RootObject);
	while(Queue.Num() != 0)
	{
		const UObject* CurrentObject = Queue.Pop();

		TArray<UObject*> References;
		FReferenceFinder Finder( References );
		Finder.FindReferences( const_cast<UObject*>(CurrentObject) );

		for(UObject* Obj: References) //-V1078
		{
			if(VisitedObjects.Contains(Obj))
			{
				continue;
			}
			if(Obj->IsIn(RootObject))
			{
				Queue.Add(Obj);
			}
...
void FBlueprintEditorUtils::RefreshAllNodes(UBlueprint* Blueprint)
{
	...
	{
		UE_LOGF(LogBlueprint, Warning, 
			"RefreshAllNodes was called on an invalid or incompletely loaded blueprint '%ls'", 
...
void FBlueprintEditorUtils::ReconstructAllNodes(UBlueprint* Blueprint)
{
	...
	{
		UE_LOGF(LogBlueprint, Warning,
			"ReconstructAllNodes was called on an invalid or incompletely loaded blueprint '%ls'",
			Blueprint ? *Blueprint->GetFullName() : TEXT("NULL"));
		return;
	}
...
void FBlueprintEditorUtils::RefreshExternalBlueprintDependencyNodes(UBlueprint* Blueprint, UStruct* RefreshOnlyChild)
{
	...
	{
		UE_LOGF(LogBlueprint, Warning,
			"RefreshExternalBlueprintDependencyNodes was called on an invalid or incompletely loaded blueprint '%ls'",
			Blueprint ? *Blueprint->GetFullName() : TEXT("NULL"));
		return;
	}
...
void FBlueprintEditorUtils::RemoveStaleFunctions(UBlueprintGeneratedClass* Class, UBlueprint* Blueprint)
{
	...
	if (Fn)
	{
		...
		const ERenameFlags RenFlags = REN_DontCreateRedirectors | REN_NonTransactional | REN_DoNotDirty | (Blueprint->bIsRegeneratingOnLoad ? REN_AllowPackageLinkerMismatch : 0);
...
void FBlueprintEditorUtils::PatchCDOSubobjectsIntoExport(UObject* PreviousCDO, UObject* NewCDO)
{
	struct PatchCDOSubobjectsIntoExport_Impl
	{
		static void PatchSubObjects(UObject* OldObj, UObject* NewObj)
		{
			...
			GetObjectsWithOuter(OldObj, OldSubObjects, EGetObjectsFlags::None);
...
{
	if (Blueprint->bSuppressStructurallyModified)
	{
		return;
	}
...
void FBlueprintEditorUtils::RecompileBeforeSaveIfNeeded(UBlueprint* Blueprint)
{
	if (Blueprint->GeneratedClass == nullptr)
	{
		return; // no class, nothing could be out of date.. it could be that we need to regenerate it here
	}

	if (Blueprint->Status == EBlueprintStatus::BS_UpToDate)
	{
		return; // the UDS editor marks blueprint as out of date, we can rely on that flag
	}

	if (Blueprint->CachedUDSDependencies.Num() == 0)
	{
		return;
	}

	// Detect references to out of date UUserDefinedStructs (or other UScriptStructs):
	TSet<const UScriptStruct*> OutOfDateStructs;
	UE::Private::ForEachObjectReference(Blueprint->GeneratedClass, [&OutOfDateStructs](const UObject*Obj){
		if (const UScriptStruct* Struct = Cast<UScriptStruct>(Obj))
		{
			const bool bStructOutOfDate = 
				(Struct->StructFlags & STRUCT_NewerVersionExists) != 0
				|| (!Struct->IsNative() && Struct->HasAnyFlags(RF_Transient));
			if (bStructOutOfDate)
			{
				OutOfDateStructs.Add(Struct);
			}
		}
	});
	const bool bHasOutOfDateStructs = OutOfDateStructs.Num() != 0;
	if (bHasOutOfDateStructs)
	{
		// full compile, really we just need to regenerate the class *and reinstance* if necessary,
		// compile is just a convenient way to do that:
...
bool FBlueprintEditorUtils::IsCompileOnLoadDisabled(UBlueprint* Blueprint)
{
	...
	if(bCompilationDisabled)
	{
		...
		if(!bHasWarned)
		{
			bHasWarned = true;
			UE_LOGF(LogBlueprint, Warning, 
				"Blueprints are loading but RecompileOnLoad is disabled - no bytecode will be available unless SkipByteCodeSerialization is disabled");
		}
	}
...
TNotNull<UBlueprint*> FBlueprintEditorUtils::FindBlueprintForNodeChecked(const UEdGraphNode* Node)
{
...
TNotNull<UBlueprint*> FBlueprintEditorUtils::FindBlueprintForGraphChecked(const UEdGraph* Graph)
{
...
UEdGraph* FBlueprintEditorUtils::CreateNewGraph(UObject* ParentScope, const FName& GraphName, TSubclassOf<class UEdGraph> GraphClass, TSubclassOf<class UEdGraphSchema> SchemaClass)
{
	...
	if (GraphName != NAME_None)
	{
		if (UObject* ExistingObject = FindObject<UObject>(ParentScope, *(GraphName.ToString())))
		{
			if (ExistingObject->IsA<UEdGraph>())
			{
				...
				ExistingObject->Rename(nullptr, ExistingObject->GetOuter(), REN_DoNotDirty | REN_AllowPackageLinkerMismatch);
			}
	...
	if (bRename)
	{
		NewGraph->Rename(*(GraphName.ToString()), ParentScope, REN_DoNotDirty | REN_AllowPackageLinkerMismatch);
	}
	return NewGraph;
}
	...
	for (UObject* TestOuter = GraphToRemove->GetOuter(); TestOuter; TestOuter = TestOuter->GetOuter())
	{
		if (TestOuter == Blueprint)
		{
			...
			if(Blueprint->MacroGraphs.Remove( GraphToRemove ) > 0 ) 
			{
				...
				GetObjectsWithOuter(Blueprint, Inners, EGetObjectsFlags::None);
...
void FBlueprintEditorUtils::RenameGraph(UEdGraph* Graph, const FString& NewNameStr)
{
	if (Graph)
	{
		...
		{
			RenameFlagsToApply |= REN_AllowPackageLinkerMismatch;
		}
		...
		auto PostValidateChildBlueprintLambda = [Blueprint, &OldGraphName, &NewGraphName, &RenameGraphLambda, &ModifiedBlueprints](UBlueprint* InChildBP, const FName /* InVariableName */, bool /* bValidatedVariable */)
		{
			...
			for (UEdGraph* FunctionGraph : InChildBP->FunctionGraphs)
			{
				...
				{
					RenameGraphLambda(FunctionGraph, OldGraphName, NewGraphName, (InChildBP->bIsRegeneratingOnLoad ? REN_AllowPackageLinkerMismatch : 0) | REN_DontCreateRedirectors);
				}
			}
...
void FBlueprintEditorUtils::RenameGraphWithSuggestion(class UEdGraph* Graph, TSharedPtr<class INameValidatorInterface> NameValidator, const FString& DesiredName )
{
	...
	Graph->Rename(*NewName, Graph->GetOuter(), (BP->bIsRegeneratingOnLoad ? REN_AllowPackageLinkerMismatch : 0) | REN_DontCreateRedirectors);
}
	...
	if (!NewName.IsNone() && !NewName.IsEqual(OldName, ENameCase::CaseSensitive))
	{
		...
		if (VarIndex != INDEX_NONE)
		{
...

			{
				...
				{
					UE_LOGF(LogBlueprint, Warning, "Could not find default value of renamed variable '%ls' (previously '%ls') in %ls", *NewName.ToString(), *OldName.ToString(), *GetPathNameSafe(Blueprint));
				}
...
bool FBlueprintEditorUtils::VerifyUserWantsToSearch(FText Reason, const FString& SearchString)
{
	...
	UE_LOGF(LogBlueprint, Warning, "%ls", *LocalizedMessage.ToString());
...
void FBlueprintEditorUtils::RemoveLocalVariable(UBlueprint* InBlueprint, const UStruct* InScope, const FName InVarName)
{
	...
	if(ScopeGraph)
	{
		...
		{
			UE_LOGF(LogBlueprint, Warning, "Could not find local variable '%ls'!", *InVarName.ToString());
		}
	}
}
...
void FBlueprintEditorUtils::RenameLocalVariable(UBlueprint* InBlueprint, const UStruct* InScope, const FName InOldName, const FName InNewName)
{
	if (!InNewName.IsNone() && !InNewName.IsEqual(InOldName, ENameCase::CaseSensitive))
	{
		...
		{
			UE_LOGF(LogBlueprint, Warning, "Cannot name local variable '%ls'. The name is already used.", *InNewName.ToString());
		}
...
FName FBlueprintEditorUtils::FindLocalVariableNameByGuid(const UBlueprint* InBlueprint, const FGuid& InVariableGuid)
{
...
FGuid FBlueprintEditorUtils::FindLocalVariableGuidByName(const UBlueprint* InBlueprint, const UStruct* InScope, const FName InVariableName)
{
	FGuid ReturnGuid;
...
FGuid FBlueprintEditorUtils::FindLocalVariableGuidByName(const UBlueprint* InBlueprint, const UEdGraph* InScopeGraph, const FName InVariableName)
{
	FGuid ReturnGuid;
...
bool FBlueprintEditorUtils::IsVariableUsed(const UBlueprint* Blueprint, const FName& VariableName, const UEdGraph* LocalGraphScope /* = nullptr */)
{
	...
	if (LocalGraphScope)
	{
		VariableGuid = FindLocalVariableGuidByName(Blueprint, LocalGraphScope, VariableName);
	}
	else
	{
...
bool FBlueprintEditorUtils::ValidateAllMemberVariables(UBlueprint* InBlueprint, UBlueprint* InParentBlueprint, const FName InVariableName)
{
	for(int32 VariableIdx = 0; VariableIdx < InBlueprint->NewVariables.Num(); ++VariableIdx)
	{
		if(InBlueprint->NewVariables[VariableIdx].VarName == InVariableName)
		{
			...
			UE_LOGF(LogBlueprint, Warning, "Blueprint %ls (child of/implements %ls) has a member variable with a conflicting name (%ls). Changing to %ls.", *InBlueprint->GetName(), *GetNameSafe(InParentBlueprint), *InVariableName.ToString(), *NewChildName.ToString());
	...
	if(InBlueprint->SimpleConstructionScript != nullptr)
	{
		...
		for(int32 NodeIndex = 0; NodeIndex < ChildSCSNodes.Num(); ++NodeIndex)
		{
			...
			if(SCS_Node != nullptr && SCS_Node->GetVariableName() == InVariableName)
			{
				...
				UE_LOGF(LogBlueprint, Warning, "Blueprint %ls (child of/implements %ls) has a component variable with a conflicting name (%ls). Changing to %ls.", *InBlueprint->GetName(), *InParentBlueprint->GetName(), *InVariableName.ToString(), *NewChildName.ToString());
	...
	for (int32 TimelineIndex=0; TimelineIndex < InBlueprint->Timelines.Num(); ++TimelineIndex)
	{
		...
		if( TimelineTemplate )
		{
			if( TimelineTemplate->GetFName() == InVariableName )
			{
				...
				UE_LOGF(LogBlueprint, Warning, "Blueprint %ls (child of/implements %ls) has a timeline with a conflicting name (%ls). Changing to %ls.", *InBlueprint->GetName(), *InParentBlueprint->GetName(), *InVariableName.ToString(), *NewName.ToString());
				return true;
			}
		}
	}
...
bool FBlueprintEditorUtils::ValidateAllFunctionGraphs(UBlueprint* InBlueprint, UBlueprint* InParentBlueprint, const FName& InVariableName)
{
	for (int32 FunctionIndex=0; FunctionIndex < InBlueprint->FunctionGraphs.Num(); ++FunctionIndex)
	{
		...
		if( FunctionGraph->GetFName() == InVariableName )
		{
			...
			UE_LOGF(LogBlueprint, Warning, "Blueprint %ls (child of/implements %ls) has a function graph with a conflicting name (%ls). Changing to %ls.", *InBlueprint->GetName(), *InParentBlueprint->GetName(), *InVariableName.ToString(), *NewName.ToString());
			return true;
		}
	}
	return false;
	...
	if (VarDesc.HasMetaData(FBlueprintMetadata::MD_BitmaskEnum))
	{
		...
		if (!BitmaskEnumTypePath.IsEmpty())
		{
			...
			if (FPackageName::IsShortPackageName(BitmaskEnumTypePath))
			{
				...
				{
					UE_LOGF(LogBlueprint, Error, "Enum %ls cannot be loaded", *BitmaskEnumTypePath);
				}
			}
			else
			{
...
static void ShowNotification(const FText& Message, EMessageSeverity::Type Severity)
{
	if(FApp::IsUnattended())
	{
		...
		case EMessageSeverity::Error:
			UE_LOGF(LogBlueprint, Error, "%ls", *Message.ToString());
			break;
		case EMessageSeverity::PerformanceWarning:
		case EMessageSeverity::Warning:
			UE_LOGF(LogBlueprint, Warning, "%ls", *Message.ToString());
			break;
		case EMessageSeverity::Info:
			UE_LOGF(LogBlueprint, Log, "%ls", *Message.ToString());
			break;
		}
	}
	else
			...
			for (TArray<UK2Node_Event*>::TIterator NodeIt(AllEvents); NodeIt; ++NodeIt)
			{
				...
				if (EventNode->EventReference.GetMemberParentClass(EventNode->GetBlueprintClassFromNode()) == InterfaceClass)
				{
					...
					if (bPreserveFunctions)
					{
						...
						{
							if (UEdGraphPin* TargetPin = NewEvent->FindPin(CurrentPin->PinName))
							{
							...
							{
								UE_LOGF(LogBlueprint, Warning,
									"Failed to transfer pin %ls on function %ls while removing interface from blueprint %ls",
									*CurrentPin->PinName.ToString(), *PreviousSignatureFunction->GetPathName(), *Blueprint->GetPathName());
							}
						}
					}
...
void FBlueprintEditorUtils::RemoveInvalidStructureMemberVariableFromBlueprint(UBlueprint* Blueprint)
{
	if (!Blueprint)
	{
		return;
	...
	const auto IsValidStruct = [FallbackStruct](const UScriptStruct* Struct)
	{
		if(Struct == nullptr || !IsValid(Struct))
		{
			return false;
		}
		...
		if(!Struct->IsNative() && Struct->HasAnyFlags(RF_Transient))
		{
			return false;
		}
		return true;
	...
	for (int32 VarIndex = 0; VarIndex < Blueprint->NewVariables.Num(); ++VarIndex)
	{
		...
		if (Var.VarType.PinCategory == UEdGraphSchema_K2::PC_Struct)
		{
			...
			bIsInvalid = !IsValidStruct(ScriptStruct);
		}
		else if (Var.VarType.IsMap() && Var.VarType.PinValueType.TerminalCategory == UEdGraphSchema_K2::PC_Struct)
		{
			...
			bIsInvalid = !IsValidStruct(Cast<UScriptStruct>(Var.VarType.PinValueType.TerminalSubCategoryObject.Get()));
		}
	...
	{
		UE_LOGF(LogBlueprint, Warning, "The following member variables in blueprint '%ls' have invalid type. Removing them.\n\n%ls", *Blueprint->GetFullName(), *DisplayList);

		Blueprint->Modify();

		for (const FName& Name : ZombieMemberNames)
		{
			Blueprint->NewVariables.RemoveAll([Name](const FBPVariableDescription& Desc){ return Desc.VarName == Name; });
	...
	static void FixOverriddenEventSignature(UK2Node_Event* EventNode, UBlueprint* Blueprint, UEdGraph* CurrentGraph)
	{
		...
		if (bNeedsToBeFixed)
		{
			...
			if (ActualTargetFunction)
			{
				...
				{
					UE_LOGF(LogBlueprint, Error
						, "FixOverriddenEventSignature function \"%ls\" is not compatible with \"%ls\" node \"%ls\""
		...
		for (int32 GraphIndex = 0; GraphIndex < CurrentInterfaceDesc.Graphs.Num(); GraphIndex++)
		{
			...
			if (!CurrentGraph || !FindUField<UFunction>(CurrentInterfaceDesc.Interface, CurrentGraph->GetFName()))
			{
				if(CurrentGraph)
				{
					...
					// REN_AllowPackageLinkerMismatch is passed to not have to force load completion before renaming
					CurrentGraph->Rename(
						nullptr,
						CurrentGraph->GetOuter(),
						REN_DoNotDirty | REN_DontCreateRedirectors | (Blueprint->bIsRegeneratingOnLoad ? REN_AllowPackageLinkerMismatch : 0));
		...
		if(AnimBlueprint->BlueprintType != BPTYPE_Interface)
		{
			...
			else
			{
				...
				for (int32 i = 0; i < AnimBlueprint->FunctionGraphs.Num(); ++i)
				{
					...
					{
						UE_LOGF(LogBlueprint, Log, "!!! Removing AnimGraph from %ls, because it has a parent anim blueprint that defines the AnimGraph", *AnimBlueprint->GetPathName());
		...
		for(UEdGraph* BoundGraph : Node->GetSubGraphs())
		{
			if (BoundGraph)
			{
				...
				if (BoundGraph->GetOuter() != Node)
				{
					...
					if (false == BoundGraph->Rename(*BoundGraph->GetName(), Node, (BoundGraph->HasAnyFlags(RF_NeedLoad | RF_NeedPostLoad) ? REN_AllowPackageLinkerMismatch : 0) | REN_DontCreateRedirectors))
					{
						UE_LOGF(LogBlueprintDebug, Log, "CompositeNode: On Blueprint '%ls' could not fix Outer() for BoundGraph of composite node '%ls'", *Blueprint->GetPathName(), *Node->GetName());
					}
				}
			}
		}
...
void FBlueprintEditorUtils::UpdateComponentTemplates(UBlueprint* Blueprint)
{
	...
	for (UK2Node_AddComponent* ComponentNode : AllComponents)
	{
		...
		if (ActorComp)
		{
			...
			{
				UE_LOGF(LogBlueprint, Warning,
					"Blueprint '%ls' has an AddComponent node '%ls' with a non-unique component template name (%ls). Moving it to a new template object with a unique name. Re-save the Blueprint to remove this warning on the next load.",
...
UTimelineTemplate* FBlueprintEditorUtils::AddNewTimeline(UBlueprint* Blueprint, const FName& TimelineVarName)
{
	...
	if (Timeline != nullptr)
	{
		UE_LOGF(LogBlueprint, Log, "AddNewTimeline: Blueprint '%ls' already contains a timeline called '%ls'", *Blueprint->GetPathName(), *TimelineVarName.ToString());
		return nullptr;
	}
	else
	{
...
bool FBlueprintEditorUtils::RenameTimeline(UBlueprint* Blueprint, const FName OldNameRef, const FName NewName)
{
	...
	if ((!NewTemplate && bUniqueNameForTemplate) || NewTemplate == Template)
	{
		if (Template)
		{
			...
			Template->Rename(*NewTemplateName, Template->GetOuter(), (Blueprint->bIsRegeneratingOnLoad ? REN_AllowPackageLinkerMismatch : REN_None));
		...
		for (FActorIterator ActorIt(InWorld); ActorIt; ++ActorIt)
		{
			if (AActor* CurrentActor = *ActorIt)
			{
				...
				for (int i = 0; i < ChildElementHandles.Num(); ++i)
				{
					...
					if (TTypedElement<ITypedElementHierarchyInterface> ChildElementHierarchyInterface = Registry->GetElement<ITypedElementHierarchyInterface>(ChildElementHandle))
					{
						ChildElementHierarchyInterface.GetChildElements(ChildElementHandles, true);
					}
					if (TTypedElement<ITypedElementObjectInterface> ChildElementObjectInterface = Registry->GetElement<ITypedElementObjectInterface>(ChildElementHandle))
					{
						if (UObject* ChildObject = ChildElementObjectInterface.GetObject())
						{
...
bool FBlueprintEditorUtils::KismetDiagnosticExec(const TCHAR* InStream, FOutputDevice& Ar)
{
	...
	{
		UE_LOGF(LogBlueprintDebug, Log, "--- LISTING ORPHANED CLASSES ---");
		for( TObjectIterator<UClass> it; it; ++it )
		{
			...
			if( CurrClass->ClassGeneratedBy != nullptr && CurrClass->GetOutermost() != GetTransientPackage() )
			{
				if( UBlueprint* GeneratingBP = Cast<UBlueprint>(CurrClass->ClassGeneratedBy) )
				{
					...
					{
						UE_LOGF(LogBlueprintDebug, Log, " - %ls", *CurrClass->GetFullName());				
					}
				}	
			}
		}
	...
	{
		UE_LOGF(LogBlueprintDebug, Log, "--- LISTING ROOTSET OBJ ---");
		for( FThreadSafeObjectIterator it; it; ++it )
		{
			UObject* CurrObj = *it;
			if( CurrObj->IsRooted() )
			{
				UE_LOGF(LogBlueprintDebug, Log, " - %ls", *CurrObj->GetFullName());
			}
		}
	}
	else
...
public:
	...
	virtual bool IsClassAllowed(const FClassViewerInitializationOptions& InInitOptions, const UClass* InClass, TSharedRef< FClassViewerFilterFuncs > InFilterFuncs ) override
	{
		...
		return InFilterFuncs->IfInChildOfClassesSet( AllowedChildrenOfClasses, InClass) != EFilterReturn::Failed && 
			...
			(!InClass->HasAnyFlags(RF_Transient) || InClass->IsNative()) && InClass->GetPackage() != GetTransientPackage() &&
...
void FBlueprintEditorUtils::PostEditChangeBlueprintActors(UBlueprint* Blueprint, bool bComponentEditChange)
{
	if (Blueprint->GeneratedClass && Blueprint->GeneratedClass->IsChildOf(AActor::StaticClass()))
	{
		...
		for (UObject* MatchingObj : MatchingBlueprintObjects)
		{
			...
			// If worlds leak or GC hasn't been performed we'll have actors in invalid worlds and reconstructing
			// them in PostEditChange will crash
			UWorld* ActorWorld = Actor->GetWorld();
			if (ActorWorld && (!IsValid(ActorWorld) || ActorWorld->IsCleanedUp()))
			{
				continue;
			}
		...
		else if (const FEnumProperty* EnumProperty = CastField<const FEnumProperty>(Property))
		{
			...
			else
			{
				UE_LOGF(
					LogBlueprint,
					Warning,
					"Member 'Enum' of EnumProperty is nullptr, copy   operation would fail. You could ignore this message if you moved the enum class. EnumProperty name:'%ls', OwningObject: '%ls', outer of OwningObject: '%ls', outer of outer: '%ls'",
...
struct FComponentInstancingDataUtils
{
	...
	{
		// Determines if a struct value matches the default value using native comparison only (if implemented).
		auto IsNativeIdentical = [](const UScriptStruct* InStruct, const uint8* InValuePtr, const uint8* InDefaultValuePtr, uint32 PortFlags)
		{
			bool bIsIdentical = false;
			UScriptStruct::ICppStructOps* CppStructOps = InStruct->GetCppStructOps();
			if (CppStructOps
				&& CppStructOps->HasIdentical()
				&& CppStructOps->Identical(InValuePtr, InDefaultValuePtr, PortFlags, bIsIdentical))
			{
				return bIsIdentical;
			}

			// Not bound or no actual comparison was performed at the native level.
			return true;
		};
				...
				&& !Property->HasAnyPropertyFlags(CPF_Transient | CPF_DuplicateTransient | CPF_NonPIEDuplicateTransient | CPF_Deprecated))
			{
				for (int32 Idx = 0; Idx < Property->ArrayDim; Idx++)
				{
					...
					if (FStructProperty* StructProperty = CastField<FStructProperty>(Property))
					{
						...
						// Prepend the struct property only if there is at least one changed sub-property, or otherwise if there are any native differences.
						if (NumChangedProperties < OutData.ChangedPropertyList.Num()
							|| !IsNativeIdentical(StructProperty->Struct, PropertyValue, DefaultPropertyValue, PPF_None))
						{
							...
							if (ArrayValueIndex < DefaultArrayValueHelper.Num())
							{
								...
								if (FStructProperty* InnerStructProperty = CastField<FStructProperty>(ArrayProperty->Inner))
								{
									...
									if (NumChangedArrayProperties < OutData.ChangedPropertyList.Num()
										|| !IsNativeIdentical(InnerStructProperty->Struct, ArrayPropertyValue, DefaultArrayPropertyValue, PPF_None))
									{
...
static FAutoConsoleCommand AuditThreadSafeFunctions(
	...
	FConsoleCommandDelegate::CreateLambda([]()
		{
			UE_LOGF(LogBlueprint, Display, "--- BEGIN audit all BlueprintThreadSafe functions ---");
			UE_LOGF(LogBlueprint, Display, "Name, Path, Type, BPCallType");
			...
			for (TObjectIterator<UFunction> It; It; ++It)
			{
				...
				if (FBlueprintEditorUtils::HasFunctionBlueprintThreadSafeMetaData(Function))
				{
					...
					UE_LOGF(LogBlueprint, Display, "%ls, %ls, %ls, %ls", *Function->GetName(), *Function->GetPathName(), Native, Purity);
				}
			}

			UE_LOGF(LogBlueprint, Display, "--- END audit all BlueprintThreadSafe functions ---");
		}));
	...
	FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& InArgs)
		{
			...
			if (Blueprint == nullptr)
			{
				UE_LOGF(LogBlueprint, Warning, "--- Could not load Blueprint %ls ---", *InArgs[0]);
				return;
			}

			if (Blueprint->GeneratedClass == nullptr)
			{
				UE_LOGF(LogBlueprint, Warning, "--- Blueprint %ls as a null GeneratedClass ---", *InArgs[0]);
				return;
			}

			UE_LOGF(LogBlueprint, Display, "--- BEGIN audit function calls for Blueprint %ls ---", *InArgs[0]);
			UE_LOGF(LogBlueprint, Display, "Name, Path, Type, BPCallType");
			...
			struct FFunctionReferenceProcessor : public FSimpleReferenceProcessorBase
			{
				FORCEINLINE void HandleTokenStreamObjectReference(FGCArrayStruct& ObjectsToSerializeStruct, UObject* ReferencingObject, UObject*& Object, UE::GC::FTokenId, EGCTokenType TokenType, bool)
				{
					if (UFunction* Function = Cast<UFunction>(Object))
					{
						...
						UE_LOGF(LogBlueprint, Display, "%ls, %ls, %ls, %ls", *Function->GetName(), *Function->GetOuterUClass()->GetPathName(), Native, Purity);
					}
				}
			} Processor;
