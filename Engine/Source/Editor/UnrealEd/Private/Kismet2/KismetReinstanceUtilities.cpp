...
bool GUseReinstancingGraphCache = true;
static FAutoConsoleVariableRef CVarUseReinstancingGraphCache(
	TEXT("bp.UseReinstancingGraphCache"),
	GUseReinstancingGraphCache,
	TEXT("Use a subobject graph cache to improve performance of reinstancing")
);
...
struct FReplaceReferenceHelper
{
	...
	static void IncludeDSOs(UObject* OldOuter, UObject* NewOuter, TMap<UObject*, UObject*>& OldToNewInstanceMap, TArray<UObject*>& SourceObjects)
	{
		...
		GetObjectsWithOuter(OldOuter, OldSubObjArray, EGetObjectsFlags::None);
	...
	static void ForEachSubObject(const FProperty* TargetProp, const UObject* Outer, const void* ContainerAddress, TFunctionRef<void(const UObject*)> ObjRefFunc)
	{
		...
		if (const FArrayProperty* ArrayProperty = CastField<const FArrayProperty>(TargetProp))
		{
			...
			for (int32 ElementIndex = 0; ElementIndex < ArrayHelper.Num(); ++ElementIndex)
			{
				...
				ForEachSubObject(ArrayProperty->Inner, Outer, ValueAddress, ObjRefFunc);
			}
		}
		else if (const FMapProperty* MapProperty = CastField<const FMapProperty>(TargetProp))
		{
			...
			for (int32 ElementIndex = 0; ElementIndex < MapHelper.GetMaxIndex(); ++ElementIndex)
			{
				if (MapHelper.IsValidIndex(ElementIndex))
				{
					...
					ForEachSubObject(MapProperty->KeyProp, Outer, KeyAddress, ObjRefFunc);
					ForEachSubObject(MapProperty->ValueProp, Outer, ValueAddress, ObjRefFunc);
		...
		else if (const FSetProperty* SetProperty = CastField<const FSetProperty>(TargetProp))
		{
			...
			for (int32 ElementIndex = 0; ElementIndex < SetHelper.GetMaxIndex(); ++ElementIndex)
			{
				if (SetHelper.IsValidIndex(ElementIndex))
				{
					...
					ForEachSubObject(SetProperty->ElementProp, Outer, ValueAddress, ObjRefFunc);
		...
		else if (const FOptionalProperty* OptionalProperty = CastField<FOptionalProperty>(TargetProp))
		{
			...
			{
				ForEachSubObject(OptionalProperty->GetValueProperty(), Outer, ValueAddress, ObjRefFunc);
			}
		}
		else if (const FStructProperty* StructProperty = CastField<const FStructProperty>(TargetProp))
		{
			for (FProperty* StructProp = StructProperty->Struct->RefLink; StructProp; StructProp = StructProp->NextRef)
			{
				for (int32 ArrayIdx = 0; ArrayIdx < StructProp->ArrayDim; ++ArrayIdx)
				{
					...
					ForEachSubObject(StructProp, Outer, ValueAddress, ObjRefFunc);
				}
			}
		}
		...
		else if (const FVerseDynamicProperty* DynamicProperty = CastField<const FVerseDynamicProperty>(TargetProp))
		{
			if (const UE::FDynamicallyTypedValue* DynamicValue = DynamicProperty->GetPropertyValuePtr(ContainerAddress))
			{
				DynamicValue->ForEachSubobject(ObjRefFunc);
			}
		}
#if WITH_VERSE_VM
		else if (const FVRestValueProperty* RestValueProperty = CastField<const FVRestValueProperty>(TargetProp))
		{
			// TODO: deal with reinstancing in the new VM.
	...
	static void GetOwnedSubobjectsRecursive(const UObject* Container, TSet<UObject*>& OutObjects, const UObject* Root = nullptr)
	{
		...
		// loose subobjects need to be reinstanced too so iterate the IDO instead if it exists so we can include loose subobjects references
		if (UObject* FoundIdo = UE::FPropertyBagRepository::Get().FindInstanceDataObject(Container))
		{
			Container = FoundIdo;
		}
		...
		for (FProperty* Prop = ContainerClass->RefLink; Prop; Prop = Prop->NextRef)
		{
			for (int32 ArrayIdx = 0; ArrayIdx < Prop->ArrayDim; ++ArrayIdx)
			{
				...
				ForEachSubObject(Prop, Container, ValuePtr, [&OutObjects, Root](const UObject* Ref)
					{
						if (Ref && !OutObjects.Contains(Ref) && Ref->IsIn(Root))
						{
...
FBlueprintCompileReinstancer::FBlueprintCompileReinstancer(UClass* InClassToReinstance, EBlueprintCompileReinstancerFlags Flags)
	: FGCObject(EFlags::RegisterLater) 
...
{
	// This is not needed during async loading since GC cannot run and we don't want
	// to expose any objects we're currently working on to other threads if they try to use
	// the GC interface to collect objects.
	if (!IsInActualLoadingThread() && !IsInParallelLoadingThread())
	{
		// Call this in the most derived class to avoid pure calls when registering from the base class
		// and the vtbl isn't yet ready.
		RegisterGCObject();
	}
...
{
	UE_LOGF(LogBlueprint, Log, "BlueprintCompileReinstancer: Doing a fast path refresh on class '%ls'.", *GetPathNameSafe(ClassToReinstance));
	...
	{
		UE_LOGF(LogBlueprint, Log, "  Fast path is refreshing (not replacing) %ls", *Obj->GetFullName());
...
TSharedPtr<FReinstanceFinalizer> FBlueprintCompileReinstancer::ReinstanceInner(bool bForceAlwaysReinstance)
{
	...
	if (ClassToReinstance && DuplicatedClass)
	{
		...
		{
			UE_LOGF(LogBlueprint, Log, "BlueprintCompileReinstancer: Doing a full reinstance on class '%ls'", *GetPathNameSafe(ClassToReinstance));
		...
		if (Obj != nullptr)
		{
			...
			else if (Obj == SearchObject || Obj->IsIn(SearchObject))
			{
				...
				if (Obj == SearchObject)
				{
					...
					UE_LOGF(LogSerialization, Log, "FArchiveReplaceObjectRef: Obj == SearchObject : '%ls'", *ObjName);
				}
#endif
...
void FBlueprintCompileReinstancer::PropagateSparseClassDataToNewClass(UClass* NewClass)
{
	...
	{
		SparseClassDataStruct->Rename(nullptr, NewClass, REN_DoNotDirty | REN_DontCreateRedirectors | REN_NonTransactional | REN_AllowPackageLinkerMismatch);
	}
	...
	const TMap<FFieldVariant, FFieldVariant>& FieldMappings)
{
	for (UBlueprint* DependentBP : DependentBPs)
	{
		...
		{
			UE_LOGF(LogBlueprint, Log, 
				"UpdateBytecodeReferences: %lld references were replaced in BP %ls", 
...
FActorAttachmentData::FActorAttachmentData(AActor* OldActor)
{
	...
	if (USceneComponent* OldRootComponent = OldActor->GetRootComponent())
	{
		if (OldRootComponent->GetAttachParent() != nullptr)
		{
			...
			{
				UE_LOGF(LogBlueprint, Warning, "ReplaceInstancesOfClass: RootComponent (%ls) attached to another component in this Actor (%ls).", *OldRootComponent->GetPathName(), *TargetAttachParent->GetPathName());
				TargetAttachParent = nullptr;
			}
...
void FBlueprintCompileReinstancer::MoveDependentSkelToReinst(UClass* const OwnerClass, TMap<UClass*, UClass*>& NewSkeletonToOldSkeleton)
{
	...
	for (int32 i = NewParentIndex; i < ClassHierarchy.Num(); ++i)
	{
		...
		// Mark this class as being an internal class to ease filtering when enumerating all BP classes. 
		if (UBlueprintGeneratedClass* BPReinstClass = Cast<UBlueprintGeneratedClass>(ReinstClass))
		{
			BPReinstClass->bIsInternalClass = true;
		}
		...
		if (OldCDO)
		{
			...
			OldCDO->Rename(nullptr, ReinstClass->GetOuter(), REN_DoNotDirty | REN_DontCreateRedirectors | REN_NonTransactional | REN_AllowPackageLinkerMismatch);
...
UClass* FBlueprintCompileReinstancer::MoveCDOToNewClass(UClass* OwnerClass, const TMap<UClass*, UClass*>& OldToNewMap, bool bAvoidCDODuplication)
{
	...
	GetObjectsWithOuter(OwnerClass, OwnedObjects, EGetObjectsFlags::None);
	...
	const int32 PrevStructSize = OwnerClass->GetStructureSize();
	...
	for( int32 I = 0; I < OwnedObjects.Num(); ++I )
	{
		...
		{
			OwnedArchetype->Rename(*OriginalNames[I].ToString(), OwnerClass, REN_DoNotDirty | REN_DontCreateRedirectors | REN_NonTransactional | REN_AllowPackageLinkerMismatch);
		}
	}
	...
	const int32 NewStructSize = CopyOfOwnerClass->GetStructureSize();
	ensure(PrevStructSize == NewStructSize);

	// Mark this class as being an internal class to ease filtering when enumerating all BP classes. 
	if (BPGDuplicatedClass)
	{
		BPGDuplicatedClass->bIsInternalClass = true;
	}
...
void FBlueprintCompileReinstancer::ReplaceObjectHelper(const FReinstancingState& State, UObject*& OldObject, UClass* OldClass, UObject*& NewUObject, UClass* NewClass, int32 OldObjIndex, TArray<UObject*>& ObjectsToReplace, TFunctionRef<TArray<TObjectPtr<USceneComponent>>& (USceneComponent*)> GetAttachChildrenArray, bool bIsComponent, bool bArchetypesAreUpToDate, const TSet<UObject*>* InstancesThatShouldUseOldClass)
{
	...
	{
		FName NewName = State.OldToNewNameMap.FindRef(OldObject);
		if (NewName == NAME_None)
		{
			// Otherwise, just use the old object's current name.
			NewName = OldObject->GetFName();
		}
		NewArchetype = UObject::GetArchetypeFromRequiredInfo(NewClass, OldObject->GetOuter(), NewName, OldObject->GetFlags() & RF_Reinst);
	}
	else
	{
		UObject* OldArchetype = OldObject->GetArchetype();
		NewArchetype = State.OldToNewInstanceMap.FindRef(OldArchetype);
	...
	// Prepares OldObject for cleanup and adds an entry for NewUObject to the replacement map.
	auto ReplaceObjectHelper_Finalize = [&State](UObject* OldObject, UObject* NewUObject)
	{
		...
		State.OldToNewInstanceMap.Add(OldObject, NewUObject);
	};
	...
	if (!State.OldToNewNameMap.Contains(OldObject))
	{
		...
		if (bIsComponent && OldObject->HasAllFlags(RF_ArchetypeObject) && OldObject->GetOuter()->IsA<UBlueprintGeneratedClass>())
		{
			...
			{
				State.OldToNewNameMap.Add(OldArchetypeObject, OldName);
				OldArchetypeObject->Rename(*OldArchetypeName, OldArchetypeObject->GetOuter(), REN_DoNotDirty | REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
			}
		}
		else
		{
						...
						
	{
		...
		if (UObject* const* ReinstancedOuter = State.OldToNewInstanceMap.Find(DestinationOuter))
		{
			if (*ReinstancedOuter != nullptr)
			{
			...
			{	
				// Our outer has been replaced with NULL, which implies that it was not reinstanced, but we may have added it to the map to ensure that references to it get replaced with NULL (see ReplaceInstancesOfClass_Inner).
				if (UNLIKELY(IsValid(DestinationOuter)))
				{
					// The outer is still valid, so we'll reuse it as the scope for reinstancing the old object below. This would generally not be expected, so emit a warning, but allow it to pass just in case, to avoid losing data.
					UE_LOGF(LogBlueprint, Warning, "ReplaceObjectHelper(): OldObject (%ls) has an outer that was replaced with NULL, but is still valid for instancing.", *GetFullNameSafe(OldObject));
				}
				else
				{
					// The outer is either missing or pending garbage collection, so there is no need to replace inner objects at this scope. But we do need to update the replacement map so any references to it are replaced with NULL.
					UE_LOGF(LogBlueprint, Log, "ReplaceObjectHelper(): OldObject (%ls) will not be replaced because its outer is no longer valid and was not reinstanced.", *GetFullNameSafe(OldObject));
					ReplaceObjectHelper_Finalize(OldObject, nullptr);
					return;
				}
			}
		...
		checkf(DestinationOuter != nullptr, TEXT("Attempting to reinstance OldObject (%s) with a NULL DestinationOuter."), *GetFullNameSafe(OldObject));

		FObjectInstancingGraph PrePopulatedInstancingGraph;
		const bool bUsePrePopulatedInstancingGraph = PrePopulateInstancingGraphForArchetype(State.GraphCache, PrePopulatedInstancingGraph, OldObject, NewArchetype, &State.OldToNewInstanceMap);
	...
	NewUObject->SetFlags(OldFlags & RF_Reinst);

	TMap<UObject*, UObject*> CreatedInstanceMap;
	TArray< TTuple<UObject*, UObject*>> OrderedListOfObjectToCopy;
	PreCreateSubObjectsForReinstantiation(State.InOldToNewClassMap, OldObject, NewUObject, CreatedInstanceMap, &State.OldToNewInstanceMap, &OrderedListOfObjectToCopy, InstancesThatShouldUseOldClass);
	State.OldToNewInstanceMap.Append(CreatedInstanceMap);
	...
	Options.OptionalReplacementMappings = &State.OldToNewInstanceMap;
	if (FOverridableManager::Get().IsEnabled(OldObject))
	{
		Options.bReplaceInternalReferenceUponRead = true;
		Options.OptionalOldToNewClassMappings = &State.InOldToNewClassMap;
	}
	...
	for (const auto& Pair : OrderedListOfObjectToCopy)	
	{
		...
		FOverridableManager::Get().RestoreOverrideState(Pair.Key, Pair.Value);
	}
	...
	ReplaceObjectHelper_Finalize(OldObject, NewUObject);
	...
	if (bIsComponent)
	{
		...
		if (OwningActor)
		{
			...
			{
				State.PotentialEditorsForRefreshing.AddUnique(OwningActor->GetClass()->ClassGeneratedBy);
			}
			...
			State.OwnersToRerunConstructionScript.Add(OwningActor);
		}
...
static void ReplaceActorHelper(AActor* OldActor, UClass* OldClass, UObject*& NewUObject, UClass* NewClass, TMap<UObject*, UObject*>& OldToNewInstanceMap, const TMap<UClass*, UClass*>& InOldToNewClassMap, TMap<FSoftObjectPath, UObject*>& ReinstancedObjectsWeakReferenceMap, TMap<UObject*, FActorAttachmentData>& ActorAttachmentData, TArray<FActorReplacementHelper>& ReplacementActors, bool bPreserveRootComponent, bool& bSelectionChanged)
{
	...
	SpawnInfo.ObjectFlags |= OldActor->GetFlags() & RF_Reinst;
	...
	OldActor->UObject::Rename(nullptr, OldActor->GetOuter(), REN_DoNotDirty | REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
...
void FBlueprintCompileReinstancer::ReplaceInstancesOfClass_Inner(const TMap<UClass*, UClass*>& InOldToNewClassMap, const FReplaceInstancesOfClassParameters& Params)
{
	...
	TMap<UObject*, FObjectInstancingGraph> GraphCache;
...

	{
		...
		FReinstancingState ReinstancingState
		{
		...
		if(Params.ConstructionVersioningData)
		{
			...
			ReinstanceWithVersioningData(ReinstancingState, InstancesThatShouldUseOldClass);
		}
		else
		{
			// Pre gather all the replacement map so that while doing reintanciation, we have all the information needed
			for (TPair<UClass*, UClass*> OldToNewClass : InOldToNewClassMap)
			{
				if (TMap<UObject*, UObject*>* ReplaceTemplateMapping = Params.OldToNewTemplates ? Params.OldToNewTemplates->Find(OldToNewClass.Key) : nullptr)
				{
		...
		for (TPair<UClass*, UClass*> OldToNewClass : InOldToNewClassMap)
		{
			...
			check(OldClass != NewClass || IsReloadActive());
			{
				...
				for (int32 OldObjIndex = 0; OldObjIndex < ObjectsToReplace.Num(); ++OldObjIndex)
				{
					...
					// with IDOs, it's possible for a loose subobject to become non-loose again which the above condition would've missed.
					// that object will be part of the new archetype however so we want to skip reinstancing it here because the outer will handle that.
					if (UE::IsInstanceDataObjectSupportEnabled(OldObject))
					{
						if (UClass*const* NewOuterClass = InOldToNewClassMap.Find(OldObject->GetOuter()->GetClass()))
						{
							// if this subobject is in the outer's CDO, we can assume that it will be handled when the outer is reinstanced
							UObject* NewOuterCDO = (*NewOuterClass)->GetDefaultObject();
							if (StaticFindObjectFast(UObject::StaticClass(), NewOuterCDO, OldObject->GetFName()))
							{
								continue;
							}
						}
					...
					if (OldActor == nullptr)
					{
						...
						ReplaceObjectHelper(ReinstancingState, OldObject, OldClass, NewUObject, NewClass, OldObjIndex, ObjectsToReplace, &FDirectAttachChildrenAccessor::Get, bIsComponent, bArchetypesAreUpToDate, InstancesThatShouldUseOldClass);
						UpdateObjectBeingDebugged(OldObject, NewUObject);
						ObjectsReplaced.Add(OldObject);

						if (bLogConversions)
						{
							UE_LOGF(LogBlueprint, Log, "Converted instance '%ls' to '%ls'", *GetPathNameSafe(OldObject), *GetPathNameSafe(NewUObject));
						}
					}
				}
			}
				...
				for (int32 OldObjIndex = 0; OldObjIndex < ObjectsToReplace.Num(); ++OldObjIndex)
				{
					...
					if (OldActor != nullptr)
					{
						...
						else
						{
							...
							ReplaceObjectHelper(ReinstancingState, OldObject, OldClass, NewUObject, NewClass, OldObjIndex, ObjectsToReplace, &FDirectAttachChildrenAccessor::Get, false, bArchetypesAreUpToDate, InstancesThatShouldUseOldClass);
						}
						UpdateObjectBeingDebugged(OldObject, NewUObject);
						ObjectsReplaced.Add(OldObject);
	...
	const TSet<UObject*>* InstancesThatShouldUseOldClass)
{
	...
	PreCreateSubObjectsForReinstantiation_Inner(OldInstancedSubObjectsPtr, OldToNewClassMap, OldObject, NewUObject, CreatedInstanceMap, OldToNewInstanceMap, OrderedListOfObjectToCopy, InstancesThatShouldUseOldClass);
...
bool FBlueprintCompileReinstancer::PrePopulateInstancingGraphForArchetype(TMap<UObject*, FObjectInstancingGraph>& GraphCache, FObjectInstancingGraph& InstancingGraph, TNotNull<UObject*> OldInstance, UObject* InArchetype, TMap<UObject*, UObject*>* OldToNewInstanceMap)
{
	...
	if (!InstancingGraphRoot || Cast<UPackage>(InstancingGraphRoot) != nullptr)
	{
		UE_LOGF(LogClass, Warning, "Object %ls was created from an archetype we could not find the root of the instancing graph.", *GetPathNameSafe(OldInstance));
		return false;
	}

	if (GUseReinstancingGraphCache)
	{
		// Look for an existing graph
		FObjectInstancingGraph* SourceGraph = GraphCache.Find(InstancingGraphRoot);

		if (!SourceGraph)
		{
			SourceGraph = &GraphCache.Add(InstancingGraphRoot);
		
			// Compute the graph for a specific top level object
			auto GatherMatchingSubObjects = [SourceGraph, InstancingGraphRoot, OldToNewInstanceMap](UObject* Instance, UObject* Template, auto GatherMatchingSubObjectsRecurse) -> void
			{
				SourceGraph->PrepopulateWithInstance(Instance, Template);

				ForEachObjectWithOuter(Instance, [Template, InstancingGraphRoot, GatherMatchingSubObjectsRecurse, OldToNewInstanceMap](UObject* SubObject)
				{
					// Just handle subobjects that are created from archetype, skip the others
					...
					if (SubObjectArchetype->GetOuter() != Template)
					{
						...
						{
							UE_LOGF(LogClass, Warning, "Missmatched while trying to rebuild instancing graph for '%ls', expecting the outer of archetype %ls to be %ls but was %ls", 
								*GetPathNameSafe(InstancingGraphRoot), *GetPathNameSafe(SubObjectArchetype), *GetPathNameSafe(Template), *GetPathNameSafe(SubObjectArchetype->GetOuter()));
							return;
						}
					}
		...
		// Copy from existing graph
		InstancingGraph = *SourceGraph;

		// Remove this instance and subobjects from graph to make sure they get properly reinstanced
		InstancingGraph.RemoveInstanceFromPrepopulate(OldInstance);

		ForEachObjectWithOuter(OldInstance, [&InstancingGraph](UObject* SubObject)
		{
			InstancingGraph.RemoveInstanceFromPrepopulate(SubObject);
		}, EGetObjectsFlags::IncludeNestedObjects);
	}
	else
	{
		// Old path without caching between calls, remove after verification
		...
		{
			InstancingGraph.PrepopulateWithInstance(Instance, Template);
			...
			ForEachObjectWithOuter(Instance, [OldInstance, Template, GatherMatchingSubObjectsRecurse, OldToNewInstanceMap](UObject* SubObject)
			{
				...
				if (SubObjectArchetype->GetOuter() != Template)
				{
					...
					{
						UE_LOGF(LogClass, VeryVerbose, "Missmatched while trying to rebuild instancing graph for '%ls', expecting the outer of archetype %ls to be %ls but was %ls", 
	...
	const TSet<UObject*>* InstancesThatShouldUseOldClass)
{
	...
	GetObjectsWithOuter(OldObject, ContainedOldSubObjects, EGetObjectsFlags::None);

	// Gather subobjects on old object and pre-create them if needed
	TArray<UObject*> ContainedNewSubObjects;
	GetObjectsWithOuter(NewUObject, ContainedNewSubObjects, EGetObjectsFlags::None);
	...
	for (int32 i = 0; i < ContainedOldSubObjects.Num(); ++i)
	{
		...
		EObjectFlags SubObjectFlags = OldSubObject->GetFlags() & RF_Reinst;
		UPackage* OldExternalPackage = OldSubObject->GetExternalPackage();
		...
		if (Archetype->HasAnyFlags(RF_ClassDefaultObject))
		{
			...
			if (UObject* const* AlreadyCreatedSubObject = OldToNewInstanceMap ? OldToNewInstanceMap->Find(OldSubObject) : nullptr)
			{
				...
				if (AlreadyCreatedSubObjectIndex > i)
				{
					ContainedOldSubObjects.RemoveAt(AlreadyCreatedSubObjectIndex);
				}
				else
				{
					// This happens on object that were reinstantiated and were not yet outered to the right object because it was not yet created
					checkf(OldInstancedSubObjects && !OldInstancedSubObjects->Contains(*AlreadyCreatedSubObject), TEXT("The object must have already been created but should have been skipped by the fact that the old object was not referencing it"));
				}
				(*AlreadyCreatedSubObject)->Rename(nullptr, NewUObject, REN_DoNotDirty | REN_DontCreateRedirectors | REN_NonTransactional | REN_AllowPackageLinkerMismatch);
			}
			// Could still be already re-instantiated but be also replaced
			else if (UObject*const* AlreadyReplacedSubObject = OldToNewInstanceMap && OldToNewInstanceMap->FindKey(OldSubObject) ? &OldSubObject : nullptr)
			{
				(*AlreadyReplacedSubObject)->Rename(nullptr, NewUObject, REN_DoNotDirty | REN_DontCreateRedirectors | REN_NonTransactional | REN_AllowPackageLinkerMismatch);
			}
			else
			{
				...
				if (UClass* const* NewSubObjectClass = (!InstancesThatShouldUseOldClass || !InstancesThatShouldUseOldClass->Contains(OldSubObject)) ? OldToNewClassMap.Find(OldSubObjectClass) : nullptr)
				{
				...
				if(!SubObjectClass->HasAnyClassFlags(CLASS_NewerVersionExists))
				{
					...
					else
					{
						...
						UObject* Template = SubObjectClass->GetDefaultObject();
						if (OldToNewInstanceMap)
						{
							FArchiveReplaceObjectRef<UObject> ReplaceInCDOAr(Template, *OldToNewInstanceMap);
						}

						NewSubObject = NewObject<UObject>(NewUObject, SubObjectClass, SubObjectName, SubObjectFlags, Template);
					}

					// Transfer external package state if necessary 
					if (OldExternalPackage)
					{
						NewSubObject->SetExternalPackage(OldExternalPackage); // place new subobject in the pre-existing external package 
						OldSubObject->SetExternalPackage(nullptr); // move old subobject outside of external package (it'll be in the transient package) so that it's doesn't cause the package to stick around
					}

					CreatedInstanceMap.Add(OldSubObject, NewSubObject);
					PreCreateSubObjectsForReinstantiation_Inner(OldInstancedSubObjects, OldToNewClassMap, OldSubObject, NewSubObject, CreatedInstanceMap, OldToNewInstanceMap, OrderedListOfObjectToCopy, InstancesThatShouldUseOldClass);
					if (OrderedListOfObjectToCopy)
					{
						// Post add for deep first order
		...
		else if (UObject** NewSubObject = ContainedNewSubObjects.FindByPredicate([SubObjectName](UObject* SubObject) { return SubObject && SubObject->GetFName() == SubObjectName; }))
		{
			...
			if (OldExternalPackage)
			{
				(*NewSubObject)->SetExternalPackage(OldExternalPackage); // place new suboject in the pre-existing external package 
				OldSubObject->SetExternalPackage(nullptr); // move old subobject outside of external package (it'll be in the transient package) so that it's doesn't cause the package to stick around
			}
...
FBlake3Hash FBlueprintCompileReinstancer::CalculateConstructionHash(const UClass* ForClass)
{
	...
	GetObjectsWithOuter(ForClass, Objects, EGetObjectsFlags::None, RF_Transient, EInternalObjectFlags::Garbage );
...
void FBlueprintCompileReinstancer::ReinstanceWithVersioningData(const FReinstancingState& ReinstancingState, const TSet<UObject*>* InstancesThatShouldUseOldClass)
{
	...
	ReinstanceNormalObjects(NormalReinstancingClasses, ReinstancingState, InstancesThatShouldUseOldClass);
	ReinstanceActors(ActorReinstancingClasses, ReinstancingState, InstancesThatShouldUseOldClass);
}
	...
	, const FBlueprintCompileReinstancer::FReinstancingState& ReinstancingState)
{
	for (TPair<UClass*, UClass*> OldToNewClass : InOldToNewClassMap)
	{
		...
		const FBlake3Hash* ConstructionVersion = ReinstancingState.ConstructionVersioningData ? ReinstancingState.ConstructionVersioningData->Find(NewClass) : nullptr;
...
void FBlueprintCompileReinstancer::UpdateObjectBeingDebugged(UObject* InOldObject, UObject* InNewObject)
{
	...
	if (UBlueprint* OldObjBlueprint = Cast<UBlueprint>(InOldObject->GetClass()->ClassGeneratedBy))
	{
		...
		if (InNewObject == nullptr || OldObjBlueprint == Cast<UBlueprint>(InNewObject->GetClass()->ClassGeneratedBy))
		{
