	...
	: FThumbnailPreviewScene()
{
	...
	GetScene()->SetFXSystem( ThumbnailFXSystem.Get());
}

FParticleSystemThumbnailScene::~FParticleSystemThumbnailScene()
{
	FFXSystemInterface::MarkPendingKill( ThumbnailFXSystem.Get() );
}
...
void FMaterialThumbnailScene::SetMaterialInterface(UMaterialInterface* InMaterial)
{
	...
	if ( InMaterial )
	{
		...
		case TPT_ShaderBall:
			PreviewActor->GetStaticMeshComponent()->SetStaticMesh(GUnrealEd->GetThumbnailManager()->EditorShaderBall);
			break;
	...
	: FThumbnailPreviewScene()
{
	...
	PreviewActor->SetActorEnableCollision(false);
	if (USkeletalMeshComponent* SkelMeshComp = PreviewActor->GetSkeletalMeshComponent())
	{
		SkelMeshComp->bSuppressNotifyEventDispatch = true;
	}
}
