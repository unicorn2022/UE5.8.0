...
FEditorDelegates::FOnPreDestructiveAssetActionWithArgs  FEditorDelegates::OnPreDestructiveAssetActionWithArgs;
			...
			if (const UClass* FactoryClass = UInterchangeManager::GetInterchangeManager().GetRegisteredFactoryClass(Obj->GetClass()))
			{
				...
				switch (FactoryBase->GetFactoryAssetType())
				{
					...
					case EInterchangeFactoryAssetType::Grooms:
						...
						break;
					case EInterchangeFactoryAssetType::Sounds:
						TranslatorAssetType = EInterchangeTranslatorAssetType::Sounds;
						break;
				};
...
void ExecuteInvalidateCachedShaders(const TArray< FString >& Args)
{
	...
	if(FileHandle)
	{
		...
		UE_LOGF(LogConsoleResponse, Display, "r.InvalidateCachedShaders succeeded\n\"ShaderVersion.ush\" was updated.\n");
	}
	else
	{
