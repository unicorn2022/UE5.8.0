...
void UUnrealEdEngine::Init(IEngineLoop* InEngineLoop)
{
	...
	if (!IsRunningCommandlet())
	{
		...
		{
			// Despite the name, the LegacyIterative cook flag does not cause CookSettings.LegacyIterative, aka
			// bLegacyBuildDependencies. It triggers the same code as the "-iterate" commandline, which causes incremental
			// cooks when LegacyIterative=false, and causes the cook to cook iteratively using the legacy method when
			// LegacyIterative=true.
			BaseCookingFlags |= ECookInitializationFlags::LegacyIterative;
			if (bLegacyIterative)
			{
				BaseCookingFlags |= CookerSettings->bIgnoreIniSettingsOutOfDateForIteration
					? ECookInitializationFlags::IgnoreIniSettingsOutOfDate : ECookInitializationFlags::None;
				BaseCookingFlags |= CookerSettings->bIgnoreScriptPackagesOutOfDateForIteration
					? ECookInitializationFlags::IgnoreScriptPackagesOutOfDate : ECookInitializationFlags::None;
			}
		}
...
void UUnrealEdEngine::AttemptModifiedPackageNotification()
{
	...
	if (PackagesDirtiedThisTick.Num() > 0 && !bIsCooking)
	{
		...
		const bool bIsCheckoutPromptEnabled = GetDefault<UEditorLoadingSavingSettings>()->bPromptForCheckoutOnAssetModification;
		if (bIsCheckoutPromptEnabled && Files.Num() > 0)
		{
			// Force source control state to be updated
			ISourceControlModule::Get().GetProvider().Execute(ISourceControlOperation::Create<FUpdateStatus>(), SourceControlHelpers::AbsoluteFilenames(Files), EConcurrency::Asynchronous, FSourceControlOperationComplete::CreateUObject(this, &UUnrealEdEngine::OnSourceControlStateUpdated, Packages));
		}
	}
...
void UUnrealEdEngine::AppendTemplateMaps(const TArray<FTemplateMapInfo>& InTemplateMapInfos)
{
	...
	for (const FTemplateMapInfo& TemplateMapInfo : InTemplateMapInfos)
	{
		...
		{
			UE_LOGF(LogUnrealEdEngine, Warning, "Attempted to register an already registered template map ('%ls'). Skipping registration.", *TemplateMapInfo.Map.ToString());
		}
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
	}
...
void UUnrealEdEngine::FixAnyInvertedBrushes(UWorld* World)
{
	...
	if (Brushes.Num() > 0)
	{
		...
		{
			UE_LOGF(LogUnrealEdEngine, Warning, "Brush '%ls' appears to be inside out - fixing.", *Brush->GetName());
