...
void AddGizmoSizeSlider(FToolMenuSection& InMenuSection)
{
	const FSlateIcon WidgetIcon = FSlateIcon(FAppStyle::GetAppStyleSetName(), "ViewportToolbar.GizmoScale");

	// Helper struct, used by both Legacy and New Gizmo scale sliders to specify their lambdas
	struct FWidgetLambdas
	{
		TFunction<FText()> Tooltip = []() { return FText::GetEmpty(); };
		TFunction<float()> Value = []() { return 0.0f; };
		TFunction<void(float)> OnValueChanged = [](float) {};
		TFunction<void(float, ETextCommit::Type)> OnCommit = TFunction<void(float, ETextCommit::Type)>();
	};

	// Creates a gizmo scale widget with a slider using the lambdas specified by InWidgetLambdas
	auto MakeGizmoScaleWidget = [](float InMin, float InMax, FWidgetLambdas InWidgetLambdas)
	{
		// Commit lambda is not set, so we use OnValueChanged for that too
		if (!InWidgetLambdas.OnCommit)
		{
			InWidgetLambdas.OnCommit = [OnValueChanged = InWidgetLambdas.OnValueChanged](float InValue, ETextCommit::Type)
			{
		...
		[
			...
			[
				...
				[
					...
					.MinValue(InMin)
					.MaxValue(InMax)
					.ToolTipText_Lambda(MoveTemp(InWidgetLambdas.Tooltip))
					.Value_Lambda(MoveTemp(InWidgetLambdas.Value))
					.OnValueChanged_Lambda(MoveTemp(InWidgetLambdas.OnValueChanged))
					.OnValueCommitted_Lambda(MoveTemp(InWidgetLambdas.OnCommit))
				]
			]
		];
		// clang-format on
	...
	// Legacy Gizmo Scale widget
	{
		FWidgetLambdas LegacyWidgetLambdas;
		LegacyWidgetLambdas.Tooltip = []()
		{
			return FText::AsNumber(GetDefault<UTransformGizmoEditorSettings>()->TransformGizmoSize);
		};
		LegacyWidgetLambdas.Value = []()
		{
			return GetDefault<UTransformGizmoEditorSettings>()->TransformGizmoSize;
		};
		LegacyWidgetLambdas.OnValueChanged = [](float InValue)
		{
			UTransformGizmoEditorSettings* ViewportSettings = GetMutableDefault<UTransformGizmoEditorSettings>();
			ViewportSettings->SetTransformGizmoSize(InValue);
		};

		TSharedRef<SWidget> LegacyGizmoScaleWidget = MakeGizmoScaleWidget(-10.0f,150.0f, LegacyWidgetLambdas);

		FToolMenuEntry& GizmoScaleEntry = InMenuSection.AddEntry(FToolMenuEntry::InitWidget("LegacyGizmoScale", LegacyGizmoScaleWidget, LOCTEXT("LegacyGizmoScaleLabel", "Gizmo Scale")));
		GizmoScaleEntry.Icon = WidgetIcon;
	...
	// New Gizmo Scale widget
	{
		FWidgetLambdas WidgetLambdas;
		WidgetLambdas.Tooltip = []()
		{
			return FText::AsNumber(GetDefault<UTransformGizmoEditorSettings>()->GetTransformGizmoAxisMultiplier());
		};
		WidgetLambdas.Value = []()
		{
			return GetDefault<UTransformGizmoEditorSettings>()->GetTransformGizmoAxisMultiplier();
		};
		WidgetLambdas.OnValueChanged = [](float InValue)
		{
			// Interactive: SetTransformGizmoAxisMultiplier will not store the new value in config.
			// Grants better performance while dragging the slider
			constexpr bool bInteractive = true;
			UTransformGizmoEditorSettings* ViewportSettings = GetMutableDefault<UTransformGizmoEditorSettings>();
			ViewportSettings->SetTransformGizmoAxisMultiplier(InValue, bInteractive);
		};
		WidgetLambdas.OnCommit = [](float InValue, ETextCommit::Type)
		{
			UTransformGizmoEditorSettings* ViewportSettings = GetMutableDefault<UTransformGizmoEditorSettings>();
			ViewportSettings->SetTransformGizmoAxisMultiplier(InValue);
		};

		TSharedRef<SWidget> GizmoScaleWidget = MakeGizmoScaleWidget(0.1f,10.0f, WidgetLambdas);

		FToolMenuEntry& GizmoScaleEntry = InMenuSection.AddEntry(FToolMenuEntry::InitWidget("GizmoScale", GizmoScaleWidget, LOCTEXT("GizmoScaleLabel", "Gizmo Scale")));
		GizmoScaleEntry.Icon = WidgetIcon;
		GizmoScaleEntry.Visibility = TAttribute<bool>::CreateLambda([]
		{
		...
		FNewToolMenuDelegate::CreateLambda(
			[](UToolMenu* Submenu) -> void
			{
										...
										FToolMenuEntry& CoordinateSystemSubmenu = TransformToolsSection.AddSubMenu(
											...
											FNewToolMenuDelegate::CreateLambda(
												[](UToolMenu* InSubmenu)
												{
													...
													if (TSharedPtr<FEditorViewportClient> Client = UUnrealEdViewportToolbarContext::GetEditorViewportClient(InSubmenu->Context))
													{
...

				{
					...
					Private::AddGizmoSizeSlider(GizmoSection);
...
FText GetViewModesSubmenuLabel(TWeakPtr<SEditorViewport> InViewport)
{
	...
	if (TSharedPtr<SEditorViewport> PinnedViewport = InViewport.Pin())
	{
		...
		else if (ViewMode == VMI_VisualizeMegaLights)
		{
			Label = ViewportClient->GetCurrentMegaLightsVisualizationModeDisplayName();
		}
...
FToolMenuEntry GetMeshEdgesMenuEntry(const TSharedPtr<SEditorViewport>& EditorViewport)
{
	const FNewToolMenuDelegate OpacityMenu = FNewToolMenuDelegate::CreateLambda(
		[EditorViewportWeak = EditorViewport.ToWeakPtr()](UToolMenu* Submenu)
		{
			...
			Submenu->AddMenuEntry("WireframeOpacitySubmenu", FToolMenuEntry::InitWidget(
				"WireframeOpacity", EditorViewport->BuildWireframeMenu(), LOCTEXT("WireframeOpacity", "Opacity")
			));
		}
	);
	
	const TSharedPtr<const FUICommandInfo> MeshCommand = FShowFlagMenuCommands::Get().FindCommandForFlag(FEngineShowFlags::EShowFlag::SF_MeshEdges);
	const FToolUIActionChoice ShowMeshMenuAction = FToolUIActionChoice(MeshCommand, *EditorViewport->GetCommandList());
	const FSlateIcon& Icon = FSlateIcon(FAppStyle::GetAppStyleSetName(), "EditorViewport.MeshEdges");

	FToolMenuEntry MeshMenuEntry = FToolMenuEntry::InitSubMenu(
		"MeshEdgesSubMenu",
		LOCTEXT("ViewMenuMeshEdges", "Mesh edges"),
		MeshCommand->GetDescription(),
		OpacityMenu,
		ShowMeshMenuAction,
		EUserInterfaceActionType::ToggleButton,
		false,
	...
	MeshMenuEntry.InputBindingLabel = MeshCommand->GetInputText();
	return MeshMenuEntry;
}
...
void PopulateViewModesMenu(UToolMenu* InMenu)
{
	...
	// View modes
	{
		...
		FToolMenuSection& AdvancedSection = InMenu->AddSection("Advanced_CustomSection", LOCTEXT("AdvancedHeader", "Advanced"));
		AdvancedSection.AddEntry(GetMeshEdgesMenuEntry(EditorViewport));
		AdvancedSection.InsertPosition = FToolMenuInsert("ViewMode", EToolMenuInsertType::After);
...

		{
			struct Local
			{
				static void BuildStreamingDeficitMenu(UToolMenu* Menu, IsViewModeSupportedDelegate IsViewModeSupported)
				{
					{
						FToolMenuSection& Section = Menu->AddSection(
							"LevelViewportStreamingDeficit", LOCTEXT("StreamingDeficitModesHeader", "Streaming Deficit")
						);

						UE::UnrealEd::Private::ViewModesSubmenu::AddModeIfSupported(
							IsViewModeSupported, Section, FEditorViewportCommands::Get().StreamingTextureDeficitMode, VMI_StreamingTextureDeficit
						);

						UE::UnrealEd::Private::ViewModesSubmenu::AddModeIfSupported(
							IsViewModeSupported, Section, FEditorViewportCommands::Get().StreamingTextureResidencyMode, VMI_StreamingTextureResidency
						);

						// Mesh LOD streaming modes are not shown in editor — mesh LODs are always fully loaded in editor.
						// Use console commands (viewmode StreamingMeshLODDeficit/StreamingMeshLODResidency) in cooked builds.
					}
				}
			};

			if (!IsViewModeSupported.IsBound()
				|| (IsViewModeSupported.Execute(VMI_StreamingTextureDeficit) || IsViewModeSupported.Execute(VMI_StreamingMeshLODDeficit)))
			{
				FUIAction MenuAction;
				MenuAction.GetActionCheckState = BuildParentCheckStateForCommands(InMenu->Context, {
					FEditorViewportCommands::Get().StreamingTextureDeficitMode,
					FEditorViewportCommands::Get().StreamingTextureResidencyMode
				});

				Section.AddSubMenu(
					"VisualizeGroupedStreamingDeficit",
					LOCTEXT("VisualizeGroupedStreamingDeficitDisplayName", "Streaming Deficit"),
					LOCTEXT("GroupedStreamingDeficitMenu_ToolTip", "Select a mode for streaming deficit visualization"),
					FNewToolMenuDelegate::CreateStatic(&Local::BuildStreamingDeficitMenu, IsViewModeSupported),
					MenuAction,
					EUserInterfaceActionType::RadioButton,
					/* bInOpenSubMenuOnClick = */ false,
			...
			FNewMenuDelegate::CreateLambda([WeakViewport = EditorViewport.ToWeakPtr()](FMenuBuilder& MenuBuilder)
			{
				if (TSharedPtr<SEditorViewport> Viewport = WeakViewport.Pin())
				{
					if (TSharedPtr<FEditorViewportClient> Client = Viewport->GetViewportClient())
					{
						...
						const FViewModeOptionsArgs ViewModeArgs
						{
							.ViewModeIndex = Client->GetViewMode(),
							.FeatureLevel = World ? World->GetFeatureLevel() : GMaxRHIFeatureLevel,
							.ParamNameMap = Client->GetViewModeParamNameMap(),
							.ViewportClient = Client
						};
						BuildViewModeOptionsMenu(MenuBuilder, ViewModeArgs);
					}
				}
			})
		);
							...
							for (int32 ProfileIndex = 0; ProfileIndex < PreviewProfiles.Num(); ProfileIndex++)
							{
								...
								PreviewProfilesSelectionSection.AddMenuEntry(
									...
									TAttribute<FText>::CreateLambda([]()->FText
										{
											const FInputBindingManager& InputManager = FInputBindingManager::Get();
											const TSharedPtr<FUICommandInfo> PreviousProfile = InputManager.FindCommandInContext(TEXT("AdvancedPreviewScene"), TEXT("PreviousProfile"));
											const TSharedPtr<FUICommandInfo> NextProfile = InputManager.FindCommandInContext(TEXT("AdvancedPreviewScene"), TEXT("NextProfile"));
			
											if (ensureMsgf(PreviousProfile, TEXT("AdvancedPreviewScene PreviousProfile command is not valid"))
												&& ensureMsgf(NextProfile, TEXT("AdvancedPreviewScene NextProfile command is not valid")))
											{
												return FText::FormatOrdered(
													LOCTEXT("AssetViewerProfileTooltip", "Set this profile as active\nUse {0} to navigate to the previous profile\nUse {1} to navigate to the next profile"),
													PreviousProfile->GetInputText(),
													NextProfile->GetInputText()
												);
											}
...
void AddDefaultShowFlags(UToolMenu* InMenu)
{
	...
	if (UUnrealEdViewportToolbarContext* EditorViewportContext = InMenu->FindContext<UUnrealEdViewportToolbarContext>())
	{
		...
		for (const EShowFlagGroup& Flag : EditorViewportContext->ExcludedShowMenuGroupFlags)
		{
			ShowFlagFilter.ExcludeGroup(Flag);
		}
	}
...

	{
		...
		FShowFlagMenuCommands::Get().PopulateAllShowFlagsSection(AllShowFlagsSection, ShowFlagFilter);
	}
}
		...
		FNewToolMenuSectionDelegate::CreateLambda(
			[](FToolMenuSection& InDynamicSection)
			{
						...
						InDynamicSection.FindContext<UUnrealEdViewportToolbarContext>())
				{
					...
					// Reset camera speed entry
					{
						// This entry is a toolbar only warning, telling the user that camera speed is extremely low and might cause imperceptible camera movements.

						FUIAction HideInMenuAction;
						HideInMenuAction.IsActionVisibleDelegate.BindLambda([]{ return false; });

						FToolMenuEntry ResetSpeedEntry = FToolMenuEntry::InitMenuEntry(
							"LowCameraSpeedWarning",
							LOCTEXT("LowCameraSpeedWarningLabel", "Reset Camera Speed"),
							LOCTEXT("LowCameraSpeedWarningTooltip", "Camera speed is extremely low. This might cause imperceptible camera movements.\nClick to reset camera speed to its default."),
							FSlateIcon(FAppStyle::GetAppStyleSetName(), "ViewportToolbar.CameraSpeedReset"),
							HideInMenuAction
						);

						ResetSpeedEntry.ToolBarData.ActionOverride = FToolMenuExecuteAction::CreateLambda(
							[WeakViewport = Context->Viewport](const FToolMenuContext&)
							{
								if (const TSharedPtr<SEditorViewport> ViewportPinned = WeakViewport.Pin())
								{
									if (const TSharedPtr<FEditorViewportClient> ViewportClient = ViewportPinned->GetViewportClient())
									{
										const float DefaultSpeed = FEditorViewportCameraSpeedSettings{}.GetCurrentSpeed();
										FEditorViewportCameraSpeedSettings Settings = ViewportClient->GetCameraSpeedSettings();
										Settings.SetCurrentSpeed(DefaultSpeed);
										ViewportClient->SetCameraSpeedSettings(Settings);
									}
								}
							}
						);

						ResetSpeedEntry.SetShowInToolbarTopLevel(
								...
								[WeakViewport = Context->Viewport]() -> bool
								{
									if (const TSharedPtr<SEditorViewport> ViewportPinned = WeakViewport.Pin())
									{
										if (const TSharedPtr<FEditorViewportClient> ViewportClient = ViewportPinned->GetViewportClient())
										{
											...
											{
												const float CurrentSpeed = ViewportClient->GetCameraSpeedSettings().GetCurrentSpeed();
												const float LowSpeedThreshold = ViewportClient->GetCameraSpeedSettings().GetLowSpeedWarningThreshold();
												return CurrentSpeed <= LowSpeedThreshold;
											}
										}
									}
									return false;
						...
						ResetSpeedEntry.StyleNameOverride = "ViewportToolbarWarning";

						InDynamicSection.AddEntry(ResetSpeedEntry);
					}
				}
