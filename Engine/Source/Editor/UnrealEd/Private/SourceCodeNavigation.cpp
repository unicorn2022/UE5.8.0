...
static bool GAsyncSourceCodeNavigation = true;
static FAutoConsoleVariableRef CVarAsyncSourceCodeNavigation(
	TEXT("SourceCodeNavigation.EnableAsync"),
	GAsyncSourceCodeNavigation,
	TEXT("When true, opening source files in the IDE is deferred so the editor stays responsive. Set to false to revert to synchronous (blocking) behavior."),
	ECVF_Default
);
...
void FSourceCodeNavigationImpl::NavigateToFunctionSource( const FString& FunctionSymbolName, const FString& FunctionModuleName, const bool bIgnoreLineNumber )
{
	...
	if (FPlatformStackWalk::GetFunctionDefinitionLocation(FunctionSymbolName, FunctionModuleName, SourceFileName, SourceLineNumber, SourceColumnNumber))
	{
		...
		if (FPaths::FileExists(SourceFileName) == false)
		{
			...
			{
				UE_LOGF(LogSelectionDetails, Warning, "NavigateToFunctionSource:  Didn't find source file for [%ls] - File [%ls], Line [%i], Column [%i]",
					*FunctionSymbolName,
					*SourceFileName,
					SourceLineNumber,
					SourceColumnNumber);
			...
			{
				UE_LOGF(LogSelectionDetails, Verbose, "NavigateToFunctionSource:  Found symbol file in renamed or moved code base.");
			}
		}

		UE_LOGF(LogSelectionDetails, Verbose, "NavigateToFunctionSource:  Found symbols for [%ls] - File [%ls], Line [%i], Column [%i]",
			*FunctionSymbolName,
			*SourceFileName,
			SourceLineNumber,
			SourceColumnNumber );
	...
	else
	{
		...
		UE_LOGF(LogSelectionDetails, Warning, "NavigateToFunctionSource:  Unable to find source file and line number for '%ls' [%ls]",
			*FunctionSymbolName,
		...
		if(Name == FunctionModuleName)
		{
            ...
            if(ReturnCode == 0 && !Results.IsEmpty())
            {
                   ...
                   && CloseIndex > ColonIndex && OpenIndex < ColonIndex)
                {
                    ...
                    {
                        UE_LOGF(LogSelectionDetails, Warning, "NavigateToFunctionSource: Unable to open file %ls and line number %ls", *FileName, *LineNumber);
                    }
                }
                else
                {
                    ...
                    {
                        UE_LOGF(LogSelectionDetails, Warning, "NavigateToFunctionSource: Missing dSYM files, please install Editor symbols for debugging");
                    }
                    else
                    {
                        UE_LOGF(LogSelectionDetails, Warning, "NavigateToFunctionSource: Unexpected SourceCodeLookup.sh output: %ls", *Results);
                    }
                }
            }
            else if (ReturnCode != 0 || !Errors.IsEmpty())
            {
                UE_LOGF(LogSelectionDetails, Warning, "NavigateToFunctionSource: SourceCodeLookup.sh failed with code: %d\n%ls", ReturnCode, *Errors);
            }
		}
	}
    
    UE_LOGF(LogSelectionDetails, Warning, "NavigateToFunctionSource: Unable to look up symbol: %ls in module:%ls", *FunctionSymbolName, *FunctionModuleName);
...
void FSourceCodeNavigationImpl::GatherFunctions( const FString& ModuleName, const FString& ClassName, const FShouldAbortDelegate& ShouldAbortDelegate )
{
	...
	{
		TCHAR ErrorBuffer[ MAX_SPRINTF ];
		UE_LOGF(LogSelectionDetails, Warning, "GatherFunctions:  Unable to enumerate symbols for module '%ls', search mask '%ls' [%ls]",
			*ModuleName,
			*SearchMask,
			FPlatformMisc::GetSystemErrorMessage( ErrorBuffer, MAX_SPRINTF, 0 ) );
	}
...
bool FSourceCodeNavigation::NavigateToProperty(const FProperty* InProperty)
{
	...
	if (InProperty && InProperty->IsNative())
	{
		...
		if (bFileLocated)
		{
			...
			OpenSourceFileAsync(AbsoluteSourcePath);
			return true;
		}
	}
	return false;
...
void FSourceCodeNavigation::OpenSourceFileAsync(const FString& AbsoluteSourcePath, int32 LineNumber, int32 ColumnNumber)
{
	if (!GAsyncSourceCodeNavigation)
	{
		OpenSourceFile(AbsoluteSourcePath, LineNumber, ColumnNumber);
		return;
	}
	...
	FNotificationInfo Info(LOCTEXT("OpeningSourceFile", "Opening source file..."));
	Info.Image = FAppStyle::GetBrush(TEXT("LevelEditor.RecompileGameCode"));
	Info.bFireAndForget = false;
	Info.ExpireDuration = 2.0f;
	...
	// Defer to the next game thread tick so the calling UI event handler returns immediately
	// and Slate can render a frame before the (potentially blocking) accessor call executes.
	// NOTE: We must stay on the game thread because some accessors (e.g. Rider) may show
	// Slate dialogs internally, which requires game thread access.
	FString PathCopy = AbsoluteSourcePath;
	AsyncTask(ENamedThreads::GameThread, [PathCopy, LineNumber, ColumnNumber, NotificationPtr]()
	{
		ISourceCodeAccessModule& SourceCodeAccessModule = FModuleManager::LoadModuleChecked<ISourceCodeAccessModule>("SourceCodeAccess");
		const bool bSuccess = SourceCodeAccessModule.GetAccessor().OpenFileAtLine(PathCopy, LineNumber, ColumnNumber);
		...
		if (NotificationPtr.IsValid())
		{
			...
			{
				NotificationPtr->SetText(LOCTEXT("OpenSourceFileFailed", "Failed to open source file"));
...
void FSourceCodeNavigation::OpenSourceFilesAsync(const TArray<FString>& AbsoluteSourcePaths)
{
	if (!GAsyncSourceCodeNavigation)
	{
	...
	FNotificationInfo Info(LOCTEXT("OpeningSourceFiles", "Opening source files..."));
	Info.Image = FAppStyle::GetBrush(TEXT("LevelEditor.RecompileGameCode"));
	Info.bFireAndForget = false;
	Info.ExpireDuration = 2.0f;
	Info.bUseThrobber = true;
	...
	TArray<FString> PathsCopy = AbsoluteSourcePaths;
	AsyncTask(ENamedThreads::GameThread, [PathsCopy = MoveTemp(PathsCopy), NotificationPtr]()
	{
		ISourceCodeAccessModule& SourceCodeAccessModule = FModuleManager::LoadModuleChecked<ISourceCodeAccessModule>("SourceCodeAccess");
		const bool bSuccess = SourceCodeAccessModule.GetAccessor().OpenSourceFiles(PathsCopy);
		...
		if (NotificationPtr.IsValid())
		{
			...
			{
				NotificationPtr->SetText(LOCTEXT("OpenSourceFilesFailed", "Failed to open source files"));
				NotificationPtr->SetCompletionState(SNotificationItem::CS_Fail);
			}
			NotificationPtr->ExpireAndFadeout();
		}
...
void FSourceCodeNavigationImpl::LaunchIDEInstaller(const FString& Filepath)
{
	...
	if (Params.Len() == 0)
	{
		UE_LOGF(LogSelectionDetails, Warning, "LaunchIDEInstaller: Failed to load suggested Visual Studio components from [%ls]", *WindowsSDKFile);
	}
