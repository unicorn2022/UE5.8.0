	...
	EWorldType::Type InWorldType)
{
	...
	if (!ScopedWorld)
	{
		UE_LOGF(LogAutomationEditorCommon, Error, "Failed to create scoped editor world. Could not resolve template name or path: %ls", *TemplateNameOrPath);
	}
	return ScopedWorld;
}
...
UObject* FAutomationEditorCommonUtils::ImportAssetUsingFactory(UFactory* ImportFactory, const FString& ObjectName, const FString& PackagePath, const FString& ImportPath)
{
	...
	if (Pkg)
	{
		...
		{
			UE_LOGF(LogAutomationEditorCommon, Display, "Imported %ls", *ImportPath);
		}
		else if (OutCanceled)
		{
			UE_LOGF(LogAutomationEditorCommon, Display, "Canceled import of %ls", *ImportPath);
		}
		else
		{
			UE_LOGF(LogAutomationEditorCommon, Error, "Failed to import asset using factory %ls!", *ImportFactory->GetName());
		}
	}
	else
	{
		UE_LOGF(LogAutomationEditorCommon, Error, "Failed to create a package!");
	}
	...
	if (TargetProperty)
	{
		...
		else
		{
			...
			else
			{
				...
				UE_LOGF(LogAutomationEditorCommon, Error, "ERROR: Unknown nested object type for property: %ls", *PropertyName);
			}
			...
			{
				UE_LOGF(LogAutomationEditorCommon, Error, "Error accessing null property: %ls", *PropertyName);
			}
		}
	}
	else
	{
		UE_LOGF(LogAutomationEditorCommon, Error, "ERROR: Could not find factory property: %ls", *PropertyName);
	}
}
...
float FAutomationEditorCommonUtils::TotalFromFloatArray(const TArray<float>& InFloatArray, bool bIsAveragedInstead)
{
	...
	{
		UE_LOGF(LogEditorAutomationTests, VeryVerbose, "Average value of the Array is %f", (TotalValue / InFloatArray.Num()));
		return (TotalValue / InFloatArray.Num());
	}

	UE_LOGF(LogEditorAutomationTests, VeryVerbose, "Total Value of the Array is %f", TotalValue);
	return TotalValue;
}
...
float FAutomationEditorCommonUtils::LargestValueInFloatArray(const TArray<float>& InFloatArray)
{
	...
	UE_LOGF(LogEditorAutomationTests, VeryVerbose, "The Largest value of the array is %f", LargestValue);
	return LargestValue;
}
...
void FAutomationEditorCommonUtils::CreateArrayFromFile(const FString& InFileLocation, TArray<FString>& OutArray)
{
	...
	{
		UE_LOGF(LogEditorAutomationTests, VeryVerbose, "Loading and parsing the data from '%ls' into an array.", *InFileLocation);
		FFileHelper::LoadFileToString(RawData, *InFileLocation);
		RawData.ParseIntoArray(OutArray, TEXT(","), false);
	}

	UE_LOGF(LogEditorAutomationTests, Warning, "Unable to create an array.  '%ls' does not exist.", *InFileLocation);
	RawData = TEXT("0");
...
bool FAutomationEditorCommonUtils::IsArchiveWriteable(const FString& InFilePath, const FArchive* InArchiveName)
{
	if (!InArchiveName)
	{
		UE_LOGF(LogEditorAutomationTests, Error, "Failed to write to the csv file: %ls", *FPaths::ConvertRelativePathToFull(InFilePath));
		return false;
	}
	return true;
}
...
bool FAutomationEditorCommonUtils::SetOrthoViewportView(const FVector& ViewLocation, const FRotator& ViewRotation)
{
	...
	UE_LOGF(LogEditorAutomationTests, Log, "An ortho viewport was not found.  May affect the test results.");
	return false;
}
...
FString FAutomationEditorCommonUtils::ConvertPackagePathToAssetPath(const FString& PackagePath)
{
	...
	{
		UE_LOGF(LogAutomationEditorCommon, Error, "PackagePath (%ls) is invalid for the current project", *PackagePath);
		return TEXT("");
	}
}
...
void FAutomationEditorCommonUtils::CollectGameContentTests(TArray<FString>& OutBeautifiedNames, TArray <FString>& OutTestCommands)
{
	...
	for (auto ObjIter = ObjectList.CreateConstIterator(); ObjIter; ++ObjIter)
	{
		...
		if (Asset.GetClass() == nullptr)
		{
			...
			UE_LOGF(LogAutomationEditorCommon, Warning, "GetClass for %ls (%ls) returned nullptr. Asset ignored", *Asset.AssetName.ToString(), *Asset.GetObjectPathString());
		}
		else 
		{
			FString Filename = Asset.GetObjectPathString();
...
bool FOpenEditorForAssetCommand::Update()
{
	...
	if ( Object )
	{
		...
		{
			UE_LOGF(LogEditorAutomationTests, Log, "Verified asset editor for: %ls.", *AssetName);
			UE_LOGF(LogEditorAutomationTests, Display, "The editor successfully loaded for: %ls.", *AssetName);
			return true;
		}
	}
	else
	{
		UE_LOGF(LogEditorAutomationTests, Error, "Failed to find object: %ls.", *AssetName);
	}
	return true;
}
...
bool FCloseAllAssetEditorsCommand::Update()
{
	...
	{
		UE_LOGF(LogEditorAutomationTests, Warning, "Not all of the editors were closed.");
		return true;
	}

	UE_LOGF(LogEditorAutomationTests, Log, "Verified asset editors were closed");
	UE_LOGF(LogEditorAutomationTests, Display, "The asset editors closed successfully");
	return true;
}
...
bool FEditorLoadMap::Update()
{
	...
	UE_LOGF(LogEditorAutomationTests, Log, "%ls has been loaded.", *ShortMapName);
	...
	UE_LOGF(LogEditorAutomationTests, Display, "%ls took %.3f to load.", *LoadedMapName, MapLoadTime);
...
bool FWaitForShadersToFinishCompiling::Update()
{
	...
	if (ShadersCompiling || TexturesCompiling)
	{
		if (TimeNow - LastReportTime > 5.0)
		{
			...
			if (ShadersCompiling)
			{
				UE_LOGF(LogEditorAutomationTests, Log, "Waiting for %i shaders to finish.", GShaderCompilingManager->GetNumRemainingJobs() + GShaderCompilingManager->GetNumPendingJobs());
			}

			if (TexturesCompiling)
			{
				UE_LOGF(LogEditorAutomationTests, Log, "Waiting for %i texures to finish.", FTextureCompilingManager::Get().GetNumRemainingTextures());
			}
		}
	...
	UE_LOGF(LogEditorAutomationTests, Log, "Done waiting for shaders to finish.");
	return true;
}
...
bool FChangeViewportToFirstAvailableBookmarkCommand::Update()
{
	...
	UE_LOGF(LogEditorAutomationTests, Log, "Attempting to change the editor viewports view to the first set bookmark.");
	...
	for (FLevelEditorViewportClient* ViewportClient : GEditor->GetLevelViewportClients())
	{
		...
		for ( ViewportIndex = 0; ViewportIndex <= NumberOfBookmarks; ViewportIndex++ )
		{
			...
			{
				UE_LOGF(LogEditorAutomationTests, VeryVerbose, "Changing a viewport view to the set bookmark %i", ViewportIndex);
...
bool FAddStaticMeshCommand::Update()
{
	...
	UE_LOGF(LogEditorAutomationTests, Log, "Static Mesh cube has been added to 0, 0, 0.")
...
bool FBuildLightingCommand::Update()
{
	...
	if ( GUsingNullRHI )
	{
		UE_LOGF(LogEditorAutomationTests, Log, "SKIPPED Build Lighting Step.  You're currently running with -NullRHI.");
		return true;
	}

	if ( GUnrealEd->WarnIfLightingBuildIsCurrentlyRunning() )
	{
		UE_LOGF(LogEditorAutomationTests, Warning, "Lighting is already being built.");
		return true;
	}
	...
	UE_LOGF(LogEditorAutomationTests, Log, "Building lighting in Production Quality.");
...
bool FWaitToFinishCookByTheBookCommand::Update()
{
	if ( !GUnrealEd->CookServer->IsCookByTheBookRunning() )
	{
		...
		{
			UE_LOGF(LogEditorAutomationTests, Log, "The cook by the book operation has finished.");
		}
		return true;
	}
	else if ( ( FPlatformTime::Seconds() - StartTime ) == COOK_TIMEOUT )
	{
		...
		UE_LOGF(LogEditorAutomationTests, Error, "It has been an hour or more since the cook has started.");
		return false;
	}
	return false;
}
...
bool FWaitToFinishBuildDeployCommand::Update()
{
	...
	{
		UE_LOGF(LogEditorAutomationTests, Log, "The build game and deploy operation has finished.");
		return true;
	}
	...
	{
		UE_LOGF(LogEditorAutomationTests, Warning, "The build was canceled.");
		return true;
	}
	return false;
}

bool FWaitForEditorNavMeshBuildCommand::Update()
{
	// Timeout to avoid hanging forever if the nav build never completes (e.g. build system stuck).
	// Configurable via UAutomationTestSettings::NavMeshBuildTimeoutSeconds in DefaultEngine.ini.
	const float ConfiguredTimeout = GetDefault<UAutomationTestSettings>()->NavMeshBuildTimeoutSeconds;
	const float NavMeshBuildTimeoutSeconds = FMath::Max(ConfiguredTimeout, 1.0f);
	if (ConfiguredTimeout < 1.0f)
	{
		static bool bWarnedAboutNavMeshTimeout = false;
		if (!bWarnedAboutNavMeshTimeout)
		{
			bWarnedAboutNavMeshTimeout = true;
			UE_LOGF(LogAutomationEditorCommon, Warning, "FWaitForEditorNavMeshBuildCommand: NavMeshBuildTimeoutSeconds is %.1f, which is invalid. Using %.1f instead.", ConfiguredTimeout, NavMeshBuildTimeoutSeconds);
		}
	}
	if (GetCurrentRunTime() > NavMeshBuildTimeoutSeconds)
	{
		UE_LOGF(LogAutomationEditorCommon, Warning, "FWaitForEditorNavMeshBuildCommand timed out after %.0f seconds waiting for editor navmesh build to complete.", NavMeshBuildTimeoutSeconds);
		return true;
	}
	...
	for (const FWorldContext& Context : GEngine->GetWorldContexts())
	{
		...
		{
			UNavigationSystemV1* NavigationSys = FNavigationSystem::GetCurrent<UNavigationSystemV1>(Context.World());
			if (NavigationSys)
			{
				// Returns true immediately if nav is already clean — zero cost for maps where the navmesh
				// is pre-built or where the test doesn't care about navigation.
				return !NavigationSys->IsNavigationBuildInProgress() && !NavigationSys->HasDirtyAreasQueued();
			}
			break;
		}
	}

	return true; // No editor world or no nav system — returns immediately on first tick, zero cost for maps that don't use navigation.
}

FStartPIEForAutomationCommand::FStartPIEForAutomationCommand(FRequestPlaySessionParams InRequestParams)
	: RequestParams(InRequestParams)
	, bPIEStarted(false)
	, bPIEFailed(false)
	, bSessionRequested(false)
{
	FEditorDelegates::PostPIEStarted.AddRaw(this, &FStartPIEForAutomationCommand::OnPostPIEStarted);
	FEditorDelegates::EndPIE.AddRaw(this, &FStartPIEForAutomationCommand::OnEndPIE);
}

FStartPIEForAutomationCommand::~FStartPIEForAutomationCommand()
{
	FEditorDelegates::PostPIEStarted.RemoveAll(this);
	FEditorDelegates::EndPIE.RemoveAll(this);
	// Balance the AddToRoot() called in AutomationLoadMap. The destructor always runs regardless
	// of whether Update() was ever called, so this covers test abort and framework teardown paths.
	// The null check guards against the unlikely case where EditorPlaySettings was never set.
	if (ULevelEditorPlaySettings* EditorPlaySettings = RequestParams.EditorPlaySettings.Get())
	{
		EditorPlaySettings->RemoveFromRoot();
	}
}

bool FStartPIEForAutomationCommand::Update()
{
	// First tick: fire the PIE request
	...
	const float ConfiguredPIETimeout = GetDefault<UAutomationTestSettings>()->PIEStartTimeoutSeconds;
	const float TimeoutSeconds = FMath::Max(ConfiguredPIETimeout, 1.0f);
	if (ConfiguredPIETimeout < 1.0f)
	{
		static bool bWarnedAboutPIETimeout = false;
		if (!bWarnedAboutPIETimeout)
		{
			bWarnedAboutPIETimeout = true;
			UE_LOGF(LogAutomationEditorCommon, Warning, "FStartPIEForAutomationCommand: PIEStartTimeoutSeconds is %.1f, which is invalid. Using %.1f instead.", ConfiguredPIETimeout, TimeoutSeconds);
		}
	}

	// PIE started successfully — wait for the world to be ready.
	// Matches the condition used by FWaitForMapToLoadCommand. HasMatchStarted() checks World->bMatchStarted,
	// which is set by AGameModeBase::StartPlay() -> GameState->HandleBeginPlay() -> NotifyMatchStarted()
	// for all AGameModeBase-derived game modes, including AFunctionalTestGameMode.
	if (bPIEStarted)
	{
		// PIE ended after starting — covers mid-initialization crashes (e.g. Blueprint load failure)
		if (bPIEFailed)
		{
			if (FAutomationTestBase* CurrentTest = FAutomationTestFramework::Get().GetCurrentTest())
			{
				CurrentTest->AddError(TEXT("FStartPIEForAutomationCommand: PIE ended before the world became ready."));
			}
			else
			{
				UE_LOGF(LogAutomationEditorCommon, Error, "FStartPIEForAutomationCommand: PIE ended before the world became ready.");
			}
			return true;
		}

		UWorld* GameWorld = AutomationCommon::GetAnyGameWorld();
		if (GameWorld && GameWorld->AreActorsInitialized())
		{
		...
		// Timeout guards against the world stalling after PIE has started
		if (GetCurrentRunTime() > TimeoutSeconds)
		{
			const FText ErrorMsg = FText::Format(
				FText::FromString(TEXT("FStartPIEForAutomationCommand: world did not become ready after {0} seconds (PIEStartTimeoutSeconds = {1}).")),
				FText::AsNumber(FMath::RoundToInt(GetCurrentRunTime())),
				FText::AsNumber(TimeoutSeconds));
			if (FAutomationTestBase* CurrentTest = FAutomationTestFramework::Get().GetCurrentTest())
			{
				CurrentTest->AddError(ErrorMsg.ToString());
			}
			else
			{
				UE_LOGF(LogAutomationEditorCommon, Error, "%ls", *ErrorMsg.ToString());
			}
			return true;
		}

		return false; // PIE is up but world not ready yet, keep waiting
	}

	// PIE failed before it started (EndPIE fired before PostPIEStarted)
	if (bPIEFailed)
	{
		if (FAutomationTestBase* CurrentTest = FAutomationTestFramework::Get().GetCurrentTest())
		{
			CurrentTest->AddError(TEXT("FStartPIEForAutomationCommand: PIE failed to start (Blueprint compilation error or other startup failure)."));
		}
		else
		{
			UE_LOGF(LogAutomationEditorCommon, Error, "FStartPIEForAutomationCommand: PIE failed to start (Blueprint compilation error or other startup failure).");
		}
		return true;
	}

	// Timeout — guards against the session request silently failing before either delegate fires.
	if (GetCurrentRunTime() > TimeoutSeconds)
	{
		const FText ErrorMsg = FText::Format(
			FText::FromString(TEXT("FStartPIEForAutomationCommand: PIE did not start after {0} seconds (PIEStartTimeoutSeconds = {1}).")),
			FText::AsNumber(FMath::RoundToInt(GetCurrentRunTime())),
			FText::AsNumber(TimeoutSeconds));
		if (FAutomationTestBase* CurrentTest = FAutomationTestFramework::Get().GetCurrentTest())
		{
	...
	return false; // still waiting for PIE to come up or fail
}

void FStartPIEForAutomationCommand::OnPostPIEStarted(const bool bInSimulateInEditor)
{
	bPIEStarted = true;
}

void FStartPIEForAutomationCommand::OnEndPIE(const bool bInSimulateInEditor)
{
	// Set bPIEFailed unconditionally — covers both "PIE never started" and "PIE started but
	// ended before the world became ready" (e.g. mid-initialization crash).
	bPIEFailed = true;
}
	...
	bool FProjectMapsPIETest::RunTest(const FString& Parameters)
{
	...
	if (FParse::Value(FCommandLine::Get(), TEXT("-maps="), ParsedMapName) || FParse::Value(FCommandLine::Get(), TEXT("-map="), ParsedMapName))
	{
		...
		UE_LOGF(LogEditorAutomationTests, Display, "Found Maps %ls on command line. PIE Test will use these maps", *ParsedMapName);
	}
	else if (FParse::Token(ParsedCmdLine, ParsedMapName, false) && ParsedMapName.StartsWith(TEXT("-")) == false)
	{
			...
			FPaths::GetExtension(InitialMapName, /*bIncludeDot=*/true) == FPackageName::GetMapPackageExtension())
		{
			...
			UE_LOGF(LogEditorAutomationTests, Display, "Found Map %ls on command line. PIE Test will be restricted to this map", *InitialMapName);
		}
	}
	...
	if (PIEMaps.Num() == 0)
	{
		...
		else
		{
			...
			UE_LOGF(LogEditorAutomationTests, Display, "No MapsToPIE or MapsToTest specified in DefaultEngine.ini [/Script/Engine.AutomationTestSettings]. Using GameStartup or EditorStartup Map");
	...
	if (PIEMaps.Num() == 0)
	{
		UE_LOGF(LogEditorAutomationTests, Fatal, "No automation or default maps are configured for PIE!");
	}
	...
	for (const FString& Map : PIEMaps)
	{
		...
		{
			UE_LOGF(LogEditorAutomationTests, Error, "Couldn't resolve map for PIE test from %ls to valid package name!", *MapPackageName);
			continue;
		}

		UE_LOGF(LogEditorAutomationTests, Display, "Queueing Map %ls for PIE Automation", *MapPackageName);
