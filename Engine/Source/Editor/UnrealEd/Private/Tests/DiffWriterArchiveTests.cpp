...
bool FDiffWriterArchiveTestsCallstacks::RunTest(const FString& Parameters)
{
	...
	// Accumulator->bFirstSaveComplete should be is false, so bCollectingCallstacks should be false.
	// We always collect a callstack struct, but in the !bCollectingCallstacks case the callstack struct
	// we collected should have null cpp callstack data.
	...
	TestTrueExpr(Callstacks.GetCallstack(0).Callstack != nullptr
		&& Callstacks.GetCallstack(0).Callstack->Callstack == nullptr);
	TestTrueExpr(Callstacks.GetCallstack(0).bSuppressLogging == false);
	TestTrueExpr(Callstacks.GetCallstack(1).Offset == 4);
	TestTrueExpr(Callstacks.GetCallstack(1).Callstack != nullptr
		&& Callstacks.GetCallstack(1).Callstack->Callstack == nullptr);
	TestTrueExpr(Callstacks.GetCallstack(1).bSuppressLogging == false);
	TestTrueExpr(Callstacks.GetCallstack(2).Offset == 8);
	TestTrueExpr(Callstacks.GetCallstack(2).Callstack != nullptr
		&& Callstacks.GetCallstack(2).Callstack->Callstack == nullptr);
