...
namespace FbxMeshUtils
{
	...
	bool ImportStaticMeshLOD( UStaticMesh* BaseStaticMesh, const FString& Filename, int32 LODLevel, bool bAsync)
	{
		if (!BaseStaticMesh)
		{
			UE_LOGF(LogExportMeshUtils, Log, "Cannot import custom LOD because the staticmesh is NULL.");
			return false;
		}
	...
	bool ImportStaticMeshHiResSourceModel(UStaticMesh* BaseStaticMesh, const FString& Filename, bool bAsync)
	{
		if (!BaseStaticMesh)
		{
			UE_LOGF(LogExportMeshUtils, Log, "Cannot import custom high res mesh because the staticmesh is NULL.");
			return false;
		}
		...
		UE_LOGF(LogExportMeshUtils, Log, "Fbx Mesh loading");
	...
	bool ImportSkeletalMeshLOD( class USkeletalMesh* SelectedSkelMesh, const FString& Filename, int32 LODLevel, bool bAsync)
	{
		...
		if (!SelectedSkelMesh)
		{
			UE_LOGF(LogExportMeshUtils, Error, "Cannot import a LOD if there is not a valid selected skeletal mesh.");
			return false;
		}
	...
	TFuture<bool> ImportMeshLODDialog(class UObject* SelectedMesh, int32 LODLevel, bool bNotifyCB /*= true*/, bool bReimportWithNewFile /*= false*/)
	{
		...
		if (bInvalidLodIndex)
		{
			UE_LOGF(LogExportMeshUtils, Warning, "ImportMeshLODDialog: Invalid mesh LOD index %d, no prior LOD index exists.", LODLevel);
		...
		if (!bSourceFileExists)
		{
			...
			if (FilePicker)
			{
				...
				if (bFilePickerResult)
				{
					...
					{
						UE_LOGF(LogExportMeshUtils, Display, "ImportMeshLODDialog: Error when picking a file to import LOD index %d.", LODLevel);
						Promise->SetValue(false);
						return Promise->GetFuture();
					}
				}
				...
				{
					UE_LOGF(LogExportMeshUtils, Display, "ImportMeshLODDialog: User cancel import LOD index %d.", LODLevel);
					Promise->SetValue(false);
					return Promise->GetFuture();
				}
			}
		...
		if (!bSourceFileExists)
		{
			UE_LOGF(LogExportMeshUtils, Display, "ImportMeshLODDialog: Cannot import LOD index %d. The filename do not exist.", LODLevel);
			Promise->SetValue(false);
			return Promise->GetFuture();
		}
