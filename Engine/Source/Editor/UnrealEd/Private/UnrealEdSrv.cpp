...
namespace UnrealEdSrvLocals
{
	UTypedElementSelectionSet* GetLevelEditorSelectionSet()
	{
	...
	UTypedElementCommonActions* GetLevelEditorCommonActions()
	{
		if (GCurrentLevelEditingViewportClient)
		{
			if (TSharedPtr<ILevelEditor> LevelEditor = GCurrentLevelEditingViewportClient->ParentLevelEditor.Pin())
			{
				return LevelEditor->GetCommonActions();
			}
		}
		return nullptr;
	}
...
static void PrivateDumpSelection(USelection* Selection)
{
	for ( FSelectionIterator Itor(*Selection) ; Itor ; ++Itor )
	{
		...
		if ( CurObject )
		{
			UE_LOGF(LogUnrealEdSrv, Log, "    %ls", *CurObject->GetClass()->GetName() );
		}
		else
		{
			UE_LOGF(LogUnrealEdSrv, Log, "    NULL object");
		}
	}
}
...
private:
	...
	FReply OnNewModalWindowClicked()
	{
		...
		UE_LOGF(LogUnrealEdSrv, Log, "Modal Window Returned");
	...
	if( FParse::Command( &Str, TEXT( "SavePackage" ) ) )
	{
		...
		if( FParse::Value( Str, TEXT( "FILE=" ), TempFname ) && ParseObject<UPackage>( Str, TEXT( "Package=" ), Pkg, NULL ) )
		{
			...
			for( int32 CurObjectIndex = 0; CurObjectIndex < BrowsableObjectsInPackage.Num(); ++CurObjectIndex )
			{
				...
				{
					UE_LOGF(LogUnrealEdSrv, Log, "Saving Thumb for %ls", *CurObject->GetFullName());
					UE_LOGF(LogUnrealEdSrv, Log, "   Thumb existed = %d", (ExistingThumbnail!=NULL) ? 1: 0);
					UE_LOGF(LogUnrealEdSrv, Log, "   Shared Thumb = %d", (bUsesGenericThumbnail) ? 1: 0);
				}
				...
				if (!ExistingThumbnail && bUsesGenericThumbnail)
				{
					...
					if( ThumbnailTools::ConditionallyLoadThumbnailsForObjects( ObjectFullNames, LoadedThumbnails ) )
					{
						...
						if (bPrintThumbnailDiagnostics)
						{
							UE_LOGF(LogUnrealEdSrv, Log, "   Unloaded thumb loaded successfully");
						}

						ExistingThumbnail = LoadedThumbnails.Find( ObjectFullNameFName );
						if (bPrintThumbnailDiagnostics)
						{
							UE_LOGF(LogUnrealEdSrv, Log, "   Newly loaded thumb exists = %d", (ExistingThumbnail!=NULL) ? 1: 0);
							if (ExistingThumbnail)
							{
								UE_LOGF(LogUnrealEdSrv, Log, "   Thumb created after proper version = %d", (ExistingThumbnail->IsCreatedAfterCustomThumbsEnabled()) ? 1: 0);
							}
						}
						...
						if (ExistingThumbnail && !ExistingThumbnail->IsCreatedAfterCustomThumbsEnabled())
						{
							if (bPrintThumbnailDiagnostics)
							{
								UE_LOGF(LogUnrealEdSrv, Log,  "   WIPING OUT THUMBNAIL!!!!");
							}
					...
					else
					{
						if (bPrintThumbnailDiagnostics)
						{
							UE_LOGF(LogUnrealEdSrv, Log, "   Unloaded thumb does not exist");
						}
					}
				}
...
bool UUnrealEdEngine::HandleDumpModelGUIDCommand( const TCHAR* Str, FOutputDevice& Ar )
{
	...
	{
		UE_LOGF(LogUnrealEdSrv, Log, "%ls Guid = '%ls'", *It->GetFullName(), *It->LightingGuid.ToString());
	}
return true;
}
...
bool UUnrealEdEngine::HandleModalTestCommand( const TCHAR* Str, FOutputDevice& Ar )
{
	...
	UE_LOGF(LogUnrealEdSrv, Log,  "User response was: %ls", MessageBox->GetResponse() ? TEXT("OK") : TEXT("Cancel") );
	return true;
}
...
bool UUnrealEdEngine::HandleDisallowExportCommand(const TCHAR* Str, FOutputDevice& Ar)
{
	...
	for (const FAssetData& AssetData : SelectedAssets)
	{
		...
		if (Object)
		{
			...
			UE_LOGF(LogUnrealEdSrv, Log, "Marked '%ls' as not exportable", *Object->GetName());
		}
	}
...
{
	UE_LOGF(LogUnrealEdSrv, Log, "--- Listing all blueprint generated classes ---");
	for( TObjectIterator<UClass> it; it; ++it )
	{
		const UClass* CurrentClass = *it;
		if( CurrentClass && CurrentClass->ClassGeneratedBy )
		{
			UE_LOGF(LogUnrealEdSrv, Log, "  %ls (%ls)", *CurrentClass->GetName(), *CurrentClass->GetOutermost()->GetName());
		}
	}
	return true;
}
...
{
	UE_LOGF(LogUnrealEdSrv, Log, "--- Finding all actor instances with outdated classes ---");
	int32 NumFound = 0;
	for( TObjectIterator<UObject> it; it; ++it )
	{
		...
		{
			UE_LOGF(LogUnrealEdSrv, Log, "  %ls (%ls)", *CurrentObj->GetName(), *CurrentObj->GetClass()->GetName());
			NumFound++;
		}
	}
	UE_LOGF(LogUnrealEdSrv, Log, "Found %d instance(s).", NumFound);
	return true;
}

bool UUnrealEdEngine::HandleDumpSelectionCommand( const TCHAR* Str, FOutputDevice& Ar )
{
	UE_LOGF(LogUnrealEdSrv, Log, "Selected Actors:");
	PrivateDumpSelection( GetSelectedActors() );
	UE_LOGF(LogUnrealEdSrv, Log, "Selected Non-Actors:");
			...
			if( SelectedMeshes.Num() )
			{
				...
				for (int32 MeshIndex = 0; MeshIndex < SelectedMeshes.Num(); ++MeshIndex)
				{
					...
					if (Mesh && Mesh->GetNumSourceModels() > 0)
					{
						...
						UE_LOGF(LogUnrealEdSrv, Log, "Rescaling mesh '%ls' with scale: %ls", *Mesh->GetName(), *Model.BuildSettings.BuildScale3D.ToString() );
		...
		struct Local
		{
			static bool RemoveSourcePath( const FAssetImportInfo& ImportInfo, const FAssetData& AssetData, const TArray<FString>* SearchTerms )
			{
				...
				for (const auto& File : ImportInfo.SourceFiles)
				{
					...
					if( bRemoveFile )
					{
						UE_LOGF(LogUnrealEdSrv, Log, "Removing Path: %ls", *File.RelativeFilename);
						bModified = true;
					}
					else
					{
...
{
	using namespace UnrealEdSrvLocals;

	const FText TransactionName = NSLOCTEXT("UnrealEd", "Cut", "Cut");
	...
	if( FParse::Command(&Str,TEXT("CUT")) )
	{
		...
		// There are a few different cases that can happen here:
		// 1. If TypedElementCommonActionsUtils::IsElementCopyAndPasteEnabled() is enabled, actors and components go through the common actions
		//	  If not, they go through the legacy code path
		// 2. If TEDS.Feature.Folders.TypedElements is enabled, folders go through common actions
		//    If not, folders rely on delegates like OnEditCutActorsBegin to get copied by the Scene Outliner widget

		// Since different copy paths for folders/actors/components can end up overwriting each other depending on what is enabled if we directly 
		// modify the clipboard, we create a temporary buffer instead to append to
		FString CopyBuffer;
		const FScopedTransaction Transaction(TransactionName);
	
		// Keep track of whether the actor cut begin delegate was fired along the various code paths so we can also fire the cut end delegate
		bool bActorCutBeginDelegateFired = false;
		
		// When typed elements copy paste is enabled for actors, we don't need to go through the legacy cut path for actors or components
		...
		else
		{
			...
			{
				edactCopySelected(InWorld, &CopyBuffer);
			...
			else
			{
				...
				bActorCutBeginDelegateFired = true;
				const bool bCheckRef = GetDefault<ULevelEditorMiscSettings>()->bCheckReferencesOnDelete;
				CopySelectedActorsToClipboard(InWorld, true, false /*bIsMove*/, bCheckRef, &CopyBuffer);
			}
		}
		
		// Unlike copy, we first go through the legacy path for actors and then the common actions for folders (if enabled). This because if we 
		// cut (copy+delete) the folder first and then cut the actor it does not have a parent folder anymore to copy onto the clipboard, which leads
		// to incorrect data on paste
		if (UTypedElementCommonActions* CommonActions = GetLevelEditorCommonActions())
		{
			FString CommonActionsCopyBuffer;
			...
			UTypedElementSelectionSet* SelectionSet = GetLevelEditorSelectionSet();
			CommonActions->CutSelectedElements(SelectionSet, InWorld, Options, &CommonActionsCopyBuffer);
			CopyBuffer.Append(CommonActionsCopyBuffer);
		}
		
		// Append all the copy contents so far into one buffer and then copy to clipboard
		if (!CopyBuffer.IsEmpty())
		{
			FPlatformApplicationMisc::ClipboardCopy(*CopyBuffer);
		}
		
		// This delegate was fired after the contents were copied onto the clipboard in the past, so we fire it at the end
		if (bActorCutBeginDelegateFired)
		{
	...
	else if( FParse::Command(&Str,TEXT("COPY")) )
	{
		...
		{
			UTypedElementSelectionSet* SelectionSet = GetLevelEditorSelectionSet();
			CommonActions->CopySelectedElementsToString(SelectionSet, CopyBuffer);
		}
		
		// Keep track of whether the actor copy begin delegate was fired along the various code paths so we can also fire the copy end delegate
		bool bActorCopyBeginDelegateFired = false;
		...
		{
			FString LegacyCopyBuffer;
			if (bComponentsSelected)
			{
				edactCopySelected(InWorld, &LegacyCopyBuffer);
			}
			else
			{
				FEditorDelegates::OnEditCopyActorsBegin.Broadcast();
				bActorCopyBeginDelegateFired = true;
				CopySelectedActorsToClipboard(InWorld, false, true, true, &LegacyCopyBuffer);
			}
			
			// The old paste logic for actors and folders combined relies on the actor copy being first in the clipboard for some reason
			// So we append the common actions copy buffer to the end of the legacy copy buffer and then do a swap
			LegacyCopyBuffer.Append(CopyBuffer);
			Swap(CopyBuffer, LegacyCopyBuffer);
		}
		...
		// This delegate was fired after the contents were copied onto the clipboard in the past, so we fire it at the end
		if (bActorCopyBeginDelegateFired)
		{
	...
	else if( FParse::Command(&Str,TEXT("PASTE")) )
	{
		...
		bool bPastedViaCommonActions = false;
		bool bClipboardCopyFailed = false;
		
		// Single transaction for all paste code paths, since similar to copy/cut the actual contents of the paste depend on the various
		// legacy code paths and enabled cvars for actors/components/folders to determine whether common actions or legacy code paths are used. 
		// But we want a single transaction for all of those to ensure that an undo reverts all of them at once
		const FScopedTransaction Transaction(NSLOCTEXT("UnrealEd", "Paste", "Paste"));
		
		if (UTypedElementCommonActions* CommonActions = GetLevelEditorCommonActions())
		{
			FString ClipboardPaste;
			FPlatformApplicationMisc::ClipboardPaste(ClipboardPaste);
			if (!ClipboardPaste.IsEmpty())
			{
				UTypedElementSelectionSet* SelectionSet = GetLevelEditorSelectionSet();
				...
				if (FParse::Value(Str, TEXT("TO="), TempStr))
				{
					if (!FCString::Strcmp(*TempStr, TEXT("HERE")))
					{
						...
						PasteLocation = SaveClickLocation;
					}
					else
					{
						if (!FCString::Strcmp(*TempStr, TEXT("ORIGIN")))
						{
							bPasteAtLocation = false;
						}
					}
				}
				...
				Options.bPasteAtLocation = bPasteAtLocation;
				Options.PasteLocation = PasteLocation;
				TArray<FScriptTypedElementHandle> PastedElements = CommonActions->PasteElementsFromString(SelectionSet, InWorld, Options, ClipboardPaste);
				bPastedViaCommonActions = !PastedElements.IsEmpty();
			}
			else
			{
				// Grabbing clipboard contents can fail on some platforms if another program has the lock.
				// Our HAL doesn't expose that though.  If the clipboard is empty then fail to go the legacy path (which internally will
				// retry the clipboard - this MAY succeed and we don't want to go down that path if it does).
				bClipboardCopyFailed = true;
			}
		}

		if (bClipboardCopyFailed || (bPastedViaCommonActions && TypedElementCommonActionsUtils::IsElementCopyAndPasteEnabled()))
		{
			// do nothing
		}
		else
...
bool UUnrealEdEngine::Exec_Pivot( const TCHAR* Str, FOutputDevice& Ar )
{
	...
	else if( FParse::Command(&Str,TEXT("CENTERSELECTION")) )
	{
		...
		if (UTypedElementSelectionSet* ElementSelectionSet = GetLevelEditorSelectionSet())
		{
			ElementSelectionSet->ForEachSelectedElement<ITypedElementWorldInterface>(
				[&Center, &Count](const TTypedElement<ITypedElementWorldInterface>& InElement)
			{
				// Special case legacy ABrush handling
				if (ABrush* Brush = Cast<ABrush>(ActorElementDataUtil::GetActorFromHandle(InElement)))
				{
				...
				// If we got here, it's not a brush

				FTransform WorldTransform;
				if (InElement.GetWorldTransform(WorldTransform))
				{
					Center += WorldTransform.GetLocation();
					++Count;
				}
			...
			if (SMComp && SMComp->IsRegistered() && SMComp->GetStaticMesh())
			{
				...
				if ( StaticMesh )
				{
					...
					if ( NumLods )
					{
						...
						FIndexArrayView Indices = MeshLodZero.IndexBuffer.GetArrayView();

						if (Indices.Num() < NumTriangles * 3)
						{
							continue;
						}
						...
						for ( int32 TriangleIndex = 0; TriangleIndex < NumTriangles; TriangleIndex++ )
						{
							...
							TUniquePtr<FPoly> Polygon = MakeUnique<FPoly>();
...
bool UUnrealEdEngine::Exec_Actor( UWorld* InWorld, const TCHAR* Str, FOutputDevice& Ar )
{
	...
	else if( FParse::Command(&Str,TEXT("SELECT")) )
	{
		...
		{
			UE_LOGF(LogUnrealEdSrv, Log, "Select relevant lights!");
	...
	else if( FParse::Command(&Str,TEXT("DELETE")) )		// ACTOR SELECT DELETE
	{
		if (UTypedElementCommonActions* CommonActions = GetLevelEditorCommonActions())
		{
			...
			if (!ensure(SelectionSet) || SelectionSet->GetNumSelectedElements() == 0)
			{
	...
	{
		UTypedElementSelectionSet* ElementSelectionSet = GetLevelEditorSelectionSet();
		if (ensure(ElementSelectionSet))
		{
			FScopedLevelDirtied				LevelDirtyCallback;
			FScopedActorPropertiesChange	ActorPropertiesChangeCallback;

			// Bakes the current pivot position into all selected elements

			FEditorModeTools& EditorModeTools = GLevelEditorModeTools();

			ElementSelectionSet->ForEachSelectedElement<ITypedElementWorldInterface>(
				[&EditorModeTools](const TTypedElement<ITypedElementWorldInterface>& InElement)
			{
				FTransform WorldTransform;
				if (InElement.GetWorldTransform(WorldTransform))
				{
					FVector Delta(EditorModeTools.PivotLocation - WorldTransform.GetLocation());
					InElement.SetPivotOffset(WorldTransform.InverseTransformVector(Delta));
				}
	...
	else if( FParse::Command(&Str,TEXT("UNBAKEPREPIVOT")) )
	{
		...
		if (ensure(ElementSelectionSet))
		{
			...
			// Resets the pivot offset of the selected elements to zero while leaving them in the same world location.

			ElementSelectionSet->ForEachSelectedElement<ITypedElementWorldInterface>(
			[](const TTypedElement<ITypedElementWorldInterface>& InElement)
			{
				InElement.SetPivotOffset(FVector::ZeroVector);
				return true; // continue
			});
			SetPivotMovedIndependently(false);
	...
	else if( FParse::Command(&Str,TEXT("TOGGLE")) )
	{
		...
		{
			ToggleSelectedItemMovementLock();
		}
