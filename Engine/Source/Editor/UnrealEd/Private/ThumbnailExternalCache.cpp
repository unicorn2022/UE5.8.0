...
void FThumbnailExternalCache::LoadCompressAndAppend(const TArrayView<FAssetData> InAssetDatas, FCombinedThumbnailCacheToSave& CombinedCache)
{
	...
	// Init() is not threadsafe, so call it here before doing ParallelFor below in case it hasn't already been previously called.
	Init();
...
void FSaveThumbnailCache::Save(FArchive& Ar, FCombinedThumbnailCacheToSave& CombinedCache, bool bSort)
{
	...
	UE_LOGF(LogThumbnailExternalCache, Log, "Saving thumbnails for %d/%d assets (%d unique thumbnails) to %ls", NumAssetDatas, CombinedCache.Tasks.Num(), CombinedCache.DeduplicateMap.Num(), *Ar.GetArchiveName());
	...
	Ar << NumThumbnails;
	{
		...
		for (FPackageThumbnailRecord& PackageThumbnailRecord : PackageThumbnailRecords)
		{
			...
			UE_LOGF(LogThumbnailExternalCache, Verbose, "\t[%lld] %ls", Index++, *ThumbnailNameString);
	...
	UE_LOGF(LogThumbnailExternalCache, Log, "Load Time: %f secs, Save Time: %f secs, Total Time: %f secs", CombinedCache.AccumlatedLoadTime, SaveTime, (FPlatformTime::Seconds() - TimeStart) + CombinedCache.AccumlatedLoadTime);
	UE_LOGF(LogThumbnailExternalCache, Log, "Thumbnails: %d, %f MB", PackageThumbnailRecords.Num(), (TotalCompressedBytes / (1024.0 * 1024.0)));
	UE_LOGF(LogThumbnailExternalCache, Log, "Duplicates: %d, %f MB", NumDuplicates, (DuplicateBytesSaved / (1024.0 * 1024.0)));
}
