...
bool ShouldFlagsInGroupKeepDeclarationOrder(EShowFlagGroup InGroup)
{
	switch (InGroup)
	{
	case SFG_Lumen:
...
TArray<FShowFlagData>& GetShowFlagMenuItems()
{
	...
	if(bFirst)
	{
		...
		// FEngineShowFlags 
		{
			...
			EngineFlagsChords.Add("Grid", FInputChord(EKeys::G, EModifierKey::Alt | EModifierKey::Shift) );
			EngineFlagsChords.Add("MeshEdges", FInputChord(EKeys::Nine, EModifierKey::Alt) );
		...
		struct FShowFlagSorter
		{
			FORCEINLINE bool operator()( const FShowFlagData& A, const FShowFlagData& B ) const
			{
				...
				else if (ShouldFlagsInGroupKeepDeclarationOrder(A.Group))
				{
					// if group wants to keep declaration order do that
					return A.EngineShowFlagIndex < B.EngineShowFlagIndex;
				}
				else
				{
		...
		OutShowFlags.Sort(FShowFlagSorter());
	}
