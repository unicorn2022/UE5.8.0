...
void FLightmassProcessor::ImportIrradianceTasks(bool& bGenerateSkyShadowing, TArray<FImportedVolumetricLightmapTaskData>& TaskDataArray)
{
	...
	while (Element)
	{
		...
		{
			UE_LOGF(LogVolumetricLightmapImport, Fatal,  "Error, failed to import volumetric lightmap %ls with error code %d", *ChannelName, Channel );
		}
...
void FLightmassProcessor::ImportVolumetricLightmap()
{
...

	{
		...
		UE_LOGF(LogVolumetricLightmapImport, Log, "Imported Volumetric Lightmap in %.3fs", ImportTime);
		UE_LOGF(LogVolumetricLightmapImport, Log, "     Indirection Texture %ux%ux%u = %.1fMb",
		...
		UE_LOGF(LogVolumetricLightmapImport, Log, "     BrickData (all levels) %.1fMb%ls",
			BrickDataSize / 1024.0f / 1024.0f,
			*TrimmedString);

		UE_LOGF(LogVolumetricLightmapImport, Log, "     Bricks at depth");
		...
		for (int32 CurrentDepth = 0; CurrentDepth < VolumetricLightmapSettings.MaxRefinementLevels; CurrentDepth++)
		{
			...
			UE_LOGF(LogVolumetricLightmapImport, Log, "         %u: %.1f%% covering %.1f%% of volume",
				CurrentDepth,
				100.0f * BricksAtCurrentDepth.Num() / TotalNumBricks,
				100.0f * BricksAtCurrentDepth.Num() * CurrentDepthBrickVolume / TotalVolume);
		}

		UE_LOGF(LogVolumetricLightmapImport, Log, "     Bricks in each level");
		...
		for (auto Pair : SubLevelTotalBricks)
		{
			...
			{
				UE_LOGF(LogVolumetricLightmapImport, Log, "         %ls \t\t\t %d bricks \t %.1f%% (Persistent Level)", *LevelName, Pair.Value, 100.0f * Pair.Value / TotalNumBricks);
			}
			else
			{
				UE_LOGF(LogVolumetricLightmapImport, Log, "         %ls \t\t\t %d bricks \t %.1f%%", *LevelName, Pair.Value, 100.0f * Pair.Value / TotalNumBricks);
			}
		}
	}
}
