...
struct FLightmassMaterialCompiler : public FProxyMaterialCompiler
{
	...
	{
		//UE_LOGF(LogLightmassRender, Log, "Lightmass material compiler has encountered WorldPosition... Forcing constant (0.0f,0.0f,0.0f).");
		return Compiler->Constant3(0.0f,0.0f,0.0f);
	}

	virtual int32 ObjectWorldPosition(EPositionOrigin OriginType) override
	{
		//UE_LOGF(LogLightmassRender, Log, "Lightmass material compiler has encountered ObjectWorldPosition... Forcing constant (0.0f,0.0f,0.0f).");
		return Compiler->Constant3(0.0f,0.0f,0.0f);
	}

	virtual int32 ObjectRadius() override
	{
		//UE_LOGF(LogLightmassRender, Log, "Lightmass material compiler has encountered ObjectRadius... Forcing constant 500.0f.");
		return Compiler->Constant(500);
	}

	virtual int32 ObjectBounds() override
	{
		//UE_LOGF(LogLightmassRender, Log, "Lightmass material compiler has encountered ObjectBounds... Forcing constant (0,0,0).");
	...
	{
		//UE_LOGF(LogLightmassRender, Log, "Lightmass material compiler has encountered CameraVector... Forcing constant (0.0f,0.0f,1.0f).");
		return Compiler->Constant3(0.0f,0.0f,1.0f);
	}

	virtual int32 LightVector() override
	{
		//UE_LOGF(LogLightmassRender, Log, "Lightmass material compiler has encountered LightVector... Forcing constant (1.0f,0.0f,0.0f).");
		return Compiler->Constant3(1.0f,0.0f,0.0f);
	}

	virtual int32 ReflectionVector() override
	{
		//UE_LOGF(LogLightmassRender, Log, "Lightmass material compiler has encountered ReflectionVector... Forcing constant (0.0f,0.0f,-1.0f).");
		return Compiler->Constant3(0.0f,0.0f,-1.0f);
	}

	virtual int32 ReflectionAboutCustomWorldNormal(int32 CustomWorldNormal, int32 bNormalizeCustomWorldNormal) override
	{
		//UE_LOGF(LogLightmassRender, Log, "Lightmass material compiler has encountered ReflectionAboutCustomNormalVector... Forcing constant (0.0f,0.0f,-1.0f).");
		return Compiler->Constant3(0.0f,0.0f,-1.0f);
	}
	...
	{
		//UE_LOGF(LogLightmassRender, Log, "Lightmass material compiler has encountered TransformVector... Passing thru source vector untouched.");
		return A;
	}
	...
	{
		//UE_LOGF(LogLightmassRender, Log, "Lightmass material compiler has encountered TransformPosition... Passing thru source vector untouched.");
		return A;
	}

	virtual int32 VertexColor() override
	{
		//UE_LOGF(LogLightmassRender, Log, "Lightmass material compiler has encountered VertexColor... Forcing constant (1.0f,1.0f,1.0f,1.0f).");
	...
	{
		//UE_LOGF(LogLightmassRender, Log, "Lightmass material compiler has encountered RealTime... Forcing constant 0.0f.");
		return Compiler->Constant(0.0f);
	}

	virtual int32 GameTime(bool bPeriodic, float Period) override
	{
		//UE_LOGF(LogLightmassRender, Log, "Lightmass material compiler has encountered GameTime... Forcing constant 0.0f.");
		return Compiler->Constant(0.0f);
	}
 ...
 */
class FLightmassMaterialProxy : public FEditorMaterialResourceBase, public FMaterialRenderProxy
{
public:
	FLightmassMaterialProxy(): FEditorMaterialResourceBase(), FMaterialRenderProxy(TEXT("FLightmassMaterialProxy"))
	{
	...
	void BeginCompiling(UMaterialInterface* InMaterialInterface, EMaterialProperty InPropertyToCompile, EMaterialShaderMapUsage::Type InUsage)
	{
		if (InMaterialInterface)
		{
			...
			ReferencedTextures = MaterialInterface->GetDefaultTextures();
	...
	bool WillGenerateUniformData(FFloat16Color& OutUniformValue)
	{
		...
		if (bSubstrateEnabled)
		{
			...
			case EMaterialShaderMapUsage::LightmassExportNormal:
				...
				break;
			default:
				UE_LOGF(LogLightmassRender, Error, "WillGenerateUniformData - cannot export a requested property for %ls", *(Material->GetPathName()));
				break;
			}
		}
		else
	...
	static bool WillFillData(EBlendMode InBlendMode, EMaterialProperty InMaterialProperty, bool bSubstrateEnabled)
	{
		...
		if (bSubstrateEnabled)
		{
			...
			default:
			{
				UE_LOGF(LogLightmassRender, Error, "FLightmassMaterialProxy::WillFillData - cannot export a requested property for");
			}
			}
		}
	...
	TArray<FFloat16Color>& OutMaterialNormal)
{
	...
	{
		UE_LOGF(LogLightmassRender, Warning, "LIGHTMASS: Material has an unsupported shading model: %d on %ls", 
	...
	if (MaterialExportEntry.DiffuseMaterialProxy)
	{
		...
		{
			UE_LOGF(LogLightmassRender, Warning, "Failed to generate diffuse material samples for %ls: %ls",
	...
	if (MaterialExportEntry.EmissiveMaterialProxy)
	{
		...
		{
			UE_LOGF(LogLightmassRender, Warning, "Failed to generate emissive material samples for %ls: %ls",
				*(InMaterial.GetLightingGuid().ToString()), *(InMaterial.GetPathName()));
			bResult = false;
	...
	if (MaterialExportEntry.OpacityMaterialProxy || bIsLandscapeMaterial)
	{
		...
		{
			UE_LOGF(LogLightmassRender, Warning, "Failed to generate transmissive material samples for %ls: %ls",
				*(InMaterial.GetLightingGuid().ToString()), *(InMaterial.GetPathName()));
			bResult = false;
	...
	if (MaterialExportEntry.NormalMaterialProxy)
	{
		...
		{
			UE_LOGF(LogLightmassRender, Warning, "Failed to generate normal material samples for %ls: %ls",
				*(InMaterial.GetLightingGuid().ToString()), *(InMaterial.GetPathName()));
			bResult = false;
	...
	TArray<FFloat16Color>& OutMaterialSamples)
{
	...
	else
	{
		...
		if (MaterialProxy->GetRenderTargetFormatAndSize(InMaterialProperty, Format, InMaterial.GetExportResolutionScale(), InOutSizeX, InOutSizeY))
		{
			...
			{
				UE_LOGF(LogLightmassRender, Warning, "Failed to create %4dx%4d render target!", InOutSizeX, InOutSizeY);
				bResult = false;
			}
			else
			{
				...
				{
					UE_LOGF(LogLightmassRender, Warning, "Failed to read Float16Pixels for 0x%08x property of %ls: %ls",
		...
		{
			UE_LOGF(LogLightmassRender, Warning, "Failed to get render target format and size for 0x%08x property of %ls: %ls",
				(uint32)InMaterialProperty, *(InMaterial.GetLightingGuid().ToString()), *(InMaterial.GetPathName()));
			bResult = false;
		}
	}
