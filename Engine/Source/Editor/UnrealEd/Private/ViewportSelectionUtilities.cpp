				...
				ElementSelectionSet->GetSelectionElement(InHitElement, SelectionMethod))
		{
			...
			const FTypedElementSelectionOptions SelectionOptions =
				FTypedElementSelectionOptions().SetAllowHidden(true).SetWarnIfLocked(true)
					.SetNameForTEDSIntegration(ElementSelectionSet->GetNameForTedsIntegration());
			...
			if (ElementSelectionSet->CanSelectElement(ResolvedElement, SelectionOptions))
			{
				...
				if (Click.IsControlDown() && bAllowSelectionModifiers)
				{
					if (ElementSelectionSet->IsElementSelected(
							ResolvedElement,
							FTypedElementIsSelectedOptions().SetAllowIndirect(true)
								.SetNameForTEDSIntegration(ElementSelectionSet->GetNameForTedsIntegration())
						))
					{
...
bool ClickActor(FEditorViewportClient* InViewportClient, AActor* InActor, const FViewportClick& Click, bool bAllowSelectionChange)
{
			 ...
			 && !InViewportClient->Viewport->KeyState(EKeys::LeftMouseButton))
	{
		...
		if (InActor)
		{
			...
			UE_LOGF(LogViewportSelectionUtilities, Log,  "Clicking on Actor (context menu): %ls (%ls)", *InActor->GetClass()->GetName(), *InActor->GetActorLabel() );
			 ...
			 && !Click.IsShiftDown())
	{
		if (InActor)
		{
			...
			UE_LOGF(LogViewportSelectionUtilities, Log,  "Clicking on Actor (double click): %ls (%ls)", *InActor->GetClass()->GetName(), *InActor->GetActorLabel());
		...
		else if (InActor)
		{
			if (bAllowSelectionChange && GEditor->CanSelectActor(InActor, true, true, true))
			{
				...
				if (Click.IsControlDown() && !bComponentSelected)
				{
					...
					if (bSelect)
					{
						UE_LOGF(LogViewportSelectionUtilities, Log,  "Clicking on Actor (CTRL LMB): %ls (%ls)", *InActor->GetClass()->GetName(), *InActor->GetActorLabel());
					}
				...
				else
				{
					...
					UE_LOGF(LogViewportSelectionUtilities, Log,  "Clicking on Actor (LMB): %ls (%ls)", *InActor->GetClass()->GetName(), *InActor->GetActorLabel());
...
bool ClickComponent(FEditorViewportClient* ViewportClient, HActor* ActorHitProxy, const FViewportClick& Click)
{
			 ...
			 && !ViewportClient->Viewport->KeyState(EKeys::LeftMouseButton))
	{
		...
		UE_LOGF(LogViewportSelectionUtilities, Log, "Clicking on Component (context menu): %ls (%ls)", *Component->GetClass()->GetName(), *Component->GetName());
	...
	else if (Click.GetKey() == EKeys::LeftMouseButton)
	{
		...
		if (Click.IsControlDown())
		{
			...
			if (bSelect)
			{
				UE_LOGF(LogViewportSelectionUtilities, Log, "Clicking on Component (CTRL LMB): %ls (%ls)", *Component->GetClass()->GetName(), *Component->GetName());
			}
		...
		else if (Click.IsShiftDown())
		{
			...
			{
				UE_LOGF(LogViewportSelectionUtilities, Log, "Clicking on Component (SHIFT LMB): %ls (%ls)", *Component->GetClass()->GetName(), *Component->GetName());
		...
		else
		{
			...
			UE_LOGF(LogViewportSelectionUtilities, Log, "Clicking on Component (LMB): %ls (%ls)", *Component->GetClass()->GetName(), *Component->GetName());
			GEditor->SelectComponent(Component, true, true, true);
			bComponentClicked = true;
		}
	}
...
void ClickSurface(FEditorViewportClient* ViewportClient, UModel* Model, int32 InSurf, const FViewportClick& Click)
{
	...
	else if (Click.IsAltDown() && Click.GetKey() == EKeys::LeftMouseButton)
	{
		...
		if (Click.IsControlDown())
		{
			...
			{
				UE_LOGF(LogViewportSelectionUtilities, Log, "WARNING: the texture coordinates were not parallel to the surface." );
			}
