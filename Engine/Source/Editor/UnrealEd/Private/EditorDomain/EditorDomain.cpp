...
public:
	...
	static IPackageResourceManager* SetPackageResourceManager()
	{
		...
		if (GIsEditor)
		{
			UE_LOGF(LogEditorDomain, Display, "EditorDomain is %ls",
...
FEditorDomain::FEditorDomain(EEditorDomainEnabled EnableLevel)
{
	...
	if (bEditorDomainWriteEnabled)
	{
		...
		{
			FCoreDelegates::GetOnPostEngineInit().AddRaw(this, &FEditorDomain::OnPostEngineInit);
		}
		else
		{
			OnPostEngineInit();
	...
	default:
		if (bEditorDomainReadEnabled)
		{
			UE_LOGF(LogEditorDomain, Warning,
				"Could not load package %ls from EditorDomain; it will be loaded from the WorkspaceDomain: %ls",
	...
	EPackageSegment PackageSegment, FPackagePath* OutUpdatedPath)
{
	...
	int64 FileSize = -1;
	{
			...
			(UE::DerivedData::FCacheGetResponse&& Response)
		{
			...
			if (!EditorDomain)
			{
				UE_LOGF(LogEditorDomain, Warning, "%ls size read after EditorDomain shutdown. Returning -1.",
					*PackagePath.GetDebugName());
				FileSize = -1;
			}
