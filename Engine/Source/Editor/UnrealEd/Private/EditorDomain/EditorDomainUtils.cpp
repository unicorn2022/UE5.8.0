...
static ValueType MapFindRef(const TMap<KeyType, ValueType, SetAllocator, KeyFuncs>& Map,
	typename TMap<KeyType, ValueType, SetAllocator, KeyFuncs>::KeyInitType Key, ValueType DefaultValue)
{
...
static void CoreRedirectClassPath(FTopLevelAssetPath& ClassPath, bool* bWasRedirected = nullptr)
{
	...
	if (ClassRedirect != RedirectedClassRedirect)
	{
		...
		{
			UE_LOGF(LogEditorDomain, Warning, "CoreRedirect for class %ls redirects to %ls which is not a TopLevelAssetPath. Ignoring it.",
...
// Enables/disables the parallel precalculation of class data digests (ease of use for debugging purposes).
constexpr bool bPrecalculateDataForClassDigests = true;
...
TArray<FGuid, TInlineAllocator<1>> GetCustomVersions(UClass& Class, FCustomVersionsStripedMap* InOutCache = nullptr);
...
{
	static FString PackageToDump;
	static bool bDumpPackageDigest = FParse::Value(FCommandLine::Get(), TEXT("-DumpPackageDigest="), PackageToDump);
	static FName PackageNameToDump(PackageToDump);

	bool bDumpCurrentPackage = PackageName == PackageNameToDump;

	TSharedPtr<FJsonObject> DigestDependencies = nullptr;
	if (bDumpCurrentPackage)
	{
		DigestDependencies = MakeShareable(new FJsonObject());
	}
	...
	if (bDumpCurrentPackage)
	{
		DigestDependencies->SetStringField(TEXT("PackageDigest.ProjectName"), FString(ProjectName));
	}

	Writer.Update(&EditorDomainVersion, sizeof(EditorDomainVersion));
	if (bDumpCurrentPackage)
	{
		DigestDependencies->SetStringField(TEXT("PackageDigest.EditorDomainVersion"), EditorDomainVersion.ToString());
	}
	...
	if (bDumpCurrentPackage)
	{
		DigestDependencies->SetStringField(TEXT("PackageDigest.EditorDomainSaveUnversioned"), FString::FromInt(EditorDomainSaveUnversioned));
	}

	Writer.Update(&PackageData.GetPackageSavedHash().GetBytes(), sizeof(PackageData.GetPackageSavedHash().GetBytes()));
	if (bDumpCurrentPackage)
	{
		DigestDependencies->SetStringField(TEXT("PackageDigest.PackageData"), LexToString(PackageData.GetPackageSavedHash()));
	}

	Writer.Update(&GPackageFileUEVersion, sizeof(GPackageFileUEVersion));
	if (bDumpCurrentPackage)
	{
		DigestDependencies->SetStringField(TEXT("PackageDigest.GPackageFileUEVersion"), FString::FromInt(GPackageFileUEVersion.ToValue()));
	}

	Writer.Update(&GPackageFileLicenseeUEVersion, sizeof(GPackageFileLicenseeUEVersion));
	if (bDumpCurrentPackage)
	{
		DigestDependencies->SetStringField(TEXT("PackageDigest.GPackageFileLicenseeUEVersion"), FString::FromInt(GPackageFileLicenseeUEVersion));
	}
	...
	if (bDumpCurrentPackage)
	{
		DigestDependencies->SetStringField(TEXT("PackageDigest.GGlobalConstructClassesHash"), LexToString(GGlobalConstructClassesHash));
	}
	...
	{
		{
			...
			while (NextClass < ImportedClassPaths.Num())
			{
				...
				if (ExistingData->ClosestNative != ClassPath)
				{
					if (bDumpCurrentPackage)
					{
						DigestDependencies->SetStringField(ClassPath.ToString(), TEXT("Non native class"));
					}
				...
				if (bDumpCurrentPackage)
				{
					DigestDependencies->SetStringField(NativeData->ClosestNative.ToString(), LexToString(NativeData->InclusiveSchemaHash));
				}
		...
		{
			// EDITORDOMAIN_TODO: Remove the clauses !IsInGameThread || UE::IsSavingPackage() once FindObject no longer asserts if UE::IsSavingPackage()
	...
	int32 CustomVersionCounter = 0;
	for (const FGuid& CustomVersionGuid : CustomVersionGuids)
	{
		Writer.Update(&CustomVersionGuid, sizeof(CustomVersionGuid));
		if (bDumpCurrentPackage)
		{
			DigestDependencies->SetStringField("PackageDigest.CustomVersionGuid_" + FString::FromInt(CustomVersionCounter), LexToString(CustomVersionGuid));
		}
		...
		if (bDumpCurrentPackage)
		{
			DigestDependencies->SetStringField("PackageDigest.CurrentVersion_" + FString::FromInt(CustomVersionCounter), FString::FromInt(CurrentVersion->Version));
			++CustomVersionCounter;
		}
	...
	if (bDumpCurrentPackage)
	{
		...
		FJsonSerializer::Serialize(DigestDependencies.ToSharedRef(), JsonWriter);

		UE_LOGF(LogEditorDomain, Display, "---Package digest dependencies---");
		UE_LOGF(LogEditorDomain, Display, "%ls", *(PackageName.ToString()));
		UE_LOGF(LogEditorDomain, Display, "%ls", *JsonStr);
		
		FString PackageDigestHash = LexToString(Result.Hash);
		UE_LOGF(LogEditorDomain, Display, "Result Hash %ls", *PackageDigestHash);
		UE_LOGF(LogEditorDomain, Display, "---");
	}
...
static const FBlake3Hash* GetClassDigestInternal(FTopLevelAssetPath ClassPath, FClassDigestMap& ClassDigests)
{
	...
	if (!ExistingData)
	{
		if (!IsInGameThread() || UE::IsSavingPackage())
		{
			return nullptr;
		}
...
{
public:
	FPrecacheClassDigest(FCustomVersionsStripedMap* CustomVersionsCache = nullptr)
		: ClassDigestsMap(GetClassDigests())
		, ClassDigests(ClassDigestsMap.Map)
		, AssetRegistry(IAssetRegistry::GetChecked())
		, CustomVersionsCache(CustomVersionsCache)
	{
...
private:
	...
	FCustomVersionsStripedMap* CustomVersionsCache;
...
{
	SCOPED_EDITORDOMAINTIMER(FPrecacheClassDigest::GetRecursive);
	...
	bool bPrintLogsDebugNullCook = bDebugNullCook && ClassPath.GetAssetName().ToString() == ClassName;

	if (bPrintLogsDebugNullCook)
	{
		UE_LOGF(LogEditorDomain, Display, "[DebugNullCook] Calculating class digest for class %ls", *ClassPath.ToString());
		UE_LOGF(LogEditorDomain, Display, "[DebugNullCook] bAllowRedirects=%d", bAllowRedirects);
#if UE_WITH_PACKAGE_ACCESS_TRACKING
		const PackageAccessTracking_Private::FTrackedData* TrackedData = PackageAccessTracking_Private::FPackageAccessRefScope::GetCurrentThreadAccumulatedData();
		if (TrackedData)
		{
			UE_LOGF(LogEditorDomain, Display, "[DebugNullCook] Package %ls", *TrackedData->PackageName.ToString());
		}
#endif
	...
	if (bAllowRedirects)
	{
		...
		if (bPrintLogsDebugNullCook)
		{
			UE_LOGF(LogEditorDomain, Display, "[DebugNullCook] Failed to find redirection for class %ls", *ClassPath.ToString());
		}
	...
	if (bIsNative)
	{
		...
		{
			UE_LOGF(LogEditorDomain, Display,
				"Class %ls is imported by a package but does not exist in memory. EditorDomain keys for packages using it will be invalid if it still exists."
	...
	if (StructAsClass)
	{
		...
		FKnownCustomVersions::FindOrAddHandles(DigestData->CustomVersionHandles, GetCustomVersions(*StructAsClass, CustomVersionsCache));
	...
	if (!ParentClassPath.IsNull())
	{
		...
		{
			UE_LOGF(LogEditorDomain, Display,
				"Parent class %ls of class %ls not found. Allow flags for editordomain and incremental cooking will be invalid.",
		...
		else
		{
			if (!ParentDigest->bConstructionComplete)
			{
				...
				{
					UE_LOGF(LogEditorDomain, Display,
						"Cycle detected in parents of class %ls. Allow flags for editordomain and incremental cooking will be invalid.",
						*ClassPath.ToString());
				}
			}
	...
	if (ConstructClasses.Num())
	{
		...
		for (int32 Index = 0; Index < ConstructClasses.Num(); ++Index)
		{
			...
			if (!ConstructClassDigest)
			{
				UE_LOGF(LogEditorDomain, Verbose,
					"Construct class %ls specified for class %ls is not found.",
		...
		if (bCycleDetected)
		{
			...
			UE_LOGF(LogEditorDomain, Error,
				"Cycle detected in ConstructClasses of class %ls. Packages with classes that can construct %ls will sometimes fail to update during incremental cooks.",
...
void PrecacheClassDigests(TConstArrayView<FTopLevelAssetPath> ClassPaths, FCustomVersionsStripedMap* CustomVersionsCache)
{
	SCOPED_EDITORDOMAINTIMER(EditorDomain::PrecacheClassDigests);
	FPrecacheClassDigest Digester(CustomVersionsCache);
...
void InitializeGlobalConstructClasses()
{
	...
	bGGlobalConstructClassesInitialized = true;
	{
		...
		for (const FString& Line : Lines)
		{
			...
			{
				UE_LOGF(LogEditorDomain, Error, "Invalid ClassPath %.*ls specified by Editor.ini:[EditorDomain]:GlobalCanConstructClasses.",
	...
	for (int32 Index = 0; Index < GGlobalConstructClasses.Num();)
	{
		...
		{
			UE_LOGF(LogEditorDomain, Display,
				"Construct class %ls specified by Editor.ini:[EditorDomain]:GlobalCanConstructClasses is not found. "
				"This is a class that can be constructed automatically by SavePackage when saving old packages. "
				"Old packages that do not yet have this class will load more slowly.",
				*ClassPath.ToString());
...
TArray<FGuid, TInlineAllocator<1>> GetCustomVersions(UClass& Class, FCustomVersionsStripedMap* InOutCache)
{
	SCOPED_EDITORDOMAINTIMER(EditorDomain::GetCustomVersions);
	TArray<FGuid, TInlineAllocator<1>> Guids;
	if (InOutCache && InOutCache->FindAndApply(&Class, [&Guids](const TArray<FGuid, TInlineAllocator<1>>& Cached) { Guids = Cached; }))
	{
		return Guids;
	}
...
TMap<FName, EDomainUse> ConstructPackageNameBlockedUses()
{
	...
	for (TArray<FString>* Array : { &BlockListArray, &LoadBlockListArray, &SaveBlockListArray })
	{
		...
		for (const FString& PackageNameOrFilename : *Array)
		{
			...
			{
				UE_LOGF(LogEditorDomain, Warning, "Editor.ini:[EditorDomain]:PackageBlocklist: Could not convert %ls to a LongPackageName: %ls",
			...
			{
				UE_LOGF(LogEditorDomain, Warning, "CoreRedirect for package %ls redirects to %ls which is not a package path. Ignoring it.",
...
{
	SCOPED_EDITORDOMAINTIMER(EditorDomain::ConstructTargetIterativeClassAllowList);
	...
	for (const FString& Line : ScriptPackageAllowListLines)
	{
		...
		{
			UE_LOGF(LogEditorDomain, Error,
				"Invalid value for Editor.ini:[CookSettings]:IncrementalClassScriptPackageAllowList:"
				"\n\t%ls"
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					// e.g. CoreOnline
					UE_LOGF(LogEditorDomain, Verbose,
						"Could not find module for script package %ls. Packages with classes from this script package will not be incrementally skippable.",
						*PackageName);
					continue;
				}
			}
	...
	FCustomVersionsStripedMap CustomVersionsCache;
	if (bPrecalculateDataForClassDigests)
	{
		TArray<UStruct*> NativeStructs;
		{
			SCOPED_EDITORDOMAINTIMER(CreateNativeCDOs);
			// Gather the native structs on which PrecacheClassDigests operates on through GetRecursive.
			for (const FTopLevelAssetPath& ClassPath : EnabledClassPathsArray)
			{
				if (FPackageName::IsScriptPackage(WriteToString<256>(ClassPath.GetPackageName())))
				{
					if (UStruct* Struct = FindObject<UStruct>(ClassPath))
					{
						NativeStructs.Add(Struct);
						if (UClass* AsClass = Cast<UClass>(Struct))
						{
							// Create uninitialized CDOs serially that could be triggered by GetCustomVersions.
							AsClass->GetDefaultObject();
						}
					}
				}
			}
		...
		{
			SCOPED_EDITORDOMAINTIMER(CacheSchemasAndCustomVersions);
			// Precalculate schema hashes and custom versions for use during PrecacheClassDigests.
			// Both GetSchemaHash and GetCustomVersions are memoized.
			ParallelFor(NativeStructs.Num(), [&NativeStructs, &CustomVersionsCache](int32 Index)
			{
				NativeStructs[Index]->GetSchemaHash(false);
				if (UClass* AsClass = Cast<UClass>(NativeStructs[Index]))
				{
					GetCustomVersions(*AsClass, &CustomVersionsCache);
				}
			});
		}
		PrecacheClassDigests(EnabledClassPathsArray, &CustomVersionsCache);
	}
	else
	{
...
TMultiMap<FTopLevelAssetPath, FTopLevelAssetPath> ConstructConstructClasses()
{
	...
	for (const FString& Line : Lines)
	{
		...
		if (NumTokens != 2)
		{
			UE_LOGF(LogEditorDomain, Warning, "Invalid value %ls in config setting Editor.ini:[EditorDomain]:PostLoadCanConstructClasses", *Line);
			continue;
		}
...
{
	SCOPED_EDITORDOMAINTIMER(EditorDomain::UtilsCookInitialize);
...
bool TrySavePackage(UPackage* Package)
{
	...
	{
		UE_LOGF(LogEditorDomain, Warning, "Could not save package to EditorDomain: %ls. Reason: %ls",
	...
	{
		UE_LOGF(LogEditorDomain, Verbose, "Skipping save of blocked package to EditorDomain: %ls.",
	...
	{
		UE_LOGF(LogEditorDomain, Verbose, "Skipping save of package with new BulkData to EditorDomain: %ls.",
			*WriteToString<256>(PackageName));
		return false;
	}
	...
	if (!PackageData)
	{
		UE_LOGF(LogEditorDomain, Verbose, "Could not save %ls to EditorDomain: It does not have An AssetPackageData in the AssetRegistry.",
			*WriteToString<256>(PackageName));
		return false;
	}
	COOK_STAT(auto Timer = UE::EditorDomain::CookStats::Usage.TimeSyncWork());

	UE_LOGF(LogEditorDomain, Verbose, "Saving to EditorDomain: %ls.", *WriteToString<256>(PackageName));
	...
	{
		UE_LOGF(LogEditorDomain, Warning, "Could not save %ls to EditorDomain: SavePackage returned %d.",
	...
	{
		UE_LOGF(LogEditorDomain, Warning, "Could not save %ls to EditorDomain: Could not read the PackageFileSummary from the saved bytes.",
			*Package->GetName());
		return false;
	}
	...
	for (FName ClassPathName : SavedImportedClassPathNames)
	{
		...
		if (!ClassPath.IsValid())
		{
			UE_LOGF(LogEditorDomain, Warning, "Could not save %ls to EditorDomain: It uses an importclass %ls that is not a TopLevelAssetPath.",
	...
	while (NextClass < SavedImportedClassPaths.Num())
	{
		...
		if (NextClass < SavedImportedClassPaths.Num())
		{
			...
			{
				UE_LOGF(LogEditorDomain, Warning, "Could not save %ls to EditorDomain: It constructed an instance of class %ls which which does not exist.",
					*Package->GetName(), *SavedImportedClassPaths[NextClass].ToString());
				return false;
			}
			// EDITORDOMAIN_TODO: Remove this !IsInGameThread check once FindObject no longer asserts if UE::IsSavingPackage()
			if (!IsInGameThread())
			{
				UE_LOGF(LogEditorDomain, Warning,
					"Could not save %ls to EditorDomain: It constructed an instance of a class %ls which is not yet saved in the EditorDomain, "
					"and we are not on the gamethread so we cannot add it.",
					*Package->GetName(), *SavedImportedClassPaths[NextClass].ToString());
				return false;
			}
			TConstArrayView<FTopLevelAssetPath> RemainingClasses =
	...
	for (const FTopLevelAssetPath& ImportedClass : SavedImportedClassPaths)
	{
		if (!KnownImportedClasses.Contains(ImportedClass))
		{
			...
			UE_LOGF(LogEditorDomain, Display, "Could not save %ls to EditorDomain: It uses an unexpected class. "
				"Optimized loading and incremental cooking will be disabled for this package.\n\t"
				"Find the class that added %ls and add it to its DeclareConstructClasses function.",
	...
	if (StorageResult == ESaveStorageResult::Valid)
	{
		...
		if (!UnknownGuids.IsEmpty())
		{
			...
			UE_LOGF(LogEditorDomain, Display, "Could not save %ls to EditorDomain: It uses an unexpected custom version. "
				"Optimized loading and incremental cooking will be disabled for this package. "
				"Modify the classes/structs to call Ar.UsingCustomVersion(Guid) in Serialize or DeclareCustomVersions.%ls",
		...
		GMaxBulkDataSize >= 0 && PackageWriter->GetBulkDataSize() > static_cast<uint64>(GMaxBulkDataSize))
	{
		...
		{
			UE_LOGF(LogEditorDomain, Display, "%ls", *Message);
		}
		else
		{
			UE_LOGF(LogEditorDomain, Verbose, "%ls", *Message);
		}
	...
	if (bStorageResultValid)
	{
		...
		UE::Cook::Artifact::FAppendPackageMetaDataContext Context;
		CookAttachments.AppendCommitAttachments(Context);
		if (Context.HasEntries())
		{
			UE::TargetDomain::CommitEditorDomainCookAttachments(Package->GetFName(), Context);
		}
	}
	return true;
}
	...
	FEditorDomainPackageWriter* PackageWriter, UE::DerivedData::FCacheGetResponse&& GetResponse)
{
	...
	{
		UE_LOGF(LogEditorDomain, Warning, "Indeterminism detected in EditorDomain save of %ls.", *PackageName.ToString());
	};

	if (bPreviousStorageResultValid != bStorageResultValid)
	{
		LogHeaderWarning();
		UE_LOGF(LogEditorDomain, Display, "\tStorageResultValid not equal. Previous: %ls, Current: %ls",
	...
	for (int32 SegmentIndex = 0; SegmentIndex < FMath::Max(PreviousValues.Num(), CurrentValues.Num()); ++SegmentIndex)
	{
		...
		{
			LogHeaderWarning();
			UE_LOGF(LogEditorDomain, Display, "\tNumber of segments differ. Previous: %d, Current: %d",
...
static void DumpClassDigests()
{
	...
	{
		UE_LOGF(LogEditorDomain, Display, "ClassDigest: %ls=%ls",
...
void FConstructionDuringUpgradeTracker::OnObjectConstructed(UObject* Constructed)
{
	...
	if (LogCore.GetVerbosity() < ELogVerbosity::Verbose)
	{
		// We only log if LogCore is set to >= Verbose, so skip everything otherwise
		return;
	}
	...
	UE_LOGF(LogCore, Verbose, "ConstructedDuringPostLoad: %ls constructed %ls."
		"\n\tThe ability to construct other objects during PostLoad needs to be declared to support incremental cooks."
		"\n\tCreate the static override function %ls%ls::DeclareConstructClasses and add %ls to OutConstructClasses.",
