...
bool FEditorDomainSaveServer::TryInitialize()
{
	...
	if (!ProcessLock)
	{
		UE_LOGF(LogEditorDomainSave, Error, "Could not create InterprocessSynchObject %ls, "
			"EditorDomain saving is not enabled.", *GetLockName());
		return false;
	}
	...
	{
		UE_LOGF(LogEditorDomainSave, Error, "Could not lock InterprocessSynchObject %ls for %lf seconds, "
			"aborting creation of EditorDomainSaveServer.",
	...
	if (!SocketSubsystem)
	{
		UE_LOGF(LogEditorDomainSave, Error, "No SocketSubsystem, aborting creation of EditorDomainServer.");
		return false;
	}
	...
	if (!ListenSocket)
	{
		UE_LOGF(LogEditorDomainSave, Error, "CreateSocket failed, aborting creation of EditorDomainServer.");
		return false;
	}
	...
	{
		UE_LOGF(LogEditorDomainSave, Error,
			"Failed to bind listen socket %ls, aborting creation of EditorDomainServer.",
			*ListenAddr->ToString(true));
		return false;
	}
	if (!ListenSocket->Listen(16))
	{
		UE_LOGF(LogEditorDomainSave, Warning,
			"Failed to listen to socket %ls, aborting creation of EditorDomainServer.",
			*ListenAddr->ToString(true));
		return false;
	}
	ListenPort = ListenSocket->GetPortNo();
	ListenAddr->SetPort(ListenPort);
	UE_LOGF(LogEditorDomainSave, Display, "EditorDomainSaveServer is listening for client connections on %ls.",
		*ListenAddr->ToString(true));
	...
	{
		UE_LOGF(LogEditorDomainSave, Warning,
			"Failed to write ServerSettings file %ls, aborting creation of EditorDomainServer.",
	...
	int32* OutListenPort)
{
	...
	else
	{
		...
		if (!RootObject)
		{
			UE_LOGF(LogEditorDomainSave, Error, "Could not read settings file %ls, %ls.",
				*GetServerSettingsFilename(), ErrorActionDesc);
			return false;
		}
		if (!RootObject->TryGetNumberField(SettingNames::ProcessIdFieldName, ServerProcessId))
		{
			UE_LOGF(LogEditorDomainSave, Error, "Could not read %ls out of settings file %ls, %ls.",
	...
	{
		UE_LOGF(LogEditorDomainSave, Error, "EditorDomainSave:Server Another process %d is registered as "
			"the singleton EditorDomainSaveServer in settings file %ls, this process %d is %ls.",
	...
	{
		UE_LOGF(LogEditorDomainSave, Error,
			"Listen port has changed from %d to %d. Change in listen port is unsupported; %ls.",
...
bool FEditorDomainSaveServer::TryAbdicate()
{
	...
	FProcessSemaphoreScopeLock ProcessScopeLock(ProcessLock, static_cast<float>(Constants::ProcessLockTimeoutSecondsWhenPolling));
	if (!ProcessScopeLock.IsLocked())
	{
		return false;
	}
	...
	UE_LOGF(LogEditorDomainSave, Display,
		"EditorDomainSaveServer has no remaining connections and is shutting down.");
	bAbdicated = true;
	ShutdownSocket();
	...
	{
		UE_LOGF(LogEditorDomainSave, Warning,
			"Unable to delete server settings file %ls. Future clients will have to delete the file.",
			*GetServerSettingsFilename());
	}
	return true;
}
...
void FEditorDomainSaveServer::PollIncomingConnections(bool& bInOutIsIdle)
{
	...
	while (ListenSocket->WaitForPendingConnection(bReadReady, FTimespan::FromSeconds(0)) && bReadReady)
	{
		...
		{
			UE_LOGF(LogEditorDomainSave, Warning,
				"Listen socket received a pending connection event but ListenSocket->Accept " "failed to create a ClientSocket.");
		}
		else
		{
	...
	UE::EditorDomainSave::EMessageType MessageType, const TArray<uint8>& MessageBuffer, bool& bOutStillAlive)
{
	...
	case EMessageType::Save:
	{
		...
		{
			UE_LOGF(LogEditorDomainSave, Warning, "EditorDomainServer received corrupt save message with invalid "
				"NumPackages %d from client, closing the connection", NumPackages);
		...
		else
		{
			for (int n = 0; n < NumPackages; ++n)
			{
				...
				if (Reader.IsError())
				{
					UE_LOGF(LogEditorDomainSave, Warning, "EditorDomainServer received corrupt save message with "
						"invalid PackageName instance %d from client, closing the connection", n);
					bOutStillAlive = false;
					break;
				}
	...
	case EMessageType::CloseConnection:
		UE_LOGF(LogEditorDomainSave, Display,
			"EditorDomainServer received CloseConnection message from client, closing the connection");
		bOutStillAlive = false;
		break;
	default:
		UE_LOGF(LogEditorDomainSave, Warning,
			"EditorDomainServer received invalid messagetype %u from client, closing the connection",
...
void FEditorDomainSaveServer::TickPendingPackages(bool& bInOutIsIdle)
{
	...
	while (PendingPackageNames.Num() > 0)
	{
		...
		{
			UE_LOGF(LogEditorDomainSave, Warning, "%ls", *ErrorMessage);
		}
		else
		{
			UE_LOGF(LogEditorDomainSave, Display, "Saved package %ls into the EditorDomain", *PackageName);
		}
	}
}
	...
	else
	{
		...
		if (BytesRead > 0)
		{
			...
			if (BytesRead == sizeof(FMessageHeader))
			{
				...
				{
					UE_LOGF(LogEditorDomainSave, Warning,
						"EditorDomainServer received invalid message header %ls from client, closing the connection.",
...
bool FEditorDomainSaveClient::TryInitializeCommunication()
{
	...
	{
		UE_LOGF(LogEditorDomainSave, Error,
			"Platform does not support network sockets, EditorDomain saving is not enabled.");
		Shutdown();
		bEnabled = false;
		return false;
	}
	...
	{
		UE_LOGF(LogEditorDomainSave, Error,
			"Could not create InterprocessSynchObject %ls, EditorDomain saving is not enabled.", *LockName);
		Shutdown();
		bEnabled = false;
		return false;
	}
...
void FEditorDomainSaveClient::RunCommunication()
{
	...
	for (;;)
	{
		...
		if (IsStopped)
		{
			...
			{
				UE_LOGF(LogEditorDomainSave, Warning,
					"Could not flush all results before shutting down. Failing to save %d remaining requests.",
					Requests.Num());
				FScopeLock ScopeLock(&Lock);
				bAsyncActive = false;
				break;
		...
		FPlatformProcess::Sleep(static_cast<float>(Constants::RunCommunicationSleepPeriodSeconds));
	}
}
...
bool FEditorDomainSaveClient::TryConnectProcess()
{
	...
	FProcessSemaphoreScopeLock ScopeLock(ProcessLock, static_cast<float>(Constants::ProcessLockTimeoutSecondsWhenPolling));
...
bool FEditorDomainSaveClient::TryConnectSocket()
{
	...
	auto PollServerSocketNotReady = [this]()
	{
		...
		{
			UE_LOGF(LogEditorDomainSave, Verbose,
				"EditorDomainSaveClient: Could not connect to %ls which should be opened by process %d",
	...
	UE_LOGF(LogEditorDomainSave, Display,
		"EditorDomainSaveClient: Connected to EditorDomainSaveServer with pid %d at %ls",
...
void FEditorDomainSaveClient::ConnectionWarning(FStringView Message, FStringView Consequence, bool bIgnoreCooldown)
{
	...
	if (bIgnoreCooldown || CurrentTime - LastConnectionWarningTimeSeconds >= Constants::ConnectionWarningCooldownSeconds)
	{
		...
		UE_LOGF(LogEditorDomainSave, Warning, "%.*ls. %.*ls.", Message.Len(), Message.GetData(), Consequence.Len(),
			Consequence.GetData());
	}
}
