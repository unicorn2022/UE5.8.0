...
bool FEditorDomainPackageSegments::Serialize(void* V, int64 Length, bool bIsError)
{
	...
	while (Pos < End)
	{
		...
		else
		{
			...
			{
				UE_CLOGF(!bIsError, LogEditorDomain, Error, "Package %ls is missing cache data in segment %d.",
	...
	TUniqueFunction<bool(FEditorDomain& EditorDomain)>&& TryCreateFallbackData)
{
	...
	if (bStorageValid)
	{
		...
		if (CompositeSize != FileSize)
		{
			...
			UE_LOGF(LogEditorDomain, Warning,
				"Package %ls received an invalid CacheRecord from DDC: %ls. Reading from workspace domain instead.",
				*PackagePath.GetDebugName(),
		...
		else
		{
			...
			else
			{
				...
				{
					UE_LOGF(LogEditorDomain, Warning, "%ls read after EditorDomain shutdown. Archive Set to Error.",
						*PackagePath.GetDebugName());
					NewAsyncSource = ESource::Segments;
	...
	UE_LOGF(LogEditorDomain, Verbose, "Loading from %ls: %ls.",
	...
	if (NewAsyncSource == ESource::Uninitialized)
	{
		...
		if (EditorDomainLocks->Owner)
		{
			...
			{
				UE_LOGF(LogEditorDomain, Error, "%ls was previously loaded from the EditorDomain but now is unavailable. This may cause failures during serialization due to changed FileSize and Format.",
					*PackagePath.GetDebugName());
			}
...
void FEditorDomainPackageSegments::SendSegmentRequest(FSegment& Segment)
{
	...
	Cache.GetChunks({SegmentChunk}, *Segment.RequestOwner,
		[this, &Segment](FCacheGetChunkResponse&& Response)
		{
			...
			if (Response.Status != EStatus::Ok)
			{
				UE_LOGF(LogEditorDomain, Error, "Package %ls is missing cache data in segment %d.", *PackagePath.GetDebugName(), (int32)(&Segment - &Segments[0]));
			}
			else
			{
				...
				{
					UE_LOGF(LogEditorDomain, Error, "Package %ls has corrupted cache data in segment %d.", *PackagePath.GetDebugName(), (int32)(&Segment - &Segments[0]));
					Segment.Data.Reset();
				}
...
bool FEditorDomainReadArchive::TryCreateFallbackData(FEditorDomain& EditorDomain)
{
	...
	{
		UE_LOGF(LogEditorDomain, Warning, "%ls could not be read from WorkspaceDomain. Archive Set to Error.",
...
void FEditorDomainReadArchive::Serialize(void* V, int64 Length)
{
	...
	case FEditorDomainPackageSegments::ESource::Fallback:
		...
		break;
	case FEditorDomainPackageSegments::ESource::Closed:
		UE_LOGF(LogEditorDomain, Error, "Requested read after close (file=%ls)", *Segments.GetPackagePath().GetDebugName());
		break;
	default:
		checkNoEntry();
		break;
