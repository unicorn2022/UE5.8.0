	...
	FOnBuildInputMetaResolved&& OnResolved)
{
	...
	Definition.IterateInputBuilds([&OtherStatus, &Definition](FUtf8StringView Key, const FBuildValueKey& ValueKey)
		{
			UE_LOGF(LogCore, Error, "FEditorBuildInputResolver: Resolving input builds is not yet implemented. "
				"Failed to resolve input build '%ls' for build of '%ls' by %ls.",
	...
	Definition.IterateInputFiles([&OtherStatus, &Definition](FUtf8StringView Key, FUtf8StringView Path)
		{
			UE_LOGF(LogCore, Error, "FEditorBuildInputResolver: Resolving input files is not yet implemented. "
				"Failed to resolve input file '%ls' for build of '%ls' by %ls.",
				*WriteToString<32>(Key), *Definition.GetName(), *WriteToString<32>(Definition.GetFunction()));
			OtherStatus = EStatus::Error;
		});
	...
	Definition.IterateInputHashes([&OtherStatus, &Definition](FUtf8StringView Key, const FIoHash& RawHash)
		{
			UE_LOGF(LogCore, Error, "FEditorBuildInputResolver: resolving input hashes is not yet implemented. "
				"Failed to resolve input hash '%ls' for build of '%ls' by %ls.",
				*WriteToString<32>(Key), *Definition.GetName(), *WriteToString<32>(Definition.GetFunction()));
			OtherStatus = EStatus::Error;
		});
...
void FEditorBuildInputResolver::ResolveInputData(const FBuildAction& Action, IRequestOwner& Owner,
	FOnBuildInputDataResolved&& OnResolved, FBuildInputFilter&& Filter)
{
	UE_LOGF(LogCore, Error, "FEditorBuildInputResolver does not implement ResolveInputData from FBuildAction. "
		"Failed to resolve input data for build of '%ls' by %ls.",
...
{
	UE_LOGF(LogCore, Error, "Failed to resolve metadata for bulk data input '%ls' for build of '%ls' by %ls.",
		*WriteToString<32>(Key), *Definition.GetName(), *WriteToString<32>(Definition.GetFunction()));
}
...
{
	UE_LOGF(LogCore, Error, "Failed to resolve data for bulk data input '%ls' for build of '%ls' by %ls.",
		*WriteToString<32>(Key), *Definition.GetName(), *WriteToString<32>(Definition.GetFunction()));
}
