...
void FBulkDataRegistryImpl::Teardown()
{
	...
	if (SharedDataLock->UpdatePayloadWorkerInFlight > 0)
	{
		...
		UE_LOGF(LogBulkDataRegistry, Log, "Waiting on %d FUpdatePayloadWorker jobs to complete...", SharedDataLock->UpdatePayloadWorkerInFlight.load());
		...
		UE_LOGF(LogBulkDataRegistry, Log, "All FUpdatePayloadWorker jobs finished, exit was stalled for %.1f(s)", FPlatformTime::Seconds() - StartTime);
	}
}
...
void FBulkDataRegistryImpl::UpdateRegistrationData(UPackage* Owner, const UE::Serialization::FEditorBulkData& BulkData)
{
	if (!BulkData.GetIdentifier().IsValid())
	{
		UE_LOGF(LogBulkDataRegistry, Warning, "UpdateRegistrationData called with invalid BulkData for Owner %ls.",
...
void FBulkDataRegistryImpl::OnExitMemory(const UE::Serialization::FEditorBulkData& BulkData)
{
	...
	if (Existing)
	{
		// Keep the FRegisteredBulk around to provide access to the BulkData in case it is requested without
		// loading the package that contains it.
		// But remove our (referenced tracked) pointer to its Payload. We do not want to force its payload
		// to remain in memory when the package is unloaded in the editor.
		// Note that we might have loaded the payload for our own purposes and set bHasTempPayload = true,
		// but that is just an optimization, and one that is not likely to occur for a BulkData in a package that
		// was loaded; bHasTempPayload is meant to provide access to the payload when the package was not loaded.
		// So clear that field as well.
		Existing->bHasTempPayload = false;
		Existing->bInMemory = false;

		// If the BulkData only existed in memory, then it is effectively gone; we clear its BulkData fields
		// and mark that we no longer have access to its payload. We keep it registered solely to keep track
		// of its package owner so we can detect duplicate Guid bugs.
		// We also do this for BulkDatas that used to have on-disk payloads, but that have been detached from their
		// linker (due e.g. to the package being unloaded or renamed) and are no longer loadable from disk.
		// For those packages, UnloadData fails to unload the Payload because it is the only accessible version
		// of the payload and UnloadData has a contract that it does not unload the data if not recoverable.
		// For our internal bulkdatas we don't need that contract; we can just let the payload become inaccessible
		// for these packages that detached their bulkdatas from disk in response to user operations.
		bool bRequiresClear = Existing->BulkData.IsMemoryOnlyPayload();
		if (!bRequiresClear)
		{
			Existing->BulkData.UnloadData();
			bRequiresClear = Existing->BulkData.IsPayloadLoaded();
		}
		if (bRequiresClear)
		{
...
void FBulkDataRegistryImpl::UpdatePlaceholderPayloadId(const UE::Serialization::FEditorBulkData& BulkData)
{
	if (!BulkData.GetIdentifier().IsValid())
	{
		UE_LOGF(LogBulkDataRegistry, Warning, "UpdatePlaceholderPayloadId called with invalid BulkData.");
		return;
	}
	if (BulkData.HasPlaceholderPayloadId())
	{
		UE_LOGF(LogBulkDataRegistry, Warning, "UpdatePlaceholderPayloadId called with a BulkData that still has a PlaceholderPayloadId.");
		return;
	}
	const FGuid& Key = BulkData.GetIdentifier();
	...
	for (TRefCountPtr<FPendingPayloadId>& NewPending : NewPendings)
	{
			...
			[this, PackageName, NewPending](FSharedBuffer Buffer)
		{
			...
			if (!Buffer.IsNull())
			{
				...
				{
					UE_LOGF(LogBulkDataRegistry, Warning, "Corrupt cache data for BulkDataPayloadId %ls.", WriteToString<192>(PackageName, TEXT("/"), BulkDataId).ToString());
