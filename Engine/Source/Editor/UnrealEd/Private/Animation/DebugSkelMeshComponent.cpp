...
void UDebugSkelMeshComponent::ResetRootTransform()
{
	...
	// Discard any runtime-extracted root motion still pending on the anim instances. RootMotionTransform
	// already represents the analytical position for the current playback time, so a pending delta would
	// be added on top of it by the next AddLocalTransform in ConsumeRootMotion, double-counting the span.
	ConsumeRootMotion_Internal(1.0f);
}
...
void UDebugSkelMeshComponent::EnablePreview(bool bEnable, UAnimationAsset* PreviewAsset)
{
	if (PreviewInstance)
	{
		if (bEnable)
		{
			...
			// When this code path gets hit during undo/redo calls and we are previewing the same asset as before 
			// we want to restore playback position and play state after SetAnimationAsset to avoid transacting actions reseting the state of anim editors
			const bool bRestoreState = GIsTransacting && IsPreviewOn() && PreviewInstance->GetCurrentAsset() == PreviewAsset;
			const bool bWasPlaying = PreviewInstance->IsPlaying();
			const float PrevTime = PreviewInstance->GetCurrentTime();
			...
			if (bRestoreState)
			{
				PreviewInstance->SetPosition(PrevTime, false);
				PreviewInstance->SetPlaying(bWasPlaying);
			}
			...
			// clear trajectory visualization when the new asset has no root motion
			if (!DoesCurrentAssetHaveRootMotion())
			{
				VisualizeRootMotionMode = EVisualizeRootMotionMode::None;
			}
		}
		else if (IsPreviewOn())
		{
...
void UDebugSkelMeshComponent::GenSpaceBases(TArray<FTransform>& OutSpaceBases)
{
	...
	DoInstanceFinalizeAnimation(true);
}
