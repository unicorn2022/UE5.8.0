...
namespace ObjectTools
{
	...
	void GatherObjectReferencersForDeletion(UObject* InObject, bool& bOutIsReferenced, bool& bOutIsReferencedInMemoryByUndo, FReferencerInformationList* OutMemoryReferences, bool bInRequireReferencingProperties)
	{
		...
		if (!CVarUseLegacyGetReferencersForDeletion.GetValueOnAnyThread())
		{
			...
			if (bOutIsReferenced && bInRequireReferencingProperties && OutMemoryReferences)
			{
				...
				if (!bOutIsReferenced)
				{
					UE_LOGF(LogObjectTools, Warning, "Detected inconsistencies between reference gathering algorithms. Switching 'Editor.UseLegacyGetReferencersForDeletion' on for the remainder of this editor session.");
		...
		if (InObjects.Num() > 0)
		{
			...
			for (UObject* ObjectToExclude : ObjectsToExclude)
			{
				...
				if (UBlueprint* BlueprintObject = Cast<UBlueprint>(ObjectToExclude))
				{
					if (BlueprintObject->GeneratedClass)
					{
						...
						GetObjectsWithOuter(BlueprintObject->GeneratedClass, ClassSubObjects, EGetObjectsFlags::None);
			...
			for (UObject* InObject : InObjects)
			{
				TArray<UObject*> AdditionalObjects;
				{
					...
					GetObjectsWithOuter(InObject, SubObjects, EGetObjectsFlags::None);
	...
	UObject* DuplicateSingleObject(UObject* Object, const FPackageGroupName& PGN, TSet<UPackage*>& InOutPackagesUserRefusedToFullyLoad, bool bPromptToOverwrite, TMap<TSoftObjectPtr<UObject>, TSoftObjectPtr<UObject>>* DuplicatedObjects /*= nullptr*/)
	{
		...
		if (ObjectsToDelete.Num() > 0)
		{
			...
			{
				UE_LOGF(LogObjectTools, Warning,
					"Existing objects could not be deleted, unable to duplicate %ls",
					*Object->GetFullName());
				return NULL;
			}
		}
	...
	FConsolidationResults ConsolidateObjects(TArrayView<FReplaceRequest> Requests, TSet<UObject*>& ObjectsToConsolidateWithin, TSet<UObject*>& ObjectsToNotConsolidateWithin, bool bShouldDeleteAfterConsolidate, bool bWarnAboutRootSet)
	{
		...
		if (bShouldDeleteAfterConsolidate)
		{
			...
			for (TMap<UObjectRedirector*, FName>::TIterator RedirectIt(RedirectorToObjectNameMap); RedirectIt; ++RedirectIt)
			{
				...
				{
					Redirector->Rename(*ObjName.ToString(), NULL, REN_DontCreateRedirectors | REN_NonTransactional | REN_AllowPackageLinkerMismatch);
		...
		if ( UnconsolidatableObjects.Num() > 0 )
		{
			...
			{
				UE_LOGF(LogObjectTools, Warning, "Failed to consolidate assets: %ls", *Message.ToString());
			}
		}
		...
		if( SelectedObjects.Num() > 0 )
		{
			...
			if ( Object )
			{
				...
				if (IsReferenced(Object, RF_Public, EInternalObjectFlags::Native, true, &Refs))
				{
					...
					UE_LOGF(LogObjectTools, Warning, "%ls", *Ar);  // also print the objects to the log so you can actually utilize the data
		...
		if( Object )
		{
			...
			TSet<UObject*> ReferencedObjects;
			{
				...
				for( int32 PackageNameItr = 0; PackageNameItr < IgnorePackageNames.Num(); ++PackageNameItr )
				{
					...
					{// An invalid package name was provided.
						UE_LOGF(LogObjectTools, Log,  "Package to ignore \"%ls\" in the list of referenced objects is NULL and should be removed from the list", *(IgnorePackageNames[PackageNameItr]) );
					}
					else
					{
			...
			if ( NumReferencedObjects > 1 || ( NumReferencedObjects == 1 && !ReferencedObjects.Contains( Object ) ) )
			{
				if (CollectionName == NAME_None)
				{
					...
					UE_LOGF(LogObjectTools, Warning, "%ls", *OutString );
	...
	void CleanupAfterSuccessfulDelete (const TArray<UPackage*>& PotentialPackagesToDelete, bool bPerformReferenceCheck)
	{
		...
		for ( int32 PackageFileIdx = 0; PackageFileIdx < PackageFilesToDelete.Num(); ++PackageFileIdx )
		{
			...
			if ( ISourceControlModule::Get().IsEnabled() )
			{
				...
				if ( bInDepot )
				{
					...
					{
						UE_LOGF(LogObjectTools, Warning, "SCC failed to open '%ls' for deletion.", *PackageFilename);
					}
				}
				else
				{
		...
		if (ISourceControlModule::Get().IsEnabled())
		{
			if (SCCFilesToRevert.Num() > 0)
			{
				...
				RevertOperation->SetSoftRevert(SourceControlProvider.UsesSoftRevertOnDelete());

				if (SourceControlProvider.Execute(RevertOperation, SCCFilesToRevert) == ECommandResult::Failed)
				{
					UE_LOG(LogObjectTools, Warning, TEXT("SCC failed to revert the selected files before deletion."));
				}
			}

			if (SCCFilesToDelete.Num() > 0)
			{
				TSharedRef<FDelete, ESPMode::ThreadSafe> DeleteOperation = ISourceControlOperation::Create<FDelete>();

				if (SourceControlProvider.Execute(DeleteOperation, SCCFilesToDelete) == ECommandResult::Failed)
				{
					UE_LOGF(LogObjectTools, Warning, "SCC failed to open the selected files for deletion.");
				}
			}
		}
	...
	bool AddExtraLocalizedVariantsToDelete(const TArray<FAssetData>& InAssetsToDelete, TArray<FAssetData>& OutAssetsWithVariantsToDelete)
	{
		...
		if (!VariantsBySourcesOnlyInRevisionControl.IsEmpty())
		{
			...
			UE_LOGF(LogObjectTools, Error, "A file that needs to be renamed was detected in Revision Control but not on your disk.");
		...
		else if (bRevisionControlWasNeeded)
		{
			...
			UE_LOGF(LogObjectTools, Error, "%ls", *LocalizedAssetTools->GetRevisionControlIsNotAvailableWarningText().ToString());
		...
		for (const FName& Package : PackagesAndVariants)
		{
			if (!PackageNames.Contains(Package))
			{
				...
				UE_LOGF(LogObjectTools, Display, "Added asset to extraction (while checking for localized variants): %ls", *PackageNameStr);
	...
	bool ContainsWorldInUse(const TArray< UObject* >& ObjectsToDelete)
	{
		...
		UE_LOGF(LogObjectTools, Log, "Deleting %d worlds: %ls", WorldsToDelete.Num(), *GetCombinedWorldNames(WorldsToDelete));
		...
		UE_LOGF(LogObjectTools, Log, "Currently %d active worlds: %ls", ActiveWorlds.Num(), *GetCombinedWorldNames(ActiveWorlds));
	...
	{
		FDeleteItemsParameters Options;
		Options.ObjectsToDelete = InObjectsToDelete;
		Options.bShowConfirmation = bShowConfirmation;
		Options.AllowCancelDuringDelete = AllowCancelDuringDelete;
		return DeleteItems(Options).ObjectsDeleted;
	}
	
	FDeleteItemsResult DeleteItems(const FDeleteItemsParameters& InOptions)
	{
		auto DeleteObjects = [&InOptions](TArray<UObject*> ObjectsToDelete, bool& bCancelled)
		{
			const FScopedBusyCursor BusyCursor;
			...
			FOnPreDestructiveAssetActionArgs PreDestructiveArgs(ObjectsToDelete, EDestructiveAssetActions::AssetDelete);
			FEditorDelegates::OnPreDestructiveAssetActionWithArgs.Broadcast(PreDestructiveArgs);

			if (!PreDestructiveArgs.Result.bSuccess)
			{
				UE_LOGF(LogObjectTools, Warning, "%ls", *PreDestructiveArgs.Result.ErrorMessage);
				return 0;
			}
			...
			if ( InOptions.bShowConfirmation )
			{
				const FVector2D WindowSize = FVector2D( 600, 700 );
				...
				TSharedRef< SDeleteAssetsDialog > DeleteDialog =
					...
					.FoldersToDelete(InOptions.FoldersToDelete)
					.OnDialogClosed(
						SDeleteAssetsDialog::FOnDialogClosed::CreateLambda(
						[&bCancelled](SDeleteAssetsDialog::EResult Result)
							{
								bCancelled = (Result == SDeleteAssetsDialog::EResult::Cancel);
							})
						)
			...
			const bool bAllowCancelDuringDelete = (InOptions.AllowCancelDuringDelete == EAllowCancelDuringDelete::AllowCancel);
			...
			if ( bUserCanceled )
			{
				UE_LOGF(LogUObjectGlobals, Warning, "User canceled delete operation");
				return 0;
			}

			if ( !DeleteModel->DoDelete() )
			{
				UE_LOGF(LogUObjectGlobals, Warning, "Could not delete");
		...
		FDeleteItemsResult Result;
		Result.ObjectsDeleted = DeleteObjects(InOptions.ObjectsToDelete, bCancelled);

		if (!bCancelled)
		{
			// Look for files on disk in case the folder contains things not tracked by the asset registry
			auto DeleteEmptyFolderFromDisk = [](const FString& InPathToDelete)->bool
				{
					FString PathToDeleteOnDisk;
					if (FPackageName::TryConvertLongPackageNameToFilename(InPathToDelete, PathToDeleteOnDisk))
					{
						TArray<FString> FileNames;
						IFileManager::Get().FindFilesRecursive(FileNames, *PathToDeleteOnDisk, TEXT(""), /** Files */true, /** Directory */false);

						if (FileNames.IsEmpty())
						{
							return IFileManager::Get().DeleteDirectory(*PathToDeleteOnDisk, /** RequireExists */false, /** Tree */true);
						}
					}
			...
			// Remove folders specified only if they are empty (no files)
			const FAssetRegistryModule& AssetRegistryModule = FModuleManager::LoadModuleChecked<FAssetRegistryModule>("AssetRegistry");
			for (const FString& PathToDelete : InOptions.FoldersToDelete)
			{
	...
	int32 PrivatizeObjects(const TArray<UObject*>& InObjectsToPrivatize, bool bShowConfirmation, EAllowCancelDuringPrivatize AllowCancelDuringPrivatize, const EAssetAccessSpecifier InAssetAccessSpecifier)
	{
		...
		FOnPreDestructiveAssetActionArgs PreDestructiveArgs(ObjectsToPrivatize, EDestructiveAssetActions::AssetPrivatize);
		FEditorDelegates::OnPreDestructiveAssetActionWithArgs.Broadcast(PreDestructiveArgs);
		...
		if (bUserCanceled)
		{
			UE_LOGF(LogUObjectGlobals, Warning, "User cancelled privatize operation");
			return 0;
		}

		if (!PrivatizeModel->DoPrivatize())
		{
			UE_LOGF(LogUObjectGlobals, Warning, "Could not mark private");
		}
	...
	int32 ForceDeleteObjects(const TArray< UObject* >& InObjectsToDelete, bool ShowConfirmation)
	{
...

		{
			...
			UE_LOGF(LogUObjectGlobals, Log, "%ls", *Msg);
		}
		...
		{
			{
				...
				// UserDefinedStructs (probably all ScriptStructs) should be replaced with the FallbackStruct - but they must be reinstanced properly
				// this is very similar to the "Reparent any direct children".. logic above, but for UScriptStruct instead of UClass.. Likely 
				// we also need similar logic for UEnum
				{
					...
					for (int32 Iter = 0; Iter < ObjectsToReplace.Num(); )
					{
						if (UUserDefinedStruct* UDStruct = Cast<UUserDefinedStruct>(ObjectsToReplace[Iter]))
						{
							...
							FBlueprintCompilationManager::ReinstanceStruct(UDStruct);
						}
						else
						{
							Iter++;
			...
			if (bNeedsGarbageCollection)
			{
				...
				// The GC above may have deleted some of the ObjectsToDelete (e.g. preview instances of blueprints) - 
				// remove them from ObjectsToDelete:
				ObjectsToDelete.SetNum(Algo::RemoveIf(ObjectsToDelete, [](const TWeakObjectPtr<UObject>& ObjectWeakPtr)
				{
					return ObjectWeakPtr.Get() == nullptr;
				}));
			}
			
			// Give systems opportunity to clean up references to the objects being deleted
			{
				TArray<UObject*> ObjectsToPreDelete;
				Algo::Transform(ObjectsToDelete, ObjectsToPreDelete, [](const TWeakObjectPtr<UObject>& ObjectWeakPtr) { return ObjectWeakPtr.Get(); });
				FEditorDelegates::OnAssetsPreDelete.Broadcast(ObjectsToPreDelete);
			}
			...
			for(auto It = ObjectsToDelete.CreateIterator(); It; ++It)
			{
				...
				{
					UE_LOGF(LogObjectTools, Warning, "ForceDeleteObject failed to delete %ls, this package is now potentially corrupt", *CurObject->GetName());
				...
				else
				{
					...
					if (bFoundCompatibleRedirector)
					{
						...
						if ( ensure(DestinationPackage) )
						{
							...
							{
								UE_LOGF(LogObjectTools, Warning, "RenameObjectsInternal failed to return a redirector '%ls' to its original location. This was because there was already an asset in the way. Deleting redirector.", *Redirector->GetName());
...
{
	FAsyncObjectThumbnail::FAsyncObjectThumbnail(int32 InWidth, int32 InHeight, TSharedRef<FRHIGPUTextureReadback> InTextureReadback)
	: ImageWidth(InWidth)
	, ImageHeight(InHeight)
	, TextureReadback(InTextureReadback)
	{}

	bool FAsyncObjectThumbnail::IsTextureReadbackReady() const { return TextureReadback->IsReady() && !ObjectThumbnailFuture.IsSet(); }
	bool FAsyncObjectThumbnail::IsObjectThumbnailReady() const { return ObjectThumbnailFuture.IsSet() && ObjectThumbnailFuture->IsReady(); }

	void FAsyncObjectThumbnail::EnqueueReadbackDataCopy()
	{
		check(TextureReadback->IsReady());
		{
			ObjectThumbnailPromise = MakeShared<TPromise<FObjectThumbnail>>();
			ObjectThumbnailFuture = ObjectThumbnailPromise->GetFuture();

			ENQUEUE_RENDER_COMMAND(CopyReadbackDataToThumbnailRTCommand)(
			[ObjectThumbnailPromise = this->ObjectThumbnailPromise, ImageWidth = this->ImageWidth, ImageHeight = this->ImageHeight, TextureReadback = this->TextureReadback]
			(FRHICommandListImmediate& RHICmdList)
			{
				...
				void* ReadbackBuffer = TextureReadback->Lock(ReadbackWidth, &ReadbackHeight);

				if (ReadbackBuffer)
				{
					TArray<uint8>& ImageData = ObjectThumbnail.AccessImageData();
					ImageData.Empty();
					ImageData.AddUninitialized(ObjectThumbnail.GetImageWidth() * ObjectThumbnail.GetImageHeight() * sizeof(FColor));

					check(ReadbackBuffer && ObjectThumbnail.GetImageWidth() <= ReadbackWidth && ObjectThumbnail.GetImageHeight() <= ReadbackHeight);

					if (ObjectThumbnail.GetImageWidth() == ReadbackWidth && ObjectThumbnail.GetImageHeight() == ReadbackHeight)
					{
						FMemory::Memcpy( (void*)ImageData.GetData(), ReadbackBuffer, ObjectThumbnail.GetImageWidth() * ObjectThumbnail.GetImageHeight() * sizeof(FColor) );
					}
					else // Copy one line at a time
					{
						int32 SrcPitchElem = ReadbackWidth;
						int32 DstPitchElem = ObjectThumbnail.GetImageWidth();

						const uint8* SrcColorData = reinterpret_cast<const uint8*>(ReadbackBuffer);

						for (int32 RowIndex = 0; RowIndex < ObjectThumbnail.GetImageHeight(); RowIndex++)
						{
							const void* SrcPtr = SrcColorData + RowIndex * SrcPitchElem * sizeof(FColor);
							void* DstPtr = ImageData.GetData() + RowIndex * DstPitchElem * sizeof(FColor);
							FMemory::Memcpy(DstPtr, SrcPtr, DstPitchElem * sizeof(FColor));
						}
					}
				}
				...
				ObjectThumbnailPromise->SetValue(MoveTemp(ObjectThumbnail));
			});
		}
	}

	void ReadbackThumbnail(FTextureRenderTargetResource* RenderTargetResource, FObjectThumbnail& OutThumbnail)
	{
	...
	FAsyncObjectThumbnail ReadbackThumbnailAsync(FTextureRenderTargetResource* RenderTargetResource, const int32 ImageWidth, const int32 ImageHeight)
	{
		TSharedRef<FRHIGPUTextureReadback> TextureReadback = MakeShared<FRHIGPUTextureReadback>("ThumbnailReadback");
		FAsyncObjectThumbnail AsyncObjectThumbnail(ImageWidth, ImageHeight, TextureReadback);

		ENQUEUE_RENDER_COMMAND(ReadbackThumbnailRTCommand)(
		[RenderTargetResource, ImageWidth, ImageHeight, TextureReadback]
		(FRHICommandListImmediate& RHICmdList) mutable
		{
			RHICmdList.Transition(FRHITransitionInfo(RenderTargetResource->GetRenderTargetTexture(), ERHIAccess::SRVMask, ERHIAccess::CopySrc));

			const FResolveRect SrcRect(0, 0, ImageWidth, ImageHeight);
			TextureReadback->EnqueueCopy(RHICmdList, RenderTargetResource->GetRenderTargetTexture(), SrcRect);
		...
		return AsyncObjectThumbnail;
	}

	bool RenderThumbnailInternal( UObject* InObject, uint32& InOutImageWidth, uint32& InOutImageHeight, EThumbnailTextureFlushMode::Type InFlushMode,
								  FTextureRenderTargetResource*& InOutTextureRenderTargetResource )
	{
		...
		const bool bIsRenderingToSpecificRenderTarget = (InOutTextureRenderTargetResource != nullptr);
		...
		FTextureRenderTargetResource* RenderTargetResource = InOutTextureRenderTargetResource;
		if ( RenderTargetResource == NULL )
		{
			// No render target was supplied, just use a scratch texture render target
			const uint32 MinRenderTargetSize = FMath::Max( InOutImageWidth, InOutImageHeight );
			...
			check( InOutImageWidth <= RenderTargetTexture->GetSurfaceWidth() );
			check( InOutImageHeight <= RenderTargetTexture->GetSurfaceHeight() );

			RenderTargetResource = RenderTargetTexture->GameThread_GetRenderTargetResource();
			InOutTextureRenderTargetResource = RenderTargetResource;
		}
		...
		if( RenderInfo != NULL && RenderInfo->Renderer != NULL )
		{
			...
			uint32 DrawWidth = InOutImageWidth;
			uint32 DrawHeight = InOutImageHeight;

			// If we're not rendering to a specific render target, then allow the thumbnail renderer to override the thumbnail size.
			// This happens when we render the thumbnail that will be saved in the asset package.
			if (!bIsRenderingToSpecificRenderTarget)
			{
				...
				if( DesiredWidth > 0 && DesiredHeight > 0 )
				{
					...
					if( DesiredWidth > InOutImageWidth )
					{
						DesiredHeight = ( DesiredHeight * InOutImageWidth ) / DesiredWidth;
						DesiredWidth = InOutImageWidth;
					}
					if( DesiredHeight > InOutImageHeight )
					{
						DesiredWidth = ( DesiredWidth * InOutImageHeight ) / DesiredHeight;
						DesiredHeight = InOutImageHeight;
					}
					...
					InOutImageWidth = DrawWidth;
					InOutImageHeight = DrawHeight;
				}
			}
	...
	{
		uint32 ImageWidth = InImageWidth;
		uint32 ImageHeight = InImageHeight;

		const bool bThumbnailRendered = RenderThumbnailInternal(InObject, ImageWidth, ImageHeight, InFlushMode, InTextureRenderTargetResource);

		if (OutThumbnail && bThumbnailRendered)
		{
			OutThumbnail->SetImageSize(ImageWidth, ImageHeight);
			ReadbackThumbnail(InTextureRenderTargetResource, *OutThumbnail);
		}
	}

	TOptional<FAsyncObjectThumbnail> RenderThumbnailAsync( UObject* InObject, const uint32 InImageWidth, const uint32 InImageHeight, EThumbnailTextureFlushMode::Type InFlushMode,
															   FTextureRenderTargetResource* InTextureRenderTargetResource )
	{
		...
		{
			return ReadbackThumbnailAsync(InTextureRenderTargetResource, ImageWidth, ImageHeight);
		}
		else
		{
			return {};
	...
	namespace Private
	{
		...
		FObjectThumbnail* FindCachedThumbnailInPackage(const FString& InPackageFileName, const FName InObjectShortClassFullName)
		{
			...
			if (FPackageName::TryConvertFilenameToLongPackageName(PackageName, PackageName))
			{
				...
				{
					UE_LOGF(LogUObjectGlobals, Warning, "Attempted to FindCachedThumbnailInPackage named 'None' - PackageName: %ls InPackageFileName: %ls", *PackageName, *InPackageFileName);
					return nullptr;
				}
			...
			*FileReader << ThumbnailCount;
			...
			for( int32 CurThumbnailIndex = 0; CurThumbnailIndex < ThumbnailCount; ++CurThumbnailIndex )
			{
				...
				*FileReader << FileOffset;
				...
				{
					UE_LOGF(LogObjectTools, Warning, "Loaded thumbnail '%ls' out of order!: FileOffset:%i    LastFileOffset:%i", *ObjectPath, FileOffset, LastFileOffset);
				}
