		...
		if( SelectedActors.Num() > 0 )
		{
			...
			if( FirstActor && !FirstActor->IsTemplate() )
			{
				...
				for ( int32 ActorIndex = 0; ActorIndex < SelectedActors.Num(); ++ActorIndex )
				{
					...
					if (!ActorInfo.bHaveBrowseOverride && BrowseToAssetOverrideSubsystem->HasBrowseToAssetOverride(CurrentActor))
					{
...
namespace AssetUtil
{
	...
	TArray<FAssetData> ExtractAssetDataFromDrag(const TSharedPtr<const FDragDropOperation>& Operation)
	{
		...
		else if (Operation->IsOfType<FAssetDragDropOp>())
		{
			...
			if (!DragDropOp->RefreshedAssetData.IsSet())
			{
				TArray<FAssetData> Refreshed;
				if (IAssetRegistry* Registry = IAssetRegistry::Get())
				{
					for (const FAssetData& OriginalData : DragDropOp->GetAssets())
					{
						FAssetData RegistryData = Registry->GetAssetByObjectPath(OriginalData.GetSoftObjectPath());
						Refreshed.Add(RegistryData.IsValid() ? MoveTemp(RegistryData) : OriginalData);
					}
				}
				else
				{
					Refreshed = DragDropOp->GetAssets();
				}
				DragDropOp->RefreshedAssetData = MoveTemp(Refreshed);
			}

			DroppedAssetData.Append(DragDropOp->RefreshedAssetData.GetValue());
		}
