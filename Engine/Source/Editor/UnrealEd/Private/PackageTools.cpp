	...
	: Super(ObjectInitializer)
{
		...
		if (PackagesBeingUnloaded && ObjectsThatHadFlagsCleared.Num() > 0)
		{
			for (UPackage* PackageBeingUnloaded : *PackagesBeingUnloaded)
			{
				ForEachObjectWithPackage(PackageBeingUnloaded, [](UObject* Object)
				{
					...
					return true;
				}, EGetObjectsFlags::IncludeNestedObjects, RF_NoFlags, EInternalObjectFlags::Unreachable);
			}
		}
	}
		...
		for( int32 PackageIndex=0; PackageIndex<TopLevelPackages.Num(); PackageIndex++ )
		{
			...
			{	
				...
				else
				{
					...
					UE_LOGF(LogPackageTools, Log, "Aborting operation as %ls was not fully loaded.",*TopLevelPackage->GetName());
					break;
				}
			}
		}
	...
	bool UPackageTools::UnloadPackages(FUnloadPackageParams& Params)
	{
		...
		if (PackagesToUnload.Num() > 0 && GEditor)
		{
			...
			for (UPackage* PackageBeingUnloaded : PackagesToUnload)
			{
				...
				GetObjectsWithPackage(PackageBeingUnloaded, ObjectsInPackage, EGetObjectsFlags::None);
				...
				GetObjectsWithPackage(PackageBeingUnloaded, ObjectsInPackage, EGetObjectsFlags::IncludeNestedObjects, RF_Transient, EInternalObjectFlags::Garbage);
	...
	struct FFilteredPackages
	{
		...
		{
			if (RealPackage->HasAnyPackageFlags(PKG_NewlyCreated))
			{
				// Newly created packages *may* exist on disk (eg, deleting an actor, submitting, and then reverting the delete)
				// so in that case we want to allow them to reload via PackagesToReload rather than block them via InMemoryPackages
				if (FPackageName::DoesPackageExist(RealPackage->GetName()))
				{
	...
	void UPackageTools::HandlePackageReloaded(const EPackageReloadPhase InPackageReloadPhase, FPackageReloadedEvent* InPackageReloadedEvent)
	{
		...
		if (InPackageReloadPhase == EPackageReloadPhase::PrePackageFixup)
		{
			...
			GetObjectsWithPackage(InPackageReloadedEvent->GetOldPackage(), Objects, EGetObjectsFlags::IncludeNestedObjects, RF_Transient, EInternalObjectFlags::Garbage);
