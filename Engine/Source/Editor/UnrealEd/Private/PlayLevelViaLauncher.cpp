...
static void HandleOutputReceived(const FString& InMessage)
{
	...
	{
		UE_LOGF(LogPlayLevel, Error, "UAT: %ls", *InMessage);
	}
	else if (InMessage.Contains(TEXT("Warning:")))
	{
		UE_LOGF(LogPlayLevel, Warning, "UAT: %ls", *InMessage);
	}
	else
	{
		UE_LOGF(LogPlayLevel, Log, "UAT: %ls", *InMessage);
	}
}
...
void UEditorEngine::StartPlayUsingLauncherSession(FRequestPlaySessionParams& InRequestParams)
{
	...
	if (LaunchPlatform != nullptr)
	{
		...
		if (LaunchPlatformNameFromID.Equals(LaunchPlatformName))
		{
			...
			for (const ITargetDevicePtr& PlayDevice : TargetDevices)
			{
				...
				else
				{
					...
					UE_LOGF(LogPlayLevel, Log, "Launcher Device ID: %ls", *PlayDeviceId);
				}
			}
		}
		else
		{
			...
			else
			{
				...
				UE_LOGF(LogPlayLevel, Log, "Launcher Device ID: %ls", *LastPlayUsingLauncherDeviceId);
			}
		}
	...
	ILauncherProfileBuildCookRunPtr BuildCookRun = LauncherProfile->GetFirstBuildCookRun();
	BuildCookRun->SetBuildMode(BuildMode);
	BuildCookRun->SetBuildConfiguration(BuildConfiguration);
	BuildCookRun->SetFastIterate(true);
	if (InRequestParams.EditorPlaySettings && !InRequestParams.EditorPlaySettings->AdditionalLaunchParameters.IsEmpty())
	{
		BuildCookRun->SetAdditionalCommandLineParameters(InRequestParams.EditorPlaySettings->AdditionalLaunchParameters);
	}

	BuildCookRun->AddCookedPlatform(LaunchPlatformName);

	BuildCookRun->SetDeviceIsASimulator(InRequestParams.LauncherTargetDevice->bIsSimulator);
	...
	{
		bInEditorCooking = Algo::AllOf(BuildCookRun->GetCookedPlatforms(),
	...
	bool bIncrementalCooking =
		!bCookOnTheFly
		&& (CookerSettings.bIterativeCookingForLaunchOn || (bLegacyIterative && ExperimentalSettings.bSharedCookedBuilds));
		...
		CurrentLauncherCookMode == ELauncherProfileCookModes::ByTheBookInEditor)
	{
		...
		if (EditorContext.World()->IsPartitionedWorld())
		{
			...
			UE_LOGF(LogPlayLevel, Error, "%ls", *ErrorMsg);
	...
	CookOptions << BuildCookRun->GetCookOptions();
	...
	if (LauncherSessionInfo->bPlayUsingLauncherHasCode)
	{
		...
		if (TargetInfo != nullptr)
		{
			...
			BuildCookRun->AddBuildTarget(TargetInfo->Name);
			BuildCookRun->SetBuildTargetSpecified(true);
		}
	}
	BuildCookRun->SetCookMode(CurrentLauncherCookMode);
	BuildCookRun->SetUnversionedCooking(!bIncrementalCooking || !bLegacyIterative); // Unversioned cooking is not allowed with legacy incremental cooking
	BuildCookRun->SetIncrementalCooking(bIncrementalCooking);
	...
	BuildCookRun->SetIncrementalDeploying(bIncrementalCooking);
	...
	BuildCookRun->SetCookOptions(*CookOptions);

	if (!InRequestParams.LauncherTargetDevice->Architecture.IsEmpty())
	{
		BuildCookRun->SetClientArchitectures({&InRequestParams.LauncherTargetDevice->Architecture, 1});
	}
	...
	BuildCookRun->SetUseZenStore(bUseZenStore);
	if (bUseZenStore)
	{
		static UE::Zen::FScopeZenService EditorStaticZenService;
	}

	if (bUseZenStore || BuildCookRun->GetCookMode() == ELauncherProfileCookModes::OnTheFlyInEditor || BuildCookRun->GetCookMode() == ELauncherProfileCookModes::OnTheFly)
	{
		BuildCookRun->SetDeploymentMode(ELauncherProfileDeploymentModes::FileServer);
	}
	...
	case EPlayOnPakFileMode::PakNoCompress:
		BuildCookRun->SetCompressed( false );
		BuildCookRun->SetDeployWithUnrealPak( true );
		break;
	case EPlayOnPakFileMode::PakCompress:
		BuildCookRun->SetCompressed( true );
		BuildCookRun->SetDeployWithUnrealPak( true );
		break;
	}
	...
	const TArray<UE::PIE::IOutOfDateAssetCompilation*> AssetCompilers = IModularFeatures::Get().GetModularFeatureImplementations<UE::PIE::IOutOfDateAssetCompilation>(UE::PIE::IOutOfDateAssetCompilation::GetModularFeatureName());
	for (UE::PIE::IOutOfDateAssetCompilation* AssetCompiler : AssetCompilers)
	{
		TArray<UObject*> AssetsWithErrors;
		AssetCompiler->HandleOutOfDateAssets(InRequestParams, AssetsWithErrors);
	}
	...
	BuildCookRun->GetDefaultLaunchRole()->SetInitialMap(InitialMapName);

	for (const FString& MapName : MapNames)
	{
		BuildCookRun->AddCookedMap(MapName);
	}

	if (BuildCookRun->GetCookMode() == ELauncherProfileCookModes::ByTheBookInEditor)
	{
		TArray<ITargetPlatform*> TargetPlatforms;
		for (const FString& PlatformName : BuildCookRun->GetCookedPlatforms())
		{
		...
		const TArray<FString> &CookedMaps = BuildCookRun->GetCookedMaps();
...
void UEditorEngine::HandleStageStarted(const FString& InStage, TWeakPtr<SNotificationItem> NotificationItemPtr)
{
	...
	{
		UE_LOGF(LogPlayLevel, Warning, "HandleStageStarted called for Stage: %ls but the session was canceled, ignoring.", *InStage);
		return;
	}
...
{
	UE_LOGF(LogPlayLevel, Log, "Completed Launch On Stage: %ls, Time: %f", *InStage, StageTime);
...
void UEditorEngine::HandleLaunchCompleted(bool Succeeded, double TotalTime, int32 ErrorCode, bool bHasCode, TWeakPtr<SNotificationItem> NotificationItemPtr)
{
	...
	if (Succeeded)
	{
		...
		UE_LOGF(LogPlayLevel, Log, "Launch On Completed. Time: %f", TotalTime);
