...
void SEditorViewportGridPanel::Construct(const FArguments& InArgs)
{
	...
	PlatformAspectRatio = 0.f;
	ForcedAspectRatio = 0.f;
	AspectRatioMode = EAspectRatioMode::Platform;	
...
void SEditorViewportGridPanel::OnArrangeChildren(const FGeometry& AllottedGeometry, FArrangedChildren& ArrangedChildren) const
{
	...
	// We wrap the main editor viewport in the middle of a 3x3 grid in order to restrict aspect ratio for preview platforms that request it.
	{
		...
		const float AspectRatio = 
			AspectRatioMode == EAspectRatioMode::Platform ? PlatformAspectRatio : 
			AspectRatioMode == EAspectRatioMode::Constrained ? ForcedAspectRatio :
			0.0;		
		...
		if (AspectRatio != 0.f)
		{
			...
			const float NewX = ContainerY * AspectRatio;
			const float NewY = ContainerX / AspectRatio;
...
{
	PlatformAspectRatio = AspectRatio;
}

void SEditorViewportGridPanel::SetAspectRatioMode(EAspectRatioMode ModeIn)
{
	AspectRatioMode = ModeIn;
}

SEditorViewportGridPanel::EAspectRatioMode SEditorViewportGridPanel::GetAspectRatioMode() const
{
	return AspectRatioMode;
}

void SEditorViewportGridPanel::SetAspectRatioConstraint(const float& AspectRatio)
{
