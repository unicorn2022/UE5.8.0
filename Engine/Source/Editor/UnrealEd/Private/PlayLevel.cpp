...
void UEditorEngine::EndPlayMap()
{
	...
	// We perform subsystem cleanup before removing the world contexts
	for (const FName& OnlineIdentifier : OnlineIdentifiers)
	{
		UOnlineEngineInterface::Get()->PIEEndCleanup(OnlineIdentifier);	
	}
	...
	{
		UE_LOGF(LogPlayLevel, Display, "Shutting down PIE online subsystems");
	...
	for (TObjectIterator<UGameInstance> It; It; ++It)
	{
		...
		ForEachObjectWithOuter(*It, MarkObjectPendingKill, EGetObjectsFlags::IncludeNestedObjects, RF_NoFlags, EInternalObjectFlags::Garbage);
		It->MarkAsGarbage();
	}
	...
	if (!UObjectBaseUtility::IsGarbageEliminationEnabled())
	{
		for (UPackage* ObjectPackage : LeakedPackages)
		{
			...
			UE_LOGF(LogTemp, Log, "Renaming PIE package from '%ls' to '%ls' to prevent future name collisions.",
				*ObjectPackage->GetName(), *NewName.ToString());
			ObjectPackage->Rename(*NewName.ToString(), nullptr, REN_DontCreateRedirectors | REN_DoNotDirty | REN_NonTransactional | REN_AllowPackageLinkerMismatch);
		}
	}
...
void UEditorEngine::CleanupPIEOnlineSessions(TArray<FName> OnlineIdentifiers)
{
	for (FName& OnlineIdentifier : OnlineIdentifiers)
	{
		...
		UE_LOGF(LogPlayLevel, Display, "Destroying online subsystem %ls", *OnlineIdentifier.ToString());
...
void UEditorEngine::TeardownPlaySession(FWorldContext& PieWorldContext)
{
	...
	if (!PieWorldContext.RunAsDedicated)
	{
		...
		if (SlatePlayInEditorSession)
		{
			...
			else if (SlatePlayInEditorSession->SlatePlayInEditorWindow.IsValid())
			{
				...
				// Viewport client is cleaned up.  Make sure it's not being accessed.
				SlatePlayInEditorSession->SlatePlayInEditorWindowViewport->SetViewportClient(nullptr);
				SlatePlayInEditorSession->SlatePlayInEditorWindowViewport.Reset();
	...
	bool bWasSimulatingInEditor = PlayInEditorSessionInfo.IsSet() ? PlayInEditorSessionInfo->OriginalRequestParams.WorldType == EPlaySessionWorldType::SimulateInEditor : false;
...
void UEditorEngine::StartQueuedPlaySessionRequest()
{
	...
	{
		UE_LOGF(LogPlayLevel, Warning, "StartQueuedPlaySessionRequest() called whith no request queued. Ignoring...");
		return;
	}
...
void UEditorEngine::StartQueuedPlaySessionRequestImpl()
{
	...
	if (bIsSeparateProcess && !SaveMapsForPlaySession())
	{
		...
		UE_LOGF(LogPlayLevel, Warning, "%ls", *ErrorMsg.ToString());
...
int32 FInternalPlayLevelUtils::ResolveDirtyBlueprints(const bool bPromptForCompile, TArray<UBlueprint*>& ErroredBlueprints, const bool bForceLevelScriptRecompile)
{
	...
	for (TObjectIterator<UBlueprint> BlueprintIt; BlueprintIt; ++BlueprintIt)
	{
		...
		// ignore garbage and up-to-date BPs
	...
	if (bRunCompilation && (InNeedOfRecompile.Num() > 0))
	{
		...
		auto OnBlueprintPreCompileLambda = [&CompiledBlueprints](UBlueprint* InBlueprint)
		{
			...
			{
				UE_LOGF(LogPlayLevel, Log, "[PlayLevel] Compiling %ls before play...", *InBlueprint->GetName());
			}
			else
			{
				UE_LOGF(LogPlayLevel, Log, "[PlayLevel]   Compiling %ls as a dependent...", *InBlueprint->GetName());
			}
		...
		UE_LOGF(LogPlayLevel, Log, "PlayLevel: Blueprint regeneration took %d ms (%i blueprints)", (int32)((FPlatformTime::Seconds() - BPRegenStartTime) * 1000), RecompiledCount);
	}
	else if (bAutoCompile)
	{
		UE_LOGF(LogPlayLevel, Log, "PlayLevel: No blueprints needed recompiling");
	}
...
bool UE::PIE::IOutOfDateAssetCompilation::ShowCompilationErrorsDialog(TConstArrayView<UObject*> InAssetsWithErrors, const FText& AssetTypeText, TFunction<void(const TWeakObjectPtr<UObject>&)> InOpenAssetFunc)
{
	if (FApp::IsUnattended() || GIsRunningUnattendedScript || !InAssetsWithErrors.Num())
	{
	...
	TSharedRef<SVerticalBox> DialogContents = SNew(SVerticalBox)
		...
		[
			SNew(STextBlock)
			.Text(FText::FormatOrdered(NSLOCTEXT("PlayInEditor", "PrePIE_AssetErrors", "Are you sure you want to Play in Editor? The following {0} {1}|plural(one=Asset,other=Assets) {1}|plural(one=has,other=have) unresolved compiler errors."), AssetTypeText, InAssetsWithErrors.Num()))
		];

	TSharedPtr<SCustomDialog> CustomDialog;

	for (UObject* Asset : InAssetsWithErrors)
	{
		TWeakObjectPtr<UObject> WeakAssetPtr = Asset;
		...
		DialogContents->AddSlot()
			...
			[
				...
				.OnNavigate(FSimpleDelegate::CreateLambda([WeakAssetPtr, &CustomDialog, InOpenAssetFunc]() 
				{
					InOpenAssetFunc(WeakAssetPtr);

					if (CustomDialog.IsValid())
					{
						CustomDialog->RequestDestroyWindow();
					}
				 }))
				.Text(FText::FromString(Asset->GetName()))
				.ToolTipText(FText::FormatOrdered(NSLOCTEXT("SourceHyperlink", "EditAsset_ToolTip", "Click to edit the {0}}"), Asset->GetClass()->GetDisplayNameText()))
			];
	}

	// Add an option to open all errored asset
	if(InAssetsWithErrors.Num() > 1)
	{
		DialogContents->AddSlot()
			...
			[
				SNew(SHyperlink)
				.Style(FAppStyle::Get(), "Common.GotoBlueprintHyperlink")
				.OnNavigate(FSimpleDelegate::CreateLambda([&InAssetsWithErrors, &CustomDialog, InOpenAssetFunc]()
				{
					for(UObject* Asset : InAssetsWithErrors)
					{
						InOpenAssetFunc(Asset);
					}
				...
				.Text(NSLOCTEXT("SourceHyperlink", "EditAllErroredAssets", "Open all assets with compilation errors"))
				.ToolTipText(FText::FormatOrdered(NSLOCTEXT("SourceHyperlink", "EditAllErroredAssets_ToolTip", "Opens all the {0} assets with compilation errors in their respective asset editor"), AssetTypeText))
			];
	}

	const FText DialogTitle = FText::FormatOrdered(NSLOCTEXT("PlayInEditor", "PrePIE_AssetErrorsTitle", "{0} Asset Compilation {1}|plural(one=Error,other=Errors)"), AssetTypeText, InAssetsWithErrors.Num());
...
void UEditorEngine::OnLoginPIEComplete_Deferred(int32 LocalUserNum, bool bWasSuccessful, FString ErrorString, FPieLoginStruct DataStruct)
{
	...
	UE_LOGF(LogPlayLevel, Verbose, "OnLoginPIEComplete LocalUserNum: %d bSuccess: %d %ls", LocalUserNum, bWasSuccessful, *ErrorString);
	...
	if (PlayInEditorSessionInfo.IsSet())
	{
		...
		if (PlayInEditorSessionInfo->NumOutstandingPIELogins == 0)
		{
			if (!bPIELoginSucceeded)
			{
				UE_LOGF(LogPlayLevel, Warning, "At least one of the requested PIE instances failed to start, ending PIE session.");
				EndPlayMap();
			}
			else
			{
...
void UEditorEngine::CreateNewPlayInEditorInstance(FRequestPlaySessionParams &InRequestParams, const bool bInDedicatedInstance, const EPlayNetMode InNetMode)
{
	...
	else
	{
		...
		if (PlayInEditorSessionInfo->bUsingOnlinePlatform)
		{
			...
			UE_LOGF(LogPlayLevel, Display, "Creating online subsystem for client %ls", *OnlineIdentifier.ToString());
...
void UEditorEngine::ToggleBetweenPIEandSIE( bool bNewSession )
{
	...
	// This is only supported inside SLevelEditor viewports currently
	if (!SlateInfoPtr || !SlateInfoPtr->DestinationSlateViewport.IsValid())
	{
		UE_LOGF(LogPlayLevel, Warning, "Failed to find valid viewport when switching between PIE and SIE.");
		return;
	}
...
UWorld* UEditorEngine::CreatePIEWorldByDuplication(FWorldContext &WorldContext, UWorld* InWorld, FString &PlayWorldMapName)
{
	...
	UE_LOGF( LogPlayLevel, Log, "Creating play world package: %ls",  *PlayWorldMapName );	
...

	{
		...
		UE_LOGF(LogPlayLevel, Log, "PIE: StaticDuplicateObject took: (%fs)",  float(FPlatformTime::Seconds() - SDOStart));		
	}
	...
	UE_LOGF(LogPlayLevel, Log, "PIE: Created PIE world by copying editor world from %ls to %ls (%fs)", *InWorld->GetPathName(), *NewPIEWorld->GetPathName(), float(FPlatformTime::Seconds() - StartTime));
	return NewPIEWorld;
}
...
void UEditorEngine::PostCreatePIEWorld(UWorld *NewPIEWorld)
{
	...
	UE_LOGF(LogPlayLevel, Log, "PIE: World Init took: (%fs)",  float(FPlatformTime::Seconds() - WorldInitStart));
...
void UEditorEngine::StartPlayInEditorSession(FRequestPlaySessionParams& InRequestParams)
{
	...
	if (GEditor->IsTransactionActive())
	{
		...
		UE_LOGF(LogPlayLevel, Warning, "Cancelling Open Transaction '%ls' to start PIE session.", *TransactionName.ToString());
	}
	...
	{
		TValueOrError<bool, FText> AuthorizationResult = Authority->RequestPIEPermission(InRequestParams.WorldType == EPlaySessionWorldType::SimulateInEditor);
		if (AuthorizationResult.HasError() || !AuthorizationResult.GetValue())
		{
			if (AuthorizationResult.HasError())
			{
				UE_LOGF(LogPlayLevel, Warning, "Play-In-Editor canceled by plugin: %ls", *AuthorizationResult.GetError().ToString());
				FSlateNotificationManager::Get().AddNotification(FNotificationInfo(AuthorizationResult.GetError()));
			}
	...
	if (ErroredBlueprints.Num() && !GIsDemoMode)
	{
		...
		bool bContinuePIE = UE::PIE::IOutOfDateAssetCompilation::ShowCompilationErrorsDialog(MakeArrayView<UObject*>((UObject**)ErroredBlueprints.GetData(),ErroredBlueprints.Num()), UBlueprint::StaticClass()->GetDisplayNameText(), [](const TWeakObjectPtr<UObject>& InAssetToOpen)
		{
	...
	// Auto-compile other dirty assets if needed, and let the user cancel PIE if there are any errors
	TArray<UE::PIE::IOutOfDateAssetCompilation*> AssetCompilers = IModularFeatures::Get().GetModularFeatureImplementations<UE::PIE::IOutOfDateAssetCompilation>(UE::PIE::IOutOfDateAssetCompilation::GetModularFeatureName());
	for (UE::PIE::IOutOfDateAssetCompilation* AssetCompiler : AssetCompilers)
	{
		TArray<UObject*> AssetsWithErrors;
		const bool bContinuePIE = AssetCompiler->HandleOutOfDateAssets(InRequestParams, AssetsWithErrors);
		if (!bContinuePIE && !GIsDemoMode)
		{
			FEditorDelegates::EndPIE.Broadcast(InRequestParams.WorldType == EPlaySessionWorldType::SimulateInEditor);
			FNavigationSystem::OnPIEEnd(*InWorld);
			CancelRequestPlaySession();
	...
	if(bRequestAllowsOnlineSubsystem)
	{
		...
		if (SupportsOnlinePIE() && InRequestParams.WorldType != EPlaySessionWorldType::SimulateInEditor)
		{
			...
			else
			{
				...
				UE_LOGF(LogPlayLevel, Verbose, "%ls", *ErrorMsg.ToString());
				FMessageLog(NAME_CategoryPIE).Warning(ErrorMsg);
			}
		}
	}
	...
	// start creating world instances and multi player clients!
	{
		...
		{
			UE_LOGF(LogPlayLevel, Warning, "PlayInEditor Session failed (%ls::PreCreatePIEInstances) and will not be started.", *GetClass()->GetName());
			return;
		}
		...
		if (bNeedsServer)
		{
			...
			if (!ServerPreCreateResult.IsSuccess())
			{
				...
				UE_LOGF(LogPlayLevel, Warning, "PlayInEditor Session Server failed Pre-Create and will not be started.");
				return;
			}
...
UGameInstance* UEditorEngine::CreateInnerProcessPIEGameInstance(FRequestPlaySessionParams& InParams, const FGameInstancePIEParameters& InPIEParameters, int32 InPIEInstanceIndex)
{
	...
	if (!InPIEParameters.bRunAsDedicated)
	{
		...
		{
			UE_LOGF(LogPlayLevel, Log, "Creating new non-realtime audio mixer");
...
FText GeneratePIEViewportWindowTitle(const EPlayNetMode InNetMode, const FRequestPlaySessionParams& InSessionParams, const int32 ClientIndex, const float FixedTick, const bool bVRPreview)
{
	...
	Args.Add(TEXT("PlatformBits"), FText::FromString(TEXT("64")));
...
TSharedRef<SPIEViewport> UEditorEngine::GeneratePIEViewportWindow(const FRequestPlaySessionParams& InSessionParams, int32 InViewportIndex, const FWorldContext& InWorldContext, EPlayNetMode InNetMode, UGameViewportClient* InViewportClient, FSlatePlayInEditorInfo& InSlateInfo)
{
	...
	// Set up a notification when the window is closed so we can clean up PIE
	{
		struct FLocal
		{
			...
			static void OnPIEWindowClosed(const TSharedRef<SWindow>& WindowBeingClosed, TWeakPtr<SViewport> PIEViewportWidget, TWeakObjectPtr<UEditorEngine> OwningEditorEngine, int32 ViewportIndex, bool bRestoreRootWindow, FDelegateHandle PreviewPlatformChangedHandle)
			{
				...
				if (OwningEditorEngine.IsValid())
				{
					...
					{
						OwningEditorEngine->OnPreviewShaderPlatformChanged().Remove(PreviewPlatformChangedHandle);
					}
				}
		...
		FDelegateHandle PreviewPlatformChangedHandle = OnPreviewShaderPlatformChanged().AddLambda([PieWindow, bHasCustomWindow, InViewportClient, InNetMode, InSessionParams, InWorldContext, bVRPreview](EShaderPlatform NewShaderPlatform)
			{
				if (!bHasCustomWindow)
				{
					FText ViewportName = GeneratePIEViewportWindowTitle(InNetMode, InSessionParams, InWorldContext.PIEInstance, InWorldContext.PIEFixedTickSeconds, bVRPreview);
				...
				InViewportClient->GetWorld()->ShaderPlatformChanged(NewShaderPlatform);
			});
		...
		PieWindow->SetOnWindowClosed(FOnWindowClosed::CreateStatic(&FLocal::OnPIEWindowClosed, TWeakPtr<SViewport>(PieViewportWidget), TWeakObjectPtr<UEditorEngine>(this),
			InViewportIndex, bShouldMinimizeRootWindowForVRPreview || bShouldMinimizeRootWindowForNonVRPreview, PreviewPlatformChangedHandle));
	}

	InSlateInfo.SlatePlayInEditorWindowViewport = FSceneViewport::Create(InViewportClient, PieViewportWidget);
