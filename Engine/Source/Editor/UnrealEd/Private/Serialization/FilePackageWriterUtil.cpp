	...
	bool bProvidePerPackageResult)
{
	...
	{
		UE_LOGF(LogSavePackage, Error, "FFilePackageWriterUtil::AsyncSaveOutputFiles : if bProvidePerPackageResult is true then AllPackageHashes can't be null.");
		return;
	}

	if (AllPackageHashes && !PackageHashesLock)
	{
		UE_LOGF(LogSavePackage, Error, "FFilePackageWriterUtil::AsyncSaveOutputFiles : if AllPackageHashes is provided, then PackageHashesLock can't be null.");
		return;
	}
		...
		if (AllPackageHashes)
		{
			...
			if (ExistingPackageHashes.IsValid())
			{
				UE_LOGF(LogSavePackage, Error,
					"FCookedFilePackageWriter encountered the same package twice in a cook! (%ls)",
