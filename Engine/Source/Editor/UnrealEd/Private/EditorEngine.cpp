...
#include "Elements/Columns/TypedElementCompatibilityColumns.h" // FTypedElementWorldColumn
#include "Elements/Columns/TypedElementMiscColumns.h" // FTypedElementSyncBackToWorldTag
...
{
	return bPreviewShaderPlatformActive ? PreviewFeatureLevel : GMaxRHIFeatureLevel;
}
...
void FAssetReferenceFilterContext::AddReferencingAssetsFromPropertyHandle(const TSharedPtr<IPropertyHandle>& PropertyHandle, FAssetData::ECreationFlags InCreationFlags)
{
	...
	// Determine if this is a soft object reference property
	if (const FSoftObjectProperty* SoftProperty = CastField<FSoftObjectProperty>(PropertyHandle->GetProperty()))
	{
		ReferencerProperties |= EAssetReferenceFilterProperties::Soft;
	}
				...
				[this, PropertyColorCustomProperty]()
				{
					...
					if (PropertyColorCustomProperty.PropertyChain.ParseIntoArray(PropertyChainNames, TEXT(".")))
					{
						...
						for (const FString& PropertyName : PropertyChainNames)
						{
							...
							{
								UE_LOGF(LogEditor, Warning, "Invalid custom property color %ls (%ls)", *PropertyColorCustomProperty.Name.ToString(), *PropertyColorCustomProperty.PropertyChain);
								break;
							}
						}
...
void UEditorEngine::InitEditor(IEngineLoop* InEngineLoop)
{
	...
	if ( FSlateApplication::IsInitialized() )
	{
		...
		// Setup a delegate to handle requests for opening debug objects
		FSlateApplication::Get().SetWidgetReflectorDebugAccessDelegate(FAccessAsset::CreateUObject(this, &UEditorEngine::HandleOpenDebugObject));
	}
	...
	if (FSlateApplication::IsInitialized() && UToolMenus::IsToolMenuUIEnabled())
	{
			...
			FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& Args)
		{
			...
			UE_LOGF(LogEditor, Log, "%ls menu editing", bNewSetEditMenusMode ? TEXT("Enable") : TEXT("Disable"));
...
bool UEditorEngine::HandleOpenDebugObject(UObject* Object)
{
	return GEditor->GetEditorSubsystem<UAssetEditorSubsystem>()->OpenDebugObjectInEditor(Object);
}
	...
	static const TCHAR* ModuleNames[] =
		{
			...
			TEXT("InputBindingSettingsViewer"),
...
void UEditorEngine::FinishDestroy()
{
	if ( !HasAnyFlags(RF_ClassDefaultObject) )
	{
		if (PlayWorld)
		{
			...
			UE_LOGF(LogEditor, Warning, "Warning: Play world is active");
		}
		...
		if( Trans )
		{
			if( GUndo )
			{
				UE_LOGF(LogEditor, Warning, "Warning: A transaction is active" );
			}
		...
		UE_LOGF(LogExit, Log, "Editor shut down" );
...
void UEditorEngine::Tick( float DeltaSeconds, bool bIdleMode )
{
	...
	// Find out if the editor has focus. Audio should only play if the editor has focus unless "Allow background audio" is set.
	...
	for (int32 ViewportIndex = AllViewportClients.Num()-1; ViewportIndex >= 0; ViewportIndex--)
	{
		...
		// Propagate pending tools-context invalidations before the below "Allowing Expensive Tasks || bNeedsRedraw" gate,
		// since those invalidations are what can set bNeedsRedraw to true.
		if (FEditorModeTools* ModeTools = ViewportClient->GetModeTools())
		{
			ModeTools->PropagateInvalidations(ViewportClient);
		}
	...
	if( FSlateThrottleManager::Get().IsAllowingExpensiveTasks() )
	{
		...
		}
		{
			...
			{
				if (!GUseUnifiedTimeBudgetForStreaming && Context.World() &&
					(Context.WorldType == EWorldType::PIE || Context.WorldType == EWorldType::Game) && !Context.RunAsDedicated)
				{
	...
	ENQUEUE_RENDER_COMMAND(TickPoolElements)([](FRHICommandListImmediate& RHICmdList)
	{
...
bool UEditorEngine::UpdateSingleViewportClient(FEditorViewportClient* InViewportClient, const bool bInAllowNonRealtimeViewportToDraw, bool bLinkedOrthoMovement, bool* bOutViewportDrawn /*= nullptr*/)
{
	...
	if( FSlateThrottleManager::Get().IsAllowingExpensiveTasks() || InViewportClient->bNeedsRedraw )
	{
		...
		else if(UWorld* World = GetWorld())
		{
			...
			WorldViewInfo.ViewMatrix = View->ViewMatrices.GetWorldToView();
			WorldViewInfo.ProjectionMatrix = View->ViewMatrices.GetViewToClip();
			WorldViewInfo.ViewProjectionMatrix = View->ViewMatrices.GetWorldToClip();
			WorldViewInfo.ViewToWorld = View->ViewMatrices.GetViewToWorld();
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UEditorEngine::Cleanse);
...
UAudioComponent* UEditorEngine::PlayPreviewSound(USoundBase* Sound,  USoundNode* SoundNode)
{
	...
	{
		AudioComponent->Play();
		OnSoundBasePreview.Broadcast(Sound);
...
void UEditorEngine::PlayEditorSound( const FString& SoundAssetName )
{
	...
	if( !UE::IsSavingPackage() && IsInGameThread() && GetDefault<ULevelEditorMiscSettings>()->bEnableEditorSounds )
	{
...
{
	// Only play sounds if the user has that feature enabled
	if (!UE::IsSavingPackage() && CanPlayEditorSound())
	{
		if (InSound != nullptr)
		{
...
void UEditorEngine::LoadMapListFromIni(const FString& InSectionName, TArray<FString>& OutMapList)
{
	...
	if (MapListList)
	{
		for (FConfigSectionMap::TConstIterator It(*MapListList) ; It ; ++It)
		{
			...
			{
				UE_LOGF(LogEditor, Warning, "Invalid entry in map ini list: %ls, %ls=%ls",
				...
				if (bAllowBrowseToAssetOverride)
				{
					if (TTypedElement<ITypedElementObjectInterface> ObjectInterface = UTypedElementRegistry::GetInstance()->GetElement<ITypedElementObjectInterface>(SelectedHandle))
					{
						if (AActor* Actor = ObjectInterface.GetObjectAs<AActor>())
						{
							...
							if (BrowseToAssetOverrideSubsystem->TryGetBrowseToAssetOverride(Actor, AssetData))
							{
...
// Note about the TEDS query getters: For now we lazily register the queries ourselves. However,
//  we may want to use queries in some shared library if the same ones are used elsewhere in the editor.
UE::Editor::DataStorage::QueryHandle UEditorEngine::GetSelectedMovementLockedItemsQuery() const
{
	using namespace UE::Editor::DataStorage;
	using namespace UE::Editor::DataStorage::Queries;

	// Lazy registration
	if (SelectedMovementLockedItemsQuery == InvalidQueryHandle)
	{
		ICoreProvider* DataStorage = GetMutableDataStorageFeature<ICoreProvider>(StorageFeatureName);
		if (ensure(DataStorage))
		{
			SelectedMovementLockedItemsQuery = DataStorage->RegisterQuery(
				Select()
				.ReadOnly<FTypedElementWorldColumn>()
				.Where()
				.All<FTypedElementSelectionColumn, FTypedElementLockTransformTag>()
				.Compile());
		}
	}
	return SelectedMovementLockedItemsQuery;
}
UE::Editor::DataStorage::QueryHandle UEditorEngine::GetSelectedItemsQuery() const
{
	using namespace UE::Editor::DataStorage;
	using namespace UE::Editor::DataStorage::Queries;

	// Lazy registration
	if (SelectedItemsQuery == InvalidQueryHandle)
	{
		ICoreProvider* DataStorage = GetMutableDataStorageFeature<ICoreProvider>(StorageFeatureName);
		if (ensure(DataStorage))
		{
			SelectedItemsQuery = DataStorage->RegisterQuery(
				Select()
				.ReadOnly<FTypedElementWorldColumn>()
				.Where()
...
void UEditorEngine::UnregisterTEDSQueries()
{
	using namespace UE::Editor::DataStorage;
	if (ICoreProvider* DataStorage = GetMutableDataStorageFeature<ICoreProvider>(StorageFeatureName))
	{
		if (SelectedMovementLockedItemsQuery != UE::Editor::DataStorage::InvalidQueryHandle)
		{
			DataStorage->UnregisterQuery(SelectedMovementLockedItemsQuery);
			SelectedMovementLockedItemsQuery = UE::Editor::DataStorage::InvalidQueryHandle;
		}
		if (SelectedItemsQuery != UE::Editor::DataStorage::InvalidQueryHandle)
		{
			DataStorage->UnregisterQuery(SelectedItemsQuery);
			SelectedItemsQuery = UE::Editor::DataStorage::InvalidQueryHandle;
		}
	}
}
...
void UEditorEngine::ToggleSelectedActorMovementLock()
{
	...
	bCheckForMovementLockedItems = true;
...
{
	// We don't replace the bHasLockedActors usage here with bHasSelectedMovementLockedItems
	//  because we don't want users of the deprecated function to incorrectly set the state
	//  of the new one with an actor-only result.
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
...
bool UEditorEngine::HasSelectedMovementLockedItems()
{
	using namespace UE::Editor::DataStorage;
	using namespace UE::Editor::DataStorage::Queries;

	if (!bCheckForMovementLockedItems)
	{
		return bHasSelectedMovementLockedItems;
	}
	ON_SCOPE_EXIT{ bCheckForMovementLockedItems = false; };

	ICoreProvider* DataStorage = GetMutableDataStorageFeature<ICoreProvider>(StorageFeatureName);
	if (!DataStorage)
	{
		// Legacy fallback if TEDS is not enabled
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		bCheckForLockActors = true;
		bHasSelectedMovementLockedItems = HasLockedActors();
		return bHasSelectedMovementLockedItems;
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
	}

	bHasSelectedMovementLockedItems = false;
	DataStorage->RunQuery(GetSelectedMovementLockedItemsQuery(), CreateDirectQueryCallbackBinding(
		[this, LevelEditorWorld = GetEditorWorldContext().World()](RowHandle Row, const FTypedElementWorldColumn& WorldColumn)
		{
			if (WorldColumn.World.Get() == LevelEditorWorld)
			{
				bHasSelectedMovementLockedItems = true;
			}
		}));
...
void UEditorEngine::ToggleSelectedItemMovementLock()
{
	using namespace UE::Editor::DataStorage;
	using namespace UE::Editor::DataStorage::Queries;
	...
	// Gather selected rows to toggle
	TArray<RowHandle> RowsToToggle;
	DataStorage->RunQuery(GetSelectedItemsQuery(), CreateDirectQueryCallbackBinding(
		[&RowsToToggle, LevelEditorWorld = GetEditorWorldContext().World()](RowHandle Row, const FTypedElementWorldColumn& WorldColumn)
		{
			if (WorldColumn.World.Get() == LevelEditorWorld)
			{
	...
	// Could have gotten this while gathering the rows, but clearer this way
	const bool bShouldLock = !HasSelectedMovementLockedItems();

	FScopedLevelDirtied LevelDirtyCallback;

	for (RowHandle Row : RowsToToggle)
	{
		if (bShouldLock)
		{
			DataStorage->AddColumn<FTypedElementLockTransformTag>(Row);
		}
		else
		{
			DataStorage->RemoveColumn<FTypedElementLockTransformTag>(Row);
		}
	...
	// Defer viewport and gizmo update until later in the tick, when TEDS processors have had a
	//  chance to deal with the column additions/removals. Else the GetElementsToManipulate
	//  call for actors may get unupdated lock state.
	ExecuteOnGameThread(UE_SOURCE_LOCATION, [WeakThis = TWeakObjectPtr<UEditorEngine>(this)]()
	{
		if (!WeakThis.IsValid())
		{
			return;
		}
		// Update viewport and gizmos (modeled on ToggleSelectedActorMovementLock)
		if (GCurrentLevelEditingViewportClient)
		{
			constexpr bool bForceCachedElementRefresh = true;
			GCurrentLevelEditingViewportClient->GetElementsToManipulate(bForceCachedElementRefresh);
		}
		WeakThis->RedrawLevelEditingViewports(false);
	});
	...
	for (int32 LevelIndex = 0; LevelIndex < LightingScenarios.Num(); LevelIndex++)
	{
		...
		{ 
			...
			if (ReadbackCaptureData.CubemapSize > 0)
			{
				...
				{
					UE_LOGF(LogEditor, Error, "Unable to build Reflection Capture %ls, requested reflection capture cube size of %d didn't fit in memory on host machine (size clamped to %d)",
						*CaptureComponent->GetPathName(), DesiredCaptureSize, ReadbackCaptureData.CubemapSize);
				}
			}
			else
			{
				UE_LOGF(LogEditor, Warning, "Unable to build Reflection Capture %ls, max number of reflection captures exceeded", *CaptureComponent->GetPathName());
			}
		}
		// Queue an update
		// Update sky light first because it's considered direct lighting, sky diffuse will be visible in reflection capture indirect specular
...
struct UEditorEngine::FWorldScopedPackageDirtyGuard final
{
	explicit FWorldScopedPackageDirtyGuard(const UWorld* InWorld)
		: World(InWorld)
	{
		const UPackage* const WorldPackage = World->GetPackage();

		bWorldPackageDirty = WorldPackage->IsDirty();

		for (const UPackage* ExternalPackage : WorldPackage->GetExternalPackages())
		{
			if (ExternalPackage->IsDirty())
			{
				DirtyExternalPackages.Add(ExternalPackage);
			}
		}
	}

	~FWorldScopedPackageDirtyGuard()
	{
		UPackage* const WorldPackage = World->GetPackage();

		for (UPackage* ExternalPackage : WorldPackage->GetExternalPackages())
		{
			if (ExternalPackage->IsDirty() && !DirtyExternalPackages.Contains(ExternalPackage))
			{
				ExternalPackage->SetDirtyFlag(false);
			}
		}

		if (WorldPackage->IsDirty() && !bWorldPackageDirty)
		{
...
private:
	...
	bool bWorldPackageDirty;
	TSet<const UPackage*> DirtyExternalPackages;
};
...
bool UEditorEngine::InitializePhysicsSceneForSaveIfNecessary(UWorld* World, bool &bOutForceInitialized)
{
	...
	if (!bHasPhysicsScene)
	{
		...
		FWorldScopedPackageDirtyGuard WorldScopedPackageDirtyGuard(World);
...
struct FEditorSaveContext
{
	...
	UE::Tasks::TTask<FSavePackageResultStruct> InnerTask;
};

void UEditorEngine::BeginEditorSave(FEditorSaveContext& Context, FSlowTask* SlowTask)
{
	bool bIsCooking = Context.SaveArgs.ArchiveCookData != nullptr;
	Context.SaveArgs.TopLevelFlags = UE::SavePackageUtilities::NormalizeTopLevelFlags(Context.SaveArgs.TopLevelFlags, bIsCooking);

	if (!Context.Asset && Context.Package)
	{
		// Check if the package contains a map and set the world object as the asset
		if (Context.Package->HasAnyPackageFlags(PKG_ContainsMap))
		{
			Context.Asset = UWorld::FindWorldInPackage(Context.Package);
		}
		else
		{
			// Otherwise find the main asset of the package
			Context.Asset = Context.Package->FindAssetInPackage();
		}
	}

	// if no save package context was passed in and the default settings were modified, install a context for the save
	if (Context.SaveArgs.SavePackageContext == nullptr && !FSavePackageSettings::GetDefaultSettings().IsDefault())
	{
		Context.UniqueContext = MakeUnique<FSavePackageContext>(nullptr, nullptr, FSavePackageSettings::GetDefaultSettings());
		Context.SaveArgs.SavePackageContext = Context.UniqueContext.Get();
	}
	...
	if (Context.SaveArgs.ArchiveCookData)
	{
		Context.ObjectSaveContext.CookType = Context.SaveArgs.ArchiveCookData->CookContext.GetCookType();
		Context.ObjectSaveContext.CookingDLC = Context.SaveArgs.ArchiveCookData->CookContext.GetCookingDLC();
		Context.ObjectSaveContext.CookInfo = Context.SaveArgs.ArchiveCookData->CookContext.GetCookInfo();
	}

	if (UWorld* World = Cast<UWorld>(Context.Asset))
	{
		Context.World = World;

		const bool bSavingConcurrent = !!(Context.SaveArgs.SaveFlags & ESaveFlags::SAVE_Concurrent);
		if (!bSavingConcurrent)
		{
			Context.bInitializedPhysicsSceneForSave = InitializePhysicsSceneForSaveIfNecessary(World, Context.bForceInitializedWorld);
			...
			if (Context.bForceInitializedWorld)
			{
				...
				if (Context.bForceInitializedWorld && LinkerLoad && LinkerLoad->GetInstancingContext().HasTag(UWorld::KeepInitializedDuringLoadTag))
				{
					Context.bForceInitializedWorld = false;
				}
			}

			OnPreSaveWorld(World, FObjectPreSaveContext(Context.ObjectSaveContext));
		}

		Context.OriginalOwningWorld = World->PersistentLevel->OwningWorld;
	...
	Context.bAutoAddPkgToSCC = false;
	if (!Context.ObjectSaveContext.bProceduralSave)
	{
		Context.bAutoAddPkgToSCC = IsPackageValidForAutoAdding(Context.Package, Context.Filename);
	}
	...
	UPackage::PreSavePackageWithContextEvent.Broadcast(Context.Package, FObjectPreSaveContext(Context.ObjectSaveContext));
}

void UEditorEngine::EndEditorSave(FEditorSaveContext& Context, FSlowTask* SlowTask)
{
	if (SlowTask)
	{
		SlowTask->EnterProgressFrame(10);
	...
	Context.ObjectSaveContext.bSaveSucceeded = Context.SavePackageResult == ESavePackageResult::Success;
	...
	if (Context.SavePackageResult == ESavePackageResult::Success && Context.bAutoAddPkgToSCC)
	{
		...
		DeferredFilesToAddToSourceControl.Add(Context.Filename);
	}
	...
	if (Context.World)
	{
		if (Context.OriginalOwningWorld)
		{
			Context.World->PersistentLevel->OwningWorld = Context.OriginalOwningWorld;
		}

		const bool bSavingConcurrent = !!(Context.SaveArgs.SaveFlags & ESaveFlags::SAVE_Concurrent);
		if (!bSavingConcurrent)
		{
			OnPostSaveWorld(Context.World, FObjectPostSaveContext(Context.ObjectSaveContext));

			if (Context.bInitializedPhysicsSceneForSave)
			{
				CleanupPhysicsSceneThatWasInitializedForSave(Context.World, Context.bForceInitializedWorld);
			}

			if (Context.SavePackageResult == ESavePackageResult::Success) // Package saved successfully?
			{
				// Rerunning construction scripts may have made it dirty again
				Context.Package->SetDirtyFlag(false);
			}
		}
	}

	if (Context.ObjectSaveContext.bUpdatingLoadedPath)
	{
		// Notify the asset registry
		IAssetRegistry& AssetRegistry = IAssetRegistry::GetChecked();
		AssetRegistry.AssetsSaved(MoveTemp(Context.SavePackageResult.SavedAssets));
	}
}
	...
	const FSavePackageArgs& InSaveArgs)
{
	...
	// Needs to be here in addition to UPackage::Save so that InitializePhysicsSceneForSaveIfNecessary and OnPreSaveWorld have appropriate referencing package info
	UE_TRACK_REFERENCING_PACKAGE_SCOPED(InOuter, PackageAccessTrackingOps::NAME_Save); 

	FScopedSlowTask SlowTask(100, FText(), InSaveArgs.bSlowTask);

	FEditorSaveContext Context
	{
		.Package = InOuter,
		.Asset = InAsset,
		.Filename = Filename,
		.SaveArgs = InSaveArgs,
		.ObjectSaveContext = FObjectSaveContextData(InOuter, InSaveArgs.GetTargetPlatform(), Filename, InSaveArgs.SaveFlags),
	};

	BeginEditorSave(Context, &SlowTask);
	Context.SavePackageResult = UPackage::Save(Context.Package, Context.Asset, *Context.Filename, Context.SaveArgs);
	EndEditorSave(Context, &SlowTask);

	return MoveTemp(Context.SavePackageResult);
}

UE::Tasks::TTask<FSavePackageResultStruct> UEditorEngine::SaveAsync(UPackage* InOuter, UObject* InAsset, const TCHAR* Filename,
		const FSavePackageArgs& InSaveArgs)
{
	FEditorSaveContext LocalContext
	{
		.Package = InOuter,
		.Asset = InAsset,
		.Filename = Filename,
	...
	auto Context = MakeShared<FEditorSaveContext>(MoveTemp(LocalContext));

	UE::Tasks::FTaskEvent TaskEvent(TEXT("EditorSaveEvent"));
	auto BeginEditorSaveTask =
		UE::Tasks::Launch(
			TEXT("BeginEditorSave"),
			[this, Context, TaskEvent]() mutable
			{
				UE_TRACK_REFERENCING_PACKAGE_SCOPED(Context->Package, PackageAccessTrackingOps::NAME_Save);

				BeginEditorSave(*Context, nullptr);

				Context->InnerTask = UPackage::SaveAsync(Context->Package, Context->Asset, *Context->Filename, Context->SaveArgs);

				// Once the result of the task is available, this will allow the endeditorsave to run
				TaskEvent.AddPrerequisites(Context->InnerTask);
				TaskEvent.Trigger();
			},
			LowLevelTasks::ETaskPriority::Normal,
			UE::Tasks::EExtendedTaskPriority::GameThreadHiPri
		);

	auto EndEditorSaveTask =
		UE::Tasks::Launch(
			TEXT("EndEditorSave"),
			[this, Context]()
			{
				UE_TRACK_REFERENCING_PACKAGE_SCOPED(Context->Package, PackageAccessTrackingOps::NAME_Save);
				Context->SavePackageResult = MoveTemp(Context->InnerTask.GetResult());
				EndEditorSave(*Context, nullptr);
				return MoveTemp(Context->SavePackageResult);
			},
			UE::Tasks::Prerequisites(BeginEditorSaveTask, TaskEvent),
			LowLevelTasks::ETaskPriority::Normal,
			UE::Tasks::EExtendedTaskPriority::GameThreadHiPri
		);

	return EndEditorSaveTask;
}
...
bool UEditorEngine::ShouldThrottleCPUUsage() const
{
	...
	if (!bIsForeground &&!FApp::HasFocus() && !IsRunningCommandlet() && !GIsAutomationTesting && !FApp::IsBenchmarking())
	{
		...
		if (bShouldThrottle)
		{
			// postloading and asset compilation needs to tick on GT and throttling them
			// can make editor loading a lot longer.
...
AActor* UEditorEngine::AddActor(ULevel* InLevel, UClass* Class, const FTransform& Transform, bool bSilent, EObjectFlags InObjectFlags, bool bSelectActor)
{
	...
	if( !bSilent )
	{
		...
		UE_LOGF(LogEditor, Log,
			"Attempting to add actor of class '%ls' to level at %0.2f,%0.2f,%0.2f",
	...
	{
		UE_LOGF(LogEditor, Error, "Class %ls is abstract.  You can't add actors of this class to the world.", *Class->GetName() );
		return NULL;
	}
	if( Class->HasAnyClassFlags(CLASS_NotPlaceable) )
	{
		UE_LOGF(LogEditor, Error, "Class %ls isn't placeable.  You can't add actors of this class to the world.", *Class->GetName() );
		return NULL;
	}
	if( Class->HasAnyClassFlags(CLASS_Transient) )
	{
		UE_LOGF(LogEditor, Error, "Class %ls is transient.  You can't add actors of this class in UnrealEd.", *Class->GetName() );
		return NULL;
	}
	...
	FVector Location;
	{
		...
		if ( Factory->FactoryCreateText( ULevel::StaticClass(), CurrentLevel, CurrentLevel->GetFName(), InObjectFlags, nullptr, TEXT("paste"), Text, Text + FCString::Strlen(Text), GWarn ) != nullptr )
		{
			...
			if ( NewActors.Num() > 0 )
			{
	            ...
	            {
		            UE_LOGF(LogEditor, Log,
			            "Added '%d' actor(s) to level at %0.2f,%0.2f,%0.2f",
...
bool UEditorEngine::LoadPreviewMesh( int32 Index )
{
	...
	{
		UE_LOGF(LogEditorViewport, Warning, "LoadPreviewMesh called while PIE world is GWorld.");
		return false;
	}
	...
	if( ViewportSettings.PreviewMeshes.IsValidIndex(Index) )
	{
		...
		{
			UE_LOGF(LogEditorViewport, Warning, "Couldn't load the PreviewMeshNames for the player at index, %d, with the name, %ls.", Index, *MeshName.ToString());
		}
	}
	else
	{
		UE_LOGF(LogEditorViewport, Log,  "Invalid array index, %d, provided for PreviewMeshNames in UEditorEngine::LoadPreviewMesh", Index );
	}
...
void UEditorEngine::UpdateAutoLoadProject()
{
	...
	if ( !GIsBuildMachine )
	{
		if(FPlatformMisc::MacOSXVersionCompare(12, 0, 0) < 0)
		{
			if(FSlateApplication::IsInitialized())
			{
				FString SuppressSettingName(FString(TEXT("UpdateMacOSX_")) + VERSION_STRINGIFY(ENGINE_MAJOR_VERSION) + TEXT("_") + VERSION_STRINGIFY(ENGINE_MINOR_VERSION) + TEXT("_") + VERSION_STRINGIFY(ENGINE_PATCH_VERSION));
				FSuppressableWarningDialog::FSetupInfo Info( LOCTEXT("UpdateMacOSX_Body","Please update to the latest version of macOS for best performance and stability."), LOCTEXT("UpdateMacOSX_Title","Update macOS"), *SuppressSettingName, GEditorSettingsIni );
			...
			{
				UE_LOGF(LogEditor, Warning, "Please update to the latest version of macOS for best performance and stability.");
			}
		}
		...
		if(FPlatformMemory::GetPhysicalGBRam() < 8)
		{
			...
			{
				UE_LOGF(LogEditor, Warning, "For best performance install at least 8GB of RAM.");
			}
		}
		...
		if(FPlatformMisc::NumberOfCores() < 4)
		{
			...
			{
				UE_LOGF(LogEditor, Warning, "For best performance a Quad-core Intel or AMD processor, 2.5 GHz or faster is recommended.");
			}
		}
	}
...
FORCEINLINE bool NetworkRemapPath_local(const FWorldContext* Context, FString& Str, bool bReading, bool bIsReplay)
{
	if (bReading)
	{
		CA_ASSUME(Context != nullptr);
		...
		const int32 PIEInstance = Context->PIEInstance;
		if (bIsReplay)
		{
			...
			const FString PrefixedFullName = UWorld::ConvertToPIEPackageName(Str, PIEInstance);
			const FString PrefixedPackageName = UWorld::ConvertToPIEPackageName(PackageNameOnly, PIEInstance);
...
bool UEditorEngine::NetworkRemapPath(UNetConnection* Connection, FString& Str, bool bReading)
{
	if (bReading)
	{
		// Reading requires a valid world which we retrieve from the supplied connection
		if (Connection == nullptr || Connection->GetWorld() == nullptr)
		{
			return false;
		}

		const bool bIsReplay = Connection->IsReplay();
		const FWorldContext& Context = GetWorldContextFromWorldChecked(Connection->GetWorld());
		return NetworkRemapPath_local(&Context, Str, bReading, bIsReplay);
	}
	else
	{
		constexpr bool bIsReplay = false;
...
{
	const FWorldContext& Context = GetWorldContextFromPendingNetGameChecked(PendingNetGame);
...
void UEditorEngine::CheckAndHandleStaleWorldObjectReferences(FWorldContext* InWorldContext)
{
	...
	for( TObjectIterator<UWorld> It; It; ++It )
	{
		...
		if (World->WorldType != EWorldType::EditorPreview && World->WorldType != EWorldType::Editor && World->WorldType != EWorldType::Inactive && World->WorldType != EWorldType::GamePreview)
		{
			...
			if (!ValidWorld)
			{
				UE_LOGF(LogLoad, Error, "Previously active world %ls not cleaned up by garbage collection!", *World->GetPathName());
				UE_LOGF(LogLoad, Error, "Once a world has become active, it cannot be reused and must be destroyed and reloaded. World referenced by:");
			
				// Only ensure because it could be an incorrectly setup preview world
				FReferenceChainSearch::FindAndPrintStaleReferencesToObject(World, (EPrintStaleReferencesOptions::Error | EPrintStaleReferencesOptions::Ensure));
			}
		}
	}
	...
	if (InWorldContext)
	{
		for (FObjectKey Key : InWorldContext->GarbageObjectsToVerify)
		{
			...
			{
				UE_LOGF(LogLoad, Error, "Object %ls not cleaned up by garbage collection!", *Object->GetPathName());
...
void UEditorEngine::AutomationLoadMap(const FString& MapName, bool bForceReload, FString* OutError)
{
	...
	if (bNeedPieStart)
	{
		UE_LOGF(LogEditor, Log, "Starting PIE for the automation tests for world, %ls", *GWorld->GetMapName());
		...
		// Keep EditorPlaySettings alive across the deferred latent command boundary - it lives inside
		// a non-UObject struct with no UPROPERTY, so the GC cannot reach it. Balanced by
		// RemoveFromRoot() in ~FStartPIEForAutomationCommand(), which always runs.
		EditorPlaySettings->AddToRoot();
		...
		// Wait for the editor world's navmesh to finish building before duplicating into PIE,
		// then start PIE and wait for the map to load (FStartPIEForAutomationCommand does both).
		ADD_LATENT_AUTOMATION_COMMAND(FWaitForEditorNavMeshBuildCommand);
	...
	if (XRSystem.IsValid() && StereoRenderingDevice.IsValid())
	{
		...
		if (StereoRenderTargetManager)
		{
			StereoRenderTargetManager->ReconfigureForShaderPlatform(
				PreviewPlatform.bPreviewShaderPlatformActive ? PreviewPlatform.ShaderPlatform : GMaxRHIShaderPlatform);
		}
	}
}
...
static void SaveShaderPlatformAsDisabled(FPreviewPlatformInfo& PreviewPlatform)
{
	...
	Settings->bPreviewShaderPlatformActive = false;
	...
	Settings->PreviewShaderPlatformFriendlyName = FText();
	...
	Settings->bPreviewShaderPlatformActive = PreviewPlatform.bPreviewShaderPlatformActive;
	...
	Settings->PreviewShaderPlatformFriendlyName = PreviewPlatform.GetFriendlyName();
}

static void PreviewPlatformChangePre()
{
	// We're changing global platform config; make sure async ops are not running

	// Wait for pending load requests.
	FlushAsyncLoading();

	// Need to block on all in-progress asset builds as we may be changing their config	
	FAssetCompilingManager::Get().FinishAllCompilation();
	
	// Stop any texture streaming in progress :
	UTexture::CancelPendingTextureStreaming();

	// Clear the Slate renderer's cached scene pointers before we recreate scenes.
	if (FSlateApplication::IsInitialized())
	{
		FSlateApplication::Get().GetRenderer()->ReleaseAccessedResources(false /* bImmediatelyFlush */);
	}

	FlushRenderingCommands();
}

extern ENGINE_API bool GDisableAutomaticTextureMaterialUpdateDependencies;
static void PreviewPlatformChangePost()
{
	// scope for RegisterTextureLODSettings ; @todo CBTLDP : remove when possible
	{
		// Need to update the cached TextureLODSettings in TargetPlatform with RegisterTextureLODSettings

		UDeviceProfile * ADP = UDeviceProfileManager::Get().GetActiveProfile();
		const UTextureLODSettings * TLS_ADP = ADP->GetTextureLODSettings();

		ITargetPlatformManagerModule* TPM = GetTargetPlatformManager();
		ITargetPlatform* TargetPlatform = TPM->GetRunningTargetPlatform();
		
		UDeviceProfile * CDP = UDeviceProfileManager::Get().FindProfile(TargetPlatform->CookingDeviceProfileName());
		UTextureLODSettings * TLS_CDP = CDP->GetTextureLODSettings();

		// typically ADP == CDP , but not always
		// when different, TextureLOD should come from CDP
		// if PreviewPlatform is enabled, then BOTH ADP and CDP should return the override

		TargetPlatform->RegisterTextureLODSettings(TLS_CDP);

		// after Register, TP->GetTextureLODSettings is equal to the CDP way
	}
	...
	for (TObjectIterator<UWorld> It; It; ++It)
	{
		if (It->Scene)
		{
			It->Scene->UpdateEarlyZPassMode();
		}
	}

	{
		// Materials are already updated by SetPreviewPlatform, so disable the automatic per-texture material dependency update
		TGuardValue<bool> Guard(GDisableAutomaticTextureMaterialUpdateDependencies, true);

		// TextureLODSettings and platform cvars may have changed
		//	need to notify all textures
		//	don't do this until all changes are done
		// We didn't do a PreEdit on all textures; rather we did a system-wide Wait at the start of this function
...
void UEditorEngine::SetPreviewPlatform(const FPreviewPlatformInfo& NewPreviewPlatform, bool bSaveSettings, bool bCompileShaders)
{
	...
	{
		UE_LOGF(LogEditor, Display, "Disable Preview Platform");
	}
	else
	{
		UE_LOGF(LogEditor, Display, "Set Preview Platform: %ls (CompileShaders=%ls)",
			*NewPreviewPlatform.GetFriendlyName().ToString(),
			bCompileShaders ? TEXT("true") : TEXT("false"));
	}

	{
		...
		SlowTask.MakeDialog(/*bShowCancelButton=*/false, /*bAllowInPIE=*/true);

		PreviewPlatformChangePre();
		...
		const bool bChangedEffectiveShaderPlatform = bChangedPreviewShaderPlatform
			? (PreviewPlatform.bPreviewShaderPlatformActive || NewPreviewPlatform.bPreviewShaderPlatformActive)
			: (NewPreviewPlatform.bPreviewShaderPlatformActive != PreviewPlatform.bPreviewShaderPlatformActive);
		...
		// Initially set preview as disabled in case shader compilation crashes
		if (bCompileShaders)
		{
		...
		// Restore device profile to defaults, then apply the preview profile
		// bRestore=true resets CVars first (needed for compile path to get a clean state).
		// bRestore=false skips the reset (toggle path avoids double CVar sink flush).
		auto UpdateDeviceProfile = [&](bool bRestore)
		{
			// Always restore if no preview profile will be set, or if explicitly requested (compile path)
			bRestore = bRestore || !PreviewPlatform.bPreviewShaderPlatformActive || PreviewPlatform.DeviceProfileName == NAME_None;
			...
			if (PreviewPlatform.bPreviewShaderPlatformActive && PreviewPlatform.DeviceProfileName != NAME_None)
			{
		...
		// Full platform change: re-register components, compile shaders
		if (bCompileShaders)
		{
			...
			// Restore editor's shader platform in case a previous SM6 preview left it stale
			...
			// Update device profiles after shaders are compiled 
			UpdateDeviceProfile(/*bRestore=*/true);
		}
		else
		{
			UpdateDeviceProfile(/*bRestore=*/false);
		}

		DefaultWorldFeatureLevel = EffectiveFeatureLevel;
		PreviewShaderPlatformChanged.Broadcast(GShaderPlatformForFeatureLevel[EffectiveFeatureLevel]);
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	...
	UE_LOGF(LogEditor, Display, "Done with Preview Platform");
}

void UEditorEngine::TogglePreviewPlatform()
{
	FPreviewPlatformInfo NewInfo = PreviewPlatform;
	NewInfo.bPreviewShaderPlatformActive ^= 1;

	// When the preview shares the editor's feature level, toggling changes GShaderPlatformForFeatureLevel which requires the compile path.
	const bool bNeedsCompile = (NewInfo.PreviewFeatureLevel == GMaxRHIFeatureLevel);
	if (bNeedsCompile)
	{
		UE_LOGF(LogEditor, Display, "Toggle requires shader compilation: preview feature level matches editor feature level (GMaxRHIFeatureLevel)");
	}

	SetPreviewPlatform(NewInfo, /*bSaveSettings=*/true, /*bCompileShaders=*/bNeedsCompile);
}

bool UEditorEngine::IsPreviewPlatformEnabled() const
{
	return PreviewPlatform.PreviewShaderFormatName != NAME_None || PreviewPlatform.PreviewShaderPlatformName != NAME_None;
}

bool UEditorEngine::IsPreviewPlatformActive() const
{
 	return PreviewPlatform.bPreviewShaderPlatformActive;
}
...
{
	return PreviewPlatform.bPreviewShaderPlatformActive ? PreviewPlatform.PreviewFeatureLevel : GMaxRHIFeatureLevel;
}
...
void UEditorEngine::LoadEditorShaderPlatformPreview()
{
	...
	if (Settings->PreviewFeatureLevel >= 0 && Settings->PreviewFeatureLevel < (int32)ERHIFeatureLevel::Num && ShaderPlatformToPreview < EShaderPlatform::SP_NumPlatforms)
	{
		...
		UE_LOGF(LogExit, Display, "LoadEditorShaderPlatformPreview: %ls %ls", *PreviewShaderPlatformName.ToString(), *Settings->PreviewPlatformName.ToString() );
		...
		SetPreviewPlatform(FPreviewPlatformInfo(
			...
			Settings->bPreviewShaderPlatformActive, 
			PreviewShaderPlatformName, 
			Settings->PreviewShaderPlatformFriendlyName,
...
namespace
{
	...
	private:
		...
		void AddExternalContent(const UE::Core::FVersePath& VersePath, TSharedPtr<IPlugin> ReferencingPlugin = {}, FAddExternalContentComplete CompleteCallback = {}) override
		{
