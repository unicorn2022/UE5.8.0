...
struct FColorGradientEditorGeometrySolver
{
	/** Constructs the solver with or without expecting an alpha channel */
	FColorGradientEditorGeometrySolver(const bool bInWithAlphaChannel)
		: bWithAlphaChannel(bInWithAlphaChannel)
	{}

	static constexpr float HandleWidth = 13.0f;
	static constexpr float HandleHeight = 16.0f;
	static constexpr float HandleColorWidth = 7.0f;
	static constexpr float HandleColorHeight = 7.0f;
	static constexpr float HandleColorXOffset = (HandleWidth - HandleColorWidth) / 2;
	static constexpr float HandleColorYOffsetForColor = HandleColorXOffset + 1;
	static constexpr float HandleColorYOffsetForAlpha = HandleHeight - HandleColorHeight - HandleColorXOffset - 1;
	...
	{
		const float ClampedHandleWidth = FMath::Max(WidgetGeometry.GetLocalSize().X - HandleWidth, 0.f);
		
		return WidgetGeometry.MakeChild(
			FVector2D(ClampedHandleWidth, HandleHeight),
	...
	TOptional<FGeometry> GetAlphaMarkAreaPaintGeometry(const FGeometry& WidgetGeometry)
	{
		...
		return TOptional<FGeometry>();
	}

	TOptional<FGeometry> GetAlphaMarkAreaMouseGeometry(const FGeometry& WidgetGeometry)
	{
		if (bWithAlphaChannel)
		{
			return WidgetGeometry.MakeChild(
	...
	FGeometry GetGradientAreaPaintGeometry(const FGeometry& WidgetGeometry, const bool bHideStops)
	{
		const float HandleWith = bHideStops ? 0.f : HandleWidth;
		const float GradientWidth = WidgetGeometry.GetLocalSize().X - HandleWith;

		const float HandlesHeight = bHideStops ? 0.f : bWithAlphaChannel ? 2.f * HandleHeight : HandleHeight;
		const float GradientHeight = WidgetGeometry.GetLocalSize().Y - HandlesHeight;

		return WidgetGeometry.MakeChild(
			FVector2D(GradientWidth, GradientHeight),
			bHideStops ? FSlateLayoutTransform() : FSlateLayoutTransform(FVector2D(HandleWidth / 2, HandleHeight))
		);
	}

	FTrackScaleInfo GetTimeScaleForWidgetGeometry(const FGeometry& WidgetGeometry, const float ViewMinInput, const float ViewMaxInput, const bool bHideStops)
	{
		return FTrackScaleInfo(ViewMinInput, ViewMaxInput, 0.0f, 1.0f, GetGradientAreaPaintGeometry(WidgetGeometry, bHideStops).GetLocalSize());
	}
	...
	float GetTimeForAbsolutePosition(const FGeometry& WidgetGeometry, const float ViewMinInput, const float ViewMaxInput, bool bHideStops, const FVector2D& AbsolutePosition)
	{
		FGeometry GradientAreaPaintGeometry = GetGradientAreaPaintGeometry(WidgetGeometry, bHideStops);
...
SColorGradientEditor::SColorGradientEditor()
	: DesiredSizeOverride(*this)
{}

bool SColorGradientEditor::ShouldHideStops() const
{
	return (bRequestHideStops.Get() && !IsEditingEnabled.Get());
}
...
void SColorGradientEditor::Construct( const FArguments& InArgs )
{
	...
	bDrawColorAndAlphaSeparate = InArgs._WithAlphaChannel && InArgs._DrawColorAndAlphaSeparate;
	bDraggingStop = false;
	DistanceDragged = 0.0f;
	ContextMenuPosition = FVector2D::ZeroVector;
	DesiredSizeOverride.Assign(*this, InArgs._DesiredSizeOverride);
	bRequestHideStops = InArgs._HideStops;

	GeometrySolver = MakePimpl<FColorGradientEditorGeometrySolver>(InArgs._WithAlphaChannel);
}
...
int32 SColorGradientEditor::OnPaint( const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled ) const
{
	...
	if( CurveOwner )
	{
		const bool bHideStops = ShouldHideStops();

		// Split the geometry into areas for stops and the gradient
		const FGeometry ColorMarkAreaGeometry = GeometrySolver->GetColorMarkAreaPaintGeometry( AllottedGeometry );
		const FGeometry GradientAreaGeometry = GeometrySolver->GetGradientAreaPaintGeometry(AllottedGeometry, bHideStops);
		const TOptional<FGeometry> AlphaMarkAreaGeometry = GeometrySolver->GetAlphaMarkAreaPaintGeometry( AllottedGeometry );

		bool bEnabled = ShouldBeEnabled( bParentEnabled );
		ESlateDrawEffect DrawEffects = bEnabled ? ESlateDrawEffect::None : ESlateDrawEffect::DisabledEffect;

		// Pixel to value input converter
		constexpr bool bWithHideStops = false;
		const FTrackScaleInfo ScaleInfo = GeometrySolver->GetTimeScaleForWidgetGeometry(AllottedGeometry, ViewMinInput.Get(), ViewMaxInput.Get(), bWithHideStops);
		...
		else if (AlphaMarkAreaGeometry.IsSet() && bAlphaAreaHovered)
		{
			// Draw a checkerboard behind there is any transparency visible
			FSlateDrawElement::MakeBox
			(
				OutDrawElements,
				LayerId,
				AlphaMarkAreaGeometry.GetValue().ToPaintGeometry(),
				WhiteBrush,
				DrawEffects,
				FLinearColor(.5f, .5f, .5f, .15f)
			);
		...
		if (!bHideStops)
		{
			...
			if (AlphaMarkAreaGeometry.IsSet())
			{
				...
				for (int32 ColorIndex = 0; ColorIndex < AlphaMarks.Num(); ++ColorIndex)
				{
					...
					// Dont draw stops which are not visible
					if (XVal >= 0 && XVal <= AlphaMarkAreaGeometry.GetValue().GetLocalSize().X)
					{
						float Alpha = CurveOwner->GetLinearColorValue(Mark.Time).A;
						DrawGradientStopMark(Mark, AlphaMarkAreaGeometry.GetValue(), XVal, FLinearColor(Alpha, Alpha, Alpha, 1.0f), OutDrawElements, LayerId, MyCullingRect, DrawEffects, false, InWidgetStyle);
					}
				}
			}
			...
			if (ColorMarks.Num() == 0 && AlphaMarks.Num() == 0 && IsEditingEnabled.Get() == true)
			{
				...
				if (AlphaMarkAreaGeometry.IsSet())
				{
					...
					const FPaintGeometry PaintGeom = AlphaMarkAreaGeometry.GetValue().ToPaintGeometry(FSlateLayoutTransform(FVector2D((AlphaMarkAreaGeometry.GetValue().GetLocalSize().X - StringSize.X) * 0.5f, 1.0f)));
...
{
	const FGeometry ColorMarkAreaMouseGeometry = GeometrySolver->GetColorMarkAreaMouseGeometry(MyGeometry);
	const TOptional<FGeometry> AlphaMarkAreaMouseGeometry = GeometrySolver->GetAlphaMarkAreaMouseGeometry(MyGeometry);
	...
	else if (AlphaMarkAreaMouseGeometry.IsSet() &&
		AlphaMarkAreaMouseGeometry.GetValue().IsUnderLocation(MouseEvent.GetScreenSpacePosition()))
	{
				...
				const float MouseTime = [&]()
					{
						...
						float Result = GeometrySolver->GetTimeForAbsolutePosition(
							MyGeometry, 
							ViewMinInput.Get(), 
							ViewMaxInput.Get(), 
							bHideStops, 
						...
						{
							Result = FMath::Clamp(Result, ViewMinInput.Get(), ViewMaxInput.Get());
						}
	...
	if( IsEditingEnabled.Get() == true )
	{
		if( MouseEvent.GetEffectingButton() == EKeys::LeftMouseButton )
		{
			...
			else if( DistanceDragged < DragThresholdDist && !SelectedStop.IsValid( *CurveOwner ) )
			{
				...
				if( ColorMarkAreaMouseGeometry.IsUnderLocation( MouseEvent.GetScreenSpacePosition() ) )
				{
					...
					AlphaMarkAreaMouseGeometry.IsSet() &&
					AlphaMarkAreaMouseGeometry.GetValue().IsUnderLocation(MouseEvent.GetScreenSpacePosition()))
				{
		...
		else if( MouseEvent.GetEffectingButton() == EKeys::RightMouseButton && !bDraggingStop)
		{
			...
			if( DistanceDragged < DragThresholdDist )
			{
				...
				{
					const bool bHideStops = ShouldHideStops();
					const FGeometry GradientAreaPaintGeometry = GeometrySolver->GetGradientAreaPaintGeometry(MyGeometry, bHideStops);
...
void SColorGradientEditor::DrawGradientStopMark( const FGradientStopMark& Mark, const FGeometry& Geometry, float XPos, const FLinearColor& Color, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FSlateRect& InClippingRect, ESlateDrawEffect DrawEffects, bool bColor, const FWidgetStyle& InWidgetStyle ) const
{
	...
	const FVector2f HandleSize = GeometrySolver->HandleRect.GetSize();
	const FVector2f HandlePosition = GeometrySolver->HandleRect.GetTopLeft2f() + FVector2f(XPos, 0);
	FSlateDrawElement::MakeBox
	( 
		OutDrawElements,
		LayerId,
	...
	const FVector2f HandleColorSize = FVector2f(GeometrySolver->HandleColorWidth, GeometrySolver->HandleColorHeight);
	const FVector2f HandleColorPosition = HandlePosition + FVector2f(
		GeometrySolver->HandleColorXOffset, 
		bColor 
			? GeometrySolver->HandleColorYOffsetForColor 
			: GeometrySolver->HandleColorYOffsetForAlpha);
	FSlateDrawElement::MakeBox
	( 
		OutDrawElements,
		LayerId+1,
...
FGradientStopMark SColorGradientEditor::GetGradientStopAtPoint( const FVector2D& MousePos, const FGeometry& MyGeometry )
{
	...
	const FGeometry ColorMarkAreaMouseGeometry = GeometrySolver->GetColorMarkAreaMouseGeometry( MyGeometry );
	const TOptional<FGeometry> AlphaMarkAreaMouseGeometry = GeometrySolver->GetAlphaMarkAreaMouseGeometry( MyGeometry );
	const bool bUnderColorMarkAreaGeometry = ColorMarkAreaMouseGeometry.IsUnderLocation(MousePos);
	const bool bUnderAlphaMarkAreaGeometry = AlphaMarkAreaMouseGeometry.IsSet() && AlphaMarkAreaMouseGeometry.GetValue().IsUnderLocation(MousePos);
	...
	if( bUnderColorMarkAreaGeometry || bUnderAlphaMarkAreaGeometry )
	{
		...
		{
			const FTrackScaleInfo TimeScaleInfo = GeometrySolver->GetTimeScaleForAreaPaintGeomtry(AreaPaintGeometry, ViewMinInput.Get(), ViewMaxInput.Get());
			for (int32 MarkIndex = 0; MarkIndex < Marks.Num(); ++MarkIndex)
			{
				...
				if (XVal >= 0)
				{
					const FVector2f HandleSize = GeometrySolver->HandleRect.GetSize2f();
					const FVector2f HandlePosition = GeometrySolver->HandleRect.GetTopLeft2f() + FVector2f(XVal, 0.0f);
					const FGeometry HandleGeometry = AreaPaintGeometry.MakeChild(HandleSize, FSlateLayoutTransform(HandlePosition));
		...
		{
			const FGeometry ColorMarkAreaPaintGeometry = GeometrySolver->GetColorMarkAreaPaintGeometry(MyGeometry);
			StopUnderMouse = GetMarkUnderMouse(MousePos, ColorMarks, ColorMarkAreaPaintGeometry);
		}
		else // bUnderAlphaMarkAreaGeometry
		{
			const TOptional<FGeometry> AlphaMarkAreaPaintGeometry = GeometrySolver->GetAlphaMarkAreaPaintGeometry(MyGeometry);
			if (AlphaMarkAreaPaintGeometry.IsSet())
			{
				StopUnderMouse = GetMarkUnderMouse(MousePos, AlphaMarks, AlphaMarkAreaPaintGeometry.GetValue());
			}
		}
	}
...
FGradientStopMark SColorGradientEditor::AddStop( const FVector2D& Position, const FGeometry& MyGeometry, bool bColorStop, TOptional<FLinearColor> OptionalColor)
{
	...
	const bool bHideStops = ShouldHideStops();
	const float NewStopTime = GeometrySolver->GetTimeForAbsolutePosition(MyGeometry, ViewMinInput.Get(), ViewMaxInput.Get(), bHideStops, Position);
	...
	if( bColorStop )
	{
		...
		const float RValue = OptionalColor.IsSet() ? OptionalColor.GetValue().R : RedCurve->Eval(NewStopTime, 1.0f);
