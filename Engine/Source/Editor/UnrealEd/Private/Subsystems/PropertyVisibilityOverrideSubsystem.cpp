...
void UPropertyVisibilityOverrideSubsystem::RegisterShouldHideDetailsPanelPropertyDelegate(const FName& DelegateName, const FShouldHideDetailsPanelPropertyDelegate& Delegate)
{
	DetailsPanelShouldHidePropertyDelegates.Add(DelegateName, Delegate);
}

void UPropertyVisibilityOverrideSubsystem::UnregisterShouldHideDetailsPanelPropertyDelegate(const FName& DelegateName)
{
	DetailsPanelShouldHidePropertyDelegates.Remove(DelegateName);
}

bool UPropertyVisibilityOverrideSubsystem::ShouldHidePropertyForDetailsPanel(const IPropertyHandle& PropertyHandle) const
{
	for (auto&& DelegatePair : DetailsPanelShouldHidePropertyDelegates)
	{
		const FShouldHideDetailsPanelPropertyDelegate& Delegate = DelegatePair.Value;
		if (Delegate.IsBound() && Delegate.Execute(PropertyHandle))
		{
			return true;
		}
	}
