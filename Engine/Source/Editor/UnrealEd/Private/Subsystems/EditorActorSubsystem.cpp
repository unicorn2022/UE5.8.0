...
namespace InternalActorUtilitiesSubsystemLibrary
{
	...
	AActor* SpawnActor(const TCHAR* MessageName, UObject* ObjToUse, FVector Location, FRotator Rotation, bool bTransient)
	{
		...
		if (!ObjToUse)
		{
			UE_LOGF(LogUtils, Error, "%ls. ObjToUse is not valid.", MessageName);
			return nullptr;
		}
		...
		if (!World)
		{
			UE_LOGF(LogUtils, Error, "%ls. Can't spawn the actor because there is no world.", MessageName);
			return nullptr;
		}
		...
		if (!DesiredLevel)
		{
			UE_LOGF(LogUtils, Error, "%ls. Can't spawn the actor because there is no Level.", MessageName);
			return nullptr;
		}
		...
		{
			UE_LOGF(LogUtils, Warning, "%ls. No actor was spawned.", MessageName);
			return nullptr;
		}
...
void UEditorActorSubsystem::SetSelectedLevelActors(const TArray<class AActor*>& ActorsToSelect)
{
	...
	if (GEdSelectionLock)
	{
		UE_LOGF(LogUtils, Warning, "SetSelectedLevelActors. The editor selection is currently locked.");
		return;
	}
	...
	if (ActorsToSelect.Num() > 0)
	{
		...
		for (AActor* Actor : ActorsToSelect)
		{
			if (InternalActorUtilitiesSubsystemLibrary::IsEditorLevelActor(Actor))
			{
				...
				{
					UE_LOGF(LogUtils, Warning, "SetSelectedLevelActors. Can't select actor '%ls'.", *Actor->GetName());
					continue;
				}
...
AActor* UEditorActorSubsystem::SpawnActorFromObject(UObject* ObjToUse, FVector Location, FRotator Rotation, bool bTransient)
{
	...
	if (!ObjToUse)
	{
		UE_LOGF(LogUtils, Error, "SpawnActorFromObject. ObjToUse is not valid.");
		return nullptr;
	}
...
bool UEditorActorSubsystem::DestroyActors(const TArray<AActor*>& ActorsToDestroy)
{
	...
	for (AActor* ActorToDestroy : ActorsToDestroy)
	{
		if (!ActorToDestroy)
		{
			UE_LOGF(LogUtils, Error, "DestroyActors. An actor to destroy is invalid.");
			return false;
		}
		if (!InternalActorUtilitiesSubsystemLibrary::IsEditorLevelActor(ActorToDestroy))
		{  
			UE_LOGF(LogUtils, Error, "DestroyActors. An actor to destroy is not part of the world editor.");
			return false;
		}
	}
	...
	if (!World)
	{
		UE_LOGF(LogUtils, Error, "DestroyActors. Can't destroy actors because there is no world.");
		return false;
	}
...
TArray<class AActor*> UEditorActorSubsystem::ConvertActors(const TArray<class AActor*>& Actors, TSubclassOf<class AActor> ActorClass, const FString& StaticMeshPackagePath)
{
	...
	{
		UE_LOGF(LogUtils, Error, "ConvertActorWith. The ActorClass is not valid.");
		return Result;
	}
	...
	if (!PackagePath.IsEmpty())
	{
		...
		if (PackagePath.IsEmpty())
		{
			UE_LOGF(LogUtils, Error, "ConvertActorWith. %ls", *FailureReason);
			return Result;
		}
	}
	...
	for (AActor* Actor : Actors)
	{
		...
		{
			UE_LOGF(LogUtils, Warning, "ConvertActorWith. %ls is not in a world. The actor will be skipped.", *Actor->GetActorLabel());
			continue;
		}
		if (ActorWorld->WorldType != EWorldType::Editor)
		{
			UE_LOGF(LogUtils, Warning, "ConvertActorWith. %ls is not in an editor world. The actor will be skipped.", *Actor->GetActorLabel());
			continue;
		}
		...
		{
			UE_LOGF(LogUtils, Warning, "ConvertActorWith. %ls must be in a valid level. The actor will be skipped.", *Actor->GetActorLabel());
			continue;
		}

		if (Cast<ABrush>(Actor) && PackagePath.Len() == 0)
		{
			UE_LOGF(LogUtils, Warning, "ConvertActorWith. %ls is a Brush and not package path was provided. The actor will be skipped.", *Actor->GetActorLabel());
			continue;
		}
	...
	UE_LOGF(LogUtils, Log, "ConvertActorWith. %d conversions occurred.", Result.Num());
	return Result;
}
...
void UEditorActorSubsystem::ConvertLightActors(UClass* InConvertToClass)
{
	...
	if (ActorsToConvert.Num())
	{
		...
		for (int32 ActorIdx = 0; ActorIdx < ActorsToConvert.Num(); ++ActorIdx)
		{
			...
			UE_LOGF(LogUtils, Log, "Converted: %ls to %ls", *ActorToConvert->GetName(), *NewActor->GetName() );
			...
			{
				UE_LOGF(LogUtils, Log, "Newly converted actor ('%ls') is pending kill", *NewActor->GetName());
			}
...
)
{
	...
	// Scope the transaction - we need it to end BEFORE we finish the slow task we just started
	{
		...
		for (int32 ActorIdx = 0; ActorIdx < InActorsToConvert.Num(); ++ActorIdx)
		{
			...
			{
				UE_LOGF(LogUtils, Error, "Actor '%ls' is invalid and cannot be converted", *ActorToConvert->GetFullName());
				continue;
			}
			...
			if (bInUseSpecialCases)
			{
				...
				{
					UE_LOGF(LogUtils, Log, "Converting light from %ls to %ls", *ActorToConvert->GetFullName(), *InConvertToClass->GetName());
				...
				{
					UE_LOGF(LogUtils, Log, "Converting brush from %ls to %ls", *ActorToConvert->GetFullName(), *InConvertToClass->GetName());
					ConvertSelectedBrushesToVolumes(InConvertToClass);
				}
				else
				{
					UE_LOGF(LogUtils, Log, "Converting actor from %ls to %ls", *ActorToConvert->GetFullName(), *InConvertToClass->GetName());
			...
			if (NewActor)
			{
				...
				UE_LOGF(LogUtils, Log, "Converted: %ls to %ls", *ActorLabel, *NewActor->GetActorLabel() );
...
private:
	...
	void InternalSetToActor(AActor* InActor) const
	{
		...
		{
			UE_LOGF(LogUtils, Fatal, "Actor was converted into a different level.");
		}
	...
	void InternalSetAnimationData(USkeletalMeshComponent* InSkeletalComp)
	{
		...
		UE_LOGF(LogAnimation, Log, "Converting animation data for AnimAsset : (%ls), bLooping(%d), bPlaying(%d), Rate(%0.2f), CurrentPos(%0.2f)", 
...
void UEditorActorSubsystem::ReplaceSelectedActors(UActorFactory* InFactory, const FAssetData& InAssetData, bool bInCopySourceProperties)
{
	...
	{
		UE_LOGF(LogUtils, Error, "UEditorEngine::ReplaceSelectedActors() called with NULL parameters!");
		return;
	}
...
)
{
	...
	for (int32 ActorIdx = 0; ActorIdx < InActorsToReplace.Num(); ++ActorIdx)
	{
		...
		if (NewActor)
		{
			// For blueprints, optionally try to copy over properties
			if (bInCopySourceProperties && InFactory->IsA(UActorFactoryBlueprint::StaticClass()))
			{
				// Only try to copy properties when the blueprint is a child of a shared native parent
				const UBlueprint* const Blueprint = CastChecked<UBlueprint>(Asset);
				UClass* const OldActorClass = OldActor->GetClass();
				const bool bUseNativeParent = GetDefault<ULevelEditorMiscSettings>()->bReplaceCopiesPropertiesWhenRelatedByNativeParent;
				const UClass* const ClassToCheck = bUseNativeParent ? GetParentNativeClass(OldActorClass) : OldActorClass;
				if (Blueprint->GeneratedClass->IsChildOf(ClassToCheck))
				{
