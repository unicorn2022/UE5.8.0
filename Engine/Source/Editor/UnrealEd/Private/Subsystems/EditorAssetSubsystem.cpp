	...
	)
{
	...
	for (const FAssetData& AssetData : Assets)
	{
		...
		{
			UE_LOGF(LogEditorAssetSubsystem, Warning, "Not all assets have the tag '%ls'", *MetaDataTag.ToString());
			return false;
		}
	}
...
UObject* UEditorAssetSubsystem::LoadAsset(const FString& AssetPath)
{
	...
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "LoadAsset failed: %ls", *LoadedAssetResult.GetError());
		return nullptr;
	}
...
UClass* UEditorAssetSubsystem::LoadBlueprintClass(const FString& AssetPath)
{
	...
	if (LoadedAssetResult.HasError())
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "LoadBlueprintClass failed: %ls", *LoadedAssetResult.GetError());
		return nullptr;
	}
	...
	if (!Blueprint)
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "LoadBlueprintClass failed: The asset is not a Blueprint.");
		return nullptr;
	}
...
FString UEditorAssetSubsystem::GetPathNameForLoadedAsset(UObject* LoadedAsset)
{
	...
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "GetPathNameForLoadedAsset failed: %ls", *Result.GetError());
	}
...
FAssetData UEditorAssetSubsystem::FindAssetData(const FString& AssetPath)
{
	...
	if (AssetDataResult.HasError())
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "FindAssetData failed: %ls", *AssetDataResult.GetError());
		return FAssetData();
	}
...
bool UEditorAssetSubsystem::DoesAssetExist(const FString& AssetPath)
{
	...
	if (ObjectPath.IsEmpty())
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "DoesAssetExist failed: %ls", *FailureReason);
		return false;
	}
...
bool UEditorAssetSubsystem::DoAssetsExist(const TArray<FString>& AssetPaths)
{
	...
	for (const FString& Path : AssetPaths)
	{
		...
		if (ObjectPath.IsEmpty())
		{
			UE_LOGF(LogEditorAssetSubsystem, Error, "DoAssetsExist failed: %ls", *FailureReason);
			return false;
		}
...
TArray<FString> UEditorAssetSubsystem::FindPackageReferencersForAsset(const FString& AnyAssetPath, bool bLoadAssetsToConfirm)
{
	...
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "FindAssetPackageReferencers failed: %ls", *FailureReason);
		return Result;
	}
	...
	if (bLoadAssetsToConfirm)
	{
		...
		if (LoadedAssetResult.HasError())
		{
			UE_LOGF(LogEditorAssetSubsystem, Error, "FindAssetPackageReferencers failed: Could not load asset. %ls", *LoadedAssetResult.GetError());
			return Result;
		}
		...
		for (FName Referencer : PackageReferencers)
		{
			...
			{
				UE_LOGF(LogEditorAssetSubsystem, Warning, "FindAssetPackageReferencers: Not able to confirm reference: %ls", *ReferencerAssetResult.GetError());
...
bool UEditorAssetSubsystem::ConsolidateAssets(UObject* AssetToConsolidateTo, const TArray<UObject*>& AssetsToConsolidate)
{
	...
	{
		UE_LOGF(LogEditorAssetSubsystem, Warning, "ConsolidateAssets: No assets to consolidate.");
		return false;
	}
	if (TError<FString> Result = UE::EditorAssetUtils::IsARegisteredAsset(AssetToConsolidateTo); Result.HasError())
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "ConsolidateAssets failed: Asset to consolidate to is not registered. %ls", *Result.GetError());
		return false;
	}
	...
	for (UObject* Object : AssetsToConsolidate)
	{
		...
		{
			UE_LOGF(LogEditorAssetSubsystem, Error, "ConsolidateAssets failed: Asset '%ls' to consolidate is not registered. %ls", *Object->GetName(), *Result.GetError());
			return false;
		}
		if (AssetToConsolidateTo->GetClass() != Object->GetClass())
		{
			UE_LOGF(LogEditorAssetSubsystem, Error, "ConsolidateAssets failed: The asset '%ls' doesn't have the same class as the AssetToConsolidateTo.", *Object->GetName());
			return false;
		}
	...
	{
		UE_LOGF(LogEditorAssetSubsystem, Warning, "ConsolidateAssets: Not all objects could be consolidated, no save has occurred");
		return false;
	}
...
bool UEditorAssetSubsystem::DeleteLoadedAsset(UObject* AssetToDelete)
{
	...
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "DeleteLoadedAsset failed: Asset is not registered. %ls", *Result.GetError());
		return false;
	}
...
bool UEditorAssetSubsystem::DeleteLoadedAssets(const TArray<UObject*>& AssetsToDelete)
{
	...
	for (UObject* Asset : AssetsToDelete)
	{
		...
		{
			UE_LOGF(LogEditorAssetSubsystem, Warning, "DeleteLoadedAssets failed: An asset is not registered. %ls", *Result.GetError());
			return false;
		}
	}
...
bool UEditorAssetSubsystem::DeleteAsset(const FString& AssetPathToDelete)
{
	...
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "DeleteAsset failed: Could not find the source asset. %ls", *AssetToDeleteResult.GetError());
		return false;
	}
...
bool UEditorAssetSubsystem::DeleteDirectory(const FString& DirectoryPath)
{
	...
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "DeleteDirectory failed: Could not convert the path. %ls", *FailureReason);
		return false;
	}
	...
	if (FilePath.IsEmpty())
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "DeleteDirectory failed: Could not convert the path '%ls' to a full path.", *ValidDirectoryPath);
		return false;
	}
	...
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "DeleteDirectory failed: Could not get asset data for the directory '%ls'", *ValidDirectoryPath);
		return false;
	}
	...
	for (const FAssetData& AssetData : AssetDatas)
	{
		...
		{
			UE_LOGF(LogEditorAssetSubsystem, Error, "DeleteDirectory failed: Some assets couldn't be loaded. %ls", *LoadedObjectResult.GetError());
			return false;
		}
	...
	{
		UE_LOGF(LogEditorAssetSubsystem, Warning, "DeleteDirectory: Not all assets were deleted.");
		return false;
	}
	...
	{
		UE_LOGF(LogEditorAssetSubsystem, Warning, "DeleteDirectory: Could not remove the directory from the Asset Registry.");
	}

	// Delete the directory from the drive
	if (!UE::EditorAssetUtils::DeleteEmptyDirectoryFromDisk(ValidDirectoryPath))
	{
		UE_LOGF(LogEditorAssetSubsystem, Warning, "DeleteDirectory: Could not remove the directory but its contained assets have been removed.");
		return false;
	}
...
namespace UE::EditorAssetUtils
{
	UObject* DuplicateAsset(UObject* SourceObject, const FString& DestinationAssetPath)
	{
		...
		{
			UE_LOGF(LogEditorAssetSubsystem, Error, "DuplicateAsset failed: The destination asset path '%ls' is not valid. %ls", *DestinationAssetPath, *FailureReason);
			return nullptr;
		}

		if (!EditorScriptingHelpers::IsAValidPathForCreateNewAsset(DestinationObjectPath, FailureReason))
		{
			UE_LOGF(LogEditorAssetSubsystem, Error, "DuplicateAsset failed: The destination path '%ls' is not valid. %ls", *DestinationObjectPath, *FailureReason);
			return nullptr;
		}
		...
		{
			UE_LOGF(LogEditorAssetSubsystem, Error, "DuplicateAsset failed: The asset at the path '%ls' already exists.", *DestinationObjectPath);
			return nullptr;
		}
...
UObject* UEditorAssetSubsystem::DuplicateLoadedAsset(UObject* SourceAsset, const FString& DestinationAssetPath)
{
	...
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "DuplicateAsset failed: Asset is not registered. %ls", *Result.GetError());
		return nullptr;
	}
...
UObject* UEditorAssetSubsystem::DuplicateAsset(const FString& SourceAssetPath, const FString& DestinationAssetPath)
{
	...
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "DuplicateAsset failed: Could not find the source asset. %ls", *SourceObjectResult.GetError());
		return nullptr;
	}
...
bool UEditorAssetSubsystem::DuplicateDirectory(const FString& SourceDirectoryPath, const FString& DestinationDirectoryPath)
{
	...
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "DuplicateDirectory failed: Could not duplicate directory. %ls", *PathsResult.GetError());
		return false;
	}
	if (TError<FString> Result = UE::EditorAssetUtils::SetupDirectoryRename(PathsResult.GetValue()); Result.HasError())
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "DuplicateDirectory failed: Could not duplicate directory. %ls", *Result.GetError());
		return false;
	}
	...
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "DuplicateDirectory failed: Could not duplicate directory. %ls", *AssetPathsResult.GetError());
        return false;
	}
	...
	{
		UE_LOGF(LogEditorAssetSubsystem, Log, "DuplicateDirectory: No assets to duplicate.");
	}
	...
	for (int32 Index = 0; Index < AssetPaths.SourceDirectoryAssetDatas.Num(); ++Index)
	{
		...
		{
			UE_LOGF(LogEditorAssetSubsystem, Warning, "DuplicateDirectory failed: Could not duplicate object '%ls'", *AssetPaths.SourceDirectoryAssetDatas[Index].GetObjectPathString());
...
namespace UE::EditorAssetUtils
{
	bool RenameLoadedAsset(UObject* SourceObject, const FString& DestinationAssetPath)
	{
		...
		if (DestinationObjectPath.IsEmpty())
		{
			UE_LOGF(LogEditorAssetSubsystem, Error, "RenameAsset failed: The destination asset path '%ls' is not valid. %ls", *DestinationAssetPath, *FailureReason);
			return false;
		}

		if (!EditorScriptingHelpers::IsAValidPathForCreateNewAsset(DestinationObjectPath, FailureReason))
		{
			UE_LOGF(LogEditorAssetSubsystem, Error, "RenameAsset failed: The destination path '%ls' is not valid. %ls", *DestinationObjectPath, *FailureReason);
			return false;
		}
		...
		if (FPackageName::DoesPackageExist(DestinationObjectPath))
		{
			UE_LOGF(LogEditorAssetSubsystem, Error, "RenameAsset failed: The destination path '%ls' already exists.", *DestinationAssetPath);
			return false;
		}
...
bool UEditorAssetSubsystem::RenameLoadedAsset(UObject* SourceAsset, const FString& DestinationAssetPath)
{
	...
	if (TError<FString> Result = UE::EditorAssetUtils::IsARegisteredAsset(SourceAsset); Result.HasError())
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "RenameAsset failed: Asset is not registered. %ls", *Result.GetError());
		return false;
	}
...
bool UEditorAssetSubsystem::RenameAsset(const FString& SourceAssetPath, const FString& DestinationAssetPath)
{
	...
	if (SourceObjectResult.HasError())
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "RenameAsset failed: Could not find the source asset. %ls", *SourceObjectResult.GetError());
		return false;
	}
...
bool UEditorAssetSubsystem::RenameDirectory(const FString& SourceDirectoryPath, const FString& DestinationDirectoryPath)
{
	...
	if (PathsResult.HasError())
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "RenameDirectory failed: Could not rename directory. %ls", *PathsResult.GetError());
		return false;
	}
	if (TError<FString> Result = UE::EditorAssetUtils::SetupDirectoryRename(PathsResult.GetValue()); Result.HasError())
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "RenameDirectory failed: Could not rename directory. %ls", *Result.GetError());
		return false;
	}
	TValueOrError<UE::EditorAssetUtils::FDirectoryRenameAssetPaths, FString> AssetPathsResult = UE::EditorAssetUtils::GetDirectoryRenameAssetPaths(PathsResult.GetValue());
	if (AssetPathsResult.HasError())
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "RenameDirectory failed: Could not rename directory. %ls", *AssetPathsResult.GetError());
		return false;
	}
	...
	{
		UE_LOGF(LogEditorAssetSubsystem, Log, "RenameDirectory. No assets to rename.");
	}
	...
	if (NumAssetsToRename > 0)
	{
		...
		for (int32 Index = 0; Index < AssetPathsResult.GetValue().SourceDirectoryAssetDatas.Num(); ++Index)
		{
			...
			{
				UE_LOGF(LogEditorAssetSubsystem, Error, "RenameDirectory failed: Some assets couldn't be renamed. %ls", *LoadResult.GetError());
				return false;
			}
		...
		{
			UE_LOGF(LogEditorAssetSubsystem, Error, "RenameDirectory failed: Could not rename the assets.");
			return false;
		}
	}
	...
	{
		UE_LOGF(LogEditorAssetSubsystem, Warning, "RenameDirectory: Could not delete the original directory but the assets have been renamed.");
	}
...
bool UEditorAssetSubsystem::CheckoutLoadedAsset(UObject* AssetToCheckout)
{
	...
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "CheckoutLoadedAsset failed: Asset is not registered. %ls", *Result.GetError());
		return false;
	}
...
bool UEditorAssetSubsystem::CheckoutLoadedAssets(const TArray<UObject*>& AssetsToCheckout)
{
	...
	for (UObject* AssetToCheckout: AssetsToCheckout)
	{
		if (TError<FString> Result = UE::EditorAssetUtils::IsARegisteredAsset(AssetToCheckout, true/*bAllowSkipBrowsableTestForExternalObject*/); Result.HasError())
		{
			UE_LOGF(LogEditorAssetSubsystem, Warning, "CheckoutLoadedAssets: An asset is not registered. %ls", *Result.GetError());
			continue;
		}
...
bool UEditorAssetSubsystem::CheckoutAsset(const FString& AssetsToCheckout)
{
	...
	if (LoadedAssetResult.HasError())
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "CheckoutAsset failed: Could not load asset: %ls", *LoadedAssetResult.GetError());
		return false;
	}
...
bool UEditorAssetSubsystem::CheckoutDirectory(const FString& DirectoryPath, bool bRecursive)
{
	...
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "CheckoutAssets failed: Could not get packages in directory. %ls", *Result.GetError());
		return false;
	}
...
bool UEditorAssetSubsystem::SaveLoadedAsset(UObject* AssetToSave, bool bOnlyIfIsDirty)
{
	...
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "SaveLoadedAsset failed: Asset is not registered. %ls", *Result.GetError());
		return false;
	}
...
bool UEditorAssetSubsystem::SaveLoadedAssets(const TArray<UObject*>& AssetsToSave, bool bOnlyIfIsDirty)
{
	...
	for (UObject* AssetToSave : AssetsToSave)
	{
		if (TError<FString> Result = UE::EditorAssetUtils::IsARegisteredAsset(AssetToSave, true/*bAllowSkipBrowsableTestForExternalObject*/); Result.HasError())
		{
			UE_LOGF(LogEditorAssetSubsystem, Warning, "SaveLoadedAsset: An asset is not registered. %ls", *Result.GetError());
		}
		else
		{
...
bool UEditorAssetSubsystem::SaveAsset(const FString& AssetsToSave, bool bOnlyIfIsDirty)
{
	...
	if (LoadedAssetResult.HasError())
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "SaveAsset failed: Could not load asset: %ls", *LoadedAssetResult.GetError());
		return false;
	}
...
bool UEditorAssetSubsystem::SaveDirectory(const FString& DirectoryPath, bool bOnlyIfIsDirty, bool bRecursive)
{
	...
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "SaveDirectory failed: Could not save. %ls", *Result.GetError());
		return false;
	}
...
bool UEditorAssetSubsystem::DoesDirectoryExist(const FString& DirectoryPath)
{
	...
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "DoesDirectoryExist failed: Could not convert path: %ls", *FailureReason);
		return false;
	}
	...
	if (FilePath.IsEmpty())
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "DoesDirectoryExist failed: Could not convert path '%ls' to a full path.", *DirectoryPackageName);
		return false;
	}
...
bool UEditorAssetSubsystem::DoesDirectoryContainAssets(const FString& DirectoryPath, bool bRecursive)
{
	...
	if (DirectoryPackageName.IsEmpty())
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "DoesDirectoryContainAssets failed: Could not convert path: %ls", *FailureReason);
		return false;
	}
	...
	if (FilePath.IsEmpty())
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "DoesDirectoryContainAssets failed: Could not convert path '%ls' to a full path.", *DirectoryPackageName);
		return false;
	}
...
bool UEditorAssetSubsystem::MakeDirectory(const FString& DirectoryPath)
{
	...
	if (DirectoryPackageName.IsEmpty())
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "MakeDirectory failed: Could not convert the path '%ls'. %ls", *DirectoryPath, *FailureReason);
		return false;
	}
	...
	if (FilePath.IsEmpty())
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "MakeDirectory failed: Could not convert the path '%ls' to a full path.", *DirectoryPackageName);
		return false;
	}
...
TArray<FString> UEditorAssetSubsystem::ListAssets(const FString& DirectoryPath, bool bRecursive, bool bIncludeFolder)
{
	...
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "ListAssets failed: Could not enumerate assets in directory '%ls'. %ls", *DirectoryPath, *Result.GetError());
		return AssetPaths;
	}
...
TArray<FString> UEditorAssetSubsystem::ListAssetsByTagValue(FName TagName, const FString& TagValue)
{
	...
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "ListAssetsByTagValue failed: The empty tag '' is not valid.");
		return Result;
	}
	...
	{
		UE_LOGF(LogEditorAssetSubsystem, Warning, "ListAssetsByTagValue failed: Could not get assets with given tag values.");
		return Result;
	}
...
TMap<FName, FString> UEditorAssetSubsystem::GetTagValues(const FString& AssetPath)
{
	...
	if (AssetDataResult.HasError())
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "GetTagValues failed: Could not get Asset Data. %ls", *AssetDataResult.GetError());
		return Result;
	}
...
TMap<FName, FString> UEditorAssetSubsystem::GetMetadataTagValues(UObject* Object)
{
	...
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "GetMetadataTagValues failed: Object is null.");
	}
...
FString UEditorAssetSubsystem::GetMetadataTag(UObject* Object, FName Tag)
{
	...
	if (!Object)
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "GetMetadataTag failed: Object is null.");
		return FString();
	}
...
void UEditorAssetSubsystem::SetMetadataTag(UObject* Object, FName Tag, const FString& Value)
{
	...
	if (!Object)
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "SetMetadataTag failed: Object is null.");
		return;
	}
	Object->Modify();
...
void UEditorAssetSubsystem::RemoveMetadataTag(UObject* Object, FName Tag)
{
	...
	if (!Object)
	{
		UE_LOGF(LogEditorAssetSubsystem, Error, "RemoveMetadataTag failed: Object is null.");
		return;
	}
	Object->Modify();
