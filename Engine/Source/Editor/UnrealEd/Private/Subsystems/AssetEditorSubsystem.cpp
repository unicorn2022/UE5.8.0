...
void UAssetEditorSubsystem::InitializeRecentAssets()
{
	...
	if(RecentAssetsList->GetNumItems() != RecentAssetEditors.Num())
	{
		...
		UE_LOGF(LogAssetEditorSubsystem, Display, "Something went wrong while loading recent assets! Num Recent Assets  = %d, Num Recent Asset Editors = %d", RecentAssetsList->GetNumItems(), RecentAssetEditors.Num());
	}
...
bool UAssetEditorSubsystem::OpenEditorForAsset(UObject* Asset, const EToolkitMode::Type ToolkitMode, TSharedPtr< IToolkitHost > OpenedFromLevelEditor, const bool bShowProgressWindow, const EAssetTypeActivationOpenedMethod OpenedMethod)
{
	...
	if(!CanOpenEditorForAsset(Asset, OpenedMethod, &ErrorMessage))
	{
		...
		if(!Asset)
		{
			UE_LOGF(LogAssetEditorSubsystem, Error, "%ls", *ErrorMessage.ToString());
		}
	...
	UE_LOGF(LogAssetEditorSubsystem, Log, "Opening Asset editor for %ls", *Asset->GetFullName());
...
bool UAssetEditorSubsystem::OpenEditorForAssets_Advanced(const TArray <UObject*>& InAssets, const EToolkitMode::Type ToolkitMode, TSharedPtr< IToolkitHost > OpenedFromLevelEditor, const EAssetTypeActivationOpenedMethod OpenedMethod)
{
	...
	if (NumNullAssets > 1)
	{
		UE_LOGF(LogAssetEditorSubsystem, Error, "Opening Asset editors failed because of null assets");
	}
	else if (NumNullAssets > 0)
	{
		UE_LOGF(LogAssetEditorSubsystem, Error, "Opening Asset editor failed because of null asset");
	}
...
bool UAssetEditorSubsystem::OpenDebugObjectInEditor(UObject* DebugObject)
{
	if (DebugObject)
	{
		DebugObjectDelegate.Broadcast(DebugObject);

		for (UClass* DebugClass = DebugObject->GetClass(); !!DebugClass; DebugClass = DebugClass->GetSuperClass())
		{
			if (const FAssetEditorOpenDebugObject* Delegate = DebugObjectDelegates.Find(DebugClass->GetFName()))
			{
				Delegate->Broadcast(DebugObject);
			}
		}
...
FAssetEditorOpenDebugObject* UAssetEditorSubsystem::GetDebugObjectDelegateForClass(UClass* InClass)
{
	if (InClass)
	{
		return &DebugObjectDelegates.FindOrAdd(InClass->GetFName());
	}
	...
	for (FThreadSafeObjectIterator EditorModeIter(UEdMode::StaticClass()); EditorModeIter; ++EditorModeIter)
	{
		...
		if (EditorModes.Contains(EditorModeInfo.ID))
		{
			...
			UE_LOGF(
				...
				Warning,
				"UAssetEditorSubsystem::RegisterEditorModes : Attempting to initialize duplicate mode with name '%ls'. Conflicting classes: '%ls' and '%ls'.",
