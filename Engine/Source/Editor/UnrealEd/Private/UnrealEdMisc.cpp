	...
	NavigationBuildingNotificationHandler(NULL)
{
	...
	const bool bBuildDependencyTrackingNeeded = GIsEditor &&
		(IsRunningCookCommandlet() || !GetDefault<UEditorExperimentalSettings>()->bDisableCookInEditor)
		&& UE::Cook::IsDependencyTrackingEnabled();
...
void FUnrealEdMisc::OnInit()
{
	...
	FCoreDelegates::OnEnsureFailed.AddRaw(this, &FUnrealEdMisc::OnEnsureFailed);
	...
	UE_LOGF(LogUnrealEdMisc, Log, "Loading editor; pre map load, took %.3f", InitialEditorStartupTime);
...

	{
		FMessageLogInitializationOptions InitOptions;
		InitOptions.bShowFilters = true;
		MessageLogModule.RegisterLogListing("EnsureFailed", LOCTEXT("EnsureFailed", "Ensure Failed"), InitOptions);
	}
	...
	UE_LOGF(LogUnrealEdMisc, Log, "Total Editor Startup Time, took %.3f", TotalEditorStartupTime);
...
void FUnrealEdMisc::EditorAnalyticsHeartbeat()
{
	...
	if (Now <= LastHeartbeatTime + HeartbeatInvervalSec*0.9)
	{
		...
		//UE_LOGF(LogUnrealEdMisc, Warning, "Heartbeat event firing too frequently (%.3f sec). This should never happen. Something is wrong with the timer delegate!", (float)(Now - LastHeartbeatTime) );
		return;
	}
...
bool FUnrealEdMisc::EnableWorldComposition(UWorld* InWorld, bool bEnable)
{
	...
	if (InWorld->WorldComposition == nullptr)
	{
		...
		auto WorldComposition = NewObject<UWorldComposition>(InWorld);
		...
		int32 NumFoundSublevels = WorldComposition->GetTilesList().Num();
		if (NumFoundSublevels)
		{
			...
			{
				WorldComposition->MarkAsGarbage();
				return false;
			}
		}

		InWorld->WorldComposition = WorldComposition;
		UWorldComposition::WorldCompositionChangedEvent.Broadcast(InWorld);
	}
...
void FUnrealEdMisc::OnExit()
{
	...
	FCoreDelegates::OnEnsureFailed.RemoveAll(this);
	...
	if( PendingProjName.Len() > 0 )
	{
		...
		if (!bSuccess)
		{
			...
			UE_LOGF(LogUnrealEdMisc, Warning, "Could not restart the editor");
	...
	if (InFlags != MapChangeEventFlags::NewMap)
	{
		...
		if (InFlags != MapChangeEventFlags::Default)
		{
			if (World->IsInitialized())
			{
				...
				// ReInitWorld (just above) cleanup and initialize the world but InitWorld does not fully initialize the navigation system for the world.
				// Do it here for map rebuilds ('Build All Levels').
				if (InFlags & MapChangeEventFlags::MapRebuild)
				{
...
void FUnrealEdMisc::OnEnsureFailed(const ANSICHAR* Expr, const ANSICHAR* File, int32 Line, const TCHAR* Msg, const TCHAR* CombinedErrorMsg)
{
	if (CombinedErrorMsg == nullptr)
	{
		return;
	}

	const UEditorPerProjectUserSettings* const EditorSettings = GetDefault<UEditorPerProjectUserSettings>();
	if (EditorSettings == nullptr || !EditorSettings->bShowEnsuresInMessageLog)
	{
		return;
	}

	FString NotifyMsg;
	if (Msg != nullptr && FCString::Strlen(Msg) > 0)
	{
		NotifyMsg = Msg;
	}
	else
	...
	// Always delay, in case we're not in the gamethread or we're in a callstack that doesn't allow slate windows to be created
	FSimpleDelegateGraphTask::CreateAndDispatchWhenReady(FSimpleDelegateGraphTask::FDelegate::CreateLambda([ErrorMsg = FString(CombinedErrorMsg), NotifyMsg = MoveTemp(NotifyMsg)]()
		{
			...
			FMessageLog EnsureFailedLogger = FMessageLog("EnsureFailed");
			EnsureFailedLogger.Error(FTextStringHelper::CreateFromBuffer(*ErrorMsg));
			EnsureFailedLogger.Notify(FText::Format(NSLOCTEXT("Core", "EnsureFailed", "Ensure condition failed: {0}"), FTextStringHelper::CreateFromBuffer(*NotifyMsg)));
	...
	{	
		...
		for (int32 iShared = 0; iShared < TemplateInfo->SharedContentPacks.Num() ; iShared++)
		{
			...
			if((EachPack.DetailLevels.Num() == 1) && ( EachEditDetail !=  EachPack.DetailLevels[0] ))
			{
				...
				UE_LOGF(LogUnrealEdMisc, Verbose, "Only 1 detail level defined for %ls in %ls. Cannot edit detail level %ls. Will fallback to %ls", *EachPack.MountName, *TemplateFilename, *DetailString,*FallbackDetailString);
			...
			else if (EachPack.DetailLevels.Num() == 0)
			{
				...
				UE_LOGF(LogUnrealEdMisc, Warning, "No detail levels defined for %ls in %ls.", *EachPack.MountName, *TemplateFilename );
				continue;
			}
			for (int32 iDetail = 0; iDetail < EachPack.DetailLevels.Num(); iDetail++)
			{
				...
				{					
					...
					if (AddedMountSources.Find(ShareMountName) == INDEX_NONE)
					{
						...
						{
							UE_LOGF(LogUnrealEdMisc, Warning, "Cannot find path %ls to mount for %ls resource in %ls.", *FullPath, *EachPack.MountName, *TemplateFilename);
						}
					}
				}
			}
