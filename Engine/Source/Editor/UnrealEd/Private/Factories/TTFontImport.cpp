...
namespace
{
	int32 FromHex(TCHAR Ch)
	{
		if (Ch >= '0' && Ch <= '9')
			...
			return 10 + Ch - 'A';
		UE_LOGF(LogTTFontImport, Fatal, "Expecting digit, got character %c", Ch);
		return 0;
	}
...
UTexture2D* UTrueTypeFontFactory::CreateTextureFromDC( UFont* Font, HDC dc, const int32 BitmapWidth, const int32 BitmapHeight, int32 TextureNum )
{
	...
	{
		UE_LOGF(LogTTFontImport, Warning, "A texture named %ls already exists!", *TextureString );
	}
	...
	const TArray< float >& ResHeights )
{
	...
	for( int32 Page = 0; Page < NumResolutions; ++Page )
	{
		...
		for( int32 CurCharIndex = 0; CurCharIndex < CharsPerPage; ++CurCharIndex )
		{
			...
			{
				UE_LOGF(LogTTFontImport, Error, "At the specified font size, at least one font glyph would be larger than the maximum texture size you specified.");
				if (bisBitmapTooNarrow)
				{
					UE_LOGF(LogTTFontImport, Error, "You should increase TexturePageWidth to at least %d.", RequiredWidth / ActualDistanceFieldScaleFactor);
				}
				else
				{
					UE_LOGF(LogTTFontImport, Error, "You should increase TexturePageMaxHeight to at least %d.", RequiredHeight / ActualDistanceFieldScaleFactor);
				}
			...
			else
			{
				...
				UE_LOGF(LogTTFontImport, Log, "OutPutGlyph X=%d Y=%d FontHeight=%d FontWidth=%d Char=%04x U=%d V=%d =Usize=%d VSIze=%d", 
	...
	const TArray< float >& ResHeights )
{
	...
	{
		UE_LOGF(LogTTFontImport, Error, "Unable to allocate bitmap buffer for font texture. Please verify the texture page size and distance field scale factor settings.");
	...
	const TArray< float >& ResHeights )
{
	...
	if( UseFiles || UseRange || UseSpecificText)
	{
		...
		if( UseFiles  || UseSpecificText)
		{
			...
			if (UseFiles)
			{
				...
				UE_LOGF(LogTTFontImport, Warning, "Checked %d files", Files.Num() );
			}
			else
			{
		...
		if( UseRange )
		{
			...
			for( const TCHAR* C=*ImportOptions->Data.UnicodeRange; *C; C++ )
			{
				...
				{
					UE_LOGF(LogTTFontImport, Warning, "Adding unicode character range %x-%x (%d-%d)",From,To,From,To);
		...
		UE_LOGF(LogTTFontImport, Warning, "Importing %d characters (unicode range %04x-%04x)", j, Min, Max);
	...
	UE_LOGF(LogTTFontImport, Log,"ImportTrueTypeFont: Total Time %0.2f", EndTime - StartTime);
