...
public:
	...
	/** Classes that are explicitly disallowed from being selected. */
	...
	virtual bool IsClassAllowed(const FClassViewerInitializationOptions& InInitOptions, const UClass* InClass, TSharedRef<FClassViewerFilterFuncs> InFilterFuncs) override
	{
		const bool bAllowed = !InClass->HasAnyClassFlags(DisallowedClassFlags)
			&& !DisallowedClasses.Contains(InClass)
...
bool UTransformProviderDataFactory::ConfigureProperties()
{
	...
	Filter->DisallowedClasses.Add(UAnimSequenceTransformProviderDataInstance::StaticClass());
	Filter->DisallowedClasses.Add(UAnimRuntimeTransformProviderData::StaticClass());
