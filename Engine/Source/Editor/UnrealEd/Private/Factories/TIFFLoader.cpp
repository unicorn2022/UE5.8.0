...
#if PLATFORM_MAC
#include "Mac/MacSystemIncludes.h" // This is needed to make sure FreeImage is not typedeffing BOOL
#endif
...
bool FTiffLoadHelper::Load(const uint8 * Buffer, uint32 Length)
{
	...
	{
		UE_LOGF(LogTIFFLoader, Error, "Can't flip TIFF image");
	}
	...
	{
		UE_LOGF(LogTIFFLoader, Error, "Unsupported TIFF format");
		return false;
	}
