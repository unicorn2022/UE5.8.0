...
TArray<FAssetCategoryPath> UFactory::GetAssetMenuPaths(const UClass* InClass, FName InCategory) const
{
	const UClass* LocalSupportedClass = InClass;

	if (UAssetDefinitionRegistry::Get() && LocalSupportedClass)
	{
		const UAssetDefinitionRegistry* AssetDefinitionRegistry = UAssetDefinitionRegistry::Get();
		const UAssetDefinition* AssetDefinition = AssetDefinitionRegistry->GetAssetDefinitionForClass(LocalSupportedClass);
		if (AssetDefinition)
		{
			TArray<FAssetCategoryPath> AssetCategoryPaths;
			for (const FAssetCategoryPath& AssetCategoryPath : AssetDefinition->GetAssetCategories())
			{
				if (AssetCategoryPath.GetCategory() == InCategory)
				{
					AssetCategoryPaths.Add(AssetCategoryPath);
				}
			}
			return AssetCategoryPaths;
		}
	}

	const static TArray<FAssetCategoryPath> DefaultSubCategories;
...
UObject* UFactory::FactoryCreateFile(UClass* InClass, UObject* InParent, FName InName, EObjectFlags Flags, const FString& Filename, const TCHAR* Parms, FFeedbackContext* Warn, bool& bOutOperationCanceled)
{
	...
	if (bText)
	{
		...
		{
			UE_LOGF(LogFactory, Error, "Failed to load file '%ls' to string", *Filename);
			return nullptr;
		}
	...
	// load as binary
	{
		...
		{
			UE_LOGF(LogFactory, Error, "Failed to load file '%ls' to array", *Filename);
			return nullptr;
		}
...
bool UFactory::FactoryCanImport( const FString& Filename )
{
	...
	if (FPaths::GetExtension(Filename) == TEXT("t3d"))
	{
		...
		if (FFileHelper::LoadFileToString(Data, *Filename))
		{
			...
			UE_LOGF(LogFactory, Warning, "Factory import failed due to invalid format: %ls", *Filename);
		}
		else
		{
			UE_LOGF(LogFactory, Warning, "Factory import failed due to inability to load file %ls", *Filename);
		}
	}
...
UObject* UFactory::ImportObject(UClass* InClass, UObject* InOuter, FName InName, EObjectFlags InFlags, const FString& Filename, const TCHAR* Parms, bool& OutCanceled)
{
	...
	if (CanCreateNew())
	{
		UE_LOGF(LogFactory, Log, "FactoryCreateNew: %ls with %ls (%i %i %ls)", *InClass->GetName(), *GetClass()->GetName(), bCreateNew, bText, *Filename);
		ParseParms(Parms);
	...
	else if (!Filename.IsEmpty())
	{
		...
		{
			UE_LOGF(LogFactory, Error, "Can't find file '%ls' for import", *Filename);
		}
		else
		{
			UE_LOGF(LogFactory, Log, "FactoryCreateFile: %ls with %ls (%i %i %ls)", *InClass->GetName(), *GetClass()->GetName(), bCreateNew, bText, *Filename);
...
TArray<FAssetCategoryPath> UFactory::GetAssetMenuPathsForCategory(FName InCategory) const
{
	return GetAssetMenuPaths(GetSupportedClass(), InCategory);
}
...
UObject* UFactory::CreateOrOverwriteAsset(UClass* InClass, UObject* InParent, FName InName, EObjectFlags InFlags, UObject* InTemplate) const
{
	...
	{
		UE_LOGF(LogFactory, Warning, "Could not delete existing asset %ls", *ExistingAsset->GetFullName());
		return nullptr;
	}
