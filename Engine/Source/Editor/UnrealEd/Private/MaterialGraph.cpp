...
void UMaterialGraph::UpdatePinTypes()
{
	...
	// Clear any previously-set value type objects, preserving UEnum objects used for inline editing.
	for (UEdGraphNode* Node : Nodes)
	{
		for (UEdGraphPin* Pin : Node->Pins)
		{
			if (Cast<UMaterialShaderValueTypeObject>(Pin->PinType.PinSubCategoryObject.Get()))
			{
	...
	{
		UMaterialShaderValueTypeObject* ValueTypeObj = UMaterialShaderValueTypeObject::Get(ConnectionInsight.ValueType);
		...
		if (UMaterialGraphNode_Base* InputNode = ::Private::FindGraphNodeForObject(ConnectionInsight.InputObject))
		{
			...
			{
				InputPin->PinType.PinSubCategoryObject = ValueTypeObj;
			}
		}
		...
		if (UMaterialGraphNode_Base* OutputNode = ::Private::FindGraphNodeForObject(ConnectionInsight.OutputExpression))
		{
			...
			{
				OutputPin->PinType.PinSubCategoryObject = ValueTypeObj;
			}
		}
	}
}
	...
	if (!MaterialFunction && !SubgraphExpression)
	{
		...
		for (int32 UVIndex = 0; UVIndex < UE_ARRAY_COUNT(Material->GetEditorOnlyData()->CustomizedUVs); UVIndex++)
		{
			MaterialInputs.Add(FMaterialInputInfo(FText::FromString(FString::Printf(TEXT("Customized UV%u"), UVIndex)), (EMaterialProperty)(MP_CustomizedUVs0 + UVIndex), 
				FText::Format(LOCTEXT("CustomizedUVTooltip", "Customized UV{0} allows running calculations in the vertex shader to increase performance over running them per-pixel.\n"
				"Use the TexCoords{0} node (or ConstCoordinate {0} in the TextureSample node) to access the interpolated custom UV in the pixel shader."), UVIndex)));
		}
...
void UMaterialGraph::GetUnusedMaterialExpressions(UMaterial* Material, TArray<UMaterialExpression*>& OutUnused)
{
	OutUnused.Empty();
	if (!Material)
	{
	...
	TArray<UMaterialExpression*> ToCheck;

	// Seed from all material property inputs
	for (int32 InputIndex = 0; InputIndex < MP_MAX; ++InputIndex)
	{
		...
		{
			ToCheck.Push(Input->Expression);
		}
	}

	// CustomOutput expressions are always considered used regardless of connections
	for (UMaterialExpression* Expression : Material->GetExpressions())
	{
		if (Expression && Expression->IsA<UMaterialExpressionCustomOutput>())
		{
			ToCheck.Push(Expression);
		}
	}

	// Depth-first reachability analysis on raw expression data
	TSet<UMaterialExpression*> Reachable;
	while (ToCheck.Num() > 0)
	{
		UMaterialExpression* Expr = ToCheck.Pop();
		...
		for (FExpressionInputIterator It(Expr); It; ++It)
		{
			if (It->Expression)
			{
				ToCheck.Push(It->Expression);
			}
		}

		// Named reroute usage has no input pins — manually follow its declaration
		if (const UMaterialExpressionNamedRerouteUsage* RerouteUsage = Cast<UMaterialExpressionNamedRerouteUsage>(Expr))
		{
			if (RerouteUsage->Declaration)
			{
				ToCheck.Push(RerouteUsage->Declaration);
			}
		}
	}
...
void UMaterialGraph::GetUnusedExpressions(TArray<UEdGraphNode*>& UnusedNodes) const
{
	...
	if (RootNode)
	{
		// Material case: delegate to the expression-based algorithm and map to graph nodes
		TArray<UMaterialExpression*> UnusedExpressions;
		GetUnusedMaterialExpressions(Material, UnusedExpressions);
		for (UMaterialExpression* Expression : UnusedExpressions)
		{
			if (Expression->GraphNode)
			{
				UnusedNodes.Add(Expression->GraphNode);
			}
		}
		return;
	}
