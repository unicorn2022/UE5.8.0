...
bool StaticMeshImportUtils::AddBoxGeomFromTris( const TArray<FPoly>& Tris, FKAggregateGeom* AggGeom, const TCHAR* ObjName )
{
	...
	for(int32 i=0; i<Tris.Num(); i++)
	{
		...
		for(int32 j=0; j<Planes.Num() && !bFoundPlane; j++)
		{
			...
			if( AreParallel( Tris[i].Normal, Planes[j].Normal ) )
			{
				...
				{
					UE_LOGF(LogStaticMeshImportUtils, Log, "AddBoxGeomFromTris (%ls): Found more than 2 planes with different distances.", ObjName);
					return false;
				}
	...
	if(Planes.Num() != 3)
	{
		UE_LOGF(LogStaticMeshImportUtils, Log, "AddBoxGeomFromTris (%ls): Not very box-like (need 3 sets of planes).", ObjName);
		return false;
	}
	...
	{
		UE_LOGF(LogStaticMeshImportUtils, Log, "AddBoxGeomFromTris (%ls): Incomplete set of planes (need 2 per axis).", ObjName);
		return false;
	}
	...
	{
		UE_LOGF(LogStaticMeshImportUtils, Log, "AddBoxGeomFromTris (%ls): Box axes are not perpendicular.", ObjName);
		return false;
	}
...
bool StaticMeshImportUtils::AddSphereGeomFromVerts( const TArray<FVector3f>& Verts, FKAggregateGeom* AggGeom, const TCHAR* ObjName )
{
	...
	{
		UE_LOGF(LogStaticMeshImportUtils, Log, "AddSphereGeomFromVerts (%ls): Sphere bounding box not square.", ObjName);
		return false;
	}
	...
	{
		UE_LOGF(LogStaticMeshImportUtils, Log, "AddSphereGeomFromVerts (%ls): Vertices not at constant radius.", ObjName );
		return false;
	}
