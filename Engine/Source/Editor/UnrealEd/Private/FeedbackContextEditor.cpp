	...
	void UpdateDynamicProgressBars()
	{
		...
		for (int32 ReverseScopeIndex = ScopeCount - 1; ReverseScopeIndex > 0 && DynamicProgressIndices.Num() <= MaxNumSecondaryBars; --ReverseScopeIndex)
		{
			...
			if (Scope->Visibility == ESlowTaskVisibility::Important)
			{
				...
				{
					UE_LOGF(LogSlate, Warning, "An important slow task had no default message, if this is intended this warning can be ignored.");
				}
		...
		for (int32 ScopeIndex = 1, InsertIndex = 1; ScopeIndex < ScopeCount && DynamicProgressIndices.Num() <= MaxNumSecondaryBars; ++ScopeIndex)
		{
			...
			switch (Scope->Visibility)
			{
				...
				case ESlowTaskVisibility::ForceVisible:
				{
					if (Scope->DefaultMessage.IsEmpty())
					{
						UE_LOGF(LogSlate, Warning, "A forced visible slow task had no default message, if this is intended this warning can be ignored.");
					}
...
static bool IsFallbackSplashScreenAllowed()
{
#if WITH_EDITOR
	return !FApp::IsUnattended() && !IsRunningDedicatedServer() && FPlatformProcess::SupportsMultithreading() && !FParse::Param(FCommandLine::Get(), TEXT("RenderOffScreen"));
#else
	return false;
#endif
}
...
void FFeedbackContextEditor::StartSlowTask( const FText& Task, bool bShowCancelButton )
{
	...
	if (GEditor)
	{
		...
		{
			UE_LOGF(LogSlate, Log, "Slow task dialog suppressed because a drag operation is in progress");
			return;
		}
		...
		if (ParentWindow.IsValid())
		{
			...
			if (bHaveOpenMenu)
			{
				UE_LOGF(LogSlate, Log, "Prevented a slow task dialog from being summoned while a context menu was open");
			}

			if (!bProgressWindowShown && !bHaveOpenMenu)
			{
				...
				else if(IsFallbackSplashScreenAllowed() && !FPlatformSplash::IsShown())
				{
					bFallbackSplashScreenVisible = true;
...
void FFeedbackContextEditor::FinalizeSlowTask()
{
	...
	if (bFallbackSplashScreenVisible)
	{
