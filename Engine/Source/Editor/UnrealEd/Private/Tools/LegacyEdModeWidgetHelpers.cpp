...
bool FLegacyEdModeWidgetHelper::ShouldDrawWidget() const
{
	...
	if (Owner->GetSelectedComponents() && Owner->GetSelectedComponents()->Num() > 0)
	{
...
bool FLegacyEdModeWidgetHelper::RequiresLegacyViewportInteractions() const
{
	return true;
}
	...
	if (UsesPropertyWidgets())
	{
		...
		if (BestSelectedItem != nullptr)
		{
			...
			for (const FPropertyWidgetInfo& WidgetInfo : WidgetInfos)
			{
				...
				const double ZoomFactor = FMath::Min(View->ViewMatrices.GetViewToClip().M[0][0], View->ViewMatrices.GetViewToClip().M[1][1]);
...
bool UBaseLegacyWidgetEdMode::RequiresLegacyViewportInteractions() const
{
	return WidgetHelper->RequiresLegacyViewportInteractions();
}
