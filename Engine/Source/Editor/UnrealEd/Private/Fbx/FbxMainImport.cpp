...
bool FFbxImporter::OpenFile(FString Filename)
{
	...
	{
		UE_LOGF(LogFbx, Error,"Call to FbxImporter::Initialize() failed.");
		UE_LOGF(LogFbx, Warning, "Error returned: %ls", UTF8_TO_TCHAR(Importer->GetStatus().GetErrorString()));

		if (Importer->GetStatus().GetCode() == FbxStatus::eInvalidFileVersion )
		{
			UE_LOGF(LogFbx, Warning, "FBX version number for this FBX SDK is %d.%d.%d",
...
bool FFbxImporter::ImportFile(FString Filename, bool bPreventMaterialNameClash /*=false*/)
{
	if (Scene)
	{
		UE_LOGF(LogFbx, Error, "FBX Scene already loaded from %ls", *Filename);
		return false;
	}
	...
	UE_LOGF(LogFbx, Log, "Loading FBX Scene from %ls", *Filename);
	...
	if(bStatus)
	{
		UE_LOGF(LogFbx, Log, "FBX Scene Loaded Succesfully");
		CurPhase = IMPORTED;
	}
	else
	{
