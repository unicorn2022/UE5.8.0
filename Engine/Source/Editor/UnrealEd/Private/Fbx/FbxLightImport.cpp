...
ALight* FFbxImporter::CreateLight(FbxLight* InLight, UWorld* InWorld)
{
	...
	if( LightClass )
	{
		...
		{
			UE_LOGF(LogFbxLightImport, Warning,"Failed to create light type %s ", InLight->GetName() );
		}
	}
	else
	{
		UE_LOGF(LogFbxLightImport, Warning,"Invalid light type %s ", InLight->GetName() );
	}
	...
	case FbxLight::ePoint:
		{
			...
			{
				UE_LOGF(LogFbxLightImport, Error,"FBX Light type 'Point' does not match unreal light component");
			}
		}
		break;
	...
	case FbxLight::eSpot:
		{
			...
			{
				UE_LOGF(LogFbxLightImport, Error,"FBX Light type 'Spot' does not match unreal light component");
			}
		}
		break;
