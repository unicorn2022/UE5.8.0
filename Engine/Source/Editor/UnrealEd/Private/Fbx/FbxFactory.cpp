 ...
 )
{
	...
	{
		UE_LOGF(LogFbx, Error, "Failed to load file '%ls'", *InFilename)
		return nullptr;
	}
			...
			for (int32 i = 0; i < Property->ArrayDim; i++)
			{
				...
				if (bFoundValue)
				{
					if (Property->ImportText_Direct(*Value, Property->ContainerPtrToValuePtr<uint8>(ObjectToLoadOptions, i), ObjectToLoadOptions, PortFlags) == NULL)
					{
						...
						UE_LOGF(LogFbx, Error, "FBX Options LoadOptions (%ls): import failed for %ls in: %ls", *ObjectToLoadOptions->GetPathName(), *Property->GetName(), *Value);
					}
				}
			}
		}
