...
/**
 * Returns true if a viewport interaction with the specified name is enabled and able to act 
 */
bool IsViewportInteractionEnabled(const FEditorModeTools* InModeTools, FName InViewportInteraction)
{
	if (InModeTools)
	{
		if (UViewportInteractionsBehaviorSource* InputToolsBehaviorSource = InModeTools->GetInteractiveToolsContext()->GetViewportInteractionsBehaviorSource())
		{
			if (UViewportInteraction* Interaction = InputToolsBehaviorSource->FindInteraction(InViewportInteraction))
			{
...
/**
 * Returns true if a viewport interaction with the specified name is active and currently acting 
 */
...
/**
 * Returns true if any viewport interaction in the specified group is enabled and able to act. 
 */
bool IsViewportInteractionGroupEnabled(const FEditorModeTools* InModeTools, FName InViewportInteractionGroup)
{
	if (InModeTools)
	{
		if (UViewportInteractionsBehaviorSource* InputToolsBehaviorSource = InModeTools->GetInteractiveToolsContext()->GetViewportInteractionsBehaviorSource())
		{
			return InputToolsBehaviorSource->IsAnyInteractionInGroupEnabled(InViewportInteractionGroup);\
		}
	}
...
{
	const FVector4 ScreenPos = View->CursorToScreen(static_cast<float>(X), static_cast<float>(Y), 0.0f, bInClampToBounds);
...
FEditorViewportClient::FEditorViewportClient(FEditorModeTools* InModeTools, FPreviewScene* InPreviewScene, const TWeakPtr<SEditorViewport>& InEditorViewportWidget)
	...
	, CurrentMegaLightsVisualizationMode(NAME_None)
...
FEditorViewportClient::~FEditorViewportClient()
{
	...
	if(Viewport)
	{
		UE_LOGF(LogEditorViewport, Fatal, "Viewport != nullptr in FEditorViewportClient destructor.");
	}
...
FEditorViewportClient::FColorChannelMaskParams::FColorChannelMaskParams()
	: ColorChannelMask(EColorChannelMask::All)
	, bInvertAlphaChannelMask(false)
	, bDrawAlphaBlendedCheckerboard(false)
{
}
...
FSceneView* FEditorViewportClient::CalcSceneView(FSceneViewFamily* ViewFamily, const int32 StereoViewIndex)
{
	...
	if (GEditor != nullptr && GEditor->IsPreviewPlatformActive() && GEditor->GetAllowConstrainedAspectRatioInPreview())
	{
		float PlatformAspectRatio = GEditor->PreviewPlatform.GetConstrainedAspectRatio();
		if (GEditor->GetFlipConstrainedAspectRatioInPreview() && PlatformAspectRatio != 0.f)
		{
			PlatformAspectRatio = 1 / PlatformAspectRatio;
		}
		FSlateApplication::Get().OnConstrainedAspectRatioChanged.Broadcast(PlatformAspectRatio);
	}
	else
	{
	...
	else
	{
		...
		else
		{
			...
			//Only lit mode render passes require the restricted depth, so wireframe and unlit can just use the standard path
			bool bShouldCalculateDepthRange = ViewFamily->ViewMode > VMI_Unlit;
			...
			CalculatePlanesViewInfo.bAutoCalculateOrthoPlanes = bShouldCalculateDepthRange;
			CalculatePlanesViewInfo.AutoPlaneShift = 0.0f;
			CalculatePlanesViewInfo.bUpdateOrthoPlanes = bShouldCalculateDepthRange;
			...
			else if (!bShouldCalculateDepthRange)
			{
	...
	View->CompositePrimitiveParams.OccludedDitheringPatternSize = EditorStyle->OccludedDitheringPatternSize;
	View->CompositePrimitiveParams.OccludedDithering = EditorStyle->OccludedDithering;
	View->CompositePrimitiveParams.OccludedBrightness = EditorStyle->OccludedBrightness;
...
void FEditorViewportClient::Tick(float DeltaTime)
{
	...
	if( !bIsAnimating )
	{
		...
		if (ModeTools && ModeTools->UsesViewportITF())
		{
			ConsumeExternalCameraMovement();
		}
		else
		{
			UpdateMouseDelta();
...
EMouseCursor::Type FEditorViewportClient::GetCursor(FViewport* InViewport,int32 X,int32 Y)
{
	...
	bool bMoveCanvasMovement = !ModeTools->UsesViewportITF() && ShouldUseMoveCanvasMovement();
...
{
	const bool bIsITFInteractions = ModeTools->UsesViewportITF();
	
	// We only want to move perspective cameras around like this
	if( Viewport && IsPerspective() && (bIsITFInteractions ? !IsViewportInteractionActive(ModeTools.Get(), UE::Editor::ViewportInteractions::Orbit) : !ShouldOrbitCamera()))
	{
		const bool bEnable = false;
		ToggleOrbitCamera(bEnable);
		...
		const bool bUsingFlightInput = (bIsITFInteractions ? ViewportNavigationHelper->bIsMouseLooking : IsFlightCameraInputModeActive()) || bIsUsingTrackpad;
		...
		if (CameraUserImpulseData->HasRotationalImpulse())
		{
			CameraController->GetConfig().bForceRotationalPhysics = true;
		}
		...
		bHasMovement = bHasMovement && (bIsITFInteractions || bIsTracking);
		...
		// However, don't move the actor if input is disabled: the only reason for transform or FOV changes to be reproduced on the actor
		// is if the user is piloting the actor directly or indirectly, which requires input.
		if ((bTransformDifferent || bFOVDifferent) && !bDisableInput)
		{
			// Something has changed!
			constexpr bool bInvalidateChildViews=true;

			// When flying the camera around the hit proxies don't need to be invalidated since we are flying around and not clicking on anything
			const bool bInvalidateHitProxies = bIsITFInteractions
...
bool FEditorViewportClient::ConsumeExternalCameraMovement()
{
	if (!Viewport || !ViewportNavigationHelper || !ViewportNavigationHelper->HasTransformDelta())
	{
		return false;
	}
	...
	ViewportNavigationHelper->ConsumeTransformDelta(Translation, Rotation, Orbit);
	
	BeginCameraMovement(!Translation.IsNearlyZero() || !Rotation.IsNearlyZero() || !Orbit.IsNearlyZero());
	...
	if (IsPerspective())
	{
		...
		if (!FMath::IsNearlyZero(Orbit.X) || !FMath::IsNearlyZero(Orbit.Y))
		{
            ...
            NewRotation.Yaw = Orbit.X;
            NewRotation.Pitch = Orbit.Y;

            OrbitPerspectiveCamera(NewRotation);
			bCameraHasMoved = true;
		}
		
		if (!FMath::IsNearlyZero(Orbit.Z))
		{
			DollyPerspectiveCamera(Orbit.Z);
			bCameraHasMoved = true;
		}
		
		if (!Translation.IsNearlyZero() || !Rotation.IsNearlyZero())
		{
			ToggleOrbitCamera(false);
			MoveViewportPerspectiveCamera(Translation * GetCameraSpeed(), Rotation);
			bCameraHasMoved = true;
		}
	...
	else
	{
		if (!FMath::IsNearlyZero(Orbit.Z))
		{
			float NewOrthoRatio = 1 + (FMath::Abs(Orbit.Z) / CAMERA_ZOOM_DAMPEN);
			if ( Orbit.Z < 0 )
			{
		...
		{
			SetViewLocation(GetViewLocation() + Translation);
		}
	}
...
void FEditorViewportClient::OrbitPerspectiveCamera(const FRotator& InOrbit)
{
	// This is pulled from InputAxisForOrbit(), with the input checking code removed
	if (GetDefault<ULevelEditorViewportSettings>()->bOrbitCameraAroundSelection)
	{
		FRotator NewRotation = InOrbit;
		
		FVector OrbitPoint;
		if (GetPivotForOrbit(OrbitPoint))
		{
		...
		{
			NewRotation.Yaw = -NewRotation.Yaw;
		}
		
		// Compute the delta rotation to apply as a transform
		FRotator DeltaRotation(NewRotation);
		...
		else
		{
			...
			// FVector::ToOrientationRotator uses an alternate computation that doesn't suffer from the singularity. However, the roll it returns
	...
	else
	{
		...
		SetViewRotation( GetViewRotation() + FRotator( InOrbit.Pitch, -InOrbit.Yaw, InOrbit.Roll ) );
...
void FEditorViewportClient::DollyPerspectiveCamera(double InDelta)
{
	ToggleOrbitCamera(true);
	...
	const FVector DeltaToTarget = LookAtLocation - ViewLocation;

	const float DistanceToTarget = DeltaToTarget.Length();
	const float CameraSpeed = GetCameraSpeed();
	const float CameraSpeedDistanceScale = ShouldScaleCameraSpeedByDistance() ? FMath::Min(DistanceToTarget / 1000.f, 1000.f) : 1.0f;
	const float DeltaLength = InDelta * CameraSpeed * CameraSpeedDistanceScale;

	// Make sure we don't move past the LookAt
	FVector DeltaLocation = FVector::UnitY() * FMath::Min(DeltaLength, DistanceToTarget);

	// Orient the delta down the view direction towards the look at
	FMatrix RotMat =
		FTranslationMatrix( -LookAtLocation ) *
...
void FEditorViewportClient::PeformDefaultCameraMovement(FVector& Drag, FRotator& Rot, FVector& Scale)
{
	...
	const bool bUseITFOrbitDelta = IsViewportInteractionEnabled(ModeTools.Get(), UE::Editor::ViewportInteractions::Orbit)
...
void FEditorViewportClient::InputAxisForOrbit(FViewport* InViewport, const FVector& DragDelta, FVector& Drag, FRotator& Rot)
{
	...
	else
	{
		...
		else if ( IsOrbitZoomMode( InViewport ) )
		{
			...
			const float DistanceToTarget = DeltaToTarget.Length();
			const float DeltaLength = bInvertY ? Drag.X + Drag.Y : Drag.X+ -Drag.Y;
...
void FEditorViewportClient::ChangeMegaLightsVisualizationMode(FName InName)
{
	SetViewMode(VMI_VisualizeMegaLights);
	CurrentMegaLightsVisualizationMode = InName;
}

bool FEditorViewportClient::IsMegaLightsVisualizationModeSelected(FName InName) const
{
	return IsViewModeEnabled(VMI_VisualizeMegaLights) && CurrentMegaLightsVisualizationMode == InName;
}

FText FEditorViewportClient::GetCurrentMegaLightsVisualizationModeDisplayName() const
{
	checkf(IsViewModeEnabled(VMI_VisualizeMegaLights), TEXT("In order to call GetCurrentMegaLightsVisualizationMode(), first you must set ViewMode to VMI_VisualizeMegaLights."));
	return GetMegaLightsVisualizationData().GetModeDisplayName(CurrentMegaLightsVisualizationMode);
}
...
bool FEditorViewportClient::Internal_InputKey(const FInputKeyEventArgs& EventArgs)
{
	...
	bool bRouteToToolsContext = !bIsTracking || ModeTools->UsesViewportITF();
	...
	const bool bEnableGizmos = UEditorInteractiveGizmoManager::AreEditorGizmosAllowed(ModeTools.Get());

	if ((IsOrtho() || InputState.IsAltButtonPressed()) && (Key == EKeys::Left || Key == EKeys::Right || Key == EKeys::Up || Key == EKeys::Down) && !bEnableGizmos)
	{
	...
	const bool bIsMultiMouseButtonInteractionEnabled = IsViewportInteractionEnabled(ModeTools.Get(), UE::Editor::ViewportInteractions::CameraDrag);

	// Make sure ITF viewport interactions dealing with multiple mouse buttons pressed at the same time DO NOT trigger StartTrackingDueToInput when a button is released
	if (!bIsMultiMouseButtonInteractionEnabled)
	{
	...
	const bool bIsITFSingleClickEnabled = IsViewportInteractionGroupEnabled(ModeTools.Get(), UE::Editor::ViewportInteractions::ViewportClick);
	...
	if ( bIsTracking && !InputState.IsAnyMouseButtonDown() && InputState.IsMouseButtonEvent() && !bIsITFSingleClickEnabled)
	{
...
void FEditorViewportClient::SetupViewForRendering(FSceneViewFamily& ViewFamily, FSceneView& View)
{
	...
	View.CurrentMegaLightsVisualizationMode = CurrentMegaLightsVisualizationMode;
...
void FEditorViewportClient::Draw(FViewport* InViewport, FCanvas* Canvas)
{
	...
	ViewFamily.ChannelMaskParams.ColorChannelMask = ChannelMaskParams.ColorChannelMask;
	ViewFamily.ChannelMaskParams.bInvertAlphaChannelMask = ChannelMaskParams.bInvertAlphaChannelMask;
	ViewFamily.ChannelMaskParams.bDrawAlphaBlendedCheckerboard = ChannelMaskParams.bDrawAlphaBlendedCheckerboard;
	...
	const bool bIsGeometryInspectionMode = CurrentViewMode == VMI_Clay || CurrentViewMode == VMI_Zebra || CurrentViewMode == VMI_FrontBackFace || CurrentViewMode == VMI_RandomColor;
	...
	if (bIsGeometryInspectionMode && bIsGeometryInspectionWireframeOn)
	{
		ViewFamily.EngineShowFlags.SetMeshEdges(true);
	}
	...
	// Setup final resolution multiplier used by system relying on the final pixel size. Only used by the high resolution screenshot
	if (GIsHighResScreenshot)
	{
		ViewFamily.FinalResolutionScale = GetHighResScreenshotConfig().ResolutionMultiplier;
	}
	...
	if (View)
	{
		...
		ModeTools->DrawHUD(this, Viewport, View, Canvas);
...
bool FEditorViewportClient::Internal_InputAxis(FViewport* InViewport, FInputDeviceId DeviceID, FKey Key, float Delta, float DeltaTime, int32 NumSamples, bool bGamepad)
{
	...
	static FName LightRotationViewportInteractionName("LightRotation");
	if( bLightMoveDown && bMouseButtonDown && PreviewScene && !IsViewportInteractionActive(ModeTools.Get(), LightRotationViewportInteractionName) )
	{
...
void FEditorViewportClient::AddAssociation(FViewport& InViewport)
{
	if (Viewport == nullptr)
	{
		Viewport = &InViewport;
...
void FEditorViewportClient::RemoveAssociation(FViewport& InViewport)
{
...
bool FEditorViewportClient::InputGesture(FViewport* InViewport, const FInputDeviceId DeviceId, EGestureEvent GestureType, EGesturePhase GesturePhase, const FVector2D& GestureDelta, bool bIsDirectionInvertedFromDevice, const uint64 Timestamp)
{
	if (bDisableInput)
	{
		return true;
...
bool FEditorViewportClient::IsFlightCameraInputModeActive() const
{
	if( (Viewport != nullptr ) && IsPerspective() )
	{
		if( CameraController != NULL )
		{
			if (ModeTools->UsesViewportITF())
			{
				return ViewportNavigationHelper->bIsMouseLooking;
			}
...
void FEditorViewportClient::UpdateRequiredCursorVisibility()
{
	...
	if (ModeTools->GetCursorVisibilityOverride(bOverrideCursorVisibility, bHardwareCursorVisible, bSoftwareCursorVisible))
	{
	...
	//if we're using the new move canvas mode, we're in an ortho viewport, and the mouse is down
	if (IsOrtho() && bMouseButtonDown && !MouseDeltaTracker->UsingDragTool() && !ModeTools->UsesViewportITF())
	{
...
void FEditorViewportClient::TranslateViewportCamera(const FVector& InTranslation)
{
	SetViewLocation(GetViewLocation() + InTranslation);
	
	if (IsPerspective())
	{
		SetLookAtLocation(GetLookAtLocation() + InTranslation);	
		PerspectiveCameraMoved();
	}
	else if (IsOrtho())
	{
...
void FEditorViewportClient::ToggleGeometryInspectionShowWireframe()
{
	bIsGeometryInspectionWireframeOn = !bIsGeometryInspectionWireframeOn;
	Invalidate();
}

bool FEditorViewportClient::IsSetGeometryInspectionShowWireframe() const
{
	return bIsGeometryInspectionWireframeOn;
}

void FEditorViewportClient::SetChannelMask(EColorChannelMask InColorChannelMask)
{
	// If the color channel mask is already set to the specified mask, toggle it off by setting the mask to All
	if (ChannelMaskParams.ColorChannelMask == InColorChannelMask)
	{
		ChannelMaskParams.ColorChannelMask = EColorChannelMask::All;
	}
	else
	{
		ChannelMaskParams.ColorChannelMask = InColorChannelMask;
	}
	
	Invalidate();
}

bool FEditorViewportClient::IsChannelBeingMasked(EColorChannelMask InColorChannelMask) const
{
	return ChannelMaskParams.ColorChannelMask == InColorChannelMask;
}
...
void FEditorViewportClient::SetGeometryInspectionViewMode(EViewModeIndex ViewModeIndex)
{
	static const IConsoleVariable* ICNaniteVar = IConsoleManager::Get().FindConsoleVariable(TEXT("r.Nanite"));
	
	const bool bNanitesEnabled = (ICNaniteVar == nullptr || ICNaniteVar->GetBool());

	if (bNanitesEnabled)
	{
		const FText WarningText(LOCTEXT("GeometryInspectionViewModesDontWorkWithNanite", "Geometry Inspection view modes only display non-Nanite geometry."));
		UE_LOGF(LogEditorModes, Warning, "%ls", *WarningText.ToString());

		FNotificationInfo Info(WarningText);
		Info.Image = FAppStyle::GetBrush("Icons.WarningWithColor");
		Info.ExpireDuration = 5.0f;
		Info.SubText = LOCTEXT("GeometryInspectionViewModesDontWorkWithNaniteSubText", "View fallback meshes by disabling Nanite in the Details panel (Rendering -> Disallow Nanite).");
...
void FEditorViewportClient::SetZebraViewMode(bool bHorizontalZebraMode)
{
	...
	SetGeometryInspectionViewMode(VMI_Zebra);
}
...
{
	if (InStripeCount>0 && InStripeCount<=64)
	{
		if (UMaterialInstanceDynamic* MaterialInstance = Cast<UMaterialInstanceDynamic>(GEngine->ZebraMaterialInstance))
		{
...
void FEditorViewportClient::CapturedMouseMove( FViewport* InViewport, int32 InMouseX, int32 InMouseY )
{
	...
	if (ModeTools->CapturedMouseMove(this, InViewport, InMouseX, InMouseY))
	{
		...
		if (Viewport->IsSoftwareCursorVisible())
		{
			Viewport->SetSoftwareCursorPosition(FVector2D(InMouseX, InMouseY));
		}
		return;
	}
...
void FEditorViewportClient::TakeScreenshot(FViewport* InViewport, bool bInValidatViewport)
{
	...
	if ( !IFileManager::Get().MakeDirectory(*Directory, true ) )
	{
		...
		UE_LOGF(LogEditorViewport, Warning, "Failed to create directory %ls", *FPaths::ConvertRelativePathToFull(Directory));
...
void FEditorViewportClient::DrawBoundingBox(FBox &Box, FCanvas* InCanvas, const FSceneView* InView, const FViewport* InViewport, const FLinearColor& InColor, const bool bInDrawBracket, const FString &InLabelText)
{
	...
	if( ProjBoxCenter.W > 0.f )
	{
		...
		ScreenBoxMax/=InCanvas->GetDPIScale();
		ScreenBoxMin/=InCanvas->GetDPIScale();
...
void FEditorViewportClient::OnObjectTransacted(UObject* Object, const FTransactionObjectEvent& Event)
{
	...
	// Skip CDOs - calling GetWorld() on a CDO with a Within specifier triggers an ensure.
	...
	if (World && (Object->GetWorld() == World) && !World->IsGameWorld())
	{
...
{
	if (GWorld == GetWorld() && !GWorld->IsGameWorld())
	{
		CachedWorldBounds.Init();
