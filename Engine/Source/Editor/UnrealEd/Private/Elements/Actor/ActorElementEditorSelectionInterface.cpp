...
private:
	...
	virtual void SetElementImpl(const FTypedElementHandle& InElementHandle) override
	{
		...
		if (!Actor)
		{
			UE_LOGF(LogTemp, Warning, "FActorElementTransactedElement::SetElementImpl: Failed to resolve actor from element handle");
		}
		ActorPtr = Actor;
	}
...
bool UActorElementEditorSelectionInterface::SelectElement(const FTypedElementHandle& InElementHandle, const FTypedElementListPtr& InSelectionSet, const FTypedElementSelectionOptions& InSelectionOptions)
{
	...
	if (const ICompatibilityProvider* Compatibility = GetDataStorageFeature<ICompatibilityProvider>(CompatibilityFeatureName))
	{
		...
		if (Row != InvalidRowHandle)
		{
			...
			{
				DataStorage->AddColumn(Row, FTypedElementSelectionColumn{ .SelectionSet = InSelectionOptions.GetNameForTEDSIntegration() });
			}
		}
	}
