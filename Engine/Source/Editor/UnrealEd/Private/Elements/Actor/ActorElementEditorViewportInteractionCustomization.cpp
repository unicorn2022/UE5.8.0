...
{
	AActor* RootActor = ActorElementDataUtil::GetActorFromHandle(InElementWorldHandle);
	if (!RootActor)
	{
		UE_LOGF(LogTemp, Warning, "FActorElementEditorViewportInteractionCustomization::GizmoManipulationStarted: Failed to resolve actor from element handle");
		return;
	}
...
void FActorElementEditorViewportInteractionCustomization::GizmoManipulationDeltaUpdate(const TTypedElement<ITypedElementWorldInterface>& InElementWorldHandle, const UE::Widget::EWidgetMode InWidgetMode, const EAxisList::Type InDragAxis, const FInputDeviceState& InInputState, const FTransform& InDeltaTransform, const FVector& InPivotLocation)
{
	...
	if (!RootActor)
	{
		UE_LOGF(LogTemp, Warning, "FActorElementEditorViewportInteractionCustomization::GizmoManipulationDeltaUpdate: Failed to resolve actor from element handle");
		return;
	}
	...
	const ETypedElementViewportInteractionGizmoManipulationType InManipulationType)
{
	...
	if (!RootActor)
	{
		UE_LOGF(LogTemp, Warning, "FActorElementEditorViewportInteractionCustomization::GizmoManipulationStopped: Failed to resolve actor from element handle");
		return;
	}
...
void FActorElementEditorViewportInteractionCustomization::MirrorElement(const TTypedElement<ITypedElementWorldInterface>& InElementWorldHandle, const FVector& InMirrorScale, const FVector& InPivotLocation)
{
	...
	if (!Actor)
	{
		UE_LOGF(LogTemp, Warning, "FActorElementEditorViewportInteractionCustomization::MirrorElement: Failed to resolve actor from element handle");
		return;
	}
...
bool FActorElementEditorViewportInteractionCustomization::GetFocusBounds( const TTypedElement<ITypedElementWorldInterface>& InElementWorldHandle, FBoxSphereBounds& OutBounds)
{
		...
		for (USceneComponent* SceneComponent : AllSceneComponents)
		{
			...
			if (PrimitiveComponent && PrimitiveComponent->IsRegistered())
			{
				...
				if (!bIgnore)
				{
					...
					// and are not yet reflected in PrimitiveComponent::Bounds. Their real bounds have to be computed via an async GPU readback.
					// We still mark bHasAtLeastOnePrimitiveComponent so the root-location fallback below does not fire for actors that are all-deformer.
#if WITH_EDITORONLY_DATA
					const USkinnedMeshComponent* SkinnedMeshComponent =  Cast<USkinnedMeshComponent>(PrimitiveComponent);
	...
	return static_cast<bool>(BoundingBox.IsValid);
}
