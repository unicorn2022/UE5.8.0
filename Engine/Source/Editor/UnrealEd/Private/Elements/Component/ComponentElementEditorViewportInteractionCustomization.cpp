...
{
	const UActorComponent* Component = ComponentElementDataUtil::GetComponentFromHandle(InElementWorldHandle);
	if (!Component)
	{
		UE_LOGF(LogTemp, Warning, "FComponentElementEditorViewportInteractionCustomization::GetGizmoPivotLocation: Failed to resolve component from element handle");
		return false;
	}
...
void FComponentElementEditorViewportInteractionCustomization::GizmoManipulationDeltaUpdate(const TTypedElement<ITypedElementWorldInterface>& InElementWorldHandle, const UE::Widget::EWidgetMode InWidgetMode, const EAxisList::Type InDragAxis, const FInputDeviceState& InInputState, const FTransform& InDeltaTransform, const FVector& InPivotLocation)
{
	...
	if (!Component)
	{
		UE_LOGF(LogTemp, Warning, "FComponentElementEditorViewportInteractionCustomization::GizmoManipulationDeltaUpdate: Failed to resolve component from element handle");
		return;
	}
...
bool FComponentElementEditorViewportInteractionCustomization::GetScaleForGizmo(const TTypedElement<ITypedElementWorldInterface>& InElementWorldHandle, FVector& OutScale)
{
	if (UActorComponent* Component = ComponentElementDataUtil::GetComponentFromHandle(InElementWorldHandle))
	{
		if (const UPrimitiveComponent* PrimitiveComponent = Cast<UPrimitiveComponent>(Component))
		{
			...
			OutScale = PrimitiveComponent->GetRelativeScale3D();
			return true;
		}
	}
