...
public:
	 ...
	 */
	bool SnapDraggedActorsToNearestVertex( FVector& DragDelta, TNotNull<FEditorViewportClient*> ViewportClient );
	 ...
	 * @return true if anything was snapped
	 */
	bool SnapDragLocationToNearestVertex( const FVector& BaseLocation, FVector& DragDelta, TNotNull<FEditorViewportClient*> ViewportClient, bool bIsPivot = false );
	 ...
	 * @return true if anything was snapped
	 */
	bool SnapLocationToNearestVertex( FVector& Location, const FVector2D& MouseLocation, TNotNull<FEditorViewportClient*> ViewportClient, FVector& OutVertexNormal, bool bDrawVertHelpers, const TSet<TWeakObjectPtr<AActor>>& ActorsToIgnore );

	bool SnapToBSPVertex( FVector& Location, FVector GridBase, FRotator& Rotation );

private:
	bool IsSnapRotationEnabledInternal() const;
	bool IsSnapScaleEnabledInternal() const;

	/**
	 * @return true if snapping to vertices is enabled
	 */
	bool IsSnapToVertexEnabledInternal(bool bIsPivot = false) const;

	/**
	 * @return true if snapping actors to other actors is enabled
	 */
	bool IsSnapToActorEnabledInternal() const;
...
{
	return IsSnappingActive() && GetDefault<ULevelEditorViewportSettings>()->GridEnabled && !IsSnapToVertexEnabledInternal();
}

bool FEditorViewportSnapping::IsSnapRotationEnabled()
{	
	return IsSnapRotationEnabledInternal();
}

bool FEditorViewportSnapping::IsSnapScaleEnabled()
{
	return IsSnapScaleEnabledInternal();
}

bool FEditorViewportSnapping::IsSnapRotationEnabledInternal() const
{
	...
	return GLevelEditorModeTools().IsSnapRotationEnabled() || (Settings->SnapToSurface.bEnabled && Settings->SnapToSurface.bSnapRotation);
}

bool FEditorViewportSnapping::IsSnapScaleEnabledInternal() const
{
	return IsSnappingActive() && GetDefault<ULevelEditorViewportSettings>()->SnapScaleEnabled;
}

bool FEditorViewportSnapping::IsSnapToVertexEnabled(bool bIsPivot)
{
	return IsSnapToVertexEnabledInternal(bIsPivot);
}

bool FEditorViewportSnapping::IsSnapToVertexEnabledInternal(bool bIsPivot) const
{
	if (!IsSnappingActive())
	{
		return false;
...
{
	return IsSnapToActorEnabledInternal();
}

bool FEditorViewportSnapping::IsSnapToActorEnabledInternal() const
{
	return IsSnappingActive() && (GetDefault<ULevelEditorViewportSettings>()->bEnableActorSnap && !IsSnapToVertexEnabledInternal());
}
...
bool FEditorViewportSnapping::IsSnappingActive() const
{
	const ULevelEditorViewportSettings* ViewportSettings = GetDefault<ULevelEditorViewportSettings>();
	return ViewportSettings->bSnappingActive;
}

bool FEditorViewportSnapping::IsTranslationSnappingEnabled() const
{
	const ULevelEditorViewportSettings* Settings = GetDefault<ULevelEditorViewportSettings>();
	return Settings->GridEnabled || Settings->SnapToSurface.bEnabled || IsSnapToVertexEnabledInternal() || IsSnapToActorEnabledInternal();
}

bool FEditorViewportSnapping::IsRotationSnappingEnabled() const
{
	return IsSnapRotationEnabledInternal();
}

bool FEditorViewportSnapping::IsScaleSnappingEnabled() const
{
	return IsSnapScaleEnabledInternal();
}

bool FEditorViewportSnapping::SnapLocationToNearestVertex( FVector& Location, const FVector2D& MouseLocation, TNotNull<FEditorViewportClient*> ViewportClient, FVector& OutVertexNormal, bool bDrawVertHelpers, const TSet<TWeakObjectPtr<AActor>>& ActorsToIgnore )
{
	bool bSnapped = false;
	if( IsSnapToVertexEnabled() )
	{
		bSnapped = VertexSnappingImpl.SnapLocationToNearestVertex( Location, MouseLocation, ViewportClient, OutVertexNormal, bDrawVertHelpers, ActorsToIgnore );
	}
	else
	{
...
bool FEditorViewportSnapping::SnapDraggedActorsToNearestVertex( FVector& DragDelta, TNotNull<FEditorViewportClient*> ViewportClient )
{
	bool bSnapped = false;
...
bool FEditorViewportSnapping::SnapDragLocationToNearestVertex( const FVector& BaseLocation, FVector& DragDelta, TNotNull<FEditorViewportClient*> ViewportClient, bool bIsPivot )
{
	bool bSnapped = false;
...
bool FSnappingUtils::IsSnappingActive()
{
	return EditorViewportSnapper.IsValid() && EditorViewportSnapper->IsSnappingActive();
}
...
bool FSnappingUtils::IsTranslationSnappingEnabled(const bool bInRequireSnappingActive)
{
	return EditorViewportSnapper.IsValid()
		&& (!bInRequireSnappingActive || EditorViewportSnapper->IsSnappingActive()) 
		&& EditorViewportSnapper->IsTranslationSnappingEnabled();
}
...
bool FSnappingUtils::IsRotationSnappingEnabled(const bool bInRequireSnappingActive)
{
	return EditorViewportSnapper.IsValid() 
		&& (!bInRequireSnappingActive || EditorViewportSnapper->IsSnappingActive()) 
		&& EditorViewportSnapper->IsRotationSnappingEnabled();
}
...
bool FSnappingUtils::IsScaleSnappingEnabled(const bool bInRequireSnappingActive)
{
	return EditorViewportSnapper.IsValid()
		&& (!bInRequireSnappingActive || EditorViewportSnapper->IsSnappingActive()) 
		&& EditorViewportSnapper->IsScaleSnappingEnabled();
}
...
bool FSnappingUtils::SnapDraggedActorsToNearestVertex(FVector& DragDelta, FEditorViewportClient* ViewportClient)
{
...
bool FSnappingUtils::SnapDragLocationToNearestVertex(const FVector& BaseLocation, FVector& DragDelta, FEditorViewportClient* ViewportClient, bool bIsPivot)
{
...
bool FSnappingUtils::SnapLocationToNearestVertex(FVector& Location, const FVector2D& MouseLocation, FEditorViewportClient* ViewportClient, FVector& OutVertexNormal, bool bDrawVertHelpers, const TSet<TWeakObjectPtr<AActor>>& ActorsToIgnore )
{
	return EditorViewportSnapper.IsValid() ? EditorViewportSnapper->SnapLocationToNearestVertex(Location, MouseLocation, ViewportClient, OutVertexNormal, bDrawVertHelpers, ActorsToIgnore) : false;
}
...
bool FSnappingUtils::SnapLocationToNearestVertex(FVector& Location, const FVector2D& MouseLocation, FLevelEditorViewportClient* ViewportClient, FVector& OutVertexNormal, bool bDrawVertHelpers )
{
	...
	if (const TSharedPtr<FTypedElementList>& Elements = ViewportClient->GetDropPreviewElements())
	{
		Elements->ForEachElementHandle([&ActorsToIgnore](const FTypedElementHandle& Element)
			{
				if (AActor* Actor = ActorElementDataUtil::GetActorFromHandle(Element, true))
				{
	...
	return SnapLocationToNearestVertex(Location, MouseLocation, ViewportClient, OutVertexNormal, bDrawVertHelpers, MoveTemp(ActorsToIgnore));
}
