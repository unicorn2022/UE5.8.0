...
FLinearColor UMaterialGraphSchema::GetColorForConnectionType(UE::Shader::EValueType ConnectionType)
{
...
FLinearColor UMaterialGraphSchema::GetPinColor(const UEdGraphPin* InPin) const
{
	if (InPin)
	{
		if (const UMaterialShaderValueTypeObject* ValueTypeObj = Cast<UMaterialShaderValueTypeObject>(InPin->PinType.PinSubCategoryObject.Get()))
		{
			return GetColorForConnectionType(ValueTypeObj->ValueType);
		}
...
void UMaterialGraphSchema::DroppedAssetsOnNode(const TArray<struct FAssetData>& Assets, const FVector2f& GraphPosition, UEdGraphNode* Node) const
{
	const UMaterialGraphNode* MaterialGraphNode = Cast<UMaterialGraphNode>(Node);
	if (!MaterialGraphNode || !MaterialGraphNode->MaterialExpression)
	{
		return;
	}
	UMaterialExpression* Expression = MaterialGraphNode->MaterialExpression;

	TSharedPtr<IAssetReferenceFilter> AssetReferenceFilter = FMaterialEditorUtilities::MakeMaterialAssetReferenceFilter(MaterialGraphNode->GetGraph());

	// Drop a texture onto a TextureSample node -> replace texture
	if (UMaterialExpressionTextureSample* TextureSampleNode = Cast<UMaterialExpressionTextureSample>(Expression))
	{
		if (Assets.Num() < 1 || (AssetReferenceFilter.IsValid() && !AssetReferenceFilter->PassesFilter(Assets[0])))
		{
			return;
		}

		if (UTexture* Tex = Cast<UTexture>(Assets[0].GetAsset()))
		{
			const FScopedTransaction Transaction(LOCTEXT("MaterialEditorReplaceTexture", "Material Editor: Replace texture"));
			TextureSampleNode->Modify();
			TextureSampleNode->Texture = Tex;
			TextureSampleNode->AutoSetSampleType();

			FMaterialEditorUtilities::ForceRefreshExpressionPreviews(Node->GetGraph());
		}
	}
}
	...
	for (const FAssetData& AssetData : AssetDataList)
	{
		...
		if ( bExposeToLibrary )
		{
			...
			if (!ActionMenuBuilder.FromPin || HasCompatibleConnection(AssetData, FromPinType, ActionMenuBuilder.FromPin->Direction))
			{
				...
				const FText AssetPath = FText::FromString(AssetData.PackagePath.ToString().RightChop(1));
				const FText Description = AssetData.GetTagValueRef<FText>("Description");
				const FText Tooltip = FText::Format(LOCTEXT("MaterialFunctionActionTooltip", "Location: {0}\n{1}"), AssetPath, Description);
...
void UMaterialGraphSchema::GetNamedRerouteActionsForGraph(FGraphActionListBuilderBase& OutAllActions, const UEdGraph* CurrentGraph)
{
	if (CurrentGraph)
	{
...
{
	GetNamedRerouteActionsForGraph(ActionMenuBuilder, CurrentGraph);
}
...
void UMaterialGraphSchema::GetAssetsNodeHoverMessage(const TArray<struct FAssetData>& Assets, const UEdGraphNode* HoverNode, FString& OutTooltipText, bool& OutOkIcon) const
{
	OutTooltipText = FString();
	OutOkIcon = false;

	const UMaterialGraphNode* MaterialGraphNode = Cast<UMaterialGraphNode>(HoverNode);
	if (!MaterialGraphNode || !MaterialGraphNode->MaterialExpression)
	{
		return;
	}
	...
	if (UMaterialExpressionTextureSample* TextureSampleNode = Cast<UMaterialExpressionTextureSample>(Expression))
	{
		...
		if (UTexture* Tex = Cast<UTexture>(AssetData.GetAsset()))
		{
			TSharedPtr<IAssetReferenceFilter> AssetReferenceFilter = FMaterialEditorUtilities::MakeMaterialAssetReferenceFilter(MaterialGraphNode->GetGraph());
