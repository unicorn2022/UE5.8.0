...
void FTransaction::FObjectRecord::SerializeContents( FArchive& Ar, int32 InOper ) const
{
	if( Array )
	{
		...
		//UE_LOGF( LogEditorTransaction, Log, "Array %ls %i*%i: %i", CurrentObject ? *CurrentObject->GetFullName() : TEXT("Invalid Object"), Index, ElementSize, InOper);
	...
	{
		//UE_LOGF(LogEditorTransaction, Log,  "Object %ls", *Object.Get()->GetFullName());
		check(Index==0);
		check(ElementSize==0);
		check(DefaultConstructor==NULL);
		check(Serializer==NULL);
...
void UTransBuffer::Initialize(SIZE_T InMaxMemory)
{
	...
	UE_LOGF(LogInit, Log, "Transaction tracking system initialized" );
}
...
void UTransBuffer::FinishDestroy()
{
	...
	{
		CheckState();
		UE_LOGF(LogExit, Log, "Transaction tracking system shut down" );
	}
	Super::FinishDestroy();
}
...
void UTransBuffer::OnObjectsReinstanced(const TMap<UObject*, UObject*>& OldToNewInstances)
{
	...
	// Reinstantiation coming from the loading thread only concerns objects still being loaded and
	// shouldn't be referenced in any way by an existing transaction.
...
void UTransBuffer::Reset( const FText& Reason )
{
	if (ensure(!GIsTransacting))
	{
		...
		if (ActiveCount != 0)
		{
			...
			UE_LOGF(LogEditorTransaction, Log, "%ls", *ErrorMessage);
...
void UTransBuffer::Cancel( int32 StartIndex /*=0*/ )
{
	...
	if ( ActiveCount > 0 )
	{
		...
		if (StartIndex != 0)
		{
			...
			UE_LOGF(LogEditorTransaction, Warning, "Canceling transaction partially is unsupported. Canceling %ls entirely.", *TransactionTitle);
			StartIndex = 0;
		}
...
bool UTransBuffer::Undo(bool bCanRedo)
{
	...
	do
	{
		...
		{
			UE_LOGF(LogEditorTransaction, Log, "Undo %ls", *Transaction.GetTitle().ToString());
...
bool UTransBuffer::Redo()
{
	...
	do
	{
		...
		if (Transaction.HasExpired() == false)
		{
			UE_LOGF(LogEditorTransaction, Log, "Redo %ls", *Transaction.GetTitle().ToString());
			CurrentTransaction = &Transaction;
			CurrentTransaction->BeginOperation();
