...
bool EditorScriptingHelpers::CheckIfInEditorAndPIE()
{
	if (!IsInGameThread())
	{
		UE_LOGF(LogUtils, Error, "You are not on the main thread.");
		return false;
	}
	if (!GIsEditor)
	{
		UE_LOGF(LogUtils, Error, "You are not in the Editor.");
		return false;
	}
	if (GEditor->PlayWorld || GIsPlayInEditorWorld)
	{
		UE_LOGF(LogUtils, Error, "The Editor is currently in a play mode.");
		return false;
	}
	return true;
}
