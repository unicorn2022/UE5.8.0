...
const TCHAR* GetFVECTORSpaceDelimited( const TCHAR* Stream, FVector& Value )
{
	...
	//UE_LOGF(LogParamParser, Warning, "Value.X %f", Value.X );
	Stream = FCString::Strchr(Stream,' ');
	if( !Stream )
	{
		return Stream;
	...
	//UE_LOGF(LogParamParser, Warning, "Value.Y %f", Value.Y );
	Stream = FCString::Strchr(Stream,' ');
	if( !Stream )
	{
		return Stream;
	...
	//UE_LOGF(LogParamParser, Warning, "Value.Z %f", Value.Z );
...
)
{
	...
	Rotation = FRotator::ZeroRotator;
	Rotation.Pitch = FCString::Atof(Stream) * ScaleFactor;
	//UE_LOGF(LogParamParser, Warning, "Rotation.Pitch %d", Rotation.Pitch );
	Stream = FCString::Strchr(Stream,' ');
	if( !Stream )
	{
		return Stream;
	...
	Rotation.Yaw = FCString::Atof(++Stream) * ScaleFactor;
	//UE_LOGF(LogParamParser, Warning, "Rotation.Yaw %d", Rotation.Yaw );
	Stream = FCString::Strchr(Stream,' ');
	if( !Stream )
	{
		return Stream;
	...
	Rotation.Roll = FCString::Atof(++Stream) * ScaleFactor;
	//UE_LOGF(LogParamParser, Warning, "Rotation.Roll %d", Rotation.Roll );
