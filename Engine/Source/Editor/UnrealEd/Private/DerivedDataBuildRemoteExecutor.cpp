...
/**
 * This implements a remote execution interface for Derived Data Build
 * using Zen Structured Compute interfaces to get work done. 
 
 * All build actions are typically submitted to the same Zen server which 
 * is responsible for build outputs (Project Store) and local DDC. This Zen 
 * server is responsible for dispatching the work elsewhere but can also 
 * execute it locally when appropriate.
 * 
 * The current implementation uses the "legacy" REST API for simplicity,
 * but should be upgraded to use the new queue-oriented API as soon
 * as possible since that is more efficient and more flexible.
 * 
 */
...
public:
	...
	, bEnabled(false)
	{
		...
		GConfig->GetBool(TEXT("DerivedDataBuildRemoteExecutor"), TEXT("bContentNegotiationEnabled"), bContentNegotiationEnabled, GEngineIni);
	...
	{
		static constexpr FUtf8StringView HostPlatforms[]{UTF8TEXTVIEW("Win64"), UTF8TEXTVIEW("Linux"), UTF8TEXTVIEW("Mac")};
	...
	bool IsContentNegotiationEnabled() const
	{
		return bContentNegotiationEnabled;
	}
...
private:
	struct FStats
	{
		...
		void Dump()
		{
			UE_LOGF(LogDerivedDataBuildRemoteExecutor, Display, "");
			UE_LOGF(LogDerivedDataBuildRemoteExecutor, Display, "DDC Remote Execution Stats");
			UE_LOGF(LogDerivedDataBuildRemoteExecutor, Display, "==========================");
	...
	public:
		...
		bool TryStartNewBuild(FStats& InStats)
		{
			...
			if ((InStats.TotalRemoteBuilds.fetch_add(1, std::memory_order_relaxed) >= MaxTotalRemoteBuilds))
			{
				InStats.TotalRemoteBuilds.fetch_sub(1, std::memory_order_relaxed);
				return false;
			}

			if ((InStats.InFlightRemoteBuilds.fetch_add(1, std::memory_order_relaxed) >= MaxInFlightRemoteBuilds))
			{
				InStats.TotalRemoteBuilds.fetch_sub(1, std::memory_order_relaxed);
	...
	bool bEnabled;
	bool bContentNegotiationEnabled = false;
...
FCbObject FRemoteBuildExecutionRequest::BuildWorkerDescriptor(const FBuildWorker& Worker, const int TimeoutSeconds)
{
	...
	Worker.IterateEnvironment([&WorkerDescriptor](FUtf8StringView Name, FUtf8StringView Value)
		{
			WorkerDescriptor.AddString(WriteToUtf8String<256>(Name, "=", Value));
		});
	WorkerDescriptor.EndArray();

	WorkerDescriptor.BeginArray(ANSITEXTVIEW("executables"));
	Worker.IterateExecutables([&WorkerDescriptor](FUtf8StringView Key, const FIoHash& RawHash, uint64 RawSize)
		{
			WorkerDescriptor.BeginObject();
	...
	Worker.IterateFiles([&WorkerDescriptor](FUtf8StringView Key, const FIoHash& RawHash, uint64 RawSize)
		{
			WorkerDescriptor.BeginObject();
			WorkerDescriptor.AddString(ANSITEXTVIEW("name"), Key);
			WorkerDescriptor.AddBinaryAttachment(ANSITEXTVIEW("hash"), RawHash);
...
bool FRemoteBuildExecutionRequest::IsResultOk(const UE::Zen::FZenHttpRequest::Result& Result, const TCHAR* OperationDesc)
{
	...
	{
		UE_LOGF(LogDerivedDataBuildRemoteExecutor, Warning, "Remote execution system error: operation '%ls' produced a failed result!", OperationDesc);
...
bool FRemoteBuildExecutionRequest::IsResponseOk(const int ResponseCode, const TCHAR* OperationDesc)
{
	...
	{
		UE_LOGF(LogDerivedDataBuildRemoteExecutor, Warning, "Remote execution system error: operation '%ls' produced an error result (%d)!", OperationDesc, ResponseCode);
	...
	UE::Tasks::Launch(TEXT("FRemoteBuildExecutionRequest::GetWorkerExists"), [this]
		{
			...
			WorkerUri.Append("/compute/workers/");
	...
	UE::Tasks::Launch(TEXT("FRemoteBuildExecutionRequest::PostWorkerPackage"), [this]
		{
...

			{
				...
				State.BuildWorker.IterateExecutables([NeedHashes = State.NeedHashes, &WorkerFileHashes](FUtf8StringView Path, const FIoHash& RawHash, uint64 RawSize)
					{
				...
				State.BuildWorker.IterateFiles([NeedHashes = State.NeedHashes, &WorkerFileHashes](FUtf8StringView Path, const FIoHash& RawHash, uint64 RawSize)
					{
						if (NeedHashes.Contains(RawHash))
						{
							WorkerFileHashes.Emplace(RawHash);
				...
				{
					UE_LOGF(LogDerivedDataBuildRemoteExecutor, Warning, "Remote execution system error: build input file missing!");
				...
				State.BuildWorker.Resolve(WorkerFileHashes.Array(), BlockingOwner, [&Package, &AttachmentBytes](FBuildWorkerResolvedParams&& Params)
					{
			...
			Executor.Stats.TotalWorkerPackagesUploaded.AddBlob(AttachmentBytes + State.WorkerDescriptor.GetSize());
...
void FRemoteBuildExecutionRequest::PrepareActionForPost()
{
	if (!State.Action) 
	{
...
void FRemoteBuildExecutionRequest::PostActionObject_Async()
{
	UE::Tasks::Launch(TEXT("FRemoteBuildExecutionRequest::PostActionObject"), [this]
		{
			PrepareActionForPost();

			TStringBuilder<128> ActionUri;
			ActionUri.Append("/compute/jobs/");
			ActionUri << State.WorkerDescriptionId;
			ActionUri << "?priority=" << (uint8)State.Owner.GetPriority();
	...
	UE::Tasks::Launch(TEXT("FRemoteBuildExecutionRequest::PostActionPackage"), [this]
		{
			...
			// It's valid for NeedHashes to be empty, in the case we go ahead and skip
			// the initial "needs" probe because the inputs are very small so we may
			// as well go ahead and send all of them at once without prior negotiation.
			...
			State.BuildInputs.Get().IterateInputs([NeedHashes = State.NeedHashes, &ActionPackage, &AttachmentBytes](FUtf8StringView Key, const FCompressedBuffer& Buffer)
				{
					if (NeedHashes.IsEmpty() || NeedHashes.Contains(Buffer.GetRawHash()))
					{
			...
			if (!State.NeedHashes.IsEmpty() && (State.NeedHashes.Num() != ActionPackage.GetAttachments().Num()))
			{
				UE_LOGF(LogDerivedDataBuildRemoteExecutor, Warning, "Remote execution system error: build input attachment missing!");
				State.Result = UE::Zen::FZenHttpRequest::Result::Failed;
				State.ResponseCode = 0;
	...
	UE::Tasks::Launch(TEXT("FRemoteBuildExecutionRequest::GetResultPackage"), [this]
		{
			...
			else if (State.ResponseCode == 404)
			{
				...
				UE_LOGF(LogDerivedDataBuildRemoteExecutor, Warning, "result package missing (uri: %ls)", *JobGetUri);
			}
...
void FRemoteBuildExecutionRequest::OnWorkerExistsDetermined(const UE::Zen::FZenHttpRequest::Result& Result)
{
	...
	if (Executor.IsContentNegotiationEnabled())
	{
		PostActionObject_Async();
	}
	else
	{
		// Skip the content negotiation and just post the action if
		// the inputs are small enough or if we just know that it's
		// very unlikely that the remote end has the content.
...
void FRemoteBuildExecutionRequest::OnPostActionObjectComplete(const UE::Zen::FZenHttpRequest::Result& Result)
{
	...
	if (!IsResponseOk(State.ResponseCode, TEXT("PostActionObject")))
	{
		return;
	}
...
void FRemoteBuildExecutionRequest::OnGetResultPackageComplete(const UE::Zen::FZenHttpRequest::Result& Result)
{
	...
	if (!IsResultOk(Result, TEXT("GetResultPackage")))
	{
		return;
	}

	if (State.ResponseCode == 202 /* Accepted */)
	{
		// Result is not yet available but the job is pending/running
		QueueGetResultPackage();
		return;
	}

	if (!IsResponseOk(State.ResponseCode, TEXT("GetResultPackage")))
	{
		return;
	}
	...
	{
		UE_LOGF(LogDerivedDataBuildRemoteExecutor, Warning, "Remote execution system error: build output blob missing!");
		State.Owner.End(this, [this]
			{
				CompletionCallback({ State.BuildAction.GetKey(), {}, {}, EStatus::Error });
				CompletionEvent->Trigger();
	...
	for (const FValueWithId& Value : RemoteBuildOutput.Get().GetValues())
	{
		...
		{
			UE_LOGF(LogDerivedDataBuildRemoteExecutor, Warning, "Remote execution system error: payload blob missing!");
			State.Owner.End(this, [this]
				{
					CompletionCallback({ State.BuildAction.GetKey(), {}, {}, EStatus::Error });
					CompletionEvent->Trigger();
