...
void UCookOnTheFlyServer::SaveCookedPackage(UE::Cook::FSaveCookedPackageContext& Context)
{
	...
	TInheritedGuardValueAccessors<GetOutputCookingWarnings, SetOutputCookingWarnings>
		ScopedOutputCookerWarnings(IsCookFlagSet(ECookInitializationFlags::OutputVerboseCookerWarnings));
	// SavePackage can CollectGarbage, so we need to store the currently-unqueued PackageData in a separate variable
	// that we register for garbage collection
	...
	// For legacy reasons we set GIsCookerLoadingPackage == true during save. Some classes use it to conditionally
	// execute cook operations in both save and load
		...
		if (Context.bPlatformSetupSuccessful)
		{
			...
			for (;;)
			{
...
#endif
				{
					...
					UE::Cook::FScopedCookActivePackage ScopedActivePackage(Context.PackageData.GetPackageName(), NAME_None);
					if (bSkipSave)
					{
				...
				{
					UE_LOGF(LogCook, Warning, "Tried to save package %ls for target platform %ls but threw an exception",
	...
	UE_CLOGF(SaveDurationSeconds >= SavePackageMinDurationLogTimeSeconds,
		LogCook, Display, "SavePackagePerformance: Package %ls took %.0fs to save.",
...
void FSaveCookedPackageContext::SetupPackage()
{
	...
	if (CommitType == EReachability::Runtime)
	{
		...
		{
			UE_LOGF(LogCook, Warning, "Package %ls marked as reloading for cook was requested to save", *PackageName);
			UE_LOGF(LogCook, Fatal, "Package %ls marked as reloading for cook was requested to save", *PackageName);
		}
...
void FSaveCookedPackageContext::SetupPlatform(const ITargetPlatform* InTargetPlatform, int32 InPlatformIndex)
{
	...
	if (CommitType == EReachability::Runtime)
	{
		...
		if (GenerationHelper && !GenerationHelper->IsSaveGenerator())
		{
			...
			{
				UE_LOGF(LogCook, Display, "Cooking %ls, Instigator: { %ls } -> Rejected %ls", *PackageName,
					*(PackageData.GetInstigator(EReachability::Runtime).ToString()), RejectedReason);
			}
			else
			{
				UE_LOGF(LogCook, Display, "Cooking %ls -> Rejected %ls", *PackageName, RejectedReason);
			}
			return;
		}
	...
	if (CommitType == EReachability::Runtime)
	{
		UE_CLOGF((GCookProgressDisplay & (int32)ECookProgressDisplayMode::Instigators) && PlatformIndex == 0,
			LogCook, Display, "Cooking %ls, Instigator: { %ls }",
		...
		UE_CLOGF(GCookProgressDisplay & (int32)ECookProgressDisplayMode::PackageNames, LogCook, Display,
			"Cooking %ls", *PackageName);
	}
	else
	{
		UE_CLOGF((GCookProgressDisplay & (int32)ECookProgressDisplayMode::Instigators) && PlatformIndex == 0,
			LogCook, Display, "Committing BuildDependencies for %ls, Instigator: { %ls }",
			*PackageName, *(PackageData.GetInstigator(EReachability::Build).ToString()));
		UE_CLOGF(GCookProgressDisplay & (int32)ECookProgressDisplayMode::PackageNames, LogCook, Display,
			"Committing BuildDependencies for %ls", *PackageName);
	}
...
void FSaveCookedPackageContext::FinishPlatform()
{
	...
	if (COTFS.IsDirectorCookByTheBook())
	{
		...
		IAssetRegistryReporter& Reporter = *(PlatformData->RegistryReporter);
		TOptional<TArray<FAssetData>> AssetDatasFromSave;
		if (bSuccessful)
		{
			AssetDatasFromSave.Emplace(MoveTemp(SavePackageResult.SavedAssets));
			// Apply the cook's tag filtering settings to the tags from SavePackage, e.g. remove tags that are allowed
			// or denied from runtime AssetRegistry by the AssetRegistry's configuration settings.
			for (FAssetData& AssetData : *AssetDatasFromSave)
			{
				FAssetRegistryState::FilterTags(AssetData, PlatformData->AssetRegistryDevelopmentSaveOptions);
			}
		}
		...
		if (!bHasRetryErrorCode)
		{
				...
				(!COTFS.CookOnTheFlyRequestManager || COTFS.CookOnTheFlyRequestManager->ShouldUseLegacyScheduling()))
			{
				...
				if (FPaths::FileExists(Filename))
				{
					...
					UE_LOGF(LogCook, Display, "UnsolicitedCookedPackages: %ls", *Filename);
#endif
				}
			}
		}
...
{
	if (!COTFS.bDependencyTrackingEnabled)
	{
		return;
	}
...
TArray<IPackageWriter::FCommitAttachmentInfo> FSaveCookedPackageContext::GetCommitAttachments()
{
	...
	FAppendPackageMetaDataContext Context;
	Context.Package = Package;

	for (const TRefCountPtr<UE::Cook::ICookArtifact>& Artifact : COTFS.CookArtifacts)
	{
		Artifact->AppendPackageMetadata(Context);
	}

	if (!COTFS.bLegacyBuildDependencies)
	{
		PlatformCookAttachments.AppendCommitAttachments(Context);
	}
	if (CookContext->DeterminismManager)
	{
		CookContext->DeterminismManager->AppendCommitAttachments(Context);
	}

	TArray<IPackageWriter::FCommitAttachmentInfo> Result;
	Result.Reserve(Context.Entries.Num());
	for (FAppendPackageMetaDataContext::FEntry& PackageMetaEntry : Context.Entries)
	{
		IPackageWriter::FCommitAttachmentInfo& Info = Result.Emplace_GetRef();
		Info.Key = PackageMetaEntry.Key;
		Info.Value = MoveTemp(PackageMetaEntry.Value);
		Info.FieldStorage = PackageMetaEntry.FieldStorage;
	}
	return Result;
}
