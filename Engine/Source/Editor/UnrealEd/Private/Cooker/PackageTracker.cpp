...
void FPackageTracker::OnCreatePackage(FName PackageName)
{
	...
	{
		UE_LOGF(LogCook, Verbose, "SoftGC PoorPerformance: Reloaded package %ls.",
		...
		if (Existing)
		{
			if (Existing->LoadedName != LoadedName)
			{
				UE_LOGF(LogCook, Error,
					"OnBeginLoadPackage was called twice for the same package with two different LoadedPaths. Ignoring the second call. PackageName: %ls. LoadedPath1: %ls. LoadedPath2: %ls.",
	...
	FPackageTracker& PackageTracker)
{
	...
	if (bAlreadyVisited)
	{
		UE_LOGF(LogCook, Error, "Cycle detected in InstancedPackage referencers. PackageName == %ls",
