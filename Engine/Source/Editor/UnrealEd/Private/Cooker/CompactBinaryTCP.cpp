	...
	uint64 DataSize, uint64& InOutBytesWritten)
{
	...
	if (BytesSent < WriteSize)
	{
		...
		if (!bResult)
		{
			...
			UE_LOGF(LogSockets, Log, "FCompactBinaryTCPImpl::PollSendBytes: Socket->Send(WriteSize==%d) failed with error %d and BytesSent == %d.",
			...
			if (Socket->GetConnectionState() == SCS_ConnectionError)
			{
				UE_LOGF(LogSockets, Error, "Could not WritePacket to Socket. Socket is in the error state.");
