...
namespace UE::Cook
{
	...
	*/
	...
	/** Placeholder to handle executing BuildDefinitions for requested but not-yet-loaded packages. */
	...
	extern FGuid ProjectCookIncrementalVersion;
...
public:
	...
	/** Should we generate a separate manifest for map dependencies */
	...
	/** True when performing a DLC plugin only cook, always ignores references outside the DLC path. */
	bool							bDlcPluginOnly = false;
...
FString InstigatorChainToString(TConstArrayView<FInstigator> Chain, FStringView FinalTargetToAppend = FStringView());
