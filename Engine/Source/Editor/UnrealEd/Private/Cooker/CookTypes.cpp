...
FString InstigatorChainToString(TConstArrayView<FInstigator> Chain, FStringView FinalTargetToAppend)
{
	...
	{
		bool bFirst = true;
		for (const FInstigator& Instigator : ReverseIterate(Chain))
		{
	...
	if (!FinalTargetToAppend.IsEmpty())
	{
		ChainText << TEXT(" -> ") << FinalTargetToAppend;
	}
	return ChainText.ToString();
}
...
void FBeginCookConfigSettings::CopyFromLocal(const UCookOnTheFlyServer& COTFS)
{
	...
	bLegacyBuildDependenciesDueToNoZenStore = COTFS.bLegacyBuildDependenciesDueToNoZenStore;
	bDependencyTrackingEnabled = COTFS.bDependencyTrackingEnabled;
...
FCbWriter& operator<<(FCbWriter& Writer, const UE::Cook::FBeginCookConfigSettings& Value)
{
	...
	Writer << "LegacyBuildDependenciesDueToNoZenStore" << Value.bLegacyBuildDependenciesDueToNoZenStore;
	Writer << "DependencyTrackingEnabled" << Value.bDependencyTrackingEnabled;
...
bool LoadFromCompactBinary(FCbFieldView Field, UE::Cook::FBeginCookConfigSettings& OutValue)
{
	...
	bOk = LoadFromCompactBinary(Field["LegacyBuildDependenciesDueToNoZenStore"], OutValue.bLegacyBuildDependenciesDueToNoZenStore) & bOk;
	bOk = LoadFromCompactBinary(Field["DependencyTrackingEnabled"], OutValue.bDependencyTrackingEnabled) & bOk;
