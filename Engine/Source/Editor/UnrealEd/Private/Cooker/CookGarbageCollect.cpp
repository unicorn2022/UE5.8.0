...
void UCookOnTheFlyServer::PollGarbageCollection(UE::Cook::FTickStackData& StackData)
{
	...
	NumPackagesHistory.AddInstance(PackageTracker->NumLoadedPackages());
...
bool UCookOnTheFlyServer::PumpHasExceededMaxMemory(uint32& OutResultFlags)
{
	...
	{
		UE_LOGF(LogCook, Display, "Running out of available UObject indices (%d remaining)", GUObjectArray.GetObjectArrayEstimatedAvailable());
		static bool bPerformedObjListWhenNearMaxObjects = false;
		if (GEngine && !bPerformedObjListWhenNearMaxObjects)
		{
			UE_LOGF(LogCook, Display, "Performing 'obj list' to show counts of types of objects due to low availability of UObject indices.");
	...
	if (MemoryMaxUsedVirtual > 0 || MemoryMaxUsedPhysical > 0)
	{
		...
		{
			UE_CALL_ONCE([&]() {
				UE_LOGF(LogCook, Warning, "Warning MemoryMaxUsed condition is larger than total memory (%dMiB >= %dMiB).  System does not have enough memory to cook this project.",
	...
	if (MemoryTriggerGCAtPressureLevel != FPlatformMemoryStats::EMemoryPressureStatus::Unknown)
	{
		...
		if (PressureStatus == FPlatformMemoryStats::EMemoryPressureStatus::Unknown)
		{
			UE_CALL_ONCE([&]() {
				...
				"MemoryPressureStatus is not available from the operating system. We may run out of memory due to lack of knowledge of when to collect garbage.");
				});
		}
		else
		{
	...
	if (!bPeriodicTriggered && LastFullGCTime + GCCooldown > CurrentTime)
	{
		if (!bIsSoftGC && !bWarnedExceededMaxMemoryWithinGCCooldown)
		{
			...
			UE_LOGF(LogCook, Display, "Garbage collection triggers ignored: Out of memory condition has been detected, but is only %.0fs after the last GC. "
				"It will be prevented until %.0f seconds have passed and we may run out of memory.\n"
				"Garbage collection triggered by conditions: %ls",
	...
	UE_LOGF(LogCook, Display, "Garbage collection triggered (%ls). Triggered by conditions:%ls",
...
void UCookOnTheFlyServer::ClearGCData()
{
	GCKeepObjects.Empty();
	UPackage::SoftGCPackageToObjectList.Empty();
...
int32 UCookOnTheFlyServer::NumLoadedPackages() const
{
	return PackageTracker->NumLoadedPackages();
}

void UCookOnTheFlyServer::EvaluateGarbageCollectionResults(bool bWasDueToOOM, bool bWasPartialGC, uint32 ResultFlags,
	int32 NumObjectsBeforeGC, const FPlatformMemoryStats& MemStatsBeforeGC, int32 NumPackagesBeforeGC,
	const FGenericMemoryStats& AllocatorStatsBeforeGC,
	int32 NumObjectsAfterGC, const FPlatformMemoryStats& MemStatsAfterGC, int32 NumPackagesAfterGC,
	...
	float GCDurationSeconds)
{
	...
	static EDisplayLevel LevelGCOther = EDisplayLevel::None;
	static EDisplayLevel LevelGCSuccess = EDisplayLevel::None;
	// LevelGCFail defined below as the return value of ParseCommandLineAndReturnLevelGCFail.
	auto ParseCommandLineAndReturnLevelGCFail = []()
	{
		EDisplayLevel LevelGCFailLocal;
#if ENABLE_LOW_LEVEL_MEM_TRACKER
		if (FLowLevelMemTracker::Get().IsEnabled())
		{
			LevelGCOther = EDisplayLevel::FullWithDiagnostics;
			LevelGCSuccess = EDisplayLevel::FullWithDiagnostics;
			LevelGCFailLocal = EDisplayLevel::FullWithDiagnostics;
		}
		else
#endif
		{
			LevelGCOther = EDisplayLevel::None;
			LevelGCSuccess = EDisplayLevel::Simple;
			LevelGCFailLocal = EDisplayLevel::Full;
		}
		// Command line overrides the defaults
		auto TryParseDisplayLevel = [](const TCHAR* FlagPattern, EDisplayLevel& OutLevel)
		{
			FString Text;
			bool bParsedValue = FParse::Value(FCommandLine::Get(), *FString::Printf(TEXT("-%s="), FlagPattern), Text);
			if (!bParsedValue && FParse::Param(FCommandLine::Get(), FlagPattern))
			{
				Text = TEXT("Full");
			}
			if (Text.IsEmpty())
			...
			for (const auto& Pair : std::initializer_list<TPair<EDisplayLevel, const ANSICHAR*>> {
				{ EDisplayLevel::None, "None" },
				{ EDisplayLevel::Simple, "Simple" },
				{ EDisplayLevel::FullWithDiagnostics, "Full" } })
			{
				if (Text == Pair.Value)
				{
					OutLevel = Pair.Key;
			...
			UE_LOGF(LogCook, Error, "Invalid commandline %ls argument for -%ls=%ls. Valid values are { None, Simple, Full }.",
				*Text, FlagPattern, *Text);
			return false;
		};

		EDisplayLevel LevelAll;
		if (TryParseDisplayLevel(TEXT("CookMemoryAnalysis"), LevelAll))
		{
			LevelGCOther = LevelAll;
			LevelGCSuccess = LevelAll;
			LevelGCFailLocal = LevelAll;
		}
		TryParseDisplayLevel(TEXT("CookMemoryAnalysis.GCOther"), LevelGCOther);
		TryParseDisplayLevel(TEXT("CookMemoryAnalysis.GCSuccess"), LevelGCSuccess);
		TryParseDisplayLevel(TEXT("CookMemoryAnalysis.GCFail"), LevelGCFailLocal);
		return LevelGCFailLocal;
	};
	static EDisplayLevel LevelGCFail = ParseCommandLineAndReturnLevelGCFail();

	if ((!bWasDueToOOM && LevelGCOther == EDisplayLevel::None)
		|| (bWasDueToOOM && LevelGCSuccess == EDisplayLevel::None && LevelGCFail == EDisplayLevel::None))
	{
		// No diagnostics requested for this case, early exit to avoid any further cost.
		return;
	}
	...
	int64 NumPackagesMin = NumPackagesHistory.GetMinimum();
	int64 NumPackagesMax = NumPackagesHistory.GetMaximum();
	int64 NumPackagesFreed = NumPackagesBeforeGC - NumPackagesAfterGC;
	...
	int64 ExpectedObjectsFreed = static_cast<int64>(MemoryExpectedFreedToSpreadRatio
		* static_cast<float>(NumObjectsBeforeGC - NumObjectsMin));
	double ExpectedMemFreed = MemoryExpectedFreedToSpreadRatio * (VirtualMemBeforeGC - VirtualMemMin);

	FString HeaderMessage = TEXT("GarbageCollection Results:");
	EDisplayLevel DisplayLevel = EDisplayLevel::None;
	bool bWasImpactful = true;
	if (!bWasDueToOOM)
	{
		DisplayLevel = LevelGCOther;
	}
	else
	{
		bWasImpactful =
		...
		if (!bWasImpactful)
		{
			HeaderMessage = TEXT("GarbageCollection Results: Garbage Collection was not very impactful.");
		}
		DisplayLevel = bWasImpactful ? LevelGCSuccess : LevelGCFail;
	}

	constexpr int32 BytesPerMeg = 1000000;
	if (DisplayLevel == EDisplayLevel::None)
	{
		// No text
	}
	else if (DisplayLevel == EDisplayLevel::Simple)
	{
		UE_LOG(LogCook, Display, TEXT("%s\n")
			TEXT("\tType: %s\n")
			TEXT("\tDuration: %.3fs\n")
			TEXT("\tNumObjects:\n")
			TEXT("\t\tCapacity:         %10lld\n")
			TEXT("\t\tBefore GC:        %10lld\n")
			TEXT("\t\tAfter GC:         %10lld\n")
			TEXT("\t\tFreed by GC:      %10.1f%%\n")
			TEXT("\tNumPackages:\n")
			TEXT("\t\tBefore GC:        %10lld\n")
			TEXT("\t\tAfter GC:         %10lld\n")
			TEXT("\t\tFreed by GC:      %10.1f%%\n")
			...
			*HeaderMessage,
			...
			GCDurationSeconds,
			NumObjectsCapacity, (int64)NumObjectsBeforeGC, (int64)NumObjectsAfterGC,
			100.f * ((float)NumObjectsFreed)/NumObjectsBeforeGC,
			(int64) NumPackagesBeforeGC, (int64) NumPackagesAfterGC,
			100.f * ((float)NumPackagesFreed)/NumPackagesBeforeGC,
			VirtualMemBeforeGC / BytesPerMeg, VirtualMemAfterGC / BytesPerMeg,
			100.f * ((float)VirtualMemFreed) / VirtualMemBeforeGC
		);
	}
	else
	{
		check(DisplayLevel == EDisplayLevel::Full || DisplayLevel == EDisplayLevel::FullWithDiagnostics);
		UE_LOGF(LogCook, Display, "%ls", *HeaderMessage);
		UE_LOGF(LogCook, Display, "\tMemoryAnalysis: General:\n"
			"\t\tType: %ls\n"
			"\tDuration: %.3fs",
			(bWasSoftGC ? TEXT("Soft") : (bWasPartialGC ? TEXT("Partial") : TEXT("Full"))),
			GCDurationSeconds);
		UE_LOG(LogCook, Display, TEXT("\tMemoryAnalysis: NumObjects:\n")
			...
			TEXT("\t\tFreed by GC:      %10.1f%%"),
			NumObjectsCapacity, NumObjectsMin, NumObjectsMax,
			(int64)NumObjectsBeforeGC, (int64)NumObjectsAfterGC,
			100.f * ((float)NumObjectsFreed)/NumObjectsBeforeGC);
		UE_LOG(LogCook, Display, TEXT("\tMemoryAnalysis: NumPackages:\n")
			TEXT("\t\tProcess Min:      %10" INT64_FMT "\n")
			TEXT("\t\tProcess Max:      %10" INT64_FMT "\n")
			TEXT("\t\tBefore GC:        %10" INT64_FMT "\n")
			TEXT("\t\tAfter GC:         %10" INT64_FMT "\n")
			TEXT("\t\tFreed by GC:      %10.1f%%"),
			NumPackagesMin, NumPackagesMax,
			(int64)NumPackagesBeforeGC, (int64)NumPackagesAfterGC,
			100.f * ((float)NumPackagesFreed)/NumPackagesBeforeGC);
		UE_LOG(LogCook, Display, TEXT("\tMemoryAnalysis: Virtual Memory:\n")
			...
			TEXT("\t\tAfter GC:         %10" INT64_FMT " MB\n")
			TEXT("\t\tFreed by GC:      %10.1f%%"),
			VirtualMemMin / BytesPerMeg, VirtualMemMax / BytesPerMeg,
			VirtualMemBeforeGC / BytesPerMeg, VirtualMemAfterGC / BytesPerMeg,
			100.f * ((float)VirtualMemFreed)/VirtualMemBeforeGC);
		...
		UE_LOGF(LogCook, Display, "\tMemoryAnalysis: Allocator Stats Before:%ls",
			*AllocatorStatsToString(AllocatorStatsBeforeGC));
		UE_LOGF(LogCook, Display, "\tMemoryAnalysis: Allocator Stats After:%ls",
			*AllocatorStatsToString(AllocatorStatsAfterGC));

		if (DisplayLevel == EDisplayLevel::Full)
		{
			UE_LOGF(LogCook, Display,
				"Extended memory diagnostics are disabled. Run with -llm or -trace=memtag or -CookMemoryAnalysis for more diagnostics.");
		}
		else if (!bWasSoftGC)
		{
			UE_LOGF(LogCook, Display, "See log for package info and LLM tag diagnostics.");
			UE::Cook::DumpPackageReferencers(TEXT("Non-startup Packages"), CookByTheBookOptions->SessionStartupObjects);
		...
		else // bWasSoftGC
		{
			...
			{
				UE::Cook::DumpPackageReferencers(TEXT("SoftGCPackages"),
					CookByTheBookOptions->SessionStartupObjects, PackagesReferencedOutsideOfCooker);
			}
		}
	}
}
