...
void FDeterminismManager::AppendCommitAttachments(UE::Cook::Artifact::FAppendPackageMetaDataContext& Context)
{
	PackageData.Sort();
	FCbWriter Writer;
	if (TrySave(Writer))
	{
		Context.SetAttachment(DeterminismManagerName, Writer.Save().AsObject());
	}
}
