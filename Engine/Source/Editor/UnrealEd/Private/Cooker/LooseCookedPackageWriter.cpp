...
void FLooseCookedPackageWriter::EndCook(const FCookInfo& Info)
{
	if (!Info.bWorkerOnSharedSandbox)
	{
		...
		{
			UE_LOGF(LogSavePackage, Error, "Failed saving package store manifest file '%ls'", *PackageStoreManifestFilePath);
		}
	}
}

void FLooseCookedPackageWriter::PopulateOplog(const FAssetRegistryState& PreviousState, int32& OutNumPackagesInOplog)
{
	...
	{
		// Remove from disk the cooked files that no longer exist in the editor.
		GetAllCookedFiles();
		TSet<FName> RemoveFromDisk;
		IAssetRegistry& AssetRegistry = IAssetRegistry::GetChecked();
		for (const TPair<FName, TArray<FString>>& Pair : PackageNameToCookedFiles)
		{
			FName PackageName = Pair.Key;

			if (!AssetRegistry.DoesPackageExistOnDisk(PackageName))
			{
				// Check that the package is not an exception before removing from cooked.
				// Generated packages do not exist uncooked.
				bool bIsCookedOnly = false;
				...
				if (!bIsCookedOnly)
				{
					RemoveFromDisk.Add(PackageName);
				}
			}
		}
		...
		OutNumPackagesInOplog = PackageNameToCookedFiles.Num();
	}
	else
	{
		// Clean the sandbox, since we are getting files from the SharedBuild location rather than from the local cook
		// directory.
		DeleteSandboxDirectory();
		// If we are legacyiterative from a shared build the cooked files do not exist in the local cooked directory;
		// we assume they are packaged in the pak file (which we don't want to extract to confirm) and keep them all.
		OutNumPackagesInOplog = PreviousState.GetNumPackages();
	}
}

void FLooseCookedPackageWriter::UpdateLastReferenceDateAndPruneStaleOps(
	UE::Cook::Artifact::FUpdateOplogPackagesContext& OplogContext)
{
	// LooseCookedPackageWriter does not support setting LastReferenceDate and pruning stale ops; it only does
	// pruning based on packages that no longer exist in editor. That more limited pruning was already done in
	// PopulateOplog.
}

TArray<FName> FLooseCookedPackageWriter::GetOplogPackageNames()
{
	TArray<FName> Result;
	PackageNameToCookedFiles.GenerateKeyArray(Result);
	return Result;
}
...
bool FLooseCookedPackageWriter::TryReadMPCookMessageForPackage(FName PackageName, FCbObjectView Message)
{
	...
	if (LoadFromCompactBinary(Message["PackageHash"], ThisPackageHashes->PackageHash))
	{
		...
		if (bOk)
		{
			...
			if (bAlreadyExisted)
			{
				UE_LOGF(LogSavePackage, Error, "FLooseCookedPackageWriter encountered the same package twice in a cook! (%ls)",
