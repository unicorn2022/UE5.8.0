...
FCookWorkerServer::~FCookWorkerServer()
{
		...
		|| ConnectStatus == EConnectStatus::WaitForDisconnect)
	{
		UE_LOGF(LogCook, Error,
			"CookWorkerServer %d was destroyed before it finished Disconnect. The remote process may linger and may interfere with writes of future packages.",
			ProfileId);
	}
...
void FCookWorkerServer::SendCrashDiagnostics()
{
	...
	UE_LOGF(LogCook, Display,
		"LostConnection to CookWorker %d. Log messages written after communication loss:", ProfileId);
	FString LogText;
	...
	{
		UE_LOGF(LogCook, Warning, "No log file found for CookWorker %d.", ProfileId);
	}
	else
	{
	...
	if (!CrashDiagnosticsError.IsEmpty())
	{
		...
		{
			UE_LOGF(LogCook, Error, "%ls", *CrashDiagnosticsError);
		}
		else
		{
			...
			UE_LOGF(LogCook, Warning, "%ls", *CrashDiagnosticsError);
		}
	}
	...
	TArray<UE::CompactBinaryTCP::FMarshalledMessage>&& OtherPacketMessages, ECookDirectorThread TickThread)
{
	...
	UE_LOGF(LogCook, Display, "CookWorker %d connected after %.3fs.", ProfileId,
	...
	for (;;)
	{
		...
		case EConnectStatus::PumpingCookComplete:
		{
			...
			if (ConnectStatus == EConnectStatus::PumpingCookComplete)
			{
				...
				UE_LOGF(LogCook, Error,
					"CookWorker process of CookWorkerServer %d failed to finalize its cook within %.0f seconds; we will tell it to shutdown.",
...
void FCookWorkerServer::LaunchProcess()
{
	...
	{
		UE_LOGF(LogCook, Display,
			"CookWorkerServer %d launched CookWorker as WorkerId %d and PID %u with commandline \"%ls\".",
...
void FCookWorkerServer::TickWaitForDisconnect()
{
	...
	{
		UE_CLOGF(!bTerminateImmediately, LogCook, Warning,
			"CookWorker process of CookWorkerServer %d failed to disconnect within %.0f seconds; we will terminate it.",
...
void FCookWorkerServer::PumpSendMessages()
{
	...
	{
		UE_LOGF(LogCook, Error,
			"CookWorkerCrash: CookWorker %d failed to write to socket, we will shutdown the remote process. Assigned packages will be returned to the director.",
			ProfileId);
		bNeedCrashDiagnostics = true;
		SendToState(EConnectStatus::WaitForDisconnect);
...
void FCookWorkerServer::SendPendingPackages()
{
	...
	for (FPackageData* PackageData : PackagesToAssign)
	{
		...
		if (NumNeedCommitPlatforms == 0)
		{
			...
			UE_LOGF(LogCook, Warning, "Package %ls was assigned to worker, but at sendmessage time it has no platforms needing commit. State = %ls. CookPhase = %ls. [ %ls ]",
...
void FCookWorkerServer::HandleReceiveMessagesInternal()
{
	while (!ReceiveMessages.IsEmpty())
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogCook, Error,
					"CookWorkerServer received message of unknown type %ls from CookWorker. Ignoring it.",
	...
	const ITargetPlatform* TargetPlatform, TArray<UE::CompactBinaryTCP::FMarshalledMessage>&& Messages)
{
	...
	for (UE::CompactBinaryTCP::FMarshalledMessage& Message : Messages)
	{
		...
		{
			UE_LOGF(LogCook, Error,
				"CookWorkerServer received PackageMessage of unknown type %ls from CookWorker. Ignoring it.",
				*Message.MessageType.ToString());
		}
	}
}
	...
	for (FPackageRemoteResult& Result : Message.Results)
	{
		...
		if (!PackageData)
		{
			UE_LOGF(LogCook, Warning,
				"CookWorkerServer %d received FPackageResultsMessage for invalid package %ls. Ignoring it.",
		...
		{
			UE_LOGF(LogCook, Display,
				"CookWorkerServer %d received FPackageResultsMessage for package %ls which is not a pending package. Ignoring it." "\n\tState = %ls, WorkerId = %ls, CookResults = { %ls }, Result.GetSuppressCookReason = %ls",
				ProfileId, *Result.GetPackageName().ToString(),
		...
		{
			UE_LOGF(LogCook, Error,
				"CookWorkerServer %d received FPackageResultsMessage for package %ls with an invalid number of platform results: expected %d, actual %d. Ignoring it.",
		...
		if (bResultIsSaveResult)
		{
			...
			for (int32 PlatformIndex = 0; PlatformIndex < NumPlatforms; ++PlatformIndex)
			{
				...
				if (ExistingData.IsCommitted())
				{
					...
					{
						UE_LOGF(LogCook, Display,
							"CookWorkerServer %d received FPackageResultsMessage for package %ls, platform %ls, but that platform has already been committed. Ignoring the results for that platform.",
				...
				else
				{
					...
					{
						UE_LOGF(LogCook, Display,
							"CookWorkerServer %d received FPackageResultsMessage for package %ls, platform %ls, but that platform has already been cooked. Ignoring the results for that platform.",
							ProfileId, *Result.GetPackageName().ToString(), *TargetPlatform->PlatformName());
					}
					else
					{
		...
		if (PackageData->IsGenerated() && bTerminalStateChange)
		{
			...
			{
				UE_LOGF(LogCook, Warning,
					"RecordResults received for generated package %ls, but its ParentGenerationHelper has already been destructed so we can not update the save flag. Leaving the save flag unupdated; this might cause workers to run out of memory due to keeping the Generator referenced.",
		...
		if (bResultIsSaveResult)
		{
			...
			if (PackageData->GetPlatformsNeedingCommitNum(CookPhase) > 0)
			{
				...
				UE_LOGF(LogCook, Display,
					"Package %ls was completed by CookWorker %d for platforms { %ls }, but it still needs to commit platforms { %ls }."
					" Sending it back to the request state.",
		...
		{
			UE_LOGF(LogCook, Error,
				"Package %ls was retracted by CookWorker %d, but it still sent a RecordResults message for the package which is supposed to be omitted for RetractedByCookDirector suppressions.",
				*PackageData->GetPackageName().ToString(), ProfileId);
...
{
	UE_LOGF(LogCook, Error,
		"CookWorkerServer received invalidly formatted message for type %ls from CookWorker. Ignoring it.",
		MessageTypeName);
}
...
void FCookWorkerServer::HandleGeneratorMessage(FGeneratorEventMessage& GeneratorMessage)
{
	...
	if (!PackageData)
	{
		...
		UE_LOGF(LogCook, Error,
			"CookWorkerServer received unexpected GeneratorMessage for package %ls. The PackageData %ls does not exist on the CookDirector. " "\n\tCook of this generator package and its generated packages will be invalid.",
	...
	FCbFieldView TargetPlatformsField = Object["TargetPlatforms"];
	{
		...
		for (FCbFieldView ElementField : TargetPlatformsField)
		{
			...
			if (LoadFromCompactBinary(ElementField, KeyName))
			{
				...
				{
					UE_LOGF(LogCook, Error, "Could not find TargetPlatform \"%.*ls\" received from CookDirector.",
	...
	FCbObjectView InMessage)
{
	...
	{
		UE_LOGF(LogCook, Error, "FLogMessagesMessageHandler received corrupted message from CookWorker");
		return;
	}
...
void FPackageWriterMPCollector::ServerReceiveMessage(FMPCollectorServerMessageContext& Context, FCbObjectView Message)
{
	...
	{
		UE_LOGF(LogCook, Error,
			"CookWorkerServer received invalidly formatted PackageWriter message from CookWorker %d. Ignoring it.",
