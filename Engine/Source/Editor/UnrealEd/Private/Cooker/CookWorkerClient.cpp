...
FCookWorkerClient::~FCookWorkerClient()
{
		...
		(EConnectStatus::FlushAndAbortFirst <= ConnectStatus && ConnectStatus <= EConnectStatus::FlushAndAbortLast))
	{
		UE_LOGF(LogCook, Warning,
			"CookWorker was destroyed before it finished Disconnect. The CookDirector may be missing some information.");
	}
...
void FCookWorkerClient::CreateServerSocket(const FDirectorConnectionInfo& ConnectInfo)
{
	...
	if (!SocketSubsystem)
	{
		UE_LOGF(LogCook, Error,
			"CookWorker initialization failure: platform does not support network sockets, cannot connect to CookDirector.");
	...
	{
		UE_LOGF(LogCook, Error,
			"CookWorker initialization failure: could not convert -CookDirectorHost=%ls into an address, cannot connect to CookDirector.",
			*DirectorURI);
		SendToState(EConnectStatus::LostConnection);
		return;
	}

	UE_LOGF(LogCook, Display, "Connecting to CookDirector at %ls...", *DirectorURI);

	ServerSocket = Sockets::ConnectToHost(*DirectorAddr, TEXT("FCookWorkerClient-WorkerConnect"));
	if (!ServerSocket)
	{
		UE_LOGF(LogCook, Error, "CookWorker initialization failure: Could not connect to CookDirector.");
		SendToState(EConnectStatus::LostConnection);
		return;
	}
	...
	{
		UE_LOGF(LogCook, Error,
			"CookWorker initialization failure: Timed out after %.0f seconds trying to connect to CookDirector.",
	...
	{
		UE_LOGF(LogCook, Error, "CookWorker initialization failure: could not send ConnectMessage.");
		SendToState(EConnectStatus::LostConnection);
		return;
	}
	LogConnected();
...
void FCookWorkerClient::PollWriteConnectMessage()
{
	...
	if (Status == EConnectionStatus::Incomplete)
	{
		...
		{
			UE_LOGF(LogCook, Error,
				"CookWorker initialization failure: timed out waiting for %fs to send ConnectMessage.",
...
void FCookWorkerClient::PollReceiveConfigMessage()
{
	...
	{
		UE_LOGF(LogCook, Error, "CookWorker initialization failure: failed to read from socket.");
		SendToState(EConnectStatus::LostConnection);
		return;
	}
	if (Messages.Num() == 0)
	{
		...
		{
			UE_LOGF(LogCook, Error,
				"CookWorker initialization failure: timed out waiting for %fs to receive InitialConfigMessage.",
				CookWorkerClient::WaitForConnectReplyTimeout);
			SendToState(EConnectStatus::LostConnection);
		}
		return;
	...
	{
		UE_LOGF(LogCook, Warning,
			"CookWorker initialization failure: Director sent a different message before sending an InitialConfigMessage. MessageType: %ls.",
	...
	{
		UE_LOGF(LogCook, Warning,
			"CookWorker initialization failure: Director sent an invalid InitialConfigMessage.");
		SendToState(EConnectStatus::LostConnection);
		return;
	}
	...
	if (OrderedSessionPlatforms.Num() != ActiveTargetPlatforms.Num())
	{
		...
		UE_LOGF(LogCook, Error,
			"CookWorker initialization failure: Director sent a mismatch in session platform quantity.\n%ls", *StringBuilder);
		SendToState(EConnectStatus::LostConnection);
		return;
	}
	...
	if (bPlatformMismatch)
	{
		...
		GetPlatformDetails(StringBuilder);
		UE_LOGF(LogCook, Error,
			"CookWorker initialization failure: Director sent a mismatch in session platform contents.\n%ls", *StringBuilder);
		SendToState(EConnectStatus::LostConnection);
		return;
	}
	...
	UE_LOGF(LogCook, Display, "Initialization from CookDirector complete.");
...
{
	UE_LOGF(LogCook, Display, "Connection to CookDirector successful.");
}
...
void FCookWorkerClient::PumpSendBuffer()
{
	...
	{
		UE_LOGF(LogCook, Error,
			"CookWorkerClient failed to write message to Director. We will abort the CookAsCookWorker commandlet.");
		SendToState(EConnectStatus::LostConnection);
	}
}
...
void FCookWorkerClient::PumpReceiveMessages()
{
	...
	for (;;)
	{
		...
		{
			UE_LOGF(LogCook, Error,
				"CookWorkerClient failed to read from Director. We will abort the CookAsCookWorker commandlet.");
			SendToState(EConnectStatus::LostConnection);
			return;
		}
		if (Messages.IsEmpty())
	...
	for (UE::CompactBinaryTCP::FMarshalledMessage& Message : Messages)
	{
		if (EConnectStatus::FlushAndAbortFirst <= ConnectStatus && ConnectStatus <= EConnectStatus::FlushAndAbortLast)
		{
			if (Message.MessageType == FAbortWorkerMessage::MessageType)
			{
				UE_LOGF(LogCook, Display,
					"CookWorkerClient received AbortWorker message from Director. Terminating flush and shutting down.");
				SendToState(EConnectStatus::LostConnection);
				break;
			}
			UE_LOGF(LogCook, Error,
				"CookWorkerClient received message %ls from Director after receiving Abort message. Message will be ignored.",
		...
		else
		{
			if (Message.MessageType == FAbortWorkerMessage::MessageType)
			{
				...
				{
					UE_LOGF(LogCook, Display,
						"CookWorkerClient received CookComplete message from Director. Flushing messages and shutting down.");
					SendToState(EConnectStatus::FlushAndAbortFirst);
				}
				else
				{
					UE_LOGF(LogCook, Display,
						"CookWorkerClient received AbortWorker message from Director. Shutting down.");
					SendToState(EConnectStatus::LostConnection);
					break;
				}
			}
			...
			{
				UE_LOGF(LogCook, Warning,
					"CookWorkerClient received unexpected repeat of InitialConfigMessage. Ignoring it.");
			}
			...
			else
			{
				...
				{
					UE_LOGF(LogCook, Error,
						"CookWorkerClient received message of unknown type %ls from CookDirector. Ignoring it.",
						*Message.MessageType.ToString());
				}
			}
		}
	...
	for (;;)
	{
		...
		case EConnectStatus::WaitForAbortAcknowledge:
		{
			...
			if (ConnectStatus == EConnectStatus::WaitForAbortAcknowledge)
			{
				...
				{
					UE_LOGF(LogCook, Warning,
						"Timedout after %.0fs waiting to send disconnect message to CookDirector.",
...
{
	UE_LOGF(LogCook, Error,
		"CookWorkerClient received invalidly formatted message for type %ls from CookDirector. Ignoring it.",
		MessageTypeName);
}
...
void FCookWorkerClient::UpdateSocketSendDiagnostics(UE::CompactBinaryTCP::EConnectionStatus Status)
{
	...
	else
	{
		...
		{
			UE_LOGF(LogCook, Display,
				"CookWorkerClient has been unable to send messages to the CookDirector for the past %.1f seconds. Continuing cooking locally and attempting to send...",
			...
			if (PackageData.IsInProgress())
			{
				...
				for (const ITargetPlatform* TargetPlatform : NeedCommitPlatforms)
				{
					...
					if (!PackageData.FindOrAddPlatformData(TargetPlatform).IsReachable(Reachability))
					{
						...
						{
							UE_LOGF(LogCook, Display,
								"Package %ls is in the save state, but the CookDirector updated the requested platforms to include the new platform %ls. Restarting the package's save.",
	...
	FRetractionRequestMessage&& Message)
{
	...
	for (FName PackageName : PackageNames)
	{
		...
		if (bShouldStall)
		{
			UE_LOGF(LogCook, Display, "Retracting generated package %ls; it will remain in memory on this worker until the generator finishes saving.",
	...
	UE_LOGF(LogCook, Display, "Retraction message received from director. %d packages retracted.",
		PackageNames.Num());
