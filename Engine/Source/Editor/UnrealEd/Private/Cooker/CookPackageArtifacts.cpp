	...
	for (int32 BatchStart = 0; BatchStart < NumDependencies; )
	{
		...
		case ECookDependency::Package:
		{
			for (FCookDependency& PackageDependency : Batch)
			{
				...
				else if (PackageDigest.Status == UE::EditorDomain::FPackageDigest::EStatus::DoesNotExistInAssetRegistry)
				{
					FBlake3Hash Hash = FBlake3Hash::Zero;
					if (bUpdateValues)
					{
						PackageDependency.SetValue(Hash);
					}

					KeyBuilder.Update(&Hash.GetBytes(), sizeof(Hash.GetBytes()));
					continue;
				}
...
/**
 * Go through all the imports of a package using the LinkerLoad and create an object redirector dependency for each import.
 * 
 * Here is an example of why we need this : Plugin 1 from DLC1 has package A, plugin 2 from DLC2 has package B. Package A contains an object 
 * redirector A.A -> B.B. Package C from DLC2 has an import (a pointer) to B.B.
 * When doing a multiproduct cook of package C, plugin 1 is not loaded because it is not needed for DLC2. The dependencies of package C will
 * contain a redirector dependency for package B. This dependency will track any redirector where B is a source or target of a redirection. In this
 * case there is no redirector so the hash will be equivalent to 0.
 * Now if I run a regular cook, I have all the plugins loaded. So when I cook package C, the redirector dependency for package B will find the object
 * redirect package A and use it to compute the hash. The dependencies are now different from the multiproduct cook.
 * 
 */
static void CollectObjectRedirectDependencies(const UPackage* Package, TArray<FCookDependency>& Dependencies)
{
	const FLinkerLoad* LinkerLoad = Package->GetLinker();
	if (!LinkerLoad)
	{
		return;
	}
	
	TSet<FName> UniqueImportNames;
	for(int ArrayIndex = 0; ArrayIndex < LinkerLoad->ImportMap.Num(); ++ArrayIndex)
	{
		FPackageIndex Index = FPackageIndex::FromImport(ArrayIndex);

		FString PathName;
		LinkerLoad->BuildPathName(PathName, Index);

		FSoftObjectPath ObjectPath(PathName);
		FName ImportPackageName = ObjectPath.GetLongPackageFName();
		...
		UniqueImportNames.Add(ImportPackageName, &bAlreadyInSet);
		...
		EPackageMountPoint MountPoint = GetPackageMountPoint(ImportPackageName);
		switch (MountPoint)
		{

		case EPackageMountPoint::Content:
			Dependencies.Add(UE::Cook::CreateObjectRedirectDependency(ImportPackageName));
			break;


		case EPackageMountPoint::Transient:
	...
	TConstArrayView<FName> UntrackedSoftPackageReferences, FGenerationHelper* GenerationHelper, bool bGenerated)
{
	...
	// For dependencies which don't exist on disk, add a package dependency. This will allow us to detect the 
	// need to recook if that dependency is later recreated on disk. 
	TArray<FName> AssetDependencies;
	AssetRegistry->GetDependencies(PackageName, AssetDependencies, UE::AssetRegistry::EDependencyCategory::Package, UE::AssetRegistry::EDependencyQuery::Hard);
	AssetRegistry->GetDependencies(PackageName, AssetDependencies, UE::AssetRegistry::EDependencyCategory::Package, UE::AssetRegistry::EDependencyQuery::Soft);

	for (FName AssetDependency : AssetDependencies)
	{
		FString DependencyName = AssetDependency.ToString();
		if (!FPackageName::IsScriptPackage(DependencyName) && !AssetRegistry->DoesPackageExistOnDisk(AssetDependency))
		{
			DefaultResultDependencies.Add(FCookDependency::Package(AssetDependency));
		}
	}
	...
	CollectObjectRedirectDependencies(Package, DefaultResultDependencies);
	...
	for (TPair<FName, TArray<FCookDependency>>& ResultPair : InOutResultDependencies)
	{
		...
		for (TArray<FCookDependency>::TIterator Iter(ResultDependencies); Iter; ++Iter)
		{
				...
				Dependency.GetType() == ECookDependency::Package)
			{
				...
				if (!bPackageExistOnDisk)
				{
					...
					if (DependencyPackage && DependencyPackage->HasAnyPackageFlags(PKG_NewlyCreated))
					{
					...
					// else the package is not on disk, in-memory and not newly created. This can happen for packages
					// with soft dependencies which have been deleted from disk - register it to the dependencies
					// so that we can later detect if it's been re-added and recook packages that depend on it.
				}
		...
		if (ResultPair.Key == BuildResult::NAME_Save)
		{
			...
			// Runtime dependencies also cause RedirectionTarget dependencies, so record those.
			for (TArray<FName>::TIterator Iter(InOutRuntimeDependencies); Iter; ++Iter)
			{
				...
				// Do not add redirection dependencies for native package. Those dependencies are too wide and already
				// included in schema hash or package digest.
				case EPackageMountPoint::Script:
					break;
		...
		for (FName RedirectionTarget : RedirectionTargets)
		{
			...
			ResultDependencies.Add(UE::Cook::CreateObjectRedirectDependency(RedirectionTarget));
		}
	}
	return true;
}
...
bool LoadFromCompactBinary(FCbObjectView ObjectView, UE::Cook::FPackageArtifacts& Artifacts)
{
	...
	for (FCbFieldViewIterator FieldView(ObjectView.CreateViewIterator()); FieldView; )
	{
		...
		if (FieldView.GetName().Equals(UTF8TEXTVIEW("IoStoreChunkId")))
		{
			if (!LoadFromCompactBinary(FieldView++, Artifacts.IoStoreChunkId))
			{
				return false;
			}
		}
...
FCbWriter& operator<<(FCbWriter& Writer, const UE::Cook::FPackageArtifacts& Artifacts)
{
	...
	Writer << "IoStoreChunkId" << Artifacts.IoStoreChunkId;
...
static void AddAttachment(UE::Cook::Artifact::FAppendPackageMetaDataContext& Context,
	WritableType&& Writable, FUtf8StringView AttachmentKey)
{
	FCbWriter Writer;
	Writer << Writable;
	Context.SetAttachment(FName(AttachmentKey), Writer.Save().AsObject(), Artifact::Attachment);
}

void FIncrementalCookAttachments::AppendCommitAttachments(UE::Cook::Artifact::FAppendPackageMetaDataContext& Context)
{
	if (Artifacts.IsValid())
	{
		AddAttachment(Context, Artifacts, PackageArtifactsAttachmentKey);
	}
	if (!BuildDefinitions.Definitions.IsEmpty())
	{
		AddAttachment(Context, BuildDefinitions, BuildDefinitionsAttachmentKey);
	}
	if (!ImportsCheckerData.IsEmpty())
	{
		AddAttachment(Context, ImportsCheckerData, ImportsCheckerAttachmentKey);
	}
	if (!LogMessages.IsEmpty())
	{
		AddAttachment(Context, FLogMessagesArray(LogMessages), LogMessagesAttachmentKey);
	}
}
	...
	TConstArrayView<FReplicatedLogData> InLogMessages)
{
	...
	if (!Result.Artifacts.IsValid())
	{
		...
		UE_LOGF(LogCook, Verbose, "%ls", *LogText);
	}
		...
		for (const FPackageIncrementalCookId& Id : PackageIds)
		{
				...
				UE_CALL_ONCE([&]
					{
						UE_LOGF(LogCook, Warning,
							"CookDependency fetch was requested for package %ls with EWhereCooked::ExtraReleaseVersionAssets, which is not handled. "
							"TransitiveBuildDepencies will be missing and false positive incremental skips could result.",
			...
			default:
				UE_CALL_ONCE([&]
					{
						UE_LOGF(LogCook, Warning,
							"CookDependency fetch was requested for package %ls with EWhereCooked value %d, which is not handled. "
							"TransitiveBuildDepencies will be missing and false positive incremental skips could result.",
							*Id.PackageName.ToString(), EnumToUnderlyingType(Id.WhereCooked));
					});
				Callback(Id.PackageName, FIncrementalCookAttachments());
				break;
			}
	...
	TArray<FName>&& InRuntimeDependencies, TArray<TPair<ELogVerbosity::Type, FString>>* OutMessages)
{
	...
	Result.IoStoreChunkId = CreatePackageDataChunkId(FPackageId::FromName(PackageName));
