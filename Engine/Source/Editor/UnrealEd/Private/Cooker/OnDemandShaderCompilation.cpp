...
void FODSCClientData::CleanupWorldPartitionAssets()
{
	...
	{
		UE_LOGF(LogShaders, Error, "UPackageTools::UnloadPackages: %ls", *OutErrorMessage.ToString());
	}
}
...
//		UE_LOGF(LogShaders, Display, "ScanAssets: %ls->%ls", *AssetData.GetSoftObjectPath().ToString(), *DynamicMaterialData.PackageName);
...
static void AddUClassPackagesToExclusionList(TSet<FName>& ExcludedPackageNames, const FString& ClassName)
{
	...
	for (FTopLevelAssetPath& DerivedClassName : DerivedClassNames)
	{
		...
		UE_LOGF(LogShaders, Display, "FODSCClientData excluding package '%ls' because '%ls' is a uasset inheriting from %ls", 
