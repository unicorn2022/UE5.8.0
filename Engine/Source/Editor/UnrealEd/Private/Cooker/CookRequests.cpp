...
void FExternalRequests::OnRemoveSessionPlatform(const ITargetPlatform* TargetPlatform)
{
	...
	for (TPair<FName, FFilePlatformRequest>& kvpair : RequestMap)
	{
		...
		{
			UE_LOGF(LogCook, Error, "RemovePlatform call has left an empty list of platforms requested in CookOnTheSide request.");
		}
	}
}
...
{
	UE_LOGF(LogCook, Display, "Listing initially requested set of files to be cooked (does not include list of transitive dependencies):");
	for (const TPair<FName, FFilePlatformRequest>& KVPair : RequestMap)
	{
		UE_LOGF(LogCook, Display, "%ls", *KVPair.Value.ToString());
	}
