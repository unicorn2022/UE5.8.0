...
TSharedPtr<FInternetAddr> GetAddressFromStringWithPort(FStringView Text)
{
	...
	if (Text.FindLastChar(':', PortStartIndex))
	{
		if (PortStartIndex == 0)
		{
			UE_LOGF(LogSockets, Warning, "Could not serialize %.*ls, it is missing the <Addr>: prefix.",
	...
	if (PortValue < 0)
	{
		UE_LOGF(LogSockets, Warning, "Could not serialize %.*ls, it is missing the :<Port> suffix.",
			Text.Len(), Text.GetData());
		return TSharedPtr<FInternetAddr>();
	}
