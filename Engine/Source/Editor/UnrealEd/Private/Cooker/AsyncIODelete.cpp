...
bool FAsyncIODelete::Delete(const FStringView& PathToDelete, EPathType ExpectedType)
{
	...
	if (bAsyncInitialized)
	{
		...
		if (!IFileManager::Get().Move(*TempPath, *PathToDeleteStr, bReplace, bEvenIfReadOnly, bMoveAttributes, bDoNotRetryOnError)) // IFileManager::Move works on either files or directories
		{
			...
			UE_LOGF(LogCook, Warning, "Failed to move path '%.*ls' for async delete (LastError == %i); falling back to synchronous delete.", PathToDelete.Len(), PathToDelete.GetData(), FPlatformMisc::GetLastError());
...
bool FAsyncIODelete::SynchronousDelete(const TCHAR* InDeletePath, EPathType PathType)
{
	...
	if (!Result)
	{
		UE_LOGF(LogCook, Warning, "Failed to asyncdelete %ls '%ls'. LastError == %i.", PathType == EPathType::Directory ? TEXT("directory") : TEXT("file"), InDeletePath, FPlatformMisc::GetLastError());
	}
	return Result;
}
...
bool FAsyncIODelete::TryPurgeOldAndCreateRoot(bool bCreateRoot, TArray<FDeleteRequest>& OutOrphanedRootsToDelete)
{
	...
	if (!FileManager.DirectoryExists(*ParentDir))
	{
		if (bCreateRoot)
		{
			...
			{
				UE_LOGF(LogCook, Error, "Could not create AsyncIoDelete parent directory %ls. LastError: %d. Falling back to synchronous delete.",
	...
	if (!TempRoot.IsEmpty())
	{
		...
		{
			UE_LOGF(LogCook, Display, "AsyncIoDelete could not clean up its root %ls. LastError: %d.",
	...
	do
	{
		...
		if (!SharedTempRootLockFile)
		{
			if (FPlatformTime::Seconds() - StartTimeSeconds > UE::AsyncIODelete::Private::MaxWaitSecondsForLock)
			{
				if (bCreateRoot)
				{
					UE_LOGF(LogCook, Error, "AsyncIoDelete could not create LockFile %ls. Falling back to synchronous delete.",
						*SharedTempRootLockFileName);
				}
				else
				{
					UE_LOGF(LogCook, Display, "AsyncIoDelete could not clean up its root %ls, because another process has LockFile %ls locked. LastError: %d.",
	...
	if (!TempRoot.IsEmpty())
	{
		...
		{
			UE_LOGF(LogCook, Display, "AsyncIoDelete could not clean up its lock file %ls. LastError: %d.",
	...
	FileManager.IterateDirectory(*SharedTempRoot,
		...
		&FirstUnusedCount, &GetCountFromFilename](const TCHAR* FilenameOrDirectory, bool bIsDirectory)
		{
			...
			if (!bIsDirectory)
			{
				...
				{
					UE_LOGF(LogCook, Warning, "AsyncIoDelete found unexpected file '%ls' in its SharedTempRoot. This file will block cleanup of the SharedTempRoot, please delete it manually.",
						FilenameOrDirectory);
	...
	for (const FString& ExistingDirLeaf : ExistingDirLeafs)
	{
		...
		{
			UE_LOGF(LogCook, Warning, "AsyncIoDelete found unexpected directory '%ls' in its SharedTempRoot. This directory will block cleanup of the SharedTempRoot, please delete it manually.",
	...
	if (bCreateRoot)
	{
		...
		{
			UE_LOGF(LogCook, Error, "AsyncIoDelete could not create its root %ls. LastError: %d. Falling back to synchronous delete.",
		...
		if (!TempRootLockFile)
		{
			UE_LOGF(LogCook, Error, "AsyncIoDelete could not create LockFile %ls. LastError: %d. Falling back to synchronous delete.",
				*TempRootLockFileName, FPlatformMisc::GetLastError());
