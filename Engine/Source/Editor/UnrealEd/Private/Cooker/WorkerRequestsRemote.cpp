...
{
	check(FunctionName);
	UE_LOGF(LogCook, Error, "Calling %ls (a CookByTheBook function) is not allowed in a CookWorker.",
		FunctionName);
}
...
{
	check(FunctionName);
	UE_LOGF(LogCook, Error, "Calling %ls (a CookOnTheFly function) is not allowed in a CookWorker.", FunctionName);
}
...
void FWorkerRequestsRemote::LogCalledPublicInterfaceError(const TCHAR* FunctionName) const
{
	...
	UE_LOGF(LogCook, Error,
		"Calling %ls (a CookOnTheFlyServer public interface function) is not allowed in a CookWorker.",
		FunctionName);
}
...
{
	check(FunctionName);
	UE_LOGF(LogCook, Error, "Calling %ls (an editor-mode-only function) is not allowed in a CookWorker.",
		FunctionName);
}
