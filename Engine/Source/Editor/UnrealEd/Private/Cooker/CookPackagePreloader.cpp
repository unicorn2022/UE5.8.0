...
bool FPackagePreloader::TryPreload(FString* OutFailReason)
{
	...
	if (bTreatPackageAsLoaded)
	{
		...
		{
			if (AsyncRequest && !AsyncRequest->IsCompleted())
			{
				...
				if (OutFailReason)
				{
					*OutFailReason = TEXT("Package is loaded but AsyncRequest is not completed");
				}
				return false;
			}
	...
	else
	{
		...
		else if (AsyncRequest && AsyncRequest->IsFailed())
		{
			SetIsPreloadAttempted(true);
			return true;
		}
	...
	{
		bool bResetOldAsyncRequest = false;
		if (AsyncRequest && AsyncRequest->IsCompleted() && !IsPackageLoaded())
		{
			// The AsyncRequest previously finished, but we garbage collected and removed the package from memory
			// before we had a chance to test IsPackageLoaded. Restart the AsyncRequest.
			AsyncRequest.Reset();
			// We will bump it back up down below.
			PackageData.GetPackageDatas().GetMonitor().OnPreloadAllocatedChanged(PackageData, false);
			bResetOldAsyncRequest = true;
			if (OutFailReason)
			{
				*OutFailReason = TEXT("AsyncRequest was completed but package is not loaded");
			}
		}
		...
		if (!AsyncRequest.IsValid())
		{
			...
			int32 RequestID = LoadPackageAsync(
				...
				FLoadPackageAsyncDelegate::CreateLambda(
					[AsyncRequest = AsyncRequest, PackageName = this->PackageData.GetPackageName()]
					(const FName&, UPackage* LoadedPackage, EAsyncLoadingResult::Type)
					{
						if (!LoadedPackage)
						{
							// PackageLoad failed, mark the preload as complete; UCookOnTheFlyServer::LoadPackageInQueue will
							// call LoadPackage and handle the error.
							AsyncRequest->SetFailed();
						}
						else if (LoadedPackage->GetFName() != PackageName)
						{
							// The PackageName is not the name that we loaded. This can happen due to CoreRedirects.
							// Do not attempt to handle this here in the preloader; just mark the preload complete.
							// UCookOnTheFlyServer::LoadPackageInQueue will call LoadPackage and will detect the renamed
							// package there and will handle the update of the cooker.
							AsyncRequest->SetFailed();
						}
						else if (!FPackagePreloader::IsPackageLoaded(PackageName))
						{
							UE_LOGF(LogCook, Error,
								"Package %ls finished loading asynchronously, but IsPackageLoaded returned false. We will try to recover, but this error is unexpected.",
								*PackageName.ToString());
							AsyncRequest->SetFailed();
						}
						else
						{
							AsyncRequest->SetSucceeded();
						}
					}
				),
				32 /* Use arbitrary higher priority for preload as we're going to need them very soon */
			);
			AsyncRequest->SetRequestID(RequestID);

			if (OutFailReason && !bResetOldAsyncRequest)
			{
				*OutFailReason = TEXT("AsyncRequest was not yet created");
			}
		}
		else
		{
			if (OutFailReason)
			{
				*OutFailReason = TEXT("AsyncRequest is not yet complete");
			}
		}
	...
	if (!FilePtr->IsInitialized())
	{
		...
		else
		{
			if (OutFailReason)
			{
				*OutFailReason = TEXT("Waiting on PreloadableFile async load");
			}
	...
	{
		UE_LOGF(LogCook, Warning, "Failed to find file when preloading %ls.",
			*PackageData.GetFileName().ToString());
		SetIsPreloadAttempted(true);
	...
	{
		UE_LOGF(LogCook, Warning, "Failed to register %ls for preload.",
			*PackageData.GetFileName().ToString());
		SetIsPreloadAttempted(true);
		PreloadableFile.Reset(PackageData);
		PreloadableFileOpenResult = FOpenPackageResult();
...
void FPackagePreloader::ClearPreload()
{
	if (AsyncRequest)
	{
		...
		{
			FlushAsyncLoading(AsyncRequest->GetRequestID());
			check(AsyncRequest->IsCompleted());
		}
		PackageData.GetPackageDatas().GetMonitor().OnPreloadAllocatedChanged(PackageData, false);
		AsyncRequest.Reset();
	}
	...
	if (IsPreloaded())
	{
		...
		if (IPackageResourceManager::UnRegisterPreloadableArchive(FPackagePath::FromLocalPath(FileNameString)))
		{
			UE_LOGF(LogCook, Display,
				"PreloadableFile was created for %ls but never used. This is wasteful and bad for cook performance.",
...
{
	return IsPackageLoaded(PackageData.GetPackageName());
}

bool FPackagePreloader::IsPackageLoaded(const FPackageData& InPackageData)
{
	return IsPackageLoaded(InPackageData.GetPackageName());
}

bool FPackagePreloader::IsPackageLoaded(FName InPackageName)
{
	return FindObjectFast<UPackage>(nullptr, InPackageName) != nullptr;
}
...
bool FPackagePreloader::PumpLoadsIsReadyToLeavePreload(TRefCountPtr<FPackagePreloader>* OutBlocker,
	FString* OutBlockerReason)
{
	...
	TraverseImportGraph(
		...
		[&Blocker, OutBlockerReason](FPackagePreloader& Preloader) -> bool // ReportAndIsContinue
		{
			if (!Preloader.TryPreload(OutBlockerReason))
			{
				Blocker = &Preloader;
				return false; // Once we find we will return false, stop searching.
			}
			return true; // Keep searching.
		},
		false /* bAllowGather */
	);
	bool bBlocked = Blocker.IsValid();
	if (OutBlocker)
	{
		*OutBlocker = MoveTemp(Blocker);
	}
	return !bBlocked;
}
