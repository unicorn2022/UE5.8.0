...
FPackageData::FPackageData(FPackageDatas& PackageDatas, const FName& InPackageName, const FName& InFileName)
	...
	, bHasReplayedLogMessages(0), bHasBeenLoadedForCooking(0)
{
	...
	if (InInstigator.Category != EInstigator::BuildDependency)
	{
		...
		if (COTFS.GetCookPhase() == ECookPhase::BuildDependencies)
		{
			UE_LOGF(LogCook, Warning,
				"Package was added to the runtime discovery queue after starting BuildDependencies phase."
				"\n\tPackage: %ls", *PackageData.GetPackageName().ToString());
...
void FPackageData::ClearCookResults()
{
	...
	bHasBeenLoadedForCooking = false;
}
...
void FPackageData::SendToState(EPackageState NextState, ESendFlags SendFlags, EStateChangeReason ReleaseSaveReason)
{
	...
	if (NextState != EPackageState::Idle && NextState != EPackageState::Request)
	{
		// Add more diagnostics for an unexpected edge case we have seen: retracted packages already have been
		// committed. In-progress packages after the request state have a contract that they have a platform
		// needing commit; give a warning if this contract is broken during a state transition.
		int32 NumNeedCommitPlatforms = 0;
		ECookPhase CookPhase = PackageDatas.GetCookOnTheFlyServer().GetCookPhase();
		for (const TPair<const ITargetPlatform*, FPackagePlatformData>& PlatformPair : GetPlatformDatas())
		{
			if (PlatformPair.Key == CookerLoadingPlatformKey)
			{
				continue;
			}
			if (PlatformPair.Value.NeedsCommit(PlatformPair.Key, CookPhase))
			{
		...
		if (NumNeedCommitPlatforms == 0)
		{
			...
			UE_LOGF(LogCook, Warning,
				"InternalCookerError: Package %ls is entering InProgress state %ls, but already has no platforms needing commit. CookPhase = %ls. [ %ls ]",
				*GetPackageName().ToString(), LexToString(NextState), LexToString(CookPhase), *PlatformDataText);
			UE_CALL_ONCE([]()
				{
...
void FPackageData::UnStall(ESendFlags SendFlags)
{
	...
	UE_LOGF(LogCook, Display, "Unstalling package %ls; it will resume saving from the point at which it was retracted.",
...
void FPackageData::CreateObjectCache()
{
	...
	{
		CreateObjectCache(CachedObjectsInOuter, LocalPackage);
...
void FPackageData::CreateObjectCache(TArray<FCachedObjectInOuter>& OutBuffer, UPackage* InPackage)
{
	TArray<UObject*> ObjectsInOuter;
	// Ignore garbage objects; they will not be serialized out so we don't need to call
	// BeginCacheForCookedPlatformData on them
	GetObjectsWithOuter(InPackage, ObjectsInOuter, EGetObjectsFlags::IncludeNestedObjects, RF_NoFlags,
		EInternalObjectFlags::Garbage);
	OutBuffer.Reset(ObjectsInOuter.Num());
	for (UObject* Object : ObjectsInOuter)
	{
		...
		OutBuffer.Emplace(ObjectWeakPointer);
	}
}
...
EPollStatus FPackageData::RefreshObjectCache(bool& bOutFoundNewObjects)
{
	...
	GetObjectsWithOuter(Package.Get(), CurrentObjects, EGetObjectsFlags::IncludeNestedObjects, RF_NoFlags,
		EInternalObjectFlags::Garbage);

	// We add any new objects but do not remove any objects moved out of the package, and do not remove any objects
	// deleted by garbage collection. FCookGenerationHelper relies on being able to store objects in the generator's
	// object cache that it needs to call BeginCache/IsCached on but that might be in other packages such as
	// ExternalActor packages that own the object on disk or generated packages the object has been moved into.
	// Objects deleted by garbage collection are detected elsewhere and cause control flow decisions.
	...
	if (bOutFoundNewObjects)
	{
		...
		// added. Caller is responsible for changing state back to calling BeginCacheForCookedPlatformData.

		if (++GetNumRetriesBeginCacheOnObjects() > FPackageData::GetMaxNumRetriesBeginCacheOnObjects())
		{
			UE_LOGF(LogCook, Error,
				"Cooker has repeatedly tried to call BeginCacheForCookedPlatformData on all objects in the package, but keeps finding new objects.\n" "Aborting the save of the package; programmer needs to debug why objects keep getting added to the package.\n" "Package: %ls. Most recent created object: %ls.",
...
void FPackageData::SetSaveSubState(ESaveSubState Value)
{
	...
	{
		UE_LOGF(LogCook, Error, "SetSaveSubState(%ls) called from invalid PackageState %ls. The call will be ignored",
...
bool FPendingCookedPlatformData::PollIsComplete()
{
	...
	{
		// Exception: allow Generator packages to keep waiting on objects that they have moved into generated packages.
		TRefCountPtr<FGenerationHelper> GenerationHelper = PackageData.GetGenerationHelper();
		if (!GenerationHelper ||
			!GenerationHelper->ShouldKeepWaitingOnMovedPendingCookedPlatformData(*this, PackageData, LocalObject))
		{
			UE_LOGF(LogCook, Display,
				"We were waiting for IsCachedCookedPlatformData to return true for %ls in package %ls, but that object has been moved out of the package. We will stop waiting on it.",
	...
	{
		UE_LOGF(LogCook, Display,
			"We were waiting for IsCachedCookedPlatformData to return true for %ls, but that object is now marked for garbage. We will stop waiting on it.",
	...
	UE_LOGF(LogCook, Display, "%ls isn't cached yet", *LocalObject->GetFullName());
#endif
	/*if ( LocalObject->IsA(UMaterial::StaticClass()) )
	{
		...
		{
			UE_LOGF(LogCook, Warning,
			"Shader compiler is in a bad state!  Shader %ls is finished compile but shader compiling manager did not notify shader.  ",
...
void FPackageDatas::SetBeginCookConfigSettings(FStringView CookShowInstigator)
{
	...
	if (!CookShowInstigator.IsEmpty())
	{
		...
		{
			UE_LOGF(LogCook, Fatal, "-CookShowInstigator argument %.*ls is not a mounted filename or packagename",
		...
		else
		{
			...
			{
				UE_LOGF(LogCook, Fatal, "-CookShowInstigator argument %.*ls could not be found on disk",
					CookShowInstigator.Len(), CookShowInstigator.GetData());
			}
		}
	}
...
{
	{
		...
		{
			FPackageData& ExistingPackageData = **PackageDataMapAddr;
			FPackageData** FileNameMapAddr = FileNameToPackageData.Find(NormalizedFileName);
			if (FileNameMapAddr == nullptr)
			{
				// Since we have an existing package, it should have registered its filename. Therefore the only
				// way the current filename could be unregistered is if the existing package's filename is different.
				FString ExistingFileNameStr = ExistingPackageData.GetFileName().ToString();
				FString NewFileNameStr = NormalizedFileName.ToString();
				FileNameMapAddr = FileNameToPackageData.Find(ExistingPackageData.GetFileName());
				checkf(ExistingPackageData.GetFileName() != NormalizedFileName,
					TEXT("InternalCookerError: FindOrAddPackageData found existing package at PackageName, and NormalizedFileName is not registered, and so we expected ExistingPackage.FileName != NormalizedFileName, but it is equal.")
					TEXT("\n\tPackageName = %s, NormalizedFileName = %s"), *PackageName.ToString(), *NormalizedFileName.ToString());
				checkf(FileNameMapAddr != nullptr,
					TEXT("InternalCookerError: FindOrAddPackageData found existing package at PackageName with a different FileName, but that existing package's FileName is not registered.")
					TEXT("\n\tPackageName = %s, NormalizedFileName = %s, ExistingFileName = %s"),
					*PackageName.ToString(), *NewFileNameStr, *ExistingFileNameStr);

				if (FPaths::GetBaseFilename(ExistingFileNameStr, false /* bRemovePath */)
					!= FPaths::GetBaseFilename(NewFileNameStr, false /* bRemovePath */))
				{
					// If the filename's Path or BaseNameWithoutExtension are different, that's a discrepency we don't handle.
					checkf(ExistingFileNameStr == NewFileNameStr,
						TEXT("Package %s is being added with filename %s, but it already exists with filename with different path %s, ")
						TEXT("and it is not present in FileNameToPackageData map under the new name."),
						*PackageName.ToString(), *NewFileNameStr, *ExistingFileNameStr);
				}
				else
				{
					// We handle the extension being different, but only in the special case that the package does not need to be saved.
					// If we are responsible for saving the package then we need to know which extension is correct, so we are broken.
					bool bAllPreviouslyCooked = true;
					for (const ITargetPlatform* TargetPlatform
						: GetCookOnTheFlyServer().PlatformManager->GetSessionPlatforms())
					{
						if (ExistingPackageData.FindOrAddPlatformData(TargetPlatform).GetWhereCooked()
							== EWhereCooked::ThisSession)
						{
							bAllPreviouslyCooked = false;
						}
					}
					if (bAllPreviouslyCooked)
					{
						// Silently accept the existing extension; we will not need it since we are not cooking it.
					}
					else
					{
						// We don't handle different extensions when we need to cook the package, but it will probably
						// not cause a crash elsewhere. Log it as an error and print the callstack.
						UE_LOGF(LogCook, Error,
							"Package %ls is being added with filename %ls, but it already exists with filename with different extension %ls. Keeping the old extension. This is unexpected and the output of the cook might be invalid.",
							*PackageName.ToString(), *NewFileNameStr, *ExistingFileNameStr);
						FDebug::DumpStackTraceToLog(ELogVerbosity::Display);
					}
					// MPCook TODO: Send a correction to all CookWorkers?
				}
			}
			checkf(*FileNameMapAddr == &ExistingPackageData,
	...
	FName* OutFoundFileName)
{
	...
	if (ExistingFileName.IsNone())
	{
		...
		UE_LOGF(LogCook, Warning,
			"Unexpected failure to cook filename '%ls'. It is mapped to PackageName '%ls', but does not exist on disk and we cannot verify the extension.",
...
bool FPackageDatas::TryLookupFileNameOnDisk(FName PackageName, FString& OutFileName)
{
	...
	if (FPackageName::IsVersePackage(PackageNameStr))
	{
		if (FindPackage(/*Outer =*/nullptr, *PackageNameStr))
		{
			...
			{
				UE_LOGF(LogCook, Warning,
					"Package %ls exists in memory but its PackageRoot is not mounted. It will not be cooked.",
					*PackageNameStr);
				return false;
			}
			return true;
...
FPackageData* FPackageDatas::UpdateFileName(FName PackageName)
{
	...
	{
		UE_LOGF(LogCook, Error, "Cannot update FileName for package %ls because the package is no longer mounted.",
	...
	int32& InOutNumBaseGamePackages)
{
	...
	UE_LOGF(LogCook, Display,
		"Cleared the cook results of %d packages because ClearCookResultsForPackages requested them to be recooked.",
		AffectedPackagesCount);
...
TArray<FPendingCookedPlatformData*> FPackageDatas::GetPendingCookedPlatformDatas(UObject* Object)
{
	check(Object);

	TArray<FPendingCookedPlatformData*> Results;
	ForEachPendingCookedPlatformData([&Results, Object](FPendingCookedPlatformData& Data)
		{
			if (Data.Object.GetEvenIfUnreachable() == Object)
			{
				Results.Add(&Data);
			}
		});
	...
	int32& OutNumRetired)
{
	...
	// Future proof as each callback thread gets its own thread_local scope so that the thread_local state
	// (GCookActivePackageName, FInheritedContextExtensionScope) is always manipulated on the correct thread.
	// The atomic counter tracks the total number of open scopes across all threads so we can assert they are
	// all closed by the time ProcessAsyncTasks returns.
	static thread_local TOptional<UE::Cook::FScopedCookActivePackage> ActivePackageScope;
	std::atomic<int32> OpenScopeCount{ 0 };
	FDelegateHandle EventHandle = FAssetCompilingManager::Get().OnPackageScopeEvent().AddLambda(
		[&OpenScopeCount](UPackage* Package, bool bEntering)
		{
			if (bEntering)
			{
				// We need the scope to always be properly balanced
				check(!ActivePackageScope.IsSet());
				ActivePackageScope.Emplace(Package->GetFName(),
				...
				OpenScopeCount.fetch_add(1, std::memory_order_relaxed);
			}
			else
			{
				ActivePackageScope.Reset();
				OpenScopeCount.fetch_sub(1, std::memory_order_relaxed);
			}
		});
	FAssetCompilingManager::Get().ProcessAsyncTasks(true);
	FAssetCompilingManager::Get().OnPackageScopeEvent().Remove(EventHandle);
	// Validate that we don't have any open scopes left after exiting
	check(OpenScopeCount.load(std::memory_order_relaxed) == 0);
...
void FPackageDatas::DebugInstigator(FPackageData& PackageData)
{
	if (ShowInstigatorPackageData == &PackageData)
	{
		...
		UE_LOGF(LogCook, Display, "Instigator chain of %ls: %ls",
			*PackageData.GetPackageName().ToString(), *UE::Cook::InstigatorChainToString(Chain));
	}
...
{
	FPackageData* PackageData = UrgentRequests.Pop();
	if (PackageData)
	{
		return PackageData;
	}
	PackageData = NormalRequests.Pop();
	if (PackageData)
	{
		return PackageData;
	}
