...
public:
	...
	/** Populate OutBuffer with all objects from the given Package; filters out objects not relevant to the cook.  */
	static void CreateObjectCache(TArray<FCachedObjectInOuter>& OutBuffer, UPackage* Package);
	/**
	...
	bool HasBeenLoadedForCooking() const;
	void SetHasBeenLoadedForCooking(bool Value);
...
private:
	...
	uint32 bHasBeenLoadedForCooking : 1;
};
...
using FPackageDataSortedSet = TFifoSet<FPackageData*>;
...
public:
	...
	FPackageDataSortedSet& GetReadyRequestsUrgent();
	FPackageDataSortedSet& GetReadyRequestsNormal();
	/**
...
private:
	...
	FPackageDataSortedSet UrgentRequests;
	FPackageDataSortedSet NormalRequests;
};
...
public:
	...
	/**
	 * Return pointers to the PendingCookedPlatformDatas for the given object, if any exist. These
	 * pointers are to TArray elements and must not be dereferenced after any modifications to
	 * PendingCookedPlatformData: e.g. AddPendingCookedPlatformData or PollPendingCookedPlatformDatas.
	 */
	TArray<FPendingCookedPlatformData*> GetPendingCookedPlatformDatas(UObject* Object);
...
inline bool FPackageData::HasBeenLoadedForCooking() const
{
	return static_cast<bool>(bHasBeenLoadedForCooking);
}

inline void FPackageData::SetHasBeenLoadedForCooking(bool Value)
{
	bHasBeenLoadedForCooking = Value != 0;
}
...
inline FPackageDataSortedSet& FRequestQueue::GetReadyRequestsUrgent()
{
	return UrgentRequests;
}

inline FPackageDataSortedSet& FRequestQueue::GetReadyRequestsNormal()
{
