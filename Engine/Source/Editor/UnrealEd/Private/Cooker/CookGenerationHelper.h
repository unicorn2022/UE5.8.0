 ...
 */
struct FGenerationHelper : public UE::Private::FQueryableRefCountedObject
{
public:
	...
	/**
	 * Called when BeginCacheForCookedPlatformData called on a UObject, and IsCachedCookedPlatformDataLoaded does
	 * not immediately return true, for the given Object assigned to the given PackageData (but possibly a child of
	 * another package). The GenerationHelper will need to coordinate save fences if the object is a child
	 * of a different generated package owned by the generator than the one it is assigned to.
	 */
	void OnAddPendingCookedPlatformData(UCookOnTheFlyServer& COTFS, FPackageData& AssignedPackageData, UObject* Obj);
	/**
	 * Adds the object to a list that is checked before any save-state action of the generator or its generated
	 * packages. If the object still has an FPendingCookedPlatformData, the action is blocked and the package
	 * save is deferred as it would be if there were a FPendingCookedPlatformData assigned to the package. This
	 * is used for objects that move between generator and generated packages in edge-case flows that we
	 * don't support efficiently but need to handle for robustness.
	 */
	void AddBlockingPendingCookedPlatformDataObject(UObject* Object);
	/**
	 * Removes completed objects from the BlockingObjects list and returns whether any remain; it may leave
	 * some completed objects in the list if others are incomplete.
	 */
	bool HasBlockingPendingCookedPlatformDataObjects();
	/**
	 * A PendingCookedPlatformData has detected an object moved out of the given package with this generation helper.
	 * Return whether the pendingCookedPlatformData should keep waiting on it. If not we will stop polling it and will
	 * no longer let it block the save of its package.
	 */
	bool ShouldKeepWaitingOnMovedPendingCookedPlatformData(FPendingCookedPlatformData& Pending,
...
private:
	...
	TArray<FWeakObjectPtr> BlockingPendingCookedPlatformDatas;
	...
	const ITargetPlatform* TargetPlatform)
{
	...
	if (!PlatformData)
	{
		UE_LOGF(LogCook, Error,
			"FCookGenerationInfo::FindCheckedPlatformData called for %ls on %ls, but doesn't already have it. This is a cook programming error.",
	...
	const ITargetPlatform* TargetPlatform)
{
	...
	{
		UE_LOGF(LogCook, Error,
			"FGenerationHelper::FindCheckedPlatformData called for %ls on %ls, but doesn't already have it. This is a cook programming error.",
	...
	const ITargetPlatform* TargetPlatform)
{
	...
	{
		UE_LOGF(LogCookGenerationHelper, Verbose, "%ls: SetKeepForIncremental for platform %ls.",
	...
	const ITargetPlatform* TargetPlatform)
{
	if (ReferenceFromKeepForIncremental)
	{
		UE_LOGF(LogCookGenerationHelper, Verbose, "%ls: ClearKeepForIncremental for platform %ls.",
			*WriteToString<256>(GenerationHelper.GetOwner().GetPackageName()), *TargetPlatform->PlatformName());
...
inline void FGenerationHelper::SetKeepForQueueResults()
{
	...
	{
		UE_LOGF(LogCookGenerationHelper, Verbose, "%ls: SetKeepForQueueResults.",
...
inline void FGenerationHelper::ClearKeepForQueueResults()
{
	if (ReferenceFromKeepForQueueResults)
	{
		UE_LOGF(LogCookGenerationHelper, Verbose, "%ls: ClearKeepForQueueResults.",
			*WriteToString<256>(GetOwner().GetPackageName()));
	...
	const ITargetPlatform* TargetPlatform)
{
	...
	{
		UE_LOGF(LogCookGenerationHelper, Verbose, "%ls: SetKeepForGeneratorSave for platform %ls.",
			*WriteToString<256>(GenerationHelper.GetOwner().GetPackageName()), *TargetPlatform->PlatformName());
	...
	const ITargetPlatform* TargetPlatform)
{
	if (ReferenceFromKeepForGeneratorSave)
	{
		UE_LOGF(LogCookGenerationHelper, Verbose, "%ls: ClearKeepForGeneratorSave for platform %ls.",
			*WriteToString<256>(GenerationHelper.GetOwner().GetPackageName()), *TargetPlatform->PlatformName());
...
inline void FGenerationHelper::SetKeepForAllSavedOrGC()
{
	...
	{
		UE_LOGF(LogCookGenerationHelper, Verbose, "%ls: SetKeepForAllSavedOrGC.",
			*WriteToString<256>(GetOwner().GetPackageName()));
...
inline void FGenerationHelper::ClearKeepForAllSavedOrGC()
{
	if (bKeepForAllSavedOrGC)
	{
		UE_LOGF(LogCookGenerationHelper, Verbose, "%ls: ClearKeepForAllSavedOrGC.",
			*WriteToString<256>(GetOwner().GetPackageName()));
...
inline void FGenerationHelper::SetKeepForCompletedAllSavesMessage()
{
	...
	{
		UE_LOGF(LogCookGenerationHelper, Verbose, "%ls: SetKeepForCompletedAllSavesMessage.",
			*WriteToString<256>(GetOwner().GetPackageName()));
...
inline void FGenerationHelper::ClearKeepForCompletedAllSavesMessage()
{
	if (bKeepForCompletedAllSavesMessage)
	{
		UE_LOGF(LogCookGenerationHelper, Verbose, "%ls: ClearKeepForCompletedAllSavesMessage.",
			*WriteToString<256>(GetOwner().GetPackageName()));
