...
{
	// Add the edge; to save time don't check for uniqueness. Uniqueness is checked on demand, in
	// UniquifyNodePrereqsEdgeList.
	NodePrereqs.FindOrAdd(SourceID).Add(TargetID);
}
...
void FEDLCookChecker::UniquifyNodePrereqsEdgeList(TArray<FEDLNodeID>& EdgeList)
{
	bool bNeedsShrink = false;

	// Avoid the cost of sort+unique if unnecessary, to reduce the cost if called multiple times.
	// This loop is similar to !Algo::IsSorted, but also returns true if any adjacent elements are equal.
	int32 NumToCompare = EdgeList.Num() - 1;
	for (int32 Index = 0; Index < NumToCompare; ++Index)
	{
		if (EdgeList[Index + 1] <= EdgeList[Index])
		{
			bNeedsShrink = true;
			break;
		}
	...
	{
		Algo::Sort(EdgeList);
		EdgeList.SetNum(Algo::Unique(EdgeList));
	}
}
...
bool FEDLCookChecker::CheckForCyclesInner(TSet<FEDLNodeID>& Visited, TSet<FEDLNodeID>& Stack, const FEDLNodeID& Visit, FEDLNodeID& FailNode)
{
	...
	else
	{
		...
		{
			Stack.Add(Visit);
			TArray<FEDLNodeID>* EdgeList = NodePrereqs.Find(Visit);
			if (EdgeList)
			{
				for (int32 EdgeListIndex = 0; !bResult && EdgeListIndex < EdgeList->Num(); ++EdgeListIndex)
				{
					bResult = CheckForCyclesInner(Visited, Stack, (*EdgeList)[EdgeListIndex], FailNode);
				}
			}
			Stack.Remove(Visit);
		}
	}
	UE_CLOGF(bResult && Stack.Contains(FailNode), LogSavePackage, Error, "Cycle Node %ls", *Nodes[Visit].ToString(*this));
	return bResult;
}
...
void FEDLCookChecker::Merge(FEDLCookChecker&& Other)
{
	...
	else
	{
		...
		for (const TPair<FEDLNodeID, TArray<FEDLNodeID>>& Prereq : Other.NodePrereqs)
		{
			// Merge the edgelists; to save time don't check for uniqueness. Uniqueness is checked on demand, in
			// UniquifyNodePrereqsEdgeList.
			FEDLNodeID SourceID = RemapIDs[Prereq.Key];
			TArray<FEDLNodeID>& EdgeList = NodePrereqs.FindOrAdd(SourceID);
			for (FEDLNodeID OtherTargetID : Prereq.Value)
			{
				FEDLNodeID TargetID = RemapIDs[OtherTargetID];
				EdgeList.Add(TargetID);
			}
		}
...
void FEDLCookChecker::Verify(const TFunction<void(UE::FLogRecord&& Record)>& MessageCallback,
	const TFunction<FString(FName PackageName)>& GetInstigatorChain,
	bool bFullReferencesExpected)
{
	...
	if (Accumulator.bIsActive)
	{
		...
		if (bFullReferencesExpected)
		{
			...
			for (const FEDLNodeData& NodeData : Accumulator.Nodes)
			{
				...
				for (FName PackageName : NodeData.ImportingPackagesSorted)
				{
					...
					Record.SetUtf8Format(UTF8TEXT("Content is missing from cook. Source package referenced an object in target package but {Reason}.")
						UTF8TEXT("\n\tSource package: {Source}")
						UTF8TEXT("\n\tTarget package: {Target}")
						UTF8TEXT("\n\tReferenced object: {ReferencedObject}")
						UTF8TEXT("\n\tSourceInstigator: {SourceInstigator}")
					);
					{
						FCbWriter Writer;
						Writer.BeginObject();
						...
						Writer << "SourceInstigator" << GetInstigatorChain(PackageName);
						Writer.EndObject();
		...
		for (TPair<FEDLNodeID, TArray<FEDLNodeID>>& NodePreqsPair : Accumulator.NodePrereqs)
		{
			UniquifyNodePrereqsEdgeList(NodePreqsPair.Value);
		}
		for (const FEDLNodeData& NodeData : Accumulator.Nodes)
		{
			if (!NodeData.bIsExport)
			...
			{
				UE_LOGF(LogSavePackage, Error, "----- %ls contained a cycle (listed above).", *Accumulator.Nodes[FailNode].ToString(Accumulator));
				bHadCycle = true;
			}
		}
		if (bHadCycle)
		{
			UE_LOGF(LogSavePackage, Fatal, "EDL dep graph contained a cycle (see errors, above). This is fatal at runtime so it is fatal at cook time.");
		}
		UE_LOGF(LogSavePackage, Display, "Took %fs to verify the EDL loading graph.", float(FPlatformTime::Seconds() - StartTime));
	}
}
...
void FEDLCookChecker::WriteToCompactBinary(FCbWriter& Writer)
{
	Writer.BeginObject();
	{
		...
		// Transmit NumSources to use for reservation size
		uint32 NumSources = static_cast<uint32>(NodePrereqs.Num());
		Writer << NumSources;
		// Transmit the edgelist as (Source, NumEdgeList, EdgeListElements). Use a flat list of concatenated
		// objects to reduce transmission size.
		for (TPair<FEDLNodeID, TArray<FEDLNodeID>>& Pair : NodePrereqs)
		{
			Writer << static_cast<uint32>(Pair.Key);
			TArray<FEDLNodeID>& EdgeList = Pair.Value;
			UniquifyNodePrereqsEdgeList(EdgeList);
			uint32 NumDests = static_cast<uint32>(EdgeList.Num());
			Writer << NumDests;
			for (FEDLNodeID Dest : EdgeList)
			{
				Writer << static_cast<uint32>(Dest);
			}
		}
		Writer.EndArray();
...
bool FEDLCookChecker::ReadFromCompactBinary(FCbFieldView Field)
{
	...
	uint32 NumSources = 0;
	if (!LoadFromCompactBinary(PrereqsIter++, NumSources)) { return false; }
	NodePrereqs.Reserve(static_cast<int32>(NumSources));
	while (PrereqsIter)
	{
		uint32 SourceAsInt;
		if (!LoadFromCompactBinary(PrereqsIter++, SourceAsInt)) { return false; }
		uint32 NumDests;
		if (!LoadFromCompactBinary(PrereqsIter++, NumDests)) { return false; }
		TArray<FEDLNodeID>& EdgeList = NodePrereqs.FindOrAdd(static_cast<FEDLNodeID>(SourceAsInt));
		EdgeList.Reserve(NumDests);
		for (uint32 DestIndex = 0; DestIndex < NumDests; ++DestIndex)
		{
			uint32 DestAsInt;
			if (!LoadFromCompactBinary(PrereqsIter++, DestAsInt)) { return false; }
			EdgeList.Add(DestAsInt);
		}
	}
