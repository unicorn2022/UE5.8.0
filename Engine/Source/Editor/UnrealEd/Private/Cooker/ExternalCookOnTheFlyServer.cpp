...
void FExternalCookOnTheFlyServer::HandleRegisterServiceMessage(const FZenCookOnTheFlyRegisterServiceMessage& Message, const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& Context)
{
	...
	{
		UE_LOGF(LogCookOnTheFly, Verbose, "Rejecting service from %ls", *Context->GetSender().ToString());
		return;
	}
	UE_LOGF(LogCookOnTheFly, Verbose, "Accepting service from %ls", *Context->GetSender().ToString());
	...
	{
		UE_LOGF(LogCookOnTheFly, Display, "Connected to server");
		AssetRegistry.OnAssetUpdatedOnDisk().AddRaw(this, &FExternalCookOnTheFlyServer::AssetUpdatedOnDisk);
	}
	else
	{
		UE_LOGF(LogCookOnTheFly, Warning, "Failed connecting to server");
	}
}
...
{
	{
		...
		if (!AllPackagesToRecook.IsEmpty())
		{
			...
			if (!Response.IsOk())
			{
				UE_LOGF(LogCookOnTheFly, Warning, "Failed to send RecookPackages request");
			}
		}
	}
	if (CookOnTheFlyServerConnection && !CookOnTheFlyServerConnection->IsConnected())
	{
		...
		UE_LOGF(LogCookOnTheFly, Display, "Disconnected from server");
	}
}
