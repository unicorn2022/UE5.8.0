...
void UCookOnTheFlyServer::SaveGlobalShaderMapFiles(const TArrayView<const ITargetPlatform* const>& Platforms, ODSCRecompileCommand RecompileCommand)
{
	...
	for (const ITargetPlatform* TargetPlatform : Platforms)
	{
		...
		UE_LOGF(LogCook, Display, "Compiling global%ls shaders for platform '%ls'",
...
void FShaderLibraryCollector::ClientTick(FMPCollectorClientTickContext& Context)
{
	...
	for (int32 NumMessages = 0; ; ++NumMessages)
	{
		...
		{
			UE_LOGF(LogCook, Warning, "FShaderLibraryCollector::ClientTick has an unexpectedly high number of loops. Infinite loop?")
		}
	}
}
...
void FShaderLibraryCollector::ServerReceiveMessage(FMPCollectorServerMessageContext& Context, FCbObjectView Message)
{
	...
	UE_CLOGF(!bSuccessful, LogCook, Error,
		"Corrupt message received from CookWorker when replicating ShaderLibrary. Shaders will be missing from the cook.");
}
...
void FShaderLibraryCookArtifact::UpdateOplogPackages(UE::Cook::Artifact::FUpdateOplogPackagesContext& Context)
{
	FShaderLibraryCooker::UpdateOplogPackages(Context);
}
...
{
	UE_SCOPED_COOKTIMER(RegisterShaderLibraryIncrementalCookArtifact);
...
{
	UE_SCOPED_COOKTIMER(BeginCookStartShaderCodeLibrary);
	const UProjectPackagingSettings* const PackagingSettings = GetDefault<UProjectPackagingSettings>();
	if (IsUsingShaderCodeLibrary())
	{
		...
		// See if we desire stable keys in DLC, PSO manual cache in DLC is not supported, so they are disabled by default
		// They have other uses so allow them to be optionally enabled
		bool bAllowShaderStableKeysInDLC = false;
		GConfig->GetBool(TEXT("DevOptions.Shaders"), TEXT("bAllowShaderStableKeysInDLC"), bAllowShaderStableKeysInDLC, GEngineIni);
		bAllPlatformsNeedStableKeys = bAllPlatformsNeedStableKeys && (!IsCookingDLC() || bAllowShaderStableKeysInDLC);
...
{
	UE_SCOPED_COOKTIMER(BeginCookFinishShaderCodeLibrary);
...
void UCookOnTheFlyServer::CreatePipelineCache(const ITargetPlatform* TargetPlatform, const FString& LibraryName)
{
	...
	if (SCLCSVPaths && SCLCSVPaths->Num())
	{
		...
		for (FName ShaderFormat : ShaderFormats)
		{
			...
			{
				UE_LOGF(LogCook, Display, "---- NOT Running UShaderPipelineCacheToolsCommandlet for platform %ls  shader format %ls, no files found at %ls, and either no stable keys or not including compute PSOs during the cook", *TargetPlatformName, *ShaderFormat.ToString(), *StablePCDir);
			}
			else
			{
				UE_LOGF(LogCook, Display, "---- Running UShaderPipelineCacheToolsCommandlet for platform %ls  shader format %ls", *TargetPlatformName, *ShaderFormat.ToString());
				...
				{
					UE_LOGF(LogCook, Warning, "Deleting %ls, cooked data doesn't belong here.", *PCUncookedPath);
				...
				if (!NumMatched)
				{
					UE_LOGF(LogCook, Warning, "Shader format %ls for platform %ls had stable pipeline cache files, but no stable keys files.", *ShaderFormat.ToString(), *TargetPlatformName);
					for (int32 Index = 0; Index < SCLCSVPaths->Num(); Index++)
					{
						UE_LOGF(LogCook, Warning, "    stable keys file: %ls", *((*SCLCSVPaths)[Index]));
					}
					continue;
				}
				...
				Args += PCPath;
				Args += TEXT("\"");
				UE_LOGF(LogCook, Display, "  With Args: %ls", *Args);
				...
				{
					UE_LOGF(LogCook, Display, "---- Done running UShaderPipelineCacheToolsCommandlet for platform %ls", *TargetPlatformName);
					...
					if (IFileManager::Get().FileExists(*PCPath))
					{
						...
						UE_LOGF(LogCook, Display, "Copying the binary PSO cache file %ls to %ls.", *PCPath, *PipelineCacheMetadataFileName);
						if (IFileManager::Get().Copy(*PipelineCacheMetadataFileName, *PCPath) != COPY_OK)
						{
							UE_LOGF(LogCook, Warning, "Failed to copy the binary PSO cache file %ls to %ls.", *PCPath, *PipelineCacheMetadataFileName);
						}
					}
				}
			}
...
void UCookOnTheFlyServer::SaveAndCloseGlobalShaderLibrary()
{
	...
	if (bCacheShaderLibraries)
	{
		...
		for (const ITargetPlatform* TargetPlatform : PlatformManager->GetSessionPlatforms())
		{
			FinishPopulateShaderLibrary(TargetPlatform, GlobalShaderLibName, false /* bInitializeReferenceTracking */);
...
void UCookOnTheFlyServer::FinishPopulateShaderLibrary(const ITargetPlatform* TargetPlatform, const FString& Name, bool bInitializeReferenceTracking)
{
	FString ShaderCodeDir;
	FString MetaDataPath;
	GetShaderLibraryPaths(TargetPlatform, ShaderCodeDir, MetaDataPath);

	FShaderLibraryCooker::FinishPopulateShaderLibrary(TargetPlatform, Name, ShaderCodeDir, MetaDataPath, bInitializeReferenceTracking);
}

void UCookOnTheFlyServer::PruneShaderLibrary(const ITargetPlatform* TargetPlatform, const FString& Name)
{
	FShaderLibraryCooker::PruneShaderLibrary(TargetPlatform, Name);
}
...
void UCookOnTheFlyServer::SaveShaderLibrary(const ITargetPlatform* TargetPlatform, const FString& Name)
{
	...
	FString ErrorString;
	bool bHasData;
	UE::Cook::EPackagingSystem PackagingSystem = IsUsingIoStore() ? UE::Cook::EPackagingSystem::IoStore
	...
	if (!FShaderLibraryCooker::SaveShaderLibraryWithoutChunking(TargetPlatform, Name, ShaderCodeDir, MetaDataPath,
		PlatformSCLCSVPaths, PackagingSystem, ErrorString, bHasData))
	{
	...
	else if (bHasData)
	{
		...
		{
			UE_LOGF(LogCook, Display, "Saved scl.csv %ls for platform %ls, %lld bytes", *Item, *TargetPlatform->PlatformName(),
...
void UCookOnTheFlyServer::DumpShaderTypeStats(const ITargetPlatform* TargetPlatform, const FString& PlatformNameString)
{
	using namespace UE::Cook;

	auto GetCookStatus = [this, TargetPlatform]
	(FName AssetName, bool& bOutImplemented, bool& bOutReferenced, bool& bOutRecooked)
		{
			bOutImplemented = true;
			bOutReferenced = false;
			bOutRecooked = false;

			FPackageData* PackageData = PackageDatas->FindPackageDataByPackageName(AssetName);
			if (!PackageData)
			{
				return;
			}
			FPackagePlatformData& PlatformData = PackageData->FindOrAddPlatformData(TargetPlatform);
			bOutReferenced = PlatformData.IsCookSucceeded();
			bOutRecooked = !PlatformData.IsIncrementallySkipped();
		};
