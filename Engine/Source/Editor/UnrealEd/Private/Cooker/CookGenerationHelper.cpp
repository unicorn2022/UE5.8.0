...
void ICookPackageSplitter::WarnIfDeprecatedVirtualNotCalled(const TCHAR* FunctionName)
{
	...
	if (!bDeprecatedVirtualCalledAsExpected)
	{
		UE_LOGF(LogCook, Warning,
			"ICookPackageSplitter::%ls has been deprecated. Implement version that takes FPopulateContext instead.",
			FunctionName);
	}
...
{
	UE_LOGF(LogCookGenerationHelper, Verbose, "%ls: Destructed.",
	...
	TUniquePtr<ICookPackageSplitter>&& InCookPackageSplitterInstance)
{
	...
	if (InitializeStatus != EInitializeStatus::Uninitialized)
	{
		UE_LOGF(LogCookGenerationHelper, Verbose, "%ls: Redundant initialize ignored.",
			*WriteToString<256>(GetOwner().GetPackageName()));
	...
	UE_LOGF(LogCookGenerationHelper, Verbose, "%ls: Initialized.", *WriteToString<256>(GetOwner().GetPackageName()));
}
...
void FGenerationHelper::Uninitialize()
{
	...
	UE_LOGF(LogCookGenerationHelper, Verbose, "%ls: Uninitialized.", *WriteToString<256>(GetOwner().GetPackageName()));
...
void FGenerationHelper::OnNumSavedUpdated()
{
	...
	UE_LOGF(LogCookGenerationHelper, Verbose, "%ls: All saves completed (%d/%d).",
	...
	auto ConditionalDemote = [&COTFS, &bFromAllSavesCompleted, this](FCookGenerationInfo& Info)
		{
			if (Info.PackageData->IsStalled())
			{
				...
				if (Info.PackageData->GetState() == EPackageState::SaveStalledAssignedToWorker)
				{
					...
					if (bFromAllSavesCompleted)
					{
						...
						UE_LOGF(LogCook, Error,
							"Package %ls is still stalled on the CookDirector during FGenerationHelper::OnAllSavesCompleted. " "This is unexpected; all stalled packages on a GenerationHelper should have completed saving and therefore unstalled before OnAllSavesCompleted is called." "\n\tNumAllSaved == %d. NumSaved == %d. Info.HasSaved == %ls.",
					...
					else
					{
						...
						UE_LOGF(LogCookGenerationHelper, Verbose,
							"%ls generated package %ls: DemoteStalledPackage from SaveStalledAssignedToWorker to AssignedToWorker.",
							*WriteToString<256>(GetOwner().GetPackageName()),
				...
				else
				{
					...
					UE_LOGF(LogCookGenerationHelper, Verbose, "%ls generated package %ls: DemoteStalledPackage from %ls to Idle.",
						*WriteToString<256>(GetOwner().GetPackageName()),
						*WriteToString<256>(Info.PackageData->GetPackageName()),
...
void FGenerationHelper::ValidateSaveStalledState(UCookOnTheFlyServer& COTFS, FPackageData& PackageData, const TCHAR* Caller)
{
	...
	if (PackageData.GetState() != ExpectedState)
	{
		...
		UE_LOGF(LogCook, Error,
			"In %ls, package %ls is stalled on %ls, but is in state %ls. We expect stalled packages on %ls to be in state %ls.",
			Caller,
...
void FGenerationHelper::DiagnoseWhyNotShutdown()
{
	...
	if (IsInitialized())
	{
		UE_LOGF(LogCook, Error, "%ls", *Message);
	}
	else
	{
		UE_LOGF(LogCook, Warning, "%ls", *Message);
	}
	UE_LOGF(LogCook, Display, "Diagnostics:\n%ls", *Lines);
}
	...
	bool& bOutNeedWaitForIsLoaded)
{
		...
		bCookedPlatformDataIsLoaded, &bOutNeedWaitForIsLoaded](UObject* Obj)
		{
			...
			for (Private::FRegisteredCookPackageSplitter* SplitterForObject : FoundRegisteredSplitters)
			{
				...
				if (SplitterForObject && SplitterForObject->ShouldSplitPackage(Obj))
				{
					...
					{
						UE_LOGF(LogCook, Error,
							"SplitterData object %ls must be publicly referenceable so we can keep them from being garbage collected",
							*Obj->GetFullName());
						return false;
					}
					...
					{
						UE_LOGF(LogCook, Error,
							"Found more than one registered Cook Package Splitter for package %ls.",
	...
	ICookPackageSplitter* SplitterInstance = SplitterType->CreateInstance(Package, LocalSplitDataObject);
	if (!SplitterInstance)
	{
		UE_LOGF(LogCook, Error, "Error instantiating Cook Package Splitter %ls for object %ls.",
...
bool FGenerationHelper::TryReportGenerationManifest()
{
	...
	if (!IsValid())
	{
		...
		UE_LOGF(LogCook, Error,
			"TryReportGenerationManifest failed for package %ls: Called on an invalid FGenerationHelper.",
	...
	if (!OwnerObject)
	{
		...
		UE_LOGF(LogCook, Error,
			"TryReportGenerationManifest failed for package %ls: Valid GenerationHelper but could not find OwnerObject.",
			*OwnerPackageName.ToString());
		FDebug::DumpStackTraceToLog(ELogVerbosity::Warning);
		return false;
	}
	...
	{
		UE::Cook::FScopedCookActivePackage ScopedActivePackage(OwnerPackageName,
		...
		if (Manifest.GeneratedPackages.IsEmpty())
		{
			...
			if (!GeneratorDatas.IsEmpty())
			{
				...
				UE_LOGF(LogCook, Warning,
					"%ls is overriding deprecated ICookPackageSplitter::GetGenerateList. Implement ReportGenerationManifest instead. - Please update your code to the new API before upgrading to the next release, otherwise your project will no longer compile.",
	...
	for (ICookPackageSplitter::FGeneratedPackage& SplitterData : Manifest.GeneratedPackages)
	{
		...
		{
			UE_LOGF(LogCook, Error,
				"PackageSplitter did not specify whether CreateAsMap is true for generated package. Splitter=%ls, Generated=%ls.",
		...
		{
			UE_LOGF(LogCook, Error,
				"PackageSplitter could not find mounted filename for generated packagepath. Splitter=%ls, Generated=%ls.",
		...
		{
			UE_LOGF(LogCook, Warning,
				"PackageSplitter specified a generated package that already exists in the workspace domain. Splitter=%ls, Generated=%ls.",
				*this->GetSplitDataObjectName().ToString(), *PackageName);
			return false;
		}
		...
		for (TArray<FAssetDependency>::TIterator Iter(GeneratedInfo->PackageDependencies); Iter; ++Iter)
		{
			...
			{
				UE_LOGF(LogCook, Error,
					"PackageSplitter specified a dependency with category %d rather than category Package. Dependency will be ignored. Splitter=%ls, Generated=%ls.",
			...
			{
				UE_LOGF(LogCook, Error,
					"PackageSplitter specified a dependency for one generated package on another generated package. Only dependencies on non-generated packages are allowed. Dependency will be ignored. Splitter=%ls, Generated=%ls, Dependency=%ls.",
	...
	if (!AlreadyExistingInfoPackageToIndex.IsEmpty())
	{
		...
		for (TPair<FPackageData*, int32>& Pair : AlreadyExistingInfoPackageToIndex)
		{
			...
			UE_LOGF(LogCook, Warning,
				"Unexpected generated package (local TryReportGenerationManifest). A remote cookworker reported generated package %ls for generator %ls," " but when TryReportGenerationManifest was called on the CookDirector, this package was not listed." " This is unexpected and causes minor performance problems in the cook.",
	...
	UE_LOGF(LogCookGenerationHelper, Verbose,
		"%ls: TryReportGenerationManifest: %d packages to generate, %d previously recorded, %d/%d already saved.",
...
UObject* GetObjectFromObjectHolder(UObject*const& ObjectHolder)
{
	return ObjectHolder;
}
UObject* GetObjectFromObjectHolder(const FCachedObjectInOuter& ObjectHolder)
{
	return ObjectHolder.Object.Get();
}
template <typename ObjectHolderType>
TMap<UObject*, UPackage*> GetPackagesForObjects(TConstArrayView<ObjectHolderType> Objects)
{
	TMap<UObject*, UPackage*> PackageForObject;
	PackageForObject.Reserve(Objects.Num());
	TArray<UObject*, TInlineAllocator<10>> ChildStack;
	for (const ObjectHolderType& ObjectHolder : Objects)
	{
		check(ChildStack.IsEmpty());
		UObject* Object = GetObjectFromObjectHolder(ObjectHolder);
		if (!Object)
		{
			continue;
		}
		...
		for (;;)
		{
			UPackage** PackagePtr = PackageForObject.Find(Object);
			Package = PackagePtr ? *PackagePtr : nullptr;
			if (Package)
			{
				break;
			}
			...
			if (Package)
			{
				PackageForObject.Add(Object, Package);
				break;
			}
			...
			ChildStack.Add(Object);
			Object = Outer;
		}
		check(Package);
		for (UObject* ChildObject : ChildStack)
		{
			PackageForObject.Add(ChildObject, Package);
		}
		ChildStack.Reset();
	}
	return PackageForObject;
}
	...
	bool bPopulateCallAllowed)
{
	...
	if (!bReportedManifest)
	{
		// Unexpected, caller should not call in this case
		UE_LOGF(LogCook, Error,
			"TryCallPopulateGeneratorPackage called for package %ls without a previous successful call to TryReportGenerationManifest.",
			*OwnerPackageName.ToString());
		FDebug::DumpStackTraceToLog(ELogVerbosity::Warning);
		return false;
	}
	...
	{
		UE_LOGF(LogCook, Error,
			"Failed to call PopulateGeneratorPackage, CookPackageSplitter missing. Splitter=%ls",
	...
	{
		UE_LOGF(LogCook, Error,
			"PackageSplitter unexpected failure: could not ConstructGeneratedPackagesForPreSave. Splitter=%ls",
			*GetSplitDataObjectName().ToString());
		return false;
	}
	...
	if (bPopulateCallAllowed)
	{
		...
		// Get the list of Objects in the package. Read them from the PackageData if it already has them,
		// otherwise calculate them into a temporary buffer.
		TArray<FCachedObjectInOuter> ObjectsInOuterBuffer;
		TConstArrayView<FCachedObjectInOuter> ObjectsInOuter;
		if (OwnerPackageData.GetHasSaveCache())
		{
			ObjectsInOuter = OwnerPackageData.GetCachedObjectsInOuter();
		}
		else
		{
			OwnerPackageData.CreateObjectCache(ObjectsInOuterBuffer, LocalOwnerPackage);
			ObjectsInOuter = ObjectsInOuterBuffer;
		}
		...
		{
			UE_LOGF(LogCook, Error, "CookPackageSplitter returned false from PopulateGeneratorPackage. Splitter=%ls",
				*GetSplitDataObjectName().ToString());
			return false;
		}
		UE_LOGF(LogCookGenerationHelper, Verbose, "%ls: PopulateGeneratorPackage.",
			*WriteToString<256>(GetOwner().GetPackageName()));

		// PopulateGenerator may have moved objects out of the package; detect them and add them to ObjectsToMove.
		bool bMovedObjects = false;
		TMap<UObject*, UPackage*> NewPackageForObject = GetPackagesForObjects(ObjectsInOuter);
		for (const TPair<UObject*, UPackage*>& PackagePair : NewPackageForObject)
		{
			UPackage* NewPackage = PackagePair.Value;
			check(NewPackage);
			if (NewPackage == LocalOwnerPackage)
			{
				continue;
			}
			FPackageData* NewPackageData = COTFS.PackageDatas->FindPackageDataByPackageName(NewPackage->GetFName());
			if (!NewPackageData)
			{
				// Object moved into a temporary or other package that the cooker does not track. The cooker will
				// treat it as still being in the package; SavePackage will make it an import if referenced.
				continue;
			}
			FCookGenerationInfo* Info = FindInfo(*NewPackageData);
			if (!Info)
			{
				// Object moved into another cookable package, but not a generated package of this generator.
				// During cook of the generator we will treat it as still being in the generator package; SavePackage
				// will make it an import if referenced.
				continue;
			}
			bMovedObjects = true;
			PopulateData.ObjectsToMove.Add(PackagePair.Key);
		}
		if (bMovedObjects)
		{
			// Enforce uniqueness on ObjectsToMove since we added without checking for duplicates
			PopulateData.ObjectsToMove.Sort();
			PopulateData.ObjectsToMove.SetNum(Algo::Unique(PopulateData.ObjectsToMove));
		}

		// If PopulateGenerator already moved any of the UObjects in the generator package out into generated packages,
		// keep those packages referenced until the generator package is destroyed, since we cannot reload
		// the generator package to recreate them until that happens. This behavior is only valid in the case where the
		// splitter has informed us !RequiresEmptyPackageBeforePopulate; otherwise it should not be accessing the
		// generated packages until their own populate calls.
		bMovedObjects = false;
		NewPackageForObject = GetPackagesForObjects<UObject*>(PopulateData.ObjectsToMove);
		for (const TPair<UObject*, UPackage*>& PackagePair : NewPackageForObject)
		{
			UPackage* NewPackage = PackagePair.Value;
			check(NewPackage);
			...
			check(Info != &OwnerInfo); // NewPackage != LocalOwnerPackage, so Info != OwnerInfo
			if (CookPackageSplitterInstance->RequiresEmptyPackageBeforePopulate(NewPackage))
			{
				UE_LOGF(LogCook, Error,
					"CookPackageSplitter moved an object from the generator package into a generated package during PopulateGeneratorPackage. " "This behavior is only allowed if RequiresEmptyPackageBeforePopulate returns false for the generated package. " "Edit the splitter code to either not move the object during Populate, or to return false for RequiresEmptyPackageBeforePopulate." "\n\tSplitter=%ls" "\n\tMoved Object=%ls",
					*GetSplitDataObjectName().ToString(), *PackagePair.Key->GetPathName());
			}
			else
			{
				bMovedObjects = true;
				PopulateData.KeepReferencedPackages.Add(NewPackage);
			}
		}
		if (bMovedObjects)
		{
			// Enforce uniqueness on KeepReferencedPackages since we added without checking for duplicates
			PopulateData.KeepReferencedPackages.Sort();
			PopulateData.KeepReferencedPackages.SetNum(Algo::Unique(PopulateData.KeepReferencedPackages));
		}
	...
	{
		UE_LOGF(LogCookGenerationHelper, Verbose, "%ls: PopulateGeneratorPackage Skipped because !IsSaveGenerator().",
			*WriteToString<256>(GetOwner().GetPackageName()));
	...
	TArray<UObject*>& OutObjectsToMove)
{
	...
	if (!bReportedManifest)
	{
		// Unexpected, caller should not call in this case
		UE_LOGF(LogCook, Error,
			"TryCallPopulateGeneratedPackage called for package %ls without a previous successful call to TryReportGenerationManifest.",
			*Info.GetPackageName());
		FDebug::DumpStackTraceToLog(ELogVerbosity::Warning);
		return false;
	}
	...
	{
		UE_LOGF(LogCook, Error,
			"Failed to call TryCallPopulateGeneratedPackage, CookPackageSplitter missing. Splitter=%ls",
			*GetSplitDataObjectName().ToString());
		return false;
	}
	...
	{
		UE_LOGF(LogCook, Error,
			"CookPackageSplitter returned false from PopulateGeneratedPackage. Splitter=%ls" "\nGeneratedPackage: %ls",
	...
	UE_LOGF(LogCookGenerationHelper, Verbose, "%ls generated package %ls: PopulateGeneratedPackage.",
		*WriteToString<256>(GetOwner().GetPackageName()),
		*WriteToString<256>(Info.PackageData->GetPackageName()));
...
void FGenerationHelper::StartOwnerSave()
{
	...
	UE_LOGF(LogCook, Display, "Splitting Package %ls with splitter %ls acting on object %ls.",
		*WriteToString<256>(GetOwner().GetPackageName()),
		*GetRegisteredSplitterType()->GetSplitterDebugName(),
...
void FGenerationHelper::StartQueueGeneratedPackages(UCookOnTheFlyServer& COTFS)
{
	...
	UE_LOGF(LogCookGenerationHelper, Verbose, "%ls: StartQueueGeneratedPackages.",
		*WriteToString<256>(GetOwner().GetPackageName()));
		...
		for (TPair<const ITargetPlatform*, FPlatformData>& PlatformPair : PlatformDatas)
		{
			...
			if (bLegacyBuildDependencies)
			{
				UE_LOGF(LogCook, Display,
					"Found %d cooked package(s) in package store for generator package %ls.",
					NumLegacyIterativePrevious, *WriteToString<256>(GetOwner().GetPackageName()));
				UE_LOGF(LogCook, Display, "Keeping %d. Recooking %d. Removing %d.",
...
{
	UE_LOGF(LogCookGenerationHelper, Verbose, "%ls: EndQueueGeneratedPackages.",
		*WriteToString<256>(GetOwner().GetPackageName()));
...
{
	UE_LOGF(LogCookGenerationHelper, Verbose, "%ls: EndQueueGeneratedPackagesOnDirector.",
		*WriteToString<256>(GetOwner().GetPackageName()));
...
void FGenerationHelper::OnRequestFencePassed(UCookOnTheFlyServer& COTFS)
{
	...
	for (TPair<const ITargetPlatform*, FPlatformData>& PlatformPair : PlatformDatas)
	{
		...
		if (OwnerInfo.FindCheckedPlatformData(TargetPlatform).IsIncrementallySkipped())
		{
			...
			UE_LOGF(LogCookGenerationHelper, Verbose,
				"%ls: RequestFencePassed and found to be entirely incrementally skipped for %ls.",
	...
	else
	{
		...
		{
			UE_LOGF(LogCookGenerationHelper, Verbose, "%ls: RequestFencePassed, but not yet HasFinishedQueueGeneratedPackages.",
				*WriteToString<256>(GetOwner().GetPackageName()));
		}
	}
}
...
{
	UE_LOGF(LogCookGenerationHelper, Verbose, "%ls: SkipSaveExistingGenerator.",
		*WriteToString<256>(GetOwner().GetPackageName()));
...
{
	UE_LOGF(LogCookGenerationHelper, Verbose, "%ls: RequestFencePassed, and HasFinishedQueueGeneratedPackages.",
		*WriteToString<256>(GetOwner().GetPackageName()));
...
UPackage* FGenerationHelper::TryCreateGeneratedPackage(FCookGenerationInfo& GeneratedInfo, bool bResetToEmpty)
{
	...
	{
		UE_LOGF(LogCook, Error,
			"TryCreateGeneratedPackage: could not load ParentGeneratorPackage %ls for GeneratedPackage %ls",
	...
	if (GeneratedPackage)
	{
		...
		// them during save.
		if (bResetToEmpty && GetCookPackageSplitterInstance()->RequiresEmptyPackageBeforePopulate(GeneratedPackage))
		{
			TArray<UObject*> ExistingObjects;
			GetObjectsWithPackage(GeneratedPackage, ExistingObjects, EGetObjectsFlags::None);
	...
	TArray<FAssetDependency>& OutPackageDependencies)
{
	...
	UE_LOGF(LogCookGenerationHelper, Verbose, "%ls: Generator package saved.",
		*WriteToString<256>(GetOwner().GetPackageName()));
}
	...
	FBuildResultDependenciesMap& OutBuildResultDependencies)
{
	...
	if (!Info)
	{
		UE_LOGF(LogCook, Error, "GeneratedInfo missing for package %ls.",
			*PackageData.GetPackageName().ToString());
		return;
	}
	UCookOnTheFlyServer& COTFS = Info->PackageData->GetPackageDatas().GetCookOnTheFlyServer();
	...
	UE_LOGF(LogCookGenerationHelper, Verbose, "%ls generated package %ls: Generated package saved for platform %ls.",
		*WriteToString<256>(GetOwner().GetPackageName()),
	...
	EStateChangeReason ReleaseSaveReason, EPackageState NewState)
{
	...
	if (Info.PackageData->GetSaveSubState() > ESaveSubState::Generation_CallPreSave)
	{
		...
		{
			UE_LOGF(LogCook, Warning,
				"PackageSplitter: %ls on %ls was GarbageCollected before we finished saving it. This prevents us from calling PostSave and may corrupt other packages that it altered during Populate. Splitter=%ls.",
		...
		{
			UE::Cook::FScopedCookActivePackage ScopedActivePackage(GetOwner().GetPackageName(),
#if UE_WITH_OBJECT_HANDLE_TRACKING
				PackageAccessTrackingOps::NAME_CookerBuildObject
#else
				FName()
	...
	if (Info.HasTakenOverCachedCookedPlatformData())
	{
		...
		if (!bDemotionAllowed)
		{
			FString DeletedObjectInOuter(TEXT("<None>"));
			int32 NumNulls = 0;
			for (const TPair<UObject*, FCachedObjectInOuterGeneratorInfo>& Pair : Info.CachedObjectsInOuterInfo)
			{
				bool bStillPresent = Info.PackageData->GetCachedObjectsInOuter().ContainsByPredicate(
						[RawPtr = Pair.Key](const FCachedObjectInOuter& CachedObject)
						{
							return CachedObject.Object.Get() == RawPtr;
						});
				if (!bStillPresent)
				{
					if (NumNulls == 0)
					{
						DeletedObjectInOuter = Pair.Value.FullName;
					}
					++NumNulls;
				}
			}
			...
			UE_LOGF(LogCook, Error,
				"CookPackageSplitter failure: We are demoting a %ls package from save and removing our references that keep its objects loaded." "\nThis will allow the objects to be garbage collected and cause failures in the splitter which expects them to remain loaded." "\n\tPackage=%ls, Splitter=%ls, " "\n\tReleaseSaveReason=%ls, CurrentState=%ls, CurrentSubState=%ls, NewState=%ls," "\n\tPackagePtr=%ls, Package->IsFullyLoaded()=%ls," "\n\tOwnerPackagePtr=%ls, OwnerPackage->IsFullyLoaded()=%ls," "\n\tGetCachedObjectsInOuter->Num()==%d, Num nulls in GetCachedObjectsInOuter==%d, First null=='%ls'",
				Info.IsGenerator() ? TEXT("generator") : TEXT("generated"),
				*Info.GetPackageName(),
				*GetSplitDataObjectName().ToString(), LexToString(ReleaseSaveReason),
				LexToString(Info.PackageData->GetState()), LexToString(Info.PackageData->GetSaveSubState()),
				LexToString(NewState),
				Package == nullptr ? TEXT("<null>") : TEXT("<nonnull>"),
				*(::LexToString(Package == nullptr ? false : Package->IsFullyLoaded())),
				LocalOwnerPackage == nullptr ? TEXT("<null>") : TEXT("<nonnull>"),
				* (::LexToString(LocalOwnerPackage == nullptr ? false : LocalOwnerPackage->IsFullyLoaded())),
				Info.PackageData->GetCachedObjectsInOuter().Num(),
				NumNulls, *DeletedObjectInOuter);
	...
	{
		UE_LOGF(LogCook, Error,
			"CookPackageSplitter logic error: A %ls package has !Info.HasTakenOverCachedCookedPlatformData(), but Info.CachedObjectsInOuterInfo.Num() == %d > 0. This should be impossible.\n" "Package=%ls, PackageState=%ls, SaveSubState=%ls",
...
{
	UE_LOGF(LogCookGenerationHelper, Verbose,
		"%ls: SetPreviousGeneratedPackages with %d generated packages for platform %ls.",
...
template <typename T>
static TArrayView<T*> AppendWeakPtrsToObjectPtrArray(TArray<T*>& Out, const TArray<TWeakObjectPtr<T>>& In)
{
	Out.Reserve(Out.Num() + In.Num());
	int32 StartIndex = Out.Num();
	...
	return TArrayView<T*>(Out).Mid(StartIndex);
}
	...
	TArray<UPackage*>& GCKeepPackages, TArray<FPackageData*>& GCKeepPackageDatas, bool& bOutShouldDemote)
{
	...
	else if (!Info.KeepReferencedPackages.IsEmpty())
	{
		...
		for (UPackage* Package : AppendWeakPtrsToObjectPtrArray(GCKeepPackages, Info.KeepReferencedPackages))
		{
			FPackageData* PackageDataOfPackage = PackageDatas.FindPackageDataByPackageName(Package->GetFName());
			if (PackageDataOfPackage)
			{
				GCKeepPackageDatas.Add(PackageDataOfPackage);
			}
		}
	}
	if (!bCurrentGCHasKeptGeneratorKeepPackages)
	{
		...
		if (!OwnerInfo.KeepReferencedPackages.IsEmpty())
		{
			...
			for (UPackage* Package : AppendWeakPtrsToObjectPtrArray(GCKeepPackages, OwnerInfo.KeepReferencedPackages))
			{
				FPackageData* PackageDataOfPackage = PackageDatas.FindPackageDataByPackageName(Package->GetFName());
				if (PackageDataOfPackage)
				{
	...
	if (bKeepingAnyObjects || Info.PackageData->GetSaveSubState() > ESaveSubState::Generation_CallPreSave)
	{
		...
		if (&Info != &OwnerInfo)
		{
			...
			if (!Package)
			{
				Package = FindObject<UPackage>(nullptr, *Info.PackageData->GetPackageName().ToString());
			}
			if (Package)
			{
	...
	if (bNeedsGeneratorPackage && !bCurrentGCHasKeptGeneratorPackage)
	{
		...
		if (!Package)
		{
			Package = FindObject<UPackage>(nullptr, *OwnerInfo.PackageData->GetPackageName().ToString());
		}
		if (Package)
		{
			GCKeepPackages.Add(Package);
	...
	const FIoHash& CurrentPackageHash)
{
	if (bReportedManifest)
	{
		...
		{
			UE_LOGF(LogCook, Warning,
				"Unexpected generated package (discovery replication). A remote cookworker reported generated package %ls for generator %ls," " but when TryReportGenerationManifest was called on the CookDirector, this package was not listed." " This is unexpected and causes minor performance problems in the cook.",
...
void FGenerationHelper::PostGarbageCollect(const TRefCountPtr<FGenerationHelper>& RefcountHeldByCaller, FCookGCDiagnosticContext& Context)
{
	...
	if (Owner.IsInStateProperty(EPackageStateProperty::Saving))
	{
		...
		if (!Owner.GetPackage())
		{
			...
			if (FoundPackage)
			{
				...
				UE_LOGF(LogCook, Warning,
					"CookPackageSplitter's package pointer was unexpectedly set to null by garbage collection while the package is still in the %ls state, %ls substate, but the package is still in memory." "\n\tPackage=%ls, Splitter=%ls.",
		...
		{
			UE_LOGF(LogCook, Error,
				"CookPackageSplitter's %ls was deleted by garbage collection while the package is still in the %ls state, %ls substate. This will break the generation." "\n\tPackage=%ls, Splitter=%ls.",
	...
	for (FCookGenerationInfo& Info : PackagesToGenerate)
	{
		if (FindObject<UPackage>(nullptr, *Info.PackageData->GetPackageName().ToString()))
		{
			...
			{
				UE_LOGF(LogCook, Warning,
					"PackageSplitter found a package it generated that was not removed from memory during garbage collection. This will cause errors later during population." "\n\tSplitter=%ls, Generated=%ls.", *GetSplitDataObjectName().ToString(),
					*Info.GetPackageName());
		...
		for (TArray<TWeakObjectPtr<UPackage>>::TIterator Iter(Info.KeepReferencedPackages); Iter; ++Iter)
		{
			...
			{
				UE_LOGF(LogCook, Warning,
					"PackageSplitter returned a package in OutKeepReferencedPackages that the cooker tried to keep referenced, but it was removed by garbage collection anyway." " This might cause errors during save of the generated packages." "\n\tSplitter=%ls, Generated=%ls.",
					*GetSplitDataObjectName().ToString(),
					*Info.GetPackageName());
...
void FGenerationHelper::UpdateSaveAfterGarbageCollect(const FPackageData& PackageData, bool& bInOutDemote)
{
	...
	if (!Info->IsGenerator())
	{
		UPackage* LocalOwnerPackage = OwnerPackage.Get();
		if (!LocalOwnerPackage || !LocalOwnerPackage->IsFullyLoaded())
		{
			if (IsUseInternalReferenceToAvoidGarbageCollect() && Info->HasTakenOverCachedCookedPlatformData())
			{
				UE_LOGF(LogCook, Error,
					"The generator package of a generated package was unloaded during garbage collection after the generated package started generating. " "This will cause errors during save of the generated package." "\n\tSplitter=%ls, Generated=%ls" "\n\tGeneratorPackage=%ls, GeneratorPackage->IsFullyLoaded()=%ls",
					*GetSplitDataObjectName().ToString(),
					*Info->PackageData->GetPackageName().ToString(),
					LocalOwnerPackage == nullptr ? TEXT("<null>") : TEXT("<nonnull>"),
					*(::LexToString(LocalOwnerPackage == nullptr ? false : LocalOwnerPackage->IsFullyLoaded())));
	...
	if (bInOutDemote && IsUseInternalReferenceToAvoidGarbageCollect() && Info->HasTakenOverCachedCookedPlatformData())
	{
		...
		UE_LOGF(LogCook, Error,
			"A %ls package had some of its UObjects deleted during garbage collection after it started generating. This will cause errors during save of the package." "\n\tDeleted object: %ls" "\n\tSplitter=%ls%ls",
			Info->IsGenerator() ? TEXT("Generator") : TEXT("Generated"),
			*DeletedObject,
			*GetSplitDataObjectName().ToString(),
	...
	TArray<FCachedObjectInOuter>& CachedObjectsInOuter, TArray<UObject*>& MovedObjects)
{
	...
	for (UObject* Object : MovedObjects)
	{
		...
		{
			UE_LOGF(LogCook, Warning,
				"CookPackageSplitter found non-valid object %ls returned from %ls on Splitter %ls%ls. Ignoring it.",
		...
		if (!Info.bInitialized)
		{
			...
			GetObjectsWithOuter(Object, ChildrenOfMovedObjects, EGetObjectsFlags::IncludeNestedObjects, RF_NoFlags,
...
{
	// As in FPackageData::RefreshObjectCache, we add any new objects but do not remove any removed objects.
	bOutFoundNewObjects = false;
	TArray<UObject*> CurrentObjectsInOuter;
	GetObjectsWithOuter(Package, CurrentObjectsInOuter, EGetObjectsFlags::IncludeNestedObjects, RF_NoFlags,
		EInternalObjectFlags::Garbage);
	...
	if (FirstNewObject != nullptr && DemotionState != ESaveSubState::Last)
	{
		...
		{
			UE_LOGF(LogCook, Error,
				"Cooker has repeatedly tried to call BeginCacheForCookedPlatformData on all objects in a generated package, but keeps finding new objects.\n" "Aborting the save of the package; programmer needs to debug why objects keep getting added to the package.\n" "Splitter: %ls%ls. Most recent created object: %ls.",
				*GenerationHelper.GetSplitDataObjectName().ToString(),
				IsGenerator() ? TEXT("") : *FString::Printf(TEXT(", Package: %s"),
					*PackageData->GetPackageName().ToString()),
...
void FGenerationHelper::OnAddPendingCookedPlatformData(UCookOnTheFlyServer& COTFS, FPackageData& AssignedPackageData,
	UObject* Obj)
{
	if (&AssignedPackageData != OwnerInfo.PackageData)
	{
		// We only need to coordinate saves if the assigned package is the generator package.
		return;
	}
	UPackage* AssignedPackage = AssignedPackageData.GetPackage();
	check(AssignedPackage); // Only be called for assigned packages in the save state.
	check(Obj); // Only called for non-null objects.
	UPackage* ParentPackage = Obj->GetPackage();
	if (ParentPackage == AssignedPackage)
	{
		// No action necessary if the object is within the assigned package.
		return;
	}
	FPackageData* ParentPackageData = COTFS.PackageDatas->FindPackageDataByPackageName(ParentPackage->GetFName());
	if (!ParentPackageData)
	{
		// Object is in a temporary or other package that the cooker does not track. The cooker will
		// treat it as still being in the assigned package; SavePackage will make it an import if referenced.
		return;
	}
	FCookGenerationInfo* ParentInfo = FindInfo(*ParentPackageData);
	if (!ParentInfo)
	{
		// Object moved into another cookable package, but not a generated package of this generator.
		// During cook of the generator we will treat it as still being in the generator package; SavePackage
	...
	// ParentPackage != AssignedPackage and AssignedPackage == OwnerPackage, so ParentInfo != OwnerInfo
	check(ParentInfo != &OwnerInfo); 

	// Block any further operations on both the generator and generated package it moved into, until the Object is no
	// longer pending. We do this because we need to guarantee the cooker contract that we do not call
	// BeginCachedCookedPlatformData on it again until it has returned true from IsCachedCookedPlatformDataLoaded, and
	// operations on the generated package might find it in the generated package and call BeginCached.
	AddBlockingPendingCookedPlatformDataObject(Obj);
}

void FGenerationHelper::AddBlockingPendingCookedPlatformDataObject(UObject* Object)
{
	BlockingPendingCookedPlatformDatas.Add(FWeakObjectPtr(Object));
}

bool FGenerationHelper::HasBlockingPendingCookedPlatformDataObjects()
{
	FPackageDatas& PackageDatas = OwnerInfo.PackageData->GetPackageDatas();
	for (auto Iter = BlockingPendingCookedPlatformDatas.CreateIterator(); Iter; ++Iter)
	{
		UObject* Object = Iter->Get();
		if (Object && !PackageDatas.GetPendingCookedPlatformDatas(Object).IsEmpty())
		{
			return true;
		}
		Iter.RemoveCurrentSwap();
...
bool FGenerationHelper::ShouldKeepWaitingOnMovedPendingCookedPlatformData(FPendingCookedPlatformData& Pending,
	FPackageData& AssignedPackage, UObject* Object)
{
	check(Object);
	// Only allow generators to keep waiting on objects in other packages
	if (&AssignedPackage != OwnerInfo.PackageData)
	{
		return false;
	}
	UPackage* CurrentPackage = Object->GetPackage();
	check(CurrentPackage);
	FPackageData* CurrentPackageData =
		AssignedPackage.GetPackageDatas().FindPackageDataByPackageName(CurrentPackage->GetFName());
	if (!CurrentPackageData)
	{
		// Only wait on objects in cookable packages
		return false;
	}
	FCookGenerationInfo* CurrentInfo = FindInfo(*CurrentPackageData);
	if (!CurrentInfo)
	{
		// Only wait on objects in generated packages of this generator.
		return false;
	}
	return true;
}
	...
	TArray<UPackage*>& InKeepReferencedPackages)
{
	...
	for (UPackage* Package : InKeepReferencedPackages)
	{
		...
		{
			UE_LOGF(LogCook, Warning,
				"PackageSplitter returned a package in OutKeepReferencedPackages that is already marked as garbage." " This might cause errors during save of the generated packages." "\n\tSplitter=%ls, Generated=%ls.",
				*GenerationHelper.GetSplitDataObjectName().ToString(),
				*GetPackageName());
...
void FGenerationHelper::SetBeginCookConfigSettings()
{
	...
	else
	{
		...
		{
			UE_LOGF(LogCook, Error,
				"Invalid setting Editor:[CookSettings]:MPCookGeneratorSaveOrder=%ls. Expected values are 'GeneratorFirst', 'GeneratedFirst', or 'None'. Falling back to default 'None'.",
				*SaveOrder);
		}
