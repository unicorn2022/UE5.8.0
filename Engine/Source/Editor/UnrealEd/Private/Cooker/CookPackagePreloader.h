 ...
 */
class FPackagePreloader : public FRefCountedObject
{
public:
	...
	static bool IsPackageLoaded(FName InPackageName);
	...
	bool PumpLoadsIsReadyToLeavePreload(TRefCountPtr<FPackagePreloader>* OutBlocker = nullptr, FString* OutBlockerReason = nullptr);
...
private:
	...
	public:
		...
		void SetRequestID(int32 InRequestID);

	private:
		enum ECompletionState : uint8
		{
			Incomplete,
			Failed,
			Succeeded
		...
		int32 RequestID{ 0 };
		std::atomic<ECompletionState> CompletionState{ ECompletionState::Incomplete };
	};
...
private:
	 ...
	 */
	bool TryPreload(FString* OutFailReason = nullptr);
...
inline bool FPackagePreloader::FAsyncRequest::IsSucceeded() const
{
	return CompletionState.load() == ECompletionState::Succeeded;
}

inline bool FPackagePreloader::FAsyncRequest::IsFailed() const
{
	return CompletionState.load() == ECompletionState::Failed;
}

inline bool FPackagePreloader::FAsyncRequest::IsCompleted() const
{
	return CompletionState.load() != ECompletionState::Incomplete;
}

inline void FPackagePreloader::FAsyncRequest::SetSucceeded()
{
	CompletionState = ECompletionState::Succeeded;
}

inline void FPackagePreloader::FAsyncRequest::SetFailed()
{
	CompletionState = ECompletionState::Failed;
}

inline int32 FPackagePreloader::FAsyncRequest::GetRequestID() const
{
	return RequestID;
}

inline void FPackagePreloader::FAsyncRequest::SetRequestID(int32 InRequestID)
{
