...
public:
	...
	virtual void ReplayLogsFromIncrementallySkipped(FName PackageName,
		TConstArrayView<FReplicatedLogData> LogMessages) override;
...
private:
	...
	void ReportActiveLog(FReplicatedLogData&& LogData, TStringView<CharType> FormatMessage, ELogVerbosity::Type Verbosity);
...
private:
	...
	TArray<FUtf8String> Utf8StringTable;
...
void FLogHandler::ReplayLogsFromIncrementallySkipped(FName PackageName,
	TConstArrayView<FReplicatedLogData> LogMessages)
{
	...
	auto TryTransformMessage = [](const FString& In, FString& Out)
		{
			Out = FString::Printf(TEXT("[IncrementalReplay]: %s"), *In);
			return true;
		};
	UE_LOGF(LogCook, Display, "IncrementalReplay of messages for %ls", *WriteToString<256>(PackageName))
...
void FLogHandler::ConditionalPruneReplay()
{
	...
	if ((StringTable.Num() > TableSizeToFlushAt)
		|| (Utf8StringTable.Num() > TableSizeToFlushAt)
...
void FLogHandler::PruneReplay()
{
	...
	if (!StringTable.IsEmpty() || !Utf8StringTable.IsEmpty() || !TemplateTable.IsEmpty())
	{
...
void FLogHandler::SerializeRecord(const UE::FLogRecord& Record)
{
	...
	if (const UTF8CHAR* Utf8Format = Record.GetUtf8Format())
	{
		ReportActiveLog<UTF8CHAR>(MoveTemp(SerializedData), Utf8Format, Record.GetVerbosity());
	}
	else
	{
		PRAGMA_DISABLE_DEPRECATION_WARNINGS;
		ReportActiveLog<TCHAR>(MoveTemp(SerializedData), Record.GetFormat(), Record.GetVerbosity());
...
void FLogHandler::Marshal(FReplicatedLogData& OutData, const FLogRecord& LogRecord)
{
	...
	if (const UTF8CHAR* Utf8Format = LogRecord.GetUtf8Format())
	{
		Writer << Utf8Format;
	}
	else
	...
	TFunctionRef<bool(const FString&, FString&)> TryTransformMessage)
{
		...
		LoadFromCompactBinary(*It++, SerializedString) && MessagePassesFilter(Category, SerializedString))
	{
		...
		FUtf8String& FormatString = Utf8StringTable.Emplace_GetRef(SerializedString);
		OutLogRecord.SetUtf8Format(*FormatString);
	}
	else
	{
		bOk = false;
	...
	{
		FUtf8String& FileString = Utf8StringTable.AddDefaulted_GetRef();
		FileString = FileStringBuilder.ToString();
		OutLogRecord.SetFile((const ANSICHAR*)*FileString);
	}
	else
	{
		bOk = false;
	...
	if (bHasTextKey)
	{
		FLogTemplate* LogTemplate = TemplateTable.Emplace_GetRef(OutLogRecord.GetTextNamespace(), OutLogRecord.GetTextKey(), OutLogRecord.GetUtf8Format()).Get();
		OutLogRecord.SetTemplate(LogTemplate);
	}
	else
	{
		FLogTemplate* LogTemplate = TemplateTable.Emplace_GetRef(OutLogRecord.GetUtf8Format()).Get();
		OutLogRecord.SetTemplate(LogTemplate);
	}
...
template <UE::CCharType CharType>
void FLogHandler::ReportActiveLog(FReplicatedLogData&& LogData, TStringView<CharType> FormatMessage,
	ELogVerbosity::Type Verbosity)
{
	...
	{
		if (FormatMessage.StartsWith(LITERAL(CharType, "[CookWorker")))
		{
