...
{
public:
	FIncrementallyModifiedDiagnostics();

	void AddPackage(FIncrementallyModifiedContext& Context);
	void WriteFinal(FStringView OutputDirectory) const;
	bool IsWriteFinalLog() const;
	bool IsWriteFinalFiles() const;
	bool IsWriteImmediateLog() const;
...
private:
	...
	void WriteToFiles(FStringView OutputDirectory) const;
	void WriteToLog() const;
	void SortPackageData(TMap<FTopLevelAssetPath, TArray<const FPackageData*>>& OutPackagesPerClass) const;
	void CreateModifiedCookedPackageContent(const TMap<FTopLevelAssetPath,
		TArray<const FPackageData*>>& PackagesPerClass, TArray<FString>& OutLines) const;
	void CreateModifiedEditorPackageContent(const TMap<FTopLevelAssetPath,
		TArray<const FPackageData*>>& PackagesPerClass, TArray<FString>& OutLines) const;

	TArray<FPackageData> PackageDatas;
	bool bWriteFinalFiles = false;
	bool bWriteFinalLog = false;
	bool bWriteImmediateLog = false;
};
