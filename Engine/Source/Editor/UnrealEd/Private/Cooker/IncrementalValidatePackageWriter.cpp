...
bool LoadFromCompactBinary(FCbFieldView Field, FIncrementalValidatePackageWriter::EPackageStatus& Status)
{
	...
	{
		UE_LOGF(LogIncrementalValidate, Error, "Failed to deserialize package status.");
	}
	else if (StatusInteger >= (uint8)FIncrementalValidatePackageWriter::EPackageStatus::Count)
	{
		UE_LOGF(LogIncrementalValidate, Error, "Unexpected package status deserialized: %d", StatusInteger);
	}
	else
	{
...
void FIncrementalValidateMPCollector::ClientTickPackage(UE::Cook::FMPCollectorClientTickPackageContext& Context)
{
	...
	{
		UE_LOGF(LogCook, Error, "Context does not contain a valid package name.")
	...
	if (LoadFromCompactBinary(Message["MessageSubtype"], MessageSubtypeAsInteger))
	{
		switch ((EMessageSubtype)MessageSubtypeAsInteger)
		{
			...
			case EMessageSubtype::ServerToClient_ReplUpdatePackageModificationStatus:
			{
				...
				{
					UE_LOGF(LogCook, Error, "Cannot process ServerToClient_ReplUpdatePackageModificationStatus without a valid package name in the current context.");
				}
				break;
			}
			default:
			{
				UE_LOGF(LogCook, Error, "Unexpected message type: %d", MessageSubtypeAsInteger);
				break;
			}
		};
	}
...
void FIncrementalValidateMPCollector::ServerReceiveMessage(UE::Cook::FMPCollectorServerMessageContext& Context, FCbObjectView Message)
{
	...
	if (LoadFromCompactBinary(Message["MessageSubtype"], MessageSubtypeAsInteger))
	{
		...
		if (PackageName.IsNone())
		{
			UE_LOGF(LogCook, Error, "Cannot process messages on server without a valid package name in the current context.");
		}
		...
		{
			UE_LOGF(LogCook, Error, "Unexpected message received. MessageSubtype == %d", MessageSubtypeAsInteger);
		}
	}
	else
	{
		UE_LOGF(LogCook, Error, "Invalid message received. No MessageSubtype field available.");
	}
}
...
void FIncrementalValidateMPCollector::ReadAndSyncPackageStatus(FCbObjectView Message, FName PackageName)
{
	...
	if (!bOk)
	{
		UE_LOGF(LogCook, Error, "Invalid message received in ReadAndSyncPackageStatus. Failed to load Info from Message[\"Status\"] for package \"%ls\"",
...
void FIncrementalValidatePackageWriter::Initialize(const FCookInfo& CookInfo)
{
	...
	case EPhase::AllInOnePhase:
		...
		{
			UE_CLOGF(COTFS.GetCookMode() != ECookMode::CookWorker,
				LogIncrementalValidate, Display,
				"The cook is running non-incrementally. All packages are reported \"modified\" and will be resaved as in a normal cook.");
	...
	case EPhase::Phase1:
		if (CookInfo.bFullBuild)
		{
			UE_CLOGF(COTFS.GetCookMode() != ECookMode::CookWorker,
				LogIncrementalValidate, Display,
				"The cook is running non-incrementally. All packages are reported \"modified\" and will be resaved during the final IncrementalValidate phase.");
		}
		break;
	case EPhase::Phase2:
		if (CookInfo.bFullBuild)
		{
			UE_CLOGF(COTFS.GetCookMode() != ECookMode::CookWorker, 
				LogIncrementalValidate, Display,
				"The cook is running non-incrementally. Packages that were incrementally skipped and found valid will be resaved anyway.");
		}
		break;
	default:
		checkNoEntry();
...
void FIncrementalValidatePackageWriter::BeginCook(const FCookInfo& Info)
{
	...
	case EPhase::AllInOnePhase:
		if (bReadOnly)
		{
			UE_CLOGF(COTFS.GetCookMode() != ECookMode::CookWorker,
				LogIncrementalValidate, Display,
				"-IncrementalValidateAllowWrite not present, read-only mode. Running -diffonly on all packages that were found to be incrementally unmodified.");
		}
		else
		{
			UE_CLOGF(COTFS.GetCookMode() != ECookMode::CookWorker,
				LogIncrementalValidate, Display,
				"-IncrementalValidateAllowWrite is present, writable mode. Resaving packages as in a normal cook, but also running -diffonly on all packages that were found to be incrementally unmodified.");
		}
		if (Info.bFullBuild)
		{
			UE_CLOGF(COTFS.GetCookMode() != ECookMode::CookWorker,
				LogIncrementalValidate, Error,
				"IncrementalValidate was bypassed on this run; it is a full cook and all packages are marked incrementally modified.");
		}
		break;
	case EPhase::Phase1:
		UE_CLOGF(COTFS.GetCookMode() != ECookMode::CookWorker,
			LogIncrementalValidate, Display,
			"Phase1: running -diffonly and a resave on all packages discovered to be incrementally unmodified.");
		if (Info.bFullBuild)
		{
			UE_CLOGF(COTFS.GetCookMode() != ECookMode::CookWorker,
				LogIncrementalValidate, Error,
	...
	case EPhase::Phase2:
		{
			...
			UE_CLOGF(COTFS.GetCookMode() != ECookMode::CookWorker,
				LogIncrementalValidate, Display,
				"Phase2: %d packages were found during Phase1 to be incrementally unmodified but had differences. "
			...
			UE_CLOGF(COTFS.GetCookMode() != ECookMode::CookWorker,
				LogIncrementalValidate, Display,
				"%d packages were found during Phase1 to be modified or new and will be resaved.",
...
void FIncrementalValidatePackageWriter::EndCook(const FCookInfo& Info)
{
	...
	case EPhase::AllInOnePhase:
	{
		...
		UE_CLOGF(COTFS.GetCookMode() != ECookMode::CookWorker,
			LogIncrementalValidate, Display,
			"Modified: %d. DetectedUnmodified: %d. ValidatedUnmodified: %d. IncrementalSkipFalsePositive: %d.",
			StatusCounts[EPackageStatus::DeclaredModified_WillNotVerify_Cooked],
			DetectedUnmodified,
		...
		if (StatusCounts[EPackageStatus::DeclaredUnmodified_FoundModified_FalsePositive] > 0)
		{
			...
			UE_CLOGF(COTFS.GetCookMode() != ECookMode::CookWorker,
				LogIncrementalValidate, Error, "%ls", *MessageWithDiagnostics);
		}
		else if (StatusCounts[EPackageStatus::DeclaredUnmodified_FoundModified_OnIgnoreList] > 0)
		{
			Message = FString::Printf(TEXT("Packages Incrementally Skipped: %d: IncrementalSkipFalsePositive (Ignored): %d."),
				...
				StatusCounts[EPackageStatus::DeclaredUnmodified_FoundModified_OnIgnoreList]);
			UE_CLOGF(COTFS.GetCookMode() != ECookMode::CookWorker,
				LogIncrementalValidate, Warning, "%ls", *Message);
		}
		else
		{
			UE_CLOGF(COTFS.GetCookMode() != ECookMode::CookWorker,
				LogIncrementalValidate, Display, "%ls", *Message);
		}
		break;
	}
	case EPhase::Phase1:
	{
		...
		UE_CLOGF(COTFS.GetCookMode() != ECookMode::CookWorker,
			LogIncrementalValidate, Display,
			"Modified: %d. DetectedUnmodified: %d. ValidatedUnmodified: %d. IncrementalSkipFalsePositiveOrIndeterminism: %d.",
			StatusCounts[EPackageStatus::DeclaredModified_WillNotVerify_Cooked],
			DetectedUnmodified,
			StatusCounts[EPackageStatus::DeclaredUnmodified_ConfirmedUnmodified]
			+ StatusCounts[EPackageStatus::DeclaredUnmodified_FoundModified_OnIgnoreList],
	...
	case EPhase::Phase2:
	{
		...
		UE_CLOGF(COTFS.GetCookMode() != ECookMode::CookWorker,
			LogIncrementalValidate, Display,
			"Modified: %d. DetectedUnmodified: %d. ValidatedUnmodified: %d. Indeterminism: %d. IncrementalSkipFalsePositive: %d.",
			StatusCounts[EPackageStatus::DeclaredModified_WillNotVerify_Cooked],
			DetectedUnmodified,
			StatusCounts[EPackageStatus::DeclaredUnmodified_ConfirmedUnmodified]
			+ StatusCounts[EPackageStatus::DeclaredUnmodified_FoundModified_OnIgnoreList],
		...
		{
			UE_CLOGF(COTFS.GetCookMode() != ECookMode::CookWorker,
				LogIncrementalValidate, Error, "%ls", *Message);
		}
		else if (StatusCounts[EPackageStatus::DeclaredUnmodified_FoundModified_OnIgnoreList] > 0)
		{
			Message = FString::Printf(TEXT("Packages Incrementally Skipped: %d: IncrementalSkipFalsePositive (Ignored): %d."),
...
void FIncrementalValidatePackageWriter::LogIncrementalDifferences()
{
	...
	{
		UE_LOGF(LogIncrementalValidate, Display, "Could not validate %ls because it has a non-deterministic save.",
...
void FIncrementalValidatePackageWriter::Save()
{
	...
	if (!DiskArchive)
	{
		UE_LOGF(LogIncrementalValidate, Error,
			"Could not write to file %ls. This file is needed to store results for the -IncrementalValidate cook.",
...
void FIncrementalValidatePackageWriter::Load()
{
	...
	if (!DiskArchive)
	{
		UE_LOGF(LogIncrementalValidate, Fatal,
			"Could not load file %ls. This file is required and should have been written by the -IncrementalValidatePhase1 cook.",
			*IncrementalValidatePath);
		return;
	}
	FNameAsStringProxyArchive Ar(*DiskArchive);
	...
	if (Ar.IsError())
	{
		UE_LOGF(LogIncrementalValidate, Fatal, "Corrupt file %ls", *IncrementalValidatePath);
	}
}
