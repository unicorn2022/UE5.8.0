...
void UEditorStateSubsystem::OutputOperationResult(const UEditorState* InState, bool bRestore, const UEditorState::FOperationResult& InOperationResult) const
{
	...
	case UEditorState::FOperationResult::Success:
		UE_LOGF(LogEditorState, Log, "[%ls] Success %ls%ls", *InState->GetCategoryText().ToString(), *ColonSeparator, *InOperationResult.GetResultText().ToString());
		break;
	case UEditorState::FOperationResult::Skipped:
		UE_LOGF(LogEditorState, Log, "[%ls] Skipping %ls%ls", *InState->GetCategoryText().ToString(), *ColonSeparator, *InOperationResult.GetResultText().ToString());
		break;
	case UEditorState::FOperationResult::Warning:
		UE_LOGF(LogEditorState, Warning, "[%ls] Warning %ls%ls", *InState->GetCategoryText().ToString(), *ColonSeparator, *InOperationResult.GetResultText().ToString());
		break;
	case UEditorState::FOperationResult::Failure:
		{
			UE_LOGF(LogEditorState, Error, "[%ls] Failure %ls%ls", *InState->GetCategoryText().ToString(), *ColonSeparator, *InOperationResult.GetResultText().ToString());
...
{
	UE_LOGF(LogEditorState, Log, "Capturing editor state...");
	...
	UE_LOGF(LogEditorState, Log, "Captured editor state... DONE");
}
...
void UEditorStateSubsystem::RestoreEditorState(const FEditorStateCollection& InState, const TArray<TSubclassOf<UEditorState>>& InEditorStateTypeFilter)
{
	...
	UE_LOGF(LogEditorState, Log, "Restoring editor state...");
	...
	UE_LOGF(LogEditorState, Log, "Restored editor state... DONE");
}
