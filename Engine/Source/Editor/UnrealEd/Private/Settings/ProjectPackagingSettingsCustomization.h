...
protected:
	...
	TSharedRef<SWidget> CreateImportFromProjectLauncherMenu( TSharedPtr<IPropertyUtilities> PropertyUtilities )
	{
		...
		{
			if (!LauncherProfilePtr->GetFirstBuildCookRun().IsValid())
			{
				continue;
			}
	...
	void ImportFromLauncherProfile( const ILauncherProfilePtr LauncherProfilePtr, TSharedPtr<IPropertyUtilities> PropertyUtilities )
	{
		...
		const ILauncherProfileBuildCookRunPtr BuildCookRunPtr = LauncherProfilePtr->GetFirstBuildCookRun();
		if (BuildCookRunPtr == nullptr)
		{
			return;
		}
		...
		ProjectBuildSettings.SpecificPlatforms  = BuildCookRunPtr->GetCookedPlatforms();
		if (ProjectBuildSettings.SpecificPlatforms.Num() == 0 && !BuildCookRunPtr->GetDefaultDeployPlatform().IsNone())
		{
			ProjectBuildSettings.SpecificPlatforms.Add(BuildCookRunPtr->GetDefaultDeployPlatform().ToString());
		}
