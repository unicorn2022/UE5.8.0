...
void UCompileAllBlueprintsCommandlet::InitCommandLine(const FString& Params)
{
	...
	if (SwitchParams.Contains(TEXT("BlueprintBaseClass")))
	{
		...
		UE_CLOGF(BlueprintBaseClassName.IsNull(), LogCompileAllBlueprintsCommandlet, Error, "Failed to convert short class name -BlueprintBaseClass=\"%ls\" to path name", *BlueprintBaseClassParam);
	}
}
...
void UCompileAllBlueprintsCommandlet::ParseAllowList(const FString& ListFilePath)
{
	...
	{
		UE_LOGF(LogCompileAllBlueprintsCommandlet, Error, "Failed to Load AllowList File! : %ls", *FilePath);
	}
}
...
void UCompileAllBlueprintsCommandlet::BuildBlueprints()
{
	...
	for (FAssetData const& Asset : BlueprintAssetList)
	{
		if (ShouldBuildAsset(Asset))
		{
			...
			UE_LOGF(LogCompileAllBlueprintsCommandlet, Display, "Loading and Compiling: '%ls'...", *AssetPath);
			...
			{
				++TotalNumFailedLoads;
				UE_LOGF(LogCompileAllBlueprintsCommandlet, Error, "Failed to Load : '%ls'.", *AssetPath);
				continue;
			}
			else
			{
...
void UCompileAllBlueprintsCommandlet::BuildBlueprintAssetList()
{
	...
	UE_LOGF(LogCompileAllBlueprintsCommandlet, Display, "Loading Asset Registry...");
	FAssetRegistryModule& AssetRegistryModule = FModuleManager::LoadModuleChecked<FAssetRegistryModule>(AssetRegistryConstants::ModuleName);
	AssetRegistryModule.Get().SearchAllAssets(/*bSynchronousSearch =*/true);
	UE_LOGF(LogCompileAllBlueprintsCommandlet, Display, "Finished Loading Asset Registry.");
	
	UE_LOGF(LogCompileAllBlueprintsCommandlet, Display, "Gathering All Blueprints From Asset Registry...");
...
bool UCompileAllBlueprintsCommandlet::ShouldBuildAsset(FAssetData const& Asset) const
{
	...
	{
		FString const AssetPath = Asset.GetObjectPathString();
		UE_LOGF(LogCompileAllBlueprintsCommandlet, Verbose, "Skipping Building %ls: As is not cooked", *AssetPath);
		bShouldBuild = false;
	}
	...
	if (IgnoreFolders.Num() > 0)
	{
		for (const FString& IgnoreFolder : IgnoreFolders)
		{
			...
			{
				FString const AssetPath = Asset.GetObjectPathString();
				UE_LOGF(LogCompileAllBlueprintsCommandlet, Verbose, "Skipping Building %ls: As Object is in an Ignored Folder", *AssetPath);
				bShouldBuild = false;
			}
		}
	}
	...
	{
		FString const AssetPath = Asset.GetObjectPathString();
		UE_LOGF(LogCompileAllBlueprintsCommandlet, Verbose, "Skipping Building %ls: As has an excluded tag", *AssetPath);
		bShouldBuild = false;
	}
	...
	{
		FString const AssetPath = Asset.GetObjectPathString();
		UE_LOGF(LogCompileAllBlueprintsCommandlet, Verbose, "Skipping Building %ls: As the asset is missing a required tag", *AssetPath);
		bShouldBuild = false;
	}
	...
	{
		FString const AssetPath = Asset.GetObjectPathString();
		UE_LOGF(LogCompileAllBlueprintsCommandlet, Verbose, "Skipping Building %ls: As the asset is not part of the allow list", *AssetPath);
		bShouldBuild = false;
	}
	...
	if (bDirtyOnly)
	{
		...
		{
			FString const AssetPath = Asset.GetObjectPathString();
			UE_LOGF(LogCompileAllBlueprintsCommandlet, Verbose, "Skipping Building %ls: As Package is not dirty", *AssetPath);
			bShouldBuild = false;
		}
	}
...
void UCompileAllBlueprintsCommandlet::CompileBlueprint(UBlueprint* Blueprint)
{
	if (KismetBlueprintCompilerModule && Blueprint)
	{
		...
		{
			UE_LOGF(LogCompileAllBlueprintsCommandlet, Display, "%ls", *Message->ToText().ToString());
		}
	}
}
...
{
	UE_LOGF(LogCompileAllBlueprintsCommandlet, Display, "Loading Kismit Blueprint Compiler...");
