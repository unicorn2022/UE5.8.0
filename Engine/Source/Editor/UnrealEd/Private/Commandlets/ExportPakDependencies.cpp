...
bool ExportDependencies(const TCHAR * PakFilename, const TCHAR* GameName, const TCHAR* OutputFilenameBase, bool bSigned)
{
	...
	if(PakFile.IsValid())
	{
		// Get size information from PAK file.
		{
			...
			{
				UE_LOGF(LogPakFile, Error, "Pakfiles were loaded without Filenames, cannot export.");
				return false;
			}
...
int32 UExportPakDependenciesCommandlet::Main(const FString& Params)
{
	...
	{
		UE_LOGF(LogPakFile,Error,"Incorrect arguments. Expected: -PakFile=<FileName> -Output=<FileName> [-Signed]");
		return false;
	}
