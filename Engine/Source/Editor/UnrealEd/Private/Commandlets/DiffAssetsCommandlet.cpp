...
bool UDiffAssetsCommandlet::ExportFilesToTextAndDiff(const FString& InParams)
{
	...
	UE_LOGF(LogDiffAssetsCommandlet, Log, "Params: %ls", *Params);
	...
	{
		UE_LOGF(LogDiffAssetsCommandlet, Warning, "Usage: UDiffAssets File1%ls File2%ls DiffCmd=\"C:/Program Files/Araxis/Araxis Merge/AraxisP4Diff.exe {1} {2}\"", *AssetPackageExtension, *AssetPackageExtension);
		return false;
	}
...
bool UDiffAssetsCommandlet::CopyFileToTempLocation(FString& InOutFilename)
{
	...
	{
		UE_LOGF(LogDiffAssetsCommandlet, Warning, "Failed to copy %ls to %ls.", *InFilename, *OutFilename);
		return false;
	}
...
bool UDiffAssetsCommandlet::LoadFile(const FString& Filename, TArray<UObject *>& LoadedObjects)
{
	...
	if (!Package)
	{
		UE_LOGF(LogDiffAssetsCommandlet, Warning, "Could not load %ls", *Filename);
		return false;
	}
	...
	{
		UE_LOGF(LogDiffAssetsCommandlet, Warning, "Loaded %ls, but it didn't contain any objects.", *Filename);
		return false;
	}
	LoadedObjects.Sort();
...
bool UDiffAssetsCommandlet::ExportFile(const FString& Filename, const TArray<UObject *>& LoadedObjects)
{
	...
	for (int32 Index = 0; Index < LoadedObjects.Num(); Index++)
	{
		...
		if (!Exporter)
		{
			UE_LOGF(LogDiffAssetsCommandlet, Warning, "Could not find exporter.");
			return false;
		}
	...
	if (!Buffer.Len())
	{
		UE_LOGF(LogDiffAssetsCommandlet, Warning, "No text was exported!");
		return false;
	}
	if( !FFileHelper::SaveStringToFile( Buffer, *Filename ) )
	{
		UE_LOGF(LogDiffAssetsCommandlet, Warning, "Could not write %ls", *Filename);
		return false;
	}
	return true;
}
...
bool UDiffAssetsCommandlet::ExportFilesToTextAndDiff(const FString& InFilename1, const FString& InFilename2, const FString& DiffCommand)
{
	...
	{
		UE_LOGF(LogDiffAssetsCommandlet, Warning, "Could not launch %ls.", *ReplacedDiffCmd);
		return false;
	}
	return true;
}
