...
bool UDiffCookCommandlet::TryParseCommandLine(const FString& CmdLineParams)
{
		...
		else if (Key == TEXT("show") || Key == TEXT("hide"))
		{
			...
			UE::String::ParseTokensMultiple(Value, { '+', ',' },
				[this, bShow](FStringView ShowStr)
				{
					...
					{
						UE_LOGF(LogCook, Warning, "Unrecognized showflag '-%ls=%.*ls'."
							" Valid Options are {'packages', 'removedpackages', 'addedpackages', 'modifiedpackages', 'headers', 'serialize'}.",
	...
	if (Args.BasePath.IsEmpty() || (!bSingleDepot && Args.CompPath.IsEmpty()))
	{
		bDisplayHelp = true;
		bResult = false;
		UE_LOGF(LogCook, Error, "Expected two paths specified with -base=<BasePath> -comp=<CompPath>, or one path -base=<BasePath> with -singledepot.");
	}
	if (bSingleDepot)
	{
		Args.CompPath.Empty();
	...
	if (bDisplayHelp)
	{
		UE_LOGF(LogCook, Display, "Usage: -run=DiffCook -base=<BasePath> -comp=<CompPath> {<Optional Arguments>}"
...
bool UDiffCookCommandlet::TryLoadDepots()
{
	...
	UE_LOGF(LogCook, Display, "Comparing depots:\n\tBase: %ls\n\tComp: %ls",
...
bool UDiffCookCommandlet::TryLoadDepotSummaries()
{
	...
	if (TargetPlatforms.Num() > 0)
	{
		...
		{
			UE_LOGF(LogCook, Warning, "Too many targetplatforms provided; using only the first one: %ls.",
	...
	for (int32 DepotIndex = 0; DepotIndex < 2; ++DepotIndex)
	{
		...
		{
			UE_LOGF(LogCook, Error, "Directory does not exist for -%ls at '%ls'.", DepotName, *Depot.CookPath);
			bResult = false;
			continue;
		}
		Depot.bValid = true;
		...
		if (ProjectNames.Num() == 0)
		{
			UE_LOGF(LogCook, Error,
				"Could not find project name in -%ls at '%ls'. Looked for a path of the form %ls, but none was found.",
		...
		{
			UE_LOGF(LogCook, Error,
				"Could not determine project name in -%ls at '%ls'; multiple candidates were found. " " Looked for paths of the form %ls, and found more than one: { %ls }.",
				DepotName, *Depot.CookPath,
				*FPaths::Combine(
		...
		if (!Depot.bARStateValid)
		{
			UE_LOGF(LogCook, Warning,
				"Loading %ls AssetRegistry from path '%ls' failed. Information requiring the AssetRegistry will not be available for the depot.",
			...
			PlatformFile.IterateDirectory(*ProjectDir,
				[&Depot, &Options, &ProjectDir, &DevelopmentSerializationOptions, DepotName](const TCHAR* FullPath, bool bDirectory)
				{
					...
					if (FileName.MatchesWildcard(TEXT("AssetRegistry*.bin")))
					{
						...
						{
							UE_LOGF(LogCook, Warning,
								"Loading %ls chunk-specific AssetRegistry from path '%ls' failed." " Information requiring the AssetRegistry will not be available for files in that chunk.",
		...
		{
			UE_LOGF(LogCook, Warning,
				"Loading %ls DevelopmentAssetRegistry from path %ls failed. Information requiring the DevelopmentAssetRegistry will not be available for the depot.",
	...
	{
		UE_LOGF(LogCook, Error,
			"Projectnames do not match for the requested depots. -base at '%ls' has projectname=%ls, -comp at '%ls' has projectname=%ls." " Projectnames are found by looking for paths of the form %ls under each cookroot.",
	...
	if (CompDepot.bValid && BaseDepot.TargetPlatform != CompDepot.TargetPlatform)
	{
		...
		UE_LOGF(LogCook, Error,
			"TargetPlatforms do not match for the requested depots. -base at '%ls' has targetplatform=%ls, -comp at '%ls' has targetplatform=%ls." " TargetPlatforms are found from the leafname of the depotpath, or from the -targetplatform argument.",
	...
	{
		UE_LOGF(LogCook, Warning,
			"TargetPlatform could not be detected. TargetPlatforms are found from the leafname of the depotpath, or from the -targetplatform argument." " Information that relies on targetplatform (such as -show=serialize) will not be available.");
	}
...
void UDiffCookCommandlet::LoadDepotPackageLists()
{
	for (FCookedDepot* DepotPtr : { &BaseDepot, &CompDepot })
	{
		...
		UE_LOGF(LogCook, Display, "Scanning filelist from %ls...", *Depot.CookPath)
...
bool UDiffCookCommandlet::TryConstructFilterLists()
{
	if (!Args.RequestedPackages.IsEmpty())
	{
		...
		for (FString& RequestedPackage : Args.RequestedPackages)
		{
			...
			{
				UE_LOGF(LogCook, Error, "Could not find PackageName for -package '%ls', it will not be diffed.",
					*RequestedPackage);
...
void UDiffCookCommandlet::LoadDepotContentRoots(FCookedDepot& Depot)
{
	...
	for (FString& ContentRoot : ContentRoots)
	{
		...
		{
			UE_LOGF(LogCook, Error, "ContentRoot %ls exists in multiple locations: %ls and %ls. Discarding %ls.",
...
{
	UE_LOGF(LogCook, Display, "CookDiff Result: %ls",
...
void UDiffCookCommandlet::PrintPackageDiffs(FDiffResult& Diff)
{
	...
	for (const FPackageDiffResult& PackageDiff : Diff.PackageDiffs)
	{
		...
		case EPackageDiffResult::Removed:
			...
			{
				UE_LOGF(LogCook, Display, "Removed:  %ls\n\tBase: %ls\n\tComp: %ls (Missing)",
		...
		case EPackageDiffResult::Added:
			...
			{
				UE_LOGF(LogCook, Display, "Added:    %ls\n\tBase: %ls (Missing)\n\tComp: %ls",
					*PackageNameStr, **BasePath, **CompPath);
			}
			break;
		...
		default:
			UE_LOGF(LogCook, Error, "Could Not Diff: %ls", *PackageNameStr);
			break;
		}
		if (!bModified)
		{
		...
		if (bShowModifiedPackages)
		{
			...
			UE_LOGF(LogCook, Display, "Modified: %ls\n\tBase: %ls\n\tComp: %ls"
				"\n\t      Different at Byte %lld in %ls%ls.",
				*PackageNameStr, **BasePath, **CompPath,
				PackageDiff.Offset, *WhichFile,
				(PackageDiff.CombinedOffset >= 0
					? *FString::Printf(TEXT(", Combined/DiffBreak Offset %lld"), PackageDiff.CombinedOffset)
					: TEXT("")));

			if (bShowHeaders)
			{
				UE_CALL_ONCE([]() { UE_LOGF(LogCook, Error, "-show=Headers is not yet implemented"); });
			}
			if (bShowSerialize)
			{
				UE_CALL_ONCE([]() { UE_LOGF(LogCook, Error, "-show=Serialize is not yet implemented"); });
			}
		}
	}
}
