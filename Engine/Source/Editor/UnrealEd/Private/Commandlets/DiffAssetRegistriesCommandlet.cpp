...
{
	UE_LOGF(LogDiffAssets, Display, "--------------------------------------------------------------------------------------------");
	UE_LOGF(LogDiffAssets, Display, "Running DiffAssetRegistries Commandlet");
	...
	{
		UE_LOGF(LogDiffAssets, Error, "No platform specified on the commandline use \"-platform=<platform>\".");
	}
	...
	{
		UE_LOGF(LogDiffAssets, Error, "No old path specified \"-oldpath=<>\", use full path to asset registry or build version.");
		return -1;
	}
	...
	{
		UE_LOGF(LogDiffAssets, Error, "No new path specified \"-newpath=<>\", use full path to asset registry or build version.");
		return -1;
	}
	...
	if (OldPath.IsEmpty())
	{
		UE_LOGF(LogDiffAssets, Error, "Unable to locate AssetRegistry.bin for supplied oldpath (%ls), use full path to asset registry or build version.", **OldPathVal);
		return -1;
	}
	if (NewPath.IsEmpty())
	{
		UE_LOGF(LogDiffAssets, Error, "Unable to locate AssetRegistry.bin for supplied newpath (%ls), use full path to asset registry or build version.", **NewPathVal);
		return -1;
	}
	...
	UE_LOGF(LogDiffAssets, Display, "Successfully finished running DiffAssetRegistries Commandlet");
	UE_LOGF(LogDiffAssets, Display, "--------------------------------------------------------------------------------------------");
	return 0;
}
...
{
	{
		...
		{
			UE_LOGF(LogDiffAssets, Error, "File '%ls' does not exist.", *OldPath);
			return;
		}
		if (!FFileHelper::LoadFileToArray(SerializedAssetData, *OldPath))
		{
			UE_LOGF(LogDiffAssets, Error, "Failed to load file '%ls'.", *OldPath);
			return;
		}
		if (!OldState.Load(SerializedAssetData))
		{
			UE_LOGF(LogDiffAssets, Error, "Failed to parse file '%ls' as asset registry.", *OldPath);
			return;
		}
	}
	{
		...
		{
			UE_LOGF(LogDiffAssets, Error, "File '%ls' does not exist.", *NewPath);
			return;
		}
		if (!FFileHelper::LoadFileToArray(SerializedAssetData, *NewPath))
		{
			UE_LOGF(LogDiffAssets, Error, "Failed to load file '%ls'.", *NewPath);
			return;
		}

		if (!NewState.Load(SerializedAssetData))
		{
			UE_LOGF(LogDiffAssets, Error, "Failed to parse file '%ls' as asset registry.", *NewPath);
			return;
		}
	}
	UE_LOGF(LogDiffAssets, Display, "Comparing asset registries '%ls' and '%ls'.", *OldPath, *NewPath);
	UE_LOGF(LogDiffAssets, Display, "Source vs Cooked Consistency Diff");
	if (bIsVerbose)
	{
		UE_LOGF(LogDiffAssets, Display, "Cooked files that differ, where source guids do not:");
	}
	...
	for (FName const &Package : CookModified)
	{
		...
		if (!HashModified.Contains(Package) && Data->DiskSize >= 0)
		{
			...
			if (bIsVerbose)
			{
				UE_LOGF(LogDiffAssets, Display, "%ls : %lld bytes", *Package.ToString(), Data->DiskSize);
			}
		}
	}
	...
	UE_LOGF(LogDiffAssets, Display, "Summary:");
	UE_LOGF(LogDiffAssets, Display, "%lld nondeterministic cooks, %8.3f %cB", Changes, ChangeValue, TCHAR(" KMGTP"[ChangeExp]));
}
...
FName UDiffAssetRegistriesCommandlet::GetClassName(FAssetRegistryState& InRegistryState, FName InAssetPath)
{
	if (AssetPathToClassName.Contains(InAssetPath) == false)
	{
		...
		{
			UE_LOGF(LogDiffAssets, Log, "Unable to find class type of asset %ls", *InAssetPath.ToString());
		}
...
TArray<int32> UDiffAssetRegistriesCommandlet::GetAssetChunks(FAssetRegistryState& InRegistryState, FName InAssetPath)
{
	if (ChunkIdByAssetPath.Contains(InAssetPath) == false)
	{
		...
		if (!ChunkIDs.IsEmpty())
		{
			if (ChunkIDs.Num() > 1)
			{
				UE_LOGF(LogDiffAssets, Log, "Multiple ChunkIds for asset %ls", *InAssetPath.ToString());
			}
		...
		{
			UE_LOGF(LogDiffAssets, Log, "Unable to find chunk ids of asset %ls", *InAssetPath.ToString());
...
void UDiffAssetRegistriesCommandlet::LogChangedFiles(FArchive *CSVFile, FString const &OldPath, FString const &NewPath)
{
	...
	if (CSVFile)
	{
		...
		UE_LOGF(LogDiffAssets, Display, "Saving CSV results to %ls", *CSVFilename);
	}
	...
	for (const FName& AssetPath : AssetPaths)
	{
		...
		if (ChangeInfo.Adds)
		{
			if (CSVFile)
			{
				CSVFile->Logf(TEXT("a,%s,%s,%lld,0,%d,%s"), *AssetPath.ToString(), *ClassName.ToString(), ChangeInfo.AddedBytes, Changelist, *GetChunkIDString(NewState));
			}

			if (bIsVerbose)
			{
				UE_LOGF(LogDiffAssets, Display, "a %ls : (Class=%ls,NewSize=%lld bytes)", *AssetPath.ToString(), *ClassName.ToString(), ChangeInfo.AddedBytes);
			}
		}
		else if (ChangeInfo.Changes)
		{
			...
			if (CSVFile)
			{
				CSVFile->Logf(TEXT("%c,%s,%s,%lld,%lld,%d,%s"), TCHAR(classification), *AssetPath.ToString(), *ClassName.ToString(), ChangeInfo.ChangedBytes, PrevData->DiskSize, Changelist, *GetChunkIDString(NewState));
			}

			if (bIsVerbose)
			{
				UE_LOGF(LogDiffAssets, Display, "%c %ls : (Class=%ls,NewSize=%lld bytes,OldSize=%lld bytes)", TCHAR(classification), *AssetPath.ToString(), *ClassName.ToString(), ChangeInfo.ChangedBytes, PrevData->DiskSize);
			}
		}
		else if (ChangeInfo.Deletes)
		{
			...
			if (CSVFile)
			{
				CSVFile->Logf(TEXT("r,%s,%s,0,%lld,0,0"), *AssetPath.ToString(), *ClassName.ToString(), PrevData->DiskSize);
			}

			if (bIsVerbose)
			{
				UE_LOGF(LogDiffAssets, Display, "r %ls : (Class=%ls,OldSize=%lld bytes)", *AssetPath.ToString(), *ClassName.ToString(), PrevData->DiskSize);
			}
		}
	}
}
...
void UDiffAssetRegistriesCommandlet::LogClassSummary(FArchive *CSVFile, const FString& HeaderPrefix, const TMap<FName, FChangeInfo>& InChangeInfoByAsset, bool bDoWarnings, TMap<FName, FDeterminismInfo> DeterminismInfo)
{
	...
	for (FName ClassName : ClassNames)
	{
		...
		// log summary & change
		UE_LOGF(LogDiffAssets, Display, "%ls%lsClass Summary: ", *HeaderPrefix, *HeaderSpacer);

		UE_LOGF(LogDiffAssets, Display, "%ls: %.02f%% changes (%.02f MB Total)",
			*ClassName.ToString(), Changes.GetChangePercentage() * 100.0, Changes.GetTotalChangeSize() * InvToMB);

		if (Changes.Adds)
		{
			UE_LOGF(LogDiffAssets, Display, "\t%lld packages added,    %8.3f MB", Changes.Adds, Changes.AddedBytes * InvToMB);
		}

		if (Changes.Changes)
		{
			UE_LOGF(LogDiffAssets, Display, "\t%lld packages modified, %8.3f MB", Changes.Changes, Changes.ChangedBytes * InvToMB);
		}

		if (Changes.Deletes)
		{
			UE_LOGF(LogDiffAssets, Display, "\t%lld packages removed,  %8.3f MB", Changes.Deletes, Changes.DeletedBytes * InvToMB);
		}

		UE_LOGF(LogDiffAssets, Display, "\t%lld packages unchanged,  %8.3f MB", Changes.Unchanged, Changes.UnchangedBytes * InvToMB);
		...
		{
			UE_LOGF(LogDiffAssets, Warning, "\t%ls Assets for %ls are %.02f%% changed. (%.02f MB of data)",
				*TargetPlatform, *ClassName.ToString(), Changes.GetChangePercentage() * 100.0, Changes.ChangedBytes * InvToMB);
		}
	}

	// log summary & change
	UE_LOGF(LogDiffAssets, Display, "%ls%ls FullSummary: ", *HeaderPrefix, *HeaderSpacer);
	UE_LOGF(LogDiffAssets, Display, "  : %.02f%% changes (%.02f MB Changed out of %.02f MB Total)", TotalSize ? (float)TotalChangesSize / (float)TotalSize * 100.0 : 0.0f, TotalChangesSize * InvToMB, TotalSize * InvToMB);
	...
	if (!bChangesPastThreshold)
	{
		...
		UE_LOGF(LogDiffAssets, Display, "%lsNo classes had changes past thresholds", *SummaryName);
	}
}
...
void UDiffAssetRegistriesCommandlet::DiffAssetRegistries(const FString& OldPath, const FString& NewPath, bool bUseSourceGuid, bool bEnginePackagesOnly)
{
	...
	if (bUseSourceGuid)
	{
		UE_LOGF(LogDiffAssets, Display, "Source Package Diff");
	}
	else
	{
		UE_LOGF(LogDiffAssets, Display, "Cooked Package Diff");
	}
	if (bIsVerbose)
	{
		UE_LOGF(LogDiffAssets, Display, "Package changes:");
	}
	...
	// start summary
	UE_LOGF(LogDiffAssets, Display, "Summary:");
	UE_LOGF(LogDiffAssets, Display, "Old AssetRegistry: %ls", *OldPath);
	UE_LOGF(LogDiffAssets, Display, "%lld packages total, %lld uncooked, %lld cooked assets", oldtotal, olduncooked, oldassets);
	UE_LOGF(LogDiffAssets, Display, "New AssetRegistry: %ls", *NewPath);
	...
	for (int32 Changelist : Changelists)
	{
		...
		if (Changelist)
		{
			UE_LOGF(LogDiffAssets, Display, "%d: %.02f%% changes (%.02f MB Total)",
				   Changelist, Changes.GetChangePercentage() * 100.0, Changes.GetTotalChangeSize() * InvToMB);
		}
		else
		{
			UE_LOGF(LogDiffAssets, Display, "Unattributable (nondeterministic?): %.02f%% changes (%.02f MB Total)",
				   Changes.GetChangePercentage() * 100.0, Changes.GetTotalChangeSize() * InvToMB);
		}
		
		if (Changes.Adds)
		{
			UE_LOGF(LogDiffAssets, Display, "\t%d packages added,    %8.3f MB", Changes.Adds, Changes.AddedBytes * InvToMB);
		}
		
		if (Changes.Changes)
		{
			UE_LOGF(LogDiffAssets, Display, "\t%d packages modified, %8.3f MB", Changes.Changes, Changes.ChangedBytes * InvToMB);
		}
		
		if (Changes.Deletes)
		{
			UE_LOGF(LogDiffAssets, Display, "\t%d packages removed,  %8.3f MB", Changes.Deletes, Changes.DeletedBytes * InvToMB);
		}
	}
#endif
	// these are parsed by scripts, so please don't modify :)
	UE_LOGF(LogDiffAssets, Display, "%lld total packages, %lld uncooked, %lld cooked assets", newtotal, newuncooked, newassets);
	UE_LOGF(LogDiffAssets, Display, "%lld total unchanged,         %8.3f MB", ChangeSummary.Unchanged, ChangeSummary.UnchangedBytes * InvToMB);
	UE_LOGF(LogDiffAssets, Display, "%lld total packages added,    %8.3f MB", ChangeSummary.Adds, ChangeSummary.AddedBytes * InvToMB);
	UE_LOGF(LogDiffAssets, Display, "%lld total packages modified, %8.3f MB", ChangeSummary.Changes, ChangeSummary.ChangedBytes * InvToMB);
	UE_LOGF(LogDiffAssets, Display, "%lld total packages removed,  %8.3f MB", ChangeSummary.Deletes, ChangeSummary.DeletedBytes * InvToMB);

	UE_LOGF(LogDiffAssets, Display, "Nondeterministic summary:");
	UE_LOGF(LogDiffAssets, Display, "direct   %lld total packages modified, %8.3f MB", NondeterministicSummary.Changes, NondeterministicSummary.ChangedBytes * InvToMB);
	UE_LOGF(LogDiffAssets, Display, "indirect %lld total packages modified, %8.3f MB", IndirectNondeterministicSummary.Changes, IndirectNondeterministicSummary.ChangedBytes * InvToMB);
	...
	{
		UE_LOGF(LogDiffAssets, Warning, "Total Changed Bytes exceeded %d MB! (%8.3f MB)", WarnTotalChangedSizeMB, ChangeSummary.ChangedBytes * InvToMB);
	}
...
//			}
	...
	{
		UE_LOGF(LogDiffAssets, Error, "Failed to launch p4.");
	}
