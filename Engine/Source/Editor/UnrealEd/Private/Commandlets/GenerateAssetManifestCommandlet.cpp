...
int32 UGenerateAssetManifestCommandlet::Main(const FString& InParams)
{
	...
	{
		UE_LOGF(LogGenerateAssetManifestCommandlet, Error, "Please specify a valid location for -ManifestFile on the commandline");
		return 1;
	}
	...
	UE_LOGF(LogGenerateAssetManifestCommandlet, Display, "Searching Asset Registry");
	...
	{
		UE_LOGF(LogGenerateAssetManifestCommandlet, Display, "Getting Assets from specified paths");
		FARFilter Filter;
	...
	{
		UE_LOGF(LogGenerateAssetManifestCommandlet, Display, "Getting Assets of specified classes");
		FARFilter Filter;
		Filter.bIncludeOnlyOnDiskAssets = true;
		...
		for (const FString& IncludedClass : IncludedClasses)
		{
			...
			{
				UE_LOGF(LogGenerateAssetManifestCommandlet, Error, "Failed to convert short class name \"%ls\" to path name. Please use class path names for IncludedClasses.", *IncludedClass);
			}
			else
			{
	...
	{
		UE_LOGF(LogGenerateAssetManifestCommandlet, Display, "Excluding Assets from specified paths");
		FARFilter Filter;
		Filter.bIncludeOnlyOnDiskAssets = true;
		Filter.bRecursivePaths = true;
	...
	{
		UE_LOGF(LogGenerateAssetManifestCommandlet, Display, "Excluding Assets of specified classes");
		FARFilter Filter;
		Filter.bIncludeOnlyOnDiskAssets = true;
		Filter.PackagePaths = ClassPackagePaths;
		Filter.bRecursivePaths = true;
		for (const FString& ExcludedClass : ExcludedClasses)
		{
			...
			{
				UE_LOGF(LogGenerateAssetManifestCommandlet, Error, "Failed to convert short class name \"%ls\" to path name. Please use class path names for ExcludedClasses.", *ExcludedClass);
			}
			else
			{
	...
	{
		UE_LOGF(LogGenerateAssetManifestCommandlet, Display, "Converting Package Names to File Paths");
		...
		{
			UE_LOGF(LogGenerateAssetManifestCommandlet, Error, "Failed to save output file '%ls'", *ManifestFile);
			return 1;
		}
	}
