...
int32 USubstrateCommandlet::Main(const FString& Params)
{
	...
	{
		UE_LOGF(LogSubstrateCommandlet, Log, "SubstrateCommandlet");
		UE_LOGF(LogSubstrateCommandlet, Log, "This commandlet will dump information about Substrate materials.");
		UE_LOGF(LogSubstrateCommandlet, Log, "A typical way to invoke it is: <YourProject> -run=Substrate -DumpSubstrateMaterials -targetplatform=Windows ,");
	...
	UE_LOGF(LogSubstrateCommandlet, Log, "Searching for materials within the project...");
	...
	UE_LOGF(LogSubstrateCommandlet, Log, "Found %d materials", MaterialAssets.Num());
	...
	if (Platforms.Num() > 0)
	{
		...
		UE_LOGF(LogSubstrateCommandlet, Display, "Compiling shaders for %ls...", *Platform->PlatformName());
...
#if SUBSTRATER_COMMANDLET_SHADER_COMPILATION
					UE_LOGF(LogSubstrateCommandlet, Display, "BeginCache for %ls", *MaterialInterface->GetFullName());
		...
		UE_LOGF(LogSubstrateCommandlet, Log, "Found %d materials to compile and check.", MaterialsToCompile.Num());
		...
		// Submit all the jobs.
		{
			...
			UE_LOGF(LogSubstrateCommandlet, Display, "Submit Jobs");
			...
			while (MaterialsToCompile.Num())
			{
				for (auto It = MaterialsToCompile.CreateIterator(); It; ++It)
				{
					...
					{
						It.RemoveCurrent();
						UE_LOGF(LogSubstrateCommandlet, Display, "Finished cache for %ls.", *MaterialInterface->GetFullName());
						UE_LOGF(LogSubstrateCommandlet, Display, "Materials remaining: %d", MaterialsToCompile.Num());
					}
					...
					while (true)
					{
						...
						{
							UE_LOGF(LogSubstrateCommandlet, Display, "Outstanding Jobs: %d", CurrentOutstandingJobs);
		...
		// Process the shader maps and save to the DDC.
		{
			...
			UE_LOGF(LogSubstrateCommandlet, Log, "ProcessAsyncResults");
		...
		// Perform cleanup and clear cached data for cooking.
		{
			...
			UE_LOGF(LogSubstrateCommandlet, Display, "Clear Cached Cooked Platform Data");
	...
	if (Switches.Contains("DumpSubstrateMaterials"))
	{
		UE_LOGF(LogSubstrateCommandlet, Log, "***********************");
		UE_LOGF(LogSubstrateCommandlet, Log, "* Substrate Materials *");
		UE_LOGF(LogSubstrateCommandlet, Log, "***********************");
		for (UMaterialInterface* MaterialInterface : SubstrateMaterials)
		{
			UE_LOGF(LogSubstrateCommandlet, Log, "  - %ls", *MaterialInterface->GetPathName());
		}

		UE_LOGF(LogSubstrateCommandlet, Log, "Substrate Material Count:.........%d.", SubstrateMaterials.Num());
		UE_LOGF(LogSubstrateCommandlet, Log, "Non Substrate Material Count:.....%d.", MaterialAssets.Num() - SubstrateMaterials.Num());
	}
	return 0;
}
