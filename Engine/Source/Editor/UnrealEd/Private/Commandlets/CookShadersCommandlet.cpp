...
int32 UCookShadersCommandlet::Main(const FString& Params)
{
	...
	{
		UE_LOGF(LogCookShadersCommandlet, Log, "CookShadersCommandlet");
		UE_LOGF(LogCookShadersCommandlet, Log, "Cook shaders based upon the options, ideal for generating symbols for shaders you need");
		UE_LOGF(LogCookShadersCommandlet, Log, "Options:");
		UE_LOGF(LogCookShadersCommandlet, Log, " Required: -targetPlatform=<platform>     (Which target platform do you want results, e.g. WindowsClient, etc.");
		UE_LOGF(LogCookShadersCommandlet, Log, " Required: -ShaderSymbolsExport=<path>    (Set shader symbols output location.");
		UE_LOGF(LogCookShadersCommandlet, Log, " Optional: -infoFile=<path>               (Path to ShaderSymbols.info file you want to find shaders from.");
		UE_LOGF(LogCookShadersCommandlet, Log, " Optional: -filter=<string>               (Recommended! Filter to shaders with <string> in their hash or info data, requires -infoFile).");
		UE_LOGF(LogCookShadersCommandlet, Log, " Optional: -material=<string>             (Cook this material if you don't have a .info file, can be Global for global shaders).");
		UE_LOGF(LogCookShadersCommandlet, Log, " Optional: -noglobals                     (Don't do global shaders, even if they match the filter.)");
		UE_LOGF(LogCookShadersCommandlet, Log, " Optional: -nomaterialinstances           (Don't do material instances)");
		UE_LOGF(LogCookShadersCommandlet, Log, " Optional: -useshortnames				  (ShaderSymbols.info was produced with r.DumpShaderDebugShortNames=1. We need to convert back the vertex factory names");
	...
	if (!InfoFilePath.IsEmpty())
	{
		...
		{
			UE_LOGF(LogCookShadersCommandlet, Log, "Unabled to read / parse info file '%ls'", *InfoFilePath);
			return 0;
		}
	}
	...
	// Load asset lists
	UE_LOGF(LogCookShadersCommandlet, Display, "Loading Asset Registry...");
	...
	{
		UE_LOGF(LogCookShadersCommandlet, Display, "Couldn't find anything to process!");
		return 0;
	}
	...
	for (int32 Index = 0; Index < Platforms.Num(); Index++)
	{
		...
		for (int32 FormatIndex = 0; FormatIndex < DesiredShaderFormats.Num(); FormatIndex++)
		{
			...
			UE_LOGF(LogCookShadersCommandlet, Log, "Working on %ls %ls", *PlatformName, *ShaderPlatformName);
			...
			if (!IndividualRequests.IsEmpty())
			{
				UE_LOGF(LogCookShadersCommandlet, Display, "Cooking Individual Shaders...");
			...
			if (!bNoGlobals && !GlobalsToFind.IsEmpty())
			{
				...
				UE_LOGF(LogCookShadersCommandlet, Display, "Cooking Global Shaders...");
			...
			{
				UE_LOGF(LogCookShadersCommandlet, Display, "Cooking Materials...");
	...
	if (!ExportPath.IsEmpty() && !InfoFilePath.IsEmpty())
	{
		for (const auto& I : Info)
		{
			...
			{
				UE_LOGF(LogCookShadersCommandlet, Warning, "Did not generate symbol file '%ls' for '%ls'", *I.Hash, *I.Name);
			}
		}
	}

	UE_LOGF(LogCookShadersCommandlet, Display, "Done CookShadersCommandlet");
	return 0;
}
