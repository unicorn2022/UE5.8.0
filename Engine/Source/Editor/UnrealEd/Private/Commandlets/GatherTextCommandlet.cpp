...
int32 UGatherTextCommandlet::Execute(const FString& Params, const TSharedPtr<const FGatherTextCommandletEmbeddedContext>& InEmbeddedContext)
{
	...
	{
		UE_LOGF(LogGatherTextCommandlet, Display, "%ls", *UsageText);
		return 0;
	}
	...
	UE_LOGF(LogGatherTextCommandlet, Display, "LegacyScheduler: %ls", bLegacyScheduler ? TEXT("Yes") : TEXT("No"));

	bRunningInPreview = Switches.Contains(UGatherTextCommandletBase::PreviewSwitch);
	if (bRunningInPreview)
	{
		UE_LOGF(LogGatherTextCommandlet, Display, "Running commandlet in preview mode. Some child commandlets and steps will be skipped.");
		...
		{
			UE_LOGF(LogGatherTextCommandlet, Display, "Running commandlet with gather type %ls. Only %ls files will be gathered.", **GatherTypeParamPtr, **GatherTypeParamPtr);
		}
		else
		{
			// if the -GatherType param is not specified, we default to gathering both source, asset and metadata
			UE_LOGF(LogGatherTextCommandlet, Display, "-GatherType param not specified. Commandlet will gather source, assset and metadata.");
		}
	}
	...
	if (CommandletSourceControlInfo.IsValid() && !bDisableSubmit)
	{
		...
		{
			UE_LOGF(LogGatherTextCommandlet, Log, "Submitted Localization files.");
		}
		else
		{
			UE_LOGFMT(LogGatherTextCommandlet, Error, "Revision control error: {error}",
	...
	UE_LOGF(LogGatherTextCommandlet, Display, "Completed all steps in %.2f seconds", FPlatformTime::Seconds() - AllCommandletExecutionStartTime);
	...
	UE_LOGF(LogGatherTextCommandlet, Display, "GatherText completed with exit code 0");
	return 0;
}
...
bool UGatherTextCommandlet::ScheduleGatherConfigs(const TArray<FString>& GatherTextConfigPaths, const TSharedPtr<FLocalizationSCC>& CommandletSourceControlInfo, const TArray<FString>& Tokens, const TArray<FString>& Switches, const TMap<FString, FString>& ParamVals)
{
	...
	for (int32 PhaseIndex = (int32)EGatherTextCommandletPhase::FirstPhase; PhaseIndex < (int32)EGatherTextCommandletPhase::NumPhases; ++PhaseIndex)
	{
		...
		{
			UE_LOGF(LogGatherTextCommandlet, Display, "Phase '%ls' has a single parallel task. Adding it to the end of its sequential tasks.", LexToString((EGatherTextCommandletPhase)PhaseIndex));
	...
	for (int32 PhaseIndex = (int32)EGatherTextCommandletPhase::FirstPhase; PhaseIndex < (int32)EGatherTextCommandletPhase::NumPhases; ++PhaseIndex)
	{
		...
		UE_LOGF(LogGatherTextCommandlet, Display, "Phase '%ls':\n\t%d sequential task(s): %ls\n\t%d parallel tasks(s): %ls",
...
{
	UE_LOGF(LogGatherTextCommandlet, Display, "Scheduling GatherText tasks for '%ls'", *GatherTextConfigPath);
	...
	for (const FString& StepName : StepNames)
	{
		...
		{
			UE_LOGF(LogGatherTextCommandlet, Display, "Should not run %ls: %ls in preview. Skipping.", *StepName, *CommandletClassName);
			continue;
		}
...
bool UGatherTextCommandlet::ExecuteGatherTextPhases(const TSharedPtr<FLocalizationSCC>& CommandletSourceControlInfo)
{
	...
	auto RunTaskCommandlet =
		[](const FGatherTextCommandletTask& Task)
		{
			...
			UE_LOGF(LogGatherTextCommandlet, Display, "Executing %ls (params: %ls)", *CommandletClassName, *Task.CommandletParams);
			...
			UE_LOGF(LogGatherTextCommandlet, Display, "Executing %ls took %.2f seconds (params: %ls)", *CommandletClassName, FPlatformTime::Seconds() - CommandletExecutionStartTime, *Task.CommandletParams);
			return true;
		};

	const bool bCanRunParallelTasks = GatherTextCommandlet::bAllowParallelTasks && (!EmbeddedContext || EmbeddedContext->IsThreadSafe());
	UE_LOGF(LogGatherTextCommandlet, Display, "CanRunParallelTasks: %ls", bCanRunParallelTasks ? TEXT("Yes") : TEXT("No"));
	...
	for (int32 PhaseIndex = (int32)EGatherTextCommandletPhase::FirstPhase; PhaseIndex < (int32)EGatherTextCommandletPhase::NumPhases; ++PhaseIndex)
	{
		...
		UE_LOGF(LogGatherTextCommandlet, Display, "Running phase '%ls' (%d sequential tasks and %d parallel tasks).", LexToString((EGatherTextCommandletPhase)PhaseIndex), Phase.SequentialTasks.Num(), Phase.ParallelTasks.Num());

		// Run sequential tasks
		if (Phase.SequentialTasks.Num() > 0)
		{
			const double SequentialTasksExecutionStartTime = FPlatformTime::Seconds();
			UE_LOGF(LogGatherTextCommandlet, Display, "Running %d sequentials task(s) in phase '%ls'...", Phase.SequentialTasks.Num(), LexToString((EGatherTextCommandletPhase)PhaseIndex));

			int32 TaskProgress = 0;
			for (const FGatherTextCommandletTask& Task : Phase.SequentialTasks)
			{
				UE_LOGF(LogGatherTextCommandlet, Display, "Starting sequential task %d of %d in phase '%ls'...", ++TaskProgress, Phase.SequentialTasks.Num(), LexToString((EGatherTextCommandletPhase)PhaseIndex));
			...
			UE_LOGF(LogGatherTextCommandlet, Display, "%d sequentials task(s) in phase '%ls' took %.2f seconds.", Phase.SequentialTasks.Num(), LexToString((EGatherTextCommandletPhase)PhaseIndex), FPlatformTime::Seconds() - SequentialTasksExecutionStartTime);
		}
		...
		if (Phase.ParallelTasks.Num() > 0)
		{
			...
			UE_LOGF(LogGatherTextCommandlet, Display, "Starting %d parallel task(s) in phase '%ls'...", Phase.ParallelTasks.Num(), LexToString((EGatherTextCommandletPhase)PhaseIndex));
			...
			UE_LOGF(LogGatherTextCommandlet, Display, "%d parallel task(s) in phase '%ls' took %.2f seconds.", Phase.ParallelTasks.Num(), LexToString((EGatherTextCommandletPhase)PhaseIndex), FPlatformTime::Seconds() - ParallelTasksExecutionStartTime);
		}
		...
		UE_LOGF(LogGatherTextCommandlet, Display, "Phase '%ls' took %.2f seconds (%d sequential tasks and %d parallel tasks).", LexToString((EGatherTextCommandletPhase)PhaseIndex), FPlatformTime::Seconds() - PhaseExecutionStartTime, Phase.SequentialTasks.Num(), Phase.ParallelTasks.Num());
