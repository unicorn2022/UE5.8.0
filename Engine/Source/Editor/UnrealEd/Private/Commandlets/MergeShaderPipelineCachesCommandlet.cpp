...
int32 UMergeShaderPipelineCachesCommandlet::Main(const FString& Params)
{
	...
	if (bOK == false)
	{
		UE_LOGF(LogMergeShaderPipelineCachesCommandlet, Warning, "Usage: FilePathA FilePathB -Sort=<Order> OutputPath.\n\tOrder Values:Default/FirstUsed/MostUsed.\n\tCache files must be have the same game version, shader platform and format version.");
        UE_LOGF(LogMergeShaderPipelineCachesCommandlet, Warning, "Provided arguments: %ls\n\tA: %ls\n\tB: %ls\n\tOutput: %ls.", *Params, *A, *B, *Output);
		return 0;
	}
