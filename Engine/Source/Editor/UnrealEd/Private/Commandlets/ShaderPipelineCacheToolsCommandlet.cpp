...
void ExpandWildcards(TArray<FString>& Parts)
{
	...
	for (const FString& OldPart : Parts)
	{
		if (OldPart.Contains(TEXT("*")) || OldPart.Contains(TEXT("?")))
		{
			...
			UE_CLOGF(!ExpandedFiles.Num(), LogShaderPipelineCacheTools, Log, "Expanding %ls....did not match anything.", *OldPart);
			UE_CLOGF(ExpandedFiles.Num(), LogShaderPipelineCacheTools, Log, "Expanding matched %4d files: %ls", ExpandedFiles.Num(), *OldPart);
			for (const FString& Item : ExpandedFiles)
			{
				UE_LOGF(LogShaderPipelineCacheTools, Log, "                             : %ls", *Item);
				NewParts.Add(Item);
			}
		}
		else
...
{
	UE_LOGF(LogShaderPipelineCacheTools, Display, "Loading %.*ls...", FileName.Len(), FileName.GetData());

	const int32 StableArrayOffset = StableArray.Num();

	if (!UE::PipelineCacheUtilities::LoadStableKeysFile(FileName, StableArray))
	{
		UE_LOGF(LogShaderPipelineCacheTools, Error, "Could not load stable shader keys from %.*ls.", FileName.Len(), FileName.GetData());
	}

	UE_LOGF(LogShaderPipelineCacheTools, Display, "Loaded %d shader info lines from %.*ls.", (StableArray.Num() - StableArrayOffset), FileName.Len(), FileName.GetData());
}

static void LoadStableShaderKeysMultiple(FStableShaderSet& StableMap, TArrayView<const FStringView> FileNames)
{
	// This is now available in PipelineCacheUtilities
	UE::PipelineCacheUtilities::LoadMultipleStableKeysFiles(FileNames, StableMap);
}

// Version optimized for ExpandPSOSC
static void LoadStableShaderKeysMultiple(TMultiMap<int32, FShaderHash>& StableMap, TArray<FStableShaderKeyAndValue>& StableShaderKeyIndexTable, TArrayView<const FStringView> FileNames)
{
...
static bool LoadAndDecompressStableCSV(const FString& Filename, TArray<FString>& OutputLines)
{
		...
		if (Ar->TotalSize() > 8)
		{
			...
			if (CompressedVersion >= STABLE_COMPRESSED_VER)
			{
				...
				for (int32 Index = 0; Index < NumChunks; ++Index)
				{
					...
					if (!bResult)
					{
						UE_LOGF(LogShaderPipelineCacheTools, Display, "Failed to decompress file %ls", *Filename);
					}
			...
			{
				UE_LOGF(LogShaderPipelineCacheTools, Warning, "File %ls is too old (version %d, we need at least %d), rejecting.", *Filename, CompressedVersion, STABLE_COMPRESSED_VER);			
			}
		}
		else
		{
			UE_LOGF(LogShaderPipelineCacheTools, Display, "Corrupted file %ls", *Filename);
		}
	...
	{
		UE_LOGF(LogShaderPipelineCacheTools, Display, "Failed to open file %ls", *Filename);
	}
...
static bool LoadStableCSV(const FString& Filename, TArray<FString>& OutputLines)
{
	...
	{
		UE_LOGF(LogShaderPipelineCacheTools, Warning, "Uncompressed CSV files are no longer supported, rejecting %ls.", *Filename);
	}
...
static int64 SaveStableCSV(const FString& Filename, const FSCDataChunk* DataChunks, int32 NumChunks)
{
	if (Filename.EndsWith(STABLE_CSV_COMPRESSED_EXT))
	{
		UE_LOGF(LogShaderPipelineCacheTools, Display, "Compressing output, %d chunks", NumChunks);
		...
		for (int32 Index = 0; Index < NumChunks; ++Index)
		{
			...
			UE_LOGF(LogShaderPipelineCacheTools, Display, "Compressing chunk %d, size = %.1fKB", Index, Chunk.UncomressedOutputLines.Num() / 1024.f);
			...
			{
				UE_LOGF(LogShaderPipelineCacheTools, Fatal, "Failed to compress chunk %d (%.1f KB)", Index, Chunk.UncomressedOutputLines.Num() / 1024.f);
			}
		}
		...
		if (!Ar)
		{
			UE_LOGF(LogShaderPipelineCacheTools, Fatal, "Failed to open %ls", *Filename);
			return -1;
		}
	...
	{
		UE_LOGF(LogShaderPipelineCacheTools, Warning, "SaveStableCSV does not support saving uncompressed files.");
	}

	int64 Size = IFileManager::Get().FileSize(*Filename);
	if (Size < 1)
	{
		UE_LOGF(LogShaderPipelineCacheTools, Error, "Failed to write %ls", *Filename);
	}

	return Size;
}

static void PrintShaders(const TMap<FShaderHash, TArray<FString>>& InverseMap, const FShaderHash& Shader)
{
	if (Shader == FShaderHash())
	{
		UE_LOGF(LogShaderPipelineCacheTools, Display, "    null");
		return;
	}
	const TArray<FString>* Out = InverseMap.Find(Shader);
	if (!Out)
	{
		UE_LOGF(LogShaderPipelineCacheTools, Display, "    No shaders found with hash %ls", *Shader.ToString());
		return;
	}

	for (const FString& Item : *Out)
	{
		UE_LOGF(LogShaderPipelineCacheTools, Display, "    %ls", *Item);
	}
}
...
bool CheckPSOStringInveribility(const FPipelineCacheFileFormatPSO& Item)
{
	...
	UE_LOGF(LogShaderPipelineCacheTools, Verbose, "CheckPSOStringInveribility: %ls", *StringRep);
...
{
	TMap<FShaderHash, TArray<FString>> InverseMap;
	...
	UE_LOGF(LogShaderPipelineCacheTools, Display, "Loading %ls....", *Token);
	if (!FPipelineFileCacheManager::LoadPipelineFileCacheInto(Token, PSOs))
	{
		UE_LOGF(LogShaderPipelineCacheTools, Error, "Could not load %ls or it was empty.", *Token);
		return 1;
	}
	...
	{
		UE_LOGF(LogShaderPipelineCacheTools, Display, "--- Entry %d --------------------------------", Count);
		FString ReadablePSODesc = Item.ToStringReadable();
		UE_LOGF(LogShaderPipelineCacheTools, Display, "%ls", *ReadablePSODesc);

		if (Item.Type == FPipelineCacheFileFormatPSO::DescriptorType::Compute)
		{
			check(!(Item.ComputeDesc.ComputeShader == FShaderHash()));
			UE_LOGF(LogShaderPipelineCacheTools, Display, "%ls", *Item.ComputeDesc.ToString());
		}
		else if (Item.Type == FPipelineCacheFileFormatPSO::DescriptorType::Graphics)
		{
			check(Item.GraphicsDesc.VertexShader != FShaderHash() || Item.GraphicsDesc.MeshShader != FShaderHash());
			UE_LOGF(LogShaderPipelineCacheTools, Display, "%ls", *Item.GraphicsDesc.ToString());

			if (InverseMap.Num())
			{
				UE_LOGF(LogShaderPipelineCacheTools, Display, "VertexShader");
				PrintShaders(InverseMap, Item.GraphicsDesc.VertexShader);
				UE_LOGF(LogShaderPipelineCacheTools, Display, "FragmentShader");
				PrintShaders(InverseMap, Item.GraphicsDesc.FragmentShader);
				UE_LOGF(LogShaderPipelineCacheTools, Display, "GeometryShader");
		...
		{
			UE_LOGF(LogShaderPipelineCacheTools, Display, "%ls", *Item.RayTracingDesc.ToString());
		}
		else
		{
			UE_LOGF(LogShaderPipelineCacheTools, Error, "Unexpected pipeline cache descriptor type %d", int32(Item.Type));
		}
		++Count;
	}
	UE_LOGF(LogShaderPipelineCacheTools, Display, "%ls", *FPipelineCacheFileFormatPSO::GraphicsDescriptor::HeaderLine());
	UE_LOGF(LogShaderPipelineCacheTools, Display, "Total PSOs logged: %d", PSOs.Num());
...
static void PrintShaders(const TMap<FShaderHash, TArray<int32>>& InverseMap, TArray<FStableShaderKeyAndValue>& StableArray, const FShaderHash& Shader, const TCHAR *Label)
{
	UE_LOGF(LogShaderPipelineCacheTools, Display, " -- %ls", Label);
	...
	{
		UE_LOGF(LogShaderPipelineCacheTools, Display, "    %ls", *StableArray[Item].ToString());
	}
}

static bool GetStableShaders(const TMap<FShaderHash, TArray<int32>>& InverseMap, TArray<FStableShaderKeyAndValue>& StableArray, const FShaderHash& Shader, TArray<int32>& StableShaders, bool& bOutAnyActiveButMissing)
{
	if (Shader == FShaderHash())
	{
		return false;
	...
	if (!Out)
	{
		UE_LOGF(LogShaderPipelineCacheTools, Warning, "No shaders found with hash %ls", *Shader.ToString());
		UE_LOGF(LogShaderPipelineCacheTools, Warning, "If you can find the old %ls file for this build, adding it will allow these PSOs to be usable.", ShaderStableKeysFileExt);
	...
	for (const int32& Item : *Out)
	{
		...
		{
			UE_LOGF(LogShaderPipelineCacheTools, Error, "Duplicate stable shader. This is bad because it means our stable key is not exhaustive.");
			UE_LOGF(LogShaderPipelineCacheTools, Error, " %ls", *StableArray[Item].ToString());
			continue;
		}
...
static void StableShadersSerializationSelfTest(const FStableShaderSet& StableMap)
{
	...
	{
		TestString.Reset();
		check(ShaderKeyPtr->OutputHash != FShaderHash());
...
// Version optimized for ExpandPSOSC
static void StableShadersSerializationSelfTest(const TMultiMap<int32, FShaderHash>& StableMap, const TArray<FStableShaderKeyAndValue>& StableArray)
{
	TAnsiStringBuilder<384> TestString;
...
bool CouldBeUsedTogether(const FStableShaderKeyAndValue& A, const FStableShaderKeyAndValue& B)
{
	...
	if ((A.PipelineHash != FShaderHash()) || (B.PipelineHash != FShaderHash()))
	{
...
int32 DumpStableKeysFile(const FString& Token, const TMap<FString, FString>& ParamVals)
{
	...
	UE_LOGF(LogShaderPipelineCacheTools, Display, "    %ls", *FStableShaderKeyAndValue::HeaderLine());
	for (const TUniquePtr<FStableShaderKeyAndValue>& ShaderKeyPtr : StableMap)
	{
		...
		UE_LOGF(LogShaderPipelineCacheTools, Display, "    %ls", *ShaderKeyPtr->ToString());
	}
	return 0;
}

int32 ExtractShaderFromUshaderbytecode(const FString& UshaderbytecodeName, const FString& ShaderHash)
{
	TUniquePtr<FArchive> FileReader(IFileManager::Get().CreateFileReader(*UshaderbytecodeName));
	if (!FileReader)
	{
		UE_LOGF(LogShaderPipelineCacheTools, Error, "Cannot open ushaderbytecode file at %ls", *UshaderbytecodeName);
		return 1;
	}
	...
	{
		UE_LOGF(LogShaderPipelineCacheTools, Error, "Error reading ushaderbytecode file at %ls", *UshaderbytecodeName);
		return 1;
	}

	FLargeMemoryReader LibraryAr(Data.GetData(), Data.Num());
	uint32 Version;
	LibraryAr << Version;

	UE_LOGF(LogShaderPipelineCacheTools, Display, "Reading ushaderbytecode file of version %d", Version);

	FSerializedShaderArchive SerializedShaders;
	LibraryAr << SerializedShaders;
	int64 OffsetToShaderCode = LibraryAr.Tell();

	UE_LOGF(LogShaderPipelineCacheTools, Display, "Number of shaders %d", SerializedShaders.GetNumShaders());

	FShaderHash HashToLookFor;
	HashToLookFor.FromString(ShaderHash);
	UE_LOGF(LogShaderPipelineCacheTools, Display, "Looking for shader hash %ls", *HashToLookFor.ToString());	// print so we know the conversion was successful

	bool bHaveAtLeastOneMatch = false;
	for (int IdxShader = 0, NumShaders = SerializedShaders.GetNumShaders(); IdxShader < NumShaders; ++IdxShader)
	{
		if (SerializedShaders.ShaderHashes[IdxShader] == HashToLookFor)
		{
			UE_LOGF(LogShaderPipelineCacheTools, Display, "Shader %d matches hash %ls", IdxShader, *SerializedShaders.ShaderHashes[IdxShader].ToString());	// print so we know the conversion was successful
			bHaveAtLeastOneMatch = true;
			const FShaderCodeEntry& Entry = SerializedShaders.ShaderEntries[IdxShader];

			// extract compressed first
			FString SavePath = FPaths::ProjectSavedDir() / TEXT("ExtractedShaders") / FPaths::GetBaseFilename(UshaderbytecodeName);
			FString ShaderName = FString::Printf(TEXT("%s_Shader_%d.bin"), *SerializedShaders.ShaderHashes[IdxShader].ToString(), IdxShader);
			FString FinalSavePath = SavePath / ShaderName;
			UE_LOGF(LogShaderPipelineCacheTools, Display, "Extracting to %ls", *FinalSavePath);
			TUniquePtr<FArchive> FileWriter(IFileManager::Get().CreateFileWriter(*FinalSavePath));

			if (FileWriter)
			{
				uint8* PtrToShader = (static_cast<uint64>(OffsetToShaderCode) + Entry.Offset) < static_cast<uint64>(Data.Num()) ? (Data.GetData() + OffsetToShaderCode + Entry.Offset) : nullptr;
				...
				if (PtrToShader)
				{
					...
					{
						FileWriter->Serialize(PtrToShader, Entry.Size);
					}
					else
					{
						uint32 UncompressedSize = Entry.GetUncompressedSize();
						TUniquePtr<uint8[]> Buffer = MakeUnique<uint8[]>(UncompressedSize);
						ShaderCodeArchive::DecompressShaderWithOodle(Buffer.Get(), UncompressedSize, PtrToShader, Entry.Size);
						FileWriter->Serialize(Buffer.Get(), UncompressedSize);
					}
				}
			}
		}
	...
	if (!bHaveAtLeastOneMatch)
	{
		UE_LOGF(LogShaderPipelineCacheTools, Warning, "Could find a shader with hash %ls among all %d shaders", *HashToLookFor.ToString(), SerializedShaders.GetNumShaders());	// print so we know the conversion was successful
	}
	return 0;
}
...
int32 CheckStableKeyAliasing(const TArray<FString>& Tokens)
{
	if (Tokens.Num() < 2)
	{
		UE_LOGF(LogShaderPipelineCacheTools, Error, "At least two shk files need to be given to check hash aliases between them.");
		return 1;
	}

	UE_LOGF(LogShaderPipelineCacheTools, Display, "Loading %d stable shader key files...", Tokens.Num());
	...
	TArray<TMultiMap<FShaderHash, const FStableShaderKeyAndValue*>> InverseStableMaps;
	...
	auto LogAllInFile = [](const FString& FileName, const TMultiMap<FShaderHash, const FStableShaderKeyAndValue*>& InverseMap, const FShaderHash& Hash)
	{
		...
		UE_LOGF(LogShaderPipelineCacheTools, Display, "In file %ls, it maps to %d shader(s):", *FileName, Values.Num());
		int32 ValuesToLog = 10;
		for (const FStableShaderKeyAndValue* Value : Values)
		{
			UE_LOGF(LogShaderPipelineCacheTools, Display, "    %ls", *Value->ToString());
			if (ValuesToLog-- <= 0)
			{
				UE_LOGF(LogShaderPipelineCacheTools, Display, "    etc (%d total)", Values.Num());
				break;
			}
		}
	};
	...
	TSet<FShaderHash> ReportedHashes;
	// check for hashes being the same across files
	for (int32 IdxFile = 0; IdxFile < Tokens.Num(); ++IdxFile)
	{
		for (const auto& Pair : InverseStableMaps[IdxFile])
		{
			TSet<FShaderHash> AliasedHashesInThisFile;	// to allow checking for more than one pair of files
			...
			for (int32 IdxOtherFile = IdxFile + 1; IdxOtherFile < Tokens.Num(); ++IdxOtherFile)
			{
				...
				{
					UE_LOGF(LogShaderPipelineCacheTools, Warning, "Shaderhash %ls is contained in both %ls and %ls files", *Pair.Key.ToString(), *Tokens[IdxFile], *Tokens[IdxOtherFile]);
	...
	{
		UE_LOGF(LogShaderPipelineCacheTools, Display, "No hash aliases between the files!");
	}
...
bool SaveStablePipelineCacheDeprecated(const FString& OutputFilename, const TArray<UE::PipelineCacheUtilities::FPermsPerPSO>& StableResults, const TArray<FStableShaderKeyAndValue>& StableShaderKeyIndexTable)
{
	...
	for (const UE::PipelineCacheUtilities::FPermsPerPSO& Item : StableResults)
	{
		if (UE_LOG_ACTIVE(LogShaderPipelineCacheTools, Verbose))
		{
			...
			{
				UE_LOGF(LogShaderPipelineCacheTools, Verbose, " Compute");
			}
			else if (Item.PSO->Type == FPipelineCacheFileFormatPSO::DescriptorType::Graphics)
			{
				UE_LOGF(LogShaderPipelineCacheTools, Verbose, " %ls", *Item.PSO->GraphicsDesc.StateToString());
			}
			else if (Item.PSO->Type == FPipelineCacheFileFormatPSO::DescriptorType::RayTracing)
			{
				UE_LOGF(LogShaderPipelineCacheTools, Verbose, " RayTracing");
			}
			else
			{
				UE_LOGF(LogShaderPipelineCacheTools, Error, "Unexpected pipeline cache descriptor type %d", int32(Item.PSO->Type));
			}
			int32 PermIndex = 0;
			for (const UE::PipelineCacheUtilities::FPermutation& Perm : Item.Permutations)
			{
				UE_LOGF(LogShaderPipelineCacheTools, Verbose, "  ----- perm %d", PermIndex);
				for (int32 SlotIndex = 0; SlotIndex < SF_NumFrequencies; SlotIndex++)
				{
					...
					ShaderKeyAndValue.OutputHash = FShaderHash(); // Saved output hash needs to be zeroed so that BuildPSOSC can use this entry even if shaders code changes in future builds
					UE_LOGF(LogShaderPipelineCacheTools, Verbose, "   %ls", *ShaderKeyAndValue.ToString());
				}
				PermIndex++;
			}

			UE_LOGF(LogShaderPipelineCacheTools, Verbose, "-----");
		}
		for (const UE::PipelineCacheUtilities::FPermutation& Perm : Item.Permutations)
		{
	...
	if (IFileManager::Get().FileExists(*UncompressedFilename))
	{
		...
		if (IFileManager::Get().FileExists(*UncompressedFilename))
		{
			UE_LOGF(LogShaderPipelineCacheTools, Error, "Could not delete %ls", *UncompressedFilename);
			return false;
		}
	}
	if (IFileManager::Get().FileExists(*CompressedFilename))
	{
		...
		if (IFileManager::Get().FileExists(*CompressedFilename))
		{
			UE_LOGF(LogShaderPipelineCacheTools, Error, "Could not delete %ls", *CompressedFilename);
			return false;
		}
	}
	...
	UE_LOGF(LogShaderPipelineCacheTools, Warning, "Wrote stable PSOs in a deprecated text format, %d lines (%.1f KB) to %ls", NumLines, FileSize / 1024.f, *OutputFilename);
	return true;
}
...
int32 ExpandPSOSC(const TArray<FString>& Tokens)
{
	...
	{
		UE_LOGF(LogShaderPipelineCacheTools, Error, "Pipeline cache filename '%ls' must end with '%ls' (or deprecated '%ls'/'%ls').",
	...
	TMultiMap<int32, FShaderHash> StableMap;
	LoadStableShaderKeysMultiple(StableMap, StableShaderKeyIndexTable, StableCSVs);
	if (!StableMap.Num())
	{
		UE_LOGF(LogShaderPipelineCacheTools, Warning, "No %ls found or they were all empty. Nothing to do.", ShaderStableKeysFileExt);
		return 0;
	}
	if (UE_LOG_ACTIVE(LogShaderPipelineCacheTools, Verbose))
	{
		UE_LOGF(LogShaderPipelineCacheTools, Verbose, "    %ls", *FStableShaderKeyAndValue::HeaderLine());
		for (const auto& Pair : StableMap)
		{
			FStableShaderKeyAndValue Temp(StableShaderKeyIndexTable[Pair.Key]);
			Temp.OutputHash = Pair.Value;
			UE_LOGF(LogShaderPipelineCacheTools, Verbose, "    %ls", *Temp.ToString());
		}
		StableShadersSerializationSelfTest(StableMap, StableShaderKeyIndexTable);
	}
	UE_LOGF(LogShaderPipelineCacheTools, Display, "Loaded %d unique shader info lines total.", StableMap.Num());
	...
	for (int32 Index = 0; Index < Tokens.Num() - 1; Index++)
	{
		...
		{
			UE_LOGF(LogShaderPipelineCacheTools, Display, "Loading %ls....", *Tokens[Index]);
			TSet<FPipelineCacheFileFormatPSO> TempPSOs;
			if (!FPipelineFileCacheManager::LoadPipelineFileCacheInto(Tokens[Index], TempPSOs))
			{
				UE_LOGF(LogShaderPipelineCacheTools, Error, "Could not load %ls or it was empty.", *Tokens[Index]);
				continue;
			}
			UE_LOGF(LogShaderPipelineCacheTools, Display, "Loaded %d PSOs", TempPSOs.Num());
			...
			for(auto& TempPSO : TempPSOs)
			{
				...
				else
				{
					...
					else
					{
						...
						UE_LOGF(LogShaderPipelineCacheTools, Warning, "Bad PSO found discarding [Invertibility=%ls Verify=%ls in: %ls]", bInvertibilityResult ? TEXT("PASS") :  TEXT("FAIL") , bVerifyResult ? TEXT("PASS") :  TEXT("FAIL"), *Tokens[Index]);
					}
				}
			}
		}
	...
	if (!PSOs.Num())
	{
		UE_LOGF(LogShaderPipelineCacheTools, Warning, "No .upipelinecache files found or they were all empty. Nothing to do.");
		return 0;
	}
	UE_LOGF(LogShaderPipelineCacheTools, Display, "Loaded %d PSOs total [Usage Mask Merged = %d].", PSOs.Num(), MergeCount);
	...
	if (UE_LOG_ACTIVE(LogShaderPipelineCacheTools, Verbose))
	{
		...
		for (const FPipelineCacheFileFormatPSO& Item : PSOs)
		{
			...
			{
			case FPipelineCacheFileFormatPSO::DescriptorType::Compute:
				UE_LOGF(LogShaderPipelineCacheTools, Verbose, "ComputeShader");
				PrintShaders(InverseMap, Item.ComputeDesc.ComputeShader);
				break;
			case FPipelineCacheFileFormatPSO::DescriptorType::Graphics:
				UE_LOGF(LogShaderPipelineCacheTools, Verbose, "VertexShader");
				PrintShaders(InverseMap, Item.GraphicsDesc.VertexShader);
				UE_LOGF(LogShaderPipelineCacheTools, Verbose, "MeshShader");
				PrintShaders(InverseMap, Item.GraphicsDesc.MeshShader);
				UE_LOGF(LogShaderPipelineCacheTools, Verbose, "AmplificationShader");
				PrintShaders(InverseMap, Item.GraphicsDesc.AmplificationShader);
				UE_LOGF(LogShaderPipelineCacheTools, Verbose, "FragmentShader");
				PrintShaders(InverseMap, Item.GraphicsDesc.FragmentShader);
				UE_LOGF(LogShaderPipelineCacheTools, Verbose, "GeometryShader");
				PrintShaders(InverseMap, Item.GraphicsDesc.GeometryShader);
				break;
			case FPipelineCacheFileFormatPSO::DescriptorType::RayTracing:
				UE_LOGF(LogShaderPipelineCacheTools, Verbose, "RayTracingShader");
	...
	TMap<FShaderHash, TArray<int32>> InverseMap;
		...
		](int32 PSOIndex)
	{
		...
		{
			FScopeLock Lock(&ConsoleOutputGuard);
			UE_LOGF(LogShaderPipelineCacheTools, Display, "PSO had an active shader slot that did not match any current shaders, ignored.");
			if (Item.Type == FPipelineCacheFileFormatPSO::DescriptorType::Compute)
			{
			...
			{
				UE_LOGF(LogShaderPipelineCacheTools, Display, "   %ls", *Item.GraphicsDesc.StateToString());
		...
		if (Item.Type == FPipelineCacheFileFormatPSO::DescriptorType::Graphics)
		{
			...
			if (!bAnyActive)
			{
				FScopeLock Lock(&ConsoleOutputGuard);
				NumSkipped++;
				UE_LOGF(LogShaderPipelineCacheTools, Verbose, "PSO did not create any stable PSOs! (no active shader slots)");
				UE_LOGF(LogShaderPipelineCacheTools, Verbose, "   %ls", *Item.GraphicsDesc.StateToString());
				return;
			}
			if (bRemovedAll)
			{
				FScopeLock Lock(&ConsoleOutputGuard);
				UE_LOGF(LogShaderPipelineCacheTools, Warning, "PSO did not create any stable PSOs! (no cross shader slot compatibility)");
				UE_LOGF(LogShaderPipelineCacheTools, Warning, "   %ls", *Item.GraphicsDesc.StateToString());
		...
		{
			FScopeLock Lock(&ConsoleOutputGuard);
			UE_LOGF(LogShaderPipelineCacheTools, Error, "PSO did not create any stable PSOs! (somehow)");
			// this is fatal because now we have a bogus thing in the list
			UE_LOGF(LogShaderPipelineCacheTools, Fatal, "   %ls", *Item.GraphicsDesc.StateToString());
			return;
		}
		...
		FScopeLock Lock(&ConsoleOutputGuard);
		UE_LOGF(LogShaderPipelineCacheTools, Verbose, "----- PSO created %d stable permutations --------------", Permutations.Num());
		TotalStablePSOs += Permutations.Num();
	},
	EParallelForFlags::Unbalanced
	);

	UE_CLOGF(NumSkipped > 0, LogShaderPipelineCacheTools, Warning, "%d/%d PSO did not create any stable PSOs! (no active shader slots)", NumSkipped, NumExamined);
	UE_LOGF(LogShaderPipelineCacheTools, Display, "Generated %d stable PSOs total", TotalStablePSOs);
	if (!TotalStablePSOs || !StableResults.Num())
	{
		UE_LOGF(LogShaderPipelineCacheTools, Error, "No stable PSOs created.");
		return 1;
	}
	...
	{
		UE_LOGF(LogShaderPipelineCacheTools, Warning, "Using a deprecated stablepc format %ls, please replace with %ls", *OutputFilename, ShaderStablePipelineFileExtWildcard);
...
static TSet<FPipelineCacheFileFormatPSO> ParseStableCSV(const FString& FileName, const TArray<FString>& CSVLines, const FStableShaderSet& StableMap, FName& TargetPlatform, int32& PSOsRejected, int32& PSOsMerged)
{
	...
	ReadStableCSV(CSVLines, [&FileName, &StableMap, &TargetPlatform, &PSOs, &LineIndex, &bParsed, &PSOsRejected, &PSOsMerged](FStringView Line)
	{
		...
		if (!bValidGraphicsDesc)
		{
			...
			UE_LOGF(LogShaderPipelineCacheTools, Warning, "File %ls is not in the correct format (GraphicsDesc) ignoring the rest of its contents.", *FileName);
			bParsed = false;
			return;
		}
		...
		for (int32 PartIndex = 2; PartIndex < Parts.Num(); ++PartIndex)
		{
			...
			if (const TUniquePtr<FStableShaderKeyAndValue>* ShaderKeyPtr = StableMap.Find(Shader))
			{
				...
				check(Match != FShaderHash());

				TargetPlatform = (*ShaderKeyPtr)->TargetPlatform;
			}
			else
			{
				UE_LOGF(LogShaderPipelineCacheTools, Verbose, "Stable PSO not found, rejecting %ls", *Shader.ToString());
				++PSOsRejected;
				return;
			}
			...
			case SF_RayCallable:
				...
				break;
			default:
				UE_LOGF(LogShaderPipelineCacheTools, Error, "Unexpected shader frequency");
			}
		}

		if (PSO.Type == FPipelineCacheFileFormatPSO::DescriptorType::Compute)
		{
			check(PSO.ComputeDesc.ComputeShader != FShaderHash() &&
				PSO.GraphicsDesc.VertexShader == FShaderHash() &&
				PSO.GraphicsDesc.MeshShader == FShaderHash() &&
				PSO.GraphicsDesc.AmplificationShader == FShaderHash() &&
				PSO.GraphicsDesc.FragmentShader == FShaderHash() &&
				PSO.GraphicsDesc.GeometryShader == FShaderHash());
		}
		...
		{
			check(PSO.RayTracingDesc.ShaderHash != FShaderHash());
		}
		else
		{
			UE_LOGF(LogShaderPipelineCacheTools, Error, "Unexpected pipeline cache descriptor type %d", int32(PSO.Type));
		}

		if (!PSO.Verify())
		{
			UE_LOGF(LogShaderPipelineCacheTools, Warning, "Bad PSO found. Verify failed. PSO discarded [Line %d in: %ls]", LineIndex, *FileName);
			++PSOsRejected;
			return;
		}
...
void FilterInvalidPSOs(TSet<FPipelineCacheFileFormatPSO>& InOutPSOs, const FStableShaderSet& StableMap)
{
	...
	UE_LOGF(LogShaderPipelineCacheTools, Display, "Running sanity check (consistency of vertex format).");

	// inverse map is needed for VS checking
	TMap<FShaderHash, TArray<FStableShaderKeyAndValue>> InverseMap;
	for (const TUniquePtr<FStableShaderKeyAndValue>& ShaderKeyPtr : StableMap)
	{
	...
	TMap<FShaderHash, FVertexDeclarationElementList> VSToVertexDescriptor;
	TSet<FShaderHash> SuspiciousVertexShaders;
	...
	for (const FPipelineCacheFileFormatPSO& CurPSO : InOutPSOs)
	{
		...
		if (CurPSO.GraphicsDesc.MeshShader != FShaderHash())
		{
			continue;
		}
	...
	UE_LOGF(LogShaderPipelineCacheTools, Display, "%d vertex shaders are used with an inconsistent vertex format", SuspiciousVertexShaders.Num());

	// remove all PSOs that have of those vertex shaders
	if (SuspiciousVertexShaders.Num() > 0)
	{
		// print what was not compatible
		UE_LOGF(LogShaderPipelineCacheTools, Display, "The following inconsistencies were noticed:");
		for (const TTuple< TTuple<EVertexElementType, EVertexElementType>, int32>& Stat : MismatchStats)
		{
			UE_LOGF(LogShaderPipelineCacheTools, Display, "%d times one PSO used the vertex shader with %ls (%d), another %ls (%d) (we don't know VS signature so assume it needs the larger type)", Stat.Value, VertexElementToString(Stat.Key.Key), Stat.Key.Key, VertexElementToString(Stat.Key.Value), Stat.Key.Value);
		}

		// print the shaders themselves
		{
			UE_LOGF(LogShaderPipelineCacheTools, Display, "These vertex shaders are used with an inconsistent vertex format:");
			int32 SuspectVSIdx = 0;
			const int32 kMaxSuspectToPrint = 50;
			for (const FShaderHash& SuspectVS : SuspiciousVertexShaders)
			{
				...
				if (Out && Out->Num() > 0)
				{
					if (Out->Num() > 1)
					{
						UE_LOGF(LogShaderPipelineCacheTools, Display, "%d: %d shaders matching hash %ls", SuspectVSIdx, Out->Num(), *SuspectVS.ToString());
						...
						if (UE_LOG_ACTIVE(LogShaderPipelineCacheTools, Verbose))
						{
							...
							{
								UE_LOGF(LogShaderPipelineCacheTools, Verbose, "    %d: %ls", SubIdx, *Item.ToString());
								++SubIdx;
							}
						}
						else
						{
							UE_LOGF(LogShaderPipelineCacheTools, Display, "    Example: %ls", *((*Out)[0].ToString()));
						}
					}
					else
					{
						UE_LOGF(LogShaderPipelineCacheTools, Display, "%d: %ls", SuspectVSIdx, *((*Out)[0].ToString()));
					}
				}
				else
				{
					UE_LOGF(LogShaderPipelineCacheTools, Warning, "Unknown shader with a hash %ls", *SuspectVS.ToString());
				}
				++SuspectVSIdx;

				if (SuspectVSIdx > kMaxSuspectToPrint)
				{
					UE_LOGF(LogShaderPipelineCacheTools, Display, "... and %d more VS hashes", SuspiciousVertexShaders.Num() - SuspectVSIdx - 1);
					break;
				}
			}
		}
	...
	auto ContainsShaderWithPossibleUnsupportedRHIFeatures = [InverseMap, ShadersUsingPossibleUnsupportedRHIFeatures](const FShaderHash& ShaderHash) -> bool
	{
		if (ShaderHash != FShaderHash())
		{
			...
			if (Shaders != nullptr)
			{
				for (const FStableShaderKeyAndValue& Shader : *Shaders)
				{
					...
					{
						UE_LOGF(LogShaderPipelineCacheTools, Display, "Filtering out PSO using shader with possible unsupported RHI feature:\n %ls", *Shader.ToString());
						return true;
					}
				}
			}
	...
	for (const FPipelineCacheFileFormatPSO& CurPSO : InOutPSOs)
	{
		...
		case FPipelineCacheFileFormatPSO::DescriptorType::Graphics:
		{
			...
			if (CurPSO.GraphicsDesc.VertexDescriptor.Num() == 0)
			{
				...
				{
					UE_LOGF(LogShaderPipelineCacheTools, Warning, "PSO with an empty vertex declaration and unknown VS %ls encountered, filtering out", *CurPSO.GraphicsDesc.VertexShader.ToString());
	...
	if (NumPSOsFilteredDueToEmptyDecls)
	{
		...
		{
			UE_LOGF(LogShaderPipelineCacheTools, Display, "");
			UE_LOGF(LogShaderPipelineCacheTools, Display, "Also, PSOs with the following vertex shaders were filtered out because VS were not known to be used with an empty declaration. ");
			UE_LOGF(LogShaderPipelineCacheTools, Display, "Check compatibility in the code and possibly add to the allowed list if a known safe usage:");

			for (const FStableShaderKeyAndValue& Shader : PossiblyIncorrectUsageWithEmptyDeclaration)
			{
				UE_LOGF(LogShaderPipelineCacheTools, Display, "  %ls", *Shader.ToString());
			}
		}
	}

	UE_LOGF(LogShaderPipelineCacheTools, Display, "=== Sanitizing results ===");
	UE_LOGF(LogShaderPipelineCacheTools, Display, "Before sanitization: .................................................................... %6d PSOs", NumPSOsOriginal);
	UE_LOGF(LogShaderPipelineCacheTools, Display, "Filtered out due to inconsistent vertex declaration for the same vertex shader:.......... %6d PSOs", NumPSOsFilteredDueToInconsistentDecls);
	UE_LOGF(LogShaderPipelineCacheTools, Display, "Filtered out due to VS being possibly incompatible with an empty vertex declaration:..... %6d PSOs", NumPSOsFilteredDueToEmptyDecls);
	UE_LOGF(LogShaderPipelineCacheTools, Display, "Filtered out due to using possible unsupported RHI features:............................. %6d PSOs", NumPSOsFilteredDueToUsingPossibleUnsupportedRHIFeatures);
	UE_LOGF(LogShaderPipelineCacheTools, Display, "-----");
	UE_LOGF(LogShaderPipelineCacheTools, Display, "Number of PSOs after sanity checks:...................................................... %6d PSOs", InOutPSOs.Num());
}
...
int32 SaveBinaryPipelineCacheFile(const FString& OutputFilename, const EShaderPlatform ShaderPlatform, const FString& ShaderFormat, const FString& ChunkInfoFilesPath, const FString& AssociatedShaderLibraryName, const FString& TargetPlatformName, const TSet<FPipelineCacheFileFormatPSO>& PSOs, const FStableShaderSet& StableMap)
{
	auto SaveSingleCacheFile = [](const FString& OutputFilename, const EShaderPlatform ShaderPlatform, const TSet<FPipelineCacheFileFormatPSO>& PSOs) -> int32
	{
		...
		if (IFileManager::Get().FileExists(*OutputFilename))
		{
			UE_LOGF(LogShaderPipelineCacheTools, Fatal, "Could not delete %ls", *OutputFilename);
		}
		...
		{
			UE_LOGF(LogShaderPipelineCacheTools, Error, "Failed to save %ls", *OutputFilename);
			return 1;
		}
		int64 Size = IFileManager::Get().FileSize(*OutputFilename);
		if (Size < 1)
		{
			UE_LOGF(LogShaderPipelineCacheTools, Fatal, "Failed to write %ls", *OutputFilename);
		}
		...
		UE_LOGF(LogShaderPipelineCacheTools, Display, "Wrote %d binary PSOs (graphics: %d compute: %d RT: %d), (%lldKB) to %ls",
	...
	else
	{
		...
		TMultiMap<FName, FShaderHash> StableNameMap;
		FGraphEventRef StableMapConvTask = FFunctionGraphTask::CreateAndDispatchWhenReady([&StableNameMap, &StableMap]
			{
				for (const TUniquePtr<FStableShaderKeyAndValue>& ShaderKeyPtr : StableMap)	// could be parallelized (skip first N*ThreadIdx iterations on each thread?)
				{
					...
					FName PackageName(*ShaderKeyPtr->ClassNameAndObjectPath.GetPackageNameOnly());
		...
		ParallelFor(ChunkInfoFilenames.Num(),
			[&ChunkDir, &ChunkInfoFilenames, &ChunkIds, &ChunkIdsAndResultAccessLock, &OverallResult, &StableNameMap, &StableMapConvTask, &ShaderPlatform, &ShaderFormat, &PSOs, &SaveSingleCacheFile](int32 Index)
			{
				...
				if (!UE::PipelineCacheUtilities::LoadChunkInfo(ChunkInfoFilenames[Index], ShaderFormat, ChunkId, OutputFilename, Packages))
				{
					...
					UE_LOGF(LogShaderPipelineCacheTools, Error, "Error loading chunk info file %ls",
...

				{
					...
					for (int32 Idx = 0, Num = ChunkInfoFilenames.Num(); Idx < Num; ++Idx)
					{
						...
						{
							UE_LOGF(LogShaderPipelineCacheTools, Error, "Error processing chunk info files: chunk info file %ls (%d-th) and %ls (%d-th) reference the same chunk Id %d",
				...
				TSet<FShaderHash> ShadersInChunk;
				for (const TPair<FName, FShaderHash>& Pair : StableNameMap)
				{	
				...
				UE_LOGF(LogShaderPipelineCacheTools, Verbose, "Shaders in chunk %d: %d, not in chunk: %d (not counting deduplicated)", ChunkId, ShadersInChunk.Num(), StableNameMap.Num() - ShadersInChunk.Num());
				...
				for (const FPipelineCacheFileFormatPSO& Item : PSOs)
				{
					...
					{
						if ((Item.GraphicsDesc.VertexShader != FShaderHash() && ShadersInChunk.Contains(Item.GraphicsDesc.VertexShader))
							|| (Item.GraphicsDesc.MeshShader != FShaderHash() && ShadersInChunk.Contains(Item.GraphicsDesc.MeshShader))
							|| (Item.GraphicsDesc.FragmentShader != FShaderHash() && ShadersInChunk.Contains(Item.GraphicsDesc.FragmentShader))
							|| (Item.GraphicsDesc.GeometryShader != FShaderHash() && ShadersInChunk.Contains(Item.GraphicsDesc.GeometryShader))
							|| (Item.GraphicsDesc.AmplificationShader != FShaderHash() && ShadersInChunk.Contains(Item.GraphicsDesc.AmplificationShader))
							)
						{
							PSOsInChunk.Add(Item);
						}
					...
					{
						if (Item.RayTracingDesc.ShaderHash != FShaderHash() && ShadersInChunk.Contains(Item.RayTracingDesc.ShaderHash))
						{
							PSOsInChunk.Add(Item);
						}
					}
				}
				UE_LOGF(LogShaderPipelineCacheTools, Verbose, "PSOs in chunk %d: %d, not in chunk: %d", ChunkId, PSOsInChunk.Num(), PSOs.Num() - PSOsInChunk.Num());
				...
				if (PSOsInChunk.Num())
				{
					...
					{
						FScopeLock Locker(&ChunkIdsAndResultAccessLock);
						UE_LOGF(LogShaderPipelineCacheTools, Error, "Couldn't write chunked cache for chunk %d (info %ls)",
		...
		for (int32 Idx = 0, Num = ChunkInfoFilenames.Num(); Idx < Num; ++Idx)
		{
			...
			{
				UE_LOGF(LogShaderPipelineCacheTools, Error, "Chunk info %ls somehow didn't yield a valid chunk id",
...
int32 BuildPSOSC(const TArray<FString>& Tokens, const TMap<FString, FString>& ParamVals)
{
	...
	for (int32 Index = 0; Index < Tokens.Num() - 1; Index++)
	{
		...
		else if (Tokens[Index].EndsWith(STABLE_CSV_EXT) || Tokens[Index].EndsWith(STABLE_CSV_COMPRESSED_EXT))
		{
			...
			UE_LOGF(LogShaderPipelineCacheTools, Warning, "Using stable pipeline cache in a deprecated text format: %ls", *Tokens[Index]);
		}
	}

	// Get the stable PC files in date order - least to most important(!?)
	UE_LOGF(LogShaderPipelineCacheTools, Display, "Sorting input stable cache files into chronological order for merge processing...");
	...
	FGraphEventRef StableMapTask = FFunctionGraphTask::CreateAndDispatchWhenReady([&StableShaderFiles, &StableMap]
	{
		...
		if (UE_LOG_ACTIVE(LogShaderPipelineCacheTools, Verbose))
		{
			...
			for (const TUniquePtr<FStableShaderKeyAndValue>& ShaderKeyPtr : StableMap)
			{
				UE_LOGF(LogShaderPipelineCacheTools, Verbose, "    %ls", *ShaderKeyPtr->ToString());
			}
				...
				LoadPSOTasks[FileIndex] = FFunctionGraphTask::CreateAndDispatchWhenReady([&StableCSV = StableCSVs[FileIndex], &FileName = StablePipelineCacheFiles[FileIndex]]
					{
						...
						{
							UE_LOGF(LogShaderPipelineCacheTools, Fatal, "Could not load %ls", *FileName);
						}
					}, TStatId());
			}
		}
			...
			if (StablePipelineCacheFiles[FileIndex].EndsWith(STABLE_CSV_EXT) || StablePipelineCacheFiles[FileIndex].EndsWith(STABLE_CSV_COMPRESSED_EXT))
			{
				...
				ParsePSOTasks[FileIndex] = FFunctionGraphTask::CreateAndDispatchWhenReady(
					...
					&TargetShaderFormat = TargetShaderFormatByFile[FileIndex]]
					{
						...
						StableCSV.Empty();
						UE_LOGF(LogShaderPipelineCacheTools, Display, "Loaded %d PSO lines from %ls. %d lines rejected, %d lines merged", PSOs.Num(), *FileName, PSOsRejected, PSOsMerged);
					}, TStatId(), & PreReqs);
			}
		}
	}
			...
			if (StablePipelineCacheFiles[FileIndex].EndsWith(ShaderStablePipelineFileExt))
			{
				...
				ParsePSOTasks[FileIndex] = FFunctionGraphTask::CreateAndDispatchWhenReady(
					...
					&TargetShaderFormat = TargetShaderFormatByFile[FileIndex]]
					{
						...
						{
							UE_LOGF(LogShaderPipelineCacheTools, Display, "Loaded %d stable PSOs from %ls. %d PSOs rejected, %d PSOs merged", PSOs.Num(), *FileName, PSOsRejected, PSOsMerged);
						}
					}, TStatId(), &PreReqs);
			}
		}
	...
	UE_LOGF(LogShaderPipelineCacheTools, Display, "Re-deduplicated into %d binary PSOs [Usage Mask Merged = %d].", PSOs.Num(), MergeCount);
	...
	if (PSOs.Num() < 1)
	{
		...
		{
			UE_LOGF(LogShaderPipelineCacheTools, Warning, "No PSOs were created!");
		}
		else
		{
			UE_LOGF(LogShaderPipelineCacheTools, Display, "No PSOs were created.");
		}
		return 0;
	}
	...
	if (UE_LOG_ACTIVE(LogShaderPipelineCacheTools, Verbose))
	{
		for (const FPipelineCacheFileFormatPSO& Item : PSOs)
		{
			...
			{
				check(Item.RayTracingDesc.ShaderHash != FShaderHash());
			...
			UE_LOGF(LogShaderPipelineCacheTools, Verbose, "%ls", *StringRep);
		}
	}
	...
	{
		UE_LOGF(LogShaderPipelineCacheTools, Display, "OpenGL detected, reducing PSOs to be BSS only as OpenGL doesn't care about the state at all when compiling shaders.");
		...
		UE_LOGF(LogShaderPipelineCacheTools, Display, "BSS only reduction produced %d binary PSOs.", PSOs.Num());
...
int32 DiffStable(const TArray<FString>& Tokens)
{
	...
	for (int32 TokenIndex = 0; TokenIndex < Tokens.Num(); TokenIndex++)
	{
		...
		UE_LOGF(LogShaderPipelineCacheTools, Display, "Loading %ls....", *Filename);
		...
		UE_LOGF(LogShaderPipelineCacheTools, Display, "Loaded %d stable PSO lines.", SourceFileContents.Num() - 1);
	...
	for (int32 TokenIndex = 0; TokenIndex < Sets.Num(); TokenIndex++)
	{
		...
		UE_LOGF(LogShaderPipelineCacheTools, Display, "********************* Loaded %d not in others %ls", InterSet.Num(), *Tokens[TokenIndex]);
...
int32 UShaderPipelineCacheToolsCommandlet::StaticMain(const FString& Params)
{
	...
	if (Tokens.Num() >= 1)
	{
		...
		else if (Tokens[0] == TEXT("Extract") && Tokens.Num() >= 3)
		{
			Tokens.RemoveAt(0);
			if (Tokens[0].EndsWith(TEXT(".ushaderbytecode")))
			{
				return ExtractShaderFromUshaderbytecode(Tokens[0], Tokens[1]);
			}
			else
			{
				UE_LOGF(LogShaderPipelineCacheTools, Warning, "Extract command expects a ushaderbytecode parameters as its first argument.\n");
			}
		}
	}
	
	UE_LOGF(LogShaderPipelineCacheTools, Warning, "Usage: Extract Foo.ushaderbytecode 123ABC (shader hash) - extracts all shaders that match the hash from this ushaderbytecode (and decompresses them).\n");
	UE_LOGF(LogShaderPipelineCacheTools, Warning, "Usage: Dump SCLInfo.%ls [...] [-outputhash=LIST] [-objname=LIST] - dumps stable keys file, optionally only dumping entries that match the specified output hashes and/or object names (hash and name lists are comma-separated).\n", ShaderStableKeysFileExt);
	UE_LOGF(LogShaderPipelineCacheTools, Warning, "Usage: Dump PSOCache.upipelinecache [SCLInfo.%ls] - dumps recorded PSOs. If optional stable keys file, converts shader hashes to readable stable shader descriptions.\n", ShaderStableKeysFileExt);
	UE_LOGF(LogShaderPipelineCacheTools, Warning, "Usage: Dump StablePSOCache.csv[.compressed] - dumps content of a stable PSO.\n");
	UE_LOGF(LogShaderPipelineCacheTools, Warning, "Usage: Diff ShaderCache1.stablepc.csv ShaderCache1.stablepc.csv [...]]\n");
	UE_LOGF(LogShaderPipelineCacheTools, Warning, "Usage: Expand Input1.upipelinecache Dir2/*.upipelinecache InputSCLInfo1.%ls Dir2/*.%ls InputSCLInfo3.%ls [...] Output.stablepc.csv\n", ShaderStableKeysFileExt, ShaderStableKeysFileExt, ShaderStableKeysFileExt);
	UE_LOGF(LogShaderPipelineCacheTools, Warning, "Usage: Build Input.stablepc.csv InputDir2/*.stablepc.csv InputSCLInfo1.%ls Dir2/*.%ls InputSCLInfo3.%ls [...] Output.upipelinecache\n", ShaderStableKeysFileExt, ShaderStableKeysFileExt, ShaderStableKeysFileExt);
	UE_LOGF(LogShaderPipelineCacheTools, Warning, "Usage: Decompress Input1.stablepc.csv.compressed Input2.stablepc.csv.compressed [...]\n");
	UE_LOGF(LogShaderPipelineCacheTools, Warning, "Usage: All commands accept stablepc.csv.compressed instead of stablepc.csv for compressing output\n");
	return 0;
}
