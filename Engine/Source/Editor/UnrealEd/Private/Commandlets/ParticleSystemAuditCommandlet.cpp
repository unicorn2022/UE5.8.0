...
bool UParticleSystemAuditCommandlet::ProcessParticleSystems()
{
	...
	for (const FAssetData& AssetIt : AssetList)
	{
		...
		if (PackageName != LastPackageName)
		{
			...
			{
				UE_LOGF(LogParticleSystemAuditCommandlet, Warning, "Failed to load package %ls processing %ls", *PackageName, *PSysName);
				CurrentPackage = NULL;
			}
		}
							...
							if ( UParticleModuleLocationBoneSocket* HighestBoneModule = Cast<UParticleModuleLocationBoneSocket>(HighestLODModule) )
							{
								...
								if (!bRequiresSourceLocationValidation )
								{
									for (int32 LODIdx = 1; LODIdx < Emitter->LODLevels.Num(); ++LODIdx)
									{
										...
										{
											UE_LOGF(LogParticleSystemAuditCommandlet, Warning, "ParticleSystem '%ls' Emitter '%ls' LOD index '%d' has incorrect module type '%ls' when should be 'UParticleModuleLocationBoneSocket'", *PSys->GetPathName(), *Emitter->GetName(), LODIdx, LODModule == nullptr ? TEXT("nullptr") : *LODModule->GetClass()->GetName());
											continue;
										}
								...
								if ( bRequiresSourceLocationValidation )
								{
									for (int32 LODIdx = 1; LODIdx < Emitter->LODLevels.Num(); ++LODIdx)
									{
										...
										if ( HighestBoneModule->SourceLocations.Num() != LODBoneModule->SourceLocations.Num() )
										{
											...
											UE_LOGF(LogParticleSystemAuditCommandlet, Log, "ParticleSystem '%ls' Emitter '%ls' LOD index '%d' SourceLocations does not match HighestLOD, this should have been fixed on load", *PSys->GetPathName(), *Emitter->GetName(), LODIdx);
											continue;
										}
										...
										for ( int32 SourceLocationIdx=0; SourceLocationIdx < HighestBoneModule->SourceLocations.Num(); ++SourceLocationIdx )
										{
											...
											{
												bMismatchedLODBoneModules = true;
												UE_LOGF(LogParticleSystemAuditCommandlet, Log, "ParticleSystem '%ls' Emitter '%ls' LOD index '%d' BoneSocketNames do not match between LOD levels, this is not fatal but may be an error", *PSys->GetPathName(), *Emitter->GetName(), LODIdx);
												break;
											}
										}
									}
		...
		{
			UE_LOGF(LogParticleSystemAuditCommandlet, Warning, "Failed to load particle system %ls", *PSysName);
		}
	}
	...
	UE_LOGF(LogParticleSystemAuditCommandlet, Log, "Took %5.3f seconds to process referenced particle systems...", ProcessParticleSystemsTime);
...
bool UParticleSystemAuditCommandlet::DumpSimpleSet(TSet<FString>& InSet, const TCHAR* InShortFilename, const TCHAR* InObjectClassName, const TCHAR* OptionalHeader)
{
	if (InSet.Num() > 0)
	{
		...
		{
			UE_LOGF(LogParticleSystemAuditCommandlet, Log, "Dumping '%ls' results...", InShortFilename);
...
FArchive* UParticleSystemAuditCommandlet::GetOutputFile(const TCHAR* InShortFilename)
{
	...
	if (OutputStream == NULL)
	{
		UE_LOGF(LogParticleSystemAuditCommandlet, Warning, "Failed to create output stream %ls", *Filename);
	}
	return OutputStream;
}
