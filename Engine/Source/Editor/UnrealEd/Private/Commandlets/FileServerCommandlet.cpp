...
int32 UFileServerCommandlet::Main( const FString& Params )
{
	...
	if (FParse::Value(*Params, TEXT("InstanceId="), InstanceIdString))
	{
		...
		{
			UE_LOGF(LogFileServerCommandlet, Warning, "Invalid InstanceId on command line: %ls", *InstanceIdString);
		}
	}
	...
	{
		UE_LOGF(LogFileServerCommandlet, Error, "Failed to create network file server");
