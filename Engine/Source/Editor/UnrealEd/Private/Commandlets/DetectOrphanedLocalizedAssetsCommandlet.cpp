...
int32 UDetectOrphanedLocalizedAssetsCommandlet::Main(const FString& Params)
{
	...
	{
		UE_LOGF(LogDetectOrphanedLocalizedAssetsCommandlet, Display, "%ls", *UsageText);
		return 0;
	}
	...
	UE_LOGF(LogDetectOrphanedLocalizedAssetsCommandlet, Display, "Detecting orphaned localized assets across all assets on disk.");
	IAssetRegistry::GetChecked().GetAllAssets(AllAssets);
	UE_LOGF(LogDetectOrphanedLocalizedAssetsCommandlet, Display, "Getting all assets from asset registry took %.2f seconds.", FPlatformTime::Seconds() - GetAllAssetsWithFirstPassFilterStartTime);

	UE_LOGF(LogDetectOrphanedLocalizedAssetsCommandlet, Display, "Processing %d assets.", AllAssets.Num());
	...
	UE_LOGF(LogDetectOrphanedLocalizedAssetsCommandlet, Display, "Iterating through all assets took %.2f seconds.", FPlatformTime::Seconds() - AllAssetsIterationStartTime);
	float LocalizedAssetsPercentage = static_cast<float>(LocalizedAssets.Num()) / static_cast<float>(AllAssets.Num()) * 100.0f;
	UE_LOGF(LogDetectOrphanedLocalizedAssetsCommandlet, Display, "Found %d localized assets out of %d assets. %.2f%% of assets are localized.", LocalizedAssets.Num(), AllAssets.Num(), LocalizedAssetsPercentage);
	...
	UE_LOGF(LogDetectOrphanedLocalizedAssetsCommandlet, Display, "Detecting orphaned localized assets took %.2f seconds.", FPlatformTime::Seconds() - DetectOrphansStartTime);
	float OrphanedPercentage = static_cast<float>(OrphanedLocalizedAssets.Num()) / static_cast<float>(LocalizedAssets.Num()) * 100.0f;
	UE_LOGF(LogDetectOrphanedLocalizedAssetsCommandlet, Display, "%d out of %d localized assets are orphaned. %.2f%% of all localized assets are orphaned.", OrphanedLocalizedAssets.Num(), LocalizedAssets.Num(), OrphanedPercentage);

	if (OrphanedLocalizedAssets.Num() == 0)
	{
		UE_LOGF(LogDetectOrphanedLocalizedAssetsCommandlet, Display, "No orphaned localized assets found.");
		return 0;
	}
	...
	{
		UE_LOGF(LogDetectOrphanedLocalizedAssetsCommandlet, Display, "An output file was provided. Dumping all found orphaned assets to the file: %ls", *ParamVals["OutputOrphans"]);
		FFileHelper::SaveStringArrayToFile(OrphanedLocalizedAssetsStrings, *ParamVals["OutputOrphans"]);
	}
	UE_LOGF(LogDetectOrphanedLocalizedAssetsCommandlet, Display, "Dumping %d found orphaned assets to the console.", OrphanedLocalizedAssetsStrings.Num());
	for (const FString& OrphanedLocalizedAssetString : OrphanedLocalizedAssetsStrings)
	{
		UE_LOGF(LogDetectOrphanedLocalizedAssetsCommandlet, Warning, "'%ls' is an orphaned localized asset. Either delete the asset or re-parent the localized asset to its source asset.", *OrphanedLocalizedAssetString);
	}
