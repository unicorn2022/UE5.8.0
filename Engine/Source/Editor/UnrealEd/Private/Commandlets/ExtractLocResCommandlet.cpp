...
int32 UExtractLocResCommandlet::Main(const FString& Params)
{
	...
	if (const FString* LocResParamPtr = Parameters.Find(TEXT("LocRes")))
	{
		...
		for (FString& PotentialLocResFileName : PotentialLocResFileNames)
		{
			...
			{
				UE_LOGF(LogExtractLocRes, Warning, "'%ls' did not map to a known file or directory.", *PotentialLocResFileName);
				continue;
			}
		}
	}
	...
	{
		UE_LOGF(LogExtractLocRes, Error, "-LocRes not specified or failed to resolve to a valid file.");
		return -1;
	}
	...
	for (const FString& LocResFileName : LocResFileNames)
	{
		...
		{
			UE_LOGF(LogExtractLocRes, Error, "'%ls' failed to load.", *LocResFileName);
			continue;
		}
		...
		{
			UE_LOGF(LogExtractLocRes, Error, "'%ls' failed to write.", *LocResCSVFileName);
			continue;
		}

		UE_LOGF(LogExtractLocRes, Display, "Extracted '%ls' as '%ls'.", *LocResFileName, *LocResCSVFileName);
	}
