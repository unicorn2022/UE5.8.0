...
public:
	...
	void PrintUniqueMaterialInstances()
	{
		...
		// Print each item.
		for (TPair<uint64, TArray<FString>> UniqueMaterialInstance : UniqueMaterialInstances)
		{
	...
	void AddMaterialInstance(const FString& MaterialInstanceName, uint64 StaticParameterHash)
	{
...
private:
	...
	/** Unique material instances. */
	TMap<uint64, TArray<FString>> UniqueMaterialInstances;
...
static void ProcessSwitchOptimizer(const ITargetPlatform* TargetPlatform, const EShaderPlatform ShaderPlatform, const TArray<FAssetData>& MaterialList, const TArray<FAssetData>& MaterialInstanceList, const FString& TimeNow)
{
	...
	struct FOuterKeyFuncs
	{
		...
		{
			return (uint32)Key.GetMaterialHash(false);
		}
	};
	...
	struct FInnerKeyFuncs
	{
		...
		static inline uint32 GetKeyHash(const StaticSwitchArrayType& Keys)
		{
			...
			return (uint32)Hash.Finalize().Hash;
		}
	};
	...
	auto LogProgressInInterval = [&LastProgressUpdateTimestamp](const TCHAR* Info, int32 Progress, int32 Max) -> void
		{
			...
			if (CurrentTimestamp - LastProgressUpdateTimestamp > ProgressUpdateIntervalInSeconds)
			{
				...
				UE_LOGF(LogDumpMaterialShaderTypesCommandlet, Display, "%ls%d/%d (%0.2f%%)", Info, Progress, Max, 100.0*static_cast<double>(Progress)/static_cast<double>(Max));
			}
		};
		...
		if(OuterElement.Value.Num() > 1)
		{
			...
			for (const StaticSwitchGroupType& Inner : OuterElement.Value)
			{
				for(int32 i = 0; i < Inner.Key.Num(); i++)
				{
					...
					{
						UE_LOGF(LogDumpMaterialShaderTypesCommandlet, Error, "Failed to find static switch parameter \"%ls\"", *Key.ParameterInfo.Name.ToString());
					}
				}
	...
	for (const FAssetData& AssetData : MaterialInstanceList)
	{
		if (UMaterialInstance* MaterialInstance = Cast<UMaterialInstance>(AssetData.GetAsset()))
		{
			...
			if (MaterialInstance->bHasStaticPermutationResource)
			{
				...
				OutHash = Hasher.Finalize().Hash;

				Output.AddMaterialInstance(AssetData.AssetName.ToString(), OutHash);
				Output.Log(FString::Printf(TEXT("Static Parameter Hash: %" UINT64_FMT), OutHash));
			}
...
int32 UDumpMaterialShaderTypesCommandlet::Main(const FString& Params)
{
	...
	{
		UE_LOGF(LogDumpMaterialShaderTypesCommandlet, Log, "DumpMaterialShaderTypes");
		UE_LOGF(LogDumpMaterialShaderTypesCommandlet, Log, "This commandlet will dump to a human readable plain text file of all the shaders that would be compiled for all materials in a project.");
		UE_LOGF(LogDumpMaterialShaderTypesCommandlet, Log, "Options:");
		UE_LOGF(LogDumpMaterialShaderTypesCommandlet, Log, " Required: -targetplatform=<platform(s)>     (Which target platform do you want results, e.g. WindowsClient, WindowsEditor. Multiple shader platforms are allowed).");
		UE_LOGF(LogDumpMaterialShaderTypesCommandlet, Log, " Optional: -collection=<name>                (You can also specify a collection of assets to narrow down the results e.g. if you maintain a collection that represents the actually used in-game assets).");
		UE_LOGF(LogDumpMaterialShaderTypesCommandlet, Log, " Optional: -analytics                        (Whether or not to send analytics data for tracking purposes).");
		UE_LOGF(LogDumpMaterialShaderTypesCommandlet, Log, " Optional: -staticswitches					 (Gain more detailed information of StaticSwitch use and cost).");
		return 0;
	}
...

	{
		...
		UE_LOGF(LogDumpMaterialShaderTypesCommandlet, Display, "Searching the asset registry for all assets...");
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	UE_LOGF(LogDumpMaterialShaderTypesCommandlet, Display, "Asset scan took: %.3f", AssetRegistryEnd - AssetRegistryStart);
	...
	for (int32 Index = 0; Index < Platforms.Num(); Index++)
	{
		...
		for (int32 FormatIndex = 0; FormatIndex < DesiredShaderFormats.Num(); FormatIndex++)
		{
			...
			UE_LOGF(LogDumpMaterialShaderTypesCommandlet, Display, "Dumping material shader types for '%ls' - '%ls'...", *Platforms[Index]->PlatformName(), *LexToString(ShaderPlatform));
			if(bStaticSwitches)
			{
	...
	UE_LOGF(LogDumpMaterialShaderTypesCommandlet, Display, "Dumping stats took: %.3f", FPlatformTime::Seconds() - AssetRegistryEnd);
