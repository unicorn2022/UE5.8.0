...
static USoundWave* LoadSoundWave(const FString& SoundWavePath)
{
	...
	if (!Sound)
	{
		UE_LOGF(AudioMixerCommandlet, Error, "Failed to find a USoundWave for asset path %ls", *SoundWavePath);
	}
	return Sound;
}
...
static void LoadEditorAndEngineObjects(TArray<FString>& SearchPaths, TArray<T*>& OutObjects, TArray<FString>* IgnoreList = nullptr)
{
	...
	UE_LOGF(AudioMixerCommandlet, Log, "Loaded %d objects from engine content directory", OutObjects.Num());
}
		...
		if (AudioDevice)
		{
			...
			if (bSuccess)
			{
				...
				while (true)
				{
					...
					UE_LOGF(AudioMixerCommandlet, Log, "Current Time: %.2f", CurrentTime);
	...
	bool Run(UWorld* World, const TArray<FString>& InArgs) override
	{
		...
		while (true)
		{
			...
			for (int32 i = 0; i < NumLoops; ++i)
			{
				...
				UE_LOGF(LogTemp, Log, "Position - X: %.2f, Y: %.2f, Z: %.2f", Position.X, Position.Y, Position.Z);
...
{
#if ENABLE_AUDIO_MIXER_COMMANDLET
	UE_LOGF(AudioMixerCommandlet, Display, "AudioMixerCommandlet Usage: {Editor}.exe UnrealEd.AudioMixerCommandlet {CommandName} {Args}");
	UE_LOGF(AudioMixerCommandlet, Display, "Possible commands:\n");

	UE_LOGF(AudioMixerCommandlet, Display, "Command Name, Command Description, Number of Arguments, Argument Description");
	...
	for (int32 i = 0; i < Commands.Num(); ++i)
	{
		...
		UE_LOGF(AudioMixerCommandlet, Display, "%ls, %ls, %d, %ls", *MixerCommand->GetName(), *MixerCommand->GetDescription(), MixerCommand->GetNumArgs(), *MixerCommand->GetArgDescription());
	}
#endif
}
...
int32 UAudioMixerCommandlet::Main(const FString& InParams)
{
	...
	for (int32 i = 0; i < Commands.Num(); ++i)
	{
		if (Commands[i]->GetName() == CommandName)
		{
			...
			UE_LOGF(AudioMixerCommandlet, Display, "Command %ls %ls.", *Commands[i]->GetName(), bSuccess ? TEXT("succeeded") : TEXT("failed"));
	...
	if (!bFoundTest)
	{
		UE_LOGF(AudioMixerCommandlet, Display, "Unknown test '%ls'. Exiting.", *CommandName);
		return 0;
	}
