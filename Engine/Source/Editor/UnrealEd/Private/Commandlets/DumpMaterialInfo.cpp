...
TArray<MaterialInfo::FPropertySet> GetMaterialInfoProperties()
{
	...
	TArray<FPropertySet> PropertySets = 
	{
		...
		FPropertySet::FromAccessor<bool, &FMaterialResource::IsUsedWithCurves>(TEXT("IsUsedWithCurves")),
...
int32 UDumpMaterialInfoCommandlet::Main(const FString& Params)
{
	...
	{
		UE_LOGF(LogDumpMaterialInfo, Log, "DumpMaterialInfo");
		UE_LOGF(LogDumpMaterialInfo, Log, "This commandlet will dump information about materials.");
		UE_LOGF(LogDumpMaterialInfo, Log, "A typical way to invoke it is: <YourProject> -run=DumpMaterialInfo -targetplatform=Windows -unattended -sm6 -allowcommandletrendering -nomaterialshaderddc -csv=C:/output.csv");
		UE_LOGF(LogDumpMaterialInfo, Log, "");
		UE_LOGF(LogDumpMaterialInfo, Log, "Options:");
		UE_LOGF(LogDumpMaterialInfo, Log, " -help           Print this message");
		UE_LOGF(LogDumpMaterialInfo, Log, " -help=columns   Print the list of available columns");
		UE_LOGF(LogDumpMaterialInfo, Log, " -csv=filename   Writes the output to a CSV file");
		UE_LOGF(LogDumpMaterialInfo, Log, " -material=name  Only dump materials matching this material name or regular expression");
		UE_LOGF(LogDumpMaterialInfo, Log, " -columns=a,b    Comma-seperated list of the columns that should be included in the output");
		return 0;
	}
	...
	if (Help)
	{
		...
		{
			UE_LOGF(LogDumpMaterialInfo, Display, "Available columns:");
			for (const MaterialInfo::FPropertySet& PropertySet : GetMaterialInfoProperties())
			{
				...
				{
					UE_LOGF(LogDumpMaterialInfo, Display, "   %ls", *PropertyName);
				}
			}
		}
		else
		{
			UE_LOGF(LogDumpMaterialInfo, Error, "Unknown help option %ls", **Help);
			return 1;
		}
		return 0;
	}
	...
	if (!CsvPath)
	{
		UE_LOGF(LogDumpMaterialInfo, Error, "No output CSV file path was specified");
		return 1;
	}
	...
	UE_LOGF(LogDumpMaterialInfo, Log, "Searching for materials within the project...");
	...
	TArray<FAssetData> MaterialInterfaceAssets;
	{
		...
		UE_LOGF(LogDumpMaterialInfo, Log, "Found %d materials", MaterialAssets.Num());
	...
	}
	{
		...
		UE_LOGF(LogDumpMaterialInfo, Log, "Found %d material instances", MaterialInstanceAssets.Num());
	...
	if (!CsvFileWriter)
	{
		UE_LOGF(LogDumpMaterialInfo, Error, "Failed to open output file %ls", **CsvPath);
		return 1;
	}
	...
	{
		UE_LOGF(LogDumpMaterialInfo, Display, "Compiling shaders for %ls...", *Platform->PlatformName());

		const int MaxBatchSize = 1000;
		int NumBatches = FMath::DivideAndRoundUp(MaterialInterfaceAssets.Num(), MaxBatchSize);
		for (int BatchIndex = 0; BatchIndex < NumBatches; BatchIndex++)
		{
			UE_LOGF(LogDumpMaterialInfo, Display, "Dumping batch %d of %d", BatchIndex, NumBatches);
			DumpMaterials(
				CsvWriter,
...
)
{
	...
	for (const FAssetData& AssetData : MaterialInterfaceAssets)
	{
		...
		if (UMaterialInterface* MaterialInterface = Cast<UMaterialInterface>(AssetData.GetAsset()))
		{
			...
			if (Material)
			{
				UE_LOGF(LogDumpMaterialInfo, Display, "BeginCache for %ls", *MaterialInterface->GetFullName());
	...
	UE_LOGF(LogDumpMaterialInfo, Log, "Found %d materials to compile.", MaterialsToCompile.Num());
	...
	// Submit all the jobs.
	{
		...
		UE_LOGF(LogDumpMaterialInfo, Display, "Submit Jobs");
		...
		while (MaterialsToCompile.Num())
		{
			for (auto It = MaterialsToCompile.CreateIterator(); It; ++It)
			{
				...
				{
					It.RemoveCurrent();
					UE_LOGF(LogDumpMaterialInfo, Display, "Finished cache for %ls.", *MaterialInterface->GetFullName());
					UE_LOGF(LogDumpMaterialInfo, Display, "Materials remaining: %d", MaterialsToCompile.Num());
				}
				...
				while (true)
				{
					...
					{
						UE_LOGF(LogDumpMaterialInfo, Display, "Outstanding Jobs: %d", CurrentOutstandingJobs);
	...
	// Process the shader maps and save to the DDC.
	{
		...
		UE_LOGF(LogDumpMaterialInfo, Log, "ProcessAsyncResults");
	...
	for (UMaterialInterface* MaterialInterface : MaterialsToAnalyse)
	{
		...
		if (Material)
		{
			...
			if (!CurrentResource->IsGameThreadShaderMapComplete()) { UE_LOGF(LogDumpMaterialInfo, Warning, "Missing shader map data"); }
	...
	// Perform cleanup and clear cached data for cooking.
	{
		...
		UE_LOGF(LogDumpMaterialInfo, Display, "Clear Cached Cooked Platform Data");
