...
static bool IsCsvSafeString(const FString& String)
{
	...
	for (struct DisallowedCharacter& DisallowedCharacter : DisallowedCharacters)
	{
		if (String.FindChar(DisallowedCharacter.Character, Index))
		{
			...
			{
				UE_LOGF(LogSummarizeTrace, Display, "A string contains disallowed character '%c'. See log for full list.", DisallowedCharacter.Character);
				DisallowedCharacter.First = false;
			}

			UE_LOGF(LogSummarizeTrace, Verbose, "String '%ls' contains disallowed character '%c', skipping...", *String, DisallowedCharacter.Character);
			bDisallowed = true;
		}
...
int32 USummarizeTraceCommandlet::Main(const FString& CmdLineParams)
{
	...
	{
		UE_LOGF(LogSummarizeTrace, Log, "SummarizeTrace");
		UE_LOGF(LogSummarizeTrace, Log, "This commandlet will summarize a utrace into something more easily ingestable by a reporting tool (csv).");
		UE_LOGF(LogSummarizeTrace, Log, "Options:");
		UE_LOGF(LogSummarizeTrace, Log, " Required: -inputfile=<utrace path>   (The utrace you wish to process)");
		UE_LOGF(LogSummarizeTrace, Log, " Optional: -testname=<string>         (Test name to use in telemetry csv)");
		UE_LOGF(LogSummarizeTrace, Log, " Optional: -alltelemetry              (Dump all data to telemetry csv)");
		return 0;
	}
	...
	{
		UE_LOGF(LogSummarizeTrace, Display, "Loading trace from %ls", *TraceFileName);
	}
	else
	{
		UE_LOGF(LogSummarizeTrace, Error, "You must specify a utrace file using -inputfile=<path>");
		return 1;
	}
	...
	if (!bFound)
	{
		UE_LOGF(LogSummarizeTrace, Error, "Trace file '%ls' was not found", *TraceFileName);
		return 1;
	}
	...
	{
		UE_LOGF(LogSummarizeTrace, Error, "Unable to open trace file '%ls' for read", *TraceFileName);
		delete DataStream;
		return 1;
	}
	...
	UE_LOGF(LogSummarizeTrace, Display, "Sorting %d events by total time accumulated...", DeduplicatedScopes.Num());
...
TUniquePtr<IFileHandle> USummarizeTraceCommandlet::OpenCSVFile(const FString& Name)
{
    ...
    UE_LOGF(LogSummarizeTrace, Display, "Writing %ls...", *CsvFileName);
    TUniquePtr<IFileHandle> CsvHandle(FPlatformFileManager::Get().GetPlatformFile().OpenWrite(*CsvFileName));

    if (!CsvHandle)
    {
    	UE_LOGF(LogSummarizeTrace, Error, "Unable to open csv '%ls' for write", *CsvFileName);
    }
	...
	bool SkipBaseline)
{
	...
	if (!bAllTelemetry)
	{
		...
		{
			UE_LOGF(LogSummarizeTrace, Display, "Loading global statistics from %ls", *GlobalStatisticsFileName);
		...
		{
			UE_LOGF(LogSummarizeTrace, Display, "Loading project statistics from %ls", *ProjectStatisticsFileName);
	...
	if (!bAllTelemetry)
	{
		...
		{
			UE_LOGF(LogSummarizeTrace, Display, "Comparing telemetry to baseline telemetry %ls...", *BaselineTelemetryCsvFilePath);
			...
			for (TelemetryDefinition& Telemetry : TelemetryData)
			{
				...
				if (RelatedStatistic)
				{
						...
						if (!RelatedStatistic->BaselineWarningThreshold.IsEmpty() || !RelatedStatistic->BaselineErrorThreshold.IsEmpty())
						{
							...
							if (TelemetryDefinition::MeasurementWithinThreshold(Telemetry.Measurement, BaselineTelemetry->Measurement, RelatedStatistic->BaselineWarningThreshold))
							{
								...
								if (!TelemetryDefinition::MeasurementWithinThreshold(Telemetry.Measurement, BaselineTelemetry->Measurement, SignFlippedWarningThreshold))
								{
									...
									UE_LOGF(LogSummarizeTrace, Warning, "Telemetry %ls,%ls,%ls,%ls significantly within baseline value %ls using warning threshold %ls. Please submit a new baseline to %ls or adjust the threshold in the statistics file.",
								...
								{
									UE_LOGF(LogSummarizeTrace, Verbose, "Telemetry %ls,%ls,%ls,%ls within baseline value %ls using warning threshold %ls",
										*Telemetry.TestName, *Telemetry.Context, *Telemetry.DataPoint, *Telemetry.Measurement,
										*BaselineTelemetry->Measurement, *RelatedStatistic->BaselineWarningThreshold);
								}
							}
							else
							{
								...
								{
									UE_LOGF(LogSummarizeTrace, Warning, "Telemetry %ls,%ls,%ls,%ls beyond baseline value %ls using warning threshold %ls. This could be a performance regression!",
										*Telemetry.TestName, *Telemetry.Context, *Telemetry.DataPoint, *Telemetry.Measurement,
										*BaselineTelemetry->Measurement, *RelatedStatistic->BaselineWarningThreshold);
								}
								else // it's outside the error threshold, hard error
								{
									UE_LOGF(LogSummarizeTrace, Error, "Telemetry %ls,%ls,%ls,%ls beyond baseline value %ls using error threshold %ls. This could be a performance regression!",
										*Telemetry.TestName, *Telemetry.Context, *Telemetry.DataPoint, *Telemetry.Measurement,
					...
					{
						UE_LOGF(LogSummarizeTrace, Display, "Telemetry for %ls,%ls has no baseline measurement, skipping...", *Telemetry.Context, *Telemetry.DataPoint);
					}
				}
			}
		}
		...
		{
			UE_LOGF(LogSummarizeTrace, Error, "Failed to find resolve telemety data for statistic \"%ls\"", *Statistic.Value.Name);
		}

		if (!NameToDefinitionMap.IsEmpty())
		{
			UE_LOGF(LogSummarizeTrace, Error, "Exiting...");
			return false;
		}
	}
