...
int32 USummarizeTraceEditorWorkflowsCommandlet::Main(const FString& CmdLineParams)
{
	...
	{
		UE_LOGF(LogSummarizeTrace, Log, "SummarizeTraceEditorWorkflows");
		UE_LOGF(LogSummarizeTrace, Log, "This commandlet will summarize a utrace into a csv to ease of parsing by reporting tool.");
	...
	for (const TMap<FString, FSummarizeBookmark>::ElementType& Bookmark : BookmarksProvider.Bookmarks)
	{
		...
		{
			UE_LOGF(LogSummarizeTrace, Error, "Unrecognized bookmark: '%ls', bookmark should end with either 'ProfileBegin' or 'ProfileEnd'", *BookmarkAnnotation);
		}
	}
	...
	for (const TMap<FString, TPair<double, double>>::ElementType& TestNameAndTime : TestNameAndTimes)
	{
		...
		if (!PlatformFile.DirectoryExists(*DirectoryPath))
		{
			...
			if (!PlatformFile.DirectoryExists(*DirectoryPath))
			{
				UE_LOGF(LogSummarizeTrace, Error, "Failed to create directory: '%ls'", *DirectoryPath);
				return 1;
			}
		}
...
TPair<int32, TArray<int32>> USummarizeTraceEditorWorkflowsCommandlet::ClipScopes(const TArray<const FSummarizeScope*>& Scopes, double BeginTime, double EndTime)
{
	...
	for (const FSummarizeScope* Scope : Scopes)
	{
		...
		if (EndIdx < BeginIdx)
		{
			UE_LOGF(LogSummarizeTrace, Error, "End index is smaller than begin index detected during clipping for ScopeName: '%ls'", *(Scope->Name));
			EndIdx = BeginIdx;
		}
