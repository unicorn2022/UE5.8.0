...
void UDerivedDataCacheCommandlet::MaybeMarkPackageAsAlreadyLoaded(UPackage* Package)
{
	...
	{
		UE_LOGF(LogDerivedDataCacheCommandlet, Verbose, "Marking %ls already loaded.", *Package->GetName());
...
static void WaitForCompilationToFinish(bool& bInOutHadActivity)
{
		...
		[](IAssetCompilingManager* CompilingManager)
	{
		...
		if (AssetCount > 0)
		{
			UE_LOGF(LogDerivedDataCacheCommandlet, Display, "Waiting for %d %ls to finish.", AssetCount, *FText::Format(CompilingManager->GetAssetNameFormat(), FText::AsNumber(AssetCount)).ToString());
		}
		else
		{
			UE_LOGF(LogDerivedDataCacheCommandlet, Display, "Done waiting for %ls to finish.", *FText::Format(CompilingManager->GetAssetNameFormat(), FText::AsNumber(100)).ToString());
		}
	};
...
void UDerivedDataCacheCommandlet::CacheLoadedPackages(UPackage* CurrentPackage, uint8 PackageFilter, TSet<FName>& OutNewProcessedPackages)
{
	...
	for (auto NewPackageIt = NewPackages.CreateIterator(); NewPackageIt; ++NewPackageIt)
	{
		...
		if (!ProcessedPackages.Contains(NewPackageName))
		{
			...
			{
				UE_LOGF(LogDerivedDataCacheCommandlet, Display, "Skipping %ls as Engine package", *NewPackageName.ToString());
			...
			{
				UE_LOGF(LogDerivedDataCacheCommandlet, Display, "Processing %ls", *NewPackageName.ToString());
				...
				GetObjectsWithOuter(NewPackage, ObjectsWithOuter, EGetObjectsFlags::IncludeNestedObjects, RF_ClassDefaultObject /* ExclusionFlags */);
	...
	while (CachingObjects.Num() > 0)
	{
		...
		if (!bHadActivity)
		{
			if (CurrentTime - LastActivityTime >= DDCCommandletMaxWaitSeconds)
			{
				...
				UE_LOGF(LogDerivedDataCacheCommandlet, Warning, "Timed out for %.2lfs waiting for %d objects to finish caching. First object: %ls.",
					DDCCommandletMaxWaitSeconds, CachingObjects.Num(), Object ? *Object->GetFullName() : TEXT("Unknown deleted object"));
				break;
			}
			else
			{
				const float WaitingForCacheSleepTime = 0.050f;
...
void UDerivedDataCacheCommandlet::CacheWorldPackages(UWorld* World, uint8 PackageFilter, TSet<FName>& OutNewProcessedPackages)
{
	...
	FWorldPartitionHelpers::ForEachActorWithLoading(WorldPartition, [this, PackageFilter, &OutNewProcessedPackages](const FWorldPartitionActorDescInstance* ActorDescInstance)
	{
		...
		{
			UE_LOGF(LogDerivedDataCacheCommandlet, Display, "Loaded actor %ls", *Actor->GetName());
...
int32 UDerivedDataCacheCommandlet::Main( const FString& Params )
{
	...
	if (!bStartupOnly && bFillCache)
	{
		...
		if (FString CollectionArg; FParse::Value(*Params, TEXT("COLLECTION="), CollectionArg))
		{
			...
			for (const FString& Collection : Collections)
			{
				...
				{
					UE_LOGF(LogDerivedDataCacheCommandlet, Error, "Found no collections for command line argument %ls", *Collection);
					continue;
				}
		...
		{
			UE_LOGF(LogDerivedDataCacheCommandlet, Display, "Adding default search tokens for all assets and maps");
		...
		for ( int32 TokenIndex = 0; TokenIndex < Tokens.Num(); TokenIndex++ )
		{
			...
			{
				UE_LOGF(LogDerivedDataCacheCommandlet, Display, "No packages found for parameter %i: '%ls'", TokenIndex, *Tokens[TokenIndex]);
				continue;
			}
		...
		for (FString& Filename : FilesInPath)
		{
			...
			{
				UE_LOGF(LogDerivedDataCacheCommandlet, Error, "Unable to resolve filename %ls to package name because: %ls", *Filename, *FailureReason);
				continue;
			}
		...
		if (!CommandLinePackageNames.IsEmpty())
		{
			...
			{
				UE_LOGF(LogDerivedDataCacheCommandlet, Display, "Failed to normalize command line package names");
			}
			else
			{
				for( const FString& PackageName : CommandLinePackageNames )
				{
					...
					{
						UE_LOGF(LogDerivedDataCacheCommandlet, Warning, "Unable to resolve filename from package name %ls", *PackageName);
						continue;
					}
				}
			}
		...
		for (const FDirectoryPath& DirToNotSearch : PackagingSettings->TestDirectoriesToNotSearch)
		{
			...
			{
				UE_LOGF(LogCook, Warning, "'ProjectSettings -> Project -> Packaging -> Test directories to not search' has invalid element '%ls'", *DirToNotSearch.Path);
			}
		}
		...
		{
			UE_LOGF(LogDerivedDataCacheCommandlet, Display, "No packages found to load from command line arguments.");
		}
		else
		{
			UE_LOGF(LogDerivedDataCacheCommandlet, Display, "%d packages to load from command line arguments", PackagePaths.Num());
			for( int32 Index=0; Index < PackagePaths.Num(); ++Index)
			{
				const TPair<FString, FName>& Pair = PackagePaths[Index];
				UE_LOGF(LogDerivedDataCacheCommandlet, Display, " %d) %ls", Index + 1, *Pair.Get<1>().ToString());
			}
		}
		...
		// Add all soft object references from no asset in particular to the packages to be processed, before filtering in the case of distributed work
		{
			...
			{
				UE_LOGF(LogDerivedDataCacheCommandlet, Display, "No packages found to load from startup soft references.");
			}
			else
			{
				UE_LOGF(LogDerivedDataCacheCommandlet, Display, "%d packages to load from startup soft references", PackagePaths.Num() - StartingPackageCount);
				for (int32 Index = StartingPackageCount; Index < PackagePaths.Num(); ++Index)
				{
					const TPair<FString, FName>& Pair = PackagePaths[Index];
					UE_LOGF(LogDerivedDataCacheCommandlet, Display, " %d) %ls", Index + 1 - StartingPackageCount, *Pair.Get<1>().ToString());
				}
			}
		}
		...
		if (bDoSubset)
		{
			...
			if (PackagePaths.Num() == 0)
			{
				UE_LOGF(LogDerivedDataCacheCommandlet, Display, "No packages to process after subset split!");
			}
			else
			{
				UE_LOGF(LogDerivedDataCacheCommandlet, Display, "%d packages to load after subset split", PackagePaths.Num());
				for (int32 Index = 0; Index < PackagePaths.Num(); ++Index)
				{
					const TPair<FString, FName>& Pair = PackagePaths[Index];
					UE_LOGF(LogDerivedDataCacheCommandlet, Display, " %d) %ls", Index + 1, *Pair.Get<1>().ToString());
		...
		while( PackagePaths.Num())
		{
			...
			UE_LOGF(LogDerivedDataCacheCommandlet, Display, "Loading (%d) %ls", ++PackageOrder, *Filename);

			UPackage* Package = LoadPackage(NULL, *Filename, LOAD_None);
			if (Package == NULL)
			{
				UE_LOGF(LogDerivedDataCacheCommandlet, Error, "Error loading %ls!", *Filename);
			...
			for( FName NewProcessedPackage : NewProcessedPackages)
			{
				...
				for (FName SoftRefName : SoftReferencedPackages)
				{
					...
					if (!PackagesToProcess.Contains(SoftRefName) && !ProcessedPackages.Contains(SoftRefName))
					{
						...
						if (FPackageName::DoesPackageExist(SoftRefName.ToString(), &SoftRefFilename))
						{
							UE_LOGF(LogDerivedDataCacheCommandlet, Log, "Package '%ls' queueing soft reference '%ls' for later processing", *NewProcessedPackage.ToString(), *SoftRefName.ToString());
							PackagePaths.Push(TPair<FString, FName>(SoftRefFilename, SoftRefName));
						}
						else
						{
							UE_LOGF(LogDerivedDataCacheCommandlet, Warning, "Package '%ls' failed to find soft reference '%ls'", *NewProcessedPackage.ToString(), *SoftRefName.ToString());
						}
					}
					else
					{
						UE_LOGF(LogDerivedDataCacheCommandlet, Verbose, "Package '%ls' skipping soft reference '%ls': %ls, %ls ", 
			...
			if (NumProcessedSinceLastGC >= GCInterval || PackagePaths.IsEmpty() || bLastPackageWasMap)
			{
				...
				{
					UE_LOGF(LogDerivedDataCacheCommandlet, Display, "GC (Full)...");
					CollectGarbage(RF_NoFlags);
					NumProcessedSinceLastGC = 0;
				}
				else
				{
					UE_LOGF(LogDerivedDataCacheCommandlet, Display, "GC...");
	...
	UE_LOGF(LogDerivedDataCacheCommandlet, Display, "BeginCacheTime=%.2lfs, FinishCacheTime=%.2lfs, GCTime=%.2lfs.", BeginCacheTime, FinishCacheTime, GCTime);
