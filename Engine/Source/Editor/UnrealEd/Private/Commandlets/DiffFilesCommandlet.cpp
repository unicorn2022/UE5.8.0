...
bool UDiffFilesCommandlet::Initialize(const TCHAR* Parms)
{
	...
	for (int32 TokenIndex = 0; TokenIndex < Tokens.Num(); TokenIndex++)
	{
		...
		if (PackageWildcard.Len() == 0)
		{
			...
			UE_LOGF(LogDiffFilesCommandlet, Error, "No package specified for parameter %i: %ls.  Use 'help DiffFilesCommandlet' to view correct usage syntax for this commandlet.",
	...
	{
		SET_WARN_COLOR(COLOR_RED);
		UE_LOGF(LogDiffFilesCommandlet, Error, "You must specify two packages to use this commandlet.  Use 'help DiffFilesCommandlet' to view correct usage syntax for this commandlet.");
...
void UDiffFilesCommandlet::LoadAndDiff()
{
	...
	{
		UE_LOGF(LogDiffFilesCommandlet, Warning, "FileDiffs are not currently supported for text based assets");
	}
	else
	{
