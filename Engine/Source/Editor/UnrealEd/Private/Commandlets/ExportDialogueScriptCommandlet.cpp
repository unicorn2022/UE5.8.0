...
int32 UExportDialogueScriptCommandlet::Main(const FString& Params)
{
	...
	FString ConfigPath;
	{
		...
		{
			UE_LOGF(LogExportDialogueScriptCommandlet, Error, "No config specified.");
			return -1;
		}
	...
	FString SectionName;
	{
		...
		{
			UE_LOGF(LogExportDialogueScriptCommandlet, Error, "No config section specified.");
			return -1;
		}
	...
	{
		UE_LOGF(LogExportDialogueScriptCommandlet, Error, "No source path specified.");
		return -1;
	}
	...
	{
		UE_LOGF(LogExportDialogueScriptCommandlet, Error, "No destination path specified.");
		return -1;
	}
	...
	{
		UE_LOGF(LogExportDialogueScriptCommandlet, Error, "No native culture specified.");
		return -1;
	}
	...
	{
		UE_LOGF(LogExportDialogueScriptCommandlet, Error, "No cultures specified for import.");
		return -1;
	}
	...
	{
		UE_LOGF(LogExportDialogueScriptCommandlet, Error, "No manifest name specified.");
		return -1;
	}
	...
	{
		UE_LOGF(LogExportDialogueScriptCommandlet, Error, "No archive name specified.");
		return -1;
	}
	...
	{
		UE_LOGF(LogExportDialogueScriptCommandlet, Error, "No dialogue script name specified.");
		return -1;
	}
	...
	{
		UE_LOGF(LogExportDialogueScriptCommandlet, Error, "bUseCultureDirectory may only be used with a single culture.");
		return -1;
	}
	...
	LocTextHelper.SetCopyrightNotice(GatherManifestHelper->GetCopyrightNotice());
	{
		...
		{
			UE_LOGF(LogExportDialogueScriptCommandlet, Error, "%ls", *LoadError.ToString());
			return -1;
		}
	}
	...
	{
		UE_LOGF(LogExportDialogueScriptCommandlet, Error, "Unable to get dialogue wave asset data from asset registry.");
		return -1;
	}
	...
	for (const FString& CultureName : CulturesToGenerate)
	{
		...
		for (const FAssetData& AssetData : AssetDataArrayForDialogueWaves)
		{
			...
			{
				UE_LOGF(LogExportDialogueScriptCommandlet, Error, "Asset registry found asset '%ls', but the asset with this name is not actually a dialogue wave.", *AssetData.AssetName.ToString());
				continue;
			}
			...
			if (!DialogueWave)
			{
				UE_LOGF(LogExportDialogueScriptCommandlet, Error, "Asset registry found asset '%ls', but the dialogue wave could not be accessed.", *AssetData.AssetName.ToString());
				continue;
			}
			...
			for (FDialogueContextMapping& ContextMapping : DialogueWave->ContextMappings)
			{
				...
				{
					UE_LOGF(LogExportDialogueScriptCommandlet, Log, "No internationalization manifest entry was found for context '%ls' in culture '%ls'. This context will be skipped.", *ContextLocalizationKey, *CultureName);
					continue;
				}
				...
				{
					UE_LOGF(LogExportDialogueScriptCommandlet, Log, "Empty translation found for context '%ls' in culture '%ls'. This context will be skipped.", *ContextLocalizationKey, *CultureName);
					continue;
				}
...

		{
			...
			if (!bCSVFileSaved)
			{
				UE_LOGF(LogExportDialogueScriptCommandlet, Error, "Failed to write CSV file for culture '%ls' to '%ls'.", *CultureName, *CSVFileName);
				continue;
			}
		}
	}
	...
	auto HasLocalizedSoundWave = [&](const FDialogueContext& InContext) -> bool
	{
		if (InLocalizedDialogueWave)
		{
			for (const FDialogueContextMapping& LocalizedContextMapping : InLocalizedDialogueWave->ContextMappings)
			{
				...
				{
					const UPackage* ObjPackage = LocalizedContextMapping.SoundWave ? LocalizedContextMapping.SoundWave->GetOutermost() : nullptr;
