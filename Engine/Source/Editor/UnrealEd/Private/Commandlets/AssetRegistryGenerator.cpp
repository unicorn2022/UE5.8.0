...
FName GetPackageNameFromDependencyPackageName(const FName RawPackageFName)
{
		...
		(FPackageName::IsScriptPackage(RawPackageFName.ToString()) == false))
	{
		...
		if (!FPackageName::IsValidLongPackageName(RawPackageFName.ToString(), true, &OutReason))
		{
			...
			UE_LOGF(LogAssetRegistryGenerator, Warning, "%ls", *(FailMessage.ToString()));
			return NAME_None;
		}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FAssetRegistryGenerator::UpdateAssetManagerDatabase);
...
bool FAssetRegistryGenerator::GenerateStreamingInstallManifest(int64 InOverrideChunkSize, const TCHAR* InManifestSubDir,
	UE::Cook::FCookSandbox& InSandboxFile, UCookOnTheFlyServer& COTFS)
{
	const FString Platform = TargetPlatform->PlatformName();
	FString ChunkManifestDir = GetChunkManifestDirectory(COTFS);
	if (InManifestSubDir)
	{
	...
	{
		UE_LOGF(LogAssetRegistryGenerator, Error, "Failed to create directory: %ls", *ChunkManifestDir);
		return false;
	}
	...
	{
		UE_LOGF(LogAssetRegistryGenerator, Error, "Failed to open output pakchunklist file %ls",
	...
	if (!TargetPlatform->HasSecurePackageFormat())
	{
		...
		for (const FContentEncryptionConfig::TGroupMap::ElementType& GroupElement : EncryptionGroups)
		{
			...
			{
				UE_LOGF(LogAssetRegistryGenerator, Log, "Updating non-asset files in manifest for group '%ls'", *GroupName.ToString());
	...
	if (bEnableGameOpenOrderSort)
	{
		...
		for (const FPakOrderFileSpec& Spec : OrderFileSpecs)
		{
			...
			UE_LOGF(LogAssetRegistryGenerator, Log, "Order file spec %ls starting at index %llu", *Spec.Pattern, StartIndex);
			...
			for (int32 i=0; i < FoundFiles.Num(); ++i)
			{
				...
				UE_LOGF(LogAssetRegistryGenerator, Display, "Found order file %ls", *Found);
		...
		UE_LOGF(LogAssetRegistryGenerator, Log, "Merging order maps in priority order");
		for (int32 SpecIndex : SpecIndicesByPriority)
		{
			const FPakOrderFileSpec& Spec = OrderFileSpecs[SpecIndex];
			UE_LOGF(LogAssetRegistryGenerator, Log, "Merging order file spec %d (%ls) at priority (%d) ", SpecIndex, *Spec.Pattern, Spec.Priority);
	...
	TArray<int32> ReferencedChunkIds;
	TArray<bool> FinalChunkIsReferenced;
	FinalChunkIsReferenced.SetNum(FinalChunkManifests.Num());
	ReferencedChunkIds.Reserve(FinalChunkManifests.Num());
	// Get the list of ChunkIds to report to ChunkDataGenerators' BeginGenerateChunkDataFiles.
	for (int32 PakchunkIndex = 0; PakchunkIndex < FinalChunkManifests.Num(); ++PakchunkIndex)
	{
		...
		FinalChunkIsReferenced[PakchunkIndex] = false;

		// Do not create any files, and do not tell ChunkDataGenerators that the chunk is referenced, if the chunk is
		// empty and is not referenced by rules applied during staging
		if (Manifest->IsEmpty())
		{
		...
		FinalChunkIsReferenced[PakchunkIndex] = true;
		ReferencedChunkIds.Add(PakchunkIndex);
	}
	...
	{
		IChunkDataGenerator::FBeginGenerateContext BeginContext;
		BeginContext.CurrentChunkIds = ReferencedChunkIds;
		BeginContext.TargetPlatform = TargetPlatform;
		BeginContext.SandboxFile = &SandboxPlatformFile;
		for (const TSharedRef<IChunkDataGenerator>& ChunkDataGenerator : ChunkDataGenerators)
		{
			ChunkDataGenerator->BeginGenerateChunkDataFiles(BeginContext);
		}
	}
	...
	{
		const FChunkPackageSet* Manifest = FinalChunkManifests[PakchunkIndex].Get();
		check(Manifest); // This was created in the loop above if not already existing
		...
		if (!FinalChunkIsReferenced[PakchunkIndex])
		{
			continue;
		}
		...
		for (int32 SubChunkIndex = 0; !bFinishedAllFiles; ++SubChunkIndex)
		{
			...
			if (!PakListFile)
			{
				UE_LOGF(LogAssetRegistryGenerator, Error, "Failed to open output paklist file %ls", *PakListFilename);
				bSucceeded = false;
				break;
			}
			...
			// If this chunk has a separate unique asset registry, add it to first subchunk's manifest here
			if (SubChunkIndex == 0)
			{
			...
			if (SubChunkIndex == 0)
			{
				...
				{		
					...
					for (const FString& ChunkFilename : ChunkFilenames)
					{
						...
						{
							UE_LOGF(LogAssetRegistryGenerator, Display, "Found file open order for %ls, %ll", *RelativeFilename, FileOpenOrder);
						}
						else
						{
							UE_LOGF(LogAssetRegistryGenerator, Display, "Didn't find openorder for %ls", *RelativeFilename, FileOpenOrder);
						}*/
...
bool FAssetRegistryGenerator::CleanManifestDirectories(UE::Cook::FCookSandbox& InSandboxFile, UCookOnTheFlyServer& COTFS)
{
	LLM_SCOPE_BYTAG(Cooker_GeneratedAssetRegistry);
	FString ChunkManifestDir = GetChunkManifestDirectory(COTFS);
	if (IFileManager::Get().DirectoryExists(*ChunkManifestDir))
	{
		...
		{
			UE_LOGF(LogAssetRegistryGenerator, Error, "Failed to delete directory: %ls", *ChunkManifestDir);
			return false;
		}
	}
	...
	if (IFileManager::Get().DirectoryExists(*ChunkListDir))
	{
		...
		{
			UE_LOGF(LogAssetRegistryGenerator, Error, "Failed to delete directory: %ls", *ChunkListDir);
			return false;
		}
	}
	return true;
...
{
	LLM_SCOPE_BYTAG(Cooker_GeneratedAssetRegistry);
	check(InPreviousState);
	this->State = MoveTemp(*InPreviousState);
}
...
void FAssetRegistryGenerator::InjectEncryptionData(FAssetRegistryState& TargetState)
{
	...
	auto ApplyEncryptionTag = 
		[&GuidCache, &AssetManager, &TargetState](FAssetData* AssetData)
		{
			...
			if (ChunkIDs.Num() > 1)
			{
				UE_LOGF(LogAssetRegistryGenerator, Warning, "Encrypted root asset '%ls' exists in two chunks. Only secondary assets should be shared between chunks.", *AssetData->GetObjectPathString());
			}
	...
	for (FContentEncryptionConfig::TGroupMap::ElementType EncryptedAssetSetElement : EncryptionConfig.GetPackageGroupMap())
	{
		...
		for (FName EncryptedRootPackageName : EncryptedRootAssets)
		{
			TargetState.EnumerateMutableAssetsByPackageName(EncryptedRootPackageName,
				[&ApplyEncryptionTag](FAssetData* AssetData)
				{
					ApplyEncryptionTag(AssetData);
	...
	// Since the encryption collections won't list all assets going into an encrypted chunk, repeat the search based on chunk id,
	// and look for any missed encrypted assets. Will not work for fast cook.
	TargetState.EnumerateAllMutableAssets(
		[&ApplyEncryptionTag](FAssetData& AssetData)
		{
			const FAssetData::FChunkArrayView ChunkIDs = AssetData.GetChunkIDs();
			const bool bEncrypted = ChunkIDs.ContainsByPredicate([](const int32 ChunkID) { return ChunkID >= 1000 && ChunkID < 2000; });
			const bool bHasTag = AssetData.TagsAndValues.Contains(UAssetManager::GetEncryptionKeyAssetTagName());

			if (bEncrypted && !bHasTag)
			{
				ApplyEncryptionTag(&AssetData);
			}
...
bool FAssetRegistryGenerator::SaveManifests(UE::Cook::FCookSandbox& InSandboxFile, UCookOnTheFlyServer& COTFS,
	int64 InOverrideChunkSize, const TCHAR* InManifestSubDir)
{
	LLM_SCOPE_BYTAG(Cooker_GeneratedAssetRegistry);
	if (!bGenerateChunks)
	{
	...
	if (!GenerateStreamingInstallManifest(InOverrideChunkSize, InManifestSubDir, InSandboxFile, COTFS))
	{
		return false;
	}
...
TMap<int32, FString> FAssetRegistryGenerator::GetPakChunkIdToStringOverrideMap()
{
	...
	for (const TPair<int32, FString>& PakChunkToName : ChunkIdStringOverride)
	{
		...
		for (TCHAR Char : PakChunkName.GetCharArray())
		{
			...
			{
				UE_LOGF(LogCook, Error, "GetPakChunkIdToStringMapping contains invalid string for chunk mapping. Character '%c' within %ls. Only the following character are allowed:\n%ls", Char, *PakChunkName, VALID_SAVEDDIRSUFFIX_CHARACTERS);
	...
	for (const int32 InvalidOverride : InvalidOverrides)
	{
		...
		UE_LOGF(LogCook, Log, "Removing PakChunk name override for chunk - %d due to invalid character.", InvalidOverride);
	}
...
void FAssetRegistryGenerator::Initialize()
{
	FGameDelegates::Get().GetAssignLayerChunkDelegate() = FAssignLayerChunkDelegate::CreateStatic(AssignLayerChunkDelegate);
}

void FAssetRegistryGenerator::InitializeAssetRegistryForCookOnTheFly(const FAssetRegistrySerializationOptions& DevelopmentSaveOptions)
{
	...
	AssetRegistry.InitializeTemporaryAssetRegistryState(State, DevelopmentSaveOptions);
}
	...
	const FAssetRegistryState& PreviousState, FAssetRegistryDifference& OutDifference)
{
	...
	AssetRegistry.EnumerateAllPackages(
		[this, &Options, &PreviousAssetPackageDataMap, &OutDifference]
		(FName PackageName, const FAssetPackageData& CurrentPackageDataRef)
	{
		const FAssetPackageData* CurrentPackageData = &CurrentPackageDataRef;
	...
	for (const TPair<FName, const FAssetPackageData*>& PackagePair : PreviousAssetPackageDataMap)
	{
		...
		TOptional<FAssetPackageData> CurrentPackageDataCopy = AssetRegistry.GetAssetPackageDataCopy(PackageName);
		const FAssetPackageData* CurrentPackageData = CurrentPackageDataCopy.GetPtrOrNull();
		...
		if (!CurrentPackageData)
		{
			...
			if (!GeneratorName.IsNone())
			{
				...
				TOptional<FAssetPackageData> GeneratorDataCopy = AssetRegistry.GetAssetPackageDataCopy(GeneratorName);
				const FAssetPackageData* GeneratorData = GeneratorDataCopy.GetPtrOrNull();
				if (!GeneratorData)
				{
	...
	if (Options.bRecurseModifications)
	{
		...
		TSet<FName> Visited;
		Visited.Reserve(PreviousState.GetNumPackages());
		...
		while (!VisitStack.IsEmpty())
		{
			...
			// Read referencers from the current global assetregistry. If there are referencers in the old state that are not in the
			// current assetregistry, then they must have changed themselves and so are already in the modified set.
			TArray<FAssetIdentifier> Referencers;
			FAssetIdentifier ModifiedPackageAssetId(ModifiedPackage);
			AssetRegistry.GetReferencers(ModifiedPackageAssetId, Referencers, UE::AssetRegistry::EDependencyCategory::Package,
			...
			for (const FAssetIdentifier& Referencer : Referencers)
			{
				...
				// it in the current assetregistry's dependency tree and so it can not be a Removed difference.
...
void FAssetRegistryGenerator::ReportGeneratorPackages(const FAssetRegistryState& PreviousState,
	TMap<FName, FGeneratorPackageInfo>& OutGeneratorPackages, int32& OutNumNeverCookPlaceHolderPackages)
{
	...
	for (const TPair<FName, const FAssetPackageData*>& PackagePair : PreviousState.GetAssetPackageDataMap())
	{
		...
		if (bGenerated)
		{
			// A generated package is always referenced by its generator, and it might be referenced by its fellow generated
			// packages. Look in its referencers for a non-generated package, and assume that is the generator.
			TArray<FAssetIdentifier> Referencers;
			PreviousState.GetReferencers(FAssetIdentifier(PackageName), Referencers,
				UE::AssetRegistry::EDependencyCategory::Package);
			for (TArray<FAssetIdentifier>::TIterator ReferencerIter(Referencers); ReferencerIter; ++ReferencerIter)
			{
				bool bReferencerIsGenerated = false;
				PreviousState.EnumerateAssetsByPackageName(ReferencerIter->PackageName,
					[&bReferencerIsGenerated](const FAssetData* AssetData)
					{
						bReferencerIsGenerated = (AssetData->PackageFlags & PKG_CookGenerated) != 0;
						return false; // stop iterating
					});
				if (bReferencerIsGenerated)
				{
					ReferencerIter.RemoveCurrent();
				}
			}
			FName GeneratorName = Referencers.Num() == 1 ? Referencers[0].PackageName : NAME_None;
			const FAssetPackageData* GeneratorData = !GeneratorName.IsNone()
				? PreviousState.GetAssetPackageData(GeneratorName) : nullptr;
			if (GeneratorData)
			{
...
void FAssetRegistryGenerator::RemoveStalePackages(
	const TMap<FName, UE::Cook::Artifact::FOplogPackageData>& OplogPackages)
{
	using namespace UE::Cook;

	TStringBuilder<256> PackageNameStr;

	// Delete stale script packages. We know which script packages are stale immediately, no LastReferencedDate timeout
	// required,by looking for them in the current editor binary.
	TSet<FAssetIdentifier> StaleScriptPackages;
	for (const TPair<FName, const FAssetPackageData*>& Pair : State.GetAssetPackageDataMap())
	{
		PackageNameStr.Reset();
		PackageNameStr << Pair.Key;
		if (FPackageName::IsScriptPackage(PackageNameStr))
		{
			if (!FindPackage(nullptr, *PackageNameStr))
			{
				StaleScriptPackages.Add(FAssetIdentifier(Pair.Key));
			}
		}
	}
	if (!StaleScriptPackages.IsEmpty())
	{
		State.RemovePackageDatas(StaleScriptPackages);
		State.RemoveDependsNodes(StaleScriptPackages);
	}

	// Delete stale content packages: if the PackageWriter removed them from the oplog for staleness then we need
	// to remove them from AssetRegistry is well.
	TArray<FAssetData*> RemoveDatas;
	for (const TPair<FName, const FAssetPackageData*>& Pair : State.GetAssetPackageDataMap())
	{
		FName PackageName = Pair.Key;
		const FAssetPackageData* PackageData = Pair.Value;

		// TODO: NeverCookPlaceholders were accidentally stripped from the AssetRegistry by CL 38685901 to support
		// build dependencies in the oplog; that changelist dropped them from the list of IgnorePackageDatas passed
		// into the AssetRegistryGenerator to keep for the DevelopmentAssetRegistry.
		// Culling of data from the DevelopmentAssetRegistry now only occurs here, so if we change this function to
		// keep NeverCookPlaceholders, we will change the behavior back to keeping them in the AssetRegistry. This
		// might no longer be needed. Investigate and enable or delete this block.
		static volatile bool bKeepNeverCookPlaceholders = false;
		if (bKeepNeverCookPlaceholders)
		{
			// Special case: NeverCookPlaceholders are not culled, because they are never put into the Oplog.
			// TODO: Need to add some way to cull stale NeverCookPlaceholders; add NeverCookPlaceholders to
			// the oplog?
			if (PackageData->DiskSize < 0)
			{
				ECookResult CookResult = DiskSizeToCookResult(PackageData->DiskSize);
				if (CookResult == ECookResult::NeverCookPlaceholder)
				{
					continue;
				}
			}
		}
		if (!OplogPackages.Contains(PackageName))
		{
			State.EnumerateMutableAssetsByPackageName(PackageName, [&RemoveDatas](FAssetData* AssetData)
				{
					RemoveDatas.Add(AssetData);
	...
	if (!RemoveDatas.IsEmpty())
	{
		...
		bRemoved.SetNumUninitialized(RemoveDatas.Num());
		bRemovedDependencyData.SetNumUninitialized(RemoveDatas.Num());
		State.RemoveAssetDatas(RemoveDatas, true /* bRemoveDependencyData */, bRemoved, bRemovedDependencyData);
	}
}

void FAssetRegistryGenerator::AddGlobalsToCookedAssetRegistry(UCookOnTheFlyServer& COTFS)
{
	const bool bCookingDLC = COTFS.IsCookingDLC();
	TStringBuilder<256> PackageNameStr;

	// Script packages: add Script packages from global assetregistry.
	// This addition of script packages is possibly no longer used, we include it for now as a legacy behavior.
	// We also take advantage of it in RemoveStalePackages to find all the stale /Script depends nodes.

	// Copy /Script PackageDatas from the current global AssetRegistry
	// DLC cooks skip this behavior to save time.
	if (!bCookingDLC)
	{
		AssetRegistry.EnumerateAllPackages([this, &PackageNameStr](FName PackageName, const FAssetPackageData& PackageData)
			{
				PackageNameStr.Reset();
				PackageNameStr << PackageName;
				if (FPackageName::IsScriptPackage(PackageNameStr))
				{
					*State.CreateOrGetAssetPackageData(PackageName) = PackageData;
				}
			});
	}

	// DependsNodes: 1) Remove dependencies to unrecorded assets. We copied dependencies for recorded assets from the
	// global assetregistry, but the targets of those dependencies could have included assets that were not cooked
	// (in this session or even in previous incremental cooks) because they are editoronly or NeverCook.
	
	// DependsNodes: 2) Manage Dependencies: add in manage dependenciesfrom the global AssetRegistry, and remove
	// any stale PrimaryAsset values from previous cooks. Mange dependencies are used by some editor tools that
	// analyze the cooked assetregistry such as AssetManagerEditorModule.
	TSet<FAssetIdentifier> RemoveNodes;
	for (TPair<FAssetIdentifier, FDependsNode*>& Pair : State.CachedDependsNodes)
	{
		const FAssetIdentifier& Id = Pair.Key;
		if (Id.IsValue())
		{
			// DependsNode for a SearchableName, keep it.
		}
		else if (Id.PrimaryAssetType.IsValid())
		{
			// DependsNode for a PrimaryAsset, remove it (to be added back below).
			RemoveNodes.Add(Pair.Key);
		}
		else if (Id.IsPackage())
		{
			// DependsNode for a Package. Either /Script or content. Keep the dependsnode for either type if and only
			// if we have a PackageAssetData for it.
			if (!State.CachedPackageData.Contains(Id.PackageName))
			{
				RemoveNodes.Add(Id);
			}
		}
		...
		{
			// A DependsNode to an object within a package. Invalid, remove it.
			RemoveNodes.Add(Id);
		}
		else
		{
	...
	{
		State.RemoveDependsNodes(RemoveNodes, FAssetRegistryState::ERemoveDependsFlags::RemoveOrphans);
	}

	// Copy Manage dependencies from the current global AssetRegistry.
	// DLC cooks skip this behavior to save time.
	if (!bCookingDLC)
	{
		AssetRegistry.CopyManageDependenciesToState(State);
	}
}

void FAssetRegistryGenerator::FinalizeChunkIDs(const TSet<FName>& CookedPackages,
	const TSet<FName>& DevelopmentOnlyPackages, UE::Cook::FCookSandbox& InSandboxFile,
	bool bGenerateStreamingInstallManifest, const TSet<FName>& StartupPackages,
	const FAssetRegistrySerializationOptions& DevelopmentSaveOptions)
{
	LLM_SCOPE_BYTAG(Cooker_GeneratedAssetRegistry);
	...
	for (FName DevelopmentOnlyPackage : DevelopmentOnlyPackages)
	{
		...
		if (PackageData && PackageData->DiskSize >= 0)
		{
			...
			UE_LOGF(LogAssetRegistryGenerator, Display,
				"Package %ls is a development-only package but was not marked with DiskSize explaining why it was not cooked. Marking it as a generic skip.",
	...
	State.EnumerateAllMutableAssets([&](FAssetData& AssetData)
	{
		for (int32 ChunkID : AssetData.GetChunkIDs())
		{
			if (ChunkID < 0)
			{
				UE_LOGF(LogAssetRegistryGenerator, Warning, "Out of range ChunkID: %d", ChunkID);
				ChunkID = 0;
			}
...
bool FAssetRegistryGenerator::SaveAssetRegistry(const FString& SandboxPath, bool bSerializeDevelopmentAssetRegistry, 
	bool bForceNoFilter, uint64& OutDevelopmentAssetRegistryHash, const TSet<FName>& CookedPackages,
	const FAssetRegistrySerializationOptions& DevelopmentSaveOptions)
{
	LLM_SCOPE_BYTAG(Cooker_GeneratedAssetRegistry);
	UE_LOGF(LogAssetRegistryGenerator, Display, "Saving asset registry v%d.", FAssetRegistryVersion::Type::LatestVersion);
	...
	// Write runtime registry, filtering further the state that we wrote out mostly unfiltered for the
	// DevelopmentAssetRegistry above.
	if (SaveOptions.bSerializeAssetRegistry)
	{
		...
		if (FinalChunkManifests.Num() > 0)
		{
			...
			for (TMap<int32, TSet<int32>>::ElementType& ChunkBucketElement : ChunkBuckets)
			{
				...
				UE_LOGF(LogAssetRegistryGenerator, Display, "Generated asset registry %lsnum assets %d, size is %5.2fkb", *FilenameForLog, NewState.GetNumAssets(), (float)SerializedAssetRegistry.Num() / 1024.f);
			}
		}
		...
		else
		{
			...
			UE_LOGF(LogAssetRegistryGenerator, Display, "Generated asset registry num assets %d, size is %5.2fkb", NumAssets, (float)SerializedAssetRegistry.Num() / 1024.f);
		}
	}
	...
	const TSet<FName>& CumulativeParentPackages, TArray<TArray<FName>>& OutPackagesMovedBetweenChunks)
{
	...
	for (FName PackageName : CumulativeParentPackages)
	{
		...
		if (NodeManifest.Remove(PackageName) > 0)
		{
			...
			UE_LOGF(LogAssetRegistryGenerator, Verbose, "Removed %ls from chunk %i because it is duplicated in another chunk.",
...
FString FAssetRegistryGenerator::GetChunkManifestDirectory(UCookOnTheFlyServer& COTFS) const
{
	return COTFS.GetCookMetadataOutputFolder(TargetPlatform) / TEXT("ChunkManifest");
}
...
void FAssetRegistryGenerator::FixupPackageDependenciesForChunks(UE::Cook::FCookSandbox& InSandboxFile)
{
	...
	{
		const bool bIsSkipResolveChunkDependencyGraphConfigured = PlatformIniFile.GetBool(TEXT("/Script/CoreUObject.ChunkDependencyInfo"), TEXT("bSkipResolveChunkDependencyGraph"), bSkipResolveChunkDependencyGraph);

		// Handle deprecated INI section since this class has moved from UnrealEd into CoreUObject module
		bool bDeprecatedSkipResolveChunkDependencyGraph = false;
		if (PlatformIniFile.GetBool(TEXT("/Script/UnrealEd.ChunkDependencyInfo"), TEXT("bSkipResolveChunkDependencyGraph"), bDeprecatedSkipResolveChunkDependencyGraph))
		{
			UE_LOGF(LogAssetRegistryGenerator, Warning,
				"Configuration [/Script/UnrealEd.ChunkDependencyInfo]:bSkipResolveChunkDependencyGraph is deprecated since UE 5.8; Use [/Script/CoreUObject.ChunkDependencyInfo]:bSkipResolveChunkDependencyGraph instead!");
			if (bIsSkipResolveChunkDependencyGraphConfigured)
			{
				if (bSkipResolveChunkDependencyGraph != bDeprecatedSkipResolveChunkDependencyGraph)
				{
					UE_LOGF(LogAssetRegistryGenerator, Error,
						"Conflict between deprecated config [/Script/UnrealEd.ChunkDependencyInfo]:bSkipResolveChunkDependencyGraph (%ls) and new config [/Script/CoreUObject.ChunkDependencyInfo]:bSkipResolveChunkDependencyGraph (%ls)",
						bDeprecatedSkipResolveChunkDependencyGraph ? TEXT("true") : TEXT("false"),
						bSkipResolveChunkDependencyGraph ? TEXT("true") : TEXT("false"));
				}
			}
			else
			{
				bSkipResolveChunkDependencyGraph = bDeprecatedSkipResolveChunkDependencyGraph;
			}
		}
	}
	...
	{
		UE_LOGF(LogAssetRegistryGenerator, Log, "Initial scan of chunks found duplicate assets in graph children");
	}
	...
	for (int32 PakchunkIndex = 0, MaxPakchunk = ChunkManifests.Num(); PakchunkIndex < MaxPakchunk; ++PakchunkIndex)
	{
		...
		{
			UE_LOGF(LogAssetRegistryGenerator, Verbose, "Chunk: %i, Started with %i packages, Final after dependency resolve: %i",
...
void FAssetRegistryGenerator::StoreUpdateAssetRegistryData(FName PackageName, UE::Cook::FAssetRegistryPackageMessage&& Message)
{
	...
	// Override the global AssetRegistry's data with the data discovered during save
	...
	const TArray<FAssetData>& AssetDatasFromSave, bool bCookSucceeded)
{
	...
	for (const FSoftObjectPath& AssetToRemove : AssetsToRemove)
	{
		...
		if (bCookSucceeded)
		{
			...
			UE_LOGF(LogAssetRegistryGenerator, Log,
				"Removing Asset %ls from the runtime AssetRegistry; it does not exist in the cooked package.",
	...
	for (const FMPCollectorClientTickPackageContext::FPlatformData& ContextPlatformData : Context.GetPlatformDatas())
	{
		...
		if (!RegistryReporter.PackageUpdateMessages.RemoveAndCopyValue(Context.GetPackageName(), Message))
		{
			...
			if (ContextPlatformData.CookResults == ECookResult::Succeeded)
			{
				...
				UE_CLOGF(!bLoggedWarning, LogAssetRegistryGenerator, Warning,
					"ClientTickPackage was called for package %ls, platform %ls, but UpdateAssetRegistryData was not called for that package. We will not have up to date AssetDataTags in the generated AssetRegistry.",
...
void FAssetRegistryMPCollector::ServerReceiveMessage(FMPCollectorServerMessageContext& Context, FCbObjectView Message)
{
	...
	{
		UE_LOGF(LogCook, Error, "Corrupt AssetRegistryPackageMessage received from CookWorker %d. It will be ignored.",
...
void FAssetRegistryGenerator::InitializeChunkIdPakchunkIndexMapping()
{
	...
	for (int32 ChunkID : AllChunkIDs)
	{
		...
		{
			UE_LOGF(LogAssetRegistryGenerator, Error, "Chunks with encryption key guid or unique assetregistry name (Chunk %d) can not be mapped with ChunkIdPakchunkIndexMapping.  Mapping is removed.", ChunkID);
