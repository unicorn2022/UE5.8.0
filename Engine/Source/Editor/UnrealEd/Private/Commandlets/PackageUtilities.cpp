...
bool NormalizePackageNames( TArray<FString> PackageNames, TArray<FString>& PackagePathNames, const FString& PackageWildcard, uint8 PackageFilter )
{
	...
	if( PackageNames.Num() == 0 )
	{
		...
		if ( GConfig->GetArray( TEXT("Core.System"), TEXT("Paths"), Paths, GEngineIni ) > 0 )
		{
			...
			for ( const FString& Path : Paths)
			{
				...
				{
					UE_LOGF(LogPackageUtilities, Warning,
						"Engine.ini:[Core.System]:Paths entry '%ls' is not mounted. Skipping it.", *Path);
					continue;
				}
				FString SearchWildcard = Path / PackageWildcard;
				UE_LOGF(LogPackageUtilities, Log, "Searching using wildcard: '%ls'", *SearchWildcard);
	...
	{
		UE_LOGF(LogPackageUtilities, Log, "No packages found using '%ls'!", *PackageWildcard);
		return false;
	}
...
bool FContentHelper::ClearAssetSet(FName InSetName, ECollectionShareType::Type InSetType )
{
	...
	{
		UE_LOGF(LogPackageUtilities, Warning, "Collection Helper is not initialized.");
		return false;
	}

	if ( AssetSetPolicy::DestroyAssetSet( *CollectionContainer, InSetName, InSetType ) == false)
	{
		UE_LOGF(LogPackageUtilities, Warning, "Collection Helper failed to destroy collection %ls.", *InSetName.ToString());
		return false;
	}
...
bool FContentHelper::AssignSetContent(FName InSetName, ECollectionShareType::Type InType, const TArray<FSoftObjectPath>& InAssetList )
{
			...
			if (CurrentAssetCount != 0)
			{
				...
				if ((TrueRemoveList.Num() + TrueAddList.Num()) < CurrentAssetCount)
				{
					...
					if (TrueRemoveList.Num() > 0)
					{
						...
						{
							UE_LOGF(LogPackageUtilities, Warning, "Collection Helper failed to remove assets from collection %ls.", *InSetName.ToString());
							bResult = false;
						}
					}
					if (TrueAddList.Num() > 0)
					{
						...
						{
							UE_LOGF(LogPackageUtilities, Warning, "Collection Helper failed to add assets to collection %ls.", *InSetName.ToString());
							bResult = false;
						}
					}
				}
				else
				{
					...
					{
						// this is a problem!!!
						UE_LOGF(LogPackageUtilities, Warning, "Collection Helper failed to clear assets for collection %ls.", *InSetName.ToString());
						bResult = false;
					}
				}
			}
	...
	{
		UE_LOGF(LogPackageUtilities, Warning, "Collection Helper failed to create collection %ls.", *InSetName.ToString());
		bResult = false;
	}
...
bool ULoadPackageCommandlet::ParseLoadListFile(FString& LoadListFilename, TArray<FString>& Tokens)
{
	...
	if (FFileHelper::LoadFileToString(Data, *LoadListFilename) == true)
	{
		...
		{
			//UE_LOGF(LogPackageUtilities, Log, "Read in: %ls", *StrLine);
			Tokens.AddUnique(StrLine);
		}

		// debugging...
		//UE_LOGF(LogPackageUtilities, Log, "\nPACKAGES TO LOAD:");
		for (int32 TokenIdx = 0; TokenIdx < Tokens.Num(); TokenIdx++)
		{
			//UE_LOGF(LogPackageUtilities, Log, "\t%ls", *(Tokens(TokenIdx)));
		}
		return (Tokens.Num() > 0);
	}
...
int32 ULoadPackageCommandlet::Main( const FString& Params )
{
	...
	for (int32 TokenIdx = 0; TokenIdx < Tokens.Num(); TokenIdx++)
	{
		...
		if (FParse::Value(*(Tokens[TokenIdx]), TEXT("LOADLIST="), LoadListFilename))
		{
			...
			//UE_LOGF(LogPackageUtilities, Log, "LoadList in file %ls", *LoadListFilename);
	...
	if ( Tokens.Num() == 0 )
	{
		UE_LOGF(LogPackageUtilities, Warning, "You must specify a package name (multiple files can be delimited by spaces) or wild-card, or specify -all to include all registered packages");
		return 1;
	}
	...
	for ( int32 TokenIndex = 0; TokenIndex < Tokens.Num(); TokenIndex++ )
	{
		...
		{
			UE_LOGF(LogPackageUtilities, Display, "No packages found for parameter %i: '%ls'", TokenIndex, *Tokens[TokenIndex]);
			continue;
		}
	...
	{
		UE_LOGF(LogPackageUtilities, Warning, "No files found.");
		return 1;
	}
	...
	for( int32 FileIndex = 0; FileIndex < FilesInPath.Num(); FileIndex++ )
	{
		...
		UE_LOGF(LogPackageUtilities, Display, "Loading %ls", *Filename );
		...
		else
		{
			...
			if(Package == nullptr)
			{
				UE_LOGF(LogPackageUtilities, Error, "Error loading %ls!", *Filename );
			}
		}
	...
	if (bCheckForLegacyPackages)
	{
		UE_LOGF(LogPackageUtilities, Log, "%d minimum UE version number.", MinVersion );
	}
			...
			for ( int32 PriorityType = 0; PriorityType < EXPORTSORT_MAX; PriorityType++ )
			{
				switch ( SortPriority[PriorityType] )
				{
					case EXPORTSORT_ExportSize:
						Result = (B.Export.SerialSize + B.BulkSize) - (A.Export.SerialSize + A.BulkSize);
						break;
...
/** Returns the size of the package on disk. Will return 0 if the file could not be found */
static uint64 GetPackageSize(const FLinkerLoad* Linker)
{
	if (int64 FileSize = IPackageResourceManager::Get().FileSize(Linker->GetPackagePath()); FileSize >= 0)
	{
...
void FPkgInfoReporter_Log::GeneratePackageReport( FLinkerLoad* InLinker /*=nullptr*/, FOutputDevice& Out /*=*GWarn*/)
{
	...
	Out.Logf(ELogVerbosity::Display, TEXT("\t       Total Size: %lld"), GetPackageSize(Linker));
	...
	if( (InfoFlags&PKGINFO_Exports) != 0 )
	{
		...
		// BulkData sizes can only be tracked for cooked packages
		if ((Linker->Summary.GetPackageFlags() & PKG_Cooked) != 0)
		{
			for (const FObjectDataResource& Resource : Linker->DataResourceMap)
			{
				// Inline data is already included in the export size so we only want to record non-inline payloads
				if (FBulkData::HasFlags((EBulkDataFlags)Resource.LegacyBulkDataFlags, BULKDATA_Force_NOT_InlinePayload))
				{
					SortedExportMap[Resource.OuterIndex.ToExport()].BulkSize += Resource.SerialSize;
				}
			}
		}
		...
		if ( (InfoFlags&PKGINFO_Compact) == 0 )
		{
			for( const auto& ExportInfo : SortedExportMap )
			{
				...
				// BulkDataSize can only be tracked for cooked packages, there is no point reporting this breakdown for non-cooked packages
				if ((InLinker->Summary.GetPackageFlags() & PKG_Cooked) != 0)
				{
					Out.Logf(ELogVerbosity::Display, TEXT("\t\t   Export Size: %" INT64_FMT), Export.SerialSize);
					Out.Logf(ELogVerbosity::Display, TEXT("\t\t BulkData Size: %" INT64_FMT), ExportInfo.BulkSize);
				}
				Out.Logf(ELogVerbosity::Display, TEXT("\t\t    Total Size: %" INT64_FMT), Export.SerialSize + ExportInfo.BulkSize );
				if ( !IsHideOffsets())
				{
		...
		else
		{
			...
			{
				const FObjectExport& Export = ExportInfo.Export;
				Out.Logf(ELogVerbosity::Display, TEXT("  %8i %10lld %10lld %10lld %32s %s"), ExportInfo.ExportIndex, Export.SerialSize, ExportInfo.BulkSize, Export.SerialSize + ExportInfo.BulkSize,
	...
	if( (InfoFlags&PKGINFO_Text) != 0 )
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogPackageUtilities, Warning,"Failed to load gatherable text data for package %ls!", *LinkerName.ToString());
			}
		}
	}
	...
	if( (InfoFlags&PKGINFO_Thumbs) != 0 )
	{
		...
		if ( Linker->SerializeThumbnails(true) )
		{
			...
			{
				UE_LOGF(LogPackageUtilities, Warning,"%ls has no thumbnail map!", *LinkerName.ToString());
			}
		}
		else
		{
			...
			{
				UE_LOGF(LogPackageUtilities, Warning,"Failed to load thumbnails for package %ls!", *LinkerName.ToString());
			}
		}
	}
...
int32 UPkgInfoCommandlet::Main( const FString& Params )
{
	...
	else
	{
		for ( int32 TokenIndex = 0; TokenIndex < Tokens.Num(); TokenIndex++ )
		{
			...
			if ( PerTokenFilesInPath.Num() == 0 )
			{
				UE_LOGF(LogPackageUtilities, Warning, "No packages found using '%ls'!", *PackageWildcard);
				continue;
			}
	...
	if (FParse::Value(*Params, TEXT("dumptopath="), OutputPath))
	{
		...
		{
			UE_LOGF(LogPackageUtilities, Warning, "-dumptopath is not supported with -dumptofile, ignoring -dumptopath.");
			OutputPath.Empty();
		}
		else if (RelPathSibling.IsEmpty())
		{
			UE_LOGF(LogPackageUtilities, Warning, "-dumptopath is only supported with -AllPackagesIn, ignoring -dumptopath.");
			OutputPath.Empty();
		}
		else
		{
	...
	for( int32 FileIndex = 0; FileIndex < FilesInPath.Num(); FileIndex++ )
	{
		...
		if (!OutputPath.IsEmpty())
		{
			...
			{
				UE_LOGF(LogPackageUtilities, Error, "Package filename '%ls' is not a child path of root content path '%ls', unable to create Outputfile, skipping the file.",
		...
		else
		{
			...
			if (Reader)
			{
				...
				{
					UE_LOGF(LogPackageUtilities, Error, "Unable to fully load package %ls", *Filename);
					bDumpProperties = false;
				}
			}
			else
			{
				UE_LOGF(LogPackageUtilities, Error, "Unable to create archive for package %ls", *Filename);
				bDumpProperties = false;
			}
		}
	...
	void DoIt( UCommandlet* Commandlet, UPackage* Package, TArray<FString>& Tokens, TArray<FString>& Switches )
	{
		...
		if( SourceControlState.IsValid() && SourceControlState->CanCheckout() )
		{
			...
			if (bAutoCheckOut == true)
			{
				...
				{
					UE_LOGF(LogPackageUtilities, Warning, "Package (%ls) checked out by other, skipping.", *PackageFileName);
				...
				{
					UE_LOGF(LogPackageUtilities, Warning, "Package (%ls) is not at head revision, skipping.", *PackageFileName );
					PackagesThatCouldNotBeSavedList.Add( PackageFileName );
					return;
				}
				...
				{
					UE_LOGF(LogPackageUtilities, Warning, "Package (%ls) is marked for delete, skipping.", *PackageFileName );
					PackagesThatCouldNotBeSavedList.Add( PackageFileName );
					return;
				}
			}
			...
			{
				UE_LOGF(LogPackageUtilities, Warning, "Package (%ls) cannot be checked out. Switch AUTOCHECKOUTPACKAGES not set. Skip.", *PackageFileName);
		...
		{
			UE_LOGF(LogPackageUtilities, Warning, "Package (%ls) name contains 'cine' and switch SKIPCINES is set. Skip.", *PackageFileName);
			PackagesThatCouldNotBeSavedList.AddUnique( PackageFileName );
			return;
		}
		...
		for (TObjectIterator<OBJECTYPE> It; It; ++It)
		{
			...
			{
				UE_LOGF(LogPackageUtilities, Warning, "Same CompressCommandletVersion (%i) skip animation: %ls (%ls)", CompressCommandletVersion, *AnimSeq->GetName(), *AnimSeq->GetFullName());
				continue;
			}

			if( !bForceCompression && bSkipLongAnimations && (AnimSeq->GetNumberOfSampledKeys() > 300) )
			{
				UE_LOGF(LogPackageUtilities, Warning, "Animation (%ls) has more than 300 keys (%i keys) and SKIPLONGANIMS switch is set. Skipping.", *AnimSeq->GetName(), AnimSeq->GetNumberOfSampledKeys());
				continue;
			}
			...
			if (Skeleton == nullptr)
			{
				UE_LOGF(LogPackageUtilities, Warning, "Animation (%ls) is missing its skeleton. Skipping.", *AnimSeq->GetName());
				continue;
			}
			...
			if( DefaultMatchRatio < 1.f )
			{
				...
				if( BestSkeletalMeshMatch )
				{
					...
					{
						UE_LOGF(LogPackageUtilities, Warning, "Found more suitable preview mesh for %ls (%ls): %ls (%f) instead of %ls (%f).", 
				...
				{
					UE_LOGF(LogPackageUtilities, Warning, "Could not find suitable mesh for %ls (%ls) !!! Default was %ls", 
			...
			UE_LOGF(LogPackageUtilities, Warning, "Compressing animation '%ls' [#%d / %d in package '%ls']",
				*AnimSeq->GetName(),
				ActiveAnimationIndex,
				NumAnimationsInPackage,
				*PackageFileName);

			UE_LOGF(LogPackageUtilities, Warning, "%ls (%ls) Resetting with to default compression settings.", *AnimSeq->GetName(), *AnimSeq->GetFullName());
			...
			if( bDirtyPackage )
			{
				...
				UE_LOGF(LogPackageUtilities, Warning, "Time since last save: %f seconds", (CurrentTime - LastSaveTime) );
				if( (CurrentTime - LastSaveTime) > 10.f * 60.f )
				{
					UE_LOGF(LogPackageUtilities, Warning, "It's been over 10 minutes (%f seconds), try to save package.", (CurrentTime - LastSaveTime) );
					...
					if( !SourceControlState.IsValid() || SourceControlState->CanEdit() )
					{
						if( SavePackageHelper( Package, PackageFileName ) == true )
						{
							...
							UE_LOGF(LogPackageUtilities, Display, "Correctly saved:  [%ls].", *PackageFileName );
						}
						else
						{
							UE_LOGF(LogPackageUtilities, Error, "Error saving [%ls]", *PackageFileName );
						}
					}
					...
					{
						PackagesThatCouldNotBeSavedList.AddUnique( PackageFileName );
						UE_LOGF(LogPackageUtilities, Warning, "%ls couldn't be saved, so abort this package, don't waste time on it.", *PackageFileName );
						// Abort!
						return;
					}
...
/*		bDirtyPackage = bDirtyPackage || Package->IsDirty();*/
		...
		if( bDirtyPackage )
		{
			...
			if( !SourceControlState.IsValid() || SourceControlState->CanEdit() )
			{
				...
				{
					UE_LOGF(LogPackageUtilities, Warning, "Error saving [%ls]", *PackageFileName );
				}
			}
...
int32 UCompressAnimationsCommandlet::Main( const FString& Params )
{
	...
	if (bAnalyze)
	{
		UE_LOGF(LogPackageUtilities, Display, "Analyzing content for uncompressed animations...");
		DoActionToAllPackages<UAnimSequence, CompressAnimationsFunctor>(this, ParamsUpperCase);

		UE_LOGF(LogPackageUtilities, Display, "Done analyzing. Potential canditates: %i", AnalyzeCompressionCandidates);
	}
	else
	{
		// Then do the animation recompression
		UE_LOGF(LogPackageUtilities, Display, "Recompressing all animations...");
		DoActionToAllPackages<UAnimSequence, CompressAnimationsFunctor>(this, ParamsUpperCase);
		...
		if (NumPackagesThatCouldNotBeSaved > 0)
		{
			...
			{
				UE_LOGF(LogPackageUtilities, Warning, "\t%ls", *PackagesThatCouldNotBeSavedList[i]);
			}
		}
	}
...
// 	{
// 		UE_LOGF(LogPackageUtilities, Warning, "Syntax: replaceactor <file/wildcard> <Package.Class to remove> <Package.Class to replace with>");
// 		return 1;
// 	}
...
// 	if (FilesInPath.Num() == 0)
// 	{
// 		UE_LOGF(LogPackageUtilities, Error, "No packages found matching %ls!", *PackageWildcard);
// 		return 2;
// 	}
...
//	}
	if( !PackageList.Num() )
	{
		UE_LOGF(LogPackageUtilities, Warning, "Found no packages to run UReplaceActorCommandlet on!" );
		return 0;
	}
	...
	{
		UE_LOGF(LogPackageUtilities, Error, "Invalid class to remove: %ls", *ClassName);
		return 4;
	}
	else
	{
	...
	{
		UE_LOGF(LogPackageUtilities, Error, "Invalid class to replace with: %ls", *ClassName);
		return 5;
	}
	else
	{
	...
	for (int32 i = 0; i < PackageList.Num(); i++)
	{
		...
		// skip if read-only
  		...
  		{
  			UE_LOGF(LogPackageUtilities, Warning, "Skipping %ls: the file can be checked out, but auto check out is disabled", *FileName);
  			continue;
 		}
 		else if(bIsAutoSave)
		{
			UE_LOGF(LogPackageUtilities, Warning, "Skipping %ls (non map)", *FileName);
			continue;
		}
		else if ( bAutoCheckOut && SourceControlState.IsValid() && !SourceControlState->IsCurrent() )
		{
			UE_LOGF(LogPackageUtilities, Warning, "Skipping %ls (Newer version exists in revision control)", *PackageName );
			continue;
		}
		else
		{
			...
			if( World == NULL )
			{
				UE_LOGF(LogPackageUtilities, Display, "%ls (not a map)", *FileName);
				for( FThreadSafeObjectIterator It; It; ++It )
				{
						...
						)
					{
						...
						{
							UE_LOGF(LogPackageUtilities, Display, "Replaced %lli class references in an Object: %ls", ReplaceAr.GetCount(), *OldObject->GetName() );
				...
				if( Package->IsDirty() == true )
				{
					...
					UE_LOGF(LogPackageUtilities, Display, "Saving %ls...", *FileName);
					FSavePackageArgs SaveArgs;
			...
			else
			{
				...
				for (TActorIterator<AActor> It(World, ClassToReplace); It; ++It)
				{
					...
					if (OldActor->GetClass() == ClassToReplace)
					{
						...
						UE_LOGF(LogPackageUtilities, Display, "Replacing actor %ls", *OldActor->GetName());
						bIsDirty = true;
						...
						if (ReplaceAr.GetCount() > 0)
						{
							UE_LOGF(LogPackageUtilities, Display, "Replaced %lli actor references in %ls", ReplaceAr.GetCount(), *It->GetName());
							Package->MarkPackageDirty();
						}
					}
					else
					{
						...
						if (ReplaceAr.GetCount() > 0)
						{
							UE_LOGF(LogPackageUtilities, Display, "Replaced %lli class references in actor %ls", ReplaceAr.GetCount(), *It->GetName());
							Package->MarkPackageDirty();
							bIsDirty = true;
						}
					}
		...
		UE_LOGF(LogPackageUtilities, Display, "GCing...");
		CollectGarbage(RF_NoFlags);
	}
