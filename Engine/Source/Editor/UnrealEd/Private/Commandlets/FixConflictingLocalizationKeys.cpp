...
int32 UFixConflictingLocalizationKeysCommandlet::Main(const FString& Params)
{
	...
	if (bEnableSourceControl)
	{
		...
		{
			UE_LOGF(LogFixConflictingLocalizationKeys, Error, "Revision Control error: %ls", *SCCErrorStr.ToString());
			return -1;
		}
	}
	...
	// We need the manifest to work with
	{
		...
		{
			UE_LOGF(LogFixConflictingLocalizationKeys, Error, "Failed to load manifest: %ls", *LoadManifestError.ToString());
			return -1;
		}
	}
	...
	UE_LOGF(LogFixConflictingLocalizationKeys, Display, "Found %d conflicting text sources...", ConflictingSources.Num());
	...
	for (const FString& ConflictingSource : ConflictingSources)
	{
		...
		{
			UE_LOGF(LogFixConflictingLocalizationKeys, Warning, "Skipping '%ls' as it doesn't look like a valid package path", *ConflictingSource);
			continue;
		}
		...
		{
			UE_LOGF(LogFixConflictingLocalizationKeys, Warning, "Skipping '%ls' as it isn't a valid package name", *ConflictingSourceParts[0]);
			continue;
		}
	...
	UE_LOGF(LogFixConflictingLocalizationKeys, Display, "Found %d packages to update...", PackageNameToConflictingSources.Num());
	...
	for (const auto& PackageNameToConflictingSourcesPair : PackageNameToConflictingSources)
	{
		...
		UE_LOGF(LogFixConflictingLocalizationKeys, Display, "Loading package: %ls", *PackageName);

		// Load the package
		UPackage* Package = LoadPackage(nullptr, *PackageName, LOAD_NoWarn | LOAD_Quiet);
		if (!Package)
		{
			UE_LOGF(LogFixConflictingLocalizationKeys, Error, "Failed to load package from: %ls", *PackageName);
			continue;
		}
		...
		for (const FString& ConflictingSource : PackageNameToConflictingSourcesPair.Value)
		{
			...
			{
				UE_LOGF(LogFixConflictingLocalizationKeys, Display, "    Automatically updated the text for: %ls", *ConflictingSource);
			}
			else
			{
				UE_LOGF(LogFixConflictingLocalizationKeys, Error, "    Failed to automatically update the text for: %ls", *ConflictingSource);
			}
		}
