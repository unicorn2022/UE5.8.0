...
bool UInternationalizationConditioningCommandlet::ProcessManifest( const FString& PrimaryLangExt, const FString& SourcePath, const FString& DestinationPath )
{
	...
	{
		UE_LOGF(LogInternationalizationConditioningCommandlet, Warning, "No primary language(%ls) loc files found!", *PrimaryLangExt);
		return false;
	}
	...
	for( int PropIndex = 0; PropIndex < IdenticalProperties.Num(); PropIndex++ )
	{
		...
		if(!bAddSuccessful)
		{
			UE_LOGF(LogInternationalizationConditioningCommandlet, Warning, "Could not add manifest entry %ls.", *PropContext.SourceLocation );
		}
	}
	...
	{
		UE_LOGF(LogInternationalizationConditioningCommandlet, Error, "Could not save file %ls", *DestinationManifestFileName);
		return false;
	}
...
bool UInternationalizationConditioningCommandlet::ProcessArchive( const FString& PrimaryLangExt, const FString& SourcePath, const FString& DestinationPath )
{
	...
	for( int32 LanguageIndex = 0; LanguageIndex < LanguagesToProcess.Num(); LanguageIndex++ )
	{
		...
		{
			UE_LOGF(LogInternationalizationConditioningCommandlet, Warning, "No foreign loc files found using language extension '%ls'", *ForeignLangExt);
			continue;
		}
		...
		for( int PropIndex = 0; PropIndex < ArchiveProperties.Num(); PropIndex++ )
		{
			...
			if( !InternationalizationArchive->AddEntry( NewNamespace, NewKey, Source, Translation, NULL, false ) )
			{
				...
				if( !ExistingConflictEntry.IsValid() )
				{
					...
					UE_LOGF(LogInternationalizationConditioningCommandlet, Warning, "Failed to add entry to archive Namespace [%ls]: (DEFAULT TEXT): %ls (EXISTING TRANSLATION): %ls", 
				...
				{
					UE_LOGF(LogInternationalizationConditioningCommandlet, Warning, "Conflicting translation ignored in Namespace [%ls]: (DEFAULT TEXT): %ls (EXISTING TRANSLATION): %ls  (REJECTED TRANSLATION): %ls", 
		...
		if (!bDidWriteArchive)
		{
			UE_LOGF(LogInternationalizationConditioningCommandlet, Error, "Could not save file %ls", *DestinationArchiveFileName);
			return false;
		}
...
int32 UInternationalizationConditioningCommandlet::Main( const FString& Params )
{
	...
	{
		UE_LOGF(LogInternationalizationConditioningCommandlet, Error, "No config specified.");
		return -1;
	}
	...
	{
		UE_LOGF(LogInternationalizationConditioningCommandlet, Error, "No config section specified.");
		return -1;
	}
	...
	if(  bGenerateManifestFromLocIni )
	{
		...
		{
			UE_LOGF(LogInternationalizationConditioningCommandlet, Error, "Failed to generate manifest file from ini files.");
			return -1;
		}
	}
