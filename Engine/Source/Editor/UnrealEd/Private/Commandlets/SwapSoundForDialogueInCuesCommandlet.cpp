...
int32 USwapSoundForDialogueInCuesCommandlet::Main(const FString& Params)
{
	...
	if (bEnableSourceControl)
	{
		...
		{
			UE_LOGF(LogSwapSoundForDialogueInCuesCommandlet, Error, "Revision Control error: %ls", *SCCErrorStr.ToString());
			return -1;
		}
	}
	...
	{
		UE_LOGF(LogSwapSoundForDialogueInCuesCommandlet, Error, "Unable to get dialogue wave asset data from asset registry.");
		return -1;
	}
	...
	for (const FAssetData& AssetData : AssetDataArrayForDialogueWaves)
	{
		...
		{
			UE_LOGF(LogSwapSoundForDialogueInCuesCommandlet, Error, "Asset registry found asset (%ls), but the asset with this name is not actually a dialogue wave.", *AssetData.AssetName.ToString());
			continue;
		}
		...
		if (!DialogueWave)
		{
			UE_LOGF(LogSwapSoundForDialogueInCuesCommandlet, Error, "Asset registry found asset (%ls), but the dialogue wave could not be accessed.", *AssetData.AssetName.ToString());
			continue;
		}
		...
		for (const FDialogueContextMapping& ContextMapping : DialogueWave->ContextMappings)
		{
			...
			if (!SoundWavePackage)
			{
				UE_LOGF(LogSwapSoundForDialogueInCuesCommandlet, Error, "Asset registry found dialogue wave (%ls) with a context referencing sound wave (%ls) but no package exists for this sound wave.", *AssetData.AssetName.ToString(), *SoundWave->GetName());
				continue;
			}
			...
			{
				UE_LOGF(LogSwapSoundForDialogueInCuesCommandlet, Error, "Asset registry found dialogue wave (%ls) with a context referencing sound wave (%ls) but failed to search for referencers of the sound wave.", *AssetData.AssetName.ToString(), *SoundWave->GetName());
				continue;
			}
			...
			{
				UE_LOGF(LogSwapSoundForDialogueInCuesCommandlet, Error, "Asset registry found dialogue wave (%ls) with a context referencing sound wave (%ls) but failed to search for sound cues referencing the sound wave.", *AssetData.AssetName.ToString(), *SoundWave->GetName());
				continue;
			}
			...
			for (const FAssetData& SoundCueAssetData : ReferencingSoundCueAssetDataArray)
			{
				...
				if (!SoundCue)
				{
					UE_LOGF(LogSwapSoundForDialogueInCuesCommandlet, Error, "Asset registry found dialogue wave (%ls) with a context referencing sound wave (%ls) but failed to access the referencing sound cue (%ls).", *AssetData.AssetName.ToString(), *SoundWave->GetName(), *SoundCueAssetData.AssetName.ToString());
					continue;
				}
