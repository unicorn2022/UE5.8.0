...
void UAssetRegUtilCommandlet::ReorderOrderFile(const FString& OrderFilePath, const FString& ReorderFileOutPath)
{
	...
	{
		UE_LOGF(LogAssetRegUtil, Warning, "Could not load specified order file.");
		return;
	}

	UE_LOGF(LogAssetRegUtil, Display, "Generating new file order via Asset Registry.");
	...
	UE_LOGF(LogAssetRegUtil, Display, "Writing output: %ls", *ReorderFileOutPath);
	...
	{
		UE_LOGF(LogAssetRegUtil, Warning, "Could not open specified output file.");
	}
}
...
bool UAssetRegUtilCommandlet::MergeOrderFiles(TMap<FString, int64>& NewOrderMap, TMap<FString, int64>& PrevOrderMap)
{
	...
	UE_LOGF(LogAssetRegUtil, Display, "Merge File Open Order intital count: %lld.", PrevEntryCount);
	...
	UE_LOGF(LogAssetRegUtil, Display, "Merge File Open Order final count: %lld.", PrevEntryCount);
...
{
	UE_LOGF(LogAssetRegUtil, Display, "Parsing order package: %ls", *OrderFilePath);
	FString Text;
	if (!FFileHelper::LoadFileToString(Text, *OrderFilePath))
	{
		UE_LOGF(LogAssetRegUtil, Error, "Could not open file %ls", *OrderFilePath);
		return false;
	}
...
{
	UE_LOGF(LogAssetRegUtil, Display, "Parsing order file: %ls", *OrderFilePath);
	FString Text;
	if (!FFileHelper::LoadFileToString(Text, *OrderFilePath))
	{
		UE_LOGF(LogAssetRegUtil, Error, "Could not open file %ls", *OrderFilePath);
	...
	for (int32 EntryIndex = 0; EntryIndex < Lines.Num(); ++EntryIndex)
	{
		...
		if (Lines[EntryIndex].FindLastChar('"', QuoteIndex))
		{
			...
			if (ReadNum.IsNumeric())
			{
				...
				{
					UE_LOGF(LogAssetRegUtil, Warning, "Unexpected order: %i vs %i", ExplicitOrder, EntryIndex + 1);
				}
			}
		}
...
int32 UAssetRegUtilCommandlet::Main(const FString& CmdLineParams)
{
	...
	UE_LOGF(LogAssetRegUtil, Display, "AssetRegUtil cmdLineParams: %ls",*CmdLineParams);
	...
	if (bMergeFileOpenOrder)
	{
		...
		{
			UE_LOGF(LogAssetRegUtil, Warning, "Could not load ReorderFile: %ls", *ReorderFile);
			return 0;
		}

		if (!FParse::Value(*CmdLineParams, TEXT("ReorderOutput="), ReorderOutput, false))
		{
			UE_LOGF(LogAssetRegUtil, Warning, "Could not load ReorderOutput: %ls", *ReorderOutput);
			return 0;
		}
		...
		{
			UE_LOGF(LogAssetRegUtil, Warning, "Could not load PrevReorderFile: %ls", *PrevReorderFile);
			return 0;
		}
	...
	else
	{
		if (FParse::Value(*CmdLineParams, TEXT("ReorderFile="), ReorderFile, false))
		{
			...
			UE_LOGF(LogAssetRegUtil, Display, "Populating the Asset Registry.");
			...
			{
				UE_LOGF(LogAssetRegUtil, Display, "Generating partially-updated order file.");
...
int32 UAssetRegistryDumpCommandlet::Main(const FString& CmdLineParams)
{
	...
	{
		UE_LOGF(LogAssetRegUtil, Warning, "Usage: -input <AssetRegistry.bin Filepath> -outdir <Directory to contain the dumped file(s)");
		return 1;
	}
	...
	{
		UE_LOGF(LogAssetRegUtil, Warning, "Could not open input file %ls", *InputFileName);
		return 3;
	}
	...
	{
		UE_LOGF(LogAssetRegUtil, Warning, "Failed to load file %ls", *InputFileName);
		return 3;
	}
	...
	if (!State.Load(Bytes))
	{
		UE_LOGF(LogAssetRegUtil, Warning, "Failed to serialize file %ls as an AssetRegistry", *InputFileName);
		return 3;
	}
	...
	if (FileManager.DirectoryExists(*DumpDir))
	{
		...
		{
			UE_LOGF(LogAssetRegUtil, Warning, "Failed to move old directory %ls to delete staging directory %ls", *DumpDir, *DeleteDirectory);
			return 4;
		}
		if (!FileManager.DeleteDirectory(*DeleteDirectory, false /* RequireExists */, true /* Tree */))
		{
			UE_LOGF(LogAssetRegUtil, Warning, "Failed to delete temporary delete-staging directory %ls", *DeleteDirectory);
		}
	}
	if (!FileManager.MakeDirectory(*DumpDir, true /* Tree */))
	{
		UE_LOGF(LogAssetRegUtil, Warning, "Failed to create directory %ls", *DumpDir);
		return 4;
	}
	...
	for (FString& PageText : Pages)
	{
		...
		{
			UE_LOGF(LogAssetRegUtil, Warning, "Failed to save file %ls", *FileName);
			Result = 4;
		}
	}
