...
int32 UDumpMaterialExpressionInfoCommandlet::Main(const FString& Params)
{
	...
	auto PrintHelp = []()
	{
		UE_LOGF(LogDumpMaterialExpressionInfo, Display, "DumpMaterialExpressionInfo");
		UE_LOGF(LogDumpMaterialExpressionInfo, Display, "Find all instances of material expressions and dump their data.");
		UE_LOGF(LogDumpMaterialExpressionInfo, Display, "For example, to dump a listing of all custom hlsl nodes along with their inputs and snippets:");
		UE_LOGF(LogDumpMaterialExpressionInfo, Display, "<YourProject> -dx12 -run=DumpMaterialExpressionInfo -unattended -expression=MaterialExpressionCustom -columns=Inputs,Code -csv=C:/output.csv");
		UE_LOGF(LogDumpMaterialExpressionInfo, Display, "");
		UE_LOGF(LogDumpMaterialExpressionInfo, Display, "Options:");
		UE_LOGF(LogDumpMaterialExpressionInfo, Display, " -help                 Print this message.");
		UE_LOGF(LogDumpMaterialExpressionInfo, Display, " -csv=filename         Write the output to a CSV file at this path.");
		UE_LOGF(LogDumpMaterialExpressionInfo, Display, " -collections=name     Optional. Comma-separated list of asset collections that should be searched.");
		UE_LOGF(LogDumpMaterialExpressionInfo, Display, " -tag:TagName=TagValue Optional. Only dump assets with a matching tag. Can have multiple of these.");
		UE_LOGF(LogDumpMaterialExpressionInfo, Display, " -material=name        Optional. Only dump materials or material functions matching this name or regular expression.");
		UE_LOGF(LogDumpMaterialExpressionInfo, Display, " -expression=name      Optional. Only dump expressions matching this name or regular expression.");
		UE_LOGF(LogDumpMaterialExpressionInfo, Display, " -columns=a,b          Optional. Comma-separated list of the properties that should be included in the output. Dumps all by default.");
	};
	...
	if (!CsvPath)
	{
		UE_LOGF(LogDumpMaterialExpressionInfo, Error, "No output CSV file path was specified. \n");
		PrintHelp();
		return 1;
	}
	...
	// Search assets
	UE_LOGF(LogDumpMaterialExpressionInfo, Display, "Searching for materials...");
	...
	UE_LOGF(LogDumpMaterialExpressionInfo, Display, "Found %d materials and material functions", Assets.Num());

	// Open CSV
	FArchive* CsvFileWriter = IFileManager::Get().CreateFileWriter(**CsvPath);
	if (!CsvFileWriter)
	{
		UE_LOGF(LogDumpMaterialExpressionInfo, Error, "Failed to open output file %ls", **CsvPath);
		return 1;
	}
	...
	for (TArray<FAssetData>::SizeType Index = 0; Index < Assets.Num(); Index++)
	{
		...
		{
			UE_LOGF(LogDumpMaterialExpressionInfo, Display, "%llu/%lld done (%f%%)", (uint64)NumAssetsDone, (uint64)Assets.Num(), (float)NumAssetsDone * 100.0f / Assets.Num());
		}
	}
