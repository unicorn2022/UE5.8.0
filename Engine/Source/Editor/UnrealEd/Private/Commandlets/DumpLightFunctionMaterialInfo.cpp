...
int32 UDumpLightFunctionMaterialInfoCommandlet::Main(const FString& Params)
{
	...
	{
		UE_LOGF(LogDumpLightFunctionMaterialInfo, Log, "DumpLightFunctionMaterialInfo");
		UE_LOGF(LogDumpLightFunctionMaterialInfo, Log, "This commandlet will dump to information about light function materials.");
		UE_LOGF(LogDumpLightFunctionMaterialInfo, Log, "A typical way to invoke it is: <YourProject> -run=DumpLightFunctionMaterialInfo -targetplatform=Windows -unattended -sm6 -allowcommandletrendering -nomaterialshaderddc.");
		return 0;
	}



	UE_LOGF(LogDumpLightFunctionMaterialInfo, Log, "Searching for materials within the project...");
	...
	UE_LOGF(LogDumpLightFunctionMaterialInfo, Log, "Found %d materials", MaterialAssets.Num());
	...
	if (Platforms.Num() > 0)
	{
		...
		UE_LOGF(LogDumpLightFunctionMaterialInfo, Display, "Compiling shaders for %ls...", *Platform->PlatformName());
		for (const FAssetData& AssetData : MaterialAssets)
		{
			if (UMaterialInterface* MaterialInterface = Cast<UMaterialInterface>(AssetData.GetAsset()))
			{
				...
				{
					UE_LOGF(LogDumpLightFunctionMaterialInfo, Display, "BeginCache for %ls", *MaterialInterface->GetFullName());
		...
		UE_LOGF(LogDumpLightFunctionMaterialInfo, Log, "Found %d light function materials to compile.", MaterialsToCompile.Num());
		...
		// Submit all the jobs.
		{
			...
			UE_LOGF(LogDumpLightFunctionMaterialInfo, Display, "Submit Jobs");
			...
			while (MaterialsToCompile.Num())
			{
				for (auto It = MaterialsToCompile.CreateIterator(); It; ++It)
				{
					...
					{
						It.RemoveCurrent();
						UE_LOGF(LogDumpLightFunctionMaterialInfo, Display, "Finished cache for %ls.", *MaterialInterface->GetFullName());
						UE_LOGF(LogDumpLightFunctionMaterialInfo, Display, "Materials remaining: %d", MaterialsToCompile.Num());
					}
					...
					while (true)
					{
						...
						{
							UE_LOGF(LogDumpLightFunctionMaterialInfo, Display, "Outstanding Jobs: %d", CurrentOutstandingJobs);
		...
		// Process the shader maps and save to the DDC.
		{
			...
			UE_LOGF(LogDumpLightFunctionMaterialInfo, Log, "ProcessAsyncResults");
		...
		// Perform cleanup and clear cached data for cooking.
		{
			...
			UE_LOGF(LogDumpLightFunctionMaterialInfo, Display, "Clear Cached Cooked Platform Data");
	...
	UE_LOGF(LogDumpLightFunctionMaterialInfo, Log, "**********************************");
	UE_LOGF(LogDumpLightFunctionMaterialInfo, Log, "* Material compatible with atlas *");
	UE_LOGF(LogDumpLightFunctionMaterialInfo, Log, "**********************************");
	for (UMaterialInterface* MaterialInterface : LightFunctionMaterialsCompatible)
	{
		UE_LOGF(LogDumpLightFunctionMaterialInfo, Log, "  - %ls", *MaterialInterface->GetPathName());
	}
	UE_LOGF(LogDumpLightFunctionMaterialInfo, Log, "**************************************");
	UE_LOGF(LogDumpLightFunctionMaterialInfo, Log, "* Material not compatible with atlas *");
	UE_LOGF(LogDumpLightFunctionMaterialInfo, Log, "**************************************");
	...
	UE_LOGF(LogDumpLightFunctionMaterialInfo, Log, "Material compatible with atlas:.....%d.", LightFunctionMaterialsCompatible.Num());
	UE_LOGF(LogDumpLightFunctionMaterialInfo, Log, "Material not compatible with atlas:.%d.", LightFunctionMaterialsNotCompatible.Num());
	return 0;
}
