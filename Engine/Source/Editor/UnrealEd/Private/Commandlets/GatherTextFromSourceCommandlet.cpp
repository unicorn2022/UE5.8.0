...
const FString UGatherTextFromSourceCommandlet::MacroString_NOTELOCTEXT(TEXT("NOTELOCTEXT"));
const FString UGatherTextFromSourceCommandlet::MacroString_NSNOTELOCTEXT(TEXT("NSNOTELOCTEXT"));
...
struct FParsedNestedMacroStats
{
	...
	std::atomic<int32> Nested_NOTELOCTEXT = 0;
	std::atomic<int32> Nested_NSNOTELOCTEXT = 0;
...
int32 UGatherTextFromSourceCommandlet::Main( const FString& Params )
{
	...
	if( FilesToProcess.Num() == 0 )
	{
		...
		UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "The GatherTextFromSource commandlet couldn't find any source files matching (%ls) in the specified directories:\n%ls", *SourceFileSearchFiltersString, *SpecifiedDirectoriesString);
	...
	if (!SkipNestedMacroPrepass && !RanPrepassOnce)
	{
		...
		UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "Ran source prepass for nested macros in %.2f seconds", Duration);
	...
	// Mainpass
	{
		...
		UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "Ran source mainpass in %.2f seconds", Duration);
	}
...
void UGatherTextFromSourceCommandlet::GetParsables(TArray<FParsableDescriptor*>& Parsables, EGatherSourcePasses Pass, TArray<FParsedNestedMacro>& PrepassResults)
{
	...
	else
	{
		...
		Parsables.Add(new FStringMacroDescriptor( FString(MacroString_NSNOTELOCTEXT),
			...
			FStringMacroDescriptor::FMacroArg(FStringMacroDescriptor::MAS_DevNotes, true)));
	
		Parsables.Add(new FStringMacroDescriptor( FString(MacroString_NOTELOCTEXT),
			FStringMacroDescriptor::FMacroArg(FStringMacroDescriptor::MAS_Identifier, true),
			FStringMacroDescriptor::FMacroArg(FStringMacroDescriptor::MAS_SourceText, true),
			FStringMacroDescriptor::FMacroArg(FStringMacroDescriptor::MAS_DevNotes, true)));
		...
		Parsables.Add(new FStringTableNotedEntryMacroDescriptor());
		...
		// UE_DECLARE_ERROR(ApiMacro, ModuleName, ErrorCode, ErrorName, FormatString, ...)
		Parsables.Add(new FDeclareErrorDescriptor(TEXT("UE_DECLARE_ERROR"), TEXT("ErrorName"), 5, 1, 3, 4));
		// UE_DECLARE_ERROR_CONTEXT(ApiMacro, ModuleName, ContextName, FormatString, ...)
		Parsables.Add(new FDeclareErrorDescriptor(TEXT("UE_DECLARE_ERROR_CONTEXT"), TEXT("ContextName"), 4, 1, 2, 3));
...
void UGatherTextFromSourceCommandlet::RunPass(EGatherSourcePasses Pass, bool ShouldGatherFromEditorOnlyData, const TArray<FString>& FilesToProcess, const FString& GatheredSourceBasePath, TArray<FParsedNestedMacro>& PrepassResults)
{
	...
	for (int32 i = 0; i < CountThreads; ++i)
	{
		...
		for (int32 j = 0; j < Batch.Files.Num(); ++j)
		{
			...
			for (const auto& ParsedStringTablePair : ParseCtxt.ParsedStringTables)
			{
				...
				else
				{
					...
					{
						if ((!ParsedStringTableEntryPair.Value.bIsEditorOnly || ParseCtxt.ShouldGatherFromEditorOnlyData) && FText::ShouldGatherForLocalization(ParsedStringTableEntryPair.Value.SourceString))
						{
							...
							SourceContext.DevNotes = ParsedStringTableEntryPair.Value.DevNotes;
		...
		else if (Pass == EGatherSourcePasses::Mainpass)
		{
			...
			{
				if (FText::ShouldGatherForLocalization(Result.Source))
				{
...
{
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "---Gather Source Stats------------------------");
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "---Prepass------------------------------------");
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "*.h =                   %14d files", FileTypeStats[EGatherSourcePasses::Prepass].FileType_h.load());
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "*.inl =                 %14d files", FileTypeStats[EGatherSourcePasses::Prepass].FileType_inl.load());
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "*.c =                   %14d files", FileTypeStats[EGatherSourcePasses::Prepass].FileType_c.load());
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "*.cpp =                 %14d files", FileTypeStats[EGatherSourcePasses::Prepass].FileType_cpp.load());
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "*.ini =                 %14d files", FileTypeStats[EGatherSourcePasses::Prepass].FileType_ini.load());
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "Other =                 %14d files", FileTypeStats[EGatherSourcePasses::Prepass].FileType_other.load());
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "Total =                 %14d files", FileTypeStats[EGatherSourcePasses::Prepass].FileType_total.load());
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "Duration =              %14.2f seconds", FileTypeStats[EGatherSourcePasses::Prepass].Duration_sec.load());

	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "---Mainpass-----------------------------------");
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "*.h =                   %14d files", FileTypeStats[EGatherSourcePasses::Mainpass].FileType_h.load());
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "*.inl =                 %14d files", FileTypeStats[EGatherSourcePasses::Mainpass].FileType_inl.load());
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "*.c =                   %14d files", FileTypeStats[EGatherSourcePasses::Mainpass].FileType_c.load());
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "*.cpp =                 %14d files", FileTypeStats[EGatherSourcePasses::Mainpass].FileType_cpp.load());
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "*.ini =                 %14d files", FileTypeStats[EGatherSourcePasses::Mainpass].FileType_ini.load());
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "Other =                 %14d files", FileTypeStats[EGatherSourcePasses::Mainpass].FileType_other.load());
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "Total =                 %14d files", FileTypeStats[EGatherSourcePasses::Mainpass].FileType_total.load());
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "Duration =              %14.2f seconds", FileTypeStats[EGatherSourcePasses::Mainpass].Duration_sec.load());

	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "---Nested Macro-------------------------------");
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "DuplicateExact =        %14d", NestedMacroStats.DuplicateExact.load());
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "DuplicateMacroName =    %14d", NestedMacroStats.DuplicateMacroName.load());
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "DuplicateExcluded =     %14d", NestedMacroStats.DuplicateExcluded.load());
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "Concatenation =         %14d", NestedMacroStats.Concatenation.load());
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "Variadic =              %14d", NestedMacroStats.Variadic.load());
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "SizeInBytes =           %14d bytes", NestedMacroStats.SizeInBytes.load());

	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "");
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "Nested_LOCTEXT =        %14d", NestedMacroStats.Nested_LOCTEXT.load());
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "Nested_NSLOCTEXT =      %14d", NestedMacroStats.Nested_NSLOCTEXT.load());
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "Nested_NOTELOCTEXT =    %14d", NestedMacroStats.Nested_NOTELOCTEXT.load());
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "Nested_NSNOTELOCTEXT =  %14d", NestedMacroStats.Nested_NSNOTELOCTEXT.load());
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "Nested_UI_COMMAND =     %14d", NestedMacroStats.Nested_UI_COMMAND.load());
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "Nested_UI_COMMAND_EXT = %14d", NestedMacroStats.Nested_UI_COMMAND_EXT.load());

	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "");
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "PrepassBegin =          %14d", NestedMacroStats.PrepassBegin.load());
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "PrepassEnd =            %14d", NestedMacroStats.PrepassEnd.load());
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "MainpassBegin =         %14d", NestedMacroStats.MainpassBegin.load());
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "MainpassMid =           %14d", NestedMacroStats.MainpassMid.load());
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "MainpassEnd =           %14d", NestedMacroStats.MainpassEnd.load());

	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "");
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "Submit =                %14d", NestedMacroStats.Submit.load());
	UE_LOGF(LogGatherTextFromSourceCommandlet, Display, "SubmitNested =          %14d", NestedMacroStats.SubmitNested.load());
}
...
bool UGatherTextFromSourceCommandlet::FSourceFileParseContext::AddStringTableEntryImpl(const FName InTableId, const FString& InKey, const FString& InSourceString, const FSourceLocation& InSourceLocation, const FName InPlatformName, const FString& InDevNotes)
{
	const bool bIsEditorOnly = EvaluateEditorOnlyDefineState() == EEditorOnlyDefineState::Defined;
	...
	else
	{
		...
		ParsedStringTableEntry.DevNotes = InDevNotes;
...
void UGatherTextFromSourceCommandlet::FSourceFileParseContext::AddStringTableFromFile(const FName InTableId, const FString& InTableNamespace, const FString& InTableFilename, const FString& InRootPath)
{
	if (AddStringTableImpl(InTableId, InTableNamespace))
	{
		...
		if (TmpStringTable->ImportStringsFromCSVFile(FullImportPath))
		{
			...
			TmpStringTable->EnumerateSourceStrings([&](const FString& InKey, const FString& InSourceString, const FString& InDevNote)
			{
				AddStringTableEntryImpl(InTableId, InKey, InSourceString, SourceLocation, TablePlatformName, InDevNote);
...
void UGatherTextFromSourceCommandlet::FSourceFileParseContext::AddStringTableEntry(const FName InTableId, const FString& InKey, const FString& InSourceString, const FString& InDevNotes)
{
	AddStringTableEntryImpl(InTableId, InKey, InSourceString, FSourceLocation(Filename, LineNumber), FilePlatformName, InDevNotes);
}
...
void UGatherTextFromSourceCommandlet::FNestedMacroPrepassDescriptor::TryParse(const FString& Text, FSourceFileParseContext& Context) const
{
	...
	bool bAdvance = false;
	if (Content.Contains(MacroString_NOTELOCTEXT, ESearchCase::CaseSensitive))
	{
		PrepassResults.Emplace(MacroName, MacroString_NOTELOCTEXT, Context.Filename, Content, LineStart, LineCount);
		++NestedMacroStats.Nested_NOTELOCTEXT;
		bAdvance = true;
	}
	if (Content.Contains(MacroString_NSNOTELOCTEXT, ESearchCase::CaseSensitive))
	{
		PrepassResults.Emplace(MacroName, MacroString_NSNOTELOCTEXT, Context.Filename, Content, LineStart, LineCount);
		++NestedMacroStats.Nested_NSNOTELOCTEXT;
		bAdvance = true;
	}
...
void UGatherTextFromSourceCommandlet::FNestedMacroDescriptor::TryParse(const FString& Text, FSourceFileParseContext& Context) const
{
	...
	else if (MacroNameNested == MacroString_NOTELOCTEXT)
	{
		InnerDescriptor = new FStringMacroDescriptor(FString(MacroString_NOTELOCTEXT),
			FStringMacroDescriptor::FMacroArg(FStringMacroDescriptor::MAS_Identifier, true),
			FStringMacroDescriptor::FMacroArg(FStringMacroDescriptor::MAS_SourceText, true),
			FStringMacroDescriptor::FMacroArg(FStringMacroDescriptor::MAS_DevNotes, true));
	}
	else if (MacroNameNested == MacroString_NSNOTELOCTEXT)
	{
		InnerDescriptor = new FStringMacroDescriptor(FString(MacroString_NSNOTELOCTEXT),
			FStringMacroDescriptor::FMacroArg(FStringMacroDescriptor::MAS_Namespace, true),
			FStringMacroDescriptor::FMacroArg(FStringMacroDescriptor::MAS_Identifier, true),
			FStringMacroDescriptor::FMacroArg(FStringMacroDescriptor::MAS_SourceText, true),
			FStringMacroDescriptor::FMacroArg(FStringMacroDescriptor::MAS_DevNotes, true));
		...
		if (ParseArgsFromMacro(StripCommentsFromToken(Text, Context), ArgArray, Context))
		{
			...
			else
			{
				...
				if (!ArgParseError && !Identifier.IsEmpty() && !SourceText.IsEmpty())
				{
					...
					MacroContext.DevNotes = DevNotes;
		...
		if (ParseArgsFromMacro(StripCommentsFromToken(Text, Context), Arguments, Context))
		{
			...
			else
			{
				...
				if (PrepareArgument(TableId, true, TableIdMacroDesc, HasQuotes) && PrepareArgument(Key, true, KeyMacroDesc, HasQuotes) && PrepareArgument(SourceString, true, SourceStringMacroDesc, HasQuotes))
				{
					...
					{
						Context.AddStringTableEntry(TableIdName, Key, SourceString, TEXT(""));
					}
				}
			}
		}
...
void UGatherTextFromSourceCommandlet::FStringTableNotedEntryMacroDescriptor::TryParse(const FString& Text, FSourceFileParseContext& Context) const
{
	// Attempt to parse something of the format
	// LOCTABLE_SETNOTESTRING(Id, Key, SourceString, DevNotes)
	...
	if (!Context.ExcludedRegion && !Context.WithinBlockComment && !Context.WithinLineComment && !Context.WithinStringLiteral)
	{
		...
		if (ParseArgsFromMacro(StripCommentsFromToken(Text, Context), Arguments, Context))
		{
			if (Arguments.Num() != GetMinNumberOfArgument())
			{
				UE_LOGF(LogGatherTextFromSourceCommandlet, Warning, "%ls(%d): Expected %d arguments for %ls macro, but got %d. %ls", *Context.Filename, Context.LineNumber, GetMinNumberOfArgument(), *GetToken(), Arguments.Num(), *FLocTextHelper::SanitizeLogOutput(Context.LineText.TrimStartAndEnd()));
			}
			else
			{
				Arguments[0].TrimStartInline();
				...
				FString DevNotes = Arguments[3];

				const FString TableIdMacroDesc = FString::Printf(TEXT("%s(%d): \"Id\" argument in %s macro"), *Context.Filename, Context.LineNumber, *GetToken());
				const FString KeyMacroDesc = FString::Printf(TEXT("%s(%d): \"Key\" argument in %s macro"), *Context.Filename, Context.LineNumber, *GetToken());
				const FString SourceStringMacroDesc = FString::Printf(TEXT("%s(%d): \"SourceString\" argument in %s macro"), *Context.Filename, Context.LineNumber, *GetToken());
				const FString DevNoteMacroDesc = FString::Printf(TEXT("%s(%d): \"DevNote\" argument in %s macro"), *Context.Filename, Context.LineNumber, *GetToken());

				bool HasQuotes;
				if (PrepareArgument(TableId, true, TableIdMacroDesc, HasQuotes) && PrepareArgument(Key, true, KeyMacroDesc, HasQuotes) && PrepareArgument(SourceString, true, SourceStringMacroDesc, HasQuotes) && PrepareArgument(DevNotes, true, DevNoteMacroDesc, HasQuotes))
				{
					const FName TableIdName = *TableId;

					if (TableIdName.IsNone() || Key.IsEmpty())
					{
						UE_LOGF(LogGatherTextFromSourceCommandlet, Warning, "%ls(%d): %ls macro has an empty identifier and cannot be gathered.", *Context.Filename, Context.LineNumber, *GetToken());
					}
					else if (!SourceString.IsEmpty())
					{
						Context.AddStringTableEntry(TableIdName, Key, SourceString, DevNotes);
					}
				}
			}
		}
...
UGatherTextFromSourceCommandlet::FDeclareErrorDescriptor::FDeclareErrorDescriptor(const TCHAR* InMacroName, const TCHAR* InKeyParameterName, int32 InMinArguments, int32 InNamespaceIndex, int32 InKeyIndex, int32 InTextIndex)
	: FMacroDescriptor(InMacroName, InMinArguments)
	, KeyParameterName(InKeyParameterName)
	, NamespaceIndex(InNamespaceIndex)
	, KeyIndex(InKeyIndex)
	, TextIndex(InTextIndex)
{
}

void UGatherTextFromSourceCommandlet::FDeclareErrorDescriptor::TryParse(const FString& Text, FSourceFileParseContext& Context) const
{
	// Attempt to parse something of the format
	// UE_DECLARE_ERROR(ApiMacro, ModuleName, ErrorCode, ErrorName, FormatString...)
	// UE_DECLARE_ERROR_CONTEXT(ApiMacro, ModuleName, ContextName, FormatString...)

	// A localization entry formed with namespace ModuleName, key ErrorName, and unlocalized text FormatString
	if (!Context.ExcludedRegion && !Context.WithinBlockComment && !Context.WithinLineComment && !Context.WithinStringLiteral)
	{
		TArray<FString> Arguments;
		if (ParseArgsFromMacro(StripCommentsFromToken(Text, Context), Arguments, Context))
		{
			...
			// Macros take plain string literals and not TEXT("...")
			constexpr bool bIsAutoText = true;
			bool bHasQuotes = false;
			// Per-argument leading whitespace from "," separators is preserved by ParseArgumentString;
			// PrepareArgument only TrimEnds, so without TrimStart the leading space prevents TrimQuotes
			// from recognizing the opening quote on string literals.
			FString Namespace = Arguments[NamespaceIndex];
			Namespace.TrimStartInline();
			if (!PrepareArgument(Namespace, bIsAutoText, FString::Printf(TEXT("%s: \"Module\" argument in %s macro"), *SourceLocation, *GetToken()), bHasQuotes))
			{
				return;
			}
			if (Namespace.IsEmpty())
			{
				UE_LOGFMT(LogGatherTextFromSourceCommandlet, Warning, "{file}({line}): {macroName} macro has an empty {parameterName} and cannot be gathered.",
					("file", *Context.Filename),
					("line", Context.LineNumber),
					("macroName", *GetToken()),
			...
			FString Key = Arguments[KeyIndex];
			Key.TrimStartInline();
			if (!PrepareArgument(Key, bIsAutoText, FString::Printf(TEXT("%s: \"ErrorName\" argument in %s macro"), *SourceLocation, *GetToken()), bHasQuotes))
			{
				UE_LOGFMT(LogGatherTextFromSourceCommandlet, Warning, "{file}({line}): {macroName} macro has an empty {parameterName} and cannot be gathered.",
					("file", *Context.Filename),
					("line", Context.LineNumber),
			...
			FString Format = Arguments[TextIndex];
			Format.TrimStartInline();
			if (!PrepareArgument(Format, bIsAutoText, FString::Printf(TEXT("%s: \"FormatString\" argument in %s macro"), *SourceLocation, *GetToken()), bHasQuotes))
			{
				UE_LOGFMT(LogGatherTextFromSourceCommandlet, Warning, "{file}({line}): {macroName} macro has an empty {parameterName} and cannot be gathered.",
					("file", *Context.Filename),
					("line", Context.LineNumber),
