...
int32 UReplaceAssetsCommandlet::Main(const FString& InParams)
{
	...
	{
		UE_LOGF(LogReplaceAssetsCommandlet, Error, "Source Path for replacement assets does not exist - please specify a valid location for -AssetSourcePath on the commandline");
		return 1;
	}
	...
	UE_LOGF(LogReplaceAssetsCommandlet, Display, "Searching Asset Registry");
	...
	{
		UE_LOGF(LogReplaceAssetsCommandlet, Display, "Getting Assets from specified paths");
		FARFilter Filter;
	...
	{
		UE_LOGF(LogReplaceAssetsCommandlet, Display, "Getting Assets of specified classes");
		FARFilter Filter;
		Filter.bIncludeOnlyOnDiskAssets = true;
		...
		for (const FString& ReplacedClass : ReplacedClasses)
		{
			...
			{
				UE_LOGF(LogReplaceAssetsCommandlet, Error, "Failed to convert short class name \"%ls\" to path name. Please use class path names for ReplacedClasses.", *ReplacedClass);
			}
			else
			{
	...
	{
		UE_LOGF(LogReplaceAssetsCommandlet, Display, "Excluding Assets from specified paths");
		FARFilter Filter;
		Filter.bIncludeOnlyOnDiskAssets = true;
		Filter.bRecursivePaths = true;
	...
	{
		UE_LOGF(LogReplaceAssetsCommandlet, Display, "Excluding Assets of specified classes");
		FARFilter Filter;
		Filter.bIncludeOnlyOnDiskAssets = true;
		Filter.PackagePaths.Add(TEXT("/Game"));
		Filter.bRecursivePaths = true;
		for (const FString& ExcludedClass : ExcludedClasses)
		{
			...
			{
				UE_LOGF(LogReplaceAssetsCommandlet, Error, "Failed to convert short class name \"%ls\" to path name. Please use class path names for ExcludedClasses.", *ExcludedClass);
			}
			else
			{
	...
	{
		UE_LOGF(LogReplaceAssetsCommandlet, Display, "Converting Package Names to File Paths");
		...
		UE_LOGF(LogReplaceAssetsCommandlet, Display, "Replacing files...");
		for (const FString& ReplacedPath : FinalFileList)
		{
			UE_LOGF(LogReplaceAssetsCommandlet, Log, "Replacing asset: %ls", *ReplacedPath);
			if (!IFileManager::Get().Delete(*ReplacedPath, false, true))
			{
				UE_LOGF(LogReplaceAssetsCommandlet, Error, "Failed to delete asset: %ls", *ReplacedPath);
			}
			...
			if (IFileManager::Get().FileExists(*ReplacementPath))
			{
				...
				{
					UE_LOGF(LogReplaceAssetsCommandlet, Error, "Failed to copy asset: %ls", *ReplacementPath);
				}
			}
		}
	}
