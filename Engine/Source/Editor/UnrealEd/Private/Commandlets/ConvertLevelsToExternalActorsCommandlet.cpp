...
ULevel* UConvertLevelsToExternalActorsCommandlet::LoadLevel(const FString& LevelToLoad) const
{
	...
	UE_LOGF(LogConvertLevelsToExternalActorsCommandlet, Log, "Loading level %ls.", *LevelToLoad);
	CLEAR_WARN_COLOR();
		...
		for (const FSoftObjectPath& DuplicatedActorFileKey : DuplicatedActorFilesKeys)
		{
			...
			for (int32 i=0; i<DuplicatedActorFilesPaths.Num(); i++)
			{
				if (i != MostRecentIndex)
				{
					...
					UE_LOGF(LogConvertLevelsToExternalActorsCommandlet, Warning, "Found duplicated actor file %ls", *Filename);
...
bool UConvertLevelsToExternalActorsCommandlet::AddPackageToSourceControl(UPackage* Package)
{
	if (UseSourceControl())
	{
		...
		{
			UE_LOGF(LogConvertLevelsToExternalActorsCommandlet, Log, "Adding package %ls to revision control", *PackageFilename);
			if (GetSourceControlProvider().Execute(ISourceControlOperation::Create<FMarkForAdd>(), Package) != ECommandResult::Succeeded)
			{
				UE_LOGF(LogConvertLevelsToExternalActorsCommandlet, Error, "Error adding %ls to revision control.", *PackageFilename);
				return false;
			}
		}
	}
...
bool UConvertLevelsToExternalActorsCommandlet::SavePackage(UPackage* Package)
{
	...
	{
		UE_LOGF(LogConvertLevelsToExternalActorsCommandlet, Error, "Error saving %ls", *PackageFileName);
		return false;
	}
...
bool UConvertLevelsToExternalActorsCommandlet::CheckoutPackage(UPackage* Package)
{
	if (UseSourceControl())
	{
		...
		if (SourceControlState.IsValid())
		{
			...
			{
				UE_LOGF(LogConvertLevelsToExternalActorsCommandlet, Error, "Overwriting package %ls already checked out by %ls, will not submit", *PackageFilename, *OtherCheckedOutUser);
				return false;
			}
			else if (!SourceControlState->IsCurrent())
			{
				UE_LOGF(LogConvertLevelsToExternalActorsCommandlet, Error, "Overwriting package %ls (not at head revision), will not submit", *PackageFilename);
				return false;
			}
			else if (SourceControlState->IsCheckedOut() || SourceControlState->IsAdded())
			{
				UE_LOGF(LogConvertLevelsToExternalActorsCommandlet, Log, "Skipping package %ls (already checked out)", *PackageFilename);
				return true;
			}
			else if (SourceControlState->IsSourceControlled())
			{
				UE_LOGF(LogConvertLevelsToExternalActorsCommandlet, Log, "Checking out package %ls from revision control", *PackageFilename);
	...
	else
	{
		...
		if (IPlatformFile::GetPlatformPhysical().FileExists(*PackageFilename))
		{
			...
			{
				UE_LOGF(LogConvertLevelsToExternalActorsCommandlet, Error, "Error setting %ls writable", *PackageFilename);
				return false;
			}
		}
	}
...
bool UConvertLevelsToExternalActorsCommandlet::DeleteFile(const FString& Filename)
{
	if (!UseSourceControl())
	{
		...
		{
			UE_LOGF(LogConvertLevelsToExternalActorsCommandlet, Error, "Error deleting %ls", *Filename);
			return false;
		}
	}
	else 
	{
		...
		if (SourceControlState.IsValid() && SourceControlState->IsSourceControlled())
		{
			...
			if (SourceControlState->IsCheckedOutOther(&OtherCheckedOutUser))
			{
				UE_LOGF(LogConvertLevelsToExternalActorsCommandlet, Error, "Overwriting package %ls already checked out by %ls, will not submit", *Filename, *OtherCheckedOutUser);
				return false;
			}
			else if (!SourceControlState->IsCurrent())
			{
				UE_LOGF(LogConvertLevelsToExternalActorsCommandlet, Error, "Overwriting package %ls (not at head revision), will not submit", *Filename);
				return false;
			}
			else if (SourceControlState->IsAdded())
			{
				...
				{
					UE_LOGF(LogConvertLevelsToExternalActorsCommandlet, Error, "Error reverting package %ls from revision control", *Filename);
					return false;
				}
			}
			else
			{
				UE_LOGF(LogConvertLevelsToExternalActorsCommandlet, Log, "Deleting package %ls from revision control", *Filename);
				...
				{
					UE_LOGF(LogConvertLevelsToExternalActorsCommandlet, Error, "Error deleting package %ls from revision control", *Filename);
					return false;
				}
			}
		}
		else
		{
			...
			{
				UE_LOGF(LogConvertLevelsToExternalActorsCommandlet, Error, "Error deleting package %ls locally", *Filename);
				return false;
			}
		}
	}
...
int32 UConvertLevelsToExternalActorsCommandlet::Main(const FString& Params)
{
	...
	if (Tokens.Num() < 1)
	{
		UE_LOGF(LogConvertLevelsToExternalActorsCommandlet, Error, "ConvertLevelToExternalActors bad parameters");
		return 1;
	}
	...
	{
		UE_LOGF(LogConvertLevelsToExternalActorsCommandlet, Error, "Unknown level '%ls'", *Tokens[0]);
		return 1;
	}
	...
	{
		UE_LOGF(LogConvertLevelsToExternalActorsCommandlet, Error, "External actor files inconsistency");
		return 1;
	}
	...
	if (!MainLevel)
	{
		UE_LOGF(LogConvertLevelsToExternalActorsCommandlet, Error, "Unable to load level '%ls'", *Tokens[0]);
		return 1;
	}
	...
	for(ULevel* Level : LevelsToConvert)
	{
		if (!Level->bContainsStableActorGUIDs)
		{
			...
			UE_LOGF(LogConvertLevelsToExternalActorsCommandlet, Error, "Unable to convert level '%ls' with non-stable actor GUIDs. Resave the level before converting.", *Level->GetPackage()->GetName());
		}
	}
	...
	// Save packages
	UE_LOGF(LogConvertLevelsToExternalActorsCommandlet, Log, "Saving %d packages.", PackagesToSave.Num());
	for (UPackage* PackageToSave : PackagesToSave)
	{
	...
	UE_LOGF(LogConvertLevelsToExternalActorsCommandlet, Log, "Conversion took %.2f seconds", ConversionTimer.GetTime());
	return 0;
}
