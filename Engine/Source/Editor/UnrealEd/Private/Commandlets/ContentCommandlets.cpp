...
int32 UResavePackagesCommandlet::InitializeResaveParameters( const TArray<FString>& Tokens, TArray<FString>& PackageNames )
{
	...
	for( int32 SwitchIdx = 0; SwitchIdx < Switches.Num(); SwitchIdx++ )
	{
		...
		if( FParse::Value( *CurrentSwitch, TEXT( "PACKAGE="), Package ) )
		{
			...
			{
				UE_LOGF(LogContentCommandlet, Error, "Failed to find the package given by the cmdline: PACKAGE=%ls", *Package);
			}
		...
		else if( FParse::Value( *CurrentSwitch, TEXT( "PACKAGEFOLDER="), PackageFolderArg) )
		{
			...
			UE_CLOGF(PackageNames.IsEmpty(), LogContentCommandlet, Warning, "Failed to find any packages in folder: '%ls'", *PackageFolderArg);
			bExplicitPackages = true;
		}
		...
		else if (FParse::Value(*CurrentSwitch, TEXT("FILE="), File))
		{
			...
			if (FFileHelper::LoadFileToString(Text, *File))
			{
				...
				for (auto Line : Lines)
				{
					...
					{
						UE_LOGF(LogContentCommandlet, Error, "Failed to find package %ls", *Line);
					}
				}

				bExplicitPackages = true;
				UE_LOGF(LogContentCommandlet, Display, "Loaded %d Packages from %ls", PackageNames.Num(), *File);
			}
			else
			{
				UE_LOGF(LogContentCommandlet, Error, "Failed to load file %ls", *File);
			}
		}
		else if (FParse::Value(*CurrentSwitch, TEXT("COLLECTION="), CollectionArg))
		{
			...
			TArray<FSoftObjectPath> CollectionObjects;

			TArray<FString> Collections;
			CollectionArg.ParseIntoArray(Collections, TEXT("+"));
			for (const FString& Collection : Collections)
			{
				if (!CollectionContainer->CollectionExists(*Collection, ECollectionShareType::CST_All))
				{
					UE_LOGF(LogContentCommandlet, Error, "Unknown source collection '%ls'", *Collection);
					continue;
				}

				CollectionObjects.Reset();
				if (!CollectionContainer->GetAssetsInCollection(*Collection, ECollectionShareType::CST_All, CollectionObjects))
				{
					UE_LOGF(LogContentCommandlet, Error, "Failed to get assets in collection '%ls'", *Collection);
					continue;
				}

				for (const FSoftObjectPath& CollectionObject : CollectionObjects)
				{
					PackageNames.Add(CollectionObject.GetLongPackageName());
					bExplicitPackages = true;
				}
			}
		}
	...
	for (const FString& CurrentSwitch : Switches)
	{
		...
		{
			UE_LOGF(LogContentCommandlet, Display, "Setting garbage collection to happen every %d packages.", GarbageCollectionFrequency);
		}
	}

	if ((bShouldBuildLighting || bShouldBuildReflectionCaptures) && !bExplicitPackages)
	{
		UE_LOGF(LogContentCommandlet, Display, "No maps found to save when building lighting, checking CommandletSettings:ResavePackages in EditorIni");
		...
		for ( const auto& ResavePackage : ResavePackages )
		{
			...
			UE_LOGF(LogContentCommandlet, Display, "Rebuilding lighting for package %ls", *PackageFile);
			PackageNames.Add(*PackageFile);
			bExplicitPackages = true;
		}
	}
	...
	{
		UE_CLOGF( bShouldBuildHLOD, LogContentCommandlet, Display, "No maps found to save when building HLODs, checking Project Settings for Directory or Asset Path(s)" );
	...
	for ( int32 SwitchIdx = 0; SwitchIdx < Switches.Num(); SwitchIdx++ )
	{
		...
		{
			UE_LOGF(LogContentCommandlet, Warning, "Only resaving a maximum of %d packages.", MaxPackagesToResave );
			break;
		}
	}
	...
	if (!ResaveOnDemandSystems.IsEmpty())
	{
		...
		UE_LOGF(LogContentCommandlet, Display, "ResaveOnDemand=%ls. Only saving packages that are reported via UE::SavePackageUtilities::OnAddResaveOnDemandPackage.",
			*SystemNames);
		...
		{
			UE_LOGF(LogContentCommandlet, Error, "The BulkDataRegistry must be enabled to use -ResaveOnDemand=BulkDataDuplicates! Try using the -AllowCommandletBulkDataRegistry cmdline switch.");
			return 1;
		}
	}
	...
	for (const FString& CurrentSwitch : Switches)
	{
		if ( FParse::Value(*CurrentSwitch, TEXT("RESAVECLASS="), ClassList, false) )
		{
			...
			for (FString& ClassName : ClassNames)
			{
				...
				{
					UE_LOGF(LogContentCommandlet, Warning, "RESAVECLASS param requires class path names. Short name provided: %ls.", *ClassName);
	...
	{
		// Sanity check fail
		UE_LOGF(LogContentCommandlet, Error, "AllowSubclasses param requires ResaveClass param.");
		return 1;
	}
...
void UResavePackagesCommandlet::ParseSourceControlOptions(const TArray<FString>& Tokens)
{
	...
	for (const FString& CurrentSwitch : Tokens)
	{
		if (FParse::Value(*CurrentSwitch, TEXT("BatchPackageLimit="), QueuedPackageFlushLimit))
		{
			...
			{
				UE_LOGF(LogContentCommandlet, Display, "Setting revision control batches to be limited to %d package(s) at a time.", QueuedPackageFlushLimit);
			...
			else
			{
				...
				UE_LOGF(LogContentCommandlet, Display, "Setting revision control batches to have no package limit!");
			}
		}
		else  if (FParse::Value(*CurrentSwitch, TEXT("BatchFileSizeLimit="), QueueFileSizeFlushLimit))
		{
			...
			{
				UE_LOGF(LogContentCommandlet, Display, "-BatchFileSizeLimit=%lld is too large! The max value allowed is %lld, clamping...", QueueFileSizeFlushLimit, SizeLimit);
				QueueFileSizeFlushLimit = SizeLimit;
			}

			if (QueueFileSizeFlushLimit >= 0)
			{
				UE_LOGF(LogContentCommandlet, Display, "Setting revision control batches to be limited to %lld MB.", QueueFileSizeFlushLimit);
			...
			else
			{
				...
				UE_LOGF(LogContentCommandlet, Display, "Setting revision control batches to have no disk space limit!");
			}
		}
	}
}
...
void UResavePackagesCommandlet::LoadAndSaveOnePackage(const FString& Filename)
{
	...
	// Convert this path to mounted before querying the asset registry or CollectionFilter because they use mounted paths 
	PackagePath.TryConvertToMounted();
	...
	if ( bIsReadOnly && !bVerifyContent && !bAutoCheckOut )
	{
		...
		{
			UE_LOGF(LogContentCommandlet, Warning, "Skipping read-only file %ls", *Filename);
		}
	}
	else
	{
		if (Verbosity != ONLY_ERRORS)
		{
			UE_LOGF(LogContentCommandlet, Display, "Loading %ls", *Filename);
		}
		...
		{
			UE_LOGF(LogContentCommandlet, Warning, "%d total errors encountered during loading", NumErrorsFromLoading);
		}
		...
		// Load the package before accessing its linker to prevent interference with async loads in case
		// that package was already queued and being loaded asynchronously.
		LoadPackage(nullptr, PackagePath, LOAD_NoVerify);
		...
		if (bSavePackage)
		{
			...
			{
				//UE_LOGF(LogContentCommandlet, Warning, "NumErrorsFromLoading: %d GWarn->Errors num: %d", NumErrorsFromLoading, GWarn->GetNumErrors() );
			...
			{
				UE_LOGF(LogContentCommandlet, Log, "Removing editor only data");
			...
			if (bSavePackage == true)
			{
				...
				if (ObjectsInOuter.IsEmpty())
				{
					...
					UE_LOGF(LogContentCommandlet, Display, "Package %ls is empty and will be deleted", *Filename);
			...
			if (bSavePackage == true)
			{
				...
				{
					UE_LOGF(LogContentCommandlet, Warning, "Package [%ls] is read-only but needs to be resaved (UE Version: %i, Licensee Version: %i  Current UE Version: %i, Current Licensee Version: %i)",
					...
					{
						UE_LOGF(LogContentCommandlet, Warning, "Correctly saved:  [Temp.temp]." );
					}
				}
				else
				{
					...
					if (bAttemptCheckoutNow)
					{
						...
						if( bIsReadOnly )
						{
							...
							if(SourceControlState.IsValid())
							{
								...
								{
									UE_LOGF(LogContentCommandlet, Warning, "[REPORT] Overwriting package %ls already checked out by someone else (%ls), will not submit", *Filename, *OtherCheckedOutUser);
								}
								else if( !SourceControlState->IsCurrent() )
								{
									UE_LOGF(LogContentCommandlet, Warning, "[REPORT] Overwriting package %ls (not at head), will not submit", *Filename);
								}
								else
								{
					...
					if (bAttemptCheckoutNow && bIsReadOnly)
					{
						...
						{
							UE_LOGF(LogContentCommandlet, Warning, "Skipping Package: %ls (unable to check out)", *Filename);
						}
						else
						{
							UE_LOGF(LogContentCommandlet, Error, "Unable to check out the Package: %ls", *Filename);
						}
					...
					if (Verbosity != ONLY_ERRORS)
					{
						UE_LOGF(LogContentCommandlet, Display, "Resaving package [%ls] (UE Version: %i, Licensee Version: %i  Saved UE Version: %i, Saved Licensee Version: %i)",
							*Filename,Linker->Summary.GetFileVersionUE().ToValue(), Linker->Summary.GetFileVersionLicenseeUE(), GPackageFileUEVersion.ToValue(), VER_LATEST_ENGINE_LICENSEEUE4 );
					}

					uint32 SaveFlags = ESaveFlags::SAVE_None;
					...
					if (bIsReadOnly == false || SourceControlQueue == nullptr)
					{
						...
						if (SavePackageHelper(Package, Filename, RF_Standalone, GWarn, (ESaveFlags)SaveFlags))
						{
							PackagesResaved++;
							if (Verbosity == VERY_VERBOSE)
							{
								UE_LOGF(LogContentCommandlet, Display, "Correctly saved:  [%ls].", *Filename);
							}
						}
					}
					else
					{
						...
						// Prevent the persistent GUID of the package being regenerated as we are saving out to a different package path.
						// We want the GUID to remain the same as we will eventually replace the source package with this temp one.
						SaveFlags |= ESaveFlags::SAVE_KeepPersistentGUID;

						if (SavePackageHelper(Package, TempFilename, RF_Standalone, GWarn, (ESaveFlags)SaveFlags))
						{
		...
		if (!GarbageCollectionFrequency || Counter++ % GarbageCollectionFrequency == 0)
		{
			...
			{
				UE_LOGF(LogContentCommandlet, Display, "GC");
			}
...
void UResavePackagesCommandlet::DeleteOnePackage(const FString& Filename)
{
	...
	{
		UE_LOGF(LogContentCommandlet, Display, "Revert '%ls' from revision control...", *Filename);
		SourceControlProvider.Execute(ISourceControlOperation::Create<FRevert>(), PackageFilename);

		UE_LOGF(LogContentCommandlet, Display, "Deleting '%ls' from revision control...", *Filename);
	...
	{
		UE_LOGF(LogContentCommandlet, Warning, "Couldn't delete '%ls' from revision control, someone has it checked out, skipping...", *Filename);
	}
	else if (SourceControlState.IsValid() && !SourceControlState->IsSourceControlled())
	{
		UE_LOGF(LogContentCommandlet, Warning, "'%ls' is not in revision control, attempting to delete from disk...", *Filename);
		...
		{
			UE_LOGF(LogContentCommandlet, Warning, "  ... failed to delete from disk.");
		}
	}
	else
	{
		UE_LOGF(LogContentCommandlet, Warning, "'%ls' is in an unknown revision control state, attempting to delete from disk...", *Filename);
		if (IFileManager::Get().Delete(*Filename, false, true)== true)
		{
			PackagesDeleted++;
		}
...
int32 UResavePackagesCommandlet::Main( const FString& Params )
{
	...
	// We load the default editor modules to ensure UPackage::OnMetaDataTransferRequestedDelegate is bound and handled
	// By default FMetaData gets thrown out, the callback re-adds it to the new packages.
	// One such example is the UserAssetTagsEditor, which needs to be loaded so the tags aren't lost during Resave.
	...
	/** force the materials auto usage flag */
	{
		int32 ForceAutoUsageFlagValue = 0;
		if (FParse::Value(FCommandLine::Get(), TEXT("ForceMaterialAutoUsageFlag="), ForceAutoUsageFlagValue))
		{
			if (bOnlyMaterials)
			{
				ForceMaterialAutoUsageFlag = (ForceAutoUsageFlagValue != 0);
			}
			else
			{
				UE_LOGF(LogContentCommandlet, Warning,
					"ForceMaterialAutoUsageFlag should only be used with -onlymaterials. Ignoring.");
			}
		}
	...
	{
		UE_LOGF(LogContentCommandlet, Warning,
			"KeepPackageGUIDOnSave is no longer supported, because package save no longer records arbitrary guids but instead records a hash of the package's bytes - PackageSavedHash.");
	}
	...
	if (!FilterByCollection.IsEmpty())
	{
		...
		{
			UE_LOGF(LogContentCommandlet, Warning, "Could not get assets in collection '%ls'. Skipping filter.", *FilterByCollection);
		}
		else
		{
	...
	if (bShouldBuildHLOD)
	{
		...
		UE_LOGF(LogContentCommandlet, Display, "Rebuilding HLODs... Options are:");
		UE_LOGF(LogContentCommandlet, Display, "  [%ls] Clusters", bGenerateClusters ? TEXT("X") : TEXT(" "));
		UE_LOGF(LogContentCommandlet, Display, "  [%ls] Proxies", bGenerateMeshProxies ? TEXT("X") : TEXT(" "));
		UE_LOGF(LogContentCommandlet, Display, "  [%ls] ForceClusters", bForceClusterGeneration ? TEXT("X") : TEXT(" "));
		UE_LOGF(LogContentCommandlet, Display, "  [%ls] ForceProxies", bForceProxyGeneration ? TEXT("X") : TEXT(" "));
		UE_LOGF(LogContentCommandlet, Display, "  [%ls] SkipSubLevels", bSkipSubLevels ? TEXT("X") : TEXT(" "));
		UE_LOGF(LogContentCommandlet, Display, "  [%ls] ForceSingleCluster", bForceSingleClusterForLevel ? TEXT("X") : TEXT(" "));
		UE_LOGF(LogContentCommandlet, Display, "  [%ls] Map Cleanup", bHLODMapCleanup ? TEXT("X") : TEXT(" "));
	...
	if (QualityStr.Len())
	{
		...
		{
			UE_LOGF(LogContentCommandlet, Fatal, "Unknown Quality(must be Preview/Medium/High/Production): %ls", *QualityStr );
		}
		UE_LOGF(LogContentCommandlet, Display, "Lighing Build Quality is %ls", *QualityStr);
	}
	...
	{
		UE_LOGF(LogContentCommandlet, Display, "Restricted to packages containing %ls", *PackageSubstring );
	}
	...
	if (bBatchSourceControl)
	{
		...
		case ONLY_ERRORS:
			...
			break;
		default:
			UE_LOGF(LogContentCommandlet, Warning, "Unknown verbosity to pass to FQueuedSourceControlOperations!");
		}
	...
	UE_LOGF(LogContentCommandlet, Display, "Flushing Async Loading");
	FlushAsyncLoading();
	...
	for( int32 PackageIndex = 0; PackageIndex < PackageNames.Num(); PackageIndex++ )
	{
		...
		if (!bCanProcessPackage)
		{
			...
			{
				UE_LOGF(LogContentCommandlet, Display, "Skipping %ls", *Filename);
				continue;
			}
		}
		...
		{
			UE_LOGF(LogContentCommandlet, Warning, "Attempting to resave more than MaxPackagesToResave; exiting" );
			break;
		}
	}
	...
	for (int32 PackageIndex = 0; PackageIndex < RedirectorsToFixup.Num(); PackageIndex++)
	{
		...
		if (Referencers.Num() > 0)
		{
			if (Verbosity != ONLY_ERRORS)
			{
				UE_LOGF(LogContentCommandlet, Display, "Can't delete redirector [%ls], unsaved packages reference it", *Filename);
			}
			continue;
		}
		...
		if (DeleteMinimumAgeDays > 0)
		{
			...
			if (DaysSinceModification < DeleteMinimumAgeDays)
			{
				if (Verbosity != ONLY_ERRORS)
				{
					UE_LOGF(LogContentCommandlet, Display,
						"Not deleting unreferenced redirector [%ls] (age threshold set to %d days, redirector was modified %d days ago)",
		...
		if (Verbosity != ONLY_ERRORS)
		{
			UE_LOGF(LogContentCommandlet, Display, "Deleting unreferenced redirector [%ls]", *Filename);
		}
	...
	UE_LOGF(LogContentCommandlet, Display, "[REPORT] %d/%d packages were considered for resaving", PackagesConsideredForResave, TotalPackagesForResave);
	UE_LOGF(LogContentCommandlet, Display, "[REPORT] %d/%d packages were resaved", PackagesResaved, PackagesConsideredForResave);
	UE_LOGF(LogContentCommandlet, Display, "[REPORT] %d/%d packages were deleted", PackagesDeleted, PackagesConsideredForResave);
...
void UResavePackagesCommandlet::PerformPreloadOperations( FLinkerLoad* PackageLinker, bool& bSavePackage )
{
	...
	{
		UE_LOGF(LogContentCommandlet, Warning, "Skipping resave of %ls due to engine version mismatch (Package:%d, Editor:%d) ", 
	...
	if (bOnlyLicenseed)
	{
		...
		{
			UE_LOGF(LogContentCommandlet, Display, "Resaving %ls that contains licensee version (%ls)",
				*PackageLinker->GetArchiveName(),
	...
	if (SourceControlState.IsValid())
	{
		...
		else if (!SourceControlState->IsSourceControlled())
		{
			if ( bAddFile )
			{
				...
				{
					UE_LOGF(LogContentCommandlet, Display, "[REPORT] %ls successfully added", *Filename);
					return true;
				}
				else
				{
					...
					{
						UE_LOGF(LogContentCommandlet, Error, "[REPORT] %ls could not be added!", *Filename);
					}
					else
					{
						UE_LOGF(LogContentCommandlet, Warning, "[REPORT] %ls could not be added!", *Filename);
					}
				}
			}
		}
		else if (!SourceControlState->IsCurrent())
		{
			if (!bIgnoreAlreadyCheckedOut)
			{
				UE_LOGF(LogContentCommandlet, Error, "[REPORT] %ls is not synced to head, can not submit", *Filename);
			}
			else
			{
				UE_LOGF(LogContentCommandlet, Warning, "[REPORT] %ls is not synced to head, can not submit", *Filename);
			}
		}
		else if (!SourceControlState->CanCheckout())
		{
			...
			if (SourceControlState->IsCheckedOutOther(&CurrentlyCheckedOutUser))
			{
				if (!bIgnoreAlreadyCheckedOut)
				{
					UE_LOGF(LogContentCommandlet, Error, "[REPORT] %ls level is already checked out by someone else (%ls), can not submit!", *Filename, *CurrentlyCheckedOutUser);
				}
				else
				{
					UE_LOGF(LogContentCommandlet, Warning, "[REPORT] %ls level is already checked out by someone else (%ls), can not submit!", *Filename, *CurrentlyCheckedOutUser);
				}
			}
			else
			{
				UE_LOGF(LogContentCommandlet, Error, "[REPORT] Unable to checkout %ls, can not submit", *Filename);
			}
		}
		else 
		{
			...
			{
				UE_LOGF(LogContentCommandlet, Display, "[REPORT] %ls Checked out successfully", *Filename);
				return true;
			}
			else
			{
				UE_LOGF(LogContentCommandlet, Warning, "[REPORT] %ls could not be checked out!", *Filename);
			}
		}
	}
	return false;
...
bool UResavePackagesCommandlet::RevertFile(const FString& Filename)
{
	...
	if (SourceControlState.IsValid())
	{
		if (SourceControlState->CanRevert() && (SourceControlProvider.Execute(ISourceControlOperation::Create<FRevert>(), *Filename) == ECommandResult::Succeeded))
		{
			...
			UE_LOGF(LogContentCommandlet, Display, "[REPORT] %ls Reverted successfully", *Filename);			
		}
		else
		{
			UE_LOGF(LogContentCommandlet, Warning, "[REPORT] %ls could not be reverted!", *Filename);
		}
	}
	...
	if (FPackageName::TryConvertLongPackageNameToFilename(Package->GetName(), PackageFilename, Package->ContainsMap() ? FPackageName::GetMapPackageExtension() : FPackageName::GetAssetPackageExtension()))
	{
		if (IFileManager::Get().FileExists(*PackageFilename))
		{
			if (CheckoutFile(PackageFilename, true, bIgnoreAlreadyCheckedOut))
			{
				...
				{
					UE_LOGF(LogContentCommandlet, Error, "Failed to save existing package %ls", *PackageFilename);
				}
			}
			else
			{
				UE_LOGF(LogContentCommandlet, Error, "Failed to check out existing package %ls", *PackageFilename);
			}
		}
		else
		{
			if (SavePackageHelper(Package, PackageFilename))
			{
				...
				{
					UE_LOGF(LogContentCommandlet, Error, "Failed to check out new package %ls", *PackageFilename);
				}
			}
			else
			{
				UE_LOGF(LogContentCommandlet, Error, "Failed to save new package %ls", *PackageFilename);
			}
		}
	}
}
...
void UResavePackagesCommandlet::CheckInFiles(const TArray<FString>& InFilesToSubmit, const FText& InDescription) const
{
	...
	if (!bSkipVirtualization)
	{
		...
		{
			UE_LOGF(LogContentCommandlet, Error, "Files will not be checked in due to virtualization failure!");
			return;
		}
	}
	else
	{
		UE_LOGF(LogContentCommandlet, Display, "Skipping virtualization due to the cmdline");
	}
...
bool UResavePackagesCommandlet::TryVirtualization(const TArray<FString>& FilesToSubmit, FText& InOutDescription)
{
	...
	else if (System.AllowSubmitIfVirtualizationFailed())
	{
		...
		{
			UE_LOGF(LogContentCommandlet, Warning, "%ls", *Error.ToString());
		}
	...
	else
	{
		for (const FText& Error : Result.Errors)
		{
			UE_LOGF(LogContentCommandlet, Error, "%ls", *Error.ToString());
		}

		UE_LOGF(LogContentCommandlet, Error, "Failed to virtualize the files being submitted!");
...
void UResavePackagesCommandlet::PerformAdditionalOperations(class UWorld* World, bool& bSavePackage)
{
	...
	if (bBuildingNonHLODData || bShouldBuildHLOD || bShouldBuildNavigationData)
	{
			...
			for (ULevelStreaming* StreamingLevel : StreamingLevels)
			{
				...
				if (FPackageName::DoesPackageExist(StreamingLevelWorldAssetPackageName, &StreamingLevelPackageFilename))
				{
					if (!bShouldCheckoutDirtyPackageOnly)
					{
						...
						{
							UE_LOGF(LogContentCommandlet, Error, "[REPORT] %ls is currently already checked out, cannot continue resaving", *StreamingLevelPackageFilename);
							bShouldProceedWithRebuild = false;
		...
		if (bShouldProceedWithRebuild)
		{
			...
			if (bShouldBuildHLOD)
			{
				UE_LOGF( LogContentCommandlet, Display, "Generating HLOD data for %ls", *World->GetOutermost()->GetName() );
				...
				if( !ForceHLODSetupAsset.IsEmpty() )
				{
					...
					{
						UE_LOGF( LogContentCommandlet, Fatal, "Could not find HLOD Setup Asset specified with the -ForceHLODSetupAsset option: %ls", *ForceHLODSetupAsset );
					}
				}
			...
			if (bShouldBuildNavigationData)
			{
				...
				UE_LOGF(LogContentCommandlet, Display, "Building navigation data for %ls", *World->GetOutermost()->GetName());
			...
			if (bShouldBuildLighting)
			{
				...
				auto BuildFailedDelegate = [&bShouldProceedWithRebuild,&World]() {
					UE_LOGF(LogContentCommandlet, Error, "[REPORT] Failed building lighting for %ls", *World->GetOutermost()->GetName());
					bShouldProceedWithRebuild = false;
				};
		...
		{
			UE_LOGF(LogContentCommandlet, Error, "[REPORT] Failed to complete steps necessary to perform build for %ls", *World->GetName());
		}
			...
			if (bShouldCheckoutDirtyPackageOnly)
			{
				...
				if (bSavePackage && bSkipCheckedOutFiles)
				{
					...
					if (!bSavePackage)
					{
						if (OutUser.Len())
						{
							UE_LOGF(LogContentCommandlet, Warning, "[REPORT] Skipping %ls as it is checked out by %ls", *WorldPackageName, *OutUser);
						}
						else
						{
							UE_LOGF(LogContentCommandlet, Warning, "[REPORT] Skipping %ls as it could not be checked out (not at head revision ?)", *WorldPackageName);
						}
					}
				}
			}
...
{
	if (bOnlyMaterials && ForceMaterialAutoUsageFlag.IsSet())
	{
		if (UMaterial* Material = Cast<UMaterial>(Object); 
			(Material != nullptr) && (Material->bAutomaticallySetUsageInEditor != *ForceMaterialAutoUsageFlag))
		{
			Material->MarkPackageDirty();
			Material->bAutomaticallySetUsageInEditor = *ForceMaterialAutoUsageFlag;
			bSavePackage = true;
		}
	}
}
...
bool UResavePackagesCommandlet::CleanClassesFromContentPackages( UPackage* Package )
{
	...
	for ( TObjectIterator<UClass> It; It; ++It )
	{
		if ( It->IsIn(Package) )
		{
			UE_LOGF(LogContentCommandlet, Warning, "Removing class '%ls' from package [%ls]", *It->GetPathName(), *Package->GetName());
...
void UResavePackagesCommandlet::VerboseMessage(const FString& Message)
{
	if (Verbosity == VERY_VERBOSE)
	{
		UE_LOGF(LogContentCommandlet, Verbose, "%ls", *Message);
	}
}
...
void UResavePackagesCommandlet::CleanTempFiles()
{
	...
	UE_LOGF(LogContentCommandlet, Display, "Cleaning temp file directory");

	if (!IFileManager::Get().DeleteDirectory(*DirPath, false, true))
	{
		UE_LOGF(LogContentCommandlet, Warning, "Failed to clean temp file directory:  %ls", *DirPath);
	}
}
...
int32 UWrangleContentCommandlet::Main( const FString& Params )
{
	...
	UE_LOGF(LogContentCommandlet, Warning, "Keeping platform-specific data for ALL platforms");
	...
	if (bShouldRestoreFromPreviousRun)
	{
		...
		{
			UE_LOGF(LogContentCommandlet, Warning, "Could not read in Wrangle.bin so not restoring and doing a full wrangle" );
		}
	}
	else
	{
		...
		{
			UE_LOGF(LogContentCommandlet, Error, "This commandlet needs a WrangleContent.ini in the Config directory with at least a [WrangleContent.PackagesToFullyLoad] section or a [WragnelContent.CollectionsToFullyLoad] section.");
			return 1;
		}
...
//			if (Object->HasAnyFlags(RF_Public))
		...
		if (CollectionsToFullyLoadSection)
		{
			...
			for (FConfigSectionMap::TConstIterator CollectionIt(*CollectionsToFullyLoadSection); CollectionIt; ++CollectionIt)
			{
				...
				if (!CollectionManager.TryParseCollectionPath(Collection, &CollectionContainer, &CollectionName, &ShareType) ||
					!CollectionContainer->GetAssetsInCollection(CollectionName, ShareType, CollectionAssets))
				{
					UE_LOGF(LogContentCommandlet, Warning, "Could not get assets in collection '%ls'. Skipping filter.", *Collection);
				}
				else
				{
					//insert all of the collection names into the set for fast filter checks
		...
		for (FConfigSectionMap::TIterator PackageIt(PackagesToFullyLoad); PackageIt; ++PackageIt)
		{
			...
			for (int32 PackageIndex = 0; PackageIndex < PackagesToLoad.Num(); PackageIndex++)
			{
				...
				UE_LOGF(LogContentCommandlet, Warning, "Fully loading %ls... [%d/%d]", *PackageFilename, PackageIndex + 1, PackagesToLoad.Num());
				CLEAR_WARN_COLOR();
	...
	// or make sure that the following redirects marks the package dirty (which maybe it shouldn't do in the editor?)
				...
				for(FThreadSafeObjectIterator ObjectIt; ObjectIt; ++ObjectIt)
				{
					...
					if (/*Object->HasAnyFlags(RF_Public) &&*/ Object->GetOuter() != NULL)
					{
						...
						{
							UE_LOGF(LogContentCommandlet, Warning, "Clearing public flag on map object %ls", *Object->GetFullName());
				...
				if (bShouldSavePackages && !bIsScriptPackage && !bForceSkipSavePackage)
				{
					...
					if (Package->IsDirty())
					{
						...
						UE_LOGF(LogContentCommandlet, Warning, "Saving fully loaded package %ls...", *CutdownPackageName);
						if (!SavePackageHelper(Package, CutdownPackageName))
						{
							UE_LOGF(LogContentCommandlet, Error, "Failed to save package %ls...", *CutdownPackageName);
						}
					}
					else
					{
						UE_LOGF(LogContentCommandlet, Warning, "Copying fully loaded package %ls...", *CutdownPackageName);
						// copy the unmodified file (faster than saving) (0 is success)
						if (IFileManager::Get().Copy(*CutdownPackageName, *PackageFilename) != 0)
						{
							UE_LOGF(LogContentCommandlet, Error, "Failed to copy package to %ls...", *CutdownPackageName);
						}
					}
				}
	...
	if (bShouldDumpUnreferencedContent || bShouldSaveUnreferencedContent)
	{
		SET_WARN_COLOR(COLOR_WHITE);
		UE_LOGF(LogContentCommandlet, Warning, "");
		UE_LOGF(LogContentCommandlet, Warning, "Looking for unreferenced objects:");
		CLEAR_WARN_COLOR();
		...
		for (int32 PackageIndex = 0; PackageIndex < AllPackages.Num(); PackageIndex++)
		{
			...
			{
				UE_LOGF(LogContentCommandlet, Warning, "Skipping unmounted path %ls...", *AllPackages[PackageIndex]);
				continue;
			}
			...
			{
				UE_LOGF(LogContentCommandlet, Warning, "Skipping map %ls...", *PackagePath.GetDebugName());
				continue;
			}
			else
			{
				...
				if (PackageObjs == NULL)
				{
					UE_LOGF(LogContentCommandlet, Warning, "No objects in %ls were referenced...", *PackagePath.GetDebugName());
				...
				{
					UE_LOGF(LogContentCommandlet, Warning, "Skipping fully loaded package %ls...", *PackagePath.GetDebugName());
					continue;
				}
				else
				{
					UE_LOGF(LogContentCommandlet, Warning, "Scanning %ls...", *PackagePath.GetDebugName());
				}
			}

			// Load the package before accessing its linker to prevent interference with async loads in case
			// that package was already queued and being loaded asynchronously.
			LoadPackage(nullptr, PackagePath, LOAD_Quiet | LOAD_NoWarn | LOAD_NoVerify);
			...
			for (int32 ExportIndex = 0; ExportIndex < Linker->ExportMap.Num(); ExportIndex++)
			{
				...
				{
					UE_LOGF(LogContentCommandlet, Warning, "    Export.ObjectName == NAME_None  for Package: %ls ", *PackagePath.GetDebugName());
					continue;
				}
				...
				if (bAreAllObjectsUnnecessary || PackageObjs->ReferencedObjects.Find(ExportName) == NULL)
				{
					...
					if ( !Class )
					{
						UE_LOGF(LogContentCommandlet, Warning, "Missing class %ls", *ClassName);
	...
	if (bShouldSavePackages)
	{
		...
		SET_WARN_COLOR(COLOR_WHITE);
		UE_LOGF(LogContentCommandlet, Warning, "");
		UE_LOGF(LogContentCommandlet, Warning, "Saving referenced objects in %d Packages:", NumPackages);
		CLEAR_WARN_COLOR();
		...
		for (TMap<FString, FPackageObjects>::TIterator It(AllReferencedPublicObjects); It; ++It, PackageIndex++ )
		{
			...
			if (Package)
			{
				...
				//UE_LOGF(LogContentCommandlet, Warning, "*It.Key(): %ls", *It.Key() );
				...
				{
					UE_LOGF(LogContentCommandlet, Fatal, "Could not find file in file cache: %ls", *PackageName);
				}
				...
				UE_LOGF(LogContentCommandlet, Warning, "Saving %ls... [%d/%d]", *CutdownPackageName, PackageIndex + 1, NumPackages);
	...
	if (bShouldDumpUnreferencedContent)
	{
		SET_WARN_COLOR(COLOR_WHITE);
		UE_LOGF(LogContentCommandlet, Warning, "");
		UE_LOGF(LogContentCommandlet, Warning, "Unreferenced Public Objects:");
		CLEAR_WARN_COLOR();
		...
		if (!CSVFile)
		{
			UE_LOGF(LogContentCommandlet, Error, "Failed to open output file %ls", *CSVFilename);
		}
		...
		for (int32 ObjectIndex = 0; ObjectIndex < UnnecessaryPublicObjects.Num(); ObjectIndex++)
		{
			...
			UE_LOGF(LogContentCommandlet, Warning, "%ls", *Object.ObjectName);
	...
	if (bShouldSaveUnreferencedContent)
	{
		...
		SET_WARN_COLOR(COLOR_WHITE);
		UE_LOGF(LogContentCommandlet, Warning, "");
		UE_LOGF(LogContentCommandlet, Warning, "Saving unreferenced objects [%d packages]:", NumPackages);
		CLEAR_WARN_COLOR();
		...
		for (TMap<FString, FPackageObjects>::TIterator PackageIt(UnnecessaryObjectsByPackage); PackageIt; ++PackageIt, PackageIndex++)
		{
			...
			UE_LOGF(LogContentCommandlet, Warning, "Processing %ls", *PackageWithUnnecessaryObjects);
			...
			else
			{
				...
				for (TMap<FString, UClass*>::TIterator ObjectIt(PackageIt.Value().ReferencedObjects); ObjectIt; ++ObjectIt)
				{
					...
					if (!Object)
					{
						UE_LOGF(LogContentCommandlet, Error, "Failed to load object %ls, it will be deleted permanently!", *ObjectPathName);
					}
				}
			}
				...
				if (UnnecessaryObjects.Find(*It->GetFullName()))
				{
						...
						UnnecessaryObject->GetOuter()->IsA(UPackage::StaticClass()))
					{
						...
						if (AssetName.Left(4) != TEXT("NFS_"))
						{
							...
							//UE_LOGF(LogContentCommandlet, Warning, "Renaming object from %ls to %ls.%ls", *UnnecessaryObject->GetPathName(), *NewPackage->GetPathName(), *UnnecessaryObject->GetName());
			...
			{
				UE_LOGF(LogContentCommandlet, Warning, "Saving package %ls [%d/%d]", *MovedFilename, PackageIndex, NumPackages);
				SavePackageHelper(MovedPackage, *MovedFilename);
			}
			else
			{
				UE_LOGF(LogContentCommandlet, Warning, "Moved package %ls not found.", *FindPackageName);
				UE_LOGF(LogContentCommandlet, Error, "Can't save package %ls [%d/%d]", *MovedFilename, PackageIndex, NumPackages);
			}
			CollectGarbage(RF_NoFlags);
		}
	}
		...
		if (Emitter && Emitter->LODLevels.Num() > 0)
		{
				...
				LODLevel->TypeDataModule->IsA( UParticleModuleTypeDataMesh::StaticClass())) // The type data module is mesh type data
			{
					...
					if(LODLevel->Modules[ModuleIdx]->IsA(UParticleModuleMeshMaterial::StaticClass()))
					{
						...
						for(int32 MatIdx = 0; MatIdx < MaterialModule->MeshMaterials.Num(); MatIdx++ )
						{
							if(MaterialModule->MeshMaterials[MatIdx])
							{
								...
								if(!MaterialModule->MeshMaterials[MatIdx]->GetUsageByFlag(MATUSAGE_MeshParticles))
								{
				...
				if(!bFoundMaterials && MeshTypeData->bOverrideMaterial)
				{
					...
					if(OverrideMaterial && !OverrideMaterial->GetUsageByFlag(MATUSAGE_MeshParticles))
					{
						OutMaterials.AddUnique(OverrideMaterial->GetPathName());
					}
				}
				...
				else if (!bFoundMaterials)
				{
					if (MeshTypeData->Mesh)
					{
						...
						{
							if(UMaterialInterface* Mat = MeshTypeData->Mesh->GetStaticMaterials()[MaterialIdx].MaterialInterface)
							{
								if(!Mat->GetUsageByFlag(MATUSAGE_MeshParticles))
								{
...
int32 UListMaterialsUsedWithMeshEmittersCommandlet::Main( const FString& Params )
{
	...
	{
		UE_LOGF(LogContentCommandlet, Warning,"No packages found");
		return 1;
	}
	...
	UE_LOGF(LogContentCommandlet, Display, "Searching Asset Registry for particle systems");
	...
	for(int32 AssetIdx = 0; AssetIdx < AssetList.Num(); ++AssetIdx )
	{
		...
		UE_LOGF(LogContentCommandlet, Display, "Processing particle system (%i/%i):  %ls ", AssetIdx, AssetList.Num(), *Filename );

		UPackage* Package = LoadPackage( NULL, *Filename, LOAD_Quiet );
		if ( Package == NULL )
		{
			UE_LOGF(LogContentCommandlet, Error, "Error loading %ls!", *Filename );
			continue;
		}
	...
	if(MaterialList.Num() > 0)
	{
		...
		UE_LOGF(LogContentCommandlet, Display, "-------------------------------------------------------------------");
		UE_LOGF(LogContentCommandlet, Display, "The following materials require bUsedWithMeshParticles to be enabled:");
		for(int32 Index = 0; Index < MaterialList.Num(); ++Index)
		{
			UE_LOGF(LogContentCommandlet, Error, "%ls", *MaterialList[Index] );
		}
		UE_LOGF(LogContentCommandlet, Display, "-------------------------------------------------------------------");
	}
	else
	{
		UE_LOGF(LogContentCommandlet, Display, "No materials require updating!");
	}
	return 0;
}
...
int32 UListStaticMeshesImportedFromSpeedTreesCommandlet::Main(const FString& Params)
{
	...
	// Update Registry Module
	UE_LOGF(LogContentCommandlet, Display, "Searching Asset Registry for static mesh ");
	AssetRegistryModule.Get().SearchAllAssets(true);
	...
	for (int32 AssetIdx = 0; AssetIdx < AssetList.Num(); ++AssetIdx)
	{
		...
		UE_LOGF(LogContentCommandlet, Display, "Processing static mesh (%i/%i):  %ls ", AssetIdx, AssetList.Num(), *Filename);
	...
	if (StaticMeshList.Num() > 0)
	{
		// Now, dump out the list of materials that require updating.
		UE_LOGF(LogContentCommandlet, Display, "-------------------------------------------------------------------");
		UE_LOGF(LogContentCommandlet, Display, "The following static meshes were imported from SpeedTrees:");
		for (int32 Index = 0; Index < StaticMeshList.Num(); ++Index)
		{
			UE_LOGF(LogContentCommandlet, Error, "%ls", *StaticMeshList[Index]);
		}
		UE_LOGF(LogContentCommandlet, Display, "-------------------------------------------------------------------");
	}
	else
	{
		UE_LOGF(LogContentCommandlet, Display, "No static meshes were imported from speedtrees in this project.");
	}
	return 0;
}
...
int32 UStaticMeshMinLodCommandlet::Main(const FString& Params)
{
	...
	if (FParse::Value(*Params, TEXT("-mapping="), MappingStr, false))
	{
		...
		for (FString& PlatformToQualityLevel : Mappings)
		{
			...
			if (Entries.Num() != 2)
			{
				UE_LOGF(LogContentCommandlet, Error, "Error bad -mapping argument: %ls", *MappingStr);
				return 1;
			}
	...
	for (const TPair<FName, OverrideGroupInfo>& Pair : MinLodStats)
	{
		...
		UE_LOGF(LogContentCommandlet, Display, "-------------------------------------------------------------------");
		UE_LOGF(LogContentCommandlet, Display, "Mask: %ls", *Pair.Key.ToString());
		UE_LOGF(LogContentCommandlet, Display, "Nb overrides: %d", Info.Count);

		for (int i = 0; i < Info.Names.Num(); i++)
		{
			UE_LOGF(LogContentCommandlet, Display, "	%ls", *Info.Names[i]);
	...
	UE_LOGF(LogContentCommandlet, Display, "-------------------------------------------------------------------");
	UE_LOGF(LogContentCommandlet, Display, "Total overrides: %d", TotalOverrideCount);
	UE_LOGF(LogContentCommandlet, Display, "Total ovestatic meshesrrides: %d", TotalStaticMeshesChecked);
	UE_LOGF(LogContentCommandlet, Display, "-------------------------------------------------------------------");
		...
		else
		{
			for (UPackage* Package : PackagesToSave)
			{
				...
				if (IPlatformFile::GetPlatformPhysical().FileExists(*PackageFilename))
				{
					...
					{
						UE_LOGF(LogContentCommandlet, Error, "Error setting %ls writable", *PackageFilename);
						return 1;
					}
				}
			}
...
int32 ULandscapeGrassTypeCommandlet::Main(const FString& Params)
{
	...
	{
		UE_LOGF(LogContentCommandlet, Warning, "No Mapping rules found");
		return 1;
	}
