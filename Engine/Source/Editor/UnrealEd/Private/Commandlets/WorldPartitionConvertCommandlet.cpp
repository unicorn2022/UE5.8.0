			...
			else if(Obj->IsInPackage(RootPackage) && !Obj->HasAnyFlags(RF_Standalone))
			{
				if(!Obj->GetTypedOuter<AActor>())
				{
					...
					if(OriginalRoot && (*OriginalRoot != Root))
					{
						...
						UE_LOGF(LogWorldPartitionConvertCommandlet, Error, "Duplicate reference %ls.%ls(%ls) (first referenced by %ls)", *Root->GetName(), *Obj->GetName(), *Obj->GetClass()->GetName(), *(*OriginalRoot)->GetName());
						CLEAR_WARN_COLOR();
					}
					else if(!OriginalRoot)
					{
						...
						else if(!Obj->IsA<ULevel>())
						{
							if(!CurrentObject || !Obj->IsIn(CurrentObject))
							{
								...
								UE_LOGF(LogWorldPartitionConvertCommandlet, Warning, "Encountered actor %ls referencing %ls (%ls)", *Root->GetName(), *Obj->GetPathName(), *Obj->GetClass()->GetName());
								CLEAR_WARN_COLOR();
							}
...
UWorld* UWorldPartitionConvertCommandlet::LoadWorld(const FString& LevelToLoad)
{
	...
	SET_WARN_COLOR(COLOR_WHITE);
	UE_LOGF(LogWorldPartitionConvertCommandlet, Log, "Loading level %ls.", *LevelToLoad);
	CLEAR_WARN_COLOR();

	UPackage* MapPackage = LoadPackage(nullptr, *LevelToLoad, LOAD_None);
	if (!MapPackage)
	{
		UE_LOGF(LogWorldPartitionConvertCommandlet, Error, "Error loading %ls.", *LevelToLoad);
		return nullptr;
	}
...
ULevel* UWorldPartitionConvertCommandlet::InitWorld(UWorld* World)
{
	...
	SET_WARN_COLOR(COLOR_WHITE);
	UE_LOGF(LogWorldPartitionConvertCommandlet, Log, "Initializing level %ls.", *World->GetName());
	CLEAR_WARN_COLOR();
...
void UWorldPartitionConvertCommandlet::GatherAndPrepareSubLevelsToConvert(ULevel* Level, TArray<ULevel*>& SubLevels)
{
	...
	for (ULevelStreaming* StreamingLevel : World->GetStreamingLevels())
	{
		...
		{
			UE_LOGF(LogWorldPartitionConvertCommandlet, Log, "Skipping conversion of streaming Level %ls", *StreamingLevel->GetWorldAssetPackageName());
		}
	}
...
bool UWorldPartitionConvertCommandlet::PrepareStreamingLevelForConversion(ULevelStreaming* StreamingLevel)
{
	...
	if (bOnlyMergeSubLevels || StreamingLevel->ShouldBeAlwaysLoaded() || StreamingLevel->bDisableDistanceStreaming)
	{
		...
		UE_LOGF(LogWorldPartitionConvertCommandlet, Log, "Converting %ls streaming level %ls", StreamingLevel->bDisableDistanceStreaming ? TEXT("non distance-based") : TEXT("always loaded"), *StreamingLevel->GetWorldAssetPackageName());
...
{
	UE_LOGF(LogWorldPartitionConvertCommandlet, Display, "WorldPartitionConvertCommandlet report:");

	auto OutputReport = [](const TCHAR* Msg, const TSet<FString>& Values)
	{
		if (Values.Num() != 0)
		{
			UE_LOGF(LogWorldPartitionConvertCommandlet, Display, "- Found %ls:", Msg);
			TArray<FString> Array = Values.Array();
			Array.Sort();
			for (const FString& Name : Array)
			{
				UE_LOGF(LogWorldPartitionConvertCommandlet, Display, "  * %ls", *Name);
			}
			UE_LOGF(LogWorldPartitionConvertCommandlet, Display, "");
		}	
	};
	...
	struct FSoftPathFixupSerializer : public FArchiveUObject
	{
		...
		FArchive& operator<<(FSoftObjectPath& Value)
		{
			...
			if (!Value.IsNull())
			{
				...
				{
					UE_LOGF(LogWorldPartitionConvertCommandlet, Verbose, "Remapped SoftObjectPath %ls to %ls", *OriginalValue, *NewValue);
					UE_LOGF(LogWorldPartitionConvertCommandlet, Verbose, "  Source: %ls", *GetSourceString());
				}
			}
...
bool UWorldPartitionConvertCommandlet::RenameWorldPackageWithSuffix(UWorld* World)
{
	...
	bRenamedSuccess = Package->Rename(*NewPackageName, nullptr, REN_NonTransactional | REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
	if (!bRenamedSuccess)
	{
		UE_LOGF(LogWorldPartitionConvertCommandlet, Error, "Unable to rename package to %ls", *NewPackageName);
		return false;
	}
	...
	bRenamedSuccess = World->Rename(*NewWorldName, nullptr, REN_NonTransactional | REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
	if (!bRenamedSuccess)
	{
		UE_LOGF(LogWorldPartitionConvertCommandlet, Error, "Unable to rename world to %ls", *NewWorldName);
		return false;
	}
...
void UWorldPartitionConvertCommandlet::SetupHLODLayerAssets()
{
	...
	for (const FHLODLayerActorMapping& Entry : HLODLayersForActorClasses)
	{
		...
		if (!HLODLayer)
		{
			UE_LOGF(LogWorldPartitionConvertCommandlet, Warning, "Unable to load HLOD Layer %ls, skipping assignment to class %ls", *Entry.HLODLayer.ToString(), *Entry.ActorClass.ToString());
			continue;
		}
...
int32 UWorldPartitionConvertCommandlet::Main(const FString& Params)
{
	...
	if (!Tokens.Num())
	{
		UE_LOGF(LogWorldPartitionConvertCommandlet, Error, "missing map name");
		return 1;
	}
	else if (Tokens.Num() > 1)
	{
		...
		UE_LOGF(LogWorldPartitionConvertCommandlet, Error, "extra parameters %ls", *BadParams);
		return 1;
	}
	...
	{
		UE_LOGF(LogWorldPartitionConvertCommandlet, Error, "Unknown level '%ls'", *Tokens[0]);
		return 1;
	}
	...
	{
		UE_LOGF(LogWorldPartitionConvertCommandlet, Error, "The option \"-AllowCommandletRendering\" is required.");
		return 1;
	}
	...
	if (!EditorHashClass)
	{
		UE_LOGF(LogWorldPartitionConvertCommandlet, Error, "Missing or invalid editor hash class");
		return 1;
	}

	if (!RuntimeHashClass)
	{
		UE_LOGF(LogWorldPartitionConvertCommandlet, Error, "Missing or invalid runtime hash class");
		return 1;
	}
	...
	if (!MainWorld)
	{
		UE_LOGF(LogWorldPartitionConvertCommandlet, Error, "Unknown world '%ls'", *Tokens[0]);
		return 1;
	}
	...
	if (!bReportOnly)
	{
		...
		if (TArray<FAssetData> EntityAssets = UAssetRegistryHelpers::GetAssetsWithOuterForPaths({ *OldLevelName }, *OldLevelName, OuterForPathsFlags); !EntityAssets.IsEmpty())
		{
			for (const FAssetData& EntityAsset : EntityAssets)
			{
				...
				{
					UE_LOGF(LogWorldPartitionConvertCommandlet, Error, "Failed to delete previous conversion package %ls", *EntityAsset.PackageName.ToString());
					return 1;
				}
			}
		}
		...
		{
			UE_LOGF(LogWorldPartitionConvertCommandlet, Error, "Failed to delete previous conversion package(s)");
			return 1;
		}
		...
		if (FPackageName::SearchForPackageOnDisk(OldLevelName, &OldLevelName))
		{
			...
			{
				UE_LOGF(LogWorldPartitionConvertCommandlet, Error, "Failed to delete previously converted level '%ls'", *Tokens[0]);
				return 1;
			}
		}
	}
	...
	{
		UE_LOGF(LogWorldPartitionConvertCommandlet, Error, "Level '%ls' is already partitioned", *Tokens[0]);
		return 1;
	}
	...
	auto PartitionFoliage = [this, MainWorld](AInstancedFoliageActor* IFA) -> bool
	{
		...
		bool bAddFoliageSucceeded = IFA->ForEachFoliageInfo([IFA, &FoliageToAdd, &NumInstances, this](UFoliageType* FoliageType, FFoliageInfo& FoliageInfo) -> bool
		{
			...
			if (FoliageType->GetTypedOuter<AInstancedFoliageActor>() != nullptr)
			{
				...
				{
					UE_LOGF(LogWorldPartitionConvertCommandlet, Error,
						   "Level contains embedded FoliageType settings: please save the FoliageType setting assets, " "use the SaveFoliageTypeToContentFolder switch, " "specify FoliageTypePath in configuration file or the commandline.");
					return false;
				}
			...
			if (FoliageInfo.Instances.Num() > 0)
			{
				...
				UE_LOGF(LogWorldPartitionConvertCommandlet, Display, "FoliageType: %ls Count: %d", *FoliageTypeToAdd->GetName(), FoliageInfo.Instances.Num());
			}
	...
	if (!bSkipStableGUIDValidation)
	{
		...
		for (ULevel* Level: SubLevelsToConvert)
		{
			if (!Level->bContainsStableActorGUIDs)
			{
				...
				UE_LOGF(LogWorldPartitionConvertCommandlet, Error, "Unable to convert level '%ls' with non-stable actor GUIDs. Resave the level before converting.", *Level->GetPackage()->GetName());
			}
		}
	...
	for(ULevel* SubLevel : SubLevelsToConvert)
	{
		...
		UE_LOGF(LogWorldPartitionConvertCommandlet, Log, "Converting %ls", *SubWorld->GetName());
		...
		for(AActor* Actor: ActorsToConvert)
		{
			if(IsValid(Actor))
			{
				...
				if (!ActorClass->IsNative() && (ActorClass->GetPackage() == SubPackage))
				{
					...
					UE_LOGF(LogWorldPartitionConvertCommandlet, Log, "Extracted non-native class %ls", *ActorClass->GetName());
				}
		...
		if (!LevelHasLevelScriptBlueprint(SubLevel))
		{
			if (!bReportOnly)
			{
				...
				for(UObject* ObjectToRename: ObjectsToRename)
				{
					...
					UE_LOGF(LogWorldPartitionConvertCommandlet, Warning, "Renamed orphan object %ls", *ObjectToRename->GetName());
				}
	...
	if (!bReportOnly)
	{
		...
		for(AActor* Actor: ActorList)
		{
			...
			if (bAlreadySet)
			{
				UE_LOGF(LogWorldPartitionConvertCommandlet, Error, "Duplicated guid actor %ls(guid:%ls) can't extract actor", *Actor->GetName(), *Actor->GetActorGuid().ToString(EGuidFormats::Digits));
				return 1;
			}
			...
			{
				UE_LOGF(LogWorldPartitionConvertCommandlet, Error, "Failed to convert actor %ls folder to persistent folder.", *Actor->GetName());
			}
			
			UPackage* ActorPackage = Actor->GetExternalPackage();
			PackagesToSave.Add(ActorPackage);

			UE_LOGF(LogWorldPartitionConvertCommandlet, Log, "Extracted actor %ls(guid:%ls) in %ls", *Actor->GetName(), *Actor->GetActorGuid().ToString(EGuidFormats::Digits), *ActorPackage->GetName());
		}
		...
		if (bDeleteSourceLevels)
		{
			...
			{
				UE_LOGF(LogWorldPartitionConvertCommandlet, Error, "Failed to delete source level package(s)");
				return 1;
			}
		}
		...
		// Checkout packages
		{
			...
			UE_LOGF(LogWorldPartitionConvertCommandlet, Log, "Checking out %d packages.", PackagesToSave.Num());
			...
			{
				UE_LOGF(LogWorldPartitionConvertCommandlet, Error, "Failed to checkout package(s)");
				return 1;
			}
		}
		...
		for (TMap<UObject*, AActor*>::TConstIterator It(PrivateRefsMap); It; ++It)
		{
			...
			{
				UE_LOGF(LogWorldPartitionConvertCommandlet, Warning, "Changing object %ls package from %ls to actor external package %ls", *It->Key->GetName(), *It->Key->GetPackage()->GetName(), *It->Value->GetPackage()->GetName());
			...
			else
			{
				...
				if (!It->Key->IsInPackage(ActorPackage))
				{
					...
					if (UObject* RemappedOuterObject = RemappedOuterPath ? FSoftObjectPath(*RemappedOuterPath).ResolveObject() : nullptr)
					{
						...
						UE_LOGF(LogWorldPartitionConvertCommandlet, Warning, "Renamed object from %ls to %ls", *OldPathName, *It->Key->GetPathName());
					}
					else
					{
						UE_LOGF(LogWorldPartitionConvertCommandlet, Error, "Failed to find a corresponding outer for object %ls.", *It->Key->GetPathName());
						bRemapError = true;
					}
				}
			}
		...
		// Save packages
		{
			...
			UE_LOGF(LogWorldPartitionConvertCommandlet, Log, "Saving %d packages.", PackagesToSave.Num());
		...
		UE_LOGF(LogWorldPartitionConvertCommandlet, Log, "######## CONVERSION COMPLETED SUCCESSFULLY ########");
	}
	...
	if (bGenerateIni || !bReportOnly)
	{
		if (bGenerateIni || !FPlatformFileManager::Get().GetPlatformFile().FileExists(*LevelConfigFilename))
		{
			...
			UE_LOGF(LogWorldPartitionConvertCommandlet, Display, "Generated ini file: %ls", *LevelConfigFilename);
		}
	}
