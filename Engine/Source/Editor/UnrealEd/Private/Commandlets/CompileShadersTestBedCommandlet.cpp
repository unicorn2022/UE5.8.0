...
int32 UCompileShadersTestBedCommandlet::Main(const FString& Params)
{
	...
	{
		UE_LOGF(LogCompileShadersTestBedCommandlet, Display, "CompileShadersTestBed");
		UE_LOGF(LogCompileShadersTestBedCommandlet, Display, "This commandlet compiles global and default material shaders.  Used to profile and test shader compilation.");
		UE_LOGF(LogCompileShadersTestBedCommandlet, Display, " Optional: -collection=<name>                (You can also specify a collection of assets to narrow down the results e.g. if you maintain a collection that represents the actually used in-game assets).");
		UE_LOGF(LogCompileShadersTestBedCommandlet, Display, " Optional: -materials=<path1>+<path2>        (You can also specify a list of material asset paths separated by a '+' to narrow down the results.)");
		UE_LOGF(LogCompileShadersTestBedCommandlet, Display, " Optional: -all                              (You can specify -all to compile all global/default shaders as well as shaders for all materials/material instances found in a project.)")
		UE_LOGF(LogCompileShadersTestBedCommandlet, Display, " Optional: -ExcludeGlobalShaders             (Skip the compilation of global shaders.)")
		UE_LOGF(LogCompileShadersTestBedCommandlet, Display, " Optional: -ExcludeDefaultMaterials          (Skip the compilation of default material shaders.)")
		UE_LOGF(LogCompileShadersTestBedCommandlet, Display, " Optional: -ExcludeMaterials                 (Skip the compilation of non default material shaders.)")
		return 0;
	}
	...
	if (Switches.Contains(TEXT("all")))
	{
		...
		UE_LOGF(LogCompileShadersTestBedCommandlet, Display, "Found %d materials/material instances in project.", MaterialList.Num());
	}
	else // otherwise parse -collection and -materials arguments
	{
		...
		if (FParse::Value(*Params, TEXT("collection="), Collection, true))
		{
			if (!Collection.IsEmpty())
			{
				...
				AssetRegistry.GetAssets(Filter, MaterialList);
				UE_LOGF(LogCompileShadersTestBedCommandlet, Display, "Found %d materials/material instances from collection %ls.", MaterialList.Num(), *Collection);
			}
		}
		...
		if (CmdLineMaterialEntries.Num())
		{
			...
			AssetRegistry.GetAssets(Filter, MaterialList);
			UE_LOGF(LogCompileShadersTestBedCommandlet, Display, "Found %d/%d requested materials/material instances.", MaterialList.Num() - MaterialsNumBefore, Filter.SoftObjectPaths.Num());
			if ((MaterialList.Num() - MaterialsNumBefore) != CmdLineMaterialEntries.Num())
			{
				UE_LOGF(LogCompileShadersTestBedCommandlet, Error, "Some materials/material instances were not found.");
			}
		}
	}
	...
	UE_LOGF(LogCompileShadersTestBedCommandlet, Display, "Begin Compiling Shaders");

	for (ITargetPlatform* Platform : Platforms)
	{
		UE_LOGF(LogCompileShadersTestBedCommandlet, Display, "Compiling shaders for %ls...", *Platform->PlatformName());
		...
		if (!Switches.Contains(TEXT("ExcludeDefaultMaterials")))
		{
			...
			UE_LOGF(LogCompileShadersTestBedCommandlet, Display, "Compile default materials");
		...
		if (!Switches.Contains(TEXT("ExcludeGlobalShaders")))
		{
			...
			if (!DesiredShaderFormats.IsEmpty())
			{
				...
				UE_LOGF(LogCompileShadersTestBedCommandlet, Display, "Compile global shaders for %ls", *ShaderFormatListString);
			...
			{
				UE_LOGF(LogCompileShadersTestBedCommandlet, Display, "No target shader formats available to compile global shaders");
			}
		}
		...
		if (!Switches.Contains(TEXT("ExcludeMaterials")))
		{
			...
			UE_LOGF(LogCompileShadersTestBedCommandlet, Display, "Begin Cache For Cooked PlatformData");
			...
			for (const FAssetData& AssetData : MaterialList)
			{
				...
				{
					UE_LOGF(LogCompileShadersTestBedCommandlet, Display, "BeginCache for %ls", *MaterialInterface->GetFullName());
		...
		// Submit all the jobs.
		{
			...
			UE_LOGF(LogCompileShadersTestBedCommandlet, Display, "Submit Jobs");
			...
			while (MaterialsToCompile.Num())
			{
				for (auto It = MaterialsToCompile.CreateIterator(); It; ++It)
				{
					...
					{
						It.RemoveCurrent();
						UE_LOGF(LogCompileShadersTestBedCommandlet, Display, "Finished cache for %ls.", *MaterialInterface->GetFullName());
						UE_LOGF(LogCompileShadersTestBedCommandlet, Display, "Materials remaining: %d", MaterialsToCompile.Num());
					}
					...
					while (true)
					{
						...
						{
							UE_LOGF(LogCompileShadersTestBedCommandlet, Display, "Outstanding Jobs: %d", CurrentOutstandingJobs);
		...
		// Process the shader maps and save to the DDC.
		{
			...
			UE_LOGF(LogCompileShadersTestBedCommandlet, Log, "ProcessAsyncResults");
		...
		// Perform cleanup and clear cached data for cooking.
		{
			...
			UE_LOGF(LogCompileShadersTestBedCommandlet, Display, "Clear Cached Cooked Platform Data");
	...
	UE_LOGF(LogCompileShadersTestBedCommandlet, Display, "End compiling shaders");
