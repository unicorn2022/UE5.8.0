...
{
	FVertexSnappingArgs( const FPlane& InActorPlane, const FVector& InCurrentLocation, TNotNull<FEditorViewportClient*> InViewportClient, const FSceneView* InView, const FVector2D& InMousePosition, EAxisList::Type InCurrentAxis, bool bInDrawVertexHelpers )
	...
	TNotNull<FEditorViewportClient*> ViewportClient;
...
void FVertexSnappingImpl::GetPossibleSnapActors( const FBox& AllowedBox, FIntPoint MouseLocation, TNotNull<FEditorViewportClient*> ViewportClient, const FSceneView* View, EAxisList::Type CurrentAxis, const TSet< TWeakObjectPtr<AActor> >& ActorsToIgnore, TArray<FSnapActor>& OutActorsInBox )
{
...
bool FVertexSnappingImpl::SnapLocationToNearestVertex(FVector& Location, const FVector2D& MouseLocation, TNotNull<FEditorViewportClient*> ViewportClient,
	FVector& OutVertexNormal, bool bDrawVertexHelpers, const TSet<TWeakObjectPtr<AActor>>& ActorsToIgnore)
{
	bool bSnapped = false;
...
bool FVertexSnappingImpl::SnapDraggedActorsToNearestVertex( FVector& DragDelta, TNotNull<FEditorViewportClient*> ViewportClient )
{
...
{	
	...
	const FPlane ActorPlane = InArgs.ActorPlane;
	TNotNull<FEditorViewportClient*> ViewportClient = InArgs.ViewportClient;
...
bool FVertexSnappingImpl::SnapDragLocationToNearestVertex( const FVector& BaseLocation, FVector& DragDelta, TNotNull<FEditorViewportClient*> ViewportClient )
{
	int32 MouseX = ViewportClient->Viewport->GetMouseX();
	int32 MouseY = ViewportClient->Viewport->GetMouseY();
	FVector2D MousePosition = FVector2D( MouseX, MouseY ) ;
