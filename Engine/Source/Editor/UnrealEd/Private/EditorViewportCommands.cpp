...
void FEditorViewportCommands::RegisterCommands()
{
	...
	UI_COMMAND( LitWireframeMode, "Lit Wireframe View Mode", "Renders the scene with normal lighting and a wireframe overlay", EUserInterfaceActionType::RadioButton, FInputChord());
	...
	UI_COMMAND( GroupStreamingDeficitMode, "Streaming Deficit View Mode", "Renders the scene using streaming deficit visualization", EUserInterfaceActionType::RadioButton, FInputChord());
	UI_COMMAND( StreamingTextureDeficitMode, "Streaming Texture Deficit", "Colors primitives by texture streaming deficit (WantedMips vs ResidentMips)", EUserInterfaceActionType::RadioButton, FInputChord());
	UI_COMMAND( StreamingTextureResidencyMode, "Streaming Texture Residency", "Colors primitives by texture residency deficit (MaxLODs vs ResidentMips)", EUserInterfaceActionType::RadioButton, FInputChord());
	UI_COMMAND( StreamingMeshLODDeficitMode, "Streaming Mesh LOD Deficit", "Colors primitives by mesh LOD streaming deficit (WantedMips vs ResidentMips)", EUserInterfaceActionType::RadioButton, FInputChord());
	UI_COMMAND( StreamingMeshLODResidencyMode, "Streaming Mesh LOD Residency", "Colors primitives by mesh LOD residency deficit (MaxLODs vs ResidentLODs)", EUserInterfaceActionType::RadioButton, FInputChord());
	...
	UI_COMMAND( VisualizeMegaLightsMode, "MegaLights Visualization View Mode", "Visualizes MegaLights debug views.", EUserInterfaceActionType::RadioButton, FInputChord());
	...
	UI_COMMAND(AllChannelsMask, "RGB", "Show all color channels", EUserInterfaceActionType::RadioButton, FInputChord());
	UI_COMMAND(RedChannelMask, "Red", "Show only the red color channel", EUserInterfaceActionType::RadioButton, FInputChord());
	UI_COMMAND(GreenChannelMask, "Green", "Show only the green color channel", EUserInterfaceActionType::RadioButton, FInputChord());
	UI_COMMAND(BlueChannelMask, "Blue", "Show only the blue color channel", EUserInterfaceActionType::RadioButton, FInputChord());
	UI_COMMAND(AlphaChannelMask, "Alpha", "Show only the alpha color channel", EUserInterfaceActionType::RadioButton, FInputChord());
	...
	UI_COMMAND( IncreaseGizmoSize, "Increase Gizmo Size", "Increases the size of the translation gizmo.", EUserInterfaceActionType::Button, FInputChord(EKeys::Equals, EModifierKey::Shift) );
	UI_COMMAND( DecreaseGizmoSize, "Decrease Gizmo Size", "Decreases the size of the translation gizmo.", EUserInterfaceActionType::Button, FInputChord(EKeys::Hyphen, EModifierKey::Shift) );
	UI_COMMAND( ResetGizmoSize, "Reset Gizmo Size", "Resets the size of the translation gizmo to its default state.", EUserInterfaceActionType::Button, FInputChord(EKeys::Equals) );
	...
	UI_COMMAND( Snapping, "Snapping", "Enables or disables all snapping", EUserInterfaceActionType::ToggleButton, FInputChord() );
	...
	UI_COMMAND( CurrentTRSSnapToggle, "Toggle Current TRS Snap", "Temporarily enables or disables snapping for the current transform mode while held; quick-press to toggle permanently.", EUserInterfaceActionType::Button, FInputChord( EKeys::X ) );
	UI_COMMAND( SurfaceSnapping, "Surface Snapping", "Toggle whether objects snap to surfaces when you drag them.\nUse the submenu to set up the snapping behavior.", EUserInterfaceActionType::ToggleButton, FInputChord( EKeys::Z ) );
...
TSharedRef<SWidget> BuildViewModeOptionsMenu(TSharedPtr<FUICommandList> CommandList, EViewModeIndex ViewModeIndex, ERHIFeatureLevel::Type FeatureLevel, TMap<int32, FName>& ParamNameMap)
{
	...
	const UE::UnrealEd::FViewModeOptionsArgs ViewModeArgs
	{
		.ViewModeIndex = ViewModeIndex,
		.FeatureLevel = FeatureLevel,
		.ParamNameMap = ParamNameMap,
		.ViewportClient = nullptr // Use UE::UnrealEd::BuildViewModeOptionsMenu instead
	};
	BuildViewModeOptionsMenu(MenuBuilder, ViewModeArgs);
	return MenuBuilder.MakeWidget();
}

void UE::UnrealEd::BuildViewModeOptionsMenu(FMenuBuilder& MenuBuilder, const FViewModeOptionsArgs& InArgs)
{
	...
	InArgs.ParamNameMap.Reset();

	if (InArgs.ViewModeIndex == VMI_MeshUVDensityAccuracy)
	{
	...
	else if (InArgs.ViewModeIndex == VMI_MaterialTextureScaleAccuracy || InArgs.ViewModeIndex == VMI_RequiredTextureResolution)
	{
		...
		if (InArgs.ViewModeIndex == VMI_MaterialTextureScaleAccuracy)
		{
		...
		else // VMI_RequiredTextureResolution
		{
			for (const UMaterialInterface* MaterialInterface : SelectedMaterials)
			{
				AppendMaterialInfoToMenu(MaterialInterface, InArgs.FeatureLevel, MenuName, DataPerTextureIndex, DataPerTextureName);
			}
		}

		if (InArgs.ViewportClient.IsValid())
		{
			TSharedRef<FEditorViewportClient> ViewportClient = InArgs.ViewportClient.ToSharedRef();
			auto GetUIAction = [&ViewportClient](int32 InViewModeIndex)
			{
				return FUIAction(
					FExecuteAction::CreateSP( ViewportClient, &FEditorViewportClient::SetViewModeParam, InViewModeIndex ),
					FCanExecuteAction(),
					FIsActionChecked::CreateSP( ViewportClient, &FEditorViewportClient::IsViewModeParam, InViewModeIndex )
				);
			};
			...
			if (DataPerTextureName.Num() > 0 && DataPerTextureName.Num() < TEXSTREAM_MAX_NUM_TEXTURES_PER_MATERIAL)
			{
				...
				{ 
					...
					MenuBuilder.AddMenuEntry(FText::FromString(MaterialInfos.DisplayName), FText::FromString(ToolTipOverride), FSlateIcon(), GetUIAction(CommandIndex), NAME_None, EUserInterfaceActionType::RadioButton);
					
					InArgs.ParamNameMap.Add(CommandIndex, Ite.Value());
					++CommandIndex;
				}
			}
			else if (DataPerTextureIndex.Num() > 0) // Otherwise show the data per index, with extra info (also hidding entries with no info)
			{
				for (int32 TextureIndex = 0; TextureIndex < TEXSTREAM_MAX_NUM_TEXTURES_PER_MATERIAL; ++TextureIndex)
				{
					...
					if (TextureDataAtIndex && TextureDataAtIndex->Num() > 0)
					{
						...
						MenuBuilder.AddMenuEntry(FText::FromString(NameOverride), FText::FromString(ToolTipOverride), FSlateIcon(), GetUIAction(TextureIndex), NAME_None, EUserInterfaceActionType::RadioButton);
					}
				}
			}
			else // If nothing selected, just display a list of name
			{
				for (int32 TextureIndex = 0; TextureIndex < TEXSTREAM_MAX_NUM_TEXTURES_PER_MATERIAL; ++TextureIndex)
				{
					const FText Label = FText::FromString(FString::Printf(TEXT("%s %d"), *MenuName, TextureIndex));
					MenuBuilder.AddMenuEntry(Label, Label, FSlateIcon(), GetUIAction(TextureIndex), NAME_None, EUserInterfaceActionType::RadioButton);
				}
			}
		}
	}
