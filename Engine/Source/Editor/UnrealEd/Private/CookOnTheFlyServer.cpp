...
#include "Cooker/CookProfilingPrivate.h"
...
public:
	...
	virtual const ITargetPlatform* AddPlatform(FName PlatformName, bool& bOutAlreadyInitialized) override
	{
		...
		if (!TargetPlatform)
		{
			UE_LOGF(LogCook, Warning, "Trying to add invalid platform '%ls' on the fly", *PlatformName.ToString());
	...
	virtual void GetUnsolicitedFiles(const FName& PlatformName, const FString& Filename, const bool bIsCookable, TArray<FString>& OutUnsolicitedFiles) override
	{
		...
		if (!TargetPlatform)
		{
			UE_LOGF(LogCook, Warning, "Trying to get unsolicited files on the fly for an invalid platform '%ls'",
	...
	virtual bool EnqueueCookRequest(UE::Cook::FCookPackageRequest CookPackageRequest) override
	{
		...
		if (!TargetPlatform)
		{
			UE_LOGF(LogCook, Warning, "Trying to cook package on the fly for invalid platform '%ls'",
					*CookPackageRequest.PlatformName.ToString());
			return false;
		}

		FName StandardFileName(*FPaths::CreateStandardFilename(CookPackageRequest.Filename));
		UE_LOGF(LogCook, Verbose, "Enqueing cook request, Filename='%ls', Platform='%ls'", *CookPackageRequest.Filename, *CookPackageRequest.PlatformName.ToString());
...
private:
	...
	const ITargetPlatform* AddPlatformInternal(const FName& PlatformName)
	{
		...
		if (!PlatformData)
		{
			UE_LOGF(LogCook, Warning, "Target platform %ls wasn't found.", *PlatformName.ToString());
			return nullptr;
		}
...
void UCookOnTheFlyServer::Tick(float DeltaTime)
{
	...
	else
	{
		if (IsCookByTheBookMode() && !GIsSlowTask && IsCookFlagSet(ECookInitializationFlags::BuildDDCInBackground))
		{
			...
			{
				TickPrecacheObjectsForPlatforms(0.001f, CacheTargetPlatforms);
			}
		}
	}
...
bool UCookOnTheFlyServer::StartCookOnTheFly(FCookOnTheFlyStartupOptions InCookOnTheFlyOptions)
{
	...
	{
		PlatformManager->CreatePlatformData(*this, TargetPlatform);
	}
	...
	UE_LOGF(LogCook, Display, "Starting '%ls' cook-on-the-fly server",
	...
	CookOnTheFlyNetworkServer->OnRequest(ECookOnTheFlyMessage::RecompileShaders).BindLambda([this](ICookOnTheFlyClientConnection& Connection, const FCookOnTheFlyRequest& Request)
		{
			...
			if (!Connection.GetTargetPlatform())
			{
				UE_LOGF(LogCook, Warning, "RecompileShadersRequest from editor client");
	...
	if (CookOnTheFlyNetworkServer->Start())
	{
		...
		{
			UE_LOGF(LogCook, Fatal, "Unable to get any ListenAddresses for Unreal Network file server!");
		}

		if (ListenAddresses.Num() > 0)
		{
			UE_LOGF(LogCook, Display, "Unreal Network File Server is ready for client connections on %ls!", *ListenAddresses[0]->ToString(true));
		}
	}
	else
	{
		UE_LOGF(LogCook, Fatal, "Failed starting Unreal Network file server!");
	}
...
{
	UE_LOGF(LogCook, Display, "Initializing shaders for cook-on-the-fly");
...
void UCookOnTheFlyServer::AddCookOnTheFlyPlatformFromGameThread(ITargetPlatform* TargetPlatform)
{
	...
	// SaveAssetRegistry is done in CookByTheBookFinished for CBTB, but we need at the start of CookOnTheFly to send
	// as startup information to connecting clients
	// We copy the data from the global AssetRegistry since we don't have the up to date data from AssetRegistry
	// saves yet. 
	PlatformData->RegistryGenerator->InitializeAssetRegistryForCookOnTheFly(
		PlatformData->AssetRegistryDevelopmentSaveOptions);
	uint64 DevelopmentAssetRegistryHash = 0;
	PlatformData->RegistryGenerator->SaveAssetRegistry(GetSandboxAssetRegistryFilename(), true, false,
		DevelopmentAssetRegistryHash, TSet<FName>(), PlatformData->AssetRegistryDevelopmentSaveOptions);
...
{
	const bool bIsFirstSession = PlatformManager->GetNumSessionPlatforms() == 0;
	if (bIsFirstSession)
	{
		InitializeSession();
	}
	...
	if (bIsFirstSession)
	{
		UE::Cook::FDelegates::ConfigureCookSession.Broadcast(*this);
	}
...
void UCookOnTheFlyServer::GetDependentPackages( const TSet<FName>& RootPackages, TSet<FName>& FoundPackages )
{
	...
	while ( FoundPackagesCounter < FoundPackagesArray.Num() )
	{
		...
		if (AssetRegistry->GetDependencies(FoundPackagesArray[FoundPackagesCounter], PackageDependencies, UE::AssetRegistry::EDependencyCategory::Package) == false)
		{
			...
			{
				UE_LOGF(LogCook, Fatal, "Unable to find package %ls in asset registry.  Can't generate cooked asset registry", *FoundPackagesArray[FoundPackagesCounter].ToString());
			}
			else
			{
				UE_LOGF(LogCook, Warning, "Unable to find package %ls in asset registry, cooked asset registry information may be invalid ", *FoundPackagesArray[FoundPackagesCounter].ToString());
			}
		}
		...
		if (Asset.IsRedirector())
		{
			...
			if (Asset.GetTagValue("DestinationObject", RedirectedPathString))
			{
				...
				else
				{
					...
					UE_LOGF(LogCook, Log, "Found redirector in package %ls pointing to deleted object %ls", *PackageName.ToString(), *RedirectedPathString);
				}
...
bool UCookOnTheFlyServer::IsCookingDLCPluginOnly() const
{
	return IsCookingDLC() && CookByTheBookOptions->bDlcPluginOnly;
}
...
COREUOBJECT_API extern void SetOutputCookingWarnings(bool bValue);
COREUOBJECT_API extern bool GetOutputCookingWarnings();
...
void UCookOnTheFlyServer::RunCookList(ECookListOptions CookListOptions)
{
	...
	for (FPackageData* PackageData : ReportedDatas)
	{
		...
		UE_LOGF(LogCookList, Display, "%ls%ls%ls%ls",
...
void UCookOnTheFlyServer::SetSaveBusy(bool bInBusy, int32 NumAsyncWorkRetired)
{
	...
	if (CurrentTime >= SaveBusyWarnTimeSeconds)
	{
		...
		{
			UE_LOGF(LogCook, Display, "%d packages in the savequeue: ", SaveQueue.Num());
			for (TSet<UPackage*>* PackageSet : { &NonExpectedPackages, &ExpectedPackages })
			{
				for (UPackage* Package : *PackageSet)
				{
					...
					{
						UE_LOGF(LogCook, Display, "    ...");
						break;
					}
					UE_LOGF(LogCook, Display, "    %ls", *Package->GetName());
					++DisplayCount;
				}
			}
			if (DisplayCount == 0)
			{
				UE_LOGF(LogCook, Display, "    <None>");
			}

			UE_LOGF(LogCook, Display, "%d objects that have not yet returned true from IsCachedCookedPlatformDataLoaded:",
			...
			for (TArray<UObject*>* ObjectArray : { &NonExpectedObjects, &ExpectedObjects })
			{
				for (UObject* Object : *ObjectArray)
				{
					...
					UE_LOGF(LogCook, Display, "    %ls", *Object->GetFullName());
					...
					UE::String::ParseTokens(AdditionalDebugInfo.ToView(), TEXT('\n'),
						[](FStringView Line)
						{
							UE_LOGF(LogCook, Display, "        %.*ls", Line.Len(), Line.GetData());
		...
		else if (SaveQueue.Num() > 0)
		{
			...
			for (FPackageData* PackageData : SaveQueue)
			{
				...
				UE_LOGF(LogCook, Display, "    %ls%ls: %ls",
...
void UCookOnTheFlyServer::SetLoadBusy(bool bInLoadBusy)
{
	...
	else if (bLoadBusy)
	{
		...
		if (CurrentTime >= LoadBusyWarnTimeSeconds)
		{
...
#endif
			UE_LOGF(LogCook, Display, "%d packages in ReadyForLoads:", LoadQueue.ReadyForLoads.Num());
			DisplayCount = 0;
			for (TRefCountPtr<FPackagePreloader>& Preloader : LoadQueue.ReadyForLoads)
			{
				...
				UE_LOGF(LogCook, Display, "    %ls: %ls", 
					*PackageData.GetPackageName().ToString(), LexToString(PackageData.GetState()));
			}

			UE_LOGF(LogCook, Display, "%d packages in Inbox:", LoadQueue.Inbox.Num());
			DisplayCount = 0;
			for (FPackageData* PackageData : LoadQueue.Inbox)
			{
				if (DisplayCount++ == DisplayMax)
				{
					UE_LOGF(LogCook, Display, "    ...");
				...
				UE_LOGF(LogCook, Display, "    %ls",
					*PackageData->GetPackageName().ToString());
			}

			TArray<TRefCountPtr<FPackagePreloader>> PoppedPendingKicks;
			while (!LoadQueue.PendingKicks.IsEmpty())
			{
				PoppedPendingKicks.Add(LoadQueue.PendingKicks.PopFront());
			}
			UE_LOGF(LogCook, Display, "%d packages in PendingKicks:", PoppedPendingKicks.Num());
			DisplayCount = 0;
			for (TRefCountPtr<FPackagePreloader>& Preloader : PoppedPendingKicks)
			{
				FPackageData& PackageData(Preloader->GetPackageData());
				if (DisplayCount++ == DisplayMax)
				{
			...
			for (TRefCountPtr<FPackagePreloader>& Preloader : PoppedPendingKicks)
			{
				LoadQueue.PendingKicks.Add(Preloader);
			}
			
			UE_LOGF(LogCook, Display, "%d packages in ActivePreloads:", LoadQueue.ActivePreloads.Num());
			DisplayCount = 0;
			for (TRefCountPtr<FPackagePreloader>& Preloader : LoadQueue.ActivePreloads)
			{
				FPackageData& PackageData(Preloader->GetPackageData());
				if (DisplayCount++ == DisplayMax)
				{
				...
				TRefCountPtr<FPackagePreloader> Blocker;
				Preloader->PumpLoadsIsReadyToLeavePreload(&Blocker, &BlockerReason);
				if (!Blocker)
				{
					BlockerReason = TEXT("UnknownReasonStillInPreload: No Blocker found");
				}
				else
				{
					BlockerReason = FString::Printf(TEXT("Waiting on %s: %s"),
						*Blocker->GetPackageData().GetPackageName().ToString(), *BlockerReason);
				}

				UE_LOGF(LogCook, Display, "    %ls: %ls",
					*PackageData.GetPackageName().ToString(), *BlockerReason);
			}
			LoadBusyWarnTimeSeconds = CurrentTime + GCookProgressWarnBusyTime;
...
void UCookOnTheFlyServer::UpdateDisplay(UE::Cook::FTickStackData& StackData, bool bForceDisplay)
{
				...
				|| DeltaProgressDisplayTime > GCookProgressRepeatTime)))
	{
		...
		UE_CLOGF(!(StackData.TickFlags & ECookTickFlags::HideProgressDisplay)
			&& (GCookProgressDisplay & (int32)ECookProgressDisplayMode::RemainingPackages),
			LogCook, Display,
			"Cooked packages %d Packages Remain %d Total %d",
	...
	if (bForceDisplay || DeltaDiagnosticsDisplayTime > GCookProgressDiagnosticTime)
	{
		...
		if (!IsCookOnTheFlyMode() || bCookOnTheFlyShouldDisplay)
		{
			if (!(StackData.TickFlags & ECookTickFlags::HideProgressDisplay) && (GCookProgressDisplay != (int32)ECookProgressDisplayMode::Nothing))
			{
				...
				UE_LOGF(LogCook, Display, "Cook Diagnostics: OpenFileHandles=%d, VirtualMemory=%lldMiB, VirtualMemoryAvailable=%lldMiB",
					OpenFileHandles,
...
FString UCookOnTheFlyServer::GetCookSettingsForMemoryLogText() const
{
	...
	return FString::Printf(
		TEXT("\n\tMemoryMaxUsedVirtual %lldMiB")
		TEXT("\n\tMemoryMaxUsedPhysical %lldMiB")
		TEXT("\n\tMemoryMinFreeVirtual %lldMiB")
		TEXT("\n\tMemoryMinFreePhysical %lldMiB")
...
{
	UE_SCOPED_COOKTIMER(InitializePollables);
	using namespace UE::Cook;
...
void UCookOnTheFlyServer::PumpRequests(UE::Cook::FTickStackData& StackData, int32& OutNumPushed)
{
	...
	for (;;)
	{
		...
		else if (!DiscoveryQueue.IsEmpty())
		{
			...
			{
				UE_LOGF(LogCook, Warning,
					"Packages were added to the runtime discovery queue after starting BuildDependencies phase. Ignoring them."
					" The first added package: %ls",
		...
		else if (!BuildDependencyDiscoveryQueue.IsEmpty())
		{
			if (GetCookPhase() == ECookPhase::Cook)
			{
				UE_LOGF(LogCook, Error,
					"Packages were added to the build dependency discovery queue before starting BuildDependencies phase. Ignoring them."
					" The first added package: %ls",
	...
	UE::Cook::ESuppressCookReason Reason)
{
	...
	if (PackageData.IsInProgress())
	{
		...
		if (bPrintDiagnostic)
		{
			...
			if (bPrintDiagnostic)
			{
				UE_CLOGF((GCookProgressDisplay & (int32)ECookProgressDisplayMode::Instigators), LogCook, Display,
					"Cooking %ls, Instigator: { %ls } -> Rejected %ls", *PackageNameStr,
					*(PackageData.GetInstigator(EReachability::Runtime).ToString()), LexToString(Reason));
				UE_CLOGF(GCookProgressDisplay & (int32)ECookProgressDisplayMode::PackageNames, LogCook, Display,
					"Cooking %ls -> Rejected %ls", *PackageNameStr, LexToString(Reason));
			}
		}
	...
	UE::Cook::ESuppressCookReason Reason)
{
	...
	UE_LOGF(LogCook, Display, "DemoteToRequest: Package %ls was sent back to request state, with reason %ls.",
...
void UCookOnTheFlyServer::PromoteToSaveComplete(UE::Cook::FPackageData& PackageData, UE::Cook::ESendFlags SendFlags)
{
	if (!PackageData.IsInProgress())
	{
		UE_LOGF(LogCook, Error, "Package %ls is in PromoteToSaveComplete but is not in progress.",
			*PackageData.GetPackageName().ToString());
	...
	while (InProgress.Num() > static_cast<int32>(DesiredQueueLength) && OutNumPushed < LoadBatchSize)
	{
		...
		if (!ReadyForLoads.IsEmpty())
		{
			...
			else
			{
				...
				{
					UE_LOGF(LogCook, Warning,
						"Package %ls is generated but is ReadyForLoad when not in load state. State == %ls, PreloaderState == %ls, CountFromRequestedLoads == %d.",
...
void UCookOnTheFlyServer::LoadPackageInQueue(UE::Cook::FPackageData& PackageData, uint32& ResultFlags, int32& OutNumPushed)
{
	...
	if (!PackageData.IsGenerated())
	{
		...
		if (!bLoadFullySuccessful)
		{
			...
			UE_LOGF(LogCook, Verbose, "Not cooking package %ls", *PackageFileName.ToString());
		...
		if (LoadedPackage->GetFName() != PackageData.GetPackageName())
		{
			...
			UE_LOGF(LogCook, Verbose, "Request for %ls received going to save %ls", *PackageFileName.ToString(),
	...
	else
	{
		...
		{
			UE_LOGF(LogCook, Error,
				"Package %ls is a generated package, but ParentGenerator '%ls' is not a generator package. The generated package cannot be loaded.",
		...
		{
			UE_LOGF(LogCook, Error,
				"Package %ls is a generated package but its generator does not have a record of it. It can not be loaded.",
			...
			UE_LOGF(LogCook, Display, "The generator has %d generated packages, but %ls is not one of them:%ls",
	...
	if (ValidateSourcePackage(PackageData, LoadedPackage) == EDataValidationResult::Invalid)
	{
		...
		{
			ValidationArtifact->MarkPackageRequiresFullValidation(LoadedPackage->GetFName());
		}

		if (EnumHasAnyFlags(CookByTheBookOptions->StartupOptions, ECookByTheBookOptions::ValidationErrorsAreFatal))
		{
			UE_LOGF(LogCook, Error, "%ls failed validation", *LoadedPackage->GetName());
		...
		UE_LOGF(LogCook, Warning, "%ls failed validation", *LoadedPackage->GetName());
	}
...
void UCookOnTheFlyServer::RejectPackageToLoad(UE::Cook::FPackageData& PackageData, const TCHAR* ReasonText, UE::Cook::ESuppressCookReason Reason)
{
	...
	for (const TPair<const ITargetPlatform*, UE::Cook::FPackagePlatformData>& Pair : PackageData.GetPlatformDatas())
	{
		...
		if (IFileManager::Get().FileExists(*SandboxFilename))
		{
			...
			UE_LOGF(LogCook, Warning, "Found cooked file '%ls' which shouldn't exist as it %ls.", *SandboxFilename, ReasonText);
...
EDataValidationResult UCookOnTheFlyServer::ValidateSourcePackage(UE::Cook::FPackageData& PackageData, UPackage* Package)
{
	...
	UE_LOGF(LogCook, Display, "Validating package %ls", *PackageName);
#endif
	...
	if (EnumHasAnyFlags(CookByTheBookOptions->StartupOptions, ECookByTheBookOptions::RunAssetValidation) && UE::Cook::FDelegates::ValidateSourcePackage.IsBound())
	{
		...
		TArray<FAssetData> ExternalObjects = UE::Cook::GetExternalObjectsForPackageValidation(Package->GetFName(), World != nullptr);
		if (ValidationArtifact)
		{
			ValidationArtifact->FilterExternalPackagesToValidate(Package->GetFName(), ExternalObjects);
		}

		FMessageLogScopedOverride AssetCheckLogOverride(UE::DataValidation::MessageLogName);
	...
	UE::Cook::FPackageData& PackageData, UE::Cook::FCookerTimer& Timer, bool bPrecaching)
{
	...
	if (!InfoPtr)
	{
		UE_LOGF(LogCook, Error, "Generated package %ls is missing its generation data and cannot be saved.",
			*PackageData.GetPackageName().ToString());
	...
	// If we moved a FPendingCookedPlatformData object between generator and a generated package, wait for it
	// to complete before we allow the generator or moved generated package to proceed. For simplicity, we currently
	// block all generated packages from proceeding rather than keeping track of which ones have the pending objects.
	if (GenerationHelper.HasBlockingPendingCookedPlatformDataObjects())
	{
		return EPollStatus::Incomplete;
	}
	...
	if (PackageData.GetSaveSubState() <= ESaveSubState::Generation_FinishCacheObjectsToMove)
	{
		...
		if (PackageData.GetNumPendingCookedPlatformData() > 0 ||
			GenerationHelper.HasBlockingPendingCookedPlatformDataObjects())
		{
			return EPollStatus::Incomplete;
		}
	...
	TArray<ICookPackageSplitter::FGeneratedPackageForPopulate>& GeneratedPackagesForPopulate)
{
	...
	{
		UE_LOGF(LogCook, Error,
			"CookPackageSplitter is missing package during BeginCacheObjectsToMove. PackageName: %ls.",
			*PackageData.GetPackageName().ToString());
		return EPollStatus::Error;
	}
	...
	TArray<ICookPackageSplitter::FGeneratedPackageForPopulate>& GeneratedPackagesForPopulate)
{
	...
	{
		UE_LOGF(LogCook, Error, "CookPackageSplitter is missing %ls during PreSaveGeneratorPackage. PackageName: %ls.",
	...
	{
		UE_LOGF(LogCook, Error, "PackageSplitter unexpected failure: could not Construct GeneratedPackagesForPopulate. Splitter=%ls",
	...
	{
		UE::Cook::FScopedCookActivePackage ScopedActivePackage(GenerationHelper.GetOwner().GetPackageName(),
		...
		{
			UE_LOGF(LogCook, Error, "PackageSplitter returned false from PreSaveGeneratorPackage. Splitter=%ls",
				*GenerationHelper.GetSplitDataObjectName().ToString());
			return EPollStatus::Error;
		}
	}
	...
	UE::Cook::FCookGenerationInfo& Info, UE::Cook::FCookerTimer& Timer)
{
	...
	if (!Package || !Splitter || !SplitDataObject)
	{
		UE_LOGF(LogCook, Error, "CookPackageSplitter is missing %ls during BeginCachePostMove. PackageName: %ls.",
			(!Package ? TEXT("Package") : (!Splitter ? TEXT("Splitter") : TEXT("SplitDataObject"))),
			*PackageData.GetPackageName().ToString());
		return EPollStatus::Error;
	}
		...
		!Info.HasIssuedUndeclaredMovedObjectsWarning())
	{
		...
		UE_LOGF(LogCook, Warning,
			"CookPackageSplitter created or moved objects during %ls that are not yet ready to save. This will cause an error if garbage collection runs before the package is saved.\n"
			"Change the splitter's %ls to construct new objects and declare existing objects that will be moved from other packages.\n"
			"SplitterObject: %ls%ls\n"
			"NumPendingObjects: %d, FirstPendingObject: %ls",
	...
	UE::Cook::FCookGenerationInfo& GeneratedInfo)
{
	...
	{
		UE_LOGF(LogCook, Error,
			"TryPreSaveGeneratedPackage: could not load ParentGeneratorPackage %ls for GeneratedPackage %ls",
	...
	{
		UE_LOGF(LogCook, Error,
			"TryPreSaveGeneratedPackage could not find the original splitting object. Generated package can not be created. Splitter=%ls, Generated=%ls.",
	...
	UE::Cook::CookPackageSplitter::FPopulateContextData PopulateData;
	{
		...
		if (!bPreSaveSucceeded)
		{
			UE_LOGF(LogCook, Error, "PackageSplitter returned false from PreSaveGeneratedPackage. Splitter=%ls, Generated=%ls.",
				*GenerationHelper.GetSplitDataObjectName().ToString(), *GeneratedPackageName);
			return EPollStatus::Error;
		}
	}
	...
	{
		UE_LOGF(LogCook, Error,
			"PackageSplitter specified generated package is %ls in ReportGenerationManifest, but then in PreSaveGeneratedPackage created it as %ls. Splitter=%ls, Generated=%ls.",
	...
	UE::Cook::FCookerTimer& Timer, bool bPrecaching, UE::Cook::ESuppressCookReason& OutDemotionRequestedReason)
{
	...
	if (Result == EPollStatus::Success && PackageData.GetIsCookLast())
	{
		...
		{
			UE_LOGF(LogCook, Display, "CookLast: All other packages cooked. Releasing %ls.", *PackageData.GetPackageName().ToString());
		}
	}
	...
	UE::Cook::FCookerTimer& Timer, bool bPrecaching, UE::Cook::ESuppressCookReason& OutDemotionRequestedReason)
{
...
#if DEBUG_COOKONTHEFLY 
	UE_LOGF(LogCook, Display, "Caching objects for package %ls", *PackageData.GetPackageName().ToString());
#endif
	UPackage* Package = PackageData.GetPackage();
	...
	if (PackageData.GetSaveSubState() < ESaveSubState::CheckForIsGenerated)
	{
		if (PackageData.GetSaveSubState() <= ESaveSubState::StartSave)
		{
			if (PackageData.GetNumPendingCookedPlatformData() > 0)
			{
				...
				{
					UE_LOGF(LogCook, Error,
						"CookerBug: Package %ls is blocked from entering save due to GetNumPendingCookedCookedPlatformData() == %d.",
			...
			if (PackageData.GetSaveSubState() <= ESaveSubState::Generation_TryReportGenerationManifest)
			{
				...
				else
				{
					...
					if (IsCookingInEditor())
					{
						...
						UE_LOGF(LogCook, Error, "Can not cook package %ls: cooking in editor doesn't support Cook Package Splitters.",
							*PackageData.GetPackageName().ToString());
						return EPollStatus::Error;
					}
	...
	if (PackageData.GetSaveSubState() < ESaveSubState::ReadyForSave)
	{
		...
		else if (PackageData.IsGenerated())
		{
			...
			{
				UE_LOGF(LogCook, Error, "Generated package %ls %ls ParentGenerator package %ls and cannot be saved.",
	...
	for (; NextIndex < NumObjects; ++NextIndex)
	{
		...
		for (const ITargetPlatform* TargetPlatform : TargetPlatforms)
		{
			...
			if (Obj->IsA(UMaterialInterface::StaticClass()))
			{
				...
				{
#if DEBUG_COOKONTHEFLY
					UE_LOGF(LogCook, Display, "Delaying shader compilation of material %ls", *Obj->GetFullName());
#endif
					return EPollStatus::Incomplete;
				}
			}
			...
			else
			{
				...
				if (GenerationHelper)
				{
					GenerationHelper->OnAddPendingCookedPlatformData(*this, PackageData, Obj);
				}
			}
			...
			{
#if DEBUG_COOKONTHEFLY
				UE_LOGF(LogCook, Display, "Object %ls took too long to cache", *Obj->GetFullName());
#endif
				return EPollStatus::Incomplete;
			}
		}
	...
	UE::Cook::FPackageData* ReportingPackageData)
{
	...
	UE::Cook::FScopedCookActivePackage ScopedActivePackage(PackageData.GetPackageName(), NAME_None);
...
#if DEBUG_COOKONTHEFLY
	UE_LOGF(LogCook, Display, "Processing request %ls", *ReportingFileName);
#endif
	TOptional<TInheritedGuardValueAccessors<GetOutputCookingWarnings, SetOutputCookingWarnings>> OutputCookingWarningsGuard;
	if (!PackageData.HasBeenLoadedForCooking())
	{
		PackageData.SetHasBeenLoadedForCooking(true);
		OutputCookingWarningsGuard.Emplace(IsCookFlagSet(ECookInitializationFlags::OutputVerboseCookerWarnings));
	}
	...
	{
		UE_LOGF(LogCook, Display, "Package already loaded %ls avoiding reload", *ReportingFileName);
	}
#endif
	...
	int32& OutNumPushed, bool& bOutBusy, int32& OutNumAsyncWorkRetired)
{
	...
	while (SaveQueue.Num() > static_cast<int32>(DesiredQueueLength))
	{
...
#if DEBUG_COOKONTHEFLY
		UE_LOGF(LogCook, Display, "Processing save for package %ls", *Package->GetName());
#endif
		...
		if (PlatformsForPackage.Num() == 0)
		{
			...
			UE_LOGF(LogCook, Warning, "Package '%ls' in SaveQueue has no more platforms left to cook; this should not be possible!", *PackageData.GetFileName().ToString());
		...
		if (PrepareSaveStatus != EPollStatus::Success)
		{
			...
			{
				// Move the PackageData to the end of the urgent packages in the SaveQueue but before all of
				// the non-urgent packages.
				TArray<FPackageData*> UrgentSaves;
				TArray<FPackageData*> NonUrgentSaves;
				UrgentSaves.Reserve(SaveQueue.Num());
				NonUrgentSaves.Reserve(SaveQueue.Num());
				for (FPackageData* Next : SaveQueue)
				{
					((Next->GetUrgency() == EUrgency::Blocking) ? &UrgentSaves : &NonUrgentSaves)->Add(Next);
				}
				// We don't have a count of the Original number of urgent packages, so just keep cycling through all
				// blocked urgent packages up to OriginalPackagesToSaveCount times. But abort that cycling if there are
				// no other urgent packages left in the SaveQueue.
				bool HasCheckedAllPackagesAreCached = HandledCount >= OriginalPackagesToSaveCount || UrgentSaves.Num() == 0;
				if (!HasCheckedAllPackagesAreCached)
				{
					SaveQueue.Reset();
					for (FPackageData* Next : UrgentSaves)
					{
						SaveQueue.Add(Next);
					}
					SaveQueue.Add(&PackageData);
					for (FPackageData* Next : NonUrgentSaves)
					{
						SaveQueue.Add(Next);
					}
					continue;
	...
	int32& OutNumPushed, bool& bOutBusy, int32& OutNumAsyncWorkRetired)
{
	...
	while (SaveQueue.Num() > static_cast<int32>(DesiredQueueLength))
	{
		...
		if (PlatformsForPackage.Num() == 0)
		{
			...
			// All places that add a package to the save queue check for existence of incomplete platforms before adding
			UE_LOGF(LogCook, Warning, "Package '%ls' in SaveQueue has no more platforms left to commit; this should not be possible!",
...
void UCookOnTheFlyServer::PumpPhaseTransitionFence(bool& bOutComplete)
{
	...
	if (IdleStatusLastReportTime + ReportPeriod < CurrentTime)
	{
		...
		UE_LOGF(LogCook, Display,
			"Waiting on MPCook fence, but { %ls } has not responded to a heartbeat request for %.1f seconds. Continuing to wait...",
...
void UCookOnTheFlyServer::KickBuildDependencies(UE::Cook::FTickStackData& StackData)
{
	...
	{
		UE_LOGF(LogCook, Display, "UncookedBuildDependencies: Queueing %d packages for load-only commit.", BuildPackages.Num());
...
void UCookOnTheFlyServer::PostLoadPackageFixup(UE::Cook::FPackageData& PackageData, UPackage* Package)
{
	...
	if (World->GetStreamingLevels().Num())
	{
		...
		UE_LOGF(LogCook, Display, "Loading secondary levels for package '%ls'", *World->GetName());
...
void UCookOnTheFlyServer::MarkPackageDirtyForCookerFromSchedulerThread(const FName& PackageName)
{
	...
	UE_LOGF(LogCook, Verbose, "Modification detected to package %ls", *PackageName.ToString());
...
void UCookOnTheFlyServer::ShutdownCookOnTheFly()
{
	...
	{
		UE_LOGF(LogCook, Display, "Shutting down cook on the fly server");
...
public:
		...
		const FString& ResolvedMetadataPath, UE::Cook::FDeterminismManager* InDeterminismManager)
	{
		...
		if (!Capabilities.bDiffModeSupported)
		{
			...
			UE_LOGF(LogCook, Fatal, 
				"A DiffMode was enabled, but the current PackageWriter has bDiffModeSupported=false.");
		}
	...
	void Initialize()
	{
		...
		auto EnsureMutualExclusion = [this]()
		{
			if (DiffMode != EDiffMode::None)
			{
				UE_LOGF(LogCook, Fatal, "-DiffOnly, and -IncrementalValidate* are mutually exclusive.");
			}
		};
...
void UCookOnTheFlyServer::Initialize( ECookMode::Type DesiredCookMode, ECookInitializationFlags InCookFlags, const FString &InOutputDirectoryOverride )
{
	...
	if (IsCookByTheBookMode() && !IsCookingInEditor())
	{
		...
		if (CookProcessCount > UE::Cook::FWorkerId::GetMaxCookWorkerCount())
		{
			...
			UE_LOGF(LogCook, Fatal, "Invalid CookProcessCount=%d, maximum value is %d.",
		...
		{
			UE_LOGF(LogCook, Display, "CookProcessCount=%d. CookMultiprocess is disabled and the cooker is running as a single process.",
				CookProcessCount);
		}
	}
	...
	if (IsCookingInEditor() || (!IsCookByTheBookMode() && !IsCookWorkerMode()))
	{
		...
		NumPackagesHistory.Initialize(PackageTracker->NumLoadedPackages());
	...
	// It is perfectly safe to allow preloading in any environment whenever async loading is active
	// since we won't try to deal with linkers directly, we'll use proper calls to LoadPackageAsync in the preloader.
...
void FInitializeConfigSettings::LoadLocal(const FString& InOutputDirectoryOverride)
{
	...
	{
		UE_LOGF(LogCook, Warning, "Setting MemoryMaxUsedVirtual will be deprecated in future version. Please remove it from the settings files."
	...
	{
		UE_LOGF(LogCook, Warning, "Setting MemoryMaxUsedPhysical will be deprecated in future version. Please remove it from the settings files."
			"On systems with too little memory to load the minimum set required to cook some packages, frequent garbage collection will stall progress, "
	...
	{
		UE_LOGF(LogCook, Error, "Unrecognized value \"%ls\" for MemoryTriggerGCAtPressureLevel. Expected None or Critical.",
			*ConfigText);
	}
...
static EMPCookGeneratorSplit ParseMPCookGeneratorSplitFromString(const FString& Text)
{
	...
	{
		UE_LOGF(LogCook, Error,
			"Invalid value -MPCookGeneratorSplit=%ls. Valid values: { AnyWorker, AllOnSameWorker, SomeOnSameWorker, NoneOnSameWorker }.",
			*Text);
		return EMPCookGeneratorSplit::AnyWorker;
	}
}
...
void UCookOnTheFlyServer::SetInitializeConfigSettings(UE::Cook::FInitializeConfigSettings&& Settings)
{
	...
	// Best preloading performance is achieved when all the loading parameters have the same value
	// and that value is high enough to get multithreaded async loading using all cores.
	if (IsAsyncLoadingMultithreaded())
	{
		DesiredLoadQueueLength = 256;
		LoadBatchSize = DesiredLoadQueueLength;
		RequestBatchSize = DesiredLoadQueueLength;
		MaxPreloadAllocated = DesiredLoadQueueLength;
	}
	else
	{
	...
	UE_LOGF(LogCook, Display, "CookSettings for Memory:%ls", *GetCookSettingsForMemoryLogText());
	...
	if (bHiddenDependenciesDebug)
	{
		UE_LOGF(LogCook, Display, "HiddenDependenciesDebug is enabled.");
		...
		if (FParse::Value(CommandLine, TEXT("-HiddenDependenciesReport="), ClassPathListStr))
		{
			...
			{
				UE_LOGF(LogCook, Error, "-HiddenDependenciesIgnore and HiddenDepenciesReport are mutually exclusive. HiddenDepenciesIgnore setting will be discarded.");
			}
			bAllowList = true;
		}
		...
		if (!bHiddenDependenciesClassPathFilterListIsAllowList)
		{
			...
			for (const FString& ClassPathLine : ClassPaths)
			{
				...
				if (!Path.IsValid())
				{
					UE_LOGF(LogCook, Error, "Invalid Editor:[CookSettings]:IncrementalClassDenyList entry %.*ls. Expected an array of fullpaths such as /Script/Engine.Material",
		...
		if (!ClassPathListStr.IsEmpty())
		{
			UE::String::ParseTokensMultiple(ClassPathListStr, UE::Cook::GetCommandLineDelimiterChars(),
				[this](FStringView Token)
				{
					...
					if (!Path.IsValid())
					{
						UE_LOGF(LogCook, Error, "Invalid %ls=<ClassPath> setting. Expected a comma-delimited list of fullpaths such as /Script/Engine.Material",
	...
	UE::Cook::ProjectCookIncrementalVersion = FGuid(0, 0, 0, 0);
	FString GuidText;
	if (GConfig->GetString(TEXT("CookSettings"), TEXT("ProjectCookIncrementalVersion"), GuidText, GEditorIni))
	{
		if (!FGuid::Parse(GuidText, UE::Cook::ProjectCookIncrementalVersion))
		{
			UE_LOGF(LogCook, Error,
				"Invalid config setting Editor.ini:[CookSettings]:ProjectCookIncrementalVersion=%ls. Expected a hexadecimal guid string: ProjectCookIncrementalVersion=A0C50A4C-3305-4AB1-A401-66635AEFDD7F. FGuid(0,0,0,0) will be used instead.",
				*GuidText);
		}
	}
	...
	bIncrementallyModifiedDiagnostics = FParse::Param(CommandLine, TEXT("CookIncrementallyModifiedDiagnostics"))
		|| FParse::Param(CommandLine, TEXT("Cook.Diagnostic.Modified"));
...
void UCookOnTheFlyServer::ParseCookFilters(const TCHAR* Parameter, const TCHAR* Message, TSet<FName>& OutFilterClasses)
{
	...
	if (FParse::Value(FCommandLine::Get(), *FullParameter, IncludeClassesString))
	{
		...
		for (FString& IncludeClassString : IncludeClasses)
		{
			...
			{
				UE_LOGF(LogCook, Error, "%ls: Could not convert string '%ls' into a class path. Ignoring it.",
			...
			if (!IncludedClass)
			{
				UE_LOGF(LogCook, Error, "%ls: Could not find class with ClassPath '%ls'. Ignoring it.",
					Parameter, *IncludeClassString);
				continue;
			}
		...
		if (!RootNames.IsEmpty())
		{
			...
			UE_LOGF(LogCook, Display, "%ls: Only cooking packages that %ls with class in { %ls }",
...
{
	UE_SCOPED_COOKTIMER(InitializeSession);
...
UE::Cook::FInstigator UCookOnTheFlyServer::GetInstigator(FName PackageName) const
{
	return GetInstigator(PackageName, UE::Cook::EReachability::All);
}

UE::Cook::FInstigator UCookOnTheFlyServer::GetInstigator(FName PackageName, UE::Cook::EReachability Reachability) const
{
	using namespace UE::Cook;
...
TArray<UE::Cook::FInstigator> UCookOnTheFlyServer::GetInstigatorChain(FName PackageName) const
{
	using namespace UE::Cook;
...
UE::Cook::ECookType UCookOnTheFlyServer::GetCookType() const
{
...
UE::Cook::ECookingDLC UCookOnTheFlyServer::GetCookingDLC() const
{
	using namespace UE::Cook;
	if (!IsCookingDLC())
	{
...
FString UCookOnTheFlyServer::GetDLCName() const
{
	return CookByTheBookOptions->DlcName;
}

FString UCookOnTheFlyServer::GetBasedOnReleaseVersion() const
{
...
FString UCookOnTheFlyServer::GetCreateReleaseVersion() const
{
	return CookByTheBookOptions->CreateReleaseVersion;
}

UE::Cook::EProcessType UCookOnTheFlyServer::GetProcessType() const
{
	using namespace UE::Cook;
	if (IsCookWorkerMode())
	{
...
UE::Cook::ECookValidationOptions UCookOnTheFlyServer::GetCookValidationOptions() const
{
	using namespace UE::Cook;
...
bool UCookOnTheFlyServer::IsIncremental() const
{
...
TArray<const ITargetPlatform*> UCookOnTheFlyServer::GetSessionPlatforms() const
{
	return PlatformManager ? PlatformManager->GetSessionPlatforms() : TArray<const ITargetPlatform*>();
}

FString UCookOnTheFlyServer::GetCookOutputFolder(const ITargetPlatform* TargetPlatform) const
{
...
FString UCookOnTheFlyServer::GetCookMetadataOutputFolderInternal(const ITargetPlatform* TargetPlatform) const
{
	if (!SandboxFile || !TargetPlatform)
	{
		return FString();
	}

	return ConvertToFullSandboxPath(GetMetadataDirectory(), true, TargetPlatform->PlatformName());
}

FString UCookOnTheFlyServer::GetCookMetadataOutputFolder(const ITargetPlatform* TargetPlatform) const
{
	if (!PlatformManager || !PlatformManager->HasSessionPlatform(TargetPlatform))
	{
		return FString();
	}

	return GetCookMetadataOutputFolderInternal(TargetPlatform);
}
...
void UCookOnTheFlyServer::RegisterArtifact(UE::Cook::ICookArtifact* Artifact)
{
	...
	if (IsCookWorkerMode() && !Artifact->RegisterOnCookWorker())
	{
		// Artifacts need to specify if used on CookWorkers
		return;
	}
	...
	{
		UE_LOGF(LogCook, Error, "RegisterArtifact called with empty GetArtifactName(), which is invalid. Ignoring the Artifact.");
	...
	for (TRefCountPtr<ICookArtifact>& Existing : CookArtifacts)
	{
		if (Existing->GetArtifactName() == ArtifactName)
		{
			...
			{
				UE_LOGF(LogCook, Error,
					"RegisterArtifact Called with already existing ArtifactName \"%ls\". ArtifactNames must be unique. Ignoring the artifact.",
					*ArtifactName);
				FDebug::DumpStackTraceToLog(ELogVerbosity::Display);
			}
			return;
	...
	IPackageWriter::EWriteOptions WriteOptions)
{
	...
	if (CookWorkerClient || CookDirector)
	{
		...
		{
			UE_LOGF(LogCook, Error,
				"Output file requested during cooking from CookAdditionalFilesOverride was invalid. " "Output files must be under the platform's cooked root output, such as %ls. " "File %ls is not under the cooked root for any platform being cooked. The file will be missing from the cooked output.",
...
UE::Cook::ICookSandbox* UCookOnTheFlyServer::GetCookSandbox()
{
	return SandboxFile.Get();
}

void UCookOnTheFlyServer::DumpStats()
{
	UE_LOGF(LogCook, Display, "IntStats:");
	UE_LOGF(LogCook, Display, "  %ls=%d", TEXT("LoadPackage"), this->StatLoadedPackageCount);
	UE_LOGF(LogCook, Display, "  %ls=%d", TEXT("SavedPackage"), this->StatSavedPackageCount);

	OutputHierarchyTimers();
#if PROFILE_NETWORK
	UE_LOGF(LogCook, Display, "Network Stats \n"
...
void GetAdditionalCurrentIniVersionStrings( const UCookOnTheFlyServer* CookOnTheFlyServer, const ITargetPlatform* TargetPlatform, TMap<FString, FString>& IniVersionMap )
{
...

	{
		...
		for ( const auto& WaveName : AllWaveFormatNames )
		{
			...
			{
				UE_LOGF(LogCook, Warning, "Unable to find audio format \"%ls\" which is required by \"%ls\"", *WaveName.ToString(), *TargetPlatform->PlatformName());
			}
...

	{
		...
		if ( TFMM == nullptr )
		{
			UE_LOGF(LogInit, Error, "Texture format manager Ptr was requested but not available.");
		}

		//note: this gets All ITextureFormat modules in the Engine, not just ones relevant to this TargetPlatform
		const TArray<const ITextureFormat*> & SupportedTextureFormats = TFMM->GetAllTextureFormats();
	...
	if (AllowShaderCompiling())
	{
		...
		for (const auto& FormatName : AllFormatNames)
		{
			...
			{
				UE_LOGF(LogCook, Warning, "Unable to find shader \"%ls\" which is required by format \"%ls\"", *FormatName.ToString(), *TargetPlatform->PlatformName());
			}
		}
...
bool UCookOnTheFlyServer::GetCurrentIniVersionStrings( const ITargetPlatform* TargetPlatform, UE::Cook::FIniSettingContainer& IniVersionStrings ) const
{
	...
	while (ConfigFileEndIndex < EndIndex)
	{
		...
		if (!ConfigFile)
		{
			...
			UE_LOGF(LogCook, Display,
				"Could not load config file '%ls'. Changes to settings in this file will not be detected in legacyiterative cooks.",
		...
		while (ConfigSectionEndIndex < ConfigFileEndIndex)
		{
 			...
 			if (!ConfigSection)
			{
				...
				UE_LOGF(LogCook, Verbose,
					"Could not find config section %ls:[%ls]. Changes to settings in this section will not be detected in legacyiterative cooks.",
				...
				AccessedRecords.Slice(ConfigSectionStartIndex, ConfigSectionEndIndex - ConfigSectionStartIndex))
			{
				...
				if (Values.IsEmpty())
				{
					...
					// after loading and never existed on disk.
					UE_LOGF(LogCook, Verbose,
						"Could not find config value %ls:[%ls]:%ls. Changes to this value will not be detected in legacyiterative cooks.",
...
{
	const FString PlatformSandboxEditorIni = GetCookMetadataOutputFolderInternal(TargetPlatform) / TEXT("CookedIniVersion.txt"); 
	...
	for (const auto& UsedSetting : *UsedSettings )
	{
		...
		{
			UE_LOGF(LogCook, Warning, "Found unparsable ini setting %ls for platform %ls, invalidating cook.",
	...
	const FString& ArtifactName) const
{
	...
	return GetCookMetadataOutputFolderInternal(TargetPlatform) / LeafName;
}
...
bool UCookOnTheFlyServer::IniSettingsOutOfDate(const ITargetPlatform* TargetPlatform) const
{
	...
	{
		UE_LOGF(LogCook, Display, "Invalidating inisettings: Unable to read previous cook inisettings for platform %ls.", *TargetPlatform->PlatformName());
		return true;
	}
	...
	for ( const auto& OldIniSetting : OldAdditionalSettings)
	{
		...
		{
			UE_LOGF(LogCook, Display,
				"Invalidating inisettings: Unable to find additional ini setting used by platform %ls: %ls was not found.",
		...
		{
			UE_LOGF(LogCook, Display,
				"Invalidating inisettings: Additional ini setting used by platform %ls is different for %ls, value '%ls' != '%ls'.",
	...
	for (const auto& OldIniFile : OldIniSettings)
	{
		...
		{
			UE_LOGF(LogCook, Warning,
				"Invalidating inisettings: Invalid filename key in old ini settings file used by platform %ls: key '%ls' is invalid.",
		...
		{
			UE_LOGF(LogCook, Display,
				"Invalidating inisettings: Unable to find config file in old ini settings file used by platform %ls: '%ls' was not found.",
				*TargetPlatform->PlatformName(), *ConfigNameKeyStr);
			return true;
		}
		...
		for ( const auto& OldIniSection : OldIniFile.Value )
		{
			...
			{
				UE_LOGF(LogCook, Display,
					"Invalidating inisettings: Inisetting used by platform %ls is different for %ls:[%ls]. The section doesn't exist in current config.",
					*TargetPlatform->PlatformName(), *ConfigNameKeyStr, *SectionName.ToString());
				UE_LOGF(LogCook, Display, "%ls", *GetDenyListMessageStart());
				return true;
			}
			...
			for ( const auto& OldIniValue : OldIniSection.Value )
			{
				...
				{
					UE_LOGF(LogCook, Display,
						"Invalidating inisettings: Inisetting used by platform %ls is different for %ls:[%ls]:%ls. Mismatched num array elements %d != %d.",
						*TargetPlatform->PlatformName(), *ConfigNameKeyStr, *SectionName.ToString(),
						*ValueName.ToString(), CurrentValues.Num(), OldIniValue.Value.Num());
					UE_LOGF(LogCook, Display, "%ls:%ls", *GetDenyListMessageStart(), *ValueName.ToString());
					return true;
				}
				for ( int Index = 0; Index < CurrentValues.Num(); ++Index )
				{
					...
					{
						UE_LOGF(LogCook, Display,
							"Invalidating inisettings: Inisetting used by platform %ls is different for %ls:[%ls]:%ls%ls. Value '%ls' != '%ls'.",
							*TargetPlatform->PlatformName(), *ConfigNameKeyStr, *SectionName.ToString(),
							*ValueName.ToString(),
...
void UCookOnTheFlyServer::PopulateCookedPackages(TArrayView<const ITargetPlatform* const> TargetPlatforms)
{
	...
	for (const ITargetPlatform* TargetPlatform : TargetPlatforms)
	{
		...
		FAssetRegistryGenerator& PlatformAssetRegistry = *(PlatformData->RegistryGenerator);
		...
		UE_LOGF(LogCook, Display, "Populating cooked package(s) from %ls package store on platform '%ls'",
			*CookSavePackageContext.WriterDebugName, *TargetPlatform->PlatformName());

		TUniquePtr<FAssetRegistryState> PreviousAssetRegistry(GlobalArtifact->DetachAssetRegistry(TargetPlatform));
		if (!PreviousAssetRegistry)
		{
			UE_LOGF(LogCook, Error, "Could not load PreviousAssetRegistry, which is unexpected when cooking incrementally. Abandoning incremental cook and executing a full cook.");
			continue;
		}
		int32 NumPreviousPackages;
		PackageWriter.PopulateOplog(*PreviousAssetRegistry, NumPreviousPackages);
		if (NumPreviousPackages == 0)
		{
			UE_LOGF(LogCook, Display, "Found %d cooked package(s) in package store.", NumPreviousPackages);
			continue;
		}
		...
		if (!bLegacyBuildDependencies)
		{
			...
			PlatformAssetRegistry.ReportGeneratorPackages(*PreviousAssetRegistry, PreviousGeneratorPackages,
				NumNeverCookPlaceHolderPackages);
		...
		else
		{
			...
			UE_LOGF(LogCook, Display, "Found %d cooked package(s) in package store.", CookedNum);
			UE_LOGF(LogCook, Display, "Keeping %d. Recooking %d. Removing %d. %d generated packages to be evaluated for legacyiterative skipping later.",
		...
		for (TPair<FName, FAssetRegistryGenerator::FGeneratorPackageInfo> Pair : PreviousGeneratorPackages)
		{
			...
			{
				UE_LOGF(LogCook, Display,
					"Previous cook results returned a record for generator package %ls, but that package can no longer be found; its generated packages will not be removed from cook results. Run a full cook to remove them.",
					*Pair.Key.ToString());
				continue;
			}
...
void UCookOnTheFlyServer::BlockOnAssetRegistry(TConstArrayView<FString> CommandlinePackages)
{
	...
	UE_LOGF(LogCook, Display, "Waiting for Asset Registry");
	...
	for (const ITargetPlatform* TargetPlatform : TargetPlatforms)
	{
		...
		if (!RegistryReporter)
		{
			if (!IsCookWorkerMode())
			{
				...
				PlatformData->RegistryGenerator->Initialize();
			}
			else
			{
...
void UCookOnTheFlyServer::RegisterValidationCookArtifact()
{
	if (ValidationArtifact)
	{
		UnregisterArtifact(ValidationArtifact);
		ValidationArtifact = nullptr;
	}

	check(!ValidationArtifact);
	ValidationArtifact = new UE::Cook::FValidationCookArtifact(*this);
	TRefCountPtr<UE::Cook::ICookArtifact> RefCounter(ValidationArtifact);
	RegisterArtifact(ValidationArtifact);
	// We keep a raw pointer and rely on RegisterArtifact holding the reference.
	check(RefCounter.GetRefCount() > 1);
}
...
const TCHAR* GCookRequestUsageMessage = TEXT(
	...
	"		-EDLCookFilters=<EDLObjectPath1>+<EDLObjectPath2>+... \n"
	"			Specify EDL object paths to cook exclusively, skipping all other EDLs used by the base map and the base map itself.\n"
	"	Ini:\n"
	"		Editor.ini\n"
	"			[AlwaysCookMaps]\n"
	"				+Map=<PackageName>\n"
	...
	const TMap<FName, TArray<FName>>& GameDefaultObjects)
{
	...
	if (FParse::Param(FCommandLine::Get(), TEXT("helpcookusage")))
	{
		UE::String::ParseLines(GCookRequestUsageMessage, [](FStringView Line)
			{
				UE_LOGF(LogCook, Warning, "%.*ls", Line.Len(), Line.GetData());
			});
	}
	UProjectPackagingSettings* PackagingSettings = Cast<UProjectPackagingSettings>(UProjectPackagingSettings::StaticClass()->GetDefaultObject());
		...
		!(FilesToCookFlags & ECookByTheBookOptions::NoAlwaysCookMaps))
	{

		{
			...
			{
				UE_LOGF(LogCook, Verbose, "Maplist contains %ls ", *MapList[MapIdx]);
...

		{
			...
			{
				UE_LOGF(LogCook, Verbose, "Loading map ini section %ls", *IniMapSection);
				MapList.Reset();
				GEditor->LoadMapListFromIni(*IniMapSection, MapList);
				FName MapSectionName(*IniMapSection);
				for (const FString& MapName : MapList)
				{
					UE_LOGF(LogCook, Verbose, "Maplist contains %ls", *MapName);
		...
		if (bFoundMapsToCook == false)
		{
			...
			{
				UE_LOGF(LogCook, Verbose, "Maps to cook list contains %ls", *MapToCook.FilePath);
		...
		if (bFoundMapsToCook == false)
		{
			UE_LOGF(LogCook, Verbose, "Loading default map ini section AllMaps");
		...
		// Also append any cookdirs from the project ini files; these dirs are relative to the game content directory or start with a / root
		{
			for (const FDirectoryPath& DirToCook : PackagingSettings->DirectoriesToAlwaysCook)
			{
				...
				{
					UE_LOGF(LogCook, Verbose, "Loading directory to always cook %ls", *DirToCook.Path);
				...
				{
					UE_LOGF(LogCook, Warning, "'ProjectSettings -> Directories to never cook -> Directories to always cook' has invalid element '%ls'", *DirToCook.Path);
				}
			}
		}
	}
		...
		&& !(FilesToCookFlags & ECookByTheBookOptions::NoGameAlwaysCookPackages))
	{
		if (bCookAll || (UE::Cook::bCookAllByDefault && NumFilesAddedByCommandLineOrGameCallback == 0))
		{
			...
			for (int32 TokenIndex = 0; TokenIndex < Tokens.Num(); TokenIndex++)
			{
				...
				{
					UE_LOGF(LogCook, Display, "No packages found for parameter %i: '%ls'", TokenIndex, *Tokens[TokenIndex]);
					continue;
				}
...
FString UCookOnTheFlyServer::GetCookedCookMetadataFilename(const ITargetPlatform* TargetPlatform)
{
	return GetCookMetadataOutputFolderInternal(TargetPlatform) / UE::Cook::GetCookMetadataFilename();
}
...
void UCookOnTheFlyServer::WriteCookMetadata(const ITargetPlatform* InTargetPlatform, uint64 InDevelopmentAssetRegistryHash)
{
	...
	{
		UE_LOGF(LogCook, Warning, "Number of plugins exceeds 64k, unable to write cook metadata file (count = %d)", EnabledPlugins.Num() + AdditionalPseudoPlugins);
	}
	else
	{
		...
		if (CustomFieldNames.Num() > 255)
		{
			...
			UE_LOGF(LogCook, Warning, "Number of custom plugin fields exceeds 255 (count = %d), custom fields will be incorrect!", CustomFieldNames.Num());
		}
		...
		for (TSharedRef<IPlugin>& EnabledPlugin : EnabledPlugins)
		{
						...
						if (Descriptor.CachedJson->TryGetArrayField(TEXT("PerPlatform") + FieldName, Array))
						{
							...
							for (const TSharedPtr<FJsonValue>& Value : *Array)
							{
								...
								if (ValueObject.IsValid())
								{
									...
									{
										UE_LOGF(LogCook, Error, "Unable to get Platform field from PerPlatform%ls array in plugin %ls json.", *FieldName, *EnabledPlugin->GetName());
										continue;
									}
									...
									if (OverridePlatformName == PlatformNameString)
									{
										...
										if (!bGotOverride)
										{
											UE_LOGF(LogCook, Error, "Unable to get Value field from PerPlatform%ls array in plugin %ls json for platform %ls", *FieldName, *EnabledPlugin->GetName(), *PlatformNameString);
											continue;
										}
										bHasField = true;
									}
			...
			for (FPluginReferenceDescriptor ChildPlugin : Descriptor.Plugins)
			{
				...
				{
					UE_LOGF(LogCook, Display, "Dependent plugin \"%ls\" referenced by \"%ls\" wasn't found in enabled plugins list when creating cook metadata file... skipping", *ChildPlugin.Name, *Descriptor.FriendlyName);
				}
			}
		...
		for (UE::Cook::FCookMetadataPluginEntry& Entry : PluginHierarchy.PluginsEnabledAtCook)
		{
			...
			{
				UE_LOGF(LogCook, Warning, "Found unassigned plugin type in cook metadata generation: %ls", *Entry.Name);
			}
		}
		...
		MetadataState.SaveToFile(GetCookedCookMetadataFilename(InTargetPlatform));
	}
}

void UCookOnTheFlyServer::WriteReferencedSet(const ITargetPlatform* InTargetPlatform, TArray<FName>&& CookedPackageNames)
{
	const FString MetadataFilename = GetCookMetadataOutputFolderInternal(InTargetPlatform) / UE::Cook::GetReferencedSetFilename();
...
{
	UE_LOGF(LogCook, Display, "---- Finalisation: Start ----");
	{
	...
	ShutdownCookSession();
	UE_LOGF(LogCook, Display, "---- Finalisation: End ----");
	UE_LOGF(LogCook, Display, "Done!");
}

void UCookOnTheFlyServer::CookByTheBookFinishedInternal()
{
	UE_LOGF(LogCookStatus, Display, "FinishedInternal: Start");
	...
	UE_LOGF(LogCookStatus, Display, "Waiting for AsyncFileWrites");
	UPackage::WaitForAsyncFileWrites();

	UE_LOGF(LogCookStatus, Display, "Waiting for BuildDefinitions");
	BuildDefinitions->Wait();
	
	UE_LOGF(LogCookStatus, Display, "Waiting for DDC");
	...
	bool bForceNoFilterAssetsFromAssetRegistry = IsForceNoFilterAssetsFromAssetRegistry();
	...
	UE_LOGF(LogCookStatus, Display, "Collecting cook information for all platforms");
	...
	// Call FinishPopulateShaderLibrary to load the previous version of the shader libraries for all platforms, and
	// call UpdateOplogPackages to report which assets are still referenced, before pruning any no-longer-referenced
	// shaders from any platforms. We do it in this order because platforms can share shaderformats, so we need to
	// know if any platform is referencing shaderformat X before we can prune it.
	if (bCacheShaderLibraries)
	{
		const bool bInitializeReferenceTracking = !bLegacyBuildDependencies;
		for (const ITargetPlatform* TargetPlatform : PlatformManager->GetSessionPlatforms())
		{
			UE_SCOPED_HIERARCHICAL_COOKTIMER(FinishPopulateShaderLibrary);
			FinishPopulateShaderLibrary(TargetPlatform, LibraryName, bInitializeReferenceTracking);
		}
	}
	...
	for (const ITargetPlatform* TargetPlatform : PlatformManager->GetSessionPlatforms())
	{
		...
		if (SaveContext.PackageWriterCapabilities.bReadOnly)
		{
			continue;
		}
		...
		TArray<FPackageData*>& CookedPackageDatas = EndCookPlatformData.CookedPackageDatas;
		TArray<FPackageData*>& IgnorePackageDatas = EndCookPlatformData.IgnorePackageDatas;
		...
		// Call UpdateOplogPackages on all Cook Artifacts. We have to do it at this point to handle shader libraries
		// correctly. FShaderLibraryCookArtifact needs UpdateOplogPackages called after FinishPopulateShaderLibrary,
		// so that it has all of its data loaded and ready to be pruned. But it has to do this pruning before any calls
		// to FinalizeChunkIds or SaveShaderLibrary; those functions need to write out the pruned version of the data.
		{
			UE_SCOPED_HIERARCHICAL_COOKTIMER(CookArtifacts.UpdateOplogPackages);
			using namespace UE::Cook::Artifact;
			FUpdateOplogPackagesContext OplogContext(*this);
			OplogContext.TargetPlatform = TargetPlatform;
			for (FPackageData* PackageData : CookedPackageDatas)
			{
				FOplogPackageData& Data = OplogContext.OplogPackages.FindOrAdd(PackageData->GetPackageName());
				Data.ReferenceType = UE::Cook::EPackageReferenceType::Stage;
				const FPackagePlatformData& PackagePlatformData = PackageData->FindOrAddPlatformData(TargetPlatform);
				Data.bIncrementallyCooked = !PackagePlatformData.IsIncrementallySkipped();
			}
			for (FPackageData* PackageData : IgnorePackageDatas)
			{
				FOplogPackageData& Data = OplogContext.OplogPackages.FindOrAdd(PackageData->GetPackageName());
				Data.ReferenceType = UE::Cook::EPackageReferenceType::Cook;
				const FPackagePlatformData& PackagePlatformData = PackageData->FindOrAddPlatformData(TargetPlatform);
				Data.bIncrementallyCooked = !PackagePlatformData.IsIncrementallySkipped();
			}

			// Call UpdateLastReferenceDateAndPruneStaleOps to remove any stale ops from the oplog, and then
			// add all of the ops in the oplog to OplogContext.OplogPackages, if they do not already exist.
			SaveContext.PackageWriter->UpdateLastReferenceDateAndPruneStaleOps(OplogContext);
			for (FName PackageName : SaveContext.PackageWriter->GetOplogPackageNames())
			{
				// We rely on ReferenceType being initialized to EPackageReferenceType::Unreferenced for newly added
				// elements. In that case, calling FindOrAdd is sufficient to add the entry we need because we only
				// need to add entries for any non-referenced-set packages; all of the referenced-set packages were
				// added in the for (PackageData : CookedPackageDatas) loop above.
				OplogContext.OplogPackages.FindOrAdd(PackageName);
			}
			...
			for (const TRefCountPtr<UE::Cook::ICookArtifact>& Artifact : CookArtifacts)
			{
				...
				{
					Artifact->UpdateOplogPackages(OplogContext);
				}
			}
		}
	}

	// Trigger the ShaderLibrary to prune the ShaderFormats used by each platform, now that we have reported
	// which assets are still in use by each platform.
	if (!bLegacyBuildDependencies && bCacheShaderLibraries)
	{
		UE_SCOPED_HIERARCHICAL_COOKTIMER(PruneShaderLibrary);
		...
		for (const ITargetPlatform* TargetPlatform : PlatformManager->GetSessionPlatforms())
		{
			...
			PruneShaderLibrary(TargetPlatform, LibraryName);
		}
	}
	...
	for (const ITargetPlatform* TargetPlatform : PlatformManager->GetSessionPlatforms())
	{
		...
		if (bSaveManifests)
		{
			...
			}
			{
				...
				if (!Generator.SaveManifests(*SandboxFile, *this))
				{
					UE_LOGF(LogCook, Warning, "Failed to save chunk manifest");
				}
				...
				if (FParse::Value(FCommandLine::Get(), TEXT("ExtraFlavorChunkSize="), ExtraFlavorChunkSize) && ExtraFlavorChunkSize > 0)
				{
					...
					if (!Generator.SaveManifests(*SandboxFile, *this, ExtraFlavorChunkSize, ManifestSubDir))
					{
						UE_LOGF(LogCook, Warning, "Failed to save chunk manifest");
					}
				}
		...
		if (bSaveAssetRegistry)
		{
			...
			Generator.SaveAssetRegistry(SandboxRegistryFilename, bSaveDevelopmentAssetRegistry,
				bForceNoFilterAssetsFromAssetRegistry, DevelopmentAssetRegistryHash, CookedPackageNames,
				PlatformData->AssetRegistryDevelopmentSaveOptions);
		}
		...
		if (bCacheShaderLibraries)
		{
			...
			DumpShaderTypeStats(TargetPlatform, PlatformNameString);
		}
	...
	if (!PlatformManager->GetSessionPlatforms().IsEmpty() && IncrementallyModifiedDiagnostics)
	{
		const ITargetPlatform* TargetPlatform = PlatformManager->GetSessionPlatforms()[0];
		FString DiagnosticsDir;
		if (GIsBuildMachine)
		{
			DiagnosticsDir = FPaths::Combine(*FPaths::EngineDir(), TEXT("Programs"), TEXT("AutomationTool"), TEXT("Saved"), TEXT("Logs"), TargetPlatform->PlatformName());
		}
		else
		{
			DiagnosticsDir = GetCookMetadataOutputFolderInternal(TargetPlatform);
		}
		IncrementallyModifiedDiagnostics->WriteFinal(DiagnosticsDir);
	}

	UE_LOGF(LogCookStatus, Display, "Shutting down shader library cooker & shader compilers");
	ShutdownShaderLibraryCookerAndCompilers(LibraryName);

	if (CookByTheBookOptions->bGenerateDependenciesForMaps)
	{
		UE_LOGF(LogCookStatus, Display, "Generating dependencies for maps");
	...
	{
		UE_LOGF(LogCookStatus, Display, "Generating editor thumbnails");
		GenerateCachedEditorThumbnails();
	}

	{
		UE_SCOPED_HIERARCHICAL_COOKTIMER(FinalizePackageStore);
		UE_LOGF(LogCook, Display, "Finalize package store(s)...");
		PlatformIndex = 0;
		for (const ITargetPlatform* TargetPlatform : PlatformManager->GetSessionPlatforms())
		{
			FEndCookPlatformData& EndCookPlatformData = EndCookPlatformDatas[PlatformIndex++];
	...
	UE_LOGF(LogCookStatus, Display, "FinishedInternal: Done");
}

bool UCookOnTheFlyServer::IsForceNoFilterAssetsFromAssetRegistry()
{
	if (IsDirectorCookByTheBook())
	{
		return IsCookingDLC() ? !FParse::Param(FCommandLine::Get(), TEXT("ApplyFiltersToDLCAssetRegistry")) : false;
	}
	else
	{
		return false;
...
void UCookOnTheFlyServer::PrintFinishStats()
{
	...
	if (IsCookByTheBookMode())
	{
		UE_LOGF(LogCook, Display, "Cook by the book total time in tick %fs total time %f", CookByTheBookOptions->CookTime, TotalCookTime);
	}
	else if (IsCookWorkerMode())
	{
		UE_LOGF(LogCook, Display, "CookWorker total time %f", TotalCookTime);
	}
	...
	UE_LOGF(LogCook, Display, "Peak Used virtual %llu MiB Peak Used physical %llu MiB", MemStats.PeakUsedVirtual / 1024 / 1024, MemStats.PeakUsedPhysical / 1024 / 1024);

	COOK_STAT(UE_LOGF(LogCook, Display,
		"Packages Cooked: %d,%ls Packages Skipped by Platform: %d, Total Packages: %d",
		ReportedNumCooked,
...
void UCookOnTheFlyServer::GenerateCachedEditorThumbnails()
{
	...
	for (const ITargetPlatform* Platform : PlatformManager->GetSessionPlatforms())
	{
		...
		UE_LOGF(LogCook, Display, "Generating %ls", *CachedEditorThumbnailsFilename);
			...
			for (const UE::Cook::FPackageData* const CookedPackage : CookedPackages)
			{
				if (CookedPackage->GetWasCookedThisSession())
				{
					...
					for (FAssetData& Asset : Assets)
					{
						...
						{
							UE_LOGF(LogCook, Verbose, "Adding thumbnail for '%ls'", *Asset.GetObjectPathString());
...
{
	UE_SCOPED_COOKTIMER(CreateSandboxFile);
...
{
	UE_SCOPED_COOKTIMER(LoadBeginCookConfigSettings);
...
struct FDependencyTrackingConfigParser
{
	void Parse();

	// Input parameters
	bool bInIssueLogs = false;
	TOptional<bool> bInUsingZenStore;

	// Output environment settings
	bool bOutLegacyBuildDependencies = false;
	bool bOutLegacyBuildDependenciesUsedDueToNoZenStore = false;
	bool bOutDependencyTrackingEnabled = true;
};

void FDependencyTrackingConfigParser::Parse()
{
	const TCHAR* CommandLine = FCommandLine::Get();
	GConfig->GetBool(TEXT("CookSettings"), TEXT("LegacyIterative"), bOutLegacyBuildDependencies, GEditorIni);

	if (!bOutLegacyBuildDependencies && FParse::Param(CommandLine, TEXT("ODSC")))
	{
		// INCREMENTALCOOK_TODO: implement incremental cook for ODSC
		UE_CLOGF(bInIssueLogs, LogCook, Display,
			"INCREMENTAL COOK DEPENDENCIES: Disabled. The cooker is running as ODSC which forces use of legacyiterative.");
		bOutLegacyBuildDependencies = true;
	}
	// TODO: Parse bIsUsingZenStore in this function from config and commandline, so that we can have a more accurate result
	// for bDependencyTrackingEnabled for callers that don't know the value of bInUsingZenStore.
	else if (!bOutLegacyBuildDependencies && bInUsingZenStore.IsSet() && !*bInUsingZenStore)
	{
		// Incremental cook uses TargetDomain storage of dependencies which is only implemented in ZenStore
		UE_CLOGF(bInIssueLogs, LogCook, Display,
			"INCREMENTAL COOK DEPENDENCIES: Disabled. Incremental Cook Dependencies are disabled because the cooker is not using ZenStore, " "and we do not yet support storage of Incremental Cook dependency data with loose cooked packages. " "Falling back to legacy iterative cook mode.");
		bOutLegacyBuildDependencies = true;
		bOutLegacyBuildDependenciesUsedDueToNoZenStore = true;
	}
	else if (!bOutLegacyBuildDependencies)
	{
		UE_CLOGF(bInIssueLogs, LogCook, Display,
			"INCREMENTAL COOK DEPENDENCIES: Enabled. Incremental Cook Dependencies are enabled in Editor.ini:[CookSettings]:LegacyIterative=false.");
	}
	else
	{
		UE_CLOGF(bInIssueLogs, LogCook, Display,
			"INCREMENTAL COOK DEPENDENCIES: Disabled. Incremental Cook Dependencies are disabled in Editor.ini:[CookSettings]:LegacyIterative=true.");
	}

	if (bOutLegacyBuildDependencies)
	{
		bOutDependencyTrackingEnabled = false;
	}
	else
	{
		bOutDependencyTrackingEnabled = true;
		FParse::Bool(CommandLine, TEXT("-Cook.DependencyTracking="), bOutDependencyTrackingEnabled);
	}
}
bool IsDependencyTrackingEnabled()
{
	FDependencyTrackingConfigParser Parser;
	Parser.Parse();
	return Parser.bOutDependencyTrackingEnabled;
}

void FBeginCookConfigSettings::LoadLocal(FBeginCookContext& BeginContext)
{
	FDependencyTrackingConfigParser Parser;
	Parser.bInIssueLogs= true;
	Parser.bInUsingZenStore.Emplace(BeginContext.COTFS.IsUsingZenStore());
	Parser.Parse();
	bLegacyBuildDependencies = Parser.bOutLegacyBuildDependencies;
	bLegacyBuildDependenciesDueToNoZenStore = Parser.bOutLegacyBuildDependenciesUsedDueToNoZenStore;
	bDependencyTrackingEnabled = Parser.bOutDependencyTrackingEnabled;
	...
	for (const ITargetPlatform* TargetPlatform : BeginContext.TargetPlatforms)
	{
		...
		{
			UE_LOGF(LogCook, Warning, "Engine.ini:[Core.System]:LegacyBulkDataOffsets is no longer supported in UE5. The intended use was to reduce patch diffs, but UE5 changed cooked bytes in every package for other reasons, so removing support for this flag does not cause additional patch diffs.");
		}
	}
}
...
void UCookOnTheFlyServer::SetBeginCookConfigSettings(FBeginCookContext& BeginContext, UE::Cook::FBeginCookConfigSettings&& Settings)
{
	...
	bLegacyBuildDependenciesDueToNoZenStore = Settings.bLegacyBuildDependenciesDueToNoZenStore;
	bDependencyTrackingEnabled = Settings.bDependencyTrackingEnabled;
...
{
	UE_SCOPED_COOKTIMER(LoadBeginCookIncrementalFlags);
...
void UCookOnTheFlyServer::LoadBeginCookIncrementalFlagsLocal(FBeginCookContext& BeginContext)
{
	...
	if (bIncrementalOrLegacyIterative && bLegacyBuildDependenciesDueToNoZenStore
		&& EnumHasAnyFlags(CookFlags, ECookInitializationFlags::Unversioned))
	{
		UE_LOGF(LogCook, Warning,
			"CookIncremental and UnversionedCookedContent were requested, but LegacyIterative is required due to ZenStore not being used. "
			"LegacyIterative incremental cooks do not robustly support unversioned cooked content due to failure to track dependencies on c++ serialization changes. "
			"The cooker will cook with these settings, but may fail to recook packages that require it those packages will cause runtime crashes when loaded. "
			"For robust incremental cooks, set bUseZenStore=true and bUseIoStore=true in ProjectPackagingSettings, or turn off UnversionedCookedContent."
		);
	}
	...
	for (FBeginCookContextPlatform& PlatformContext : BeginContext.PlatformContexts)
	{
		...
		if (bIncrementalOrLegacyIterative && bIsSharedLegacyIterativeCook && !PlatformData->bIsSandboxInitialized)
		{
				...
				PreviousSharedCookedBuild >= PreviousLocalCookedBuild)
			{
				...
				UE_LOGF(LogCook, Display, "Shared legacyiterative build is newer then local cooked build, legacyiteratively cooking from shared build.");
			}
			else
			{
				UE_LOGF(LogCook, Display, "Local cook is newer then shared cooked build, legacyiteratively cooking from local build.");
			}
		}
		for (const TRefCountPtr<UE::Cook::ICookArtifact>& Artifact : CookArtifacts)
		{
		...
		{
			UE_LOGF(LogCook, Display,
				"INCREMENTAL COOK: cooking incrementally due to -DiffOnly flag. Keeping previously cooked packages for platform %ls and cooking into memory buffers.",
		...
		else
		{
			...
			if (!bIncrementalOrLegacyIterative && !PlatformData->bIsSandboxInitialized)
			{
				if (!bLegacyBuildDependencies)
				{
					...
					{
						UE_LOGF(LogCook, Display,
							"FULL COOK: Incremental Cooks are disabled by default in Editor.ini:[CookSettings]:CookIncrementalDefaultIncremental=false, and commandline did not specify -forcerecook=false. " "Deleting previously cooked packages for platform %ls and recooking all packages discovered in the current cook.",
							*TargetPlatform->PlatformName());
					}
					else
					{
						UE_LOGF(LogCook, Display,
							"FULL COOK: -forcerecook=true (or an equivalent) was specified. Deleting previously cooked packages for platform %ls and recooking all packages discovered in the current cook.",
							*TargetPlatform->PlatformName());
					}
				}
				else
				{
					UE_LOGF(LogCook, Display,
						"FULL COOK: Incremental Cook Dependencies are disabled, and -legacyiterative was not specified. Deleting previously cooked packages for platform %ls and recooking all packages discovered in the current cook.",
						*TargetPlatform->PlatformName());
				}
				...
				{
					UE_LOGF(LogCook, Display,
						"'-odsc' was passed on commandline, but '-legacyiterative' was not, so the cooker as a side effect is clearing cook results. The build will need to be recooked before it can be staged. Add the commandline argument '-legacyiterative' to avoid this unnecessary clear."
					);
				}
			}

			if (bIncrementalOrLegacyIterativeAllowed && !bLegacyBuildDependencies && !bDependencyTrackingEnabled)
			{
				UE_LOGF(LogCook, Display,
					"FULL COOK: -cookincremental was specified, but -Cook.DependencyTracking=0 was present on commandline and so cookincremental is not supported. Deleting previously cooked packages for platform %ls and recooking all packages discovered in the current cook.",
					*TargetPlatform->PlatformName());
				bIncrementalOrLegacyIterativeAllowed = false;
			}
			...
			if (bIncrementalOrLegacyIterativeAllowed)
			{
				...
				for (const TRefCountPtr<UE::Cook::ICookArtifact>& Artifact: CookArtifacts)
				{
					...
					UE::Cook::Artifact::FCompareSettingsContext Context(*this,
						FindOrCreateCookArtifactReader(TargetPlatform),
						Previous, Current.Settings, PreviousFileName);
					...
					UE_LOGF(LogCook, Display,
						"FULL COOK: %ls was specified, but %ls settings have changed and all previously cooked packages are invalidated. Deleting previously cooked packages for platform %ls and recooking all packages discovered in the current cook.",
						bLegacyBuildDependencies ? TEXT("-legacyiterative") : TEXT("-cookincremental"), *ArtifactName, *TargetPlatform->PlatformName());
					if (bRunningAsShaderServer)
					{
						UE_LOGF(LogCook, Display,
							"'-odsc -legacyiterative' was passed on commandline, but due to unrelated changes in global settings the cooker has to clear cook results. The build will need to be recooked before it can be staged."
						);
					}
					bIncrementalOrLegacyIterativeAllowed = false;
					break;
			...
			{
				UE_LOGF(LogCook, Display,
					"INCREMENTAL COOK: %ls settings are still valid. Keeping previously cooked packages for platform %ls and cooking only packages that have been modified.",
		...
		COOK_STAT(DetailedCookStats::IsIncrementalCook = IsIncremental());
		
		if (PlatformData->bFullBuild && !bFullBuildAllowed)
		{
			UE_LOGF(LogCook, Fatal,
				"INCREMENTAL COOK: A diffonly cook mode was specified (-diffonly or -incrementalvalidate), " "and we guarantee state will not be wiped. But an incremental cook was found to not be possible (see logs above). Aborting the cook.");
		}
	}
}
...
{
	UE_SCOPED_COOKTIMER(BeginCookSandbox);
	using namespace UE::Cook;
...
#endif
	{
		...
		{
			const ITargetPlatform* TargetPlatform = PlatformContext.TargetPlatform;
			FPlatformData* PlatformData = PlatformContext.PlatformData;
			FCookSavePackageContext& SavePackageContext = FindOrCreateSaveContext(TargetPlatform);
			...
			if (!PlatformContext.bWorkerOnSharedSandbox)
			{
				...
				PlatformData->RegistryGenerator->CleanManifestDirectories(*SandboxFile, *this);
			}
			...
			{
				check(!IsCookWorkerMode());
				for (const TRefCountPtr<ICookArtifact>& Artifact: CookArtifacts)
				{
					FCookArtifactsCurrentSettings& Current
						= PlatformContext.CookArtifactsCurrentSettings.FindChecked(Artifact.GetReference());
					FString ArtifactName = Artifact->GetArtifactName();
					...
					if (!bLegacyBuildDependencies || PlatformContext.bFullBuild)
					{
						...
						{
							UE_LOGF(LogCook, Display,
								"COOKINCREMENTAL: CLEARING ARTIFACT: -cookincremental was specified, but %ls settings have changed and the artifact is invalidated. Deleting the artifact; it will be reconstructed from the packags that cook during this session.",
								*ArtifactName);
		...
		else if (AlreadyCookedPlatforms.Num())
		{
			...
			PackageDatas->LockAndEnumeratePackageDatas([TargetPlatform](FPackageData* PackageData)
			{
	...
	UE_LOGF(LogCook, Display, "Sandbox cleanup took %5.3f seconds for platforms %ls", CleanSandboxTime, *PlatformNames);
#endif
}
...
UE::Cook::FCookSavePackageContext* UCookOnTheFlyServer::CreateSaveContext(const ITargetPlatform* TargetPlatform)
{
	...
	const FString ResolvedMetadataPath = GetCookMetadataOutputFolderInternal(TargetPlatform);
	...
	if (IsUsingZenStore())
	{
		...
		ZenWriter->InitializeConnection();
		ZenReader->InitializeConnection();
...
const TSet<IPlugin*>* UCookOnTheFlyServer::GetEnabledPlugins(const ITargetPlatform* TargetPlatform) const
{
	using namespace UE::Cook;
...
void UCookOnTheFlyServer::FinalizePackageStore(const ITargetPlatform* TargetPlatform, const TSet<FName>& CookedPackageNames)
{
	...
	ICookedPackageWriter& CookWriter = FindOrCreatePackageWriter(TargetPlatform);
	// Collect end-of-cook metadata from artifacts and the asset manager.
	if (!CookInfo.bWorkerOnSharedSandbox)
	{
		Artifact::FStoreDataInOplogContext StoreDataContext;
		for (const TRefCountPtr<ICookArtifact>& Artifact : CookArtifacts)
		{
			Artifact->StoreDataInOplog(StoreDataContext);
		}
		for (Artifact::FStoreDataInOplogContext::FEntry& AdditionalOp : StoreDataContext.Entries)
		{
			CookWriter.AppendOp(AdditionalOp.Key, AdditionalOp.Value);
		}
	}
	CookWriter.EndCook(CookInfo);
}
...
void UCookOnTheFlyServer::StartCookByTheBook( const FCookByTheBookStartupOptions& CookByTheBookStartupOptions )
{
	...
	UE_LOGF(LogCook, Display, "---- Post Content Initialisation: Start ----");
	...
	UE::Cook::FDelegates::ConfigureCookSession.Broadcast(*this);
	CalculateGlobalDependenciesHashes();
	RegisterValidationCookArtifact();
	...
	UE_LOGF(LogCook, Display, "---- Post Content Initialisation: End ----");
	BroadcastCookStarted();
}
	...
	TOptional<FCookByTheBookStartupOptions>& ModifiedStartupOptions)
{
	...
	if (IsCookByTheBookMode() && !IsCookingInEditor())
	{
		UE_LOGF(LogCook, Display, "---- Pre Content Initialisation: End ----");
			...
			[this, &CookByTheBookStartupOptions, &ModifiedStartupOptions, &EffectiveStartupOptions, &bOutAbortCook] (FStringView CommandContents)
		{
			UE::String::ParseTokensMultiple(CommandContents, { ' ', '\t', '\r', '\n' },
				[this, &CookByTheBookStartupOptions, &ModifiedStartupOptions, &EffectiveStartupOptions, &bOutAbortCook] (FStringView Token)
				{
					...
					{
						UE_LOGF(LogCook, Display, "Received CookAbort token in cook command file, exiting cook.");
						bOutAbortCook = true;
					}
					...
					else if (Token.StartsWith(PackageToken))
					{
						...
						UE::String::ParseTokens(Token.RightChop(PackageToken.Len()), TEXT('+'),
							[&ModifiedStartupOptions](FStringView PackageToken)
							{
								...
								UE_LOGF(LogCook, Display, "Received Package token in cook command file, adding package '%.*ls' to cook workload.", PackageToken.Len(), PackageToken.GetData());
							}, UE::String::EParseTokensOptions::SkipEmpty);
					}
					else if (Token.StartsWith(TEXTVIEW("-DLCName=")))
					{
						...
						UE_LOGF(LogCook, Display, "Updated DLCName=%ls", *ModifiedStartupOptions->DLCName);
					}
					else if (Token.StartsWith(TEXTVIEW("-RunAssetValidation")))
					{
						...
						UE_LOGF(LogCook, Display, "RunAssetValidation enabled");
					}
					else if (Token.StartsWith(TEXTVIEW("-ValidationErrorsAreFatal")))
					{
						...
						UE_LOGF(LogCook, Display, "ValidationErrorsAreFatal enabled");
					}
						...
						for (const FString& CVarValueKey : CVars)
						{
							...
							if (CVarValueKey.Split(TEXT("="), &CVarKey, &CVarValue))
							{
								...
								if (ConsoleVariable)
								{
									...
									UE_LOGF(LogCook, Display, "Set CVar %ls", *CVarValueKey);
								}
								else
								{
									UE_LOGF(LogCook, Display, "Failed to find cvar '%ls'", *CVarKey);
								}
							}
							else
							{
								UE_LOGF(LogCook, Warning, "Failed to parse cvar from:%ls", *CVarValueKey);
							}
						}
					}
					else if (Token.StartsWith(TEXTVIEW("-ini:")))
					{
						...
						if (Tokens.Num() == 4)
						{
							...
							{
								UE_LOGF(LogCook, Warning, "Failed to find config file:%ls", *Tokens[1]);
							}
						}
						else
						{
							UE_LOGF(LogCook, Warning, "Failed to parse ini from:%ls", *TokenValue);
						}
					}
					...
					{
						UE_LOGF(LogCook, Warning, "Ignoring unknown/unsupported token in cook command file: %.*ls", Token.Len(), Token.GetData());
					}
				}, UE::String::EParseTokensOptions::SkipEmpty);
		});
	}
...
{
	UE_SCOPED_COOKTIMER(CreateBeginCookByTheBookContext);
	...
	if (bZenStore)
	{
		bool bFallbackToLooseFileWriter = false;
		GConfig->GetBool(TEXT("CookSettings"), TEXT("FallbackToLooseFileWriter"), bFallbackToLooseFileWriter, GEditorIni);
		if (bFallbackToLooseFileWriter && !FZenStoreWriter::CanUse())
		{
			UE_LOGF(LogCook, Warning, "Zen storage not available, falling back to loose files");
			bZenStore = false;
		}
	}
	...
	CookByTheBookOptions->bDlcPluginOnly = FParse::Param(FCommandLine::Get(), TEXT("DlcPluginOnly"));
	if (CookByTheBookOptions->bSkipHardReferences && !CookByTheBookOptions->bSkipSoftReferences)
	{
		UE_LOGF(LogCook, Display, "Setting bSkipSoftReferences to true since bSkipHardReferences is true and skipping hard references requires skipping soft references.");
...
FBeginCookContext UCookOnTheFlyServer::CreateAddPlatformContext(ITargetPlatform* TargetPlatform)
{
	...
	PlatformContext.PlatformData = &PlatformManager->CreatePlatformData(*this, TargetPlatform);
...
{
	UE_SCOPED_COOKTIMER(GenerateInitialRequests);
	...
	if (FParse::Value(FCommandLine::Get(), TEXT("-CookPackageDir="), Text))
	{
		TArray<FString> Array;
		Text.ParseIntoArray(Array, CommandLineDelimiters.GetData(), CommandLineDelimiters.Num(), true);
		FARFilter Filter;
		Filter.bRecursivePaths = true;
		// search in all paths specified
		for (FString& PackageDir : Array)
		{
			Filter.PackagePaths.Add(*PackageDir);
		}

		// add each discovered package to the list of packages to cook
		TArray<FAssetData> FoundAssets;
		AssetRegistry->GetAssets(Filter, FoundAssets);
		for (FAssetData& AssetData : FoundAssets)
		{
			// add path to package, not any asset within
			CookMaps.AddUnique(AssetData.GetSoftObjectPath().GetLongPackageName());
		}
	}
	...
	for (const FString& PackageName : CookLastPackages)
	{
		...
		{
			UE_LOGF(LogCook, Error, "-CookFirst and -CookLast are mutually exclusive. Ignoring -CookLast for %ls.",
				*PackageName);
		}
		else
		{
	...
	if (!IsCookingDLC() && !BasedOnReleaseVersion.IsEmpty())
	{
		...
		UE_CLOGF(CreateReleaseVersion.IsEmpty(), LogCook, Fatal, "-BasedOnReleaseVersion must be used together with either -dlcname or -CreateReleaseVersion.");
...
{
	UE_SCOPED_COOKTIMER(RecordDLCPackagesFromBaseGame);
	using namespace UE::Cook;
	...
	// If we're only cooking the DLC plugin we don't need Development Asset Registry.
	// All references outside the plugin are never explored or cooked, just mark any
	// existing PackageData's as already cooked.
	if (IsCookingDLCPluginOnly())
	{
		FString DLCPath = GetMountedAssetPathForDLC();
		UE_LOGF(LogCook, Log, "Performing plugin only cook for: %ls", *DLCPath);

		PackageDatas->LockAndEnumeratePackageDatas([&BeginContext,&DLCPath, this](FPackageData* PackageData)
		{
			FString PackageName = PackageData->GetPackageName().ToString();

			if (!FStringView(PackageName).StartsWith(DLCPath))
			{
				for (const ITargetPlatform* TargetPlatform : BeginContext.TargetPlatforms)
				{
				...
				// Incremented only for the first platform, similar to 
				// NumPackagesIncrementallySkipped and other relevant stats. 
	...
	(TArray<FConstructPackageData>& OutPackageList, const FString& InPlatformName)
	{
		...
		if (bSucceeded)
		{
			UE_LOGF(LogCook, Log, "Loaded assetregistry: %ls", *OriginalSandboxRegistryFilename);
		}
		else
		{
			UE_LOGF(LogCook, Log, "Failed to load DevelopmentAssetRegistry for platform %ls. Attempted the following names:\n%ls", *InPlatformName, *FString::Join(AttemptedNames, TEXT("\n")));
		}
		return bSucceeded;
	};
	...
	if (bUsingDevRegistryOverride)
	{
		...
		{
			UE_LOGF(LogCook, Fatal,
				"%ls based-on AssetRegistry file %ls for DevelopmentAssetRegistryPlatformOverride %ls. "
				"When cooking DLC, if DevelopmentAssetRegistryPlatformOverride is specified %ls is expected to exist under Release/<override> and contain some valid data. Terminating the cook.",
	...
	for (const ITargetPlatform* TargetPlatform : BeginContext.TargetPlatforms)
	{
		...
		if (!bUsingDevRegistryOverride)
		{
			...
			{
				UE_LOGF(LogCook, Fatal,
					"Could not find based-on AssetRegistry file %ls for platform %ls. "
					"When cooking DLC, %ls is expected to exist Release/<platform> for each platform being cooked. (Or use DevelopmentAssetRegistryPlatformOverride=<PlatformName> to specify an override platform that all platforms should use to find the %ls file). Terminating the cook.",
	...
	if (bUsingExtraReleaseVersionAssets)
	{
		...
		for (const FString& AssetPath : OutAssetPaths)
		{
			...
			{
				UE_LOGF(LogCook, Error, "Failed to resolve package data for ExtraReleaseVersionAsset [%ls]", *AssetPath);
			}
		}
	}
}
...
{
	UE_SCOPED_COOKTIMER(BeginCookPackageWriters);
...
{
	UE_SCOPED_COOKTIMER(SelectSessionPlatforms);
...
{
	UE_SCOPED_COOKTIMER(CalculateGlobalDependenciesHashes);
...
{
	UE_SCOPED_COOKTIMER(BeginCookEditorSystems);
	if (!IsCookingInEditor())
	{
		return;
	}
...
{
	UE_SCOPED_COOKTIMER(BeginCookDirector);
	if (CookDirector)
	{
...
{
	UE_SCOPED_COOKTIMER(BeginCookEDLCookInfo);
	if (IsCookingInEditor())
	{
		return;
	}
...
{
	UE_SCOPED_COOKTIMER(RecordCookByTheBookDelegates);
	if (!IsCookingInEditor())
	{
	...
	auto AddDirectoryPathArray = [&NeverCookDirectories](const TArray<FDirectoryPath>& DirectoriesToNeverCook, const TCHAR* SettingName)
	{
		for (const FDirectoryPath& DirToNotCook : DirectoriesToNeverCook)
		{
			...
			else
			{
				...
				UE_LOGF(LogCook, Verbose, "'%ls' has invalid element '%ls'", SettingName, *DirToNotCook.Path);
			}
		}
...
void UCookOnTheFlyServer::MaybeMarkPackageAsAlreadyLoaded(UPackage *Package)
{
	...
	if (PackageData->HasAnyCookedPlatform())
	{
		...
		if (IsCookFlagSet(ECookInitializationFlags::LogDebugInfo))
		{
			...
			{
				UE_LOGF(LogCook, Display, "Reloading package %ls slowly because it wasn't cooked for all platforms %ls.", *StandardName.ToString(), *Platforms);
			}
			else
			{
				UE_LOGF(LogCook, Display, "Marking %ls as reloading for cooker because it's been cooked for platforms %ls.", *StandardName.ToString(), *Platforms);
			}
		}
	}
...
void UCookOnTheFlyServer::GetCookOnTheFlyUnsolicitedFiles(const ITargetPlatform* TargetPlatform, const FString& PlatformName, TArray<FString>& UnsolicitedFiles, const FString& Filename, bool bIsCookable)
{
	...
	for (const FName& UnsolicitedFile : UnsolicitedFilenames)
	{
		...
		{
			UE_LOGF(LogCook, Warning, "Unsolicited file doesn't exist in sandbox, ignoring %ls", *StandardFilename);
		}
	}
}
	...
	bool bReevaluateUncookedPackages, TArray<UE::Cook::FConstructPackageData>& OutPackageDatas) const
{
	...
	if (Reader)
	{
		...
		if (AssetRegistryPath == GPreloadedARPath)
		{
			...
			UE_LOGF(LogCook, Display, "Blocked %.4f ms waiting for AR to finish loading", TimeWaiting * 1000.0);
		...
		ParallelFor(NumAssets,
			[&](int32 Index)
			{
				...
				{
					UE_LOGF(LogCook, Warning, "Could not resolve package %ls from %ls",
...
UE::Cook::ICookArtifactReader& UCookOnTheFlyServer::FindOrCreateCookArtifactReader(
	const ITargetPlatform* TargetPlatform)
{
	return *FindOrCreateSaveContext(TargetPlatform).ArtifactReader;
}

const UE::Cook::ICookArtifactReader* UCookOnTheFlyServer::FindCookArtifactReader(
...
{
	UE_SCOPED_COOKTIMER(InitializeAllCulturesToCook);
...
{
	UE_SCOPED_COOKTIMER(CompileDLCLocalization);
	if (!IsCookingDLC())
	{
		return;
	}
	if (!GetDefault<UUserGeneratedContentLocalizationSettings>()->bCompileDLCLocalizationDuringCook)
	{
		UE_LOGF(LogCook, Display, "DLC plugin '%ls' skipped compiling UGC localization as bCompileDLCLocalizationDuringCook is false.", *CookByTheBookOptions->DlcName);
		return;
	}
	...
	if (TSharedPtr<IPlugin> Plugin = IPluginManager::Get().FindPlugin(CookByTheBookOptions->DlcName))
	{
		if (Plugin->GetDescriptor().LocalizationTargets.Num() > 0)
		{
			...
			for (const FLocalizationTargetDescriptor& LocalizationTarget : Plugin->GetDescriptor().LocalizationTargets)
			{
				...
				const FString InputLocalizationTargetDirectory = UserGeneratedContentLocalization::GetLocalizationTargetDirectory(LocalizationTarget.Name, InputContentDirectory);
				for (const FBeginCookContextPlatform& PlatformContext : BeginContext.PlatformContexts)
				{
					const FString PlatformName = PlatformContext.TargetPlatform->PlatformName();
					const FString OutputContentDirectory = ConvertToFullSandboxPath(InputContentDirectory, /*bForWrite*/true, PlatformName);
					const FString OutputLocalizationTargetDirectory = ConvertToFullSandboxPath(InputLocalizationTargetDirectory, /*bForWrite*/true, PlatformName);
					if (UserGeneratedContentLocalization::CompileLocalization(LocalizationTarget.Name, InputLocalizationTargetDirectory, OutputLocalizationTargetDirectory, GetDefault<UUserGeneratedContentLocalizationSettings>()->bValidateDLCLocalizationDuringCook ? &DefaultUGCLocDescriptor : nullptr))
					{
						IPlatformFile& PlatformFile = FPlatformFileManager::Get().GetPlatformFile();
						PlatformFile.IterateDirectoryRecursively(*OutputLocalizationTargetDirectory,
							[this, &PlatformName, &OutputContentDirectory](const TCHAR* Path, bool bIsDirectory) -> bool
							{
								if (!bIsDirectory)
								{
									FString RelativePath = Path;
									FPaths::MakePathRelativeTo(RelativePath, *OutputContentDirectory);
									UE_LOGF(LogCook, Display, "DLC plugin '%ls' UGC localization compile generated '%ls' (%ls).", *CookByTheBookOptions->DlcName, *RelativePath, *PlatformName);
								}
								return true;
							});
					}
		...
		{
			UE_LOGF(LogCook, Display, "DLC plugin '%ls' skipped compiling UGC localization as it has no localization targets.", *CookByTheBookOptions->DlcName);
		}
	}
	else
	{
		UE_LOGF(LogCook, Display, "DLC plugin '%ls' was not known to IPluginManager. UGC localization has not been compiled and will be missing at runtime.", *CookByTheBookOptions->DlcName);
	}
}

void UCookOnTheFlyServer::GenerateLocalizationReferences()
{
	UE_SCOPED_COOKTIMER(GenerateLocalizationReferences);
	...
	UE_LOGF(LogCook, Display, "Discovering localized assets for cultures: %ls", *FString::Join(CookByTheBookOptions->AllCulturesToCook, TEXT(", ")));
	...
	UE_LOGF(LogCook, Display, "Found %d localized assets", AssetDataForCultures.Num());
...
void UCookOnTheFlyServer::RegisterLocalizationChunkDataGenerator()
{
	...
	FLocalizationCookArtifact* LocalizationArtifact = new FLocalizationCookArtifact(CookByTheBookOptions->AllCulturesToCook);
	TRefCountPtr<UE::Cook::ICookArtifact> LocalizationRefCounter(LocalizationArtifact);
	RegisterArtifact(LocalizationArtifact);
}
	...
	const ITargetPlatform* TargetPlatform, UE::Cook::ECachedCookedPlatformDataEvent* ExistingEvent)
{
	...
	// Set scopes for e.g. TObjectPtr reads around the call to BeginCacheForCookedPlatformData.
	// When called from inside an existing tracking scope (e.g. SavePackage), preserve the outer
	// scope's tracking op rather than overriding it with NAME_CookerBuildObject.
	UE::Cook::FScopedCookActivePackage ScopedActivePackage(PackageName,
#if UE_WITH_OBJECT_HANDLE_TRACKING
		PackageAccessTracking_Private::FPackageAccessRefScope::GetCurrentThreadScope()
			? NAME_None : PackageAccessTrackingOps::NAME_CookerBuildObject
#else
		FName()
#endif
	);
	...
	const ITargetPlatform* TargetPlatform, UE::Cook::ECachedCookedPlatformDataEvent* ExistingEvent)
{
	...
	// Set scopes for e.g. TObjectPtr reads around the call to IsCachedCookedPlatformDataLoaded.
	// When called from inside an existing tracking scope (e.g. SavePackage), preserve the outer
	// scope's tracking op rather than overriding it with NAME_CookerBuildObject.
	UE::Cook::FScopedCookActivePackage ScopedActivePackage(PackageName,
#if UE_WITH_OBJECT_HANDLE_TRACKING
		...
		&& *ExistingEvent != ECachedCookedPlatformDataEvent::IsCachedCookedPlatformDataLoadedCalled)
	{
		...
		UE_LOGF(LogCook, Display,
			"%ls was reallocated after BeginCacheForCookedPlatformData and before IsCacheCookedPlatformData returned true. Calling BeginCacheForCookedPlatformData on it again.",
			*Obj->GetFullName());
		Obj->BeginCacheForCookedPlatformData(TargetPlatform);
		*ExistingEvent = ECachedCookedPlatformDataEvent::BeginCacheForCookedPlatformDataCalled;
	}
	...
	if (!PendingObjects.IsEmpty())
	{
		...
		for (;;)
		{
			...
			if (FPlatformTime::Seconds() > EndTimeSeconds)
			{
				...
				{
					UE_LOGF(LogCook, Warning,
						"SavePackageBeginCacheForCookedPlatformData Error for package %ls: GetNumPendingCookedPlatformData() != 0 but no Culprit found. Ignoring it and continuing.",
						*PackageName.ToString());
					break;
				}
				UE_LOGF(LogSavePackage, Error, "Save of %ls failed: timed out waiting for IsCachedCookedPlatformDataLoaded on %ls.",
...
void ConditionalWaitOnCommandFile(FStringView GateName, TFunctionRef<void (FStringView)> CommandHandler)
{
	...
	UE_LOGF(LogCook, Display, "---- Waiting For Content: Start ----");
	...
	{
		UE_LOGF(LogCook, Display, "Waiting for %.*ls command file at %ls...", GateName.Len(), GateName.GetData(), *WaitOnCommandFile);
		...
		while (!FLockFile::TryReadAndClear(*WaitOnCommandFile, CommandContents))
		{
			...
			if (FPlatformTime::ToSeconds64(LoopTime - LastMessageTime) > 60.f)
			{
				...
				UE_LOGF(LogCook, Display, "Waited %.1fs for %.*ls command file at %ls...", TimeSinceWaitStartTime, GateName.Len(), GateName.GetData(), *WaitOnCommandFile);
	...
	UE_LOGF(LogCook, Display, "---- Waiting For Content: End ----");
}

void UCookOnTheFlyServer::BroadcastCookStarted()
{
	UE_SCOPED_COOKTIMER(BroadcastCookStarted);
