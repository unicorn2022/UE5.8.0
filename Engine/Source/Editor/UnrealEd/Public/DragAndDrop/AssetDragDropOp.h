...
private:
	...
	/**
	 * Cached asset data with refreshed tags from the asset registry.  The FAssetData captured at drag start may have stale
	 * on-disk (Fixed) tags that are missing dynamically computed values (e.g. IsSourceValid).  After the asset is loaded
	 * during drag-over, the registry has fresh Loose tags.  Populated once per drag op by ExtractAssetDataFromDrag.
	 */
	mutable TOptional<TArray<FAssetData>> RefreshedAssetData;

	/** Allow access to RefreshedAssetData from ExtractAssetDataFromDrag. */
	friend TArray<FAssetData> AssetUtil::ExtractAssetDataFromDrag(const TSharedPtr<const FDragDropOperation>& Operation);
