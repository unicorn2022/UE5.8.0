...
/**
 * Arguments for FOnPreDestructiveAssetActionWithArgs delegate
 */
struct FOnPreDestructiveAssetActionArgs
{
public:
	FOnPreDestructiveAssetActionArgs(const TArray<UObject*>& InObjects, const EDestructiveAssetActions InAction) :
		Objects(InObjects),
		Action(InAction)
	{
	...
	FOnPreDestructiveAssetActionArgs(const TArray<UObject*>& InObjects, const EDestructiveAssetActions InAction, TArrayView<const FTopLevelAssetPath> InDestinations) :
		Objects(InObjects),
		Action(InAction),
	...
	FOnPreDestructiveAssetActionArgs(const FOnPreDestructiveAssetActionArgs&) = delete;
	FOnPreDestructiveAssetActionArgs(FOnPreDestructiveAssetActionArgs&&) = delete;

	const TArray<UObject*>& Objects;
	const EDestructiveAssetActions Action;

	/* Destinations must equal the number of objects when Action set to AssetMove */
	const TArrayView<const FTopLevelAssetPath> Destinations;
...
struct FEditorDelegates
{
	...
	/** delegate for when assets are about to undergo a destructive action caused by the Editor UI (Delete, Rename, Move, Privatize, etc.) */
	DECLARE_MULTICAST_DELEGATE_OneParam(FOnPreDestructiveAssetActionWithArgs, FOnPreDestructiveAssetActionArgs&);
	...
	UE_DEPRECATED(5.8, "Use FOnPreDestructiveAssetActionWithArgs instead.")
	static UNREALED_API FOnPreDestructiveAssetAction OnPreDestructiveAssetAction;
	/** Called when an asset is about to undergo a destructive action caused by the Editor UI (Delete, Move, Rename, Privatize, etc.) */
	static UNREALED_API FOnPreDestructiveAssetActionWithArgs OnPreDestructiveAssetActionWithArgs;
...
namespace EditorUtilities
{
	...
	struct FCopyOptions
	{
		...
		FCopyOptions(const ECopyOptions::Type InFlags, TFunction<bool(FProperty&, UObject&)> InPropertyFilter = TFunction<bool(FProperty&, UObject&)>()) 
			: Flags(InFlags)
