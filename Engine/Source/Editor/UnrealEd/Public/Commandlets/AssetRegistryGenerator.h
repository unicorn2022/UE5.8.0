...
public:
	...
	void Initialize();
	/**
	 * Called at the beginning of CookOnTheFly to bootstrap the targetplatform's AssetRegistry by copying it from the
	 * editor's AssetRegistry.
	 */
	void InitializeAssetRegistryForCookOnTheFly(const FAssetRegistrySerializationOptions& DevelopmentSaveOptions);
	...
	/** Reads Generator packages and other cooking data from the previous incrementalcook's DevelopmentAssetRegistry. */
	void ReportGeneratorPackages(const FAssetRegistryState& PreviousState,
		TMap<FName, FGeneratorPackageInfo>& OutGeneratorPackages, int32& OutNumNeverCookPlaceHolderPackages);

	/**
	 * Given the list of packages still in the oplog, remove any packages from the AssetRegistryState that are no longer
	 * in the oplog (they were not referenced in the current session and were pruned for being too old or invalid).
	 */
	void RemoveStalePackages(const TMap<FName, UE::Cook::Artifact::FOplogPackageData>& OplogPackages);

	/**
	 * Add or update data in the generated AssetRegistry for non-package data such as management references from
	 * PrimaryAssets and /Script PackageDatas.
	 */
	void AddGlobalsToCookedAssetRegistry(UCookOnTheFlyServer& COTFS);
	...
	void FinalizeChunkIDs(const TSet<FName>& CookedPackages, const TSet<FName>& DevelopmentOnlyPackages,
		...
		const TSet<FName>& StartupPackages, const FAssetRegistrySerializationOptions& DevelopmentSaveOptions);
	...
	*/
	 ...
	 */
	bool CleanManifestDirectories(UE::Cook::FCookSandbox& InSandboxFile, UCookOnTheFlyServer& COTFS);
	 ...
	 */
	bool SaveManifests(UE::Cook::FCookSandbox& InSandboxFile, UCookOnTheFlyServer& COTFS,
		int64 InOverrideChunkSize = 0, const TCHAR* InManifestSubDir = nullptr);

	/**
	* Saves generated asset registry data for each platform.
	*/
	bool SaveAssetRegistry(const FString& SandboxPath, bool bSerializeDevelopmentAssetRegistry,
		bool bForceNoFilterAssets, uint64& OutDevelopmentAssetRegistryHash, const TSet<FName>& CookedPackages,
...
private:
	 ...
	 * FGeneratorPackageInfo returned from ComputePackageDifferences that are used for incremental cook,
	...
	*/
	...
	/** The output path for this generator's platform where chunk manifests are written for use during staging. */
	FString GetChunkManifestDirectory(UCookOnTheFlyServer& COTFS) const;
...
{
public:
	virtual ~IAssetRegistryReporter() = default;
