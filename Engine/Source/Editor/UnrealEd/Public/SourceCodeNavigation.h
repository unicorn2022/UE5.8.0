...
{
public:
	virtual ~ISourceCodeNavigationHandler() = default;
...
public:
	...
	*/
	...
	/** Opens a single source file asynchronously, showing a notification while the IDE loads. */
	UNREALED_API static void OpenSourceFileAsync(const FString& AbsoluteSourcePath, int32 LineNumber = 0, int32 ColumnNumber = 0);

	/** Opens multiple source files asynchronously, showing a notification while the IDE loads. */
	UNREALED_API static void OpenSourceFilesAsync(const TArray<FString>& AbsoluteSourcePaths);
