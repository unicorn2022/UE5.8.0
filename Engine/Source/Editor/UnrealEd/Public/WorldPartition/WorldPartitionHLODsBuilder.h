...
public:
	virtual void SetUseSlowTask(bool bInUseSlowTask) { bUseSlowTask = bInUseSlowTask; }
	virtual void SetSaveActors(bool bInSaveActors) { bSaveActors = bInSaveActors; }
	virtual void SetConsiderUnsavedHLODActors(bool bInConsiderUnsavedHLODActors) { bConsiderUnsavedHLODActors = bInConsiderUnsavedHLODActors; }
	virtual void SetFilters(const TArray<TSharedPtr<IHLODCreationFilter>> InFilters) { Filters = InFilters; }

	UNREALED_API static bool BuildWithFilters(UWorld* InWorld, const TArray<TSharedPtr<IHLODCreationFilter>>& InFilters);

	DECLARE_MULTICAST_DELEGATE(FOnWorldPartitionHLODBuildWithFiltersCompleted);
	static FOnWorldPartitionHLODBuildWithFiltersCompleted& GetOnWorldPartitionHLODBuildWithFiltersCompleted() { return OnWorldPartitionHLODBuildWithFiltersCompleted; }
...
protected:
	...
	EHLODRebuildPolicyDecision EvaluateHLODBuildConditions(class AWorldPartitionHLOD* HLODActor, const FHLODRebuildPolicyDataSet& InOldData, const FHLODRebuildPolicyDataSet& InNewData);
...
private:
	...
	static UNREALED_API FOnWorldPartitionHLODBuildWithFiltersCompleted OnWorldPartitionHLODBuildWithFiltersCompleted;
};