...
struct FBuildOptions
{
	...
	/** Build HLODs for selected actors */
	UNREALED_API static const FName BuildHierarchicalLOD_SelectedActors;
	/** Build minimap */
...
public:
	...
	*/
	 ...
	 * @param OnProcRunningTick Function that will be called periodically while the builder process is running (can be used to update progress).
	 * 
	...
	static UNREALED_API bool RunWorldPartitionBuilder(
		...
		TFunction<void(FScopedSlowTask&)> UpdateSlowTask = TFunction<void(FScopedSlowTask&)>()
		);
private:
