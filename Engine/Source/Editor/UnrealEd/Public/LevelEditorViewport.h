...
public:
	...
	/** Returns the current drop preview elements, or a null pointer if there are none. */
	const TSharedPtr<FTypedElementList>& GetDropPreviewElements() const
	{
		return DropPreviewElements;
	}
	...
	UNREALED_API virtual bool OnTransformCanceled(const FGizmoState& InState) override; 
	 ...
	 */
	UE_DEPRECATED(5.8, "View is redundant so use method with no parameters")
	UNREALED_API void UpdateAudioListener( const FSceneView& View );
	
	/**
	 * Updates the audio listener for this viewport.
	 */
	UNREALED_API void UpdateAudioListener();
	 ...
	 * @param bInvalidateHitProxies Whether to invalidate hit proxies when refreshing the viewport. Set to false during mouse-look to avoid unnecessary rebuilds.
	 */
	UNREALED_API void MoveCameraToLockedActor(const bool bInvalidateHitProxies = true);
	...
	/**
	 * Duplicates the current Level Editor selection. Newly created elements can be selected after creation or not.
	 * E.g. Alt + Gizmo Drag creates a copy and then selects it, while pressing Alt while Gizmo Drag is being performed
	 * "stamps" a duplicate of the dragged selection, without selecting it.
	 *
	 * @param bInSelectNewElements whether the duplicates should be selected or not
	 */
	UNREALED_API void DuplicateSelection(bool bInSelectNewElements);
...
private:
	// Currently used by a couple friended classes 
	// TODO: Should probably either rewrite those to not use it, or make some element-based version public
	UNREALED_API FTransform GetPreDragActorTransform(const AActor* InActor) const;

	/**
	 * Checks to see the viewports locked actor need updating
	 * @param InActor The actor to check against each viewport's locked actor.
	 * @param bCheckRealtime If true, only update viewports that have realtime enabled.
	 * @param bInvalidateHitProxies Whether to invalidate hit proxies when refreshing the viewport. Set to false during mouse-look to avoid unnecessary rebuilds.
	 */
	UNREALED_API void UpdateLockedActorViewports(const AActor* InActor, const bool bCheckRealtime, const bool bInvalidateHitProxies = true);

	/**
	 * Checks to see the viewports locked actor need updating
	 * @param InActor The actor to check against this viewport's locked actor.
	 * @param bCheckRealtime If true, only update this viewport if realtime is enabled.
	 * @param bInvalidateHitProxies Whether to invalidate hit proxies when refreshing the viewport. Set to false during mouse-look to avoid unnecessary rebuilds.
	 */
	UNREALED_API void UpdateLockedActorViewport(const AActor* InActor, const bool bCheckRealtime, const bool bInvalidateHitProxies = true);
	...
	*/
	...
	/** Project the specified elements into the world according to the current drag parameters */
	void ProjectSelectedElementsIntoWorld(FViewport* Viewport, const FVector& Drag, const FRotator& Rot);
...
private:
	...
	/** Map of element locations before a drag operation, keyed by their handle. */
