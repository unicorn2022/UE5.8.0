...
public:
	...
	/** Callback for toggling the geometry inspection show wireframe flag. */
	UNREALED_API void ToggleGeometryInspectionShowWireframe();

	/** Callback for checking the geometry inspection show wireframe flag. */
	UNREALED_API bool IsSetGeometryInspectionShowWireframe() const;

	/** Callback for setting a specific channel mask flag */
	UNREALED_API void SetChannelMask(EColorChannelMask InColorChannelMask);

	/** Callback for checking a specific channel mask flag */
	UNREALED_API bool IsChannelBeingMasked(EColorChannelMask InColorChannelMask) const;
	...
	UNREALED_API virtual bool InputGesture(FViewport* Viewport, const FInputDeviceId DeviceId, EGestureEvent GestureType, EGesturePhase GesturePhase, const FVector2D& GestureDelta, bool bIsDirectionInvertedFromDevice, const uint64 Timestamp) override;
	...
	virtual bool OnTransformCanceled(const FGizmoState& InState) { return EndTransform(InState); }
...
public:
	UNREALED_API virtual void AddAssociation(FViewport& InViewport) override;
	UNREALED_API virtual void RemoveAssociation(FViewport& InViewport) override;
...
public:
	...
	/**
	 * Sets how the viewport is displayed (Geometry Inspection Modes) and displays warning
	 * 
	 * @param	InViewModeIndex				View mode to set for the current viewport type
	 */
	UNREALED_API virtual void SetGeometryInspectionViewMode(EViewModeIndex InViewModeIndex);
...
public:
	...
	/**
	 * Changes the MegaLights visualization mode for this viewport.
	 *
	 * @param InName	The ID of the required visualization mode
	 */
	UNREALED_API void ChangeMegaLightsVisualizationMode(FName InName);

	/**
	 * Checks if a MegaLights visualization mode is selected.
	 *
	 * @param InName	The ID of the required visualization mode
	 * @return	true if the supplied MegaLights visualization mode is checked
	 */
	UNREALED_API bool IsMegaLightsVisualizationModeSelected(FName InName) const;

	/**
	 * Returns the FText display name associated with CurrentMegaLightsVisualizationMode.
	 */
	UNREALED_API FText GetCurrentMegaLightsVisualizationModeDisplayName() const;
	...
	*/
	...
	/**
	 * Translates the viewport camera by the specified amount.
	 */
	UNREALED_API void TranslateViewportCamera(const FVector& InTranslation);
...
private:
	...
	/**
	 * Updates the camera based on external movement sources.
	 */
	bool ConsumeExternalCameraMovement();

	/**
	 * Directs the camera to orbit based on a screen space movement delta
	 */
	void OrbitPerspectiveCamera(const FRotator& InOrbit);

	/**
	 * Dollies the camera towards or away from its orbit point
	 */
	void DollyPerspectiveCamera(double InDelta);
...
public:
	...
	struct FColorChannelMaskParams
	{
		...
		EColorChannelMask ColorChannelMask;
...
public:
	...
	{
	public:
		virtual ~ICustomTemporalUpscalerData() = default;
	};
...
private:
	...
	/** true if Geometry Inspection Mode Wireframe option is on. */
	bool bIsGeometryInspectionWireframeOn = true;
};
