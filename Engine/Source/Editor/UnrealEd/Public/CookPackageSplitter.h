...
public:
	...
	virtual ~ICookPackageSplitter() = default;

	/**
	 * Called immediately after construction. This is useful for adding a reference to the OwnerObject
	 * to prevent it from being garbage collected, if the class does not want to allow that possibility.
	 */
	virtual void Initialize(UPackage* OwnerPackage, UObject* OwnerObject)
	{
	}
	...
	/**
	 * If true, this splitter requires that the given generated package be empty before calling the populate function.
	 * Returning false will allow the splitter to precreate packages instead of relying on the cooker to create them.
	 */
	virtual bool RequiresEmptyPackageBeforePopulate(const UPackage* Package)
	{
		return true;
	}
	 ...
	 *
	 * If this splitter returns false for virtual RequiresEmptyPackageBeforePopulate(GeneratedPackage), it can add or
	 * move objects into that generated package during PopulateGeneratorPackage. Doing so will prevent the targeted
	 * generated package from being garbage collected until the generator package can be destroyed. This is worse
	 * performance, but can be useful to reduce Splitter complexity.
	 * 
	 * If the splitter returns true for virtual RequiresEmptyPackageBeforePopulate(GeneratedPackage), adding or moving
	 * objects into that generated package during PopulateGeneratorPackage is an error, and those objects will be
	 * missing from the generated package when it is saved.
	 *
...
private:
	UE_DEPRECATED(5.7, "Deprecation support, do not read/write outside of cooker code.")
...
public:
	...
	virtual ICookPackageSplitter* CreateInstance(UPackage* OwnerPackage, UObject* OwnerObject) const = 0;
...
friend class UE_JOIN(SplitterClass, SplitDataClass, _Register)

#define REGISTER_COOKPACKAGE_SPLITTER(SplitterClass, SplitDataClass) \
class UE_JOIN(SplitterClass, SplitDataClass, _Register) \
...
{ \
	...
	virtual ICookPackageSplitter* CreateInstance(UPackage* OwnerPackage, UObject* OwnerObject) const override \
	{ \
		ICookPackageSplitter* Result = new SplitterClass(); \
		Result->Initialize(OwnerPackage, OwnerObject); \
		return Result; \
	} \
...
namespace UE_JOIN(SplitterClass, SplitDataClass) \
{ \
	static UE_JOIN(SplitterClass, SplitDataClass, _Register) DefaultObject; \
}
