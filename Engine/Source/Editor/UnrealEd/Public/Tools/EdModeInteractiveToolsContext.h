...
public:
	...
	// Convert any pending PostInvalidation timestamp bumps into ViewportClient->Invalidate() calls
	UE_API virtual void PropagateInvalidations(FEditorViewportClient* ViewportClient);
	...
	/*
	* Configure whether tools should shutdown when the level gets changed.
	* By default, the context will shut down any active tools on level change.
	* Setting this to false will prevent this, but the system makes no promises about whether tools work properly across a level switch.
	* Therefore, do not use this if you do not know for certain that it is safe to persist tools across a level change in your situation.
	*/
	UE_API void SetDeactivateToolsOnLevelChange(bool bDeactivateTools);

	/** @return true if tools should be deactivated when level change is started */
	bool GetDeactivateToolsOnLevelChange() const { return bDeactivateOnLevelChange; }
...
protected:
	...
	/** Returns a ViewInteractionState based on the input ViewportClient, and context's ModeManager. */
	UE_API EViewInteractionState GetViewInteractionStateFromViewportClient(const FEditorViewportClient* InViewportClient) const;
...
private:
	...
	bool bDeactivateOnLevelChange = true;
};

DECLARE_MULTICAST_DELEGATE_OneParam(FGetUnsupportedInteractions, TArray<FName>& /* Interactions */)
...
public:
	...
	UE_API virtual void PropagateInvalidations(FEditorViewportClient* ViewportClient) override;

	UE_API virtual bool ProcessEditDelete() override;

	/**
	 * Called when a new interaction is about to be added. Argument is a TArray<FName>& list of interactions which should not be currently
	 * supported. For example, an Ed Mode could prevent a specific interaction from being added by registering to this delegate, and adding the interaction name to the array.
	 */
	FGetUnsupportedInteractions& OnGetUnsupportedInteractions()
	{
		return GetUnsupportedInteractionsDelegate;
	}
...
public:
	...
	UE_EXPERIMENTAL(5.8, "The extension of the visibility override interface into the editor context is experimental")
	/** @return true when the out values have been set. */
	UE_API bool GetCursorVisibilityOverride(bool& bOutHardwareCursorVisible, bool& bOutSoftwareCursorVisible) const;
...
protected:
	...
	FDeviceButtonState OnGetButtonStateFallback(const FKey& Key) const;
...
private:
	void UpdateMousePosition(FEditorViewportClient* InViewportClient, FViewport* InViewport, int32 X, int32 Y);
	void OnApplicationFocusChanged(bool bInIsFocused);
	
	bool bIsTrackingMouse;

	FGetUnsupportedInteractions GetUnsupportedInteractionsDelegate;
...
public:
	...
	*/
	 ...
	 */
	UE_DEPRECATED(5.8, "Drag tools have been merged with VIewport Interactions")
	void SetDragToolsEnabled(bool bInEnabled) { SetViewportInteractionsEnabled(bInEnabled); }

	/**
	 * Enable ITF Input tools support (e.g. Viewport Interactions as camera movement and selection)
	 */
	UE_API void SetViewportInteractionsEnabled(bool bInEnabled);
	 ...
	 */
	UE_DEPRECATED(5.8, "Drag tools have been merged with VIewport Interactions")
	bool UsesDragTools() const { return UsesViewportInteractions(); } 

	/**
	 * Are Viewport Interactions supported? e.g. camera movements, commands, clicking, etc
	 */
	UE_API bool UsesViewportInteractions() const;
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	UE_DEPRECATED(5.8, "Use OnBuildViewportInteractions instead.")
	DECLARE_MULTICAST_DELEGATE(FViewportInteractionsRegisterStateChanged)
	UE_DEPRECATED(5.8, "Use OnBuildViewportInteractions instead.")
	FViewportInteractionsRegisterStateChanged& OnRegisterViewportInteractions()
	{
		return OnRegisterViewportInteractionsDelegate;
	}
	UE_DEPRECATED(5.8, "Use OnBuildViewportInteractions instead.")
	FViewportInteractionsRegisterStateChanged OnRegisterViewportInteractionsDelegate;
PRAGMA_ENABLE_DEPRECATION_WARNINGS

PRAGMA_DISABLE_EXPERIMENTAL_WARNINGS	
	UE_EXPERIMENTAL(5.8, "Viewport interactions are experimental")
	DECLARE_MULTICAST_DELEGATE_OneParam(FViewportInteractionsSourceMessage, UViewportInteractionsBehaviorSource*)
	
	UE_EXPERIMENTAL(5.8, "Viewport interactions are experimental")
	DECLARE_MULTICAST_DELEGATE_OneParam(FViewportInteractionsSourceConstMessage, const UViewportInteractionsBehaviorSource*)
	
	UE_EXPERIMENTAL(5.8, "Viewport interactions are experimental")
	/**
	 * Called when viewport interactions are built. Add any custom interactions to the behavior source that is passed in via the event.
	 * Interactions are automatically removed when editor modes change.
	 */
	FViewportInteractionsSourceMessage& OnBuildViewportInteractions() { return OnBuildViewportInteractionsEvent; }


	UE_EXPERIMENTAL(5.8, "Viewport interactions are experimental")
	/**
	 * Called after viewport interactions are built. Can be used to apply any post-processing of behaviors after all behaviors are present.
	 */
	FViewportInteractionsSourceConstMessage& PostBuildViewportInteractions() { return PostBuildViewportInteractionsEvent; }
...
protected:
	...
	PRAGMA_DISABLE_EXPERIMENTAL_WARNINGS
	UE_EXPERIMENTAL(5.8, "Viewport interactions are experimental")
	FViewportInteractionsSourceMessage OnBuildViewportInteractionsEvent;
	
	UE_EXPERIMENTAL(5.8, "Viewport interactions are experimental")
	FViewportInteractionsSourceConstMessage PostBuildViewportInteractionsEvent;
	PRAGMA_ENABLE_EXPERIMENTAL_WARNINGS

	/** Whether viewport interactions can ever be supported */
	bool bUsesViewportInteractions = false;
	/** Whether the currently active editor modes currently support viewport interactions */
	bool bModesSupportViewportInteractions = false;
	/** A flag that allows de/composing viewport interactions once per set of mode changes */
	bool bViewportInteractionsDirty = false;
};
