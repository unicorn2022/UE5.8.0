...
public:
	...
	virtual ~FBasePinChangeHelper() = default;
...
public:
	...
	/**
	 * Helper function to get the blueprint that ultimately owns a node.
	 * This will assert if the node is inside a deleted graph.
	 *
	 * @param	InNode	Node to find the blueprint for.
	 * @return	A valid blueprint.
	 */
	static UNREALED_API TNotNull<UBlueprint*> FindBlueprintForNodeChecked(const UEdGraphNode* Node);
	...
	/**
	 * Helper function to get the blueprint that ultimately owns a graph.
	 * This will assert if the graph is for a deleted function.
	 *
	 * @param	InGraph	Graph to find the blueprint for.
	 * @return	A valid blueprint.
	 */
	static UNREALED_API TNotNull<UBlueprint*> FindBlueprintForGraphChecked(const UEdGraph* Graph);
	...
	/**
	 * Handles regenerating a blueprint class such that it is in a good state for saving, currently this is 
	 * required to get UserDefinedStruct types up to date before saving. It's likely that there may be
	 * other situations that require fixup in this way.
	 */
	static UNREALED_API void RecompileBeforeSaveIfNeeded(UBlueprint* Blueprint);
	...
	*/
	...
	/**
	 * Removes a local variable if it was declared in this blueprint and not in a base class.
	 *
	 * @param	InBlueprint			The Blueprint the local variable can be found in
	 * @param	InScope				Local variable's scope
	 ...
	 */
	static UNREALED_API FBPVariableDescription* FindLocalVariable(const UBlueprint* InBlueprint, const UEdGraph* InScopeGraph, const FName InVariableName, class UK2Node_FunctionEntry** OutFunctionEntry = nullptr);
	 ...
	 * @return					The local variable description
	 */
	static UNREALED_API FBPVariableDescription* FindLocalVariable(const UBlueprint* InBlueprint, const UStruct* InScope, const FName InVariableName, class UK2Node_FunctionEntry** OutFunctionEntry = nullptr);
	 ...
	 * @return					Local variable's name
	 */
	static UNREALED_API FName FindLocalVariableNameByGuid(const UBlueprint* InBlueprint, const FGuid& InVariableGuid);
	 ...
	 * @return					The Guid associated with the local variable
	 */
	static UNREALED_API FGuid FindLocalVariableGuidByName(const UBlueprint* InBlueprint, const UStruct* InScope, const FName InVariableName);
	 ...
	 * @return					The Guid associated with the local variable
	 */
	static UNREALED_API FGuid FindLocalVariableGuidByName(const UBlueprint* InBlueprint, const UEdGraph* InScopeGraph, const FName InVariableName);
	...
	*/
	...
	/** Removes all invalid structure members from the blueprint - e.g. because they were deleted */
