...
{
public:
	virtual ~IPropertyAccessEditor() = default;
	...
	/** 
	 * EXPERIMENTAL: Make a property binding menu for a struct.
	 * @param	MenuBuilder					The menu to add entries to
	 * @param	InArgs						arguments for filtering properties etc. 
	 * @param	InPropertyName				The Property Name to pass to the OnAddBinding callback
	 * @param	InOwnerStruct				The Struct/Class to get properties from
	 * @param	InBindingChain				The base binding chain to InOwnerStruct
	 */
	virtual void FillPropertyMenu(FMenuBuilder& MenuBuilder, const FPropertyBindingWidgetArgs& InArgs, FName InPropertyName, UStruct* InOwnerStruct,
										TArray<TSharedPtr<FBindingChainElement>> InBindingChain) const = 0;
