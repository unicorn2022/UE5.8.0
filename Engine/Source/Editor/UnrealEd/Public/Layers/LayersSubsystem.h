...
private:
	...
	// Update the layers column on the given actor's row in TEDS
	void AddActorToLayerInTeds(ULayer* Layer, AActor* Actor);
	void RemoveActorFromLayerInTeds(ULayer* Layer, AActor* Actor);
	
	// Internal function to update all actors visibility and also optionally update the TEDS state. This is only needed in specific 
	// situations and not every time UpdateAllActorsVisibility is called, but we also don't want to iterate all actors twice so the internal function
	// can be called manually after things like undo/redo when TEDS needs to be updated
	bool UpdateAllActorsVisibility_Internal(const bool bNotifySelectionChange, const bool bRedrawViewports, bool bUpdateTedsState = false);
	
	// Force sync the layers state for the given actor from the world to TEDS (stomping the old state in TEDS)
	void SyncActorLayersToTeds(AActor* Actor);
