...
protected:
	// protected destructor since IAssetEditorInstance isn't meant to be used to delete derived class instances
	~IAssetEditorInstance() =  default;
};
...
DECLARE_MULTICAST_DELEGATE_OneParam(FAssetEditorOpenDebugObject, UObject* /* DebugObject */);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FAssetEditorOpenDebugObjectDynamic, UObject*, DebugObject);
...
public:
	...
	/**
	 * Attempts to open a debug object in an already-open asset editor. Will not open the asset editor responsible for this
	 * type of object, only cause the editor to debug the specified object.
	 * Returns true if an asset editor accepted the debug object.
	 */
	UNREALED_API bool OpenDebugObjectInEditor(UObject* DebugObject);

	/**
	 * Returns the debug object delegate for a specific class.
	 */
	UNREALED_API FAssetEditorOpenDebugObject* GetDebugObjectDelegateForClass(UClass* InClass);
...
private:
	...
	/** Callbacks to open specific debug objects based on their type. */
	TMap<FName, FAssetEditorOpenDebugObject> DebugObjectDelegates;

	/** Callbacks to open specific debug objects exposed to blueprint. */
	UPROPERTY(BlueprintAssignable, meta = (AllowPrivateAccess = "true", Category = "Debug"))
	FAssetEditorOpenDebugObjectDynamic DebugObjectDelegate;
};
