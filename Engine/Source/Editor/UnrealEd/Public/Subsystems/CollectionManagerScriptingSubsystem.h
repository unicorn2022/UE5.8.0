...
public:
	...
	/**
	 * Get the color of the given collection.
	 *
	 * @param Collection The collection to get the color of.
	 * @param OutColor The collection's color.
	 *
	 * @return True if the collection exists and has a color set, false otherwise (see the output log for details on error).
	 */
	UFUNCTION(BlueprintCallable, Category = "CollectionManager")
	UE_API bool GetCollectionColor(const FCollectionScriptingRef& Collection, FLinearColor& OutColor);

	/**
	 * Set the color of the given collection.
	 *
	 * @param Collection The collection to set the color of.
	 * @param Color The new collection color.
	 *
	 * @return True if the collection exists and the color was set, false otherwise (see the output log for details on error).
	 */
	UFUNCTION(BlueprintCallable, Category = "CollectionManager")
	UE_API bool SetCollectionColor(const FCollectionScriptingRef& Collection, const FLinearColor& Color);

	/**
	 * Clear the color of the given collection.
	 *
	 * @param Collection The collection to clear the color of.
	 *
	 * @return True if the collection exists and the color was cleared, false otherwise (see the output log for details on error).
	 */
	UFUNCTION(BlueprintCallable, Category = "CollectionManager")
	UE_API bool ClearCollectionColor(const FCollectionScriptingRef& Collection);
	...
	/**
	 * Selects the collections in the most recently used Content Browser (summoning one if necessary)
	 * This is an asynchronous operation that can take a couple of frames to resolve the request
	 * 
	 * @param Collections The list of collections to sync to in the Content Browser
	 */
	UFUNCTION(BlueprintCallable, Category = "CollectionManager | Content Browser")
	UE_API void SyncBrowserToCollections(const TArray<FCollectionScriptingRef>& Collections);

private:
	bool SetCollectionColor(const FCollectionScriptingRef& Collection, const TOptional<FLinearColor>& Color);
};
