...
public:
	...
	{}
		...
		SLATE_ATTRIBUTE(bool, HideStops)
		SLATE_ARGUMENT( bool, ClampStopsToViewRange )
		SLATE_ARGUMENT( bool, WithAlphaChannel )
		SLATE_ARGUMENT(bool, DrawColorAndAlphaSeparate )
		SLATE_ATTRIBUTE(TOptional<FVector2D>, DesiredSizeOverride)
	SLATE_END_ARGS()

	UNREALED_API SColorGradientEditor();
	inline virtual ~SColorGradientEditor() = default;
...
private:
	...
	/** True to request hide stops (will only be fullfilled when IsEditingEnabled = false) */
	TAttribute<bool> bRequestHideStops;
	/** desired size override */
	TSlateAttribute<TOptional<FVector2D>, EInvalidateWidgetReason::Layout> DesiredSizeOverride;

	/** Solver for the geometry in this widget */
	TPimplPtr<FColorGradientEditorGeometrySolver> GeometrySolver;
};
