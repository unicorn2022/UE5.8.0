...
{
public:
	virtual ~FExtensibilityManager() = default;
...
protected:
	...
	/**
	 * Allows to extend dock tab context menu for an asset editor
	 */
	UNREALED_API virtual void OnExtendTabContextMenu(FMenuBuilder& InMenuBuilder);

	/**
	 * Are there any assets to browse to
	 */
	bool OnCanBrowseAssetInContentBrowser() const;

	/**
	 * Browse to assets in content browser
	 */
	void OnBrowseAssetInContentBrowser();
	...
	/** 
	 * Enables standalone toolkit implementations to override the tab layout identifier so it differs from "StandaloneToolkit" or one of its variations. 
	 * If it's not overridden or NAME_None is returned, the default id will be used.
	 */
	virtual FName GetOverrideTabId() const { return NAME_None; }
	
	UE_EXPERIMENTAL(5.8, "Viewport Interactions are Experimental")
	UNREALED_API virtual void OnBuildViewportInteractions(UViewportInteractionsBehaviorSource* Source);
	
	UE_EXPERIMENTAL(5.8, "Viewport Interactions are Experimental")
	virtual void PostBuildViewportInteractions(const UViewportInteractionsBehaviorSource* Source) {};
