...
/**
 * Waits for the editor world's navmesh to finish building before proceeding.
 * Ensures PIE duplicates a fully built navmesh rather than an empty one.
 * Timeout duration is configurable via UAutomationTestSettings::NavMeshBuildTimeoutSeconds.
 */
DEFINE_EXPORTED_LATENT_AUTOMATION_COMMAND(UNREALED_API, FWaitForEditorNavMeshBuildCommand);

/**
 * Starts a PIE session for an automation test and waits for it to be ready.
 * Defers PIE start so it can be preceded by latent wait commands (e.g. FWaitForEditorNavMeshBuildCommand).
 * Handles failure at all stages: if PIE fails to start, starts but ends before the world is ready,
 * or the world stalls during initialization, the test is marked as failed via AddError() and the
 * command terminates. All failure paths are guarded by PIEStartTimeoutSeconds.
 */
class UNREALED_API FStartPIEForAutomationCommand : public IAutomationLatentCommand
{
public:
	FStartPIEForAutomationCommand(FRequestPlaySessionParams InRequestParams);
	virtual ~FStartPIEForAutomationCommand();

	// Update returns false until its work is complete, then it returns true to indicate its work is complete.
	virtual bool Update() override;

private:
	void OnPostPIEStarted(const bool bInSimulateInEditor);
