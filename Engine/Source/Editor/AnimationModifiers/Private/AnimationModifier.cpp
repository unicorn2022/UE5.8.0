...
void UAnimationModifier::PostLoad()
{
	...
	if (PreviouslyAppliedModifier_DEPRECATED)
	{
		...
		if (UAnimationModifiersAssetUserData* AssetData = Cast<UAnimationModifiersAssetUserData>(GetOuter()))
		{
			...
			{
				UE_LOGF(LogAnimation, Log, "Upgrading Applied Animation Modifier on Sequence %ls.", *AssetData->GetOuter()->GetName());
			...
			{
				UE_LOGF(LogAnimation, Warning, "Upgrading Applied Animation Modifier %ls on Skeleton %ls, please reapply the modifier.", *GetName(), *AssetData->GetOuter()->GetName());
				UAnimationModifier* AppliedModifier = PreviouslyAppliedModifier_DEPRECATED;
				PreviouslyAppliedModifier_DEPRECATED = nullptr;
			...
			{
				UE_LOGF(LogAnimation, Error, "Upgrading Applied Animation Modifier on Unknown type of asset %ls, Ignored.", *AssetData->GetOuter()->GetName());
		...
		{
			UE_LOGF(LogAnimation, Error, "Upgrading Applied Animation Modifier on Unknown asset, Ignored.");
			PreviouslyAppliedModifier_DEPRECATED->MarkAsGarbage();
			PreviouslyAppliedModifier_DEPRECATED = nullptr;
		}
		this->Modify();
			...
			for (UAnimationModifier* Modifier : AssetUserData->AnimationModifierInstances)
			{
				if (Modifier->GetClass() == ModifierClass && IsValidChecked(Modifier))
				{
					...
					else if (Skeleton) 
					{
						...
						UE_LOGF(LogAnimation, Warning, "Animation Modifier %ls on Skeleton %ls was skipped, please manually apply it from Skeleton", *ModifierClass->GetName(), *Skeleton->GetName());
					}
				}
			}
		}
