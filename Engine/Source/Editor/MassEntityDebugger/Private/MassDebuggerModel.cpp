...
namespace UE::Mass::Debugger::Private
{
	...
	float CalcArchetypeBitDistance(FMassDebuggerArchetypeData& A, FMassDebuggerArchetypeData& B)
	{
		...
		return float(BitSetDistance(A.Composition.GetElementsBitSet(), B.Composition.GetElementsBitSet())) 
			/ TotalLength;
	}
	...
	void MakeDisplayName(const FString& InName, FString& OutDisplayName)
	{
		...
		const UMassDebuggerSettings* DebuggerSettings = GetDefault<UMassDebuggerSettings>();
		check(DebuggerSettings);
		if (DebuggerSettings->bStripMassPrefix == false)
		{
			return;
		}
...
FMassDebuggerArchetypeData::FMassDebuggerArchetypeData(const FMassArchetypeHandle& ArchetypeHandle)
{
	...
	SparseElementsBitset = FMassDebugger::GetArchetypeSparseElementsBitset(ArchetypeHandle);
	...
	if (DebugNames.IsEmpty())
	{
		...
		if (FMassFragmentBitSet::FIndexIterator It = Composition.DebugGetFragments().GetIndexIterator())
		{
			const FName FirstStructName = Composition.DebugGetFragments().DebugGetStructTypeName(*It);
...
namespace UE::MassDebugger
{
	...
	bool FFragmentEntry::DeserializeFromJson(const TSharedPtr<FJsonObject>& JsonObject)
	{
		...
		FString StructPath = JsonObject->GetStringField(TEXT("StructType"));
		if (StructPath.IsEmpty())
		{
			return false;
		}

		// Honor struct redirects so JSON authored against an older struct path (e.g. before a USTRUCT
		// was moved between modules) still resolves. FindObject/LoadObject only consult Type_Package
		// redirects internally; Type_Struct must be applied explicitly here. Mirrors the pattern in
		// FInstancedStruct::ImportTextItem.
		{
			const FCoreRedirectObjectName OldName(StructPath);
			const FCoreRedirectObjectName NewName = FCoreRedirects::GetRedirectedName(
				ECoreRedirectFlags::Type_Struct, OldName, ECoreRedirectMatchFlags::AllowPartialMatch);
			if (OldName != NewName)
			{
				StructPath = NewName.ToString();
			}
		}

		// Validate before LoadObject to suppress the warning it emits internally on malformed paths
		// (clipboard JSON can carry arbitrary strings).
		if (!FPackageName::IsValidObjectPath(StructPath))
		{
			UE_LOGF(LogMass, Warning, "Mass Debugger: invalid struct path '%ls' - entry skipped.", *StructPath);
			return false;
		}

		// LoadObject (rather than FindObject) so the new struct's module is loaded if it isn't already
		// — important when a redirect points to a package the editor session hasn't touched yet.
		UScriptStruct* const Found = LoadObject<UScriptStruct>(nullptr, *StructPath);
		if (Found == nullptr)
		{
			UE_LOGF(LogMass, Warning, "Mass Debugger: cannot resolve struct type '%ls' - entry skipped.", *StructPath);
			return false;
		}

		const FString AccessStr = JsonObject->GetStringField(TEXT("AccessMode"));
		UEnum* AccessEnum = StaticEnum<EMassFragmentAccess>();
		const int64 AccessVal = AccessEnum->GetValueByNameString(AccessStr);
		if (AccessVal == INDEX_NONE)
		{
			UE_LOGF(LogMass, Warning, "Mass Debugger: cannot resolve AccessMode '%ls' for type '%ls' - entry skipped.", *AccessStr, *StructPath);
			return false;
		}

		const FString PresenceStr = JsonObject->GetStringField(TEXT("Presence"));
		UEnum* PresenceEnum = StaticEnum<EMassFragmentPresence>();
		const int64 PresenceVal = PresenceEnum->GetValueByNameString(PresenceStr);
		if (PresenceVal == INDEX_NONE)
		{
			UE_LOGF(LogMass, Warning, "Mass Debugger: cannot resolve Presence '%ls' for type '%ls' - entry skipped.", *PresenceStr, *StructPath);
			return false;
		}
	...
	TSharedPtr<FJsonObject> FEditableQuery::SerializeToJson() const
	{
		...
		for (const TSharedPtr<FFragmentEntry>& Entry : FragmentRequirements)
		{
			if (!Entry.IsValid())
			{
				continue;
			}
			if (TSharedPtr<FJsonObject> EntryJson = Entry->SerializeToJson())
			{
				JsonFragments.Add(MakeShared<FJsonValueObject>(MoveTemp(EntryJson)));
			}
		}
		...
		for (const TSharedPtr<FFragmentEntry>& Entry : TagRequirements)
		{
			if (!Entry.IsValid())
			{
				continue;
			...
			if (TSharedPtr<FJsonObject> EntryJson = Entry->SerializeToJson())
			{
				JsonTags.Add(MakeShared<FJsonValueObject>(MoveTemp(EntryJson)));
			}
		}
		...
		if (JsonObject->TryGetArrayField(TEXT("FragmentRequirements"), JsonFragments))
		{
			for (const TSharedPtr<FJsonValue>& Val : *JsonFragments)
			{
				if (TSharedPtr<FJsonObject> Obj = Val->AsObject())
				{
					...
					{
						// Skip individual unresolvable entries so the rest of the query still loads.
						continue;
					}
