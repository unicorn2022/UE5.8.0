...
void SOutlinerView::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	...
	if (bWasPendingRefresh && TrackArea)
	{
		// Rows retired by the generation pass that just ran in STreeView::Tick may
		// have dropped the last strong reference to their track lanes. Remove the
		// resulting empty slots from the track area so they don't accumulate.
		TrackArea->PurgeDeadTrackSlots();
	}
}
...
int32 SOutlinerView::CreateOutlinerColumnsForGroup(int32 ColumnIndex, EOutlinerColumnGroup Group)
{
	...
	for ( ; ColumnIndex < NumColumns; ++ColumnIndex, ++NumAdded)
	{
		...
		ColumnGenerators.Add(ColumnName, [Column, ColumnName](const FCreateOutlinerColumnParams& Params, const TSharedRef<SOutlinerViewRow>& Row){
			return (Params.OutlinerExtension.IsValid() && Params.OutlinerExtension->SupportsOutlinerColumn(ColumnName) && Column->IsItemCompatibleWithColumn(Params))
...
void SOutlinerView::Refresh()
{
	...
	// Capture before GetTopLevelItems() consumes the flag
	const bool bHierarchyChanged = Outliner->IsHierarchyInvalidated();
	...
	{
		// Hierarchy changed (tracks/bindings added or removed): discard all cached row widgets
		// so that SOutlinerViewRowPanel::UpdateCells re-runs compatibility checks and generates
		// fresh column widgets for every row. Without this, newly compatible columns (e.g. the
		// AutoBake icon after adding a Control Rig) stay invisible until the user manually
		// triggers a full refresh (F5 / scroll).
		RebuildList();
	}
	else
	{
		// State-only change (mute, solo, deactivate, etc.): run an incremental generation pass
		// that reuses existing row widgets. This keeps section widgets, keys, decoration
		// backgrounds, and column key editors painted continuously without flickering.
		RequestListRefresh();
	}
	...
	for (const TViewModelPtr<ITrackAreaExtension>& ParentTrackArea : InDataModel.AsModel()->GetAncestorsOfType<ITrackAreaExtension>())
	{
		...
		if (TrackAreaParameters.LaneType == ETrackAreaLaneType::Nested)
		{
			...
			if (auto AncestorsIt = ParentTrackArea.AsModel()->GetAncestorsOfType<IOutlinerExtension>(true); AncestorsIt)
			{
				...
				if (!TrackLane)
				{
					// Wire up the grandparent chain so PositionParentTrackLanes can climb past this lane
					// to ancestor lanes that own widgets like layer bars when this lane was never directly
					// created via CreateTrackLanesForRow.
					TSharedPtr<STrackLane> GrandparentLane = FindOrCreateParentLane(OutlinerExtItem);
					TrackLane = SNew(STrackLane, TrackArea, OutlinerExtItem, GrandparentLane, TrackAreaParameters, SharedThis(this));
