...
bool STrackLane::IsPinned() const
{
	...
	if ((Pinnable && Pinnable->IsPinned()) || (Outliner && Outliner->ShouldAnchorToTop()))
	{
		return true;
	}

	// When the parent lane is pinned, then the child lane should also act as if it was pinned.
	return ParentLane && ParentLane->IsPinned();
}
