...
void STrackAreaView::PurgeDeadTrackSlots()
{
	for (auto It = TrackSlots.CreateIterator(); It; ++It)
	{
		if (!It.Value().IsValid())
		{
			It.RemoveCurrent();
	...
	{
		if (!Children[ChildIndex].TrackLane.IsValid())
		{
...
void STrackAreaView::UpdateHoverStates( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
	...
	bool bHoveredItemChanging = OutlinerModel && (NewHoveredItem != OutlinerModel->GetHoveredItem());
	// Suppress hover for items outside the drag owner's track during section drag
	if (bHoveredItemChanging && NewHoveredItem && ViewModel)
	{
		if (ISequencerEditTool* EditTool = ViewModel->GetEditTool())
		{
			if (ISequencerEditToolDragOperation* DragOp = EditTool->GetCurrentDragOperation())
			{
				if (TSharedPtr<FViewModel> DragOwner = DragOp->GetDragOwnerTrackModel())
				{
					if (!DragOwner->IsAncestorOf(NewHoveredItem.AsModel(), /*bIncludeThis*/ true))
					{
						NewHoveredItem = nullptr;
					}
				}
			}
		}
...
FReply STrackAreaView::OnMouseMove( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
	...
	if (Reply.IsEventHandled() && InputStack.GetCapturedIndex() == 1)
	{
		...
		{
			if (const TSharedPtr<SOutlinerView> Outliner = WeakOutliner.Pin())
			{
				Outliner->ScrollByDelta(-MouseEvent.GetCursorDelta().Y);
			}
		}
	}
...
FReply STrackAreaView::OnMouseWheel( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
	...
	if (const TSharedPtr<SOutlinerView> Outliner = WeakOutliner.Pin())
	{
		...
		Outliner->ScrollByDelta(ScrollAmount);
	}
...
void STrackAreaView::OnMouseEnter(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	TSharedPtr<FTrackAreaViewModel> TrackArea = WeakViewModel.Pin();
	ISequencerEditTool* const EditTool = TrackArea ? TrackArea->GetEditTool() : nullptr;

	if (ISequencerInputHandler* const InputHandler = EditTool)
	{
		InputHandler->OnMouseEnter(*this, MyGeometry, MouseEvent);
	}
}
...
void STrackAreaView::OnMouseLeave(const FPointerEvent& MouseEvent)
{
	...
	{
		if (ISequencerInputHandler* const ToolInputHandler = TrackArea->GetEditTool())
		{
			ToolInputHandler->OnMouseLeave(*this, MouseEvent);
		}
	}
}
...
void STrackAreaView::OnMouseCaptureLost(const FCaptureLostEvent& CaptureLostEvent)
{
	if (TSharedPtr<FTrackAreaViewModel> TrackArea = WeakViewModel.Pin())
	{
		if (ISequencerInputHandler* const ToolInputHandler = TrackArea->GetEditTool())
		{
			ToolInputHandler->OnMouseCaptureLost();
		}
	}
}
...
FCursorReply STrackAreaView::OnCursorQuery( const FGeometry& MyGeometry, const FPointerEvent& CursorEvent ) const
{
	...
	if (TSharedPtr<FTrackAreaViewModel> TrackArea = WeakViewModel.Pin())
	{
		...
		if (ISequencerInputHandler* const ToolInputHandler = TrackArea->GetEditTool())
		{
			return ToolInputHandler->OnCursorQuery(*this, MyGeometry, CursorEvent);
		}
	}
