...
public:
	...
	{
		if (auto AncestorsIt = GetAncestorsOfType<T>(bIncludeThis); AncestorsIt)
		{
			return *AncestorsIt;
		}
		return nullptr;
	...
	/**
	 * Returns true if this view model is an ancestor of the given view model.
	 * @param InDescendant The potential descendant to check
	 * @param bIncludeThis If true, returns true when InDescendant.Get() == this
	 */
	bool IsAncestorOf(const TSharedPtr<FViewModel>& InDescendant, bool bIncludeThis = false) const
	{
		TSharedPtr<FViewModel> Current = bIncludeThis ? InDescendant : (InDescendant ? InDescendant->WeakParent.Pin() : nullptr);
		while (Current)
		{
			if (Current.Get() == this)
			{
			...
			Current = Current->WeakParent.Pin();
		}
		return false;
	}
