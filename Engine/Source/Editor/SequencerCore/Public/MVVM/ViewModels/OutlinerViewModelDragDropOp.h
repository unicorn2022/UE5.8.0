...
public:
	...
	/*
	* Based on the current view model being hovered, return the 'target' model and the 'drop model'. 
	* DropModel is the model we will call to execute the drop. 
	* TargetModel is only set if we're inserting before or after a child model of DropModel. 
	* We default to using the CurrentViewModel for the DropModel unless we've dragged above or below the CurrentViewModel,
	* in which we default to the CurrentViewModel's parent for the DropModel and the TargetModel as the CurrentViewModel.
	*/
	UE_API virtual void GetTargetAndDropModel(const FViewModelPtr& CurrentViewModel, EItemDropZone InItemDropZone, FViewModelPtr& DropModel, FViewModelPtr& TargetModel) const;
