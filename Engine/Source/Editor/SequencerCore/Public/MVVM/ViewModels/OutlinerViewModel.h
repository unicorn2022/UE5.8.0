...
public:
	 ...
	 * Destructor
	 */
	virtual ~FOutlinerViewModel() = default;
	...
	/**
	 * Returns true if the root item hierarchy has been invalidated since the last call to GetTopLevelItems.
	 * Used by SOutlinerView::Refresh to decide between a full list rebuild (hierarchy changed) and an
	 * incremental refresh pass (state-only change such as mute/solo/deactivate).
	 */
	bool IsHierarchyInvalidated() const { return bRootItemsInvalidated; }
