...
void FAnimationEditor::HandleObjectsSelected(const TArray<UObject*>& InObjects)
{
	if (DetailsView.IsValid())
	{
		...
		OnAnimationEditorObjectsSelectedDelegate.Broadcast(InObjects);
	}
}
...
void FAnimationEditor::HandleSelectionChanged(const TArrayView<TSharedPtr<ISkeletonTreeItem>>& InSelectedItems, ESelectInfo::Type InSelectInfo)
{
	...
	if (DetailsView.IsValid())
	{
		...
		OnAnimationEditorObjectsSelectedDelegate.Broadcast(Objects);
	}
}

void FAnimationEditor::AddViewportOverlayWidget(TSharedRef<SWidget> InOverlaidWidget, int32 ZOrder)
{
	if (Viewport.IsValid())
	{
...
void FAnimationEditor::RemoveViewportOverlayWidget(TSharedRef<SWidget> InOverlaidWidget)
{
	if (Viewport.IsValid())
	{
...
void FAnimationEditor::HandleObjectSelected(UObject* InObject)
{
	if (DetailsView.IsValid())
	{
		...
		OnAnimationEditorObjectsSelectedDelegate.Broadcast({InObject});
	}
}
