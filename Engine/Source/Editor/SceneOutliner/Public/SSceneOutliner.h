...
public:
	...
	*/
	...
	/** @return Returns an Event to use for highlighting changes in the outliner list */
	UE_API SceneOutliner::TreeItemTextFilter::FChangedEvent& OnTextFilterChanged() const;
	...
	UE_API virtual void CustomAddToToolbar(TSharedPtr<class SHorizontalBox> Toolbar);
...
public:
	...
	/** Event that's broadcast after the scene outliner has been re-populated. */
	FSimpleMulticastDelegate& GetOnFullRefreshDone() { return OnFullRefreshDone; }
	...
	UE_API void RemoveFromSelection(const FSceneOutlinerTreeItemPtr& InItem, ESelectInfo::Type SelectInfo = ESelectInfo::Direct);
	...
	/** Called when a drag-over/enter/leave warrants updating the DragDrop decorator */ 
	UE_API bool UpdateOperationDecorator(const FDragDropEvent& Event, const FSceneOutlinerDragValidationInfo* ValidationInfo) const;	
...
private:
	/** BindCommands for this Outliner with additional commands to append */
	UE_API void BindCommands(const TSharedPtr<FUICommandList>& InCommandListToAppend);
...
public:
	...
	UE_API virtual void SetItemExpansion(const FSceneOutlinerTreeItemPtr& Item, bool bIsExpanded) override;
...
private:
	...
	FSimpleMulticastDelegate OnFullRefreshDone;
...
public:
	...
	/**
	 * Add a type filter to the scene outliner's filter bar. Added separately so it can appear in the 'Type Filters' section
	 * of the Add Filter menu rather than "Other Filters". The filter will appear under its primary category (set in the Filter)
	 ...
	 */
	UE_API void AddTypeFilterToFilterBar(const TSharedRef<FFilterBase<SceneOutliner::FilterBarType>>& InFilter,
