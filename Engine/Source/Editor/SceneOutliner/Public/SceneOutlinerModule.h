...
public:
	...
	UE_INTERNAL SCENEOUTLINER_API virtual void RegisterCustomSceneOutlinerFactory(FName ID, FSceneOutlinerFactory InOutlinerFactory) final;

	/** Unregister a factory to create a custom Scene Outliner */
	UE_INTERNAL SCENEOUTLINER_API virtual void UnregisterCustomSceneOutlinerFactory(FName ID) final;

	/** Try to create a custom scene outliner using a registered factory (nullptr if ID was not registered) */
	UE_INTERNAL SCENEOUTLINER_API virtual TSharedPtr<ISceneOutliner> CreateCustomRegisteredOutliner(FName ID, FSceneOutlinerInitializationOptions InInitOptions) final;

	/** Check if a custom scene outliner factory is registered with the given ID */
	UE_INTERNAL SCENEOUTLINER_API virtual bool IsCustomSceneOutlinerFactoryRegistered(FName ID) final;
	...
	/** Delegate that broadcasts when creating actor browser columns. */
	DECLARE_MULTICAST_DELEGATE_TwoParams(FOnCreateActorBrowserColumns, FSceneOutlinerInitializationOptions&, UWorld*);
	FOnCreateActorBrowserColumns& OnCreateActorBrowserColumns() { return CreateActorBrowserColumnsDelegate; }
...
private:
	...
	/** Delegate that broadcasts when creating actor browser columns. */
	FOnCreateActorBrowserColumns CreateActorBrowserColumnsDelegate;
