...
public:
	...
	virtual ~ISceneOutlinerMode() = default;
...
public:
	...
	/** Called when a drag-over/enter/leave warrants updating the DragDrop decorator, (ValidationInfo == nullptr, indicates resetting the decorator) */ 
	virtual bool UpdateOperationDecorator(const FDragDropEvent& Event, const FSceneOutlinerDragValidationInfo* ValidationInfo) const { return false; }
public:
	/* Events */
	...
	/** Add mode-specific controls to the toolbar. Called when CanCustomizeToolbar() returns true. */
	virtual void CustomAddToToolbar(TSharedPtr<SHorizontalBox> Toolbar) {}
