...
TOptional<FLinearColor> FSceneOutlinerCommonLabelData::GetForegroundColor(TWeakPtr<ISceneOutliner> WeakSceneOutliner, const ISceneOutlinerTreeItem& TreeItem)
{
	if (!TreeItem.IsValid())
	{
		return DarkColor;
	...
	if (FSlateApplication::Get().IsDragDropping())
	{
		...
		if (const TSharedPtr<ISceneOutliner> Outliner = WeakSceneOutliner.Pin())
		{
...
{
	return GetForegroundColor(WeakSceneOutliner, TreeItem);
}
