...
static void UpdateOperationDecorator(TWeakPtr<SSceneOutliner> WeakSceneOutlinerPtr, const FDragDropEvent& Event, const FSceneOutlinerDragValidationInfo& ValidationInfo)
{
	if (TSharedPtr<SSceneOutliner> SharedSceneOutliner = WeakSceneOutlinerPtr.Pin())
	{
		if (SharedSceneOutliner->UpdateOperationDecorator(Event, &ValidationInfo))
		{
			return;
		}
	}
...
static void ResetOperationDecorator(TWeakPtr<SSceneOutliner> WeakSceneOutlinerPtr, const FDragDropEvent& Event)
{
	if (TSharedPtr<SSceneOutliner> SharedSceneOutliner = WeakSceneOutlinerPtr.Pin())
	{
		if (SharedSceneOutliner->UpdateOperationDecorator(Event, nullptr))
		{
			return;
		}
	}
...
FReply SSceneOutlinerTreeView::OnDragOver(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent)
{
	...
	{
		UpdateOperationDecorator(SceneOutlinerWeak, DragDropEvent, ValidationInfo);
	}
...
void SSceneOutlinerTreeView::OnDragLeave(const FDragDropEvent& DragDropEvent)
{
	...
	{
		ResetOperationDecorator(SceneOutlinerWeak, DragDropEvent);
	}
}
...
void SSceneOutlinerTreeRow::OnDragEnter( const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent )
{
	...
	if (ItemPtr.IsValid() && SceneOutlinerPtr.IsValid())
	{
		...
		if (Reply.IsEventHandled())
		{
			UpdateOperationDecorator(SceneOutlinerPtr, DragDropEvent, ValidationInfo);
		}
	}
}
...
bool SSceneOutlinerTreeRow::ComputeVolatility() const
{
	const double CurrentTime = FSlateApplication::Get().GetCurrentTime();
	const float TimeSinceHighlightInteraction = (float)( CurrentTime - LastHighlightInteractionTime );
	return TimeSinceHighlightInteraction <= HighlightTargetEffectDuration;
}
...
void SSceneOutlinerTreeRow::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
	...
	// Update highlight 'target' effect
	{
		...
		else if (const float LastTimeSinceHighlightInteraction = (float)( (InCurrentTime - InDeltaTime) - LastHighlightInteractionTime );
			LastTimeSinceHighlightInteraction <= HighlightTargetEffectDuration)
		{
			// We only want to invalidate once after the highlight animation is done, so if it was shown last frame, we recompute volatility
			Invalidate(EInvalidateWidgetReason::PaintAndVolatility);
		}
	}
}
