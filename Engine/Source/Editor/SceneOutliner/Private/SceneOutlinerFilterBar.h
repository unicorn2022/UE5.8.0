...
public:
	...
	/**
	 * Register a filter as a "type filter" so it appears as an entry in the "Type Filters" section of the Add Filter menu
	 * rather than under "Other Filters". The filter will appear under its primary category (set in the Filter)
	 * and each additional category in the "Add Filter" menu. Toggling from any category activates the same filter instance,
	 * providing a shared state.
	 */
	void AddTypeFilter(const TSharedRef<FFilterBase<SceneOutliner::FilterBarType>>& InFilter,
		TArrayView<const TSharedPtr<FFilterCategory>> InAdditionalCategories = {});
	
	/** Override from the AssetFilterBar to allow non-FCustomClassFilterData to populate the TypeFilter section */
...
private:
	/**
	 * Unified representation of a single "Type Filters" menu category.
	 * A category may contain FCustomClassFilterData entries, FFilterBase entries, or both.
	 */
	struct FTypeFilterCategoryEntry
	{
		/** Extension hook and heading used when this category is rendered as a section or submenu */
		FName SectionExtensionHook;
		FText SectionHeading;

		/** Asset class filters managed by the asset filter bar infrastructure */
		TArray<TSharedPtr<FCustomClassFilterData>> Classes;

		/** Type filters managed via AllFrontendFilters */
		TArray<TSharedPtr<FFilterBase<SceneOutliner::FilterBarType>>> Filters;
	};
	...
	void PopulateTypeFilterMap(const FOnFilterAssetType& OnFilterAssetType, TMap<TSharedPtr<FFilterCategory>, FTypeFilterCategoryEntry>& TypeFilterMap);

	/** Populate a submenu section from an explicit filter list, bypassing GetCategory() matching. */
	void CreateTypeFiltersMenuSection(UToolMenu* InMenu, TArray<TSharedPtr<FFilterBase<SceneOutliner::FilterBarType>>> InFilters);

	/** Toggle all filters in the explicit list on or off (analogous to FrontendFilterCategoryClicked). */
	void TypeFilterCategoryClicked(TArray<TSharedPtr<FFilterBase<SceneOutliner::FilterBarType>>> InFilters);

	/** Return the tri-state check state for the category toggle button (analogous to IsFrontendFilterCategoryChecked). */
	ECheckBoxState IsTypeFilterCategoryChecked(TArray<TSharedPtr<FFilterBase<SceneOutliner::FilterBarType>>> InFilters) const;
...
private:
	...
	/**
	 * Filters registered via AddTypeFilter, used when building the 'Type Filters' menu section. Can be populated with extra categories
	 * registered for specific type filters. PopulateTypeFilterMap distributes each filter across its primary category plus any entries
	 * in the value array.
	 */
	TMap<TSharedPtr<FFilterBase<SceneOutliner::FilterBarType>>, TArray<TSharedPtr<FFilterCategory>>> TypeFilters;
};