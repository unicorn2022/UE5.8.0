...
{
	UE_LOGF(LogSceneOutliner, VeryVerbose, "FullRefresh requested by FActorBrowsingMode::OnComponentsUpdated");
...
void FActorBrowsingMode::OnItemSelectionChanged(FSceneOutlinerTreeItemPtr TreeItem, ESelectInfo::Type SelectionType, const FSceneOutlinerItemSelection& Selection)
{
	...
	if (UTypedElementSelectionSet* SelectionSet = ActorSelection->GetElementSelectionSet())
	{
		...
		{
			for (AActor* SelectedActor : EditorSelectedActors)
			{
				// If we can't find item in tree == item is filtered out, since CTRL is held, add it to current selection
				FSceneOutlinerTreeItemPtr Outliner = SceneOutliner->GetTreeItem(SelectedActor);
				if (!Outliner && SelectedActor != nullptr)
				{
					OutlinerSelectedActors.Add(SelectedActor);
				}
			}
		}
		...
		if (bChanged)
		{
			...
			{
				UE_LOGF(LogActorBrowser, Verbose, "Clicking on Actor (world outliner): %ls (%ls)", *Actor->GetClass()->GetName(), *Actor->GetActorLabel());
...
void FActorBrowsingMode::BindCommands(const TSharedRef<FUICommandList>& OutCommandList)
{
	...
	// Override DELETE command with custom delete
	if (SceneOutliner && SceneOutliner->GetSharedData().CustomDelete.IsBound())
	{
		OutCommandList->MapAction(
...
{
	// If TEv1 interfaces for folders is enabled, we don't want to rely on the legacy delegate based approach
