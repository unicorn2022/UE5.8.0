...
public:
	...
	~FSkelMeshRecorderState() = default;
...
public:
	 ...
	 * @param SequenceClass The class of sequence. Used only if SectionToDuplicate is not provided.
	 * @return The new subsequence.
	 */
	static UE_API UMovieSceneSequence* CreateSequence(FString& NewSequenceName, FString& NewSequencePath, UMovieSceneSubSection* SectionToDuplicate = nullptr, TSubclassOf<UMovieSceneSequence> SequenceClass = nullptr);
	...
	*/
	...
	static TArray<CurveValueType> GetChannelValues(const int32 StartIndex, const int32 EndIndex, const TArray<UMovieSceneSection*>& Sections, const TArray<UMovieSceneSection*>& AbsoluteSections, const FFrameNumber& FrameTime)
	{
		...
		{
			UE_LOGF(LogMovieScene, Warning, "GetChannelValues:: Invalid number of channels");
			return Values;
		}
		if (StartIndex < 0 || EndIndex >= NumChannels || EndIndex < StartIndex)
		{
			UE_LOGF(LogMovieScene, Warning, "GetChannelValues:: Invalid Start/End indices");
			return Values;
		}
	...
	* @param Channels  Set of channels of this type that we will merge together onto the first specified one.
	* @param Sections Set of corresponding sections for each channel specified.
	* @param Range Minimal range of time over which we will merge, we will always extend to key extents if possible
	* @param MergeAlgorithm The algorithm to use to blend the channels together
	...
	*/	
		...
		FChannelMergeAlgorithm MergeAlgorithm, const TArray<UMovieSceneSection*>& TrackSections, const int32* Increment = nullptr)
	{
		...
		{
			UE_LOGF(LogMovieScene, Warning, "MergeChannels:: Invalid number of channels");
			return false;
		}
		...
		{
			UE_LOGF(LogMovieScene, Warning, "MergeChannels:: Can only do Override Blend with two sections");
			return false;
		}
		...
		if (MergeAlgorithm == FChannelMergeAlgorithm::Override) //if we are overriding we need to get the absolute value at the frame!
	{
		...
		//use following to get key extents
		...
		TRange<FFrameNumber> MaxRange(Min, Max);
		...
		if (Increment != nullptr && *Increment > 0)
		{
			...
			{
				TRange<FFrameNumber> RangeToUse = Range;
				for (int32 ChannelIndex = 0; ChannelIndex < Channels.Num(); ++ChannelIndex)
				{
					KeyTimes.Reset();
					Handles.Reset();
					ChannelType* Channel = Channels[ChannelIndex];
					Channel->GetKeys(MaxRange, &KeyTimes, &Handles);
					if (KeyTimes.Num() > 0)
					{
						if (KeyTimes[0] < RangeToUse.GetLowerBoundValue())
						{
							RangeToUse.SetLowerBoundValue(KeyTimes[0]);
						}
						if (KeyTimes[KeyTimes.Num() - 1] > RangeToUse.GetUpperBoundValue())
						{
							RangeToUse.SetUpperBoundValue(KeyTimes[KeyTimes.Num() - 1]);
						}
					}
					
				}
				TArray<FFrameNumber> Frames;
				CalculateFramesBetween(MovieScene, RangeToUse.GetLowerBoundValue(), RangeToUse.GetUpperBoundValue(), *Increment, Frames);
	...
	*/	
	...
	static bool MergeTwoSections(const FBaseTopSections& BT, const TRange<FFrameNumber>& Range, const TArray<UMovieSceneSection*>& TrackSections, const int32* Increment = nullptr)
	{
		...
		{
			UE_LOGF(LogMovieScene, Warning, "MergeSections:: Invalid number of channels");
			return false;
		}
		if (BT.BaseStartIndex < 0 || BT.BaseEndIndex >= BaseChannels.Num() || BT.BaseEndIndex < BT.BaseStartIndex)
		{
			UE_LOGF(LogMovieScene, Warning, "MergeSections:: Invalid Start/End indices");
			return false;
		}
		...
		for (ChannelIndex = BT.BaseStartIndex; ChannelIndex <= BT.BaseEndIndex; ++ChannelIndex)
		{
			...
			{
				UE_LOGF(LogMovieScene, Warning, "MergeSections:: Could not merge channels");
				return false;
			}
		}
	...
	*/	
		...
		int32 StartIndex, int32 EndIndex, const TRange<FFrameNumber>& Range, const int32* Increment = nullptr)
	{
		...
		if (BaseChannels.Num() > 0)
		{
			...
			{
				UE_LOGF(LogMovieScene, Warning, "MergeSections:: No Owner Track");
				return false;
			}
			TArray<ChannelType*> Channels;
	...
	/**
	* Adds transform keys to a specific object, using it's current transform

	* @param Object The object to add keys to.
	* @param ChannelToKey The channels to add keys to.
*	*/
	static UE_API void AddTransformKeysForObject(UObject* Object, EMovieSceneTransformChannel ChannelToKey);
	...
	* @param Sequencer the sequencer to evaluate.
	* @param World the world context (used to retrieve constraints, can be null).
	* @param Frames the times at which the sequencer should be evaluated.
	* @param Tasks the tasks executed at each frame.
	*/
	static UE_API void RunSequencerTasks(
		ISequencer* Sequencer,
		UWorld* World,
		const TArray<FFrameNumber>& Frames,
		const TArrayView<TFunction<void(int32)>>& Tasks);

	/* Executes a task at specified times.
	* @param Sequencer the sequencer to evaluate.
	* @param World the world context (used to retrieve constraints, can be null).
	* @param Frames the times at which the sequencer should be evaluated.
	* @param Task the task executed at each frame.
	*/
	static UE_API void RunSequencerTask(
		ISequencer* Sequencer,
		UWorld* World,
		const TArray<FFrameNumber>& Frames,
