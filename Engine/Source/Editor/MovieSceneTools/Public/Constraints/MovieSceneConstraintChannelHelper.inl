...
{
	// nothing to process or invalid channels array 
	const int32 NumChannels = InChannels.Num();
	if (NumChannels == 0 || NumChannels > 9)
	{
		return;
	}
	...
	auto ProcessAllChannels = [&WithinRange, InChannels]()
	{
		TArray<FFrameNumber> KeyTimes;
		TArray<FKeyHandle> KeyHandles;
	...
	if (InChannelsToKey == EMovieSceneTransformChannel::AllTransform)
	{
		return ProcessAllChannels();
	}

	const bool bProcessTranslation = EnumHasAllFlags(InChannelsToKey, EMovieSceneTransformChannel::Translation);
	const bool bProcessRotation = EnumHasAllFlags(InChannelsToKey, EMovieSceneTransformChannel::Rotation);
	const bool bProcessScale = EnumHasAllFlags(InChannelsToKey, EMovieSceneTransformChannel::Scale);
	if (!(bProcessTranslation || bProcessRotation || bProcessScale))
	{
		// nothing to process
		return;
	}

	TArray<uint32> ChannelsIndexToProcess;
	if (bProcessTranslation)
	{
		ChannelsIndexToProcess.Append({0, 1, 2});
	}
	if (bProcessRotation)
	{
		ChannelsIndexToProcess.Append({3, 4, 5});
	}
	if (bProcessScale)
	{
		ChannelsIndexToProcess.Append({6, 7, 8});
	}
	const int32 NumChannelToProcess = ChannelsIndexToProcess.Num();
	
	// process all channels
	if (NumChannelToProcess == NumChannels)
	{
		return ProcessAllChannels();
	}
	
	if (!ensureAlways(NumChannels == 9))
	{
		// if NumChannelToProcess != NumChannels and NumChannels is lower than 9 then we don't know what to remove...
		return;
	}
	
	TArray<FFrameNumber> KeyTimes;
	TArray<FKeyHandle> KeyHandles;
	for (const int32 ChannelIndex : ChannelsIndexToProcess)
	{
		if (InChannels.IsValidIndex(ChannelIndex))
		{
	...
	const TArray<FMovieSceneTangentData>& InTangents)
{
	...
	if (!InTangents.IsValidIndex(NumChannels-1))
	{
		return;
	}
	
	const TArrayView<ChannelType*> Channels = Section->GetChannelProxy().GetChannels<ChannelType>();
	if (!Channels.IsValidIndex(ChannelIndex) || !Channels.IsValidIndex(ChannelIndex+NumChannels-1))
	{
		return;
	}
