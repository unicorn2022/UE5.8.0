...
bool FSkeletalAnimationTrackEditMode::BeginTransform(const FGizmoState& InState)
{
	if (!bIsTransacting)
	{
		bIsTransacting = (IsSomethingSelected());
...
bool FSkeletalAnimationTrackEditMode::EndTransform(const FGizmoState& InState)
{
	if (bIsTransacting)
	{
		if (bManipulatorMadeChange)
