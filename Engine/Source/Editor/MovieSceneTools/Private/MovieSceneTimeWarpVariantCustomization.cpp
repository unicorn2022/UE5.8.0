...
void FMovieSceneTimeWarpVariantCustomization::ChangeClassType(UClass* InClass)
{
	...
	auto SetFixedPlayRate = [InClass, &Objects, &bNeedsRefresh](void* RawData, const int32 Index, const int32 Num) -> bool
	{
		...
		if (!Existing || Existing->GetClass() != InClass)
		{
			...
			UMovieSceneSection* Section = Cast<UMovieSceneSection>(Object); 
			if (!Section)
			{
				Section = Object->GetTypedOuter<UMovieSceneSection>();
			}
			ensure(Section);

			UObject* GetterOuter = Object->GetTypedOuter<UMovieSceneSequence>();
			UMovieSceneTimeWarpGetter* New = NewObject<UMovieSceneTimeWarpGetter>(GetterOuter ? GetterOuter : Object, InClass, NAME_None, RF_Transactional);
			New->InitializeDefaults(Section);
			Variant->Set(New);
		}
		return true;
	};
