...
template<typename ChannelType, typename ChannelValue, typename KeyType>
bool FBezierChannelCurveModel<ChannelType, ChannelValue, KeyType>::HasDefaultValue() const
{
	if (const ChannelType* Channel = this->GetChannelHandle().Get())
	{
...
template<typename ChannelType, typename ChannelValue, typename KeyType>
double FBezierChannelCurveModel<ChannelType, ChannelValue, KeyType>::GetDefaultValue() const
{
	if (const ChannelType* Channel = this->GetChannelHandle().Get())
	{
		if (HasDefaultValue())
		{
			return static_cast<double>(Channel->GetDefault().GetValue());
		}
	}
	return 0.0;
}
