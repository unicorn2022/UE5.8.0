...
void FChannelCurveModel<ChannelType, ChannelValue, KeyType>::DrawCurve(const FCurveEditor& CurveEditor, const FCurveEditorScreenSpace& ScreenSpace, TArray<TTuple<double, double>>& OutInterpolatingPoints) const
{
	...
	if (Channel && Section)
	{
		...
		const int32 StartingIndex = [&Times, &StartFrame]()
			{
				const int32 StartingFrameIndex = Algo::LowerBound(Times, StartFrame);
				if (Times.IsValidIndex(StartingFrameIndex - 1))
				{
					return StartingFrameIndex - 1;
				}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FChannelCurveModel::AddKeys)
	...
	if (Channel && MetaData && Section && !IsReadOnly())
	{
		...
		for (int32 Index = 0; Index < InKeyPositions.Num(); ++Index)
		{
			...
			const FKeyHandle NewHandle = [&]
			{
				TRACE_CPUPROFILER_EVENT_SCOPE(FChannelCurveModel::SetKeyAttributes);
				return ChannelData.UpdateOrAddKey(Time, Value);
			}();
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FChannelCurveModel::RemoveKeys)
	...
	if (Channel && Section && !IsReadOnly())
	{
		...
		// Iterate in reverse so RemoveAt always operates at or after the current index, avoiding O(n²) index shifting.
		// TMovieSceneChannelData stores keys sorted by time, so back-to-front removal keeps earlier indices stable.
		// InKeys is usually passed in pre-sorted by ascending time, which we abuse here for performance gains.
		for (int32 Index = InKeys.Num() - 1; Index >= 0; --Index)
		{
			const FKeyHandle Handle = InKeys[Index];
			const int32 KeyIndex = ChannelData.GetIndex(Handle);
			if (KeyIndex != INDEX_NONE)
			{
				TRACE_CPUPROFILER_EVENT_SCOPE(FChannelCurveModel::RemoveKey)
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FChannelCurveModel::ReplaceKeyHandles)
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FChannelCurveModel::PostProcessChanges)
	LastSignature.Invalidate();
...
void FChannelCurveModel<ChannelType, ChannelValue, KeyType>::InvokePostChannelChangedEvents(TNotNull<UMovieSceneSection*> Section)
{
	...
	// OnChannelChangedWithTime may cause Modify() calls.
	...
	if (WeakSequencer.IsValid())
	{
		...
		WeakSequencer.Pin()->OnChannelChangedWithTime().Broadcast(Frames, MetaData, Section);
	}
