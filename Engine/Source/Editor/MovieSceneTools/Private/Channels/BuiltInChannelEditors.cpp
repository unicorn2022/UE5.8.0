...
FKeyHandle AddOrUpdateKey(FMovieSceneActorReferenceChannel* Channel, UMovieSceneSection* SectionToKey, FFrameNumber InTime, ISequencer& Sequencer, const FGuid& InObjectBindingID, FTrackInstancePropertyBindings* PropertyBindings)
{
...
bool CanCreateKeyEditor(const FMovieSceneActorReferenceChannel* Channel)
{
	return true;
}
...
TSharedRef<SWidget> CreateKeyEditor(const TMovieSceneChannelHandle<FMovieSceneActorReferenceChannel>& Channel, const UE::Sequencer::FCreateKeyEditorParams& Params)
{
	const FMovieSceneActorReferenceChannel* RawChannel = Channel.Get();
	if (!RawChannel)
	{
		return SNullWidget::NullWidget;
	}
	...
	TSequencerKeyEditor<FMovieSceneActorReferenceChannel, FMovieSceneActorReferenceKey> KeyEditor(Params.ObjectBindingID, Channel, Params.OwningSection, Params.Sequencer, Params.PropertyBindings, Func);
...
UMovieSceneKeyStructType* InstanceGeneratedStruct(FMovieSceneByteChannel* Channel, FSequencerKeyStructGenerator* Generator)
{
	...
	FByteProperty* NewValueProperty = new FByteProperty(NewStruct, "Value");
...
UMovieSceneKeyStructType* InstanceGeneratedStruct(FMovieSceneTimeWarpChannel* Channel, FSequencerKeyStructGenerator* Generator)
{
	...
	FStructProperty* NewValueProperty = new FStructProperty(NewStruct, "Value");
	NewValueProperty->SetPropertyFlags(CPF_Edit);
	NewValueProperty->SetMetaData("Category", TEXT("Key"));
...
UMovieSceneKeyStructType* InstanceGeneratedStruct(FMovieSceneTextChannel* Channel, FSequencerKeyStructGenerator* Generator)
{
	...
	FTextProperty* NewValueProperty = new FTextProperty(NewStruct, "Value");
	NewValueProperty->SetPropertyFlags(CPF_Edit);
	NewValueProperty->SetMetaData("Category", TEXT("Key"));
	NewValueProperty->ArrayDim = 1;
...
UMovieSceneKeyStructType* InstanceGeneratedStruct(FMovieSceneObjectPathChannel* Channel, FSequencerKeyStructGenerator* Generator)
{
	...
	FObjectProperty* NewValueProperty = new FObjectProperty(NewStruct, "Value");
