...
void FMovieSceneEventUtils::RemoveEndpointsForEventSection(UMovieSceneEventSectionBase* EventSection, UBlueprint* DirectorBP)
{
	...
	if (SequenceEventGraph)
	{
		for (FMovieSceneEvent& EntryPoint : EventSection->GetAllEntryPoints())
		{
			...
			if (Endpoint)
			{
				UE_LOGF(LogMovieScene, Display, "Removing event: %ls from: %ls", *GetNameSafe(Endpoint), *GetNameSafe(DirectorBP));
	...
	if (Sequence)
	{
		...
		if (InvalidEntryPoints.Num())
		{
			...
			for (TTuple<FMovieSceneEvent*, UObject*, UMovieSceneSequence*> InvalidEntryPoint : InvalidEntryPoints)
			{
				...
				UE_LOGF(LogMovieScene, Display, "Clearing event %ls from %ls because it refers to a different sequence %ls", *GetNameSafe(EndPoint), *GetNameSafe(Sequence), *GetNameSafe(FunctionBlueprintSequence));
	...
	if (SequenceEventGraph)
	{
		...
		for (UK2Node_CustomEvent* ExistingNode : ExistingNodes)
		{
			...
			if (!bHasEntryPoint)
			{
				...
				UE_LOGF(LogMovieScene, Display, "Remove unused custom event %ls from %ls", *GetNameSafe(ExistingNode), *GetNameSafe(DirectorBP));
