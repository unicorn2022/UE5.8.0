...
{
	TViewModelPtr<ISectionOwnerExtension> SectionOwner = GetLinkedOutlinerItem().AsModel()->FindAncestorOfType<ISectionOwnerExtension>();
	if (SectionOwner)
	{
		FSequencerUtilities::PopulateTimeWarpChannelSubMenu(MenuBuilder, SectionOwner.AsWeak());
	}
}
