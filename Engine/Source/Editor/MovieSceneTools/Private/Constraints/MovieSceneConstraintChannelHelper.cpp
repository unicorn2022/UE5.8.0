...
void FCompensationEvaluator::ComputeLocalTransforms(UWorld* InWorld, const FEvalParameters& InEvalParams)
{
	...
	for (int32 Index = 0; Index < NumFrames + 1; ++Index)
	{
		...
		// evaluate animation
		{
			...
			UE::Anim::Constraints::EvaluateConstraints(AllConstraints);
...
{
	if (!InEvalParams.Sequencer)
	{
		return;
	}

	const TConstArrayView<FFrameNumber> Frames(InEvalParams.Frames);
	const bool bFromCurrentFrame = Frames.IsEmpty() && InEvalParams.OptCurrentFrame.IsSet();
	if (Frames.IsEmpty() && !bFromCurrentFrame)
	{
		return;
	}
	...
	if (bFromCurrentFrame)
	{
		// store child transforms        	
		FTransform& ChildLocal = ChildLocals.Emplace_GetRef(Handle->GetLocalTransform());
		FTransform& ChildGlobal = ChildGlobals.Emplace_GetRef(Handle->GetGlobalTransform());
		FTransform& SpaceGlobal = SpaceGlobals.Emplace_GetRef(FTransform::Identity);
...
void FCompensationEvaluator::ComputeCompensation(UWorld* InWorld, const TSharedPtr<ISequencer>& InSequencer, const FFrameNumber& InTime)
{
	...
	FMovieSceneInverseSequenceTransform LocalToRootTransform = InSequencer->GetFocusedMovieSceneSequenceTransformUnwarped().Inverse();
	...
	UMovieSceneSection* InSection)
{
	...
	if (const UTransformableHandle* Handle = Constraint->ChildTRSHandle)
	{
		const TArrayView<const FFrameNumber> Times = InConstraintChannel->GetData().GetTimes();
		if (Times.IsEmpty() && !InConstraintChannel->IsInfinite())
		{
			return;
		}
		...
		// update channels' default values if there are no frames to compensate
		if (FramesToCompensate.IsEmpty())
		{
			const FFrameTime CurrentTime = InSequencer->GetLocalTime().ConvertTo(TickResolution);
			EvalParams.OptCurrentFrame = CurrentTime.GetFrame();
		}
		...
		Handle->AddTransformKeys(FramesToCompensate, ChildLocals, ChannelsToKey, TickResolution, InSection);
...
void ShowSpawnableWarning(const FText& InNotification)
{
	...
	UE_LOGF(LogTemp, Warning, "%ls", *InNotification.ToString());
