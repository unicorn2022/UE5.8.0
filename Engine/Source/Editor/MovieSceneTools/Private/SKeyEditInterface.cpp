...
void SKeyEditInterface::Initialize()
{
	...
	WeakOwningObject = NewEditData.OwningObject;
...
void SKeyEditInterface::ModifySectionAndOwner()
{
	if (UMovieSceneSection* Section = WeakSection.Get())
	{
		Section->Modify();
	}
	// When the channel lives on a nested object (e.g. a decoration), Modify it too so its
	// signature regenerates and the transaction captures its state.
	UObject* OwningObject = WeakOwningObject.Get();
	if (OwningObject && OwningObject != static_cast<UObject*>(WeakSection.Get()))
	{
		OwningObject->Modify();
	}
}
