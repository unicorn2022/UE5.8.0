...
static bool bOutlineAudioActorAttachments = false;

static FAutoConsoleVariableRef CVarOutlineAudioActorAttachments(
	TEXT("au.OutlineAudioActorAttachments"),
	bOutlineAudioActorAttachments,
	TEXT("Enables red outlines of audio sections in sequencer when the actor binding is broken. Off by default\n")
	TEXT("0: Disable red outlines on audio sections\n")
	TEXT("1: Enable red outlines on audio sections\n"),
	ECVF_Default
);

// Tolerance for Section vs. Sound duration to be considered nearly equal.
constexpr float SectionToSoundDurationToleranceInSeconds = 0.1f;
...
// Binding picker to handle batch editing the actor bindings
class FAudioAttachBindingPicker : public FMovieSceneObjectBindingIDPicker
{
public:
	friend class FAudioSectionDetailsCustomization;

	FAudioAttachBindingPicker(TWeakPtr<ISequencer> InSequencer,
		TArray<TWeakObjectPtr<UMovieSceneAudioSection>> InSections)
		: FMovieSceneObjectBindingIDPicker(GetFocusedSequenceID(InSequencer), InSequencer)
		, WeakSections(MoveTemp(InSections))
	{
		Initialize();
	}
	...
	virtual UMovieSceneSequence* GetSequence() const override
	{
		...
		return Sequencer ? Sequencer->GetFocusedMovieSceneSequence() : nullptr;
	}

	virtual void SetCurrentValue(const FMovieSceneObjectBindingID& NewBinding) override
	{
		FScopedTransaction Transaction(LOCTEXT("SetAttachActor", "Set Attach Actor"));
		for (const TWeakObjectPtr<UMovieSceneAudioSection>& WeakSection : WeakSections)
		{
			if (TStrongObjectPtr<UMovieSceneAudioSection> Section = WeakSection.Pin())
			{
				Section->SetDefaultActorKey(NewBinding);
			}
		}
	...
	{
		if (WeakSections.Num() <= 1)
		{
			return false;
		}
		
		// Iterate over all sections to see if they have the same actor binding value set
		// this allows us to determine if we show "multiple values" in the dropdown or the single actor they all are bound to
		FMovieSceneObjectBindingID FirstBinding;
		bool bFirst = true;
		for (const TWeakObjectPtr<UMovieSceneAudioSection>& WeakSection : WeakSections)
		{
			if (const TStrongObjectPtr<UMovieSceneAudioSection> Section = WeakSection.Pin())
			{
				const FMovieSceneObjectBindingID Binding = Section->GetAttachActorData().GetDefault().Object;
				if (bFirst)
				{
	...
	{
		// If there are multiple values selected, return an empty object otherwise return the object that is selected
		...
		{
			if (const TStrongObjectPtr<UMovieSceneAudioSection> Section = WeakSections[0].Pin())
			{
				return Section->GetAttachActorData().GetDefault().Object;
			}
		}
		return FMovieSceneObjectBindingID();
	}
...
private:
	static FMovieSceneSequenceID GetFocusedSequenceID(TWeakPtr<ISequencer> InSequencer)
	{
		TSharedPtr<ISequencer> Sequencer = InSequencer.Pin();
		return Sequencer.IsValid() ? Sequencer->GetFocusedTemplateID() : MovieSceneSequenceID::Root;
	}

	TArray<TWeakObjectPtr<UMovieSceneAudioSection>> WeakSections;
};

// Adds a custom detail to appear in the property window so batch property editing sees the actor attach data
class FAudioSectionDetailsCustomization : public IDetailCustomization
{
public:
	FAudioSectionDetailsCustomization(
		const FSequencerSectionPropertyDetailsViewCustomizationParams& InParams)
		: Params(InParams)
	{
	}

	static TSharedRef<IDetailCustomization> MakeInstance(
		FSequencerSectionPropertyDetailsViewCustomizationParams InParams)
	{
		return MakeShared<FAudioSectionDetailsCustomization>(InParams);
	}

	virtual void CustomizeDetails(IDetailLayoutBuilder& DetailBuilder) override
	{
		// Collect all selected audio sections from the outer objects
		TArray<TWeakObjectPtr<UObject>> Objects;
		DetailBuilder.GetObjectsBeingCustomized(Objects);

		TArray<TWeakObjectPtr<UMovieSceneAudioSection>> AudioSections;
		for (const TWeakObjectPtr<UObject>& Obj : Objects)
		{
			if (UMovieSceneAudioSection* AudioSection = Cast<UMovieSceneAudioSection>(Obj.Get()))
			{
		...
		// Set up the dropdown combo button with the audio sections selected
		IDetailCategoryBuilder& Category = DetailBuilder.EditCategory(TEXT("Attach"), LOCTEXT("AttachCategory", "Attach"));
		TSharedRef<FAudioAttachBindingPicker> Picker = MakeShared<FAudioAttachBindingPicker>(Params.SequencerWeak, AudioSections);

		Category.AddCustomRow(LOCTEXT("AttachActorLabel", "Attach Actor"))
			.NameContent()
			[
				SNew(STextBlock)
					.Text(LOCTEXT("AttachActorLabel", "Attach Actor"))
					.Font(IDetailLayoutBuilder::GetDetailFont())
			]
			.ValueContent()
			[
				SNew(SComboButton)
					.OnGetMenuContent_Lambda([Picker]() { return Picker->GetPickerMenu(); })
					.ContentPadding(FMargin(4.f, 2.f))
					.ButtonContent()
					[
						Picker->GetCurrentItemWidget(SNew(STextBlock).Font(IDetailLayoutBuilder::GetDetailFont()))
					]
			];
	}
...
void FAudioThumbnail::GenerateWaveformPreview(TArray<uint8>& OutData, TRange<float> DrawRange, float DisplayScale)
{
	...
	{
		UE_LOGF(LogMovieScene, Warning, "Failed to get sound wave data for: %ls Waveform will not be visible.", *SoundWave->GetPathName());
		return;
	}
	...
	for (int32 X = FirstSample; X < LastSample; ++X)
	{
		...
		// Draw looped waveform if section is repeating or asset is natively looping
		const bool bDrawLoopedWaveform = AudioSection->IsLoopingOrRepeating();
		if (!bDrawLoopedWaveform && LookupFraction > 1.f)
		{
			break;
		}
...
FText FAudioSection::GetSectionToolTip() const
{
	...
	if (Sound && AudioSection->HasStartFrame() && AudioSection->HasEndFrame())
	{
		...
		if (AudioSection->IsRepeating())
		{
			BaseToolTip = FText::Format(LOCTEXT("ToolTipContentFormatRepeating", "Start: {0}s\nDuration: {1}s\nRepeating"),
				AudioStartTime,
				SectionDuration);
		}
		else
		{
			...
			if (Duration > 0.0f)
			{
				BaseToolTip = FText::Format(LOCTEXT("ToolTipContentFormat", "{0}s - {1}s ({2} seconds)"),
					AudioStartTime,
		...
		// EvalOptions.bCanEditCompletionMode is the section's CVAR mirror: false when
		// Sequencer.Audio.UsePlayUntilFinished is on, true (the default) for the legacy path.
		FText CompletionModeToolTip;
		if (!AudioSection->EvalOptions.bCanEditCompletionMode)
		{
			CompletionModeToolTip = AudioSection->GetPlayUntilFinished()
				? LOCTEXT("ToolTipPlayUntilFinished", "\nPlays until sound finishes")
				: LOCTEXT("ToolTipStopsOnSectionEnd", "\nStops on section end");
		}
		else
		{
			switch (AudioSection->EvalOptions.CompletionMode)
			{
			case EMovieSceneCompletionMode::KeepState:
				CompletionModeToolTip = LOCTEXT("ToolTipCompletionModeKeep", "\nPlays to end of sound");
				break;
			case EMovieSceneCompletionMode::RestoreState:
				CompletionModeToolTip = LOCTEXT("ToolTipCompletionModeRestore", "\nStops on section end");
				break;
			default:
				CompletionModeToolTip = LOCTEXT("ToolTipCompletionModeDefault", "\nCompletionMode: ProjectDefault");
			}
		}

		FText LoopStateDivergenceToolTip;
		if (AudioSection->IsRepeating() && !AudioSection->IsSoundLoopingAsset())
		{
			// Section repeat state does not match the underlying Sound.
			LoopStateDivergenceToolTip = LOCTEXT("ToolTipRepeatStateDivergence", "\nRepeating non-looping content");
		}

		FText DurationDivergenceToolTip;
		check(AudioSection->HasStartFrame());
		check(AudioSection->HasEndFrame());
		if (!AudioSection->IsLoopingOrRepeating() && !Sound->IsProcedurallyGenerated())
		{
			const float SoundDurationInSeconds = DeriveUnloopedDuration(AudioSection);
			if (!FMath::IsNearlyEqual(SectionDuration, SoundDurationInSeconds, SectionToSoundDurationToleranceInSeconds))
			{
				// Section duration does not match the underlying Sound.
				DurationDivergenceToolTip = LOCTEXT("ToolTipDurationDivergence", "\nOverriding duration from source content");
			}
		}

		ToolTip = FText::Format(LOCTEXT("FullToolTipContentFormat", "{0}{1}{2}{3}"), BaseToolTip, LoopStateDivergenceToolTip, DurationDivergenceToolTip, CompletionModeToolTip);
	}
...
int32 FAudioSection::OnPaintSection( FSequencerSectionPainter& Painter ) const
{
	...
	// If outline highlights are enabled, detect if the attach actor bindings are valid and put a red outline if they are broken
	if (bOutlineAudioActorAttachments && AudioSection != nullptr)
	{
		const FMovieSceneActorReferenceChannel& AttachActorData = AudioSection->GetAttachActorData();

		// Fetch the binding guid
		FMovieSceneActorReferenceKey SectionAttachKey;
		AttachActorData.Evaluate(0, SectionAttachKey);
		FMovieSceneObjectBindingID SectionAttachBindingID = SectionAttachKey.Object;
		FGuid BindingGuid = SectionAttachKey.Object.GetGuid();

		bool bActorBindingHasValidActor = false;

		// Attempt to evaluate through the current sequence for bindings as this also evaluates subsequences
		// This path resolves the binding ids properly in the hierarchy allowing for resolving to the movie scene the binding object belongs to which may not be the same scene as the audio section
		if (TSharedPtr<ISequencer> SequencerPin = Sequencer.Pin())
		{
			const FMovieSceneSequenceID ResolvedSequenceID = SectionAttachBindingID.ResolveSequenceID(SequencerPin->GetFocusedTemplateID(), *SequencerPin);

			if (const UMovieSceneSequence* ResolvedSequence = SequencerPin->GetSharedPlaybackState()->GetSequence(ResolvedSequenceID))
			{
				if (UMovieScene* ResolvedMovieScene = ResolvedSequence->GetMovieScene())
				{
					bActorBindingHasValidActor = ResolvedMovieScene->FindPossessable(BindingGuid) != nullptr
						|| ResolvedMovieScene->FindSpawnable(BindingGuid) != nullptr;
				}
			}
		}
		else if (MovieScene != nullptr)// Fallback in case the sequencer is invalid to just check the current scene asset only
		{
			bActorBindingHasValidActor = MovieScene->FindPossessable(BindingGuid) != nullptr
				|| MovieScene->FindSpawnable(BindingGuid) != nullptr;
		}

		// If the section binding is set, but the binding does not point to a valid object then we consider it a broken binding and highlight it to the user
		if (SectionAttachBindingID.IsValid() && !bActorBindingHasValidActor)
		{
			...
			FSlateDrawElement::MakeBox(
				...
				Painter.LayerId,
				Painter.SectionGeometry.ToPaintGeometry(Painter.SectionGeometry.GetLocalSize() - FVector2D(1.f, 1.f), FSlateLayoutTransform(FVector2D(1.f, 1.f))),
	...
	if (AudioDuration > KINDA_SMALL_NUMBER)
	{
		...
		{
			float OffsetPixel = TimeToPixel->SecondsToPixel(SectionStartTime + OffsetTime);
			...
			if (!AudioSection->IsLoopingOrRepeating())
			{
				break;
			}
		}
...
void FAudioSection::Tick( const FGeometry& AllottedGeometry, const FGeometry& ParentGeometry, const double InCurrentTime, const float InDeltaTime )
{
	...
	if (Track && SoundWave && (SoundWave->NumChannels == 1 || SoundWave->NumChannels == 2))
	{
		...
		if (!FMath::IsNearlyEqual(DrawRange.GetLowerBoundValue(), StoredDrawRange.GetLowerBoundValue()) ||
			...
			bStoredRepeating != AudioSection->IsRepeating())
		{
...
void FAudioSection::ResizeSection(ESequencerSectionResizeMode ResizeMode, FFrameNumber ResizeTime)
{
	...
	else if (ResizeMode == SSRM_TrailingEdge && AudioSection && AudioSection->GetPlayUntilFinished() && AudioSection->HasEndFrame())
	{
		// Auto-disable PlayUntilFinished only when the trailing edge crosses below the sound's natural duration.
		// Subsequent trims of an already-too-short section (e.g. user re-enabled PUF manually) leave PUF alone.
		if (TOptional<FFrameNumber> SoundEndFrame = AudioSection->ComputeDeterministicSoundEndFrame())
		{
			const FFrameNumber PreResizeEnd = AudioSection->GetExclusiveEndFrame();

			if (PreResizeEnd >= *SoundEndFrame && ResizeTime < *SoundEndFrame)
			{
				AudioSection->SetPlayUntilFinished(false);
			}
		}
	}
...
void FAudioSection::CustomizePropertiesDetailsView(TSharedRef<IDetailsView> DetailsView, const FSequencerSectionPropertyDetailsViewCustomizationParams& InParams) const
{
	DetailsView->RegisterInstancedCustomPropertyLayout(
		UMovieSceneAudioSection::StaticClass(),
		FOnGetDetailCustomizationInstance::CreateStatic(
			&FAudioSectionDetailsCustomization::MakeInstance, InParams));
}
...
TSharedPtr<SWidget> FAudioTrackEditor::BuildOutlinerColumnWidget(const FBuildColumnWidgetParams& Params, const FName& ColumnName)
{
	...
	TViewModelPtr<ITrackExtension> TrackModel = Params.ViewModel->FindAncestorOfType<ITrackExtension>(true);
	UMovieSceneTrack* Track = TrackModel ? TrackModel->GetTrack() : nullptr;
	if (!Track)
	{
	...
	{
		if (TrackModel.AsModel()->IsA<FTrackModel>())
		{
