...
void FActorReferencePropertyTrackEditor::ProcessKeyOperation(FFrameNumber InKeyTime, const UE::Sequencer::FKeyOperation& Operation, ISequencer& InSequencer, TArray<UE::Sequencer::FAddKeyResult>* OutResults)
{
...
void FActorReferencePropertyTrackEditor::GenerateKeysFromPropertyChanged(const FPropertyChangedParams& PropertyChangedParams, UMovieSceneSection* SectionToKey, FGeneratedTrackKeys& OutGeneratedKeys)
{
	...
	if (NewReferencedActor != nullptr)
	{
		...
		if (Binding.IsValid())
		{
			...
			OutGeneratedKeys.Add(FMovieSceneChannelValueSetter::Create<FMovieSceneActorReferenceChannel>(0, NewKey, true));
		}
	}
}
...
{
	// When the changed object is a spawnable, force a manual key so that the change will generate an actor reference track. 
	// Otherwise, the object will get referenced by an unstable soft object ptr.
	FPropertyChangedParams NewPropertyChangedParams(PropertyChangedParams);
	...
	if (NewReferencedActor != nullptr)
	{
		...
		if (Spawnable.IsSet())
		{
			NewPropertyChangedParams.KeyMode = ESequencerKeyMode::ManualKey;
		}
	}

	FPropertyTrackEditor::OnAnimatedPropertyChanged(NewPropertyChangedParams);
}
