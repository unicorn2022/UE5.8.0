...
F3DTransformTrackEditor* F3DTransformTrackEditor::CurrentEditor = nullptr;
	...
	: FKeyframeTrackEditor<UMovieScene3DTransformTrack>( InSequencer ) 
{
	...
	F3DTransformTrackEditor::CurrentEditor = this;

}

F3DTransformTrackEditor::~F3DTransformTrackEditor()
{
	F3DTransformTrackEditor::CurrentEditor = nullptr;
...
void F3DTransformTrackEditor::OnDeleteTransformKeysForSelectedObjects(EMovieSceneTransformChannel Channel)
{
	// WASD hotkeys to fly the viewport can conflict with hotkeys for setting keyframes (ie. s).
	// If the viewport is moving, disregard deleting keyframes.
	for (FLevelEditorViewportClient* LevelVC : GEditor->GetLevelViewportClients())
	{
		if (LevelVC && LevelVC->IsMovingCamera())
		{
	...
	FFrameNumber CurrentFrame = SequencerPtr->GetLocalTime().Time.FrameNumber;
	...
	// Channel index order in UMovieScene3DTransformSection: Translation X=0, Y=1, Z=2, Rotation X=3, Y=4, Z=5, Scale X=6, Y=7, Z=8
	const TTuple<EMovieSceneTransformChannel, int32> ChannelMap[] =
	{
		MakeTuple(EMovieSceneTransformChannel::TranslationX, 0),
		MakeTuple(EMovieSceneTransformChannel::TranslationY, 1),
		MakeTuple(EMovieSceneTransformChannel::TranslationZ, 2),
		MakeTuple(EMovieSceneTransformChannel::RotationX,    3),
		MakeTuple(EMovieSceneTransformChannel::RotationY,    4),
		MakeTuple(EMovieSceneTransformChannel::RotationZ,    5),
		MakeTuple(EMovieSceneTransformChannel::ScaleX,       6),
		MakeTuple(EMovieSceneTransformChannel::ScaleY,       7),
		MakeTuple(EMovieSceneTransformChannel::ScaleZ,       8),
	};
	...
	{
		FGuid ObjectGuid = SequencerPtr->GetHandleToObject(SelectedObject, false);
		if (!ObjectGuid.IsValid())
		{
			if (USceneComponent* SceneComp = Cast<USceneComponent>(SelectedObject))
			{
				ObjectGuid = SequencerPtr->GetHandleToObject(SceneComp->GetOwner(), false);
			}
		}
		if (!ObjectGuid.IsValid())
		{
		...
		UMovieScene3DTransformTrack* TransformTrack = MovieScene->FindTrack<UMovieScene3DTransformTrack>(ObjectGuid, TransformPropertyName);
		if (!TransformTrack)
		{
			continue;
		}
		...
		for (UMovieSceneSection* Section : TransformTrack->GetAllSections())
		{
			...
			EMovieSceneTransformChannel SectionMask = TransformSection->GetMask().GetChannels();
			EMovieSceneTransformChannel ChannelsToDelete = Channel & SectionMask;

			TArrayView<FMovieSceneDoubleChannel*> Channels = TransformSection->GetChannelProxy().GetChannels<FMovieSceneDoubleChannel>();

			bool bModified = false;
			for (const TTuple<EMovieSceneTransformChannel, int32>& ChannelPair : ChannelMap)
			{
				if (!EnumHasAnyFlags(ChannelsToDelete, ChannelPair.Key))
				{
					continue;
				}
				if (ChannelPair.Value >= Channels.Num())
				{
					continue;
				}

				FMovieSceneDoubleChannel* DoubleChannel = Channels[ChannelPair.Value];
				TArray<FFrameNumber> KeyTimes;
				TArray<FKeyHandle> KeyHandles;
				DoubleChannel->GetData().GetKeys(FrameRange, &KeyTimes, &KeyHandles);
				if (KeyHandles.Num() > 0)
				{
					if (!bModified)
					{
					...
					DoubleChannel->DeleteKeys(MakeArrayView(KeyHandles));
				}
			}
		}
	}
...
FTransform F3DTransformTrackEditor::GetRootMotionOffset(UObject* AnimatedObject) const
{
	...
	// Each provider returns Identity for objects it doesn't manage, so the
	// first non-identity result is the one that owns this object.
	...
	{
		FTransform Offset = Provider->GetRootMotionOffset(EntityLinker, AnimatedObject);
...
void F3DTransformTrackEditor::UpdateRootMotionOffset(UObject* AnimatedObject)
{
	...
	for (IMovieSceneRootMotionOffsetProvider* Provider : Providers)
	{
		Provider->UpdateRootMotionOffset(EntityLinker, AnimatedObject);
	}
}
