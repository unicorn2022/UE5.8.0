...
{
	return UE::Sequencer::MakeAddButton(GetSubTrackName(), FOnGetContent::CreateSP(this, &FSubTrackEditor::HandleAddSubSequenceComboButtonGetMenuContent, Params.ViewModel.AsWeak()), Params.ViewModel);
}
...
void FSubTrackEditor::OnInitialize()
{
	...
	GetSequencer()->OnChannelChangedWithTime().AddSP(this, &FSubTrackEditor::ResetSectionPreviews);
}
...
void FSubTrackEditor::OnRelease()
{
	...
	GetSequencer()->OnChannelChangedWithTime().RemoveAll(this);
}
...
void FSubTrackEditor::OnDeleteTransformKeysForSelectedObjects(EMovieSceneTransformChannel Channel)
{
	// WASD hotkeys to fly the viewport can conflict with hotkeys for setting keyframes (ie. s).
	...
	const FFrameNumber CurrentFrame = PinnedSequencer->GetLocalTime().Time.FrameNumber;
	...
	FScopedTransaction DeleteKeyTransaction(NSLOCTEXT("AnimatablePropertyTool", "PropertyChanged", "Animatable Property Changed"), bShouldActuallyTransact);

	TArray<UMovieSceneSubSection*> SectionsToKey;
	GetSectionsToKey(SectionsToKey);

	// Channel names and their corresponding transform channel mask bits
	struct FOriginChannelInfo { const TCHAR* Name; EMovieSceneTransformChannel MaskBit; };
	const FOriginChannelInfo OriginChannels[] =
	{
		{ TEXT("Override.Location.X"), EMovieSceneTransformChannel::TranslationX },
		{ TEXT("Override.Location.Y"), EMovieSceneTransformChannel::TranslationY },
		{ TEXT("Override.Location.Z"), EMovieSceneTransformChannel::TranslationZ },
		{ TEXT("Override.Rotation.X"), EMovieSceneTransformChannel::RotationX    },
		{ TEXT("Override.Rotation.Y"), EMovieSceneTransformChannel::RotationY    },
		{ TEXT("Override.Rotation.Z"), EMovieSceneTransformChannel::RotationZ    },
	};

	for (UMovieSceneSubSection* SubSection : SectionsToKey)
	{
		FMovieSceneChannelProxy& SectionChannelProxy = SubSection->GetChannelProxy();
		bool bModified = false;

		for (const FOriginChannelInfo& Info : OriginChannels)
		{
			if (!EnumHasAnyFlags(Channel, Info.MaskBit))
			{
				continue;
			}

			FMovieSceneDoubleChannel* DoubleChannel = SectionChannelProxy.GetChannelByName<FMovieSceneDoubleChannel>(Info.Name).Get();
			if (!DoubleChannel)
			{
				continue;
			}
...
void FSubTrackEditor::InsertSection(UMovieSceneTrack* Track)
{
	...
	TSubclassOf<UMovieSceneSequence> OwningSequenceClass = nullptr;
	if (UMovieSceneSequence* OwningSequence = SubTrack->GetTypedOuter<UMovieSceneSequence>())
	{
		OwningSequenceClass = OwningSequence->GetClass();
	}
...
TSharedRef<SWidget> FSubTrackEditor::HandleAddSubSequenceComboButtonGetMenuContent(UE::Sequencer::TWeakViewModelPtr<UE::Sequencer::FViewModel> WeakViewModel)
{
	using namespace UE::Sequencer;
...
void FSubTrackEditor::AddSequenceToTrack(UMovieSceneSequence* InMovieSceneSequence, UMovieSceneTrack* InTrack)
{
	int32 RowIndex = INDEX_NONE;
	AnimatablePropertyChanged(FOnKeyProperty::CreateRaw(this, &FSubTrackEditor::AddKeyInternal, InMovieSceneSequence, InTrack, RowIndex));
}
