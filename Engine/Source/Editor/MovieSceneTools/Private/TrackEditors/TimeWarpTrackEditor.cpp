...
void FTimeWarpTrackEditor::HandleAddTimeWarpTrack(TSubclassOf<UMovieSceneTimeWarpGetter> ClassType)
{
	...
	UObject* GetterOuter = FocusedMovieScene->GetTypedOuter<UMovieSceneSequence>();
	UMovieSceneTimeWarpGetter* NewGetter = NewObject<UMovieSceneTimeWarpGetter>(GetterOuter ? GetterOuter : NewSection, ClassType.Get(), NAME_None, RF_Transactional);
	NewGetter->InitializeDefaults(NewSection);
