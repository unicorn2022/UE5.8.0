		...
		if (NumMaterials > 0 || MeshComponent)
		{
			MenuBuilder.BeginSection(NAME_None, LOCTEXT("MaterialSwitcherTitle", "Material Switchers"));
			{
				...
				if (MeshComponent)
				{
					...
					const TArray<TObjectPtr<UMaterialInterface>>& SlotOverlayMaterials = MeshComponent->GetComponentMaterialSlotsOverlayMaterial();
					for (int32 SlotIndex = 0; SlotIndex < SlotOverlayMaterials.Num(); ++SlotIndex)
					{
						UMaterialInterface* SlotOverlayMaterial = SlotOverlayMaterials[SlotIndex];
						if (!SlotOverlayMaterial)
						{
							continue;
						}
						FName SlotName = MaterialSlotNames.IsValidIndex(SlotIndex) ? MaterialSlotNames[SlotIndex] : FName();
						FComponentMaterialInfo SlotMaterialInfo{ SlotName, SlotIndex, EComponentMaterialType::SlotOverlayMaterial };
						const bool bSlotAlreadyExists = Algo::FindBy(Binding->GetTracks(), SlotMaterialInfo, GetMaterialInfoForTrack) != nullptr;
						if (bSlotAlreadyExists)
						{
							continue;
						}
						FUIAction AddSlotOverlaySwitcherAction(FExecuteAction::CreateSP(this, &FPrimitiveMaterialTrackEditor::CreateTrackForElement, ObjectBindings, SlotMaterialInfo));
						FText SlotOverlaySwitcherLabel = !SlotName.IsNone() ?
							FText::Format(LOCTEXT("SlotOverlayMaterialSwitcher_Format", "Overlay Slot {0} Switcher"), FText::FromName(SlotName)) :
							FText::Format(LOCTEXT("IndexedSlotOverlayMaterialSwitcher_Format", "Overlay Element {0} Switcher"), FText::AsNumber(SlotIndex));
						FText SlotOverlaySwitcherTooltip = !SlotName.IsNone() ?
							FText::Format(LOCTEXT("SlotOverlayMaterialSwitcherTooltip_Format", "Add per-slot overlay material switcher for slot {0}, index {1}"), FText::FromName(SlotName), FText::AsNumber(SlotIndex)) :
							FText::Format(LOCTEXT("IndexedSlotOverlayMaterialSwitcherTooltip_Format", "Add per-slot overlay material switcher for element {0}"), FText::AsNumber(SlotIndex));
						MenuBuilder.AddMenuEntry(SlotOverlaySwitcherLabel, SlotOverlaySwitcherTooltip, FSlateIcon(), AddSlotOverlaySwitcherAction);
					}
				}
			}
...
void FPrimitiveMaterialTrackEditor::CreateTrackForElement(TArray<FGuid> ObjectBindingIDs, FComponentMaterialInfo MaterialInfo)
{
	...
	for (FGuid ObjectBindingID : ObjectBindingIDs)
	{
		...
		case EComponentMaterialType::VolumetricCloudMaterial:
				...
				break;
		case EComponentMaterialType::SlotOverlayMaterial:
			TrackDisplayName = !MaterialInfo.MaterialSlotName.IsNone() ?
				FText::Format(LOCTEXT("SlotOverlayMaterialSwitcherTrackName", "Overlay Slot: {0}"), FText::FromName(MaterialInfo.MaterialSlotName)) :
				FText::Format(LOCTEXT("IndexedSlotOverlayMaterialSwitcherTrackName", "Overlay Element {0}"), FText::AsNumber(MaterialInfo.MaterialSlotIndex));
			break;
		default:
			break;
