...
UMaterialInterface* FComponentMaterialTrackEditor::GetMaterialInterfaceForTrack( FGuid ObjectBinding, UMovieSceneMaterialTrack* MaterialTrack )
{
	...
	case EComponentMaterialType::SlotOverlayMaterial:
		if (UMeshComponent* Component = Cast<UMeshComponent>(Object))
		{
			return Component->GetOverlayMaterial(true, MaterialInfo.MaterialSlotIndex);
		}
		break;
	default:
		break;
			...
			MenuBuilder.BeginSection("Materials", LOCTEXT("MaterialSection", "Material Parameters"));
			{
				...
				if (MeshComponent)
				{
					...
					for (int32 SlotIndex = 0; SlotIndex < SlotOverlayMaterials.Num(); ++SlotIndex)
					{
						...
						FComponentMaterialInfo MaterialInfo{ SlotName, SlotIndex, EComponentMaterialType::SlotOverlayMaterial };
						const bool bAlreadyExists = Algo::FindBy(Binding->GetTracks(), MaterialInfo, GetMaterialInfoForTrack) != nullptr;
						if (bAlreadyExists)
						{
							continue;
						...
						FUIAction AddComponentMaterialAction(FExecuteAction::CreateRaw(this, &FComponentMaterialTrackEditor::HandleAddComponentMaterialActionExecute, SceneComponent, MaterialInfo));
						FText AddSlotOverlayLabel = !SlotName.IsNone() ?
							FText::Format(LOCTEXT("AddSlotOverlayMaterialLabelFormat", "Overlay Slot: {0}"), FText::FromName(SlotName)) :
							FText::Format(LOCTEXT("AddSlotOverlayMaterialIndexLabelFormat", "Overlay Element {0}"), FText::AsNumber(SlotIndex));
						FText AddSlotOverlayToolTip = !SlotName.IsNone() ?
							FText::Format(LOCTEXT("AddSlotOverlayMaterialToolTipFormat", "Add per-slot overlay material for slot {0}, index {1}"), FText::FromName(SlotName), FText::AsNumber(SlotIndex)) :
							FText::Format(LOCTEXT("AddSlotOverlayMaterialIndexToolTipFormat", "Add per-slot overlay material for element {0}"), FText::AsNumber(SlotIndex));
						MenuBuilder.AddMenuEntry(AddSlotOverlayLabel, AddSlotOverlayToolTip, FSlateIcon(), AddComponentMaterialAction);
					}
				}
			}
			MenuBuilder.EndSection();
		...
		if (!bAlreadyExists)
		{
			...
			case EComponentMaterialType::SlotOverlayMaterial:
				TrackDisplayName = !MaterialInfo.MaterialSlotName.IsNone() ?
					FText::Format(LOCTEXT("SlotOverlayMaterialTrackName", "Overlay Slot: {0}"), FText::FromName(MaterialInfo.MaterialSlotName)) :
					FText::Format(LOCTEXT("IndexedSlotOverlayMaterialTrackName", "Overlay Element {0}"), FText::AsNumber(MaterialInfo.MaterialSlotIndex));
				break;
			default:
				break;
