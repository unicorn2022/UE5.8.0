...
struct FMovieSceneTimeController_FrameStep : FMovieSceneTimeController
{
	...
	virtual FFrameTime OnRequestCurrentTime(const FQualifiedFrameTime& InCurrentTime, float InPlayRate) override
	{
		...
		{
			UE_LOGF(LogMovieSceneCapture, VeryVerbose, "FMovieSceneTimeController_FrameStep::OnRequestCurrentTime = Paused at frame %d subframe %f", InCurrentTime.Time.FrameNumber.Value, InCurrentTime.Time.GetSubFrame());
		...
		else
		{
			...
			UE_LOGF(LogMovieSceneCapture, VeryVerbose, "FMovieSceneTimeController_FrameStep::OnRequestCurrentTime = Playing at frame %d subframe %f", NewTime.FrameNumber.Value, NewTime.GetSubFrame());
			return NewTime;
		}
	}
...
void UAutomatedLevelSequenceCapture::Initialize(TSharedPtr<FSceneViewport> InViewport, int32 PIEInstance)
{
	...
	if (Settings.bUsePathTracer)
	{
		...
		{
			UE_LOGF(LogMovieSceneCapture, Log, "Delay every frame overridden by path tracer sample per pixel: %f", PathTracerSamplePerPixel);
	...
	if (Actor && !ShotName.IsEmpty())
	{
		...
		if (CinematicShotTrack && MovieScene)
		{
			...
			if (CinematicShotSection)
			{
				...
				UE_LOGF(LogMovieSceneCapture, Log, "Found shot '%ls' to capture. Setting custom start and end frames to %d and %d.", *ShotName, CustomStartFrame.Value, CustomEndFrame.Value);
			}
			else
			{
				UE_LOGF(LogMovieSceneCapture, Error, "Could not find named shot '%ls' to capture", *ShotName);
			}
		}
	}
	...
	{
		UE_LOGF(LogMovieSceneCapture, Error, "Could not find or create a Level Sequence Actor for this capture. Capturing will fail.");
	}
...
void UAutomatedLevelSequenceCapture::OnTick(float DeltaSeconds)
{
	...
	{
		UE_LOGF(LogMovieSceneCapture, Log, "[%lld] Waiting for %d shaders to finish compiling...", GFrameCounter, GShaderCompilingManager->GetNumRemainingJobs());
		GShaderCompilingManager->FinishAllCompilation();
		UE_LOGF(LogMovieSceneCapture, Log, "[%lld] Done waiting for shaders to finish.", GFrameCounter);
	}

	if (GDistanceFieldAsyncQueue && GDistanceFieldAsyncQueue->GetNumOutstandingTasks() > 0)
	{
		UE_LOGF(LogMovieSceneCapture, Log, "[%lld] Waiting for %d Mesh Distance Fields to finish building...", GFrameCounter, GDistanceFieldAsyncQueue->GetNumOutstandingTasks());
		GDistanceFieldAsyncQueue->BlockUntilAllBuildsComplete();
		UE_LOGF(LogMovieSceneCapture, Log, "[%lld] Done waiting for Mesh Distance Fields to build.", GFrameCounter);
	}

	if (GCardRepresentationAsyncQueue && GCardRepresentationAsyncQueue->GetNumOutstandingTasks() > 0)
	{
		UE_LOGF(LogMovieSceneCapture, Log, "[%lld] Waiting for %d Mesh Cards to finish building...", GFrameCounter, GCardRepresentationAsyncQueue->GetNumOutstandingTasks());
		GCardRepresentationAsyncQueue->BlockUntilAllBuildsComplete();
		UE_LOGF(LogMovieSceneCapture, Log, "[%lld] Done waiting for Mesh Cards to build.", GFrameCounter);
	}
...
void UAutomatedLevelSequenceCapture::PauseFinished()
{
	...
	if (bIsAudioCapturePass)
	{
		UE_LOGF(LogMovieSceneCapture, Log, "WarmUp pause finished. Resuming the capture of audio.");
	}
	else
	{
		UE_LOGF(LogMovieSceneCapture, Log, "WarmUp pause finished. Resuming the capture of images.");
	}
}
	...
	if (bCapturing)
	{
		...
		if (Actor && Actor->GetSequencePlayer())
		{
			...
			if (Actor->GetSequencePlayer()->IsPlaying() && ( bDelayingBeforeShotWarmUp || bDelayingEveryFrame ))
			{
				if (bIsAudioCapturePass)
				{
					UE_LOGF(LogMovieSceneCapture, Log, "Entering WarmUp pause, pausing audio capture.");
				...
				{
					UE_LOGF(LogMovieSceneCapture, Log, "Entering WarmUp pause, pausing image capture.");
