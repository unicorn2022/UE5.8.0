...
{
	virtual ~ISequencerKeyEditor()= default;
...
struct TSequencerKeyEditor
{
	...
	void SetValue(const ValueType& InValue)
	{
		...
		// When the owning object is different from the section (e.g., decoration
		// channels), also modify the section so its signature updates and
		// FChannelModel's cached keys detect the change.
		if (Owner != OwningSection)
		{
		...
		else if (bPreserveRatio && bCanPreserveRatio)
		{
			...
			for (FMovieSceneChannelHandle GroupChannelHandle : GroupedChannels)
			{
				if (ChannelType* GroupChannel = reinterpret_cast<ChannelType*>(GroupChannelHandle.Get()))
				{
					...
					Sequencer->OnChannelChangedWithTime().Broadcast(Frames, GroupChannelMetaData, OwningSection);
				}
			}
		}
		else
		{
			...
			Sequencer->OnChannelChangedWithTime().Broadcast(Frames, MetaData, OwningSection);
		}
	}
