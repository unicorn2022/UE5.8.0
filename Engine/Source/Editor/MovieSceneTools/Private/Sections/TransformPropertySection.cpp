...
{
	static const FName ConstraintsSectionName(TEXT("Constraints"));
}

bool FTransformSection::RequestDeleteCategory(const TArray<FName>& CategoryNamePaths)
{
	if (CategoryNamePaths.IsEmpty())
	{
		return false;
	}
	...
	if (CategoryNamePaths[0] == UE::Private::ConstraintsSectionName)
	{
		constexpr bool bDoNotCompensate = false;
		if (UWorld* World = GEditor ? GEditor->GetEditorWorldContext().World() : nullptr)
		{
			const FScopedTransaction Transaction(LOCTEXT("DeleteConstraintChannels", "Delete Constraint Channels"));
			if (TransformSection->TryModify())
			{
				if (TransformSection->RemoveAllConstraints(World, bDoNotCompensate))
				{
					SequencerPtr->NotifyMovieSceneDataChanged(EMovieSceneDataChangeType::MovieSceneStructureItemsChanged);
				}
				return true;
...
bool FTransformSection::RequestDeleteKeyArea(const TArray<FName>& KeyAreaNamePaths)
{
	...
	if (KeyAreaNamePaths.Num() == 2 && KeyAreaNamePaths[0] == UE::Private::ConstraintsSectionName)
	{
		UTickableConstraint* Constraint = TransformSection->FindConstraint(KeyAreaNamePaths[1]);
		if (!Constraint)
		{
			return false;
		}
