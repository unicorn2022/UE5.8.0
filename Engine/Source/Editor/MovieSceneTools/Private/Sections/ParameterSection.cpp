...
bool FParameterSection::RequestDeleteCategory( const TArray<FName>& CategoryNamePath )
{
	...
	UMovieSceneParameterSection* ParameterSection = Cast<UMovieSceneParameterSection>( WeakSection.Get() );
	if (ParameterSection && ParameterSection->Modify() && !CategoryNamePath.IsEmpty())
	{
		const FName& ChannelName = CategoryNamePath.Last();

		const bool bVectorParameterDeleted = ParameterSection->RemoveVectorParameter(ChannelName);
		const bool bColorParameterDeleted = ParameterSection->RemoveColorParameter(ChannelName);
		const bool bScalarParameterDeleted = ParameterSection->RemoveScalarParameter(ChannelName);
		const bool bBoolParameterDeleted = ParameterSection->RemoveBoolParameter(ChannelName);
		const bool bVector2DParameterDeleted = ParameterSection->RemoveVector2DParameter(ChannelName);
		const bool bTransformParameterDeleted = ParameterSection->RemoveTransformParameter(ChannelName);
...
bool FParameterSection::RequestDeleteKeyArea( const TArray<FName>& KeyAreaNamePath )
{
	...
	if (KeyAreaNamePath.Num() == 1)
	{
		const FScopedTransaction Transaction(LOCTEXT("DeleteParameter", "Delete parameter"));
		UMovieSceneParameterSection* ParameterSection = Cast<UMovieSceneParameterSection>( WeakSection.Get() );
		if (ParameterSection && ParameterSection->TryModify())
		{
