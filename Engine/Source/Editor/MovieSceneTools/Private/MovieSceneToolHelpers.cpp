...
UMovieSceneSequence* MovieSceneToolHelpers::CreateSequence(FString& NewSequenceName, FString& NewSequencePath, UMovieSceneSubSection* SectionToDuplicate, TSubclassOf<UMovieSceneSequence> SequenceClass)
{
	// Create a new level sequence asset with the appropriate name
	IAssetTools& AssetTools = FModuleManager::GetModuleChecked<FAssetToolsModule>("AssetTools").Get();
	if (!SequenceClass)
	{
		SequenceClass = ULevelSequence::StaticClass();
	}
	...
	for (TObjectIterator<UClass> It; It; ++It)
	{
		...
		if (CurrentClass->IsChildOf(UFactory::StaticClass()) && !(CurrentClass->HasAnyClassFlags(CLASS_Abstract)))
		{
			...
			if (Factory->CanCreateNew() && Factory->ImportPriority >= 0 && Factory->SupportedClass == SequenceClass)
			{
				...
				{
					NewAsset = AssetTools.CreateAssetWithDialog(NewSequenceName, NewSequencePath, SequenceClass, Factory);
				}
				break;
			}
		}
...
void MovieSceneToolHelpers::MovieSceneTranslatorLogOutput(FMovieSceneTranslator *InTranslator, TSharedRef<FMovieSceneTranslatorContext> InContext)
{
	...
	for (TSharedRef<FTokenizedMessage> Message : InContext->GetMessages())
	{
		...
		{
			UE_LOGF(LogMovieScene, Error, "%ls", *Message->ToText().ToString());
		}
		else if (Message->GetSeverity() == EMessageSeverity::Warning)
		{
			UE_LOGF(LogMovieScene, Warning, "%ls", *Message->ToText().ToString());
...
private:
	...
	FReply OnExportFBXClicked()
	{
		...
		{
			RootToLocalTransform = Sequencer.Pin()->GetFocusedMovieSceneSequenceTransformUnwarped();
		}
...
bool ImportFBXTransform(FString NodeName, FGuid ObjectBinding, UnFbx::FFbxCurvesAPI& CurveAPI, UMovieSceneSequence* InSequence, IMovieScenePlayer* Player, ISequencer* Sequencer)
{
	...
	if (DoubleChannels[4].Get())
	{
		const bool bNegateTangents = ImportFBXSettings->bForceFrontXAxis;
...
private:
	...
	FReply OnImportFBXClicked()
	{
		...
		{
			UE_LOGF(LogMovieScene, Display, "Fbx Import: Importing onto one selected binding, disabling match by name only.");
	...
	const FFBXInOutParameters& InParams, ISequencer* Sequencer)
{
	...
	if (!ImportFBXSettings->bMatchByNameOnly)
	{
		for (int32 NodeIndex = 0; NodeIndex < AllNodeNames.Num(); )
		{
			...
			if (It)
			{
				...
				UE_LOGF(LogMovieScene, Warning, "Fbx Import: Failed to find any matching node for (%ls). Defaulting to first available (%ls).", *NodeName, *It.Value());
	...
	{
		UE_LOGF(LogMovieScene, Warning, "Fbx Import: Failed to find any matching node for (%ls).", *NodeName);
	}
	...
	FInitAnimationCB InitCallback, FStartAnimationCB StartCallback, FTickAnimationCB TickCallback, FEndAnimationCB EndCallback)
{
	...
	if (!ExportOptions)
	{
		UE_LOGF(LogMovieScene, Warning, " MovieSceneToolHelpers::BakeToSkelMesh functions require a valid AnimSeqExportOption");
		return false;
	}
	...
	if (InSkelMeshComp->GetBoneSpaceTransforms().Num() <= 0)
	{
		...
		if (InSkelMeshComp->GetBoneSpaceTransforms().Num() <= 0)
		{
			UE_LOGF(LogMovieScene, Error, "Error Ba");
			return false;
		}
	}
...
static FString GetSlateFromMovieSceneSequence(UMovieSceneSequence* MovieSceneSequence)
{
	...
	if (UObject* MetaDataObject = LevelSequence->FindMetaDataByClass(TakeMetaDataClass))
	{
		if (const FProperty* SlateProperty = TakeMetaDataClass->FindPropertyByName("Slate"))
		{
			...
			{
				UE_LOGF(LogMovieScene, Error, "TakesMetaData::Slate property was found, but it unexpectedly was not a String property.");
			}
		}
		else
		{
			UE_LOGF(LogMovieScene, Error, "TakesMetaData class was found, but it unexpectedly did not have a Slate property.");
		}
	}
	...
	USkeletalMeshComponent* SkelMeshComp)
{
	...
	{
		UE_LOGF(LogMovieScene, Error, "MovieSceneToolHelpers::ExportToAnimSequence All parameters must be valid.");
		return false;
	}
	...
	FInitAnimationCB InitCallback = FInitAnimationCB::CreateLambda([&AnimationRecorder, SampleRate, ExportOptions, SkelMeshComp, AnimSequence]
		{
			...
			RecordingSettings.bSetRetargetSourceAsset = ExportOptions->bSetRetargetSourceAsset;
			...
			RecordingSettings.bSkipCurvesWithZeroValue = ExportOptions->bSkipCurvesWithZeroValue;
			...
			RecordingSettings.bRemoveExcludedCurves = ExportOptions->bRemoveExcludedCurves;
			RecordingSettings.bTransactRecording = false; //don't do this it breaks undo's
...
void FAllSpawnableRestoreState::SaveStateForSpawnable(bool& bInOutWasChanged, TMap<FGuid, ESpawnOwnership>& InOutSpawnOwnershipMap, TWeakObjectPtr<UMovieScene>& WeakMovieScene, FGuid BindingGuid, ESpawnOwnership PreviousSpawnOwnership, TFunction<void(ESpawnOwnership)> SpawnOwnershipSetter)
{
	...
	const UMovieSceneSpawnTrack* SpawnTrack = WeakMovieScene->FindTrack<UMovieSceneSpawnTrack>(BindingGuid);
	if (SpawnTrack && !SpawnTrack->IsEmpty())
	{
		if (UMovieSceneSpawnSection* SpawnSection = Cast<UMovieSceneSpawnSection>(SpawnTrack->GetAllSections()[0]))
		{
			// no need to modify the spawned state if it's constant
			auto ShouldSectionBeModified = [SpawnSection]()
			{
				const FMovieSceneBoolChannel& SpawnedChannel = SpawnSection->GetChannel();
				const TArrayView<const bool>& Values = SpawnedChannel.GetValues();
				if (!Values.IsEmpty())
				{
	...
	: SharedPlaybackState(InSharedPlaybackState)
{
	...
	if (!SharedPlaybackStateToUse)
	{
		UE_LOGF(LogMovieScene, Warning, "FAllSpawnableRestoreState requires a valid FSharedPlaybackState");
		return;
	}
	if (InMovieScene)
	{
	...
	else
	{
		...
		if (!Hierarchy)
		{
			...
			{
				UE_LOGF(LogMovieScene, Warning, "FAllSpawnableRestoreState requires a valid Root Sequence from FSharedPlaybackState");
				return;
			}
		}
		else
...
void MovieSceneToolHelpers::RunSequencerTasks(
	ISequencer* Sequencer,
	UWorld* World,
	const TArray<FFrameNumber>& Frames,
	const TArrayView<TFunction<void(int32)>>& Tasks)
{
	if (Frames.IsEmpty() || Tasks.IsEmpty())
	{
		return;
	}
	...
	const FMovieSceneInverseSequenceTransform InverseSequenceTransform = Sequencer->GetFocusedMovieSceneSequenceTransformUnwarped().Inverse();
	
	FMovieSceneToolsAnimationBakingWrapper BakeWrapper(MovieScene);
	BakeWrapper.Start();
	...
	{
		FFrameNumber FrameNumber = Frames[Index];
		FFrameTime GlobalTime = InverseSequenceTransform.TryTransformTime(FrameNumber).Get(FrameNumber); //player evals in root time so need to go back to it.

		BakeWrapper.PreEvaluate(FrameNumber);
		...
		BakeWrapper.PostEvaluate(FrameNumber);

		for (const TFunction<void(int32)>& Task: Tasks)
		{
			Task(Index);
		}
	}
...
void MovieSceneToolHelpers::RunSequencerTask(
		ISequencer* Sequencer,
		UWorld* World,
		const TArray<FFrameNumber>& Frames,
		const TFunctionRef<void(int32)>& Task)
{
	if (Frames.IsEmpty())
	{
		return;
...
void MovieSceneToolHelpers::GetActorWorldTransforms(ISequencer* Sequencer, const FActorForWorldTransforms& ActorSelection, const TArray<FFrameNumber>& Frames, TArray<FTransform>& OutWorldTransforms)
{
	...
	if (!bAvoidEvaluates && ObjectHandle.IsValid())
	{
		...
		{
			GetSequencerActorWorldTransforms(Sequencer, Sequencer->GetFocusedMovieSceneSequenceTransformUnwarped(), Sequencer->GetFocusedMovieSceneSequence(), Template, ActorSelection, Frames, OutWorldTransforms);
			return;
		}
	}
	...
	if (bAvoidEvaluates)
	{
		...
		GetNonSequencerActorWorldTransforms(Sequencer, Sequencer->GetFocusedMovieSceneSequenceTransformUnwarped(), Sequencer->GetFocusedMovieSceneSequence(), Template, ActorSelection, NoFrame, OutWorldTransforms);
	}
	else
	{
		GetNonSequencerActorWorldTransforms(Sequencer, Sequencer->GetFocusedMovieSceneSequenceTransformUnwarped(), Sequencer->GetFocusedMovieSceneSequence(), Template, ActorSelection, Frames, OutWorldTransforms);
	}
}
...
bool MovieSceneToolHelpers::IsValidAsset(UMovieSceneSequence* Sequence, const FAssetData& InAssetData)
{
	...
	{
		UE_LOGF(LogMovieScene, Warning, "%ls cannot reference because: %ls", *GetNameSafe(Sequence), *FailureReason.ToString());
		return false;
	}
	return true;
}
...
void MovieSceneToolHelpers::SetOrAddKey(TMovieSceneChannelData<FMovieSceneDoubleValue>& ChannelData, FFrameNumber Time, FMovieSceneDoubleValue Value)
{
	...
	if (ExistingIndex != INDEX_NONE)
	{
		FMovieSceneDoubleValue& DoubleValue = ChannelData.GetValues()[ExistingIndex];
	...
	const FFrameNumber& StartFrame, const FFrameNumber& EndFrame, TSortedMap<FFrameNumber, FFrameNumber>& OutFrameMap)
{
	...
	for (FActorForWorldTransforms& Actor : ParentActors)
	{
		...
		if (Guid.IsValid())
		{
			for (UMovieSceneTrack* Track : MovieScene->FindTracks(UMovieScene3DTransformTrack::StaticClass(), Guid))
			{	
				if (Track)
				{
					for (UMovieSceneSection* Section : Track->GetAllSections())
...
void MovieSceneToolHelpers::AddTransformKeysForObject(UObject* Object, EMovieSceneTransformChannel ChannelToKey)
{
	if (F3DTransformTrackEditor::CurrentEditor)
	{
		F3DTransformTrackEditor::CurrentEditor->AddTransformKeysForObject(Object, ChannelToKey, ESequencerKeyMode::ManualKeyForced);
	}
}
