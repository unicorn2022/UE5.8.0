...
class UAnimStateTransitionNode : public UAnimStateNodeBase
{
	...
	/** If true, this transition rule will not be evaluated when the state machine's update context is inactive (e.g. when blending out). */
	UPROPERTY(EditAnywhere, Category=Transition)
	bool bOnlyEvaluateWhenActive;
