...
{
	UAnimBlueprint* AnimBlueprint = GetAnimBlueprint();
	if (!GIsCookerLoadingPackage && !IsRunningCookCommandlet() && AnimBlueprint->TargetSkeleton) // Don't modify skeleton during cook
	{
		AnimBlueprint->TargetSkeleton->RegisterSlotNode(Node.SlotName);
	}
}

void UAnimGraphNode_Slot::GetRequiredExtensions(TArray<TSubclassOf<UAnimBlueprintExtension>>& OutExtensions) const
{
	Super::GetRequiredExtensions(OutExtensions);
	
	if (const IAnimGraph_SequencerMixerTargetConnector* MixerTargetConnector = IAnimGraph_SequencerMixerTargetConnector::Get())
	{
		MixerTargetConnector->GetSequencerMixerTargetRequiredExtensions(OutExtensions);
	}
}
