...
{
public:
	FCachedPoseNameValidator(const UBlueprint* InBlueprint, const UAnimGraphNode_SaveCachedPose* CurrentNode, const FString& InExistingName)
		: FStringSetNameValidator(InExistingName)
	{
		TArray<const UAnimGraphNode_SaveCachedPose*> Nodes;
		FBlueprintEditorUtils::GetAllNodesOfClass<const UAnimGraphNode_SaveCachedPose>(InBlueprint, Nodes);

		for (const UAnimGraphNode_SaveCachedPose* Node : Nodes)
		{
			if (Node != CurrentNode)
			{
...
{
	return MakeShared<FCachedPoseNameValidator>(GetBlueprint(), this, CacheName);
}

void UAnimGraphNode_SaveCachedPose::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	const FName PropertyName = PropertyChangedEvent.GetPropertyName();

	if (PropertyName == GET_MEMBER_NAME_CHECKED(UAnimGraphNode_SaveCachedPose, CacheName))
	{
	 	if (MakeUniqueCacheName())
	 	{
			FBlueprintEditorUtils::MarkBlueprintAsStructurallyModified(GetBlueprint());
		}
	}
...
void UAnimGraphNode_SaveCachedPose::PostPasteNode()
{
	Super::PostPasteNode();
	if (MakeUniqueCacheName())
	{
...
bool UAnimGraphNode_SaveCachedPose::MakeUniqueCacheName()
{
	bool bHasChanged = false;
	// Disallow empty names
	if (CacheName.IsEmpty())
	{
		CacheName = TEXT("CachedPose");
		bHasChanged = true;
	}
	// Check for duplicate names among other SaveCachedPose nodes
	FCachedPoseNameValidator Validator(GetBlueprint(), this, "");
	if (Validator.IsValid(CacheName,true) != EValidatorResult::Ok)
	{
		Validator.FindValidString(CacheName);
		bHasChanged = true;
	}
	return bHasChanged;
}
