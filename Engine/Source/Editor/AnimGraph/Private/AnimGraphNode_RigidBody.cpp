...
void UAnimGraphNode_RigidBody::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	// Add per node check box for allowing bone transform scale propagation. This check box is hidden unless the the feature is enabled globally via CVar.
	{
		IDetailCategoryBuilder& SettingsCategory = DetailBuilder.EditCategory(TEXT("Settings"));
		TSharedRef<IPropertyHandle> AllowPoseScalePropagationHandle = DetailBuilder.GetProperty(TEXT("Node.bAllowPoseScalePropagation"), GetClass());

		TAttribute<EVisibility> VisiblityAttr = TAttribute<EVisibility>::Create(TAttribute<EVisibility>::FGetter::CreateStatic(&UAnimGraphNode_RigidBody::ShouldShowPoseScalePropagationSettings));

		SettingsCategory.AddCustomRow(AllowPoseScalePropagationHandle->GetPropertyDisplayName(), true)
			.NameContent()
			[
				AllowPoseScalePropagationHandle->CreatePropertyNameWidget()
			]
			.ValueContent()
			[
				AllowPoseScalePropagationHandle->CreatePropertyValueWidget()
			]
			.Visibility(VisiblityAttr);

		AllowPoseScalePropagationHandle->MarkHiddenByCustomization();
	}
}
...
EVisibility UAnimGraphNode_RigidBody::ShouldShowPoseScalePropagationSettings()
{
	return RigidBodyNodeIsPoseScalePropagationEnabledGlobally() ? EVisibility::Visible : EVisibility::Collapsed;
}
