...
void FAnimNodeEditMode::ConvertToComponentSpaceTransform(const USkeletalMeshComponent* SkelComp, const FTransform & InTransform, FTransform & OutCSTransform, int32 BoneIndex, EBoneControlSpace Space)
{
	...
	default:
		if (SkelMesh)
		{
			UE_LOGF(LogAnimation, Warning, "ConvertToComponentSpaceTransform: Unknown BoneSpace %d  for Mesh: %ls", (uint8)Space, *SkelMesh->GetFName().ToString());
		}
		else
		{
			UE_LOGF(LogAnimation, Warning, "ConvertToComponentSpaceTransform: Unknown BoneSpace %d  for Skeleton: %ls", (uint8)Space, *Skeleton->GetFName().ToString());
		}
		break;
	}
}
...
void FAnimNodeEditMode::ConvertToBoneSpaceTransform(const USkeletalMeshComponent* SkelComp, const FTransform & InCSTransform, FTransform & OutBSTransform, int32 BoneIndex, EBoneControlSpace Space)
{
	...
	switch(Space)
	{
		...
		default:
		{
			UE_LOGF(LogAnimation, Warning, "ConvertToBoneSpaceTransform: Unknown BoneSpace %d  for Mesh: %ls", (int32)Space, *GetNameSafe(SkelMesh));
			break;
		}
	}
}
	...
	case BCS_ParentBoneSpace:
	{
		if (InMeshBases.GetPose().IsValid())
		{
			...
			if (CompactBoneIndex != INDEX_NONE)
			{
				...
				{
					UE_LOGF(LogAnimation, Warning, "Using socket(%d), Socket name(%ls), Bone name(%ls)", 
