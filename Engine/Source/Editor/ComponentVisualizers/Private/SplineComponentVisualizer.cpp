...
namespace SplineComponentVisualizerLocals
{
	...
	bool GIgnoreSpuriousDeselect = true;
	FAutoConsoleVariableRef CVarIgnoreSpuriousDeselect(
		TEXT("SplineComponentVisualizer.IgnoreSpuriousDeselect"),
		GIgnoreSpuriousDeselect,
		TEXT("When true, OnDeselectedInEditor ignores deselect broadcasts whose component we are no longer editing or that is still in the editor's selection set. Set false to restore the prior unconditional EndEditing.")
	);
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	OnObjectTransactedHandle = FCoreUObjectDelegates::OnObjectTransacted.AddLambda([this](UObject* Object, const class FTransactionObjectEvent& Transaction)
	{
		if (Object == SelectionState && Transaction.GetEventType() == ETransactionObjectEventType::UndoRedo)
		{
			// Because the set of elements to draw is a function of a spline and a selection,
			// changing the selection invalidates all caches, not just the cached elements for any particular spline.
			PDICache.Empty();
		}
	});
}
...
void FSplineComponentVisualizer::DrawVisualization(const UActorComponent* Component, const FSceneView* View, FPrimitiveDrawInterface* PDI)
{
	...
	if (const USplineComponent* SplineComp = Cast<const USplineComponent>(Component))
	{
		...
		if (!PDICache.Find(SplineComp))
		{
			...
			// We don't retain the handles so lets just make sure we don't double register if our cache got cleared.
			MutableSplineComponent->TransformUpdated.RemoveAll(this);
			MutableSplineComponent->GetOnSplineChanged().RemoveAll(this);
			MutableSplineComponent->GetOnSplineUpdated().RemoveAll(this);
			MutableSplineComponent->GetOnSplineDisplayChanged().RemoveAll(this);
...
bool FSplineComponentVisualizer::HandleInputDelta(FEditorViewportClient* ViewportClient, FViewport* Viewport, FVector& DeltaTranslateIn, FRotator& DeltaRotate, FVector& DeltaScale)
{
	...
	// (see FLevelEditorViewportClient::ProjectSelectedElementsIntoWorld). However we can at least support the limited but useful case
	...
	{
		const bool bHasLastKey = SelectionState->GetLastKeyIndexSelected() != INDEX_NONE;
		const bool bHasSelectedKeys = SelectionState->GetSelectedKeys().Num() > 0;
		if (bHasLastKey && bHasSelectedKeys)
		{
		...
		else if (bHasLastKey != bHasSelectedKeys)
		{
			// One half of the selection is set, the other isn't. Reconcile -- TransformSelectedKeys would assert.
			UE_LOG(LogSplineComponentVisualizer, Log, TEXT("Stale selection drift at TransformSelectedKeys gate -- clearing"));
			ChangeSelectionState(INDEX_NONE, false);
		}
	}
...
void FSplineComponentVisualizer::OnSnapKeyToNearestSplinePoint(ESplineComponentSnapMode InSnapMode)
{
	...
	{
		UE_LOGF(LogSplineComponentVisualizer, Warning, "No nearest spline point found.");
		return;
	}
...
void FSplineComponentVisualizer::OnDeleteKey()
{
	...
	// Select the first valid key before the deleted range
	const int32 LastKeyIndex = SelectedKeysSorted.Last();
	ChangeSelectionState(LastKeyIndex > 1 ? LastKeyIndex - 1 : 0, false);
	SelectionState->Modify();
	SelectionState->ClearSelectedSegmentIndex();
	SelectionState->ClearSelectedTangentHandle();
...
void FSplineComponentVisualizer::OnAddAttributeKey()
{
	...
	const int32 NewAttributeIndex = SplineComp->AddFloatPropertyAtSplineInputKey(NearestParam, CurrentValue, SelectionState->GetSelectedAttributeName());
...
{
	if (SplineComponentVisualizerLocals::GIgnoreSpuriousDeselect && SplineComponent)
	{
		// Broadcast is for a component we've already moved past. HandleProxyForComponentVis updates our
		// path before queued deselect notifications fire for the previously-edited spline.
		if (SplineComponent != GetEditedSplineComponent())
		{
			return;
		}

		// Component is still in the editor's selection set -- transient deselect-then-reselect cycle.
		if (GEditor && GEditor->GetSelectedComponents()->IsSelected(SplineComponent))
		{
			return;
		}
	}
