...
{
	if (!bAreContentSourcesUpToDate)
	{
		// Using mutable to avoid removing the const interface since this is just a caching mechanism.
		FFeaturePackContentSourceProvider* MutableThis = const_cast<FFeaturePackContentSourceProvider*>(this);
		MutableThis->RefreshFeaturePacks();
	}
...
{
	bAreContentSourcesUpToDate = false;
}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FFeaturePackContentSourceProvider::RefreshFeaturePacks);
	...
	bAreContentSourcesUpToDate = true;

	// Assume for now that this delegate doesn't support being called from any thread.
