...
bool FFeaturePackContentSource::InstallToProject(FString InstallPath)
{
	...
	{
		UE_LOGF(LogFeaturePack, Warning, "Trying to install invalid pack %ls", *InstallPath);
	}
	else
	{
		...
		if( bContentsInPakFile == true)
		{
			...
			{
				UE_LOGF(LogFeaturePack, Warning, "No objects imported installing pack %ls", *InstallPath);
			}
			else
			{
...
void FFeaturePackContentSource::ParseAndImportPacks()
{
	...
	for (int32 iPackEntry = 0; iPackEntry < PacksToAdd.Num(); iPackEntry++)
	{
		...
		if ((EachPackData.PackSource.IsEmpty() == false) && (EachPackData.PackName.IsEmpty() == false))
		{
			...
			{
				UE_LOGF(LogFeaturePack, Warning, "No objects imported installing pack %ls", *EachPackData.PackSource);
			}
			else
			{
				// Save any imported assets.
	...
	UE_LOGF(LogFeaturePack, Log, "Inserted %d feature packs", PacksInserted);
}

void FFeaturePackContentSource::RecordAndLogError(const FString& ErrorString)
{
	UE_LOGF(LogFeaturePack, Error, "%ls", *ErrorString);
...
void FFeaturePackContentSource::CopyAdditionalFilesToFolder( const FString& DestinationFolder, TArray<FString>& FilesCopied, bool &bHasSourceFiles, FString InGameFolder )
{
	...
	for (int32 iFile = 0; iFile < FilesToAdd.Num(); ++iFile)
	{
		...
		if (BaseFolderNameIndex != INDEX_NONE)
		{
			...
			{
				UE_LOGF(LogFeaturePack, Warning, "Failed to copy %ls to %ls", *EachFile, *FinalDestination);
			}
		}
	}
}
	...
	{		
		...
		else
		{
			...
			if (FPlatformFileManager::Get().GetPlatformFile().FileExists(*FullPath))
			{
				...
				{
					UE_LOGF(LogFeaturePack, Warning, "Unable to install feature pack: '%ls'", *FullPath);
				}
			}
			else
			{
				UE_LOGF(LogFeaturePack, Warning, "Unable to find feature pack: '%ls'", *FullPath);
			}
		}
