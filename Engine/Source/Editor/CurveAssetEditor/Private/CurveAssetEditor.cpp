...
struct FCurveAssetEditorTreeItem : public ICurveEditorTreeItem
{
	...
	virtual void CreateCurveModels(TArray<TUniquePtr<FCurveModel>>& OutCurveModels) override
	{
		...
		NewCurve->SetChannelName(EditInfo.CurveName);
 ...
 */
static TUniquePtr<FCurveModel> ResolveCurveModel(const CurveEditor::FCurveModelLookUpArgs& InArgs)
{
	...
	UCurveBase* CurveOwner = Cast<UCurveBase>(Metadata->Owner.Get());
	if (!CurveOwner)
	{
		return nullptr;
	}

	TArray<FRichCurveEditInfo> CurveInfos = CurveOwner->GetCurves();
	if (CurveInfos.Num() == 1)
	{
		return MakeUnique<FRichCurveEditorModelRaw>(static_cast<FRichCurve*>(CurveInfos[0].CurveToEdit), CurveOwner);
	}
	const FRichCurveEditInfo* Data = CurveInfos.FindByPredicate([Metadata](const FRichCurveEditInfo& Data)
	{
		return Data.CurveName == Metadata->ChannelName;
	});
	return Data ? MakeUnique<FRichCurveEditorModelRaw>(static_cast<FRichCurve*>(Data->CurveToEdit), CurveOwner) : nullptr;
}
}
...
TSharedRef<SDockTab> FCurveAssetEditor::SpawnTab_CurveAsset( const FSpawnTabArgs& Args )
{
	...
	InitParams.ResolveCurveModelDelegate.BindStatic(&UE::CurveAssetEditor::ResolveCurveModel); // Needed for undo / redo
	...
	if (Curve)
	{
		...
		{
			UE_LOGF(LogTemp, Error, "Non-rich curves are not supported in the Curve Asset editor at this time.");
		}
	}
