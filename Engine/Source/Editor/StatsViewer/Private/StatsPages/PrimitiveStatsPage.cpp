...
struct PrimitiveStatsGenerator
{
	...
	//	check(World); // @todo: re-instate this check once the GWorld migration has completed
		...
		{
			// Retrieve number of relevant lights.
			const int32 RelevantLightCount = World->Scene->GetRelevantLightCount(InPrimitiveComponent);
			...
			if( !bUsesOnlyUnlitMaterials )
			{
				...
				LightsOtherCount = RelevantLightCount >= LightsLMCount ? RelevantLightCount - LightsLMCount : 0;
			}
