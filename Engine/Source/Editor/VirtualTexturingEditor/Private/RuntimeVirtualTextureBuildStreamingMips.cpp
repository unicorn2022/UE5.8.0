...
namespace RuntimeVirtualTexture
{
	...
	bool BuildStreamedMips(EShadingPath ShadingPath, URuntimeVirtualTextureComponent* InComponent)
	{
		...
		{
			UE_LOGF(LogVirtualTexturing, Error, "Trying to build streaming mips for a component (from actor %ls) that is not registered. It will be ignored", *InComponent->GetOwner()->GetActorNameOrLabel());
			return false;
		}
		...
		UE_LOGF(LogVirtualTexturing, Display, "Allocating %uMiB for RenderTileResourcesBytes",(uint32)(RenderTileResourcesBytes/(1024*1024)));
	...
	FBuildAllStreamedMipsResult BuildAllStreamedMips(const FBuildAllStreamedMipsParams& InParams)
	{
		...
		for (URuntimeVirtualTextureComponent* Component : InParams.Components)
		{
			...
			{
				UE_LOGF(LogVirtualTexturing, Error, "Trying to build streaming mips for a component (from actor %ls) that is not registered. It will be ignored", *Component->GetOwner()->GetActorNameOrLabel());
			}
		}
			...
			{
				{
					...
					InParams.World->ShaderPlatformChanged(GShaderPlatformForFeatureLevel[AltFeatureLevel]);
				...
				if (InParams.bRestoreFeatureLevelAfterBuilding)
				{
					...
					InParams.World->ShaderPlatformChanged(GShaderPlatformForFeatureLevel[CurFeatureLevel]);
				}
			}
		}
