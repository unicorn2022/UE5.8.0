	...
	FScopedSlowTask& Task)
{
	...
	while (MaterialHeap.Num() > 0)
	{
		...
		{
			UE_LOGF(LogVirtualTextureConversion, Display, "Circular inheritance chain!? %ls", *ParentMaterial->GetName());
			continue;
		}
		...
		for (FMaterialParameterInfo& ParamInfo : ParameterInfos)
		{
			...
			if (ParentMaterial->GetTextureParameterValue(ParamInfo, ParamValue))
			{
				...
				if (ParamValue2D != nullptr)
				{
					...
					{
						//UE_LOGF(LogVirtualTextureConversion, Display, "Adding parameter %ls because it references  %ls on %ls >> %ls", *ParamInfo.Name.ToString(), *ParamValue2D->GetPathName(), *ParentMaterial->GetMaterial()->GetPathName(), *ParentMaterial->GetPathName());
...
void FVirtualTextureConversionWorker::DoConvert()
{
	...
	UE_LOGF(LogVirtualTextureConversion, Display, "Beginning conversion...");
	SlowTask.EnterProgressFrame();
	{
		...
		for (UTexture2D *Tex : Textures)
		{
			...
			if (TextureToUpdate->GetPathName().StartsWith(TEXT("/Engine/")) && TextureToUpdate->GetPackage() != GetTransientPackage())
			{
				...
				if (!TextureToUpdate)
				{
					UE_LOGF(LogVirtualTextureConversion, Warning, "Failed to duplicate engine texture %ls", *Tex->GetPathName());
				}
			}

			if (TextureToUpdate)
			{
				UE_LOGF(LogVirtualTextureConversion, Display, "Texture %ls", *TextureToUpdate->GetName());
	...
	SlowTask.EnterProgressFrame();
	{
		...
		{
			UE_LOGF(LogVirtualTextureConversion, Display, "Material %ls", *Mat->GetName());
			...
			for (UMaterialExpression *Expr : Mat->GetExpressions())
			{
				...
				if (TexExpr)
				{
					...
					{
						UE_LOGF(LogVirtualTextureConversion, Display, "Adjusting sampler %ls.", *TexExpr->GetName());
			...
			if (MatModified)
			{
				UE_LOGF(LogVirtualTextureConversion, Display, "Material %ls added to update list.", *Mat->GetName());
				ModifiedMaterials.Add(Mat);
			}
			else
			{
				UE_LOGF(LogVirtualTextureConversion, Display, "Material %ls was not modified, skipping.", *Mat->GetName());
			}
		}
		...
		{
			UE_LOGF(LogVirtualTextureConversion, Display, "Function %ls", *Func->GetName());
			...
			if (FuncModified)
			{
				UE_LOGF(LogVirtualTextureConversion, Display, "Function %ls added to update list.", *Func->GetName());
				ModifiedMaterialFunctions.Add(Func);
			}
			else
			{
				UE_LOGF(LogVirtualTextureConversion, Display, "Function %ls was not modified, skipping.", *Func->GetName());
			}
		}
		...
		UE_LOGF(LogVirtualTextureConversion, Display, "Broadcasting to editor.");
