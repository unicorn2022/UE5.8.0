...
public:
	...
	*/
	...
	/** Get selected prims */
	virtual const TArray<FPrimData>& GetSelectedPrims() const = 0;
	...
	/** Returns whether simple collision is currently shown in the viewport. */
	virtual bool IsShowSimpleCollisionChecked() const = 0;

	/** Toggles simple collision visibility in the viewport. */
	virtual void ToggleShowSimpleCollision() = 0;
	...
	*/
	...
	DECLARE_DELEGATE_RetVal_OneParam(bool, FAddSaveAssetValidator, FString&);
	virtual FDelegateHandle AddSaveAssetValidator(const FAddSaveAssetValidator& Validator)
	{
		check(Validator.IsBound());
		const FDelegateHandle ValidatorHandle = Validator.GetHandle();
		SaveAssetValidators.Add(ValidatorHandle, Validator);
		return ValidatorHandle;
	};
	virtual bool RemoveSaveAssetValidator(const FDelegateHandle& ValidatorHandle)
	{
		return SaveAssetValidators.Remove(ValidatorHandle) > 0;
	};
...
protected:
	...
	{
		TArray<FDelegateHandle> ValidatorHandles;
		SaveAssetValidators.GetKeys(ValidatorHandles);

		for (const FDelegateHandle& Handle : ValidatorHandles)
		{
			const FAddSaveAssetValidator* Validator = SaveAssetValidators.Find(Handle);
			if (Validator && Validator->IsBound())
			{
				FString Err;
				if (!Validator->Execute(Err))
				{
					FMessageDialog::Open(
						EAppMsgType::Ok,
...
private:
	...
	TMap<FDelegateHandle, FAddSaveAssetValidator> SaveAssetValidators;
};
