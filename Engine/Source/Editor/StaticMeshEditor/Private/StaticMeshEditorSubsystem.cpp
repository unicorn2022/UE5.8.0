...
namespace InternalEditorMeshLibrary
{
	...
	bool IsUVChannelValid(UStaticMesh* StaticMesh, int32 LODIndex, int32 UVChannelIndex)
	{
		...
		{
			UE_LOGF(LogStaticMeshEditorSubsystem, Error, "The StaticMesh is null.");
			return false;
		}

		if (LODIndex >= StaticMesh->GetNumLODs() || LODIndex < 0)
		{
			UE_LOGF(LogStaticMeshEditorSubsystem, Error, "The StaticMesh doesn't have LOD %d.", LODIndex);
			return false;
		}

		if (!StaticMesh->IsMeshDescriptionValid(LODIndex))
		{
			UE_LOGF(LogStaticMeshEditorSubsystem, Error, "No mesh description for LOD %d.", LODIndex);
			return false;
		}
		...
		{
			UE_LOGF(LogStaticMeshEditorSubsystem, Error, "The given UV channel index %d is out of bounds.", UVChannelIndex);
			return false;
		}
	...
	bool FindValidActorAndComponents(TArray<AStaticMeshActor*> ActorsToTest, TArray<AStaticMeshActor*>& OutValidActor, TArray<TPrimitiveComponent*>& OutPrimitiveComponent, FVector& OutAverageLocation, FString& OutFailureReason)
	{
		...
		OutValidActor.Reset(ActorsToTest.Num());
		{
			...
			for (AStaticMeshActor* MeshActor : ActorsToTest)
			{
				...
				{
					UE_LOGF(LogStaticMeshEditorSubsystem, Log, "Not all actors are from the same level. The Actor will be created in the first level found.");
...
int32 UStaticMeshEditorSubsystem::SetLodsWithNotification(UStaticMesh* StaticMesh, const FStaticMeshReductionOptions& ReductionOptions, bool bApplyChanges)
{
	...
	if (StaticMesh == nullptr)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "SetLODs: The StaticMesh is null.");
		return -1;
	}
	...
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "SetLODs: This StaticMesh does not have LOD 0.");
		return -1;
	}

	if (ReductionOptions.ReductionSettings.Num() == 0)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "SetLODs: Nothing done as no LOD settings were provided.");
		return -1;
	}
...
void UStaticMeshEditorSubsystem::GetLodReductionSettings(const UStaticMesh* StaticMesh, const int32 LodIndex, FMeshReductionSettings& OutReductionOptions)
{
	...
	if (StaticMesh == nullptr)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "GetLodReductionSettings: The StaticMesh is null.");
		return;
	}
	...
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "GetLodReductionSettings: Invalid LOD index.");
		return;
	}
...
void UStaticMeshEditorSubsystem::SetLodReductionSettings(UStaticMesh* StaticMesh, const int32 LodIndex, const FMeshReductionSettings& ReductionOptions)
{
	...
	if (StaticMesh == nullptr)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "SetLodReductionSettings: The StaticMesh is null.");
		return;
	}
	...
	if (LodIndex < 0 || StaticMesh->GetNumSourceModels() <= LodIndex)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "SetLodReductionSettings: Invalid LOD index.");
		return;
	}
...
void UStaticMeshEditorSubsystem::GetLodBuildSettings(const UStaticMesh* StaticMesh, const int32 LodIndex, FMeshBuildSettings& OutBuildOptions)
{
	...
	if (StaticMesh == nullptr)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "GetLodBuildSettings: The StaticMesh is null.");
		return;
	}
	...
	if (LodIndex < 0 || StaticMesh->GetNumSourceModels() <= LodIndex)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "GetLodBuildSettings: Invalid LOD index.");
		return;
	}
...
void UStaticMeshEditorSubsystem::SetLodBuildSettings(UStaticMesh* StaticMesh, const int32 LodIndex, const FMeshBuildSettings& BuildOptions)
{
	...
	if (StaticMesh == nullptr)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "SetLodBuildSettings: The StaticMesh is null.");
		return;
	}
	...
	if (LodIndex < 0 || StaticMesh->GetNumSourceModels() <= LodIndex)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "SetLodBuildSettings: Invalid LOD index.");
		return;
	}
...
FName UStaticMeshEditorSubsystem::GetLODGroup(const UStaticMesh* StaticMesh)
{
	if (StaticMesh == nullptr)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "StaticMesh GetLODGroup: The StaticMesh is null.");
		return NAME_None;
	}
...
bool UStaticMeshEditorSubsystem::SetLODGroup(UStaticMesh* StaticMesh, FName LODGroup, bool bRebuildImmediately)
{
	if (StaticMesh == nullptr)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "StaticMesh SetLODGroup: The StaticMesh is null.");
		return false;
	}
	...
	UE_LOGF(LogStaticMeshEditorSubsystem, Error, "SetLODGroup: %ls is not a valid LODGroup", *LODGroup.ToString());
	return false;
...
int32 UStaticMeshEditorSubsystem::ImportLOD(UStaticMesh* BaseStaticMesh, const int32 LODIndex, const FString& SourceFilename)
{
	...
	if (!EditorScriptingHelpers::CheckIfInEditorAndPIE())
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "StaticMesh ImportLOD: Cannot import or re-import when editor PIE is active.");
		return INDEX_NONE;
	}

	if (BaseStaticMesh == nullptr)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "StaticMesh ImportLOD: The StaticMesh is null.");
		return INDEX_NONE;
	}
	...
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "StaticMesh ImportLOD: Invalid LODIndex, the LOD index cannot be greater the the number of LOD, static mesh cannot have hole in the LOD array.");
		return INDEX_NONE;
	}
	...
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "StaticMesh ImportLOD: Invalid source filename.");
		return INDEX_NONE;
	}
	...
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "StaticMesh ImportLOD: Cannot import mesh LOD.");
		return INDEX_NONE;
	}
...
bool UStaticMeshEditorSubsystem::ReimportAllCustomLODs(UStaticMesh* StaticMesh)
{
	...
	if (!EditorScriptingHelpers::CheckIfInEditorAndPIE())
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "StaticMesh ReimportAllCustomLODs: Cannot import or re-import when editor PIE is active.");
		return false;
	}

	if (StaticMesh == nullptr)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "StaticMesh ReimportAllCustomLODs: The StaticMesh is null.");
		return false;
	}
	...
	for (int32 LODIndex = 1; LODIndex < LODNumber; ++LODIndex)
	{
		...
		{
			UE_LOGF(LogStaticMeshEditorSubsystem, Error, "StaticMesh ReimportAllCustomLODs: Cannot re-import LOD %d.", LODIndex);
			bResult = false;
		}
	}
	return bResult;
...
int32 UStaticMeshEditorSubsystem::SetLodFromStaticMesh(UStaticMesh* DestinationStaticMesh, int32 DestinationLodIndex, UStaticMesh* SourceStaticMesh, int32 SourceLodIndex, bool bReuseExistingMaterialSlots)
{
	...
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "SetLodFromStaticMesh: The DestinationStaticMesh is null.");
		return -1;
	}

	if (SourceStaticMesh == nullptr)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "SetLodFromStaticMesh: The SourceStaticMesh is null.");
		return -1;
	}

	if (!SourceStaticMesh->IsSourceModelValid(SourceLodIndex))
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "SetLodFromStaticMesh: SourceLodIndex is invalid.");
		return -1;
	}
	...
	while (!SourceStaticMesh->IsMeshDescriptionValid(BaseSourceLodIndex))
	{
		...
		{
			UE_LOGF(LogStaticMeshEditorSubsystem, Error, "SetLodFromStaticMesh: The SourceStaticMesh is in a invalid state.");
			return -1;
		}
	...
	if (bIsReductionSettingAproximated)
	{
		...
		UE_LOGF(LogStaticMeshEditorSubsystem, Warning, "%ls", *FString::Format(TEXT("SetLodFromStaticMesh: The reduction settings from the SourceStaticMesh {0} LOD {1} were approximated."
...
int32 UStaticMeshEditorSubsystem::GetLodCount(const UStaticMesh* StaticMesh)
{
	...
	if (StaticMesh == nullptr)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "GetLODCount: The StaticMesh is null.");
		return -1;
	}
...
bool UStaticMeshEditorSubsystem::RemoveLods(UStaticMesh* StaticMesh)
{
	...
	if (StaticMesh == nullptr)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "RemoveLODs: The StaticMesh is null.");
		return false;
	}
	...
	if (StaticMesh->GetNumSourceModels() == 0)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "RemoveLODs: This StaticMesh does not have LOD 0.");
		return false;
	}
...
TArray<float> UStaticMeshEditorSubsystem::GetLodScreenSizes(const UStaticMesh* StaticMesh)
{
	...
	if (StaticMesh == nullptr)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "GetLodScreenSizes: The StaticMesh is null.");
		return ScreenSizes;
	}
	...
	for (int i = 0; i < StaticMesh->GetNumLODs(); i++)
	{
		...
		{
			UE_LOGF(LogStaticMeshEditorSubsystem, Warning, "GetLodScreenSizes: The RenderData is invalid for LOD %d.", i);
		}
	}
...
bool UStaticMeshEditorSubsystem::SetLodScreenSizes(UStaticMesh* StaticMesh, const TArray<float>& ScreenSizes)
{
	...
	if (StaticMesh == nullptr)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "SetLodScreenSizes: Input StaticMesh is null.");
		return false;
	}
	...
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "SetLodScreenSizes: Input StaticMesh is invalid (missing RenderData or meshes).");
		return false;
	}

	if (ScreenSizes.Num() == 0)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "SetLodScreenSizes: Input ScreenSizes array is empty.");
		return false;
	}
	...
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Warning, "SetLodScreenSizes: Only %d of %d ScreenSizes provided, remainder will be set to arbitrary defaults.",
	...
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Warning, "SetLodScreenSizes: Some input values were sanitized to be monotonic.");
	}
...
bool UStaticMeshEditorSubsystem::ImportNaniteHiResMesh(UStaticMesh* StaticMesh, const FString& SourceFilename, bool bShowDialogWhenFileMissing)
{
	if (!StaticMesh)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "ImportNaniteHiResMesh: The StaticMesh is null.");
		return false;
	}
	...
	if(bSourceFileExists)
	{
		...
		{
			UE_LOGF(LogStaticMeshEditorSubsystem, Error, "ImportNaniteHiResMesh: Cannot import hi res mesh.");
			bResult = false;
		}
	}
	else if (bShowDialogWhenFileMissing)
	{
		...
		{
			UE_LOGF(LogStaticMeshEditorSubsystem, Error, "ImportNaniteHiResMesh: Cannot import hi res mesh with dialog.");
			bResult = false;
		}
	}
	else
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "ImportNaniteHiResMesh: Cannot import hi res mesh, the provided file is invalid.");
		bResult = false;
	}
...
FString UStaticMeshEditorSubsystem::GetNaniteSourceFilename(const UStaticMesh* StaticMesh)
{
	...
	if (!StaticMesh)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "GetNaniteSourceFilename: The StaticMesh is null.");
...
bool UStaticMeshEditorSubsystem::UpdateNaniteSourceFilename(UStaticMesh* StaticMesh, const FString& NewSourceFilename)
{
	...
	if (!StaticMesh)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "UpdateNaniteSourceFilename: The StaticMesh is null.");
		return bResult;
	}
	if (!FPaths::FileExists(ResolveFilename))
	{
	...
	if (!StaticMesh->GetHiResSourceModel().SourceImportFilename.Equals(ResolveFilename))
	{
		...
		{
			UE_LOGF(LogStaticMeshEditorSubsystem, Error, "UpdateNaniteSourceFilename: The new source filename is invalid (it can be empty or must point to an existing filename).");
		}
	}
	return bResult;
}
...
int32 UStaticMeshEditorSubsystem::AddSimpleCollisionsWithNotification(UStaticMesh* StaticMesh, const EScriptCollisionShapeType ShapeType, bool bApplyChanges)
{
	...
	if (StaticMesh == nullptr)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "AddSimpleCollisions: The StaticMesh is null.");
		return INDEX_NONE;
	}
...
int32 UStaticMeshEditorSubsystem::GetSimpleCollisionCount(const UStaticMesh* StaticMesh)
{
	...
	if (StaticMesh == nullptr)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "GetSimpleCollisionCount: The StaticMesh is null.");
		return -1;
	}
...
TEnumAsByte<ECollisionTraceFlag> UStaticMeshEditorSubsystem::GetCollisionComplexity(const UStaticMesh* StaticMesh)
{
	...
	if (StaticMesh == nullptr)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "GetCollisionComplexity: The StaticMesh is null.");
...
int32 UStaticMeshEditorSubsystem::GetConvexCollisionCount(const UStaticMesh* StaticMesh)
{
	...
	if (StaticMesh == nullptr)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "GetConvexCollisionCount: The StaticMesh is null.");
		return -1;
	}
...
bool UStaticMeshEditorSubsystem::BulkSetConvexDecompositionCollisionsWithNotification(const TArray<UStaticMesh*>& InStaticMeshes, int32 HullCount, int32 MaxHullVerts, int32 HullPrecision, bool bApplyChanges)
{
	...
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "SetConvexDecompositionCollisions: The StaticMesh is null.");
		return false;
	}

	if (HullCount < 0 || HullPrecision < 0)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "SetConvexDecompositionCollisions: Parameters HullCount and HullPrecision must be positive.");
		return false;
	}
...
bool UStaticMeshEditorSubsystem::RemoveCollisionsWithNotification(UStaticMesh* StaticMesh, bool bApplyChanges)
{
	...
	if (StaticMesh == nullptr)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "RemoveCollisions: The StaticMesh is null.");
		return false;
	}

	if (StaticMesh->GetBodySetup() == nullptr)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Log, "RemoveCollisions: No collision set up. Nothing to do.");
		return true;
	}
...
bool UStaticMeshEditorSubsystem::CanModifyStaticMeshSection(const UStaticMesh* StaticMesh, int32 LODIndex, int32 SectionIndex, const FString& LogFunctionName)
{
	...
	if (StaticMesh == nullptr)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "%ls: The StaticMesh is null.", *LogFunctionName);
		return false;
	}

	if (LODIndex >= StaticMesh->GetNumLODs())
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "%ls: Invalid LOD index %d (of %d).", *LogFunctionName, LODIndex, StaticMesh->GetNumLODs());
		return false;
	}

	if (SectionIndex >= StaticMesh->GetNumSections(LODIndex))
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "%ls: Invalid section index %d (of %d).", *LogFunctionName, SectionIndex, StaticMesh->GetNumSections(LODIndex));
		return false;
	}
...
void UStaticMeshEditorSubsystem::SetLODMaterialSlot(UStaticMesh* StaticMesh, int32 MaterialSlotIndex, int32 LODIndex, int32 SectionIndex)
{
	...
	if (StaticMesh == nullptr)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "SetLODMaterialSlot: The StaticMesh is null.");
		return;
	}

	if (LODIndex >= StaticMesh->GetNumLODs())
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "SetLODMaterialSlot: Invalid LOD index %d (of %d).", LODIndex, StaticMesh->GetNumLODs());
		return;
	}

	if (SectionIndex >= StaticMesh->GetNumSections(LODIndex))
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "SetLODMaterialSlot: Invalid section index %d (of %d).", SectionIndex, StaticMesh->GetNumSections(LODIndex));
		return;
	}

	if (MaterialSlotIndex >= StaticMesh->GetStaticMaterials().Num())
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "SetLODMaterialSlot: Invalid slot index %d (of %d).", MaterialSlotIndex, StaticMesh->GetStaticMaterials().Num());
		return;
	}
...
int32 UStaticMeshEditorSubsystem::GetLODMaterialSlot(const UStaticMesh* StaticMesh, int32 LODIndex, int32 SectionIndex )
{
	...
	if ( StaticMesh == nullptr )
	{
		UE_LOGF( LogStaticMeshEditorSubsystem, Error, "GetLODMaterialSlot: The StaticMesh is null." );
		return INDEX_NONE;
	}

	if ( LODIndex >= StaticMesh->GetNumLODs() )
	{
		UE_LOGF( LogStaticMeshEditorSubsystem, Error, "GetLODMaterialSlot: Invalid LOD index %d (of %d).", LODIndex, StaticMesh->GetNumLODs() );
		return INDEX_NONE;
	}

	if ( SectionIndex >= StaticMesh->GetNumSections( LODIndex ) )
	{
		UE_LOGF( LogStaticMeshEditorSubsystem, Error, "GetLODMaterialSlot: Invalid section index %d (of %d).", SectionIndex, StaticMesh->GetNumSections( LODIndex ) );
		return INDEX_NONE;
	}
...
bool UStaticMeshEditorSubsystem::HasVertexColors(const UStaticMesh* StaticMesh)
{
	...
	if (StaticMesh == nullptr)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "HasVertexColors: The StaticMesh is null.");
		return false;
	}
...
bool UStaticMeshEditorSubsystem::HasInstanceVertexColors(const UStaticMeshComponent* StaticMeshComponent)
{
	...
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "HasInstanceVertexColors: The StaticMeshComponent is null.");
		return false;
	}
...
bool UStaticMeshEditorSubsystem::SetGenerateLightmapUVs(UStaticMesh* StaticMesh, bool bGenerateLightmapUVs)
{
	...
	if (StaticMesh == nullptr)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "SetGenerateLightmapUVs: The StaticMesh is null.");
		return false;
	}
...
int32 UStaticMeshEditorSubsystem::GetNumberVerts(const UStaticMesh* StaticMesh, int32 LODIndex)
{
	...
	if (StaticMesh == nullptr)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "GetNumberVerts: The StaticMesh is null.");
		return 0;
	}
...
int32 UStaticMeshEditorSubsystem::GetNumberMaterials(const UStaticMesh* StaticMesh)
{
	...
	if (StaticMesh == nullptr)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "GetNumberMaterials: The StaticMesh is null.");
		return 0;
	}
...
void UStaticMeshEditorSubsystem::SetAllowCPUAccess(UStaticMesh* StaticMesh, bool bAllowCPUAccess)
{
	...
	if (StaticMesh == nullptr)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "SetAllowCPUAccess: The StaticMesh is null.");
		return;
	}
...
int32 UStaticMeshEditorSubsystem::GetNumUVChannels(const UStaticMesh* StaticMesh, int32 LODIndex)
{
	...
	if (StaticMesh == nullptr)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "GetNumUVChannels: The StaticMesh is null.");
		return 0;
	}

	if (LODIndex >= StaticMesh->GetNumLODs() || LODIndex < 0)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "GetNumUVChannels: The StaticMesh doesn't have LOD %d.", LODIndex);
		return 0;
	}
...
bool UStaticMeshEditorSubsystem::AddUVChannel(UStaticMesh* StaticMesh, int32 LODIndex)
{
	...
	if (StaticMesh == nullptr)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "AddUVChannel: The StaticMesh is null.");
		return false;
	}

	if (LODIndex >= StaticMesh->GetNumLODs() || LODIndex < 0)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "AddUVChannel: The StaticMesh doesn't have LOD %d.", LODIndex);
		return false;
	}

	if (StaticMesh->GetNumUVChannels(LODIndex) >= MAX_MESH_TEXTURE_COORDS_MD)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "AddUVChannel: Cannot add UV channel. Maximum number of UV channels reached (%d).", MAX_MESH_TEXTURE_COORDS_MD);
		return false;
	}
...
bool UStaticMeshEditorSubsystem::InsertUVChannel(UStaticMesh* StaticMesh, int32 LODIndex, int32 UVChannelIndex)
{
	...
	if (StaticMesh == nullptr)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "InsertUVChannel: The StaticMesh is null.");
		return false;
	}

	if (LODIndex >= StaticMesh->GetNumLODs() || LODIndex < 0)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "InsertUVChannel: The StaticMesh doesn't have LOD %d.", LODIndex);
		return false;
	}
	...
	if (UVChannelIndex < 0 || UVChannelIndex > NumUVChannels)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "InsertUVChannel: Cannot insert UV channel. Given UV channel index %d is out of bounds.", UVChannelIndex);
		return false;
	}

	if (NumUVChannels >= MAX_MESH_TEXTURE_COORDS_MD)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "InsertUVChannel: Cannot add UV channel. Maximum number of UV channels reached (%d).", MAX_MESH_TEXTURE_COORDS_MD);
		return false;
	}
...
bool UStaticMeshEditorSubsystem::RemoveUVChannel(UStaticMesh* StaticMesh, int32 LODIndex, int32 UVChannelIndex)
{
	...
	if (StaticMesh == nullptr)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "RemoveUVChannel: The StaticMesh is null.");
		return false;
	}

	if (LODIndex >= StaticMesh->GetNumLODs() || LODIndex < 0)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "RemoveUVChannel: The StaticMesh doesn't have LOD %d.", LODIndex);
		return false;
	}
	...
	if (NumUVChannels == 1)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "RemoveUVChannel: Cannot remove UV channel. There must be at least one channel.");
		return false;
	}

	if (UVChannelIndex < 0 || UVChannelIndex >= NumUVChannels)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "RemoveUVChannel: Cannot remove UV channel. Given UV channel index %d is out of bounds.", UVChannelIndex);
		return false;
	}
...
void UStaticMeshEditorSubsystem::ReplaceMeshComponentsMaterials(const TArray<UMeshComponent*>& MeshComponents, UMaterialInterface* MaterialToBeReplaced, UMaterialInterface* NewMaterial)
{
	...
	UE_LOGF(LogStaticMeshEditorSubsystem, Log, "ReplaceMeshComponentsMaterials. %d material(s) changed occurred.", ChangeCounter);
}
...
void UStaticMeshEditorSubsystem::ReplaceMeshComponentsMaterialsOnActors(const TArray<AActor*>& Actors, UMaterialInterface* MaterialToBeReplaced, UMaterialInterface* NewMaterial)
{
	...
	UE_LOGF(LogStaticMeshEditorSubsystem, Log, "ReplaceMeshComponentsMaterialsOnActors. %d material(s) changed occurred.", ChangeCounter);
}
...
void UStaticMeshEditorSubsystem::ReplaceMeshComponentsMeshes(const TArray<UStaticMeshComponent*>& MeshComponents, UStaticMesh* MeshToBeReplaced, UStaticMesh* NewMesh)
{
	...
	UE_LOGF(LogStaticMeshEditorSubsystem, Log, "ReplaceMeshComponentsMeshes. %d mesh(es) changed occurred.", ChangeCounter);
}
...
void UStaticMeshEditorSubsystem::ReplaceMeshComponentsMeshesOnActors(const TArray<AActor*>& Actors, UStaticMesh* MeshToBeReplaced, UStaticMesh* NewMesh)
{
	...
	UE_LOGF(LogStaticMeshEditorSubsystem, Log, "ReplaceMeshComponentsMeshesOnActors. %d mesh(es) changed occurred.", ChangeCounter);
}
...
AActor* UStaticMeshEditorSubsystem::JoinStaticMeshActors(const TArray<AStaticMeshActor*>& ActorsToMerge, const FJoinStaticMeshActorsOptions& JoinOptions)
{
	...
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "JoinStaticMeshActors failed. %ls", *FailureReason);
		return nullptr;
	}

	if (AllActors.Num() < 2)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "JoinStaticMeshActors failed. A merge operation requires at least 2 valid Actors.");
		return nullptr;
	}
	...
	if (!NewActor)
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "JoinStaticMeshActors failed. Internal error while creating the join actor.");
		return nullptr;
	}
	...
	UE_LOGF(LogStaticMeshEditorSubsystem, Log, "JoinStaticMeshActors joined %d actors toghether in actor '%ls'.", AllComponents.Num(), *NewActor->GetActorLabel());
	return NewActor;
}
...
bool UStaticMeshEditorSubsystem::MergeStaticMeshActors(const TArray<AStaticMeshActor*>& ActorsToMerge, const FMergeStaticMeshActorsOptions& MergeOptions, AStaticMeshActor*& OutMergedActor)
{
	...
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "MergeStaticMeshActors. Failed to convert the BasePackageName. %ls", *FailureReason);
		return false;
	}
	...
	if (!InternalEditorMeshLibrary::FindValidActorAndComponents(ActorsToMerge, AllActors, AllComponents, PivotLocation, FailureReason))
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "MergeStaticMeshActors failed. %ls", *FailureReason);
		return false;
	}
	...
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "MergeStaticMeshActors failed. No mesh was created.");
		return false;
	}
	...
	if (MergeOptions.bSpawnMergedActor)
	{
		...
		if (!OutMergedActor)
		{
			UE_LOGF(LogStaticMeshEditorSubsystem, Error, "MergeStaticMeshActors failed. Internal error while creating the merged actor.");
			return false;
		}
...
bool UStaticMeshEditorSubsystem::CreateProxyMeshActor(const TArray<class AStaticMeshActor*>& ActorsToMerge, const FCreateProxyMeshActorOptions& MergeOptions, class AStaticMeshActor*& OutMergedActor)
{
	...
	if (PackageName.IsEmpty())
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "CreateProxyMeshActor. Failed to convert the BasePackageName. %ls", *FailureReason);
		return false;
	}
	...
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "CreateProxyMeshActor failed. %ls", *FailureReason);
		return false;
	}
	...
	if (!CreatedAssets.FindItemByClass(&MergedMesh))
	{
		UE_LOGF(LogStaticMeshEditorSubsystem, Error, "CreateProxyMeshActor failed. No mesh created.");
		return false;
	}
	...
	if (MergeOptions.bSpawnMergedActor)
	{
		...
		if (!OutMergedActor)
		{
			UE_LOGF(LogStaticMeshEditorSubsystem, Error, "CreateProxyMeshActor failed. Internal error while creating the merged actor.");
			return false;
		}
