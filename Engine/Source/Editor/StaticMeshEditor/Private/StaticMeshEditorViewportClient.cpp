...
void FStaticMeshEditorViewportClient::TrackingStarted( const struct FInputEventState& InInputState, bool bIsDraggingWidget, bool bNudge )
{
	...
	{
		const bool bDuplicate = InInputState.IsLeftMouseButtonPressed()
			&& InInputState.IsAltButtonPressed()
			&& (Widget->GetCurrentAxis() & EAxisList::XYZ);
		
		BeginManipulation(bDuplicate);
	}
...
bool FStaticMeshEditorViewportClient::BeginTransform(const FGizmoState& InState)
{
	BeginManipulation(IsAltDown(Viewport) && GetWidgetMode() == UE::Widget::WM_Translate);
	return true;
}

void FStaticMeshEditorViewportClient::BeginManipulation(bool bDuplicate)
{
...
bool FStaticMeshEditorViewportClient::EndTransform(const FGizmoState& InState)
{
	if (bManipulating)
	{
		bManipulating = false;
...
void FStaticMeshEditorViewportClient::DrawCanvas(FViewport& InViewport, FSceneView& View, FCanvas& Canvas)
{
	...
	StaticMeshEditorViewport->SetCurrentScreenPercentage(CurrentScreenSize);
	...
	if (StaticMesh->GetBodySetup() && (!(StaticMesh->GetBodySetup()->bHasCookedCollisionData || StaticMesh->GetBodySetup()->bNeverNeedsCookedCollisionData) || StaticMesh->GetBodySetup()->bFailedToCreatePhysicsMeshes))
	{
		...
		Canvas.DrawShadowedText(static_cast<float>(X), static_cast<float>(Y), Message, GetStatsFont(), FLinearColor(1.0f, 0.05f, 0.05f, 1.0f));
	}
