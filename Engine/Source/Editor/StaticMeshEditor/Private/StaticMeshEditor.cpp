...
#include "StaticMeshEditorEdMode.h"
...
public:
	...
	{
		UE_LOGF(LogStaticMesh, Log, "%ls", *InMessage.ToString());
		MakeDialog();
	}
};
...
void FStaticMeshEditor::InitStaticMeshEditor( const EToolkitMode::Type Mode, const TSharedPtr< class IToolkitHost >& InitToolkitHost, UStaticMesh* ObjectToEdit )
{
	...
	GetEditorModeManager().SetDefaultMode(FStaticMeshEditorEdMode::Id);
	GetEditorModeManager().ActivateDefaultMode();

	GetEditorModeManager().SetSupportsViewportITF(true);
...
void FStaticMeshEditor::OnBuildViewportInteractions(UViewportInteractionsBehaviorSource* Source)
{
	FAssetEditorToolkit::OnBuildViewportInteractions(Source);
	FAdvancedPreviewScene::AddPreviewSceneInteractions(Source);
}
...
const TArray<IStaticMeshEditor::FPrimData>& FStaticMeshEditor::GetSelectedPrims() const
{
...
bool FStaticMeshEditor::IsShowSimpleCollisionChecked() const
{
	return GetStaticMeshViewport().IsValid() && GetStaticMeshViewport()->GetViewportClient().IsShowSimpleCollisionChecked();
}

void FStaticMeshEditor::ToggleShowSimpleCollision()
{
	if (GetStaticMeshViewport().IsValid())
	{
		GetStaticMeshViewport()->GetViewportClient().ToggleShowSimpleCollision();
