...
void FStaticMeshComponentDetails::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	IDetailCategoryBuilder& StaticMeshCategory = DetailBuilder.EditCategory("StaticMesh", FText::GetEmpty(), ECategoryPriority::Important);

	// Customize the StaticMesh property to add a refresh button via CustomButtonSlot
	StaticMeshProperty = DetailBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(UStaticMeshComponent, StaticMesh));
	if (StaticMeshProperty->IsValidHandle())
	{
		IDetailPropertyRow& StaticMeshRow = StaticMeshCategory.AddProperty(StaticMeshProperty.ToSharedRef());

		TSharedPtr<IPropertyHandle> StaticMeshHandle = StaticMeshProperty;
		TSharedRef<SObjectPropertyEntryBox> ObjectEntryBox =
			SNew(SObjectPropertyEntryBox)
			.PropertyHandle(StaticMeshProperty)
			...
			[
				PropertyCustomizationHelpers::MakeCustomButton(
					...
					FSimpleDelegate::CreateLambda([StaticMeshHandle]()
					{
						...
						StaticMeshHandle->GetOuterObjects(OuterObjects);

						TArray<UMeshComponent*> ComponentsToClean;
						for (UObject* Obj : OuterObjects)
						{
							if (UMeshComponent* MeshComp = Cast<UMeshComponent>(Obj))
							{
								if (MeshComp->GetNumOverrideMaterials() > MeshComp->GetNumMaterials())
								{
									ComponentsToClean.Add(MeshComp);
								}
							}
						}

						if (ComponentsToClean.Num() > 0)
						{
							FScopedTransaction Transaction(LOCTEXT("CleanUpOverrideMaterials", "Clean Up Override Materials"));

							for (UMeshComponent* MeshComp : ComponentsToClean)
							{
								MeshComp->Modify();
								MeshComp->CleanUpOverrideMaterials();
							}
						}
					}),
					LOCTEXT("CleanUpOverrideMaterialsToolTip", "Clean up override materials that exceed the mesh material count"),
					true,
					TAttribute<EVisibility>::CreateLambda([StaticMeshHandle]()
					{
						if (!StaticMeshHandle.IsValid())
						{
		...
		StaticMeshRow.CustomWidget()
			.NameContent()
			[
				StaticMeshProperty->CreatePropertyNameWidget()
			]
			.ValueContent()
