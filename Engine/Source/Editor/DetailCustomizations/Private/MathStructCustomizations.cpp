...
void FMathStructCustomization::ExtractNumericMetadata(TSharedRef<IPropertyHandle>& PropertyHandle, FNumericMetadata<NumericType>& MetadataOut)
{
	...
	{
		//UE_LOGF(LogPropertyNode, Warning, "Clamp Min (%ls) >= Clamp Max (%ls) for Ranged Numeric", *ClampMinString, *ClampMaxString);
	}
	...
	{
		//UE_LOGF(LogPropertyNode, Warning, "UI Min (%ls) >= UI Max (%ls) for Ranged Numeric", *UIMinString, *UIMaxString);
	}
	...
	if (bPreserveScaleRatio)
	{
		...
		if (WeakHandlePtr.Pin()->GetPerObjectValues(OldValues) == FPropertyAccess::Success)
		{
			...
			for (int32 OutputIndex = 0; OutputIndex < OldValues.Num(); ++OutputIndex)
			{
				...
				// Use double-precision division to compute the ratio, avoiding truncation for integer
				// NumericType (e.g. int32: 2/3 = 0, but 2.0/3.0 = 0.667).
				const double Ratio = (OldValue == NumericType(0))
					? static_cast<double>(NewValue)
					: static_cast<double>(NewValue) / static_cast<double>(OldValue);
					...
					if (ChildHandle != WeakHandlePtr.Pin())
					{
						...
						if (ChildHandle->GetPerObjectValues(ObjectChildValues) == FPropertyAccess::Success)
						{
							...
							for (int32 ChildOutputIndex = 0; ChildOutputIndex < ObjectChildValues.Num(); ++ChildOutputIndex)
							{
								...
								// Apply ratio in double precision; round to nearest integer for integral types.
								NumericType ChildNewValue;
								if constexpr (TIsIntegral<NumericType>::Value)
								{
									ChildNewValue = static_cast<NumericType>(FMath::RoundToInt64(static_cast<double>(ChildOldValue) * Ratio));
								}
								else
								{
									ChildNewValue = static_cast<NumericType>(static_cast<double>(ChildOldValue) * Ratio);
								}
