	...
	if (SelectedKeys.Num() > 0)
	{
		...
		for (int32 Index : SelectedKeys)
		{
			if (Index < 0 || Index >= SplineComp->GetNumberOfSplinePoints())
			{
				...
				{
					UE_LOGF(LogSplineComponentDetails, Error, "Spline component details selected keys contains invalid index %d for spline %ls with %d points",
						Index,
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	...
	if ((Index > 0 && NewValue <= SplineComp->GetInputKeyValueAtSplinePoint(Index - 1)) ||
		(Index < NumPoints - 1 && NewValue >= SplineComp->GetInputKeyValueAtSplinePoint(Index + 1)))
	{
	...
	// Scope the transaction to only include the value change and none of the derived data changes that might arise from NotifyPropertyModified
	{
		...
		if (bModifyOtherPoints)
		{
			...
			if (Index > 0 && NewValue <= SplineComp->GetInputKeyValueAtSplinePoint(Index - 1))
			{
				float Delta = (NewValue - SplineComp->GetInputKeyValueAtSplinePoint(Index));
				for (int32 PrevIndex = 0; PrevIndex < Index; PrevIndex++)
				{
					SplineComp->SetInputKeyValueAtSplinePoint(PrevIndex, SplineComp->GetInputKeyValueAtSplinePoint(PrevIndex) + Delta, false);
				}
			}
			else if (Index < NumPoints - 1 && NewValue >= SplineComp->GetInputKeyValueAtSplinePoint(Index + 1))
			{
				float Delta = (NewValue - SplineComp->GetInputKeyValueAtSplinePoint(Index));
				for (int32 NextIndex = NumPoints - 1; NextIndex >= Index + 1; NextIndex--)
				{
					SplineComp->SetInputKeyValueAtSplinePoint(NextIndex, SplineComp->GetInputKeyValueAtSplinePoint(NextIndex) + Delta, false);
				}
			}
		}

		SplineComp->SetInputKeyValueAtSplinePoint(Index, NewValue, false);
	}
...
void FSplinePointDetails::OnSetPosition(FVector::FReal NewValue, ETextCommit::Type CommitInfo, EAxis::Type Axis)
{
	...
	// Scope the transaction to only include the value change and none of the derived data changes that might arise from NotifyPropertyModified
	{
		...
		for (int32 Index : SelectedKeys)
		{
			...
			{
				UE_LOGF(LogSplineComponentDetails, Error, "Set spline point location: invalid index %d in selected points for spline component %ls which contains %d spline points.",
...
void FSplinePointDetails::OnSetArriveTangent(FVector::FReal NewValue, ETextCommit::Type CommitInfo, EAxis::Type Axis)
{
	...
	// Scope the transaction to only include the value change and none of the derived data changes that might arise from NotifyPropertyModified
	{
		...
		for (int32 Index : SelectedKeys)
		{
			if (Index < 0 || Index >= NumPoints)
			{
				UE_LOGF(LogSplineComponentDetails, Error, "Set spline point arrive tangent: invalid index %d in selected points for spline component %ls which contains %d spline points.",
					Index, *SplineComp->GetPathName(), NumPoints);
				continue;
			}
...
void FSplinePointDetails::OnSetLeaveTangent(FVector::FReal NewValue, ETextCommit::Type CommitInfo, EAxis::Type Axis)
{
	...
	// Scope the transaction to only include the value change and none of the derived data changes that might arise from NotifyPropertyModified
	{
		...
		for (int32 Index : SelectedKeys)
		{
			if (Index < 0 || Index >= NumPoints)
			{
				UE_LOGF(LogSplineComponentDetails, Error, "Set spline point leave tangent: invalid index %d in selected points for spline component %ls which contains %d spline points.",
					Index, *SplineComp->GetPathName(), NumPoints);
				continue;
			}
...
void FSplinePointDetails::OnSetRotation(FRotator::FReal NewValue, ETextCommit::Type CommitInfo, EAxis::Type Axis)
{
	...
	// Scope the transaction to only include the value change and none of the derived data changes that might arise from NotifyPropertyModified
	{
		...
		for (int32 Index : SelectedKeys)
		{
			if (Index < 0 || Index >= NumPoints)
			{
				UE_LOGF(LogSplineComponentDetails, Error, "Set spline point rotation: invalid index %d in selected points for spline component %ls which contains %d spline points.",
					Index, *SplineComp->GetPathName(), NumPoints);
				continue;
			}
...
void FSplinePointDetails::OnSetScale(FVector::FReal NewValue, ETextCommit::Type CommitInfo, EAxis::Type Axis)
{
	...
	// Scope the transaction to only include the value change and none of the derived data changes that might arise from NotifyPropertyModified
	{
		...
		for (int32 Index : SelectedKeys)
		{
			if (Index < 0 || Index >= NumPoints)
			{
				UE_LOGF(LogSplineComponentDetails, Error, "Set spline point scale: invalid index %d in selected points for spline component %ls which contains %d spline points.",
					Index, *SplineComp->GetPathName(), NumPoints);
				continue;
			}
	...
	// Scope the transaction to only include the value change and none of the derived data changes that might arise from NotifyPropertyModified
	{
		...
		if (NewValue.IsValid() && SplinePointTypes.ContainsByPredicate([&NewValue](const TSharedPtr<FString>& InSplinePointType) { return (*InSplinePointType == *NewValue); }))
		{
			...
			for (int32 Index : SelectedKeys)
			{
				if (Index < 0 || Index >= NumPoints)
				{
					UE_LOGF(LogSplineComponentDetails, Error, "Set spline point type: invalid index %d in selected points for spline component %ls which contains %d spline points.",
						Index, *SplineComp->GetPathName(), NumPoints);
					continue;
				}
...
FReply FSplinePointDetails::OnSelectFirstLastSplinePoint(bool bFirst)
{
	...
	if (SplineDetailsInterface)
	{
		...
		if (SplineComp)
		{
			...
			{
				// SplineComp can be cached here while the component visualizer is not the active visualizer,
				// so always activate -- the previous "only on fresh acquisition" gate was unsound.
