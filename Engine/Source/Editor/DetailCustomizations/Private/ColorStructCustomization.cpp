...
namespace UE::Editor::DetailCustomizations
{
	bool IsPropertyInBlueprint(const TSharedPtr<IPropertyHandle>& StructHandle)
	{
		if (!StructHandle)
		{
			return false;
		...
		while (!OuterObjects.IsEmpty())
		{
			const UObject* Object = OuterObjects.Pop();
			if (!Object)
			{
				continue;
			}
...
void FColorStructCustomization::CustomizeHeader(TSharedRef<class IPropertyHandle> InStructPropertyHandle, class FDetailWidgetRow& InHeaderRow, IPropertyTypeCustomizationUtils& StructCustomizationUtils)
{
	...
	bIsInBlueprint = UE::Editor::DetailCustomizations::IsPropertyInBlueprint(InStructPropertyHandle);
...
void FColorStructCustomization::CreateColorPicker(bool bUseAlpha)
{
	...
	// Close the color picker when a property is edited on the same blueprint actor 
	// since it will trigger a rerun construction script and duplicate actor and its subobjects
	// and invalidate property handle since details panel refreshes with objects replaced
	if (!FCoreUObjectDelegates::OnObjectPropertyChanged.IsBoundToObject(this))
	{
		FCoreUObjectDelegates::OnObjectPropertyChanged.AddSPLambda(this, [this](UObject* InObject, FPropertyChangedEvent& InEvent)
			{
				const TSharedPtr<SColorPicker> ColorPicker = GetColorPicker();
				if (StructPropertyHandle.IsValid()
					&& ColorPicker.IsValid()
					&& bIsInBlueprint 
					&& InEvent.ChangeType != EPropertyChangeType::Interactive
					&& InEvent.Property != StructPropertyHandle->GetProperty())
				{
					TArray<UObject*> OuterObjects;
					StructPropertyHandle->GetOuterObjects(OuterObjects);
					
					const UObject* RootOwner = InObject->IsA<AActor>() ? InObject : InObject->GetTypedOuter<AActor>();
					for (const UObject* Object : OuterObjects)
					{
						if (Object == RootOwner || Object->GetTypedOuter<AActor>() == RootOwner)
						{
							DestroyColorPicker();
							return;
						}
...
void FColorStructCustomization::OnSetColorFromColorPicker(FLinearColor NewColor)
{
	...
	PropertyFlags |= (bIsInteractive || bIsInBlueprint) ? EPropertyValueSetFlags::InteractiveChange : 0;
...
void FColorStructCustomization::OnColorPickerInteractiveEnd()
{
	if (bIsInteractive)
	{
		...
		{
			EPropertyValueSetFlags::Type PropertyFlags = EPropertyValueSetFlags::NotTransactable;
			PropertyFlags |= bIsInBlueprint ? EPropertyValueSetFlags::InteractiveChange : 0;
			StructPropertyHandle->SetValueFromFormattedString(LastPickerColorString, PropertyFlags);
		}
	}
}
