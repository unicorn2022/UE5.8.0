	...
	for (int32 ObjectIndex = 0; ObjectIndex < SelectedObjects.Num(); ++ObjectIndex)
	{
		...
		if (ObjectPtr.IsValid())
		{
			...
			if (SceneComponent)
			{
				...
				// Clamp location to world size
				if (TransformField == ETransformField::Location)
				{
					double MaxMagnitude = HALF_WORLD_MAX;
					if (UWorld* World = SceneComponent->GetWorld())
					{
						AWorldSettings* WorldSettings = World->GetWorldSettings(false);
						bool bEnableWorldBoundsChecks = WorldSettings ? WorldSettings->bEnableWorldBoundsChecks : true;
						if (!bEnableWorldBoundsChecks)
						{
							MaxMagnitude = UE_DOUBLE_HUGE_DISTANCE;
						}
					}

					NewComponentValue = FVector
					{
						FMath::Clamp(NewComponentValue.X, -MaxMagnitude, MaxMagnitude),
						FMath::Clamp(NewComponentValue.Y, -MaxMagnitude, MaxMagnitude),
						FMath::Clamp(NewComponentValue.Z, -MaxMagnitude, MaxMagnitude)
					};
				}
	...
	if (ModifiedObjects.Num())
	{
		for (UObject* Object : ModifiedObjects)
		{
			...
			if (SceneComponent)
			{
				...
				if (SceneComponent && !Object->IsTemplate())
				{
