...
void FSceneCaptureDetails::CustomizeDetails( IDetailLayoutBuilder& DetailLayout )
{
	...
	ShowFlagsToAllowForCaptures.Add(FEngineShowFlags::EShowFlag::SF_DeferredAtmospherePass);
	...
	for (int32 GroupIndex = 0; GroupIndex < SFG_Max; ++GroupIndex)
	{
		...
		if (ShowFlagsByGroup[GroupIndex].Num() >= 1)
		{
			...
			case SFG_Lumen:
				...
				break;
			case SFG_MegaLights:
				GroupName = LOCTEXT("MegaLightsFeaturesShowFlagsMenu", "MegaLights Show Flags");
				break;
			case SFG_Nanite:
