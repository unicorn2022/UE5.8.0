...
void FCustomPrimitiveDataCustomization::CustomizeChildren(TSharedRef<IPropertyHandle> PropertyHandle, IDetailChildrenBuilder& ChildBuilder, IPropertyTypeCustomizationUtils& CustomizationUtils)
{
	...
	for (uint8 PrimIdx = 0; PrimIdx < NumPrimitiveIndices; ++PrimIdx)
	{
		...
		if (VectorGroup == NULL && VectorParameterData.Contains(PrimIdx))
		{
			...
			for (const FCustomPrimitiveDataParameter& ParameterData : VectorParameterData[PrimIdx])
			{
...
void FCustomPrimitiveDataCustomization::CreateParameterRow(IDetailChildrenBuilder& ChildBuilder, uint8 PrimIdx, TSharedPtr<IPropertyHandle> ElementHandle, bool bDataEditable, IDetailGroup* VectorGroup, IPropertyTypeCustomizationUtils& CustomizationUtils)
{
	...
	{
		for (const FCustomPrimitiveDataParameter& ParameterData : ScalarParameterData[PrimIdx])
		{
			if (AddedParametersForThisRow.Contains(ParameterData.ExpressionID))
			{
				continue;
...
void FCustomPrimitiveDataCustomization::PopulateParameterData(UPrimitiveComponent* PrimitiveComponent, int32& MaxPrimitiveDataIndex)
{
	...
	PrimitiveComponent->GatherDefaultCustomPrimitiveDataParametersFromMaterials(Parameters, MaxPrimitiveDataIndex);
	...
	for (auto& [PrimIdx, Parameter] : Parameters)
	{
		UMaterial* Material = Parameter.Material.IsValid() ? Parameter.Material->GetMaterial() : nullptr;
		...
		{
			ScalarParameterData.FindOrAdd(PrimIdx).Add(MoveTemp(Parameter));
		}
		else if (Parameter.Type == EMaterialParameterType::Vector)
		{
			// Add each element individually, so that we can overlap vector parameter names. Can move the last one.
			VectorParameterData.FindOrAdd(PrimIdx).Emplace_GetRef(Parameter).IndexOffset = 0;
			VectorParameterData.FindOrAdd(PrimIdx + 1).Emplace_GetRef(Parameter).IndexOffset = 1;
			VectorParameterData.FindOrAdd(PrimIdx + 2).Emplace_GetRef(Parameter).IndexOffset = 2;
			VectorParameterData.FindOrAdd(PrimIdx + 3).Emplace_GetRef(MoveTemp(Parameter)).IndexOffset = 3;
		}
	}
}
...
void FCustomPrimitiveDataCustomization::OnRemovedPrimitiveData(uint8 PrimIdx)
{
	...
	if (GetNumElements(NumElements) == FPropertyAccess::Success && PrimIdx < NumElements)
	{
		...
		for (uint32 i = NumElements; i-- > static_cast<uint32>(PrimIdx); )
		{
