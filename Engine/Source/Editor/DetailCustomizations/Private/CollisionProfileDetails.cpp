...
constexpr ECollisionChannel MinCustomCollisionChannel = ECC_GameTraceChannel1;
constexpr ECollisionChannel MaxCustomCollisionChannel = ECC_GameTraceChannel50;
constexpr uint32 MaxNumCustomCollisionChannels = MaxCustomCollisionChannel - MinCustomCollisionChannel + 1;
constexpr uint32 MaxNumCollisionChannels = ECC_OverlapAll_Deprecated;
static_assert(MaxNumCollisionChannels == 64);
...
void SChannelEditDialog::Construct(const FArguments& InArgs)
{
	...
	check(ChannelSetup.Channel >= MinCustomCollisionChannel && ChannelSetup.Channel <= MaxCustomCollisionChannel);
...
{
	if(ValidIndex >= 0 && ValidIndex < MaxNumCollisionChannels)
	{
	...
	[
		...
		[
			...
			[
				...
				.Text(LOCTEXT("ObjectChannel_Menu_Description", "You can have up to 50 custom channels including object and trace channels. This is the list of object types for your project. If you delete an object type that is being used by the game, any uses of that type will revert to WorldStatic."))
			]
	...
	[
		...
		[
			...
			[
				...
				.Text(LOCTEXT("TraceChannel_Menu_Description", "You can have up to 50 custom channels including object and trace channels. This is the list of trace channels for your project. If you delete a trace channel that is being used by the game, the behavior of the trace is undefined."))
			]
...
void FCollisionProfileDetails::RefreshChannelList(bool bTraceType)
{
	if(bTraceType)
	{
		...
		for(auto Iter = CollisionProfile->DefaultChannelResponses.CreateIterator(); Iter; ++Iter)
		{
			...
			if(Iter->Channel >= MinCustomCollisionChannel && Iter->bTraceType)
			{
...
{
	if(CollisionProfile->DefaultChannelResponses.Num() < MaxNumCustomCollisionChannels)
	{
		// this is very inefficient
		for(int32 ChannelIndex = MinCustomCollisionChannel; ChannelIndex < MinCustomCollisionChannel + MaxNumCustomCollisionChannels; ++ChannelIndex)
		{
...
{
	return (CollisionProfile && CollisionProfile->DefaultChannelResponses.Num() < MaxNumCustomCollisionChannels);
}
...
FReply FCollisionProfileDetails::OnNewChannel(bool bTraceType)
{
	...
	if (ensure(NewChannel >= MinCustomCollisionChannel && NewChannel <= MaxCustomCollisionChannel))
	{
