...
void FCameraFilmbackSettingsCustomization::CustomizeChildren(TSharedRef<IPropertyHandle> StructPropertyHandle, class IDetailChildrenBuilder& ChildBuilder, IPropertyTypeCustomizationUtils& StructCustomizationUtils)
{
	...
	SensorHorizontalOffsetHandle = PropertyHandles.FindChecked(GET_MEMBER_NAME_CHECKED(FCameraFilmbackSettings, SensorHorizontalOffset));
	SensorVerticalOffsetHandle = PropertyHandles.FindChecked(GET_MEMBER_NAME_CHECKED(FCameraFilmbackSettings, SensorVerticalOffset));
...
{
	bool bEnabled = false;
	if (SensorHeightHandle.IsValid() && 
			SensorWidthHandle.IsValid() &&
			SensorHorizontalOffsetHandle.IsValid() &&
			SensorVerticalOffsetHandle.IsValid())
	{
		bEnabled = (
				SensorHeightHandle->IsEditable() &&
				SensorWidthHandle->IsEditable() &&
				SensorHorizontalOffsetHandle->IsEditable() &&
				SensorVerticalOffsetHandle->IsEditable() &&
	...
	if (SelectInfo != ESelectInfo::Direct)
	{
		...
		for (int32 PresetIdx = 0; PresetIdx < NumPresets; ++PresetIdx)
		{
			...
			if (P.Name == NewPresetName)
			{
				...
				ensure(SensorHorizontalOffsetHandle->SetValue(P.FilmbackSettings.SensorHorizontalOffset, EPropertyValueSetFlags::InteractiveChange | EPropertyValueSetFlags::NotTransactable) == FPropertyAccess::Result::Success);
				ensure(SensorVerticalOffsetHandle->SetValue(P.FilmbackSettings.SensorVerticalOffset, EPropertyValueSetFlags::InteractiveChange | EPropertyValueSetFlags::NotTransactable) == FPropertyAccess::Result::Success);
...
TSharedPtr<FString> FCameraFilmbackSettingsCustomization::GetPresetString() const
{
	...
	float CurSensorHorizontalOffset;
	SensorHorizontalOffsetHandle->GetValue(CurSensorHorizontalOffset);

	float CurSensorVerticalOffset;
	SensorVerticalOffsetHandle->GetValue(CurSensorVerticalOffset);
	...
	{
		FNamedFilmbackPreset const& P = Presets[PresetIdx];
		if ((P.FilmbackSettings.SensorWidth == CurSensorWidth) && 
				(P.FilmbackSettings.SensorHeight == CurSensorHeight) &&
				(P.FilmbackSettings.SensorHorizontalOffset == CurSensorHorizontalOffset) &&
				(P.FilmbackSettings.SensorVerticalOffset == CurSensorVerticalOffset))
		{
			// this is the one
