...
float FSlateFontInfoStructCustomization::ConvertFontSizeFromNativeToDisplay(float NativeFontSize)
{
	...
	if (UISettings)
	{
		DisplaySize = UISettings->ConvertFontSizeFromNativeToDisplay(NativeFontSize);
	}
	return DisplaySize;
}
...
float FSlateFontInfoStructCustomization::ConvertFontSizeFromDisplayToNative(float DisplayFontSize)
{
	...
	if (UISettings)
	{
		NativeSize = UISettings->ConvertFontSizeFromDisplayToNative(DisplayFontSize);
	}
	return NativeSize;
}
...
void FSlateFontInfoStructCustomization::OnFontChanged(const FAssetData& InAssetData)
{
	...
	if(!FontAsset)
	{
		...
		{
			UE_LOGF(LogSlate, Warning, "FSlateFontInfo property '%ls' on object '%ls' was set to use a null UFont. Slate will be forced to use the fallback font path which may be slower.", *PropertyPath, *OuterObject->GetPathName());
		}
	}
}
