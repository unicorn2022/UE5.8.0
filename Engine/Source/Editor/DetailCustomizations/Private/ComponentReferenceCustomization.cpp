...
AActor* FComponentReferenceCustomization::GetFirstOuterActor() const
{
	...
	for (UObject* Obj : ObjectList)
	{
		while (Obj)
		{
			...
			{
				if (Component->HasAnyFlags(RF_ArchetypeObject))
				{
					if (UClass* OuterClass = Cast<UClass>(Component->GetOuter()))
					{
						return Cast<AActor>(OuterClass->GetDefaultObject());
					}
				}
...
bool FComponentReferenceCustomization::IsComponentReferenceValid(const FComponentReference& Value) const
{
	...
	AActor* CachedActor = CachedFilterActor.Get();
...
void FComponentReferenceCustomization::UpdateCachedValues(bool bResetValueIfInvalid)
{
	...
	CachedFilterActor = CachedFirstOuterActor;
	UWorld* PickerWorld = nullptr;
	RedirectActorToBPPreviewActor(CachedFilterActor, PickerWorld);
	...
	CachedComponent = TmpComponentReference.GetComponent(CachedFilterActor.Get());
...
UActorComponent* FComponentReferenceCustomization::GetCachedComponent() const
{
	UActorComponent* Component = CachedComponent.Get();
	if (Component == nullptr)
	{
		const_cast<FComponentReferenceCustomization*>(this)->UpdateCachedValues(false);
	}
...
{
	if (UActorComponent* Component = GetCachedComponent())
	{
...
{
	if (const UActorComponent* ActorComponent = GetCachedComponent())
	{
...
FText FComponentReferenceCustomization::OnGetComponentName() const
{
	if (CachedPropertyAccess == FPropertyAccess::Success)
	{
		if (UActorComponent* ActorComponent = GetCachedComponent())
		{
...
bool FComponentReferenceCustomization::RedirectActorToBPPreviewActor(TWeakObjectPtr<AActor>& InOutActor, UWorld*& OutWorld) const
{
	bool bShowTransient = false;
	if (AActor* Actor = InOutActor.Get())
	{
		if (Actor->HasAnyFlags(RF_ArchetypeObject))
		{
			if (UBlueprint* Blueprint = Cast<UBlueprint>(Actor->GetClass()->ClassGeneratedBy))
			{
				...
				for (TSharedRef<IBlueprintEditor>& EditorInterface : BlueprintEditorModule.GetBlueprintEditors())
				{
					TSharedRef<FBlueprintEditor> Editor = StaticCastSharedRef<FBlueprintEditor>(EditorInterface);
					if (Editor->GetBlueprintObj() == Blueprint)
					{
						if(AActor* PreviewActor = Editor->GetPreviewActor())
						{
							InOutActor = PreviewActor;
							OutWorld = PreviewActor->GetWorld();
							bShowTransient = true;
						}
						break;
					}
...
TSharedRef<SWidget> FComponentReferenceCustomization::OnGetMenuContent()
{
	...
	// If the actor we are looking at is a BP, use its BP editor preview world, if any
	CachedFilterActor = CachedFirstOuterActor;
	UWorld* PickerWorld = nullptr;
	bool bShowTransient = RedirectActorToBPPreviewActor(CachedFilterActor, PickerWorld);
...
{
	return bAllowAnyActor || Actor == CachedFilterActor.Get();
}
...
bool FComponentReferenceCustomization::IsFilteredComponent(const UActorComponent* const Component) const
{
	...
	const AActor* FilterActor = CachedFilterActor.Get();

	return Component->GetOwner()
		&& (bAllowAnyActor || Component->GetOwner() == FilterActor)
		&& (!bAllowAnyActor || (FilterActor && Component->GetOwner()->GetLevel() == FilterActor->GetLevel()))
...
void FComponentReferenceCustomization::OnComponentSelected(const UActorComponent* InComponent)
{
	...
	FComponentReference ComponentReference = FComponentEditorUtils::MakeComponentReference(CachedFilterActor.Get(), InComponent);
