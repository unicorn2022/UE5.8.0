	...
	if (!bInSettingsClass)
	{
		HeaderRow.
			...
			[
				...
				.IsEnabled(this, &FCameraLensSettingsCustomization::IsPresetEnabled)
				.ContentPadding(2.f)
				.Content()
				[
					SNew(STextBlock)
...
bool FCameraLensSettingsCustomization::IsPresetEnabled() const
{
	bool bEnabled = false;
	if (MinFocalLengthHandle.IsValid() &&
			MaxFocalLengthHandle.IsValid() &&
			MinFStopHandle.IsValid() &&
			MaxFStopHandle.IsValid() &&
			MinFocusDistanceHandle.IsValid() &&
			SqueezeFactorHandle.IsValid() &&
			DiaphragmBladeCountHandle.IsValid())
	{
		bEnabled = (MinFocalLengthHandle->IsEditable() &&
				MaxFocalLengthHandle->IsEditable() &&
				MinFStopHandle->IsEditable() &&
				MaxFStopHandle->IsEditable() &&
				MinFocusDistanceHandle->IsEditable() &&
				SqueezeFactorHandle->IsEditable() &&
				DiaphragmBladeCountHandle->IsEditable() &&
