...
struct FBlendSpacePreviewArgs
{
	...
	// Allows the current preview position to be programmatically driven
	TAttribute<FVector> PreviewFilteredPosition;
...
public:
	...
	virtual TSharedRef<FWorkflowTabFactory> CreateCurveViewerTabFactory(const TSharedRef<class FWorkflowCentricApplication>& InHostingApp, const TSharedPtr<class IEditableSkeleton>& InEditableSkeleton, const TSharedRef<class IPersonaPreviewScene>& InPreviewScene, FOnObjectsSelected InOnObjectsSelected, bool bInReadOnlyCurveViewer = true) const;
	...
	/** Create a tab factory for previewing morph targets */
...
public:
	// Register a new blend provider in the editor picker for FBlendProfileInterfaceWrapper
