...
, bCurveRemoved(false)
{
	...
	if (Type == ERawCurveTrackTypes::RCT_Transform)
	{
		...
		const UEnum* TransformChannelEnum = StaticEnum<ETransformCurveChannel>();
		const UEnum* VectorChannelEnum = StaticEnum<EVectorCurveChannel>();
		check(TransformChannelEnum && VectorChannelEnum);

		const FString ChannelString = TransformChannelEnum->GetAuthoredNameStringByValue((int64)CurveId.Channel);
		const FString AxisString = VectorChannelEnum->GetAuthoredNameStringByValue((int64)CurveId.Axis);
		const FString ChannelAxisName = ChannelString + TEXT(".") + AxisString;
		const FString LongName = FString::Printf(TEXT("%s.%s"), *CurveName.ToString(), *ChannelAxisName);

		SetShortDisplayName(FText::FromString(AxisString));
		SetLongDisplayName(FText::FromString(LongName));
		SetIntentionName(ChannelAxisName);
		SetLongIntentionName(LongName);
		SetChannelName(FName(*ChannelAxisName));
	}
	else
	{
		const FText DisplayText = FText::FromName(CurveName);
		SetShortDisplayName(DisplayText);
		SetLongDisplayName(DisplayText);
		SetIntentionName(CurveName.ToString());
		SetLongIntentionName(CurveName.ToString());
		SetChannelName(CurveName);
	}
...
void SAnimSequenceCurveEditor::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	IAnimSequenceCurveEditor::Tick(AllottedGeometry, InCurrentTime, InDeltaTime);
	
	for (const FAnimationCurveIdentifier& CurveToRemove : CurvesToRemove)
	{
		const int32 ChannelIndices = CurveToRemove.CurveType == ERawCurveTrackTypes::RCT_Transform ? 9 : 1;
		for (int32 ChannelIndex = 0; ChannelIndex < ChannelIndices; ++ChannelIndex)
		{
			RemoveCurve(CurveToRemove.CurveName, CurveToRemove.CurveType, ChannelIndex);
		}
	}
	CurvesToRemove.Reset();
}
...
void SAnimSequenceCurveEditor::OnModelHasChanged(const EAnimDataModelNotifyType& NotifyType, IAnimationDataModel* Model, const FAnimDataModelNotifPayload& Payload)
{
	...
	case EAnimDataModelNotifyType::CurveRenamed:
		{
			const FCurvePayload& TypedPayload = Payload.GetPayload<FCurvePayload>();
			CurvesToRemove.Add(TypedPayload.Identifier);
			break;
		}			
	case EAnimDataModelNotifyType::CurveFlagsChanged:
		{
