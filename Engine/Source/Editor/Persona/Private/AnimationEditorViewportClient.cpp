...
void FAnimationViewportClient::SetCameraFollowMode(EAnimationViewportCameraFollowMode InCameraFollowMode, FName InBoneName)
{
	...
	// When switching out of bone as camera we want to reset our coordinate system to something sensible
	if (InCameraFollowMode != EAnimationViewportCameraFollowMode::BoneAsCamera && CameraFollowMode == EAnimationViewportCameraFollowMode::BoneAsCamera)
	{
	...
	if(bCanFollow && InCameraFollowMode != EAnimationViewportCameraFollowMode::None)
	{
		...
		bCameraLock = true;
		bUsingOrbitCamera = InCameraFollowMode != EAnimationViewportCameraFollowMode::BoneAsCamera;
		bDisableInput = InCameraFollowMode == EAnimationViewportCameraFollowMode::BoneAsCamera;
		...
		if (PreviewMeshComponent != nullptr)
		{
			...
			case EAnimationViewportCameraFollowMode::Bone:
				...
				break;
				case EAnimationViewportCameraFollowMode::BoneAsCamera:
				{
					SetViewLocation(PreviewMeshComponent->GetBoneLocation(CameraFollowBoneName));
					SetViewRotation(PreviewMeshComponent->GetBoneQuaternion(CameraFollowBoneName).Rotator());
				}
				break;
			}
		
			OrbitRotation = FQuat::Identity;
			SetLookAtLocation(LookAtLocation, bUsingOrbitCamera);
...
EAxis::Type FAnimationViewportClient::GetBoneAsCameraAxis(EDirection Direction) const
{
	if (Direction == EDirection::Forward)
	{
		return BoneAsCameraForward;
	}
	if (Direction == EDirection::Right)
	{
		return BoneAsCameraRight;
	}
	return BoneAsCameraUp;
}

UE::Math::TVector<double> FAnimationViewportClient::GetBoneAsCameraPositionOffset() const
{
	return BoneAsCameraPositionOffset;
}

FRotator FAnimationViewportClient::GetBoneAsCameraRotationOffset() const
{
	return BoneAsCameraRotationOffset;
}

void FAnimationViewportClient::SetBoneAsCameraAxis(EDirection Direction, EAxis::Type Axis)
{
	if (Direction == EDirection::Forward)
	{
		BoneAsCameraForward = Axis;
	}
	else if (Direction == EDirection::Right)
	{
		BoneAsCameraRight = Axis;
	}
	else
	{
		BoneAsCameraUp = Axis;
...
void FAnimationViewportClient::SetBoneAsCameraPositionOffset(UE::Math::TVector<double> Offset)
{
	BoneAsCameraPositionOffset = Offset;
}

void FAnimationViewportClient::SetBoneAsCameraRotationOffset(const FRotator& Offset)
{
	BoneAsCameraRotationOffset = Offset;
}
			...
			case EAnimationViewportCameraFollowMode::BoneAsCamera:
				{
					...
					if (BoneIndex != INDEX_NONE)
					{
						FTransform BoneTransform = PreviewMeshComponent->GetBoneTransform(CameraFollowBoneName);

						FViewportCameraTransform& ViewTransform = GetViewTransform();

						const FVector Forward = BoneTransform.GetUnitAxis(BoneAsCameraForward);
						const FVector Right = BoneTransform.GetUnitAxis(BoneAsCameraRight);
						const FVector Up = BoneTransform.GetUnitAxis(BoneAsCameraUp);
						const FVector Origin = BoneTransform.GetTranslation() + BoneAsCameraPositionOffset;
						FTransform NewViewTransform = FTransform(Forward, Right, Up, Origin);
						NewViewTransform.SetRotation(BoneAsCameraRotationOffset.Quaternion() * NewViewTransform.Rotator().Quaternion());

						ViewTransform.SetLocation(NewViewTransform.GetLocation());
						ViewTransform.SetRotation(NewViewTransform.GetRotation().Rotator());
					}
				}
				break;
			}
...
FText FAnimationViewportClient::GetDisplayInfo(bool bDisplayAllInfo) const
{
	...
	for (int32 Index = 0; Index < ClothingSimulationInstances.Num(); ++Index)
	{
		...
		{
			// Cloth stats
			if (ClothingSimulationInstances.Num() > 1)
			{
				TextValue = ConcatenateLine(TextValue, FText::Format(LOCTEXT("SimulationInstance", "--- Simulation #{0} ---"), Index));
			}
...
void FAnimationViewportClient::ReceivedFocus(FViewport* InViewport)
{
...
void FAnimationViewportClient::LostFocus(FViewport* InViewport)
{
	ClearAudioFocus();
...
void FAnimationViewportClient::PerspectiveCameraMoved()
{
...
void FAnimationViewportClient::UpdateAudioListener() const
{
