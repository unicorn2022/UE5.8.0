	...
	, bAdditionalMeshesSelectable(true)
{
	...
	// Create a new audio device for this scene
	if (FAudioDeviceManager* AudioDeviceManager = GEngine->GetAudioDeviceManager())
	{
		...
		const FAudioDeviceHandle AudioDevice = AudioDeviceManager->RequestAudioDevice(DeviceParams);
...
{
	if (NewPreviewMesh != nullptr && GetEditableSkeleton().IsValid() && GetEditableSkeleton()->IsSkeletonValid() && 
		!GetEditableSkeleton()->GetSkeleton().IsCompatibleMesh(NewPreviewMesh))
	{
...
{
	auto ValidateSelfOverlay = [InOverlayOrSubAnimBlueprint, InAnimBlueprint]()
	{
		if (InOverlayOrSubAnimBlueprint == InAnimBlueprint)
		{
			UE_LOGFMT(LogAnimation, Warning, "Anim BP '{AnimBP}' is using itself as a preview overlay, which would cause crashes or infinite recursion. Skipping and applying non-overlaid anim BP instead", InAnimBlueprint->GetName());
			return false;
		}
		return true;
	};

	if (InOverlayOrSubAnimBlueprint && ValidateSelfOverlay())
	{
...
void FAnimationEditorPreviewScene::RefreshAdditionalMeshes(bool bAllowOverrideBaseMesh)
{
	...
	// add new components
	UDataAsset* PreviewSceneAdditionalMeshes = 
		GetEditableSkeleton().IsValid() && GetEditableSkeleton()->IsSkeletonValid() 
		? GetEditableSkeleton()->GetSkeleton().GetAdditionalPreviewSkeletalMeshes() : nullptr;
...
void FAnimationEditorPreviewScene::AddPreviewAttachedObjects()
{
	...
	if(GetEditableSkeleton().IsValid() && GetEditableSkeleton()->IsSkeletonValid())
	{
		const USkeleton& Skeleton = GetEditableSkeleton()->GetSkeleton();
...
void FAnimationEditorPreviewScene::SetPreviewAnimationAsset(UAnimationAsset* AnimAsset, bool bEnablePreview /*= true*/)
{
	if (SkeletalMeshComponent)
	{
		const USkeleton* Skeleton = GetEditableSkeleton().IsValid() && GetEditableSkeleton()->IsSkeletonValid() 
			? &GetEditableSkeleton()->GetSkeleton() : nullptr;
...
{
	const int32 BoneIndex = GetEditableSkeleton().IsValid() && GetEditableSkeleton()->IsSkeletonValid() 
		? GetEditableSkeleton()->GetSkeleton().GetReferenceSkeleton().FindBoneIndex(BoneName) : INDEX_NONE;
