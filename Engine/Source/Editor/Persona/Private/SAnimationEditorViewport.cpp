...
void SAnimationEditorViewport::Construct(const FArguments& InArgs, const FAnimationEditorViewportRequiredArgs& InRequiredArgs)
{
	...
	PreviewFeatureLevelChangedHandle = Editor->OnPreviewShaderPlatformChanged().AddLambda(
		[PreviewScenePtrWeak = PreviewScenePtr](EShaderPlatform NewShaderPlatform)
		{
...
{
	UEditorEngine* Editor = (UEditorEngine*)GEngine;
	Editor->OnPreviewShaderPlatformChanged().Remove(PreviewFeatureLevelChangedHandle);
}
...
void SAnimationEditorViewportTabBody::BindCommands()
{
	...
	CommandList.MapAction(
		ViewportShowMenuCommands.ToggleShowBoneDrawAll,
		FExecuteAction::CreateSP(this, &SAnimationEditorViewportTabBody::ToggleShowAllBones),
		FCanExecuteAction(),
		FIsActionChecked::CreateSP(this, &SAnimationEditorViewportTabBody::IsBoneDrawModeSet, (int32)EBoneDrawMode::All));
...
void SAnimationEditorViewportTabBody::ToggleShowAllBones()
{
	if (const TSharedPtr<FAnimationViewportClient>& AnimViewportClient = GetAnimationViewportClient())
	{
		const EBoneDrawMode::Type DrawMode = IsBoneDrawModeSet(static_cast<int32>(EBoneDrawMode::All)) ? 
			EBoneDrawMode::None : 
			EBoneDrawMode::All;

		AnimViewportClient->SetBoneDrawMode(DrawMode);
	}
}
...
EAxis::Type SAnimationEditorViewportTabBody::GetBoneAsCameraAxis(FAnimationViewportClient::EDirection Direction) const
{
	if (const TSharedPtr<FAnimationViewportClient>& AnimViewportClient = GetAnimationViewportClient())
	{
		return AnimViewportClient->GetBoneAsCameraAxis(Direction);
	}

	return EAxis::Type::None;
}

UE::Math::TVector<double> SAnimationEditorViewportTabBody::GetBoneAsCameraPositionOffset() const
{
	if (const TSharedPtr<FAnimationViewportClient>& AnimViewportClient = GetAnimationViewportClient())
	{
		return AnimViewportClient->GetBoneAsCameraPositionOffset();
	}

	return UE::Math::TVector<double>::Zero();
}

FRotator SAnimationEditorViewportTabBody::GetBoneAsCameraRotationOffset() const
{
	if (const TSharedPtr<FAnimationViewportClient>& AnimViewportClient = GetAnimationViewportClient())
	{
		return AnimViewportClient->GetBoneAsCameraRotationOffset();
	}

	return FRotator();
}

void SAnimationEditorViewportTabBody::SetBoneAsCameraAxis(FAnimationViewportClient::EDirection Direction, EAxis::Type Axis)
{
	if (const TSharedPtr<FAnimationViewportClient>& AnimViewportClient = GetAnimationViewportClient())
	{
		return AnimViewportClient->SetBoneAsCameraAxis(Direction, Axis);
	}
}

void SAnimationEditorViewportTabBody::SetBoneAsCameraPositionOffset(UE::Math::TVector<double> Offset)
{
	if (const TSharedPtr<FAnimationViewportClient>& AnimViewportClient = GetAnimationViewportClient())
	{
		AnimViewportClient->SetBoneAsCameraPositionOffset(Offset);
	}
}

void SAnimationEditorViewportTabBody::SetBoneAsCameraRotationOffset(const FRotator& Offset)
{
	if (const TSharedPtr<FAnimationViewportClient>& AnimViewportClient = GetAnimationViewportClient())
	{
		AnimViewportClient->SetBoneAsCameraRotationOffset(Offset);
	}
}
