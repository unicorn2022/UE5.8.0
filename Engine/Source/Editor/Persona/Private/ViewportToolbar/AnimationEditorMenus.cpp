...
{
TSharedRef<SWidget> MakeBoneAsCameraWidget(const TWeakPtr<SAnimationEditorViewportTabBody>& InViewport, const TWeakPtr<SComboButton>& InWeakComboButton)
{
			...
			SAssignNew(BoneTreeMenu, SBoneTreeMenu)
				...
				.OnBoneSelectionChanged_Lambda(
					[ViewportWeak = Viewport.ToWeakPtr()](FName InBoneName)
					{
						...
						Viewport->SetCameraFollowMode(EAnimationViewportCameraFollowMode::BoneAsCamera, InBoneName);
						...
						if (const TSharedPtr<IPinnedCommandList>& PinnedCommands = Viewport->GetPinnedCommands())
						{
							...
							PinnedCommands->AddCustomWidget(TEXT("BoneAsCameraWidget"));
						}
					})
...
TSharedRef<SWidget> SetupBoneAsCameraAxisRadioButton(TSharedPtr<SAnimationEditorViewportTabBody>& ViewportTab,
	const FName InName,
	const TAttribute<FText>& InLabel,
	const TAttribute<FText>& InToolTip,
	FAnimationViewportClient::EDirection Direction,
	EAxis::Type Axis)
{
	auto IsCheckedLambda = [ViewportWeak = ViewportTab.ToWeakPtr(), Direction, Axis]() //IsChecked
		{
			if (TSharedPtr<SAnimationEditorViewportTabBody> Viewport = ViewportWeak.Pin())
			{
				const bool bIsSameAxis = Viewport->GetBoneAsCameraAxis(Direction) == Axis;
				return bIsSameAxis ? ECheckBoxState::Checked : ECheckBoxState::Unchecked;
			}

			return ECheckBoxState::Unchecked;
		};

	auto OnStateChangedLambda = [ViewportWeak = ViewportTab.ToWeakPtr(), Direction, Axis](ECheckBoxState State) //Exec
		{
			if (TSharedPtr<SAnimationEditorViewportTabBody> Viewport = ViewportWeak.Pin())
			{
				Viewport->SetBoneAsCameraAxis(Direction, Axis);
			}
		};

	const ISlateStyle* StyleSet = FSlateStyleRegistry::FindSlateStyle("CoreStyle");

	TSharedRef<SWidget> Button =
		SNew(SCheckBox)
		.Visibility(EVisibility::Visible)
		.Style(StyleSet, "Menu.RadioButton")
		.IsChecked_Lambda(IsCheckedLambda)
		.OnCheckStateChanged_Lambda(OnStateChangedLambda);

	return Button;
}

void SetupBoneAsCameraForward(FToolMenuSection& Section, TSharedPtr<SAnimationEditorViewportTabBody>& ViewportTab)
{
	TSharedRef<SWidget> XRadioButton = SetupBoneAsCameraAxisRadioButton(ViewportTab,
		"CameraForwardX",
		LOCTEXT("BoneAsCameraForwardSectionX_Label", "X"),
		LOCTEXT("BoneAsCameraForwardSectionX_ToolTip", "Select X bone axis to use as camera forward"),
		FAnimationViewportClient::EDirection::Forward,
		EAxis::Type::X);

	TSharedRef<SWidget> YRadioButton = SetupBoneAsCameraAxisRadioButton(ViewportTab,
		"CameraForwardY",
		LOCTEXT("BoneAsCameraForwardSectionY_Label", "Y"),
		LOCTEXT("BoneAsCameraForwardSectionY_ToolTip", "Select Y bone axis to use as camera forward"),
		FAnimationViewportClient::EDirection::Forward,
		EAxis::Type::Y);

	TSharedRef<SWidget> ZRadioButton = SetupBoneAsCameraAxisRadioButton(ViewportTab,
		"CameraForwardZ",
		LOCTEXT("BoneAsCameraForwardSectionZ_Label", "Z"),
		LOCTEXT("BoneAsCameraForwardSectionZ_ToolTip", "Select Z bone axis to use as camera forward"),
		FAnimationViewportClient::EDirection::Forward,
		EAxis::Type::Z);

	TSharedRef<SWidget> CheckboxRow = SNew(SHorizontalBox)
		+ SHorizontalBox::Slot()
		...
		.HAlign(HAlign_Left)
		.AutoWidth()
		.Padding(FMargin(10, 0, 50, 0))
		[
			SNew(STextBlock)
				.Text(LOCTEXT("CameraForwardRadioButtonTextX", "X"))
		]
		+ SHorizontalBox::Slot()
		.VAlign(VAlign_Center)
		.HAlign(HAlign_Left)
		...
		[
			SNew(STextBlock)
				.Text(LOCTEXT("CameraForwardRadioButtonTextY", "Y"))
		]
		+ SHorizontalBox::Slot()
		.VAlign(VAlign_Center)
		.HAlign(HAlign_Left)
		...
		[
			SNew(STextBlock)
				.Text(LOCTEXT("CameraForwardRadioButtonTextZ", "Z"))
		];

	Section.AddEntry(FToolMenuEntry::InitWidget("BoneAsCameraForwardAxisWidget", CheckboxRow, FText()));

}

void SetupBoneAsCameraRight(FToolMenuSection& Section, TSharedPtr<SAnimationEditorViewportTabBody>& ViewportTab)
{
	TSharedRef<SWidget> XRadioButton = SetupBoneAsCameraAxisRadioButton(ViewportTab,
		"CameraRightX",
		LOCTEXT("BoneAsCameraRightSectionX_Label", "X"),
		LOCTEXT("BoneAsCameraRightSectionX_ToolTip", "Select X bone axis to use as camera forward"),
		FAnimationViewportClient::EDirection::Right,
		EAxis::Type::X);

	TSharedRef<SWidget> YRadioButton = SetupBoneAsCameraAxisRadioButton(ViewportTab,
		"CameraRightY",
		LOCTEXT("BoneAsCameraRightSectionY_Label", "Y"),
		LOCTEXT("BoneAsCameraRightSectionY_ToolTip", "Select Y bone axis to use as camera forward"),
		FAnimationViewportClient::EDirection::Right,
		EAxis::Type::Y);

	TSharedRef<SWidget> ZRadioButton = SetupBoneAsCameraAxisRadioButton(ViewportTab,
		"CameraRightZ",
		LOCTEXT("BoneAsCameraRightSectionZ_Label", "Z"),
		LOCTEXT("BoneAsCameraRightSectionZ_ToolTip", "Select Z bone axis to use as camera forward"),
		FAnimationViewportClient::EDirection::Right,
		EAxis::Type::Z);
	...
	TSharedRef<SWidget> CheckboxRow = SNew(SHorizontalBox)
		...
		[
			SNew(STextBlock)
				.Text(LOCTEXT("CameraRightRadioButtonTextX", "X"))
		]
		+ SHorizontalBox::Slot()
		.VAlign(VAlign_Center)
		.HAlign(HAlign_Left)
		...
		[
			SNew(STextBlock)
				.Text(LOCTEXT("CameraRightRadioButtonTextY", "Y"))
		]
		+ SHorizontalBox::Slot()
		.VAlign(VAlign_Center)
		.HAlign(HAlign_Left)
		...
		[
			SNew(STextBlock)
				.Text(LOCTEXT("CameraRightRadioButtonTextZ", "Z"))
		];

	Section.AddEntry(FToolMenuEntry::InitWidget("BoneAsCameraRightAxisWidget", CheckboxRow, FText()));
}

void SetupBoneAsCameraUp(FToolMenuSection& Section, TSharedPtr<SAnimationEditorViewportTabBody>& ViewportTab)
{
	TSharedRef<SWidget> XRadioButton = SetupBoneAsCameraAxisRadioButton(ViewportTab,
		"CameraUpX",
		LOCTEXT("BoneAsCameraUpSectionX_Label", "X"),
		LOCTEXT("BoneAsCameraUpSectionX_ToolTip", "Select X bone axis to use as camera forward"),
		FAnimationViewportClient::EDirection::Up,
		EAxis::Type::X);

	TSharedRef<SWidget> YRadioButton = SetupBoneAsCameraAxisRadioButton(ViewportTab,
		"CameraUpY",
		LOCTEXT("BoneAsCameraUpSectionY_Label", "Y"),
		LOCTEXT("BoneAsCameraUpSectionY_ToolTip", "Select Y bone axis to use as camera forward"),
		FAnimationViewportClient::EDirection::Up,
		EAxis::Type::Y);

	TSharedRef<SWidget> ZRadioButton = SetupBoneAsCameraAxisRadioButton(ViewportTab,
		"CameraUpZ",
		LOCTEXT("BoneAsCameraUpSectionZ_Label", "Z"),
		LOCTEXT("BoneAsCameraUpSectionZ_ToolTip", "Select Z bone axis to use as camera forward"),
		FAnimationViewportClient::EDirection::Up,
		EAxis::Type::Z);
	...
	TSharedRef<SWidget> CheckboxRow = SNew(SHorizontalBox)
		...
		[
			SNew(STextBlock)
				.Text(LOCTEXT("CameraUpRadioButtonTextX", "X"))
		]
		+ SHorizontalBox::Slot()
		.VAlign(VAlign_Center)
		.HAlign(HAlign_Left)
		...
		[
			SNew(STextBlock)
				.Text(LOCTEXT("CameraUpRadioButtonTextY", "Y"))
		]
		+ SHorizontalBox::Slot()
		.VAlign(VAlign_Center)
		.HAlign(HAlign_Left)
		...
		[
			SNew(STextBlock)
				.Text(LOCTEXT("CameraUpRadioButtonTextZ", "Z"))
		];

	Section.AddEntry(FToolMenuEntry::InitWidget("BoneAsCameraUpAxisWidget", CheckboxRow, FText()));
}

void SetupBoneAsCameraPositionOffset(FToolMenuSection& Section, TSharedPtr<SAnimationEditorViewportTabBody>& ViewportTab)
{
	const TSharedRef<SWidget> CameraOffsets =
		SNew(SNumericVectorInputBox<double>)
		.IsEnabled(true)
		.Vector_Lambda([ViewportWeak = ViewportTab.ToWeakPtr()]()
			{
				if (TSharedPtr<SAnimationEditorViewportTabBody> Viewport = ViewportWeak.Pin())
				{
					return Viewport->GetBoneAsCameraPositionOffset();
				}

				return UE::Math::TVector<double>::Zero();
			}
		)
		.OnXChanged_Lambda(
			[ViewportWeak = ViewportTab.ToWeakPtr()](double Value)
			{
				if (TSharedPtr<SAnimationEditorViewportTabBody> Viewport = ViewportWeak.Pin())
				{
					UE::Math::TVector<double> Offset = Viewport->GetBoneAsCameraPositionOffset();
					Offset.SetComponentForAxis(EAxis::Type::X, Value);
					Viewport->SetBoneAsCameraPositionOffset(Offset);
				}
			}
		)
		.OnYChanged_Lambda(
			[ViewportWeak = ViewportTab.ToWeakPtr()](double Value)
			{
				...
				{
					UE::Math::TVector<double> Offset = Viewport->GetBoneAsCameraPositionOffset();
					Offset.SetComponentForAxis(EAxis::Type::Y, Value);
					Viewport->SetBoneAsCameraPositionOffset(Offset);
				}
			}
		)
		.OnZChanged_Lambda(
			[ViewportWeak = ViewportTab.ToWeakPtr()](double Value)
			{
				...
				{
					UE::Math::TVector<double> Offset = Viewport->GetBoneAsCameraPositionOffset();
					Offset.SetComponentForAxis(EAxis::Type::Z, Value);
					Viewport->SetBoneAsCameraPositionOffset(Offset);
				}
			}
		)
	...
	Section.AddEntry(FToolMenuEntry::InitWidget("BoneAsCameraPositionOffsetWidget", CameraOffsets, FText()));
}

void SetupBoneAsCameraRotationOffset(FToolMenuSection& Section, TSharedPtr<SAnimationEditorViewportTabBody>& ViewportTab)
{
	const TSharedRef<SWidget> CameraOffsets =
		SNew(SNumericVectorInputBox<double>)
		.IsEnabled(true)
		.Vector_Lambda([ViewportWeak = ViewportTab.ToWeakPtr()]()
			{
				if (TSharedPtr<SAnimationEditorViewportTabBody> Viewport = ViewportWeak.Pin())
				{
					const FRotator Offset = Viewport->GetBoneAsCameraRotationOffset();
					return FVector(Offset.Roll, Offset.Pitch, Offset.Yaw);
				}
		...
		.OnXChanged_Lambda(
			[ViewportWeak = ViewportTab.ToWeakPtr()](double Value)
			{
				if (TSharedPtr<SAnimationEditorViewportTabBody> Viewport = ViewportWeak.Pin())
				{
					FRotator Offset = Viewport->GetBoneAsCameraRotationOffset();
					Offset.SetComponentForAxis(EAxis::Type::X, Value);
					Viewport->SetBoneAsCameraRotationOffset(Offset);
				}
			}
		)
		.OnYChanged_Lambda(
	...
	Section.AddEntry(FToolMenuEntry::InitWidget("BoneAsCameraRotationOffsetWidget", CameraOffsets, FText()));
}
	...
	Section.AddSubMenu(
		...
		FNewToolMenuDelegate::CreateLambda(
			[](UToolMenu* InSubMenu)
			{
				...
				{
					UE::Persona::ViewportToolbar::MakeShowBonesMenuSection(InSubMenu);
				}
			}
		)
	);
...
void UE::AnimationEditor::FillFollowModeSubmenu(UToolMenu* InMenu)
{
	...
	CameraFollowModeSection.AddSubMenu(
		"BoneAsCamera",
		LOCTEXT("BoneAsCamera_DisplayName", "Bone As Camera"),
		LOCTEXT("BoneAsCamera_ToolTip", "Select a bone for the camera to look out of"),
		FNewToolMenuDelegate::CreateLambda(
			[](UToolMenu* InSubmenu)
			{
				InSubmenu->AddDynamicSection(
					...
					FNewToolMenuDelegate::CreateLambda(
						[](UToolMenu* InSectionMenu)
						{
							...
							FToolMenuSection& BoneAsCameraModeSelection = InSectionMenu->FindOrAddSection(
								"BoneAsCameraSection",
								LOCTEXT("BoneAsCameraSection_Label", "Bone Selector")
							);

							BoneAsCameraModeSelection.AddEntry(FToolMenuEntry::InitWidget(
								"BoneAsCameraWidget", UE::AnimationEditor::Private::MakeBoneAsCameraWidget(ViewportTab, nullptr), FText()
							));

							FToolMenuSection& BoneAsCameraForwardAxisSection = InSectionMenu->FindOrAddSection(
								"BoneAsCameraForwardAxisSection",
								LOCTEXT("BoneAsCameraForwardSection_Label", "Forward Axis Selector")
							);

							UE::AnimationEditor::Private::SetupBoneAsCameraForward(BoneAsCameraForwardAxisSection, ViewportTab);

							FToolMenuSection& BoneAsCameraRightAxisSection = InSectionMenu->FindOrAddSection(
								"BoneAsCameraRightAxisSection",
								LOCTEXT("BoneAsCameraRightSection_Label", "Right Axis Selector")
							);

							UE::AnimationEditor::Private::SetupBoneAsCameraRight(BoneAsCameraRightAxisSection, ViewportTab);

							FToolMenuSection& BoneAsCameraUpAxisSection = InSectionMenu->FindOrAddSection(
								"BoneAsCameraAxisUpSection",
								LOCTEXT("BoneAsCameraUpSection_Label", "Up Axis Selector")
							);

							UE::AnimationEditor::Private::SetupBoneAsCameraUp(BoneAsCameraUpAxisSection, ViewportTab);

							FToolMenuSection& BoneAsCameraPositionOffsetSection = InSectionMenu->FindOrAddSection(
								"BoneAsCameraPositionOffset",
								LOCTEXT("BoneAsCameraPositoinOffsetSection_Label", "Position Offset Selector")
							);

							UE::AnimationEditor::Private::SetupBoneAsCameraPositionOffset(BoneAsCameraPositionOffsetSection, ViewportTab);

							FToolMenuSection& BoneAsCameraRotationOffsetSection = InSectionMenu->FindOrAddSection(
								"BoneAsCameraRotationOffset",
								LOCTEXT("BoneAsCameraRotationOffsetSection_Label", "Rotation Offset Selector")
							);

							UE::AnimationEditor::Private::SetupBoneAsCameraRotationOffset(BoneAsCameraRotationOffsetSection, ViewportTab);
						}
					)
				);
			}
		),
		false //ShouldCloseWindowAfterSelection
	);
}
