...
void FAnimModel_AnimSequenceBase::CopyToClipboard() const
{
	if (!SelectedTracks.IsEmpty())
	{
		...
		{
			UE_LOGF(LogAnimation, Warning, "Failed to get create valid clipboard object for Animation Timeline while attempting to copy data");
		}
	}
}
...
void FAnimModel_AnimSequenceBase::PasteDataFromClipboardToSelectedCurve()
{
	...
	{
		UE_LOGF(LogAnimation, Warning, "Failed to get valid clipboard for Animation Timeline while attempting to paste data");
	}
}
...
bool FAnimModel_AnimSequenceBase::IsNotifiesTimingElementDisplayEnabled(ENotifyTimingElementType::Type ElementType) const
{
	return NotifiesTimingElementNodeDisplayFlags[ElementType];
}

void FAnimModel_AnimSequenceBase::ToggleNotifiesTimingElementDisplayEnabled(ENotifyTimingElementType::Type ElementType)
{
