...
FReply FAnimTimeSliderController::OnMouseMove( SWidget& WidgetOwner, const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
	...
	if (bHandleRightMouseButton)
	{
		...
		else
		{
			...
			// Guard against zero pixels-per-input, happens when LocalViewRange = (0,0) in case of an empty montage/composite timeline
			InputDelta.X = ScaleInfo.PixelsPerInput > 0.f ? ScreenDelta.X/ScaleInfo.PixelsPerInput : 0.f;
...
FCursorReply FAnimTimeSliderController::OnCursorQuery( const SWidget& WidgetOwner, const FGeometry& MyGeometry, const FPointerEvent& CursorEvent ) const
{
