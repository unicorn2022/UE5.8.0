...
void FAnimTimelineClipboardUtilities::OverwriteOrAddCurvesFromClipboardContent(const UAnimTimelineClipboardContent* ClipboardContent, UAnimSequenceBase* InTargetSequence)
{
	...
	if (ClipboardContent->Curves.Num())
	{
	    ...
	    for (const TObjectPtr<UAnimCurveBaseCopyObject>& InCurveCopyObj : ClipboardContent->Curves)
	    {
		    ...
		    {
			    UE_LOGF(LogAnimation, Warning, "Attempting to paste curve { %ls } of unsupported type { %ls } by animation controller. ", *InCurveCopyObj->CurveName.ToString(), *UEnum::GetValueAsString(InCurveCopyObj->CurveType));
			    continue;
		    }
		    ...
		    {
			    // Do nothing and exit.
				UE_LOGF(LogAnimation, Warning, "Attempting to paste already existing curve: %ls", *InCurveCopyObj->CurveName.ToString());
		    }
		    else
		    {
