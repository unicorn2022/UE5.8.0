...
public:
	...
	void AppendSelectionToArray(TArray<INotifyNodeObjectInterface*>& Selection) const;
	...
	INotifyNodeObjectInterface* GetNodeObjectInterface(int32 NodeIndex) { return NotifyNodes[NodeIndex]->NodeObjectInterface; }
	/**
...
public:
	...
	virtual void OnDragged( const class FDragDropEvent& DragDropEvent ) override
	{
		...
		const float ClampedEnd = FMath::Clamp(SelectionBeginLocalPositionX + NodeGroupWidth, LocalTrackMin, LocalTrackMax);
		...
		else if(ClampedEnd < SelectionBeginLocalPositionX + NodeGroupWidth)
		{
			SelectionBeginLocalPositionX = ClampedEnd - NodeGroupWidth;
		}
	...
	float GetSnapPosition(const FTrackClampInfo& ClampInfo, float WidgetSpaceNotifyPosition, bool& bOutSnapped)
	{
		...
		bOutSnapped = OnSnapPosition.IsBound() && !FSlateApplication::Get().GetModifierKeys().IsControlDown() && ScaleInfo.PixelsPerInput > 0.0f && OnSnapPosition.Execute(SnapPosition, MaxSnapDist / ScaleInfo.PixelsPerInput, TArrayView<const FName>(), SnapTypeUnused);
	...
	float								NodeGroupWidth;			// Horizontal size of the selection in track-local pixels
		...
		)
	{
		...
		Operation->NodeGroupWidth = SelectionWidth;
		...
		for(int32 i = 0; i < NotifyTracks.Num(); ++i)
		{
			...
			// Use absolute size to keep TrackSnapTestPos in absolute space alongside TrackPos.
			Info.TrackPos = static_cast<int32>(CachedGeometry.AbsolutePosition.Y);
			Info.TrackSnapTestPos = Info.TrackPos + static_cast<int32>(CachedGeometry.GetAbsoluteSize().Y / 2);
	...
	Menu->AddDynamicSection(NAME_None, FNewToolMenuDelegate::CreateLambda([](UToolMenu* InMenu)
		{
							...
							.OnValueCommitted_Lambda([SourceTrack, MenuContext](float InValue, ETextCommit::Type InCommitType)
								{
									if (InCommitType == ETextCommit::OnEnter && SourceTrack->NotifyNodes.IsValidIndex(MenuContext->NodeIndex))
									{
										...
										INotifyNodeObjectInterface* LocalNodeObject = SourceTrack->NotifyNodes[MenuContext->NodeIndex]->NodeObjectInterface;
			...
			if (MenuContext->NodeObject)
			{
				...
				{
					if (MenuContext->NodeObject->GetType() == ENotifyNodeObjectTypes::SYNC_MARKER)
					{
			...
			else if (MenuContext->NodeObject && MenuContext->NodeObject->GetType() == ENotifyNodeObjectTypes::SYNC_MARKER)
			{
				FUIAction NewAction;
				NewAction.ExecuteAction.BindRaw(
					SourceTrack.Get(), &SAnimNotifyTrack::OnFindReferences, MenuContext->NodeObject->GetName(), true);
...
void SAnimNotifyTrack::Update()
{
	...
	if ( AnimNotifies.Num() > 0 )
	{
		...
		for (int32 NotifyIndex = 0; NotifyIndex < AnimNotifies.Num(); ++NotifyIndex)
		{
			...
			for(int32 Idx = 0 ; Idx < TimingElements.Num() ; ++Idx)
			{
				...
				if(Element->GetType() == ENotifyTimingElementType::NotifyStateBegin
				   || Element->GetType() == ENotifyTimingElementType::BranchPointNotify
				   || Element->GetType() == ENotifyTimingElementType::QueuedNotify)
				{
			...
			TAttribute<EVisibility> TimingNodeVisibility = TAttribute<EVisibility>::Create(TAttribute<EVisibility>::FGetter::CreateLambda(
				[this]()
				{
					...
					{
						return OnGetTimingNodeVisibility.Execute(ENotifyTimingElementType::QueuedNotify);
					}
			...
			if(Event->NotifyStateClass)
			{
				...
				{
					if(ElementToTest.IsValid() && ElementToTest->GetType() == ENotifyTimingElementType::NotifyStateEnd)
					{
			...
			// Check if there is a factory that wants to create a custom widget. 
			for (const FAnimNotifyNodeFactoryFunction& Factory : FAnimNotifyNodeFactory::GetRegisteredFactories())
			{
				AnimNotifyNode = Factory(Event);
				if (AnimNotifyNode.IsValid())
				{
					break;
				}
			}
			...
			AnimNotifyNode->SetSequence(Sequence);
			AnimNotifyNode->SetAnimNotify(Event);
			AnimNotifyNode->SetOnNotifyNodeDragStarted(FOnNotifyNodeDragStarted::CreateSP(this, &SAnimNotifyTrack::OnNotifyNodeDragStarted, NotifyIndex));
			AnimNotifyNode->SetOnNotifyStateHandleBeingDragged(OnNotifyStateHandleBeingDragged);
			AnimNotifyNode->SetOnUpdatePanel(OnUpdatePanel);
			AnimNotifyNode->SetPanTrackRequest(OnRequestTrackPan);
			AnimNotifyNode->SetViewInputMin(ViewInputMin);
			AnimNotifyNode->SetViewInputMax(ViewInputMax);
			AnimNotifyNode->SetOnSnapPosition(OnSnapPosition);
			AnimNotifyNode->SetOnTrackSelectionChanged(OnSelectionChanged);
			AnimNotifyNode->SetStateEndTimingNode(EndTimingNode);
	...
	for (FAnimSyncMarker* SyncMarker : AnimSyncMarkers)
	{
		...
		AnimSyncMarkerNode->SetSequence(Sequence);
		AnimSyncMarkerNode->SetAnimSyncMarker(SyncMarker);
		AnimSyncMarkerNode->SetOnNotifyNodeDragStarted(FOnNotifyNodeDragStarted::CreateSP(this, &SAnimNotifyTrack::OnNotifyNodeDragStarted, NodeIndex));
		AnimSyncMarkerNode->SetOnUpdatePanel(OnUpdatePanel);
		AnimSyncMarkerNode->SetPanTrackRequest(OnRequestTrackPan);
		AnimSyncMarkerNode->SetViewInputMin(ViewInputMin);
		AnimSyncMarkerNode->SetViewInputMax(ViewInputMax);
		AnimSyncMarkerNode->SetOnSnapPosition(OnSnapPosition);
		AnimSyncMarkerNode->SetOnTrackSelectionChanged(OnSelectionChanged);
		AnimSyncMarkerNode->SetStateEndTimingNode(EndTimingNode);
...
void SAnimNotifyTrack::HandleNodeDrop(TSharedPtr<SAnimNotifyNode> Node, float Offset)
{
	...
	if (Node->NodeObjectInterface->GetType() == ENotifyNodeObjectTypes::SYNC_MARKER)
	{
		UBlendSpace::UpdateBlendSpacesUsingAnimSequence(Sequence);
	}

	// Apply the local-space Offset after converting the absolute screen position to local.
	const float LocalX = static_cast<float>(GetCachedGeometry().AbsoluteToLocal(Node->GetScreenPosition()).X + Offset);
...
void SAnimNotifyTrack::AppendSelectionToArray(TArray<INotifyNodeObjectInterface*>& Selection) const
{
...
const EVisibility SAnimNotifyTrack::GetTimingNodeVisibility(TSharedPtr<SAnimNotifyNode> NotifyNode)
{
	if(OnGetTimingNodeVisibility.IsBound())
	{
		...
		{
			return Event->IsBranchingPoint() ? OnGetTimingNodeVisibility.Execute(ENotifyTimingElementType::BranchPointNotify) : OnGetTimingNodeVisibility.Execute(ENotifyTimingElementType::QueuedNotify);
		}
	}
...
FReply SAnimNotifyPanel::OnNotifyNodeDragStarted(TArray<TSharedPtr<SAnimNotifyNode>> NotifyNodes, TSharedRef<SWidget> Decorator, const FVector2D& ScreenCursorPos, const FVector2D& ScreenNodePosition, const bool bDragOnMarker)
{
	...
	// Absolute screen-space top-left of the selection — used downstream for cursor-relative drag tracking.
	FVector2D OverlayOrigin = Nodes[0]->GetScreenPosition();

	// Track-local horizontal width of the selection — used downstream for track-local clamping math.
	const float FirstNodeLocalX = PanelGeometry.AbsoluteToLocal(OverlayOrigin).X;
	float LocalMinX = FirstNodeLocalX;
	float LocalMaxX = FirstNodeLocalX + Nodes[0]->GetDurationSize();

	for (int32 Idx = 1; Idx < Nodes.Num(); ++Idx)
	{
		const TSharedPtr<SAnimNotifyNode>& Node = Nodes[Idx];
		const FVector2D NodeAbsPos = Node->GetScreenPosition();
		const float NodeLocalX = PanelGeometry.AbsoluteToLocal(NodeAbsPos).X;
		const float NodeDuration = Node->GetDurationSize();

		OverlayOrigin = FVector2D::Min(OverlayOrigin, NodeAbsPos);
		LocalMinX = FMath::Min(LocalMinX, NodeLocalX);
		LocalMaxX = FMath::Max(LocalMaxX, NodeLocalX + NodeDuration);
	}
	...
	return FReply::Handled().BeginDragDrop(FNotifyDragDropOp::New(Nodes, NodeDragDecorator, NotifyAnimTracks, Sequence, ScreenCursorPos, OverlayOrigin, LocalMaxX - LocalMinX, CurrentDragXPosition, PanRequestDelegate, OnSnapPosition, UpdateDelegate, OnNotifyNodesBeingDragged));
}
...
{
	TArray<INotifyNodeObjectInterface*> SelectedNodes;
	for (TSharedPtr<SAnimNotifyTrack> Track : NotifyAnimTracks)
	{
		Track->AppendSelectionToArray(SelectedNodes);
	}

	const bool bContainsSyncMarkers = SelectedNodes.ContainsByPredicate([](const INotifyNodeObjectInterface* Interface) { return Interface->GetType() == ENotifyNodeObjectTypes::NOTIFY; });
	...
	if (SelectedNodes.Num() > 0)
	{
		...
		for (INotifyNodeObjectInterface* NodeObject : SelectedNodes)
		{
...
void SAnimNotifyPanel::OnTrackSelectionChanged()
{
	if(!bIsSelecting)
	{
		...
		for(int32 TrackIdx = 0 ; TrackIdx < NotifyAnimTracks.Num() ; ++TrackIdx)
		{
			...
			{
				INotifyNodeObjectInterface* NodeObjectInterface = Track->GetNodeObjectInterface(Idx);
...
void SAnimNotifyPanel::CopySelectedNodesToClipboard() const
{
	...
	if (SelectedNodes.Num() > 0)
	{
		...
		// Sort by track
		SelectedNodes.Sort([](const INotifyNodeObjectInterface& A, const INotifyNodeObjectInterface& B)
		{
		...
		for (const INotifyNodeObjectInterface* NodeObject : SelectedNodes)
		{
...
void SAnimNotifyPanel::OnReplaceSelectedWithNotify(FString NewNotifyName, UClass* NewNotifyClass)
{
	...
	{
		INotifyNodeObjectInterface* NodeObject = SelectedNodes[index];
...
void SAnimNotifyPanel::OnReplaceSelectedWithSyncMarker(FString NewSyncMarkerName)
{
	if (UAnimSequence* Seq = Cast<UAnimSequence>(Sequence))
	{
		...
		for (INotifyNodeObjectInterface* NodeObject : SelectedNodes)
		{
			if (NodeObject->GetType() == ENotifyNodeObjectTypes::SYNC_MARKER)
			{
		...
		while(FParse::Line(&Buffer, CurrentLine))
		{
			...
			if (FParse::Value(*CurrentLine, TEXT("TrackIndex="), OriginalTrack) && FParse::Value(*CurrentLine, TEXT("AbsTime="), OrigTime) && FParse::Value(*CurrentLine, TEXT("NodeObjectType="), NodeObjectType))
			{
				...
				if (PasteIdx + TrackOffset < NotifyAnimTracks.Num())
				{
					...
					if (NodeObjectType == ENotifyNodeObjectTypes::NOTIFY)
					{
						TrackToUse->PasteSingleNotify(NotifyExportString, TimeToPaste);
					}
					else if (NodeObjectType == ENotifyNodeObjectTypes::SYNC_MARKER)
					{
