...
void FBlendSpaceDetails::CustomizeDetails(class IDetailLayoutBuilder& DetailBuilder)
{
	...
	if (BlendSpacePtr)
	{
		...
		//==============================================================================================================
		{
			...
			BlendSamplesArrayProperty->SetOnNumElementsChanged(FSimpleDelegate::CreateSP(this, &FBlendSpaceDetails::RefreshDetails));
			...
			for (uint32 SampleIndex = 0; SampleIndex < NumBlendSampleEntries; ++SampleIndex)
			{
				...
				TSharedPtr<IPropertyHandle> MirrorProperty = BlendSampleProperty->GetChildHandle(GET_MEMBER_NAME_CHECKED(FBlendSample, bMirror));
				...
				if(BlendSpace->IsAsset())
				{
					...
					IDetailPropertyRow& MirrorPropertyRow = Group.AddPropertyRow(MirrorProperty.ToSharedRef());
					TAttribute<bool> IsMirrorDataTableValid = TAttribute<bool>::CreateLambda([this]() { return BlendSpace->MirrorDataTable != nullptr; });
					MirrorPropertyRow.EditCondition(IsMirrorDataTableValid, nullptr);
				}
				else if(BlendSpaceNode.Get())
				{
					// Expose mirror option
					IDetailPropertyRow& MirrorPropertyRow = Group.AddPropertyRow(MirrorProperty.ToSharedRef());
		...
		/// Mirroring - only allow to set MDT that share the same skeleton.
		{
			TSharedPtr<IPropertyHandle> MDTPropertyHandle =  DetailBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(UBlendSpace, MirrorDataTable));

			if (!MDTPropertyHandle->IsValidHandle())
			{
				return;
			}
			
			IDetailCategoryBuilder& MirroringCategory = DetailBuilder.EditCategory(FName("Mirroring"));
			
			IDetailPropertyRow& MirroringPropertyRow = MirroringCategory.AddProperty(MDTPropertyHandle);
			FDetailWidgetRow& Row = MirroringPropertyRow.CustomWidget(true);
			
			TSharedPtr<SWidget> ValueWidget = SNew(SObjectPropertyEntryBox)
				.PropertyHandle(MDTPropertyHandle)
				.AllowedClass(UMirrorDataTable::StaticClass())
				.AllowClear(true)
				.OnShouldFilterAsset(FOnShouldFilterAsset::CreateLambda([Skeleton = BlendSpace->GetSkeleton(), bGraph = !BlendSpace->IsAsset() && BlendSpaceNode.IsValid(), BlendSpaceNode = BlendSpaceNode](const FAssetData& InAssetData)
				{
					if (!bGraph)
					{
						return Skeleton ? !Skeleton->IsCompatibleForEditor(InAssetData) : true;
					}
					else
					{
						if (UAnimBlueprint* AnimBlueprint = BlendSpaceNode->GetAnimBlueprint())
						{
							return AnimBlueprint->TargetSkeleton ? !AnimBlueprint->TargetSkeleton->IsCompatibleForEditor(InAssetData) : true;
						}
			...
			Row.NameContent()
			[
				MDTPropertyHandle->CreatePropertyNameWidget()
			];
...
void FBlendSpaceDetails::RefreshDetails() const
{
	if (Builder)
	{
		Builder->ForceRefreshDetails();
