...
class FRealCurveModel : public FCurveModel
{
  public:
	...
	explicit FRealCurveModel(FCurveTableEditorHandle InRealCurveHandle, UObject* InOwner);
	...
	virtual bool IsValid() const { return RealCurveHandle.IsValid(); }

  private:

  	const TWeakObjectPtr<> WeakOwner;
	const FCurveTableEditorHandle RealCurveHandle;
	
	FRealCurve* ResolveCurve(bool bEnsure = true) const
	{
		FRealCurve* Result = RealCurveHandle.GetCurve();
		ensure(!bEnsure || Result);
		return Result;
	}
};