...
FRealCurveModel::FRealCurveModel(FCurveTableEditorHandle InRealCurveHandle, UObject* InOwner)
	: WeakOwner(InOwner)
	, RealCurveHandle(MoveTemp(InRealCurveHandle))
{}
...
{
	if (FRealCurve* RealCurve = ResolveCurve())
	{
...
void FRealCurveModel::GetKeyDrawInfo(ECurvePointType PointType, const FKeyHandle InKeyHandle, FKeyDrawInfo& OutDrawInfo) const
{
	...
	else if (FRealCurve* RealCurve = ResolveCurve())
	{
