...
{
	UE_CLOGF(InClass == nullptr, LogBehaviorTreeEditor, Error, "Trying to handle new node of NULL class for Behavior Treee %ls "
...
void FBehaviorTreeEditor::CreateNewBlackboard()
{
	...
	if (NewAsset != nullptr)
	{
		...
		UE_CLOGF(BehaviorTree->BlackboardAsset != nullptr, LogBehaviorTreeEditor, Log, "Blackboard data asset %ls has been replaced by %ls", *GetNameSafe(BlackboardData), *GetNameSafe(NewAsset));
