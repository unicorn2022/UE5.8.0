...
bool FPluginWardenModule::RunAuthorizationPipeline(const FText& PluginFriendlyName, const FString& PluginItemId, const FString& PluginOfferId, const EEntitlementCacheLevelRequest CacheLevel)
{
	...
	while (!bAuthorizationCompleted)
	{
		...
		if (PreviousState != AuthorizationState)
		{
			switch ( AuthorizationState )
			{
				...
				{
					UE_LOGF(PluginWarden, Log, "Waiting for launcher ...");
					break;
				}
				case EPluginAuthorizationState::SigninRequired_Waiting:
				{
					UE_LOGF(PluginWarden, Log, "Sign-in required ...");
					break;
				}
				case EPluginAuthorizationState::Signin_Waiting:
				{
					UE_LOGF(PluginWarden, Log, "Signing in ...");
					break;
				}
				case EPluginAuthorizationState::AuthorizePlugin_Waiting:
				{
					UE_LOGF(PluginWarden, Log, "Waiting for authorization on plug-in ...");
					break;
				}
			}
		}
	...
	switch (AuthorizationState)
	{
		...
		case EPluginAuthorizationState::Unauthorized:
		{
			UE_LOGF(PluginWarden, Warning, "It looks like your Epic Games account doesn't have entitlements for %ls.", *PluginFriendlyName.ToString());
			break;
		}
		case EPluginAuthorizationState::Timeout:
		{
			UE_LOGF(PluginWarden, Error, "We were unable to verify your access to the plugin before timing out.");
			break;
		}
		case EPluginAuthorizationState::LauncherStartFailed:
		{
			UE_LOGF(PluginWarden, Error, "Cannot start the launcher. Please open the launcher and sign in manually.");
			break;
		}
		case EPluginAuthorizationState::SigninFailed:
		{
			UE_LOGF(PluginWarden, Error, "Sign-in failed. Please sign in manually through the launcher.");
			break;
		}
	}
