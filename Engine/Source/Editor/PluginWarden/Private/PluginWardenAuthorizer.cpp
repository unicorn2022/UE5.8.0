...
EPluginAuthorizationState FPluginWardenAuthorizer::UpdateAuthorizationState(float DeltaTime)
{
	...
	switch ( CurrentState )
	{
		...
		{
			UE_LOGF(PluginWarden, Log, "Waiting for launcher to run for the past %f seconds", WaitingTime);
		...
		{
			WaitingTime += DeltaTime;
			UE_LOGF(PluginWarden, Log, "Waiting for sign in for the past %f seconds", WaitingTime);
