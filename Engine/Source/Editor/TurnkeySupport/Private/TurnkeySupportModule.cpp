	...
	static bool ShouldBuildProject(const UProjectPackagingSettings* PackagingSettings, const ITargetPlatform* TargetPlatform)
	{
		...
		else if (PackagingSettings->Build == EProjectPackagingBuild::IfProjectHasCode)
		{
			...
			if (FApp::GetEngineIsPromotedBuild())
			{
				...
				{
					UE_LOGF(LogTurnkeySupport, Log, "Selected target: %ls", *Target->Name);
					BaseDir = ProjectDir;
				}
				else
				{
					...
					{
						UE_LOGF(LogTurnkeySupport, Log, "Project requires temp target (%ls)", *Reason.ToString());
						BaseDir = ProjectDir;
					}
					else
					{
						UE_LOGF(LogTurnkeySupport, Log, "Project does not require temp target");
		...
		{
			bBuild = !FApp::GetEngineIsPromotedBuild() && !FApp::IsEngineInstalled();
		}
	...
	static void CookOrPackage(FName IniPlatformName, const FString& BuildTargetName, EPrepareContentMode Mode)
	{
		...
		if (Mode == EPrepareContentMode::Package)
		{
			...
			if (PackagingSettings->ForDistribution)
			{
				...
				constexpr const TCHAR* ReleaseUrl = TEXT("https://www.unrealengine.com/release");
				RoyaltySubText = FText::Format(
					LOCTEXT("RoyaltyTrackingSubText", "Please ensure a <a id=\"a\" href=\"{0}\">Notification of Release</> is submitted for tracking any future royalties."),
					FText::FromString(ReleaseUrl));
			}
		...
		TFunction<void(FString, double)> ResultCallback;
		FTurnkeyEditorSupport::RunUAT(CommandLine, PlatformInfo->DisplayName, ContentPrepDescription, ContentPrepTaskName, ContentPrepIcon, &AnalyticsParamArray, ResultCallback, RoyaltySubText);
	}
		...
		if (bIsControlHeld)
		{
			...
			FTurnkeyEditorSupport::RunUAT(CommandLine, PlatformInfo->DisplayName, LOCTEXT("Turnkey_GettingCommandLine", "Copying Commandline"), LOCTEXT("Turnkey_CustomTaskNameCmdLine", "CustomCommandLine"), nullptr /* TaskIcon */, nullptr /* OptionalAnalyticsParamArray */,
				[BuildName=Build.Name, ReportFilename](FString, double)
				{
					...
					UE_LOGF(LogTurnkeySupport, Log, "Custom command '%ls' gave commandline: %ls", *BuildName, *ReportContents);
					...
					for (const FString& Line : Contents)
					{
						if (Line.StartsWith(TEXT("set ")))
						{
							...
							if (VariableLine.FindChar('=', Equals))
							{
								...
								UE_LOGF(LogTurnkeySupport, Log, "Turnkey setting env var: %ls = %ls", *Key, *Value);
							}
						}
					}
				}
...
static FSlateIcon MakePlatformSdkIconAttribute(FName IniPlatformName, TSharedPtr< ITargetDeviceProxy> DeviceProxy)
{
	...
	else
	{
			...
			Platform->SupportsFeature(ETargetPlatformFeatures::SupportsMultipleConnectionTypes))
		{
			...
			{
				UE_LOGF(LogTurnkeySupport, Warning, "Unknown ConnectionType:%ls", *DeviceProxy->GetConnectionType());
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FTurnkeySupportModule::UpdateSdkInfo);
	...
	UE_LOGF(LogTurnkeySupport, Log, "Running Turnkey SDK detection: '%ls'", *Commandline);
	...
	{
		UE_LOGF(LogTurnkeySupport, Log, "Completed SDK detection: ExitCode = %d", ExitCode);
		...
		AsyncTask(ENamedThreads::GameThread, [this, ReportFilename, ExitCode, TurnkeyProcess, OnExitHandle]()
		{
			...
			if (ExitCode == 0 || ExitCode == 10)
			{
				...
				if (FFileHelper::LoadFileToStringArray(Contents, *ReportFilename))
				{
					...
					{
 						UE_LOGF(LogTurnkeySupport, Log, "Turnkey Platform: %ls", *Line);
						...
						{
							UE_LOGF(LogTurnkeySupport, Log, "Received platform %ls from Turnkey, but the engine doesn't know about it. Skipping...", *PlatformName.ToString());
							continue;
						}
						...
						if (VanillaInfo != nullptr)
						{
							...
							if (ValidFlavors.Num() > 1)
							{
								...
								{
									UE_LOGF(LogTurnkeySupport, Log, "Platform %ls uses flavors but has no default flavor set. Setting to %ls", *PlatformName.ToString(), *(ValidFlavors[0]->Name.ToString()));
			...
			else
			{
				...
				UE_LOGF(LogTurnkeySupport, Warning, "Turnkey failed to run properly, full Turnkey output:\n%ls", *TurnkeyProcess->GetFullOutputWithoutDelegate());
			}
		...
		for (const auto& Pair : FDataDrivenPlatformInfoRegistry::GetAllPlatformInfos())
		{
			...
			if (!Pair.Value.bIsFakePlatform && !FDataDrivenPlatformInfoRegistry::IsPlatformHiddenFromUI(PlatformName))
			{
				...
				for (ITargetDevicePtr Device : Devices)
				{
					if (!Device.IsValid())
					{
						UE_LOGF(LogTurnkeySupport, Log, "Platform %ls returned an invlid device from GetAllDevices, which is not expected", *PlatformName.ToString());
						continue;
					}
...
void FTurnkeySupportModule::UpdateSdkInfoForDevices(TArray<FString> PlatformDeviceIds)
{
	...
	UE_LOGF(LogTurnkeySupport, Log, "Running Turnkey device detection: '%ls'", *Commandline);
	...
	{
		UE_LOGF(LogTurnkeySupport, Log, "Completed device detection: Code = %d", ExitCode);
			...
			if (!IsEngineExitRequested() && (ExitCode == 0 || ExitCode == 10))
			{
				...
				if (FFileHelper::LoadFileToStringArray(Contents, *ReportFilename))
				{
					for (FString& Line : Contents)
					{
						...
						{
							UE_LOGF(LogTurnkeySupport, Error, "Received DeviceId %ls from Turnkey, but the engine doesn't know about it.", *DDPIDeviceId);
							continue;
						}

						UE_LOGF(LogTurnkeySupport, Log, "Turnkey Device: %ls", *Line);
...
{
	UE_LOGF(LogTurnkeySupport, Display, "Launching on %ls", *DeviceId);
