...
bool UActorRecording::StartRecording(ULevelSequence* CurrentSequence, float CurrentSequenceTime, const FString& BaseAssetPath, const FString& SessionName)
{
	...
	if(Actor)
	{
		if (TargetAnimation != nullptr)
		{
			...
			if (EditorInstance)
			{
				UE_LOGF(LogAnimation, Log, "Closing '%ls' so we don't invalidate the open version when unloading it.", *TargetAnimation->GetName());
...
void UActorRecording::StartRecordingActorProperties(ULevelSequence* CurrentSequence, float CurrentSequenceTime)
{
	...
	if(CurrentSequence != nullptr)
	{
		...
		if(Guid.IsValid())
		{
			...
			{
				UE_LOGF(LogAnimation, Warning, "Transform recording is not enabled for '%ls'. Resulting animation may not match gameplay.", *Actor->GetActorLabel());
			}
