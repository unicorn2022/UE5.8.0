	...
	FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& Args)
	{
		...
		if (Args.Num() == 1)
		{
			...
			if (ParseResult.IsValid())
			{
				...
				UE_LOGF(LogEngine, Display, "Sequence recorder default sample frame rate set to: %ls", *(GetMutableDefault<USequenceRecorderSettings>()->DefaultAnimationSettings.SampleFrameRate.ToPrettyText().ToString()));
			}
			else
			{
				UE_LOGF(LogAnimation, Warning, "Invalid Frame Rate format provided for AnimRecorder.SampleRate:\n%ls", *ParseResult.GetError().Text.ToString());
			}
		}
		else
		{
			UE_LOGF(LogAnimation, Warning, "Missing Frame Rate argument for AnimRecorder.SampleRate");
		}
	}),
	ECVF_Default);
...
class FSequenceRecorderModule : public ISequenceRecorder, private FSelfRegisteringExec
{
	...
	static bool HandleRecordSequenceCommand(UWorld* InWorld, const TCHAR* InStr, FOutputDevice& Ar)
	{
		...
		if(FParse::Token(Str, Filter, UE_ARRAY_COUNT(Filter), 0))
		{
			...
			{
				UE_LOGF(LogAnimation, Warning, "Couldnt parse recording filter, using actor filters from settings.");
			}
		}
		...
		if(FilterType == EFilterType::Actor || FilterType == EFilterType::Class)
		{
			...
			if(FParse::Token(Str, Specifier, UE_ARRAY_COUNT(Specifier), 0))
			{
				...
				else
				{
					...
					{
						UE_LOGF(LogAnimation, Warning, "Couldnt parse class filter, aborting recording.");
					}
				}
			}
		}
	...
	virtual bool StartRecording(TArrayView<AActor* const> ActorsToRecord, const FString& PathToRecordTo, const FString& SequenceName) override
	{
		...
		else if (!FSequenceRecorder::Get().HasQueuedRecordings())
		{
			...
			UE_LOGF(LogAnimation, Display, "Couldn't find actor to record");
		}
