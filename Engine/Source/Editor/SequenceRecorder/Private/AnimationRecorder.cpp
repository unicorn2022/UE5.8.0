...
void FAnimationRecorder::StartRecord(USkeletalMeshComponent* Component, UAnimSequence* InAnimationObject)
{
	...
	if (bRemoveExcludedCurves)
	{
		TArray<FName> CurvesToRemove;
		for (const FFloatCurve& FloatCurve : AnimationObject->GetDataModel()->GetFloatCurves())
		{
			if (ShouldSkipName(FloatCurve.GetName()))
			{
				CurvesToRemove.Add(FloatCurve.GetName());
			}
		}
		for (const FName& CurveName : CurvesToRemove)
		{
			const FAnimationCurveIdentifier CurveId(CurveName, ERawCurveTrackTypes::RCT_Float);
			Controller.RemoveCurve(CurveId, bTransactRecording);
		}
	}
	...
	{
		if (AnimationObject->GetRetargetSourceAsset())
		{
			AnimationObject->SetRetargetSourceAsset(nullptr);
			AnimationObject->UpdateRetargetSourceAssetData();
		}
	}
	...
	if (IsValid(AnimationObject) && !AnimationObject->HasAnyFlags(RF_BeginDestroyed | RF_FinishDestroyed))
	{
		...
		if (RecordedCurves.Num() == NumKeys)
		{
			...
			for(FBlendedHeapCurve& Curve : RecordedCurves)
			{
				Curve.ForEachElement([this, &AllCurveNames](const UE::Anim::FCurveElement& InElement)
				{
					...
					bool bSkipCurve = (bMorphTarget && !bRecordMorphTargets) ||
			...
			for (FName CurveName : AllCurveNames)
			{
				...
				if (FloatCurveData)
				{
					//since default is true to check nonzero values we avoid the if state to test and just
					//always check
					bool bHasNonZeroValue = false;
					TArray<FRichCurveKey> Keys;
					for (int32 Index = 0; Index < WriteIndex; ++Index)
					{
						float Value = ValuesToRecord[Index];
						if (bHasNonZeroValue == false)
						{
							if (FMath::IsNearlyZero(Value, UE_KINDA_SMALL_NUMBER) == false)
							{
								bHasNonZeroValue = true;
							}
						}
						FRichCurveKey Key(TimesToRecord[Index], Value);
					...
					if(bSkipCurvesWithZeroValue == false || bHasNonZeroValue)
					{ 
			...
			UE_LOGF(LogAnimation, Log, "Animation Recorder set keys in %0.02f seconds", ElapsedTime);
		}
		...
		for (int32 TrackIndex = 0; TrackIndex < TrackNames.Num(); ++TrackIndex)
		{
			...
			else
			{
				...
				UE_LOGF(LogAnimation, Log, "Animation Recorder skipping bone: %ls", *BoneName.ToString());
			}
		}
...
void FAnimationRecorder::ProcessRecordedTimes(UAnimSequence* AnimSequence, USkeletalMeshComponent* SkeletalMeshComponent, const FTimecodeBoneMethod& TimecodeBoneMethod, const FProcessRecordedTimeParams& InParams)
{
	...
	if (TimecodeBoneMethod.BoneMode == ETimecodeBoneMode::UserDefined)
	{
		...
		{
			UE_LOGF(LogAnimation, Warning, "User defined bone name: %ls not found. Falling back to assigning timecodes to root bone", *TimecodeBoneMethod.BoneName.ToString());
		}
	}
...
void FAnimationRecorder::UpdateRecord(USkeletalMeshComponent* Component, float DeltaTime)
{
	...
	if (FramesRecorded < FramesToRecord)
	{
		...
		{
			UE_LOGF(LogAnimation, Log, "Current Num of Spaces %d don't match with the previous number %d so we are stopping recording", SpaceBases.Num(),PreviousSpacesBases.Num());
			StopRecord(true);
			return;
		}
		...
		UE_LOGF(LogAnimation, Log, "DeltaTime : %0.2f, Current Frame Count : %d, Frames To Record : %d, TimePassed : %0.2f", DeltaTime
		...
		while (FramesToRecord > FramesRecorded)
		{
			...
			UE_LOGF(LogAnimation, Log, "Current Frame Count : %d, BlendAlpha : %0.2f", FramesRecorded + 1, BlendAlpha);
	...
	{
		UE_LOGF(LogAnimation, Log, "Animation Recording exceeds the time limited (%f seconds). Stopping recording animation... ", RecordingRate.AsSeconds(MaxFrame));
		...
		for (int32 TrackIndex = 0; TrackIndex < TrackNames.Num(); ++TrackIndex)
		{
			...
			if (BoneTreeIndex != INDEX_NONE)
			{
				...
				if (bRecordTransforms)
				{
					...
					{
						UE_LOGF(LogAnimation, Warning, "Mismatch in animation frames. Trying to record frame: %d, but only: %d frame(s) exist. Changing skeleton while recording is not supported.", FrameToAdd, RawTrack.PosKeys.Num());
						return false;
					}
				}
				else
...
void FAnimationRecorder::RecordNotifies(USkeletalMeshComponent* Component, const TArray<FAnimNotifyEventReference>& AnimNotifies, float DeltaTime, float RecordTime)
{
	if (ensure(AnimationObject))
	{
		...
		UE_LOGF(LogAnimation, Log, "Added notifies : %d", AddedThisFrame);
	}
}
...
void FAnimRecorderInstance::InitInternal(USkeletalMeshComponent* InComponent, const FAnimationRecordingSettings& Settings, FAnimationSerializer *InAnimationSerializer)
{
	...
	Recorder->bSetRetargetSourceAsset = Settings.bSetRetargetSourceAsset;
	...
	Recorder->bSkipCurvesWithZeroValue = Settings.bSkipCurvesWithZeroValue;
	...
	Recorder->bRemoveExcludedCurves = Settings.bRemoveExcludedCurves;
...
bool FAnimRecorderInstance::BeginRecording()
{
	...
	{
		UE_LOGF(LogAnimation, Log, "Animation Recorder:  Begin Recording: SkelMeshComp not Valid, No Recording will occur.");
		return false;
	}
	if (Recorder.IsValid())
	{
	...
	UE_LOGF(LogAnimation, Log, "Animation Recorder: Begin Recording: Recorder not Valid, No Recording will occur.");
	return false;
}
...
void FAnimRecorderInstance::Update(float DeltaTime)
{
	if (SkelComp.IsValid() == false)
	{
		UE_LOGF(LogAnimation, Log, "Animation Recorder:  Update: SkelMeshComp not Valid, No Recording will occur.");
		return;
	}
	if (Recorder.IsValid())
	{
	...
	{
		UE_LOGF(LogAnimation, Log, "Animation Recorder:  Update: Recoder not Valid, No Recording will occur.");
	}
}
