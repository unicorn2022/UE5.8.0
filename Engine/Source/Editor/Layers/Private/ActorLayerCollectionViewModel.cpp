	...
	, WorldLayers(Editor.IsValid() ? Editor->GetEditorSubsystem<ULayersSubsystem>() : nullptr)
{
	...
	if (!Editor.IsValid())
	{
		UE_LOGF(LogActorLayerCollectionViewModel, Fatal, "This function requires Editor.IsValid() == true.");
	}
	else if (WorldLayers == nullptr)
	{
		UE_LOGF(LogActorLayerCollectionViewModel, Fatal, "This function requires Editor->GetEditorSubsystem<ULayersSubsystem>() to be already loaded rather than being a nullptr.");
	}
}
