...
void SSequencerSearchBox::Construct(const FArguments& InArgs, const TWeakPtr<ISequencerFilterBar>& InWeakFilterBar)
{
	...
	const TSharedPtr<ISequencerFilterBar> FilterBarPin = WeakFilterBar.Pin();
	check(FilterBarPin);
	FilterBarPin->OnTextFilterTextChanged().AddSP(this, &SSequencerSearchBox::OnFilterTextChanged);
	FilterBarPin->OnMuteFiltersChanged().AddSP(this, &SSequencerSearchBox::OnMuteStateChanged);
	
	OnFilterTextChanged();
	OnMuteStateChanged(FilterBarPin->AreFiltersMuted());
}
...
void SSequencerSearchBox::DefaultKeySuggestions(const FString& InKeyValue, TArray<FSequencerFilterSuggestion>& OutPossibleSuggestions)
{
	...
	for (const TSharedRef<ISequencerTextFilterExpressionContext>& Expression : FilterExpressions)
	{
		...
		for (const FName Key : Keys)
		{
			...
			NewSuggestion.Suggestion = Key.ToString(); 
			NewSuggestion.DisplayName = FText::FromName(Key);
	...
	// Add keys for all common filters to support suggestions for these text expressions
	FilterBar->GetCommonFilterKeySuggestions(OutPossibleSuggestions);
...
void SSequencerSearchBox::DefaultValueSuggestions(const FString& InKeyValue, TArray<FSequencerFilterSuggestion>& OutPossibleSuggestions)
{
	...
	{
		if (FoundExpression->GetValueType() == ESequencerTextFilterValueType::Boolean)
		{
			FSequencerFilterSuggestion::MakeBooleanSuggestions(OutPossibleSuggestions);
		}
		else
		{
			const TArray<FSequencerTextFilterKeyword> ValueKeywords = FoundExpression->GetValueKeywords();
			for (const FSequencerTextFilterKeyword& ValueKeyword : ValueKeywords)
			{
				FSequencerFilterSuggestion NewSuggestion;
				NewSuggestion.Suggestion = ValueKeyword.Keyword;
				NewSuggestion.DisplayName = FText::FromString(ValueKeyword.Keyword);
				NewSuggestion.Description = ValueKeyword.Description;
	...
	{
		// Add keys for all common filters to support suggestions for these text expressions
		FilterBar->GetCommonFilterValueSuggestions(InKeyValue, OutPossibleSuggestions);
	}
...
void SSequencerSearchBox::OnFilterTextChanged()
{
	const TSharedPtr<ISequencerFilterBar> FilterBar = WeakFilterBar.Pin();
	if (!FilterBar)
	{
	...
	// If the filter was changed externally, keep the widget in sync.
	// Skip SetText if the change was instantiated by this widget: when focused and calling SetText it can cause the cursor location to change.
	const FString FilterString = FilterBar->GetTextFilterString();
	const bool bHasChanged = GetText().ToString() != FilterString;
	if (bHasChanged)
	{
		const FText NewText = FText::FromString(FilterString);
		SetText(NewText);
	}
}

void SSequencerSearchBox::OnMuteStateChanged(bool bIsMuted)
{
	SetEnabled(!bIsMuted);
}
