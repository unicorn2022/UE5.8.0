...
void FSequencerTrackFilterCommands::RegisterCommands()
{
	...
	UI_COMMAND(ToggleLinkedFiltering, "Toggle Linked Filtering", "Toggles whether filtering should use the filter state shared with other UI or its own instance.", EUserInterfaceActionType::ToggleButton, FInputChord());
	UI_COMMAND(TogglePreserveFiltersOnUnlink, "Preserve Filters on Unlink", "Toggles whether when you unlink your filters, you want the filters in your linked filter bar to be copied into the unlinked filter bar.", EUserInterfaceActionType::ToggleButton, FInputChord())
	...
	UI_COMMAND(ToggleFilter_Selected, "Toggle Selected In Viewport Filter", "Toggle the filter for Selected In Viewport tracks", EUserInterfaceActionType::Button, FInputChord(EKeys::F10));
