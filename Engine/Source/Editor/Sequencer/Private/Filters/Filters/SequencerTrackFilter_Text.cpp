...
#include "Filters/TextExpressions/SequencerTextFilterExpression_Deactivated.h"
#include "Filters/TextExpressions/SequencerTextFilterExpression_EmptyBinding.h"
#include "Filters/TextExpressions/SequencerTextFilterExpression_Filters.h"
	...
	, TextFilterExpressionEvaluator(ETextFilterExpressionEvaluatorMode::Complex)
{
	...
	TextFilterExpressionContexts.Add(MakeShared<FSequencerTextFilterExpression_Filters>(InFilterInterface));
	...
	TextFilterExpressionContexts.Add(MakeShared<FSequencerTextFilterExpression_Deactivated>(InFilterInterface));
	...
	for (TObjectIterator<USequencerTrackFilterTextExpressionExtension> ExtensionIt(RF_NoFlags); ExtensionIt; ++ExtensionIt)
	{
			...
			&& !PotentialExtension->GetClass()->HasAnyClassFlags(CLASS_Deprecated | CLASS_Abstract))
		{
			...
			TextFilterExpressionContexts.Append(ExtendedTextExpressions);
		}
	}
...
void FSequencerTrackFilter_Text::VerifyNoDuplicateKeys() const
{
	TSet<FName> SeenKeys;

	for (const TSharedRef<FSequencerTextFilterExpressionContext>& TextExpression : TextFilterExpressionContexts)
	{
		const TSet<FName> TextExpressionKeys = TextExpression->GetKeys();
		for (const FName Key : TextExpressionKeys)
		{
			ensureMsgf(!SeenKeys.Contains(Key), TEXT("Sequencer text expression filter key already exists: %s"), *Key.ToString());
		}

		SeenKeys.Append(TextExpressionKeys);
	}
}
