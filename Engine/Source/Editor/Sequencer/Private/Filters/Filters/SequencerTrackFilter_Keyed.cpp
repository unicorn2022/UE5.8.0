...
bool FSequencerTrackFilter_Keyed::PassesFilter(FSequencerTrackFilterType InItem) const
{
	...
	else if (const TViewModelPtr<ISectionOwnerExtension> WeakSectionOwner = InItem.ImplicitCast())
	{
		for (const UMovieSceneSection* const Section : WeakSectionOwner->GetSections())
		{
