...
bool FSequencerTrackFilter_Unbound::PassesFilter(FSequencerTrackFilterType InItem) const
{
	...
	if (const TViewModelPtr<FObjectBindingModel> ObjectBindingModel = InItem->FindAncestorOfType<FObjectBindingModel>(true))
	{
		if (const TViewModelPtr<FSequenceModel> SequenceModel = InItem->FindAncestorOfType<FSequenceModel>())
		{
			...
			bool bAnyValidBinding = false;

			for (const TWeakObjectPtr<>& WeakObj : BoundObjects)
			{
				if (WeakObj.IsValid())
				{
					bAnyValidBinding = true;
					break;
				}
			}
