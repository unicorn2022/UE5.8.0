...
void FSequencerTrackFilterMenu::FillCustomTextFiltersMenu(UToolMenu* const InMenu)
{
	...
	CustomTextOptionsSection.AddMenuEntry(TEXT("TextExpressionHelp"),
		...
		FUIAction(FExecuteAction::CreateLambda([WeakFilterBar = FilterBar.ToWeakPtr()]
			{
				if (const TSharedPtr<FSequencerFilterBar> FilterBarPin = WeakFilterBar.Pin())
				{
					UE::Sequencer::OpenDialog_TextExpressionHelp(FilterBarPin.ToSharedRef());
				}
			}))
		);
		...
		FUIAction(FExecuteAction::CreateLambda([WeakFilterBar = FilterBar.ToWeakPtr()]
			{
				if (const TSharedPtr<FSequencerFilterBar> FilterBarPin = WeakFilterBar.Pin())
				{
					UE::Sequencer::OpenDialog_SaveCurrentFilterSetAsCustomTextFilter(FilterBarPin.ToSharedRef());
				}
			}))
		);
...
void FSequencerTrackFilterMenu::OpenNodeGroupsManager()
{
	...
	const FSequencer* Sequencer = static_cast<FSequencer*>(&FilterBar->GetSequencer());
