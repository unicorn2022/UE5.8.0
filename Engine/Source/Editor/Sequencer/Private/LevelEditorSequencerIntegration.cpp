...
{
static void ReRenderLevelViewports()
{
...
static void InvalidateLevelViewports(bool bInvalidateHitProxies)
{
	for (FEditorViewportClient* LevelVC : GEditor->GetLevelViewportClients())
	{
		if (LevelVC)
...
void FLevelEditorSequencerIntegration::OnSequencerEvaluated()
{
	...
	UE::Sequencer::ReRenderLevelViewports();
	
	// Summary: When Sequencer plays back, moving the mouse in the viewport used to cause a drop in FPS. See CL description.
	const bool bInvalidateHitProxies = !bDeferUpdates;
	UE::Sequencer::InvalidateLevelViewports(bInvalidateHitProxies);
