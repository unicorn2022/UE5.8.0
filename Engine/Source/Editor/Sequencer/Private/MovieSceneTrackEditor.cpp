...
TSharedPtr<SWidget> FMovieSceneTrackEditor::BuildOutlinerColumnWidget(const FBuildColumnWidgetParams& Params, const FName& ColumnName)
{
	...
	TViewModelPtr<ITrackRowExtension> TrackRowModel = Params.ViewModel.ImplicitCast();
	...
	if (ColumnName == FCommonOutlinerNames::Edit)
	{
		...
		EditWidgetParams.RowIndex = TrackRowModel ? TrackRowModel->GetRowIndex() : 0;
		if (Params.ViewModel->FindAncestorOfType<ITrackExtension>())
		{
			EditWidgetParams.TrackInsertRowIndex = TrackRowModel ? TrackRowModel->GetRowIndex() : 0;
		}
...
{
	const bool bHasSections = Track && Track->GetSupportedBlendTypes().Num() > 0;
	const bool bHasDecorations = SequencerHelpers::HasCompatibleDecorations(Track);

	if (bHasSections || bHasDecorations)
	{
		...
		auto SubMenuCallback = [=, this]() -> TSharedRef<SWidget>
		{
			...
			// Add section creation entries if track supports sections
			if (bHasSections)
			{
				FSequencerUtilities::PopulateMenu_CreateNewSection(MenuBuilder, RowIndex, Track, WeakSequencer);
			}

			// Add decoration entries if track supports decorations
			if (bHasDecorations)
			{
				SequencerHelpers::BuildDecorationMenu(MenuBuilder, Track, Track->FindObjectBindingGuid(), WeakSequencer);
			}
