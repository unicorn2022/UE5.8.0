	...
	: Super( ObjectInitializer )
{
	...
	bSynchronizeOutlinerSelectionToCurveEditor = true;
	...
	PlayheadScrubbingLimit = ESequencerPlayheadScrubbingLimit::SPSL_None;
	...
	SectionColorAlpha = 1.0f;
	bDrawHorizontalTrackSeparatorLines = false;
	...
	bAutoExpandOnCurveEditorSelection = true;
	bRestoreOriginalViewportOnCameraCutUnlock = true;
	TreeViewWidth = SSequencer::DefaultOutlinerWidth;
	bShowTickLines = true;
	...
	bFilterBarVisible = true;
	bNavigationToolVisible = false;
	bSelectionDetailsPoppedOut = false;
	bShowTransportControls = true;
	bTransportControlsPoppedOut = false;
	bRestoreTransportControlsPoppedOut = false;
...
bool USequencerSettings::GetShowBottomTimeSlider() const
{
	return bShowBottomTimeSlider;
}

void USequencerSettings::SetShowBottomTimeSlider(const bool bInShow)
{
	if (bShowBottomTimeSlider != bInShow)
	{
		bShowBottomTimeSlider = bInShow;
		SaveConfig();
	}
}
...
ESequencerPlayheadScrubbingLimit USequencerSettings::GetPlayheadScrubbingLimit() const
{
	return PlayheadScrubbingLimit;
}

void USequencerSettings::SetPlayheadScrubbingLimit(ESequencerPlayheadScrubbingLimit InPlayheadScrubbingLimit)
{
	if (PlayheadScrubbingLimit != InPlayheadScrubbingLimit)
	{
		PlayheadScrubbingLimit = InPlayheadScrubbingLimit;
		SaveConfig();
	}
}

TRange<FFrameNumber> USequencerSettings::GetPlayheadScrubbingRange(TWeakPtr<ISequencer> InSequencer) const
{
	...
	TRange<FFrameNumber> PlayheadScrubbingRange;
	if (PlayheadScrubbingLimit == ESequencerPlayheadScrubbingLimit::SPSL_PlaybackRange)
	{
		return PlayheadScrubbingRange = Sequencer->GetSubSequenceRange().Get(Sequencer->GetFocusedMovieSceneSequence()->GetMovieScene()->GetPlaybackRange());
	}
	else if (PlayheadScrubbingLimit == ESequencerPlayheadScrubbingLimit::SPSL_SelectionRange)
	{
		return PlayheadScrubbingRange = Sequencer->GetFocusedMovieSceneSequence()->GetMovieScene()->GetSelectionRange();
	}
	return TRange<FFrameNumber>::Empty();
}
...
{
	return bLinkFiltersWithCurveEditor_DEPRECATED;
}
...
{
	if (bLinkFiltersWithCurveEditor_DEPRECATED != bInLinkFiltersWithCurveEditor)
	{
		bLinkFiltersWithCurveEditor_DEPRECATED = bInLinkFiltersWithCurveEditor;
		SaveConfig();
	}
}
PRAGMA_ENABLE_DEPRECATION_WARNINGS
...
void USequencerSettings::SyncOutlinerSelectionToCurveEditor(bool bInValue)
{
	if (bSynchronizeOutlinerSelectionToCurveEditor != bInValue)
	{
		bSynchronizeOutlinerSelectionToCurveEditor = bInValue;
		SaveConfig();
	}
}
...
float USequencerSettings::GetSectionColorAlpha() const
{
	return SectionColorAlpha;
}

void USequencerSettings::SetSectionColorAlpha(const float InAlpha)
{
	if (SectionColorAlpha != InAlpha)
	{
		SectionColorAlpha = InAlpha;
		SaveConfig();
	}
}

bool USequencerSettings::ShouldDrawHorizontalTrackSeparatorLines() const
{
	return bDrawHorizontalTrackSeparatorLines;
}

void USequencerSettings::SetDrawHorizontalTrackSeparatorLines(const bool bInDraw)
{
	if (bDrawHorizontalTrackSeparatorLines != bInDraw)
	{
		bDrawHorizontalTrackSeparatorLines = bInDraw;
		SaveConfig();
	}
}
...
void USequencerSettings::SetAutoExpandCurveEditorOnSelection(bool bValue)
{
	if (bValue != bAutoExpandOnCurveEditorSelection)
	{
		bAutoExpandOnCurveEditorSelection = bValue;
		SaveConfig();
	}
}
...
FSequencerFilterAreaConfig& USequencerSettings::FindOrAddTrackFilterArea(const FName InIdentifier, bool bInSaveConfig)
{
	FSequencerFilterAreaConfig& FilterBarSettings = TrackFilterAreas.FindOrAdd(InIdentifier);
...
FSequencerFilterAreaConfig* USequencerSettings::FindTrackFilterArea(const FName InIdentifier)
{
	return TrackFilterAreas.Find(InIdentifier);
}

bool USequencerSettings::RemoveTrackFilterArea(const FName InIdentifier)
{
	const int32 RemovedCount = TrackFilterBars.Remove(InIdentifier) > 0;
	const bool bHasChanged = RemovedCount > 0;
	if (bHasChanged)
	{
		SaveConfig();
	}
...
{
	return LastFilterBarLayout_DEPRECATED;
}
PRAGMA_ENABLE_DEPRECATION_WARNINGS
...
{
	LastFilterBarLayout_DEPRECATED = InLayout;
	SaveConfig();
}
PRAGMA_ENABLE_DEPRECATION_WARNINGS
...
void USequencerSettings::SetLastFilterBarSizeCoefficient(const float bInSizeCoefficient, const bool bInSaveConfig)
{
...
void USequencerSettings::SetSelectionDetailsPoppedOut(const bool bInPoppedOut)
{
	if (bSelectionDetailsPoppedOut != bInPoppedOut)
	{
		bSelectionDetailsPoppedOut = bInPoppedOut;
		SaveConfig();
	}
}

bool USequencerSettings::ShouldShowTransportControls() const
{
	return bShowTransportControls;
}

void USequencerSettings::SetShowTransportControls(const bool bInShowTransportControls)
{
	if (bShowTransportControls != bInShowTransportControls)
	{
		bShowTransportControls = bInShowTransportControls;
		SaveConfig();
	}
}

bool USequencerSettings::AreTransportControlsPoppedOut() const
{
	return bTransportControlsPoppedOut;
}

void USequencerSettings::SetTransportControlsPoppedOut(const bool bInPoppedOut)
{
	if (bTransportControlsPoppedOut != bInPoppedOut)
	{
		bTransportControlsPoppedOut = bInPoppedOut;
		SaveConfig();
	}
}

bool USequencerSettings::ShouldRestoreTransportControlsPoppedOut() const
{
	return bRestoreTransportControlsPoppedOut;
}

void USequencerSettings::SetRestoreTransportControlsPoppedOut(const bool bInRestorePoppedOut)
{
	if (bRestoreTransportControlsPoppedOut != bInRestorePoppedOut)
	{
		bRestoreTransportControlsPoppedOut = bInRestorePoppedOut;
		SaveConfig();
	}
}

void USequencerSettings::SetIsSimpleView(const bool bInSimpleView)
{
	if (bSimpleView != bInSimpleView)
	{
		bSimpleView = bInSimpleView;
		SaveConfig();
	}
}
