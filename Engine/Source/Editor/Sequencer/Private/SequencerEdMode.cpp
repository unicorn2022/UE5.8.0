...
#include "ViewportInteractions/MoveSequencerTimeSliderViewportInteraction.h"
...
void FSequencerEdMode::Enter()
{
	...
	if (UModeManagerInteractiveToolsContext* Context = GetModeManager()->GetInteractiveToolsContext())
	{
		Context->OnBuildViewportInteractions().AddSPLambda(this, [this](UViewportInteractionsBehaviorSource* Source)
		{
			UMoveSequencerTimeSliderViewportInteraction* MoveInteraction = Source->AddInteraction<UMoveSequencerTimeSliderViewportInteraction>(true);
			MoveInteraction->EdMode = this;
		});
	}
...
void FSequencerEdMode::Exit()
{
	...
	if (UModeManagerInteractiveToolsContext* Context = GetModeManager()->GetInteractiveToolsContext())
	{
		Context->OnBuildViewportInteractions().RemoveAll(this);
	}
...
{
	UE::CurveEditor::InputState::SetCommandPressed(FSequencerCommands::Get().ScrubTimeViewport, Key, Event != IE_Released);

	if (Event != IE_Released && !ViewportClient->IsFlightCameraActive())
	{
...
bool FSequencerEdMode::StartTracking(FEditorViewportClient* InViewportClient, FViewport* InViewport)
{
	...
	{
		UE::CurveEditor::InputState::SetCommandPressed(FSequencerCommands::Get().ScrubTimeViewport, InViewport, true);
...
bool FSequencerEdMode::ProcessCapturedMouseMoves(FEditorViewportClient* InViewportClient, FViewport* InViewport, const TArrayView<FIntPoint>& CapturedMouseMoves)
{
	...
	auto SetNextPlayerStatus = [&ActiveSequencer](EMovieScenePlayerStatus::Type InNextPlayerStatus)
	{
		if (ActiveSequencer.IsValid())
		{
			if (InNextPlayerStatus == EMovieScenePlayerStatus::Stopped)
			{
			...
			{
				ActiveSequencer->SetPlaybackStatus(InNextPlayerStatus);
			}
		}
	};
	if (CapturedMouseMoves.Num() > 0)
	{
		if (ActiveSequencer.IsValid())
		{
			...
			{
				bIsTracking = false;
				SetNextPlayerStatus(NextPlayerStatus);
...
void FSequencerEdMode::DrawAudioTracks(FPrimitiveDrawInterface* PDI)
{
	...
	for (TWeakPtr<ISequencer> WeakSequencer : Sequencers)
	{
		...
		TSet<UMovieSceneAudioTrack*> SelectedAudioTracks;
		...
		for (TViewModelPtr<ITrackExtension> TrackModel : Sequencer->GetViewModel()->GetSelection()->Outliner.Filter<ITrackExtension>())
		{
			...
			SelectedAudioTracks.Add(AudioTrack);
		}
		
		for (TViewModelPtr<ITrackRowExtension> TrackRowModel : Sequencer->GetViewModel()->GetSelection()->Outliner.Filter<ITrackRowExtension>())
		{
			UMovieSceneAudioTrack* AudioTrack = Cast<UMovieSceneAudioTrack>(TrackRowModel->GetParentTrack());
		...
		for (UMovieSceneAudioTrack* AudioTrack : SelectedAudioTracks)
		{
					...
					for (TWeakObjectPtr<> WeakObject : AttachBindingID.ResolveBoundObjects(Sequencer->GetFocusedTemplateID(), *Sequencer))
					{
						...
						if (AttachActor)
						{
							...
							if (AttachComponent)
							{
								...
								float Scale = PDI->View->WorldToScreen(Location).W * (4.0f / PDI->View->UnscaledViewRect.Width() / PDI->View->ViewMatrices.GetViewToClip().M[0][0]);
