...
static void CreateKeyStructForSelection(TWeakPtr<ISequencer> InWeakSequencer, TSharedPtr<FStructOnScope>& OutKeyStruct, TWeakObjectPtr<UMovieSceneSection>& OutKeyStructSection, TWeakObjectPtr<UObject>& OutKeyStructOwningObject)
{
...
void FKeyContextMenu::PopulateMenu(FMenuBuilder& MenuBuilder, TSharedPtr<FExtender> MenuExtender)
{
	...
	TWeakObjectPtr<UObject> UnusedKeyStructOwningObject;
	CreateKeyStructForSelection(WeakSequencer, KeyStruct, KeyStructSection, UnusedKeyStructOwningObject);
	...
	if (KeyStruct.IsValid())
	{
		MenuBuilder.AddSubMenu(
			...
			FNewMenuDelegate::CreateLambda([Shared](FMenuBuilder& SubMenuBuilder){ Shared->AddPropertiesMenu(SubMenuBuilder); }),
			FUIAction(
				FExecuteAction(),
	...
	{
		UToolMenu* const KeyContextMenu = UToolMenus::Get()->RegisterMenu(TEXT("Sequencer.KeyContextMenu"), NAME_None, EMultiBoxType::Menu);
		KeyContextMenu->bSearchable = false;

		KeyContextMenu->AddDynamicSection(TEXT("Edit"), FNewToolMenuDelegate::CreateStatic([](UToolMenu* InMenu)
		{
			USequencerToolMenuContext* const ContextObject = InMenu->FindContext<USequencerToolMenuContext>();
			const TSharedPtr<ISequencer>     Sequencer     = ContextObject ? ContextObject->WeakSequencer.Pin() : nullptr;
			if (!Sequencer.IsValid())
			{
				return;
			}
			...
			{
				FToolMenuSection& Section = InMenu->AddSection(TEXT("SequencerKeyEdit"), LOCTEXT("EditMenu", "Edit"));
			...
			FToolMenuSection& KeysRemovalSection = InMenu->AddSection(TEXT("KeyRemoval")
				, FText::GetEmpty(), FToolMenuInsert(NAME_None, EToolMenuInsertType::Last));
			...
			KeysRemovalSection.AddMenuEntry(
				...
				FUIAction(
					FExecuteAction::CreateLambda([WeakSequencer = TWeakPtr<ISequencer>(Sequencer)]
					{
						if (const TSharedPtr<FSequencer> Sequencer = StaticCastSharedPtr<FSequencer>(WeakSequencer.Pin()))
						{
		...
		KeyContextMenu->AddDynamicSection(TEXT("SequencerKeys"), FNewToolMenuDelegate::CreateStatic([](UToolMenu* InMenu)
		{
			USequencerToolMenuContext* const ContextObject = InMenu->FindContext<USequencerToolMenuContext>();
			const TSharedPtr<ISequencer> Sequencer = ContextObject ? ContextObject->WeakSequencer.Pin() : nullptr;
			if (!Sequencer.IsValid())
			...
			FToolMenuSection& KeysSection = InMenu->FindOrAddSection(TEXT("SequencerKeys"), LOCTEXT("KeysMenu", "Keys"));

			KeysSection.AddMenuEntry(SequencerCommands.SetKeyTime
				, SequencerCommands.SetKeyTime->GetLabel()
				, SequencerCommands.SetKeyTime->GetDescription()
				, FSlateIcon(FAppStyle::GetAppStyleSetName(), TEXT("TimecodeProvider.TabIcon")));

			KeysSection.AddMenuEntry(SequencerCommands.Rekey
				, SequencerCommands.Rekey->GetLabel()
				, SequencerCommands.Rekey->GetDescription()
				, FSlateIcon(FAppStyle::GetAppStyleSetName(), TEXT("Sequencer.KeyDiamond")));

			KeysSection.AddMenuEntry(SequencerCommands.SnapToFrame
				, SequencerCommands.SnapToFrame->GetLabel()
				, SequencerCommands.SnapToFrame->GetDescription()
				, FSlateIcon(FAppStyle::GetAppStyleSetName(), TEXT("Icons.Snap")));

			// @TODO: Simple view will sync selection to curve editor and use its commands to perform the operations
			// on the selected keys, so we can't use this in Sequencer full view. We would need more abstraction so
			// Curve Editor and Sequencer can have their own versions.
			if (Sequencer->IsSimpleView())
			{
				if (const TSharedPtr<FCurveEditor> CurveEditor = ToolableTimeline::Utils::GetSequencerCurveEditor(*Sequencer))
				{
					if (const TSharedPtr<FUICommandList> CurveEditorCommandBindings = CurveEditor->GetCommands())
					{
						const FCurveEditorCommands& CurveEditorCommands = FCurveEditorCommands::Get();

						KeysSection.AddMenuEntryWithCommandList(CurveEditorCommands.FlattenTangents, CurveEditorCommandBindings);
						KeysSection.AddMenuEntryWithCommandList(CurveEditorCommands.StraightenTangents, CurveEditorCommandBindings);
						KeysSection.AddMenuEntryWithCommandList(CurveEditorCommands.SmartSnapKeys, CurveEditorCommandBindings);
					}
				}
			}
		}));
	...
	const TSharedRef<SWidget> Widget = UToolMenus::Get()->GenerateWidget(TEXT("Sequencer.KeyContextMenu"), MenuContext);
...
void FKeyContextMenu::AddPropertiesMenu(FMenuBuilder& MenuBuilder)
{
	...
	auto UpdateAndRetrieveEditData = [this]
	{
		...
		CreateKeyStructForSelection(WeakSequencer, EditData.KeyStruct, EditData.OwningSection, EditData.OwningObject);
		return EditData;
	};
...
void FSectionContextMenu::PopulateMenu(FMenuBuilder& MenuBuilder, TSharedPtr<FExtender> MenuExtender)
{
	...
	MenuBuilder.BeginSection("SequencerSections", LOCTEXT("SectionsMenu", "Sections"));
	{
		...
		MenuBuilder.AddMenuEntry(
			...
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateLambda([=] { return Shared->DeleteSection(); }),
				FCanExecuteAction::CreateLambda([=] { return Shared->CanDeleteSection(); })
			)
		);
...
void FSectionContextMenu::AutoSizeSection()
{
	...
	for (UMovieSceneSection* Section : Sequencer->GetViewModel()->GetSelection()->GetSelectedSections())
	{
		if (Section && Section->GetAutoSizeRange().IsSet())
		{
			...
			if (DefaultSectionLength.IsSet())
			{
				...
				// Notify the track that the section was resized so it can update any dependent state
	...
	EMovieSceneDataChangeType ChangeType = EnumHasAnyFlags(SectionMovedResult, EMovieSceneSectionMovedResult::SectionsChanged)
		? EMovieSceneDataChangeType::MovieSceneStructureItemAdded
		: EMovieSceneDataChangeType::TrackValueChanged;
	Sequencer->NotifyMovieSceneDataChanged(ChangeType);
}
...
bool FSectionContextMenu::CanDeleteSection() const
{
	using namespace UE::Sequencer;
	...
	// Check if any selected section is deletable
	for (UMovieSceneSection* Section : Sequencer->GetViewModel()->GetSelection()->GetSelectedSections())
	{
		if (!Section || Section->IsLocked())
		{
...
void FPasteContextMenu::Setup()
{
	...
	if (Args.DestinationNodes.Num())
	{
		...
		if (Args.Clipboard->GetKeyTrackGroups().Num() == 1)
		{
			...
			{
				TViewModelPtr<ISectionOwnerExtension> SectionOwner = Node.AsModel()->FindAncestorOfType<ISectionOwnerExtension>(true);
				if (!SectionOwner)
				{
					continue;
				}
		...
		for (const TViewModelPtr<IOutlinerExtension>& Node : Args.DestinationNodes)
		{
			...
			UMovieSceneSection* Section = MovieSceneHelpers::FindNearestSectionAtTime(SectionOwner->GetSections(), Args.PasteAtTime);
			TSharedPtr<FSectionModel> SectionModel = Sequencer->GetNodeTree()->GetSectionModel(Section);
			if (SectionModel)
			{
	...
	for (const TTuple<FName, TArray<TSharedPtr<FSectionModel>>>& Pair : SectionsByType)
	{
		...
		if (Pair.Value.Num() == 1)
		{
			TSharedPtr<FViewModel> Model = Pair.Value[0]->FindAncestorOfTypes({ ISectionOwnerExtension::ID, IOutlinerExtension::ID});
			if (ensure(Model))
			{
		...
		{
			FViewModelPtr Model = Section->FindAncestorOfTypes({ISectionOwnerExtension::ID, IOutlinerExtension::ID});
