...
struct FSequencerSectionPainterImpl : FSequencerSectionPainter
{
	...
	virtual int32 PaintSectionBackground(const FLinearColor& Tint) override
	{
		...
		if (SequencerSettings->ShouldShowPrePostRoll())
		{
		...
		const FLinearColor BlendedTint = BlendColor(Tint).CopyWithNewOpacity(SequencerSettings->GetSectionColorAlpha());
		...
		else
		{
			...
			// Draw contents background at base LayerId (not incremented) so child track lanes
			// that paint at the same layer can draw their content on top
			FSlateDrawElement::MakeBox(
				DrawElements,
				LayerId,
	...
	void DrawEmptySpace()
	{
		...
		// Remove invalid track lanes and lanes that have their own section widgets
		// (sections handle their own background painting). Lanes with only channel/key
		// area widgets still need background from this parent section.
		for (int32 Index = EmptyChildLanes.Num()-1; Index >= 0; --Index)
		{
			if (EmptyChildLanes[Index])
			{
				bool bHasOwnSection = false;
				for (const auto& Pair : EmptyChildLanes[Index]->GetAllWidgets())
				{
					if (TSharedPtr<FViewModel> Model = Pair.Key.Pin())
					{
						if (Model->IsA<FSectionModel>())
						{
							bHasOwnSection = true;
							break;
						}
		...
		if (EmptyChildLanes.IsEmpty())
		{
			return;
		}

		// Paint section contents background for remaining track lanes (e.g., decoration channels)
		// Use the same brush and color computation as the section contents area
		const ESlateDrawEffect DrawEffects = bParentEnabled ? ESlateDrawEffect::None : ESlateDrawEffect::DisabledEffect;
		static const FSlateBrush* ContentsBrush = FAppStyle::GetBrush("Sequencer.Section.Background_Contents");
		if (!ContentsBrush)
		{
			return;
		}

		// Compute the fill tint the same way PaintSectionBackground does for expanded section contents
		const USequencerSettings* const SequencerSettings = Sequencer.GetSequencerSettings();
		...
		UMovieSceneSection* SectionObject = SectionModel->GetSection();
		UMovieSceneTrack* OuterTrack = SectionObject ? SectionObject->GetTypedOuter<UMovieSceneTrack>() : nullptr;
		const FLinearColor TrackColor = OuterTrack ? OuterTrack->GetColorTint() : FLinearColor::White;
		const FLinearColor BlendedTint = BlendColor(TrackColor).CopyWithNewOpacity(SequencerSettings->GetSectionColorAlpha());
		FLinearColor FillTint = BlendedTint.LinearRGBToHSV();
		FillTint.G *= .5f;
		FillTint.B = FMath::Max(.03f, FillTint.B * .1f);
		const FLinearColor FinalFillColor = FillTint.HSVToLinearRGB();
...
EVisibility SSequencerSection::GetTopLevelChannelGroupVisibility() const
{
	...
	// Allow sections to opt out of showing the key area when collapsed
	if (SectionInterface && !SectionInterface->ShouldShowKeyAreaWhenCollapsed())
	{
		return EVisibility::Collapsed;
	}
...
int32 SSequencerSection::OnPaint( const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled ) const
{
	...
	const bool bSectionLocalDisabled = SectionObject->IsLocalEvalDisabled();
	const bool bEnabled = bParentEnabled && SectionObject->IsActive() && bConditionPasses && !(bTrackDisabled) && !bSectionLocalDisabled;
...
FReply SSequencerSection::OnMouseMove(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	if (CurveEditor::InputState::IsScrubTimeCommandPressed(FSequencerCommands::Get().ScrubTimeViewport))
	{
		if (TSharedPtr<FTrackAreaViewModel> TrackAreaViewModel = GetTrackAreaViewModel())
		{
			TrackAreaViewModel->SetHotspot(nullptr);
		}
		return FReply::Unhandled();
