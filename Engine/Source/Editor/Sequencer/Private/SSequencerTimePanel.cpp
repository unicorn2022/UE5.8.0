...
FReply SSequencerTimePanel::Apply()
{
	...
	if (MovieScene)
	{
		...
		// firing off the warning for every sequence, though we will print in the output log which ones were unlocked.
		{
			...
			{
				UE_LOGF(LogMovieScene, Log, "The following sequence was read-only but had its tick resolution adjusted automatically. No action is required. %ls", *ReadOnlyMovieScene->GetPathName());
			}
