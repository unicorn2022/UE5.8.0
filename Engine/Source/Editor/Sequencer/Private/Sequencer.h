...
namespace UE
{
	...
	namespace Sequencer
	{
		...
		class FSequencerEditorViewModelImpl;
		class FSequencerSelection;
		class FViewModel;
		class SSequencerWrapperWidget;
	...
	, private UE::MovieScene::FCameraCutViewTargetCacheCapability
{
	...
	friend class UE::Sequencer::ToolableTimeline::FToolableTimeline;

public:
	SEQUENCER_API static const FName SelectionDrawerId;
	SEQUENCER_API static const FName SelectionDetailsTabId;
...
public:
	...
	/** Select all that fall into the current selection range. */
	void SelectInSelectionRange(bool bSelectKeys, bool bSelectSections, bool bSelectMarkers);
	void SelectMarkersInRange(const TRange<FFrameNumber>& InRange);

	/** Select all keys, sections and markers forward from the current time */
	void SelectForward();

	/** Select all keys, sections and markers backward from the current time */
	void SelectBackward();
...
public:
	...
	TSharedRef<UE::Sequencer::FSequencerEditorViewModelImpl> GetViewModelImpl() const
	{
	...
	void TransformSelection(FFrameTime InDeltaTime, float InScale);

	/** Translate the selection by the time snap interval */
	void TranslateSelection(bool bTranslateLeft);
	...
	/** @return Decoration editors created for this sequencer */
	const TArray<TSharedPtr<ISequencerDecorationEditor>>& GetDecorationEditors() const
	{
		return DecorationEditors;
	}

	virtual ISequencerDecorationEditor* FindDecorationEditor(UClass* DecorationClass) const override;
	virtual TTuple<ISequencerDecorationEditor*, UObject*> FindDecorationOwner(
		const FMovieSceneChannelHandle& ChannelHandle,
		UMovieSceneSection* Section) const override;

private:
	/** Returns a shared no-op decoration editor used as the keying fallback for decorations without a registered editor. Lazily created on first use. */
	ISequencerDecorationEditor* GetOrCreateDefaultDecorationEditor() const;
...
public:
	...
	/** Zooms to the movie scene selection range, or falls back to time bounds if empty. */
	void ZoomToFitSelectionRange();
...
public:
	...
	FReply OnRecord();
	DLLEXPORT FReply OnPlayForward(bool bTogglePlay);
	DLLEXPORT FReply OnPlayBackward(bool bTogglePlay);
	...
	bool CanAddTransformKeysForSelectedObjects(const EMovieSceneTransformChannel InChannel = EMovieSceneTransformChannel::All) const;
	bool CanDeleteTransformKeysForSelectedObjects(const EMovieSceneTransformChannel InChannel = EMovieSceneTransformChannel::All) const;
	void OnAddTransformKeysForSelectedObjects(const EMovieSceneTransformChannel InChannel);
	void OnDeleteTransformKeysForSelectedObjects(const EMovieSceneTransformChannel InChannel);
	...
	void SetLocalTimeLooped(FFrameTime InTime, const FMovieSceneTransformBreadcrumbs& Breadcrumbs=FMovieSceneTransformBreadcrumbs(), bool bForwards=true);

	ESequencerLoopMode GetLoopMode() const;

	DLLEXPORT EPlaybackMode::Type GetPlaybackMode() const;
	...
	EPasteResult PasteDecorations(const FString& TextToImport, TArray<FNotificationInfo>& PasteErrors);
...
public:
	...
	TSharedRef<SSequencer> GetUnderlyingSequencerWidget() const { return SequencerWidget.ToSharedRef(); }
	...
	virtual UE::Sequencer::ISequencerWidget& GetSequencerWidgetInterface() const override;
	...
	virtual FMovieSceneSequenceTransform GetFocusedMovieSceneSequenceTransformUnwarped() const override;
...
public:
	...
	UE_DEPRECATED(5.8, "FOnChannelChanged OnChannelChanged() is now deprecated. Use FMovieSceneOnChannelChangedWithTime OnChannelChangedWithTime()")
	FOnChannelChanged& OnChannelChanged() { return OnChannelChangedDelegate_DEPRACRATED; }
	PRAGMA_ENABLE_DEPRECATION_WARNINGS

	virtual FMovieSceneOnChannelChangedWithTime& OnChannelChangedWithTime() override { return OnChannelChangedDelegate; }
	...
	virtual FOnPreNavigateToSequence& OnPreNavigateToSequence() override;
...
public:
	...
	DLLEXPORT virtual EMovieScenePlayerStatus::Type GetPlaybackStatus() const override;
...
private:
	...
	bool CanAddTransformKeysForSelectedObjectsInternal(const EMovieSceneTransformChannel InChannel = EMovieSceneTransformChannel::All) const;
	void AddTransformKeysForSelectedObjectsInternal(const EMovieSceneTransformChannel InChannel);
...
public:
	...
	TSharedRef<STemporarilyFocusedSpinBox<double>> MakePlayTimeDisplay();
	...
	/** @return True if the sequencer is in the simplified view */
	virtual bool IsSimpleView() const override;

	/** Enables or disables the Sequencer simplified view */
	virtual void EnableSimpleView(const bool bInEnable) override;

	/** Toggles the enabled state of the Sequencer simplified view */
	...
	/** Toggles the state of the Sequencer selection details between docked in the Sequencer tab and being popped out to its own tab window. */
	void ToggleSelectionDetailsPoppedOut();

	/** @return True if the Sequencer selection details can be popped out to its own tab window. */
	bool CanSelectionDetailsPopOut() const;

	/** @return True if the Sequencer selection details are currently popped out to its own tab window. */
	bool IsSelectionDetailsPoppedOut() const;

	/** Set the popped out state of the selection details in the Sequencer tab. */
	void SetSelectionDetailsPoppedOut(const bool bInPoppedOut);
	...
	/** @return Whether the sequencer selection is being updated from changes to the external selection. */
	bool IsUpdatingSequencerSelection() const { return bUpdatingSequencerSelection; }
...
private:
	...
	TSharedPtr<UE::Sequencer::SimpleView::FSimpleViewTimeline> GetSimpleViewTimeline() const;
	...
	/** Decoration editors owned by this sequencer */
	TArray<TSharedPtr<ISequencerDecorationEditor>> DecorationEditors;

	/** Lazily created keying fallback used for decoration channels whose owning class has no registered editor. */
	mutable TSharedPtr<ISequencerDecorationEditor> DefaultDecorationEditor;
	...
	/** Returned by the public API GetSequencerWidget. Allows for the underlying SSequencer widget to be destroyed by ~FSequencer. */
	TSharedPtr<UE::Sequencer::SSequencerWrapperWidget> SequencerWrapperWidget;
	...
	* in sync with the ActiveTemplateIDs as you enter a sequence via the specific subsection node.
	...
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	FOnChannelChanged OnChannelChangedDelegate_DEPRACRATED;
	PRAGMA_ENABLE_DEPRECATION_WARNINGS

	/** A delegate which is called when the channel is changed by Sequencer. */
	FMovieSceneOnChannelChangedWithTime OnChannelChangedDelegate;
	...
	FOnPreNavigateToSequence OnPreNavigateToSequenceEvent;
	...
	bool bIsDraggingPlaybackRange = false;
	bool bPlaybackRangeChangedDuringDrag = false;
	...
	TSharedPtr<UE::Sequencer::FSequencerEditorViewModelImpl> ViewModel;
