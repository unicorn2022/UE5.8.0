...
void FMovieScenePasteSectionsParams::AddTarget(
	UMovieSceneTrack* Track,
	int32 RowIndex,
	UMovieSceneDecorationContainerObject* DecorationContainer,
	UObject* PreferredDecoration)
{
	Tracks.Add(Track);
	TrackRowIndices.Add(RowIndex);

	if (DecorationContainer || PreferredDecoration)
	{
		DecorationTargets.Add({DecorationContainer, PreferredDecoration});
	}
}
...
void FSequencerUtilities::MakeTimeWarpMenuEntry(FMenuBuilder& MenuBuilder, UE::Sequencer::TWeakViewModelPtr<UE::Sequencer::FViewModel> WeakViewModel)
{
	using namespace UE::Sequencer;

	TViewModelPtr<ISectionOwnerExtension> SectionOwner = WeakViewModel.ImplicitPin();
	if (!SectionOwner)
	{
		return;
	}
	...
	for (const TViewModelPtr<FSectionModel>& SectionModel : SectionOwner->GetSectionModels().IterateSubList<FSectionModel>())
	{
	...
	MenuBuilder.AddSubMenu(
		TimeWarpLabel,
		TimeWarpToolTip,
		FNewMenuDelegate::CreateStatic(PopulateTimeWarpChannelSubMenu, SectionOwner.AsWeak())
	);
}
...
{
	using namespace UE::Sequencer;
	TViewModelPtr<ISectionOwnerExtension> SectionOwner = WeakTrackModel.ImplicitPin();
	return PopulateTimeWarpChannelSubMenu(MenuBuilder, SectionOwner.AsWeak());
}

void FSequencerUtilities::PopulateTimeWarpChannelSubMenu(FMenuBuilder& MenuBuilder, UE::Sequencer::TWeakViewModelPtr<UE::Sequencer::ISectionOwnerExtension> WeakSectionOwner)
{
	using namespace UE::Sequencer;

	auto HandleTimeWarpSelection = [WeakSectionOwner](TSubclassOf<UMovieSceneTimeWarpGetter> Class)
	{
		TViewModelPtr<ISectionOwnerExtension> SectionOwner = WeakSectionOwner.Pin();
		if (!SectionOwner)
		{
			return;
		}

		// Snapshot the section list: the mutation loop below may re-enter the section owner's
		// layout and re-parent these models, which must not happen while iterating them.
		TArray<TSharedPtr<FSectionModel>> SectionsToModify;
		for (const TViewModelPtr<FSectionModel>& SectionModel : SectionOwner->GetSectionModels().IterateSubList<FSectionModel>())
		{
			SectionsToModify.Add(SectionModel);
		}

		FScopedTransaction Transaction(LOCTEXT("ChangeTimeWarpType", "Changed Time Warp type"));

		for (const TSharedPtr<FSectionModel>& SectionModel : SectionsToModify)
		{
			UMovieSceneSection*         Section = SectionModel ? SectionModel->GetSection() : nullptr;
			...
			if (Variant)
			{
				...
				UObject* GetterOuter = Section->GetTypedOuter<UMovieSceneSequence>();
				UMovieSceneTimeWarpGetter* Getter = NewObject<UMovieSceneTimeWarpGetter>(GetterOuter ? GetterOuter : Section, Class.Get(), NAME_None, RF_Transactional);
				Getter->InitializeDefaults(Section);

				Variant->Set(Getter);

				Section->InvalidateChannelProxy();

				TViewModelPtr<IOutlinerExtension> Outliner = SectionOwner.ImplicitCast();
...
TArray<FMovieSceneSpawnable*> FSequencerUtilities::ConvertToSpawnable(TSharedRef<ISequencer> Sequencer, FGuid PossessableGuid)
{
	...
	if (FoundObjects.Num() == 0)
	{
		...
		UE_LOGF(LogSequencer, Error, "Failed to convert %ls to spawnable because there are no objects bound to it", Possessable ? *Possessable->GetName() : TEXT(""));
	}
...
void ExportObjectsToText(const TArray<UObject*>& ObjectsToExport, FString& ExportedText)
{
	...
	for (UObject* ObjectToExport : ObjectsToExport)
	{
		if (ObjectToExport)
		{
			...
			{
				UE_LOGF(LogSequencer, Error, "Cannot copy objects from different outers. Only copying from %ls", *LastOuter->GetName());
				continue;
			}
			LastOuter = ThisOuter;
...
bool FSequencerUtilities::PasteSections(const FString& TextToImport, FMovieScenePasteSectionsParams PasteSectionsParams, TArray<UMovieSceneSection*>& OutSections, TArray<FNotificationInfo>& OutErrors)
{
	...
	// Route decoration-hosted sections (e.g. mask sections inside
	// UMovieSceneAnimationMaskDecoration) to their host decoration. Resolution:
	//   A. A user-selected decoration on a target wins.
	//   B. Otherwise, auto-create from a registered-compatible decoration class
	//      on a target container. Decorations not registered for the container
	//      class are never created.
	// Hosted sections that find no compatible host are dropped, never falling
	// through to the per-track loop.

	struct FDecorationHostInfo
	{
		UClass* DecorationClass = nullptr;
		TSubclassOf<UMovieSceneSection> HostedSectionClass;
	};
	TArray<FDecorationHostInfo> AllDecorationHosts;
	{
		TSet<UClass*> RegisteredDecorationClasses;
		IMovieSceneModule::Get().GetAllRegisteredDecorationClasses(RegisteredDecorationClasses);
		for (UClass* TestClass : RegisteredDecorationClasses)
		{
			if (!TestClass || !TestClass->ImplementsInterface(UMovieSceneSectionProviderDecoration::StaticClass()))
			{
				continue;
			}
			IMovieSceneSectionProviderDecoration* Provider = Cast<IMovieSceneSectionProviderDecoration>(TestClass->GetDefaultObject());
			if (!Provider || !Provider->SupportsMultipleSections())
			{
				continue;
			}
			TSubclassOf<UMovieSceneSection> Hosted = Provider->GetHostedSectionClass();
			if (!*Hosted)
			{
				continue;
			}
			AllDecorationHosts.Add({TestClass, Hosted});
		}
	}

	auto IsDecorationHostedSectionClass = [&](UClass* SectionClass) -> bool
	{
		for (const FDecorationHostInfo& Host : AllDecorationHosts)
		{
			if (SectionClass->IsChildOf(*Host.HostedSectionClass))
			{
				return true;
			}
		}
	...
	// Resolve per-track containers up-front so callers that pass plain Tracks
	// without explicit DecorationTargets still benefit (e.g. anim mixer track
	// returns its layer for a row).
	TArray<FMovieScenePasteDecorationTarget> ResolvedDecorationTargets = PasteSectionsParams.DecorationTargets;
	for (int32 Index = 0; Index < PasteSectionsParams.Tracks.Num(); ++Index)
	{
		...
		if (PasteSectionsParams.TrackRowIndices.Num() == PasteSectionsParams.Tracks.Num())
		{
			TargetRowIndex = PasteSectionsParams.TrackRowIndices[Index];
		}
		else if (PasteSectionsParams.TrackRowIndices.Num() == 1)
		{
			TargetRowIndex = PasteSectionsParams.TrackRowIndices[0];
		}

		if (UMovieSceneDecorationContainerObject* RowContainer = Track->GetDecorationContainerForRow(TargetRowIndex))
		{
			const bool bAlreadyPresent = ResolvedDecorationTargets.ContainsByPredicate(
				[RowContainer](const FMovieScenePasteDecorationTarget& Target) { return Target.Container == RowContainer; });
			if (!bAlreadyPresent)
			{
				ResolvedDecorationTargets.Add({RowContainer, nullptr});
			}
		}
	}

	TArray<UMovieSceneSection*> RoutedToDecoration;
	TArray<UMovieSceneSection*> HostedButOrphaned;
	for (UMovieSceneSection* Section : ImportedSections)
	{
		UClass* SectionClass = Section->GetClass();
		if (!IsDecorationHostedSectionClass(SectionClass))
		{
			continue;
		}

		UObject* HostDecoration = nullptr;
		UMovieSceneDecorationContainerObject* HostContainer = nullptr;
		UClass* MatchedDecorationClass = nullptr;

		// User-selected decoration wins over auto-create.
		for (const FMovieScenePasteDecorationTarget& Target : ResolvedDecorationTargets)
		{
			if (!Target.PreferredDecoration)
			{
				continue;
			}
			IMovieSceneSectionProviderDecoration* Provider = Cast<IMovieSceneSectionProviderDecoration>(Target.PreferredDecoration.Get());
			if (!Provider)
			{
				continue;
			}
			...
			if (SectionClass->IsChildOf(*Hosted))
			{
				HostDecoration = Target.PreferredDecoration.Get();
				HostContainer = Target.Container.Get();
				MatchedDecorationClass = Target.PreferredDecoration->GetClass();
				break;
			}
		}

		// Auto-create on a registered-compatible target container.
		if (!HostDecoration)
		{
			for (const FMovieScenePasteDecorationTarget& Target : ResolvedDecorationTargets)
			{
				...
				IMovieSceneModule::Get().GetCompatibleDecorationsForContainer(Target.Container.Get(), CompatibleDecorations);
				for (UClass* DecorationClass : CompatibleDecorations)
				{
					if (!DecorationClass || !DecorationClass->ImplementsInterface(UMovieSceneSectionProviderDecoration::StaticClass()))
					{
						continue;
					}
					IMovieSceneSectionProviderDecoration* CDO = Cast<IMovieSceneSectionProviderDecoration>(DecorationClass->GetDefaultObject());
					if (!CDO || !CDO->SupportsMultipleSections())
					{
						continue;
					}
					TSubclassOf<UMovieSceneSection> Hosted = CDO->GetHostedSectionClass();
					if (!*Hosted)
					{
						continue;
					}
					...
					{
						HostContainer = Target.Container.Get();
						MatchedDecorationClass = DecorationClass;
						break;
					}
				}
		...
		if (!HostDecoration && !HostContainer)
		{
			HostedButOrphaned.Add(Section);
			continue;
		}

		if (!HostDecoration)
		{
			check(HostContainer && MatchedDecorationClass);
			HostDecoration = HostContainer->FindDecoration(MatchedDecorationClass);
			if (!HostDecoration)
			{
				HostContainer->Modify();
				HostDecoration = HostContainer->GetOrCreateDecoration(MatchedDecorationClass);
			}
		}

		IMovieSceneSectionProviderDecoration* HostProvider = Cast<IMovieSceneSectionProviderDecoration>(HostDecoration);
		if (!HostProvider)
		{
			HostedButOrphaned.Add(Section);
			continue;
		...
		Section->PostPaste();
		if (!Section->Rename(nullptr, HostDecoration, REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch))
		{
			HostedButOrphaned.Add(Section);
			continue;
		}

		// Move before AddSection so any overlap fixup the decoration runs sees
		// the destination range, not the source range.
		if (Section->HasStartFrame() && FirstFrame.IsSet())
		{
			FFrameNumber NewStartFrame = PasteSectionsParams.Time.FrameNumber + (Section->GetInclusiveStartFrame() - FirstFrame.GetValue());
			FFrameNumber Delta = NewStartFrame - Section->GetInclusiveStartFrame();
			if (Delta != 0)
			{
		...
		HostProvider->AddSection(Section);

		RoutedToDecoration.Add(Section);
		OutSections.Add(Section);
	}

	for (UMovieSceneSection* Section : RoutedToDecoration)
	{
		ImportedSections.Remove(Section);
	}
	for (UMovieSceneSection* Section : HostedButOrphaned)
	{
		UE_LOGF(LogSequencer, Error, "Could not paste section of type %ls: no compatible decoration host on the paste target.",
	...
	for (int32 Index = 0; Index < PasteSectionsParams.Tracks.Num(); ++Index)
	{
		...
		for (int32 SectionIndex = 0; SectionIndex < ImportedSections.Num(); ++SectionIndex)
		{
			...
			// If there is only 1 track row index, paste all sections onto that row.
			// INDEX_NONE means "no override / no filter" -- keep section's source row.
			if (PasteSectionsParams.TrackRowIndices.Num() == 1)
			{
				if (PasteSectionsParams.TrackRowIndices[0] != INDEX_NONE)
				{
			...
			{
				const int32 OverrideRowIndex = PasteSectionsParams.TrackRowIndices[Index];
				if (OverrideRowIndex != INDEX_NONE && OverrideRowIndex != Section->GetRowIndex())
				{
					continue;
				}
			}
			...
			if (Section->HasStartFrame())
			{
				...
				{
					Section->MoveSection(Delta);
					FMovieSceneSectionMovedParams MovedParams(EPropertyChangeType::ValueSet);
					Track->OnSectionMoved(*Section, MovedParams);
				}
			}
			...
			if (!bAllowOverlap)
			{
				if (Section->OverlapsWithSections(Track->GetAllSections()))
				{
					...
					UE_LOGF(LogSequencer, Error, "Could not paste section because it overlaps with existing sections and this track type does not allow overlaps");
					continue;
				}
			}
	...
	for (int32 SectionIndex = 0; SectionIndex < ImportedSections.Num(); ++SectionIndex)
	{
		...
		{
			UE_LOGF(LogSequencer, Error, "Could not paste section of type %ls", *ImportedSections[SectionIndex]->GetClass()->GetName());
		}
	}

	if (SectionIndicesImported.Num() == 0 && RoutedToDecoration.Num() == 0)
	{
...
/**
 *
 * Copy/paste decorations
 *
 */

void FSequencerUtilities::CopyDecorations(const TArray<UObject*>& Decorations, FString& ExportedText)
{
	TArray<UObject*> Objects;
...
class FDecorationObjectTextFactory : public FCustomizableTextObjectFactory
{
public:
	FDecorationObjectTextFactory()
		: FCustomizableTextObjectFactory(GWarn)
	{
	}
	...
	virtual bool CanCreateClass(UClass* InObjectClass, bool& bOmitSubObjs) const override
	{
		// Section-provider decorations only. Their inner section sub-objects
		// must remain nested under the decoration's Begin Object block during
		// import; accepting UMovieSceneSection here would hoist them to
		// top-level and leave the decoration's section array dangling.
		return InObjectClass->ImplementsInterface(UMovieSceneSectionProviderDecoration::StaticClass());
	}

	virtual void ProcessConstructedObject(UObject* NewObject) override
	{
		check(NewObject);
		NewDecorations.Add(NewObject);
	}
...
static void ImportDecorationsFromText(const FString& TextToImport, /*out*/ TArray<UObject*>& ImportedDecorations)
{
	UPackage* TempPackage = NewObject<UPackage>(nullptr, TEXT("/Engine/Sequencer/Editor/Transient"), RF_Transient);
	TempPackage->AddToRoot();

	FDecorationObjectTextFactory Factory;
	Factory.ProcessBuffer(TempPackage, RF_Transactional, TextToImport);

	ImportedDecorations = Factory.NewDecorations;

	TempPackage->RemoveFromRoot();
}

bool FSequencerUtilities::PasteDecorations(const FString& TextToImport, FMovieScenePasteDecorationsParams PasteDecorationsParams, TArray<UObject*>& OutDecorations, TArray<FNotificationInfo>& OutErrors)
{
	TArray<UObject*> SourceDecorations;
	ImportDecorationsFromText(TextToImport, SourceDecorations);
	if (SourceDecorations.Num() == 0)
	{
		return false;
	}

	if (PasteDecorationsParams.Containers.Num() == 0)
	{
	...
	const FScopedTransaction Transaction(LOCTEXT("PasteDecorations", "Paste Decorations"));

	int32 NumPasted = 0;
	int32 NumIncompatible = 0;
	int32 NumDuplicates = 0;

	for (UMovieSceneDecorationContainerObject* Container : PasteDecorationsParams.Containers)
	{
		if (!Container)
		{
			continue;
		...
		TSet<UClass*> CompatibleDecorations;
		IMovieSceneModule::Get().GetCompatibleDecorationsForContainer(Container, CompatibleDecorations);

		for (UObject* SourceDecoration : SourceDecorations)
		{
			if (!SourceDecoration)
			{
				continue;
			...
			UClass* DecorationClass = SourceDecoration->GetClass();
			if (!CompatibleDecorations.Contains(DecorationClass))
			{
				++NumIncompatible;
				continue;
			}

			// Decorations are singletons today (FindDecoration / GetOrCreateDecoration are class-keyed).
			if (Container->FindDecoration(DecorationClass) != nullptr)
			{
				++NumDuplicates;
				continue;
			}
			...
			Container->Modify();
			UObject* NewDecoration = StaticDuplicateObject(SourceDecoration, Container);
			NewDecoration->ClearFlags(RF_Transient);
			ForEachObjectWithOuter(NewDecoration, [](UObject* InObject) {
				if (!InObject->GetClass() || !InObject->GetClass()->HasAnyClassFlags(CLASS_Transient))
				{
					InObject->ClearFlags(RF_Transient);
				}
			...
			if (IMovieSceneSectionProviderDecoration* Provider = Cast<IMovieSceneSectionProviderDecoration>(NewDecoration))
			{
				for (UMovieSceneSection* Section : Provider->GetSections())
				{
					if (Section)
					{
						Section->PostPaste();
			...
			OutDecorations.Add(NewDecoration);
			++NumPasted;
		}
	}
	...
	{
		FText Reason;
		if (NumDuplicates > 0 && NumIncompatible == 0)
		{
			Reason = LOCTEXT("PasteDecorations_AlreadyExists", "Can't paste decoration. Target already has a decoration of that type.");
		}
		else
		{
			Reason = LOCTEXT("PasteDecorations_Incompatible", "Can't paste decoration. Target does not support that decoration type.");
		}
...
bool FSequencerUtilities::CanPasteDecorations(const FString& TextToImport)
{
	FDecorationObjectTextFactory Factory;
...
FGuid CreateGenericBinding(TSharedPtr<ISequencer> Sequencer, UMovieSceneSequence* OwnerSequence, UObject* InObject, FMovieSceneBindingReferences* BindingReferences, const UE::Sequencer::FCreateBindingParams& InParams)
{
	...
	if (bAllowCustom)
	{
		...
		{
			UObject* PlaybackContext = Sequencer->GetPlaybackContext();
			World = PlaybackContext ? PlaybackContext->GetWorld() : nullptr;
		}
		else
		{
		...
		if (FactoryCreatedActorInstance && World)
		{
	...
	for (int32 Index = 0; Index < ObjectsToPossess.Num(); ++Index)
	{
		...
		if (!NewPossessable)
		{
			...
			UClass* CurrentClass = CurrentObject ? CurrentObject->GetClass() : nullptr;
		...
		if (InParams.ReplacementGuid.IsValid()
			|| (Sequencer && (!CurrentObject || !NewPossessable->BindSpawnableObject(Sequencer->GetFocusedTemplateID(), CurrentObject, Sequencer->GetSharedPlaybackState()))))
		{
	...
	for (int32 TargetIndex = 0; TargetIndex < NumTargets; ++TargetIndex)
	{
		...
		for (UMovieSceneCopyableBinding* CopyableBinding : ImportedBindings)
		{
			...
			if (CopyableBinding->Possessable.GetGuid().IsValid())
			{
				...
				if (World)
				{
					// If PastedActors are specified, replace the existing bound object name with the matching pasted actor path name
					for (const TPair<FName, TObjectPtr<AActor>>& PastedActorPair : PasteBindingsParams.PastedActors)
					{
						for (FString& BoundObjectName : CopyableBinding->BoundObjectNames)
						{
							if (BoundObjectName == PastedActorPair.Key)
							{
								BoundObjectName = PastedActorPair.Value->GetPathName(World);
							}
						}
					}
...
void FSequencerUtilities::RemoveAllBindings(TSharedRef<ISequencer> Sequencer, const FMovieSceneBindingProxy& ObjectBinding)
{
	UMovieSceneSequence* Sequence = ObjectBinding.Sequence;
	UMovieScene* MovieScene = ObjectBinding.GetMovieScene();
...
void FSequencerUtilities::RemoveInvalidBindings(TSharedRef<ISequencer> Sequencer, const FMovieSceneBindingProxy& ObjectBinding)
{
	UMovieSceneSequence* Sequence = ObjectBinding.Sequence;
	UMovieScene* MovieScene = ObjectBinding.GetMovieScene();
	if (!Sequence || !MovieScene)
...
bool FSequencerUtilities::BakeTransform(TSharedRef<ISequencer> Sequencer, const TArray<FMovieSceneBindingProxy>& ObjectBindings, const FBakingAnimationKeySettings& InSettings)
{
	...
	{
		UE_LOGF(LogSequencer, Warning, "Bake Transform failed.");
		return false;
	}
	...
	for (TPair<FGuid, FBakeData>& BakeData : BakeDataMap)
	{
		...
		for (UMovieSceneTrack* Track : FocusedMovieScene->FindTracks(UMovieScene3DConstraintTrack::StaticClass(), Guid))
		{
			if (UMovieScene3DConstraintTrack* ConstraintTrack = Cast<UMovieScene3DConstraintTrack>(Track))
			{
				...
				{
					MovieSceneHelpers::DisableTrack(ConstraintTrack);
				}
				else
				{
		...
		for (UMovieSceneTrack* Track : FocusedMovieScene->FindTracks(UMovieScene3DTransformTrack::StaticClass(), Guid))
		{
			if (UMovieScene3DTransformTrack* TransformTrack = Cast<UMovieScene3DTransformTrack>(Track))
			{
				if (bDisableSectionsAfterBaking)
				{
					MovieSceneHelpers::DisableTrack(TransformTrack);
				}
				else
				{
		...
		for (UMovieSceneTrack* Track : FocusedMovieScene->FindTracks(UMovieSceneCameraShakeTrack::StaticClass(), Guid))
		{
			if (UMovieSceneCameraShakeTrack* CameraShakeTrack = Cast<UMovieSceneCameraShakeTrack>(Track))
			{
				if (bDisableSectionsAfterBaking)
				{
					MovieSceneHelpers::DisableTrack(CameraShakeTrack);
				}
				else
				{
...
void UpdatePossessedClasses(UMovieScene* MovieScene, FMovieSceneSequenceIDRef SequenceID, const FMovieSceneSequenceHierarchy* Hierarchy, FGuid ObjectBindingID, UClass* ChosenClass)
{
	...
	{
		FMovieScenePossessable& Possessable = MovieScene->GetPossessable(Index);
		const bool bIsSelf = Possessable.GetGuid() == ObjectBindingID;
		const bool bTracksSpawnable = Possessable.GetSpawnableObjectBindingID().GetGuid() == ObjectBindingID;
		if ((bIsSelf || bTracksSpawnable) && Possessable.GetPossessedObjectClass() != ChosenClass)
		{
			MovieScene->Modify();
...
void FSequencerUtilities::SnapSectionsToTimelineUsingSourceTimecode(TSharedRef<ISequencer> Sequencer, const TArray<UMovieSceneSection*>& Sections)
{
...
void FSequencerUtilities::SyncSectionsUsingSourceTimecode(TSharedRef<ISequencer> Sequencer, const TArray<UMovieSceneSection*>& Sections)
{
	const UMovieScene* FocusedMovieScene = Sequencer->GetFocusedMovieSceneSequence()->GetMovieScene();
	if (!FocusedMovieScene)
	{
...
void FSequencerUtilities::FixActorReferences(TSharedRef<ISequencer> Sequencer)
{
	UObject* PlaybackContext = Sequencer->GetPlaybackContext();
	if (!PlaybackContext)
	{
	...
	for (const FMovieScenePossessable& ActorPossessableToFix : ActorsPossessablesToFix)
	{
		...
		if (ActorPtr != nullptr)
		{
			...
			FGuid NewGuid = AssignActor(Sequencer, ActorPtr, ActorPossessableToFix.GetGuid());
