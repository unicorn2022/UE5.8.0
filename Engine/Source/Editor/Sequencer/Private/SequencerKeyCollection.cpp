...
TOptional<FFrameNumber> GetNextKeyInternal(const TArray<FFrameNumber>& Times, FFrameNumber FrameNumber, EFindKeyDirection Direction, const TRange<FFrameNumber>& Range, const bool bAllowLoop)
{
	...
	else if (KeysInRange.Num() > 0 && bAllowLoop)
	{
		if (Direction == EFindKeyDirection::Forwards)
		{
...
TOptional<FFrameNumber> FSequencerKeyCollection::GetNextKey(FFrameNumber FrameNumber, EFindKeyDirection Direction, const TRange<FFrameNumber>& Range, EFindKeyType FindKeyType, const bool bAllowLoop) const
{
	if (FindKeyType == EFindKeyType::FKT_Keys)
	{
		return GetNextKeyInternal(GroupedTimes, FrameNumber, Direction, Range, bAllowLoop);
	}
	else if (FindKeyType == EFindKeyType::FKT_Sections)
	{
		return GetNextKeyInternal(GroupedSectionTimes, FrameNumber, Direction, Range, bAllowLoop);
	}
	else if (FindKeyType == EFindKeyType::FKT_All)
	{
		return GetNextKeyInternal(AllGroupedTimes, FrameNumber, Direction, Range, bAllowLoop);
	}
	return TOptional<FFrameNumber>();
}
