...
FSequencerNodeTree::FSequencerNodeTree(FSequencer& InSequencer)
	...
	, FilterEvaluator(MakeShared<UE::Sequencer::FFilterEvaluator>())
{
}
...
{
	return FilterEvaluator->UpdateFiltersOnTrackValueChanged();
}
...
void FSequencerNodeTree::Update()
{
	...
	FilterEvaluator->UpdateFilters(bForce);
	...
	// This will cause relevant systems to update. E.g. curve editor, if enabled, will updated based on the new filtered hierarchy.
...
void FSequencerNodeTree::RequestFilterUpdate()
{
	FilterEvaluator->RequestFilterUpdate(Sequencer.GetFilterBar());
}

bool FSequencerNodeTree::NeedsFilterUpdate() const
{
	return FilterEvaluator->NeedsFilterUpdate();
}
