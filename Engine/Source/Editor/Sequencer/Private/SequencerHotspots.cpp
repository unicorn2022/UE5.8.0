...
// Iterate section decorations and let registered decoration editors
// contribute to the context menu.
static void BuildDecorationContextMenu(
	...
	TWeakPtr<ISequencer> Sequencer)
{
	...
	for (TObjectPtr<UObject> DecorationObject : Section->GetDecorations())
	{
		if (!DecorationObject)
		{
			continue;
		...
		if (ISequencerDecorationEditor* Editor = SequencerPtr->FindDecorationEditor(DecorationObject->GetClass()))
		{
			Editor->BuildSectionContextMenu(MenuBuilder, *Section, *DecorationObject, ObjectBinding, Sequencer);
		}
	}
}
...
bool FKeyBarHotspot::PopulateContextMenu(FMenuBuilder& MenuBuilder, TSharedPtr<FExtender> MenuExtender, FFrameTime MouseDownTime)
{
	...
	SectionModel->GetSectionInterface()->BuildSectionContextMenu(MenuBuilder, ObjectBindingGuid);
	BuildDecorationContextMenu(MenuBuilder, SectionModel->GetSection(), ObjectBindingGuid, Sequencer);
...
void FSectionHotspotBase::UpdateOnHover(FTrackAreaViewModel& InTrackArea) const
{
	...
	{
		// Check if the section interface wants to show key areas (and thus supports key selection)
		TSharedPtr<FSectionModel> SectionModel = WeakSectionModel.Pin();
		TSharedPtr<ISequencerSection> SectionInterface = SectionModel ? SectionModel->GetSectionInterface() : nullptr;
		const bool bShowKeyArea = !SectionInterface || SectionInterface->ShouldShowKeyAreaWhenCollapsed();

		// Activate selection mode if the section has keys and wants to show them
		if (bShowKeyArea)
		{
...
bool FSectionHotspotBase::PopulateContextMenu(FMenuBuilder& MenuBuilder, TSharedPtr<FExtender> MenuExtender, FFrameTime MouseDownTime)
{
	...
	if (ThisSection)
	{
		...
		FGuid ObjectBindingGuid = ObjectBinding ? ObjectBinding->GetObjectGuid() : FGuid();
		SectionModel->GetSectionInterface()->BuildSectionContextMenu(MenuBuilder, ObjectBindingGuid);
		BuildDecorationContextMenu(MenuBuilder, ThisSection, ObjectBindingGuid, WeakSequencer.Pin());
	}
...
bool FSectionEasingAreaHotspot::PopulateContextMenu(FMenuBuilder& MenuBuilder, TSharedPtr<FExtender> MenuExtender, FFrameTime MouseDownTime)
{
	...
	if (ThisSection)
	{
		...
		FGuid ObjectBindingGuid = ObjectBinding ? ObjectBinding->GetObjectGuid() : FGuid();
		SectionModel->GetSectionInterface()->BuildSectionContextMenu(MenuBuilder, ObjectBindingGuid);
		BuildDecorationContextMenu(MenuBuilder, ThisSection, ObjectBindingGuid, Sequencer);
	}
