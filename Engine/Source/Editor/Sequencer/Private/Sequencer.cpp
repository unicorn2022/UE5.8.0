...
#include "Widgets/SSequencerWrapper.h"
...
static TAutoConsoleVariable<bool> CVarRestorePreAnimatedStateOnBlueprintPreCompile(
	TEXT("Sequencer.RestorePreAnimatedStateOnBlueprintPreCompile"),
	false,
	TEXT("Enable/disable restoring pre-animated state before Blueprint classes get (re)compiled"));
...
namespace UE
{
	...
	static bool bEvaluateOnUndo = true;
	static FAutoConsoleVariableRef CVarEvaluateOnUndo(
		TEXT("Sequencer.EvaluateOnUndo"),
		bEvaluateOnUndo,
		TEXT("Enable/disable Sequencer evaluating on undo/redo"));
		...
		struct FDeferredSignedObjectChangeHandler : UE::MovieScene::IDeferredSignedObjectChangeHandler
		{
			...
			void Init()
			{
				...
				{
					FCoreDelegates::GetOnPostEngineInit().AddLambda([this] { this->Init(); });
				}
			}
...
const FName FSequencer::SelectionDetailsTabId = TEXT("SequencerSelectionDetails");
...
void FSequencer::InitSequencer(const FSequencerInitParams& InitParams
	...
	, const TArray<FOnCreateDecorationEditor>& DecorationEditorDelegates
	...
	, const TArray<FOnCreateOutlinerIndicator>& OutlinerIndicatorDelegates)
{
...

	{
		...
		{
			if (CVarRestorePreAnimatedStateOnBlueprintPreCompile->GetBool())
			{
	...
	{
		FDelegateHandle OnAssetRenamedHandle = AssetRegistryModule->Get().OnAssetRenamed().AddLambda([this](const FAssetData& AssetData, const FString& AssetOldName)
		{
			UMovieSceneSequence* MovieScene = GetFocusedMovieSceneSequence();
			if (AssetData.GetAsset() == MovieScene)
			{
				UMovieSceneSequence* RootSequencePtr = RootSequence.Get();
				if (RootSequencePtr)
				{
					ResetToNewRootSequence(*RootSequencePtr);
				}
			}
		});
		AcquiredResources.Add([=] { AssetRegistryModule->Get().OnAssetRenamed().Remove(OnAssetRenamedHandle); });
	}
	...
	// Create per-sequencer decoration editors via the registered factories.
	for (const FOnCreateDecorationEditor& Factory : DecorationEditorDelegates)
	{
		check(Factory.IsBound());
		DecorationEditors.Add(Factory.Execute(SharedThis(this)));
	}
	...
	ViewModel = MakeShared<FSequencerEditorViewModelImpl>(SharedThisRef, GetHostCapabilities());
	...
	SequencerWrapperWidget = SNew(SSequencerWrapperWidget, SequencerWidget.ToSharedRef());
	...
	if (HostCapabilities.bSupportsSidebar)
	{
		...
		SequencerWidget->SetSelectionDetailsPoppedOut(Settings->IsSelectionDetailsPoppedOut());
	}
...
void FSequencer::OnPlaybackContextChanged()
{
	...
	// Update runner to match the newly created shared payback state of the root template instance.
	Runner = RootTemplateInstance.GetRunner();
	InitRootSequenceInstance();
}
...
namespace
{
	// Used by FindDecorationOwner when no class-specific editor has been registered.
	// Inherits the base ISequencerDecorationEditor::AddOrUpdateKey, which keys the
	// channel via the standard channel editor interface.
	class FDefaultDecorationEditor : public ISequencerDecorationEditor
	{
	public:
		virtual UClass* GetDecorationClass() const override { return nullptr; }
	};
}

ISequencerDecorationEditor* FSequencer::FindDecorationEditor(UClass* DecorationClass) const
{
	if (!DecorationClass)
	{
		return nullptr;
	}
	for (const TSharedPtr<ISequencerDecorationEditor>& Editor : DecorationEditors)
	{
		if (Editor && Editor->GetDecorationClass() == DecorationClass)
		{
			return Editor.Get();
		}
	}
...
ISequencerDecorationEditor* FSequencer::GetOrCreateDefaultDecorationEditor() const
{
	if (!DefaultDecorationEditor.IsValid())
	{
		DefaultDecorationEditor = MakeShared<FDefaultDecorationEditor>();
	}
	return DefaultDecorationEditor.Get();
}

TTuple<ISequencerDecorationEditor*, UObject*> FSequencer::FindDecorationOwner(
	const FMovieSceneChannelHandle& ChannelHandle, UMovieSceneSection* Section) const
{
	if (const FMovieSceneChannelMetaData* MetaData = ChannelHandle.GetMetaData())
	{
		UObject* OwningObject = MetaData->WeakOwningObject.Get();
		if (OwningObject && OwningObject != static_cast<UObject*>(Section))
		{
			ISequencerDecorationEditor* Editor = FindDecorationEditor(OwningObject->GetClass());
			if (!Editor)
			{
				Editor = GetOrCreateDefaultDecorationEditor();
			}
			return MakeTuple(Editor, OwningObject);
		}
	}
	return MakeTuple(nullptr, static_cast<UObject*>(nullptr));
}
...
void FSequencer::Tick(float InDeltaTime)
{
	...
	if (AutoscrubOffset.IsSet() && PlaybackState == EMovieScenePlayerStatus::Scrubbing )
	{
		...
		const bool bForwards = PlaybackSpeed > 0;
		SetLocalTimeLooped(CurrentTime.Time + Offset, CurrentTimeBreadcrumbs, bForwards);
	}
	...
	if (PlaybackState == EMovieScenePlayerStatus::Playing)
	{
		...
		const bool bForwards = PlaybackSpeed > 0;
		SetLocalTimeLooped(LocalTime, Breadcrumbs, bForwards);
	...
	// Tick the per-sequencer decoration editors.
	for (const TSharedPtr<ISequencerDecorationEditor>& DecorationEditor : DecorationEditors)
	{
		if (DecorationEditor)
		{
			DecorationEditor->Tick(InDeltaTime);
		}
	}
...
{
	return SequencerWrapperWidget.ToSharedRef();
}

UE::Sequencer::ISequencerWidget& FSequencer::GetSequencerWidgetInterface() const
{
	return *SequencerWrapperWidget;
}
...
FMovieSceneSequenceTransform FSequencer::GetFocusedMovieSceneSequenceTransformUnwarped() const
{
	return RootToUnwarpedLocalTransform;
}
...
void FSequencer::ResetToNewRootSequence(UMovieSceneSequence& NewSequence)
{
	...
	OnPreNavigateToSequenceEvent.Broadcast();
...
void FSequencer::DeleteSections(const TSet<UMovieSceneSection*>& Sections)
{
	...
	for (UMovieSceneSection* Section : Sections)
	{
		...
		// Check if section interface allows deletion
		TSharedPtr<UE::Sequencer::FSectionModel> SectionModel = GetNodeTree()->GetSectionModel(Section);
		if (SectionModel && SectionModel->GetSectionInterface() && !SectionModel->GetSectionInterface()->IsDeletable())
		{
			continue;
		}

		UObject* SectionOuter = Section->GetOuter();

		// Check if section comes from a decoration that provides sections
		if (IMovieSceneSectionProviderDecoration* SectionProvider = Cast<IMovieSceneSectionProviderDecoration>(SectionOuter))
		{
			// Section is provided by a decoration - remove section if there are more than one, or remove the decoration from its container if it's the last section
			UMovieSceneDecorationContainerObject* Container = Cast<UMovieSceneDecorationContainerObject>(SectionOuter->GetOuter());
			if (Container)
			{
				Container->SetFlags(RF_Transactional);
				Container->Modify();
				if (SectionProvider->SupportsMultipleSections() && SectionProvider->GetSections().Num() > 1)
				{
					SectionOuter->SetFlags(RF_Transactional);
					SectionOuter->Modify();
					SectionProvider->RemoveSection(*Section);
					if (UMovieSceneTrack* OuterTrack = Section->GetTypedOuter<UMovieSceneTrack>())
					{
						OuterTrack->Modify();
						OuterTrack->UpdateEasing();
					}
				}
				else
				{
					Container->RemoveDecoration(SectionOuter->GetClass());
				}
				bAnythingRemoved = true;
			}
		}
		else if (UMovieSceneTrack* Track = Cast<UMovieSceneTrack>(SectionOuter))
		{
			// Standard case: section is owned by a track
...
void FSequencer::TransformSelection(FFrameTime InDeltaTime, float InScale)
{
	using namespace UE::Sequencer;
	...
	const TViewModelPtr<FSequenceModel> RootSequenceModel = ViewModel->GetRootSequenceModel();
	...
	FSequencerSelectedKey::AppendKeySelection(SelectedKeys, GetSelection().KeySelection);

	TSet<TSharedPtr<FChannelModel>> AllChannels;
	SequencerHelpers::GetAllChannels(RootSequenceModel, AllChannels);

	TSet<TWeakViewModelPtr<FChannelModel>> WeakChannels;
	for (const TSharedPtr<FChannelModel>& Channel : AllChannels)
	{
		if (Channel.IsValid())
		{
			WeakChannels.Add(TWeakViewModelPtr<FChannelModel>(Channel));
		}
	}

	const TSet<int32> SelectedMarkedFrames = GetSelection().MarkedFrames.GetSelected();
	const TSet<UMovieSceneSection*> SelectedSections = GetSelection().GetSelectedSections();
	if (SelectedMarkedFrames.IsEmpty() && SelectedSections.IsEmpty())
	{
		KeyHelperUtils::TransformSelectedOrRelativeKeys(*this, WeakChannels, SelectedKeys, InDeltaTime, InScale);
		return;
	}

	const FScopedTransaction Transaction(LOCTEXT("TransformSelection_Transaction", "Transform Selection"));

	for (TSet<FSequencerSelectedKey>::TIterator It = SelectedKeys.CreateIterator(); It; ++It)
	{
		if (SelectedSections.Contains(It->Section))
		{
			It.RemoveCurrent();
		}
	}

	const bool bSectionsChanged = KeyHelperUtils::TransformSectionSelection(*this, SelectedSections, InDeltaTime, InScale, false);
	const bool bKeysChanged = KeyHelperUtils::TransformKeySelection(*this, SelectedKeys, InDeltaTime, InScale, false);
	const bool bMarksChanged = KeyHelperUtils::TransformMarkedFrameSelection(*this, SelectedMarkedFrames, InDeltaTime, InScale);

	if (bMarksChanged && !bKeysChanged && !bSectionsChanged)
	{
		NotifyMovieSceneDataChanged(EMovieSceneDataChangeType::TrackValueChanged);
	}
}

void FSequencer::TranslateSelection(bool bTranslateLeft)
{
	if (IsReadOnly())
	{
		FSequencerUtilities::ShowReadOnlyError();
...
void FSequencer::StretchTime(FFrameTime InDeltaTime)
{
	...
	SelectInSelectionRange(true, true, true);
	TransformSelection(InDeltaTime, 1.f);
...
TSharedPtr<UE::Sequencer::SimpleView::FSimpleViewTimeline> FSequencer::GetSimpleViewTimeline() const
{
	return SequencerWidget.IsValid() ? SequencerWidget->GetSimpleViewTimeline() : nullptr;
}

bool FSequencer::CanAddTransformKeysForSelectedObjects(const EMovieSceneTransformChannel InChannel) const
{
	const TSharedPtr<UE::Sequencer::SimpleView::FSimpleViewTimeline> Timeline = GetSimpleViewTimeline();
	if (Timeline.IsValid() && Timeline->IsInSimpleView())
	{
		return Timeline->CanExecuteTransformKeyCommand(InChannel);
	}

	return CanAddTransformKeysForSelectedObjectsInternal(InChannel);
}

bool FSequencer::CanAddTransformKeysForSelectedObjectsInternal(const EMovieSceneTransformChannel InChannel) const
{
...
bool FSequencer::CanDeleteTransformKeysForSelectedObjects(const EMovieSceneTransformChannel InChannel) const
{
	const TSharedPtr<UE::Sequencer::SimpleView::FSimpleViewTimeline> Timeline = GetSimpleViewTimeline();
	return (Timeline.IsValid() && Timeline->IsInSimpleView())
		? Timeline->CanExecuteDeleteTransformKeyCommand(InChannel)
		: CanAddTransformKeysForSelectedObjects(InChannel);
}

void FSequencer::OnAddTransformKeysForSelectedObjects(const EMovieSceneTransformChannel InChannel)
{
	const TSharedPtr<UE::Sequencer::SimpleView::FSimpleViewTimeline> Timeline = GetSimpleViewTimeline();
	if (Timeline.IsValid() && Timeline->IsInSimpleView())
	{
		Timeline->ExecuteTransformKeyCommand(InChannel);
		return;
	}

	AddTransformKeysForSelectedObjectsInternal(InChannel);
}

void FSequencer::AddTransformKeysForSelectedObjectsInternal(const EMovieSceneTransformChannel InChannel)
{
	...
	for (int32 i = 0; i < PossibleTrackEditors.Num(); ++i)
	{
		if (AtLeastOneHasPriority)
		{
			...
			{
				PossibleTrackEditors[i]->OnAddTransformKeysForSelectedObjects(InChannel);
			}
		}
		else
		{
...
void FSequencer::OnDeleteTransformKeysForSelectedObjects(const EMovieSceneTransformChannel InChannel)
{
	const TSharedPtr<UE::Sequencer::SimpleView::FSimpleViewTimeline> Timeline = GetSimpleViewTimeline();
	if (Timeline.IsValid() && Timeline->IsInSimpleView())
	{
		Timeline->ExecuteDeleteTransformKeyCommand(InChannel);
		return;
	}

	FScopedTransaction DeleteKeyTransaction(NSLOCTEXT("Sequencer", "DeleteTransformKey_Transaction", "Delete Transform Key"));
	...
	for (int32 i = 0; i < PossibleTrackEditors.Num(); ++i)
	{
		if (AtLeastOneHasPriority)
		{
			if (PossibleTrackEditors[i]->HasTransformKeyOverridePriority())
			{
				PossibleTrackEditors[i]->OnDeleteTransformKeysForSelectedObjects(InChannel);
			}
		}
		else
		{
...
void FSequencer::NotifyMovieSceneDataChanged( EMovieSceneDataChangeType DataChangeType )
{
	if (!GetFocusedMovieSceneSequence()->GetMovieScene())
	{
		...
		{
			UE_LOGF(LogSequencer, Error, "Fatal error, focused movie scene no longer valid and there is no root sequence to default to.");
		}
	}
...
void FSequencer::RecreateCurveEditor()
{
	...
	FSequencerCurveEditorApp* CurveEditorIntegration = FSequencerCurveEditorApp::Get(*this);
	if (ensure(CurveEditorIntegration))
	{
		CurveEditorIntegration->ResetCurveEditor();
		CurveEditorIntegration->RequestSyncSelection();
	}
}
...
void FSequencer::SelectInSelectionRange(bool bSelectKeys, bool bSelectSections, bool bSelectMarkers)
{
	using namespace UE::Sequencer;
	...
	if (bSelectMarkers)
	{
		SelectMarkersInRange(SelectionRange);
	}
}

void FSequencer::SelectMarkersInRange(const TRange<FFrameNumber>& Range)
{
	using namespace UE::Sequencer;
	...
	for (int32 Index = 0; Index < MovieScene->GetMarkedFrames().Num(); ++Index)
	{
		if (Range.Contains(MovieScene->GetMarkedFrames()[Index].FrameNumber))
		{
			Selection->MarkedFrames.Select(Index);
		}
	}
}
	...
	if (ensure(Range.HasLowerBound() && Range.HasUpperBound()))
	{
		if (!IsPlaybackRangeLocked())
		{
			...
			if (FocusedMovieScene)
			{
				...
				if (!bIsDraggingPlaybackRange)
				{
					Transaction.Emplace(LOCTEXT("SetPlaybackRange_Transaction", "Set Playback Range"));
				}
				else if (!bPlaybackRangeChangedDuringDrag)
				{
					FocusedMovieScene->Modify();
				}

				FocusedMovieScene->SetPlaybackRange(Range, !bIsDraggingPlaybackRange);
				...
				if (ActiveTemplateIDs.Num() > 1)
				{
					if (UMovieSceneSubSection* SubSection = FindSubSection(ActiveTemplateIDs.Last()))
					{
						...
						if (!bIsDraggingPlaybackRange || !bPlaybackRangeChangedDuringDrag)
						{
							SubSection->Modify();
						}
				...
				if (bIsDraggingPlaybackRange)
				{
					bPlaybackRangeChangedDuringDrag = true;
				}
				else
				{
					NotifyMovieSceneDataChanged(EMovieSceneDataChangeType::TrackValueChanged);
...
void FSequencer::SnapSequencerTime(FFrameTime& ScrubTime)
{
	...
	TRange<FFrameNumber> PlayheadScrubbingRange = GetSequencerSettings()->GetPlayheadScrubbingRange(TWeakPtr<ISequencer>(AsShared()));
...
void FSequencer::UpdateAutoScroll(double NewTime, float ThresholdPercentage)
{
	...
	if (!AutoscrubOffset.IsSet())
	{
		...
		{
			SetLocalTimeLooped( (ViewRange.GetLowerBoundValue() + Threshold) * LocalTime.Rate, CurrentTimeBreadcrumbs, bForwards);
		}
		else if (AutoscrollOffset.GetValue() > 0 && LocalTime.AsSeconds() < ViewRange.GetUpperBoundValue() - Threshold)
		{
			SetLocalTimeLooped( (ViewRange.GetUpperBoundValue() - Threshold) * LocalTime.Rate, CurrentTimeBreadcrumbs, bForwards);
		}
	}
...
ISequencer::FOnPreNavigateToSequence& FSequencer::OnPreNavigateToSequence()
{
	return OnPreNavigateToSequenceEvent;
}
...
void FSequencer::DestroySequencerWidget()
{
	const TWeakPtr<SSequencer> WeakSequencerWidget = SequencerWidget.ToWeakPtr();
	...
	if (SequencerWrapperWidget)
	{
		SequencerWrapperWidget->DestroySequencerWidget();
		SequencerWrapperWidget.Reset();
	}
	
	// Last reference to SSequencer should have been cleared now.
	ensure(!WeakSequencerWidget.IsValid());
}
...
TSharedRef<STemporarilyFocusedSpinBox<double>> FSequencer::MakePlayTimeDisplay()
{
...
UObject* FSequencer::FindSpawnedObjectOrTemplate(const FGuid& BindingId)
{
	...
	// If we're a possessable that references a spawnable in another sequence via SpawnableObjectBindingID,
	// resolve the spawnable's template from that sequence (needed for operations like Bake Animation Sequence
	// when evaluating in isolation mode where the referenced spawnable isn't actually spawned)
	else if (Possessable && Possessable->GetSpawnableObjectBindingID().IsValid())
	{
		const FMovieSceneObjectBindingID& SpawnableBindingID = Possessable->GetSpawnableObjectBindingID();
		FMovieSceneSequenceID ResolvedSequenceID = SpawnableBindingID.ResolveSequenceID(GetFocusedTemplateID(), GetSharedPlaybackState());
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		UMovieSceneSequence* ResolvedSequence = State.FindSequence(ResolvedSequenceID);			
		PRAGMA_ENABLE_DEPRECATION_WARNINGS

		if (ResolvedSequence)
		{
			if (UObject* Template = MovieSceneHelpers::GetObjectTemplate(ResolvedSequence, SpawnableBindingID.GetGuid(), GetSharedPlaybackState()))
			{
				return Template;
			}
		}
...
FReply FSequencer::OnPlay(bool bTogglePlay)
{
	...
	{
		TRange<FFrameNumber> LocalTimeBounds = GetTimeBounds();

		FMovieSceneInverseSequenceTransform LocalToRootTransform = RootToUnwarpedLocalTransform.Inverse();

		TOptional<FFrameTime> MinInclusiveTime = LocalToRootTransform.TryTransformTime(UE::MovieScene::DiscreteInclusiveLower(LocalTimeBounds));
		TOptional<FFrameTime> MaxInclusiveTime = LocalToRootTransform.TryTransformTime(UE::MovieScene::DiscreteExclusiveUpper(LocalTimeBounds) - 1);

		FFrameTime GlobalTime = GlobalPlaybackWarpTransform.TransformTime(GetGlobalTime().Time);

		if (PlaybackSpeed > 0)
		{
			if (MinInclusiveTime.IsSet())
			{
				if (GlobalTime <= MinInclusiveTime.GetValue())
				{
					SetGlobalTime(MinInclusiveTime.GetValue());
				}
				else if (MaxInclusiveTime.IsSet() && GlobalTime >= MaxInclusiveTime.GetValue())
				{
					SetGlobalTime(MinInclusiveTime.GetValue());
				}
			}
		...
		{
			if (MaxInclusiveTime.IsSet())
			{
				if (GlobalTime >= MaxInclusiveTime.GetValue())
				{
					SetGlobalTime(MaxInclusiveTime.GetValue());
				}
				else if (MinInclusiveTime.IsSet() && GlobalTime <= MinInclusiveTime.GetValue())
				{
					SetGlobalTime(MaxInclusiveTime.GetValue());
				}
			}
...
FReply FSequencer::OnStepForward(FFrameNumber Increment)
{
	...
	if (GetLoopMode() == ESequencerLoopMode::SLM_Loop || GetLoopMode() == ESequencerLoopMode::SLM_LoopSelectionRange)
	{
		...
		SetLocalTimeLooped(NewPosition, CurrentTimeBreadcrumbs, bForwards);
	}
	else
	{
...
FReply FSequencer::JumpToPreviousKey()
{
	...
	if (SelectedKeyCollection.IsValid())
	{
		...
		const bool bAllowLoop = GetLoopMode() != SLM_NoLoop;
		TOptional<FFrameNumber> NewTime = SelectedKeyCollection->GetNextKey(FrameNumber, EFindKeyDirection::Backwards, Range, EFindKeyType::FKT_All, bAllowLoop);
		if (NewTime.IsSet())
		{
			SetPlaybackStatus(EMovieScenePlayerStatus::Stepping);
...
FReply FSequencer::JumpToNextKey()
{
	...
	if (SelectedKeyCollection.IsValid())
	{
		...
		FFrameNumber FrameNumber = GetLocalTime().Time.FloorToFrame();
		const bool bAllowLoop = GetLoopMode() != SLM_NoLoop;
		TOptional<FFrameNumber> NewTime = SelectedKeyCollection->GetNextKey(FrameNumber, EFindKeyDirection::Forwards, Range, EFindKeyType::FKT_All, bAllowLoop);
		if (NewTime.IsSet())
		{
			SetPlaybackStatus(EMovieScenePlayerStatus::Stepping);
...
void FSequencer::SetLocalTimeLooped(FFrameTime NewLocalTime, const FMovieSceneTransformBreadcrumbs& Breadcrumbs, bool bForwards)
{
	using namespace UE::MovieScene;
	using namespace UE::Sequencer;
	...
	if (PauseOnFrame.IsSet() && ((bForwards && NewLocalTime > PauseOnFrame.GetValue()) || (!bForwards && NewLocalTime < PauseOnFrame.GetValue())))
	{
	...
	else if (GetLoopMode() == ESequencerLoopMode::SLM_Loop || GetLoopMode() == ESequencerLoopMode::SLM_LoopSelectionRange)
	{
		...
		{
			NewGlobalTime = LocalToRootTransform.TryTransformTime((bForwards ? MinInclusiveTime : MaxInclusiveTime), BreadcrumbsToUse);
			if (NewGlobalTime)
			{
				bResetPosition = true;
				bHasJumped = true;
	...
	else
	{
		...
		if (NewGlobalTime)
		{
			...
			if (bReachedEnd)
			{
				NewGlobalTime = GlobalPlaybackWarpTransform.Inverse().TryTransformTime(bForwards ? MaxInclusiveTime : MinInclusiveTime);
				NewPlaybackStatus = EMovieScenePlayerStatus::Stopped;
			}
		}
	}
...
FFrameNumber FSequencer::OnGetNearestKey(FFrameTime InTime, ENearestKeyOption NearestKeyOption)
{
	...
	if (EnumHasAnyFlags(NearestKeyOption, ENearestKeyOption::NKO_SearchMarkers))
	{
		...
		{
			Markers.Add(GlobalMarkedFrame.FrameNumber);
		}

		if (UMovieSceneSequence* Sequence = GetFocusedMovieSceneSequence())
		{
			const bool bInsetDisplayFrame = ScrubStyle == ESequencerScrubberStyle::FrameBlock && Settings->GetForceWholeFrames();
			FFrameRate LocalResolution = GetFocusedTickResolution();
			FFrameRate DisplayRate = GetFocusedDisplayRate();
			...
			Markers.Add(UE::MovieScene::DiscreteInclusiveLower(PlaybackRange));
			Markers.Add(UE::MovieScene::DiscreteExclusiveUpper(PlaybackRange) - OneFrame.FrameNumber);

			TRange<FFrameNumber> SelectionRange = Sequence->GetMovieScene()->GetSelectionRange();
			Markers.Add(UE::MovieScene::DiscreteInclusiveLower(SelectionRange));
			Markers.Add(UE::MovieScene::DiscreteExclusiveUpper(SelectionRange) - OneFrame.FrameNumber);
		}

		for (const FFrameNumber& FrameNumber : Markers)
		{
			if (!NearestTime.IsSet())
			{
				NearestTime = FrameNumber;
			}
			else if (FMath::Abs(FrameNumber - CurrentTime) < FMath::Abs(NearestTime.GetValue() - CurrentTime))
			{
				NearestTime = FrameNumber;
			}
		}
...
{
	bIsDraggingPlaybackRange = true;
	bPlaybackRangeChangedDuringDrag = false;
...
void FSequencer::OnPlaybackRangeEndDrag()
{
	...
	bIsDraggingPlaybackRange = false;

	if (bPlaybackRangeChangedDuringDrag)
	{
		bPlaybackRangeChangedDuringDrag = false;
...
bool FSequencer::OnRequestNodeDeleted( TSharedRef<FViewModel> NodeToBeDeleted, const bool bKeepState )
{
	...
	{
		if (!Deletable->CanDelete(nullptr))
		{
			return false;
		}
...
{
	if (UE::Sequencer::bEvaluateOnUndo)
	{
		NotifyMovieSceneDataChanged(EMovieSceneDataChangeType::Unknown);
	}
	SynchronizeSequencerSelectionWithExternalSelection();
	...
	if (bAddSpawnable || bAddPossessable || bAddReplaceable)
	{
		...
		for ( AActor* Actor : DroppedActors )
		{
			if (AActor* NewActor = Actor)
			{
				...
				OnActorAddedToSequencerEvent.Broadcast(NewActor, NewGuid);
...
void FSequencer::HandleSelectedOutlinerNodesChanged()
{
	...
	for (TViewModelPtr<ITrackRowExtension> TrackRowModel : ViewModel->GetSelection()->Outliner.Filter<ITrackRowExtension>())
	{
		if (UMovieSceneTrack* Track = TrackRowModel->GetParentTrack())
		{
			SelectedTracks.Add(Track);
		}
	}
...
{
	GetFilterBar()->SetTextFilterString(FString());
	GetFilterBar()->EnableFilters(false);
	GetSequencerSettings()->SetShowSelectedNodesOnly(false);

	UMovieSceneSequence* FocusedMovieSequence = GetFocusedMovieSceneSequence();
	UMovieScene* FocusedMovieScene = FocusedMovieSequence ? FocusedMovieSequence->GetMovieScene() : nullptr;
	if (FocusedMovieScene)
	{
...
void FSequencer::SelectNodesByPath(const TSet<FString>& NodePaths)
{
	...
	if (!bAllAlreadySelected || (NodesToSelect.Num() != CurrentSelection.Num()))
	{
		...
		TSharedPtr<SOutlinerView> TreeView = SequencerWidget->GetTreeView();
		if (const TWeakViewModelPtr<IOutlinerExtension>* Node = Selection->Outliner.GetSelected().FindArbitraryElement())
		{
			for (TViewModelPtr<IOutlinerExtension> Parent : Node->Pin().AsModel()->GetAncestorsOfType<IOutlinerExtension>())
			{
				TreeView->SetItemExpansion(Parent, true);
					TreeView->SetItemExpansion(Parent, true);
					Parent = Parent.AsModel()->FindAncestorOfType<IOutlinerExtension>();
...
void FSequencer::ZoomToFitSelectionRange()
{
	FFrameRate TickResolution = GetFocusedTickResolution();

	// Use the movie scene selection range if set, otherwise fall back to time bounds
	TRange<FFrameNumber> BoundsHull = TRange<FFrameNumber>::Empty();
	if (const UMovieSceneSequence* FocusedSequence = GetFocusedMovieSceneSequence())
	{
		if (const UMovieScene* MovieScene = FocusedSequence->GetMovieScene())
		{
			BoundsHull = MovieScene->GetSelectionRange();
		}
	}
	...
	if (!BoundsHull.IsEmpty() && !BoundsHull.IsDegenerate())
	{
		...
		if (!ViewRangeBeforeZoom.IsEmpty() &&
			FMath::IsNearlyEqual(BoundsHull.GetLowerBoundValue() / TickResolution,
			                      GetViewRange().GetLowerBoundValue(), Tolerance) &&
			FMath::IsNearlyEqual(BoundsHull.GetUpperBoundValue() / TickResolution,
			                      GetViewRange().GetUpperBoundValue(), Tolerance))
		{
			SetViewRange(ViewRangeBeforeZoom, EViewRangeInterpolation::Animated);
		}
		else
...
void FSequencer::GetSelectedKeyAreas(TArray<const IKeyArea*>& OutSelectedKeyAreas, bool bIncludeSelectedKeys)
{
	...
	TArray<TSharedRef<FViewModel>> NodesToKey;
	{
		...
		for (FViewModelPtr Node : ViewModel->GetSelection()->Outliner)
		{
			...
			if (Node->IsA<IObjectBindingExtension>() || Node->IsA<ITrackExtension>() || Node->IsA<ITrackRowExtension>() || Node->IsA<FFolderModel>())
			{
				continue;
			}
	...
	for (TSharedRef<FViewModel> Node : NodesToKey)
	{
		...
		if (!Node->IsA<ITrackExtension>() && !Node->IsA<ITrackRowExtension>() && !Node->IsA<IObjectBindingExtension>() && !Node->IsA<FFolderModel>())
		{
...
void FSequencer::SelectByNthCategoryNode(UMovieSceneSection* Section, int Index, bool bSelect)
{
	...
	if (SectionHandle)
	{
		TSharedPtr<FViewModel> TrackNode = SectionHandle->GetParentSectionOwnerModel();
	...
	if (bSelect)
	{
		if (Settings->GetAutoExpandNodesOnSelection())
		{
			for (FViewModelPtr DisplayNode : NodesToSelect)
			{
				...
				TViewModelPtr<ITrackExtension> ParentTrack = Parent.ImplicitCast();
				TViewModelPtr<ITrackRowExtension> ParentTrackRow = Parent.ImplicitCast();
				if (ParentTrack || ParentTrackRow)
				{
...
void FSequencer::SelectByChannels(UMovieSceneSection* Section, TArrayView<const FMovieSceneChannelHandle> InChannels, bool bSelectParentInstead, bool bSelect)
{
	...
	const TSharedPtr<FViewModel>& TrackNode = SectionHandle->GetParentSectionOwnerModel();
	auto FindChannelGroupModel = [TrackNode](const FMovieSceneChannelHandle& Handle) -> TSharedPtr<FChannelGroupModel>
	{
		for (TSharedPtr<FChannelGroupModel> KeyAreaNode : TrackNode->GetDescendantsOfType<FChannelGroupModel>())
		{
			const TArrayView<const TWeakViewModelPtr<FChannelModel>>& Channels = KeyAreaNode->GetChannels();
			const bool bContainsHandle = Channels.ContainsByPredicate([&Handle](const TWeakViewModelPtr<FChannelModel>& WeakChannel)
			{
				if (const TSharedPtr<FChannelModel>& Channel = WeakChannel.Pin())
				{
					const TSharedPtr<IKeyArea>& KeyArea = Channel->GetKeyArea();
					return KeyArea && KeyArea->GetChannel() == Handle;
				}
				return false;
			});
	...
	TSet<TViewModelPtr<IOutlinerExtension>> NodesToUnselect, NodesToSelect;
	TViewModelPtr<IOutlinerExtension> FirstSelectedNode = nullptr;
		
	for (const FMovieSceneChannelHandle& Channel: InChannels)
	{
		if (const TSharedPtr<FChannelGroupModel>& ChannelGroupModel = FindChannelGroupModel(Channel))
		{
			if (bSelectParentInstead || !bSelect)
			{
				if (TViewModelPtr<IOutlinerExtension> OutlinerNode = ChannelGroupModel->FindAncestorOfType<IOutlinerExtension>())
				{
					if (bSelect)
					{
						...
						{
							FirstSelectedNode = OutlinerNode;
						}
					}
					else
					{
						NodesToUnselect.Add(OutlinerNode);
					}
				}
			}
			...
			if (!bSelectParentInstead || !bSelect)
			{
				...
				TViewModelPtr<IOutlinerExtension> OutlinerNode = CastViewModel<IOutlinerExtension>(ChannelGroupModel);
				if (!OutlinerNode)
				{
					OutlinerNode = ChannelGroupModel->FindAncestorOfType<IOutlinerExtension>();
				}
	...
	if (bSelect)
	{
		if (NodesToSelect.Num() > 0)
		{
			...
			SequencerWidget->GetTreeView()->RequestScrollIntoView(CastViewModel<IOutlinerExtension>(FirstSelectedNode));
			Selection->Outliner.SelectRange(NodesToSelect);
		}
	}
	else if (NodesToUnselect.Num() > 0)
	{
		for (const TViewModelPtr<IOutlinerExtension>& DisplayNode : NodesToUnselect)
		{
			Selection->Outliner.Deselect(DisplayNode);
		}
	}
	...
	if (bSelect)
	{
		for (const TViewModelPtr<IOutlinerExtension>& DisplayNode : Nodes)
		{
			if (Settings->GetAutoExpandNodesOnSelection())
			{
				...
				if (Parent && (Parent.AsModel()->IsA<ITrackExtension>() || Parent.AsModel()->IsA<ITrackRowExtension>()) && !Parent->IsExpanded())
				{
...
void FSequencer::SelectTrack(UMovieSceneTrack* Track)
{
	...
	TViewModelPtr<ITrackExtension> TrackExtension = TrackStorage->FindTrackExtensionForTrack(Track);
	if (TrackExtension)
	{
		ViewModel->GetSelection()->Outliner.Select(TrackExtension.ImplicitCast());
	}
}
...
TArray<TSharedRef<UE::Sequencer::FViewModel> > FSequencer::GetSelectedNodesInFolders()
{
	...
	for (FViewModelPtr SelectedNode : GetSelection().Outliner)
	{
		...
		if (Folder.IsValid())
		{
			...
			else if (ITrackRowExtension* TrackRowNode = SelectedNode->CastThis<ITrackRowExtension>())
			{
				if (TrackRowNode->GetParentTrack())
				{
					if (Folder->GetFolder()->GetChildTracks().Contains(TrackRowNode->GetParentTrack()))
					{
						NodesToFolders.Add(SelectedNode.AsModel().ToSharedRef());
					}
				}
...
void FSequencer::RemoveSelectedNodesFromFolders()
{
	...
	for (TSharedRef<FViewModel> NodeInFolder : NodesToFolders)
	{
		...
		if (Folder.IsValid())
		{
			...
			else if (ITrackRowExtension* TrackRowNode = NodeInFolder->CastThis<ITrackRowExtension>())
			{
				if (TrackRowNode->GetParentTrack())
				{
					Folder->GetFolder()->RemoveChildTrack(TrackRowNode->GetParentTrack());
				}
			}
		}
	}
...
void FSequencer::CopySelectedTracks(TArray<TSharedPtr<FViewModel>>& TrackNodes, const TArray<UMovieSceneFolder*>& Folders, FString& ExportedText)
{
	...
	for (TSharedPtr<FViewModel> TrackNode : TrackNodes)
	{
		...
		{
			// If this is a track row, only copy the sections that belong to this row. otherwise copying the entire track will copy all the sections across all the rows
			if (ITrackRowExtension* TrackRowExtension = TrackNode->CastThis<ITrackRowExtension>())
			{
				if (ISectionOwnerExtension* SectionOwner = TrackNode->CastThis<ISectionOwnerExtension>())
				{
					TArray<UMovieSceneSection*> Sections = SectionOwner->GetSections();
					CopyableSections.Append(Sections);
				}
			}
...
bool FSequencer::DoPaste(bool bClearSelection)
{
	...
	// Decorations and tracks coexist in mixed clipboards; run unconditionally so
	// neither path eats the other.
	if (EnumHasAnyFlags(PasteSupport, ESequencerPasteSupport::Decorations))
	{
		EPasteResult PasteDecorationsResult = PasteDecorations(TextToImport, PasteErrors);
		if (PasteDecorationsResult == EPasteResult::Cancelled)
		{
			return false;
		}

		bAnythingPasted |= PasteDecorationsResult == EPasteResult::Success;
	}

	// Sections still gate on no-other-paste because a decoration's text contains
	// nested section blocks that FSectionObjectTextFactory would otherwise lift
	// to top-level and double-paste.
	if (!bAnythingPasted)
	{
...
FSequencer::EPasteResult FSequencer::PasteSections(const FString& TextToImport, TArray<FNotificationInfo>& PasteErrors)
{
	...
	FMovieScenePasteSectionsParams PasteParams;
	PasteParams.Time = GetLocalTime().Time;
	for (TSharedRef<FViewModel> Node : SelectedNodes)
	{
		if (FDecorationOutlinerModel* DecoNode = Node->CastThis<FDecorationOutlinerModel>())
		{
			UMovieSceneDecorationContainerObject* Container = DecoNode->GetDecorationContainer().Get();
			UObject* Decoration = DecoNode->GetDecoration();
			if (Container || Decoration)
			{
				PasteParams.AddTarget(/*Track*/ nullptr, INDEX_NONE, Container, Decoration);
			}
			continue;
		}

		// Prefer the row interface so we get a row index for per-row container
		// lookup (a node may implement both, e.g. FAnimationMixerLayerModel).
		if (ITrackRowExtension* TrackRowNode = Node->CastThis<ITrackRowExtension>())
		{
			UMovieSceneTrack* TrackForNode = TrackRowNode->GetParentTrack();
			if (TrackForNode)
			{
				const int32 RowIndex = TrackRowNode->GetRowIndex();
				UMovieSceneDecorationContainerObject* Container = TrackForNode->GetDecorationContainerForRow(RowIndex);
				PasteParams.AddTarget(TrackForNode, RowIndex, Container);
			}
		}
		else if (ITrackExtension* TrackNode = Node->CastThis<ITrackExtension>())
		{
			UMovieSceneTrack* TrackForNode = TrackNode->GetTrack();
			if (TrackForNode)
			{
				UMovieSceneDecorationContainerObject* Container = TrackForNode->GetDecorationContainerForRow(INDEX_NONE);
				PasteParams.AddTarget(TrackForNode, INDEX_NONE, Container);
			}
		}
	}

	// Otherwise, look at all child nodes
	if (PasteParams.Tracks.Num() == 0 && PasteParams.DecorationTargets.Num() == 0)
	{
		for (TSharedRef<FViewModel> Node : SelectedNodes)
		{
			TSet<TSharedRef<FViewModel> > DescendantNodes;
			...
			{
				// Don't automatically paste onto row-level models because that would lead to multiple paste destinations
				if (ITrackRowExtension* DescendantTrackRowNode = DescendantNode->CastThis<ITrackRowExtension>())
				{
					continue;
				}
				...
				if (ITrackExtension* TrackNode = DescendantNode->CastThis<ITrackExtension>())
				{
					...
					if (TrackForNode)
					{
						// DescendantNode is guaranteed not to be an ITrackRowExtension
						// (filtered by the continue above), so no per-row context here.
						UMovieSceneDecorationContainerObject* Container = TrackForNode->GetDecorationContainerForRow(INDEX_NONE);
						PasteParams.AddTarget(TrackForNode, INDEX_NONE, Container);
					}
				}
	...
	if (!FSequencerUtilities::PasteSections(TextToImport, PasteParams, NewSections, PasteErrors))
	{
		return EPasteResult::Failure;
	}
...
FSequencer::EPasteResult FSequencer::PasteDecorations(const FString& TextToImport, TArray<FNotificationInfo>& PasteErrors)
{
	using namespace UE::Sequencer;
	...
	TArray<UMovieSceneDecorationContainerObject*> Containers;

	auto AddContainerUnique = [&Containers](UMovieSceneDecorationContainerObject* Container)
	{
		if (Container)
		{
	...
	for (FViewModelPtr SelectedNode : Selection->Outliner)
	{
		TSharedPtr<FViewModel> Node = SelectedNode.AsModel();
		if (!Node)
		{
			continue;
		}
		...
		if (FDecorationOutlinerModel* DecoNode = Node->CastThis<FDecorationOutlinerModel>())
		{
			AddContainerUnique(DecoNode->GetDecorationContainer().Get());
			continue;
		}

		if (FSectionOutlinerModel* SectionOutliner = Node->CastThis<FSectionOutlinerModel>())
		{
			AddContainerUnique(SectionOutliner->GetSection());
			continue;
		}

		// Prefer per-row container, fall back to the track itself.
		if (ITrackRowExtension* TrackRowNode = Node->CastThis<ITrackRowExtension>())
		{
			if (UMovieSceneTrack* TrackForNode = TrackRowNode->GetParentTrack())
			{
				if (UMovieSceneDecorationContainerObject* RowContainer = TrackForNode->GetDecorationContainerForRow(TrackRowNode->GetRowIndex()))
				{
					AddContainerUnique(RowContainer);
				}
				else
				{
					AddContainerUnique(TrackForNode);
				}
			}
			continue;
		}
		...
		{
			AddContainerUnique(TrackNode->GetTrack());
		}
	}

	if (Containers.Num() == 0)
	{
		FNotificationInfo Info(LOCTEXT("PasteDecorations_NoTargets", "Can't paste decoration. No selected target to paste onto."));
		PasteErrors.Add(Info);
		return EPasteResult::Failure;
	}

	TArray<UObject*> NewDecorations;
	if (!FSequencerUtilities::PasteDecorations(TextToImport, FMovieScenePasteDecorationsParams(Containers), NewDecorations, PasteErrors))
	{
		return EPasteResult::Failure;
	}
...
bool FSequencer::CanPaste(const FString& TextToImport)
{
	return (FSequencerUtilities::CanPasteBindings(AsShared(), TextToImport) ||
			...
			FSequencerUtilities::CanPasteDecorations(TextToImport) ||
...
void FSequencer::ConvertSelectedNodesToSpawnables()
{
	...
	for (IObjectBindingExtension* ObjectBindingNode : ObjectBindingNodes)
	{
		...
		if (Possessable)
		{
			TArray<FGuid> ConvertedGuids;

			if (IsLevelEditorSequencer())
			{
				if (FMovieScenePossessable* Spawnable = FSequencerUtilities::ConvertToCustomBinding(AsShared(), Possessable->GetGuid(), UMovieSceneSpawnableActorBinding::StaticClass()))
				{
					ConvertedGuids.Add(Spawnable->GetGuid());
				}
			}
			else
			{
			...
			for (FGuid Guid : ConvertedGuids)
			{
				for (TWeakObjectPtr<> WeakObject : FindBoundObjects(Guid, GetFocusedTemplateID()))
				{
...
void FSequencer::OnLoadRecordedData()
{
	...
	if (ModularFeatures.IsModularFeatureAvailable(ISerializedRecorder::ModularFeatureName))
	{
		...
		if (Recorder)
		{
			...
			{
				Recorder->LoadRecordedSequencerFile(FocusedMovieSceneSequence, World, FileName, OnReadComplete);
			}
		}
	}
...
void GatherTracksAndObjectsToCopy(TSharedRef<UE::Sequencer::FViewModel> Node, TArray<TSharedPtr<UE::Sequencer::FViewModel>>& TracksToCopy, TArray<TSharedPtr<UE::Sequencer::FObjectBindingModel>>& ObjectsToCopy, TArray<UMovieSceneFolder*>& FoldersToCopy, TArray<UObject*>& DecorationsToCopy)
{
	using namespace UE::Sequencer;

	// Decoration rows take precedence over track interfaces a model may also implement.
	if (FDecorationOutlinerModel* DecoNode = Node->CastThis<FDecorationOutlinerModel>())
	{
		if (UObject* Decoration = DecoNode->GetDecoration())
		{
			DecorationsToCopy.AddUnique(Decoration);
		}
	}
	else if (ITrackExtension* TrackNode = Node->CastThis<ITrackExtension>())
	{
	...
	else if (ITrackRowExtension* TrackRowNode = Node->CastThis<ITrackRowExtension>())
	{
		if (!TracksToCopy.Contains(Node))
		{
			TracksToCopy.Add(Node);
...
void FSequencer::CopySelection()
{
	...
	else
	{
		...
		TArray<UObject*> DecorationsToCopy;

		for (FViewModelPtr Node : ViewModel->GetSelection()->Outliner)
		{
			GatherTracksAndObjectsToCopy(Node.AsModel().ToSharedRef(), TracksToCopy, ObjectsToCopy, FoldersToCopy, DecorationsToCopy);
		}
		...
		FString DecorationsExportedText;
		...
		if (DecorationsToCopy.Num())
		{
			FSequencerUtilities::CopyDecorations(DecorationsToCopy, DecorationsExportedText);
			GClipboardStack.Empty();
		}
		...
		ExportedText += DecorationsExportedText;
...
void FSequencer::FixPossessableObjectClassInternal(UMovieSceneSequence* Sequence, FMovieSceneSequenceIDRef SequenceID)
{
	...
	for (int32 Index = 0; Index < MovieScene->GetPossessableCount(); ++Index)
	{
		...
		{
			UE_LOGF(LogSequencer, Display, "Updated possessed object class in: %ls for: %ls, from: %ls, to: %ls", *GetNameSafe(Sequence), *Possessable.GetName(), *GetNameSafe(Possessable.GetPossessedObjectClass()), *GetNameSafe(CommonBaseClass.GetValue()));
...
void FSequencer::TrimOrExtendSection(bool bTrimOrExtendLeft)
{
	...
	if (ViewModel->GetSelection()->Outliner.Num() > 0)
	{
		for (FViewModelPtr Node : ViewModel->GetSelection()->Outliner)
		{
			...
			{
				UMovieSceneTrack* Track = TrackRowNode->GetParentTrack();
				if (Track)
				{
					MovieSceneToolHelpers::TrimOrExtendSection(Track, TrackRowNode->GetRowIndex(), GetLocalTime(), bTrimOrExtendLeft, Settings->GetDeleteKeysWhenTrimming());
				}
			}
...
void FSequencer::BindCommands()
{
	...
	SequencerCommandBindings->MapAction(
		Commands.ToggleAutoExpandCurveEditorOnSelection,
		FExecuteAction::CreateLambda([this] { Settings->SetAutoExpandCurveEditorOnSelection(!Settings->GetAutoExpandCurveEditorOnSelection()); }),
		FCanExecuteAction::CreateLambda([] { return true; }),
		FIsActionChecked::CreateLambda([this] { return Settings->GetAutoExpandCurveEditorOnSelection(); }) );
	...
	SequencerCommandBindings->MapAction(
		...
		FExecuteAction::CreateSP( this, &FSequencer::TranslateSelection, true) );

	SequencerCommandBindings->MapAction(
		Commands.TranslateRight,
		FExecuteAction::CreateSP( this, &FSequencer::TranslateSelection, false) );
	...
	auto CanSelectedNodesBeConvertedToSpawnables = [this]{
		UMovieSceneSequence* Sequence = GetFocusedMovieSceneSequence();
	...
	SequencerCommandBindings->MapAction(
		...
		FCanExecuteAction::CreateLambda(CanSelectedNodesBeConvertedToSpawnables)
	);
	...
	SequencerCommandBindings->MapAction(
		Commands.ToggleShowTransportControl,
		FExecuteAction::CreateLambda( [this]{ SequencerWidget->ToggleTransportControlsVisible(); } ),
		FCanExecuteAction(),
		FIsActionChecked::CreateLambda( [this]{ return SequencerWidget->IsTransportControlsVisible(); } ) );

	SequencerCommandBindings->MapAction(
		Commands.ToggleTransportControlsPoppedOut,
		FExecuteAction::CreateLambda( [this]{ SequencerWidget->ToggleTransportControlsPoppedOut(); } ),
		FCanExecuteAction::CreateLambda( [this]{ return Settings->ShouldShowTransportControls(); } ),
		FIsActionChecked::CreateLambda( [this]{ return Settings->ShouldRestoreTransportControlsPoppedOut(); } ) );
	...
	SequencerCommandBindings->MapAction(
		Commands.ToggleShowBottomTimeSlider,
		FExecuteAction::CreateLambda( [this]{ Settings->SetShowBottomTimeSlider( !Settings->GetShowBottomTimeSlider() ); } ),
		FCanExecuteAction::CreateLambda( []{ return true; } ),
		FIsActionChecked::CreateLambda( [this]{ return Settings->GetShowBottomTimeSlider(); } ) );
	...
	auto CanCutOrCopy = [this]{
		...
		if (SelectedNodes.Num() == 0)
		{
			...
			for (TWeakViewModelPtr<IOutlinerExtension> WeakNode : SelectedNodes)
			{
				...
				else if (Node->IsA<FDecorationOutlinerModel>())
				{
					return true;
				}
				else if (Node->GetParent().IsValid() && (Node->GetParent()->IsA<ITrackExtension>() || Node->GetParent()->IsA<ITrackRowExtension>()) && !Node->IsA<FCategoryGroupModel>())
				{
					return true;
				}
			}
	...
	auto CanDuplicate = [this]{
		...
		{
			// if contains one node that can be copied we allow the action
			return Selection->Outliner.Filter<IObjectBindingExtension>().SelectionSet->Num() > 0;
		}
		return false;
	};
	...
	SequencerCommandBindings->MapAction(
		Commands.SetPlayheadScrubbingLimitNone,
		FExecuteAction::CreateLambda([this] { Settings->SetPlayheadScrubbingLimit(ESequencerPlayheadScrubbingLimit::SPSL_None); }),
		FCanExecuteAction::CreateLambda([] { return true; }),
		FIsActionChecked::CreateLambda([this] { return Settings->GetPlayheadScrubbingLimit() == ESequencerPlayheadScrubbingLimit::SPSL_None; }));
	
	SequencerCommandBindings->MapAction(
		Commands.SetPlayheadScrubbingLimitPlaybackRange,
		FExecuteAction::CreateLambda([this] { Settings->SetPlayheadScrubbingLimit(ESequencerPlayheadScrubbingLimit::SPSL_PlaybackRange); }),
		FCanExecuteAction::CreateLambda([] { return true; }),
		FIsActionChecked::CreateLambda([this] { return Settings->GetPlayheadScrubbingLimit() == ESequencerPlayheadScrubbingLimit::SPSL_PlaybackRange; }));

	SequencerCommandBindings->MapAction(
		Commands.SetPlayheadScrubbingLimitSelectionRange,
		FExecuteAction::CreateLambda([this] { Settings->SetPlayheadScrubbingLimit(ESequencerPlayheadScrubbingLimit::SPSL_SelectionRange); }),
		FCanExecuteAction::CreateLambda([] { return true; }),
		FIsActionChecked::CreateLambda([this] { return Settings->GetPlayheadScrubbingLimit() == ESequencerPlayheadScrubbingLimit::SPSL_SelectionRange; }));
	...
	SequencerCommandBindings->MapAction(
		...
		FCanExecuteAction::CreateSP(this, &FSequencer::CanAddTransformKeysForSelectedObjects, EMovieSceneTransformChannel::All));
	SequencerCommandBindings->MapAction(
		...
		FCanExecuteAction::CreateSP(this, &FSequencer::CanAddTransformKeysForSelectedObjects, EMovieSceneTransformChannel::Translation));
	SequencerCommandBindings->MapAction(
		...
		FCanExecuteAction::CreateSP(this, &FSequencer::CanAddTransformKeysForSelectedObjects, EMovieSceneTransformChannel::Rotation));
	SequencerCommandBindings->MapAction(
		...
		FCanExecuteAction::CreateSP(this, &FSequencer::CanAddTransformKeysForSelectedObjects, EMovieSceneTransformChannel::Scale));
	SequencerCommandBindings->MapAction(
		Commands.DeleteTransformKey,
		FExecuteAction::CreateSP(this, &FSequencer::OnDeleteTransformKeysForSelectedObjects, EMovieSceneTransformChannel::All),
		FCanExecuteAction::CreateSP(this, &FSequencer::CanDeleteTransformKeysForSelectedObjects, EMovieSceneTransformChannel::All));
	SequencerCommandBindings->MapAction(
		Commands.DeleteTranslationKey,
		FExecuteAction::CreateSP(this, &FSequencer::OnDeleteTransformKeysForSelectedObjects, EMovieSceneTransformChannel::Translation),
		FCanExecuteAction::CreateSP(this, &FSequencer::CanDeleteTransformKeysForSelectedObjects, EMovieSceneTransformChannel::Translation));
	SequencerCommandBindings->MapAction(
		Commands.DeleteRotationKey,
		FExecuteAction::CreateSP(this, &FSequencer::OnDeleteTransformKeysForSelectedObjects, EMovieSceneTransformChannel::Rotation),
		FCanExecuteAction::CreateSP(this, &FSequencer::CanDeleteTransformKeysForSelectedObjects, EMovieSceneTransformChannel::Rotation));
	SequencerCommandBindings->MapAction(
		Commands.DeleteScaleKey,
		FExecuteAction::CreateSP(this, &FSequencer::OnDeleteTransformKeysForSelectedObjects, EMovieSceneTransformChannel::Scale),
		FCanExecuteAction::CreateSP(this, &FSequencer::CanDeleteTransformKeysForSelectedObjects, EMovieSceneTransformChannel::Scale));
	...
	SequencerCommandBindings->MapAction(
		Commands.ToggleSimpleView,
		FExecuteAction::CreateSP(this, &FSequencer::ToggleSimpleView));
	...
	*SequencerSharedBindings = *SequencerCommandBindings;
	...
	SequencerCommandBindings->MapAction(
		...
		FExecuteAction::CreateSP(this, &FSequencer::SelectInSelectionRange, true, false, false),
		FCanExecuteAction::CreateLambda(IsSelectionRangeNonEmpty));

	SequencerCommandBindings->MapAction(
		Commands.SelectSectionsInSelectionRange,
		FExecuteAction::CreateSP(this, &FSequencer::SelectInSelectionRange, false, true, false),
		FCanExecuteAction::CreateLambda(IsSelectionRangeNonEmpty));

	SequencerCommandBindings->MapAction(
		Commands.SelectMarkersInSelectionRange,
		FExecuteAction::CreateSP(this, &FSequencer::SelectInSelectionRange, false, false, true),
		FCanExecuteAction::CreateLambda(IsSelectionRangeNonEmpty));

	SequencerCommandBindings->MapAction(
		Commands.SelectAllInSelectionRange,
		FExecuteAction::CreateSP(this, &FSequencer::SelectInSelectionRange, true, true, true),
		FCanExecuteAction::CreateLambda(IsSelectionRangeNonEmpty));
	...
	SequencerCommandBindings->MapAction(
		Commands.ZoomToFitSelectionRange,
		FExecuteAction::CreateSP( this, &FSequencer::ZoomToFitSelectionRange) );
	...
	if (HostCapabilities.bSupportsSidebar)
	{
		...
		SequencerCommandBindings->MapAction(
			Commands.ToggleSelectionDetailsPoppedOut,
			FExecuteAction::CreateSP(this, &FSequencer::ToggleSelectionDetailsPoppedOut),
			FCanExecuteAction(),
			FIsActionChecked::CreateSP(this, &FSequencer::IsSelectionDetailsPoppedOut));
	}
	...
	ViewModel->GetOutlinerFilterArea()->BindCommands(SequencerCommandBindings);
	
	// If this sequencer supports a curve editor, let's add bindings for it. 
	// As FOutlinerFilterAreaManager::BindCommands binds the shared filter bar commands, which the curve editor also binds to, this must happen after.
	if (FSequencerCurveEditorApp* CurveEditorIntegration = FSequencerCurveEditorApp::Get(*this))
	{
		...
		CurveEditorIntegration->BindCommands();
	}
}
...
bool FSequencer::IsSimpleView() const
{
	return SequencerWidget.IsValid() ? SequencerWidget->IsSimpleView() : false;
}

void FSequencer::EnableSimpleView(const bool bInEnable)
{
	if (SequencerWidget.IsValid())
	{
		SequencerWidget->EnableSimpleView(bInEnable);
	}
}

void FSequencer::ToggleSimpleView()
{
	if (SequencerWidget.IsValid())
	{
		SequencerWidget->ToggleSimpleView();
	}
}
...
void FSequencer::ToggleSidebar()
{
	if (SequencerWidget.IsValid())
	{
		SequencerWidget->ToggleSidebarVisible();
	}
}
...
void FSequencer::ToggleSelectionDetailsPoppedOut()
{
	if (SequencerWidget.IsValid())
	{
		SequencerWidget->ToggleSelectionDetailsPoppedOut();
	}
}
...
bool FSequencer::IsSelectionDetailsPoppedOut() const
{
	return SequencerWidget.IsValid() ? SequencerWidget->IsSelectionDetailsPoppedOut() : false;
}

void FSequencer::SetSelectionDetailsPoppedOut(const bool bInPoppedOut)
{
	if (SequencerWidget.IsValid())
	{
		SequencerWidget->SetSelectionDetailsPoppedOut(bInPoppedOut);
	}
}
...
{
	return ViewModel->GetLinkedFilterViewModelImpl()->GetActiveFilterBarImpl();
}
...
void FSequencer::ToggleTrackSelectionLock()
{
	...
	const FScopedTransaction Transaction(NSLOCTEXT("Sequencer", "ToggleLock", "Toggle Lock"));
...
void FSequencer::ToggleTrackSelectionDeactive()
{
	...
	const FScopedTransaction Transaction(NSLOCTEXT("Sequencer", "ToggleDeactivate", "Toggle Deactivate"));
