...
void SSequencerLayerBar::Construct(const FArguments& InArgs, TWeakPtr<STrackAreaView> InWeakTrackArea, TWeakPtr<FSequencerEditorViewModel> InWeakEditor, TWeakPtr<FLayerBarModel> InWeakLayerBar)
{
	...
	WeakTrackArea  = InWeakTrackArea;
	OverlayText    = InArgs._OverlayText;
	...
	[
		...
		// Overlay text (always present, visible when text is non-empty)
		+ SOverlay::Slot()
		.HAlign(HAlign_Left)
		...
		// Stretch handles (visible on hover)
		+ SOverlay::Slot()
		[
			SNew(SHorizontalBox)
...
{
	return !CurveEditor::InputState::IsScrubTimeCommandPressed(FSequencerCommands::Get().ScrubTimeViewport) && IsHovered()
