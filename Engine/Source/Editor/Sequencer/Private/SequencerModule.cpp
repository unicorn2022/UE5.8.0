...
#include "SimpleView/Menus/SequencerToolbarExtension.h"
...
public:
	...
	virtual TSharedRef<ISequencer> CreateSequencer(const FSequencerInitParams& InitParams) override
	{
		...
		Sequencer->InitSequencer(InitParams, ObjectChangeListener, TrackEditorDelegates, DecorationEditorDelegates, EditorObjectBindingDelegates, OutlinerColumnDelegates, OutlinerIndicatorDelegates);
	...
	virtual FDelegateHandle RegisterDecorationEditor(FOnCreateDecorationEditor InOnCreateDecorationEditor) override
	{
		DecorationEditorDelegates.Add(InOnCreateDecorationEditor);
		return DecorationEditorDelegates.Last().GetHandle();
	}

	virtual void UnRegisterDecorationEditor(FDelegateHandle InHandle) override
	{
		DecorationEditorDelegates.RemoveAll([=](const FOnCreateDecorationEditor& Delegate) { return Delegate.GetHandle() == InHandle; });
	}
	...
	void RegisterMenus()
	{
		...
		UE::Sequencer::SimpleView::FSequencerToolbarExtension::AddExtension();
	}
	...
	virtual void StartupModule() override
	{
		...
		if (GIsEditor)
		{
			...
			{
				FCoreDelegates::GetOnPostEngineInit().AddStatic(&FSequencerCommands::Register);
				FCoreDelegates::GetOnPostEngineInit().AddRaw(this, &FSequencerModule::RegisterMenus);
			}
		...
		FSequenceModel::CreateExtensionsEvent.AddLambda(
			[&](TSharedPtr<FEditorViewModel> InEditor, TSharedPtr<FSequenceModel> InModel)
			{
				...
				InModel->AddDynamicExtension(FDecorationModelStorageExtension::ID);
			}
		);
	...
	{
		UE::Sequencer::SimpleView::FSequencerToolbarExtension::RemoveExtension();
...
private:
	...
	/** List of decoration editor factory delegates sequencers will execute when they are created */
	TArray< FOnCreateDecorationEditor > DecorationEditorDelegates;
