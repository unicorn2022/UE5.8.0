...
FKeyHandle IKeyArea::AddOrUpdateKey(FFrameNumber Time, const FGuid& ObjectBindingID, ISequencer& InSequencer)
{
	...
	// Decoration-owned channels route through the decoration editor for keying.
	auto [DecorationEditor, Decoration] = InSequencer.FindDecorationOwner(ChannelHandle, Section);
	if (DecorationEditor)
	{
		return DecorationEditor->AddOrUpdateKey(*Decoration, *Section, ChannelHandle, Time, ObjectBindingID, InSequencer);
	}
	...
	// If track does not allow auto expanding sections, check if key time is within bounds. We include exclusive end frame to allow keyframes to visually align with section rendering
	if (!Track->bAutoExpandToKeyframes && (Time < Section->GetInclusiveStartFrame() || Time > Section->GetExclusiveEndFrame()))
	{
		return FKeyHandle();
	}
...
TSharedRef<SWidget> IKeyArea::CreateKeyEditor(TWeakPtr<ISequencer> Sequencer, const FGuid& ObjectBindingID)
{
	...
	{
		// Let decoration editors customize key editors for their channels
		UObject* OwningObject = MetaData->WeakOwningObject.Get();
		if (OwningObject && OwningObject != OwningSection)
		{
			if (TSharedPtr<ISequencer> SequencerPtr = Sequencer.Pin())
			{
				...
				{
					if (TSharedPtr<SWidget> CustomEditor = DecorationEditor->CreateKeyEditor(*OwningObject, *OwningSection, ChannelHandle, Sequencer, ObjectBindingID))
					{
						return CustomEditor.ToSharedRef();
					}
				}
			}
		}
