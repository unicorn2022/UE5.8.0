...
TSet<UMovieSceneTrack*> FSequencerSelection::GetSelectedTracks() const
{
	...
	for (TViewModelPtr<ITrackRowExtension> TrackRowExtension : Outliner.Filter<ITrackRowExtension>())
	{
		if (UMovieSceneTrack* Track = TrackRowExtension->GetParentTrack())
		{
			SelectedTracks.Add(Track);
		}
	}
...
TSet<TPair<UMovieSceneTrack*, int32>> FSequencerSelection::GetSelectedTrackRows() const
{
	...
	// Only add a 'track row' as selected if either we have a row-level model selected (FTrackRowModel or layer model),
	// or else we have a track selected and there's only a single track row, and the track allows multiple rows.
	for (TViewModelPtr<ITrackRowExtension> TrackRowExtension : Outliner.Filter<ITrackRowExtension>())
	{
		if (UMovieSceneTrack* Track = TrackRowExtension->GetParentTrack())
		{
			SelectedTrackRows.Add(TPair<UMovieSceneTrack*, int32>(Track, TrackRowExtension->GetRowIndex()));
		}
	}
	...
	for (TViewModelPtr<ITrackExtension> TrackExtension : Outliner.Filter<ITrackExtension>())
	{
		if (UMovieSceneTrack* Track = TrackExtension->GetTrack())
		{
			...
			{
				SelectedTrackRows.Add(TPair<UMovieSceneTrack*, int32>(Track, 0));
			}
		}
	}
