...
FReply SSequencerTrackAreaView::OnMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	const TSharedPtr<FTrackAreaViewModel> ViewModel = WeakViewModel.Pin();
	if (ViewModel.IsValid() && CurveEditor::InputState::IsScrubTimeCommandPressed(FSequencerCommands::Get().ScrubTimeViewport))
	{
		ViewModel->SetHotspot(nullptr);
		ViewModel->AttemptToActivateTool(FSequencerEditTool_Selection::Identifier);
	}

	return STrackAreaView::OnMouseButtonDown(MyGeometry, MouseEvent);
}
...
FReply SSequencerTrackAreaView::OnDragOver(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent)
{
	...
	{
		TSharedPtr<ITrackExtension> TrackDestination = DroppedItem.ImplicitCast();
		TSharedPtr<ITrackRowExtension> TrackRowDestination = DroppedItem.ImplicitCast();
		if (TrackDestination || TrackRowDestination)
		{
			UMovieSceneTrack* Track    = TrackDestination ? TrackDestination->GetTrack() : TrackRowDestination->GetParentTrack();
			const int32       TrackRow = TrackRowDestination ? TrackRowDestination->GetRowIndex() : 0;
...
FReply SSequencerTrackAreaView::OnDrop(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent)
{
	...
	if (Sequencer && DroppedItem)
	{
		...
		ITrackRowExtension* TrackRowDestination = DroppedItem->CastThis<ITrackRowExtension>();
		if (TrackDestination || TrackRowDestination)
		{
			UMovieSceneTrack* Track    = TrackDestination ? TrackDestination->GetTrack() : TrackRowDestination->GetParentTrack();
			const int32       TrackRow = TrackRowDestination ? TrackRowDestination->GetRowIndex() : 0;
