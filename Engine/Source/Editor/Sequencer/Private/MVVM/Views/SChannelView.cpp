...
struct FSequencerKeyRendererInterface : IKeyRendererInterface
{
	...
	FSequencerKeyRendererInterface(FSequencer* Sequencer, TSharedPtr<ITrackAreaHotspot> Hotspot)
	{
		...
		HoveredKeys      = (KeyHotspot.IsValid() && !CurveEditor::InputState::IsScrubTimeCommandPressed(FSequencerCommands::Get().ScrubTimeViewport))
			? &KeyHotspot->RawKeys : nullptr;
	}
...
FChannelViewKeyCachedState::FChannelViewKeyCachedState(TRange<FFrameTime> InVisibleRange, TSharedPtr<ITrackAreaHotspot> Hotspot, FViewModelPtr Model, FSequencer* Sequencer)
{
	...
	const TSharedPtr<FLinkedFilterViewModel> LinkedFilterViewModel = Sequencer->GetViewModelImpl()->GetLinkedFilterViewModelImpl();
	FilterSerial = LinkedFilterViewModel ? LinkedFilterViewModel->GetActiveFilterBarImpl()->GetFilterSerial() : 0;
...
EViewDependentCacheFlags FChannelViewKeyCachedState::CompareTo(const FChannelViewKeyCachedState& Other) const
{
	...
	if (FilterSerial != Other.FilterSerial)
	{
		// Filters have re-run since the last paint, so the set of filtered-in descendants the
		// key renderer collapses into this section may have changed. Force CacheKeyExtensions
		// to walk the descendants again.
...
FReply SChannelView::OnMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	if (!MouseEvent.GetModifierKeys().AnyModifiersDown() && MouseEvent.GetEffectingButton() == EKeys::MiddleMouseButton)
	{
		...
		bCreateKeyTransactionActive = true;
...
{
	// Drive the end of the transaction off the flag set on mouse down rather than re-checking the button/modifier state, which can diverge by the time the release arrives (e.g. user grabs a modifier mid-drag, or a different button's up event reaches us first) and would otherwise strand the transaction. See UE-381007.
	EndCreateKeyTransactionIfActive();
...
void SChannelView::EndCreateKeyTransactionIfActive()
{
	if (!bCreateKeyTransactionActive)
	{
		return;
	}

	bCreateKeyTransactionActive = false;
...
int32 SChannelView::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	...
	PaintArgs.KeySelectionFillColor = FAppStyle::GetSlateColor(SelectionColorName).GetColor(InWidgetStyle);
...
void SChannelView::CreateKeysUnderMouse(const FVector2D& MousePosition, const FGeometry& AllottedGeometry, const TSet<FSequencerSelectedKey>& InPressedKeys, TArray<FSequencerSelectedKey>& OutKeys)
{
	...
	{
		TViewModelPtr<ITrackExtension>      TrackModel             = Section->FindAncestorOfType<ITrackExtension>();
