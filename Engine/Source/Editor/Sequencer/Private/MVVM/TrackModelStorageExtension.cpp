...
TSharedPtr<FTrackModel> FTrackModelStorageExtension::CreateModelForTrack(UMovieSceneTrack* InTrack, TSharedPtr<FViewModel> DesiredParent)
{
	...
	TSharedPtr<FTrackModel> TrackModel = TrackToModel.FindRef(TrackAsKey).Pin().ImplicitCast();
	if (TrackModel)
	{
...
{
	FObjectKey TrackAsKey(InTrack);
	TWeakViewModelPtr<ITrackExtension> WeakModel = TrackToModel.FindRef(TrackAsKey);
	TViewModelPtr<ITrackExtension> Model = WeakModel.Pin();
	return Model.ImplicitCast();
}

TViewModelPtr<ITrackExtension> FTrackModelStorageExtension::FindTrackExtensionForTrack(UMovieSceneTrack* InTrack) const
{
	FObjectKey TrackAsKey(InTrack);
	return TrackToModel.FindRef(TrackAsKey).Pin();
}

void FTrackModelStorageExtension::RegisterTrackExtensionForTrack(UMovieSceneTrack* InTrack, TViewModelPtr<ITrackExtension> InModel)
{
	FObjectKey TrackAsKey(InTrack);
	TrackToModel.Add(TrackAsKey, InModel);
}
...
void FTrackModelStorageExtension::OnTrackRemoved(UMovieSceneTrack* InTrack)
{
	...
	if (TViewModelPtr<ITrackExtension> Model = TrackToModel.FindRef(TrackAsKey).Pin())
	{
		if (TSharedPtr<FViewModel> ViewModel = Model.AsModel())
		{
			ViewModel->RemoveFromParent();
		}
	}

	TrackToModel.Remove(TrackAsKey);
}
