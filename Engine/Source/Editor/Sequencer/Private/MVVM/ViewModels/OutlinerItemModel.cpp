...
FOutlinerItemModelMixin::FOnPinStateChanged FOutlinerItemModelMixin::PinStateChangedDelegate;
...
void FOutlinerItemModelMixin::SetPinned(const bool bInIsPinned)
{
	FPinnableExtensionShim::SetPinned(bInIsPinned);

	PinStateChangedDelegate.Broadcast(AsViewModel()->CastThisShared<IPinnableExtension>(), bInIsPinned);
}
...
ECheckBoxState FOutlinerItemModelMixin::SelectedModelsSoloState() const
{
	...
	int32 NumSoloed = 0;
	for (FViewModelPtr Soloable : Selection->Outliner.Filter<ISoloableExtension>())
	{
		++NumSoloables;
...
ECheckBoxState FOutlinerItemModelMixin::SelectedModelsMuteState() const
{
	...
	int32 NumMutables = 0;
	int32 NumMuted = 0;
	for (FViewModelPtr Mutable : Selection->Outliner.Filter<IMutableExtension>())
	{
		++NumMutables;
...
void FOutlinerItemModelMixin::BuildContextMenu(FMenuBuilder& MenuBuilder)
{
	...
	TSharedPtr<FSequencer> Sequencer = StaticCastSharedPtr<FSequencer>(EditorViewModel->GetSequencer());
	if (!Sequencer)
	{
		return;
	}
...
void FOutlinerItemModelMixin::BuildOrganizeContextMenu(FMenuBuilder& MenuBuilder)
{
	...
	const bool bFilterableNode = (ThisNode->IsA<ITrackExtension>() || ThisNode->IsA<ITrackRowExtension>() || ThisNode->IsA<IObjectBindingExtension>() || ThisNode->IsA<FFolderModel>());
	const bool bIsReadOnly = Sequencer->IsReadOnly();
	...
	for (const FViewModelPtr Node : EditorViewModel->GetSelection()->Outliner)
	{
		if (ITrackExtension* TrackExtension = Node->CastThis<ITrackExtension>())
		{
			...
			if (Track)
			{
				AllTracks.AddUnique(Track);
			}
		}

		if (ITrackRowExtension* TrackRowExtension = Node->CastThis<ITrackRowExtension>())
		{
...
TArray<UMovieSceneSection*> FOutlinerItemModelMixin::GetSelectedSections() const
{
	...
	if (Sections.IsEmpty())
	{
		for (const TViewModelPtr<ISectionOwnerExtension> SectionOwner : Selection->Outliner.Filter<ISectionOwnerExtension>())
		{
			for (UMovieSceneSection* const Section : SectionOwner->GetSections())
			{
				Sections.Add(Section);
			}
		}
...
TArray<TPair<UMovieSceneTrack*, int32>> FOutlinerItemModelMixin::GetSelectedTrackRows() const
{
	...
	for (const TViewModelPtr<ITrackRowExtension> TrackRowExtension : Selection->Outliner.Filter<ITrackRowExtension>())
	{
		UMovieSceneTrack* const Track = TrackRowExtension->GetParentTrack();
		if (IsValid(Track))
		{
			AllTrackRows.Add(TPair<UMovieSceneTrack*, int32>(Track, TrackRowExtension->GetRowIndex()));
		}
	}
...
{
	const TParentFirstChildIterator<ITrackExtension> TrackDescendants = GetDescendantsOfType<ITrackExtension>(true);
	const TParentFirstChildIterator<ITrackRowExtension> TrackRowDescendants = GetDescendantsOfType<ITrackRowExtension>(true);
	
	if (!TrackDescendants && !TrackRowDescendants)
	{
		return false;
	}
	...
	for (const TViewModelPtr<ITrackExtension>& TrackNode : TrackDescendants)
	{
	...
	for (const TViewModelPtr<ITrackRowExtension>& TrackRowNode : TrackRowDescendants)
	{
		const UMovieSceneTrack* const Track = TrackRowNode->GetParentTrack();
		if (!IsValid(Track))
		{
			continue;
		}

		const TViewModelPtr<ITrackAreaExtension> TrackAreaModel = TrackRowNode.ImplicitCast();
		if (!TrackAreaModel->GetTrackAreaModelList())
		{
			continue;
		}
		...
		if (!Track->IsRowEvalDisabled(TrackRowNode->GetRowIndex(), /*bInCheckLocal=*/false))
		{
			return false;
		}
	}
...
void FEvaluableOutlinerItemModel::SetIsDeactivated(const bool bInIsDeactivated)
{
	...
	for (const TViewModelPtr<ITrackRowExtension>& TrackRowNode : GetDescendantsOfType<ITrackRowExtension>(true))
	{
		...
		// Deactive state (dirtying, saved with asset, evaluation)
		if (bInIsDeactivated != Track->IsRowEvalDisabled(TrackRowNode->GetRowIndex(), /*bInCheckLocal=*/false))
		{
			Track->Modify();
			Track->SetRowEvalDisabled(bInIsDeactivated, TrackRowNode->GetRowIndex());
			bAnyChanged = true;
		}
	}
