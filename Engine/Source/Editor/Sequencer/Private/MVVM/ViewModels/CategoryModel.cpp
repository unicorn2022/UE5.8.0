...
TSharedPtr<FCategoryModel> FCategoryGroupModel::CreateTrackAreaCategory()
{
	TSharedPtr<FCategoryModel> Category = MakeShared<FCategoryModel>(CategoryName);
	AddCategory(Category);
	Category->SetLinkedOutlinerItem(SharedThis(this));
	return Category;
}
...
void FCategoryGroupModel::BuildSidebarMenu(FMenuBuilder& MenuBuilder)
{
	...
	if (const TViewModelPtr<ISectionOwnerExtension> SectionOwner = FindAncestorOfType<ISectionOwnerExtension>())
	{
		TArray<TWeakObjectPtr<>> WeakSectionObjects;
		Algo::Transform(SectionOwner->GetSections(), WeakSectionObjects, [](UMovieSceneSection* const InSection)
			{
				return InSection;
			});
