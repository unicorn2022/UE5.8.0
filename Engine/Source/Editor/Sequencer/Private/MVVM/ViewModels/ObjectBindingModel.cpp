...
void FObjectBindingModel::OnConstruct()
{
	...
	{
		// Don't show child tracks here- they'll be shown underneath their proper outliner parent (e.g. anim mixer)
		if (Track && Track->ParentTrack != nullptr)
		{
			continue;
		}
		AddTrack(Track);
...
TSharedRef<SWidget> FObjectBindingModel::GetAddTrackMenuContent()
{
	...
	UObject* BoundObject = GetFirstBoundObject(ObjectBindingID);
...
void FObjectBindingModel::BuildContextMenu(FMenuBuilder& MenuBuilder)
{
	...
	// Add other selected objects with the same class as the first one
	for (TViewModelPtr<FObjectBindingModel> ObjectBindingNode : OwnerModel->GetEditor()->GetSelection()->Outliner.Filter<FObjectBindingModel>())
	{
		FGuid Guid = ObjectBindingNode->GetObjectGuid();
		if (Guid != ObjectBindingID)
		{
			if (UObject* OtherBoundObject = GetFirstBoundObject(Guid))
			{
				if (ObjectClass == ObjectBindingNode->FindObjectClass())
				{
					ObjectBindings.Add(Guid);
				}
			}
		}
	}
...
UObject* FObjectBindingModel::GetFirstBoundObject(FGuid InGuid)
{
	TSharedPtr<ISequencer> Sequencer = OwnerModel->GetSequencer();
	if (!Sequencer)
	{
	...
	for (TWeakObjectPtr<UObject> RuntimeObject : Sequencer->FindObjectsInCurrentSequence(InGuid))
	{
