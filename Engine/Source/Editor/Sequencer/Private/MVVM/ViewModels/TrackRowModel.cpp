...
{
	UMovieSceneTrack* Track = GetParentTrack();
	...
	if (Track && SequenceModel)
	{
		...
		if (Sequencer)
		{
			TrackEditor = Sequencer->GetTrackEditor(Track);
			ensure(TrackEditor);
...
UMovieSceneTrack* FTrackRowModel::GetParentTrack() const
{
...
{
	UMovieSceneNameableTrack* NameableTrack = Cast<UMovieSceneNameableTrack>(GetParentTrack());
...
void FTrackRowModel::RefreshLayout(bool bChildrenNeedLayout)
{
	...
	RecycledModels = MakeUnique<FScopedViewModelListHead>(SharedThis(this), EViewModelListType::Recycled);
	FViewModelChildren RecycledRowModels = RecycledModels->GetChildren();

	GetSectionModels().MoveChildrenTo<IRecyclableExtension>(RecycledRowModels, IRecyclableExtension::CallOnRecycle);

	if (UMovieSceneTrack* Track = WeakTrack.Get())
	{
		for (UMovieSceneSection* Section : Track->GetAllSections())
		{
			if (Section && Section->GetRowIndex() == RowIndex)
			{
				...
				if (!SectionModel && TrackEditor)
				{
					...
					bChildrenNeedLayout = true;
				}
				else if (SectionModel)
				{
					bChildrenNeedLayout |= SectionModel->NeedsLayout();
				}

				if (SectionModel)
				{
					GetSectionModels().InsertChild(SectionModel, SectionsTail);
	...
	const bool bStillHasRecycledChildren = RecycledModels && !RecycledModels->GetChildren().IsEmpty();
	if (bChildrenNeedLayout || bStillHasRecycledChildren)
	{
		GetChildList(EViewModelListType::Outliner).MoveChildrenTo<IRecyclableExtension>(RecycledRowModels, IRecyclableExtension::CallOnRecycle);
		GetTopLevelChannels().MoveChildrenTo<IRecyclableExtension>(RecycledRowModels, IRecyclableExtension::CallOnRecycle);

		FTrackModelLayoutBuilder LayoutBuilder(AsShared());
		for (TSharedPtr<FSectionModel> Section : GetChildrenOfType<FSectionModel>(EViewModelListType::TrackArea))
		{
...
void FTrackRowModel::BuildContextMenu(FMenuBuilder& MenuBuilder)
{
	...
	UMovieSceneTrack* const Track = GetParentTrack();
	if (!IsValid(Track))
	{
		return;
	}
	...
	{
		SequencerHelpers::BuildNewSectionMenu(Sequencer, GetRowIndex() + 1, GetParentTrack(), MenuBuilder);
	}
...
bool FTrackRowModel::SetRowIndex(int32 NewRowIndex)
{
	...
	// Update all sections on this row to the new row index
...
bool FTrackRowModel::CanDrag() const
{
	// For now, regular track rows cannot be dragged in the outliner
	return false;
}
