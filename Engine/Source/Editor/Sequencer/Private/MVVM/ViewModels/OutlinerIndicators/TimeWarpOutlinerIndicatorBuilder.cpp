...
{
	if (TViewModelPtr<ISectionOwnerExtension> SectionOwner = InParams.OutlinerExtension.ImplicitCast())
	{
		for (UMovieSceneSection* Section : SectionOwner->GetSections())
		{
			...
			{
				if (Variant->GetType() == EMovieSceneTimeWarpType::Custom && Variant->AsCustom() != nullptr)
				{
					return true;
				}
			}
