...
bool FChannelModel::UpdateCachedKeys(TSharedPtr<FCachedKeys>& OutCachedKeys) const
{
	struct FSequencerCachedKeys : FCachedKeys
	{
		...
		bool Update(const FChannelModel* InChannel)
		{
			...
			// Check both the section signature and the owning object signature.
			// For decoration channels, the owning object (decoration) changes when
			// keys are modified, but the host section's signature may not update
			// since it's not in the decoration's outer chain.
			FGuid OwnerSignature;
			if (const TSharedPtr<IKeyArea>& ChannelKeyArea = InChannel->GetKeyArea())
			{
				if (const FMovieSceneChannelMetaData* MetaData = ChannelKeyArea->GetChannel().GetMetaData())
				{
					if (UMovieSceneSignedObject* SignedOwner = Cast<UMovieSceneSignedObject>(MetaData->WeakOwningObject.Get()))
					{
						if (SignedOwner != Section)
						{
							OwnerSignature = SignedOwner->GetSignature();
						}
					}
				}
			}

			const bool bSectionChanged = !Section || !CachedSignature.IsValid() || Section->GetSignature() != CachedSignature;
			const bool bOwnerChanged = OwnerSignature.IsValid() && OwnerSignature != CachedOwnerSignature;

			if (bSectionChanged || bOwnerChanged)
			{
				CachedSignature = Section && InChannel->GetKeyArea() && InChannel->GetKeyArea()->ResolveChannel() ? Section->GetSignature() : FGuid();
				CachedOwnerSignature = OwnerSignature;
...
TSharedPtr<SWidget> FChannelGroupOutlinerModel::CreateOutlinerViewForColumn(const FCreateOutlinerViewParams& InParams, const FName& InColumnName)
{
	...
	// Helper to check if any channel in this group can be keyed
	auto CanAnyChannelBeKeyed = [this]()
	{
		for (const TWeakViewModelPtr<FChannelModel>& WeakChannel : GetChannels())
		{
			if (TSharedPtr<FChannelModel> Channel = WeakChannel.Pin())
			{
				if (TSharedPtr<IKeyArea> KeyArea = Channel->GetKeyArea())
				{
					...
					if (MetaData && MetaData->bCanBeKeyed)
					{
						return true;
					}
				}
	...
	// Check if any channel's decoration editor provides a custom keyframe widget
	// (e.g., bone match channel replaces "add key" with a dialog launcher).
	// Used by both KeyFrame and Nav columns since the add-key button can appear in either.
	auto TryCreateDecorationKeyFrameWidget = [this, &Editor]() -> TSharedPtr<SWidget>
	{
		for (const TWeakViewModelPtr<FChannelModel>& WeakChannel : GetChannels())
		{
			if (TSharedPtr<FChannelModel> Channel = WeakChannel.Pin())
			{
				if (TSharedPtr<IKeyArea> KeyArea = Channel->GetKeyArea())
				{
					...
					if (OwningObject && Section && OwningObject != Section)
					{
						if (TSharedPtr<ISequencer> SequencerPtr = Editor->GetSequencer())
						{
							if (ISequencerDecorationEditor* DecorationEditor = SequencerPtr->FindDecorationEditor(OwningObject->GetClass()))
							{
								FGuid ObjectBindingID;
								if (TViewModelPtr<IObjectBindingExtension> ObjectBinding = FindAncestorOfType<IObjectBindingExtension>())
								{
									ObjectBindingID = ObjectBinding->GetObjectGuid();
								}

								if (TSharedPtr<SWidget> CustomWidget = DecorationEditor->CreateKeyFrameWidget(
									*OwningObject, *Section, KeyArea->GetChannel(), Editor->GetSequencer(), ObjectBindingID,
									SharedThis(this)))
								{
									return CustomWidget;
								}
						...
						// Fall through to standard key navigation.
					}
				}
			}
		}
	...
	{
		if (TSharedPtr<SWidget> CustomWidget = TryCreateDecorationKeyFrameWidget())
		{
			return CustomWidget;
		}

		if (!CanAnyChannelBeKeyed())
		{
			return nullptr;
		}
