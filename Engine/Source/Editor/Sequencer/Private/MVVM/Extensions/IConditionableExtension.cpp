...
namespace UE::Sequencer
{
	...
	ECachedConditionState FConditionStateCacheExtension::ComputeFlagsForModel(const FViewModelPtr& ViewModel)
	{
		...
		if (TViewModelPtr<IConditionableExtension> Conditionable = ViewModel.ImplicitCast())
		{
			...
			if (TViewModelPtr<ITrackRowExtension> TrackRowExtension = ViewModel.ImplicitCast())
			{
				RowIndex = TrackRowExtension->GetRowIndex();
			}

			if (TViewModelPtr<ISectionOwnerExtension> SectionOwner = ViewModel.ImplicitCast())
			{
