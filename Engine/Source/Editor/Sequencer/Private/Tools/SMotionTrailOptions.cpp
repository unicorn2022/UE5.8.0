...
void UMotionTrailToolOptions::AddPinned(const FPinnedTrail& InPinnedTrail)
{
	...
	{
		UE_LOGF(LogTemp, Warning, "MotionTrails: Cannot Pin trail %ls Max number reached. Please delete pinned trail if you want to add this one.", *InPinnedTrail.TrailName.ToString());
		return;
	}
...
void UMotionTrailToolOptions::PinSelection(ETrailCategory Category) const
{
	OnPinSelection.Broadcast(Category);
}

void UMotionTrailToolOptions::UnPinSelection(ETrailCategory Category)
{
	OnUnPinSelection.Broadcast(Category);
}
...
void UMotionTrailToolOptions::DeleteAllPinned(ETrailCategory Category)
{
	if (Category == ETrailCategory::All)
	{
		PinnedTrails.SetNum(0);
	}
	else
	{
		for (int32 Index = PinnedTrails.Num() - 1; Index >= 0; --Index)
		{
			if (EnumHasAnyFlags(PinnedTrails[Index].Category, Category))
			{
				FGuid Guid = PinnedTrails[Index].TrailGuid;
				PinnedTrails.RemoveAt(Index);
				OnDeletePinned.Broadcast(Guid);
	...
	OnDeleteAllPinned.Broadcast(Category);
}
