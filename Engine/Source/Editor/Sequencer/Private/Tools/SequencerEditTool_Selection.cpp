...
public:
	...
	FMarqueeDragOperation(FSequencer& InSequencer, UE::Sequencer::STrackAreaView& InTrackArea)
		...
		, SequencerWidget(InSequencer.GetUnderlyingSequencerWidget())
...
public:
	...
	virtual int32 OnPaint(const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId) const override
	{
		...
		// Both the pinned and non-pinned track areas share the same FTrackAreaViewModel and
		// thus the same edit tool, so OnPaint is invoked for both areas. Only draw the
		// marquee in the owner track area.
		//
		// SWidget::Paint sets GetPaintSpaceGeometry() (PersistentState.AllottedGeometry)
		// before calling OnPaint, so when the owner area is currently executing its OnPaint,
		// TrackArea.GetPaintSpaceGeometry() == AllottedGeometry. For the non-owner area,
		// AllottedGeometry is that area's own geometry, which differs from TrackArea's.
		const FVector2D PaintOrigin = AllottedGeometry.GetAccumulatedLayoutTransform().GetTranslation();
		const FVector2D OwnerOrigin = TrackArea.GetPaintSpaceGeometry().GetAccumulatedLayoutTransform().GetTranslation();
		if (!PaintOrigin.Equals(OwnerOrigin))
		{
			return LayerId;
		}
...
FCursorReply FSequencerEditTool_Selection::OnCursorQuery(const SWidget& OwnerWidget, const FGeometry& MyGeometry, const FPointerEvent& CursorEvent) const
{
	if (UE::CurveEditor::InputState::IsScrubTimeCommandPressed(FSequencerCommands::Get().ScrubTimeViewport))
	{
		return FCursorReply::Cursor(EMouseCursor::ResizeLeftRight);
	}
...
FReply FSequencerEditTool_Selection::OnMouseButtonDown(SWidget& OwnerWidget, const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	bIsScrubbingTime = UE::CurveEditor::InputState::IsScrubTimeCommandPressed(FSequencerCommands::Get().ScrubTimeViewport);
...
FReply FSequencerEditTool_Selection::OnMouseMove(SWidget& OwnerWidget, const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	if (DelayedDrag.IsSet())
	{
		...
		// Use the track area that actually received this event so the drag operates in the
		// correct coordinate space (pinned or non-pinned) and selects the right set of keys.
		STrackAreaView& EventTrackArea = static_cast<STrackAreaView&>(OwnerWidget);
		const FVirtualTrackArea VirtualTrackArea = SequencerWidget->GetVirtualTrackArea(&EventTrackArea);
		...
		else if (DelayedDrag->AttemptDragStart(MouseEvent))
		{
			...
			if (!DragOperation.IsValid())
			{
				if (bIsScrubbingTime)
				{
					DragOperation = MakeShareable(new FScrubTimeDragOperation(Sequencer, EventTrackArea));
				}
				else
				{
					DragOperation = MakeShareable(new FMarqueeDragOperation(Sequencer, EventTrackArea));
				}
			}
...
FReply FSequencerEditTool_Selection::OnMouseButtonUp(SWidget& OwnerWidget, const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	if (DragOperation.IsValid())
	{
		...
		FVector2D LocalPosition = MyGeometry.AbsoluteToLocal(MouseEvent.GetScreenSpacePosition());
		STrackAreaView& EventTrackArea = static_cast<STrackAreaView&>(OwnerWidget);
		DragOperation->OnEndDrag(MouseEvent, LocalPosition, SequencerWidget->GetVirtualTrackArea(&EventTrackArea));
...
{
	const bool bIsScrubCommand = UE::CurveEditor::InputState::IsCommandPressed(FSequencerCommands::Get().ScrubTimeViewport, InKeyEvent);
	if (bIsScrubCommand)
	{
		UE::CurveEditor::InputState::SetScrubTimeKeyPressed(InKeyEvent.GetKey(), true);
...
{
	const bool bIsScrubCommand = UE::CurveEditor::InputState::IsCommandPressed(FSequencerCommands::Get().ScrubTimeViewport, InKeyEvent);
	if (bIsScrubCommand && bIsScrubbingTime)
	{
		UE::CurveEditor::InputState::SetScrubTimeKeyPressed(InKeyEvent.GetKey(), false);
		bIsScrubbingTime = false;
...
{
	const bool bIsScrubCommand = UE::CurveEditor::InputState::IsScrubTimeCommandPressed(FSequencerCommands::Get().ScrubTimeViewport);
	return !bIsScrubCommand && !DelayedDrag.IsSet();
}
