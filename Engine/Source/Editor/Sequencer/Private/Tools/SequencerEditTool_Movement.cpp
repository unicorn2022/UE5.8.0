...
int32 FSequencerEditTool_Movement::OnPaint(const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId) const
{
	...
	// Let the drag operation paint first (for preview indicators)
	if (DragOperation.IsValid())
	{
...
FCursorReply FSequencerEditTool_Movement::OnCursorQuery(const SWidget& OwnerWidget, const FGeometry& MyGeometry, const FPointerEvent& CursorEvent) const
{
	using namespace UE::Sequencer;
...
UE::Sequencer::ISequencerEditToolDragOperation* FSequencerEditTool_Movement::GetCurrentDragOperation() const
{
	return DragOperation.Get();
}
