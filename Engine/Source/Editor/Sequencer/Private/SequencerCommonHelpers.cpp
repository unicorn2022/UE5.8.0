...
void SequencerHelpers::RemoveDuplicateKeys(const UE::Sequencer::FKeySelection& KeySelection, TArrayView<FKeyHandle> KeyHandles)
{
	...
	TMap<TViewModelPtr<FChannelModel>, bool> ChannelsToCheck;
	auto ShouldCheckChannel = [&ChannelsToCheck](const TViewModelPtr<FChannelModel>& ChannelModel)
	{
		if (ChannelModel)
		{
			if (bool* Found = ChannelsToCheck.Find(ChannelModel))
			{
				return *Found;
			}
			...
			if (FMovieSceneChannel* Channel = ChannelModel->GetChannel())
			{
				...
				if (AllTimes.Num() != AllHandles.Num())
				{
					// should never happen btw...
					bCheck = true;
				}
				else if (AllTimes.Num() > 1) // no need to check if there's only one key
				{
					// if there is more than one key, check is only needed if the times are not increasing 
					for (int32 Index = 1; Index < AllTimes.Num(); Index++)
					{
						if (AllTimes[Index] <= AllTimes[Index-1])
						{
							bCheck = true;
							break;
						}
			...
			ChannelsToCheck.Add(ChannelModel, bCheck);
			return bCheck;
		}
		return false;
	};
	...
	for (const FKeyHandle& KeyHandle : KeyHandles)
	{
		...
		if (ChannelModel)
		{
			if (!ShouldCheckChannel(ChannelModel))
			{
				continue;
			}
			...
			{	
				...
				// nothing to do if there's only one handle
				if (OutKeyHandles.Num() > 1)
				{	
...
TSharedPtr<SWidget> SequencerHelpers::SummonContextMenu(FSequencer& Sequencer, const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	TSharedRef<SSequencer> SequencerWidget = Sequencer.GetUnderlyingSequencerWidget();
	...
	, const bool bInSubMenu)
{
	...
	auto BuildSection = [InWeakSequencer, InWeakSections](FMenuBuilder& LambdaMenuBuilder)
	{
		...
		DetailsView->RegisterInstancedCustomPropertyTypeLayout("MovieSceneDecorationContainer", FOnGetPropertyTypeCustomizationInstance::CreateLambda([]() {
			return FMovieSceneDecorationContainerCustomization::MakeInstance(); }));
...
void SequencerHelpers::BuildEditDecorationMenu(const TWeakPtr<FSequencer>& InWeakSequencer
	, const TArray<TWeakObjectPtr<>>& InWeakDecorations
	, FMenuBuilder& MenuBuilder)
{
	using namespace UE::Sequencer;

	if (InWeakDecorations.Num() == 0)
	{
		return;
	}
	...
	// Group decorations by class so each class gets its own details view
	TMap<UClass*, TArray<TWeakObjectPtr<>>> DecorationsByClass;
	for (const TWeakObjectPtr<>& WeakDecoration : InWeakDecorations)
	{
		if (UObject* Decoration = WeakDecoration.Get())
		{
			DecorationsByClass.FindOrAdd(Decoration->GetClass()).Add(WeakDecoration);
		}
	}

	for (const auto& Pair : DecorationsByClass)
	{
		UClass* DecorationClass = Pair.Key;
		const TArray<TWeakObjectPtr<>>& ClassDecorations = Pair.Value;

		// Check if this class has any editable properties - skip if empty
		TArray<TWeakObjectPtr<>> ObjectsToDisplay;
		bool bIsSectionProvider = false;
		{
			bool bHasEditableProperties = false;
			for (TFieldIterator<FProperty> PropIt(DecorationClass); PropIt; ++PropIt)
			{
				if (PropIt->HasAnyPropertyFlags(CPF_Edit))
				{
					bHasEditableProperties = true;
					break;
				}
			}
			if (!bHasEditableProperties)
			{
				continue;
			}

			ObjectsToDisplay = ClassDecorations;

			for (const TWeakObjectPtr<>& WeakDecoration : ClassDecorations)
			{
				if (Cast<IMovieSceneSectionProviderDecoration>(WeakDecoration.Get()))
				{
					bIsSectionProvider = true;
					break;
				}
			}
		}

		if (ObjectsToDisplay.IsEmpty())
		{
			continue;
		}
		...
		DetailsView->SetIsPropertyVisibleDelegate(FIsPropertyVisible::CreateLambda([bIsSectionProvider](const FPropertyAndParent& PropertyAndParent)
			{
				// Hide the Decorations property when showing sections from section-providing decorations
				// to prevent the "Modifiers" category with decoration management UI from appearing
				if (bIsSectionProvider && PropertyAndParent.Property.GetFName() == TEXT("Decorations"))
				{
					return false;
				}
				return FPropertyEditorPermissionList::Get().DoesPropertyPassFilter(PropertyAndParent.Property.GetOwnerStruct(), PropertyAndParent.Property.GetFName());
		...
		Sequencer->OnInitializeDetailsPanel().Broadcast(DetailsView, Sequencer.ToSharedRef());
		DetailsView->SetObjects(ObjectsToDisplay);

		DetailsNotifyWrapper->SetDetailsAndSequencer(DetailsView, InWeakSequencer);
		DetailsNotifyWrapper->SetEnabled(!Sequencer->IsReadOnly());

		// Use the decoration class display name as the section header
		FText SectionLabel = DecorationClass->GetDisplayNameText();
		MenuBuilder.BeginSection(*DecorationClass->GetName(), SectionLabel);
		{
...
void SequencerHelpers::BuildObjectBindingMenu(const TWeakPtr<FSequencer>& InWeakSequencer
	, const UE::Sequencer::TViewModelPtr<UE::Sequencer::FObjectBindingModel>& InObjectBindingModel
	, FMenuBuilder& MenuBuilder)
{
	TSharedPtr<FSequencer> Sequencer = InWeakSequencer.Pin();
	if (!Sequencer || !InObjectBindingModel)
	{
		return;
	}

	const TSharedPtr<UE::Sequencer::FSequencerEditorViewModel> EditorViewModel = InObjectBindingModel->GetEditor();
	if (!EditorViewModel)
	{
		return;
	}
	...
	for (TWeakObjectPtr<UObject> RuntimeObject : Sequencer->FindObjectsInCurrentSequence(InObjectBindingModel->GetObjectGuid()))
	{
		UObject* BoundObject = RuntimeObject.Get();
		if (BoundObject)
		{
			ContextObjects.Add(BoundObject);
		}
	}

	const TSharedPtr<FExtender> Extender = EditorViewModel->GetSequencerMenuExtender(
		SequencerModule.GetSidebarExtensibilityManager(), 
		ContextObjects,
		&FSequencerCustomizationInfo::OnBuildSidebarMenu, 
		InObjectBindingModel);
	...
	MenuBuilder.BeginSection(TEXT("ObjectBindingActions"));
	MenuBuilder.EndSection();
}
...
bool SequencerHelpers::HasCompatibleDecorations(const UMovieSceneDecorationContainerObject* DecorationContainer)
{
	...
	IMovieSceneModule::Get().GetCompatibleDecorationsForContainer(DecorationContainer, CompatibleDecorations);

	// Check if any decoration is not already added
	for (UClass* DecorationClass : CompatibleDecorations)
	{
		bool bAlreadyExists = DecorationContainer->GetDecorations().ContainsByPredicate(
			[DecorationClass](const TObjectPtr<UObject>& Existing)
			{
				return Existing && Existing->GetClass() == DecorationClass;
			});
		if (!bAlreadyExists)
		{
			return true;
...
bool SequencerHelpers::HasOutlinerVisibleDecorations(const UMovieSceneDecorationContainerObject* Container)
{
	if (!Container)
	{
		return false;
	...
	const FText& SectionLabel)
{
	...
	if (CompatibleDecorations.Num() == 0)
	{
		return;
	}
	...
	if (const TSharedPtr<ISequencer> PinnedSequencer = InSequencer.Pin())
	{
		const TArrayView<TWeakObjectPtr<>> Objects = PinnedSequencer->FindObjectsInCurrentSequence(ObjectBindingID);
		if (Objects.Num())
		{
			BoundObject = Objects[0].Get();
		}
	}

	// Get menu providers
	TArray<IMovieSceneDecorationMenuProvider*> MenuProviders = IModularFeatures::Get().GetModularFeatureImplementations<IMovieSceneDecorationMenuProvider>(IMovieSceneDecorationMenuProvider::GetModularFeatureName());

	// Sort the menu providers so their entries are added in the order specified by their priority.
	MenuProviders.Sort([](const IMovieSceneDecorationMenuProvider& A, const IMovieSceneDecorationMenuProvider& B)
	{
		return A.GetDecorationMenuPriority() < B.GetDecorationMenuPriority();
	});

	const FText& Label = SectionLabel.IsEmpty() ? NSLOCTEXT("SequencerHelpers", "Modifiers_Label", "Modifiers") : SectionLabel;
	MenuBuilder.BeginSection(NAME_None, Label);
	{

		// First, allow menu providers to handle decorations.
		for (IMovieSceneDecorationMenuProvider* MenuProvider : MenuProviders)
		{
			if (!MenuProvider)
			{
				continue;
			}
			const UClass* HandledClass = MenuProvider->GetHandledDecorationClass();
			if (CompatibleDecorations.Contains(HandledClass))
			{
				// The custom menu provider should handle the case when the decoration already exists, so PopulateAddDecorationMenu is always called.
				MenuProvider->PopulateAddDecorationMenu(MenuBuilder, BoundObject, DecorationContainer, InSequencer);
				// Remove class, so its entry isn't duplicated. 
				CompatibleDecorations.Remove(HandledClass);
			}
		}

		for (UClass* DecorationClass : CompatibleDecorations)
		{
			const TObjectPtr<UObject>* ExistingDecoration = DecorationContainer->GetDecorations().FindByPredicate(
				[DecorationClass](const TObjectPtr<UObject>& Existing)
				{
					return Existing && Existing->GetClass() == DecorationClass;
				});

			IMovieSceneSectionProviderDecoration* SectionProvider = ExistingDecoration ? Cast<IMovieSceneSectionProviderDecoration>(ExistingDecoration->Get()) : nullptr;
			const bool bSupportsMultipleSections = SectionProvider ? SectionProvider->SupportsMultipleSections() : false;

			// Let a registered decoration editor supply an icon for the menu entry.
			FSlateIcon MenuIcon;
			if (TSharedPtr<ISequencer> PinnedSequencer = InSequencer.Pin())
			{
				if (ISequencerDecorationEditor* DecorationEditor = PinnedSequencer->FindDecorationEditor(DecorationClass))
				{
					MenuIcon = DecorationEditor->GetMenuIcon();
				}
			}
			...
			if (!ExistingDecoration)
			{
				MenuBuilder.AddMenuEntry(
					DecorationClass->GetDisplayNameText(),
					DecorationClass->GetToolTipText(),
					MenuIcon,
					FUIAction(FExecuteAction::CreateLambda([DecorationContainer, DecorationClass]()
					{
						FScopedTransaction Transaction(
							FText::Format(NSLOCTEXT("SequencerHelpers", "AddDecoration", "Add {0}"),
							DecorationClass->GetDisplayNameText()));

						DecorationContainer->Modify();
						UObject* NewDecoration = NewObject<UObject>(
							DecorationContainer, DecorationClass, NAME_None, RF_Transactional);
						DecorationContainer->AddDecoration(NewDecoration);
					}))
				);
			}
			else if (bSupportsMultipleSections)
			{
				MenuBuilder.AddMenuEntry(
					DecorationClass->GetDisplayNameText(),
					DecorationClass->GetToolTipText(),
					MenuIcon,
					FUIAction(FExecuteAction::CreateLambda([DecorationContainer, DecorationClass, SectionProvider, InSequencer]()
					{
						FScopedTransaction Transaction(
							FText::Format(NSLOCTEXT("SequencerHelpers", "AddDecorationSection", "Add {0}"),
							DecorationClass->GetDisplayNameText()));
						...
						DecorationContainer->Modify();
						UMovieSceneSection* NewSection = SectionProvider->CreateNewSection();
						...
						SectionProvider->AddSection(NewSection);
...
FPropertyPath SequencerHelpers::PropertyHandleToPropertyPath(const IPropertyHandle& InPropertyHandle)
{
	TSharedRef<const IPropertyHandle> CurrentHandle = InPropertyHandle.AsShared();
	TSharedPtr<const IPropertyHandle> ParentHandle = CurrentHandle->GetParentHandle();
