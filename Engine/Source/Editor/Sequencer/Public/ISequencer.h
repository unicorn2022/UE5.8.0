...
public:
	...
	UE_DEPRECATED(5.8, "FOnChannelChanged is now deprecated. Use FMovieSceneOnChannelChangedWithTime")
	DECLARE_MULTICAST_DELEGATE_TwoParams(FOnChannelChanged, const FMovieSceneChannelMetaData* MetaData, UMovieSceneSection*)
	
	DECLARE_MULTICAST_DELEGATE_ThreeParams(FMovieSceneOnChannelChangedWithTime, const TArray<FFrameNumber>&, const FMovieSceneChannelMetaData* MetaData, UMovieSceneSection*)
...
public:
	...
	/** @return Gets the interface with which you can interact with the widget used to display Sequencer. */
	virtual UE::Sequencer::ISequencerWidget& GetSequencerWidgetInterface() const = 0;
	...
	/**@return Returns the time transform from the focused sequence back to the root, this includes global time warp*/
	virtual FMovieSceneSequenceTransform GetFocusedMovieSceneSequenceTransform() const = 0;

	/**@return Returns the time transform from the focused sequence back to the root, without timewarp, to be used for example when baking and we need to pass time for evaluation*/
	virtual FMovieSceneSequenceTransform GetFocusedMovieSceneSequenceTransformUnwarped() const = 0;
	...
	*/
	...
	DECLARE_EVENT(ISequencer, FOnPreNavigateToSequence)
	virtual FOnPreNavigateToSequence& OnPreNavigateToSequence() = 0;
...
public:
	...
	/** Find the decoration editor that handles a given decoration class on this sequencer. */
	virtual ISequencerDecorationEditor* FindDecorationEditor(UClass* DecorationClass) const { return nullptr; }

	/** For a channel handle that may be owned by a decoration on its section, return the decoration editor and the owning decoration. */
	virtual TTuple<ISequencerDecorationEditor*, UObject*> FindDecorationOwner(
		const FMovieSceneChannelHandle& ChannelHandle,
		UMovieSceneSection* Section) const { return MakeTuple(nullptr, static_cast<UObject*>(nullptr)); }
	...
	virtual FMovieSceneOnChannelChangedWithTime& OnChannelChangedWithTime() = 0;
...
public:
	...
	*/
	...
	virtual bool IsSimpleView() const = 0;

	/** Enables or disables the Sequencer simplified view */
	virtual void EnableSimpleView(const bool bInEnable) = 0;
