...
public:
	 ...
	 * @param FindKeyType Whether to find all keys, sections or all
	...
	* @param Range       The range to search within
	* @param FindKeyType Whether to find all keys, sections or all
	* @param bAllowLoop  Whether to loop around if past the last key when retrieving the next key (or past the first key when retrieving the previous key)
	* @return (Optional)  Frame number of the key that's next or previous from that time 
	*/
	SEQUENCER_API TOptional<FFrameNumber> GetNextKey(FFrameNumber FrameNumber, EFindKeyDirection Direction, const TRange<FFrameNumber>& Range, EFindKeyType FindKeyType = EFindKeyType::FKT_All, const bool bAllowLoop = true) const;
