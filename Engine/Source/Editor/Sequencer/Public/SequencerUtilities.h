...
USTRUCT(BlueprintType)
struct FMovieScenePasteDecorationTarget
{
	GENERATED_BODY()

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Movie Scene")
	TObjectPtr<UMovieSceneDecorationContainerObject> Container;

	/** Decoration the user selected directly. Hosted sections matching this decoration's GetHostedSectionClass() are routed here. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Movie Scene")
	TObjectPtr<UObject> PreferredDecoration;
};
...
struct FMovieScenePasteSectionsParams
{
	...
	/** Decoration paste destinations. Each entry has at least Container or PreferredDecoration set. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Movie Scene")
	TArray<FMovieScenePasteDecorationTarget> DecorationTargets;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Movie Scene")
	FFrameTime Time;

	/** Append a paste target. Decoration info is recorded in DecorationTargets only when at least one of Container / PreferredDecoration is supplied. */
	SEQUENCER_API void AddTarget(
		UMovieSceneTrack* Track,
		int32 RowIndex = INDEX_NONE,
		UMovieSceneDecorationContainerObject* DecorationContainer = nullptr,
		UObject* PreferredDecoration = nullptr);
};

/* Paste decorations params */
USTRUCT(BlueprintType)
struct FMovieScenePasteDecorationsParams
{
	GENERATED_BODY()

	FMovieScenePasteDecorationsParams() {}
	FMovieScenePasteDecorationsParams(const TArray<UMovieSceneDecorationContainerObject*>& InContainers)
		: Containers(InContainers) {}

	/** Targets to receive a clipboard decoration. Each target must be registered-compatible (via IMovieSceneModule::RegisterCompatibleDecoration) and must not already host a decoration of the same class. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Movie Scene")
	TArray<TObjectPtr<UMovieSceneDecorationContainerObject>> Containers;
};
...
struct FMovieScenePasteBindingsParams
{
	...
	/** Original (template) actor path name to world to the pasted actor */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Movie Scene")
...
struct FSequencerUtilities
{
	...
	static UE_API void MakeTimeWarpMenuEntry(FMenuBuilder& MenuBuilder, UE::Sequencer::TWeakViewModelPtr<UE::Sequencer::FViewModel> WeakViewModel);
	...
	UE_DEPRECATED(5.8, "Please use the version that takes an ISectionOwnerExtension")
	...
	static UE_API void PopulateTimeWarpChannelSubMenu(FMenuBuilder& MenuBuilder, UE::Sequencer::TWeakViewModelPtr<UE::Sequencer::ISectionOwnerExtension> SectionOwner);
	...
	static UE_API void CopyDecorations(const TArray<UObject*>& Decorations, FString& ExportedText);
	static UE_API bool PasteDecorations(const FString& TextToImport, FMovieScenePasteDecorationsParams PasteDecorationsParams, TArray<UObject*>& OutDecorations, TArray<FNotificationInfo>& OutErrors);
	static UE_API bool CanPasteDecorations(const FString& TextToImport);
	...
	static UE_API void RemoveAllBindings(TSharedRef<ISequencer> Sequencer, const FMovieSceneBindingProxy& ObjectBinding);
	static UE_API void RemoveInvalidBindings(TSharedRef<ISequencer> Sequencer, const FMovieSceneBindingProxy& ObjectBinding);
	...
	static UE_API void SnapSectionsToTimelineUsingSourceTimecode(TSharedRef<ISequencer> Sequencer, const TArray<UMovieSceneSection*>& Sections);

	/** Sync section using source timecode */
	static UE_API void SyncSectionsUsingSourceTimecode(TSharedRef<ISequencer> Sequencer, const TArray<UMovieSceneSection*>& Sections);

	/** Attempts to automatically fix up broken actor references in the current scene */
	static UE_API void FixActorReferences(TSharedRef<ISequencer> Sequencer);
};
