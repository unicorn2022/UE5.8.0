...
{
	FBuildColumnWidgetParams(UE::Sequencer::TViewModelPtr<UE::Sequencer::FViewModel> InViewModelItem, const UE::Sequencer::FCreateOutlinerViewParams& InBaseParams)
		: UE::Sequencer::FCreateOutlinerViewParams(InBaseParams)
		, ViewModel(InViewModelItem.AsModel())
	{}
...
public:
	...
	*/
	...
	/** Whether or not we can add or remove a transform key for a selected object
	...
	*/
	...
	/**
	* Removes transform keys at the current time from the selected objects in the level.
	*
	* @param Channel The transform channel to remove keys for.
	*/
	virtual void OnDeleteTransformKeysForSelectedObjects(EMovieSceneTransformChannel Channel) {};
...
public:
	...
	/** Virtual destructor. */
	virtual ~ISequencerTrackEditor() = default;
};
