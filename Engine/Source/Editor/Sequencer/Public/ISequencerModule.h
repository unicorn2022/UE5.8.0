...
/** A delegate which will create a decoration editor for a particular sequencer instance. */
DECLARE_DELEGATE_RetVal_OneParam(TSharedRef<ISequencerDecorationEditor>, FOnCreateDecorationEditor, TSharedRef<ISequencer>);
...
struct FSequencerHostCapabilities
{
	...
	/** Do we support toggling between a minimalized, simple view instead of the full Sequencer view */
	bool bSupportsSimpleView;
...
public:
	...
	/**
	 * Registers a delegate that will create a decoration editor per-Sequencer.
	 *
	 * @param InOnCreateDecorationEditor Delegate to register.
	 * @return A handle to the newly-added delegate.
	 */
	virtual FDelegateHandle RegisterDecorationEditor(FOnCreateDecorationEditor InOnCreateDecorationEditor) = 0;

	/**
	 * Unregisters a previously registered delegate for creating a decoration editor.
	 *
	 * @param InHandle Handle to the delegate to unregister.
	 */
	virtual void UnRegisterDecorationEditor(FDelegateHandle InHandle) = 0;
