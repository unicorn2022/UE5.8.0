...
public:
	...
	/**
	 * @return An icon brush for the section's outliner row, or nullptr for no icon
	 */
	...
	/**
	 * @return Whether this section can be deleted by the user.
	 */
	virtual bool IsDeletable() const { return true; }

	/**
	 * Controls whether a given outliner column toggle (mute, solo, lock, deactivate, etc.)
	 * is available for this section's outliner item. Default is true for all columns.
	 * Override to return false for specific columns on sections like transitions that should
	 * not expose certain controls.
	 */
	virtual bool SupportsOutlinerColumnToggle(FName ColumnName) const { return true; }

	/**
	 * Controls whether the section's key area should be visible when the section's parent
	 * outliner item is collapsed. Default is true.
	 * Override to return false for sections that draw their own visualization but don't want
	 * standard key rendering when collapsed.
	 */
	virtual bool ShouldShowKeyAreaWhenCollapsed() const { return true; }
