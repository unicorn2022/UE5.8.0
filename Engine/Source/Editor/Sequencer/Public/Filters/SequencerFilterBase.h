...
/** Sequencer base filter for all Sequencer filters (Sequencer and Sequence Navigator) */
...
public:
	...
	{
		FFilterBase<InFilterType>::SetActive(bInActive);
		ActiveStateChanged(bInActive);
	}
	...
	/** Event called before nodes are filtered to give the filter a chance to pre-process */
