...
struct FChannelViewKeyCachedState
{
	...
	/** The value of FSequencerFilterBar::GetFilterSerial from the active filter bar when this cache was created */
	uint32 FilterSerial;
...
protected:
	...
	UE_API void EndCreateKeyTransactionIfActive();
...
protected:
	...
	/** True while a Create Key transaction started in OnMouseButtonDown is still open, so OnMouseButtonUp can close it regardless of which button or modifier state arrives on release. */
	bool bCreateKeyTransactionActive = false;
};
