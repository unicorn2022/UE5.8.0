...
public:
	...
	/** Gets the parent section owner model (which could be a track, track row, or something else like a mixer layer) */
	UE_API TViewModelPtr<ISectionOwnerExtension> GetParentSectionOwnerModel() const;

	/** Gets the ancestor track model */
	UE_API TViewModelPtr<ITrackExtension> GetParentTrackModel() const;

	/** Gets the ancestor track model as an ITrackExtension */
