...
void SWorldHierarchyImpl::AddUnfilteredItemToTree(WorldHierarchy::FWorldTreeItemRef InItem)
{
	...
	{
		UE_LOGF(LogWorldHierarchy, Error, "(%d | %ls) already exists in the World Hierarchy. Dumping map...", GetTypeHash(ItemID), *InItem->GetDisplayString());

		for (const auto& Entry : TreeItemMap)
		{
			UE_LOGF(LogWorldHierarchy, Log, "(%d | %ls)", GetTypeHash(Entry.Key), *Entry.Value->GetDisplayString());
		}
