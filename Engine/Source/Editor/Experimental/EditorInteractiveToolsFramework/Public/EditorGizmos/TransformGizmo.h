...
#include "GizmoElementRotate.h"
#include "GizmoElementScale.h"
#include "GizmoElementTranslate.h"
...
enum class ETransformGizmoShowFlags : uint32
{
	None = 0 UMETA(Hidden),
	DeltaLabel = 1 << 0,
	DeltaLine = 1 << 1,
...
ENUM_CLASS_FLAGS(ETransformGizmoShowFlags);
...
public:
	...
	UE_API FTransformGizmoStyle();
	FTransformGizmoStyle(const FTransformGizmoStyle&) = default;
	FTransformGizmoStyle& operator=(const FTransformGizmoStyle&) = default;
	PRAGMA_ENABLE_DEPRECATION_WARNINGS

	/** Clamps and otherwise corrects values within the style. */
	UE_API void MakeValid();
...
#pragma region Shared
	/** Applies to axis lengths and radii. Use the GetModifiedAxisSizeMultiplier to account for various other project/editor settings. */
	UPROPERTY(EditAnywhere, Category = "Shared", meta = (ClampMin = 0.1f, DisplayPriority = -1))
	float AxisSizeMultiplier = 1.0f;

	/** Applies to arrow heads, planar and uniform handles. */
	UPROPERTY(EditAnywhere, Category = "Shared", meta = (ClampMin = 0.01f, DisplayPriority = -1))
	float HandleSizeMultiplier = 1.0f;

	/** Applies to both translate and scale planar handles. */
	UPROPERTY(EditAnywhere, Category = "Shared", meta = (ClampMin = 0.01f, DisplayPriority = -1))
	float PlanarAxisOffsetFromOrigin = 50.0f;

	/** Used to prevent sub-pixel line sizes. */
	UPROPERTY(EditAnywhere, Category = "Shared", meta = (ClampMin = 0.01f))
	float MinLineThickness = 1.0f;

	UPROPERTY(EditAnywhere, Category = "Shared", meta = (ClampMin = 0.01f))
	float LineThicknessMultiplier = 1.0f;

	UPROPERTY(EditAnywhere, Category = "Shared", meta = (ClampMin = 0.01f))
	float HoverLineThicknessMultiplier = 1.5f;

	/** The current color to use. */
	UE_DEPRECATED(5.7, "Use HoverColor instead")
	UPROPERTY(meta = (HideAlphaChannel, DeprecatedProperty, DeprecationMessage = "Use HoverColor instead."))
	FLinearColor CurrentColor = FLinearColor(1.0f, 1.0f, 0.0f);

	/** The color to use on hover. */
	UPROPERTY(EditAnywhere, Category = "Shared", meta = (HideAlphaChannel))
	FLinearColor HoverColor = FLinearColor(1.0f, 0.552619f, 0.024453f);

	/** The color to use on select. */
	UPROPERTY(EditAnywhere, Category = "Shared", meta = (HideAlphaChannel))
	FLinearColor SelectColor = FLinearColor(0.8f, 0.8f, 0.0f);

	/** The color to use on interact. */
	UPROPERTY(EditAnywhere, Category = "Shared", meta = (HideAlphaChannel))
	FLinearColor InteractColor = FLinearColor(1.0f, 1.0f, 0.0f);

	/** The color to use on subdue. Depending on the effect, we may only use the alpha value. */
	UPROPERTY(EditAnywhere, Category = "Shared")
	FLinearColor SubdueColor = FLinearColor::Black.CopyWithNewOpacity(0.7f);

	UPROPERTY(EditAnywhere, Category = "Shared", meta = (HideAlphaChannel))
	FLinearColor GreyColor = FLinearColor(0.6f, 0.6f, 0.6f);

	UPROPERTY(EditAnywhere, Category = "Shared", meta = (HideAlphaChannel))
	FLinearColor WhiteColor = FLinearColor(0.9f, 0.9f, 0.9f);

	/** Whether to show shading on the gizmo, or unlit (if false). */
	UPROPERTY(EditAnywhere, Category = "Shared")
	bool bUseShading = true;

	UPROPERTY(EditAnywhere, Category = "Shared", meta = (EditCondition = "bUseShading", ClampMin = 0.0f, ClampMax = 2.0f))
	float ShadingAmbient = 0.8f;

	UPROPERTY(EditAnywhere, Category = "Shared", meta = (EditCondition = "bUseShading", ClampMin = 0.0f, ClampMax = 1.0f))
	float ShadingSpecularGlossiness = 0.1f;

	/* Pixel hit distance threshold - where the default is generally 7px. */
	UPROPERTY(EditAnywhere, Category = "Shared", meta = (ClampMin = 0.1f))
	float PixelHitDistanceThreshold = 15.0f;

	/* Minimum pixel hit distance threshold. */
	UPROPERTY(EditAnywhere, Category = "Shared", meta = (ClampMin = 0.0f))
	float MinimumPixelHitDistanceThreshold = 0.0f;

	/** Represents the background color for text labels. */
	UPROPERTY(EditAnywhere, Category = "Shared")
	FLinearColor DeltaTextBackgroundColor = FLinearColor(0.0f, 0.0f, 0.0f, 0.65f);

	/** Synchronizes the cursor size to that specified for the platform/OS. If false, CursorSize is used. */
	UPROPERTY(EditAnywhere, Category = "Shared")
	bool bUsePlatformCursorSize = true;

	/** Defines the size of the custom cursor. */
	UPROPERTY(EditAnywhere, Category = "Shared", meta = (EditCondition = "!bUsePlatformCursorSize"))
	float CursorSize = 16.0f;

	/** Sets the fixed visibility of various elements. */
	UPROPERTY(EditAnywhere, Category = "Shared", meta = (Bitmask, BitmaskEnum = "/Script/EditorInteractiveToolsFramework.ETransformGizmoShowFlags"))
	uint32 ShowFlags = static_cast<uint32>(ETransformGizmoShowFlags::All);
#pragma endregion Shared

#pragma region Translate
	UPROPERTY(EditAnywhere, Category = "Translate", meta = (ShowOnlyInnerProperties, DisplayPriority = 10))
	FGizmoElementTranslateStyle TranslateStyle;
#pragma endregion Translate

#pragma region Rotate
	UPROPERTY(EditAnywhere, Category = "Rotate", meta = (ShowOnlyInnerProperties, DisplayPriority = 11))
	FGizmoElementRotateStyle RotateStyle;
#pragma endregion Rotate

#pragma region Scale
	UPROPERTY(EditAnywhere, Category = "Scale", meta = (ShowOnlyInnerProperties, DisplayPriority = 12))
	FGizmoElementScaleStyle ScaleStyle;
#pragma endregion Scale

public:
#pragma region Shared
	/** Gets the AxisSizeMultiplier, modified by project/editor level settings, etc., or the given Modifier if specified. */
	UE_API float GetModifiedAxisSizeMultiplier(const TOptional<TUniqueFunction<float(const float)>>& InModifierOverride = { }) const;
#pragma endregion Shared
};
...
struct FTransformGizmoInteraction
{
	...
	/** Whether to allow handle selection to persist and reference for indirect operations. */ 
	UPROPERTY()
	bool bPersistHandleSelection = false;

	/** Stores additional coordinate systems enabled for this gizmo. By default, all are available. */
	UPROPERTY(EditAnywhere, Category = "Shared", meta = (Bitmask, BitmaskEnum = "/Script/EditorFramework.ECoordSystem"))
	int32 AdditionalCoordinateSystems = INDEX_NONE;

	UPROPERTY(EditAnywhere, Category = "Translate", meta = (ShowOnlyInnerProperties, DisplayPriority = 10))
	FGizmoElementTranslateInteraction TranslateInteraction;

	UPROPERTY(EditAnywhere, Category = "Rotate", meta = (ShowOnlyInnerProperties, DisplayPriority = 11))
	FGizmoElementRotateInteraction RotateInteraction;

	UPROPERTY(EditAnywhere, Category = "Scale", meta = (ShowOnlyInnerProperties, DisplayPriority = 12))
	FGizmoElementScaleInteraction ScaleInteraction;

	/** Returns true if the given coordinate system is enabled for this gizmo. */
	UE_API bool IsCoordinateSystemEnabled(const ECoordSystem InCoordinateSystem) const;

	/** Sets whether the given coordinate system is enabled for this gizmo. */
	UE_API void SetCoordinateSystemEnabled(const ECoordSystem InCoordinateSystem, const bool bInIsEnabled);
};
...
struct FGizmosParameters
{
	 ...
	 */
	UPROPERTY(EditAnywhere, Category = "Experimental", DisplayName = "Additive Indirect Manipulation")
	 ...
	 */
	UPROPERTY(EditAnywhere, Category = "Experimental", DisplayName = "Screen Space Indirect Orthographic Manipulation")
	 ...
	 * When enabled LMB is mapped to X, MMB is mapped to Y, and RMB is mapped to Z. 
	 */
	UPROPERTY(EditAnywhere, Category = "Experimental")
	 ...
	 */
	UPROPERTY(EditAnywhere, Category = "Experimental", DisplayName = "Indirect Scale is always Uniform")
	...
	/** Whether to allow persistent handle selection. If disabled, implicit indirect modification will also be disabled. Explicit (ctrl+mouse) will still work.  */
	UPROPERTY(EditAnywhere, Category = "Experimental")
	bool bPersistHandleSelection = false;

	/** When enabled, drag duplication will work on both translation and rotation (currently Level Editor only). */
	UPROPERTY(EditAnywhere, Category=GizmoInteraction)
	bool bDragDuplicateOnRotation = false;
	 ...
	 * When option is disabled, controls will be axis-based: Left/Right will control X, Up/Down will control Y, secondary Up/Down will control Z.
	 * Rotation nudging always does axis-based controls.
	 */
	UPROPERTY(EditAnywhere, Category=GizmoInteraction)
	bool bScreenSpaceNudge = true;
	...
	UPROPERTY(Config, EditAnywhere, Category = "Debug", meta=(HideInDetailPanel))
	FGizmoDebugSettings Debug;
};
...
{
	// ray based on the current input device
	FInputDeviceRay Ray;
	// whether the interaction is direct or indirect
	bool bIndirect = false;
};

DECLARE_MULTICAST_DELEGATE_OneParam(FOnGizmoInteraction, const FGizmoInteractionDescription&);
...
public:
	...
	/**
	 * True if gizmo is being used for indirect manipulation
	 */
	bool IsIndirectManipulation() const
	{
		return bIndirectManipulation;
	}
...
protected:
	...
	UE_API virtual FVector2D GetScreenRotateAxisDir(const FInputDeviceRay& InPressPos);
	
	/** Get screen-space axis for gimbal rotation drag */
	UE_API virtual FVector2D GetScreenGimbalRotateAxisDir(const FInputDeviceRay& InPressPos);
	...
	UE_API virtual FQuat ComputeGimbalRotateDelta(const FVector2D& InStartPos, const FVector2D& InEndPos, const double InAngleSign = 1.0);
	...
	UE_API virtual bool OnClickPressRotateArc( const FInputDeviceRay& InPressPos,
	...
	*/
	...
	UE_API virtual bool CanInteract(const EViewportContext InViewportContext = EViewportContext::Focused) const;
...
protected:
	...
	UE_API EAxisList::Type GetInteractionAxisList() const;
	UE_API virtual void SetInteractionAxisList(const EAxisList::Type InAxisList);
	...
	UE_DEPRECATED(5.8, "Direct access to InteractionAxisList is deprecated. Please use the getter/setter.")
	...
	/**
	 * When disabled, Ctrl+LMB manipulates X, Ctrl+RMB manipulates Y, and Ctrl+LMB+RMB manipulates Z.
	 * When enabled LMB is mapped to X, MMB is mapped to Y, and RMB is mapped to Z. 
	 */
	UPROPERTY()
	bool bSequentialIndirectAxesButtons = true;
	...
	UPROPERTY()
	TEnumAsByte<EAxisRotateMode::Type> DefaultRotateMode = EAxisRotateMode::ScreenArc;
	...
	/** pre-click delegate called before a click & drag interaction can start. */
	UE_INTERNAL FOnGizmoInteraction OnPreCanInteractDelegate;
	
	/** conditional post-release delegate after a click & drag interaction ends. */
	UE_INTERNAL FOnGizmoInteraction OnPostInteractionDelegate;
	
public:
	
	/** Delegate to a pre-click called delegate before a click & drag interaction can start. */
	UE_INTERNAL UE_API FOnGizmoInteraction& OnPreCanInteract();
	
	/** Delegate to a conditional post-release delegate after a click & drag interaction ends. */
	UE_INTERNAL UE_API FOnGizmoInteraction& OnPostInteraction();
