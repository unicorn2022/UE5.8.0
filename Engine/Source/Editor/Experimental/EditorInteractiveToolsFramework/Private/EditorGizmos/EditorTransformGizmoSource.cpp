...
bool UEditorTransformGizmoSource::CanInteract(const EViewportContext InViewportContext) const
{
	if (const FEditorViewportClient* ViewportClient = GetViewportClient(InViewportContext))
	{
		...
		if (ModeTools.GetShowWidget() && ModeTools.UsesTransformWidget())
		{
			...
			if (!bUseLegacyWidget)
			{
				bUseLegacyWidget = !UEditorInteractiveGizmoManager::AreEditorGizmosAllowed(&ModeTools);
			}
...
{
	EGizmoTransformScaleType ResolvedScaleType = FGizmoElementScaleInteraction::GetDefaultScaleType();
	if (const UTransformGizmoEditorSettings* Settings = GetDefault<UTransformGizmoEditorSettings>())
	{
		ResolvedScaleType = Settings->GizmosParameters.ScaleType;
	}
	
	if (ResolvedScaleType != EGizmoTransformScaleType::Legacy)
	{
		return ResolvedScaleType;
	}
