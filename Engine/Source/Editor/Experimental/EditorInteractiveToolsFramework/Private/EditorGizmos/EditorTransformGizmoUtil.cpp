...
void UEditorTransformGizmoContextObject::UpdateGizmo(const TArray<FEditorViewportClient*>& InViewportClients) const
{
	...
	{
		TransformGizmo->SetVisibility(!ModeTools->IsModeActive(FBuiltinEditorModes::EM_Default) ? ModeTools->UsesViewportITF() : false);
	}
}

void UEditorTransformGizmoContextObject::SetGizmosEnabled(bool bInEnabled)
{
	if (!bInEnabled)
	{
...
void UEditorTransformGizmoContextObject::InitializeGizmoManagerBinding()
{
	...
	{
		bool bEnableGizmos = bUseNewTRSGizmos;
	
		if (UModeManagerInteractiveToolsContext* ToolsContext = ModeTools->GetInteractiveToolsContext())
		{
			if (UEditorInteractiveGizmoManager* GizmoManager = Cast<UEditorInteractiveGizmoManager>(ToolsContext->GizmoManager))
			{
				// When the full context of the manager is available, rely on its state.
				// The setting to allow `NewTRSGizmo` may be on while a particular mode manager has disabled them.
				bEnableGizmos = GizmoManager->AreEditorGizmosAllowed();
			}
		}
		
		SetGizmosEnabled(bEnableGizmos);
	};
