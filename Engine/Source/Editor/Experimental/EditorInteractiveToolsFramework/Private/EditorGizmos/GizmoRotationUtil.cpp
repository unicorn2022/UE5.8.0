...
void FRelativeTransformInterfaceRegistry::LogOnce(const UClass* InClass) const
{
	if (InClass && !QueriedUnregisteredClass.Contains(InClass))
	{
		UE_LOGF(LogTemp, Warning, "No relative transform interface found for class %ls. Did you call RegisterRelativeTransformInterface<> for that class?", *InClass->GetName());
