...
FTransformGizmoStyle::FTransformGizmoStyle()
{
}

void FTransformGizmoStyle::MakeValid()
{
	constexpr float AxisSizeMultiplierMin = 0.1f;
	AxisSizeMultiplier = FMath::Max(AxisSizeMultiplierMin, AxisSizeMultiplier);
}

float FTransformGizmoStyle::GetModifiedAxisSizeMultiplier(const TOptional<TUniqueFunction<float(const float)>>& InModifierOverride) const
{
	if (InModifierOverride.IsSet())
	{
		return InModifierOverride.GetValue()(AxisSizeMultiplier);
	}

	return GetDefault<UTransformGizmoEditorSettings>()->GetTransformGizmoAxisMultiplier();
}

bool FTransformGizmoInteraction::IsCoordinateSystemEnabled(const ECoordSystem InCoordinateSystem) const
{
	return (AdditionalCoordinateSystems & (1 << InCoordinateSystem)) != 0;
}

void FTransformGizmoInteraction::SetCoordinateSystemEnabled(const ECoordSystem InCoordinateSystem, const bool bInIsEnabled)
{
	if (bInIsEnabled)
	{
		AdditionalCoordinateSystems |= (1 << InCoordinateSystem);
	}
	else
	{
		AdditionalCoordinateSystems &= ~(1 << InCoordinateSystem);
...
void UTransformGizmo::SetupIndirectBehaviors()
{
	...
	MiddleClickDragBehavior->OnClickPressFunc = [this](const FInputDeviceRay& InPressPos)
	{
		...
		if (LastHitPart != ETransformGizmoPartIdentifier::Default && LastHitPart != GetCurrentModeLastHitPart())
		{
			UpdateInteractingState(false, LastHitPart, true);
		}
	...
	{
		if (FInputDeviceState::IsAltKeyDown(InputState))
		{
			// Do not capture alt-clicks so that they can be handled by camera movements
			return false;
		}
		return !bSequentialIndirectAxesButtons || !FInputDeviceState::IsCtrlKeyDown(InputState);
	};
	...
	MultiIndirectClickDragBehavior->CanBeginClickDragFunc = [this](const FInputDeviceRay& PressPos)
	{
		if (OnPreCanInteractDelegate.IsBound())
		{
			FGizmoInteractionDescription Desc({.Ray=PressPos, .bIndirect=true});
	...
	auto GetAxis = [this](const FInputDeviceState& InInput)
	{
		...
		return InInput.Mouse.Middle.bDown || (bAddX && bAddY) ? EAxis::Z : bAddX ? EAxis::X : bAddY ? EAxis::Y : EAxis::None;
	};
...
bool UTransformGizmo::CanRender(IToolsContextRenderAPI* InRenderAPI) const
{
	return CanInteract() && GizmoElementRoot && InRenderAPI;
}
...
void UTransformGizmo::Render(IToolsContextRenderAPI* RenderAPI)
{
	if (CanRender(RenderAPI))
	{
		...
		if (IsVisible())
		{
			const UE::GizmoRenderingUtil::FSceneViewWrapper WrappedSceneView(*RenderAPI->GetSceneView());
		
			UGizmoElementBase::FRenderTraversalState RenderState;
			RenderState.Initialize(WrappedSceneView, GetGizmoTransform(), RenderAPI->GetCameraState().DPIScale);
...
bool UTransformGizmo::VerifyPartIdentifier(uint32 InPartIdentifier) const
{
	...
	{
		UE_LOGF(LogTransformGizmo, Warning, "Unrecognized transform gizmo part identifier %d, valid identifiers are between 0-%d.", 
...
EAxisList::Type UTransformGizmo::GetInteractionAxisList() const
{
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	return InteractionAxisList;
PRAGMA_ENABLE_DEPRECATION_WARNINGS
}

void UTransformGizmo::SetInteractionAxisList(const EAxisList::Type InAxisList)
{
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	InteractionAxisList = InAxisList;
PRAGMA_ENABLE_DEPRECATION_WARNINGS
}
...
void UTransformGizmo::OnParametersChanged(const FGizmosParameters& InParameters)
{
	...
	// Unlike the EditorTRSGizmo, these are fixed (used to be in the now-deleted FGizmoDevParameters)
	constexpr float LineThicknessMultiplier = 1.0f;
	constexpr float TranslateAxisLength = 70.0f;
	constexpr float ScaleAxisLength = 70.0f;
	constexpr float RotateAxisRadius = 70.0f;
	...
	Style.LineThicknessMultiplier = FMath::Max(0.01f, LineThicknessMultiplier);
	Style.TranslateAxisLength = FMath::Max(1.0f, TranslateAxisLength);
	Style.RotateArcballSphereRadius = FMath::Max(1.0f, RotateAxisRadius);
	Style.RotateAxisRadius = Style.RotateArcballSphereRadius;
	Style.RotateCircleRadius = Style.RotateArcballSphereRadius;
	Style.ScaleAxisLength = FMath::Max(1.0f, ScaleAxisLength);
	...
	const EGizmoElementDrawType InDrawType, const TOptional<FLinearColor>& InLineColorOverride)
{
	...
	CircleElement->SetAxisBitangent(FVector::UpVector);
	CircleElement->SetAxisTangent(-FVector::RightVector);
...
void UTransformGizmo::OnClickRelease(const FInputDeviceRay& ReleasePos)
{
	...
	{
		FGizmoInteractionDescription Desc({.Ray=ReleasePos, .bIndirect=bIndirectManipulation});
...
void UTransformGizmo::SnapTranslateDelta(FVector& InOutWorldDelta) const
{
	...
	if (const USceneSnappingManager* SnapManager = USceneSnappingManager::Find(GizmoManager))
	{
		...
		SnapRequest.RequestCoordinateSpace = QueriesAPI->GetCurrentCoordinateSystem();
		SnapRequest.RequestType = ESceneSnapQueryType::Position;
		SnapRequest.AxisList = GetInteractionAxisList();
		...
		if (SnapRequest.RequestCoordinateSpace == EToolContextCoordinateSystem::Local)
		{
			...
			const FQuat Rotation = ActiveTarget->GetTransform().GetRotation();
		...
		if (SnapManager->ExecuteSceneSnapQuery(SnapRequest, SnapResults))
		{
			...
			const FSceneSnapQueryResult& SnapResult = SnapResults[0];

			// Convert the delta back to the desired space
...
void UTransformGizmo::SnapRotateDelta(FQuat& InOutWorldDelta) const
{
	...
	if (const USceneSnappingManager* SnapManager = USceneSnappingManager::Find(GizmoManager))
	{
		...
		if (SnapManager->ExecuteSceneSnapQuery(SnapRequest, SnapResults))
		{
			...
			InOutWorldDelta = SnapResults[0].DeltaRotation;
		}
	}
}
...
void UTransformGizmo::SnapRotateAngleDelta(double& InOutAngleDelta) const
{
	...
	if (const USceneSnappingManager* SnapManager = USceneSnappingManager::Find(GizmoManager))
	{
		...
		if (SnapManager->ExecuteSceneSnapQuery(SnapRequest, SnapResults))
		{
			...
			InOutAngleDelta = SnapResults[0].RotationAngle;
		}
	}
}
...
void UTransformGizmo::SnapScaleDelta(FVector& InOutLocalScaleDelta) const
{
	...
	if (const USceneSnappingManager* SnapManager = USceneSnappingManager::Find(GizmoManager))
	{
		...
		// Scale should always be in local space
		...
		if (SnapManager->ExecuteSceneSnapQuery(SnapRequest, SnapResults))
		{
			...
			InOutLocalScaleDelta = SnapResults[0].Scale;
		}
	}
}
...
void UTransformGizmo::OnClickDragTranslateAxis(const FInputDeviceRay& DragPos)
{
	...
	if (bIndirectManipulation)
	{
		...
		const EAxisList::Type AxisList = GetInteractionAxisList();
		FVector Delta((AxisList == EAxisList::X) ? PixelToWorldRatio * FVector2D::DotProduct(XAxisDir, DragDir) : 0.0,
					  (AxisList == EAxisList::Y) ? PixelToWorldRatio * FVector2D::DotProduct(YAxisDir, DragDir) : 0.0,
					  (AxisList == EAxisList::Z) ? PixelToWorldRatio * FVector2D::DotProduct(ZAxisDir, DragDir) : 0.0);
...
FVector UTransformGizmo::ComputeScaleDelta(const FVector2D& InStartPos, const FVector2D& InEndPos, FVector2D& OutScreenDelta)
{
	...
	const EAxisList::Type AxisList = GetInteractionAxisList();
	const FVector Scale(
		(AxisList & EAxisList::X) ? ScaleDelta : 0.0f,
		(AxisList & EAxisList::Y) ? ScaleDelta : 0.0f,
		(AxisList & EAxisList::Z) ? ScaleDelta : 0.0f);
...
bool UTransformGizmo::OnClickPressRotateArc(
	...
	const FVector& InPlaneAxis1, const FVector& InPlaneAxis2, const EAxis::Type InAxis)
{
...
void UTransformGizmo::OnClickPressRotateAxis(const FInputDeviceRay& InPressPos)
{
	...
	SetInteractionAxisList(AxisList[RotateID]);
...
FVector2D UTransformGizmo::GetWorldToScreenRotateAxisDir(const FInputDeviceRay& InPressPos, const FVector& InWorldAxis)
{
	...
	const auto PrintProjection = [this](const TCHAR* const InMessage)
	{
		...
		{
			UE_LOGF(LogTransformGizmo, Warning, "%ls", InMessage);
		}
	};
...
FQuat UTransformGizmo::ComputeAxisRotateDelta(const double& InDeltaAngle)
{
	...
	double DeltaAngle = InDeltaAngle;
	if (AxisDisplayInfo::GetAxisDisplayCoordinateSystem() == EAxisList::LeftUpForward && AxisList == EAxisList::Left)
	{
		DeltaAngle *= -1;
	}

	FRotator DeltaRot(
		AxisList == EAxisList::Y || AxisList == EAxisList::Left ? DeltaAngle : 0.0,
		AxisList == EAxisList::Z || AxisList == EAxisList::Up ? DeltaAngle : 0.0,
		AxisList == EAxisList::X || AxisList == EAxisList::Forward ? DeltaAngle : 0.0);
...
FQuat UTransformGizmo::ComputeGimbalRotateDelta(const FVector2D& InStartPos, const FVector2D& InEndPos, const double InAngleSign)
{
	...
	const FVector DeltaRot(
	 	   AxisList == EAxisList::X || AxisList == EAxisList::Forward ? Delta : 0.0,
	 	   AxisList == EAxisList::Y || AxisList == EAxisList::Left ? Delta : 0.0,
	 	   AxisList == EAxisList::Z || AxisList == EAxisList::Up ? Delta : 0.0);
	...
	case EAxisRotateMode::ScreenArc:
		{
			...
			if (GetRayParamIntersectionWithInteractionPlane(InDragPos, HitDepth))
			{
				...
				const EAxisList::Type AxisList = GetInteractionAxisList();
				const FVector DeltaRot(
					AxisList == EAxisList::X ? DeltaAngle : 0.0,
					AxisList == EAxisList::Y ? DeltaAngle : 0.0,
					AxisList == EAxisList::Z ? DeltaAngle : 0.0);
...
FOnGizmoInteraction& UTransformGizmo::OnPreCanInteract()
{
	return OnPreCanInteractDelegate;
}
	
FOnGizmoInteraction& UTransformGizmo::OnPostInteraction()
{
	return OnPostInteractionDelegate;
}
