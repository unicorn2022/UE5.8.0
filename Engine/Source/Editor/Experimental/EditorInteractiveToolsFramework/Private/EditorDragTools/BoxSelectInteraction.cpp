...
UBoxSelectInteraction::UBoxSelectInteraction()
{
	InteractionName = UE::Editor::ViewportInteractions::BoxSelect;

	if (UViewportClickDragBehavior* ClickDragBehavior = ClickDragInputBehavior.Get())
	{
		ClickDragBehavior->SetBindings({ UE::Editor::ViewportInteractions::FButtonBinding(EKeys::LeftMouseButton).TriggersStart() });
	}
}

FInputRayHit UBoxSelectInteraction::CanBeginClickDragSequence(const FInputDeviceRay& InPressPos)
{
...
void UBoxSelectInteraction::OnDragStart(const FInputDeviceRay& InPressPos)
{
	...
	Start = FVector(MouseDownPosition.X, MouseDownPosition.Y, 0.0);
	End = Start;
...
void UBoxSelectInteraction::OnDrag(const FDragArgs& InDrag)
{
	...
	FEditorModeTools* const ModeTools = EditorViewportClient->GetModeTools();
	if (!ModeTools)
	{
		return;
	}
...
void UBoxSelectInteraction::OnDragEnd(const FInputDeviceRay& InReleasePos)
{
	if (EditorViewportClientProxy)
	{
		if (FEditorViewportClient* const EditorViewportClient = GetEditorViewportClient())
		{
			...
			// TODO: with ITF Gizmos indirect interactions, this is probably not going to be triggered
...
TArray<FEditorModeID> UBoxSelectInteraction::GetUnsupportedModes() const
{
...
void UBoxSelectInteraction::CalculateBox(FBox& OutBox) const
{
	FEditorViewportClient* const EditorViewportClient = GetEditorViewportClient();
	if (!EditorViewportClient)
	{
...
bool UBoxSelectInteraction::IntersectsBox(const UModel& InModel, int32 InNodeIndex, const FBox& InBox, bool bInUseStrictSelection)
{
	FBox NodeBB;
...
void UBoxSelectInteraction::AddHoverEffect(AActor& InActor)
{
	const FViewportHoverTarget HoverTarget(&InActor);
	UE::EditorDragTools::Private::AddHoverEffect(HoverTarget);
}

void UBoxSelectInteraction::AddHoverEffect(UModel& InModel, int32 InSurfIndex)
{
	FViewportHoverTarget HoverTarget(&InModel, InSurfIndex);
	UE::EditorDragTools::Private::AddHoverEffect(HoverTarget);
}

void UBoxSelectInteraction::RemoveHoverEffect(AActor& InActor)
{
	FViewportHoverTarget HoverTarget(&InActor);
	UE::EditorDragTools::Private::RemoveHoverEffect(HoverTarget);
}

void UBoxSelectInteraction::RemoveHoverEffect(UModel& InModel, int32 InSurfIndex)
{
	FViewportHoverTarget HoverTarget(&InModel, InSurfIndex);
	UE::EditorDragTools::Private::RemoveHoverEffect(HoverTarget);
}
