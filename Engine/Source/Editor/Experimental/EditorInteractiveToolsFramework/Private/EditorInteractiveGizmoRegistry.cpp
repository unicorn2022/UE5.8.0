...
void UEditorGizmoRegistryCategoryEntry::RegisterGizmoType(UInteractiveGizmoBuilder* InGizmoBuilder)
{
	...
	{
		UE_LOGF(LogEditorInteractiveGizmoRegistry, Warning,
			"gizmo builder type %ls has already been registered!", *InGizmoBuilder->GetPathName());
		return;
	}
...
void UEditorGizmoRegistryCategoryEntry::DeregisterGizmoType(UInteractiveGizmoBuilder* InGizmoBuilder)
{
	...
	{
		UE_LOGF(LogEditorInteractiveGizmoRegistry, Warning,
			"UInteractiveGizmoRegistry::DeregisterEditorGizmoType: gizmo type not found in registry %ls for category %ls", *InGizmoBuilder->GetName(), *CategoryName);
	}
}
...
void UEditorGizmoRegistryCategoryEntry_ConditionalSelection::RegisterGizmoType(UInteractiveGizmoBuilder* InGizmoBuilder)
{
	...
	{
		UE_LOGF(LogEditorInteractiveGizmoRegistry, Warning,
			"%ls gizmo builder '%ls' of type '%ls' does not implement the IEditorInteractiveGizmoConditionalBuilder interface! Skipping registration.", *CategoryName, *InGizmoBuilder->GetPathName(), *InGizmoBuilder->GetClass()->GetName());
		return;
	}
	...
	{
		UE_LOGF(LogEditorInteractiveGizmoRegistry, Warning,
			"%ls gizmo builder '%ls' of type '%ls' does not implement the IEditorInteractiveGizmoSelectionBuilder interface! Skipping registration.", *CategoryName, *InGizmoBuilder->GetPathName(), *InGizmoBuilder->GetClass()->GetName());
		return;
	}
