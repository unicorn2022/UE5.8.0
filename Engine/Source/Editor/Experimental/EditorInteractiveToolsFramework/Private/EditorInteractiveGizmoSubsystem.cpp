...
void UEditorInteractiveGizmoSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	{
		FCoreDelegates::GetOnPostEngineInit().AddUObject(this, &UEditorInteractiveGizmoSubsystem::RegisterBuiltinEditorGizmoTypes);
	}
}
...
{
	// Setup gizmo transform builder (UEditorTRSGizmoBuilder extends UEditorTransformGizmoBuilder
	// and falls back to it when bUseEditorTRSGizmo is false)
	RegisterTransformGizmoBuilder(NewObject<UEditorTRSGizmoBuilder>());
