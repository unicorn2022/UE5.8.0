...
{
	InteractionName = UE::Editor::ViewportInteractions::CameraTranslate;
		...
		if (InputDeviceState.InputDevice == EInputDevices::Keyboard)
		{
				...
				CommandMatchesKey(FViewportNavigationCommands::Get().LocalDown, ActiveKey))
			{
				...
				if (bResult)
				{
					if (UViewportInteractionsBehaviorSource* BehaviorSource = GetViewportInteractionsBehaviorSource())
					{
						BehaviorSource->SetIsMouseLooking(!BehaviorSource->IsGizmoDragging());
					}
				}
			}
		}

		return bResult || FSlateApplication::Get().IsUsingTrackpad();
	};
