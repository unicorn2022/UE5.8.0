...
R"_JSON({ 
	...
	TestEqual(TEXT("Nested"), Top->Values[FJsonObject::FStringType(TEXT("Nested"))]->AsObject(), Nested);
...
R"_JSON({
	...
	FooValue = Object->Values[FJsonObject::FStringType(TEXT("Foo"))];
	TestEqual(TEXT("Before Set"), FooValue->AsNumber(), 1.0);

	BarValue = Object->Values[FJsonObject::FStringType(TEXT("Bar"))];
