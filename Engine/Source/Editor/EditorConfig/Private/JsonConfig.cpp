...
namespace UE
{
	...
	void FJsonPath::ParsePath(const FString& InPath)
	{
		...
		for (const FString& Part : PathStrings)
		{
			...
			{
				PathParts.Empty();
				UE_LOGF(LogJsonConfig, Warning, "Path part was empty in JSON path \"%ls\"", *InPath);
				return;
			}
			...
			if (Part.FindChar('[', OpenBracket))
			{
					...
					CloseBracket == Part.Len() - 1)
				{
					...
					{
						PathParts.Empty();
						UE_LOGF(LogJsonConfig, Warning, "Path part \"%ls\" looked to be index into array but was malformed in JSON path \"%ls\"", *Part, *InPath);
						return;
					}
				}
				else
	...
	bool FJsonConfig::LoadFromFile(FStringView FilePath)
	{
		...
		{
			UE_LOGF(LogEditorConfig, Error, "Failed to load JSON file into JsonConfig %ls", FilePath.GetData());
			return false;
		}
		return true;
	}
	...
	bool FJsonConfig::LoadFromString(FStringView Content)
	{
		...
		{
			UE_LOGF(LogEditorConfig, Error, "Failed to deserialize JSON string: %ls", *JsonReader->GetErrorMessage());
			return false;
		}
	...
	static bool ApplyDeltaOperationsToArray(const TSharedPtr<FJsonObject>& OverrideObject, TArray<TSharedPtr<FJsonValue>>& DestArray)
	{
		...
		{
			UE_LOGF(LogJsonConfig, Warning, "JSON container is malformed and contains both set and add/remove values.");
		}
	...
	{
		for (const TPair<FJsonObject::FStringType, TSharedPtr<FJsonValue>>& Override : Overrides->Values)
		{
			...
			else
			{
				switch (Override.Value->Type)
				{
					case EJson::Boolean:
					{
						...
						{
							Dest->SetBoolField(FString(Override.Key), OverrideBool);
						}
						continue;
					}
					case EJson::Number:
					{
						...
						{
							Dest->SetNumberField(FString(Override.Key), OverrideNumber);
						}
						continue;
					}
					case EJson::String:
					{
						...
						{
							Dest->SetStringField(FString(Override.Key), OverrideString);
						}
						continue;
					}
					case EJson::Array:
	...
	static TSharedPtr<FJsonValue> FindPartInObject(const TSharedPtr<FJsonObject>& Object, const FJsonPath::FPart& PathPart)
	{
		TSharedPtr<FJsonValue> NextValue = Object->GetFieldUntyped(FStringView(PathPart.Name));
	...
	bool FJsonConfig::TryGetJsonValue(const FJsonPath& Path, TSharedPtr<FJsonValue>& OutValue) const
	{
		...
		for (int32 Idx = 0; Idx < Path.Length() - 1; ++Idx)
		{
			...
			if (!NextValue.IsValid() || !NextValue->TryGetObject(CurrentObject))
			{
				return false;
			}
		}
	...
	static bool SetObjectValueHelper(const TSharedPtr<FJsonObject>& CurrentObject, const TSharedPtr<FJsonObject>& NewObject, const TSharedPtr<FJsonValue>& ParentValue)
	{
		...
		if (ParentObject == nullptr)
		{
			...
			for (const TPair<FJsonObject::FStringType, TSharedPtr<FJsonValue>>& NewValuePair : NewObject->Values)
			{
	...
	bool FJsonConfig::SetRootObject(const TSharedPtr<FJsonObject>& Object)
	{
		...
		TArray<FJsonObject::FStringType> NewKeys;
		Object->Values.GenerateKeyArray(NewKeys);

		TArray<FJsonObject::FStringType> ExistingKeys;
		OverrideObject->Values.GenerateKeyArray(ExistingKeys);

		for (const TPair<FJsonObject::FStringType, TSharedPtr<FJsonValue>>& Pair : Object->Values)
		{
		...
		for (const FJsonObject::FStringType& Key : ExistingKeys)
		{
			if (!NewKeys.Contains(Key))
			{
				OverrideObject->RemoveField(FStringView(Key));
			}
		}
