	...
	if (FApp::CanEverRender() && !IsRunningCommandlet())
	{
		...
		if (const UDocumentationSettings* DocSettings = GetDefault<UDocumentationSettings>())
		{
			for (const FDocumentationBaseUrl& BaseUrl : DocSettings->DocumentationBaseUrls)
			{
				...
				{
					UE_LOGF(LogDocumentation, Warning, "Could not register documentation base URL: %ls", *BaseUrl.Id);
				}
			}
		}
...
bool FDocumentation::RegisterBaseUrl(const FString& Id, const FString& Url)
{
	if (!Id.IsEmpty() && !Url.IsEmpty())
	{
		...
		UE_LOGF(LogDocumentation, Warning, "Could not register documentation base URL with ID: %ls. This ID is already in use.", *Id);
		return false;
	}
	return false;
}
...
FString FDocumentation::GetBaseUrl(const FString& Id) const
{
	if (!Id.IsEmpty())
	{
		...
		UE_LOGF(LogDocumentation, Warning, "Could not resolve base URL with ID: %ls. It may not have been registered.", *Id);
	}
...
FDocumentationUrl FDocumentation::RedirectUrl(const FDocumentationUrl& OriginalUrl) const
{
	...
	{
		UE_LOGF(LogDocumentation, Log, "Documentation link \"%ls\" was redirected to \"%ls\"", *OriginalUrl.ToString(), *Redirect.ToUrl.ToString());
