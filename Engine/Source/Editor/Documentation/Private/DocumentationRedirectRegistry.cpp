...
bool FDocumentationRedirectRegistry::Register(const FName& Owner, const FDocumentationRedirect& Redirect)
{
	...
	{
		UE_LOGF(LogDocumentation, Warning, "Documentation redirect target for \"%ls\" is not valid.", *Redirect.From);
		return false;
	}
	...
	UE_LOGF(LogDocumentation, Log, "[%ls] Documentation redirect registered: %ls -> %ls.", !Owner.IsNone() ? *Owner.ToString() : TEXT("Global"), *FromLink, *Redirect.ToUrl.ToString());
