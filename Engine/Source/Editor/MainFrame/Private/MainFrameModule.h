...
#include "Interfaces/IHomeScreenProvider.h"

/** 
 * Utility class, which hooks into the editor's home screen tab and
 * supply it with its own settings if it is the higher priority provider
 */
class FEditorHomeScreenProvider : public IHomeScreenProvider
{
public:
	void Register();
	void UnRegister();

	//~ Begin IHomeScreenProvider interface
	virtual bool IsRequestingHomeScreenPageControl() const override;
	virtual int32 GetHomeScreenPriority() override;
	virtual FHomeScreenWidgetSettings GetHomeScreenWidgetSettings() const override;
	//~ End IHomeScreenProvider interface
};
...
public:
	...
	virtual void ReconstructDefaultMainFrameContent(const bool bStartImmersive, const bool bStartPIE) override;
private:
	/**
	 ...
	 * @param bIsJustCreatingContent False if it is being called by first time (CreateDefaultMainFrame), and true if the content is being reconstructed (ReconstructDefaultMainFrameContent).
	 */
	virtual void CreateDefaultMainFrameAuxiliary(const bool bStartImmersive, const bool bStartPIE, const bool bIsBeingRecreated, const bool bIsJustCreatingContent = false);
...
public:
	...
	// Opens the project browser in a new window
	virtual void ShowProjectBrowser(bool bAllowProjectOpening, bool bAllowProjectCreate) override;

	// Overrides the functionality of ShowProjectBrowser with the provided delegate
	virtual FDelegateHandle RegisterShowProjectBrowserOverride(const FShowProjectBrowser& InShowProjectBrowserOverride) override
	{
		ShowProjectBrowserOverride = InShowProjectBrowserOverride;
		return ShowProjectBrowserOverride.GetHandle();
	}

	// Removes the override of ShowProjectBrowser if it matches the handle
	virtual void UnregisterShowProjectBrowserOverride(FDelegateHandle InHandle) override
	{
		if (ShowProjectBrowserOverride.GetHandle() == InHandle)
		{
			ShowProjectBrowserOverride = FShowProjectBrowser{};
		}
	}
...
public:
	...
	virtual TSharedRef<SWidget> CreateHomeScreenWidget(const TSharedPtr<SDockTab>& InParentTab, FHomeScreenWidgetSettings InSettings = FHomeScreenWidgetSettings()) override;

private:

	// Spawns the home screen tab
	TSharedRef<SDockTab> SpawnHomeScreenWindow(const FSpawnTabArgs& InArgs);
	...
	// Destroy the given array of windows if they are regular windows and not the root one
	void DestroyRegularAndNonRootWindows(TArray<TSharedRef<SWindow>> InWindowsToDestroy) const;
...
private:
	...
	// Instantiation of the object responsible for getting the home screen settings to use if active
	FEditorHomeScreenProvider HomeScreenProvider;
	...
	TSharedPtr<FTabManager> HomeScreenTabManager = nullptr;

	// Stores the delegate for the override of ShowProjectBrowser, if any
	FShowProjectBrowser ShowProjectBrowserOverride;
};
