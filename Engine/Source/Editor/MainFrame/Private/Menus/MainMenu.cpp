...
void PopulateTabSpawnerToolSection(TSharedPtr<FTabManager> TabManager, FToolMenuSection& InSection, const FName& TabType)
{
	...
	{
		UE_LOGF(LogMainFrame, Warning, "PopulateTabSpawnerMenu failed to find entry for %ls", *(TabType.ToString()));
	}
}
...
void FMainMenu::RegisterEditMenu()
{
...

	{
		...
		{
			const FGlobalEditorCommonCommands& GlobalEditorCommands = FGlobalEditorCommonCommands::Get();
			Section.AddMenuEntry(GlobalEditorCommands.OpenEditorPreferences);
			Section.AddMenuEntry(GlobalEditorCommands.OpenKeyboardShortcuts);
			Section.AddMenuEntry(GlobalEditorCommands.OpenProjectSettings);
		}
		...
		if (USlateTabManagerContext* TabManagerContext = InMenu->FindContext<USlateTabManagerContext>())
		{
			if (TSharedPtr<FTabManager> TabManager = TabManagerContext->TabManager.Pin())
			{
				...
				if(TabManager != FGlobalTabmanager::Get())
				{
					// Local editor tabs
					{
						constexpr bool bDisplayOrphanedTabs = false;
						UE::MainMenu::Private::PopulateTabSpawnerToolMenu(TabManager, InMenu, TabManager->GetLocalWorkspaceMenuRoot(), bDisplayOrphanedTabs);
					}

					// General tabs
					{
						constexpr bool bDisplayOrphanedTabs = true;
						const IWorkspaceMenuStructure& MenuStructure = WorkspaceMenu::GetMenuStructure();
						UE::MainMenu::Private::PopulateTabSpawnerToolMenu(TabManager, InMenu, MenuStructure.GetStructureRoot(), bDisplayOrphanedTabs);
					}
				}
			}
		}
...
void FMainMenu::RegisterToolsMenu()
{
	...
	SourceControlSection.AddDynamicEntry("BranchExplorerTab", FNewToolMenuSectionDelegate::CreateLambda([](FToolMenuSection& InSection)
	{
		ISourceControlModule& SourceControlModule = ISourceControlModule::Get();
		...
		{
			InSection.AddMenuEntry(
				FMainFrameCommands::Get().ViewBranchExplorer,
				TAttribute<FText>(),
				TAttribute<FText>(),
				FSlateIcon(FRevisionControlStyleManager::Get().GetStyleSetName(), "RevisionControl.BranchExplorerTab")
			);
		}
	}));
