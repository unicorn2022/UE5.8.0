...
TWeakPtr<SWindow> FMainFrameActionCallbacks::NewProjectWindowWeak;
...
void FMainFrameCommands::RegisterCommands()
{
	...
	UI_COMMAND( ViewBranchExplorer, "Open Branch Explorer (Beta)", "See all the branches which this project uses.", EUserInterfaceActionType::Button, FInputChord() );
	ActionList->MapAction( ViewBranchExplorer, FExecuteAction::CreateStatic( &FMainFrameActionCallbacks::ViewBranchExplorer ), FCanExecuteAction::CreateStatic( &FMainFrameActionCallbacks::CanViewBranchExplorer ), FGetActionCheckState(), FIsActionButtonVisible::CreateStatic( &FMainFrameActionCallbacks::IsVisibleBranchExplorer ) );
...
void FMainFrameActionCallbacks::ViewBranchExplorer()
{
	ISourceControlWindowsModule::Get().ShowBranchExplorerTab();
}

bool FMainFrameActionCallbacks::CanViewBranchExplorer()
{
	return ISourceControlWindowsModule::Get().CanShowBranchExplorerTab();
}

bool FMainFrameActionCallbacks::IsVisibleBranchExplorer()
{
	return ISourceControlWindowsModule::Get().IsVisibleBranchExplorerTab();
}
...
void FMainFrameActionCallbacks::NewProject( bool bAllowProjectOpening, bool bAllowProjectCreate )
{
	...
	if (const TSharedPtr<SWindow> ExistingWindow = NewProjectWindowWeak.Pin())
	{
		ExistingWindow->ShowWindow();
		ExistingWindow->BringToFront(/* bForce */ false);
	...
	NewProjectWindowWeak = NewProjectWindow;
