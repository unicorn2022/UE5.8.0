...
public:
	...
	TSharedPtr< FUICommandInfo > ViewBranchExplorer;
...
public:
	...
	/** Opens a dialog to view the branch explorer */
	static void ViewBranchExplorer();

	/** Determines whether we can show the branch explorer view */
	static bool CanViewBranchExplorer();

	/** Determines whether we can show the branch explorer menu item */
	static bool IsVisibleBranchExplorer();
...
private:
	...
	/** New project window weak ptr, used to give the window focus if trying to open a new window when there is already an existing one */
	static TWeakPtr<SWindow> NewProjectWindowWeak;
};
