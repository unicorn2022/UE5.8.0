...
{
	// Get the default UE home screen resource entries data
	TArray<FHomeScreenResourceCommunityEntry> GetUEResourceEntries()
	{
		TArray<FHomeScreenResourceCommunityEntry> DefaultEntries;

		FString EngineVersionString = FEngineVersion::Current().ToString(EVersionComponent::Minor);
		EngineVersionString = EngineVersionString.Replace(TEXT("."), TEXT("-"));

		// Forum
		FHomeScreenResourceCommunityEntry ForumEntry;
		ForumEntry.DisplayName = LOCTEXT("ProjectHomeForumsButton", "Forums");
		ForumEntry.Link = TAttribute<FString>::CreateLambda([]()
			{
				const FString FullLocale = FInternationalization::Get().GetCurrentLanguage()->GetName();
				const FString ForumLink = 
					TEXT("https://forums.unrealengine.com/categories?tag=unreal-engine&locale=")
					+ FullLocale;
				return ForumLink;
			});
		ForumEntry.IconBrushName = TEXT("HomeScreen.Forum");
		DefaultEntries.Add(ForumEntry);

		// Documentation
		FHomeScreenResourceCommunityEntry DocumentationEntry;
		DocumentationEntry.DisplayName = LOCTEXT("ProjectHomeDocumentation", "Documentation");
		DocumentationEntry.Link = TAttribute<FString>::CreateLambda([EngineVersionString]()
			{
				const FString FullLocale = FInternationalization::Get().GetCurrentLanguage()->GetName();
				const FString DocumentationLink = 
					TEXT("https://dev.epicgames.com/documentation/")
					+ FullLocale
					+ TEXT("/unreal-engine/unreal-engine-")
					+  EngineVersionString
					+ TEXT("-documentation");
				return DocumentationLink;
			});
		DocumentationEntry.IconBrushName = TEXT("HomeScreen.Documentation");
		DefaultEntries.Add(DocumentationEntry);

		// Tutorial
		FHomeScreenResourceCommunityEntry TutorialEntry;
		TutorialEntry.DisplayName = LOCTEXT("ProjectHomeTutorialsButton", "Tutorials");
		TutorialEntry.Link = TAttribute<FString>::CreateLambda([]()
			{
				const FString FullLocale = FInternationalization::Get().GetCurrentLanguage()->GetName();
				const FString TutorialLink = 
					TEXT("https://dev.epicgames.com/community/unreal-engine/learning?locale=")
					+ FullLocale;
				return TutorialLink;
			});
		TutorialEntry.IconBrushName = TEXT("HomeScreen.Tutorial");
		DefaultEntries.Add(TutorialEntry);

		// Roadmap
		FHomeScreenResourceCommunityEntry RoadmapEntry;
		RoadmapEntry.DisplayName = LOCTEXT("ProjectHomeRoadmapButton", "Roadmap");
		// This link will always point to the latest roadmap and its intended to always point to latest
		RoadmapEntry.Link = TEXT("https://unrealengine.com/roadmap");
		RoadmapEntry.IconBrushName = TEXT("HomeScreen.Roadmap");
		DefaultEntries.Add(RoadmapEntry);

		// Release
		FHomeScreenResourceCommunityEntry ReleaseEntry; 
		ReleaseEntry.DisplayName = LOCTEXT("ProjectHomeReleaseNotesButton", "Release Notes");
		ReleaseEntry.Link = TAttribute<FString>::CreateLambda([EngineVersionString]()
			{
				const FString FullLocale = FInternationalization::Get().GetCurrentLanguage()->GetName();
				const FString ReleaseLink = 
					TEXT("https://dev.epicgames.com/documentation/")
					+ FullLocale
					+ TEXT("/unreal-engine/unreal-engine-")
					+  EngineVersionString
		...
		ReleaseEntry.IconBrushName = TEXT("HomeScreen.ReleaseNotes");
		DefaultEntries.Add(ReleaseEntry);
		
		return DefaultEntries;
	}

	// Get the default UE home screen main section entries data
	FHomeScreenSectionToLinkMap GetUESectionLinks()
	{
		FHomeScreenSectionToLinkMap OutSectionLinks;
		const FString EmptyURL = TEXT("");
		const FString BaseURL = TEXT("https://editor.unrealengine.com");
		OutSectionLinks.Add(EMainSectionMenu::News, 
			{ BaseURL,  TEXT("news"), EmptyURL});
		OutSectionLinks.Add(EMainSectionMenu::GettingStarted, 
			{ BaseURL,  TEXT("get-started-with-unreal-engine"), EmptyURL});
		OutSectionLinks.Add(EMainSectionMenu::SampleProjects, 
			{ BaseURL,  TEXT("samples"), EmptyURL});
		OutSectionLinks.Add(EMainSectionMenu::Home, 
			{ BaseURL,  TEXT("recent-projects"), TEXT("no-projects")});
		return OutSectionLinks;
	}

	TSharedRef<SWidget> GetRecentProjectListWidget()
	{
		return SNew(SBox)
			.Padding(8.f)
			[
				...
				[
					// Embedded should not show the HeaderBar and should not allow scrolling
					FGameProjectGenerationModule::Get()
						.CreateEmbeddedProjectBrowser()
				]
			];
	}

	// Get the default UE home screen settings
	FHomeScreenWidgetSettings GetHomeScreenSettings()
	{
		FHomeScreenWidgetSettings HomeScreenWidgetSettings;
		HomeScreenWidgetSettings.ResourcesEntries = GetUEResourceEntries();
		HomeScreenWidgetSettings.SectionLinks = GetUESectionLinks();
		HomeScreenWidgetSettings.bAlwaysShowRecentList = false;
		HomeScreenWidgetSettings.RecentProjectListWidget = GetRecentProjectListWidget();
		HomeScreenWidgetSettings.UnrealLogoBrush = FAppStyle::GetBrush("HomeScreen.UnrealLogo");
		HomeScreenWidgetSettings.OnGetStartupSetting.BindUObject(GetMutableDefault<UHomeScreenSettings>(), &UHomeScreenSettings::GetLoadAtStartupSetting);
		return HomeScreenWidgetSettings;
	}
}

/* FEditorHomeScreenProvider
 *****************************************************************************/

void FEditorHomeScreenProvider::Register()
{
	IModularFeatures::Get().RegisterModularFeature(IHomeScreenProvider::GetModularFeatureName(), this);
}

void FEditorHomeScreenProvider::UnRegister()
{
	IModularFeatures::Get().UnregisterModularFeature(IHomeScreenProvider::GetModularFeatureName(), this);
}

bool FEditorHomeScreenProvider::IsRequestingHomeScreenPageControl() const
{
	return true;
}

int32 FEditorHomeScreenProvider::GetHomeScreenPriority()
{
	// Lower priority since this is the default behavior
	return -1;
}

FHomeScreenWidgetSettings FEditorHomeScreenProvider::GetHomeScreenWidgetSettings() const
{
	// No need to add settings as we want them to be the default
	return HomeScreenHelper::GetHomeScreenSettings();
}
...
TSharedRef<SWidget> FProjectDialogProvider::CreateMainFrameContentWidget() const
{
	...
	{
		return IMainFrameModule::Get().CreateHomeScreenWidget({}, HomeScreenHelper::GetHomeScreenSettings());
	}
...
void FMainFrameModule::ReconstructDefaultMainFrameContent(const bool bStartImmersive, const bool bStartPIE)
{
	// bRecreatingDefaultMainFrame make sure that the DestroyWindow called on the RootWindow will not be executed
	check(!bRecreatingDefaultMainFrame);
	TGuardValue<bool> GuardRecreatingDefaultMainFrame(bRecreatingDefaultMainFrame, true);
	...
	if (IsWindowInitialized())
	{
		...
		// Iterate every TopLevelWindows and destroy them unless the window is not regular or is the MainFrame window
		TArray<TSharedRef<SWindow>> TopLevelWindows = FSlateApplication::Get().GetTopLevelWindows();
		DestroyRegularAndNonRootWindows(TopLevelWindows);

		// Iterate every visible windows and destroy them unless the window is not regular or is the MainFrame window
		TArray<TSharedRef<SWindow>> OutVisibleWindows;
		FSlateApplication::Get().GetAllVisibleWindowsOrdered(OutVisibleWindows);
		DestroyRegularAndNonRootWindows(OutVisibleWindows);

		// (Re-)populate the default main frame
		CreateDefaultMainFrameAuxiliary(bStartImmersive, bStartPIE, false, true);
	}
}

void FMainFrameModule::CreateDefaultMainFrameAuxiliary(const bool bStartImmersive, const bool bStartPIE, const bool bIsBeingRecreated, const bool bIsJustCreatingContent)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FMainFrameModule::CreateDefaultMainFrame);

	if (!IsWindowInitialized() || bIsJustCreatingContent)
	{
		...
		const FText CloseButtonToolTipText = LOCTEXT("MainFrameModuleCloseButtonToolTip", "Close Unreal Editor");

		// If the parent window is not valid it will be initialized below
		TSharedPtr<SWindow> RootWindowPtr = GetParentWindow();
		if (!RootWindowPtr.IsValid())
		{
			constexpr float MinWidthToAvoidOverlap = 900.f;
			constexpr float MinHeightToAvoidOverlap = 640.f;
...
#if PLATFORM_MAC
				// this is needed to allow Slate-rendered Metal views to show up on XCode's view debugger
				.SupportsTransparency(EWindowTransparency::PerPixel)
#endif
				.SupportsMinimize(WindowConfig.bSupportsMinimize)
				.MinWidth(MinWidthToAvoidOverlap)
				.MinHeight(MinHeightToAvoidOverlap);

			// Avoid using the is initially maximized argument when creating the window so that the code below can change the window without this initial value taking over taking the changes when the window is first shown.
			if (DefaultWindowLocation.InitiallyMaximized)
			{
				RootWindow->Maximize();
			}
			...
			RootWindowPtr = RootWindow;
		}

		// The window has to be valid
		check(RootWindowPtr.IsValid())

		TSharedRef<SWindow> RootWindow = RootWindowPtr.ToSharedRef();

		if (bIsJustCreatingContent)
		{
			if (DefaultWindowLocation.InitiallyMaximized)
			{
				// if initially maximized get the fullscreen size and use that for the size to make the Maximize jump less visible
				// TODO: Should be looking at maximizing without showing the window or to make the InitiallyMaximized work on second window constructs
				DefaultWindowLocation.WindowSize = RootWindow->GetFullScreenInfo().GetSize();
			}
			else if (DefaultWindowLocation.WindowSize.IsNearlyZero())
			{
				// Resize the window with the previous size if not maximized and if the window size was not set (or nearly 0)
				UE::Slate::FDeprecateVector2DResult SizeToRestore = RootWindow->GetSizeInScreen();
				DefaultWindowLocation.WindowSize = SizeToRestore;
			}

			SWindow::FArguments Args;
			Args.AutoCenter(WindowConfig.CenterRules)
				.Title(WindowConfig.WindowTitle)
				.CloseButtonToolTipText(CloseButtonToolTipText)
			...
			RootWindow->Construct(Args);
			// InitiallyMaximized is not correctly used if the window was already created and visible, so do it manually if requested
			if (DefaultWindowLocation.InitiallyMaximized)
			{
				RootWindow->Maximize();
			}
		...
		if (!bShowStartupDialogInPlaceOfMainEditor)
		{
			...
			// IMPORTANT: If you want to change the default value of "LevelEditor_Layout_v1.1" or "UnrealEd_Layout_v1.6" (even if you only change their version numbers), these are the steps to follow:
			...
			// 4. Close the editor.
			// 5. Manually remove Engine\Saved\Config\Layouts\EditorLayout.ini
			...
			// 11. Also update these instructions if you change the version number (e.g., from "UnrealEd_Layout_v1.5" to "UnrealEd_Layout_v1.6").
			const FName LayoutName = TEXT("UnrealEd_Layout_v1.6");
			...
			const TSharedRef<FTabManager::FLayout> LoadedLayout = bIsReCreatingDefaultLayoutAsFallback
				? DefaultLayout
			...
			if (RemovedOlderLayoutVersions.Num() > 0)
			{
				...
				UE_LOGF(LogMainFrame, Warning, "%ls", *WarningText.ToString());
			...
			/**
			 * Workaround a compatibility issue that was introduced in the version 1.5
			 * The engine is supposed to be able to reset the layout for that type of accidental issue but that mechanism has been dysfunctional since 2021 and was fixed in 5.8
			 * This fix will be here for an engine a release to help auto fix corrupt config that were created during the window were the issue existed to avoid the crash in the previous release of the engine caused by this issue.
			 */
			{
				static bool bDidPathIssueInPreviousVersion = false;
				if (!bDidPathIssueInPreviousVersion)
				{
					bDidPathIssueInPreviousVersion = true;
					RemovedOlderLayoutVersions.Reset();

					const FName ProblematicLayoutName = TEXT("UnrealEd_Layout_v1.5");
					const TSharedRef<FTabManager::FLayout> Layout =
						// We persist the positioning of the level editor and the content browser.
						// The asset editors currently do not get saved.
						FTabManager::NewLayout(ProblematicLayoutName)
						->AddArea
						(
							// level editor window
							FTabManager::NewPrimaryArea()
					...
					const TSharedRef<FTabManager::FLayout> LayoutToFix = FLayoutSaveRestore::LoadFromConfig(GEditorLayoutIni, Layout, OutputCanBeNullptr, RemovedOlderLayoutVersions);

					const TSharedPtr<FTabManager::FArea> PrimaryArea = LayoutToFix->GetPrimaryArea().Pin();
					if (PrimaryArea && PrimaryArea->WindowPlacement != FTabManager::FArea::EWindowPlacementInternal::Placement_NoWindow)
					{
						PrimaryArea->WindowPlacement = FTabManager::FArea::EWindowPlacementInternal::Placement_NoWindow;
						FLayoutSaveRestore::SaveToConfig(GEditorLayoutIni, LayoutToFix);
					}
				}
			} 
			...
			if (!MainFrameContent.IsValid())
			{
				...
				// Warn user/developer
				UE_LOGF(LogMainFrame, Warning, "UnrealEd layout could not be loaded from the config files, we will use the default provided by the code");
...
void FMainFrameModule::ShowProjectBrowser(bool bAllowProjectOpening, bool bAllowProjectCreate)
{
	if (ShowProjectBrowserOverride.IsBound())
	{
		ShowProjectBrowserOverride.Execute(bAllowProjectOpening, bAllowProjectCreate);
		return;
	}

	FMainFrameActionCallbacks::NewProject(bAllowProjectOpening, bAllowProjectCreate);
}
...
void FMainFrameModule::StartupModule( )
{
	...
	if (UE::Editor::HomeScreen::IsHomeScreenEnabled())
	{
		FGlobalTabmanager::Get()->RegisterTabSpawner(
			"HomeScreen", FOnSpawnTab::CreateRaw(this, &FMainFrameModule::SpawnHomeScreenWindow))
...
void FMainFrameModule::HandleLevelEditorModuleCompileFinished(const FString& LogDump, ECompilationResult::Type CompilationResult, bool bShowLog)
{
	// Track stats
	{
		...
		UE_LOGF(LogMainFrame, Log, "MainFrame: Module compiling took %.3f seconds", ModuleCompileDuration);
...
TSharedRef<SDockTab> FMainFrameModule::SpawnHomeScreenWindow(const FSpawnTabArgs& InArgs)
{
	TSharedRef<SDockTab> HomeScreenTab = SNew(SDockTab)
		...
		// Assign a display name that will be used when switching tabs with the ctrl + tab feature to remove this tab from the list
		.Label(IMainFrameModule::GetHomeScreenTabLabel())
	...
	HomeScreenTab->SetOnTabClosed(SDockTab::FOnTabClosedCallback::CreateLambda([this] (TSharedRef<SDockTab>) { HomeScreenTabManager.Reset(); }));

	HomeScreenTab->SetTabIcon(FAppStyle::Get().GetBrush("HomeScreen.Home"));

	TArray<IHomeScreenProvider*> HomeScreenProviders = IModularFeatures::Get().GetModularFeatureImplementations<IHomeScreenProvider>(IHomeScreenProvider::GetModularFeatureName());		
	IHomeScreenProvider* ActiveProvider = nullptr;
	for (IHomeScreenProvider* Provider : HomeScreenProviders)
	{
		if (Provider && Provider->IsRequestingHomeScreenPageControl())
		{
			if (!ActiveProvider || Provider->GetHomeScreenPriority() > ActiveProvider->GetHomeScreenPriority())
			{
	...
	TSharedPtr<SWidget> HomeScreenWidget;
	if (ActiveProvider)
	{
		HomeScreenWidget = CreateHomeScreenWidget(HomeScreenTab, ActiveProvider->GetHomeScreenWidgetSettings());
	}
	else
	{
		HomeScreenWidget = CreateHomeScreenWidget(HomeScreenTab);
	}
	...
	HomeScreenTab->SetContent(HomeScreenWidget.ToSharedRef());

	return HomeScreenTab;
}

TSharedRef<SWidget> FMainFrameModule::CreateHomeScreenWidget(const TSharedPtr<SDockTab>& InParentDockTab, FHomeScreenWidgetSettings InSettings)
{
	return SNew(SHomeScreen)
		.SectionLinks(InSettings.SectionLinks)
		.ResourceCommunityEntries(InSettings.ResourcesEntries)
		.AlwaysShowRecentList(InSettings.bAlwaysShowRecentList)
		.RecentProjectList(InSettings.RecentProjectListWidget)
		.UnrealLogoBrush(InSettings.UnrealLogoBrush)
		.OnGetStartupSetting(InSettings.OnGetStartupSetting);
}
...
void FMainFrameModule::DestroyRegularAndNonRootWindows(TArray<TSharedRef<SWindow>> InWindowsToDestroy) const
{
	// If the root window was destroyed skip this
	if (IsWindowInitialized())
	{
		// Get the root window
		TSharedRef<SWindow> RootWindow = GetParentWindow().ToSharedRef();
		
		// ::RequestDestroyWindow will remove the window from the TArray so we
		// should iterate over it backwards to make sure that the WindowIndex is still correct.
		for (int32 WindowIndex = InWindowsToDestroy.Num() -1; WindowIndex >= 0; --WindowIndex)
		{
			// Skip the root window
			if (InWindowsToDestroy[WindowIndex] == RootWindow)
			{
				continue;
			}

			// If it is not a regular window skip it since it may be a pop-up window containing a progress bar for example
			const bool bIsRegularWindow = InWindowsToDestroy[WindowIndex]->IsRegularWindow();
			if (!bIsRegularWindow)
			{
				continue;
			}

			InWindowsToDestroy[WindowIndex]->RequestDestroyWindow();
		}
		
		// Destroy the windows immediately from the WindowDestroyQueue
