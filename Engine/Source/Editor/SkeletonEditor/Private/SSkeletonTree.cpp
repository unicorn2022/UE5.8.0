...
public:
	...
	{
		if ( this->GetSelectionMode() == ESelectionMode::None )
		{
			return;
		}
	...
	{
		CachedSparseItemInfos = STreeView<ItemType>::SparseItemInfos;
	}
	
	/** Restores sparse item infos from last known infos */
	void RestoreSparseItemInfos(const TArray<TSharedPtr<ISkeletonTreeItem>> TreeItems)
	{		
		for (auto It = STreeView<ItemType>::SparseItemInfos.CreateIterator(); It; ++It)
		{
		...
		for (const auto& [TreeItem, SparseItemInfo] : CachedSparseItemInfos)
		{
			const TSharedPtr<ISkeletonTreeItem>* TreeItemPtr = TreeItems.FindByPredicate(
				[&TreeItem](const TSharedPtr<ISkeletonTreeItem>& OtherTreeItem)
				{
					return TreeItem->GetRowItemName() == OtherTreeItem->GetRowItemName();
				});
				
			if (TreeItemPtr)
			{
				STreeView<ItemType>::SparseItemInfos.Add(*TreeItemPtr, SparseItemInfo);
			}
		}
	}
...
private:
	...
	STreeView<ItemType>::TSparseItemMap CachedSparseItemInfos;
};
...
void SSkeletonTree::Construct(const FArguments& InArgs, const TSharedRef<FEditableSkeleton>& InEditableSkeleton, const FSkeletonTreeArgs& InSkeletonTreeArgs)
{
	...
	bAllowInvisibleItemSelection = InSkeletonTreeArgs.bAllowInvisibleItemSelection;
		...
		[
			...
			[
				...
				[
					...
					.Visibility_Lambda([this](){ return bAllowSkeletonOperations ? EVisibility::Visible : EVisibility::Collapsed; })
...
void SSkeletonTree::BindCommands()
{
	...
	CommandList.MapAction(
		...
		FIsActionChecked(),
		FIsActionButtonVisible::CreateSP(this, &SSkeletonTree::ShouldAllowSkeletonOperations));
	...
	CommandList.MapAction(
		...
		FCanExecuteAction(),
		FIsActionChecked(),
		FIsActionButtonVisible::CreateSP(this, &SSkeletonTree::ShouldAllowMeshOperations));
...
void SSkeletonTree::CreateTreeColumns()
{
...

	{
		...
		.AllowInvisibleItemSelection(bAllowInvisibleItemSelection)
		.HeaderRow
		(
			TreeHeaderRow
		);
...
{	
	SkeletonTreeView->StoreSparseItemInfos();
	...
	SkeletonTreeView->RestoreSparseItemInfos(LinearItems);
}
	...
	FMenuBuilder MenuBuilder( CloseAfterSelection, UICommandList, Extenders );
	{
		...
		if (bAllowSkeletonOperations)
		{
			if(BoneTreeSelection.HasSelectedOfType<FSkeletonTreeBoneItem>())
			{
				...
				if(IsShowingAdvancedOptions() && bAllowSkeletonOperations)
				{
		...
		if(bAllowMeshOperations)
		{
			MenuBuilder.BeginSection("SkeletonTreeBoneReductionForLOD", LOCTEXT("BoneReductionHeader", "LOD Bone Reduction"));
			{
				...
				MenuBuilder.AddMenuEntry(
					LOCTEXT("SkeletonTreeBoneReductionForLOD_RemoveAllUnskinnedBones", "Remove All Unused")
					, LOCTEXT("SkeletonTreeBoneReductionForLOD_RemoveAllUnskinnedBonesTooltip", "Remove all bones that have no vertices weighted against them from LOD 0 and below. Parent bones, and parents of parents of needed bones will be kept.")
					, FSlateIcon()
					, FUIAction(FExecuteAction::CreateSP(this, &SSkeletonTree::RemoveFromLOD, 0, EBoneReductionSelectionMode::AllUnusedBones, true))
					);
			}
			MenuBuilder.EndSection();
		}
		...
		if (bAllowSkeletonOperations)
		{
			...
			if(BoneTreeSelection.HasSelectedOfType<FSkeletonTreeSocketItem>())
			{
				...
				if(BoneTreeSelection.IsSingleOfTypeSelected<FSkeletonTreeSocketItem>())
				{
					...
					if (SocketItem->IsSocketCustomized() && SocketItem->GetParentType() == ESocketParentType::Mesh && bAllowMeshOperations)
					{
					...
					{
						if (SocketItem->GetParentType() == ESocketParentType::Skeleton && bAllowMeshOperations)
						{
...
void SSkeletonTree::OnVirtualTargetBonePicked(FName TargetBoneName, TArray<TSharedPtr<ISkeletonTreeItem>> SourceBones)
{
	...
	for (const TSharedPtr<ISkeletonTreeItem>& SourceBone : SourceBones)
	{
		...
		if(GetSourceNameFromItem(SourceBone, SourceBoneName))
		{
			...
			{
				UE_LOGF(LogAnimation, Log, "Could not create space switch bone from %ls to %ls, it already exists", *SourceBoneName.ToString(), *TargetBoneName.ToString());
			}
			else
			{
...
{
	const EBoneReductionSelectionMode SelectionMode = bIncludeSelected ? EBoneReductionSelectionMode::SelectedBones : EBoneReductionSelectionMode::ChildrenOfSelectedBones;
	MenuBuilder.AddMenuEntry
	(FText::FromString(FString::Printf(TEXT("From LOD %d and below"), LODIndex))
		...
		, FSlateIcon()
		, FUIAction(FExecuteAction::CreateSP(Widget, &SSkeletonTree::RemoveFromLOD, LODIndex, SelectionMode, true))
		);
	...
	(FText::FromString(FString::Printf(TEXT("From LOD %d only"), LODIndex))
		...
		, FSlateIcon()
		, FUIAction(FExecuteAction::CreateSP(Widget, &SSkeletonTree::RemoveFromLOD, LODIndex, SelectionMode, false))
		);
}
...
void SSkeletonTree::RemoveFromLOD(int32 LODIndex, EBoneReductionSelectionMode SelectionMode, bool bIncludeBelowLODs)
{
	...
	if (FMessageDialog::Open(EAppMsgType::YesNo, Message) == EAppReturnType::Yes)
	{
		...
		TArray<FName> ForceKeepBones;

		if (SelectionMode == EBoneReductionSelectionMode::AllUnusedBones)
		{
		...
		else
		{
			for (const TSharedPtr<FSkeletonTreeBoneItem>& Item : TreeSelection.GetSelectedItems<FSkeletonTreeBoneItem>())
			{
				...
				{
					if (SelectionMode == EBoneReductionSelectionMode::SelectedBones)
					{
		...
		MeshBoneReduction.ReduceBoneCounts(SkeletalMesh, ForceKeepBones, BonesToRemove, LODIndex, bIncludeBelowLODs);
...
TArray<TSharedPtr<ISkeletonTreeItem>> SSkeletonTree::GetSelectedItems() const 
{ 
	return SkeletonTreeView->GetSelectedItems(); 
}
...
void SSkeletonTree::RegisterNewMenu()
{
...

	{
		...
		CreateSection.AddDynamicEntry("SocketAndVirtualBone", FNewToolMenuSectionDelegate::CreateLambda([](FToolMenuSection& InSection)
		{
			USkeletonTreeMenuContext* Context = InSection.FindContext<USkeletonTreeMenuContext>();
			if (!Context)
			{
				return;
			}

			TSharedPtr<SSkeletonTree> SkeletonTree = Context->SkeletonTree.Pin();
			if (!SkeletonTree.IsValid() || !SkeletonTree->ShouldAllowSkeletonOperations())
			{
				return;
			}

			const FSkeletonTreeCommands& Actions = FSkeletonTreeCommands::Get();
			InSection.AddMenuEntry(Actions.AddSocket);
				...
				FNewToolMenuDelegate::CreateLambda([](UToolMenu* InMenu)
					{
						...
						TSharedPtr<SSkeletonTree> InnerSkeletonTree = MenuContext->SkeletonTree.Pin();
						if (!InnerSkeletonTree.IsValid())
						{
							return;
						}

						TSharedRef<SBoneTreeMenu> MenuContent = SSkeletonTree::CreateVirtualBoneMenu(InnerSkeletonTree.Get());
...

	{
		...
		BlendSection.AddDynamicEntry("BlendProfileEntries", FNewToolMenuSectionDelegate::CreateLambda([](FToolMenuSection& InSection)
		{
			USkeletonTreeMenuContext* Context = InSection.FindContext<USkeletonTreeMenuContext>();
			if (!Context)
			{
			...
			const FSkeletonTreeCommands& Actions = FSkeletonTreeCommands::Get();
			InSection.AddMenuEntry(Actions.CreateTimeBlendProfile);
			InSection.AddMenuEntry(Actions.CreateWeightBlendProfile);
			InSection.AddMenuEntry(Actions.CreateBlendMask);
		}));
	}
}
