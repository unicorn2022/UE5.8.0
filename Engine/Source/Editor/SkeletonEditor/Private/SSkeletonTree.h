...
class SComboButton;
template <typename ItemType> class SSkeletonTreeView;
class UBlendProfile;
class UToolMenu;
...
public:
	...
	virtual TArray<TSharedPtr<ISkeletonTreeItem>> GetSelectedItems() const override;
	...
	bool ShouldAllowSkeletonOperations() const { return bAllowSkeletonOperations; }
	bool ShouldAllowMeshOperations() const { return bAllowMeshOperations; }
	...
	enum class EBoneReductionSelectionMode : uint8
	{
		SelectedBones,
		ChildrenOfSelectedBones,
		AllUnusedBones
	};
	void RemoveFromLOD(int32 LODIndex, EBoneReductionSelectionMode SelectionMode, bool bIncludeBelowLODs);

	/** Remove the selected bones from LOD of LODIndex when using simplygon **/
	UE_DEPRECATED(5.8, "RemoveFromLOD(int32, bool, bool) is deprecated. Use RemoveFromLOD(int32, bool, ERemoveFromLODMode).")
	inline void RemoveFromLOD(int32 LODIndex, bool bIncludeSelected, bool bIncludeBelowLODs)
	{
		...
		RemoveFromLOD(LODIndex, SelectionMode, bIncludeBelowLODs);
	}
...
private:
	...
	TSharedPtr<SSkeletonTreeView<TSharedPtr<class ISkeletonTreeItem>>> SkeletonTreeView;
	...
	/** Wether children can remain selected while their parent is collapsed in the tree */
