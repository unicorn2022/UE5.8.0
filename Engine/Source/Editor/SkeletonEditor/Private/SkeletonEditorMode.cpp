...
{
	FWorkflowAllowedTabSet& TabFactories = ApplicationModeTabFactories;
	...
	ViewportArgs.OnViewportCreated = FOnViewportCreated::CreateSP(&SkeletonEditor.Get(), &FSkeletonEditor::HandleViewportCreated);
...
{
	if (TSharedPtr<FWorkflowCentricApplication> HostingApp = GetHost())
	{
		TSharedRef<FTabManager> TabManager = InTabManager.ToSharedRef();
		HostingApp->RegisterTabSpawners(TabManager);
		RegisterTabFactoriesWithAppAndManager(HostingApp.Get(), TabManager);
	}
}
