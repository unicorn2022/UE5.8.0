...
namespace UE::AssetView
{
	...
	bool AreBackendFiltersDifferent(const FARFilter& A, const FARFilter& B)
	{
		...
		|| A.PackagePaths.Num() != B.PackagePaths.Num()
	...
	FAssetViewFrontendFilterHelper& InHelper, double InEndTime, TArray<TSharedPtr<FAssetViewItem>>& OutItems)
{
	...
	} while(FPlatformTime::Seconds() < InEndTime && FrontendFilterProgress < Items.Num());
...
SAssetView::~SAssetView()
{
	...
	for (const FCollectionRef& Collection : ContentSources.GetCollections())
	{
		...
		if (!bIsAlreadyInSet)
		{
			Collection.Container->OnAssetsAddedToCollection().RemoveAll(this);
			Collection.Container->OnAssetsRemovedFromCollection().RemoveAll(this);
			Collection.Container->OnCollectionRenamed().RemoveAll(this);
			Collection.Container->OnCollectionUpdated().RemoveAll(this);
		}
	}
...
void SAssetView::Construct( const FArguments& InArgs )
{
	...
	AssetShowWarningTextJustification = InArgs._AssetShowWarningTextJustification;
	AssetShowWarningTextSelectable = InArgs._AssetShowWarningTextSelectable;
	...
	OnColumnVisibilityChanged = InArgs._OnColumnVisibilityChanged;
	...
	[
		...
		[
			...
			[
				...
				.Padding(8.f)
				[
					// A warning to display when there are no assets to show, allow text copy
					SNew(SMultiLineEditableText)
					.IsReadOnly(true)
					.Justification(AssetShowWarningTextJustification)
					.Text(this, &SAssetView::GetAssetShowWarningText)
					.AutoWrapText(true)
					.IsEnabled(AssetShowWarningTextSelectable)
				]
			]
...
void SAssetView::NewFolderItemRequested(const FContentBrowserItemTemporaryContext& NewItemContext)
{
	...
	UE_LOGF(LogContentBrowser, Log, "Deferred new asset folder creation: %ls", *NewItemContext.GetItem().GetItemName().ToString());
}
...
void SAssetView::NewFileItemRequested(const FContentBrowserItemDataTemporaryContext& NewItemContext)
{
	...
	UE_LOGF(LogContentBrowser, Log, "Deferred new asset file creation: %ls", *NewItemContext.GetItemData().GetItemName().ToString());
}
...
void SAssetView::BeginCreateDeferredItem()
{
	if (DeferredItemToCreate.IsValid() && !DeferredItemToCreate->bWasAddedToView)
	{
		...
		UE_LOGF(LogContentBrowser, Log, "Creating deferred item: %ls", *NewItem->GetItem().GetItemName().ToString());
	}
}
...
FContentBrowserItem SAssetView::EndCreateDeferredItem(const TSharedPtr<FAssetViewItem>& InItem, const FString& InName, const bool bFinalize, FText& OutErrorText)
{
	...
	UE_LOGF(LogContentBrowser, Log, "End creating deferred item %ls", *InItem->GetItem().GetItemName().ToString());
...
{
	SyncToItems(ContentBrowserUtils::ConvertLegacySelectionToItems(AssetDataList, FolderList), bFocusOnSync);
}
...
bool SAssetView::HasItemsPendingFilter() const
{
	...
	if (AssetSearchOverride.IsValid() && AssetSearchOverride->IsOverrideActive() && (AssetSearchOverride->HasPendingResults() || !AssetSearchOverride->GetInternalApi().PublishedResults.IsEmpty()))
	{
		return true;
	}
...
void SAssetView::ProcessItemsPendingFilter(const double TickStartTime)
{
	...
	TArray<TSharedPtr<FAssetViewItem>> FilterResults;

	Items->UpdateItemFiltering(FrontendFilterHelper, bFlushAllPendingItems ? MAX_dbl : TickStartTime + MaxSecondsPerFrame, FilterResults);

	PRAGMA_DISABLE_EXPERIMENTAL_WARNINGS
		if (AssetSearchOverride.IsValid() && AssetSearchOverride->IsOverrideActive())
		{
			AssetSearchOverride->OnItemsAvailable(MakeArrayView(FilterResults));
			FilterResults.Reset();

			FilterResults.Append(MoveTemp(AssetSearchOverride->GetInternalApi().PublishedResults));
			AssetSearchOverride->GetInternalApi().PublishedResults.Reset();

			if (!Items->HasItemsPendingFilter())
			{
				AssetSearchOverride->GetInternalApi().OnAllKnownItemsAvailable();
			}
		}
	PRAGMA_ENABLE_EXPERIMENTAL_WARNINGS
	...
	// exclude duplicates
	for (TSharedPtr<FAssetViewItem>& Result : FilterResults)
	{
		bool bAlreadyAdded = false;
		const FContentBrowserItem& Item = Result->GetItem();
		
		if (FSoftObjectPath ItemPath; Item.IsValid() && Item.GetItemAliasedPath(ItemPath))
		{
			AlreadyAddedOriginalAssets.Add(ItemPath, &bAlreadyAdded);
		}
		if (!bAlreadyAdded)
		{
			FilteredAssetItems.Add(MoveTemp(Result));
		}
	}
	...
	UE_LOGF(LogContentBrowser, VeryVerbose, "AssetView - ProcessItemsPendingFilter completed in %0.4f seconds", FPlatformTime::Seconds() - ProcessItemsPendingFilterStartTime);
}
...
TSharedRef<SAssetListView> SAssetView::CreateListView()
{
	...
	TSharedPtr<SAssetListView> NewListView = SNew(SAssetListView)
		...
		(
			...
			.DefaultTooltip(LOCTEXT("Column_RC_Tooltip", "Revision Control Status"))
			[
...
void SAssetView::RefreshSourceItems()
{
	...
	UE_LOGF(LogContentBrowser, VeryVerbose, "AssetView - RefreshSourceItems completed in %0.4f seconds", 
...
bool SAssetView::ShouldFilterRecursively() const
{
	...
	if (IsFilteringRecursively())
	{
		...
		if (FrontendFilters.IsValid())
		{
			for (const TSharedPtr<IFilter<FAssetFilterType>>& Filter : *FrontendFilters.Get())
			{
				if (const auto* FrontendFilter = static_cast<FFrontendFilter*>(Filter.Get()))
				{
...
void SAssetView::RefreshFilteredItems()
{
	...
	if (AssetSearchOverride.IsValid() && AssetSearchOverride->IsOverrideActive())
	{
		UE::Editor::ContentBrowser::Extension::FInternalApiWrapper& SearchOverrideApi = AssetSearchOverride->GetInternalApi();
		SearchOverrideApi.OnFilteringReset();
	}
	PRAGMA_ENABLE_EXPERIMENTAL_WARNINGS
	...
	if (IsFrontendFilterActive() && FrontendFilters.IsValid())
	{
		...
		for (const TSharedPtr<IFilter<FAssetFilterType>>& Filter : *FrontendFilters.Get())
		{
			...
			const TSharedPtr<FFrontendFilter>& FrontendFilter = StaticCastSharedPtr<FFrontendFilter>(Filter);
			if (FrontendFilter.IsValid())
			{
				FrontendFilter->SetCurrentFilter(DataSourcePaths, DataFilter);
			}
		}
	}

	UE_LOGF(LogContentBrowser, VeryVerbose, "AssetView - RefreshFilteredItems completed in %0.4f seconds", FPlatformTime::Seconds() - RefreshFilteredItemsStartTime);
}
...
void SAssetView::SetMajorityAssetType(FName NewMajorityAssetType)
{
	...
	bool bHasDynamicColumns = ContentBrowserModule.IsDynamicTagAssetClass(FTopLevelAssetPath(NewMajorityAssetType.ToString()));

	if ( NewMajorityAssetType != MajorityAssetType || bHasDynamicColumns)
	{
		UE_LOGF(LogContentBrowser, Verbose, "The majority of assets in the view are of type: %ls", *NewMajorityAssetType.ToString());

		MajorityAssetType = NewMajorityAssetType;
		MajorityAssetTypeText = FText::GetEmpty();
		...
		if (NewMajorityAssetType != NAME_None)
		{
			...
			MajorityAssetTypeText = MajorityAssetItem ? MajorityAssetItem->GetAssetClassText() : FText::GetEmpty();
...
void SAssetView::OnFrontendFiltersChanged()
{
	...
	{ 
		...
		for (const TSharedPtr<IFilter<FAssetFilterType>>& Filter : *FrontendFilters.Get())
		{
			...
			if (Filter.IsValid())
			{
				TOptional<FText> Text = FrontendFilter->GetAsCustomTextFilter();
				if (Text.IsSet())
				{
...
void SAssetView::SortList(bool bSyncToSelection)
{
	...
	{
		PRAGMA_DISABLE_EXPERIMENTAL_WARNINGS
		if (AssetSearchOverride.IsValid() && AssetSearchOverride->IsOverrideActive() && AssetSearchOverride->IsSortOverridden())
		{
			AssetSearchOverride->SortItemList(FilteredAssetItems);
		}
		PRAGMA_ENABLE_EXPERIMENTAL_WARNINGS
		else
		{
...
void SAssetView::PopulateSortingButtonMenu(UToolMenu* InToolMenu)
{
	...
	FToolMenuSection& SortBySection = InToolMenu->AddSection("SortBy", LOCTEXT("SortByHeading", "Sort By"));
	{
		...
		static const TPair<FName, FText> SortableColumnIds[] =
		{
			{ FAssetViewSortManager::NameColumnId, LOCTEXT("SortByOptionLabel_Name", "Name") }, { FAssetViewSortManager::DiskSizeColumnId, LOCTEXT("SortByOptionLabel_Size", "Size") }
		};

		for (const TPair<FName, FText>& SortableColumnId : SortableColumnIds)
		{
			const TOptional<const FName> ColumnId = SortableColumnId.Key;
			...
			const FText& SortableColumnLabel = SortableColumnId.Value;
...
TSharedRef<ITableRow> SAssetView::MakeTileViewWidget(TSharedPtr<FAssetViewItem> AssetItem, const TSharedRef<STableViewBase>& OwnerTable)
{
	...
	else
	{
		...
		if (!AssetThumbnail)
		{
			AssetThumbnail = MakeShared<FAssetThumbnail>(FAssetData(), CurrentThumbnailSize, CurrentThumbnailSize, AssetThumbnailPool);
			AssetItem->GetItem().UpdateThumbnail(*AssetThumbnail);
			AssetThumbnail->GetViewportRenderTargetTexture(); // Access the texture once to trigger it to render
		}
...
FReply SAssetView::OnDraggingAssetItem( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
	if (bAllowDragging)
	{
		...
		if (MouseEvent.IsMouseButtonDown(EKeys::LeftMouseButton))
		{
			...
			{
				OnItemsDragged.ExecuteIfBound(SelectedItems);
...
void SAssetView::AssetRenameBegin(const TSharedPtr<FAssetViewItem>& Item, const FString& NewName, const FSlateRect& MessageAnchor)
{
	...
	{
		UE_LOGF(LogContentBrowser, Log, "Renaming the item being created (Deferred Item: %ls).", *Item->GetItem().GetItemName().ToString());
	}
}
...
void SAssetView::AssetRenameCommit(const TSharedPtr<FAssetViewItem>& Item, const FString& NewName, const FSlateRect& MessageAnchor, const ETextCommit::Type CommitType)
{
	...
	UE_LOGF(LogContentBrowser, Log, "Attempting asset rename: %ls -> %ls", *Item->GetItem().GetItemName().ToString(), *NewName);
...
EVisibility SAssetView::GetMajorityAssetTypeTextVisibility() const
{
	return (CurrentViewType == EAssetViewType::Column) ? EVisibility::Visible : EVisibility::Collapsed;
}

FText SAssetView::GetMajorityAssetTypeText() const
{
	if ((CurrentViewType != EAssetViewType::Column) || MajorityAssetType.IsNone() || MajorityAssetTypeText.IsEmpty())
	{
		return FText::GetEmpty();
	}

	const uint32 NumVisibleColumns = IntCastChecked<uint32>(Algo::CountBy(ColumnView->GetHeaderRow()->GetColumns(), /*InValue = */true, &SHeaderRow::FColumn::bIsVisible));

	return FText::Format(LOCTEXT("MajorityAssetTypeText", "Showing {0} {0}|plural(one=column,other=columns) for asset type \"{1}\""), 
		FText::AsNumber(NumVisibleColumns), MajorityAssetTypeText);
}

FText SAssetView::GetMajorityAssetTypeToolTipText() const
{
	if ((CurrentViewType != EAssetViewType::Column) || MajorityAssetType.IsNone() || MajorityAssetTypeText.IsEmpty())
	{
		return FText::GetEmpty();
	...
	return LOCTEXT("MajorityAssetTypeTooltip", "This view shows columns for the asset type with the most occurrences. \n\r"
		"\n\r"
		"If you need to see data for a different asset type, try filtering the view to show only assets of that type.");
}
...
{
	const bool bTextSelectable = AssetShowWarningTextSelectable.Get(/* Default **/false);
	return (FilteredAssetItems.Num() > 0 || bQuickFrontendListRefreshRequested) ? EVisibility::Collapsed : (bTextSelectable ? EVisibility::SelfHitTestInvisible : EVisibility::HitTestInvisible);
}
...
{
	if (const UContentBrowserSettings* ContentBrowserSettings = GetDefault<UContentBrowserSettings>())
	{
		if (!ContentBrowserSettings->bDisplayQuickJumpText)
		{
...
void SAssetView::OnHiddenColumnsChanged()
{
	...
	OnColumnVisibilityChanged.ExecuteIfBound(*HiddenColumnsToUse);
}
...
void SAssetView::HandleItemDataUpdated(TArrayView<const FContentBrowserItemDataUpdate> InUpdatedItems)
{
	...
	UE_LOGF(LogContentBrowser, VeryVerbose, "AssetView - HandleItemDataUpdated completed in %0.4f seconds for %d items (%d available items)", 
...
void SAssetView::SetAssetSearchOverride(TSharedPtr<UE::Editor::ContentBrowser::Extension::ICBSearchOverrideInstance> InOverride)
{
	AssetSearchOverride = MoveTemp(InOverride);
	if (AssetSearchOverride.IsValid())
	{
		AssetSearchOverride->GetInternalApi().OnRequestSetUserSearching.BindSP(this, &SAssetView::SetUserSearching);
		AssetSearchOverride->GetInternalApi().OnRequestQuickFrontendListRefresh.BindSP(this, &SAssetView::RequestQuickFrontendListRefresh);
	}
	RequestQuickFrontendListRefresh();
}
...
void SAssetView::CancelPendingRename()
{
	if (IsRenamingAsset())
	{
		if (TSharedPtr<FAssetViewItem> Item = RenamingAsset.Pin())
		{
			/**
			...
			if (Item && Item->IsTemporary())
			{
				...
				{
					DeferredItemToCreate.Reset();
					Item->OnRenameCanceled().ExecuteIfBound();
				}
			}
			else
			{
