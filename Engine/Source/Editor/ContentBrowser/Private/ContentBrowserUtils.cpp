...
bool ContentBrowserUtils::DisplayConfirmationDialog(const FText& InTitle, const FText& InMessage)
{
	return FMessageDialog::Open(EAppMsgType::YesNo, InMessage, InTitle) == EAppReturnType::Type::Yes;
}
...
TArray<FContentBrowserItem> ContentBrowserUtils::ConvertLegacySelectionToItems(TArrayView<const FAssetData> InAssets, TArrayView<const FString> InFolders)
{
	UContentBrowserDataSubsystem* ContentBrowserData = IContentBrowserDataModule::Get().GetSubsystem();

	TArray<FContentBrowserItem> Items;
	Items.Reserve(InAssets.Num() + InFolders.Num());
	...
	if (!InAssets.IsEmpty())
	{
		...
		VirtualPaths.Reserve(InAssets.Num());

		auto AppendVirtualPath = [&VirtualPaths](FName VirtualPath)
		{
		...
		{
			if (FolderPermissions->PassesStartsWithFilter(AssetData.GetObjectPathString()))
			{
				const int32 PreviousNumVirtualPaths = VirtualPaths.Num();
				ContentBrowserData->Legacy_TryConvertAssetDataToVirtualPaths(AssetData, /*UseFolderPaths*/false, AppendVirtualPath);
				if (VirtualPaths.Num() > PreviousNumVirtualPaths)
				{
					// If we found a virtual path, continue, otherwise the item may only exist as an alias, so try those next.
					continue;
				}
			}

			TArray<FContentBrowserItemPath> Aliases = ContentBrowserData->GetAliasesForPath(*AssetData.GetObjectPathString());
			for (const FContentBrowserItemPath& Alias : Aliases)
			{
				if (FolderPermissions->PassesStartsWithFilter(Alias.GetInternalPathName()))
				{
					VirtualPaths.Add(Alias.GetVirtualPathName());
					break;
				}
			}
		}
		...
		{
			FContentBrowserItem ItemToSync = ContentBrowserData->GetItemAtPath(VirtualPath, EContentBrowserItemTypeFilter::IncludeFiles);
			if (ItemToSync.IsValid())
			{
				Items.Add(MoveTemp(ItemToSync));
			}
		}
	}
	...
	{
		TArray<FName> VirtualPaths;
		VirtualPaths.Reserve(InFolders.Num());
		...
		for (const FName& VirtualPath : VirtualPaths)
		{
			FContentBrowserItem ItemToSync = ContentBrowserData->GetItemAtPath(VirtualPath, EContentBrowserItemTypeFilter::IncludeFolders);
			if (ItemToSync.IsValid())
			{
				Items.Add(MoveTemp(ItemToSync));
			}
...
bool ContentBrowserUtils::IsFavoriteFolder(const FContentBrowserItemPath& FolderPath)
{
	...
	{
		if (const UContentBrowserFavoriteProjectSettings* FavoriteSettings = UContentBrowserFavoriteProjectSettings::Get())
		{
			const FContentBrowserFavoriteItem Favorite
			{
				.Path = InvariantPath.ToString()
			};
			return FavoriteSettings->FavoriteItems.Contains(Favorite);
		}
	}
...
{
	// Insert at the end
	InsertFavoriteFolder(FolderPath, FContentBrowserItemPath(), EItemDropZone::OntoItem);
}

void ContentBrowserUtils::InsertFavoriteFolder(const FContentBrowserItemPath& InFolderPath, const FContentBrowserItemPath& InFolderContext, EItemDropZone InPositionContext)
{
	const FName InvariantPath = ContentBrowserUtils::GetInvariantPath(InFolderPath);
	if (InvariantPath.IsNone())
	{
		return;
	}
	
	const FName TargetFolderPath = ContentBrowserUtils::GetInvariantPath(InFolderContext);
	const FString TargetFolder = TargetFolderPath.ToString();

	// Do not move item on itself
	if (InvariantPath == TargetFolderPath)
	{
		return;
	}
	
	UContentBrowserFavoriteProjectSettings* FavoriteSettings = UContentBrowserFavoriteProjectSettings::Get();
	if (!FavoriteSettings)
	{
		return;
	}
	
	FContentBrowserFavoriteItem FavoriteItem
	{
		.Path = InvariantPath.ToString()
	};
	const int32 CurrentIndex = FavoriteSettings->FavoriteItems.Find(FavoriteItem);
	
	// Item already exists, do not proceed further
	...
	if (!TargetFolderPath.IsNone())
	{
		const FContentBrowserFavoriteItem TargetItem
		{
			.Path = TargetFolder
		};

		TargetIndex = FavoriteSettings->FavoriteItems.Find(TargetItem);
		
		// Target item was not found, do not proceed further
	...
	// If no target specified, move it at the end
	if (TargetIndex == INDEX_NONE)
	{
		FavoriteSettings->FavoriteItems.Add(FavoriteItem);
	}
	else
	{
		FavoriteSettings->FavoriteItems.Insert(
			FavoriteItem,
			InPositionContext == EItemDropZone::AboveItem ? TargetIndex : TargetIndex + 1
		);
	}

	FavoriteSettings->SaveConfig();
	FContentBrowserSingleton::Get().BroadcastFavoritesChanged(InFolderPath, /** Added */true);
}

void ContentBrowserUtils::MoveFavoriteFolder(const FContentBrowserItemPath& InFolderPath, const FContentBrowserItemPath& InFolderContext, EItemDropZone InPositionContext)
{
	const FName InvariantPath = ContentBrowserUtils::GetInvariantPath(InFolderPath);
	if (InvariantPath.IsNone())
	{
	...
	// Item does not exist, do not proceed further
	if (CurrentIndex == INDEX_NONE)
	{
		return;
	}
	...
	// In case of a move, get the actual item with all its options first then remove it to insert it again
	FavoriteItem = FavoriteSettings->FavoriteItems[CurrentIndex];
	FavoriteSettings->FavoriteItems.RemoveAt(CurrentIndex, EAllowShrinking::No);
	...
	FavoriteSettings->SaveConfig();
	FContentBrowserSingleton::Get().BroadcastFavoritesChanged(InFolderPath, /** Added */false);
}

FContentBrowserFavoriteItem* ContentBrowserUtils::FindFavoriteFolder(const FContentBrowserItemPath& InFolderPath)
{
	const FName InvariantPath = ContentBrowserUtils::GetInvariantPath(InFolderPath);
	if (InvariantPath.IsNone())
	{
	...
	return FavoriteSettings->FavoriteItems.FindByPredicate(
		[Path = InvariantPath.ToString()](const FContentBrowserFavoriteItem& InItem)
		{
			return InItem.Path.Equals(Path);
		}
	);
}
...
void ContentBrowserUtils::RemoveFavoriteFolder(const FContentBrowserItemPath& FolderPath)
{
	...
	const FContentBrowserFavoriteItem FavoriteItem
	{
		.Path = InvariantPath.ToString()
	};

	if (FavoriteSettings->FavoriteItems.Remove(FavoriteItem) > 0)
	{
		FavoriteSettings->SaveConfig();
...
TArray<FString> ContentBrowserUtils::GetFavoriteFolders()
{
	TArray<FString> FavoriteFolders;
	if (const UContentBrowserFavoriteProjectSettings* FavoriteSettings = UContentBrowserFavoriteProjectSettings::Get())
	{
		FavoriteFolders.Reserve(FavoriteSettings->FavoriteItems.Num());
		for (const FContentBrowserFavoriteItem& FavoriteItem : FavoriteSettings->FavoriteItems)
		{
			FavoriteFolders.Emplace(FavoriteItem.Path);
		}
	}
