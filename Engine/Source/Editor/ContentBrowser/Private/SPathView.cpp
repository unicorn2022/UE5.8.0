...
public:
	...
	void ProcessVirtualPaths(TConstArrayView<FName> InVirtualPaths, TFunction<FContentBrowserDataCompiledFilter()> CompiledFilterGetter);
	void ProcessVirtualPaths(TConstArrayView<FName> InVirtualPaths, TFunction<FContentBrowserDataCompiledFilter()> CompiledFilterGetter, TSet <TSharedRef<FTreeItem>>& OutItems);
...
void FPathViewData::PopulateFullFolderTree(const FContentBrowserDataCompiledFilter& CompiledDataFilter)
{
	...
	ContentBrowserData->EnumerateItemsMatchingFilter(CompiledDataFilter,
		[this,
			...
			{
				UE_LOGF(LogPathView, VeryVerbose, "[%ls] Populate: skipping folder %ls:%ls that fails current pre-text filtering",
			...
			UE_LOGF(LogPathView, VeryVerbose, "[%ls] Populate: adding folder %ls:%ls",
				*WriteToString<256>(OwningContentBrowserName),
				*WriteToString<256>(Source->GetFName()),
				*WriteToString<256>(InItemData.GetVirtualPath()));
		...
		ContentBrowserData->EnumerateItemsAtPath(*Path,
			...
			[this, &CompiledDataFilter, EmptyFilter, &OldItemsByInvariantPath](FContentBrowserItemData&& InItemData) {
				...
				if (!ItemDataSource->IsFolderVisible(InItemData.GetVirtualPath(), EmptyFilter.FolderFlags, EmptyFilter.FolderFilter))
				{
					UE_LOGF(LogPathView,
						VeryVerbose,
						"Hiding folder %ls that fails current pre-text filtering",
						*WriteToString<256>(InItemData.GetVirtualPath()));
					return true; // continue enumerating
				}
	...
	const FContentBrowserDataCompiledFilter& CompiledDataFilter)
{
	...
	auto DoesItemPassFilter = [this, EmptyFilter, ContentBrowserData, &CompiledDataFilter](
		...
		if (!ItemDataSource->DoesItemPassFilter(InItemData, CompiledDataFilter))
		{
			UE_LOGF(LogPathView, VeryVerbose, "[%ls] Fails compiled data filter",
				*WriteToString<256>(OwningContentBrowserName));
			return false;
		}
		...
		{
			UE_LOGF(LogPathView, VeryVerbose, "[%ls] Fails folder visibility filter",
				*WriteToString<256>(OwningContentBrowserName));
			return false;
		}
	...
	for (const FContentBrowserItemDataUpdate& ItemDataUpdate : InUpdatedItems)
	{
		...
		switch (ItemDataUpdate.GetUpdateType())
		{
			case EContentBrowserItemUpdateType::Added:
				UE_LOGF(LogPathView, VeryVerbose, "[%ls] Added item %ls:%ls", *WriteToString<256>(OwningContentBrowserName), 
			...
			case EContentBrowserItemUpdateType::Modified:
				UE_LOGF(LogPathView, VeryVerbose, "[%ls] Modified item %ls:%ls", *WriteToString<256>(OwningContentBrowserName), 
					*WriteToString<256>(ItemData.GetOwnerDataSource()->GetFName()),
					*WriteToString<256>(ItemData.GetVirtualPath()));
				if (DoesItemPassFilter(ItemData))
				{
			...
			{
				UE_LOGF(LogPathView, VeryVerbose, "[%ls] Moved item %ls:%ls->%ls", *WriteToString<256>(OwningContentBrowserName), 
					*WriteToString<256>(ItemData.GetOwnerDataSource()->GetFName()),
			...
			case EContentBrowserItemUpdateType::Removed:
				UE_LOGF(LogPathView, VeryVerbose, "[%ls] Removed item %ls:%ls", *WriteToString<256>(OwningContentBrowserName), 
					*WriteToString<256>(ItemData.GetOwnerDataSource()->GetFName()),
					*WriteToString<256>(ItemData.GetVirtualPath()));
				TryRemoveFolderItemInternal(ItemData);
				break;

			case EContentBrowserItemUpdateType::Unmounted:
				UE_LOGF(LogPathView, VeryVerbose, "[%ls] Unmounted item %ls:%ls", *WriteToString<256>(OwningContentBrowserName),
					*WriteToString<256>(ItemData.GetOwnerDataSource()->GetFName()),
					*WriteToString<256>(ItemData.GetVirtualPath()));
				TryRemoveFolderItemInternal(ItemData);
				break;
...
void FPathViewData::ProcessVirtualPaths(TConstArrayView<FName> InVirtualPaths, TFunction<FContentBrowserDataCompiledFilter()> CompiledFilterGetter)
{
	UContentBrowserDataSubsystem* ContentBrowserData = IContentBrowserDataModule::Get().GetSubsystem();

	TArray<FContentBrowserItemDataUpdate> FolderAtPath;
	for (const FName& Path : InVirtualPaths)
	{
		ContentBrowserData->EnumerateItemsAtPath(Path, EContentBrowserItemTypeFilter::IncludeFolders, [&FolderAtPath](FContentBrowserItemData&& InItem) -> bool
		{
			FolderAtPath.Add(FContentBrowserItemDataUpdate::MakeItemAddedUpdate(MoveTemp(InItem)));
			return true;
		});
	}

	if (!FolderAtPath.IsEmpty())
	{
		ProcessDataUpdates(FolderAtPath, CompiledFilterGetter());
	}
}

void FPathViewData::ProcessVirtualPaths(TConstArrayView<FName> InVirtualPaths, TFunction<FContentBrowserDataCompiledFilter()> CompiledFilterGetter, TSet<TSharedRef<FTreeItem>>& OutItems)
{
	ProcessVirtualPaths(InVirtualPaths, MoveTemp(CompiledFilterGetter));

	for (const FName& VirtualPath : InVirtualPaths)
	{
		TSharedPtr<FTreeItem> Item = FindTreeItem(VirtualPath);
		if (Item.IsValid())
		{
	...
	TMap<FName, TSharedPtr<FTreeItem>>* OldItemsByInvariantPath)
{
	...
	if (LeafItem.IsValid())
	{
		UE_LOGF(LogPathView, VeryVerbose, "[%ls] Appending data to existing item %ls:%ls", 
			*WriteToString<256>(OwningContentBrowserName),
	...
	UE_LOGF(LogPathView, VeryVerbose, "[%ls] Creating new tree item for %ls:%ls", 
		*WriteToString<256>(OwningContentBrowserName),
		*WriteToString<256>(OriginalDataSource->GetFName()),
		*WriteToString<256>(ItemVirtualPath));
	...
	FPathViews::IterateAncestors(PathBuffer.ToView(),
		[this, &PathBuffer, &PreviousItem, OriginalDataSource, OldItemsByInvariantPath, &ContentBrowserData](FStringView PathView) {
			...
			{
				UE_LOGF(LogPathView, VeryVerbose, "[%ls] Adding root item %ls:%.*ls", 
					*WriteToString<256>(OwningContentBrowserName),
					*WriteToString<256>(OriginalDataSource->GetFName()),
			...
			{
				UE_LOGF(LogPathView, VeryVerbose, "[%ls] Creating placeholder or virtual parent %ls:%.*ls", 
					*WriteToString<256>(OwningContentBrowserName),
					*WriteToString<256>(OriginalDataSource->GetFName()),
					PathView.Len(), PathView.GetData());
			...
			{
				UE_LOGF(LogPathView, VeryVerbose, "[%ls] Found existing parent %.*ls", 
					*WriteToString<256>(OwningContentBrowserName),
					PathView.Len(), PathView.GetData());
			}
...
SLATE_IMPLEMENT_WIDGET(SPathView)
void SPathView::PrivateRegisterAttributes(FSlateAttributeInitializer& AttributeInitializer)
{
	struct FInvalidation
	{
		static void HandleShowRedirectorsChanged(SWidget& OwningWidget)
		{
			SPathView& PathView = static_cast<SPathView&>(OwningWidget);
			UE_LOGF(LogPathView, Verbose, "PathView bShowRedirectors changed to %d", PathView.bShowRedirectorsAttribute.Get());
			PathView.HandleSettingChanged("ShowRedirectors");
		}
		static void RequestTreeRefresh(SWidget& OwningWidget)
		{
			SPathView& PathView = static_cast<SPathView&>(OwningWidget);
			if (ensure(PathView.TreeViewPtr))
			{
				PathView.TreeViewPtr->RequestTreeRefresh();
			}
		}
	};
	SLATE_ADD_MEMBER_ATTRIBUTE_DEFINITION(AttributeInitializer, bShowRedirectorsAttribute, EInvalidateWidgetReason::Paint)
		.OnValueChanged(FSlateAttributeDescriptor::FAttributeValueChangedDelegate::CreateStatic(FInvalidation::HandleShowRedirectorsChanged));
	SLATE_ADD_MEMBER_ATTRIBUTE_DEFINITION(AttributeInitializer, TreeDataVersionAttribute, EInvalidateWidgetReason::Paint)
		.OnValueChanged(FSlateAttributeDescriptor::FAttributeValueChangedDelegate::CreateStatic(FInvalidation::RequestTreeRefresh));

	SLATE_ADD_MEMBER_ATTRIBUTE_DEFINITION(AttributeInitializer, bPopulationRequestedAttribute, [](const SWidget& OwningWidget)
	{
		// Invalidate Paint only if value changed to true as this is where we process the Populate
		return static_cast<const SPathView&>(OwningWidget).bPopulationRequestedAttribute.Get() ? EInvalidateWidgetReason::Paint : EInvalidateWidgetReason::None;
	});
	SLATE_ADD_MEMBER_ATTRIBUTE_DEFINITION(AttributeInitializer, bInitialPopulationRequestedAttribute, [](const SWidget& OwningWidget)
	{
		// Invalidate Paint only if value changed to true as this is where we process the Populate
		return static_cast<const SPathView&>(OwningWidget).bInitialPopulationRequestedAttribute.Get() ? EInvalidateWidgetReason::Paint : EInvalidateWidgetReason::None;
	});
	SLATE_ADD_MEMBER_ATTRIBUTE_DEFINITION(AttributeInitializer, bPopulationForSettingChangeRequestedAttribute, [](const SWidget& OwningWidget)
	{
		// Invalidate Paint only if value changed to true as this is where we process the Populate
		return static_cast<const SPathView&>(OwningWidget).bPopulationForSettingChangeRequestedAttribute.Get() ? EInvalidateWidgetReason::Paint : EInvalidateWidgetReason::None;
	});
}
...
SPathView::SPathView()
	: bShowRedirectorsAttribute(*this, false)
	, TreeDataVersionAttribute(*this, 0)
	, bPopulationRequestedAttribute(*this, false)
	, bInitialPopulationRequestedAttribute(*this, false)
	, bPopulationForSettingChangeRequestedAttribute(*this, false)
{
}
...
void SPathView::Construct( const FArguments& InArgs )
{
	...
	bShowRedirectorsAttribute.Assign(*this, InArgs._ShowRedirectors);
	TreeDataVersionAttribute.Assign(*this, TAttribute<uint64>::CreateLambda([this](){return TreeData->GetVersion();}));

	// Add all paths currently gathered from the asset registry
	bInitialPopulationRequestedAttribute.Assign(*this, true);
...
int32 SPathView::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	SPathView* MutableThis = const_cast<SPathView*>(this);

	if (IsPopulationRequested())
	{
		MutableThis->Populate();
	}

	MutableThis->UpdateLastExpandedPathsIfDirty();
...
void SPathView::PluginPathFilterClicked(TSharedRef<FContentBrowserPluginFilter> Filter)
{
	...
	bPopulationRequestedAttribute.Assign(*this, true);
}
...
bool SPathView::IsPluginPathFilterInUse(TSharedRef<FContentBrowserPluginFilter> Filter) const
{
	if (PluginPathFilters.IsValid())
	{
		for (const TSharedPtr<IFilter<FPluginFilterType>>& PluginFilter : *PluginPathFilters.Get())
		{
...
void SPathView::SetPluginPathFilterActive(const TSharedRef<FContentBrowserPluginFilter>& Filter, bool bActive)
{
	...
	UE_LOGF(LogPathView, Verbose, "[%ls] Setting%ls plugin filter %ls to %ls", 
	...
	{
		const FString FilterName = Filter->GetName();
		if (bActive)
		{
			if (!PathViewConfig->PluginFilters.Contains(FilterName))
			{
				PathViewConfig->PluginFilters.Add(FilterName);
				UContentBrowserConfig::Get()->SaveEditorConfig();
			}
		}
		else
		{
			PathViewConfig->PluginFilters.Remove(FilterName);
			UContentBrowserConfig::Get()->SaveEditorConfig();
		}
	}
}
...
void SPathView::SetSelectedPaths(const TArray<FString>& Paths)
{
	...
	if (ShouldDoJustInTimePopulation())
	{
		TArray<const FName> InPathToPopulate;
		for (const FString& Path : Paths)
		{
			FName PathAsName = *Path;
			TSharedPtr<FTreeItem> Item = TreeData->FindTreeItem(PathAsName);
			if (!Item)
			{
				InPathToPopulate.Add(PathAsName);
			}
		}

		if (!InPathToPopulate.IsEmpty())
		{
			TreeData->ProcessVirtualPaths(InPathToPopulate,
				[this]() { return CreateCompiledFolderFilter(); });
		}
	}
...
{
	UE_LOGF(LogPathView, VeryVerbose, "[%ls] Creating folder filter", *WriteToString<256>(OwningContentBrowserName));
	...
	UE_LOGF(LogPathView, VeryVerbose, "[%ls] bDisplayPluginFolders:%d ItemCategoryFilter:%d ItemAttributeFilter:%d", 
		*WriteToString<256>(OwningContentBrowserName), bDisplayPluginFolders, EnumToUnderlyingType(DataFilter.ItemCategoryFilter), EnumToUnderlyingType(DataFilter.ItemAttributeFilter));
	...
	if (PluginPathFilters.IsValid() && PluginPathFilters->Num() > 0 && bDisplayPluginFolders)
	{
		UE_SUPPRESS(LogPathView, VeryVerbose, {
			...
			UE_LOGF(LogPathView, VeryVerbose, "[%ls] Active plugin filters: %ls", 
	...
	UE_LOGF(LogPathView, VeryVerbose, "Compiled folder permission list: %ls", CombinedFolderPermissionList.IsValid() ? *CombinedFolderPermissionList->ToString() : TEXT("null"));
...
EContentBrowserItemCategoryFilter SPathView::GetContentBrowserItemCategoryFilter() const
{
	...
	if (bShowRedirectorsAttribute.Get())
	{
...
void SPathView::SyncToItems(TArrayView<const FContentBrowserItem> ItemsToSync, const bool bAllowImplicitSync)
{
	...
	TArray<const FName> VirtualPathsNotFound;
	for (const FContentBrowserItem& ItemToSync : ItemsToSync)
	{
		FName VirtualPathToSync;
		if (ItemToSync.IsFile())
		{
			// Files need to sync their parent folder in the tree, so chop off the end of their path
			VirtualPathToSync = FName(*FPaths::GetPath(ItemToSync.GetVirtualPath().ToString()));
		}
		else
		{
			VirtualPathToSync = ItemToSync.GetVirtualPath();
		}

		bool bShouldDoJustInTimePopulation = ShouldDoJustInTimePopulation();
		...
		else if (bShouldDoJustInTimePopulation)
		{
			VirtualPathsNotFound.Add(VirtualPathToSync);
		}
	}

	if (!VirtualPathsNotFound.IsEmpty())
	{
		TreeData->ProcessVirtualPaths(VirtualPathsNotFound,
			[this]() { return CreateCompiledFolderFilter(); },
			SyncTreeItems);
	}
...
void SPathView::SaveSettings(const FString& IniFilename, const FString& IniSection, const FString& InstanceName) const
{
	...
	UContentBrowserDataSubsystem* DataSubsystem = IContentBrowserDataModule::Get().GetSubsystem();
	{
		TArray< TSharedPtr<FTreeItem> > PathItems = TreeViewPtr->GetSelectedItems();

		for (const TSharedPtr<FTreeItem>& Item : PathItems)
		{
			if (!SelectedPathsString.IsEmpty())
			{
				SelectedPathsString += TEXT(",");
			}

			FName InvariantPath;
			DataSubsystem->TryConvertVirtualPath(Item->GetItem().GetVirtualPath(), InvariantPath);
	...
	{
		/**
		 * We didn't finish selecting the initial paths so save those instead
		 * This can happen if the we save the settings before the view is populated or the ar scan is finished and the user didn't change the selected paths.
		 */
		for (FName SelectedPath : PendingInitialPaths)
		{
			if (!SelectedPathsString.IsEmpty())
			{
				SelectedPathsString += TEXT(",");
			...
			FName InvariantPath;
			DataSubsystem->TryConvertVirtualPath(SelectedPath, InvariantPath);
			InvariantPath.AppendString(SelectedPathsString);
		}
	}
	...
	if (PluginPathFilters.IsValid())
	{
		for (const TSharedPtr<IFilter<FPluginFilterType>>& Filter : *PluginPathFilters.Get())
		{
			...
			TSharedPtr<FContentBrowserPluginFilter> PluginFilter = StaticCastSharedPtr<FContentBrowserPluginFilter>(Filter);
			PluginFiltersString += PluginFilter->GetName();
		}
...
void SPathView::LoadSettings(const FString& IniFilename, const FString& IniSection, const FString& SettingsString)
{
	...
	UE_LOGF(LogPathView, Verbose, "[%ls] LoadSettings: SelectedPaths: %ls", 
	...
	if (PluginPathFilters.IsValid())
	{
		...
		if (FPathViewConfig* PathViewConfig = GetPathViewConfig())
		{
			UE_LOGF(LogPathView, Verbose, "[%ls] LoadSettings: Loading plugin filters from editor config: %ls", 
...
void SPathView::Populate(const bool bIsRefreshingFilter)
{
	...
	UE_LOGF(LogPathView, Verbose, "Repopulating path view");

	const bool bFilteringByText = !TreeData->GetFolderPathTextFilter().GetRawFilterText().IsEmpty();

	// Batch the selection changed event
	// Only emit events when the user isn't filtering
	FScopedSelectionChangedEvent ScopedSelectionChangedEvent(SharedThis(this),
	...
	if (bPopulationForSettingChangeRequestedAttribute.Get())
	{
		PostPopulateHandleSettingChanged(!ScopedSelectionChangedEvent.IsInitialSelectionEmpty());
	}

	if (bInitialPopulationRequestedAttribute.Get())
	{
	...
	{
		UE_LOGF(LogPathView, VeryVerbose, "[%ls] Resetting pending initial paths as all are selected", *WriteToString<256>(OwningContentBrowserName));
		PendingInitialPaths.Reset();
	}
...
void SPathView::SharedPostPopulate()
{
	...
	bPopulationRequestedAttribute.Assign(*this, false);
	bInitialPopulationRequestedAttribute.Assign(*this, false);
	bPopulationForSettingChangeRequestedAttribute.Assign(*this, false);
}
...
void SPathView::HandleItemDataUpdated(TArrayView<const FContentBrowserItemDataUpdate> InUpdatedItems)
{
	...
	{
		UE_LOGF(LogPathView, VeryVerbose, "[%ls] Skipping item data update because there were no folders present", *WriteToString<256>(OwningContentBrowserName));
		return;
	}
	...
	UE_LOGF(LogPathView, VeryVerbose, "[%ls] PathView - HandleItemDataUpdated completed in %0.4f seconds for %d items",
		*WriteToString<256>(OwningContentBrowserName),
...
void SPathView::HandleSettingChanged(FName PropertyName)
{
		...
		|| (PropertyName == NAME_None)) // @todo: Needed if PostEditChange was called manually, for now
	{
		UE_LOGF(LogPathView,
			Log,
			"[%ls][PathView] HandleSettingChanged %ls",
			*WriteToString<256>(OwningContentBrowserName),
		...
		bPopulationForSettingChangeRequestedAttribute.Assign(*this, true);
	}
}
...
{
	return bPopulationRequestedAttribute.Get() || bPopulationForSettingChangeRequestedAttribute.Get() || bInitialPopulationRequestedAttribute.Get();
}
...
void SFavoriteAssetTreeItem::Construct(const FArguments& InArgs)
{
	SAssetTreeItem::Construct(InArgs._Parent);

	UpdateItem(InArgs._Parent._TreeItem->GetItem().GetItemPath());

	// Once favorites are updated, check if this item is concerned to update alias
	FContentBrowserSingleton::Get().RegisterOnFavoritesChangedHandler(
		FOnFavoritesChanged::FDelegate::CreateSP(this, &SFavoriteAssetTreeItem::OnFavoriteUpdated)
	);
}

FText SFavoriteAssetTreeItem::GetNameText() const
{
	if (!Alias.IsEmpty())
	{
		return FText::Format(LOCTEXT("FavoriteAlias", "@{0}"), Alias);
	}

	return SAssetTreeItem::GetNameText();
}

FText SFavoriteAssetTreeItem::GetToolTipText() const
{
	FText BaseTooltip = SAssetTreeItem::GetToolTipText();

	if (!Shortcut.IsEmpty())
	{
		return FText::Format(LOCTEXT("FavoriteTooltip", "{0}\nShortcut : {1}"), BaseTooltip, Shortcut);
	}

	return BaseTooltip;
}

void SFavoriteAssetTreeItem::OnDragEnter(const FGeometry& InGeometry, const FDragDropEvent& InDragDropEvent)
{
	if (const TSharedPtr<FContentBrowserDataDragDropOp> DragDropOp = GetFavoriteMoveOperation(InDragDropEvent))
	{
		DragDropOp->SetToolTip(LOCTEXT("AddOrReorderFavoriteFolder", "Add or Reorder favorite folder"), FAppStyle::GetBrush(TEXT("Graph.ConnectorFeedback.OK")));
	}
	else
	{
		SAssetTreeItem::OnDragEnter(InGeometry, InDragDropEvent);
	}
}

void SFavoriteAssetTreeItem::OnDragLeave(const FDragDropEvent& InDragDropEvent)
{
	if (const TSharedPtr<FContentBrowserDataDragDropOp> DragDropOp = GetFavoriteMoveOperation(InDragDropEvent))
	{
		DragDropOp->SetToolTip(FText::GetEmpty(), nullptr);
		bDraggedOver = false;
	}
	else
	{
		SAssetTreeItem::OnDragLeave(InDragDropEvent);
	}
}

FReply SFavoriteAssetTreeItem::OnDragOver(const FGeometry& InGeometry, const FDragDropEvent& InDragDropEvent)
{
	if (!GetFavoriteMoveOperation(InDragDropEvent))
	{
		return SAssetTreeItem::OnDragOver(InGeometry, InDragDropEvent);
	}

	// Let the tree handle this
	return FReply::Unhandled();
}

FReply SFavoriteAssetTreeItem::OnDrop(const FGeometry& InGeometry, const FDragDropEvent& InDragDropEvent)
{
	if (!GetFavoriteMoveOperation(InDragDropEvent))
	{
		return SAssetTreeItem::OnDrop(InGeometry, InDragDropEvent);
	}
...
void SFavoriteAssetTreeItem::OnFavoriteUpdated(const FContentBrowserItemPath& InItemPath, bool bInAdded)
{
	if (const TSharedPtr<FTreeItem> Item = TreeItem.Pin())
	{
		if (Item->GetItem().GetItemPath().GetVirtualPathName() == InItemPath.GetVirtualPathName())
		{
			UpdateItem(InItemPath);
		}
	}
}

void SFavoriteAssetTreeItem::UpdateItem(const FContentBrowserItemPath& InPath)
{
	if (const FContentBrowserFavoriteItem* FavoriteItem = ContentBrowserUtils::FindFavoriteFolder(InPath))
	{
		Alias = FText::FromString(FavoriteItem->Alias);

		FContentBrowserCommands& ContentBrowserCommands = FContentBrowserCommands::Get();
		const TSharedPtr<FUICommandInfo>* FavoriteShortcut = ContentBrowserCommands.FavoriteShortcuts.FindByPredicate([CommandName = FavoriteItem->ShortcutCommandName](const TSharedPtr<FUICommandInfo>& InCommand)
		{
			return CommandName == InCommand->GetCommandName();
		});

		if (FavoriteShortcut && FavoriteShortcut->IsValid())
		{
			Shortcut = (*FavoriteShortcut)->GetInputText();
		}
		else
		{
			Shortcut = FText::GetEmpty();
		}
	}
}

TSharedPtr<FContentBrowserDataDragDropOp> SFavoriteAssetTreeItem::GetFavoriteMoveOperation(const FDragDropEvent& InDragDropEvent)
{
	if (const TSharedPtr<FContentBrowserDataDragDropOp> DragDropOp = InDragDropEvent.GetOperationAs<FContentBrowserDataDragDropOp>())
	{
		// Drag operation contains only folders and CTRL is not pressed => Add/Reordering
	...
	// Drag operation contains assets files or folders while CTRL is pressed => Move/Copy
	return nullptr;
}
...
void SFavoritePathView::LoadSettings(const FString& IniFilename, const FString& IniSection, const FString& SettingsString)
{
	...
	// Migrate favorites to new project settings
	if (UContentBrowserFavoriteProjectSettings* FavoriteSettings = UContentBrowserFavoriteProjectSettings::Get())
	{
		...
		if (FavoriteSettings->FavoriteItems.IsEmpty())
		{
			// Favorite Paths
			...
			// If the selected paths is empty, the path was "All assets"
			// This should handle that case properly
			FavoriteSettings->FavoriteItems.Reserve(NewFavoritePaths.Num());
			...
			UE_LOGF(LogContentBrowser, Log, "Migrated %i editor favorites to project favorites", FavoriteSettings->FavoriteItems.Num())

			// Discard previous data
			GConfig->RemoveKey(*IniSection, TEXT("FavoritePaths"), IniFilename);
		}
...
TSharedPtr<FContentBrowserDataDragDropOp> SFavoritePathView::GetContentBrowserDragDropOpFromEvent(const FDragDropEvent& DragDropEvent) const
{
	...
	if (Operation.IsValid() && OnFolderFavoriteUpdated.IsBound())
	{
...
FReply SFavoritePathView::OnDrop(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent)
{
	...
	if (!DragDropOp.IsValid() || !TreeData.IsValid())
	{
		return FReply::Unhandled();
	}

	if (OnFolderFavoriteUpdated.IsBound())
	{
		TArray<FString> FolderPaths;
		FolderPaths.Reserve(DragDropOp->GetDraggedFolders().Num());
		for (const FContentBrowserItem& BrowserItem: DragDropOp->GetDraggedFolders())
		{
			const FName VirtualPath = BrowserItem.GetVirtualPath();
			if (!TreeData->FindTreeItem(VirtualPath))
			{
				FolderPaths.Add(VirtualPath.ToString());
			}
		}

		if (!FolderPaths.IsEmpty())
		{
			OnFolderFavoriteUpdated.Execute(FolderPaths, FString(), EItemDropZone::BelowItem);
		}
	}
...
void SFavoritePathView::SetOnFolderFavoriteUpdated(const FOnFolderFavoriteUpdated& InOnFolderFavoriteUpdated)
{
	OnFolderFavoriteUpdated = InOnFolderFavoriteUpdated;
}
...
TSharedRef<ITableRow> SFavoritePathView::GenerateTreeRow(TSharedPtr<FTreeItem> TreeItem, const TSharedRef<STableViewBase>& OwnerTable)
{
	...
	SAssetTreeItem::FArguments Arguments;
	Arguments
		.TreeItem(TreeItem)
	...
	return
		...
		.OnCanAcceptDrop(this, &SFavoritePathView::OnFolderCanAcceptDrop)
		.OnAcceptDrop(this, &SFavoritePathView::OnFolderAcceptDrop)
		[
			SNew(SFavoriteAssetTreeItem)
			.Parent(Arguments)
		];
}

TOptional<EItemDropZone> SFavoritePathView::OnFolderCanAcceptDrop(const FDragDropEvent& InDragDropEvent, EItemDropZone InItemDropZone, TSharedPtr<FTreeItem> InTreeItem)
{
	...
	TSharedPtr<FContentBrowserDataDragDropOp> DragDropOp = GetContentBrowserDragDropOpFromEvent(InDragDropEvent);
	if (!DragDropOp.IsValid() || !InTreeItem.IsValid() || !TreeData.IsValid())
	{
		return TOptional<EItemDropZone>();
	}

	const int32 TargetIndex = TreeData->GetVisibleRootItems()->Find(InTreeItem);
	if (TargetIndex == INDEX_NONE)
	{
		return TOptional<EItemDropZone>();
	}

	TSet<FContentBrowserItemKey> DisallowDropItemKeys
	{
		// Don't allow drop on itself
		FContentBrowserItemKey(InTreeItem->GetItem())
	};

	if (InItemDropZone == EItemDropZone::BelowItem)
	{
		/*
		 * If we drop onto or below a target item, make sure next item is not contained within dragged items, eg:
		 * - A <-
		 * - B
		 * - C
		 * - D
		 * If B,C are dragged, disallow dropping after A and before B, only before A and after D
		 */
		if (TreeData->GetVisibleRootItems()->IsValidIndex(TargetIndex + 1))
		{
			if (const TSharedPtr<FTreeItem> NextItem = (*TreeData->GetVisibleRootItems())[TargetIndex + 1])
			{
				const FContentBrowserItemKey NextItemKey(NextItem->GetItem());
				DisallowDropItemKeys.Add(NextItemKey);
			}
		}
	}
	else
	{
		/*
		 * If we drop above a target item, make sure prev item is not contained within dragged items, eg:
		 * - A
		 * - B
		 * - C
		 * - D <-
		 * If B,C are dragged, disallow dropping after C and before D, only before A and after D
		 */
		if (TreeData->GetVisibleRootItems()->IsValidIndex(TargetIndex - 1))
		{
			if (const TSharedPtr<FTreeItem> PrevItem = (*TreeData->GetVisibleRootItems())[TargetIndex - 1])
			{
				const FContentBrowserItemKey PrevItemKey(PrevItem->GetItem());
				DisallowDropItemKeys.Add(PrevItemKey);
			}
		}
	}

	const bool bDropZoneDisallowed = DragDropOp->GetDraggedFolders().ContainsByPredicate([&DisallowDropItemKeys](const FContentBrowserItem& InDraggedFolder)
		{
			return DisallowDropItemKeys.Contains(FContentBrowserItemKey(InDraggedFolder));
		});

	if (bDropZoneDisallowed)
	{
		return TOptional<EItemDropZone>();
	}
...
FReply SFavoritePathView::OnFolderAcceptDrop(const FDragDropEvent& InDragDropEvent, EItemDropZone InItemDropZone, TSharedPtr<FTreeItem> InTreeItem)
{
	TSharedPtr<FContentBrowserDataDragDropOp> DragDropOp = GetContentBrowserDragDropOpFromEvent(InDragDropEvent);
	if (!DragDropOp.IsValid() || !InTreeItem.IsValid())
	{
		return FReply::Unhandled();
	}
	...
	if (OnFolderFavoriteUpdated.IsBound())
	{
		...
		const FString FolderContext = InTreeItem->GetItem().GetVirtualPath().ToString();
		OnFolderFavoriteUpdated.Execute(FolderPaths, FolderContext, InItemDropZone);
	}
...
void SFavoritePathView::HandleItemDataUpdated(TArrayView<const FContentBrowserItemDataUpdate> InUpdatedItems)
{
	...
	{
		UE_LOGF(LogPathView, VeryVerbose, "[%ls] Skipping item data update because there were no favorites present", *WriteToString<256>(OwningContentBrowserName));
		return;
	}
	...
	for (const FContentBrowserItemDataUpdate& ItemDataUpdate : InUpdatedItems)
	{
		...
		switch (ItemDataUpdate.GetUpdateType())
		{
			...
			case EContentBrowserItemUpdateType::Removed:
				...
				break;
			case EContentBrowserItemUpdateType::Unmounted:
				// Do not remove favorite because a plugin can be unmounted then mounted again later
				break;
			default:
				checkf(false, TEXT("Unexpected EContentBrowserItemUpdateType!"));
				break;
	...
	UE_LOGF(LogPathView, VeryVerbose, "FavoritePathView - HandleItemDataUpdated completed in %0.4f seconds for %d items", FPlatformTime::Seconds() - HandleItemDataUpdatedStartTime, InUpdatedItems.Num());
}
