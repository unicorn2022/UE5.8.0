...
FText FAssetViewItem::GetAssetClassText() const
{
	if (IsFolder())
	{
	...
	FContentBrowserItemDataAttributeValue DisplayNameAttributeValue = Item.GetItemAttribute(ContentBrowserItemAttributes::ItemTypeDisplayName);
	if (!DisplayNameAttributeValue.IsValid())
	{
		DisplayNameAttributeValue = Item.GetItemAttribute(ContentBrowserItemAttributes::ItemTypeName);
	}
...
FText FAssetViewItem::GetItemAssetAccessSpecifierText() const
{
	...
	{
		return AssetAccessSpecifierAsText(AssetAccessSpecifier);
	}
...
FName FAssetViewItem::GetItemAssetAccessSpecifierIconStyleName(const TCHAR* InSizeSuffix) const
{
	...
	if (CanModifyItemAssetAccessSpecifier() && GetItemAssetAccessSpecifier(AssetAccessSpecifier) )
	{
		switch (AssetAccessSpecifier)
		{
			...
			default:
			{
				ensureMsgf(0, TEXT("Unrecognized EAssetAccessSpecifier value '%d' found for asset: '%s'"), EnumToUnderlyingType(AssetAccessSpecifier), *GetItem().GetItemName().ToString());
				break;
			}
		}
	}
