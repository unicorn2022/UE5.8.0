...
public:
	...
	/**
	 * Changes sources to show the specified collections and selects them in the collection view
	 *
	 *	@param CollectionsToSync	- A list of collections to sync the view to
	 */
	void SyncToCollections(TArrayView<const FCollectionRef> CollectionsToSync);
	...
	/** Returns true if the legacy (asset view) content source is the active content source */
	...
	/** Edit favorite folder options */
	void EditFolderFavorite(const FContentBrowserItemPath& InItemPath);
...
private:
	...
	/** Forwards the user-searching state to the asset view */
	void SetAssetViewUserSearching(bool bIsSearching);
	...
	/** Handler to check if filter by selection command is allowed */
	bool HandleFilterBySelectionCommandCanExecute() const;

	/** Handler for filter by selection */
	void HandleFilterBySelectionCommandExecute();
	
	/** Handler for favorite shortcuts */
	void HandleFavoriteShortcutCommandExecute(FName InCommandName);
	...
	/** Updates favorite status of an array of folders */
	void UpdateFolderFavorite(const TArray<FString>& FolderPaths, const FString& InFolder, EItemDropZone InPosition);
	...
	void OnSearchOverrideToggled();

private:

	/**
	 * Struct that contains the widgets used by content browser's default layout (i.e Legacy Content Source)
	 * ContentBrowser 2.0 will use TEDS, this is moved here but is still in use and correct while Content Browser refactor is on hold
	 */
	struct FLegacyContentSourceWidgets
	{
		...
		/** Per-view instance of the active search override, if any */
		TSharedPtr<UE::Editor::ContentBrowser::Extension::ICBSearchOverrideInstance> AssetSearchOverrideInstance;
	...
	TUniquePtr<FLegacyContentSourceWidgets> LegacyContentSourceWidgets;
