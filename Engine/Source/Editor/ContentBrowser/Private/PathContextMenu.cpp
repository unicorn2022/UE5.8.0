...
void FPathContextMenu::SetOnFolderFavoriteEdited(const FOnFolderFavoriteEdited& InOnFolderFavoriteEdited)
{
	OnFolderFavoriteEdited = InOnFolderFavoriteEdited;
}
		...
		// Common operations section //
		{
...

			{
				...
				if (Context->bCanBeModified)
				{
					FToolMenuEntry& NewAssetEntry = Section.AddSubMenu(
						...
						NewAssetToolTip,
						FNewToolMenuDelegate::CreateSP(this, &FPathContextMenu::MakeNewAssetSubMenu),
						FUIAction(),
			...
			if ( ContentBrowserUtils::HasCustomColors() )
			{
				...
				Section.AddSubMenu(
					...
					FNewToolMenuDelegate::CreateSP( this, &FPathContextMenu::MakeSetColorSubMenu ),
					false,
			...
			if (ContentBrowserUtils::IsFavoriteFolder(SelectedFolderItemPath))
			{
				...
				if (SelectedFolders.Num() == 1)
				{
					...
					Section.AddMenuEntry(
						"EditFavorite",
						LOCTEXT("EditFavorite", "Edit Favorite Item"),
						LOCTEXT("EditFavoriteTooltip", "Edit and configure favorite item options"),
						FSlateIcon(FAppStyle::GetAppStyleSetName(), "Icons.Edit"),
						FUIAction(FExecuteAction::CreateSP(this, &FPathContextMenu::ExecuteEditFavorite))
					);
				}
			}
			else
...
{
	if (TSharedPtr<SWidget> PinnedParentContent = ParentContent.Pin())
	{
		ContentBrowserUtils::ExploreFolders(SelectedFolders, PinnedParentContent.ToSharedRef());
	}
}
...
void FPathContextMenu::ExecuteEditFavorite()
{
	if (SelectedFolders.Num() == 1)
	{
		OnFolderFavoriteEdited.ExecuteIfBound(SelectedFolders[0].GetItemPath());
	}
}
