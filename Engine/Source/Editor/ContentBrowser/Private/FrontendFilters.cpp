...
/////////////////////////////////////////
// FFrontendFilter_Favorites
/////////////////////////////////////////

FFrontendFilter_Favorites::FFrontendFilter_Favorites(TSharedPtr<FFrontendFilterCategory> InCategory)
	:FFrontendFilter(InCategory)
{
...
bool FFrontendFilter_Favorites::PassesFilter(FAssetFilterType InItem) const
{
	using namespace UE::Editor::DataStorage;
	...
	{
		if (FAssetData AssetData; InItem.Legacy_TryGetAssetData(AssetData))
		{
			Path = FMapKey(AssetData.GetSoftObjectPath());
		}
	}
	...
	const RowHandle Row = DataStorage->LookupMappedRow("Asset", Path);

	if (DataStorage->IsRowAvailable(Row))
	{
		return DataStorage->HasColumns<FFavoriteTag>(Row);
	}
