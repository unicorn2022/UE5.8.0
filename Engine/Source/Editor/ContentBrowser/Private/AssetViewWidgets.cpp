...
FText SAssetViewItem::GetAssetClassText() const
{
	...
	return AssetItem->GetAssetClassText();
}
			...
			if (!bHasSetDisplayValue && AttributeMetaData.AttributeType == UObject::FAssetRegistryTag::TT_Dimensional)
			{
				...
				if (AttributeValueStr.FindChar(TEXT('x'), FirstXPos))
				{
					...
					if (Remainder.FindChar(TEXT('*'), RemainderSeparatorPos))
					{
						...
						int SecondXPos;
						if (SecondPart.FindChar(TEXT('x'), SecondXPos))
						{
							// AxBxC*D form (3D array)
							FString ThirdPart = Remainder.Mid(SecondXPos + 1);
							FString FourthPart = Remainder.Mid(RemainderSeparatorPos + 1);

							bHasSetDisplayValue = true;
							DisplayValue = FText::Format(LOCTEXT("DisplayTag3xArrayFmt", "{0} \u00D7 {1} \u00D7 {2} ({3} elements)"),
								...
								ReformatNumberStringForDisplay(FourthPart));
						}
						else
						{
							// AxB*C form (2D array)
...
template <>
struct TWidgetTypeTraits<class SAssetColumnItemNameBox>
{
