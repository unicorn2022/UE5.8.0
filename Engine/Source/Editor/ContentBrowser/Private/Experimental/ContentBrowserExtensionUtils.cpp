...
namespace UE::Editor::ContentBrowser::ExtensionUtils
{
	...
	bool IsFolderFavorite(const FContentBrowserItemPath& ItemPath)
	{
		return ContentBrowserUtils::IsFavoriteFolder(ItemPath);
	}

	void SetFolderFavorite(const FString& FolderPath, bool bIsFavorite)
	{
		if (bIsFavorite)
		{
			ContentBrowserUtils::AddFavoriteFolder(FContentBrowserItemPath(FolderPath, EContentBrowserPathType::Internal));
		}
		else
		{
			ContentBrowserUtils::RemoveFavoriteFolder(FContentBrowserItemPath(FolderPath, EContentBrowserPathType::Internal));
		}
