	...
	MenuBuilder.BeginSection("CollectionOptions", LOCTEXT("CollectionListOptionsMenuHeading", "Collection Options"));
	{
...

		{
			...
			MenuBuilder.AddSubMenu(
				...
				FNewMenuDelegate::CreateSP( this, &FCollectionContextMenu::MakeNewCollectionSubMenu, ECollectionStorageMode::Static, SCollectionView::FCreateCollectionPayload( ParentCollection ) ),
				FUIAction(
					FExecuteAction(),
		...
		MenuBuilder.AddSubMenu(
			...
			FNewMenuDelegate::CreateSP( this, &FCollectionContextMenu::MakeCollectionShareTypeSubMenu ),
			FUIAction(
				FExecuteAction(),
		...
		if ( CanExecuteColorChange() && CollectionViewUtils::HasCustomColors(*CollectionViewPtr->GetCollectionContainer()))
		{
			...
			MenuBuilder.AddSubMenu(
				...
				FNewMenuDelegate::CreateSP( this, &FCollectionContextMenu::MakeSetColorSubMenu )
				);
		}
		else
		{
...
void FCollectionContextMenu::ExecuteDestroyCollection()
{
	...
	if (ContentBrowserUtils::DisplayConfirmationDialog(LOCTEXT("CollectionDestroyConfirm_Yes", "Delete"), Prompt))
	{
		ExecuteDestroyCollectionConfirmed(SelectedCollections);
	}
}
