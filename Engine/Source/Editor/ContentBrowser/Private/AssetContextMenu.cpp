...
{
	bool bHasDisallowSetPublicReason = !DisallowSetPublicReason.IsEmpty();
	if (!(bHasDisallowSetPublicReason || bCanShowSetAssetAccessMenu || bCanExecuteSetPublicAsset || bCanExecuteSetEpicInternalAsset || bCanExecuteSetPrivateAsset || bCanExecuteBulkSetPublicAsset || bCanExecuteBulkSetEpicInternalAsset || bCanExecuteBulkSetPrivateAsset))
	{
		return false;
	}
	...
	UContentBrowserDataMenuContext_FileMenu* Context = Menu->FindContext<UContentBrowserDataMenuContext_FileMenu>();
	{
		...
		{
			if (bCanExecuteSetPublicAsset || bCanExecuteSetEpicInternalAsset || bCanExecuteSetPrivateAsset || bHasDisallowSetPublicReason || bCanShowSetAssetAccessMenu)
			{
				Section.AddMenuEntry(
					"PublicAsset",
					LOCTEXT("PublicAssetToggle", "Public Asset"),
					bHasDisallowSetPublicReason ? DisallowSetPublicReason : LOCTEXT("PublicAssetToggleTooltip", "Sets the asset to be referencable by other Plugins"),
				...
				if (bCanExecuteSetEpicInternalAsset)
				{
					Section.AddMenuEntry(
						...
						bHasDisallowSetPublicReason ? DisallowSetPublicReason : LOCTEXT("SetInternalAssetTooltip", "Sets the asset to be referencable by Epic internal plugins and mount points"),
...
int32 FAssetContextMenu::CompareAssetAccessSpecifiers(EAssetAccessSpecifier A, EAssetAccessSpecifier B)
{
	checkf((A == EAssetAccessSpecifier::Private || A == EAssetAccessSpecifier::EpicInternal || A == EAssetAccessSpecifier::Public) &&
		(B == EAssetAccessSpecifier::Private || B == EAssetAccessSpecifier::EpicInternal || B == EAssetAccessSpecifier::Public),
		TEXT("Unknown EAssetAccessSpecifier"));
	...
	if (A == EAssetAccessSpecifier::Private || B == EAssetAccessSpecifier::Public)
	{
		return -1;
	}
...
void FAssetContextMenu::ExecuteBulkSetAssetAccessSpecifier(EAssetAccessSpecifier DestScope)
{
...

	{
		...
		SlowTask.MakeDialogDelayed(0.5f, /*bShowCancelButton*/true);
		...
		for (const FContentBrowserItem& SelectedItem : SelectedFiles)
		{
			...
			for (const FContentBrowserItemData& ItemData : ItemDataArray)
			{
				...
				{
					FAssetData ItemAssetData;
					if (!ItemDataSource->Legacy_TryGetAssetData(ItemData, ItemAssetData))
					{
						continue;
					}

					if (DestScope != EAssetAccessSpecifier::Private)
					{
						const FNameBuilder ItemInternalPath(ItemData.GetInternalPath());
						const FStringView AssetPath(ItemInternalPath);
						const bool bCanBePublic = IAssetTools::Get().CanAssetBePublic(AssetPath);
						if (!bCanBePublic)
						{
							// Assets without public permissions might end up in our bulk selection. Make sure to skip those.
							continue;
						}
					}

					const int32 Result = CompareAssetAccessSpecifiers(DestScope, ItemAssetData.GetAssetAccessSpecifier());
					if (Result > 0)
					{
						// Ok to asset raise the access specifier to DestScope without calling BulkPrivatizeItems
						if (UPackage* ItemAssetPackage = ItemAssetData.GetPackage())
						{
							if (ItemAssetPackage->SetAssetAccessSpecifier(DestScope))
							{
								if (UObject* Asset = ItemAssetData.GetAsset())
								{
									// Let the data source know we updated the asset access specifier.
					...
					{
						// Lowering asset access specifier uses BulkPrivatizeItems workflow instead of directly calling SetAssetAccessSpecifier
						...
						{
							if (ItemAssetData.GetPackage()) // Load under the slow task.
							{
	...
	// Calling PostEditChange is enough to refresh the view, but there is an issue where if you open the context menu again on the same frame,
	// SetSelectedItems gets called making a copy of the items before they get updated when the UContentBrowserDataSubsystem ticks,
	// so you end up with stale data.  Refreshing the view here will cause the SContentBrowser to call SetSelectedItems again after it ticks
	// which fixes the issue, though it creates its own problems since other decisions based on the original selection are cached (e.g. CacheCanExecuteVars).
...
void FAssetContextMenu::CacheCanExecuteVars()
{
	...
	bCanShowSetAssetAccessMenu = false;
	...
	DisallowSetPublicReason = FText::GetEmpty();

	FContentBrowserModule& ContentBrowserModule = FModuleManager::GetModuleChecked<FContentBrowserModule>(TEXT("ContentBrowser"));

	const FOnContentBrowserCanShowSetAssetAccessMenu& OnContentBrowserCanShowSetAssetAccessMenu = ContentBrowserModule.GetOnContentBrowserCanShowSetAssetAccessMenu();
	...
	auto CheckCanExecuteFindInExplorer = [](const FContentBrowserItem& SelectedItem) -> bool
	{
		FString ItemFilename;
		return SelectedItem.GetItemPhysicalPath(ItemFilename) && FPaths::FileExists(ItemFilename);
	};

	auto CheckCanAssetBePublic = [](const FStringView AssetPath, FText* DisallowReason) -> bool
	{
		return IAssetTools::Get().CanAssetBePublic(AssetPath, DisallowReason);
	};

	auto CheckCanShowSetAssetAccessMenu = [&OnContentBrowserCanShowSetAssetAccessMenu](const FStringView AssetPath, const FAssetData& Asset) -> bool
	{
		return !OnContentBrowserCanShowSetAssetAccessMenu.IsBound() || OnContentBrowserCanShowSetAssetAccessMenu.Execute(AssetPath, Asset);
	};

	if (SelectedFiles.Num() == 1)
	{
		const FContentBrowserItem& SelectedItem = SelectedFiles[0];
		bCanExecuteFindInExplorer = CheckCanExecuteFindInExplorer(SelectedItem);
		...
		if (SelectedItem.CanEdit())
		{
			...
			if (AssetData.IsValid() && CheckCanShowSetAssetAccessMenu(AssetPath, AssetData))
			{
				bCanShowSetAssetAccessMenu = true;

				// Only show menu entries when asset can be made externally referenceable 
				if (CheckCanAssetBePublic(AssetPath, &DisallowSetPublicReason))
				{
					bCanExecuteSetPublicAsset = true;
					bCanExecuteSetEpicInternalAsset = bIsEpicInternalAssetFeatureEnabled && (ShowEpicInternalContentPermissionList->PassesStartsWithFilter(AssetPath) || ShowPrivateContentPermissionList->PassesStartsWithFilter(AssetPath));
					bCanExecuteSetPrivateAsset = true;
				}
				else if (AssetData.GetAssetAccessSpecifier() != EAssetAccessSpecifier::Private)
				{
					// Allow user to manually fix asset that should be private
					bCanExecuteSetPrivateAsset = true;
				}
			}
		}
		...
		for (const FContentBrowserItem& SelectedItem : SelectedFiles)
		{
			...
			if (SelectedItem.CanEdit())
			{
				...
				if (AssetData.IsValid())
				{
					...
					if (!bCanExecuteBulkSetPublicAsset && CheckCanAssetBePublic(AssetPath, nullptr))
					{
					...
					{
						if (CheckCanAssetBePublic(AssetPath, nullptr))
						{
