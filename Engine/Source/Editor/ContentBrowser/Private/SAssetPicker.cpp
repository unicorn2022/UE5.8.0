...
void SAssetPicker::Construct( const FArguments& InArgs )
{
	...
	OnAssetsDragged = InArgs._AssetPickerConfig.OnAssetsDragged;
	...
	OnSearchTextChangedDelegate = InArgs._AssetPickerConfig.OnSearchTextChanged;
	...
	for (auto DelegateIt = InArgs._AssetPickerConfig.SaveAssetViewSettingsDelegates.CreateConstIterator(); DelegateIt; ++DelegateIt)
	{
		if ((*DelegateIt) != nullptr)
		{
			(**DelegateIt) = FSaveAssetViewSettingsDelegate::CreateSP(this, &SAssetPicker::SaveSettings);
		}
	}

	for (auto DelegateIt = InArgs._AssetPickerConfig.LoadAssetViewSettingsDelegates.CreateConstIterator(); DelegateIt; ++DelegateIt)
	{
		if ((*DelegateIt) != nullptr)
		{
			(**DelegateIt) = FLoadAssetViewSettingsDelegate::CreateSP(this, &SAssetPicker::LoadSettings);
		}
	}
	...
	[
		...
		.OnItemsDragged(this, &SAssetPicker::HandleItemsDragged)
		...
		.AssetShowWarningTextJustification( InArgs._AssetPickerConfig.AssetShowWarningTextJustification )
		.AssetShowWarningTextSelectable( InArgs._AssetPickerConfig.AssetShowWarningTextSelectable )
		...
		.OnColumnVisibilityChanged(InArgs._AssetPickerConfig.OnColumnVisibilityChanged)
	...
	InArgs._AssetPickerConfig.OnExtendAssetPickerTopBar.ExecuteIfBound(HorizontalBox);
...
void SAssetPicker::ClearSearchText()
{
...
TSharedPtr<SFilterList> SAssetPicker::GetFilterList() const
{
...
void SAssetPicker::OnSearchBoxChanged(const FText& InSearchText)
{
	...
	OnSearchTextChangedDelegate.ExecuteIfBound(InSearchText);
}
...
void SAssetPicker::HandleItemsDragged(TArrayView<const FContentBrowserItem> DraggedItems)
{
	...
	for (const FContentBrowserItem& DraggedItem : DraggedItems)
	{
		...
		{
			FAssetData ItemAssetData;
			if (DraggedItem.Legacy_TryGetAssetData(ItemAssetData))
			{
	...
	OnAssetsDragged.ExecuteIfBound(DraggedAssets);
}
...
bool SAssetPicker::IsShowingOtherDevelopersContent() const
{
	...
	else
	{
		...
		{
			if (Filter == HideOtherDevelopersFilter)
			{
				return false;
			}
		}
...
bool SAssetPicker::CanFilterBySelection()
{
	if (!AssetViewPtr.IsValid() || !FilterListPtr.IsValid())
	{
		return false;
	}
...
void SAssetPicker::OnFilterBySelection()
{
	if (!AssetViewPtr.IsValid() || !FilterListPtr.IsValid())
	{
		return;
	...
	AssetPaths.Reserve(SelectedItems.Num());
	for (const FContentBrowserItem& SelectedItem : SelectedItems)
	{
		FContentBrowserItemDataAttributeValue AttributeValue = SelectedItem.GetItemAttribute(ContentBrowserItemAttributes::ItemTypeName);
		if (AttributeValue.IsValid())
		{
			AssetPaths.Add(FTopLevelAssetPath(AttributeValue.GetValueString()));
		}
	}

	FilterListPtr->SetActiveFilters(AssetPaths);
}
...
void SAssetPicker::BindCommands()
{
	...
	Commands->MapAction(
		FContentBrowserCommands::Get().FilterBySelection,
		FUIAction(
			FExecuteAction::CreateSP(this, &SAssetPicker::OnFilterBySelection),
			FCanExecuteAction::CreateSP(this, &SAssetPicker::CanFilterBySelection)
		)
	);
}
