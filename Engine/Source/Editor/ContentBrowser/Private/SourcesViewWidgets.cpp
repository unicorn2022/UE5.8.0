...
void SAssetTreeItem::Construct( const FArguments& InArgs )
{
	...
	{
		if ( const TSharedPtr<FTreeItem> TreeItemPtr = TreeItem.Pin() )
		{
			EnterEditingModeDelegateHandle = TreeItemPtr->OnRenameRequested().AddSP( InlineRenameWidget.Get(), &SInlineEditableTextBlock::EnterEditingMode );
		}
	}
}
...
SAssetTreeItem::~SAssetTreeItem()
{
	if( InlineRenameWidget.IsValid() )
	{
		if ( const TSharedPtr<FTreeItem> TreeItemPtr = TreeItem.Pin() )
		{
			TreeItemPtr->OnRenameRequested().Remove( EnterEditingModeDelegateHandle );
		}
	}
}
...
{
	if ( TSharedPtr<FTreeItem> TreeItemPtr = TreeItem.Pin() )
	{
...
bool SAssetTreeItem::IsReadOnly() const
{
	if ( const TSharedPtr<FTreeItem> TreeItemPtr = TreeItem.Pin() )
	{
		return !TreeItemPtr->IsNamingFolder();
	}
	else
	{
		return true;
...
{
	const bool bIsItemExpanded = IsItemExpanded.Get(false);
	// First see if this item has a custom implementation
	if (TSharedPtr<FTreeItem> TreeItemPin = TreeItem.Pin())
	{
		FContentBrowserItemDataAttributeValue FolderAttributeValue = TreeItemPin->GetItem().GetItemAttribute(bIsItemExpanded ? ContentBrowserItemAttributes::ItemAssetTreeFolderOpenBrush : ContentBrowserItemAttributes::ItemAssetTreeFolderClosedBrush);
	...
	case EFolderType::Code:
		return bIsItemExpanded ? Brushes.FolderOpenCodeBrush : Brushes.FolderClosedCodeBrush;

	case EFolderType::Developer:
		return bIsItemExpanded ? Brushes.FolderOpenDeveloperBrush : Brushes.FolderClosedDeveloperBrush;

	case EFolderType::CustomVirtual:
		return bIsItemExpanded ? Brushes.FolderOpenVirtualBrush : Brushes.FolderClosedVirtualBrush;
	case EFolderType::PluginRoot:
		return bIsItemExpanded ? Brushes.FolderOpenPluginRootBrush : Brushes.FolderClosedPluginRootBrush;

	default:
		return bIsItemExpanded ? Brushes.FolderOpenBrush : Brushes.FolderClosedBrush;
	}
}
	...
	if ( CollectionItemPtr.IsValid() )
	{
		if ( CollectionItemPtr->bRenaming )
		{
			...
			if ( OnNameChangeCommit.IsBound() )
			{
				...
				if ( !OnNameChangeCommit.Execute(CollectionItemPtr, NewText.ToString(), bIsCommitted, WarningMessage) && bIsCommitted )
				{
					if (TSharedPtr<SWidget> PinnedParentWidget = ParentWidget.Pin())
					{
						// Failed to rename/create a collection, display a warning.
						ContentBrowserUtils::DisplayMessage(WarningMessage, CachedGeometry.GetLayoutBoundingRect(), PinnedParentWidget.ToSharedRef(), ContentBrowserUtils::EDisplayMessageType::Error);
					}
				}
			}				
		}
