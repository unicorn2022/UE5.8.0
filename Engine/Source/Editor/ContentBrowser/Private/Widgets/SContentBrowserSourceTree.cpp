...
namespace UE::Editor::ContentBrowser::Private
{
	...
	SContentBrowserSourceTree::FScopedWidgetSlotArguments SContentBrowserSourceTree::AddSlot(const int32 InAtIndex)
	{
		return FScopedWidgetSlotArguments{ MakeUnique<FSlot>(), Slots, InAtIndex,
			[this, HostSplitter = Splitter](const FSlot* InSlotAdded, int32 InSlotIdx)
			{
				...
				HostSplitter->AddSlot(InSlotIdx)
	...
	int32 SContentBrowserSourceTree::RemoveSlotAt(int32 InIndex)
	{
		if (!Splitter.IsValid())
		{
			return INDEX_NONE;
		...
		{
			Slots.RemoveAt(InIndex);

			Splitter->RemoveAt(InIndex);
		...
		TSharedRef<IScrollableWidget> InBody)
	{
		...
		if (InSearch.IsValid())
		{
			...
			SearchButtonWidget =
				...
				.OnSearchBoxShown(FOnSearchBoxShown::CreateSPLambda(this, [this]() { SetExpanded(true); }));
		}
		...
		{
			BodyContentIndexAttribute = MakeAttributeSPLambda(
				this,
				[this]()
				{
					return IsEmpty() ? 1 : 0;
	...
	void SContentBrowserSourceTreeArea::OnDragEnter(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent)
	{
		if (BodyScrollableWidget.IsValid())
		{
			BodyScrollableWidget->GetScrollWidget()->OnDragEnter(MyGeometry, DragDropEvent);
		}
	}

	void SContentBrowserSourceTreeArea::OnDragLeave(const FDragDropEvent& DragDropEvent)
	{
		if (BodyScrollableWidget.IsValid())
		{
			BodyScrollableWidget->GetScrollWidget()->OnDragLeave(DragDropEvent);
		}
	}

	FReply SContentBrowserSourceTreeArea::OnDrop(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent)
	{
		if (BodyScrollableWidget.IsValid())
		{
			return BodyScrollableWidget->GetScrollWidget()->OnDrop(MyGeometry, DragDropEvent);
		}
