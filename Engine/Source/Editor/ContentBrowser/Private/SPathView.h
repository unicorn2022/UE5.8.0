...
template<>
struct TWidgetTypeTraits<class SPathView>
{
...
{
	SLATE_DECLARE_WIDGET(SPathView, SCompoundWidget)
...
public:
	...
	/** Paint to poll attributes. This avoids the need for the widget to tick each frame */
...
protected:
	...
	constexpr virtual bool ShouldDoJustInTimePopulation() const
	{
		return true;
	}
...
private:
	TSlateAttribute<bool> bShowRedirectorsAttribute;
	/** Last version number retrieved from TreeData so we can decide if the tree view need rebuilding */
	TSlateAttribute<uint64> TreeDataVersionAttribute;
	...
	TSlateAttribute<bool> bPopulationRequestedAttribute;

	/** Request the initial population of the path view */
	TSlateAttribute<bool> bInitialPopulationRequestedAttribute;

	/** Request a population of the path view but because of a setting change */
	TSlateAttribute<bool> bPopulationForSettingChangeRequestedAttribute;
...
template<>
struct TWidgetTypeTraits<class SFavoriteAssetTreeItem>
{
	static constexpr bool SupportsInvalidation() { return true; }
};

/** AssetTreeItem for the Favorite view to override some behavior */
class SFavoriteAssetTreeItem : public SAssetTreeItem
{
	SLATE_BEGIN_ARGS(SFavoriteAssetTreeItem){}
		SLATE_ARGUMENT(SAssetTreeItem::FArguments, Parent)
	SLATE_END_ARGS()

	void Construct(const FArguments& InArgs);

protected:
	//~ Begin SAssetTreeItem
	...
	void OnFavoriteUpdated(const FContentBrowserItemPath& InItemPath, bool bInAdded);
	void UpdateItem(const FContentBrowserItemPath& InPath);

private:
	/** Retrieves only drag operation that favorites view will handle */
	static TSharedPtr<FContentBrowserDataDragDropOp> GetFavoriteMoveOperation(const FDragDropEvent& InDragDropEvent);
...
template<>
struct TWidgetTypeTraits<class SFavoritePathView>
{
	static constexpr bool SupportsInvalidation() { return true; }
};
...
public:
	SFavoritePathView();
	virtual ~SFavoritePathView() override;
	...
	DECLARE_DELEGATE_ThreeParams(FOnFolderFavoriteUpdated, const TArray<FString>& /** UpdatedFolders */, const FString& /** FolderContext */, EItemDropZone /** PositionContext */)
	void SetOnFolderFavoriteUpdated(const FOnFolderFavoriteUpdated& InOnFolderFavoriteUpdated);

protected:
	constexpr virtual bool ShouldDoJustInTimePopulation() const override
	{
		return false;
	}
...
private:
	...
	/** Handler for tree view to accept drop */
	TOptional<EItemDropZone> OnFolderCanAcceptDrop(const FDragDropEvent& InDragDropEvent, EItemDropZone InItemDropZone, TSharedPtr<FTreeItem> InTreeItem);

	/** Handler for tree view when dropping items */
	FReply OnFolderAcceptDrop(const FDragDropEvent& InDragDropEvent, EItemDropZone InItemDropZone, TSharedPtr<FTreeItem> InTreeItem);
	...
	FOnFolderFavoriteUpdated OnFolderFavoriteUpdated; 
