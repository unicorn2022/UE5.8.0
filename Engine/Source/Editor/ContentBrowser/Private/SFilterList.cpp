...
namespace UE::Editor::ContentBrowser
{
	namespace Private
	{
		static bool bUseFavoritesFilters = false;
		static FAutoConsoleVariableRef CVarUseFavoritesFilters(
			TEXT("UnifiedFavorites.ContentBrowser.Filter"),
			Private::bUseFavoritesFilters,
			TEXT("Use unified favorites filters in the Content Browser")
		);
	}
}
...
void SFilterList::Construct( const FArguments& InArgs )
{
	...
	AllFrontendFilters_Internal.Add(MakeShareable(new FFrontendFilter_Favorites(DefaultCategory)));
	...
	Args._OnFilterBarLayoutChanging = InArgs._OnFilterBarLayoutChanging;
	...
	Args._CommandList = InArgs._CommandList;
...
void SFilterList::ActivateClassFilterByPath(const FTopLevelAssetPath& ClassPath)
{
	if (GetAssetTypeFilterCheckState(ClassPath) == ECheckBoxState::Checked)
	{
		// Checked means there's already a filter for it (checked in filter list), enable
		ToggleAssetTypeFilterEnabled(ClassPath, true);
	}
	else
	{
		// Create and enable the filter if needed
		SetAssetTypeFilterCheckState(ClassPath, ECheckBoxState::Checked, true);
	}
}
...
void SFilterList::PopulateAddFilterMenu(UToolMenu* Menu, TSharedPtr<FFilterCategory> MenuExpansion, FOnFilterAssetType OnFilterAssetType)
{
	...
	// Populate the common filter sections (Reset Filters etc)
	{
		...
		if (FToolMenuSection* ManageSection = Menu->FindSection("FilterBarResetFilters"))
		{
			...
			// Add filter by selection if command is mapped in the command list
			if (const TSharedPtr<FUICommandList> CommandList = CommandListWeak.Pin())
			{
				const FContentBrowserCommands& ContentBrowserCommands = FContentBrowserCommands::Get();
				if (CommandList->IsActionMapped(ContentBrowserCommands.FilterBySelection))
				{
					ManageSection->AddMenuEntry(ContentBrowserCommands.FilterBySelection);
				}
			}

			ManageSection->AddMenuEntry(
				"FilterAlphabetically",
				LOCTEXT("FilterAlphabetically", "Sort Filters Alphabetically"),
				LOCTEXT("FilterAlphabeticallyToolTip", "Active filters will be sorted and displayed in alphabetical order in the layout instead of their insertion order"),
				FSlateIcon(),
				FUIAction(
					FExecuteAction::CreateSP(this, &SFilterList::ToggleSortFiltersAlphabetically),
					FCanExecuteAction(),
					FIsActionChecked::CreateSP(this, &SFilterList::GetSortFiltersAlphabetically)),
				EUserInterfaceActionType::ToggleButton
			);
		}
		...
		if (UE::Editor::ContentBrowser::Private::bUseFavoritesFilters)
		{
			// Add Favorites section before the BasicFilterBarFiltersMenu filters
			FToolMenuSection& FavoritesSection = Menu->FindOrAddSection(
				"FilterBarFavorites",
				LOCTEXT("FilterBarFavoritesSection", "Favorites Filter"),
				FToolMenuInsert("FilterBarDisplay", EToolMenuInsertType::After));

			// Find the Favorites filter and add it directly to the favorites section
			for (const TSharedRef<FFilterBase<FAssetFilterType>>& Filter : AllFrontendFilters)
			{
				if (Filter->GetName() == "Favorites")
				{
					FavoritesSection.AddMenuEntry(
						"FavoritesFilter",
						Filter->GetDisplayName(),
						Filter->GetToolTipText(),
						FSlateIcon(FAppStyle::GetAppStyleSetName(), "Icons.Star"),
						FUIAction(
							FExecuteAction::CreateSP(this, &SFilterList::FrontendFilterClicked, Filter),
							FCanExecuteAction(),
							FIsActionChecked::CreateSP(this, &SFilterList::IsFrontendFilterInUse, Filter)),
						EUserInterfaceActionType::ToggleButton
					);
					break;
				}
	...
	CategoryToMenuMap.KeySort(
		[](const TSharedPtr<FFilterCategory>& A, const TSharedPtr<FFilterCategory>& B)
		{
			return A.IsValid()
				&& B.IsValid()
				&& A->Title.CompareTo(B->Title) < 0;
		}
	);
	...
	{
		CategoryToMenu.Value.Classes.Sort(
			[](const TSharedPtr<FCustomClassFilterData>& A, const TSharedPtr<FCustomClassFilterData>& B)
			{
				return A.IsValid()
					&& B.IsValid()
	...
	// Sort frontend filters
	AllFrontendFilters.Sort(
		[](const TSharedRef<FFilterBase<FAssetFilterType>>& A, const TSharedRef<FFilterBase<FAssetFilterType>>& B)
		{
	...
	// Sort user filters
	CustomTextFilters.Sort(
		[](const TSharedRef<ICustomTextFilter<FAssetFilterType>>& A, const TSharedRef<ICustomTextFilter<FAssetFilterType>>& B)
		{
			return A->GetFilter().IsValid()
				&& B->GetFilter().IsValid()
				&& A->GetFilter()->GetDisplayName().CompareTo(B->GetFilter()->GetDisplayName()) < 0;
		}
	);
...
void SFilterList::ToggleSortFiltersAlphabetically()
{
	bSortFiltersAlphabetically = !bSortFiltersAlphabetically;
...
void SFilterList::SaveSettingsInternal(FFilterBarSettings* InSettingsToSave)
{
	...
	if(!bUseSharedSettings)
	{
		...
		InSettingsToSave->bSortFiltersAlphabetically = this->bSortFiltersAlphabetically;
	...
	{
		UE_LOGF(LogSlate, Error, "SFilterList Requires that you specify a FilterBarIdentifier to save settings");
		return;
	}
	...
	// Save all the programmatically added filters normally
...
void SFilterList::LoadSettings(const FName& InInstanceName)
{
	...
	if(!bUseSharedSettings)
	{
		...
		if(!FilterBarConfig)
		{
			UE_LOGF(LogSlate, Error, "SFilterBar Requires that you specify a FilterBarIdentifier to load settings");
			return;
		}
		...
		if(FilterBarConfig->bIsLayoutSaved)
		{
			...
			this->SetSortFiltersAlphabetically(FilterBarConfig->bSortFiltersAlphabetically);
		}
	...
	{
		UE_LOGF(LogSlate, Error, "SFilterList Requires that you specify a FilterBarIdentifier to load settings");
		return;
	}
	...
	// Load the filters specified programmatically normally
	...
	for(const FCustomTextFilterState& FilterState : InstanceSettings->CustomTextFilters)
	{
		...
		{
			UE_LOGF(LogSlate, Warning, "SFilterList was unable to load the following custom text filter: %ls", *FilterState.FilterData.FilterLabel.ToString());
		}
	}
	...
	if(InstanceSettings->bIsLayoutSaved)
	{
		...
		SetSortFiltersAlphabetically(InstanceSettings->bSortFiltersAlphabetically);
	}
