			...
			if (UContentBrowserAssetViewContextMenuContext* Context = InMenu->FindContext<UContentBrowserAssetViewContextMenuContext>())
			{
				if (Context->AssetView.IsValid())
				{
					...
					// Thumbnails Section
					{
						...
						auto CreateThumbnailSizeSubMenu = [AssetView = &AssetView.Get()](UToolMenu* SubMenu)
						{
						...
						ThumbnailSection.AddEntry(FToolMenuEntry::InitSubMenu(
							...
							FNewToolMenuDelegate::CreateSPLambda(AssetView, CreateThumbnailSizeSubMenu),
							false
						));
						...
						if (Context->AssetView.IsValid())
						{
							...
							const TAttribute<FSlateIcon> GetIcon = TAttribute<FSlateIcon>::Create(
								TAttribute<FSlateIcon>::FGetter::CreateSPLambda(
									AssetView,
									[AssetView = &AssetView.Get()]()
									{
										EColumnSortMode::Type ActiveSortMode = EColumnSortMode::Ascending;
										if (const TSharedPtr<FAssetViewSortManager> StrongSortManager = AssetView->GetSortManager().Pin())
										{
							...
							SortAction.IsActionVisibleDelegate = FToolMenuIsActionButtonVisible::CreateSPLambda(
								AssetView,
								[AssetView = &AssetView.Get()](const FToolMenuContext&)
								{
...
FDelayedAutoRegisterHelper SAssetView::NavigationBarMenuExtensionRegistration(
	...
	[]
	{
		...
		if (UToolMenu* NavigationBarMenu = UToolMenus::Get()->ExtendMenu(NavigationBarName))
		{
			...
			{
				FToolMenuSection& AssetViewInfoSection = NavigationBarMenu->FindOrAddSection("AssetViewInfo", { }, FToolMenuInsert("History", EToolMenuInsertType::Last));
				AssetViewInfoSection.Alignment = EToolMenuSectionAlign::Last;

				AssetViewInfoSection.AddDynamicEntry(
					"AssetViewInfo",
					FNewToolMenuSectionDelegate::CreateLambda([](FToolMenuSection& InSection)
					{
						if (const UContentBrowserToolbarMenuContext* Context = InSection.FindContext<UContentBrowserToolbarMenuContext>())
						{
							if (Context->AssetView.IsValid())
							{
								...
								FToolMenuEntry& AssetViewInfoEntry = InSection.AddEntry(
									FToolMenuEntry::InitWidget(
										...
										[
											...
											[
												SNew(STextBlock)
												.Text(AssetView, &SAssetView::GetMajorityAssetTypeText)
												.Visibility(AssetView, &SAssetView::GetMajorityAssetTypeTextVisibility)
												.ToolTipText(AssetView, &SAssetView::GetMajorityAssetTypeToolTipText)
											]
										],
										FText::GetEmpty()));
								AssetViewInfoEntry.WidgetData.StyleParams.VerticalAlignment = VAlign_Center;
								AssetViewInfoEntry.WidgetData.StyleParams.HorizontalAlignment = HAlign_Fill;
								AssetViewInfoEntry.WidgetData.StyleParams.FillSize = 1.0f;
								AssetViewInfoEntry.WidgetData.StyleParams.SizeRule = FSizeParam::SizeRule_Stretch;
							}
						}
					}));
			}
