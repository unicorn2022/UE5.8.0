...
namespace ContentBrowserUtils
{
	...
	/** Displays a modal dialog asking yes or no question (blocking) */
	bool DisplayConfirmationDialog(const FText& InTitle, const FText& InMessage);
	...
	/** Convert a legacy asset and path selection to their corresponding content browser data items */
	TArray<FContentBrowserItem> ConvertLegacySelectionToItems(TArrayView<const FAssetData> InAssets, TArrayView<const FString> InFolders);
	...
	/** Adds a new favorite folder at a specific position */
	void InsertFavoriteFolder(const FContentBrowserItemPath& InFolderPath, const FContentBrowserItemPath& InFolderContext, EItemDropZone InPositionContext);
	
	/** Moves an existing favorite folder at a new position */
	void MoveFavoriteFolder(const FContentBrowserItemPath& InFolderPath, const FContentBrowserItemPath& InFolderContext, EItemDropZone InPositionContext);

	/** Finds existing favorite item based on path */
	FContentBrowserFavoriteItem* FindFavoriteFolder(const FContentBrowserItemPath& InFolderPath);
	...
	TArray<FString> GetFavoriteFolders();
