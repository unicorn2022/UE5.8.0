...
public:
	FCollectionSource(SContentBrowser& InContentBrowser, const TSharedRef<ICollectionContainer>& InCollectionContainer)
	{
		...
		CollectionViewPtr = SNew(SCollectionView)
			.OnCollectionSelected_Lambda([ContentBrowser = StaticCastWeakPtr<SContentBrowser>(InContentBrowser.AsWeak()), CollectionContainer = InCollectionContainer](const FCollectionNameType& SelectedCollection)
			{
				if (TSharedPtr<SContentBrowser> PinnedContentBrowser = ContentBrowser.Pin())
				{
					PinnedContentBrowser->CollectionSelected(CollectionContainer, SelectedCollection);
				}
			})
...
void SContentBrowser::Construct( const FArguments& InArgs, const FName& InInstanceName, const FContentBrowserConfig* Config )
{
	...
	// Init favorites settings object
	UContentBrowserFavoriteProjectSettings::Get();
...
void SContentBrowser::BindCommands()
{
	...
	Commands->MapAction(GenericCommands.Rename, FUIAction(
	...
	Commands->MapAction(GenericCommands.Delete, FUIAction(
	...
	Commands->MapAction(ContentBrowserCommands.OpenAssetsOrFolders, FUIAction(
		FExecuteAction::CreateSP(this, &SContentBrowser::HandleOpenAssetsOrFoldersCommandExecute)
	));

	Commands->MapAction(ContentBrowserCommands.PreviewAssets, FUIAction(
		FExecuteAction::CreateSP(this, &SContentBrowser::HandlePreviewAssetsCommandExecute)
	));

	Commands->MapAction(ContentBrowserCommands.CreateNewFolder, FUIAction(
		FExecuteAction::CreateSP(this, &SContentBrowser::HandleCreateNewFolderCommandExecute)
	));

	Commands->MapAction(ContentBrowserCommands.GoUpToParentFolder, FUIAction(
	...
	Commands->MapAction(ContentBrowserCommands.SaveSelectedAsset, FUIAction(
	...
	Commands->MapAction(ContentBrowserCommands.SaveAllCurrentFolder, FUIAction(
		FExecuteAction::CreateSP(this, &SContentBrowser::HandleSaveAllCurrentFolderCommand)
	));

	Commands->MapAction(ContentBrowserCommands.ResaveAllCurrentFolder, FUIAction(
		FExecuteAction::CreateSP(this, &SContentBrowser::HandleResaveAllCurrentFolderCommand)
	));

	Commands->MapAction(ContentBrowserCommands.EditPath, FUIAction(
		FExecuteAction::CreateSP(this, &SContentBrowser::EditPathCommand)
	));

	Commands->MapAction(ContentBrowserCommands.FilterBySelection, FUIAction(
		FExecuteAction::CreateSP(this, &SContentBrowser::HandleFilterBySelectionCommandExecute),
		FCanExecuteAction::CreateSP(this, &SContentBrowser::HandleFilterBySelectionCommandCanExecute)
	));
	
	// Favorites shortcuts
	for (const TSharedPtr<FUICommandInfo>& FavoriteShortcut : ContentBrowserCommands.FavoriteShortcuts)
	{
		Commands->MapAction(
			FavoriteShortcut, 
			FUIAction(
				FExecuteAction::CreateSP(this, &SContentBrowser::HandleFavoriteShortcutCommandExecute, FavoriteShortcut->GetCommandName())
			)
		);
	}
	...
	TArray<FContentBrowserCommandExtender>& CommandExtenderDelegates = ContentBrowserModule.GetAllContentBrowserCommandExtenders();

	for (int32 i = 0; i < CommandExtenderDelegates.Num(); ++i)
	{
		if (CommandExtenderDelegates[i].IsBound())
		{
			CommandExtenderDelegates[i].Execute(Commands.ToSharedRef(), FOnContentBrowserGetSelection::CreateSP(this, &SContentBrowser::GetSelectionState));
		}
	}
...
void SContentBrowser::UpdateFolderFavorite(const TArray<FString>& FolderPaths, const FString& InFolder, EItemDropZone InPosition)
{
	const FContentBrowserItemPath TargetPath(InFolder, EContentBrowserPathType::Virtual);
	
	auto InsertOrMoveFolder = [&TargetPath, &InPosition](const FString& InPath)
		{
			const FContentBrowserItemPath ItemPath(InPath, EContentBrowserPathType::Virtual);
			if (ContentBrowserUtils::IsFavoriteFolder(ItemPath))
			{
				ContentBrowserUtils::MoveFavoriteFolder(ItemPath, TargetPath, InPosition);
			}
			else
			{
				ContentBrowserUtils::InsertFavoriteFolder(ItemPath, TargetPath, InPosition);
			}
		};

	// When dropping onto/below an item, insert in reverse to keep order
	if (InPosition == EItemDropZone::OntoItem || InPosition == EItemDropZone::BelowItem)
	{
		for (int32 Index = FolderPaths.Num() - 1; Index >= 0; --Index)
		{
			InsertOrMoveFolder(FolderPaths[Index]);
		}
	}
	else
	{
...
void SContentBrowser::EditFolderFavorite(const FContentBrowserItemPath& InItemPath)
{
	if (const FContentBrowserFavoriteItem* FavoriteItem = ContentBrowserUtils::FindFavoriteFolder(InItemPath))
	{
		FContentBrowserSingleton::Get().OpenEditFavoriteItemWindow(*FavoriteItem);
	}
}
...
void SContentBrowser::SetFilterLayout(const EFilterBarLayout InFilterBarLayout) const
{
	...
	{
		UE_LOGF(LogContentBrowser, Warning, "SetFilterLayout failed: %ls is invalid. FilterListPtr: %ls, AssetViewPtr: %ls",
...
EFilterBarLayout SContentBrowser::GetFilterLayout() const
{
	...
	UE_LOGF(LogContentBrowser, Warning, "GetFilterLayout: FilterListPtr is invalid, returning default layout.");
...
TSharedPtr<SWidget> SContentBrowser::GetActiveFilterContainer() const
{
	...
	UE_LOGF(LogContentBrowser, Warning, "GetFilterLayout: FilterListPtr is invalid, returning nullptr.");
	return nullptr;
}
...
TSharedRef<SWidget> SContentBrowser::CreateAssetView(const FContentBrowserConfig* Config)
{
	...
	LegacyContentSourceWidgets->FilterListPtr = SNew(SFilterList)
					...
					.CreateTextFilter(SFilterList::FCreateTextFilter::CreateSPLambda(this, [this]
					{
	...
	// If a search override is registered, create a per-view instance and retrieve its toggle widget
	PRAGMA_DISABLE_EXPERIMENTAL_WARNINGS
	if (UE::Editor::ContentBrowser::Extension::IAssetSearchOverride* ActiveOverride = FModuleManager::LoadModuleChecked<FContentBrowserModule>("ContentBrowser").GetActiveAssetSearchOverride())
	{
		LegacyContentSourceWidgets->AssetSearchOverrideInstance = ActiveOverride->MakePerViewOverride();
		LegacyContentSourceWidgets->AssetSearchOverrideInstance->GetInternalApi().OnOverrideActiveChanged.AddSP(this, &SContentBrowser::OnSearchOverrideToggled);
		// Pass the instance to the asset view so it can take over the last step of the filtering and the sort.
		LegacyContentSourceWidgets->AssetViewPtr->SetAssetSearchOverride(LegacyContentSourceWidgets->AssetSearchOverrideInstance);
	}

	TSharedPtr<SWidget> SearchOverrideToggleWidget;
	if (LegacyContentSourceWidgets->AssetSearchOverrideInstance.IsValid())
	{
		SearchOverrideToggleWidget = LegacyContentSourceWidgets->AssetSearchOverrideInstance->GetOverrideModeToggleWidget();
	}
	PRAGMA_ENABLE_EXPERIMENTAL_WARNINGS

	LegacyContentSourceWidgets->SearchBoxPtr = SNew(SAssetSearchBox)
				   .SuggestionListPlacement(EMenuPlacement::MenuPlacement_AboveAnchor) // Avoid showing suggestion on filtered content browser results...
				   ...
				   .ExtraWidget(SearchOverrideToggleWidget)
...
SContentBrowser::FCollectionSource& SContentBrowser::AddSlotForCollectionContainer(int32 Index, const TSharedRef<ICollectionContainer>& CollectionContainer)
{
	...
	else
	{
		...
		const TSharedRef<UE::Editor::ContentBrowser::Private::SContentBrowserSourceTreeArea> CollectionArea = CreateCollectionsView(*CollectionSource);

		LegacyContentSourceWidgets->PathFavoriteSplitterPtr->AddSlot(SlotIndex)
			.SizeRule_Lambda([CollectionArea]()
			{
...
void SContentBrowser::RemoveSlotForCollectionContainer(const TSharedRef<ICollectionContainer>& CollectionContainer)
{
	...
	if (ensure(Index != INDEX_NONE))
	{
		...
		if (UE::Editor::ContentBrowser::IsNewStyleEnabled())
		{
			LegacyContentSourceWidgets->SourceTreePtr->RemoveSlotAt(SlotIndex);
		}
		else
		{
...
TSharedRef<SWidget> SContentBrowser::CreateFavoritesView(const FContentBrowserConfig* Config)
{
	...
	LegacyContentSourceWidgets->FavoritePathViewPtr->SetOnFolderFavoriteUpdated(SFavoritePathView::FOnFolderFavoriteUpdated::CreateSP(this, &SContentBrowser::UpdateFolderFavorite));
	return ViewWidget;
}
...
TSharedRef<SWidget> SContentBrowser::CreatePathView(const FContentBrowserConfig* Config)
{
	SAssignNew(LegacyContentSourceWidgets->PathViewPtr, SPathView)
		...
		.ShowTreeTitle(false)
		.ShowSeparator(false)
		.ShowRedirectors(MakeAttributeSPLambda(this, [this]() { return ContentBrowserUtils::ShouldShowRedirectors(LegacyContentSourceWidgets->FilterListPtr); }))
...
TSharedRef<UE::Editor::ContentBrowser::Private::SContentBrowserSourceTreeArea> SContentBrowser::CreateCollectionsView(FCollectionSource& CollectionSource)
{
	return SAssignNew(
			...
			SNew(SHorizontalBox)
				...
				[
					SNew(SButton)
						...
						.OnClicked(FOnClicked::CreateSPLambda(this, [this, CollectionContainer = CollectionSource.GetCollectionContainer()]()
						{
							for (const TUniquePtr<FCollectionSource>& CollectionSource : CollectionSources)
							{
								...
								{
									CollectionSource->CollectionArea->SetExpanded(true);
									CollectionSource->CollectionViewPtr->MakeAddCollectionMenu(SharedThis(this));
									break;
								}
							}
		...
		.IsEmpty_Lambda([CollectionView = CollectionSource.CollectionViewPtr]
		{
...
void SContentBrowser::SyncToCollections(TArrayView<const FCollectionRef> CollectionsToSync)
{
	// For now we just forcefully enable the legacy content source when a sync is requested so it succeeds
	ContentSourcesContainer->ActivateLegacyContentSource();
	...
	SelectedCollections.Reserve(CollectionsToSync.Num());
	{
		TArray<FCollectionNameType> CollectionsToSelect;
		for (const TUniquePtr<FCollectionSource>& CollectionSource : CollectionSources)
		{
			CollectionsToSelect.Reset(CollectionsToSync.Num());
			for (const FCollectionRef& CollectionToSync : CollectionsToSync)
			{
				if (CollectionSource->GetCollectionContainer() == CollectionToSync.Container)
				{
					CollectionsToSelect.Emplace(CollectionToSync.Name, CollectionToSync.Type);
				}
			}
			CollectionSource->CollectionViewPtr->SetSelectedCollections(CollectionsToSelect),

			Algo::Transform(
				CollectionSource->CollectionViewPtr->GetSelectedCollections(),
				SelectedCollections,
				[&CollectionSource](const FCollectionNameType& Collection) { return FCollectionRef(CollectionSource->GetCollectionContainer(), Collection); });
		}
	}

	if (SelectedCollections.Num() == 0 && !CollectionSources.IsEmpty())
	{
		// Select a dummy "None" collection to avoid the sources view switching to the paths view
		SelectedCollections.Emplace(CollectionSources[0]->GetCollectionContainer(), FCollectionNameType(NAME_None, ECollectionShareType::CST_System));
	}

	SourcesChanged({}, SelectedCollections);
}
...
void SContentBrowser::SyncToLegacy(TArrayView<const FAssetData> AssetDataList, TArrayView<const FString> FolderList, const bool bAllowImplicitSync, const bool bDisableFiltersThatHideAssets)
{
	...
	TArray<FContentBrowserItem> ItemsToSync = ContentBrowserUtils::ConvertLegacySelectionToItems(AssetDataList, FolderList);
...
bool SContentBrowser::IsLegacyContentSourceActive() const
{
	return ContentSourcesContainer.IsValid() && ContentSourcesContainer->IsLegacyContentSourceActive();
}
...
void SContentBrowser::SourcesChanged(const TArray<FString>& SelectedPaths, const TArray<FCollectionRef>& SelectedCollections)
{
	...
	UE_LOGF(LogContentBrowser, VeryVerbose, "The content browser source was changed by the sources view to '%ls'", *NewSource);
...
void SContentBrowser::SetSelectedPaths(const TArray<FString>& FolderPaths, bool bNeedsRefresh/* = false */)
{
	if (FolderPaths.Num() > 0)
	{
		...
		if (bNeedsRefresh)
		{
			LegacyContentSourceWidgets->PathViewPtr->RequestPopulation();	
			LegacyContentSourceWidgets->FavoritePathViewPtr->Populate();
		}

		// Need to set selected paths on PathViewPtr here even if we just called RequestPopulation above because PathSelected below expects it to be set 
...
void SContentBrowser::SetAssetViewUserSearching(bool bIsSearching)
{
	LegacyContentSourceWidgets->AssetViewPtr->SetUserSearching(bIsSearching);
}
...
{
	bool bOverrideHandled = false;

	PRAGMA_DISABLE_EXPERIMENTAL_WARNINGS
	if (LegacyContentSourceWidgets->AssetSearchOverrideInstance.IsValid() &&
		LegacyContentSourceWidgets->AssetSearchOverrideInstance->IsOverrideActive())
	{
		LegacyContentSourceWidgets->AssetSearchOverrideInstance->OnSearchTextChanged(InSearchText);
		bOverrideHandled = true;
	}
	PRAGMA_ENABLE_EXPERIMENTAL_WARNINGS
...
void SContentBrowser::OnSearchBoxCommitted(const FText& InSearchText, ETextCommit::Type CommitInfo)
{
	...
	if (LegacyContentSourceWidgets->AssetSearchOverrideInstance.IsValid() &&
		LegacyContentSourceWidgets->AssetSearchOverrideInstance->IsOverrideActive())
	{
		LegacyContentSourceWidgets->AssetSearchOverrideInstance->OnSearchTextCommitted(InSearchText, CommitInfo);
		bOverrideHandled = true;
	}
	PRAGMA_ENABLE_EXPERIMENTAL_WARNINGS
		...
		else
		{
			...
			for (const TUniquePtr<FCollectionSource>& CollectionSource : CollectionSources)
			{
				...
				MenuBuilder.AddSubMenu(
					...
					TAttribute<FText>(),
					FNewMenuDelegate::CreateSP(CollectionSource->CollectionViewPtr.ToSharedRef(), &SCollectionView::MakeSaveDynamicCollectionMenu, FinalQueryText));
			}
...
TArray<FNavigationBarComboOption> SContentBrowser::OnCompletePathPrefix(const FString& Prefix) const
{
	...
	{	
		...
		SuffixView = Prefix;
		SuffixView.RightChopInline(PrefixView.Len() + 1);
	}
	...
	{
		FStringView ChildSuffix = ChildPath;
		ChildSuffix.RightChopInline(PrefixView.Len() + 1);
		if (ChildSuffix.StartsWith(SuffixView))
		{
...
{
	LegacyContentSourceWidgets = MakeUnique<FLegacyContentSourceWidgets>();
	...
	LegacyContentSourceWidgets->PathContextMenu->SetOnFolderFavoriteEdited(FPathContextMenu::FOnFolderFavoriteEdited::CreateSP(this, &SContentBrowser::EditFolderFavorite));
	...
	if (bShowBottomToolbar)
	{
		if (UE::Editor::ContentBrowser::IsNewStyleEnabled())
		{
			ViewOptions =
				...
				.AddMetaData<FTagMetaData>(FTagMetaData(TEXT("AssetViewMenuButton")));
		}
		else
		{
			ViewOptions =
...
void SContentBrowser::OnSearchOverrideToggled()
{
	if (LegacyContentSourceWidgets->AssetSearchOverrideInstance && LegacyContentSourceWidgets->AssetSearchOverrideInstance->IsOverrideActive())
	{
		// Use enter since this was based on a user action
		LegacyContentSourceWidgets->AssetSearchOverrideInstance->OnSearchTextCommitted(LegacyContentSourceWidgets->SearchBoxPtr->GetText(), ETextCommit::OnEnter);
		SetSearchBoxText(FText::GetEmpty());
	}
	else
	{
		SetSearchBoxText(LegacyContentSourceWidgets->SearchBoxPtr->GetText());
	}
	
	LegacyContentSourceWidgets->AssetViewPtr->RequestQuickFrontendListRefresh();
}
...
bool SContentBrowser::HandleFilterBySelectionCommandCanExecute() const
{
	if (!LegacyContentSourceWidgets.IsValid() || !LegacyContentSourceWidgets->FilterListPtr.IsValid())
	{
		return false;
	}
...
void SContentBrowser::HandleFilterBySelectionCommandExecute()
{
	if (!LegacyContentSourceWidgets.IsValid() || !LegacyContentSourceWidgets->FilterListPtr.IsValid())
	{
		return;
	...
	TArray<FTopLevelAssetPath> SelectedAssetPaths;
	SelectedAssetPaths.Reserve(SelectedAssets.Num());
	for (const FAssetData& SelectedAsset : SelectedAssets)
	{
		SelectedAssetPaths.Add(SelectedAsset.AssetClassPath);
	}

	LegacyContentSourceWidgets->FilterListPtr->SetActiveFilters(SelectedAssetPaths);
}

void SContentBrowser::HandleFavoriteShortcutCommandExecute(FName InCommandName)
{
	if (InCommandName.IsNone())
	{
		return;
	}

	const UContentBrowserFavoriteProjectSettings* FavoriteSettings = UContentBrowserFavoriteProjectSettings::Get();
	if (!FavoriteSettings)
	{
		return;
	}

	// Look for the item with this command assigned
	for (const FContentBrowserFavoriteItem& FavoriteItem : FavoriteSettings->FavoriteItems)
	{
		if (FavoriteItem.ShortcutCommandName == InCommandName)
		{
			const FContentBrowserModule& ContentBrowserModule = FModuleManager::Get().LoadModuleChecked<FContentBrowserModule>("ContentBrowser");
			ContentBrowserModule.Get().SyncBrowserToFolders({ FavoriteItem.Path });
			break;
		}
	}
}
...
bool SContentBrowser::HandleDeleteCommandCanExecute() const
{
	...
	// To avoid processing a delete operation for viewport/outliner when content browser is focus but nothing selected
	if (LegacyContentSourceWidgets->PathViewPtr->HasFocusedDescendants() || LegacyContentSourceWidgets->AssetViewPtr->HasFocusedDescendants())
	{
		return true;
	}
...
void SContentBrowser::HandleDeleteFavorite(TSharedPtr<SWidget> ParentWidget)
{
	...
	if (ParentWidget.IsValid() && SelectedFolders.Num() > 0)
	{
		...
		if (ContentBrowserUtils::DisplayConfirmationDialog(LOCTEXT("FavoriteRemoveConfirm_Yes", "Remove"), Prompt))
		{
...
void SContentBrowser::GetSelectionState(TArray<FAssetData>& SelectedAssets, TArray<FString>& SelectedPaths)
{
	...
	// Fallback
	else if (const FContentBrowserModule* ContentBrowserModule = FModuleManager::GetModulePtr<FContentBrowserModule>("ContentBrowser"))
	{
...
FText SContentBrowser::GetSearchAssetsHintText() const
{
	if (LegacyContentSourceWidgets->PathViewPtr.IsValid())
	{
		...
		if (Paths.Num() > 0)
		{
			...
			{
				const FString PathDisplayNameString = Paths[i].GetDisplayName().ToString();
				if (PathDisplayNameString.IsEmpty())
				{
					continue;
				}

				SearchHint += PathDisplayNameString;
				if (i + 1 < Paths.Num())
				{
					SearchHint += ", ";
				}
