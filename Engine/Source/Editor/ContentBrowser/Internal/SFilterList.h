...
public:
	...
	/** Toggle whether the filters are sorted and displayed alphabetically */
	void ToggleSortFiltersAlphabetically();
	...
	/** Returns all frontend filters registered with this filter bar (built-in, extra, and extension-provided). */
	const TArray<TSharedRef<FFrontendFilter>>& GetAllFrontendFilters() const { return AllFrontendFilters_Internal; }

	/** Returns all custom class/type filters registered with this filter bar. */
	const TArray<TSharedRef<FCustomClassFilterData>>& GetCustomClassFilters() const { return CustomClassFilters; }

	/**
	 * Activates a class filter by class path:
	 * - If not yet pinned: pins and enables it.
	 * - If already pinned (e.g. disabled): enables it in-place without re-pinning.
	 */
	CONTENTBROWSER_API void ActivateClassFilterByPath(const FTopLevelAssetPath& ClassPath);
