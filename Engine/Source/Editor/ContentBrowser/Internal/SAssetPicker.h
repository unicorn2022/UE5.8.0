...
public:
	...
	/** Clears the search box text programmatically. Triggers OnSearchTextChanged -> OnSearchTextChangedDelegate. */
	CONTENTBROWSER_API void ClearSearchText();

	/** Returns the filter list widget, or nullptr if none was created. */
	CONTENTBROWSER_API TSharedPtr<SFilterList> GetFilterList() const;
...
private:
	...
	/** Handler for when the user drags an asset */
	void HandleItemsDragged(TArrayView<const FContentBrowserItem> DraggedItems);
	...
	/** Returns true if the user is able to execute filter by selection request */
	bool CanFilterBySelection();

	/** Executes command and applies filters based on active selection */
	void OnFilterBySelection();
...
private:
	...
	/** Called any number of assets are dragged */
	FOnAssetsDragged OnAssetsDragged;

	/** Called when a folder is entered in the asset view */
	FOnPathSelected OnFolderEnteredDelegate;

	/** Fired when the search box text changes, forwarded from FAssetPickerConfig::OnSearchTextChanged. */
