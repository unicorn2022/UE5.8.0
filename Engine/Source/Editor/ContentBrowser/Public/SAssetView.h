...
public:
	SLATE_BEGIN_ARGS( SAssetView )
		...
		, _AssetShowWarningTextJustification(ETextJustify::Center)
		, _AssetShowWarningTextSelectable(false)
		...
		/** Called when the user double clicks, presses enter, or presses space on an Content Browser item */
		SLATE_EVENT( FOnContentBrowserItemsDragged, OnItemsDragged )
		...
		/** The warning text to show as heading (e.g. when there are no assets to show) */
		SLATE_ATTRIBUTE( FText, AssetShowWarningText )

		/** The justification to use for the AssetShowWarning text (see AssetShowWarningText) */
		SLATE_ATTRIBUTE( ETextJustify::Type, AssetShowWarningTextJustification )

		/** Whether the warning text is selectable or not (see AssetShowWarningText) */
		SLATE_ATTRIBUTE( bool, AssetShowWarningTextSelectable )
		...
		SLATE_EVENT( FOnColumnVisibilityChanged, OnColumnVisibilityChanged )
	...
	UE_DEPRECATED(5.8, "Use SyncToItems() instead.")
	...
	/** Gets the visibility for the majority asset type label */
	EVisibility GetMajorityAssetTypeTextVisibility() const;

	/** Gets the text for the majority asset type label */
	UE_API FText GetMajorityAssetTypeText() const;

	/** Gets the tooltip text for the majority asset type label */
	UE_API FText GetMajorityAssetTypeToolTipText() const;
	...
	/**
	 * Pass the active per-view search override instance to the asset view.
	 * The asset view will intercept items at the publish step and wire publish-results back.
	 * EXPERIMENTAL: This API is subject to change.
	 */
	PRAGMA_DISABLE_EXPERIMENTAL_WARNINGS
	UE_API void SetAssetSearchOverride(TSharedPtr<UE::Editor::ContentBrowser::Extension::ICBSearchOverrideInstance> InOverride);
	...
	UE_INTERNAL UE_API bool IsShowingFavorites() const;

	/** @return true when we are showing C++ content */
	UE_INTERNAL UE_API bool IsShowingCppContent() const;

	/** @return true when we are showing folders */
	UE_INTERNAL UE_API bool IsShowingFolders() const;

	/** @return true when we are showing empty folders */
	UE_INTERNAL UE_API bool IsShowingEmptyFolders() const;

	/** @return true when we are showing plugin content */
	UE_INTERNAL UE_API bool IsShowingPluginContent() const;

	/** @return true when we are showing engine content */
	UE_INTERNAL UE_API bool IsShowingEngineContent() const;

	/** @return true when we are showing the developers content */
	UE_INTERNAL UE_API bool IsShowingDevelopersContent() const;

	/** @return true when we are showing the all virtual folder */
	UE_INTERNAL UE_API bool IsShowingAllFolder() const;

	/** @return true when we are organizing folders into virtual locations */
	UE_INTERNAL UE_API bool IsOrganizingFolders() const;

	/** Is the current thumbnail size menu option checked */
	UE_INTERNAL UE_API bool IsThumbnailSizeChecked(EThumbnailSize InThumbnailSize) const;

	/** Returns true if ViewType is the current view type */
	UE_INTERNAL UE_API bool IsCurrentViewType(EAssetViewType::Type ViewType) const;
...
private:
	...
	/** Gets the warning text to show as heading (e.g. when there are no assets to show) */
...
private:
	/** Cancel the ongoing rename if there is one before starting another one. */
	...
	/** Active per-view search override instance; when set, intercepts items at the publish step. */
	PRAGMA_DISABLE_EXPERIMENTAL_WARNINGS
	TSharedPtr<UE::Editor::ContentBrowser::Extension::ICBSearchOverrideInstance> AssetSearchOverride;
	PRAGMA_ENABLE_EXPERIMENTAL_WARNINGS
	...
	/** Called when the user drags a Content Browser item */
	FOnContentBrowserItemsDragged OnItemsDragged;
	...
	/** Called when the user changes column visibility */
	FOnColumnVisibilityChanged OnColumnVisibilityChanged;
	...
	/** When in columns view, this is the display name of the asset type which is most commonly found in the recent results */
	FText MajorityAssetTypeText;
	...
	/** The justification to use for the AssetShowWarning text (see AssetShowWarningText) */
	TAttribute<ETextJustify::Type> AssetShowWarningTextJustification;
	
	/** Whether to allow selection of the warning text or not */
	TAttribute<bool> AssetShowWarningTextSelectable;
	...
	/** Existing item paths to avoid duplicates */
