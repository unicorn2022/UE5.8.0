...
/** Called when the user drags one or more Content Browser items */
DECLARE_DELEGATE_OneParam(FOnContentBrowserItemsDragged, TArrayView<const FContentBrowserItem> /*Items*/);
...
DECLARE_DELEGATE_OneParam(FOnSearchTextChanged, const FText& /*InSearchText*/)
...
/** Called to save the asset view settings (column widths, sort order, hidden columns, etc.) */
DECLARE_DELEGATE(FSaveAssetViewSettingsDelegate);

/** Called to load/restore the asset view settings */
DECLARE_DELEGATE(FLoadAssetViewSettingsDelegate);
...
/** Called when asset(s) are dragged in the asset view */
DECLARE_DELEGATE_OneParam(FOnAssetsDragged, const TArray<FAssetData>& /*DraggedAssets*/);

/** Called when the user changes which columns are hidden in the asset column/list view */
DECLARE_DELEGATE_OneParam(FOnColumnVisibilityChanged, const TArray<FString>& /*HiddenColumnIds*/);
...
/** Called to check if should show menu entries to set asset public/private */
DECLARE_DELEGATE_RetVal_TwoParams(bool, FOnContentBrowserCanShowSetAssetAccessMenu, const FStringView /*AssetPath*/, const FAssetData& /*Asset*/);
