...
/** A filter that displays only assets the are mark as Favorite */
class FFrontendFilter_Favorites : public FFrontendFilter
{
public:
	UE_API FFrontendFilter_Favorites(TSharedPtr<FFrontendFilterCategory> InCategory);
	~FFrontendFilter_Favorites() = default;

	// FFrontendFilter implementation
	virtual FString GetName() const override { return "Favorites"; }
	virtual FText GetDisplayName() const override { return LOCTEXT("FrontendFilter_Favorite", "Favorites"); }
	virtual FText GetToolTipText() const override { return LOCTEXT("FrontendFilter_FavoriteTooltip", "Filter by favorite assets."); }
	virtual FLinearColor GetColor() const override { return FStyleColors::AccentYellow.GetSpecifiedColor(); }
