...
struct FAssetPickerConfig
{
	...
	/** A pointer to an existing delegate that, when executed, will save the asset view settings. */
	TArray<FSaveAssetViewSettingsDelegate*> SaveAssetViewSettingsDelegates;

	/** A pointer to an existing delegate that, when executed, will load/restore the asset view settings. */
	TArray<FLoadAssetViewSettingsDelegate*> LoadAssetViewSettingsDelegates;
	...
	/** The delegate that fires when any number of assets are dragged */
	FOnAssetsDragged OnAssetsDragged;

	/** Fires when the user changes which columns are hidden in the column or list view. Useful for syncing column state. */
	...
	/** Fired when the search box text changes. Allows embedding widgets to observe the search text without replacing the built-in search behavior. */
	...
	/**
	 * Can be used to extend the asset picker top bar via slot manipulation. Current default slots are as follows:
	 * 
	 * 0. Add Filter Menu
	 * 1. SearchBar
	 * 2. Show Developer Content Checkbox
	 * 3. Asset View Options
	 */
	TDelegate<void(TSharedRef<SHorizontalBox>)> OnExtendAssetPickerTopBar;
...
struct FSharedAssetDialogConfig
{
	...
	virtual ~FSharedAssetDialogConfig() = default;
};
...
public:
	...
	/** Virtual destructor */
	virtual ~IContentBrowserSingleton() = default;
	...
	/**
	 * Selects the supplied collections in the primary content browser.
	 *
	 * @param CollectionsToSync				An array of collections to sync.
	 * @param bAllowLockedBrowsers 	        When true, even locked browsers may handle the sync. Only set to true if the sync doesn't seem external to the content browser.
	 * @param bFocusContentBrowser          When true, brings the ContentBrowser into the foreground.
	 * @param InstanceName					When supplied, will only sync the Content Browser with the matching InstanceName.  bAllowLockedBrowsers is ignored.
	 * @param bNewSpawnBrowser				When supplied, will spawn a new Content Browser instead of selecting the assets in an existing one.
	 */
	virtual void SyncBrowserToCollections(const TArray<FCollectionRef>& CollectionsToSync, bool bAllowLockedBrowsers = false, bool bFocusContentBrowser = true, const FName& InstanceName = FName(), bool bNewSpawnBrowser = false) = 0;
	...
	/** Generates a list of assets that are selected across all open content browsers */
	virtual void GetAllSelectedAssets(TArray<FAssetData>& SelectedAssets) = 0;
	...
	*/
	...
	/** Triggers a refresh of favorite path views in content browser */
	UE_EXPERIMENTAL(5.8, "Experimental way to refresh the content browser favorites, API may change in the future")
	virtual void RefreshFavoritesPathView() = 0;
