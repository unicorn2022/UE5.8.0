...
public:
	...
	void OnCommitAssetName(const FText& InText, ETextCommit::Type CommitInfo)
	{
		if(Item.IsValid())
		{
			...
			{
				if (const FText TrimText = FText::TrimPrecedingAndTrailing(InText); !TrimText.IsEmpty())
				{
					const FString SanitizedName = ObjectTools::SanitizeObjectName(TrimText.ToString());

					if(Asset->GetName() != SanitizedName)
					{
						// Check for an existing object, and if we find one build a unique name based on the request
						const FString NewName = StaticFindObject(UClothingAssetBase::StaticClass(), Asset->GetOuter(), *SanitizedName) ?
							MakeUniqueObjectName(Asset->GetOuter(), UClothingAssetBase::StaticClass(), FName(SanitizedName)).ToString() :
							SanitizedName;

						FScopedTransaction CurrTransaction(LOCTEXT("RenameClothingAsset_Transaction", "Rename Clothing Data."));
						Asset->Modify();
						Asset->Rename(*NewName, Asset->GetOuter());
					}
				}
			}
		}
	...
	void BindCommands()
	{
		...
		UICommandList->MapAction(
			...
			FExecuteAction::CreateSP(this, &SAssetListRow::RenameAsset)
		);
		...
		UICommandList->MapAction(
			Commands.DeleteAssetLastLod,
			FExecuteAction::CreateSP(this, &SAssetListRow::DeleteAssetLastLod),
			FCanExecuteAction::CreateSP(this, &SAssetListRow::CanDeleteAssetLastLod)
		);
	...
	virtual FReply OnMouseButtonUp(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent) override
	{
		...
		{
			const UClothingAssetCommon* const Asset = Cast<UClothingAssetCommon>(Item->ClothingAsset.Get());  // Only the UClothingAssetCommon type have LODs
			const int32 LastLodIndex = Asset ? Asset->GetNumLods() - 1 : INDEX_NONE;
			...
			Builder.BeginSection(NAME_None, LOCTEXT("AssetActions_SectionName", "Actions"));
			{
				...
				if (LastLodIndex > 0)  // Can't delete LOD0
				{
					Builder.AddMenuEntry(Commands.DeleteAssetLastLod, NAME_None,
						FText::Format(LOCTEXT("DeleteAssetLastLod", "Delete LOD{0}"), LastLodIndex));
				}
...
private:
	...
	void DeleteAsset()
	{
		...
		if(UClothingAssetBase* Asset = Item->ClothingAsset.Get())
		{
			if(USkeletalMesh* SkelMesh = Cast<USkeletalMesh>(Asset->GetOuter()))
			{
				...
				{
					FScopedTransaction CurrTransaction(LOCTEXT("DeleteClothingAsset_Transaction", "Delete Clothing Data."));
					SkelMesh->Modify();
					Asset->Modify();
					...
					Asset->UnbindFromSkeletalMesh(SkelMesh, INDEX_NONE, INDEX_NONE);
					SkelMesh->GetMeshClothingAssets().RemoveAt(AssetIndex);
					Asset->Rename(nullptr, GetTransientPackage());  // Rename so that the old object name can be reused if needed
					Asset = nullptr;
	...
	bool CanDeleteAssetLastLod()
	{
		if (Item.IsValid())
		{
			if (UClothingAssetCommon* const Asset = Cast<UClothingAssetCommon>(Item->ClothingAsset.Get()))  // Only the UClothingAssetCommon type have LODs
			{
				return Asset->GetNumLods() > 1;  // Can't delete LOD0
			}
		}
		return false;
	}

	void DeleteAssetLastLod()
	{
		if (Item.IsValid())
		{
			if (UClothingAssetCommon* const Asset = Cast<UClothingAssetCommon>(Item->ClothingAsset.Get()))  // Only the UClothingAssetCommon type have LODs
			{
				if (ensure(Asset->GetNumLods() > 1))  // Can't delete LOD0
				{
					if (USkeletalMesh* const SkeletalMesh = Cast<USkeletalMesh>(Asset->GetOuter()))
					{
						FScopedSuspendAlternateSkinWeightPreview ScopedSuspendAlternateSkinnWeightPreview(SkeletalMesh);
						FScopedSkeletalMeshPostEditChange ScopedPostEditChange(SkeletalMesh);
						FScopedTransaction CurrTransaction(LOCTEXT("DeleteLastLod_Transaction", "Delete the last Clothing Data LOD."));
						SkeletalMesh->Modify();
						Asset->Modify();
...
public:
	...
	virtual TSharedRef<SWidget> GenerateWidgetForColumn(const FName& InColumnName) override
	{
		...
		if(InColumnName == Column_CurrentTarget)
		{
			...
			if (const TSharedPtr<const SClothAssetSelector> AssetSelector = AssetSelectorPtr.Pin())
			{
				if (const TStrongObjectPtr<const UClothingAssetBase> ClothingAsset = AssetSelector->GetSelectedAsset().Pin())
				{
					if (const UClothingSimulationFactory* const ClothingSimulationFactory = UClothingSimulationFactory::GetClothingSimulationFactory(ClothingAsset.Get()))
					{
						const UEnum* const Enum = ClothingSimulationFactory->GetWeightMapTargetEnum();
		...
		Builder.BeginSection(NAME_None, LOCTEXT("MaskTargets_SectionName", "Targets"));
		{
			...
			if (const TSharedPtr<const SClothAssetSelector> AssetSelector = AssetSelectorPtr.Pin())
			{
				if (const TStrongObjectPtr<const UClothingAssetBase> ClothingAsset = AssetSelector->GetSelectedAsset().Pin())
				{
					if (const UClothingSimulationFactory* const ClothingSimulationFactory = UClothingSimulationFactory::GetClothingSimulationFactory(ClothingAsset.Get()))
					{
						if (const UEnum* const Enum = ClothingSimulationFactory->GetWeightMapTargetEnum())
						{
...
void SClothAssetSelector::RefreshAssetList()
{
	...
	// Refresh LOD list
	constexpr bool bRefreshMasks = false;  // Will be done in RefreshMaskList() running afterwards
	if (const UClothingAssetCommon* const ClothingAssetCommon = Cast<UClothingAssetCommon>(CurrSelectedAsset))  // Only the UClothingAssetCommon type assets have LODs
	{
		SetSelectedLod(FMath::Min(SelectedLod, ClothingAssetCommon->GetNumLods() - 1), bRefreshMasks);
	}
	else
	{
		SetSelectedLod(INDEX_NONE, bRefreshMasks);
	}
}
