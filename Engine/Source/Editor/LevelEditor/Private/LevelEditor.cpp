...
public:
	...
	void Construct(const FArguments& InArgs)
	{
		...
		FFormatNamedArguments LabelArgs;
		LabelArgs.Add(TEXT("ProjectNameWatermarkPrefix"), FText::FromString(ProjectNameWatermarkPrefix));
		LabelArgs.Add(TEXT("Branch"), FEngineBuildSettings::IsPerforceBuild() ? FText::FromString(TEXT(" ") + FEngineVersion::Current().GetBranch()) : FText::GetEmpty());
		LabelArgs.Add(TEXT("GameName"), FText::FromString(TEXT(" ") + FString(FApp::GetProjectName())));
		LabelArgs.Add(TEXT("EngineVersion"), (GetDefault<UEditorPerProjectUserSettings>()->bDisplayEngineVersionInBadge) ? FText::FromString(TEXT(" (") + EngineVersionString + TEXT(")")) : FText::GetEmpty());
		LabelArgs.Add(TEXT("Config"), bAppendBuildConfig ? FText::Format(NSLOCTEXT("UnrealEditor", "TitleBarConfig", " [{0}]"), EBuildConfigurations::ToText(BuildConfig)) : FText::GetEmpty());

		const FText RightContentText = FText::Format(NSLOCTEXT("UnrealEditor", "TitleBarRightContentAndConfig", "{ProjectNameWatermarkPrefix}{GameName}{Config}{Branch}{EngineVersion}"), LabelArgs);
	
		// Create the tooltip showing more detailed information
		{
			const FText RightContentTooltip = FText::FormatOrdered(NSLOCTEXT("UnrealEditor", "TitleBarRightContentInfoTooltip", "{0}\n\nRight Click to open context menu with various clipboard options\nDouble Click to quickly copy changelist to clipboard"), GetAllInfoText());
		...
		SBox::Construct(SBox::FArguments()
			...
			[
				SNew(SBorder)
				.OnMouseDoubleClick(this, &SProjectBadge::OnBadgeDoubleClick)
				.OnMouseButtonDown(this, &SProjectBadge::OnOpenContextMenu)
...
private:
	static void CopyToClipboard(const FText& InKey, const FText& InValue)
	{
		FPlatformApplicationMisc::ClipboardCopy(*InValue.ToString());
		
		// Notify of the action
		FNotificationInfo NotificationInfo(FText::Format(LOCTEXT("ProjectBadgeCopyNotification", "{0} value copied to clipboard\n{1}"), InKey, InValue));
	...
	static FText GetProjectNameText()
	{
		return FText::FromString(FApp::GetProjectName());
	}

	static FText GetVersionText()
	{
		const FString EngineVersionString = FEngineVersion::Current().ToString(FEngineVersion::Current().HasChangelist() ? EVersionComponent::Changelist : EVersionComponent::Patch);
		return FText::FromString(EngineVersionString);
	}
	
	static FText GetChangelistText()
	{
		return FText::FromString(LexToString(FEngineVersion::Current().GetChangelist()));
	}
	
	static FText GetBranchText()
	{
		return FText::FromString(FEngineVersion::Current().GetBranch());
	}
	
	static FText GetBuildConfigurationText()
	{
		return EBuildConfigurations::ToText(FApp::GetBuildConfiguration());
	}

	static FText GetBuildDateText()
	{
		return FText::FromString(FApp::GetBuildDate());
	}
	
	static FText GetGraphicsRHIText()
	{
		return FText::FromString(FApp::GetGraphicsRHI());
	}
	
	static FText GetAllInfoText()
	{
		FFormatNamedArguments TooltipArgs;
		TooltipArgs.Add(TEXT("Version"), GetVersionText());
		TooltipArgs.Add(TEXT("Changelist"), GetChangelistText());
		TooltipArgs.Add(TEXT("Branch"), GetBranchText());
		TooltipArgs.Add(TEXT("BuildConfiguration"), GetBuildConfigurationText());
		TooltipArgs.Add(TEXT("BuildDate"), GetBuildDateText());
		TooltipArgs.Add(TEXT("GraphicsRHI"), GetGraphicsRHIText());
		return FText::Format(NSLOCTEXT("UnrealEditor", "TitleBarRightContentTooltip", "Version: {Version}\nBranch: {Branch}\nBuild Configuration: {BuildConfiguration}\nBuild Date: {BuildDate}\nGraphics RHI: {GraphicsRHI}"), TooltipArgs);
	}

	static TSharedRef<SWidget> CreateContextMenu()
	{
		FMenuBuilder MenuBuilder(/** CloseAfterSelection */true, /** CommandList */nullptr);
		
		const FText LabelFormat = LOCTEXT("ProjectBadgeContextLabel", "Copy {0} ({1})");
		const FText TooltipFormat = LOCTEXT("ProjectBadgeContextTooltip", "Copy {0} value to the clipboard:\n{1}");

		// ProjectName
		{
			const FText ProjectNameLabel = LOCTEXT("ProjectNameLabel", "Project Name");
			const FText ProjectNameValue = GetProjectNameText();

			MenuBuilder.AddMenuEntry(
				FText::Format(LabelFormat, ProjectNameLabel, ProjectNameValue),
				FText::Format(TooltipFormat, ProjectNameLabel, ProjectNameValue),
				FSlateIcon(FCoreStyle::Get().GetStyleSetName(), "GenericCommands.Copy"),
				FUIAction(FExecuteAction::CreateLambda(&SProjectBadge::CopyToClipboard, ProjectNameLabel, ProjectNameValue))
			);
		}
		
		// Version
		{
			const FText VersionLabel = LOCTEXT("VersionLabel", "Version");
			const FText VersionValue = GetVersionText();

			MenuBuilder.AddMenuEntry(
				FText::Format(LabelFormat, VersionLabel, VersionValue),
				FText::Format(TooltipFormat, VersionLabel, VersionValue),
				FSlateIcon(FCoreStyle::Get().GetStyleSetName(), "GenericCommands.Copy"),
				FUIAction(FExecuteAction::CreateLambda(&SProjectBadge::CopyToClipboard, VersionLabel, VersionValue))
			);
		}

		// Changelist
		{
			const FText ChangelistLabel = LOCTEXT("ChangelistLabel", "Changelist");
			const FText ChangelistValue = GetChangelistText();

			MenuBuilder.AddMenuEntry(
				FText::Format(LabelFormat, ChangelistLabel, ChangelistValue),
				FText::Format(TooltipFormat, ChangelistLabel, ChangelistValue),
				FSlateIcon(FCoreStyle::Get().GetStyleSetName(), "GenericCommands.Copy"),
				FUIAction(FExecuteAction::CreateLambda(&SProjectBadge::CopyToClipboard, ChangelistLabel, ChangelistValue))
			);
		}
		
		// Branch
		{
			const FText BranchLabel = LOCTEXT("BranchLabel", "Branch");
			const FText BranchValue = GetBranchText();

			MenuBuilder.AddMenuEntry(
				FText::Format(LabelFormat, BranchLabel, BranchValue),
				FText::Format(TooltipFormat, BranchLabel, BranchValue),
				FSlateIcon(FCoreStyle::Get().GetStyleSetName(), "GenericCommands.Copy"),
				FUIAction(FExecuteAction::CreateLambda(&SProjectBadge::CopyToClipboard, BranchLabel, BranchValue))
			);
		}
		
		// BuildConfiguration
		{
			const FText BuildConfigurationLabel = LOCTEXT("BuildConfigurationLabel", "Build Configuration");
			const FText BuildConfigurationValue = GetBuildConfigurationText();

			MenuBuilder.AddMenuEntry(
				FText::Format(LabelFormat, BuildConfigurationLabel, BuildConfigurationValue),
				FText::Format(TooltipFormat, BuildConfigurationLabel, BuildConfigurationValue),
				FSlateIcon(FCoreStyle::Get().GetStyleSetName(), "GenericCommands.Copy"),
				FUIAction(FExecuteAction::CreateLambda(&SProjectBadge::CopyToClipboard, BuildConfigurationLabel, BuildConfigurationValue))
			);
		}
		
		// BuildDate
		{
			const FText BuildDateLabel = LOCTEXT("BuildDateLabel", "Build Date");
			const FText BuildDateValue = GetBuildDateText();

			MenuBuilder.AddMenuEntry(
				FText::Format(LabelFormat, BuildDateLabel, BuildDateValue),
				FText::Format(TooltipFormat, BuildDateLabel, BuildDateValue),
				FSlateIcon(FCoreStyle::Get().GetStyleSetName(), "GenericCommands.Copy"),
				FUIAction(FExecuteAction::CreateLambda(&SProjectBadge::CopyToClipboard, BuildDateLabel, BuildDateValue))
			);
		}
		
		// GraphicsRHI
		{
			const FText GraphicsRHILabel = LOCTEXT("GraphicsRHILabel", "Graphics RHI");
			const FText GraphicsRHIValue = GetGraphicsRHIText();

			MenuBuilder.AddMenuEntry(
				FText::Format(LabelFormat, GraphicsRHILabel, GraphicsRHIValue),
				FText::Format(TooltipFormat, GraphicsRHILabel, GraphicsRHIValue),
				FSlateIcon(FCoreStyle::Get().GetStyleSetName(), "GenericCommands.Copy"),
				FUIAction(FExecuteAction::CreateLambda(&SProjectBadge::CopyToClipboard, GraphicsRHILabel, GraphicsRHIValue))
			);
		}
		
		// All info
		{
			const FText AllInfoLabel = LOCTEXT("AllLabel", "Copy All Info");
			const FText AllInfoValue = GetAllInfoText();

			MenuBuilder.AddMenuEntry(
				AllInfoLabel,
				FText::Format(TooltipFormat, AllInfoLabel, AllInfoValue),
				FSlateIcon(FCoreStyle::Get().GetStyleSetName(), "GenericCommands.Copy"),
				FUIAction(FExecuteAction::CreateLambda(&SProjectBadge::CopyToClipboard, AllInfoLabel, AllInfoValue))
			);
		}
	...
	FReply OnBadgeDoubleClick(const FGeometry& InGeometry, const FPointerEvent& InEvent)
	{
		const FText ChangelistText = GetChangelistText();
		if (!ChangelistText.IsEmpty())
		{
			CopyToClipboard(LOCTEXT("ChangelistLabel", "Changelist"), ChangelistText);
			return FReply::Handled();
		}
		return FReply::Unhandled();
	}

	FReply OnOpenContextMenu(const FGeometry& InGeometry, const FPointerEvent& InEvent)
	{
...
template <>
struct TWidgetTypeTraits<SProjectBadge>
{
...
TSharedRef<SDockTab> FLevelEditorModule::SpawnLevelEditor( const FSpawnTabArgs& InArgs )
{
	...
	TSharedRef<SDockTab> LevelEditorTab = SNew(SDockTab)
		.OnExtendContextMenu(SDockTab::FExtendContextMenu::CreateRaw(this, &FLevelEditorModule::OnExtendTabContextMenu))
	...
	if (OwnerWindow.IsValid())
	{
		// Make sure that any mode that could hold on to the old SLevelEditor releases it before setting the new one below
...
void FLevelEditorModule::OnExtendTabContextMenu(FMenuBuilder& InMenuBuilder)
{
	InMenuBuilder.BeginSection("DockingTabLevelOptions", LOCTEXT("LevelTabSection", "Level"));
	{
		InMenuBuilder.AddMenuEntry(
			LOCTEXT("BrowseLevelInContentBrowser", "Browse to Level"),
			LOCTEXT("BrowseLevelInContentBrowserTooltip", "Browse to level in content browser"),
			FSlateIcon(FAppStyle::GetAppStyleSetName(), "Icons.BrowseContent"),
			FUIAction(
				FExecuteAction::CreateRaw(this, &FLevelEditorModule::OnBrowseLevelInContentBrowser),
				FCanExecuteAction::CreateRaw(this, &FLevelEditorModule::OnCanBrowseLevelInContentBrowser)
			)
		);
	}
...
bool FLevelEditorModule::OnCanBrowseLevelInContentBrowser()
{
	if (const TSharedPtr<ILevelEditor> LevelEditor = GetLevelEditorInstance().Pin())
	{
		const UWorld* World = LevelEditor->GetWorld();

		return !!World
			&& !!World->PersistentLevel
			&& !!World->PersistentLevel->GetPackage()
			&& !World->PersistentLevel->GetPackage()->GetName().StartsWith("/Temp/");
	}

	return false;
}

void FLevelEditorModule::OnBrowseLevelInContentBrowser()
{
	if (const TSharedPtr<ILevelEditor> LevelEditor = GetLevelEditorInstance().Pin())
	{
		UWorld* World = LevelEditor->GetWorld();
		...
		if (!!World
			...
			&& !World->PersistentLevel->GetPackage()->GetName().StartsWith("/Temp/"))
		{
			if (const FContentBrowserModule* ContentBrowserModule = FModuleManager::GetModulePtr<FContentBrowserModule>("ContentBrowser"))
			{
				ContentBrowserModule->Get().SyncBrowserToAssets({World->PersistentLevel});
			}
		}
	}
}
...
void FLevelEditorModule::StartupModule()
{
	...
	FCoreDelegates::GetOnPostEngineInit().AddStatic(&UE::Editor::LevelEditor::Private::OnPostEngineInit);
}
...
void FLevelEditorModule::BindGlobalLevelEditorCommands()
{
	...
	ActionList.MapAction(
		...
		FExecuteAction::CreateStatic(&FLevelEditorActionCallbacks::PlayFromHere)
	);
	...
	ActionList.MapAction(Commands.BuildHLODs_SelectedActors,
		FExecuteAction::CreateStatic(&FLevelEditorActionCallbacks::BuildHLODs_SelectedActors_Execute),
		FCanExecuteAction::CreateStatic(&FLevelEditorActionCallbacks::IsWorldPartitionStreamingEnabled));
	...
	ActionList.MapAction(
		Commands.TogglePreviewPlatform,
		FExecuteAction::CreateStatic(&FLevelEditorActionCallbacks::TogglePreviewPlatform),
		FIsActionChecked::CreateStatic(&FLevelEditorActionCallbacks::IsPreviewPlatformEnabled),
		FIsActionChecked::CreateStatic(&FLevelEditorActionCallbacks::IsPreviewPlatformActive),
				...
				if (CommandListJson != nullptr)
				{
					for (const FLevelEditorCommands::PreviewPlatformCommand& PreviewJsonPlatform : *CommandListJson)
					{
						...
						else
						{
							...
							ActionJsonFile.ExecuteAction = FExecuteAction::CreateStatic(&FLevelEditorActionCallbacks::PreviewJson_Clicked, Item.PlatformName, PreviewJsonPlatform.FilePath);
...
void FLevelEditorModule::RefreshLevelEditorOutliners() const
{
	static const FName TabIDS[] = {
		LevelEditorTabIds::LevelEditorSceneOutliner,
		LevelEditorTabIds::LevelEditorSceneOutliner2,
		LevelEditorTabIds::LevelEditorSceneOutliner3,
		LevelEditorTabIds::LevelEditorSceneOutliner4 };
