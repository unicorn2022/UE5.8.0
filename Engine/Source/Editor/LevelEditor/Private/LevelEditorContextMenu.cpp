...
public:
	...
	/** Fills in menu options for typed element visibility menu */
	static void FillElementVisibilityMenu(UToolMenu* Menu);
...
void FLevelEditorContextMenu::RegisterActorContextMenu()
{
	...
	Menu->AddDynamicSection("ActorContextMenuDynamic", FNewToolMenuDelegate::CreateLambda([](UToolMenu* InMenu)
	{
		...
		if(FModuleManager::LoadModuleChecked<FPropertyEditorModule>( "PropertyEditor" ).GetCanUsePropertyMatrix() 
			&& NumSelectedActors >= 2)
		{
			// Options that relate to bulk editing assets
			// In most cases, you DO NOT want to extend this section; look at ActorUETools or ActorTypeTools below
			
			// Figure out whether the selected actors are actually the same thing we would see in a detail panel,
			//  or whether they are actually stand-ins for some non-actors.
			bool bDetailsObjectsAreNotActors = InternalEditorLevelLibrary::DoesDetailSelectionContainNonActorsOrComponents(LevelEditorContext->CurrentSelection);

			if (!bDetailsObjectsAreNotActors || FPropertyEditorModule::CVarAllowPropertyMatrixForNonActors.GetValueOnAnyThread())
			{
				...
				// The previous entry for opening the selection in the property matrix works even in cases where 
				//  the selected actors are stand-ins for something different that is editable (since the true 
				//  targets will be gotten via ITypedElementDetailsInterface). On the other hand, editing components
				//  of the selection is harder to generalize in a way that is overridable by plugins. So, we will
				//  not add this entry in that case. A plugin that uses stand-in actors can add its own "Edit Components"
				//  entry if it likes.
				if (!bDetailsObjectsAreNotActors)
				{
					Section.AddSubMenu(
			...
			if (SelectedActors.Num() == 1)
			{
				...
				if (Viewport.IsValid())
				{
					...
					if (ViewportClient.IsPerspective() && !ViewportClient.IsLockedToCinematic())
					{
						...
						// Only allowed on actor with bCanPlayFromHere = true
						FToolMenuEntry& PlayFromHereEntry = Section.AddMenuEntry(FLevelEditorCommands::Get().PlayFromHere);
						PlayFromHereEntry.Visibility = FIsActionButtonVisible::CreateStatic(&FLevelEditorActionCallbacks::PlayFromHere_IsVisible);
					}
				}
			}
		}
...

		{
			...
			if (LevelEditorContext->ContextType == ELevelEditorMenuContext::Viewport)
			{
				...
				if (!FLevelEditorActionCallbacks::PlayFromHere_IsVisible())
				{
					...
					Section.AddMenuEntry(FLevelEditorCommands::Get().PlayFromHere, PlayFromHereLabel);
				}
			}
...
void FLevelEditorContextMenu::RegisterElementContextMenu()
{
	...
	Menu->AddDynamicSection("ElementContextMenuDynamic", FNewToolMenuDelegate::CreateLambda([](UToolMenu* InMenu)
	{
		...
		// This menu will mostly follow FLevelEditorContextMenu::RegisterActorContextMenu as more support for
		//  non-actors is added.

		{
			// Options that affect the current viewport.
			FToolMenuSection& Section = InMenu->AddSection("ElementViewOptions", LOCTEXT("ViewOptionsHeading", "View Options"));
			const FVector* ClickLocation = &GEditor->ClickLocation;
			...
			// TODO: Support piloting like we do for actors. This requires changes to viewport code.
		}

		{
			// Options for editing, transforming, and manipulating this element
			FToolMenuSection& Section = InMenu->AddSection("ElementOptions", LOCTEXT("ElementOptionsHeading", "Element Options"));

			// TODO: Duplicate and Rename in this submenu still need implementing. Duplicate requires implementing
			//  UEntityElementWorldInterface::DuplicateElements, whereas Rename requires support from the outliner,
			//  as the outliner requires something to be selected in it.
			Section.AddSubMenu(
				"EditSubMenu",
				LOCTEXT("EditSubMenu", "Edit"),
				FText::GetEmpty(),
			...
			Section.AddSubMenu(
				"VisibilitySubMenu",
				LOCTEXT("VisibilityElementSubMenu", "Visibility"),
				LOCTEXT("VisibilityElementSubMenu_ToolTip", "Selected element visibility options"),
				FNewToolMenuDelegate::CreateStatic(&FLevelEditorContextMenuImpl::FillElementVisibilityMenu),
				false, // default value
				FSlateIcon(FAppStyle::GetAppStyleSetName(), "Icons.Visibility"));

			Section.AddSubMenu(
				"TransformSubMenu",
				LOCTEXT("TransformElementSubMenu", "Transform"),
				LOCTEXT("TransformElementSubMenu_ToolTip", "Element transform utils"),
				FNewToolMenuDelegate::CreateStatic(&FLevelEditorContextMenuImpl::FillTransformMenu),
				false, // default value
				FSlateIcon(FAppStyle::GetAppStyleSetName(), "Icons.Transform"));
			...
			Section.AddSubMenu(
				"SnapAlignSubMenu",
				LOCTEXT("SnapAlignElementSubMenu", "Snapping"),
				LOCTEXT("SnapAlignElementSubMenu_ToolTip", "Element snap/align utils"),
				FNewToolMenuDelegate::CreateStatic(&FLevelEditorContextMenuImpl::FillSnapAlignMenu),
				false, // default value
				FSlateIcon(FAppStyle::GetAppStyleSetName(), "Icons.Snap"));
			...
			Section.AddSubMenu(
				"PivotSubMenu",
				LOCTEXT("PivotElementSubMenu", "Pivot"),
				LOCTEXT("PivotElementSubMenu_ToolTip", "Element pivoting utils"),
				FNewToolMenuDelegate::CreateStatic(&FLevelEditorContextMenuImpl::FillPivotMenu),
				false, // default value
				FSlateIcon(FAppStyle::GetAppStyleSetName(), "Icons.SetShowPivot"));

			// TODO: Attach/detach requires additional support. Attach, in particular, requires ability to show
			//  elements to which attachment is possible.
			//Section.AddSubMenu(
			//	"ElementAttachToSubMenu",
			//	LOCTEXT("ElementAttachToSubMenu", "Attach To"),
			//	LOCTEXT("ElementAttachToSubMenu_ToolTip", "Attach Element as child"),
			//	FNewToolMenuDelegate::CreateStatic(&FLevelEditorContextMenuImpl::FillActorMenu),
			//	false, // default value
			//	FSlateIcon(FAppStyle::GetAppStyleSetName(), "Actors.Attach"));
			//Section.AddMenuEntry(
		...
		// General-purpose extension point for tools that apply to many types of elements, analogous to ActorUETools
		if (LevelEditorContext->ContextType == ELevelEditorMenuContext::MainMenu)
		{
			FToolMenuSection& Section = InMenu->AddSection("ElementUETools", LOCTEXT("UEToolsHeading", "UE Tools"));
		}

		// General-purpose extension point for tools that only appear for certain types of elements, analogous to ActorTypeTools
		FToolMenuSection& ElementTypeToolsSection = InMenu->AddSection("ElementTypeTools");

		{
			// Play from here
			// TODO: May someday support custom play-from-here behavior the way that some actors do, and
			//  may someday support simulation changes
			FToolMenuSection& Section = InMenu->AddSection("ElementPreview", LOCTEXT("PreviewHeading", "Preview"));
			const FText PlayFromHereLabel = GEditor->OnlyLoadEditorVisibleLevelsInPIE() ? LOCTEXT("PlayFromHereVisible", "Play From Here (visible levels)") : LOCTEXT("PlayFromHere", "Play From Here");
			Section.AddMenuEntry(FLevelEditorCommands::Get().PlayFromHere, PlayFromHereLabel);
		}
	}));
...
void FLevelEditorContextMenu::RegisterAsDerivedMenu(FName MenuNameToUse, FName BaseMenuName, const FToolMenuInsert& InsertLocation)
{
	UToolMenus* ToolMenus = UToolMenus::Get();
	if (!ensure(ToolMenus))
	{
	...
	if (!ToolMenus->IsMenuRegistered(MenuNameToUse))
	{
		// Inherit from the passed-in context
		UToolMenu* Menu = ToolMenus->RegisterMenu(MenuNameToUse, BaseMenuName);

		// Add everything that we would show in the level editor context menu
		...
		Section.InsertPosition = InsertLocation;
	}
}
...
void FLevelEditorContextMenuImpl::FillElementVisibilityMenu(UToolMenu* Menu)
{
	{
...
{
	FillElementVisibilityMenu(Menu);
