...
#include "Elements/Columns/TypedElementCompatibilityColumns.h" // FTypedElementActorTag
...
namespace LevelEditorActionsHelpers
{
	...
	// Call VisitFn on each selected non-actor with a FVisibleInEditorColumn. Returns true if completed fully.
	bool ForEachSelectedHideableNonActor(UWorld* WorldIn,
		TFunctionRef<void(UE::Editor::DataStorage::Queries::EFlowControl& Flow, const FVisibleInEditorColumn& VisibilityColumn)> VisitFn)
	{
		...
		// Note: this query currently never gets unregistered because it is constantly used in CanExecute calls
		//  while the visibility submenu is open, and we can't unregister on module shutdown because the level
		//  editor isn't really a module that would be deactivated. 
		static QueryHandle Query = DataStorage->RegisterQuery(
			Select()
			.ReadOnly<FTypedElementWorldColumn, FVisibleInEditorColumn>()
			.Where()
			.None<FTypedElementActorTag>()
			.All<FTypedElementSelectionColumn>()
			.Compile());

		FQueryResult QueryResult = DataStorage->RunQuery(Query, ERunQueryFlags::Default,  
			[WorldIn, &VisitFn](EFlowControl& Flow, const FTypedElementWorldColumn& WorldColumn, const FVisibleInEditorColumn& VisibilityColumn)
				{
					if (WorldColumn.World.Get() != WorldIn)
					{
						return;
					}
					VisitFn(Flow, VisibilityColumn);
				});

		return QueryResult.Completed == FQueryResult::ECompletion::Fully;
	}

	bool HaveSelectedVisibleNonActors(UWorld* WorldIn)
	{
		using namespace UE::Editor::DataStorage;
		using namespace UE::Editor::DataStorage::Queries;

		bool bToReturn = false;
		bool bSuccess = ForEachSelectedHideableNonActor(WorldIn, [&bToReturn](EFlowControl& Flow, const FVisibleInEditorColumn& VisibilityColumn)
		{
			if (VisibilityColumn.bIsVisibleInEditor)
			{
				bToReturn = true;
				Flow = EFlowControl::Break;
			}
		});
		return bSuccess && bToReturn;
	}

	bool HaveSelectedHiddenNonActors(UWorld* WorldIn)
	{
		using namespace UE::Editor::DataStorage;
		using namespace UE::Editor::DataStorage::Queries;
	...
	bool HaveSelectedHideableNonActors(UWorld* WorldIn)
	{
		using namespace UE::Editor::DataStorage;
		using namespace UE::Editor::DataStorage::Queries;
...
void FLevelEditorActionCallbacks::NewLevel(bool& bOutLevelCreated)
{
	...
	else
	{
		...
		if (!TemplateFound)
		{
			UE_LOGF( LevelEditorActions, Warning, "Couldn't find template map package %ls", *TemplateMapPackageName);
...
void FLevelEditorActionCallbacks::PreviewJson_Clicked(FName PlatformName, FString JsonFile)
{
	if (JsonFile.IsEmpty())
	{
	...
	if (!JsonFile.IsEmpty())
	{
		...
		if (PlatformEngineIni && PlatformEngineIni->GetString(TEXT("DeviceProfileManager"), TEXT("PreviewDeviceProfileSelectionModule"), DeviceProfileSelectionModule, GEngineIni))
		{
			...
			{
				if(DPSelectorModule->IsJsonCompatible(JsonFile))
				{
					...
					EShaderPlatform PreviewShaderPlatform = DPSelectorModule->GetPreviewShaderPlatform();

					// Apply the PreviewPlatform returned by the DPSelectorModule.
					...
					{
						FName PreviewShaderPlatformName = FDataDrivenShaderPlatformInfo::GetName(PreviewShaderPlatform);

						FName FriendlyDeviceProfilePreviewName = FName(FDataDrivenShaderPlatformInfo::GetFriendlyName(PreviewShaderPlatform).ToString() + TEXT(" DeviceProfile: ") + DevileProfileName);
						...
						if (DevileProfile)
						{
							auto GetPreviewFeatureLevelInfo = [&]()
								{
									const ERHIFeatureLevel::Type FeatureLevel = GetMaxSupportedFeatureLevel(PreviewShaderPlatform);
									return FPreviewPlatformInfo(
										FeatureLevel,
										PreviewShaderPlatform, PlatformName,
										FDataDrivenShaderPlatformInfo::GetShaderFormat(PreviewShaderPlatform),
										DevileProfile->GetFName(),
										true,
										PreviewShaderPlatformName,
										FText::FromName(FriendlyDeviceProfilePreviewName),
						...
						{
							UE_LOGF(LevelEditorActions, Warning, "JsonFile %ls did not match any MatchProfile", *JsonFile);
						}
					}
					else
					{
						UE_LOGF(LevelEditorActions, Warning, "JsonFile %ls did not return a valid Preview Platform", *JsonFile);
					}
				}
				else
				{
					FMessageDialog::Open(EAppMsgType::Ok, LOCTEXT("PreviewJsonError_NoFiles", "The Json is not compatible with this Engine Version. Please see log for details."), LOCTEXT("PreviewJsonError_Title", "Preview Json"));
				}
			}
		}
	}
...
void FLevelEditorActionCallbacks::GeneratePreviewJson_Clicked(FString PlatformName)
{
	FString AbsoluteDebugInfoDirectory = IFileManager::Get().ConvertToAbsolutePathForExternalAppForWrite(*(FPaths::ProjectSavedDir() / TEXT("PreviewJsonDevices") / PlatformName));
	{
		...
		if (PlatformEngineIni && PlatformEngineIni->GetString(TEXT("DeviceProfileManager"), TEXT("PreviewDeviceProfileSelectionModule"), DeviceProfileSelectionModule, GEngineIni))
		{
			if (IDeviceProfileSelectorModule* DPSelectorModule = FModuleManager::LoadModulePtr<IDeviceProfileSelectorModule>(*DeviceProfileSelectionModule))
			{
				TArray<FString> OutExportedFiles;
				DPSelectorModule->ExportDeviceParametersToJson(AbsoluteDebugInfoDirectory, OutExportedFiles);
				
				if (!OutExportedFiles.IsEmpty())
				{
					FString FormatedString = FString::Join(OutExportedFiles, TEXT("\n"));
					FMessageDialog::Open(EAppMsgType::Ok, FText::Format(LOCTEXT("GeneratePreviewJsonDialog", "{0}"), FText::FromString(FormatedString)), LOCTEXT("GeneratePreviewJsonTitle", "Generated Preview Json Files"));
				}
				else
				{
					FMessageDialog::Open(EAppMsgType::Ok, LOCTEXT("GeneratePreviewJsonDialog_NoFiles", "No Files Generated"), LOCTEXT("GeneratePreviewJsonTitle", "Generated Preview Json Files"));
				}
			}
		}
	}
...
void FLevelEditorActionCallbacks::SetMaterialQualityLevel( EMaterialQualityLevel::Type NewQualityLevel )
{
	...
	EConsoleVariableFlags MaxPriority = GEditor && GEditor->IsPreviewPlatformActive() ? ECVF_SetByPreview: ECVF_SetByScalability;
...
void FLevelEditorActionCallbacks::TogglePreviewPlatform()
{
	GEditor->TogglePreviewPlatform();
}

bool FLevelEditorActionCallbacks::IsPreviewPlatformEnabled()
{
	if (GUnrealEd->IsLightingBuildCurrentlyRunning())
	{
		return false;
	}
	return GEditor->IsPreviewPlatformEnabled();
}

bool FLevelEditorActionCallbacks::IsPreviewPlatformActive()
{
	return GEditor->IsPreviewPlatformEnabled() && GEditor->IsPreviewPlatformActive();
}
...
{
	// TODO: Carl - Disable SM6 platforms for Metal SM6 until we fix issues.
	if (IsMetalSM6Platform(GMaxRHIShaderPlatform) && NewPreviewPlatform.PreviewFeatureLevel >= GMaxRHIFeatureLevel)
	{
		return false;
	}
...
void FLevelEditorActionCallbacks::BuildHLODs_SelectedActors_Execute()
{
	...
	if (WorldPartitionSubsystem)
	{
		...
		{
			FilterVolumes.Add(SelectedActorHandle->GetRuntimeBounds());
		}
	}

	if (FilterVolumes.Num() > 0)
	{
		UWorldPartitionHLODsBuilder::BuildWithFilters(GetWorld(), { MakeShared<FHLODCreationVolumeFilter>(MoveTemp(FilterVolumes)) });
	}
	else
	{
		UE_LOGF(LevelEditorActions, Warning, "Build HLODs - No actors selected");
	}
}
...
{
	// Gather the selected objects through the ITypedElementDetailsInterface so that any items that use
	//  stand-in actors are properly gathered instead of their actors.
	TArray<UObject*> SelectedObjects;
	USelection* Selection = GEditor->GetSelectedActors();
	UTypedElementSelectionSet* SelectionSet = Selection ? Selection->GetElementSelectionSet() : nullptr;
	if (ensure(SelectionSet))
	{
		SelectionSet->ForEachSelectedElement<ITypedElementDetailsInterface>([&SelectedObjects](const TTypedElement<ITypedElementDetailsInterface>& InDetailsElement)
		{
			if (TUniquePtr<ITypedElementDetailsObject> ElementDetailsObject = InDetailsElement.GetDetailsObject())
			{
				if (UObject* Object = ElementDetailsObject->GetObject())
				{
...
{
	auto SelectionIsNonActors = []()->bool
	{
		USelection* Selection = GEditor->GetSelectedActors();
		if (!Selection)
		{
		...
		return InternalEditorLevelLibrary::DoesDetailSelectionContainNonActorsOrComponents(Selection->GetElementSelectionSet());
	};

	return FModuleManager::LoadModuleChecked<FPropertyEditorModule>( "PropertyEditor" ).GetCanUsePropertyMatrix() 
		&& GEditor->GetSelectedActorCount() > 1
		&& (FPropertyEditorModule::CVarAllowPropertyMatrixForNonActors.GetValueOnAnyThread()
			|| !SelectionIsNonActors());
}

void FLevelEditorActionCallbacks::LockActorMovement_Clicked()
{
	GEditor->ToggleSelectedItemMovementLock();
}
...
bool FLevelEditorActionCallbacks::PlayFromHere_IsVisible()
{
	if (GEditor->GetSelectedActorCount() == 1)
	{
		if (const AActor* Actor = Cast<AActor>(*GEditor->GetSelectedActorIterator()))
		{
...
void FLevelEditorActionCallbacks::PlayFromHere()
{
	FPlayWorldCommandCallbacks::FStartPlayFromHereOptions PlayOptions;

	// Determine whether to spawn a new window or reuse existing viewport
	const ULevelEditorPlaySettings* PlaySettings = GetDefault<ULevelEditorPlaySettings>();
	const bool bSpawnNewWindow = PlaySettings->LastExecutedPlayModeType != EPlayModeType::PlayMode_InViewPort;
	if (!bSpawnNewWindow)
	{
		FLevelEditorModule& LevelEditorModule = FModuleManager::GetModuleChecked<FLevelEditorModule>("LevelEditor");
		PlayOptions.DestinationSlateViewport = LevelEditorModule.GetFirstActiveViewport();
	}
	...
	if (GEditor->GetSelectedActorCount() == 1)
	{
		if (const AActor* Actor = Cast<AActor>(*GEditor->GetSelectedActorIterator()))
		{
			if (Actor->CanPlayFromHere())
			{
				// Play from selected actor position
				PlayOptions.Mode = FPlayWorldCommandCallbacks::SelectedActor;
				if (FPlayWorldCommandCallbacks::StartPlayFromHere(PlayOptions))
				{
					return;
				}
			}
			...
			{
				UE_LOGF(LevelEditorActions, Log, "PlayFromHere: Selected actor to play from must set bCanPlayFromHere to true, falling back on last clicked location...");
			}
		}
	}

	// Fallback to click position
	PlayOptions.Mode = FPlayWorldCommandCallbacks::Click;
	FPlayWorldCommandCallbacks::StartPlayFromHere(PlayOptions);
}
...
{
	return GEditor->HasSelectedMovementLockedItems();
}
...
bool FLevelEditorActionCallbacks::Cut_CanExecute()
{
	...
	FTypedElementCommonActions_CopyPasteCapability CanCopy;
	{
		static const FName NAME_LevelEditor = "LevelEditor";
		if (TSharedPtr<ILevelEditor> LevelEditor = FModuleManager::GetModuleChecked<FLevelEditorModule>(NAME_LevelEditor).GetLevelEditorInstance().Pin())
		{
			const UTypedElementSelectionSet* SelectionSet = LevelEditor->GetElementSelectionSet();
			UTypedElementCommonActions* CommonActions = LevelEditor->GetCommonActions();
			
			if (CommonActions && SelectionSet)
			{
				CanCopy = CommonActions->CanCopySelectedElements(SelectionSet);
			}
		}
	}
	
	// If the level editor common action customizations are capable of copying the elements
	// then defer to it to allowing cut
	if (CanCopy.bCapable)
	{
		bCanCut = CanCopy.bAllowed;
	} 
...
bool FLevelEditorActionCallbacks::Copy_CanExecute()
{
	...
	// If the level editor common action customizations are capable of copying the elements
	// then defer to it to allowing copy
	if (CanCopy.bCapable)
	{
		bCanCopy = CanCopy.bAllowed;
	} 
	else if (GEditor->GetSelectedComponentCount() > 0)
	{
		TArray<UActorComponent*> SelectedComponents;
...
bool FLevelEditorActionCallbacks::Paste_CanExecute()
{
	...
	FTypedElementCommonActions_CopyPasteCapability CanPaste;
	{
		static const FName NAME_LevelEditor = "LevelEditor";
		if (TSharedPtr<ILevelEditor> LevelEditor = FModuleManager::GetModuleChecked<FLevelEditorModule>(NAME_LevelEditor).GetLevelEditorInstance().Pin())
		{
			...
			if (CommonActions && SelectionSet)
			{
				FTypedElementPasteOptions Options;
				Options.SelectionSetToModify = SelectionSet;
				CanPaste = CommonActions->CanPasteElements(Options);
			}
		}
	}
	
	bool bCanPaste = false;
	if (CanPaste.bCapable)
	{
		bCanPaste = CanPaste.bAllowed;
	}
...
bool FLevelEditorActionCallbacks::CanHideSelectedActors()
{
	...
	return LevelEditorActionsHelpers::HaveSelectedVisibleNonActors(GetWorld());
}
...
bool FLevelEditorActionCallbacks::CanShowSelectedActors()
{
	...
	return LevelEditorActionsHelpers::HaveSelectedHiddenNonActors(GetWorld());
}

bool FLevelEditorActionCallbacks::CanToggleSelectedActorsVisibility()
{
	return GEditor->GetSelectedActorCount() > 0
		|| LevelEditorActionsHelpers::HaveSelectedHideableNonActors(GetWorld());
}
				...
				for (auto & Comp : SimSkelComponents)
				{
					if (Comp->GetSkeletalMeshAsset() && Comp->GetSkeletalMeshAsset()->GetSkeleton() && Comp->IsSimulatingPhysics())
					{
						...
						if(Sequence)
						{
							...
							{
								UE_LOGF(LevelEditorActions, Warning, "Matching component could not be found %ls(%ls)", *GetNameSafe(Comp), *GetNameSafe(EditorActor));
							}
						}
					}
				}
...
bool FLevelEditorActionCallbacks::OnIsVertexSnapEnabled()
{
	...
	return ViewportSettings->bSnappingActive && ViewportSettings->bSnapVertices;
}
...
void FLevelEditorCommands::RegisterCommands()
{
	...
	UI_COMMAND( BuildHLODs_SelectedActors, "Build HLODs (Selected Actors)", "Builds HLODs that are affected by selected actors", EUserInterfaceActionType::Button, FInputChord() );
	...
	UI_COMMAND( PlayFromHere, "Play From Here", "Starts a game preview from the selected actor or clicked location as fallback", EUserInterfaceActionType::Button, FInputChord(EKeys::P, EModifierKey::Alt | EModifierKey::Shift) );
	...
	auto MakeAxisUICommand = [this](TSharedPtr<FUICommandInfo>& Command, const TCHAR* CommandName, EAxisList::Type Axis)
	{
		const FText AxisDisplayName = AxisDisplayInfo::GetAxisDisplayName(Axis);
		FUICommandInfo::MakeCommandInfo(
			AsShared(),
			Command,
			CommandName,
			FText::Format(LOCTEXT("MirrorAxisLabel", "Mirror {0} Axis"), AxisDisplayName),
			FText::Format(LOCTEXT("MirrorAxisTooltip", "Mirrors the element along the {0} axis"), AxisDisplayName),
			FSlateIcon(),
	...
	#define UI_COMMAND_AXIS(Command, Axis) MakeAxisUICommand(Command, TEXT(#Command), Axis);

	UI_COMMAND_AXIS(MirrorActorY, EAxisList::Left)
	UI_COMMAND_AXIS(MirrorActorZ, EAxisList::Up)
	UI_COMMAND_AXIS(MirrorActorX, EAxisList::Forward)

	#undef UI_COMMAND_AXIS

	UI_COMMAND( LockActorMovement, "Lock Item Movement", "Locks the item so it cannot be moved", EUserInterfaceActionType::ToggleButton, FInputChord() );
	...
	UI_COMMAND( SavePivotToPrePivot, "Set as Pivot Offset", "Sets the current pivot location as the pivot offset for this element", EUserInterfaceActionType::Button, FInputChord() );
	UI_COMMAND( ResetPrePivot, "Reset Pivot Offset", "Resets the pivot offset for this element", EUserInterfaceActionType::Button, FInputChord() );
	UI_COMMAND( ResetPivot, "Reset Pivot", "Resets the pivot", EUserInterfaceActionType::Button, FInputChord() );
	UI_COMMAND( MovePivotHere, "Set Pivot Offset Here", "Sets the pivot to the clicked location", EUserInterfaceActionType::Button, FInputChord() );
	UI_COMMAND( MovePivotHereSnapped, "Set Pivot Offset Here (Snapped)", "Sets the pivot to the nearest grid point to the clicked location", EUserInterfaceActionType::Button, FInputChord() );
...
#if PLATFORM_MAC
	UI_COMMAND( ShowAll, "Show All", "Shows all items", EUserInterfaceActionType::Button, FInputChord( EModifierKey::Command, EKeys::H ) );
#else
	UI_COMMAND( ShowAll, "Show All", "Shows all items", EUserInterfaceActionType::Button, FInputChord( EModifierKey::Control, EKeys::H ) );
#endif
	UI_COMMAND( ShowSelectedOnly, "Show Only Selected", "Shows only the selected items", EUserInterfaceActionType::Button, FInputChord() );
	UI_COMMAND( ShowSelected, "Show Selected", "Shows the selected items", EUserInterfaceActionType::Button, FInputChord() );
	UI_COMMAND( HideSelected, "Hide Selected", "Hides the selected items", EUserInterfaceActionType::Button, FInputChord() );
	UI_COMMAND( ToggleSelectedVisibility, "Toggle Selected Visibility", "Hides or shows the selected items", EUserInterfaceActionType::Button, FInputChord() );
	UI_COMMAND( ShowSelectedHierarchy, "Show Selected Hierarchy", "Shows the selected items and their children", EUserInterfaceActionType::Button, FInputChord( EModifierKey::Shift, EKeys::H ) );
	UI_COMMAND( HideSelectedHierarchy, "Hide Selected Hierarchy", "Hides the selected items and their children", EUserInterfaceActionType::Button, FInputChord() );
	UI_COMMAND( ToggleSelectedHierarchyVisibility, "Toggle Selected Hierarchy Visibility", "Hides or shows the selected items and their children", EUserInterfaceActionType::Button, FInputChord( EKeys::H ) );
	...
	UI_COMMAND(TogglePreviewPlatform, "Preview Mode Toggle", "Toggles the Preview Mode on or off for the currently selected Preview target", EUserInterfaceActionType::ToggleButton, FInputChord());
			...
			if (IDeviceProfileSelectorModule* DPSelectorModule = FModuleManager::LoadModulePtr<IDeviceProfileSelectorModule>(*DeviceProfileSelectionModule))
			{
				...
				if (!PlatformToPreviewJsonPlatformOverridesValue)
				{
					...
					PreviewJsonPlatform.CommandInfo = FUICommandInfoDecl(
						this->AsShared(),
						FName(*FString::Printf(TEXT("Preview %s via Json"), *Item.PlatformName.ToString())),
					...
					for (auto Iter = DirectoryToJsonFiles.CreateConstIterator(); Iter; ++Iter)
					{
						...
						for (const FString& JsonFile : Iter.Value())
						{
							...
							{
								FString FullFilePath = DirectoryName / JsonFile;
								if(DPSelectorModule->IsJsonCompatible(FullFilePath))
								{
									...
									PreviewJsonFilePlatform.CommandInfo = FUICommandInfoDecl(
										this->AsShared(),
										FName(*FString::Printf(TEXT("Preview %s with Json"), *JsonFile)),
