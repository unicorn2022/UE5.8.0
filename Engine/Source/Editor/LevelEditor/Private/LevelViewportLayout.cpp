	...
	if( !MaximizedViewport.IsNone() || bWasImmersive )
	{
		...
		if( bIsImmersive || ( bIsTransitioning && bWasImmersive ) )
		{
			...
			if( OwnerWindow.IsValid() )
			{
				...
				const float ScaleFactor = OwnerWindow->GetNativeWindow()->GetDPIScaleFactor() * FSlateApplication::Get().GetApplicationScale();
				...
				if( OwnerWindow->IsWindowMaximized() )
				{
					...
					const FMargin& WindowContentMargin = OwnerWindow->GetWindowBorderSize() * ScaleFactor;
