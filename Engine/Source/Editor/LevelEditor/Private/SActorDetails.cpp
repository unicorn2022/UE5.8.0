...
class SActorDetails::FInternalSubobjectCustomization : public ISCSEditorUICustomization
{
public:

	explicit FInternalSubobjectCustomization(TSharedPtr<FActorDetailsSCSEditorUICustomization>& ParentCustomization);
	virtual ~FInternalSubobjectCustomization() override = default;

	void SetHideAddComponentButton(bool bShouldHide);
	void SetHideBlueprintButton(bool bShouldHide);
	...
	virtual TSharedPtr<SWidget> GetControlsWidget(TSharedRef<SSubobjectEditor>& InSubobjectEditor, const FSubobjectData&) const override;
	virtual bool OverrideDesiredVisibility(const UE::Editor::FSubobjectEditorContext& Context, EVisibility& VisibilityOut) const override;
	...
	virtual TSharedRef<class SWidget> ConstructTreeViewOverlay(TWeakPtr<class SSubobjectEditor> Editor) const override;
...
private:
	TSharedPtr<FActorDetailsSCSEditorUICustomization> ParentCustomization;
	
	bool bHideAddComponentButton = true;
	bool bHideBlueprintButton = true;
};

bool SActorDetails::FInternalSubobjectCustomization::HideAddComponentButton(TArrayView<UObject*> Context) const
{
	if (const bool bParentHides = ParentCustomization && ParentCustomization->HideAddComponentButton(Context))
	{
		return bParentHides;
	}
	
	return bHideAddComponentButton;
}

bool SActorDetails::FInternalSubobjectCustomization::HideBlueprintButtons(TArrayView<UObject*> Context) const
{
	if (const bool bParentHides = ParentCustomization && ParentCustomization->HideBlueprintButtons(Context))
	{
		return bParentHides;
	}
	
	return bHideBlueprintButton;
}

bool SActorDetails::FInternalSubobjectCustomization::HideComponentsTree(TArrayView<UObject*> Context) const
{
	if (const bool bParentHides = ParentCustomization && ParentCustomization->HideComponentsTree(Context))
	{
		return bParentHides;
	}
...
bool SActorDetails::FInternalSubobjectCustomization::HideComponentsFilterBox(TArrayView<UObject*> Context) const
{
	if (const bool bParentHides = ParentCustomization && ParentCustomization->HideComponentsFilterBox(Context))
	{
		return bParentHides;
	}
...
const FSlateBrush* SActorDetails::FInternalSubobjectCustomization::GetIconBrush(const FSubobjectData& SubobjectData) const
{
	if (ParentCustomization)
	{
		return ParentCustomization->GetIconBrush(SubobjectData);
	}
	
	return ISCSEditorUICustomization::GetIconBrush(SubobjectData);
}

TSharedPtr<SWidget> SActorDetails::FInternalSubobjectCustomization::GetControlsWidget(TSharedRef<SSubobjectEditor>& InSubobjectEditor, const FSubobjectData& SubobjectData) const
{
	if (ParentCustomization)
	{
		return ParentCustomization->GetControlsWidget(InSubobjectEditor, SubobjectData);
	}

	return ISCSEditorUICustomization::GetControlsWidget(InSubobjectEditor, SubobjectData);
}

bool SActorDetails::FInternalSubobjectCustomization::OverrideDesiredVisibility(const UE::Editor::FSubobjectEditorContext& Context, EVisibility& VisibilityOut) const
{
	if (ParentCustomization)
	{
		if (ParentCustomization->OverrideDesiredVisibility(Context, VisibilityOut))
		{
			return true;
		}
	}
...
EChildActorComponentTreeViewVisualizationMode SActorDetails::FInternalSubobjectCustomization::GetChildActorVisualizationMode() const
{
	if (ParentCustomization)
	{
		return ParentCustomization->GetChildActorVisualizationMode();
	}

	return EChildActorComponentTreeViewVisualizationMode::UseDefault;
}

TSubclassOf<UActorComponent> SActorDetails::FInternalSubobjectCustomization::GetComponentTypeFilter(TArrayView<UObject*> Context) const
{
	if (ParentCustomization)
	{
		return ParentCustomization->GetComponentTypeFilter(Context);
	}

	return nullptr;
}

bool SActorDetails::FInternalSubobjectCustomization::FilterSubobjectData(TArray<FSubobjectDataHandle>& SubobjectData) const
{
	if (ParentCustomization)
	{
		return ParentCustomization->FilterSubobjectData(SubobjectData);
	}
	return false;
}

bool SActorDetails::FInternalSubobjectCustomization::SortSubobjectData(TArray<FSubobjectDataHandle>& SubobjectData)
{
	if (ParentCustomization)
	{
		return ParentCustomization->SortSubobjectData(SubobjectData);
	}
	return false;
}

bool SActorDetails::FInternalSubobjectCustomization::SupportsTreeViewOverlay(const SSubobjectEditor& Editor) const
{
	if (ParentCustomization)
	{
		return ParentCustomization->SupportsTreeViewOverlay(Editor);
	}
	return false;
}

TSharedRef<SWidget> SActorDetails::FInternalSubobjectCustomization::ConstructTreeViewOverlay(TWeakPtr<SSubobjectEditor> Editor) const
{
	if (ParentCustomization)
	{
		return ParentCustomization->ConstructTreeViewOverlay(Editor);
	}
	return SNullWidget::NullWidget;
}

void SActorDetails::FInternalSubobjectCustomization::PrePotentialTargetChange(SSubobjectEditor& InSubobjectEditor)
{
	if (ParentCustomization)
	{
		ParentCustomization->PrePotentialTargetChange(InSubobjectEditor);
	}
	ISCSEditorUICustomization::PrePotentialTargetChange(InSubobjectEditor);
}

bool SActorDetails::FInternalSubobjectCustomization::SelectDefaultSubobject(SSubobjectEditor& InSubobjectEditor)
{
	if (ParentCustomization)
	{
		return ParentCustomization->SelectDefaultSubobject(InSubobjectEditor);
	}
	return ISCSEditorUICustomization::SelectDefaultSubobject(InSubobjectEditor);
}

SActorDetails::FInternalSubobjectCustomization::FInternalSubobjectCustomization(
	TSharedPtr<FActorDetailsSCSEditorUICustomization>& InParentCustomization)
		: ParentCustomization(InParentCustomization)
{
}

void SActorDetails::FInternalSubobjectCustomization::SetHideAddComponentButton(bool bShouldHide)
{
	bHideAddComponentButton = bShouldHide;
}

void SActorDetails::FInternalSubobjectCustomization::SetHideBlueprintButton(bool bShouldHide)
{
	bHideBlueprintButton = bShouldHide;
}

namespace ActorDetails
{
	bool bCVarDisableComponentEditorButtonsAreaOnMultiSelect = false;
	FAutoConsoleVariableRef CVarLegacyAlwaysDisableComponentEditorButtonsAreaOnMultiSelect(
		TEXT("Editor.LevelEditor.LegacyAlwaysDisableComponentEditorButtonsAreaOnMultiSelect"),
		bCVarDisableComponentEditorButtonsAreaOnMultiSelect,
		TEXT("Disables all component editor buttons in a level editor details panel.  CVar used for backwards compatibility. Please update extension widgets to change visibility based on selected subobject nodes."),
		ECVF_Default);
}

void SActorDetails::Construct(
	...
	TWeakPtr<FActorDetailsSCSEditorUICustomization> InLevelEditorSubobjectEditorCustomization)
{
	SelectionSet = InSelectionSet;
	checkf(SelectionSet, TEXT("SActorDetails must be constructed with a valid selection set!"));

	FCoreUObjectDelegates::OnObjectsReinstanced.AddRaw(this, &SActorDetails::OnObjectsReinstanced);
	...
	LevelEditorSubobjectEditorCustomization = InLevelEditorSubobjectEditorCustomization.Pin();
	
	TSharedPtr<ISCSEditorUICustomization> SubobjectEditorCustomization = nullptr;
	if (ActorDetails::bCVarDisableComponentEditorButtonsAreaOnMultiSelect)
	{
		SubobjectEditorCustomization = LevelEditorSubobjectEditorCustomization;
	}
	else
	{
		InternalSubobjectEditorCustomization = MakeShared<FInternalSubobjectCustomization>(LevelEditorSubobjectEditorCustomization);
		SubobjectEditorCustomization = InternalSubobjectEditorCustomization;
	}
	...
	SubobjectEditor = SNew(SSubobjectInstanceEditor)
		...
		.SCSEditorUICustomization(SubobjectEditorCustomization);
	
	SubobjectEditor->SetSelectionSet(SelectionSet);
...
void SActorDetails::RefreshTopLevelElements(TArrayView<const TTypedElement<ITypedElementDetailsInterface>> InDetailsElements, const bool bForceRefresh, const bool bOverrideLock)
{
	...
	if (InternalSubobjectEditorCustomization)
	{
		const bool bHideButtons = TopLevelElements.Num() != 1;
		InternalSubobjectEditorCustomization->SetHideAddComponentButton(bHideButtons);
		InternalSubobjectEditorCustomization->SetHideBlueprintButton(bHideButtons);
	}
	...
	// Update the Subobject tree
	{
		...
		SubobjectEditor->PrePotentialTargetChange();
		// Make our subobject tree correspond to our selected top-level elements (e.g. rebuild from current actor)
		SubobjectEditor->UpdateTree();
		// Adjust the selection of subobjects based on editor selection (e.g. based on actor component selection)
			...
			if (SelectedNode)
			{
				if(const FSubobjectData* Data = SelectedNode->GetDataSource())
				{
					...
					if (Data->IsComponent())
					{
						...
						if (TedsRowAdapter)
						{
							RowHandle ComponentRowHandle = TedsRowAdapter->GetRow();
							
							if (const FSubobjectDataSubsystemRowReferenceCollection* Collection = CoreProvider->GetColumn<FSubobjectDataSubsystemRowReferenceCollection>(ComponentRowHandle))
							{
								for (UE::Editor::DataStorage::RowHandle Row : Collection->Rows)
								{
									if (const Compatibility::FTypedElementColumn* TypedElementColumn = CoreProvider->GetColumn<UE::Editor::DataStorage::Compatibility::FTypedElementColumn>(Row))
									{
										Components.Add(TypedElementColumn->Handle);
									}
								}
							}
...
void SActorDetails::OnSubobjectEditorTreeViewSelectionChanged(const TArray<FSubobjectEditorTreeNodePtrType>& SelectedNodes)
{
	...
	if (AActor* ActorContext = GetActorContext())
	{
		...
		if (!DetailsView->IsLocked())
		{
			...
			// Make sure we clear component visualizers if active. This ensures things like the viewport gizmos reflect the actual selection.
			if (GUnrealEd && GUnrealEd->ComponentVisManager.IsActive())
			{
	...
	else if (!TopLevelElements.IsEmpty())
	{
		RefreshSubobjectTreeElements(SelectedNodes, /*bForceRefresh*/false, DetailsView->IsLocked());
	}
}
...
void SActorDetails::UpdateComponentTreeFromEditorSelection()
{
	...
	if (SelectedNodes.Num() == 0)
	{
		SubobjectEditor->SelectDefaultSubobject();
...
void SActorDetails::OnNativeComponentWarningHyperlinkClicked(const FSlateHyperlinkRun::FMetadata& Metadata)
{
	...
	if (ParentClass)
	{
		...
		if (bFileFound)
		{
			...
			FSourceCodeNavigation::OpenSourceFileAsync(AbsoluteHeaderPath);
		}
	}
}
...
EVisibility SActorDetails::GetComponentEditorVisibility() const
{
	...
	if (bHideEditorFromDetailsView)
	{
		return EVisibility::Collapsed;
	}

	return SubobjectEditor.IsValid() ? SubobjectEditor->GetDesiredVisibility() : EVisibility::Collapsed;
}
...
void SActorDetails::OnObjectsReinstanced(const TMap<UObject*, UObject*>& InReplacementObjects)
{
...
FReply SActorDetails::OnKeyDown(const FGeometry& InGeometry, const FKeyEvent& InKeyEvent)
{
	if (DetailsView.IsValid())
    {
		// This leaves a chance to events coming from the whole Details tab that do not come directly from Detail view to be handled
		FReply Reply = DetailsView->OnKeyDown(InGeometry, InKeyEvent);
