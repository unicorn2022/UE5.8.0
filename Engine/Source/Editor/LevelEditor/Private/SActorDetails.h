...
public:
	...
	virtual ~SActorDetails() override;
...
private:
	/**
	 * A customization that the ActorDetails widget manages that maintains the visibility of the
	 * add component and convert to blueprint buttons.
	 */
	class FInternalSubobjectCustomization;
	...
	void OnObjectsReinstanced(const TMap<UObject*, UObject*>& InReinstancedObjects);
	...
	// Subobject Editor customization that hosts other customizations. Scoped to the level editor
	TSharedPtr<FActorDetailsSCSEditorUICustomization> LevelEditorSubobjectEditorCustomization;
	// Subobject Editor customization that is maintained as last priority in the LevelEditorCustomization
	// and is specific to the SActorDetails
	TSharedPtr<FInternalSubobjectCustomization> InternalSubobjectEditorCustomization;
