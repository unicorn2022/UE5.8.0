...
public:
	...
	virtual TMap< int32, TArray< TSharedPtr< SLevelViewport > > > GetViewportsByTab() const override;
	...
	/** Gets the default FSceneOutlinerInitializationOptions for this Level Editor's Scene Outliners */
	virtual void GetSceneOutlinerInitializationOptions(const FName TabIdentifier, FSceneOutlinerInitializationOptions& OutInitOptions) override;
...
private:
	...
	void RegisterViewportInteractions(UViewportInteractionsBehaviorSource* Source) const;
...
private:
	...
	/** Context row for the Editor Scope Tree system */
	UE::Editor::DataStorage::RowHandle LevelEditorScopeRow = UE::Editor::DataStorage::InvalidRowHandle;

	/** If this flag is raised we will force refresh on next selection update. */
	std::atomic<bool> bNeedsRefresh;
