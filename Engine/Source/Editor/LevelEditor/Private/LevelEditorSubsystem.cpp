...
bool ULevelEditorSubsystem::GetLevelViewportCameraInfo(FVector& CameraLocation, FRotator& CameraRotation, FName ViewportConfigKey)
{
	...
	if (LevelViewport.IsValid())
	{
		FLevelEditorViewportClient* LevelVC = &LevelViewport->GetLevelViewportClient();
		if (LevelVC)
		{
...
void ULevelEditorSubsystem::SetLevelViewportCameraInfo(FVector CameraLocation, FRotator CameraRotation, FName ViewportConfigKey)
{
	TSharedPtr<SLevelViewport> LevelViewport = InternalEditorLevelLibrary::GetLevelViewport(ViewportConfigKey);
	if (LevelViewport.IsValid())
	{
...
bool ULevelEditorSubsystem::GetLevelViewportFOV(float& FOV, FName ViewportConfigKey)
{
	FOV = 0.f;
	TSharedPtr<SLevelViewport> LevelViewport = InternalEditorLevelLibrary::GetLevelViewport(ViewportConfigKey);
	if (LevelViewport.IsValid())
...
void ULevelEditorSubsystem::SetLevelViewportFOV(float FOV, FName ViewportConfigKey)
{
	TSharedPtr<SLevelViewport> LevelViewport = InternalEditorLevelLibrary::GetLevelViewport(ViewportConfigKey);
	if (LevelViewport.IsValid())
	{
		...
		if (LevelVC)
		{
			LevelVC->ViewFOV = FMath::Clamp(FOV, 5.f, 170.f);
		}
	}
}
...
bool ULevelEditorSubsystem::NewLevel(const FString& AssetPath, bool bIsPartitionedWorld)
{
	...
	{
		UE_LOGF(LevelEditorSubsystem, Error, "NewLevel. Failed to create the level. %ls", *FailureReason);
		return false;
	}

	if (!EditorScriptingHelpers::IsAValidPathForCreateNewAsset(ObjectPath, FailureReason))
	{
		UE_LOGF(LevelEditorSubsystem, Error, "NewLevel. Failed to validate the destination. %ls", *FailureReason);
		return false;
	}

	if (FPackageName::DoesPackageExist(ObjectPath))
	{
		UE_LOGF(LevelEditorSubsystem, Error, "NewLevel. Failed to validate the destination '%ls'. There's alreay an asset at the destination.", *ObjectPath);
		return false;
	}
	...
	if (!World)
	{
		UE_LOGF(LevelEditorSubsystem, Error, "NewLevel. Failed to create the new level.");
		return false;
	}
	...
	{
		UE_LOGF(LevelEditorSubsystem, Warning, "NewLevel. Failed to save the new level.");
		return false;
	}
...
bool ULevelEditorSubsystem::NewLevelFromTemplate(const FString& AssetPath, const FString& TemplateAssetPath)
{
	...
	if (ObjectPath.IsEmpty())
	{
		UE_LOGF(LevelEditorSubsystem, Error, "NewLevelFromTemplate. Failed to create the level. %ls", *FailureReason);
		return false;
	}

	if (!EditorScriptingHelpers::IsAValidPathForCreateNewAsset(ObjectPath, FailureReason))
	{
		UE_LOGF(LevelEditorSubsystem, Error, "NewLevelFromTemplate. Failed to validate the destination. %ls", *FailureReason);
		return false;
	}
	...
	if (FPackageName::DoesPackageExist(ObjectPath))
	{
		UE_LOGF(LevelEditorSubsystem, Error, "NewLevelFromTemplate. Failed to validate the destination '%ls'. There's alreay an asset at the destination.", *ObjectPath);
		return false;
	}
	...
	{
		UE_LOGF(LevelEditorSubsystem, Error, "NewLevelFromTemplate. Failed to create the new level from template.");
		return false;
	}
	...
	if (!World)
	{
		UE_LOGF(LevelEditorSubsystem, Error, "NewLevelFromTemplate. Failed to find the new created world.");
		return false;
	}
	...
	if (!UEditorLoadingAndSavingUtils::SaveMap(World, DestinationLongPackagePath))
	{
		UE_LOGF(LevelEditorSubsystem, Error, "NewLevelFromTemplate. Failed to save the new level.");
		return false;
	}
...
bool ULevelEditorSubsystem::LoadLevel(const FString& AssetPath)
{
	...
	if (ObjectPath.IsEmpty())
	{
		UE_LOGF(LevelEditorSubsystem, Error, "LoadLevel. Failed to load level: %ls", *FailureReason);
		return false;
	}
...
bool ULevelEditorSubsystem::SaveCurrentLevel()
{
	...
	if (!World)
	{
		UE_LOGF(LevelEditorSubsystem, Error, "SaveCurrentLevel. Can't save the current level because there is no world.");
		return false;
	}
	...
	if (!Level)
	{
		UE_LOGF(LevelEditorSubsystem, Error, "SaveCurrentLevel. Can't save the level because there is no current level.");
		return false;
	}
	...
	if (Filename.Len() == 0)
	{
		UE_LOGF(LevelEditorSubsystem, Error, "SaveCurrentLevel. Can't save the level because it doesn't have a filename. Use EditorLoadingAndSavingUtils.");
		return false;
	}
...
bool ULevelEditorSubsystem::SaveAllDirtyLevels()
{
	...
	if (!World)
	{
		UE_LOGF(LevelEditorSubsystem, Error, "SaveAllDirtyLevels. Can't save the current level because there is no world.");
		return false;
	}
	...
	for (ULevel* Level : World->GetLevels())
	{
		if (Level)
		{
			...
			if (OutermostPackage->IsDirty())
			{
				...
				if (Filename.Len() == 0)
				{
					UE_LOGF(LevelEditorSubsystem, Warning, "SaveAllDirtyLevels. Can't save the level '%ls' because it doesn't have a filename. Use EditorLoadingAndSavingUtils.", *OutermostPackage->GetName());
					continue;
				}
	...
	if (DirtyMapPackages.Num() > 0)
	{
		...
		for (ULevel* Level : DirtyLevels)
		{
			...
			if (!bSaved)
			{
				UE_LOGF(LevelEditorSubsystem, Warning, "SaveAllDirtyLevels. Can't save the level '%ls'.", *World->GetOutermost()->GetName());
				bAllSaved = false;
			}
		}
	}
	...
	{
		UE_LOGF(LevelEditorSubsystem, Log, "SaveAllDirtyLevels. There is no dirty level.");
	}
...
bool ULevelEditorSubsystem::SetCurrentLevelByName(FName LevelName)
{
	...
	{
		UE_LOGF(LevelEditorSubsystem, Error, "SetCurrentLevel. LevelName is invalid.");
		return false;
	}
	...
	if (!World)
	{
		UE_LOGF(LevelEditorSubsystem, Warning, "SetCurrentLevel. Can't set the current level because there is no world.");
		return false;
	}
...
bool ULevelEditorSubsystem::DeleteActorFolder(FName FolderPath)
{
	TGuardValue<bool> UnattendedScriptGuard(GIsRunningUnattendedScript, true);
	...
	if (!World)
	{
		UE_LOGF(LevelEditorSubsystem, Error, "DeleteActorFolder. Can't delete folder because there is no world.");
		return false;
	}

	FActorFolders& ActorFolders = FActorFolders::Get();
	FFolder Folder(FFolder::GetWorldRootFolder(World).GetRootObject(), FolderPath);
	if (!ActorFolders.ContainsFolder(*World, Folder))
	{
		return false;
	}
...
ULevel* ULevelEditorSubsystem::GetCurrentLevel()
{
	...
	if (!World)
	{
		UE_LOGF(LevelEditorSubsystem, Error, "GetCurrentLevel. Can't Get the current level because there is no world.");
		return nullptr;
	}
...
bool ULevelEditorSubsystem::BuildLightMaps(ELightingBuildQuality Quality, bool bWithReflectionCaptures)
{
	...
	if (!World)
	{
		UE_LOGF(LevelEditorSubsystem, Error, "BuildLightMaps. Can't build the light maps of the current level because there is no world.");
		return false;
	}
	...
	auto BuildFailedDelegate = [&World, &Success]() {
		UE_LOGF(LevelEditorSubsystem, Error, "BuildLightMaps. Failed building lighting for %ls", *World->GetOutermost()->GetName());
		Success = false;
	};
	...
	auto BuildSucceededDelegate = [&World, &Success]() {
		UE_LOGF(LevelEditorSubsystem, Log, "BuildLightMaps. Successfully built lighting for %ls", *World->GetOutermost()->GetName());
		Success = true;
	};
	FDelegateHandle BuildSucceededDelegateHandle = FEditorDelegates::OnLightingBuildSucceeded.AddLambda(BuildSucceededDelegate);

	UE_LOGF(LevelEditorSubsystem, Log, "BuildLightMaps. Start building lighting for %ls", *World->GetOutermost()->GetName());
