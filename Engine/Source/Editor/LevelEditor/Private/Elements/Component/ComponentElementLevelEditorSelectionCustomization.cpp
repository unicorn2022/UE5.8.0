...
{
	UActorComponent* Component = ComponentElementDataUtil::GetComponentFromHandle(InElementSelectionHandle);
	if (!Component)
	{
		UE_LOGF(LogComponentLevelEditorSelection, Warning, "GetNormalizedElements: Failed to resolve component from element handle");
		return;
	}
...
{
	const UActorComponent* Component = ComponentElementDataUtil::GetComponentFromHandle(InComponentSelectionHandle);
	if (!Component)
	{
		UE_LOGF(LogComponentLevelEditorSelection, Warning, "CanSelectComponentElement: Failed to resolve component from element handle");
		return false;
	}
...
bool FComponentElementLevelEditorSelectionCustomization::CanDeselectComponentElement(const TTypedElement<ITypedElementSelectionInterface>& InComponentSelectionHandle, const FTypedElementSelectionOptions& InSelectionOptions) const
{
	...
	if (!Component)
	{
		UE_LOGF(LogComponentLevelEditorSelection, Warning, "CanDeselectComponentElement: Failed to resolve component from element handle");
		return true;
	}
...
{
	UActorComponent* Component = ComponentElementDataUtil::GetComponentFromHandle(InComponentSelectionHandle);
	if (!Component)
	{
		UE_LOGF(LogComponentLevelEditorSelection, Warning, "SelectComponentElement: Failed to resolve component from element handle");
		return false;
	}
	...
	UE_LOGF(LogComponentLevelEditorSelection, Verbose, "Selected Component: %ls (%ls)", *Component->GetPathName(), *Component->GetClass()->GetName());
...
bool FComponentElementLevelEditorSelectionCustomization::DeselectComponentElement(const TTypedElement<ITypedElementSelectionInterface>& InComponentSelectionHandle, FTypedElementListRef InSelectionSet, const FTypedElementSelectionOptions& InSelectionOptions)
{
	...
	if (!Component)
	{
		UE_LOGF(LogComponentLevelEditorSelection, Warning, "DeselectComponentElement: Failed to resolve component from element handle");
		return false;
	}
	...
	UE_LOGF(LogComponentLevelEditorSelection, Verbose, "Deselected Component: %ls (%ls)", *Component->GetPathName(), *Component->GetClass()->GetName());
