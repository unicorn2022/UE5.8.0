...
{
	AActor* Actor = ActorElementDataUtil::GetActorFromHandle(InElementSelectionHandle);
	if (!Actor)
	{
		UE_LOGF(LogActorLevelEditorSelection, Warning, "GetNormalizedElements: Failed to resolve actor from element handle");
		return;
	}
...
bool IsActorReachable(const AActor* Actor)
{
	...
	{
		UE_LOGF(LogActorLevelEditorSelection, Warning, "SelectActor: %ls (%ls) Flags: (0x%08X)",
...
{
	AActor* Actor = ActorElementDataUtil::GetActorFromHandle(InActorSelectionHandle);
	if (!Actor)
	{
		UE_LOGF(LogActorLevelEditorSelection, Warning, "CanSelectActorElement: Failed to resolve actor from element handle");
		return false;
	}
	...
	{
		UE_CLOGF(InSelectionOptions.WarnIfLocked(), LogActorLevelEditorSelection, Warning, "SelectActor: %ls (%ls)", TEXT("The requested operation could not be completed because the level is locked."), *Actor->GetActorLabel());
		return false;
	}
...
bool FActorElementLevelEditorSelectionCustomization::CanDeselectActorElement(const TTypedElement<ITypedElementSelectionInterface>& InActorSelectionHandle, const FTypedElementSelectionOptions& InSelectionOptions) const
{
	...
	if (!Actor)
	{
		UE_LOGF(LogActorLevelEditorSelection, Warning, "CanDeselectActorElement: Failed to resolve actor from element handle");
		return true;
	}
...
bool FActorElementLevelEditorSelectionCustomization::SelectActorElement(const TTypedElement<ITypedElementSelectionInterface>& InActorSelectionHandle, FTypedElementListRef InSelectionSet, const FTypedElementSelectionOptions& InSelectionOptions)
{
	...
	if (!Actor)
	{
		UE_LOGF(LogActorLevelEditorSelection, Warning, "SelectActorElement: Failed to resolve actor from element handle");
		return false;
	}
	...
	UE_LOGF(LogActorLevelEditorSelection, Verbose, "Selected Actor: %ls (%ls)", *Actor->GetActorLabel(), *Actor->GetClass()->GetName());
...
bool FActorElementLevelEditorSelectionCustomization::DeselectActorElement(const TTypedElement<ITypedElementSelectionInterface>& InActorSelectionHandle, FTypedElementListRef InSelectionSet, const FTypedElementSelectionOptions& InSelectionOptions)
{
	...
	if (!Actor)
	{
		UE_LOGF(LogActorLevelEditorSelection, Warning, "DeselectActorElement: Failed to resolve actor from element handle");
		return false;
	}
	...
	UE_LOGF(LogActorLevelEditorSelection, Verbose, "Deselected Actor: %ls (%ls)", *Actor->GetActorLabel(), *Actor->GetClass()->GetName());
