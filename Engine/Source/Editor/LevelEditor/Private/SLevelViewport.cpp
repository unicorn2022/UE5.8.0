...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(SLevelViewport::Construct);
	...
	ViewportInstanceSettings.OrthoViewModeIndex = VMI_Unlit;
...
void SLevelViewport::ConstructViewportOverlayContent()
{
	...
	// Editor debug canvas (swapped for SGameLayerManager during PIE).
	SAssignNew(EditorDebugCanvas, SDebugCanvas).SceneViewport(ActiveViewport.Get());
	ViewportOverlay->AddSlot(SlotIndex++)
	[
		EditorDebugCanvas.ToSharedRef()
	];
...
bool SLevelViewport::IsActorEditorContextVisible() const
{
	return GetDefault<ULevelEditorViewportSettings>()->bShowActorEditorContext &&
		...
		(ActiveViewport->GetPlayInEditorIsSimulate() || (ActiveViewport->GetClient()->GetWorld() && !ActiveViewport->GetClient()->GetWorld()->IsGameWorld())) &&
...
void SLevelViewport::ConstructLevelEditorViewportClient(FLevelEditorViewportInstanceSettings& ViewportInstanceSettings)
{
	...
	LevelViewportClient->CurrentMegaLightsVisualizationMode = ViewportInstanceSettings.MegaLightsVisualizationMode;
	...
	LevelViewportClient->ChannelMaskParams.ColorChannelMask = (EColorChannelMask)ViewportInstanceSettings.ColorChannelMask;
	LevelViewportClient->ChannelMaskParams.bInvertAlphaChannelMask = ViewportInstanceSettings.bInvertAlphaChannelMask;
	LevelViewportClient->ChannelMaskParams.bDrawAlphaBlendedCheckerboard = ViewportInstanceSettings.bDrawAlphaBlendedCheckerboard;
...
void SLevelViewport::OnToggleEditorShortcutsInPIE()
{
	ULevelEditorPlaySettings* const PlaySettings = GetMutableDefault<ULevelEditorPlaySettings>();
	PlaySettings->EnableEditorShortcutsInPIE = !PlaySettings->EnableEditorShortcutsInPIE;
}

bool SLevelViewport::IsEditorShortcutsEnabledInPIE()
{
	return GetDefault<ULevelEditorPlaySettings>()->EnableEditorShortcutsInPIE;
}
...
FReply SLevelViewport::OnKeyDown( const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent )
{
	...
	bool bEnableEditorShortcuts = true;
	if (ULevelEditorPlaySettings* const PlaySettings = GetMutableDefault<ULevelEditorPlaySettings>())
	{
		bEnableEditorShortcuts = PlaySettings->EnableEditorShortcutsInPIE;
	}
	...
	if( HasPlayInEditorViewport() || LevelViewportClient->IsSimulateInEditorViewport() )
	{
		...
		if (bEnableEditorShortcuts && HasPlayInEditorViewport())
		{
...
void SLevelViewport::OnDragEnter( const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent )
{
	...
	if ( !bDragEnterReentranceGuard )
	{
		...
		if ( CurrentLevel && !FLevelUtils::IsLevelLocked(CurrentLevel) )
		{
			...
			{
				// The operation can be null if HandlePlaceDraggedObjects opens a modal dialog,
				// causing a window focus change that triggers OnDragLeave and resets the drag content.
...
bool SLevelViewport::HandleDragObjects(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent)
{
	...
	if ( bValidDrag )
	{
		...
		const FVector2D ToViewportPixelScale = UE::Slate::Viewport::GetViewportLocalToPixelScale(MyGeometry, ViewportSize);
		CachedOnDropLocalMousePos = FVector2f(MyGeometry.AbsoluteToLocal( DragDropEvent.GetScreenSpacePosition() ) * ToViewportPixelScale).IntPoint();
...
{
	// The drag operation can be null if a modal dialog shown during OnDragOver causes a window focus change, 
	// which triggers OnDragLeave and resets the drag content before OnDrop is called.
	TSharedPtr<FDragDropOperation> Operation = DragDropEvent.GetOperation();
	if (!Operation.IsValid())
	{
		return FReply::Unhandled();
	...
	if (Operation->IsOfType<FInViewportUIDragOperation>())
	{
...
{
	// If the piloted actor is being deleted, stop piloting before it becomes invalid.
	// OnLevelActorDeleted fires while the actor is still fully alive, so SetActorLock
	// can safely restore the transform here.
	if (InActor && LevelViewportClient->IsActorLocked(InActor))
	{
		OnActorUnlock();
	}
...
void SLevelViewport::BindOptionCommands( FUICommandList& OutCommandList )
{
	...
	OutCommandList.MapAction(
		...
		FIsActionButtonVisible::CreateLambda([]() { return GEditor->IsPreviewPlatformActive() && (GEditor->PreviewPlatform.GetConstrainedAspectRatio() != 0.0f); })
	);

	OutCommandList.MapAction(
		ViewportActions.ToggleFlipConstrainedAspectRatioInPreview,
		FExecuteAction::CreateLambda([]() { GEditor->ToggleFlipConstrainedAspectRatioInPreview(); }),
		FCanExecuteAction::CreateLambda([]() { return GEditor->GetAllowConstrainedAspectRatioInPreview(); }),
		FIsActionChecked::CreateLambda([]() { return GEditor->GetFlipConstrainedAspectRatioInPreview(); }),
		FIsActionButtonVisible::CreateLambda([]() { return GEditor->IsPreviewPlatformActive() && (GEditor->PreviewPlatform.GetConstrainedAspectRatio() != 0.0f); })
	);
	...
	OutCommandList.MapAction(
		ViewportActions.EnableEditorShortcutsInPIE,
		FExecuteAction::CreateSP( this, &SLevelViewport::OnToggleEditorShortcutsInPIE ),
		FCanExecuteAction(),
		FIsActionChecked::CreateSP( this, &SLevelViewport::IsEditorShortcutsEnabledInPIE )
		);
}
...
void SLevelViewport::BindViewCommands( FUICommandList& OutCommandList )
{
	...
	{
		FRayTracingDebugVisualizationMenuCommands::Get().BindCommands(OutCommandList, Client);
	}
	...
	FMegaLightsVisualizationMenuCommands::Get().BindCommands(OutCommandList, Client);
...
void SLevelViewport::RefreshPIEViewport()
{
	if (UGameViewportClient* GameClient = GetPlayClient())
	{
		if (UWorld* World = GameClient->GetWorld())
		{
...
FName SLevelViewport::RegisterViewportToolbar() const
{
	...
	if (!UToolMenus::Get()->IsMenuRegistered(ViewportToolbarName))
	{
		...
		// Add the right-aligned part of the viewport toolbar.
		{
			...
			{
				RightSection.AddEntry(UE::LevelEditor::CreateDynamicChannelsSubmenu());
			}
...
{
	static const TSet<FTabId> Exceptions = { FTabId(TEXT("Sequencer")) };
	ParentLevelEditor.Pin()->GetTabManager()->ToggleSidebarOpenTabs(Exceptions);
}
...
void SLevelViewport::SaveConfig(const FString& ConfigName) const
{
	if(GUnrealEd && GetDefault<ULevelEditorViewportSettings>())
	{
		...
		ViewportInstanceSettings.MegaLightsVisualizationMode = LevelViewportClient->CurrentMegaLightsVisualizationMode;
		...
		ViewportInstanceSettings.ColorChannelMask = (int32)LevelViewportClient->ChannelMaskParams.ColorChannelMask;
		ViewportInstanceSettings.bInvertAlphaChannelMask = LevelViewportClient->ChannelMaskParams.bInvertAlphaChannelMask;
		ViewportInstanceSettings.bDrawAlphaBlendedCheckerboard = LevelViewportClient->ChannelMaskParams.bDrawAlphaBlendedCheckerboard;
...
FLevelEditorViewportInstanceSettings SLevelViewport::LoadLegacyConfigFromIni(const FString& InConfigKey, const FLevelEditorViewportInstanceSettings& InDefaultSettings)
{
	...
	{
		// Default to Unlit for an orthographic viewport
		ViewportInstanceSettings.OrthoViewModeIndex = (ViewportInstanceSettings.ViewportType != LVT_Perspective && LegacyViewModeIndex != VMI_Unknown) ? LegacyViewModeIndex : VMI_Unlit;
	}
	...
	if( NewActorsToPreview.Num() > 0 )
	{
		for (AActor* CurActor : NewActorsToPreview)
		{
			...
			if (bNeedsLevelViewport)
			{
				...
				ensure(ActorPreviewViewportWidget.IsValid());
				ActorPreviewSceneViewport = FSceneViewport::Create(ActorPreviewLevelViewportClient, ActorPreviewViewportWidget);
			}
...
void SLevelViewport::StartPlayInEditorSession(UGameViewportClient* InPlayClient, const bool bInSimulateInEditor)
{
	...
	// Unregister the editor debug canvas from slot 0 before AddAssociation inserts SGameLayerManager there.
	EditorDebugCanvas->SetSceneViewport(nullptr);
	ViewportOverlay->RemoveSlot(0);
	...
	// register for preview platform change
	UEditorEngine* Editor = CastChecked<UEditorEngine>(GEngine);

	PIEPreviewFeatureLevelChangedHandle = Editor->OnPreviewShaderPlatformChanged().AddLambda([InPlayClient](EShaderPlatform NewShaderPlatform)
		{
			InPlayClient->GetWorld()->ShaderPlatformChanged(NewShaderPlatform, true);
		});
...
void SLevelViewport::EndPlayInEditorSession()
{
	...
	if (ViewportOverlay->HasSlotWithZOrder(0))
	{
		ViewportOverlay->RemoveSlot(0);
	}

	// Restore the editor debug canvas now that ActiveViewport is back to the level editor viewport.
	EditorDebugCanvas->SetSceneViewport(ActiveViewport.Get());
	...
	{
		CastChecked<UEditorEngine>(GEngine)->OnPreviewShaderPlatformChanged().Remove(PIEPreviewFeatureLevelChangedHandle);
...
void SLevelViewport::SwapViewportsForSimulateInEditor()
{
	...
	if (TSharedPtr<IGameLayerManager> GameLayerManager = PlayClient.IsValid() ? PlayClient->GetGameLayerManager() : nullptr)
	{
