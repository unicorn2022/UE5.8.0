...
#include "ToolMenuMisc.h" // FToolMenuInsert
...
public:
	...
	/**
	 * Registers a menu with name MenuNameToUse that is derived from BaseMenuName, but has
	 *  the level editor context menu items added at InsertLocation.
	 * This is used to add the level editor context menu to a scene outliner context menu.
	 */
	UE_EXPERIMENTAL(5.8, "This helper might be removed when a single outliner implementation is used.")
	static UE_API void RegisterAsDerivedMenu(FName MenuNameToUse, FName BaseMenuName,
		const FToolMenuInsert& InsertLocation = FToolMenuInsert(NAME_None, EToolMenuInsertType::Last));
