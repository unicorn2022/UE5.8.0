...
public:
	 ...
	 * @param	ViewportConfigKey the viewport you wish to get from
	 * @return	Whether we were able to get the camera position for a level editing viewport
	 */
	UFUNCTION(BlueprintPure, Category = "Editor Scripting | Level Utility", meta = (DevelopmentOnly))
	UE_API bool GetLevelViewportCameraInfo(FVector& CameraLocation, FRotator& CameraRotation, FName ViewportConfigKey);

	
	/**
	* Sets information about the camera position for the given level editor viewport.
	*
	* @param	CameraLocation	Location the camera will be moved to
	* @param	CameraRotation	Rotation the camera will be set to
	* @param	ViewportConfigKey	the viewport you wish to target
	*/
	UFUNCTION(BlueprintCallable, Category = "Editor Scripting | Level Utility", meta = (DevelopmentOnly))
	UE_API void SetLevelViewportCameraInfo(FVector CameraLocation, FRotator CameraRotation, FName ViewportConfigKey);
	
	/**
	 * Gets the horizontal field of view for the given level editor viewport. 
	 *
	 * @param	FOV				Field of View of the Level editing viewport camera
	 * @param	ViewportConfigKey	the viewport you wish to get from
	 * @return	Whether we were able to get the FOV for a level editing viewport
	 */
	UFUNCTION(BlueprintPure, Category = "Editor Scripting | Level Utility", meta=(DisplayName="Get Level Viewport Field Of View", DevelopmentOnly))
	UE_API bool GetLevelViewportFOV(float& FOV, FName ViewportConfigKey);

	/**
	* Sets the horizontal field of view for the given level editor viewport.
	*
	* @param	FOV					Field of View to set the camera to. Value will be clamped between 5 and 170 deg
	* @param	ViewportConfigKey	the viewport you wish to target
	*/
	UFUNCTION(BlueprintCallable, Category = "Editor Scripting | Level Utility", meta=(DisplayName="Set Level Viewport Field Of View", DevelopmentOnly))
	UE_API  void SetLevelViewportFOV(float FOV, FName ViewportConfigKey);
	...
	*/
	 ...
	 * Does not move or delete actors; only removes the folder entry itself.
	 * @param	FolderPath	The path of the folder to delete (e.g. 'Lighting/Spotlights').
	 * @return	True if the folder existed and was deleted.
	 */
	UFUNCTION(BlueprintCallable, Category = "Editor Scripting | Level Utility")
	UE_API bool DeleteActorFolder(FName FolderPath);
