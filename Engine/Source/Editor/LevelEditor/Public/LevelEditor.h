...
public:
	...
	*/
	...
	/** Delegate used to capture skeletal meshes to single-frame animations when 'keeping simulation changes' */
	...
	/** Delegate used to determine if users should be able to enable the alpha-blended checkerboard for the level viewport */
	DECLARE_DELEGATE_RetVal(bool, FGetVisualizeCheckerboardEnabled);
	virtual FGetVisualizeCheckerboardEnabled& OnGetVisualizeCheckerboardEnabled() { return GetVisualizeCheckerboardEnabled; }
...
public:
	...
	/** Refresh all of the level editor's outliners (close and recreate them) */
	virtual void RefreshLevelEditorOutliners() const;
...
private:
	...
	/** Extend the dock tab context menu */
	void OnExtendTabContextMenu(FMenuBuilder& InMenuBuilder);

	/** Is the active level an asset that can be browsed to */
	bool OnCanBrowseLevelInContentBrowser();

	/** Browse to active level in content browser */
	void OnBrowseLevelInContentBrowser();
...
private:
	...
	/** Delegate used to determine if 'Visualize Alpha with Checkerboard' checkbox is enabled in the level viewports  */
	FGetVisualizeCheckerboardEnabled GetVisualizeCheckerboardEnabled;
