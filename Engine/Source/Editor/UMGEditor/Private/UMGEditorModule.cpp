...
DEFINE_LOG_CATEGORY(LogUMGEditor);
...
public:
	...
	virtual void StartupModule() override
	{
		...
		FCoreDelegates::GetOnPostEngineInit().AddRaw(this, &FUMGEditorModule::OnPostEngineInit);
		...
		if (UEditorEngine* EditorEngine = Cast<UEditorEngine>(GEngine))
		{
			if (TOptional<FRequestPlaySessionParams> PlayRequest = EditorEngine->GetPlaySessionRequest())
			{
				...
				if (NumClients > 1)
				{
					...
					{
						UE_LOGF(LogUMGEditor, Log, "Disabling Slate Post Buffers for multi-window PIE session, currently not supported.");
	...
	void OnEndPIE(const bool bIsSimulating)
	{
		if (bCachedArePostBuffersEnabled)
		{
			if (IConsoleVariable* PostBuffersEnabled = IConsoleManager::Get().FindConsoleVariable(TEXT("Slate.CopyBackbufferToSlatePostRenderTargets")))
			{
				...
				{
					UE_LOGF(LogUMGEditor, Warning, "Restoring Slate Post Buffers, previously disabled due to multi-window PIE session.");
