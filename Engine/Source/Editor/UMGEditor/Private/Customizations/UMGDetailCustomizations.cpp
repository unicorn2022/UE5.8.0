...
bool FBlueprintWidgetCustomization::IsBindableEvent(const FProperty* Property)
{
...
TSharedRef<SWidget> FBlueprintWidgetCustomization::MakePropertyBindingWidget(TWeakPtr<FWidgetBlueprintEditor> InEditor, UFunction* SignatureFunction, TSharedRef<IPropertyHandle> InPropertyHandle, TWeakPtr<IDetailsView> InDetailsView, bool bInGeneratePureBindings, bool bAllowDetailsPanelLegacyBinding, EPropertyBindingMode Mode)
{
	...
	const bool bIsBindableEvent = IsBindableEvent(InPropertyHandle->GetProperty());
	UUMGEditorProjectSettings* UMGEditorProjectSettings = GetMutableDefault<UUMGEditorProjectSettings>();
	const bool bCanUseNewBinding = UMGEditorProjectSettings->bUseNewBindingFromDetailsView && !bIsBindableEvent;
	...
	Args.OnCanUseCustomGenerateMenu = FOnHasAnyBindings::CreateLambda([InEditor, InPropertyHandle, Mode, bCanUseNewBinding]()
		{
			UUMGEditorProjectSettings* UMGEditorProjectSettings = GetMutableDefault<UUMGEditorProjectSettings>();
			return bCanUseNewBinding &&
	...
	Args.bAllowNewBindings = bAllowDetailsPanelLegacyBinding || bIsBindableEvent;
	Args.bUseLinkIconStyle = true;
	Args.bAllowFunctionBindings = !bCanUseNewBinding;
	Args.bAllowPropertyBindings = !bCanUseNewBinding;

	if (Mode == EPropertyBindingMode::Extension && !bIsBindableEvent)
	{
	...
	else
	{
		...
		Args.bCanDisplayLinkIcon = bIsBindableEvent;
	}
...
{
	if (UWidgetBlueprint* BlueprintObj = Blueprint.Get())
	{
		FText ErrorMessage;
		FWidgetBlueprintOperationUtils::BindToEventProperty(BlueprintObj, EventName, PropertyName, PropertyClass.Get(), true, ErrorMessage);
	}
