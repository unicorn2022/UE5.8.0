...
private:
	...
	FReply OnNewAnimationClicked()
	{
		...
		const UUMGEditorProjectSettings* UMGEditorSettings = GetDefault<UUMGEditorProjectSettings>();
		NewAnimation->MovieScene->SetDisplayRate(FFrameRate(UMGEditorSettings->DefaultWidgetAnimationFrameRate, 1));
	...
	void OnDuplicateAnimation()
	{
		...
		NewAnimation->MovieScene->Rename(*NewAnimation->GetName(), nullptr, REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
