...
void UWidgetBlueprint::PreSave(FObjectPreSaveContext ObjectSaveContext)
{
	...
	if (UWidgetBlueprintGeneratedClass* BPGC = Cast< UWidgetBlueprintGeneratedClass>(GeneratedClass))
	{
		FWidgetBlueprintCompilerContext::ClearGeneratedClassWidgetTree(BPGC, false);
		FWidgetBlueprintCompilerContext::UpdateGeneratedClassWidgetTree(this, BPGC, BPGC->GetWidgetTreeArchetype(), nullptr);
	}
}
...
EDataValidationResult UWidgetBlueprint::IsDataValid(FDataValidationContext& Context) const
{
	...
	if (IsInGameThread())
	{
		if (DetectSlateWidgetLeaks(Context))
		{
	...
	if (const UUIComponentWidgetBlueprintExtension* UIExtension =
		UWidgetBlueprintExtension::GetExtension<UUIComponentWidgetBlueprintExtension>(this))
	{
		Result = CombineDataValidationResults(Result, UIExtension->ValidateComponents(Context));
	}
...
{
	bool bDummyFlag = false;
	UpdateTickabilityStatsAndInputRegistration(OutHasLatentActions, OutHasAnimations, OutClassRequiresNativeTick, bDummyFlag);
}
PRAGMA_ENABLE_DEPRECATION_WARNINGS

void UWidgetBlueprint::UpdateTickabilityStatsAndInputRegistration(bool& OutHasLatentActions, bool& OutHasAnimations, bool& OutClassRequiresNativeTick, bool& OutClassRequiresInputReg)
{
	if (GeneratedClass && GeneratedClass->ClassConstructor)
	{
		...
		bool bParentsRequireInputReg = false;
		...
		for (UBlueprint* Blueprint : BlueprintParents)
		{
			...
			if (WidgetBP)
			{
				// Check this blueprint parent for if it requires input registration
				UWidgetBlueprintGeneratedClass* GenClass = CastChecked<UWidgetBlueprintGeneratedClass>(WidgetBP->GeneratedClass);
				if (const UUserWidget* WidgetBPCDO = GenClass->GetDefaultObject<UUserWidget>())
				{
					bParentsRequireInputReg |= WidgetBPCDO->bAutomaticallyRegisterInputOnConstruction;
				}                           		                   	
		...
		OutClassRequiresInputReg = bParentsRequireInputReg;
	}
}
