...
void FWidgetBlueprintCompilerContext::CleanAndSanitizeClass(UBlueprintGeneratedClass* ClassToClean, UObject*& InOutOldCDO)
{
	...
	if ( !Blueprint->bIsRegeneratingOnLoad && bIsFullCompile )
	{
		...
		{
			ClearGeneratedClassWidgetTree(WBC_ToClean, bRecompilingOnLoad);
		}
	}
...
void FWidgetBlueprintCompilerContext::CopyTermDefaultsToDefaultObject(UObject* DefaultObject)
{
	...
	const bool bRequiresInputEvents = GetAutomaticallyRegisterInputEvents();
	...
	if ( DefaultWidget )
	{
		...
		// This "bAutomaticallyRegisterInputEvents" is set to true on node expansion of an input event. 
		DefaultWidget->bAutomaticallyRegisterInputOnConstruction = bRequiresInputEvents;
	}

	bool bClassOrParentsHaveLatentActions = false;
	bool bClassOrParentsHaveAnimations = false;
	bool bClassRequiresNativeTick = false;
	bool bParentRequiresInputReg = false;

	WidgetBP->UpdateTickabilityStatsAndInputRegistration(bClassOrParentsHaveLatentActions, bClassOrParentsHaveAnimations, bClassRequiresNativeTick, bParentRequiresInputReg);
	WidgetClass->SetClassRequiresNativeTick(bClassRequiresNativeTick);
	
	// If either this BP or any of its parents require the input registration then we want to use it
	DefaultWidget->bAutomaticallyRegisterInputOnConstruction = bRequiresInputEvents || bParentRequiresInputReg;	
...
void FWidgetBlueprintCompilerContext::FixAbandonedWidgetTree(UWidgetBlueprint* WidgetBP)
{
	...
	if (ensure(WidgetTree))
	{
		if (WidgetTree->GetName() != TEXT("WidgetTree"))
		{
			...
			WidgetTree->Rename(TEXT("WidgetTree"), nullptr, REN_DontCreateRedirectors | REN_NonTransactional | REN_DoNotDirty | REN_AllowPackageLinkerMismatch);
			WidgetTree->SetFlags(RF_DefaultSubObject);
		}
	}
}

void FWidgetBlueprintCompilerContext::UpdateGeneratedClassWidgetTree(UWidgetBlueprint* WidgetBP, UWidgetBlueprintGeneratedClass* BPGClass, UWidgetTree* OldWidgetTree, FCompilerResultsLog* MessageLog)
{
...
void FWidgetBlueprintCompilerContext::ClearGeneratedClassWidgetTree(UWidgetBlueprintGeneratedClass* WBC_ToClean, bool bRecompilingOnLoad)
{
	auto RenameObjectToTransientPackage = [bRecompilingOnLoad](UObject* ObjectToRename, const FName BaseName, bool bClearFlags)
		{
			ObjectToRename->SetFlags(RF_Transient);
...
void FWidgetBlueprintCompilerContext::FinishCompilingClass(UClass* Class)
{
	...
	if ( !bIsSkeletonOnly )
	{
		...
		UpdateGeneratedClassWidgetTree(WidgetBP,  BPGClass, OldWidgetTree, &MessageLog);
...
void FWidgetBlueprintCompilerContext::ValidateDesiredFocusWidgetName()
{
	if (UWidgetBlueprintGeneratedClass* WidgetClass = NewWidgetBlueprintClass)
	{
		...
		if (WidgetBP && UserWidgetCDO)
		{
			...
			{
				// Check the current tree first as it could includes named slot content
				if (WidgetBP->WidgetTree->FindWidget(DesiredFocusWidgetName))
				{
					return;
				}

				// If there is a root widget, also check the latest merged tree (which may include parent-inherited widgets)
				if (WidgetBP->WidgetTree->RootWidget)
				{
					const UWidgetTree* LatestWidgetTree = FWidgetBlueprintEditorUtils::FindLatestWidgetTree(WidgetBP, UserWidgetCDO);
					if (LatestWidgetTree->FindWidget(DesiredFocusWidgetName))
					{
						return;
					}
				}
