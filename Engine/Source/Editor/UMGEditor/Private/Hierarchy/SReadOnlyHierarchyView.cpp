...
void SReadOnlyHierarchyView::Construct(const FArguments& InArgs, const UWidgetBlueprint* InWidgetBlueprint)
{
	...
	InheritedNamedSlotSelectionMode = InArgs._InheritedNamedSlotSelectionMode;
...
FName SReadOnlyHierarchyView::GetItemName(const TSharedPtr<FItem>& Item) const 
{
	...
	const FName NamedSlotName = Item->InheritedNamedSlotName;
	if (!NamedSlotName.IsNone())
	{
		return NamedSlotName;
	}
...
FText SReadOnlyHierarchyView::GetItemText(TSharedPtr<FItem> Item) const
{
	if (Item)
	{
		...
		if (!NamedSlotName.IsNone())
		{
			return FText::FromName(NamedSlotName);
		}
	}
...
TSharedRef<ITableRow> SReadOnlyHierarchyView::GenerateRow(TSharedPtr<FItem> Item, const TSharedRef<STableViewBase>& OwnerTable) const
{
	...
	const FName NamedSlotName = Item->InheritedNamedSlotName;
	if (!NamedSlotName.IsNone() && InheritedNamedSlotSelectionMode == EInheritedNamedSlotSelectionMode::Disabled)
	{
		bIsEnabled = false;
	}
...
void SReadOnlyHierarchyView::GetFilterStringsForItem(TSharedPtr<FItem> Item, TArray<FString>& OutStrings) const
{
	...
	else if (const UWidgetBlueprint* Blueprint = Item->WidgetBlueprint.Get())
	{
		OutStrings.Add(Blueprint->GetName());
	}
	else
	{
		const FName NamedSlotName = Item->InheritedNamedSlotName;
		if (!NamedSlotName.IsNone())
		{
			OutStrings.Add(NamedSlotName.ToString());
		}
	}
}
...
void SReadOnlyHierarchyView::RebuildTree()
{
	...
	// Also show content placed in named slots inherited from a parent blueprint,
	// regardless of whether this blueprint has its own root widget.
	for (const FName& SlotName : WidgetBP->GetInheritedAvailableNamedSlots())
	{
		TSharedRef<FItem> NamedSlotItem = MakeShared<FItem>(SlotName);
		if (RootItem.IsValid())
		{
			RootItem->Children.Add(NamedSlotItem);
		}
		else
		{
			RootWidgets.Add(NamedSlotItem);
		}

		if (const UWidget* SlotContent = WidgetBP->WidgetTree->GetContentForSlot(SlotName))
		{
			BuildWidgetChildren(SlotContent, NamedSlotItem);
		}
	}
...
TSharedPtr<SReadOnlyHierarchyView::FItem> SReadOnlyHierarchyView::FindItem(const TArray<TSharedPtr<SReadOnlyHierarchyView::FItem>>& RootItems, FName Name) const
{
	...
	for (int32 Idx = 0; Idx < Items.Num(); ++Idx)
	{
		...
		{
			const FName NamedSlotName = Item->InheritedNamedSlotName;
			if (!NamedSlotName.IsNone() && NamedSlotName == Name)
			{
				return Item;
			}
		}
