...
TSharedRef<SWidget> SDesignerView::CreateOverlayUI()
{
	...
	[
		...
		[
			...
			.BorderBackgroundColor(FLinearColor(0.10f, 0.10f, 0.10f, 0.75f))
...
void SDesignerView::BindCommands()
{
	...
	CommandList->MapAction(
		FSequencerCommands::Get().ToggleLimitViewportSelection,
		FExecuteAction::CreateSP(this, &SDesignerView::ToggleViewportSelectionLimited),
		FCanExecuteAction::CreateSP(this, &SDesignerView::CanToggleViewportSelectionLimited),
		FIsActionChecked::CreateSP(this, &SDesignerView::IsViewportSelectionLimited)
	);
}
...
void SDesignerView::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
	...
	// Update the preview flow direction based on the active game preview language (if any)
	{
		TOptional<EFlowDirectionPreference> FlowDirectionOverride;
		if (FTextLocalizationManager::Get().IsGameLocalizationPreviewEnabled())
		{
			if (FCulturePtr PreviewLanguage = FInternationalization::Get().GetCulture(FTextLocalizationManager::Get().GetConfiguredGameLocalizationPreviewLanguage()))
			{
				FlowDirectionOverride = PreviewLanguage->IsRightToLeft() ? EFlowDirectionPreference::RightToLeft : EFlowDirectionPreference::LeftToRight;
			}
		}
		PreviewSizeConstraint->SetFlowDirectionPreference(FlowDirectionOverride.Get(EFlowDirectionPreference::Inherit));
	}
...
int32 SDesignerView::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	// SDesignerView defines a relationship between a UWorld and a portion of the Slate hierarchy (its superclass and children), therefore it is
	// responsible for pushing its scene onto the stack so that any child widgets that use Material Parameter Collection will resolve to the MPC
	// instance associated with that scene.
	...
	if (TSharedPtr<FWidgetBlueprintEditor> BlueprintEditorPtr = BlueprintEditor.Pin())
	{
		if (FPreviewScene* Preview = BlueprintEditorPtr->GetPreviewScene())
		{
			...
			{
				SceneScope = FSlateApplication::Get().GetRenderer()->RegisterCurrentScene(Scene);
			}
		}
	}

	return SDesignSurface::OnPaint(Args, AllottedGeometry, MyCullingRect, OutDrawElements, LayerId, InWidgetStyle, bParentEnabled);
}
...
FSlateColor SDesignerView::GetDesignerOutlineColor() const
{
	if ( GEditor->bIsSimulatingInEditor || GEditor->PlayWorld != nullptr )
	{
		FLinearColor SimulatingIndicatorColor = FLinearColor(0.863f, 0.407f, 0.0f);
...
void SDesignerView::HandleOnCommonResolutionSelected(const FPlayScreenResolution InResolution)
{
	...
	// Recreate the preview widget so PreConstruct fires with the new preview dimensions.
	// UpdatePreviewWidget (on next Tick) will call BroadcastDesignerChanged once the
	// new preview and its child widget trees are fully initialized.
...
void SDesignerView::ToggleViewportSelectionLimited()
{
	const TSharedPtr<FWidgetBlueprintEditor> WidgetBlueprintEditor = BlueprintEditor.Pin();
	if (!WidgetBlueprintEditor.IsValid() || WidgetBlueprintEditor->GetCurrentAnimation() == UWidgetAnimation::GetNullAnimation())
	{
		return;
	}
	...
	ActiveSequencer->SetViewportSelectionLimited(!ActiveSequencer->IsViewportSelectionLimited());
}

bool SDesignerView::CanToggleViewportSelectionLimited() const
{
	const TSharedPtr<FWidgetBlueprintEditor> WidgetBlueprintEditor = BlueprintEditor.Pin();
	if (!WidgetBlueprintEditor.IsValid() || WidgetBlueprintEditor->GetCurrentAnimation() == UWidgetAnimation::GetNullAnimation())
	{
...
bool SDesignerView::IsViewportSelectionLimited() const
{
	const TSharedPtr<FWidgetBlueprintEditor> WidgetBlueprintEditor = BlueprintEditor.Pin();
	if (!WidgetBlueprintEditor.IsValid() || WidgetBlueprintEditor->GetCurrentAnimation() == UWidgetAnimation::GetNullAnimation())
	{
	...
	return ActiveSequencer->IsViewportSelectionLimited();
}
