...
FWidgetBlueprintEditor::~FWidgetBlueprintEditor()
{
	...
	if (Blueprint)
	{
		...
		{
			AssetEditorSubsystem->GetDebugObjectDelegateForClass(Blueprint->GeneratedClass)->RemoveAll(this);
		}
	}
}
...
void FWidgetBlueprintEditor::InitWidgetBlueprintEditor(const EToolkitMode::Type Mode, const TSharedPtr< IToolkitHost >& InitToolkitHost, const TArray<UBlueprint*>& InBlueprints, bool bShouldOpenInDefaultsMode)
{
	...
	if (UAssetEditorSubsystem* AssetEditorSubsystem = GEditor->GetEditorSubsystem<UAssetEditorSubsystem>())
	{
		AssetEditorSubsystem->GetDebugObjectDelegateForClass(Blueprint->GeneratedClass)
			->AddSP(this, &FWidgetBlueprintEditor::OnDebugObjectAccess);
	}
}
...
void FWidgetBlueprintEditor::OnDebugObjectAccess(UObject* DebugObject)
{
	if (DebugObject)
	{
		if (UWidgetBlueprint* WidgetBlueprint = GetWidgetBlueprintObj())
		{
			if (WidgetBlueprint->GetObjectBeingDebugged() != DebugObject)
			{
				WidgetBlueprint->SetObjectBeingDebugged(DebugObject);
				RefreshMyBlueprint();
			}
		}
	}
