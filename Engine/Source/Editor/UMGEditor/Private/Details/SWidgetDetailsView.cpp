...
void SWidgetDetailsView::Construct(const FArguments& InArgs, TSharedPtr<FWidgetBlueprintEditor> InBlueprintEditor)
{
	...
	TSharedRef<SWidget> IsVariableCheckbox =
		...
		.AddMetaData<FTagMetaData>(TEXT("Details.IsVariable"))
		[
			SNew(STextBlock)
				...
				[
					...
					[
						...
						[
							...
							.AddMetaData<FTagMetaData>(TEXT("Details.WidgetName"))
						]
					]
...
void SWidgetDetailsView::HandleNameTextChanged(const FText& Text)
{
	...
	// We delay the error message to be sure we are outside of attribute processing.
	// Otherwise, we might add or remove a widget (the error reporting one) which is not allowed during Prepass
	TWeakPtr<SEditableTextBox> WeakNameTextBox = NameTextBox;
	RegisterActiveTimer(0.f, FWidgetActiveTimerDelegate::CreateLambda(
		[OutErrorMessage = MoveTemp(OutErrorMessage), WeakNameTextBox = MoveTemp(WeakNameTextBox)](double InCurrentTime, float InDeltaTime)
		{
			if (TSharedPtr<SEditableTextBox> NameTextBox = WeakNameTextBox.Pin())
			{
...
void SWidgetDetailsView::HandleNameTextCommitted(const FText& Text, ETextCommit::Type CommitType)
{
	...
	if ( !IsReentrant )
	{
		...
		if ( SelectedObjects.Num() == 1 )
		{
			...
			{
				UWidget* Widget = Cast<UWidget>(SelectedObjects[0].Get());
				if (Widget && !Widget->GetLabelText().EqualTo(Text))
				{
...
void SWidgetDetailsView::HandleIsVariableChanged(ECheckBoxState CheckState)
{
	if (SelectedObjects.Num() > 0)
	{
		...
		if (WidgetToModify.Num() > 0)
		{
			...
			{
				FWidgetBlueprintOperationUtils::ToggleWidgetAsVariable(Blueprint, Widget, CheckState == ECheckBoxState::Checked, false);
			}
