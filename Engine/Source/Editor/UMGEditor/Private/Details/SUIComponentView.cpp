...
private:
	...
	FReply OnAcceptDrop(const FDragDropEvent& DragDropEvent, EItemDropZone InItemDropZone, TSharedPtr<FUIComponentListItem> InListItem)
	{
			...
			)
		{
			...
			if (UWidgetBlueprint* WidgetBlueprint = WidgetBlueprintEditor ? WidgetBlueprintEditor->GetWidgetBlueprintObj() : nullptr)
			{
				...
				FText MoveErrorMessage;
				if (!FUIComponentUtils::MoveComponent(WidgetBlueprintEditor.Get(),
					WidgetBlueprint,
				...
				{
					UE_LOGF(LogUMGEditor, Error, "%ls", *MoveErrorMessage.ToString());
				}
...
FReply SUIComponentView::OnAddComponentButtonClicked()
{
	...
	{
		// Gather classes already present on the selected widget so the class picker can hide related classes.
		TSet<const UClass*> ExistingComponentClasses;
		if (const UUserWidget* PreviewUserWidget = WidgetBlueprintEditor->GetPreview())
		{
			if (const UUIComponentUserWidgetExtension* Extension = PreviewUserWidget->GetExtension<UUIComponentUserWidgetExtension>())
			{
				...
				{
					for (const UUIComponent* Component : Extension->GetComponentsFor(SelectedWidget))
					{
						if (Component)
						{
							ExistingComponentClasses.Add(Component->GetClass());
						}
					}
				}
			}
		...
		const bool bPressedOk = SClassPickerDialog::PickClass(TitleText, FUIComponentUtils::CreateClassViewerInitializationOptions(ExistingComponentClasses), ChosenClass, UUIComponent::StaticClass());
		if (ChosenClass)
		{
			...
			for (const FWidgetReference& SelectedObject : SelectedWidgets)
			{
				if (const UWidget* SelectedWidget = Cast<UWidget>(SelectedObject.GetPreview()))
				{
					...
					FText AddErrorMessage;
					if (!FUIComponentUtils::AddComponent(WidgetBlueprintEditor.Get(), WidgetBlueprint, ChosenClass, SelectedWidget->GetFName(), AddErrorMessage))
					{
						UE_LOGF(LogUMGEditor, Error, "%ls", *AddErrorMessage.ToString());
					}
				}
			}
		}
	...
	if (const TSharedPtr<FWidgetBlueprintEditor> WidgetBlueprintEditor = BlueprintEditor.Pin())
	{
		if (UWidget* SelectedWidget = GetSelectedWidget())
		{
			for (TSharedPtr<FUIComponentListItem>& Item : ComponentListView->GetSelectedItems())
			{
				...
				{
					UWidgetBlueprint* WidgetBlueprint = WidgetBlueprintEditor->GetWidgetBlueprintObj();
					FText RemoveErrorMessage;
					if (!FUIComponentUtils::RemoveComponent(WidgetBlueprintEditor.Get(), WidgetBlueprint, Item->Component->GetClass(), SelectedWidget->GetFName(), RemoveErrorMessage))
					{
						UE_LOGF(LogUMGEditor, Error, "%ls", *RemoveErrorMessage.ToString());
					}
				}
			}
		}		 
