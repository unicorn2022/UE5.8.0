...
enum class EOutputFlags : uint32
{
	None					= 0,
	DetailedFilterReasons	= 1 << 0,
	MountPointPaths			= 1 << 1
};

ENUM_CLASS_FLAGS(EOutputFlags);
...
FMountPointStatistics ProcessMountPoint(FStringView RootPath, const TArray<FString>& FilePaths)
{
	...
	ParallelForWithTaskContext(ContextStats, FilePaths.Num(),
		[&FilePaths](FMountPointStatistics& Stats, int32 Index)
		{
			...
			{
				UE_LOGF(LogVirtualization, Error, "Unable to find file '%ls'", *FilePath);
				return;
			}
...
bool OutputMountPointStatistics(const FString& OutputFilePath, const TArray<FMountPointStatistics>& Statistics, EOutputFlags Flags, EByteFormatting ByteFormat)
{
...

	{
		...
		if (EnumHasAllFlags(Flags, EOutputFlags::DetailedFilterReasons))
		{
		...
		if (EnumHasAllFlags(Flags, EOutputFlags::MountPointPaths))
		{
			Heading << ",Path";
		}
	...
	{
		const double VirtualizedPercent = Stats.NumFiles > 0 ? ((double)(Stats.NumFiles - Stats.NumFilesWithPendingPayloads) / (double)Stats.NumFiles) * 100.0 : 0.0;
		...
		Line << "\"" << FPathViews::GetPathLeaf(Stats.RootPath) << "\",";
...
int32 UGenerateMountPointPayloadManifestCommandlet::Main(const FString& Params)
{
	...
	UE_LOGF(LogVirtualization, Display, "Generating mount point summary for all files...");
	...
	UE_LOGF(LogVirtualization, Display, "Found %d files to look in", PackageNames.Num());

	// RootPath of the mount point, array of file paths under that mount point
	TMap<FString, TArray<FString>> MountPointMap;

	{
		TRACE_CPUPROFILER_EVENT_SCOPE(SortingMountPoints);
		UE_LOGF(LogVirtualization, Display, "Sorting by mount point...");
		...
		for (FString& Path : PackageNames)
		{
			...
			if (ContentIndex != INDEX_NONE)
			{
				...
				if (MountPointIndex != INDEX_NONE)
				{
					...
					FStringView MountPoint = FStringView(Path).SubStr(0, ContentIndex);
			...
			{
				UE_LOGF(LogVirtualization, Warning, "Package '%ls' not under a valid content directory, skipping!", *Path);
			}
		}
	}

	UE_LOGF(LogVirtualization, Display, "Found %d mountpoints", MountPointMap.Num());

	TArray<FMountPointStatistics> Stats;
	Stats.Reserve(MountPointMap.Num());

	UE_LOGF(LogVirtualization, Display, "Processing mountpoints...");
	...
	UE_LOGF(LogVirtualization, Display, "Processing mountpoints completed");

	EOutputFlags Flags = EOutputFlags::None;
	if (bDetailedFilterReasons)
	{
		Flags |= EOutputFlags::DetailedFilterReasons;
	}

	if (bOutputPaths)
	{
		Flags |= EOutputFlags::MountPointPaths;
	}

	if (!OutputMountPointStatistics(OutputFilePath, Stats, Flags, ByteFormat))
	{
		return -1;
	}
...
bool UGenerateMountPointPayloadManifestCommandlet::TryParseCmdline(const FString& Params)
{
	...
	if (Switches.Contains(TEXT("NoPaths")))
	{
		bOutputPaths = false;
	}
