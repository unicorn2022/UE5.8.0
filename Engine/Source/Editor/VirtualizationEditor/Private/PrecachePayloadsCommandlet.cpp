...
int32 UPrecachePayloadsCommandlet::Main(const FString& Params)
{
	...
	UE_LOGF(LogVirtualization, Display, "Found %d packages", PackageNames.Num());

	TArray<FIoHash> PayloadIds = UE::Virtualization::FindVirtualizedPayloads(PackageNames);

	if (PayloadIds.IsEmpty())
	{
		UE_LOGF(LogVirtualization, Display, "No virtualized payloads found");
		return 0;
	}

	UE_LOGF(LogVirtualization, Display, "Found %d virtualized payloads to precache", PayloadIds.Num());
	UE_LOGF(LogVirtualization, Display, "Precaching payloads...");
	...
	UE::Virtualization::PullPayloadsThreaded(PayloadIds, BatchSize, TEXT("Cached"), [](const UE::Virtualization::FPullRequest& Request)
		{
			...
			{
				UE_LOGF(LogVirtualization, Error, "%ls: Failed to precache payload", *LexToString(Request.GetIdentifier()));
			}
		});

	UE_LOGF(LogVirtualization, Display, "Precaching complete!");
