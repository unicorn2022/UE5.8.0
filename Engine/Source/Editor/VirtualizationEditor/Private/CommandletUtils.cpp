...
bool PullPayloadsThreaded(TConstArrayView<FIoHash> PayloadIds, int32 BatchSize, const TCHAR* ProgressString, TUniqueFunction<void(const FPullRequest& Response)>&& Callback)
{
	...
	UE_LOGF(LogVirtualization, Display, "Will run up to %d pull batches concurrently", ConcurrentTasks);
	...
	while (NumTasks != 0 || !WorkQueue.IsEmpty())
	{
		...
		if (FPlatformTime::Seconds() - LogTimer >= 30.0)
		{
			...
			UE_LOGF(LogVirtualization, Display, "%ls Cached %d/%d (%.1f%%)", 
...
TArray<FString> FindPackages(EFindPackageFlags Flags)
{
	...
	UE_LOGF(LogVirtualization, Display, "Searching for packages under the current project...");
...
{
	UE_LOGF(LogVirtualization, Display, "Searching for packages under '%ls'...", *DirectoryToSearch);
