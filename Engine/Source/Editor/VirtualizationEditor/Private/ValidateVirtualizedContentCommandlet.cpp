...
public:
	...
	void PrintError(const FIoHash& Id, const TCHAR* ErrorMessage) const
	{
		...
		UE_LOGF(LogVirtualization, Error, "%ls", FinalMessage.ToString());
	}
...
int32 UValidateVirtualizedContentCommandlet::Main(const FString& Params)
{
	...
	UE_LOGF(LogVirtualization, Display, "Finding packages in the project...");
	TArray<FString> PackagePaths = FindPackages(EFindPackageFlags::ExcludeEngineContent);
	UE_LOGF(LogVirtualization, Display, "Found %d package(s)", PackagePaths.Num());

	TMap<FString, UE::FPackageTrailer> Packages;
	TSet<FIoHash> Payloads;

	UE_LOGF(LogVirtualization, Display, "Scanning package(s) for virtualized payloads...");
	FindVirtualizedPayloadsAndTrailers(PackagePaths, Packages, Payloads);
	UE_LOGF(LogVirtualization, Display, "Found %d virtualized package(s) with %d unique payload(s)", Packages.Num(), Payloads.Num());
	...
	if (ErrorCount == 0)
	{
		UE_LOGF(LogVirtualization, Display, "All virtualized payloads could be found in persistent storage");
		return 0;
	}
	else
	{
		UE_LOGF(LogVirtualization, Error, "%d/%d package(s) had at least one virtualized payload missing from persistent storage", ErrorCount, Packages.Num());
		return 0;
	}
}
...
int32 UValidateVirtualizedContentCommandlet::ValidatePayloadsExists(const TMap<FString, UE::FPackageTrailer>& Packages, const TSet<FIoHash>& Payloads)
{
	...
	UE_LOGF(LogVirtualization, Display, "Validating payloads existence..");

	IVirtualizationSystem& System = IVirtualizationSystem::Get();

	UE_LOGF(LogVirtualization, Display, "Querying the state of the virtualized payload(s) in persistent storage...");
	...
	{
		UE_LOGF(LogVirtualization, Error, "Failed to query the statuses of the payload(s)");
		return 1;
	}
...

	{
		...
		UE_LOGF(LogVirtualization, Display, "Checking for missing payloads...");
		...
		for (const TPair<FString, UE::FPackageTrailer>& Package : Packages)
		{
			...
			for (const FIoHash& PayloadId : VirtualizedPayloads)
			{
				...
				if (PayloadStatuses[Index] == EPayloadStatus::FoundPartial)
				{
					...
					UE_LOGF(LogVirtualization, Error, "%ls: Payload '%ls' could not be found in all persistent backends", *Package.Key, *LexToString(PayloadId));
					bFoundErrors = true;
				}
				else if (PayloadStatuses[Index] != EPayloadStatus::FoundAll)
				{
					UE_LOGF(LogVirtualization, Error, "%ls: Payload '%ls' could not be found in any persistent backend", *Package.Key, *LexToString(PayloadId));
					bFoundErrors = true;
				}
			}
...
{
	UE_LOGF(LogVirtualization, Display, "Validating payloads existence and content...");
