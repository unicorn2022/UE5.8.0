...
public:
	...
	bool OpenNewSheet()
	{
		...
		{
			UE_LOGF(LogVirtualization, Error, "Failed to open '%ls' for write", CSVPath.ToString());
			return false;
		}
	}
...
int32 UGeneratePayloadManifestCommandlet::Main(const FString& Params)
{
...

	{
		...
		UE_LOGF(LogVirtualization, Display, "Parsing files...");
		...
		for (const FString& PackagePath : PackageNames)
		{
			...
			if (++FilesParsed % 10000 == 0)
			{
				...
				UE_LOGF(LogVirtualization, Display, "Parsed %d/%d (%.0f%%)", FilesParsed, PackageNames.Num(), PercentCompleted);
			}
	...
	UE_LOGF(LogVirtualization, Display, "Found %d package trailers with %d payloads", PackageTrailerCount, PayloadCount);
	UE_LOGF(LogVirtualization, Display, "%ls", *Sheet.GetDebugInfo());
	...
	auto ApplyFilter = [this, &Switches](const TCHAR* FilterName, EPayloadFilter FilterType)->bool
		{
			if (Switches.Contains(FilterName))
			{
				...
				{
					UE_LOGF(LogVirtualization, Error, "Comandlet cannot have multiple filters active at the same time");
					return false;
				}
