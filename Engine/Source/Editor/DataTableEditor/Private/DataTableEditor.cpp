...
#include "DataTableEditorSettings.h"
...
void FDataTableEditor::OnCopySheetClicked()
{
	...
	for (const TTuple<FName, unsigned char*>& RowMap : TablePtr->GetRowMap())
	{
		Rows.Emplace(RowMap.Key, RowMap.Value);
	}

	const FString ClipboardValue = USheetSerializationSubSystem::Get()->ExportForSheet(Rows, TablePtr, true);
	FPlatformApplicationMisc::ClipboardCopy(*ClipboardValue);
}

void FDataTableEditor::OnPasteSheetClicked()
{
	UDataTable* TablePtr = Cast<UDataTable>(GetEditingObject());
	if (!TablePtr)
	{
	...
	ClipboardValue.TrimEndInline();

	const FScopedTransaction Transaction(LOCTEXT("PasteSheetOverTable", "Paste Spreadsheet Over Data Table"));
	TablePtr->Modify();

	FDataTableEditorUtils::BroadcastPreChange(TablePtr, FDataTableEditorUtils::EDataTableChangeInfo::RowList);

	if (!USheetSerializationSubSystem::Get()->ImportFromSheet(ClipboardValue, TablePtr, GWarn))
	{
		FNotificationInfo Info(LOCTEXT("FailedPasteSheet", "Failed to paste rows from sheet"));
		FSlateNotificationManager::Get().AddNotification(Info);
	}

	TablePtr->HandleDataTableChanged(NAME_None);
	TablePtr->MarkPackageDirty();
	FDataTableEditorUtils::BroadcastPostChange(TablePtr, FDataTableEditorUtils::EDataTableChangeInfo::RowList);
}
...
void FDataTableEditor::FillToolbar(FToolBarBuilder& ToolbarBuilder)
{
	ToolbarBuilder.BeginSection("DataTableCommands");
	{
		...
		ToolbarBuilder.AddToolBarButton(
			FUIAction(FExecuteAction::CreateSP(this, &FDataTableEditor::OnCopySheetClicked)),
			NAME_None,
			LOCTEXT("CopySheetText", "Copy Sheet"),
			LOCTEXT("CopySheetTooltip", "Copy this DataTable as a spreadsheet"),
			FSlateIcon(FAppStyle::GetAppStyleSetName(), "GenericCommands.Copy"));
		ToolbarBuilder.AddToolBarButton(
			FUIAction(FExecuteAction::CreateSP(this, &FDataTableEditor::OnPasteSheetClicked)),
			NAME_None,
			LOCTEXT("PasteSheetText", "Paste Sheet"),
			LOCTEXT("PasteSheetTooltip", "Paste spreadsheet values over the DataTable"),
			FSlateIcon(FAppStyle::GetAppStyleSetName(), "GenericCommands.Paste"));
	}
...
void FDataTableEditor::CopySelectedRow()
{
	...
	FString ClipboardValue;
	if (GetDefault<UDataTableEditorSettings>()->bCopyAsSpreadsheetCells)
	{
		TArray<FDataTableRowData> Rows;
		Rows.Emplace(HighlightedRowName, RowPtr);
		ClipboardValue = USheetSerializationSubSystem::Get()->ExportForSheet(Rows, TablePtr, false);
	}
	else
	{
...
void FDataTableEditor::PasteOnSelectedRow()
{
	...
	if (!ClipboardValue.IsEmpty())
	{
		...
		// It is a reasonably safe assumption that if the entire contents is in parentheses, it's in Unreal's own format
		if (ClipboardValue[0] == TCHAR('(') && ClipboardValue[ClipboardValue.Len() - 1] == TCHAR(')'))
		{
			Result = TablePtr->RowStruct->ImportText(*ClipboardValue, RowPtr, TablePtr, PPF_Copy, GWarn, GetPathNameSafe(TablePtr->RowStruct));
		}
		// Otherwise, we'll assume it's from a spreadsheet
		else
		{
			FName RowName = HighlightedRowName;
			if (USheetSerializationSubSystem::Get()->ImportRowFromSheet(ClipboardValue, TablePtr, RowName, RowPtr, GWarn))
			{
				Result = *ClipboardValue;
				if (RowName != HighlightedRowName)
				{
					if (!TablePtr->GetRowMap().Contains(RowName))
					{
						if (FDataTableEditorUtils::RenameRow(TablePtr, HighlightedRowName, RowName))
						{
							SetHighlightedRow(RowName);
						}
					}
					...
					{
						FNotificationInfo Info(LOCTEXT("RowNameAlreadyExists", "Unable to rename row. Name already in use."));
						FSlateNotificationManager::Get().AddNotification(Info);
					}
				}
			}
