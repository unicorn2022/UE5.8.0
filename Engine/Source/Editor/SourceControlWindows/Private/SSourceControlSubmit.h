...
private:
	...
	/** Check if the warning should be visible in the warning panel. */
	bool IsValidationWarningVisible() const;
	bool IsCheckoutWarningVisible() const;
	bool IsDescriptionWarningVisible() const;
...
private:
	...
	/** Collection of states that are checked out by other users. */
	TArray<FSourceControlStateRef> CheckedOutByOther;
	...
	/** Whether the content verse path should be shown or not */
