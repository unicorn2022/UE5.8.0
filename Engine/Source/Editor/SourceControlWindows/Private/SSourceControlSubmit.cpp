...
bool TryToVirtualizeFilesToSubmit(const TArray<FSourceControlStateRef>& FileStates, FText& Description, FText& OutFailureMsg)
{
	...
	TArray<FString> FilesToSubmit;
	{
		...
		for (const FSourceControlStateRef& State : FileStates)
		{
			if (State->IsDeleted())
			{
				UE_LOGF(LogVirtualization, Verbose, "Ignoring package marked for delete '%ls'", *State->GetFilename());
				continue;
			}

			if (State->IsIgnored())
			{
				UE_LOGF(LogVirtualization, Verbose, "Ignoring package marked for ignore '%ls'", *State->GetFilename());
				continue;
			}
...
void SSourceControlSubmitWidget::Construct(const FArguments& InArgs)
{
	...
	if (CanCheckOut() && !bAllowUncheckFiles)
	{
		for (const auto& Item : InArgs._Items.Get())
		{
			if (Item->IsCheckedOutOther() && !Item->CanCheckout()) // Is it an exclusive checkout?
			{
				CheckedOutByOther.Add(Item);
			}
		}
	}
			...
			[
				...
				.Visibility_Lambda(
					[this]()
					{
						...
						{
							return IsDescriptionWarningVisible() ? EVisibility::Visible : EVisibility::Collapsed;
						}
						else
						{
			...
			[
				...
				.Visibility_Lambda(
					[this]()
					{
						if (IsWarningPanelVisible() == EVisibility::Visible)
						{
							return IsValidationWarningVisible() ? EVisibility::Visible : EVisibility::Collapsed;
						}
						else
						{
							return EVisibility::Collapsed;
				...
				[
					SNew(SErrorText)
					.ErrorText(NSLOCTEXT("SourceControl.SubmitPanel", "ChangeListErrorWarning", "Errors need to be fixed to submit"))
				]
			]
			+ SVerticalBox::Slot()
			.AutoHeight()
			[
				...
				.Visibility_Lambda(
					[this]()
					{
						if (IsWarningPanelVisible() == EVisibility::Visible)
						{
							return IsCheckoutWarningVisible() ? EVisibility::Visible : EVisibility::Collapsed;
						}
						else
						{
							return EVisibility::Collapsed;
				...
				[
					SNew(SErrorText)
					.ErrorText(NSLOCTEXT("SourceControl.SubmitPanel", "ChangeListCheckedOutWarning", "Checkouts by other users need to be cleared to submit"))
				]
			]
		];
	}
...
void SSourceControlSubmitWidget::OnRevert()
{
	...
	auto RemoveItemsFromListView = [this](const TSet<FString>& ItemsToRemove)
	{
		ListViewItems.RemoveAll([&ItemsToRemove](const FChangelistTreeItemPtr& ListViewItem) -> bool
			{
				const FString FilePath = GetFileItem(ListViewItem)->GetFileName().ToString();
				return ItemsToRemove.Contains(FilePath);
			}
		);

		CheckedOutByOther.RemoveAll([&ItemsToRemove](const FSourceControlStateRef& StateItem) -> bool
			{
				const FString& FilePath = StateItem->GetFilename();
				return ItemsToRemove.Contains(FilePath);
			}
		);
	};
...

	{
		...
		if (PackagesToRevert.Num() > 0)
		{
			...
			RemoveItemsFromListView(TSet<FString>(PackagesToRevert));
		}
		if (FilesToRevert.Num() > 0)
		{
			bAnyReverted |= SourceControlHelpers::RevertFiles(FilesToRevert);
			RemoveItemsFromListView(TSet<FString>(FilesToRevert));
		}
...
void SSourceControlSubmitWidget::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	...
	// The user can release locks in-widget, so need to check for lock status changes.
	if (CheckedOutByOther.Num() > 0)
	{
		for (int32 Index = 0; Index < CheckedOutByOther.Num(); /* */)
		{
			const FSourceControlStateRef& State = CheckedOutByOther[Index];
			if (State->IsCheckedOutOther() && !State->CanCheckout())
			{
				++Index;
			}
			else
			{
				CheckedOutByOther.RemoveAtSwap(Index);
			}
		}
	}
}
...
{
	return bAllowSubmit && !ChangeListDescriptionTextCtrl->GetText().ToString().TrimStartAndEnd().IsEmpty() && ListViewItems.Num() > 0 && CheckedOutByOther.Num() == 0;
}
...
bool SSourceControlSubmitWidget::IsValidationWarningVisible() const
{
	const bool bValidationWarningVisible = !bAllowSubmit;
	return bValidationWarningVisible;
}

bool SSourceControlSubmitWidget::IsCheckoutWarningVisible() const
{
	const bool bValidationWarningVisible = IsValidationWarningVisible();
	const bool bCheckoutWarningVisible = CheckedOutByOther.Num() > 0;
	return bCheckoutWarningVisible && !bValidationWarningVisible;
}

bool SSourceControlSubmitWidget::IsDescriptionWarningVisible() const
{
	const bool bValidationWarningVisible = IsValidationWarningVisible();
	const bool bCheckoutWarningVisible = IsCheckoutWarningVisible();
	const bool bDescriptionWarningVisible = ChangeListDescriptionTextCtrl->GetText().ToString().TrimStartAndEnd().IsEmpty();
	return bDescriptionWarningVisible && !bValidationWarningVisible && !bCheckoutWarningVisible;
}
