...
static bool SaveDirtyPackages(bool bUseDialog)
{
	...
	if (bSaved)
	{
		bSaved = !FEditorFileUtils::HasDirtyPackages();
	}
...
{
	FSourceControlPreSubmitFilesValidationExDelegate ValidationDelegate = ISourceControlModule::Get().GetRegisteredPreSubmitFilesValidationEx();

	FSourceControlValidationDelegateResult ValidationResult;

	bool bValidationResult = true;

	if (ValidationDelegate.ExecuteIfBound(InStates, ValidationResult))
	{
...
bool FSourceControlWindows::CanChoosePackagesToCheckIn()
{
	...
	{
		TOptional<bool> bHasChangesToCheckIn = SourceControlModule.GetProvider().HasChangesToCheckIn();
		if (bHasChangesToCheckIn.IsSet())
		{
			return bHasChangesToCheckIn.GetValue();
		}
		else
		{
			return true;
...
bool FSourceControlWindows::CanSyncLatest()
{
	...
	{
		TOptional<bool> bHasChangesToSync = SourceControlModule.GetProvider().HasChangesToSync();
		if (bHasChangesToSync.IsSet())
		{
			return bHasChangesToSync.GetValue();
		}
		else
		{
			return true;
...
bool FSourceControlWindows::PromptForCheckin(FCheckinResultInfo& OutResultInfo, const TArray<FString>& InPackageNames, const TArray<FString>& InPendingDeletePaths, const TArray<FString>& InConfigFiles, bool bUseSourceControlStateCache)
{
	...
	//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	// Generate the submit files window title
	FSubmitTitleOverrideParameters SubmitTitleOverrideParameters;
	SubmitTitleOverrideParameters.ToSubmit.SetSubtype<TArray<FString>>(AllFiles);
	...
	TArray<FString> CombinedFileList;
	CombinedFileList.Append(Description.FilesForAdd);
	...
	if (Description.FilesForAdd.Num() > 0)
	{
		...
		const bool bAddSuccess = SourceControlProvider.Execute(ISourceControlOperation::Create<FMarkForAdd>(), Description.FilesForAdd) == ECommandResult::Succeeded;
		if (!bAddSuccess)
		{
	...
	//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	// Virtualize the files that need to be submitted
	...
	bool bSyncLatestNeeded = bUsesSnapshots ? FSourceControlWindows::CanSyncLatest() : false;

	FText CheckinSuccessMessage;

	do
	{
		//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		// Sync to latest if using snapshots as we cannot check in unless we're at head
		if (bSyncLatestNeeded)
		{
		...
		//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		// Check if another user beat us to snapshot creation
		// This can happen - for example - when taking a long time writing a description
		bSyncLatestNeeded = bUsesSnapshots ? FSourceControlWindows::CanSyncLatest() : false;

		//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		// Store the success message for later use
		CheckinSuccessMessage = CheckInOperation->GetSuccessMessage();

	} while ((bUsesSnapshots && bSyncLatestNeeded) && !bCheckinSuccess && !bCheckinCancelled);
	...
	if (bCheckinSuccess)
	{
		...
		FNotificationInfo Info(CheckinSuccessMessage);
		...
		// also add to the log
		FMessageLog("SourceControl").Info(CheckinSuccessMessage);

		OutResultInfo.Result         = ECommandResult::Succeeded;
		OutResultInfo.Description    = CheckinSuccessMessage;
...
bool FSourceControlWindows::PromptForCheckedOut(bool bUseSourceControlStateCache, const TArray<FString>& InFileNames, const FCheckedOutSetupInfo& InSetupInfo)
{
