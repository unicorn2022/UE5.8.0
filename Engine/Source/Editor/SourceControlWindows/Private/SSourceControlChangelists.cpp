...
bool AnyUncontrolledChangelistContainsFilename(const FString& Filename)
{
	FUncontrolledChangelistsModule& UncontrolledChangelistsModule = FUncontrolledChangelistsModule::Get();

	if (UncontrolledChangelistsModule.IsEnabled())
	{
		TArray<TSharedRef<FUncontrolledChangelistState>> UncontrolledChangelistStates = UncontrolledChangelistsModule.GetChangelistStates();
		...
		else if (Item->GetTreeItemType() == IChangelistTreeItem::OfflineFile)
		{
			if (TSharedPtr<IChangelistTreeItem> Parent = Item->GetParent())
			{
				...
				else if (Parent->GetTreeItemType() == IChangelistTreeItem::UnsavedAssets)
				{
					...
					if (AnyUncontrolledChangelistContainsFilename(Filename))
					{
						OutUncontrolledFiles.Add(Filename);
					}
					else
...
static bool GetChangelistValidationResult(FSourceControlChangelistPtr InChangelist, FString& OutValidationTitleText, FString& OutValidationWarningsText, FString& OutValidationErrorsText)
{
	...
	if (ValidationDelegate.ExecuteIfBound(InChangelist, ValidationResult))
	{
		EMessageSeverity::Type MessageSeverity = EMessageSeverity::Info;

		if (ValidationResult.Result == EDataValidationResult::Invalid || ValidationResult.Errors.Num() > 0)
		{
		...
		else if (ValidationResult.Result == EDataValidationResult::NotValidated || ValidationResult.Warnings.Num() > 0)
		{
		...
		AppendInfo(ValidationResult.Errors, LOCTEXT("ValidationErrorsHeader", "Encountered {Count} {Count}|plural(one=error,other=errors):"), LOCTEXT("ValidationErrorsOverflow", "See log for complete list of errors"), OutValidationErrorsText);
		AppendInfo(ValidationResult.Warnings, LOCTEXT("ValidationWarningsHeader", "Encountered {Count} {Count}|plural(one=warning,other=warnings):"), LOCTEXT("ValidationWarningsOverflow", "See log for complete list of warnings"), OutValidationWarningsText);
	}
...
void SSourceControlChangelistsWidget::OnSubmitChangelist()
{
	...
	// The title from the window
	FSubmitTitleOverrideParameters SubmitTitleOverrideParameters;
	SubmitTitleOverrideParameters.ToSubmit.SetSubtype<FString>(ChangelistState->GetChangelist()->GetIdentifier());

	const FText Title = ISourceControlWindowsModule::Get().SubmitTitleOverrideDelegate.IsBound()
		? ISourceControlWindowsModule::Get().SubmitTitleOverrideDelegate.Execute(SubmitTitleOverrideParameters)
		: NSLOCTEXT("SourceControl.ConfirmSubmit", "Title", "Confirm Submit");
...
TValueOrError<void, void> SSourceControlChangelistsWidget::TryMoveFiles()
{
	...
	else if ((bAddNewUncontrolledChangelistEntry && bAddNewChangelistEntry && PickedItem == 1) || (bAddNewUncontrolledChangelistEntry && !bAddNewChangelistEntry && PickedItem == 0)) // Move files to a new uncontrolled changelist
	{
		ExecuteUncontrolledChangelistOperation(LOCTEXT("Moving_Files_New_Uncontrolled_Changelist", "Moving file(s) to a new uncontrolled changelist..."), [&]()
		{
			...
			// SelectedControlledFiles and/or SelectedUncontrolledFiles can contain UnsavedAssets (IChangelistTreeItem::OfflineFile)
			else
			{
				if (!SelectedControlledFiles.IsEmpty())
				{
				...
				if (!SelectedUncontrolledFiles.IsEmpty())
				{
					UncontrolledChangelistsModule.MoveFilesToUncontrolledChangelist(SelectedUncontrolledFiles, NewUncontrolledChangelist.GetValue());
				}
			}
		});
	}
		...
		else if (MoveDestination->GetTreeItemType() == IChangelistTreeItem::UncontrolledChangelist)
		{
			...
			ExecuteUncontrolledChangelistOperation(LOCTEXT("Moving_Uncontrolled_Changelist_To", "Moving uncontrolled files..."), [&]()
			{
				...
				else
				{
					...
					if (!SelectedUncontrolledFiles.IsEmpty())
					{
						FUncontrolledChangelistsModule::Get().MoveFilesToUncontrolledChangelist(SelectedUncontrolledFiles, UncontrolledChangelist);
					}
				}
			});
		}
...
void SSourceControlChangelistsWidget::OnSaveAssetsInChangelist()
{
	TArray<TSharedPtr<IChangelistTreeItem>> Files = FileListNodes;

	TArray<UPackage*> Packages;
	Packages.Reserve(Files.Num()); 
	{
		for (const TSharedPtr<IChangelistTreeItem>& File : Files)
		{
			if (TSharedPtr<IFileViewTreeItem> FileItem = StaticCastSharedPtr<IFileViewTreeItem>(File); FileItem.IsValid())
			{
				FString Filename = FileItem->GetFullPathname();
	...
	FEditorFileUtils::PromptForCheckoutAndSave(Packages, bOnlyDirty, bPromptToSave);

	bShouldRefresh = true;
}

bool SSourceControlChangelistsWidget::CanSaveAssetsInChangelist()
{
	TArray<TSharedPtr<IChangelistTreeItem>> Files = FileListNodes;
...
TSharedPtr<SWidget> SSourceControlChangelistsWidget::OnOpenContextMenu()
{
	...
	if (bHasSelectedChangelist || bHasSelectedUncontrolledChangelist)
	{
		Section.AddSeparator("SaveAssets");

		Section.AddMenuEntry(
			"Save assets",
			LOCTEXT("SourceControl_SaveAssets", "Save All"),
			LOCTEXT("SourceControl_SaveAssets_Tooltip", "Save all unsaved assets in the selected changelist."),
			FSlateIcon(),
			FUIAction(FExecuteAction::CreateSP(this, &SSourceControlChangelistsWidget::OnSaveAssetsInChangelist), 
				FCanExecuteAction::CreateSP(this, &SSourceControlChangelistsWidget::CanSaveAssetsInChangelist)));
	}
