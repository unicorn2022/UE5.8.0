...
public:
	...
	DECLARE_DELEGATE_OneParam(FOnOverrideAssetPickerConfig, FAssetPickerConfig&)
	DECLARE_DELEGATE_OneParam(FOnActiveSectionChanged, const UTaggedAssetBrowserSection*)
	DECLARE_DELEGATE(FOnPrimaryFilterSelectionChanged)
	...
	SLATE_BEGIN_ARGS(STaggedAssetBrowser)
		...
		/**
		 * Generic Callback for editors to customize our asset picker config before we create it. So that we don't have to update our API every time the asset picker changes.
		 * Use with care, as we do not guarantee STaggedAssetBrowser functionality will work depending on customizations done. 
		 * Consider capturing existing values if previous functionality may be important to you.
		 */
		SLATE_EVENT(FOnOverrideAssetPickerConfig, OnOverrideAssetPickerConfig)
		...
		/** Determines the asset selection mode; single asset selection by default. */
		SLATE_ARGUMENT(ESelectionMode::Type, AssetSelectionMode)

		/** The event that triggers when an asset has been selected. */
		SLATE_EVENT(FOnAssetSelected, OnAssetSelected)

		/** The event that triggers when an asset has been activated (double clicked, custom buttons to proceed etc.). */
		...
		/** Called when the active section changes. Fired after internal state has been updated. */
		SLATE_EVENT(FOnActiveSectionChanged, OnActiveSectionChanged)

		/** Called when the primary filter tree selection changes. */
		SLATE_EVENT(FOnPrimaryFilterSelectionChanged, OnPrimaryFilterSelectionChanged)
	SLATE_END_ARGS()

	
	UE_API void Construct(const FArguments& InArgs, const UTaggedAssetBrowserConfiguration& InTaggedAssetBrowserConfiguration);
	...
	/** Returns the currently active section, or nullptr if no section is active. */
	UE_API const UTaggedAssetBrowserSection* GetActiveSection() const;

	/** Returns the asset picker widget inside this browser, if it exists */
	UE_API TSharedPtr<SAssetPicker> GetAssetPicker() const;

	/** Returns the currently selected primary filter items. */
	UE_API TArray<const UTaggedAssetBrowserFilterBase*> GetSelectedPrimaryFilters() const;

	/** Returns all primary filter items in the current section, regardless of selection. */
	UE_API TArray<const UTaggedAssetBrowserFilterBase*> GetAllPrimaryFilters() const;
...
private:
	...
	bool NeedsCustomTabArea() const;

	void PopulateSectionsSlot();
	void PopulatePrimaryFiltersSlot();
	void PopulateAssetBrowserSplitterSlot();
...
private:
	...
	FOnOverrideAssetPickerConfig OnOverrideAssetPickerConfigDelegate;
	FOnActiveSectionChanged OnActiveSectionChangedDelegate;
	FOnPrimaryFilterSelectionChanged OnPrimaryFilterSelectionChangedDelegate;
...
private:
	...
	SHorizontalBox::FSlot* AssetBrowserAreaSplitterSlot = nullptr;
...
public:
	...
	STaggedAssetBrowserWindow() {}
	virtual ~STaggedAssetBrowserWindow() override {}
