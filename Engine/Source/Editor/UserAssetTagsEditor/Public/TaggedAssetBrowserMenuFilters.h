...
/** How to resolve extension filter merging */
UENUM()
enum class ETaggedAssetBrowserFilterMergeBehavior
{
	Append, // Add Filter at end
	Insert, // Insert before or after a filter by name
};

/** How to insert a filter */
UENUM()
enum class ETaggedAssetBrowserFilterInsertBehavior
{
	Before,
	After
};

/** Data relevant for extension filters. */
USTRUCT()
struct FTaggedAssetBrowserFilterData_Extension 
{
	GENERATED_BODY()

public:

	/** How this filter should be merged */
	UPROPERTY(EditAnywhere, Category = "Filter")
	ETaggedAssetBrowserFilterMergeBehavior MergeBehavior = ETaggedAssetBrowserFilterMergeBehavior::Append;

	/** How this filter should be inserted */
	UPROPERTY(EditAnywhere, Category = "Filter", meta = (EditCondition = "MergeBehavior==ETaggedAssetBrowserFilterMergeBehavior::Insert", EditConditionHides))
	ETaggedAssetBrowserFilterInsertBehavior InsertBehavior = ETaggedAssetBrowserFilterInsertBehavior::After;

	/** Name for filter this filter should be inserted relative to, assuming this is from / in an extension. */
	UPROPERTY(EditAnywhere, Category = "Filter", meta = (EditCondition = "MergeBehavior==ETaggedAssetBrowserFilterMergeBehavior::Insert", EditConditionHides))
	FName InsertFilterName;
};
...
public:
	...
	/** Data for this filter that is only needed when we are in an extension. */
	UPROPERTY(EditAnywhere, Category = "Filter", meta = (EditCondition = "IsExtensionFilter", EditConditionHides))
	FTaggedAssetBrowserFilterData_Extension ExtensionData;

public:
	/** Check outer for if we are a filter in an extension config */
	UFUNCTION()
	bool IsExtensionFilter() const;

public:	
	/** The filter has a chance to initialize itself with contextual data to actually work. */
	UE_API void Initialize(const FTaggedAssetBrowserContext& InContext);

	UE_API FName GetIdentifier() const;
