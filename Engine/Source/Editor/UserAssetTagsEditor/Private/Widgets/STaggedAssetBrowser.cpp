...
void STaggedAssetBrowser::Construct(const FArguments& InArgs, const UTaggedAssetBrowserConfiguration& InTaggedAssetBrowserConfiguration)
{
	...
	OnOverrideAssetPickerConfigDelegate = InArgs._OnOverrideAssetPickerConfig;
	OnActiveSectionChangedDelegate = InArgs._OnActiveSectionChanged;
	OnPrimaryFilterSelectionChangedDelegate = InArgs._OnPrimaryFilterSelectionChanged;
	...
	[
		...
		[
			...
			[
				...
				.AutoWidth()
				+ SHorizontalBox::Slot().Expose(AssetBrowserAreaSplitterSlot)
			]
			+ SVerticalBox::Slot()
			.AutoHeight()
			[
...
{
	SaveSettings();
	if (UTaggedAssetBrowserConfig::HasInstance())
	{
...
TSharedPtr<SAssetPicker> STaggedAssetBrowser::GetAssetPicker() const
{
	return AssetBrowserContent.IsValid() ? AssetBrowserContent->GetAssetPicker() : nullptr;
}
...
void STaggedAssetBrowser::ApplyFilterExtensions()
{
	...
	{
		TMap<FName, const UHierarchySection*> MergeAllowedSectionMap;
		TMap<FName, const UHierarchySection*> MergeDisabledSectionMap;

		// Build a map of sections already present in the base config so we can merge into them
		TStrongObjectPtr<const UTaggedAssetBrowserConfiguration> Config = Configuration.Pin();
		bool bMergeExtensions = Config && Config->StandaloneData.bMergeExtensionsPerSection;
		if (bMergeExtensions)
		{
			for (const UHierarchySection* ExistingSection : FilterHierarchyRoot->GetSectionData())
			{
				const UTaggedAssetBrowserSection* TaggedAssetBrowserSection = Cast<UTaggedAssetBrowserSection>(ExistingSection);
				if (TaggedAssetBrowserSection && TaggedAssetBrowserSection->bAllowSectionMerge)
				{
					MergeAllowedSectionMap.Add(ExistingSection->GetSectionName(), ExistingSection);
				}
				else
				{
					MergeDisabledSectionMap.Add(ExistingSection->GetSectionName(), ExistingSection);
				}
			}
		}
		...
		for(const UTaggedAssetBrowserConfiguration* ExtensionConfigurationAsset : GetExtensionAssets())
		{
			for(const UHierarchySection* Section : ExtensionConfigurationAsset->FilterRoot->GetSectionData())
			{
				...
				if(!MergeAllowedSectionMap.Contains(SectionName) || !bMergeExtensions)
				{
					// No matching section in the base config — add a new one
					...
					const UTaggedAssetBrowserSection* TaggedAssetBrowserSection = Cast<UTaggedAssetBrowserSection>(CopiedSection);
					if (TaggedAssetBrowserSection && TaggedAssetBrowserSection->bAllowSectionMerge)
					{
						MergeAllowedSectionMap.Add(CopiedSection->GetSectionName(), CopiedSection);
					}
					else
					{
						MergeDisabledSectionMap.Add(CopiedSection->GetSectionName(), CopiedSection);
					}
				}
				else
				{
					// Do nothing. If the section already exists, we will add the elements to it below
				}
			}
			...
			{
				// Manually insert into appropriate location 
				UHierarchyElement* NewChild = Cast<UHierarchyElement>(StaticDuplicateObject(Element, FilterHierarchyRoot.Get()));
				if (NewChild->GetPersistentIdentity() != Element->GetPersistentIdentity())
				{
					check(false);
				}

				bool bFilterInserted = false;
				if (const UTaggedAssetBrowserFilterBase* Filter = Cast<UTaggedAssetBrowserFilterBase>(Element))
				{
					auto IsInsertTargetFilter = [FilterName = Filter->ExtensionData.InsertFilterName](const UHierarchyElement* ExistingElement)
					{
						if (const UTaggedAssetBrowserFilterBase* ExistingFilter = Cast<UTaggedAssetBrowserFilterBase>(ExistingElement))
						{
							return ExistingFilter->ToString() == FilterName.ToString();
						}
					...
					if (bMergeExtensions 
						&& Filter->ExtensionData.MergeBehavior == ETaggedAssetBrowserFilterMergeBehavior::Insert
						&& !Filter->ExtensionData.InsertFilterName.IsNone())
					{
						int32 InsertIndex = FilterHierarchyRoot->GetChildrenMutable().IndexOfByPredicate(IsInsertTargetFilter);

						if (InsertIndex != INDEX_NONE)
						{
							if (Filter->ExtensionData.InsertBehavior == ETaggedAssetBrowserFilterInsertBehavior::After)
							{
								InsertIndex++;
							}
						}
						...
						{
							InsertIndex = FilterHierarchyRoot->GetChildrenMutable().Num();
						}

						FilterHierarchyRoot->GetChildrenMutable().Insert(NewChild, InsertIndex);
						bFilterInserted = true;
					}
				}

				if (!bFilterInserted)
				{
					FilterHierarchyRoot->GetChildrenMutable().Add(NewChild);
				}

				// We also need to fix up section associations
				FDataHierarchyElementMetaData_SectionAssociation* SectionAssociation = NewChild->FindMetaDataOfType<FDataHierarchyElementMetaData_SectionAssociation>();
				if(SectionAssociation && SectionAssociation->Section.IsValid())
				{
					if(MergeAllowedSectionMap.Contains(SectionAssociation->Section->GetSectionName()))
					{
						SectionAssociation->Section = MergeAllowedSectionMap[SectionAssociation->Section->GetSectionName()];
					}
					else if (MergeDisabledSectionMap.Contains(SectionAssociation->Section->GetSectionName()))
					{
						SectionAssociation->Section = MergeDisabledSectionMap[SectionAssociation->Section->GetSectionName()];
					}
					else
					{
...
bool STaggedAssetBrowser::NeedsCustomTabArea() const
{
	return DefaultDetailsTabConfiguration.bUseDefaultDetailsTab || !CustomTabInfos.IsEmpty();
}
...
void STaggedAssetBrowser::PopulateAssetBrowserSplitterSlot()
{
	// @TODO: The splitter logic here is duplicated since SSplitter currently doesn't support 
	// after creation slot modification like horizontal / vertical box do.

	if (NeedsCustomTabArea())
	{
		AssetBrowserAreaSplitterSlot->AttachWidget(SNew(SSplitter)
			.Orientation(Orient_Horizontal)
...
void STaggedAssetBrowser::PopulateAssetBrowserContentSlot()
{
	...
	OnOverrideAssetPickerConfigDelegate.ExecuteIfBound(Config);
...
void STaggedAssetBrowser::InitializeFilters()
{
	...
	FilterContext.OnGetSelectedFilters = FTaggedAssetBrowserContext::FOnGetSelectedFilters::CreateSP(this, &STaggedAssetBrowser::GetSelectedPrimaryFilters);
...
void STaggedAssetBrowser::OnSectionSelected(const UTaggedAssetBrowserSection* InTaggedAssetBrowserSection)
{
	...
	RefreshBackendFilter();
	OnActiveSectionChangedDelegate.ExecuteIfBound(InTaggedAssetBrowserSection);
}
...
TArray<const UTaggedAssetBrowserFilterBase*> STaggedAssetBrowser::GetSelectedPrimaryFilters() const
{
...
TArray<const UTaggedAssetBrowserFilterBase*> STaggedAssetBrowser::GetAllPrimaryFilters() const
{
	TArray<UTaggedAssetBrowserFilterBase*> AllFilters;
	FilterHierarchyRoot->GetChildrenOfType(AllFilters, true);

	// Second array for const
	TArray<const UTaggedAssetBrowserFilterBase*> Result;
	Result.Reserve(AllFilters.Num());
	for (UTaggedAssetBrowserFilterBase* Filter : AllFilters)
	{
		Result.Add(Filter);
	}
...
{	
	...
	SaveSettings();
	OnPrimaryFilterSelectionChangedDelegate.ExecuteIfBound();
}
...
{	
	...
	SAssignNew(AssetBrowser, STaggedAssetBrowser, Configuration)
		...
		.OnAssetsActivated(InArgs._AssetBrowserArgs._OnAssetsActivated)
