...
FString UTaggedAssetBrowserFilter_Directories::ToString() const
{
	...
	{
		const FDirectoryPath& FirstDirectoryPath = DirectoryPaths[0];

		int32 LeafIndex = INDEX_NONE;
		if (FirstDirectoryPath.Path.FindLastChar(TEXT('/'), LeafIndex))
		{
			if (LeafIndex != INDEX_NONE)
			{
				return FirstDirectoryPath.Path.RightChop(LeafIndex + 1);
			}
		}
	}
...
FText UTaggedAssetBrowserFilter_Directories::GetTooltip() const
{
	if (!DirectoryPaths.IsEmpty())
	{
		FText BaseTooltip = LOCTEXT("UTaggedAssetBrowserFilter_DirectoriesTooltip", "Directories: \n {0}.");
		FTextBuilder TooltipListBuilder;

		for (const FDirectoryPath& DirectoryPath : DirectoryPaths)
		{
			TooltipListBuilder.AppendLine(DirectoryPath.Path);
		}

		return FText::FormatOrdered(BaseTooltip, TooltipListBuilder.ToText());
	}
...
FString UTaggedAssetBrowserFilter_Class::ToString() const
{
	if (!FilterName.IsNone())
	{
		return FilterName.ToString();
	...
	if (!Classes.IsEmpty())
	{
		const UClass* FirstClass = Classes[0];
		if (FirstClass)
		{
			return FirstClass->GetDisplayNameText().ToString();
		}
	}
...
FText UTaggedAssetBrowserFilter_Class::GetTooltip() const
{
	if (!Classes.IsEmpty())
	{
		FText BaseTooltip = LOCTEXT("UTaggedAssetBrowserFilter_ClassTooltip", "Classes: \n {0}.");
		FTextBuilder TooltipListBuilder;
		...
		for (const UClass* Class : Classes)
		{
			if (Class)
			{
				TooltipListBuilder.AppendLine(Class->GetDisplayNameText().ToString());
			}
		}
...
FSlateIcon UTaggedAssetBrowserFilter_Class::GetIcon() const
{
	if (!Classes.IsEmpty())
	{
		const UClass* FirstClass = Classes[0];
		if (FirstClass)
		{
			return FSlateIconFinder::FindIconForClass(FirstClass);
		}
	}

	return UTaggedAssetBrowserFilterBase::GetIcon();
}
