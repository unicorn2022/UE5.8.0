...
static FAutoConsoleCommand WorldBookmarkCaptureToStringCommand(
	...
	FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& Args)
	{
		...
		UE_LOGF(LogWorldBookmark, Display, "Restore the bookmark with 'WorldBookmark.Restore' and the following argument:\n%ls", *Str);
	})
);
...
static FAutoConsoleCommand WorldBookmarkCaptureToClipboardCommand(
	...
	FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& Args)
	{
		...
		UE_LOGF(LogWorldBookmark, Display, "WorldBookmark captured to clipboard");
	})
);
	...
	if (Settings->bEnableDefaultBookmarks)
	{
		...
		if (!UEditorStateSubsystem::Get()->IsRestoringEditorState())
		{
			if (IsDefaultBookmarkValid(InWorld->GetWorldSettings()))
			{
				...
				{
					UE_LOGF(LogWorldBookmark, Display, "Loading default bookmark %ls", *WorldBookmark->GetPathName());
					WorldBookmark->Load();
				}
			}
			else
...
void FWorldBookmarkModule::OnEditorLoadDefaultStartupMap(FCanLoadMap& InOutCanLoadDefaultStartupMap)
{
	...
	if (Settings->bEnableHomeBookmark)
	{
		...
		{
			UE_LOGF(LogWorldBookmark, Display, "Loading home bookmark %ls", *Settings->HomeBookmark.ToString());
			HomeBookmark->Load();
...
bool FWorldBookmarkModule::RestoreFromString(const FString& InBookmarkAsString)
{
	...
	{
		UE_LOGF(LogWorldBookmark, Error, "RestoreFromString: Invalid bookmark string");
	...
	{
		UE_LOGF(LogWorldBookmark, Error, "RestoreFromString: Failed to decode base64 bookmark data");
		ShowInvalidBookmarkDataNotification();
		return false;
	}
	...
	{
		UE_LOGF(LogWorldBookmark, Error, "RestoreFromString: Failed to read compressed bookmark data");
		ShowInvalidBookmarkDataNotification();
		return false;
	}
	...
	{
		UE_LOGF(LogWorldBookmark, Error, "RestoreFromString: Failed to uncompress bookmark data");
		ShowInvalidBookmarkDataNotification();
		return false;
	}
	...
	{
		UE_LOGF(LogWorldBookmark, Error, "RestoreFromString: Failed to restore bookmark data");
		ShowInvalidBookmarkDataNotification();
		return false;
	}
