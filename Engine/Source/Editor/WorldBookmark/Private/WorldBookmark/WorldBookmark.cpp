...
void UWorldBookmark::LoadStates(const TArray<TSubclassOf<UEditorState>>& InStatesToLoad)
{
	...
	else
	{
		...
		UE_LOGF(LogWorldBookmark, Error, "Failed to load bookmark: %ls", *CanLoadFailureReason.ToString());
	}
}
...
void UWorldBookmark::UpdateStates(const TArray<TSubclassOf<UEditorState>>& InStatesToUpdate)
{
	...
	else
	{
		...
		UE_LOGF(LogWorldBookmark, Error, "Failed to update bookmark: %ls", *CanUpdateFailureReason.ToString());
	}
}
