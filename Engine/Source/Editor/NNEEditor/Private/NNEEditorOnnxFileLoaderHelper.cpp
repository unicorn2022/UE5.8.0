...
namespace UE::NNEEditor::Internal::OnnxFileLoaderHelper
{
	...
	bool InitUNNEModelDataFromFile(UNNEModelData& ModelData, int64& ModelFileSize, const FString& Filename)
	{
		...
		{
			UE_LOGF(LogNNE, Error, "Failed to load file '%ls'", *Filename);
			return false;
		}
		...
		if (UE::NNEEditor::Private::OnnxModelInspectorHelper::IsSharedLibFunctionPointerSetup())
		{
			...
			{
				UE_LOGF(LogNNE, Warning, "Could not parse the input model as ONNX ModelProto, model external data won't be imported if any.");
			}
		}
		...
		if (!ExternalDataRelativeFilePaths.IsEmpty())
		{
			...
			for (const FString& DataRelativeFilePath : ExternalDataRelativeFilePaths)
			{
				...
				if (FileSize < 0)
				{
					UE_LOGF(LogNNE, Error, "Failed to find file size for external data file '%ls' for Onnx Model '%ls'", *DataFilePath, *Filename);
					return false;
				}
			...
			for (const FString& DataRelativeFilePath : ExternalDataRelativeFilePaths)
			{
				...
				{
					UE_LOGF(LogNNE, Error, "Failed to load external data file '%ls' for Onnx Model '%ls'", *DataFilePath, *Filename);
					return false;
				}

				//Uncomment to trace loading of external onnx data from files.
				//UE_LOGF(LogNNE, Display, "Loaded external data file '%ls' while importing Onnx Model '%ls'.", *DataFilePath, *Filename);
			}
