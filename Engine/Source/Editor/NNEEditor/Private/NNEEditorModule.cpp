...
namespace UE::NNEEditor::Private::OnnxHelper
{
	void* GetSharedLibHandle(const FString& SharedLibPath)
	{
		...
		{
			UE_LOGF(LogNNE, Error, "Failed to find the NNEEditorOnnxTools shared library %ls.", *SharedLibPath);
			return nullptr;
		}
		...
		if (!SharedLibHandle)
		{
			UE_LOGF(LogNNE, Error, "Failed to load the NNEEditorOnnxTools shared library %ls.", *SharedLibPath);
		}
...
#endif
		const FString NNEEditorOnnxToolsSharedLibPath = FPaths::Combine(ModuleDir, TEXT(UE_STRINGIZE(NNEEDITORONNXTOOLS_SHAREDLIB_FILENAME)));
