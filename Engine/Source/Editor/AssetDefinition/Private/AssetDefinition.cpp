...
FAssetCategoryPath EAssetCategoryPaths::Data(LOCTEXT("Data", "Data"));
...
{
	CategoryPath = { TPair<FName, FCategoryPath>(FName(*FTextInspector::GetSourceString(InCategory)), FCategoryPath(InCategory)) };
}

void FAssetCategoryPath::AppendSubMenu(const FCategoryPath& InSubMenu)
{
	CategoryPath.Add(TPair<FName, FCategoryPath>(FName(*FTextInspector::GetSourceString(InSubMenu.GetSubMenuName())), InSubMenu));
}
...
FAssetCategoryPath::FAssetCategoryPath(TConstArrayView<FText> InCategoryPath)
{
	...
	{
		CategoryPath.Add(TPair<FName, FCategoryPath>(FName(*FTextInspector::GetSourceString(CategoryChunk)), FCategoryPath(CategoryChunk)));
	}
}
...
void FAssetCategoryPath::GetSubCategoriesText(TArray<FText>& SubCategories) const
{
	if (HasSubCategory())
	{
		...
		for (int32 i = 1; i < CategoryPath.Num(); i++)
		{
			SubCategories.Add(CategoryPath[i].Value.GetSubMenuName());
		}
	}
}

void FAssetCategoryPath::GetSubCategoriesInfo(TArray<FCategoryPath>& SubCategories) const
{
	if (HasSubCategory())
	{
		SubCategories.Reserve(SubCategories.Num() + NumSubCategories());
