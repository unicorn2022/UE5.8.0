...
void UAssetDefinitionRegistry::RegisterAssetDefinition(UAssetDefinition* AssetDefinition)
{
	...
	if (TSoftClassPtr<UObject> SupportedClass = AssetDefinition->GetAssetClass())
	{
		...
		else
		{
			...
			//UE_LOGF(LogTemp, Warning, "Unable to register %ls for (%ls), %ls is already registered",  *GetPathNameSafe(AssetDefinition), *SupportedClass.ToString(), *GetPathNameSafe(ExistingDefinition));
		}
	}
}
