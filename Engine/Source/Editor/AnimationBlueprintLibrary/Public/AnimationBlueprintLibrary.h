...
public:
	...
	*/
	...
	/** Sets the preview skeletal mesh on a skeleton, optionally marks the skeleton as dirty */
	UFUNCTION(BlueprintCallable, Category=Skeleton, meta = (ScriptMethod))
	static UE_API void SetSkeletonPreviewMesh(USkeleton* Skeleton, USkeletalMesh* PreviewMesh, bool bMarkAsDirty=true);

	/** Returns the preview skeletal mesh on a skeleton, optionally finds one if a preview mesh hasn't been set */
	UFUNCTION(BlueprintCallable, Category=Skeleton, meta = (ScriptMethod))
	static UE_API USkeletalMesh* GetSkeletonPreviewMesh(USkeleton* Skeleton, bool bFindIfNotSet = false);

	/** Sets the additional preview skeletal meshes on a skeleton */
	UFUNCTION(BlueprintCallable, Category=Skeleton, meta = (ScriptMethod))
	static UE_API void SetSkeletonAdditionalPreviewMeshes(USkeleton* Skeleton, UDataAsset* AdditionalMeshes);

	/** Returns the additional preview skeletal meshes on a skeleton */
	UFUNCTION(BlueprintPure, Category=Skeleton, meta = (ScriptMethod))
	static UE_API UDataAsset* GetSkeletonAdditionalPreviewMeshes(const USkeleton* Skeleton);
};
