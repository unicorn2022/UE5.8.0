	...
	{		
		...
		for (const FBoneIndexType BoneIndex : ContextBoneContainer.GetBoneIndicesArray())
		{
			...
			if (SkeletonBoneIndex != INDEX_NONE)
			{
				...
				{
					UE_LOGF(LogAnimationPoseScripting, Warning, "Failed to find local Bone Index for Skeleton Bone Index %i [%ls]", SkeletonBoneIndex, *ContextBoneContainer.GetReferenceSkeleton().GetBoneName(SkeletonBoneIndex).ToString());
				}
			}
			else
			{
				UE_LOGF(LogAnimationPoseScripting, Warning, "Failed to find Skeleton index for Compact Pose Bone Index %i", BoneIndex);
			}
		}
...
void FAnimPose::GenerateWorldSpaceTransforms()
{
	...
	{
		UE_LOGF(LogAnimationPoseScripting, Warning, "Anim Pose was not previously populated");
	}
}
...
void FAnimPose::SetPose(const FAnimationPoseData& PoseData)
{
	...
	if (IsInitialized())
	{
		...
		for (const FCompactPoseBoneIndex BoneIndex : CompactPose.ForEachBoneIndex())
		{
			...
			{
				UE_LOGF(LogAnimationPoseScripting, Warning, "Failed to find Skeleton index for Compact Pose Bone Index %i", BoneIndex.GetInt());
			}
		}
	...
	{
		UE_LOGF(LogAnimationPoseScripting, Warning, "Anim Pose was not previously initialized");
	}
}
...
const FTransform& UAnimPoseExtensions::GetBonePose(const FAnimPose& Pose, FName BoneName, EAnimPoseSpaces Space /*= EAnimPoseSpaces::Local*/)
{
	if (Pose.IsValid())
	{
		...
		{
			UE_LOGF(LogAnimationPoseScripting, Warning, "No bone with name %ls was found", *BoneName.ToString());
		}
	}
	else
	{		
		UE_LOGF(LogAnimationPoseScripting, Error, "Provided Pose is not valid");	
	}
...
FTransform UAnimPoseExtensions::GetRelativeTransform(const FAnimPose& Pose, FName FromBoneName, FName ToBoneName, EAnimPoseSpaces Space /*= EAnimPoseSpaces::Local*/)
{
	if (Pose.IsValid())
	{
		...
		{
			UE_LOGF(LogAnimationPoseScripting, Warning, "No bone with name %ls or %ls was found", *FromBoneName.ToString(), *ToBoneName.ToString());
		}
	}
	else
	{		
...
FTransform UAnimPoseExtensions::GetSocketPose(const FAnimPose& Pose, FName SocketName, EAnimPoseSpaces Space)
{
	if (Pose.IsValid())
	{
		...
		{
			UE_LOGF(LogAnimationPoseScripting, Warning, "No socket with name %ls was found", *SocketName.ToString());
		}
	}
	else
	{		
			...
			if (AnimationBlueprint)
			{
				...
				if (AnimGeneratedClass)
				{
					if (AnimGeneratedClass->TargetSkeleton == TargetSkeletalMesh->GetSkeleton())
					{
						...
						{
							UE_LOGF(LogAnimationPoseScripting, Warning, "Failed to retrieve Input Pose Node from Animation Graph %ls", *AnimationBlueprint->GetName());	
						}
					}
					else
					{
						UE_LOGF(LogAnimationPoseScripting, Error, "Animation Blueprint target Skeleton %ls does not match Target Skeletal Mesh its Skeleton %ls", *AnimGeneratedClass->TargetSkeleton->GetName(), *TargetSkeletalMesh->GetSkeleton()->GetName());	
					}
				
				}
				else
				{
					UE_LOGF(LogAnimationPoseScripting, Warning, "Failed to retrieve Animation Blueprint generated class");	
				}
			}
			else
			{
				UE_LOGF(LogAnimationPoseScripting, Warning, "Invalid Animation Blueprint");			
			}	
		}
		else
		{		
			UE_LOGF(LogAnimationPoseScripting, Warning, "Invalid Target Skeletal Mesh");
		}
	}
	else
	{		
...
void UAnimPoseExtensions::GetReferencePose(USkeleton* Skeleton, FAnimPose& OutPose)
{
	...
	{		
		UE_LOGF(LogAnimationPoseScripting, Error, "Invalid Skeleton provided");	
	}
}
...
void UAnimPoseExtensions::GetAnimPoseAtTimeIntervals(const UAnimSequenceBase* AnimationSequenceBase, TArray<double> TimeIntervals, FAnimPoseEvaluationOptions EvaluationOptions, TArray<FAnimPose>& InOutPoses)
{
	if (AnimationSequenceBase && AnimationSequenceBase->GetSkeleton())
	{
		...
		for (int32 Index = 0; Index < TimeIntervals.Num(); ++Index)
		{
			...
			{
				UE_LOGF(LogAnimationPoseScripting, Warning, "Invalid time value %f for Animation Sequence %ls supplied for GetBonePosesForTime", EvalInterval, *AnimationSequenceBase->GetName());
			}
		}
	}
	else
	{
		UE_LOGF(LogAnimationPoseScripting, Warning, "Invalid Animation Sequence supplied for GetBonePosesForTime");
	}
}
