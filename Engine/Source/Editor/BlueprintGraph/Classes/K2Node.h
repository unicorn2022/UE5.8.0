...
{
public:
	virtual ~FOptionalPinManager() = default;
...
public:
	 ...
	 * This is useful for checking validity of the node for UI list refreshes, 
	 * which may operate on stale nodes for a frame until the list is refreshed.
	 */
	BLUEPRINTGRAPH_API bool HasValidBlueprint() const;

	/** Get the Blueprint object to which this node belongs. This will return null if HasValidBlueprint is false */
	BLUEPRINTGRAPH_API UBlueprint* GetBlueprint() const;

	/** Get the Blueprint object to which this node belongs. This will assert if HasValidBlueprint is false */
	BLUEPRINTGRAPH_API TNotNull<UBlueprint*> GetBlueprintChecked() const;
...
protected:
	...
	void Message_Note(const FString& Message, ArgTypes... Args) const
	{
		...
		{
			UE_LOGF(LogBlueprint, Log, "%ls", *Message);
		}
	}
	...
	void Message_Warn(const FString& Message, ArgTypes... Args) const
	{
		...
		{
			UE_LOGF(LogBlueprint, Warning, "%ls", *Message);
		}
	}
	...
	void Message_Error(const FString& Message, ArgTypes... Args) const
	{
		...
		{
			UE_LOGF(LogBlueprint, Error, "%ls", *Message);
		}
	}
...
private:
	...
	/** A fast pin rewiring function that does not handle split pins or orphaned pins, but safely aborts if any are found */
	bool TryRewireOldPinsToNewPinsFast(TArray<UEdGraphPin*>& InOldPins, TArray<UEdGraphPin*>& InNewPins, TMap<UEdGraphPin*, UEdGraphPin*>* NewPinToOldPin);
public:
