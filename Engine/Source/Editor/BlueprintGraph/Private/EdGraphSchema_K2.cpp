...
namespace UE::EdGraphSchemaK2::Private
{
	...
	UClass* GetAuthoritativeClass(const TProperty& Property)
	{
		...
		{
			UE_LOGF(LogBlueprint, Warning, "'%ls' is a skeleton class. SubCategoryObject will serialize to a null value.", *PropertyClass->GetFullName());
		}
...
FText UEdGraphSchema_K2::FPinTypeTreeInfo::GenerateDescription()
{
	...
	if (UObject* SubCategoryObject = PinType.PinSubCategoryObject.Get(); ensure(SubCategoryObject))
	{
		...
		{
			DisplayName = FText::FromString(FName::NameToDisplayString(SubCategoryObject->GetName(), PinType.PinCategory == PC_Boolean));
		}
	}
...
const UScriptStruct* UEdGraphSchema_K2::Vector2DStruct = nullptr;
	...
	: Super(ObjectInitializer)
{
	...
	if (VectorStruct == nullptr)
	{
		...
		Vector2DStruct = TBaseStructure<FVector2D>::Get();
	...
	if (Property)
	{
		...
		if (bMeta != bFlag)
		{
			...
			UE_LOGF(LogBlueprint, Warning
				, "ExposeOnSpawn ambiguity. Property '%ls', MetaData '%ls', Flag '%ls'"
...
bool UEdGraphSchema_K2::PinDefaultValueIsEditable(const UEdGraphPin& InGraphPin) const
{
	...
	if( InGraphPin.PinType.PinCategory == PC_Struct )
	{
		...
		return &SubCategoryObject == VectorStruct 
			...
			|| &SubCategoryObject == Vector2DStruct
...
#define SCHEMA_K2_AUTOCASTFUNCTIONMAP_LOG_TIME 0
		...
		UE_LOGF(LogBlueprint, Warning, "FAutocastFunctionMap::Refresh took %fs", EndTime - StartTime);
#endif //SCHEMA_K2_AUTOCASTFUNCTIONMAP_LOG_TIME
		...
		else if (Output.PinCategory == PC_Interface)
		{
				...
				|| !InputClass->IsChildOf(UInterface::StaticClass()))
			{
				UE_LOGF(LogBlueprint, Error,
					"UEdGraphSchema_K2::ArePinTypesCompatible invalid interface types - OutputClass: %ls, InputClass: %ls, CallingContext: %ls",
				...
				if (UK2Node_CallFunction* CallFunctionNode = Cast<UK2Node_CallFunction>(Pin->GetOwningNode()))
				{
					if (UFunction* FunctionRef = CallFunctionNode->GetTargetFunction())
					{
						...
						if (HideAssetPickerMetaData.IsEmpty())
						{
							...
							if (!HideAssetPickerMetaData.IsEmpty())
							{
								UE_LOGF(LogBlueprint, Warning, "Use of deprecated meta 'HideAssetPicker' for function %ls. Change to HidePinAssetPicker.", *FunctionRef->GetPathName());
							}
						}
...
bool UEdGraphSchema_K2::FindFunctionParameterDefaultValue(const UFunction* Function, const FProperty* Param, FString& OutString)
{
	...
	if (!MetadataDefaultValue.IsEmpty())
	{
		...
		{
			UE_LOGF(LogBlueprint, Warning, "Short class name \"%ls\" in meta data \"%ls\" for function %ls", *OutString, *Param->GetName(), *Function->GetPathName());
...
)
{
		...
		!AutogeneratedDefaultValue.IsEmpty())
	{
		...
		if (!PackageNamespace.IsEmpty())
		{
			...
			// Does auto-generated default text have a mismatched package ID?
			if (!AutogeneratedDefaultTextValue.IsEmpty() && !AutogeneratedDefaultTextValue.IsCultureInvariant() && !AutogeneratedDefaultTextValue.IsFromStringTable())
			{
				if (const FString CurrentPackageNamespace = TextNamespaceUtil::ExtractPackageNamespace(FTextInspector::GetNamespace(AutogeneratedDefaultTextValue).Get(FString()));
					!CurrentPackageNamespace.IsEmpty() && !CurrentPackageNamespace.Equals(PackageNamespace, ESearchCase::CaseSensitive))
				{
					// Conform the auto-generated default against this package ID, preserving its key to avoid determinism issues
					AutogeneratedDefaultTextValue = TextNamespaceUtil::CopyTextToPackage(AutogeneratedDefaultTextValue, PackageNamespace, TextNamespaceUtil::ETextCopyMethod::PreserveKey);
					// Fix-up the auto-generated default from the conformed value
					AutogeneratedDefaultValue.Reset();
					FTextStringHelper::WriteToBuffer(AutogeneratedDefaultValue, AutogeneratedDefaultTextValue);
				}
			}
		}
	}
...
void UEdGraphSchema_K2::DroppedAssetsOnGraph(const TArray<FAssetData>& Assets, const FVector2f& GraphPosition, UEdGraph* Graph) const 
{
	...
	if ((Blueprint != nullptr) && FBlueprintEditorUtils::IsActorBased(Blueprint))
	{
		...
		for(int32 AssetIdx=0; AssetIdx < Assets.Num(); AssetIdx++)
		{
			...
			if (!Asset)
			{
				UE_LOGF(LogBlueprint, Warning, "Failed to load Asset '%ls'.", *Assets[AssetIdx].GetFullName());
				continue;
			}
			...
			if (DestinationComponentType != nullptr)
			{
				...
				{
					UE_LOGF(LogBlueprint, Warning, "Component Node could not be created for dropped Asset '%ls'.", *Asset->GetName());
				}
			}
		}
	}
...
struct FBackwardCompatibilityConversionHelper
{
		...
		bool bOnlyWithDefaultBlueprint)
	{
		...
		if (!Graph)
		{
			UE_LOGF(LogBlueprint, Warning, "BackwardCompatibilityNodeConversion Error bp: '%ls' node: '%ls'. No graph containing the node.",
		...
		if (!OldBlueprintPin)
		{
			UE_LOGF(LogBlueprint, Warning, "BackwardCompatibilityNodeConversion Error bp: '%ls' node: '%ls'. No bp pin found '%ls'",
				Blueprint ? *Blueprint->GetName() : TEXT("Unknown"),
				*OldNode->GetName(),
				*BlueprintPinName);
			return false;
		...
		if (bTryConvert)
		{
			...
			if (!ClassPin)
			{
				UE_LOGF(LogBlueprint, Warning, "BackwardCompatibilityNodeConversion Error bp: '%ls' node: '%ls'. No class pin found '%ls'",
					Blueprint ? *Blueprint->GetName() : TEXT("Unknown"),
					*NewNode->GetName(),
					*ClassPinName);
				return false;
			...
			if (!TargetClass)
			{
				UE_LOGF(LogBlueprint, Warning, "BackwardCompatibilityNodeConversion Error bp: '%ls' node: '%ls'. No class found '%ls'",
					Blueprint ? *Blueprint->GetName() : TEXT("Unknown"),
					*NewNode->GetName(),
					*ClassPinName);
				return false;
			...
			if (!bNondefaultBPConnected)
			{
				...
				if (ClassPin->DefaultObject != UsedClass)
				{
					...
					UE_LOGF(LogBlueprint, Warning, "BackwardCompatibilityNodeConversion Error 'cannot set class' in blueprint: %ls node: '%ls' actor bp: %ls, reason: %ls",
						Blueprint ? *Blueprint->GetName() : TEXT("Unknown"),
						*OldNode->GetName(),
			...
			else
			{
				...
				{
					UE_LOGF(LogBlueprint, Warning, "BackwardCompatibilityNodeConversion Error 'cannot connect' in blueprint: %ls, pin: %ls",
						Blueprint ? *Blueprint->GetName() : TEXT("Unknown"),
			...
			for (UEdGraphPin* Pin : NewNode->Pins)
			{
				...
				if (ClassPin != Pin)
				{
					...
					{
						UE_LOGF(LogBlueprint, Warning, "BackwardCompatibilityNodeConversion Error 'missing old pin' in blueprint: %ls, pin: %ls",
							Blueprint ? *Blueprint->GetName() : TEXT("Unknown"),
			...
			for (UEdGraphPin* Pin : OldNode->Pins)
			{
				...
				{
					UE_LOGF(LogBlueprint, Warning, "BackwardCompatibilityNodeConversion Error 'missing new pin' in blueprint: %ls, pin: %ls",
						Blueprint ? *Blueprint->GetName() : TEXT("Unknown"),
						Pin ? *Pin->PinName.ToString() : TEXT("Unknown"));
				}
			}
...
bool UEdGraphSchema_K2::ReplaceOldNodeWithNew(UEdGraphNode* OldNode, UEdGraphNode* NewNode, const TMap<FName, FName>& OldPinToNewPinMap) const
{
	...
	for (int32 PinIdx = 0; PinIdx < OldNode->Pins.Num(); ++PinIdx)
	{
		...
		if (NewPin == nullptr)
		{
			...
			UE_LOGF(LogBlueprint, Warning, "BackwardCompatibilityNodeConversion Error 'cannot find pin %ls in node %ls' in: %ls",
	...
	if (!bFailedToFindPin)
	{
		for (int32 PinIdx = 0; PinIdx < OldNode->Pins.Num(); ++PinIdx)
		{
			...
			{
				UE_LOGF(LogBlueprint, Warning, "BackwardCompatibilityNodeConversion Error 'cannot safely move pin %ls to %ls' in: %ls",
					*OldPin->PinName.ToString(),
...
void UEdGraphSchema_K2::BackwardCompatibilityNodeConversion(UEdGraph* Graph, bool bOnlySafeChanges) const 
{
	if (Graph)
	{
...

		{
			...
			{
				UE_LOGF(LogBlueprint, Log, "BackwardCompatibilityNodeConversion: Blueprint '%ls' cannot be fully converted. It has no skeleton class!",
					Blueprint ? *Blueprint->GetName() : TEXT("Unknown"));
			}
		}
...
UEdGraphNode* UEdGraphSchema_K2::CreateSubstituteNode(UEdGraphNode* Node, const UEdGraph* Graph, FObjectInstancingGraph* InstanceGraph, TSet<FName>& InOutExtraNames) const
{
	...
	if(EventNode)
	{
		...
		if(Blueprint && Blueprint->SkeletonGeneratedClass)
		{
			...
			const ERenameFlags RenameFlags = (Blueprint->bIsRegeneratingOnLoad ? REN_AllowPackageLinkerMismatch : 0);
...
/**
 * Attempts to best-guess the width of the node. This is necessary because we don't know the actual
 * size of the node until the next Slate tick
 *
 * @param Node The node to guess the width of
 * @return The estimated width of the specified node
 */
float UEdGraphSchema_K2::EstimateNodeWidth( const UEdGraphNode* Node )
{
...
UEdGraphPin* UEdGraphSchema_K2::DropPinOnNode(UEdGraphNode* InTargetNode, const FName& InSourcePinName, const FEdGraphPinType& InSourcePinType, EEdGraphPinDirection InSourcePinDirection) const
{
	...
	if (UK2Node_EditablePinBase* EditablePinNode = Cast<UK2Node_EditablePinBase>(InTargetNode))
	{
		...
		FEdGraphPinType SourcePinType = InSourcePinType;

		// When a user creates a new float pin, we need to ensure that it's a double.
		// Otherwise, we're unintentionally creating a loophole where users can create single-precision floats.
		if (SourcePinType.PinCategory == PC_Real)
		{
			SourcePinType.PinSubCategory = PC_Double;
		}
		...
		for (UK2Node_EditablePinBase* CurrentEditablePinNode : EditablePinNodes)
		{
			...
			UEdGraphPin* CreatedPin = CurrentEditablePinNode->CreateUserDefinedPin(InSourcePinName, SourcePinType, (InSourcePinDirection == EGPD_Input) ? EGPD_Output : EGPD_Input);
...
float UEdGraphSchema_K2::GetActionFilteredWeight(const FEdGraphSchemaAction& InCurrentAction, const TArray<FString>& InFilterTerms, const TArray<FString>& InSanitizedFilterTerms, const TArray<UEdGraphPin*>& DraggedFromPins) const
{
	...
	// For every filter item the user has typed in (the text in the search bar, separated by spaces)
