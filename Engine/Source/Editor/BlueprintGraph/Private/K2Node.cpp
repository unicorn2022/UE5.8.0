		...
		if (bNeedsConversionNode && !bHasMultipleSelfsConnections)
		{
			if (K2Schema->CreateAutomaticConversionNodeAndConnections(OutputPin, InputPin))
			{
				...
				else
				{
					...
					UE_LOGF(LogBlueprint, Verbose, "Missing compiler relevant pin '%ls' on node '%ls'", *OutputPin->GetName(), (OutputPinOwner ? *OutputPinOwner->GetFullName() : TEXT("<none>")));
				}
			}
		}
	}
...
bool UK2Node::TryRewireOldPinsToNewPinsFast(TArray<UEdGraphPin*>& InOldPins, TArray<UEdGraphPin*>& InNewPins, TMap<UEdGraphPin*, UEdGraphPin*>* NewPinToOldPin)
{
	if(InOldPins.Num() != InNewPins.Num() )
	{
		return false;
	}
	...
	{
		if(Pin->ParentPin || Pin->bOrphanedPin)
		{
			return false;
		}
	}
	
	const int32 NumPins = InOldPins.Num();
	for(int32 Idx = 0; Idx < NumPins; ++Idx)
	{
		UEdGraphPin* OldPin = InOldPins[Idx];
		UEdGraphPin* NewPin = InNewPins[Idx];
		// don't trust anything except the base implementation - customizations
		// may do very relaxed matching (e.g. K2Node_MakeStruct):
		ERedirectType RedirectType = UK2Node::DoPinsMatchForReconstruction(NewPin, Idx, OldPin, Idx);
	...
	// We are commmitted, preconditions validated, naive, fast rewiring can be performed:

	// check for mutation buried in DoPinsMatchForReconstruction:
	checkSlow(InOldPins.Num() == InNewPins.Num() && InOldPins.Num() == NumPins);
	// all pins match, rewire them:
	for(int32 Idx = 0; Idx < NumPins; ++Idx)
	{
		UEdGraphPin* OldPin = InOldPins[Idx];
		UEdGraphPin* NewPin = InNewPins[Idx];
		ReconstructSinglePin(NewPin, OldPin, ERedirectType_Name);
		checkSlow(InOldPins.Num() == InNewPins.Num() && InOldPins.Num() == NumPins);
		if (NewPinToOldPin)
		{
...
{
	if(TryRewireOldPinsToNewPinsFast(InOldPins, InNewPins, NewPinToOldPin))
	{
		return;
	}
	...
	//       (after their parents) 
	const auto FindNewPin = [&](UEdGraphPin* const OldPin, const int32 OldPinIndex, TFunctionRef<ERedirectType(UEdGraphPin*, int32)> Matcher) -> bool
	{
		bool bMatched = false;
		const int32 NumNewPins = InNewPins.Num();
		for (int32 NewPinIndex = NumNewPins - 1; NewPinIndex >= 0; --NewPinIndex)
		{
			UEdGraphPin* NewPin = InNewPins[NewPinIndex];

			ERedirectType RedirectType = Matcher(NewPin, NewPinIndex);
	...
	for (int32 OldPinIndex = InOldPins.Num()-1; OldPinIndex >= 0; --OldPinIndex)
	{
		...
		bool bMatched = FindNewPin(OldPin, OldPinIndex, [this, OldPin, OldPinIndex](UEdGraphPin* NewPin, int32 NewPinIndex)
		{
		...
		if(!bMatched)
		{
			// do proper vtable dispatch of DoPinsMatchForReconstruction in a second pass:
			bMatched = FindNewPin(OldPin, OldPinIndex, [this, OldPin, OldPinIndex](UEdGraphPin* NewPin, int32 NewPinIndex)
				{
					return DoPinsMatchForReconstruction(NewPin, NewPinIndex, OldPin, OldPinIndex);
				});
		}
...
{
	// Perform an unchecked search here, so we don't crash if this is a transient node without a valid outer blueprint
	return (FBlueprintEditorUtils::FindBlueprintForNode(this) != nullptr);
}

UBlueprint* UK2Node::GetBlueprint() const
{
	return FBlueprintEditorUtils::FindBlueprintForNode(this);
}

TNotNull<UBlueprint*> UK2Node::GetBlueprintChecked() const
{
