...
UFunction* UK2Node_BaseAsyncTask::GetFactoryFunction() const
{
	...
	{
		UE_LOGF(LogBlueprint, Error, "ProxyFactoryClass null in %ls. Was a class deleted or saved on a non promoted build?", *GetFullName());
		return nullptr;
	}
	...
	if (FactoryFunction == nullptr)
	{
		...
		UE_CLOGF(FactoryFunction == nullptr, LogBlueprint, Error, "FactoryFunction %ls null in %ls. Was a class deleted or saved on a non promoted build?", *ProxyFactoryFunctionName.ToString(), *GetFullName());
	}
