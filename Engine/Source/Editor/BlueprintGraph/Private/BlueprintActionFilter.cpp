...
static bool BlueprintActionFilterImpl::IsGloballyAccessible(FFieldVariant Field)
{
	...
	// todo: Consider using !HasAnyFlags(CPF_NativeAccessSpecifierProtected|CPF_NativeAccessSpecifierPrivate) for non-UObject fields
	// (i.e. FProperties) to match the documentation of this function.
	bool bIsPublic = Field.IsUObject() ? Field.ToUObject()->HasAnyFlags(RF_Public) : true; 
	
	UClass* ClassOuter = Cast<UClass>(Field.IsUObject() ? Field.ToUObject()->GetOuter() : Field.ToField()->GetOwner<UObject>());
	// For UObjects like global enums, structs, etc., the outer may be a UPackage, treat these a globally accessible.
	// For FProperties the outer may be a UClass or a UStruct, and we are currently
	// treating all UStruct properties (including private and non-static ones) as globally accessible.
