				...
				if(bOverrideFunction)
				{
					...
					if(!bDisallowPaste)
					{
						...
						if(!bDisallowPaste)
						{
							...
							if(bDisallowPaste)
							{
								UE_LOGF(LogBlueprint, Log, "Cannot paste event node (%ls) directly because the event signature class (%ls) is incompatible with this Blueprint.", *GetFName().ToString(), EventReference.GetMemberParentClass(GetBlueprintClassFromNode()) ? *EventReference.GetMemberParentClass(GetBlueprintClassFromNode())->GetFName().ToString() : TEXT("NONE"));
							}
						}
						else
						{
							UE_LOGF(LogBlueprint, Log, "Cannot paste event node (%ls) directly because the event function (%ls) is already handled.", *GetFName().ToString(), *EventReference.GetMemberName().ToString());
						}
					...
					{
						UE_LOGF(LogBlueprint, Log, "Cannot paste event node (%ls) directly because the event function (%ls) is hidden by the Blueprint parent class (%ls).", *GetFName().ToString(), *EventReference.GetMemberName().ToString(), EventReference.GetMemberParentClass(GetBlueprintClassFromNode()) ? *EventReference.GetMemberParentClass(GetBlueprintClassFromNode())->GetFName().ToString() : TEXT("NONE"));
					}
				}
				else if(CustomFunctionName != NAME_None)
				{
					...
					if(!bDisallowPaste)
					{
						...
						if(EventReference.GetMemberName() != NAME_None)
						{
							...
							if(bDisallowPaste)
							{
								UE_LOGF(LogBlueprint, Log, "Cannot paste event node (%ls) directly because the custom event function (%ls) with event signature name (%ls) has an event signature class (%ls) that is incompatible with this Blueprint.", *GetFName().ToString(), *CustomFunctionName.ToString(), *EventReference.GetMemberName().ToString(), EventReference.GetMemberParentClass(GetBlueprintClassFromNode()) ? *EventReference.GetMemberParentClass(GetBlueprintClassFromNode())->GetFName().ToString() : TEXT("NONE"));
							}
						}
					}
					else
					{
						UE_LOGF(LogBlueprint, Log, "Cannot paste event node (%ls) directly because the custom event function (%ls) is already handled.", *GetFName().ToString(), *CustomFunctionName.ToString());
					}
				}
				else
				{
					UE_LOGF(LogBlueprint, Log, "Cannot paste event node (%ls) directly because the event configuration is not specifically handled (EventSignatureName=%ls, EventSignatureClass=%ls).", *GetFName().ToString(), *EventReference.GetMemberName().ToString(), EventReference.GetMemberParentClass(GetBlueprintClassFromNode()) ? *EventReference.GetMemberParentClass(GetBlueprintClassFromNode())->GetFName().ToString() : TEXT("NONE"));
				}
			}
			else
			{
				UE_LOGF(LogBlueprint, Log, "Cannot paste event node (%ls) directly because it is flagged as an internal event.", *GetFName().ToString());
			}
		}
	}
