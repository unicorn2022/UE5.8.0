	...
	if(!BlueprintActionDatabaseImpl::PendingModules.IsEmpty())
	{
		...
		for (const FName& LoadedModule : BlueprintActionDatabaseImpl::PendingModules)
		{
			...
			if (const UPackage* ModuleScriptPackage = FindPackage(nullptr, *ModuleScriptPackageName.ToString()))
			{
				...
				GetObjectsWithPackage(ModuleScriptPackage, ObjectsToProcess, EGetObjectsFlags::None, RF_ClassDefaultObject);
...
void FBlueprintActionDatabase::RefreshClassActions(UClass* const Class)
{
	...
	// transient non native classes can't be persistently referenced - carve out for blueprint classes is for skeleton classes
	// which are transient but serve as a proxy for the persistent generated class:
	bool const bIsTransientClass = Class->HasAnyFlags(RF_Transient) && !Class->IsNative() && !bIsBlueprintClass;

	if (bOutOfDateClass || bIsLevelScript || bHiddenClass || bIsTransientClass)
	{
...
bool FBlueprintActionDatabase::IsFunctionAllowed(UFunction const* InFunction, EPermissionsContext InContext)
{
	...
	if (UClass* Class = InFunction->GetOuterUClass())
	{
		...
		{
			UE_LOGF(LogBlueprint, Warning, "Function %ls was filtered because its class (%ls) was filtered", *InFunction->GetPathName(), *Class->GetName());
			return false;
		}
	}
		...
		for (TFieldIterator<FProperty> PropertyIt(InFunction); PropertyIt; ++PropertyIt)
		{
			...
			if (FStructProperty* StructProperty = CastField<FStructProperty>(Property))
			{
				if (StructProperty->Struct)
				{
					...
					{
						UE_LOGF(LogBlueprint, Warning, "Function %ls was filtered because one of its parameters %ls (struct %ls) was filtered", *InFunction->GetPathName(), *Property->GetName(), *StructProperty->Struct->GetPathName());
						return false;
					}
				}
			}
			else if (FEnumProperty* EnumProperty = CastField<FEnumProperty>(Property))
			{
				if (UEnum* Enum = EnumProperty->GetEnum())
				{
					...
					{
						UE_LOGF(LogBlueprint, Warning, "Function %ls was filtered because one of its parameters %ls (enum %ls) was filtered", *InFunction->GetPathName(), *Property->GetName(), *Enum->GetPathName());
						return false;
					}
				}
			}
			else
			{
				...
				if(GetDefault<UEdGraphSchema_K2>()->ConvertPropertyToPinType(Property, PinType))
				{
					...
					{
						UE_LOGF(LogBlueprint, Warning, "Function %ls was filtered because one of its parameters %ls (%ls) was filtered", *InFunction->GetPathName(), *Property->GetName(), *Property->GetCPPType());
						return false;
					}
				}
				else
				{
					UE_LOGF(LogBlueprint, Warning, "Function %ls was filtered because one of its parameters %ls (%ls) was not able to be converted to a pin", *InFunction->GetPathName(), *Property->GetName(), *Property->GetCPPType());
					return false;
				}
			}
		}
