...
void UK2Node_PromotableOperator::ConvertComparisonOperatorNode(UEdGraphNode* Node, const FName NewOpName)
{
	...
	if (const UFunction* BestMatchingFunc = FTypePromotion::FindBestMatchingFunc(NewOpName, PinsToConsider))
	{
		...
		UE_LOGF(LogBlueprint, Verbose, "Converting node '%ls' from '%ls' to '%ls'...", *GetNameSafe(OpNode), *OpNode->OperationName.ToString(), *NewOpName.ToString());
	...
	{
		UE_LOGF(LogBlueprint, Error, "Failed to convert Convert node from '%ls' to '%ls'", *OpNode->OperationName.ToString(), *NewOpName.ToString());
	}
}
...
void UK2Node_PromotableOperator::ExpandNode(FKismetCompilerContext& CompilerContext, UEdGraph* SourceGraph)
{
	...
	if (!bValidOpName)
	{
		UE_LOGF(LogBlueprint, Error, "Could not find matching operation name for this function!");
	...
	if (!OpFunction)
	{
		UE_LOGF(LogBlueprint, Error, "Could not find matching op function during expansion!");
