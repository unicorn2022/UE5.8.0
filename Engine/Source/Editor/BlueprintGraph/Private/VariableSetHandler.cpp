...
void FKCHandler_VariableSet::Transform(FKismetFunctionContext& Context, UEdGraphNode* Node)
{
	...
	const bool bIsLocalScope = SetNode->VariableReference.IsLocalScope();

	// Local function properties can't be used in replication.
	// Additionally, if HasFieldNotificationBroadcast is true, then the net code broadcast will be executed in native code.
	if (!bIsLocalScope && !SetNode->HasFieldNotificationBroadcast())
	{
		if (SetNode->ShouldFlushDormancyOnSet())
		{
			// Create CallFuncNode
		...
		{
			UK2Node_CallFunction* CallFuncNode = Node->GetGraph()->CreateIntermediateNode<UK2Node_CallFunction>();
			CallFuncNode->FunctionReference.SetExternalMember(SetNode->GetRepNotifyName(), SetNode->GetVariableSourceClass());
			CallFuncNode->AllocateDefaultPins();
		...
		if (SetNode->IsNetProperty())
		{
			 ...
			 */
			if (FProperty* Property = SetNode->GetPropertyForVariable())
			{
