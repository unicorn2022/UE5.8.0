				...
				.OnClicked_Lambda([WeakObject = ObjectsBeingCustomized[0]]()
				{
					if (UTexture2D* Texture = Cast<UTexture2D>(WeakObject.Get()))
					{
						if (!Texture->Source.AreAllBlocksPowerOfTwo() && Texture->PowerOfTwoMode == ETexturePowerOfTwoSetting::None)
						{
							FMessageDialog::Open(
								EAppMsgType::Ok,
								LOCTEXT("Dialog_ConvertTexturePow2_Msg", "Failed to convert a non Power of Two sized texture.\nSet a valid Padding and Resizing mode."),
								LOCTEXT("Dialog_ConvertTexture_Title", "Convert Texture"));
						}
						else
						{
