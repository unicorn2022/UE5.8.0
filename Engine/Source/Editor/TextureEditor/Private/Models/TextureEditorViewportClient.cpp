...
void FTextureEditorViewportClient::Draw(FViewport* Viewport, FCanvas* Canvas)
{
	...
	double ReportingLineHeight = 0;
	{
		const UTextureEditorSettings* TextureEditorSettings = GetDefault<UTextureEditorSettings>();
		const TSharedPtr<FSlateFontCache> FontCache = FEngineFontServices::Get().GetFontCache();
		if (TextureEditorSettings && FontCache)
		{
			FontScale = FMath::Max(0.1f, TextureEditorSettings->ViewportTextScale);
			ReportingLineHeight = FontCache->GetCharacterList(FontInfo, FontScale).GetMaxHeight();
		}
		else
		{
			ReportingLineHeight = FMath::CeilToInt(ReportingFont->GetMaxCharHeight());
		}
		ReportingLineHeight += ReportingLineSpacing;
	}
	
	auto DrawText = [&FontInfo, &Canvas, &FontScale](const double LineX, const double LineY, const FText& InText, const FLinearColor& InColor)->FVector2D
		{
	...
	ON_SCOPE_EXIT {
		...
		{
			ReportingLineY += DrawText(ReportingLineX, ReportingLineY, FText::FromString(ReportedLine.Value), ReportedLine.Key ? FLinearColor::Red : FLinearColor::Yellow).Y;
	...
	if ( Texture->IsCompiling() )
	{
		...
		DrawText(ReportingLineX, ReportingLineY, Message, FLinearColor::White);
	...
	if (Texture->DeferCompression)
	{
		...
		DrawText(ReportingLineX, ReportingLineY, Message, FLinearColor::Yellow);
		ReportingLineY += ReportingLineHeight;
	}
	else
	{
			...
			PlatformDataPtr[0]->ResultMetadata.bIsValid)
		{
			...
			if (PlatformDataPtr[0]->ResultMetadata.bWasEditorCustomEncoding)
			{
				...
				DrawText(ReportingLineX, ReportingLineY, LeadInText, FLinearColor::Yellow);
				ReportingLineY += ReportingLineHeight;
			}
				...
				PlatformDataPtr[0]->ResultMetadata.EncodeSpeed != (uint8)ETextureEncodeSpeed::Final)
			{
				...
				double CurrentX = ReportingLineX;

				auto DrawComma = [&DrawText, &CurrentX, &ReportingLineY]()
				{
					CurrentX += DrawText(CurrentX, ReportingLineY, FText::FromString(TEXT(", ")), FLinearColor::Yellow).X;
				};

				const FText LeadInText = NSLOCTEXT("TextureEditor", "EncodingDifference", "Viewing non-shipping encoding, differences are: ");
				CurrentX += DrawText(CurrentX, ReportingLineY, LeadInText, FLinearColor::Yellow).X;

				const FText HelpText = NSLOCTEXT("TextureEditor", "ShowFinal", "Check \"Editor Show Final Encoding\" to see shipping encoding.");
				DrawText(ReportingLineX, ReportingLineY + ReportingLineHeight, HelpText, FLinearColor::Yellow);
				...
				if (EncodeSettings.Project.bFastUsesRDO != EncodeSettings.Project.bFinalUsesRDO)
				{
					...
					CurrentX += DrawText(CurrentX, ReportingLineY, RDOText, FLinearColor::Yellow).X;
					bNeedComma = true;
				}
				else if (EncodeSettings.Project.bFinalUsesRDO)
				{
					...
					if (EncodeSettings.Project.FastUniversalTiling != EncodeSettings.Project.FinalUniversalTiling)
					{
						...
						CurrentX += DrawText(CurrentX, ReportingLineY, EffortText, FLinearColor::Yellow).X;
						bNeedComma = true;
					}
				}
...
bool FTextureEditorViewportClient::InputGesture(FViewport* Viewport, const FInputDeviceId DeviceId, EGestureEvent GestureType, EGesturePhase GesturePhase, const FVector2D& GestureDelta, bool bIsDirectionInvertedFromDevice, const uint64 Timestamp)
{
