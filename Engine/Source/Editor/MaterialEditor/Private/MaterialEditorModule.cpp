...
public:
	...
	virtual void StartupModule() override
	{
		...
		// Register a callback to bulk update materials when many materials are edited and saved via the property matrix.
		OnPropertyMatrixAssetsSavedDelegateHandle = PropertyModule.OnPropertyMatrixAssetsSaved().AddLambda([](const TArray<UObject*>& SavedObjects)
		{
			TArray<UMaterial*> Materials;
			Materials.Reserve(SavedObjects.Num());
			...
			UMaterialEditingLibrary::RecompileMaterials(Materials, UMaterialEditingLibrary::FOnItemComplete::CreateLambda([]() {}));
		});
	...
	virtual void ShutdownModule() override
	{
		...
		if (FModuleManager::Get().IsModuleLoaded("PropertyEditor"))
		{
			...
			PropertyModule.UnregisterCustomClassLayout("MaterialExpressionOperator");
			PropertyModule.OnPropertyMatrixAssetsSaved().Remove(OnPropertyMatrixAssetsSavedDelegateHandle);
		}
...
private:
	...
	FDelegateHandle OnPropertyMatrixAssetsSavedDelegateHandle;
};
