...
void MaterialExpressionClasses::InitMaterialExpressionClasses()
{
	if(!bInitialized)
	{
		...
		UMaterialEditorOptions* EditorOptions = UMaterialEditorOptions::Get();
				...
				if( !Class->HasAnyClassFlags(CLASS_Abstract | CLASS_Deprecated) )
				{
					if( Class->IsChildOf(UMaterialExpression::StaticClass()) )
					{
							...
							&& Class != UMaterialExpressionNamedRerouteUsage::StaticClass() )
						{
							...
							for (int32 FavoriteIndex = 0; FavoriteIndex < EditorOptions->FavoriteExpressions.Num(); FavoriteIndex++)
							{
								if (Class->GetName() == EditorOptions->FavoriteExpressions[FavoriteIndex])
								{
