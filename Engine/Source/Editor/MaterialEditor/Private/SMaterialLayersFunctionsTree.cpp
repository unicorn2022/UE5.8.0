...
// PROPERTY ----------------------------------------------
	if (StackParameterData->StackDataType == EStackDataType::Property)
	{
		...
		ResetWidget = SNew(SResetToDefaultPropertyEditor, PropertyHandle)
			...
			.CustomResetToDefault(FMaterialPropertyHelpers::CreateResetToDefaultOverride(StackParameterData->Parameter, MaterialEditorInstance, PropertyHandle));
...
void SMaterialLayersFunctionsInstanceWrapper::Refresh()
{
	...
	const bool bHasLayers = HasLayers();
...
bool SMaterialLayersFunctionsInstanceWrapper::HasLayers() const
{
