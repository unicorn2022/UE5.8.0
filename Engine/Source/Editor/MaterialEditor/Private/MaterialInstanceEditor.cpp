...
static TAutoConsoleVariable<bool> CVarEnableTextureValidationInMaterialInstanceEditor(
	...
	true,
	TEXT("Enables expensive texture validation in the material instance editor.  On by default.  Disable this to improve performance when changing texture parameters on large materials."),
	ECVF_Default
);
...
void FMaterialInstanceEditor::InitMaterialInstanceEditor( const EToolkitMode::Type Mode, const TSharedPtr< class IToolkitHost >& InitToolkitHost, UObject* ObjectToEdit )
{
	...
	if ( !SetPreviewAssetByName( *InstanceConstant->PreviewMesh.ToString() ) )
	{
		...
		{
			// Fallback on the default material editor preview mesh
...
void FMaterialInstanceEditor::OnApply()
{
	...
	{
		UE_LOGF(LogMaterialInstanceEditor, Log, "Applying instance %ls", *GetEditingObjects()[0]->GetName());
		...
		[
			...
			[
				...
				[
					...
					.AutoWidth()
					[
						SAssignNew(NumShadersPillText, STextBlock)
					]
				]
			]
		];
...
{
	return MaterialEditorInstance->PreviewInstance ? MaterialEditorInstance->PreviewInstance : MaterialEditorInstance->SourceInstance;
}
...
void FMaterialInstanceEditor::NotifyPostChange( const FPropertyChangedEvent& PropertyChangedEvent, FProperty* PropertyThatChanged)
{
	...
	if(PropertyClass && (PropertyClass == UDEditorStaticSwitchParameterValue::StaticClass() || 
		PropertyClass == FEditorParameterGroup::StaticStruct() || 
		PropertyClass == FMaterialInstanceBasePropertyOverrides::StaticStruct())
	...
	if (PropertyClass && 
	   (PropertyClass == UDEditorTextureParameterValue::StaticClass() ||
		PropertyClass == UDEditorRuntimeVirtualTextureParameterValue::StaticClass()||
		PropertyClass == UDEditorSparseVolumeTextureParameterValue::StaticClass()))
	{
		RefreshOnScreenTextureWarnings();
	}
	
	const bool bRefreshStats = PropertyClass && (
		PropertyClass == UDEditorStaticSwitchParameterValue::StaticClass() ||
		PropertyClass == FMaterialInstanceBasePropertyOverrides::StaticStruct()
		);
...
void FMaterialInstanceEditor::RefreshPreviewAsset()
{
	...
	if (!PreviewAsset)
	{
		// Attempt to use the parent material's preview mesh if the instance's preview mesh is invalid, and use the default
		//	preview mesh instead if the parent's mesh is also invalid
		UMaterialInterface* ParentMaterial = MaterialEditorInstance->SourceInstance->Parent;

		PreviewAsset = ParentMaterial != nullptr ? ParentMaterial->PreviewMesh.TryLoad() : nullptr;
		...
		if (!PreviewAsset)
		{
			...
			if (const UMaterialEditorOptions* MaterialEditorOptions = UMaterialEditorOptions::Get())
			{
				PreviewAsset = MaterialEditorOptions->GetPreviewMesh();
			}
		}
...
void FMaterialInstanceEditor::RefreshOnScreenTextureWarnings()
{
	...
	if (bValidateMaterials && MaterialEditorInstance->SourceInstance)
	{
		...
		if (BaseMaterial)
		{
			...
			TMap<FGuid, TObjectPtr<UMaterialExpression>> MaterialGuids;
			BaseMaterial->BuildExpressionParametersGuidsMap(MaterialGuids);
			...
			for (int32 GroupIndex = 0; GroupIndex < GroupCount; ++GroupIndex)
			{
				...
				for (int32 ParameterIndex = 0; ParameterIndex < ParameterCount; ++ParameterIndex)
				{
					...
					if (TextureParameterValue && TextureParameterValue->ExpressionId.IsValid())
					{
						...
						if (Texture)
						{
							EMaterialSamplerType SamplerType = MaterialExpressionUtils::GetSamplerTypeForTexture(Texture);
							UMaterialExpressionTextureSampleParameter* Expression = nullptr;

							TObjectPtr<UMaterialExpression>* MapExpressions = MaterialGuids.Find(TextureParameterValue->ExpressionId);
							if (MapExpressions)
							{
								Expression = Cast<UMaterialExpressionTextureSampleParameter>(*MapExpressions);
							}
					...
					if (RuntimeVirtualTextureParameterValue && RuntimeVirtualTextureParameterValue->ExpressionId.IsValid())
					{
						...
						{
							UMaterialExpressionRuntimeVirtualTextureSampleParameter* Expression = nullptr;

							TObjectPtr<UMaterialExpression>* MapExpression = MaterialGuids.Find(RuntimeVirtualTextureParameterValue->ExpressionId);
							if (MapExpression)
							{
								Expression = Cast<UMaterialExpressionRuntimeVirtualTextureSampleParameter>(*MapExpression);
							}
					...
					if (SparseVolumeTextureParameterValue && SparseVolumeTextureParameterValue->ExpressionId.IsValid())
					{
						...
						{
							UMaterialExpressionSparseVolumeTextureSampleParameter* Expression = nullptr;

							TObjectPtr<UMaterialExpression>* MapExpression = MaterialGuids.Find(SparseVolumeTextureParameterValue->ExpressionId);
							if (MapExpression)
							{
								Expression = Cast<UMaterialExpressionSparseVolumeTextureSampleParameter>(*MapExpression);
							}
...
void FMaterialInstanceEditor::Tick(float DeltaTime)
{
	...
	if (NumShadersPillText.IsValid())
	{
		// Update the Num Shaders text in the viewport.
		if (MaterialEditorInstance->SourceInstance->bHasStaticPermutationResource)
		{
			...
			NumShadersPillText->SetText(MaterialStatsManager->GetShaderCountText(Platform, GetCachedScalabilityCVars().MaterialQualityLevel, 0));
		}
		else
		{
			NumShadersPillText->SetText(NSLOCTEXT("MaterialInstanceEditor", "NumShadersText", "Shaders: 0"));
		}
	}
}
...
void FMaterialInstanceEditor::SaveAsset_Execute()
{
	if (bIsFunctionPreviewMaterial && MaterialEditorInstance)
	{
		UE_LOGF(LogMaterialInstanceEditor, Log, "Saving and applying instance %ls", *GetEditingObjects()[0]->GetName());
		MaterialEditorInstance->ApplySourceFunctionChanges();
	}
...
void FMaterialInstanceEditor::SaveSettings()
{
	...
	{
		MaterialEditorOptions->SaveConfig();
	}
}
