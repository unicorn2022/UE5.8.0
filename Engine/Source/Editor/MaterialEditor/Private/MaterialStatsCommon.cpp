...
FString FMaterialStatsUtils::RepresentativeShaderTypeToString(const ERepresentativeShader ShaderType)
{
	switch (ShaderType)
	{
		...
		case ERepresentativeShader::UIDynamicFragmentShader:
			return TEXT("UI Pixel Shader (Dynamic)");
...
void FMaterialStatsUtils::GetRepresentativeShaderTypesAndDescriptions(TMap<FName, TArray<FRepresentativeShaderInfo>>& ShaderTypeNamesAndDescriptions, const FMaterial* TargetMaterial)
{
	...
	if (TargetMaterial->IsUIMaterial())
	{
		...
		static FName TSlateMaterialShaderPSDynamicName = TEXT("TSlateMaterialShaderPSDynamic");
		ShaderTypeNamesAndDescriptions.FindOrAdd(FLocalVertexFactoryName)
		.Add(FRepresentativeShaderInfo(ERepresentativeShader::UIDynamicFragmentShader, TSlateMaterialShaderPSDynamicName, TEXT("Dynamic UI Pixel Shader")));
	...
	else if (TargetMaterial->GetFeatureLevel() >= ERHIFeatureLevel::SM5)
	{
		...
		const bool bTranslucent = IsTranslucentBlendMode(*TargetMaterial);
		if (TargetMaterial->GetShadingModels().IsUnlit())
		{
			ShaderTypeNamesAndDescriptions.FindOrAdd(FLocalVertexFactoryName)
				.Add(FRepresentativeShaderInfo(
						ERepresentativeShader::StationarySurface, 
						TBasePassPSFNoLightMapPolicyName, 
						TBasePassPSFNoLightMapPolicyName.ToString()));
		}
		else
		{
			ShaderTypeNamesAndDescriptions.FindOrAdd(FLocalVertexFactoryName)
				.Add(FRepresentativeShaderInfo(
						ERepresentativeShader::DynamicallyLitObject, 
						TBasePassPSFNoLightMapPolicyName, 
						TBasePassPSFNoLightMapPolicyName.ToString()));
	...
	else
	{
		...
		static const FName TMobileBasePassPSFNoLightMapPolicyName = TEXT("TMobileBasePassPSFNoLightMapPolicy");
			...
			if (IsStaticLightingAllowed() && TargetMaterial->IsUsedWithStaticLighting())
			{
				...
				if (bAllowDistanceFieldShadows)
				{
					...
					static const FName TMobileBasePassPSFMobileDistanceFieldShadowsAndLQLightMapPolicyName =
						TEXT("TMobileBasePassPSFMobileDistanceFieldShadowsAndLQLightMapPolicy");
				...
				else //no shadows & lightmapped
				{
					static const FName TMobileBasePassPSTLightMapPolicyLQName =
						TEXT("TMobileBasePassPSTLightMapPolicyLQ");
...
void FMaterialStatsUtils::ExtractMatertialStatsInfo(EShaderPlatform ShaderPlatform, FShaderStatsInfo& OutInfo, const FMaterialResource* MaterialResource)
{
	...
	if (!bNoErrors || !MaterialShaderMap)
	{
		return;
	}
	...
	const FMaterialCompilationOutput& CompilationOutput = MaterialShaderMap->GetCompilationOutput();
	TArray<FDebugShaderTypeInfo> OutShaderInfo;
	MaterialResource->GetShaderTypes(LayoutParams, OutShaderInfo, FCompilationShaderParameters(MaterialResource, CompilationOutput));
