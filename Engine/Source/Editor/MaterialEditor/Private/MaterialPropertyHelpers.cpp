...
void FMaterialPropertyHelpers::ConfigurePropertyRowForParameter(IDetailPropertyRow& PropertyRow, UDEditorParameterValue* Parameter, TSharedPtr<IPropertyHandle> ParameterValueProperty, UMaterialEditorInstanceConstant* MaterialEditorInstance, const FMaterialParameterPropertyRowOverrides& RowOverrides /*= FMaterialParameterPropertyRowOverrides()*/)
{
	...
	PropertyRow
		...
		.OverrideResetToDefault(FMaterialPropertyHelpers::CreateResetToDefaultOverride(Parameter, MaterialEditorInstance, ParameterValueProperty));
...
{
	return GetParameterTooltip(Parameter, static_cast<UMaterialEditorInstanceConstant*>(MaterialEditorInstance));
}

FText FMaterialPropertyHelpers::GetParameterTooltip(UDEditorParameterValue* Parameter, UMaterial* Material)
{
	const FMaterialCachedExpressionData* CachedExpressionData = Material ? &Material->GetCachedExpressionData() : nullptr;

	FText AssetPath;
	if (CachedExpressionData)
	{
		const TSharedPtr<FMaterialCachedExpressionEditorOnlyData>& EditorOnlyData = CachedExpressionData->EditorOnlyData;
		if (EditorOnlyData)
		{
			if (const FParameterAssetPathsEntry* Entry = EditorOnlyData->AssetPathsMap.Find(Parameter->ParameterInfo.Name))
			{
				AssetPath = FText::FromString(Entry->ToString());
			}
		}
	}
...
FText FMaterialPropertyHelpers::GetParameterTooltip(UDEditorParameterValue* Parameter, UMaterialEditorInstanceConstant* MaterialEditorInstance)
{
	// If the material instance disallows the creation of new shader permutations, prevent overriding the static parameter.
	if (Parameter->IsStaticParameter() && MaterialEditorInstance->SourceInstance && MaterialEditorInstance->SourceInstance->bDisallowStaticParameterPermutations)
	{
	...
	// Otherwise, fallback to the generic material parameter tooltip 
	return GetParameterTooltip(Parameter, MaterialEditorInstance->Parent ? MaterialEditorInstance->Parent->GetMaterial() : nullptr);
}

FResetToDefaultOverride FMaterialPropertyHelpers::CreateResetToDefaultOverride(UDEditorParameterValue* Parameter, UMaterialEditorInstanceConstant* MaterialEditorInstance, TSharedPtr<IPropertyHandle> ParameterValueProperty)
{
	...
	TAttribute<bool> IsResetVisible = TAttribute<bool>::Create(TAttribute<bool>::FGetter::CreateStatic(&FMaterialPropertyHelpers::ShouldShowResetToDefault, Parameter, MaterialEditorInstance));
	FSimpleDelegate ResetHandler = FSimpleDelegate::CreateStatic(&FMaterialPropertyHelpers::ResetToDefault, Parameter, MaterialEditorInstance, ParameterValueProperty);
	return FResetToDefaultOverride::Create(IsResetVisible, ResetHandler);
}

void FMaterialPropertyHelpers::ResetToDefault(class UDEditorParameterValue* Parameter, UMaterialEditorInstanceConstant* MaterialEditorInstance, TSharedPtr<IPropertyHandle> ParameterValueProperty)
{
	...
	if (Parameter->GetValue(EditorValue))
	{
		...
		{
			ParameterValueProperty->NotifyPreChange();
			Parameter->SetValue(DefaultValue.Value);
			ParameterValueProperty->NotifyPostChange(EPropertyChangeType::ValueSet|EPropertyChangeType::ResetToDefault);
...
TSharedRef<SWidget> FMaterialPropertyHelpers::CreateScalarEnumerationParameterValueWidget(TSoftObjectPtr<UObject> const& SoftEnumeration, TSharedPtr<IPropertyHandle> ParameterValueProperty)
{
	...
	FOnGetPropertyComboBoxStrings GetStrings = FOnGetPropertyComboBoxStrings::CreateLambda([WeakEnumeration](TArray<TSharedPtr<FString>>& OutStrings, TArray<TSharedPtr<SToolTip>>&, TArray<bool>&)
	{
		...
		{
			// UUserDefinedEnum always add a _MAX entry to the enum. We want to ignore that for populating the combo box.
			// C++ UEnum usually use UMETA(Hidden) to hide any _MAX entry (and other entries too).
			const bool bUserDefinedEnum = Enum->IsA<UUserDefinedEnum>();
			const int32 NumEnums = bUserDefinedEnum ? Enum->NumEnums() - 1 : Enum->NumEnums();
