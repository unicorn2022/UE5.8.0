...
static TAutoConsoleVariable<bool> CVarMaterialEdGroupSortIncludeFunctions(
	TEXT("r.MaterialEditor.GroupSortIncludeFunctions"),
	true,
	TEXT("Whether Groups inside used Material Functions should be included in the sorting data."),
	ECVF_Default);
...
// ~RF_Standalone lets the preview GC once the editor releases its reference.
static UMaterial* DuplicatePreviewMaterial(UMaterial* Source)
{
	UMaterial* Preview = (UMaterial*)StaticDuplicateObject(Source, GetTransientPackage(), NAME_None, ~RF_Standalone, UPreviewMaterial::StaticClass());
	Preview->CancelOutstandingCompilation();
	Preview->bAllowDevelopmentShaderCompile = CVarMaterialEdUseDevShaders.GetValueOnGameThread();
	Preview->bIsPreviewMaterial = true;
	return Preview;
}

// Usage flags are duplicate-transient (prevents accidental new special-engine materials); copy manually.
static void CopyMaterialUsageFlags(const UMaterial* From, UMaterial* To)
{
	for (int32 Usage = 0; Usage < MATUSAGE_MAX; ++Usage)
	{
		const EMaterialUsage UsageEnum = (EMaterialUsage)Usage;
		To->SetUsageByFlag(UsageEnum, From->GetUsageByFlag(UsageEnum));
	}
	To->bUsedAsSpecialEngineMaterial = From->bUsedAsSpecialEngineMaterial;
}

// Restore back-pointers that serializers and duplicators null out.
static void RestoreExpressionBackReferences(UMaterial* Material)
{
	for (UMaterialExpression* MaterialExpression : Material->GetExpressions())
	{
		...
		if (UMaterialGraphNode* GraphNode = Cast<UMaterialGraphNode>(MaterialExpression->GraphNode))
		{
...
// Function-edit mode wraps a UMaterialFunction's expressions in a wrapper UMaterial so the material editor can drive them. The expressions must report Material = the
// wrapper and Function = nullptr; dispatch code that keys off Expression->Function would otherwise route them through the function path instead of the material path.
static void RebindExpressionsToMaterial(UMaterial* Material)
{
	...
	, Expression(InExpression)
{
	...
	ReferencedTextures = InExpression->Material->GetDefaultTextures();
...
int32 FMatExpressionPreview::CompilePropertyAndSetMaterialProperty(EMaterialProperty Property, FMaterialCompiler* Compiler, EShaderFrequency OverrideShaderFrequency, bool bUsePreviousFrameTime) const
{
	...
	if( Property == MP_EmissiveColor && Expression.IsValid())
	{
		...
		if (PreviewCodeChunk != INDEX_NONE)
		{
			const EMaterialValueType CompiledType = Compiler->GetType(PreviewCodeChunk);
			CompiledPreviewNumChannels = static_cast<int32>(GetNumComponents(MakeNonLWCType(CompiledType)));
		}
		else
		{
			CompiledPreviewNumChannels = 0;
		}
...
void FMaterialEditor::RegisterToolbarTab(const TSharedRef<class FTabManager>& InTabManager)
{
	...
	InTabManager->RegisterTabSpawner(FMaterialEditorTabs::NamedReroutesTabId, FOnSpawnTab::CreateSP(this, &FMaterialEditor::SpawnTab_NamedReroutes))
		.SetDisplayName(LOCTEXT("NamedReroutesTab", "Named Reroutes"))
		.SetGroup(WorkspaceMenuCategoryRef)
		.SetIcon(FSlateIcon(FAppStyle::GetAppStyleSetName(), "Kismet.Tabs.Palette"));
...
void FMaterialEditor::UnregisterTabSpawners(const TSharedRef<class FTabManager>& InTabManager)
{
	...
	InTabManager->UnregisterTabSpawner(FMaterialEditorTabs::NamedReroutesTabId);
...
void FMaterialEditor::InitEditorForMaterial(UMaterial* InMaterial)
{
	...
	Material = DuplicatePreviewMaterial(OriginalMaterial);

	// Ensure there are no null entries
	check(Material->GetExpressions().Find(nullptr) == INDEX_NONE);

	TArray<FString> Groups;
	UpdateAndGetMaterialExpressionGroups(&Groups);
}
...
void FMaterialEditor::InitMaterialEditor( const EToolkitMode::Type Mode, const TSharedPtr< class IToolkitHost >& InitToolkitHost, UObject* ObjectToEdit )
{
	...
	CopyMaterialUsageFlags(OriginalMaterial, Material);
...
FMaterialEditor::FMaterialEditor()
	...
	, bSelectFocusedNodes(false)
	, bFocusSelectionInProgress(false)
...
FMaterialEditor::~FMaterialEditor()
{
	...
	if (OnGraphChangedDelegateHandle.IsValid() && Material && Material->MaterialGraph)
	{
		Material->MaterialGraph->RemoveOnGraphChangedHandler(OnGraphChangedDelegateHandle);
		OnGraphChangedDelegateHandle.Reset();
	}
...
void FMaterialEditor::UpdateAndGetMaterialExpressionGroups(TArray<FString>* OutGroups)
{
	...
	UpdateMaterialExpressionGroups();
	for (const FParameterGroupData& GroupData : EditorOnlyData->ParameterGroupData)
	{
		OutGroups->Add(GroupData.GroupName);
	}
}

void FMaterialEditor::UpdateMaterialExpressionGroups()
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FMaterialInstanceEditor::UpdateMaterialExpressionGroups)
	...
	TArray<UMaterialExpression*> ExpressionsToParse = TArray<UMaterialExpression*>(Material->GetExpressions());

	if (CVarMaterialEdGroupSortIncludeFunctions.GetValueOnGameThread())
	{
		TSet<UMaterialExpression*> FunctionsExpressions;

		for (const UMaterialExpression* MaterialExpression : Material->GetExpressions())
		{
			if (const UMaterialExpressionMaterialFunctionCall* FunctionCall = Cast<UMaterialExpressionMaterialFunctionCall>(MaterialExpression))
			{
				if (FunctionCall->MaterialFunction)
				{
					TArray<UMaterialExpression*> FunctionExpressions;
					FunctionCall->MaterialFunction->GetAllExpressionsOfType<UMaterialExpression>(FunctionExpressions);
					FunctionsExpressions.Append(FunctionExpressions);
				}
			}
		}

		ExpressionsToParse.Append(FunctionsExpressions.Array());
	}

	TSet<FParameterGroupData> UpdatedGroups;
	for (const UMaterialExpression* MaterialExpression : ExpressionsToParse)
	{
		FMaterialParameterMetadata ParameterMeta;
		if (MaterialExpression->GetParameterValue(ParameterMeta))
		{
			if (ParameterMeta.Group.IsNone())
			{
				continue;
			}
			...
			if (const FParameterGroupData* GroupData = Material->GetSortingGroup(GroupName))
			{
	...
	TArray<FParameterGroupData> UpdatedGroupsArray = UpdatedGroups.Array();
	UpdatedGroupsArray.Sort(); // Sort groups based on name

	// This indirectly gets rid of any groups which might have been removed
	EditorOnlyData->ParameterGroupData = UpdatedGroupsArray;
}
...
EMaterialQualityLevel::Type FMaterialEditor::GetQualityPreviewLevel() const
{
	return NodeQualityLevel == EMaterialQualityLevel::Num ? GetCachedScalabilityCVars().MaterialQualityLevel : NodeQualityLevel;
}
...
void FMaterialEditor::CreateInternalWidgets()
{
	...
	FOnGetDetailCustomizationInstance LayoutExpressionParameterDetails = FOnGetDetailCustomizationInstance::CreateStatic(
		&FMaterialExpressionParameterDetails::MakeInstance, FOnCollectParameterGroups::CreateSP(this, &FMaterialEditor::UpdateAndGetMaterialExpressionGroups) );
	...
	FOnGetDetailCustomizationInstance LayoutLayerExpressionParameterDetails = FOnGetDetailCustomizationInstance::CreateStatic(
		&FMaterialExpressionLayersParameterDetails::MakeInstance, FOnCollectParameterGroups::CreateSP(this, &FMaterialEditor::UpdateAndGetMaterialExpressionGroups));
	...
	MaterialCustomPrimitiveDataWidget = SNew(SMaterialCustomPrimitiveDataPanel, this, MaterialEditorInstance);
	...
	NamedReroutesPanel = SNew(SMaterialNamedReroutesPanel, SharedThis(this));

	if (Material && Material->MaterialGraph)
	{
		OnGraphChangedDelegateHandle = Material->MaterialGraph->AddOnGraphChangedHandler(FOnGraphChanged::FDelegate::CreateSP(this, &FMaterialEditor::OnGraphChanged));
	}
...
void FMaterialEditor::OnFinishedChangingProperties(const FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	// Refresh the Named Reroutes panel when a declaration's name or color changes
	if (NamedReroutesPanel.IsValid())
	{
		for (int32 Index = 0; Index < PropertyChangedEvent.GetNumObjectsBeingEdited(); ++Index)
		{
			if (PropertyChangedEvent.GetObjectBeingEdited(Index)->IsA<UMaterialExpressionNamedRerouteDeclaration>())
			{
				NamedReroutesPanel->RequestRefresh();
				break;
			}
		}
	}
...
void FMaterialEditor::OnGraphChanged(const FEdGraphEditAction& Action)
{
	// This will handle adding nodes, removing nodes, and duplicating nodes.
	if (NamedReroutesPanel.IsValid())
	{
		NamedReroutesPanel->RequestRefresh();
	}
...
{
	UpdateMaterialPlatformStats();
	UpdateMaterialInfoLog();
...
bool FMaterialEditor::ApproveSetPreviewAsset(UObject* InAsset)
{
	...
	// Only permit the use of a skeletal mesh if the material has SkeletalMesh usage flag.
	if (USkeletalMesh* SkeletalMesh = Cast<USkeletalMesh>(InAsset))
	{
		if (!Material->CheckMaterialUsage(MATUSAGE_SkeletalMesh))
		{
			FMessageDialog::Open(EAppMsgType::Ok, NSLOCTEXT("UnrealEd", "Error_MaterialEditor_CantPreviewOnSkelMesh", "Can't preview on the specified skeletal mesh because the material has not been compiled with SkeletalMesh usage flag."));
			bApproved = false;
		}
	}
...
{
	UE_LOGF(LogMaterialEditor, Log, "Saving and Compiling material %ls", *GetEditingObjects()[0]->GetName());
...
void FMaterialEditor::RefreshPreviewViewport()
{
	if (PreviewViewport.IsValid())
	{
		UpdatePreviewCustomPrimitiveData();
		PreviewViewport->RefreshViewport();
	}
}

void FMaterialEditor::UpdatePreviewCustomPrimitiveData()
{
	if (!PreviewViewport.IsValid() || !PreviewViewport->PreviewMeshComponent || !Material)
	{
		return;
	}

	UPrimitiveComponent* PreviewComponent = PreviewViewport->PreviewMeshComponent;

	PreviewComponent->ResetCustomPrimitiveData();
	PreviewComponent->SetCustomPrimitiveDataFloatArray(0, CustomPrimitiveDataFloats);
	PreviewComponent->MarkRenderStateDirty();
}

void FMaterialEditor::LoadEditorSettings()
{
	EditorOptions = UMaterialEditorOptions::Get();
	...
	{
		// The material preview mesh couldn't be found or isn't loaded. Default to the saved editor preview mesh.
		SetPreviewAsset(EditorOptions->GetPreviewMesh());
	}
}
...
bool FMaterialEditor::UpdateOriginalMaterial()
{
	...
	if (MaterialFunction)
	{
		...
		// Since MaterialFunction->ParentFunction gets initialized as a duplicate here and bExposeToLibrary 
		// is protected against duplicate for non-transient objects, we need to manually copy it
		MaterialFunction->ParentFunction->bExposeToLibrary = MaterialFunction->bExposeToLibrary;
...
void FMaterialEditor::UpdateMaterialInfoLog()
{
	...
	for (int32 i = 0; i < NumFeatureLevels; ++i)
	{
		...
		{
			const FString ErrorString = FString::Printf(TEXT("[%s] %s"), *FeatureLevelName, *CompileErrors[ErrorIndex]);
			...
			if(FailingExpression.Num() && ensure(FailingExpression.Num() == CompileErrors.Num()) && FailingExpression[ErrorIndex])
			{
				...
				const FString& CompilerError = CompileErrors[ErrorIndex];
				int32 NodeNameEndPos = INDEX_NONE;

				// If we have a failing expression, first matching parantheses will be for its name
				if (CompilerError.FindChar(L')', NodeNameEndPos))
				{
					bHyperlinkAdded = true;
					Line->AddToken(FTextToken::Create(FText::FromString(FeatureLevelName)));

					const FString NodeName = CompilerError.Left(NodeNameEndPos + 1);
					const FString ErrorBody = CompilerError.RightChop(NodeNameEndPos + 2);

					Line->AddToken(FUObjectToken::Create(FailingExpression[ErrorIndex], FText::FromString(NodeName)));
					Line->AddToken(FTextToken::Create(FText::FromString(ErrorBody)));
				}
			}
...
void FMaterialEditor::UpdateMaterialPlatformStats()
{
	...
	// Update the num shaders UI
	if (NumShadersText.IsValid())
	{
		EShaderPlatform Platform = GEditor->GetActiveShaderPlatform();
		if (FDataDrivenShaderPlatformInfo::GetIsPreviewPlatform(Platform))
		{
			Platform = FDataDrivenShaderPlatformInfo::GetPreviewShaderPlatformParent(Platform);
		}
		NumShadersText->SetText(MaterialStatsManager->GetShaderCountText(Platform, GetQualityPreviewLevel(), 0));
	}
...
{
	UE_LOGF(LogMaterialEditor, Log, "Applying material %ls", *GetEditingObjects()[0]->GetName());
...
bool FMaterialEditor::IsSelectFocusedNodesEnabled() const
{
	return bFocusWholeChain;
}
...
void FMaterialEditor::HideUnrelatedNodes()
{
	...
	const TArray<UMaterialGraphNode*> NodesToShow = GetSelectionRelatedNodes();
	...
	if (bFocusWholeChain && bSelectFocusedNodes)
	{
		SelectMaterialGraphNodes(*FocusedGraphEd, NodesToShow);
	}
}

void FMaterialEditor::SelectMaterialGraphNodes(SGraphEditor& InGraphEd, const TArray<UMaterialGraphNode*>& InNodes)
{
	TArray<UObject*> SelectedObjects;
	SelectedObjects.Reserve(InNodes.Num());

	bFocusSelectionInProgress = true;
	for (UMaterialGraphNode* Node : InNodes)
	{
		SelectedObjects.Add(Node->MaterialExpression);
		InGraphEd.SetNodeSelection(Node, true);
	}
	bFocusSelectionInProgress = false;
...
void FMaterialEditor::MakeHideUnrelatedNodesOptionsMenu(UToolMenu* Menu)
{
	...
	TSharedRef<SWidget> SelectFocusedNodesCheckBox = SNew(SBox)
		.IsEnabled(this, &FMaterialEditor::IsSelectFocusedNodesEnabled)
		[
			SNew(SCheckBox)
				.IsChecked(bSelectFocusedNodes ? ECheckBoxState::Checked : ECheckBoxState::Unchecked)
				.OnCheckStateChanged(this, &FMaterialEditor::OnSelectFocusedNodesCheckStateChanged)
				.Style(FAppStyle::Get(), "Menu.CheckBox")
				.ToolTipText(
					LOCTEXT("SelectFocusedNodesTooltip", "Select focused nodes in the chain. \n" 
					"This only works if Focus Whole Chain is also enabled."))
				.Content()
				[
					SNew(SHorizontalBox)
						...
						[
							SNew(STextBlock)
								.Text(LOCTEXT("SelectFocusedNodes", "Select Focused Nodes"))
	...
	OptionsSection.AddEntry(FToolMenuEntry::InitMenuEntry("SelectFocusedNodesCheckBox", FUIAction(), SelectFocusedNodesCheckBox));
}
...
void FMaterialEditor::OnSelectFocusedNodesCheckStateChanged(ECheckBoxState NewCheckedState)
{
	TSharedPtr<SGraphEditor> FocusedGraphEd = FocusedGraphEdPtr.Pin();
	if (!FocusedGraphEd)
	{
	...
	bSelectFocusedNodes = NewCheckedState == ECheckBoxState::Checked;

	if (bHideUnrelatedNodes && bSelectRegularNode)
	{
		const TArray<UMaterialGraphNode*> NodesToSelect = GetSelectionRelatedNodes();
		SelectMaterialGraphNodes(*FocusedGraphEd, NodesToSelect);
	}
}
...
void FMaterialEditor::SelectNamedRerouteUsages(UMaterialExpressionNamedRerouteDeclaration* Declaration)
{
	if (!Declaration)
	{
		return;
	...
	for (UMaterialExpression* Expression : Material->GetExpressions())
	{
		...
		{
			if (UEdGraphNode* UsageGraphNode = Usage->GraphNode)
			{
	...
	if (TSharedPtr<SGraphEditor> FocusedGraphEd = FocusedGraphEdPtr.Pin())
	{
		...
		if (SelectedNodes.Num() == 1)
		{
			...
			for (FGraphPanelSelectionSet::TConstIterator NodeIt(SelectedNodes); NodeIt; ++NodeIt)
			{
				...
				{
					if (UMaterialExpressionNamedRerouteDeclaration* Declaration = Cast<UMaterialExpressionNamedRerouteDeclaration>(GraphNode->MaterialExpression))
					{
						SelectNamedRerouteUsages(Declaration);
					}
				}
			}
		}
...
void FMaterialEditor::SetCustomPrimitiveData(TConstArrayView<float> CustomFloats)
{
	CustomPrimitiveDataFloats = CustomFloats;
	UpdatePreviewCustomPrimitiveData();
}
...
void FMaterialEditor::OnResetToDefault(const FToolMenuContext& InMenuContext) const
{
	...
	if (RootPinNode != nullptr)
	{
		...
		{
			case MP_BaseColor:
				EditorOnlyData->BaseColor.Constant = FMaterialAttributeDefinitionMap::GetDefaultValue(MP_BaseColor);
...
void FMaterialEditor::OnMaterialUsageFlagsChanged(UMaterial* MaterialThatChanged, int32 FlagThatChanged)
{
	...
	// Update only if a modification has been done to the original material by the user and guard when Editor
	// is opened for MF, as we don't have an original material there, thus OriginalMaterial == Material 
	// and Material can be modified and received here, depending on which Preview Mesh is selected
	if((MaterialThatChanged == OriginalMaterial) && !MaterialFunction)
	{
		Material->SetUsageByFlag(Flag, OriginalMaterial->GetUsageByFlag(Flag));
		UpdatePreviewMaterial();
	}
}
		...
		[
			...
			[
				...
				[
					...
					.AutoWidth()
					[
						SAssignNew(NumShadersText, STextBlock)
					]
				]
			]
		];

	NumShadersText->SetVisibility(MaterialFunction ? EVisibility::Collapsed : EVisibility::Visible);
...
TSharedRef<SDockTab> FMaterialEditor::SpawnTab_NamedReroutes(const FSpawnTabArgs& Args)
{
	check(Args.GetTabId() == FMaterialEditorTabs::NamedReroutesTabId);

	TSharedRef<SDockTab> SpawnedTab = SNew(SDockTab)
		.Label(LOCTEXT("MaterialNamedReroutesTitle", "Named Reroutes"))
		[
			SNew(SBox)
			.AddMetaData<FTagMetaData>(FTagMetaData(TEXT("MaterialNamedReroutes")))
			[
				NamedReroutesPanel.ToSharedRef()
			]
		];
...
UMaterialExpression* FMaterialEditor::CreateNewMaterialExpression(UClass* NewExpressionClass, const FVector2D& NodePos, bool bAutoSelect, bool bAutoAssignResource, const UEdGraph* Graph)
{
	...
	{
		UE_LOGF(LogMaterialEditor, Warning, "Trying to create a disallowed material expression of type %ls.", *NewExpressionClass->GetName());
		return NULL;
	}
...
void FMaterialEditor::PostPasteMaterialExpression(UMaterialExpression* NewExpression)
{
	...
	if (FunctionCall)
	{
		...
		// Some data is lost due to being transient on paste, we need to reconstruct that data now
		// PinCategory - Ensure we set PC_Optional as needed, otherwise they default to PC_Required
		// PinName - Add back in the type information (V4) / (S) / etc
		if (UMaterialGraphNode* MaterialGraphNode = Cast<UMaterialGraphNode>(FunctionCall->GraphNode))
		{
			for (UEdGraphPin* Pin : MaterialGraphNode->Pins)
			{
				if (Pin->Direction == EGPD_Input)
				{
					const int32 InputIndex = Pin->SourceIndex;
					if (InputIndex >= 0 && InputIndex < FunctionCall->FunctionInputs.Num())
					{
						Pin->PinType.PinCategory = FunctionCall->IsInputConnectionRequired(InputIndex) ? UMaterialGraphSchema::PC_Required : UMaterialGraphSchema::PC_Optional;

						const FName NewPinName = UMaterialGraphNode::GetShortenPinName(FunctionCall->GetInputName(InputIndex));
...
void FMaterialEditor::CreateDerivedMaterialInstancesPreviews()
{
	...
	else
	{
		...
		UE_LOGF(LogMaterialEditor, Warning, "MaterialEditor missing valid OriginalMaterial.");
		UE_LOGF(LogMaterialEditor, Warning, "OriginalMaterialObject = %ls", *(OriginalMaterialObject ? OriginalMaterialObject->GetPathName() : MissingEntry));
		UE_LOGF(LogMaterialEditor, Warning, "Material = %ls", *(Material ? Material->GetPathName() : MissingEntry));
		UE_LOGF(LogMaterialEditor, Warning, "ExpressionPreviewMaterial = %ls", *(ExpressionPreviewMaterial ? ExpressionPreviewMaterial->GetPathName() : MissingEntry));
		UE_LOGF(LogMaterialEditor, Warning, "PreviewExpression = %ls", *(PreviewExpression ? PreviewExpression->GetPathName() : MissingEntry));
		UE_LOGF(LogMaterialEditor, Warning, "MaterialFunction = %ls", *(MaterialFunction ? MaterialFunction->GetPathName() : MissingEntry));
		UE_LOGF(LogMaterialEditor, Warning, "bMaterialDirty = %ls", *LexToString(bMaterialDirty));
		UE_LOGF(LogMaterialEditor, Warning, "bStatsFromPreviewMaterial = %ls", *LexToString(bStatsFromPreviewMaterial));
		UE_LOGF(LogMaterialEditor, Warning, "bDestructing = %ls", *LexToString(bDestructing));

		UE_LOGF(LogMaterialEditor, Fatal, "OriginalMaterial is invalid in the material editor, with no deterministic repro of the problem. This message is to highlight the crash/callstack via the crash reporter with further Log info for Epic to investigate.");
	}
	...
	if (MaterialStatsManager->GetProvideDerivedMIFlag() && MaxCount != 0)
	{
		...
		for (TObjectIterator<UMaterialInstance> It(/*AdditionalExclusionFlags = */RF_ClassDefaultObject, /*bIncludeDerivedClasses = */true, /*InInternalExclusionFlags = */EInternalObjectFlags::Garbage); It; ++It)
		{
			...
			if (bSkip)
			{
				UE_LOGF(LogMaterialEditor, Display, "Skipping material instance '%ls' with static parameters due to depending on other material instances with static parameters", *Instance->GetName());
				continue;
			}
			...
			if (bSkip)
			{
				UE_LOGF(LogMaterialEditor, Display, "Skipping material instance '%ls' because instance with same static parameters and base material is already present", *Instance->GetName());
				continue;
			}
			...
			if (bIsLandscapeMaterial)
			{
				...
				{
					UE_LOGF(LogMaterialEditor, Display, "Skipping material instance '%ls' because it's used by landscape editor", *Instance->GetName());
					continue;
				}
...
void FMaterialEditor::PostUndo(bool bSuccess)
{
	...
	{	
		...
		RestoreExpressionBackReferences(Material);
...
void FMaterialEditor::NotifyPostChange( const FPropertyChangedEvent& PropertyChangedEvent, FProperty* PropertyThatChanged)
{
	...
	if ( PropertyThatChanged )
	{
		...
		{
			// Revert to editor preview mesh when usage isn't supported.
			if (!SetPreviewAssetByName(*Material->PreviewMesh.ToString()))
			{
				if (EditorOptions)
				{
...
void FMaterialEditor::RefreshExpressionPreviews(bool bForceRefreshAll /*= false*/)
{
	...
	{
		UE_LOGF(LogMaterialEditor, Warning, "Null expressions found in expressions list during expression previews refresh in material '%ls'.", *GetEditingObjects()[0]->GetName());
	}
	...
	if (!GraphEditorCommands)
{
	...
	return SNew(SGraphEditor)
		.GraphSupportsInvalidation(true)
...
TArray<UMaterialGraphNode*> FMaterialEditor::GetSelectionRelatedNodes()
{
	TArray<UMaterialGraphNode*> RelatedNodes;
	const FGraphPanelSelectionSet SelectedNodes = GetSelectedNodes();
	...
	for (FGraphPanelSelectionSet::TConstIterator NodeIt(SelectedNodes); NodeIt; ++NodeIt)
	{
		...
		if (SelectedNode)
		{
			...
			CollectDownstreamNodes(SelectedNode, RelatedNodes);
			CollectUpstreamNodes(SelectedNode, RelatedNodes);
		}
	}
...
void FMaterialEditor::FixupMismatchedSamplerExpressions()
{
	...
	for (UMaterialExpression* MaterialExpression : Material->GetExpressions())
	{
		if (UMaterialExpressionTextureBase* TextureBase = Cast<UMaterialExpressionTextureBase>(MaterialExpression))
		{
			...
			if (!MaterialExpressionUtils::VerifySamplerType(GEditor->PreviewPlatform.GetShaderPlatform(), nullptr, TextureBase->Texture, TextureBase->SamplerType, SamplerTypeError))
			{
...
void FMaterialEditor::OnSelectedNodesChanged(const TSet<class UObject*>& NewSelection)
{
	...
	if (SelectedObjects.Num() == 1 && SelectedObjects[0] == Material)
	{
		// Update when we display the material details, in case any MF parameter was modified
		UpdateMaterialExpressionGroups();
	}

	if (bHideUnrelatedNodes)
	{
		if (!bLockNodeFadeState)
		{
			if (TSharedPtr<SGraphEditor> FocusedGraphEd = FocusedGraphEdPtr.Pin())
			{
				FocusedGraphEd->ResetAllNodesUnrelatedStates();
		...
		else if(bSelectFocusedNodes)
		{
			// We still want to select nodes
			if (TSharedPtr<SGraphEditor> FocusedGraphEd = FocusedGraphEdPtr.Pin())
			{
				const TArray<UMaterialGraphNode*> NodesToSelect = GetSelectionRelatedNodes();
				SelectMaterialGraphNodes(*FocusedGraphEd, NodesToSelect);
...
void FMaterialEditor::OnNodeTitleCommitted(const FText& NewText, ETextCommit::Type CommitInfo, UEdGraphNode* NodeBeingChanged)
{
	if (NodeBeingChanged)
	{
		...
		if (NewName.IsEmpty())
		{
			...
			UE_LOGF(LogMaterialEditor, Warning, "New material graph node name '%ls...' exceeds maximum length of %d and thus was truncated.", *NewName.Left(8), NAME_SIZE - 1);
...
void FMaterialEditor::RefreshGraphFromOriginal()
{
	// Rebuild inside an in-flight transaction would clobber it.
	if (GIsTransacting)
	{
		return;
	}

	const bool bIsFunctionEditor = (MaterialFunction != nullptr);

	if (bIsFunctionEditor)
	{
		if (!MaterialFunction->ParentFunction || !Material)
		{
			return;
		}
	}
	...
	// Surface the swap so external rebuilds don't look like a glitch. Routine toasts suppress
	// if one is still on screen (no notification stacking); the discarded-edits toast bypasses
	// suppression because losing user work silently is worse than stacking.
	const bool bDiscardingDirty = bMaterialDirty;
	if (bDiscardingDirty || !ExternalRefreshNotificationPtr.IsValid())
	{
		FNotificationInfo Info(NSLOCTEXT("MaterialEditor", "ExternalRefreshToast", "Material refreshed from external edit."));
		Info.ExpireDuration = 4.0f;
		Info.bFireAndForget = true;
		if (bDiscardingDirty)
		{
			Info.SubText = NSLOCTEXT("MaterialEditor", "ExternalRefreshToastDirty", "Pending in-editor changes were discarded.");
		}
		ExternalRefreshNotificationPtr = FSlateNotificationManager::Get().AddNotification(Info);
	}

	// Carrying PreviewExpression across the swap would leave it pointing into the orphaned old asset.
	if (PreviewExpression)
	{
		SetPreviewExpression(nullptr);
	}

	// Cached preview material samples Material's state at lazy init; drop it so the next preview picks up the new asset.
	ExpressionPreviewMaterial = nullptr;

	if (bIsFunctionEditor)
	{
		UMaterialFunction* SourceFunction = MaterialFunction->ParentFunction;

		UMaterialFunction* NewPreview = (UMaterialFunction*)StaticDuplicateObject(SourceFunction, GetTransientPackage(), NAME_None, ~RF_Standalone, UMaterialFunction::StaticClass());
		NewPreview->ParentFunction = SourceFunction;
		NewPreview->SetFlags(RF_Transactional);

		// Function-mode wiring: throwaway Material shadows the function's expression collection (see InitMaterialEditor).
		NewPreview->MaterialGraph = Material->MaterialGraph;
		NewPreview->EditorMaterial = Material;
		Material->AssignExpressionCollection(NewPreview->GetExpressionCollection());
		Material->bEnableNewHLSLGenerator = NewPreview->bEnableNewHLSLGenerator;

		Material->MaterialGraph->MaterialFunction = NewPreview;
		MaterialFunction = NewPreview;

		RestoreExpressionBackReferences(Material);
		RebindExpressionsToMaterial(Material);
	}
	else
	{
		if (OriginalMaterial->IsSelected())
		...
		UMaterial* OldMaterial = Material;
		UMaterialGraph* ExistingGraph = Material ? Material->MaterialGraph : nullptr;

		UMaterial* NewPreview = DuplicatePreviewMaterial(OriginalMaterial);
		CopyMaterialUsageFlags(OriginalMaterial, NewPreview);

		// Reparent the existing UMaterialGraph onto the new preview so the user's pan/zoom survives the swap.
		if (ExistingGraph)
		{
			ExistingGraph->Rename(nullptr, NewPreview, REN_DontCreateRedirectors | REN_DoNotDirty);
			ExistingGraph->Material = NewPreview;
			NewPreview->MaterialGraph = ExistingGraph;
		}

		Material = NewPreview;
		// Drop the old preview from the toolkit's strong-rooted EditingObjects so it can GC; register the new one.
		if (OldMaterial)
		{
			RemoveEditingObject(OldMaterial);
		}
		AddEditingObject(NewPreview);
		SetPreviewMaterial(Material);
		FMaterialEditorUtilities::InitExpressions(Material);
	}

	Material->BuildEditorParameterList();
	// RebuildGraph appends to SubGraphs but never empties it; clear before the rebuild so composite-expression
	// materials don't accumulate stale subgraph entries pointing at the orphaned old preview.
	Material->MaterialGraph->SubGraphs.Empty();
	Material->MaterialGraph->RebuildGraph();
	...
	if (TSharedPtr<SGraphEditor> FocusedGraphEd = FocusedGraphEdPtr.Pin())
	{
		// RebuildGraph created fresh subgraph instances; an open subgraph document tab is now showing an orphan.
		// Close the stale tab and reinvoke the root graph editor (matches PostUndo).
		UEdGraph* CurrentGraph = FocusedGraphEd->GetCurrentGraph();
		if (CurrentGraph && CurrentGraph != Material->MaterialGraph)
		{
			CloseDocumentTab(CurrentGraph);
			DocumentManager->CleanInvalidTabs();
			DocumentManager->RefreshAllTabs();
			GetTabManager()->TryInvokeTab(FMaterialEditorTabs::GraphEditor);
		}
		...
		// Active graph can change above, re-acquire ptr
		if (TSharedPtr<SGraphEditor> RefocusedGraphEd = FocusedGraphEdPtr.Pin())
		{
			RefocusedGraphEd->ClearSelectionSet();
			RefocusedGraphEd->NotifyGraphChanged();
		}
	}

	UpdatePreviewMaterial(/*bForce=*/true);
	ForceRefreshExpressionPreviews();
	bMaterialDirty = false;
}
...
FMaterialEditor::FScopedMaterialChecker::~FScopedMaterialChecker()
{
	...
	{
		UE_LOGF(LogMaterialEditor, Warning, 
			"Material Editor internal error after operation '%s': null expressions found in material '%ls' expressions list.", 
