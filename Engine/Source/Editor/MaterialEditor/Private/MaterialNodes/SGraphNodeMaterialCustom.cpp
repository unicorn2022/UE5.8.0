...
void SGraphNodeMaterialCustom::CreateBelowPinControls(TSharedPtr<SVerticalBox> MainBox)
{
	...
	WeakCustomExpression = Cast<UMaterialExpressionCustom>(MaterialNode->MaterialExpression);
	...
	MainBox->AddSlot()
	.AutoHeight()
	.Padding(FMargin(10.f, 0.f, 10.f, 5.f))
	[
		SNew(STextBlock)
		.Text(NSLOCTEXT("MaterialEditor", "CustomNodePixelDiscard", "The code contains clip() or discard instructions."))
		.ToolTipText(NSLOCTEXT("MaterialEditor", "CustomNodePixelDiscardWarning_Tooltip", "Use Blend Mode Masked to create supporting shaders.\nUse ContainsClipInstruction=No to override if this is a false positive."))
		.Visibility(this, &SGraphNodeMaterialCustom::PixelDiscardVisibility)
		.AutoWrapText(true)
	];
}
...
EVisibility SGraphNodeMaterialCustom::PixelDiscardVisibility() const
{
	UMaterialExpressionCustom const* CustomExpression = WeakCustomExpression.Get();
	if (CustomExpression == nullptr)
	{
		return EVisibility::Collapsed;
	}
	return CustomExpression->HasPixelDiscard() ? EVisibility::Visible : EVisibility::Collapsed;
}
