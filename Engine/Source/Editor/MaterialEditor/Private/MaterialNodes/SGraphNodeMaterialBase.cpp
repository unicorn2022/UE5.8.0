...
TUniquePtr<UE::Slate::FPaintScope> FPreviewViewport::Paint(const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, const int32 LayerId, const FWidgetStyle& InWidgetStyle, const bool bParentEnabled)
{
	...
	if (PreviewElement->BeginRenderingCanvas(CanvasRect, ClippingRect, MaterialNode, bIsRealtime, HoveredUV.IsSet()))
	{
...
FReply FPreviewViewport::OnMouseMove(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	const FVector2D LocalPos = MyGeometry.AbsoluteToLocal(MouseEvent.GetScreenSpacePosition());
	const FVector2D Size = MyGeometry.GetLocalSize();
	HoveredUV = FVector2D(LocalPos.X / Size.X, LocalPos.Y / Size.Y);
	return FReply::Unhandled();
}

void FPreviewViewport::OnMouseEnter(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	const FVector2D LocalPos = MyGeometry.AbsoluteToLocal(MouseEvent.GetScreenSpacePosition());
	const FVector2D Size = MyGeometry.GetLocalSize();
	HoveredUV = FVector2D(LocalPos.X / Size.X, LocalPos.Y / Size.Y);
...
void FPreviewViewport::OnMouseLeave(const FPointerEvent& MouseEvent)
{
	HoveredUV.Reset();
	if (PreviewElement)
	{
		PreviewElement->ClearReadback();
	}
}
...
FPreviewElement::FPreviewElement()
	...
	, ReadbackRenderTarget(new FSlateMaterialPreviewRenderTarget)
{
}

bool FPreviewElement::BeginRenderingCanvas(const FIntRect& InCanvasRect, const FIntRect& InClippingRect, UMaterialGraphNode* InGraphNode, bool bInIsRealtime, bool bInShouldReadback)
{
	if(InCanvasRect.Size().X > 0 && InCanvasRect.Size().Y > 0 && InClippingRect.Size().X > 0 && InClippingRect.Size().Y > 0 && InGraphNode != NULL)
	{
		...
		RenderInfo.bShouldReadback = bInShouldReadback;
		...
		ENQUEUE_RENDER_COMMAND(BeginRenderingPreviewCanvas)(
			[PreviewElement, RenderInfo](FRHICommandListImmediate& RHICmdList)
			{
				...
				PreviewElement->bShouldReadback = RenderInfo.bShouldReadback;
			}
			);
		return true;
	}
...
TOptional<FLinearColor> FPreviewElement::GetReadbackValue(const FVector2D& UV) const
{
	UE::TScopeLock ReadLock(ReadbackDataGuard);
	if (ReadbackData.IsEmpty())
	{
		return {};
	}

	const int32 X = FMath::Clamp(FMath::CeilToInt(UV.X * ReadbackDimension), 0, ReadbackDimension - 1);
	const int32 Y = FMath::Clamp(FMath::CeilToInt(UV.Y * ReadbackDimension), 0, ReadbackDimension - 1);

	FLinearColor Value = ReadbackData[(Y * ReadbackDimension) + X].GetFloats();

	// Note: Gamma is baked into the preview shader (see FMatExpressionPreview::CompilePropertyAndSetMaterialProperty) so we need to undo it here
	const float Gamma = 2.2f;
	Value.R = FMath::Pow(Value.R, Gamma);
	Value.G = FMath::Pow(Value.G, Gamma);
	Value.B = FMath::Pow(Value.B, Gamma);
	return Value;
}

void FPreviewElement::ClearReadback()
{
	ENQUEUE_RENDER_COMMAND(ClearReadbackPreviewElement)(
		[this](FRHICommandListImmediate& RHICmdList)
		{
			GPUTextureReadback.Reset();
...
void FPreviewElement::Draw_RenderThread(FRDGBuilder& GraphBuilder, const FDrawPassInputs& Inputs)
{
	...
	RenderTarget->SetRenderTargetTexture(Inputs.OutputTexture);
	{
		...
		FCanvas* Canvas = GraphBuilder.AllocObject<FCanvas>(RenderTarget.Get(), nullptr, FGameTime::CreateUndilated(CurrentTime, DeltaTime), GMaxRHIFeatureLevel);
		{
	...
	if (bShouldReadback)
	{
		Readback_RenderThread(GraphBuilder);
		return;
	}
}

void FPreviewElement::Readback_RenderThread(FRDGBuilder& GraphBuilder)
{
	// Do we have a readback pending?  If so don't issue another one until we process the pending one
	if (bReadbackPending)
	{
		...
		const FFloat16Color* SrcData = static_cast<const FFloat16Color*>(GPUTextureReadback->Lock(RowPitchInPixels));
		{
			UE::TScopeLock WriteLock(ReadbackDataGuard);
			ReadbackData.SetNumUninitialized(ReadbackDimension * ReadbackDimension);
			for (int32 Y=0; Y < ReadbackDimension; ++Y)
			{
				FMemory::Memcpy(ReadbackData.GetData() + (Y * ReadbackDimension), SrcData + (Y * RowPitchInPixels), sizeof(FFloat16Color) * ReadbackDimension);
			}
		}
		GPUTextureReadback->Unlock();
	}

	// Issue a new render & readback request
	const FIntPoint ReadbackSize(ReadbackDimension, ReadbackDimension);

	const FRDGTextureDesc ReadbackTextureDesc = FRDGTextureDesc::Create2D(ReadbackSize, PF_FloatRGBA, FClearValueBinding::Black, TexCreate_RenderTargetable | TexCreate_ShaderResource);
	FRDGTextureRef ReadbackTexture = GraphBuilder.CreateTexture(ReadbackTextureDesc, TEXT("MaterialNodeReadbackRT"));

	ReadbackRenderTarget->SetViewRect(FIntRect(FIntPoint::ZeroValue, ReadbackSize));
	ReadbackRenderTarget->SetClippingRect(FIntRect(FIntPoint::ZeroValue, ReadbackSize));
	ReadbackRenderTarget->SetRenderTargetTexture(ReadbackTexture);

	{
		const double CurrentTime = bIsRealtime ? (FApp::GetCurrentTime() - GStartTime) : 0.0;
		const float  DeltaTime = bIsRealtime ? static_cast<float>(FApp::GetDeltaTime()) : 0.0f;

		FCanvas* ReadbackCanvas = GraphBuilder.AllocObject<FCanvas>(ReadbackRenderTarget.Get(), nullptr, FGameTime::CreateUndilated(CurrentTime, DeltaTime), GMaxRHIFeatureLevel);
		ReadbackCanvas->SetAllowedModes(0);
		ReadbackCanvas->SetRenderTargetRect(ReadbackRenderTarget->GetViewRect());
		ReadbackCanvas->SetRenderTargetScissorRect(ReadbackRenderTarget->GetClippingRect());
		FCanvasTileItem TileItem(FVector2D::ZeroVector, ExpressionPreview, FVector2D(ReadbackSize));
		ReadbackCanvas->DrawItem(TileItem);
		ReadbackCanvas->Flush_RenderThread(GraphBuilder, true);
	}
	ReadbackRenderTarget->ClearRenderTargetTexture();

	if (!GPUTextureReadback)
	{
		GPUTextureReadback = MakeUnique<FRHIGPUTextureReadback>(TEXT("MaterialNodeReadback"));
	}
	AddEnqueueCopyPass(GraphBuilder, GPUTextureReadback.Get(), ReadbackTexture);
	...
	if (MaterialNode->MaterialExpression->ShouldShowPreview())
	{
		...
		return SNew(SBox)
			...
			.ToolTip(
				...
				[
					SNew(STextBlock)
					.Text(this, &SGraphNodeMaterialBase::ExpressionPreviewValueText)
				]
			)
...
FText SGraphNodeMaterialBase::ExpressionPreviewOverlayText() const
{
	...
	return MaterialExpression->GetPreviewOverlayText();
}

FText SGraphNodeMaterialBase::ExpressionPreviewValueText() const
{
	FText ValueText;
	if (PreviewViewport->HoveredUV.IsSet())
	{
		TOptional<FLinearColor> Value = PreviewViewport->PreviewElement->GetReadbackValue(PreviewViewport->HoveredUV.GetValue());
		if (Value.IsSet())
		{
			int32 NumChannels = 0;
			if (FMaterialRenderProxy* Proxy = MaterialNode->GetExpressionPreview())
			{
				// FMatExpressionPreview is the only concrete type returned here
				if (const FMatExpressionPreview* ExprPreview = static_cast<const FMatExpressionPreview*>(Proxy))
				{
					NumChannels = ExprPreview->GetCompiledPreviewNumChannels();
				}
			}
			...
			{
			case 1:
				ValueText = FText::FromString(FString::Printf(TEXT("%.2g"), Value->R));
				break;
			case 2:
				ValueText = FText::FromString(FString::Printf(TEXT("%.2g, %.2g"), Value->R, Value->G));
				break;
			case 3:
				ValueText = FText::FromString(FString::Printf(TEXT("%.2g, %.2g, %.2g"), Value->R, Value->G, Value->B));
				break;
			default:
				ValueText = FText::FromString(FString::Printf(TEXT("%.2g, %.2g, %.2g, %.2g"), Value->R, Value->G, Value->B, Value->A));
				break;
			}
		}
	}
