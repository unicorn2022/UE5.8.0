...
{
	check(InArgs._OldMaterialGraph || InArgs._NewMaterialGraph || InArgs._OldMaterialInstance || InArgs._NewMaterialInstance);

	if (InArgs._OldMaterialInstance || InArgs._NewMaterialInstance)
	{
		PanelOld.MaterialInstance = InArgs._OldMaterialInstance;
		PanelNew.MaterialInstance = InArgs._NewMaterialInstance;

		// Create UMaterialEditorInstanceConstant proxies for each side so that ALL parameter types
		// (including static switches and component masks) are visible in the details view.
		// This is the same proxy object the Material Instance Editor uses to display parameters.
		auto CreateEditorProxy = [](UMaterialInstance* MI) -> UMaterialEditorInstanceConstant*
		{
			...
			UMaterialEditorInstanceConstant* Proxy = NewObject<UMaterialEditorInstanceConstant>(GetTransientPackage(), NAME_None, RF_Transactional);
			Proxy->SetSourceInstance(MIC);
			return Proxy;
		};

		PanelOld.EditorProxy = CreateEditorProxy(PanelOld.MaterialInstance);
		PanelNew.EditorProxy = CreateEditorProxy(PanelNew.MaterialInstance);

		PanelOld.RevisionInfo = InArgs._OldRevision;
		PanelNew.RevisionInfo = InArgs._NewRevision;

		// Build resolved material graphs from each instance's parent material.
		// Each side gets its own duplicated parent so expression objects are independent.
		auto BuildResolvedGraph = [](UMaterial* ParentCopy, UMaterialInstance* Instance) -> UMaterialGraph*
		{
			UMaterialGraph* Graph = CastChecked<UMaterialGraph>(
				FBlueprintEditorUtils::CreateNewGraph(
					GetTransientPackage(),
					MakeUniqueObjectName(GetTransientPackage(), UMaterialGraph::StaticClass()),
					UMaterialGraph::StaticClass(),
					UMaterialGraphSchema::StaticClass()));
			Graph->Material = ParentCopy;
			Graph->MaterialFunction = nullptr;
			Graph->RebuildGraph();

			// Apply static switch overrides from the instance
			if (Instance)
			{
				FStaticParameterSet StaticParams = Instance->GetStaticParameters();

				TMap<FName, bool> SwitchOverrides;
				for (const FStaticSwitchParameter& Param : StaticParams.StaticSwitchParameters)
				{
					if (Param.bOverride)
					{
						SwitchOverrides.Add(Param.ParameterInfo.Name, Param.Value);
					}
				}
				...
				for (UEdGraphNode* Node : Graph->Nodes)
				{
					...
					{
						if (UMaterialExpressionStaticSwitchParameter* SwitchExpr = Cast<UMaterialExpressionStaticSwitchParameter>(MatNode->MaterialExpression))
						{
							if (const bool* Override = SwitchOverrides.Find(SwitchExpr->ParameterName))
							{
								SwitchExpr->DefaultValue = *Override;
							}
						}
					}
				}
		...
		if (UMaterial* OldParent = PanelOld.MaterialInstance ? PanelOld.MaterialInstance->GetMaterial() : nullptr)
		{
			PanelOld.MaterialGraph = BuildResolvedGraph(DuplicateObject<UMaterial>(OldParent, GetTransientPackage()), PanelOld.MaterialInstance);
		}
		if (UMaterial* NewParent = PanelNew.MaterialInstance ? PanelNew.MaterialInstance->GetMaterial() : nullptr)
		{
			PanelNew.MaterialGraph = BuildResolvedGraph(DuplicateObject<UMaterial>(NewParent, GetTransientPackage()), PanelNew.MaterialInstance);
		}

		// Only display the graph diff if we have two material instances
		if (PanelOld.MaterialInstance && PanelNew.MaterialInstance)
		{
			const bool bSameParent = PanelOld.MaterialInstance->GetMaterial() == PanelNew.MaterialInstance->GetMaterial();
			const bool bHasStaticPermutation = PanelOld.MaterialInstance->bHasStaticPermutationResource || PanelNew.MaterialInstance->bHasStaticPermutationResource;
			bShowGraphDiff = bSameParent && bHasStaticPermutation;
		}
	}
	else
	{
	...
	if (PanelOld.EditorProxy)
	{
		OldAsset = PanelOld.EditorProxy;
	}
	else if (PanelOld.MaterialGraph && PanelOld.MaterialGraph->MaterialFunction)
	{
	...
	if (PanelNew.EditorProxy)
	{
		NewAsset = PanelNew.EditorProxy;
	}
	else if (PanelNew.MaterialGraph && PanelNew.MaterialGraph->MaterialFunction)
	{
				...
				[
					...
					[
						...
						[
							...
							.OnCustomizeDetailsWidget_Raw(this, &SMaterialDiff::AddMaterialInstanceDetailCustomization)
						]
					]
				]
	...
	// Default to Properties tab for instances, Graph tab for materials
	CurrentWidgetIndex = (PanelOld.MaterialInstance || PanelNew.MaterialInstance) ? 1 : 0;
...
void SMaterialDiff::AddMaterialInstanceDetailCustomization(const TSharedRef<IDetailsView>& DetailsView)
{
	if (!PanelOld.EditorProxy && !PanelNew.EditorProxy)
	{
		return;
	}

	// Determine which panel this details view belongs to by checking which EditorProxy it is displaying.
	// SDetailsDiff calls this callback twice: once for the Old side and once for the New side.
	UMaterialEditorInstanceConstant* ProxyForThisView = nullptr;
	...
	{
		UObject* DisplayedObject = SelectedObjects[0].Get();
		if (DisplayedObject == PanelOld.EditorProxy)
		{
			ProxyForThisView = PanelOld.EditorProxy;
		}
		else if (DisplayedObject == PanelNew.EditorProxy)
		{
			ProxyForThisView = PanelNew.EditorProxy;
		}
	}
	...
	FOnGetDetailCustomizationInstance LayoutMICDetails = FOnGetDetailCustomizationInstance::CreateStatic(
		&FMaterialInstanceParameterDetails::MakeInstance,
		ProxyForThisView,
		static_cast<SMaterialLayersFunctionsInstanceWrapper*>(nullptr),
		FGetShowHiddenParameters());

	DetailsView->RegisterInstancedCustomPropertyLayout(UMaterialEditorInstanceConstant::StaticClass(), LayoutMICDetails);
	DetailsView->ForceRefresh();
}

TSharedPtr<SWindow> SMaterialDiff::CreateDiffWindow(FText WindowTitle, TObjectPtr<UMaterialInstance> OldMaterialInstance, TObjectPtr<UMaterialInstance> NewMaterialInstance, const FRevisionInfo& OldRevision, const FRevisionInfo& NewRevision)
{
	TSharedPtr<SWindow> Window = SNew(SWindow)
									 .Title(WindowTitle)
									 .ClientSize(FVector2D(1000, 800));
...
TSharedPtr<SWindow> SMaterialDiff::CreateDiffWindow(TObjectPtr<UMaterialInstance> OldMaterialInstance, TObjectPtr<UMaterialInstance> NewMaterialInstance, const FRevisionInfo& OldRevision, const FRevisionInfo& NewRevision, const UClass* ObjectClass)
{
	...
	FText WindowTitle = FText::Format(LOCTEXT("NamelessMaterialInstanceDiff", "{0} Diff"), ObjectClass->GetDisplayNameText());
	// if we're diffing one asset against itself
	if (bIsSingleAsset)
	{
		// identify the assumed single asset in the window's title
		FString MaterialInstanceName = OldMaterialInstance ? OldMaterialInstance->GetName() : NewMaterialInstance->GetName();
		WindowTitle = FText::Format(LOCTEXT("NamedMaterialInstanceDiff", "{0} - {1} Diff"), FText::FromString(MaterialInstanceName), ObjectClass->GetDisplayNameText());
	}
