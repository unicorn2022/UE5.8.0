...
void FMaterialInstanceParameterDetails::CustomizeDetails(IDetailLayoutBuilder& DetailLayout)
{
	...
	FMaterialEditorValidatorContext ValidationContext;
	// Properties are updated when a static switch is modified so need to run once per update
	if (UMaterialEditorValidationSubsystem* Subsystem = GEditor->GetEditorSubsystem<UMaterialEditorValidationSubsystem>())
	{
		const bool bWarn = !Subsystem->Validate(MaterialEditorInstance->GetMaterialInterface(), ValidationContext);
		if (bWarn)
		{
			// Insert optional validation warning row at the top of the category.
			const bool bHasHyperlink = ValidationContext.HyperlinkAction.IsBound();
			const FSimpleDelegate HyperlinkAction = ValidationContext.HyperlinkAction;
			const FText  HyperlinkText = ValidationContext.HyperlinkText;
			GroupsCategory.AddCustomRow(LOCTEXT("ValidationWarningRow", "Validation Warning"))
			.WholeRowContent()
			[
				...
				[
					SNew(SButton)
					.ContentPadding(FMargin(4.0, 6.0))
					.ToolTipText(HyperlinkText)
					.Visibility(bHasHyperlink ? EVisibility::Visible : EVisibility::Collapsed)
					.OnClicked_Lambda([HyperlinkAction]()
					{
						HyperlinkAction.ExecuteIfBound();
	...
	if (MaterialEditorInstance->bIsFunctionPreviewMaterial)
	{
		...
		DetailLayout.HideProperty("bOverrideToonProfile");
		DetailLayout.HideProperty("ToonProfile");
	...
	else
	{
		...
		{
			DetailLayout.HideProperty("PhysMaterial");
			DetailLayout.HideProperty("bOverridePhysMaterial");

			// Set up the override logic for PhysMaterial
			{
				TAttribute<bool> IsPhysMatOverrideEnabled = TAttribute<bool>::Create(TAttribute<bool>::FGetter::CreateLambda([this]() { return (bool)MaterialEditorInstance->bOverridePhysMaterial; }));
				TSharedRef<IPropertyHandle> PhysMatPropHandle = DetailLayout.GetProperty("PhysMaterial");
				IDetailPropertyRow& PhysMatPropertyRow = DefaultCategory.AddProperty(PhysMatPropHandle);

				FResetToDefaultHandler ResetPhysMatPropertyHandler = FResetToDefaultHandler::CreateLambda([this](TSharedPtr<IPropertyHandle> InHandle) {
					if (MaterialEditorInstance->Parent)
					{
						InHandle->SetValue(MaterialEditorInstance->Parent->GetPhysicalMaterial());
					}
				});

				FResetToDefaultOverride ResetPhysMatPropertyOverride = FResetToDefaultOverride::Create(ResetPhysMatPropertyHandler);

				PhysMatPropertyRow
					.EditCondition(IsPhysMatOverrideEnabled,
						FOnBooleanValueChanged::CreateLambda([this](bool NewValue) {
							MaterialEditorInstance->bOverridePhysMaterial = (uint32)NewValue;
							MaterialEditorInstance->PostEditChange();
							FEditorSupportDelegates::RedrawAllViewports.Broadcast();
							}))
					.OverrideResetToDefault(ResetPhysMatPropertyOverride);
			}
		}
...

		{
			...
			// Set up the override logic for the specular and toon profiles
			if (Substrate::IsSubstrateEnabled())
			{
				TAttribute<bool> IsSpecProfileParamEnabled = TAttribute<bool>::Create(TAttribute<bool>::FGetter::CreateLambda([this](){ return (bool)MaterialEditorInstance->bOverrideSpecularProfile; }));

				IDetailPropertyRow& SpecProfilePropertyRow = MaterialPropertyOverrideGroup.AddPropertyRow(DetailLayout.GetProperty("SpecularProfile"));
				SpecProfilePropertyRow
				.EditCondition(IsSpecProfileParamEnabled,
								FOnBooleanValueChanged::CreateLambda([this](bool NewValue) {
				...
				TAttribute<bool> IsToonProfileParamEnabled = TAttribute<bool>::Create(TAttribute<bool>::FGetter::CreateLambda([this](){ return (bool)MaterialEditorInstance->bOverrideToonProfile; }));

				IDetailPropertyRow& ToonProfilePropertyRow = MaterialPropertyOverrideGroup.AddPropertyRow(DetailLayout.GetProperty("ToonProfile"));
				ToonProfilePropertyRow
				.EditCondition(IsToonProfileParamEnabled,
								FOnBooleanValueChanged::CreateLambda([this](bool NewValue) {
									MaterialEditorInstance->bOverrideToonProfile = (uint32)NewValue;
									MaterialEditorInstance->PostEditChange();
									FEditorSupportDelegates::RedrawAllViewports.Broadcast();
				}))
				.Visibility(TAttribute<EVisibility>::Create(TAttribute<EVisibility>::FGetter::CreateSP(this, &FMaterialInstanceParameterDetails::ShouldShowToonProfile)));
			}
			...
			// Add the usage flag overrides
			CreateUsageFlagOverrideWidgets(DetailLayout, DefaultCategory);
		}
...
void FMaterialInstanceParameterDetails::CreateGroupsWidget(TSharedRef<IPropertyHandle> ParameterGroupsProperty, IDetailCategoryBuilder& GroupsCategory)
{
	...
	struct FGroupData
	{
		...
		TArray<int32> AffectedGroupsIndices;
	};
	TMap<FName, FGroupData> GroupNameToData;
	...
	for (int32 GroupIdx = 0; GroupIdx < MaterialEditorInstance->ParameterGroups.Num(); ++GroupIdx)
	{
		...
		ParameterGroup.GroupName.ToString().ParseIntoArray(GroupParts, TEXT("|"));

		FString Path;
		for (FString Part : GroupParts)
		{
			...
			FGroupData& GroupData = GroupNameToData.FindOrAdd(FName(Path));
			if (!GroupData.Group)
			{
				if (!DetailGroup)
				{
					GroupData.Group = &GroupsCategory.AddGroup(FName(Part), FText::FromString(FName::NameToDisplayString(Part, false)), false, true);
				}
				else
				{
					GroupData.Group = &DetailGroup->AddGroup(FName(Part), FText::FromString(FName::NameToDisplayString(Part, false)), true);
				}
			}

			DetailGroup = GroupData.Group;
			GroupData.AffectedGroupsIndices.Add(GroupIdx);
		}
	...
	for (const TPair<FName, FGroupData>& It : GroupNameToData)
	{
		FUIAction CopyAction(
			FExecuteAction::CreateSP(this, &FMaterialInstanceParameterDetails::OnCopyParameterValues, It.Value.AffectedGroupsIndices),
			FCanExecuteAction::CreateSP(this, &FMaterialInstanceParameterDetails::CanCopyParameterValues, It.Value.AffectedGroupsIndices));
		FUIAction PasteAction(
			FExecuteAction::CreateSP(this, &FMaterialInstanceParameterDetails::OnPasteParameterValues, It.Value.AffectedGroupsIndices),
			FCanExecuteAction::CreateSP(this, &FMaterialInstanceParameterDetails::CanPasteParameterValues, It.Value.AffectedGroupsIndices));
		FDetailWidgetRow& HeaderRow = It.Value.Group->HeaderRow()
			.CopyAction(CopyAction)
			.PasteAction(PasteAction)
			.NameContent()
			[
				SNew(STextBlock)
					.Text(FText::FromName(It.Value.Group->GetGroupName()))
			];

		HeaderRow.AddCustomContextMenuAction(FUIAction(
			FExecuteAction::CreateSP(this, &FMaterialInstanceParameterDetails::EnableGroupParameters, It.Value.AffectedGroupsIndices, true)),
			LOCTEXT("ToggleParametersEnable", "Enable All Parameters"),
			LOCTEXT("ToggleParametersEnableTooltip", "Enable All Parameters in group"),
			FSlateIcon());

		HeaderRow.AddCustomContextMenuAction(FUIAction(
			FExecuteAction::CreateSP(this, &FMaterialInstanceParameterDetails::EnableGroupParameters, It.Value.AffectedGroupsIndices, false)),
...
void FMaterialInstanceParameterDetails::EnableGroupParameters(const TArray<int32> ParameterGroupIndices, const bool bShouldEnable)
{
	...
	for (const int32 ParameterGroupIndex : ParameterGroupIndices)
	{
		if (!MaterialEditorInstance->ParameterGroups.IsValidIndex(ParameterGroupIndex))
		{
			continue;
		}
...
EVisibility FMaterialInstanceParameterDetails::ShouldShowToonProfile() const
{
	const bool bHasProfiles = Substrate::IsSubstrateEnabled() && (MaterialEditorInstance->SourceInstance->NumToonProfile_Internal() > 0 || MaterialEditorInstance->SourceInstance->GetToonProfileOverride_Internal() != nullptr);
	return bHasProfiles ? EVisibility::Visible : EVisibility::Collapsed;
}

void FMaterialInstanceParameterDetails::OnCopyParameterValues(const TArray<int32> ParameterGroupIndices)
{
	if (!MaterialEditorInstance)
	{
		return;
...
bool FMaterialInstanceParameterDetails::CanCopyParameterValues(const TArray<int32> ParameterGroupIndices)
{
	if (!MaterialEditorInstance)
	{
		return false;
...
void FMaterialInstanceParameterDetails::OnPasteParameterValues(const TArray<int32> ParameterGroupIndices)
{
	if (!MaterialEditorInstance)
	{
		return;
...
bool FMaterialInstanceParameterDetails::CanPasteParameterValues(const TArray<int32> ParameterGroupIndices)
{
	// First check the same criteria as copying.
	if (!CanCopyParameterValues(ParameterGroupIndices))
	{
		return false;
	}
...
void FMaterialInstanceParameterDetails::CreateBasePropertyOverrideWidgets(IDetailLayoutBuilder& DetailLayout, IDetailGroup& MaterialPropertyOverrideGroup)
{
	...
	const FText ParameterDisabledToolTipString = LOCTEXT("DisableParameterOverride", "This material instance parent restricts the creation of new shader permutations. Overriding this parameter would result in the generation of additional shader permutations.");
...
void FMaterialInstanceParameterDetails::CreateUsageFlagOverrideWidgets(IDetailLayoutBuilder& DetailLayout, IDetailCategoryBuilder& CategoryBuilder)
{
	static FName GroupName(TEXT("UsageFlagOverrideGroup"));
	IDetailGroup& Group = CategoryBuilder.AddGroup(GroupName, LOCTEXT("UsageFlagOverrideGroup", "Usage Flag Overrides"), false, false);
	TSharedRef<IPropertyHandle> ParentProperty = DetailLayout.GetProperty("BasePropertyOverrides");
	TSharedPtr<IPropertyHandle> OverrideProperty = ParentProperty->GetChildHandle(GET_MEMBER_NAME_CHECKED(FMaterialInstanceBasePropertyOverrides, bOverride_UsageFlags));
	TSharedPtr<IPropertyHandle> UsageProperty = ParentProperty->GetChildHandle(GET_MEMBER_NAME_CHECKED(FMaterialInstanceBasePropertyOverrides, UsageFlags));
	UEnum* UsageEnum = StaticEnum<EMaterialUsage>();
	
	// Disable overrides for case where parent material instance is locked.
	const bool bIsWidgetDisabled = MaterialEditorInstance->SourceInstance->bDisallowStaticParameterPermutations;
	const FText DisabledToolTipText = LOCTEXT("DisableUsageOverride", "This material instance parent restricts the creation of new shader permutations. Overriding this usage flag would result in the generation of additional shader permutations.");

	for (int32 UsageIndex = 0; UsageIndex < MATUSAGE_MAX; UsageIndex++)
	{
		const FText UsageName = FText::FromString(UMaterialInterface::GetUsageName((EMaterialUsage)UsageIndex));
		const FText UsageToolTip = bIsWidgetDisabled ? DisabledToolTipText : FText::FromString(UsageEnum->GetMetaData(TEXT("ToolTip"), UsageIndex));

		Group.AddWidgetRow()
		.PropertyHandleList({UsageProperty})
		.IsEnabled(!bIsWidgetDisabled)
		.NameContent()
		[
			SNew(STextBlock)
			.Text(UsageName)
			.Font(IDetailLayoutBuilder::GetDetailFont())
			.ToolTip(SNew(SToolTip).Text(UsageToolTip))
		]
		.ValueContent()
		[
			SNew(SCheckBox)
			.IsChecked_Lambda([UsageProperty, UsageIndex]() -> ECheckBoxState
			{
				uint32 Value = 0;
				UsageProperty->GetValue(Value);
				return (Value & (1u << UsageIndex)) != 0 ? ECheckBoxState::Checked : ECheckBoxState::Unchecked;
			})
			.OnCheckStateChanged_Lambda([UsageProperty, UsageIndex](ECheckBoxState InState)
			{
				uint32 Value = 0;
				UsageProperty->GetValue(Value);
				...
				UsageProperty->SetValue(Value);
			})
		]
		.EditCondition(
			TAttribute<bool>::CreateLambda([OverrideProperty, UsageIndex]
			{
				uint32 Value = 0;
				OverrideProperty->GetValue(Value);
				return (Value & (1u << UsageIndex)) != 0;
			}),
			FOnBooleanValueChanged::CreateLambda([OverrideProperty, UsageIndex, bIsWidgetDisabled](bool bNewValue)
			{
				// Edit condition doesn't have a disabled flag and doesn't respect the row setting. So ignore changes when we are disabled.
				if (bIsWidgetDisabled)
				{
					return;
				}
				...
				OverrideProperty->SetValue(Value);
			})
		)
		.OverrideResetToDefault(FResetToDefaultOverride::Create(
			FIsResetToDefaultVisible::CreateLambda([this, UsageIndex](TSharedPtr<IPropertyHandle>)
			{
				TObjectPtr<UMaterialInterface> Material = MaterialEditorInstance->GetMaterialInterface();
				TObjectPtr<UMaterialInterface> ParentMaterial = MaterialEditorInstance->GetParentMaterialInterface();
				if (!Material || !ParentMaterial)
				{
					return false;
				}
				return Material->GetUsageByFlag((EMaterialUsage)UsageIndex) != ParentMaterial->GetUsageByFlag((EMaterialUsage)UsageIndex);
			}), 
			FResetToDefaultHandler::CreateLambda([UsageProperty, UsageIndex](TSharedPtr<IPropertyHandle>)
			{
				// Simply toggling the bit is enough here since we can call this after the
				// IsResetToDefaultVisible above returns that we are different to the parent
				uint32 Value = 0;
				UsageProperty->GetValue(Value);
				Value ^= (1u << UsageIndex);
				UsageProperty->SetValue(Value);
