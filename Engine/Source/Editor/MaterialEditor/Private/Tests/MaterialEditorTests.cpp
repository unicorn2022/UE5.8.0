...
namespace MaterialEditorPromotionTestUtils
{
	...
	*/
	static bool AssignNormalToMaterial(UTexture* InNormalTexture, UMaterial* InMaterial)
	{
		...
		{
			UE_LOGF(LogEditorMaterialEditorPromotionTests, Display, "Created a new texture sample expression");
			...
			UE_LOGF(LogEditorMaterialEditorPromotionTests, Display, "Assigned the normal map texture to the new node");
			...
			UE_LOGF(LogEditorMaterialEditorPromotionTests, Display, "Connected the new node to the normal pin");
		...
		{
			UE_LOGF(LogEditorMaterialEditorPromotionTests, Error, "Could not add a texture sample to %ls", *InMaterial->GetName());
			return true;
		}
	}
	...
	private:
		...
		*/
		bool ContentBrowser_CreateAMaterial_Part1()
		{
			...
			{
				UE_LOGF(LogEditorMaterialEditorPromotionTests, Warning, "Skipping material creation test: No texture asset defined.");
				return true;
			}
			...
			if (DiffuseTexture)
			{
				...
				if (CreatedMaterial)
				{
					UE_LOGF(LogEditorMaterialEditorPromotionTests, Display, "Created new material (%ls) from texture (%ls)", *CreatedMaterial->GetName(), *DiffuseTexture->GetName());
					// @TODO: Break out TEST 2: Open a material
					GEditor->GetEditorSubsystem<UAssetEditorSubsystem>()->OpenEditorForAsset(CreatedMaterial);
					UE_LOGF(LogEditorMaterialEditorPromotionTests, Display, "Opened the material editor");
				}
				else
				{
					UE_LOGF(LogEditorMaterialEditorPromotionTests, Error, "Failed to create material fom texture");
				}
			}
			else
			{
				UE_LOGF(LogEditorMaterialEditorPromotionTests, Error, "Failed to load texture asset");
			}
		...
		*/
		...
		{ 
			if (CreatedMaterial)
			{
				...
				{
					UE_LOGF(LogEditorMaterialEditorPromotionTests, Warning, "Skipping material change test: No normal texture asset defined.");
					return true;
				}
				...
				if (NormalTexture)
				{
					...
					if (MaterialEditorPromotionTestUtils::AssignNormalToMaterial(NormalTexture, CreatedMaterial))
					{
						...
						UE_LOGF(LogEditorMaterialEditorPromotionTests, Display, "Compiled the new material");
				...
				{
					UE_LOGF(LogEditorMaterialEditorPromotionTests, Error, "Failed to load normal texture asset");
				}
			}
			else
			{
				UE_LOGF(LogEditorMaterialEditorPromotionTests, Warning, "Skipping material change test: Previous test step either did not run, or did not succeed.");
			}
