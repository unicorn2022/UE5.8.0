...
/** Helper struct for containing data for the material slot submenu UI and where to apply the preview material */
struct MaterialSlotPreviewInfo
{
	FName SlotName;
	TObjectPtr<UMaterialInterface> OriginalMaterial;
	mutable bool bSlotEnabled;
};
...
public:
	...
	void OnSetPreviewMeshMaterialSlots();
	bool DoesPreviewMeshContainSlots() const;
...
protected:
	...
	TSharedRef<SWidget> GenerateMaterialSlotMenu();
	...
	//~ Begin SWidget
	virtual FReply OnDrop(const FGeometry& InGeometry, const FDragDropEvent& InEvent) override;
	//~ End SWidget
...
private:
	...
	/** Helper function for applying state logic to all material slots + updates*/
	void ApplyStateToAllSlots(bool bState);

	/** Set true for usage to all material slots */
	void OnSelectAllSlots();

	/** Set false for usage to all material slots */
	void OnSelectNoSlots();

	/** Updates the scene preview mesh when a checkbox state changes */
	void OnMaterialSlotCheckStateChanged(ECheckBoxState NewState, int32 SlotIndex);

	/** Returns if a material slot is checked/should show the preview material */
	ECheckBoxState IsMaterialSlotChecked(int32 SlotIndex) const;
	...
	/** Material Slot data for populating and processing the material slot selections */
	TArray<const MaterialSlotPreviewInfo> MaterialSlots;
};
