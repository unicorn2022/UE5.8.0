...
double STimeRangeSlider::ComputeDragDelta(const FPointerEvent& MouseEvent, double GeometryWidth) const
{
	...
	const double Width = (GeometryWidth - TimeRangeSliderConstants::HandleSize * 2);
	const double PixelToUnits = FMath::IsNearlyZero(Width) ? 0 : (EndTime - StartTime) / Width;
...
FReply STimeRangeSlider::OnMouseMove( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
	if (HasMouseCapture())
	{
		double DragDelta = ComputeDragDelta(MouseEvent, MyGeometry.GetAbsoluteSize().X);
