...
void LogAndToastWarning(const FText& Error)
{
	...
	UE_LOGF(LogClothingAssetFactory, Warning, "%ls", *Error.ToString());
}
...
UClothingAssetBase* UClothingAssetFactory::CreateFromSkeletalMesh(USkeletalMesh* TargetMesh, FSkeletalMeshClothBuildParams& Params)
{
	...
	const FString SanitizedName = ObjectTools::SanitizeObjectName(Params.AssetName);
	const FName ObjectName = StaticFindObject(UClothingAssetBase::StaticClass(), TargetMesh, *SanitizedName) ?
		MakeUniqueObjectName(TargetMesh, UClothingAssetBase::StaticClass(), FName(SanitizedName)) :
		FName(SanitizedName);
...
UClothingAssetBase* UClothingAssetFactory::CreateFromExistingCloth(USkeletalMesh* TargetMesh, const USkeletalMesh* /*SourceMesh*/, const UClothingAssetBase* SourceAsset) const
{
	...
	const FName NewAssetName = StaticFindObject(UClothingAssetBase::StaticClass(), TargetMesh, *SourceClothingAsset->GetName()) ?
		MakeUniqueObjectName(TargetMesh, UClothingAssetBase::StaticClass(), SourceClothingAsset->GetFName()) :
		SourceClothingAsset->GetFName();
