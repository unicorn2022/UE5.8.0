...
public:
	...
	*/
	...
	void ForEachNodeImpl(TFunctionRef<void(FSubobjectEditorTreeNode& Data)> Ftor);
	void DisposeUnsafe();
protected: 
...
protected:
	...
	UE_API virtual ~SSubobjectEditor();
	...
	/** Create and parent the Editor Scope Tree row.  Call once from derived Construct(). */
	UE_API void InitScopeRow(FStringView Label);

	/** Context row owned by this subobject editor for Editor Scope Tree.
	 *  Created by InitScopeRow(), parented to the current context at that time.
	 *  Pushed onto TLS in event handlers so property visibility checks see the correct editing scope. */
	UE::Editor::DataStorage::RowHandle ScopeRow = UE::Editor::DataStorage::InvalidRowHandle;
...
public:
	UE_API void SetSelectionSet(UTypedElementSelectionSet* InSelectionSet);
	...
	UE_API virtual void UpdateTree(bool bRegenerateTreeNodes = true);
	...
	/** 
	 * Called before a potential change of subobject editor target, so that customizations can be
	 *  given a chance to gather context they might need for a subsequent SelectDefaultSubobject call.
	 */
	UE_EXPERIMENTAL(5.8, "This function is experimental for now.")
	UE_API void PrePotentialTargetChange();

	/** Select the default subobject, which is typically the root, but may be adjusted by a customization*/
	UE_EXPERIMENTAL(5.8, "This function is experimental for now.")
	UE_API void SelectDefaultSubobject();
	...
	/** 
	 * Returns whether the editor wants to be visible, which can be optionally used by the containing
	 *  widget. If not overriden by a customization, this will be Collapsed or Visible depending on 
	 *  whether ObjectContext was null or not, respectively, but a customization can drive this by
	 *  some other state.
	 */
	UE_API EVisibility GetDesiredVisibility() const;
	...
	UE_API FReply TryHandleAssetDragDropOperation(const FDragDropEvent& InDragDropEvent, FSubobjectEditorTreeNodePtrType InTargetItem);
	...
	*/
	...
	/** Get the SubobjectData for the current context represented by the subobject editor
	 * This is a newer API than GetObjectContext + GetObjectContextHandle.  It is introduced to allow
	 * access to non-UObject based data and for cases where SubobjectData is a proxy for multiple objects
	 * in bulk-edit cases.
	 */
	UE_EXPERIMENTAL(5.8, "This function is experimental for now.")
	UE_API FSubobjectData* GetDataContextHandle() const;
...
protected:
	friend class FComponentElementSubobjectEditorCommonActionsCustomization;
	...
	void ForEachNode(TFunctionRef<void(FSubobjectEditorTreeNode& Data)> Ftor);
private:
