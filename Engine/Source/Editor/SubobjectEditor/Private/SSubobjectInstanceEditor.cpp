...
void SSubobjectInstanceEditor::Construct(const FArguments& InArgs)
{
	...
	InitScopeRow(TEXT("Instance Subobject Editor"));
	...
	[
		...
		+ SOverlay::Slot()
		[
			SAssignNew(TreeViewOverlay, SBox)
			.Visibility_Lambda([this]()
			{
				const bool bTreeIsVisible = GetComponentsTreeVisibility() == EVisibility::Visible;
				const bool bSupportsOverlay = UICustomization && UICustomization->SupportsTreeViewOverlay(*this);
				
				if (bTreeIsVisible && bSupportsOverlay)
				{
					// TODO: Could add more criteria like whether the thing we're dragging can actually be dropped here
					const bool bIsDragging = FSlateApplication::Get().IsDragDropping();
					return bIsDragging ? EVisibility::Visible : EVisibility::HitTestInvisible;
				} 
			...
			.Content()
			[
				UICustomization ? UICustomization->ConstructTreeViewOverlay(SharedThis(this)) : SNullWidget::NullWidget
			]
...
void SSubobjectInstanceEditor::UpdateTree(bool bRegenerateTreeNodes)
{
	SSubobjectEditor::UpdateTree(bRegenerateTreeNodes);

	if (TreeViewOverlay.IsValid())
	{
		if (UICustomization && UICustomization->SupportsTreeViewOverlay(*this))
		{
			TreeViewOverlay->SetContent(UICustomization->ConstructTreeViewOverlay(SharedThis(this)));
		}
	}
}
