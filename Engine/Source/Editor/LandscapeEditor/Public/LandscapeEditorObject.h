...
struct FGizmoImportLayer
{
	...
	UPROPERTY(Category="Import", EditAnywhere, meta = (DisplayName = "Target Layer Filename"))
	FString LayerFilename;

	UPROPERTY(Category="Import", EditAnywhere, meta = (DisplayName = "Target Layer Name", ToolTip = "Layer Name must match an existing target layer on the Landscape or the import will fail."))
	FString LayerName;
...
UENUM()
enum class ELandscapeWeightBlendPaintMode
{
	/** 
	 * Exclusive Mode for weight-blended layers will be active only when the Ctrl key is held. Non-Exclusive Mode is used otherwise.
	 * Note : this mode is incompatible with LandscapeEditorControlType "RequireCtrl" since the Ctrl key is then reserved for activating the landscape tools in that case. Use one of the manual modes in that case. 
	 */
	ExclusiveRequiresCtrl UMETA(DisplayName = "Hold Ctrl"),

	/** 
	 * Exclusive Mode for weight-blended layers will be active only when the Ctrl key is not held. Non-Exclusive Mode is used otherwise.
	 * Note : this mode is incompatible with LandscapeEditorControlType "RequireNoCtrl" since the Ctrl key is then reserved for activating the landscape tools in that case. Use one of the manual modes in that case. 
	 */
	ExclusiveRequiresNoCtrl UMETA(DisplayName = "Don't Hold Ctrl"),

	/** Force use Exclusive Mode for weight-blended layers. The Ctrl key is ineffective in that mode. */
	ForceExclusive UMETA(DisplayName = "Exclusive"),

	/** Force use Non-Exclusive Mode for weight-blended layers. The Ctrl key is ineffective in that mode. */
	ForceNonExclusive UMETA(DisplayName = "Non-Exclusive"),
};
...
class ULandscapeEditorObject : public UObject
{
	...
	// Allows to snap the copied/imported data to a grid
	...
	UPROPERTY(Category="Tool Settings", EditAnywhere, NonTransactional, AdvancedDisplay, meta=(DisplayName="Target Layers", ShowForTools="CopyPaste"))
	...
	// Describes the transform that should be applied to the import data 
...
public:
	 ...
	 * Use the '=' key and activate the landscape brush to add the selected layer to the painted components' allow list. 
	 * Use the '-' key and activate the landscape brush to remove the selected layer from the painted components' allow list. 
	 */ 
	...
	/*
	 * Defines how to activate the Exclusive Paint mode for weight-blended target layers.
	 * Exclusive Paint mode allows all weight-blended target layers from the same blend group to be painted in the same brush stroke. 
	 * The selected target layer is painted while the unselected ones are "inversely" painted (erased). 
	 * This allows the painted target layer to "override" the other ones instead of being merely being "blended" with them. It therefore (destructively) removes the weight from the other target layers. 
	 */
	UPROPERTY(Category = "Target Layers", EditAnywhere, NonTransactional, meta = (DisplayName = "Exclusive Paint Mode", ShowForTools="Paint", ShowForTargetTypes = "Weightmap"))
	ELandscapeWeightBlendPaintMode WeightBlendedTargetLayerPaintMode = ELandscapeWeightBlendPaintMode::ExclusiveRequiresNoCtrl;
