...
namespace
{
	void WriteMetaData(const TCHAR* InMapFilename, FLandscapeFileResolution InDataResolution, int32 InBitsPerPixel)
	{
		...
		JsonObject->SetField(TEXT("width"), MakeShared<FJsonValueNumber>(InDataResolution.Width));
		JsonObject->SetField(TEXT("height"), MakeShared<FJsonValueNumber>(InDataResolution.Height));
		JsonObject->SetField(TEXT("bpp"), MakeShared<FJsonValueNumber>(InBitsPerPixel));
