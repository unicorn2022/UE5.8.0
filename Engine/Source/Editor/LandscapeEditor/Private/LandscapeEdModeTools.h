...
public:
	...
	TLandscapeEditCache(ULandscapeInfo* InLandscapeInfo, ULandscapeLayerInfoObject* InLayerInfo)
		: DataAccess(InLandscapeInfo, InLayerInfo)
	...
	bool GetDataAndCacheWithVisibilityOverride(int32 X1, int32 Y1, int32 X2, int32 Y2, TArray<AccessorType>& OutData, TBitArray<> LayerVisibility, ALandscape* Landscape)
	{
		...
		if (!HasCachedData(X1, Y1, X2, Y2))
		{
			...
			// The bounds we calculate here need to be what would be the result of calling CacheData with this region, meaning that they should include the previous bounds
...
#if 0
			UE_LOGF(LogLandscapeTools, Warning, "PartialRender - New ---- (fr%u)  Base [%d, %d]-[%d, %d]  s[%d, %d],   Padded [%d, %d]-[%d, %d] s[%d, %d],   Evaluate [%d, %d]-[%d, %d] s[%d, %d]", GFrameNumber,
...
{
	FHeightmapAccessorTool(ULandscapeInfo* InLandscapeInfo, ULandscapeLayerInfoObject* InLayerInfo = nullptr)
	:	FHeightmapAccessor<bInUseInterp>(InLandscapeInfo, InLayerInfo)
	{
		// We don't care about InLayerInfo since heightmaps are not a target layer per se
		check(InLayerInfo == nullptr);
	}
};

struct FLandscapeHeightCache : public TLandscapeEditCache<FHeightmapAccessorTool</*bInUseInterp = */true>, uint16>
{
	static uint16 ClampValue(int32 Value) { return static_cast<uint16>(FMath::Clamp(Value, 0, LandscapeDataAccess::MaxValue)); }

	FLandscapeHeightCache(ULandscapeInfo* InLandscapeInfo, ULandscapeLayerInfoObject* InLayerInfo = nullptr)
		: TLandscapeEditCache<FHeightmapAccessorTool</*bInUseInterp = */true>, uint16>(InLandscapeInfo, /*InLayerInfo = */nullptr)
	{
		// We don't care about InLayerInfo since heightmaps are not a target layer per se
		check(InLayerInfo == nullptr);
	}
...
struct FLandscapeAlphaCache : public TLandscapeEditCache<TAlphamapAccessorTool</*bInUseInterp = */true>, uint8>
{
	...
	FLandscapeAlphaCache(ULandscapeInfo* InLandscapeInfo, ULandscapeLayerInfoObject* InLayerInfo)
		: TLandscapeEditCache<TAlphamapAccessorTool</*bInUseInterp = */true>, uint8>(InLandscapeInfo, InLayerInfo)
	{
	}
};
...
struct FLandscapeVisCache : public TLandscapeEditCache<TAlphamapAccessorTool</*bInUseInterp = */false>, uint8>
{
	...
	FLandscapeVisCache(ULandscapeInfo* InLandscapeInfo)
		: TLandscapeEditCache<TAlphamapAccessorTool</*bInUseInterp = */false>, uint8>(InLandscapeInfo, ALandscapeProxy::VisibilityLayer)
	{
	}
};

// Allows to cache data from the combined edit layer stack (from the bottom of the stack up to the specified edit layer if bCombinedLayerOperation, 
//  normal cache operation otherwise
template<class ToolTarget>
class FLandscapeEditLayerStackDataCache
{
public:
	FLandscapeEditLayerStackDataCache(ULandscapeInfo* InLandscapeInfo, ToolTarget::CacheClass& Cache)
		: LandscapeInfo(InLandscapeInfo)
		, Landscape(nullptr)
		, EditingLayerGuid()
		...
		, bTargetIsHeightmap(ToolTarget::TargetType == ELandscapeToolTargetType::Heightmap)
		, CacheUpToEditingLayer(Cache)
		, CacheBottomLayers(InLandscapeInfo, Cache.LayerInfo)
	{
	}
	...
	void Initialize(bool bInCombinedLayerOperation)
	{
		...
		if (!bIsInitialized)
		{
			...
			bCombinedLayerOperation = Landscape && bInCombinedLayerOperation && bTargetIsHeightmap;
...
struct FFullWeightmapAccessor
{
	...
	FFullWeightmapAccessor(ULandscapeInfo* InLandscapeInfo, ULandscapeLayerInfoObject* InLayerInfo)
		: LandscapeInfo(InLandscapeInfo)
		, LandscapeEdit(InLandscapeInfo)
	{
		// We don't care about InLayerInfo, actually, since we access all target layers here
	}
...
{
	FLandscapeFullWeightCache(ULandscapeInfo* InLandscapeInfo)
		: TLandscapeEditCache<FFullWeightmapAccessor</*bInUseInterp = */false>, TArray<uint8>>(InLandscapeInfo, /*InLayerInfo = */ nullptr)
	{
	}
	...
	void AddDirtyLayer(ULandscapeLayerInfoObject* InLayerInfo)
	{
		DataAccess.DirtyLayerInfos.Add(InLayerInfo);
	}
};
...
{
	enum { bUseInterp = bInUseInterp };
	FDatamapAccessor(ULandscapeInfo* InLandscapeInfo, ULandscapeLayerInfoObject* InLayerInfo = nullptr)
		: LandscapeEdit(InLandscapeInfo)
	{
		// We don't care about InLayerInfo since data map is not a target layer per se
		check(InLayerInfo == nullptr);
	}
...
struct FLandscapeDataCache : public TLandscapeEditCache<FDatamapAccessor</*bInUseInterp = */false>, uint8>
{
	static uint8 ClampValue(int32 Value) { return static_cast<uint8>(FMath::Clamp(Value, 0, 255)); }

	FLandscapeDataCache(ULandscapeInfo* InLandscapeInfo)
		: TLandscapeEditCache<FDatamapAccessor</*bInUseInterp = */false>, uint8>(InLandscapeInfo, /*InLayerInfo = */ nullptr)
	{
	}
};
...
public:
	...
	{
		check(InEdMode->CurrentTool != nullptr);
		ELandscapeToolTargetType SupportedTargetType = InEdMode->CurrentTool->GetSupportedTargetType();
		bUseContinuousApply = (SupportedTargetType == ELandscapeToolTargetType::Weightmap) ? InEdMode->UISettings->bApplyWithoutMovingPaint : InEdMode->UISettings->bApplyWithoutMovingSculpt;
	}
	...
	bool GetUseContinuousApply() const
	{
		return bUseContinuousApply;
	}
...
public:
	...
	{
		bool bUpdateHeightmap = this->EdMode->CurrentToolTarget.TargetType == ELandscapeToolTargetType::Heightmap;
		return bUpdateHeightmap ? ELandscapeLayerUpdateMode::Update_Heightmap_All_Modes : ELandscapeLayerUpdateMode::Update_Weightmap_All_Modes;
	}
	...
	virtual bool BeginTool(FEditorViewportClient* ViewportClient, const FLandscapeToolTarget& InTarget, const FVector& InHitLocation) override
	{
		...
		InteractorPositions.Emplace(LastInteractorPosition, IsModifierPressed(ViewportClient), IsAlternateModifierPressed(ViewportClient));
		TimeSinceLastInteractorMove = 0.0f;
	...
	virtual void Tick(FEditorViewportClient* ViewportClient, float DeltaTime) override
	{
		if (IsToolActive())
		{
			...
			else if (ToolStroke->GetUseContinuousApply() && TimeSinceLastInteractorMove >= 0.25f)
			{
				InteractorPositions.Emplace(LastInteractorPosition, IsModifierPressed(ViewportClient), IsAlternateModifierPressed(ViewportClient));
				ToolStroke->Apply(ViewportClient, EdMode->CurrentBrush, EdMode->UISettings, InteractorPositions);
				ViewportClient->Invalidate(false, false);
...
protected:
	...
	bool IsModifierPressed(const class FEditorViewportClient* ViewportClient)
	{
		UE_LOGF(LogLandscapeTools, VeryVerbose, "ViewportClient = %d, IsModifierDown = %d", (ViewportClient != nullptr), (ViewportClient != nullptr && IsShiftDown(ViewportClient->Viewport)));
		return ViewportClient != nullptr && IsShiftDown(ViewportClient->Viewport);
	}

	bool IsAlternateModifierPressed(const FEditorViewportClient* ViewportClient)
	{
		UE_LOGF(LogLandscapeTools, VeryVerbose, "ViewportClient = %d, IsAlternateModifierDown = %d", (ViewportClient != nullptr), (ViewportClient != nullptr && IsCtrlDown(ViewportClient->Viewport)));
		return ViewportClient != nullptr && IsCtrlDown(ViewportClient->Viewport);
	}
};
