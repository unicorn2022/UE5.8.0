...
{
	FString FullyQualifiedFileName = FPaths::ConvertRelativePathToFull(FString(InImageFilename));

	FCacheEntry* CacheEntry = ChooseCache<T>().Find(FullyQualifiedFileName);
	...
	Add<T>(FullyQualifiedFileName, OutImageData);
...
void FLandscapeImageFileCache::MonitorCallback(const TArray<struct FFileChangeData>& Changes)
{
	for (const FFileChangeData& Change : Changes)
	{
		...
		{
			// The file cache expects path to be fully qualified
			FString FullyQualifiedFileName = FPaths::ConvertRelativePathToFull(Change.Filename);
			Remove(FullyQualifiedFileName);
		}
	}
}

bool FLandscapeImageFileCache::MonitorFile(const FString& InFullyQualifiedFileName)
{
	check(!FPaths::IsRelative(InFullyQualifiedFileName));

	FString Directory = FPaths::GetPath(InFullyQualifiedFileName);
...
void FLandscapeImageFileCache::UnmonitorFile(const FString& InFullyQualifiedFileName)
{
	check(!FPaths::IsRelative(InFullyQualifiedFileName));
...
template <typename T>
void FLandscapeImageFileCache::Add(const FString& InFullyQualifiedFileName, FLandscapeImageDataRef NewImageData)
{
	check(!FPaths::IsRelative(InFullyQualifiedFileName));

	CacheType& Cache = ChooseCache<T>();
	check(Cache.Find(InFullyQualifiedFileName) == nullptr);

	Cache.Add(InFullyQualifiedFileName, FCacheEntry(NewImageData));
	MonitorFile(InFullyQualifiedFileName);
	CacheSize += NewImageData.Data->Num();
}

void FLandscapeImageFileCache::Remove(const FString& InFullyQualifiedFileName)
{
	check(!FPaths::IsRelative(InFullyQualifiedFileName));

	for (CacheType& Cache : CachedImages)
	{
		if (FCacheEntry* CacheEntry = Cache.Find(InFullyQualifiedFileName))
		{
			CacheSize -= CacheEntry->ImageData.Data->Num();
			Cache.Remove(InFullyQualifiedFileName);
			UnmonitorFile(InFullyQualifiedFileName);
		}
	}
}
