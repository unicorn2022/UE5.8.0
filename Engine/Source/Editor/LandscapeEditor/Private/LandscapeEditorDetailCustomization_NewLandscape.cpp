...
void FLandscapeEditorDetailCustomization_NewLandscape::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	[
		...
		[
			...
			.ToolTipText_Static(&GetPropertyValueText, PropertyHandle_HeightmapFilename)
			[
	...
	[
		...
		[
			...
			.ToolTipText(LOCTEXT("ImportResolutionTooltip", "Possible resolutions for the selected image"))
		]
	]
	.ValueContent()
	[
	...
	[
		...
		[
			...
			.ToolTipText(LOCTEXT("FillWorldTooltip", "Automatically set the component count such that the landscape fills the entire world (up to the maximum supported landscape resolution)"))
		...
		[
			...
			.ToolTipText(LOCTEXT("FitToDataTooltip", "Try to find a section size and number of sections that exactly matches the dimensions of the import data"))
		...
		[
			...
			.ToolTipText(LOCTEXT("CreateTooltip", "Create the landscape based on the currently selected parameters"))
		...
		[
			...
			.ToolTipText(LOCTEXT("ImportTooltip", "Create the landscape based on the currently selected parameters and applies the import data onto it"))
			.OnClicked(this, &FLandscapeEditorDetailCustomization_NewLandscape::OnCreateButtonClicked)
			.IsEnabled(this, &FLandscapeEditorDetailCustomization_NewLandscape::GetImportButtonIsEnabled)
		]
	];
...
void FLandscapeEditorDetailCustomization_NewLandscape::AddComponents(ULandscapeInfo* InLandscapeInfo, ULandscapeSubsystem* InLandscapeSubsystem, const TArray<FIntPoint>& InComponentCoordinates, TArray<ALandscapeProxy*>& OutCreatedStreamingProxies)
{
	...
	for (const FIntPoint& ComponentCoordinate : InComponentCoordinates)
	{
		...
		HeightData.Init(DefaultHeightColor, FMath::Square(ComponentVerts));
...
FReply FLandscapeEditorDetailCustomization_NewLandscape::OnCreateButtonClicked()
{
	...
	if (bIsTempPackage && bNeedsLandscapeRegions)
	{
		...
		{
			UE_LOGF(LogLandscapeTools, Error, "Unable to save current level");
			return FReply::Handled();
		}
	}
	...
	{
		UE_LOGF(LogLandscapeTools, Error, "Unable to import weight maps");
		return FReply::Handled();
	}
