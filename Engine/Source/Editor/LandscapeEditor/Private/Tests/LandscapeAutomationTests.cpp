...
namespace LandscapeTestUtils
{
	...
	*/
	...
	{
		UE_LOGF(LogLandscapeAutomationTests, Display, "Creating a new map...");
		UWorld* NewMap = FAutomationEditorCommonUtils::CreateNewMap();
		if (!NewMap)
		{
			UE_LOGF(LogLandscapeAutomationTests, Error, "Unable to create new map");
			return false;
		}

		UE_LOGF(LogLandscapeAutomationTests, Display, "Switching to Landscape Editor Mode...");
		GLevelEditorModeTools().ActivateMode(FBuiltinEditorModes::EM_Landscape);
		...
		{
			UE_LOGF(LogLandscapeAutomationTests, Error, "Unable to enter Landscape Edit Mode");
			return false;
		}

		UE_LOGF(LogLandscapeAutomationTests, Display, "Creating a new landscape...");
		...
		{
			UE_LOGF(LogLandscapeAutomationTests, Error, "Unable to create a new landscape");
			return false;
		}
...
bool FBeginModifyLandscapeCommand::Update()
{
	...
	UE_LOGF(LogLandscapeAutomationTests, Display, "Modified the landscape using the sculpt tool");
...
bool FLandscapeEditorCreateNewLayer::RunTest(const FString& Parameters)
{
	...
	UE_LOGF(LogLandscapeAutomationTests, Display, "Verifying that new landscapes start with 1 layer...");
	FEdModeLandscape* LandscapeEditMode = (FEdModeLandscape*)GLevelEditorModeTools().GetActiveMode(FBuiltinEditorModes::EM_Landscape);
	...
	UE_LOGF(LogLandscapeAutomationTests, Display, "Verifying that we successfully added a new layer...");
