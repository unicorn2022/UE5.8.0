...
TEST_CLASS_WITH_FLAGS(LandscapeCoreWorkflow, "Editor.Landscape", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
{
	...
	static ALandscape* CreateLandscape()
	{
		...
		{
			UE_LOGF(LandscapeTestsLog, Error, "Failed to retrieve the Landscape Edit Mode.");
			return nullptr;
		}
		...
		// The tests expect the default landscape material, not the cached material from the editor ini.
		if (const ULandscapeSettings* LandscapeSettings = GetDefault<ULandscapeSettings>())
		{
			LandscapeEditMode->UISettings->NewLandscape_Material = LandscapeSettings->GetDefaultLandscapeMaterial().LoadSynchronous();
		}
		...
		{
			UE_LOGF(LandscapeTestsLog, Error, "Create button click event was not handled successfully.");
			return nullptr;
		}
		...
		UE_LOGF(LandscapeTestsLog, Error, "No landscape was created in the viewport.");
		return nullptr;
	}
};
