...
void ULandscapeSplineSelection::DeselectControlPoint(ULandscapeSplineControlPoint* ControlPoint, ESplineNavigationFlags Flags)
{
	...
	// Update the Spline Selection state. Selection was previously disabled if multiple splines/linear paths were selected 
	// After deselecting, update the linear path to see if the selection is now valid
	SelectedSplineControlPoints.Remove(ControlPoint);
	if (!SelectedSplineControlPoints.IsEmpty())
	{
...
void ULandscapeSplineSelection::DeSelectSegment(ULandscapeSplineSegment* Segment, ESplineNavigationFlags Flags)
{
	...
	if (!SelectedSplineSegments.IsEmpty())
	{
		BuildLinearPathFromLastSelectedPointInternal();
	}
...
bool ULandscapeSplineSelection::HasAdjacentSegmentInLinearPath(ESplineNavigationFlags Flags) const
{
	...
	// Early out when the current selection disables navigation/selection (GetAdjacentSegmentInLinearPath reqiures a valid linear path)
	if (!IsSelectionValidForNavigation())
	{
		return false;
	}
...
bool ULandscapeSplineSelection::HasAdjacentControlPointInLinearPath(ESplineNavigationFlags Flags) const
{
	...
	// Early out when the current selection disables navigation/selection (GetAdjacentControlPointInPath reqiures a valid linear path)
	if (!IsSelectionValidForNavigation())
	{
		return false;
	}
