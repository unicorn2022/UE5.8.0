...
protected:
	...
	// TODO [jonathan.bard] : This should move one level up (in FLandscapeBrush), so that FLandscapeBrushComponent can also disable painting on the edges and with the allow list
	bool bCanPaint;
...
public:
	...
	virtual FLandscapeBrush::FOverlapInfo GetOverlapInfo(const TArray<FLandscapeToolInteractorPosition>& InInteractorPositions) const override
	{
		ULandscapeInfo* LandscapeInfo = EdMode->CurrentToolTarget.LandscapeInfo.Get();
		const float ScaleXY = static_cast<float>(FMath::Abs(LandscapeInfo->DrawScale.X));
		const float TotalRadius = EdMode->UISettings->GetCurrentToolBrushRadius() / ScaleXY;

		TArray<FLandscapeToolInteractorPosition> InteractorPositions = CapInteractorPositions(InInteractorPositions);
		FIntRect ExclusiveBounds;
		TSet<ULandscapeComponent*> OverlappedComponents;
		for (const FLandscapeToolInteractorPosition& InteractorPosition : InteractorPositions)
		{
			...
			TSet<ULandscapeComponent*> SpotOverlappedComponents;
			LandscapeInfo->GetComponentsInRegion(SpotExclusiveBounds.Min.X + 1, SpotExclusiveBounds.Min.Y + 1, SpotExclusiveBounds.Max.X - 1, SpotExclusiveBounds.Max.Y - 1, SpotOverlappedComponents);
			OverlappedComponents.Append(SpotOverlappedComponents);
		}
		...
		if (!ensure(LandscapeInfo->GetLandscapeExtent(MinX, MinY, MaxX, MaxY)))
		{
			...
			return FLandscapeBrush::FOverlapInfo();
		}
		...
		FLandscapeBrush::FOverlapInfo Result;
		Result.ExclusiveBounds = ExclusiveBounds;
		Result.OverlappedLoadedComponents = OverlappedComponents;
		...
		Result.bOverlapsUnloadedComponents = LandscapeInfo->HasUnloadedComponentsInRegion(ExclusiveBounds.Min.X, ExclusiveBounds.Min.Y, ExclusiveBounds.Max.X, ExclusiveBounds.Max.Y);
	...
	virtual FLandscapeBrushData ApplyBrush(const TArray<FLandscapeToolInteractorPosition>& InInteractorPositions) override
	{
		...
		FLandscapeBrush::FOverlapInfo OverlapInfo = GetOverlapInfo(InInteractorPositions);
		// Disable brush if we don't overlap with anything or if it overlaps with some unloaded components (to prevent data from differing on the edges)
		if (OverlapInfo.ExclusiveBounds.IsEmpty() || OverlapInfo.bOverlapsUnloadedComponents)
		{
			return FLandscapeBrushData();
		}
		...
		FLandscapeBrushData BrushData(OverlapInfo.ExclusiveBounds);
		...
		for (const FLandscapeToolInteractorPosition& InteractorPosition : InteractorPositions)
		{
			...
			SpotExclusiveBounds.Max.X = FMath::CeilToInt32(InteractorPosition.Position.X + TotalRadius) + 1;
			SpotExclusiveBounds.Max.Y = FMath::CeilToInt32(InteractorPosition.Position.Y + TotalRadius) + 1;
			SpotExclusiveBounds.Clip(OverlapInfo.ExclusiveBounds);
...
private:
	static TArray<FLandscapeToolInteractorPosition> CapInteractorPositions(const TArray<FLandscapeToolInteractorPosition>& InInteractorPositions)
	{
		// Cap number of mouse positions to a sensible number
		static constexpr int32 MaxInteractorPositions = 10;
		TArray<FLandscapeToolInteractorPosition> OutInteractorPositions;
		if (InInteractorPositions.Num() > MaxInteractorPositions)
		{
			OutInteractorPositions.Reserve(MaxInteractorPositions);
			for (int32 i = 0; i < MaxInteractorPositions; ++i)
			{
				// Scale so we include the first and last of the input positions
				OutInteractorPositions.Add(InInteractorPositions[(i * (InInteractorPositions.Num() - 1)) / (MaxInteractorPositions - 1)]);
			}
		}
		else
		{
			OutInteractorPositions = InInteractorPositions;
		}
		return OutInteractorPositions;
	}
};
...
public:
	...
	virtual void LeaveBrush() override
	{
		for (TWeakObjectPtr<ULandscapeComponent> ComponentWeakPtr : BrushMaterialComponents)
		{
			if (ULandscapeComponent* Component = ComponentWeakPtr.Get())
			{
				Component->EditToolRenderData.ToolMaterial = nullptr;
				Component->UpdateEditToolRenderData();
			}
	...
	virtual FLandscapeBrushData ApplyBrush(const TArray<FLandscapeToolInteractorPosition>& InteractorPositions) override
	{
		...
		{
			// Make sure to remove all stale weak ptrs (in case ApplyBrush is called twice before a Tick) 
			...
			check((ExclusiveBounds.Min.X != INT_MAX) && (ExclusiveBounds.Min.Y != INT_MAX) && (ExclusiveBounds.Max.X != INT_MIN) && (ExclusiveBounds.Max.Y != INT_MIN));
	...
	virtual void MouseMove(float LandscapeX, float LandscapeY) override
	{
		...
		if (EdMode->UISettings->bAlphaBrushAutoRotate)
		{
			...
			if (MouseDelta.SizeSquared() >= FMath::Square(0.5f))
			{
				...
				OldMousePosition = LastMousePosition;
				// UE_LOGF(LogLandscape, Log, "(%f,%f) delta (%f,%f) angle %f", LandscapeX, LandscapeY, MouseDirection.X, MouseDirection.Y, MouseAngle);
			}
		}
	}
