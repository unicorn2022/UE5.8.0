...
public:
	...
	void SplitSegment(ULandscapeSplineSegment* Segment, const FVector& LocalLocation)
	{
		...
		SplinesComponent->MarkRenderStateDirty();
		SplinesComponent->CheckSplinesValid();
	}
					...
					if (ClickedControlPoint != nullptr)
					{
						...
						if (SelectedSplineControlPoints.Num() == 1)
						{
							...
							if (SourceSplineActor && ClickedSplineActor && SourceSplineActor != ClickedSplineActor)
							{
								...
								{
									UE_LOGF(LogLandscapeEdMode, Warning, "Can't merge LandscapeSplineActors belonging to different Landscapes");
									return true;
								}
								...
								SourceComponent->CheckSplinesValid();
		...
		if (InKey == EKeys::LeftMouseButton)
		{
			...
			if (InEvent == IE_Pressed)
			{
				...
				if (HitProxy)
				{
					...
					{
						// BeginTransform isn't called for Legacy gizmos, so we call it here for a common codepath with new gizmos
						const bool bIsDuplicatingControlPoints = InViewportClient->IsAltPressed();
						BeginTransformInternal(FGizmoState(), bIsDuplicatingControlPoints);
	...
	virtual bool BeginTransform(const FGizmoState& InState) override
	{
		...
		const bool bShouldDuplicateSelectedControlPoints = 
			SelectedSplineControlPoints.Num() == 1 
			&& FSlateApplication::Get().GetModifierKeys().IsAltDown()
			&& InState.TransformMode == EGizmoTransformMode::Translate;
		return BeginTransformInternal(InState, bShouldDuplicateSelectedControlPoints);
	}

	bool BeginTransformInternal(const FGizmoState& InState, const bool bInIsDuplicating)
	{
		TArray<ULandscapeSplineControlPoint*> SelectedSplineControlPoints = SplineSelection->GetSelectedSplineControlPoints();
