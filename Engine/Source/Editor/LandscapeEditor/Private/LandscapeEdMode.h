...
public:
	...
	/** FEdMode: Called once per frame per viewport */
	...
	bool IsGlobalModifierPressed() const;
	bool IsGlobalAlternateModifierPressed() const;

	/**	See GetTargetLayerActionType, except it's a global function meant to be used in the UX, where there's no viewport, and the action type for a given layer depends on 
	  the pressed modifier buttons (only available per-viewport), so this will give an "estimate" of the result, based on the fact that the modifier button was pressed on 
	  at least one of the active viewports. Not pretty, I know... */
	ELandscapeTargetLayerActionType GetEstimatedTargetLayerActionType(const TSharedRef<FLandscapeTargetListInfo> Target);
...
private:
	...
	// Implement a IInputProcessor to be able to track the shift/ctrl modifier key's states globally (i.e. not per-viewport), in order to access it in the editor mode UX code
	struct FInputProcessor : public IInputProcessor, public TSharedFromThis<FInputProcessor>
	{
		...
		{ 
			return TEXT("FEdModeLandscape::FInputProcessor"); 
		}
	...
	TSharedRef<FInputProcessor> InputProcessor;
};