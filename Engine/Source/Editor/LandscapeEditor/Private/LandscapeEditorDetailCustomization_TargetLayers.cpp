	...
	HeaderRow
		...
		[
			SNew(SHorizontalBox)
				...
				.HAlign(HAlign_Fill)
				[
					PropertyHandle_TargetLayerDirectoryPath->CreatePropertyNameWidget(LOCTEXT("TargetLayerAssetPath", "Default Layer Asset Path"), LOCTEXT("TargetLayerAssetPath_ToolTip", "Asset folder where Landscape Layer Info assets will be searched when auto-populating landscape target layers"))
				]
		]
		.ValueContent()
		.HAlign(HAlign_Fill)
		...
		[
			SNew(SHorizontalBox)
				...
				[
					SNew(SBox)
					// This path is edited via a SEditableTextBox (rather than using CreatePropertyValueWidget) so we have to specify a tooltip in order to display the full path if 
					//  the text box is to small to display the entire thing
					.ToolTipText_Static(&FLandscapeEditorStructCustomization_FTargetLayerAssetPath::GetTargetLayerAssetFilePath)
					[
...
void FLandscapeEditorDetailCustomization_TargetLayers::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	TSharedRef<IPropertyHandle> PropertyHandle_WeightBlendedTargetLayerPaintMode = DetailBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(ULandscapeEditorObject, WeightBlendedTargetLayerPaintMode));
	...
	if (!ShouldShowTargetLayers())
	{
		...
		PropertyHandle_WeightBlendedTargetLayerPaintMode->MarkHiddenByCustomization();
	...
	TargetsCategory.AddProperty(PropertyHandle_PaintingRestriction)
		.Visibility(TAttribute<EVisibility>::CreateStatic(&FLandscapeEditorDetailCustomization_TargetLayers::GetPaintingRestrictionVisibility));

	TargetsCategory.AddProperty(PropertyHandle_WeightBlendedTargetLayerPaintMode)
		.Visibility(TAttribute<EVisibility>::CreateStatic(&FLandscapeEditorDetailCustomization_TargetLayers::GetWeightBlendedTargetLayerPaintModeVisibility));

	TargetsCategory.AddProperty(PropertyHandle_TargetLayerAssetFilePath);

	TargetsCategory.AddCustomRow(FText())
		.Visibility(TAttribute<EVisibility>::CreateStatic(&FLandscapeEditorDetailCustomization_TargetLayers::GetVisibilityMaskTipVisibility))
		[
	...
	TargetsCategory.AddCustomRow(FText())
		.Visibility(TAttribute<EVisibility>::CreateStatic(&FLandscapeEditorDetailCustomization_TargetLayers::GetPopulateTargetLayersInfoTipVisibility))
		[
			SNew(SMultiLineEditableTextBox)
				.IsReadOnly(true)
				.Font(DetailBuilder.GetDetailFontBold())
	...
	TargetsCategory.AddCustomRow(FText())
		.Visibility(TAttribute<EVisibility>::CreateStatic(&FLandscapeEditorDetailCustomization_TargetLayers::GetTargetLayersInvalidInfoAssetTipVisibility))
		[
			SNew(SMultiLineEditableTextBox)
				.IsReadOnly(true)
				.Font(DetailBuilder.GetDetailFontBold())
	...
	TargetsCategory.AddCustomRow(FText())
		.Visibility(TAttribute<EVisibility>::CreateStatic(&FLandscapeEditorDetailCustomization_TargetLayers::GetFilteredTargetLayersListInfoTipVisibility))
		[
			SNew(SMultiLineEditableTextBox)
				.IsReadOnly(true)
				.Font(DetailBuilder.GetDetailFontBold())
...
{
	FEdModeLandscape* LandscapeEdMode = GetEditorMode();
	if (LandscapeEdMode && LandscapeEdMode->CurrentToolMode && LandscapeEdMode->CurrentTool)
	{
...
EVisibility FLandscapeEditorDetailCustomization_TargetLayers::GetWeightBlendedTargetLayerPaintModeVisibility()
{
	FEdModeLandscape* LandscapeEdMode = GetEditorMode();
	...
	if (LandscapeEdMode && LandscapeEdMode->CurrentTool)
	{
		...
		// Tool target type "Invalid" means Weightmap with no valid paint layer, so technically, it is weightmap and we therefore choose to show WeightBlendedTargetLayerPaintMode : 
		if ((LandscapeEdMode->CurrentToolTarget.TargetType == ELandscapeToolTargetType::Weightmap && CurrentToolName != TEXT("BlueprintBrush"))
			|| (LandscapeEdMode->CurrentToolTarget.TargetType == ELandscapeToolTargetType::Invalid))
		{
			return EVisibility::Visible;
...
void FLandscapeEditorCustomNodeBuilder_TargetLayers::GenerateHeaderRowContent(FDetailWidgetRow& NodeRow)
{
	...
	if ((LandscapeEdMode == nullptr) || (LandscapeEdMode->CurrentToolMode == nullptr))
	{
		return;	
	}
...
const TArray<const TSharedRef<FLandscapeTargetListInfo>> FLandscapeEditorCustomNodeBuilder_TargetLayers::GetUnassignedTargetLayersFromMaterial() const
{
	...
	if (const FEdModeLandscape* LandscapeEdMode = GetEditorMode(); (LandscapeEdMode != nullptr) && LandscapeEdMode->CurrentToolTarget.LandscapeInfo.IsValid())
	{
		...
		LandscapeActor->GetLandscapeInfo()->ForEachLandscapeProxy([&LayerNames](ALandscapeProxy* Proxy)
			{
				LayerNames.Append(Proxy->RetrieveTargetLayerNamesFromMaterials(/*bIncludeVisibilityLayer =*/false));
				return true;
			});
...
const FSlateBrush* FLandscapeEditorCustomNodeBuilder_TargetLayers::GetShowUnusedBrush() const
{
	...
	if (FEdModeLandscape* LandscapeEdMode = GetEditorMode(); (LandscapeEdMode != nullptr) && LandscapeEdMode->UISettings && !LandscapeEdMode->UISettings->ShowUnusedLayers)
	{
...
TSharedPtr<SWidget> FLandscapeEditorCustomNodeBuilder_TargetLayers::GenerateRow(const TSharedRef<FLandscapeTargetListInfo> Target)
{
	...
	if (FEdModeLandscape* LandscapeEdMode = GetEditorMode(); LandscapeEdMode && LandscapeEdMode->CurrentTool && LandscapeEdMode->CurrentToolMode)
	{
		ELandscapeToolTargetTypeFlags SupportedTargetTypes = UE::Landscape::GetLandscapeToolTargetTypeAsFlags(LandscapeEdMode->CurrentTool->GetSupportedTargetType())
			& LandscapeEdMode->CurrentToolMode->SupportedTargetTypes
			& UE::Landscape::GetLandscapeToolTargetTypeAsFlags(Target->TargetType);
		if (SupportedTargetTypes == ELandscapeToolTargetTypeFlags::None)
		{
			return RowWidget;
		}
	}
	...
	if (Target->TargetType != ELandscapeToolTargetType::Weightmap)
	{
		...
		[
			...
			[
				...
				// Bottom-right corner text indicating how the current tool will affect the target layer
				+ SOverlay::Slot()
				.Padding(2)
				...
				[
					...
					[
						SNew(SImage)
						.Image_Static(&FLandscapeEditorCustomNodeBuilder_TargetLayers::GetTargetLayerActionTypeBrush, Target)
	...
	else
	{
		...
		ULandscapeLayerInfoObject* LayerInfo = Target->LayerInfoObj.Get();
		UMaterialInstanceConstant* ThumbnailMIC = Target->ThumbnailMIC.Get();
		UObject* ThumbnailObject = nullptr;

		if (Target->bIsLayerReferencedByMaterial)
		{
			// If the layer info is valid and has a custom thumbnail, it's considered as an override, so use this as the thumbnail object : 
			ThumbnailObject = ((LayerInfo != nullptr) && ThumbnailTools::AssetHasCustomThumbnail(LayerInfo->GetFullName())) ? static_cast<UObject*>(LayerInfo) : static_cast<UObject*>(ThumbnailMIC);
		}


		TSharedRef<SWidget> DeleteButton = PropertyCustomizationHelpers::MakeDeleteButton(
			FSimpleDelegate::CreateStatic(&FLandscapeEditorCustomNodeBuilder_TargetLayers::OnTargetLayerDeleteClicked, Target),
			LOCTEXT("Tooltip_Delete", "Delete Layer"));

		TSharedRef<SWidget> CreateButton = PropertyCustomizationHelpers::MakeCustomButton(
			FAppStyle::GetBrush(TEXT("Icons.Plus")),
			FSimpleDelegate::CreateStatic(&FLandscapeEditorCustomNodeBuilder_TargetLayers::OnTargetLayerCreateClicked, Target),
			LOCTEXT("Tooltip_Create", "Create Layer Info"),
			TAttribute<bool>::CreateStatic(&FLandscapeEditorCustomNodeBuilder_TargetLayers::GetTargetLayerCreateEnabled, Target));

		// The tooltip mimics the asset thumbnail tooltips since we're using SLandscapeAssetThumbnail. It's displaying information about the layer info, 
		//  especially its blend group
		TSharedRef<SToolTip> TargetLayerToolTip = 
			SNew(SToolTip)
			.TextMargin(FMargin(1.f, -3.f))
			.BorderImage(FAppStyle::GetBrush("AssetThumbnail.Tooltip.Border"))
			.Visibility((LayerInfo != nullptr) ? EVisibility::Visible : EVisibility::Collapsed)
			.Content()
			[
				...
				[
					...
					[
						SNew(STextBlock)
						.Text(Target->TargetLayerDisplayName)
						.Font(FAppStyle::GetFontStyle("AssetThumbnail.Font"))
					...
					[
						...
						[
							SNew(STextBlock)
							.Text(LOCTEXT("BlendMethodHeader", "Blend Method:"))
						]
						...
						[
							SNew(STextBlock)
							.ColorAndOpacity(FStyleColors::White)
							.Text_Static(&FLandscapeEditorCustomNodeBuilder_TargetLayers::GetTargetBlendMethodText, Target, /*bInIncludeBlendGroup = */true)
						]
					]

					// Blend Group (if applicable)
					+ SVerticalBox::Slot()
					.Padding(0.f, 0.f, 0.f, 6.f)
					.AutoHeight()
					[
						SNew(SHorizontalBox)
						.Visibility_Static(&FLandscapeEditorCustomNodeBuilder_TargetLayers::GetTargetLayerBlendGroupStripVisibility, Target)
						...
						[
							SNew(STextBlock)
							.Text(LOCTEXT("BlendGroupHeader", "Blend Group:"))
						]
						...
						.VAlign(VAlign_Center)
						[
							// Display a color box to explain the color in the thumbnail color strip
							SNew(SColorBlock)
							.Color_Static(&FLandscapeEditorCustomNodeBuilder_TargetLayers::GetTargetLayerBlendGroupStripColor, Target)
							.Size(12.f)
						]
						...
						[
							SNew(STextBlock)
							.ColorAndOpacity(FStyleColors::White)
							.Text_Static(&FLandscapeEditorCustomNodeBuilder_TargetLayers::GetTargetBlendGroupText, Target, /*bInAllowNoName = */true)
						]
					]
				]
			];
		...
		RowWidget = 
			...
			[
				...
				[
					SNew(SOverlay)
					.ToolTip(TargetLayerToolTip)
					+ SOverlay::Slot()
					[
						SNew(SLandscapeAssetThumbnail, ThumbnailObject, ThumbnailPool, Target->bIsLayerReferencedByMaterial ? FName("LandscapeEditor.Target_Weightmap") : FName("LandscapeEditor.Target_Unknown"))
					...
					// Mimic the asset color strip to show a color for the blend group :
					+ SOverlay::Slot()
					.HAlign(HAlign_Fill)
					.VAlign(VAlign_Bottom)
					[
					...
					[
						...
						[
							...
							[
								...
								.ToolTipText(LOCTEXT("SubtractiveBlendTooltip", "If enabled, this target layer's effect on this edit layer will be inverted, "
									"allowing a given edit layer to effectively decrease the target layer's weight from edit layers underneath."))
								.ColorAndOpacity_Static(&FLandscapeEditorCustomNodeBuilder_TargetLayers::GetTargetTextColor, Target)
							]
						]
					]
...
ELandscapeTargetLayerActionType FLandscapeEditorCustomNodeBuilder_TargetLayers::GetTargetLayerActionType(const TSharedRef<FLandscapeTargetListInfo> Target)
{
	FEdModeLandscape* LandscapeEdMode = GetEditorMode();
	if (LandscapeEdMode == nullptr)
	{
	...
	return LandscapeEdMode->GetEstimatedTargetLayerActionType(Target);
}

const FSlateBrush* FLandscapeEditorCustomNodeBuilder_TargetLayers::GetTargetLayerActionTypeBrush(const TSharedRef<FLandscapeTargetListInfo> Target)
{
	switch (GetTargetLayerActionType(Target))
	{
		case ELandscapeTargetLayerActionType::Normal:
		{
			return FAppStyle::GetBrush("LandscapeEditor.Brush.Action_Regular");
		}
		case ELandscapeTargetLayerActionType::Inverted:
		{
			return FAppStyle::GetBrush("LandscapeEditor.Brush.Action_Inverse");
		}
		default:
			break;
	}
...
FLinearColor FLandscapeEditorCustomNodeBuilder_TargetLayers::GetTargetLayerBlendGroupStripColor(const TSharedRef<FLandscapeTargetListInfo> Target)
{
	FLinearColor Color = FLinearColor::Gray;
	if (Target->LandscapeInfo.IsValid() && Target->LayerInfoObj.IsValid())
	{
		TOptional<FTargetLayerBlendGroup> BlendGroup = Target->LandscapeInfo->GetBlendGroupForTargetLayer(Target->LayerInfoObj.Get());
		if (BlendGroup.IsSet())
		{
			// Generate a random color name based on the blend group's data (in order to have a consistent result for a given blend method / group name)
			const FString BlendGroupName = BlendGroup->GetName().ToString();
			uint32 Hash = FCrc::TypeCrc32<uint32>(static_cast<uint32>(BlendGroup->GetBlendMethod()));
			Hash = FCrc::MemCrc32(*BlendGroupName, BlendGroupName.Len() * sizeof(BlendGroupName[0]), Hash);
...
EVisibility FLandscapeEditorCustomNodeBuilder_TargetLayers::GetTargetLayerBlendGroupStripVisibility(const TSharedRef<FLandscapeTargetListInfo> Target)
{
...
void FLandscapeEditorCustomNodeBuilder_TargetLayers::OnTargetLayerCreateClicked(const TSharedRef<FLandscapeTargetListInfo> Target)
{
...
void FLandscapeEditorCustomNodeBuilder_TargetLayers::OnTargetLayerDeleteClicked(const TSharedRef<FLandscapeTargetListInfo> Target)
{
...
FText FLandscapeEditorCustomNodeBuilder_TargetLayers::GetTargetBlendMethodText(const TSharedRef<FLandscapeTargetListInfo> InTarget, bool bInIncludeBlendGroup)
{
	...
	if (bInIncludeBlendGroup && (LayerInfo->GetBlendMethod() == ELandscapeTargetLayerBlendMethod::PremultipliedAlphaBlending) && !LayerInfo->GetBlendGroup().IsNone())
	{
		FText BlendGroupName = GetTargetBlendGroupText(InTarget, /*bInAllowNoName = */false);
		if (!BlendGroupName.IsEmpty())
		{
			Result = FText::Format(LOCTEXT("PremultipliedAlphaBlendingBlendGroupName", "{0} ({1})"), Result, BlendGroupName);
		}
	}
	return Result;
}

FText FLandscapeEditorCustomNodeBuilder_TargetLayers::GetTargetBlendGroupText(const TSharedRef<FLandscapeTargetListInfo> InTarget, bool bInAllowNoName)
{
	const ULandscapeLayerInfoObject* LayerInfo = InTarget->LayerInfoObj.Get();
	if (LayerInfo == nullptr)
	{
	...
	FName LayerGroupName;
	if (LayerInfo->GetBlendMethod() == ELandscapeTargetLayerBlendMethod::PremultipliedAlphaBlending)
	{
		LayerGroupName = LayerInfo->GetBlendGroup();
	}

	if (!LayerGroupName.IsNone())
	{
		return FText::FromName(LayerGroupName);
	}
	else if (bInAllowNoName)
	{
		return LOCTEXT("UnnamedBlendGroupName", "(unnamed)");
	}
	return FText::GetEmpty();
}
