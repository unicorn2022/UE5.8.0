...
FEdModeLandscape::FEdModeLandscape()
	...
	, InputProcessor(MakeShared<FInputProcessor>())
{
...
bool FEdModeLandscape::IsGlobalModifierPressed() const
{
	return InputProcessor->bIsShiftDown;
}

bool FEdModeLandscape::IsGlobalAlternateModifierPressed() const
{
	return InputProcessor->bIsCtrlDown;
}
...
void FEdModeLandscape::Enter()
{
	...
	InputProcessor->StartTrackingInput();
}
...
void FEdModeLandscape::Exit()
{
	...
	InputProcessor->StopTrackingInput();
	...
	if (World != nullptr)
	{
		...
		World->OnLevelsChanged().Remove(OnLevelsChangedDelegateHandle);
	}
...
ELandscapeTargetLayerActionType FEdModeLandscape::GetEstimatedTargetLayerActionType(const TSharedRef<FLandscapeTargetListInfo> Target)
{
	const bool bIsGlobalModifierPressed = IsGlobalModifierPressed();
	const bool bIsGlobalAlternateModifierPressed = IsGlobalAlternateModifierPressed();

	// Bail out if one of the LandscapeEditorControlType's "Hold/Don't Hold Ctrl" condition is not met, so that the tools don't mistakenly let the user know that they will do anything :
	const ELandscapeFoliageEditorControlType LandscapeEditorControlType = GetDefault<ULevelEditorViewportSettings>()->LandscapeEditorControlType;
	if ((LandscapeEditorControlType == ELandscapeFoliageEditorControlType::RequireCtrl && !bIsGlobalAlternateModifierPressed)
		|| (LandscapeEditorControlType == ELandscapeFoliageEditorControlType::RequireNoCtrl && bIsGlobalAlternateModifierPressed))
	{
		return ELandscapeTargetLayerActionType::None;
	}

	return CurrentTool 
		? CurrentTool->GetTargetLayerActionType(Target->LayerInfoObj.Get(), bIsGlobalModifierPressed, bIsGlobalAlternateModifierPressed)
...
bool FEdModeLandscape::BeginTransform(const FGizmoState& InState)
{
	if (!IsEditingEnabled())
	{
		return false;
	...
	if (CurrentTool && CurrentTool->BeginTransform(InState))
	{
		return true;
	}
...
bool FEdModeLandscape::EndTransform(const FGizmoState& InState)
{
	if (!IsEditingEnabled())
	{
		return false;
	...
	if (CurrentTool && CurrentTool->EndTransform(InState))
	{
		return true;
	}
...
bool FEdModeLandscape::InputKey(FEditorViewportClient* ViewportClient, FViewport* Viewport, FKey Key, EInputEvent Event)
{
	...
	else
	{
		...
		if (Key == EKeys::LeftMouseButton && Event == IE_Pressed)
		{
			...
			{
				if (CurrentTool && (CurrentTool->GetSupportedTargetType() == ELandscapeToolTargetType::Invalid || CurrentToolTarget.TargetType != ELandscapeToolTargetType::Invalid))
				{
					FVector HitLocation;
...
void FEdModeLandscape::SetCurrentTool(FName ToolName, FName TargetLayerName)
{
	...
	for (int32 i = 0; i < LandscapeTools.Num(); ++i)
	{
		...
		{
			if (EnumHasAnyFlags(UE::Landscape::GetLandscapeToolTargetTypeAsFlags(Tool->GetSupportedTargetType()), CurrentToolMode->SupportedTargetTypes))
			{
				ToolIndex = i;
				break;
			}
...
void FEdModeLandscape::SetCurrentTool(int32 ToolIndex, FName TargetLayerName)
{
	...
	if (CurrentTool->GetSupportedTargetType() == ELandscapeToolTargetType::Invalid)
	{
	...
	{
		if (CurrentTool->GetSupportedTargetType() != CurrentToolTarget.TargetType)
		{
			const ELandscapeToolTargetType TargetType = CurrentTool->GetSupportedTargetType();
			auto Filter = [TargetType, TargetLayerName](const TSharedRef<FLandscapeTargetListInfo>& Target) { return (Target->TargetType == TargetType) && (TargetLayerName == NAME_None || TargetLayerName == Target->LayerName); };
...
void FEdModeLandscape::DeleteLandscapeComponents(ULandscapeInfo* LandscapeInfo, TSet<ULandscapeComponent*> ComponentsToDelete)
{
	...
	ALandscapeProxy* MainLandscapeProxy = LandscapeInfo->GetLandscapeProxy();
	check(MainLandscapeProxy != nullptr);

	TSet<ALandscapeProxy*> AffectedProxies;
	for (ULandscapeComponent* Component : ComponentsToDelete)
	{
		if (ALandscapeProxy* Proxy = Component->GetLandscapeProxy())
		{
			AffectedProxies.Add(Proxy);
		}
		Component->Modify();
	...
	for (ULandscapeComponent* Component : ComponentsToDelete)
	{
		...
		if (CollisionComp)
		{
			AInstancedFoliageActor::DeleteInstancesForComponent(MainLandscapeProxy->GetWorld(), CollisionComp);
		}
	}
	...
	for (ULandscapeComponent* Component : ComponentsToDelete)
	{
		...
		// The deletion of components will affect the Nanite landscape components, which therefore need to be ticked so that they can appropriately update if needed 
		for (ALandscapeProxy* AffectedProxy : AffectedProxies)
		{
			AffectedProxy->UpdateRenderingMethod();
		}
	}
	...
	if (ensure(LandscapeInfo != nullptr))
	{
		...
		if (LandscapeInfo->GetLandscapeExtent(OldMinX, OldMinY, OldMaxX, OldMaxY))
		{
				...
				for (const TPair<FIntPoint, ULandscapeComponent*>& Entry : LandscapeInfo->XYtoComponentMap)
				{
					...
					if (NewComponent)
					{
						...
						if (OldCollisionComponent && NewCollisionComponent)
						{
							...
							if (GetDefault<ULandscapeSettings>()->GetFoliageLandscapeSnappingMode() != EFoliageLandscapeSnappingMode::DontSnap)
							{
								NewCollisionComponent->SnapFoliageInstances(ULandscapeHeightfieldCollisionComponent::ESnapFoliageInstancesFlags::ForceAll);
							}
						}
					}
				}
			...
			else
			{
				...
				{
					ULandscapeHeightfieldCollisionComponent* NewCollisionComponent = NewEntry.Value->GetCollisionComponent();
					if (NewCollisionComponent && (GetDefault<ULandscapeSettings>()->GetFoliageLandscapeSnappingMode() != EFoliageLandscapeSnappingMode::DontSnap))
					{
						NewCollisionComponent->SnapFoliageInstances(ULandscapeHeightfieldCollisionComponent::ESnapFoliageInstancesFlags::ForceAll);
					}
				}
...
void FEdModeLandscape::FInputProcessor::StartTrackingInput()
{
	FSlateApplication::Get().RegisterInputPreProcessor(AsShared());
}

void FEdModeLandscape::FInputProcessor::StopTrackingInput()
{
	FSlateApplication::Get().UnregisterInputPreProcessor(AsShared());
}

bool FEdModeLandscape::FInputProcessor::HandleKeyDownEvent(FSlateApplication& SlateApp, const FKeyEvent& InKeyEvent)
{
	bIsShiftDown = SlateApp.GetModifierKeys().IsShiftDown();
	bIsCtrlDown = SlateApp.GetModifierKeys().IsControlDown();
	// return false, as we don't want to disturb the input handling, we just want to track whether the modifier keys are pressed at the application level
	return false;
}

bool FEdModeLandscape::FInputProcessor::HandleKeyUpEvent(FSlateApplication& SlateApp, const FKeyEvent& InKeyEvent)
{
	bIsShiftDown = SlateApp.GetModifierKeys().IsShiftDown();
	bIsCtrlDown = SlateApp.GetModifierKeys().IsControlDown();
	// return false, as we don't want to disturb the input handling, we just want to track whether the modifier keys are pressed at the application level
