				...
				if (PropertyHandle->IsValidHandle())
				{
					...
					[
						...
						[
							...
							.IsChecked(this, &FLandscapeProxyUIDetails::IsOverriddenPropertyChecked, EditingStreamingProxies, PropertyName)
							.OnCheckStateChanged(this, &FLandscapeProxyUIDetails::OnCheckStateChanged, EditingStreamingProxies, PropertyName)
						]
						+ SHorizontalBox::Slot()
						.AutoWidth()
						[
					...
					// Hook the EnabledAttribute on the 2 sub-widgets by hand, so that we don't disable the entire custom widget
					//  when the property is not overridden (since that would also disable the override checkbox...)
					NameWidget->SetEnabled(EnabledAttribute);
					ValueWidget->SetEnabled(EnabledAttribute);

					// The Reset To Default button corresponds to the overridden state of the overridable property : 
					DetailRow->OverrideResetToDefault(FResetToDefaultOverride::Create(
						FIsResetToDefaultVisible::CreateLambda([EditingStreamingProxies, PropertyName](TSharedPtr<IPropertyHandle> InPropertyHandle) -> bool
							{
								return Algo::AnyOf(EditingStreamingProxies, [PropertyName](TWeakObjectPtr<ALandscapeStreamingProxy> StreamingProxy) { return StreamingProxy.IsValid() && StreamingProxy->IsSharedPropertyOverridden(PropertyName); });
							}),
						FResetToDefaultHandler::CreateLambda([EditingStreamingProxies, PropertyName](TSharedPtr<IPropertyHandle> InPropertyHandle)
							{
								const FScopedTransaction Transaction(LOCTEXT("ResetSetSharedPropertyOverride", "Reset Property Override"));
								for (const TWeakObjectPtr<ALandscapeStreamingProxy> StreamingProxy : EditingStreamingProxies)
								{
									...
									StreamingProxy->SetSharedPropertyOverride(PropertyName, false);
								}
							})));
				}
			}
...
ECheckBoxState FLandscapeProxyUIDetails::IsOverriddenPropertyChecked(TArray<TWeakObjectPtr<ALandscapeStreamingProxy>> InEditingStreamingProxies, FName InPropertyName) const
{
	bool bContainsOverriddenProperty = false;
	bool bContainsDefaultProperty = false;

	for (const TWeakObjectPtr<ALandscapeStreamingProxy> StreamingProxy : InEditingStreamingProxies)
	{
		if (!StreamingProxy.IsValid())
		{
			continue;
		...
		const bool bIsPropertyOverridden = StreamingProxy->IsSharedPropertyOverridden(InPropertyName);
...
void FLandscapeProxyUIDetails::OnCheckStateChanged(ECheckBoxState InNewState, TArray<TWeakObjectPtr<ALandscapeStreamingProxy>> InEditingStreamingProxies, FName InPropertyName)
{
	...
	for (const TWeakObjectPtr<ALandscapeStreamingProxy> StreamingProxy : InEditingStreamingProxies)
	{
		...
		StreamingProxy->SetSharedPropertyOverride(InPropertyName, bChecked);
	}

	check(DetailLayoutBuilder != nullptr);
	// Refresh the details to disable/enable the widgets that should be : we need this to force CanEditChange to be re-evaluated after the property was overridden/de-overridden
