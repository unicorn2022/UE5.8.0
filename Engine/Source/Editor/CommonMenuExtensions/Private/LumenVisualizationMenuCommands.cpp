...
void FLumenVisualizationMenuCommands::BuildCommandMap()
{
	...
	for (FLumenVisualizationData::TModeMap::TConstIterator It = ModeMap.CreateConstIterator(); It; ++It)
	{
		...
		case FLumenVisualizationData::FModeType::General:
			Record.Type = FLumenVisualizationType::General;
			break;

		case FLumenVisualizationData::FModeType::Performance:
			Record.Type = FLumenVisualizationType::Performance;
			break;
		}
	}
}
...
void FLumenVisualizationMenuCommands::BuildVisualisationSubMenu(FMenuBuilder& Menu)
{
	...
	if (Commands.IsPopulated())
	{
		// Overview
		{
			Menu.BeginSection("LevelViewportLumenVisualizationOverview", LOCTEXT("LumenVisualizationOverviewHeader", "Overview"));
			Commands.AddCommandTypeToMenu(Menu, FLumenVisualizationType::Overview);
			Menu.EndSection();
		}

		// General
		{
			Menu.BeginSection("LevelViewportLumenVisualizationGeneral", LOCTEXT("LumenVisualizationGeneralHeader", "General"));
			Commands.AddCommandTypeToMenu(Menu, FLumenVisualizationType::General);
			Menu.EndSection();
		}

		// Performance
		{
			Menu.BeginSection("LevelViewportLumenVisualizationPerformance", LOCTEXT("LumenVisualizationPerformanceHeader", "Performance"));
			Commands.AddCommandTypeToMenu(Menu, FLumenVisualizationType::Performance);
			Menu.EndSection();
		}
	}
}
