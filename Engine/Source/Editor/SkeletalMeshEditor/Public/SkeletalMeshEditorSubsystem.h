...
public:
	 ...
	 * @param SkeletalMesh	The mesh from which we are removing a socket
	 * @param SocketName    The name of the socket to remove
	 * @return true if the removal succeeded.
	 */
	UFUNCTION(BlueprintCallable, Category = "Skeletal Mesh Utilities", meta = (ScriptMethod))
	static UE_API bool RemoveSocket(USkeletalMesh* SkeletalMesh, FName SocketName);

	/** Get the parent bone name of a bone in a skeletal mesh.
	 * @param SkeletalMesh  The mesh to query.
	 * @param BoneName      The name of the bone to query.
	 * @return The name of the parent bone, or NAME_None if the bone is the root or does not exist.
	 */
	UFUNCTION(BlueprintCallable, Category = "Skeletal Mesh Utilities", meta = (ScriptMethod))
	static UE_API FName GetBoneParent(const USkeletalMesh* SkeletalMesh, FName BoneName);

	/** Get the direct children of a bone in a skeletal mesh.
	 * @param SkeletalMesh  The mesh to query.
	 * @param BoneName      The name of the bone to query.
	 * @return An array of bone names that are direct children of the given bone.
	 */
	UFUNCTION(BlueprintCallable, Category = "Skeletal Mesh Utilities", meta = (ScriptMethod))
	static UE_API TArray<FName> GetBoneChildren(const USkeletalMesh* SkeletalMesh, FName BoneName);
	...
	*/
	...
	/** Sets Skeletal Mesh Flag bAllowCPUAccess on all LOD levels. */
	UFUNCTION(BlueprintCallable, Category = "Editor Scripting | SkeletalMesh")
	static UE_API void SetAllowCPUAccess(USkeletalMesh* TargetMesh, bool bAllowCPUAccess);
