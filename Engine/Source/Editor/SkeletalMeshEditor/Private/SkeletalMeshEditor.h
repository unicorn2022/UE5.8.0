...
protected:
	void OnBuildViewportInteractions(UViewportInteractionsBehaviorSource* Source) override;
...
private:
	...
	// Refreshes the skeleton tree when our edited mesh is re-imported outside of this editor's
	// own re-import button (Content Browser, scripted, drag-drop, etc.). The in-editor button
	// has its own refresh path inside HandleReimportMeshInternal's OnDone callback.
