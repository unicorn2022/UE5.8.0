...
bool USkeletalMeshEditorSubsystem::RegenerateLOD(USkeletalMesh* SkeletalMesh, int32 NewLODCount /*= 0*/, bool bRegenerateEvenIfImported /*= false*/, bool bGenerateBaseLOD /*= false*/)
{
	...
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "RegenerateLOD: The SkeletalMesh is null.");
		return false;
	}
...
int32 USkeletalMeshEditorSubsystem::GetNumVerts(USkeletalMesh* SkeletalMesh, int32 LODIndex)
{
	...
	if (SkeletalMesh == nullptr)
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "GetNumVerts: The SkeletalMesh is null.");
		return 0;
	}
...
int32 USkeletalMeshEditorSubsystem::GetNumSections( USkeletalMesh* SkeletalMesh, int32 LODIndex )
{
	...
	if ( SkeletalMesh == nullptr )
	{
		UE_LOGF( LogSkeletalMeshEditorSubsystem, Error, "GetNumSections: The SkeletalMesh is null." );
		return INDEX_NONE;
	}
...
bool USkeletalMeshEditorSubsystem::GetSectionRecomputeTangent(const USkeletalMesh* SkeletalMesh, const int32 LODIndex, const int32 SectionIndex, bool& bOutRecomputeTangent)
{
	...
	if (SkeletalMesh == nullptr)
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "GetSectionRecomputeTangent: The SkeletalMesh is null.");
		return false;
	}

	if (!SkeletalMesh->GetResourceForRendering() || !(SkeletalMesh->GetResourceForRendering()->LODRenderData.Num() > 0))
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "GetSectionRecomputeTangent: The render data is null.");
		return false;
	}
	...
	if (!LodRenderData.IsValidIndex(LODIndex))
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "GetSectionRecomputeTangent: The LOD index is invalid.");
		return false;
	}
	if (!LodRenderData[LODIndex].RenderSections.IsValidIndex(SectionIndex))
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "GetSectionRecomputeTangent: The section index is invalid.");
		return false;
	}
...
bool USkeletalMeshEditorSubsystem::SetSectionRecomputeTangent(USkeletalMesh* SkeletalMesh, const int32 LODIndex, const int32 SectionIndex, const bool bRecomputeTangent)
{
	...
	if (SkeletalMesh == nullptr)
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "SetSectionRecomputeTangent: The SkeletalMesh is null.");
		return false;
	}

	if (!SkeletalMesh->GetImportedModel())
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "SetSectionRecomputeTangent: The imported model is null.");
		return false;
	}

	if (!SkeletalMesh->GetImportedModel()->LODModels.IsValidIndex(LODIndex))
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "SetSectionRecomputeTangent: The LOD index is invalid.");
		return false;
	}
	FSkeletalMeshLODModel& LODModel = SkeletalMesh->GetImportedModel()->LODModels[LODIndex];
	if (!LODModel.Sections.IsValidIndex(SectionIndex))
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "SetSectionRecomputeTangent: The section index is invalid.");
		return false;
	}
...
bool USkeletalMeshEditorSubsystem::GetSectionRecomputeTangentsVertexMaskChannel(const USkeletalMesh* SkeletalMesh, const int32 LODIndex, const int32 SectionIndex, uint8& OutRecomputeTangentsVertexMaskChannel)
{
	...
	if (SkeletalMesh == nullptr)
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "GetSectionRecomputeTangentsVertexMaskChannel: The SkeletalMesh is null.");
		return false;
	}

	if (!SkeletalMesh->GetResourceForRendering() || !(SkeletalMesh->GetResourceForRendering()->LODRenderData.Num() > 0))
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "GetSectionRecomputeTangentsVertexMaskChannel: The render data is null.");
		return false;
	}
	...
	if (!LodRenderData.IsValidIndex(LODIndex))
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "GetSectionRecomputeTangentsVertexMaskChannel: The LOD index is invalid.");
		return false;
	}
	if (!LodRenderData[LODIndex].RenderSections.IsValidIndex(SectionIndex))
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "GetSectionRecomputeTangentsVertexMaskChannel: The section index is invalid.");
		return false;
	}
...
bool USkeletalMeshEditorSubsystem::SetSectionRecomputeTangentsVertexMaskChannel(USkeletalMesh* SkeletalMesh, const int32 LODIndex, const int32 SectionIndex, const uint8 RecomputeTangentsVertexMaskChannel)
{
	...
	if (SkeletalMesh == nullptr)
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "SetSectionRecomputeTangentsVertexMaskChannel: The SkeletalMesh is null.");
		return false;
	}

	if (!SkeletalMesh->GetImportedModel())
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "SetSectionRecomputeTangentsVertexMaskChannel: The imported model is null.");
		return false;
	}

	if (!SkeletalMesh->GetImportedModel()->LODModels.IsValidIndex(LODIndex))
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "SetSectionRecomputeTangentsVertexMaskChannel: The LOD index is invalid.");
		return false;
	}
	FSkeletalMeshLODModel& LODModel = SkeletalMesh->GetImportedModel()->LODModels[LODIndex];
	if (!LODModel.Sections.IsValidIndex(SectionIndex))
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "SetSectionRecomputeTangentsVertexMaskChannel: The section index is invalid.");
		return false;
	}
...
bool USkeletalMeshEditorSubsystem::GetSectionCastShadow(const USkeletalMesh* SkeletalMesh, const int32 LODIndex, const int32 SectionIndex, bool& bOutCastShadow)
{
	...
	if (SkeletalMesh == nullptr)
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "GetSectionCastShadow: The SkeletalMesh is null.");
		return false;
	}

	if (!SkeletalMesh->GetResourceForRendering() || !(SkeletalMesh->GetResourceForRendering()->LODRenderData.Num() > 0))
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "GetSectionCastShadow: The render data is null.");
		return false;
	}
	...
	if (!LodRenderData.IsValidIndex(LODIndex))
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "GetSectionCastShadow: The LOD index is invalid.");
		return false;
	}
	if (!LodRenderData[LODIndex].RenderSections.IsValidIndex(SectionIndex))
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "GetSectionCastShadow: The section index is invalid.");
		return false;
	}
...
bool USkeletalMeshEditorSubsystem::SetSectionCastShadow(USkeletalMesh* SkeletalMesh, const int32 LODIndex, const int32 SectionIndex, const bool bCastShadow)
{
	...
	if (SkeletalMesh == nullptr)
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "SetSectionCastShadow: The SkeletalMesh is null.");
		return false;
	}

	if (!SkeletalMesh->GetImportedModel())
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "SetSectionCastShadow: The imported model is null.");
		return false;
	}

	if (!SkeletalMesh->GetImportedModel()->LODModels.IsValidIndex(LODIndex))
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "SetSectionCastShadow: The LOD index is invalid.");
		return false;
	}
	FSkeletalMeshLODModel& LODModel = SkeletalMesh->GetImportedModel()->LODModels[LODIndex];
	if (!LODModel.Sections.IsValidIndex(SectionIndex))
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "SetSectionCastShadow: The section index is invalid.");
		return false;
	}
...
bool USkeletalMeshEditorSubsystem::GetSectionVisibleInRayTracing(const USkeletalMesh* SkeletalMesh, const int32 LODIndex, const int32 SectionIndex, bool& bOutVisibleInRayTracing)
{
	...
	if (SkeletalMesh == nullptr)
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "GetSectionVisibleInRayTracing: The SkeletalMesh is null.");
		return false;
	}

	if (!SkeletalMesh->GetResourceForRendering() || !(SkeletalMesh->GetResourceForRendering()->LODRenderData.Num() > 0))
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "GetSectionVisibleInRayTracing: The render data is null.");
		return false;
	}
	...
	if (!LodRenderData.IsValidIndex(LODIndex))
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "GetSectionVisibleInRayTracing: The LOD index is invalid.");
		return false;
	}
	if (!LodRenderData[LODIndex].RenderSections.IsValidIndex(SectionIndex))
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "GetSectionVisibleInRayTracing: The section index is invalid.");
		return false;
	}
...
bool USkeletalMeshEditorSubsystem::SetSectionVisibleInRayTracing(USkeletalMesh* SkeletalMesh, const int32 LODIndex, const int32 SectionIndex, const bool bVisibleInRayTracing)
{
	...
	if (SkeletalMesh == nullptr)
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "SetSectionVisibleInRayTracing: The SkeletalMesh is null.");
		return false;
	}

	if (!SkeletalMesh->GetImportedModel())
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "SetSectionVisibleInRayTracing: The imported model is null.");
		return false;
	}

	if (!SkeletalMesh->GetImportedModel()->LODModels.IsValidIndex(LODIndex))
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "SetSectionVisibleInRayTracing: The LOD index is invalid.");
		return false;
	}
	FSkeletalMeshLODModel& LODModel = SkeletalMesh->GetImportedModel()->LODModels[LODIndex];
	if (!LODModel.Sections.IsValidIndex(SectionIndex))
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "SetSectionVisibleInRayTracing: The section index is invalid.");
		return false;
	}
...
UMaterialInterface* USkeletalMeshEditorSubsystem::GetMaterialSlotOverlayMaterial(const USkeletalMesh* SkeletalMesh, const int32 SlotIndex)
{
	...
	if (SkeletalMesh == nullptr)
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "GetMaterialSlotOverlayMaterial: The SkeletalMesh is null.");
		return nullptr;
	}
	...
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "GetMaterialSlotOverlayMaterial: Invalid skeletal mesh material slot index.");
		return nullptr;
	}
...
int32 USkeletalMeshEditorSubsystem::GetLODMaterialSlot( USkeletalMesh* SkeletalMesh, int32 LODIndex, int32 SectionIndex )
{
	...
	if ( SkeletalMesh == nullptr )
	{
		UE_LOGF( LogSkeletalMeshEditorSubsystem, Error, "GetLODMaterialSlot: The SkeletalMesh is null." );
		return INDEX_NONE;
	}
	...
	if ( SkeletalMesh->GetResourceForRendering() && SkeletalMesh->GetResourceForRendering()->LODRenderData.Num() > 0 )
	{
		...
		if ( LodRenderData.IsValidIndex( LODIndex ) )
		{
			...
			{
				UE_LOGF( LogSkeletalMeshEditorSubsystem, Error, "GetLODMaterialSlot: Invalid SectionIndex." );
			}
		}
		else
		{
			UE_LOGF( LogSkeletalMeshEditorSubsystem, Error, "GetLODMaterialSlot: Invalid LODIndex." );
		}
	}
...
bool USkeletalMeshEditorSubsystem::RenameSocket(USkeletalMesh* SkeletalMesh, FName OldName, FName NewName)
{
	if (SkeletalMesh == nullptr)
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "RenameSocket: The SkeletalMesh is null.");
		return false;
	}

	if (SkeletalMesh->GetSkeleton() == nullptr)
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "RenameSocket: The SkeletalMesh's Skeleton is null.");
		return false;
	}

	if (NewName == NAME_None)
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "RenameSocket: The NewName is None.");
		return false;
	}
	...
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "RenameSocket: The socket named '%ls' does not exist on the SkeletalMesh.", *OldName.ToString());
		return false;
	}
...
bool USkeletalMeshEditorSubsystem::RemoveSocket(USkeletalMesh* SkeletalMesh, FName SocketName)
{
	if (SkeletalMesh == nullptr)
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "RemoveSocket: The SkeletalMesh is null.");
		return false;
	}

	if (SkeletalMesh->GetSkeleton() == nullptr)
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "RemoveSocket: The SkeletalMesh's Skeleton is null.");
		return false;
	}

	if (SocketName == NAME_None)
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "RemoveSocket: The SocketName is None.");
		return false;
	}

	USkeletalMeshSocket* FoundSocket = SkeletalMesh->FindSocket(SocketName);
	if (FoundSocket == nullptr)
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "RemoveSocket: The socket named '%ls' does not exist on the SkeletalMesh.", *SocketName.ToString());
		return false;
	}

	USkeletalMeshSocket* SkeletonSocket = SkeletalMesh->GetSkeleton()->FindSocket(SocketName);
	// If they're the same pointer, FindSocket fell back to the skeleton — there is no mesh-specific socket.
	if (SkeletonSocket == FoundSocket)
	{
		SkeletonSocket = nullptr;
	}

	FScopedTransaction Transaction(LOCTEXT("RemoveSocket", "Remove Socket"));

	TArray<TObjectPtr<USkeletalMeshSocket>>& MeshSocketList = SkeletalMesh->GetMeshOnlySocketList();
	if (MeshSocketList.Contains(FoundSocket))
	{
		FoundSocket->SetFlags(RF_Transactional);
		FoundSocket->Modify();
		SkeletalMesh->Modify();
		MeshSocketList.Remove(FoundSocket);
	}
	else
	{
		// Socket exists only on the skeleton (FindSocket fell through to it).
		FoundSocket->SetFlags(RF_Transactional);
		FoundSocket->Modify();
		SkeletalMesh->GetSkeleton()->Modify();
		SkeletalMesh->GetSkeleton()->Sockets.Remove(FoundSocket);
	}
	...
	if (SkeletonSocket != nullptr)
	{
		...
		SkeletonSocket->Modify();
		SkeletalMesh->GetSkeleton()->Modify();
		SkeletalMesh->GetSkeleton()->Sockets.Remove(SkeletonSocket);
	}

	// Clean up any preview-attached assets referencing the removed socket on the mesh.
	bool bMeshModified = false;
	FPreviewAssetAttachContainer& MeshPreviewContainer = SkeletalMesh->GetPreviewAttachedAssetContainer();
	for (int32 Index = MeshPreviewContainer.Num() - 1; Index >= 0; --Index)
	{
		if (MeshPreviewContainer[Index].AttachedTo == SocketName)
		{
			if (!bMeshModified)
			{
				SkeletalMesh->Modify();
			...
			MeshPreviewContainer.RemoveAtSwap(Index);
		}
	}

	// Clean up any preview-attached assets referencing the removed socket on the skeleton.
	bool bSkeletonModified = false;
	FPreviewAssetAttachContainer& SkeletonPreviewContainer = SkeletalMesh->GetSkeleton()->PreviewAttachedAssetContainer;
	for (int32 Index = SkeletonPreviewContainer.Num() - 1; Index >= 0; --Index)
	{
		if (SkeletonPreviewContainer[Index].AttachedTo == SocketName)
		{
			if (!bSkeletonModified)
			{
				SkeletalMesh->GetSkeleton()->Modify();
			...
			SkeletonPreviewContainer.RemoveAtSwap(Index);
		}
	}
...
FName USkeletalMeshEditorSubsystem::GetBoneParent(const USkeletalMesh* SkeletalMesh, FName BoneName)
{
	if (!SkeletalMesh)
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "GetBoneParent: The SkeletalMesh is null.");
		return NAME_None;
	}
	...
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "GetBoneParent: Bone '%ls' not found.", *BoneName.ToString());
		return NAME_None;
	}
	...
	return RefSkel.GetBoneName(ParentIndex);
}

TArray<FName> USkeletalMeshEditorSubsystem::GetBoneChildren(const USkeletalMesh* SkeletalMesh, FName BoneName)
{
	TArray<FName> Children;

	if (!SkeletalMesh)
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "GetBoneChildren: The SkeletalMesh is null.");
		return Children;
	}
	...
	if (BoneIndex == INDEX_NONE)
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "GetBoneChildren: Bone '%ls' not found.", *BoneName.ToString());
		return Children;
	}
	...
	for (int32 i = 0; i < NumBones; ++i)
	{
		...
		{
			Children.Add(RefSkel.GetBoneName(i));
		}
	}
...
int32 USkeletalMeshEditorSubsystem::GetLODCount(USkeletalMesh* SkeletalMesh)
{
	if (SkeletalMesh == nullptr)
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "SkeletalMesh GetLODCount: The SkeletalMesh is null.");
		return INDEX_NONE;
	}
...
int32 USkeletalMeshEditorSubsystem::ImportLOD(USkeletalMesh* BaseMesh, const int32 LODIndex, const FString& SourceFilename)
{
	...
	if (!EditorScriptingHelpers::CheckIfInEditorAndPIE())
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "SkeletalMesh ImportLOD: Cannot import or re-import when editor PIE is active.");
		return INDEX_NONE;
	}

	if (BaseMesh == nullptr)
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "SkeletalMesh ImportLOD: The SkeletalMesh is null.");
		return INDEX_NONE;
	}
	...
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "SkeletalMesh ImportLOD: Invalid LODIndex, the LOD index cannot be greater the the number of LOD, skeletal mesh cannot have hole in the LOD array.");
		return INDEX_NONE;
	}
	...
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "SkeletalMesh ImportLOD: Invalid source filename.");
		return INDEX_NONE;
	}
	...
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "SkeletalMesh ImportLOD: Cannot import mesh LOD.");
		return INDEX_NONE;
	}
...
bool USkeletalMeshEditorSubsystem::ReimportAllCustomLODs(USkeletalMesh* SkeletalMesh)
{
	...
	if (!EditorScriptingHelpers::CheckIfInEditorAndPIE())
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "SkeletalMesh ReimportAllCustomLODs: Cannot import or re-import when editor PIE is active.");
		return false;
	}

	if (SkeletalMesh == nullptr)
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "SkeletalMesh ReimportAllCustomLODs: The SkeletalMesh is null.");
		return false;
	}
	...
	for (int32 LODIndex = 1; LODIndex < LODNumber; ++LODIndex)
	{
		...
		{
			UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "SkeletalMesh ReimportAllCustomLODs: Cannot re-import LOD %d.", LODIndex);
			bResult = false;
		}
	}
	return bResult;
...
void USkeletalMeshEditorSubsystem::GetLodBuildSettings(const USkeletalMesh* SkeletalMesh, const int32 LodIndex, FSkeletalMeshBuildSettings& OutBuildOptions)
{
	...
	if (SkeletalMesh == nullptr)
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "GetLodReductionSettings: The SkeletalMesh is null.");
		return;
	}
	...
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "GetLodReductionSettings: Invalid LOD index.");
		return;
	}
...
void USkeletalMeshEditorSubsystem::SetLodBuildSettings(USkeletalMesh* SkeletalMesh, const int32 LodIndex, const FSkeletalMeshBuildSettings& BuildOptions)
{
	...
	if (SkeletalMesh == nullptr)
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "SetLodReductionSettings: The SkeletalMesh is null.");
		return;
	}
...
UPhysicsAsset* USkeletalMeshEditorSubsystem::CreatePhysicsAsset(USkeletalMesh* SkeletalMesh, bool bSetToMesh /*= true*/, int32 LodIndex /*= 0*/)
{
	if (!SkeletalMesh)
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "CreatePhysicsAsset failed: The SkeletalMesh is null.");
		return nullptr;
	}
	...
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "CreatePhysicsAsset failed: An object that is not a Physics Asset already exists with the name %ls.", *ObjectName);
		return nullptr;
	}
	...
	if (NewPhysicsAsset)
	{
		...
		if (!bSuccess)
		{
			UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "CreatePhysicsAsset failed: Couldn't create PhysicsAsset for the SkeletalMesh.");
...
bool USkeletalMeshEditorSubsystem::IsPhysicsAssetCompatible(USkeletalMesh* TargetMesh, UPhysicsAsset* PhysicsAsset)
{
	...
	if (PhysicsAsset)
	{
		...
		for (USkeletalBodySetup* Setup : PhysicsAsset->SkeletalBodySetups)
		{
			...
			{
				UE_LOGF(LogSkeletalMeshEditorSubsystem, 
				       Error, 
				       "IsPhysicsAssetCompatible failed: bone %ls is not present for skeletal mesh %ls", 
...
void USkeletalMeshEditorSubsystem::SetAllowCPUAccess(USkeletalMesh* TargetMesh, bool bAllowCPUAccess)
{
	TGuardValue<bool> UnattendedScriptGuard(GIsRunningUnattendedScript, true);
	...
	{
		UE_LOGF(LogSkeletalMeshEditorSubsystem, Error, "SetAllowCPUAccess: The SkeletalMesh is null.");
		return;
	}

	TargetMesh->Modify();

	for (int32 i=0; i < TargetMesh->GetLODNum(); ++i)
	{
		TargetMesh->GetLODInfo(i)->bAllowCPUAccess = bAllowCPUAccess;
	}
