...
{	
	...
	{
		UE_LOGF(LogDataHierarchyEditor, Warning, "More than one child with the same identity has been found in parent %ls", *ToString());
	}
...
void UHierarchyElement::PostLoad()
{
	...
	if(bAnyChildNullptr)
	{
		...
		UE_LOGF(LogDataHierarchyEditor, Warning, "HierarchyElement %ls found nullptr child in asset %ls. Removed all nullptr children. This is indicative of something wrong. Check if the hierarchy is still correct and fix it, if necessary.", *ToString(), *GetNameSafe(Package))	
	}
...
{		
	...
	AllSection = NewObject<UHierarchySection>(this, GetSectionDataClass(), AllSectionHierarchyObjectName,  RF_Transient);
	DefaultHierarchySectionViewModel = StaticCastSharedPtr<FHierarchySectionViewModel>(CreateViewModelForElement(AllSection, GetHierarchyRootViewModel().ToSharedRef()));
...
void UDataHierarchyViewModelBase::OnHierarchyChanged(const TInstancedStruct<FHierarchyChangedPayload>& Payload)
{
	if(Payload.IsValid())
	{
		if(Payload.GetScriptStruct() == FHierarchyChangedPayload_ElementsAdded::StaticStruct())
		{
			TArray<TSharedPtr<FHierarchyElementViewModel>> AddedElementViewModels = Payload.GetPtr<FHierarchyChangedPayload_ElementsAdded>()->AddedElementViewModels;
			
			bool bContainsHierarchyElement = AddedElementViewModels.ContainsByPredicate([](TSharedPtr<FHierarchyElementViewModel> Candidate)
			{
				return Candidate->GetData()->IsA<UHierarchyItem>() || Candidate->GetData()->IsA<UHierarchyCategory>();
			});
			
			bool bContainsSection = AddedElementViewModels.ContainsByPredicate([](TSharedPtr<FHierarchyElementViewModel> Candidate)
			{
				return Candidate->GetData()->IsA<UHierarchySection>();
			});
			
			// when a new item is created (opposed to dragged & dropped from source view, i.e. only categories so far)
			// we make sure to request a tree refresh, select the row, and request a pending rename since the widget will be created a frame later
			if (bContainsHierarchyElement)
			{
				RefreshHierarchyView(false);
				
				if (AddedElementViewModels.Num() == 1)
				{
					NavigateToElementInHierarchy(AddedElementViewModels[0].ToSharedRef());
				}
			}
			...
			if (bContainsSection)
			{
				...
				if (AddedElementViewModels.Num() == 1)
				{
					SetActiveHierarchySection(StaticCastSharedPtr<FHierarchySectionViewModel>(AddedElementViewModels[0]));
				}
			}
			
			// For any element added to the root, we fixup section association
			TSharedPtr<FHierarchySectionViewModel> ActiveSectionViewModel = GetActiveHierarchySectionViewModel();
			for (TSharedPtr<FHierarchyElementViewModel> AddedElement : AddedElementViewModels)
			{
				if (AddedElement->GetData()->IsA<UHierarchySection>())
				{
					continue;
				}
				
				TSharedPtr<FHierarchyElementViewModel> ParentViewModel = AddedElement->GetParent().Pin();
				if (!ParentViewModel->GetData()->IsA<UHierarchyRoot>())
				{
					continue;
				}
				
				// If we are in the all section, we clear the section association
				if (ActiveSectionViewModel->IsAllSection())
				{
					AddedElement->GetDataMutable()->DeleteMetaDataOfType<FDataHierarchyElementMetaData_SectionAssociation>();
				}
				// If not, we attempt to update the section association
				else
				{
					if (ActiveSectionViewModel->CanContain(AddedElement->GetData()->GetClass()).bResult)
					{
						FDataHierarchyElementMetaData_SectionAssociation* SectionAssociation = AddedElement->GetDataMutable()->FindOrAddMetaDataOfType<FDataHierarchyElementMetaData_SectionAssociation>();
						SectionAssociation->Section = ActiveSectionViewModel->GetSectionData();
					}
				}
			}
			...
			if (AddedElementViewModels.Num() == 1)
			{
				AddedElementViewModels[0]->RequestRenamePending();
			}
		}

		if(Payload.GetScriptStruct() == FHierarchyChangedPayload_ElementsDeleted::StaticStruct())
		{
			TArray<TSharedPtr<FHierarchyElementViewModel>> DeletedElementViewModels = Payload.GetPtr<FHierarchyChangedPayload_ElementsDeleted>()->DeletedElementViewModels;
			
			bool bContainsHierarchyElement = DeletedElementViewModels.ContainsByPredicate([](TSharedPtr<FHierarchyElementViewModel> Candidate)
			{
				return Candidate->GetData()->IsA<UHierarchyItem>() || Candidate->GetData()->IsA<UHierarchyCategory>();
			});
			
			bool bContainsSection = DeletedElementViewModels.ContainsByPredicate([](TSharedPtr<FHierarchyElementViewModel> Candidate)
			{
				return Candidate->GetData()->IsA<UHierarchySection>();
			});
			
			bool bContainsActiveSection = DeletedElementViewModels.Contains(ActiveHierarchySectionViewModel.Pin());
			...
			else if(bContainsSection)
			{
				RefreshSectionsView();

				if(bContainsActiveSection)
				{
					SetActiveHierarchySection(GetDefaultHierarchySectionViewModel());
				}
			}
		}
	}
	
	OnHierarchyChangedDelegate.Broadcast(Payload);
}
...
void UDataHierarchyViewModelBase::ToolMenuDelete(const FToolMenuContext& Context)
{
	UHierarchyMenuContext* HierarchyMenuContext = Context.FindContext<UHierarchyMenuContext>();
...
TSharedRef<FHierarchyDragDropOp> UDataHierarchyViewModelBase::CreateDragDropOp(const FDataHierarchyDragDropContext& DragContext)
{
	TSharedRef<FHierarchyDragDropOp> DragDropOp = MakeShared<FHierarchyDragDropOp>(DragContext);
...
TSharedPtr<FHierarchyElementViewModel> FHierarchyElementViewModel::AddChild(TSubclassOf<UHierarchyElement> NewChildClass, int32 InsertIndex, FHierarchyElementIdentity ChildIdentity)
{
	...
	TInstancedStruct<FHierarchyChangedPayload_ElementsAdded> Payload = TInstancedStruct<FHierarchyChangedPayload_ElementsAdded>::Make();
	Payload.GetMutable<FHierarchyChangedPayload_ElementsAdded>().AddedElementViewModels.Add(ElementViewModel);
...
{
	// Guard against re-entry on the same element. A listener on OnHierarchyChangedDelegate can respond to a mutation that
	// fires mid-walk (e.g. a Child->Delete() in RefreshChildrenData) by synchronously calling SyncViewModelsToData on the
	// root again. Without this guard that inner call rebuilds Children while outer frames are still iterating it. The flag
	// is per-element, so the legitimate recursive descent into child view models is unaffected.
	if (bIsSyncInProgress)
	{
		UE_LOGFMT(LogDataHierarchyEditor, Warning, "Re-entrant SyncViewModelsToData call on element {ElementName} skipped. The outer sync in progress will produce the final state. Did you call SyncViewModelsToData in the middle of another sync (for example from an OnHierarchyChanged listener)? Set LogDataHierarchyEditor to Verbose to dump the callstack and help find the callsite.", ToString());
		if (UE_LOG_ACTIVE(LogDataHierarchyEditor, Verbose))
		{
			FDebug::DumpStackTraceToLog(TEXT("Re-entrant SyncViewModelsToData callstack:"), ELogVerbosity::Log);
		}
		return;
	}
	TGuardValue<bool> SyncGuard(bIsSyncInProgress, true);
...
TSharedRef<FHierarchyElementViewModel> FHierarchyElementViewModel::DuplicateToThis(TSharedPtr<FHierarchyElementViewModel> ItemToDuplicate, int32 InsertIndex)
{
	...
	TInstancedStruct<FHierarchyChangedPayload_ElementsAdded> Payload = TInstancedStruct<FHierarchyChangedPayload_ElementsAdded>::Make();
	Payload.GetMutablePtr<FHierarchyChangedPayload_ElementsAdded>()->AddedElementViewModels.Add(ViewModel);
	HierarchyViewModel->OnHierarchyChanged(Payload);
...
TSharedRef<FHierarchyElementViewModel> FHierarchyElementViewModel::ReparentToThis(TSharedPtr<FHierarchyElementViewModel> ItemToMove, int32 InsertIndex)
{
	...
	TInstancedStruct<FHierarchyChangedPayload_ElementsMoved> Payload = TInstancedStruct<FHierarchyChangedPayload_ElementsMoved>::Make();
	Payload.GetMutablePtr<FHierarchyChangedPayload_ElementsMoved>()->MovedElementViewModels.Add(ViewModel);
	HierarchyViewModel->OnHierarchyChanged(Payload);
...
TSharedPtr<FHierarchyElementViewModel> FHierarchyElementViewModel::FindViewModelForObject(const UObject* Object, bool bSearchRecursively) const
{
	// If it's a hierarchy element we use the dedicated function
	if (Object->IsA<UHierarchyElement>())
	{
		return FindViewModelForChild(Cast<UHierarchyElement>(Object), bSearchRecursively);
	}
	
	// If not, we are looking for the view model that points to the object for editing
	for (TSharedPtr<FHierarchyElementViewModel> ChildViewModel : Children)
	{
		if (UObject* ObjectForEditing = ChildViewModel->GetObjectForEditing())
		{
			if (ObjectForEditing == Object)
			{
	...
	if (bSearchRecursively)
	{
		for(TSharedPtr<FHierarchyElementViewModel> ChildViewModel : Children)
		{
			TSharedPtr<FHierarchyElementViewModel> FoundViewModel = ChildViewModel->FindViewModelForObject(Object, bSearchRecursively);
...
{	
	...
	TInstancedStruct<FHierarchyChangedPayload_ElementsDeleted> Payload = TInstancedStruct<FHierarchyChangedPayload_ElementsDeleted>::Make();
	Payload.GetMutable<FHierarchyChangedPayload_ElementsDeleted>().DeletedElementViewModels.Add(Child);
	HierarchyViewModel->OnHierarchyChanged(Payload);
}
...
TOptional<EItemDropZone> FHierarchyElementViewModel::OnCanRowAcceptDrop(const FDragDropEvent& DragDropEvent, EItemDropZone ItemDropZone, TSharedPtr<FHierarchyElementViewModel> Item)
{
	if(TSharedPtr<FHierarchyDragDropOp> DragDropOp = DragDropEvent.GetOperationAs<FHierarchyDragDropOp>())
	{
		...
		{
			TSharedPtr<FHierarchyElementViewModel> DraggedElement = TopLevelEntries[0].Element.Pin();
			if (!DraggedElement.IsValid())
			{
				return TOptional<EItemDropZone>();
			}

			FResultWithUserFeedback Result = CanDropOn(DraggedElement, ItemDropZone);
			if (Result.UserFeedback.IsSet())
			{
				DragDropOp->SetDescription(Result.UserFeedback.GetValue());
			}

			return Result.bResult ? TOptional<EItemDropZone>(ItemDropZone) : TOptional<EItemDropZone>();
		}
		...
		if (TopLevelEntries.Num() > 1)
		{
			...
			{
				TSharedPtr<FHierarchyElementViewModel> DraggedElement = Entry.Element.Pin();
				if (!DraggedElement.IsValid())
				{
					continue;
				}
				...
				else if (RejectionMessage.IsEmpty() && Result.UserFeedback.IsSet())
				{
					RejectionMessage = Result.UserFeedback.GetValue();
				}
			}

			if (SuccessCount == 0)
			{
				if (RejectionMessage.IsEmpty() == false)
				{
					DragDropOp->SetDescription(RejectionMessage);
				}
				return TOptional<EItemDropZone>();
			}

			const bool bAddingItems = TopLevelEntries[0].Element.IsValid() && (TopLevelEntries[0].Element.Pin()->IsForHierarchy() == false);
			const FText TargetText = this->ToStringAsText();
			FText MultiDropDescription;
			if (SuccessCount == TotalCount)
			{
				const FText FormatText = bAddingItems ? LOCTEXT("MultiDropAdd", "Add {0} items to {1}") : LOCTEXT("MultiDropMove", "Move {0} items to {1}");
				MultiDropDescription = FText::Format(FormatText, TotalCount, TargetText);
			}
			else
			{
				const FText FormatText = bAddingItems ? LOCTEXT("MultiDropAddPartial", "Add {0} of {1} items to {2}") : LOCTEXT("MultiDropMovePartial", "Move {0} of {1} items to {2}");
				MultiDropDescription = FText::Format(FormatText, SuccessCount, TotalCount, TargetText);
			}

			DragDropOp->SetDescription(MultiDropDescription);
...
FReply FHierarchyElementViewModel::OnDroppedOnRow(const FDragDropEvent& DragDropEvent, EItemDropZone ItemDropZone, TSharedPtr<FHierarchyElementViewModel> Item)
{
	if(TSharedPtr<FHierarchyDragDropOp> HierarchyDragDropOp = DragDropEvent.GetOperationAs<FHierarchyDragDropOp>())
	{
		...
		FScopedTransaction Transaction(LOCTEXT("Transaction_MultiDrop", "Dropped items in the hierarchy"));
		HierarchyViewModel->GetHierarchyRoot()->Modify();

		for(const FDraggedElementEntry& Entry : TopLevelEntries)
		{
			TSharedPtr<FHierarchyElementViewModel> DroppedElement = Entry.Element.Pin();
			if (DroppedElement.IsValid() && CanDropOn(DroppedElement, ItemDropZone).bResult)
			{
				OnDroppedOn(DroppedElement, ItemDropZone);
			}
		}
...
void FHierarchyElementViewModel::Rename(FName NewName)
{
	FName OldName = FName(ToString());
	
	RenameInternal(NewName); 
	
	TInstancedStruct<FHierarchyChangedPayload_ElementNameChanged> Payload = TInstancedStruct<FHierarchyChangedPayload_ElementNameChanged>::Make();
	Payload.GetMutable<FHierarchyChangedPayload_ElementNameChanged>().ElementViewModel = AsShared();
	Payload.GetMutable<FHierarchyChangedPayload_ElementNameChanged>().OldName = OldName;
	Payload.GetMutable<FHierarchyChangedPayload_ElementNameChanged>().NewName = NewName;
...
void FHierarchyElementViewModel::OnDroppedOn(TSharedPtr<FHierarchyElementViewModel> DroppedItem, EItemDropZone ItemDropZone)
{
	...
	HierarchyViewModel->GetHierarchyRoot()->Modify();
	HierarchyViewModel->GetHierarchyRootViewModel()->PreEditHierarchyStructure();
	...
	{
		HierarchyViewModel->GetHierarchyRootViewModel()->PostEditHierarchyStructure();
...
FHierarchyElementViewModel::FResultWithUserFeedback FHierarchyRootViewModel::CanDropOnInternal(TSharedPtr<FHierarchyElementViewModel> DraggedElement, EItemDropZone ItemDropZone)
{
	...
	if (TSharedPtr<FHierarchySectionViewModel> HierarchySectionViewModel = HierarchyViewModel->GetActiveHierarchySectionViewModel())
	{
		if(HierarchySectionViewModel->IsAllSection() == false && !DraggedElement->GetData()->IsA<UHierarchyCategory>())
		{
			Results.bResult = false;
			...
			Results.UserFeedback = FText::FormatOrdered(BaseMessage, HierarchySectionViewModel->GetSectionNameAsText());
		}
	}
	else
	{
		UE_LOGF(LogDataHierarchyEditor, Warning, "HierarchyViewModel (%ls) has a null section view model", *ToString());
	}
...
TSharedPtr<FHierarchySectionViewModel> FHierarchyRootViewModel::AddSection()
{
	...
	TInstancedStruct<FHierarchyChangedPayload_ElementsAdded> Payload = TInstancedStruct<FHierarchyChangedPayload_ElementsAdded>::Make();
	Payload.GetMutable<FHierarchyChangedPayload_ElementsAdded>().AddedElementViewModels.Add(SectionViewModel);
	HierarchyViewModel->OnHierarchyChanged(Payload);
...
void FHierarchyRootViewModel::DeleteSection(TSharedPtr<FHierarchyElementViewModel> InSectionViewModel)
{
	...
	TInstancedStruct<FHierarchyChangedPayload_ElementsDeleted> Payload = TInstancedStruct<FHierarchyChangedPayload_ElementsDeleted>::Make();
	Payload.GetMutable<FHierarchyChangedPayload_ElementsDeleted>().DeletedElementViewModels.Add(SectionViewModel);
	HierarchyViewModel->OnHierarchyChanged(Payload);
...
TOptional<bool> FHierarchySectionViewModel::OnDroppedOnOverride(TSharedPtr<FHierarchyElementViewModel> DroppedElementViewModel, EItemDropZone ItemDropZone)
{
	if(DroppedElementViewModel->GetData()->IsA<UHierarchySection>())
	{
		...
		if(bDropSucceeded)
		{
			...
			HierarchyViewModel->OnHierarchyChanged(TInstancedStruct<FHierarchyChangedPayload>::Make("Dropped section next to section"));
		}
	...
	if(UHierarchyElement* DroppedElement = DroppedElementViewModel->GetDataMutable())
	{
		...
		HierarchyViewModel->RefreshHierarchyView();
		HierarchyViewModel->OnHierarchyChanged(TInstancedStruct<FHierarchyChangedPayload>::Make("Dropped element on section"));
...
void UDataHierarchyViewModelBase::AddCategory(TSharedPtr<FHierarchyElementViewModel> CategoryParent)
{
...
void UDataHierarchyViewModelBase::GenerateDynamicContextMenuInternal(UToolMenu* DynamicToolMenu)
{
...
void UDataHierarchyViewModelBase::DeleteElementWithIdentity(FHierarchyElementIdentity Identity)
{
	...
	TSharedPtr<FHierarchyElementViewModel> ViewModelToDelete;
	
	if(TSharedPtr<FHierarchyElementViewModel> ViewModel = HierarchyRootViewModel->FindViewModelForChild(Identity, true))
	{
		if(ViewModel->CanDelete())
		{
			ViewModelToDelete = ViewModel;
		}
	}
	
	// If we haven't found an element in the hierarchy, check the sections
	if (!ViewModelToDelete.IsValid())
	{
		for(TSharedPtr<FHierarchySectionViewModel> SectionViewModel : HierarchyRootViewModel->GetSectionViewModels())
		{
			if(SectionViewModel->GetData()->GetPersistentIdentity() == Identity && SectionViewModel->CanDelete())
			{
				ViewModelToDelete = SectionViewModel;
			}
		}
	}
	...
	if (ViewModelToDelete.IsValid())
	{
		ViewModelToDelete->Delete();
		
		TInstancedStruct<FHierarchyChangedPayload_ElementsDeleted> Payload = TInstancedStruct<FHierarchyChangedPayload_ElementsDeleted>::Make();
		Payload.GetMutable<FHierarchyChangedPayload_ElementsDeleted>().DeletedElementViewModels.Add(ViewModelToDelete);
...
void UDataHierarchyViewModelBase::DeleteElements(TArray<TSharedPtr<FHierarchyElementViewModel>> ViewModels)
{
	...
	TArray<TSharedPtr<FHierarchyElementViewModel>> ViewModelsToDelete;
	...
	for (TSharedPtr<FHierarchyElementViewModel> ViewModelToDelete : ViewModelsToDelete)
	{
		ViewModelToDelete->Delete();
	...
	if(bAnyItemsDeleted)
	{
		...
		TInstancedStruct<FHierarchyChangedPayload_ElementsDeleted> Payload = TInstancedStruct<FHierarchyChangedPayload_ElementsDeleted>::Make();
		Payload.GetMutablePtr<FHierarchyChangedPayload_ElementsDeleted>()->DeletedElementViewModels = ViewModelsToDelete;
		OnHierarchyChanged(Payload);
	}
	else
	{
...
TArray<FDraggedElementEntry> FDataHierarchyDragDropContext::GetTopLevelEntries() const
{
	TArray<FDraggedElementEntry> TopLevel;
	for(const FDraggedElementEntry& Entry : Entries)
	{
		if(!Entry.bIsContainedInAnotherDraggedElement)
		{
			TopLevel.Add(Entry);
		}
	}
...
int32 FDataHierarchyDragDropContext::GetTopLevelCount() const
{
	int32 Count = 0;
	for(const FDraggedElementEntry& Entry : Entries)
	{
...
FDataHierarchyDragDropContext::~FDataHierarchyDragDropContext() = default;
FDataHierarchyDragDropContext::FDataHierarchyDragDropContext(const TArray<FDraggedElementEntry>& InEntries) : Entries(InEntries) {}
FDataHierarchyDragDropContext::FDataHierarchyDragDropContext(TArray<FDraggedElementEntry>&& InEntries) : Entries(MoveTemp(InEntries)) {}

const TArray<FDraggedElementEntry>& FDataHierarchyDragDropContext::GetEntries() const { return Entries; }
TWeakPtr<FHierarchyElementViewModel> FDataHierarchyDragDropContext::GetPrimaryElement() const { return Entries.Num() > 0 ? Entries[0].Element : nullptr; }
int32 FDataHierarchyDragDropContext::Num() const { return Entries.Num(); }
bool FDataHierarchyDragDropContext::IsEmpty() const { return Entries.IsEmpty(); }

FHierarchyDragDropOp::FHierarchyDragDropOp(const FDataHierarchyDragDropContext& InDragContext) : DragContext(InDragContext)
{
	TWeakPtr<FHierarchyElementViewModel> Primary = DragContext.GetPrimaryElement();
	if(Primary.IsValid())
	{
		int32 TopLevelCount = DragContext.GetTopLevelCount();
		FText PrimaryLabel = Primary.Pin()->ToStringAsText();

		if(TopLevelCount > 1)
		{
			SetLabel(FText::Format(NSLOCTEXT("DataHierarchyEditor", "MultiDragLabel", "{0} (+{1})"), PrimaryLabel, TopLevelCount - 1));
		}
		else
		{
