...
public:
	...
	virtual ~UHierarchyElement() override = default;
	...
	template<class PREDICATE_CLASS>
	void StableSortChildren(const PREDICATE_CLASS& Predicate, bool bRecursive = false);
...
template <class PREDICATE_CLASS>
void UHierarchyElement::StableSortChildren(const PREDICATE_CLASS& Predicate, bool bRecursive)
{
	Children.StableSort(Predicate);

	if(bRecursive)
	{
		for(TObjectPtr<UHierarchyElement> ChildElement : Children)
		{
			ChildElement->StableSortChildren(Predicate, bRecursive);
		}
	}
}
...
public:
	UHierarchyRoot() {}
	virtual ~UHierarchyRoot() override = default;
...
public:
	UHierarchyItem() {}
	virtual ~UHierarchyItem() override = default;
};
...
{
	TWeakPtr<FHierarchyElementViewModel> Element;
	/** True if this element is a descendant of another element in the same drag set.
	 *  When true, drop execution should skip this entry -- the parent's drop already includes it. */
	bool bIsContainedInAnotherDraggedElement = false;
};
...
{
	FDataHierarchyDragDropContext() = default;
	UE_API ~FDataHierarchyDragDropContext();
	UE_API explicit FDataHierarchyDragDropContext(const TArray<FDraggedElementEntry>& InEntries);
	UE_API explicit FDataHierarchyDragDropContext(TArray<FDraggedElementEntry>&& InEntries);

	/** All dragged element entries, including those contained in other dragged elements. */
	UE_API const TArray<FDraggedElementEntry>& GetEntries() const;

	/** Returns only top-level entries (not contained in another dragged element). */
	UE_API TArray<FDraggedElementEntry> GetTopLevelEntries() const;
	/** Returns the number of top-level dragged elements. */
	UE_API int32 GetTopLevelCount() const;

	/** Returns the first/primary dragged element. */
	UE_API TWeakPtr<FHierarchyElementViewModel> GetPrimaryElement() const;

	UE_API int32 Num() const;
	UE_API bool IsEmpty() const;
private:
	TArray<FDraggedElementEntry> Entries;
};
...
public:
	...
	UE_API FHierarchyDragDropOp(const FDataHierarchyDragDropContext& InDragContext);
	...
	/** Returns the first/primary dragged element. */
	TWeakPtr<FHierarchyElementViewModel> GetDraggedElement() const { return DragContext.GetPrimaryElement(); }
	/** Returns the full drag context. */
	const FDataHierarchyDragDropContext& GetDragContext() const { return DragContext; }
...
protected:
	FDataHierarchyDragDropContext DragContext;
...
USTRUCT()
struct FHierarchyChangedPayload
{
	GENERATED_BODY()
	
	FHierarchyChangedPayload() = default;
	FHierarchyChangedPayload(FString InMessage) : Message(InMessage) {}
	
	/** An optional message that can be set. */
	FString Message;
};

USTRUCT()
struct FHierarchyChangedPayload_ElementsAdded : public FHierarchyChangedPayload
{
	GENERATED_BODY()
	
	TArray<TSharedPtr<FHierarchyElementViewModel>> AddedElementViewModels;
};

USTRUCT()
struct FHierarchyChangedPayload_ElementsMoved : public FHierarchyChangedPayload
{
	GENERATED_BODY()
	
	TArray<TSharedPtr<FHierarchyElementViewModel>> MovedElementViewModels;
};

USTRUCT()
struct FHierarchyChangedPayload_ElementsDeleted : public FHierarchyChangedPayload
{
	GENERATED_BODY()

	TArray<TSharedPtr<FHierarchyElementViewModel>> DeletedElementViewModels;
};

USTRUCT()
struct FHierarchyChangedPayload_ElementPropertyChanged : public FHierarchyChangedPayload
{
	GENERATED_BODY()
	
	TSharedPtr<FHierarchyElementViewModel> ElementViewModel;
	const FPropertyChangedEvent* PropertyChangedEvent = nullptr;
	FProperty* PropertyThatChanged = nullptr;
};

USTRUCT()
struct FHierarchyChangedPayload_ElementNameChanged : public FHierarchyChangedPayload
{
	GENERATED_BODY()
...
{
public:
	DECLARE_MULTICAST_DELEGATE_OneParam(FOnHierarchyChanged, const TInstancedStruct<FHierarchyChangedPayload>&)
...
public:
	...
	/** Generic callback for when something in the hierarchy has changed. */
	UE_API virtual void OnHierarchyChanged(const TInstancedStruct<FHierarchyChangedPayload>& Payload);
	...
	/** Function to implement drag drop ops. FHierarchyDragDropOp is a default implementation. Inherit from it and override CreateCustomDecorator for custom UI. */
	UE_API virtual TSharedRef<FHierarchyDragDropOp> CreateDragDropOp(const FDataHierarchyDragDropContext& DragContext);
	/** This needs to return true if you want the details panel to show up. */
	virtual bool SupportsDetailsPanel() { return true; }
	/** If true, selecting multiple elements will show a multi-object details panel. If false, only the last selected element is shown. */
	virtual bool SupportsMultiEditInDetailsPanel() { return true; }

	/** If false, disables source item functionality (left panel). */
	virtual bool SupportsSourcePanel() const { return true; }

	/** The selection mode used for the hierarchy tree view. Override to change the default multi-selection behavior. */
	virtual ESelectionMode::Type GetHierarchySelectionMode() const { return ESelectionMode::Multi; }
...
private:
	...
	UE_API virtual void GenerateDynamicContextMenuInternal(UToolMenu* DynamicToolMenu);
public:
	...
	UE_API void AddCategory(TSharedPtr<FHierarchyElementViewModel> CategoryParent = nullptr);

	/** Delete all specified elements. */
	UE_API void DeleteElements(TArray<TSharedPtr<FHierarchyElementViewModel>> ViewModels);
	...
	FOnHierarchyChanged& GetOnHierarchyChangedDelegate() { return OnHierarchyChangedDelegate; } 
...
private:
	...
	UE_API void ToolMenuDelete(const FToolMenuContext& Context);
...
struct FHierarchyElementViewModel : TSharedFromThis<FHierarchyElementViewModel>, public FTickableEditorObject
{
	...
	/** Finds the view model for a given hierarchy element. Needs to be a child of the current element somewhere down the hierarchy. */
	...
	/** Finds the view model for a given hierarchy element identity. Needs to be a child of the current element somewhere down the hierarchy. */
	...
	UE_API TSharedPtr<FHierarchyElementViewModel> FindViewModelForObject(const UObject* Object, bool bSearchRecursively = false) const;
	...
	UE_API FResultWithUserFeedback CanDropOn(TSharedPtr<FHierarchyElementViewModel> DraggedElement, EItemDropZone ItemDropZone);

	/** Gets executed when an item was dropped on this. */
	UE_API void OnDroppedOn(TSharedPtr<FHierarchyElementViewModel> DroppedItem, EItemDropZone ItemDropZone);
	...
	virtual UObject* GetObjectForEditing() { return Element; }
...
protected:
	...
	virtual void PostOnDroppedOn(TSharedPtr<FHierarchyElementViewModel> DroppedElementViewModel) { }
...
struct FHierarchyRootViewModel : FHierarchyElementViewModel
{
	...
	virtual void PreEditHierarchyStructure() {}
	virtual void PostEditHierarchyStructure() {}
protected:
...
struct FHierarchySectionViewModel : FHierarchyElementViewModel
{
	...
	virtual ~FHierarchySectionViewModel() override = default;
	...
	virtual const FSlateBrush* GetSectionImageBrush() const { return SectionImage ? SectionImage : FAppStyle::GetNoBrush(); }
...
struct FHierarchyItemViewModel : FHierarchyElementViewModel
{
	...
	virtual ~FHierarchyItemViewModel() override = default;
...
struct FHierarchyCategoryViewModel : FHierarchyElementViewModel
{
	...
	virtual ~FHierarchyCategoryViewModel() override= default;
...
public:
	...
	FSectionDragDropOp(TSharedPtr<FHierarchySectionViewModel> SectionViewModel) : FHierarchyDragDropOp(FDataHierarchyDragDropContext({FDraggedElementEntry{SectionViewModel}})) {}

	TWeakPtr<FHierarchySectionViewModel> GetDraggedSection() const { return StaticCastSharedPtr<FHierarchySectionViewModel>(GetDraggedElement().Pin()); }
private:
