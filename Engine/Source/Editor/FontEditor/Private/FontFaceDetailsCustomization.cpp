...
namespace Private
{
	static const FString FileTypes = TEXT("All Font Files (*.ttf, *.ttc, *.otf, *.otc)|*.ttf;*.ttc;*.otf;*.otc|TrueType fonts (*.ttf, *.ttc)|*.ttf;*.ttc|OpenType fonts (*.otf, *.otc)|*.otf;*.otc");
}
			...
			[
				...
				+ SHorizontalBox::Slot()
					...
					[
						SNew(SButton)
							.ButtonStyle(FAppStyle::Get(), "HoverHintOnly")
							.ToolTipText(LOCTEXT("FontFileExportFontToolTip", "Export the font file to this computer"))
							.OnClicked(this, &FFontFaceDetailsCustomization::OnExportFont)
							.ContentPadding(2.0f)
							.ForegroundColor(FSlateColor::UseForeground())
							.IsFocusable(false)
							[
...
FReply FFontFaceDetailsCustomization::OnExportFont()
{
	IDesktopPlatform* const DesktopPlatform = FDesktopPlatformModule::Get();
	if (DesktopPlatform)
	{
		...
		for (const TWeakObjectPtr<UObject>& ObjectBeingEdited : ObjectsBeingEdited)
		{
			...
			if (FontFace)
			{
				const FString CurrentFilename = FPaths::GetBaseFilename(FontFace->SourceFilename);
				const FText Title = FText::Format(LOCTEXT("FontFace_ExportFontFileDialogTitle", "Export font '{0}'..."), FText::FromString(CurrentFilename));

				TArray<FString> OutFilenames;
				DesktopPlatform->SaveFileDialog(
					ParentWindowWindowHandle,
					Title.ToString(),
					(FontFace->SourceFilename.IsEmpty()) ? TEXT("") : FPaths::GetPath(FontFace->SourceFilename),
					(CurrentFilename.IsEmpty()) ? TEXT("") : CurrentFilename + TEXT(".ttf"),
					Private::FileTypes,
					EFileDialogFlags::None,
					OutFilenames
				);
				...
				{
					FFileHelper::SaveArrayToFile(FontFace->FontFaceData.Get().GetData(), *OutFilenames[0]);
				}
			}
		}
	}
