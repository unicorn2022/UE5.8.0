...
if [ $UID -eq 0 ]; then
	...
	if [ ${DISTRO} == "CentOS7" ]; then
		...
		yum install -y libXcursor-devel libXinerama-devel libxi-dev libXrandr-devel libXScrnSaver-devel libXi-devel libXtst-devel mesa-libGL-devel mesa-libEGL-devel pulseaudio-libs-devel wayland-protocols-devel wayland-devel libxkbcommon-devel mesa-libwayland-egl-devel alsa-lib-devel libudev-devel
		...
		yum install -y gcc gcc-c++ git-core make cmake \
			...
			libXi-devel libXScrnSaver-devel libXtst-devel dbus-devel systemd-devel \
