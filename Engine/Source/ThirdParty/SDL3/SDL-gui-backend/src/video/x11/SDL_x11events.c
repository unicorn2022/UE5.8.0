...
static SDL_X11ErrorCallback     g_X11ErrorCallback = NULL;
static void *g_X11ErrorCallbackData = NULL;
static int (*Prev_X11_Handler)(Display *, XErrorEvent *) = NULL;

static SDL_X11IOErrorCallback   g_X11IOErrorCallback = NULL;
static void *g_X11IOErrorCallbackData = NULL;
static int (*Prev_X11IO_Handler)(Display *) = NULL;

static int global_X11ErrorHandler(Display *d, XErrorEvent *e)
{
    if (g_X11ErrorCallback)
    {
        char errortext[512];
        X11_XGetErrorText(d, e->error_code, errortext, sizeof(errortext));

        return (*g_X11ErrorCallback)(errortext, e->error_code, e->request_code, e->minor_code, g_X11ErrorCallbackData);
    }

    if (Prev_X11_Handler)
    {
        return Prev_X11_Handler(d, e);
    }

    return 1;
}

static int global_X11IOErrorHandler(Display *d)
{
    if (g_X11IOErrorCallback)
    {
        return (*g_X11IOErrorCallback)(g_X11IOErrorCallbackData);
    }

    if (Prev_X11IO_Handler)
    {
        return Prev_X11IO_Handler(d);
    }

    return 1;
}

void SDL_SetX11ErrorHandler(SDL_X11ErrorCallback callbackerror, void *callbackerrordata, SDL_X11IOErrorCallback callbackioerror, void *callbackioerrordata)
{
    // only support setting this once
    if ((Prev_X11_Handler && callbackerror) || (Prev_X11IO_Handler && callbackioerror))
    {
        SDL_Log("SDL_SetX11ErrorHandler can only set the callbacks once.  Clear them before setting to a new function");
        return;
    }
    
    g_X11ErrorCallback = callbackerror;
    g_X11ErrorCallbackData = callbackerrordata;

    if (callbackerror != NULL)
    {
        Prev_X11_Handler = X11_XSetErrorHandler(global_X11ErrorHandler);
    }
    else
    {
        X11_XSetErrorHandler(Prev_X11_Handler);
        Prev_X11_Handler = NULL;
    }

    g_X11IOErrorCallback = callbackioerror;
    g_X11IOErrorCallbackData = callbackioerrordata;
    if (callbackioerror != NULL)
    {
        Prev_X11IO_Handler = X11_XSetIOErrorHandler(global_X11IOErrorHandler);
    }
    else
    {
        X11_XSetIOErrorHandler(Prev_X11IO_Handler);
        Prev_X11IO_Handler = NULL;
    }
}
