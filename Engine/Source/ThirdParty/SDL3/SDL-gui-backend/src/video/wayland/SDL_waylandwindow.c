...
#include "SDL_waylandshmbuffer.h"  // EG ADD
...
static void handle_xdg_surface_configure(void *data, struct xdg_surface *xdg, uint32_t serial)
{
    ...
    /* EG BEGIN */
    /* Epic Games: A Vulkan window keeps its xdg_surface alive across Hide
     * (see Wayland_HideWindow's early return), so configure events can still
     * be dispatched for a window that is mid-destroy. Touching the geometry
     * path in that state racing with Wayland_DestroyWindow's teardown
     * roundtrip crashes in wl_proxy_marshal_flags. Bail out.
     */
                                          ...
                                          struct wl_array *states)
{
    ...
    /* EG BEGIN */
    /* Epic Games: Companion guard to handle_configure_xdg_shell_surface for
     * the toplevel-state half of a configure. See note there.
     */
    if (window->is_destroying) {
        return;
    }
                                       ...
                                       int32_t height)
{
    ...
    /* EG BEGIN */
    /* Epic Games: Companion guard — tooltips and menus are Vulkan popups
     * whose xdg_popup is kept alive across Hide, so a mid-destroy configure
     * would touch a window whose state is being torn down.
     */
    if (wind->sdlwindow->is_destroying) {
        return;
    }
    /* EG END */
...
/* EG BEGIN */
    /* Epic Games: the early xdg role block in Wayland_CreateWindow (for Vulkan
     * toplevels) advances shell_surface_status before ShowWindow runs, so only
     * initialize it here from pre-show states. On a SHOW_PENDING retry, if the
     * xdg role already exists and the initial configure has been acked, resume
     * as WAITING_FOR_FRAME — a new configure is not coming, so jumping back to
     * WAITING_FOR_CONFIGURE would strand the surface. Leave any state past
     * WAITING_FOR_CONFIGURE alone so the event handlers retain control of the
     * state machine. */
    ...
    else if (data->shell_surface_status == WAYLAND_SHELL_SURFACE_STATUS_SHOW_PENDING)
    {
        // SDL 3.4.4 dropped the explicit initial_configure_seen flag; the xdg_surface
        // configure handler stamps the most recent configure serial into xdg.serial,
        // so a non-zero serial means at least one configure has been received.
        const bool role_assigned_and_acked =
            data->shell_surface.xdg.surface != NULL &&
            data->shell_surface.xdg.serial != 0;
        data->shell_surface_status = role_assigned_and_acked
     ...
     */
    /* EG BEGIN */
    /* Epic Games: Skip the buffer detach+commit when the caller has indicated that
     * an external renderer manages surface commits. This prevents a Wayland protocol
     * violation (xdg_wm_base error 4) when the external renderer has already committed
     * a buffer to the surface.
     */
    if (!SDL_GetBooleanProperty(SDL_GetWindowProperties(window), "UE.wayland.skip_surface_detach", false)) {
    ...
    if (data->shell_surface_type == WAYLAND_SHELL_SURFACE_TYPE_XDG_TOPLEVEL || data->shell_surface_type == WAYLAND_SHELL_SURFACE_TYPE_XDG_POPUP) {
        /* EG BEGIN */
        /* Epic Games: The xdg_surface may have already been created during window creation
         * for Vulkan windows (to prevent protocol violations). Skip if already assigned.
         */
        ...
        if (data->shell_surface_type == WAYLAND_SHELL_SURFACE_TYPE_XDG_POPUP) {
            ...
            // Destroy any previous positioner first (Vulkan windows keep roles alive on hide,
            // so the old positioner may still exist from a previous ShowWindow cycle).
            if (data->shell_surface.xdg.popup.xdg_positioner) {
                xdg_positioner_destroy(data->shell_surface.xdg.popup.xdg_positioner);
            }
            /* EG END */
            ...
            /* EG BEGIN */
            // Assign the popup role (Epic Games: skip if already assigned — Vulkan windows keep roles on hide)
            if (!data->shell_surface.xdg.popup.xdg_popup) {
                ...
                // Reposition the existing popup
                xdg_popup_reposition(data->shell_surface.xdg.popup.xdg_popup, data->shell_surface.xdg.popup.xdg_positioner, 0);

                // Note: Re-parenting a popup to a different xdg_surface is not possible
                // in Wayland. Reposition alone can't change the parent. Tooltip re-parenting
                // across toplevel windows is handled at the Slate level (per-window tooltips).
            }
            /* EG END */
            ...
            /* EG BEGIN */
            /* Epic Games: The toplevel may have been created during window creation for Vulkan. */
            if (!data->shell_surface.xdg.toplevel.xdg_toplevel) {
...
bool Wayland_CreateWindow(SDL_VideoDevice *_this, SDL_Window *window, SDL_PropertiesID create_props)
{
    ...
    /* EG BEGIN */
    /* Epic Games: For SDL_WINDOW_VULKAN windows, assign the xdg shell role here in
     * CreateWindow rather than waiting for ShowWindow. Vulkan's vkCreateSwapchainKHR
     * (called by UE between CreateWindow and ShowWindow when setting up the viewport)
     * internally commits a buffer to the wl_surface via Mesa's WSI. xdg-shell requires
     * the role to be assigned BEFORE any buffer commit; otherwise the compositor
     * raises xdg_wm_base error 4: "wl_surface already has a buffer committed".
     *
     * This is independent of the (now-rolled-back) transparent-buffer keep-alive trick:
     * even with upstream's destroy-role-on-hide pattern, the role still needs to exist
     * before Vulkan presents to the surface. ShowWindow's role-creation guards detect
     * the existing role and skip re-creation.
     *
     * libdecor manages the entire xdg lifecycle through its frame API and doesn't
     * support pre-creating the surface, so force libdecor windows to plain XDG.
     */
    if ((window->flags & SDL_WINDOW_VULKAN) && !custom_surface_role && c->shell.xdg &&
        !data->shell_surface.xdg.surface) {
        if (data->shell_surface_type == WAYLAND_SHELL_SURFACE_TYPE_LIBDECOR) {
            data->shell_surface_type = WAYLAND_SHELL_SURFACE_TYPE_XDG_TOPLEVEL;
        }

        /* Popups get their role during ShowWindow -- early assignment causes sizing
         * issues because the window dimensions aren't finalized at creation time. */
        if (data->shell_surface_type == WAYLAND_SHELL_SURFACE_TYPE_XDG_TOPLEVEL) {
            data->shell_surface.xdg.surface = xdg_wm_base_get_xdg_surface(c->shell.xdg, data->surface);
            xdg_surface_set_user_data(data->shell_surface.xdg.surface, data);
            xdg_surface_add_listener(data->shell_surface.xdg.surface, &_xdg_surface_listener, data);
            SDL_SetPointerProperty(SDL_GetWindowProperties(window), SDL_PROP_WINDOW_WAYLAND_XDG_SURFACE_POINTER, data->shell_surface.xdg.surface);
            ...
            data->shell_surface.xdg.toplevel.xdg_toplevel = xdg_surface_get_toplevel(data->shell_surface.xdg.surface);
            xdg_toplevel_add_listener(data->shell_surface.xdg.toplevel.xdg_toplevel, &toplevel_listener_xdg, data);
            SDL_SetPointerProperty(SDL_GetWindowProperties(window), SDL_PROP_WINDOW_WAYLAND_XDG_TOPLEVEL_POINTER, data->shell_surface.xdg.toplevel.xdg_toplevel);

            /* The commit below triggers the compositor to send the initial configure.
             * That event may dispatch before Wayland_ShowWindow runs (for child
             * toplevels whose first show is deferred until the parent's show iterates
             * its children). The toplevel configure handler only transitions the
             * status when it is already WAITING_FOR_CONFIGURE, so set that state here
             * rather than let the configure race against ShowWindow. */
            data->shell_surface_status = WAYLAND_SHELL_SURFACE_STATUS_WAITING_FOR_CONFIGURE;
...
void Wayland_SetWindowSize(SDL_VideoDevice *_this, SDL_Window *window)
{
    ...
    /* EG BEGIN */
    /* Epic Games: popup reposition is async (xdg_positioner -> xdg_popup.configure);
     * drain the round-trip so SDL_GetWindowSize reflects the new dimensions. */
    if (wind->shell_surface_type == WAYLAND_SHELL_SURFACE_TYPE_XDG_POPUP)
    {
        WAYLAND_wl_display_roundtrip(_this->internal->display);
    }
...
void Wayland_DestroyWindow(SDL_VideoDevice *_this, SDL_Window *window)
{
    ...
    if (data && wind) {
        /* EG BEGIN */
        /* Epic Games: Wayland_HideWindow's Vulkan early-return preserves xdg_popup,
         * xdg_toplevel, and xdg_surface so that a subsequent Show can reuse them
         * without destroying role objects the RHI-thread swapchain still references.
         * By the time Wayland_DestroyWindow runs, UE has deferred destruction long
         * enough for the RHI thread to drain (see
         * FLinuxPlatformApplicationMisc::PumpMessages's GWindowsPendingDestroy), so
         * releasing the roles here is safe. If we don't, the proxies are leaked and
         * their event listeners keep pointing at the SDL_WindowData we are about to
         * SDL_free below, so any compositor-queued event (e.g. a late xdg_popup
         * configure after tooltip dismissal) dispatched by the roundtrip on the
         * next line lands in freed memory.
         */
        if (wind->shell_surface_type == WAYLAND_SHELL_SURFACE_TYPE_XDG_TOPLEVEL ||
            wind->shell_surface_type == WAYLAND_SHELL_SURFACE_TYPE_XDG_POPUP) {
            if (wind->shell_surface_type == WAYLAND_SHELL_SURFACE_TYPE_XDG_POPUP) {
                Wayland_ReleasePopup(_this, window);
            } else if (wind->shell_surface.xdg.toplevel.xdg_toplevel) {
                xdg_toplevel_destroy(wind->shell_surface.xdg.toplevel.xdg_toplevel);
                wind->shell_surface.xdg.toplevel.xdg_toplevel = NULL;
                SDL_SetPointerProperty(SDL_GetWindowProperties(window), SDL_PROP_WINDOW_WAYLAND_XDG_TOPLEVEL_POINTER, NULL);
            }
            if (wind->shell_surface.xdg.surface) {
                xdg_surface_destroy(wind->shell_surface.xdg.surface);
                wind->shell_surface.xdg.surface = NULL;
                SDL_SetPointerProperty(SDL_GetWindowProperties(window), SDL_PROP_WINDOW_WAYLAND_XDG_SURFACE_POINTER, NULL);
            }
        }
        /* libdecor frames and CUSTOM surface types are handled by the normal hide-path
         * teardown and have no xdg roles for us to release here — leaving them alone
         * also avoids misreading the shell_surface union. */

        /* Pristine Wayland_HideWindow destroys these on hide, but our Vulkan
         * early-return above skips that path — meaning a Vulkan window that
         * was made modal (xdg_dialog_v1) or exported (zxdg_exported_v2) would
         * leak its protocol proxy across each hide/show cycle and never get
         * cleaned up at all. UE flags modal dialogs via
         * SDL_PROP_WINDOW_CREATE_MODAL_BOOLEAN, so the xdg_dialog_v1 path is
         * exercised in practice. Release them here at the final teardown.
         */
        if (wind->xdg_dialog_v1) {
            xdg_dialog_v1_destroy(wind->xdg_dialog_v1);
            wind->xdg_dialog_v1 = NULL;
        ...
        if (wind->exported) {
            zxdg_exported_v2_destroy(wind->exported);
            wind->exported = NULL;
            SDL_SetStringProperty(SDL_GetWindowProperties(window), SDL_PROP_WINDOW_WAYLAND_XDG_TOPLEVEL_EXPORT_HANDLE_STRING, NULL);
        }
        /* EG END */
        ...
        if (!(window->flags & SDL_WINDOW_EXTERNAL)) {
            /* EG BEGIN */
            /* Epic Games: Detach any attached buffer before destroying the surface.
             * Without this, Mutter can crash during wl_client_destroy with a
             * use-after-free when the shared transparent buffer is freed before
             * the surface's signal handlers are disconnected.
             */
            wl_surface_attach(wind->surface, NULL, 0, 0);
            wl_surface_commit(wind->surface);
            /* EG END */
