...
/* EG BEGIN */
/**
 * A callback to be used with SDL_SetX11ErrorHandler.
 *
 * Epic Games only extension to SDL3 to set the X11 error handler
 * call back functions
 *
 * These params are all extracted from the native X11 XErrorEvent and passed as native types to avoid having to have X11 headers
 * \param errortext     a temp buffer containing the X11 error text, only valid during the callback
 * \param error_code    the XErrorEvent error_code
 * \param request_code  the XErrorEvent request_code
 * \param minor_code    the XErrorEvent minor_code
 * \param userdata      a pointer to pass to every iteration of `callback`.
 *
 * \returns return value is actually ignored by X11
 *
 * \threadsafety X11 can call this on any thread.
  *
 * \since Epic Games only function available since 3.2.10
 *
 * \sa SDL_X11ErrorCallback
 */
typedef int (SDLCALL *SDL_X11ErrorCallback)(const char *errortext, int error_code, int request_code, int minor_code, void *userdata);
 ...
 * call back functions
 *
 * IO errors are generally fatal but this callback should let the app cleanly exit instead of having X11 call exit() directly
 *
 * \param userdata a pointer to pass to every iteration of `callback`.
 * 
 * \returns return value is actually ignored by X11.  Callback should normally force an exit for X11 IO errors if it does return exit() will be called
 *
 * \threadsafety X11 can call this on any thread.
 *
 * \since Epic Games only function available since 3.2.10
 *
 * \sa SDL_X11IOErrorCallback
 */
typedef int (SDLCALL *SDL_X11IOErrorCallback)(void *userdata);

/**
 * Set a callback for X11 errors.  Only one global callback can be registered via SDL at a time.
 *
 * \param callbackerror         The SDL_X11ErrorCallback to call.    Passing null clears the callback
 * \param callbackerrordata     a pointer to pass to every iteration of error callback. 
 * \param callbackioerror       The SDL_X11IOErrorCallback to call.  Passing null clears the callback
 * \param callbackioerrordata   a pointer to pass to every iteration of error callback. 
 
 * \since Epic Games only function available since 3.2.10* 
 *
 * \sa SDL_SetX11ErrorHandler
 */
extern SDL_DECLSPEC void SDLCALL SDL_SetX11ErrorHandler(SDL_X11ErrorCallback callbackerror, void *callbackerrordata, SDL_X11IOErrorCallback callbackioerror, void *callbackioerrordata);
/* EG END */
