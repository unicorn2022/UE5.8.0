...
rsync -a -v --delete --exclude="bazel" --exclude="cmake" --exclude="libcef_dll" --exclude="tests" --exclude="Resources" --exclude="*.so" --exclude="*.so.1" --exclude="*.bin" --exclude="*.txt" --exclude="*.md" --exclude="*.html" --exclude="*.bazel" --exclude="*.gypi" --exclude="chrome-sandbox" --exclude="vk_swiftshader_icd.json" --exclude="Doxyfile" --exclude="WORKSPACE" "$CEF_DROP_SRC_DIR" "$SCRIPT_DIR"
