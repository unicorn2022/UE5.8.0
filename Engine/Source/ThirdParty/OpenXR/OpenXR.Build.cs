...
public class OpenXR : ModuleRules
{
	...
	public OpenXR(ReadOnlyTargetRules Target) : base(Target)
	{
		...
		{
			string OpenXRPath = "$(EngineDir)/Binaries/ThirdParty/OpenXR/LinuxArm64/aarch64-unknown-linux-gnueabi/libopenxr_loader.so";
		...
		else if (Target.Platform == UnrealTargetPlatform.Android)
		{
			...
			string BasePath = "$(EngineDir)/Binaries/ThirdParty/OpenXR/";
			foreach (string Architecture in Architectures)
			{
				string OpenXRPath = Path.Combine(BasePath, "Android", Architecture, "libopenxr_loader.so");
				PublicAdditionalLibraries.Add(OpenXRPath);
				RuntimeDependencies.Add(OpenXRPath);
			}
