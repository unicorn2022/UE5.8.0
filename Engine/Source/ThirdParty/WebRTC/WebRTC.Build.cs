...
public class WebRTC : ModuleRules
{
	...
	public WebRTC(ReadOnlyTargetRules Target) : base(Target)
	{
		...
		if (bShouldUseWebRTC)
		{
			...
			{
				// NOTE: We can't use Target.Platform.IsInGroup(UnrealPlatformGroup.Apple) because that includes tvOS
			...
			else if (Target.Platform == UnrealTargetPlatform.IOS && bShouldUse5414WebRTC)
			{
				// NOTE: We can't use Target.Platform.IsInGroup(UnrealPlatformGroup.Apple) because that includes tvOS
				PublicDefinitions.Add("WEBRTC_MAC=1");
				PublicDefinitions.Add("WEBRTC_POSIX=1");
				PublicDefinitions.Add("WEBRTC_IOS=1");

				string LibraryPath = Path.Combine(WebRtcSdkPath, "Lib", PlatformSubdir, ConfigPath);
				PublicAdditionalLibraries.Add(Path.Combine(LibraryPath, "libwebrtc.a"));
			}
			else if (Target.IsInPlatformGroup(UnrealPlatformGroup.Android) && bShouldUse5414WebRTC)
			{
				PublicDefinitions.Add("WEBRTC_POSIX=1");
				PublicDefinitions.Add("WEBRTC_ANDROID=1");
				...
				{				
					string LibraryDirectory = Path.Combine(WebRtcSdkPath, "Lib", PlatformSubdir, Architecture, ConfigPath);
					PublicAdditionalLibraries.Add(Path.Combine(LibraryDirectory, "libwebrtc.a"));
				}

				string PluginPath = Utils.MakePathRelativeTo(ModuleDirectory, Target.RelativeEnginePath);
				AdditionalPropertiesForReceipt.Add("AndroidPlugin", Path.Combine(PluginPath, "WebRTC_UPL.xml"));
			}
