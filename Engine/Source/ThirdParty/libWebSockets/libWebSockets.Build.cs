...
public class libWebSockets : ModuleRules
{
	...
	protected virtual string IncludeDirectory
	{
		get
		{
			...
			{
				return Path.Combine(WebSocketsPackagePath, "include", "Win64", "VS" + Target.WindowsPlatform.GetVisualStudioCompilerVersionName());
			}
	...
	protected virtual string LibraryDirectory
	{
		get
		{
			if (Target.IsInPlatformGroup(UnrealPlatformGroup.Windows))
			{
				return Path.Combine(WebSocketsPackagePath, "lib", "Win64", "VS" + Target.WindowsPlatform.GetVisualStudioCompilerVersionName(), ConfigName);
			}
			else if (Target.IsInPlatformGroup(UnrealPlatformGroup.Unix))
			{
