...
public class Steamworks : ModuleRules
{
	public Steamworks(ReadOnlyTargetRules Target) : base(Target)
	{
        ...
        double SteamVersionNumber = 1.64;
		...
		{
			SteamBinariesDir += "androidarm64";
			LibraryPath = LibraryPath + "android/arm64/libsteam_api.so";

			if (!Directory.Exists(SteamBinariesDir))
			{
				string Err = string.Format("steamworks bins directory not found in not found in {0}", SteamBinariesDir);
				System.Console.WriteLine(Err);
				throw new BuildException(Err);
			}
			...
			{
				string Err = string.Format("steamworks lib not found in not found in {0}", LibraryPath);
				System.Console.WriteLine(Err);
				throw new BuildException(Err);
			}
			...
			AdditionalPropertiesForReceipt.Add("AndroidPlugin", Path.Combine(PluginPath, "Steamworks_APL.xml"));
		}
	}
}