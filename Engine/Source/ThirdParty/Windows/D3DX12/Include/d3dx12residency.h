...
#define MAX_NUM_CONCURRENT_CMD_LISTS 1024
	...
	public:
		...
		// BEGIN EPIC MOD
		void Initialize(ID3D12Pageable* pUnderlyingIn, UINT64 ObjectSize, UINT64 InitialGPUSyncPoint = 0, RESIDENCY_STATUS InitialResidencyStatus = RESIDENCY_STATUS::RESIDENT)
		{
			RESIDENCY_CHECK(pUnderlying == nullptr);
			pUnderlying = pUnderlyingIn;
			Size = ObjectSize;
			LastGPUSyncPoint = InitialGPUSyncPoint;
			ResidencyStatus = InitialResidencyStatus;
		}
		// END EPIC MOD
			...
			void BeginTrackingObject(ManagedObject* pObject)
			{
				...
				if (pObject)
				{
					...
					if (cStartEvicted)
					{
						...
						{
							pObject->ResidencyStatus = ManagedObject::RESIDENCY_STATUS::EVICTED;
							hr = Device->Evict(1, &pObject->pUnderlying);

							// Failed to evict, resource state is still resident
							if (hr != S_OK)
							{
								pObject->ResidencyStatus = ManagedObject::RESIDENCY_STATUS::RESIDENT;
							}
					...
					else
					{
						...
						{
							pObject->ResidencyStatus = ManagedObject::RESIDENCY_STATUS::RESIDENT;
							hr = Device->MakeResident(1, &pObject->pUnderlying);

							// Failed to make resident, resource state is still evicted
							if (hr != S_OK)
							{
								pObject->ResidencyStatus = ManagedObject::RESIDENCY_STATUS::EVICTED;
							}
	...
	public:
		...
		// BEGIN EPIC MOD
		FORCEINLINE void SetStartEvicted(bool bStartEvicted) { Manager.cStartEvicted = bStartEvicted; }
