...
// -------------------------------------------------------------------------------------------------------
// Setting the allocation functions used internally by mimalloc; these functions can be overridden by the
// program including mimalloc for tracking. These are called from every place where mimalloc talks to OS.
// Using these overrides has a small cost, so they are only used if MI_USE_EXTERNAL_OS_TRACKING is defined.
// -------------------------------------------------------------------------------------------------------
#if !defined(MI_USE_EXTERNAL_OS_TRACKING)
#define MI_USE_EXTERNAL_OS_TRACKING 0
#endif

typedef struct mi_ext_os_tracking_s
{
	// Called when OS memory was allocated.
	void (*on_alloc)(void* addr, size_t size, bool is_large, bool committed);

	// Called when OS memory was freed.
	void (*on_free)(void* addr, size_t size);

	// Called when OS memory was committed.
	void (*on_commit)(void* addr, size_t size);

	// Called when OS memory was decommitted.
	void (*on_decommit)(void* addr, size_t size, bool needs_recommit);
} mi_ext_os_tracking_t;

#if MI_USE_EXTERNAL_OS_TRACKING
mi_decl_export void mi_ext_set_os_tracking(const mi_ext_os_tracking_t* tracking);
#endif
// END EPIC MOD
