...
// BEGIN EPIC MOD - OsAllocatorTrackingHooks
static void _mi_ext_os_tracking_alloc_noop(void* addr, size_t size, bool is_large, bool committed) {
  MI_UNUSED(addr); MI_UNUSED(size); MI_UNUSED(is_large); MI_UNUSED(committed);
}

static void _mi_ext_os_tracking_free_noop(void* addr, size_t size) {
  MI_UNUSED(addr); MI_UNUSED(size);
}

static void _mi_ext_os_tracking_commit_noop(void* addr, size_t size) {
  MI_UNUSED(addr); MI_UNUSED(size);
}

static void _mi_ext_os_tracking_decommit_noop(void* addr, size_t size, bool needs_recommit) {
  MI_UNUSED(addr); MI_UNUSED(size); MI_UNUSED(needs_recommit);
}

static mi_ext_os_tracking_t _mi_ext_os_tracking = {
  .on_alloc    = _mi_ext_os_tracking_alloc_noop,
  .on_free     = _mi_ext_os_tracking_free_noop,
  .on_commit   = _mi_ext_os_tracking_commit_noop,
  .on_decommit = _mi_ext_os_tracking_decommit_noop,
};

void mi_ext_set_os_tracking(const mi_ext_os_tracking_t* tracking) {
  mi_assert(tracking);
  mi_assert(tracking->on_alloc);
  mi_assert(tracking->on_free);
  mi_assert(tracking->on_commit);
  mi_assert(tracking->on_decommit);

  _mi_ext_os_tracking = *tracking;
}

#if MI_USE_EXTERNAL_OS_TRACKING
  #define _mi_ext_os_track_alloc(addr,size,is_large,committed) \
    _mi_ext_os_tracking.on_alloc((addr), (size), (is_large), (committed))
  #define _mi_ext_os_track_free(addr,size) \
    _mi_ext_os_tracking.on_free((addr), (size))
  #define _mi_ext_os_track_commit(addr,size) \
    _mi_ext_os_tracking.on_commit((addr), (size))
  #define _mi_ext_os_track_decommit(addr,size,needs_recommit) \
    _mi_ext_os_tracking.on_decommit((addr), (size), (needs_recommit))
#else // MI_USE_EXTERNAL_OS_TRACKING
  #define _mi_ext_os_track_alloc(addr,size,is_large,committed)
  #define _mi_ext_os_track_free(addr,size)
  #define _mi_ext_os_track_commit(addr,size)
  #define _mi_ext_os_track_decommit(addr,size,needs_recommit)
#endif // !MI_USE_EXTERNAL_OS_TRACKING
// END EPIC MOD - OsAllocatorTrackingHooks
...
static void mi_os_prim_free(void* addr, size_t size, bool still_committed, mi_stats_t* tld_stats) {
  ...
  // BEGIN EPIC MOD - OsAllocatorTrackingHooks
  _mi_ext_os_track_free(addr, size);
  // END EPIC MOD
}
...
static void* mi_os_prim_alloc(size_t size, size_t try_alignment, bool commit, bool allow_large, bool* is_large, bool* is_zero, mi_stats_t* stats) {
  ...
  *is_zero = false;
  ...
  if (p != NULL) {
    ...
    // BEGIN EPIC MOD - OsAllocatorTrackingHooks
    _mi_ext_os_track_alloc(p, size, *is_large, commit);
    // END EPIC MOD
  }
  return p;
}
...
bool _mi_os_commit(void* addr, size_t size, bool* is_zero, mi_stats_t* tld_stats) {
  ...
  // BEGIN EPIC MOD - OsAllocatorTrackingHooks
  _mi_ext_os_track_commit(start, csize);
  // END EPIC MOD
  return true;
}
...
static bool mi_os_decommit_ex(void* addr, size_t size, bool* needs_recommit, mi_stats_t* tld_stats) {
  ...
  *needs_recommit = true;
  ...
  // BEGIN EPIC MOD - OsAllocatorTrackingHooks
  if (err == 0) {
    _mi_ext_os_track_decommit(start, csize, *needs_recommit);
  }
  // END EPIC MOD
...
void* _mi_os_alloc_huge_os_pages(size_t pages, int numa_node, mi_msecs_t max_msecs, size_t* pages_reserved, size_t* psize, mi_memid_t* memid) {
  *memid = _mi_memid_none();
  ...
  while (page < pages) {
    ...
    _mi_ext_os_track_alloc(p, MI_HUGE_OS_PAGE_SIZE, /*is_large*/ true, /*committed*/ true);
