...
static SpvReflectPrvNode* FindPreviousNode(SpvReflectPrvParser* p_parser, uint32_t result_id, uint32_t node_index_end)
{
  SpvReflectPrvNode* p_node = NULL;
  for (size_t i = node_index_end; i > 0; --i) {
    SpvReflectPrvNode* p_elem = &(p_parser->nodes[i - 1]);
    if (p_elem->result_id == result_id) {
      p_node = p_elem;
      break;
    }
...
static SpvReflectPrvNode* FetchOpVariableFromOpLoad(SpvReflectPrvParser* p_parser, uint32_t node_index_end, uint32_t loadop_id, SpvOp variable_type)
{
  SpvReflectPrvNode* p_node_opload = FindPreviousNode(p_parser, loadop_id, node_index_end);
  if (p_node_opload == NULL || p_node_opload->op != SpvOpLoad) {
    return NULL;
  }

  SpvReflectPrvNode* p_node_result_type = FindNode(p_parser, p_node_opload->result_type_id);
  if (p_node_result_type == NULL || p_node_result_type->op != variable_type) {
    return NULL;
  }

  SpvReflectPrvNode* p_node_result = FindNode(p_parser, p_node_opload->pointer_id);
...
// UE Change Begin
      case SpvOpSampledImage:
      {
		...
		SpvReflectPrvNode* p_image_var = FetchOpVariableFromOpLoad(p_parser, node_index, p_sampled_image->image, SpvOpTypeImage);
        if (p_image_var != NULL) {
		...
		SpvReflectPrvNode* p_sampler_var = FetchOpVariableFromOpLoad(p_parser, node_index, p_sampled_image->sampler, SpvOpTypeSampler);
...
static SpvReflectResult ParseSamplerImageUsage(SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module)
{
...
// UE Change Begin
  if (result == SPV_REFLECT_RESULT_SUCCESS && (flags & SPV_REFLECT_MODULE_FLAG_SAMPLER_IMAGE_USAGE) != 0) {
...
)
{
	...
	for (size_t i = 0; i < parser.node_count; ++i) {
		SpvReflectPrvNode* p_node = &(parser.nodes[i]);
		if (p_node->op == SpvOpName && p_node->target_id == p_entry_point->id) {
			++num_opnames;
		}
	}

	// We will not call ParseString/ParseFunctions, so remove the counts to make sure we don't touch null pointers
	...
	if (new_name_word_count != old_name_word_count) {
		...
		SpvReflectModuleFlags reflect_flags = SPV_REFLECT_MODULE_FLAG_NONE;
		if (p_module->_internal->binding_association_count > 0) {
			reflect_flags |= SPV_REFLECT_MODULE_FLAG_SAMPLER_IMAGE_USAGE;
		}

		// Replace old module
		DestroyParser(&parser);
		spvReflectDestroyShaderModule(p_module);
		result = spvReflectCreateShaderModule2(reflect_flags, new_spirv_size, new_spirv, p_module);
