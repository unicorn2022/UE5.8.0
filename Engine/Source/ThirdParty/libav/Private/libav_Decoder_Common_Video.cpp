...
ILibavDecoder::EDecoderError FLibavDecoderVideoCommon::Initialize(int64 InCodecID, ILibavDecoderVideoResourceAllocator* InVideoResourceAllocator, const TMap<FString, FVariant>& InOptions)
{
	...
	if (bUseHW)
	{
		...
		for(auto& Pri : HWPriorities)
		{
			...
			{
				HWDevType = DevType;
				UE_LOGF(LogLibAV, VeryVerbose, "Preferred HW accelerator \"%ls\" found", *Pri);
				break;
			}
			else
			{
				UE_LOGF(LogLibAV, VeryVerbose, "Preferred HW accelerator \"%ls\" not supported", *Pri);
			}
		}
	...
	if (Codec)
	{
		...
		if (HWDevType != AV_HWDEVICE_TYPE_NONE)
		{
			...
			for(int32 i=0; ; ++i)
			{
				...
				if (!HWConfig)
				{
					UE_LOGF(LogLibAV, Warning, "Decoder %ls does not support device type %ls", *FString(Codec->name), *FString(av_hwdevice_get_type_name(HWDevType)));
					break;
				}
		...
		if (Context)
		{
			if (HWPixelFormat != AV_PIX_FMT_NONE)
			{
				...
				else
				{
					...
					UE_LOGF(LogLibAV, Error, "Could not create %ls hardware context for decoder %ls", *FString(Codec->name), *FString(av_hwdevice_get_type_name(HWDevType)));
...
ILibavDecoder::EDecoderError FLibavDecoderVideoCommon::DecodeAccessUnit(const ILibavDecoderVideoCommon::FInputAU& InInputAccessUnit)
{
	...
	AVBufferRef* OpaqueRef = av_buffer_alloc(sizeof(int64));
	check(OpaqueRef);
	FMemory::Memcpy(OpaqueRef->data, &InInputAccessUnit.UserValue, sizeof(int64));
	Packet->opaque_ref = OpaqueRef;
#else
...
#ifdef AV_CODEC_FLAG_COPY_OPAQUE
			CurrentOutputInfo.UserValue = 0;
			if (Frame->opaque_ref)
			{
				FMemory::Memcpy(&CurrentOutputInfo.UserValue, Frame->opaque_ref->data, sizeof(int64));
			}
#else
			...
			if (HWPixelFormat != AV_PIX_FMT_NONE && Frame->format == HWPixelFormat)
			{
				...
				if (av_hwframe_transfer_get_formats(Frame->hw_frames_ctx, AV_HWFRAME_TRANSFER_DIRECTION_FROM, &TargetPixelFmts, 0) == 0)
				{
					for(int32 i=0; TargetPixelFmts[i] != AV_PIX_FMT_NONE; ++i)
					{
						...
						//UE_LOGF(LogLibAV, Log, "xfer %ls", *FString(av_get_pix_fmt_name(pf)));
					}
				}
				...
				{
					UE_LOGF(LogLibAV, Warning, "Hardware decoder output cannot be provided in one of the preferred formats, skipping.");
