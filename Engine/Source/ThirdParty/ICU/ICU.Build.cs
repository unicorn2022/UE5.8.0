...
public class ICU : ModuleRules
{
	...
	protected const string ICU78VersionString = "icu4c-78_1";
	...
	protected virtual string ICULibPath
	{
		get
		{
			switch (ICUVersion)
			{
				...
				case ICU78VersionString: return Path.Combine(PlatformModuleDirectory, ICUVersion, "lib", PlatformName ?? ".");
	...
	public ICU(ReadOnlyTargetRules Target) : base(Target)
	{
		...
		// @TODOICU: Once all platforms are on version 78, replace these defines with ICU native version macros
		if (ICUVersion == ICU64VersionString)
		{
			PublicDefinitions.Add("WITH_ICU_V64=1");
		}
		else if (ICUVersion == ICU78VersionString)
		{
			PublicDefinitions.Add("WITH_ICU_V78=1");
		}
		...
		{
			string LibraryPath;
			if (ICUVersion == ICU78VersionString)
			{
				...
				{
					LibraryPath = Path.Combine(ICULibPath, Target.Architecture.WindowsLibDir);
				}
			}
			else
			{
				LibraryPath = Path.Combine(ICULibPath, "VS" + Target.WindowsPlatform.GetVisualStudioCompilerVersionName());
		...
		else if (Target.IsInPlatformGroup(UnrealPlatformGroup.Unix))
		{
			if (ICUVersion == ICU78VersionString)
			{
				// ICU 78 layout: <linuxname>/<Release|Debug>/libicu.a
				PublicAdditionalLibraries.Add(Path.Combine(ICULibPath, Target.Architecture.LinuxName, UseDebugLibs ? "Debug" : "Release", "libicu.a"));
			}
			else
			{
