...
void UnicodeTest::TestBinaryCharacterProperties() {
    ...
    for (int32_t prop = 0; prop < UCHAR_BINARY_LIMIT; ++prop) {
        const USet* uset = u_getBinaryPropertySet(static_cast<UCharProperty>(prop), errorCode);
        ...
        if (count == 0) {
            assertFalse(UnicodeString("!hasBinaryProperty(U+0020, ") + prop + u")",
                u_hasBinaryProperty(0x20, static_cast<UCharProperty>(prop)));
            assertFalse(UnicodeString("!hasBinaryProperty(U+0061, ") + prop + u")",
                u_hasBinaryProperty(0x61, static_cast<UCharProperty>(prop)));
            assertFalse(UnicodeString("!hasBinaryProperty(U+4E00, ") + prop + u")",
                u_hasBinaryProperty(0x4e00, static_cast<UCharProperty>(prop)));
        } else {
            ...
            if (c > 0) {
                assertFalse(
                    UnicodeString("!hasBinaryProperty(") + TestUtility::hex(c - 1) +
                        u", " + prop + u")",
                    u_hasBinaryProperty(c - 1, static_cast<UCharProperty>(prop)));
            }
            assertTrue(
                UnicodeString("hasBinaryProperty(") + TestUtility::hex(c) +
                    u", " + prop + u")",
                u_hasBinaryProperty(c, static_cast<UCharProperty>(prop)));
...
void UnicodeTest::TestIntCharacterProperties() {
    ...
    for (int32_t prop = UCHAR_INT_START; prop < UCHAR_INT_LIMIT; ++prop) {
        const UCPMap* map = u_getIntPropertyMap(static_cast<UCharProperty>(prop), errorCode);
        ...
        assertEquals(UnicodeString("int property first range value at ") + TestUtility::hex(c),
            u_getIntPropertyValue(c, static_cast<UCharProperty>(prop)), value);
        ...
        assertEquals(UnicodeString("int property later range value at ") + TestUtility::hex(end),
            u_getIntPropertyValue(end, static_cast<UCharProperty>(prop)), value);
        // ucpmap_get() API coverage
        // TODO: move to cucdtst.c
        assertEquals(
            "int property upcmap_get(U+0061)",
            u_getIntPropertyValue(0x61, static_cast<UCharProperty>(prop)), ucpmap_get(map, 0x61));
    }
#endif
}
...
const char *getPropName(UCharProperty property, int32_t nameChoice) UPRV_NO_SANITIZE_UNDEFINED {
    const char* name = u_getPropertyName(property, static_cast<UPropertyNameChoice>(nameChoice));
    return name != nullptr ? name : "null";
}

const char *getValueName(UCharProperty property, int32_t value, int32_t nameChoice)
...
void UnicodeTest::TestPropertyNames() {
    ...
    UCharProperty prop = UCHAR_WHITE_SPACE;
...
public:
    ...
    int32_t getPropertyValueEnum(int32_t property, const char *name) const override {
        return u_getPropertyValueEnum(static_cast<UCharProperty>(property), name);
    }
};
...
void UnicodeTest::TestPropertiesUsingPpucd() {
    ...
    struct TestProp {
        const UCharProperty prop;
        const int32_t value = 1;  // binary "Yes"
        UnicodeSet set;

        TestProp(UCharProperty binaryProp) : prop(binaryProp) {}
        TestProp(UCharProperty intProp, int32_t v) : prop(intProp), value(v) {}
