...
/* Function pointers for user-supplied data file open/close callbacks. */
static const void      *gDataFileContext = nullptr;
static UDataFileOpenFn *gDataFileOpenFn  = nullptr;
static UDataFileCloseFn *gDataFileCloseFn = nullptr;

U_CAPI void U_EXPORT2
u_setDataFileFunctions(const void *context, UDataFileOpenFn *openFn, UDataFileCloseFn *closeFn, UErrorCode *status) {
    if (U_FAILURE(*status)) {
        return;
    }

    /* Both callbacks must be provided, or both must be nullptr. */
    if ((openFn == nullptr) != (closeFn == nullptr)) {
        *status = U_ILLEGAL_ARGUMENT_ERROR;
        return;
    }

    gDataFileContext  = context;
    gDataFileOpenFn   = openFn;
    gDataFileCloseFn  = closeFn;
}
...
uprv_mapFileFromCallback(UDataMemory *pData, const char *path, UErrorCode *status) {
    ...
    if (pData == nullptr || gDataFileOpenFn == nullptr) {
        return false;
    }
    UDataMemory_init(pData); /* Clear the output struct. */
    int32_t length = -1;
    UBool result = gDataFileOpenFn(gDataFileContext, (void**)&pData->map, (void**)&pData->pHeader, path, &length);
    if (result) {
        pData->length = length;
    }
    return result;
...
uprv_unmapFileFromCallback(UDataMemory *pData) {
    if (pData == nullptr || gDataFileCloseFn == nullptr) {
        return;
    }
    gDataFileCloseFn(gDataFileContext, (void*)pData->map, (void*)pData->pHeader);
    pData->map = nullptr;
