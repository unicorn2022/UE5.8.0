...
UBool
EmojiProps::hasBinaryProperty(UChar32 c, UCharProperty which) {
...
UBool
EmojiProps::hasBinaryPropertyImpl(UChar32 c, UCharProperty which) const {
...
UBool
EmojiProps::hasBinaryProperty(const char16_t *s, int32_t length, UCharProperty which) {
    UErrorCode errorCode = U_ZERO_ERROR;
    const EmojiProps *ep = getSingleton(errorCode);
...
UBool
EmojiProps::hasBinaryPropertyImpl(const char16_t *s, int32_t length, UCharProperty which) const {
    ...
    UCharProperty firstProp = which, lastProp = which;
...
void
EmojiProps::addStrings(const USetAdder *sa, UCharProperty which, UErrorCode &errorCode) const {
    if (U_FAILURE(errorCode)) { return; }
    if (which < UCHAR_BASIC_EMOJI || UCHAR_RGI_EMOJI < which) {
        return;
    }
