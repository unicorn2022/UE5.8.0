...
do
  "${ENGINE_ROOT}/Build/BatchFiles/RunUAT.command" BuildCMakeLib -TargetPlatform=IOS -TargetArchitecture=${ARCH} -TargetLib=ICU -TargetLibVersion=${ICU_IOS_VERSION} -TargetConfigs=Debug+Release -LibOutputPath=lib -CMakeGenerator=Makefile -CMakeAdditionalArguments="${CMAKE_ADDITIONAL_ARGUMENTS}" -MakeTarget=${MAKE_TARGET} -SkipCreateChangelist
done

pushd "${ENGINE_ROOT}/Source/ThirdParty/ICU/${ICU_IOS_VERSION}/lib/IOS"
...
lipo -create iossimulator/Debug/libicu.a x86_64/Debug/libicu.a -output Simulator/Debug/libicu.a
lipo -create iossimulator/Release/libicu.a x86_64/Release/libicu.a -output Simulator/Release/libicu.a
