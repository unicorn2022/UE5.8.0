...
{
	UE_LOGF(LogInterchangeWorker, Verbose, "connect to %d...", ServerPort);
	bool bConnected = NetworkInterface.Connect(TEXT("Interchange Worker"), ServerPort, Config::ConnectTimeout_s);
	UE_LOGF(LogInterchangeWorker, Verbose, "connected to %d %ls", ServerPort, bConnected ? TEXT("OK") : TEXT("FAIL"));
	if (bConnected)
	{
	...
	{
		UE_LOGF(LogInterchangeWorker, Error, "Server connection failure. exit");
		return false;
	}
	...
	if (bVersionError)
	{
		...
		while (TimeOut > 0)
		{
			if (TSharedPtr<ICommand> Command = CommandIO.GetNextCommand(0.02))
			{
				...
				{
					UE_LOGF(LogInterchangeWorker, Verbose, "Terminate command received. Exiting.");
					break;
				}
			}
			TimeOut -= 0.02;
	...
	while (bIsRunning)
	{
		...
		else
		{
			if (bIsRunning)
			{
				...
				UE_CLOGF(!bIsRunning, LogInterchangeWorker, Error, "Worker failure: server lost");
			}
		}
	...
	UE_CLOGF(!bIsRunning, LogInterchangeWorker, Verbose, "Worker loop exit...");
...
void FInterchangeWorkerImpl::ProcessCommand(const FBackPingCommand& BackPingCommand)
{
	if (PingStartCycle)
	{
		...
		UE_LOGF(LogInterchangeWorker, Verbose, "Ping %f s", ElapsedTime_s);
	}
	PingStartCycle = 0;
}
...
void FInterchangeWorkerImpl::ProcessCommand(const TSharedPtr<UE::Interchange::ICommand> Command, const FString& ThreadName)
{
	if (!Command.IsValid())
	{
		UE_LOGF(LogInterchangeWorker, Error, "Process command error: The run task command is invalid");
		return;
	}
	...
	UE_LOGF(LogInterchangeWorker, Verbose, "Process %ls", *JsonToProcess);
	...
	UE_LOGF(LogInterchangeWorker, Verbose, "CompletedTask.Result : %d\n %ls", (int32)ProcessResult , *CompletedTask.JSonResult);
	int32 Index = 0;
	for (const FString& Message : CompletedTask.JSonMessages)
	{
		UE_LOGF(LogInterchangeWorker, Verbose, "CompletedTask.Message #%d : %ls", ++Index, *Message);
	}
	CommandIO.SendCommand(CompletedTask, Config::SendCommandTimeout_s);
	UE_LOGF(LogInterchangeWorker, Verbose, "End of Process %ls", *JsonToProcess);
...
ETaskState FInterchangeWorkerImpl::LoadFbxFile(const FJsonFBXLoadSourceCmd& LoadSourceCommand, FString& OutJSonResult, TArray<FString>& OutJSonMessages)
{
	...
	UE_LOGF(LogInterchangeWorker, Verbose, "LoadFbxFile : %ls", *SourceFilename);
	FbxParser.Reset();
	FbxParser.SetConvertSettings(LoadSourceCommand.GetDoesConvertScene(), LoadSourceCommand.GetDoesForceFrontXAxis(), LoadSourceCommand.GetDoesConvertSceneUnit(), LoadSourceCommand.GetDoesKeepFbxNamespace(), LoadSourceCommand.GetDoesUseFbxClustersForBindPoseMatrices());
	...
	UE_LOGF(LogInterchangeWorker, Verbose, "ResultParser : %ls", *OutJSonResult);
