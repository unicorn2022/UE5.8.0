...
bool FRecoveryService::CollectFiles(const FString& DestDir, bool bMetaDataOnly, bool bAnonymizeMetaData)
{
	...
	{
		UE_LOGF(CrashReportClientLog, Error, "Failed to collect recovery session file(s). %ls", Reason);
	};
...
bool FRecoveryService::Startup()
{
	...
	{
		UE_LOGF(CrashReportClientLog, Error, "MessageBus is not enabled in this configuration. Recovery service will be disabled!");
		return false;
	}

	if (!IConcertSyncServerModule::IsAvailable())
	{
		UE_LOGF(CrashReportClientLog, Error, "ConcertSyncServer Module is missing. Recovery service will be disabled!");
		return false;
	}
	...
	if (!Plugin || !Plugin->IsEnabled())
	{
		...
		UE_LOGF(CrashReportClientLog, Error, "The 'UDP Messaging' plugin is disabled. The Concert server only supports UDP protocol. Recovery service will be disabled!");
		return false;
	}
	...
	UE_LOGF(CrashReportClientLog, Display, "%ls Initialized (Name: %ls, Version: %d.%d, Role: %ls)", RecoveryServiceName, *Server->GetConcertServer()->GetServerInfo().ServerName, ENGINE_MAJOR_VERSION, ENGINE_MINOR_VERSION, *Server->GetConcertServer()->GetRole());
	return true;
}
...
void FRecoveryService::Shutdown()
{
	if (Server)
	{
		Server->Shutdown();
		Server.Reset();
		UE_LOGF(CrashReportClientLog, Display, "%ls Shutdown", RecoveryServiceName);
	}
}
