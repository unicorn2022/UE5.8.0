...
void FTextSerializer::WriteValueFloatAuto(float Value)
{
	...
	else if (Value == std::numeric_limits<float>::denorm_min())
	{
		Append("DENORM_MIN_flt");
	}
...
void FTextSerializer::WriteValueDoubleAuto(double Value)
{
	...
	else if (Value == std::numeric_limits<double>::denorm_min())
	{
		Append("DENORM_MIN_dbl");
	}
...
{
#if UE_TRACE_ANALYSIS_DEBUG && (UE_TRACE_ANALYSIS_DEBUG_LOG_IMPL <= 2)
	...
	: Handle(InHandle)
{
	...
	FormatBuffer = FMemory::Malloc(FormatBufferSize);
	check(FormatBuffer);
}
