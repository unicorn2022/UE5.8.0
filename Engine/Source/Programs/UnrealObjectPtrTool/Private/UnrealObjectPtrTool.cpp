...
bool FPointerUpgrader::TryUpgradesInFile(const FString& FilenameToUpgrade, TConstArrayView<FMemberDeclaration> MemberDeclarations, FPendingWrites& PendingWrites)
{
	...
	{
		UE_LOGF(LogUnrealObjectPtrTool, Error, "Unable to load file to upgrade: %ls", *FilenameToUpgrade);
		return true;
	}
	...
	for (const FMemberDeclaration& MemberDeclaration : MemberDeclarations)
	{
		...
		{
			UE_LOGF(LogUnrealObjectPtrTool, Error, "Member declaration line '%d' is out of range for the lines (%d) in file '%ls' ", MemberDeclaration.LineNumber, FileContentsToUpgrade.Num(), *FilenameToUpgrade);
			continue;
		}
		...
		if (LinesToUpgrade.ReplaceInline(*MemberDeclaration.TypeName, *UpgradedTypeName) < 1)
		{
			...
			{
				UE_LOGF(LogUnrealObjectPtrTool, Warning, "%ls(%d): Type name string '%ls' not found for upgrading, skipping upgrading this line:\n\t%ls", *FilenameToUpgrade, MemberDeclaration.LineNumber, *MemberDeclaration.TypeName, *LinesToUpgrade.TrimStartAndEnd());
			}
			continue;
		}
		...
		UE_LOGF(LogUnrealObjectPtrTool, Display, "%ls(%d): %ls\n\tFrom:\t%ls\n\tTo:\t%ls", *FilenameToUpgrade, MemberDeclaration.LineNumber, EnumHasAllFlags(BehaviorFlags, EPointerUpgradeBehaviorFlags::PreviewOnly) ? TEXT("Previewing upgrade") : TEXT("Upgrading"), *OriginalLinesToUpgrade.TrimStartAndEnd(), *LinesToUpgrade.TrimStartAndEnd());
	}
...
bool FPointerUpgrader::TryFlushPendingWrites(FPendingWrites& PendingWrites)
{
	...
	if (!SCCCommand.IsEmpty())
	{
		...
		{
			UE_LOGF(LogUnrealObjectPtrTool, Display, "Would perform SCC command: %ls", *FileSCCCommand);
		}
		else
		{
			UE_LOGF(LogUnrealObjectPtrTool, Display, "Perform SCC command: %ls", *FileSCCCommand);
		}
		...
		if (!EnumHasAllFlags(BehaviorFlags, EPointerUpgradeBehaviorFlags::PreviewOnly))
		{
			...
			{
				UE_LOGF(LogUnrealObjectPtrTool, Error, "Failed to perform SCC command: %ls", *FileSCCCommand);
				return false;
			}
			...
			if (RetCode != 0)
			{
				UE_LOGF(LogUnrealObjectPtrTool, Error, "SCC command returned an unexpected return code: %d", RetCode);
				return false;
			}
		}
	}
	...
	for (const FPendingWrites::ElementType& PendingWrite : PendingWrites)
	{
		if (EnumHasAllFlags(BehaviorFlags, EPointerUpgradeBehaviorFlags::PreviewOnly))
		{
			UE_LOGF(LogUnrealObjectPtrTool, Display, "File would be modified and saved: %ls", *PendingWrite.Key);
		}
		else
		{
			UE_LOGF(LogUnrealObjectPtrTool, Display, "Saving modified file: %ls", *PendingWrite.Key);
			if (!SaveStringArrayWithNewlinesToFile(PendingWrite.Value.Contents, PendingWrite.Value.BOM, *PendingWrite.Key))
			{
				UE_LOGF(LogUnrealObjectPtrTool, Error, "Failed to save upgraded file: %ls", *PendingWrite.Key);
				return false;
			}
		}
	}
...
INT32_MAIN_INT32_ARGC_TCHAR_ARGV()
{
	...
	{
		UE_LOGF(LogUnrealObjectPtrTool, Error, "Upgrade log parsing error: %ls", *UpgradeListResult.GetError());
		return 1;
	}
	...
	if (EnumHasAllFlags(BehaviorFlags, EPointerUpgradeBehaviorFlags::PreviewOnly))
	{
		UE_LOGF(LogUnrealObjectPtrTool, Display, "Previewed upgrade successfully.  %d upgrades found; %d upgrades would be performed", PointerUpgrader.GetTotalUpgradesFound(), PointerUpgrader.GetTotalUpgradesPerformed());
	}
	else
	{
		UE_LOGF(LogUnrealObjectPtrTool, Display, "Upgraded successfully.  %d upgrades found; %d upgrades performed", PointerUpgrader.GetTotalUpgradesFound(), PointerUpgrader.GetTotalUpgradesPerformed());
	}
