...
{
	FHttpConnection(const std::string_view InHostAddress, uint16 InPort, uint32 InRecvTimeoutSeconds) : FHttpConnection(InHostAddress, InPort, ETlsRequirement::None, FTlsClientSettings{}, InRecvTimeoutSeconds) {}
	FHttpConnection(const std::string_view InHostAddress, uint16 InPort, ETlsRequirement InTlsRequirement, const FTlsClientSettings& InTlsSettings, uint32 InRecvTimeoutSeconds);
	FHttpConnection(const FHttpConnection& Other);

	[[nodiscard]] static FHttpConnection CreateDefaultHttp(const std::string_view InHostAddress, uint16 Port = 80, uint32 RecvTimeoutSeconds = 0);
	[[nodiscard]] static FHttpConnection CreateDefaultHttps(const std::string_view InHostAddress, uint16 Port = 443, uint32 RecvTimeoutSeconds = 0);
