...
struct FCloneCommand
{
	// Number of bytes preceding the aligned range
	uint64 PreBytes = 0;

	// Number of bytes in the aligned range.
	uint64 CloneBytes = 0;

	// Number of bytes after the aligned range.
	uint64 PostBytes = 0;

	uint64 SourcePreBytesOffset = 0;
	uint64 DestPreBytesOffset = 0;

	uint64 SourceCloneOffset = 0;
	uint64 DestCloneOffset = 0;

	uint64 SourcePostBytesOffset = 0;
	uint64 DestPostBytesOffset = 0;

	static std::optional<FCloneCommand> TryCreate(const FCopyCommand& CopyCommand, uint64 AlignBytes);
};

// Construct an FCloneCommand for the given alignment, or nullopt if the clone cannot be aligned.
std::optional<FCloneCommand> FCloneCommand::TryCreate(const FCopyCommand& CopyCommand, uint64 AlignBytes)
{
	if (CopyCommand.SourceOffset % AlignBytes != CopyCommand.TargetOffset % AlignBytes)
	{
		return std::nullopt;
	}

	const uint64 PreBytes = (AlignBytes - (CopyCommand.SourceOffset % AlignBytes)) % AlignBytes;
	const uint64 PostBytes = (CopyCommand.SourceOffset + CopyCommand.Size) % AlignBytes;

	// Check for writes that are too small to clone any blocks.
	if (PreBytes >= CopyCommand.Size || PostBytes >= CopyCommand.Size || CopyCommand.Size < PreBytes + PostBytes + AlignBytes)
	{
		return std::nullopt;
	}

	const uint64 CloneBytes = (CopyCommand.Size - PreBytes - PostBytes);

	UNSYNC_ASSERT(CloneBytes % AlignBytes == 0);
	UNSYNC_ASSERT(PreBytes + CloneBytes + PostBytes == CopyCommand.Size);

	FCloneCommand CloneCommand{
		.PreBytes = PreBytes,
		.CloneBytes = CloneBytes,
		.PostBytes = PostBytes,

		.SourcePreBytesOffset = CopyCommand.SourceOffset,
		.DestPreBytesOffset = CopyCommand.TargetOffset,

		.SourceCloneOffset = CopyCommand.SourceOffset + PreBytes,
		.DestCloneOffset = CopyCommand.TargetOffset + PreBytes,

		.SourcePostBytesOffset = CopyCommand.SourceOffset + PreBytes + CloneBytes,
		.DestPostBytesOffset = CopyCommand.TargetOffset + PreBytes + CloneBytes,
	};

	UNSYNC_ASSERT(CloneCommand.SourceCloneOffset % AlignBytes == 0);
	UNSYNC_ASSERT(CloneCommand.DestCloneOffset % AlignBytes == 0);
	UNSYNC_ASSERT(CloneCommand.SourcePostBytesOffset % AlignBytes == 0);
	UNSYNC_ASSERT(CloneCommand.DestPostBytesOffset % AlignBytes == 0);
...
int64
FIOWriter::WriteFrom(FIOReader& Reader, const FCopyCommand& CopyCommand, FBufferView FileData, uint64* NumBytesClonedPtr)
{
	if (FileData.Size != CopyCommand.Size)
	{
		return -1;
	}

	return Write(FileData.Data, CopyCommand.TargetOffset, FileData.Size);
}
...
bool
CloneFileRange(FWindowsFile& SourceFile, FWindowsFile& DestFile, const FCopyCommand& CopyCommand, FBufferView FileData, uint64* ReflinkedBytesPtr)
{
#if true
	// Disabled due to reports of slowness on NTFS. UE-354599
	return false;
#else // false
	// ReFS has a number of preconditions for block cloning. See
	// https://learn.microsoft.com/en-us/windows-server/storage/refs/block-cloning#functionality-restrictions-and-remarks

	// Block clones must be aligned on cluster boundaries, typically 4 kiB.
	// TODO: Query the filesystem cluster size, since ReFS also supports 64 kiB clusters.
	constexpr uint64 AlignBytes = 4096;
	std::optional<FCloneCommand> OptCloneCommand = FCloneCommand::TryCreate(CopyCommand, AlignBytes);
	if (!OptCloneCommand)
	{
		return false;
	}
	FCloneCommand& CloneCommand = *OptCloneCommand;

	// Check whether the files are on the same filesystem, and the filesystem supports block cloning.
	{
		DWORD SourceVolumeSerialNumber = 0;
		DWORD DestVolumeSerialNumber   = 0;
		DWORD DestVolumeFlags		   = 0;

		if (GetVolumeInformationByHandleW(SourceFile.FileHandle,
										  nullptr,
										  0,
										  &SourceVolumeSerialNumber,
										  nullptr,
										  nullptr,
										  nullptr,
										  0) != TRUE ||
			GetVolumeInformationByHandleW(DestFile.FileHandle,
										  nullptr,
										  0,
										  &DestVolumeSerialNumber,
										  nullptr,
										  &DestVolumeFlags,
										  nullptr,
										  0) != TRUE)
		...
		if (SourceVolumeSerialNumber != DestVolumeSerialNumber || (DestVolumeFlags & FILE_SUPPORTS_BLOCK_REFCOUNTING) == 0)
		{
			return false;
		}
	}

	// Ensure target file is sparse if input file is.
	{
		FILE_BASIC_INFO SourceFileInfo{};
		if (GetFileInformationByHandleEx(SourceFile.FileHandle, FileBasicInfo, &SourceFileInfo, sizeof(SourceFileInfo)))
		{
			FILE_BASIC_INFO DestFileInfo{};
			if (GetFileInformationByHandleEx(DestFile.FileHandle, FileBasicInfo, &DestFileInfo, sizeof(DestFileInfo)))
			{
				if ((SourceFileInfo.FileAttributes & FILE_ATTRIBUTE_SPARSE_FILE) !=
					(DestFileInfo.FileAttributes & FILE_ATTRIBUTE_SPARSE_FILE))
				{
					FILE_SET_SPARSE_BUFFER Sparse{};
					Sparse.SetSparse = (SourceFileInfo.FileAttributes & FILE_ATTRIBUTE_SPARSE_FILE) ? TRUE : FALSE;
					DWORD BytesReturned{};
					if (!DeviceIoControl(DestFile.FileHandle,
										 FSCTL_SET_SPARSE,
										 &Sparse,
										 sizeof(Sparse),
										 nullptr,
										 ...
										 nullptr))
					{
						UNSYNC_VERBOSE("Unable to set target file sparseness to match source file: %s",
	...
	// Ensure the output file is big enough
	{
		LARGE_INTEGER ExistingFileSize{};
		if (!GetFileSizeEx(DestFile.FileHandle, &ExistingFileSize))
		{
			return false;
		}

		const int64 NewMinSize = CopyCommand.TargetOffset + CopyCommand.Size;
		if (ExistingFileSize.QuadPart < NewMinSize)
		{
			FILE_END_OF_FILE_INFO EOFInfo{};
			EOFInfo.EndOfFile.QuadPart = NewMinSize;
			if (!SetFileInformationByHandle(DestFile.FileHandle, FileEndOfFileInfo, &EOFInfo, sizeof(EOFInfo)))
			{
				return false;
			}
		}
	...
	Extent.FileHandle				 = SourceFile.FileHandle;
	Extent.SourceFileOffset.QuadPart = CloneCommand.SourceCloneOffset;
	Extent.TargetFileOffset.QuadPart = CloneCommand.DestCloneOffset;
	Extent.ByteCount.QuadPart		 = CloneCommand.CloneBytes;
	DWORD	BytesReturned{};
	if (DeviceIoControl(DestFile.FileHandle, FSCTL_DUPLICATE_EXTENTS_TO_FILE, &Extent, sizeof(Extent), NULL, 0, &BytesReturned, NULL) != TRUE)
	{
		return false;
	}

	// Write unaligned pre/post data
	if (FileData.Size == CopyCommand.Size)
	{
		// Use file data that was already read into memory.
		if (DestFile.Write(FileData.Data, CloneCommand.DestPreBytesOffset, CloneCommand.PreBytes) != CloneCommand.PreBytes ||
			DestFile.Write(FileData.Data + CloneCommand.PreBytes + CloneCommand.CloneBytes, CloneCommand.DestPostBytesOffset, CloneCommand.PostBytes) != CloneCommand.PostBytes)
		{
			return false;
		}
	}
	...
	{
		// It's preferable for callers to pass in the already-in-memory data via `FileData` rather than re-reading it here.
		uint8 Buffer[AlignBytes];
		if (CloneCommand.PreBytes > 0)
		{
			if (SourceFile.Read(Buffer, CloneCommand.SourcePreBytesOffset, CloneCommand.PreBytes) != CloneCommand.PreBytes ||
				DestFile.Write(Buffer, CloneCommand.DestPreBytesOffset, CloneCommand.PreBytes) != CloneCommand.PreBytes)
			{
				return false;
			}
		}
		if (CloneCommand.PostBytes > 0)
		{
			if (SourceFile.Read(Buffer, CloneCommand.SourcePostBytesOffset, CloneCommand.PostBytes) != CloneCommand.PostBytes ||
				DestFile.Write(Buffer, CloneCommand.DestPostBytesOffset, CloneCommand.PostBytes) != CloneCommand.PostBytes)
			{
				return false;
			}
		}
	...
	if (ReflinkedBytesPtr)
	{
		*ReflinkedBytesPtr += CloneCommand.CloneBytes;
	}
...
FUnixFile::FUnixFile(const FPath& InFilename, EFileMode InMode, uint64 in_size) : Filename(InFilename), Mode(InMode)
{
	FileDescriptor = open(
		...
		IsReadOnly(Mode) ? O_RDONLY : (O_RDWR | O_CREAT | O_TRUNC),
	...
	if (IsReadOnly(Mode))
	{
		...
		if (fstat(FileDescriptor, &stat_buf) == 0)
		{
	...
	{
		// Note: Do not ftruncate() here as it inhibits copy-on-write reflinking.
		// Delayed allocation will minimize fragmentation.
		FileSize = in_size;
	}
}
...
FUnixFile::Close()
{
	if (FileDescriptor != -1)
	{
		...
		{
			LastError = errno;
			// Notify the user as this could mean file contents are not as expected. 
			UNSYNC_WARNING(L"Failed to close %hs: %hs", Filename.string().c_str(), strerror(LastError));
		}
		FileDescriptor = -1;
	}
}
...
bool
CloneFileRange(FUnixFile& SourceFile, FUnixFile& DestFile, const FCopyCommand& CopyCommand, FBufferView FileData, uint64* NumReflinkedBytesPtr)
{
#if UNSYNC_PLATFORM_LINUX
	if (const std::optional<FCloneCommand> OptCloneCommand = FCloneCommand::TryCreate(CopyCommand, 4096))
	{
		const FCloneCommand& CloneCommand = *OptCloneCommand;

		// Most filesystems, including btrfs, will only reflink if the copy is block-aligned. This is also true of the FICLONERANGE ioctl.
		// Unaligned copy_file_range() will succeed, but all file extents are duplicated, not reused.
		// However by copying the aligned range separately, those blocks will be reflinked.

		off_t SourcePreBytesOffset = CloneCommand.SourcePreBytesOffset;
		off_t DestPreBytesOffset = CloneCommand.DestPreBytesOffset;
		off_t SourceCloneOffset = CloneCommand.SourceCloneOffset;
		off_t DestCloneOffset = CloneCommand.DestCloneOffset;
		off_t SourcePostBytesOffset = CloneCommand.SourcePostBytesOffset;
		off_t DestPostBytesOffset = CloneCommand.DestPostBytesOffset;

		ssize_t PreBytesWritten = 0;
		ssize_t CloneBytesWritten = 0;
		ssize_t PostBytesWritten = 0;
		if (
			(PreBytesWritten = copy_file_range(
				SourceFile.FileDescriptor, &SourcePreBytesOffset,
				DestFile.FileDescriptor, &DestPreBytesOffset,
				CloneCommand.PreBytes, 0)) == CloneCommand.PreBytes
			&& 
			(CloneBytesWritten = copy_file_range(
				SourceFile.FileDescriptor, &SourceCloneOffset,
				DestFile.FileDescriptor, &DestCloneOffset,
				CloneCommand.CloneBytes, 0)) == CloneCommand.CloneBytes
			&&
			(PostBytesWritten = copy_file_range(
				SourceFile.FileDescriptor, &SourcePostBytesOffset,
				DestFile.FileDescriptor, &DestPostBytesOffset,
				CloneCommand.PostBytes, 0)) == CloneCommand.PostBytes)
		{
			UNSYNC_ASSERT(PreBytesWritten + CloneBytesWritten + PostBytesWritten == CopyCommand.Size);

			if (NumReflinkedBytesPtr)
			{
				*NumReflinkedBytesPtr += CloneBytesWritten;
			}
	...
	off_t SourceOffset = CopyCommand.SourceOffset;
	off_t DestOffset = CopyCommand.TargetOffset;

	ssize_t Written = copy_file_range(
		SourceFile.FileDescriptor,
		&SourceOffset,
		DestFile.FileDescriptor,
		&DestOffset,
		CopyCommand.Size,
		0);
	...
	if (Written == -1)
	{
		UNSYNC_VERBOSE2("copy_file_range failed: %s", strerror(errno));
		return false;
	}

	return Written == CopyCommand.Size;
#else // !UNSYNC_PLATFORM_LINUX
	return false;
#endif // !UNSYNC_PLATFORM_LINUX
}
