...
// Decodes hex character pairs into bytes. HexString must have exactly NumBytes*2 characters.
// When bReverseOrder is true, bytes are written in reverse order (for big-endian hex into little-endian memory).
template<bool bReverseOrder = false>
inline bool
ParseBytesFromHexString(std::string_view HexString, uint8* OutputBytes, uint64 NumBytes)
{
	...
	for (uint64 Index = 0; Index < NumBytes; ++Index)
	{
		...
		uint64 OutputIndex = bReverseOrder ? (NumBytes - 1 - Index) : Index;
		OutputBytes[OutputIndex] = (H << 4) | L;
	}
...
{
	FGenericHash Result = {};
	Result.Type			= ToHashType(StrongHasher);

	if (!ParseBytesFromHexString(HexString, Result.Data, Result.Size()))
	{
		return false;
	}
...
bool
ParseUint64FromHexString(std::string_view HexString, uint64& Output)
{
	uint64 Result = 0;
	if (!ParseBytesFromHexString</*bReverseOrder*/ true>(HexString, reinterpret_cast<uint8*>(&Result), 8))
	{
		return false;
	}
