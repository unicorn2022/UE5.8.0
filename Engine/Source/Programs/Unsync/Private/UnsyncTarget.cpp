...
BuildTarget(FIOWriter& Output, FIOReader& Source, FIOReader& Base, const FNeedList& NeedList, const FBuildTargetParams& Params)
{
	...
	struct FStats
	{
		...
		// Bytes reused on disk via block cloning (if available).
		// Subset of WrittenBytesFromBase.
		std::atomic<uint64> BytesUsedInSitu		   = {};
	};
	FStats Stats;
	...
	if (ScavengeDatabase)
	{
		...
		Stats.BytesUsedInSitu += ScavengeResult.InSituBytes;
			...
			std::atomic<bool>&			   bCompletionFlag)
	{
		...
		while (!ReadSchedule.Requests.empty() && !Error)
		{
			...
			auto ReadCallback = [&DataProvider, &ReadBytes, &Output, &WriteTasks, &Error, &Stats, Block, ListType](FIOBuffer CmdBuffer,
																												   uint64	  CmdOffset,
																												   uint64	  CmdReadSize,
																												   uint64	  CmdUserData)
			{
				if (CmdReadSize == uint64(-1))
				{
					UNSYNC_ERROR(L"Read failed: %hs", FormatSystemErrorMessage(DataProvider.GetError()).c_str());
					Error.Set(AppError(L"Failed to read blocks"));
					return;
				}

				WriteTasks.run(
					[Buffer = MakeShared(std::move(CmdBuffer)), CmdReadSize, Block, &DataProvider, &Output, &Error, &Stats, ListType]()
					{
						uint64		InSituBytes	 = 0;
						const int64 WrittenBytes = Output.WriteFrom(DataProvider, Block, Buffer->GetBufferView(), &InSituBytes);
						Stats.BytesUsedInSitu += InSituBytes;

						if (WrittenBytes < 0 || uint64(WrittenBytes) != CmdReadSize)
						{
	...
	if (Stats.BytesUsedInSitu > 0)
	{
		UNSYNC_VERBOSE2("%.2f MB reused in situ", SizeMb(Stats.BytesUsedInSitu));
	}
	...
	BuildResult.InSituBytes = Stats.BytesUsedInSitu;
