...
FHttpConnection::FHttpConnection(const std::string_view	   InHostAddress,
								 ...
								 uint32					   InRecvTimeoutSeconds)
: HostAddress(InHostAddress)
, HostPort(InPort)
, TlsRequirement(InTlsRequirement)
, RecvTimeoutSeconds(InRecvTimeoutSeconds)
{
...
, RecvTimeoutSeconds(Other.RecvTimeoutSeconds)
{
}

FHttpConnection
FHttpConnection::CreateDefaultHttp(const std::string_view InHostAddress, uint16 Port, uint32 RecvTimeoutSeconds)
{
	return FHttpConnection(InHostAddress, Port, RecvTimeoutSeconds);
}

FHttpConnection
FHttpConnection::CreateDefaultHttps(const std::string_view InHostAddress, uint16 Port, uint32 RecvTimeoutSeconds)
{
	FTlsClientSettings TlsSettings;
	TlsSettings.Subject = InHostAddress.data();
	return FHttpConnection(InHostAddress, Port, ETlsRequirement::Required, TlsSettings, RecvTimeoutSeconds);
}
...
{
	FTlsClientSettings TlsSettings = RemoteDesc.GetTlsClientSettings();
	return FHttpConnection(RemoteDesc.Host.Address, RemoteDesc.Host.Port, ETlsRequirement::Required, TlsSettings, RemoteDesc.RecvTimeoutSeconds);
}
...
FHttpConnection::Open()
{
	...
	if (TlsRequirement != ETlsRequirement::None)
	{
		...
		SocketSetRecvTimeout(RawSocketHandle, RecvTimeoutSeconds);
