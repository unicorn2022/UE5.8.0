...
FScavengeDatabase::BuildFromFileSyncTasks(const FSyncDirectoryOptions& SyncOptions, TArrayView<FFileSyncTask> AllFileTasks)
{
	...
	UNSYNC_VERBOSE(L"Scanning '%ls' for usable manifests", SyncOptions.ScavengeRoot.wstring().c_str());
							 ...
							 THashSet<FHash128>&			OutScavengedBlocks)
{
	...
	for (const FPossibleSource& PossibleSource : SortedPossibleSources)
	{
		...
		if (LocalSourceFile.IsValid())
		{
			...
			auto ReadCallback = [&Output, &OutScavengedBlocks, &LocalSourceFile, &ProgressLogger, &BuildResult, &bFoundInvalidBlock, StrongHasher](
									FIOBuffer Buffer,
									uint64	  SourceOffset,
									uint64	  ReadSize,
									uint64	  UserData) {
				...
				if (bBlockHashOk)
				{
					Output.WriteFrom(LocalSourceFile, CopyCommand, Buffer.GetBufferView(), &BuildResult.InSituBytes);
