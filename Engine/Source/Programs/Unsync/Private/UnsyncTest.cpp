...
class FTestFile : public FNativeFile
{
public:
	explicit FTestFile(const FPath& Filename, EFileMode Mode = EFileMode::ReadOnly, uint64 InSize = 0)
		: FNativeFile(Filename, Mode, InSize)
	{
#if UNSYNC_PLATFORM_WINDOWS
		if (DWORD FileAttributes = GetFileAttributesW(Filename.c_str()); FileAttributes != INVALID_FILE_ATTRIBUTES)
		{
			FileAttributes |= FILE_ATTRIBUTE_TEMPORARY;
			SetFileAttributesW(Filename.c_str(), FileAttributes);
		}
#endif
	}

	virtual ~FTestFile() override {
		if (!Filename.empty())
		{
			Close();

			std::error_code Error{};
			std::filesystem::remove(Filename.native(), Error);
			if (Error)
			{
				UNSYNC_ERROR("Failed to remove test file %s: %s", Filename.c_str(), Error.message().c_str());
			}
		}
	}
};
...
TestFiles()
{
	...
	// Whether to remove the temp dir after running file tests.
	const bool bRemoveTempDir = !std::filesystem::exists(TempDirPath, ErrorCode);
	...
	std::unique_ptr<uint8[]> TempBuffer = std::make_unique<uint8[]>(TestFileSize);
	...
	// CloneFileRange
	{
		constexpr uint64 CloneFileSize = 8 * 1024 * 1024;
		std::unique_ptr<uint8[]> CloneData = std::make_unique<uint8[]>(CloneFileSize);
		UNSYNC_ASSERT(CloneData);
		FTestFile InputFile(TempDirPath / "input.bin", EFileMode::CreateReadWrite, CloneFileSize);
		UNSYNC_ASSERT(InputFile.IsValid());
		FTestFile OutputFile(TempDirPath / "output.bin", EFileMode::CreateReadWrite, CloneFileSize);
		UNSYNC_ASSERT(OutputFile.IsValid());

		const int Seed = __COUNTER__;
		FillRandomBytes(CloneData.get(), CloneFileSize, Seed);
		uint64 Written = InputFile.Write(CloneData.get(), 0, CloneFileSize);
		UNSYNC_ASSERT(Written == CloneFileSize);

		// Copy the file with the 1st/2nd halves swapped.
		{
			const FCopyCommand Copy1{.Size = CloneFileSize / 2, .SourceOffset = 0, .TargetOffset = CloneFileSize / 2};
			const FCopyCommand Copy2{.Size = CloneFileSize / 2, .SourceOffset = CloneFileSize / 2, .TargetOffset = 0};

			if (CloneFileRange(InputFile, OutputFile, Copy1) && CloneFileRange(InputFile, OutputFile, Copy2))
			{
				UNSYNC_VERBOSE("Successfully cloned file contents");
				memset(CloneData.get(), 0, CloneFileSize);
				OutputFile.Read(CloneData.get(), CloneFileSize / 2, CloneFileSize / 2);
				OutputFile.Read(CloneData.get() + CloneFileSize / 2, 0, CloneFileSize / 2);
				VerifyRandomBytes(CloneData.get(), CloneFileSize, Seed);
			}
			else
			{
				UNSYNC_VERBOSE("Unable to clone file contents—probably an unsupported filesystem.");
			}
		}

		// Non-cluster-aligned clone
		{
			constexpr int	   UnalignedOffset		 = 3;
			constexpr int	   UnalignedCloneSize	 = CloneFileSize / 2 - UnalignedOffset;
			constexpr int	   UnalignedSourceOffset = UnalignedOffset;
			constexpr int	   UnalignedTargetOffset = CloneFileSize / 2 + UnalignedOffset;
			const FCopyCommand CopyUnaligned{.Size		   = UnalignedCloneSize,
											 .SourceOffset = UnalignedSourceOffset,
											 .TargetOffset = UnalignedTargetOffset};
			if (CloneFileRange(InputFile, OutputFile, CopyUnaligned))
			{
				UNSYNC_VERBOSE("Successfully cloned unaligned file contents");
				std::unique_ptr<uint8[]> UnalignedReadData = std::make_unique<uint8[]>(UnalignedCloneSize);
				UNSYNC_ASSERT(UnalignedReadData);
				const uint64 ReadLen = OutputFile.Read(UnalignedReadData.get(), UnalignedTargetOffset, UnalignedCloneSize);
				UNSYNC_ASSERT(ReadLen == UnalignedCloneSize);
				UNSYNC_ASSERT(memcmp(UnalignedReadData.get(), CloneData.get() + UnalignedOffset, UnalignedCloneSize) == 0);
			}
		}

		// Unaligned source/dest. Not supported on ReFS.
		{
			const FCopyCommand UnalignedCopy{.Size = 16384, .SourceOffset = 1, .TargetOffset = 2};
			(void)CloneFileRange(InputFile, OutputFile, UnalignedCopy);
		}

		// Small ranges; too small to block clone on ReFS (with 4 kiB clusters)
		// OK for clones to fail.
		{
			(void)CloneFileRange(InputFile, OutputFile, FCopyCommand{.Size = 100, .SourceOffset = 200, .TargetOffset = 200});
			(void)CloneFileRange(InputFile, OutputFile, FCopyCommand{.Size = 100, .SourceOffset = 4000, .TargetOffset = 4000});
			(void)CloneFileRange(InputFile, OutputFile, FCopyCommand{.Size = 4096, .SourceOffset = 100, .TargetOffset = 100});
			(void)CloneFileRange(InputFile, OutputFile, FCopyCommand{.Size = 5096, .SourceOffset = 100, .TargetOffset = 100});
		}
	}
	...
	if (bRemoveTempDir)
	{
		std::filesystem::remove_all(TempDirPath, ErrorCode);
		if (ErrorCode)
		{
			UNSYNC_LOG("Failed to remove TestFiles temp dir %s: %s", TempDirPath.c_str(), ErrorCode.message().c_str());
		}
	}
}
...
TestBasicHash()
{
	...
	// ParseHashFromHexString
	{
		FGenericHash Parsed;
		UNSYNC_ASSERT(ParseHashFromHexString(EStrongHashAlgorithmID::Blake3_128, "2c4c1fa09b1a3459bc56ac6af6b446c8", Parsed));
		std::string RoundTrip = BytesToHexString(Parsed.Data, Parsed.Size());
		UNSYNC_ASSERT(RoundTrip == "2c4c1fa09b1a3459bc56ac6af6b446c8");

		UNSYNC_ASSERT(ParseHashFromHexString(EStrongHashAlgorithmID::Blake3_160, "2c4c1fa09b1a3459bc56ac6af6b446c89c784cf9", Parsed));
		RoundTrip = BytesToHexString(Parsed.Data, Parsed.Size());
		UNSYNC_ASSERT(RoundTrip == "2c4c1fa09b1a3459bc56ac6af6b446c89c784cf9");

		// Wrong length
		UNSYNC_ASSERT(!ParseHashFromHexString(EStrongHashAlgorithmID::Blake3_128, "2c4c1fa0", Parsed));
		// Invalid hex char
		UNSYNC_ASSERT(!ParseHashFromHexString(EStrongHashAlgorithmID::Blake3_128, "zz4c1fa09b1a3459bc56ac6af6b446c8", Parsed));
	}

	// ParseUint64FromHexString
	{
		uint64 Value = 0;
		UNSYNC_ASSERT(ParseUint64FromHexString("0000000000000000", Value));
		UNSYNC_ASSERT(Value == 0);

		UNSYNC_ASSERT(ParseUint64FromHexString("0000000000000001", Value));
		UNSYNC_ASSERT(Value == 1);

		UNSYNC_ASSERT(ParseUint64FromHexString("ffffffffffffffff", Value));
		UNSYNC_ASSERT(Value == UINT64_MAX);

		UNSYNC_ASSERT(ParseUint64FromHexString("1584c5a558e0bc7f", Value));
		UNSYNC_ASSERT(Value == 0x1584c5a558e0bc7f);

		UNSYNC_ASSERT(ParseUint64FromHexString("ABCDEF0123456789", Value));
		UNSYNC_ASSERT(Value == 0xABCDEF0123456789);

		// Wrong length
		UNSYNC_ASSERT(!ParseUint64FromHexString("1234", Value));
		UNSYNC_ASSERT(!ParseUint64FromHexString("00000000000000001", Value));
		// Invalid hex char
		UNSYNC_ASSERT(!ParseUint64FromHexString("zzzzzzzzzzzzzzzz", Value));
	}
}
