...
InnerMain(int Argc, char** Argv)
{
	...
	auto AddProxyOptions = [&RemoteAddressUtf8, &ProtocolName, &bNoProxySelect, &bNoCompression](CLI::App* App)
	{
		...
		App->add_flag("--no-server-announcements", GNoServerAnnouncements, "Suppress server announcement messages");
	};
	...
	SubTest->add_option("Preset, --preset", PresetUtf8, "Test preset")->default_str(PresetUtf8);
	...
	for (CLI::App* Subcommand : SubCommands)
	{
		...
		Subcommand->add_flag("--stdout", GLogStdOut, "Send all output to stdout (no stderr)");
