...
: FUnsyncBaseProtocolImpl(RemoteDesc, InRequestMap, InFeatures)
{
	...
	if (IsValid() && Features.bAuthentication && RemoteDesc.bAuthenticationRequired)
	{
		if (InAuthDesc)
		{
			...
			TResult<FAuthToken> AuthTokenResult = Authenticate(*InAuthDesc, RemoteDesc.RecvTimeoutSeconds);
...
ProxyQuery::Hello(const FRemoteDesc& RemoteDesc, const FAuthDesc* OptAuthDesc, bool bDisplayAnnouncements)
{
	FTlsClientSettings TlsSettings = RemoteDesc.GetTlsClientSettings();
	FHttpConnection	   Connection(RemoteDesc.Host.Address, RemoteDesc.Host.Port, RemoteDesc.TlsRequirement, TlsSettings, RemoteDesc.RecvTimeoutSeconds);

	return Hello(RemoteDesc.Protocol, Connection, OptAuthDesc, bDisplayAnnouncements);
}

TResult<ProxyQuery::FHelloResponse>
ProxyQuery::Hello(EProtocolFlavor Protocol, FHttpConnection& Connection, const FAuthDesc* OptAuthDesc, bool bDisplayAnnouncements)
{
	TResult<FHelloResponse> Result = [&]() -> TResult<FHelloResponse>
	{
	...
	if (Result.IsOk() && bDisplayAnnouncements)
	{
...
FUnsyncProtocolImpl::QueryHello(FHttpConnection& HttpConnection, const FAuthDesc* OptAuthDesc)
{
	...
	if (OptAuthDesc)
	{
		TResult<FAuthToken> AuthTokenResult = Authenticate(*OptAuthDesc, HttpConnection.RecvTimeoutSeconds);
		if (AuthTokenResult.IsOk())
		{
	...
	if (auto& Field = JsonObject["features"]; Field.is_array())
	{
		...
		for (auto& Elem : Field.array_items())
		{
			if (Elem.is_string())
			{
				...
				else if (Elem.string_value() == "announcements")
				{
					Result.Features.bAnnouncements = true;
				}
			}
		}
	}

	if (auto& Field = JsonObject["announcements"]; Field.is_array())
	{
		for (const auto& Elem : Field.array_items())
		{
			if (!Elem.is_object())
			{
				continue;
			...
			ProxyQuery::FAnnouncement Announcement;

			const auto& IdField = Elem["id"];
			if (!IdField.is_string() || !ParseUint64FromHexString(IdField.string_value(), Announcement.Id))
			{
				continue;
			}

			Announcement.Level	 = Elem["level"].string_value();
			Announcement.Message = Elem["message"].string_value();

			Result.Announcements.push_back(std::move(Announcement));
		}
	}
...
bool GNoServerAnnouncements = false;

void
ProxyQuery::DisplayAnnouncements(const std::vector<ProxyQuery::FAnnouncement>& Announcements)
{
	if (GNoServerAnnouncements || Announcements.empty())
	{
		return;
	}

	static std::unordered_set<uint64> DisplayedIds;

	for (const FAnnouncement& Announcement : Announcements)
	{
		if (!DisplayedIds.insert(Announcement.Id).second)
		{
			continue;
		}

		// Split on newlines so each line gets the appropriate log prefix
		const std::string& Msg = Announcement.Message;
		uint64 Pos = 0;
		while (Pos <= Msg.size())
		{
			uint64 End = Msg.find('\n', Pos);
			...
			if (Announcement.Level == "error")
			{
				UNSYNC_ERROR(L"%hs", Line.c_str());
			}
			else if (Announcement.Level == "warning")
			{
				UNSYNC_WARNING(L"%hs", Line.c_str());
			}
			else
			{
				UNSYNC_LOG(L"%hs", Line.c_str());
			}
...
FUnsyncProtocolImpl::QueryListDirectory(FHttpConnection& Connection, const FAuthDesc* AuthDesc, const std::string& Path)
{
	...
	if (AuthDesc)
	{
		TResult<FAuthToken> AuthToken = Authenticate(*AuthDesc, Connection.RecvTimeoutSeconds);
		if (!AuthToken.IsOk())
		{
						 ...
						 ProxyQuery::FDownloadOutputCallback OutputCallback)
{
	...
	if (AuthDesc)
	{
		TResult<FAuthToken> AuthToken = Authenticate(*AuthDesc, InConnection.RecvTimeoutSeconds);
		if (!AuthToken.IsOk())
		{
...
, bValid(InRemoteDesc.IsValid())
{
	...
	auto CreateHttpConnection = [Remote = RemoteDesc]
	{
		...
		return new FHttpConnection(Remote.Host.Address, Remote.Host.Port, Remote.TlsRequirement, TlsSettings, Remote.RecvTimeoutSeconds);
	};
...
FProxyPool::GetAccessToken()
{
	...
	if (AuthDesc)
	{
		TResult<FAuthToken> AuthTokenResult = Authenticate(*AuthDesc, RemoteDesc.RecvTimeoutSeconds);
