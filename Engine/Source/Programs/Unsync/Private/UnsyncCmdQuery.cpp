...
RunHttpPing(std::string_view Address, uint16 Port)
{
	...
	constexpr uint32 RecvTimeoutSeconds = 4;

	FHttpConnection Connection(Address, Port, TlsRequirement, TlsSettings, RecvTimeoutSeconds);
...
CmdQueryFile(const FCmdQueryOptions& Options)
{
	...
	TResult<FAuthToken> AuthToken = Authenticate(AuthDesc, Options.Remote.RecvTimeoutSeconds);
