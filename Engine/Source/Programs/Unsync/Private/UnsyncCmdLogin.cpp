...
CmdLogin(const FCmdLoginOptions& Options)
{
	...
	TResult<FAuthToken> AuthTokenResult = Authenticate(AuthDesc, Options.Remote.RecvTimeoutSeconds, RefreshThreshold);
	...
	if (AuthTokenResult.IsOk())
	{
		if (!Options.bQuick)
		{
			...
			else if (Options.Remote.Protocol == EProtocolFlavor::Horde)
			{
