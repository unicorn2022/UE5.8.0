...
struct FIOReader : virtual FIOBase
{
	...
	// Get the actual reader, eg. for a deferred reader that has an inner file reader.
	virtual FIOReader& ActualReader() { return *this; }
};
...
struct FIOWriter : virtual FIOBase
{
	...
	// Write file data from Reader. File-backed writers may use block cloning to copy the data by reference.
	// FileData.Size must equal CopyCommand.Size.
	// NumBytesClonedPtr may be null. Otherwise the number of bytes cloned is added to the target variable.
	virtual int64 WriteFrom(FIOReader& Reader, const FCopyCommand& CopyCommand, FBufferView FileData, uint64* NumBytesClonedPtr = nullptr);
};
...
// File reader/writer that uses CRTP to implement block-cloned copies.
template <typename InFileType>
class FFileReaderWriter : public FIOReaderWriter
{
public:
	using FileType = InFileType;

	virtual int64 WriteFrom(FIOReader& Reader, const FCopyCommand& CopyCommand, FBufferView FileData, uint64* NumBytesClonedPtr) override
	{
		FileType* FileReader = dynamic_cast<FileType*>(&Reader.ActualReader());
		if (FileReader && FileReader->IsValid())
		{
			// Attempt block cloning
			FileType* FileWriter = dynamic_cast<FileType*>(this);
			UNSYNC_ASSERT(FileWriter);

			if (CloneFileRange(*FileReader, *FileWriter, CopyCommand, FileData, NumBytesClonedPtr))
			{
				return CopyCommand.Size;
			}
		}
...
struct FWindowsFile : FFileReaderWriter<FWindowsFile>
{
	friend struct FWindowsAsyncFileReader;

	explicit FWindowsFile(const FPath& Filename, EFileMode Mode = EFileMode::ReadOnly, uint64 InSize = 0);
	~FWindowsFile();
...
// Clone file blocks via ref counting if possible.
// FileData optionally supplies an in-memory view of the file contents that's used for writing unaligned bytes at the beginning and end of the cloned range.
bool CloneFileRange(FWindowsFile& SourceFile, FWindowsFile& DestFile, const FCopyCommand& CopyCommand, FBufferView FileData = FBufferView(), uint64* NumBytesClonedPtr = nullptr);
...
// Copy file data using in-kernel copy, which also provides block cloning (reflinking) on filesystems that support it.
// The `FileData` parameter is not used; it's only for API compatiblity with the Windows CloneFileRange() function.
bool CloneFileRange(FUnixFile& SourceFile, FUnixFile& DestFile, const FCopyCommand& CopyCommand, FBufferView FileData = FBufferView(), uint64* NumReflinkedBytesPtr = nullptr);

struct FUnixFile : FFileReaderWriter<FUnixFile>
{
	explicit FUnixFile(const FPath& InFilename, EFileMode InMode = EFileMode::ReadOnly, uint64 InSize = 0);
	~FUnixFile();

	// IOBase
	virtual uint64 GetSize() override { return FileSize; }
	virtual bool   IsValid() override { return FileDescriptor != -1 && LastError == 0; }
	virtual void   Close() override;
	virtual int32  GetError() override { return LastError; }
	...
	friend bool CloneFileRange(FUnixFile& SourceFile, FUnixFile& DestFile, const FCopyCommand& CopyCommand, FBufferView FileData, uint64* NumReflinkedBytesPtr);

private:
	EFileMode Mode			 = EFileMode::None;
...
struct FDeferredOpenReader : FIOReader
{
	...
	virtual FIOReader& ActualReader() override { return *GetOrOpenInner(); }
