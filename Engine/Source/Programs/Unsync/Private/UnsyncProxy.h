...
extern bool GNoServerAnnouncements;

namespace ProxyQuery {

struct FAnnouncement
{
	uint64		Id = 0;	 // 64-bit hash, parsed from hex string in JSON
...
struct FHelloResponse
{
	...
	std::vector<FAnnouncement> Announcements;
...
TResult<FHelloResponse> Hello(const FRemoteDesc& RemoteDesc, const FAuthDesc* OptAuthDesc = nullptr, bool bDisplayAnnouncements = true);
TResult<FHelloResponse> Hello(EProtocolFlavor Protocol, FHttpConnection& Connection, const FAuthDesc* OptAuthDesc = nullptr, bool bDisplayAnnouncements = true);

void DisplayAnnouncements(const std::vector<FAnnouncement>& Announcements);
