...
StringEscape(const std::string_view Input)
{
	...
	for (size_t i = 0; i < Input.length(); i++)
	{
		...
		else if (static_cast<uint8_t>(C) == 0xe2 && i + 2 < Input.length() &&
				 static_cast<uint8_t>(Input[i + 1]) == 0x80 && static_cast<uint8_t>(Input[i + 2]) == 0xa8)
		{
			Result += "\\u2028";
			i += 2;
		}
		else if (static_cast<uint8_t>(C) == 0xe2 && i + 2 < Input.length() &&
				 static_cast<uint8_t>(Input[i + 1]) == 0x80 && static_cast<uint8_t>(Input[i + 2]) == 0xa9)
		{
			Result += "\\u2029";
			i += 2;
		}
