...
TResult<FAuthToken>
AcquireAuthToken(const FAuthDesc& AuthDesc, uint32 RecvTimeoutSeconds, const FOpenIdConfig& OpenIdConfig)
{
	...
	TResult<FRemoteDesc> AuthRemoteDesc = FRemoteDesc::FromUrl(AuthDesc.AuthServer, unsync::EProtocolFlavor::Unknown, RecvTimeoutSeconds);
...
TResult<FAuthToken>
RefreshAuthToken(const FAuthDesc& AuthDesc, uint32 RecvTimeoutSeconds, const FOpenIdConfig& OpenIdConfig, const FAuthToken& PreviousToken)
{
	if (OpenIdConfig.TokenEndpoint.empty())
	{
...
TResult<FAuthToken>
RefreshOrAcquireToken(const FAuthDesc& AuthDesc, uint32 RecvTimeoutSeconds, const FOpenIdConfig& OpenIdConfig, const FAuthToken& PreviousToken)
{
	if (!PreviousToken.Refresh.empty())
	{
		UNSYNC_VERBOSE(L"Refreshing access token");
		TResult<FAuthToken> RefreshResult = RefreshAuthToken(AuthDesc, RecvTimeoutSeconds, OpenIdConfig, PreviousToken);
	...
	return AcquireAuthToken(AuthDesc, RecvTimeoutSeconds, OpenIdConfig);
}
...
TResult<FAuthToken>
Authenticate(const FAuthDesc& AuthDesc, uint32 RecvTimeoutSeconds, int32 RefreshThreshold)
{
	...
	TResult<FOpenIdConfig> OpenIdConfigResult = GetOpenIdConfig(AuthDesc, RecvTimeoutSeconds);
	if (OpenIdConfigResult.IsError())
	{
		return MoveError<FAuthToken>(OpenIdConfigResult);
	}

	TResult<FAuthToken> FreshTokenResult = RefreshOrAcquireToken(AuthDesc, RecvTimeoutSeconds, * OpenIdConfigResult, PreviousToken);
...
GetOpenIdConfig(const FAuthDesc& AuthDesc, uint32 RecvTimeoutSeconds)
{
	if (!AuthDesc.IsValid())
	{
	...
	TResult<FRemoteDesc> AuthServerDescResult = FRemoteDesc::FromUrl(AuthDesc.AuthServer, unsync::EProtocolFlavor::Unknown, RecvTimeoutSeconds);
