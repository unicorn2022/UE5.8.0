...
namespace PerfSummaries
{
	class FPSChartSummary : Summary
	{
		public FPSChartSummary(XElement element, XmlVariableMappings vars, string baseXmlDirectory)
		{
			...
			recommendedFps = element.GetSafeAttribute<int>(vars, "recommendedFps", 0);
			slowFrameTimeBuffer = element.GetSafeAttribute<float>(vars, "slowFrameTimeBuffer", 0);
			...
			bIgnoreSlowFrameTimePercent = element.GetSafeAttribute<bool>(vars, "ignoreSlowFrameTimePercent", false);
		}
		...
		struct FpsChartData
		{
			...
			public float? SlowFrameTimePerecent;
		};
		...
		FpsChartData ComputeFPSChartDataForFrames(List<float> frameTimes, bool skiplastFrame)
		{
			...
			if (!bIgnoreSlowFrameTimePercent && recommendedFps > 0 && fps > 0 && TotalSeconds > 0.0f)
			{
				// If the user is targeting a lower fps than we recommend then use that as the baseline instead.
				// Otherwise we'll incorrectly compute a high SFP due to the frame rate being limited to a lower value.
				float minTargetFps = (float)Math.Min(recommendedFps, fps);
				float frameTimeTargetMs = (1.0f / minTargetFps) * 1000.0f;
				// We allow for some tolerance in determining what is a slow frame to account for natural noise in timings.
				float frameTimeThreshold = frameTimeTargetMs + slowFrameTimeBuffer;

				// Accumulate the total time spent over the frame time target each frame.
				float totalOverBudget = 0.0f;
				for (int i = 0; i < frameCount; i++)
				{
					if (frameTimes[i] > frameTimeThreshold)
					{
						totalOverBudget += frameTimes[i] - frameTimeTargetMs;
					}
				}

				// The final value is the % of time spent over the frame time target.
				float totalOverBudgetSeconds = totalOverBudget / 1000.0f;
				outData.SlowFrameTimePerecent = (totalOverBudgetSeconds / TotalSeconds) * 100.0f; // Scale 0-100
			}
			else
			{
				// Avoid outputting 0 if the required values weren't available as it may be misleading.
				outData.SlowFrameTimePerecent = null;
			}
		...
		public override HtmlSection WriteSummaryData(bool bWriteHtml, CsvStats csvStats, CsvStats csvStatsUnstripped, bool bWriteSummaryCsv, SummaryTableRowData rowData, string htmlFileName)
		{
			...
			if (!bIgnoreSlowFrameTimePercent && fpsChartData.SlowFrameTimePerecent.HasValue)
			{
				Columns.Add(new ColumnInfo("SlowFrameTimePercent", fpsChartData.SlowFrameTimePerecent.Value, GetStatColourThresholdList("SlowFrameTimePercent")));
			}
			...
			if (bWriteHtml)
			{
				htmlSection = new HtmlSection("FPSChart", bStartCollapsed, MakeSectionId("FPSChart"));
				string HeaderRow = "";
				string ValueRow = "";
			...
			if (csvStats.Events.Count > 0)
			{
				...
				foreach (CaptureRange CapRange in captures)
				{
					...
					if (!bIgnoreSlowFrameTimePercent && captureFpsChartData.SlowFrameTimePerecent.HasValue)
					{
						ColumnDict["SlowFrameTimePercent"].Value = captureFpsChartData.SlowFrameTimePerecent.Value;
					}
					...
					if (rowData != null)
                    {
						...
						if (!bIgnoreSlowFrameTimePercent && captureFpsChartData.SlowFrameTimePerecent.HasValue)
						{
							rowData.Add(SummaryTableElement.Type.SummaryTableMetric, CapRange.name + "_SlowFrameTimePercent", captureFpsChartData.SlowFrameTimePerecent.Value, GetStatColourThresholdList("SlowFrameTimePerecent"));
						}
