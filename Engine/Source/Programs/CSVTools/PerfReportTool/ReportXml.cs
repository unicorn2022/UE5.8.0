...
namespace PerfReportTool
{
	...
	class ReportXML
	{
			...
			string summaryTableXmlRowSortAppendStr )
		{
			...
			statDisplayNameMapping = new Dictionary<string, StatDisplayInfo>();
			...
			if (displayNameElement != null)
			{
				foreach (XElement mapping in displayNameElement.Elements("mapping"))
				{
					...
					string toolTip = mapping.GetSafeAttribute<string>("toolTip");

					if (statName != null && (displayName != null || toolTip != null))
					{
						statDisplayNameMapping.Add(statName.ToLower(), new StatDisplayInfo(displayName, toolTip));
					}
				}
			}
		...
		public Dictionary<string, StatDisplayInfo> GetDisplayNameMapping() { return statDisplayNameMapping; }
