...
namespace CSVStats
{
	...
	public class CsvEvent
    {
		...
		public CsvEvent(string NameIn, int FrameIn, double TimestampIn)
        {
            Name = NameIn;
            Frame = FrameIn;
		...
		public bool HasTimestamp() { return Timestamp != double.MaxValue; }

        public string Name;
        public int Frame;
		public double Timestamp = double.MaxValue;
    };
	...
	public class CsvStats
	{
		...
		public Tuple<CsvEvent, CsvEvent> FindEventPair(string patternBegin, string patternEnd, bool bFindLast = false)
		{
			IEnumerable<CsvEvent> eventsOrdered = bFindLast == false ? Events : Events.Reverse<CsvEvent>();
			CsvEvent beginEvent = patternBegin != null? FindEventInternal(eventsOrdered, patternBegin) : null;
			double startTime = (beginEvent != null) ? beginEvent.Timestamp : 0.0;

			CsvEvent endEvent = (patternEnd != null) ? FindEventInternal(eventsOrdered, patternEnd, startTime, double.MaxValue) : null;
			return Tuple.Create(beginEvent, endEvent);
		}

		public CsvEvent FindEvent(string pattern, double startTime = 0.0, double endTime = double.MaxValue)
		{
			return FindEventInternal(Events, pattern, startTime, endTime);
		}

		private CsvEvent FindEventInternal(IEnumerable<CsvEvent> InEvents, string pattern, double startTime = 0.0, double endTime = double.MaxValue)
		{
			foreach (CsvEvent ev in InEvents)
			{
				if (CsvStats.DoesSearchStringMatch(ev.Name, pattern) && ev.Timestamp >= startTime && ev.Timestamp <= endTime)
				{
					return ev;
				}
			}
		...
		public List<CsvEvent> FindEvents(string pattern, double startTime = 0.0, double endTime = double.MaxValue)
		{
			return FindEventsInternal(Events, pattern, startTime, endTime);
		}

		private List<CsvEvent> FindEventsInternal(IEnumerable<CsvEvent> InEvents, string pattern, double startTime = 0.0, double endTime = double.MaxValue)
		{
			List<CsvEvent> eventList = new List<CsvEvent>();
			foreach (CsvEvent ev in InEvents)
			{
				if (CsvStats.DoesSearchStringMatch(ev.Name, pattern) && ev.Timestamp >= startTime && ev.Timestamp <= endTime)
				{
		...
		enum BuiltinFunctionType { addstats, meta, eventcount, framedelta, cumulative, COUNT };
		string ReadNextToken(string expression, ref int index, Dictionary<string, StatSamples> statsOut, int tokenIndex)
		{
			...
			for ( int i=0; i<(int)BuiltinFunctionType.COUNT; i++)
			{
					...
					remaining.StartsWith(funcName+"("))
				{
					...
					else if (funcType == BuiltinFunctionType.cumulative)
					{
						// Quotes are optional. They're supported for consistency with stats
						string srcStatName = ReadQuotedString(paramString);
						srcStatName = (srcStatName == null) ? paramString : srcStatName;
						if (srcStatName == null)
						{
							throw new DerivedStatException("Cumulative requires an arg: " + expression[oldIndex..index]);
						}
						...
						string destStatName = "__STAT__CUMULATIVE__" + tokenIndex.ToString();
						StatSamples newStat = new StatSamples(destStatName, srcStat.samples.Count);
						double total = 0.0;
						for (int j = 0; j < srcStat.samples.Count; j++)
						{
							total += srcStat.samples[j];
							newStat.samples[j] = (float)total;
						}
						statsOut[destStatName] = newStat;
						return destStatName;
					}
		...
		public static string GetColorFromString(string input, int minBrightness = 64, int maxBrightness = 200)
		{
			using (var md5 = MD5.Create())
			{
				byte[] hash = md5.ComputeHash(Encoding.UTF8.GetBytes(input));
				byte r = (byte)(hash[0] % (maxBrightness - minBrightness) + minBrightness);
				byte g = (byte)(hash[1] % (maxBrightness - minBrightness) + minBrightness);
				byte b = (byte)(hash[2] % (maxBrightness - minBrightness) + minBrightness);
		...
		public static CsvStats ReadBinFile(string filename, string[] statNamesToRead=null, int numRowsToSkip=0, bool justHeader=false, CsvFileInfo FileInfoOut=null)
		{
			...
			for (int i = 0; i < eventCount; i++)
			{
				...
				double timestamp = double.MaxValue;
				if (version >= (int)CsvBinVersion.EventTimestamps)
				{
					timestamp = fileReader.ReadDouble();
				}
				if (frame >= numRowsToSkip)
				{
					CsvEvent ev = new CsvEvent(name, frame, timestamp);
		...
		public void WriteToCSV(string filename, bool bWriteMetadataRow = true)
        {
            ...
            // Store the event text in a dictionary so we can lookup by frame
            Dictionary<int, string> eventsLookup = new Dictionary<int, string>();
            foreach (CsvEvent ev in Events)
            {
				string eventText = ev.HasTimestamp() ? ev.Name + "##" + ev.Timestamp.ToString(CultureInfo.InvariantCulture) : ev.Name;
                if (!eventsLookup.ContainsKey(ev.Frame))
                {
                    eventsLookup.Add(ev.Frame, eventText);
                }
                else
                {
                    eventsLookup[ev.Frame] += ";" + eventText;
                }
            }
			...
			if (Events.Count > 0)
			{
				...
				for (int i = 0; i < SampleCount; i++)
				{
					...
					while (eventIndex<Events.Count && Events[eventIndex].Frame==i)
					{
						...
						if (bFrameExists)
						{
							newEvents.Add(new CsvEvent(currentEvent.Name, currentEvent.Frame-skippedFrameCount, currentEvent.Timestamp));
						}
						eventIndex++;
					}
					if (!bFrameExists)
                ...
                for (int i = 0; i < eventStrings.Length; i++)
                {
                    ...
                    if (!string.IsNullOrEmpty(eventString))
                    {
                        ...
                        foreach (string EventString in Events)
                        {
                            ...
                            {
								double timestamp = double.MaxValue;
								string name = EventString;

								// Check if there's a timestamp embedded
								int suffixIndex = EventString.LastIndexOf("##");
								if (suffixIndex != -1)
								{
									name = EventString.Substring(0, suffixIndex);
									if ( double.TryParse(EventString.Substring(suffixIndex + 2), NumberStyles.Float, CultureInfo.InvariantCulture, out double result) )
									{
										timestamp = result;
									}
								...
								CsvEvent ev = new CsvEvent(name, i, timestamp);
