...
namespace Lightmass
{
		...
		const TMap<FGuid,TArray<FVolumeLightingSample> >& VolumeSamples) const
	{
		if (bExportVolumeLightingDebugOutput)
		{
			...
			{
				UE_LOGF(LogLightmass, Log, "Failed to open volume sample debug output channel!");
			}
		}
		...
		{
			UE_LOGF(LogLightmass, Log, "Failed to open volume samples channel!");
		}
	}
	...
	void FLightmassSolverExporter::ExportStaticShadowDepthMap(const FGuid& LightGuid, const FStaticShadowDepthMap& StaticShadowDepthMap) const
	{
		...
		{
			UE_LOGF(LogLightmass, Log, "Failed to open static shadow depth map channel!");
		}
	}
	...
	void FLightmassSolverExporter::ExportMeshAreaLightData(const TIndirectArray<FMeshAreaLight>& MeshAreaLights, float MeshAreaLightGeneratedDynamicLightSurfaceOffset) const
	{
		...
		{
			UE_LOGF(LogLightmass, Log, "Failed to open dominant shadow channel!");
		}
	}
...
#pragma pack (pop)
	...
	int32 FLightmassSolverExporter::BeginExportResults(struct FTextureMappingStaticLightingData& LightingData, uint32 NumMappings) const
	{
		...
		if( ErrorCode < 0 )
		{
			UE_LOGF(LogLightmass, Log, "Failed to open texture mapping channel %ls!", *ChannelName);
		}
		else
		{
	...
	{
		UE_LOGF(LogLightmass, Verbose, "Exporting texture lighting %ls [%.3fs]", *(LightingData.Mapping->Guid.ToString()), LightingData.ExecutionTime);
...
//		UE_LOGF(LogLightmass, Log, "LM data went from %d to %d bytes", LightingData.LightMapData->UncompressedDataSize, LightingData.LightMapData->CompressedDataSize);
...
#pragma pack (pop)
	...
	void FLightmassSolverExporter::ExportResults(const FPrecomputedVisibilityData& TaskData) const
	{
		...
		{
			UE_LOGF(LogLightmass, Log, "Failed to open precomputed visibility channel!");
		}
	}
	...
	void FLightmassSolverExporter::ExportResults(const FVolumetricLightmapTaskData& TaskData) const
	{
		...
		{
			UE_LOGF(LogLightmass, Log, "Failed to open volumetric lightmap channel!");
		}
	}
