...
,	Exporter(InExporter)
{
	...
	UE_LOGF(LogLightmass, Log, "FStaticLightingSystem started using GKDOPMaxTrisPerLeaf: %d", DEFAULT_MAX_TRIS_PER_LEAF);
	...
	UE_LOGF(LogLightmass, Log,  "Number of texture mappings: %d", InScene.TextureLightingMappings.Num() );
	for (int32 MappingIndex = 0; MappingIndex < InScene.TextureLightingMappings.Num(); MappingIndex++)
	{
		...
		if (bDumpAllMappings)
		{
			UE_LOGF(LogLightmass, Log, "\t%ls", *(Mapping->Guid.ToString()));
		}
	}

	UE_LOGF(LogLightmass, Log,  "Number of fluid mappings:   %d", InScene.FluidMappings.Num());
	...
	UE_LOGF(LogLightmass, Log,  "Number of landscape mappings:   %d", InScene.LandscapeMappings.Num());
	...
	UE_LOGF(LogLightmass, Log,  "Number of BSP mappings:     %d", InScene.BspMappings.Num() );
	for( int32 MeshIdx=0; MeshIdx < InScene.BspMappings.Num(); ++MeshIdx )
	{
		...
		if (bDumpAllMappings)
		{
			UE_LOGF(LogLightmass, Log, "\t%ls", *(BSPMapping->Mapping.Guid.ToString()));
		}
	}

	UE_LOGF(LogLightmass, Log,  "Number of volume mappings:   %d", InScene.VolumeMappings.Num());
	...
	UE_LOGF(LogLightmass, Log,  "Number of landscape volume mappings:   %d", InScene.LandscapeVolumeMappings.Num());
	...
	UE_LOGF(LogLightmass, Log,  "Number of static mesh instance mappings: %d", InScene.StaticMeshInstances.Num() );
...
void FStaticLightingSystem::MultithreadProcess()
{
	...
	UE_LOGF(LogLightmass, Log,  "Processing..." );
			...
			if ( Threads[ThreadIndex].Thread != NULL )
			{
				...
				if ( Threads[ThreadIndex].CheckHealth( true ) )
				{
					...
					if (Threads[ThreadIndex].IsComplete())
					{
						...
						{
							UE_LOGF(LogLightmass, Log,  "Thread %d finished: %ls", ThreadIndex, *FPlatformTime::PrettyTime(Threads[ThreadIndex].ThreadStatistics.TotalTime) );
						}
...
void FStaticLightingSystem::DumpStats(float TotalStaticLightingTime) const
{
	...
	if (bDumpMemoryStats)
	{
		...
		UE_LOGF(LogLightmass, Log, "Virtual memory used %.1fMb, Peak %.1fMb", 
...
#elif PLATFORM_MAC
		{
			...
			UE_LOGF(LogLightmass, Log, "Virtual memory used %.1fMb",
				TaskInfo.virtual_size / 1048576.0f);	// can't get peak virtual memory on Mac
		}
#endif
		AggregateMesh->DumpStats();
		UE_LOGF(LogLightmass, Log, "DirectPhotonMap");
		DirectPhotonMap.DumpStats(false);
		UE_LOGF(LogLightmass, Log, "FirstBouncePhotonMap");
		FirstBouncePhotonMap.DumpStats(false);
		UE_LOGF(LogLightmass, Log, "FirstBounceEscapedPhotonMap");
		FirstBounceEscapedPhotonMap.DumpStats(false);
		UE_LOGF(LogLightmass, Log, "FirstBouncePhotonSegmentMap");
		FirstBouncePhotonSegmentMap.DumpStats(false);
		UE_LOGF(LogLightmass, Log, "SecondBouncePhotonMap");
		SecondBouncePhotonMap.DumpStats(false);
		UE_LOGF(LogLightmass, Log, "IrradiancePhotonMap");
		...
		UE_LOGF(LogLightmass, Log, "%.3fMb for Irradiance Photon surface caches", IrradiancePhotonCacheBytes / 1048576.0f);
	}
}
...
bool FStaticLightingThreadRunnable::CheckHealth(bool bReportError) const
{
	...
	{
		UE_LOGF(LogLightmass, Fatal, "Static lighting thread exception:\r\n%ls", *ErrorMessage );
	}
...
uint32 FMappingProcessingThreadRunnable::Run()
{
	...
	if(!FPlatformMisc::IsDebuggerPresent())
	{
		__try
		{
			...
			{
				UE_LOGF(LogLightmass, Fatal, "Unsupported task type");
			}
		}
				...
				else
				{
					...
					else
					{
						...
						else
						{
							...
							UE_LOGF(LogLightmass, Log, "Lightmass - Rejecting task (%08X%08X%08X%08X)!", TaskGuid.A, TaskGuid.B, TaskGuid.C, TaskGuid.D);
...
void FStaticLightingSystem::ThreadLoop(bool bIsMainThread, int32 ThreadIndex, FThreadStatistics& ThreadStatistics, float& IdleTime)
{
	...
	while (!bIsDone)
	{
		...
		// Process any existing local tasks before fetching new tasks from Swarm
		{
			...
			{
				//UE_LOGF(LogLightmass, Warning, "Thread %u picked up Cache Indirect task for %u", ThreadIndex, NextCacheTask->TextureMapping->Guid.D);
			...
			{
				//UE_LOGF(LogLightmass, Warning, "Thread %u picked up Interpolate indirect task for %u", ThreadIndex, NextInterpolateTask->TextureMapping->Guid.D);
