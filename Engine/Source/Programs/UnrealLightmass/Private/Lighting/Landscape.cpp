...
namespace Lightmass
{
	...
	void FLandscapeStaticLightingMesh::GetStaticLightingVertex(int32 VertexIndex, FStaticLightingVertex& OutVertex) const
	{
		...
		//UE_LOGF(LogLightmass, Log, "%d, %d, %d, %d, %d, %d, X:%f, Y:%f, Z:%f ", SectionBaseX + LocalX - ExpandQuadsX, SectionBaseY + LocalY - ExpandQuadsY, ClampedLocalX, ClampedLocalY, SectionBaseX, SectionBaseY, WorldPos.X, WorldPos.Y, WorldPos.Z);
