...
{
	UE_LOGF(LogLightmass, Log, "Reserving memory for %d meshes, %d vertices, %d triangles", NumMeshes, NumVertices, NumTriangles);
...
{
	UE_LOGF(LogLightmass, Log, "Preallocated %.1fGb for kDOP nodes and triangles", kDopTree.kDOPPreallocatedMemory / 1024.0f / 1024.0f / 1024.0f);
	UE_LOGF(LogLightmass, Log, "Building kDOP took %5.2f seconds.", kDopTree.kDOPBuildTime);

	// Log information about the aggregate mesh.
	UE_LOGF(LogLightmass, Log, "Static lighting kDOP: %u nodes, %u leaves, %u triangles, %u vertices", kDopTree.GKDOPNodes, kDopTree.GKDOPNumLeaves, kDopTree.GKDOPTriangles, Vertices.Num());
	UE_LOGF(LogLightmass, Log, "Static lighting kDOP: %.3f%% wasted space in leaves", ((kDopTree.GKDOPTriangles - kDOPTriangles.Num()) / (float)kDopTree.GKDOPTriangles) * 100.0f);
	...
	UE_LOGF(LogLightmass, Log, "kDopTree.Nodes        : %7.1fMb", kDopTree.Nodes.GetAllocatedSize() / 1048576.0f);
	UE_LOGF(LogLightmass, Log, "kDopTree.SOATriangles : %7.1fMb", kDopTree.SOATriangles.GetAllocatedSize() / 1048576.0f);
	UE_LOGF(LogLightmass, Log, "kDOPTriangles         : %7.1fMb", kDOPTriangles.GetAllocatedSize() / 1048576.0f);
	UE_LOGF(LogLightmass, Log, "TrianglePayloads      : %7.1fMb", TrianglePayloads.GetAllocatedSize() / 1048576.0f);
	UE_LOGF(LogLightmass, Log, "MeshInfos             : %7.1fMb", MeshInfos.GetAllocatedSize() / 1048576.0f);
	UE_LOGF(LogLightmass, Log, "Vertices              : %7.1fMb", Vertices.GetAllocatedSize() / 1048576.0f);
	UE_LOGF(LogLightmass, Log, "UVs                   : %7.1fMb", UVs.GetAllocatedSize() / 1048576.0f);
	UE_LOGF(LogLightmass, Log, "LightmapUVs           : %7.1fMb", LightmapUVs.GetAllocatedSize() / 1048576.0f);
	UE_LOGF(LogLightmass, Log, "Static lighting kDOP: %u nodes, %u leaves, %u triangles, %u vertices, %.1f Mb", kDopTree.GKDOPNodes, kDopTree.GKDOPNumLeaves, kDopTree.GKDOPTriangles, Vertices.Num(), kDOPTreeBytes / 1048576.0f);
}
...
void FDefaultAggregateMesh::DumpPostBuildStats() const
{
	...
	UE_LOGF(LogLightmass, Log, "kDOP traversals (in millions): %.3g parents, %.3g leaves, %.3g triangles (%.3g, %.3g%%, real triangles).",
