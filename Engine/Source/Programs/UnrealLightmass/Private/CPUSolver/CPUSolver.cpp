...
void ReportStatistics()
{
	if ( GReportDetailedStats )
	{
		...
		if ( GDebugMode == false )
		{
			UE_LOGF(LogLightmass, Log,  "Time in SendMessage() = %ls", *FPlatformTime::PrettyTime(GStatistics.SendMessageTime) );
			UE_LOGF(LogLightmass, Log,  "Task request roundtrip = %ls", *FPlatformTime::PrettyTime(FTiming::GetAverageTiming()) );
		}
	}
	else
	{
...
void ReportLogFile()
{
	...
	if ( !bIsOk )
	{
		UE_LOGF(LogLightmass, Log, "Failed to send back log file through Swarm!");
	}
}
...
void BuildStaticLighting(const FGuid& SceneGuid, int32 NumThreads, bool bDumpTextures)
{
	...
	UE_LOGF(LogLightmass, Log, "Building static lighting...");
	...
	{
		UE_LOGF(LogLightmass, Log, "Failed to import scene file");
		exit( 1 );
	}
	...
	UE_LOGF(LogLightmass, Log, "Lighting complete [Startup = %ls, Lighting = %ls]", *FPlatformTime::PrettyTime(LightTimeStart - SetupTimeStart), *FPlatformTime::PrettyTime(EndTime - LightTimeStart));
