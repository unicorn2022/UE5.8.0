...
public:
	...
	virtual uint32 Run(void)
	{
		...
		{
			UE_LOGF(LogLightmass, Display, "Thread counter %d", i);
			FPlatformProcess::Sleep(1.0f);
		}
		UE_LOGF(LogLightmass, Display, "Thread done!!");
...
void TestLightmass()
{
	...
	UE_LOGF(LogLightmass, Display, "===============================================================================================");
	UE_LOGF(LogLightmass, Display, "Running \"unit test\". This will take several seconds, and will end with an assertion.");
	UE_LOGF(LogLightmass, Display, "This is on purpose, as it's testing the callstack gathering...");
	UE_LOGF(LogLightmass, Display, "===============================================================================================");
	UE_LOGF(LogLightmass, Display, "\n\n");
	...
	FGuid Guid(1, 2, 3, 4);
	UE_LOGF(LogLightmass, Display, "Guid is %ls", *Guid.ToString());

	TMap<FString, int32> TestMap;
	TestMap.Add(FString(TEXT("Five")), 5);
	TestMap.Add(TEXT("Ten"), 10);

	UE_LOGF(LogLightmass, Display, "Map[Five] = %d, Map[Ten] = %d", TestMap.FindRef(TEXT("Five")), TestMap.FindRef(FString(TEXT("Ten"))));

	FMatrix44f TestMatrix(FVector3f(0, 0, 0.1f), FVector3f(0, 1.0f, 0), FVector3f(0.9f, 0, 0), FVector3f(0, 0, 0));

	UE_LOGF(LogLightmass, Display, "Mat=\n  [%0.2f, %0.2f, %0.2f, %0.2f]\n  [%0.2f, %0.2f, %0.2f, %0.2f]\n  [%0.2f, %0.2f, %0.2f, %0.2f]\n  [%0.2f, %0.2f, %0.2f, %0.2f]", 
	...
	UE_LOGF(LogLightmass, Display, "Transposed Mat=\n  [%0.2f, %0.2f, %0.2f, %0.2f]\n  [%0.2f, %0.2f, %0.2f, %0.2f]\n  [%0.2f, %0.2f, %0.2f, %0.2f]\n  [%0.2f, %0.2f, %0.2f, %0.2f]", 
		TestMatrix.M[0][0], TestMatrix.M[0][1], TestMatrix.M[0][2], TestMatrix.M[0][3], 
		TestMatrix.M[1][0], TestMatrix.M[1][1], TestMatrix.M[1][2], TestMatrix.M[1][3], 
		TestMatrix.M[2][0], TestMatrix.M[2][1], TestMatrix.M[2][2], TestMatrix.M[2][3], 
		TestMatrix.M[3][0], TestMatrix.M[3][1], TestMatrix.M[3][2], TestMatrix.M[3][3]
	...
	UE_LOGF(LogLightmass, Display, "Inverted Mat=\n  [%0.2f, %0.2f, %0.2f, %0.2f]\n  [%0.2f, %0.2f, %0.2f, %0.2f]\n  [%0.2f, %0.2f, %0.2f, %0.2f]\n  [%0.2f, %0.2f, %0.2f, %0.2f]", 
		TestMatrix.M[0][0], TestMatrix.M[0][1], TestMatrix.M[0][2], TestMatrix.M[0][3], 
		TestMatrix.M[1][0], TestMatrix.M[1][1], TestMatrix.M[1][2], TestMatrix.M[1][3], 
		TestMatrix.M[2][0], TestMatrix.M[2][1], TestMatrix.M[2][2], TestMatrix.M[2][3], 
		TestMatrix.M[3][0], TestMatrix.M[3][1], TestMatrix.M[3][2], TestMatrix.M[3][3]
	...
	UE_LOGF(LogLightmass, Display, "sizeof FDirectionalLight = %zu, FLight = %zu, FDirectionalLightData = %zu", sizeof(FDirectionalLight), sizeof(FLight), sizeof(FDirectionalLightData));
	...
	UE_LOGF(LogLightmass, Display, "%.2f seconds have passed, killing thread", FPlatformTime::Seconds() - Start);
	...
	UE_LOGF(LogLightmass, Display, "It took %.2f seconds to kill the thread [should be < 1 second]", FPlatformTime::Seconds() - KillStart);
