...
int LightmassMain(int argc, ANSICHAR* argv[])
{
	...
	UE_LOGF(LogLightmass, Display,  "Lightmass %ls started on: %ls. Command-line: %ls", FPlatformMisc::GetUBTPlatform(), FPlatformProcess::ComputerName(), FCommandLine::Get() );
	...
	for (int32 ArgIndex = 1; ArgIndex < argc; ArgIndex++)
	{
		...
		{
			UE_LOGF(LogLightmass, Display, "Usage:\n  UnrealLightmass\n\t[SceneGuid]\n\t[-debug]\n\t[-unittest]\n\t[-dumptex]\n\t[-numthreads N]\n\t[-compare Dir1 Dir2 [-error N]]");
			UE_LOGF(LogLightmass, Display, "");
			UE_LOGF(LogLightmass, Display, "  SceneGuid : Guid of a scene file. 0x0000012300004567000089AB0000CDEF is the default");
			UE_LOGF(LogLightmass, Display, "  -debug : Processes all mappings in the scene, instead of getting tasks from Swarm Coordinator");
			UE_LOGF(LogLightmass, Display, "  -unittest : Runs a series of validations, then quits");
			UE_LOGF(LogLightmass, Display, "  -dumptex : Outputs .bmp files to the current directory of 2D lightmap/shadowmap results");
			UE_LOGF(LogLightmass, Display, "  -compare : Compares the binary dumps created by UnrealEd to compare Unreal vs LM lighting runs");
			UE_LOGF(LogLightmass, Display, "  -error : Controls the threshold that an error is counted when comparing with -compare");
			return 0;
		}
		...
		else if (FCStringAnsi::Stricmp(argv[ArgIndex], "-numthreads") == 0)
		{
			...
			if (NumThreads == 0)
			{
				UE_LOGF(LogLightmass, Display, "The number of threads was not specified properly, use \"-numthreads N\"");
				return 1;
			}
		}
		else if (FCStringAnsi::Stricmp(argv[ArgIndex], "-compare") == 0)
		{
			...
			if (ArgIndex >= argc - 2)
			{
				UE_LOGF(LogLightmass, Display, "-compare requires two directories to compare (-compare Dir1 Dir2)");
				return 1;
			}
		...
		else if (FCStringAnsi::Stricmp(argv[ArgIndex], "-error") == 0)
		{
			...
			if (ArgIndex >= argc - 1)
			{
				UE_LOGF(LogLightmass, Display, "-error requires an error value following (-error N)");
				return 1;
			}
	...
	UE_LOGF(LogLightmass, Display,  "Processing scene GUID: %08X%08X%08X%08X with %d threads", SceneGuid.A, SceneGuid.B, SceneGuid.C, SceneGuid.D, NumThreads );
...
{
	UE_LOGF(LogLightmass, Display, "");
	UE_LOGF(LogLightmass, Display, "Comparing '%ls' vs '%ls'", Dir1, Dir2);
...
void CriticalErrorCallback()
{
	...
	{
		UE_LOGF(LogLightmass, Display, "--- Critical Error! Machine: %ls. Logfile: %ls. Crash report: %ls. ---", FPlatformProcess::ComputerName(), FLightmassLog::Get()->GetLogFilename(), TEXT("")/**CrashReporterURL*/ );
	}
}
...
bool VerifyDLL( const TCHAR* DLLFilename )
{
	...
	if ( DbgHelpDll == NULL )
	{
		UE_LOGF(LogLightmass, Display,  "Failed to load %ls!", DLLFilename );
		return false;
	}
	return true;
}
