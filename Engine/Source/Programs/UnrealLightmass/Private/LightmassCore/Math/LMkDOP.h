...
struct TkDOPTree
{
	...
	void Dump()
	{
		UE_LOGF(LogLightmass, Log, "kDOPTree [%x], %d nodes, %d triangles", this, Nodes.Num(), GKDOPTriangles);
		UE_LOGF(LogLightmass, Log, "");
		for (int32 NodeIndex = 0; NodeIndex < Nodes.Num(); NodeIndex++)
		{
			const NodeType& Node = Nodes(NodeIndex);
			UE_LOGF(LogLightmass, Log, " Node %03d: %ls", NodeIndex, Node.bIsLeaf ? TEXT("leaf") : TEXT("nonleaf"));
			if (Node.bIsLeaf)
			{
				UE_LOGF(LogLightmass, Log, "  StartIndex = %d, NumTris = %d", Node.t.StartIndex, Node.t.NumTriangles);
			}
			else
			{
				UE_LOGF(LogLightmass, Log, "  LeftChild = %d, RightChild= %d", Node.n.LeftNode, Node.n.RightNode);
			}
		}
		UE_LOGF(LogLightmass, Log, "");
		UE_LOGF(LogLightmass, Log, "");
