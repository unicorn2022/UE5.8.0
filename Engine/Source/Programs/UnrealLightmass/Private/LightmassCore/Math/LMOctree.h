...
void TOctree<ElementType,OctreeSemantics>::DumpStats(bool bDetailed) const
{
	...
	if (NumElements > 0)
	{
		UE_LOGF(LogLightmass, Log, "Octree overview:");
		UE_LOGF(LogLightmass, Log, "\t%zu bytes per node", sizeof(FNode));
		UE_LOGF(LogLightmass, Log, "\t%i nodes, for %.1f Mb", NumNodes, NodeBytes / 1048576.0f);
		UE_LOGF(LogLightmass, Log, "\t%i leaves", NumLeaves);
		UE_LOGF(LogLightmass, Log, "\t%i elements", NumElements);
		UE_LOGF(LogLightmass, Log, "\t%.1f%% elements in leaves", 100.0f * NumLeafElements / (float)NumElements);
		UE_LOGF(LogLightmass, Log, "\t%.1f avg elements per node, %i max elements per node", NumElements / (float)NumNodes, MaxElementsPerNode);
		if (bDetailed)
		{
			UE_LOGF(LogLightmass, Log, "Octree node element distribution:");
			for( int32 i=0; i<NodeElementDistribution.Num(); i++ )
			{
				if( NodeElementDistribution[i] > 0 )
				{
					UE_LOGF(LogLightmass, Log, "\tElements: %3i, Nodes: %3i",i,NodeElementDistribution[i]);
				}
			}
		}
	}
