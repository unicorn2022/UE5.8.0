...
TArray<FOnlineAccountCredentials> FOnlineSubsystemTestBaseFixture::GetIniCredentials(int32 TestAccountIndex) const
{
	...
	{
		UE_LOGF(LogOSSTests, Error, "Attempted to GetCredentials for more than we have stored! Add more credentials to the DefaultEngine.ini for OssTests");
...
bool FOnlineSubsystemFixtureInvoker::IsRunningTestSkipOnTags(const FString& ServiceName, const FReportingSkippableTags& SkippableTags, FString& InTag)
{
	...
	switch (Reason)
	{
		...
		{
			UE_LOGF(LogOSSTests, Verbose, "Test skipped due to run against this service.");
			break;
		}
		case EDisableReason::ExclusiveService:
		{
			UE_LOGF(LogOSSTests, Verbose, "Test skipped due to exclusive service run.");
			break;
		}
		case EDisableReason::DisableTagPresence:
		{
			UE_LOGF(LogOSSTests, Verbose, "Test skipped due to disable tag presence.");
			break;
		}
		default:
		{
			UE_LOGF(LogOSSTests, Error, "Test skipped due to unknown reason!");
			break;
		}
	}
