...
namespace {\
class UE_JOIN(OnlineSubsystemTest_,RegName) : public ClassName {\
public:\
	virtual void invoke() const override;\
	void test();\
};\
	FOnlineSubsystemAutoReg RegName( ::MakeTestInvokerFixture( UE_JOIN(&OnlineSubsystemTest_,RegName)::test ), CATCH_INTERNAL_LINEINFO, Name, Tags, "");\
}\
void UE_JOIN(OnlineSubsystemTest_,RegName)::test()\
{\
	invoke();\
}\
void UE_JOIN(OnlineSubsystemTest_,RegName)::invoke() const\
