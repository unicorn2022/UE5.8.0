...
namespace {\
class UE_JOIN(OnlineTest_,RegName) : public OnlineTestBase {\
protected:\
	virtual void invoke() const override;\
};\
	OnlineTestBase* UE_JOIN(Construct_,RegName) (){\
		return new UE_JOIN(OnlineTest_,RegName) ();\
	}\
	OnlineAutoReg RegName( UE_JOIN(&Construct_,RegName) , CATCH_INTERNAL_LINEINFO , Name, Tags, "");\
}\
void UE_JOIN(OnlineTest_,RegName)::invoke() const\
