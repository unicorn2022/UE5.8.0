...
{
	FTestPipeline& TestPipeline = GetPipeline();
	TArray<FString> CacheDurationTestHierarchy = { TEXT("OnlineTests.TestCacheDuration") };
	...
	// bCacheError=false
	TestPipeline.EmplaceLambda([&CacheDurationTestHierarchy](const IOnlineServicesPtr& Services)
	{
		FOnlineServicesCommon& ServicesCommon = static_cast<FOnlineServicesCommon&>(*Services);
		TOnlineAsyncOpRef<FTestOp> Op = ServicesCommon.GetJoinableOp<FTestOp>(FTestOp::Params(), CacheDurationTestHierarchy);
		Op->Then([](FOnlineAsyncOp&) {})
		.Then([](TOnlineAsyncOp<FTestOp>& Op) { Op.SetResult(FTestOp::Result()); })
		.Enqueue(ServicesCommon.GetParallelQueue());
	})
	.EmplaceLambda([&CacheDurationTestHierarchy](const IOnlineServicesPtr& Services)
	{
		FOnlineServicesCommon& ServicesCommon = static_cast<FOnlineServicesCommon&>(*Services);
		TOnlineAsyncOpRef<FTestOp> Op = ServicesCommon.GetJoinableOp<FTestOp>(FTestOp::Params(), CacheDurationTestHierarchy);
	...
	.EmplaceAsyncLambda([&CacheDurationTestHierarchy](FAsyncLambdaResult Promise, const IOnlineServicesPtr& Services)
	{
		FOnlineServicesCommon& ServicesCommon = static_cast<FOnlineServicesCommon&>(*Services);
		TOnlineAsyncOpRef<FTestOp> Op = ServicesCommon.GetJoinableOp<FTestOp>(FTestOp::Params(), CacheDurationTestHierarchy);
...
{
	FTestPipeline& TestPipeline = GetPipeline();
	TArray<FString> DontCacheErrorHierarchy = { TEXT("OnlineTests.TestDontCacheError") };
	// Uses the following operation configuration in OnlineTests' DefaultEngine.ini
	// [OnlineTests.TestDontCacheError]
	// CacheExpiration=Duration
	// CacheExpirySeconds=1
	// bCacheError=false
	TArray<FString> CacheErrorHierarchy = { TEXT("OnlineTests.TestCacheError") };
	// Uses the following operation configuration in OnlineTests' DefaultEngine.ini
	// [OnlineTests.TestCacheError]
	// CacheExpiration=Duration
	// CacheExpirySeconds=1
	// bCacheError=true
	TestPipeline.EmplaceAsyncLambda([&DontCacheErrorHierarchy](FAsyncLambdaResult Promise, const IOnlineServicesPtr& Services)
	{
		FOnlineServicesCommon& ServicesCommon = static_cast<FOnlineServicesCommon&>(*Services);
		TOnlineAsyncOpRef<FTestOp> Op = ServicesCommon.GetJoinableOp<FTestOp>(FTestOp::Params(), DontCacheErrorHierarchy);
		Op->Then([](FOnlineAsyncOp&) {})
		.Then([Promise](TOnlineAsyncOp<FTestOp>& Op) { Op.SetError(Errors::Unknown()); Promise->SetValue(true); })
		.Enqueue(ServicesCommon.GetParallelQueue());
	})
	.EmplaceAsyncLambda([&CacheErrorHierarchy](FAsyncLambdaResult Promise, const IOnlineServicesPtr& Services)
	{
		FOnlineServicesCommon& ServicesCommon = static_cast<FOnlineServicesCommon&>(*Services);
		TOnlineAsyncOpRef<FTestOp> Op = ServicesCommon.GetJoinableOp<FTestOp>(FTestOp::Params(), CacheErrorHierarchy);
...
{
	FTestPipeline& TestPipeline = GetPipeline();
	TArray<FString> CacheExpireUponCompletionHierarchy = { TEXT("OnlineTests.TestCacheExpireUponCompletion") };
	// Uses the following operation configuration in OnlineTests' DefaultEngine.ini
	// [OnlineTests.TestCacheExpireUponCompletion]
	// CacheExpiration=UponCompletion
	// bCacheError=false
	TestPipeline.EmplaceAsyncLambda([&CacheExpireUponCompletionHierarchy](FAsyncLambdaResult Promise, const IOnlineServicesPtr& Services)
	{
		FOnlineServicesCommon& ServicesCommon = static_cast<FOnlineServicesCommon&>(*Services);
		TOnlineAsyncOpRef<FTestOp> Op = ServicesCommon.GetJoinableOp<FTestOp>(FTestOp::Params(), CacheExpireUponCompletionHierarchy);
		Op->Then([](FOnlineAsyncOp&) {})
		.Then([Promise](TOnlineAsyncOp<FTestOp>& Op) { Op.SetResult(FTestOp::Result()); Promise->SetValue(true); })
		.Enqueue(ServicesCommon.GetParallelQueue());
		TOnlineAsyncOpRef<FTestOp> Op2 = ServicesCommon.GetJoinableOp<FTestOp>(FTestOp::Params(), CacheExpireUponCompletionHierarchy);
		CHECK(Op2->IsReady());
		CHECK(!Op2->IsComplete());
	})
	.EmplaceAsyncLambda([&CacheExpireUponCompletionHierarchy](FAsyncLambdaResult Promise, const IOnlineServicesPtr& Services)
	{
		FOnlineServicesCommon& ServicesCommon = static_cast<FOnlineServicesCommon&>(*Services);
		TOnlineAsyncOpRef<FTestOp> Op = ServicesCommon.GetJoinableOp<FTestOp>(FTestOp::Params(), CacheExpireUponCompletionHierarchy);
		CHECK(!Op->IsReady());
...
ASYNCOP_TEST_CASE("Verify that Then called from inside a running step appends and runs the new step")
{
	FTestPipeline& TestPipeline = GetPipeline();
	TSharedPtr<TArray<int32>> ExecutionOrder = MakeShared<TArray<int32>>();
	TestPipeline.EmplaceAsyncLambda([ExecutionOrder](FAsyncLambdaResult Promise, const IOnlineServicesPtr& Services)
	{
		FOnlineServicesCommon& ServicesCommon = static_cast<FOnlineServicesCommon&>(*Services);
		TOnlineAsyncOpRef<FTestOp> Op = ServicesCommon.GetOp<FTestOp>(FTestOp::Params());

		// Only one step is chained before Enqueue. It appends two more steps to the op while running.
		Op->Then([ExecutionOrder, Promise](TOnlineAsyncOp<FTestOp>& InAsyncOp)
		{
			ExecutionOrder->Add(1);

			InAsyncOp.Then([ExecutionOrder](TOnlineAsyncOp<FTestOp>& InAsyncOp)
			{
				ExecutionOrder->Add(2);
			})
			.Then([ExecutionOrder](TOnlineAsyncOp<FTestOp>& InAsyncOp)
			{
				ExecutionOrder->Add(3);
				InAsyncOp.SetResult(FTestOp::Result());
			});
		})
		.Enqueue(ServicesCommon.GetParallelQueue());
		Op->GetHandle().OnComplete([ExecutionOrder, Promise](const TOnlineResult<FTestOp>& Result)
			{
				CHECK(*ExecutionOrder == TArray<int32>({ 1, 2, 3 }));
				Promise->SetValue(true);
			});
	});
...
ASYNCOP_TEST_CASE("Verify that Then appended from inside a running step is enqueued at the tail (FIFO order)")
{
	FTestPipeline& TestPipeline = GetPipeline();
	TSharedPtr<TArray<int32>> ExecutionOrder = MakeShared<TArray<int32>>();
	TestPipeline.EmplaceAsyncLambda([ExecutionOrder](FAsyncLambdaResult Promise, const IOnlineServicesPtr& Services)
	{
		...
		// Pre-built chain: A -> B. A appends C -> D while it runs.
		// AddStep appends to the tail of the Steps array, so the expected order is A, B, C, D
		// (not A, C, D, B). The appended steps run after any step already queued past A.
		Op->Then([ExecutionOrder, Promise](TOnlineAsyncOp<FTestOp>& InAsyncOp)
		{
			ExecutionOrder->Add(1); // A

			InAsyncOp.Then([ExecutionOrder](TOnlineAsyncOp<FTestOp>& InAsyncOp)
			{
				ExecutionOrder->Add(3); // C
			})
			.Then([ExecutionOrder](TOnlineAsyncOp<FTestOp>& InAsyncOp)
			{
				ExecutionOrder->Add(4); // D
				InAsyncOp.SetResult(FTestOp::Result());
			});
		})
		.Then([ExecutionOrder](TOnlineAsyncOp<FTestOp>& InAsyncOp)
		{
			ExecutionOrder->Add(2); // B - runs before the dynamically appended C and D
		})
		.Enqueue(ServicesCommon.GetParallelQueue());

		Op->GetHandle().OnComplete([ExecutionOrder, Promise](const TOnlineResult<FTestOp>& Result)
			{
				CHECK(*ExecutionOrder == TArray<int32>({ 1, 2, 3, 4 }));
				Promise->SetValue(true);
			});
	});
