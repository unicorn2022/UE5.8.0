...
FSubcommand* SubcommandList = nullptr;

FSubcommand::FSubcommand()
{
	Next = SubcommandList;
	SubcommandList = this;
}
...
FSharedParameters::FSharedParameters(CLI::App* InApp)
{
...
FArchiveStdOut::~FArchiveStdOut()
{
	fflush(stdout);
}

// Both formatter types provide utf8 text
void FArchiveStdOut::Serialize(void* Data, int64 Len) 
{
#if PLATFORM_WINDOWS
...
TArray<FString> GatherAssetsInPaths(TConstArrayView<FString> Roots, bool bRecursive)
{
	...
	ParallelFor(TEXT("GatherAssestInPaths"), Roots.Num(), 1, [&PackagePaths, &Roots, &Visitor](int32 Index)
	{
		...
		{
			UE_LOGF(LogUnrealPackageTool, Error, "Input path %ls is neither a file nor directory", *Path);
		}
...
{	
	...
	void Main()
	{
		UE_LOGF(LogUnrealPackageTool, Log, "Checking packages for licensee version");
	...
	void ScanPackage(const TCHAR* Path)
	{
		...
		{
			UE_LOGF(LogUnrealPackageTool, Log, "Scanning package %ls", Path);
			...
			*Ar << Summary;
			if (Ar->Close())
			{
				...
				{
					UE_LOGF(LogUnrealPackageTool, Error, "Package has a licensee version: %ls", Path);
				}
			}
			else
			{
				UE_LOGF(LogUnrealPackageTool, Warning, "Failed to read package file summary: %ls", Path);
			}
		}
		else
		{
			UE_LOGF(LogUnrealPackageTool, Warning, "Failed to open package: %ls", Path);
		}
	}
};

int32 FTextOutputFormatter::GetIndent()
{
	if (Stack.Num() == 0)
	{
...
FTextOutputFormatter::FBuffer& FTextOutputFormatter::GetValueBuffer()
{
...
void FTextOutputFormatter::FlushBuffer(FBuffer& Buffer)
{
	Ar.Serialize((void*)Buffer.GetData(), Buffer.Len() * sizeof(FBuffer::ElementType));
	Buffer.Reset();
}

bool FTextOutputFormatter::LooksLikeBinary(FStringView View)
{
...
bool FTextOutputFormatter::CharNeedsEscaping(TCHAR C)
{
	switch (C)
	{
	case '\"':
...
bool FTextOutputFormatter::NeedsEscaping(FStringView View)
{
	for (const TCHAR& C : View)
	{
...
void FTextOutputFormatter::WriteEscaped(FBuffer& Buffer, FStringView View)
{
	while(!View.IsEmpty())
	{
...
FTextOutputFormatter::FTextOutputFormatter(FArchive& InAr)
	: Ar(InAr)
{
	Ar.SetIsTextFormat(true);
}

FTextOutputFormatter::~FTextOutputFormatter()
{
}

FArchive& FTextOutputFormatter::GetUnderlyingArchive()
{
	return Ar;
}

bool FTextOutputFormatter::HasDocumentTree() const
{
	return true;
}

// Records: accumulate field-value pairs and write out so that key names are aligned together
void FTextOutputFormatter::EnterRecord()
{
	if (Stack.Num() == 0)
	{
...
void FTextOutputFormatter::LeaveRecord()
{
...
void FTextOutputFormatter::EnterField(FArchiveFieldName Name)
{
	if (Stack.Num() == 1 )
	{
...
void FTextOutputFormatter::LeaveField()
{
	if (Stack.Num() != 1)
	{
		LeaveMapElement();
...
bool FTextOutputFormatter::TryEnterField(FArchiveFieldName Name, bool bEnterWhenSaving)
{
	EnterField(Name);
	return true;
}

void FTextOutputFormatter::EnterArray(int32& NumElements)
{
	if (NumElements == 0)
	{	
...
void FTextOutputFormatter::LeaveArray()
{
	LeaveStream();
}
void FTextOutputFormatter::EnterArrayElement()
{
	EnterStreamElement();
}
void FTextOutputFormatter::LeaveArrayElement()
{
	LeaveStreamElement();
}

void FTextOutputFormatter::EnterStream()
{
	if (Stack.Num() == 0)
	{
...
void FTextOutputFormatter::LeaveStream()
{
...
void FTextOutputFormatter::EnterStreamElement()
{
...
void FTextOutputFormatter::LeaveStreamElement()
{
...
void FTextOutputFormatter::EnterMap(int32& NumElements)
{
	if (NumElements == 0)
	{
		GetValueBuffer() << UTF8TEXTVIEW("Empty");
...
void FTextOutputFormatter::LeaveMap()
{
	LeaveRecord();
}
void FTextOutputFormatter::EnterMapElement(FString& Name)
{
...
void FTextOutputFormatter::LeaveMapElement()
{
	TUniquePtr<FBuffer> Value = MoveTemp(Stack.Top().ValueBuffer);
	Stack.Top().MapEntries.Last().Value = Value->ToString();
}

void FTextOutputFormatter::EnterAttributedValue()
{
}
void FTextOutputFormatter::EnterAttribute(FArchiveFieldName AttributeName)
{
}
void FTextOutputFormatter::EnterAttributedValueValue()
{
}
void FTextOutputFormatter::LeaveAttribute()
{
}
void FTextOutputFormatter::LeaveAttributedValue()
{
}
bool FTextOutputFormatter::TryEnterAttribute(FArchiveFieldName AttributeName, bool bEnterWhenSaving)
{
	EnterAttribute(AttributeName);
	return true;
}

bool FTextOutputFormatter::TryEnterAttributedValueValue()
{
	EnterAttributedValueValue();
	return true;
}

void FTextOutputFormatter::Serialize(uint8& Value)
{
	GetValueBuffer() << Value;
}
void FTextOutputFormatter::Serialize(uint16& Value)
{
	GetValueBuffer() << Value;
}
void FTextOutputFormatter::Serialize(uint32& Value)
{
	GetValueBuffer() << Value;
}
void FTextOutputFormatter::Serialize(uint64& Value)
{
	GetValueBuffer() << Value;
}
void FTextOutputFormatter::Serialize(int8& Value)
{
	GetValueBuffer() << Value;
}
void FTextOutputFormatter::Serialize(int16& Value)
{
	GetValueBuffer() << Value;
}
void FTextOutputFormatter::Serialize(int32& Value)
{
	GetValueBuffer() << Value;
}
void FTextOutputFormatter::Serialize(int64& Value)
{
	GetValueBuffer() << Value;
}
void FTextOutputFormatter::Serialize(float& Value)
{
	GetValueBuffer().Appendf("%f", Value);
}
void FTextOutputFormatter::Serialize(double& Value)
{
	GetValueBuffer().Appendf("%f", Value);
}
void FTextOutputFormatter::Serialize(bool& Value)
{
	GetValueBuffer() << (Value ? UTF8TEXTVIEW("true") : UTF8TEXTVIEW("false"));
}
void FTextOutputFormatter::Serialize(UTF32CHAR& Value)
{
	GetValueBuffer() << Value;
}
void FTextOutputFormatter::Serialize(FString& Value)
{
...
void FTextOutputFormatter::Serialize(FName& Value)
{
	GetValueBuffer() << Value;
}
void FTextOutputFormatter::Serialize(UObject*& Value)
{
	FSoftObjectPath Path(Value);
	Serialize(Path);
}
void FTextOutputFormatter::Serialize(FText& Value)
{
	GetValueBuffer() << UTF8TEXTVIEW("\"") << Value.ToString() << UTF8TEXTVIEW("\"");
}
void FTextOutputFormatter::Serialize(FWeakObjectPtr& Value)
{
...
void FTextOutputFormatter::Serialize(FSoftObjectPtr& Value)
{
	FSoftObjectPath Path = Value.GetUniqueID();
	Serialize(Path);
}
void FTextOutputFormatter::Serialize(FSoftObjectPath& Value)
{
	GetValueBuffer() << WriteToString<FName::StringBufferSize>(Value);
}
void FTextOutputFormatter::Serialize(FLazyObjectPtr& Value)
{
	FString S = Value.GetUniqueID().ToString();
	Serialize(S);
}
void FTextOutputFormatter::Serialize(FObjectPtr& Value)
{
	FSoftObjectPath Path = Value.GetPathName();
	Serialize(Path);
}
void FTextOutputFormatter::Serialize(TArray<uint8>& Value)
{
	Serialize((void*)Value.GetData(), Value.Num());
}
void FTextOutputFormatter::Serialize(void* Data, uint64 DataSize)
{
	FSHAHash Hash;
...
struct FSubcommand_PackageInfo
{
		...
		: Shared(InShared)
	{
		CLI::App* Sub = InApp->add_subcommand("PackageInfo", "Print information contained within asset files.")
		 	// Allow global options to fall through and be parsed
			->fallthrough()
			->preparse_callback([this](std::size_t)
			{
				 PackagePaths.Reset(); 
	...
	void Main()
	{
		...
		if (OutPath.Len() != 0 && AllPackagePaths.Num() > 1 && !FM.DirectoryExists(*OutPath))
		{
			...
			{
				UE_LOGF(LogUnrealPackageTool, Error, "Cannot output multiple packages to directory %ls, a file with that name exists", *OutPath);
				return;
			}
			else if (!IFileManager::Get().MakeDirectory(*OutPath, true))
			{
				UE_LOGF(LogUnrealPackageTool, Error, "Failed to create output directory %ls", *OutPath);
				return;
			}
		}
		...
		for (FString& PackagePath : AllPackagePaths)
		{
				...
				case FPackageReader::EOpenPackageResult::CustomVersionMissing:
					{
						...
						for (const FCustomVersion& Version : Versions)
						{
							...
							{
								UE_LOGF(LogUnrealPackageTool, Verbose, "Continuing to load package %ls with missing custom version %ls", *PackagePath, *WriteToString<128>(Version.Key));
			...
			if (!bContinue)
			{
				UE_LOGF(LogUnrealPackageTool, Error, "Error opening package file %ls: %ls", *PackagePath, LexToString(ErrorCode)); 
				continue;
			}
			...
			else 
			{
				...
				if (!Output.IsValid())
				{
					UE_LOGF(LogUnrealPackageTool, Error, "Error opening output file %ls", *OutFilePath);
					continue;
				}
			...
			if (bAll || bNames)
			{
				...
				{
					UE_LOGF(LogUnrealPackageTool, Error, "Error reading name table for package file %ls", *PackagePath);
				}
			}
			...
			if (bAll || bSoftPaths )
			{
				...
				{
					UE_LOGF(LogUnrealPackageTool, Error, "Error reading soft object path table for package file %ls", *PackagePath);
				}
			}
			...
			if (bAll || bImports || bExports || bSoftPackageReferences || bDepends)
			{
				...
				{
					UE_LOGF(LogUnrealPackageTool, Error, "Error reading import table for package file %ls", *PackagePath);
				}
				if (!Reader.GetExports(Tables.ExportMap))
				{
					UE_LOGF(LogUnrealPackageTool, Error, "Error reading export table for package file %ls", *PackagePath);
				}
				if (!Reader.GetDependsMap(Tables.DependsMap))
				{
					UE_LOGF(LogUnrealPackageTool, Error, "Error reading depends map for package file %ls", *PackagePath);
				}
				if (!Reader.GetSoftPackageReferenceList(Tables.SoftPackageReferenceList))
				{
					UE_LOGF(LogUnrealPackageTool, Error, "Error reading soft package reference list for package file %ls", *PackagePath);
				}
			}
			...
			if (bAll || bText)
			{
				...
				{
					UE_LOGF(LogUnrealPackageTool, Error, "Error reading gatherable text data map for package file %ls", *PackagePath);
				}
			}
			...
			if (bAll || bThumbnails)
			{
				...
				{
					UE_LOGF(LogUnrealPackageTool, Error, "Error reading thumbnail map package file %ls", *PackagePath);
				}
			}
			...
			if (bAll || bAssetRegistry)
			{
				...
				{
					UE_LOGF(LogUnrealPackageTool, Error, "Error reading asset registry data from package file %ls", *PackagePath);
				}
			}
		}
	}
...
INT32_MAIN_INT32_ARGC_TCHAR_ARGV()
{
	...
	try 
	{
		App.ignore_case();
		App.set_help_flag();	
		App.set_help_all_flag("--help,-h", "Expand all help");	
		...
		for (FSubcommand* Sub = SubcommandList; Sub != nullptr; Sub = Sub->Next)
		{
			Sub->RegisterSubcommand(&Params, &App);
		}
	...
	{
		Out->Logf(TEXT("Error: %s"), UTF8_TO_TCHAR(e.what()));
		Out->Logf(TEXT("%s"), UTF8_TO_TCHAR(App.help().c_str()));
		Ret = e.get_exit_code();	
	}
	catch (CLI::Error& e)
