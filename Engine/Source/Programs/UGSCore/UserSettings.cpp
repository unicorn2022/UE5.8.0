	...
	: FileName(InFileName)
{
	...
	ConfigFile.TryGetValues(TEXT("General.SyncIncludedCategories"), SyncIncludedCategories);
...
TSharedRef<FUserWorkspaceSettings> FUserSettings::FindOrAddWorkspace(const TCHAR* ClientBranchPath)
{
	...
	if(CurrentWorkspaceEntry == nullptr)
	{
		...
		if(!WorkspaceSection.IsValid())
		{
			...
			CurrentWorkspace->SyncIncludedCategories.Empty();
		...
		else
		{
			...
			WorkspaceSection->TryGetValues(TEXT("SyncIncludedCategories"), CurrentWorkspace->SyncIncludedCategories);
...
void FUserSettings::Save()
{
	...
	GeneralSection->SetValues(TEXT("SyncIncludedCategories"), SyncIncludedCategories);
...
//		WindowSection.SetValue(TEXT("Height"), WindowRectangle.Height);
	...
	for(TTuple<FString, TSharedRef<FUserWorkspaceSettings>>& Pair : WorkspaceKeyToSettings)
	{
		...
		WorkspaceSection->SetValues(TEXT("SyncIncludedCategories"), CurrentWorkspace.SyncIncludedCategories);
...
static FString GetGlobalJsonPath()
{
	FPlatformProcess::ApplicationSettingsContext Context;
	Context.Location = FPlatformProcess::ApplicationSettingsContext::Context::LocalUser;
	Context.bIsEpic = false;
	return FPlatformProcess::GetApplicationSettingsDir(Context) / TEXT("UnrealGameSync") / TEXT("Global.json");
}

void FUserSettings::LoadGlobalSettings()
{
	FString GlobalJsonPath = GetGlobalJsonPath();

	FString JsonString;
	if (!FFileHelper::LoadFileToString(JsonString, *GlobalJsonPath))
	{
		return;
	}
	...
	if (!RootObject->TryGetObjectField(TEXT("Filter"), FilterObject))
	{
		return;
	}

	const TArray<TSharedPtr<FJsonValue>>* ExcludeArray;
	if ((*FilterObject)->TryGetArrayField(TEXT("ExcludeCategories"), ExcludeArray))
	{
		SyncExcludedCategories.Empty();
		for (const TSharedPtr<FJsonValue>& Value : *ExcludeArray)
		{
			FGuid Guid;
			if (FGuid::Parse(Value->AsString(), Guid))
			{
				SyncExcludedCategories.Add(Guid);
			}
		}
	}

	const TArray<TSharedPtr<FJsonValue>>* IncludeArray;
	if ((*FilterObject)->TryGetArrayField(TEXT("IncludeCategories"), IncludeArray))
	{
		SyncIncludedCategories.Empty();
		for (const TSharedPtr<FJsonValue>& Value : *IncludeArray)
		{
			FGuid Guid;
			if (FGuid::Parse(Value->AsString(), Guid))
			{
				SyncIncludedCategories.Add(Guid);
			}
		}
	}

	const TArray<TSharedPtr<FJsonValue>>* ViewArray;
	if ((*FilterObject)->TryGetArrayField(TEXT("View"), ViewArray))
	{
		SyncView.Empty();
		for (const TSharedPtr<FJsonValue>& Value : *ViewArray)
		{
			SyncView.Add(Value->AsString());
		}
	}
}

void FUserSettings::SaveGlobalSettings()
{
	FString GlobalJsonPath = GetGlobalJsonPath();

	// Read-modify-write to preserve other fields in Global.json
	TSharedPtr<FJsonObject> RootObject;
	FString JsonString;
	if (FFileHelper::LoadFileToString(JsonString, *GlobalJsonPath))
	{
	...
	TSharedPtr<FJsonObject> FilterObject = MakeShared<FJsonObject>();

	TArray<TSharedPtr<FJsonValue>> IncludeValues;
	for (const FGuid& Guid : SyncIncludedCategories)
	{
		IncludeValues.Add(MakeShared<FJsonValueString>(Guid.ToString(EGuidFormats::DigitsWithHyphensLower)));
	}
	FilterObject->SetArrayField(TEXT("IncludeCategories"), IncludeValues);

	TArray<TSharedPtr<FJsonValue>> ExcludeValues;
	for (const FGuid& Guid : SyncExcludedCategories)
	{
		ExcludeValues.Add(MakeShared<FJsonValueString>(Guid.ToString(EGuidFormats::DigitsWithHyphensLower)));
	}
	FilterObject->SetArrayField(TEXT("ExcludeCategories"), ExcludeValues);

	TArray<TSharedPtr<FJsonValue>> ViewValues;
	for (const FString& ViewLine : SyncView)
	{
		ViewValues.Add(MakeShared<FJsonValueString>(ViewLine));
	}
	FilterObject->SetArrayField(TEXT("View"), ViewValues);

	RootObject->SetObjectField(TEXT("Filter"), FilterObject);

	FString OutputString;
	TSharedRef<TJsonWriter<>> Writer = TJsonWriterFactory<>::Create(&OutputString);
	FJsonSerializer::Serialize(RootObject.ToSharedRef(), Writer);
	FFileHelper::SaveStringToFile(OutputString, *GlobalJsonPath);
}
