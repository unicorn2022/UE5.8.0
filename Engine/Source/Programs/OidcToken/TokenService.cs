...
namespace OidcToken
{
	...
	internal class ZenAddOidcTokenRequest
	{
		public string? ProviderName { get; set; }
		public string? RefreshToken { get; set; }
	}

	internal class TokenService : IHostedService
	{
		...
		public async Task Main()
		{
			...
			switch (Settings.CurrentValue.Mode)
			{
				...
				{
					await OutputStatus(providerName, await TokenManager.GetStatusForProvider(providerName));
					break;
				}
				case TokenServiceOptions.TokenServiceMode.GetToken:
				{
					OidcStatus status = await TokenManager.GetStatusForProvider(providerName);
					Logger.LogInformation("Determined status of provider {ProviderName} was {Status}", providerName, status);

					IOidcTokenInfo? tokenInfo;
					...
					if (Settings.CurrentValue.Zen)
					{
						...
						try
						{
							...
							ZenAddOidcTokenRequest request = new ZenAddOidcTokenRequest { ProviderName = providerName, RefreshToken = tokenInfo.RefreshToken };
		...
		private async Task OutputStatus(string service, OidcStatus status)
		{
			...
			else
			{
				...
				OidcStatus refreshedStatus = await TokenManager.GetStatusForProvider(service);
	...
	internal class UnableToAllocateTokenException : Exception
	{
	}

	internal class TokenServiceOptions: IValidatableObject
	{
