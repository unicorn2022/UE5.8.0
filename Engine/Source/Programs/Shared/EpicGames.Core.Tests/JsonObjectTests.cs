...
namespace EpicGames.Core.Tests
{
	...
	public class JsonObjectTests
	{
		...
		{
			if (Directory.Exists(s_tempDirectory.FullName))
			{
				Directory.Delete(s_tempDirectory.FullName, true);
			}
		}
		...
		public void ParseInvalidJsonText()
		{
			...
			Assert.Throws<JsonException>(() => JsonObject.Parse(_testJsonText));
		}

		[TestMethod]
		public void ParseEmptyText()
		{
			Assert.Throws<JsonException>(() => JsonObject.Parse(""));
		}
		...
		public void ReadInvalidJsonText()
		{
			...
			Assert.Throws<JsonException>(() => JsonObject.Read(inputFileReference));
		}
		...
		public void GetIntegerInvalidField()
		{
			...
			int correctValue = _validJsonDataSource.Hp;
			Assert.ThrowsExactly<JsonException>(() => _validTestObject.GetIntegerField(fieldName));
			int outValue;
		...
		public void GetStringInvalidField()
		{
			string fieldName = "Invalid";
			string correctValue = _validJsonDataSource.Name;
			Assert.ThrowsExactly<JsonException>(() => _validTestObject.GetStringField(fieldName));
			string? outValue;
		...
		{
			string fieldName = "Invalid";
			Assert.ThrowsExactly<JsonException>(() => _validTestObject.GetObjectField(fieldName));
			JsonObject? outValue;
		...
		public void GetObjectArrayValidField()
		{
			...
			JsonObject[] correctValue = [.. _validJsonDataSource.Attacks.Select(x => x.ToJsonObject())];
		...
		{
			string fieldName = "Invalid";
			Assert.ThrowsExactly<JsonException>(() => _validTestObject.GetObjectArrayField(fieldName));
			JsonObject[]? outValue;
		...
		public void GetStringArrayInvalidField()
		{
			string fieldName = "Invalid";
			string[] correctValue = [.. _validJsonDataSource.Skills];
			Assert.ThrowsExactly<JsonException>(() => _validTestObject.GetStringArrayField(fieldName));
			string[]? outValue;
		...
		public void GetEnumInvalidField()
		{
			string fieldName = "Invalid";
			StatusEffect correctValue = _validJsonDataSource.CurrentStatusEffect;
			Assert.ThrowsExactly<JsonException>(() => _validTestObject.GetEnumField<StatusEffect>(fieldName));
			StatusEffect outValue;
			Assert.IsFalse(_validTestObject.TryGetEnumField<StatusEffect>(fieldName, out outValue));
			Assert.AreEqual(default, outValue);
		}
		...
		{
			string fieldName = "Invalid";
			Assert.ThrowsExactly<JsonException>(() => _validTestObject.GetBoolField(fieldName));
			bool outValue = false;
		...
		public void SetObjectField()
		{
			string fieldName = "StrongestAttack";
			Assert.IsTrue(_validTestObject.ContainsField(fieldName));
			EnemyAttack setValue = new("Ultima", 300, StatusEffect.None);
		...
		public void SetObjectArrayField()
		{
			...
			Assert.IsTrue(_validTestObject.ContainsField(fieldName));
			JsonObject[] setValue = [.. attacks.Select(x => x.ToJsonObject())];
			_validTestObject.AddOrSetFieldValue(fieldName, setValue);
		...
		{
			string stringToMatch = new(_testJsonText.ToCharArray());
			string[] lines = stringToMatch.Split([Environment.NewLine], StringSplitOptions.None);
		...
		private static FileReference CreateTempJsonFile(string fileName, string? fileContent)
		{
			...
			FileReference inputFileReference = new(inputFile);
