...
namespace EpicGames.Horde
{
	...
	public static class HordeHttpRequest
	{
		...
		/// <summary>
		/// Patches a resource at an HTTP endpoint with a JSON object
		/// </summary>
		/// <typeparam name="TRequest">The object type to send</typeparam>
		/// <param name="httpClient">Http client instance</param>
		/// <param name="relativePath">The url to patch</param>
		/// <param name="request">The object to send</param>
		/// <param name="cancellationToken">Cancels the request</param>
		public static async Task PatchAsync<TRequest>(HttpClient httpClient, string relativePath, TRequest request, CancellationToken cancellationToken = default)
		{
			using HttpResponseMessage response = await httpClient.PatchAsJsonAsync(relativePath, request, s_jsonSerializerOptions, cancellationToken);
			response.EnsureSuccessStatusCode();
		}
	}
}
