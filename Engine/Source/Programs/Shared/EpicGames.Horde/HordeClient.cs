	...
	public abstract class HordeClient : IHordeClient, IAsyncDisposable
	{
		...
		async Task<GrpcChannel> CreateGrpcChannelInternalAsync(CancellationToken cancellationToken)
		{
			...
			if (useInsecureConnection)
			{
				...
				{
					UriBuilder builder = new(serverUri);
...
#pragma warning restore CA2000 // Dispose objects before losing scope
		...
		public HordeHttpClient CreateHttpClient()
			=> new(CreateAuthenticatedHttpClient());
		...
		public IStorageNamespace GetStorageNamespace(string basePath, string? accessToken = null)
		{
			...
			// Upload redirect clients talk directly to S3 (or similar) via presigned URLs.
			// Use a bare SocketsHttpHandler without the Polly retry/timeout policies that are
			// designed for Horde server communication. The single outer retry policy in
			// WriteBlobAsync provides adequate retry behavior for transient S3 errors.
			static HttpClient CreateUploadRedirectClient() => new(HttpMessageHandlers.CreateDefaultHandler());

			HttpStorageBackend httpStorageBackend = new(basePath, CreateClient, CreateUploadRedirectClient, _loggerFactory.CreateLogger<HttpStorageBackend>());
	...
	class HordeClientWithStaticCredentials : HordeClient
	{
		...
		public override async Task<bool> LoginAsync(bool allowLogin, CancellationToken cancellationToken)
		{
			...
			{
				using HttpRequestMessage request = new(HttpMethod.Get, "api/v1/dashboard/challenge");
		...
		{
			HttpClient httpClient = new(_httpMessageHandler.Instance, false);
	...
	{
		private const string AllowInteractiveLoginHeaderKey = "Horde-AllowInteractiveLogin";
		private static readonly HttpRequestOptionsKey<bool> s_allowInteractiveLogin = new(AllowInteractiveLoginHeaderKey);

		readonly OidcAuthState _authHandlerState;
		readonly IHordeHttpMessageHandler _baseHttpMessageHandler;
		private readonly ITokenStore _tokenStore;
		private readonly HordeAuthConfigProvider _hordeAuthConfigProvider;
			...
			: base(serverUrl, bundleCache, hordeOptions, loggerFactory)
		{
			...
			_tokenStore = TokenStoreFactory.CreateTokenStore(loggerFactory);
			_hordeAuthConfigProvider =
				new HordeAuthConfigProvider(_baseHttpMessageHandler.Instance, serverUrl, loggerFactory.CreateLogger<HordeAuthConfigProvider>());
			IOidcTokenManager oidcTokenManager = new HordeOidcTokenManager(_hordeAuthConfigProvider, _tokenStore, loggerFactory.CreateLogger<HordeOidcTokenManager>(), loggerFactory);
			IStaticAccessTokenProvider staticAccessTokenProvider = new HordeStaticAccessTokenProvider(hordeOptions);
			HttpOidcAuthHandlerConfig httpOidcAuthHandlerConfig = HordeOptionsToIHttpOidcAuthHandlerConfig(hordeOptions.Value);
			IClock clock = new DefaultClock();
			_authHandlerState = new OidcAuthState(oidcTokenManager, staticAccessTokenProvider, clock, _hordeAuthConfigProvider, loggerFactory.CreateLogger<OidcAuthState>());
			_authHttpMessageHandler = new HttpOidcAuthHandler(_baseHttpMessageHandler.Instance, _authHandlerState, httpOidcAuthHandlerConfig, loggerFactory.CreateLogger<HttpOidcAuthHandler>());
		...
		public override async ValueTask DisposeAsync()
		{
			...
			_hordeAuthConfigProvider.Dispose();
			_tokenStore.Dispose();
			await base.DisposeAsync();
		}
		...
		public override async Task<bool> LoginAsync(bool allowLogin, CancellationToken cancellationToken)
		{
			...
			{
				using HttpRequestMessage request = new(HttpMethod.Get, "api/v1/dashboard/challenge");
				request.Options.Set(s_allowInteractiveLogin, allowLogin);
		...
		protected override HttpClient CreateAuthenticatedHttpClient()
		{
			HttpClient httpClient = new(_authHttpMessageHandler, false);
			httpClient.BaseAddress = ServerUrl;
			return httpClient;
		}
		...
		protected override HttpClient CreateUnauthenticatedHttpClient()
		{
			HttpClient httpClient = new(_baseHttpMessageHandler.Instance, false);
			httpClient.BaseAddress = ServerUrl;
			return httpClient;
		}

		private static HttpOidcAuthHandlerConfig HordeOptionsToIHttpOidcAuthHandlerConfig(HordeOptions options) =>
			new(AllowInteractiveLoginHeaderKey, options.AllowAuthPrompt);
	}
