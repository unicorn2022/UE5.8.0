...
namespace EpicGames.Horde.Logs
{
	...
	public class ServerLogPacketBuilder
	{
		...
		static readonly Utf8String s_escapedNewline = new("\\n");

		readonly List<JsonLogEvent> _logEvents = [];
		...
		public int SanitizeAndWriteEvent(JsonLogEvent jsonLogEvent)
		{
			...
			{
				StringBuilder escapedLineBuilder = new();
				...
				KeyValuePair<string, object?>[] properties = [new KeyValuePair<string, object?>("Text", escapedLine)];
				LogEvent newLogEvent = new(DateTime.UtcNow, LogLevel.Error, default, $"Invalid json log event: {escapedLineBuilder}", "Invalid json log event: {Text}", properties, LogException.FromException(ex));
				JsonLogEvent newJsonLogEvent = new(newLogEvent);
		...
		int WriteEventWithFormat(JsonLogEvent jsonLogEvent, JsonObject obj, string format)
		{
			...
			IEnumerable<KeyValuePair<string, object?>> propertyValueList = [];
		...
		void WriteSingleEvent(JsonLogEvent jsonLogEvent, JsonObject obj, string message, string? format, int line, int lineCount)
		{
			...
			_lineWriter.Clear();
			using (Utf8JsonWriter jsonWriter = new(_lineWriter))
			{
				obj.WriteTo(jsonWriter);
			}

			JsonLogEvent nextEvent = new(jsonLogEvent.Level, jsonLogEvent.EventId, line, lineCount, _lineWriter.WrittenMemory.ToArray());
		...
		void WriteEventInternal(JsonLogEvent jsonLogEvent)
		{
			if (jsonLogEvent.Data.Length > MaxLineLength)
			{
				...
				logEvent = new LogEvent(logEvent.Time, logEvent.Frame, logEvent.Level, logEvent.Id, logEvent.LineIndex, logEvent.LineCount, $"{message} [...]", null, null, logEvent.Exception);
