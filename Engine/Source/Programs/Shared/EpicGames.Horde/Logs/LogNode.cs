...
namespace EpicGames.Horde.Logs
{
	...
	public class LogBuilder
	{
		...
		public async Task<IHashedBlobRef<LogNode>> FlushAsync(IBlobWriter writer, bool complete, CancellationToken cancellationToken)
		{
			...
			{
				_textBuilder.Flush();
				writeTextChunks = [.. _textBuilder.Chunks];

				_indexTextBuilder.Flush();
				writeIndexTextChunks = [.. _indexTextBuilder.Chunks];
			}
			...
			List<LogChunkRef> newJsonChunkRefs = [.. _root?.TextChunkRefs ?? []];
			...
			LogNode newRoot = new(_format, lineCount, length, newJsonChunkRefs, newIndexRef, complete);
