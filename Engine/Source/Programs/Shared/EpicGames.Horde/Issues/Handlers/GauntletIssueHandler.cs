...
namespace EpicGames.Horde.Issues.Handlers
{
	...
	public class GauntletIssueHandler : IssueHandler
	{
		...
		/// </summary>
		static readonly Dictionary<EventId, string> s_knownGauntletEvents = new()
		{
		...
		/// </summary>
		static readonly HashSet<EventId> s_knownSevereGauntletEvents =
		[
		...
		/// </summary>
		static readonly HashSet<EventId> s_knownTestGauntletEvents =
		[
		...
		/// </summary>
		static readonly HashSet<EventId> s_skipGauntletEvents =
		[
		...
		private void GetHash(IssueEvent issueEvent, HashSet<IssueKey> keys, HashSet<IssueMetadata> metadata, bool hasCallstack)
		{
			...
			else
			{
				...
				issueEvent.AuditLogger?.LogDebug("Fingerprint key: '{Key}' generated from event: '{Event}'", $"{_context.NodeName}", issueEvent.Render());
			}
		...
		private static JsonProperty? FindNestedPropertyOfType(JsonLogEvent logEvent, string searchType)
		{
			...
			if (line.TryGetProperty("properties", out properties) && properties.ValueKind == JsonValueKind.Object)
			{
				...
				{
					if (property.Name.StartsWith(searchType, StringComparison.OrdinalIgnoreCase))
					{
		...
		public override bool HandleEvent(IssueEvent issueEvent)
		{
			if (issueEvent.EventId != null && IsMatchingEventId(issueEvent.EventId.Value))
			{
				...
				bool hasCallstack = EventHasCallstackProperty(issueEvent);

				if (issueEvent.Severity == LogLevel.Warning && !hasCallstack)
				{
					// Do not generate a flood of Build Health issues for smallset of known Gauntlet log event warnings
					// If we identify cases where we want these warnings, they should be defined as new log events, and force their warning in the issue handler, here
					// Though, we have been unable to come up with a single case in exploration					
					return true;
				}
				...
				if (hasCallstack)
				{
					string rendered = issueEvent.Render();
					if (issueEvent.Render().Contains("Ensure condition failed", StringComparison.OrdinalIgnoreCase))
					{
						isEnsure = true;
					}
				}

				string callstack = isEnsure ? "ensure callstack" : "callstack";

				string summaryTemplate = hasCallstack ? "Automation {Meta:GauntletType} " + callstack + " {Severity} in {Meta:Node}" : "Automation {Meta:GauntletType} {Severity} in {Meta:Node}";

				string gauntletType = GetEventPrefix(issueEvent.EventId!.Value);
				IssueEventGroup issue = new($"Gauntlet:{gauntletType}", summaryTemplate, IssueChangeFilter.All);
				issue.Events.Add(issueEvent);
				GetHash(issueEvent, issue.Keys, issue.Metadata, hasCallstack);
