...
namespace EpicGames.Horde.Compute
{
	...
	internal abstract class ComputeBufferDetail : IDisposable
	{
		...
		protected internal readonly unsafe struct ChunkStatePtr
		{
			...
			public ChunkState Get() => new(Interlocked.CompareExchange(ref *_data, 0, 0));
		...
		protected internal readonly unsafe struct ReaderStatePtr
		{
			...
			// Current value of the chunk state
			public ReaderState Get() => new(Interlocked.CompareExchange(ref *_data, 0, 0));
		...
		protected internal readonly unsafe struct WriterStatePtr
		{
			...
			public WriterState Get() => new(Interlocked.CompareExchange(ref *_data, 0, 0));
		...
		protected internal readonly unsafe struct HeaderPtr
		{
			...
			public WriterStatePtr GetWriterStatePtr() => new(_data + 1);

			public ReaderStatePtr GetReaderStatePtr(int readerIdx) => new(_data + 2 + readerIdx);

			public ChunkStatePtr GetChunkStatePtr(int chunkIdx) => new(_data + 2 + ComputeBuffer.MaxReaders + chunkIdx);
		}
