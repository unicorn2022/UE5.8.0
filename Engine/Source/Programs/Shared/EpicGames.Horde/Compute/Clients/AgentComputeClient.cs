...
namespace EpicGames.Horde.Compute.Clients
{
	...
	public sealed class AgentComputeClient : IComputeClient
	{
		...
		{
			using Socket listener = new(SocketType.Stream, ProtocolType.IP);
		...
		static async Task RunAgentAsync(string hordeAgentAssembly, int port, ILogger logger, CancellationToken cancellationToken)
		{
			...
			{
				List<string> arguments = [hordeAgentAssembly, "computeworker", $"-port={port}"];

				using ManagedProcess process = new(group, "dotnet", CommandLineArguments.Join(arguments), null, null, ProcessPriorityClass.Normal);
