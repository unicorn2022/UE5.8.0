...
#pragma warning restore CA1508
		...
		{
			using AgentStorageBackend innerStore = new(channel);
			await using BundleCache cache = new(new BundleCacheOptions { HeaderCacheSize = 10 * 1024 * 1024, PacketCacheSize = 128 * 1024 * 1024 });

			BundleOptions bundleOptions = ComputeProtocolUtilities.GetBundleOptions(channel.Protocol);
			BundleStorageNamespace store = new(innerStore, cache, bundleOptions, _logger);
		...
		{
			FileFilter filter = new(deleteFiles);
		...
		{
			Dictionary<string, string?> newEnvVars = new(_envVars);
			if (envVars != null)
			{
		...
		internal static async Task ProcessIpcMessagesAsync(ComputeSocket socket, ComputeBufferReader ipcReader, CancellationToken[] cancellationTokens, ILogger logger)
		{
			...
			List<SharedMemoryBuffer> buffers = [];
			try
			{
				List<(int, ComputeBufferWriter)> writers = [];
		...
		class ProcessOutputWriter
		{
			...
			readonly ByteArrayBuilder _lineBuffer = new();
			readonly ILogger _logger;
		...
		{
			List<(string, string?)> prevEnvVars = [];
			if (envVars != null)
			{
			...
			try
			{
				...
				string[] mainArgs = [.. arguments.Skip(1)];

				_logger.LogWarning("Note: Loading and running {Assembly} in process", assemblyPath);

				TaskCompletionSource<int> resultTcs = new(TaskCreationOptions.RunContinuationsAsynchronously);

				Thread thread = new(() => resultTcs.SetResult(AppDomain.CurrentDomain.ExecuteAssembly(assemblyPath, mainArgs)));
				thread.Start();
		...
		async Task ExecuteProcessInternalAsync(AgentMessageChannel channel, string executable, IReadOnlyList<string> arguments, string? workingDir, IReadOnlyDictionary<string, string?>? envVars, ExecuteProcessFlags flags, CancellationToken cancellationToken)
		{
			...
			else
			{
				...
				if (flags.HasFlag(ExecuteProcessFlags.UseWine) && _wineExecutablePath != null)
				{
					...
					resolvedCommandLine = CommandLineArguments.Join([resolvedExecutable, .. arguments]);
				...
				using ManagedProcessGroup group = new();
				using ManagedProcess process = new(group, resolvedExecutable, resolvedCommandLine, resolvedWorkingDir, resolvedEnvVars, null, ProcessPriorityClass.Normal);
				byte[] buffer = new byte[1024];

				ProcessOutputWriter outputWriter = new($"{Path.GetFileNameWithoutExtension(resolvedExecutable)}> ", _logger);
				for (; ; )
				{
