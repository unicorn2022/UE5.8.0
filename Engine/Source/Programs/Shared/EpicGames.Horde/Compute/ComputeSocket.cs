...
namespace EpicGames.Horde.Compute
{
	...
	public sealed class WorkerComputeSocket : ComputeSocket, IDisposable
	{
		...
		readonly List<ComputeBuffer> _buffers = [];
		...
		{
			MemoryWriter writer = new(_commandBufferWriter.GetWriteBuffer());
	...
	public class RemoteComputeSocket : ComputeSocket, IAsyncDisposable
	{
		...
		readonly Dictionary<int, RecvBuffer> _recvBuffers = [];

		readonly SemaphoreSlim _sendSemaphore = new(1, 1);
		readonly Dictionary<int, SendBuffer> _sendBuffers = [];
		...
		readonly SendSegment _headerSegment = new();
		readonly SendSegment _bodySegment = new();
		...
		async ValueTask SendInternalAsync(int id, int size, ReadOnlyMemory<byte> memory, CancellationToken cancellationToken)
		{
			...
			try
			{
				...
				ReadOnlySequence<byte> sequence = new(_headerSegment, 0, _bodySegment, memory.Length);
		...
		async Task DetachAllBuffersAsync(bool recvBuffers, bool sendBuffers, CancellationToken cancellationToken)
		{
			...
			{
				_complete = true;
				recvChannelIds = [.. _recvBuffers.Keys];
				sendChannelIds = [.. _sendBuffers.Keys];
			}
		...
		public override void AttachSendBuffer(int channelId, ComputeBuffer sendBuffer)
		{
			...
			lock (_lockObject)
			{
				...
				SendBuffer sendBufferInfo = new(sendBufferReader, (buffer, ctx) => SendFromBufferAsync(channelId, buffer, ctx));
