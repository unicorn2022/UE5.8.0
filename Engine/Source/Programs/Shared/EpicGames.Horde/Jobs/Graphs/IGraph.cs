...
namespace EpicGames.Horde.Jobs.Graphs
{
	...
	public static class GraphExtensions
	{
		...
		{
			return Enumerable.Concat(node.InputDependencies, node.OrderDependencies).Concat(node.OptionalInputDependencies).Select(x => graph.GetNode(x));
		}
	}
	...
	public interface INode
	{
		...
		/// <summary>
		/// References to optional inputs for this node
		/// </summary>
		public IReadOnlyList<NodeOutputRef> OptionalInputs { get; }
		...
		/// </summary>
		public NodeRef[] OptionalInputDependencies { get; }
	...
	public class NewNode
	{
		...
		/// <summary>
		/// List of nodes which may have succeeded when this node is run
		/// </summary>
		...
		/// <param name="optionalInputs">List of optional inputs for the node</param>
		/// <param name="outputs">List of output names for the node</param>
		/// <param name="inputDependencies">List of nodes which must have completed successfully for this node to run</param>
		/// <param name="optionalInputDependencies">List of nodes which could have completed successfully for this node to run</param>
		...
		public NewNode(string name, List<string>? inputs = null, List<string>? optionalInputs = null, List<string>? outputs = null, List<string>? inputDependencies = null, List<string>? optionalInputDependencies = null, List<string>? orderDependencies = null, Priority? priority = null, bool? allowRetry = null, bool? runEarly = null, bool? warnings = null, Dictionary<string, string>? credentials = null, Dictionary<string, string>? properties = null, IReadOnlyNodeAnnotations? annotations = null)
		{
			...
			OptionalInputDependencies = optionalInputDependencies;
		...
		public NewNode(IGraph graph, INode node)
			: this(node.Name, [.. node.Inputs.Select(x => graph.GetNode(x.NodeRef).Name)], [.. node.OptionalInputs.Select(x => graph.GetNode(x.NodeRef).Name)], [.. node.OutputNames], [.. node.InputDependencies.Select(x => graph.GetNode(x).Name)], [.. node.OptionalInputDependencies.Select(x => graph.GetNode(x).Name)], [.. node.OrderDependencies.Select(x => graph.GetNode(x).Name)], node.Priority, node.AllowRetry, node.RunEarly, node.Warnings, node.Credentials?.ToDictionary(x => x.Key, x => x.Value), node.Properties?.ToDictionary(x => x.Key, x => x.Value), node.Annotations)
		{
		}
	}
	...
	public class NewGroup
	{
		...
		public NewGroup(IGraph graph, INodeGroup group)
			: this(group.AgentType, [.. group.Nodes.Select(x => new NewNode(graph, x))])
		{
		}
	}
