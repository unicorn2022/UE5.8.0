...
{
	/// <summary>
	/// Sort order for commit queries
	/// </summary>
	public enum CommitSortOrder
	{
		/// <summary>
		/// Newest commits first (default)
		/// </summary>
		Descending,

		/// <summary>
		/// Oldest commits first
		/// </summary>
		Ascending
	}
	...
	public interface ICommitCollection
	{
		...
		/// <summary>
		/// Finds changes submitted to a stream.
		/// </summary>
		...
		/// <param name="sortOrder">Sort order for results (Descending = newest first, Ascending = oldest first)</param>
		/// <param name="cancellationToken">Cancellation token for the operation</param>
		/// <returns>Changelist information</returns>
		IAsyncEnumerable<ICommit> FindAsync(CommitId? minCommitId = null, bool includeMinCommit = true, CommitId? maxCommitId = null, bool includeMaxCommit = true, int? maxResults = null, IReadOnlyList<CommitTag>? tags = null, CommitSortOrder sortOrder = CommitSortOrder.Descending, CancellationToken cancellationToken = default);
	...
	public static class CommitCollectionExtensions
	{
		...
		{
			return await commitCollection.FindAsync(minCommitId: null, maxCommitId: maxCommitId, maxResults: 1, tags: [CommitTag.Code], cancellationToken: cancellationToken).FirstOrDefaultAsync(cancellationToken);
		}
