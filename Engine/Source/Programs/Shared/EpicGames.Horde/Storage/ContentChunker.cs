...
namespace EpicGames.Horde.Storage
{
	...
	public class FileChunkerSource : ChunkerSource
	{
		...
		async Task HandleReadAsync(long offset, Memory<byte> memory, CancellationToken cancellationToken)
		{
			...
			FileStreamOptions options = new() { Mode = FileMode.Open, Access = FileAccess.Read, Options = FileOptions.Asynchronous };
	...
	public class SerialBuzHashChunker : ContentChunker
	{
		...
		readonly Queue<Output> _order = new();
	...
	public class ParallelBuzHashChunker : ContentChunker
	{
		...
		class RingBuffer
		{
			...
			readonly AsyncEvent _freeEvent = new();
		...
		{
			InputRequest inputRequest = new(input, new TaskCompletionSource<Block>(TaskCreationOptions.RunContinuationsAsynchronously));
			...
			while (await reader.WaitToReadAsync(cancellationToken))
			{
				while (reader.TryRead(out InputRequest? inputRequest))
				{
					...
					for (long sourceOffset = 0; blockTcs != null;)
					{
						...
						BlockRequest blockRequest = new(bufferOffset, readLength, readTask, blockTcs, nextBlockTcs?.Task);
		...
		async Task ProcessBlockRequestsAsync(CancellationToken cancellationToken)
		{
			...
			while (await reader.WaitToReadAsync(cancellationToken))
			{
				while (reader.TryRead(out BlockRequest? blockRequest))
				{
					...
					// Create the block
					Block block = new(blockRequest.BufferOffset, blockRequest.Length, boundaries, blockRequest.NextBlock);
