...
namespace EpicGames.Horde.Storage.Bundles.V2
{
	...
	public sealed class PacketWriter : IDisposable
	{
		...
		readonly List<BlobType> _types = [];
		readonly List<PacketImport> _imports = [];
		readonly List<object> _importHandles = [];
		readonly Dictionary<object, int> _importMap = [];
		readonly List<int> _exportOffsets = [];
		readonly HashSet<int> _bundleImportIndices = [];
		...
		/// Constructor
		/// </summary>
		public PacketWriter(BundleHandle bundleHandle, PacketHandle packetHandle, IMemoryAllocator<byte> allocator, Lock lockObject)
		{
			...
			BundleSignature signature = new(BundleVersion.LatestV2, 0);
		...
		public BlobData GetExport(int exportIdx)
		{
			lock (_lockObject)
			{
				...
				PacketExport export = new(_buffer.Slice(offset, length));
		...
		{
			PacketImport import = new(baseIdx, fragment);
