...
namespace EpicGames.Horde.Storage.Bundles.V2
{
	...
	public sealed class BundleWriter : BlobWriter
	{
		...
		internal sealed class PendingBundleHandle : BundleHandle, IDisposable
		{
			...
			readonly HashSet<BlobLocator> _bundleImports = [];
			...
			public HashedExportHandle CompleteExport(BlobType type, int size, IReadOnlyList<IBlobRef> imports, IReadOnlyList<AliasInfo> aliases)
			{
				...
				HashedExportHandle exportHandle = new(hash, _packetHandle, exportIdx);

				if (aliases.Count > 0)
				{
					_pendingExportAliases ??= [];
			...
			void FinishPacket()
			{
				...
				if (_packetWriter.GetExportCount() > 0)
				{
					...
					lock (LockObject)
					{
						FlushedPacketHandle flushedPacketHandle = new(_storageNamespace, this, packetOffset, packetLength, _bundleCache);
			...
			public async Task WriteAsync(CancellationToken cancellationToken = default)
			{
				...
				using (ReadOnlySequenceStream stream = new(_encodedPacketWriter.AsSequence()))
				{
			...
			public override async ValueTask<IReadOnlyMemoryOwner<byte>> ReadAsync(int offset, int? length, CancellationToken cancellationToken = default)
			{
				if (_flushedHandle == null)
				{
					lock (LockObject)
					{
#pragma warning disable CA1508 // Avoid dead conditional code - need to re-check after locking.
						if (_flushedHandle == null)
#pragma warning restore CA1508
		...
		BlobLocator GetNextLocator()
			=> new($"{_locatorPrefix}_{Interlocked.Increment(ref _locatorSuffix)}");
		...
		/// </summary>
		static void RuntimeAssert([DoesNotReturnIf(false)] bool condition, [CallerArgumentExpression(nameof(condition))] string? message = null)
		{
			if (!condition)
			{
