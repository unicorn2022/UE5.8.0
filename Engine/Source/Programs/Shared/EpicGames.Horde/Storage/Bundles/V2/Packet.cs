...
namespace EpicGames.Horde.Storage.Bundles.V2
{
	...
	public sealed class Packet
	{
		...
		class DebugProxy
		{
			...
			{
				Types = [.. Enumerable.Range(0, packet.GetTypeCount()).Select(packet.GetType)];
				Imports = [.. Enumerable.Range(0, packet.GetImportCount()).Select(packet.GetImport)];
				Exports = [.. Enumerable.Range(0, packet.GetExportCount()).Select(packet.GetExport)];
			}
		}
		...
		public void Encode(BundleCompressionFormat format, IMemoryWriter writer)
		{
			...
			BundleSignature signature = new(BundleVersion.LatestV2, BundleSignature.NumBytes + encodedLength + sizeof(int) + 1);
	...
	public readonly record struct PacketImport(int BaseIdx, Utf8String Fragment)
	{
		...
		public static PacketImport Read(ReadOnlyMemory<byte> data)
		{
			...
			Utf8String fragment = new(data.Slice(bytesRead));
