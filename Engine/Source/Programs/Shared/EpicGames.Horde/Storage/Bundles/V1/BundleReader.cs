...
namespace EpicGames.Horde.Storage.Bundles.V1
{
	...
	public class BundleReader
	{
		...
		class QueuedBundle
		{
			...
			public List<QueuedPacket> QueuedPackets { get; } = []; // Sorted by index
			...
			{
				QueuedPacket packet = new(packetIdx);
		...
		class QueuedPacket : IComparable<QueuedPacket>
		{
			...
			public readonly TaskCompletionSource<ReadOnlyMemory<byte>> CompletionSource = new(TaskCreationOptions.RunContinuationsAsynchronously);
		...
		readonly List<QueuedBundle> _queuedBundles = [];
		readonly Dictionary<string, Task<ReadOnlyMemory<byte>>> _decodeTasks = new(StringComparer.Ordinal);
		...
		async Task<BundleInfo> ReadBundleInfoAsync(QueuedBundle bundle, CancellationToken cancellationToken)
		{
			...
			for (; ; )
			{
				await using (Stream stream = await _store.Backend.OpenBlobAsync(bundle.Locator, 0, prefetchSize, cancellationToken))
				{
					...
					BundleInfo bundleInfo = new(bundle.Locator, header, signature.HeaderLength);
		...
		public async ValueTask<BlobData> ReadNodeDataAsync(BlobLocator bundleLocator, int exportIdx, CancellationToken cancellationToken = default)
		{
			...
			Dictionary<BlobLocator, BundleHandle> locatorToBundleHandle = [];

			List<IBlobRef> imports = new(export.References.Count);
