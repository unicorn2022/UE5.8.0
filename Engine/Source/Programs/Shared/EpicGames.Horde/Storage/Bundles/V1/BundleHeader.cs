...
namespace EpicGames.Horde.Storage.Bundles.V1
{
	...
	public class BundleHeader
	{
		...
		public void AppendTo(ReadOnlySequenceBuilder<byte> builder)
		{
			...
			ChunkedArrayMemoryWriter writer = new();
			...
			BundleSignature signature = new(BundleVersion.LatestV1, writer.Length);
...
#pragma warning disable VSTHRD002
		...
		{
			using ReadOnlyMemoryStream stream = new(memory);
...
#pragma warning restore VSTHRD002
		...
		public static async Task<BundleHeader> ReadAsync(BundleSignature signature, Stream stream, CancellationToken cancellationToken = default)
		{
			...
			{
				return new BundleHeader(new BundleTypeCollection(), new BundleImportCollection(), new BundleExportCollection(), new BundlePacketCollection());
			}
		...
		{
			byte[] exportHeaders = [];
			byte[] exportReferences = [];

			BundleTypeCollection types = new();
			BundleImportCollection imports = new();
			BundleExportCollection exports = new();
			BundlePacketCollection packets = new();
		...
		{
			MemoryReader reader = new(memory);
			...
			List<BlobType> types = new(numTypes);
			...
			List<BlobLocator> imports = new(numImports);
			List<BundleExportRef> allExportReferences = [];

			for (int importIdx = 0; importIdx < numImports; importIdx++)
			{
				BlobLocator locator = new(reader.ReadUtf8String());
				imports.Add(locator);
				...
				{
					BundleExportRef exportReference = new(imports.Count - 1, exportIndexes[exportIdx]);
			...
			List<ExportInfo> exportInfos = new(numExports);
	...
	public readonly struct BundleTypeCollection : IReadOnlyList<BlobType>
	{
		...
		/// Constructor
		/// </summary>
		public BundleTypeCollection() : this([]) { }
	...
	public readonly struct BundleImportCollection : IReadOnlyList<BlobLocator>
	{
		...
		/// Constructor
		/// </summary>
		public BundleImportCollection() : this([]) { }
	...
	public readonly struct BundlePacketCollection : IReadOnlyCollection<BundlePacket>
	{
		...
		/// Constructor
		/// </summary>
		public BundlePacketCollection() : this([]) { }
	...
	public readonly struct BundleExport
	{
		...
		/// </summary>
		public IoHash Hash => new(Data.Slice(0, 20).Span);
		...
		/// </summary>
		public BundleExportRefCollection References => new(Data.Slice(32));
	...
	public readonly struct BundleExportCollection : IReadOnlyList<BundleExport>
	{
		...
		/// Constructor
		/// </summary>
		public BundleExportCollection() : this([]) { }
		...
		public static BundleExportCollection Read(ReadOnlyMemory<byte> data, ReadOnlyMemory<byte> refs, BundleVersion version)
		{
			...
			for (int index = 0; index < count; index++)
			{
				...
				BundleExportRefCollection exportRefs = new();
				if (refs.Length > 0)
				{
