...
namespace EpicGames.Horde.Storage.Bundles.V1
{
	...
	sealed class FlushedNodeHandle : IBlobRef
	{
		...
		/// <inheritdoc/>
		public ValueTask FlushAsync(CancellationToken cancellationToken = default) => new();
	...
	public sealed class BundleWriter : BlobWriter
	{
		...
		internal class PendingNode : IHashedBlobRef
		{
			...
			public PendingNode(BundleReader reader, BlobType blobType, IoHash hash, int packet, int offset, int length, IReadOnlyList<IBlobRef> imports, IReadOnlyList<AliasInfo> aliases, PendingBundle pendingBundle)
			{
				...
				Offset = offset;
				Length = length;
				Imports = [.. imports.Select(x => x.Innermost)];
				Aliases = [.. aliases];
		...
		internal class PendingBundle : IDisposable
		{
			...
			readonly HashSet<Task> _dependencies = [];
			...
			readonly TaskCompletionSource<bool> _completeEvent = new(TaskCreationOptions.RunContinuationsAsynchronously);
			...
			public PendingNode WriteNode(BlobType blobType, int size, IReadOnlyList<IBlobRef> refs, IReadOnlyList<AliasInfo> aliases)
			{
				...
				PendingNode pendingNode = new(_treeReader, blobType, hash, _currentPacketIdx, _currentPacketLength, (int)size, refs, aliases, this);
			...
			void CompressPacket(IMemoryOwner<byte> packetData, int length)
			{
				lock (_lockObject)
				{
					...
					BundlePacket packet = new(_compressionFormat, encodedOffset, encodedLength, length);
					_packets.Add(packet);
			...
			public async Task WriteAsync(BundleStorageNamespace store, string? basePath, ILogger? traceLogger)
			{
				...
				try
				{
					...
					ReadOnlySequenceBuilder<byte> sequence = new();
					...
					// Write it
					BlobLocator locator;
					using (ReadOnlySequenceStream stream = new(sequence.Construct()))
					{
					...
					FlushedBundleHandle handle = new(store, locator);
					for (int idx = 0; idx < _queue.Count; idx++)
					{
						PendingNode node = _queue[idx];

						FlushedNodeHandle flushedHandle = new(_treeReader, locator, handle, idx);
			...
			(BundleHeader, List<ReadOnlyMemory<byte>>) CreateBundleInternal()
			{
				...
				List<BlobLocator> imports = [];
				Dictionary<BlobLocator, int> importToIndex = [];

				// List of types in the bundle
				List<BlobType> types = [];
				Dictionary<BlobType, int> typeToIndex = [];

				// Map of node handle to reference
				Dictionary<IBlobRef, BundleExportRef> nodeHandleToExportRef = [];
				...
				foreach (PendingNode nodeInfo in _queue)
				{
					...
					BundleExport export = new(typeIdx, nodeInfo.Packet, nodeInfo.Offset, nodeInfo.Length, exportRefs);
					exports.Add(export);
				}

				// Get the memory for each packet
				List<ReadOnlyMemory<byte>> packetData = new(_packets.Count);
				...
				// Create the bundle
				BundleHeader header = new([.. types], [.. imports], [.. exports], [.. _packets]);
		...
		class WriteQueue
		{
			...
			readonly AsyncEvent _completeEvent = new();
			int _refCount;
			readonly List<Task> _writeTasks = [];
		...
		static readonly BundleOptions s_defaultOptions = new();
