...
namespace EpicGames.Horde.Storage.Bundles
{
	...
	public sealed class BundleStorageNamespace : IStorageNamespace
	{
		...
		readonly PacketReaderStats _packetReaderStats = new();
		...
		{
			FileStorageBackend backend = new(new FileObjectStore(rootDir, memoryMappedFileCache), logger);
		...
		{
			HashSet<BlobLocator> locators = [];
			while (data.Length > 0)
			{
				BundleSignature signature = BundleSignature.Read(data.Span);

				using IRefCountedHandle<Bundles.V2.Packet> packet = Packet.Decode(data, Cache.Allocator, nameof(ReadImportsFromDataV2));
		...
		public IBlobRef CreateBlobRef(BlobLocator locator)
		{
			...
			FlushedBundleHandle bundleHandle = new(this, baseLocator);
