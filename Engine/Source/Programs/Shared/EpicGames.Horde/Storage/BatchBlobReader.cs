...
namespace EpicGames.Horde.Storage
{
		...
		where TRequest : BlobReadRequest
	{
		...
		public BatchReaderStats GetStats()
			=> new(_numRequests, _numReads, _numBundles, _numPackets, _bytesRead, _bytesDecoded);
...
#pragma warning restore CA2000
		...
		{
			int queueLength = 0;
			Queue<BundleHandle> bundleQueue = new();
			Dictionary<BundleHandle, List<PacketExport>> bundleHandleToExportBatch = [];
		...
		async Task WriteBundleRequestsAsync(BundleHandle bundleHandle, List<PacketExport> exports, CancellationToken cancellationToken)
		{
			...
			List<PacketRequest> packetRequests = [];
			foreach (IGrouping<int, PacketExport> group in exports.GroupBy(x => x.PacketHandle.PacketOffset))
			{
				List<ExportRequest> exportRequests = [.. group.Select(x => new ExportRequest(x.ExportHash, x.ExportHandle, x.OriginalRequest))];
				PacketRequest packetRequest = new(group.First().PacketHandle, exportRequests);
			...
			for (int maxIdx = 0; maxIdx < packetRequests.Count; maxIdx++)
			{
				...
				BundleRequest request = new(bundleHandle, minOffset, maxOffset, packetRequests.GetRange(minIdx, (maxIdx + 1) - minIdx));
...
#pragma warning disable CA2000
					BundleResponse response = new(data, request.MinOffset, request.MaxOffset, request.Packets);
...
#pragma warning disable CA2000
				List<BlobReadResponse<TRequest>> responses = [];
				try
				{
