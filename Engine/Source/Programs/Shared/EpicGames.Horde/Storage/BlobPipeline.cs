...
namespace EpicGames.Horde.Storage
{
	...
	public sealed class BlobPipeline<TUserData> : IAsyncDisposable
	{
		...
		{
			int queueLength = 0;
			Queue<BundleBatchRequest> bundleBatchQueue = new();
			Dictionary<BundleHandle, BundleBatchRequest> bundleHandleToBatch = [];
			...
			for (; ; )
			{
				...
				for (; ; )
				{
					...
					if (_requestChannel.Reader.TryRead(out request))
					{
						...
						{
							await _requestBatchChannel.Writer.WriteAsync([request], cancellationToken);
						}
					}
					else
					{
				...
				BlobRequest<TUserData>[] chunkBatch = [.. exportBatch.Requests.OrderBy(x => x.PacketOffset).ThenBy(x => x.ExportIdx).Select(x => x.OriginalRequest)];
		...
		static bool TryAddBundleRequest(BlobRequest<TUserData> request, Queue<BundleBatchRequest> bundleBatchQueue, Dictionary<BundleHandle, BundleBatchRequest> bundleHandleToBatch)
		{
			...
			{
				bundleBatchRequest = new BundleBatchRequest(packetHandle.Bundle, []);
				bundleBatchQueue.Enqueue(bundleBatchRequest);
				bundleHandleToBatch.Add(packetHandle.Bundle, bundleBatchRequest);
			}

			BundleRequest bundleRequest = new(packetHandle.Bundle, packetHandle.PacketOffset, exportHandle.ExportIdx, request);
			...
			while (await _requestBatchChannel.Reader.WaitToReadAsync(cancellationToken))
			{
				...
				if (_requestBatchChannel.Reader.TryRead(out requestBatch))
				{
					...
					for (int idx = 0; idx < requestBatch.Length; idx++)
					{
						...
						BlobResponse<TUserData> response = new(blobData, request.UserData);
