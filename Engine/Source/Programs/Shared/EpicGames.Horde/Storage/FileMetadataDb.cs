...
namespace EpicGames.Horde.Storage
{
	...
	public sealed class FileMetadataDb : IDisposable
	{
		...
		static readonly IoHashTypeMapper s_hashTypeMapper = new();
		...
		{
			FileRow row = new(directoryId, name, time, length);
		...
		{
			ChunkRow row = new(fileId, offset, length, hash);
		...
		{
			DirectoryRow row = new() { ParentDirectoryId = parentDirectoryId, Name = name };
