...
namespace EpicGames.Horde.Storage.Nodes
{
	...
	public class DirectoryNode
	{
		...
		readonly SortedDictionary<string, FileEntry> _nameToFileEntry = new(StringComparer.Ordinal);
		readonly SortedDictionary<string, DirectoryEntry> _nameToDirectoryEntry = new(StringComparer.Ordinal);
		...
		{
			FileEntry entry = new(name, flags, data);
			AddFile(entry);
			return entry;
		}
	...
	class DirectoryNodeConverter : BlobConverter<DirectoryNode>
	{
		...
		{
			DirectoryNode directoryNode = new((DirectoryFlags)reader.ReadUnsignedVarInt());
			...
			for (int idx = 0; idx < fileCount; idx++)
			{
				...
				ChunkedDataNodeRef target = new(targetType, length, rollingHash, targetHandle);
