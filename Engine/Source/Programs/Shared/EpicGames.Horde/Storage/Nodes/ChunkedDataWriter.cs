...
namespace EpicGames.Horde.Storage.Nodes
{
	...
	public sealed class LeafChunkedDataWriter : IDisposable
	{
		...
		// Tree state
		long _totalLength;
		readonly List<ChunkedDataNodeRef> _leafHandles = [];
		...
		public async Task<LeafChunkedData> CompleteAsync(CancellationToken cancellationToken)
		{
			...
			await FlushLeafNodeAsync(cancellationToken);
			LeafChunkedData leafChunkedData = new(hash, [.. _leafHandles]);
	...
	{
		static readonly ChunkingOptions s_defaultChunkingOptions = new();
		...
		{
			using ChunkedDataWriter chunkedWriter = new(blobWriter, chunkingOptions);
