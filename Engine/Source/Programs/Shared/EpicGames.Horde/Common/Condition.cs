...
namespace EpicGames.Horde.Common
{
	...
	class TokenReader
	{
		...
		public StringView Token => new(Input, Offset, Length);
		...
		static readonly Dictionary<StringView, ulong> s_sizeSuffixes = new(StringViewComparer.OrdinalIgnoreCase)
		{
			["kb"] = 1024UL,
			["Mb"] = 1024UL * 1024,
			["Gb"] = 1024UL * 1024 * 1024,
		...
		bool ParseNumber()
		{
			...
			if (endIdx < Input.Length && IsIdentifier(Input[endIdx]))
			{
				...
				StringView suffix = new(Input, offset, endIdx - offset);
	...
	public class Condition
	{
		...
		static readonly IEnumerable<string> s_trueScalar = ["true"];
		static readonly IEnumerable<string> s_falseScalar = ["false"];
		...
		IEnumerable<string> EvaluateScalar(ref int idx, Func<string, IEnumerable<string>> getPropertyValues)
		{
			...
			return token.Type switch
			{
				...
				TokenType.Scalar => [_strings[token.Index]],
