...
namespace EpicGames.LogMatchers
{
	class XoreaxEventMatcher : ILogEventMatcher
	{
		...
		static readonly Regex s_buildService = new(@"\(BuildService.exe\) is not running");

		static readonly Regex s_xgConsole = new(@"BUILD FAILED: (.*)xgConsole\.exe(.*)");

		static readonly Regex s_buildSystemError = new(@"^\s*--------------------Build System Error");
		static readonly Regex s_buildSystemErrorEnd = new(@"^(\s*)--------------------");

		static readonly Regex s_buildSystemWarning = new(@"^\s*--------------------Build System Warning[- ]");
		static readonly Regex s_buildSystemWarningNext = new(@"^(\s*)([^ ].*):");

		static readonly Regex s_buildSystemWarningSummary = new(@"^\d+ build system warning\(s\):");
		static readonly Regex s_buildSystemWarningSummaryNext = new(@"^(\s*-\s*)([^ ].*)");

		static readonly Regex s_cacheWarning = new(@"^\s*WARNING: \d+ items \([^\)]*\) removed from the cache due to reaching the cache size limit");
		static readonly Regex s_cacheWarning2 = new(@"^\s*WARNING: Several items removed from the cache due to reaching the cache size limit");
		static readonly Regex s_cacheWarning3 = new(@"^\s*WARNING: The Build Cache is close to full");
		...
		public LogEventMatch? Match(ILogCursor cursor)
		{
			...
			if (cursor.IsMatch(s_buildSystemWarning))
			{
				...
				if (builder.Next.TryMatch(s_buildSystemWarningNext, out Match? prefix))
				{
					...
					Regex prefixPattern = new($"^{Regex.Escape(prefix.Groups[1].Value)}\\s");
