...
namespace EpicGames.LogMatchers
{
	...
	{
		static readonly Regex s_undefinedReferencePattern = new(
			@": undefined reference to |undefined symbol|^\s*(ld|ld.lld|lld-link|(.*clang)):|^[^:]*[^a-zA-Z][a-z]?ld: |^\s*>>>");

		static readonly Regex s_undefinedSymbolsForArchPattern = new(
			@"^(\s*)Undefined symbols for architecture");

		static readonly Regex s_ldClangPattern = new(
			@"^\s*(ld|ld.lld|lld-link|clang|clang\+\+):");

		static readonly Regex s_ldClangErrorPattern = new(
			@"^\s*(ld|ld.lld|lld-link|clang|clang\+\+):\s+error:\s+(?<error>.+)$");

		static readonly Regex s_ldMultiplyDefinedPattern = new(
			@"(?<severity>error): L\d+: symbol `(?<symbol>[^`]+)' multiply defined$");

		static readonly Regex s_ldMultiplyDefinedInfoPattern = new(
			@"error:  \.\.\.");

		static readonly Regex s_microsoftErrorPattern = new(
			@"error (?<code>LNK\d+):");

		static readonly Regex s_ignoredLinkWarningPattern = new(
