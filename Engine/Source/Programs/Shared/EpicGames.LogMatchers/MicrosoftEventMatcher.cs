...
namespace EpicGames.LogMatchers
{
	...
	{
		static readonly Regex s_warningCodePattern = new(
			@"(?<severity>(?:error|warning)) " +
			@"(?<code>[a-zA-Z]+[0-9]+)\s*:");

		static readonly Regex s_fileOrToolPattern = new(@"^\s*(.*[^\s])\s*:");

		static readonly Regex s_fileLinePattern = new(@"^\s*(?<file>.*)\((?<line>\d+)(?:, (?<column>\d+))?\)\s*:$");
