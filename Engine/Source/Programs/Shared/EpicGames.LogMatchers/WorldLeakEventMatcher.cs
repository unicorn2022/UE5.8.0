...
namespace EpicGames.LogMatchers
{
	...
	public class WorldLeakEventMatcher : ILogEventMatcher
	{
		...
		/// </summary>
		public static readonly Regex ReportStartPattern = new(@"^====Fatal World Leaks====$", RegexOptions.Multiline | RegexOptions.ExplicitCapture);

		/// <summary>
		/// Regex pattern that matches Report End
		/// </summary>
		public static readonly Regex ReportEndPattern = new(@"^====End Fatal World Leaks====$", RegexOptions.Multiline | RegexOptions.ExplicitCapture);

		/// <summary>
		/// Regex pattern that matches callstacks reported during leak reports
		/// </summary>
		public static readonly Regex ReportCallstackPattern = new(
			@"^[\s]*\^\s" +
			@"(?<Symbol>.+?)\s" +
			@"\[(?<SourceFile>(\/|\w:).+?)" +
		...
		/// </summary>
		public static readonly Regex ReportLingeringReferenceUnderlinePattern = new(@"^\s*\^+\s*$", RegexOptions.Multiline | RegexOptions.ExplicitCapture);

		/// <summary>
		/// Regex pattern that matches leaking reference chain text
		/// </summary>
		public static readonly Regex ReportLingeringReference = new(@"^\s*\->\s*(?<ObjectText>.+)$", RegexOptions.Multiline | RegexOptions.ExplicitCapture);

		/// <summary>
		/// Regex pattern that matches reference chain text
		/// </summary>
		public static readonly Regex ReportBodyPattern = new(@"^\s*(?<ObjectFlags>((\([\w\s]+\)\s))+)?\s*(?<ObjectText>(.+)\s/(.)+/(.)+)$", RegexOptions.Multiline | RegexOptions.ExplicitCapture);
		...
		public LogEventMatch? Match(ILogCursor cursor)
		{
			...
			if (cursor.TryMatch(ReportStartPattern, out _))
			{
				...
				while (builder.Current.CurrentLine != null)
				{
					...
					if (!bFoundLingeringReference && builder.Next != null && builder.Next.TryMatch(ReportLingeringReferenceUnderlinePattern, out _))
					{
