...
namespace EpicGames.Tracing.Tests.UnrealInsights
{
	...
	public class TransportPacketTest
	{
		...
		public void WritePackedUid()
		{
			...
			Assert.ThrowsExactly<NotImplementedException>(() => AssertWriteUid(127, true));
			Assert.ThrowsExactly<NotImplementedException>(() => AssertWriteUid(128, true));
			Assert.ThrowsExactly<NotImplementedException>(() => AssertWriteUid(4362, true));
		}
		...
		public void Deserialize7Bit()
		{
			static void Assert7Bit(ulong expectedValue, string actualBytesHex)
			{
				...
				using MemoryStream ms = new(actualBytes);
		...
		{
			StubTraceEvent event1 = new(1000, 20);
			StubTraceEvent event2 = new(2000, 30);
			ushort uid1 = 5555;
			ushort uid2 = 6666;

			{
				...
				packet.Serialize(writer, [(uid1, event1 as ITraceEvent), (uid2, event2 as ITraceEvent)]);
		...
		{
			TraceNewTraceEvent event1 = new(25582215261913, 10000000, 21069, 8);
			DiagnosticsSession2Event event2 = new("Win64", "UnrealEditor", @" D:\depot\starship-main\QAGame\QAGame.uproject -trace=cpu -game", "++UE5+Main", "++UE5+Main-CL-17442524", 17442524, 3, 4);
			CpuProfilerEventSpecEvent event3 = new(9524, "K2Node_VariableGet /Engine/EngineSky/BP_Sky_Sphere.BP_Sky_Sphere:UserConstructionScript.K2Node_VariableGet_983");
