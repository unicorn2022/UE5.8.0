...
namespace EpicGames.Tracing.Tests.UnrealInsights
{
	...
	public class GenericEventTest
	{
		...
		{
			SerializeAndAssertGenericEvent(_memoryMemoryScopeHex1, MemoryMemoryScopeEvent.EventType, false, 18,
			[
				Field.FromInt(0x27),
			], 45);

			SerializeAndAssertGenericEvent(_memoryMemoryScopeHex2, MemoryMemoryScopeEvent.EventType, false, 18,
			[
				Field.FromInt(0x27),
			], 46);

			SerializeAndAssertGenericEvent(_memoryMemoryScopeHex3, MemoryMemoryScopeEvent.EventType, false, 18,
			[
				Field.FromInt(0x33),
			], 5832765);
		}
		...
		{
			SerializeAndAssertGenericEvent(_traceNewTraceHex, TraceNewTraceEvent.EventType, true, 16,
			[
		...
		{
			TraceNewTraceEvent @event = new(25582215261913, 10000000, 21069, 8);
		...
		{
			{
				TraceThreadInfoEvent @event = new(2, 171380, -1, "GameThread");
				SerializeAndAssertEvent(_traceThreadInfoHex1, @event, true, 36);
			}

			{
				TraceThreadInfoEvent @event = new(3, 0, 2147483647, "Trace");
				SerializeAndAssertEvent(_traceThreadInfoHex2, @event, true, 36);
			}

			{
				TraceThreadInfoEvent @event = new(4, 441304, 80, "Foreground Worker #0");
		...
		{
			SerializeAndAssertGenericEvent(_diagnosticsSession2Hex, DiagnosticsSession2Event.EventType, true, 34,
			[
		...
		{
			DiagnosticsSession2Event @event = new("Win64", "UnrealEditor", @" D:\depot\starship-main\QAGame\QAGame.uproject -trace=cpu -game", "++UE5+Main", "++UE5+Main-CL-17442524", 17442524, 3, 4);
		...
		{
			SerializeAndAssertGenericEvent(_cpuProfilerEventSpecHex, CpuProfilerEventSpecEvent.EventType, true, 33,
			[
				Field.FromInt(9524),
		...
		{
			CpuProfilerEventSpecEvent @event = new(9524, "K2Node_VariableGet /Engine/EngineSky/BP_Sky_Sphere.BP_Sky_Sphere:UserConstructionScript.K2Node_VariableGet_983");
		...
		public void CpuProfiler_EventBatch_Deserialize()
		{
			...
			CpuProfilerSerializer serializer = new(10000000);
		...
		public void CpuProfiler_EventBatch_Serialize()
		{
			...
			CpuProfilerEventBatchEvent @event = new(data);
		...
		public void SerializeEventWithAuxAndImportant()
		{
			...
			GenericEvent @event = new(98989, fields, CpuProfilerEventSpecEvent.EventType);
			Assert.AreEqual(0x77, @event.Size);

			TraceImportantEventHeader eventHeader = new(uid, @event.Size);
		...
		public void AuxHeaderDeserialize()
		{
			...
			using MemoryStream ms = new([0x01, 0xC1, 0x0D, 0x00]);
		...
		private static GenericEvent DeserializeAndAssertGenericEvent(string hexData, EventType eventType, bool isImportant, ushort expectedUid, uint? expectedSerial = 0)
		{
			...
			using MemoryStream ms = new(block);
			using BinaryReader reader = new(ms);
		...
		private static void SerializeAndAssertGenericEvent(string expectedHexData, EventType eventType, bool isImportant, ushort uid, Field[] fields, uint serial = 0)
		{
			...
			GenericEvent @event = new(serial, fields, eventType);
