...
namespace EpicGames.LogMatchers.Tests
{
	[TestClass]
	public class AutomationToolLogEventParserTests
	{
		const string LogLine = "LogLine";

		static readonly DirectoryReference s_rootDir = new(RuntimeInformation.IsOSPlatform(OSPlatform.Windows) ? "C:\\Horde" : "/horde");
		...
		public void LogChannelMatcher()
		{
...

			{
				...
				CheckEventGroup(logEvents.Slice(0, 3), 1, 3, LogLevel.Error, KnownLogEvents.Engine_LogChannel, false);
				CheckEventGroup(logEvents.Slice(3, 1), 4, 1, LogLevel.Information, KnownLogEvents.Engine_LogChannel, false);
			}

			{
				List<LogEvent> logEvents = Parse(String.Join("\n", lines).Replace("Error:", "Warning:", StringComparison.Ordinal));
				Assert.AreEqual(4, logEvents.Count);
				CheckEventGroup(logEvents.Slice(0, 3), 1, 3, LogLevel.Warning, KnownLogEvents.Engine_LogChannel, false);
				CheckEventGroup(logEvents.Slice(3, 1), 4, 1, LogLevel.Information, KnownLogEvents.Engine_LogChannel, false);
			}
		}

		[TestMethod]
		public void LogChannelMatcher_CrashCallstackGrouping()
		{
			// Critical error with callstack and trailing blank lines - all should be grouped into one event
			string[] lines =
			[
				@"LogWindows: Error: === Critical error: ===",
				@"LogWindows: Error:",
				@"LogWindows: Error: Assertion failed: Pair != nullptr [File:Map.h] [Line: 728]",
				@"LogWindows: Error:",
				@"LogWindows: Error: [Callstack] 0x00007ffe5b17b248 UnrealEditor-Core.dll!FDebug::CheckVerifyFailedImpl2() []",
				@"LogWindows: Error: [Callstack] 0x00007ffe589a608c UnrealEditor-Engine.dll!UWorld::Tick() []",
				@"LogWindows: Error: [Callstack] 0x00007ffe89f44cb0 KERNEL32.DLL!UnknownFunction []",
				@"LogWindows: Error:",
				@"LogWindows: Error:",
			];

			List<LogEvent> logEvents = Parse(lines);
			// All 9 lines (header + context + callstack + trailing blank lines) should be grouped
			CheckEventGroup(logEvents, 0, 9, LogLevel.Error, KnownLogEvents.Engine_LogChannel);
		}

		[TestMethod]
		public void LogChannelMatcher_TrailingBlankConsumptionStopsAtRealContent()
		{
			// A real error line after trailing blanks must NOT be consumed into the crash group
			string[] lines =
			[
				@"LogWindows: Error: === Critical error: ===",
				@"LogWindows: Error:",
				...
				@"LogWindows: Error: [Callstack] 0x00007ffe89f44cb0 KERNEL32.DLL!UnknownFunction []",
				@"LogWindows: Error:",
				@"LogWindows: Error: A completely unrelated error message",
			];

			List<LogEvent> logEvents = Parse(lines);
			// First 6 lines grouped (header + context + callstack + trailing blank)
			CheckEventGroup(logEvents.Slice(0, 6), 0, 6, LogLevel.Error, KnownLogEvents.Engine_LogChannel);
			// 7th line is a separate event
			CheckEventGroup(logEvents.Slice(6, 1), 6, 1, LogLevel.Error, KnownLogEvents.Engine_LogChannel);
		}

		[TestMethod]
		public void LogChannelMatcher_CallstackOnlyGrouping()
		{
			// [Callstack] lines without a preceding crash header should still be grouped, including trailing blanks
			string[] lines =
			[
				@"LogWindows: Error: [Callstack] 0x00007ffe5b17b248 UnrealEditor-Core.dll!FDebug::CheckVerifyFailedImpl2() []",
				@"LogWindows: Error: [Callstack] 0x00007ffe589a608c UnrealEditor-Engine.dll!UWorld::Tick() []",
			...
			List<LogEvent> logEvents = Parse(lines);
			// All 4 lines (callstack + trailing blank) should be grouped
			CheckEventGroup(logEvents, 0, 4, LogLevel.Error, KnownLogEvents.Engine_LogChannel);
		}

		[TestMethod]
		public void LogChannelMatcher_CrashHeaderWithoutCallstackFallsThrough()
		{
			// A crash header without [Callstack] lines within range should fall through to normal behavior
			string[] lines =
			[
				@"LogWindows: Error: === Critical error: ===",
				@"LogWindows: Error: Some other error info",
			];

			List<LogEvent> logEvents = Parse(lines);
			// Without callstack lines, each line is separate (second line is not a hanging indent)
			Assert.AreEqual(2, logEvents.Count);
			Assert.AreEqual(KnownLogEvents.Engine_LogChannel, logEvents[0].Id);
			Assert.AreEqual(KnownLogEvents.Engine_LogChannel, logEvents[1].Id);
		}
		...
		[TestMethod]
		public void ShaderEventMatcherWithFilePaths()
		{
			string[] lines =
			[
				@"LogCook: Display: Cook Diagnostics: OpenFileHandles=10353, VirtualMemory=20078MiB",
				@"LogShaderCompilers: Warning: Failed to compile Material /MeshModelingToolsetExp/Materials/PointSetOverlaidComponentMaterial.PointSetOverlaidComponentMaterial for platform SF_PS5, Default Material will be used in game.",
				@"D:\build\U5M+Inc\Sync\Engine\Shaders\Private\LocalVertexFactory.ush(731:11): Shader FLumenCardVS, Permutation 0, VF FLocalVertexFactory:",
				@"    /Engine/Private/LocalVertexFactory.ush(731:11): (error, code:5430) - 'FMaterialVertexParameters' does not contain a member named 'TexCoords'",
				@"D:\build\U5M+Inc\Sync\Engine\Shaders\Private\MaterialTemplate.ush(4282:31): Shader TBasePassVSFNoLightMapPolicy, Permutation 0, VF FLocalVertexFactory:",
				@"    /Engine/Generated/Material.ush(4282:31): (error, code:5430) - 'FMaterialVertexParameters' does not contain a member named 'TexCoords'",
				@"Internal Error!",
				@"    PointSetOverlaidComponentMaterial_6dd3f421e64e95eb/Default/StandardVelocityPipeline/FLocalVertexFactory/TVelocityPS<EVelocityPassMode::Velocity_Standard>/0:/Engine/Private/VelocityShader.usf|MainPixelShader VF 'FLocalVertexFactory' Type 'TVelocityPS<EVelocityPassMode::Velocity_Standard>' '/Engine/Private/VelocityShader.usf' Entry 'MainPixelShader' Permutation 0",
				@"D:\build\U5M+Inc\Sync\Engine\Shaders\Private\MaterialTemplate.ush(4353:31): Shader TVelocityVS<EVelocityPassMode::Velocity_Standard>, Permutation 0, VF FLocalVertexFactory:",
				@"    /Engine/Generated/Material.ush(4353:31): (error, code:5430) - 'FMaterialVertexParameters' does not contain a member named 'TexCoords'",
				@"LogCook: Display: Excluding /Interchange/gltf/MaterialInstances/MI_ClearCoat_Mask_DS"
			];

			{
				List<LogEvent> logEvents = Parse(lines);
				Assert.AreEqual(11, logEvents.Count);
				CheckEventGroup(logEvents.Slice(1, 9), 1, 9, LogLevel.Warning, KnownLogEvents.Engine_ShaderCompiler);

				// Verify the material property is extracted from "Failed to compile Material" warnings
				LogEvent shaderEvent = logEvents[1];
				Assert.IsNotNull(shaderEvent.Properties);
				object? materialValue;
				Assert.IsTrue(shaderEvent.TryGetProperty("material", out materialValue));
				Assert.AreEqual("/MeshModelingToolsetExp/Materials/PointSetOverlaidComponentMaterial.PointSetOverlaidComponentMaterial", materialValue);
			}
		}

		[TestMethod]
		public void ShaderEventMatcherWithoutMaterialProperty()
		{
			// Shader errors that don't match "Failed to compile Material" pattern should NOT have material property
			string[] lines =
			[
				@"LogShaderCompilers: Error: Shader compilation failed due to syntax error",
...

			{
				List<LogEvent> logEvents = Parse(lines);
				Assert.AreEqual(2, logEvents.Count);
				CheckEventGroup(logEvents.Slice(0, 2), 0, 2, LogLevel.Error, KnownLogEvents.Engine_ShaderCompiler);

				// Verify NO material property is set for non-material errors
				LogEvent shaderEvent = logEvents[0];
				bool hasMaterial = shaderEvent.TryGetProperty("material", out _);
				Assert.IsFalse(hasMaterial);
			}
		}
		...
		public void ThreadSanitizerErrorMatcher_SummaryWithSourceFileFromLibrary()
		{
			...
			Assert.IsTrue(logEvents[SummaryIndex].Message.StartsWith("SUMMARY: ThreadSanitizer", StringComparison.Ordinal));
		...
		[TestMethod]
		public void RaceDetectorSanitizerErrorMatcher()
		{
			string[] lines =
			[
				@"=========================================",
				@"WARNING: RaceDetector: data race detected",
				@"Read of size 8 at 00007506CACF3510 made at clock 103 by thread FAssetDataDiscovery (36672) which is now at clock 103:",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\AssetRegistry\Private\AssetDataGatherer.cpp (1648):     [Inline Frame] UE::AssetDataGather::Private::FAssetDataDiscovery::IsSynchronous() 0x00007FFD6F7501B7",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\AssetRegistry\Private\AssetDataGatherer.cpp (2272):     UE::AssetDataGather::Private::FAssetDataDiscovery::SetIsIdle() 0x00007FFD6F7501B7",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\AssetRegistry\Private\AssetDataGatherer.cpp (1969):     UE::AssetDataGather::Private::FAssetDataDiscovery::TickInternal() 0x00007FFD6F74A451",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\AssetRegistry\Private\AssetDataGatherer.cpp (1677):     UE::AssetDataGather::Private::FAssetDataDiscovery::Run() 0x00007FFD6F746CDB",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Private\Windows\WindowsRunnableThread.cpp (156):   FRunnableThreadWin::Run() 0x00007FFD8100B4A9",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Private\Windows\WindowsRunnableThread.cpp (79):    FRunnableThreadWin::GuardedRun() 0x00007FFD8100B2BB",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Private\Microsoft\MicrosoftRunnableThread.h (38):  FRunnableThreadWin::_ThreadProc() 0x00007FFD80FFF476",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Private\Sanitizer\RaceDetectorWindows.cpp (165):   InstrumentedStartThread() 0x00007FFD80D79704",
				@"Previous Write of size 8 at 00007506CACF3510 at clock 2768446 by thread GameThread (5288 alive) which is now at clock 2768848:",
				@"/build\++Fortnite\Sync\Engine\Source\Runtime\AssetRegistry\Private\AssetDataGatherer.cpp (1641):     [Inline Frame] UE::AssetDataGather::Private::FAssetDataDiscovery::StartAsync() 0x00007FFD6F7F096F",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\AssetRegistry\Private\AssetDataGatherer.cpp (3740):     UE::AssetDataGather::Private::FPreloader::DelayedInitialize() 0x00007FFD6F7F096F",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\AssetRegistry\Private\AssetDataGatherer.cpp (3695):     [Inline Frame] UE::AssetDataGather::Private::FPreloader::FPreloader::<lambda_1>::operator()() 0x00007FFD6F7F06C9",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Public\Templates\Invoke.h (47):                    [Inline Frame] Invoke() 0x00007FFD6F7F06C9",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Public\Templates\Function.h (292):                 UE::Core::Private::Function::TFunctionRefCaller<`lambda at .\Runtime\AssetRegistry\Private\AssetDataGatherer.cpp:3695:73',void>::Call() 0x00007FFD6F7F06C9",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Public\Templates\Function.h (414):                 [Inline Frame] UE::Core::Private::Function::TFunctionRefBase<UE::Core::Private::Function::TFunctionStorage<0>,void ()>::operator()() 0x00007FFD80BB9D1C",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Private\Misc\DelayedAutoRegister.cpp (26):         [Inline Frame] FDelayedAutoRegisterHelper::FDelayedAutoRegisterHelper::<lambda_18>::operator()() 0x00007FFD80BB9D1C",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Public\Templates\Invoke.h (47):                    [Inline Frame] Invoke() 0x00007FFD80BB9D1C",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Public\Templates\Function.h (292):                 UE::Core::Private::Function::TFunctionRefCaller<`lambda at .\Runtime\Core\Private\Misc\DelayedAutoRegister.cpp:26:3',void,EDelayedRegisterRunPhase>::Call() 0x00007FFD80BB9D1C",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Public\Templates\Function.h (414):                 [Inline Frame] UE::Core::Private::Function::TFunctionRefBase<UE::Core::Private::Function::TFunctionStorage<0>,void (EDelayedRegisterRunPhase)>::operator()() 0x00007FFD80BBA88D",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Public\Templates\Invoke.h (47):                    [Inline Frame] Invoke() 0x00007FFD80BBA88D",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Public\Templates\Tuple.h (256):                    [Inline Frame] UE::Core::Private::Tuple::TTupleBase<TIntegerSequence<unsigned int> >::ApplyAfter() 0x00007FFD80BBA88D",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Public\Delegates\DelegateInstancesImpl.h (805):    TBaseFunctorDelegateInstance<void (EDelayedRegisterRunPhase),FDefaultDelegateUserPolicy,TFunction<void (EDelayedRegisterRunPhase)> >::ExecuteIfSafe() 0x00007FFD80BBA88D",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Public\Delegates\MulticastDelegateBase.h (286):    TMulticastDelegateBase<FDefaultDelegateUserPolicy>::Broadcast<IBaseDelegateInstance<void (EDelayedRegisterRunPhase),FDefaultDelegateUserPolicy>,EDelayedRegisterRunPhase>() 0x00007FFD80BBAA26",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Public\Delegates\DelegateSignatureImpl.inl (1134): [Inline Frame] TMulticastDelegate<void (EDelayedRegisterRunPhase),FDefaultDelegateUserPolicy>::Broadcast() 0x00007FFD80B272F4",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Private\Misc\DelayedAutoRegister.cpp (64):         FDelayedAutoRegisterHelper::RunAndClearDelayedAutoRegisterDelegates() 0x00007FFD80B272F4",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Launch\Private\LaunchEngineLoop.cpp (6537):             FEngineLoop::AppInit() 0x00007FF6002ABB5C",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Launch\Private\LaunchEngineLoop.cpp (2752):             FEngineLoop::PreInitPreStartupScreen() 0x00007FF60029FA14",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Launch\Private\LaunchEngineLoop.cpp (4369):             [Inline Frame] FEngineLoop::PreInit() 0x00007FF600291DAB",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Launch\Private\Launch.cpp (40):                         [Inline Frame] EnginePreInit() 0x00007FF600291DAB",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Launch\Private\Launch.cpp (146):                        GuardedMain() 0x00007FF600291DAB",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Launch\Private\Windows\LaunchWindows.cpp (123):         GuardedMainWrapper() 0x00007FF6002C255B",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Launch\Private\Windows\LaunchWindows.cpp (277):         LaunchWindowsStartup() 0x00007FF6002C3046",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Launch\Private\Windows\LaunchWindows.cpp (334):         WinMain() 0x00007FF6002C3272",
				@"D:\a\_work\1\s\src\vctools\crt\vcstartup\src\startup\exe_common.inl (102):                             [Inline Frame] invoke_main() 0x00007FF6002ECC72",
				@"D:\a\_work\1\s\src\vctools\crt\vcstartup\src\startup\exe_common.inl (288):                             __scrt_common_main_seh() 0x00007FF6002ECC72",
				@"C:\WINDOWS\System32\KERNEL32.DLL:                                                                      [Unknown Function] 0x00007FFDC87F4ED0",
				@"========================================"
			];

			List<LogEvent> logEvents = Parse(lines);
			Assert.AreEqual(40, logEvents[0].LineCount);
			CheckEventGroup(logEvents, 0, 40, LogLevel.Warning, KnownLogEvents.Sanitizer_RaceDetector);

			Assert.AreEqual(@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\AssetRegistry\Private\AssetDataGatherer.cpp", logEvents[3].GetProperty("SourceFile").ToString());
			Assert.AreEqual("1648", logEvents[3].GetProperty("Line").ToString());
			Assert.AreEqual("[Inline Frame] UE::AssetDataGather::Private::FAssetDataDiscovery::IsSynchronous()", logEvents[3].GetProperty("Symbol").ToString());
			Assert.AreEqual("data race detected", logEvents[1].GetProperty("SummaryReason").ToString());

			Assert.AreEqual(@"C:\WINDOWS\System32\KERNEL32.DLL", logEvents[38].GetProperty("SourceFile").ToString());
			Assert.AreEqual("[Unknown Function]", logEvents[38].GetProperty("Symbol").ToString());
		}
		...
		public void AddressSanitizerErrorMatcher()
		{
			...
			Assert.IsFalse(logEvents[7].TryGetProperty("Column", out _));
		...
		[TestMethod]
		public void AddressSanitizerErrorMatcher_SummaryWithEboot()
		{
			string[] lines =
			[
				@"=================================================================",
		...
		public void ZenCliEventMatcher()
		{
			...
			[
				...
				"[00:03:13.217] [warning] HttpClient request failed (session: 0900d2e1756b2902977dcb54): Url: https://jupiter.devtools.epicgames.com/api/v2/builds/ue.oplog/UE.installed-build.ue5-main.Windows/0900d2e1fbe05d9d083c8de7/blobs/b0b64bfd6d096164927606b5d0f945bc2193c6ad, Status: 504, Error: '' (0), Bytes: 1572747/132 (Up/Down), Elapsed: 1m00s, Response: '<html>" ,
				...
				"[00:03:13.217] [info] Retry: HTTP error 504 Gateway Timeout (<html>" ,
				"                      <head><title>504 Gateway Time-out</title></head>",
				"                      <body>" ,
				"                      <center><h1>504 Gateway Time-out</h1></center>" ,
				"                      </body>" ,
			...
			List<LogEvent> logEvents = Parse(lines);
			// generic error level matching is currently disabled
			//Assert.AreEqual(8, logEvents.Count);
			Assert.AreEqual(7, logEvents.Count);

			// the last error event is on line 15
			// generic error level matching is currently disabled
			// CheckEventGroup(logEvents.Slice(logEvents.Count - 2, 1), 14, 1, LogLevel.Error, KnownLogEvents.ZenCli_Event);

			// the last event is an information on line 19
			CheckEventGroup(logEvents.Slice(logEvents.Count - 1, 1), 33, 1, LogLevel.Information, KnownLogEvents.ZenCli_Event);
		}
		...
		static List<LogEvent> Parse(string text, DirectoryReference workspaceDir)
		{
			...
			LoggerCapture logger = new();

			PerforceMetadataLogger perforceLogger = new(logger);
			perforceLogger.AddClientView(workspaceDir, "//UE4/Main/...", 12345);

			using (LogEventParser parser = new(perforceLogger))
			{
				parser.AddMatchers(LogMatcherCollections.AutomationUtilsMatchers);
				parser.AddMatchers(LogMatcherCollections.UBTMatchers);
		...
		static void CheckEventGroup(IEnumerable<LogEvent> logEvents, int index, int count, LogLevel level, EventId eventId = default, bool isComplete = true)
		{
			...
			for (int idx = 0; idx < count; idx++)
			{
				...
				if (isComplete || idx + 1 == count)
				{
					Assert.AreEqual(count, logEvent.LineCount, "Line count mismatch");
				}
				else
				{
					Assert.IsLessThanOrEqualTo(count, logEvent.LineCount, "Line count is greater than it should be");
				}
