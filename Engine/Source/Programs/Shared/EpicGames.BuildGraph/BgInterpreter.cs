...
namespace EpicGames.BuildGraph
{
	...
	public class BgInterpreter
	{
		...
		readonly List<BgOptionDef> _optionDefs = [];
		...
		public object Evaluate()
		{
			...
			return Evaluate(new Frame(_fragments[0], []));
		}
				...
				case BgOpcode.ListSelect:
					{
						...
						return source.Select(x => Call(fragment, [x]));
					}
				case BgOpcode.ListWhere:
					{
						...
						return source.Where(x => (bool)Call(fragment, [x]));
					}
				...
				case BgOpcode.ListOption:
					{
						...
						return value!.Split(['+', ';'], StringSplitOptions.RemoveEmptyEntries).Select(x => (object)x).ToList();
					}
		...
		{
			return [.. ((IEnumerable<object>)value).Select(x => (T)x)];
		}
		...
		public void Disassemble(ILogger logger)
		{
			...
			for (int idx = 0; idx < _fragments.Length; idx++)
			{
				...
				Disassemble(new Frame(_fragments[idx], []), logger);
			}
		}
