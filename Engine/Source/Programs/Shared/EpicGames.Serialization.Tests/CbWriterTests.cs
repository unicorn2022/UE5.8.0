...
namespace EpicGames.Serialization.Tests
{
	...
	public class CbWriterTests
	{
		...
		{
			List<IoHash> objHashes = [.. Enumerable.Range(0, 5).Select(x => IoHash.Compute(Encoding.UTF8.GetBytes($"obj{x}")))];
			List<IoHash> binHashes = [.. Enumerable.Range(0, 5).Select(x => IoHash.Compute(Encoding.UTF8.GetBytes($"bin{x}")))];
