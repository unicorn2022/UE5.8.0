...
namespace EpicGames.Serialization.Tests
{
	...
	public class EngineTests
	{
		...
		readonly Dictionary<CbFieldType, CbFieldAccessors> _typeAccessors = new()
		{
		...
		void TestField(CbFieldType fieldType, CbField field, object? expectedValue = null, object? defaultValue = null, CbFieldError expectedError = CbFieldError.None, CbFieldAccessors? accessors = null)
		{
			...
			if (expectedError == CbFieldError.None && !field.IsBool())
			{
				...
				Assert.AreEqual(CbFieldError.TypeError, field.GetError());
			}
			...
			Assert.AreEqual(expectedError, field.GetError());
		}
		...
		{
			CbField field = new(payload, fieldType);
			...
			Assert.AreEqual(CbFieldError.None, field.GetError());
		...
		public void CbFieldNoneTest()
		{
			// Test CbField()
			{
				...
				Assert.AreEqual(1, defaultField.GetSize());
				Assert.AreEqual(0, defaultField.GetName().Length);
				Assert.IsFalse(defaultField.HasName());
				Assert.IsFalse(defaultField.HasValue());
				Assert.IsFalse(defaultField.HasError());
				Assert.AreEqual(CbFieldError.None, defaultField.GetError());
			...
			// Test CbField(None)
			{
				CbField noneField = new(ReadOnlyMemory<byte>.Empty, CbFieldType.None);
				Assert.AreEqual(1, noneField.GetSize());
				Assert.AreEqual(0, noneField.GetName().Length);
				Assert.IsFalse(noneField.HasName());
				Assert.IsFalse(noneField.HasValue());
				Assert.IsFalse(noneField.HasError());
				Assert.AreEqual(CbFieldError.None, noneField.GetError());
			...
			// Test CbField(None|Type|Name)
			{
				...
				CbField noneField = new(noneBytes);
			...
			// Test CbField(None|Name)
			{
				CbFieldType fieldType = CbFieldType.None | CbFieldType.HasFieldName;
				byte[] noneBytes = [(byte)fieldType, 4, (byte)'N', (byte)'a', (byte)'m', (byte)'e'];
				CbField noneField = new(noneBytes.AsMemory(1), fieldType);
				Assert.AreEqual(noneField.GetSize(), noneBytes.Length);
				Assert.AreEqual(noneField.GetName(), new Utf8String("Name"));
				Assert.IsTrue(noneField.HasName());
				Assert.IsFalse(noneField.HasValue());
		...
		public void CbFieldNullTest()
		{
			// Test CbField(Null)
			{
				CbField nullField = new(ReadOnlyMemory<byte>.Empty, CbFieldType.Null);
				Assert.AreEqual(1, nullField.GetSize());
				Assert.IsTrue(nullField.IsNull());
				Assert.IsTrue(nullField.HasValue());
				Assert.IsFalse(nullField.HasError());
				Assert.AreEqual(CbFieldError.None, nullField.GetError());
		...
		public void CbFieldObjectTest()
		{
			static void TestIntObject(CbObject obj, int expectedNum, int expectedPayloadSize)
			{
				...
				{
					++actualNum;
					Assert.AreNotEqual(0, field.GetName().Length);
			...
			// Test CbField(Object, NotEmpty)
			{
				...
				CbField field = new(payload, CbFieldType.Object);
				...
				Assert.AreEqual(2, @object.Find("B").AsInt32());
				Assert.AreEqual(4, @object.Find("b").AsInt32(4));
				Assert.AreEqual(2, @object.FindIgnoreCase("B").AsInt32());
				Assert.AreEqual(2, @object.FindIgnoreCase("b").AsInt32());
				Assert.AreEqual(2, @object["B"].AsInt32());
				Assert.AreEqual(4, @object["b"].AsInt32(4));
			}
			...
			// Test CbField(UniformObject, NotEmpty)
			{
				...
				CbField field = new(payload, CbFieldType.UniformObject);
				...
				CbField namedField = new(namedPayload, CbFieldType.UniformObject | CbFieldType.HasFieldName);
			...
			// Test FCbObjectView(ObjectWithName) and CreateIterator
			{
				...
				CbObject @object = new(buffer);
				Assert.AreEqual(6, @object.GetSize());
				CbObject objectClone = CbObject.Clone(@object);
				Assert.AreEqual(6, objectClone.GetSize());
				...
				for (CbFieldIterator it = objectClone.CreateIterator(); it; ++it)
				{
					...
					Assert.AreEqual(8, field.AsInt32());
				}
				for (CbFieldIterator it = objectClone.CreateIterator(), end = new(); it != end; ++it)
				{
				}
		...
		public void CbFieldArrayTest()
		{
			static void TestIntArray(CbArray array, int expectedNum, int expectedPayloadSize)
			{
				...
				Assert.AreEqual(expectedNum, array.Count);
			...
			TestField(CbFieldType.Array, [1, 0]);
			...
			// Test CbField(Array, NotEmpty)
			{
				...
				CbField field = new(payload, CbFieldType.Array);
			...
			// Test CbField(UniformArray)
			{
				...
				CbField field = new(payload, CbFieldType.UniformArray);
				...
				CbField namedField = new(namedPayload, CbFieldType.UniformArray | CbFieldType.HasFieldName);
			...
			// Test CbArray(ArrayWithName) and CreateIterator
			{
				...
				CbArray array = new(buffer);
				Assert.AreEqual(5, array.GetSize());
				CbArray arrayClone = array;
				Assert.AreEqual(5, arrayClone.GetSize());
				...
				for (CbFieldIterator it = arrayClone.CreateIterator(), end = new(); it != end; ++it)
				{
				}
		...
		public void CbFieldBinaryTest()
		{
			...
			TestField(CbFieldType.Binary, [0]);

			// Test CbField(Binary, Value)
			{
				byte[] payload = [3, 4, 5, 6]; // Size: 3, Data: 4/5/6
				CbField fieldView = new(payload, CbFieldType.Binary);
		...
		public void CbFieldStringTest()
		{
			...
			TestField(CbFieldType.String, [0]);
		...
		public void CbFieldFloatTest()
		{
			// Test CbField(Float, 32-bit)
			{
				...
				CbField field = new(payload, CbFieldType.Float32);
			...
			// Test CbField(Float, 64-bit)
			{
				...
				CbField field = new(payload, CbFieldType.Float64);
			...
			// Test CbField(Integer+, MaxBinary32) as Float
			{
				...
				CbField field = new(payload, CbFieldType.IntegerPositive);
			...
			// Test CbField(Integer-, MaxBinary32) as Float
			{
				...
				CbField field = new(payload, CbFieldType.IntegerNegative);
				TestField(CbFieldType.Float32, field, -16_777_215.0f);
				TestField(CbFieldType.Float64, field, -16_777_215.0);
			}
		...
		public void CbFieldObjectAttachmentTest()
		{
			...
			{
				CbField field = new(sequentialBytes, CbFieldType.ObjectAttachment);
		...
		public void CbFieldBinaryAttachmentTest()
		{
			...
			{
				CbField field = new(sequentialBytes, CbFieldType.BinaryAttachment);
		...
		public void CbFieldDateTimeTest()
		{
			...
			TestField(CbFieldType.DateTime, [0, 0, 0, 0, 0, 0, 0, 0]);

			// Test CbField(DateTime, 0x1020_3040_5060_7080)
			TestField(CbFieldType.DateTime, [0x10, 0x20, 0x30, 0x40, 0x50, 0x60, 0x70, 0x80], new DateTime(0x1020_3040_5060_7080L, DateTimeKind.Utc));

			// Test CbField(DateTime, Zero) as FDateTime
			{
				byte[] payload = [0, 0, 0, 0, 0, 0, 0, 0];
				CbField field = new(payload, CbFieldType.DateTime);
			...
			// Test CbField(None) as DateTime
			{
				...
				DateTime defaultValue = new(0x1020_3040_5060_7080L, DateTimeKind.Utc);
		...
		public void CbFieldTimeSpanTest()
		{
			...
			TestField(CbFieldType.TimeSpan, [0, 0, 0, 0, 0, 0, 0, 0]);

			// Test CbField(TimeSpan, 0x1020_3040_5060_7080)
			TestField(CbFieldType.TimeSpan, [0x10, 0x20, 0x30, 0x40, 0x50, 0x60, 0x70, 0x80], new TimeSpan(0x1020_3040_5060_7080L));

			// Test CbField(TimeSpan, Zero) as FTimeSpan
			{
				byte[] payload = [0, 0, 0, 0, 0, 0, 0, 0];
				CbField field = new(payload, CbFieldType.TimeSpan);
			...
			// Test CbField(None) as TimeSpan
			{
				...
				TimeSpan defaultValue = new(0x1020_3040_5060_7080L);
