...
namespace EpicGames.Serialization.Tests
{
	...
	public class SerializerTests
	{
		...
		public void RoundTrip()
		{
			...
			Assert.AreEqual("hello", cbObj1.Find("D").AsString());
			...
			Assert.AreEqual((int)TestEnum.Value3, cbObj2.Find("Enum").AsInt32());
		}
		...
		{
			Assert.ThrowsExactly<CbEmptyClassException>(() => CbSerializer.Serialize(new EmptyObject()));
			Assert.ThrowsExactly<CbEmptyClassException>(() => CbSerializer.Deserialize<EmptyObject>(CbField.Empty));

			EmptyObjectWithAttribute obj2 = new();
			CbObject cbObj2 = CbSerializer.Serialize(obj2);
		...
		{
			ObjectWithNullField input = new() { Value = 123 };
			...
			Assert.AreEqual(null, output.Items);
		}
		...
		{
			ObjectWithReadOnlyCollection input = new();
		...
		{
			ObjectWithStringDictCollection input = new();
			input.Dictionary.Add("hello", 123);
			input.Dictionary.Add("world", 456);
			CbObject obj = CbSerializer.Serialize(input);
		...
		[TestMethod]
		public void ObjectWithByteArrayTest()
		{
			ObjectWithByteArray input = new();
			byte[] b = Encoding.ASCII.GetBytes("test-string");
			input.Bytes = b;
			CbObject obj = CbSerializer.Serialize(input);

			ObjectWithByteArray output = CbSerializer.Deserialize<ObjectWithByteArray>(obj.AsField());
			CollectionAssert.AreEqual(b, output.Bytes.ToArray());
		}
	}
}
