...
{
	/// <summary>
	/// Result of an OIDC token refresh operation.
	/// </summary>
	public record TokenRefreshResult(
		bool IsError, string? Error, string? ErrorDescription,
		string? AccessToken, string? RefreshToken, DateTimeOffset AccessTokenExpiration);

	public interface IOidcTokenManager
	{
		public Task<IOidcTokenInfo> LoginAsync(string providerIdentifier, CancellationToken cancellationToken = default);

		public Task<IOidcTokenInfo> GetAccessToken(string providerIdentifier, CancellationToken cancellationToken = default);

		public Task<IOidcTokenInfo?> TryGetAccessToken(string providerIdentifier, CancellationToken cancellationToken = default);

		public Task<OidcStatus> GetStatusForProvider(string providerIdentifier, CancellationToken cancellationToken = default);
	}

	public class OidcTokenManager : IOidcTokenManager
	{
		private readonly Lock _lockObject = new();
		private readonly Dictionary<string, OidcTokenClient> _tokenClients = new(StringComparer.OrdinalIgnoreCase);
		...
		public OidcTokenManager(IOptionsMonitor<OidcTokenOptions> settings, ITokenStore tokenStore, IOidcTokenClientFactory tokenClientFactory, List<string>? allowedProviders = null)
			: this(tokenStore)
		{
			List<string> providerKeys = [.. settings.CurrentValue.Providers.Keys.Where(k => allowedProviders == null || allowedProviders.Contains(k))];
		...
		public static OidcTokenManager CreateTokenManager(IConfiguration providerConfiguration, ITokenStore tokenStore, List<string>? allowedProviders = null, ILoggerFactory? loggerFactory = null)
		{
			return new OidcTokenManager(providerConfiguration, tokenStore, allowedProviders, loggerFactory);
		}

		private OidcTokenManager(IConfiguration providerConfiguration, ITokenStore tokenStore, List<string>? allowedProviders = null, ILoggerFactory? loggerFactory = null)
			: this(tokenStore)
		{
			OidcTokenOptions options = new();
			providerConfiguration.Bind(options);

			List<string> providerKeys = [.. options.Providers.Keys.Where(k => allowedProviders == null || allowedProviders.Contains(k))];
			...
			foreach ((string key, ProviderInfo providerInfo) in options.Providers)
			{
				...
				OidcTokenClient tokenClient = new(key, providerInfo, options.LoginTimeout, _tokenStore, loggerFactory?.CreateLogger<OidcTokenClient>());
				_tokenClients.Add(key, tokenClient);
			}
		}
		...
		public Task<OidcStatus> GetStatusForProvider(string providerIdentifier, CancellationToken cancellationToken = default)
		{
			return Task.FromResult(_tokenClients[providerIdentifier].GetStatus());
		}
	}
...
#pragma warning restore CA1716
	...
	{
		/// <summary>Max seconds the refresh HTTP call is allowed before cancellation.</summary>
		private const int RefreshHttpTimeoutSeconds = 30;

		/// <summary>Max seconds to wait for another process to release the refresh lock.</summary>
		private const int LockAcquireTimeoutSeconds = 60;
		...
		public OidcTokenClient(string name, ProviderInfo providerInfo, TimeSpan loginTimeout, ITokenStore tokenStore, ILogger<OidcTokenClient>? logger = null)
		{
			...
			{
				_tokenStore.TryGetRefreshToken(name, out string? existingToken);
				_logger.LogDebug("OidcTokenClient created for provider '{Name}'. Refresh token from store: {HasToken}", name, !String.IsNullOrEmpty(existingToken));
			}
		}
		...
		public async Task<IOidcTokenInfo> LoginAsync(CancellationToken cancellationToken)
		{
			_logger?.LogInformation("Starting interactive login for provider '{Name}'.", _name);
			IOidcTokenInfo tokenInfo = await DoLoginAsync(cancellationToken);
			...
			_logger?.LogDebug("Saving refresh token to store after interactive login for provider '{Name}'.", _name);
			// After browser login, token MUST be persisted regardless of external cancellation.
			using CancellationTokenSource loginPersistCts = new(TimeSpan.FromSeconds(LockAcquireTimeoutSeconds));
			using IDisposable _ = await AcquireRefreshLockAsync(loginPersistCts.Token);
			_tokenStore.AddRefreshToken(_name, tokenInfo.RefreshToken);

			_logger?.LogInformation("Interactive login completed for provider '{Name}'. Token expiry: {Expiry}", _name, tokenInfo.TokenExpiry);
			return tokenInfo;
		}

		private async Task<IOidcTokenInfo> DoLoginAsync(CancellationToken cancellationToken)
		{
					...
					case "POST":
						{
							...
							using StreamReader reader = new(body, request.BodyEncoding);
...
#pragma warning restore CA1054
		...
		/// <summary>
		/// Performs the OIDC token refresh via HTTP. Virtual for testability.
		/// </summary>
		protected virtual async Task<TokenRefreshResult> DoRefreshAsync(string refreshToken, CancellationToken cancellationToken)
		{
			...
			RefreshTokenResult r = await oidcClient.RefreshTokenAsync(refreshToken, cancellationToken: cancellationToken);
			return new TokenRefreshResult(r.IsError, r.Error, r.ErrorDescription, r.AccessToken, r.RefreshToken, r.AccessTokenExpiration);
		}

		/// <summary>
		/// Acquire machine-wide lock for the refresh operation. Virtual for testability.
		/// </summary>
		protected virtual async Task<IDisposable> AcquireRefreshLockAsync(CancellationToken cancellationToken)
		{
			using CancellationTokenSource timeoutCts = new(TimeSpan.FromSeconds(LockAcquireTimeoutSeconds));
			...
			try
			{
				handle = await SingleInstanceMutex.AcquireAsync(@"Global\OidcRefreshLock", linkedCts.Token);
			}
			catch (OperationCanceledException) when (timeoutCts.IsCancellationRequested && !cancellationToken.IsCancellationRequested)
			{
				throw new TimeoutException($"Timed out after {LockAcquireTimeoutSeconds}s waiting to acquire OIDC refresh lock.");
			}

			_logger?.LogInformation("Acquired refresh lock for '{Name}' (waited {Elapsed}ms).", _name, sw.ElapsedMilliseconds);
			return handle;
		}

		private async Task<OidcTokenInfo?> TryDoRefreshTokenAsync(string refreshToken, CancellationToken cancellationToken)
		{
			try
			{
				_logger?.LogDebug("Starting OIDC refresh HTTP call for '{Name}' (token: {Fingerprint}).",
					_name, TokenUtils.Fingerprint(refreshToken));

				TokenRefreshResult result = await DoRefreshAsync(refreshToken, cancellationToken);
				_logger?.LogDebug("OIDC refresh HTTP call completed for '{Name}'. IsError={IsError}.", _name, result.IsError);

				// Fallback for non-cooperating processes (old binaries without new lock)
				if (result is { IsError: true, Error: "invalid_grant" })
				{
					_logger?.LogWarning("Refresh token for provider '{Name}' returned invalid_grant. May have been rotated by another process.", _name);

					if (_tokenStore.TryGetRefreshToken(_name, out string? freshToken) &&
					    !String.Equals(freshToken, refreshToken, StringComparison.Ordinal))
					{
						_logger?.LogInformation("Found rotated refresh token for provider '{Name}'. Retrying.", _name);
						result = await DoRefreshAsync(freshToken, cancellationToken);
					}
				}
				...
				if (result.IsError)
				{
					...
					{
						_logger?.LogWarning("Refresh token for provider '{Name}' still invalid. User must re-authenticate.", _name);
						return null;
					}
					_logger?.LogWarning("Refresh token error for provider '{Name}': {Error} - {Description}", _name, result.Error, result.ErrorDescription);
					throw new Exception($"Error using the refresh token: {result.Error} , details: {result.ErrorDescription}");
				}

				// Persist new token to disk immediately
				if (String.IsNullOrEmpty(result.RefreshToken))
				{
					throw new Exception("No refresh token was provided in response to refresh.");
				}
				_logger?.LogDebug("Persisting new refresh token to store for '{Name}' (token: {Fingerprint}).",
					_name, TokenUtils.Fingerprint(result.RefreshToken));
				_tokenStore.AddRefreshToken(_name, result.RefreshToken);
				_logger?.LogDebug("Refresh token persisted successfully for '{Name}'.", _name);

				// Cache access token in memory (short-lived)
				_accessToken = result.AccessToken;
				_tokenExpiry = result.AccessTokenExpiration;

				_logger?.LogInformation("Successfully refreshed token for provider '{Name}'. New expiry: {Expiry}", _name, result.AccessTokenExpiration);
			...
			{
				_logger?.LogWarning(
					"OIDC refresh for '{Name}' timed out after {TimeoutSeconds}s (token: {Fingerprint}). "
					+ "If Okta already rotated the token, the old token on disk may now be invalid.",
					_name, RefreshHttpTimeoutSeconds, TokenUtils.Fingerprint(refreshToken));
				throw;
			}
		}
		...
		private async Task<DiscoveryDocumentResponse> GetDiscoveryDocumentAsync(CancellationToken cancellationToken)
		{
			...
			using DiscoveryDocumentRequest doc = new()
			{
				Address = discoUrl,
				Policy =
				{
		...
		public async Task<IOidcTokenInfo> GetAccessTokenAsync(CancellationToken cancellationToken)
		{
			return await TryGetAccessTokenAsync(cancellationToken) ?? throw new NotLoggedInException();
		}

		public async Task<IOidcTokenInfo?> TryGetAccessTokenAsync(CancellationToken cancellationToken)
		{
			// Unlocked fast-path check; authoritative read happens inside lock below
			if (!_tokenStore.TryGetRefreshToken(_name, out string? refreshToken) || String.IsNullOrEmpty(refreshToken))
			{
				_logger?.LogDebug("No refresh token for provider '{Name}'.", _name);
				throw new NotLoggedInException();
			}

			// Check cached access token (short-lived, OK to cache)
			...
			_logger?.LogInformation("Access token for provider '{Name}' expired or expiring. Attempting refresh.", _name);

			// Machine-wide lock - only one process refreshes at a time
			using IDisposable refreshLock = await AcquireRefreshLockAsync(cancellationToken);

			// Re-read from disk inside lock — another process/thread may have refreshed while we waited
			if (!_tokenStore.TryGetRefreshToken(_name, out refreshToken) || String.IsNullOrEmpty(refreshToken))
			{
				throw new NotLoggedInException();
			}

			// If another in-process caller refreshed while we waited for the lock, skip the redundant rotation.
			OidcTokenInfo freshCheck = new()
			{
				RefreshToken = refreshToken,
				AccessToken = _accessToken,
				TokenExpiry = _tokenExpiry
			};
			if (freshCheck.IsValid(DateTime.Now.AddMinutes(-2)))
			{
				_logger?.LogInformation("Access token for '{Name}' became valid while waiting for lock. Skipping refresh.", _name);
				return freshCheck;
			}

			// Once committed to refresh: only a timeout can cancel, NOT external cancellation.
			// Prevents losing a rotated refresh token if the caller cancels mid-HTTP-call (e.g., process shutdown via BackgroundTask disposal).
			if (cancellationToken.IsCancellationRequested)
			{
				_logger?.LogInformation("Cancellation requested before starting refresh for '{Name}'. Releasing lock without refreshing.", _name);
				cancellationToken.ThrowIfCancellationRequested();
			}

			_logger?.LogInformation("Starting refresh for '{Name}' with timeout-only cancellation ({Timeout}s)", _name, RefreshHttpTimeoutSeconds);

			using CancellationTokenSource refreshCts = new();
			refreshCts.CancelAfter(TimeSpan.FromSeconds(RefreshHttpTimeoutSeconds));

			return await TryDoRefreshTokenAsync(refreshToken, refreshCts.Token);
		}

		public OidcStatus GetStatus()
		{
			_tokenStore.TryGetRefreshToken(_name, out string? rt);
			return GetStatus(rt, _accessToken, _tokenExpiry);
		}
