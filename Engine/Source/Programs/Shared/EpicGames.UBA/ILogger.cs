...
namespace EpicGames.UBA
{
	...
	public interface ILogger : IBaseInterface
	{
		...
		/// <param name="makeGlobal">Set this log as the one to be used by all sessions</param>
		/// <returns>The ILogger</returns>
		public static ILogger CreateLogger(Microsoft.Extensions.Logging.ILogger logger, bool makeGlobal)
		{
			try
			{
				return new LoggerImpl(logger, makeGlobal);
			}
			catch
			{
		...
		/// Creates null logger
		/// </summary>
		public static ILogger CreateNullLogger() => new NullLogger();

		/// <summary>
		/// Get the global trace if there is one
		/// </summary>
		public static ILogger? GlobalLogger => LoggerImpl.GlobalLoggerImpl;
	}
}