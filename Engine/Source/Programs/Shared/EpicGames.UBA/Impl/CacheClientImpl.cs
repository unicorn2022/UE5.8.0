...
namespace EpicGames.UBA.Impl
{
	internal class CacheClientImpl : ICacheClient
	{
		...
		public void PopulatePathHashes()
		{
			foreach (KeyValuePair<DirectoryReference, string> item in Utils.PathHashes)
			{
				CacheClient_RegisterPathHash(_handle, item.Key.FullName, item.Value);
			}
		}
