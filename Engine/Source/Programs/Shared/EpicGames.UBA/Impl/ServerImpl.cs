...
namespace EpicGames.UBA.Impl
{
	internal class ServerImpl : IServer
	{
		...
		[DllImport("UbaHost", CharSet = CharSet.Auto)]
		static extern byte NetworkServer_SetClientsConfig(nint server, nint config);
		...
		/// <inheritdoc/>
		public void SetClientsConfig(IConfig config)
		{
			NetworkServer_SetClientsConfig(_handle, config.GetHandle());
		}
	...
	{
		/// <inheritdoc/>
		public bool StartServer(string ip = "", int port = -1, string crypto = "") => false;
		/// <inheritdoc/>
		public void StopServer() { }
		/// <inheritdoc/>
		public bool AddNamedConnection(string name) => false;
		/// <inheritdoc/>
		public bool AddClient(string ip, int port, string crypto = "") => false;
		/// <inheritdoc/>
		public void SetClientsConfig(IConfig config) { }
		/// <inheritdoc/>
