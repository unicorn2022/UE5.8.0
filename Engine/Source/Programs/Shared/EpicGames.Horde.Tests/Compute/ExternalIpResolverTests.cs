...
public class ExternalIpResolverTests
{
	...
	public async Task BasicAsync()
	{
		...
		await Assert.ThrowsExactlyAsync<ExternalIpResolverException>(() => GetIpAsync("bad-ip"));
		await Assert.ThrowsExactlyAsync<ExternalIpResolverException>(() => GetIpAsync("192.168.2.3", HttpStatusCode.NotFound));
	}
