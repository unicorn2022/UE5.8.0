...
namespace EpicGames.Horde.Tests.Storage
{
	...
	public class WorkspaceTests
	{
		...
		{
			DirectoryInfo tempDir = new($"temp/{callerName ?? Guid.NewGuid().ToString()}");
			tempDir.Create();
		...
		{
			await using MemoryBlobWriter blobWriter = new(new BlobSerializerOptions());
		...
		public async Task LayerTestAsync()
		{
			...
			WorkspaceLayerId layerA = new("layer-a");
			ws.AddLayer(layerA);

			WorkspaceLayerId layerB = new("layer-b");
			ws.AddLayer(layerB);

			// Add the files to layer A
			WorkspaceSyncStats stats = new();
			await ws.SyncAsync(layerA, initialState, stats: stats);
			Assert.AreEqual(0, stats.NumFilesKept);
			Assert.AreEqual(10, stats.NumBytesDownloaded);

			// Add the same files to layer B
			stats = new WorkspaceSyncStats();
			await ws.SyncAsync(layerB, initialState, stats: stats);
			Assert.AreEqual(2, stats.NumFilesKept);
			Assert.AreEqual(0, stats.NumBytesDownloaded);
		...
		public async Task SwapTestAsync()
		{
			...
			WorkspaceSyncStats syncStats = new();
