...
namespace EpicGames.Horde.Tests.Storage
{
	...
	public class ChunkingTests
	{
		...
		public async Task ChunkingCompatV2Async()
		{
			...
			{
				using ChunkedDataWriter chunkedWriter = new(writer, chunkingOptions);
		...
		public async Task HashAsync()
		{
			...
			using (ChunkedDataWriter writer = new(blobWriter, chunkingOptions))
			{
		...
		{
			InteriorChunkedDataNodeOptions interiorOptions = new(2, 2, 2);
			...
			ChunkedDataNodeRef leafChunkedRef = new(3, 0, leafRef);

			List<ChunkedDataNodeRef> leafNodeRefs = [.. Enumerable.Repeat(leafChunkedRef, 10000)];
