...
namespace EpicGames.Horde.Tests.Storage
{
	...
	public class BlobDataTests
	{
		...
		{
			List<IBlobRef> refs = [new DummyHandle("hello"), new DummyHandle("world")];

			using BlobData blobData = new(new BlobType(Guid.NewGuid(), 4), new byte[] { 1, 2, 3 }, refs);

			byte[] data = EncodedBlobData.Create(blobData);
			EncodedBlobData packet = new(data);
