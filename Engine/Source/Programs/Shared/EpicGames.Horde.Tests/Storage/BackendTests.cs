...
namespace EpicGames.Horde.Tests.Storage
{
	...
	public class BackendTests
	{
		...
		{
			MemoryStorageBackend backend = new();
		...
		{
			using (TempDir tempDir = new("Cache-" + DateTime.UtcNow.Ticks))
			{
				using MemoryMappedFileCache memoryMappedFileCache = new();
				FileStorageBackend backend = new(tempDir.Location, memoryMappedFileCache, NullLogger.Instance);
				await TestBackendAsync(backend);
			}
		}
		...
		public async Task TestCacheBackendAsync()
		{
			using (TempDir tempDir = new("Cache-" + DateTime.UtcNow.Ticks))
			{
				using StorageBackendCache cache = new(tempDir.Location, 12, NullLogger.Instance);

				MemoryStorageBackend memoryBackend = new();
				...
				HashSet<BlobLocator> paths = [.. cache.GetLocators()];
