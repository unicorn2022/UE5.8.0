...
public class HordeHttpAuthHandlerTests
{
	...
	private (HttpClient client, FakeServerWithAuth server) CreateClientServer(FakeOidcTokenManager oidcTokenManager)
	{
		...
		ILogger<OidcAuthState> logger = loggerFactory.CreateLogger<OidcAuthState>();
		IOidcAuthStateConfig oidcAuthStateConfig = new OidcAuthStateConfig("Horde");
		OidcAuthState state = new(oidcTokenManager, new NullStaticAccessTokenProvider(), _clock, oidcAuthStateConfig, logger);
		HttpOidcAuthHandler authHandler = new(server, state, new HttpOidcAuthHandlerConfig("Horde-AllowInteractiveLogin"));
