...
namespace EpicGames.Horde.Tests
{
	...
	public class ServerLogPacketBuilderTests
	{
		...
		{
			JsonLoggerImpl impl = new();
			impl.LogInformation("Hello {Text}", "world");
			impl.LogInformation("Hello {Text}", "world");

			Assert.AreEqual(2, impl._lines.Count);
			Assert.AreEqual("Hello world", impl._lines[0].ToString());
			Assert.AreEqual("world", impl._lines[0].GetProperty("Text")?.ToString());
			Assert.AreEqual("Hello world", impl._lines[1].ToString());
			Assert.AreEqual("world", impl._lines[1].GetProperty("Text")?.ToString());
		}
		...
		public void MultiLineMessageTest()
		{
			...
			Assert.AreEqual(@"{""level"":""Information"",""message"":""This is "",""line"":0,""lineCount"":2}", result[0]);
			Assert.AreEqual(@"{""level"":""Information"",""message"":""a multi-line string"",""line"":1,""lineCount"":2}", result[1]);
		}
		...
		{
			await using BundleCache cache = new();
		...
		{
			DateTime time = new(2023, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc);
			...
			LogEvent baseEvent = new(time, LogLevel.Information, default, $"start {longLineA} x {longLineB} y\nz {longLineC} end", "start {LongLineA} x {LongLineB} y\nz {LongLineC} end", properties, null);
		...
		public void MaxPacketLengthTest()
		{
			...
			LogEvent baseEvent = new(time, LogLevel.Information, default, "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ", null, null, null);

			ServerLogPacketBuilder writer = new(maxPacketLength: 85);
			writer.SanitizeAndWriteEvent(new JsonLogEvent(baseEvent));
			writer.SanitizeAndWriteEvent(new JsonLogEvent(baseEvent));
		...
		public void MaxLineLengthTest()
		{
			...
			ServerLogPacketBuilder writer = new(85);
			writer.SanitizeAndWriteEvent(new JsonLogEvent(baseEvent));
