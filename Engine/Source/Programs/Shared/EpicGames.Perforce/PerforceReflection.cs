...
namespace EpicGames.Perforce
{
	...
	{
		public static readonly Utf8String True = new("true");
		public static readonly Utf8String False = new("false");
		public static readonly Utf8String New = new("new");
		public static readonly Utf8String None = new("none");
		public static readonly Utf8String Default = new("default");
	}
	...
	class TaggedPropertyInfo
	{
		...
		{
			StringBuilder fullName = new(Tag.ToString());
	...
	class CachedRecordInfo
	{
		...
		/// </summary>
		public List<TaggedPropertyInfo> Properties { get; } = [];

		/// <summary>
		/// Map of name to tag info
		/// </summary>
		public Dictionary<TaggedPropertyNameAndRank, TaggedPropertyInfo> NameAndRankToInfo { get; set; } = [];
	...
	class CachedEnumInfo
	{
		...
		/// </summary>
		public Dictionary<Utf8String, int> _nameToValue = [];

		/// <summary>
		/// Map of value to name
		/// </summary>
		public Dictionary<int, Utf8String> _valueToName = [];

		/// <summary>
		/// List of name/value pairs
		/// </summary>
		public List<KeyValuePair<string, int>> _nameValuePairs = [];
		...
		public CachedEnumInfo(Type enumType)
		{
			...
			foreach (FieldInfo field in fields)
			{
				...
				if (attribute != null)
				{
					...
					if (value != null)
					{
						Utf8String name = new(attribute.Name);
	...
	static class PerforceReflection
	{
		...
		/// </summary>
		static readonly ConcurrentDictionary<Type, CachedEnumInfo> s_enumTypeToInfo = new();

		/// <summary>
		/// Cached set of record 
		/// </summary>
		static readonly ConcurrentDictionary<Type, CachedRecordInfo> s_recordTypeToInfo = new();
		...
		public static CachedRecordInfo GetCachedRecordInfo(Type recordType)
		{
			...
			if (!s_recordTypeToInfo.TryGetValue(recordType, out record))
			{
				...
				AddRecordProperties(recordType, record, []);
		...
		static void AddRecordProperties(Type recordType, CachedRecordInfo rootRecord, NestedRecordInfo[] parentRecords)
		{
			...
			foreach (PropertyInfo property in properties)
			{
				...
				if (tagAttribute != null)
				{
					...
					TaggedPropertyInfo tagInfo = new(new Utf8String(tagName), tagAttribute.Optional, parentRecords, property, requiredTagBitMask);
					...
					{
						tagInfo.Write = (writer, value) => WriteUtf8StringWithTag(writer, new Utf8String(((long)((DateTime)value - UnixEpoch).TotalSeconds).ToString()));
					...
					{
						tagInfo.ReadFromString = (obj, value) => propertyCopy.SetValue(obj, ParseStringAsDateTimeOffset(value));
					}
				...
				if (subElementAttribute != null)
				{
					...
					NestedRecordInfo newParentRecord = new(property, GetCreateRecordDelegate(newRecordType));
					AddRecordProperties(newRecordType, rootRecord, [.. parentRecords, newParentRecord]);
				}
			}
		}
		...
		static CreateRecordDelegate GetCreateRecordDelegate(Type type)
		{
			...
			DynamicMethod dynamicMethod = new("_", type, null);
		...
		static string ParseString(Utf8String str)
		{
			return str.ToString();
		}

		static DateTime ParseStringAsDateTime(Utf8String str)
		{
			...
			{
				return UnixEpoch + TimeSpan.FromSeconds(Int64.Parse(text));
			}
		}
		...
		static long ParseStringAsLong(Utf8String str)
		{
			long value;
			int bytesConsumed;
			if (!Utf8Parser.TryParse(str.Span, out value, out bytesConsumed) || bytesConsumed != str.Length)
		...
		static DateTimeOffset ParseStringAsDateTimeOffset(Utf8String str)
		{
			string text = str.ToString();
