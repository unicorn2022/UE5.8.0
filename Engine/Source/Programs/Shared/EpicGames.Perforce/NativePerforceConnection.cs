...
namespace EpicGames.Perforce
{
	...
	public sealed class NativePerforceConnection : IPerforceConnection, IDisposable
	{
		...
		[DllImport(NativeDll, CallingConvention = CallingConvention.Cdecl)]
		static extern void Client_CancelCommand(IntPtr client);
		...
		{
			static readonly byte[] s_guardBytes = [.. Enumerable.Repeat<byte>(0xfd, 16)];
		...
		class Response : IPerforceOutput
		{
			...
			async Task<(PinnedBuffer?, int)> GetNextReadBufferAsync(CancellationToken token = default)
			{
				for (; ; )
				{
					if (!await _readBuffers.Reader.WaitToReadAsync(token))
					{
						return (null, 0);
					}
			...
			{
				if (!_cancelRegistered && token.CanBeCanceled)
				{
					_cancelRegistered = true;
					IntPtr clientPtr = _outer._client;
					_cancelRegistration = token.Register(() => Client_CancelCommand(clientPtr));
				}

				// If we don't have any data yet, wait until a read completes
				if (_buffer == null)
				{
					(_buffer, _bufferLen) = await GetNextReadBufferAsync(token);
					return _buffer != null;
				}
				...
				if (_nextBuffer == null)
				{
					(_nextBuffer, _nextBufferLen) = await GetNextReadBufferAsync(token);
					if (_nextBuffer == null)
					{
						return false;
					}
		...
		readonly BlockingCollection<(Action, Response)?> _requests = [];
		readonly BlockingCollection<PinnedBuffer> _writeBuffers = [];
		Response? _currentResponse;
		readonly ManualResetEvent _responseCompleteEvent;
		readonly Stopwatch _stallTimer = new();
		...
		public NativePerforceConnection(IPerforceSettings settings, ILogger logger, Tracer? tracer = null)
			: this(settings, 2, 128 * 1024, logger, tracer)
		{
		}
		...
		/// <param name="logger">Logger for messages</param>
		/// <param name="tracer">Tracer</param> 
		public NativePerforceConnection(IPerforceSettings settings, int bufferCount, int bufferSize, ILogger logger, Tracer? tracer = null)
		{
			Settings = settings;
			Logger = logger;
			Tracer = tracer ?? PerforceConnection.DefaultTracer;
		...
		/// </summary>
		public static async Task<NativePerforceConnection> CreateAsync(IPerforceSettings settings, ILogger logger, Tracer? tracer = null)
		{
			NativePerforceConnection? connection = null;
			try
			{
				connection = new NativePerforceConnection(settings, logger, tracer);
		...
		{
			await using Response response = new(this);
			...
			_requests.Add((() =>
			{
				NativeWriteBuffer writeBuffer = new();
		...
		public IPerforceOutput Command(string command, IReadOnlyList<string> arguments, IReadOnlyList<string>? fileArguments, byte[]? inputData, string? promptResponse, bool interceptIo)
		{
			...
			List<string> allArguments = [.. arguments];
			if (fileArguments != null)
			{
				allArguments.AddRange(fileArguments);
			}

			Response response = new(this);
		...
		private void ExecCommand(string command, List<string> args, byte[]? inputData, string? promptResponse, bool interceptIo)
		{
			...
			try
			{
				...
				try
				{
					Client_Command(_client, command, nativeArgs.Count, [.. nativeArgs], inputData, inputData?.Length ?? 0, promptResponseBytes, interceptIo);
				}
				catch (Exception ex)
				{
