...
namespace EpicGames.Perforce.Managed
{
	...
	/// <param name="UseModTime">Whether to set a synced file's timestamp to match P4 stream/depot timestamp ("modtime" client option)</param>
	...
	{
		/// <inheritdoc cref="ManagedWorkspaceOptions(Int32, Int32, Int32, Int64, Boolean, Boolean, Boolean, Boolean)"/>
		public ManagedWorkspaceOptions(int? numParallelSyncThreads = null, int? maxFileConcurrency = null, int? maxFileScanConcurrency = null, long minScratchSpace = 50L * 1024, bool useHaveTable = true, bool partitioned = false, bool preferNativeClient = false, bool useModTime = false)
			: this(
	...
	public class ManagedWorkspace
	{
		...
		/// </summary>
		readonly Dictionary<string, ClientRecord> _createdClients = [];

		/// <summary>
		/// Set of unique cache entries. We use this to ensure new names in the cache are unique.
		/// </summary>
		readonly HashSet<ulong> _cacheEntries = [];
		...
		/// </summary>
		Dictionary<FileContentId, CachedFileInfo> _contentIdToTrackedFile = [];
		...
		*/
		...
		public static async Task<ManagedWorkspace> CreateAsync(string hostName, DirectoryReference baseDir, ManagedWorkspaceOptions options, Tracer tracer, ILogger logger, CancellationToken cancellationToken)
		{
			...
			ManagedWorkspace repo = new(hostName, 1, baseDir, options, tracer, logger);
			...
			using (BinaryWriter writer = new(File.Open(signatureFile.FullName, FileMode.Create, FileAccess.Write, FileShare.Read)))
			{
		...
		public static bool Exists(DirectoryReference baseDir)
		{
			...
			{
				using (BinaryReader reader = new(File.Open(signatureFile.FullName, FileMode.Open, FileAccess.Read, FileShare.Read)))
				{
		...
		public static async Task<ManagedWorkspace> LoadAsync(string hostName, DirectoryReference baseDir, ManagedWorkspaceOptions options, Tracer tracer, ILogger logger, CancellationToken cancellationToken)
		{
			...
			MemoryReader reader = new(data.AsMemory());
		...
		private async Task SaveAsync(TransactionState state, CancellationToken cancellationToken)
		{
			...
			MemoryWriter writer = new(buffer.AsMemory());
		...
		private async Task CleanInternalAsync(bool removeUntracked, CancellationToken cancellationToken)
		{
			...
			using (ILoggerProgress status = _logger.BeginProgressScope("Finding files to clean..."))
			{
				...
				(FileInfo[] filesToDelete, DirectoryInfo[] directoriesToDelete) refreshResult = await _workspace.RefreshAsync(removeUntracked, _options.MaxFileScanConcurrency);
		...
		public void Dump()
		{
			...
			WorkspaceFileInfo[] workspaceFiles = [.. _workspace.GetFiles().OrderBy(x => x.GetLocation().FullName)];
		...
		public async Task RepairCacheAsync(CancellationToken cancellationToken)
		{
			...
			using (ILoggerProgress status = _logger.BeginProgressScope("Checking cache..."))
			{
				...
				List<CachedFileInfo> trackedFiles = [.. _contentIdToTrackedFile.Values];
		...
		public async Task PurgeAsync(long maxSize, CancellationToken cancellationToken)
		{
			...
			using (_logger.BeginIndentScope("  "))
			{
				List<CachedFileInfo> cachedFiles = [.. _contentIdToTrackedFile.Values.OrderBy(x => x.SequenceNumber)];
			...
			using (_logger.BeginIndentScope("  "))
			{
				...
				using (ILoggerProgress scope = _logger.BeginProgressScope("Finding usage stats..."))
				{
					...
					for (int idx = 0; idx < streamNames.Count; idx++)
					{
						...
						filesInStream[idx] = [.. files.Select(x => x.ContentId)];
					}
		...
		public async Task StatusAsync()
		{
			...
			string[] differences = await _workspace.FindDifferencesAsync(_options.MaxFileScanConcurrency);
		...
		public async Task UnshelveAsync(IPerforceConnection perforce, int unshelveChangelist, CancellationToken cancellationToken)
		{
			...
			// Both written and deleted files must be flagged so the next sync re-syncs them

			Stopwatch timer = Stopwatch.StartNew();
			_logger.LogInformation("Unshelving changelist {Change}...", unshelveChangelist);

			// query the contents of the shelved changelist
			List<DescribeRecord> records = await perforce.DescribeAsync(DescribeOptions.Shelved, -1, [unshelveChangelist], cancellationToken);
			if (records.Count != 1)
			{
			...
			// Add all modified and deleted files to the workspace with invalid metadata. This will ensure they're re-synced next time.
			List<WhereRecord> invalidateFiles = [..writeFiles, ..deleteFiles];
			if (invalidateFiles.Count > 0)
			{
				_logger.LogInformation("Invalidating {NumFiles} files in tracked workspace", invalidateFiles.Count);
				foreach (WhereRecord file in invalidateFiles)
				{
					string path = Regex.Replace(file.ClientFile, "^//[^/]+/", "");
			...
			{
				_logger.LogWarning("Unable to print shelved changelist: {Error}", printResponse.FirstError?.ToString());
			}
		...
		public async Task<Tuple<int, StreamSnapshot>[]> PopulateCleanAsync(List<PopulateRequest> requests, CancellationToken cancellationToken)
		{
			...
			using (ILoggerProgress status = _logger.BeginProgressScope("Updating cache..."))
			{
				...
				HashSet<FileContentId> commonContentIds = [];
				Dictionary<FileContentId, long> contentIdToLength = [];
				for (int idx = 0; idx < requests.Count; idx++)
				{
					List<StreamFile> files = streamState[idx].Item2.GetFiles();
		...
		private async Task<ClientRecord> GetOrCreateClientAsync(IPerforceConnection perforceClient, string streamName, CancellationToken cancellationToken)
		{
			...
			if (_options.UseModTime)
			{
				client.Options = ClientOptions.ModTime;
			}
		...
		private async Task RevertInternalAsync(IPerforceConnection perforceClient, CancellationToken cancellationToken)
		{
			...
			using (ILoggerProgress status = _logger.BeginProgressScope("Reverting changes..."))
			{
				...
				if (openedFilesResponse.Count != 0)
				{
				...
				foreach (ChangesRecord change in changes)
				{
					...
					List<DescribeRecord> describeResponse = await perforceClient.DescribeAsync(DescribeOptions.Shelved, -1, [change.Number], cancellationToken);
		...
		private async Task<StreamSnapshotFromMemory> FindClientContentsAsync(IPerforceConnection perforceClient, int changeNumber, CancellationToken cancellationToken)
		{
			...
			using (ILoggerProgress scope = _logger.BeginProgressScope("Fetching metadata..."))
			{
				...
				Utf8String clientPrefix = new($"//{perforceClient.Settings.ClientName}/");
				...
				List<(Utf8String, StreamTreeBuilder)> fragments = [];

				int numFiles = 0;

				// Handler for each returned record
				FStatIndexedRecord record = new();
				void HandleRecord(PerforceRecord rawRecord)
				{
					...
					for (int fragmentIdx = 0; ; fragmentIdx++)
					{
						...
						Utf8String fragment = new(clientFile.Memory.Slice(fragmentMinIdx, fragmentMaxIdx - fragmentMinIdx));
					...
					FileContentId contentId = new(digest, record.HeadType.Clone());
				...
				[
					"-Ol", // Output fileSize and digest field
					"-Op", // Output clientFile field in both server and local path syntax
					"-Os", // Shorten output by excluding client workspace data (for instance, the clientFile field).
					"-Rh", // Limit output to files on your have list;
					"-T", // Include only the fields listed below
					String.Join(",", FStatIndexedRecord.FieldNames),
					$"//{perforceClient.Settings.ClientName}/...@{changeNumber}",
				];
		...
		public async Task<StreamSnapshotFromMemory> FindClientContentsWithoutHaveTableAsync(IPerforceConnection perforceClient, string streamName, IReadOnlyList<string> view, int changeNumber, CancellationToken cancellationToken)
		{
			...
			using (ILoggerProgress scope = _logger.BeginProgressScope("Fetching metadata (without have table)..."))
			{
				...
				[
					"-Ol", // Output fileSize and digest field
					"-Os", // Shorten output by excluding client workspace data (for instance, the clientFile field).
					"-F", // Filter any files not existing at current revision (filter below)
					"^headAction=delete&^headAction=move/delete&^headAction=purge",
					"-T", // Include only the fields listed below
					String.Join(",", FStatRecordWithoutHaveTable.FieldNames),
					fileSpec,
				];
		...
		private async Task<StreamSnapshotFromMemory> FindAndSaveClientContentsAsync(IPerforceConnection perforceClient, Utf8String basePath, IReadOnlyList<string> view, int changeNumber, FileReference cacheFile, CancellationToken cancellationToken)
		{
			...
			using (ILoggerProgress scope = _logger.BeginProgressScope($"Saving metadata to {cacheFile}..."))
			{
				...
				FileReference tempCacheFile = new(String.Format("{0}.{1}", cacheFile, Guid.NewGuid()));
		...
		private async Task RemoveFilesFromWorkspaceAsync(StreamSnapshot contents, CancellationToken cancellationToken)
		{
			...
			using (ILoggerProgress scope = _logger.BeginProgressScope("Gathering files to remove..."))
			{
				...
				transaction = await RemoveTransaction.CreateAsync(_workspace, contents, _contentIdToTrackedFile, _options.MaxFileScanConcurrency);

				scope.Progress = $"({timer.Elapsed.TotalSeconds:0.0}s)";
			}

			// Move files into the cache
			KeyValuePair<FileContentId, WorkspaceFileInfo>[] filesToMove = [.. transaction._filesToMove];
			if (filesToMove.Length > 0)
			{
				...
				using (ILoggerProgress scope = _logger.BeginProgressScope(String.Format("Moving {0} {1} to cache...", filesToMove.Length, (filesToMove.Length == 1) ? "file" : "files")))
				{
					...
					List<(FileContentId ContentId, FileReference Source, FileReference Target)> files = [];
					foreach (KeyValuePair<FileContentId, WorkspaceFileInfo> fileToMove in filesToMove)
					{
						...
						CachedFileInfo newTrackingInfo = new(_cacheDir, fileToMove.Key, cacheId, fileToMove.Value._length, fileToMove.Value._lastModifiedTicks, fileToMove.Value._readOnly, _nextSequenceNumber);
			...
			WorkspaceFileInfo[] filesToDelete = [.. transaction._filesToDelete];
			...
			WorkspaceDirectoryInfo[] directoriesToDelete = [.. transaction._directoriesToDelete];
		...
		private async Task AddFilesToWorkspaceAsync(IPerforceConnection client, StreamSnapshot stream, bool fakeSync, CancellationToken cancellationToken)
		{
			...
			using (ILoggerProgress status = _logger.BeginProgressScope("Gathering files to add..."))
			{
				...
				transaction = await AddTransaction.CreateAsync(_workspace, stream, _contentIdToTrackedFile, _options.MaxFileScanConcurrency);
				_workspace = transaction._newWorkspaceRootDir;
				await SaveAsync(TransactionState.Dirty, cancellationToken);
			...
			WorkspaceFileToMove[] filesToMove = [.. transaction._filesToMove.Values];
			if (filesToMove.Length > 0)
			{
			...
			// Swap files in and out of the cache
			WorkspaceFileToCopy[] filesToCopy = [.. transaction._filesToCopy];
			...
			WorkspaceFileToSync[] filesToSync = [.. transaction._filesToSync];
			if (filesToSync.Length > 0)
			{
				...
				{
					ulong freeSpace;
					bool success = GetDiskFreeSpaceEx(_baseDir.FullName, out _, out _, out freeSpace);
					if (!success)
					{
						throw new InsufficientSpaceException($"Can't get free space for {_baseDir.FullName}");
					}
					long minScratchSpaceBytes = _options.MinScratchSpace * 1024 * 1024;
					if ((long)freeSpace - syncSize < minScratchSpaceBytes)
					{
				...
				using (ILoggerProgress status = _logger.BeginProgressScope(String.Format("Syncing {0} {1} using {2} threads...", filesToSync.Length, (filesToSync.Length == 1) ? "file" : "files", _options.NumParallelSyncThreads)))
				{
					...
					Dictionary<Task, int> tasks = [];
					Stack<IPerforceConnection> connectionPool = new();
					try
					{
						while (tasks.Count > 0 || nextBatchIdx < batches.Count)
						{
							...
							int[] indexesToUpdate = [.. Enumerable.Range(beginIdx, endIdx - beginIdx)];
							ParallelOptions options = new() { MaxDegreeOfParallelism = _options.MaxFileScanConcurrency, CancellationToken = cancellationToken };
		...
		async Task SyncBatchAsync(IPerforceConnection client, WorkspaceFileToSync[] filesToSync, int beginIdx, int endIdx, bool fakeSync, Stack<IPerforceConnection> connectionPool, CancellationToken cancellationToken)
		{
			...
			if (fakeSync)
			{
				for (int idx = beginIdx; idx < endIdx; idx++)
				{
					...
					await FileReference.WriteAllBytesAsync(localFile, [], cancellationToken);
				}
			}
...
#pragma warning restore CA2000
			...
			else
			{
				...
				using (StreamWriter writer = new(syncFileName.FullName))
				{
					for (int idx = beginIdx; idx < endIdx; idx++)
					{
		...
		void MoveFileFromCache(WorkspaceFileToMove fileToMove, ConcurrentQueue<WorkspaceFileToSync> filesToSync)
		{
			try
			{
				...
				try
				{
					DateTime lastWriteTime = _options.UseModTime
						? new DateTime(fileToMove._workspaceFile._lastModifiedTicks, DateTimeKind.Utc)
						: DateTime.UtcNow;

					FileReference.SetLastWriteTimeUtc(targetFile, lastWriteTime);
