...
namespace EpicGames.Tracing.UnrealInsights.Events
{
	...
	{
		public static readonly EventType EventType = new(0, "CpuProfiler", "EventSpec", EventType.FlagImportant | EventType.FlagMaybeHasAux | EventType.FlagNoSync,
			[
	...
	{
		public static readonly EventType EventType = new(0, "CpuProfiler", "EventBatch", EventType.FlagMaybeHasAux | EventType.FlagNoSync,
			[new EventTypeField(0, 0, EventTypeField.TypeArray, "Data")]);
	...
	public class CpuProfilerSerializer
	{
		...
		public List<CpuProfilerScopeEvent> ScopeEvents { get; } = [];
		...
		{
			using MemoryStream ms = new(eventBatchData);
			...
			while (reader.BaseStream.Position != reader.BaseStream.Length)
			{
				...
				if (isEnterScope)
				{
					...
					CpuProfilerScopeEvent scopeEvent = new(timestamp, true, (uint?)specId);
					ScopeEvents.Add(scopeEvent);
				}
				else
				{
					CpuProfilerScopeEvent scopeEvent = new(timestamp, false, null);
					ScopeEvents.Add(scopeEvent);
				}
			}
		}
		...
		public List<byte[]> Write()
		{
			...
			return [.. streams.Select(x => x.ToArray())];
		}
