...
public class ManagedWorkspaceTest : BasePerforceFixtureTest
{
	...
	public async Task SyncWithViewExclusivePathFirstAsync(bool useHaveTable)
	{
		...
		List<string> view = ["-/Data/...", "-/shared.h",];
		...
		List<DepotFileFixture> filtered = [.. cl.StreamFiles
			...
			.Where(x => !x.DepotFile.Contains("Data/", StringComparison.Ordinal))];
	...
	public async Task SyncWithViewInclusivePathFirstAsync(bool useHaveTable)
	{
		...
		List<string> view = ["/...", "-/Data/..."];
		await ws.SyncAsync(PerforceConnection, StreamName, cl.Number, view, true, false, null, CancellationToken.None);

		List<DepotFileFixture> filtered = [.. cl.StreamFiles.Where(x => !x.DepotFile.Contains("Data/", StringComparison.Ordinal))];
	...
	{
		ManagedWorkspace ws = await CreateManagedWorkspaceAsync(useHaveTable);
		List<PopulateRequest> populateRequests = [new PopulateRequest(PerforceConnection, StreamName, [])];
	...
	public async Task PopulateWithViewAsync(bool useHaveTable)
	{
		...
		List<PopulateRequest> populateRequests = [new PopulateRequest(PerforceConnection, StreamName, view)];
		await ws.PopulateAsync(populateRequests, false, CancellationToken.None);
	...
	{
		ManagedWorkspace mw = await CreateManagedWorkspaceAsync(useHaveTable);
		await SyncAsync(mw, 7);
		await mw.UnshelveAsync(PerforceConnection, 8, CancellationToken.None);
	...
	[DataRow(true, DisplayName = "With have-table")]
	[DataRow(false, DisplayName = "Without have-table")]
	public async Task Unshelve_WithDeletedFiles_NextSync_RestoresDeletedFilesAsync(bool useHaveTable)
	{
		ManagedWorkspace mw = await CreateManagedWorkspaceAsync(useHaveTable);

		// Sync CL 7 (has main.cpp)
		await SyncAsync(mw, 7);
		Stream.GetChangelist(7).AssertDepotFiles(SyncDir);

		// Unshelve CL 8 (deletes main.cpp)
		await mw.UnshelveAsync(PerforceConnection, 8, CancellationToken.None);
		Stream.GetChangelist(8).AssertDepotFiles(SyncDir);

		// Sync CL 7 again - main.cpp should be restored
		await SyncAsync(mw, 7);
		Stream.GetChangelist(7).AssertDepotFiles(SyncDir);
		await Stream.GetChangelist(7).AssertHaveTableAsync(PerforceConnection, useHaveTable);
	}
	...
	public async Task Caching_SyncingNewChangelist_UnusedFilesMovedToCacheAsync(bool useHaveTable)
	{
		...
		PerforceFixture.AssertCacheEquals(CacheDir, [.. Stream.GetChangelist(7).StreamFiles.Select(x => x.Digest)]);
	}
	...
	public async Task Caching_SyncingWithCachedData_FilesPopulatedFromCacheAsync(bool useHaveTable)
	{
		...
		PerforceFixture.AssertCacheEquals(CacheDir, []);
	}

	[TestMethod]
	[DataRow(true, DisplayName = "With have-table")]
	[DataRow(false, DisplayName = "Without have-table")]
	public async Task ModTime_NormalSync_PreservesDepotTimestamp_Async(bool useHaveTable)
	{
		// Arrange
		ManagedWorkspace ws = await CreateManagedWorkspaceAsync(useHaveTable, true);

		// Act
		await SyncAsync(ws, 7);

		// Assert
		ChangelistFixture cl = Stream.GetChangelist(7);
		DepotFileFixture file = cl.GetFile("//Foo/Main/main.cpp")!;
		Assert.IsNotNull(file);
		Assert.AreEqual(await GetDepotFileModTimeAsync(file), GetLocalFileModTime(file));
	}

	[TestMethod]
	[DataRow(true, DisplayName = "With have-table")]
	[DataRow(false, DisplayName = "Without have-table")]
	public async Task ModTime_CachedSync_PreservesDepotTimestamp_Async(bool useHaveTable)
	{
		// Arrange
		ManagedWorkspace ws = await CreateManagedWorkspaceAsync(useHaveTable, true);
		await SyncAsync(ws, 7);
	...
	private DateTime GetLocalFileModTime(DepotFileFixture file)
	{
		FileInfo fi = new(Path.Join(SyncDir, file.ClientFile));
		return fi.LastWriteTimeUtc;
	}

	private async Task<DateTime> GetDepotFileModTimeAsync(DepotFileFixture file)
	{
		List<FStatRecord> fstatRecords = await PerforceConnection.FStatAsync($"{file.DepotFile}#{file.Revision}").ToListAsync();
		Assert.AreEqual(1, fstatRecords.Count);
		return fstatRecords[0].HeadModTime;
	}
	...
	public async Task FixtureMetadataMatchesServerMetadataAsync()
	{
		...
		{
			HashSet<(string clientFile, int rev, long size, string digest)> depotFiles = [];
			...
			HashSet<(string clientFile, int rev, long size, string digest)> fixtureFiles = [];
	...
	private async Task<ManagedWorkspace> CreateManagedWorkspaceAsync(bool useHaveTable, bool useModTime = false)
	{
		ManagedWorkspaceOptions options = new() { UseHaveTable = useHaveTable, UseModTime = useModTime };
	...
	{
		await managedWorkspace.SyncAsync(PerforceConnection, StreamName, changeNumber, [], removeUntracked, false, cacheFile, CancellationToken.None);
		if (assertFiles)
		{
