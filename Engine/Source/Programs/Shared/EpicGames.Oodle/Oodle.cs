...
#pragma warning restore CA1008 // Add a member that has a value of zero with a suggested name of None
	...
	/// </summary>
	public sealed partial class Oodle
	{
...
#pragma warning restore IDE1006
		...
		private static unsafe partial long OodleLZ_Compress(OodleCompressorType compressor, byte* rawBuf, long rawLen, byte* compBuf, OodleCompressionLevel level, long pOptions, byte[]? dictionaryBase, byte[]? lrm, byte* scratchMem, long scratchSize);

		[LibraryImport("oo2core")]
		private static partial long OodleLZ_GetCompressedBufferSizeNeeded(OodleCompressorType compressor, long rawSize);

		[LibraryImport("oo2core")]
		private static partial int OodleLZDecoder_MemorySizeNeeded(OodleCompressorType compressor, long rawLen = -1);

		[DllImport("oo2core")]
		private static extern int OodleLZ_GetCompressScratchMemBound(OodleCompressorType compressor, OodleCompressionLevel level, int rawLen, OodleConfigValues configValues);
		...
		[DllImport("oo2core")]
		private static extern void Oodle_GetConfigValues(OodleConfigValues configValues);

		[DllImport("oo2core")]
		private static extern void Oodle_SetConfigValues(OodleConfigValues configValues);

		private static readonly OodleConfigValues s_configValues = new();

		/// <summary>
		/// Static constructor. Sets the backwards compatibility version for Oodle.
		/// </summary>
		static Oodle()
		{
