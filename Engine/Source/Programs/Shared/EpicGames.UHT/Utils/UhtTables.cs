...
namespace EpicGames.UHT.Utils
{
	...
	public class UhtLookupTables<TTable> where TTable : UhtLookupTableBase, new()
	{
		...
		/// </summary>
		public Dictionary<string, TTable> Tables { get; } = [];
		...
		public void Merge()
		{
			...
			HashSet<UhtLookupTableBase> doneTables = [];
	...
	public static class UhtStandardTables
	{
		...
		public static void InitStandardTables(UhtTables tables)
		{
			...
			CreateTables(tables, UhtTableNames.Partial, "Partial", EUhtCreateTablesFlags.Specifiers | EUhtCreateTablesFlags.Keyword, UhtTableNames.ScriptStruct, false);
		}
	...
	public class UhtTables
	{
		...
		private void CheckForAttributes(Assembly? assembly)
		{
			if (assembly != null)
			{
				foreach (Type type in assembly.SafeGetLoadedTypes())
				{
					...
					if (type == typeof(UhtType) || type.IsSubclassOf(typeof(UhtType)))
					{
		...
		/// <summary>
		/// Verify that all the known types support caching
		/// </summary>
		/// <param name="logger"></param>
		/// <exception cref="UhtException"></exception>
		public void ValidateCacheableTypes(ILogger logger)
		{
			bool hasBadType = false;
			foreach (Type type in _knownTypes)
			{
				if (type != typeof(UhtPackage) && !type.IsAbstract)
				{
					ConstructorInfo? constructor = type.GetConstructor([typeof(UhtInputCacheReader), typeof(UhtType)]);
					if (constructor == null)
					{
						logger.LogError("The UhtType '{TypeName}' must defined the constructor '{ConstructorName}(UhtInputCacheReader reader, UhtType outer)'", type.Name, type.Name);
						hasBadType = true;
					}
					MethodInfo? method = type.GetMethod("Write", [typeof(UhtInputCacheWriter)]);
					if (method == null || method.DeclaringType != type)
					{
						logger.LogError("The UhtType '{TypeName}' must defined the method 'public override void Write(UhtInputCacheWriter writer)'", type.Name);
						hasBadType = true;
					}
				}
			}
			if (hasBadType)
			{
				throw new UhtIceException("Not all types implement the expect input cache methods");
			}
		}
		...
		private void HandleUnrealHeaderToolAttribute(Type type, UnrealHeaderToolAttribute parserAttribute)
		{
			...
			foreach (MethodInfo methodInfo in type.GetMethods(BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic))
			{
				...
				foreach (Attribute methodAttribute in methodInfo.GetCustomAttributes())
				{
					...
					{
						CodeGeneratorInjectorTable.OnUhtCodeGeneratorInjectorAttribute(methodInfo, codeGeneratorInjectorAttribute);
					}
				}
			}
		}
