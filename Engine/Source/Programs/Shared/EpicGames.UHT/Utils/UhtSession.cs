...
namespace EpicGames.UHT.Utils
{
	...
	/// </summary>
	sealed class UhtExportFactory : IUhtExportFactory
	{
		...
		/// </summary>
		public Dictionary<string, bool> ReferenceErrorMessages { get; } = [];
		...
		/// </summary>
		public HashSet<string> ExternalDependencies { get; } = [];
		...
		/// <returns>Task object or Task.CompletedTask if the task was immediately executed.</returns>
		public Task CreateTask(List<Task?>? prereqs, UhtExportTaskDelegate action)
		{
			if (Session.GoWide)
			{
				...
				{
					return Task.Factory.ContinueWhenAll(prereqTasks, tasks => { Session.TryNoErrorCheck(null, () => action(this)); });
				}
				else
				{
		...
		public string GetShortestIncludePath(UHTManifest.Module moduleObj, string filePath)
		{
			...
			{
				DirectoryReference includePathRef = new(includePath);
		...
		internal void SaveIfChanged(string filePath, string tempFilePath, StringView exported)
		{
			...
			// Check to see if the contents have changed.  We only do this is output is enabled or we are checking for changes
			bool save = false;
			if (!Session.NoOutput || Session.FailIfGeneratedCodeChanges)
			{
			...
			// If we aren't saving output, then clear out the save flag
			save &= !Session.NoOutput;
		...
		public void Run()
		{
			...
			if (Outputs.Count > 0)
			{
				List<UhtExportFactory.Output> saves = [];
				...
				if (Session.GoWide)
				{
					Parallel2.ForEach(saves, output =>
					{
						RenameSource(output);
					});
				}
				...
				(Exporter.CppFilters != null || Exporter.HeaderFilters != null || Exporter.OtherFilters != null))
			{
				if (Session.GoWide)
				{
					Parallel2.ForEach(outputsByDirectory, kvp =>
					{
	...
	/// <summary>
	/// Helper class that implements a simple counter
	/// </summary>
	public class UhtCounter
	{
	...
	public partial class UhtSession : IUhtMessageSite, IUhtMessageSession
	{
		...
		#region Local structures and enumerations
		/// <summary>
		...
		/// <summary>
		/// Report on any header that didn't contain any types
		/// </summary>
		public bool ReportEmptyHeaders { get; set; } = false;
		#endregion

		#region Target settings reflection and overrides
		/// <summary>
		...
		/// </summary>
		public UhtCompiledInObjectFormat CompiledInObjectFormat
		{
			get => _compiledInObjectFormat ?? Manifest?.TargetSettings.CompiledInObjectFormat ?? UhtCompiledInObjectFormat.Default;
			set => _compiledInObjectFormat = value;
		}
		private UhtCompiledInObjectFormat? _compiledInObjectFormat = null;

		/// <summary>
		/// If true, input cache read will be enabled from the UHT command line.  Use Manifest.TargetSettings to 
		/// query this setting.
		/// </summary>
		public bool EnableInputCacheRead
		{
			get => _enableInputCacheRead ?? Manifest?.TargetSettings.EnableInputCacheRead ?? false;
			set => _enableInputCacheRead = value;
		}
		private bool? _enableInputCacheRead = null;

		/// <summary>
		/// If true, input cache write will be enabled from the UHT command line.  Use Manifest.TargetSettings to 
		/// query this setting.
		/// </summary>
		public bool EnableInputCacheWrite
		{
			get => _enableInputCacheWrite ?? Manifest?.TargetSettings.EnableInputCacheWrite ?? false;
			set => _enableInputCacheWrite = value;
		}
		private bool? _enableInputCacheWrite = null;
		...
		/// <summary>
		/// Block of configuration data from the config and manifest.  Used by the input cache.
		/// </summary>
		public ReadOnlyMemory<byte> ConfigData { get; set; } = new();
		...
		/// <summary>
		/// Latest date/time for the main UHT assembly and the plugins
		/// </summary>
		public DateTime AssemblyDateTime { get; private set; } = DateTime.MaxValue;

		#region Well known types (only avialable after symbol table is built
		/// <summary>
		/// After headers are parsed, returns the UObject class.
		/// </summary>
		public UhtClass UObject => CheckNull(_uobject, "UObject");
		private UhtClass? _uobject = null;

		/// <summary>
		/// After headers are parsed, returns the UPackage class.
		/// </summary>
		public UhtClass UPackage => CheckNull(_upackage, "UPackage");
		private UhtClass? _upackage = null;

		/// <summary>
		/// After headers are parsed, returns the UClass class.
		/// </summary>
		public UhtClass UClass => CheckNull(_uclass, "UClass");
		private UhtClass? _uclass = null;

		/// <summary>
		/// After headers are parsed, returns the UClass class.
		/// </summary>
		public UhtClass UVerseClass => CheckNull(_uverseclass, "UVerseClass");
		private UhtClass? _uverseclass = null;

		/// <summary>
		/// After headers are parsed, returns the UScriptStruct class.
		/// </summary>
		public UhtClass UScriptStruct => CheckNull(_uscriptstruct, "UScriptStruct");
		private UhtClass? _uscriptstruct = null;

		/// <summary>
		/// After headers are parsed, returns the UScriptStruct class.
		/// </summary>
		public UhtClass UVerseStruct => CheckNull(_uversestruct, "UVerseStruct");
		private UhtClass? _uversestruct = null;

		/// <summary>
		/// After headers are parsed, returns the UFunction class.
		/// </summary>
		public UhtClass UFunction => CheckNull(_ufunction, "UFunction");
		private UhtClass? _ufunction = null;

		/// <summary>
		/// After headers are parsed, returns the UDelegateFunction class.
		/// </summary>
		public UhtClass UDelegateFunction => CheckNull(_udelegatefunction, "UDelegateFunction");
		private UhtClass? _udelegatefunction = null;

		/// <summary>
		/// After headers are parsed, returns the USparseDelegateFunction class.
		/// </summary>
		public UhtClass USparseDelegateFunction => CheckNull(_usparsedelegatefunction, "USparseDelegateFunction");
		private UhtClass? _usparsedelegatefunction = null;

		/// <summary>
		/// After headers are parsed, returns the UFunction class.
		/// </summary>
		public UhtClass UVerseFunction => CheckNull(_uversefunction, "UVerseFunction");
		private UhtClass? _uversefunction = null;

		/// <summary>
		/// After headers are parsed, returns the UEnum class.
		/// </summary>
		public UhtClass UEnum => CheckNull(_uenum, "UEnum");
		private UhtClass? _uenum = null;

		/// <summary>
		/// After headers are parsed, returns the UEnum class.
		/// </summary>
		public UhtClass UVerseEnum => CheckNull(_uverseenum, "UVerseEnum");
		private UhtClass? _uverseenum = null;

		/// <summary>
		/// After headers are parsed, returns the UInterface class.
		/// </summary>
		public UhtClass UInterface => CheckNull(_uinterface, "UInterface");
		private UhtClass? _uinterface = null;

		/// <summary>
		/// After headers are parsed, returns the IInterface class.
		/// </summary>
		public UhtClass IInterface => CheckNull(_iinterface, "IInterface");
		...
		/// <summary>
		/// After headers are parsed, returns the FSubScriptStructOf script struct.  Unlike such properties as 
		/// "UObject", there is no requirement for FSubScriptStructOf to be defined.  May be null.
		/// </summary>
		public UhtScriptStruct? FSubScriptStructOf { get; set; } = null;
		...
		/// <summary>
		/// Given a nullable type, throw an exception if not set.
		/// </summary>
		/// <typeparam name="T">Type of the object</typeparam>
		/// <param name="type">Type to return</param>
		/// <param name="typeName">Name of the type</param>
		/// <returns>The type</returns>
		private static T CheckNull<T>(T? type, string typeName) where T : class
		{
			if (type == null)
			{
				throw new UhtIceException($"{typeName} was not defined.");
		...
		/// <summary>
		/// Look up all the commonly needed types from the symbol table
		/// </summary>
		private void ResolveWellKnownTypes()
		{
			...
			FSubScriptStructOf = (UhtScriptStruct?)FindType(null, UhtFindOptions.SourceName | UhtFindOptions.ScriptStruct, "FSubScriptStructOf");
		...
		/// </summary>
		public UhtCounter ErrorCount { get; } = new();

		/// <summary>
		/// The number of warnings
		/// </summary>
		public UhtCounter WarningCount { get; } = new();

		/// <summary>
		/// True if any errors have occurred or warnings if warnings are to be treated as errors 
		/// </summary>
		public bool HasErrors => ErrorCount.Count > 0 || (WarningsAsErrors && WarningCount.Count > 0);

		/// <summary>
		/// Number of new/modified files written to disk
		/// </summary>
		public UhtCounter FilesWritten { get; } = new();

		/// <summary>
		/// Number of source files read directly from disk
		/// </summary>
		public UhtCounter FilesRead { get; } = new();

		/// <summary>
		/// Number of source files read from cache
		/// </summary>
		public UhtCounter FilesCacheRead { get; } = new();

		/// <summary>
		/// Number of source files failed to deserialize
		/// </summary>
		public UhtCounter FilesCacheReadErrors { get; } = new();

		/// <summary>
		/// Number of source files failed to serialize
		/// </summary>
		public UhtCounter FilesCacheWriteErrors { get; } = new();

		/// <summary>
		/// Number of cache files read
		/// </summary>
		public UhtCounter CacheFilesRead { get; } = new();

		/// <summary>
		/// Number of cache files written
		/// </summary>
		public UhtCounter CacheFilesWritten { get; } = new();

		/// <summary>
		/// Number of skipped caches files because they were not found
		/// </summary>
		public UhtCounter CacheFilesSkippedFNF { get; } = new();

		/// <summary>
		/// Number of skipped caches files because the assembly time did not match
		/// </summary>
		public UhtCounter CacheFilesSkippedAssemblyDateTime { get; } = new();

		/// <summary>
		/// Number of skipped caches files because version number mismatch
		/// </summary>
		public UhtCounter CacheFilesSkippedVersion { get; } = new();

		/// <summary>
		/// Number of skipped caches files because configuration changed
		/// </summary>
		public UhtCounter CacheFilesSkippedConfigChanged { get; } = new();

		/// <summary>
		/// Number of skipped caches files because file is corrupted
		/// </summary>
		public UhtCounter CacheFilesSkippedCorrupted { get; } = new();
		#endregion

		private Task? _referenceDeleteTask = null;
		private Task[]? _inputCacheWriteTasks = null;
		...
		public UhtSession(ILogger logger)
		{
			...
			_namespaces.TryAdd(GlobalNamespace.SourceName, GlobalNamespace);
		}
		...
		public void Run(string manifestFilePath, CommandLineArguments? arguments = null)
		{
			...
			// Create a binary buffer that describes the configuration used for parsing
			{
				using ChunkedMemoryWriter writer = new();
				Config!.Write(writer);
				Manifest!.TargetSettings.Write(writer);
				ConfigData = writer.ToByteArray();
			}
			...
			// If we are writing input cache files, wait for completion
			if (_inputCacheWriteTasks != null)
			{
				Logger.LogTrace("Step - Waiting input cache files to be written.");
				Task.WaitAll(_inputCacheWriteTasks);
			}
		}
		...
		/// has occurred, the action will not be executed
		/// </summary>
		...
		public void ForEachHeaderNoErrorCheck(Action<UhtHeaderFile> action, bool allowGoWide)
		{
			...
			{
				Parallel2.ForEach(_headerFiles, headerFile =>
				{
		...
		#region File manager forwording methods
		/// <summary>
		...
		/// <summary>
		/// Collection of all defined namespaces
		/// </summary>
		...
		/// Global namespace
		/// </summary>
		public UhtNamespace GlobalNamespace { get; } = new(null, "", "", true);
		...
		/// <summary>
		/// Given a complete string (no leading ::), return the namespace
		/// </summary>
		/// <param name="fullName">Namespace name</param>
		/// <returns>Namespace</returns>
		public UhtNamespace GetNamespace(string fullName)
		{
			...
			List<UhtNamespaceElement> elements = [];
			elements.Add(new("", GlobalNamespace));

			StringView parts = new(fullName);
			while (parts.Length > 0)
			{
				int sep = parts.Span.IndexOf("::", StringComparison.Ordinal);
				if (sep == -1)
				{
					sep = parts.Length;
				}
				elements.Add(new(new(parts.Memory[..sep]), null));
				if (sep == parts.Length)
				{
					break;
				}
				parts = new(parts.Memory[(sep + 2)..]);
			}

			return GetNamespace(elements, elements.Count - 1, fullName, null);
		}
		...
		#region Message and error support
		/// <summary>
		...
		private void LogMessage(UhtMessage msg)
		{
			...
			Logger.Log(logLevel, KnownLogEvents.UHT, "{File}({Line}){FileFragment}: {Severity}: {Message}",
		...
		private EngineVersion LoadEngineVersion()
		{
			...
			Logger.LogDebug("Reading engine version from file: {VersionHeader}", versionHeader);
			string[] lines;
			try
			{
			...
			EngineVersion loadedVersion = new();
			...
			Logger.LogDebug("Got engine version {EngineVersion}", loadedVersion);
		...
		private void StepReadManifestFile(string manifestFilePath)
		{
			...
			{
				Logger.LogTrace("Step - Read Manifest File");
				...
				if (Tables != null)
				{
					...
					if (EnableInputCacheRead || EnableInputCacheWrite)
					{
						Tables.ValidateCacheableTypes(Logger);
					}
				}

				if (Manifest != null)
				{
					AssemblyDateTime = UhtUtilities.GetLatestAssemblyDateTime(Manifest.UhtPlugins);
				}
				Logger.LogInformation("UHT compiled-in object format {CompiledInObjectFormat}", CompiledInObjectFormat);
			});
		}
		...
		private void StepPrepareModules()
		{
			...
			Try(ManifestFile.MessageSource, () =>
			{
				Logger.LogTrace("Step - Prepare Modules");
		...
		private void StepPrepareHeaders()
		{
			...
			Try(ManifestFile.MessageSource, () =>
			{
				Logger.LogTrace("Step - Prepare Headers");
				...
				// Locate the NoExportTypes.h file
		...
		private void StepParseHeaders()
		{
			...
			Logger.LogDebug("Step - Parse Headers");
			ForEachHeader(headerFile =>
			{
				headerFile.ReadFromCache();
				if (!headerFile.WasCached)
				{
					headerFile.Read();
					UhtHeaderFileParser.Parse(headerFile);

					if (EnableInputCacheWrite && !headerFile.WasCached)
					{
						try
						{
							//using MemoryStream stream = new MemoryStream();
							using UhtInputCacheWriter writer = new(headerFile);
							headerFile.InputCacheEntry = writer.WriteHeader(headerFile.InputCacheEntry.LastWriteTimeUtc);
						}
						catch (Exception ex)
						{
							Logger.LogInformation(ex, "File {Filename} failed to serialize to input cache: {Message}", headerFile.FilePath, ex.Message);
							FilesCacheWriteErrors.Increment();
						}
					}

					if (ReportEmptyHeaders)
					{
						Logger.LogWarning("Header '{Filename}' was parsed but contains no definitions.", headerFile.FilePath);
					}
				}
			}, true);
		}

		private void StepWriteInputCache()
		{
			if (HasErrors)
			{
				return;
			...
			if (EnableInputCacheWrite)
			{
				Logger.LogDebug("Step - Write Input Caches");
				List<Task> tasks = [];
				foreach (UhtModule module in Modules)
				{
					bool isDirty = false;
					...
					if (isDirty)
					{
						tasks.Add(Task.Factory.StartNew(() => { UhtInputCache.Write(module, ConfigData.Span); }, CancellationToken.None, TaskCreationOptions.None, TaskScheduler.Default));
					}
				}
				_inputCacheWriteTasks = [.. tasks];
			}
		}
		...
		private void StepPopulateTypeTable()
		{
			Try(null, () =>
			{
				Logger.LogDebug("Step - Populate symbol table");
		...
		{
			Logger.LogTrace("Step - Bind super and bases");
			ForEachHeader(headerFile => headerFile.BindSuperAndBases(), true);
		}

		private void StepResolveBases()
		{
			Logger.LogTrace("Step - Resolve bases");
			ResolveAllHeaders(UhtResolvePhase.Bases);
		}

		private void StepResolveInvalidCheck()
		{
			Logger.LogTrace("Step - Resolve invalid check");
			ResolveAllHeaders(UhtResolvePhase.InvalidCheck);
		}

		private void StepResolveProperties()
		{
			Logger.LogTrace("Step - Resolve properties");
			ResolveAllHeaders(UhtResolvePhase.Properties);
		}

		private void StepResolveFinal()
		{
			Logger.LogTrace("Step - Resolve final");
			ResolveAllHeaders(UhtResolvePhase.Final);
		}

		private void StepResolveValidate()
		{
			Logger.LogTrace("Step - Resolve validate");
			ForEachHeader(headerFile => headerFile.Validate(UhtValidationOptions.None), true);
		}

		private void StepCollectReferences()
		{
			Logger.LogTrace("Step - Collect references");
			ForEachHeader(headerFile => headerFile.CollectReferences(), true);
		}

		private void StepLinkModules()
		{
			Log.Logger.LogTrace("Step - Link modules");

			foreach (UhtModule module in Modules)
			{
				Dictionary<UhtModule, int> referencedModules = module.Headers.SelectMany(h => h.References.Modules)
					.Distinct()
					.OrderBy(m => m.ShortName)
					.Select((module, index) => (module, index))
					.ToDictionary();

				if (TargetSettings?.ValidateModuleDependencies is true && IsUsingCompiledInObjectFormat(UhtCompiledInObjectFormat.ConstInit))
				{
					foreach (UhtModule otherModule in referencedModules.Keys)
					{
						if (!module.UBTModuleDependencies.Contains(otherModule.ShortName))
						{
							this.LogError($"Module {module.ShortName} needs a link dependency on module {otherModule.ShortName} to link UObjects, but it does not have one.");
						}
					}
				}

				// Top-level objects, excluding those with alternate objects (native inteface objects)
				Dictionary<UhtObject, int> linkableObjects = module.Packages.SelectMany(p => p.Children.OfType<UhtObject>().Where(o => o.AlternateObject is null && o is not UhtPartial))
					// Objects whose outer is not a package but which need to be registered
					.Concat(module.Headers.SelectMany(h => h.References.Singletons).Where(o => o.Package != o.Outer))
					.OrderBy(o => (o.LinkOrder, o.SourceName, o.Outer!.SourceName))
					.Select((obj, index) => (obj, index))
					.ToDictionary();

				module.Linking = new UhtModuleLinking(referencedModules, linkableObjects);
			}
		}
		...
		#region Symbol table support
		/// <summary>
		/// Collection of reserved names
		/// </summary>
		private static readonly HashSet<string> s_reservedNames = ["none"];

		/// <summary>
		/// Symbol table for names as they appear in the C++ source.  This is case sensitive
		/// </summary>
		private UhtSymbolTable _sourceNameSymbolTable = new(0);

		/// <summary>
		/// Symbol table for the engine names derived from the source name.  This is case insensitive
		/// </summary>
		private UhtSymbolTable _engineNameSymbolTable = new(0);

		/// <summary>
		/// If true, the symbol tables have been created following parsing
		/// </summary>
		...
		{
			//COMPATIBILITY-TODO - See comment below on a more robust version of the enum lookup
		...
		private void TopologicalSortHeaderFiles()
		{
			Try(null, () =>
			{
				Logger.LogTrace("Step - Topological Sort Header Files");
		...
		private void RecursiveStructCheck()
		{
			Try(null, () =>
			{
				Logger.LogTrace("Step - Check for recursive structs");
		...
		private readonly HashSet<UhtScriptStruct> _scriptStructsValidForNet = [];
			...
			Try(null, () =>
			{
				...
				foreach (UhtExporter exporter in ExporterTable)
				{
					...
					if (!String.IsNullOrEmpty(exporter.ModuleName))
					{
						...
						if (pluginModule == null)
						{
							// Downgraded from a warning, due to the inability to override
							// the exporter via .ini per build configuration, and due to FN Cook 
							// Process this has been downgraded from warn to info.
							Logger.LogInformation("Exporter \"{ExporterName}\" skipped because module \"{ModuleName}\" was not found in manifest", exporter.Name, exporter.ModuleName);
							continue;
						}
					}
