...
namespace EpicGames.UHT.Parsers
{
	...
	public class UhtParsingScope : IDisposable
	{
		...
		public UhtParsingScope CurrentClassScope
		{
			get
			{
				...
				{
					if (currentScope.ScopeType is UhtClass or UhtPartial)
					{
						return currentScope;
					}
	...
	public struct UhtTokenRecorder : IDisposable
	{
		...
		public UhtTokenRecorder(UhtParsingScope scope, ref UhtToken initialToken)
		{
			...
			_function = null;
			_flushed = false;
			if (_scope.ScopeType is UhtClass or UhtPartial)
			{
		...
		/// Stop the recording
		/// </summary>
		/// <returns>True if the recorded content was added to a class or partial</returns>
		public bool Stop()
		{
			if (!_flushed)
			{
				...
				else if (_scope.ScopeType is UhtPartial partialObj)
				{
					partialObj.AddDeclaration(_compilerDirective, _scope.TokenReader.RecordedTokens, _function);
					_scope.TokenReader.DisableRecording();
					return true;
				}
			}
