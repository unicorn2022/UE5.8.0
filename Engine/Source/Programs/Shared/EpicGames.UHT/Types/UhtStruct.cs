...
namespace EpicGames.UHT.Types
{
	...
	public abstract class UhtStruct : UhtField
	{
		...
		/// <param name="outer">Outer type</param>
		protected UhtStruct(UhtInputCacheReader reader, UhtType outer) : base(reader, outer)
		{
			GeneratedCodeVersion = (EGeneratedCodeVersion)reader.ReadUInt8();
			SuperIdentifier = reader.ReadTokenArray();
			BaseIdentifiers = reader.ReadOptionalList<UhtToken[]>((reader) => reader.ReadTokenArray());
		}
		...
		{
			base.Write(writer);
			writer.WriteUInt8((byte)GeneratedCodeVersion);
			writer.WriteVariableLengthTokenArray(SuperIdentifier);
			writer.WriteOptionalVariableLengthArray(BaseIdentifiers, (writer, tokens) => writer.WriteVariableLengthTokenArray(tokens));
		}
		...
		private static readonly List<UhtStruct> s_emptyBases = [];
		...
		{
			bool inputFlag = child.MetaData.ContainsKey(UhtNames.Input);
			bool visibleFlag = child.MetaData.ContainsKey(UhtNames.Visible);
			if (!inputFlag && !visibleFlag)
			{
				return true;
			}
		...
		protected override void ValidateDocumentationPolicy(UhtDocumentationPolicy policy)
		{
			...
			{
				Dictionary<string, UhtProperty> toolTipToType = [];
				foreach (UhtProperty property in Properties)
				{
					// we don't require tooltips / comments on hidden properties.
					// only properties exposed to the user interface require documentation.
					bool inputFlag = property.MetaData.ContainsKey(UhtNames.Input);
					bool outputFlag = property.MetaData.ContainsKey(UhtNames.Output);
					bool visibleFlag = property.MetaData.ContainsKey(UhtNames.Visible);
					if (!inputFlag && !outputFlag && !visibleFlag)
					{
						continue;
					}
			...
			if (policy.FunctionToolTipsRequired)
			{
				if (this is UhtClass)
				{
					Dictionary<string, UhtType> toolTipToType = [];
					foreach (UhtType child in Children)
					{
		...
		public override void CollectReferences(IUhtReferenceCollector collector)
		{
			...
			collector.AddObjectReference(SparseClassDataStruct, UhtSingletonType.ConstInit);
		}
	}
}
