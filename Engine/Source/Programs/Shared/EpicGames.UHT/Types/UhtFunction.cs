...
namespace EpicGames.UHT.Types
{
	...
	public class UhtFunction : UhtStruct
	{
		...
		/// </summary>
		[JsonIgnore]
		public ReadOnlyMemory<UhtType> ParameterProperties => new([.. Children], 0, HasReturnProperty ? Children.Count - 1 : Children.Count);
		...
		public UhtFunction(UhtInputCacheReader reader, UhtType outer) : base(reader, outer)
		{
			_strippedFunctionName = reader.ReadOptionalString();
			FunctionFlags = (EFunctionFlags)reader.ReadUInt64();
			FunctionExportFlags = (UhtFunctionExportFlags)reader.ReadUInt64();
			FunctionType = (UhtFunctionType)reader.ReadUInt64();
			MacroLineNumber = reader.ReadInt32();
			SparseOwningClassName = reader.ReadOptionalString();
			SparseDelegateName = reader.ReadOptionalString();
			RPCId = reader.ReadUInt16();
			RPCResponseId = reader.ReadUInt16();
			CppImplName = reader.ReadString();
			CppValidationImplName = reader.ReadString();
			MarshalAndCallName = reader.ReadString();
			UnMarshalAndCallName = reader.ReadString();
			EndpointName = reader.ReadString();
		}
		...
		{
			base.Write(writer);
			writer.WriteOptionalString(_strippedFunctionName);
			writer.WriteUInt64((ulong)FunctionFlags);
			writer.WriteUInt64((ulong)FunctionExportFlags);
			writer.WriteUInt64((ulong)FunctionType);
			writer.WriteInt32(MacroLineNumber);
			writer.WriteOptionalString(SparseOwningClassName);
			writer.WriteOptionalString(SparseDelegateName);
			writer.WriteUInt16(RPCId);
			writer.WriteUInt16(RPCResponseId);
			writer.WriteString(CppImplName);
			writer.WriteString(CppValidationImplName);
			writer.WriteString(MarshalAndCallName);
			writer.WriteString(UnMarshalAndCallName);
		...
		protected override bool ResolveSelf(UhtResolvePhase phase)
		{
			...
			bool storeCppDefaultValueInMetaData = 
				FunctionFlags.HasAnyFlags(EFunctionFlags.BlueprintCallable | EFunctionFlags.Exec) || MetaData.ContainsKey("AICallable");
		...
		protected override UhtValidationOptions Validate(UhtValidationOptions options)
		{
			...
			if (DefineScope.HasAnyFlags(UhtDefineScope.Tests) && Outer is UhtClass && !Outer.DefineScope.HasAnyFlags(UhtDefineScope.Tests))
			{
			...
			switch (FunctionType)
			{
				case UhtFunctionType.Function:
					...
					// Get the outer class - either directly or from partial's Owner class
					UhtClass? outerClass = Outer as UhtClass;
					if (outerClass == null && Outer is UhtPartial partial)
					{
						outerClass = partial.OwnerClass;
					}
		...
		public override void CollectReferences(IUhtReferenceCollector collector)
		{
			...
			{
				case UhtFunctionType.Function:
					collector.AddObjectReference(Session.UFunction, UhtSingletonType.ConstInit);
					collector.AddObjectReference(this, UhtSingletonType.ConstInit);
					collector.AddObjectReference(Package, UhtSingletonType.ConstInit);
					break;
				case UhtFunctionType.Delegate:
					collector.AddObjectReference(Session.UDelegateFunction, UhtSingletonType.ConstInit);
					break;
				case UhtFunctionType.SparseDelegate:
					collector.AddObjectReference(Session.USparseDelegateFunction, UhtSingletonType.ConstInit);
					break;
				default:
					break;
			}
