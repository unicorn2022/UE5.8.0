...
namespace EpicGames.UHT.Types
{
	...
	public class UhtTextProperty : UhtProperty
	{
		...
		/// <inheritdoc/>
		protected override string CodeGenParamsStruct => "FTextPropertyParams";

		/// <inheritdoc/>
		protected override string CodeGenParamsFlags => "UECodeGen_Private::EPropertyGenFlags::Text";
		...
		public UhtTextProperty(UhtInputCacheReader reader, UhtType outer) : base(reader, outer)
		{
		}
		...
		public override bool SanitizeDefaultValue(IUhtTokenReader defaultValueReader, StringBuilder innerDefaultValue)
		{
			...
			else if (token.IsIdentifier("NOTELOCTEXT"))
			{
				//ETSTODO - Add a test case for this
				this.LogError($"NOTELOCTEXT default parameter values are not supported; use NSNOTELOCTEXT instead: {SourceName}");
				return false;
			}
			else
			{
		...
		private static UhtTextProperty? TextProperty(UhtPropertyResolveArgs args)
		{
		...
		[UhtLocTextDefaultValue(Name = "NOTELOCTEXT")]
		[SuppressMessage("CodeQuality", "IDE0051:Remove unused private members", Justification = "Attribute accessed method")]
		[SuppressMessage("Style", "IDE0060:Remove unused parameter", Justification = "Attribute accessed method")]
		private static bool NoteLocTextDefaultValue(UhtTextProperty property, IUhtTokenReader defaultValueReader, ref UhtToken macroToken, StringBuilder innerDefaultValue)
		{
			property.LogError($"NOTELOCTEXT default parameter values are not supported; use NSNOTELOCTEXT instead: {property.SourceName}");
			return false;
		}
		...
		[UhtLocTextDefaultValue(Name = "NSNOTELOCTEXT")]
		[SuppressMessage("CodeQuality", "IDE0051:Remove unused private members", Justification = "Attribute accessed method")]
		[SuppressMessage("Style", "IDE0060:Remove unused parameter", Justification = "Attribute accessed method")]
		private static bool NsNoteLocTextDefaultValue(UhtTextProperty property, IUhtTokenReader defaultValueReader, ref UhtToken macroToken, StringBuilder innerDefaultValue)
		{
			defaultValueReader.Require('(');
			StringView namespaceString = defaultValueReader.GetConstQuotedString();
			defaultValueReader.Require(',');
			...
			StringView sourceString = defaultValueReader.GetConstQuotedString();
			defaultValueReader.Require(',');
			StringView devNotesString = defaultValueReader.GetConstQuotedString();
			defaultValueReader.Require(')');
			...
			innerDefaultValue.Append("NSNOTELOCTEXT(\"").Append(strippedNS).Append("\", ").Append(keyString).Append(", ").Append(sourceString).Append(", ").Append(devNotesString).Append(')');
			return true;
		}
		...
		private static bool SanitizeDefaultValue(UhtTextProperty property, IUhtTokenReader defaultValueReader, ref UhtToken token, StringBuilder innerDefaultValue, bool allowNumerics)
		{
			switch (token.TokenType)
			{
				case UhtTokenType.Identifier:
					if (property.Session.TryGetLocTextDefaultValue(token.Value, out UhtLocTextDefaultValue locTextDefaultValue))
					{
