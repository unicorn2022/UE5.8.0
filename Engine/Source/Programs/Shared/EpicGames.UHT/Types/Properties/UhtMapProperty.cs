...
namespace EpicGames.UHT.Types
{
	...
	public class UhtMapProperty : UhtContainerBaseProperty
	{
		...
		/// <inheritdoc/>
		protected override string CodeGenParamsStruct => "FMapPropertyParams";

		/// <inheritdoc/>
		protected override string CodeGenParamsFlags => "UECodeGen_Private::EPropertyGenFlags::Map";
		...
		public UhtMapProperty(UhtInputCacheReader reader, UhtType outer) : base(reader, outer)
		{
			KeyProperty = (reader.ReadType() as UhtProperty)!;
		}
		...
		public override IEnumerable<UhtChildProperty> EnumerateChildProperties(UhtCppIdentifier identifier, UhtChildPropertyOrder order)
		{
			...
			if (order == UhtChildPropertyOrder.Forward)
			{
				yield return new(KeyProperty, identifier.AppendSuffix("_Key_KeyProp"), "0");
				yield return new(ValueProperty, identifier.AppendSuffix("_ValueProp"), "1");
			}
			else
			{
				yield return new(ValueProperty, identifier.AppendSuffix("_ValueProp"), "1");
		...
		{
			return builder.Append(Allocator == UhtPropertyAllocator.MemoryImage ? "EMapPropertyFlags::UsesMemoryImageAllocator" : "EMapPropertyFlags::None").Append(", ");
		}
		...
		private static UhtMapProperty? MapProperty(UhtPropertyResolveArgs args)
		{
