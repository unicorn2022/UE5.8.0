...
{
	/// <summary>
	/// Index assignments for other modules which are a dependency of this module, and objects which are linkable in this module from others.
	/// </summary>
	/// <param name="ReferencedModules">Mapping of dependency module to the index to use for encoded pointers when linking to it from this module.</param>
	/// <param name="LinkableObjects">Mapping of public linkable objects in this module to indexes that other modules can use when encoding pointers to them.</param>
	public record UhtModuleLinking(IReadOnlyDictionary<UhtModule, int> ReferencedModules, IReadOnlyDictionary<UhtObject, int> LinkableObjects);
	...
	{
		private readonly List<UhtHeaderFile> _headers = [];
		private readonly SortedDictionary<string, UhtPackage> _packages = [];
		private readonly HashSet<string> _ubtModuleDependencies = [];
		private readonly Lazy<UhtInputCache> _inputCache;
		...
		/// <summary>
		/// Collection of module dependencies from UBT (may also include the names of modules not present in UHT)
		/// </summary>
		public IReadOnlySet<string> UBTModuleDependencies => _ubtModuleDependencies;
		...
		/// <summary>
		/// Information about linking objects between this module and others generated by UhtSession after reference collection. 
		/// </summary>
		public UhtModuleLinking? Linking { get; set; }
		...
		public UhtModule(UhtSession session, UHTManifest.Module module)
		{
			Session = session;
			Module = module;
			foreach (string dep in module.ModuleDependencies)
			{
				_ubtModuleDependencies.Add(dep);
			}
			...
			Api = $"{ShortName.ToUpperInvariant()}_API ";
			NonAttributedApi = $"{ShortName.ToUpperInvariant()}_NON_ATTRIBUTED_API ";

			ScriptPackage = new(this, packageName, packageFlags);

			_inputCache = new(() => UhtInputCache.Read(this, Session.ConfigData.Span));
		}
		...
		/// <summary>
		/// Try to fetch a cached version of the file from the input cache
		/// </summary>
		/// <param name="fileName"></param>
		/// <param name="entry"></param>
		/// <returns></returns>
		public bool TryGetCachedInput(string fileName, out UhtInputCacheEntry entry)
		{
			if (!Session.EnableInputCacheRead)
			{
				entry = new();
				return false;
			}

			return _inputCache.Value.TryGetFile(fileName, out entry);
		}
