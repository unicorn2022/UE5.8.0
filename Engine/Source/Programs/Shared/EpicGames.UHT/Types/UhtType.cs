...
namespace EpicGames.UHT.Types
{
	...
	public enum UhtDefineScope
	{
		...
		/// <summary>
		/// Exists inside of a WITH_EDITOR block
		/// </summary>
		Editor = 1 << 4,
	...
	public static class UhtDefineScopeExtensions
	{
		...
		/// <returns></returns>
		public static UhtCompilerDirective GetCompilerDirective(this UhtDefineScope defineScope)
		{
			switch (defineScope)
			{
				case UhtDefineScope.EditorOnlyData:
					return UhtCompilerDirective.WithEditorOnlyData;
				case UhtDefineScope.Editor:
					return UhtCompilerDirective.WithEditor;
	...
	public class UhtDocumentationPolicy
	{
		...
		/// <summary>
		/// Write the structure contents
		/// </summary>
		/// <param name="writer">Destination writer</param>
		public void Write(IMemoryWriter writer)
		{
			writer.WriteBoolean(ClassOrStructCommentRequired);
			writer.WriteBoolean(FunctionToolTipsRequired);
			writer.WriteBoolean(MemberToolTipsRequired);
			writer.WriteBoolean(ParameterToolTipsRequired);
			writer.WriteBoolean(FloatRangesRequired);
		}
	};
	...
	public abstract class UhtType : IUhtMessageSite, IUhtMessageLineNumber
	{
		...
		/// </summary>
		private static readonly List<UhtType> s_emptyTypeList = [];
		...
		/// <summary>
		/// #define scope where the type was defined. None means defined regardless of preprocessor definitions.
		/// </summary>
		...
		public List<UhtType> Children => _children ?? s_emptyTypeList;
		...
		protected UhtType(UhtInputCacheReader reader, UhtType outer)
		{
			reader.AddType(this);
			_headerFile = reader.Header;
			Module = _headerFile.Module;
			Outer = outer;
			TypeIndex = Session.GetNextTypeIndex();
			MetaData = new UhtMetaData(this, Session.Config);
			...
			_engineName = reader.ReadOptionalString();
			DefineScope = (UhtDefineScope)reader.ReadUInt64();
		...
		public virtual void Write(UhtInputCacheWriter writer)
		{
			writer.WriteInt32(LineNumber);
			MetaData.Write(writer);
			writer.WriteString(SourceName);
			writer.WriteOptionalString(_engineName);
			writer.WriteUInt64((ulong)DefineScope);
		...
		private void ValidateDocumentationPolicy()
		{
			...
			// deprecated structs don't require the documentation policy.
			string deprecatedValue = MetaData.GetValueOrDefaultHierarchical(UhtNames.Deprecated);
