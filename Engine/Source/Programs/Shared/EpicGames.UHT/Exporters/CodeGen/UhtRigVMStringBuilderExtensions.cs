...
namespace EpicGames.UHT.Exporters.CodeGen
{
	internal static class UhtRigVMStringBuilderExtensions
	{
		...
		public static StringBuilder AppendTypeConstRef(this StringBuilder builder, UhtRigVMParameter parameter, bool castType = false)
		{
			...
			if (parameterTypeSpan.EndsWith("&", StringComparison.Ordinal))
			{
				parameterTypeSpan = parameterTypeSpan[0..^1];
			}
			builder.Append(parameterTypeSpan);

			if (parameterTypeSpan.Length > 0)
			{
				int namespaceEndIndex = parameterTypeSpan.LastIndexOf(':');
				int firstTypeCharIndex = namespaceEndIndex >= 0 ? (namespaceEndIndex + 1) : 0;
				if (parameterTypeSpan[firstTypeCharIndex] == 'T' || parameterTypeSpan[firstTypeCharIndex] == 'F')
				{
					builder.Append('&');
				}
			}
