...
namespace EpicGames.UHT.Tokenizer
{
	...
	public static class UhtTokenReaderStringExtensions
	{
		...
		/// <summary>
		/// Get a typed const string that can optionally be wrapped with a TEXT() macro
		/// </summary>
		/// <param name="tokenReader">Token reader</param>
		/// <param name="typeName">Expected type name</param>
		/// <param name="exceptionContext">If not null, an exception will be thrown with the given text as part of the message.</param>
		/// <returns>The value of the string</returns>
		public static StringView GetTypedConstString(this IUhtTokenReader tokenReader, string typeName, object? exceptionContext = null)
		{
			StringView output = new();
			if (tokenReader.TryOptional(typeName))
			{
				tokenReader.Require('(');
				if (!tokenReader.PeekToken().IsSymbol(')'))
				{
					output = tokenReader.GetWrappedConstString(exceptionContext);
				}
				tokenReader.Require(')');
			}
			else
