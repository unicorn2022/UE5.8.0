...
namespace EpicGames.UHT.Tokenizer
{
	...
	{
		private static readonly StringView[] s_emptyComments = [];

		private readonly IUhtMessageSite _messageSite;
		private List<StringView>? _comments = null;
		private readonly List<UhtToken> _recordedTokens = [];
		...
		public ReadOnlySpan<StringView> Comments
		{
			get
			{
				...
				{
					return new ReadOnlySpan<StringView>([.. _comments], 0, _committedComments);
				}
				else
				{
					return new ReadOnlySpan<StringView>(s_emptyComments);
				}
			}
		}
...
ResetStateMachineCheck:
											...
											{
												this.LogError($"'{localValueSpan}' must not be inside preprocessor blocks, except for WITH_EDITORONLY_DATA");
											}
