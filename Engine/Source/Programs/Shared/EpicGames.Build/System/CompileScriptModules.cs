...
namespace UnrealBuildBase
{
	public static class CompileScriptModule
	{
		...
		class Hook : CsProjBuildHook
		{
			...
			private Dictionary<FileReference, CsProjBuildRecordEntry> ValidBuildRecords = [];
			...
			{
				ValidateBuildRecordRecursively(BuildRecords, ProjectPath, this, Logger);
			}
			...
			DirectoryReference IMsBuildRegistration.DotnetDirectory => Unreal.DotnetDirectory;

			FileReference IMsBuildRegistration.DotnetPath => Unreal.DotnetPath;
		}
		...
		/// <param name="AutomationProjectIncludeFilter"></param>
		...
		public static HashSet<FileReference> InitializeScriptModules(Rules.RulesFileType RulesFileType,
			string? ScriptsForProjectFileName, List<string>? AdditionalScriptsFolders, List<string>? AutomationProjectIncludeFilter, bool bForceCompile, bool bNoCompile, bool bUseBuildRecords,
			out bool bBuildSuccess, ILogger Logger)
		{
			...
			BaseDirectories = [.. BaseDirectories.Distinct()];
			Logger.LogDebug("Find {Count} directories time: {TimeSeconds:0.00} s", BaseDirectories.Count, Timer.Elapsed.TotalSeconds);
			...
			if (AutomationProjectIncludeFilter != null && AutomationProjectIncludeFilter.Count > 0)
			{
				FoundProjects = [.. FoundProjects.Where(x => (AutomationProjectIncludeFilter.Contains(x.GetFileNameWithoutAnyExtensions(), StringComparer.OrdinalIgnoreCase)))];
			}
			Logger.LogDebug("Find {Count} projects time: {TimeSeconds:0.00} s", FoundProjects.Count, Timer.Elapsed.TotalSeconds);
			...
			if (Unreal.IsEngineInstalled())
			{
				...
				Dictionary<FileReference, CsProjBuildRecordEntry> EngineBuildResults = Build(
					RulesFileType,
					[.. FoundProjects.Where(x => x.IsUnderDirectory(Unreal.EngineDirectory))],
					[.. BaseDirectories.Where(x => x.IsUnderDirectory(Unreal.EngineDirectory))],
					null,
					EngineBuildFlags,
			...
			{
				BuildResults = Build(RulesFileType, FoundProjects, BaseDirectories, null, BuildFlags, out bBuildSuccess, Logger);
			}
		...
		public static Dictionary<FileReference, CsProjBuildRecordEntry> Build(Rules.RulesFileType RulesFileType,
			...
			BuildFlags BuildFlags, out bool bBuildSuccess, ILogger Logger)
		{
			CsProjBuildHook Hook = new Hook(Logger, RulesFileType);
			...
			if (BuildFlags.HasFlag(BuildFlags.NoCompile))
			{
				...
				Dictionary<FileReference, CsProjBuildRecordEntry> OutRecords = new(BuildRecords.Count);
			...
			return Build(FoundProjects, BuildFlags.HasFlag(BuildFlags.ForceCompile), out bBuildSuccess, Hook, BaseDirectories, DefineConstants, Logger);
		}
		...
		{
			return CsProjBuilder.Build(FoundProjects, bForceCompile, out bBuildSuccess, Hook, BaseDirectories, DefineConstants ?? ([]), Logger);
		}
		...
		static Dictionary<FileReference, CsProjBuildRecordEntry> LoadExistingBuildRecords(CsProjBuildHook Hook, IEnumerable<DirectoryReference> BaseDirectories, ILogger Logger)
		{
			...
			foreach (DirectoryReference Directory in BaseDirectories)
			{
				...
				foreach (FileReference JsonFile in DirectoryReference.EnumerateFiles(IntermediateDirectory, "*.json"))
				{
					...
					{
						Logger.LogWarning(ex, "[{JsonFile}] Failed to load build record: {Message}", JsonFile, ex.Message);
					}

					if (BuildRecord != null && BuildRecord.ProjectPath != null)
					{
						CsProjBuildRecordEntry Entry = new(
							new(Path.GetFullPath(BuildRecord.ProjectPath, JsonFile.Directory.FullName)),
							JsonFile,
							BuildRecord);
		...
		private static bool ValidateBuildRecord(CsProjBuildRecord BuildRecord, DirectoryReference ProjectDirectory, out string ValidationFailureMessage, CsProjBuildHook Hook)
		{
			...
			if (BuildRecord.RuntimeVersion != Environment.Version)
			{
				ValidationFailureMessage =
					$"runtime version does not match: build record has version {BuildRecord.RuntimeVersion}; current version is {Environment.Version}";
				return false;
			}

			if (BuildRecord.TargetConfiguration != CsProjBuildRecord.CurrentConfiguration)
			{
				ValidationFailureMessage =
					$"recorded target configuration ({BuildRecord.TargetConfiguration}) does not match ({CsProjBuildRecord.CurrentConfiguration})";
				return false;
			}
			...
			FileReference ProjectPath, CsProjBuildHook Hook, ILogger Logger)
		{
			...
			{
				FileReference FullProjectPath = new(Path.GetFullPath(ReferencedProject.ProjectPath, ProjectPath.Directory.FullName));
		...
		static List<DirectoryReference> GetGameDirectories(string? ScriptsForProjectFileName, ILogger Logger)
		{
			...
			{
				GameDirectories = [.. NativeProjectsBase.EnumerateProjectFiles(Logger).Select(x => x.Directory)];
			}
			else
			{
				DirectoryReference ScriptsDir = new(Path.GetDirectoryName(ScriptsForProjectFileName)!);
		...
		static List<DirectoryReference> GetAdditionalDirectories(List<string>? AdditionalScriptsFolders) =>
			AdditionalScriptsFolders == null ? [] :
				[.. AdditionalScriptsFolders.Select(x => DirectoryReference.FindCorrectCase(new DirectoryReference(x)))];

		static List<DirectoryReference> GetAdditionalBuildDirectories(List<DirectoryReference> GameDirectories) =>
			[.. GameDirectories.Select(x => DirectoryReference.Combine(x, "Build")).Where(x => DirectoryReference.Exists(x))];
	}
}
