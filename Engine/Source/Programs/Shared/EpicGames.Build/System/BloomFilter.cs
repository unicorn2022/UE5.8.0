...
namespace UnrealBuildBase
{
	...
	public sealed class BloomFilter
	{
		...
		{
			uint hash = (uint)IoHash.Compute(s).GetHashCode();
		...
		{
			int separatorIndex;
			while ((separatorIndex = span.IndexOf(separator)) != -1)
			{
				if (MightContain(span.Slice(0, separatorIndex)))
				{
					return true;
				}
				span = span.Slice(separatorIndex + 1);
			}
			return MightContain(span);
		}
	}
}