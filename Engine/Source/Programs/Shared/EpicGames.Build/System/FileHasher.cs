...
namespace UnrealBuildBase
{
	...
	public class FileHasher(ILogger? logger = null)
	{
		...
		public void PurgeStale()
		{
			...
			IEnumerable<KeyValuePair<string, CachedDigest>> valid = CachedDigests.Where((x) => x.Value.UpToDate(FileItem.GetItemByFileReference(new(x.Key))));
		...
		{
			CachedDigest digest = new(item, hash);
		...
		static async Task<CachedDigest> ComputeDigest(FileItem item, ILogger? logger, CancellationToken CancellationToken = default)
		{
			...
			CachedDigest digest = new(item, await IoHash.ComputeAsync(stream, item.Length, CancellationToken));
