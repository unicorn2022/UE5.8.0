...
namespace EpicGames.Serialization
{
	...
	public partial class CbConverter
	{
		...
		/// </summary>
		private static Dictionary<Type, CbConverter> TypeToConverter { get; } = new Dictionary<Type, CbConverter>()
		{
		...
		/// <summary>
		/// Adds a new converter to the type to converter mapping if it does not exist
		/// </summary>
		/// <param name="type">The type to convert to/from</param>
		/// <param name="converter">The converter that converts to/from the type</param>
		public static void AddConverter(Type type, CbConverter converter)
		{
			lock (s_lockObject)
			{
				TypeToConverter.TryAdd(type, converter);
			}
		}
