...
namespace EpicGames.Serialization
{
	...
	public record struct CbFieldName(Utf8String Text)
	{
		...
		/// </summary>
		public static implicit operator CbFieldName(string text) => new(new Utf8String(text));

		/// <summary>
		/// Implicit conversion from a utf8 string
		/// </summary>
		public static implicit operator CbFieldName(Utf8String text) => new(text);
	}
	...
	public abstract class CbWriterBase : ICbWriter
	{
		...
		readonly Scope _rootScope = new() { _fieldType = CbFieldType.Array };
		readonly Stack<Scope> _openScopes = new();
		readonly Stack<Scope> _freeScopes = new();
	...
	public static class CbWriterExtensions
	{
		...
		/// <param name="writer">Writer for output data</param>
		/// <param name="value">Value to be written</param>
		public static void WriteBinaryArrayValue(this ICbWriter writer, byte[] value) => writer.WriteBinarySpanValue(value);
		...
		/// <param name="name">Name of the field</param>
		/// <param name="value">Value to be written</param>
		public static void WriteBinaryArray(this ICbWriter writer, CbFieldName name, byte[] value) => writer.WriteBinarySpan(name, value);
	}
}