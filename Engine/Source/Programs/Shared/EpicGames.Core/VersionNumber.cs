...
namespace EpicGames.Core
{
	...
	internal class VersionNumberJsonConverter : JsonConverter<VersionNumber>
	{
		public override VersionNumber? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
		{
			string? versionNumber = reader.GetString();
			return versionNumber is null ? null : VersionNumber.Parse(versionNumber);
		}

		public override VersionNumber ReadAsPropertyName(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
		{
			if (reader.TokenType != JsonTokenType.PropertyName)
			{
				throw new JsonException("VersionNumber keys must be represented as JSON strings");
			}
			return VersionNumber.Parse(reader.GetString()!);
		}
		...
		public override void WriteAsPropertyName(Utf8JsonWriter writer, [DisallowNull] VersionNumber value, JsonSerializerOptions options)
		{
