...
namespace EpicGames.Core
{
	...
	public abstract class FileSet : IEnumerable<FileReference>
	{
		...
		/// <summary>
		/// Creates a file tree from a given file
		/// </summary>
		/// <param name="file">File to add</param>
		/// <returns>Tree containing the given files</returns>
		public static FileSet FromFile(FileReference file)
		{
			return FromFiles([(file.GetFileName(), file)]);
		}
		...
		{
			Dictionary<string, FileReference> pathToSourceFile = new(StringComparer.OrdinalIgnoreCase);
	...
	class FileSetFromFiles : FileSet
	{
			...
			: this(String.Empty)
		{
			...
			{
				string[] fragments = path.Split(Separator, StringSplitOptions.RemoveEmptyEntries);
	...
	class FileSetFromUnion : FileSet
	{
		...
		{
			Dictionary<string, FileReference> files = new(_lhs.EnumerateFiles(), StringComparer.OrdinalIgnoreCase);
		...
		{
			Dictionary<string, FileSet> nameToSubTree = new(_lhs.EnumerateDirectories(), StringComparer.OrdinalIgnoreCase);
	...
	class FileSetFromExcept : FileSet
	{
		...
		public override IEnumerable<KeyValuePair<string, FileReference>> EnumerateFiles()
		{
			HashSet<string> rhsFiles = new(_rhs.EnumerateFiles().Select(x => x.Key), StringComparer.OrdinalIgnoreCase);
		...
		public override IEnumerable<KeyValuePair<string, FileSet>> EnumerateDirectories()
		{
			Dictionary<string, FileSet> rhsDirs = new(_rhs.EnumerateDirectories(), StringComparer.OrdinalIgnoreCase);
	...
	class FileSetFromFilter : FileSet
	{
		...
		public override IEnumerable<KeyValuePair<string, FileSet>> EnumerateDirectories()
		{
			foreach (KeyValuePair<string, FileSet> item in _inner.EnumerateDirectories())
			{
				...
				FileFilter[] possibleFilters = [.. _filters.Where(x => x.PossiblyMatches(filterName))];
				if (possibleFilters.Length > 0)
				{
					FileSetFromFilter subTreeFilter = new(item.Value, possibleFilters);
