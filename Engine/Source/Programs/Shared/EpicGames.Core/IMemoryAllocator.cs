...
namespace EpicGames.Core
{
	...
	public class GlobalHeapAllocator : IMemoryAllocator<byte>
	{
		unsafe class Allocation : MemoryManager<byte>
		{
			...
			/// <inheritdoc/>
			public override Span<byte> GetSpan() => new(GetPointer(), _length);

			/// <inheritdoc/>
			public override MemoryHandle Pin(int elementIndex) => new(GetPointer() + elementIndex);
	...
	public class VirtualMemoryAllocator : IMemoryAllocator<byte>
	{
		unsafe class Allocation : MemoryManager<byte>
		{
			...
			/// <inheritdoc/>
			public override MemoryHandle Pin(int elementIndex) => new(_pointer + elementIndex);
	...
	public class TrackingMemoryAllocator : IMemoryAllocator<byte>
	{
		...
		long _allocatedSize;
		readonly LinkedList<MemoryAllocation> _allocations = new();
