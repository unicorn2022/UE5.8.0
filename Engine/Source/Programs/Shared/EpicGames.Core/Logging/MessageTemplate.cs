...
namespace EpicGames.Core
{
	...
	public static class MessageTemplate
	{
		...
		public static void ParsePropertyValues(string format, object[] args, Dictionary<string, object?> properties)
		{
			...
			if (offsets != null)
			{
				for (int idx = 0; idx < offsets.Count; idx++)
				{
					...
					// Parse for format specifier
					int specifierIdx = name.IndexOf(':', 0);
					if (specifierIdx != -1)
					{
						name = name.Substring(0, specifierIdx);
					}
		...
		{
			int number;
			if (Int32.TryParse(name, NumberStyles.Integer, null, out number))
			{
