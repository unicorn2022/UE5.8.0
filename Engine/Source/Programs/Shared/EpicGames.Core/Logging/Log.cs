...
#pragma warning restore CA1069 // Enums values should not be duplicated
	...
	public static class Log
	{
		...
		/// </summary>
		private static readonly LegacyEventLogger s_legacyLogger = new(s_defaultLogger);
		...
		/// <summary>
		/// When true, create a backup of any text log file that would be overwritten by a new log
		...
		/// <summary>
		/// Enable threaded logging. This can be used for performance reasons to prevent stalls when lots of logging is made from multiple threads
		/// </summary>
		public static void EnableThreaded()
		{
			s_legacyLogger.EnableThreaded();
		}

		/// <summary>
		/// Disable threaded logging. will flush queued log entries and get back to synchronous logging
		/// </summary>
		public static void DisableThreaded()
		{
			s_legacyLogger.DisableThreaded();
		}

		/// <summary>
		/// Backup an existing log file if it already exists at the output path
		/// </summary>
		/// <param name="outputFile">The file to back up</param>
		public static void BackupLogFile(FileReference outputFile)
		{
			if (!BackupLogFiles || !FileReference.Exists(outputFile))
			{
				return;
			}
			...
			Regex backupForm =
				new(filenameWithoutExtension + @"-backup-\d\d\d\d\.\d\d\.\d\d-\d\d\.\d\d\.\d\d" + extension);
		...
		{
			s_defaultLogger.AddFileWriter(name, outputFile);
		}
		...
		/// <summary>
		/// Adds a <see cref="StartupTraceListener"/> to the collection in a safe manner.
		/// </summary>
		public static void AddStartupTraceListener()
		{
			s_defaultLogger.AddStartupTraceListener();
		}
	...
	{
		readonly LoggerScopeCollection _scopeCollection = new();
	...
	class DefaultLogger : ILogger, IDisposable
	{
		...
		private readonly LoggerScopeCollection _scopeCollection = new();
		...
		/// </summary>
		private readonly Stack<StatusMessage> _statusMessageStack = new();
		...
		/// </summary>
		private ImmutableArray<FileStream> _jsonStreams = [];
		...
		public DefaultLogger()
		{
			...
			bool bJsonStdOut = Environment.CommandLine.Contains("JsonStdOut", StringComparison.Ordinal);
		...
		private TextWriterTraceListener AddTextWriter(string name, FileReference outputFile)
		{
			try
			{
				OutputFile = outputFile;
				DirectoryReference.CreateDirectory(outputFile.Directory);
				TextWriterTraceListener logTraceListener = new(new StreamWriter(outputFile.FullName), name);
				lock (_syncObject)
				{
			...
			{
				ex.Data["LogFile"] = outputFile;
				throw;
			}
		}

		/// <summary>
		/// Adds a JSON stream to the collection
		/// </summary>
		/// <param name="outputFile">The file to write to</param>
		private void AddJsonWriter(FileReference outputFile)
		{
			try
			{
				DirectoryReference.CreateDirectory(outputFile.Directory);
				lock (_syncObject)
				{
					FileStream jsonStream = FileReference.Open(outputFile, FileMode.Create, FileAccess.Write, FileShare.Read | FileShare.Delete);
					_jsonStreams = _jsonStreams.Add(jsonStream);
				}
			}
			catch (Exception ex)
			{
		...
		/// Adds a <see cref="StartupTraceListener"/> to the collection in a safe manner.
		/// </summary>
		public void AddStartupTraceListener()
		{
			lock (_syncObject)
			{
				if (!Trace.Listeners.OfType<StartupTraceListener>().Any())
				{
					Trace.Listeners.Add(new StartupTraceListener());
					WriteInitialTimestamp();
				}
			}
		}
		...
		/// Removes a <see cref="StartupTraceListener"/> from the collection in a safe manner.
		/// </summary>
		public StartupTraceListener? RemoveStartupTraceListener()
		{
			lock (_syncObject)
			{
				StartupTraceListener? startupListener = Trace.Listeners.OfType<StartupTraceListener>().SingleOrDefault();
				if (startupListener is not null)
				{
		...
		public void AddFileWriter(string name, FileReference outputFile)
		{
			this.LogInformation("Log file: {OutputFile}", outputFile);
			Core.Log.BackupLogFile(outputFile);
			Core.Log.BackupLogFile(outputFile.ChangeExtension(".json"));
		...
		{
			TextWriterTraceListener textWriter = AddTextWriter(name, outputFile);
			// Note that we can't correctly copy the startup listener to the JSON writer as the output has already been formatted.
			AddJsonWriter(outputFile.ChangeExtension(".json"));

			lock (_syncObject)
			{
				StartupTraceListener? startupListener = RemoveStartupTraceListener();
				startupListener?.CopyTo(textWriter);
			}
		}
		...
		public void PushStatus(string message)
		{
			lock (_syncObject)
			{
				...
				StatusMessage newStatusMessage = new(message, message);
