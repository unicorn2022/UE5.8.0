...
namespace EpicGames.Core
{
	...
	public class LogEventLine
	{
		...
		{
			LogEventSpan span = new(offset, Text.Substring(offset, length));
			Spans.Add(name, span);
			return span;
		}
	...
	public class LogEventBuilder
	{
		...
		{
			LogSpan span = new(name, offset, length, value);
		...
		/// <returns></returns>
		public LogEventMatch ToMatch(LogEventPriority priority, LogLevel level, EventId eventId, bool isComplete = true)
		{
			return new LogEventMatch(priority, ToArray(level, eventId), isComplete);
		}
		
		/// <summary>
		/// Creates a match object at the given priority with hanging lines
		/// </summary>
		/// <param name="level"></param>
		/// <param name="eventId">The event id</param>
		/// <param name="priority"></param>
		/// <returns></returns>
		public LogEventMatch ToMatchWithHangingLines(LogEventPriority priority, LogLevel level, EventId eventId)
		{
			// If there are more lines that are not hanging, is must be complete
			bool isComplete = Next.Length > 0;
			for (; isComplete && IsNextLineHanging(); isComplete = Next.Length > 0)
			{
				MoveNext();
			}
