...
namespace EpicGames.Core
{
	...
	public static class BinaryReaderExtensions
	{
		...
		public static Dictionary<TKey, TValue>? ReadDictionary<TKey, TValue>(this BinaryReader reader, Func<TKey> readKey, Func<TValue> readValue) where TKey : notnull
		{
			...
			Dictionary<TKey, TValue> items = new(numItems);
			for (int idx = 0; idx < numItems; idx++)
			{
				TKey key = readKey();
		...
		public static object? ReadObject(this BinaryReader reader, Type objectType)
		{
			...
			else if (objectType.IsArray && objectType.GetElementType()!.IsEnum)
			{
				Type elementType = objectType.GetElementType()!;
				int[]? enumValuesAsInts = reader.ReadArray(reader.ReadInt32);
				if (enumValuesAsInts is null)
				{
					return null;
				}
				Array array = Array.CreateInstance(elementType, enumValuesAsInts.Length);
				enumValuesAsInts.CopyTo(array, 0);
				return array;
			}
			else
			{
