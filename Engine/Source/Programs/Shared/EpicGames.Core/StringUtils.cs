...
namespace EpicGames.Core
{
	...
	public static class StringUtils
	{
		...
		{
			return FormatList([.. arguments], conjunction);
		}
		...
		/// <param name="decimalPlaces">The number of decimal places to round the resulting value</param>
		/// <returns>Human readable string based on the value of bytes</returns>
		public static string FormatBytesString(ulong bytes, int decimalPlaces = 2)
		{
			if (bytes == 0)
			{
				return $"0 {s_byteSizes[0]}";
			...
			int power = Convert.ToInt32(Math.Floor(Math.Log(bytes, 1024)));
			double value = Math.Round(bytes / Math.Pow(1024, power), decimalPlaces);
			return $"{value} {s_byteSizes[power]}";
		}
