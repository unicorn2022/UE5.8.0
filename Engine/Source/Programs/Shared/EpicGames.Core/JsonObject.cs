...
namespace EpicGames.Core
{
	...
	public static class OrderedDictionaryExtensions
	{
		...
		private static object? DeepCopyHelper(object? original)
		{
			...
			{
				OrderedDictionary copy = new(StringComparer.InvariantCultureIgnoreCase);
	...
	public class JsonObject
	{
		...
		public static object? ParseElement(JsonElement element)
		{
			switch (element.ValueKind)
			{
				...
				case JsonValueKind.Object:
					OrderedDictionary dictionary = new(StringComparer.InvariantCultureIgnoreCase);
					foreach (JsonProperty property in element.EnumerateObject())
					{
		...
		public bool TryGetStringArrayField(string fieldName, [NotNullWhen(true)] out string[]? result)
		{
			...
			{
				result = [.. enumValue.Select(x => (string)x)];
				return true;
			}
			else
			{
		...
		/// <param name="fieldName">Name of the field to get</param>
		/// <param name="result">On success, receives the field value</param>
		/// <param name="errors">On failure, details the error(s) that occurred</param>
		/// <returns>True if the field could be read, false otherwise</returns>
		public bool TryGetEnumArrayField<T>(string fieldName, [NotNullWhen(true)] out T[]? result, [NotNullWhen(false)] out string? errors) where T : struct
		{
			if (!ContainsField(fieldName))
			{
				result = null;
				errors = $"Field '{fieldName}' not found";
				return false;
			}
			...
			{
				result = null;
				errors = $"Field '{fieldName}' has an incorrect type - it must be an array of strings";
				return false;
			}
			...
			{
				if (!Enum.TryParse(stringValues[idx], true, out enumValues[idx]))
				{
					success = false;
					errorBuilder ??= new StringBuilder();
					errorBuilder.AppendLine($"Field '{fieldName}' entry at index {idx} '{stringValues[idx]}' is not a valid {typeof(T).Name}");
				}
			}
			...
			{
				result = null;
#pragma warning disable CS8602 // If !success then errorBuilder != null
...
#pragma warning restore CS8602
		...
		public bool TryGetEnumArrayField<T>(string fieldName, [NotNullWhen(true)] out T[]? result) where T : struct
			=> TryGetEnumArrayField(fieldName, out result, out _);
		...
		public bool TryGetObjectArrayField(string fieldName, [NotNullWhen(true)] out JsonObject[]? result)
		{
			...
			{
				result = [.. enumValue.Select(x => new JsonObject((OrderedDictionary)x, false))];
				return true;
			}
			else
			{
		...
		public void AddOrSetFieldValue(string fieldName, string?[]? value)
		{
			...
			string[] stringArray = [.. value.Select(x => x ?? "")];
		...
		public void AddOrSetFieldValue<T>(string fieldName, T[]? value) where T : Enum
		{
			...
			string[] stringArray = [.. value.Select(x => x.ToString()!)];
			_rawOrderedObject[fieldName] = stringArray;
		}
		...
		public string ToJsonString()
		{
			...
			using MemoryStream jsonMemoryStream = new();
			using (Utf8JsonWriter writer = new(jsonMemoryStream, options))
			{
			...
			string[] lines = jsonString.Split([Environment.NewLine], StringSplitOptions.None);
