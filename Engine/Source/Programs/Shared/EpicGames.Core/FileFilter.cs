...
namespace EpicGames.Core
{
	...
	public class FileFilter
	{
		...
		public static FileFilter? Parse(IReadOnlyList<string>? rules)
		{
			...
			{
				FileFilterType defaultType = rules[0].StartsWith('-') ? FileFilterType.Include : FileFilterType.Exclude;
		...
		public void AddRule(string rule, params string[] allowTags)
		{
			...
			if (cleanRule.StartsWith('{'))
			{
				...
				string[] ruleTags = [.. cleanRule.Substring(1, conditionEnd - 1).Split(',').Select(x => x.Trim())];
		...
		public bool PossiblyMatches(string folderName)
		{
			...
			FileFilterNode matchingNode = FindMatchingNode(_rootNode, [.. tokens.Union([""])], 0, _defaultNode);
		...
		{
			FileFilter filter = new(FileFilterType.Exclude);
