...
namespace EpicGames.Core
{
	...
	[JsonConverter(typeof(CommandLineArgumentsJsonConverter))]
	public class CommandLineArguments : IReadOnlyList<string>, IReadOnlyCollection<string>, IEnumerable<string>, IEnumerable
	{
			...
			for (int idx = 0; idx < arguments.Length; idx++)
			{
				...
				else if (arguments[idx].StartsWith('-'))
				{
					...
					if (separatorIdx == -1)
					{
						...
						if (!_argumentToFirstIndex.TryAdd(arguments[idx], idx))
						{
		...
		static List<ArgumentTarget> GetArgumentTargetsForType(Type? targetType)
		{
			...
			return s_cachedArgumentTargetsForType.GetOrAdd(targetType, static targetType => new Lazy<List<ArgumentTarget>>(() =>
			{
				...
				foreach (FieldInfo fieldInfo in targetType.GetFields(BindingFlags.Instance | BindingFlags.GetField | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.DeclaredOnly))
				{
					...
					if (attributes.Any())
					{
						targets.Add(new ArgumentTarget(fieldInfo, fieldInfo.FieldType, fieldInfo.SetValue, fieldInfo.GetValue, [.. attributes]));
					}
				}
				foreach (PropertyInfo propertyInfo in targetType.GetProperties(BindingFlags.Instance | BindingFlags.GetProperty | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.DeclaredOnly))
				{
					...
					if (attributes.Any())
					{
						targets.Add(new ArgumentTarget(propertyInfo, propertyInfo.PropertyType, propertyInfo.SetValue, propertyInfo.GetValue, [.. attributes]));
					}
				}
		...
		{
			CommandLineArguments newArguments = new([.. Enumerable.Concat(_arguments, appendArguments)]);
		...
		public CommandLineArguments Remove(string prefix, out List<string> values)
		{
			...
			List<string> newArgumentList = new(_arguments.Length);
			for (int idx = 0; idx < _arguments.Length; idx++)
			{
			...
			CommandLineArguments newArguments = new([.. newArgumentList]);
			for (int idx = 0; idx < _arguments.Length; idx++)
			{
	...
	/// <summary>
	/// Serializes command-line arguments to and from an array of strings.
	/// </summary>
	public class CommandLineArgumentsJsonConverter : JsonConverter<CommandLineArguments>
	{
		/// <inheritdoc />
		public override CommandLineArguments? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
		{
			List<string> arguments = [];
		...
		/// <inheritdoc />
		public override void Write(Utf8JsonWriter writer, CommandLineArguments value, JsonSerializerOptions options)
		{
