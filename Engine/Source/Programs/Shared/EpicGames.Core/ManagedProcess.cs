...
#pragma warning restore CA2213
		...
		/// </summary>
		static readonly Lock s_lockObject = new();
		...
		public async Task WaitForExitAsync(CancellationToken cancellationToken = default)
		{
			if (_frameworkProcess == null)
			{
				using ProcessWaitHandle waitHandle = new(_processHandle!);
		...
		public bool HasExited
		{
			get
			{
				if (_frameworkProcess != null)
				{
					try
					{
						// throws exception if the process is still running
						return _frameworkProcess.HasExited;
					}
					catch (Exception)
					{
						return false;
				...
				{
					// returns non-zero on success and success means the process has exited
					if (GetExitCodeProcess(_processHandle!, out _) != 0)
					{
						return true;
					}
					return false;
		...
		/// <summary>
		/// Kill the process if it's still running, optionally kill child processes
		/// </summary>
		/// <param name="killDescendants"></param>
		public void Kill(bool killDescendants = true)
		{
			if (_frameworkProcess != null)
			{
				if (_frameworkProcess.HasExited)
				...
				try
				{
					_frameworkProcess.Kill(killDescendants);
				}
				catch { }
