...
namespace EpicGames.Core
{
	...
	public static class AssemblyUtils
	{
		...
		/// This method is using Assembly.Location property to properly resolve original
		...
		/// <summary>
		/// Adds a file to the assembly cache.
		/// </summary>
		...
		public static void AddFileToAssemblyCache(string assemblyPath)
		{
			...
			{
				if (!s_assemblyLocationCache.TryAdd(assemblyName, assemblyPath))
				{
			...
			AddAssemblyCacheResolverIfNecessary();
		}

		private static void AddAssemblyCacheResolverIfNecessary()
		{
				...
				AppDomain.CurrentDomain.AssemblyResolve += (sender, args) =>
				{
					...
					lock (s_assemblyLocationCache)
					{
						if (s_assemblyLocationCache.TryGetValue(assemblyName, out string? assemblyLocation))
						{
							...
							{
								// The assembly still exists, so load it into the default, shared context, so we only get one version of the assembly.
								return AssemblyLoadContext.Default.LoadFromAssemblyPath(assemblyLocation);
							}
							else
							{
		...
		private static readonly Dictionary<string, string> s_assemblyLocationCache = new(StringComparer.OrdinalIgnoreCase);
		...
		private static readonly Dictionary<string, DateTime> s_assemblyWriteTimes = new(StringComparer.OrdinalIgnoreCase);
