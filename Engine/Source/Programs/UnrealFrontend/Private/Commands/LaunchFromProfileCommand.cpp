...
void FLaunchFromProfileCommand::Run(const FString& Params)
{
	...
	{
		UE_LOGF(LogUFECommands, Warning, "Profile name was found.  Please use '-PROFILENAME=' in your command line.");
		return;
	}
	...
	UE_LOGF(LogUFECommands, Display, "Begin the process of launching a project using the provided profile.");
	...
	UE_LOGF(LogUFECommands, Display, "There are '%i' tasks to be completed.", NumOfTasks);
	...
	while (bTestRunning)
	{
		...
		if (CurrentTask->GetStatus() == ELauncherTaskStatus::Busy)
		{
			...
			UE_LOGF(LogUFECommands, Display, "Current Task is %ls", *TriggeredTask);
			TaskIndex++;
		}
	}
}
...
{
	UE_LOGF(LogUFECommands, Log, "Profile launch command %ls.", Outcome ? TEXT("is SUCCESSFUL") : TEXT("has FAILED"));
	bTestRunning = false;
}

void FLaunchFromProfileCommand::LaunchCanceled(double ExecutionTime)
{
	UE_LOGF(LogUFECommands, Log, "Profile launch command was canceled.");
	bTestRunning = false;
}