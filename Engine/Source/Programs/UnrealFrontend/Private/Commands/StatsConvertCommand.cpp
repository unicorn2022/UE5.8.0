...
void FCSVStatsProfiler::ReadStatsFrame( const TArray<FStatMessage>& CondensedMessages, const int64 Frame )
{
	...
	for (int32 Index = 0; Index < Stats.Num(); ++Index)
	{
		...
		//UE_LOGF(LogTemp, Display, "Stat: %ls", *Meta.NameAndInfo.GetShortName().ToString());
...
void FStatsConvertCommand::InternalRun()
{
	...
	if (!FileWriter)
	{
		UE_LOGF( LogStats, Error, "Could not open output file: %ls", *OutFile );
		return;
	}
	...
	if (Instance)
	{
		...
		while (Instance->IsBusy())
		{
			...
			UE_LOGF( LogStats, Log, "FStatsConvertCommand: Stage: %ls / %3i%%", *Instance->GetProcessingStageAsString(), Instance->GetStageProgress() );
		}
	}
}
