...
namespace UnrealGameSync
{
	...
	static class Launcher
	{
		...
		public static LauncherResult SyncAndRunLatest(Mutex instanceMutex, string[] args)
		{
			...
			// add application folder to the path
			try
			{
				string? path = Environment.GetEnvironmentVariable("Path", EnvironmentVariableTarget.User);
				if (String.IsNullOrWhiteSpace(path))
				{
					Environment.SetEnvironmentVariable("Path", applicationFolder.FullName, EnvironmentVariableTarget.User);
				}
				else if (!path.Contains(applicationFolder.FullName, StringComparison.OrdinalIgnoreCase))
				{
					Environment.SetEnvironmentVariable("Path", path + $";{applicationFolder.FullName}", EnvironmentVariableTarget.User);
				}
			}
			catch (Exception)
			{
				// Ignore failures to modify the path
			}
...
#pragma warning restore CA1508
					...
					if (syncText == null || syncText != requiredSyncText)
					{
						...
						if (syncPath.EndsWith(".zip", StringComparison.OrdinalIgnoreCase))
						{
							...
							await ZipFile.ExtractToDirectoryAsync(localPath, applicationFolder, cancellationToken);
						}
