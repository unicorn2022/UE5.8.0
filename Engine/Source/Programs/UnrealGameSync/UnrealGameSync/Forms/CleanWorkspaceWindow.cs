	...
	partial class CleanWorkspaceWindow : ThemedForm
	{
		...
		private TreeNode BuildTreeViewStructure(FolderToClean folder, string folderPath, bool parentFolderSelected, int depth)
		{
			...
			foreach (FolderToClean subFolder in folder._nameToSubFolder.OrderBy(x => x.Key).Select(x => x.Value))
			{
				...
				TreeNodeData childNodeData = childNode.Tag as TreeNodeData ?? throw new InvalidOperationException("Missing node data");
		...
		private void TreeView_NodeMouseClick(object sender, TreeNodeMouseClickEventArgs e)
		{
			...
			if (e.Button == System.Windows.Forms.MouseButtons.Right)
			{
				...
				FolderContextMenu.Tag = node;
				FolderContextMenu.Show(TreeView.PointToScreen(e.Location));
			}
			else if (e.X >= node.Bounds.Left - 32 && e.X < node.Bounds.Left - 16)
			{
				TreeNodeData nodeData = node.Tag as TreeNodeData ?? throw new InvalidOperationException("Missing node data");
		...
		{
			TreeNodeData parentNodeData = parentNode.Tag as TreeNodeData ?? throw new InvalidOperationException("Missing node data");
			...
			if (deltaNumSelectedFiles != 0)
			{
				...
				{
					TreeNodeData nextParentNodeData = nextParentNode.Tag as TreeNodeData ?? throw new InvalidOperationException("Missing node data");
		...
		{
			TreeNode node = (TreeNode)FolderContextMenu.Tag!;
			TreeNodeData? nodeData = node.Tag as TreeNodeData;
