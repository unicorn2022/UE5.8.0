...
#pragma warning restore CA2213
		...
		public WorkspaceControl(IWorkspaceControlOwner owner, DirectoryReference appDataFolder, string? apiUrl, OpenProjectInfo openProjectInfo, IServiceProvider serviceProvider, UserSettings settings)
		{
			...
			if (String.IsNullOrWhiteSpace(_projectSettings.Version))
			{
				_projectSettings.Preset = _workspaceSettings.Preset;
				_projectSettings.Save(_logger);
			}

			bool forceDefaultPreset = false;

			if (ConfigUtils.GetForceDefaultPreset(openProjectInfo.LatestProjectConfigFile, ProjectInfo.ProjectIdentifier, out bool configForceDefaultPreset))
			{
				forceDefaultPreset = configForceDefaultPreset;
			}

			string defaultPreset = String.Empty;
			if (ConfigUtils.GetDefaultPreset(openProjectInfo.LatestProjectConfigFile, ProjectInfo.ProjectIdentifier, out string configDefaultPreset))
			{
			...
			{
				if (ConfigUtils.IsPresetAvailable(openProjectInfo.LatestProjectConfigFile, ProjectInfo.ProjectIdentifier, defaultPreset))
				{
			...
			else
			{
				...
				else if (!ConfigUtils.IsPresetAvailable(openProjectInfo.LatestProjectConfigFile, ProjectInfo.ProjectIdentifier, _projectSettings.Preset))
				{
			...
			{
				_workspaceSettings.Preset = currentPreset;
				_projectSettings.Preset = currentPreset;
				_projectSettings.Save(_logger);
			}

			if (ProjectInfo.LocalFileName.HasExtension(".uefnproject") && ConfigUtils.GetForceSyncBaseContent(openProjectInfo.LatestProjectConfigFile, SelectedProjectIdentifier, out bool forceSyncBaseContent))
			{
				if (forceSyncBaseContent && _workspaceSettings.SyncBaseContent == false)
				{
					_workspaceSettings.SyncBaseContent = forceSyncBaseContent;
			...
			_perforceMonitor.OnStreamViewChange += StreamViewChanged;
			_perforceMonitor.OnLoginExpired += LoginExpired;

			string projectOverride = String.Empty;

			ConfigSection? projectConfigSection = openProjectInfo.LatestProjectConfigFile.FindSection(SelectedProjectIdentifier);
			if (projectConfigSection != null)
			{
				projectOverride = projectConfigSection.GetValue("CloudArchivesProjectOverride", String.Empty);
			}
			...
			{
				_eventMonitor = new EventMonitor(_apiUrl, PerforceUtils.GetClientOrDepotDirectoryName(projectOverride), openProjectInfo.PerforceSettings.UserName, _serviceProvider);
			}
		...
		{
			RefreshAfterStreamChange();

			// Flag that the stream has been switched so we can prompt to clean the workspace
			// We only do this when the stream itself has switched not the view
			//_workspace?.ModifyState(x =>
			//{
			//	x.StreamSwitched = true;
			//});
		}

		private void StreamViewChanged()
		{
			RefreshAfterStreamChange();
		}

		private void RefreshAfterStreamChange()
		{
		...
		async void StartWorkspaceUpdate(int changeNumber, WorkspaceUpdateOptions options, WorkspaceUpdateCallback? callback, bool skipKnownChangeLists)
		{
			try
			{
				...
				if (ProjectInfo.IsUEFNProject && _workspaceSettings.SyncBaseContent)
				{
					options |= WorkspaceUpdateOptions.SyncBaseContent;
				}

				string[] combinedSyncFilter = UserSettings.GetCombinedSyncFilter(
					GetSyncCategories(),
					_projectSettings.Preset,
					GetPresets(),
					_settings.Global.Filter,
					_workspaceSettings.Filter,
					_perforceMonitor.LatestPerforceConfigSection());

				WorkspaceUpdateContext context = new WorkspaceUpdateContext(
					_serviceProvider,
					_settings,
					_workspaceSettings,
					_projectSettings,
					...
					combinedSyncFilter,
					_projectSettings.BuildSteps,
					null);

				context.AdditionalPathsToSync = GetAdditionalPathsToSync();

				if (options.HasFlag(WorkspaceUpdateOptions.SyncBaseContent) && ProjectInfo.IsUEFNProject)
				{
					context.BaseContentPaths = GetBaseContentPaths();
				}

				if (options.HasFlag(WorkspaceUpdateOptions.SyncArchives) && !_projectSettings.ArchivesDeactivated)
				{
					if (!ProjectInfo.IsUEFNProject || options.HasFlag(WorkspaceUpdateOptions.SyncBaseContent))
					{
						...
						// check if there is any project override and take precedence on the global archive
						List<IArchiveChannel> selectedProjectOverrideArchiveChannels = GetSelectedProjectOverrideArchiveChannels(archives);
						if (selectedProjectOverrideArchiveChannels.Any())
						{
							selectedArchiveChannels = selectedProjectOverrideArchiveChannels;
						}

						int archivesChangeNumber = changeNumber;

						ConfigSection? projectSection = _perforceMonitor.LatestProjectConfigFile.FindSection(SelectedProjectIdentifier);
						if (projectSection != null)
						{
							string changeListOverride = projectSection.GetValue("ArchivesChangeListOverride", String.Empty);
							if (!String.IsNullOrWhiteSpace(changeListOverride))
							{
								archivesChangeNumber = await ArchiveOverrideFactory.Create(changeListOverride, _perforceSettings, _logger).GetChangelistAsync(changeNumber);
								skipKnownChangeLists = true;
								SyncLog.AppendLine($"Using CL override '{changeNumber}' -> '{archivesChangeNumber}' for '{SelectedProjectIdentifier}'");
							}
						}

						foreach (IArchiveChannel archiveChannel in selectedArchiveChannels)
						{
							IArchive? archivePath = GetArchiveForChangeNumber(archiveChannel, archivesChangeNumber, skipKnownChangeLists);
							if (archivePath == null)
							{
			...
			{
				_logger.LogError(e, "Exception raised in StartWorkspaceUpdate");
			}
		}
		...
		void UpdateComplete(WorkspaceUpdateContext context, WorkspaceUpdateResult result)
		{
			...
			else if (result == WorkspaceUpdateResult.FilesToClobber)
			{
				...
				else
				{
					...
					{
						_workspace?.ModifyState(x => x.CurrentChangeNumber = -1);
					}
				}
			}
		...
		{
			if (ConfigUtils.GetDefaultPreset(_perforceMonitor.LatestProjectConfigFile, SelectedProjectIdentifier, out string? defaultPreset))
			{
		...
		bool GetForceDefaultPreset()
		{
			if (ConfigUtils.GetForceDefaultPreset(_perforceMonitor.LatestProjectConfigFile, SelectedProjectIdentifier, out bool forceDefaultPreset))
			{
				return forceDefaultPreset;
			}

			return false;
		}

		ISet<string> GetAdditionalPathsToSync()
		{
			return ConfigUtils.GetAdditionalPathsToSync(_perforceMonitor.LatestProjectConfigFile);
		}

		ISet<string> GetBaseContentPaths()
		{
			return ConfigUtils.GetBaseContentPaths(_perforceMonitor.LatestProjectConfigFile, SelectedProjectIdentifier);
		}
		...
		private bool CanSyncChange(int changeNumber, bool skipKnownChangeLists)
		{
			...
			// if the PCBS are deactivated for this project we can sync any changelist
			if (_projectSettings.ArchivesDeactivated)
			{
				return true;
			}

			// get the list of available archive channels
			IReadOnlyList<IArchiveChannel> archiveChannels = GetArchiveChannels();

			List<IArchiveChannel> selectedArchives = GetSelectedArchiveChannels(archiveChannels);

			// check if there is a project override, it takes precedence on the global selected archive
			List<IArchiveChannel> selectedProjectOverrideArchives = GetSelectedProjectOverrideArchiveChannels(archiveChannels);
			if (selectedProjectOverrideArchives.Any())
			{
				selectedArchives = selectedProjectOverrideArchives;
			}
		...
		private void ShowEditorLaunchError(string message)
		{
			if (MessageBox.Show($"The editor needs to be built before you can run it. Build it now?\r\n\r\n({message})", "Editor out of date", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.Yes)
			{
				...
				WorkspaceUpdateContext context = new WorkspaceUpdateContext(_serviceProvider, _settings, _workspaceSettings, _projectSettings, _workspace.CurrentChangeNumber, options, _settings.CompiledEditorBuildConfig, null, _projectSettings.BuildSteps, null);
		...
		private void UpdateStatusPanel()
		{
			...
			else
			{
				...
				if (_workspace.State.StreamSwitched)
				{
					ApplicationTheme theme = ProgramApplicationContext.GetTheme();

					lines.Add(new StatusLine() { LineHeight = 0.5f });

					StatusLine streamSwitchLine = new StatusLine();
					streamSwitchLine.AddIcon(Properties.Resources.StatusIcons, new Size(16, 16), 4);
					streamSwitchLine.AddText("  Workspace may be broken after switching stream  ", theme.WarningColor);
					streamSwitchLine.AddLink("Clean Workspace To Fix", FontStyle.Bold | FontStyle.Underline, () => { CleanWorkspace(); });
					streamSwitchLine.AddText("  |  ");
					streamSwitchLine.AddLink("Ignore", FontStyle.Regular, () => 
					{ 
						_workspace?.ModifyState(x => 
						{ 
							x.StreamSwitched = false; 
						}); 
					});

					lines.Add(streamSwitchLine);
				}
				...
				if (_workspace.State.LastSyncChangeNumber > 0)
				{
					...
					{
						float spacing = _workspace.State.StreamSwitched ? 0.3f : 0.5f;
						lines.Add(new StatusLine() { LineHeight = spacing });
			...
			List<StatusLine> infoLines = new List<StatusLine>();
			...
			if (projectConfigFile != null)
			{
				...
				if (TryGetProjectSetting(projectConfigFile, "CloudArchivesProjectOverride", out string? projectOverride))
				{
					if (String.IsNullOrWhiteSpace(projectOverride) == false)
					{
						if (ConfigUtils.TryGetProjectSetting(projectConfigFile, projectOverride, "Message", out string? value))
						{
				...
				{
					if (TryGetProjectSetting(projectConfigFile, "Message", out string? value))
					{
						if (!String.IsNullOrWhiteSpace(value))
						{
							message = value;
			...
			StatusPanel.Set(lines, infoLines, caption, alert, tintColor);
		}
...
#pragma warning restore CA2000
		...
		private static StatusLine CreateStatusLineFromMarkdown(string text, bool applyDpiScalingToIcons = true)
		{
			...
			for (int idx = 0; idx < text.Length;)
			{
				...
				if (text[idx] == ':')
				{
					...
					if (endIdx != -1)
					{
						...
						{
							FlushMarkdownText(line, elementText, style);
							line.AddIcon(Properties.Resources.StatusIcons, new Size(16, 16), 4, applyDpiScalingToIcons);
							idx = endIdx + 1;
							continue;
						}

						if (String.Compare(":tick:", 0, text, idx, endIdx - idx, StringComparison.Ordinal) == 0)
						{
							FlushMarkdownText(line, elementText, style);
							line.AddIcon(Properties.Resources.StatusIcons, new Size(16, 16), 0, applyDpiScalingToIcons);
							idx = endIdx + 1;
							continue;
						}
					}
...
#pragma warning restore CA2000 // Dispose objects before losing scope
		...
		private void ShowChangelistContextMenu(Control parentControl, bool showTimeRelatedItems, Point location, ToolStripDropDownDirection direction = ToolStripDropDownDirection.BelowRight)
		{
			...
			{
				// Disable options that are not available for UEFn Projects
				BuildListContextMenu_GenerateProjectFiles.Enabled = !ProjectInfo.IsUEFNProject;
				BuildListContextMenu_Build.Enabled = !ProjectInfo.IsUEFNProject;
				BuildListContextMenu_Rebuild.Enabled = !ProjectInfo.IsUEFNProject;
				BuildListContextMenu_OpenVisualStudio.Enabled = !ProjectInfo.IsUEFNProject;
...
#pragma warning restore CA2000
		...
		private static List<IArchiveChannel> GetSelectedArchiveChannels(IReadOnlyList<IArchiveChannel> archives, IReadOnlyList<ArchiveSettings> archiveSettingsList)
		{
			...
			foreach (IArchiveChannel archive in archives)
			{
				ArchiveSettings? archiveSettings = archiveSettingsList.FirstOrDefault(x => x.Type == archive.Type);
		...
		{
			return GetSelectedArchiveChannels(archives, _settings.Archives);
		}

		private List<IArchiveChannel> GetSelectedProjectOverrideArchiveChannels(IReadOnlyList<IArchiveChannel> archives)
		{
			return GetSelectedArchiveChannels(archives, _projectSettings.Archives);
		}
		...
		private void ClearSelectedProjectOverrideArchives()
		{
			foreach (ArchiveSettings archiveSettings in _projectSettings.Archives)
			{
				archiveSettings.Enabled = false;
			}

			_projectSettings.ArchivesDeactivated = false;
			_projectSettings.Save(_logger);

			UpdateSelectedArchives();
		}

		private void DeactivateSelectedProjectOverrideArchives()
		{
			foreach (ArchiveSettings archiveSettings in _projectSettings.Archives)
			{
				archiveSettings.Enabled = false;
			...
			_projectSettings.ArchivesDeactivated = !_projectSettings.ArchivesDeactivated;
			_projectSettings.Save(_logger);
		...
		private void SetSelectedArchiveProjectOverride(IArchiveChannel archive, bool selected)
		{
			ArchiveSettings? archiveSettings = _projectSettings.Archives.FirstOrDefault(x => x.Type == archive.Type);
			if (archiveSettings == null)
			{
				archiveSettings = new ArchiveSettings(selected, archive.Type, new string[] { archive.Name });
				_projectSettings.Archives.Add(archiveSettings);
			}
			else if (selected)
			{
				archiveSettings.Enabled = true;
		...
		private void UpdateSelectedArchivesLabel()
		{
			string text = ProjectInfo.IsUEFNProject ? "PCB: Do not sync PCBs" : "PCB: Compile locally";

			if (!ProjectInfo.IsUEFNProject || _workspaceSettings.SyncBaseContent)
			{
				IReadOnlyList<IArchiveChannel> channels = GetArchiveChannels();

				foreach (IArchiveChannel selectedArchive in GetSelectedArchiveChannels(channels))
				{
					text = $"PCB: {selectedArchive.Name}";
					break;
				}

				if (_projectSettings.ArchivesDeactivated)
				{
					text = "PCB: Do not sync PCBs (project)";
				}
				else
				{
					foreach (IArchiveChannel selectedArchive in GetSelectedProjectOverrideArchiveChannels(channels))
					{
						text = $"PCB: {selectedArchive.Name} (project)";
						break;
					}
				}
			}

			PCBLabel.Enabled = true;
			PCBLabel.Text = text;
		}
		...
		private void OptionsButton_Click(object sender, EventArgs e)
		{
			...
			OptionsContextMenu_SyncPrecompiledBinariesProjectOverride.DropDownItems.Clear();

			IReadOnlyList<IArchiveChannel> channels = GetArchiveChannels();

			if (ProjectInfo.IsUEFNProject && _workspaceSettings.SyncBaseContent == false)
			{
				OptionsContextMenu_SyncPrecompiledBinaries.Enabled = false;
				OptionsContextMenu_SyncPrecompiledBinaries.ToolTipText = "Go to Sync Filter... and tick 'Sync Base Content' to unlock";
				OptionsContextMenu_SyncPrecompiledBinaries.Checked = false;

				OptionsContextMenu_SyncPrecompiledBinariesProjectOverride.Enabled = false;
				OptionsContextMenu_SyncPrecompiledBinariesProjectOverride.ToolTipText = "Go to Sync Filter... and tick 'Sync Base Content' to unlock";
				OptionsContextMenu_SyncPrecompiledBinariesProjectOverride.Checked = false;
			}
			else if (channels.Count == 0)
			{
				OptionsContextMenu_SyncPrecompiledBinaries.Enabled = false;
				OptionsContextMenu_SyncPrecompiledBinaries.ToolTipText = $@"Precompiled binaries are not available for {SelectedProjectIdentifier}";
				OptionsContextMenu_SyncPrecompiledBinaries.Checked = false;

				OptionsContextMenu_SyncPrecompiledBinariesProjectOverride.Enabled = false;
				OptionsContextMenu_SyncPrecompiledBinariesProjectOverride.ToolTipText = $@"Precompiled binaries are not available for {SelectedProjectIdentifier}";
				OptionsContextMenu_SyncPrecompiledBinariesProjectOverride.Checked = false;
			}
			else
			{
				// setup global archives channels
				{
					IReadOnlyList<IArchiveChannel> selectedArchives = GetSelectedArchiveChannels(channels);
				...
				// setup project archives channels
				{
					IReadOnlyList<IArchiveChannel> selectedArchives = GetSelectedProjectOverrideArchiveChannels(channels);

					OptionsContextMenu_SyncPrecompiledBinariesProjectOverride.Enabled = true;
					OptionsContextMenu_SyncPrecompiledBinariesProjectOverride.ToolTipText = null;
					OptionsContextMenu_SyncPrecompiledBinariesProjectOverride.Checked = selectedArchives.Count > 0;

					if (channels.Any())
					{
						ToolStripMenuItem disableItem = new ToolStripMenuItem("Unset (no override)");
						disableItem.Checked = (selectedArchives.Count == 0) && (_projectSettings.ArchivesDeactivated == false);
						disableItem.Click += (sender, args) => ClearSelectedProjectOverrideArchives();
						OptionsContextMenu_SyncPrecompiledBinariesProjectOverride.DropDownItems.Add(disableItem);

						ToolStripMenuItem deactivateItem = new ToolStripMenuItem("Do not sync PCBs");
						deactivateItem.Checked = _projectSettings.ArchivesDeactivated;
						deactivateItem.Click += (sender, args) => DeactivateSelectedProjectOverrideArchives();
						OptionsContextMenu_SyncPrecompiledBinariesProjectOverride.DropDownItems.Add(deactivateItem);

						ToolStripSeparator separator = new ToolStripSeparator();
						OptionsContextMenu_SyncPrecompiledBinariesProjectOverride.DropDownItems.Add(separator);
						...
						foreach (IArchiveChannel channel in channels)
						{
							...
							item.ToolTipText = channel.ToolTip;
							item.Checked = selectedArchives.Contains(channel);
							item.Click += (sender, args) => SetSelectedArchiveProjectOverride(channel, !item.Checked);
							OptionsContextMenu_SyncPrecompiledBinariesProjectOverride.DropDownItems.Add(item);
						}
					}
				}
			}
		...
		public void ExecCommand(string description, string statusText, string fileName, string arguments, string workingDir, bool useLogWindow)
		{
			if (_workspace != null && !_workspace.IsBusy())
			{
				...
				WorkspaceUpdateContext context = new WorkspaceUpdateContext(_serviceProvider, _settings, _workspaceSettings, _projectSettings, _workspace.CurrentChangeNumber, WorkspaceUpdateOptions.Build, GetEditorBuildConfig(), null, userBuildSteps, new HashSet<Guid> { customBuildStep.UniqueId });
				StartWorkspaceUpdate(context, null);
			}
		}
		...
		public void ScheduleTimerElapsed()
		{
			if (_settings.ScheduleEnabled)
			{
				...
				{
					WorkspaceUpdateOptions options = WorkspaceUpdateOptions.Sync
												 | WorkspaceUpdateOptions.ScheduledBuild
												 | WorkspaceUpdateOptions.SyncArchives
												 | WorkspaceUpdateOptions.GenerateProjectFiles
		...
		private void CleanWorkspace()
		{
			...
			string[] combinedSyncFilter = UserSettings.GetCombinedSyncFilter(GetSyncCategories(), _projectSettings.Preset, GetPresets(), _settings.Global.Filter, _workspaceSettings.Filter, _perforceMonitor.LatestPerforceConfigSection());
			ISet<string> baseContentPaths = GetBaseContentPaths();
			ISet<string> additionalPathsTosync = GetAdditionalPathsToSync();
			WorkspaceUpdateOptions options = WorkspaceUpdateContext.GetOptionsFromConfig(_settings.Global, _workspaceSettings);
			List<string> syncPaths = WorkspaceUpdate.GetSyncPaths(_workspace.Project, options, combinedSyncFilter, additionalPathsTosync, baseContentPaths);
...
#pragma warning restore CA2000
			...
			MoreToolsContextMenu.ResumeLayout();
		}
		...
		private void RunCustomTool(BuildStep step, List<BuildStep> allSteps)
		{
			if (_workspace != null)
			{
				...
				else
				{
					...
					{
						RequirementCount += step.LocalCompileRequires.Count;
					}
					ConfigFile projectConfigFile = _workspace.ProjectConfigFile;
					if (projectConfigFile != null && RequirementCount > 0)
					{
						...
						while (stack.Count > 0)
						{
							...
							if (nextStep != null)
							{
								...
								if (!UsingPreCompiledExecutables)
								{
									foreach (Guid requiresId in nextStep.LocalCompileRequires)
									{
										if (stepSet.Add(requiresId))
										{
											stack.Push(requiresId);
					...
					WorkspaceUpdateContext context = new WorkspaceUpdateContext(_serviceProvider, _settings, _workspaceSettings, _projectSettings, _workspace.CurrentChangeNumber, WorkspaceUpdateOptions.Build, GetEditorBuildConfig(), null, _projectSettings.BuildSteps, stepSet);
		...
		private bool ShouldSyncPrecompiledEditor => ShouldSyncPrecompiledEditor_Impl();

		private bool ShouldSyncPrecompiledEditor_Impl()
		{
			if (ProjectInfo.IsUEFNProject && _workspaceSettings.SyncBaseContent == false)
			{
				return false;
			...
			IReadOnlyList<IArchiveChannel> channels = GetArchiveChannels();
			if (channels.Any(x => x.Type == "Editor"))
			{
				if (_projectSettings.ArchivesDeactivated)
				{
					return false;
				...
				List<IArchiveChannel> overrides = GetSelectedProjectOverrideArchiveChannels(channels);
				if (overrides.Any(x => x.Type == IArchiveChannel.EditorArchiveType))
				{
					return true;
				}

				List<IArchiveChannel> globals = GetSelectedArchiveChannels(channels);
				if (globals.Any(x => x.Type == IArchiveChannel.EditorArchiveType))
				{
					return true;
				}
			}
		...
		private Dictionary<Guid, ConfigObject> GetProjectBuildStepObjects(ConfigFile projectConfigFile)
		{
			...
			foreach (string line in projectConfigFile.GetValues("Build.Step", [], ProjectInfo.ProjectIdentifier))
			{
		...
		private void OptionsContextMenu_SyncPrecompiledBinariesProjectOverride_Click(object sender, EventArgs e)
		{
			if (OptionsContextMenu_SyncPrecompiledBinariesProjectOverride.DropDownItems.Count == 0)
			{
				IArchiveChannel? editorArchive = GetArchiveChannels().FirstOrDefault(x => x.Type == IArchiveChannel.EditorArchiveType);
				if (editorArchive != null)
				{
					SetSelectedArchiveProjectOverride(editorArchive, !OptionsContextMenu_SyncPrecompiledBinariesProjectOverride.Checked);
				}
			}
		}
		...
		{
			using SyncFilter filter = new SyncFilter(
				_workspaceSettings,
				_projectSettings,
				GetSyncCategories(), 
				_workspaceSettings.Preset, 
				GetDefaultPreset(),
				GetForceDefaultPreset(),
				GetPresets(), 
				GetBaseContentPaths(),
				_settings.Global.Filter,
				_workspaceSettings.Filter,
				_perforceMonitor.LatestPerforceConfigSection(),
				ProjectInfo.IsUEFNProject);
		...
		private void OptionsContextMenu_Presets_Click(object sender, EventArgs e)
		{
			...
			{
				_projectSettings.Preset = presetWindow.PresetName;
				_projectSettings.PresetSetByUser = true;
				_projectSettings.Save(_logger);
			}
		}
