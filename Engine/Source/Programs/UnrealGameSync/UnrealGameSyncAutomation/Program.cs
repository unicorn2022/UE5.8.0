	...
	class Program
	{
		...
		static Tuple<AutomationRequestResult, byte[]> SendRequest(AutomationRequestType Request, byte[] RequestData)
		{
			object PortValue = Registry.GetValue("HKEY_CURRENT_USER\\Software\\Epic Games\\UnrealGameSync", "AutomationPort", null)
				?? throw new InvalidOperationException("AutomationPort registry value is not set.");
			if (!int.TryParse(PortValue.ToString(), out int PortNumber) || PortNumber < 1 || PortNumber > 65535)
			{
				throw new InvalidOperationException($"AutomationPort registry value '{PortValue}' is not a valid TCP port number (1-65535).");
			}
