...
namespace UnrealGameSync
{
	...
	public class ConfigFile
	{
		...
		public bool GetValue(string key, bool defaultValue, string? projectIdentifier)
		{
			int dotIdx = key.IndexOf('.', StringComparison.Ordinal);

			if (!String.IsNullOrWhiteSpace(projectIdentifier))
			{
				string? sectionName = key.Substring(0, dotIdx);
				string? extension = Path.GetExtension(projectIdentifier);
				if (!String.IsNullOrWhiteSpace(sectionName) && !String.IsNullOrWhiteSpace(extension))
				{
					ConfigSection? projectSection = FindSection($"{sectionName}{extension}");
					if (projectSection != null)
					{
						return projectSection.GetValue(key.Substring(dotIdx + 1), defaultValue);
					}
				}
			}
		...
		public int GetValue(string key, int defaultValue, string? projectIdentifier)
		{
			int dotIdx = key.IndexOf('.', StringComparison.Ordinal);
		...
		public int? GetOptionalIntValue(string key, int? defaultValue, string? projectIdentifier)
		{
			int dotIdx = key.IndexOf('.', StringComparison.Ordinal);
			...
			if (!String.IsNullOrWhiteSpace(projectIdentifier))
			{
				...
				if (!String.IsNullOrWhiteSpace(sectionName) && !String.IsNullOrWhiteSpace(extension))
				{
					...
					if (projectSection != null)
					{
						return projectSection.GetOptionalIntValue(key.Substring(dotIdx + 1), defaultValue);
					}
				}
			}
		...
		[return: NotNullIfNotNull("defaultValue")]
		public string? GetValue(string key, string? defaultValue, string? projectIdentifier)
		{
			int dotIdx = key.IndexOf('.', StringComparison.Ordinal);
		...
		public TEnum GetEnumValue<TEnum>(string key, TEnum defaultValue, string? projectIdentifier) where TEnum : struct
		{
			int dotIdx = key.IndexOf('.', StringComparison.Ordinal);
			...
			if (!String.IsNullOrWhiteSpace(projectIdentifier))
			{
				...
				if (!String.IsNullOrWhiteSpace(sectionName) && !String.IsNullOrWhiteSpace(extension))
				{
					...
					if (projectSection != null)
					{
						return projectSection.GetEnumValue(key.Substring(dotIdx + 1), defaultValue);
					}
				}
			}
		...
		[return: NotNullIfNotNull("defaultValue")]
		public string[]? GetValues(string key, string[]? defaultValue, string? projectIdentifier)
		{
			...
			if (section != null)
			{
				values.AddRange(section.GetValues(key.Substring(dotIdx + 1), (string[]?)[]) ?? []);
			}
			...
			if (!String.IsNullOrWhiteSpace(projectIdentifier))
			{
				...
				if (!String.IsNullOrWhiteSpace(sectionName) && !String.IsNullOrWhiteSpace(extension))
				{
					...
					if (projectSection != null)
					{
						values.AddRange(projectSection.GetValues(key.Substring(dotIdx + 1), (string[]?)[]) ?? []);
					}
				}
			}
		...
		[return: NotNullIfNotNull("defaultValue")]
		public Guid[]? GetGuidValues(string key, Guid[]? defaultValue, string? projectIdentifier)
		{
			...
			if (section != null)
			{
				values.AddRange(section.GetValues(key.Substring(dotIdx + 1), (Guid[]?)[]) ?? []);
			}
			...
			if (!String.IsNullOrWhiteSpace(projectIdentifier))
			{
				...
				if (!String.IsNullOrWhiteSpace(sectionName) && !String.IsNullOrWhiteSpace(extension))
				{
					...
					if (projectSection != null)
					{
						values.AddRange(projectSection.GetValues(key.Substring(dotIdx + 1), (Guid[]?)[]) ?? []);
					}
				}
			}
