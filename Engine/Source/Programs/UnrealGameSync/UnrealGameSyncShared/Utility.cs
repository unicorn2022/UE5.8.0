	...
	public static class Utility
	{
		...
		public static async IAsyncEnumerable<PerforceChangeDetails> EnumerateChangeDetails(IPerforceConnection perforce, IAsyncEnumerable<int> changeNumbers, IEnumerable<string> codeRules, IEnumerable<string> uefnRules, [EnumeratorCancellation] CancellationToken cancellationToken)
		{
			...
			for (; ; )
			{
				...
				while (changeBatch.Count < BatchSize && await changeNumberEnumerator.MoveNextAsync())
				{
		...
		public static List<FileInfo> GetLocalConfigPaths(DirectoryInfo engineDir, FileInfo projectFile)
		{
			...
			else if (projectFile.Name.EndsWith(".uefnproject", StringComparison.OrdinalIgnoreCase))
			{
				...
				string searchFolder = projectFile.FullName.Substring(0, projectFile.FullName.LastIndexOf(Path.DirectorySeparatorChar));

				string root = Path.GetFullPath($"{engineDir}/..").Replace(Path.AltDirectorySeparatorChar, Path.DirectorySeparatorChar);

				while (searchFolder.Equals(root, StringComparison.OrdinalIgnoreCase) == false)
				{
					int index = searchFolder.LastIndexOf(Path.DirectorySeparatorChar);
					if (index <= 0)
					{
						break;
					}
					...
					AddLocalConfigPaths_WithExtensionDirs(new DirectoryInfo(searchFolder), "Build", "UnrealGameSync.ini", searchPaths);
				}
			}
			else
			{
		...
		{
			searchPaths.Add($"{basePath}/{fileName}");
			searchPaths.Add($"{basePath}/*/{fileName}");
		}
		...
		public static List<string> GetDepotConfigPaths(string enginePath, string projectPath, string branchClientPath)
		{
			...
			else if (projectPath.EndsWith(".uefnproject", StringComparison.OrdinalIgnoreCase))
			{
				...
				while (searchFolder.Equals(branchClientPath, StringComparison.OrdinalIgnoreCase) == false)
				{
