...
namespace UnrealGameSync
{
	...
	public class WorkspaceUpdateContext
	{
		...
		public UserWorkspaceSettings? UserWorkspaceSettings { get; init; }

		public UserProjectSettings? UserProjectSettings { get; init; }
...
#pragma warning disable CA2227 // Collection properties should be read only
		...
		public ISet<string> BaseContentPaths { get; set; } = new HashSet<string>();
		public ISet<string> AdditionalPathsToSync { get; set; } = new HashSet<string>();
#pragma warning restore CA2227

		public WorkspaceUpdateContext(
			IServiceProvider serviceProvider,
			UserSettings? usersettings,
			UserWorkspaceSettings? userWorkspaceSettings,
			UserProjectSettings? userProjectSettings,
			int changeNumber, 
			WorkspaceUpdateOptions options, 
			BuildConfig editorConfig, 
			string[]? syncFilter, 
			List<ConfigObject> userBuildSteps, 
			HashSet<Guid>? customBuildSteps)
		{
	...
	public class ProjectInfo
	{
		...
		// derived properties
		public FileReference LocalFileName { get; } // local path to the project file
		...
		public DirectoryReference? ProjectDir => ProjectPath.EndsWith(".uproject", StringComparison.OrdinalIgnoreCase)|| ProjectPath.EndsWith(".uefnproject", StringComparison.OrdinalIgnoreCase) ? LocalFileName.Directory : null;
		...
		public ProjectInfo(DirectoryReference localRootPath, string clientName, string branchPath, string projectPath, string localProjectPath, string? streamName, string projectIdentifier, bool isEnterpriseProject)
		{
			...
			LocalFileName = new FileReference(localProjectPath);
		}
		...
		public static async Task<ProjectInfo> CreateAsync(IPerforceConnection perforceClient, UserWorkspaceSettings settings, CancellationToken cancellationToken)
		{
			...
			{
				string text;
				if (settings.LocalProjectPath!= null && FileReference.Exists(settings.LocalProjectPath))
				{
			...
			return new ProjectInfo(settings.RootDir, settings.ClientName, settings.BranchPath, settings.ProjectPath, settings.LocalProjectPath!.FullName, streamName, newSelectedProjectIdentifier, isEnterpriseProject);
		}

		public static async Task<string?> TryGetStreamPrefixAsync(IPerforceConnection perforce, string streamName, CancellationToken cancellationToken)
		{
		...
		public async Task<(WorkspaceUpdateResult, string)> ExecuteAsync(IPerforceSettings perforceSettings, ProjectInfo project, WorkspaceStateWrapper stateMgr, ILogger logger, CancellationToken cancellationToken)
		{
			...
			if (Context.Options.HasFlag(WorkspaceUpdateOptions.Sync) || Context.Options.HasFlag(WorkspaceUpdateOptions.SyncSingleChange))
			{
				using (TelemetryStopwatch syncTelemetryStopwatch = new TelemetryStopwatch("Workspace_Sync", project.TelemetryProjectIdentifier))
				{
					...
					List<string> relativeSyncPaths = GetRelativeSyncPaths(project, Context.Options, Context.SyncFilter, Context.AdditionalPathsToSync, Context.BaseContentPaths);
...
#pragma warning restore CA5350 // Do Not Use Weak Cryptographic Algorithms
					...
					using (RecordCounter counter = new RecordCounter(Progress, "Filtering files..."))
					{
						using (TelemetryStopwatch filterStopwatch = new TelemetryStopwatch("Workspace_Sync_FilteringFiles", project.TelemetryProjectIdentifier))
						{
							...
							{
								int maxChangelistDistance = 500_000;
								bool changelistDistanceInRange = Math.Abs(Context.ChangeNumber - state.CurrentChangeNumber) < maxChangelistDistance;
								bool targetChangelistBiggerThanCurrent = (Context.ChangeNumber >= state.CurrentChangeNumber);

								if (state.CurrentChangeNumber > 0 && changelistDistanceInRange && targetChangelistBiggerThanCurrent)
								{
								...
								else
								{
									...
									else if (!targetChangelistBiggerThanCurrent)
									{
									...
									else if (!changelistDistanceInRange)
									{
										logger.LogInformation("Fast Sync cannot be used with current changelist {CurrentChangelist} and target changelist {TargetChangelist} because the distance between them is > {MaxDistance}K changelists.", state.CurrentChangeNumber, Context.ChangeNumber, maxChangelistDistance / 1000);
									}
								}
							}
							...
							{
								async Task<(WorkspaceUpdateResult, string)> inlineGetSyncRecords(bool firstTry, RecordCounter counter)
								{
									...
									await foreach (PerforceResponse<SyncRecord> response in perforce.TrySyncAsync(SyncOptions.PreviewOnly, -1, 0, -1, -1, -1, -1, $"{syncPath}@{syncFilter}", cancellationToken))
									{
										if (!response.Succeeded)
										{
											...
											&& Context.Options.HasFlag(WorkspaceUpdateOptions.FastSync))
											{
												...
												return await inlineGetSyncRecords(false, counter);
											}
								...
								(WorkspaceUpdateResult, string) ret = await inlineGetSyncRecords(true, counter);
					...
					// Save the current changelist before clearing, so we can restore it if the sync
					// transfers files successfully but fails in a post-sync step (e.g. files to resolve).
					// This preserves the ability to use fast sync on retry instead of forcing a full sync.
					int previousChangeNumber = state.CurrentChangeNumber;
					...
					using (TelemetryStopwatch transferStopwatch = new TelemetryStopwatch("Workspace_Sync_TransferFiles", project.TelemetryProjectIdentifier))
					{
						...
						{
							// For retryable results (e.g. FilesToClobber), restore the previous changelist
							// so fast sync remains usable on the next attempt instead of forcing a full sync.
							if (syncResult.Item1 == WorkspaceUpdateResult.FilesToClobber
								&& !Context.Options.HasFlag(WorkspaceUpdateOptions.SyncSingleChange)
								&& previousChangeNumber > 0)
							{
								state = stateMgr.Modify(x => x.CurrentChangeNumber = previousChangeNumber);
							}
					...
					// File transfer succeeded. Restore the previous changelist so that if a post-sync
					// step fails (e.g. files to resolve, version file update), fast sync remains usable
					// on the next retry. The success path below will overwrite this with Context.ChangeNumber.
					if (!Context.Options.HasFlag(WorkspaceUpdateOptions.SyncSingleChange) && previousChangeNumber > 0)
					{
						state = stateMgr.Modify(x => x.CurrentChangeNumber = previousChangeNumber);
					}
					...
					if (Context.Options.HasFlag(WorkspaceUpdateOptions.Sync) && !Context.Options.HasFlag(WorkspaceUpdateOptions.UpdateFilter))
					{
						...
						Context.ProjectConfigFile = await ReadProjectConfigFile(perforce, project, Context.ServiceProvider.GetRequiredService<ILogger<WorkspaceUpdate>>());
						...
						if (!project.ClientFileName.EndsWith(".uefnproject", StringComparison.OrdinalIgnoreCase) 
						  || project.ClientFileName.EndsWith(".uefnproject", StringComparison.OrdinalIgnoreCase) && Context.Options.HasFlag(WorkspaceUpdateOptions.SyncBaseContent))
						{
					...
					if (Context.Options.HasFlag(WorkspaceUpdateOptions.Sync) && !Context.Options.HasFlag(WorkspaceUpdateOptions.UpdateFilter))
					{
						Context.ProjectConfigFile ??= await ConfigUtils.ReadProjectConfigFileAsync(perforce, project, Context.ServiceProvider.GetRequiredService<ILogger<WorkspaceUpdate>>(), cancellationToken);

						// Execute any project specific post-sync steps
						string[]? postSyncSteps = Context.ProjectConfigFile.GetValues("Sync.Step", null, project.ProjectIdentifier);
						...
						if (postSyncSteps != null)
						{
							using (TelemetryStopwatch postSyncStopwatch = new TelemetryStopwatch("Workspace_Sync_PostSyncSteps", project.TelemetryProjectIdentifier))
							{
								...
								Dictionary<string, string> postSyncVariables = ConfigUtils.GetWorkspaceVariables(
									project, 
									Context.ChangeNumber,
									versionChangeNumber, 
									null, 
									Context.ProjectConfigFile, 
									perforceSettings);

								using (PostSyncExtension postSyncExtension = new PostSyncExtension(logger, Context, perforce, project))
								{
									try
									{
										foreach (string postSyncStep in postSyncSteps.Select(x => x.Trim()))
										{
											...
											Dictionary<string, string> expandedPostSyncVariables = await postSyncExtension.ExpandAsync(postSyncStep, postSyncVariables, cancellationToken);

											string toolFileName = Utility.ExpandVariables(postSyncStepObject.GetValue("FileName", ""), expandedPostSyncVariables);
											if (toolFileName != null)
											{
												string toolArguments =
													Utility.ExpandVariables(postSyncStepObject.GetValue("Arguments", ""), expandedPostSyncVariables);
												...
												int resultFromTool = await Utility.ExecuteProcessAsync(
													toolFileName,
													null,
													toolArguments,
													line => ProcessOutput(line, "post-sync> ", Progress, logger),
													cancellationToken);
												if (resultFromTool != 0)
												{
													return (WorkspaceUpdateResult.FailedToSync,
									...
									{
										logger.LogError(e, "Exception raised in post-sync scripts");
										return (WorkspaceUpdateResult.FailedToSync,
														$"Post-sync step terminated with exception {e.Message}.");
									}
								}
			...
			{
				if (!project.IsUEFNProject || Context.Options.HasFlag(WorkspaceUpdateOptions.SyncBaseContent))
				{
					using (TelemetryStopwatch stopwatch = new TelemetryStopwatch("Workspace_SyncArchives", project.TelemetryProjectIdentifier))
					{
						...
						if (project.LocalFileName.HasExtension(".uproject") || project.LocalFileName.HasExtension(".uefnproject"))
						{
			...
			if (Context.Options.HasFlag(WorkspaceUpdateOptions.Build))
			{
				...
				BuildStep.MergeBuildStepObjects(buildStepObjects, Context.ProjectConfigFile.GetValues("Build.Step", [], project.ProjectIdentifier).Select(x => new ConfigObject(x)));
		...
		{
			string? parsedLine = ProgressTextReader.ParseLine(line, progress);
			if (parsedLine != null)
			{
		...
		public static List<string> GetSyncPaths(ProjectInfo project, WorkspaceUpdateOptions options, string[] syncFilter, ISet<string> additionalPathsToSync, ISet<string> baseContentPaths)
		{
			List<string> syncPaths = GetRelativeSyncPaths(project, options, syncFilter, additionalPathsToSync, baseContentPaths);
			return syncPaths.Select(x => project.ClientRootPath + x).ToList();
		}

#pragma warning disable IDE0060 // Remove unused parameter
		public static List<string> GetRelativeSyncPaths(ProjectInfo project, WorkspaceUpdateOptions options, IReadOnlyList<string>? syncFilter, ISet<string> additionalPathsToSync, ISet<string> baseContentPaths)
#pragma warning restore IDE0060 // Remove unused parameter
		{
			...
			else if(project.IsUEFNProject && options.HasFlag(WorkspaceUpdateOptions.SyncBaseContent))
			{
				syncPaths.AddRange(baseContentPaths);
			}
			...
			if (syncFilter != null)
			{
				...
				{
					applySyncPath(syncPaths, syncPath);
				}
			}
		...
		static void applySyncPath(List<string> syncPaths, string syncPath)
		{
		...
		public static async Task<ConfigFile> ReadProjectConfigFile(IPerforceConnection perforce, ProjectInfo projectInfo, ILogger logger)
		{
			// Find the valid config file paths
			DirectoryInfo engineDir = DirectoryReference.Combine(projectInfo.LocalRootPath, "Engine").ToDirectoryInfo();
			List<FileInfo> localConfigFiles = Utility.GetLocalConfigPaths(engineDir, projectInfo.LocalFileName.ToFileInfo());
			...
			foreach (FileInfo localConfigFile in localConfigFiles)
			{
				try
				{
					...
					projectConfig.Parse(lines, projectInfo.ProjectIdentifier);
			...
			await ConfigUtils.ExecuteConfigInjectionScriptsAsync(
				perforce, 
				projectInfo.BranchPath,
				projectInfo.ClientFileName,
				projectInfo.ProjectIdentifier, 
				projectInfo.CacheFolder, 
				projectConfig,
				localConfigFiles.Select(l => new KeyValuePair<FileReference, DateTime>(new FileReference(l.FullName), l.LastWriteTimeUtc)).ToList(),
				logger);
