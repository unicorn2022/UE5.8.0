...
#pragma warning restore CA1027 // Mark enums with FlagsAttribute
	...
	public class UserWorkspaceSettings
	{
		...
		public bool SyncBaseContent { get; set; } = false;
		...
		[JsonIgnore]
		public FileReference? LocalProjectPath { get; set; } = null;

		public void Init(string? serverAndPort, string? userName, string clientName, string branchPath, string projectPath, string localPath)
		{
			...
			LocalProjectPath = new FileReference(localPath);
		}
		...
		{
			return Save(logger, ConfigFile);
		}

		public bool Save(ILogger logger, FileReference workspaceSettingsFile)
		{
			try
			{
				SaveInternal(workspaceSettingsFile);
				return true;
			}
			catch (Exception ex)
			{
		...
		private void SaveInternal(FileReference fileReference)
		{
			lock (_syncRoot)
			{
				UserSettings.CreateConfigDir(fileReference.Directory);
				Utility.SaveJson(fileReference, this);
				LastModifiedTimeUtc = FileReference.GetLastWriteTimeUtc(fileReference).Ticks;
			}
		}
	...
	public class UserProjectSettings
	{
		...
		[JsonPropertyName("FilterBadges")]
		public HashSet<string> FilterBadges { get; init; } = new HashSet<string>(StringComparer.OrdinalIgnoreCase);

		[JsonPropertyName("Preset")]
		public string Preset { get; set; } = String.Empty;

		[JsonPropertyName("PresetSetByUser")]
		public bool PresetSetByUser { get; set; } = false;

		[JsonPropertyName("Archives")]
		public List<ArchiveSettings> Archives { get; init; } = new List<ArchiveSettings>();

		[JsonPropertyName("ArchivesDeactivated")]
		public bool ArchivesDeactivated { get; set; } = false;
		...
		private static string s_currentVersion = String.Empty;
		public static string CurrentVersion()
		{
			if (!String.IsNullOrWhiteSpace(s_currentVersion))
			{
				return s_currentVersion;
			}

			Queue<PropertyInfo> properties = new Queue<PropertyInfo>(typeof(UserProjectSettings).GetProperties());
			StringBuilder sb = new StringBuilder();
			while (properties.TryDequeue(out PropertyInfo? prop))
			{
				JsonPropertyNameAttribute? attribute = prop.GetCustomAttribute<JsonPropertyNameAttribute>();
				if (attribute != null)
				{
			...
			s_currentVersion = Convert.ToHexString(hashBytes);
		...
		public bool Save(ILogger logger, FileReference projectSettingsFile)
		{
			try
			{
				SaveInternal(projectSettingsFile);
				return true;
			}
			catch (Exception ex)
			{
		...
		private void SaveInternal(FileReference fileReference)
		{
			lock (s_syncRoot)
			{
				...
				// save the file with the current version
				Utility.SaveJson(fileReference, this);
			}
		}
	}
	...
	public class UserSettings : GlobalSettingsFile
	{
		...
		public bool Save(ILogger logger, FileReference globalSettingsJsonFile, FileReference globalSettingsIniFile)
		{
			if (!base.Save(logger, globalSettingsJsonFile))
			{
				return false;
			}

			PrepareConfigFileforSave();

			// Save the file
			try
			{
				_configFile.Save(globalSettingsIniFile);
				return true;
			}
			catch (Exception ex)
			{
				logger.LogError(ex, "Unable to save config file {FileName}: {Message}", globalSettingsIniFile, ex.Message);
				return false;
			}
		}
		...
		private void PrepareConfigFileforSave()
		{
			// General settings
