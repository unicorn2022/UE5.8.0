...
#pragma warning restore CA2213
		...
		IReadOnlyDictionary<string, ToolInfo> _perforceTools = new Dictionary<string, ToolInfo>(StringComparer.Ordinal);
		...
		async Task ReadPerforceToolsAsync(IPerforceConnection perforce, List<ToolInfo> tools, CancellationToken cancellationToken)
		{
			...
			{
				ConcurrentDictionary<string, ToolInfo> newPerforceTools = new ConcurrentDictionary<string, ToolInfo>(StringComparer.Ordinal);
				...
				await Parallel.ForEachAsync(fileRecords, cancellationToken, async (fileRecord, ct) =>
				{
					if (fileRecord.DepotFile != null && fileRecord.DepotFile.EndsWith(".ini", StringComparison.OrdinalIgnoreCase))
					{
						...
						if (zipRecord != null)
						{
							...
							{
								toolInfo = await ReadToolDefinitionAsync(perforce, $"{fileRecord.DepotFile}@{fileRecord.Change}", toolRevision, ct);
							}
							if (toolInfo != null)
							{
								newPerforceTools.TryAdd(toolRevision, toolInfo);
							}
						}
					}
				});
