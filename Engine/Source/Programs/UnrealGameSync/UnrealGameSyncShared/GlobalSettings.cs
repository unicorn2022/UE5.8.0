...
namespace UnrealGameSync
{
	...
	public class GlobalSettingsFile
	{
		...
		public bool Save(ILogger logger, FileReference globalSettingsJsonFile)
		{
			try
			{
				Utility.SaveJson(globalSettingsJsonFile, Global);
				return true;
			}
			catch (Exception ex)
			{
				logger.LogError(ex, "Unable to save {File}: {Message}", globalSettingsJsonFile, ex.Message);
				return false;
			}
		}
		...
		public UserWorkspaceSettings FindOrAddWorkspaceSettings(DirectoryReference rootDir, string? serverAndPort, string? userName, string clientName, string branchPath, string projectPath, string localPath, ILogger logger)
		{
			...
			settings.Init(serverAndPort, userName, clientName, branchPath, projectPath, localPath);
			...
			ConfigSection? perforceSection)
		{
			...
			if (perforceSection != null)
			{
				...
				lines.AddRange(additionalPaths);
			}
