...
namespace UnrealGameSyncCmd.Utils
{
	...
	{
		internal static string[] ReadSyncFilter(
			UserWorkspaceSettings workspaceSettings, 
			UserProjectSettings contextUserProjectSettings,
			...
			string projectIdentifier)
		{
			...
			if (roles.TryGetValue(contextUserProjectSettings.Preset, out Preset? role))
			{
			...
			string[] combinedSyncFilter = GlobalSettingsFile.GetCombinedSyncFilter(syncCategories, contextUserProjectSettings.Preset, roles, userSettings.Global.Filter, workspaceSettings.Filter, perforceSection);
