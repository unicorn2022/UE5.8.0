...
namespace UnrealGameSyncCmd.Commands
{
	...
	internal class SwitchCommand : Command
	{
		public override async Task ExecuteAsync(CommandContext context)
		{
			...
			if (targetName.StartsWith("//", StringComparison.Ordinal))
			{
				await SwitchStreamAsync(context.PerforceClient!, targetName, options.Force, context.Logger!);
			}
			else
			{
				await SwitchProjectAsync(context.PerforceClient!, context.UserWorkspaceSettings!, targetName, context.Logger!);
			}
		}

		private static async Task SwitchStreamAsync(IPerforceConnection perforceClient, string streamName, bool force, ILogger logger)
		{
		...
		private static async Task SwitchProjectAsync(IPerforceConnection perforceClient, UserWorkspaceSettings settings, string projectName, ILogger logger)
		{
