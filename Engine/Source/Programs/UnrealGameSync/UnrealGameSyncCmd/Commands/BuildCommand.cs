...
namespace UnrealGameSyncCmd.Commands
{
	...
	internal class BuildCommand : Command
	{
		public override async Task ExecuteAsync(CommandContext context)
		{
			...
			Dictionary<Guid, ConfigObject> buildStepObjects = ConfigUtils.GetDefaultBuildStepObjects(
				projectInfo, 
				editorTarget.GetFileNameWithoutAnyExtensions(), 
				EditorConfig, 
				projectConfig, 
				false);
			...
			{
				BuildStep.MergeBuildStepObjects(buildStepObjects, projectConfig.GetValues("Build.Step", [], projectInfo.ProjectIdentifier).Select(x => new ConfigObject(x)));
			...
			WorkspaceUpdateContext updateContext = new WorkspaceUpdateContext(
				...
				context.UserWorkspaceSettings,
				context.UserProjectSettings,
			...
			(WorkspaceUpdateResult result, string message) = await update.ExecuteAsync(perforceClient.Settings, projectInfo, state, logger, CancellationToken.None);
