	...
	public static class Program
	{
		...
		public static async Task<int> Main(string[] rawArgs)
		{
			...
			try
			{
				...
				ICommand instance = (ICommand)Activator.CreateInstance(command.Type)!;

				switch (instance)
				{
					//////////////////////////////////////
					case ClientsCommand _:
						{
							await ClientsCommand.ExecuteAsync(logger, args, loggerFactory);
							break;
						}
					...
					case InitCommand _:
						{
							await InitCommand.ExecuteAsync(logger, args, loggerFactory, globalSettings);
							break;
						}

					//////////////////////////////////////
					case SettingsCommand _:
						{
							await SettingsCommand.ExecuteAsync(logger, loggerFactory, args, globalSettings, settings);
							break;
						}
					...
					case ToolsCommand _:
						{
							await ToolsCommand.ExecuteAsync(logger, args, globalSettings);
							break;
						}

					//////////////////////////////////////
					case UpgradeCommand _:
						{
							await UpgradeCommand.ExecuteAsync(logger, args);
							break;
						}
					...
					case Command contextCommand:
						{
							using CommandContext commandContext = await CommandContext.CreateAsync(args, serviceProvider,logger, loggerFactory, settings, globalSettings, hordeClient, cloudStorage);

							await contextCommand.ExecuteAsync(commandContext!);
							break;
						}
				}
		...
		private static bool IsUpgradeAvailable(GlobalSettingsFile settings, string version)
		{
		...
		private static string GetUsage(CommandInfo commandInfo)
		{
