	...
	internal static class CommandsFactory
	{
		...
		private static List<CommandInfo> PortableCommands()
		{
			return new List<CommandInfo>
			{
				...
				new CommandInfo("run", typeof(RunCommand), typeof(RunCommandOptions),
				"ugs run",
		...
		private static List<CommandInfo> GetWinCommand()
		{
			return new List<CommandInfo>()
			{
				...
				new CommandInfo("archive", typeof(ArchiveCommand), typeof(ArchiveCommandOptions),
				"ugs archive",
				"Execute a command related to archives."
				),
				new CommandInfo("archive-override", typeof(ArchiveOverrideCommand), typeof(ArchiveOverrideCommandOptions),
					"ugs archive-override",
					"Execute a command related to archives overrides per project."
				),
			};
		}
	}
