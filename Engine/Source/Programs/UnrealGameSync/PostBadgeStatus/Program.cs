...
namespace WriteBadgeStatus
{
	class Program
	{
		static int Main(string[] Args)
		{
			...
			{
				Build.Metadata = JsonSerializer.Deserialize<JsonElement>(Metadata);
			}
			int NumRetries = 0;
			while (true)
			{
				try
				{
					return SendRequest(RestUrl, "Build", "POST", JsonSerializer.Serialize(Build));
				}
				catch (Exception ex)
				{
					if (++NumRetries <= 3)
		...
		static int SendRequest(string URI, string Resource, string Method, string? RequestBody = null, params string[] QueryParams)
		{
			...
			HttpWebRequest Request = (HttpWebRequest)WebRequest.Create(new Uri(TargetURI.ToString()));
			...
			catch (WebException ex)
			{
				...
				{
					throw new Exception(String.Format("Request returned message: {0}", ex.InnerException?.Message ?? ex.Message));
				}
			}
			catch (Exception ex)
			{
