...
{
	const TCHAR* EngineExitInvalidArgsReason = TEXT("Invalid arguments");
...
void FUserInterfaceCommand::InitializeSlateApplication(bool bFrontendMode, const FString& TraceFileToOpen)
{
	...
	if (!bFrontendMode) // viewer mode
	{
		...
		uint16 SecurePort = 0;
		const bool bConnectToSecureTrace = FParse::Value(FCommandLine::Get(), TEXT("-TraceSecurePort="), SecurePort);
		...
		if (bConnectToSecureTrace)
		{
			FString SecureAuthBase64, SecureCertBase64, SecureHost(TEXT("localhost"));
			FString SecureCertPath, SecureIdentity;
			TArray<uint8> SecureAuthBin, SecureCertBin;

			FParse::Value(FCommandLine::Get(), TEXT("-TraceSecureHost="), SecureHost);
			FParse::Value(FCommandLine::Get(), TEXT("-TraceSecureAuth="), SecureAuthBase64);
			FParse::Value(FCommandLine::Get(), TEXT("-TraceSecureCertPath="), SecureCertPath);
			FParse::Value(FCommandLine::Get(), TEXT("-TraceSecureCert="), SecureCertBase64);
			FParse::Value(FCommandLine::Get(), TEXT("-TraceSecureIdentity="), SecureIdentity);

			FAnsiString SecureIdentityAnsi(SecureIdentity);

			if (!SecureAuthBase64.IsEmpty() && !FBase64::Decode(SecureAuthBase64, SecureAuthBin))
			{
				UE_LOGF(LogInit, Error, "Failed to decode secure authorization token.");
				RequestEngineExit(UserInterfaceCommand::EngineExitInvalidArgsReason);
				return;
			}

			if (!SecureCertBase64.IsEmpty() && !FBase64::Decode(SecureCertBase64, SecureCertBin))
			{
				UE_LOGF(LogInit, Error, "Failed to decode secure certificate.");
				RequestEngineExit(UserInterfaceCommand::EngineExitInvalidArgsReason);
				return;
			}
			
			TraceInsightsModule.StartAnalysisWithSecureTrace(
				*SecureHost, 
				SecurePort, 
				SecureAuthBin, 
				*SecureCertPath,
	...
	UE_LOGF(LogInit, Display, "Insights slate application initialized successfully.");
}
