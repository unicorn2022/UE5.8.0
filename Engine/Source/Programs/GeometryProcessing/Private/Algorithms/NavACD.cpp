...
DefineAlgorithm(NavACD)
{
	...
	if (HasTag(TEXT("help")))
	{
		UE_LOGF(LogGeometryProcessing, Display, "Navigation-Driven Approximate Convex Decomposition arguments:");
		UE_LOGF(LogGeometryProcessing, Display, "-input: Path to input mesh");
		UE_LOGF(LogGeometryProcessing, Display, "-output: Path to output mesh; convex hulls will be assigned separate groups and colors");
		UE_LOGF(LogGeometryProcessing, Display, "-stats: If this tag is present, will output stats on a successful run (timings, hull counts)");
		UE_LOGF(LogGeometryProcessing, Display, "-protect_unreachable: If this tag is present, will protect unreachable space where the min radius sphere could fit.");
		UE_LOGF(LogGeometryProcessing, Display, "-r: Navigable space min radius parameter, as a fraction of the longest bounding box axis");
		UE_LOGF(LogGeometryProcessing, Display, "-t: Navigable space tolerance parameter, as a fraction of the longest bounding box axis");
		return true;
	}
	...
	if (HasTag("stats"))
	{
		UE_LOGF(LogGeometryProcessing, Display, "Algorithm time (excluding file read/write): %f seconds", EndTime - StartTime);
		UE_LOGF(LogGeometryProcessing, Display, "Number of hulls used: %d", ConvexDecomposition.NumHulls());
	}
	...
	auto WriteDecomp = [](FConvexDecomposition3& Decomp, const FString& Path)
	{
		...
		if (!FileStream)
		{
			UE_LOGF(LogGeometryProcessing, Error, "Failed to open output path: %ls", *Path);
			Fail();
		}
