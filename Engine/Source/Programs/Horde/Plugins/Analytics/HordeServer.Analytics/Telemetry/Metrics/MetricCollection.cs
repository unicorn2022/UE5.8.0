	...
	class MetricCollection : IMetricCollection, IHostedService, IAsyncDisposable
	{
		...
		async Task BackgroundProcessAsync(CancellationToken cancellationToken)
		{
			await foreach ((TelemetryStoreId storeId, JsonNode node) in _events.Reader.ReadAllAsync(cancellationToken))
			{
				...
				{
					// This is currently a narrow exception capture for a common event where malformed payloads are passed the background processor.
					// We will emit this as a logDebug along with the exception while we fix call sites that are malformed.
					_logger.LogDebug(ex, "Skipping telemetry emission for event store {StoreId}; processing failed due to malformed payload.", storeId);
				}
				catch(Exception ex)
				{
					// All other exceptions should be logged as errors.
					_logger.LogError(ex, "Failed to process telemetry event for store {StoreId}", storeId);
				}
			}
		}
