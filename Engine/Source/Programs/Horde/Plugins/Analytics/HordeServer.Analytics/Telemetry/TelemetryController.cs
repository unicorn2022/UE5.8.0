...
namespace HordeServer.Telemetry
{
	...
	public class TelemetryController : HordeControllerBase
	{
		...
		/// <param name="sessionId">Identifier used to group telemetry within a single session.</param>
		[HttpPost]
		[Route("/api/v1/telemetry")]
		public ActionResult PostEvent([FromBody] PostTelemetryEventStreamRequest request, [FromQuery][Required] string appId, [FromQuery][Required] string appVersion, [FromQuery][Required] string appEnvironment, [FromQuery][Required] TelemetryUploadType uploadType, [FromQuery] string? sessionId = null)
		{
			return PostEvent(TelemetryStoreId.Default, request, appId, appVersion, appEnvironment, uploadType, sessionId);
		}
		...
		public ActionResult PostEvent(TelemetryStoreId storeId, [FromBody] PostTelemetryEventStreamRequest request, [FromQuery][Required] string appId, [FromQuery][Required] string appVersion, [FromQuery][Required] string appEnvironment, [FromQuery][Required] TelemetryUploadType uploadType, [FromQuery] string? sessionId = null)
		{
			...
			TelemetryRecordMeta recordMeta = new TelemetryRecordMeta(AppId: appId, AppVersion: appVersion, AppEnvironment: appEnvironment, SessionId: sessionId);
			foreach (JsonElement eventPayload in request.Events)
			{
