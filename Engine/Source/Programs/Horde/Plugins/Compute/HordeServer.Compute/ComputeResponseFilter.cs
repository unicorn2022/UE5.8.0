	...
	class ComputeResponseFilter : IPluginResponseFilter
	{
		...
		public void Apply(HttpContext context, object response)
		{
			if (response is GetDashboardFeaturesResponse featuresResponse)
			{
				...
				bool hasDbPools = computeConfig.VersionEnum < ConfigVersion.PoolsInConfigFiles;
				featuresResponse.ShowPoolEditor = hasDbPools
					&& (computeConfig.Authorize(PoolAclAction.CreatePool, context.User)
						|| computeConfig.Authorize(PoolAclAction.UpdatePool, context.User));
