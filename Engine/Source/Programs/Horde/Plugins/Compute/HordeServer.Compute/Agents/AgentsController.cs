...
namespace HordeServer.Agents
{
	...
	public class AgentsController : HordeControllerBase
	{
		...
		{
			string[] outcomeReasons = ToFlagNames(lease.OutcomeReason);
			return new GetAgentLeaseResponse(
				lease.Id, lease.ParentId, lease.AgentId, agentRate, lease.Name, lease.LogId, lease.StartTime,
				lease.FinishTime, lease.FinishTime == null, details, lease.Outcome, outcomeReasons, (long)lease.OutcomeReason, null);
		}
		
		private static string[] ToFlagNames(LeaseOutcomeReason reason)
		{
		...
		public async Task<ActionResult<GetAgentTelemetryResponse>> FindTelemetryAsync(AgentId agentId, [FromQuery] DateTime minTime, [FromQuery] DateTime maxTime)
		{
			...
			samples.AddRange(telemetry.Select(x => new GetAgentTelemetrySampleResponse(x.TimeUtc, x.UserCpu, x.IdleCpu, x.SystemCpu, x.FreeRam, x.UsedRam, x.TotalRam, x.WorkingFreeDisk, x.WorkingTotalDisk, x.SystemFreeDisk, x.SystemTotalDisk)));
