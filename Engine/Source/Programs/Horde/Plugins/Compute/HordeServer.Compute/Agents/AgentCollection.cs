...
namespace HordeServer.Agents
{
	...
	public sealed class AgentCollection : IAgentCollection, IHostedService
	{
		class Agent : IAgent
		{
			...
			/// <inheritdoc/>
			public async Task<IAgent?> TryCancelLeaseAsync(int leaseIdx, LeaseOutcomeReason? reason = null, CancellationToken cancellationToken = default)
				=> await _collection.TryCancelLeaseAsync(this, leaseIdx, reason, cancellationToken);
			...
			await foreach (RpcSession session in _scheduler.FindExpiredSessionsAsync(cancellationToken))
			{
				...
				if (document != null)
				{
					...
					if (agent != null && agent.Session != null && agent.Session.UpdateTicks == session.UpdateTicks)
					{
						...
						await TryUpdateSessionAsync(agent, new UpdateSessionOptions { Status = AgentStatus.Stopped, DefaultLeaseOutcomeReason = LeaseOutcomeReason.ServerInitiated | LeaseOutcomeReason.AgentSessionTerminated }, cancellationToken);
						c++;
					}
				}
			}
...
#pragma warning restore CS0618
		...
		async Task<List<IAgent>> CreateAgentObjectsBatchedAsync(List<AgentDocument> documents, CancellationToken cancellationToken)
		{
			// Collect all session IDs for a single batched Redis MGET
			List<SessionId> sessionIds = new (documents.Count);
			foreach (AgentDocument document in documents)
			{
				if (document.SessionId != null)
				{
					sessionIds.Add(document.SessionId.Value);
				}
			}

			// Batch fetch all sessions in one round-trip
			Dictionary<SessionId, RpcSession> sessions = await _scheduler.TryGetSessionsAsync(sessionIds, cancellationToken);

			// Create agent objects using pre-fetched sessions
			List<IAgent> agents = new (documents.Count);
			foreach (AgentDocument document in documents)
			{
				Agent? agent;
				if (document.SessionId == null)
				{
					agent = new Agent(this, document, null);
				}
				else if (sessions.TryGetValue(document.SessionId.Value, out RpcSession? session))
				{
					// Safe to bypass the reconciliation loop in CreateAgentObjectAsync; the dictionary is keyed by
					// document.SessionId so session.SessionId == document.SessionId is guaranteed.
					agent = new Agent(this, document, session);
				}
				else
				{
					// Session missing from Redis — fall back to sequential handling which clears the stale session
					agent = await CreateAgentObjectAsync(document, cancellationToken);
				}
		...
		async ValueTask<Agent?> CreateAgentObjectAsync(AgentDocument? document, RpcSession? session, CancellationToken cancellationToken)
		{
			...
			for (; ; )
			{
				...
				for (; ; )
				{
					// Fetch the document again if update failed below.
					// On first iteration, document is already set.
					if (document == null)
					{
					...
					// If there's no matching session, try to remove it from the document. We can expect the document to be authoritative here, since we always create the session before
		...
		public async Task<IReadOnlyList<IAgent>> GetManyAsync(List<AgentId> agentIds, CancellationToken cancellationToken)
		{
			...
			return await CreateAgentObjectsBatchedAsync(documents, cancellationToken);
		}
		...
		public async IAsyncEnumerable<IAgent> FindAsync(PoolId? poolId, DateTime? modifiedAfter, string? property, AgentStatus? status, bool? enabled, bool includeDeleted, bool consistentRead, [EnumeratorCancellation] CancellationToken cancellationToken)
		{
			...
			// Use a large batch size to minimize getMore commands when reading from secondaries.
			// The agents collection is updated frequently, making multi-batch cursor reads error-prone.
			FindOptions findOptions = new() { BatchSize = 10000 };
			List<AgentDocument> allDocuments = await collection.Find(filter, findOptions).ToListAsync(cancellationToken);
			allDocuments = await PostLoadAsync(allDocuments, cancellationToken);
			foreach (IAgent agent in await CreateAgentObjectsBatchedAsync(allDocuments, cancellationToken))
			{
		...
		async Task<Agent?> TryUpdateSessionAsync(Agent agent, UpdateSessionOptions options, CancellationToken cancellationToken)
		{
			...
			AgentDocument? newDocument = agent.Document;
			{
				...
				List<string> newProperties = newDocument.Properties ?? [];
				Dictionary<string, int>? newResources = null;
				if (options.Capabilities != null)
				{
					newProperties = [.. options.Capabilities.Properties];
					newResources = options.Capabilities.Resources.ToDictionary();
				}

				if (options.CapabilityOverrides != null)
				{
					if (options.CapabilityOverrides.Properties.Count > 0)
					{
						newProperties = MergeOverrideProperties(agentId, newProperties, options.CapabilityOverrides.Properties);
					}
					if (options.CapabilityOverrides.Resources.Count > 0)
					{
						newResources = MergeOverrideResources(agentId, newResources ?? newDocument.Resources, options.CapabilityOverrides.Resources);
					}
				}
				...
				if (newResources != null && !ResourcesEqual(newResources, newDocument.Resources))
				{
			...
			{
				// Merges outcome reasons if lease was already finished by another process (race condition)
				async Task TryMergeOutcomeReasonsAsync(bool updated, LeaseId leaseId, LeaseOutcomeReason reason)
				{
					if (!updated && reason != LeaseOutcomeReason.None)
					{
						_logger.LogInformation("Merging outcome reasons {Reason} for already-finished lease {LeaseId}", reason, leaseId);
						await _leaseCollection.TryAddOutcomeReasonsAsync(leaseId, reason, CancellationToken.None);
					}
				}
				...
				foreach (RpcSessionLease oldLease in agent.Session.Leases.Where(x => !newLeaseIds.Contains(x.Id)))
				{
					...
					if (rpcLease is { State: RpcLeaseState.Completed })
					{
						LeaseOutcomeReason reason = rpcLease.ToNativeOutcomeReason();
						_logger.LogInformation("Setting outcome {Outcome} for completed lease {LeaseId}", oldLease.Id, rpcLease.Outcome);
						bool updated = await _leaseCollection.TrySetOutcomeAsync(oldLease.Id, _clock.UtcNow, (LeaseOutcome)rpcLease.Outcome, reason, rpcLease.Output.ToArray(), ToNativeLeaseMetrics(rpcLease.Metrics), CancellationToken.None);
						await TryMergeOutcomeReasonsAsync(updated, oldLease.Id, reason);
					}
					else
					{
						LeaseOutcomeReason reason = rpcLease?.ToNativeOutcomeReason() ?? options.DefaultLeaseOutcomeReason ?? LeaseOutcomeReason.NoReasonReported;
						_logger.LogInformation("Setting outcome {Outcome} with reason {Reason} for lease {LeaseId} ({State})", rpcLease?.Outcome, reason, oldLease.Id, rpcLease?.State);
						bool updated = await _leaseCollection.TrySetOutcomeAsync(oldLease.Id, _clock.UtcNow, LeaseOutcome.Cancelled, reason, null, ToNativeLeaseMetrics(rpcLease?.Metrics), CancellationToken.None);
						await TryMergeOutcomeReasonsAsync(updated, oldLease.Id, reason);
					}
				}
			}
		...
		[SuppressMessage("Maintainability", "CA1508:Avoid dead conditional code", Justification = "Protobuf generated code may contain null fields")]
		static LeaseMetrics? ToNativeLeaseMetrics(RpcLeaseMetrics? metrics)
		{
			if (metrics == null)
			{
				return null;
			...
			LeaseCpuTime cpuTime = metrics.GlobalCpuTime != null
				? new LeaseCpuTime(
					TimeSpan.FromMilliseconds(metrics.GlobalCpuTime.UserMs),
					TimeSpan.FromMilliseconds(metrics.GlobalCpuTime.SystemMs),
					TimeSpan.FromMilliseconds(metrics.GlobalCpuTime.IdleMs))
				: new LeaseCpuTime(TimeSpan.Zero,  TimeSpan.Zero, TimeSpan.Zero);

			return new LeaseMetrics(
				TimeSpan.FromMilliseconds(metrics.SetupTimeMs),
				TimeSpan.FromMilliseconds(metrics.TeardownTimeMs),
				metrics.CpuCount,
				metrics.GlobalCpuUtilization,
				cpuTime);
		}
		...
		/// <summary>
		/// Merges override properties into a property list. For each key present in both the overrides
		/// and the existing properties, existing entries with that key are replaced by the override entries.
		/// Override entries with keys not present in the existing properties are ignored.
		/// </summary>
		List<string> MergeOverrideProperties(AgentId agentId, IEnumerable<string> properties, IReadOnlyList<string> overrides)
		{
			List<string> newProperties = [.. properties];

			HashSet<string> existingKeys = new(StringComparer.OrdinalIgnoreCase);
			foreach (string prop in newProperties)
			{
				existingKeys.Add(GetPropertyKey(prop));
			}

			HashSet<string> overrideKeys = new(StringComparer.OrdinalIgnoreCase);
			foreach (string prop in overrides)
			{
				...
				{
					_logger.LogWarning("Agent {AgentId}: capability override property {Key} ignored - key not present in existing properties", agentId, key);
				}
			}

			newProperties.RemoveAll(x => overrideKeys.Contains(GetPropertyKey(x)));
			newProperties.AddRange(overrides.Where(x => overrideKeys.Contains(GetPropertyKey(x))));
			newProperties.Sort(StringComparer.OrdinalIgnoreCase);
			return newProperties;
		}

		/// <summary>
		/// Merges override resources into an existing resource dictionary, replacing values for existing keys.
		/// Override entries with keys not present in the existing resources are ignored.
		/// </summary>
		Dictionary<string, int>? MergeOverrideResources(AgentId agentId, IReadOnlyDictionary<string, int>? resources, IReadOnlyDictionary<string, int> overrides)
		{
			if (resources == null)
			{
				return null;
			}
			Dictionary<string, int> newResources = new(resources, StringComparer.OrdinalIgnoreCase);
			foreach ((string key, int value) in overrides)
			{
				...
				{
					_logger.LogWarning("Agent {AgentId}: capability override resource {Key} ignored - key not present in existing resources", agentId, key);
				}
			}
			return newResources;
		}
		...
		/// <summary>
		/// Extracts the key portion from a "KEY=VALUE" property string.
		/// Returns the full string if no '=' is found.
		/// </summary>
		static string GetPropertyKey(string property)
		{
			int eqIdx = property.IndexOf('=', StringComparison.Ordinal);
			return eqIdx >= 0 ? property[..eqIdx] : property;
		}
		...
		async Task<Agent?> TryTerminateSessionAsync(Agent agent, CancellationToken cancellationToken)
		{
			...
			return await TryUpdateSessionAsync(agent, new UpdateSessionOptions(AgentStatus.Stopped, DefaultLeaseOutcomeReason: LeaseOutcomeReason.ServerInitiated | LeaseOutcomeReason.AgentSessionTerminated), cancellationToken);
		}
		...
		/// <inheritdoc/>
		async Task<Agent?> TryCancelLeaseAsync(Agent agent, int leaseIdx, LeaseOutcomeReason? reason, CancellationToken cancellationToken)
		{
			if (agent.Session == null)
			{
				return null;
			...
			RpcSession? newSession = new RpcSession(agent.Session);
			LeaseId leaseId = newSession.Leases[leaseIdx].Id;

			if (newSession.Leases[leaseIdx].State == RpcLeaseState.Pending)
			{
				newSession.Leases.RemoveAt(leaseIdx);

				// Record the outcome immediately for pending leases since they'll be removed from the session
				if (reason.HasValue)
				{
					await _leaseCollection.TrySetOutcomeAsync(leaseId, _clock.UtcNow, LeaseOutcome.Cancelled, reason.Value, null, null, cancellationToken);
				}
			}
			else
			{
				...
				// Pre-set the outcome for active leases - this ensures the reason is captured
				// When the agent reports completion, TryMergeOutcomeReasonsAsync will merge additional reasons
				if (reason.HasValue)
				{
					bool updated = await _leaseCollection.TrySetOutcomeAsync(leaseId, _clock.UtcNow, LeaseOutcome.Cancelled, reason.Value, null, null, cancellationToken);
					if (!updated)
					{
						// Outcome was already set by another path, add our reasons to it
						await _leaseCollection.TryAddOutcomeReasonsAsync(leaseId, reason.Value, cancellationToken);
					}
				}
			}
