...
namespace HordeServer.Agents.Leases
{
	...
	{
		record LeaseMetricsDocument(
			[property: BsonElement("setup")] int SetupTimeMs,
			[property: BsonElement("teardown")] int TeardownTimeMs,
			[property: BsonElement("cpus")] int CpuCount,
			[property: BsonElement("cpuUtil")] double GlobalCpuUtilization,
			[property: BsonElement("cpuUsr")] long GlobalCpuUserMs,
			[property: BsonElement("cpuSys")] long GlobalCpuSystemMs,
			[property: BsonElement("cpuIdl")] long GlobalCpuIdleMs
		)
		{
			public LeaseMetrics ToLeaseMetrics()
			{
				return new LeaseMetrics(
					TimeSpan.FromMilliseconds(SetupTimeMs),
					TimeSpan.FromMilliseconds(TeardownTimeMs),
					CpuCount,
					GlobalCpuUtilization,
					new LeaseCpuTime(TimeSpan.FromMilliseconds(GlobalCpuUserMs), TimeSpan.FromMilliseconds(GlobalCpuSystemMs), TimeSpan.FromMilliseconds(GlobalCpuIdleMs))
				);
			}

			public static LeaseMetricsDocument FromLeaseMetrics(LeaseMetrics metrics)
			{
				return new LeaseMetricsDocument(
					(int)metrics.SetupTime.TotalMilliseconds,
					(int)metrics.TeardownTime.TotalMilliseconds,
					metrics.CpuCount,
					metrics.GlobalCpuUtilization,
					(long)metrics.GlobalCpuTime.User.TotalMilliseconds,
					(long)metrics.GlobalCpuTime.System.TotalMilliseconds,
					(long)metrics.GlobalCpuTime.Idle.TotalMilliseconds
				);
			}
		}
		...
		class LeaseDocument : ILease
		{
			...
			public LeaseMetricsDocument? Metrics { get; set; }
			
			[BsonRepresentation(BsonType.Int64)]
			public LeaseOutcomeReason OutcomeReason { get; set; } = LeaseOutcomeReason.None;
			...
			LeaseMetrics? ILease.Metrics => Metrics?.ToLeaseMetrics();
		...
		/// <inheritdoc/>
		public async Task<bool> TrySetOutcomeAsync(LeaseId leaseId, DateTime finishTime, LeaseOutcome outcome, LeaseOutcomeReason outcomeReason, byte[]? output, LeaseMetrics? metrics, CancellationToken cancellationToken)
		{
			FilterDefinitionBuilder<LeaseDocument> filterBuilder = Builders<LeaseDocument>.Filter;
			...
			UpdateDefinition<LeaseDocument> update = updateBuilder
				.Set(x => x.FinishTime, finishTime)
				.Set(x => x.Outcome, outcome)
				.Set(x => x.OutcomeReason, outcomeReason);
			...
			if (metrics != null)
			{
				update = update.Set(x => x.Metrics, LeaseMetricsDocument.FromLeaseMetrics(metrics));
			}
		...
		/// <inheritdoc/>
		public async Task<bool> TryAddOutcomeReasonsAsync(LeaseId leaseId, LeaseOutcomeReason outcomeReason, CancellationToken cancellationToken)
		{
			if (outcomeReason == LeaseOutcomeReason.None)
			{
				return true; // Nothing to add
			}

			FilterDefinition<LeaseDocument> filter = Builders<LeaseDocument>.Filter.Eq(x => x.Id, leaseId);
			UpdateDefinition<LeaseDocument> update = Builders<LeaseDocument>.Update.BitwiseOr(x => x.OutcomeReason, outcomeReason);
