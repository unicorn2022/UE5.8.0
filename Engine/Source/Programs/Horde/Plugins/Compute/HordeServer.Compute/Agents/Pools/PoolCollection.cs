...
#pragma warning restore CS0618 // Type or member is obsolete
		...
		public async Task<IReadOnlyList<IPoolConfig>> GetConfigsAsync(CancellationToken cancellationToken)
		{
			...
			{
				// V0/V1: include all DB pools (no dedup — legacy behavior preserved)
		...
		public async Task<IPool?> GetAsync(PoolId id, CancellationToken cancellationToken)
		{
			...
			{
				// V0/V1: fall back to database if not found in config
			...
			if (state == null)
			{
				PoolDocumentV2 newState = new PoolDocumentV2 { Id = id };

				// Try to copy scaling state from V1 document.
				// Use `as` cast so poolV1 is declared in outer scope for both paths.
				PoolDocumentV1? poolV1 = poolConfig as PoolDocumentV1;
				if (poolV1 != null)
				{
					newState.LastScaleUpTime = poolV1.LastScaleUpTime;
					newState.LastScaleDownTime = poolV1.LastScaleDownTime;
					newState.LastScaleResult = poolV1.LastScaleResult;
					newState.LastAgentCount = poolV1.LastAgentCount;
					newState.LastDesiredAgentCount = poolV1.LastDesiredAgentCount;
				}

				if (!await _poolsV2.InsertOneIgnoreDuplicatesAsync(newState, cancellationToken))
				{
					// Another instance created the V2 doc first; read the actual persisted state
					// to ensure UpdateIndex matches for optimistic concurrency in TryUpdateAsync.
					state = await _poolsV2.Find(x => x.Id == id).FirstOrDefaultAsync(cancellationToken) ?? newState;
				}
				else
				{
					state = newState;
		...
		public async Task<List<PoolId>> GetPoolIdsAsync(CancellationToken cancellationToken)
		{
			...
			if (computeConfig.VersionEnum < ConfigVersion.PoolsInConfigFiles)
			{
				// V0/V1: include all DB pool IDs (no dedup — legacy behavior preserved)
