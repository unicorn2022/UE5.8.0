...
namespace HordeServer.Agents.Leases
{
	...
	public interface ILeaseCollection
	{
		...
		/// <param name="outcomeReason">Reason for the outcome</param>
		/// <param name="output">Output data from the task</param>
		/// <param name="metrics">Metrics</param>
		/// <param name="cancellationToken">Cancellation token for the operation</param>
		/// <returns>True if the lease was updated, false otherwise</returns>
		Task<bool> TrySetOutcomeAsync(LeaseId leaseId, DateTime finishTime, LeaseOutcome outcome, LeaseOutcomeReason outcomeReason, byte[]? output, LeaseMetrics? metrics, CancellationToken cancellationToken = default);

		/// <summary>
		/// Adds outcome reasons to an existing lease. This can be used to merge reasons even after the lease has finished.
		/// </summary>
		/// <param name="leaseId">The lease to update</param>
		/// <param name="outcomeReason">Additional reasons to add (will be ORed with existing reasons)</param>
		/// <param name="cancellationToken">Cancellation token for the operation</param>
		/// <returns>True if the lease was updated, false if lease doesn't exist</returns>
		Task<bool> TryAddOutcomeReasonsAsync(LeaseId leaseId, LeaseOutcomeReason outcomeReason, CancellationToken cancellationToken = default);
	}
}
