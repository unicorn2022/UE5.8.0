	...
	public sealed class ComputeService : IHostedService, IAsyncDisposable
	{
		...
		public async Task<ComputeResource> TryAllocateResourceAsync(AllocateResourceParams arp, CancellationToken cancellationToken)
		{
			...
			if (arp.RequesterIp != null)
			{
				...
				// Normalize IPv6-mapped IPv4 addresses (e.g. ::ffff:192.168.1.1) to plain IPv4
				// to ensure consistent string comparison with agent ComputeIp values
				IPAddress requesterIp = arp.RequesterIp.IsIPv4MappedToIPv6
					? arp.RequesterIp.MapToIPv4()
					: arp.RequesterIp;
				conditions.Add(Condition.Parse($"{KnownPropertyNames.ComputeIp} != '{requesterIp}'"));
			}
			...
			{
				_logger.LogError(are, "Agent relay error while allocating compute resource for cluster {ClusterId}: {Message}", arp.ClusterId, are.Message);
				span.SetStatus(Status.Error);
				span.RecordException(are);
				throw new ComputeServiceException(are.Message, are) { ShowToUser = true };
			}
		}
