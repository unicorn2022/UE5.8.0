...
namespace HordeServer.Logs
{
	...
	public class LogsController : ControllerBase
	{
		...
		/// Constructor
		/// </summary>
		public LogsController(ILogCollection logCollection, StorageService storageService, IEnumerable<ILogExtAuthProvider> authProviders, IEnumerable<ILogExtIssueProvider> issueProviders, IOptionsSnapshot<ComputeConfig> computeConfig, ILogger<LogsController> logger)
		{
		...
		public async Task<ActionResult<List<GetLogEventResponse>>> GetEventsAsync(LogId logId, [FromQuery] int? index = null, [FromQuery] int? count = null, CancellationToken cancellationToken = default)
		{
			...
			{
				ILogEventData logEventData;
				try
				{
					logEventData = await anchor.GetDataAsync(cancellationToken);
				}
				catch (StorageException ex)
				{
					_logger.LogWarning(ex, "Unable to read log event data for log {LogId} at line {LineIndex} (data may have been garbage collected)", logId, anchor.LineIndex);
					continue;
				}
		...
		/// <summary>
		/// Retrieve events for multiple logs in a single request
		/// </summary>
		/// <param name="request">Request containing log ids to fetch events for</param>
		/// <param name="cancellationToken">Cancellation token for the request</param>
		/// <returns>Events grouped by log id</returns>
		[HttpPost]
		[Route("/api/v1/logs/events")]
		[ProducesResponseType(typeof(GetBatchLogEventsResponse), 200)]
		public async Task<ActionResult<GetBatchLogEventsResponse>> GetBatchEventsAsync([FromBody] GetBatchLogEventsRequest request, CancellationToken cancellationToken = default)
		{
			GetBatchLogEventsResponse response = new GetBatchLogEventsResponse();

			foreach (string logIdStr in request.LogIds)
			{
				...
				List<ILogAnchor> anchors = await log.GetAnchorsAsync(null, null, null, cancellationToken);

				IEnumerable<ILogAnchor> matchingAnchors = anchors;
				if (request.Severity != null)
				{
					matchingAnchors = anchors.Where(a => a.Severity == request.Severity.Value);
				}

				List<ILogAnchor> matchingList = matchingAnchors.ToList();
				...
				IEnumerable<ILogAnchor> anchorsToFetch = request.Count != null ? matchingList.Take(request.Count.Value) : matchingList;

				Dictionary<ObjectId, int?> spanIdToIssueId = new Dictionary<ObjectId, int?>();
				List<GetLogEventResponse> events = new List<GetLogEventResponse>();

				foreach (ILogAnchor anchor in anchorsToFetch)
				{
					ILogEventData logEventData;
					try
					{
					...
					events.Add(CreateGetLogEventResponse(anchor, logEventData, issueId));
				}

				response.LogEvents[logIdStr] = new BatchLogEventsEntry { Total = total, Events = events };
			}
