...
namespace HordeServer.Tests.Agents
{
	...
	public class AgentSchedulerTests : ComputeTestSetup
	{
		...
		[TestMethod]
		public async Task FindTerminatedSessionsAsync()
		{
			RpcSession? session = await AgentScheduler.TryCreateSessionAsync(new AgentId("foo"), SessionIdUtils.GenerateNewId(), new RpcAgentCapabilities());
			Assert.IsNotNull(session);
			Assert.AreEqual(0, (await AgentScheduler.RemoveOrphanedSessionsAsync()).Count);

			SessionId orphanedId = SessionIdUtils.GenerateNewId();
			IDatabase database = GetRedisServiceSingleton().GetDatabase();
			string orphanedKey = HordeServer.Agents.AgentScheduler.SessionStateKey(orphanedId.ToString());
			await database.StringSetAsync(orphanedKey, "not-relevant");

			// Ensure only orphaned session are removed and valid ones are left intact
			List<SessionId> orphanedIds = await AgentScheduler.RemoveOrphanedSessionsAsync();
			CollectionAssert.AreEquivalent(new List<SessionId> { orphanedId }, orphanedIds);
			session = await AgentScheduler.TryGetSessionAsync(session.SessionId);
			Assert.IsNotNull(session);
		}
