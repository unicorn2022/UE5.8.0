...
namespace HordeServer.Tests.Tools
{
	...
	public class ToolTests : ServerTestSetup
	{
		...
		[TestMethod]
		public async Task MaxDeploymentsExceededPurgesOldestAsync()
		{
			// Reconfigure the tool with a low MaxDeployments limit
			const int MaxDeployments = 3;
			...
			ToolsConfig toolsConfig = new ToolsConfig();
			toolsConfig.Tools.Add(new ToolConfig(_toolId) { Name = "UnrealGameSync", Description = "Tool for syncing content from source control", MaxDeployments = MaxDeployments });
			...
			for (int i = 0; i < MaxDeployments + 2; i++)
			{
				string version = $"{i + 1}.0";
				versions.Add(version);

				byte[] zipData = await CreateZipFileDataAsync($"file-{version}.txt", $"content-{version}");
				using MemoryStream stream = new MemoryStream(zipData);
				tool = Deref(await tool.CreateDeploymentAsync(new ToolDeploymentConfig { Version = version }, stream, CancellationToken.None));
			}

			// Verify only MaxDeployments remain and they are the most recent ones
			tool = Deref(await collection.GetAsync(_toolId));
			Assert.AreEqual(MaxDeployments, tool.Deployments.Count);

			List<string> expectedVersions = versions.Skip(versions.Count - MaxDeployments).ToList();
			for (int i = 0; i < MaxDeployments; i++)
			{
				Assert.AreEqual(expectedVersions[i], tool.Deployments[i].Version);
			}
		}
