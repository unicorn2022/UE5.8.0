...
namespace HordeServer.Tools
{
	...
	public class ToolCollection : IToolCollection
	{
		...
		async Task<ToolDocument?> CreateDeploymentInternalAsync(ToolDocument tool, ToolConfig toolConfig, ToolDeploymentId deploymentId, ToolDeploymentConfig options, IStorageNamespace storageNamespace, IHashedBlobRef content, CancellationToken cancellationToken)
		{
			...
			for (; ; )
			{
				newTool = await TryAddDeploymentAsync(newTool, deployment, toolConfig.MaxDeployments, cancellationToken);
				if (newTool != null)
				{
					return newTool;
				}
		...
		async ValueTask<ToolDocument?> TryAddDeploymentAsync(ToolDocument tool, ToolDeploymentDocument deployment, int maxDeploymentCount, CancellationToken cancellationToken)
		{
			maxDeploymentCount = Math.Max(1, maxDeploymentCount);
