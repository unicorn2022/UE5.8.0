...
namespace HordeServer.Storage
{
	...
	public class StoragePlugin : IPluginStartup
	{
		...
		public void ConfigureServices(IServiceCollection services)
		{
			...
			services.AddSingleton<GcMetricsCollection>();
			services.AddSingleton<CollectionStatsService>();
			...
			if (_serverInfo.IsRunModeActive(RunMode.Worker) && !_serverInfo.ReadOnlyMode)
			{
				...
				services.AddHostedService(provider => provider.GetRequiredService<GcMetricsCollection>());
				services.AddHostedService(provider => provider.GetRequiredService<CollectionStatsService>());
			}
		}
