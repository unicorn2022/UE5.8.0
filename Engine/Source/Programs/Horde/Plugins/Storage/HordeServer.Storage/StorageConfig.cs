...
namespace HordeServer.Storage
{
	...
	public class StorageConfig : IPluginConfig
	{
		...
		/// <summary>
		/// Number of concurrent GC reachability-check workers. Default 8.
		/// Reachability checks are I/O-bound (MongoDB queries), so higher values
		/// scale throughput roughly linearly up to the database's capacity.
		/// </summary>
		public int GcWorkerCount { get; set; } = 8;

		/// <summary>
		/// Maximum number of entries in a namespace's Redis GC queue before the
		/// blob scanner pauses. Default 50,000.
		/// </summary>
		public long GcQueueLimit { get; set; } = 50000;

		/// <summary>
		/// Whether to await Redis writes when enqueuing blobs for GC checking.
		/// When false (default), uses fire-and-forget which can silently drop
		/// entries on Redis failures. Set true for reliable enqueue.
		/// </summary>
		public bool EnableReliableGcEnqueue { get; set; }

		/// <summary>
		/// Whether to isolate GC queue limits per-namespace. When false (default),
		/// one backed-up namespace pauses the scanner for ALL namespaces. When true,
		/// only the backed-up namespace is paused and its blobs are deferred.
		/// </summary>
		public bool EnablePerNamespaceQueueIsolation { get; set; }

		/// <summary>
		/// Number of blobs to process per blob-ingestion tick (TickBlobsAsync).
		/// Default 500. Raise to increase scan throughput when catching up on a
		/// backlog of unscanned blobs.
		/// </summary>
		public int GcBlobIngestionBatchSize { get; set; } = 500;

		/// <summary>
		/// Number of blob IDs to batch together for the import-reachability check.
		/// Default 1 (current per-blob behavior). When greater than 1, uses $in query to check
		/// multiple blobs in a single MongoDB round-trip. Values of 50-200 recommended.
		/// </summary>
		public int GcBatchSize { get; set; } = 1;
