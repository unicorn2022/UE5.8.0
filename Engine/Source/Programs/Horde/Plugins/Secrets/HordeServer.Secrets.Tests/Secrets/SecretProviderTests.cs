...
namespace HordeServer.Tests.Secrets
{
	...
	public class SecretProviderTests
	{
		...
		public async Task GetSecretAsync()
		{
			...
			Assert.AreEqual("j.doe", secret?.Data["username"]);
			Assert.AreEqual("password123", secret?.Data["password"]);
		}
		...
		public async Task GetSecretFromProviderConfigAsync()
		{
			...
			Assert.AreEqual("user01", secret?.Data["username"]);
			Assert.AreEqual("password01", secret?.Data["password"]);
		}
		...
		public async Task GetEmptySecretAsync()
		{
			...
			Assert.AreEqual(0, secret?.Data.Count);
		}
		...
		public async Task GetMultipleSecretFromVaultAsync()
		{
			...
			Assert.AreEqual("1", secret?.Data["one"]);
			Assert.AreEqual("2", secret?.Data["two"]);
			Assert.AreEqual("3", secret?.Data["three"]);
		}
		...
		public async Task SecretNotFoundFromVaultAsync()
		{
			...
			Assert.AreEqual("Unable to fetch secret '/v1/secret/data/notfound' from HCP Vault Status = NotFound Body={\"errors\":[]}\n", ex.Message);
		}
		...
		public async Task GetSecretFromVaultWithAwsAuthAsync()
		{
			...
			Assert.AreEqual("test", secret?.Data["test"]);
		}
		...
		public async Task GetSecretFromAzureKeyVaultAsync()
		{
			...
			Assert.AreEqual("test", secret?.Data["secret"]);
		}
