...
namespace HordeServer.Tests.Streams
{
	...
	public class StreamCollectionTests : BuildTestSetup
	{
		...
		public void ValidateUndefinedTemplateIdInTabs()
		{
			...
			Assert.ThrowsExactly<InvalidStreamException>(() => HordeServer.Streams.StreamCollection.Validate(_streamId, config));
		...
		public async Task ValidateGetChangesAsync()
		{
			...
			PerforceService.AddChange(fixture.StreamId, firstChangelist, jerry, "Edited file1.cpp.", new[] { "file1.cpp" });
		...
		[TestMethod]
		public async Task ValidateGetBatchedChangesAsync()
		{
			Fixture fixture = await CreateFixtureAsync();
			IUser jerry = await UserCollection.FindOrAddUserByLoginAsync("Jerry");
			...
			// Single element edge case
			{
				ActionResult<List<object>> results = await StreamsController.GetBatchedChangesAsync(fixture.StreamId, [firstCommitId]);
			...
			// Multi element nominal case
			{
				ActionResult<List<object>> results = await StreamsController.GetBatchedChangesAsync(fixture.StreamId, [firstCommitId, secondCommitId]);
			...
			{
				StreamsController httpContextPreservedStreamsController = StreamsController;
				ActionResult<List<object>> results = await httpContextPreservedStreamsController.GetBatchedChangesAsync(fixture.StreamId,[firstCommitId, invalidCommitId]);
			...
			// Empty edge case
			{
				ActionResult<List<object>> results = await StreamsController.GetBatchedChangesAsync(fixture.StreamId, []);
