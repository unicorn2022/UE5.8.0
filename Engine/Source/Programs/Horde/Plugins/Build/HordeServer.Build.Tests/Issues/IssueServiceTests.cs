...
namespace HordeServer.Tests.Issues
{
	...
	public abstract class AbstractIssueServiceTests : BuildTestSetup
	{
		...
		public async Task SetupAsync()
		{
			...
			buildConfig.Issues = new IssueConfig { ChannelGroups = new List<IssueChannelGroupConfig> { 
				new IssueChannelGroupConfig { Channels = new List<string> { "AssetCheck" } },
				new IssueChannelGroupConfig { Channels = new List<string> { "LogLinker", "LogBlueprint" } },
			} };
			...
			NodeAnnotations createWarningIssuesAnnotatons = new NodeAnnotations();
			createWarningIssuesAnnotatons.CreateWarningIssues = false;
			nodes.Add(MockNode("Cook ShooterGame Win64 Ignore Warnings", createWarningIssuesAnnotatons));
	...
	public class IssueServiceTests : AbstractIssueServiceTests
	{
		...
		public async Task DefaultIssueTestAsync()
		{
			...
			// Add a description to the issue
			{
				...
				Assert.AreEqual("Hello world!", newIssue?.Description);
			}
		}
		...
		public async Task DefaultIssueTest3Async()
		{
			...
			{
				// #2
				// Scenario: Subsequent job has a warning in a step with warning issue creation suppressed
				// Expected: Issues match previous job
				{
					IJob job = CreateJob(MainStreamId, 110, "Test Build", Graph);
					await AddEventAsync(job, 0, 6, LogLevel.Warning);
					await UpdateCompleteStepAsync(job, 0, 6, JobStepOutcome.Warnings);
		...
		public async Task PerforceCaseIssueTestAsync()
		{
			...
			// Expected: Creates issue, identifies source file correctly
			{
				...
				IUser anne = await UserCollection.FindOrAddUserByLoginAsync("Anne");
				_perforce.AddChange(MainStreamId, 170, anne, "Description", new string[] { "Engine/Foo/Baz.txt" });
		...
		public async Task ThreadSanitizerDeduplicateSimilarIssuesTestAsync()
		{
			...
			Assert.AreEqual(1, issue1.Fingerprints.Count);
			Assert.AreEqual(@"ThreadSanitizer:data race:/mnt/horde/++UE5/Sync/Engine/Source/./Runtime/Core/Private/Misc/CoreGlobals.cpp", issue1.Fingerprints[0].Type);

			IIssue issue2 = issues[1];
			Assert.AreEqual(1, issue2.Fingerprints.Count);
			Assert.AreEqual(@"ThreadSanitizer:data race:/mnt/horde/++UE5/Sync/Engine/Source/./Runtime/Core/Private/Misc/Paths.cpp", issue2.Fingerprints[0].Type);
		}
		...
		public async Task ThreadSanitizerEnsureUniqueIssuesForSameFileTestAsync()
		{
			...
			IIssue issue1 = issues[0];
			Assert.AreEqual(1, issue1.Fingerprints.Count);
			Assert.AreEqual(@"ThreadSanitizer:lock-order-inversion (potential deadlock):/mnt/horde/++UE5/Sync/Engine/Source/./Runtime/Core/Private/Misc/Paths.cpp", issue1.Fingerprints[0].Type);
		...
		[TestMethod]
		public async Task ThreadSanitizer_EnsureIssuesInPrecompiledLibsAreStableAsync()
		{
			string[] lines =
			{
				@"==================",
				@"WARNING: ThreadSanitizer: data race (pid=12631)",
				@"  Write of size 1 at 0x728800d2f0a4 by thread T35:",
				@"    #0 memcpy :115:5 (CitySampleEditor+0x10986af0) (BuildId: 2c4990f085335a7b909a90d0fb0e7f4b9983212d)",
				@"    #1 memcpy :107:1 (CitySampleEditor+0x10986af0)",
				@"    #2 <null> <null> (libEOSSDK-Linux-Shipping.so+0xcb8a2d) (BuildId: 1ba34a78527d8842)",
				@"  Previous write of size 8 at 0x728800d2f0a0 by main thread:",
				@"    #0 memset :87:3 (CitySampleEditor+0x109868d8) (BuildId: 2c4990f085335a7b909a90d0fb0e7f4b9983212d)",
				@"    #1 memset :85:1 (CitySampleEditor+0x109868d8)",
				@"    #2 <null> <null> (libEOSSDK-Linux-Shipping.so+0x16e5250) (BuildId: 1ba34a78527d8842)",
				@"SUMMARY: ThreadSanitizer: data race (/mnt/horde/++Fortnite/Sync/Samples/Showcases/CitySample/Binaries/Linux/libEOSSDK-Linux-Shipping.so+0xcb8a2d) (BuildId: 1ba34a78527d8842)",
				@"==================",
				@"==================",
				@"WARNING: ThreadSanitizer: data race (pid=12631)",
				@"  Write of size 1 at 0x728800d2f0a4 by thread T35:",
				...
				@"    #1 memset :85:1 (CitySampleEditor+0x109868d8)",
				@"    #2 <null> <null> (libEOSSDK-Linux-Shipping.so+0x16e5250) (BuildId: 1ba34a78527d8842)",
				@"SUMMARY: ThreadSanitizer: data race (/mnt/horde/++Fortnite/Sync/Samples/Showcases/CitySample/Binaries/Linux/libEOSSDK-Linux-Shipping.so+0x12345678) (BuildId: deadc0dec0ffee55)",
				@"=================="
			};
			...
			// The two issues, despite the different buildid and function offset, are collapsed to the same single issue/fingerprint
			IReadOnlyList<IIssue> issues = await IssueCollection.FindIssuesAsync();
			Assert.AreEqual(1, issues.Count);

			IIssue issue1 = issues[0];
			Assert.AreEqual(1, issue1.Fingerprints.Count);
			Assert.AreEqual(@"ThreadSanitizer:data race:/mnt/horde/++Fortnite/Sync/Samples/Showcases/CitySample/Binaries/Linux/libEOSSDK-Linux-Shipping.so", issue1.Fingerprints[0].Type);
		}

		[TestMethod]
		public async Task RaceDetectorSanitizerIssuesTestAsync()
		{
			// We have three unique errors reported, but we key issues on the file since file lines change over time without fixes, so only two unique issues should be found
			string[] lines =
			{
				@"=========================================",
				@"WARNING: RaceDetector: data race detected",
				@"Read of size 8 at 00007549CACE3090 made at clock 103 by thread FAssetDataDiscovery (38316) which is now at clock 103:",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\AssetRegistry\Private\AssetDataGatherer.cpp (1648):     [Inline Frame] UE::AssetDataGather::Private::FAssetDataDiscovery::IsSynchronous() 0x00007FFD6F6801B7",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\AssetRegistry\Private\AssetDataGatherer.cpp (2272):     UE::AssetDataGather::Private::FAssetDataDiscovery::SetIsIdle() 0x00007FFD6F6801B7",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\AssetRegistry\Private\AssetDataGatherer.cpp (1969):     UE::AssetDataGather::Private::FAssetDataDiscovery::TickInternal() 0x00007FFD6F67A451",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\AssetRegistry\Private\AssetDataGatherer.cpp (1677):     UE::AssetDataGather::Private::FAssetDataDiscovery::Run() 0x00007FFD6F676CDB",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Private\Windows\WindowsRunnableThread.cpp (156):   FRunnableThreadWin::Run() 0x00007FFD7E52B4A9",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Private\Windows\WindowsRunnableThread.cpp (79):    FRunnableThreadWin::GuardedRun() 0x00007FFD7E52B2BB",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Private\Microsoft\MicrosoftRunnableThread.h (38):  FRunnableThreadWin::_ThreadProc() 0x00007FFD7E51F476",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Private\Sanitizer\RaceDetectorWindows.cpp (165):   InstrumentedStartThread() 0x00007FFD7E299704",
				@"Previous Write of size 8 at 00007549CACE3090 at clock 2766722 by thread GameThread (26476 alive) which is now at clock 2767117:",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\AssetRegistry\Private\AssetDataGatherer.cpp (1641):     [Inline Frame] UE::AssetDataGather::Private::FAssetDataDiscovery::StartAsync() 0x00007FFD6F72096F",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\AssetRegistry\Private\AssetDataGatherer.cpp (3740):     UE::AssetDataGather::Private::FPreloader::DelayedInitialize() 0x00007FFD6F72096F",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\AssetRegistry\Private\AssetDataGatherer.cpp (3695):     [Inline Frame] UE::AssetDataGather::Private::FPreloader::FPreloader::<lambda_1>::operator()() 0x00007FFD6F7206C9",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Public\Templates\Invoke.h (47):                    [Inline Frame] Invoke() 0x00007FFD6F7206C9",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Public\Templates\Function.h (292):                 UE::Core::Private::Function::TFunctionRefCaller<`lambda at .\Runtime\AssetRegistry\Private\AssetDataGatherer.cpp:3695:73',void>::Call() 0x00007FFD6F7206C9",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Public\Templates\Function.h (414):                 [Inline Frame] UE::Core::Private::Function::TFunctionRefBase<UE::Core::Private::Function::TFunctionStorage<0>,void ()>::operator()() 0x00007FFD7E0D9D1C",
				@"Last known sync clock between both threads is 2766693:",
				@"C:\WINDOWS\System32\KERNEL32.DLL:                                                                      [Unknown Function] 0x00007FFDC87F4ED0",
				@"=========================================",
				@"=========================================",
				@"WARNING: RaceDetector: data race detected",
				@"Write of size 8 at 00007FFD7F64CCA8 made at clock 6266164 by thread GameThread (26476) which is now at clock 6266164:",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Private\Misc\CoreMisc.cpp (177):                              GetTargetPlatformManager() 0x00007FFD7DFCB11E",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\RenderCore\Private\RenderUtils.cpp (819):                          RenderUtilsInit() 0x00007FFD814EE045",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Launch\Private\LaunchEngineLoop.cpp (3148):                        FEngineLoop::PreInitPreStartupScreen() 0x00007FF7D0901838",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Launch\Private\LaunchEngineLoop.cpp (4369):                        [Inline Frame] FEngineLoop::PreInit() 0x00007FF7D08F1DAB",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Launch\Private\Launch.cpp (40):                                    [Inline Frame] EnginePreInit() 0x00007FF7D08F1DAB",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Launch\Private\Launch.cpp (146):                                   GuardedMain() 0x00007FF7D08F1DAB",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Launch\Private\Windows\LaunchWindows.cpp (123):                    GuardedMainWrapper() 0x00007FF7D092255B",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Launch\Private\Windows\LaunchWindows.cpp (277):                    LaunchWindowsStartup() 0x00007FF7D0923046",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Launch\Private\Windows\LaunchWindows.cpp (334):                    WinMain() 0x00007FF7D0923272",
				@"D:\a\_work\1\s\src\vctools\crt\vcstartup\src\startup\exe_common.inl (102):                                        [Inline Frame] invoke_main() 0x00007FF7D094CC72",
				@"D:\a\_work\1\s\src\vctools\crt\vcstartup\src\startup\exe_common.inl (288):                                        __scrt_common_main_seh() 0x00007FF7D094CC72",
				@"Previous Read of size 8 at 00007FFD7F64CCA8 at clock 44 by thread FIOSDeviceHelper.QueryTask_1 (3956 alive) which is now at clock 44:",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Private\Misc\CoreMisc.cpp (186):                              GetTargetPlatformManager() 0x00007FFD7DFCB1E6",
				@"D:\build\++Fortnite\Sync\Engine\Source\Developer\IOS\IOSTargetPlatformControls\Private\IOSDeviceHelper.cpp (362): FDeviceQueryTask::Run() 0x00007FFD3A5875FA",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Private\Windows\WindowsRunnableThread.cpp (156):              FRunnableThreadWin::Run() 0x00007FFD7E52B4A9",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Private\Windows\WindowsRunnableThread.cpp (79):               FRunnableThreadWin::GuardedRun() 0x00007FFD7E52B2BB",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Private\Microsoft\MicrosoftRunnableThread.h (38):             FRunnableThreadWin::_ThreadProc() 0x00007FFD7E51F476",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Private\Sanitizer\RaceDetectorWindows.cpp (165):              InstrumentedStartThread() 0x00007FFD7E299704",
				@"Last known sync clock between both threads is 43:",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Private\Microsoft\MinimalWindowsApi.cpp (60):                 Windows::EnterCriticalSection() 0x00007FFD7DED131D",
				@"=========================================",
				@"=========================================",
				@"WARNING: RaceDetector: data race detected",
				@"Write of size 8 at 00007FFD7F64CCA8 made at clock 6266164 by thread GameThread (26476) which is now at clock 6266164:",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Private\Misc\CoreMisc.cpp (120):                              GetTargetPlatformManager() 0x00007FFD7DFCB11E",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\RenderCore\Private\RenderUtils.cpp (819):                          RenderUtilsInit() 0x00007FFD814EE045",
				@"Previous Read of size 8 at 00007FFD7F64CCA8 at clock 44 by thread FIOSDeviceHelper.QueryTask_1 (3956 alive) which is now at clock 44:",
				@"D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Private\Misc\CoreMisc.cpp (186):                              GetTargetPlatformManager() 0x00007FFD7DFCB1E6",
				@"D:\build\++Fortnite\Sync\Engine\Source\Developer\IOS\IOSTargetPlatformControls\Private\IOSDeviceHelper.cpp (362): FDeviceQueryTask::Run() 0x00007FFD3A5875FA",
			...
			IJob job = CreateJob(MainStreamId, 56, "Sanitizer Report for 'SomeJob'", Graph);
			await ParseEventsAsync(job, 0, 0, lines);
			await UpdateCompleteStepAsync(job, 0, 0, JobStepOutcome.Failure);
			...
			IIssue issue1 = issues[0];
			Assert.AreEqual(1, issue1.Fingerprints.Count);
			Assert.AreEqual(@"RaceDetectorSanitizer:data race detected:D:\build\++Fortnite\Sync\Engine\Source\Runtime\Core\Private\Misc\CoreMisc.cpp", issue1.Fingerprints[0].Type);

			IIssue issue2 = issues[1];
			Assert.AreEqual(1, issue2.Fingerprints.Count);
			Assert.AreEqual(@"RaceDetectorSanitizer:data race detected:D:\build\++Fortnite\Sync\Engine\Source\Runtime\AssetRegistry\Private\AssetDataGatherer.cpp", issue2.Fingerprints[0].Type);
		}
		...
		public async Task AddressSanitizerDeduplicateSimilarIssuesTestAsync()
		{
			...
			IIssue issue1 = issues[0];
			Assert.AreEqual(1, issue1.Fingerprints.Count);
			Assert.AreEqual(@"AddressSanitizer:heap-use-after-free:/mnt/horde/FNR+Main+Inc/Sync/Engine/Source/Runtime/CoreUObject/Public/UObject/SomeOtherFile.h", issue1.Fingerprints[0].Type);

			IIssue issue2 = issues[1];
			Assert.AreEqual(1, issue2.Fingerprints.Count);
			Assert.AreEqual(@"AddressSanitizer:heap-use-after-free:/mnt/horde/FNR+Main+Inc/Sync/Engine/Source/Runtime/CoreUObject/Public/UObject/UObjectBase.h", issue2.Fingerprints[0].Type);
		}
		...
		public async Task AddressSanitizerEnsureUniqueIssuesForSameFileTestAsync()
		{
			...
			IIssue issue1 = issues[0];
			Assert.AreEqual(1, issue1.Fingerprints.Count);
			Assert.AreEqual(@"AddressSanitizer:stack-buffer-overflow:/mnt/horde/FNR+Main+Inc/Sync/Engine/Source/Runtime/CoreUObject/Public/UObject/UObjectBase.h", issue1.Fingerprints[0].Type);
		...
		public async Task WorldLeaksDeduplicateSimilarIssuesTestAsync()
		{
			...
			Assert.AreEqual(1, issue0.Fingerprints.Count);
		...
		[TestMethod]
		public async Task EnsureNoWarningTestAsync()
		{
			// #1
			// Scenario: Job step completes successfully at CL 105
			// Expected: No issues are created
		...
		public async Task CallstackTestAsync()
		{
			...
			Assert.AreEqual(1, issue1.Fingerprints.Count);
			Assert.AreEqual("Hashed", issue1.Fingerprints[0].Type);
		...
		[TestMethod]
		public async Task HashedIssueTest4Async()
		{
			// Previous successful build on CL with no author, to test no suspects path
			IJob job = CreateJob(MainStreamId, 118, "Compile Test", Graph);
			await UpdateCompleteStepAsync(job, 0, 0, JobStepOutcome.Success);
			await UpdateCompleteStepAsync(job, 0, 1, JobStepOutcome.Success);

			// Errors in different steps of the same job, still no suspects
			IJob job2 = CreateJob(MainStreamId, 119, "Compile Test", Graph);

			await ParseEventsAsync(job2, 0, 0, new[] { "Warning: This is a warning from the editor" });
			await UpdateCompleteStepAsync(job2, 0, 0, JobStepOutcome.Failure);

			await ParseEventsAsync(job2, 0, 1, new[] { "Warning: This is a warning from the editor" });
			await UpdateCompleteStepAsync(job2, 0, 1, JobStepOutcome.Failure);

			// Issues in different steps of the same job are grouped together
			IReadOnlyList<IIssue> issues = await IssueCollection.FindIssuesAsync();
			Assert.AreEqual(1, issues.Count);
		...
		static List<JsonLogEvent> ParseJSONStructuredLogs(string[] jsonStructuredLogs)
		{
			List<JsonLogEvent> jsonEvents = new List<JsonLogEvent>();
			foreach (string jsonStructuredLog in jsonStructuredLogs)
			{
				byte[] bytes = Encoding.UTF8.GetBytes(jsonStructuredLog);
				JsonLogEvent jsonEvent;
				Assert.IsTrue(JsonLogEvent.TryParse(bytes, out jsonEvent));
		...
		[TestMethod]
		public async Task ChannelGroupingTestAsync()
		{
			string[] jsonStructuredLogs =
			{
				@"{""time"":""2025-10-15T18:10:10.409Z"",""level"":""Error"",""message"":""AssetCheck: Error: This is an error"",""properties"":{ ""_channel"":{ ""$type"":""Channel"",""$text"":""AssetCheck""},""_severity"":{ ""$type"":""Severity"",""$text"":""Error""} },""format"":""{_channel}: {_severity}: This is an error""}",
				@"{""time"":""2025-10-15T18:10:11.409Z"",""level"":""Error"",""message"":""LogBlueprint: Error: This is a blueprint error 1"",""properties"":{ ""_channel"":{ ""$type"":""Channel"",""$text"":""LogBlueprint""},""_severity"":{ ""$type"":""Severity"",""$text"":""Error""} },""format"":""{_channel}: {_severity}: This is a blueprint error 1""}",
				@"{""time"":""2025-10-15T18:10:12.409Z"",""level"":""Error"",""message"":""LogBlueprint: Error: This is a blueprint error 2"",""properties"":{ ""_channel"":{ ""$type"":""Channel"",""$text"":""LogBlueprint""},""_severity"":{ ""$type"":""Severity"",""$text"":""Error""} },""format"":""{_channel}: {_severity}: This is a blueprint error 2""}",
				@"{""time"":""2025-10-15T18:10:13.409Z"",""level"":""Warning"",""message"":""LogLinker: Error: This is a linker error"",""properties"":{ ""_channel"":{ ""$type"":""Channel"",""$text"":""LogBlueprint""},""_severity"":{ ""$type"":""Severity"",""$text"":""Warning""} },""format"":""{_channel}: {_severity}: This is a linker error""}",
				@"{""time"":""2025-10-15T18:10:13.409Z"",""level"":""Error"",""message"":""LogOther: Error: This is another error"",""properties"":{ ""_channel"":{ ""$type"":""Channel"",""$text"":""LogOther""},""_severity"":{ ""$type"":""Severity"",""$text"":""Error""} },""format"":""{_channel}: {_severity}: This is another error""}",
				@"{""time"":""2025-10-15T18:10:13.409Z"",""level"":""Error"",""message"":""NoChannel: Error: This is an error with no channel"",""properties"":{ ""_severity"":{ ""$type"":""Severity"",""$text"":""Error""} },""format"":""{_channel}: {_severity}: This is an error with no channel""}"
			};
			//LogBlueprint
			List<JsonLogEvent> logEvents = ParseJSONStructuredLogs(jsonStructuredLogs);

			// #2
			// Scenario: Log Channel Grouping
			// Expected: Issues are generated upon log channel boundaries
			{
				IJob job = CreateJob(MainStreamId, 120, "Test Build", Graph);
				await using (TestJsonLogger logger = await CreateLoggerAsync(job, 0, 0))
				{
					foreach (JsonLogEvent outJsonLogEvent in logEvents)
					{
				...
				IReadOnlyList<IIssue> issues = await IssueCollection.FindIssuesAsync();
				issues = issues.OrderBy(x => x.Summary).ToList();
				Assert.AreEqual(3, issues.Count);

				Assert.AreEqual("AssetCheck errors in Update Version Files", issues[0].Summary);
				Assert.AreEqual("Hashed:AssetCheck", issues[0].Fingerprints[0].Type);

				Assert.AreEqual("LogBlueprint errors in Update Version Files", issues[1].Summary);
				Assert.AreEqual("Hashed:LogBlueprint_LogLinker", issues[1].Fingerprints[0].Type);

				Assert.AreEqual("LogOther errors in Update Version Files", issues[2].Summary);
				Assert.AreEqual("Hashed", issues[2].Fingerprints[0].Type);
			}
		}
		...
		{
			string logAssertionMessage = "Assertion failed: State.bGfxPSOSet [File:D:\\build\\U5M+Inc\\Sync\\Engine\\Source\\Runtime\\RHI\\Public\\RHIValidationContext.h] [Line: 809]\n"
			+ "A Graphics PSO has to be set to set resources into a shader!";

			string logEnsureMessage = "Ensure: Ensure condition failed: State.bGfxPSOSet [File:D:\\build\\U5M+Inc\\Sync\\Engine\\Source\\Runtime\\RHI\\Public\\RHIValidationContext.h] [Line: 809]\n"
			+ "A Graphics PSO has to be set to set resources into a shader!";
			...
			// Expected: Gauntlet fingerprint using step identifiers
			{
				...
				Assert.AreEqual(new IssueKey($"Update Version Files", IssueKeyType.None), issues[0].Fingerprints[0].Keys.First());
			...
			// Expected: Gauntlet fingerprint using hash prefix and generate only one issue as they share the same summary
			{
				...
				{
					LogValue logSummary = new LogValue(new Utf8String("Summary"), logAssertionMessage);
				...
				Assert.AreEqual("Automation fatal callstack errors in Update Version Files, Compile UnrealHeaderTool Win64 and Compile ShooterGameEditor Win64", issues[0].Summary);
			}
			// #2.5
			// Scenario: Gauntlet ensure event
			// Expected: Gauntlet fingerprint using hash prefix and generate only one issue as they share the same summary
			{
				IJob job = CreateJob(MainStreamId, 120, "Test Build", Graph);
				await using (TestJsonLogger logger = await CreateLoggerAsync(job, 0, 0))
				{
					logger.LogWarning(KnownLogEvents.Gauntlet, "{Summary}\n{Callstack}", logEnsureMessage, logCallstack);
				}
				await UpdateCompleteStepAsync(job, 0, 0, JobStepOutcome.Warnings);
				await using (TestJsonLogger logger = await CreateLoggerAsync(job, 0, 1))
				{
					Dictionary<string, object?> properties = new Dictionary<string, object?>
					{
						["Summary"] = logEnsureMessage,
						["Callstack"] = logAlternateCallstack
					};
					foreach (JsonLogEvent outJsonLogEvent in MultilineLogEvent(LogLevel.Warning, KnownLogEvents.Gauntlet, "Engine encounter a critical failure\n{Summary}\n{Callstack}", properties))
					{
						logger.LogJsonLogEvent(outJsonLogEvent);
					}
				}
				...
				await using (TestJsonLogger logger = await CreateLoggerAsync(job, 0, 2))
				{
					LogValue logSummary = new LogValue(new Utf8String("Summary"), logEnsureMessage);
					logger.LogWarning(KnownLogEvents.Gauntlet, "{Summary}\n{Callstack}", logSummary, logAlternateCallstack);
				}
				await UpdateCompleteStepAsync(job, 0, 2, JobStepOutcome.Warnings);

				IReadOnlyList<IIssue> issues = await IssueCollection.FindIssuesAsync();
				Assert.AreEqual(2, issues.Count);
				Assert.AreEqual(1, issues[0].Fingerprints.Count);
				Assert.AreEqual(true, issues[0].Fingerprints[0].Type.StartsWith("Gauntlet:test framework:with-callstack:", StringComparison.OrdinalIgnoreCase));
				Assert.AreEqual("hash:", issues[0].Fingerprints[0].Keys.First().Name.Substring(0, 5));
				Assert.AreEqual(37, issues[0].Fingerprints[0].Keys.First().Name.Length);
				Assert.AreEqual("Automation test framework ensure callstack warnings in Update Version Files, Compile UnrealHeaderTool Win64 and Compile ShooterGameEditor Win64", issues[0].Summary);
				Assert.AreEqual("Automation fatal callstack errors in Update Version Files, Compile UnrealHeaderTool Win64 and Compile ShooterGameEditor Win64", issues[1].Summary);
			}
			...
			// Expected: Gauntlet Test fingerprint using hash prefix and job step salt
			{
				...
				Assert.AreEqual("hash:", issues[0].Fingerprints[0].Keys.First().Name.Substring(0, 5));
				Assert.AreEqual($":Update Version Files", issues[0].Fingerprints[0].Keys.First().Name.Substring(37));
				Assert.AreEqual("Automation test errors in Update Version Files", issues[0].Summary);
			}
			// #4
			...
			// Expected: Fatal event will prune any issues that are not either a Fatal event or a Drop Build event
			{
				...
				Assert.AreEqual($"Update Version Files", issues[0].Fingerprints[0].Keys.First().Name);
		...
		public async Task QuarantineTestAsync()
		{
			...
			// Expected: Default issue is created
			{
				...
				Assert.AreEqual(1, details.Steps.Count);
		...
		[TestMethod]
		public async Task BidirectionalQueryTestAsync()
		{
			// Enable bidirectional query feature
			await UpdateConfigAsync(x => x.Plugins.GetBuildConfig().IssueFindSuspectsEnableBidirectionalQuery = true);

			// Add many changes to test bidirectional query - we need commits spanning a wide range
			// to verify that both ends of the range are queried
			IUser anne = await UserCollection.FindOrAddUserByLoginAsync("Anne");
			IUser bob = await UserCollection.FindOrAddUserByLoginAsync("Bob");

			// Add changes from 400-450 to create a range for testing
			for (int i = 400; i <= 450; i += 5)
			{
				_perforce.AddChange(MainStreamId, i, i % 2 == 0 ? anne : bob, "Description", new string[] { $"a/file{i}.cpp" });
			}

			// Create successful job at CL 400
			IJob job1 = CreateJob(MainStreamId, 400, "Test Build", Graph);
			await UpdateCompleteStepAsync(job1, 0, 0, JobStepOutcome.Success);

			// Create failing job at CL 450 with compile error
			string[] lines =
			{
				FileReference.Combine(WorkspaceDir, "foo.cpp").FullName + @"(10): error C1234: test error",
			};
			IJob job2 = CreateJob(MainStreamId, 450, "Test Build", Graph);
			await ParseEventsAsync(job2, 0, 0, lines);
			await UpdateCompleteStepAsync(job2, 0, 0, JobStepOutcome.Failure);

			// Verify issue was created and suspects were found using bidirectional query
			IReadOnlyList<IIssue> issues = await IssueCollection.FindIssuesAsync();
			Assert.AreEqual(1, issues.Count);
			...
			// All commits between 400 and 450 (exclusive of 400) should be suspects
			// since none touch the specific file
			Assert.IsTrue(suspects.Count > 0, "Expected suspects to be found with bidirectional query");
		}

		[TestMethod]
		public async Task BidirectionalQueryMergeDeduplicationTestAsync()
		{
			// Enable bidirectional query feature with small MaxChanges to force overlap
			await UpdateConfigAsync(x =>
			{
				x.Plugins.GetBuildConfig().IssueFindSuspectsEnableBidirectionalQuery = true;
				x.Plugins.GetBuildConfig().IssueFindSuspectsMaxChanges = 10;
			});

			IUser anne = await UserCollection.FindOrAddUserByLoginAsync("Anne");

			// Add exactly 10 changes - bidirectional with halfLimit=5 should deduplicate overlapping commits
			for (int i = 500; i <= 545; i += 5)
			{
				_perforce.AddChange(MainStreamId, i, anne, "Description", new string[] { $"a/test{i}.cpp" });
			}

			// Create successful job at CL 500
			IJob job1 = CreateJob(MainStreamId, 500, "Test Build", Graph);
			await UpdateCompleteStepAsync(job1, 0, 0, JobStepOutcome.Success);

			// Create failing job at CL 545
			string[] lines =
			{
				FileReference.Combine(WorkspaceDir, "bar.cpp").FullName + @"(10): error C1234: test error",
			};
			IJob job2 = CreateJob(MainStreamId, 545, "Test Build", Graph);
			await ParseEventsAsync(job2, 0, 0, lines);
			await UpdateCompleteStepAsync(job2, 0, 0, JobStepOutcome.Failure);
			...
			// Verify no duplicate suspects - each commit should appear only once
			HashSet<int> uniqueCommits = new HashSet<int>();
			foreach (IIssueSuspect suspect in suspects)
			{
				int change = suspect.CommitId.GetPerforceChange();
				Assert.IsTrue(uniqueCommits.Add(change), $"Duplicate suspect found for CL {change}");
			}
		}

		[TestMethod]
		public async Task ParallelScoringProducesSameResultsAsSequentialTestAsync()
		{
			// Test with sequential scoring first
			IUser chris = await UserCollection.FindOrAddUserByLoginAsync("Chris");

			// Add specific changes where one touches the error file
			_perforce.AddChange(MainStreamId, 600, chris, "Description", new string[] { "a/errorfile.cpp" });
			_perforce.AddChange(MainStreamId, 605, chris, "Description", new string[] { "a/otherfile.cpp" });
			_perforce.AddChange(MainStreamId, 610, chris, "Description", new string[] { "a/anotherfile.cpp" });

			// Create successful job
			IJob job1 = CreateJob(MainStreamId, 135, "Test Build", Graph);
			await UpdateCompleteStepAsync(job1, 0, 0, JobStepOutcome.Success);

			// Create failing job with error in errorfile.cpp
			string[] lines =
			{
				FileReference.Combine(WorkspaceDir, "a", "errorfile.cpp").FullName + @"(10): error C1234: test error",
			};
			IJob job2 = CreateJob(MainStreamId, 610, "Test Build", Graph);
			await ParseEventsAsync(job2, 0, 0, lines);
			await UpdateCompleteStepAsync(job2, 0, 0, JobStepOutcome.Failure);

			IReadOnlyList<IIssue> sequentialIssues = await IssueCollection.FindIssuesAsync();
			Assert.AreEqual(1, sequentialIssues.Count);
			IReadOnlyList<IIssueSuspect> sequentialSuspects = await IssueCollection.FindSuspectsAsync(sequentialIssues[0]);

			// Store sequential results
			HashSet<int> sequentialCommits = new HashSet<int>(sequentialSuspects.Select(s => s.CommitId.GetPerforceChange()));

			// Now enable parallel scoring and create a new issue
			await UpdateConfigAsync(x => x.Plugins.GetBuildConfig().IssueFindSuspectsEnableParallelScoring = true);

			// Add more changes for parallel test
			_perforce.AddChange(MainStreamId, 700, chris, "Description", new string[] { "a/errorfile2.cpp" });
			_perforce.AddChange(MainStreamId, 705, chris, "Description", new string[] { "a/otherfile2.cpp" });
			_perforce.AddChange(MainStreamId, 710, chris, "Description", new string[] { "a/anotherfile2.cpp" });

			// Create successful job at 610
			IJob job3 = CreateJob(MainStreamId, 610, "Test Build 2", Graph);
			await UpdateCompleteStepAsync(job3, 0, 0, JobStepOutcome.Success);

			// Create failing job with error in errorfile2.cpp
			string[] lines2 =
			{
				FileReference.Combine(WorkspaceDir, "a", "errorfile2.cpp").FullName + @"(10): error C1234: test error",
			};
			IJob job4 = CreateJob(MainStreamId, 710, "Test Build 2", Graph);
			await ParseEventsAsync(job4, 0, 0, lines2);
			await UpdateCompleteStepAsync(job4, 0, 0, JobStepOutcome.Failure);

			IReadOnlyList<IIssue> parallelIssues = await IssueCollection.FindIssuesAsync();
			// Filter to the new issue
			IIssue? parallelIssue = parallelIssues.FirstOrDefault(i => i.Fingerprints.Any(f => f.Keys.Any(k => k.Name.Contains("errorfile2", StringComparison.Ordinal))));
			Assert.IsNotNull(parallelIssue, "Parallel issue should be created");

			IReadOnlyList<IIssueSuspect> parallelSuspects = await IssueCollection.FindSuspectsAsync(parallelIssue);

			// Verify parallel scoring found suspects
			Assert.IsTrue(parallelSuspects.Count > 0, "Parallel scoring should find suspects");

			// The suspect with the matching file (CL 700) should be identified
			Assert.IsTrue(parallelSuspects.Any(s => s.CommitId.GetPerforceChange() == 700),
				"Parallel scoring should identify the commit touching the error file");
		}

		[TestMethod]
		public async Task ParallelScoringWithMaxParallelismTestAsync()
		{
			// Enable parallel scoring with low parallelism
			await UpdateConfigAsync(x =>
			{
				x.Plugins.GetBuildConfig().IssueFindSuspectsEnableParallelScoring = true;
				x.Plugins.GetBuildConfig().IssueFindSuspectsMaxParallelism = 2;
			});

			IUser anne = await UserCollection.FindOrAddUserByLoginAsync("Anne");

			// Add several changes
			for (int i = 800; i <= 820; i += 5)
			{
				_perforce.AddChange(MainStreamId, i, anne, "Description", new string[] { $"a/file{i}.cpp" });
			}

			// Create successful job
			IJob job1 = CreateJob(MainStreamId, 800, "Test Build", Graph);
			await UpdateCompleteStepAsync(job1, 0, 0, JobStepOutcome.Success);

			// Create failing job
			string[] lines =
			{
				FileReference.Combine(WorkspaceDir, "a", "file815.cpp").FullName + @"(10): error C1234: test error",
			};
			IJob job2 = CreateJob(MainStreamId, 820, "Test Build", Graph);
			await ParseEventsAsync(job2, 0, 0, lines);
			await UpdateCompleteStepAsync(job2, 0, 0, JobStepOutcome.Failure);

			IReadOnlyList<IIssue> issues = await IssueCollection.FindIssuesAsync();
			IIssue? issue = issues.FirstOrDefault(i => i.Fingerprints.Any(f => f.Keys.Any(k => k.Name.Contains("file815", StringComparison.Ordinal))));
			Assert.IsNotNull(issue, "Issue should be created");

			IReadOnlyList<IIssueSuspect> suspects = await IssueCollection.FindSuspectsAsync(issue);

			// CL 815 should be the primary suspect since it touches the error file
			Assert.IsTrue(suspects.Any(s => s.CommitId.GetPerforceChange() == 815),
				"Parallel scoring with limited parallelism should still identify correct suspect");
		}

		[TestMethod]
		public async Task EarlyExitWhenMaxScoreAchievedTestAsync()
		{
			// Enable early exit
			await UpdateConfigAsync(x =>
			{
				x.Plugins.GetBuildConfig().IssueFindSuspectsEnableEarlyExit = true;
			});

			IUser jerry = await UserCollection.FindOrAddUserByLoginAsync("Jerry");

			// Add changes - put the matching file commit early in the list
			_perforce.AddChange(MainStreamId, 900, jerry, "Description", new string[] { "a/earlyexit.cpp" });
			_perforce.AddChange(MainStreamId, 905, jerry, "Description", new string[] { "a/other1.cpp" });
			_perforce.AddChange(MainStreamId, 910, jerry, "Description", new string[] { "a/other2.cpp" });
			_perforce.AddChange(MainStreamId, 915, jerry, "Description", new string[] { "a/other3.cpp" });
			_perforce.AddChange(MainStreamId, 920, jerry, "Description", new string[] { "a/other4.cpp" });

			// Create successful job
			IJob job1 = CreateJob(MainStreamId, 135, "Test Build", Graph);
			await UpdateCompleteStepAsync(job1, 0, 0, JobStepOutcome.Success);

			// Create failing job with error in earlyexit.cpp
			string[] lines =
			{
				FileReference.Combine(WorkspaceDir, "a", "earlyexit.cpp").FullName + @"(10): error C1234: test error",
			};
			IJob job2 = CreateJob(MainStreamId, 920, "Test Build", Graph);
			await ParseEventsAsync(job2, 0, 0, lines);
			await UpdateCompleteStepAsync(job2, 0, 0, JobStepOutcome.Failure);

			IReadOnlyList<IIssue> issues = await IssueCollection.FindIssuesAsync();
			IIssue? issue = issues.FirstOrDefault(i => i.Fingerprints.Any(f => f.Keys.Any(k => k.Name.Contains("earlyexit", StringComparison.Ordinal))));
			Assert.IsNotNull(issue, "Issue should be created");

			IReadOnlyList<IIssueSuspect> suspects = await IssueCollection.FindSuspectsAsync(issue);

			// CL 900 should be identified as the suspect (file match)
			Assert.IsTrue(suspects.Any(s => s.CommitId.GetPerforceChange() == 900),
				"Early exit should still identify the correct suspect");
		}

		[TestMethod]
		public async Task FingerprintFallbackWhenNoSuspectsFoundTestAsync()
		{
			// Enable fingerprint fallback
			await UpdateConfigAsync(x =>
			{
				x.Plugins.GetBuildConfig().IssueFindSuspectsEnableFingerprintFallback = true;
				x.Plugins.GetBuildConfig().IssueFingerprintFallbackMaxAgeDays = 14;
				x.Plugins.GetBuildConfig().IssueFingerprintFallbackMinScore = 50;
			});

			IUser bob = await UserCollection.FindOrAddUserByLoginAsync("Bob");

			// First, create an issue with a specific fingerprint by having a compile error
			_perforce.AddChange(MainStreamId, 1000, bob, "Description", new string[] { "a/fallbacktest.cpp" });
			...
			string[] lines =
			{
				FileReference.Combine(WorkspaceDir, "a", "fallbacktest.cpp").FullName + @"(10): error C1234: specific error message",
			};
			IJob job2 = CreateJob(MainStreamId, 1000, "Test Build", Graph);
			await ParseEventsAsync(job2, 0, 0, lines);
			await UpdateCompleteStepAsync(job2, 0, 0, JobStepOutcome.Failure);

			IReadOnlyList<IIssue> initialIssues = await IssueCollection.FindIssuesAsync();
			IIssue? originalIssue = initialIssues.FirstOrDefault(i => i.Fingerprints.Any(f => f.Keys.Any(k => k.Name.Contains("fallbacktest", StringComparison.Ordinal))));
			Assert.IsNotNull(originalIssue, "Original issue should be created");
			int originalIssueId = originalIssue.Id;

			// Now simulate a scenario where a new failure happens but no suspects can be found
			// because the commit range is too large (we won't add commits between 1000 and 2000)
			// The fingerprint fallback should match the existing issue

			// Create a new failure with the same fingerprint pattern
			_perforce.AddChange(MainStreamId, 2000, bob, "Description", new string[] { "a/unrelated.cpp" });

			// Use the same error file to generate matching fingerprint
			string[] lines2 =
			{
				FileReference.Combine(WorkspaceDir, "a", "fallbacktest.cpp").FullName + @"(10): error C1234: specific error message",
			};
			IJob job3 = CreateJob(MainStreamId, 2000, "Test Build", Graph);
			await ParseEventsAsync(job3, 0, 0, lines2);
			await UpdateCompleteStepAsync(job3, 0, 0, JobStepOutcome.Failure);

			// The fingerprint fallback should have matched the existing issue instead of creating a new one
			IReadOnlyList<IIssue> finalIssues = await IssueCollection.FindIssuesAsync();
			IReadOnlyList<IIssue> matchingIssues = finalIssues.Where(i => i.Fingerprints.Any(f => f.Keys.Any(k => k.Name.Contains("fallbacktest", StringComparison.Ordinal)))).ToList();

			// With fingerprint fallback, should still only have 1 issue (the original)
			// because the new span should have been added to the existing issue
			IReadOnlyList<IIssueSpan> spans = await IssueCollection.FindSpansAsync(originalIssueId);
			Assert.IsTrue(spans.Count >= 1, "Original issue should have spans");
		}

		[TestMethod]
		public async Task FingerprintFallbackExcludedTypesTestAsync()
		{
			// Enable fingerprint fallback but with excluded types
			await UpdateConfigAsync(x =>
			{
				x.Plugins.GetBuildConfig().IssueFindSuspectsEnableFingerprintFallback = true;
				x.Plugins.GetBuildConfig().IssueFingerprintFallbackExcludedTypes = new List<string> { "Default", "ExitCode", "Generic" };
			});

			IUser anne = await UserCollection.FindOrAddUserByLoginAsync("Anne");

			// Create a generic/default issue first
			_perforce.AddChange(MainStreamId, 1100, anne, "Description", new string[] { "a/generic.cpp" });

			IJob job1 = CreateJob(MainStreamId, 135, "Test Build", Graph);
			await UpdateCompleteStepAsync(job1, 0, 0, JobStepOutcome.Success);

			// Create a generic warning (should create Default type issue)
			IJob job2 = CreateJob(MainStreamId, 1100, "Test Build", Graph);
			await AddEventAsync(job2, 0, 0, LogLevel.Warning);
			await UpdateCompleteStepAsync(job2, 0, 0, JobStepOutcome.Warnings);

			IReadOnlyList<IIssue> issues1 = await IssueCollection.FindIssuesAsync();
			Assert.IsTrue(issues1.Count >= 1, "Default issue should be created");

			int issueCountBefore = issues1.Count;

			// Now create another warning - since Default type is excluded from fingerprint fallback,
			// a new issue should be created if no suspects found
			_perforce.AddChange(MainStreamId, 1200, anne, "Description", new string[] { "a/generic2.cpp" });

			IJob job3 = CreateJob(MainStreamId, 1200, "Test Build", Graph);
			await AddEventAsync(job3, 0, 0, LogLevel.Warning);
			await UpdateCompleteStepAsync(job3, 0, 0, JobStepOutcome.Warnings);

			IReadOnlyList<IIssue> issues2 = await IssueCollection.FindIssuesAsync();

			// Default type should be excluded from fingerprint matching,
			// so this depends on whether normal matching finds the issue
			Assert.IsTrue(issues2.Count >= issueCountBefore, "Issues should be tracked");
		}

		[TestMethod]
		public async Task FingerprintFallbackMaxAgeDaysTestAsync()
		{
			// Enable fingerprint fallback with short max age
			await UpdateConfigAsync(x =>
			{
				x.Plugins.GetBuildConfig().IssueFindSuspectsEnableFingerprintFallback = true;
				x.Plugins.GetBuildConfig().IssueFingerprintFallbackMaxAgeDays = 1; // Very short for testing
			});

			IUser chris = await UserCollection.FindOrAddUserByLoginAsync("Chris");

			// Create an initial issue
			_perforce.AddChange(MainStreamId, 1300, chris, "Description", new string[] { "a/agefile.cpp" });
			...
			string[] lines =
			{
				FileReference.Combine(WorkspaceDir, "a", "agefile.cpp").FullName + @"(10): error C1234: age test error",
			};
			IJob job2 = CreateJob(MainStreamId, 1300, "Test Build", Graph);
			await ParseEventsAsync(job2, 0, 0, lines);
			await UpdateCompleteStepAsync(job2, 0, 0, JobStepOutcome.Failure);

			IReadOnlyList<IIssue> issues = await IssueCollection.FindIssuesAsync();
			int initialCount = issues.Count;
			Assert.IsTrue(initialCount >= 1, "Initial issue should be created");

			// Advance time beyond the max age
			await Clock.AdvanceAsync(TimeSpan.FromDays(2));

			// Create another failure with the same fingerprint
			_perforce.AddChange(MainStreamId, 1400, chris, "Description", new string[] { "a/unrelated.cpp" });

			string[] lines2 =
			{
				FileReference.Combine(WorkspaceDir, "a", "agefile.cpp").FullName + @"(10): error C1234: age test error",
			};
			IJob job3 = CreateJob(MainStreamId, 1400, "Test Build", Graph);
			await ParseEventsAsync(job3, 0, 0, lines2);
			await UpdateCompleteStepAsync(job3, 0, 0, JobStepOutcome.Failure);

			// After advancing time, the old issue is too old for fingerprint fallback
			// A new issue may be created depending on normal issue matching behavior
			IReadOnlyList<IIssue> finalIssues = await IssueCollection.FindIssuesAsync();
			Assert.IsTrue(finalIssues.Count >= initialCount, "Issues should be tracked appropriately");
		}

		[TestMethod]
		public async Task FingerprintFallbackMinScoreTestAsync()
		{
			// Enable fingerprint fallback with high min score
			await UpdateConfigAsync(x =>
			{
				x.Plugins.GetBuildConfig().IssueFindSuspectsEnableFingerprintFallback = true;
				x.Plugins.GetBuildConfig().IssueFingerprintFallbackMinScore = 100; // Maximum score required
			});

			IUser bob = await UserCollection.FindOrAddUserByLoginAsync("Bob");

			// Create an initial issue with a specific fingerprint
			_perforce.AddChange(MainStreamId, 1500, bob, "Description", new string[] { "a/scorefile.cpp" });
			...
			string[] lines =
			{
				FileReference.Combine(WorkspaceDir, "a", "scorefile.cpp").FullName + @"(10): error C1234: score test error",
			};
			IJob job2 = CreateJob(MainStreamId, 1500, "Test Build", Graph);
			await ParseEventsAsync(job2, 0, 0, lines);
			await UpdateCompleteStepAsync(job2, 0, 0, JobStepOutcome.Failure);

			IReadOnlyList<IIssue> issues = await IssueCollection.FindIssuesAsync();
			Assert.IsTrue(issues.Count >= 1, "Initial issue should be created");

			// With high min score requirement, partial matches won't trigger fallback
			// This test verifies the configuration is respected
		}

		[TestMethod]
		public async Task CombinedBidirectionalAndParallelScoringTestAsync()
		{
			...
			await UpdateConfigAsync(x =>
			{
				x.Plugins.GetBuildConfig().IssueFindSuspectsEnableBidirectionalQuery = true;
				x.Plugins.GetBuildConfig().IssueFindSuspectsEnableParallelScoring = true;
				x.Plugins.GetBuildConfig().IssueFindSuspectsMaxParallelism = 5;
			});

			IUser jerry = await UserCollection.FindOrAddUserByLoginAsync("Jerry");

			// Add many changes to test both features working together
			for (int i = 1600; i <= 1650; i += 5)
			{
				_perforce.AddChange(MainStreamId, i, jerry, "Description", new string[] { $"a/combined{i}.cpp" });
			}

			// Create successful job
			IJob job1 = CreateJob(MainStreamId, 1600, "Test Build", Graph);
			await UpdateCompleteStepAsync(job1, 0, 0, JobStepOutcome.Success);

			// Create failing job with specific file error
			string[] lines =
			{
				FileReference.Combine(WorkspaceDir, "a", "combined1625.cpp").FullName + @"(10): error C1234: combined test error",
			};
			IJob job2 = CreateJob(MainStreamId, 1650, "Test Build", Graph);
			await ParseEventsAsync(job2, 0, 0, lines);
			await UpdateCompleteStepAsync(job2, 0, 0, JobStepOutcome.Failure);

			IReadOnlyList<IIssue> issues = await IssueCollection.FindIssuesAsync();
			IIssue? issue = issues.FirstOrDefault(i => i.Fingerprints.Any(f => f.Keys.Any(k => k.Name.Contains("combined1625", StringComparison.Ordinal))));
			Assert.IsNotNull(issue, "Issue should be created with combined features");

			IReadOnlyList<IIssueSuspect> suspects = await IssueCollection.FindSuspectsAsync(issue);

			// CL 1625 should be identified
			Assert.IsTrue(suspects.Any(s => s.CommitId.GetPerforceChange() == 1625),
				"Combined bidirectional + parallel should identify correct suspect");
		}

		[TestMethod]
		public async Task AllFeaturesEnabledIntegrationTestAsync()
		{
			// Enable all new features together
			await UpdateConfigAsync(x =>
			{
				x.Plugins.GetBuildConfig().IssueFindSuspectsEnableBidirectionalQuery = true;
				x.Plugins.GetBuildConfig().IssueFindSuspectsEnableParallelScoring = true;
				x.Plugins.GetBuildConfig().IssueFindSuspectsEnableEarlyExit = true;
				x.Plugins.GetBuildConfig().IssueFindSuspectsEnableFingerprintFallback = true;
				x.Plugins.GetBuildConfig().IssueFindSuspectsMaxParallelism = 10;
				x.Plugins.GetBuildConfig().IssueFingerprintFallbackMinScore = 50;
			});
			...
			for (int i = 1700; i <= 1750; i += 5)
			{
				_perforce.AddChange(MainStreamId, i, anne, "Description", new string[] { $"a/integration{i}.cpp" });
			}

			// Create successful job
			IJob job1 = CreateJob(MainStreamId, 1700, "Test Build", Graph);
			await UpdateCompleteStepAsync(job1, 0, 0, JobStepOutcome.Success);

			// Create failing job
			string[] lines =
			{
				FileReference.Combine(WorkspaceDir, "a", "integration1730.cpp").FullName + @"(10): error C1234: integration test error",
			};
			IJob job2 = CreateJob(MainStreamId, 1750, "Test Build", Graph);
			await ParseEventsAsync(job2, 0, 0, lines);
			await UpdateCompleteStepAsync(job2, 0, 0, JobStepOutcome.Failure);

			IReadOnlyList<IIssue> issues = await IssueCollection.FindIssuesAsync();
			IIssue? issue = issues.FirstOrDefault(i => i.Fingerprints.Any(f => f.Keys.Any(k => k.Name.Contains("integration1730", StringComparison.Ordinal))));
			Assert.IsNotNull(issue, "Issue should be created with all features enabled");

			IReadOnlyList<IIssueSuspect> suspects = await IssueCollection.FindSuspectsAsync(issue);

			// The commit touching the error file should be identified
			Assert.IsTrue(suspects.Any(s => s.CommitId.GetPerforceChange() == 1730),
				"All features enabled should correctly identify suspect");

			// Verify no duplicate suspects
			HashSet<int> uniqueCommits = new HashSet<int>();
			foreach (IIssueSuspect suspect in suspects)
			{
				int change = suspect.CommitId.GetPerforceChange();
				Assert.IsTrue(uniqueCommits.Add(change), $"No duplicate suspects should exist for CL {change}");
			}
		}
	}
}
