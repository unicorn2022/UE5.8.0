...
namespace HordeServer.Tests.Fleet
{
	...
	public class AwsRecyclingFleetManagerTest : BuildTestSetup
	{
		...
		public async Task ExpandAmazonEc2ExceptionsArePropagatedAsync()
		{
			...
			await Assert.ThrowsExactlyAsync<AmazonEC2Exception>(() => ExpandPoolAsync(ec2.Get(), 1, new()));
		}
		...
		[TestMethod]
		public async Task ExpandOneAzUnsupportedWithFallbackAsync()
		{
			FakeAmazonEc2 ec2 = new();
			Instance i1 = ec2.AddInstance(FakeAmazonEc2.StateStopped, InstanceType.M5Large, FakeAmazonEc2.AzUsEast1A, tagPoolName: PoolName, tagPoolId: PoolId);
			Instance i2 = ec2.AddInstance(FakeAmazonEc2.StateStopped, InstanceType.M5Large, FakeAmazonEc2.AzUsEast1A, tagPoolName: PoolName, tagPoolId: PoolId);
 
			ec2.SetUnsupported(FakeAmazonEc2.AzUsEast1A, InstanceType.M5Large.Value);
			ec2.SetCapacity(FakeAmazonEc2.AzUsEast1A, InstanceType.C5Large, 2);
 
			await ExpandPoolAsync(ec2.Get(), 2, new(new List<string> { InstanceType.M5Large, InstanceType.C5Large }));
		...
		[TestMethod]
		public async Task ExpandUnsupportedNoFallbackAsync()
		{
			FakeAmazonEc2 ec2 = new();
			ec2.AddInstance(FakeAmazonEc2.StateStopped, InstanceType.M5Large, FakeAmazonEc2.AzUsEast1A, tagPoolName: PoolName);
			ec2.SetUnsupported(FakeAmazonEc2.AzUsEast1A, InstanceType.M5Large.Value);
		...
		[TestMethod]
		public async Task ExpandMixedErrorsWithFallbackAsync()
		{
			FakeAmazonEc2 ec2 = new();
			ec2.AddInstance(FakeAmazonEc2.StateStopped, InstanceType.M5Large, FakeAmazonEc2.AzUsEast1A, tagPoolName: PoolName);
 
			// First: unsupported, Second: no capacity, Third: works
			ec2.SetUnsupported(FakeAmazonEc2.AzUsEast1A, InstanceType.M5Large.Value);
			ec2.SetCapacity(FakeAmazonEc2.AzUsEast1A, InstanceType.C5Large, 0);
			ec2.SetCapacity(FakeAmazonEc2.AzUsEast1A, InstanceType.R5Large, 1);
 
			await ExpandPoolAsync(ec2.Get(), 1, new(new List<string> {
