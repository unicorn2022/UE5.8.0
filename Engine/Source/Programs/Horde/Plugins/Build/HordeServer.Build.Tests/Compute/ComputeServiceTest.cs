...
namespace HordeServer.Tests.Compute
{
	...
	public class ComputeServiceTest : BuildTestSetup
	{
		...
		public async Task Assignment_RequesterAlsoRunsAgent_IsNotAssignedAsync()
		{
			...
			await Assert.ThrowsExactlyAsync<NoComputeResourcesException>(() => ComputeService.TryAllocateResourceAsync(CreateParams("11.0.0.1"), CancellationToken.None));
			await Assert.ThrowsExactlyAsync<NoComputeResourcesException>(() => ComputeService.TryAllocateResourceAsync(CreateParams("11.0.0.1"), CancellationToken.None));
		...
		public async Task Assignment_PoolInRequirementsAsync()
		{
			...
			await Assert.ThrowsExactlyAsync<NoComputeResourcesException>(() => ComputeService.TryAllocateResourceAsync(CreateParams("does-not-exist"), CancellationToken.None));
		}
		...
		public async Task Assignment_NoResources_Async()
		{
			...
			NoComputeResourcesException ex1 = await Assert.ThrowsExactlyAsync<NoComputeResourcesException>(() => ComputeService.TryAllocateResourceAsync(CreateParams("foo"), CancellationToken.None));
			Assert.AreEqual(NoComputeResourcesException.AllResourcesInUse, ex1.Message);
			
			NoComputeResourcesException ex2 = await Assert.ThrowsExactlyAsync<NoComputeResourcesException>(() => ComputeService.TryAllocateResourceAsync(CreateParams("bar"), CancellationToken.None));
		...
		public async Task Assignment_ComputeClusterMemberConditionAsync()
		{
...

			{
				...
				await Assert.ThrowsExactlyAsync<NoComputeResourcesException>(() => ComputeService.TryAllocateResourceAsync(arpBar, CancellationToken.None));
			}

			{
				AllocateResourceParams arpFoo = new(cluster2, _user, ComputeProtocol.Latest, new Requirements { Pool = poolFoo.ToString() });
				await Assert.ThrowsExactlyAsync<NoComputeResourcesException>(() => ComputeService.TryAllocateResourceAsync(arpFoo, CancellationToken.None));
		...
		public void Cluster_FindBest()
		{
			...
			Assert.ThrowsExactly<ComputeServiceException>(() => ComputeService.FindBestComputeClusterId(computeConfig, IPAddress.Parse("123.123.123.123")));
		...
		{
			ComputeServiceException cse1 = await Assert.ThrowsExactlyAsync<ComputeServiceException>(() => AllocateWithAgentAsync(useUbaCache: true, ubaConfig: null));
			...
			ComputeServiceException cse2 = await Assert.ThrowsExactlyAsync<ComputeServiceException>(() => AllocateWithAgentAsync(useUbaCache: true, ubaConfig: uccc));
