...
{
	public class PerforceServiceStub : IPerforceService
	{
		...
		*/
		class CommitCollection : ICommitCollection
		{
			...
			public async IAsyncEnumerable<ICommit> FindAsync(CommitId? minChange, bool includeMinChange, CommitId? maxChange, bool includeMaxChange, int? maxResults, IReadOnlyList<CommitTag>? tags, CommitSortOrder sortOrder = CommitSortOrder.Descending, [EnumeratorCancellation] CancellationToken cancellationToken = default)
			{
				IEnumerable<ICommit> commits = await _owner.GetChangesAsync(_streamConfig, minChange?.GetPerforceChange(), includeMinChange, maxChange?.GetPerforceChange(), includeMaxChange, null, cancellationToken);

				// Handle sort order - the test stub stores in descending order by default
