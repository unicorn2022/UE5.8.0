...
namespace HordeServer.Tests.Jobs
{
	...
	public class JobCollectionTests : BuildTestSetup
	{
		...
		{
			return AddNode(group, name, inputDependencies, null, action);
		}

		static NewNode AddNode(NewGroup group, string name, string[]? inputDependencies, string[]? optionalInputDependencies, Action<NewNode>? action = null)
		{
			HashSet<string>? orderDependencies = [];
			if (inputDependencies != null)
			{
				orderDependencies.UnionWith(inputDependencies);
			}
			if (optionalInputDependencies != null)
			{
				orderDependencies.UnionWith(optionalInputDependencies);
			}

			NewNode node = new(name, inputDependencies: inputDependencies?.ToList(), optionalInputDependencies: optionalInputDependencies?.ToList(), orderDependencies: orderDependencies?.ToList());
			action?.Invoke(node);
		...
		[TestMethod]
		public async Task TestStatesSkippedWhenFailureOfRequiredAsync()
		{
			...
			options.Arguments.Add("-Target=Node C");
			options.Arguments.Add("-Target=Node D");
			...
			NewGroup groupA = AddGroup(newGroups);
			AddNode(groupA, "Node A", null);

			NewGroup groupB = AddGroup(newGroups);
			AddNode(groupB, "Node B", null, ["Node A"]);

			NewGroup groupC = AddGroup(newGroups);
			AddNode(groupC, "Node C", ["Node B"]);

			NewGroup groupD = AddGroup(newGroups);
			AddNode(groupD, "Node D", ["Node B"]);

			IGraph graph = await GraphCollection.AppendAsync(baseGraph, newGroups, null, null);
			job = Deref(await job.TryUpdateGraphAsync(graph));

			job = await RunBatchAsync(job, 1);
			job = await RunStepAsync(job, 1, 0, JobStepOutcome.Success); // A

			job = await RunBatchAsync(job, 2);
			job = await RunStepAsync(job, 2, 0, JobStepOutcome.Failure); // B
			...
			Assert.AreEqual(JobStepState.Skipped, job.Batches[3].Steps[0].State);
			Assert.AreEqual(JobStepState.Skipped, job.Batches[4].Steps[0].State);
		}

		[TestMethod]
		public async Task TestStatesCompleteWhenNoFailureAsync()
		{
			Mock<ITemplate> templateMock = new(MockBehavior.Strict);
			templateMock.SetupGet(x => x.InitialAgentType).Returns((string?)null);
			...
			job = await RunBatchAsync(job, 2);
			job = await RunStepAsync(job, 2, 0, JobStepOutcome.Success); // B

			job = await RunBatchAsync(job, 3);
			job = await RunStepAsync(job, 3, 0, JobStepOutcome.Success); // C

			job = await RunBatchAsync(job, 4);
			job = await RunStepAsync(job, 4, 0, JobStepOutcome.Success); // D

			Assert.AreEqual(JobStepState.Completed, job.Batches[1].Steps[0].State);
			Assert.AreEqual(JobStepState.Completed, job.Batches[2].Steps[0].State);
			Assert.AreEqual(JobStepState.Completed, job.Batches[3].Steps[0].State);
			Assert.AreEqual(JobStepState.Completed, job.Batches[4].Steps[0].State);
		}

		[TestMethod]
		public async Task TestStatesCompleteWhenFailureOfOptionalAsync()
		{
			Mock<ITemplate> templateMock = new(MockBehavior.Strict);
			templateMock.SetupGet(x => x.InitialAgentType).Returns((string?)null);
			...
			job = await RunBatchAsync(job, 1);
			job = await RunStepAsync(job, 1, 0, JobStepOutcome.Failure); // A
		...
		[TestMethod]
		public async Task TestStatesCompleteWithMixOfRequiredAndOptionalAsync()
		{
			Mock<ITemplate> templateMock = new(MockBehavior.Strict);
			templateMock.SetupGet(x => x.InitialAgentType).Returns((string?)null);
			...
			NewGroup groupB = AddGroup(newGroups);
			AddNode(groupB, "Node B", null);

			NewGroup groupC = AddGroup(newGroups);
			AddNode(groupC, "Node C", ["Node A"], ["Node B"]);
			...
			Assert.AreEqual(JobStepState.Ready, job.Batches[3].Steps[0].State);
		...
		[TestMethod]
		public async Task TestStatesSkippedWithMixOfRequiredAndOptionalAsync()
		{
			Mock<ITemplate> templateMock = new(MockBehavior.Strict);
			templateMock.SetupGet(x => x.InitialAgentType).Returns((string?)null);
		...
		[TestMethod]
		public async Task TestStatesReadyWhenAgentAbortedOptionalAsync()
		{
			Mock<ITemplate> templateMock = new(MockBehavior.Strict);
			templateMock.SetupGet(x => x.InitialAgentType).Returns((string?)null);
			...
			CreateJobOptions options = new();
			options.Arguments.Add("-Target=Node B");
			...
			// Node A is started and cancelled by agent
			job = await RunBatchAsync(job, 1);
			job = Deref(await job.TryUpdateStepAsync(job.Batches[1].Id, job.Batches[1].Steps[0].Id, JobStepState.Running, JobStepOutcome.Success));
			job = Deref(await job.TryUpdateStepAsync(job.Batches[1].Id, job.Batches[1].Steps[0].Id, JobStepState.Aborted, JobStepOutcome.Failure));

			Assert.AreEqual(JobStepState.Aborted, job.Batches[1].Steps[0].State);
			Assert.IsFalse(job.Batches[1].Steps[0].AbortRequested);

			// Node B should still be ready since A is optional and not aborted by user
			Assert.AreEqual(JobStepState.Ready, job.Batches[2].Steps[0].State);
		}

		[TestMethod]
		public async Task TestStatesSkippedWhenUserAbortedOptionalAsync()
		{
			Mock<ITemplate> templateMock = new(MockBehavior.Strict);
			templateMock.SetupGet(x => x.InitialAgentType).Returns((string?)null);
			...
			// Node A is started and cancelled by user request
			job = await RunBatchAsync(job, 1);
			job = Deref(await job.TryUpdateStepAsync(job.Batches[1].Id, job.Batches[1].Steps[0].Id, JobStepState.Running, JobStepOutcome.Success));
			job = Deref(await job.TryUpdateStepAsync(job.Batches[1].Id, job.Batches[1].Steps[0].Id, newAbortRequested: true));

			Assert.AreEqual(JobStepState.Running, job.Batches[1].Steps[0].State);
			Assert.IsTrue(job.Batches[1].Steps[0].AbortRequested);

			// Node B should be skipped since user explicitly aborted A even though A is optional
			Assert.AreEqual(JobStepState.Skipped, job.Batches[2].Steps[0].State);
		}

		[TestMethod]
		public async Task TestStatesCompleteWhenFailureOfChainedOptionalAsync()
		{
			Mock<ITemplate> templateMock = new(MockBehavior.Strict);
			templateMock.SetupGet(x => x.InitialAgentType).Returns((string?)null);
			...
			NewGroup groupC = AddGroup(newGroups);
			AddNode(groupC, "Node C", null, ["Node B"]);
		...
		[TestMethod]
		public async Task TestRetryWithOptionalDependencyAsync()
		{
			Mock<ITemplate> templateMock = new(MockBehavior.Strict);
			templateMock.SetupGet(x => x.InitialAgentType).Returns((string?)null);
			...
			Assert.AreEqual(JobStepState.Completed, job.Batches[2].Steps[0].State);
			Assert.AreEqual(JobStepOutcome.Failure, job.Batches[2].Steps[0].Outcome);

			Assert.AreEqual(3, job.Batches.Count);
			job = Deref(await job.TryUpdateStepAsync(job.Batches[2].Id, job.Batches[2].Steps[0].Id, newRetryByUserId: UserId.Anonymous));
			Assert.AreEqual(4, job.Batches.Count);
		}

		[TestMethod]
		public async Task TestRetrySkippedNodeWithAbortedOptionalDependencyAsync()
		{
			Mock<ITemplate> templateMock = new(MockBehavior.Strict);
			templateMock.SetupGet(x => x.InitialAgentType).Returns((string?)null);
			...
			// Node B has ONLY optional dependency on Node A
			NewGroup groupB = AddGroup(newGroups);
			AddNode(groupB, "Node B", null, ["Node A"]);
			...
			job = Deref(await job.TryUpdateStepAsync(job.Batches[0].Id, job.Batches[0].Steps[0].Id, newAbortRequested: true));
			job = Deref(await job.TryUpdateStepAsync(job.Batches[0].Id, job.Batches[0].Steps[0].Id, JobStepState.Aborted, JobStepOutcome.Failure));

			Assert.AreEqual(JobStepState.Aborted, job.Batches[0].Steps[0].State); // A
			Assert.AreEqual(JobStepState.Skipped, job.Batches[1].Steps[0].State); // B

			// Retry Node B
			job = Deref(await job.TryUpdateStepAsync(job.Batches[1].Id, job.Batches[1].Steps[0].Id, newRetryByUserId: UserId.Anonymous));

			// Nodes A and B to be retried
			Assert.AreEqual(3, job.Batches.Count);

			Assert.AreEqual(JobStepState.Ready, job.Batches[1].Steps[0].State); // A
			...
			Assert.AreEqual(JobStepState.Waiting, job.Batches[2].Steps[0].State); // B
		...
		[TestMethod]
		public async Task TestRetrySkippedNodeWithSkippedOptionalDependencyAsync()
		{
			Mock<ITemplate> templateMock = new(MockBehavior.Strict);
			templateMock.SetupGet(x => x.InitialAgentType).Returns((string?)null);
			...
			job = await RunBatchAsync(job, 0);
			job = await RunStepAsync(job, 0, 0, JobStepOutcome.Failure); // A

			Assert.AreEqual(3, job.Batches.Count);
			Assert.AreEqual(JobStepState.Completed, job.Batches[0].Steps[0].State); // A
			Assert.AreEqual(JobStepState.Skipped, job.Batches[1].Steps[0].State); // B
			Assert.AreEqual(JobStepState.Skipped, job.Batches[2].Steps[0].State); // C
			...
			Assert.AreEqual(JobStepState.Waiting, job.Batches[3].Steps[0].State); // C
			Assert.AreEqual(2, job.Batches[3].GroupIdx);
			Assert.AreEqual(1, job.Batches[3].Steps.Count);
			Assert.AreEqual(0, job.Batches[3].Steps[0].NodeIdx);
		}

		[TestMethod]
		public async Task TestRetryCompletedNodeDoesNotRetryOptionalDependencyAsync()
		{
			Mock<ITemplate> templateMock = new(MockBehavior.Strict);
			templateMock.SetupGet(x => x.InitialAgentType).Returns((string?)null);
			...
			job = await RunBatchAsync(job, 0);
			job = Deref(await job.TryUpdateStepAsync(job.Batches[0].Id, job.Batches[0].Steps[0].Id, JobStepState.Running, JobStepOutcome.Success));
			job = Deref(await job.TryUpdateStepAsync(job.Batches[0].Id, job.Batches[0].Steps[0].Id, JobStepState.Completed, JobStepOutcome.Failure));
			...
			Assert.IsFalse(job.Batches[0].Steps[0].AbortRequested);
			Assert.AreEqual(JobStepState.Ready, job.Batches[1].Steps[0].State);
			...
			Assert.AreEqual(JobStepState.Completed, job.Batches[1].Steps[0].State);
			Assert.AreEqual(JobStepOutcome.Failure, job.Batches[1].Steps[0].Outcome);
		...
		public async Task TestParentChildStepsAsync()
		{
			...
			Assert.AreEqual(1, parentJob.Batches[0].Steps[0].SpawnedJobs?.Count);
		...
		public async Task JobMetaDataAsync()
		{
			...
			Assert.AreEqual("TestMeta=42", job.Metadata[0]);

			Assert.AreEqual(1, job.Batches[0].Steps[0].Metadata.Count);
			Assert.AreEqual("TestStepMeta=2025", job.Batches[0].Steps[0].Metadata[0]);
			...
			Assert.AreEqual(1, jobStepRef.Metadata.Count);
			Assert.AreEqual("TestStepMeta=2025", jobStepRef.Metadata[0]);
		}
		...
		[TestMethod]
		public async Task RetryRunningStepShouldAbortOldStepAsync()
		{
			// Setup: Create a job with a single step
			List<NewGroup> newGroups = new List<NewGroup>();
			NewGroup group = AddGroup(newGroups);
			AddNode(group, "Step 1", null);
			...
			// Run the batch and start the step (but don't complete it)
			job = await RunBatchAsync(job, 0);
			job = Deref(await job.TryUpdateStepAsync(job.Batches[0].Id, job.Batches[0].Steps[0].Id, JobStepState.Running, JobStepOutcome.Success));

			Assert.AreEqual(JobStepState.Running, job.Batches[0].Steps[0].State);

			// Retry the RUNNING step
			job = Deref(await job.TryUpdateStepAsync(job.Batches[0].Id, job.Batches[0].Steps[0].Id, newRetryByUserId: UserId.Anonymous));

			// Verify: Old step should be marked for abort and a new batch should be created for the retry
			Assert.AreEqual(2, job.Batches.Count, "Should have old batch + new retry batch");
			Assert.IsTrue(job.Batches[0].Steps[0].AbortRequested, "Old running step should be aborted when retried");
		}

		[TestMethod]
		public async Task AbortRunningStepShouldSetBatchErrorAsync()
		{
			// Setup: Create a job with a single step
			List<NewGroup> newGroups = new List<NewGroup>();
			NewGroup group = AddGroup(newGroups);
			...
			// Run the batch and start the step
			job = await RunBatchAsync(job, 0);
			job = Deref(await job.TryUpdateStepAsync(job.Batches[0].Id, job.Batches[0].Steps[0].Id, JobStepState.Running, JobStepOutcome.Success));
			...
			job = Deref(await job.TryUpdateStepAsync(job.Batches[0].Id, job.Batches[0].Steps[0].Id, newAbortRequested: true, newAbortByUserId: UserId.Anonymous));

			// Verify: Step is aborted and batch error is set
			Assert.IsTrue(job.Batches[0].Steps[0].AbortRequested, "Step should have AbortRequested=true");
			Assert.AreEqual(JobStepBatchError.Cancelled, job.Batches[0].Error, "Batch error should be Cancelled when running step is aborted");
		}

		[TestMethod]
		public async Task RetryRunningStepShouldMarkOldBatchAsNoLongerNeededAsync()
		{
			// Setup: Create a job with a single step
			List<NewGroup> newGroups = new List<NewGroup>();
			NewGroup group = AddGroup(newGroups);
			...
			// Verify: Old batch should be marked NoLongerNeeded
			Assert.AreEqual(2, job.Batches.Count, "Should have old batch + new retry batch");
			Assert.AreEqual(JobStepBatchError.NoLongerNeeded, job.Batches[0].Error, "Old batch should be marked NoLongerNeeded when step is retried");
			Assert.AreEqual(JobStepBatchError.None, job.Batches[1].Error, "New batch should have no error");
		}

		[TestMethod]
		public async Task UpdateGraphWhileBatchRunningPreservesDependenciesAsync()
		{
			// Regression test: When UpdateGraphForHorde runs while a batch is already executing
			// (due to RunEarly), nodes added to that batch with dependencies on nodes in new
			// (later) batches must stay in Waiting state. Previously, RefreshDependentJobSteps
			// built the stepForNode dictionary incrementally during iteration, so dependencies
			// in later batches were invisible and the step was incorrectly marked Ready.
			...
			// Initial graph: single group with a RunEarly node
			List<NewGroup> initialGroups = [];
			NewGroup cookGroup = AddGroup(initialGroups);
			AddNode(cookGroup, "Cook", null, n => { n.RunEarly = true; });

			IGraph graph1 = await GraphCollection.AppendAsync(baseGraph, initialGroups);

			CreateJobOptions options = new CreateJobOptions();
			options.Arguments.Add("-Target=Cook");
			options.Arguments.Add("-Target=Download Artifacts");

			IJob job = await JobCollection.AddAsync(JobIdUtils.GenerateNewId(), _streamId, _templateId1, ContentHash.SHA1("hello"), baseGraph, "Test job", CommitIdWithOrder.FromPerforceChange(123), CommitIdWithOrder.FromPerforceChange(123), options);

			job = await RunBatchAsync(job, 0); // Setup Build
			job = await RunStepAsync(job, 0, 0, JobStepOutcome.Success); // Setup Build

			job = Deref(await job.TryUpdateGraphAsync(graph1));

			// Cook batch should be Ready due to RunEarly (Setup Build + Cook)
			Assert.HasCount(2, job.Batches);
			Assert.AreEqual(JobStepBatchState.Ready, job.Batches[1].State); // Cook batch

			// Start the Cook batch and step (simulating RunEarly agent pickup)
			job = await RunBatchAsync(job, 1);
			job = Deref(await job.TryUpdateStepAsync(job.Batches[1].Id, job.Batches[1].Steps[0].Id, JobStepState.Running, JobStepOutcome.Success));
			Assert.AreEqual(JobStepBatchState.Running, job.Batches[1].State);
			Assert.AreEqual(JobStepState.Running, job.Batches[1].Steps[0].State);

			// Now build an updated graph that adds a new node in a new group (Upload Artifacts)
			// and a dependent node in Cook's group. The Upload Artifacts group must be declared
			// first so the graph can resolve the dependency reference; but at runtime the
			// new batch for Upload Artifacts will be appended AFTER the existing Running batch,
			// which is the core of the bug.
			List<NewGroup> updatedGroups = [];

			NewGroup uploadGroup = AddGroup(updatedGroups);
			AddNode(uploadGroup, "Upload Artifacts", null);

			NewGroup cookGroupUpdated = AddGroup(updatedGroups);
			AddNode(cookGroupUpdated, "Cook", null, n => { n.RunEarly = true; });
			AddNode(cookGroupUpdated, "Download Artifacts", ["Upload Artifacts"]);

			IGraph graph2 = Deref(await GraphCollection.AppendAsync(baseGraph, updatedGroups));

			// Update the graph while the Cook batch is Running
			job = Deref(await job.TryUpdateGraphAsync(graph2));

			Assert.HasCount(3, job.Batches);
			// Upload Artifacts has no dependencies and should be Ready.
			Assert.AreEqual(JobStepState.Ready, job.Batches[2].Steps[0].State);
			// Download Artifacts should remain Waiting until Upload Artifacts completes.
			// If this is Ready, the dependency was silently dropped during graph update.
			Assert.AreEqual(JobStepState.Waiting, job.Batches[1].Steps[1].State);
		}
	}
}
