...
namespace HordeServer.Tests.Jobs
{
	...
	public class JobServiceTests : BuildTestSetup
	{
		...
		public async Task TestJobTemplateSettingsAsync()
		{
			...
			Assert.AreEqual(2, settings.JobTemplateSettings!.Count);
			...
			Assert.AreEqual("-Target=targetd", templateSettings.Arguments[0]);
			...
			Assert.IsNotNull(templateSettings);
			Assert.AreEqual("-Target=targete", templateSettings.Arguments[0]);
		}
