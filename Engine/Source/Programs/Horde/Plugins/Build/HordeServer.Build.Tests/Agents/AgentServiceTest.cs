...
public class AgentServiceTest : BuildTestSetup
{
	...
	[TestMethod]
	public async Task ConformRemovedAfterUpdateWorkspacesAsync()
	{
		Fixture fixture = await CreateFixtureAsync();
		AgentId agentId = new(fixture.Agent1Name);

		IAgent agent = Deref(await AgentService.GetAgentAsync(agentId));
		agent = Deref(await agent.TryUpdateAsync(new UpdateAgentOptions(RequestFullConform: true)));
		Assert.IsTrue(agent.RequestFullConform);

		await AgentService.TryUpdateWorkspacesAsync(agent, agent.Workspaces.ToList(), false, CancellationToken.None);

		agent = Deref(await AgentService.GetAgentAsync(agentId));
