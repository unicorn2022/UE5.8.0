...
public class AgentRelayServiceTests : BuildTestSetup
{
	...
	public AgentRelayServiceTests()
	{
		...
		_service.SetRedisChannel($"relay/update/{Guid.NewGuid()}");
	}
	...
	public async Task PortAssignment_OutOfPorts_Async()
	{
		...
		await AddPortMappingAsync("cluster1", _pm1.LeaseId, null, _pm1.AgentIp, _pm1.Ports);
		await AddPortMappingAsync("cluster1", _pm3.LeaseId, null, _pm3.AgentIp, _pm3.Ports);
		await Assert.ThrowsExactlyAsync<ArgumentException>(() => AddPortMappingAsync("cluster1", _pm2.LeaseId, null, _pm2.AgentIp, _pm2.Ports));
	}
	...
	public void FindAvailablePorts()
	{
		...
		Assert.ThrowsExactly<ArgumentException>(() => Actual(8, [2, 1, 9]));
		
		Exception e1 = Assert.ThrowsExactly<ArgumentException>(() => Actual(11));
