...
public class PoolUpdateServiceTest : BuildTestSetup
{
	...
	public async Task SetupAsync()
	{
		...
		_pus = new PoolUpdateService(AgentService, PoolService, PoolCollection, Clock, ServiceProvider.GetRequiredService<IOptionsMonitor<BuildConfig>>(), ServiceProvider.GetRequiredService<IOptionsMonitor<ComputeConfig>>(), Tracer, new NullLogger<PoolUpdateService>());
	...
	private async Task<IAgent> CreateAutoConformAgentAsync(int freeDiskSpaceMb, params int?[] autoConformThresholdsMb)
	{
		...
		IAgent? agent = await CreateAgentAsync(_pool, properties: [$"{KnownPropertyNames.WorkingDiskFreeSpace}={freeDiskSpaceMb * 1024 * 1024}"]);
	...
	[TestMethod]
	public async Task UpdatePools_V1_UpdatesAllPoolsRegardlessAsync()
	{
		await UpdateConfigAsync(config =>
		{
			config.Version = (int)ConfigVersion.Initial;
			config.Plugins.GetBuildConfig().Projects.Clear();
			config.Plugins.GetBuildConfig().Projects.Add(new ProjectConfig
			{
				Streams =
				[
					new StreamConfig
					{
						WorkspaceTypes = new Dictionary<string, WorkspaceConfig>
						{
							["ws3"] = new WorkspaceConfig { Identifier = "ws3" }
						},
						AgentTypes = new Dictionary<string, AgentConfig>
						{
							["at3"] = new AgentConfig { Pool = _pool.Id, Workspace = "ws3" }
						}
					}
				]
			});
		...
		IPool? pool = await PoolCollection.GetAsync(_pool.Id);
		Assert.IsNotNull(pool);
		Assert.AreEqual(1, pool.Workspaces.Count);
	}
