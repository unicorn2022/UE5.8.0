...
namespace HordeServer.Tests.Artifacts
{
	...
	public class ArtifactTests : BuildTestSetup
	{
		...
		[TestMethod]
		public async Task EnableArtifactExpirationFalseBlocksExpiryAsync()
		{
			FakeClock clock = ServiceProvider.GetRequiredService<FakeClock>();
			ArtifactCollection expirationService = ServiceProvider.GetRequiredService<ArtifactCollection>();
			...
			// Configure artifact type with 1 day keep and set the safety valve
			await UpdateConfigAsync(config =>
			{
				config.Plugins.GetBuildConfig().ArtifactTypes.Add(new ArtifactTypeConfig { Type = type, KeepDays = 1 });
				StreamConfig stream = config.Plugins.GetBuildConfig().Projects[0].Streams[0];
				stream.RetentionOptions.EnableArtifactExpiration = false;
			});

			await expirationService.StartAsync(CancellationToken.None);

			IArtifactCollection artifactCollection = ServiceProvider.GetRequiredService<IArtifactCollection>();
			IArtifactBuilder artifact = await artifactCollection.CreateAsync(new ArtifactName("default"), type, null, streamId, CommitIdWithOrder.FromPerforceChange(1), new string[] { "test1" }, Array.Empty<string>());

			// Advance well past KeepDays — artifact must survive
			await clock.AdvanceAsync(TimeSpan.FromDays(5.0));

			List<IArtifact> artifacts = await artifactCollection.FindAsync(streamId, keys: new[] { "test1" }).ToListAsync();
			Assert.AreEqual(1, artifacts.Count, "Artifact should not be expired when EnableArtifactExpiration == false");
		}
