...
namespace HordeServer.VersionControl.Perforce
{
	...
	{
		/// <summary>
		/// Max number of files in a changelist to query for, or store
		/// This is to prevent performance issues with large merge commits with lots of files.
		/// </summary>
		protected const int MaxFiles = 1000;
		...
		protected class PooledConnectionHandle : IPooledPerforceConnection
		{
			...
			/// <inheritdoc/>
			public Tracer Tracer => _inner.Perforce.Tracer;
		...
		public string Name { get; } = "Perforce";
		...
		async Task<PooledConnectionHandle> ConnectAsync(PerforceCluster cluster, string? userName, CancellationToken cancellationToken)
		{
			...
			if (handle == null)
			{
				...
				if (!String.IsNullOrEmpty(credentials.Password) && String.IsNullOrEmpty(credentials.Ticket))
				{
					...
					{
						_logger.LogError(ex, "Unable to login: {Message}", ex.Message);
						handle.Dispose();
						throw;
					}
				}
		...
		protected async ValueTask<ICommit> CreateCommitAsync(IPooledPerforceConnection perforce, StreamConfig streamConfig, DescribeRecord describeRecord, bool hasAllFiles, InfoRecord serverInfo, CancellationToken cancellationToken, List<string>? preComputedFiles = null)
		{
			DateTime timeUtc = describeRecord.Time.Kind == DateTimeKind.Utc ? describeRecord.Time : new DateTime(describeRecord.Time.Ticks - serverInfo.TimeZoneOffsetSecs * TimeSpan.TicksPerSecond, DateTimeKind.Utc);
			...
			List<string> files;
			if (preComputedFiles != null)
			{
				files = preComputedFiles;
			}
			else
			{
				files = await perforce.GetStreamFilesAsync(streamConfig, describeRecord, cancellationToken);
		...
		{
			DateTime timeUtc = changesRecord.Time.Kind == DateTimeKind.Utc ? changesRecord.Time : new DateTime(changesRecord.Time.Ticks - serverInfo.TimeZoneOffsetSecs * TimeSpan.TicksPerSecond, DateTimeKind.Utc);
		...
		protected class CommitSource : ICommitCollection
		{
			...
			public IAsyncEnumerable<ICommit> FindAsync(CommitId? minChange = null, bool includeMinChange = true, CommitId? maxChange = null, bool includeMaxChange = true, int? maxResults = null, IReadOnlyList<CommitTag>? tags = null, CommitSortOrder sortOrder = CommitSortOrder.Descending, CancellationToken cancellationToken = default)
			{
				return FindAsync(minChange?.GetPerforceChange(), includeMinChange, maxChange?.GetPerforceChange(), includeMaxChange, maxResults, tags, sortOrder, cancellationToken);
			}

			public virtual async IAsyncEnumerable<ICommit> FindAsync(int? minChange, bool includeMinChange, int? maxChange, bool includeMaxChange, int? maxResults, IReadOnlyList<CommitTag>? tags, CommitSortOrder sortOrder, [EnumeratorCancellation] CancellationToken cancellationToken = default)
			{
				...
				using (PooledConnectionHandle perforce = await PerforceService.ConnectWithStreamClientAsync(StreamConfig, null, cancellationToken))
				{
					...
					// Apply sort order - Reverse flag gives oldest first (ascending)
					ChangesOptions changesOptions = ChangesOptions.IncludeTimes | ChangesOptions.LongOutput;
					if (sortOrder == CommitSortOrder.Ascending)
					{
						changesOptions |= ChangesOptions.Reverse;
					}
					...
					if (tags == null || tags.Count == 0)
					{
						...
						List<ChangesRecord> changes = await perforce.GetChangesAsync(changesOptions, maxResults ?? -1, ChangeStatus.Submitted, filter, cancellationToken);
						foreach (ChangesRecord change in changes)
						{
					...
					{
						// Do the query in batches, navigating in the appropriate direction based on sort order
						ChangesOptions tagChangesOptions = sortOrder == CommitSortOrder.Ascending ? ChangesOptions.Reverse : ChangesOptions.None;
						...
						while (numResults > 0)
						{
							...
							List<ChangesRecord> changesRecords = await perforce.GetChangesAsync(tagChangesOptions, null, BatchSize, ChangeStatus.Submitted, null, filter, cancellationToken);
							...
							// Navigate in the appropriate direction for the next batch
							if (sortOrder == CommitSortOrder.Ascending)
							{
								minChange = changesRecords[^1].Number + 1;
							}
							else
							{
		...
		/// The user collection.
		/// </summary>
		protected IUserCollection UserCollection => _userCollection;
	}
}
