...
namespace HordeServer.VersionControl.Perforce
{
	...
	class PerforceServiceCache : PerforceService, IHostedService
	{
		...
		sealed class ClusterTicker : IAsyncDisposable
		{
			...
			// Volatile ensures the reference assignment is immediately visible to reader threads. The list contents are never mutated after assignment.
			private volatile List<StreamInfo> _streams;
		...
		class CachedCommitDoc : ICommit
		{
			...
			[BsonElement("haf")]
			public bool HasAllFiles { get; set; } = true;
			...
			public CachedCommitDoc(ICommit commit, List<CommitTag> commitTags, List<string> files, bool hasAllFiles)
			{
			...
			{
				List<CommitTag> commitTags = new(await commit.GetTagsAsync(cancellationToken));
				List<string> files = new(await commit.GetFilesAsync(null, MaxFiles, cancellationToken));
				bool hasAllFiles = files.Count < MaxFiles;
				return new CachedCommitDoc(commit, commitTags, files, hasAllFiles);
			}
			...
			public async ValueTask<IReadOnlyList<string>> GetFilesAsync(int? minFiles, int? maxFiles, CancellationToken cancellationToken)
			{
				...
				// The cached commit document does not have a complete list of files for the CL because it exceeded max files to be saved
				// Query Perforce server for the complete list and return that, but do not store it
				using IPooledPerforceConnection perforce = await _owner.ConnectAsync(_streamConfig.ClusterName, null, cancellationToken);
				DescribeRecord describeRecord = await perforce.DescribeAsync(DescribeOptions.None, maxFiles ?? -1, Number, cancellationToken);
				return await perforce.GetStreamFilesAsync(_streamConfig, describeRecord, cancellationToken);
			}
		}
			...
			try
			{
				...
				for (; ; )
				{
					if (!_downtimeService.IsDowntimeActive)
					{
						...
						// Start any new tickers that don't exist yet, or update existing ones
						foreach ((string clusterName, List<StreamInfo> streamInfos) in clusterToStreams)
						{
							...
							{
								// Update existing ticker's stream list (includes refreshed Views from P4 stream changes)
								if (!clusterToTicker[clusterName].Streams.Select(x => x.Hash).SequenceEqual(streamInfos.Select(x => x.Hash)))
							{
								_logger.LogInformation("Refreshing stream definitions for cluster {ClusterName}", clusterName);
							}
								clusterToTicker[clusterName].Streams = streamInfos;
							}
						}
					}
		...
		async Task<Dictionary<string, List<StreamInfo>>> CreateStreamInfoAsync(CancellationToken cancellationToken)
		{
			...
			foreach (IGrouping<string, StreamConfig> group in streams.Where(x => x.VCS.Equals("Perforce", StringComparison.OrdinalIgnoreCase)).GroupBy(x => x.ClusterName, StringComparer.OrdinalIgnoreCase))
			{
				try
				{
		...
		internal async Task UpdateClusterInternalAsync(string clusterName, List<StreamInfo> streamInfos, SpanContext? parentContext, CancellationToken cancellationToken)
		{
			using TelemetrySpan span = _tracer.StartActiveSpan($"{nameof(PerforceServiceCache)}.{nameof(UpdateClusterInternalAsync)}", parentContext: parentContext ?? default);
					...
					foreach (PerforceResponse<DescribeRecord> describeRecordResponse in describeRecordResponses)
					{
						...
						if (files.Count > 0)
						{
							try
							{
								...
								ICommit commit = await CreateCommitAsync(perforce, streamInfo.StreamConfig, describeRecord, hasAllFiles, info, cancellationToken, files.ToList());
								...
								List<string> cappedFiles = files.Take(MaxFiles).ToList();
								CachedCommitDoc commitDoc = new(commit, commitTags.ToList(), cappedFiles, hasAllFiles);
								...
								// Emit telemetry
								if (!streamInfo.StreamConfig.TelemetryStoreId.IsEmpty)
								{
									try
									{
										CommitTelemetry newTelemetry = await CommitTelemetry.CreateCommitTelemetryWithUserIdAsync(UserCollection, commit, CommitDescriptionTagContainer.Create(commitDoc.Description));
										_telemetryWriter.WriteEvent(streamInfo.StreamConfig.TelemetryStoreId, newTelemetry);
									}
									catch (Exception ex)
									{
										_logger.LogError(ex, "Failed to publish Commit telemetry.");
									}
								}
							}
							catch (PerforceException ex)
							{
								// We want to make sure a single perforce exception doesn't create batch abandonment for other streams, and records.
								// We are currently keeping this narrow to PerforceException to prevent too wide a capture, in the event there is something more systemic.
								_logger.LogError(ex, "Skipping CL {Change} for stream {StreamId}: {Message}", describeRecord.Number, streamInfo.StreamConfig.Id, ex.Message);
							}
						}
					}
				}
...
#pragma warning restore CA1308 // Normalize strings to uppercase
		...
		class CachedCommitSource : CommitSource
		{
			...
			public override async IAsyncEnumerable<ICommit> FindAsync(int? minChange = null, bool includeMinChange = true, int? maxChange = null, bool includeMaxChange = true, int? maxResults = null, IReadOnlyList<CommitTag>? tags = null, CommitSortOrder sortOrder = CommitSortOrder.Descending, [EnumeratorCancellation] CancellationToken cancellationToken = default)
			{
				...
				using TelemetrySpan span = _owner._tracer.StartActiveSpan($"{nameof(Perforce.PerforceService)}.{nameof(CachedCommitSource)}.{nameof(FindAsync)}");
				...
				span.SetAttribute("sortOrder", sortOrder.ToString());
				...
				if (state.Clusters.TryGetValue(StreamConfig.ClusterName, out ClusterState? clusterState))
				{
					...
					if (clusterState.MinChanges.TryGetValue(StreamConfig.Id, out minReplicatedChange) && (maxChange == null || maxChange.Value > minReplicatedChange))
					{
						...
						IFindFluent<CachedCommitDoc, CachedCommitDoc> query = _owner._commits.Find(filter);
						query = sortOrder == CommitSortOrder.Ascending
							? query.SortBy(x => x.Number)
							: query.SortByDescending(x => x.Number);
						using (IAsyncCursor<CachedCommitDoc> cursor = await query.Limit(maxResults).ToCursorAsync(cancellationToken))
						{
						...
						if ((maxResults == null || maxResults.Value > 0) && maxChange > 0)
						{
							...
							await foreach (ICommit commit in base.FindAsync(minChange, true, maxChange, true, maxUnfilteredResults, null, sortOrder, cancellationToken))
							{
								...
								_owner._logger.LogDebug("Adding new cached commit for {StreamId} at change {Change} with {NumFiles} (hasAllFiles={HasAllFiles})",
									StreamConfig.Id, commit.Id, cachedCommit.Files.Count, cachedCommit.HasAllFiles);
				...
				if (maxResults == null || maxResults.Value > 0)
				{
					...
					await foreach (ICommit commit in base.FindAsync(minChange, true, maxChange, true, maxResults, tags, sortOrder, cancellationToken))
					{
						yield return commit;
					}
				}
