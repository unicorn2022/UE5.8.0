...
namespace HordeServer.Notifications.Sinks
{
	...
	/// </summary>
	public sealed class SlackNotificationSink : IHostedService, INotificationSink, IAvatarService, IRcaNotifier, IAsyncDisposable
	{
		...
		// Max attempts when looking up an existing triage thread for auto-RCA posting.
		const int RcaThreadLookupMaxAttempts = 4;

		// Base delay in seconds for exponential backoff between thread lookup retries (5s, 10s, 20s).
		const int RcaThreadLookupBaseDelaySecs = 5;
		...
		static readonly RedisSortedSetKey<int> s_escalateRootCauseAnalysis = "slack/escalate-rca";
		...
		/// <summary>
		/// Map of Slack user IDs to DM channel.
		/// </summary>
		private readonly MemoryCache _dmChannelCache = new MemoryCache(new MemoryCacheOptions() { SizeLimit = 1024, ExpirationScanFrequency = TimeSpan.FromMinutes(10) });
		...
		public SlackNotificationSink(IMongoService mongoService, IRedisService redisService, IssueService issueService, ICommitService commitService, IUserCollection userCollection, ILogCollection logCollection, IExternalIssueService externalIssueService, IWebHostEnvironment environment, IOptions<BuildServerConfig> settings, IClock clock, IServerInfo serverInfo, IOptionsMonitor<BuildConfig> buildConfig, ILogger<SlackNotificationSink> logger)
		{
			...
			_slackClient = SlackClient.Create(_settings.SlackToken, _logger);

			if (!String.IsNullOrEmpty(_settings.SlackAdminToken))
			{
				_adminSlackClient = SlackClient.Create(_settings.SlackAdminToken, _logger);
			}
		...
		public async ValueTask DisposeAsync()
		{
			...
			_userCache.Dispose();
			_dmChannelCache.Dispose();
			_slackClient.Dispose();
			_adminSlackClient?.Dispose();
		...
		async Task InviteUsersAsync(string channel, IEnumerable<UserId> userIds, bool inviteUsersAsAdmin, CancellationToken cancellationToken)
		{
			...
			foreach (string slackUserId in slackUserIds)
			{
				...
				if (errorCode != null)
				{
					...
					else if (!errorCode.Equals("already_in_channel", StringComparison.Ordinal))
					{
...
#pragma warning restore CA1505
			...
			if (!isSecondThread)
			{
				...
				if (isNew)
				{
					...
					else
					{
						...
						List<string> lowContextTypes = _buildConfig.CurrentValue.LowContextFingerprintTypes;
						bool isLowContextType = lowContextTypes.Any(prefix => span.Fingerprint.Type.StartsWith(prefix, StringComparison.Ordinal));

						if (suspectGroups.Length > 0
							&& suspectGroups.Length <= workflow.MaxMentions
							&& isLowContextType
							&& span.MaxSuspectRank != null
							&& span.MaxSuspectRank.Value == 0)
						{
							// PATH 0: Low-context type with all suspects tied at rank 0.
							// List suspects without @ mentions; post explanatory note.
							_issueService.Collection.GetLogger(issue.Id).LogInformation(
								"Suppressing @ mentions for issue with MaxSuspectRank={MaxSuspectRank} ({SuspectCount} suspects, fingerprint type: {FingerprintType})",
								span.MaxSuspectRank, suspectGroups.Length, span.Fingerprint.Type);

							List<string> suspectList = new List<string>();
							foreach (IGrouping<UserId, IIssueSuspect> suspectGroup in suspectGroups)
							{
								string name = await FormatNameAsync(suspectGroup.Key, cancellationToken);
								string changes = String.Join(", ", suspectGroup.Select(x => FormatChange(x.CommitId)));
								suspectList.Add($"{name} ({changes})");
							}
							...
							await _slackClient.PostMessageToThreadAsync(threadId,
								$"_This issue does not have enough context (files, symbols) to identify a specific suspect. Mentions suppressed._",
								cancellationToken: cancellationToken);
						}
						else if (suspectGroups.Length > 0 && suspectGroups.Length <= workflow.MaxMentions)
						{
							// PATH 1: @ mentions via FormatMentionAsync
							List<string> suspectList = new List<string>();
							foreach (IGrouping<UserId, IIssueSuspect> suspectGroup in suspectGroups)
							{
						...
						{
							// PATH 2: Swarm/Commits links only, notifyTriageAlias = true
							...
							else if (span.LastSuccess == null)
							{
								suspectMessage += $"No previous successes for span. See <{GetStepUrl(span.FirstFailure.JobId, span.FirstFailure.StepId)}|step history panel> and click on change buttons for CL ranges.";
							}
				...
				if ((issue.ResolvedAt != null && issue.DuplicateIssueId == null) && workflow.EscalateRootCauseAnalysisTimes.Count > 0)
				{
					DateTime escalateTime = issue.ResolvedAt.Value.AddMinutes(workflow.EscalateRootCauseAnalysisTimes[0]);
					if (await _redisService.GetDatabase().SortedSetAddAsync(s_escalateRootCauseAnalysis, issue.Id, (escalateTime - DateTime.UnixEpoch).TotalSeconds, SortedSetWhen.NotExists))
					{
						_logger.LogInformation("First escalation time for root cause analysis {IssueId} is {Time}", issue.Id, escalateTime);
					}
				}
				...
				if (issue.OwnerId != null && issue.NominatedById != null && issue.NominatedById != issue.OwnerId)
				{
					...
					// Invite the nominated user to the channel so they receive the notification
					if (_environment.IsProduction() && workflow.AllowMentions)
					{
						await InviteUsersAsync(state.Channel, new[] { issue.OwnerId.Value }, workflow.InviteRestrictedUsers, cancellationToken);
					}
				}
			}
		...
		/// <inheritdoc/>
		public async Task NotifyRcaCompleteAsync(int issueId, RcaResult result, CancellationToken cancellationToken)
		{
			// Global kill switch check
			if (!_buildConfig.CurrentValue.EnableAutoRcaSlackNotifications)
			{
				_logger.LogDebug("Skipping auto-RCA notification for issue {IssueId}: EnableAutoRcaSlackNotifications is disabled globally", issueId);
				return;
			}

			// Get issue details to find the workflow
			IIssueDetails? details = await _issueService.GetIssueDetailsAsync(issueId, cancellationToken);
			if (details == null || details.Spans.Count == 0)
			{
				_logger.LogDebug("Skipping auto-RCA notification for issue {IssueId}: issue not found or has no spans", issueId);
				return;
			}
			...
			if (workflowId == null)
			{
				_logger.LogDebug("Skipping auto-RCA notification for issue {IssueId}: no workflow annotation on span", issueId);
				return;
			}
			...
			{
				_logger.LogDebug("Skipping auto-RCA notification for issue {IssueId}: stream {StreamId} not found in config", issueId, span.StreamId);
				return;
			}
			if (!streamConfig.TryGetWorkflow(workflowId.Value, out WorkflowConfig? workflow))
			{
				_logger.LogDebug("Skipping auto-RCA notification for issue {IssueId}: workflow {WorkflowId} not found in stream {StreamId}", issueId, workflowId.Value, span.StreamId);
				return;
			}
			if (!workflow.AutoRcaSlackEnabled || String.IsNullOrEmpty(workflow.TriageChannel))
			{
				_logger.LogDebug("Skipping auto-RCA notification for issue {IssueId}: AutoRcaSlackEnabled={Enabled}, TriageChannel={Channel}",
					issueId, workflow.AutoRcaSlackEnabled, workflow.TriageChannel ?? "(empty)");
				return;
			}

			// Find the existing Slack thread (with exponential backoff in case thread creation is delayed)
			MessageStateDocument? state = null;
			for (int attempt = 0; attempt < RcaThreadLookupMaxAttempts && state == null; attempt++)
			{
				if (attempt > 0)
				{
					int delaySecs = RcaThreadLookupBaseDelaySecs * (1 << (attempt - 1));
					_logger.LogDebug("Auto-RCA thread lookup for issue {IssueId}: attempt {Attempt}/{MaxAttempts}, waiting {DelaySecs}s",
						issueId, attempt + 1, RcaThreadLookupMaxAttempts, delaySecs);
					await Task.Delay(TimeSpan.FromSeconds(delaySecs), cancellationToken);
				}
				state = await GetMessageStateAsync(workflow.TriageChannel, GetTriageThreadEventId(issueId), cancellationToken);
			}
			if (state == null)
			{
				_logger.LogWarning("No Slack thread found for issue {IssueId} after {MaxAttempts} attempts, skipping auto-RCA notification",
					issueId, RcaThreadLookupMaxAttempts);
				return;
			}

			// Post via idempotent one-time pattern
			string rcaEventId = $"issue_{issueId}_auto_rca";
			string rcaMessage = FormatAutoRcaMessage(result);
			await PostSingleMessageToThreadAsync(workflow.TriageChannel, rcaEventId, state.MessageId, rcaMessage, cancellationToken);
		}

		const int MaxRunnerUpsInSlack = 3;
		const int MaxReasonLength = 700;

		static string TruncateReason(string reason)
			=> reason.Length > MaxReasonLength ? reason[..MaxReasonLength] + "..." : reason;

		string FormatAutoRcaMessage(RcaResult result)
		{
			StringBuilder sb = new();
			sb.AppendLine(":robot_face: *Auto-RCA Suggestion*");
			sb.AppendLine("_:warning: This analysis was generated by AI and may be inaccurate. Use as a starting point for triage only._");

			if (result.CulpritCommitId != null)
			{
				sb.AppendLine($"Suspected culprit: {FormatChange(result.CulpritCommitId)}");
				if (!String.IsNullOrEmpty(result.Reason))
				{
					sb.AppendLine($"> {TruncateReason(result.Reason)}");
				}
			}

			if (result.RunnerUps != null && result.RunnerUps.Count > 0)
			{
				sb.AppendLine("Other suspects:");
				foreach (RcaRunnerUp runnerUp in result.RunnerUps.Take(MaxRunnerUpsInSlack))
				{
					string line = $"- {FormatChange(runnerUp.Commit)}";
					if (!String.IsNullOrEmpty(runnerUp.BriefReason))
					{
						line += $": {TruncateReason(runnerUp.BriefReason)}";
					}
					sb.AppendLine(line);
				}
				if (result.RunnerUps.Count > MaxRunnerUpsInSlack)
				{
					sb.AppendLine($"_...and {result.RunnerUps.Count - MaxRunnerUpsInSlack} more (see dashboard)_");
				}
			}
		...
		public async Task NotifyConfigUpdateAsync(ConfigUpdateInfo info, CancellationToken cancellationToken)
		{
			...
			if (ex != null)
			{
				// Exception stacktraces can be really deep, which can lead to parsing issues on the log receiving end (OpenTelemetry, Datadog, etc)
				_logger.LogInformation(
					"Sending config update failure notification: {Message}, Exception: {ExceptionType} Stacktrace: {Stacktrace}", 
					ex.Message,
					ex.GetType().FullName,
					ex.StackTrace?.Length > 500 ? ex.StackTrace[..500] : ex.StackTrace);
		...
		#region test health notification
		...
		{
			string eventId = GetTestHealthEventId(report);
			if (report.IsHealthy)
			{
				// make sure there is a thread to update if test is now healthy
			...
			if (carbonCopies?.Any() ?? false)
			{
				foreach (string user in carbonCopies)
				{
					UserId userId;
					if (UserId.TryParse(user, out userId))
					{
						ccUsers.Add(await FormatMentionAsync(userId, true, cancellationToken));
					}
				}
			}

			SlackMessage message = new SlackMessage();
			string testHealthDetailsUrlLink = new Uri(_serverInfo.DashboardUrl, $"test-automation?stream={report.StreamId}&view=1&health={report.TestId}").ToString();

			string title = GenerateTestHealthSlackMessageTitle(report, streamConfig, testHealthDetailsUrlLink);
			message.Blocks.Add(new SectionBlock(title));
			if (ccUsers.Any())
			{
				message.Blocks.Add(new SectionBlock(String.Join(" ", ccUsers)));
			}

			(MessageStateDocument state, bool isNew) = await SendOrUpdateMessageAsync(recipient, eventId, null, message, cancellationToken);
			SlackMessageId threadId = state.MessageId;
			if (!isNew)
			{
				if (report.IsHealthy)
				{
					// mark fixed
					SlackMessage fixedMessage = new SlackMessage();
					fixedMessage.Text = $"Test Health was restored to {report.State}.";
					await _slackClient.PostMessageToThreadAsync(threadId, fixedMessage, cancellationToken);
					await _slackClient.AddReactionAsync(threadId, "tick", cancellationToken);

					// delete message state
					await DeleteMessageStateAsync(recipient, eventId, cancellationToken);
				}
				else
				{
					// ping
					int daysSinceLastUpdate = (int)(DateTime.UtcNow - report.LastUpdateDateUtc).TotalDays;
					SlackMessage pingMessage = new SlackMessage();
					pingMessage.Text = report.State == report.PreviousState ?
							$"No health improvement has been recorded for {daysSinceLastUpdate} days. Consider disabling the Test or set it under Audit." : $"Test health has changed from {report.PreviousState} to {report.State}.";
					await _slackClient.PostMessageToThreadAsync(threadId, pingMessage, cancellationToken);
				}
			}
		}

		static string GenerateTestHealthSlackMessageTitle(ITestHealthReport report, StreamConfig streamConfig, string testHealthDetailsUrlLink)
		{
			string state = report.PreviousState is not null && report.IsHealthy ? report.PreviousState : report.State;
			string title = $"*[{streamConfig.Name}]* {RedBadge} test <{testHealthDetailsUrlLink}|*{report.TestName}* health> has degraded to *{state}*";
			if (report.IsHealthy)
			{
				// strike title
				title = $"~{title}~";
		...
		static string GetTestHealthEventId(ITestHealthReport report) => $"test-health-{report.Id}";
		...
		async ValueTask EscalateAsync(CancellationToken cancellationToken)
		{
			...
			await EscalateIssuesForKeyAsync(s_escalateRootCauseAnalysis, time, utcNow, "root cause analysis",
				EscalateSingleRootCauseAnalysisAsync, cancellationToken);
		}

		async ValueTask EscalateIssuesForKeyAsync(RedisSortedSetKey<int> key, double time, DateTime utcNow, string notificationName,
			Func<int, DateTime, string, CancellationToken, Task<double?>> escalateSingleIssueAsync, CancellationToken cancellationToken)
		{
			int[] issueIds = await _redisService.GetDatabase().SortedSetRangeByScoreAsync(key, 0, time);
			if (issueIds.Length > 0)
			{
				_logger.LogInformation("Escalating {NotificationName} for {Time} ({TimeSecs})", notificationName, utcNow, time);
				foreach (int issueId in issueIds)
				{
					cancellationToken.ThrowIfCancellationRequested();
					try
					{
						double? nextTime = await escalateSingleIssueAsync(issueId, utcNow, notificationName, cancellationToken);
						if (nextTime == null)
						{
							_logger.LogInformation("Cancelling escalation for {NotificationName} {IssueId}", notificationName, issueId);
							await _redisService.GetDatabase().SortedSetRemoveAsync(key, issueId);
						}
						else
						{
							_logger.LogInformation("Next escalation for {NotificationName} {IssueId} is at timestamp {Time}", notificationName, issueId, nextTime.Value);
							await _redisService.GetDatabase().SortedSetAddAsync(key, issueId, nextTime.Value);
						}
					}
					catch (SlackException ex)
					{
						if (ex.Message.Equals("channel_not_found", StringComparison.Ordinal))
						{
							_logger.LogWarning(ex, "Slack channel not found while escalating {NotificationName} {IssueId}; cancelling.", notificationName, issueId);
						}
						else
						{
							_logger.LogError(ex, "Slack exception while escalating {NotificationName} {IssueId}; cancelling.", notificationName, issueId);
						}
		...
		async Task<double?> EscalateSingleRootCauseAnalysisAsync(int issueId, DateTime utcNow, string notificationName, CancellationToken cancellationToken)
		{
			const string CanceledMessage = "Canceling escalation for {NotificationName} {IssueId} {Reason}";

			IIssue? issue = await _issueService.Collection.GetIssueAsync(issueId, cancellationToken);
			if (issue == null)
			{
				_logger.LogInformation(CanceledMessage, notificationName, issueId, "Cannot Find Issue");
				return null;
			}
			...
			if (spans.Count == 0)
			{
				_logger.LogInformation(CanceledMessage, notificationName, issueId, "Cannot Find Issue Spans");
				return null;
			}

			if (issue.ResolvedAt == null)
			{
				_logger.LogInformation(CanceledMessage, notificationName , issueId, "Issue Not Resolved");
				return null;
			}

			if (issue.DuplicateIssueId != null)
			{
				_logger.LogInformation(CanceledMessage, notificationName, issueId, "Duplicate Issue");
				return null;
			}

			UserId? ownerId = issue.RootCauseOwnerId ?? issue.OwnerId;
			if (ownerId == null || ownerId == UserId.Empty)
			{
				_logger.LogInformation(CanceledMessage, notificationName, issueId, "No Root Cause Owner");
				return null;
			}

			// RootCauseCategory is an optional input in the UI because the issue closer may not have the information 
			// when marking as fixed, but it is required to have a value for Root Cause Analysis to be considered closed.
			if (!String.IsNullOrEmpty(issue.RootCauseCategory))
			{
				_logger.LogInformation(CanceledMessage, notificationName, issueId, "RCA Completed");
				return null;
			}
			...
			if (workflowId == null)
			{
				_logger.LogInformation(CanceledMessage, notificationName, issueId, "Cannot Find Issue WorkflowID");
				return null;
			}

			if (!_buildConfig.CurrentValue.TryGetStream(span.StreamId, out StreamConfig? streamConfig))
			{
				_logger.LogInformation(CanceledMessage, notificationName, issueId, "Cannot Find Issue StreamConfig");
				return null;
			}
			if (!streamConfig.TryGetWorkflow(workflowId.Value, out WorkflowConfig? workflow))
			{
				_logger.LogInformation(CanceledMessage, notificationName, issueId, "Cannot Find Issue StreamConfig Workflow");
				return null;
			}
			if (String.IsNullOrEmpty(workflow.TriageChannel) || workflow.EscalateRootCauseAnalysisTimes.Count == 0)
			{
				_logger.LogInformation(CanceledMessage, notificationName, issueId, "No Channel or Escalation Times");
				return null;
			}

			if (!issue.IsMarkedFixed())
			{
				_logger.LogInformation(CanceledMessage, notificationName, issueId, "Issue Not Fixed");
				return null;
			}
			...
			if (state == null)
			{
				_logger.LogInformation(CanceledMessage, notificationName, issueId, "Cannot Find Issue Message State");
				return null;
			}

			if (!_buildConfig.CurrentValue.EnableIssueRootCauseAnalysisNotifications)
			{
				// When notifications are disabled still allow notifications to resume for issues if enabled later
				return GetNextEscalationTime(issue, issue.ResolvedAt.Value, utcNow, workflow.EscalateRootCauseAnalysisTimes, notificationName);
			}

			if (DateTime.Now.DayOfWeek == DayOfWeek.Saturday || DateTime.Now.DayOfWeek == DayOfWeek.Sunday)
			{
				_logger.LogInformation("Skipping escalation message for {NotificationName} {IssueId} since it's a weekend", notificationName, issueId);
			}
			else
			{
				string mention = await FormatMentionAsync(ownerId.Value, workflow.AllowMentions, cancellationToken);

				_logger.LogInformation("Sending escalation message for {NotificationName} {IssueId} {User} {Channel}", notificationName, issueId, mention, state.MessageId);

				Uri issueUrl = GetIssueUrl(issue, span.FirstFailure);
				string message = $"{mention} this root cause analysis requires your attention - Issue <{issueUrl}|{issue.Id}> - Select ...More > Modify Issue and choose a category to complete the root cause analysis.";
				await _slackClient.PostMessageToThreadAsync(state.MessageId, message, cancellationToken);
			}
		...
		double GetNextEscalationTime(IIssue issue, DateTime whence, DateTime utcNow, List<int> escalationTimes, string notificationName)
		{
			DateTime nextEscalationTime = whence;
			for (int idx = 0; ; idx++)
			{
				if (idx >= escalationTimes.Count)
				{
					nextEscalationTime = utcNow + TimeSpan.FromMinutes(escalationTimes[^1]);
					_logger.LogInformation("Next escalation time for {NotificationName} {IssueId} is {NextEscalationTime} (now + {Time}m [{Idx}/{Count}])",
						notificationName, issue.Id, nextEscalationTime, escalationTimes[^1], escalationTimes.Count, escalationTimes.Count);
					break;
				}

				DateTime prevEscalationTime = nextEscalationTime;
				nextEscalationTime += TimeSpan.FromMinutes(escalationTimes[idx]);

				if (nextEscalationTime > utcNow + TimeSpan.FromMinutes(5.0))
				{
					_logger.LogInformation("Next escalation time for {NotificationName} {IssueId} is {NextEscalationTime} ({PrevEscalationTime} + {Time}m [{Idx}/{Count}])",
						notificationName, issue.Id, nextEscalationTime, prevEscalationTime, escalationTimes[idx], idx + 1, escalationTimes.Count);
					break;
				}
			}
				...
				else if (String.Equals(payload.Type, "view_submission", StringComparison.Ordinal))
				{
					if (payload.View != null && payload.View.State != null && payload.View.CallbackId != null)
					{
						...
						if (TryMatch(payload.View.CallbackId, @"^issue_(\d+)_markfixed_([a-fA-F0-9]{24})$", out match))
						{
							...
							{
								fixCommitId = new CommitId(fixChangeStr);
								_logger.LogInformation("Marking issue {IssueId} fixed by user {UserId} in {Change}", issueId, resolvedById, fixCommitId);
							}

							UserId? rootCauseOwnerId = null;
							if (payload.View.State.TryGetValue("rootcause_owner", "rootcause_owner_action", out string? rootCauseOwnerStr))
							{
								if (UserId.TryParse(rootCauseOwnerStr, out UserId rcaId))
								{
									rootCauseOwnerId = rcaId;
								}
							}

							string? rootChange;
							CommitId rootCommit = new CommitId("");
							if (payload.View.State.TryGetValue("rootcause_cl", "rootcause_cl_action", out rootChange))
							{
								if (rootChange != null && rootChange.Length > 0)
								{
									if (Int32.TryParse(rootChange.Trim(), out int result))
									{
										rootCommit = CommitId.FromPerforceChange(result);
										_logger.LogInformation("Setting issue {IssueId} root cause change by user {UserId} to {Change}", issueId, userId, rootCommit);
									}
									else
									{
										_logger.LogInformation("Setting issue {IssueId} root cause change by user {UserId} not set to invalid {Change}", issueId, userId, rootChange);
									}
								}
							}

							string? rootCategory;
							if (payload.View.State.TryGetValue("rootcause_category", "rootcause_category_action", out rootCategory))
							{
								if (rootCategory != null && rootCategory.Length > 0)
								{
									_logger.LogInformation("Setting issue {IssueId} root cause category by user {UserId} to {Category}", issueId, userId, rootCategory);
								}
								else
								{
									rootCategory = "";
							...
							string? rootSummary;
							if (payload.View.State.TryGetValue("rootcause_summary", "rootcause_summary_action", out rootSummary))
							{
								if (rootSummary != null && rootSummary.Length > 0)
								{
									_logger.LogInformation("Setting issue {IssueId} root cause summary by user {UserId} to {Summary}", issueId, userId, rootSummary);
								}
								else
								{
									rootSummary = "";
							...
							int? dupeIssueId = null;
							if (payload.View.State.TryGetValue("rootcause_dupeid", "rootcause_dupeid_action", out string? dupeIdStr))
							{
								if (dupeIdStr != null && dupeIdStr.Length > 0)
								{
									if (Int32.TryParse(dupeIdStr.Trim(), out int result))
									{
										dupeIssueId = result;
										_logger.LogInformation("Setting issue {IssueId} dupliicate issue id by user {UserId} to {DupeId}", issueId, userId, dupeIssueId);
									}
									else
									{
										_logger.LogInformation("Setting issue {IssueId} duplicate issue id by user {UserId} not set to invalid {DupeId}, clearing", issueId, userId, dupeIdStr);
										dupeIssueId = -1;
									}
								}
							}
							...
							if (issue != null)
							{
								string curSummary = issue.Summary;
								bool markedDupe = curSummary.StartsWith("(Dupe) ", StringComparison.OrdinalIgnoreCase);
								if (!markedDupe && dupeIssueId.HasValue && dupeIssueId != -1)
								{
									curSummary = "(Dupe) " + curSummary;
								}
								else if (markedDupe && (!dupeIssueId.HasValue || dupeIssueId == -1))
								{
									curSummary = curSummary.Substring(7);
								}

								summary = curSummary;
							}

							// Note that the current issue OwnerId is passed in this payload to avoid reassigning the issue to the user resolving it, unintentionally.
							await _issueService.UpdateIssueAsync(issueId, summary: summary, ownerId: issue?.OwnerId ?? null, fixCommitId: fixCommitId, resolvedById: fixCommitId != null ? resolvedById : null, initiatedById: userId, duplicateIssueId: dupeIssueId, rootCommitId: rootCommit, rootCauseOwnerId: rootCauseOwnerId, rootCauseCategory: rootCategory, rootCauseSummary: rootSummary, cancellationToken: cancellationToken);
						}
		...
		async Task HandleIssueChannelResponseAsync(int issueId, string verb, string slackUserId, string triggerId, CancellationToken cancellationToken)
		{
			...
			else if (String.Equals(verb, "markfixed", StringComparison.Ordinal))
			{
				...
				if (issue.OwnerId != null && issue.OwnerId != user.Id)
				{
					...
					if (owner != null)
					{
						List<SlackOption> ownerOptions = new List<SlackOption>();
						ownerOptions.Add(new SlackOption("Me", user.Id.ToString()));
						ownerOptions.Add(new SlackOption($"Current owner ({owner.Name})", owner.Id.ToString()));

						RadioButtonGroupElement ownership = new RadioButtonGroupElement("fixed_by_action", ownerOptions);
						ownership.InitialOption = ownerOptions[0];
						view.AddInput("Fixed By:", ownership).BlockId = "fixed_by";
					}
				}

				InputBlock fixBlock = view.AddInput("Fix Changelist:", new PlainTextInputElement("fix_cl_action", placeholder: "Fix Changelist"));
				fixBlock.BlockId = "fix_cl";
				fixBlock.Hint = "Change which fixed this issue. Enter 0 to mark fixed, without a change";

				view.AddDivider();

				List<SlackOption> categoryOptions = new List<SlackOption>
				{
				   new SlackOption( "Asset/Content", "Asset/Content"),
				   new SlackOption( "Code", "Code"),
				   new SlackOption( "UI/Localization", "UI/Localization"),
				   new SlackOption( "Build/Deployment/Version", "Build/Deployment/Version"),
				   new SlackOption( "Configuration/Data", "Configuration/Data"),
				   new SlackOption( "Gameplay/Engine", "Gameplay/Engine"),
				   new SlackOption( "Backend/Server/Network", "Backend/Server/Network"),
				   new SlackOption( "Monitoring/No Code Change", "Monitoring/No Code Change"),
				   new SlackOption( "Unintended Side Effect", "Unintended Side Effect"),
				   new SlackOption( "Build Agent", "Build Agent"),
				   new SlackOption( "Change Backout", "Change Backout"),
				   new SlackOption( "Other", "Other"),
				};

				StaticSelectMenuElement categorySelect = new StaticSelectMenuElement("rootcause_category_action", "Select an Issue Category", categoryOptions);
				if (!String.IsNullOrEmpty(issue.RootCauseCategory))
				{
					SlackOption? existing = categoryOptions.FirstOrDefault(i => i.Value == issue.RootCauseCategory);
					if (existing != null)
					{
						// Only can get, issue with Epic Slack
						//categorySelect.InitialOption = existing;
					}
				}

				if (issue.DuplicateIssueId == null)
				{
					List<SlackOption> rootCauseOwnerOptions =
					[
						new SlackOption("Me", user.Id.ToString())
					];
					if (issue.RootCauseOwnerId != null && issue.RootCauseOwnerId != user.Id)
					{
						IUser? rootCauseOwner = await _userCollection.GetCachedUserAsync(issue.RootCauseOwnerId.Value, cancellationToken);
						if (rootCauseOwner != null)
						{
							rootCauseOwnerOptions.Add(new SlackOption($"Current root cause owner ({rootCauseOwner.Name})", rootCauseOwner.Id.ToString()));
						}
					}
					if (issue.OwnerId != null && issue.OwnerId != user.Id && issue.OwnerId != issue.RootCauseOwnerId)
					{
						IUser? owner = await _userCollection.GetCachedUserAsync(issue.OwnerId.Value, cancellationToken);
						if (owner != null)
						{
							rootCauseOwnerOptions.Add(new SlackOption($"Current owner ({owner.Name})", owner.Id.ToString()));
						}
					}
					if (issue.ResolvedById != null && issue.ResolvedById != user.Id && issue.ResolvedById != issue.OwnerId && issue.ResolvedById != issue.RootCauseOwnerId)
					{
						IUser? resolvedBy = await _userCollection.GetCachedUserAsync(issue.ResolvedById.Value, cancellationToken);
						if (resolvedBy != null)
						{
							rootCauseOwnerOptions.Add(new SlackOption($"Fixed by ({resolvedBy.Name})", resolvedBy.Id.ToString()));
						}
					}

					RadioButtonGroupElement rootCauseOwnership = new("rootcause_owner_action", rootCauseOwnerOptions);
					//rootCauseOwnership.InitialOption = rootCauseOwnerOptions[0];
					InputBlock rootCauseOwnerBlock = view.AddInput("Root Cause Owner", rootCauseOwnership);
					rootCauseOwnerBlock.BlockId = "rootcause_owner";
					rootCauseOwnerBlock.Optional = true;

					InputBlock selectBlock = view.AddInput("Root Cause Category:", categorySelect);
					selectBlock.BlockId = "rootcause_category";
					selectBlock.Optional = true;

					InputBlock summaryBlock = view.AddInput("Root Cause Summary:", new PlainTextInputElement("rootcause_summary_action", multiLine: true, placeholder: "Please enter a brief summary describing the root cause of this issue and any suggestions to prevent it", initialValue: issue?.RootCauseSummary));
					summaryBlock.BlockId = "rootcause_summary";
					summaryBlock.Optional = true;

					InputBlock changeBlock = view.AddInput("Root Cause Changelist:", new PlainTextInputElement("rootcause_cl_action", placeholder: "Root Cause Changelist", initialValue: issue?.RootCommitId?.Name));
					changeBlock.BlockId = "rootcause_cl";
					changeBlock.Hint = "Change which caused this issue, please only enter if certain";
					changeBlock.Optional = true;
				}

				InputBlock dupeBlock = view.AddInput("Duplicate Issue Id:", new PlainTextInputElement("rootcause_dupeid_action", placeholder: "Duplicate Issue Id", initialValue: issue?.DuplicateIssueId?.ToString()));
				dupeBlock.BlockId = "rootcause_dupeid";
				dupeBlock.Optional = true;

				if (issue?.DuplicateIssueId != null)
				{
					dupeBlock.Hint = "Issue ID of the source issue this issue duplicates.  Root cause inputs hidden for duplicate issue, leave blank and update to clear";
				}
				else
				{
					dupeBlock.Hint = "Issue ID of the source issue this issue duplicate. Enter 0 or an issue id to set this as a duplicate issue";
				}
		...
		/// <inheritdoc/>
		public async Task SendSessionConflictReportAsync(IReadOnlyList<(AgentId Id, int Count)> conflicts, CancellationToken cancellationToken)
		{
			if (conflicts.Count == 0 || String.IsNullOrEmpty(_settings.AgentNotificationChannel))
			{
				return;
			}

			_logger.LogInformation("Sending session conflict report ({Count} agents) to {Channel}", conflicts.Count, _settings.AgentNotificationChannel);

			const int NumItems = 10;
			StringBuilder message = new StringBuilder("*Session conflicts (last 12h):*\n");
			foreach ((AgentId agentId, int count) in conflicts.OrderByDescending(x => x.Count).ThenBy(x => x.Id.ToString()).Take(NumItems))
			{
				Uri agentUrl = new Uri(_serverInfo.DashboardUrl, $"agents?agentId={agentId}");
				message.Append($"\u2022 *<{agentUrl}|{agentId}>* \u2014 {count:N0} mismatches\n");
			}
			if (conflicts.Count > NumItems)
			{
				message.Append($"+ {conflicts.Count - NumItems} other(s)\n");
			}

			await _slackClient.PostMessageAsync(_settings.AgentNotificationChannel, message.ToString(), cancellationToken);
		}

		const string DeepLinkUrl = "https://slack.com/app_redirect";
		const int MaxDirectMessageUsers = 8;

		/// <inheritdoc/>
		public async Task<string?> GetDirectMessageLinkAsync(IReadOnlyList<UserId> userIds, CancellationToken cancellationToken)
		{
			if (userIds.Count == 0 || userIds.Count > MaxDirectMessageUsers)
			{
				_logger.LogWarning("Number of users is out of bound: {Count}. Slack supports 1 to 8 users for DM conversation.", userIds.Count);
				return null;
			}

			string?[] taskResults = await Task.WhenAll(userIds.Distinct().Select(u => GetSlackUserIdAsync(u, cancellationToken)));
			List<string> slackUserIds = taskResults.OfType<string>().ToList();
			if (slackUserIds.Count == 0)
			{
				_logger.LogWarning("Could not retrieve enough valid Slack user ids for DM conversation.");
				return null;
			}
			slackUserIds.Sort();
			string slackUserIdsString = String.Join(",", slackUserIds);

			string? channelId;
			if (!_dmChannelCache.TryGetValue(slackUserIdsString, out channelId))
			{
				channelId = await _slackClient.OpenDirectMessageAsync(slackUserIds, cancellationToken);
				if (channelId is null)
				{
					_logger.LogWarning("Could not open DM conversation for users {Users}.", slackUserIdsString);
					return null;
				}
				using (ICacheEntry entry = _dmChannelCache.CreateEntry(slackUserIdsString))
				{
					entry.SlidingExpiration = TimeSpan.FromMinutes(10.0);
			...
			return $"{DeepLinkUrl}?channel={channelId}";
		}

		async Task<string?> GetSlackUserIdAsync(UserId userId, CancellationToken cancellationToken)
		{
			IUser? user = await _userCollection.GetUserAsync(userId, cancellationToken);
			if (user == null)
			{
		...
		/// <inheritdoc/>
		public Task<string?> GetChannelLinkAsync(string channelId, CancellationToken cancellationToken)
		{
			return Task.FromResult<string?>($"{DeepLinkUrl}?channel={UrlEncoder.Default.Encode(channelId)}");
		}
	}
}
