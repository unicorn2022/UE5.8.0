...
namespace HordeServer.Notifications
{
	...
	public sealed class NotificationService : IHostedService, INotificationService, IAsyncDisposable
	{
		...
		/// <summary>
		/// Async notification tasks currently executing. Tracked so they can be awaited before shutting down
		/// </summary>
		...
		readonly SessionConflictService _sessionConflictService;
		...
		public async Task StopAsync(CancellationToken cancellationToken)
		{
			...
			await WaitForAsyncTasksToCompleteAsync(cancellationToken);
		}
		...
		private async Task WaitForAsyncTasksToCompleteAsync(CancellationToken ct)
		{
			Task[] allTasks;
			lock (_tasksLock)
			{
				allTasks = _tasks.ToArray();
			}

			if (allTasks.Length > 0)
			{
				_logger.LogInformation("Waiting for {Count} notification tasks to complete...", allTasks.Length);
				...
				{
					int incomplete = allTasks.Count(t => !t.IsCompleted);
					_logger.LogWarning("Stop timeout reached with {Count} notification tasks still in flight", incomplete);
				}
			}
		}
		...
		/// <summary>
		/// Notify all subscribers that a job step was scheduled
		/// </summary>
		/// <param name="pool">The pool job was scheduled in</param>
		/// <param name="poolHasAgentsOnline">Whether the pool has agents online</param>
		/// <param name="job">The job containing the step that has finished</param>
		/// <param name="graph">Graph of the job</param>
		...
		public async Task NotifyTestHealthReportAsync(ITestHealthReport report, string recipient, string[]? carbonCopies, CancellationToken cancellationToken)
		{
			foreach (INotificationSink sink in _sinks)
			{
				try
				{
					await sink.NotifyTestHealthReportAsync(report, recipient, carbonCopies, cancellationToken);
				}
				catch (Exception e)
				{
					_logger.LogError(e, "Failed sending test health report to {Recipient}", recipient);
				}
			}
		}
		...
		void EnqueueTask(Func<CancellationToken, Task> taskFunc)
		{
			Task task = Task.Run(async () =>
			{
				...
				{
					_logger.LogError(e, "Exception while executing notification task: {Message}", e.Message);
				}
			});
		...
		private async ValueTask TickEveryTwelveHoursAsync(CancellationToken cancellationToken)
		{
			...
			try
			{
				List<(AgentId Id, int Count)> conflicts = await _sessionConflictService.DrainConflictsAsync(cancellationToken);
				if (conflicts.Count > 0)
				{
					_logger.LogInformation("Drained {Count} agents with session conflicts", conflicts.Count);
					foreach (INotificationSink sink in _sinks)
					{
						try
						{
							await sink.SendSessionConflictReportAsync(conflicts, cancellationToken);
						}
						catch (Exception e)
						{
							_logger.LogError(e, "Failed sending session conflict report");
						}
					}
				}
			}
			...
			{
				_logger.LogError(e, "Failed draining session conflicts from Redis");
			}
		}
		...
		/// <inheritdoc/>
		/// <remarks>It will return the first non null output from the sinks ignoring the rest</remarks>
		public async Task<string?> GetDirectMessageLinkAsync(IReadOnlyList<UserId> userIds, CancellationToken cancellationToken)
		{
			int sinksLeft = _sinks.Count;
			foreach (INotificationSink sink in _sinks)
			{
				sinksLeft--;
				try
				{
					string? url = await sink.GetDirectMessageLinkAsync(userIds, cancellationToken);
					if (url is not null)
					{
						if (sinksLeft > 0)
						{
							_logger.LogInformation("A DM link was generated from {Sink} ignoring the remaining {Count} sink(s)", sink.GetType().Name, sinksLeft);
						}
						return url;
					}
				}
				...
				{
					string users = String.Join(", ", userIds.Select(x => x.ToString()));
					_logger.LogError(e, "Failed getting direct message link for users {Users}", users);
				}
			}
		...
		/// <inheritdoc/>
		/// <remarks>It will return the first non null output from the sinks ignoring the rest</remarks>
		public async Task<string?> GetChannelLinkAsync(string channel, CancellationToken cancellationToken)
		{
			int sinksLeft = _sinks.Count;
			foreach (INotificationSink sink in _sinks)
			{
				...
				try
				{
					string? url = await sink.GetChannelLinkAsync(channel, cancellationToken);
					if (url is not null)
					{
						if (sinksLeft > 0)
						{
							_logger.LogInformation("A Channel link was generated from {Sink} ignoring the remaining {Count} sink(s)", sink.GetType().Name, sinksLeft);
						}
						return url;
					}
				}
				...
				{
					_logger.LogError(e, "Failed getting channel link for channel {Channel}", channel);
				}
			}
