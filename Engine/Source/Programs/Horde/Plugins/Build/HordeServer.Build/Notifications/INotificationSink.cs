...
namespace HordeServer.Notifications
{
	...
	public interface INotificationSink
	{
		...
		/// <summary>
		/// Send a batched report of session conflicts detected in the last 12 hours
		/// </summary>
		/// <param name="conflicts">Agent IDs paired with their total mismatch count</param>
		/// <param name="cancellationToken">Cancellation token for the operation</param>
		Task SendSessionConflictReportAsync(IReadOnlyList<(AgentId Id, int Count)> conflicts, CancellationToken cancellationToken);

		/// <summary>
		/// Post a notification for test health report
		/// </summary>
		/// <param name="report"></param>
		...
		/// <param name="cancellationToken">Cancellation token for the operation</param>
		/// <returns></returns>
		Task NotifyTestHealthReportAsync(ITestHealthReport report, string recipient, string[]? carbonCopies, CancellationToken cancellationToken);

		/// <summary>
		/// Get Direct Message Link
		/// </summary>
		/// <param name="userIds"></param>
		/// <param name="cancellationToken">Cancellation token for the operation</param>
		/// <returns></returns>
		Task<string?> GetDirectMessageLinkAsync(IReadOnlyList<UserId> userIds, CancellationToken cancellationToken = default);
		...
		/// <param name="cancellationToken">Cancellation token for the operation</param>
		/// <returns></returns>
		Task<string?> GetChannelLinkAsync(string channel, CancellationToken cancellationToken = default);
	}
}
