...
namespace HordeServer.Agents.Pools
{
	...
	public sealed class PoolUpdateService : IHostedService, IAsyncDisposable
	{
		...
		/// Constructor
		/// </summary>
		public PoolUpdateService(AgentService agentService, PoolService poolService, IPoolCollection pools, IClock clock, IOptionsMonitor<BuildConfig> buildConfig, IOptionsMonitor<ComputeConfig> computeConfig, Tracer tracer, ILogger<PoolUpdateService> logger)
		{
		...
		internal async ValueTask AutoConformAgentsAsync(CancellationToken cancellationToken)
		{
			...
			{
				await ConformUtilities.AutoConformAgentAsync(agent, _poolService, _buildConfig.CurrentValue, _logger, cancellationToken);
			}
		}
		...
		internal async ValueTask UpdatePoolsAsync(CancellationToken cancellationToken)
		{
			...
			while (retryUpdate && !cancellationToken.IsCancellationRequested)
			{
				...
				if (_computeConfig.CurrentValue.VersionEnum >= ConfigVersion.PoolsInConfigFiles)
				{
					return;
				}
