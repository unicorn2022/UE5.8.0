...
namespace HordeServer.Issues
{
	class IssueCollection : IIssueCollection, ILogExtIssueProvider
	{
		...
		class IssueDocument : IIssue
		{
			...
			[BsonIgnore]
			public CommitId? RootCommitId 
			{
				get => (RootCommitName != null) ? new CommitId(RootCommitName) : null;
				set => RootCommitName = value?.Name;
			}

			[BsonElement("RootCommit"), BsonIgnoreIfNull]
			public string? RootCommitName { get; set; }
...
#pragma warning restore IDE0051
		...
		class IssueSpanDocument : IIssueSpan
		{
			...
			[BsonIgnoreIfNull]
			public int? MaxSuspectRank { get; set; }
			...
			int? IIssueSpan.MaxSuspectRank => MaxSuspectRank;
			...
			public IssueSpanDocument(int issueId, NewIssueSpanData newSpan)
			{
				...
				MaxSuspectRank = newSpan.MaxSuspectRank;
				IssueId = issueId;
			}
		}
		...
		sealed class IssueLock : IAsyncDisposable
		{
			...
			public IssueLock(RedisLock redisLock, string purpose, Tracer tracer, ILogger logger)
			{
				_redisLock = redisLock;
				_purpose = purpose;
				_telemetrySpan = tracer.StartActiveSpan("Holding Issue Lock").SetAttribute("horde.purpose", _purpose);
				_logger = logger;
			}
				...
				try
			{
				...
				{
						_logger.LogWarning("Issue lock '{Purpose}' held for {TimeSpan}s", _purpose, (int)_timer.Elapsed.TotalSeconds);
					_timer.Reset();
				}
				}
				finally
		...
		public IssueCollection(IMongoService mongoService, IRedisService redisService, IUserCollection userCollection, ICommitService commitService, IAuditLogFactory<int> auditLogFactory, ITelemetryWriter telemetryWriter, IOptionsMonitor<BuildConfig> buildConfig, Tracer tracer, ILogger<IssueCollection> logger)
		{
			...
			// This index is created, though we need to move away from these C# index declarations, so only documenting it here
			// issueSpanIndexes.Add(keys => keys.Ascending(x => x.StreamId).Ascending(x => x.ResolvedAt));
		...
		/// <inheritdoc/>
		public async Task<IAsyncDisposable> EnterCriticalSectionAsync(string purpose)
		{
			Stopwatch timer = Stopwatch.StartNew();
			TimeSpan nextNotifyTime = TimeSpan.FromSeconds(10.0);

			RedisLock issueLock = new(_redisService.GetDatabase(), "issues/lock");
			using (TelemetrySpan telemetrySpan = _tracer.StartActiveSpan("Wait for Issue Lock").SetAttribute("horde.purpose", purpose))
			{
				while (!await issueLock.AcquireAsync(TimeSpan.FromMinutes(1)))
				{
					if (timer.Elapsed > nextNotifyTime)
					{
						_logger.LogWarning("Waiting on lock '{Purpose}' over issue collection for {TimeSpan} ms", purpose, timer.ElapsedMilliseconds);
						nextNotifyTime *= 2;
					}
					await Task.Delay(TimeSpan.FromMilliseconds(100));
				}
			}
			return new IssueLock(issueLock, purpose, _tracer, _logger);
		}
		...
		async Task LogIssueChangesImplAsync(ILogger issueLogger, IssueDocument oldIssue, IssueDocument newIssue, CancellationToken cancellationToken)
		{
			...
			if (newIssue.RootCommitId != oldIssue.RootCommitId)
			{
				if (newIssue.RootCommitId == null)
				{
					issueLogger.LogInformation("Root commit for issue was cleared");
				}
				else
				{
					issueLogger.LogInformation("{Commit} marked as root commit for Issue", newIssue.RootCommitId);
				}
			}

			if (newIssue.RootCauseOwnerId != oldIssue.RootCauseOwnerId)
			{
				if (newIssue.RootCauseOwnerId == null)
				{
					issueLogger.LogInformation("Root cause owner was cleared");
				}
				else
				{
					issueLogger.LogInformation("Root cause owner is {UserName} ({UserId})", await GetUserNameAsync(newIssue.RootCauseOwnerId, cancellationToken), newIssue.RootCauseOwnerId);
				}
			}

			if (newIssue.RootCauseCategory != oldIssue.RootCauseCategory)
			{
				issueLogger.LogInformation("Changed root cause category to \"{Category}\"", newIssue.RootCauseCategory);
			}

			if (newIssue.RootCauseSummary != oldIssue.RootCauseSummary)
			{
				issueLogger.LogInformation("Changed root cause summary to \"{Summary}\"", newIssue.RootCauseSummary);
			}

			if (newIssue.DuplicateIssueId != oldIssue.DuplicateIssueId)
			{
				if (newIssue.DuplicateIssueId == null)
				{
					issueLogger.LogInformation("Duplicate Issue Id cleared");
				}
				else
				{
					issueLogger.LogInformation("Duplicate Issue Id set to {IssueId}", newIssue.DuplicateIssueId);
				}
			}
		...
		public async Task<IIssue?> TryUpdateIssueAsync(IIssue issue, UserId? initiatedByUserId, UpdateIssueOptions options, CancellationToken cancellationToken = default)
		{
			...
			{
				if (options.OwnerId != null && options.RootCauseOwnerId == null && issueDocument.RootCauseOwnerId == null)
				{
					options = options with { RootCauseOwnerId = options.OwnerId };
				}
			...
			if (options.RootCommitId != null)
			{
				if (String.IsNullOrEmpty(options.RootCommitId.Name))
				{
					updates.Add(Builders<IssueDocument>.Update.Unset(x => x.RootCommitName));
				}
				else
				{
					updates.Add(Builders<IssueDocument>.Update.Set(x => x.RootCommitName, options.RootCommitId.Name));
				}
			}

			if (options.RootCauseOwnerId != null)
			{
				if (options.RootCauseOwnerId.Value == UserId.Empty)
				{
					updates.Add(Builders<IssueDocument>.Update.Unset(x => x.RootCauseOwnerId));
				}
				else
				{
					updates.Add(Builders<IssueDocument>.Update.Set(x => x.RootCauseOwnerId, options.RootCauseOwnerId.Value));
				}
			}

			if (options.RootCauseCategory != null)
			{
				if (options.RootCauseCategory.Length == 0)
				{
					updates.Add(Builders<IssueDocument>.Update.Unset(x => x.RootCauseCategory));
				}
				else
				{
					updates.Add(Builders<IssueDocument>.Update.Set(x => x.RootCauseCategory, options.RootCauseCategory));
				}
			}

			if (options.RootCauseSummary != null)
			{
				if (options.RootCauseSummary.Length == 0)
				{
					updates.Add(Builders<IssueDocument>.Update.Unset(x => x.RootCauseSummary));
				}
				else
				{
					updates.Add(Builders<IssueDocument>.Update.Set(x => x.RootCauseSummary, options.RootCauseSummary));
				}
			}

			if (options.DuplicateIssueId != null)
			{
				if (options.DuplicateIssueId < 0)
				{
					updates.Add(Builders<IssueDocument>.Update.Unset(x => x.DuplicateIssueId));
				}
				else if (options.DuplicateIssueId != issue.Id)
				{
					updates.Add(Builders<IssueDocument>.Update.Set(x => x.DuplicateIssueId, options.DuplicateIssueId));
				}
			}
		...
		/// <inheritdoc/>
		public async Task<IIssueSpan?> TryUpdateSpanAsync(IIssueSpan span, NewIssueStepData? newLastSuccess = null, NewIssueStepData? newFailure = null, NewIssueStepData? newNextSuccess = null, List<NewIssueSpanSuspectData>? newSuspects = null, int? newMaxSuspectRank = null, int? newIssueId = null, CancellationToken cancellationToken = default)
		{
			...
			if (newMaxSuspectRank != null)
			{
				updates.Add(Builders<IssueSpanDocument>.Update.Set(x => x.MaxSuspectRank, newMaxSuspectRank.Value));
			}
			if (newIssueId != null)
			{
		...
		/// <inheritdoc/>
		public async Task<IReadOnlyList<int>> FindSpanIssueIdsAsync(StreamId streamId, CommitId? minCommitId, CommitId? maxCommitId, bool? resolved, CancellationToken cancellationToken)
		{
			FilterDefinition<IssueSpanDocument> filter = Builders<IssueSpanDocument>.Filter.Eq(x => x.StreamId, streamId);

			if (minCommitId != null)
			{
				CommitIdWithOrder minCommitIdWithOrder = await _commitService.GetOrderedAsync(streamId, minCommitId, cancellationToken);
				filter &= Builders<IssueSpanDocument>.Filter.Not(Builders<IssueSpanDocument>.Filter.Lt(x => x.MaxCommitOrder, minCommitIdWithOrder.Order));
			}
			if (maxCommitId != null)
			{
				CommitIdWithOrder maxCommitIdWithOrder = await _commitService.GetOrderedAsync(streamId, maxCommitId, cancellationToken);
				filter &= Builders<IssueSpanDocument>.Filter.Not(Builders<IssueSpanDocument>.Filter.Gt(x => x.MinCommitOrder, maxCommitIdWithOrder.Order));
			}
			...
			List<ProjectedIssueId> result = await _issueSpans.Aggregate().Match(filter).Group(x => x.IssueId, x => new ProjectedIssueId { _id = x.Key }).ToListAsync(cancellationToken);
			return result.Where(x => x._id != null).Select(x => x._id!.Value).ToList();
		}
