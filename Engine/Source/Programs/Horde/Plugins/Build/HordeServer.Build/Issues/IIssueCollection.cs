...
namespace HordeServer.Issues
{
	...
	public interface IIssueCollection
	{
		...
		/// </summary>
		/// <param name="purpose">Descriptive name for what purpose lock is held. Solely for logging and debugging</param>
		/// <returns>The disposable critical section</returns>
		Task<IAsyncDisposable> EnterCriticalSectionAsync(string purpose);
		...
		/// <param name="newMaxSuspectRank">New maximum suspect rank for the span</param>
		/// <param name="newIssueId">The new issue id for this span</param>
		/// <param name="cancellationToken">Cancellation token for the operation</param>
		/// <returns>The updated span, or null on failure</returns>
		Task<IIssueSpan?> TryUpdateSpanAsync(IIssueSpan span, NewIssueStepData? newLastSuccess = null, NewIssueStepData? newFailure = null, NewIssueStepData? newNextSuccess = null, List<NewIssueSpanSuspectData>? newSuspects = null, int? newMaxSuspectRank = null, int? newIssueId = null, CancellationToken cancellationToken = default);
		...
		/// <summary>
		/// Gets distinct issue ids from spans matching the given filter, using an aggregate pipeline to avoid transferring full span documents.
		/// </summary>
		/// <param name="streamId">The stream to query</param>
		/// <param name="minCommitId">Minimum commit id filter</param>
		/// <param name="maxCommitId">Maximum commit id filter</param>
		/// <param name="resolved">Include resolved or unresolved spans</param>
		/// <param name="cancellationToken">Cancellation token for the operation</param>
		/// <returns>List of distinct issue ids</returns>
		Task<IReadOnlyList<int>> FindSpanIssueIdsAsync(StreamId streamId, CommitId? minCommitId = null, CommitId? maxCommitId = null, bool? resolved = null, CancellationToken cancellationToken = default);
	...
	/// <param name="RootCommitId">Root changelist for the issue. Pass an empty value to clear the root changelist.</param>
	/// <param name="RootCauseOwnerId">New owner of the root cause</param>
	/// <param name="RootCauseCategory">Root cause category</param>
	/// <param name="RootCauseSummary">Root cause summary.</param>
	...
	public class NewIssueSpanData
	{
		...
		/// <inheritdoc cref="IIssueSpan.MaxSuspectRank"/>
