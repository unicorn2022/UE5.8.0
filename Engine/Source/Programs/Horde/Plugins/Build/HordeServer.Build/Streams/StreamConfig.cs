...
namespace HordeServer.Streams
{
	...
	public class StreamConfig
	{
		...
		/// <summary>
		/// Version control service for stream commits
		/// </summary>
		public string VCS { get; set; } = "Perforce";

		/// <summary>
		/// Repository name for version control systems that support multiple repositories
		/// </summary>
		public string? RepositoryName { get; set; }

		/// <summary>
		/// Default branch name for version control systems that use branches (e.g., "main").
		...
		/// <summary>
		/// Additional file paths (relative to workspace root) containing ignore patterns for the log parser.
		/// Each file should contain one regex per line, with lines starting with '#' treated as comments.
		/// Inherits from ProjectConfig and appends stream-level entries.
		/// </summary>
		[ConfigMergeStrategy(ConfigMergeStrategy.Append)]
		public List<string>? IgnorePatternFiles { get; set; }
		...
		/// <summary>
		/// Storage retention options for this stream
		/// </summary>
		public RetentionOptions RetentionOptions { get; set; } = new RetentionOptions();
		...
		/// <summary>
		/// Stream aliases configured for this stream.
		/// Each alias provides a named subset of the stream's templates.
		/// </summary>
		public List<StreamAliasConfig> Aliases { get; set; } = new List<StreamAliasConfig>();
		...
		public void PostLoad(StreamId id, ProjectConfig projectConfig, IEnumerable<ArtifactTypeConfig> parentArtifactTypes, ILogger? logger)
		{
			...
			RetentionOptions.MergeDefaults(projectConfig.RetentionOptions);
			...
			if (projectConfig.IgnorePatternFiles != null && projectConfig.IgnorePatternFiles.Count > 0)
			{
				if (IgnorePatternFiles == null)
				{
					IgnorePatternFiles = new List<string>(projectConfig.IgnorePatternFiles);
				}
				else
				{
					IgnorePatternFiles.InsertRange(0, projectConfig.IgnorePatternFiles);
				}
			}

			ConfigObject.MergeDefaults(AgentTypes.Select(x => (x.Key, x.Value.Base, x.Value)), logger);
			ConfigObject.MergeDefaults(WorkspaceTypes.Select(x => (x.Key, x.Value.Base, x.Value)), logger);
			ConfigObject.MergeDefaults(Templates.Select(x => (x.Id, x.Base, x)), logger);
...
#pragma warning restore CS0618 // Type or member is obsolete
	...
	/// </summary>
	public class StreamAliasConfig : IStreamAlias
	{
		/// <inheritdoc/>
		[Required]
		public string Name { get; set; } = String.Empty;

		/// <inheritdoc cref="IStreamAlias.Templates"/>
		[Required]
		public List<TemplateId> Templates { get; set; } = new List<TemplateId>();
		IReadOnlyList<TemplateId> IStreamAlias.Templates => Templates;
	}
