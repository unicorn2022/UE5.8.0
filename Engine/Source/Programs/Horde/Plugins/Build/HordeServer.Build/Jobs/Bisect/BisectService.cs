...
namespace HordeServer.Jobs.Bisect
{
	class BisectService : IHostedService
	{
		...
		readonly IOptions<BuildServerConfig> _staticConfig;
		readonly ILogger _logger;
		readonly ITicker _ticker;

		public BisectService(IBisectTaskCollection bisectTaskCollection, IJobCollection jobCollection, IJobStepRefCollection jobStepRefCollection, ITemplateCollectionInternal templateCollection, IGraphCollection graphCollection, ICommitService commitService, IOptionsMonitor<BuildConfig> buildConfig, IOptions<BuildServerConfig> staticConfig, IClock clock, ILogger<BisectService> logger)
		{
		...
		{
			if (_staticConfig.Value.DisableBisection)
			{
				return;
			}
		...
		async Task<BisectTaskState?> TryUpdateBisectTaskAsync(IBisectTask bisectTask, BuildConfig buildConfig, CancellationToken cancellationToken)
		{
			...
			List<ICommit> commits = await commitCollection.FindAsync(previousJobStepRef.CommitId, includeMinCommit: false, bisectTask.CurrentCommitId, includeMaxCommit: false, 100, bisectTask.CommitTags, CommitSortOrder.Descending, cancellationToken).ToListAsync(cancellationToken);
		...
		async Task<(IJobStepRef? Current, IJobStepRef? Next)> GetBisectRangeAsync(IBisectTask bisectTask, CancellationToken cancellationToken)
		{
			...
			{
				List<IJobStepRef> jobStepRefs = await _jobStepRefCollection.GetStepsForNodeAsync(bisectTask.StreamId, bisectTask.TemplateId, bisectTask.NodeName, bisectTask.InitialCommitId, true, maxJobCount, false, cancellationToken);
