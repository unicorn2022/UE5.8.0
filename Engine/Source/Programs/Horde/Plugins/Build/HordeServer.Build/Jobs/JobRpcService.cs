...
namespace HordeServer.Jobs
{
	...
	public class JobRpcService : JobRpc.JobRpcBase
	{
		...
		public override async Task<RpcGetJobResponse> GetJob(RpcGetJobRequest request, ServerCallContext context)
		{
			...
			response.Change = job.CommitId.TryGetPerforceChange() ?? 0;
			response.CodeChange = job.CodeCommitId?.TryGetPerforceChange() ?? 0;
			response.PreflightChange = job.PreflightCommitId?.TryGetPerforceChange() ?? 0;
			response.CommitId = job.CommitId.ToString();
			if (job.PreflightCommitId != null)
			{
				response.PreflightCommitId = job.PreflightCommitId.ToString();
			}
		...
		public override async Task<RpcBeginBatchResponse> BeginBatch(RpcBeginBatchRequest request, ServerCallContext context)
		{
			...
			bool expandedIgnorePatterns = _buildConfig.Value.EnableExpandedIgnorePatterns;
			response.EnableExpandedIgnorePatterns = expandedIgnorePatterns;
			if (expandedIgnorePatterns && streamConfig.IgnorePatternFiles != null)
			{
				response.IgnorePatternFiles.AddRange(streamConfig.IgnorePatternFiles);
			}
		...
		async Task<RpcBeginStepResponse?> TryBeginStepAsync(RpcBeginStepRequest request, Boxed<ILog?> log, ServerCallContext context)
		{
			...
			if (newJob != null)
			{
				...
				{
					INode optionalInputNode = graph.GetNode(optionalInput.NodeRef);
					response.OptionalInputs.Add($"{optionalInputNode.Name}/{optionalInputNode.OutputNames[optionalInput.OutputIdx]}");
				}
				...
				foreach (INodeGroup otherGroup in graph.Groups)
				{
					if (otherGroup != graph.Groups[batch.GroupIdx])
					{
						...
						foreach (NodeOutputRef outputRef in otherGroup.Nodes.SelectMany(x => x.OptionalInputs))
						{
							if (outputRef.NodeRef.GroupIdx == batch.GroupIdx && outputRef.NodeRef.NodeIdx == step.NodeIdx && !response.PublishOutputs.Contains(outputRef.OutputIdx))
							{
								response.PublishOutputs.Add(outputRef.OutputIdx);
		...
		public override async Task<RpcUpdateGraphResponse> UpdateGraph(RpcUpdateGraphRequest request, ServerCallContext context)
		{
			...
			for (; ; )
			{
				...
				foreach (RpcCreateGroupRequest group in request.Groups)
				{
					...
					{
						NewNode newNode = new NewNode(node.Name, node.Inputs.ToList(), node.OptionalInputs.ToList(), node.Outputs.ToList(), node.InputDependencies.ToList(), node.OptionalInputDependencies.ToList(), node.OrderDependencies.ToList(), (Priority)node.Priority, node.AllowRetry, node.RunEarly, node.Warnings, new Dictionary<string, string>(node.Credentials), new Dictionary<string, string>(node.Properties), new NodeAnnotations(node.Annotations));
		...
		public override async Task<RpcCreateReportResponse> CreateReport(RpcCreateReportRequest request, ServerCallContext context)
		{
			...
			JobReport newReport = new JobReport { Name = request.Name, Placement = (JobReportPlacement)request.Placement, Content = request.Content, Message = request.Message, Severity = request.Severity };
			...
			else
			{
				...
				await _jobService.UpdateStepAsync(job, batch.Id, stepId, streamConfig, newReports: new List<JobReport> { newReport });

				if (!String.IsNullOrEmpty(newReport.Message))
				{
					List<string> reportMetaData = new List<string>() { $"{JobReportMetadataKeys.MessageKey}={newReport.Message}"};

					if (!String.IsNullOrEmpty(newReport.Severity))
					{
						reportMetaData.Add($"{JobReportMetadataKeys.MessageSeverityKey}={newReport.Severity}");
					}

					if (!String.IsNullOrEmpty(newReport.Color))
					{
						reportMetaData.Add($"{JobReportMetadataKeys.ColorKey}={newReport.Color}");
					}

					Dictionary<JobStepId, List<string>> stepMetaData = new Dictionary<JobStepId, List<string>>() { { stepId, reportMetaData } };					
					
					await _jobService.UpdateMetadataAsync(job.Id, null, stepMetaData, context.CancellationToken);
				}
			}
