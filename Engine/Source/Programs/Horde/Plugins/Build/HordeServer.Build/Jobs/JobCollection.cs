...
namespace HordeServer.Jobs
{
	...
	{
		/// <summary>
		/// Ease of use struct used to pass telemetry context for job telemetry.
		/// </summary>
		internal struct TelemetryContext
		{
			/// <summary>
			/// The job that relates to the telemetry.
			/// </summary>
			internal IJob? Job { get; set; }

			/// <summary>
			/// The build config object to use when deriving telemetry sinks.
			/// </summary>
			internal IOptionsMonitor<BuildConfig> BuildConfig { get; set; }

			/// <summary>
			/// The telemetry writer used to publish telemetry.
			/// </summary>
			internal ITelemetryWriter TelemetryWriter { get; set; }
		}
		...
		class JobStep : IJobStep
		{
			...
			List<JobStepOutputRef>? _optionalInputs;
			IReadOnlyList<JobStepOutputRef> IJobStep.OptionalInputs => _optionalInputs ??= CreateOptionalInputDependencyList();
			...
			List<JobStepId>? _optionalInputDependencies;
			IReadOnlyList<JobStepId> IJobStep.OptionalInputDependencies => _optionalInputDependencies ??= CreateOptionalInputDependenciesList();
			...
			List<JobStepOutputRef> CreateOptionalInputDependencyList()
			{
				List<JobStepOutputRef> dependencies = [];
				foreach (NodeOutputRef optionalInput in Node.OptionalInputs)
				{
					if (Job.NodeRefToStepId.TryGetValue(optionalInput.NodeRef, out JobStepId jobStepId))
					{
						dependencies.Add(new JobStepOutputRef(jobStepId, optionalInput.OutputIdx));
					}
					else
					{
						MissingNodeRef(optionalInput.NodeRef);
					}
				}
				return dependencies;
			}

			List<JobStepId> CreateOptionalInputDependenciesList()
			{
				List<JobStepId> dependencies = [];
				foreach (NodeRef nodeRef in Node.OptionalInputDependencies)
				{
					if (Job.NodeRefToStepId.TryGetValue(nodeRef, out JobStepId jobStepId))
					{
						dependencies.Add(jobStepId);
...
#pragma warning restore CA2254 // Template should be a static expression
		...
		public async Task<IJob> AddAsync(JobId jobId, StreamId streamId, TemplateId templateRefId, ContentHash templateHash, IGraph graph, string name, CommitId commitId, CommitId? codeCommitId, CreateJobOptions options, CancellationToken cancellationToken)
		{
			...
			CreateBatchesInternal(newJob, graph, _logger, GetJobLogger(jobId));
			...
			{
				_telemetryWriter.WriteEvent(streamConfig.TelemetryStoreId, new JobCreatedTelemetry(CreateJobObject(newJob, graph)));
			}
		...
		async Task<JobDocument?> TryUpdateJobAsync(JobDocument jobDocument, IGraph graph, string? name, Priority? priority, bool? autoSubmit, int? autoSubmitChange, string? autoSubmitMessage, UserId? abortedByUserId, NotificationTriggerId? notificationTriggerId, List<JobReport>? reports, List<string>? arguments, KeyValuePair<int, NotificationTriggerId>? labelIdxToTriggerId, KeyValuePair<TemplateId, JobId>? jobTrigger, string? cancellationReason, CancellationToken cancellationToken)
		{
			...
			if (updateBatches)
			{
				UpdateBatchesInternal(jobDocument, graph, updates, _logger, GetJobLogger(jobId));
			}
		...
		async Task<JobDocument?> TryUpdateBatchAsync(JobDocument jobDocument, IGraph graph, JobStepBatchId batchId, LogId? newLogId, JobStepBatchState? newState, JobStepBatchError? newError, CancellationToken cancellationToken)
		{
			...
			if (newError != null && newError.Value != batch.Error)
			{
				...
				updates.Clear();
				UpdateBatchesInternal(jobDocument, graph, updates, _logger, GetJobLogger(jobDocument.Id));
				...
				for (int stepIdx = batch.Steps.Count - 1; stepIdx >= 0; stepIdx--)
				{
					...
					if (retryNodes.Remove(nodeRef))
					{
						if (step.State == JobStepState.Skipped && batch.Error == JobStepBatchError.None)
						{
							...
							NodeRef[] optionalDependencies = graph.GetNode(nodeRef).OptionalInputDependencies;
							retryNodes.UnionWith(optionalDependencies);
						}
						else
						{
							// Retry this step
							...
							// If the step is currently running, abort it and mark the batch as no longer needed
							// This ensures the agent stops executing the old step and the lease is cancelled
							if (step is { State: JobStepState.Running, AbortRequested: false })
							{
								step.AbortRequested = true;
								updates.Add(updateBuilder.Set(x => x.Batches[lambdaBatchIdx].Steps[lambdaStepIdx].AbortRequested, true));

								if (batch.Error == JobStepBatchError.None)
								{
									batch.Error = JobStepBatchError.NoLongerNeeded;
									updates.Add(updateBuilder.Set(x => x.Batches[lambdaBatchIdx].Error, JobStepBatchError.NoLongerNeeded));
								}
							}
						}
					}
		...
		async Task<JobDocument?> TryUpdateStepAsync(JobDocument jobDocument, IGraph graph, JobStepBatchId batchId, JobStepId stepId, JobStepState newState, JobStepOutcome newOutcome, JobStepError? newError, bool? newAbortRequested, UserId? newAbortByUserId, LogId? newLogId, NotificationTriggerId? newNotificationTriggerId, UserId? newRetryByUserId, Priority? newPriority, List<JobReport>? newReports, Dictionary<string, string?>? newProperties, string? newCancellationReason, JobId? newSpawnedJob, CancellationToken cancellationToken)
		{
					...
					for (int loopStepIdx = 0; loopStepIdx < batch.Steps.Count; loopStepIdx++)
					{
						...
						if (step.Id == stepId)
						{
							...
							if (newAbortRequested != null && step.AbortRequested == false)
							{
								...
								// If the step IS running, mark the batch for cancellation to trigger lease cancellation
								else if (step.State == JobStepState.Running && batch.Error == JobStepBatchError.None)
								{
									batch.Error = JobStepBatchError.Cancelled;
									updates.Add(updateBuilder.Set(x => x.Batches[batchIdx].Error, JobStepBatchError.Cancelled));
								}
			...
			{
				IJob? jobObject = CreateJobObject(jobDocument, graph);
				RefreshDependentJobSteps(jobDocument, graph, updates, _logger, GetJobLogger(jobDocument.Id), new TelemetryContext() { BuildConfig = _buildConfig, TelemetryWriter = _telemetryWriter, Job = jobObject });
		...
		async Task<JobDocument?> TryUpdateGraphAsync(JobDocument jobDocument, IGraph oldGraph, IGraph newGraph, CancellationToken cancellationToken)
		{
			...
			UpdateBatchesInternal(jobDocument, newGraph, updates, _logger, GetJobLogger(jobDocument.Id));
		...
		static bool NodesMatch(IGraph oldGraph, INode oldNode, IGraph newGraph, INode newNode)
		{
			...
			IEnumerable<string> oldOptionalInputDependencies = oldNode.OptionalInputDependencies.Select(x => oldGraph.GetNode(x).Name);
			IEnumerable<string> newOptionalInputDependencies = newNode.OptionalInputDependencies.Select(x => newGraph.GetNode(x).Name);
			if (!CompareListsIgnoreOrder(oldOptionalInputDependencies, newOptionalInputDependencies))
			{
				return false;
			}
			...
			IEnumerable<string> oldOptionalInputs = oldNode.OptionalInputs.Select(x => oldGraph.GetNode(x.NodeRef).OutputNames[x.OutputIdx]);
			IEnumerable<string> newOptionalInputs = newNode.OptionalInputs.Select(x => newGraph.GetNode(x.NodeRef).OutputNames[x.OutputIdx]);
			if (!CompareListsIgnoreOrder(oldOptionalInputs, newOptionalInputs))
			{
				return false;
			}
...
#pragma warning disable CA1822 // Mark members as static
		private void UpdateBatchesInternal(JobDocument job, IGraph graph, List<UpdateDefinition<JobDocument>> updates, ILogger logger, IAuditLogChannel<JobId> jobAudit)
#pragma warning restore CA1822 // Mark members as static
		{
			UpdateBatches(job, graph, updates, logger, jobAudit);
		}

#pragma warning disable CA1822 // Mark members as static
		private void CreateBatchesInternal(JobDocument job, IGraph graph, ILogger logger, IAuditLogChannel<JobId> jobAudit)
#pragma warning restore CA1822 // Mark members as static
		{
			CreateBatches(job, graph, logger, jobAudit);
		}
		...
		private void CreateBatches(JobDocument job, IGraph graph, ILogger logger, IAuditLogChannel<JobId> jobAudit)
		{
		...
		/// <param name="jobAudit"></param>
		void UpdateBatches(JobDocument job, IGraph graph, List<UpdateDefinition<JobDocument>> updates, ILogger logger, IAuditLogChannel<JobId> jobAudit)
		{
			UpdateDefinitionBuilder<JobDocument> updateBuilder = Builders<JobDocument>.Update;
			...
			IJob? jobObject = CreateJobObject(job, graph);
			RefreshDependentJobSteps(job, graph, new List<UpdateDefinition<JobDocument>>(), logger, jobAudit, new TelemetryContext() { BuildConfig = _buildConfig, TelemetryWriter = _telemetryWriter, Job = jobObject });
			...
			foreach (JobStepBatchDocument batch in job.Batches)
			{
				...
				foreach (JobStepDocument step in batch.Steps)
				{
					...
					else if (step.State == JobStepState.Skipped)
					{
						...
						// Optional input dependencies are not checked here, failing optional dependencies do not cause the node to be skipped
					}
			...
			for (int groupIdx = graph.Groups.Count - 1; groupIdx >= 0; groupIdx--)
			{
				...
				for (int nodeIdx = group.Nodes.Count - 1; nodeIdx >= 0; nodeIdx--)
				{
					...
					if (newNodesToExecute.Contains(node))
					{
						...
						foreach (NodeRef dependency in node.OptionalInputDependencies)
						{
							newNodesToExecute.Add(graph.GetNode(dependency));
						}
					}
		...
		/// <param name="jobAudit"></param>
		/// <param name="telemetryContext">Context object used in telemetry constuction, and emissions.</param>
		static void RefreshDependentJobSteps(JobDocument job, IGraph graph, List<UpdateDefinition<JobDocument>> updates, ILogger logger, IAuditLogChannel<JobId> jobAudit, TelemetryContext? telemetryContext = null)
		{
			// Update the batches
			UpdateDefinitionBuilder<JobDocument> updateBuilder = Builders<JobDocument>.Update;
			if (job.Batches != null)
			{
				// First pass: build a complete mapping of nodes to steps so that dependency lookups
				// are not affected by the order batches appear in the list. This fixes a bug where
				// steps in earlier batches could have their dependencies missed when those dependencies
				// were in batches appended later (e.g. during a graph update while a batch is Running).
				...
				// Second pass: evaluate dependencies and update step/batch states
				for (int loopBatchIdx = 0; loopBatchIdx < job.Batches.Count; loopBatchIdx++)
				{
					...
					for (int loopStepIdx = 0; loopStepIdx < batch.Steps.Count; loopStepIdx++)
					{
						...
						if (newState == JobStepState.Waiting)
						{
							...
							HashSet<JobStepDocument> requiredSteps = new(steps);
							List<JobStepDocument> optionalSteps = GetOptionalInputDependentSteps(graph, node, stepForNode);
							requiredSteps.ExceptWith(optionalSteps);
							// An optional input step aborted by the driver (not user requested) still allows the step to move to ready.
							// A skipped optional input step does not allow continuation; only allow continuation when the dependency ran to
							// completion and failed, not when it was skipped entirely.
							// Check all dependencies for skipped, including optional ones.
							// A skipped optional dependency means its required subgraph failed, which is different from ran and failed.
							// An optional dependency that ran and failed is tolerated but one that was skipped entirely is not.
							if (steps.Any(x => x.AbortRequested || x.State == JobStepState.Skipped) || requiredSteps.Any(x => x.Outcome == JobStepOutcome.Failure))
							{
								newState = JobStepState.Skipped;
								newOutcome = JobStepOutcome.Failure;

								// [TEMPORARY][START] This explicit emission of a skipped job step ref should be removed upon UE-357024.
								try
								{
									if (telemetryContext != null && telemetryContext.Value.BuildConfig.CurrentValue.TryGetStream(job.StreamId, out StreamConfig? streamConfig) && !streamConfig.TelemetryStoreId.IsEmpty)
									{
										JobStepRefTelemetry? telemetry = JobStepRefTelemetry.ConstructJobStepRefTelemetryFromJob(telemetryContext.Value.Job, batchIdx, stepIdx, newOutcome, newState);

										if (telemetry != null)
										{
											telemetryContext.Value.TelemetryWriter.WriteEvent(streamConfig.TelemetryStoreId, telemetry);
										}
									}
								}
								catch (Exception ex)
								{
									logger.LogError(ex, "Unable to create synthesized job step ref telemetry.");
								}
								// [TEMPORARY][END] This explicit emission of a skipped job step ref should be removed upon UE-357024.
							}
							else if (!steps.Any(x => !x.AbortRequested && JobStepExtensions.IsPendingState(x.State)))
							{
								if (optionalSteps.Any(x => x.Outcome == JobStepOutcome.Failure))
								{
									logger.LogInformation("Allowing job {JobId}, batch {BatchId}, step {StepId} to transition to ready state when optional dependencies failed ({Dependencies})", job.Id, batch.Id, step.Id, String.Join(", ", optionalSteps.Select(x => x.Id.ToString())));
									JobAudit(job.Id, logger, jobAudit, "Allowing job {JobId}, batch {BatchId}, step {StepId} to transition to ready state when optional dependencies failed ({Dependencies})", job.Id, batch.Id, step.Id, String.Join(", ", optionalSteps.Select(x => x.Id.ToString())));
								}
		...
		static List<JobStepDocument> GetOptionalInputDependentSteps(IGraph graph, INode node, Dictionary<INode, JobStepDocument> stepForNode)
		{
			List<JobStepDocument> steps = [];
			foreach (NodeRef optionalInputRef in node.OptionalInputDependencies)
			{
				if (stepForNode.TryGetValue(graph.GetNode(optionalInputRef), out JobStepDocument? step))
				{
					steps.Add(step);
				}
			}
