...
namespace HordeServer.Jobs.Graphs
{
	...
	public sealed class GraphCollection : IGraphCollection, IDisposable
	{
		...
		class Node : INode
		{
			...
			[BsonIgnoreIfNull]
			public NodeOutputRef[]? OptionalInputs { get; set; }
			...
			[BsonIgnoreIfNull]
			public NodeRef[]? OptionalInputDependencies { get; set; }
			...
			IReadOnlyList<NodeOutputRef> INode.Inputs => Inputs ?? [];
			IReadOnlyList<NodeOutputRef> INode.OptionalInputs => OptionalInputs ?? [];
			IReadOnlyList<string> INode.OutputNames => OutputNames ?? [];
			...
			NodeRef[] INode.OptionalInputDependencies => OptionalInputDependencies ?? [];
			...
			public Node(string name, NodeOutputRef[]? inputs, NodeOutputRef[]? optionalInputs, string[]? outputNames, NodeRef[] inputDependencies, NodeRef[]? optionalInputDependencies, NodeRef[] orderDependencies, Priority priority, bool allowRetry, bool runEarly, bool warnings, Dictionary<string, string>? credentials, Dictionary<string, string>? properties, IReadOnlyNodeAnnotations? annotations)
			{
				Name = name;
				Inputs = (inputs != null && inputs.Length > 0) ? inputs : null;
				OptionalInputs = (optionalInputs != null && optionalInputs.Length > 0) ? optionalInputs : null;
				OutputNames = (outputNames != null && outputNames.Length > 0) ? outputNames : null;
				InputDependencies = inputDependencies;
				OptionalInputDependencies = (optionalInputDependencies != null && optionalInputDependencies.Length > 0) ? optionalInputDependencies : null;
				...
				if (newGroupRequests != null)
				{
					foreach (NewGroup newGroupRequest in newGroupRequests)
					{
						...
						foreach (NewNode newNodeRequest in newGroupRequest.Nodes)
						{
							...
							NodeOutputRef[]? optionalInputs = null;
							if (newNodeRequest.OptionalInputs != null && newNodeRequest.OptionalInputs.Count > 0)
							{
								optionalInputs = newNodeRequest.OptionalInputs.Select(x => nodeOutputNameToRef[x]).ToArray();
							}

							NodeRef[] inputDependencies = (newNodeRequest.InputDependencies == null) ? [] : newNodeRequest.InputDependencies.Select(x => nodeNameToRef[x]).ToArray();
							NodeRef[] optionalInputDependencies = (newNodeRequest.OptionalInputDependencies == null) ? [] : newNodeRequest.OptionalInputDependencies.Select(x => nodeNameToRef[x]).ToArray();
							NodeRef[] orderDependencies = (newNodeRequest.OrderDependencies == null) ? [] : newNodeRequest.OrderDependencies.Select(x => nodeNameToRef[x]).ToArray();
							orderDependencies = orderDependencies.Union(inputDependencies).Union(optionalInputDependencies).ToArray();
							nodes.Add(new Node(newNodeRequest.Name, inputs, optionalInputs, newNodeRequest.Outputs?.ToArray(), inputDependencies, optionalInputDependencies, orderDependencies, priority, allowRetry, runEarly, warnings, newNodeRequest.Credentials, newNodeRequest.Properties, newNodeRequest.Annotations));
		...
		{
			Node node = new Node(IJob.SetupNodeName, null, null, null, [], [], [], Priority.High, true, false, true, null, null, null);
