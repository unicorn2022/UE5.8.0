...
namespace HordeServer.Jobs.Schedules
{
	...
	public sealed class ScheduleService : IHostedService, IAsyncDisposable
	{
		...
		internal async Task<bool> TriggerAsync(StreamId streamId, TemplateId templateId, DateTime utcNow, CancellationToken cancellationToken)
		{
			...
			try
			{
				await TriggerAsync(stream, templateId, templateRef, schedule, schedule.ActiveJobs.Count - removeJobIds.Count, utcNow, scheduleLogger, span, cancellationToken);
			}
			catch (Exception ex)
			{
		...
		/// <param name="linkedSpan">Span to link with</param>
		/// <param name="cancellationToken"></param>
		/// <returns>Async task</returns>
		private async Task TriggerAsync(IStream stream, TemplateId templateId, ITemplateRef templateRef, ISchedule schedule, int numActiveJobs, DateTime utcNow, ILogger scheduleLogger, TelemetrySpan linkedSpan, CancellationToken cancellationToken)
		{
			// Create a new root span as it may contain lots of child spans, this to prevent a huge single trace/span
			using TelemetrySpan span = _tracer.StartRootSpan($"{nameof(ScheduleService)}.{nameof(TriggerAsync)}.Full", links: [new Link(linkedSpan.Context)]);
			span.SetAttribute("horde.stream_id", stream.Id);
			span.SetAttribute("horde.template_id", templateId);
			...
			while (triggerChanges.Count < maxNewChanges)
			{
				...
				else if (await commitEnumerator.MoveNextAsync())
				{
			...
			if (triggerChanges.Count == 0)
			{
				...
				if (maxNewChanges <= 0)
				{
					scheduleLogger.LogInformation("MaxNewChanges is {MaxNewChanges} for {StreamId} template {TemplateId} - no changes were evaluated", maxNewChanges, stream.Id, templateId);
				}
				else if (totalExamined > 0)
				{
					scheduleLogger.LogInformation("Examined {TotalExamined} changes after CL {LastTriggerChange}, all {TotalFiltered} were filtered out", totalExamined, schedule.LastTriggerCommitId, totalFiltered);
				}
				return;
			}
			...
			foreach ((CommitIdWithOrder commitId, CommitIdWithOrder codeCommitId) in triggerChanges.OrderBy(x => x.CommitId))
			{
				...
				CreateJobOptions options = new (templateRefConfig);
				options.Priority = template.Priority;
				options.Claims = schedule.Claims?.Select(x => new AclClaimConfig(x.Type, x.Value)).ToList() ?? [];
		...
		{
			using TelemetrySpan span = _tracer.StartRootSpan($"{nameof(ScheduleService)}.{nameof(GetNextChangeForGateAsync)}");
			span.SetAttribute("horde.gate.template_id", gate.TemplateId);
			span.SetAttribute("horde.gate.target", gate.Target);
			span.SetAttribute("horde.min_commit_id", minCommitId?.ToString());
			span.SetAttribute("horde.max_commit_id", maxCommitId?.ToString());

			int jobsEvaluated = 0;
			int jobsSkippedFailed = 0;
			int jobsSkippedNotComplete = 0;
			int jobsSkippedNoTarget = 0;

			for (int c = 0; ; c++)
			{
				cancellationToken.ThrowIfCancellationRequested();
				span.SetAttribute("horde.iteration", c);
				span.AddEvent("Fetching job", new SpanAttributes(new KeyValuePair<string, object?>[] { new("horde.max_commit_id", maxCommitId) }));

				FindJobOptions options = new (StreamId: streamId, Templates: [gate.TemplateId], MinCommitId: minCommitId, MaxCommitId: maxCommitId);
				IReadOnlyList<IJob> jobs = await _jobCollection.FindAsync(options, count: 2, cancellationToken: cancellationToken);

				IJob? job = jobs.FirstOrDefault(x => maxCommitId == null || x.CommitId < maxCommitId);
				if (job == null)
				{
					span.AddEvent("Gate search exhausted", new SpanAttributes(new KeyValuePair<string, object?>[]
					{
						new("horde.jobs_evaluated", jobsEvaluated),
						new("horde.jobs_skipped_failed", jobsSkippedFailed),
						new("horde.jobs_skipped_not_complete", jobsSkippedNotComplete),
						new("horde.jobs_skipped_no_target", jobsSkippedNoTarget)
					}));

					if (jobsEvaluated > 0)
					{
						scheduleLogger.LogInformation("Gate search for {StreamId} template {TemplateId} (gate: {GateTemplateId}/{GateTarget}) exhausted after evaluating {JobsEvaluated} jobs: {Failed} failed, {NotComplete} not complete, {NoTarget} missing target",
							streamId, templateRefId, gate.TemplateId, gate.Target, jobsEvaluated, jobsSkippedFailed, jobsSkippedNotComplete, jobsSkippedNoTarget);
					}
					return null;
				}
				...
				if (graph != null)
				{
					(JobStepState stepState, JobStepOutcome stepOutcome)? state = job.GetTargetState(graph, gate.Target);

					if (state == null)
					{
						jobsSkippedNoTarget++;
						_logger.LogDebug("Gate: Job {JobId} (CL {CommitId}) - target '{Target}' not found in graph or missing steps", job.Id, job.CommitId, gate.Target);
					}
					else if (state is { stepState: JobStepState.Completed })
					{
						if (state is { stepOutcome: JobStepOutcome.Success or JobStepOutcome.Warnings})
						{
							span.SetAttribute("horde.jobs_evaluated", jobsEvaluated);
							return job.CommitId;
						}
						jobsSkippedFailed++;
						_logger.LogDebug("Gate: Job {JobId} (CL {CommitId}) - target '{Target}' completed with {Outcome}", job.Id, job.CommitId, gate.Target, state.Value.stepOutcome);
					...
					{
						jobsSkippedNotComplete++;
						_logger.LogDebug("Gate: Job {JobId} (CL {CommitId}) - target '{Target}' not complete (state: {State})", job.Id, job.CommitId, gate.Target, state.Value.stepState);
					}
				}
