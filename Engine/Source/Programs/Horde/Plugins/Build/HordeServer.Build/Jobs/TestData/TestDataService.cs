...
namespace HordeServer.Jobs.TestData
{
	...
	public sealed class TestDataService : IHostedService, IAsyncDisposable
	{
		...
		/// <param name="cancellationToken">Cancellation token for the operation</param>
		/// <returns></returns>
		public async Task<IReadOnlyList<ITestRecipeRef>> FindTestRecipesV2Async(TestRecipeId[]? recipeIds = null, TestId[]? testIds = null, StreamId[]? streamIds = null, CancellationToken cancellationToken = default)
		{
			return await _testDataV2.FindTestRecipesAsync(recipeIds, testIds, streamIds, cancellationToken);
		}
		...
		{
			return await _testDataV2.AddTestAuditAsync(userId, testId, options, cancellationToken);
		}

		/// <summary>
		/// Find audit associated with the tests
		/// </summary>
		/// <param name="testIds"></param>
		/// <param name="minCreateTime"></param>
		/// <param name="cancellationToken"></param>
		/// <returns></returns>
		public async Task<IReadOnlyList<ITestAudit>> FindTestAuditsV2Async(TestId[] testIds, DateTime? minCreateTime = null, CancellationToken cancellationToken = default)
		{
			return await _testDataV2.FindTestAuditsAsync(testIds, minCreateTime, cancellationToken);
		}
		...
		/// <param name="cancellationToken"></param>
		/// <returns></returns>
		public async Task<ITestHealthReport> AddOrUpdateTestHealthReportV2Async(TestId testId, StreamId streamId, string testName, ComputedTestHealth testHealth, CancellationToken cancellationToken = default)
		{
			return await _testDataV2.AddOrUpdateTestHealthReportAsync(testId, streamId, testName, testHealth, cancellationToken);
		}
		...
		{
			return _testDataV2.GetTestAuditLogger(testId);
		}

		/// <summary>
		/// Find Test health report associated with stream and test ids
		/// </summary>
		/// <param name="streamIds"></param>
		/// <param name="testIds"></param>
		/// <param name="cancellationToken"></param>
		/// <returns></returns>
		public async Task<IReadOnlyList<ITestHealthReport>> FindTestHealthReportsV2Async(StreamId[] streamIds, TestId[]? testIds, CancellationToken cancellationToken = default)
		{
			return await _testDataV2.FindTestHealthReportsAsync(streamIds, testIds, cancellationToken);
		}

		/// <summary>
		/// Update test health report notification information
		/// </summary>
		...
		{
			await _testDataV2.UpdateTestHealthReportNotificationDateAsync(reportId, resetDate, cancellationToken);
		}
	}
}