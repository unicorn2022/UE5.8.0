...
namespace HordeServer.Jobs
{
	...
	public class JobService : ILogExtAuthProvider
	{
		...
		public async Task<IJob> CreateJobAsync(JobId? jobId, StreamConfig streamConfig, TemplateId templateRefId, ContentHash templateHash, IGraph graph, string name, CommitId commitId, CommitId? codeCommitId, CreateJobOptions options, CancellationToken cancellationToken = default)
		{
			...
			span.SetAttribute("PreflightChange", options.PreflightCommitId?.ToString());
			...
			for (IJob? newJob = job; newJob != null; newJob = await GetJobAsync(job.Id, cancellationToken))
			{
				...
				if (newJob != null)
				{
					...
					foreach (IJobStepBatch batch in newJob.Batches)
					{
						...
						{
							LeaseOutcomeReason reason = LeaseOutcomeReason.ServerInitiated;
							if (abortedByUserId != null)
							{
								reason |= LeaseOutcomeReason.UserInitiated;
							}
							if (batch.Error == JobStepBatchError.NoLongerNeeded)
							{
								reason |= LeaseOutcomeReason.NoLongerNeeded;
							}
							await CancelLeaseAsync(batch.AgentId.Value, batch.LeaseId.Value, reason, cancellationToken);
						}
					}
		...
		/// <param name="reason">Reason for cancelling the lease</param>
		/// <param name="cancellationToken">Cancellation token for the operation</param>
		/// <returns></returns>
		async Task CancelLeaseAsync(AgentId agentId, LeaseId leaseId, LeaseOutcomeReason reason, CancellationToken cancellationToken)
		{
			...
			span.SetAttribute("Reason", reason.ToString());
			...
			for (; ; )
			{
				...
				IAgent? newAgent = await agent.TryCancelLeaseAsync(index, reason, cancellationToken);
				if (newAgent != null)
				{
		...
		async Task<JobStepTimingData> GetStepTimingInfoAsync(StreamId streamId, TemplateId templateId, string nodeName, CommitIdWithOrder? commitId, CancellationToken cancellationToken)
		{
			...
			List<IJobStepRef> steps = await _jobStepRefs.GetStepsForNodeAsync(streamId, templateId, nodeName, commitId, false, 10, false, cancellationToken);
		...
		async Task<JobStepBatchError?> CheckJobAsync(IJob job, StreamConfig streamConfig, CancellationToken cancellationToken)
		{
			try
			{
				// Only check P4 shelves for numeric preflight commit IDs (P4 changelists)
				// Hash-based preflights use commit IDs and don't have shelves to check
				int? preflightChange = job.PreflightCommitId?.TryGetPerforceChange();
				if (preflightChange != null)
				{
					(CheckShelfResult result, _) = await _perforceService.CheckShelfAsync(streamConfig, preflightChange.Value, cancellationToken);
				...
				if (oldState != updatedState || oldOutcome != updatedOutcome)
				{
					...
					if (job.PreflightCommitId != null && updatedState == JobStepState.Completed)
					{
						...
						if (state == JobStepState.Completed)
						{
							...
							{
								// Only tag P4 changelists (numeric preflight commit IDs), not hash-based revisions
								int? p4Change = job.PreflightCommitId?.TryGetPerforceChange();
								if (p4Change != null)
								{
									...
									if (settings.AlwaysTagPreflightCL)
									{
										...
										await _perforceService.UpdateChangelistDescriptionAsync(streamConfig.ClusterName, p4Change.Value, x => x.TrimEnd() + $"\n#preflight {job.Id}", cancellationToken);
									}
								}
							}
						}
							...
							if (job.TryGetStep(batchId, stepId, out completingJobStep) && completingJobStep.FinishTimeUtc != null)
							{
								if (!streamConfig.TelemetryStoreId.IsEmpty)
								{
									try
									{
										...
										JobLabelTelemetryHelpers.EmitJobLabelTelemetry(_telemetryWriter, streamConfig.TelemetryStoreId, job, graph);
									}
									catch (Exception ex)
									{
		...
		private async Task<IJob> AutoSubmitChangeAsync(StreamConfig streamConfig, IJob job, IGraph graph, CancellationToken cancellationToken)
		{
			...
			// Auto-submit is only supported for P4 preflights (numeric commit IDs), not hash-based preflights
			if (job.PreflightCommitId.TryGetPerforceChange() == null)
			{
				_logger.LogInformation("Skipping auto-submit for job {JobId} - preflight commit {CommitId} is not a P4 changelist", job.Id, job.PreflightCommitId);
				return job;
			}
