...
namespace HordeServer
{
	...
	public class BuildConfig : IPluginConfig
	{
		...
		/// <summary>
		/// RoboMerge branchmap file configuration for the merge graph service.
		/// When null or empty, the service is inert (no Perforce connections, no API data).
		/// </summary>
		public RoboMergeConfig? Robomerge { get; set; }
		...
		/// <summary>
		/// Whether to check if a conform is required on an agent before assigning a lease for a job task source.
		/// </summary>
		public bool EnableJobTaskConformCheck { get; set; }
		...
		/// <summary>
		/// Build health issue configurations
		/// </summary>
		public IssueConfig? Issues { get; set; }

		/// <summary>
		/// Whether to allow notifications for Root Cause Analysis to be sent
		/// </summary>
		public bool EnableIssueRootCauseAnalysisNotifications { get; set; }

		/// <summary>
		/// Global kill switch for posting auto-RCA results to Slack triage threads.
		/// When false (default), no auto-RCA Slack notifications are sent regardless of per-workflow AutoRcaSlackEnabled settings.
		/// </summary>
		public bool EnableAutoRcaSlackNotifications { get; set; }

		/// <summary>
		/// Whether to use multi-stage LLM analysis for auto-RCA. Multi-stage is slower but potentially more accurate. Set to false for faster processing with higher throughput.
		/// </summary>
		public bool AutoRcaUseMultiStage { get; set; } = true;

		/// <summary>
		/// Maximum number of concurrent auto-RCA requests. Limits load on the external LLM API.
		/// Set to 0 to disable auto-RCA globally (acts as a kill switch).
		/// </summary>
		public int MaxConcurrentRcaRequests { get; set; }

		/// <summary>
		/// Issue channel groupings
		/// </summary>
		[JsonIgnore]
		public IReadOnlyDictionary<string, string> IssueChannelGroups { get; private set; } = null!;

		/// <summary>
		/// The maximum days since node success to scan for suspects
		/// </summary>
		public int IssueFindSuspectsMaxSuccessDays { get; set; } = 14;

		/// <summary>
		/// The maximum number of seconds to spend scording suspects
		/// </summary>
		public int IssueFindSuspectsMaxScoringSeconds { get; set; } = 60;

		#region Issue Suspect Finding Optimization Configuration

		/// <summary>
		/// Maximum number of commits to examine for suspects.
		/// Previously hardcoded as const MaxChanges = 500.
		/// </summary>
		public int IssueFindSuspectsMaxChanges { get; set; } = 500;

		/// <summary>
		/// Enable parallel scoring of commits for suspect finding.
		/// When enabled, commits are scored concurrently up to MaxParallelism.
		/// </summary>
		public bool IssueFindSuspectsEnableParallelScoring { get; set; } = false;

		/// <summary>
		/// Maximum concurrent commit file lookups when parallel scoring is enabled.
		/// Higher values improve speed but increase Perforce server load.
		/// </summary>
		public int IssueFindSuspectsMaxParallelism { get; set; } = 50;

		/// <summary>
		/// Enable early exit when theoretical maximum score is achieved.
		/// Only exits early if the best possible score has been found.
		/// </summary>
		public bool IssueFindSuspectsEnableEarlyExit { get; set; } = false;

		/// <summary>
		/// Enable bidirectional commit query to capture both recent and old commits.
		/// Queries half the limit from each end of the commit range.
		/// </summary>
		public bool IssueFindSuspectsEnableBidirectionalQuery { get; set; } = false;

		/// <summary>
		/// Enable fingerprint-based fallback when no suspects are found.
		/// Helps prevent duplicate issues when commit range exceeds MaxChanges.
		/// </summary>
		public bool IssueFindSuspectsEnableFingerprintFallback { get; set; } = false;

		/// <summary>
		/// Minimum score required for fingerprint fallback matching.
		/// Higher values are more conservative (fewer false matches).
		/// </summary>
		public int IssueFingerprintFallbackMinScore { get; set; } = 50;

		/// <summary>
		/// Maximum age in days for issues to be considered in fingerprint fallback.
		/// </summary>
		public int IssueFingerprintFallbackMaxAgeDays { get; set; } = 14;

		/// <summary>
		/// Fingerprint types excluded from fallback matching (too generic).
		/// </summary>
		public List<string> IssueFingerprintFallbackExcludedTypes { get; set; } =
			new() { "Default", "ExitCode", "Generic" };

		#endregion

		/// <summary>
		/// Whether to enable expanded IgnorePatterns discovery (workspace-wide scanning + config-specified files).
		/// When false (default), only Engine directory is scanned for ignore patterns.
		/// </summary>
		public bool EnableExpandedIgnorePatterns { get; set; } = true;

		/// <summary>
		/// Fingerprint type prefixes considered "low context" for suspect mention suppression.
		/// When an issue's fingerprint type starts with any of these prefixes AND all suspects are tied
		/// at rank 0, @ mentions are suppressed in Slack. Suspects are still listed without mentions.
		/// Set to an empty array to disable suppression entirely.
		/// </summary>
		public List<string> LowContextFingerprintTypes { get; set; } = new List<string> { "Hashed", "Gauntlet" };
		...
		public void PostLoad(PluginConfigOptions configOptions)
		{
			...
			Dictionary<string, string> issueChannelGroups = new Dictionary<string, string>();
			IssueChannelGroups = issueChannelGroups;

			// build up the issue channel groupings
			Issues?.PostLoad();

			Issues?.ChannelGroups?.ForEach(group =>
			{
				if (group.Channels == null || group.Channels.Count == 0)
				{
					return;
				}

				string channelGroup = String.Join("_", group.Channels);

				group.Channels.ForEach(channel =>
				{
					if (String.IsNullOrEmpty(channel) || issueChannelGroups.ContainsKey(channel))
					{
						return;
					}

					issueChannelGroups[channel] = channelGroup;
				});
			});
	...
	/// </summary>
	public class RoboMergeConfig
	{
		/// <summary>
		/// Perforce depot paths to branchmap JSON files.
		/// Plain depot paths (e.g. "//MyProject/Main/Build/Streams/myproject.branchmap.json").
		/// All files are read from the Perforce cluster specified by <see cref="PerforceCluster"/>.
		/// </summary>
		public List<string> BranchmapFiles { get; set; } = new List<string>();

		/// <summary>
		/// Perforce cluster name to connect to for reading branchmap files.
		/// Maps to a connection configured in BuildServerConfig.Perforce[].
		/// Default: "default" (uses the default Perforce connection).
		/// </summary>
		public string PerforceCluster { get; set; } = "default";

		/// <summary>
		/// How often to poll Perforce for branchmap file changes. Default: 5 minutes.
		/// Changing this requires a server restart (ticker interval is set at construction time).
		/// </summary>
