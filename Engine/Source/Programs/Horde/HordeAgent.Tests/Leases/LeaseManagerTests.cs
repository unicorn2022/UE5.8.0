...
public class FakeSystemMetrics : ISystemMetrics
{
	public long DiskFreeSpace { get; set; } = 1000;
	public CpuMetrics? GetCpu() => null;
	public CpuTimeMetrics? GetCpuTime() => null;
	public MemoryMetrics? GetMemory() => null;
	public DiskMetrics? GetWorkingDisk() => new() { FreeSpace = DiskFreeSpace, TotalSize = 10000 };
...
public sealed class LeaseManagerTests
{
	...
	[TestMethod]
	public async Task UpdateSession_SendsDiskFreeSpaceInCapabilitiesAsync()
	{
		using CancellationTokenSource cts = new(5000);
		FakeSystemMetrics fakeMetrics = new() { DiskFreeSpace = 2000 };
		FakeHordeRpcServer server = new(CreateConsoleLogger<FakeHordeRpcServer>());
		LeaseManager leaseManager = CreateLeaseManager(server.GetHordeClient(), systemMetrics: fakeMetrics);
		leaseManager.TerminateSessionAfterLease = true;

		await leaseManager.RunAsync(cts.Token);
		server.UpdateSessionRequests.Writer.Complete();

		List<RpcUpdateSessionRequest> requests = await server.UpdateSessionRequests.Reader.ReadAllAsync(cts.Token).ToListAsync(cts.Token);
		RpcUpdateSessionRequest request = requests.Last();
		Assert.Contains($"{KnownPropertyNames.WorkingDiskFreeSpace}=2000", request.CapabilityOverrides.Properties, StringComparer.Ordinal);
	}

	[TestMethod]
	public async Task UpdateSession_SendsDiskFreeSpaceInCapabilityOverridesAsync()
	{
		using CancellationTokenSource cts = new(5000);
		FakeSystemMetrics fakeMetrics = new() { DiskFreeSpace = 2000 };
		FakeCapabilitiesService capsService = new()
		{
			Capabilities = new RpcAgentCapabilities { Properties = { $"{KnownPropertyNames.WorkingDiskFreeSpace}=1000" } }
		};

		FakeHordeRpcServer server = new(CreateConsoleLogger<FakeHordeRpcServer>());
		LeaseManager leaseManager = CreateLeaseManager(server.GetHordeClient(), capsService, systemMetrics: fakeMetrics);
		leaseManager.TerminateSessionAfterLease = true;
		...
		List<RpcUpdateSessionRequest> requests = await server.UpdateSessionRequests.Reader.ReadAllAsync(cts.Token).ToListAsync(cts.Token);
		RpcUpdateSessionRequest request = requests.Last();
		Assert.Contains($"{KnownPropertyNames.WorkingDiskFreeSpace}=1000", request.Capabilities.Properties, StringComparer.Ordinal);
		Assert.Contains($"{KnownPropertyNames.WorkingDiskFreeSpace}=2000", request.CapabilityOverrides.Properties, StringComparer.Ordinal);
	}

	[TestMethod]
	public async Task UpdateSession_NoDiskMetrics_NoCapabilityOverridesAsync()
	{
		using CancellationTokenSource cts = new(5000);
		FakeHordeRpcServer server = new(CreateConsoleLogger<FakeHordeRpcServer>());
		LeaseManager leaseManager = CreateLeaseManager(server.GetHordeClient());
		leaseManager.TerminateSessionAfterLease = true;
		...
		List<RpcUpdateSessionRequest> requests = await server.UpdateSessionRequests.Reader.ReadAllAsync(cts.Token).ToListAsync(cts.Token);
		RpcUpdateSessionRequest request = requests.Last();
		Assert.IsNull(request.CapabilityOverrides);
	}
	...
	private static LeaseManager CreateLeaseManager(IHordeClient hordeClient, FakeCapabilitiesService? capsService = null, LeaseResult? leaseResult = null, ISystemMetrics? systemMetrics = null)
	{
		...
		return new LeaseManager(
			...
			statusService,
			systemMetrics ?? new DefaultSystemMetrics(),
