...
namespace HordeServer.Configuration
{
	...
	public sealed class ConfigService : IConfigService, IOptionsFactory<GlobalConfig>, IOptionsChangeTokenSource<GlobalConfig>, IHostedService, IAsyncDisposable
	{
		...
		(Dictionary<Uri, OverrideConfigFile> OverrideFiles, ConfigContext Context) CreateOverrideContext(Dictionary<Uri, byte[]> files)
		{
			...
			return (overrideFiles, context);
		}

		async Task<GlobalConfig> ReadAndPostLoadOverridesAsync(ConfigContext context, CancellationToken cancellationToken)
		{
		...
		string FormatConfigException(Exception ex, ConfigContext context, string logMessage)
		{
			...
			{
				message.Append($"\n\nTrace:\n{ex.StackTrace}");
				_logger.LogWarning(ex, "{Message}: {ExMessage}", logMessage, ex.Message);
			}
		...
		/// </summary>
		public async Task<ConfigValidationResult> ValidateAsync(Dictionary<Uri, byte[]> files, CancellationToken cancellationToken)
		{
			(Dictionary<Uri, OverrideConfigFile> overrideFiles, ConfigContext context) = CreateOverrideContext(files);
			try
			{
				await ReadAndPostLoadOverridesAsync(context, cancellationToken);

				foreach (OverrideConfigFile file in overrideFiles.Values)
				{
					if (!file.WasRead)
					{
						return new ConfigValidationResult { Error = $"File {file.Uri} was not read by server", Warnings = context.Warnings };
					}
				}

				return new ConfigValidationResult { Warnings = context.Warnings };
			}
			catch (Exception ex)
			{
				return new ConfigValidationResult { Error = FormatConfigException(ex, context, "Unhandled exception while validating config files"), Warnings = context.Warnings };
			}
		}

		/// <inheritdoc/>
		public async Task<ConfigResolveResult> ResolveWithOverridesAsync(Dictionary<Uri, byte[]> files, CancellationToken cancellationToken)
		{
			(Dictionary<Uri, OverrideConfigFile> overrideFiles, ConfigContext context) = CreateOverrideContext(files);
			try
			{
				GlobalConfig globalConfig = await ReadAndPostLoadOverridesAsync(context, cancellationToken);

				// Collect unread files as warnings (not errors)
				List<Uri> unreadFiles = new List<Uri>();
				foreach (OverrideConfigFile file in overrideFiles.Values)
				{
					if (!file.WasRead)
					{
						unreadFiles.Add(file.Uri);
					}
				}

				return new ConfigResolveResult { Config = globalConfig, Plugins = globalConfig.Plugins, UnreadFiles = unreadFiles, Warnings = context.Warnings };
			}
			catch (Exception ex)
			{
				return new ConfigResolveResult { Error = FormatConfigException(ex, context, "Unhandled exception while resolving config overrides"), Warnings = context.Warnings };
			}
		}
		...
		async Task<ConfigSnapshot> CreateSnapshotAsync(CancellationToken cancellationToken)
		{
			...
			try
			{
				...
				if (globalConfig.VersionEnum < ConfigVersion.Latest)
				{
					...
					{
						message.Add($"- v{(int)ConfigVersion.PoolsInConfigFiles}: Pools should now be configured through the globals.json file rather than REST API or database.");
					}
					message.Add("");
