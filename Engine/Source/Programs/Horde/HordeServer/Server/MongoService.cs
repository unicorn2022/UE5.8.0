...
#pragma warning restore CA2254 // Template should be a static expression
		...
		static X509Certificate2Collection ImportCertificateBundle(string fileName)
		{
			...
			for (int offset = 0; offset < text.Length;)
			{
				...
				collection.Add(X509CertificateLoader.LoadCertificate(Encoding.UTF8.GetBytes(certificateText)));
		...
		async Task<bool> SetSchemaVersionAsync(SemVer schemaVersion, CancellationToken cancellationToken)
		{
			...
			for (; ; )
			{
				...
				if (await TryUpdateSingletonStaticAsync(SingletonsV2, currentSchema, cancellationToken))
				{
					return true;
				}
			}
		...
		{
			return GetSingletonStaticAsync(SingletonsV1, SingletonsV2, () => new T(), cancellationToken);
		}

		/// <inheritdoc/>
		public Task<T> GetSingletonAsync<T>(Func<T> constructor, CancellationToken cancellationToken) where T : SingletonBase, new()
		{
			return GetSingletonStaticAsync(SingletonsV1, SingletonsV2, constructor, cancellationToken);
		}
		...
		public async Task UpdateSingletonAsync<T>(Func<T, bool> updater, CancellationToken cancellationToken) where T : SingletonBase, new()
		{
			for (; ; )
			{
				T document = await GetSingletonStaticAsync(SingletonsV1, SingletonsV2, () => new T(), cancellationToken);
				if (!updater(document))
				{
					break;
				}
				if (await TryUpdateSingletonStaticAsync(SingletonsV2, document, cancellationToken))
				{
					break;
				}
			}
		...
		{
			return TryUpdateSingletonStaticAsync(SingletonsV2, singletonObject, cancellationToken);
		}

		/// <summary>
		/// Gets a singleton document by id
		/// </summary>
		/// <param name="singletonsV1">Collection of Singleton documents V1</param>
		/// <param name="singletonsV2">Collection of Singleton documents V2</param>
		/// <param name="constructor">Method to use to construct a new object</param>
		/// <param name="cancellationToken">Cancellation token for the operation</param>
		/// <returns>The document</returns>
		internal static async Task<T> GetSingletonStaticAsync<T>(IMongoCollection<BsonDocument> singletonsV1, IMongoCollection<BsonDocument> singletonsV2, Func<T> constructor, CancellationToken cancellationToken) where T : SingletonBase, new()
		{
		...
		internal static async Task<bool> TryUpdateSingletonStaticAsync<T>(IMongoCollection<BsonDocument> singletonsV2, T singletonObject, CancellationToken cancellationToken) where T : SingletonBase
		{
