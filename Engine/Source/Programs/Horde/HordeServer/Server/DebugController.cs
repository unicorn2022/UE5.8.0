...
namespace HordeServer.Server
{
	...
	public class DebugController : HordeControllerBase
	{
		...
		private readonly IAgentScheduler _agentScheduler;
...
#pragma warning restore CA2254
		...
		/// <returns></returns>
		[HttpGet]
		[Route("/api/v1/debug/agent-orphaned-session-cleanup")]
		public async Task<ActionResult<object>> AgentSessionCleanupAsync(CancellationToken cancellationToken = default)
		{
			if (!_globalConfig.Value.Authorize(ServerAclAction.Debug, User))
			{
				return Forbid(ServerAclAction.Debug);
			...
			return Ok(new 
			{ 
				SessionsRemoved = await _agentScheduler.RemoveOrphanedSessionsAsync(cancellationToken),
			});
		}
	}
}
