...
namespace HordeServer
{
	...
	static class LoggerExtensions
	{
		...
		public static LoggerConfiguration Console(this LoggerSinkConfiguration sinkConfig, ServerSettings settings)
		{
			...
			{
				return sinkConfig.Console(new SizeLimitedJsonFormatter(new JsonFormatter(renderMessage: true)));
			}
			else
			{
				ConsoleTheme theme;
		...
		public static async Task<int> Main(string[] args)
		{
			...
			Serilog.Log.Logger = new LoggerConfiguration()
				...
				.Enrich.WithExceptionDetails(new DestructuringOptionsBuilder()
					...
					.WithDestructurers([new RpcExceptionDestructurer()])
				...
				.WriteTo.File(new SizeLimitedJsonFormatter(new JsonFormatter(renderMessage: true)), Path.Combine(logDir.FullName, "Log.json"), rollingInterval: RollingInterval.Day, rollOnFileSizeLimit: true, fileSizeLimitBytes: 20 * 1024 * 1024, retainedFileCountLimit: 10)
