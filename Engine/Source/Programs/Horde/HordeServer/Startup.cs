...
#pragma warning restore CA2000 // Dispose objects before losing scope
			...
			authBuilder.AddCookie(options =>
			{
				if (settings.AuthMethod == AuthMethod.Horde)
				{
					...
					// .NET 10 breaking change: cookie auth no longer redirects to login page for [ApiController] endpoints by default.
					// AccountController.Login() is a web navigation endpoint that must redirect to the login form, so restore the redirect behavior.
