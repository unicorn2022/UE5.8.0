...
#pragma warning restore CA1416
		...
		public static X509Certificate2? ReadGrpcCertificate(ServerSettings hordeSettings)
		{
			...
			else if (hordeSettings.ServerPrivateCert.StartsWith(base64Prefix, StringComparison.Ordinal))
			{
				try
				{
					byte[] certData = Convert.FromBase64String(hordeSettings.ServerPrivateCert.Substring(base64Prefix.Length));
					return X509CertificateLoader.LoadPkcs12(certData, null);
				}
				catch (Exception ex) when (ex is FormatException or CryptographicException)
				{
					throw new InvalidOperationException($"Failed to load PKCS#12 certificate from base64 value in setting '{nameof(ServerSettings.ServerPrivateCert)}': {ex.Message}", ex);
				}
			}
			else
			{
				...
				try
				{
					return X509CertificateLoader.LoadPkcs12(FileReference.ReadAllBytes(serverPrivateCert), null);
				}
				catch (Exception ex) when (ex is IOException or UnauthorizedAccessException or CryptographicException)
				{
					throw new InvalidOperationException($"Failed to load PKCS#12 certificate from path '{serverPrivateCert}' (setting '{nameof(ServerSettings.ServerPrivateCert)}'): {ex.Message}", ex);
				}
			}
		}
