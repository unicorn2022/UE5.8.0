...
namespace HordeServer.Accounts
{
	...
	public class OAuthController : Controller
	{
		...
		string CreateAccessToken(IGlobals globals, IAccount subject, long expiresIn, string? nonce)
		{
			...
			// A single claim type can occur multiple times, so ensure all values captured and grouped as an array-value when more than one exists
			foreach (IGrouping<string, IUserClaim> group in subject.Claims.GroupBy(c => c.Type))
			{
				string[] values = group.Select(c => c.Value).ToArray();
		...
		static void GetUserInfoClaims(IAccount account, Dictionary<string, object> properties)
		{
			...
			// A single claim type can occur multiple times, so ensure all values captured and grouped as an array-value when more than one exists
			foreach (IGrouping<string, IUserClaim> group in account.Claims.GroupBy(c => c.Type))
			{
				string[] values = group.Select(c => c.Value).ToArray();
				properties[group.Key] = values.Length == 1 ? values[0] : values;
			}
		}
