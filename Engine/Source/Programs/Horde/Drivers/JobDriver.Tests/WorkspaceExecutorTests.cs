...
internal class MaterializerStub(string name) : IWorkspaceMaterializer
{
	...
	public DirectoryReference SyncDir { get; } = new("not-set");
	public DirectoryReference BaseDir { get; } = new("not-set");
...
public class WorkspaceExecutorTests
{
	...
	public void Factory()
	{
		Assert.ThrowsExactly<ArgumentException>(() => GetMaterializer(new RpcJobOptions(), new RpcAgentWorkspace(), []));
