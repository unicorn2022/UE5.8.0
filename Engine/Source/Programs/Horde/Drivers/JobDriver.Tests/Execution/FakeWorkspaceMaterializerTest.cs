...
public sealed class FakeWorkspaceMaterializerTest : IDisposable
{
	...
	public async Task RemoveUntrackedFilesAsync()
	{
		_wm.SetFile(1, "foo/bar/baz.txt", "fortnite");
		_wm.SetFile(2, "foo/main.cpp", "main");
		await File.WriteAllTextAsync(Path.Join(_wm.SyncDir.FullName, "external.txt"), "external");
	...
	private string GetAbsPath(string relativeFilePath) { return Path.Join(_wm.SyncDir.FullName, relativeFilePath); }
}
