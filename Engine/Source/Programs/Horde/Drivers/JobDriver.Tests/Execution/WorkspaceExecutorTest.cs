...
{
	internal class FakeHordeClient : IHordeClient
	{
		class FakeJobRpc : JobRpc.JobRpcClient
		{
		}

		private readonly JobRpc.JobRpcClient? _jobRpcClient = null;
		...
		public FakeHordeClient(JobRpc.JobRpcClient? jobRpcClient = null)
		{
		...
		public Task<GrpcChannel> CreateGrpcChannelAsync(CancellationToken cancellationToken = default) => throw new NotImplementedException();
		public Task<TClient> CreateGrpcClientAsync<TClient>(CancellationToken cancellationToken = default) where TClient : ClientBase<TClient>
		{
			...
			{
				return _jobRpcClient != null
					? Task.FromResult((TClient)(object)_jobRpcClient)
					: Task.FromResult((TClient)(object)new FakeJobRpc());
			}
		...
		public Task<string?> GetAccessTokenAsync(bool interactive, CancellationToken cancellationToken = default) => Task.FromResult<string?>("access-token");
		public bool HasValidAccessToken() => true;
		public Task<bool> LoginAsync(bool allowPrompt, CancellationToken cancellationToken) => throw new NotImplementedException();
	}
	...
	public sealed class WorkspaceExecutorTest : IDisposable
	{
		...
		public async Task EnvVarsAsync()
		{
			...
			Assert.AreEqual(_workspace.SyncDir.FullName, envVars["uebp_LOCAL_ROOT"]);
			...
			Assert.AreEqual(_autoSdkWorkspace.SyncDir.FullName, envVars["UE_SDKS_ROOT"]);
		...
		public async Task JobWithNoChangeAsync()
		{
			...
			await Assert.ThrowsExactlyAsync<WorkspaceMaterializationException>(() => executor.InitializeAsync(batch, _logger, CancellationToken.None));
		}

		private static void AssertWorkspaceFile(IWorkspaceMaterializer workspace, string relativePath, string expectedContent)
		{
			DirectoryReference workspaceDir = workspace.SyncDir;
