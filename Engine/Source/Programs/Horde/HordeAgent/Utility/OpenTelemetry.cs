...
public static class OpenTelemetryHelper
{
	...
	private static readonly HttpRequestOptionsKey<string> s_urlTemplateKey = new("url.template");
	...
	{
		void DatadogHttpRequestEnricher(Activity activity, HttpRequestMessage request)
		{
			activity.SetTag("service.name", settings.ServiceName + "-http-client");
			activity.SetTag("operation.name", "http.request");
			string url = $"{request.Method} {request.Headers.Host}{request.RequestUri?.LocalPath}";
			if (request.Options.TryGetValue(s_urlTemplateKey, out string? template))
			{
				activity.DisplayName = $"{request.Method} {template}";
			}
			else
			{
				activity.DisplayName = $"{request.Method} {request.Headers.Host}";
			}
