...
namespace HordeAgent.Utility
{
	static class Shutdown
	{
		...
		{
			if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows) && !RuntimePlatform.IsRunningOnWine())
			{
			...
			else if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX) || RuntimeInformation.IsOSPlatform(OSPlatform.Linux) || RuntimePlatform.IsRunningOnWine())
			{
				string shutdownArgs;
				if (restartAfterShutdown)
				{
