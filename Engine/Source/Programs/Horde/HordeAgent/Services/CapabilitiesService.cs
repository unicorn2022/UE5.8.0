...
namespace HordeAgent.Services
{
	...
	class CapabilitiesService : ICapabilitiesService
	{
		...
		async Task<RpcAgentCapabilities> GetCapabilitiesInternalAsync(DirectoryReference? workingDir)
		{
			...
			// Add system disk info
			try
			{
				string? systemDriveName;
				systemDriveName = RuntimeInformation.IsOSPlatform(OSPlatform.Windows)
					? Path.GetPathRoot(Environment.SystemDirectory) // Windows (C:\)
					: "/"; // *nix

				if (systemDriveName != null)
				{
					DriveInfo systemInfo = new DriveInfo(systemDriveName);
					capabilities.Properties.Add($"{KnownPropertyNames.SystemDiskFreeSpace}={systemInfo.AvailableFreeSpace}");
					capabilities.Properties.Add($"{KnownPropertyNames.SystemDiskTotalSize}={systemInfo.TotalSize}");
				}
			}
			catch (Exception ex)
			{
				logger.LogWarning(ex, "Unable to query system disk info");
			}
			...
			if (workingDir != null)
			{
				...
				if (driveName != null)
				{
					try
					{
						...
						capabilities.Properties.Add($"{KnownPropertyNames.WorkingDiskFreeSpace}={info.AvailableFreeSpace}");
						capabilities.Properties.Add($"{KnownPropertyNames.WorkingDiskTotalSize}={info.TotalSize}");
					}
					catch (Exception ex)
					{
