...
namespace HordeAgent.Leases
{
	...
	abstract class LeaseHandler : IDisposable
	{
		...
		/// </summary>
		public string CancellationReason => _outcomeReason.ToString();

		readonly CancellationTokenSource _cancellationSource = new ();
		LeaseOutcomeReason _outcomeReason = LeaseOutcomeReason.None;
		...
		/// </summary>
		public void Start(ISession session, ISystemMetrics systemMetrics, Tracer tracer, ILogger logger, LeaseLoggerFactory leaseLoggerFactory)
		{
			Result = Task.Run(async () =>
			{
				try
				{
					return await HandleLeaseAsync(session, systemMetrics, tracer, logger, leaseLoggerFactory);
				}
				catch (Exception e)
				{
					logger.LogError(e, "Unhandled exception while executing lease: {Message} ", e.Message);
					throw;
				}
			});
		}
		...
		/// Cancels the lease
		/// </summary>
		public void Cancel(LeaseOutcomeReason reason)
		{
			_outcomeReason = reason;
		...
		/// </summary>
		async Task<LeaseResult> HandleLeaseAsync(ISession session, ISystemMetrics systemMetrics, Tracer tracer, ILogger logger, LeaseLoggerFactory leaseLoggerFactory)
		{
			...
			Stopwatch leaseDuration = Stopwatch.StartNew();
			CpuTimeMetrics? cpuTimeStart = systemMetrics.GetCpuTime();

			// Get the lease outcome
			LeaseResult result = new (LeaseOutcome.Failed, LeaseOutcomeReason.UnexpectedError);
			try
			{
			...
			CpuTimeMetrics? cpuTimeEnd = systemMetrics.GetCpuTime();
			...
			if (_cancellationSource.IsCancellationRequested)
			{
				...
				newRpcLease.OutcomeReasons.AddRange(RpcLease.ToProtobufOutcomeReasons(result.OutcomeReason | _outcomeReason));
				newRpcLease.Output = Google.Protobuf.ByteString.Empty;
			}
			else
			{
				LeaseOutcomeReason reason = result.Outcome is LeaseOutcome.Success ? result.OutcomeReason : result.OutcomeReason | _outcomeReason;
				...
				newRpcLease.OutcomeReasons.AddRange(RpcLease.ToProtobufOutcomeReasons(reason));
				newRpcLease.Output = (result.Output != null) ? ByteString.CopyFrom(result.Output) : ByteString.Empty;
			}

			newRpcLease.Metrics = CalculateLeaseMetrics(cpuTimeStart, cpuTimeEnd, leaseDuration.Elapsed, span, logger);
		...
		protected static async Task<int> RunProcessAsync(string executable, IEnumerable<string> arguments, IReadOnlyDictionary<string, string>? environment, ILogger logger, CancellationToken cancellationToken)
		{
			...
			try
			{
				...
				return await RunProcessCoreAsync(process, logger, cancellationToken);
			}
			catch (Exception ex)
			{
		...
		/// <summary>
		/// Core process execution loop
		/// </summary>
		private static async Task<int> RunProcessCoreAsync(ManagedProcess process, ILogger logger, CancellationToken cancellationToken)
		{
			for (; ; )
			{
				...
				if (JsonLogEvent.TryParse(line, out JsonLogEvent jsonLogEvent))
				{
		...
		private static RpcLeaseMetrics CalculateLeaseMetrics(CpuTimeMetrics? cpuTimeStart, CpuTimeMetrics? cpuTimeEnd, TimeSpan leaseDuration, TelemetrySpan span, ILogger logger)
		{
			RpcLeaseMetrics metrics = new()
			{
				SetupTimeMs = 0, // TODO: Implement
				TeardownTimeMs = 0, // TODO: Implement
				GlobalCpuUtilization = 0.0f,
				CpuCount = Environment.ProcessorCount,
			};
			try
			{
				if (cpuTimeStart != null && cpuTimeEnd != null)
				{
					CpuTimeMetrics cpuTimeSpent = cpuTimeEnd.Subtract(cpuTimeStart);
					TimeSpan totalCpuTimeSpent = cpuTimeSpent.User + cpuTimeSpent.System;
					TimeSpan totalCpuTimeAvailable = Environment.ProcessorCount * leaseDuration;
				
					// Prevent the unlikely divide by zero below
					totalCpuTimeAvailable = totalCpuTimeAvailable == TimeSpan.Zero ? TimeSpan.FromMilliseconds(1) : totalCpuTimeAvailable;
				
					metrics.GlobalCpuUtilization = totalCpuTimeSpent / totalCpuTimeAvailable;
					metrics.GlobalCpuTime = new RpcLeaseMetricCpuTime
					{
						UserMs = (uint)Math.Min(uint.MaxValue, cpuTimeSpent.User.TotalMilliseconds),
						SystemMs = (uint)Math.Min(uint.MaxValue, cpuTimeSpent.System.TotalMilliseconds),
						IdleMs = (uint)Math.Min(uint.MaxValue, cpuTimeSpent.Idle.TotalMilliseconds),
					};
					span.SetAttribute("horde.lease.cpu.time_spent_ms", (uint)totalCpuTimeSpent.TotalMilliseconds);
					span.SetAttribute("horde.lease.cpu.time_available_ms", (uint)totalCpuTimeAvailable.TotalMilliseconds);
					span.SetAttribute("horde.lease.cpu.utilization", metrics.GlobalCpuUtilization);
					span.SetAttribute("horde.lease.cpu.num_cores", metrics.CpuCount);
				}
			}
			catch (Exception e)
			{
				logger.LogWarning(e, "Failed to calculate lease metrics");
			}
			return metrics;
		}
	}
