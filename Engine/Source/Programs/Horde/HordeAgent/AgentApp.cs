...
#pragma warning restore IL3000 // Avoid accessing Assembly file path when publishing as a single file		
		...
		public static async Task<int> Main(string[] args)
		{
			...
			services.AddSingleton<RuntimeWatchdog>();
			services.AddHostedService(sp => sp.GetRequiredService<RuntimeWatchdog>());
		...
		static ISystemMetrics CreateSystemMetrics(DirectoryReference workingDir, ILogger logger)
		{
			try
			{
				...
				{
					return new MacSystemMetrics(workingDir);
				}
			}
			catch (Exception e)
			{
		...
		internal static void LogGcStats(ILogger logger)
		{
			string concurrentGc = "<unknown>";
			if (GC.GetConfigurationVariables().TryGetValue("ConcurrentGC", out object? concurrentGcObj))
			{
				concurrentGc = concurrentGcObj?.ToString() ?? "null";
			}
			
			logger.LogInformation("GC: ServerGc={ServerGc} LatencyMode={LatencyMode} ConcurrentGc={ConcurrentGc}",
				GCSettings.IsServerGC, GCSettings.LatencyMode, concurrentGc);
		}
	}
}
