...
import { AuditLogEntry, AuditLogQuery, GetUserResponse } from "horde/backend/Api";
...
/// Test description
export type TestSessionDescriptionResponse = {
    ...
    /// The result profile associated with this test
    profile?: string;
...
/// Test name reference
export type GetTestNameResponse = TestSessionDescriptionResponse & {
...
export enum TestPhaseOutcome {
    ...
    /// The test phase was successful but with warning
    Warning = "Warning",
...
/// Error Fingerprint Entry
export type GetErrorFingerprintEntry = {

    /// The error fingerprint key
    key: string;

    /// The list of impacted phases
    phases: string[];
}
...
export type GetTestSessionResponse = {
    ...
    /// The recipe associated with the test session
    recipeId?: string;
    ...
    /// The error finerprints associated with this test session
    errorFingerprints?: GetErrorFingerprintEntry[];
}
...
export type GetTestPhaseSessionResponse = {
    ...
    /// The tag ids associated with the phase session
    tagIds?: string[]
}
...
export type TestSessionSummaryResponse = {
    ...
    /// The command line used to run the session
    commandline?: string;
}
...
export type PhaseSessionDetailsResponse = {
    ...
    /// The tag associated with the phase session
    tags?: string[]
}
...
export type TestSessionRecipeResponse = {
    ...
    /// The timeout in minutes used by this recipe
    timeoutMinutes: number,

    /// The command line parameters associated with this recipe
...
export type TestSessionDetailsResponse = {
    ...
    /// The test description associated with the test session
    description?: TestSessionDescriptionResponse;

    /// The test recipe associated with the test session
    recipe?: TestSessionRecipeResponse;
}
...
export type TestRecipeRequest = {

    /// The recipe ids requested 
    recipeIds?: string[],

    /// The test ids associated with the recipe requested
    testIds?: string[],

    /// The stream ids associated with the recipe requested
    streamIds?: string[]
}

export type TestRecipeResponse = TestSessionRecipeResponse & {

    /// The test recipe id
    id: string,

    /// The test name reference
    nameRef: string,
...
export type TestAuditPost = {

    /// Whether or not the test is scheduled to run
    enableSchedule: boolean,

    /// The date of the last audit (ISO formatted)
    ...
    /// The customers of the test
    customers?: string[],

    /// The notification settings as stream/workflow pair
    notifications?: Record<string, string>,

    // The team associated with this test
    team?: string,

    /// The hardness associated with this test
    harness?: string,
    ...
    /// The notes associated with this audit post
    notes?: string
}

export type TestAuditPostRequest = TestAuditPost & {

    /// The test id associated with the audit post
    testId: string
}

export type TestAuditResponse = TestAuditPost & {

    /// The test id associated with this audit
    testId: string,

    /// The used id associated with this audit
    userId: string,

    /// The date of the audit post
...
export async function getTestRecipesV2(recipeIds?: string[], testIds?: string[], streamIds?: string[]): Promise<TestRecipeResponse[]> {
    const request: TestRecipeRequest = { recipeIds: recipeIds, testIds: testIds, streamIds: streamIds };
    return new Promise<TestRecipeResponse[]>((resolve, reject) => {
        backend.fetch.post(`/api/v3/testdata/recipes`, request).then((value) => {
            resolve(value.data as TestRecipeResponse[]);
        }).catch((reason) => {
            reject(reason);
        });
    });
...
export async function postTestAuditV2(testId: string, auditPost: TestAuditPost): Promise<TestAuditResponse> {
    const request: TestAuditPostRequest = { testId: testId, ...auditPost };
    return new Promise<TestAuditResponse>((resolve, reject) => {
        backend.fetch.post(`/api/v3/testdata/audit/post`, request).then((value) => {
            resolve(value.data as TestAuditResponse);
        }).catch((reason) => {
            reject(reason);
        });
    });
...
export async function getTestAuditsV2(testIds: string[], minCreateTime?: string): Promise<TestAuditResponse[]> {
    const query: any = {};
    if (minCreateTime) {
        query.minCreateTime = minCreateTime;
    }
    const request = { testIds: testIds };
    return new Promise<TestAuditResponse[]>((resolve, reject) => {
        backend.fetch.post(`/api/v3/testdata/audit/history`, request, query).then((value) => {
            resolve(value.data as TestAuditResponse[]);
        }).catch((reason) => {
            reject(reason);
        });
...
export async function getTestAuditLogHistoryV2(id: string, query: AuditLogQuery): Promise<AuditLogEntry[]> {
    return new Promise<AuditLogEntry[]>((resolve, reject) => {
        backend.fetch.get(`/api/v3/testdata/audit/log/${encodeURIComponent(id)}`, { params: query }).then((response) => {
...
export async function getUsersInfo(userIds: string[]): Promise<GetUserResponse[]> {
