...
import { IPhaseFilter } from './testAutomationCommon';

export const defaultQueryWeeks = 2;
const filterRefreshMS = 200;
export const consecutivePassesThreshold = 4;
...
type TestRecipeId = string;
type ErrorFingerprintId = string;
...
export interface TestVersionFetcher {
    ...
    fetchPhaseSessions(streamRef: TestStreamRef, status: TestMetaStatus, phaseKeys: string[], weeks: number): Promise<boolean>;

    /// fetch test phases and populate test status phase
    fetchTestPhases(status: TestMetaStatus, phaseKeys?: string[]): Promise<void>;
    ...
    /// fetch test recipes and attached them to their associated test
    fetchTestRecipes(testRef: TestNameRef, recipeIds: TestRecipeId[]): Promise<void>;

    /// fetch test audits associated with a list of tests
    fetchTestAudit(testRefs: TestNameRef[], auditMap: Map<TestNameRef, TestAuditRecords>, weeks?: number): Promise<void>;
...
export class PhaseSessionResult {
    ...
    stepId?: string;
    hasWarning?: boolean;
    errorFingerprint?: ErrorFingerprintId;
    tagIds?: TagId[];
}
...
export class TestSessionResult {
    ...
    errorFingerprints?: Map<ErrorFingerprintId, PhaseKey[]>;
...
export class TestSessionStatus {
    ...
    sortSessions() {
        ...
        this.history.sort((a, b) => {
            if (b.commitOrder !== a.commitOrder) {
                return b.commitOrder - a.commitOrder;
            }
            // if commitOrder is equal, sort by date descending
    ...
    sortPhaseSessions(phaseId: string) {
        /// sort phase session history by commit order descending, then by date descending
        this.phaseHistories.get(phaseId)?.sort((a, b) => {
            if (b.commitOrder !== a.commitOrder) {
                return b.commitOrder - a.commitOrder;
            }
            // if commitOrder is equal, sort by date descending
    ...
    includeTags(tags: TestTagRef[]): boolean {
        const sessionTagIds = this.getLastSession()?.tagIds;
        return !!sessionTagIds && tags.some(tag => sessionTagIds.some(tagId => tag.id === tagId));
    }
...
export class DeviceRef {
    ...
    isSame(other: DeviceRef): boolean {

        if (this.name === other.name)
        {
            const metaKeys = new Set(Object.keys(this.metadata));
            const otherMetaKeys = new Set(Object.keys(other.metadata));
            if (metaKeys.size === otherMetaKeys.size && metaKeys.difference(otherMetaKeys).size === 0) {
                const metaValues = new Set(Object.values(this.metadata));
                const otherMetaValues = new Set(Object.values(other.metadata));
                return metaValues.difference(otherMetaValues).size === 0;
            }
        }
...
export abstract class TestPhaseStatus implements IPhaseFilter {
    ...
    isMatch(names: string[], outcomes?: Set<TestPhaseOutcome>, tags?: string[]) {
        if (!!outcomes && !outcomes.has(this.outcome)
            && (!outcomes.has(TestPhaseOutcome.Warning) || this.outcome !== TestPhaseOutcome.Success || !this.hasWarning)) /** allow filtering of success but with warning */
                return false;
        const lowerTags = this.tags?.map(t => t.toLowerCase());
        if (!!tags && (!lowerTags || !lowerTags.some((t) => tags.includes(t)))) return false;
...
export class TestNameRef {
    ...
    includesTags(toMatch: string[]): boolean {
        return toMatch.some(tag => this.tagRefs?.values().some(r => r.isMatch(tag)));
    }

    isMetaMatch(metaFilter: MetadataFilter): boolean {
        // match test metadata (not session metadata)
        for (const [key, values] of metaFilter.entries()) {
            ...
            const toValidate = values.some((v) => v.indexOf('+') > -1) ? [thisValue.toLowerCase()] : thisValue.split('+').map((v) => v.toLowerCase());
    ...
    updateRecipes(recipes: TestRecipeRef[]) {
        const newRecipes = new Set<TestRecipeRef>(recipes);
        this.recipeRefs = this.recipeRefs? this.recipeRefs.union(newRecipes) : newRecipes;
    }

    getRecipe(recipeId: TestRecipeId) {
        return this.recipeRefs?.keys().find(r => r.id === recipeId);
    }
...
export class MetadataRef {
    ...
    isMatch(metaFilter: MetadataFilter, extact?: boolean): boolean {
        if (extact && Object.keys(this.entries).length !== metaFilter.size) {
            return false;
        }
        for (const [key, values] of metaFilter.entries()) {
            ...
            if (!toValidate.some((v) => values.includes(v)) || (extact && values.length  !== toValidate.length)) {
                return false;
            }
        }
        return true;
    ...
    private static keysOrderPriority = ["Project", "Platform", "BuildTarget"]; // Should those be set from a config?
...
export class TestRecipeRef {
    constructor(id: string, streamId: string, jobTemplateId: string, jobStep: string, nodeClass: string, testVersion: string, timeoutMinutes: number, version: number) {
        this.id = id;
        this.streamId = streamId;
    ...
    isMatch(recipeId: string) {
        return this.id === recipeId;
    }

    includeParams(paramKeys: string[]) {
        return paramKeys.some(key => this.params?.has(key));
    }

    isParamValueMatch(key: string, value: any) {
        return !!this.params && this.params.get(key) === value;
    }
...
export class TestAudit {
    constructor(testId: string, userId: string, userInputDate: Date, enableSchedule: boolean) {
        this.testId = testId;
        this.userId = userId;
    ...
    clone(): TestAudit {
        return {
            ...
            notifications: this.notifications? new Map(this.notifications.entries()) : undefined
        };
    }
    ...
    owner?: string;
    customers?: string[];
    notifications?: Map<StreamId, WorkflowId>;
    team?: string;
    harness?: string;
    intent?: string;
    profile?: string;
...
export abstract class TestAuditRecords {
    constructor(test: TestNameRef) {
        this.history = [];
        this.test = test;
    }
    ...
    appendRecord(audit: TestAudit) {
        this.history.push(audit);
    }

    sortRecords() {
        this.history.sort((a, b) => b.userInputDate.getTime() - a.userInputDate.getTime());
    }

    abstract post(entry: TestAudit): Promise<boolean>;
...
export class TestStreamRef {
    ...
    addTest(item: TestNameRef) {
        ...
        // test metadata
        if (item.team) {
            this.testmetaMap.get('team')!.add(item.team);
        }
        if (item.harness) {
            this.testmetaMap.get('harness')!.add(item.harness);
        }
        if (item.profile) {
            this.testmetaMap.get('profile')!.add(item.profile);
        }
    }
    ...
    getAllTestMetaKeys(): string[] {
        return this.testmetaMap.keys().toArray();
    }

    getAllTestMetaValues(key: string): string[] | undefined {
        return this.testmetaMap.get(key)?.keys().toArray();
    }
    ...
    getMatchingTests(toMatchNames?: string[], toMatchTags?: string[], toMatchMeta?: MetadataFilter): Set<TestNameRef> {
        const lowerToMatchNames = toMatchNames?.map((i) => i.toLowerCase());
        const lowerToMatchTags = toMatchTags?.map((i) => i.toLowerCase());
        const lowerToMatchMeta = toMatchMeta? new Map(toMatchMeta.entries().map(([key, values]) => [key, values.map((v) => v.toLowerCase())])) : undefined;
        ...
        this.tests.forEach((test) => {
            ...
            const isMatchMeta = !lowerToMatchMeta || test.isMetaMatch(lowerToMatchMeta);
            if (isMatchNames && isMatchTags && isMatchMeta) {
                matches.add(test);
            }
        })
    ...
    getMatchingTags(toMatchTags?: string[]): Set<TestTagRef> {
        const lowerToMatchTags = toMatchTags?.map((i) => i.toLowerCase());
        const matches: Set<TestTagRef> = new Set();
        this.tags.forEach((tag) => {
            if (!lowerToMatchTags || lowerToMatchTags.some((t) => tag.isMatch(t))) {
                matches.add(tag);
            }
        });
    ...
    private testmetaMap: Map<string, Set<string>>;
}
...
export class TestDataHandler {
    ...
    private checkSearchCallback(previous: string): boolean {
    ...
    setSearchParam(key: string, value: string | string[], replace?: boolean) {
        const previous = this.search.toString();
        this.search.set(key, null, value);
        this.checkSearchCallback(previous) && replace && this.queueHistoryReplace();
    }
    ...
    addTestMeta(key: string, value: string) {

        const state = this.filterState;
        const meta = state.testmeta?.get(key);
        ...
        if (!state.testmeta) {
            state.testmeta = new Map();
        }

        if (!meta) {
            state.testmeta!.set(key, [value]);
        } else {
            state.testmeta!.get(key)!.push(value);
        }

        this.setFilterUpdated();
    }

    removeTestMeta(key: string, value?: string) {
    ...
    /// All test metadata keys from current stream
    get allStreamTestMetaKeys(): string[] {
        ...
        return testStreamRef.getAllTestMetaKeys().sort((a, b) => a.localeCompare(b));
    }

    /// All test metadata values for a key from current stream
    getAllStreamTestMetaValues(key: string): string[] {
        ...
        return testStreamRef.getAllTestMetaValues(key)?.sort((a, b) => a.localeCompare(b)) ?? [];
    }
    ...
    get filteredTests(): TestNameRef[] {
        ...
        const testNameRefs = testStreamRef!.getMatchingTests(state.tests, state.tags, state.testmeta);
    ...
    /// Tags that match current tag filters, return undefined if no tag is selected
    get selectedTags(): TestTagRef[] | undefined {
        ...
        return testStreamRef!.getMatchingTags(lowerTags).keys().toArray().sort((a, b) => a.name.localeCompare(b.name));
    }
    ...
    private checkUpdateFilter(): boolean {
        ...
        state.testmeta = state.testmeta && state.testmeta.size > 0 ? state.testmeta : undefined;
        state.tests = state.tests?.sort((a, b) => a.localeCompare(b));
        state.tags = state.tags?.sort((a, b) => a.localeCompare(b));
        ...
        needUpdate = needUpdate || search.get('tags')?.toString() !== state.tags?.toString();
        if (needUpdate) {
            search.set("tags", null, state.tags);
        }
        ...
        const testMetaKeys = new Set<string>(search.getGroup("tmeta")?.keys());
        testMetaKeys.union(new Set(state.testmeta?.keys())).keys().forEach(key => {
            needUpdate = needUpdate || search.get('tmeta', key)?.toString() !== state.testmeta?.get(key)?.toString();
            search.set("tmeta", key, state.testmeta?.get(key));
        });
    ...
    private filterFromSearch(searchInfo: string): TestDataState {
        ...
        const testmeta = search.getGroup("tmeta");
        ...
        state.testmeta = testmeta && testmeta.size > 0 ? testmeta : undefined;
        state.tests = tests;
        state.tags = tags;
        ...
        for (const fetcherVersion of TestDataVersionRegistrar.manifest) {
            ...
            try {
                ...
                if (sessionDetails) {
                    ...
                    this.query(sessionDetails.streamId).then(() => {
    ...
    async queryPhaseSessions(phases: TestPhaseStatus[], streamId: StreamId, isSubQuery: boolean = false) {

        if (phases.length === 0) {
            return;
        }

        const test = phases[0].test!;
        if (!phases.every(phase => phase.test === test)) {
            console.error('Phase sessions query is not for the same test');
            return;
        }

        const testStatus = this.testStatus.get(streamId)?.tests.get(test);
        const phaseKeys = Array.from(new Set(phases.map(phase => phase.key)));

        const associatePhaseSessions = () => {
            if (testStatus) {
                phases.forEach(phase => {
                    if (!phase.session) {
                        /// associate the corresponding phase session with the phase status
                        phase.associateSession(testStatus.getPhaseSessions(phase.key, phase.testMetadata));
                        /// associate the corresponding phase reference
                        phase.phaseRef = test.phases?.get(phase.key);
                    }
                });
            }
        }

        // Check if all phases already have sessions
        if (!testStatus || phaseKeys.every(phaseKey => testStatus.hasPhaseSession(phaseKey))) {
            associatePhaseSessions();
        ...
        for (const fetcherVersion of TestDataVersionRegistrar.manifest) {
            ...
            if (fetcherVersion.version === test.version) {
                ...
                try {
                    await fetcherVersion.fetchPhaseSessions(testStreamRef, testStatus, phaseKeys, queryWeeks);
                } catch (reason) {
        ...
        /// sort phase session history by commit order descending for all phases
    ...
    async queryTestPhases(test: TestMetaStatus, phaseKeys?: string[]) {

        if (!!phaseKeys && phaseKeys.every(phaseKey => test.test.phases?.get(phaseKey))) {
            this.setPhaseQueryLoading(false);
            return;
        }

        this.setPhaseQueryLoading(true);

        for (const fetcherVersion of TestDataVersionRegistrar.manifest) {
            /// fetch only from the matching version
            console.debug(`Fetching from version ${fetcherVersion.version} test phases`);
            try {
                await fetcherVersion.fetchTestPhases(test, phaseKeys);
                break;
            } catch (reason) {
                /** ignore failed attempt to try with next version */
                console.debug(`Failed to fetch phases from version ${fetcherVersion.version}: ${reason}`);
            }
        }
    ...
    async queryRecipes(test: TestNameRef, recipeIds: TestRecipeId[]) {

        const missingRecipes = recipeIds.filter(r => !test.getRecipe(r));
        if (missingRecipes.length === 0) {
            this.setRecipeQueryLoading(false);
            return;
        }

        this.setRecipeQueryLoading(true);

        for (const fetcherVersion of TestDataVersionRegistrar.manifest) {
            /// fetch only from the matching version
            if (fetcherVersion.version === test.version) {
                console.debug(`Fetching from version ${fetcherVersion.version} test recipes`);
                try {
                    await fetcherVersion.fetchTestRecipes(test, missingRecipes);
                } catch (reason) {
                    console.debug(`Failed to fetch recipes from version ${fetcherVersion.version}: ${reason}`);
                }
                break;
            }
        }
    ...
    async queryAudits(tests: TestNameRef[], withHistory?: boolean) {

        const missingTests = tests.filter(t => !this.testAudits.get(t) || (withHistory && this.testAudits.get(t)!.history.length === 1));

        if (missingTests.length === 0) {
            this.setAuditQueryLoading(false);
            return;
        }

        let queryWeeks: number | undefined;
        if (withHistory) {
            const state = this.filterState;
            queryWeeks = state.weeks && state.weeks > defaultQueryWeeks ? state.weeks : defaultQueryWeeks;
        }

        this.setAuditQueryLoading(true);

        for (const fetcherVersion of TestDataVersionRegistrar.manifest) {
            console.debug(`Fetching from version ${fetcherVersion.version} test audits`);
            try {
                await fetcherVersion.fetchTestAudit(missingTests, this.testAudits, queryWeeks);
            } catch (reason) {
                console.debug(`Failed to fetch audits from version ${fetcherVersion.version}: ${reason}`);
            }
        }

        if (withHistory) {
            missingTests.forEach(test => {
                const records = this.testAudits.get(test);
                records?.sortRecords();
            });
        }
    ...
    @action
    setRecipeQueryLoading(value: boolean) {
        this.queryRecipesLoading = value;
    }

    @action
    setAuditQueryLoading(value: boolean) {
        this.queryAuditsLoading = value;
    }
    ...
    @observable
    queryRecipesLoading: boolean = false;

    @observable
    queryAuditsLoading: boolean = false;
    ...
    subscribeToRecipesLoading() {
        if (this.queryRecipesLoading) { }
    }

    subscribeToAuditsLoading() {
        if (this.queryAuditsLoading) { }
    }
    ...
    testAudits: Map<TestNameRef, TestAuditRecords> = new Map();
