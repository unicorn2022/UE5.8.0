...
import { CollapseAllVisibility, ConstrainMode, DetailsHeader, DetailsList, DetailsListLayoutMode, DetailsRow, DirectionalHint, FontIcon, IColumn, Icon, IDetailsListProps, ProgressIndicator, SelectionMode, Stack, Text, TooltipHost } from "@fluentui/react";
...
import { getStepMessageInfo } from "./JobDetailsUtils";
...
const RenderDynamic: React.FC<{ jobDetails: JobDetailsV2, dataView: StepsDataView, step?: StepData, batch?: GetBatchResponse, column: string }> = observer(({ jobDetails, dataView, column, step, batch }) => {
   ...
   if (step && column === "Progress") {
      ...
      // Handle step message
      const report = step?.reports?.find(r => !!r.message);
      const { message: stepMessage, color: stepMessageColor } = getStepMessageInfo(report);
      if (stepMessage) {
         return <Stack horizontal verticalFill verticalAlign="center" tokens={{ childrenGap: 6 }} horizontalAlign="center">
            {!!stepMessageColor && <FontIcon style={{ fontSize: 13, color: stepMessageColor }} iconName="Square" />}
            <Text variant="tiny" style={{ overflow: "hidden", textOverflow: "ellipsis", wordBreak: "normal" }}>{stepMessage}</Text>
         </Stack>
      }
...
const StepsPanelInner: React.FC<{ jobDetails: JobDetailsV2, depStepId?: string }> = observer(({ jobDetails, depStepId }) => {
   ...
   function renderAgentRow(item: StepItem, index?: number, column?: IColumn) {
      ...
      // Create tooltip content from leaseOutcomeReasons
      const leaseOutcomeReasonsTooltip = item.batch?.leaseOutcomeReasons?.length ? (
         <Stack tokens={{ childrenGap: 4 }}>
            <Text style={{ fontWeight: 600 }}>Cancellation reasons</Text>
            {item.batch.leaseOutcomeReasons.map((reason, idx) => (
               <Text key={idx}>• {reason}</Text>
            ))}
         </Stack>
      ) : undefined;

      // Helper to wrap element with tooltip if reasons exist
      const wrapWithTooltip = (element: JSX.Element) => leaseOutcomeReasonsTooltip
         ? <TooltipHost content={leaseOutcomeReasonsTooltip} id={`batch_reasons_${item.batch?.id}`} directionalHint={DirectionalHint.rightCenter}>{element}</TooltipHost>
         : element;
      ...
      switch (column!.key) {
         ...
         case 'Name':
            if (!item.agentId) {
               const batchTextElement = <Text styles={{ root: { color: errorColor, fontWeight: errorWeight, paddingRight: leaseOutcomeReasonsTooltip ? 4 : undefined } }} nowrap={true}>{batchText}</Text>;
               if (item.agentPool && (item.batch?.state === JobStepBatchState.Ready || item.batch?.error === JobStepBatchError.NoAgentsOnline || item.batch?.error === JobStepBatchError.NoAgentsInPool)) {
                  return <Stack horizontal disableShrink={true} horizontalAlign="start">
                     ...
                     <Link to={`/pools?pool=${encodeURI(item.agentPool)}`}>{wrapWithTooltip(batchTextElement)}</Link>
                  </Stack>
               }
               return <Stack horizontal disableShrink={true} horizontalAlign="start">
                  {(error || warning) && <Icon styles={{ root: { paddingTop: 3, paddingRight: 8, color: statusColors.get(StatusColor.Failure), fontWeight: errorWeight } }} iconName="Error" />}
                  {wrapWithTooltip(batchTextElement)}
               </Stack>;
            } else {
               const batchTextElement = <Text styles={{ root: { cursor: 'pointer', color: errorColor, fontWeight: errorWeight, paddingRight: leaseOutcomeReasonsTooltip ? 4 : undefined } }}>{batchText}</Text>;
               return <Stack horizontal disableShrink={true}>{(error || warning) && <Icon styles={{ root: { paddingTop: 3, paddingRight: 8, color: statusColors.get(StatusColor.Failure), fontWeight: errorWeight } }} iconName="Error" />}
                  <Stack>
                     <Link to={url} onClick={(ev) => { ev.stopPropagation(); ev.preventDefault(); setLastSelectedAgent(item.agentId); }}>{wrapWithTooltip(batchTextElement)}</Link>
                  </Stack>
               </Stack>
            }
         case 'Duration':
   ...
   if (stepId) {
      ...
      const getStepsRecursive = (stepId: string) => {
         ...
         [rstep.inputDependencies, rstep.optionalInputDependencies, rstep.orderDependencies].flat().forEach(id => {
