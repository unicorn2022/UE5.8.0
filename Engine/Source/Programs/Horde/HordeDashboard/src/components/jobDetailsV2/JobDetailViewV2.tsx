...
import { getStreamDisplayName, getStreamLink } from "../../base/utilities/streamUtils";
...
const JobBreadCrumbs: React.FC<{ jobDetails: JobDetailsV2 }> = observer(({ jobDetails }) => {
   ...
   if (stream) {
      ...
      let streamLink = getStreamLink(stream.id, aliasParam);
      if (tab) {
         streamLink += aliasParam ? `&tab=${tab.title}` : `?tab=${tab.title}`;
      }

      crumbItems.push({ text: stream.project!.name, link: `/project/${stream.project!.id}` });
      crumbItems.push({ text: getStreamDisplayName(stream, aliasParam), link: streamLink });
   ...
   let clText = "";
   const preflightDisplay = getPreflightDisplay(jobData.preflightCommitId, jobData.preflightChange);
   if (preflightDisplay) {
      clText = `Preflight ${preflightDisplay} `;
   ...
   if (stream) {
      crumbTitle = `${stream.fullname} - ${jobData.name} - ${clText}`;
   }
