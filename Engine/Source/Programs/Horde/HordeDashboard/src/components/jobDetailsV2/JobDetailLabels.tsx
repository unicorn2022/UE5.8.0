...
import { LabelHoverCard } from "../labels/LabelHoverCard";
         ...
         case 'Labels':
            return <Stack wrap horizontal tokens={{ childrenGap: 4 }} styles={{ root: { paddingTop: 2 } }}>
               {item.labels.map(label => {
                  ...
                  const labelIdx = label.stateResponse.dashboardName?.length
                     ? jobDetails.labelIndex(label.stateResponse.dashboardName, label.stateResponse.dashboardCategory)
                     : -1;

                  const badge = <Stack key={`labels_${label.stateResponse.dashboardCategory}_${label.stateResponse.dashboardName}`} className={hordeClasses.badge}>
                     <DefaultButton
                        onClick={() => {
                           if (qlabel?.stateResponse.dashboardCategory === label.stateResponse.dashboardCategory && qlabel?.stateResponse.dashboardName === label.stateResponse.dashboardName) {
                              ...
                              if (labelIdx >= 0) {
                                 navigate(location.pathname + `?label=${labelIdx}`)
                              } else {
                  ...
                  return <LabelHoverCard
                     key={`labelhover_${label.stateResponse.dashboardCategory}_${label.stateResponse.dashboardName}`}
                     jobId={jobDetails.jobId!}
                     labelIndex={labelIdx}
                     streamId={jobDetails.jobData?.streamId ?? ''}
                     templateId={jobDetails.jobData?.templateId ?? ''}
                     templateName={jobDetails.jobData?.name}
                     dashboardName={label.stateResponse.dashboardName}
                     dashboardCategory={label.stateResponse.dashboardCategory}
                     state={label.stateResponse.state}
                     outcome={label.stateResponse.outcome}
                     change={jobDetails.jobData?.change}
                  >
                     {badge}
                  </LabelHoverCard>;
               })}</Stack>;
