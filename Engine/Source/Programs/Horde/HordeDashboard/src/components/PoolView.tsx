   ...
   async poll(): Promise<void> {
      ...
      try {
         ...
         if (streamJobIds.size) {
            ...
            const streamJobs = await backend.getJobs({ id: Array.from(streamJobIds), filter: "id,name,batches,change,createTime,streamId,preflightChange,preflightCommitId" });
         ...
         if (jobIds.length) {
            ...
            const query: JobQuery = {
               id: jobIds,
               filter: "id,name,batches,change,createTime,streamId,preflightChange,preflightCommitId",
            }
...
const PoolAgentPanel: React.FC<{ poolId: string }> = ({ poolId }) => {
   ...
   return <Stack styles={{ root: { paddingTop: 18, paddingLeft: 12, paddingRight: 12 } }} >
      <Stack tokens={{ childrenGap: 12 }}>
         ...
         <AgentPanel poolId={poolId} agents={handler.agents} />
      </Stack>
   </Stack>
}
...
const AutoScalerPanel: React.FC = () => {
   ...
   const disabled = !dashboard.user?.dashboardFeatures?.showPoolEditor || state.submitting || pool.configManaged === true;
...
const PoolPanel: React.FC = () => {
   ...
   const [state, setState] = useState<{ showSettings?: boolean, showDeleteShadow?: boolean, deletingShadow?: boolean, deleteError?: string }>({})
   ...
   const onDeleteShadow = async () => {
      setState({ ...state, deletingShadow: true, deleteError: undefined });
      try {
         await backend.deletePool(pool.id);
         handler.pool = await backend.getPool(pool.id);
         setState({ showDeleteShadow: false, deletingShadow: false });
      } catch (reason) {
         setState({ ...state, deletingShadow: false, deleteError: `${reason}` });
      }
   };
            ...
            <Stack horizontal tokens={{ childrenGap: 48 }}>
               <Stack style={{ minWidth: 224 }}>
                  <Stack horizontal style={{ paddingBottom: 18 }} tokens={{ childrenGap: 8 }}>
                     ...
                     {!!dashboard.user?.dashboardFeatures?.showPoolEditor && pool.configManaged !== true && <IconButton iconProps={{ iconName: "Edit" }} onClick={() => { setState({ ...state, showSettings: true }) }} style={{ color: "#FFFFFF", backgroundColor: color, border: "unset", flexShrink: 1 }} />}
                  </Stack>
                  {pool.configManaged === true && pool.hasDatabaseEntry === true && (
                     <Stack tokens={{ childrenGap: 8 }} styles={{ root: { marginBottom: 8 } }}>
                        <MessageBar messageBarType={MessageBarType.warning} isMultiline={false}>
                           This pool is managed via config files but also has a database entry (shadow pool). The database entry can be deleted to complete migration.
                        </MessageBar>
                        <DefaultButton text="Delete Database Entry" iconProps={{ iconName: "Delete" }} onClick={() => setState({ ...state, showDeleteShadow: true })} />
                        {!!state.showDeleteShadow && <Dialog
                           hidden={false}
                           onDismiss={() => setState({ ...state, showDeleteShadow: false, deleteError: undefined })}
                           minWidth={480}
                           dialogContentProps={{
                              type: DialogType.largeHeader,
                              title: `Delete Shadow Pool Entry`,
                              subText: `This will delete the database entry for pool "${pool.name}". The pool will continue to exist via config files.`
                           }}
                           modalProps={{ isBlocking: true }}>
                           {!!state.deleteError && <MessageBar messageBarType={MessageBarType.error}>{state.deleteError}</MessageBar>}
                           <DialogFooter>
                              <PrimaryButton onClick={onDeleteShadow} text={state.deletingShadow ? "Deleting..." : "Delete"} disabled={state.deletingShadow} />
                              <DefaultButton onClick={() => setState({ ...state, showDeleteShadow: false, deleteError: undefined })} text="Cancel" />
                           </DialogFooter>
                        </Dialog>}
                     </Stack>
                  )}
                  {pool.configManaged === true && pool.hasDatabaseEntry !== true && (
                     <MessageBar messageBarType={MessageBarType.info} isMultiline={false} styles={{ root: { marginBottom: 8 } }}>
                        This pool is managed via config files. To modify settings, edit pools.global.json in Perforce.
                     </MessageBar>
                  )}
                  <Stack>
                     <DetailsList
