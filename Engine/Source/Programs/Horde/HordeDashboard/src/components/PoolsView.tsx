...
import { DefaultButton, DetailsHeader, DetailsList, FontIcon, IColumn, IDetailsHeaderStyles, IDetailsListProps, ITag, Icon, Pivot, PivotItem, SelectionMode, Spinner, SpinnerSize, Stack, Sticky, StickyPositionType, TagPicker, Text, TooltipHost, mergeStyleSets, mergeStyles } from "@fluentui/react";
...
const PoolList: React.FC = observer(() => {
   ...
   const columns: IColumn[] = [{
      ...
      onRender: (pool: GetPoolSummaryResponse) => {
         ...
         return <Stack horizontal verticalAlign="center" verticalFill tokens={{ childrenGap: 4 }}>
            <DefaultButton
               text={pool.name}
               primary
            ...
            {pool.configManaged === true && pool.hasDatabaseEntry === true && (
               <TooltipHost content="Shadow pool - has database entry that can be cleaned up">
                  <Icon iconName="Warning" styles={{ root: { fontSize: 12, color: statusColors.get(StatusColor.Warnings) }}} />
               </TooltipHost>
            )}
            {pool.configManaged === true && pool.hasDatabaseEntry !== true && (
               <TooltipHost content="Managed via config files (read-only)">
                  <Icon iconName="Lock" styles={{ root: { fontSize: 12, color: statusColors.get(StatusColor.Skipped) }}} />
               </TooltipHost>
            )}
         </Stack>
      }
...
const PoolPicker: React.FC = observer(() => {
   ...
   const poolTags: ITag[] = handler.pools
