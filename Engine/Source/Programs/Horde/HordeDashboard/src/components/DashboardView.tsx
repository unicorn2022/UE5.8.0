...
const colorBlind1 = new Map<StatusColor, string>([
   ...
   [StatusColor.User1, "#8e4747"],
   [StatusColor.User2, "#785EF0"],
   [StatusColor.User3, "#FE6100"]
...
const statusText = new Map<StatusColor, string>([
   ...
   [StatusColor.User1, "User1"],
   [StatusColor.User2, "User2"],
   [StatusColor.User3, "User3"]
]);
...
const statusPrefs = new Map<StatusColor, DashboardPreference>([
   ...
   [StatusColor.User1, DashboardPreference.ColorUser1],
   [StatusColor.User2, DashboardPreference.ColorUser2],
   [StatusColor.User3, DashboardPreference.ColorUser3]
]);
...
const ColorPreferenceDialog: React.FC<{ shown: boolean, statusIn: StatusColor, onClose: () => void }> = ({ shown, statusIn, onClose }) => {
   ...
   const suggestions = tolColors.map((c, index) => {
      return <DefaultButton key={`color_${index}`} style={{ backgroundColor: c, width: 80 }} text={statusText.get(status)} onClick={() => { setColor(c) }} />
   })
   ...
   const BadgeColors: React.FC = () => {
      ...
      const buttons = orderedStatus.map(status => {
         ...
         return <div className="horde-no-darktheme">
            <Stack className={hordeClasses.badgeNoIcon} key={`badge_${status}`}> <DefaultButton style={{ backgroundColor: state.colors.get(status) ?? defaultStatusColors.get(status) }} text={statusText.get(status)} onClick={() => { setStatus(status) }} />
            </Stack>
         </div>
      });
