...
import { DatePicker, Dropdown, DropdownMenuItemType, IDatePickerStrings, IDropdownOption, IDropdownStyles, ISliderStyles, mergeStyleSets, Slider, Stack, Callout, DirectionalHint } from '@fluentui/react';
...
export const UtilizationReportView: React.FC = observer(() => {
   ...
   const [pinnedCallout, setPinnedCallout] = useState<PinnedCalloutState | null>(null);
            ...
            if (isAllPoolKeys) {
               ...
               for (let streamKey in streamTotalTime) {
                  if (streamTotalTime[streamKey] !== 0) {
                     const streamTime = streamTotalTime[streamKey];
                     chartPoints.push({
                        legend: streamKey,
                        data: streamTime,
                        color: getPoolColor(streamKey),
                        yAxisCalloutData: `${(streamTime / totalTime).toLocaleString(undefined, { style: 'percent', maximumFractionDigits: 1 })} (${streamTime.toFixed(1)} agent hours)`
                     });
                              ...
                              <Stack className="horde-no-darktheme" style={{ paddingLeft: 12 }}>
                                 {!optionsState.updating ? <div>
                                    ...
                                    <VerticalStackedBarChart
                                       ...
                                       onBarClick={(event, data: any) => {
                                          if (pinnedCallout?.legend === data.legend) {
                                             setPinnedCallout(null);
                                          } else {
                                             setPinnedCallout({
                                                legend: data.legend,
                                                data: data.data,
                                                color: data.color,
                                                yAxisCalloutData: data.yAxisCalloutData,
                                                x: event.clientX,
                                                y: event.clientY
                                             });
                                          }
                                    ...
                                    {pinnedCallout && (
                                       <Callout
                                          target={{ x: pinnedCallout.x, y: pinnedCallout.y }}
                                          onDismiss={() => setPinnedCallout(null)}
                                          directionalHint={DirectionalHint.bottomLeftEdge}
                                          isBeakVisible={true}
                                          gapSpace={10}
                                          setInitialFocus
                                       >
                                          <div style={{ padding: 12, minWidth: 150 }}>
                                             <div style={{ fontWeight: 600, color: pinnedCallout.color, marginBottom: 4 }}>
                                                {pinnedCallout.legend}
                                             </div>
                                             <div>{pinnedCallout.yAxisCalloutData}</div>
                                          </div>
                                       </Callout>
                                    )}
                                 </div>
