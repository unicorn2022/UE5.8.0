...
import { IIconProps, mergeStyleSets, SearchBox, Stack } from '@fluentui/react';
...
export const SchedulePane: React.FC<{ streamId: string, templates: GetTemplateRefResponse[] }> = ({ streamId, templates }) => {
   ...
   const items = templates.filter(t => !!t.schedule?.enabled).map(t => {
      ...
      if (template.name.toLowerCase().includes(lowerSearch)) return true;

      const gateTemplateId = schedule.gate?.templateId;
      if (gateTemplateId) {
         const gateTemplate = templates.find(t => t.id === gateTemplateId);
         if (gateTemplate?.name.toLowerCase().includes(lowerSearch)) return true;
      }

      const target = schedule.gate?.target;
      if (target?.toLowerCase().includes(lowerSearch)) return true;
   ...
   return <Stack>
      <Stack horizontalAlign="end">
         <SearchBox
            ...
            styles={{ root: { marginBottom: 8, width: 400 } }}
         />
      </Stack>
      <div className={classes.detailsRow} style={{ overflowY: 'auto', overflowX: 'hidden', maxHeight: "820px" }} data-is-scrollable={true}>
