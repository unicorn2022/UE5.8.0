...
import { Checkbox, CommandBar, CommandBarButton, DefaultButton, Dialog, DialogFooter, DialogType, Dropdown, DropdownMenuItemType, getFocusStyle, ICommandBarItemProps, Icon, IconButton, IContextualMenuItem, IContextualMenuItemProps, IContextualMenuProps, IDropdownOption, IGroup, Label, List, mergeStyleSets, MessageBar, MessageBarType, Modal, Persona, PersonaSize, Pivot, PivotItem, PrimaryButton, Separator, Spinner, SpinnerSize, Stack, TagPicker, Text, TextField, TooltipHost } from "@fluentui/react";
...
import { CommitIdField } from "./commits/commitField";
import { hasPluginPermission } from 'hordePlugins';
...
const IssueSummaryPanel: React.FC = () => {
   ...
   if (issue.duplicateIssueId !== undefined) {
      items.push({
         title: `Duplicate of Issue`,
         text: issue.duplicateIssueId === 0 ? "Unspecified" : issue.duplicateIssueId.toString(),
         link: issue.duplicateIssueId === 0 ? undefined : `/index?issue=${issue.duplicateIssueId}`
      })
   }
   ...
   if (issue.rootCommitId) {
      items.push({
         title: "Root Change",
         text: `${issue.rootCommitId}`,
         link: dashboard.swarmUrl ? `${dashboard.swarmUrl}/changes/${issue.rootCommitId}` : undefined
      })
   }
     if (issue.rootCauseOwnerInfo?.name ) {
      items.push({
         title: "Root Cause Owner",
         text: issue.rootCauseOwnerInfo.name
      })
   }
   if (issue.rootCauseCategory) {
      items.push({
         title: "Root Category",
         text: issue.rootCauseCategory
      })
   }
   if (issue.rootCauseSummary) {
      items.push({
         title: "Root Summary",
         text: issue.rootCauseSummary
      })
   }
...
const IssueCommandBar: React.FC = () => {
   ...
   const [markFixedShown, setMarkFixedShown] = useState<{shown?:boolean, autoFix?:boolean}>({shown: !!query.has("modify"), autoFix: !!query.has("markfix")});
   ...
   const statusList: IContextualMenuItem[] = [{
      ...
      onClick: () => { setMarkFixedShown({shown: true, autoFix: true}); }
   }];
   ...
   const moreList: IContextualMenuItem[] = [
      {
         key: 'more_modify_issue',
         iconProps: { iconName: "Edit" },
         text: `Modify Issue`,
         style: { color: "unset" },
         onClick: () => { setMarkFixedShown({shown: true}); }
      },
      {
   ...
   // Add AI Analysis link if user has BuildHealthViewAI permission (insert after Modify Issue)
   if (hasPluginPermission('buildhealth', 'BuildHealthViewAI')) {
      moreList.splice(1, 0, {
         key: 'more_ai_analysis',
         iconProps: { iconName: "PredictiveAnalysis" },
         text: `AI Analysis`,
         href: `/buildhealth/ai?issue=${issue.id}`,
         target: "_blank",
         style: { color: "unset" }
      });
   }
   ...
   return <Stack >
      ...
      {markFixedShown.shown && <MarkFixedModal autoFix={markFixedShown.autoFix} onClose={() => { setMarkFixedShown({}) }} />}
...
const AssignToOtherModal: React.FC<{ defaultUser?: GetUserResponse, onClose: () => void }> = ({ defaultUser, onClose }) => {
   ...
   const [state, setState] = useState<{ error?: string, submitting?: boolean, userId?: string, reopen?: boolean }>({});
   ...
   let showReopenOption = issue.resolvedAt ? true: false;
   ...
   const onAssign = async () => {
      ...
      const resolved = state.reopen ? false : undefined;
      const fixChange = state.reopen ? 0 : undefined;
   ...
   let height = state.error ? 265 : 225;
   if (showReopenOption) {
      height += 35;
   }
   ...
   return <Modal isOpen={true} className={hordeClasses.modal} styles={{ main: { padding: 8, width: 540, height: height, minHeight: height } }} onDismiss={() => { close() }}>
      ...
      <Stack styles={{ root: { padding: 8 } }}>
         ...
         {showReopenOption && <Stack styles={{ root: { paddingTop: 20} }}>
            <Checkbox label='Reopen issue?'
               defaultChecked={false}
               onChange={(ev, newValue) => {
                  setState({ ...state, reopen: !!newValue});
               }} />
         </Stack>}
...
const rcaCategoryOptions: IDropdownOption[] = [
   { key: 'rca_asset_content', text: 'Asset/Content' },
   { key: 'rca_code', text: 'Code' },
   { key: 'rca_ui', text: 'UI/Localization' },
   { key: 'rca_deployment', text: 'Build/Deployment/Version' },
   { key: 'rca_data', text: 'Configuration/Data' },
   { key: 'rca_gameplay', text: 'Gameplay/Engine' },
   { key: 'rca_backend', text: 'Backend/Server/Network' },
   { key: 'rca_monitoring', text: 'Monitoring/No Code Change' },
   { key: 'rca_use', text: 'Unintended Side Effect' },
   { key: 'rca_buildagent', text: 'Build Agent' },
   { key: 'rca_changebackout', text: 'Change Backout' },
   { key: 'rca_other', text: 'Other' },
   { key: 'rca_clear', text: 'Clear Selection' }
];

const MarkFixedModal: React.FC<{ onClose: () => void, autoFix?:boolean } > = ({ onClose, autoFix }) => {

   const issue = details.issue!;

   const [state, setState] = useState<{ error?: string, fixCL?: number, rootCommitId?: CommitId, rootCauseOwnerId?: string, rootCauseSummary?: string, rootCauseCategory?: string, submitting?: boolean, dupeCheck?: boolean, dupeId?: string }>({
      fixCL: issue.fixChange ?? (autoFix ? 0 : undefined),
      rootCommitId: issue.rootCommitId,
      dupeId: issue.duplicateIssueId ? (issue.duplicateIssueId.toString() !== "0" ? issue.duplicateIssueId.toString() : undefined) : undefined,
      dupeCheck: issue.duplicateIssueId === 0 || !!issue.duplicateIssueId,
      rootCauseOwnerId: issue.rootCauseOwnerInfo?.id,
   ...
   const headerText = `Modify Issue ${issueId}`;

   const close = () => {

      const url = new URL(window.location.href);
      if (url.searchParams.has("modify")) {
         url.searchParams.delete("modify");
         url.searchParams.delete("markfix");
         navigate(url.pathname + url.search, { replace: true });
      }
      onClose();
   };

   const onUpdate = async (clearFixed?: boolean) => {

      state.submitting = true;
      setState({ ...state });

      let fixCL: number | null | undefined;
      if (clearFixed) {
         fixCL = -1;
      } else {
         fixCL = state.fixCL;
      ...
      // Root Cause Analysis
      let rootCommitId = state.rootCommitId?.length ? state.rootCommitId : null;
      const rootCauseOwnerId = state.rootCauseOwnerId?.length ? state.rootCauseOwnerId : issue.rootCauseOwnerInfo?.id?.length ? "" : null;
      const rootCauseCategory = state.rootCauseCategory?.length ? state.rootCauseCategory : "";
      const rootCauseSummary = state?.rootCauseSummary?.trim().length ? state.rootCauseSummary.trim() : "";

      let dupeId = state.dupeCheck ? 0 : -1;
      if (dupeId === 0 && state.dupeId) {
         dupeId = parseInt(state.dupeId)
         if (dupeId === issueId) {
            dupeId = -1;
         }
      }
      ...
      if (dupeId >= 0) {
         if (!issue.summary.startsWith("(Dupe) ")) {
            summary = "(Dupe) " + issue.summary;
         }
      } else if (issue.summary.startsWith("(Dupe) ")) {
         summary = issue.summary.replace("(Dupe) ", "");
      }
      ...
      const request: UpdateIssueRequest = {
         ...
         resolved: clearFixed ? false : (fixCL != undefined && fixCL >= 0),
         duplicateIssueId: dupeId
      }
   ...
   let height = state.error ? 300 : 260;

   height += 384;

   const selectedCatKey = rcaCategoryOptions.find(c => c.text === state.rootCauseCategory)?.key ?? null;

   const disableRoot = !!state.dupeCheck;

   // Unique set of users that are most likely to be the RCA owner
   const rcaOwnerCandidates = [... new Set([ dashboard.userId, issue.rootCauseOwnerInfo?.id ?? null, issue.ownerInfo?.id ?? null, issue.resolvedByInfo?.id ?? null])];


   return <Modal className={hordeClasses.modal} isOpen={true} styles={{ main: { padding: 8, width: 540, height: height, minHeight: height } }}>
      <Stack horizontal styles={{ root: { padding: 8 } }}>
         <Stack.Item grow={2}>
            <Text variant="mediumPlus">{headerText}</Text>
         </Stack.Item>
      ...
      <Stack style={{ paddingLeft: 8, paddingRight: 8 }}>
         <Stack styles={{ root: { paddingLeft: 8, width: 500, paddingTop: 12 } }}>
            <CommitIdField label="Fix Change" placeholder="Change which fixed this issue. Enter 0 to mark as fixed, without a change" defaultCommitId={state.fixCL?.toString()} onChange={(commitId => { 
               setState({ ...state, fixCL: commitId ? parseInt(commitId) : undefined }) })} />
         </Stack>

         <Stack styles={{ root: { paddingLeft: 8, width: 500, paddingTop: 12 } }}>
            <Stack style={{ paddingLeft: 8, width: 500, paddingBottom: 12 }}>
               <Separator />
            </Stack>
            <Stack tokens={{ childrenGap: 16 }}>
               <Stack>
                  <Label>Root Cause Owner</Label>
                  <UserSelect defaultId={state.rootCauseOwnerId} handleSelection={(id => {
                     setState({ ...state, rootCauseOwnerId: id })
                  })} userIdHints={rcaOwnerCandidates.filter(c => c !== null)} noResultsFoundText="No users found" />
               </Stack>
               <Stack>
                  <CommitIdField disabled={disableRoot} label="Root Cause Change" defaultCommitId={state.rootCommitId} placeholder="Change which caused this issue, please only enter if certain on root cause" onChange={(commitId => { setState({ ...state, rootCommitId: commitId }) })} />
               </Stack>
               <Stack>
                  <Dropdown
                     ...
                     label="Root Cause Category (required)"
                     options={rcaCategoryOptions}
                     onChange={(event, option) => {
                        if (option?.key === "rca_clear") {
                           option = undefined;
                        }
                        setState({ ...state, rootCauseCategory: option?.text as string | undefined })
                     }}
                  />
               </Stack>
               <Stack>
                  <TextField disabled={disableRoot} spellCheck={false} autoComplete="off" label="Root Cause Summary" placeholder="Please enter a brief summary describing the root cause of this issue and any suggestions to prevent it." value={state.rootCauseSummary} style={{ minHeight: 120, maxHeight: 120 }} multiline resizable={false} onChange={(ev, newValue) => {
                     ev.preventDefault();
                     setState({ ...state, rootCauseSummary: newValue ? newValue : "" })
                  }} />
               </Stack>

               <Stack horizontal verticalFill verticalAlign="center" tokens={{ childrenGap: 24 }}>
                  <Checkbox label="Duplicate Issue" checked={state.dupeCheck} onChange={(_, checked) => setState({ ...state, dupeCheck: checked })} />
                  <TextField style={{width: 352}} disabled={!state.dupeCheck} spellCheck={false} autoComplete="off" placeholder="Optional Issue ID of the source issue this issue duplicates" value={state.dupeId} onChange={(ev, newValue) => {
                     ev.preventDefault();

                     newValue = newValue?.trim();

                     // support putting in a link
                     if (newValue?.startsWith("http")) {
                        const lastIndex = newValue.lastIndexOf("/");
                        const issueId = parseInt( newValue.slice(lastIndex + 1));
                        if (typeof(issueId) === "number" && !isNaN(issueId)) {
                           newValue = issueId.toString();
                        }
                     }
                     setState({ ...state, dupeId: newValue ? newValue?.trim() : "" })
                  }} />
               </Stack>
         ...
         <Stack tokens={{ childrenGap: 16 }} styles={{ root: { paddingTop: 24, paddingLeft: 8, paddingBottom: 8 } }}>
            <Stack horizontal>
               <Stack>
                  {!!issue.resolvedAt && <DefaultButton text="Clear Fixed" disabled={state.submitting} onClick={() => { onUpdate(true); }} />}
               </Stack>
               <Stack grow />
               <Stack>
                  <Stack horizontal tokens={{ childrenGap: 12 }} style={{ paddingRight: 24 }}>
                     <PrimaryButton text="Update" disabled={state.submitting ?? false} onClick={() => { onUpdate(); }} />
