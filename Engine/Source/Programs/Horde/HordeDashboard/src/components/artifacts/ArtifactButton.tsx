...
import dashboard, { UserPlatform } from "horde/backend/Dashboard";
...
export const ArtifactButton: React.FC<{ jobId?: string, stepId?: string, artifact?: GetArtifactResponse, pathIn?: string, selection?: ArtifactSelection, openArtifactInfo?: () => void, disabled?: boolean, minWidth?: number }> = ({ jobId, stepId, artifact, pathIn, selection, openArtifactInfo, disabled, minWidth }) => {
   ...
   function openInBuildStorageTool(zenUri: string) {
      if (zenUri) {
         window.location.assign(`ugs://runcustomtool?toolName=build%20storage%20tool&args=RunBuildStorageTool.bat%20-GoToBuild%20${encodeURIComponent(zenUri)}`);
      }
   }
   ...
   if (artifact?.id) {
      ...
      if (!zenArtifact) {
         ...
         if (zenUri) {
            ...
            if (dashboard.platform === UserPlatform.Windows) {
               downloadProps.items.push(
                  {
                     key: 'open_build_storage_tool',
                     text: 'Open in Build Storage Tool',
                     onClick: () => {
                        openInBuildStorageTool(zenUri);
                     }
                  }
               )
            }
