...
const jobHandler = new JobHandler(true, 300, commitCache);
...
const JobViewAllInner: React.FC<{ filter: JobFilterSimple }> = observer(({ filter }) => {
   ...
   const renderItemInner = (item: JobItem, index?: number, column?: IColumn) => {
      ...
      if (column!.name === "Change") {
         const commit = streamId ? commitCache.getCommit(streamId, job.change!) : undefined;
         return <ChangeButton job={job} commit={commit} />;
   ...
   const JobLabel: React.FC<{ item: JobItem; label: GetLabelStateResponse }> = ({ item, label }) => {
      ...
      const badge = <Link to={url}><Stack className={hordeClasses.badgeNoIcon}>
