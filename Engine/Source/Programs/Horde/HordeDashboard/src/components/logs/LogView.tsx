...
import { getStepMessageInfo } from '../jobDetailsV2/JobDetailsUtils';
...
const LogList: React.FC<{ logId: string }> = observer(({ logId }) => {
   ...
   const doQuery = (newValue: string) => {
      ...
      if (!newValue.trim()) {
         ...
         inTimeout = true;
         setTimeout(() => {
            inTimeout = false; listRef?.scrollToIndex(lineIdx, () => { return handler.lineHeight; }, ScrollToMode.top);
         }, 10)
   ...
   const report = jobDetails?.stepByLogId(logId)?.reports?.find(r => !!r.message);
               ...
               <Stack horizontal style={{ paddingBottom: 4 }}>
                  <Stack className={hordeClasses.button} horizontal horizontalAlign={"start"} verticalAlign="center" tokens={{ childrenGap: 8 }}>
                     ...
                     <Stack>
                        ...
                        {!!stepMessage && <Stack horizontalAlign='center'>
                           <Stack style={{ paddingLeft: 4 }} horizontal verticalAlign="center" tokens={{ childrenGap: 4 }}>
                              <Text style={{ fontSize: 11, fontFamily: "Horde Open Sans Semibold" }}>Step:</Text>
                              <Stack horizontal verticalFill verticalAlign="center" style={{ paddingTop: 2 }} tokens={{ childrenGap: 4 }}>
                                 {!!stepMessageColor && <FontIcon style={{ fontSize: 13, color: stepMessageColor, paddingTop: 1 }} iconName="Square" />}
                                 <Text style={{ maxWidth: 190, fontSize: 11, fontFamily: "Horde Open Sans Semibold", overflow: "hidden", textOverflow: "ellipsis" }}>{stepMessage}</Text>
                              </Stack>
                           </Stack>
                        </Stack>}
