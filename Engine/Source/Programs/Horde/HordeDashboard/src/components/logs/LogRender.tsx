...
const renderTags = (navigate: NavigateFunction, line: LogLine, lineNumber: number | undefined, logStyle: any, tags: string[], search?: string) => {
   ...
   let renderedTags = tags.map((tag, index) => {
      ...
      switch(tagType) {
         ...
         case TagType.JobStepId:            
            const jobId = (line as any)?.properties?.JobId?.$text
            if (jobId) {
               const stepUrl = `/job/${jobId}?step=${text}`;
               linkProps = {
                  href: stepUrl,
                  onClick: (ev) => { ev.stopPropagation(); ev.preventDefault(); navigate(stepUrl) }
               }
            }
            break;
         ...
         case TagType.IgnoredLine:
            return (
               <span key={key}>
                  <span style={{ fontFamily: "Horde Open Sans SemiBold, sans-serif", fontWeight: 600 }}>IGNORED:</span> {highlight}
               </span>
            );
      }
