...
import { copyToClipboard, copyToClipboardAsync } from "../../base/utilities/clipboard";
...
type OptionsConstruction = {
   ...
   jobKey?: string;
   readOnly?: boolean;
   aliasTemplates?: string[];
}

class BuildOptions {

   constructor(streamId: string, projectStore: ProjectStore, onClose: (newJobId: string | undefined) => void, jobDetails?: JobDetailsV2, jobKey?: string, readOnly?: boolean, aliasTemplates?: string[]) {
   ...
   init() {
      ...
      this.changeCommitId = undefined;
      this.preflightChange = undefined;
      this.preflightCommitId = undefined;
      this.queryShelvedChange = undefined;
      this.queryShelvedCommitId = undefined;
      ...
      const queryShelvedParam = query.get("shelvedchange") ?? "";
      if (queryShelvedParam) {
         const parsed = parseInt(queryShelvedParam);
         if (!isNaN(parsed) && parsed) {
            this.queryShelvedChange = parsed;
         } else if (/^[a-f0-9]+$/i.test(queryShelvedParam)) {
            this.queryShelvedCommitId = queryShelvedParam.trim().toLowerCase();
         }
      }

      this.preflightChange = jobDetails?.jobData?.preflightChange;
      this.preflightCommitId = jobDetails?.jobData?.preflightCommitId;
      this.change = jobDetails?.jobData?.change;

      if (!this.preflightChange && !this.preflightCommitId) {
         this.preflightChange = this.queryShelvedChange;
         this.preflightCommitId = this.queryShelvedCommitId;
      }
   ...
   onPreflightCommitIdChange(commitId?: string) {
      if (this.preflightCommitId === commitId) {
         return;
      }
      this.preflightCommitId = commitId;
      if (commitId) {
         this.preflightChange = undefined;
      }
   ...
   validate(errors: ValidationError[]): boolean {
      ...
      if (this.preflightChange && !this.template.allowPreflights) {
         errors.push({ error: `Template "${this.template.name}" does not allow preflights`, paramString: "Preflight Change", value: this.preflightChange.toString() })
      }
      ...
      if (this.preflightChange !== undefined && isNaN(this.preflightChange)) {
         errors.push({
            paramString: "Preflight Change",
            value: this.preflightChange.toString(),
            error: "Invalid shelved changelist"
         });
      }

      if (this.preflightCommitId && !this.template.allowPreflights) {
         errors.push({ error: `Template "${this.template.name}" does not allow preflights`, paramString: "Preflight Change", value: this.preflightCommitId })
      }
   ...
   private selectDefaultTemplate() {
      ...
      if (!t && this.isPreflightSubmit && !jobDetails) {
   ...
   private filterTemplates() {
      ...
      const aliasTemplates = this.construction.aliasTemplates;
      if (aliasTemplates && aliasTemplates.length > 0) {
         const aliasTemplateSet = new Set(aliasTemplates);
         templates = templates.filter(t => aliasTemplateSet.has(t.id));
      }
...
async function buildParametersUrl(template: GetTemplateRefResponse): Promise<boolean> {

   // Note: can't rely on current path for template as user can change that via All Templates and URL won't update.
   const setParams = new URLSearchParams(`${location.search}&newbuild=true&templateId=${template.id}`);
   const args = BuildOptions.get().parameters;

   for (const p of template.parameters) {
      switch (p.type) {
         case ParameterType.Bool: {
            const b = p as BoolParameterData;
            if (args[b.id] && args[b.id] !== b.default.toString()) {
               setParams.set(`id-${b.id}`, args[b.id]);
            }
            break;
         }
         case ParameterType.Text: {
            const t = p as TextParameterData;
            if (args[t.id] && args[t.id] !== t.default) {
               setParams.set(`id-${t.id}`, args[t.id]);
            }
            break;
         }
         case ParameterType.List: {
            ...
            for (const i of list.items) {
               // Note: need to include only true values for lists, as setDefaultParameters clears all values if any are present in the url.
               if (args[i.id] && args[i.id].toLowerCase() === "true") {
                  setParams.set(`id-${i.id}`, args[i.id]);
               }
            }
            break;
         }
   ...
   query.search = setParams.toString();
   return await copyToClipboardAsync(query.toString());
}

const BuildModal: React.FC<{ setUseLegacyDialog: (value: boolean) => void }> = observer(({ setUseLegacyDialog }) => {

   const [parametersUrlCopied, setParametersUrlCopied] = useState(false);
   ...
   if (options.changeCommitId) {
      changeText = options.changeCommitId;
   }
   ...
   const onSubmit = () => {
      ...
      if (typeof options.change !== 'number' && !options.changeCommitId) {
      ...
      if (options.changeCommitId) {
         data.commitId = options.changeCommitId;
      }
      ...
      if (options.preflightCommitId) {
         data.preflightCommitId = options.preflightCommitId;
         data.updateIssues = false;
      }
            ...
            <Stack styles={{ root: { paddingLeft: 8, paddingRight: 8, paddingTop: 4 } }}>
               <Stack tokens={{ childrenGap: 8 }}>
                  <Stack horizontal tokens={{ childrenGap: 18 }} style={{ paddingBottom: 4 }}>
                     ...
                     <Stack>
                        <ComboBox style={{ width: 204 }} key={`build_change_key_${options.currentRenderKey}`} label="Change" text={changeText} options={changeItems} disabled={options.readOnly} allowFreeform autoComplete="off" placeholder="Latest Change" defaultValue={"-1"} onChange={(ev, option, index, value) => {
                           ...
                           if (option) {
                              ...
                              options.changeCommitId = undefined;
                              ...
                              if (!value) {
                                 ...
                                 if (!isNaN(nvalue)) {
                                    options.change = nvalue;
                                    options.changeCommitId = undefined;
                                 } else {
                                    options.change = undefined;
                                    options.changeCommitId = value.trim().toLowerCase();
                                 }
                              }
                           }
                           options.setChanged();
                     ...
                     <Stack>
                        <TextField
                           key={`build_preflight_key_${options.currentRenderKey}`}
                           label="Shelved Change"
                           style={{ width: 120 }}
                           placeholder={!template!.allowPreflights ? "Disabled" : ""}
                           autoComplete="off"
                           spellCheck={false}
                           value={options.preflightChange?.toString() ?? options.preflightCommitId ?? ""}
                           disabled={!template!.allowPreflights || options.readOnly || options.isPreflightSubmit}
                           onChange={(ev, newValue) => {
                              ev.preventDefault();
                              const trimmed = newValue?.trim() ?? "";
                              if (!trimmed) {
                                 options.onPreflightChange(undefined);
                                 options.onPreflightCommitIdChange(undefined);
                                 return;
                              }
                              if (/^\d+$/.test(trimmed)) {
                                 options.onPreflightChange(parseInt(trimmed));
                              } else {
                                 options.onPreflightCommitIdChange(trimmed.toLowerCase());
                              }
                           }}
                        />
                     </Stack>
               ...
               <Stack horizontal tokens={{ childrenGap: 16 }} styles={{ root: { paddingTop: 32, paddingLeft: 8, paddingBottom: 8 } }}>
                  {!options.jobDetails && !options.readOnly && <Stack>
                     <DefaultButton text="Reset" style={{ width: 110 }} onClick={() => options.onResetToDefaults()} />
                  </Stack>}
                  <DefaultButton text={parametersUrlCopied ? "URL copied!": "Create URL"} disabled={!template}
                     onClick={async () => { 
                        const copySuccess = await buildParametersUrl(template);
                        setParametersUrlCopied(copySuccess);
                        if (copySuccess) {
                           setTimeout(() => setParametersUrlCopied(false), 5000);
                        }
                  }} />
...
export const NewBuildV2: React.FC<{ streamId: string; show: boolean; onClose: (newJobId: string | undefined) => void, jobKey?: string; jobDetails?: JobDetailsV2, readOnly?: boolean, aliasTemplates?: string[] }> = ({ streamId, jobKey, show, onClose, jobDetails, readOnly, aliasTemplates }) => {
   ...
   if (!BuildOptions.initialized()) {
      new BuildOptions(streamId, projectStore, onClose, jobDetails, jobKey, readOnly, aliasTemplates);
   }
