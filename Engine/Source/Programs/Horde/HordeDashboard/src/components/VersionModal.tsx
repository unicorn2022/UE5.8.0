...
export const VersionModal: React.FC<{ show: boolean, onClose: () => void }> = ({ show, onClose }) => {
   ...
   const columns = [
      { key: 'column1', name: 'Name', fieldName: 'name', minWidth: 120, maxWidth: 120 },
      { key: 'column2', name: 'Value', fieldName: 'value', minWidth: 100, maxWidth: 200 },
      { key: 'column3', name: 'User ACLs', fieldName: 'userAcls', minWidth: 200, maxWidth: 400 },
   ];
   ...
   if (version.serverInfo?.plugins?.length) {
      ...
      version.serverInfo?.plugins.forEach(p => {
         versionItems.push({ name: p.name, value: p.version ?? "", userAcls: p.userAcls });
      })
   
   }


   const onRenderItemColumn = (item: GeneralItem, _index?: number, columnIn?: IColumn) => {
      ...
      switch (column.name) {
         ...
         case 'Value':
            ...
            break;
         case 'User ACLs':
            if (item.userAcls === undefined) {
               return null; // Plugin doesn't use ACLs
            }
            if (item.userAcls.length === 0) {
               return <Text variant="small" styles={{ root: { fontFamily: "Horde Open Sans Regular", color: "#888" } }}>None</Text>
            }
            return <Text variant="small" styles={{ root: { fontFamily: "Horde Open Sans Regular" } }}>{item.userAcls.join(", ")}</Text>
      }
   ...
   return <Modal className={hordeClasses.modal} isOpen={show} styles={{ main: { padding: 4, width: 800 } }} onDismiss={() => { if (show) close() }}>
