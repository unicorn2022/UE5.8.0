...
import { getHordePlugins, getFilteredMounts, MountType } from 'hordePlugins';
import { SupportModal } from './SupportModal';
import { getAliasDisplayName, getStreamLink } from '../base/utilities/streamUtils';
...
const generateProjectMenu = (store: ProjectStore) => {
   ...
   projects.sort((a: ProjectData, b: ProjectData) => {
      ...
      const allAliases: IProjectContextualMenuItem[] = [];
      p.streams?.forEach(stream => {
         stream.aliases?.forEach(alias => {
            allAliases.push({
               style: { color: modeColors.text, fontSize: 12 },
               key: `${stream.id}_alias_${alias.name}`,
               text: getAliasDisplayName(alias.name),
               data: { project: p, stream: stream, alias: alias },
               link: getStreamLink(stream.id, alias.name)
            });
         });
      });
...
export const TopNav: React.FC<{ suppressServer?: boolean }> = observer(({ suppressServer }) => {
   ...
   const generateToolsMenu = () => {
      ...
      // get the plugins and their filtered mounts (based on user ACLs)
      const plugins = getHordePlugins();

      // Add plugin topnav mount points (filtered by user ACLs)
      plugins.forEach(p => {
         const mounts = getFilteredMounts(p).filter(m => m.type === MountType.TopNav);
         mounts.forEach(m => {
            serviceItems.push({
