...
const JobList: React.FC<{ tab: string; filter: JobFilterSimple; aliasTemplates?: string[] }> = observer(({ tab, filter, aliasTemplates }) => {
   ...
   const JobLabel: React.FC<{ item: JobItem; column: GetJobsTabLabelColumnResponse; label: GetLabelStateResponse }> = ({ item, column, label }) => {
      ...
      const badge = <Link to={url}><Stack className={hordeClasses.badgeNoIcon}><DefaultButton key={label.dashboardName} style={{ backgroundColor: color.primaryColor }} text={label.dashboardName}
   ...
   function onRenderItemColumnInner(item: JobItem, index?: number, column?: IColumn) {
      ...
      if (column!.key === startedByName) {
         ...
         const pfDisplay = getPreflightDisplay(item.job?.preflightCommitId, item.job?.preflightChange);
         if (pfDisplay) {
            change = `PF ${pfDisplay}`;
         }
...
export const JobViewIncremental: React.FC<{ tab: string; filter: JobFilterSimple; aliasTemplates?: string[] }> = ({ tab, filter, aliasTemplates }) => {
