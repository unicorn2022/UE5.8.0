...
import { historyModalStore, processLeaseData, ProcessedLease } from "./HistoryModalStore";
...
const TelemetryTooltip: React.FC<{ id: "cpu" | "ram" | "workingDisk" | "systemDisk", cpuKey?: string }> = observer(({ id, cpuKey }) => {
   ...
   if (lease?.details?.logId) {
      elements.push(dataElement("Log:", `Lease (${lease.id})`, `/log/${lease.details.logId}`));
   }
   ...
   // Calculate and display lease duration
   if (lease) {
      const startMs = new Date(lease.startTime).getTime();
      const endMs = lease.finishTime ? new Date(lease.finishTime).getTime() : Date.now();
      ...
      let durationStr = parts.join(" ");
      if (!lease.finishTime) {
         durationStr += " (ongoing)";
      }

      elements.push(dataElement("Duration:", durationStr));
   }
   ...
   if (tooltip.sample) {
      ...
      switch (id) {
         ...
         case "workingDisk":
            title = "Disk"
            value = formatBytes((d.workingTotalDisk - d.workingFreeDisk) * 1024 * 1024) + " Used";
            break;
         case "systemDisk":
            title = "Disk"
            value = formatBytes((d.systemTotalDisk - d.systemFreeDisk) * 1024 * 1024) + " Used";
            break;
         case "ram":
            title = "Ram"
...
const timeSelections: TimeSelection[] = [
   ...
   {
      text: "Past 8 Hours", key: "time_8_hours", minutes: 60 * 8
   },
   {
      text: "Past 12 Hours", key: "time_12_hours", minutes: 60 * 12
   },
   {
...
// 10 distinct colors for lease overlays - more transparent to not occlude graph data
const leaseColorsDark = [
   ...
   "rgba(64, 224, 208, 0.15)",   // Turquoise
   "rgba(255, 218, 185, 0.15)",  // Peach puff
   "rgba(135, 206, 235, 0.15)",  // Sky blue
   "rgba(255, 160, 122, 0.15)",  // Light salmon
   "rgba(152, 251, 152, 0.15)",  // Pale green
   "rgba(216, 191, 216, 0.15)",  // Thistle
];

const leaseColorsLight = [
   "rgba(65, 105, 225, 0.12)",   // Royal blue
   "rgba(34, 139, 34, 0.12)",    // Forest green
   "rgba(255, 140, 0, 0.12)",    // Dark orange
   "rgba(148, 0, 211, 0.12)",    // Dark violet
   "rgba(0, 139, 139, 0.12)",    // Dark cyan
   "rgba(184, 134, 11, 0.12)",   // Dark goldenrod
   "rgba(70, 130, 180, 0.12)",   // Steel blue
   "rgba(205, 92, 92, 0.12)",    // Indian red
   "rgba(60, 179, 113, 0.12)",   // Medium sea green
   "rgba(186, 85, 211, 0.12)",   // Medium orchid
];

/**
 * Renders colored overlay rectangles for leases on a Sparklines chart.
 * Each lease gets a distinct color. Overlays are hidden entirely if all leases are too narrow.
 */
const LeaseOverlayRects: React.FC<{
   processedLeases: ProcessedLease[];
   sparkHeight: number;
   sparkWidth: number;
   chartId: string;
}> = ({ processedLeases, sparkHeight, sparkWidth, chartId }) => {
   const colors = dashboard.darktheme ? leaseColorsDark : leaseColorsLight;
   const margin = 2; // Sparklines default margin
   const minWidthThreshold = 4; // Hide all overlays if every lease is narrower than this

   const sortedLeases = [...processedLeases].sort((a, b) => a.startX - b.startX);

   // Check if any lease is wide enough to show overlays
   const hasWideEnoughLease = sortedLeases.some(pl => {
      const endX = pl.endX ?? sparkWidth - margin;
      return (endX - pl.startX) >= minWidthThreshold;
   });

   if (!hasWideEnoughLease) {
      return null;
   }

   const elements: JSX.Element[] = [];

   sortedLeases.forEach((pl, idx) => {
      const x = pl.startX;
      const endX = pl.endX ?? sparkWidth - margin;
      const width = endX - x;
      ...
      const color = colors[processedLeases.indexOf(pl) % colors.length];

      elements.push(
         <rect key={`${chartId}-lease-${idx}`} x={x} y={0} width={width} height={sparkHeight} fill={color} />
      );
   });
...
const TelemetryPanel: React.FC<{}> = observer(({ }) => {
   ...
   const isLoading = !currentData.length;

   const statusColors = dashboard.getStatusColors();

   const sparkWidth = 1100;
   const sparkHeight = 128;
   const data = isLoading ? [] : (currentData[0] as GetAgentTelemetrySampleResponse[]);
   const leases = (!isLoading && currentData.length > 1) ? currentData[1] as GetAgentLeaseResponse[] : [];

   const hasData = data.length > 0;

   const processedData = hasData ? processLeaseData(data, leases, sparkWidth) : null;
   const processedLeases = processedData?.processedLeases ?? [];

   const cpuData = hasData ? data.map(d => {
   ...
   const min = hasData ? data[0].time : "";
   const max = hasData ? data[data.length - 1].time : "";

   const minTime = hasData ? getShortNiceTime(min, true) : "";
   const maxTime = hasData ? getShortNiceTime(max, true) : "";

   const calculateDiskUtilization = (
      data: GetAgentTelemetrySampleResponse[], 
      totalKey: keyof GetAgentTelemetrySampleResponse, 
      freeKey: keyof GetAgentTelemetrySampleResponse
   ) => 
      data.map(d => {
         const total = d[totalKey];
         const free = d[freeKey];
   ...
   const diskData = calculateDiskUtilization(data, 'workingTotalDisk', 'workingFreeDisk');
   const systemDiskData = calculateDiskUtilization(data, 'systemTotalDisk', 'systemFreeDisk');
   ...
   const formatDiskSize = (bytes: number, decimals = 0) => {
   ...
   if (devices?.length) {
      ...
      device.properties?.forEach(v => {
         ...
         if (v.startsWith("WorkingDiskTotalSize=")) {
            const elements = v.split("=");
            if (elements.length === 2) {
               diskText = formatDiskSize(parseInt(elements[1]))
            }
         }

         if (v.startsWith("SystemDiskTotalSize=")) {
            const elements = v.split("=");
            if (elements.length === 2) {
               systemDiskText = formatDiskSize(parseInt(elements[1]))
            }
         }
   ...
   // Padding value used in Sparklines style - must match for coordinate calculations
   const sparkPadding = 8;
   const sparkMargin = 2; // Sparklines internal margin
   ...
   function relativeCoords(event: any, id: string) {
      ...
      // Use currentTarget to always get the event handler element, not whatever child is under the cursor
      let bounds = event.currentTarget.getBoundingClientRect();
      let x = event.clientX - bounds.left;
      let y = event.clientY - bounds.top;

      // Adjust for padding and border to get position within the actual chart area
      // Structure: handler div > tooltip div > styled container (border 1px, padding 8px) > inner div > SVGs
      const chartX = x - sparkPadding - 1; // -1 for border, now in SVG coordinates (0-1100)

      // Sparklines draws data from x=sparkMargin to x=(sparkWidth-sparkMargin)
      // Must account for the margin to correctly map to sample indices
      const drawWidth = sparkWidth - 2 * sparkMargin;
      const dataX = chartX - sparkMargin; // Position within drawable area
      const percent = dataX / drawWidth;

      // Clamp and calculate sample index
      // Use (data.length - 1) to match how processLeaseData calculates positions
      const clampedPercent = Math.max(0, Math.min(1, percent));
      const sampleIdx = Math.min(Math.round(clampedPercent * (data.length - 1)), data.length - 1);
      ...
      // Find lease by visual X position (using processedLeases which have padding applied)
      // This ensures the tooltip matches what the user sees visually
      // Sort by startX to match drawing order - leases drawn later are on top
      // When leases overlap, use the LAST matching one (drawn on top, visually visible)
      const sortedLeases = [...processedLeases].sort((a, b) => a.startX - b.startX);
      let lease: GetAgentLeaseResponse | undefined;
      for (const pl of sortedLeases) {
         const leaseEndX = pl.endX ?? sparkWidth; // ongoing leases extend to end
         if (chartX >= pl.startX && chartX <= leaseEndX) {
            lease = pl.lease; // Keep updating to get the last (topmost) match
         }
      }
   ...
   return <Stack tokens={{ childrenGap: 24 }} style={{ paddingBottom: 32, paddingLeft: 8 }}>
      <Stack horizontal verticalAlign="center" verticalFill>
         <Text variant="mediumPlus">{hasData ? `${minTime} - ${maxTime}` : "Select a time range"}</Text>
         <Stack grow />
      ...
      {isLoading && <Stack horizontalAlign="center" style={{ paddingTop: 48 }}>
         <Spinner size={SpinnerSize.large} label="Loading telemetry data..." />
      </Stack>}

      {/* Show message when no data is available for the selected time range */}
      {!isLoading && !hasData && <Stack horizontalAlign="center" style={{ paddingTop: 48 }}>
         <Text variant="large">No telemetry data available for this time range.</Text>
         <Text variant="medium" style={{ paddingTop: 8 }}>Try selecting a different time range above.</Text>
      </Stack>}
      ...
      <Stack verticalAlign="center" verticalFill>
         ...
         <div onMouseLeave={() => state.setLeaseTooltip(undefined)} onMouseMove={(ev) => { relativeCoords(ev, "cpu") }} onClick={() => {
            ...
            <div style={{ backgroundColor: dashboard.darktheme ? "#060709" : "#F3F2F1", padding: 8, border: "solid 1px #181A1B" }}>
               <div style={{ position: "relative" }}>
                  <Sparklines width={sparkWidth} height={sparkHeight} svgWidth={sparkWidth} svgHeight={sparkHeight} data={cpuData} min={0} max={100}>
                  ...
                  <svg style={{ position: "absolute", top: 0, left: 0, pointerEvents: "none" }} width={sparkWidth} height={sparkHeight} viewBox={`0 0 ${sparkWidth} ${sparkHeight}`} preserveAspectRatio="none">
                     <LeaseOverlayRects processedLeases={processedLeases} sparkHeight={sparkHeight} sparkWidth={sparkWidth} chartId="cpu" />
                  </svg>
               </div>
            </div>
         </div>
      ...
      <Stack verticalAlign="center" verticalFill>
         ...
         <div onMouseLeave={() => state.setLeaseTooltip(undefined)} onMouseMove={(ev) => { relativeCoords(ev, "ram") }} onClick={() => {
            ...
            <div style={{ backgroundColor: dashboard.darktheme ? "#060709" : "#F3F2F1", padding: 8, border: "solid 1px #181A1B" }}>
               <div style={{ position: "relative" }}>
                  <Sparklines width={sparkWidth} height={sparkHeight} svgWidth={sparkWidth} svgHeight={sparkHeight} data={ramData} min={0} max={1}>
                     <SparklinesLine color={dashboard.darktheme ? "lightblue" : "#1E90FF"} />
                     <SparklinesReferenceLine type="custom" value={0} style={{ stroke: statusColors.get(StatusColor.Failure) }} />
                     <SparklinesReferenceLine type="custom" value={32} style={{ stroke: statusColors.get(StatusColor.Warnings) }} />
                     <SparklinesReferenceLine type="custom" value={64} style={{ stroke: statusColors.get(StatusColor.Success) }} />
                  ...
                  <svg style={{ position: "absolute", top: 0, left: 0, pointerEvents: "none" }} width={sparkWidth} height={sparkHeight} viewBox={`0 0 ${sparkWidth} ${sparkHeight}`} preserveAspectRatio="none">
                     <LeaseOverlayRects processedLeases={processedLeases} sparkHeight={sparkHeight} sparkWidth={sparkWidth} chartId="ram" />
                  </svg>
               </div>
            </div>
         </div>
      ...
      {!!diskText && <Stack verticalAlign="center" verticalFill>
         <Stack>
            <Label>Agent Working Drive Utilization - {diskText}</Label>
         </Stack>
         <div onMouseLeave={() => state.setLeaseTooltip(undefined)} onMouseMove={(ev) => { relativeCoords(ev, "workingDisk") }} onClick={() => {
            ...
            <div style={{ position: "relative" }}>
               <TelemetryTooltip id="workingDisk" />
            </div>
            <div style={{ backgroundColor: dashboard.darktheme ? "#060709" : "#F3F2F1", padding: 8, border: "solid 1px #181A1B" }}>
               <div style={{ position: "relative" }}>
                  <Sparklines width={sparkWidth} height={sparkHeight} svgWidth={sparkWidth} svgHeight={sparkHeight} data={diskData} min={0} max={1}>
                     <SparklinesLine color={dashboard.darktheme ? "lightblue" : "#1E90FF"} />
                     <SparklinesReferenceLine type="custom" value={0} style={{ stroke: statusColors.get(StatusColor.Failure) }} />
                     <SparklinesReferenceLine type="custom" value={32} style={{ stroke: statusColors.get(StatusColor.Warnings) }} />
                     <SparklinesReferenceLine type="custom" value={64} style={{ stroke: statusColors.get(StatusColor.Success) }} />
                  ...
                  <svg style={{ position: "absolute", top: 0, left: 0, pointerEvents: "none" }} width={sparkWidth} height={sparkHeight} viewBox={`0 0 ${sparkWidth} ${sparkHeight}`} preserveAspectRatio="none">
                     <LeaseOverlayRects processedLeases={processedLeases} sparkHeight={sparkHeight} sparkWidth={sparkWidth} chartId="workingDisk" />
                  </svg>
               </div>
            </div>
         </div>
      ...
      {!!systemDiskText && <Stack verticalAlign="center" verticalFill>
         <Stack>
            <Label>System Drive Utilization - {systemDiskText}</Label>
         </Stack>
         <div onMouseLeave={() => state.setLeaseTooltip(undefined)} onMouseMove={(ev) => { relativeCoords(ev, "systemDisk") }} onClick={() => {
            ...
            <div style={{ position: "relative" }}>
               <TelemetryTooltip id="systemDisk" />
            </div>
            <div style={{ backgroundColor: dashboard.darktheme ? "#060709" : "#F3F2F1", padding: 8, border: "solid 1px #181A1B" }}>
               <div style={{ position: "relative" }}>
                  <Sparklines width={sparkWidth} height={sparkHeight} svgWidth={sparkWidth} svgHeight={sparkHeight} data={systemDiskData} min={0} max={1}>
                     <SparklinesLine color={dashboard.darktheme ? "lightblue" : "#1E90FF"} />
                     <SparklinesReferenceLine type="custom" value={0} style={{ stroke: statusColors.get(StatusColor.Failure) }} />
                     <SparklinesReferenceLine type="custom" value={32} style={{ stroke: statusColors.get(StatusColor.Warnings) }} />
                     <SparklinesReferenceLine type="custom" value={64} style={{ stroke: statusColors.get(StatusColor.Success) }} />
                  ...
                  <svg style={{ position: "absolute", top: 0, left: 0, pointerEvents: "none" }} width={sparkWidth} height={sparkHeight} viewBox={`0 0 ${sparkWidth} ${sparkHeight}`} preserveAspectRatio="none">
                     <LeaseOverlayRects processedLeases={processedLeases} sparkHeight={sparkHeight} sparkWidth={sparkWidth} chartId="systemDisk" />
                  </svg>
               </div>
            </div>
         </div>
