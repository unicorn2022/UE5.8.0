...
import { getPreflightDisplay } from '../base/utilities/commitUtils';
...
export const ChangeContextMenu: React.FC<{ target: ChangeContextMenuTarget, onDismiss?: () => void, job?: JobParameters, stepRef?: GetJobStepRefResponse, commit?: GetChangeSummaryResponse, rangeCL?: number, cl?: number }> = ({ target, onDismiss, job, stepRef, commit, rangeCL, cl }) => {

   let jobChange = commit?.number ?? cl;

   if (jobChange === undefined) {
      jobChange = stepRef?.change ?? job?.change;
   }

   // Need at least a change number to show the menu
   if (jobChange === undefined) {
      return null;
   }

   let change = jobChange.toString();
   const pfMenuDisplay = !stepRef ? getPreflightDisplay(job?.preflightCommitId, job?.preflightChange) : undefined;
   if (pfMenuDisplay) {
      change = `PF ${pfMenuDisplay}`;
   }
   ...
   menuItems.push({
      ...
      onClick: () => copyToClipboard(job?.preflightCommitId ?? change.replace("PF ", ""))
   });
   ...
   if (dashboard.swarmUrl) {
      ...
      // Stream-dependent options (history, range) only available when we have stream info
      const stream = job?.streamId ? projectStore.streamById(job.streamId) : undefined;
      if (stream) {
         ...
         if (rangeCL !== undefined) {
            const lowCL = Math.min(jobChange, rangeCL);
            const highCL = Math.max(jobChange, rangeCL);
...
export const ChangeButton: React.FC<{ job?: JobParameters, stepRef?: GetJobStepRefResponse, commit?: GetChangeSummaryResponse, hideAborted?: boolean, rangeCL?: number, pinned?: boolean, prefix?: string, buttonColor?: string }> = ({ job, stepRef, commit, hideAborted, rangeCL, pinned, prefix, buttonColor }) => {
   ...
   const pfDisplay = getPreflightDisplay(job?.preflightCommitId, job?.preflightChange);
   if (pfDisplay) {
      change = `PF ${pfDisplay}`;
   }
