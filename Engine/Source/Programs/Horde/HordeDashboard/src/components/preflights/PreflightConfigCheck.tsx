...
const PreflightConfigPanel: React.FC = () => {
   ...
   const [state, setState] = useState<{ initialCL?: string, submitting?: boolean, success?: boolean, message?: string, warnings?: string[], cluster?: string }>({ initialCL: shelvedChange, cluster: queryCluster === undefined ? "Default" : queryCluster });
   ...
   const checkPreflight = async (preflightCL?: string) => {
      ...
      try {
         ...
         setState({ submitting: false, success: response.result, message: response.message, warnings: response.warnings });
      } catch (error) {
   ...
   return (<Stack style={{ flex: 1, minHeight: 0 }}>
      <Stack styles={{ root: { paddingTop: 18, paddingLeft: 0, paddingRight: 0, width: "100%", flex: 1, minHeight: 0 } }} >
         <Stack tokens={{ childrenGap: 12 }} style={{ flex: 1, minHeight: 0, display: "flex" }}>
            <Stack style={{ width: 800, paddingLeft: 12, flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
               ...
               {(!!state.message || !!state.warnings?.length) && <Stack style={{ paddingTop: 18, paddingRight: 2, paddingBottom: 18, flex: 1, minHeight: 0 }}>
                  <Stack style={{ paddingBottom: 12 }}>
                     <Text styles={{ root: { fontFamily: "Horde Open Sans SemiBold" } }}>{state.success === false ? "Error Message" : "Result"}</Text>
                  </Stack>
                  <Stack style={{ border: "1px solid #CDCBC9", flex: 1, minHeight: 0, overflowY: "auto", padding: 18 }} tokens={{ childrenGap: 18 }}>
                     {!!state.message && <Text style={{ whiteSpace: "pre-wrap" }}>{state.message}</Text>}
                     {!!state.warnings?.length && (() => {
                        const warningColor = dashboard.getStatusColors().get(StatusColor.Warnings)!;
                        return <Stack tokens={{ childrenGap: 8 }}>
                           <Text styles={{ root: { fontFamily: "Horde Open Sans SemiBold", color: warningColor } }}>Warnings</Text>
                           <Stack style={{ border: `1px solid ${warningColor}`, backgroundColor: warningColor + "1A", padding: 12 }} tokens={{ childrenGap: 8 }}>
                              {state.warnings.map((warning, index) => (
                                 <Text key={index} style={{ whiteSpace: "pre-wrap" }}>{warning}</Text>
                              ))}
                           </Stack>
                        </Stack>;
                     })()}
   ...
   return <Stack>
      <Modal isOpen={true} isBlocking={true} topOffsetFixed={true} styles={{ main: { padding: 8, width: 880, height: 720, hasBeenOpened: false, top: "80px", position: "absolute" } }} onDismiss={() => onClose()} className={hordeClasses.modal}>
         <Stack className="horde-no-darktheme" styles={{ root: { paddingTop: 10, paddingRight: 0 } }}>
            <Stack style={{ paddingLeft: 24, paddingRight: 12, flex: 1, minHeight: 0 }}>
               <Stack tokens={{ childrenGap: 12 }} style={{ height: 700, display: "flex", flexDirection: "column" }}>
                  ...
                  <Stack styles={{ root: { paddingLeft: 4, paddingRight: 0, paddingTop: 8, paddingBottom: 4, flex: 1, minHeight: 0 } }}>
