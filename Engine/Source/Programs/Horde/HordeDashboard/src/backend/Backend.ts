...
import { AccountClaimMessage, AddBuildHealthFiltersRequest, AgentData, AgentQuery, ApproveAgentsRequest, AuditLogEntry, AuditLogQuery, BatchUpdatePoolRequest, ChangeSummaryData, CreateAccountRequest, CreateBisectTaskRequest, CreateBisectTaskResponse, CreateDeviceRequest, CreateDeviceResponse, CreateExternalIssueRequest, CreateExternalIssueResponse, CreateJobRequest, CreateJobResponse, CreateNoticeRequest, CreatePoolRequest, CreateServiceAccountRequest, CreateServiceAccountResponse, CreateSoftwareResponse, CreateSubscriptionRequest, CreateSubscriptionResponse, CreateZipRequest, DashboardPreference, DeepLinkResponse, DevicePoolTelemetryQuery, DeviceTelemetryQuery, EventData, FindArtifactsResponse, FindIssueResponse, FindJobTimingsResponse, GetAccountResponse, GetAgentLeaseResponse, GetAgentTelemetrySampleResponse, GetArtifactDirectoryResponse, GetArtifactResponse, GetBatchLogEventsRequest, GetBatchLogEventsResponse, GetBisectTaskResponse, GetBuildHealthFilterResponse, GetBuildHealthFiltersRequest, GetDashboardConfigResponse, GetDevicePlatformResponse, GetDevicePoolResponse, GetDevicePoolTelemetryResponse, GetDeviceReservationResponse, GetDeviceResponse, GetDeviceTelemetryResponse, GetExternalIssueProjectResponse, GetExternalIssueResponse, GetIssueStreamResponse, GetJobsTabResponse, GetJobStepRefResponse, GetJobStepTraceResponse, GetJobTimingResponse, GetLabelDetailsResponse, GetLogEventResponse, GetLogFileResponse, GetNoticeResponse, GetNotificationResponse, GetPendingAgentsResponse, GetPerforceServerStatusResponse, GetPoolResponse, GetPoolSummaryResponse, GetServerInfoResponse, GetServerSettingsResponse, GetServiceAccountResponse, GetStorageDashboardResponse, GetSoftwareResponse, GetSubscriptionResponse, GetTestDataDetailsResponse, GetTestDataRefResponse, GetTestMetaResponse, GetTestResponse, GetTestsRequest, GetTestStreamResponse, GetToolResponse, GetToolSummaryResponse, GetUserResponse, GetUtilizationTelemetryResponse, GlobalConfig, IssueData, IssueQuery, IssueQueryV2, JobData, JobQuery, JobsTabColumnType, JobStepOutcome, JobStreamQuery, JobTimingsQuery, LeaseData, LogData, LogEventQuery, LogLineData, PoolData, PoolQuery, PreflightConfigResponse, ProjectData, ScheduleData, ScheduleQuery, SearchLogFileResponse, ServerPluginInfoResponse, ServerStatusResponse, ServerUpdateResponse, SessionData, StreamData, TabType, TestData, UpdateAccountRequest, UpdateAgentRequest, UpdateBisectTaskRequest, UpdateBuildHealthFiltersRequest, UpdateCurrentAccountRequest, UpdateDeploymentRequest, UpdateDeviceRequest, UpdateGlobalConfigRequest, UpdateIssueRequest, UpdateJobRequest, UpdateLeaseRequest, UpdateNoticeRequest, UpdateNotificationsRequest, UpdatePoolRequest, UpdateServerSettingsRequest, UpdateServiceAccountRequest, UpdateServiceAccountResponse, UpdateStepRequest, UpdateStepResponse, UpdateTemplateRefRequest, UpdateUserRequest, UsersQuery } from './Api';
...
export class Backend {
    ...
    getAgentLeases(agentId: string, minTime: Date, maxTime: Date): Promise<GetAgentLeaseResponse[]> {
        ...
        params.push("finishTime=" + maxTime.toISOString());
        const paramString = params.join("&");
    ...
    getLabelDetails(jobId: string, labelIndex: number, config?: { signal?: AbortSignal }): Promise<GetLabelDetailsResponse> {
        return new Promise<GetLabelDetailsResponse>((resolve, reject) => {
            this._fetch.get(`/api/v1/jobs/${jobId}/labels/${labelIndex}/details`, { signal: config?.signal }).then((response) => {
                resolve(response.data as GetLabelDetailsResponse);
            }).catch((reason) => {
                reject(reason);
            });
        });
    ...
    getBatchLogEvents(request: GetBatchLogEventsRequest): Promise<GetBatchLogEventsResponse> {
        return new Promise<GetBatchLogEventsResponse>((resolve, reject) => {
            this._fetch.post(`/api/v1/logs/events`, request).then((value) => {
                resolve(value.data as GetBatchLogEventsResponse);
            }).catch(reason => {
                reject(reason);
            });
        });
    ...
    getJobStepHistoryBatch(streamId: string, stepNames: string[], count: number = 10, templateId: string) {
        return new Promise<GetJobStepRefResponse[]>((resolve, reject) => {
            ...
            this._fetch.post(`/api/v1/streams/${streamId}/history`, {}, {
                params: params
            }).then((value) => {
                let results = (value.data ?? []) as GetJobStepRefResponse[];
                results = results.filter(r => r.outcome !== JobStepOutcome.Unspecified);
    ...
    getBatchChangeSummaries(streamId: string, changeNumbers?: number[], filter?: string): Promise<ChangeSummaryData[]> {
        return new Promise<ChangeSummaryData[]>((resolve, reject) => {
            const body = changeNumbers ?? [];
            const params: any = {
            };
            ...
            this._fetch.post(`/api/v1/streams/${streamId}/changes`, body, { params }
            ).then((value) => {
                resolve(value.data as ChangeSummaryData[]);
            }).catch(reason => {
                reject(reason);
    ...
    getStorageDashboard(minTime?: Date, maxTime?: Date, trendCount?: number): Promise<GetStorageDashboardResponse> {
        const params = new URLSearchParams();
        if (minTime) params.append("minTime", minTime.toISOString());
        if (maxTime) params.append("maxTime", maxTime.toISOString());
        if (trendCount) params.append("trendCount", trendCount.toString());

        return new Promise<GetStorageDashboardResponse>((resolve, reject) => {
            this._fetch.get(`/api/v1/storage/dashboard?${params.toString()}`).then((response) => {
                const data = response.data as GetStorageDashboardResponse;
                resolve(data);
            }).catch(reason => { reject(reason); });
        });
    }
    ...
    // get account access for a specific scope and action
    getAccountAccess(scope: string, action: string): Promise<any> {
        return new Promise<any>((resolve, reject) => {
            this._fetch.get(`/account/access`, { params: { scope: scope, action: action } }).then((value) => {
                resolve(value.data as any);
            }).catch(reason => {
                reject(reason);
            });
    ...
    //#region -- Build Health Filters --

    // add a build health project filter
    addBuildHealthProjectFilter(request: AddBuildHealthFiltersRequest): Promise<GetBuildHealthFilterResponse> {
        return new Promise<GetBuildHealthFilterResponse>((resolve, reject) => {
            this._fetch.post(`/api/v1/buildhealth/filters`, request).then((value) => {
                resolve(value.data as GetBuildHealthFilterResponse);
            }).catch(reason => {
                reject(reason);
            });
        });
    ...
    // update a build health project filter
    updateBuildHealthProjectFilter(filterId: string, request: UpdateBuildHealthFiltersRequest): Promise<GetBuildHealthFilterResponse> {
        return new Promise<GetBuildHealthFilterResponse>((resolve, reject) => {
            this._fetch.put(`/api/v1/buildhealth/filters/${filterId}`, request).then((value) => {
                resolve(value.data as GetBuildHealthFilterResponse);
            }).catch(reason => {
                reject(reason);
            });
    ...
    // delete a build health project filter
    deleteBuildHealthProjectFilter(filterId: string): Promise<boolean> {
        return new Promise<boolean>((resolve, reject) => {
            this._fetch.delete(`/api/v1/buildhealth/filters/${filterId}`).then(() => {
                resolve(true);
            }).catch(() => {
                reject(false);
            });
    ...
    // get build health project filters
    getBuildHealthProjectFilters(query: GetBuildHealthFiltersRequest): Promise<GetBuildHealthFilterResponse[]> {
        return new Promise<GetBuildHealthFilterResponse[]>((resolve, reject) => {
            this._fetch.get(`/api/v1/buildhealth/filters`, { params: query }).then((value) => {
                resolve(value.data as GetBuildHealthFilterResponse[]);
            }).catch(reason => {
                reject(reason);
            });
    ...
    // #endregion -- Build Health Filters --

    // #region -- Deep Links --

    /// Get a deep link to a channel
    getDeepLinkChannelLink(channelId: string): Promise<DeepLinkResponse> {
        return new Promise<DeepLinkResponse>((resolve, reject) => {
            this._fetch.get(`/api/v1/deeplink/channel`, {
                params: {channelId: channelId}
            }).then((value) => {
                resolve(value.data as DeepLinkResponse);
            }).catch((reason) => {
                reject(reason);
            });
        });
    ...
    /// Get a deep link to a direct message
    getDeepLinkDirectMessageLink(userIds: string[]): Promise<DeepLinkResponse> {
        const request = { userIds: userIds };
        return new Promise<DeepLinkResponse>((resolve, reject) => {
            this._fetch.post(`/api/v1/deeplink/directmessage`, request).then((value) => {
                resolve(value.data as DeepLinkResponse);
            }).catch((reason) => {
                reject(reason);
            });
