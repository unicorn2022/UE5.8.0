...
export type GetBatchLogEventsRequest = {
	logIds: string[];
	count?: number;
	severity?: EventSeverity;
}

export type BatchLogEventsEntry = {
	total: number;
	events: EventData[];
}

export type GetBatchLogEventsResponse = {
	logEvents: Record<string, BatchLogEventsEntry>;
}
...
export type GetDashboardConfigResponse = {
	...
	/** The server's global config version number */
...
export type GetPoolResponse = {
	...
	/** True if pool is managed via config files (read-only in dashboard). Null on pre-V3 servers. */
	configManaged?: boolean;

	/** True if this config-managed pool also has a database entry (shadow pool) that can be deleted. */
	hasDatabaseEntry?: boolean;
...
export type CreateJobRequest = {
	...
	/** The commit id to build. Can be null for latest. */
	commitId?: string;
	...
	/** Commit hash for branch-based preflights */
...
export type GetReportResponse = {
	...
	/** The job or step message */
	message?: string;

	/** The job or step severity */
	severity?: string;

	/** Color to display for message severity */
	color?: string;
}
...
export type GetStepResponse = {
	...
	/// Indices of nodes which may have succeeded for this node to run
	optionalInputDependencies?: JobStepId[];
...
export type GetBatchResponse = {
	...
	/** Human-readable names of lease outcome reasons (only populated when batch has an error) */
	leaseOutcomeReasons?: string[];
}
...
export type GetAgentTelemetrySampleResponse = {
	...
	usedRam: number;
	totalRam: number;
	workingFreeDisk: number;
	workingTotalDisk: number;
	systemFreeDisk: number;
	systemTotalDisk: number;
}
...
export type WorkflowConfig = {
	...
	/** Channel where items to triage are sent */
	triageChannel?: string;
}

/**Response describing a stream alias */
export type GetStreamAliasResponse = {

	/**The unique name/identifier for this alias */
	name: string;

	/**The subset of template IDs available through this alias */
	templates: string[];
...
export type GetStreamResponse = {
	...
	/** Stream aliases for this stream */
	aliases?: GetStreamAliasResponse[];
...
// --- Label Details (hover card) ---

/** Aggregated label details for hover card display */
export interface GetLabelDetailsResponse {
	dashboardName?: string;
	dashboardCategory?: string;
	state: LabelState;
	outcome: LabelOutcome;
	steps: GetLabelStepResponse[];
	logEvents: Record<string, GetLabelLogEventsResponse>;
	totalStepCount: number;
	issues: GetLabelIssueResponse[];
	previousBuild?: GetLabelPreviousBuildResponse;
	crossStreamChain?: GetLabelCrossStreamChainResponse;
}

/** Step summary within a label */
export interface GetLabelStepResponse {
	id: string;
	name: string;
	state: JobStepState;
	outcome: JobStepOutcome;
...
/** Log events for a single step's log */
export interface GetLabelLogEventsResponse {
	total: number;
	events: GetLogEventResponse[];
}

/** Build health issue summary for label hover */
export interface GetLabelIssueResponse {
	id: number;
	summary: string;
	severity: IssueSeverity;
	owner?: GetThinUserInfoResponse;
	acknowledged: boolean;
	resolvedInStream: boolean;
	externalIssueKey?: string;
	workflowThreadUrl?: string;
}

/** Previous build's label status */
export interface GetLabelPreviousBuildResponse {
	jobId: string;
	change: number;
	labelIndex: number;
	state: LabelState;
...
export interface GetLabelCrossStreamChainResponse {
	botName: string;
	entries: GetLabelCrossStreamEntry[];
}

/** Single entry in the cross-stream merge chain */
export interface GetLabelCrossStreamEntry {
	streamId: string;
	branchName: string;
	jobId?: string;
	state?: LabelState;
	outcome?: LabelOutcome;
	hasApprovalGateBefore: boolean;
	isCurrent: boolean;
}
...
export type GetIssueResponse = {
	...
	/**Root cause commit id */
	rootCommitId?: CommitId;

	/**Root cause owner info*/
	rootCauseOwnerInfo?: GetThinUserInfoResponse;
...
export type UpdateIssueRequest = {
	...
	/**Root cause commit id */
	rootCommitId?: CommitId | null;

	/**Root cause owner id*/
	rootCauseOwnerId?: string | null;
...
export enum DashboardPreference {
	...
	ColorUser1 = "ColorUser1",
	ColorUser2 = "ColorUser2",
	ColorUser3 = "ColorUser3",
...
export type GetDashboardFeaturesResponse = {
	...
	/** Show automated tests (legacy) on the server menu */
	showTests?: boolean;
...
export type ServerPluginInfoResponse = {
	...
	loaded: boolean,
	version?: string;
	/// List of ACL actions the current user has for this plugin
	userAcls?: string[];
};
...
export type FindIssueResponse = {
	...
	/**Root cause owner id*/
	rootCauseOwner?: GetThinUserInfoResponse;
...
/// Build Health Filters

/**
 * Datastructure that describes an add request for Build Health Filters.
 */
export type AddBuildHealthFiltersRequest = {
	/// The project id.
...
/**
 * Datastructure that describes an update request for Build Health Filters.
 */
export type UpdateBuildHealthFiltersRequest = {
	/// The filter name.
	filterName: string;
...
/** Parameters to request project Build Health Filters */
export type GetBuildHealthFiltersRequest = {
	/// The project id to obtain filters for.
	projectId: string;
}

/** Response object representing a Build Health Filter. */
export type GetBuildHealthFilterResponse = {
	/// The filter id.
	id: string;
...
// #region -- Storage Dashboard --

/** Consolidated storage dashboard response */
export type GetStorageDashboardResponse = {
	status: StorageDashboardStatus;
	gcMetrics: StorageDashboardGcMetrics;
	collectionStats: StorageDashboardCollectionStats;
}

/** Live GC configuration and namespace status */
export type StorageDashboardStatus = {
	enableGc: boolean;
	enableGcVerification: boolean;
	gcWorkerCount: number;
	gcQueueLimit: number;
	enableReliableGcEnqueue: boolean;
	enablePerNamespaceQueueIsolation: boolean;
	gcBlobIngestionBatchSize: number;
	gcBatchSize: number;
	bundleCacheDir: string;
	bundleCacheSize: string;
	namespaces: StorageNamespaceStatus[];
	backends: StorageBackendInfo[];
}

/** Per-namespace status */
export type StorageNamespaceStatus = {
	id: string;
	backend: string;
	prefix: string;
...
export type StorageDashboardGcMetrics = {
	entries: StorageGcMetricEntry[];
}

/** Single GC metric entry (per-tick snapshot) */
export type StorageGcMetricEntry = {
	time: string;
	namespaceId: string;
	blobsDeleted: number;
	blobsChecked: number;
...
/** MongoDB collection stats container */
export type StorageDashboardCollectionStats = {
	entries: CollectionStatsSnapshot[];
}
