...
export class Dashboard {
    ...
    get hordeGenericAdmin(): boolean {
        return this.hordeAdmin || !!this.claims?.find(c => c.type === "http://epicgames.com/ue/horde/role" && c.value === "admin");
    }
    ...
    get platform(): UserPlatform {

        const uaData = (window.navigator as any).userAgentData?.platform?.toLowerCase() ?? "";
        if (uaData) {
            if (uaData === "windows") return UserPlatform.Windows;
            if (uaData === "macos") return UserPlatform.Mac;
            if (uaData === "linux") return UserPlatform.Linux;
        }

        const userAgent = window.navigator.userAgent.toLowerCase();
        if (userAgent.indexOf("windows") > -1) return UserPlatform.Windows;
        if (userAgent.indexOf("macintosh") > -1 || userAgent.indexOf("mac os") > -1) return UserPlatform.Mac;
        if (userAgent.indexOf("linux") > -1) return UserPlatform.Linux;
    ...
    getDefaultStatusColors = (): Map<StatusColor, string> => {
        ...
        const colors = new Map<StatusColor, string>([
            ...
            [StatusColor.User1, dark ? "#8e4747" : "#8e4747"],
            [StatusColor.User2, dark ? "#785EF0" : "#785EF0"],
            [StatusColor.User3, dark ? "#FE6100" : "#FE6100"],            
    ...
    getStatusColors = (): Map<StatusColor, string> => {
        ...
        const user1 = this.getPreference(DashboardPreference.ColorUser1);
        const user2 = this.getPreference(DashboardPreference.ColorUser2);
        const user3 = this.getPreference(DashboardPreference.ColorUser3 );
        ...
        const colors = new Map<StatusColor, string>([
            ...
            [StatusColor.User1, user1 ? user1 : defaultStatusColors.get(StatusColor.User1)!],
            [StatusColor.User2, user2 ? user2 : defaultStatusColors.get(StatusColor.User2)!],
            [StatusColor.User3, user3 ? user3 : defaultStatusColors.get(StatusColor.User3)!],
