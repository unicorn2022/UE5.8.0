...
public class FakeHordeWebApp : IAsyncDisposable
{
	...
	private readonly DatabaseIntegrationTest _dbIntegrationTest;
	...
	public FakeHordeWebApp(Action<IServiceCollection>? configureServices = null, Dictionary<string, string>? settings = null)
	{
		...
		_dbIntegrationTest = new DatabaseIntegrationTest();

		void ConfigureServices(IServiceCollection services)
		{
			configureServices?.Invoke(services);
			services.AddSingleton<IMongoService>(_dbIntegrationTest.GetMongoServiceSingleton());
			services.AddSingleton<IRedisService>(_dbIntegrationTest.GetRedisServiceSingleton());
		}
	...
	public async ValueTask DisposeAsync()
	{
		try
		{
			...
			await _dbIntegrationTest.DisposeAsync();
