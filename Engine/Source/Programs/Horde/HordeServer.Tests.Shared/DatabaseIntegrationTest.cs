...
namespace HordeServer.Tests
{
	...
	public sealed class RedisInstance : IDisposable
	{
		...
		private static (int min, int max) DatabaseNumbers { get; } = (1000, 2000);
		private static readonly BlockingCollection<int> s_availableRedisDbs = new ();
		...
		private const bool UseExistingRedisInstance = true;
		private const int RedisPort = 6379;
		
		static RedisInstance()
		{
			for (int i = DatabaseNumbers.min; i < DatabaseNumbers.max; i++)
			{
				s_availableRedisDbs.Add(i);
			}
		}
		...
		public RedisInstance()
		{
			lock (s_lockObject)
			{
				...
				DatabaseNumber = CheckoutDatabase();
			}
		}

		public void Dispose()
		{
			ReturnDatabase(DatabaseNumber);
		}
		
		private static int CheckoutDatabase()
		{
			TimeSpan timeoutToUse = TimeSpan.FromMinutes(1);
			if (s_availableRedisDbs.TryTake(out int dbNumber, timeoutToUse))
			{
				return dbNumber;
			}
        
			throw new TimeoutException($"Failed to checkout database within {timeoutToUse}");
		}

		private static void ReturnDatabase(int dbNumber)
		{
			s_availableRedisDbs.Add(dbNumber);
		}
	...
	{
		private static readonly object s_lockObject = new object();
		private static MongoService? s_globalMongoService;
		...
		public override async ValueTask DisposeAsync()
		{
			...
			if (_mongoService is IAsyncDisposable ad)
			{
		...
		public IMongoService GetMongoServiceSingleton()
		{
			lock (s_lockObject)
			{
				if (s_globalMongoService == null)
				{
					...
					{
						MongoDatabaseName = _mongoInstance.DatabaseName,
						MongoConnectionString = _mongoInstance.ConnectionString
					};
					
					MongoCommandTracer mongoTracer = new(OpenTelemetryTracers.MongoDb, _loggerFactory.CreateLogger<MongoCommandTracer>());
					s_globalMongoService = new MongoService(Options.Create(ss), redisService, mongoTracer, OpenTelemetryTracers.Horde, _loggerFactory.CreateLogger<MongoService>(), _loggerFactory);
				}

				if (_mongoService == null)
				{
					_mongoService = new TestScopedMongoService(s_globalMongoService.GetClient());
				}
			}
