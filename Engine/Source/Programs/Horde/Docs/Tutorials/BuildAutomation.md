...
4. Enable the example project by uncommenting the following lines in `globals.json`:
   ...
   * A project in Horde parlance is typically a game or project, analagous to a stream depot in
   ...
   `//UE5/Release-5.8`, `//UE5/Dev-Main-HordeDemo` and so on.
   * The referenced config file, `ue5.project.json` exists in the same directory and references a stream configured
   in `ue5-release-5.8.stream.json`. The name of this file is not important for this tutorial.

5. Open the `ue5-release-5.8.stream.json` file and update the `name` property to a stream on your Perforce server.
   The default is `//UE5/Release-5.8-HordeSync`.
...
The example `ue5-release-5.8.stream.json` file configures the appearance of its page in the Horde Dashboard, as
