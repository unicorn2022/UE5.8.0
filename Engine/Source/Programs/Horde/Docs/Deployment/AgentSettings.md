...
`ramGb` | `integer`<br>Maximum available RAM in gigabytes. Defaults to available RAM.
...
`tempStorageMaxBatchSize` | `integer`<br>Maximum number of temp storage exports to write in a single request
`logOptionalInputFailureAsWarning` | `boolean`<br>Whether to log optional input retrieval failures as warnings instead of information messages. Defaults to false (information level).
