...
`exampleplugin` | [ExamplePluginConfig](#examplepluginconfig)<br>Configuration for the exampleplugin plugin
...
`robomerge` | [RoboMergeConfig](#robomergeconfig)<br>RoboMerge branchmap file configuration for the merge graph service. When null or empty, the service is inert (no Perforce connections, no API data).
...
`enableJobTaskConformCheck` | `boolean`<br>Whether to check if a conform is required on an agent before assigning a lease for a job task source.
`issueFixedTag` | `string`<br>Commit tag to use for marking issues as fixed
`issues` | [IssueConfig](Issues.md)<br>Build health issue configurations
`enableIssueRootCauseAnalysisNotifications` | `boolean`<br>Whether to allow notifications for Root Cause Analysis to be sent
`enableAutoRcaSlackNotifications` | `boolean`<br>Global kill switch for posting auto-RCA results to Slack triage threads. When false (default), no auto-RCA Slack notifications are sent regardless of per-workflow AutoRcaSlackEnabled settings.
`autoRcaUseMultiStage` | `boolean`<br>Whether to use multi-stage LLM analysis for auto-RCA. Multi-stage is slower but potentially more accurate. Set to false for faster processing with higher throughput.
`maxConcurrentRcaRequests` | `integer`<br>Maximum number of concurrent auto-RCA requests. Limits load on the external LLM API. Set to 0 to disable auto-RCA globally (acts as a kill switch).
`issueFindSuspectsMaxSuccessDays` | `integer`<br>The maximum days since node success to scan for suspects
`issueFindSuspectsMaxScoringSeconds` | `integer`<br>The maximum number of seconds to spend scording suspects
`issueFindSuspectsMaxChanges` | `integer`<br>Maximum number of commits to examine for suspects. Previously hardcoded as const MaxChanges = 500.
`issueFindSuspectsEnableParallelScoring` | `boolean`<br>Enable parallel scoring of commits for suspect finding. When enabled, commits are scored concurrently up to MaxParallelism.
`issueFindSuspectsMaxParallelism` | `integer`<br>Maximum concurrent commit file lookups when parallel scoring is enabled. Higher values improve speed but increase Perforce server load.
`issueFindSuspectsEnableEarlyExit` | `boolean`<br>Enable early exit when theoretical maximum score is achieved. Only exits early if the best possible score has been found.
`issueFindSuspectsEnableBidirectionalQuery` | `boolean`<br>Enable bidirectional commit query to capture both recent and old commits. Queries half the limit from each end of the commit range.
`issueFindSuspectsEnableFingerprintFallback` | `boolean`<br>Enable fingerprint-based fallback when no suspects are found. Helps prevent duplicate issues when commit range exceeds MaxChanges.
`issueFingerprintFallbackMinScore` | `integer`<br>Minimum score required for fingerprint fallback matching. Higher values are more conservative (fewer false matches).
`issueFingerprintFallbackMaxAgeDays` | `integer`<br>Maximum age in days for issues to be considered in fingerprint fallback.
`issueFingerprintFallbackExcludedTypes` | `string[]`<br>Fingerprint types excluded from fallback matching (too generic).
`enableExpandedIgnorePatterns` | `boolean`<br>Whether to enable expanded IgnorePatterns discovery (workspace-wide scanning + config-specified files). When false (default), only Engine directory is scanned for ignore patterns.
`lowContextFingerprintTypes` | `string[]`<br>Fingerprint type prefixes considered "low context" for suspect mention suppression. When an issue's fingerprint type starts with any of these prefixes AND all suspects are tied at rank 0, @ mentions are suppressed in Slack. Suspects are still listed without mentions. Set to an empty array to disable suppression entirely.
...
Configuration for the RoboMerge branchmap service

Name | Description
---- | -----------
`branchmapFiles` | `string[]`<br>Perforce depot paths to branchmap JSON files. Plain depot paths (e.g. "//MyProject/Main/Build/Streams/myproject.branchmap.json"). All files are read from the Perforce cluster specified by .
`perforceCluster` | `string`<br>Perforce cluster name to connect to for reading branchmap files. Maps to a connection configured in BuildServerConfig.Perforce[]. Default: "default" (uses the default Perforce connection).
`tickInterval` | `string`<br>How often to poll Perforce for branchmap file changes. Default: 5 minutes. Changing this requires a server restart (ticker interval is set at construction time).
...
`macros` | [ConfigMacro](#configmacro)`[]`<br>Macros within the compute config
`acl` | [AclConfig](#aclconfig)<br>Inherited root acl
`versionEnum` | [ConfigVersion](#configversion-enum)<br>Config version number
`telemetryStoreId` | `string`<br>Telemetry store for lease and compute telemetry data
...
`PoolsInConfigFiles` | All pools are managed through config files. Database pool reads are disabled.
`Latest` | Latest version number
`LatestPlusOne` | One after the last defined version number
...
Name | Description
---- | -----------
`greetingMessage` | `string`<br>The Greeting provided by the example plugin
...
`gcWorkerCount` | `integer`<br>Number of concurrent GC reachability-check workers. Default 8. Reachability checks are I/O-bound (MongoDB queries), so higher values scale throughput roughly linearly up to the database's capacity.
`gcQueueLimit` | `integer`<br>Maximum number of entries in a namespace's Redis GC queue before the blob scanner pauses. Default 50,000.
`enableReliableGcEnqueue` | `boolean`<br>Whether to await Redis writes when enqueuing blobs for GC checking. When false (default), uses fire-and-forget which can silently drop entries on Redis failures. Set true for reliable enqueue.
`enablePerNamespaceQueueIsolation` | `boolean`<br>Whether to isolate GC queue limits per-namespace. When false (default), one backed-up namespace pauses the scanner for ALL namespaces. When true, only the backed-up namespace is paused and its blobs are deferred.
`gcBlobIngestionBatchSize` | `integer`<br>Number of blobs to process per blob-ingestion tick (TickBlobsAsync). Default 500. Raise to increase scan throughput when catching up on a backlog of unscanned blobs.
`gcBatchSize` | `integer`<br>Number of blob IDs to batch together for the import-reachability check. Default 1 (current per-blob behavior). When greater than 1, uses $in query to check multiple blobs in a single MongoDB round-trip. Values of 50-200 recommended.
...
`maxDeployments` | `integer`<br>Maximum number of deployments to retain for this tool. When exceeded, the oldest deployments are removed.
