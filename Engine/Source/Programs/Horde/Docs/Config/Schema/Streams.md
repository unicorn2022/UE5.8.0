...
`vCS` | `string`<br>Version control service for stream commits
`repositoryName` | `string`<br>Repository name for version control systems that support multiple repositories
`defaultBranchName` | `string`<br>Default branch name for version control systems that use branches (e.g., "main"). If not set, defaults to "main".
...
`ignorePatternFiles` | `string[]`<br>Additional file paths (relative to workspace root) containing ignore patterns for the log parser. Each file should contain one regex per line, with lines starting with '#' treated as comments. Inherits from ProjectConfig and appends stream-level entries.
...
`retentionOptions` | [RetentionOptions](#retentionoptions)<br>Storage retention options for this stream
...
`aliases` | [StreamAliasConfig](#streamaliasconfig)`[]`<br>Stream aliases configured for this stream. Each alias provides a named subset of the stream's templates.
...
Options controlling storage retention and GC behavior for a stream. Boolean flags are safety valves: only false is meaningful (disables behavior). null means "not disabled" (default). Setting true is identical to null.

Name | Description
---- | -----------
`enableJobExpiration` | `boolean`<br>Whether job expiration is enabled for this stream. When null (default), expiration runs normally if ExpireAfterDays is greater than 0. When explicitly false, disables expiration regardless of ExpireAfterDays (safety valve).
`enableArtifactExpiration` | `boolean`<br>Override for artifact retention behavior. When false, artifact expiry is paused for this stream (useful during investigations).
`maxJobDeletionsPerTick` | `integer`<br>Maximum number of jobs to delete per hourly expiration tick, per stream. Prevents thundering-herd behavior when ExpireAfterDays is first enabled on a stream with a large backlog. Default null, treated as 1000 by consumers via null-coalescing (safe-by-default cap). Set to 0 for unlimited.
...
Configuration for a stream alias, which represents a named subset of templates

Name | Description
---- | -----------
`name` | `string`<br>The unique name/identifier for this alias
`templates` | `string[]`<br>The subset of template IDs available through this alias. These must be a subset of the enclosing IStream's Templates.
...
`escalateRootCauseAnalysisTimes` | `integer[]`<br>Times after a manual root cause analysis has been opened to escalate to the owner, in minutes. Continues to notify on the last interval once reaching the end of the list.
`autoRcaEnabled` | `boolean`<br>Whether to automatically run RCA processing for issues in this workflow
`autoRcaSlackEnabled` | `boolean`<br>Whether to post auto-RCA results to the issue's Slack triage thread
