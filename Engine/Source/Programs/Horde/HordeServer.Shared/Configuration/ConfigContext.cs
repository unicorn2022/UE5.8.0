...
namespace HordeServer.Configuration
{
	...
	public class ConfigContext
	{
		...
		/// <summary>
		/// Tracks files already processed as includes within the current include root scope,
		/// preventing diamond dependencies from causing the same file to be merged multiple times.
		/// Maps each included URI to the URI of the file that first included it.
		/// Each [ConfigIncludeRoot] entered from property/array traversal saves and restores this
		/// so sibling roots (e.g. two StreamConfigs in the same project) get independent scopes.
		/// Roots entered from include processing share the parent's scope for diamond detection.
		/// </summary>
		public Dictionary<Uri, Uri?> ProcessedIncludes { get; set; } = new Dictionary<Uri, Uri?>();

		/// <summary>
		/// Depth counter for nested [ConfigIncludeRoot] processing.
		/// </summary>
		public int IncludeRootDepth { get; set; }

		/// <summary>
		/// When true, the current PreprocessAsync call is processing an included file (recursive
		/// from the include loop). [ConfigIncludeRoot] entries should share the parent's diamond
		/// dedup scope rather than creating a fresh one, so that diamonds within the same include
		/// tree are still detected.
		/// </summary>
		public bool InIncludeScope { get; set; }
