...
namespace HordeServer.Configuration
{
	...
	public static class ConfigObject
	{
		...
		/// <param name="logger">Optional logger. When provided, invalid bases and circular dependencies are logged as warnings instead of throwing exceptions.</param>
		public static void MergeDefaults<TKey, TValue>(IEnumerable<(TKey Key, TKey? BaseKey, TValue Value)> objects, ILogger? logger = null)
			where TKey : notnull
			where TValue : class
		{
			...
			if (remainingObjects.Count > 0)
			{
				...
				foreach ((TKey key, TKey baseKey, TValue value) in remainingObjects)
				{
					if (!validKeys.Contains(baseKey!))
					{
						if (logger != null)
						{
							logger.LogWarning("{Key} has invalid/missing base {BaseKey}; skipping merge", key, baseKey);
							handledValues.Add(key, value);
						}
						else
						{
				...
				// Remove items that were handled above (invalid base, logged as warning)
				remainingObjects.RemoveAll(x => handledValues.ContainsKey(x.Key));

				if (remainingObjects.Count > 0)
				{
					List<TKey> circularKeys = remainingObjects.Select(x => x.Key).ToList();
					if (logger != null)
					{
						if (circularKeys.Count == 1)
						{
							logger.LogWarning("{Key} has a circular dependency on itself; skipping merge", circularKeys[0]);
						}
						else
						{
							logger.LogWarning("{Keys} have circular dependencies; skipping merge", StringUtils.FormatList(circularKeys.Select(x => x.ToString() ?? "(unknown)")));
						}
					}
					else
					{
