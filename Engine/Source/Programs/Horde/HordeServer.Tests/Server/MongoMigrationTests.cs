...
public class MongoMigratorTests : DatabaseIntegrationTest
{
	...
	{
		MigrationException me = Assert.ThrowsExactly<MigrationException>(() => _migrator.AddMigration(new MissingAttributeMigration()));
	...
	public void AddMigration_WithDuplicateVersion()
	{
		...
		MigrationException me = Assert.ThrowsExactly<MigrationException>(() => _migrator.AddMigration(new DuplicateVersionMigrationB()));
	...
	public async Task Upgrade_WithVersionGap_Async()
	{
		_migrator.AddMigration(new FooMigration1());
		_migrator.AddMigration(new FooMigration3());
		MigrationException me = await Assert.ThrowsExactlyAsync<MigrationException>(() => _migrator.UpgradeAsync("foo", null, CancellationToken.None));
	...
	{
		MigrationException me = Assert.ThrowsExactly<MigrationException>(() => _migrator.AddMigration(new InvalidVersionMigration()));
	...
	{
		MigrationException me = Assert.ThrowsExactly<MigrationException>(() => _migrator.AddMigration(new ShortNameMigration()));
	...
	{
		MigrationException me = await Assert.ThrowsExactlyAsync<MigrationException>(() => _migrator.GetSchemaStateAsync("unknown-plugin", CancellationToken.None));
