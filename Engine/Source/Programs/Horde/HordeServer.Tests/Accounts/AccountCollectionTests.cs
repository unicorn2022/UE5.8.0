...
namespace HordeServer.Tests.Accounts
{
	...
	public class AccountCollectionTests : ServerTestSetup
	{
		...
		public async Task CaseInsensitiveAsync()
		{
			...
			await Assert.ThrowsExactlyAsync<LoginAlreadyTakenException>(() => accountCollection.CreateAsync(new CreateAccountOptions("Bill", "bob")));
			await Assert.ThrowsExactlyAsync<LoginAlreadyTakenException>(() => account2.UpdateAsync(new UpdateAccountOptions { Login = "admin" }));
		}
	}
}
