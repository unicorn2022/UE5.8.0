...
namespace ChaosTest {
	...
	GTEST_TEST(AllEvolutions, SimTests_BoxBoxSimTest_DisableParticles)
	{
		FParticleUniqueIndicesMultithreaded UniqueIndices;
		FPBDRigidsSOAs Particles(UniqueIndices);
		THandleArray<FChaosPhysicsMaterial> PhysicalMaterials;
		...
		// Step only a few times not to trigger sleeping
		const FReal Dt = 1 / 60.f;
		for (int i = 0; i < 10; ++i)
		{
			Evolution.AdvanceOneTimeStep(Dt);
		...
		// Make sure there is at least one collisions
		EXPECT_GT(Dynamic->ParticleCollisions().Num(), 0);

		// Make sure there is at least one active collision in the constraint graph
		int32 CountActiveCollisions = 0;
		for (int32 Index = 0; Index < Dynamic->ParticleCollisions().Num(); Index++)
		{
			FParticlePairMidPhase* MidPhase = Dynamic->ParticleCollisions().GetMidPhase(Index);
			MidPhase->VisitCollisions([&CountActiveCollisions](FPBDCollisionConstraint& Constraint)
				{
					if (!Constraint.GetDisabled() && Constraint.IsCurrent() && Constraint.IsInConstraintGraph())
					{
						++CountActiveCollisions;
					}
					return ECollisionVisitorResult::Continue;
				});
		}
		EXPECT_GT(CountActiveCollisions, 0);

		Evolution.DisableParticle(Dynamic);
		Evolution.DisableParticle(Static);

		// When particles are disabled, all collisions and other constraints should also be disabled.
		// Collisions and particles should also be removed from the constraint graph.
		EXPECT_EQ(Dynamic->ParticleCollisions().Num(), 0);
		EXPECT_EQ(Evolution.GetIslandManager().GetNumConstraints(), 0);
		EXPECT_EQ(Evolution.GetIslandManager().GetNumParticles(), 0);
	}

	GTEST_TEST(AllEvolutions, SimTests_BoxBoxSimTest_DestroyParticles)
	{
		FParticleUniqueIndicesMultithreaded UniqueIndices;
		FPBDRigidsSOAs Particles(UniqueIndices);
		THandleArray<FChaosPhysicsMaterial> PhysicalMaterials;
		...
		Evolution.DestroyParticle(Dynamic);
		Evolution.DestroyParticle(Static);

		// When particles are destroyed, all collisions and other constraints should also be destroyed.
		// Collisions and particles should also be removed from the constraint graph.
		//EXPECT_EQ(Dynamic->ParticleCollisions().Num(), 0);
		EXPECT_EQ(Evolution.GetIslandManager().GetNumConstraints(), 0);
		EXPECT_EQ(Evolution.GetIslandManager().GetNumParticles(), 0);
	...
	GTEST_TEST(AllEvolutions, SimTests_BoxBoxSimTest_DisableConstraints)
	{
		FParticleUniqueIndicesMultithreaded UniqueIndices;
		FPBDRigidsSOAs Particles(UniqueIndices);
		THandleArray<FChaosPhysicsMaterial> PhysicalMaterials;
		...
		Evolution.DisableConstraints(Dynamic);
		Evolution.DisableConstraints(Static);

		// Only non-collision constraints should be disabled
		// Collisions and particles should still be active and in the constraint graph.
		EXPECT_GT(Dynamic->ParticleCollisions().Num(), 0);
		EXPECT_GT(Evolution.GetIslandManager().GetNumConstraints(), 0);
		EXPECT_GT(Evolution.GetIslandManager().GetNumParticles(), 0);
		CountActiveCollisions = 0;
		for (int32 Index = 0; Index < Dynamic->ParticleCollisions().Num(); Index++)
		{
			FParticlePairMidPhase* MidPhase = Dynamic->ParticleCollisions().GetMidPhase(Index);
			MidPhase->VisitCollisions([&CountActiveCollisions](FPBDCollisionConstraint& Constraint)
	...
	GTEST_TEST(AllEvolutions, SimTests_BoxBoxSimTest_DestroyTransientConstraints)
	{
		FParticleUniqueIndicesMultithreaded UniqueIndices;
		FPBDRigidsSOAs Particles(UniqueIndices);
		THandleArray<FChaosPhysicsMaterial> PhysicalMaterials;
		...
		Evolution.DestroyTransientConstraints(Dynamic);
		Evolution.DestroyTransientConstraints(Static);

		// Collision constraints should be destroyed
		// Collisions should not be active and in the constraint graph.
		// Particles would be removed from constraint graph in the next tick
		EXPECT_EQ(Dynamic->ParticleCollisions().Num(), 0);
		EXPECT_EQ(Evolution.GetIslandManager().GetNumConstraints(), 0);
		EXPECT_GT(Evolution.GetIslandManager().GetNumParticles(), 0);
		CountActiveCollisions = 0;
		...
		EXPECT_EQ(CountActiveCollisions, 0);
	}

	GTEST_TEST(AllEvolutions, SimTests_BoxBoxSimTest_DisconnectConstraints)
	{
		FParticleUniqueIndicesMultithreaded UniqueIndices;
		FPBDRigidsSOAs Particles(UniqueIndices);
		THandleArray<FChaosPhysicsMaterial> PhysicalMaterials;
		...
		Evolution.DisconnectPersistentConstraints(TSet<FGeometryParticleHandle*>({ Dynamic, Static }));

		// Only non-collision constraints should be disconnected
		// Collisions and particles should still be active and in the constraint graph.
		EXPECT_GT(Dynamic->ParticleCollisions().Num(), 0);
		EXPECT_GT(Evolution.GetIslandManager().GetNumConstraints(), 0);
		EXPECT_GT(Evolution.GetIslandManager().GetNumParticles(), 0);
