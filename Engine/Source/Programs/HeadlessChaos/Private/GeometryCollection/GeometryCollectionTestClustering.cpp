...
namespace GeometryCollectionTest
{
	...
	GTEST_TEST(AllTraits, GeometryCollection_RigidBodies_ClusterTest_BreakClusterParticle)
	{
		...
		for (int Frame = 1; Frame < 4; Frame++)
		{
			...
			//UE_LOGF(GCTCL_Log, Verbose, "FRAME : %d", Frame);
			//for (int32 rdx = 0; rdx < (int32)Particles.Size(); rdx++)
			//{
			//  UE_LOGF(GCTCL_Log, Verbose, "... ... ...Disabled[%d] : %d", rdx, Particles.Disabled(rdx));
			//  UE_LOGF(GCTCL_Log, Verbose, "... ... ...    InvM[%d] : %f", rdx, Particles.InvM(rdx));
			//}
