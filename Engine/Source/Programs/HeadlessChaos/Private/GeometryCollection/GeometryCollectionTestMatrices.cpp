...
#define EXPECT_FVECTOR_NEAR(A,B,T) EXPECT_THAT(A, VectorNear(B, T)) << *FString("Expected: " + B.ToString() + "\nActual:   " + A.ToString());
	...
	{ 
		...
		EXPECT_TRUE(FrameRot.Equals(FVector3f(0, 0, -180), 0.0001f) || FrameRot.Equals(FVector3f(0, 0, 180), 0.0001f));
		EXPECT_FVECTOR_NEAR(Frame.GetTranslation(), FVector3f(-10, 10, 0), 0.0001f);
	}
	...
	void TransformMatrixElement()
	{
		...
		//{
		//	UE_LOGF(GCTM_Log, Verbose, "... Position[%d] : (%3.5f,%3.5f,%3.5f)", rdx, (Collection.Transform)[rdx].GetTranslation().X, (Collection.Transform)[rdx].GetTranslation().Y, (Collection.Transform)[rdx].GetTranslation().Z);
		//	UE_LOGF(GCTM_Log, Verbose, "... GlobalM0[%d] : (%3.5f,%3.5f,%3.5f)", rdx, GlobalMatrices0[rdx].GetTranslation().X, GlobalMatrices0[rdx].GetTranslation().Y, GlobalMatrices0[rdx].GetTranslation().Z);
		//	UE_LOGF(GCTM_Log, Verbose, "... GlobalM1[%d] : (%3.5f,%3.5f,%3.5f)", rdx, GlobalMatrices1[rdx].GetTranslation().X, GlobalMatrices1[rdx].GetTranslation().Y, GlobalMatrices1[rdx].GetTranslation().Z);
		//}
