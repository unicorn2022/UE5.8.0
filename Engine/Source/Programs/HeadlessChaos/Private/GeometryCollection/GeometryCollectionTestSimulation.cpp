...
namespace GeometryCollectionTest
{
	...
	GTEST_TEST(AllTraits, GeometryCollection_RigidBodies_SingleKinematicBody)
	{
		...
		{
			EXPECT_EQ(Collection->DynamicCollection->GetNumTransforms(), 1);
			//UE_LOGF(LogTest, Verbose, "Position : (%3.5f,%3.5f,%3.5f)", Transform[0].GetTranslation().X, Transform[0].GetTranslation().Y, Transform[0].GetTranslation().Z);
	...
	GTEST_TEST(AllTraits, GeometryCollection_RigidBodies_SleepingDontMove)
	{
		...
		{
			UnitTest.Advance();
			//UE_LOGF(LogTest, Verbose, "Position[0] : (%3.5f,%3.5f,%3.5f)", Transform0.GetTranslation().X, Transform0.GetTranslation().Y, Transform0.GetTranslation().Z);
		}
	...
	GTEST_TEST(AllTraits, GeometryCollection_RigidBodies_SleepingActivation)
	{
		...
		for (int i = 0; i < 15; i++)
		{
			...
			//UE_LOGF(LogTest, Verbose, "Position[0] : (%3.5f,%3.5f,%3.5f)", Transform0.GetTranslation().X, Transform0.GetTranslation().Y, Transform0.GetTranslation().Z);
			//UE_LOGF(LogTest, Verbose, "Position[1] : (%3.5f,%3.5f,%3.5f)", Transform1.GetTranslation().X, Transform1.GetTranslation().Y, Transform1.GetTranslation().Z);
		}
