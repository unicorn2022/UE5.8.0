...
namespace ChaosTest
{
	...
	class SleepingTests : public ::testing::TestWithParam<bool>
	{
		...
		void SetPartialIslandSleepUnitTestDefaults()
		{
			...
			check(CVarPartialSleepCollisionConstraintsOnly && CVarPartialSleepCollisionConstraintsOnly->IsVariableInt());
			PartialSleepCollisionConstraintsOnly_EditorDefault = CVarPartialSleepCollisionConstraintsOnly->GetInt();
			CVarPartialSleepCollisionConstraintsOnly->Set(PartialSleepCollisionConstraintsOnly_UnitTestDefault);
		...
		void RestorePartialIslandSleepEditorDefaults()
		{
			...
			CVarPartialSleepCollisionConstraintsOnly->Set(PartialSleepCollisionConstraintsOnly_EditorDefault);
		...
		int32 PartialSleepCollisionConstraintsOnly_UnitTestDefault = 0;
		...
		int32 PartialSleepCollisionConstraintsOnly_EditorDefault;
	...
	public:
		FSleepingTest(const bool bPartialSleeping)
			...
			, CVarPartialSleeping()
		{
			// Create materials before creating the solver so they are included in SimMaterials
			...
			FMaterialHandle NoSleepMaterialHandle = FPhysicalMaterialManager::Get().Create();
			FPhysicsMaterial* NoSleepPhysicsMaterial = NoSleepMaterialHandle.Get();
			NoSleepPhysicsMaterial->Friction = 0.7;
			NoSleepPhysicsMaterial->StaticFriction = 0.0;
			NoSleepPhysicsMaterial->Restitution = 0.0;
			NoSleepPhysicsMaterial->Density = 1.0;
			NoSleepPhysicsMaterial->SleepingLinearThreshold = 0.0;
			NoSleepPhysicsMaterial->SleepingAngularThreshold = 0.0;
			NoSleepPhysicsMaterial->SleepCounterThreshold = 4;
			NoSleepMaterialData.Materials.Add(NoSleepMaterialHandle);
		...
		FRigidBodyHandle_External* MakeCube(const FVec3& Position, const FReal Size = 200, const FReal Mass = 1, const FMaterialData* OverrideMaterialData = nullptr)
		{
			...
			const FMaterialData& ShapeMaterial = OverrideMaterialData ? *OverrideMaterialData : MaterialData;
			for (const TUniquePtr<FPerShapeData>& Shape : CubeParticle->ShapesArray())
			{
				Shape->SetMaterialData(ShapeMaterial);
			}
		...
		FMaterialData NoSleepMaterialData;
	...
	{
		const int32 PartialSleepCollisionConstraintsOnly = GetParam() ? 1 : 0;
		const int32 PartialSleepCollisionConstraintsOnlyDefault = CVarPartialSleepCollisionConstraintsOnly->GetInt();
		CVarPartialSleepCollisionConstraintsOnly->Set(PartialSleepCollisionConstraintsOnly);
		...
		if (PartialSleepCollisionConstraintsOnly == 1)
		{
		...
		else if (PartialSleepCollisionConstraintsOnly == 0)
		{
		...
		CVarPartialSleepCollisionConstraintsOnly->Set(PartialSleepCollisionConstraintsOnlyDefault);
	}
	...
	TEST_P(SleepingTests, StackOfCubesNeverSleep)
	{
		const bool bPartialSleeping = GetParam();
		FSleepingTest Test(bPartialSleeping);

		// This test builds the stack top-down to reverse the processing order SleepNonMovingParticles() vs level information.
		// If we build the stack bottom-up, we always put particles to sleep bottom-up. 
		// This hides issues with sleeping particles on top of particles which cannot sleep.
		const int32 Num = 6;
		const FReal Size = 200;
		const FVec3 Pos = FVec3(0);
		const FReal InitialHeight = Size / 2.0f;
		for (int32 Id = Num - 1; Id >= 0; --Id)
		{
			const FReal Z = InitialHeight + Size * Id;
			Test.MakeCube(Pos + FVec3(0, 0, Z), Size);
		}
		Test.MakeFloor();

		// Tick once to create constraints
		Test.Advance();

		// The SingleParticlePhysicsProxy will automatically update sleep thresholds when calling SetSleepType.
		Test.ParticleHandles[2]->SetSleepType(ESleepType::NeverSleep);
	...
	TEST_P(SleepingTests, StackOfCubesZeroSleepThreshold)
	{
		const bool bPartialSleeping = GetParam();
		FSleepingTest Test(bPartialSleeping);

		// Build the stack from top to bottom so that the island manager processes sleeping from highest to lowest level.
		// Inject one cube into the stack that has zero sleep threshold.
		// All levels above should remain awake.
		const FReal Size = 200;
		const int32 Num = 6;
		Test.MakeFloor();
		const FReal InitialHeight = Size / 2.0f;
		for (int32 Id = Num - 1; Id >= 0; --Id)
		{
			// ParticleHandles[2] cannot sleep (3rd cube from bottom starting from 0)
			const FMaterialData* OverrideMaterialData = (Id == 3) ? &Test.NoSleepMaterialData : nullptr;
			const FReal Z = InitialHeight + Size * Id;
			Test.MakeCube(FVec3(0, 0, Z), Size, 1, OverrideMaterialData);
		}
		...
		if (bPartialSleeping)
		{
			// ParticleHandles[2] keeps all cubes above and one cube below awake
			EXPECT_TRUE(Test.ParticleHandles[5]->ObjectState() == EObjectStateType::Sleeping);
			EXPECT_TRUE(Test.ParticleHandles[4]->ObjectState() == EObjectStateType::Sleeping);
			EXPECT_TRUE(Test.ParticleHandles[3]->ObjectState() == EObjectStateType::Dynamic);
			EXPECT_TRUE(Test.ParticleHandles[2]->ObjectState() == EObjectStateType::Dynamic);
