...
namespace ChaosTest {
	...
	struct FSimpleRaycastVisitor: ISpatialVisitor<FAccelerationStructureHandle>
	{
		...
		bool bHit = false;
		bool bQueryGameThread = true; // Query game thread or physics thread data?

		bool bUseQueryFilter = false;
		Chaos::Filter::FQueryFilterData FilterData;
		...
		FSimpleRaycastVisitor(const FVec3& InStart, const Chaos::Filter::FQueryFilterData& InFilterData, bool bInQueryGameThread)
			: Start(InStart)
			, bHit(false)
			, bQueryGameThread(bInQueryGameThread)
		...
		virtual bool PrePreFilter(const FAccelerationStructureHandle& Payload) const override
		{
			if (bUseQueryFilter)
			{
				return Payload.PrePreFilter(FilterData);
			}
			return false;
		}
	...
	FSQHitBuffer<ChaosInterface::FOverlapHit> InSphereHelper(const FChaosScene& Scene, const FTransform& InTM, const FReal Radius)
	{
		...
		FOverlapAllCollisionQueryFilterCallback QueryCallback;
		ChaosInterface::FSceneQueryCommonParams CommonParams(QueryCallback);
		SQAccelerator.Overlap(Chaos::FSphere(FVec3(0), Radius), InTM, HitBuffer, CommonParams);
		return HitBuffer;
	}

	bool PrePreFilter(const Chaos::Filter::FQueryFilterData& QueryFilter, const Chaos::Filter::FShapeFilterData& ShapeFilter)
	{
		Chaos::Filter::FShapeUnionFilterData ParticleFilter;
		ParticleFilter.Combine(ShapeFilter);
		return ParticleFilter.BroadFilter(QueryFilter) == Chaos::Filter::EBroadFilterResult::Reject;
	}
	...
	GTEST_TEST(EngineInterface, UpdatingAccelerationStructurePrePreFilterOnShapeFilterChange)
	{
		...
		// Make a query filter that will block against a particle that blocks all channels but fails on the default particle.
		// The channel index of 7 is arbitrary.
		PRAGMA_DISABLE_INTERNAL_WARNINGS
		Chaos::Filter::FQueryTraceFilterBuilder Builder;
		Builder.SetCollisionChannelIndex(7);
		const Chaos::Filter::FQueryFilterData QueryFilterData = Builder.Build();
		...
		{
			const Chaos::Filter::FShapeFilterData& ShapeFilter = Shape->GetShapeFilterData();
			// Verify query is filtered out with default collision data on shape
			bool bFiltered = PrePreFilter(QueryFilterData, ShapeFilter);
		...
		// Query against particle on game thread, should fail to hit due to particle filter being defaulted, no touch/block set.
		{
			...
			FSimpleRaycastVisitor Visitor(Start, QueryFilterData, bQueryGameThread);
		...
		ShapeFilterBuilder.SetBlockChannelMask(TNumericLimits<uint64>::Max());
		ShapeFilterBuilder.SetOverlapChannelMask(TNumericLimits<uint64>::Max());
		ShapeFilterBuilder.SetFilterFlags(Chaos::EFilterFlags::All);
		const Chaos::Filter::FShapeFilterData NewParticleQueryFilter = ShapeFilterBuilder.Build();
		PRAGMA_ENABLE_INTERNAL_WARNINGS

		for (const TUniquePtr<Chaos::FPerShapeData>& Shape : Particle.ShapesArray())
		{
			// Update filter
			Shape->SetShapeFilterData(NewParticleQueryFilter);

			// Filter with new data, ensuring we pass and are not filtered out.
			bool bFiltered = PrePreFilter(QueryFilterData, NewParticleQueryFilter);
	...
	GTEST_TEST(EngineInterface, OverlapOffsetActor)
	{
		...
		SQ.Overlap(Chaos::FSphere(FVec3(0.0f, 0.0f, 100.0f), SphereRadius), QueryTM, HitBuffer, CommonParams);
	...
	GTEST_TEST(EngineInterface, SweepOffsetActor)
	{
		...
		FBlockAllCollisionQueryFilterCallback QueryCallback;
		ChaosInterface::FSceneQueryCommonParams CommonParams(QueryCallback);
		...
		SQ.Sweep(QueryBox, QueryTM, Dir, Length, HitBuffer, EHitFlags::None, CommonParams);
	...
	GTEST_TEST(EngineInterface, BodyWithTwoShapes_SweepWithInitialOverlap_MinimalTOIIsReturned)
	{
		...
		const FVec3 Dir(0,0,-1);
		const FReal Length = 200;
		SQ.Sweep(QuerySphere, QueryTM, Dir, Length, HitBuffer, EHitFlags::MTD, CommonParams);
