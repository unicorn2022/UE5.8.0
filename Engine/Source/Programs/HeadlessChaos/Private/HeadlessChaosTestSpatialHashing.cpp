...
namespace ChaosTest {
	...
	void SpatialHashing()
	{
		UE_LOGF(AHSP_Test, Verbose, "SpatialHashing");
		...
		for (int32 Index1 = 0; Index1 < Samples.Num(); Index1++)
		{
			for (int32 Index2 = 0; Index2 < Samples.Num(); Index2++)
			{
				if (Index1 != Index2 && Index1<Index2)
				{
					...
					if (Delta < 1.0)
					{
						UE_LOGF(AHSP_Test, Verbose, "... [%d](%3.5f,%3.5f,%3.5f)", Index1, Sample1.X, Sample1.Y, Sample1.Z);
						UE_LOGF(AHSP_Test, Verbose, "... [%d](%3.5f,%3.5f,%3.5f)", Index2, Sample2.X, Sample2.Y, Sample2.Z);
						UE_LOGF(AHSP_Test, Verbose, "... ... %3.5f", Delta);
						EXPECT_TRUE(false);
					}
				}
			}
