...
namespace ChaosTest {
	...
	GTEST_TEST(LargeScaleTests, TestSmallBoxLargeBoxGJKIntersection)
	{
		const FVec3 FloorMin(-250000.09155273438, -250000.01525878906, -50.000015258789062);
		const FVec3 FloorMax(250000.00000000000, 249999.98474121094, 2.8421709430404007e-14);
		const FVec3 BoxPos(6400.000000, 6400.000000, 95.304688);
		const FVec3 BoxMin(5040.0000000000000, 5040.0000000000000, -4548.6328119999998);
		const FVec3 BoxMax(7760.0000000000000, 7760.0000000000000, 4739.2421880000002);
		const FRigidTransform3 AT(FVec3::Zero(), FRotation3::FromIdentity());
		const FRigidTransform3 BT(FVec3::Zero(), FRotation3::FromIdentity());
		const FRigidTransform3 BTtoAT = BT.GetRelativeTransform(AT);
		FImplicitBox3 Box(BoxMin, BoxMax);
		FImplicitBox3 Floor(FloorMin, FloorMax);
		FVec3 InitialDir = -BoxPos;
		FReal Thickness = 0;
		const bool bResult = GJKIntersection(Floor, Box, BTtoAT, Thickness, InitialDir);
		EXPECT_TRUE(bResult);
	}
