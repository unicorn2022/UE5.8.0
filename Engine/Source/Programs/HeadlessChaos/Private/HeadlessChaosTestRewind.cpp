...
namespace ChaosTest {
		...
		TRewindHelper::TestDynamicSphere([](auto* Solver, FReal SimDt, int32 Optimization, auto Proxy, auto Sphere)
			{
				...
				{
					if (!bHasResimmed && Floor && Floor->GetPhysicsThreadAPI())
					{
						bHasResimmed = true;
						return 0;
					}
				...
				Helper->ProcessInputsFunc = [Proxy, RewindData = Solver->GetRewindData(), SimDt, &Floor, &bHasResimmed](int32 PhysicsStep, bool bIsResimming)
					{
						...
						if(Proxy)
						{ 
							if (FRigidBodyHandle_Internal* PT0 = Proxy->GetPhysicsThreadAPI())
							{ 
								if (!bHasResimmed || Time < 4.5)
								{
									//simply movement without hitting floor because it's spawned too late
									const float ExpectedLocation = FMath::Max(1.5, 14.5 - Time);
									EXPECT_NEAR(PT0->X()[2], ExpectedLocation, 1e-2);
								}
								else
								{
									//floor spawned earlier so we hit it
									EXPECT_GE(PT0->X()[2], 10 - 1e-2);
								}
							}
						}
				};
		...
		TRewindHelper::TestDynamicSphere([](auto* Solver, FReal SimDt, int32 Optimization, auto Proxy, auto Sphere)
			{
				...
				{
					if(!bHasResimmed && SpawnedProxy && SpawnedProxy->GetPhysicsThreadAPI())
					{
						bHasResimmed = true;
						return 0;
					}
				...
				Helper->ProcessInputsFunc = [RewindData = Solver->GetRewindData(), SimDt, &SpawnedProxy, &SpawnedProxyNoCorrection, &bHasResimmed](int32 PhysicsStep, bool bIsResimming)
				{
					...
					if(SpawnedProxy && RewindData)
					{ 
						if(FRigidBodyHandle_Internal* PT0 = SpawnedProxy->GetPhysicsThreadAPI())
						{ 
							if (bIsResimming && (PhysicsStep == 10))
							{
								RewindData->SpawnProxyIfNeeded(*SpawnedProxy);
								PT0->SetX(FVec3(500, 0, 100.0));
							}

							if (SpawnedProxyNoCorrection)
							{
								if (FRigidBodyHandle_Internal* PT1 = SpawnedProxyNoCorrection->GetPhysicsThreadAPI())  
								{
		...
		TRewindHelper::TestDynamicSphere([](auto* Solver, FReal SimDt, int32 Optimization, auto Proxy, auto Sphere)
			{
				...
				const int32 LastGameStep = 20;
				const int32 NumPhysSteps = FMath::TruncToInt(LastGameStep / SimDt) - 1;
				...
				if(!Solver->IsUsingAsyncResults())
				{
					...
					const int32 NumSteps = NumPhysSteps * 2 + (LastGameStep - NumPhysSteps);
					EXPECT_EQ(Count, NumSteps);	
				}
	...
	GTEST_TEST(AllTraits, RewindTest_ResimFallingObjectWithTeleport)
	{
		TRewindHelper::TestDynamicSphere([](auto* Solver, FReal SimDt, int32 Optimization, auto Proxy, auto Sphere)
		{
			...
			const int32 NumPhysSteps = FMath::TruncToInt(LastGameStep / SimDt) - 1;
			FRewindCallbackTestHelper* Helper = RegisterCallbackHelper(Solver, NumPhysSteps);
