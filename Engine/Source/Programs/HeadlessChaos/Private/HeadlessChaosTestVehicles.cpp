...
namespace ChaosTest
{
	...
	GTEST_TEST(AllTraits, VehicleTest_Aerodynamics)
	{
		...
		/*{
			...
			for (int i = 0; i < 200; i++)
			{
				...
				UE_LOGF(LogChaos, Warning, "Drag %f %f", DragA, DragB);
				UE_LOGF(LogChaos, Warning, "Vel %f %f", VelA, VelB);
	...
	GTEST_TEST(AllTraits, VehicleTest_EngineRPM)
	{
		...
		//	//UE_LOGF(LogChaos, Warning, "EngineSpeed %.2f rad/sec (%.1f RPM)", Engine.GetEngineSpeed(), Engine.GetEngineRPM());
		//}

		//EXPECT_GT(Engine.GetEngineRPM(), Engine.Setup().EngineIdleRPM);

		////UE_LOGF(LogChaos, Warning, "");
		////UE_LOGF(LogChaos, Warning, "No Throttle");
		...
		)
	{
		...
		if (bLoggingEnabled)
		{
			UE_LOGF(LogChaos, Warning, "--------------------START---------------------");
		}
		...
		while (SimulationTimeOut < MaxSimTime)
		{
			...
			if (bLoggingEnabled)
			{
				UE_LOGF(LogChaos, Warning, "Wheel.GetForceFromFriction() %ls", *Wheel.GetForceFromFriction().ToString());
			}
	...
	GTEST_TEST(AllTraits, VehicleTest_SuspensionSprungMassesFourWheels)
	{
...

		{
			...
			//for (int i = 0; i < 4; i++)
			//{
			//	UE_LOGF(LogChaos, Warning, "SM = %f", OutSprungMasses[i]);
			//}
	...
	GTEST_TEST(AllEvolutions, VehicleTest_SuspensionSpringLoad)
	{
		...
		//UE_LOGF(LogChaos, Warning, "SuspendedDisplacement %.1f cm", SuspendedDisplacement);
