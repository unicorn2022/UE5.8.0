...
class UEGTestPrinter : public ::testing::EmptyTestEventListener
{
    ...
    {
        UE_LOGF(LogHeadlessChaos, Verbose, "Test %ls.%ls Starting", *FString(TestInfo.test_case_name()), *FString(TestInfo.name()));
    }
    ...
    virtual void OnTestPartResult(const ::testing::TestPartResult& TestPartResult)
    {
        ...
        {
            UE_LOGF(LogHeadlessChaos, Error, "FAILED in %ls:%d\n%ls", *FString(TestPartResult.file_name()), TestPartResult.line_number(), *FString(TestPartResult.summary()))
        }
        else
        {
            UE_LOGF(LogHeadlessChaos, Verbose, "Succeeded in %ls:%d\n%ls", *FString(TestPartResult.file_name()), TestPartResult.line_number(), *FString(TestPartResult.summary()))
        }
    }

    virtual void OnTestEnd(const ::testing::TestInfo& TestInfo)
    {
        UE_LOGF(LogHeadlessChaos, Verbose, "Test %ls.%ls Ending", *FString(TestInfo.test_case_name()), *FString(TestInfo.name()));
    }
};
