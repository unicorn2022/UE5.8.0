...
namespace ChaosTest 
{
	...
	GTEST_TEST(JointSolverTests, TestJointSolver_KinematicDynamic_SoftPositionConstraint_ForceMode)
	{
		...
		while (It++ < NumIts)
		{
			...
			// Check for settling
			//UE_LOGF(LogChaos, Warning, "%d: %f %f %f: %f", It, OutDelta1.X, OutDelta1.Y, OutDelta1.Z, AverageOutDelta1Z);
	...
	void KinematicDynamic_SLerpDrive(FJointSolverTest& SolverTestA, FReal Mass, FReal Inertia, EJointForceMode ForceMode, FReal Stiffness, FReal Damping, FReal AngAcc, FReal Offset)
	{
		...
		while (It++ < NumIts)
		{
			...
			// Check for settling
			//UE_LOGF(LogChaos, Warning, "%d: %f deg : %f deg/s", It, FMath::RadiansToDegrees(OutAngles1.Y), FMath::RadiansToDegrees(SolverTestA.W1.Y));
