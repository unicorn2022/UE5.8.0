	...
	GTEST_TEST(AllEvolutions, DISABLED_PerfTests_Sim)
	{
		...
		for (int32 x = 0; x < GridSize; ++x)
		{
			for (int32 y = 0; y < GridSize; ++y)
			{
				...
				BoxDynamic->I() = TVec3<FRealSingle>(100000.f);
				BoxDynamic->InvI() = TVec3<FRealSingle>(1.f / 100000.f);
				...
				SphereDynamic->I() = TVec3<FRealSingle>(100000.f);
				SphereDynamic->InvI() = TVec3<FRealSingle>(1.f / 100000.f);
		...
		for (int Sample = 0; Sample < NumSamples; ++Sample)
		{
			...
			//UE_LOGF(LogChaos, Log, "[%d/%d] PerfTests_Sim (RealSize = %d)- Elapsed Time = %fs", Sample, NumSamples, sizeof(FReal), elapsed.count());
		...
		UE_LOGF(LogChaos, Log, "[SUMMARY] PerfTests_Sim (RealSize = %d) - Min Time = %fs | Avg Time = %fs | Max Time = %fs", sizeof(FReal), MinTime, AvgTime, MaxTime);
	}
}
