...
public class AutoRTFMTestsTarget : TargetRules
{
	...
	public AutoRTFMTestsTarget(TargetInfo Target) : base(Target)
	{
		...
		// Disable unity builds (stresses all the corners of the AutoRTFM runtime lookup code).
		bUseUnityBuild = false;

		// Server has ICU off so use that as the default.
		bCompileICU = false;
		...
		// CallstackTrace tests rely on this being enabled!
		if (Target.Configuration != UnrealTargetConfiguration.Shipping)
		{
			GlobalDefinitions.Add("UE_CALLSTACK_TRACE_ENABLED=1");
		}

		// Need to force the heartbeat on so we can test with it!
		GlobalDefinitions.Add("USE_HITCH_DETECTION=1");

		// Support unittests for LowLevelMemTracker
		// GlobalDefinitions.Add("LLM_ALLOWED_IN_APPLICATION_TYPE=1");
