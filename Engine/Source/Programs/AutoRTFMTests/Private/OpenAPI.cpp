...
TEST_CASE("OpenAPI.CommitScopedFromOpen")
{
...
TEST_CASE("OpenAPI.AbortTransaction.Lambda")
{
	auto TransactResult = AutoRTFM::Transact([&]()
	{
		AutoRTFM::Open([&] AUTORTFM_DISABLE
...
TEST_CASE("OpenAPI.AbortTransaction.AUTORTFM_OPEN")
{
	struct S
	{
		AUTORTFM_OPEN static void AbortTransaction()
		{
			AutoRTFM::AbortTransaction();
		}
	};
...
TEST_CASE("OpenAPI.AbortTransaction.AUTORTFM_OPEN_NO_VALIDATION")
{
	struct S
	{
		AUTORTFM_OPEN_NO_VALIDATION static void AbortTransaction()
		{
			AutoRTFM::AbortTransaction();
		}
	};
...
TEST_CASE("OpenAPI.AbortTransaction.UE_AUTORTFM_ALWAYS_OPEN")
{
	struct S
	{
		UE_AUTORTFM_ALWAYS_OPEN static void AbortTransaction()
		{
			AutoRTFM::AbortTransaction();
		}
	};
...
TEST_CASE("OpenAPI.AbortTransaction.UE_AUTORTFM_ALWAYS_OPEN_NO_MEMORY_VALIDATION")
{
	struct S
	{
		UE_AUTORTFM_ALWAYS_OPEN_NO_MEMORY_VALIDATION static void AbortTransaction()
		{
			AutoRTFM::AbortTransaction();
		}
	};
...
TEST_CASE("OpenAPI.Commit_TransactOpenCloseRollback")
{
	...
	AutoRTFM::Testing::Commit([&]()
	{
		...
		bExecutedAfterOpen = true;
	});

	REQUIRE(Value == 11);
	REQUIRE(!AutoRTFM::IsTransactional());
	REQUIRE(bExecutedAfterOpen);
}
...
TEST_CASE("OpenAPI.Open(StartTransaction, StackWrite, AbortTransaction)")
{
	int Value = 0;
	bool bRanPostOpen = false;
	AutoRTFM::Testing::Commit([&]()
	{
		AutoRTFM::Open([&] AUTORTFM_DISABLE
		{
	...
	REQUIRE(Value == 0);
	REQUIRE(bRanPostOpen);
}

TEST_CASE("OpenAPI.Open(StackWrite, AbortTransaction)")
{
	int Value = 0;
	bool bRanPostOpen = false;
	AutoRTFM::Testing::Abort([&]()
...
TEST_CASE("OpenAPI.Footgun1")
{
	...
	AutoRTFM::Transact([&]()
	{
		...
		REQUIRE(AutoRTFM::ETransactionStatus::AbortedByRequest == Status);
	});
...
TEST_CASE("OpenAPI.TransOpenStartCloseAbortAbort")
{
	...
	AutoRTFM::Transact([&]() 
	{
		AutoRTFM::Open([&] AUTORTFM_DISABLE
		{
			...
			Value = 42;
			AutoRTFM::ETransactionStatus CloseStatus = AutoRTFM::Close([&]
			{
				Value = 420;
				AutoRTFM::AbortTransaction();
				bGetsToA = true;
			...
			REQUIRE(CloseStatus == AutoRTFM::ETransactionStatus::AbortedByRequest);
...
TEST_CASE("OpenAPI.CheckRaceAgainstOtherThread")
{
	...
	AutoRTFM::ETransactionResult Result = AutoRTFM::Transact([&]
	{
		AutoRTFM::Open([&]
		{
			std::thread([&] AUTORTFM_ENABLE // SOL-8741
			{
