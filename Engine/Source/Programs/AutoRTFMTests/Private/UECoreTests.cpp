...
{
	struct AUTORTFM_ENABLE MyStruct : TThreadSingleton<MyStruct>
	{
		int I;
		float F;
	};
...
TEST_CASE("UECore.FUObjectItem")
{
	...
	{
		// This test case performs a mixed open / closed write in Item.CreateStatID()
		AUTORTFM_SCOPED_SANITIZER_MODE_DISABLE();
...
TEST_CASE("UECore.FString")
{
	SECTION("Printf")
	{
		...
		// FString::operator== uses case insensitive comparison (Stricmp).
...
TEST_CASE("UECore.UE_LOG_CONTEXT")
{
	SECTION("Commit")
	{
		AutoRTFM::Testing::Commit([]
		{
			UE_LOG_CONTEXT("Test", "Commit");
		});
	}
	SECTION("Abort")
	{
...
TEST_CASE("UECore.CoreRedirects")
{
	...
	// See FORT-823809
	auto Redirects = [From, To] AUTORTFM_ENABLE
	{
...
TEST_CASE("FDynamicallyTypedValue")
{
	...
	struct AUTORTFM_ENABLE FType : UE::FDynamicallyTypedValueType
	{
		...
		void InitializeValueFrom(void* DestData, const void* SourceData, const UE::Verse::FRuntimeType& SourceType) const override
		{
		...
		void ForEachSubobject(const void* Data, TFunctionRef<void(const UObject*)> ObjectFunc) const {};
	} Type;

	auto GetValue = [] AUTORTFM_ENABLE (UE::FDynamicallyTypedValue& Value)
	{
...
TEST_CASE("UECore.UnifiedError")
{
	SECTION("Commit")
	{
		AutoRTFM::Testing::Commit([&]
		{
			UE::UnifiedError::FError Err = UE::Core::CancellationError();
...
TEST_CASE("UECore.ArchiveObjectCrc32")
{
	SECTION("Constructed in a transaction")
	{
		AutoRTFM::Testing::Commit([&]
		{
			FArchiveObjectCrc32 Archive;
			Archive.Crc32(NewObject<UMyAutoRTFMTestObject>());
		});
	}

	SECTION("Constructed outwith a transaction")
	{
		FArchiveObjectCrc32 Archive;
...
TEST_CASE("UECore.FConcurrentLinearAllocator")
{
	SECTION("Malloc")
	{
		int32* Allocation = nullptr;

		AutoRTFM::Testing::Commit([&]
		{
			Allocation = static_cast<int32*>(FConcurrentLinearAllocator::Malloc(sizeof(int32), alignof(int32)));
			*Allocation = 42;
		});

		REQUIRE(Allocation);
		REQUIRE(*Allocation == 42);

		FConcurrentLinearAllocator::Free(Allocation);
	}

	SECTION("Free")
	{
		int32* Allocation = static_cast<int32*>(FConcurrentLinearAllocator::Malloc(sizeof(int32), alignof(int32)));
		REQUIRE(Allocation);
	...
	SECTION("Realloc")
	{
		...
		AutoRTFM::Testing::Commit([&]
		{
			Allocation = static_cast<int32*>(FConcurrentLinearAllocator::Realloc(Allocation, 2 * sizeof(int32), alignof(int32)));
			Allocation[1] = 13;
		});

		REQUIRE(Allocation);
		REQUIRE(Allocation[0] == 42);
		REQUIRE(Allocation[1] == 13);
		FConcurrentLinearAllocator::Free(Allocation);
	}
	...
	SECTION("GetAllocationSize")
	{
		...
		const size_t AllocationSize = FConcurrentLinearAllocator::GetAllocationSize(Allocation);

		AutoRTFM::Testing::Commit([&]
		{
			REQUIRE(AllocationSize == FConcurrentLinearAllocator::GetAllocationSize(Allocation));
		});
...
TEST_CASE("UECore.FConcurrentLinearBulkObjectAllocator")
{
	SECTION("Malloc")
	{
		...
		AutoRTFM::Testing::Commit([&]
		{
			Allocation = static_cast<int32*>(Allocator.Malloc(sizeof(int32), alignof(int32)));
			*Allocation = 42;
		});
...
TEST_CASE("UECore.TextTransformer")
{
	SECTION("ToLower")
	{
		FString String(TEXT("WOWWEE"));

		AutoRTFM::Testing::Commit([&]
		{
			REQUIRE(FTextTransformer::ToLower(String).Equals(TEXT("wowwee")));
		});
	}
	...
	SECTION("ToUpper")
	{
		...
		AutoRTFM::Testing::Commit([&]
		{
			REQUIRE(FTextTransformer::ToUpper(String).Equals(TEXT("WOWWEE")));
		});
	}
}

TEST_CASE("UECore.FSharedString")
{
	SECTION("Create In Transaction")
	{
		AutoRTFM::Testing::Commit([&]
		{
			UE::FSharedString Str(TEXT("WOWWEE"));
		});
	}

	SECTION("Create From StringView In Transaction")
	{
		TStringView<UE::FSharedString::ElementType> View(TEXT("WOWWEE"));

		AutoRTFM::Testing::Commit([&]
		{
			UE::FSharedString Str(View);
		});
	}

	SECTION("Copy Into Transaction")
	{
		UE::FSharedString Str(TEXT("WOWWEE"));

		AutoRTFM::Testing::Commit([&]
		{
			UE::FSharedString Copy(Str);
		});
	}

	SECTION("Assign In Transaction")
	{
		UE::FSharedString Str(TEXT("WOWWEE"));
		UE::FSharedString Other;
	...
	SECTION("Assign Over Existing Data Transaction")
	{
		UE::FSharedString Str(TEXT("WOWWEE"));
		UE::FSharedString Other(TEXT("NARP"));
	...
	SECTION("Assign By Move Over Existing Data Transaction")
	{
		UE::FSharedString Other(TEXT("NARP"));
		...
		UE::FSharedString Compare(TEXT("WOWWEE"));

		REQUIRE(Compare == Other);
	}
}

TEST_CASE("UECore.LowLevelMemTracker.Scopes")
{
	FName StartScopeName(TEXTVIEW("AutoRTFMTesting/StartScope"));
	FName ApplyScopeName(TEXTVIEW("AutoRTFMTesting/ApplyScope"));
	int64 StartSize = 0;

	struct AUTORTFM_INFER FDeleterCallFree
	{
	...
	auto FetchStartSize = [&StartSize, ApplyScopeName]()
		{
			StartSize = 0;
			...
			if (FLowLevelMemTracker::IsEnabled())
			{
				...
				LLM.Tick();
				StartSize = LLM.GetTagAmountForTracker(ELLMTracker::Default, ApplyScopeName, ELLMTagSet::None);
			}
#endif
		};
	auto RequireEndSize = [&StartSize, ApplyScopeName](int64 DeltaSize)
		{
#if ENABLE_LOW_LEVEL_MEM_TRACKER
			if (FLowLevelMemTracker::IsEnabled())
			{
				FLowLevelMemTracker& LLM = FLowLevelMemTracker::Get();
				LLM.Tick();
				int64 EndSize = LLM.GetTagAmountForTracker(ELLMTracker::Default, ApplyScopeName, ELLMTagSet::None);
				REQUIRE(EndSize == StartSize + DeltaSize);
			}
#endif
		};

	// Set the default scope that should be used when allocating outside of LLM_SCOPE et al declared in the tests.
	LLM_SCOPE(StartScopeName);

	SECTION("LLM_SCOPE")
	{
		SECTION("Commit")
		{
			TUniquePtr<int32, FDeleterCallFree> Allocation;
			TUniquePtr<int32, FDeleterCallFree> Allocation2;
			int64 DeltaSize = sizeof(int32);
			FetchStartSize();

			AutoRTFM::Testing::Commit([&]
				{
					LLM_SCOPE(ApplyScopeName);
					Allocation.Reset(static_cast<int32*>(FMemory::Malloc(DeltaSize, alignof(int32))));
				});
			REQUIRE(Allocation);

			RequireEndSize(DeltaSize);
			// ApplyScope should no longer be applied and should not record this second DeltaSize.
			Allocation2.Reset(static_cast<int32*>(FMemory::Malloc(DeltaSize, alignof(int32))));
			RequireEndSize(DeltaSize);
		}
		SECTION("Abort")
		{
			...
			AutoRTFM::Testing::Abort([&]
				{
					LLM_SCOPE(ApplyScopeName);
					int32* Allocation = static_cast<int32*>(FMemory::Malloc(DeltaSize, alignof(int32)));
					AutoRTFM::AbortTransaction();
				});

			// Abort calls Free and undoes the allocation, so DeltaSize should be 0.
			RequireEndSize(0);
			// ApplyScope should no longer be applied and should not record this second DeltaSize.
			Allocation2.Reset(static_cast<int32*>(FMemory::Malloc(DeltaSize, alignof(int32))));
			RequireEndSize(0);
	...
	SECTION("LLM_SCOPE_DYNAMIC")
	{
		SECTION("Commit")
		{
			...
			AutoRTFM::Testing::Commit([&]
				{
					FString LLMStatNameString(TEXT("AutoRTFMTesting"));
					LLM_SCOPE_DYNAMIC(ApplyScopeName, ELLMTracker::Default, ELLMTagSet::None, FLLMDynamicTagConstructorStatString(LLMStatNameString));
					Allocation.Reset(static_cast<int32*>(FMemory::Malloc(DeltaSize, alignof(int32))));
				});
			REQUIRE(Allocation);
	...
	SECTION("LLM_SCOPED_PAUSE_TRACKING")
	{
		SECTION("Commit")
		{
			int64 DeltaSize = sizeof(int32);
			...
			// Activate the Apply scope so we can verify that PAUSE_TRACKING blocks/unblocks it at the proper times.
			LLM_SCOPE(ApplyScopeName);
			AutoRTFM::Testing::Commit([&]
				{
			...
			// PauseTracking means that allocations are not recorded.
			RequireEndSize(0);
			Allocation.Reset();
			// Allocations made during PauseTracking are not recorded when they free.
			RequireEndSize(0);

			// PauseScope should no longer be applied and LLM should record this second DeltaSize.
			Allocation2.Reset(static_cast<int32*>(FMemory::Malloc(DeltaSize, alignof(int32))));
			RequireEndSize(DeltaSize);
		}
		...
		SECTION("Abort")
		{
			...
			// Abort calls Free and undoes the allocation. Also Pause prevented it from being reported.
			RequireEndSize(0);
	...
	SECTION("LLM_REALLOC_SCOPE")
	{
		SECTION("Commit")
		{
			int64 DeltaSize = 2 * sizeof(int32);
			TUniquePtr<int32, FDeleterCallFree> Allocation;
			TUniquePtr<int32, FDeleterCallFree> Allocation2;
			FetchStartSize();
			...
			AutoRTFM::Testing::Commit([&]
				{
					{
						LLM_SCOPE(ApplyScopeName);
						Allocation.Reset(static_cast<int32*>(FMemory::Malloc(sizeof(int32), alignof(int32))));
					}

					int32* OldAllocation = Allocation.Release();
					LLM_REALLOC_SCOPE(OldAllocation);
					Allocation.Reset(static_cast<int32*>(FMemory::Realloc(OldAllocation, DeltaSize, alignof(int32))));
				});
			REQUIRE(Allocation);

			// The second allocation should be the only one recorded since we called Realloc
			RequireEndSize(DeltaSize);
			// ReallocScope should no longer be applied and should not record this second DeltaSize.
			Allocation2.Reset(static_cast<int32*>(FMemory::Malloc(DeltaSize, alignof(int32))));
			RequireEndSize(DeltaSize);
		}
		SECTION("Abort")
			...
			AutoRTFM::Testing::Abort([&]
				{
					...
					{
						LLM_SCOPE(ApplyScopeName);
						Allocation = static_cast<int32*>(FMemory::Malloc(sizeof(int32), alignof(int32)));
					}

					int32* OldAllocation = Allocation;
					LLM_REALLOC_SCOPE(OldAllocation);
					Allocation = static_cast<int32*>(FMemory::Realloc(OldAllocation, DeltaSize, alignof(int32)));
					AutoRTFM::AbortTransaction();
				});
