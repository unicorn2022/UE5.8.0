...
TEST_CASE("SharedPointer.ReturnFromOpen")
{
	TSharedPtr<int> Shared = AutoRTFM::Open([]
	{
		return TSharedPtr<int>{new int(42)};
	});

	REQUIRE(*Shared == 42);
}

TEST_CASE("SharedReference.ReturnFromOpen")
{
	TSharedRef<int> Shared = AutoRTFM::Open([]
	{
		return MakeShared<int>(42);
	});

	REQUIRE(*Shared == 42);
}

TEST_CASE("WeakPointer.ReturnFromOpen")
{
	TSharedPtr<int> Shared = TSharedPtr<int>{new int(42)};
	TWeakPtr<int> Weak = AutoRTFM::Open([&]
	{
		return TWeakPtr<int>{Shared};
	});
