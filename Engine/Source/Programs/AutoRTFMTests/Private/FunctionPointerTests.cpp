...
TEST_CASE("FunctionPointer.TFunction")
{
	...
	{
		int Result = 0;
		TFunction<void()> MyFunc = [&Result] AUTORTFM_ENABLE
			{
				Result = 42;
			};
...
TEST_CASE("FunctionPointer.TUniqueFunction")
{
	SECTION("Created inside transaction")
	{
		...
		AutoRTFM::Testing::Commit([&]
			{
				TUniqueFunction<void()> MyFunc = [&Result]
					{
						Result = 42;
					};
	...
	{
		int Result = 0;
		TUniqueFunction<void()> MyFunc = [&Result] AUTORTFM_ENABLE
			{
				Result = 42;
			};
