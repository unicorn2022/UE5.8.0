...
AutoRTFM::ETransactionResult FakeVM(const FOpcode* Opcodes, const int Length, int& Value)
{
	...
	// locations, which will cause AutoRTFM sanitizer errors.
	AUTORTFM_SCOPED_SANITIZER_MODE_DISABLE();

	return AutoRTFM::Transact([Opcodes, Length, &Value]
	{
		AutoRTFM::Open([Opcodes, Length, &Value] AUTORTFM_DISABLE
		{
			...
			for (int i = 0; i < Length; i++)
			{
				...
				case EOpcodeType::Call:
				{
					...
					const AutoRTFM::ETransactionStatus Status = AutoRTFM::Close([&Value, Arg, Call]
						{
							Call(&Value, Arg);
						});

					if (AutoRTFM::HasAborted(Status))
					{
