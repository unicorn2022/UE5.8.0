...
TEST_CASE("StatsTests")
{
	SECTION("Enable AutoRTFM Stats Gathering")
	{
		...
		UE::Stats::DirectStatsCommand(TEXT("stat AutoRTFM_Tests"), /*bBlockForCompletion=*/true);
	...
	SECTION("Stats FScopedCycleCounter Outside Transaction")
	{
		SCOPE_CYCLE_COUNTER(STAT_StandardCycleCounter);
	}

	SECTION("Stats FScopedCycleCounter Inside Transaction")
	{
	...
	SECTION("Stats Multi-FScopedCycleCounter Re-Entrant w/ Transaction")
	{
		auto ChainCall = [] AUTORTFM_ENABLE(TFunctionRef<void()> ChainedFunc)
		{
			SCOPE_CYCLE_COUNTER(STAT_StandardCycleCounter);
			ChainedFunc();
		};
	...
	SECTION("Stats FScopedCycleCounter w/ Manual Counter")
	{
		auto ChainCall = [] AUTORTFM_ENABLE(TFunctionRef<void()> ChainedFunc)
		{
			SCOPE_CYCLE_COUNTER(STAT_StandardCycleCounter);
