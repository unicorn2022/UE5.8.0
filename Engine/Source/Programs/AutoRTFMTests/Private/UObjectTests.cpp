...
{
	struct AUTORTFM_ENABLE FTestAnnotation
	{
		FTestAnnotation()
...
TEST_CASE("UObject.AtomicallySetFlags")
{
	...
	constexpr EObjectFlags FlagsToAdd = EObjectFlags::RF_Transient | EObjectFlags::RF_NonPIEDuplicateTransient;
...
TEST_CASE("UObject.AtomicallyClearFlags")
{
	...
	constexpr EObjectFlags OldFlags = EObjectFlags::RF_Public | EObjectFlags::RF_Transient;
	constexpr EObjectFlags FlagsToClear = EObjectFlags::RF_Transient | EObjectFlags::RF_NonPIEDuplicateTransient;
...
TEST_CASE("FGCObject")
{
	struct AUTORTFM_ENABLE FMyGCObject : FGCObject
	{
		bool bAlive = true;
	...
	auto Test = [&] AUTORTFM_ENABLE (AUTORTFM_IMPLICIT_ENABLE auto&& Callback)
	{
...
TEST_CASE("UObject.FUniversallyUniqueObjectNameGenerator")
{
	SECTION("Constructed outside of transaction")
	{
		FGuid Seed;
		FUniversallyUniqueObjectNameGenerator Generator(Seed);

		const uint32 Value = Generator.GetNewValue();

		AutoRTFM::Testing::Commit([&]
		{
			REQUIRE(Generator.GetNewValue() != Value);
		});
	}

	SECTION("Constructed inside of transaction")
	{
		AutoRTFM::Testing::Commit([&]
		{
			FGuid Seed;
			FUniversallyUniqueObjectNameGenerator Generator(Seed);
			uint32 Value = Generator.GetNewValue();

			// Just check that we are definitely generating the pseudo-random values even in transactions.
			REQUIRE(Value != Generator.GetNewValue());
		});
	}

	SECTION("Seed is not reset by aborting transaction")
	{
		FGuid Seed;
		FUniversallyUniqueObjectNameGenerator Generator(Seed);
		...
		AutoRTFM::Testing::Abort([&]
		{
			const uint32 TransactionalValue = Generator.GetNewValue();

			AutoRTFM::OnAbort([&Value, TransactionalValue]
			{
