...
TEST_CASE("Delegate.Broadcast")
{
	...
	int CallCount = 0;
	Delegate.AddLambda([&CallCount] AUTORTFM_OPEN
	{ 
		CallCount++;
	});
	...
	{
		Delegate.AddStatic([] AUTORTFM_ENABLE { AutoRTFM::AbortTransaction(); });
		AutoRTFM::ETransactionResult Result = AutoRTFM::Transact([&]
		{
			Delegate.Broadcast();
		});
...
TEST_CASE("Delegate.Recursive")
{
	SECTION("Open <-> Close")
	{
		...
		Delegate.AddLambda([&] AUTORTFM_ENABLE (int X, int& R)
		{
	...
	SECTION("Open <-> Close With Abort")
	{
		...
		Delegate.AddLambda([&] AUTORTFM_ENABLE (int X, int& R, bool& bAborting)
		{
			REQUIRE(AutoRTFM::IsTransactional());
			...
			if (X <= 1)
			{
				...
				bAborting = AutoRTFM::HasAborted(Status);
			}
...
TEST_CASE("Delegate.CauseCompaction")
{
	...
	SECTION("Transact")
	{
		...
		Delegate.AddLambda([&] AUTORTFM_ENABLE
		{
			if (!bHitOnce)
			{
