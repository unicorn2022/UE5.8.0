...
TEST_CASE("Allocators.MemoryStats.GetTotalBytesAllocated")
{
	size_t StartingSize = AutoRTFM::FMemoryStats::GetTotalBytesAllocated();

	void* const Allocate1MBytes = AutoRTFM::FAllocator::Allocate(1'000'000, /*Alignment*/ 1);
	size_t CurrentSize = AutoRTFM::FMemoryStats::GetTotalBytesAllocated();
	REQUIRE(StartingSize + 1'000'000 == CurrentSize);

	void* const Allocate49MBytes = AutoRTFM::FAllocator::AllocateZeroed(49'000'000, /*Alignment*/ 1);
	CurrentSize = AutoRTFM::FMemoryStats::GetTotalBytesAllocated();
	REQUIRE(StartingSize + 50'000'000 == CurrentSize);

	AutoRTFM::FAllocator::Free(Allocate1MBytes, 1'000'000);
	CurrentSize = AutoRTFM::FMemoryStats::GetTotalBytesAllocated();
	REQUIRE(StartingSize + 49'000'000 == CurrentSize);

	void* const Allocate7MBytes = AutoRTFM::FAllocator::Reallocate(Allocate49MBytes, 7'000'000, /*Alignment*/ 1, /*PreviousSize*/ 49'000'000);
	CurrentSize = AutoRTFM::FMemoryStats::GetTotalBytesAllocated();
	REQUIRE(StartingSize + 7'000'000 == CurrentSize);

	AutoRTFM::FAllocator::Free(Allocate7MBytes, 7'000'000);
	CurrentSize = AutoRTFM::FMemoryStats::GetTotalBytesAllocated();
	REQUIRE(CurrentSize == StartingSize);
}
