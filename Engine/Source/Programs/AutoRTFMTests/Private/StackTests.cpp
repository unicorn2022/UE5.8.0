...
TEMPLATE_TEST_CASE("Stack", "", TrivialStack, NonTrivialStack)
{
	...
	auto Check = [&] AUTORTFM_DISABLE (StackType& Stack, std::vector<ElementType> Expected)
	{
	...
	SECTION("Copy Assign")
	{
		StackType Source;
		StackType Target;
		auto CheckCopy = [&] AUTORTFM_DISABLE
		{
	...
	SECTION("Move Assign")
	{
		StackType Source;
		StackType Target;
		auto CheckMove = [&] AUTORTFM_DISABLE
		{
			SECTION("Source Empty")
			{
	...
	SECTION("Soak")
	{
		...
		{
			unsigned Mod100 = (I * 15485863) % 100;
			switch (Mod100)
			{
				case 0:
				{
