...
void FSBLHelperService::Start(uint16 Port)
{
	...
	if (SocketListener->IsActive())
	{
		...
		UE_LOGF(LogSwitchboardListenerHelper, Display, "Started listening on %ls:%d", *SocketListener->GetLocalEndpoint().Address.ToString(), SocketListener->GetLocalEndpoint().Port);
	}
}

bool FSBLHelperService::OnIncomingConnection(FSocket* InSocket, const FIPv4Endpoint& InEndpoint)
{
	UE_LOGF(LogSwitchboardListenerHelper, Display, "Incoming connection via %ls:%d", *InEndpoint.Address.ToString(), InEndpoint.Port);
...
bool FSBLHelperService::ParseIncomingMessage(const FString& Message, const FIPv4Endpoint& InEndpoint)
{
	...
	{
		UE_LOGF(LogSwitchboardListenerHelper, Error, "Unsupported command '%ls'", *Command);
	}
...
bool FSBLHelperService::HandleCmdLock(const TSharedPtr<FJsonObject>& Json)
{
	...
	if (!PidField.IsValid())
	{
		UE_LOGF(LogSwitchboardListenerHelper, Error, "Could not get valid pid from lock command");
		return false;
	}
	...
	{
		UE_LOGF(LogSwitchboardListenerHelper, Warning, "Received Gpu clock lock request for Pid %u, but could not open a handle to it", Pid);
		return false;
	}

	UE_LOGF(LogSwitchboardListenerHelper, Display, "Received Gpu clock lock request for Pid %u", Pid);
...
bool FSBLHelperService::SendMessage(const FString& InMessage, const FIPv4Endpoint& InEndpoint)
{
	...
	if (!Connections.Contains(InEndpoint))
	{
		...
		UE_LOGF(LogSwitchboardListenerHelper, Verbose, "Trying to send message to disconnected client %ls", *InEndpoint.ToString());
	...
	UE_LOGF(LogSwitchboardListenerHelper, Verbose, "Sending message %ls", *InMessage);
...
void FSBLHelperService::HandlePendingDisconnections()
{
	...
	for (const TPair<FIPv4Endpoint, double>& LastActivity : LastActivityTime)
	{
		...
		{
			UE_LOGF(LogSwitchboardListenerHelper, Display, "Client %ls has been inactive for more than %.1fs", 
				*Client.ToString(), 
...
void FSBLHelperService::DisconnectClient(const FIPv4Endpoint& InClientEndpoint)
{
	...
	UE_LOGF(LogSwitchboardListenerHelper, Display, "Disconnected from Client %ls", *Client);
...
void FSBLHelperService::ParseIncomingData()
{
	for (const TPair<FIPv4Endpoint, TSharedPtr<FSocket>>& Connection : Connections)
	{
		...
		while (ClientSocket->HasPendingData(PendingDataSize))
		{
			...
			{
				UE_LOGF(LogSwitchboardListenerHelper, Error, "Error while receiving data via endpoint %ls", *ClientEndpoint.ToString());
				continue;
			}
...
void FSBLHelperService::ManageGpuClocks()
{
	if (NumProcessesHoldingGpuClocks)
	{
		if (!bGpuClocksLocked)
		{
			UE_LOGF(LogSwitchboardListenerHelper, Display, "Locking Gpu Clocks");
	...
	else
	{
		if (bGpuClocksLocked)
		{
			UE_LOGF(LogSwitchboardListenerHelper, Display, "Unlocking Gpu Clocks");
