...
namespace UninstallHelper
{
	...
	int32 ExecProcess(const FExecActionInfo& Action)
	{
		...
		if (Result)
		{
			UE_LOGF(LogUninstallHelper, Error, "Action %ls %ls failed with exit code %d", *Action.URL, *Action.Params, Result);
		}
		return Result;
	}
	...
	EReturnCode Execute(const FUninstallOptions& Options)
	{
		...
		if (Options.bDeletePersistentDownloadDir)
		{
			...
			{
				UE_LOGF(LogUninstallHelper, Log, "Deleting the PersistentDownloadDir located at %ls", *PersistentDownloadDir);
		...
		if (Options.bDeleteConfig)
		{
			...
			{
				UE_LOGF(LogUninstallHelper, Log, "Deleting the Config directory located at %ls", *ConfigDir);
		...
		for (const FString& Artifact : Options.AdditionalDeleteArtifacts)
		{
			...
			{
				UE_LOGF(LogUninstallHelper, Log, "Deleting directory %ls", *CleanArtifactPath);
			...
			{
				UE_LOGF(LogUninstallHelper, Log, "Deleting file %ls", *CleanArtifactPath);
			...
			{
				UE_LOGF(LogUninstallHelper, Log, "Unable to find artifact '%ls' on the disk, skipping.", *CleanArtifactPath);
			}
		}
		...
		{
			UE_LOGF(LogUninstallHelper, Log, "Executing process %ls %ls...", *Action.URL, *Action.Params);
			if (Options.bShouldExecute && (LastProcessResult = ExecProcess(Action)) != 0)
			{
				UE_LOGF(LogUninstallHelper, Log, "Process %ls %ls failed with exit code %d", *Action.URL, *Action.Params, LastProcessResult);
		...
		{
			UE_LOGF(LogUninstallHelper, Error, "UninstallHelper exited with code %d", static_cast<int32>(Result));
		}
...
INT32_MAIN_INT32_ARGC_TCHAR_ARGV()
{
	...
	{
		UE_LOGF(LogUninstallHelper, Error, "Failed to initialize command line. Exiting.");
	...
	{
		UE_LOGF(LogUninstallHelper, Error, "An app name must be provided in order for this tool to run. Exiting.");
	...
	{
		UE_LOGF(LogUninstallHelper, Error, "InstallDir must be provided in order for this tool to run. Exiting.");
	...
	{
		UE_LOGF(LogUninstallHelper, Error, "InstallDir %ls does not exist. Exiting.", *Options.InstallDir);
		return static_cast<int32>(UninstallHelper::EReturnCode::InvalidInstallDir);
	}
	...
	if (FParse::Value(*CmdLine, TEXT("UninstallArtifactsManifest="), UninstallArtifactsManifestFilename))
	{
		...
		{
			UE_LOGF(LogUninstallHelper, Warning, "Failed to find uninstall artifacts manifest file '%ls', skipping.", *ManifestPath);
		}
	...
	if (FParse::Value(*CmdLine, TEXT("UninstallActionsManifest="), UninstallActionsManifestFilename))
	{
		...
		{
			UE_LOGF(LogUninstallHelper, Warning, "Failed to find uninstall actions manifest file '%ls', skipping.", *ManifestPath);
		}
	}
	...
	{
		UE_LOGF(LogUninstallHelper, Error, "UninstallHelper failed to execute with exit code %d", static_cast<int32>(Result));
	}
