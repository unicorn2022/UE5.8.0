...
void FTelemetryService::Init(const FTelemetryParameters& InTelemetryParameters, const FString& InStreamName)
{
	if (!InTelemetryParameters.RegexStreamAllowlist.IsEmpty())
	{
		FRegexPattern Pattern = FRegexPattern(InTelemetryParameters.RegexStreamAllowlist, ERegexPatternFlags::CaseInsensitive);
		FRegexMatcher regex = FRegexMatcher(Pattern, InStreamName);
...
void FTelemetryService::Set(TSharedPtr<ITelemetry> InInstance)
{
	...
	{
		UE_LOGF(LogSubmitTool, Warning, "Trying to set a null telemetry instance. Operation aborted.");
		return;
	}
