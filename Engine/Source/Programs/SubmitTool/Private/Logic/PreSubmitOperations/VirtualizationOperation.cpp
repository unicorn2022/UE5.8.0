...
REGISTER_VALIDATOR_TYPE(SubmitToolParseConstants::VirtualizationToolOp, FVirtualizationOperation)
...
bool FVirtualizationOperation::Validate(const FString& InCLDescription, const TArray<FSCFileRef>& InFilteredFilesInCL, const TArray<const FTag*>& InTags)
{
...
bool FVirtualizationOperation::StartBuildingTool()
{
	...
	UE_LOGF(LogValidators, Log, "[%ls] Building Virtualization Tool", *ValidatorName);
	UE_LOGF(LogValidatorsResult, Log, "[%ls] Building Virtualization Tool", *ValidatorName);
...
void FVirtualizationOperation::Tick(float InDeltatime)
{
	...
	if (BuildProcessHandle.IsValid())
	{
		...
		else
		{
			...
			if (CompileResult == 0)
			{
				UE_LOGF(LogValidators, Log, "[%ls] Virtualization Tool built successfully - %.1f(s)", *ValidatorName, TotalCompileTime);
				UE_LOGF(LogValidatorsResult, Log, "[%ls] Virtualization built successfully - %.1f(s)", *ValidatorName, TotalCompileTime);
...
void FVirtualizationOperation::ProcessOutput(const FString& InOutput)
{
	...
	for (const FString& Line : Lines)
	{
		if (    Line.Contains(" error ")
			...
			&& !Line.Contains(" Log: "))
		{
			UE_LOGF(LogValidators, Warning, "[%ls]: %ls", *ValidatorName, *Line);
		}
		else
		{
			UE_LOGF(LogValidators, Log, "[%ls]: %ls", *ValidatorName, *Line);
		}
	}
}
