...
REGISTER_VALIDATOR_TYPE(SubmitToolParseConstants::UBTValidator, FUBTValidator)
...
bool FUBTValidator::Validate(const FString& InCLDescription, const TArray<FSCFileRef>& InFilteredFilesInCL, const TArray<const FTag*>& InTags)
{
	...
	if(TypedDefinition->Platforms.Num() != 0)
	{
		...
		{
			UE_LOGF(LogValidators, Error, "[%ls] Selected option %ls is not contained in the Platforms:\n%ls", *GetValidatorName(), *SelectedOption, *FString::Join(TypedDefinition->Platforms, TEXT("\n")));
			return false;
		}
	...
	if(TypedDefinition->Configurations.Num() != 0)
	{
		...
		{
			UE_LOGF(LogValidators, Error, "[%ls] Selected option %ls is not contained in the Configurations:\n%ls", *GetValidatorName(), *SelectedOption, *FString::Join(TypedDefinition->Configurations, TEXT("\n")));
			return false;
		}
	...
	if(TypedDefinition->Targets.Num() != 0)
	{
		...
		{
			UE_LOGF(LogValidators, Error, "[%ls] Selected option %ls is not contained in the Targets:\n%ls", *GetValidatorName(), *SelectedOption, *FString::Join(TypedDefinition->Targets, TEXT("\n")));
			return false;
		}
	...
	if(TypedDefinition->bUseStaticAnalyser)
	{
		...
		if(TypedDefinition->StaticAnalysers.Num() > 0)
		{
			...
			{
				UE_LOGF(LogValidators, Error, "[%ls] Selected option %ls is not contained in the StaticAnalysers:\n%ls", *GetValidatorName(), *SelectedOption, *FString::Join(TypedDefinition->StaticAnalysers, TEXT("\n")));
				return false;
			}
	...
	if(!ProgramFiles.IsEmpty())
	{
		for(const TPair<FString, TArray<FString>>& ProgramFileList : ProgramFiles)
		{
			...
			if (ProgramProjectFiles.Num() > 0)
			{
				...
				{
					UE_LOGF(LogSubmitTool, Warning, "More than one uprojects found for Program %ls, using the first one of:\n%ls", *ProgramFileList.Key, *FString::Join(ProgramProjectFiles, TEXT("\n")));
				}
			...
			UE_LOGF(LogValidators, Log, "[%ls] Using Target: %ls", *GetValidatorName(), *TargetStr);
	...
	return QueueProcess(GetProcessName(AbsoluteExecutablePath), AbsoluteExecutablePath, FinalArgs);
}
