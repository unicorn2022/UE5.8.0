...
UENUM()
enum class EP4ClientUserFlags : uint64
{
	None = 0,
...
public:
	...
	FSTClientUser(FSCCommand& InCommand, EP4ClientUserFlags InFlags)
		: ClientUser()
		, Flags(InFlags)
		, Command(InCommand)
	{
	...
	/** Called by P4API on "change -i" command to fill OutBuffer. */
	virtual void InputData(StrBuf* OutBuffer, Error* OutError) override
	{
		if (!Command.InputData.IsEmpty())
		{
			OutBuffer->Append(FROM_TCHAR(*Command.InputData, IsUnicodeServer()));
		}
	}
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(FSTClientUser::OutputStat);
		...
		if(IsCollectingData())
		{
			if(File.IsValid())
			{
				...
				Command.Results.ErrorMessages.Add(MoveTemp(Message));
			}

			File.Initialize(Record);
		}

		Command.Values.Add(Record);
	}
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(FSTClientUser::OutputText);
		...
		if(IsCollectingData())
		{
			if(File.OnDataDownloaded(DataPtr, DataLength))
			{
				...
				{
					const FString Filename = File.GetDepotPath();
					Files.Add(Filename, File.Release());
				}
			}
			else
			{
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(FSTClientUser::OutputBinary);
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(FSTClientUser::Message);
		...
		{
			Command.Results.InfoMessages.Add(FText::FromString(MoveTemp(Message)));
		}
		else
		{
			Command.Results.ErrorMessages.Add(FText::FromString(MoveTemp(Message)));
		}
	}

	virtual void OutputInfo(char Indent, const char* InInfo) override
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(FSTClientUser::OutputInfo);

		// We don't expect this to ever be called (info messages should come
		// via ClientUser::Message) but implemented just to be safe.

		Command.Results.InfoMessages.Add(FText::FromString(FString(TO_TCHAR(InInfo, IsUnicodeServer()))));
	}

	virtual void OutputError(const char* errBuf) override
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(FSTClientUser::OutputError);
		...
		Command.Results.ErrorMessages.Add(FText::FromString(FString(TO_TCHAR(errBuf, IsUnicodeServer()))));
	}
	...
	inline TMap<FString, FSharedBuffer> ReleaseData()
	{
...
private:
	TMap<FString, FSharedBuffer> Files;
