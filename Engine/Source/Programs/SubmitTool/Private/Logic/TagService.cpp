...
FTagService::FTagService(const FSubmitToolParameters& InParameters, TSharedPtr<IChangelistService> CLService) :
...
void FTagService::ReApplyTags()
{
	TArray<FTag*> TagsToReApply;

	for (TPair<FString, FTag>& TagPair : RegisteredTags)
	{
		if (TagPair.Value.StartPos != std::numeric_limits<size_t>::max() && !GetCLDescription().Contains(TagPair.Value.Definition.TagId))
		{
			TagPair.Value.StartPos = std::numeric_limits<size_t>::max();
			TagPair.Value.LastSize = std::numeric_limits<size_t>::max();
			TagPair.Value.SetTagState(ETagState::Unchecked);
			TagsToReApply.Add(&TagPair.Value);
		}
	}

	for (FTag* Tag : TagsToReApply)
	{
		ApplyTag(*Tag);
	}

	// Parse to ensure we're matching the current description
	ParseCLDescription();
}
...
{
	UE_LOGF(LogSubmitToolDebug, Log, "Trying to apply tag: %ls", *Tag.Definition.GetTagId());

	if(Tag.StartPos != std::numeric_limits<size_t>::max())
	{
		UE_LOGF(LogSubmitTool, Error, "Tried to apply an already applied Tag: %ls. Something has gone wrong.", *Tag.Definition.GetTagId());
	}
	else
	{
		...
		UE_LOGF(LogSubmitToolDebug, Log, "Tag applied: %ls", *FullTag.TrimChar(TEXT('\n')));
...
void FTagService::RemoveTag(FTag& Tag)
{
	if(Tag.IsEnabled())
	{
		UE_LOGF(LogSubmitToolDebug, Log, "Removing tag: %ls", *Tag.Definition.GetTagId());
