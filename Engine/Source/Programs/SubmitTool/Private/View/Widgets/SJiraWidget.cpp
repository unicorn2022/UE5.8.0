...
void SJiraWidget::Open(const FTag* InTargetTag)
{
	if(!ParentWindow.IsValid())
	{
		...
		ParentWindow->SetRequestDestroyWindowOverride(FRequestDestroyWindowOverride::CreateLambda([this](const TSharedRef<SWindow>& Window) { Window->HideWindow(); }));
...
{ 
	return SNew(SBox)
		...
		[
			...
			[
				SNew(SHorizontalBox)
				.Visibility_Lambda([this]() { return ModelInterface->GetParameters().JiraParameters.OAuthClientId.IsEmpty() ? EVisibility::All : EVisibility::Collapsed; })
				+SHorizontalBox::Slot()
				.AutoWidth()
				.HAlign(EHorizontalAlignment::HAlign_Center)
				.VAlign(EVerticalAlignment::VAlign_Center)
			...
			[				
				SNew(STextBlock)
				.Visibility_Lambda([this]() { return ModelInterface->GetParameters().JiraParameters.OAuthClientId.IsEmpty() ? EVisibility::Collapsed : EVisibility::All; })
				.Text(FText::FromString(TEXT("Authentication with Atlassian OAuth is needed.")))				
			]
			+SVerticalBox::Slot()
			.AutoHeight()
			.Padding(5)
...
{
	if (ModelInterface->GetParameters().JiraParameters.OAuthClientId.IsEmpty())
	{
		ModelInterface->SetLogin(UsernameField->GetText().ToString(), PasswordField->GetText().ToString());
	}
	else
	{
		JiraService.Pin()->ObtainOAuth([this](bool bSuccess) {
			if (bSuccess)
			{
