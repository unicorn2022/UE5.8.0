		...
		[
			...
			{			
				case ETagState::Unchecked:
					if(Tag->GetCurrentValidationConfig(ModelInterface->GetFilesInCL()).bIsMandatory && !Tag->IsEnabled())
					{
			...
			.Text_Lambda([this](){ return FText::FromString(FString::Printf(TEXT("%s%s"), *this->Tag->Definition.TagLabel, Tag->GetCurrentValidationConfig(ModelInterface->GetFilesInCL()).bIsMandatory ? TEXT(" *") : TEXT(""))); })
				...
				[
					...
					[
						...
						.IsEnabled_Lambda([&ModelInterface = ModelInterface]() { 
							if (!ModelInterface->HasSwarmReview())
							{
								return ModelInterface->GetInputEnabled() && ModelInterface->IsSwarmServiceValid() && !ModelInterface->GetSwarmService().Pin()->IsRequestRunning();
							}
							else
							{
								return ModelInterface->IsSwarmServiceValid() && !ModelInterface->GetSwarmService().Pin()->IsRequestRunning();
							}
						})
						[									
							SNew(STextBlock)
	...
	if (this->Tag->Definition.InputType.Equals("MultiSelect", ESearchCase::IgnoreCase))
	{
		...
		HorizontalBox->AddSlot()
			...
			[
				...
				[
					SNew(SComboButton)
					.IsEnabled_Lambda([this](){ return FModelInterface::GetInputEnabled() && SelectValues.Num() > 0;})
	...
	if (this->Tag->Definition.InputType.Equals("JiraIssue", ESearchCase::IgnoreCase))
	{
		...
		HorizontalBox->AddSlot()
			...
			[
				...
				[
					SNew(SButton)
					.IsEnabled_Lambda([this] { return ModelInterface->GetInputEnabled() && !ModelInterface->GetParameters().JiraParameters.bDisableJira; })
					.ToolTipText_Lambda([this] { return ModelInterface->GetParameters().JiraParameters.bDisableJira ? FText::FromString(TEXT("Jira integration is disabled")) : FText::FromString(TEXT("Browse Jira issues")); })
	...
	{		
		...
		HorizontalBox->AddSlot()
			...
			[
				...
				[
					SNew(SComboButton)
					.IsEnabled_Lambda([this](){ return ModelInterface->GetInputEnabled() && PreflightListUI->Num() > 0; })
		...
		HorizontalBox->AddSlot()
			...
			[
				...
				[
					SNew(SButton)
					.ContentPadding(-5)
					.IsEnabled_Lambda([this]{ return Tag->GetValues().Num() > 0; })
					.IsFocusable(true)
						...
						.Text_Lambda([&ModelInterface = ModelInterface]()
							{
								...
								{
									if(ModelInterface->IsShelfReady())
									{ 
										return FText::FromString(TEXT("Requesting..."));
									}
									else
									{ 
										return FText::FromString(TEXT("Waiting for Perforce"));
									}
								}
...
{
	if (!Tag->GetValuesText().Equals(InText.ToString(), ESearchCase::IgnoreCase))
	{
