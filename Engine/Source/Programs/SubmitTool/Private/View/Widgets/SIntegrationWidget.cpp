		...
		[
			...
			[		
				...
				.IsEnabled_Lambda([this]
					{
						return ModelInterface->GetState() != ESubmitToolAppState::Finished && bAreFieldsValid && !ModelInterface->IsBlockingOperationRunning() && ModelInterface->HasSubmitToolTag() && ModelInterface->IsIntegrationRequired();
					})
...
TSharedRef<SWidget> SIntegrationWidget::BuildOptions()
{
	...
	for(const TPair<FString, TSharedPtr<FIntegrationOptionBase>>& Pair : ModelInterface->GetIntegrationOptions())
	{
		...
		default:
			...
			UE_LOGF(LogSubmitTool, Error, "Invalid type specified for Integration Option");
			break;
		}
...
TSharedRef<SHorizontalBox> SIntegrationWidget::TextWithLabel(TSharedPtr<FIntegrationTextOption> InOutOption)
{
	TSharedRef<SHorizontalBox> Widget = SNew(SHorizontalBox)
			...
			Widget->EnableToolTipForceField(true);
