...
void SValidatorsWidget::Construct(const FArguments& InArgs)
{
	Columns = {
		...
		FValidatorColumn(ValidatorsColumns::Status, 150, false, [](TWeakPtr<const FValidatorBase> In) {return In.Pin()->GetStatusText(); }),
...
TSharedRef<SWidget> SValidatorsWidget::BuildValidatorsView(bool bListPreSubmitOperations)
{
	...
	AddValidatorSectionToWidget(VBox, ActiveSection, bListPreSubmitOperations);
	AddValidatorSectionToWidget(VBox, InactiveSection, bListPreSubmitOperations);

	return VBox;
}

void SValidatorsWidget::AddValidatorSectionToWidget(TSharedRef<SVerticalBox> InVBox, const FName& InDisplayName, bool InbListPreSubmitOperations)
{
	bool bInitiallyCollapsed = false;
	if (!(InbListPreSubmitOperations && InDisplayName.IsEqual(ActiveSection)))
	{
		bInitiallyCollapsed = !FSubmitToolUserPrefs::Get()->UISectionExpandState.FindOrAdd(InDisplayName, false);
	}
	...
	(
		...
		.OnAreaExpansionChanged_Lambda([DisplayName = InDisplayName](bool bExpanded) { FSubmitToolUserPrefs::Get()->UISectionExpandState.FindOrAdd(DisplayName, false) = bExpanded; })
		.BorderBackgroundColor_Lambda([DisplayName = InDisplayName, this]
			{
				...
				bool bRunning = false;
				bool bValid = true;
				for (const TWeakPtr<const FValidatorBase>& Validator : Validators.FindOrAdd(DisplayName))
				{
		...
		[
			SNew(STextBlock)
				.Text_Lambda([DisplayName = InDisplayName] {return FText::FromName(DisplayName); })
		]
		.BodyContent()
		[
			SNew(SBox)
				...
				[
					SAssignNew(ValidatorsListView, SListView<TWeakPtr<const FValidatorBase>>)
						...
						.ListItemsSource(&Validators.FindOrAdd(InDisplayName))
						.HeaderRow(ConstructHeadersRow(InDisplayName))
...
public:
	...
	virtual TSharedRef<SWidget> GenerateWidgetForColumn(const FName& InColumnName) override
	{
		...
		{
			const FString NameStr = Node.Pin()->Definition->IsRequired || Node.Pin()->Definition->bRequireCompleteWhenOptional ? Node.Pin()->GetValidatorName() : Node.Pin()->GetValidatorName() + TEXT(" - Optional");
		...
		{
			FText Text;			
			if (Node.Pin()->Definition->IsRequired)
			{
				Text = FText::FromString(TEXT("Success"));
			}
			else if(Node.Pin()->Definition->bRequireCompleteWhenOptional)
			{
				Text = FText::FromString(TEXT("Complete"));
			}
			else
			{
				Text = FText::FromString(TEXT("Optional"));
			}
		...
		else if(InColumnName == ValidatorsColumns::Actions)
		{
			box->SetContent(
				...
				[
					SNew(SButton)
					.IsEnabled_Lambda([this]() {return (!Node.Pin()->GetIsRunningOrQueued() || Node.Pin()->Definition->bCanBeStoppedByUser) && ModelInterface->GetInputEnabled() && Node.Pin()->GetValidatorState() != EValidationStates::Disabled && bIsRelevantToCL; })
					...
					.ToolTipText_Lambda([this]
					{
						...
						{
							return FText::FromString(Node.Pin()->Definition->bCanBeStoppedByUser ? TEXT("Stops this validator.") : TEXT("Stopping this validator is disabled"));
						}
						else
						{
							return FText::FromString(TEXT("Runs this validator. To force run, hold CTRL."));
						}
					})						
				...
				[
					SNew(SButton)
					.IsEnabled_Lambda([this] { return Node.Pin()->Definition->bCanBeDisabledByUser && ModelInterface->GetInputEnabled(); })
					.HAlign(EHorizontalAlignment::HAlign_Center)
					.VAlign(EVerticalAlignment::VAlign_Center)
					.OnClicked_Lambda([this]() 
					{
