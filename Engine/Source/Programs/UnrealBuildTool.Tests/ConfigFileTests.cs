...
namespace UnrealBuildTool.Tests
{
	...
	public class ConfigFileTests
	{
		...
		private class TestConfigFile : IDisposable
		{
			...
			internal TestConfigFile(string fileName, string contents)
			{
				try
				{
					...
					using (StreamWriter writer = new(TemporaryFile))
					{
		...
		public void ConfigFileTestsQuotations()
		{
			...
			using (TestConfigFile temporaryConfigFile = new(TEST_FILE_NAME, testContents))
			{
				...
				FileReference configFileLocation = new(testFilePath);
