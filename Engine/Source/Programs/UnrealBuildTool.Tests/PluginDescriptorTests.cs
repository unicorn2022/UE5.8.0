...
namespace UnrealBuildTool.Tests
{
	...
	public class PluginDescriptorTests
	{
		...
		public void ReadUpluginMissingFileVersion()
		{
			...
			FileReference fileToWrite = CreateTempUPluginFile(filename, json);
			Assert.ThrowsExactly<BuildException>(() => PluginDescriptor.FromFile(fileToWrite));
		}
		...
		{
			return String.IsNullOrEmpty(fileName) || s_tempDirectory == null ? "" : Path.Join(s_tempDirectory.FullName, fileName);
		}
		...
		private static string FixVerbatimStringIdentation(string verbatimString)
		{
			...
			string[] lines = verbatimString.Split([Environment.NewLine], StringSplitOptions.None);
