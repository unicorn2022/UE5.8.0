...
namespace UnrealBuildTool.Tests.TestUtilities
{
	...
	{
		[AssemblyInitialize]
		[System.Diagnostics.CodeAnalysis.SuppressMessage("Style", "IDE0060:Remove unused parameter", Justification = "Required for UnitTesting")]
		public static void InitializeBuildEnvironment(TestContext context)
		{
			...
			string[] subfolders = Directory.GetDirectories(TestDirs.UBTTestFolderRoot, $"{TestDirs.UBTTestFolderPrefix}*", SearchOption.TopDirectoryOnly);
			...
			{
				MicrosoftPlatformSDK sdk = new(logger);
			...
			{
				AndroidPlatformSDK asdk = new(logger);
