...
namespace UnrealBuildTool.Tests
{
	...
	public class CppCompileWarningsTests
	{
		...
		{
			CppCompileWarnings defaultBuildSettings = new();
			...
			Assert.AreEqual(WarningLevel.Off, defaultBuildSettings.UnreachableCodeWarningLevel);
		...
		public void TestClonedMutableReturn()
		{
			...
			defaultBuildSettings.UnreachableCodeWarningLevel = WarningLevel.Warning;
			...
			ReadOnlyCppCompileWarnings wrapper = new(defaultBuildSettings);
...

			{
				...
				Assert.AreEqual(defaultBuildSettings.UnreachableCodeWarningLevel, cloned.UnreachableCodeWarningLevel);
			...
			// Modify and assert != to each other - the same underyling reference has not been returned
			{
				...
				cloned.UnreachableCodeWarningLevel = WarningLevel.Error;
				...
				Assert.AreNotEqual(defaultBuildSettings.UnreachableCodeWarningLevel, cloned.UnreachableCodeWarningLevel);
		...
		{
			TestBuildSettingsProvider buildContextProvider = new();
			buildContextProvider._testBuildSettings = BuildSettingsVersion.V1;

			CppCompileWarnings newWarnings = new(buildContextProvider, null);
		...
		public void TestParentSettings()
		{
			TestBuildSettingsProvider buildContextProvider = new();
			buildContextProvider._testBuildSettings = BuildSettingsVersion.V1;
			CppCompileWarnings parentWarnings = new(buildContextProvider, null);
			CppCompileWarnings newWarnings = new(buildContextProvider, null, new ReadOnlyCppCompileWarnings(parentWarnings));

			// Assert invariants
			{
				Assert.AreEqual(WarningLevel.Default, newWarnings.ShadowVariableWarningLevel);
				Assert.AreEqual(WarningLevel.Default, newWarnings.UndefinedIdentifierWarningLevel);
				Assert.AreEqual(WarningLevel.Default, newWarnings.UnreachableCodeWarningLevel);
			...
			// Assert new parent values
			{
				...
				parentWarnings.UnreachableCodeWarningLevel = WarningLevel.Error;
				...
				Assert.AreEqual(WarningLevel.Error, newWarnings.UnreachableCodeWarningLevel);
		...
		public void TestReparentOperations()
		{
			...
			// Assert reparenting
			CppCompileWarnings parentWarnings2 = new(buildContextProvider, null);
			{
				...
				parentWarnings2.UnreachableCodeWarningLevel = WarningLevel.Warning;
				...
				Assert.AreEqual(WarningLevel.Warning, newWarnings.UnreachableCodeWarningLevel);
			...
			// Assert removal of parentWarnings2 - UndefinedIdentifierWarningLevel, UnreachableCodeWarningLevel, and UnsafeTypeCastWarningLevel will now be the resolved items
			{
				...
				Assert.AreEqual(parentWarnings.UnreachableCodeWarningLevel, newWarnings.UnreachableCodeWarningLevel);
		...
		public void TestChildOverrides()
		{
			...
			newWarnings.UnreachableCodeWarningLevel = WarningLevel.Warning;
		...
		public void TestGenerateWarningCommandLineAgsOrder()
		{
			...
			using (SafeTestFile testTargetTestFile = new(File.ReadAllText(Path.Combine(testStubsDirectory.FullName, "TargetStubs", "TestStub.Target.ubttest")), "TestStub.Target.cs", testSourceDirectory.TemporaryDirectory))
			...
			{
				CppCompileEnvironment newCompileEnvironment = new(UnrealTargetPlatform.Win64, CppConfiguration.Development, new UnrealArchitectures(UnrealArch.X64), null!);
				...
				CppCompileWarnings specializedWarnings = new(testTargetRulesNonWin64, null);

				// Verify the ordering
				{
					VersionNumber newVersionNumber = new(18, 9, 9);
		...
		public void TestGenerateWarningCommandLineAgs()
		{
			...
			using (SafeTestFile testUprojectTestFile = new(File.ReadAllText(Path.Combine(testStubsDirectory.FullName, "UprojectStubs", "TestStub.uproject.ubttest")), "TestStub.uproject", testRootDirectory.TemporaryDirectory))
			{
				...
				CppCompileWarnings baseWarnings = new(testTargetRules, null);
				...
				// Specialized VC toolchain check; UndefinedIdentifier
				{
					...
					CppCompileEnvironment preprocessOnlyCompileEnvironment = new(newCompileEnvironment);
				...
				// Specialized VC toolchain check; UnreachableCode
				{
					baseWarnings.UnreachableCodeWarningLevel = WarningLevel.Warning;
					args = baseWarnings.GenerateWarningCommandLineArgs(newCompileEnvironment, typeof(VCToolChain));

					Assert.IsTrue(args.Count() == 1);
					Assert.IsTrue(args.Contains("/w44702"));
					...
					baseWarnings.UnreachableCodeWarningLevel = WarningLevel.Default;
				}

				{
					...
					CppCompileEnvironment deterministicCompileEnvironment = new(UnrealTargetPlatform.Win64, CppConfiguration.Debug, new UnrealArchitectures(UnrealArch.X64), null!);
			...
			using (SafeTestFile testUprojectTestFile = new(File.ReadAllText(Path.Combine(testStubsDirectory.FullName, "UprojectStubs", "TestStub.uproject.ubttest")), "TestStub.uproject", testRootDirectory.TemporaryDirectory))
			{
				...
				IEnumerable<string> args = null!;
				{
					...
					// PGOOptimizedFilter
					{
						...
						CppCompileEnvironment pgoOptimizedCompileEnvironment = new(UnrealTargetPlatform.Win64, CppConfiguration.Debug, new UnrealArchitectures(UnrealArch.X64), null!);
					...
					// AndroidNDKR28ToolChainVersionExclusion
					{
						...
						VersionNumber newVersionNumber = new(19, 0, 0);
						CppCompileEnvironment androidCompileEnvironment = new(UnrealTargetPlatform.Android, CppConfiguration.Development, new UnrealArchitectures(UnrealArch.X64), null!);
					...
					// AndroidNDKR26ToolChainVersionExclusion
					{
						...
						// Exclusion cases
						{
							VersionNumber newVersionNumber = new(17, 0, 2);
							args = baseWarnings.GenerateWarningCommandLineArgs(androidCompileEnvironment, typeof(ClangToolChain), newVersionNumber);
						...
						// Inclusion cases
						{
							VersionNumber newVersionNumber = new(17, 1);
					...
					// UnusedValueClangToolChainAttribute
					{
						...
						CppCompileEnvironment invalidCompileEnvironment = new(UnrealTargetPlatform.Win64, CppConfiguration.Debug, new UnrealArchitectures(UnrealArch.X64), null!);
						CppCompileEnvironment validCompileEnvironment = new(UnrealTargetPlatform.Win64, CppConfiguration.Shipping, new UnrealArchitectures(UnrealArch.X64), null!);
		...
		public void TestCppStandardConstraintCommandLineArgs()
		{
			...
			using (SafeTestFile testUprojectTestFile = new(File.ReadAllText(Path.Combine(testStubsDirectory.FullName, "UprojectStubs", "TestStub.uproject.ubttest")), "TestStub.uproject", testRootDirectory.TemporaryDirectory))
			{
				...
				// CppStandardConstrainedFilterAttribute
				{
					...
					CppCompileEnvironment cppStandardCompileEnvironment = new(UnrealTargetPlatform.Win64, CppConfiguration.Debug, new UnrealArchitectures(UnrealArch.X64), null!);
		...
		public void TestVersionConstraintCommandLineArgs()
		{
			...
			using (SafeTestFile testUprojectTestFile = new(File.ReadAllText(Path.Combine(testStubsDirectory.FullName, "UprojectStubs", "TestStub.uproject.ubttest")), "TestStub.uproject", testRootDirectory.TemporaryDirectory))
			{
				...
				// Clang toolchain version check - valid versions;
				{
					...
					VersionNumber newVersionNumber = new(16, 9, 5);
