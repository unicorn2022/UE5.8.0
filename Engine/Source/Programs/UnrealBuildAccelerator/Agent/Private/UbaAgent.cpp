...
namespace uba
{
	...
	bool PrintHelp(const tchar* message)
	{
		...
		logger.Info(TC("  -disablecrawler         Disables file crawler for known process types (clang/msvc)"));
	...
	Atomic<bool> g_ctrlPressed = false;
	...
	bool WrappedMain(int argc, tchar*argv[])
	{
		...
		for (int i=1; i!=argc; ++i)
		{
			...
			else if (name.Equals(TCV("-disablecrawler")))
			{
				useCrawler = false;
			}
			else if (name.Equals(TCV("-usecrawler"))) // Ignored, flag has been inverted
			{
				useCrawler = true;
			}
			#if PLATFORM_WINDOWS
		...
		#if PLATFORM_WINDOWS
		if (IsRunningWine() && UsingNativeSockets())
			extraInfo.Append(TCV(", NativeSock"));
		#endif
		...
		u32 logLinesIndex = 0;
		Futex logLinesLock;
		Event logLinesAvailable(EventResetType_Auto);
		...
		do
		{
			...
			if (useListen)
			{
				while (true)
				{
					...
					{
						killTcpHogs = false;
						if (!NetworkBackendTcp::KillTcpHogs(logger, port))
							return false;
						Sleep(2000);
						continue;
					}
			...
			auto startProxy = [&](u16 proxyPort, const Guid& storageServerUid)
				{
					...
					if (!proxyServer->StartListen(*networkBackend, proxyPort))
					{
						if (sessionClient)
							sessionClient->SendNotification(TCV("Failed to create proxy server"));
						//NetworkBackendTcp::KillTcpHogs(logger, proxyPort)
						return false;
					}
			...
			Atomic<bool> isDisconnected = false;
			...
			auto secsg = MakeGuard([&]() { delete sessionClient; sessionClient = nullptr; });
			...
			while (!ShouldExit())
			{
				...
				if (auto proxyServer = proxyNetworkServer.load())
				{
					...
					{
						if (maxIdleSeconds < 30) // A bit hacky solution.. but we really want to try preventing idling out the proxy if possible when other agents are connected
							sessionClient->SetMaxIdleTime(maxIdleSeconds*2);
					...
					{
						// Restore idle time if reaching 0 connections.
						sessionClient->SetMaxIdleTime(maxIdleSeconds);
					}
				}

				if (u32 desiredConnectionCount = sessionClient->GetDesiredConnectionCount())
				{
					targetConnectionCount = desiredConnectionCount;
				}
				else
				{
