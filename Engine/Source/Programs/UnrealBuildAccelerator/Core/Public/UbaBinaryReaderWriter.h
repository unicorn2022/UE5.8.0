...
namespace uba
{
	...
	enum ProcessCommunication { ProcessCommunication_Write };
	...
	struct BinaryWriter
	{
		...
		bool WriteBytesCompressed(const void* data, u64 size);
		...
		BinaryReader Flush(bool waitOnResponse = true);
		BinaryWriter(ProcessCommunication);
	...
	struct BinaryReader
	{
		...
		void ReadBytesCompressed(void* outData, u64 outCapacity);
	...
	u64 BinaryReader::InternalReadString(tchar* str, u64 charLen)
	{
		...
		while (left--)
		{
			...
			if (a >= 240 && a <= 253)
			{
				// Wide chars cannot exceed 16 bits
				UBA_ASSERTF(false, TC("This should not happen, data is corrupt (Reader Start:0x%llx Pos:%llu Length:%llu Data:%.*s)"), u64(m_begin), u64(pos - m_begin), u64(m_end - m_begin), u32(it - str), str); // Wide chars cannot exceed 16 bits
				*it++ = tchar(~0);
				continue;
			}
			UBA_ASSERTF(false, TC("This should not happen, data is corrupt (Reader Start:0x%llx Pos:%llu Length:%llu Data:%.*s)"), u64(m_begin), u64(pos - m_begin), u64(m_end - m_begin), u32(it - str), str); // Wide chars cannot exceed 16 bits
