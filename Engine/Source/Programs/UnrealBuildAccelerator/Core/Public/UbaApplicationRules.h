...
namespace uba
{
	...
	public:
		...
		virtual bool CanDetour(StringView file, bool isRunningRemote) const
		{
			return true;
		}
		...
		// When writing this type of file, how big commits should shared memory system take. We want as few as possible without wasting memory
		...
		virtual bool IsMaybeReadAfterWritten(const StringView& fileName) const
		{
			return false;
		}
		...
		// Files that can be written as placeholders if WritePlaceholders is enabled
		virtual bool SupportsPlaceholder(StringView fileName) const
		{
			return false;
		}

		virtual bool SupportsRacing() const
		{
			return false;
		}

		virtual bool Time64AlwaysReturnZero() const
		{
			return false;
		}
