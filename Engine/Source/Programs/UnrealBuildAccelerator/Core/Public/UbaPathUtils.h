...
namespace uba
{
	...
	inline bool IsUncPath(const tchar* path) { return IsWindows ? (path[0] == '\\' && path[1] == '\\') : false; }
	inline bool IsUncPath(StringView path) { return IsUncPath(path.data); }

	inline bool FixPath2(const tchar* fileName, u32 fileNameLen, const tchar* workingDir, u64 workingDirCharLen, tchar* buffer, u64 bufferCharCapacity, u32* outBufferCharLen)
	{
		buffer[1] = 0;
		UBA_ASSERT(fileName);
		...
		StringBuffer<1024> fullName;
		while (true)
		{
			tchar temp[1024];
			if (read == fullName.data)
			{
				TStrcpy_s(temp, sizeof_array(temp), read);
				read = temp;
			}

			StringView sv(read, fileNameLen);
			u32 tildePos;
			if (!sv.Contains('~', &tildePos))
				break;
			
			// Since this might be a in memory file we can't use the actual full name, so let's find first slash after '~'
			StringView tilde = sv.Skip(tildePos);
			u32 backslashAfterTilde = tilde.count;
			bool hasBackslashAfterTilde = tilde.Contains('\\', &backslashAfterTilde);

			fullName.Clear().Append(sv.data, tildePos + backslashAfterTilde);
			u32 len = Local_GetLongPathNameW(fullName.data, fullName.data, sizeof_array(fullName));
			if (!len)
			{
				if (outBufferCharLen)
					*outBufferCharLen = 0;
			...
			fullName.count = len;
			if (hasBackslashAfterTilde)
				fullName.Append(tilde.Skip(backslashAfterTilde));
			read = fullName.data;
			fileNameLen = fullName.count;
		}

		if (read[0] == '\\')
		{
			const tchar* prevRead = read;
			if (read[1] == '\\')
			{
				if (read[2] == '?' && read[3] == '\\')
					read += 4;
			...
			fileNameLen -= u32(read - prevRead);
		}

		bool isAbsolute = false;

		const tchar* readEnd = read + fileNameLen;
		while (read < readEnd)
		{
			tchar c = *read++;
			if (!c)
			...
			{
				if ((write - buffer) != 1) // colon must be on position 1
					return false;
				isAbsolute = true;
			}
			if (c == '\\')
	...
	inline bool FixPath(const tchar* fileName, const tchar* workingDir, u64 workingDirCharLen, StringBufferBase& buffer)
	{
		...
		bool res = FixPath2(fileName, ~0u, workingDir, workingDirCharLen, buffer.data + count, buffer.capacity - count, &buffer.count);
		buffer.count += count;
		return res;
	}

	inline bool FixPath(StringBufferBase& out, StringView fileName, StringView workingDir)
	{
		u32 count = out.count;
		bool res = FixPath2(fileName.data, fileName.count, workingDir.data, workingDir.count, out.data + count, out.capacity - count, &out.count);
		out.count += count;
		return res;
	}
