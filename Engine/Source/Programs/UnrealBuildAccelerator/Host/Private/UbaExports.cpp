...
extern "C"
{
	...
	uba::LogWriter* LogWriter_CreateCallback(uba::CallbackLogWriter::BeginScopeCallback begin, uba::CallbackLogWriter::EndScopeCallback end, uba::CallbackLogWriter::LogCallback log)
	{
		...
		LogWriter* writer = new CallbackLogWriter(begin, end, log);

		LoggerWithWriter logger(*writer, TC(""));
		SetMemoryWorkingSet(logger, DefaultMemoryWorkingSet);

		return writer;
	}

	void LogWriter_SetGlobal(uba::LogWriter* writer)
	{
		uba::g_globalLogWriter = writer;
	}
	...
	uba::NetworkServer* NetworkServer_Create(uba::LogWriter& writer, uba::u32 workerCount, uba::u32 sendSize, uba::u32 receiveTimeoutSeconds, bool useQuic)
	{
		...
		info.useThreadedSend = !IsLinux; // Threaded send is turned off for linux since it is not needed (it improves perf/throughput on win and mac)
		info.Apply(GetConfig());
	...
	bool ProcessHandle_IsCancelled(const uba::ProcessHandle* handle)
	{
		return handle->GetExitCode() == uba::ProcessCancelExitCode;
	}
	...
	uba::SessionServerCreateInfo* SessionServerCreateInfo_Create(uba::StorageServer& storage, uba::NetworkServer& client, uba::LogWriter& writer, const uba::tchar* rootDir, const uba::tchar* traceOutputFile, bool disableCustomAllocator, bool launchVisualizer, bool resetCas, bool writeToDisk, bool detailedTrace, bool allowWaitOnMem, bool allowKillOnMem, bool storeIntermediateFilesCompressed)
	{
		...
		info->rootDir = uba::TStrdup(rootDir);
		info->traceOutputFile = uba::TStrdup(traceOutputFile);
	...
	{
		server->SetRemoteProcessSlotAvailableEvent([available, userData](bool isCrossArchitecture, bool isCrossPlatform) { available(userData, isCrossArchitecture); });
	}
	...
	bool SessionServer_CreateVirtualFile(uba::SessionServer* server, const uba::tchar* filename, const void* memory, uba::u64 memorySize, bool transient, bool registerToDir)
	{
		return server->CreateVirtualFile(filename, memory, memorySize, transient, registerToDir);
	}
	bool SessionServer_DeleteVirtualFile(uba::SessionServer* server, const uba::tchar* filename, bool registerToDir)
	{
		return server->DeleteVirtualFile(filename, registerToDir);
	}
	...
	bool SessionServer_DisableRemoteExecutionOnAgent(uba::SessionServer* server, const uba::tchar* agentName)
	{
		return server->DisableRemoteExecutionOnAgent(agentName);
	}
	...
	{
		return Scheduler_EnqueueProcess4(scheduler, info, weight, canDetour, canExecuteRemotely, false, false, dependencies, dependencyCount, knownInputs, knownInputsBytes, knownInputsCount, cacheBucket, memoryGroup, predictedMemoryUsage);
	}

	uba::u32 Scheduler_EnqueueProcess4(uba::Scheduler* scheduler, const uba::ProcessStartInfo& info, float weight, bool canDetour, bool canExecuteRemotely, bool canCrossArchitecture, bool canCrossPlatform, const uba::u32* dependencies, uba::u32 dependencyCount, const void* knownInputs, uba::u32 knownInputsBytes, uba::u32 knownInputsCount, uba::u32 cacheBucket, uba::u32 memoryGroup, uba::u64 predictedMemoryUsage)
	{
		...
		epi.canCrossArchitecture = canCrossArchitecture;
		epi.canCrossPlatform = canCrossPlatform;
	...
	{
		float total;
		float crossArchitecture;
		float crossPlatform;
		scheduler->GetProcessWeightThatCanRunRemotelyNow(total, crossArchitecture, crossPlatform);
		return total;
	}

	void Scheduler_GetProcessWeightThatCanRunRemotelyNow2(uba::Scheduler* scheduler, float& outTotal, float& outCrossArchitecture, float& outCrossPlatform)
	{
		scheduler->GetProcessWeightThatCanRunRemotelyNow(outTotal, outCrossArchitecture, outCrossPlatform);
	}
	...
	{
		using namespace uba;
		auto trace = new Trace(g_globalLogWriter ? *g_globalLogWriter : g_consoleLogWriter);
		...
		if (ownerInfo.pid)
			fullName.Appendf(TC("_%s%u"), ownerInfo.id, ownerInfo.pid);
	...
	{
		using namespace uba;
		static auto f = [](const tchar* text, u32 textLen, void* userData) { ((Uba_CustomAssertHandler*)userData)(text); };
		SetCustomAssertHandler(f, (void*)handler);
	}
	...
	void Uba_FindImports(const uba::tchar* binary, ImportFunc* func, void* userData)
	{
		...
		uba::BinaryInfo info;
		uba::ParseBinary(uba::ToView(binary), {}, info, [&](const uba::tchar* importName, bool isKnown, const uba::tchar* const* importLoaderPaths) { func(importName, userData); }, errors);
#else
		UBA_ASSERT(false);
#endif
	}
		...
		auto createCoordinator = [&]()
			{
				if (auto coordinatorTable = config.GetTable(TC("Coordinator")))
				{
					...
					if (coordinatorTable->GetValueAsString(coordinatorName, TC("Name")))
					{
						...
						u32 maxCoreCount = 0;
						coordinatorTable->GetValueAsU32(maxCoreCount, TC("MaxCoreCount"));
						...
						if (!ubaInstance->coordinator.Create(logger, coordinatorName, cinfo, *networkServer->backend, *networkServer, sessionServer->GetTrace(), 0))
							return;
						ubaInstance->coordinator.SetScheduler(scheduler);
						ubaInstance->coordinator.RequestHelpers(maxCoreCount);
					}
				}
			};
		createCoordinator();
