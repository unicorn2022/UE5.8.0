...
extern "C"
{
	...
	// Same as above but with a timeout how long it can wait before giving up to get next process and a "should exit" variable telling the calling process to exit
	UBA_DETOURED_API bool UbaRequestNextProcess2(unsigned int prevExitCode, wchar_t* outArguments, unsigned int outArgumentsCapacity, unsigned int timeOutMs, bool* outShouldExit);
}
