...
wchar_t* Detoured__wfullpath(wchar_t* absPath, const wchar_t* relPath, size_t maxLength)
{
	...
	if (!absPath)
	{
		...
		FixPath2(relPath, ~0u, g_virtualWorkingDir.data, g_virtualWorkingDir.count, res, maxLength, nullptr);
	...
	{
		FixPath2(relPath, ~0u, g_virtualWorkingDir.data, g_virtualWorkingDir.count, absPath, maxLength, nullptr);
...
static WCHAR* g_fake_wpgmptr = g_virtualApplication.data;

WCHAR** Detoured___p__wpgmptr()
{
	DETOURED_CALL(__p__wpgmptr);
	DEBUG_LOG_DETOURED(L"__p__wpgmptr", L"(%ls)", g_virtualApplication.data);
	return &g_fake_wpgmptr;
}

__time64_t Detoured__time64(__time64_t* destTime)
{
	DETOURED_CALL(_time64);
	if (g_rules->Time64AlwaysReturnZero())
	{
		if (destTime)
			*destTime = 0;
		return 0;
	}
	return True__time64(destTime);
}
...
errno_t Detoured__waccess_s(const wchar_t* path, int mode)
{
	...
	if (!CanDetour(fixedPath))
	{
		auto res = True__waccess_s(fixedPath.data, mode);
	...
	FileAttributes attr;
	Shared_GetFileAttributes(attr, fixedPath);
...
intptr_t Detoured__get_osfhandle(int fd)
{
	...
	//DEBUG_LOG_TRUE(L"_get_osfhandle", L"(%i) -> %llu", fd, u64(res)); // Spam
	return res;
}
...
int Detoured_fputs(const char* str, FILE* stream)
{
	...
	// - Decided to remove this function because I think it was written before the refactoring where detoured processes spawned their own children
	// and pipes became a huge mess
	/*
