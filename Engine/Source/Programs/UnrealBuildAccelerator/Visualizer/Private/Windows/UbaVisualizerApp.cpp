...
#include "Windows/UbaVisualizerWin.h"
#include <tlhelp32.h>
#include <psapi.h>
#include <imm.h>
...
int WINAPI wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ PWSTR pCmdLine, _In_ int nShowCmd)
{
	...
	for (int i=1; i!=argc; ++i)
	{
		...
		else if (name.Equals(TCV("-gdi")))
		{
			useGDI = true;
		}
		else if (name.Equals(TCV("-d2d")))
		{
			useD2D = true;
		}
	...
	if (!configPath.count)
	{
		...
		if (ownerInfo.pid)
		{
			...
			//showAllTraces = false; // It seems like showAllTraces is actually what most people want regardless of where a trace is initiated.
		}
	...
	if (useGDI)
		visualizerConfig.useGDI = true;
	if (useD2D)
		visualizerConfig.useGDI = false;

	NetworkBackendTcp networkBackend(logWriter);
	VisualizerWin visualizer(visualizerConfig, logger);
