...
namespace uba
{
	...
	{
		return u64(double(time) * view.timeMultiplier);
	}
	...
	bool TraceReader::ReadFile(TraceView& out, const tchar* fileName, bool replay)
	{
		...
		m_memoryHandle = FileMapping_CreateFromFile(m_logger, readHandle, PAGE_READONLY, 0, fileName);
		if (!m_memoryHandle.IsValid())
			return false;
		auto closeFileMapping = MakeGuard([&]() { FileMapping_Close(m_logger, m_memoryHandle, fileName); m_memoryHandle = {}; });

		m_memoryBegin = FileMapping_Map(m_logger, m_memoryHandle, FILE_MAP_READ, 0, fileSize);
		if (!m_memoryBegin)
			return false;
		auto closeView = MakeGuard([&]() { FileMapping_Unmap(m_logger, m_memoryBegin, fileSize, TC("TraceReader")); m_memoryBegin = nullptr; m_memoryPos = nullptr; m_memoryEnd = nullptr; });
		...
		out.timeMultiplier = double(GetFrequency()) / double(out.frequency);
	...
	bool TraceReader::StartReadClient(TraceView& out, NetworkClient& client)
	{
		...
		m_memoryHandle = FileMapping_Create(m_logger, PAGE_READWRITE, traceMemSize, 0, TC("NetworkClient"));
		if (!m_memoryHandle.IsValid())
			return false;
		m_memoryBegin = FileMapping_Map(m_logger, m_memoryHandle, FILE_MAP_ALL_ACCESS, 0, traceMemSize);
		m_memoryPos = m_memoryBegin;
	...
	bool TraceReader::UpdateReceiveClient(NetworkClient& client)
	{
		...
		bool hasLock = false;
		auto lg = MakeGuard([&]() { if (hasLock) m_memoryFutex.Leave(); });
	...
	bool TraceReader::StartReadNamed(TraceView& out, const tchar* namedTrace, bool silentFail, bool replay)
	{
		...
		if (*namedTrace && m_namedTrace != namedTrace)
		{
#if PLATFORM_WINDOWS
			u64 traceCapacity = 0;
			m_memoryHandle.internal = u64(::OpenFileMappingW(PAGE_READWRITE, false, namedTrace));
#else
			u64 traceCapacity = 128 * 1024 * 1024;
			int oflags = O_RDONLY | O_NOFOLLOW;
			m_memoryHandle.shmFd = shm_open(namedTrace, oflags, S_IRUSR | S_IWUSR);
			m_memoryHandle.internal = ~0ull;

			// We don't want to unlink
			//if (m_memoryHandle.shmFd != -1)
			//	m_memoryHandle.uid = atoi(strchr(namedTrace, '_') + 1);
			//printf("%s: %i\n", namedTrace, m_memoryHandle.shmFd);
#endif
			...
			m_memoryBegin = FileMapping_Map(m_logger, m_memoryHandle, FILE_MAP_READ, 0, traceCapacity);
			if (!m_memoryBegin)
			{
				if (!silentFail)
					m_logger.Error(TC("MapViewOfFile - Failed to map file mapping %s (%s)"), namedTrace, LastErrorToText().data);
				return false;
			}
		}
	...
	bool TraceReader::UpdateReadNamed(TraceView& out, u64 maxTime, bool& outChanged)
	{
...
#else
		if (m_hostProcess && kill(m_hostProcess, 0) != 0)
#endif
		{
		...
		FileMappingHandle memoryHandle = FileMapping_Create(m_logger, PAGE_READWRITE, traceMemSize, nullptr, TC("UpdateReadNamed"));
		if (memoryHandle.IsValid())
			if ((memoryBegin = FileMapping_Map(m_logger, memoryHandle, FILE_MAP_ALL_ACCESS, 0, traceMemSize)) != nullptr)
	...
	bool TraceReader::ReadTrace(TraceView& out, BinaryReader& reader, u64 maxTime)
	{
		...
		case TraceType_ProcessEnvironmentUpdated:
		{
			...
			if (reason.Equals(TCV("IDLE")))
				process2.isIdle = true;
			else
		...
		case TraceType_ProcessRace:
		{
			u32 id = reader.ReadU32();
			u32 remoteId = reader.ReadU32();(void)remoteId;
		...
		case TraceType_StatusUpdate2:
		{
			...
			status.type = LogEntryType_Info;
			break;
		}
		case TraceType_TableUpdate:
		{
			u32 name = u32(reader.Read7BitEncoded());
			TraceView::Table& table = out.tables[name];
			table.rowCharWidth = u32(reader.Read7BitEncoded());
			table.rowMaxCount = u32(reader.Read7BitEncoded());
			u32 itemCount = u32(reader.Read7BitEncoded());
			table.items.resize(itemCount);
			for (u32 i = 0; i != itemCount; ++i)
				table.items[i] = out.strings[reader.Read7BitEncoded()];
			break;
		}
		...
		case TraceType_SchedulerKillProcess:
		{
			...
			const tchar* killReason = nullptr;
			if (out.version >= 52)
				killReason = out.strings[reader.Read7BitEncoded()];
...
#else
		u64 unmapSize = 128 * 1024 * 1024;
#endif
		m_hostProcess = 0;

		if (m_memoryBegin)
			FileMapping_Unmap(m_logger, m_memoryBegin, unmapSize, TC("TraceReader"));
		m_memoryBegin = nullptr;
		if (m_memoryHandle.IsValid())
			FileMapping_Close(m_logger, m_memoryHandle, TC("TraceReader"));
		m_memoryHandle = {};
		m_namedTrace.clear();
	}
