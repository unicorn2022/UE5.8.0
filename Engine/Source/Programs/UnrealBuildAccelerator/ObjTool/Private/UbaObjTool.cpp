		...
		auto parseArg = [&](const tchar* arg, bool isRsp)
			{
				...
				else
				{
					...
					if (colon && (!IsWindows || (colon[1] != '\\' && colon[1] != '/')))
					{
				...
				auto checkHasValue = [&]()
					{
						if (value.count)
							return true;
						LoggerWithWriter(g_consoleLogWriter, TC("")).Error(TC("   ERROR: %s needs a filename"), name.data);
						return false;
					};
				...
				if (isImpLibRsp)
				{
					...
					else if (name.StartsWith(TCV("/ALTERNATENAME")))
					{
					}
