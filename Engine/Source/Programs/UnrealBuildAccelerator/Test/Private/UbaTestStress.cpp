...
namespace uba
{
	...
	bool TestStress(LoggerWithWriter& logger, const StringBufferBase& testRootDir)
	{
		...
		if (useCoordinator)
		{
			...
			CHECK_TRUE(GetDirectoryOfCurrentModule(logger, binariesDir));
			...
			cinfo.logging = true;
			CHECK_TRUE(coordinator.Create(logger, TC("Horde"), cinfo, networkBackend, networkServer, sessionServer.GetTrace()));
		}
