...
namespace uba
{
	...
	#else
	Atomic<u32> g_tempFileCounter = 0;
	#endif
	...
	{
		#if PLATFORM_WINDOWS
		UBA_ASSERT((flagsAndAttributes & FILE_FLAG_OVERLAPPED) == 0 || EventIsNative);
		#endif
		...
		while (true)
		{
			...
			#if PLATFORM_WINDOWS
			constexpr u32 ERROR_INSUFFICIENT_RESOURCES = 1450;
			if (lastError == ERROR_SHARING_VIOLATION || lastError == ERROR_USER_MAPPED_FILE || lastError == ERROR_ACCESS_DENIED || lastError == ERROR_INSUFFICIENT_RESOURCES)
			{
				...
				// File can also be read-only for whatever reason
				FileBasicInformation info;
				if (lastError == ERROR_ACCESS_DENIED && !additionalInfo.count)
					if (GetFileBasicInformation(info, m_logger, realFileName, false))
						if (IsReadOnly(info.attributes))
							additionalInfo.Append(TCV(" - File has read-only attribute set"));
			...
			return m_logger.Warning(TC("ERROR opening file %s for write%s (%s%s)"), realFileName, retryText, LastErrorToText(lastError).data, additionalInfo.data);
		}
	...
	bool FileAccessor::CreateMemoryWrite(bool allowRead, u32 flagsAndAttributes, u64 size, const tchar* tempPath)
	{
		...
		// Size 0 can never be written to so just return a pointer to some memory
		if (size == 0)
		{
			m_data = (u8*)"";
			return true;
		...
		m_mappingHandle = FileMapping_CreateFromFile(m_logger, m_fileHandle, PAGE_READWRITE, size, realFileName);
		...
		m_data = FileMapping_Map(m_logger, m_mappingHandle, FILE_MAP_WRITE, 0, size);
		if (!m_data)
	...
	bool FileAccessor::InternalWrite(const void* data, u64 dataLen, u64 offset, bool lastWrite)
	{
		...
		if ((u64)m_fileHandle & OverlappedIoFlag)
		{
			...
			while (writeLeft)
			{
				...
				if (i < BlockCount)
					ev[i].Create(EventResetType_Manual);
				else
				{
		...
		while (writeLeft)
		{
			...
			{
				DWORD lastError = GetLastError();
				if ((lastError == ERROR_WORKING_SET_QUOTA || lastError == ERROR_PAGEFILE_QUOTA) && retryIndex < 5)
				{
					if (retryIndex == 0)
					{
						const tchar* quotaType = lastError == ERROR_WORKING_SET_QUOTA ? TC("Workingset") : TC("Pagefile");
						m_logger.Info(TC("Insufficient %s quota while writing %s. Will retry..."), quotaType, m_fileName);
					}
					++retryIndex;
					Sleep(retryIndex*1000);
					continue;
				}
	...
	bool FileAccessor::OpenMemoryRead(u64 offset, bool errorOnFail)
	{
		if (!OpenFileSequentialRead(m_logger, m_fileName, m_fileHandle))
			return errorOnFail ? m_logger.Error(TC("Failed to open file %s for read (%s)"), m_fileName, LastErrorToText().data) : false;
...
#if PLATFORM_WINDOWS
		if (m_size)
			m_mappingHandle = FileMapping_CreateFromFile(m_logger, m_fileHandle, PAGE_READONLY, m_size, m_fileName);
		else
			m_mappingHandle = FileMapping_CreateFromFile(m_logger, InvalidFileHandle, PAGE_READONLY, 1, m_fileName);

		if (!m_mappingHandle.IsValid())
			return m_logger.Error(TC("Failed to create mapping handle for %s with size %llu (%s)"), m_fileName, m_size, LastErrorToText().data);
#else
		m_mappingHandle.internal = ~0ull;
		m_mappingHandle.shmFd = AsFileDescriptor(m_fileHandle);
		if (offset == m_size)
			return true;
#endif
		m_data = FileMapping_Map(m_logger, m_mappingHandle, FILE_MAP_READ, offset, m_size);
		if (!m_data)
	...
	bool FileAccessor::InternalClose(bool success, u64* lastWriteTime)
	{
		if (m_data)
		{
			if (m_size && !FileMapping_Unmap(m_logger, m_data, m_size, m_fileName))
		...
		if (m_mappingHandle.IsValid())
		{
			if (!FileMapping_Close(m_logger, m_mappingHandle, m_fileName))
