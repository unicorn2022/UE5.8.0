...
namespace uba
{
	...
	ANALYSIS_NORETURN void UbaAssert(const tchar* text, const char* file, u32 line, const char* expr, bool allowTerminate, u32 terminateCode, void* context, u32 skipCallstackCount)
	{
		...
		static ParkingLot::WordMutex assertMutex;
		SCOPED_CRITICAL_SECTION(assertMutex, scopedLock);
		#else
		...
		if (g_assertHandler)
		{
			g_assertHandler(sb.data, sb.count, g_assertHandlerUserData);
			return;
		}
	...
	void SetCustomAssertHandler(CustomAssertHandler* handler, void* userData)
	{
		g_assertHandler = handler;
		g_assertHandlerUserData = userData;
	}
	...
	Logger& Logger::Logf(LogEntryType type, const tchar* format, ...)
	{
		va_list arg;
		va_start(arg, format);
	...
	public:
		...
		void LogNoLock(LogEntryType type, const tchar* str, u32 strLen, const tchar* prefix, u32 prefixLen, bool addEndOfLine);
		Futex m_lock;
#if PLATFORM_WINDOWS
		HANDLE m_stdout = 0;
	...
	class NullLogWriter final : public LogWriter
	{
	public:
		virtual void BeginScope() override {}
		virtual void EndScope() override {}
	...
	{
		if (t_consoleLogScopeCount)
			return LogNoLock(type, str, strLen, prefix, prefixLen, true);
		SCOPED_FUTEX(m_lock, lock);
		LogNoLock(type, str, strLen, prefix, prefixLen, true);
#if PLATFORM_WINDOWS
		if (!m_stdout)
#endif
			fflush(stdout);
	...
	void ConsoleLogWriter::LogNoLock(LogEntryType type, const tchar* str, u32 strLen, const tchar* prefix, u32 prefixLen, bool addEndOfLine)
	{
#if PLATFORM_WINDOWS
		if (!m_stdout)
		{
	...
	void LogToConsoleNoLineFeed(LogEntryType type, const tchar* str, u32 strLen)
	{
		auto& logWriter = (ConsoleLogWriter&)g_consoleLogWriter;
		logWriter.BeginScope();
		logWriter.LogNoLock(type, str, strLen, nullptr, 0, false);
		logWriter.EndScope();
	}
	...
	class DebugLogWriter final : public LogWriter
	{
	public:
		virtual void BeginScope() override
		{
