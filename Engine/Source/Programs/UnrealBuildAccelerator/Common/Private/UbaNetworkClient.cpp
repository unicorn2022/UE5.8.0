...
namespace uba
{
	...
	{
		Connection(NetworkClient& o) : owner(o), disconnectedEvent(EventResetType_Manual) {}
		NetworkClient& owner;
		void* backendConnection = nullptr;
		Atomic<u32> connected = 0;
		Event disconnectedEvent;
		NetworkBackend* backend = nullptr;
		Guid cryptoIV;

		#if UBA_TRACK_NETWORK_TIMES
		Atomic<u64> lastSendTime = 0;
		Atomic<u64> lastHeaderRecvTime = 0;
		Atomic<u64> lastBodyRecvTime = 0;
		#endif
	};
	...
	,	m_isConnected(EventResetType_Manual)
	,	m_isOrWasConnected(EventResetType_Manual)
	{
		outCtorSuccess = true;
	...
	bool NetworkClient::AddConnection(NetworkBackend& backend, void* backendConnection, bool* timedOut)
	{
		struct RecvContext
		{
			RecvContext(NetworkClient& c, NetworkBackend& b, void* bc) : client(c), backend(b), backendConnection(bc), recvEvent(EventResetType_Manual), exitScopeEvent(EventResetType_Manual)
			{
			}
	...
	bool NetworkClient::ReceiveResponseBody(void* context, bool recvError, u8* headerData, void* bodyContext, u8* bodyData, u32 bodySize)
	{
		...
		if (recvError)
		{
			...
			if (!found)
			{
				if (client.m_isDisconnecting)
					return false;
				u16 messageId = u16(headerData[0] << 8) | u16((*(u32*)(headerData + 1) & 0xff000000) >> 24);
				u32 messageSize = *(u32*)(headerData + 1) & 0x00FFFFFF;
				return client.m_logger.Error(TC("NetworkClient got receive response body for message that does not exist (Id: %u, Size: %u)"), u32(messageId), messageSize);
			}
			msg.m_error = 2;
		}
		#if UBA_TRACK_NETWORK_TIMES
	...
	void NetworkClient::TraverseConnections(const Function<void(NetworkBackend&, void*)>& func)
	{
		SCOPED_WRITE_LOCK(m_connectionsLock, lock);
		for (auto& c : m_connections)
			func(*c.backend, c.backendConnection);
	}
	...
	bool NetworkClient::Send(NetworkMessage& message, void* response, u32 responseCapacity, bool async)
	{
		...
		if (response)
		{
			if (!async)
			{
				if (!gotResponse.Create(EventResetType_Manual))
				{
			...
			while (true)
			{
				...
				if (!async)
				{
					...
					message.m_doneFunc = [](bool error, void* userData) { ((EventSlim*)userData)->Set(); };
				}
				break;
			}
		}
		...
		*(u32*)(data + 2) = dataSize | u32(messageId) << 24;
		...
		if (response)
		{
			...
			{
				m_logger.Warning(TC("Timed out after %s waiting for message response from server. (%s)"), TimeToText(GetTime() - waitStart, true).str, MessageToString(message.GetServiceId(), message.GetMessageType()).data);
				message.m_error = 4;
	...
	bool NetworkMessage::Send(Timer& outTimer)
	{
