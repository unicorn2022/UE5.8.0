...
#if UBA_CACHE_USE_SEND_MAX_BUFFER_CACHE
#define UBA_CACHE_BINARY_WRITER_MAX_SIZE(name) BinaryWriterUsingCache name(m_sendBufferSlots);
#define UBA_CACHE_BINARY_READER_MAX_SIZE(name) BinaryReaderUsingCache name(m_sendBufferSlots);
#else
#define UBA_CACHE_BINARY_WRITER_MAX_SIZE(name) StackBinaryWriter<SendMaxSize> name;
#define UBA_CACHE_BINARY_READER_MAX_SIZE(name) StackBinaryReader<SendMaxSize> name;
#endif
...
namespace uba
{
...
#endif
	{
		...
		m_useMappingForCreatedCas = info.useMappingForCreatedCas;
	...
	u64 CacheClient::RegisterSpeculativeOutputs(FetchContext& context)
	{
		UBA_ASSERT(context.bucket);
		auto& bucket = *context.bucket;

		SCOPED_FUTEX(m_speculativeCasKeysLock, lock);
		u64 id = m_nextSpeculativeOutputId++;
		auto& keys = m_speculativeOutputsById[id];

		for (u32 offset : context.casKeyOffsets)
		{
			StringBuffer<MaxPath> path;
			CasKey casKey;
			if (!GetLocalPathAndCasKey(bucket, context.rootPaths, path, casKey, bucket.serverCasKeyTable, bucket.serverPathTable, offset))
			{
				UBA_ASSERT(false);
				continue;
			}
			...
			m_speculativeCasKeys.emplace(fileNameKey, casKey);
			keys.push_back(fileNameKey);
		}
		return id;
	}

	void CacheClient::UnregisterSpeculativeOutputs(u64 id)
	{
		SCOPED_FUTEX(m_speculativeCasKeysLock, lock);
		auto it = m_speculativeOutputsById.find(id);
		if (it == m_speculativeOutputsById.end())
			return;
		for (const StringKey& key : it->second)
			m_speculativeCasKeys.erase(key);
		m_speculativeOutputsById.erase(it);
	}
	...
	bool CacheClient::WriteToCache(const RootPaths& rootPaths, u32 bucketId, const ProcessStartInfo& info, const u8* inputs, u64 inputsSize, const u8* outputs, u64 outputsSize, const u8* logLines, u64 logLinesSize, u32 processId)
	{
		...
		// This is here to make sure cache client can use dropped cas entries without making them permanent
		// .. which means that it needs to ref count the cas entries until it is done with them
		Vector<StorageImpl::CasEntry*> refCountedCas;
		auto refCountedCasGuard = MakeGuard([&]()
			{
				for (auto casEntry : refCountedCas)
					m_storage.ReleaseCasEntry(*casEntry, true);
			});
		...
		while (true)
		{
			...
			if (m_useRoots)
			{
				...
				if (!root)
				{
					// Temporarily disabling invalid roots warnings to mitigate a log spam until an issue with them can be fixed
			...
			if (casKey == CasKeyZero)
			{
				...
				if (isOutput)
				{
					StorageImpl::CasEntry* casEntry = nullptr;
					if (!m_storage.StoreCasFile(casKey, path.data, CasKeyZero, deferCreation, &casEntry))
						return m_logger.Info(TC("Failed to store output file %s when writing to cache"), path.data).ToFalse();
					if (casEntry)
						refCountedCas.push_back(casEntry);
				}
				else
				{
		...
		if (!SendCacheEntry(tws, bucket, stats, rootPaths, cmdKey, inputsStringToCasKey, outputsStringToCasKey, logLines, logLinesSize, info))
			return false;
	...
	bool CacheClient::FetchFromCache(CacheResult& outResult, RootsHandle rootsHandle, u32 bucketId, const ProcessStartInfo& info)
	{
		...
		if (!outResult.hit)
			return false;
		return FetchFilesFromCache(outResult, context, false);
	}
	...
	bool CacheClient::FetchEntryFromCache(CacheResult& outResult, FetchContext& context, u32 bucketId)
	{
...
#if UBA_CACHE_USE_SEND_MAX_BUFFER_CACHE
		u8* memory = m_sendBufferSlots.Pop();
		auto g = MakeGuard([this, memory]() { m_sendBufferSlots.Push(memory); });
#else
		...
		auto IsCasKeyMatch = [&, normalizedCasKeys = UnorderedMap<StringKey, CasKey>(), isCasKeyMatchCache = UnorderedMap<u32, bool>()](bool& outIsMatch, u32 casKeyOffset, u32 entryIndex, bool useLookup) mutable
			{
				...
				else
				{
					...
					{
						{
							SCOPED_FUTEX_READ(m_speculativeCasKeysLock, slock);
							auto speculativeIt = m_speculativeCasKeys.find(fileNameKey);
							if (speculativeIt != m_speculativeCasKeys.end())
								localCasKey = speculativeIt->second;
						}
						if (localCasKey == CasKeyZero)
						{
		...
		// Read entries
		{
			...
			for (; entryIndex!=entryCount; ++entryIndex)
			{
				...
				// Make sure caskey/path tables has enough data for outputs
	...
	bool CacheClient::FetchFilesFromCache(CacheResult& outResult, FetchContext& context, bool writePlaceholders)
	{
		if (context.casKeyOffsets.empty())
			return true;
		...
		m_storage.m_workManager.ParallelFor(u32(context.casKeyOffsets.size() - 1), context.casKeyOffsets, [&](const WorkContext&, auto& it)
			{
				...
				if (!FetchFile(*context.bucket, context.rootPaths, context.info, scope.cacheStats, scope.storageStats, *it, writePlaceholders))
					fetchSuccess = false;
	...
	bool CacheClient::FetchFile(Bucket& bucket, const RootPaths& rootPaths, const ProcessStartInfo& info, CacheFetchStats& cacheStats, StorageStats& storageStats, u32 casKeyOffset, bool writePlaceholders)
	{
		StringBuffer<MaxPath> path;
		CasKey casKey;
		...
		{
			UBA_ASSERT(info.rules);
			if (info.rules->SupportsPlaceholder(path))
			{
				FileAccessor destinationFile(m_logger, path.data);
				return destinationFile.CreateWrite() && destinationFile.Close();
			}
		}
		...
		if (IsNormalized(casKey))
		{
			...
			MemoryBlock normalizedBlock(TC("FetchFileNormalized"), 4*1024*1024);
			...
			MemoryBlock localBlock(TC("FetchFileDenormalize"), 4*1024*1024);
			...
			if (useFileMapping)
			{
				...
				FileMapping_CopyMem(destFile.GetData(), localBlock.memory, localBlock.writtenSize);
			}
			else
			{
		...
		else
		{
			...
			#if 0
			{
				StringKey fileNameKey = ToStringKeyLower(path);
				CasKey existingCasKey;
				if (m_storage.CalculateCasKey(existingCasKey, fileNameKey, path.data))
					if (existingCasKey == casKey)
						return true;
			}
			#endif
			...
			if (fetcher.sizeOnDisk == 0 && info.rules->SupportsPlaceholder(path))
			{
				DeleteFileW(path.data);
				return logger.Info(TC("Fetched file that seems to be placeholder, this should never happen %s for %s"), path.data, info.GetDescription()).ToFalse();
			}
		}
	...
	bool CacheClient::ExecuteCommand(Logger& logger, const tchar* command, const tchar* destinationFile, const tchar* additionalInfo)
	{
		...
		{
			MemoryBlock block(TC("ExecuteCommand"), 512*1024*1024);
	...
	bool CacheClient::SendPathTable(Bucket& bucket, CacheSendStats& stats, u32 requiredPathTableSize)
	{
		...
		while (left)
		{
			...
			UBA_CACHE_BINARY_WRITER_MAX_SIZE(writer);
	...
	bool CacheClient::SendCacheEntry(TrackWorkScope& tws, Bucket& bucket, CacheSendStats& stats, const RootPaths& rootPaths, const CasKey& cmdKey, const Map<u32, u32>& inputsStringToCasKey, const Map<u32, u32>& outputsStringToCasKey, const u8* logLines, u64 logLinesSize, const ProcessStartInfo& info)
	{
		StackBinaryReader<24*1024> reader;
		...
		while (reader.GetLeft())
		{
			...
			if (m_storage.HasCasFile(casKey, &casEntry, m_useMappingForCreatedCas, false))
			{
				createCasTs.Leave();

				auto refCountGuard = MakeGuard([&]()
					{
						m_storage.ReleaseCasEntry(*casEntry, true); // HasCasFile increase refcount
					});
				...
				auto mh = casEntry->memoryHandle;
				if (mh.IsValid()) // If file was created by helper it will be in the transient mapped memory
				{
					u64 memoryOffset = casEntry->memoryOffset;
					u64 memorySize = casEntry->memorySize;

					mappedView = m_storage.m_casDataBuffer.MapView(mh, memoryOffset, memorySize, path.data);
				...
				{
					UBA_ASSERT(info.rules);
					if (info.rules->SupportsPlaceholder(path)) // TODO: This should also check if placeholder writing is enabled but not available
						return m_logger.Warning(TC("File %s has size 0 and supports placeholders.. so it is likely we are sending a placeholder file. Investigate!!"), path.data);
				}

				// TODO: Use WorkManager for this!.. flush them in the end
			...
			else // If we don't have the cas key it should be one of the normalized files.... otherwise there is a bug
			{
				...
				if (!file.OpenMemoryRead())
					return false;
				MemoryBlock block(TC("SendNormalizedFile"), AlignUp(file.GetSize() + 16, PageSize));
	...
	bool CacheClient::FetchCompactTable(u32 bucketId, TableType& table, u32 requiredTableSize, u8 messageType)
	{
		...
		struct Entry
		{
			Entry(u8* slot, u32 i, u32 messageMaxSize) : reader(slot + i * messageMaxSize, 0, SendMaxSize), done(EventResetType_Manual) {}
		...
		for (u32 i=0; i!=messageCount; ++i)
		{
			...
			if (e.message.SendAsync(e.reader, [](bool error, void* userData) { ((EventSlim*)userData)->Set(); }, &e.done))
				continue;
			entries.pop_back();
			success = false;
			break;
	...
	bool CacheClient::ReportUsedEntry(Vector<ProcessLogLine>& outLogLines, bool ownedLogLines, Bucket& bucket, const CasKey& cmdKey, u32 entryId)
	{
		...
		UBA_CACHE_BINARY_READER_MAX_SIZE(reader);
		if (!msg.Send(reader))
			return false;
	...
	CasKey CacheClient::GetCmdKey(const RootPaths& rootPaths, const ProcessStartInfo& info, bool report, u32 bucketId)
	{
		...
		if (m_useRoots)
		{
			...
			hasher.Update(info.arguments, TStrlen(info.arguments)*sizeof(tchar));
	...
	void CacheClient::PreparseDirectory(const StringKey& fileNameKey, const StringBufferBase& filePath)
	{
		...
		#if PLATFORM_WINDOWS
		SCOPED_FUTEX_READ(m_directoryPreparserLock, preparserLockRead);
		auto findIt = m_directoryPreparser.find(pathKey);
		bool found = findIt != m_directoryPreparser.end();
		preparserLockRead.Leave();
		if (found)
			dir = &findIt->second;
		else
		#endif
