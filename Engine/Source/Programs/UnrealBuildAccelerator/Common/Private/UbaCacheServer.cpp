...
namespace uba
{
	...
	bool CacheServer::RunMaintenanceInternal(const Function<bool()>& shouldExit, bool allowSave)
	{
		...
		struct CasFileInfo { CasFileInfo(u32 s = 0) : size(s) {} u32 size; Atomic<bool> isUsed = false; }; // These are compressed cas, should never be over 4gb

		MemoryBlock existingCasMemoryBlock(TC("ExistingCasMemoryBlock"));
		...
		Atomic<u64> totalEntryCount = 0;
		Atomic<u64> deleteEntryCount = 0;
		Atomic<u64> expiredEntryCount = 0;
		Atomic<u64> overflowedEntryCount = 0;
		Atomic<u64> missingOutputEntryCount = 0;
		Atomic<u64> missingInputEntryCount = 0;

		Atomic<u64> activeDropCount = 0;
		...
		do
		{
			...
			m_server.ParallelFor(workerCountToUseForBuckets, sortedBuckets, [&](const WorkContext&, auto& it)
			{
				...
				auto context = bucket.m_maintenanceContext;
				if (!context)
					context = bucket.m_maintenanceContext = new Bucket::MaintenanceContext(TC("MaintenanceContext"));
			...
			}, TCV("ParallelForBucket1"), WorkPriority_High);
		...
		Event saveCasEvent(EventResetType_Manual);
		Event saveDbEvent(EventResetType_Manual);
		if (shouldSave)
		{
		...
		m_server.ParallelFor(workerCountToUseForBuckets, sortedBuckets, [&](const WorkContext& wc, auto& it)
		{
			...
			if (oldToNewCasKeyOffset.Size() > 0)
			{
				...
				while (memLeft)
				{
					...
					OodleLZ_Compress(compressor, mem, (OO_SINTa)toCompress, buffer, compressionLevel);
					memLeft -= toCompress;
				}
				#endif
		...
		}, TCV("ParallelForBucket2"), WorkPriority_High);
	...
	UBA_NOINLINE bool CacheServer::HandleMessage(const ConnectionInfo& connectionInfo, u8 messageType, BinaryReader& reader, BinaryWriter& writer)
	{
		if (messageType != CacheMessageType_Connect && m_isRunningMaintenance)
			return m_logger.Debug(TC("Can't handle network message %s while running maintenance mode"), ToString(CacheMessageType(messageType)).data).ToFalse();
	...
	bool CacheServer::HandleStoreEntry(ConnectionBucket& connectionBucket, BinaryReader& reader, BinaryWriter& writer, u32 clientVersion, u32 clientId)
	{
		...
		else
		{
			...
			auto& deferredEntry = connectionBucket.deferredCacheEntryLookup[cmdKey];
			if (deferredEntry.id != 0)
				return m_logger.Warning(TC("Client %u %s adding deferred entry that already exists"), clientId);
			deferredEntry = std::move(newEntry);
		}
	...
	bool CacheServer::HandleExecuteCommand(BinaryReader& reader, BinaryWriter& writer)
	{
		...
		{
			HashMap2<u32, u32> oldToNewCasKeyOffset;
			MemoryBlock memory(TC("UpdateEntries"), 64*1024);
			oldToNewCasKeyOffset.Init(memory, 1);
			Vector<u32> temp;
			Vector<u8> temp2;
			Vector<u8> temp3;
