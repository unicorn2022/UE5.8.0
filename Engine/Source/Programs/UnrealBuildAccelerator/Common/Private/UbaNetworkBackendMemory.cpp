...
namespace uba
{
	...
	bool NetworkBackendMemory::Send(Logger& logger, void* connection, const void* data, u32 dataSize, SendContext& sendContext, const tchar* sendHint)
	{
		...
		if (auto c = m_connection->peer[from].dataSentCallback)
			c(m_connection->peer[from].dataSentContext, dataSize);
		if (auto c = m_connection->peer[to].dataRecvCallback)
			c(m_connection->peer[to].dataRecvContext, dataSize);
	...
	void NetworkBackendMemory::SetDataSentCallback(void* connection, void* context, DataSentCallback* callback)
	{
		UBA_ASSERT(m_connection);
		u64 from = ((uintptr_t)connection) - 1;
		m_connection->peer[from].dataSentCallback = callback;
		m_connection->peer[from].dataSentContext = context;
	}

	void NetworkBackendMemory::SetDataReceivedCallback(void* connection, void* context, DataReceivedCallback* callback)
	{
		UBA_ASSERT(m_connection);
		u64 from = ((uintptr_t)connection) - 1;
		m_connection->peer[from].dataRecvCallback = callback;
		m_connection->peer[from].dataRecvContext = context;
	}
	...
	void NetworkBackendMemory::SetThreadedSend(void* connection)
	{
	}
