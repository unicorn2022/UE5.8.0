...
namespace uba
{
	void NetworkServerCreateInfo::Apply(Config& config, const tchar* tableName)
	{
		...
		table.GetValueAsU32(receiveTimeoutSeconds, TC("ReceiveTimeoutSeconds"));
		...
		table.GetValueAsBool(useThreadedSend, TC("UseThreadedSend"));
	}
	...
	public:
		...
		u32 storageVersion = 0;
		Atomic<u32> refCount = 0;
		Atomic<u32> connectionCount = 0;
		Atomic<u64> sendBytes = 0;
		Atomic<u64> recvBytes = 0;
	};

	struct NetworkServer::WorkerContext
	{
		WorkerContext(NetworkServer& s) : server(s), workAvailable(EventResetType_Auto)
		{
	...
	{
	public:
		static bool DefaultRecvTimeout(void* context, u32 timeoutMs, const tchar* recvHint, const tchar* hint)
		{
		...
		,	m_disconnectCallbackCalled(EventResetType_Manual)
		,	m_id(id)
		,	m_backendConnection(backendConnection)
		{
			...
			// We expect fast response on these messages. If we don't, then something is wrong
			// This is on a new socket so there should be no contention for networking here.
			// No message communication can happen until it has gone through these first steps
			u32 initialTimeoutMs = Min(10u*1000, m_server.m_receiveTimeoutMs);
			m_backend.SetRecvTimeout(m_backendConnection, initialTimeoutMs, this, DefaultRecvTimeout);
		...
		bool EndStop()
		{
			...
			m_backend.SetDisconnectCallback(m_backendConnection, nullptr, [](void* context, const Guid& connectionUid, void* connection) {});
			m_backend.SetRecvCallbacks(m_backendConnection, nullptr, 0, [](void*, const Guid&, u8*, void*&, u8*&, u32&) { return false; }, nullptr, TC("Stopped"));
			m_server.m_logger.Info(TC("Disconnect callback event timed out. This should never happen!!"));
			#if UBA_DEBUG
			PrintAllCallstacks(m_server.m_logger);
			#endif
			return true;
		...
		static bool ReceiveVersion(void* context, const Guid& connectionUid, u8* headerData, void*& outBodyContext, u8*& outBodyData, u32& outBodySize)
		{
			...
			// Ok, we got a version, meaning that there is someone on the other side of the connection. Set to the actual timeout.. 10 minutes or so
			conn.m_backend.SetRecvTimeout(conn.m_backendConnection, conn.m_server.m_receiveTimeoutMs, &conn, DefaultRecvTimeout);
		...
		static bool KeepAliveRecvTimeout(void* context, u32 timeoutMs, const tchar* recvHint, const tchar* hint)
		{
			auto& conn = *(Connection*)context;
			++conn.m_recvTimeoutCount;
			if (!conn.SendKeepAlive())
				return false;
			conn.m_backend.SetRecvTimeout(conn.m_backendConnection, KeepAliveIntervalSeconds*1000, context, KeepAliveRecvTimeout);
		...
		static bool ReceiveClientUid(void* context, const Guid& connectionUid, u8* headerData, void*& outBodyContext, u8*& outBodyData, u32& outBodySize)
		{
			...
			if (server.m_useKeepAlive)
				conn.m_backend.SetRecvTimeout(conn.m_backendConnection, KeepAliveIdleSeconds*1000, context, KeepAliveRecvTimeout);
			...
			if (oldCount == 0)
			{
				...
				if (server.m_logConnections)
					server.m_logger.Detail(TC("Client %u (%s) connected on connection %u (%s)"), clientId, GuidToString(clientUid).str, conn.m_id, GuidToString(connectionUid).str);
			}
			else
			{
				if (server.m_logConnections)
					server.m_logger.Detail(TC("Client %u (%s) additional connection %u (%s) connected"), clientId, GuidToString(clientUid).str, conn.m_id, GuidToString(connectionUid).str);
		...
		static bool ReceiveMessageHeader(void* context, const Guid& connectionUid, u8* headerData, void*& outBodyContext, u8*& outBodyData, u32& outBodySize)
		{
			...
			if (messageSize > SendMaxSize)
				return conn.m_server.m_logger.Warning(TC("Client %u Got message size %u which is larger than max %u. Protocol error? (serviceId %u, messageType %u, messageId %u)"), conn.m_client->id, messageSize, SendMaxSize, u32(serviceId), u32(messageType), u32(messageId));
			if (serviceId >= sizeof(NetworkServer::m_workerFunctions))
				return conn.m_server.m_logger.Warning(TC("Client %u Got message with service id %u which is out of range. Protocol error?"), conn.m_client->id, serviceId);

			if (conn.m_recvTimeoutCount) // This can only be set if keepalive is enabled.
			{
				conn.m_recvTimeoutCount = 0; // Getting any message means we reset the timeout counter and time.
				conn.m_backend.SetRecvTimeout(conn.m_backendConnection, KeepAliveIdleSeconds*1000, context, KeepAliveRecvTimeout);
			}
		...
		static bool ReceiveMessageBody(void* context, bool recvError, u8* headerData, void* bodyContext, u8* bodyData, u32 bodySize)
		{
			...
			// If the connection is being shut down, don't dispatch new work. A client can
			// drop connection at any time, so the local lifecycle must already tolerate
			// un-drained bytes. Recycling the worker (instead of bumping m_activeWorkerCount)
			// lets EndStop() unblock as soon as already-running workers finish, rather than
			// waiting on every late message — typically SendFileToServer pushes for processes
			// we just cancelled.
			if (conn.m_shouldDisconnect)
			{
				conn.m_server.PushWorker(worker);
				return true;
			}
		...
		Atomic<int> m_activeWorkerCount = 1;
		Atomic<int> m_disconnectCalled = 0;
		Atomic<bool> m_disconnected = false;
		u32 m_id = 0;
	...
	void ConnectionInfo::SetStorageVersion(u32 version) const
	{
		auto& conn = *(NetworkServer::Connection*)internalData;
		conn.m_client->storageVersion = version;
	}

	u32 ConnectionInfo::GetStorageVersion() const
	{
		auto& conn = *(NetworkServer::Connection*)internalData;
		return conn.m_client->storageVersion;
	}
	...
	{
		auto& additionalWorkQueue = server.m_additionalWorkQueue;
		...
		while (true)
		{
			while (true)
			{
				Work* work = additionalWorkQueue.Dequeue(ticket);
				if (!work)
					break;
				...
				additionalWorkQueue.Finish(work, ticket);
			}
			...
			SCOPED_FUTEX_READ(additionalWorkQueue.lock, lock2);
			...
			if (!additionalWorkQueue.IsEmptyNoLock())
				continue;
	...
	:	m_logger(info.logWriter, name)
	{
		...
		m_useThreadedSend = info.useThreadedSend;
	...
	void NetworkServer::DisconnectClients()
	{
...

		{
			...
			for (auto& c : connections)
				success = c.BeginStop() && success;

			for (auto& c : connections)
			{
				success = c.EndStop() && success;
	...
	void NetworkServer::AddWork(WorkFunction&& work, u32 count, const tchar* desc, const Color& color, WorkPriority priority, Bottleneck* bottleneck)
	{
		m_additionalWorkQueue.Enqueue(std::move(work), count, desc, color, priority, bottleneck, m_workTracker != nullptr);
	...
	bool NetworkServer::FlushWork(u32 timeoutMs)
	{
		...
		while (true)
		{
			bool noAdditionalWork = m_additionalWorkQueue.IsEmpty();
	...
	bool NetworkServer::DoAdditionalWork()
	{
		...
		Work* work = m_additionalWorkQueue.Dequeue(ticket);
		if (work)
		{
			#if UBA_TRACK_WORK
			...
			m_additionalWorkQueue.Finish(work, ticket);
			return true;
		}
	...
	NetworkServer::Worker* NetworkServer::PopWorker()
	{
		while (true)
		{
			...
			req.ev.Create(EventResetType_Manual);
	...
	bool NetworkServer::AddConnection(NetworkBackend& backend, void* backendConnection, const sockaddr& remoteSocketAddr, bool requiresCrypto, CryptoKey cryptoKey)
	{
		...
		if (m_useThreadedSend)
			backend.SetThreadedSend(backendConnection);
