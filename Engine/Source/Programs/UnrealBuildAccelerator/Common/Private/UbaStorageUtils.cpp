...
namespace uba
{
	...
	CasKey CalculateCasKey(const u8* fileMem, u64 fileSize, bool storeCompressed, WorkManager* workManager, const tchar* hint)
	{
		CasKeyHasher hasher;
		...
		if (fileSize > BufferSlotSize) // Note that when filesize is larger than BufferSlotSize the hash becomes a hash of hashes
		{
			struct WorkRec
			{
				Atomic<u64> refCount = 2;
				Atomic<u64> counter = 0;
				Atomic<u64> doneCounter = 0;
				const u8* fileMem = nullptr;
				u64 workCount = 0;
				u64 fileSize = 0;
				bool error = false;
				Vector<CasKey> keys;
			...
			rec->done.Create(EventResetType_Manual);
			...
			auto work = [rec](const WorkContext& context)
			{
				while (true)
				{
					...
					const u8* slot = rec->fileMem + startOffset;
					CasKeyHasher hasher;
			...
			if (workManager)
			{
				...
				workManager->AddWork(WorkFunction(work), workerCount, TC("CalculateKey"));
			}
	...
	bool SendBatchMessages(Logger& logger, NetworkClient& client, u16 fetchId, u8* slot, u64 capacity, u64 left, u32 messageMaxSize, u32& readIndex, u32& responseSize, const Function<bool()>& runInWaitFunc, const tchar* hint, u32* outError, const Function<bool(u64)>& progressFunc)
	{
		...
		for (u32 i=0; i!=sendCount; ++i)
		{
			...
			if (entry.message.SendAsync(entry.reader, [](bool error, void* userData) { ((EventSlim*)userData)->Set(); }, &entry.done))
				continue;
	...
	bool SendFile(Logger& logger, NetworkClient& client, WorkManager* workManager, const CasKey& casKey, const u8* sourceMem, u64 sourceSize, const tchar* hint)
	{
		...
		{
			IndexContainer container(sendCount);
	...
	bool FileSender::SendFileCompressed(const CasKey& casKey, const tchar* fileName, const u8* sourceMem, u64 sourceSize, const tchar* hint)
	{
		...
		MemoryBlock memoryBlock(TC("FileSender"), sourceSize + messageHeaderMaxSize + 1024);
		{
		...
		{
			u64 writerStartOffset = messageHeader + firstMessageOverHead;
			BinaryWriter writer(readData - writerStartOffset, 0, client.GetMessageMaxSize());
			NetworkMessage msg(client, StorageServiceId, StorageMessageType_StoreBegin, writer);
			writer.WriteCasKey(casKey);
			...
			if (sendLeft != 0 && m_sendOneBigFileAtTheTime)
			{
		...
		if (sendLeft != 0)
		{
			...
			if (storeId == u16(~0)) // File already exists on server
				return sendEndMessage(); //m_logger.Info(TC("Server already has file %s (%s)"), CasKeyString(casKey).str, hint);

			u64 writerHeaderSize = messageHeader + (sizeof(u16) + sizeof(u64));
			u64 writerCapacity = client.GetMessageMaxSize() - writerHeaderSize;
			u64 sendCount = (sendLeft + writerCapacity - 1) / writerCapacity;

			auto SendSegment = [&](u64 index)
				{
					u64 segmentPos = sendPos + writerCapacity*index;
					u8* writerStart = readData + segmentPos - writerHeaderSize;
					u8 prevData[32];
					memcpy(prevData, writerStart, writerHeaderSize);

					BinaryWriter writer(writerStart, 0, client.GetMessageMaxSize());
					NetworkMessage msg(client, StorageServiceId, StorageMessageType_StoreSegment, writer);
					writer.WriteU16(storeId);
					writer.WriteU64(segmentPos);
					u64 toWrite = Min(fileSize - segmentPos, writerCapacity);
					writer.AllocWrite(toWrite);
					bool res = msg.Send();
					memcpy(writerStart, prevData, writerHeaderSize);
					return res;
				};

			// If we have more than one connection we can use parallel for to send to multiple connections at the same time.
			u32 connectionCount = m_parallelSend && sendCount > 1 ? client.GetConnectionCount() : 1;
			if (connectionCount > 1)
			{
				// We have to leapfrog every other message since the SendSegment modifies the memory just before the sent segment
				for (u32 i=0;i!=2; ++i)
				{
					Atomic<bool> error;
					IndexContainer container((sendCount+1)/2);
					m_client.ParallelFor(connectionCount - 1, container, [&](const WorkContext&, auto& it)
					{
						u32 index = it.index*2+i;
						if (index < sendCount)
							if (!SendSegment(index))
								error = true;
					}, TCV("SendCompressedFile"));
					if (error)
						return false;
				}
			}
	...
	bool FileFetcher::RetrieveFile(Logger& logger, NetworkClient& client, const CasKey& casKey, const tchar* destination, bool writeCompressed, MemoryBlock* destinationMem, u32 attributes)
	{
		...
		bool useFileMapping = sizeOnDisk > 0;
		...
		auto ReportBadCas = [&]()
			{
				StackBinaryWriter<1024> writer;
				NetworkMessage msg(client, StorageServiceId, StorageMessageType_ReportBadCas, writer);
				writer.WriteCasKey(casKey);
				writer.WriteString(destination);
				StackBinaryReader<8> reader;
				msg.Send(reader);
			};
		...
		auto WriteDestination = [&](const void* source, u64 sourceSize)
			{
				if (fileMappingMem)
				{
					...
					FileMapping_CopyMem(fileMappingMem + destOffset, source, sourceSize);
	...
	CompressWriter::CompressWriter(Logger& l, SharedMemoryAllocator& a, BufferCache& bs, WorkManager& wm, StorageStats& s, u8 cc, u8 ccl, bool avof)
	:	m_logger(l)
	,	m_allocator(a)
	,	m_bufferSlots(bs)
	,	m_workManager(wm)
	...
	bool CompressWriter::CompressToMapping(SharedMemoryHandle& outMappingHandle, u64& outMappingSize, u8* readMem, u64 fileSize, const tchar* hint)
	{
		u64 mappingSize = fileSize + 1024;

		SharedMemoryHandle destMapping = m_allocator.CreateHandle(m_logger, hint);
		m_allocator.ExtendMemory(destMapping, mappingSize, hint, false);
		auto destGuard = MakeGuard([&]() { m_allocator.CloseHandle(m_logger, destMapping, hint); });
		u8* compressedData = m_allocator.MapView(destMapping, hint, SharedMemoryMapType_ReadWrite);
		if (!compressedData)
			return false;
		auto unmapGuard = MakeGuard([&]() { m_allocator.UnmapView(destMapping, compressedData); });
		...
		u64 written = destination.GetWritten();
		m_allocator.TrimMemory(destMapping, written);

		outMappingHandle = destMapping;
		outMappingSize = written;
		return true;
	}

	bool CompressWriter::CompressToMappingFromFile(SharedMemoryHandle& outMappingHandle, u64& outMappingSize, FileHandle readHandle, u64 fileSize, const tchar* hint)
	{
		u64 mappingSize = fileSize + 1024;
		...
		LinearWriterMem destination(compressedData, mappingSize);
		if (!CompressFromMemOrFile(destination, hint, readHandle, nullptr, fileSize))
			return false;
	...
	bool CompressWriter::CompressFromMem(LinearWriter& destination, u32 workCount, const u8* uncompressedData, u64 fileSize, u64 maxUncompressedBlock, u64& totalWritten)
	{
		...
		struct WorkRec
		{
			...
			WorkRec(u32 wc)
			{
				...
				#if PLATFORM_WINDOWS
				events = new EventSlim[workCount];
				#else
				events = (EventSlim*)aligned_alloc(alignof(EventSlim), sizeof(EventSlim)*workCount);
				for (auto i=0;i!=workCount; ++i)
					new (events + i) EventSlim();
				#endif
			}
			~WorkRec()
			{
			...
			BufferCache* bufferSlots;
			Atomic<u64> refCount;
			Atomic<u64> compressCounter = 0;
			EventSlim* events;
		...
		for (u32 i=0; i!=workCount; ++i)
			rec.events[i].Create(EventResetType_Manual);
		...
		m_workManager.AddWork(WorkFunction(work), workerCount-1, TC("Compress")); // We are a worker ourselves
		{
			TrackWorkScope tws;
			work({tws});
		}
	...
	bool CompressWriter::CompressFromMemOrFile(LinearWriter& destination, const tchar* from, FileHandle readHandle, u8* readMem, u64 fileSize)
	{
		...
		if (workCount > 1)
		{
			if (!readMem)
			{
				FileMappingHandle fileMapping = FileMapping_CreateFromFile(m_logger, readHandle, PAGE_READONLY, fileSize, from);
				if (!fileMapping.IsValid())
					return m_logger.Error(TC("Failed to create file mapping for %s (%s)"), from, LastErrorToText().data);

				auto fmg = MakeGuard([&]() { FileMapping_Close(m_logger, fileMapping, from); });
				u8* uncompressedData = FileMapping_Map(m_logger, fileMapping, FILE_MAP_READ, 0, fileSize);
				if (!uncompressedData)
				...
				auto udg = MakeGuard([&]()
					{
						if (m_asyncUnmapViewOfFile)
							m_workManager.AddWork([uncompressedData, fileSize, f = TString(from), loggerPtr = &m_logger](const WorkContext&) { FileMapping_Unmap(*loggerPtr, uncompressedData, fileSize, f.c_str()); }, 1, TC("UnmapFile"));
						else
							FileMapping_Unmap(m_logger, uncompressedData, fileSize, from);
					});
	...
	bool DecompressWriter::DecompressMemoryToMemory(const u8* compressedData, u64 compressedSize, u8* writeData, u64 decompressedSize, const tchar* readHint, const tchar* writeHint)
	{
		...
		if (decompressedSize > BufferSlotSize * 4) // Arbitrary size threshold. We want to at least catch the pch here
		{
			...
			workerCount = Min(workerCount, MaxWorkItemsPerAction2); // Cap this to not starve other things
			rec->refCount += workerCount;
			m_workManager.AddWork(WorkFunction(work), workerCount, TC("DecompressMemToMem"));
	...
	bool DecompressWriter::DecompressFileToMemory(const tchar* fileName, FileHandle fileHandle, u8* dest, u64 decompressedSize, const tchar* writeHint, u64 fileStartOffset)
	{
		if (decompressedSize > BufferSlotSize*4) // Arbitrary size threshold. We want to at least catch the pch here
		{
			...
			FileMappingHandle fileMapping = FileMapping_CreateFromFile(m_logger, fileHandle, PAGE_READONLY, compressedSize, fileName);
			if (!fileMapping.IsValid())
			...
			u8* fileData = FileMapping_Map(m_logger, fileMapping, FILE_MAP_READ, 0, compressedSize);
			if (!fileData)
				return m_logger.Error(TC("Failed to map view of file mapping for %s (%s)"), fileName, LastErrorToText().data);
			auto udg = MakeGuard([&]() { FileMapping_Unmap(m_logger, fileData, compressedSize, fileName); });
