...
namespace uba
{
	...
	{
		auto& header = *(Elf64FileHeader*)m_data;
	...
	{
		u8* data = (u8*)memoryBlock.Allocate(sizeof(Elf64FileHeader) + sizeof(Elf64SectionHeader) + 1, 1, TC(""));
		auto& header = *(Elf64FileHeader*)data;
		...
		header.e_version = 1;
		header.e_ehsize = sizeof(Elf64FileHeader);
		header.e_flags = 0;//0x04000000;
		header.e_shoff = sizeof(Elf64FileHeader);
		header.e_shentsize = sizeof(Elf64SectionHeader);
		header.e_shnum = 1;
		header.e_shstrndx = 0;

		auto& section = *(Elf64SectionHeader*)(data + sizeof(Elf64FileHeader));
		section.sh_name = 0;
		section.sh_type = 3;
		section.sh_flags = 0;//0xC35d00;
		section.sh_addralign = 1;
		section.sh_offset = sizeof(Elf64FileHeader) + sizeof(Elf64SectionHeader);
		section.sh_size = 1;
		return true;
	}
}
