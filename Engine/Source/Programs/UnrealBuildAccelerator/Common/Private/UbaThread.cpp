...
namespace uba
{
	...
	void Thread::Start(Function<u32()>&& f, const tchar* description)
	{
		...
		m_finished.Create(EventResetType_Manual);
		...
		err = pthread_create(&pth, &tattr, [](void* p) -> void*
			{
				...
				#if UBA_DISABLE_PTHREAD_CANCELLATIONS
				int old;
	...
	bool TraverseAllThreads(const TraverseThreadFunc& func, const TraverseThreadErrorFunc& errorFunc)
	{
		...
		static Event s_ev(EventResetType_Auto);
		...
		for (int threadIt = 0; threadIt < thread_count; threadIt++)
		{
			auto thread = threads[threadIt];
