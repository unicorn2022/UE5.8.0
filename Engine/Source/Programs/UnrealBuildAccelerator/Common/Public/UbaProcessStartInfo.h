...
namespace uba
{
	...
	struct ProcessStartInfo
	{
		...
		bool expanded = false;					// Internal: set to true by ProcessStartInfoHolder::Expand so subsequent Expand calls are no-ops. Lives on the base struct so it survives slicing via ProcessStartInfoHolder's conversion constructor (which copies the base via *(ProcessStartInfo*)this = si).
		...
		const tchar* overlayFiles = TC("");   // Semi colon separated string containing paths to overlay files (files that are only visible to this process)
