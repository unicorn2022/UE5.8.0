...
namespace uba
{
	...
	public:
		...
		// Callbacks to be able to track stats
		using DataSentCallback = void(void* context, u32 bytes);
		using DataReceivedCallback = void(void* context, u32 bytes);
		virtual void SetDataSentCallback(void* connection, void* context, DataSentCallback* callback) {}
		virtual void SetDataReceivedCallback(void* connection, void* context, DataReceivedCallback* callback) {}
		virtual void TraverseConnections(const Function<void(void*)>& func) {}

		// Change so send is handled in a separate thread instead of directly on calling thread
		virtual void SetThreadedSend(void* connection) = 0;
