...
namespace uba
{
	...
	public:
		...
		ProcessHandle RunProcessRemote(const ProcessStartInfo& startInfo, float weight = 1.0f, const void* knownInputs = nullptr, u32 knownInputsCount = 0, bool canCrossArchitecture = false, bool canCrossPlatform = false);
		...
		using ProcessToRaceCallback = Function<bool(ProcessHandle&, ProcessStartInfo&)>;
		ProcessHandle RunProcessRacing(const ProcessToRaceCallback& callback);
		...
		bool DisableRemoteExecutionOnAgent(const tchar* agentName);
		...
		using RemoteProcessAvailableCallback = Function<void(bool isCrossArchitecture, bool isCrossPlatform)>;
		void SetRemoteProcessSlotAvailableEvent(const RemoteProcessAvailableCallback& remoteProcessSlotAvailableEvent);
		...
		bool RegisterCrossArchitectureMapping(const tchar* from, const tchar* to);
	...
	protected:
		...
		struct ReceivedFile;
		bool WriteReceivedFile(ReceivedFile& file, RemoteProcess* process, const ConnectionInfo& connectionInfo);
		bool WriteReceivedFiles(Vector<ReceivedFile>& receivedFiles, RemoteProcess& process, Timer& writeFilesTimer, const ConnectionInfo& connectionInfo);
		void SkipReceivedFiles(Vector<ReceivedFile>& receivedFiles, const ConnectionInfo& connectionInfo);
		void ReleaseReceivedFiles(Vector<ReceivedFile>& receivedFiles);
		...
		u32 m_uiLanguage;
		Atomic<u32> m_maxRemoteProcessCount = ~0u;
		bool m_resetCas;
		...
		RemoteProcessAvailableCallback m_remoteProcessSlotAvailableEvent;
		...
		Futex m_binKeysLock;
		CasKey m_detoursBinaryKey[2][2]; // [platform][isarm]
		CasKey m_agentBinaryKey[2]; // [isarm]

		struct ClientSession
		{
			TString name;
			UnorderedSet<CasKey> sentKeys;
			Vector<u32> processesLostRace;
			...
			bool isArm = false;
			bool isLinux = false; // Only used for non-linux
		...
		Atomic<bool> m_nameToHashInitialized = false;
		...
		bool m_customCasKeysEnabled = false;
		bool m_useFillLock = false;
