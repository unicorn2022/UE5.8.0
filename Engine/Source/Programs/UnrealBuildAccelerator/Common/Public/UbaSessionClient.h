...
namespace uba
{
	...
	struct SessionClientCreateInfo : SessionCreateInfo
	{
		...
		bool useKnownLocalBinaries = true; // Will use local binaries like sh, ln, cp etc if available
	...
	public:
		...
		void SetMaxIdleTime(u32 seconds);
		void SetAllowSpawn(bool allow);

		u64 GetBestPing();
		u32 GetMaxProcessCount();
		u32 GetDesiredConnectionCount();

		bool Exists(const StringView& path, u32& outAttributes);

		void SendNotification(StringView text);

	private:
		bool RetrieveCasFile(CasKey& outNewKey, u64& outSize, const CasKey& casKey, StringKey keyHint, const tchar* hint, bool storeUncompressed, bool allowProxy = true);
		...
		virtual Vector<tchar>& GetProcessEnvironmentVariables() override;
		...
		virtual bool GetListDirectoryInfo(ListDirectoryResponse& out, const StringView& dirName, const StringKey& dirKey, DirectoryTableOverlay* overlay) override;
		...
		virtual bool GetNextProcess(Process& process, bool& outNewProcess, bool& outShouldExit, NextProcessInfo& outNextProcess, u32 prevExitCode, BinaryReader& statsReader, u32 timeoutMs) override;
		...
		bool ReadModules(List<ModuleInfo>& outModules, bool& outIsShebang, StringBufferBase& outVirtualApplication, u32 processId, StringView application, StringView workingDir);
		...
		bool ShouldSend(ProcessImpl& process, StringView path);
		bool SendFileSystemOperations(ProcessImpl& process);
		bool SendFiles(ProcessImpl& process);
		...
		bool SendProcessAvailable(Vector<InternalProcessStartInfo>& outStartInfos, Vector<u32>& outCancelled, float availableWeight);
		bool SendGetNextMessage();
		void CancelGetNextMessageQueue();
		bool HandleGetNextResponse(BinaryReader& reader, u32 processId, bool& outNewProcess, bool& outShouldExit, NextProcessInfo& outNextProcess);
		...
		void SendLogEntry(StringView text, LogEntryType type = LogEntryType_Info, bool ack = false);
		...
		bool EntryExists(StringView path, DirTableOffset& outTableOffset);
		void RunSpeedTest(StringView args);
		...
		Atomic<u32> m_maxIdleSeconds = ~0u;
		u32 m_osVersion = 0;
		...
		bool m_useKnownLocalBinaries = true;
		
		Atomic<const tchar*> m_terminationReason = nullptr;
		Atomic<u64> m_terminationTime = 0;
		Atomic<u32> m_maxProcessCount = 0;
		Atomic<u32> m_desiredConnectionCount = 0;
		Atomic<float> m_cpuUsage = 0.0f;

		struct ApplicationEnvironment { Futex lock; TString virtualApplication; TString realApplication; bool isShebang = false; };
		...
		Atomic<bool> m_sendPing = false;
		Atomic<bool> m_allowSpawn = true;
		...
		struct GetNextEntry;
		GetNextEntry* m_firstGetNextEntry = nullptr;
		Futex m_getNextEntryLock;

		Atomic<u64> m_bestPing = 0;
		Atomic<u64> m_lastPing = 0;
		u64 m_lastPingSendTime = 0;
		u32 m_pingTimeoutSecondsPrintCallstacks = 0;

		Atomic<u64> m_memAvail = 0;
		Atomic<u64> m_memTotal = 0;
