...
namespace uba
{
	...
	class NetworkBackendTcp final : public NetworkBackend
	{
	public:
		...
		virtual void SetThreadedSend(void* connection) override;
		...
		virtual void SetDataReceivedCallback(void* connection, void* context, DataReceivedCallback* callback) override;
		virtual void TraverseConnections(const Function<void(void*)>& func) override;
		...
		static bool KillTcpHogs(Logger& logger, u16 port);
	...
	private:
		...
		void ThreadSend(Connection& connection);
		bool HandleSendError(Connection& connection, Logger& logger, const tchar* hint);
		...
		Atomic<u64> m_totalSend = 0;
		Atomic<u64> m_totalRecv = 0;
