...
namespace uba
{
	...
	{
	public:
		inline DependencyCrawler(Logger& logger, WorkManager& workManager, u32 bottleneckMax = ~0u);
		...
		using CreateFileFunc = Function<bool(TrackWorkScope& tracker, const StringView& fileName, const AccessFileFunc& func, bool& stopCrawling)>;

		inline void Init(FileExistsFunc&& fileExistsFunc, TraverseFilesFunc&& traverseFilesFunc, bool useBloomFilter = true);
		inline bool AddRsp(const tchar* rsp, const tchar* workDir, CreateFileFunc&& createFileFunc, DevirtualizePathFunc&& devirtualizePathFunc, const tchar* app, DependencyCrawlerType type, u32 ruleIndex);
		inline bool AddCommandLine(const tchar* commandLine, const tchar* workDir, CreateFileFunc&& createFileFunc, DevirtualizePathFunc&& devirtualizePathFunc, const tchar* app, DependencyCrawlerType type, u32 ruleIndex);
		...
		template<typename CharType> inline bool ParseRsp2(TrackWorkScope& tracker, Instance& instance, const void* data, u64 dataSize, const tchar* rsp, CodeFiles& outCodeFiles);
		...
		inline bool TraverseCodeFiles(const WorkContext& context, Instance& instance, const CodeFiles& codeFiles, u32 ruleIndex);

		inline u32 GetBottleneckMax(WorkManager& workManager, u32 bottleneckMax)
		{
			// We never allow the crawler to use all workers since there could be jobs that do garbage collecting of cas entries 
			u32 max = workManager.GetWorkerCount();
			if (max <= 2)
				return 0;
			return Min(max - 2u, bottleneckMax);
		}
	};

	DependencyCrawler::DependencyCrawler(Logger& logger, WorkManager& workManager, u32 bottleneckMax)
	:	m_logger(logger)
	,	m_workManager(workManager)
	,	m_bottleneck(GetBottleneckMax(workManager, bottleneckMax))
	{
	}

	void DependencyCrawler::Init(FileExistsFunc&& fileExistsFunc, TraverseFilesFunc&& traverseFilesFunc, bool useBloomFilter)
	{
		UBA_ASSERT(m_bottleneck.GetMaxCount() > 0);
		m_fileExistsFunc = std::move(fileExistsFunc);
		m_traverseFilesFunc = std::move(traverseFilesFunc);
		m_useBloomFilter = useBloomFilter;
	}

	bool DependencyCrawler::AddRsp(const tchar* rsp, const tchar* workDir, CreateFileFunc&& createFileFunc, DevirtualizePathFunc&& devirtualizePathFunc, const tchar* app, DependencyCrawlerType type, u32 ruleIndex)
	{
		...
		m_workManager.AddWork([this, ref = InstanceRef(instance), ruleIndex](const WorkContext& context)
			{
				...
				if (ParseRsp(context.tracker, instance, instance.rsp, codeFiles))
					TraverseCodeFiles(context, instance, codeFiles, ruleIndex);
			}, 1, TC("CrawlRsp"), ColorWork, WorkPriority_Low, &m_bottleneck);
		return true;
	}

	bool DependencyCrawler::AddCommandLine(const tchar* commandLine, const tchar* workDir, CreateFileFunc&& createFileFunc, DevirtualizePathFunc&& devirtualizePathFunc, const tchar* app, DependencyCrawlerType type, u32 ruleIndex)
	{
		auto& instance = *new Instance();
		...
		instance.application = app;
		//instance.rsp = rsp;
		instance.platform = TC("Win64"); // Hard coded platform for now
		instance.workDir = workDir;
		UBA_ASSERT(*workDir);
		if (CaseInsensitiveFs)
			ToLower(instance.workDir.data());
		...
		m_workManager.AddWork([this, ref = InstanceRef(instance), ruleIndex, cmdLine = TString(commandLine)](const WorkContext& context)
			{
				auto& instance = ref.instance;
				CodeFiles codeFiles;
				if (ParseRsp2<tchar>(context.tracker, instance, cmdLine.data(), cmdLine.size(), TC(""), codeFiles))
					TraverseCodeFiles(context, instance, codeFiles, ruleIndex);
			}, 1, TC("CrawlCmd"), ColorWork, WorkPriority_Low, &m_bottleneck);
		return true;
	}
	...
	bool DependencyCrawler::ParseRsp(TrackWorkScope& tracker, Instance& instance, const StringView& rsp, CodeFiles& outCodeFiles)
	{
		...
		if (!instance.createFileFunc(tracker, fixedPath, [&](const void* data, u64 dataSize) { return ParseRsp2<char>(tracker, instance, data, dataSize, rsp.data, outCodeFiles); }, stopCrawling))
			return stopCrawling ? false : m_logger.Warning(TC("Failed to parse rsp %s"), rsp.data);
		return true;
	}
	...
	bool DependencyCrawler::ParseRsp2(TrackWorkScope& tracker, Instance& instance, const void* data, u64 dataSize, const tchar* rsp, CodeFiles& outCodeFiles)
	{
		...
		if (instance.type == DependencyCrawlerType_MsvcCompiler)
		{
			...
			bool addRoot = false;
			bool handled = false;
			ParseArguments((const CharType*)data, dataSize, [&](const CharType* arg, u32 argLen)
				{
					StringBuffer<> sb;
					...
					else if (sb[0] == '-')
					{
						...
						else if (sb.StartsWith(TCV("-I"), false)) // clang-cl.exe allows this
						{
							AddRoot(StringView(sb).Skip(2));
						}
						else
							IgnoreOption(sb);
					}
			...
			ParseArguments((const char*)data, dataSize, [&](const char* arg, u32 argLen)
				{
					...
					else
					{
						if (sb.EndsWith(TCV(".obj")))
							AddCodeFile(sb, false, false);
						if (sb.Contains('/') && sb.EndsWith(TCV(".lib")))
							AddCodeFile(sb, false, false);
					}
				});
		}
	...
	{
		bool stopCrawling = false;
		if (!instance.createFileFunc(tracker, codeFile, [&](const void* data, u64 dataSize) { return ParseCodeFile2(tracker, instance, handledFiles, data, dataSize, codeFile, parseDefines); }, stopCrawling))
			return stopCrawling ? false : m_logger.Warning(TC("Failed to parse code file %s found in %s"), codeFile.data, caller.data);
		return true;
	}
	...
	{
		UBA_CRAWLER_DEBUG_PRINT("PARSE", codeFile.data);
		...
		while (it < end)
		{
			if (*it == '#' && !hasNonSpace)
			{
				...
				if (isInclude || isImport)
				{
					...
					{
						isQuote = *it == '\"';
						char endChar = isQuote ? '\"' : '>';
						++it;
						const char* includeStart = it;
						const char* includeEnd = strchr(it, endChar);
						if (!includeEnd)
							break;
						u64 includeLen = includeEnd - includeStart;
						if (includeLen >= include.capacity)
							break;
						it = includeEnd + 1;
						include.Append(includeStart, u32(includeLen));
					}
					else
					{
						...
						if (argBegin)
						{
							u64 argLen = argEnd - argBegin;
							if (argLen < include.capacity - 32)
							{
								if (strncmp(defineBegin, "UE_INLINE_GENERATED_CPP_BY_NAME", defineLen) == 0)
								{
									include.Append(argBegin, u32(argLen)).Append(TCV(".gen.cpp"));
								}
								else if (strncmp(defineBegin, "COMPILED_PLATFORM_HEADER", defineLen) == 0)
								{
									if (!instance.platform.empty())
									{
										...
										include.Append(instance.platform).Append(argBegin, u32(argLen));
									}
								}
							}
						}
					...
					if (!include.IsEmpty())
					{
								...
								if (!instance.frameworksDir.empty())
								{
									if (const tchar* firstSlash = TStrchr(include2.c_str(), '/'))
									{
										...
										for (auto frameworkRoot : frameworkRoots)
										{
											...
											lock.Leave();
											bool stopCrawling = false;
											instance.createFileFunc(context.tracker, tmp, {}, stopCrawling);
											return;
										}
									}
								}
						...
						if (parseDefines)
							...
							m_workManager.AddWork(std::move(work), 1, TC("CrawlIncludes"), ColorWork, WorkPriority_Low, &m_bottleneck);
					}
				}
		...
		m_traverseFilesFunc(rootPathWithSlash, [&](const StringView& file, bool isDirectory)
			{
				...
				if (isDirectory)
				{
					...
					m_workManager.AddWork([this, ref = InstanceRef(instance), codeFile = codeFilePath.ToString()](const WorkContext& context)
						...
						}, 1, TC("CrawlBiDir"), ColorWork, WorkPriority_Low, &m_bottleneck);
				}
				else
				{
					#if PLATFORM_MAC
					...
					m_workManager.AddWork([this, ref = InstanceRef(instance), codeFile = codeFilePath.ToString()](const WorkContext& context)
						{
							bool stopCrawling = false;
							if (!ref.instance.createFileFunc(context.tracker, codeFile, {}, stopCrawling))
								if (!stopCrawling)
									m_logger.Warning(TC("Failed to open file %s from builtindir"), codeFile.c_str());
						}, 1, TC("CrawlBiFile"), ColorWork, WorkPriority_Low, &m_bottleneck);
				}
			});
		return true;
	}

	inline bool DependencyCrawler::TraverseCodeFiles(const WorkContext& context, Instance& instance, const CodeFiles& codeFiles, u32 ruleIndex)
	{
		for (auto& cf : codeFiles)
		{
			...
			else if (instance.type == DependencyCrawlerType_MsvcLinker)
			{
				...
				m_workManager.AddWork([this, ref = InstanceRef(instance), codeFile = cf.path](const WorkContext& context)
					{
						bool stopCrawling = false;
						ref.instance.createFileFunc(context.tracker, codeFile, {}, stopCrawling);
					}, 1, TC("CrawlForDecomp"), ColorWork, WorkPriority_Low, &m_bottleneck);
			}
			else
			{
