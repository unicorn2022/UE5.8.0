...
namespace uba
{
	...
	inline constexpr u32 ERROR_FILE_EXISTS = EEXIST;
	inline constexpr u32 ERROR_ACCESS_DENIED = EACCES;
	inline constexpr u32 ERROR_DIRECTORY = ENOTDIR;
	inline constexpr u32 ERROR_DIR_NOT_EMPTY = ENOTEMPTY;
	...
	inline constexpr u32 CREATE_NEW = 1;
	...
	bool CopyFileW(const tchar* existingFileName, const tchar* newFileName, bool bFailIfExists, u8* scratch = nullptr, u64 scratchSize = 0);
	...
	private:
		Futex m_lock;
		UnorderedMap<TString, TString> m_map;
	};
	...
	private:
		...
		struct CreatedDir { Futex lock; Atomic<bool> handled = false; };
