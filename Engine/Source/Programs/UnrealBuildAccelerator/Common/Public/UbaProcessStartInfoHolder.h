...
namespace uba
{
	...
	inline void FixFileName(StringBufferBase& out, const tchar* fileName, const tchar* workingDir)
	{
		...
		FixPath2(fileName, ~0u, workingDir, workingDirLen, buffer, sizeof_array(buffer), &charLen);
		out.Append(buffer);
	}
	...
	struct ProcessStartInfoHolder : public ProcessStartInfo
	{
		...
		ProcessStartInfoHolder(const ProcessStartInfo& si)
		{
			*(ProcessStartInfo*)this = si;
			...
			overlayFilesStr = si.overlayFiles;
		...
		void Write(BinaryWriter& writer, const StringView& applicationOverride)
		{
			...
			writer.WriteLongString(argumentsStr, 1000);
		...
		void Read(BinaryReader& reader)
		{
			...
			argumentsStr = reader.ReadLongString();
			...
			// The wire form was written by Write() from a Holder that had
			// already been Expanded on the host — do not re-wrap it here.
			expanded = true;
		}
		...
		void operator=(const ProcessStartInfoHolder& o)
		{
			*(ProcessStartInfo*)this = o;
			...
			breadcrumbs = breadcrumbsStr.c_str();
			overlayFilesStr = o.overlayFiles;
			overlayFiles = overlayFilesStr.c_str();

			weight = o.weight;
			// Propagate the "already-expanded" flag so callers that copy an
			// expanded Holder into a new one (ProcessImpl::Start does this via
			// m_startInfo = startInfo) don't trigger a second wrap on the same
			// command — the second wrap produces `-c "sh -c \"\\"...\""` which
			// ParseArguments can't re-tokenize correctly.
			expanded = o.expanded;
		}
