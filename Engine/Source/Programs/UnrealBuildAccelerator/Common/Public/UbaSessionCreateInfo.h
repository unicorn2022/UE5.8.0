...
namespace uba
{
	...
	struct SessionCreateInfo
	{
		...
		bool allowMemoryMaps = true;			// Use memory maps where possible. Session creates memory maps of files that processes use
		bool allowKeepFilesInMemory = true;		// Allow detoured process to keep output files in memory and send them to session through file mapping handle
		bool allowOutputFiles = true;			// Allow (selected) output files to be written to disk after process has ended.
		...
		bool writePlaceholders = false;			// If this is true, 1-byte placeholder files will be written where possible
		...
		bool allowDiscardVirtualMemory = false;	// Set to true to use DiscardVirtualMemory around map/unmap views of file mappings. This seems to speed up perf quite a lot
		bool allowLinkDependencyCrawler = true;	// Enable crawler that reads ahead to find needed obj files for linker
		int linkCrawlerMaxParallel = -2;		// If allowLinkDependencyCrawler is set, this controls number of decompress in parallel. If negative it is max cores - value
