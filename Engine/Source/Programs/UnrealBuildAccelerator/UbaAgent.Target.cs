...
public class UbaAgentTarget : TargetRules
{
	...
	public static void CommonUbaSettings(TargetRules Rules, TargetInfo Target, bool ShouldExport = false)
	{
		...
		Rules.WindowsPlatform.TargetWindowsMinorVersion = 0x0A000005;
		...
		{
			Rules.AdditionalLinkerArguments += " /NODEFAULTLIB:kernel32.lib";
			Rules.AdditionalLinkerArguments += " mincore.lib";
			...
			Rules.GlobalDefinitions.AddRange(new string[] {
				...
				"UBA_DETOURS_LIBRARY_LINUX=\"libUbaDetours.so\"",
			});
		}
		else
		{
		...
		if (Target.Platform.IsInGroup(UnrealPlatformGroup.Apple))
		{
			...
			if (Rules.LaunchModuleName != "UbaVisualizer") // Revisit this. UbaVisualizer fails packaging if name is changed
			{
				string BinaryPrefix = Rules.bShouldCompileAsDLL ? "lib" : string.Empty;
			...
			{
				Rules.ExeBinariesSubFolder = "UnrealBuildAccelerator";
				//Rules.OutputFile = Path.Combine(BinariesFolder, "UbaVisualizer");
			}
			Rules.GlobalDefinitions.AddRange(new string[] {
