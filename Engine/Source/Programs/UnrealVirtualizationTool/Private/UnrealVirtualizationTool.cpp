...
int32 UnrealVirtualizationToolMain(int32 ArgC, TCHAR* ArgV[])
{
	...
	UE_LOGF(LogVirtualizationTool, Display, "Running UnrealVirtualization Tool");
	...
	if (InitResult == EInitResult::Success)
	{
		...
		{
			UE_LOGF(LogVirtualizationTool, Error, "UnrealVirtualizationTool ran with errors");
		}
	}	
	else if(InitResult == EInitResult::Error)
	{
		UE_LOGF(LogVirtualizationTool, Error, "UnrealVirtualizationTool failed to initialize");
		ProcessResult = EProcessResult::Error;
	}

	UE_CLOGF(ProcessResult == EProcessResult::Success, LogVirtualizationTool, Display, "UnrealVirtualizationTool ran successfully");
