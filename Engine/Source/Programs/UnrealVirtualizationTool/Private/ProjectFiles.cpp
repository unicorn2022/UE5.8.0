...
void ReloadConfigs(FConfigFile& PluginConfig)
{
	...
	for (const auto& ConfigEntry : AsConst(PluginConfig))
	{
		...
		if (PerObjConfigDelimIdx != INDEX_NONE)
		{
			...
			{
				PLATFORM_BREAK();
				//	UE_LOGF(LogGameFeatures, Warning, "[GameFeatureData %ls]: Couldn't find PerObjectConfig class %ls for %ls while processing %ls, config changes won't be reloaded.", *GetPathNameSafe(this), *ClassName, *ObjectName, *PluginConfig.Name.ToString());
			}
		}
...
bool TryFindProjectInPathHierarchy(FString DirectoryOrPath, const FString& ProjectExtension, FString& OutProjectFilePath)
{
	...
	while (Index != INDEX_NONE)
	{
		...
		{
			UE_LOGF(LogVirtualization, Warning, "Found multiple .uproject files for '%ls' at '%ls'", *ProjectExtension, *DirectoryOrPath);
			return false;
		}
