...
{
	UE_LOGF(LogVirtualizationTool, Display, "Attempting to initialize command '%ls'...", *ModeName);
...
EInitResult FUnrealVirtualizationToolApp::Initialize()
{
	...
	UE_LOGF(LogVirtualizationTool, Display, "Initializing...");
	...
	UE_LOGF(LogVirtualizationTool, Display, "Logging process to '%ls'", *LogFilePath);
	...
	if (!IsChildProcess())
	{
		...
		UE_LOGF(LogVirtualizationTool, Display, "\tFound %d package file(s)", Packages.Num());
	...
	UE_LOGF(LogVirtualizationTool, Display, "Initialization complete");
...
EProcessResult FUnrealVirtualizationToolApp::Run()
{
	...
	{
		UE_LOGF(LogVirtualizationTool, Error, "Command '%ls' failed!", *CurrentCommand->GetName());
		return Result;
	}
	...
	UE_LOGF(LogVirtualizationTool, Display, "Command '%ls' succeeded!", *CurrentCommand->GetName());
...
EProcessResult FUnrealVirtualizationToolApp::ProcessProjects(TArray<TUniquePtr<FCommandOutput>>& OutputArray)
{
	...
	if (Projects.IsEmpty())
	{
		UE_LOGF(LogVirtualizationTool, Display, "No projects requiring processing were found for the '%ls' command", *CurrentCommand->GetName());
		return EProcessResult::Success;
	}

	UE_LOGF(LogVirtualizationTool, Display, "Running the '%ls' command...", *CurrentCommand->GetName());

	OutputArray.Reserve(Projects.Num());

	for (const FProject& Project : Projects)
	{
		UE_LOGF(LogVirtualizationTool, Display, "Processing project: %ls", *Project.GetProjectFilePath());
		...
		if (IsChildProcess() || ProjectPath == Project.GetProjectFilePath())
		{
			if (IsChildProcess() && ProjectPath != Project.GetProjectFilePath())
			{
				...
				UE_LOGF(LogVirtualizationTool, Error, "The child process was created with project path '%ls' but expected '%ls'", *ProjectPath, *Project.GetProjectFilePath());
				return EProcessResult::Error;
			}
...
{
	UE_LOGF(LogVirtualizationTool, Display, "Usage:");

	UE_LOGF(LogVirtualizationTool, Display, "Commands:");
	...
	UE_LOGF(LogVirtualizationTool, Display, "Legacy Commands:");
	UE_LOGF(LogVirtualizationTool, Display, "-Mode=Changelist -ClientSpecName=<name> [optional] -Changelist=<number> -nosubmit [optional]");
	UE_LOGF(LogVirtualizationTool, Display, "-Mode=PackageList -Path=<string>");
	
	UE_LOGF(LogVirtualizationTool, Display, "");
	UE_LOGF(LogVirtualizationTool, Display, "Global Options:");
	UE_LOGF(LogVirtualizationTool, Display, "\t-MinimalLogging (demote log messages with 'display' verbosity to 'log' verbosity except those using the LogVirtualizationTool category)");
}
...
bool FUnrealVirtualizationToolApp::TryLoadModules()
{
	...
	{
		UE_LOGF(LogVirtualizationTool, Error, "Failed to load the 'Virtualization' module");
	}
...
bool FUnrealVirtualizationToolApp::TryInitEnginePlugins()
{
	...
	UE_LOGF(LogVirtualizationTool, Log, "Loading Engine Plugins");
	...
	auto LoadPlugin = [](const FString& PlugInName) -> bool
		{
			...
			{
				UE_LOGF(LogVirtualizationTool, Error, "The plugin '%ls' is disabled.", *PlugInName);
				return false;
			}
...
EInitResult FUnrealVirtualizationToolApp::TryParseCmdLine()
{
	...
	UE_LOGF(LogVirtualizationTool, Log, "Parsing the commandline");

	const TCHAR* CmdLine = FCommandLine::Get();

	if (CmdLine == nullptr || CmdLine[0] == TEXT('\0'))
	{
		UE_LOGF(LogVirtualizationTool, Error, "No commandline parameters found!");
		PrintCmdLineHelp();
		return EInitResult::Error;
	}
	...
	{
		UE_LOGF(LogVirtualizationTool, Display, "Commandline help requested");
		PrintCmdLineHelp();
	...
	{
		UE_LOGF(LogVirtualizationTool, Error, "Failed to find cmdline switch 'Mode', this is a required parameter!");
		PrintCmdLineHelp();
		return EInitResult::Error;
	}
...
EInitResult FUnrealVirtualizationToolApp::CreateCommandFromString(const FString& CommandName, const TCHAR* Cmdline)
{
	...
	switch (Mode)
	{
		...
		case EMode::Unknown:
		default:
			UE_LOGF(LogVirtualizationTool, Error, "Unexpected value for the cmdline switch 'Mode', this is a required parameter!");
			PrintCmdLineHelp();
			return EInitResult::Error;
	}
}
...
bool FUnrealVirtualizationToolApp::TrySortFilesByProject(const TArray<FString>& Packages)
{
	...
	UE_LOGF(LogVirtualizationTool, Display, "\tSorting files by project...");
	...
	UE_LOGF(LogVirtualizationTool, Display, "\tFound the following project(s):");
	...
	{
		UE_LOGF(LogVirtualizationTool, Display, "\tCould not find a project for %d package(s) which will be ignored", Packages.Num() - TotalPackagesAssigned);
	}
...
bool FUnrealVirtualizationToolApp::TryReadChildProcessOutputFile(const FGuid& ChildProcessId, const FCommand& Command, TArray<TUniquePtr<FCommandOutput>>& OutputArray)
{
	...
	{
		UE_LOGF(LogVirtualizationTool, Error, "Could not open child process output file '%ls'", FilePath.ToString());
		return false;
	}
	...
	{
		UE_LOGF(LogVirtualizationTool, Error, "Failed to parse child process output file '%ls'", FilePath.ToString());
		return false;
	}
	...
	if (JsonRootObject->TryGetArrayField(TEXT("OutputArray"), OutputJsonArray))
	{
		...
		for (TSharedPtr<FJsonValue> OutputValue : *OutputJsonArray)
		{
			...
			{
				UE_LOGF(LogVirtualizationTool, Error, "Invalid syntax found in child process output file '%ls'", FilePath.ToString());
				return false;
			}
			...
			{
				UE_LOGF(LogVirtualizationTool, Error, "Failed to read FCommandOutput from the child process output file '%ls'", FilePath.ToString());
				return false;
			}
...
bool FUnrealVirtualizationToolApp::TryWriteChildProcessOutputFile(const FString& ChildProcessId, const TArray<TUniquePtr<FCommandOutput>>& OutputArray)
{
	...
	if (!Writer->Close())
	{
		UE_LOGF(LogVirtualizationTool, Display, "Failed to create child process output file json document");
		return false;
	}
	...
	{
		UE_LOGF(LogVirtualizationTool, Display, "Failed to create child process output file '%ls'", FilePath.ToString());
		return false;
	}
...
{
	UE_LOGF(LogVirtualizationTool, Display, "Parsing child process input file...");

	FString JsonText;
	if (!FFileHelper::LoadFileToString(JsonText, *InputPath))
	{
		UE_LOGF(LogVirtualizationTool, Error, "Could not open child process input file '%ls'", *InputPath);
		return false;
	}
	...
	if (!FJsonSerializer::Deserialize(Reader, JsonRootObject) || !JsonRootObject.IsValid())
	{
		UE_LOGF(LogVirtualizationTool, Error, "Failed to parse child process input file '%ls'", *InputPath);
		return false;
	}
	...
	if (TSharedPtr<FJsonValue> JsonValue = JsonRootObject->TryGetField(TEXT("CommandName")))
	{
		...
		{
			UE_LOGF(LogVirtualizationTool, Error, "Failed to create command '%ls'", *CommandName);
			return false;
		}
	}
	else
	{
		UE_LOGF(LogVirtualizationTool, Error, "Failed to find 'CommandName' in child process input file '%ls'", *InputPath);
		return false;
	}
	...
	if (TSharedPtr<FJsonObject> JsonObject = JsonRootObject->GetObjectField(TEXT("ProjectData")))
	{
		...
		{
			UE_LOGF(LogVirtualizationTool, Error, "Failed to serialize project data from child process input file '%ls'", *InputPath);
			return false;
		}
	}
	else
	{
		UE_LOGF(LogVirtualizationTool, Error, "Failed to find 'ProjectData' in child process input file '%ls'", *InputPath);
		return false;
	}
...
bool FUnrealVirtualizationToolApp::TryWriteChildProcessInputFile(const FGuid& ChildProcessId, const FCommand& Command, const FProject& Project, FStringBuilderBase& OutPath)
{
	...
	if (!Writer->Close())
	{
		UE_LOGF(LogVirtualizationTool, Display, "Failed to create child process input file json document");
		return false;
	}
	...
	{
		UE_LOGF(LogVirtualizationTool, Display, "Failed to save child process input file '%ls'", OutPath.ToString());
		return false;
	}
...
void FUnrealVirtualizationToolApp::CleanUpChildProcessFiles(const FGuid& ChildProcessId)
{
	...
	for (const TCHAR* Extension : FileExtensions)
	{
		...
		if (!IFileManager::Get().Delete(FilePath.ToString()))
		{
			...
			UE_LOGF(LogVirtualizationTool, Warning, "Failed to clean up temp file '%ls' due to: %ls", FilePath.ToString(), SystemErrorMsg.ToString());
		}
	}
}
...
EProcessResult FUnrealVirtualizationToolApp::LaunchChildProcess(const FCommand& Command, const FProject& Project, FStringView GlobalOptions, TArray<TUniquePtr<FCommandOutput>>& OutputArray)
{
	...
	UE_LOGF(LogVirtualizationTool, Display, "Launching and waiting on a new instance of the tool...");
...

	{
		...
		{
			UE_LOGF(LogVirtualizationTool, Display, "Failed to retrieve the return value of the child process");
			return EProcessResult::Error;
		}

		if (ReturnCode != 0)
		{
			UE_LOGF(LogVirtualizationTool, Display, "Child process failed with error code: %d", ReturnCode);
