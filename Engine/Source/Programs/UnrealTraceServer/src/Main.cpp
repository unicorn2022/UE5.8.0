...
////////////////////////////////////////////////////////////////////////////////
static bool IsAllowedToBreakAway()
{
	...
	{
		return !!(JobLimits.LimitFlags & JOB_OBJECT_LIMIT_BREAKAWAY_OK);
	}
	return false;
}
...
static bool LaunchUnprivileged(const wchar_t* Binary, wchar_t* CommandLine)
{
	...
	if (bOk == FALSE)
	{
		TS_LOG("Unable to create safe level to launch (error %0xd)", GetLastError());
		return false;
	}
	...
	if (SaferComputeTokenFromLevel(SafeLevel, nullptr, &AccessToken, 0, nullptr))
	{
		...
		if (!bOk)
		{
			TS_LOG("Failed to create process as user (error %0xd)", GetLastError());
		}

		CloseHandle(AccessToken);
	}
	else {
		TS_LOG("Unable to create safe level token to launch (error %0xd)", GetLastError());
	}
...
static bool LaunchUnelevated(const wchar_t* Binary, wchar_t* CommandLine)
{
	...
	{
		TS_LOG("User is admin, switching to LaunchUnpriviledged");
	...
	if (!bOk)
	{
		TS_LOG("Unable to update thread attributes");
		return false;
	}
	...
	uint32 CreateProcFlags = EXTENDED_STARTUPINFO_PRESENT;
	if (IsAllowedToBreakAway())
	{
		CreateProcFlags |= CREATE_BREAKAWAY_FROM_JOB;
	}
	...
	if (bOk == FALSE)
	{
		TS_LOG("Create process failed with error %0xd", GetLastError());
		return false;
	}
...
////////////////////////////////////////////////////////////////////////////////
// macOS shm_open rejects O_CLOEXEC (EINVAL); set it via fcntl after open instead.
#if TS_USING(TS_PLATFORM_MAC)
static int SharedMemoryOpen(const char* Name, int Flags, mode_t Mode)
{
	int Fd = shm_open(Name, Flags & ~O_CLOEXEC, Mode);
	if (Fd >= 0 && (Flags & O_CLOEXEC))
		fcntl(Fd, F_SETFD, FD_CLOEXEC);
	return Fd;
}
#else
#define SharedMemoryOpen shm_open
#endif
...
static int MainKill(int ArgC, char** ArgV, const FOptions& Options)
{
	...
	int Fd = SharedMemoryOpen("/UnrealTraceServer", O_RDONLY | O_CLOEXEC, 0666);
	if (Fd > 0)
	{
...
{
	TS_LOG("Opening shared memory");
	int Fd = SharedMemoryOpen("/UnrealTraceServer", O_RDWR | O_CLOEXEC, 0666);
	if (Fd > 0)
	{
...
{
	TS_LOG("Opening shared memory");
	int Fd = SharedMemoryOpen("/UnrealTraceServer", O_RDWR | O_CREAT | O_CLOEXEC, 0666);
	if (Fd < 0)
	{
