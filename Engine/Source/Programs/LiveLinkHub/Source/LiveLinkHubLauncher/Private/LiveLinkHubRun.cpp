...
int32 RunLiveLinkHub(const TCHAR* CommandLine)
{
	...
	FCoreDelegates::OnSyncLoadPackage.AddLambda([](const FString& PackageName)
		{
			UE_LOGF(LogLiveLinkHub, Display, "Audit: Loaded %ls", *PackageName);
		});
#endif

	bool bPendingRestart = false;
	
	FString DevelopmentProjectPath = TEXT("../../Source/Programs/LiveLinkHub/LiveLinkHub.uproject");
	FString StagedProjectPath = TEXT("../../../LiveLinkHub/LiveLinkHub.uproject");

#if PLATFORM_MAC
	// On Mac the following FileExists calls will fail unless it's an absolute path.
	DevelopmentProjectPath = FPaths::ConvertRelativePathToFull(DevelopmentProjectPath);
	StagedProjectPath = FPaths::ConvertRelativePathToFull(StagedProjectPath);
#endif
	...
	ON_SCOPE_EXIT
	{
		...
		if (bPendingRestart)
		{
			...
			{
				UE_LOGF(LogLiveLinkHub, Error, "Pending restart: FPlatformProcess::CreateProc failed");
			}
		}
	};
	...
	if (Result != 0)
	{
		UE_LOGF(LogLiveLinkHub, Error, "EngineLoop PreInit failed (%i)", Result);
	}
