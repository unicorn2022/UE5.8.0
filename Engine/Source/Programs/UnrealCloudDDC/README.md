...
You can also specify the `*` claim which grants access for any valid token no matter which claims it has, this is mostly for debug / testing scenarios and should not be used for production data.
...
Multiple statements under the namespaces and buckets directly are ANDed (all of them have to be satisfied).
...
This is an example of an advanced setup for builds that exclude access to certain platforms (in this example linux is considered a secret platform) as well as excluding access to a build part called "sensitive" unless 
explicit access is granted via a seperate claim:
...
```
    - claims:
      ...
      scopes:
        ...
        - values: 
          ...
          not: true
        resources:
        # access to all parts except the sensitive part
        - resourceType: "parts"
          values: 
          - "sensitive"
          not: true
    ...
    - claims:
      - groups=app-ddc-storage-ue-sensitive-access
      actions:
      - ReadBuildPart
      - WriteBuildPart
      scopes:
