...
public enum JupiterAclAction
{
	...
	/// <summary>
	/// Access to read build parts
	/// </summary>
	ReadBuildPart,

	/// <summary>
	/// Access to write build parts
	/// </summary>
	WriteBuildPart,
...
public class AclPolicy
{
	...
	public IEnumerable<JupiterAclAction> Resolve(ClaimsPrincipal user, AccessScope scope, ILogger logger, out List<string> matchedClaims)
	{
		...
		bool requiresResourceScope = scope.ResourceType != null && scope.ResourceName != null;

		if (!requiresResourceScope && bucketFound)
		{
			// this policy match all claims and scopes, we grant the actions it contains
			return Actions;
		}

		if (requiresResourceScope)
		{
			bool resourceFound = false;
			foreach (AclResourceScopeEntry resourceScope in Scope.Resources)
			{
				if (string.Equals(resourceScope.ResourceType, scope.ResourceType, StringComparison.OrdinalIgnoreCase))
				{
					bool isMatch = resourceScope.Matches(scope.ResourceName ?? string.Empty, logger, Name);

					if (isMatch)
					{
						logger.LogDebug("Resource scope matched for policy \'{Name}\'.", Name);
						resourceFound = true;
						break;
					}
				}
...
public class AclScope
{
	...
	/// <summary>
	///  List of one or more resources (by-name and type) that this policy applies to
	/// </summary>
	[System.Diagnostics.CodeAnalysis.SuppressMessage("Usage", "CA2227:Collection properties should be read only", Justification = "Used by the configuration system")]
	// ReSharper disable once CollectionNeverUpdated.Global
	public List<AclResourceScopeEntry> Resources { get; set; } = new List<AclResourceScopeEntry>();
}

/// <summary>
/// Potential mapping of a scope for a set of resources
/// </summary>
public class AclResourceScopeEntry : AclScopeEntry
{
	[Required]
