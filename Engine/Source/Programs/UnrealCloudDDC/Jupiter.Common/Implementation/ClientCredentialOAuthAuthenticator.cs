...
namespace Jupiter.Implementation
{
	...
	public class ClientCredentialOAuthAuthenticator : IAuthenticator
	{
		...
		private async Task PreAuthenticateAsync(CancellationToken cancellationToken = default)
		{
			(ClientCredentialsResponse result, string body) = await DoAuthenticationRequestAsync(cancellationToken);
		...
		private async Task<(ClientCredentialsResponse, string)> DoAuthenticationRequestAsync(CancellationToken cancellationToken = default)
		{
			...
			ClientCredentialsResponse? responseBody = await response.Content.ReadFromJsonAsync<ClientCredentialsResponse>(cancellationToken);
