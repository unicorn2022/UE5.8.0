...
* Added `ReadBuildPart` and `WriteBuildPart` actions - these allows access control per part based on its name for builds. This allows for more sensitive parts of builds to still be distributed as the same thing but those files can be excluded from some users. This is a very advanced use case and we generally recommend to just assign Read and WriteBuildPart to everyone like you would `ReadObject` and `WriteObject` .

# 1.4.0
Noteworthy and potentially breaking changes:
* All endpoints have been updated to use the new per-bucket authorization by default. Set the `Auth.UseLegacyConfiguration` setting to `true` to enable the legacy authentication behavior.
* Namespace-based authorization is deprecated, along with the `Auth.UseLegacyConfiguration` setting, and will be removed in a future release.
* The `ConnectionString` setting for replicators is no longer strictly required, allowing a `SourceSite` to be defined instead, which will look up the connection string of the relevant peer in the `cluster.peers` configuration. `SourceSite` will be respected over a `ConnectionString`, and an error will be thrown if neither are provided.
* The `ReferenceResolver.UseV2ReferenceResolving` setting is now enabled by default.
* The `UnrealCloudDDC.EnableForceSubmitRefBlobToBlobStore` setting is now disabled by default. When disabled, inlined blobs are only written to Scylla, significantly reducing the number of small blobs in S3. When `UnrealCloudDDC.EnableInlineSmallBlobs` is disabled (not recommended), then blobs are still written to S3.
* The `UnrealCloudDDC.EnableBucketStatsTracking` setting is now disabled by default, as it can lead to performance issues when querying sizes for many attachments. The strategy for collecting these stats will be reworked in a future release.
* Removed bucket enumeration endpoint (`GET /api/v1/refs/{ns}/{bucket}`) as its performance was inefficient and discouraged, and did not function correctly when `EnableBucketStatsTracking` was set to `false`.
* Removed the `UnrealCloudDDC.RequirePrivatePortForEnumeration` setting since its relevant endpoint has been removed.
* Regular expressions passed to the build search endpoint (`POST /api/v2/builds/{ns}/search`) now ignore the case of the provided `bucketRegex`.
* The `S3.MultiPartMaxMemoryBufferSize` setting has been reduced to `67108864` (was `int.MaxValue`).
* Added a service to periodically clean up temp files, controlled using the `GC.CleanTempFiles` setting (defaults to `true`).

New features:
* Added the `Scylla.UseConsistentContentIdResolve` setting, which uses local_quorum consistency when resolving content IDs.
* Added the `GET /api/v2/builds/{ns}/{bucket}/{buildId}/parts/{partId}/fileList` endpoint, useful for inspecting which files belong to a build.
* Added the `includePartial` option to `POST /api/v2/builds/{ns}/{bucket}/search` endpoint, used to include non-finalized builds to search results.
* Added the `S3.SiteBucketsPerStoragePool` setting, useful for defining which site-specific S3 bucket to use for each storage pool. 
* Added the `SourceSite` setting to each replicator, used to look up the connection URL using a peer by its `Name` in the `cluster.peers` configuration, rather than defining its `ConnectionString` explicitly.
* Added the `S3.DisablePayloadChecksums` setting, which reduces CPU usage at the cost of payload verification.
* Added the `GC.AllowBlockMetadataCleanup` setting (defaults to `true`).
* Added the `UnrealCloudDDC.RemoveReferencesOnBlobDeletion` setting (defaults to `true`).
* Added the `UnrealCloudDDCOrphanBlobCutoffDurationMinutes` setting (defaults to `60`).

Minor improvements:
* Added retries for Amazon S3 multipart uploads.
* Return 408 status codes when blob upload tasks are cancelled.
* Improved Amazon S3 rate limit handling.
* Improved blob replication stability.
* Return 503 status codes when the underlying database is unavailable.
* Added retries to build existence checks.
* Added multipart support for blob replication.
* Improved audit logging parallelism in `BuildsController`.
* Added string-to-number conversions for build search API when using compact binary objects.
* Increased the default maximum number of builds scanned during a search from `2000` to `20000`.
* Validate that all parts of a build are finalized before the build itself can be finalized.
* Use check-and-set pattern for blob replication state to allow state to be mutated externally.
* Added optimization to move files that are already buffered on the file system to their final destination, rather than copying the file by reading into an in-memory buffer.

Bug fixes:
* Fallback to file system buffering when unable to allocate enough memory, even for small payloads.
* Fixed ObjectId JSON serialization.
* Fixed retry logic for blob replication.
* Fixed an issue where last access background service was running more frequently than intended.
* Fixed NPE in `BuildsController` audit logging when no part ID or build ID was specified, but a namespace was.
* Removed redundant telemetry tracing scope in blob replication.
* Fixed an issue where setting `EnableInlineSmallBlobs` to `false` would incorrectly attempt to inline all blobs.
* Fixed incorrect dependency injection type in `ReplicationController`.

Helm chart changes:
* Dependency on `bitnami/common` is now vendored into the source tree.
* Changed the `HorizontalPodAutoscaler` to use container-specific metrics for CPU and memory rather than Pod-wide metrics.
* Fixed ConfigMap generation when omitting any `Jupiter` configuration.
* Added missing replicator version configuration to ConfigMap.
