...
namespace Jupiter
{
	...
	public class JupiterStartup : BaseStartup
	{
		...
		protected override void OnAddService(IServiceCollection services)
		{
			...
			services.AddOptions<DataStoreSettings>().Configure(o => Configuration.GetSection("DataStore").Bind(o)).ValidateDataAnnotations();
			...
			// TODO: Expose all compatible object stores as a IDataStoreObjectStore
			services.AddSingleton<IDataStoreObjectStore>(provider => provider.GetRequiredService<AmazonS3Store>());

			services.AddSingleton<IDataStoreTableService, DataStoreTableService>();
			services.AddSingleton<IDataStoreTableLogService, DataStoreTableLogService>();
			services.AddHostedService<DataStoreTableLogService>(p => (p.GetRequiredService<IDataStoreTableLogService>() as DataStoreTableLogService)!);
			...
			services.AddHostedService<TempCleanupService>(p => p.GetService<TempCleanupService>()!);
			...
			if (settings.UseLastAccessBackgroundService)
			{
				services.AddSingleton<LastAccessBackgroundService>();
				services.AddHostedService<LastAccessBackgroundService>(p => p.GetService<LastAccessBackgroundService>()!);
			}
			else
			{
		...
		private IBlobIndex BlobIndexFactory(IServiceProvider provider)
		{
			...
			switch (settings.BlobIndexImplementation)
			{
				...
				case UnrealCloudDDCSettings.BlobIndexImplementations.DataStore:
					return ActivatorUtilities.CreateInstance<DataStoreBlobIndex>(provider);
				default:
		...
		private object ContentIdStoreFactory(IServiceProvider provider)
		{
			...
			IContentIdStore store = settings.ContentIdStoreImplementation switch
			{
				...
				UnrealCloudDDCSettings.ContentIdStoreImplementations.DataStore => ActivatorUtilities
					.CreateInstance<DataStoreContentIdStore>(provider),
		...
		private IScyllaSessionManager ScyllaFactory(IServiceProvider provider)
		{
			...
			Builder clusterBuilder = Cluster.Builder()
				...
				.WithExecutionProfiles(options =>
					options.WithProfile("default", builder => builder.WithConsistencyLevel(settings.DefaultConsistencyLevel))
						.WithProfile("local_consistent", builder =>
						{
							ConsistencyLevel level;

							// Use the DefaultConsistencyLevel if it provides higher consistency than LocalQuorum
							switch (settings.DefaultConsistencyLevel)
							{
								case ConsistencyLevel.Quorum:
								case ConsistencyLevel.EachQuorum:
								case ConsistencyLevel.All:
									level = settings.DefaultConsistencyLevel;
									break;
								default:
									level = ConsistencyLevel.LocalQuorum;
									break;
							}
							
							builder.WithConsistencyLevel(level);
						}));
			...
			if (settings.UseAzureCosmosDB || settings.UseAWSKeyspace)
			{
			...
			bool isScylla = !settings.UseAzureCosmosDB && !settings.UseAWSKeyspace;
			return new ScyllaSessionManager(replicatedSession, localSession, isScylla, !isScylla, settings.UseAWSKeyspace);
		}
		...
		private IReferencesStore ObjectStoreFactory(IServiceProvider provider)
		{
			...
			IReferencesStore store = settings.ReferencesDbImplementation switch
			{
				...
				UnrealCloudDDCSettings.ReferencesDbImplementations.Galileo => ActivatorUtilities.CreateInstance<GalileoReferencesStore>(provider),
				UnrealCloudDDCSettings.ReferencesDbImplementations.DataStore => ActivatorUtilities.CreateInstance<DataStoreReferencesStore>(provider),
				_ => throw new NotImplementedException(
			...
			// Memory cache can not be enabled when using data store as it in turn has an in memory cache
			bool useMemoryCache = settings.ReferencesDbImplementation != UnrealCloudDDCSettings.ReferencesDbImplementations.DataStore && settings.ReferencesDbImplementation != UnrealCloudDDCSettings.ReferencesDbImplementations.Galileo;
			if (useMemoryCache && memoryCacheSettings.Enabled)
			{
