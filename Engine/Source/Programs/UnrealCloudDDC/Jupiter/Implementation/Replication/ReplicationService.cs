			...
			foreach (IReplicator replicator in state.Replicators)
			{
				if (_currentReplications.TryGetValue(replicator.Info.ReplicatorName, out PendingReplication? pendingReplication))
				{
					...
					if (replicationTask.IsCompleted)
					{
						...
						if (replicationTask.IsCanceled)
						{
							// Similarly, we check for cancellations but make sure all replicators get to run
							_logger.LogWarning("Replicator {Name} was canceled", replicator.Info.ReplicatorName);
							continue;
						}
