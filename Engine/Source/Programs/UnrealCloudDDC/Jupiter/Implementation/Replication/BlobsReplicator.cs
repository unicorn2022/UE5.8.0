...
namespace Jupiter.Implementation
{
	public class BlobsReplicator : IReplicator
	{
		...
		private CancellationTokenSource _replicationTokenSource = new CancellationTokenSource();
		...
		/// <summary>
		/// Updates the state of the current replicator using the provided new state.
		/// 
		/// If an expected old state is provided, then ensures that the existing state matches prior to saving the new state. This allows the admin
		/// endpoints to rewind or fast-forward the replicator's state without worrying about being overwritten.
		///
		/// If an existing state does not exist at all, then the new state will be inserted regardless of the provided expected old state.
		/// </summary>
		/// <param name="newState">New state to persist</param>
		/// <param name="expectedOldState">Optional old state that is expected to exist before persisting the new state</param>
		private async Task<bool> SaveStateAsync(BlobsState newState, BlobsState? expectedOldState = null)
		{
			if (expectedOldState != null)
			{
				ReplicatorState? existingState = await _replicationLog.GetReplicatorStateAsync(Info.NamespaceToReplicate, _name);

				if (existingState != null && existingState.LastBucket != expectedOldState.LastBucket)
				{
					_logger.LogWarning("Existing replicator state for {Name} does not match expected value ({Expected} != {Actual}). Skipping persistence of new state.", _name, expectedOldState.LastBucket, existingState.LastBucket);
					return false;
				}
			}
		...
		public async Task<bool> TriggerNewReplicationsAsync()
		{
			...
			try
			{
				...
				if (!_replicationTokenSource.TryReset())
				{
					_replicationTokenSource.Dispose();
		...
		private async Task<int> ReplicateIncrementallyAsync(NamespaceId ns, string lastBucket, CancellationToken replicationToken)
		{
			...
			_logger.LogInformation("{Name} Starting blob replication maxParallelism: {MaxParallelism} Last Bucket {LastBucket}", _name, _replicatorSettings.MaxParallelReplications, lastBucket);
			...
			foreach (string refBucket in GetRefBuckets(lastBucket, replicationToken))
			{
				...
				await Parallel.ForEachAsync(GetBlobEventsAsync(ns, refBucket, replicationToken),
					new ParallelOptions { MaxDegreeOfParallelism = _replicatorSettings.MaxParallelReplications, CancellationToken = replicationToken },
					async (BlobReplicationLogEvent @event, CancellationToken ctx) =>
				{
					...
					try
					{
						...
						{
							blobWasReplicated = await ReplicateBlobAsync(@event.Namespace, @event.Blob, @event.BucketHint, ctx);
						}
				...
				_logger.LogDebug("{Name} finished enumeration of bucket: {EventBucket}", _name, refBucket);
				...
				else
				{
					...
					BlobsState previousState = new BlobsState() { LastBucket = _replicationState.LastBucket };
					_replicationState.LastBucket = refBucket;

					if (!await SaveStateAsync(_replicationState, expectedOldState: previousState))
					{
						_logger.LogWarning("Replication state for {Name} has changed during processing. Aborting.", _name);
						break;
					}
				}
		...
		private async IAsyncEnumerable<BlobReplicationLogEvent> GetBlobEventsAsync(NamespaceId ns, string refBucket, [EnumeratorCancellation] CancellationToken cancellationToken)
		{
			...
			for (int i = 0; i < RetryAttempts; i++)
			{
				...
				{
					_logger.LogError("Invalid operation when calling requesting blobs for {Namespace} {RefBucket}", ns, refBucket);
					throw;
				}
			}
		...
		private async Task<bool> ReplicateBlobAsync(NamespaceId ns, BlobId blob, BucketId? bucketHint, CancellationToken cancellationToken)
		{
			...
			if (_replicatorSettings.EnableMultipartReplications)
			{
				return await ReplicateMultipartBlobAsync(ns, blob, bucketHint, cancellationToken);
			}
			
			return await ReplicateBlobHttpAsync(ns, blob, bucketHint, cancellationToken);
		}

		private async Task<bool> ReplicateMultipartBlobAsync(NamespaceId ns, BlobId blob, BucketId? bucketHint, CancellationToken cancellationToken)
		{
			const int RetryAttempts = 5;
			Exception? lastException = null;
			for (int retryAttempts = 0; retryAttempts < RetryAttempts; retryAttempts++)
			{
				lastException = null;
				
				try
				{
					const long InitialRangeFrom = 0;
					// we just assume 32 MB chunks right now, the chunk size doesn't matter much but its ideal if we stick to what the root backing store wants, which is 32 MB for S3, this should likely be configurable
					ulong chunkSize = 32 * 1024 * 1024 ;
					long initialRangeTo = (long)chunkSize - 1;
					...
					blobRequest.Headers.Range = new RangeHeaderValue(InitialRangeFrom, initialRangeTo);
					...
					// Requested blob is smaller than the initial range. Fallback to request the entire blob
					if (blobResponse.StatusCode == HttpStatusCode.RequestedRangeNotSatisfiable)
					{
						return await ReplicateBlobHttpAsync(ns, blob, bucketHint, cancellationToken);
					}

					// Server didn't respect the range request for some reason, but successfully retrieved the content
					...
					if (blobResponse.StatusCode != HttpStatusCode.PartialContent)
					{
					...
					// Received the first chunk. Use response headers to determine how to parallelize the rest
					ContentRangeHeaderValue responseRange = blobResponse.Content.Headers.ContentRange!;

					if (responseRange.From != InitialRangeFrom || responseRange.To > initialRangeTo)
					{
						throw new Exception("Response range does not match requested range");
					}

					if (!responseRange.HasLength)
					{
						throw new Exception("Expected length in Content-Range header");
					}
					
					ulong blobSize = (ulong)responseRange.Length!;
					int partsCount = (int)(blobSize / chunkSize) + (blobSize % chunkSize > 0 ? 1 : 0);

					IBufferedPayload[] parts = new IBufferedPayload[partsCount];
					long[] lengths = new long[partsCount];

					try
					{
						await using Stream blobStream = await blobResponse.Content.ReadAsStreamAsync(cancellationToken);
						long? contentLength = blobResponse.Content.Headers.ContentLength;
						...
						{
							parts[0] = await _bufferedPayloadFactory.CreateFromStreamAsync(blobStream, contentLength.Value, "blob-replicator", cancellationToken);
							lengths[0] = contentLength.Value;
						}
						...
						{
							ulong firstByte = chunkSize * ((ulong)part - 1);
							ulong lastByte = Math.Min(chunkSize * (ulong)part - 1, (ulong)blobSize -1);

							// part starts with part 1 which we skipped
							MultipartByteRange range = new MultipartByteRange()
							{
								FirstByte = firstByte, LastByte = lastByte, PartId = part.ToString()
							};
							IBufferedPayload? partPayload = await ReplicateMultipartBlobRangeHttpAsync(ns, blob, range, token);

							parts[part-1] = partPayload ?? throw new NotImplementedException($"Got null part for blob {blob} and part {part}");
							lengths[part-1] = partPayload.Length;
						});

						long totalSize = lengths.Sum();
						using FilesystemBufferedPayloadWriter writer = _bufferedPayloadFactory.CreateFilesystemBufferedPayloadWriter("blob-replicator-combine");

						{
							await using Stream writableStream = writer.GetWritableStream();
							foreach (IBufferedPayload part in parts)
							{
								await using Stream partStream = part.GetStream();
								await partStream.CopyToAsync(writableStream, cancellationToken);
							}
						}
						using IBufferedPayload combinedPayload = writer.Done();
						if (totalSize != combinedPayload.Length)
						{
							throw new Exception($"Multipart replicated blob did not match expected size of {totalSize} was instead {combinedPayload.Length}");
						}
						await _blobService.PutObjectAsync(ns, combinedPayload, blob, bucketHint: bucketHint, cancellationToken: cancellationToken);
					}
					finally
					{
					...
					// break retry loop, we were successful
					break;
				}
				catch (AggregateException ae)
				{
					ae.Handle(e =>
					{
						if (e is not IOException or HttpIOException)
						{
							_logger.LogError(e, "Failed to replicate {Blob} in {Namespace} due to unknown exception", blob, ns);
							return false;
						}
						
						_logger.LogWarning("Failed to replicate {Blob} in {Namespace} due to retriable exception {Exception}", blob, ns, e);
						lastException = e;
						return true;
					});
				}
				...
				{
					// rethrow unknown exceptions
					if (e.InnerException is not IOException && e.InnerException is not HttpIOException)
					{
						_logger.LogError(e, "Failed to replicate {Blob} in {Namespace} due to unknown exception", blob, ns);
						throw;
					}
		...
		private async Task<IBufferedPayload?> ReplicateMultipartBlobRangeHttpAsync(NamespaceId ns, BlobId blob, MultipartByteRange range, CancellationToken cancellationToken)
		{
			_logger.LogDebug("Retrieving blob {Blob} chunk range {Start}-{End} in {Namespace}.", blob, range.FirstByte, range.LastByte, ns);
			...
			for (int i = 0; i < RetryAttempts; i++)
			{
				...
				{
					using HttpRequestMessage blobRequest = await BuildHttpRequestAsync(HttpMethod.Get, new Uri($"api/v1/blobs/{ns}/{blob}", UriKind.Relative));
					blobRequest.Headers.Range = new RangeHeaderValue((long)range.FirstByte, (long)range.LastByte);

					using HttpResponseMessage blobResponse = await _httpClient.SendAsync(blobRequest, HttpCompletionOption.ResponseHeadersRead, cancellationToken);

					if (blobResponse.StatusCode != HttpStatusCode.PartialContent)
					{
						_logger.LogError("Bad http response when retrieving {Blob} chunk range {Start}-{End} in {Namespace} . Status code: {StatusCode}", blob, range.FirstByte, range.LastByte, ns, blobResponse.StatusCode);
					}
					
					ContentRangeHeaderValue responseRange = blobResponse.Content.Headers.ContentRange!;

					if (responseRange.From != (long)range.FirstByte || responseRange.To != (long)range.LastByte)
					{
						throw new Exception("Response range does not match requested range");
					}
					...
					IBufferedPayload payload = await _bufferedPayloadFactory.CreateFromStreamAsync(s, contentLength.Value, "blob-replicator-part", cancellationToken);
					return payload;
				}
				catch (AggregateException ae)
				{
