...
namespace Jupiter.Implementation
{
	public class ObjectService : IRefService
	{
		...
		public async Task<(RefRecord, BlobContents?)> GetAsync(NamespaceId ns, BucketId bucket, RefId key, string[]? fields = null, bool consistentRead = false, bool doLastAccessTracking = true, CancellationToken cancellationToken = default)
		{
...
#pragma warning restore CA2000 // Dispose objects before losing scope
		...
		public async Task<(ContentId[], BlobId[])> PutAsync(NamespaceId ns, BucketId bucket, RefId key, BlobId blobHash, CbObject payload, Action<BlobId>? onBlobFound = null, bool allowOverwrite = false, CancellationToken cancellationToken = default)
		{
			...
			{
				// if we are never inlining the blob then remove the payload bytes from the ref entry and just put them in the blob store
				blobPayloadBytes = Array.Empty<byte>();
				putIntoBlobStore = true;
			}
		...
		public async Task<(ContentId[], BlobId[])> FinalizeAsync(NamespaceId ns, BucketId bucket, RefId key, BlobId blobHash, Action<BlobId>? onBlobFound = null, CancellationToken cancellationToken = default)
		{
			...
			(RefRecord o, BlobContents? blob) = await GetAsync(ns, bucket, key, consistentRead: true, cancellationToken: cancellationToken);
			if (blob == null)
			{
		...
		public async Task<bool> ExistsAsync(NamespaceId ns, BucketId bucket, RefId key, CancellationToken cancellationToken)
		{
			try
			{
				(RefRecord, BlobContents?) _ = await GetAsync(ns, bucket, key, new string[] { "name" }, doLastAccessTracking: false, consistentRead: true, cancellationToken: cancellationToken);
			}
