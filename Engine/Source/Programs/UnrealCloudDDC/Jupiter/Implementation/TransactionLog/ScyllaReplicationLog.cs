...
namespace Jupiter.Implementation
{
	class ScyllaReplicationLog : IReplicationLog
	{
		...
		public ScyllaReplicationLog(IScyllaSessionManager scyllaSessionManager, IOptionsMonitor<ScyllaSettings> settings, Tracer tracer)
		{
			...
			string suffix = scyllaSessionManager.IsAWSKeyspace ? "WITH CUSTOM_PROPERTIES={'ttl':{'status':'enabled'}}" : "";
		...
		{
			IEnumerable<ScyllaNamespace> namespaces;
			
			try
			{
				namespaces = await _mapper.FetchAsync<ScyllaNamespace>("");
			}
		...
		public async IAsyncEnumerable<ReplicationLogEvent> GetAsync(NamespaceId ns, string? lastBucket, Guid? lastEvent)
		{
			...
			await foreach (long bucketField in buckets)
			{
				...
				IEnumerable<ScyllaReplicationLogEvent> events;
				try
				{
					events = await _mapper.FetchAsync<ScyllaReplicationLogEvent>("WHERE namespace = ? AND replication_bucket = ?", ns.ToString(), bucketField);
				}
				catch (QueryExecutionException e)
				{
					throw new RemoteServiceException("Error communicating with Scylla", e);
		...
		{
			IEnumerable<ScyllaSnapshot> snapshots;
			try
			{
				snapshots = await _mapper.FetchAsync<ScyllaSnapshot>("WHERE namespace = ?", ns.ToString());
			}
			catch (QueryExecutionException e)
			{
				throw new RemoteServiceException("Error communicating with Scylla", e);
		...
		public async IAsyncEnumerable<BlobReplicationLogEvent> GetBlobEventsAsync(NamespaceId ns, string replicationBucket)
		{
			...
			IEnumerable<ScyllaBlobReplicationLogEvent> events;
			try
			{
				long bucketId = replicationBucket.FromReplicationBucketIdentifier().ToFileTimeUtc();
				events = await _mapper.FetchAsync<ScyllaBlobReplicationLogEvent>("WHERE namespace = ? AND replication_bucket = ?", ns.ToString(), bucketId);
			}
			catch (QueryExecutionException e)
			{
				throw new RemoteServiceException("Error communicating with Scylla", e);
