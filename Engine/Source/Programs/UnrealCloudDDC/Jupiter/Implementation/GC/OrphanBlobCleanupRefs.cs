	...
	public class OrphanBlobCleanupRefs : IBlobCleanup
	{
		...
		public async Task<ulong> CleanupAsync(CancellationToken cancellationToken)
		{
			...
			foreach (NamespaceId @namespace in namespaces)
			{
				...
				// only consider blobs that have been around for long enough
				DateTime cutoff = DateTime.Now.AddMinutes(-1 * _gcSettings.CurrentValue.OrphanBlobCutoffDurationMinutes);
		...
		private async Task<bool> GCBlobAsync(string storagePool, List<NamespaceId> namespacesThatSharePool, BlobId blob, DateTime lastModifiedTime, CancellationToken cancellationToken)
		{
			...
			if (noReferences)
			{
				// this blob had no references pointing to it which is not what we would expect, we would expect it to have some reference but that this reference in turn didn't exist
				_logger.LogWarning("{Blob} from {StoragePool} was deleted and had no references to it. This is safe but a indication that data is added and never referenced", blob, storagePoolName);
			}

			if (_gcSettings.CurrentValue.AllowBlockMetadataCleanup)
			{
