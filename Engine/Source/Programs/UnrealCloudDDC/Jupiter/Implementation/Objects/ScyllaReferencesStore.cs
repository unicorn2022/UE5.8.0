...
namespace Jupiter.Implementation
{
	public class ScyllaReferencesStore : IReferencesStore
	{
		...
		private readonly ConcurrentDictionary<NamespaceId, ConcurrentDictionary<BucketId, DateTime>> _addedBuckets = new ConcurrentDictionary<NamespaceId, ConcurrentDictionary<BucketId, DateTime>>();
		...
		public ScyllaReferencesStore(IScyllaSessionManager scyllaSessionManager, IOptionsMonitor<ScyllaSettings> settings, INamespacePolicyResolver namespacePolicyResolver, Tracer tracer, ILogger<ScyllaReferencesStore> logger)
		{
			...
			_getNamespacesStatement = scyllaSessionManager.IsAWSKeyspace ? _session.Prepare("SELECT namespace FROM buckets_v2") :  _session.Prepare("SELECT DISTINCT namespace FROM buckets_v2");
			_getNamespacesOldStatement = scyllaSessionManager.IsAWSKeyspace ? _session.Prepare("SELECT namespace FROM buckets") : _session.Prepare("SELECT DISTINCT namespace FROM buckets");
		...
		public async Task<RefRecord> GetAsync(NamespaceId ns, BucketId bucket, RefId name, IReferencesStore.FieldFlags fieldFlags, IReferencesStore.OperationFlags opFlags, CancellationToken cancellationToken)
		{
			...
			try
			{
				...
				if (includePayload)
				{
					o = await _mapper.SingleOrDefaultAsync<ScyllaObject>("WHERE namespace = ? AND bucket = ? AND name = ?", ns.ToString(),
						bucket.ToString(), name.ToString());
				}
				else
				{
					string cqlOptions = "";
					...
					o = await _mapper.SingleOrDefaultAsync<ScyllaObject>(
						$"SELECT namespace, bucket, name , payload_hash, is_finalized, last_access_time FROM objects WHERE namespace = ? AND bucket = ? AND name = ? {cqlOptions}",
						ns.ToString(), bucket.ToString(), name.ToString());
				}
				...
				return new RefRecord(new NamespaceId(o.Namespace!), new BucketId(o.Bucket!), new RefId(o.Name!), o.LastAccessTime, o.InlinePayload,
					o.PayloadHash!.AsBlobIdentifier(), o.IsFinalized!.Value);
			}
		...
		public async Task PutAsync(NamespaceId ns, BucketId bucket, RefId name, BlobId blobHash, byte[] blob, bool isFinalized, bool allowOverwrite = false, CancellationToken cancellationToken = default)
		{
			...
			try
			{
				Task? insertLastAccess = gcMethod == NamespacePolicy.StoragePoolGCMethod.LastAccess
					? _mapper.InsertAsync<ScyllaObjectLastAccess>(new ScyllaObjectLastAccess(ns, bucket, name, DateTime.Now))
					: null;
				...
				if (!allowOverwrites)
				{
					...
					try
					{
						RefRecord oldRecord = await GetAsync(ns, bucket, name, IReferencesStore.FieldFlags.IncludePayload,
							IReferencesStore.OperationFlags.None, cancellationToken);
					...
					if (insertRef)
					{
						await _mapper.InsertAsync<ScyllaObject>(new ScyllaObject(ns, bucket, name, blob, blobHash, isFinalized), ttl: ttl,
							insertNulls: false);
					}
				}
				else
			...
			else
			{
				...
				do
				{
					...
					if (timeoutException != null)
					{
						...
						throw new RemoteServiceException("Error communicating with Scylla", timeoutException);
					}
		...
		public async IAsyncEnumerable<NamespaceId> GetNamespacesAsync([EnumeratorCancellation] CancellationToken cancellationToken)
		{
...

			{
				...
				try
				{
					rowSet = await _session.ExecuteAsync(_getNamespacesStatement.Bind());
				}
				catch (QueryExecutionException e)
				{
					throw new RemoteServiceException("Error communicating with Scylla", e);
				...
				if (_scyllaSessionManager.IsAWSKeyspace)
				{
					// keyspaces do not support the distinct keyword so we filter here on the client instead

					List<NamespaceId> namespaces = new List<NamespaceId>();
					foreach (Row row in rowSet)
					{
					...
					foreach (NamespaceId namespaceId in namespaces)
					{
			...
			if (_settings.CurrentValue.ListObjectsFromOldNamespaceTable)
			{
				...
				try
				{
					rowSet = await _session.ExecuteAsync(_getNamespacesOldStatement.Bind());
				}
				catch (QueryExecutionException e)
				{
					throw new RemoteServiceException("Error communicating with Scylla", e);
		...
		{
			IEnumerable<ScyllaBucket> buckets;
			
			try
			{
				buckets = await _mapper.FetchAsync<ScyllaBucket>("WHERE namespace=?", ns.ToString());
			}
			catch (QueryExecutionException e)
			{
				throw new RemoteServiceException("Error communicating with Scylla", e);
			...
			foreach (ScyllaBucket scyllaBucket in buckets)
			{
		...
		private async Task AddBucketAsync(NamespaceId ns, BucketId bucket)
		{
			...
			ConcurrentDictionary<BucketId, DateTime> addedBuckets = _addedBuckets.GetOrAdd(ns, id => new ConcurrentDictionary<BucketId, DateTime>());

			DateTime now = DateTime.UtcNow;
			if (addedBuckets.TryGetValue(bucket, out DateTime added))
			{
				if (added > now - TimeSpan.FromHours(1))
				{
					return;
				}
			}
			...
			try
			{
				...
				await addTask;
				addedBuckets.AddOrUpdate(bucket, now, (_, _) => now);
			}
			catch (QueryExecutionException e)
			{
				throw new RemoteServiceException("Error communicating with Scylla", e);
			...
			try
			{
				...
				// build lookup of which namespaces and buckets are actually references by live objects
				{
					...
					foreach (Row row in rowSet)
					{
						...
						ConcurrentDictionary<BucketId, bool> bucketTracking =
							namespacesWithRefs.GetOrAdd(ns, new ConcurrentDictionary<BucketId, bool>());
				...
				await foreach (NamespaceId ns in GetNamespacesAsync(CancellationToken.None))
				{
					...
					await foreach (BucketId bucket in GetBucketsAsync(ns, CancellationToken.None))
					{
						...
						{
							await _session.ExecuteAsync(new SimpleStatement("DELETE FROM buckets_v2 WHERE namespace = ? and bucket = ?",
								ns.ToString(), bucket.ToString()));
						}
					}
