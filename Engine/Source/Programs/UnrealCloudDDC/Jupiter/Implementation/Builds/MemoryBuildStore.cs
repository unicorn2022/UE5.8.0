...
public class MemoryBuildStore : IBuildStore
{
	...
	public Task PutBuildAppendixAsync(NamespaceId ns, BucketId bucket, CbObjectId buildId, string appendixName, uint ttl)
	{
		string key = BuildKey(ns, bucket, buildId);
		...
		build.BuildAppendix[appendixName] = appendixName;
		return Task.CompletedTask;
	}
	...
	public async IAsyncEnumerable<string> GetBuildAppendixesAsync(NamespaceId ns, BucketId bucket, CbObjectId buildId)
	{
		...
		if (_builds.TryGetValue(key, out MemoryBuildRecord? value))
		{
			foreach (string appendixName in value.BuildAppendix.Keys)
			{
				yield return appendixName;
			}
		}
	...
	public Task DeleteBuildAppendixAsync(NamespaceId ns, BucketId bucket, CbObjectId buildId, string appendixFileName)
	{
		string key = BuildKey(ns, bucket, buildId);

		if (_builds.TryGetValue(key, out MemoryBuildRecord? value))
		{
			value.BuildAppendix.Remove(appendixFileName, out _);
		}
	...
	public async IAsyncEnumerable<(NamespaceId, BucketId, CbObjectId, bool)> ListAllBuildsAsync([EnumeratorCancellation] CancellationToken cancellationToken)
	{
		...
		foreach (MemoryBuildRecord build in _builds.Values.OrderByDescending(record => record.CreationDate))
		{
			yield return (build.Namespace, build.Bucket, build.BuildId, build.IsFinalized);
		}
	}
...
internal class MemoryBuildRecord
{
	...
	public ConcurrentDictionary<string, string> BuildAppendix { get; set; } = new();
