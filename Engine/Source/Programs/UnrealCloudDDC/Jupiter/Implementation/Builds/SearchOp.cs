...
namespace Jupiter.Implementation.Builds
{
	...
	class CompareOp : ISearchOp
	{
		...
		private static bool CbFieldEquals(CbField cbField, CbField expectedField)
		{
			if (cbField.IsString() && expectedField.IsString())
			{
				// special case handle string comparisons so that they are not case-sensitive
				string a = cbField.AsString();
				string b = expectedField.AsString();
				return string.Equals(a, b, StringComparison.OrdinalIgnoreCase);
			}

			// check for string and number comparisons to allow mixing string and number representation of fields
			bool? CheckStringAndNumberEquality(CbField p0, CbField p1)
			{
				if (p0.IsString())
				{
					string s = p0.AsString();
					if (p1.IsInteger())
					{
						long a = p1.AsInt64();
						if (long.TryParse(s, CultureInfo.InvariantCulture, out long b))
						{
							return a.Equals(b);
						}
					}
					if (p1.IsFloat())
					{
						double a = p1.AsDouble();
						if (double.TryParse(s, CultureInfo.InvariantCulture, out double b))
						{
							return a.Equals(b);
						}
					}
			...
			bool? equality = CheckStringAndNumberEquality(cbField, expectedField);
			if (equality.HasValue)
			{
				return equality.Value;
			}
			equality = CheckStringAndNumberEquality(expectedField, cbField);
			if (equality.HasValue)
			{
				return equality.Value;
			}

			return cbField.ValueEquals(expectedField);
		}
