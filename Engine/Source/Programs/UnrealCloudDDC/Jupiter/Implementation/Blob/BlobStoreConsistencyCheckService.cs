		...
		private async Task RunConsistencyCheckAsync(CancellationToken cancellationToken)
		{
			foreach (IBlobStore blobStore in BlobService.GetBlobStores(_provider, _unrealCloudDDCSettings).Where(RunConsistencyCheckOnBlobStore))
			{
				...
				foreach (NamespaceId ns in namespaces)
				{
					...
					await foreach ((BlobId blob, DateTime lastModified) in blobStore.ListObjectsAsync(ns, cancellationToken))
					{
						...
						BlobContents contents = await blobStore.GetObjectAsync(ns, blob, LastAccessTrackingFlags.SkipTracking, cancellationToken: cancellationToken);
						...
						if (!blob.Equals(newHash))
						{
							...
							await blobStore.DeleteObjectAsync(ns, blob, cancellationToken);
