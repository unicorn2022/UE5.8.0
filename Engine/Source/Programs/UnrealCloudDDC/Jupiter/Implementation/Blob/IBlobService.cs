...
public interface IBlobService
{
	...
	Task<(string?, string?)> StartMultipartUploadAsync(NamespaceId ns, CancellationToken cancellationToken = default);
	Task CompleteMultipartUploadAsync(NamespaceId ns, string blobName, string uploadId, List<string> partIds, CancellationToken cancellationToken = default);
	Task PutMultipartUploadAsync(NamespaceId ns, string blobName, string uploadId, string partIdentifier, byte[] blobData, CancellationToken cancellationToken = default);
	Task<Uri?> MaybePutMultipartUploadWithRedirectAsync(NamespaceId ns, string blobName, string uploadId, string partId, CancellationToken cancellationToken = default);
	...
	List<MultipartByteRange> GetMultipartRanges(NamespaceId ns, string blobName, string uploadId, ulong blobLength, CancellationToken cancellationToken = default);
	MultipartLimits? GetMultipartLimits(NamespaceId ns);
	Task CopyBlobAsync(NamespaceId ns, NamespaceId targetNamespace, BlobId blobId, BucketId? bucketHint = null, CancellationToken cancellationToken = default);
}
...
public static class BlobServiceExtensions
{
	public static async Task<(ContentId, BlobId)> PutCompressedObjectMetadataAsync(this IBlobService blobService, NamespaceId ns, IBufferedPayload payload, ContentId? id, IServiceProvider provider, CancellationToken cancellationToken)
	{
		...
		ulong compressedSize = (ulong)payload.Length;
		await contentIdStore.PutAsync(ns, identifierDecompressedPayload, new BlobId[] {identifierCompressedPayload}, compressedSize, decompressedSize, cancellationToken);
