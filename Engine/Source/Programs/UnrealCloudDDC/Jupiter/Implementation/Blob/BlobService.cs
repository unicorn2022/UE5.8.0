...
public class BlobService : IBlobService
{
	...
	public async Task<BlobId> PutObjectAsync(NamespaceId ns, IBufferedPayload payload, BlobId identifier, BucketId? bucketHint = null, bool? bypassCache = null, CancellationToken cancellationToken = default)
	{
		...
		await _blobIndex.AddBlobToIndexAsync(ns, id, (ulong)payload.Length, cancellationToken: cancellationToken);
	...
	public async Task<Uri?> MaybePutObjectWithRedirectAsync(NamespaceId ns, BlobId identifier, BucketId? bucketHint = null, CancellationToken cancellationToken = default)
	{
		...
		IBlobStore store = _blobStores.Last();
		{
			...
			Uri? redirectUri = await store.PutObjectWithRedirectAsync(ns, identifier, cancellationToken);
			if (redirectUri != null)
			{
	...
	public async Task<BlobId> PutObjectKnownHashAsync(NamespaceId ns, IBufferedPayload content, BlobId identifier, BucketId? bucketHint, bool? bypassCache = null, CancellationToken cancellationToken = default)
	{
		...
		await _blobIndex.AddBlobToIndexAsync(ns, identifier, (ulong)content.Length, cancellationToken: cancellationToken);
	...
	private async Task<BlobId> PutObjectToStoresAsync(NamespaceId ns, IBufferedPayload bufferedPayload, BlobId identifier, BucketId? bucketHint, bool? bypassCache, CancellationToken cancellationToken = default)
	{
		...
		foreach (IBlobStore store in _blobStores)
		{
			...
			await store.PutObjectAsync(ns, s, identifier, cancellationToken);
		}
		...
		if (policy.PopulateFallbackNamespaceOnUpload && policy.FallbackNamespace.HasValue)
		{
			...
			await _blobIndex.AddBlobToIndexAsync(policy.FallbackNamespace.Value, identifier, (ulong)bufferedPayload.Length, cancellationToken: cancellationToken);
		}
	...
	public async Task<BlobContents> GetObjectAsync(NamespaceId ns, BlobId blob, List<string>? storageLayers = null, bool supportsRedirectUri = false, bool allowOndemandReplication = true, BucketId? bucketHint = null, bool bypassCache = false, CancellationToken cancellationToken = default)
	{
		...
		catch (BlobNotFoundException)
		{
			...
			if (policy.FallbackNamespace != null)
			{
				...
				AccessScope accessScope = bucketHint == null ? new AccessScope(policy.FallbackNamespace) : new AccessScope(policy.FallbackNamespace, bucketHint.Value); 
				ActionResult? result = await _requestHelper.HasAccessToScopeAsync(user, request, accessScope, new[] { JupiterAclAction.ReadObject });
				if (result != null)
				{
	...
	private async Task<BlobContents> GetObjectFromStoresAsync(NamespaceId ns, BlobId blob, List<string>? storageLayers = null, bool supportsRedirectUri = false, CancellationToken cancellationToken = default)
	{
		...
		foreach (IBlobStore store in _blobStores)
		{
			...
			try
			{
				blobContents = await store.GetObjectAsync(ns, blob, supportsRedirectUri: supportsRedirectUri, cancellationToken: cancellationToken);
		...
		if (numStoreMisses >= 1 && blobContents.RedirectUri == null)
		{
			...
			{
				if (numStoreMisses == 1 && blobContents.BufferedPayload is FilesystemBufferedPayload payload && _blobStores[0] is FileSystemStore fileSystemStore)
				{
					IBlobStore blobStore = _blobStores[0];
					using TelemetrySpan scope = _tracer.StartActiveSpan("HierarchicalStore.PopulateStore")
						...
						.SetAttribute("Mode", "MoveBufferedPayload")
						.SetAttribute("BlobStore", blobStore.GetType().Name);

					// blob is buffered on filesystem and needs to be written into the filesystem cache only this allows us to move the file instead of copying
					await fileSystemStore.MoveBufferedPayloadAsync(ns, blob, payload, cancellationToken: cancellationToken);

					return await blobStore.GetObjectAsync(ns, blob, cancellationToken: cancellationToken);
				}
			}

			// {
			// 	if (numStoreMisses == 1 && _blobStores[0] is FileSystemStore fileSystemStore)
			// 	{
			// 		// if we are only missing data from the filesystem store, buffer to it and then send using the data in the filesystem so that we can use sendfile for this data
			// 		using TelemetrySpan scope = _tracer.StartActiveSpan("HierarchicalStore.PopulateStore")
			// 			.SetAttribute("operation.name", "HierarchicalStore.PopulateStore")
			// 			.SetAttribute("Mode", "UseFilesystemPayload")
			// 			.SetAttribute("BlobStore", nameof(FileSystemStore));
			//
			// 		await using Stream s = blobContents.Stream;
			// 		await fileSystemStore.PutObjectAsync(ns, s, blob, cancellationToken);
			//
			// 		return await fileSystemStore.GetObjectAsync(ns, blob, flags: LastAccessTrackingFlags.DoTracking, cancellationToken: cancellationToken);
			// 	}
			// }
			...
			for (int i = 0; i < numStoreMisses; i++)
			{
				...
				try
				{
					...
					await blobStore.PutObjectAsync(ns, s, blob, cancellationToken);
				}
				finally
				{
					if (s != null)
	...
	public async Task<Uri?> GetObjectWithRedirectAsync(NamespaceId ns, BlobId blob, List<string>? storageLayers = null, CancellationToken cancellationToken = default)
	{
		...
		foreach (IBlobStore store in _blobStores)
		{
			...
			try
			{
				redirectUri = await store.GetObjectByRedirectAsync(ns, blob, cancellationToken);
				scope.SetAttribute("ObjectFound", true.ToString());
				break;
			}
			catch (BlobNotFoundException)
	...
	public async Task<BlobMetadata> GetObjectMetadataAsync(NamespaceId ns, BlobId blobId, CancellationToken cancellationToken)
	{
		...
		foreach (IBlobStore store in _blobStores)
		{
			...
			try
			{
				BlobMetadata metadata = await store.GetObjectMetadataAsync(ns, blobId, cancellationToken);
				scope.SetAttribute("ObjectFound", true.ToString());
				return metadata;
			}
			catch (BlobNotFoundException)
	...
	public async Task<BlobContents> ReplicateObjectAsync(NamespaceId ns, BlobId blob, bool supportsRedirectUri = false, bool force = false, BucketId? bucketHint = null, CancellationToken cancellationToken = default)
	{
		...
		foreach ((int latency, string? region) in possiblePeers)
		{
			...
			using ServerTimingMetricScoped? serverTimingScope = serverTiming?.CreateServerTimingMetricScope($"blob.replicate.{region}", "Replicating blob from remote instances");
			...
			using HttpRequestMessage blobRequest = await BuildHttpRequestAsync(HttpMethod.Get, new Uri($"{url}api/v1/blobs/{ns}/{blob}?allowOndemandReplication=false"), cancellationToken);
	...
	public async Task<(string?, string?)> StartMultipartUploadAsync(NamespaceId ns, CancellationToken cancellationToken = default)
	{
		foreach (IBlobStore store in _blobStores)
		{
			if (store is IMultipartBlobStore multipartBlobStore)
			{
				...
				return (await multipartBlobStore.StartMultipartUploadAsync(ns, blobName, cancellationToken), blobName);
			}
		}
	...
	public async Task CompleteMultipartUploadAsync(NamespaceId ns, string blobName, string uploadId, List<string> partIds, CancellationToken cancellationToken = default)
	{
		foreach (IBlobStore store in _blobStores)
		{
			if (store is IMultipartBlobStore multipartBlobStore)
			{
				...
				await multipartBlobStore.CompleteMultipartUploadAsync(ns, blobName, uploadId, partIds, cancellationToken);
				return;
			}
		}
	...
	public async Task PutMultipartUploadAsync(NamespaceId ns, string blobName, string uploadId, string partIdentifier, byte[] blobData, CancellationToken cancellationToken = default)
	{
		foreach (IBlobStore store in _blobStores)
		{
			if (store is IMultipartBlobStore multipartBlobStore)
			{
				...
				await multipartBlobStore.PutMultipartPartAsync(ns, blobName, uploadId, partIdentifier, blobData, cancellationToken);
				return;
			}
		}
	...
	public async Task<Uri?> MaybePutMultipartUploadWithRedirectAsync(NamespaceId ns, string blobName, string uploadId, string partIdentifier, CancellationToken cancellationToken = default)
	{
		foreach (IBlobStore store in _blobStores)
		{
			if (store is IMultipartBlobStore multipartBlobStore)
			{
				...
				return await multipartBlobStore.GetWriteRedirectForPartAsync(ns, blobName, uploadId, partIdentifier, cancellationToken);
			}
		}
	...
	public async Task<(ContentId, BlobId)> VerifyMultipartUpload(NamespaceId ns, BlobId id, string blobName, bool isCompressed, CancellationToken cancellationToken = default)
	{
		...
		BlobContents? blobContents = await blobStore.GetMultipartObjectByNameAsync(ns, blobName, cancellationToken);
		...
		if (isCompressed)
		{
			...
			await _blobIndex.AddBlobToIndexAsync(ns, blobId, rawSize: (ulong)bufferedPayload.Length, cancellationToken: cancellationToken);
		}
		else
		{
		...
		await blobStore.RenameMultipartBlobAsync(ns, blobName, blobId, cancellationToken);
		
		return (cid, blobId);

	}

	public List<MultipartByteRange> GetMultipartRanges(NamespaceId ns, string uploadId, string blobName, ulong blobLength, CancellationToken cancellationToken = default)
	{
		IMultipartBlobStore? blobStore = null;
		foreach (IBlobStore store in _blobStores)
		{
		...
		return blobStore.GetMultipartRanges(ns, uploadId, blobLength, cancellationToken);
	}
	...
	public async Task CopyBlobAsync(NamespaceId ns, NamespaceId targetNamespace, BlobId blobId, BucketId? bucketHint = null, CancellationToken cancellationToken = default)
	{
		...
		ulong rawSize = await rootStore.CopyBlobAsync(ns, targetNamespace, blobId, cancellationToken);

		await _blobIndex.AddBlobToIndexAsync(targetNamespace, blobId, rawSize: rawSize, cancellationToken: cancellationToken);
		await _replicationLog.InsertAddBlobEventAsync(targetNamespace, blobId, DateTime.UtcNow.ToReplicationBucket(), bucketHint: bucketHint);
	}

	private async Task<HttpRequestMessage> BuildHttpRequestAsync(HttpMethod httpMethod, Uri uri, CancellationToken cancellationToken)
	{
		...
		else
		{
			foreach (IBlobStore store in _blobStores)
			{
				...
				using TelemetrySpan scope = _tracer.StartActiveSpan("HierarchicalStore.ObjectExists")
					...
					.SetAttribute("BlobStore", blobStoreName)
					;
				if (await store.ExistsAsync(ns, blob, cancellationToken: cancellationToken))
				{
					scope.SetAttribute("ObjectFound", true.ToString());
					return true;
				}
	...
	public async Task DeleteObjectAsync(NamespaceId ns, BlobId blob, CancellationToken cancellationToken = default)
	{
		...
		if (_settings.CurrentValue.RemoveReferencesOnBlobDeletion)
		{
		...
		foreach (IBlobStore store in _blobStores)
		{
			try
			{
				...
				await store.DeleteObjectAsync(ns, blob, cancellationToken);
	...
	public async Task DeleteObjectAsync(List<NamespaceId> namespaces, BlobId blob, CancellationToken cancellationToken = default)
	{
		...
		{
			await _blobIndex.RemoveBlobFromAllRegionsAsync(ns, blob, cancellationToken: token);
			if (_settings.CurrentValue.RemoveReferencesOnBlobDeletion)
			{
				await _blobIndex.RemoveReferencesAsync(ns, blob, null, token);
			}
		});
		...
		foreach (IBlobStore store in _blobStores)
		{
			...
			await store.DeleteObjectAsync(namespaces, blob, cancellationToken);
		}
	}
	...
	public async Task DeleteNamespaceAsync(NamespaceId ns, CancellationToken cancellationToken = default)
	{
		...
		foreach (IBlobStore store in _blobStores)
		{
			...
			try
			{
				await store.DeleteNamespaceAsync(ns, cancellationToken);
				deletedAtLeastOnce = true;
			}
			catch (NamespaceNotFoundException)
			{
	...
	public IAsyncEnumerable<(BlobId, DateTime)> ListObjectsAsync(NamespaceId ns, CancellationToken cancellationToken)
	{
		...
		return _blobStores.Last().ListObjectsAsync(ns, cancellationToken);
	}
