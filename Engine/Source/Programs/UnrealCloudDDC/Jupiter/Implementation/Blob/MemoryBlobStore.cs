...
namespace Jupiter.Implementation
{
	internal class MemoryBlobStore : IBlobStore
	{
		...
		public Task<BlobId> PutObjectAsync(NamespaceId ns, byte[] blob, BlobId identifier, CancellationToken cancellationToken = default)
		{
			...
			return PutObjectAsync(ns, stream, identifier, cancellationToken);
		}
		...
		{
			if (await GetBackend(targetNamespace).ExistsAsync(GetPath(blobId), cancellationToken))
			{
				// blob already exists, do not copy
				return (ulong)(await GetBackend(targetNamespace).GetObjectMetadataAsync(GetPath(blobId)))!.Length;
			}

			BlobContents? contents = await GetBackend(ns).TryReadAsync(GetPath(blobId), cancellationToken: cancellationToken);
			if (contents == null)
			{
				throw new BlobNotFoundException(ns, blobId);
			}

			await PutObjectAsync(targetNamespace, contents.Stream, blobId, cancellationToken);

			return (ulong)contents.Stream.Length;
		}
		...
		public async Task<BlobId> PutObjectAsync(NamespaceId ns, ReadOnlyMemory<byte> blob, BlobId identifier, CancellationToken cancellationToken = default)
		{
			return await PutObjectAsync(ns, blob: blob.ToArray(), identifier, cancellationToken);
		}

		public async Task<BlobId> PutObjectAsync(NamespaceId ns, Stream blob, BlobId identifier, CancellationToken cancellationToken = default)
		{
			IStorageBackend backend = GetBackend(ns);
			await backend.WriteAsync(GetPath(identifier), blob, cancellationToken);
			return identifier;
		}
		...
		{
			BlobContents? contents = await GetBackend(ns).TryReadAsync(GetPath(blob), flags, cancellationToken);
			if (contents == null)
			{
		...
		public Task DeleteObjectAsync(NamespaceId ns, BlobId blob, CancellationToken cancellationToken = default) => GetBackend(ns).DeleteAsync(GetPath(blob), cancellationToken);
		...
		public Task DeleteObjectAsync(IEnumerable<NamespaceId> namespaces, BlobId blob, CancellationToken cancellationToken = default)
		{
			...
			{
				DeleteObjectAsync(ns, blob, cancellationToken);
			}

			return Task.CompletedTask;
		}

		public Task<bool> ExistsAsync(NamespaceId ns, BlobId blob, bool forceCheck = false, CancellationToken cancellationToken = default) => GetBackend(ns).ExistsAsync(GetPath(blob), cancellationToken);
