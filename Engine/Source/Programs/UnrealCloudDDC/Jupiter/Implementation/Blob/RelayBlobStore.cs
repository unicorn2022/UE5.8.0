...
namespace Jupiter.Implementation
{
	public class RelayBlobStore : RelayStore, IBlobStore
	{
		...
		public async Task<BlobId> PutObjectAsync(NamespaceId ns, byte[] blob, BlobId identifier, CancellationToken cancellationToken = default)
		{
		...
		{
			return PutObjectAsync(ns, blob.ToArray(), identifier, cancellationToken);
		}

		public async Task<BlobId> PutObjectAsync(NamespaceId ns, Stream content, BlobId identifier, CancellationToken cancellationToken = default)
		{
			using HttpRequestMessage putObjectRequest = await BuildHttpRequestAsync(HttpMethod.Put, new Uri($"api/v1/blobs/{ns}/{identifier}", UriKind.Relative));
		...
		public async Task<bool> ExistsAsync(NamespaceId ns, BlobId blob, bool forceCheck, CancellationToken cancellationToken = default)
		{
			...
			HttpResponseMessage response = await HttpClient.SendAsync(headObjectRequest, cancellationToken);
			if (response.StatusCode == HttpStatusCode.NotFound)
			{
				return false;
			}
