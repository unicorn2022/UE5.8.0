...
public class ScyllaBlobIndex : IBlobIndex
{
	...
	public async Task AddBlobToIndexAsync(NamespaceId ns, BlobId id, ulong rawSize, string? region = null,
	...
	public async Task<bool> BlobExistsInRegionAsync(NamespaceId ns, BlobId blobIdentifier, string? region = null, CancellationToken cancellationToken = default)
	{
		...
		if (blobMissing && _scyllaSettings.CurrentValue.MigrateFromOldBlobIndex)
		{
			...
			{
				await AddBlobToIndexAsync(ns, blobIdentifier, rawSize: 0, region: oldRegion, cancellationToken: cancellationToken);
			}
	...
	public async IAsyncEnumerable<(NamespaceId, BlobId)> GetAllBlobsAsync([EnumeratorCancellation] CancellationToken cancellationToken)
	{
		...
		IEnumerable<ScyllaBlobIndexEntry> entries;
		try
		{
			entries = await _mapper.FetchAsync<ScyllaBlobIndexEntry>(cqlOptions);
		}
		catch (QueryExecutionException e)
		{
			throw new RemoteServiceException("Error communicating with Scylla", e);
		...
		foreach (ScyllaBlobIndexEntry blobIndex in entries)
		{
	...
	public async IAsyncEnumerable<(NamespaceId, BaseBlobReference)> GetAllBlobReferencesAsync([EnumeratorCancellation] CancellationToken cancellationToken = default)
	{
		...
		foreach (int index in tableRangeIndices)
		{
			...
			try
			{
				rowSet = await _session.ExecuteAsync(_getBlobReferenceStatement.Bind(range.Item1, range.Item2));
			}
			catch (QueryExecutionException e)
			{
				throw new RemoteServiceException("Error communicating with Scylla", e);
	...
	public async Task<List<string>> GetBlobRegionsAsync(NamespaceId ns, BlobId blob, CancellationToken cancellationToken)
	{
		...
		try
		{
			entries = await _mapper.FetchAsync<ScyllaBlobIndexEntry>("WHERE namespace = ? AND blob_id = ?", ns.ToString(), blob.HashData);
		}
		catch (QueryExecutionException e)
		{
			throw new RemoteServiceException("Error communicating with Scylla", e);
		...
		if (regions.Count == 0 && _scyllaSettings.CurrentValue.MigrateFromOldBlobIndex)
		{
			...
			{
				await AddBlobToIndexAsync(ns, blob, rawSize: 0, region: oldRegion, cancellationToken: cancellationToken);
			}
	...
	public async IAsyncEnumerable<BaseBlobReference> GetBlobReferencesAsync(NamespaceId ns, BlobId id, [EnumeratorCancellation] CancellationToken cancellationToken)
	{
		...
		IEnumerable<ScyllaBlobIncomingReference>? references = null;
		...
		for (int i = 0; i < MaxRetryAttempts; i++)
		{
			try
			{
				...
				references = await _mapper.FetchAsync<ScyllaBlobIncomingReference>("WHERE namespace = ? AND blob_id = ? " + cqlOptions, ns.ToString(), id.HashData);
				break;
			}
			catch (ReadTimeoutException e)
			{
				// wait 2 seconds and try again as the Db is under heavy load right now
		...
		if (references == null)
		{
			throw new NotImplementedException("References not set, unhandled exception?");
		}

		foreach (ScyllaBlobIncomingReference incomingReference in references)
		{
	...
	public async Task AddRefToBlobsAsync(NamespaceId ns, BucketId bucket, RefId key, BlobId[] blobs, CancellationToken cancellationToken)
	{
		...
		{
			if (e.InnerExceptions.Any(exception => exception.GetType() == typeof(QueryExecutionException)))
			{
				throw new RemoteServiceException("Error communicating with Scylla", e);
			}
