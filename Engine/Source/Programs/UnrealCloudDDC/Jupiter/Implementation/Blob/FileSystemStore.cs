...
namespace Jupiter.Implementation
{
	public class FileSystemStore : IBlobStore, IBlobCleanup
	{
		...
		public async Task<BlobId> PutObjectAsync(NamespaceId ns, ReadOnlyMemory<byte> content, BlobId blobIdentifier, CancellationToken cancellationToken = default)
		{
			using EpicGames.Core.ReadOnlyMemoryStream stream = new EpicGames.Core.ReadOnlyMemoryStream(content);
			return await PutObjectAsync(ns, stream, blobIdentifier, cancellationToken);
		}

		public async Task<BlobId> PutObjectAsync(NamespaceId ns, Stream content, BlobId blobIdentifier, CancellationToken cancellationToken = default)
		{
			string path = GetFilesystemPath(blobIdentifier);
			await GetBackend(ns).WriteAsync(path, content, cancellationToken);
			return blobIdentifier;
		}

		public async Task<BlobId> PutObjectAsync(NamespaceId ns, byte[] content, BlobId blobIdentifier, CancellationToken cancellationToken = default)
		{
			using MemoryStream stream = new MemoryStream(content);
			return await PutObjectAsync(ns, stream, blobIdentifier, cancellationToken);
		}

		public async Task<BlobContents> GetObjectAsync(NamespaceId ns, BlobId blob, LastAccessTrackingFlags flags, bool supportsRedirectUri = false, CancellationToken cancellationToken = default)
		{
			string path = GetFilesystemPath(blob);

			BlobContents? contents = await GetBackend(ns).TryReadAsync(path, flags, cancellationToken);
			if (contents == null)
			{
		...
		public async Task<bool> ExistsAsync(NamespaceId ns, BlobId blob, bool forceCheck, CancellationToken cancellationToken = default)
		{
			string path = GetFilesystemPath(blob);
			return await GetBackend(ns).ExistsAsync(path, cancellationToken);
		}

		public async Task DeleteObjectAsync(NamespaceId ns, BlobId objectName, CancellationToken cancellationToken = default)
		{
			string path = GetFilesystemPath(objectName);
			await GetBackend(ns).DeleteAsync(path, cancellationToken);
		}

		public async Task DeleteObjectAsync(IEnumerable<NamespaceId> namespaces, BlobId objectName, CancellationToken cancellationToken = default)
		{
			string path = GetFilesystemPath(objectName);
		...
		public async Task<ulong> CleanupAsync(CancellationToken cancellationToken = default)
		{
		...
		/// <returns></returns>
		public async Task<ulong> CleanupInternalAsync(CancellationToken cancellationToken = default)
		{
		...
		private async Task<ulong> RunGCAsync(NamespaceId? ns, CancellationToken cancellationToken = default)
		{
			ulong countOfBlobsRemoved = 0;
		...
		public async Task MoveBufferedPayloadAsync(NamespaceId ns, BlobId blobId, FilesystemBufferedPayload bufferedPayload, CancellationToken cancellationToken)
		{
			string path = GetFilesystemPath(blobId);
			await GetBackend(ns).MoveBufferedPayloadAsync(path, bufferedPayload, cancellationToken);
		}
	}
	...
	public class FileStorageBackend : IStorageBackend
	{
		...
		public async Task WriteAsync(string path, Stream content, CancellationToken cancellationToken = default)
		{
			...
			if (!File.Exists(filePath.FullName))
			{
				...
				catch (UnauthorizedAccessException) when (File.Exists(filePath.FullName))
				{
				}
		...
		public Task<BlobContents?> TryReadAsync(string path, LastAccessTrackingFlags flags, CancellationToken cancellationToken = default)
		{
			FileInfo filePath = GetFilesystemPath(path);
			...
			return Task.FromResult<BlobContents?>(new BlobContents(fs, fs.Length, localPath: $"{_namespaceId}/{path}", absolutePath: filePath.FullName));
		}
		...
		public Task<bool> ExistsAsync(string path, CancellationToken cancellationToken = default)
		{
			...
			return Task.FromResult(File.Exists(filePath.FullName));
		}
		...
		public IAsyncEnumerable<(string, DateTime)> ListAsync(CancellationToken cancellationToken = default)
		{
		...
		public Task MoveBufferedPayloadAsync(string path, FilesystemBufferedPayload bufferedPayload, CancellationToken cancellationToken)
		{
			FileInfo fileInfo = GetFilesystemPath(path);
			...
			try
			{
				File.Move(bufferedPayload.TempFile.FullName, fileInfo.FullName, true);
			}
			catch (IOException) when (File.Exists(fileInfo.FullName))
			{
			}

			UpdateLastWriteTime(fileInfo.FullName, DateTime.UnixEpoch);
