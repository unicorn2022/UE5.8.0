...
namespace Jupiter.Implementation
{
	public class NginxRedirectHelper
	{
		...
		public bool CanRedirect(HttpRequest request, BlobContents blobContents)
		{
			...
			// suppress warning about accepting paths from user requests while this is technically based on a blob name send by the user that name has to be a hash and the actual path is built from the hash
#pragma warning disable CA3003 
			if (!File.Exists(blobContents.AbsolutePath))
			{
				return false;
			}
