...
namespace Jupiter.Implementation
{
	public class CacheContentIdStore : RelayStore, IContentIdStore
	{
		...
		public async Task PutAsync(NamespaceId ns, ContentId contentId, BlobId[] blobIdentifiers, ulong compressedSize, ulong rawSize, CancellationToken cancellationToken)
		{
			foreach (BlobId blobIdentifier in blobIdentifiers)
			{
				using HttpRequestMessage putContentIdRequest = await BuildHttpRequestAsync(HttpMethod.Put, new Uri($"api/v1/content-id/{ns}/{contentId}/update/{blobIdentifier}/{compressedSize}/{rawSize}", UriKind.Relative));
