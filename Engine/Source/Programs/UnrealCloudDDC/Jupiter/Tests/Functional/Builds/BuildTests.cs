	...
	public abstract class BuildTests
	{
		...
		protected virtual async Task SeedAsync(IServiceProvider services)
		{
			...
			// compress the content and use that
			{
				...
				CompressedBuffer.CompressContent(ms, OodleCompressorType.Kraken, OodleCompressionLevel.Fast, looseFileContents);
				_looseFileContents = ms.ToArray();
			}

			{
				using MemoryStream ms = new MemoryStream();
				CompressedBuffer.CompressContent(ms, OodleCompressorType.Kraken, OodleCompressionLevel.Fast, smallFile1Contents);
				_smallFile1Contents = ms.ToArray();
			}

			{
				using MemoryStream ms = new MemoryStream();
				CompressedBuffer.CompressContent(ms, OodleCompressorType.Kraken, OodleCompressionLevel.Fast, smallFile2Contents);
				_smallFile2Contents = ms.ToArray();
			}

			{
				using MemoryStream ms = new MemoryStream();
				CompressedBuffer.CompressContent(ms, OodleCompressorType.Kraken, OodleCompressionLevel.Fast, smallFile3Contents);
				_smallFile3Contents = ms.ToArray();
			}

			{
				using MemoryStream ms = new MemoryStream();
				CompressedBuffer.CompressContent(ms, OodleCompressorType.Kraken, OodleCompressionLevel.Fast, largeFileContents);
			...
			{
				using MemoryStream ms = new MemoryStream();
				CompressedBuffer.CompressContent(ms, OodleCompressorType.Kraken, OodleCompressionLevel.Fast, block1Contents);
		...
		public async Task GetPutDeleteNewBuildAsync()
		{
			...
			// upload a file appendix
			{
				using HttpContent requestContent = new StringContent("{\"value\":\"foo\"}");
				requestContent.Headers.ContentType = new MediaTypeHeaderValue(MediaTypeNames.Application.Json);

				using HttpResponseMessage result = await _httpClient!.PutAsync(new Uri($"api/v2/builds/{TestNamespace}/{BucketId}/{buildId}/appendix/test.json", UriKind.Relative), requestContent);
				await result.EnsureSuccessStatusCodeWithMessageAsync();
				PutObjectResponse response = await result.Content.ReadAsAsync<PutObjectResponse>();
			...
			// fetch the build appendix
			{
				using HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Get, new Uri($"api/v2/builds/{TestNamespace}/{BucketId}/{buildId}/appendix/test.json", UriKind.Relative));
				request.Headers.Add("Accept", MediaTypeNames.Application.Json);
		...
		public async Task GetPutDeleteCopyNewNamespaceBuildAsync()
		{
			...
			// fetch the submitted build appendix
			{
				using HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Get, new Uri($"api/v2/builds/{TestNamespace}/{BucketId}/{buildId}/appendix/test.json", UriKind.Relative));
				request.Headers.Add("Accept", MediaTypeNames.Application.Json);
			...
			// fetch the copied build appendix
			{
				using HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Get, new Uri($"api/v2/builds/{TestNamespaceCopy}/{copyBuildBucketId}/{copyBuildId}/appendix/test.json", UriKind.Relative));
				request.Headers.Add("Accept", MediaTypeNames.Application.Json);
		...
		public async Task ListAndUseBlocksCompressedBufferAsync()
		{
			...
			{
				using MemoryStream ms = new MemoryStream();
				CompressedBuffer.CompressContent(ms, OodleCompressorType.Kraken, OodleCompressionLevel.Fast, newBlockContents);
		...
		public async Task SearchBuildNoBucketJsonAsync()
		{
			// Null Query
			{
				using StringContent content = new StringContent($"{{ \"bucketRegex\":\"{SearchBucketId}\", \"query\": null}}");
				content.Headers.ContentType = new MediaTypeHeaderValue(MediaTypeNames.Application.Json);
				...
				ValidationProblemDetails error = await result.Content.ReadAsCompactBinaryAsync<ValidationProblemDetails>();
				Assert.AreEqual("One or more validation errors occurred.", error.Title);
				Assert.AreEqual(1, error.Errors.Count);
				Assert.AreEqual(1, error.Errors["Query"].Length);
				Assert.AreEqual("The Query field is required.", error.Errors["Query"][0]);
			}
			
			// Null Options
			{
				using StringContent content = new StringContent($"{{ \"bucketRegex\":\"{SearchBucketId}\", \"query\": {{}}, \"options\": null}}");
				content.Headers.ContentType = new MediaTypeHeaderValue(MediaTypeNames.Application.Json);
				using HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Post, new Uri($"api/v2/builds/{TestNamespace}/search", UriKind.Relative));
				request.Content = content;
				request.Headers.Add("Accept", CustomMediaTypeNames.UnrealCompactBinary);
				...
				Assert.AreEqual("One or more validation errors occurred.", error.Title);
				Assert.AreEqual(1, error.Errors.Count);
				Assert.AreEqual(1, error.Errors["Options"].Length);
				Assert.AreEqual("The Options field is required.", error.Errors["Options"][0]);
			}
			
			// Null Query and Options
			{
				using StringContent content = new StringContent($"{{ \"bucketRegex\":\"{SearchBucketId}\", \"query\": null, \"options\": null}}");
				content.Headers.ContentType = new MediaTypeHeaderValue(MediaTypeNames.Application.Json);
				using HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Post, new Uri($"api/v2/builds/{TestNamespace}/search", UriKind.Relative));
				request.Content = content;
				request.Headers.Add("Accept", CustomMediaTypeNames.UnrealCompactBinary);
			...
			// Null Options and Query
			{
				using StringContent content = new StringContent($"{{ \"bucketRegex\":\"{SearchBucketId}\", \"options\": null, \"query\": null}}");
				content.Headers.ContentType = new MediaTypeHeaderValue(MediaTypeNames.Application.Json);
				using HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Post, new Uri($"api/v2/builds/{TestNamespace}/search", UriKind.Relative));
				request.Content = content;
				request.Headers.Add("Accept", CustomMediaTypeNames.UnrealCompactBinary);
		...
		public async Task SearchBuildCompactBinaryAsync()
		{
			...
			// Equal search integer cast
			{
				CbWriter writer = new CbWriter();
				writer.BeginObject();
				writer.BeginObject("query");
				writer.BeginObject("changelist");
				writer.WriteString("$eq", "40091037");
				writer.EndObject(); // changelist
				writer.EndObject(); // query
				writer.EndObject();
			...
			// Equal search double cast
			{
				CbWriter writer = new CbWriter();
				writer.BeginObject();
				writer.BeginObject("query");
				writer.BeginObject("changelist");
				writer.WriteString("$eq", "40091040");
				writer.EndObject(); // changelist
				writer.EndObject(); // query
				writer.EndObject();
		...
		public async Task ListNamespacesAsync()
		{
			...
			// fetch namespaces as json
			{
				...
				await result.EnsureSuccessStatusCodeWithMessageAsync();
				Assert.AreEqual(MediaTypeNames.Application.Json, result!.Content.Headers.ContentType!.MediaType);
		...
		[TestMethod]
		public async Task AppendixUploadsAsync()
		{
			CbObjectId buildId = CbObjectId.NewObjectId();
			await UploadSampleBuildAsync(buildId);
			...
			byte[] contents = await File.ReadAllBytesAsync("ContentId/Payloads/smallerfile");

			CbObject cbObject = writer.ToObject();

			// upload a file appendix as json
			{
				using HttpContent requestContent = new StringContent("{\"value\":\"foo\"}");
				requestContent.Headers.ContentType = new MediaTypeHeaderValue(MediaTypeNames.Application.Json);
			...
			// upload a file appendix as cb
			{
				using HttpContent requestContent = new ByteArrayContent(writer.ToByteArray());
				requestContent.Headers.ContentType = new MediaTypeHeaderValue(CustomMediaTypeNames.UnrealCompactBinary);

				using HttpResponseMessage result = await _httpClient!.PutAsync(new Uri($"api/v2/builds/{TestNamespace}/{BucketId}/{buildId}/appendix/test.cbo", UriKind.Relative), requestContent);
				await result.EnsureSuccessStatusCodeWithMessageAsync();
				PutObjectResponse response = await result.Content.ReadAsAsync<PutObjectResponse>();
			...
			// upload a file appendix as binary blob
			{
				using HttpContent requestContent = new ByteArrayContent(Encoding.ASCII.GetBytes("foo"));
				requestContent.Headers.ContentType = new MediaTypeHeaderValue(MediaTypeNames.Application.Octet);

				using HttpResponseMessage result = await _httpClient!.PutAsync(new Uri($"api/v2/builds/{TestNamespace}/{BucketId}/{buildId}/appendix/test.txt", UriKind.Relative), requestContent);
				await result.EnsureSuccessStatusCodeWithMessageAsync();
				PutObjectResponse response = await result.Content.ReadAsAsync<PutObjectResponse>();
			...
			// upload a file appendix as compressed buffer
			{
				using HttpContent requestContent = new ByteArrayContent(contents);
				requestContent.Headers.ContentType = new MediaTypeHeaderValue(CustomMediaTypeNames.UnrealCompressedBuffer);

				using HttpResponseMessage result = await _httpClient!.PutAsync(new Uri($"api/v2/builds/{TestNamespace}/{BucketId}/{buildId}/appendix/test.bin", UriKind.Relative), requestContent);
				await result.EnsureSuccessStatusCodeWithMessageAsync();
				PutObjectResponse response = await result.Content.ReadAsAsync<PutObjectResponse>();
			...
			// fetch the build appendix as json
			{
				using HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Get, new Uri($"api/v2/builds/{TestNamespace}/{BucketId}/{buildId}/appendix/test.json", UriKind.Relative));
				request.Headers.Add("Accept", MediaTypeNames.Application.Json);
			...
			// fetch the build appendix as cb object
			{
				using HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Get, new Uri($"api/v2/builds/{TestNamespace}/{BucketId}/{buildId}/appendix/test.cbo", UriKind.Relative));
				request.Headers.Add("Accept", CustomMediaTypeNames.UnrealCompactBinary);
				...
				Assert.AreEqual(IoHash.Compute(cbObject.GetView().ToArray()), IoHash.Compute(b));
			}

			// fetch the build appendix as binary blob
			{
				using HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Get, new Uri($"api/v2/builds/{TestNamespace}/{BucketId}/{buildId}/appendix/test.txt", UriKind.Relative));
				request.Headers.Add("Accept", MediaTypeNames.Application.Octet);
			...
			// fetch the build appendix as compressed buffer
			{
				using HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Get, new Uri($"api/v2/builds/{TestNamespace}/{BucketId}/{buildId}/appendix/test.bin", UriKind.Relative));
				request.Headers.Add("Accept", CustomMediaTypeNames.UnrealCompressedBuffer);

				using HttpResponseMessage result = await _httpClient!.SendAsync(request);
				await result.EnsureSuccessStatusCodeWithMessageAsync();
				byte[] b = await result.Content.ReadAsByteArrayAsync();

				Assert.AreEqual(IoHash.Compute(contents), IoHash.Compute(b));
			}

			// list all appendices for the build
			{
				using HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Get, new Uri($"api/v2/builds/{TestNamespace}/{BucketId}/{buildId}/appendix", UriKind.Relative));
				request.Headers.Add("Accept", CustomMediaTypeNames.UnrealCompactBinary);
				...
				string[] found = o["appendices"].AsArray().Select(f => f.AsString()).ToArray();
				string[] expected = new[] {"test.cbo", "test.bin", "test.json", "test.txt"};
