...
namespace Jupiter.FunctionalTests.References
{
	...
	[TestClass]
	public class GalileoReferencesTests : ReferencesTests
	{
		public GalileoReferencesTests() : base("galileo")
		{
		}
		...
		protected override IEnumerable<KeyValuePair<string, string?>> GetSettings()
		{
			return new[]
			{
				new KeyValuePair<string, string?>("UnrealCloudDDC:ReferencesDbImplementation", UnrealCloudDDCSettings.ReferencesDbImplementations.Galileo.ToString()),
				new KeyValuePair<string, string?>("UnrealCloudDDC:ContentIdStoreImplementation", UnrealCloudDDCSettings.ContentIdStoreImplementations.Scylla.ToString()),
				new KeyValuePair<string, string?>("UnrealCloudDDC:BlobIndexImplementation", UnrealCloudDDCSettings.BlobIndexImplementations.Scylla.ToString()),
				new KeyValuePair<string, string?>("UnrealCloudDDC:ReplicationLogWriterImplementation", UnrealCloudDDCSettings.ReplicationLogWriterImplementations.Scylla.ToString()),
				new KeyValuePair<string, string?>("CacheRef:Enabled", bool.FalseString), // we do not want memory caching with datastore backends
			};
		}
		...
		protected override async Task SeedDbAsync(IServiceProvider provider)
		{
			IReferencesStore referencesStore = provider.GetService<IReferencesStore>()!;
			//verify we are using the expected store
			Assert.IsTrue(referencesStore.GetType() == typeof(GalileoReferencesStore));
	...
	[TestClass]
	public class DataStoreReferencesTests : ReferencesTests
	{
		public DataStoreReferencesTests() : base("datastore")
		{
		}
		...
		protected override IEnumerable<KeyValuePair<string, string?>> GetSettings()
		{
			return new[]
			{
				new KeyValuePair<string, string?>("UnrealCloudDDC:ReferencesDbImplementation", UnrealCloudDDCSettings.ReferencesDbImplementations.DataStore.ToString()),
				new KeyValuePair<string, string?>("UnrealCloudDDC:ContentIdStoreImplementation", UnrealCloudDDCSettings.ContentIdStoreImplementations.DataStore.ToString()),
				new KeyValuePair<string, string?>("UnrealCloudDDC:BlobIndexImplementation", UnrealCloudDDCSettings.BlobIndexImplementations.DataStore.ToString()),
				new KeyValuePair<string, string?>("UnrealCloudDDC:ReplicationLogWriterImplementation", UnrealCloudDDCSettings.ReplicationLogWriterImplementations.Memory.ToString()),
				...
				new KeyValuePair<string, string?>("DataStore:RootDir", $"$(ExecutableLocation)/DataStore/{CbObjectId.NewObjectId()}"),
				new KeyValuePair<string, string?>("DataStore:Enabled", bool.TrueString),
				new KeyValuePair<string, string?>("S3:BucketName", $"tests-{TestNamespace}"),
				new KeyValuePair<string, string?>("S3:SiteBucketsPerStoragePool:test:default", $"tests-{TestNamespace}"),
				new KeyValuePair<string, string?>("CacheRef:Enabled", bool.FalseString), // we do not want memory caching with datastore backends
			};
		}
		...
		protected override async Task SeedDbAsync(IServiceProvider provider)
		{
			IReferencesStore referencesStore = provider.GetService<IReferencesStore>()!;
			//verify we are using the expected store
			Assert.IsTrue(referencesStore.GetType() == typeof(DataStoreReferencesStore));
			...
			Assert.IsTrue(contentIdStore.GetType() == typeof(DataStoreContentIdStore));
	...
	public abstract class ReferencesTests
	{
...

		{
			...
			await SeedDbAsync(_server.Services);
		}
		...
		public async Task PutGetCompactBinaryHierarchyAsync()
		{
			...
			// make sure data we want to upload has been deleted
			{
				await Service.DeleteObjectAsync(TestNamespace, childObjectHash);
			}
		...
		public async Task PutContentIdMissingBlobAsync()
		{
			...
			await contentIdStore.PutAsync(TestNamespace, contentId, new BlobId[] {blobHash}, (ulong)blobData.Length, (ulong)blobData.Length);
		...
		public async Task PutAndFinalizeAsync()
		{
			...
			// make sure data we want to upload has been deleted
			{
				...
				await Service.DeleteObjectAsync(TestNamespace, blobHashChild);
			}
		...
		public async Task PutNoOverwriteAsync()
		{
			...
			// make sure data we want to upload has been deleted
			{
				await Service.DeleteObjectAsync(TestNamespaceNoOverwrite, blobHashChild);
				await Service.DeleteObjectAsync(TestNamespaceNoOverwrite, blobHash);
			}
		...
		public async Task GetMissingContentIdRecordAsync()
		{
			...
			CompressedBuffer.CompressContent(compressedStream, OodleCompressorType.Mermaid, OodleCompressionLevel.VeryFast, blobData);
