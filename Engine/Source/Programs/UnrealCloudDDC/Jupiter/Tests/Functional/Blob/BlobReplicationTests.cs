...
namespace Jupiter.FunctionalTests.Storage
{
	...
	public class BlobReplicationTests
	{
		...
		public async Task ReplicateBlobFromRegionAsync()
		{
							...
							collection.Configure<ClusterSettings>(settings =>
							{
								settings.Peers = new PeerSettings[]
								{
									new PeerSettings()
									{
										...
										{
											new PeerEndpoints() { Url = new Uri("http://siteA.com/internal"), IsInternal = true },
											new PeerEndpoints() { Url = new Uri("http://siteA.com/public") },
										}.ToList()
									},
									new PeerSettings()
									{
										...
										Endpoints = new PeerEndpoints[]
										{
											new PeerEndpoints() { Url = new Uri("http://siteB.com/internal"), IsInternal = true },
											new PeerEndpoints() { Url = new Uri("http://siteB.com/public") },
										}.ToList()
									},
								}.ToList();
							});
								...
								{
									{
										...
										new NamespacePolicy()
										{
											...
											Acls = new List<AclEntry>()
											{
												new AclEntry()
												{
													Actions = new List<JupiterAclAction>() { JupiterAclAction.ReadObject, },
			...
			await blobIndex.AddBlobToIndexAsync(TestNamespaceName, blobIdentifier, (ulong)bytes.LongLength, region: "siteB");
