...
namespace Jupiter.FunctionalTests.Storage
{
	...
	public class PeerBlobTest
	{
				...
				.ConfigureWebHost(webHostBuilder =>
				{
					webHostBuilder.UseTestServer()
						...
						.ConfigureTestServices(collection =>
						{
							...
							_otherPeerMock
								.SetupRequest(
									$"http://other-peer-instance.com/api/v1/blobs/{TestNamespaceName}/{S3FileContentHash}?storageLayers={storageLayerName}")
								.ReturnsResponse(HttpStatusCode.OK, S3FileContent).Verifiable();
							_otherPeerMock
								.SetupRequest(
									$"http://other-peer-instance.com/api/v1/blobs/{TestNamespaceName}/{OtherPeerContentHash}?storageLayers={storageLayerName}")
								.ReturnsResponse(HttpStatusCode.OK, OtherPeerContent).Verifiable();
		...
		public async Task ForceFetchBlobFromS3Async()
		{
			...
			{
				Assert.Fail($"Non-success status code ({putResponse.StatusCode}) with body: {await putResponse.Content.ReadAsStringAsync()}");
			}
