...
namespace Jupiter.FunctionalTests.Storage
{
	public abstract class BlobIndexTests
	{
		...
		{
			byte[] randomData = Encoding.ASCII.GetBytes($"I contain some random content for {nameof(UploadRefAsync)}");
			IoHash dataHash = IoHash.Compute(randomData);
			BlobId dataBlobId = BlobId.FromIoHash(dataHash);

			CbWriter writer = new CbWriter();
			writer.BeginObject();
			writer.WriteString("stringField", nameof(UploadRefAsync));
			writer.WriteBinaryAttachment("attachmentField", dataHash);
			writer.EndObject();

			byte[] objectData = writer.ToByteArray();
			BlobId objectHash = BlobId.FromBlob(objectData);

			using HttpContent requestBlobContent = new ByteArrayContent(randomData);
			requestBlobContent.Headers.ContentType = new MediaTypeHeaderValue(MediaTypeNames.Application.Octet);
			requestBlobContent.Headers.Add(CommonHeaders.HashHeaderName, dataHash.ToString());
			HttpResponseMessage resultBlob = await _httpClient!.PutAsync(new Uri($"api/v1/blobs/{_testNamespaceName}/{dataHash}", UriKind.Relative), requestBlobContent);
			await resultBlob.EnsureSuccessStatusCodeWithMessageAsync();
			...
			IBlobIndex? index = _server!.Services.GetService<IBlobIndex>();
			Assert.IsNotNull(index);
			Assert.IsTrue(await index.BlobExistsInRegionAsync(_testNamespaceName, dataBlobId, "test"));

			IAsyncEnumerable<BaseBlobReference> blobReferences = index.GetBlobReferencesAsync(_testNamespaceName, dataBlobId);
		...
		public async Task DeleteBlobAsync()
		{
			...
			await Assert.ThrowsExactlyAsync<BlobNotFoundException>(async () =>
			{
