...
namespace Jupiter.UnitTests
{
	...
	public class HierarchicalBlobStorageTest
	{
		...
		public async Task GetObjectAsync()
		{
			...
			await Assert.ThrowsExactlyAsync<BlobNotFoundException>(() => _chained.GetObjectAsync(Ns, _nonExisting));
			await Assert.ThrowsExactlyAsync<BlobNotFoundException>(() => _chained.GetObjectAsync(new NamespaceId("non-existing-ns"), _nonExisting));
		...
		{
			await Assert.ThrowsExactlyAsync<NamespaceNotFoundException>(() => _chained.DeleteNamespaceAsync(NsnonExistingNs));

			Assert.IsTrue(await _first.ExistsAsync(Ns, _allId));
			Assert.IsTrue(await _second.ExistsAsync(Ns, _allId));
			await _chained.DeleteNamespaceAsync(Ns);
			await Assert.ThrowsExactlyAsync<NamespaceNotFoundException>(() => _first.DeleteNamespaceAsync(Ns));
			await Assert.ThrowsExactlyAsync<NamespaceNotFoundException>(() => _second.DeleteNamespaceAsync(Ns));
		}
