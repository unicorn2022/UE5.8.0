...
namespace Jupiter.Controllers
{
	...
	public class ReferencesController : ControllerBase
	{
		...
		/// <param name="forceConsistentRead">Optional header to force consistent reads disabling all uses of caches</param>
		...
		public async Task<IActionResult> GetAsync(
			...
			[FromHeader(Name = CommonHeaders.ForceConsistentRead)] bool forceConsistentRead = false)
		{
			...
			try
			{
				...
				(RefRecord objectRecord, BlobContents? maybeBlob) = await _refService.GetAsync(ns, bucket, key, Array.Empty<string>(), consistentRead: forceConsistentRead);
					...
					case CustomMediaTypeNames.JupiterInlinedPayload:
						{
							...
							if (countOfBinaryAttachmentFields == 1 && countOfAttachmentFields == 1)
							{
								...
								{
									Tracer.CurrentSpan.SetAttribute("error-type", "MissingContentIds");
								...
								{
									BlobId? blobId = e.MissingBlobs.FirstOrDefault();
									Tracer.CurrentSpan.SetAttribute("error-type", "MissingBlobs");
									return NotFound(new ProblemDetails { Title = $"Object {bucket} {key} in namespace {ns} was missing {e.MissingBlobs.Count} blobs. Sample: {blobId}" });
								}
			...
			[FromRoute][Required] RefId key)
		{
			...
			try
			{
				(RefRecord record, blob) = await _refService.GetAsync(ns, bucket, key);
				...
				// if the record does not inline its blob the actual blob needs to be present as well
				if (record.InlinePayload == null)
				{
					BlobId[] unknownBlobs =
						await _blobStore.FilterOutKnownBlobsAsync(ns, new BlobId[] { record.BlobIdentifier }, HttpContext.RequestAborted);
		...
		{
			ActionResult? accessResultNamespace = await _requestHelper.HasAccessToScopeAsync(User, Request, new AccessScope(ns), new[] { JupiterAclAction.ReadObject });
			if (accessResultNamespace != null)
			{
				return accessResultNamespace;
			}
			...
			// verify we have access to each of the buckets we are requesting info on
			foreach (BucketId bucket in requestedNames.Select(tuple => tuple.Item1).Distinct())
			{
				ActionResult? accessResult = await _requestHelper.HasAccessToScopeAsync(User, Request, new AccessScope(ns, bucket), new[] { JupiterAclAction.ReadObject });
				if (accessResult != null)
				{
...
#pragma warning restore IDE0060
			...
			[FromBody][Required] BatchOps ops)
		{
			...
			foreach (BatchOps.BatchOp op in ops.Ops)
			{
				ActionResult? accessResult = await _requestHelper.HasAccessToScopeAsync(User, Request, new AccessScope(ns, op.Bucket), new[] { ActionForOp(op.Op) });
				if (accessResult != null)
				{
					return accessResult;
				}
			...
			async Task<(CbObject, HttpStatusCode)> BatchGetOp(BatchOps.BatchOp op)
			{
				...
				{
					// this isn't really an error so no need to build a stacktrace which is very expensive
					return ToErrorResult(ex, HttpStatusCode.NotFound, includeStacktrace: false);
				}
				catch (Exception e)
				{
					Tracer.CurrentSpan.SetStatus(Status.Error);
			...
			async Task<(CbObject, HttpStatusCode)> BatchHeadOp(BatchOps.BatchOp op)
			{
				...
				try
				{
					(RefRecord record, blob) = await _refService.GetAsync(ns, op.Bucket, op.Key);
		...
		private static (CbObject, HttpStatusCode) ToErrorResult(Exception exception, HttpStatusCode statusCode = HttpStatusCode.InternalServerError, bool includeStacktrace = true)
		{
		...
		public async Task<IActionResult> DeleteBucketAsync(
			...
			[FromRoute][Required] BucketId bucket)
		{
			ActionResult? accessResult = await _requestHelper.HasAccessToScopeAsync(User, Request, new AccessScope(ns, bucket), new[] { JupiterAclAction.DeleteBucket });
			if (accessResult != null)
			{
				return accessResult;
			}
