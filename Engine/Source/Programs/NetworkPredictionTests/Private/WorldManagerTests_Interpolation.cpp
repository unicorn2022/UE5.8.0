...
{
	// Disabling time dilation for this test since it dynamically affects how far ahead the client gets. Restored at end.
	const bool bDefaultEnableTimeDilation = NetworkPredictionCVars::EnableTimeDilation->GetBool();
	NetworkPredictionCVars::EnableTimeDilation->Set(false);
	ON_SCOPE_EXIT{ NetworkPredictionCVars::EnableTimeDilation->Set(bDefaultEnableTimeDilation); };
