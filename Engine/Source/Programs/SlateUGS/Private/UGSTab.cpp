...
#include "Widgets/SPopupTextWindow.h"
...
namespace
{
	...
	public:
		...
		{
			UE_LOGF(LogSlateUGS, Log, "%ls", *Line);
		}
	};
...
TArray<FString> UGSTab::GetAllStreamNames() const
{
	...
	if (TaskResult->Failed())
	{
		FSlateApplication::Get().AddModalWindow(
			SNew(SPopupTextWindow)
				.TitleText(LOCTEXT("FindStreamsErrorTitle", "Error"))
				.BodyText(TaskResult->GetMessage()),
			FSlateApplication::Get().GetActiveModalWindow());
...
bool UGSTab::SetupWorkspace()
{
	...
	if (Result->Failed())
	{
		FSlateApplication::Get().AddModalWindow(
			SNew(SPopupTextWindow)
				.TitleText(LOCTEXT("SetupWorkspaceErrorTitle", "Error"))
				.BodyText(Result->GetMessage()),
			FSlateApplication::Get().GetActiveModalWindow());
		return false;
	}
