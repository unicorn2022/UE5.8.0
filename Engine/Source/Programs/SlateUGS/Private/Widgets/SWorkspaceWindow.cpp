...
{
	if (!FPaths::FileExists(WorkspacePathText))
	{
	...
	const bool bIsWorkspaceValid = Tab->OnWorkspaceChosen(WorkspacePathText);
...
FReply SWorkspaceWindow::OnBrowseClicked()
{
	...
	FDesktopPlatformModule::Get()->OpenFileDialog(
		...
		PreviousProjectPath,
		TEXT(""),
		TEXT("Project files (*.uproject;*.uefnproject)|*.uproject;*.uefnproject|Project directory lists (*.uprojectdirs)|*.uprojectdirs|All supported files (*.uproject;*.uefnproject;*.uprojectdirs)|*.uproject;*.uefnproject;*.uprojectdirs|"),
