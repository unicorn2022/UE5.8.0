...
{
	// Only store the delta from defaults, (all built-in categories default to enabled):
	//   checked   → matches default → not stored
	//   unchecked → differs from default → stored in ExcludeCategories
	...
	TArray<FGuid> EmptyIncludeCategories;
	Tab->OnSyncFilterWindowSaved(SyncViewCurrent, EmptyIncludeCategories, SyncExcludedCategoriesCurrent, SyncViewAll, EmptyIncludeCategories, SyncExcludedCategoriesAll);
