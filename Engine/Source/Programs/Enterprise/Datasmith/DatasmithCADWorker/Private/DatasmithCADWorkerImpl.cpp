...
FDatasmithCADWorkerImpl::FDatasmithCADWorkerImpl(int32 InServerPID, int32 InServerPort, const FString& InEnginePluginsPath, const FString& InCachePath)
	...
	, PingStartCycle(0)
{
	UE_LOGF(LogDatasmithCADWorker, Verbose, "connect to %d...", ServerPort);
	bool bConnected = NetworkInterface.Connect(TEXT("Datasmith CAD Worker"), ServerPort, Config::ConnectTimeout_s);
	UE_LOGF(LogDatasmithCADWorker, Verbose, "connected to %d %ls", ServerPort, bConnected ? TEXT("OK") : TEXT("FAIL"));
	if (bConnected)
	{
	...
	{
		UE_LOGF(LogDatasmithCADWorker, Error, "Server connection failure. exit");
		return;
	}
	...
	while (bIsRunning)
	{
		if (TSharedPtr<ICommand> Command = CommandIO.GetNextCommand(1.0))
		{
			...
			case ECommandId::Terminate:
				UE_LOGF(LogDatasmithCADWorker, Verbose, "Terminate command received. Exiting.");
				bIsRunning = false;
				break;
		...
		else
		{
			if (bIsRunning)
			{
				...
				UE_CLOGF(!bIsRunning, LogDatasmithCADWorker, Error, "Worker failure: server lost");
			}
		}
	}

	UE_CLOGF(!bIsRunning, LogDatasmithCADWorker, Verbose, "Worker loop exit...");
...
void FDatasmithCADWorkerImpl::InitiatePing()
{
	...
	UE_LOGF(LogDatasmithCADWorker, Verbose, "InitiatePing...");
	FPingCommand Ping;
...
void FDatasmithCADWorkerImpl::ProcessCommand(const FBackPingCommand& BackPingCommand)
{
	if (PingStartCycle)
	{
		...
		UE_LOGF(LogDatasmithCADWorker, Verbose, "Ping %f s", ElapsedTime_s);
	}
	PingStartCycle = 0;
}
...
uint64 DefineMaximumAllowedDuration(const CADLibrary::FFileDescriptor& FileDescriptor, bool& bEnableTimeControl)
{
	...
	UE_LOGF(LogDatasmithCADWorker, Verbose, "    - File size %lld KB", FileStatData.FileSize / OneKiloBit);
...
void FDatasmithCADWorkerImpl::ProcessCommand(const FRunTaskCommand& RunTaskCommand)
{
	...
	UE_LOGF(LogDatasmithCADWorker, Verbose, "Process %ls %ls", *FileToProcess.GetFileName(), *FileToProcess.GetConfiguration());
	...
	if (CompletedTask.ProcessResult == ETaskState::ProcessOk)
	{
		...
		UE_LOGF(LogDatasmithCADWorker, Verbose, "=> Process %ls %ls saved into %ls%ls and %ls%ls.", *FileToProcess.GetFileName(), *FileToProcess.GetConfiguration(), *CompletedTask.SceneGraphFileName, TEXT(".sg"), *CompletedTask.GeomFileName, TEXT(".gm"));
		UE_LOGF(LogDatasmithCADWorker, Verbose, "     It generates %d bodies", CADFileData.GetBodyMeshes().Num());
		for (const FBodyMesh& BodyMesh : CADFileData.GetBodyMeshes())
		{
			...
			UE_LOGF(LogDatasmithCADWorker, Verbose, "     - Body %ls", *BodyFileName);
		}
	}
	else if(CompletedTask.ProcessResult == ETaskState::FileNotFound)
	{
		UE_LOGF(LogDatasmithCADWorker, Verbose, "=> File not found %ls", *FileToProcess.GetFileName());
	}
	else
	{
		UE_LOGF(LogDatasmithCADWorker, Verbose, "=> Process %ls %ls failed", *FileToProcess.GetFileName(), *FileToProcess.GetConfiguration());
	}
	UE_LOGF(LogDatasmithCADWorker, Verbose, "End of Process %ls %ls", *FileToProcess.GetFileName(), *FileToProcess.GetConfiguration());
...
void FDatasmithCADWorkerImpl::CheckDuration(const CADLibrary::FFileDescriptor& FileToProcess, const int64 MaxDuration)
{
	...
	while (bProcessIsRunning)
	{
		...
		{
			UE_LOGF(LogDatasmithCADWorker, Verbose, "Time exceeded to process %ls %ls. The maximum allowed duration is %ld s", *FileToProcess.GetFileName(), *FileToProcess.GetConfiguration(), MaxDuration);
			FPlatformMisc::RequestExit(true);
		}
	}
	double Duration = (FPlatformTime::Cycles64() - StartTime) * FPlatformTime::GetSecondsPerCycle64();
	UE_LOGF(LogDatasmithCADWorker, Verbose, "    - Processing Time: %f s", Duration);
}
...
void FDatasmithCADWorkerImpl::CheckMemory()
{
	...
	UE_LOGF(LogDatasmithCADWorker, Verbose, "    - Start Ram used %llu MB", MaxMemoryUsed / OneMegaBit);
	...
	UE_LOGF(LogDatasmithCADWorker, Verbose, "    - End Ram used %llu MB", EndMemoryUsed / OneMegaBit);
	UE_LOGF(LogDatasmithCADWorker, Verbose, "    - Max Ram used %llu MB", MaxMemoryUsed / OneMegaBit);
	if(EndMemoryUsed > GigaBit)
	{
		UE_LOGF(LogDatasmithCADWorker, Verbose, "    - Ram used (%llu MB) after cleanup exceeds limit to start new process. CADWorker restart is requested", EndMemoryUsed / OneMegaBit);
