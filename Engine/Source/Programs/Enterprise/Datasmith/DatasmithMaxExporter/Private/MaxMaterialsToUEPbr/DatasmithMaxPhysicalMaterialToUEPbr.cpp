...
{
	IDatasmithMaterialExpression& FresnelOrig = Fresnel(&Scalar(1.0f), &Scalar(0.0f));
	...
	IDatasmithMaterialExpression& Mul2NCPre = Multiply(IORn, Scalar(2.0f));
	...
	IDatasmithMaterialExpression& RpNum = Add(RpNumPre, Scalar(1.0f));
	IDatasmithMaterialExpression& RpDenPre = Add(AddN2K2MulC2, Mul2NC);
	IDatasmithMaterialExpression& RpDen = Add(RpDenPre, Scalar(1.0f));
	IDatasmithMaterialExpression& Rp = Divide(RpNum, RpDen);

	IDatasmithMaterialExpression& Res = Multiply(Add(Rp, Rs), Scalar(0.5f));

	return Lerp(ToBeConnected0, ToBeConnected90, Power(Res, Scalar(0.5f)));
}
...
void FDatasmithMaxPhysicalMaterialToUEPbr::Convert( TSharedRef< IDatasmithScene > DatasmithScene, TSharedPtr< IDatasmithBaseMaterialElement >& MaterialElement, Mtl* Material, const TCHAR* AssetsPath )
{
	...
	if (TransparencyExpression)
	{
		...
		else
		{
			if (PhysicalMaterialProperties.MaterialMode == 1) // Advanced mode, setup refractive glass
			{
				...
				IDatasmithMaterialExpression& Multiply09 = Multiply(Scalar(0.9f), *TransparencyExpression);

				// Modulate opacity by fresnel 
				IDatasmithMaterialExpression& OpacityWithFresnelExpression = OneMinus(Desaturate(CalcIORComplex(Ior, 0, Multiply(Scalar(0.5f), *TransparencyExpression), Multiply09)));
				...
				BaseColorExpression = &Multiply(Scalar(0.6f), Power(Multiply09, Scalar(5.0f))); 
			}
			else
			{
				Connect(PbrMaterialElement->GetOpacity(), COMPOSE_OR_DEFAULT2(&OpacityFromTransparency, Multiply, &OpacityFromTransparency, CutOutExpression));
	...
	if (ClearCoatProperties.HasClearCoat())
	{
		...
		if (ClearCoatExpression)
		{
			...
			IDatasmithMaterialExpression& ToBeConnected0 = Multiply(*ClearCoatExpression, Scalar(0.1f));
