	...
	if (TransparencyExpression)
	{
		...
		else
		{
			...
			if (bIsRefractiveGlass)
			{
				...
				IDatasmithMaterialExpression& Multiply09 = Multiply(Scalar(0.9f), TransparencyExpressionRef);

				BaseOpacityExpression = &OneMinus(Desaturate(CalcIORComplex(Ior, 0, Multiply(Scalar(0.5f), TransparencyExpressionRef), Multiply09)));
