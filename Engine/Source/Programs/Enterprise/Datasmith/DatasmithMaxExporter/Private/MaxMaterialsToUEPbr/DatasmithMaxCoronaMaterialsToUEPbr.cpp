	...
	TSharedPtr<IDatasmithBaseMaterialElement>& MaterialElement, Mtl* Material, const TCHAR* AssetsPath)
{
	...
	else if (OpacityColorExpression || bHasRefraction)  // Transparent non-metal
	{
		...
		else if (bHasRefraction) // Refractive glass
		{
			...
			IDatasmithMaterialExpression& Multiply09 = Multiply(Scalar(0.9f), RefractionAmountExpression);
			IDatasmithMaterialExpression& FresnelOpacity = OneMinus(CalcIORComplex(Ior, 0, Multiply(Scalar(0.5f), RefractionAmountExpression), Multiply09));
	...
	if (bHasClearCoat)
	{
		...
		if (ClearCoatExpression)
		{
			IDatasmithMaterialExpression& ClearCoatAmountByFresnelExpression = MaterialProperties.ClearcoatIorTexmap.IsMapPresentAndEnabled()
				? CalcIORComplex(*TextureOrScalar(TEXT("Clear Coat Ior"), MaterialProperties.ClearcoatIorTexmap, MaterialProperties.ClearcoatIor), Scalar(0.f), *ClearCoatExpression, Multiply(*ClearCoatExpression, Scalar(0.1f)))
				: CalcIORComplex(MaterialProperties.ClearcoatIor, 0, *ClearCoatExpression, Multiply(*ClearCoatExpression, Scalar(0.1f)));
