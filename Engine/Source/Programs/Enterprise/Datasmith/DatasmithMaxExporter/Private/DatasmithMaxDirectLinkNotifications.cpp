...
void FNotifications::On3dsMaxNotification(void* param, NotifyInfo* info)
{
	...
	case NOTIFY_NODE_LINKED:
		{
#if MAX_PRODUCT_YEAR_NUMBER >= 2027
		INode* Node = GetNotifyParam<NOTIFY_NODE_UNLINKED, NOTIFY_NODE_CREATED, NOTIFY_NODE_LINKED>(info);
#else
	...
	if (NotificationsHandler.bSceneChangeTracking)
	{
...
#if MAX_PRODUCT_YEAR_NUMBER >= 2027
			INode* Node = GetNotifyParam<NOTIFY_SCENE_ADDED_NODE>(info);
#else
			INode* Node = reinterpret_cast<INode*>(info->callParam);
#endif
		...
		{
#if MAX_PRODUCT_YEAR_NUMBER >= 2027
			INode* Node = GetNotifyParam<NOTIFY_NODE_UNLINKED>(info);
#else
			INode* Node = reinterpret_cast<INode*>(info->callParam);
#endif
			LogDebugNode(NotificationsHandler.ConvertNotificationCodeToString(NOTIFY_NODE_UNLINKED), Node);
			break;
		}
		...
		{
#if MAX_PRODUCT_YEAR_NUMBER >= 2027
			INode* Node = GetNotifyParam<NOTIFY_NODE_HIDE, NOTIFY_NODE_UNHIDE>(info);
#else
			INode* Node = reinterpret_cast<INode*>(info->callParam);
#endif
			SceneTracker.NodeHideChanged(Node);
			break;
		}
		...
		{
#if MAX_PRODUCT_YEAR_NUMBER >= 2027
			INode* Node = GetNotifyParam<NOTIFY_SCENE_XREF_POST_MERGE>(info);
#else
			INode* Node = reinterpret_cast<INode*>(info->callParam);
#endif
			LogDebugNode(NotificationsHandler.ConvertNotificationCodeToString(info->intcode), Node);
