...
struct FChangeDetector
{
	...
	{
		UE_LOGF(LogTestPAL, Display, "  -- %ls %d change(s) detected", *Title, static_cast<int32>(FileChanges.Num()));

		int ChangeIdx = 0;
		for (const auto& ThisEntry : FileChanges)
		{
			UE_LOGF(LogTestPAL, Display, "      Change %d: %ls was %ls",
				++ChangeIdx,
				*ThisEntry.Filename,
...
static void DoCommand(IDirectoryWatcher *DirectoryWatcher, const FString& Command, const FString& Arg)
{
	...
	{
		// delete file
		UE_LOGF(LogTestPAL, Display, "Deleting FILE '%ls'", *Arg);
...
int32 DirectoryWatcherTest(const TCHAR* CommandLine)
{
	...
	UE_LOGF(LogTestPAL, Display, "Running directory watcher test.");
	...
	if (PlatformFile.CreateDirectory(*TestDir) && PlatformFile.CreateDirectory(*SubtestDir))
	{
		...
		if (DirectoryWatcher)
		{
			...
			UE_LOGF(LogTestPAL, Display, "Registered callback for changes in '%ls'", *TestDir);
			...
			UE_LOGF(LogTestPAL, Display, "Registered callback 2 for changes in '%ls'", *SubtestDir);
		}
		else
		{
			UE_LOGF(LogTestPAL, Fatal, "Could not get DirectoryWatcher module");
		}
		...
		UE_LOGF(LogTestPAL, Display, "Moving DIRECTORY '%ls' -> '%ls'", *(SubtestDir + TEXT("/blah")), *(SubtestDir + TEXT("/blah2")));
		...
		UE_LOGF(LogTestPAL, Display, "End of test");
	}
	else
	{
		UE_LOGF(LogTestPAL, Fatal, "Could not create test directory %ls.", *TestDir);
	}
