...
FProcHandle FParent::Launch(bool bDetached)
{
	...
	if (!WorkerHandle.IsValid())
	{
		...
		UE_LOGF(LogTestPAL, Fatal, "Couldn't launch %ls! Make sure the file is in your binaries folder.", *WorkerName);
	}
...
void FParent::Run()
{
	// test launching detached
	{
		for (int i = 0; i < 100; ++i)
		{
			UE_LOGF(LogTestPAL, Log, "Launching a detached child to see if we leak a zombie.");
	...
	{
		UE_LOGF(LogTestPAL, Log, "Launching a child to wait for it.");
		FProcHandle Child = Launch();

		UE_LOGF(LogTestPAL, Log, "Closing child's handle (FPlatformProcess::CloseProc)");
		FPlatformProcess::CloseProc(Child);
	}

	{
		UE_LOGF(LogTestPAL, Log, "Launching a child to terminate it.");
		FProcHandle Child = Launch();

		UE_LOGF(LogTestPAL, Log, "Sleeping for a bit to let the child ramp up.");
		FPlatformProcess::Sleep(0.1f);

		UE_LOGF(LogTestPAL, Log, "Terminating the child (FPlatformProcess::TerminateProc())");
	...
	UE_LOGF(LogTestPAL, Log, "Proceeding to test multiple children.");
	...
	while (NumTotalChildren > 0)
	{
		...
		{
			UE_LOGF(LogTestPAL, Log, "Launching a child (%d more to go).", NumTotalChildren - 1);
			FProcHandle Child = Launch();
			UE_LOGF(LogTestPAL, Log, "Launch successful");
		...
		for (int ChildIdx = 0; ChildIdx < Children.Num();)
		{
			...
			if (FPlatformProcess::GetProcReturnCode(ChildCopy, &ReturnCode))
			{
				...
				UE_LOGF(LogTestPAL, Log, "Child finished, return code %d", ReturnCode);
