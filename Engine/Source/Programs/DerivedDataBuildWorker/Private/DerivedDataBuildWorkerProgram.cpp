...
bool FBuildWorkerProgram::ParseCommandLine(const TCHAR* CommandLine)
{
	...
	{
		UE_LOGF(LogDerivedDataBuildWorker, Error, "A maximum of one input directory can be specified, but %d were specified.", InputDirectoryCount);
	...
	{
		UE_LOGF(LogDerivedDataBuildWorker, Error, "A maximum of one input directory can be specified, but %d were specified.", OutputDirectoryCount);
		bCommandLineIsValid = false;
	}
	...
	{
		UE_LOGF(LogDerivedDataBuildWorker, Error, "No build action files or version files specified on the command line.");
		bCommandLineIsValid = false;
	}
	...
	for (const FString& ActionPathPattern : ActionPathPatterns)
	{
		...
		{
			UE_LOGF(LogDerivedDataBuildWorker, Error, "Wildcards in build action file paths are not supported yet: '%ls'", *ActionPathWildcard);
			bCommandLineIsValid = false;
		}
		else
		{
...
bool FBuildWorkerProgram::ReportVersions()
{
	...
	UE_LOGF(LogDerivedDataBuildWorker, Display, "BuildSystemVersion: '%ls'", *WriteToString<64>(BuildSystemVersion));
	UE_LOGF(LogDerivedDataBuildWorker, Display, "Functions:");
	...
	for (const TPair<FString, FGuid>& Function : Functions)
	{
		...
		Writer.EndObject();
		UE_LOGF(LogDerivedDataBuildWorker, Display, "%30ls : '%ls'", *Function.Key, *WriteToString<64>(Function.Value));
	}
	Writer.EndArray();
	...
	for (const FString& VersionPath : VersionPaths)
	{
		...
		{
			UE_LOGF(LogDerivedDataBuildWorker, Error, "Unable to open %ls for writing", *VersionPath);
		}
	}
...
bool FBuildWorkerProgram::Build()
{
	...
	FRequestOwner Owner(EPriority::Normal);
	{
		...
		{
			UE_LOGF(LogDerivedDataBuildWorker, Log, "Loading build action '%ls'", *ActionPath);
			...
			{
				UE_LOGF(LogDerivedDataBuildWorker, Error, "Missing build action '%ls'", *ActionPath);
				return false;
			}
			if (FOptionalBuildAction Action = FBuildAction::Load({ActionPath}, MoveTemp(ActionObject)); Action.IsNull())
			{
				UE_LOGF(LogDerivedDataBuildWorker, Error, "Invalid build action '%ls'", *ActionPath);
				return false;
			}
...
void FBuildWorkerProgram::BuildComplete(FBuildCompleteParams&& Params) const
{
	...
	for (const FBuildOutputMessage& Message : Output.GetMessages())
	{
		...
		case EBuildOutputMessageLevel::Error:
			UE_LOGF(LogDerivedDataBuildWorker, Error, "%ls (Build of '%ls' by %ls.)",
				*WriteToString<256>(Message.Message), *Name, *WriteToString<32>(Function));
			break;
		case EBuildOutputMessageLevel::Warning:
			UE_LOGF(LogDerivedDataBuildWorker, Warning, "%ls (Build of '%ls' by %ls.)",
				*WriteToString<256>(Message.Message), *Name, *WriteToString<32>(Function));
			break;
		case EBuildOutputMessageLevel::Display:
			UE_LOGF(LogDerivedDataBuildWorker, Display, "%ls (Build of '%ls' by %ls.)",
				*WriteToString<256>(Message.Message), *Name, *WriteToString<32>(Function));
			break;
		default:
			checkNoEntry();
	...
	for (const FValueWithId& Value : Output.GetValues())
	{
		if (Value.HasData())
		{
			...
			UE_LOGF(LogDerivedDataBuildWorker, Error,
				"Failed to store build output %ls for build of '%ls' by %ls.",
	...
	{
		UE_LOGF(LogDerivedDataBuildWorker, Error,
			"Failed to store build output to '%ls' for build of '%ls' by %ls.",
...
bool FBuildWorkerProgram::ResolveInputExists(const FBuildAction& Action) const
{
	...
	Action.IterateInputs([this, &Action, &bValid](FUtf8StringView Key, const FIoHash& RawHash, uint64 RawSize)
	{
		...
		if (!IFileManager::Get().FileExists(*Path))
		{
			bValid = false;
			UE_LOGF(LogDerivedDataBuildWorker, Error,
				"Input '%ls' with raw hash %ls is missing for build of '%ls' by %ls.", *WriteToString<64>(Key),
...
DERIVEDDATABUILDWORKER_API int32 BuildWorkerMain(int32 ArgC, TCHAR* ArgV[], const TCHAR* ExtraArgs, const TCHAR* LoadModules)
{
	ON_SCOPE_EXIT
	{
	...
	if (const int32 ErrorLevel = GEngineLoop.PreInit(ArgC, ArgV, ExtraArgs))
	{
		return ErrorLevel;
	}
	...
	UE::String::ParseTokens(LoadModules, '+', [&ModuleManager](FStringView ModulePattern)
	{
		TArray<FName> Modules;
		ModuleManager.FindModules(*WriteToString<64>(ModulePattern), Modules);
	...
	UE_LOGF(LogDerivedDataBuildWorker, Log, "Worker completed in %.3fms", FPlatformTime::ToMilliseconds64(FPlatformTime::Cycles64()-WorkerStartTime));
...
{
	return BuildWorkerMain(ArgC, ArgV, TEXT("-Unattended"), TEXT("*"));
}
