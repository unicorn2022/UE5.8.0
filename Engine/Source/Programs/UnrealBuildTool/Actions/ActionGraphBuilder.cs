...
namespace UnrealBuildTool
{
	...
	sealed class NullActionGraphBuilder : IActionGraphBuilder
	{
		...
		/// Constructor
		/// </summary>
		public NullActionGraphBuilder(ILogger InLogger, FileHashCache? fileHashCache = null)
		{
			Logger = InLogger;
		...
		{
			if (_fileHashCache != null)
			{
				_fileHashCache.WriteFileIfChanged(FileItem, Contents, Logger, false);
			}
			else
			{
		...
		public void CreateIntermediateTextFile(FileItem FileItem, IEnumerable<string> ContentLines, bool AllowAsync = true)
		{
			if (_fileHashCache != null)
			{
				_fileHashCache.WriteFileIfChanged(FileItem, ContentLines, Logger, false);
			}
			else
			{
	...
	static class ActionGraphBuilderExtensions
	{
		...
		public static Action CreateRecursiveAction<T>(this IActionGraphBuilder Graph, ActionType Type, string Arguments) where T : IToolMode<T>, new()
		{
			...
			NewAction.CommandArguments = $"\"{Unreal.UnrealBuildToolDllPath}\" -Session=\"{UnrealBuildTool.SessionIdentifier}\" -Mode={T.Name} {Arguments}";
			NewAction.CommandDescription = T.Name;
