...
namespace UnrealBuildTool
{
	internal static class ActionGraph
	{
		...
		{
			using ITimelineEvent _ = Timeline.ScopeEvent($"LinkActions");

			// Build a map from item to its producing action
			Dictionary<FileItem, LinkedAction> ItemToProducingAction = new (Actions.Count * 3);
			...
			if (BuildHostPlatform.Current.Platform == UnrealTargetPlatform.Win64)
			{
				...
				bool ShouldWarn(FileItem item)
				{
					...
					if (item.Location.FullName.Contains("/Intermediate/", StringComparison.OrdinalIgnoreCase))
					{
						...
						if (item.Location.FullName.EndsWith(".i.PVS-Studio.log", StringComparison.Ordinal)) // Ignore PVS Studio intermediate items
						{
							return false;
						}
					}
		...
		/// </summary>
		public static async Task ExecuteActionsAsync(BuildConfiguration BuildConfiguration, List<LinkedAction> ActionsToExecute, List<TargetDescriptor> TargetDescriptors, ILogger Logger, ActionExecutor? executor = null)
		{
			if (ActionsToExecute.Count == 0)
			{
				...
				if (executor != null)
				{
					// We still want to run the executor to give it a chance to clean up in case it has tasks running that can be cancelled
					await InternalExecuteActions(executor, ActionsToExecute, Logger);
				}
			}
			else
			{
				...
				if (executor == null)
				{
					using ActionExecutor SelectedExecutor = ExecutorFactory.Instance.GetExecutor(BuildConfiguration, ActionsToExecute.Count, TargetDescriptors, Logger);
					await InternalExecuteActions(SelectedExecutor, ActionsToExecute, Logger);
				}
				else
				{
					await InternalExecuteActions(executor, ActionsToExecute, Logger);
		...
		static async Task InternalExecuteActions(ActionExecutor Executor, List<LinkedAction> ActionsToExecute, ILogger Logger)
		{
			Logger.LogInformation("Using {ExecutorName} executor to run {ActionCount} action(s)", Executor.Name, ActionsToExecute.Count);

			// Force an aggressive garbage collection in the background before running actions to free up as much memory as possible as compiling and linking can use a significant amount of memory
			Task gcTask = Task.Run(() => {
				using ITimelineEvent _ = Timeline.ScopeEvent("GarbageCollect");
		...
		{
			using ITimelineEvent _ = Timeline.ScopeEvent($"DetectActionGraphCycles");
			...
			if (ActionIsNonCyclical.Count < Actions.Count)
			{
				...
				foreach (LinkedAction Action in Actions)
				{
					if (!ActionIsNonCyclical.ContainsKey(Action))
					{
						...
						Logger.LogInformation("{CycleDescription}", CycleDescription);
					}
				}
		...
		{
			using ITimelineEvent _ = Timeline.ScopeEvent($"GatherPrerequisiteActions");
			...
			ActionHistory ActionHistory, CppDependencyCache CppDependencies, bool bIgnoreOutdatedImportLibraries, ILogger Logger)
		{
			...
			{
				ProducedItem.Directory.CacheFiles();

				if (!ProducedItem.HasInfo)
				{
					//Console.WriteLine("HASINFO " + ProducedItem);
				}
			...
			if (!bIsOutdated)
			{
				foreach (FileItem PrerequisiteItem in RootAction.PrerequisiteItems)
				{
					...
					PrerequisiteItem.Directory.CacheFiles();
		...
		static bool IsImportLibraryDependency(LinkedAction RootAction, FileItem PrerequisiteItem)
		{
			if (IsLibraryFile(PrerequisiteItem.Location))
			{
				foreach (LinkedAction PrerequisiteAction in RootAction.PrerequisiteActions)
				{
					if (PrerequisiteAction.bProducesImportLibrary && PrerequisiteAction.ProducedItems.FirstOrDefault(PrerequisiteItem) != null)
					{
						return true;
					}
				}
		...
		{
			using ITimelineEvent __ = Timeline.ScopeEvent($"GatherOutdatedActions");

			using (GlobalTracer.Instance.BuildSpan("Cache individual outdated actions").StartActive())
			{
				Parallel2.ForEach(Actions,
		...
		{
			using ITimelineEvent __ = Timeline.ScopeEvent($"DeleteOutdatedProducedItems");

			Parallel2.ForEach(OutdatedActions.SelectMany(oa => oa.DeleteItems), DeleteItem =>
			{
		...
		{
			using ITimelineEvent __ = Timeline.ScopeEvent($"CreateDirectoriesForProducedItems");
