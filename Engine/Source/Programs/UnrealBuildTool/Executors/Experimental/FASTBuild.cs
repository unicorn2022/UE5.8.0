...
namespace UnrealBuildTool
{
	...
	sealed class FASTBuild : ActionExecutor
	{
		...
		/// </summary>
		readonly UBAExecutor LocalExecutor;
		...
		/// Constructor
		/// </summary>
		public FASTBuild(BuildConfiguration BuildConfiguration, ILogger Logger, IEnumerable<TargetDescriptor> TargetDescriptors)
			: base(Logger)
		{
			...
			// Disable remote, as this is intended as a local executor.
			Config.bDisableRemote = true;
		...
		internal const string ExecutorName = "FASTBuild";
		...
		private static readonly Tuple<string, Func<LinkedAction, string>, FBBuildType>[] BuildTypeSearchParams = new Tuple<string, Func<LinkedAction, string>, FBBuildType>[]
		{
			...
			(
				"apple",
#pragma warning disable CA1308 // Normalize strings to uppercase - external command may be case-sensitive
				Action => Action.CommandArguments.ToLowerInvariant(),
#pragma warning restore CA1308
			...
			(
				"/bin/sh",
#pragma warning disable CA1308 // Normalize strings to uppercase - external command may be case-sensitive
				Action => Action.CommandPath.FullName.ToLowerInvariant(),
#pragma warning restore CA1308
				FBBuildType.Apple
			),
			Tuple.Create<string, Func<LinkedAction, string>, FBBuildType>
		...
		private bool DetectBuildType(IEnumerable<LinkedAction> Actions, ILogger Logger)
		{
			foreach (LinkedAction Action in Actions)
			{
				foreach (Tuple<string, Func<LinkedAction, string>, FBBuildType> BuildTypeSearchParam in BuildTypeSearchParams)
				{
					...
					if (BuildTypeSearchParam.Item2(Action).Contains(BuildTypeSearchParam.Item1, StringComparison.Ordinal))
					{
		...
		private static string SubstituteEnvironmentVariables(string commandLineString)
		{
			return commandLineString
				.Replace("$(DXSDK_DIR)", "$DXSDK_DIR$", StringComparison.Ordinal)
				.Replace("$(CommonProgramFiles)", "$CommonProgramFiles$", StringComparison.Ordinal);
		}

		private static Dictionary<string, string> ParseCommandLineOptions(string LocalToolName, string CompilerCommandLine, string[] SpecialOptions, ILogger Logger, bool SaveResponseFile = false)
		{
			...
			int ResponseFileTokenIndex = Array.FindIndex(RawTokens, RawToken => RawToken.StartsWith("@\"", StringComparison.Ordinal));
			if (ResponseFileTokenIndex == -1)
			{
				ResponseFileTokenIndex = Array.FindIndex(RawTokens, RawToken => RawToken.StartsWith("@", StringComparison.Ordinal));
			}
			if (ResponseFileTokenIndex > -1) //Response files are in 4.13 by default. Changing VCToolChain to not do this is probably better.
			{
				...
				{
					if (RawTokens[i - 1].Contains(".response", StringComparison.Ordinal) || RawTokens[i - 1].Contains(".rsp", StringComparison.Ordinal))
					{
						break;
					}
			...
			for (int i = 0; i < AllTokens.Count; ++i)
			{
				...
				if ((Token.StartsWith("/D", StringComparison.Ordinal) || Token.StartsWith("-D", StringComparison.Ordinal)) && !QuotesOpened)
				{
					...
					else
					{
						...
						for (; i < AllTokens.Count; ++i)
						{
							...
							else if (!NextToken.EndsWith("\\\"", System.StringComparison.Ordinal) && NextToken.EndsWith("\"", System.StringComparison.Ordinal)) //Looking for a token that ends with a non-escaped "
							{
			...
			foreach (string specialOption in SpecialOptions)
			{
				for (int i = 0; i < ProcessedTokens.Count; ++i)
				{
					...
					else if (ProcessedTokens[i].StartsWith(specialOption, StringComparison.Ordinal))
					{
						ParsedCompilerOptions[specialOption] = ProcessedTokens[i].Replace(specialOption, null, StringComparison.Ordinal);
			...
			for (int i = 0; i < ProcessedTokens.Count; ++i)
			{
				...
				if ((Token == "/I") ||
					...
					(Token.Contains("/experimental", StringComparison.Ordinal)))
				{
					++i;
				}
				...
				else if (Token.Contains("clang++", StringComparison.Ordinal))
				{
					ProcessedTokens.RemoveAt(i);
					i--;
				}
				...
				else if (!Token.StartsWith("/", StringComparison.Ordinal) && !Token.StartsWith("-", StringComparison.Ordinal) && !Token.Contains(".framework", StringComparison.Ordinal))
				{
		...
		private static string GetOptionValue(Dictionary<string, string> OptionsDictionary, string Key, LinkedAction Action, ILogger Logger, bool ProblemIfNotFound = false)
		{
			...
			if (VCEnv != null)
			{
				...
				if (MsvcCRTRedistVersion.Length > 0)
				{
					PrefferedMSVCRedistPath = PotentialMSVCRedistPaths.Find(
						delegate (string str)
						{
							return str.Contains(MsvcCRTRedistVersion, StringComparison.Ordinal);
						});
				}
				...
				FinalMSVCRedistPath = PotentialMSVCRedistPaths.Find(x => x.Contains(".CRT", StringComparison.Ordinal));
		...
		private void AddCompileAction(LinkedAction Action, IEnumerable<LinkedAction> DependencyActions, ILogger Logger)
		{
			...
			if (Action.CommandPath.FullName.Contains("rc.exe", StringComparison.Ordinal))
			{
			...
			IntermediatePath = IsApple() ? IntermediatePath.Replace("\\", "/", StringComparison.Ordinal) : IntermediatePath;
			...
			{
				OtherCompilerOptions = OtherCompilerOptions.Replace("/WX", "", StringComparison.Ordinal);
			}
			...
			else if (ParsedCompilerOptions.ContainsKey("/Yu")) //Use PCH
			{
				...
				string PCHOutputFile = GetOptionValue(ParsedCompilerOptions, "/Fp", Action, Logger, ProblemIfNotFound: true);
				string PCHToForceInclude = PCHOutputFile.Replace(".pch", "", StringComparison.Ordinal);
		...
		private void AddExecAction(LinkedAction Action, IEnumerable<LinkedAction> DependencyActions)
		{
		...
		private bool CreateBffFile(IEnumerable<LinkedAction> Actions, string BffFilePath, ILogger Logger)
		{
			try
			{
				...
				foreach (LinkedAction Action in Actions)
				{
					...
					{
						AddExecAction(Action, DependencyActions);
					}
				}
		...
		private bool ExecuteBffFile(string BffFilePath, ILogger Logger)
		{
			...
			string MaxProcesses = "-j" + LocalExecutor.NumParallelProcesses;
