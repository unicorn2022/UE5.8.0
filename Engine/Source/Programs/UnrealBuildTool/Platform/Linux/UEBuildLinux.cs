...
namespace UnrealBuildTool
{
	...
	{
		private static readonly IReadOnlyDictionary<UnrealArch, string> LinuxToolchainArchitectures = new Dictionary<UnrealArch, string>()
		{
	...
	/// <summary>
	/// Available runtime ray tracing modes on Linux platform
	/// </summary>
	public enum LinuxRayTracingRuntimeMode
	{
		/// <summary>
	...
	public class LinuxTargetRules
	{
		...
		/// <summary>
		/// If specified along with -PGOProfile, use sample-based PGO instead of instrumented.
		/// </summary>
		[XmlConfigFile(Category = "BuildConfiguration")]
		[CommandLine("-LinuxSampleBasedPGO")]
		public bool bLinuxSampleBasedPGO = false;

		/// <summary>
		/// Enables runtime ray tracing support.
		/// </summary>
		[ConfigFile(ConfigHierarchyType.Engine, "/Script/LinuxTargetPlatform.LinuxTargetSettings", "RayTracingMode")]
		public LinuxRayTracingRuntimeMode RayTracingMode = LinuxRayTracingRuntimeMode.Disabled;
	}
...
#pragma warning disable CS1591
		...
		public LinuxRayTracingRuntimeMode RayTracingMode => Inner.RayTracingMode;

		public bool bDisableStripSymbols => Inner.bDisableStripSymbols;

		public bool bLinuxSampleBasedPGO => Inner.bLinuxSampleBasedPGO;
#pragma warning restore CS1591
	...
	class LinuxPlatform : UEBuildPlatform
	{
		...
		public static bool IsLTOEnabled(ReadOnlyTargetRules Target)
		{
		...
		public override void ValidateTarget(TargetRules Target)
		{
			...
			if (!Target.IsNameOverriden() && !String.IsNullOrEmpty(Target.BuildVersion) && Target.Type != TargetType.Editor)
			{
				...
				if (Target.LinuxPlatform.bEnableAutoRTFMSanitizer)
				{
					SanitizerSuffix += "AutoRTFMSan";
				}
		...
		public virtual void SetUpSpecificEnvironment(ReadOnlyTargetRules Target, CppCompileEnvironment CompileEnvironment, LinkEnvironment LinkEnvironment)
		{
			...
			CompileEnvironment.Definitions.Add("UE_PLATFORM_LINUX=1");
			CompileEnvironment.Definitions.Add("UE_PLATFORM_UNIX=1");
		...
		public override void SetUpEnvironment(ReadOnlyTargetRules Target, CppCompileEnvironment CompileEnvironment, LinkEnvironment LinkEnvironment)
		{
			...
			if (CompileEnvironment.bPGOOptimize)
			{
				...
				if (!Info.Exists || Info.Length == 0)
				{
					if (!Info.Exists)
					{
						Logger.LogWarning("Warning: PGO file '{PGOFilePath}' does not exist, disabling optimization", PGOFilePath);
					}
					else
					{
						Logger.LogInformation("PGO file '{PGOFilePath}' exists but has 0 length. Assuming that PGO data is temporarily missing, disabling optimization without a warning.", PGOFilePath);
					}
			...
			if (Target.LinuxPlatform.RayTracingMode != LinuxRayTracingRuntimeMode.Disabled && Target.Type != TargetType.Server)
			{
			...
			{
				// #jira SOL-9041: Sections prefixes cause relocation errors and thus we avoid using them.
				string DisableSectionsGuidedSectionPrefix = "-bbsections-guided-section-prefix=0";
				string DisableProfileGuidedSectionPrefix = "-profile-guided-section-prefix=0";

				CompileEnvironment.AdditionalArguments += $" -Xclang -mllvm -Xclang {DisableSectionsGuidedSectionPrefix} -Xclang -mllvm -Xclang {DisableProfileGuidedSectionPrefix}";
				LinkEnvironment.AdditionalArguments += $" -Wl,-mllvm -Wl,{DisableSectionsGuidedSectionPrefix} -Wl,-mllvm -Wl,{DisableProfileGuidedSectionPrefix}";
			}
		...
		public override UEToolChain CreateToolChain(ReadOnlyTargetRules Target)
		{
			...
			if (Target.LinuxPlatform.bEnableAutoRTFMSanitizer)
			{
				Options |= ClangToolChainOptions.EnableAutoRTFMSanitizer;
			}
			...
			if (Target.LinuxPlatform.bLinuxSampleBasedPGO)
			{
				Options |= ClangToolChainOptions.SampleBasedPGO;
			}
