		...
		public static bool IsRunningUnderRosetta
		{
			get
			{
				if (!IsRunningUnderRosettaVar.HasValue)
				{
					...
					IsRunningUnderRosettaVar = TranslatedOutput.Contains("sysctl.proc_translated: 1", StringComparison.Ordinal);
				}
		...
		public static bool IsRunningOnAppleArchitecture
		{
			get
			{
				if (!IsRunningOnAppleArchitectureVar.HasValue)
				{
					...
					IsRunningOnAppleArchitectureVar = BrandOutput.Contains("Apple", StringComparison.Ordinal) || IsRunningUnderRosetta;
				}
