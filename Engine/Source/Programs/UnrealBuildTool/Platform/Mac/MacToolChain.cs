...
namespace UnrealBuildTool
{
	...
	class MacToolChain : AppleToolChain
	{
		...
		protected override string XnuPlatformDefine => "XNU_PLATFORM_MacOSX";

		protected override ClangToolChainInfo GetToolChainInfo()
		{
			FileReference CompilerPath = FileReference.Combine(AppleToolChainSettings.ToolchainDir, MacCompiler);
			FileReference ArchiverPath = FileReference.Combine(AppleToolChainSettings.ToolchainDir, MacArchiver);
...
!String.Equals(ProgramFinder.GetDirectoryName(), "Source", StringComparison.OrdinalIgnoreCase) &&
!String.Equals(ProgramFinder.GetDirectoryName(), "Intermediate", StringComparison.OrdinalIgnoreCase) &&
!String.Equals(ProgramFinder.GetDirectoryName(), "Binaries", StringComparison.OrdinalIgnoreCase))
			{
		...
		void AddFrameworkToLinkCommand(LinkEnvironment linkEnvironment, List<string> LinkArguments, string FrameworkName, string Arg = "-framework")
		{
			if (FrameworkName.EndsWith(".framework", StringComparison.Ordinal))
			{
				FileReference frameworkPath = new FileReference(FrameworkName);
				LinkArguments.Add($"-F \"{NormalizeCommandLinePath(frameworkPath.Directory, linkEnvironment.RootPaths)}\"");
		...
		private void AddLibraryPathToRPaths(string Library, string ExeAbsolutePath, ref List<string> RPaths, List<string> LinkArguments, bool bIsBuildingAppBundle, ILogger Logger)
		{
			...
			if ((Library.EndsWith("dylib", StringComparison.Ordinal) || Library.EndsWith(".framework", StringComparison.Ordinal))
				&& !LibraryFullPath.Contains("/Engine/Source/ThirdParty/", StringComparison.Ordinal) && LibraryDir != ExeDir && !RPaths.Contains(LibraryDir))
			{
				...
				bool bCanUseMultipleRPATHs = !ExeAbsolutePath.Contains("EpicGamesLauncher-Mac-Shipping", StringComparison.Ordinal) || !Library.Contains("CEF3", StringComparison.Ordinal);
				...
				string RelativePath = Utils.MakePathRelativeTo(LibraryDir, FinalExeDir).Replace("\\", "/", StringComparison.Ordinal);
				...
				if (bCanUseMultipleRPATHs)
				{
					...
					if (!bIsBuildingAppBundle && LibraryDir.Contains("/Engine/Plugins/", StringComparison.Ordinal) && ExeAbsolutePath.EndsWith("dylib", StringComparison.Ordinal))
					{
						int EngineDirStrIndex = LibraryDir.IndexOf("Engine", StringComparison.Ordinal);
				...
				if (bIsBuildingAppBundle)
				{
					...
					if (LibraryDir.StartsWith(EngineDir + "/", StringComparison.Ordinal))
					{
						BundleLibraryDir = LibraryDir.Replace(EngineDir, BundleUEDir, StringComparison.Ordinal);
					}
					else if (ProjectDir != null && LibraryDir.StartsWith(ProjectDir + "/", StringComparison.Ordinal))
					{
						string GameName = GetGameNameFromExecutablePath(ExeAbsolutePath);
						BundleLibraryDir = LibraryDir.Replace(ProjectDir, BundleUEDir + "/" + GameName, StringComparison.Ordinal);
					}

					if (BundleLibraryDir != null)
					{
						string BundleRelativeDir = Utils.MakePathRelativeTo(BundleLibraryDir, ExeDir).Replace("\\", "/", StringComparison.Ordinal);
					...
					if (bCanUseMultipleRPATHs)
					{
						...
						string StagedLibraryDir = LibraryDir.Replace(EngineDir, StagedUEDir, StringComparison.Ordinal);
						string StagedRelativeDir = Utils.MakePathRelativeTo(StagedLibraryDir, ExeDir).Replace("\\", "/", StringComparison.Ordinal);
		...
		public override FileItem[] LinkAllFiles(LinkEnvironment LinkEnvironment, bool bBuildImportLibraryOnly, IActionGraphBuilder Graph)
		{
			...
			if (LinkEnvironment.Architectures.bIsMultiArch)
			{
				...
				if (bBuildImportLibraryOnly)
				{
					return [.. PerArchOutputFiles];
				}

				FileItem OutputFileItem = FileItem.GetItemByFileReference(LinkEnvironment.OutputFilePath);
		...
		private FileItem MakeStubItem(LinkEnvironment LinkEnvironment, string DylibPath)
		{
			...
			string IntermediateDirectory = (ProjectFile == null || LinkEnvironment.OutputFilePaths.All(x => x.IsUnderDirectory(Unreal.EngineDirectory)) || bIsEngineDylib ? Unreal.EngineDirectory : ProjectFile.Directory) + "/Intermediate/Build/Mac";
			string DylibFilename = Path.GetFileName(DylibPath);

			string AppName = String.Empty;
			int firstDash = DylibFilename.IndexOf('-', StringComparison.Ordinal);
			if (firstDash != -1)
			{
				AppName = DylibFilename.Substring(0, firstDash);
			}
			else if (DylibFilename.StartsWith("lib", StringComparison.Ordinal)) // This is expected to exist. if there is no dash involved this is the target lib itself and name is part of the name
			{
				AppName = Path.GetFileNameWithoutExtension(DylibFilename).Substring(3);
			}
			return FileItem.GetItemByPath(Path.Combine(IntermediateDirectory, LinkEnvironment.Architecture.ToString(), AppName, LinkEnvironment.Configuration.ToString(), "Stubs", DylibFilename));
		}
		...
		private FileItem LinkArchitectureFiles(LinkEnvironment LinkEnvironment, FileReference MultiArchOutputFile, bool bBuildImportLibraryOnly, IActionGraphBuilder Graph)
		{
			...
			string AbsolutePath = MultiArchOutputFile.FullName.Replace("\\", "/", StringComparison.Ordinal);
			if (!LinkEnvironment.bIsBuildingLibrary)
			{
			...
			else if (!LinkEnvironment.bIsBuildingLibrary)
			{
				...
				if (bCanUseSimpleRPaths)
				{
					...
					void ProcessLib(FileReference SourceLib, FileReference DestLib, FileReference ReferencingFile)
					{
						if (String.Equals(SourceLib.GetExtension(), ".dylib", StringComparison.OrdinalIgnoreCase))
						{
				...
				foreach (string AdditionalLibrary in AdditionalLibraries)
				{
					if (!String.IsNullOrEmpty(Path.GetDirectoryName(AdditionalLibrary)) &&
							 (Path.GetDirectoryName(AdditionalLibrary)!.Contains("Binaries/Mac", StringComparison.Ordinal) ||
							 Path.GetDirectoryName(AdditionalLibrary)!.Contains("Binaries\\Mac", StringComparison.Ordinal)))
					{
					...
					else if (AdditionalLibrary.Contains(".framework/", StringComparison.Ordinal))
					{
				...
				{
					LinkArguments.Add($"-weak_library \"{NormalizeCommandLinePath(FileItem.GetItemByPath(AdditionalLibrary), LinkEnvironment.RootPaths)}\"");
				...
				{
					AddFrameworkToLinkCommand(LinkEnvironment, LinkArguments, Framework.Key, Framework.Value ? "-weak_framework" : "-framework");
				...
				if (LinkEnvironment.bCreateMapFile)
				{
					...
					int AppIdx = MapFileBaseName.IndexOf(".app/Contents/MacOS", StringComparison.Ordinal);
					if (AppIdx != -1)
					{
			...
			if (LinkEnvironment.bIsBuildingDLL)
			{
				...
				InstallName ??= String.Format("{0}/{1}", DylibsPath, Path.GetFileName(OutputFile.AbsolutePath).Replace($".dylib_{LinkEnvironment.Architecture}", ".dylib", StringComparison.Ordinal));
		...
		public static FileItem GenerateDebugInfo(FileItem MachOBinary, LinkEnvironment LinkEnvironment, IActionGraphBuilder Graph)
		{
			string BinaryPath = MachOBinary.AbsolutePath;
			if (BinaryPath.Contains(".app", StringComparison.Ordinal))
			{
				while (BinaryPath.Contains(".app", StringComparison.Ordinal))
				{
		...
		private static readonly Dictionary<ReadOnlyTargetRules, DirectoryReference> BundleContentsDirectories = new();
		...
		public override void ModifyBuildProducts(ReadOnlyTargetRules Target, UEBuildBinary Binary, IEnumerable<string> Libraries, IEnumerable<UEBuildBundleResource> BundleResources, Dictionary<FileReference, BuildProductType> BuildProducts)
		{
			if (Target.bUsePDBFiles == true)
			{
				...
				foreach (KeyValuePair<FileReference, BuildProductType> BuildProductPair in BuildProductsArray)
				{
					...
					else if (BuildProductPair.Value == BuildProductType.SymbolFile && BuildProductPair.Key.FullName.Contains(".app", StringComparison.Ordinal))
					{
			...
			DirectoryReference? BundleContentsDirectory;
			lock (BundleContentsDirectories)
			{
				...
				BundleContentsDirectory = BundleContentsDirectories.GetValueOrDefault(Target);
			}

			if (BundleContentsDirectory == null)
			{
				return;
			}
			...
			if (Binary.Type != UEBuildBinaryType.StaticLibrary)
			{
				foreach (UEBuildBundleResource Resource in BundleResources)
				{
					if (Directory.Exists(Resource.ResourcePath))
					{
						...
						{
							BuildProducts.Add(
								FileReference.Combine(BundleContentsDirectory!, Resource.BundleContentsSubdir!,
									ResourceFile.Substring(Path.GetDirectoryName(Resource.ResourcePath)!.Length + 1)),
								BuildProductType.RequiredResource);
						}
					}
					else
					{
						BuildProducts.Add(
							FileReference.Combine(BundleContentsDirectory, Resource.BundleContentsSubdir!,
								Path.GetFileName(Resource.ResourcePath)!), BuildProductType.RequiredResource);
					}
				}
			}
			...
			if (Binary.Type == UEBuildBinaryType.Executable)
			{
				...
				BuildProducts.Add(FileReference.Combine(BundleContentsDirectory!, "_CodeSignature", "CodeResources"),
					BuildProductType.RequiredResource);
			}
		}

		private readonly ConcurrentBag<FileItem> DebugInfoFiles = new ConcurrentBag<FileItem>();
		...
		public override void FinalizeOutput(ReadOnlyTargetRules Target, TargetMakefileBuilder MakefileBuilder, DirectoryReference ExeDir)
		{
			...
			{
				if (OutputFiles.Any(Item => String.Equals(Item, DebugItem.FullName, StringComparison.OrdinalIgnoreCase)))
				{
		...
		public override ICollection<FileItem> PostBuild(ReadOnlyTargetRules Target, FileItem Executable, LinkEnvironment BinaryLinkEnvironment, IActionGraphBuilder Graph)
		{
			...
			if ((BinaryLinkEnvironment.bIsBuildingDLL && (Options & ClangToolChainOptions.OutputDylib) == 0) ||
				(BinaryLinkEnvironment.bIsBuildingConsoleApplication && Executable.Name.EndsWith("-Cmd", StringComparison.Ordinal)))
			{
				return OutputFiles;
			}
