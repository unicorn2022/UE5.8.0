...
#pragma warning restore CS1591
	...
	class AndroidArchitectureConfig : UnrealArchitectureConfig
	{
		...
		public override UnrealArchitectures ActiveArchitectures(FileReference? ProjectFile, string? TargetName) => GetProjectArchitectures(ProjectFile);
		...
		private static UnrealArchitectures GetProjectArchitectures(FileReference? ProjectFile)
		{
	...
	class AndroidPlatform : UEBuildPlatform
	{
		...
		/// <inheritdoc/>
		public override VCProjectFileFormat GetRequiredVisualStudioVersion()
		{
			IEnumerable<VisualStudioInstallation> Installations = MicrosoftPlatformSDK.FindVisualStudioInstallations(Logger);

			Version? version = Installations
				.SelectMany(x => x.Packages.Where(x => x.Key.StartsWith("Component.GoogleAndroid", StringComparison.Ordinal)).Select(x => x.Value.Version))
				.Max();

			if (version != null && version < GetVersionRequiredForVisualStudio(VCProjectFileFormat.VisualStudio2026))
			{
				return VCProjectFileFormat.VisualStudio2022;
			}
		...
		/// <inheritdoc/>
		public override Version GetVersionRequiredForVisualStudio(VCProjectFileFormat Format)
		{
			return Format >= VCProjectFileFormat.VisualStudio2026
				? new Version(26, 1, 102)
				: new Version();
		}
		...
		static bool IsBuildProductWithArch(string Name, string[] NamePrefixes, string[] NameSuffixes, string Extension)
		{
			...
			if (Name.EndsWith(Extension, StringComparison.InvariantCultureIgnoreCase))
			{
				...
				foreach (string CpuSuffix in AndroidToolChain.AllCpuSuffixes.Values)
				{
					...
					if (CpuIdx > 0 && String.Compare(Name, CpuIdx, CpuSuffix, 0, CpuSuffix.Length, StringComparison.OrdinalIgnoreCase) == 0)
					{
			...
			if ((Target.Platform == UnrealTargetPlatform.Win64) || (Target.Platform == UnrealTargetPlatform.Mac) || (Target.Platform == UnrealTargetPlatform.Linux))
			{
				...
				if (!Target.bBuildRequiresCookedData && Target.bBuildDeveloperTools)
				{
					...
					else if (ModuleName == "TargetPlatform")
					{
						...
						Rules.DynamicallyLoadedModuleNames.Add("TextureFormatOodle");
		...
		public override void ModifyModuleRulesForActivePlatform(string ModuleName, ModuleRules Rules, ReadOnlyTargetRules Target)
		{
			if (Target.StaticAnalyzer == StaticAnalyzer.Default)
			{
				Rules.StaticAnalyzerDisabledCheckers.UnionWith(["nullability.NullPassedToNonnull"]);
			}
		}
		...
		public virtual void SetUpSpecificEnvironment(ReadOnlyTargetRules Target, CppCompileEnvironment CompileEnvironment, LinkEnvironment LinkEnvironment, ILogger Logger)
		{
			...
			// There's some conspicuous static initialization that sets the Clang version
			_ = new AndroidToolChain(Target.ProjectFile, Logger);
			...
			Logger.LogInformation("NDK toolchain: {Version}, NDK version: {NdkVersion}, ClangVersion: {ClangVersion}", NDKToolchainVersion, NDKVersionInt, AndroidToolChain.GetClangVersionString());
			...
			CompileEnvironment.Definitions.Add("UE_PLATFORM_ANDROID=1");
		...
		private static bool UseTegraGraphicsDebugger(ReadOnlyTargetRules _)
		{
			// Disable for now
			return false;
		}
		...
		public override void Deploy(TargetReceipt Receipt)
		{
			...
			if (ProjectFile == null)
			{
				Utils.TryParseProjectFileArgument(new CommandLineArguments(Environment.GetCommandLineArgs()), Logger, out ProjectFile, MarkArgumentAsUsed: false);
				if (ProjectFile != null)
				{
					Logger.LogInformation("Deploy: receipt has no ProjectFile; recovered {ProjectFile} from command line.", ProjectFile);
				}
			}

			new UEDeployAndroid(ProjectFile, false, Logger).PrepTargetForDeployment(Receipt);
		}
	}
