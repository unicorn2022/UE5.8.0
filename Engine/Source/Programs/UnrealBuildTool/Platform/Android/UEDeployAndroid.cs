...
namespace UnrealBuildTool
{
	class UEDeployAndroid : UEBuildDeploy, IAndroidDeploy
	{
		...
		/// <summary>
		/// Won't generate or use OBBs if set
		/// </summary>
		[CommandLine("-NoObbs", Value = "true")]
		public bool bNoObbs = false;
		bool IAndroidDeploy.bNoObbs => bNoObbs;
		...
		public void SetAndroidPluginData(UnrealArchitectures Architectures, List<string> inPluginExtraData)
		{
			...
			foreach (string Plugin in inPluginExtraData)
			{
				...
				if (Plugin.Contains("OnlineSubsystemFacebook_UPL", StringComparison.Ordinal))
				{
				...
				if (Plugin.Contains("GoogleARCoreBase_APL", StringComparison.Ordinal))
				{
				...
				if (Plugin.Contains("EOSSDK", StringComparison.Ordinal))
				{
		...
		private static bool ValidateSDK(string PlatformsDir, string ApiString)
		{
		...
		private int GetApiLevelInt(string ApiString)
		{
			...
			if (DashIndex != -1 && Int32.TryParse(ApiString.Substring(DashIndex + 1), out int Version))
			{
				return Version;
			}
		...
		private string GetSdkApiLevel(AndroidToolChain ToolChain)
		{
			if (CachedSDKLevel == null)
			{
				...
				if (SDKLevel == "latest")
				{
					SDKLevel = AndroidToolChain.GetLargestApiLevel();
				}
		...
		private string GetBuildToolsVersion(AndroidToolChain ToolChain)
		{
			...
			CachedBuildToolsVersion = ToolChain.GetBuildToolsVersion();
		...
		public bool IsPackagingForMetaQuest(string CookFlavor, ConfigHierarchy? Ini = null)
		{
			...
			return (result && bPackageForMetaQuest) || CookFlavor == "_OpenXR";
		}
		...
		public static void CopyFileDirectory(string SourceDir, string DestDir, Dictionary<string, string>? Replacements = null, string[]? Excludes = null)
		{
			...
			foreach (string Filename in Files)
			{
				...
				if (Ext == ".xml" && Replacements != null)
				{
					...
					{
						Contents = Contents.Replace(Pair.Key, Pair.Value, StringComparison.Ordinal);
					}
		...
		public static void DeleteDirectory(string InPath, ILogger Logger, string SubDirectoryToKeep = "")
		{
			...
			if (String.Equals(Path.GetFileName(InPath), SubDirectoryToKeep, StringComparison.OrdinalIgnoreCase))
			{
				return;
			}
		...
		private static bool BinaryFileEquals(string SourceFilename, string DestFilename)
		{
		...
		public static string GetUnrealBuildFilePath(string EngineDirectory)
		{
			return Path.GetFullPath(Path.Combine(EngineDirectory, "Build/Android/Java"));
		}

		public static string GetUnrealPreBuiltFilePath(string EngineDirectory)
		{
			return Path.GetFullPath(Path.Combine(EngineDirectory, "Build/Android/Prebuilt"));
		}

		public static string GetUnrealJavaSrcPath()
		{
			return Path.Combine("src", "com", "epicgames", "unreal");
		}

		public static string GetUnrealJavaFilePath(string EngineDirectory)
		{
			return Path.GetFullPath(Path.Combine(GetUnrealBuildFilePath(EngineDirectory), GetUnrealJavaSrcPath()));
		}

		public static string GetUnrealJavaBuildSettingsFileName(string EngineDirectory)
		{
			return Path.Combine(GetUnrealJavaFilePath(EngineDirectory), "JavaBuildSettings.java");
		}

		public static string GetUnrealJavaDownloadShimFileName(string Directory)
		{
			return Path.Combine(Directory, "DownloadShim.java");
		}

		public static string GetUnrealTemplateJavaSourceDir(string Directory)
		{
			return Path.Combine(GetUnrealBuildFilePath(Directory), "JavaTemplates");
		}

		public static string GetUnrealTemplateJavaDestination(string Directory, string FileName)
		{
			return Path.Combine(Directory, FileName);
		}

		public static string GetUnrealJavaOBBDataFileName(string Directory)
		{
		...
		private static void MakeDirectoryIfRequired(string DestFilename)
		{
		...
		public int GetStoreVersion(UnrealArch? Architecture)
		{
			if (CachedStoreVersion < 1)
			{
				...
				if (bUseChangeListAsStoreVersion)
				{
					...
					if (EngineVersion == null || !(Int32.TryParse(EngineChangelist, out int Changelist)) || Changelist == 0)
					{
		...
		public void WriteJavaOBBDataFile(string FileName, string PackageName, List<string> ObbSources, string CookFlavor, bool bPackageDataInsideApk, UnrealArch UnrealArch)
		{
			...
			string AppType = "";
			if (CookFlavor.EndsWith("Client", StringComparison.Ordinal))
			{
			...
			foreach (string ObbSource in ObbSources)
			{
				...
				obbData.Append("new XAPKFile(\n" + (ObbSource.Contains(".patch.", StringComparison.Ordinal) ? "false, // false signifies a patch file\n" : "true, // true signifies a main file\n"));
			...
			if (!bMustWriteFile && bFileExists)
			{
				...
				{
					if (FileLine.Contains("AppType =", StringComparison.Ordinal))
					{
						bHasAppType = true;
						break;
					}
		...
		public void WriteJavaDownloadSupportFiles(string ShimFileName, IEnumerable<TemplateFile> TemplateFiles, Dictionary<string, string> replacements)
		{
			...
			foreach (TemplateFile template in TemplateFiles)
			{
				...
				for (int i = 0; i < templateSrc.Length; ++i)
				{
					...
					{
						if (srcLine.Contains(kvp.Key, StringComparison.Ordinal))
						{
							srcLine = srcLine.Replace(kvp.Key, kvp.Value, StringComparison.Ordinal);
							changed = true;
						}
					}
					if (changed)
		...
		public void WriteCrashlyticsResources(string UEBuildPath, string PackageName, bool bIsEmbedded, UnrealArch UnrealArch)
		{
		...
		public static string GetNDKArch(UnrealArch UnrealArch)
		{
		...
		{
			AndroidToolChain.StripSymbols(new FileReference(SourceFileName), new FileReference(TargetFileName), UnrealArch, Logger, bStripAll);
		}

		private static List<string> GetSODependencies(string SOFilename)
		{
			List<string> DependencyList = new List<string>();
			string ReadElfExecutable = AndroidToolChain.GetReadElfExecutablePath();
			...
			if (Output != null)
			{
				...
				int ScanStart = 0;
				int SharedIndex = Output.IndexOf("Shared library: [", ScanStart, StringComparison.Ordinal);
				while (SharedIndex >= 0)
				{
					...
					if (EndIndex > 0)
					{
						...
						if (SharedLib.StartsWith("lib", StringComparison.Ordinal))
						{
							SharedLib = SharedLib.Substring(3);
						}
						// strip off .so (not needed for System.loadLibrary()
						if (SharedLib.EndsWith(".so", StringComparison.Ordinal))
						{
					...
					SharedIndex = Output.IndexOf("Shared library: [", ScanStart, StringComparison.Ordinal);
				}
			}
			return DependencyList;
		}

		private static bool IsSO16KbCompatible(string FileName)
		{
			string ReadElfExecutable = AndroidToolChain.GetReadElfExecutablePath();
		...
		private void CheckSOInFolderAre16KbCompatible(string FolderPath)
		{
			try
			{
		...
		private void CheckSOInGradleFolderAre16KbCompatible(string GradlePath)
		{
			CheckSOInFolderAre16KbCompatible($"{GradlePath}/jniLibs/arm64-v8a");
			CheckSOInFolderAre16KbCompatible($"{GradlePath}/libs/arm64-v8a");
		}
		...
		public static string GetPlatformNDKHostName()
		{
		...
		private static void CopySTL(string UnrealBuildPath, string NDKArch)
		{
			// copy it in!
		...
		private static void CopyPSOService(string UnrealBuildPath, string PreBuiltPath, string NDKArch)
		{
			// copy it in!
		...
		private static void CopySwappy(string UnrealBuildPath, string NDKArch, UnrealTargetConfiguration Configuration)
		{
		...
		void CopyVulkanValidationLayers(string UnrealBuildPath, string NDKArch, string Configuration)
		{
			ConfigHierarchy Ini = GetConfigCacheIni(ConfigHierarchyType.Engine);
			Ini.GetBool("/Script/AndroidRuntimeSettings.AndroidRuntimeSettings", "bSupportsVulkan", out bool bSupportsVulkan);
			...
			if (bCopyVulkanLayers)
			{
				...
				AndroidPlatformSDK.GetPath(Ini, "/Script/AndroidRuntimeSettings.AndroidRuntimeSettings", "DebugVulkanLayerDirectory", out string DebugVulkanLayerDirectory);
		...
		void CopyClangSanitizerLib(AndroidToolChain ToolChain, string UnrealBuildPath, string NDKArch, AndroidToolChain.ClangSanitizer Sanitizer)
		{
				...
				case AndroidToolChain.ClangSanitizer.HwAddress:
					{
						...
						LibName = AndroidToolChain.HasEmbeddedHWASanSupport() ? String.Empty : "hwasan";
						break;
					}
			...
			string WrapShName = Sanitizer == AndroidToolChain.ClangSanitizer.HwAddress && AndroidToolChain.HasEmbeddedHWASanSupport() ? "hwasan.sh" : "asan.sh";
			...
			string LibsVersion = AndroidToolChain.GetClangVersionString();
			if (ToolChain.GetNdkVersionInt() >= 270000)
			{
				LibsVersion = AndroidToolChain.GetClangVersionMajor().ToString(CultureInfo.InvariantCulture);
			}

			string SanitizerLib = String.IsNullOrEmpty(SanitizerFullLibName) ?
				String.Empty :
				Path.Combine(Environment.ExpandEnvironmentVariables("%NDKROOT%"), "toolchains", "llvm", "prebuilt", PlatformHostName, (AndroidToolChain.HasEmbeddedHWASanSupport() ? "lib" : "lib64"), "clang", LibsVersion, "lib", "linux", SanitizerFullLibName);
			...
			if (!String.IsNullOrEmpty(SanitizerLib))
			{
				if (File.Exists(SanitizerLib))
				{
					...
					Logger.LogInformation("Copying {LibName} lib from {SanitizerLib} to {LibDestFilePath}", LibName, SanitizerLib, LibDestFilePath);
					File.Copy(SanitizerLib, LibDestFilePath, true);
				}
				else
				{
					throw new BuildException("No {0} lib found in {1}", LibName, SanitizerLib);
				}
			}
			...
			if (File.Exists(WrapShFilePath))
			{
				...
				string WrapShContent = File.ReadAllText(WrapShFilePath);
				// replace the hwasan.sh with a debuggable script.
				// see https://developer.android.com/ndk/guides/wrap-script#debugging_when_using_wrapsh
				// first, check that this is the hwasan.sh we're expecting:
				if (WrapShFilePath.Contains("hwasan.sh", StringComparison.Ordinal))
				{
					if (WrapShContent.Replace("\n", "", StringComparison.Ordinal).Replace("\r", "", StringComparison.Ordinal) == @"#!/system/bin/shLD_HWASAN=1 exec ""$@""")
					{
						Logger.LogInformation("Substituting {WrapShFilePath} with debuggable version.", WrapShFilePath);
						WrapShContent =
@"#!/system/bin/sh
...
LD_HWASAN=1 exec $cmd";
					...
					{
						Logger.LogWarning("Warning: The generated APK may not be debuggable. Unrecognized {WrapShFilePath} was not replaced with a debuggable script.", WrapShFilePath);
					}
				}
				// handle any CRLF that may have crept in from windows text file handling.
				WrapShContent = WrapShContent.Replace("\r", "", StringComparison.Ordinal);
				File.WriteAllText(WrapDestFilePath, WrapShContent);
		...
		void CopyScudoMemoryTracerLib(string UnrealBuildPath, UnrealArch UnrealArch, string NDKArch, string PackageName)
		{
			...
			if (File.Exists(ScudoMemoryTraceLib) && File.Exists(WrapSh))
			{
				...
				foreach (KeyValuePair<string, string> KVP in Replacements)
				{
					WrapShText = WrapShText.Replace(KVP.Key, KVP.Value, StringComparison.Ordinal);
				}
		...
		private static int RunCommandLineProgramAndReturnResultOutput(string WorkingDirectory, string Command, string Params, ILogger Logger, out string Output, string? OverrideDesc = null, bool bUseShellExecute = false)
		{
			...
			if (Params.Contains('\'', StringComparison.Ordinal))
			{
				Params = Params.Replace("\"", "\\\"", StringComparison.Ordinal);
		...
		static void ParseFilterFile(string Filename)
		{
			...
			if (File.Exists(Filename))
			{
				...
				foreach (string FileLine in FilterContents)
				{
					...
					if (Parts.Count > 1)
					{
						if (Parts[0].Equals("S", StringComparison.Ordinal))
						{
							ActiveStdOutFilter.Add(new FilterOperation(FilterAction.Skip, Parts[1], "", ""));
						}
						else if (Parts[0].Equals("R", StringComparison.Ordinal))
						{
							if (Parts.Count == 4)
							{
						...
						else if (Parts[0].Equals("E", StringComparison.Ordinal))
						{
							if (Parts.Count == 4)
							{
		...
		static void FilterStdOutErr(DataReceivedEventArgs e, ILogger Logger)
		{
			if (e.Data != null)
			{
				if (ActiveStdOutFilter != null)
				{
					...
					{
						if (e.Data.Contains(FilterOp.Condition, StringComparison.Ordinal))
						{
							switch (FilterOp.Action)
							{
								...
								case FilterAction.Replace:
									Logger.LogInformation("{Output}", e.Data.Replace(FilterOp.Match, FilterOp.ReplaceWith, StringComparison.Ordinal));
									break;

								case FilterAction.Error:
									Logger.LogError("{Output}", e.Data.Replace(FilterOp.Match, FilterOp.ReplaceWith, StringComparison.Ordinal));
									break;
		...
		private static void RunCommandLineProgramWithExceptionAndFiltering(string WorkingDirectory, string Command, string Params, ILogger Logger, string? OverrideDesc = null, bool bUseShellExecute = false)
		{
			...
			Process Proc = new Process();
			Proc.StartInfo = StartInfo;
			Proc.OutputDataReceived += (_, e) => FilterStdOutErr(e, Logger);
			Proc.ErrorDataReceived += (_, e) => FilterStdOutErr(e, Logger);
		...
		private bool CheckApplicationName(string UnrealBuildPath, string ProjectName, out string? ApplicationDisplayName)
		{
			...
			int KeyIndex = Contents.IndexOf(AppNameTag, StringComparison.Ordinal);
			...
			int TagEnd = Contents.IndexOf("</string>", KeyIndex, StringComparison.Ordinal);
			if (TagEnd < 0)
			{
				return true;
			}
			...
			bool bRequiresOBB)
		{
			...
			CurrentSettings.AppendLine(String.Format("bNoObbs={0}", bNoObbs));
			CurrentSettings.AppendLine(String.Format("bRequiresOBB={0}", bRequiresOBB));
			...
			if (Section != null)
			{
				foreach (string Key in Section.KeyNames)
				{
					...
					if (Key.Equals("SDKAPILevelOverride", StringComparison.Ordinal) || Key.Equals("NDKAPILevelOverride", StringComparison.Ordinal))
					{
						continue;
					}
			...
			if (Section != null)
			{
				foreach (string Key in Section.KeyNames)
				{
					...
					if (Key.Equals("SDKAPILevel", StringComparison.Ordinal) || Key.Equals("NDKAPILevel", StringComparison.Ordinal))
					{
						continue;
					}
			...
			if (Section != null)
			{
				foreach (string Key in Section.KeyNames)
				{
					if (!Key.Equals("GameDefaultMap", StringComparison.Ordinal) &&
						!Key.Equals("GlobalDefaultGameMode", StringComparison.Ordinal))
					{
						continue;
					}
			...
			foreach (string Line in UPLBuildSettings.Split('\n', StringSplitOptions.RemoveEmptyEntries))
			{
				...
				int KeyIndex = SectionName.IndexOf('[', StringComparison.Ordinal);
				if (KeyIndex > 0)
				{
					string KeyList = SectionName.Substring(KeyIndex + 1);
					SectionName = SectionName.Substring(0, KeyIndex);
					int CloseIndex = KeyList.IndexOf(']', StringComparison.Ordinal);
					if (CloseIndex > 1)
					{
		...
		private bool CheckDependencies(UnrealArchitectures Architectures, string ProjectName, string ProjectDirectory, string IntermediateAndroidPath,
			string UnrealBuildFilesPath, string GameBuildFilesPath, List<string> SettingsFiles, string CookFlavor,
			string OutputPath, bool bMakeSeparateApks, bool bPackageDataInsideApk, bool bDontBundleLibrariesInAPK)
		{
			...
			foreach (UnrealArch Arch in Architectures.Architectures)
			{
				...
				string ApkFilename = Path.GetFileNameWithoutExtension(OutputPath).Replace("UnrealGame", ProjectName, StringComparison.Ordinal);
				...
				if (!bNoObbs && bPackageDataInsideApk)
				{
		...
		private static int ConvertDepthBufferIniValue(string IniValue)
		{
			switch (IniValue.ToUpperInvariant())
			{
				case "BITS16":
					return 16;
				case "BITS24":
		...
		private static string ConvertOrientationIniValue(string IniValue)
		{
			switch (IniValue.ToUpperInvariant())
			{
				case "PORTRAIT":
		...
		private void DetermineScreenOrientationRequirements(string Arch, out bool bNeedPortrait, out bool bNeedLandscape)
		{
			...
			switch (GetOrientation(Arch).ToUpperInvariant())
			{
				case "PORTRAIT":
					bNeedPortrait = true;
					break;
		...
		private void PickSplashScreenOrientation(string CookFlavor, string UnrealBuildPath, bool bNeedPortrait, bool bNeedLandscape)
		{
			...
			if (bPackageForMetaQuest)
			{
				...
				if (bShowLaunchImage)
				{
					...
					{
						Logger.LogWarning("Warning: Landscape splash screen source image {LandscapeFilename} not available, Oculus splash screen will not function properly!", LandscapeFilename);
					}
					else if (FilesAreDifferent(LandscapeFilename, OculusSplashTargetPath))
					{
						...
						Logger.LogInformation("Copying {LandscapeFilename} to {OculusSplashTargetPath} for Oculus splash", LandscapeFilename, OculusSplashTargetPath);
					}
				}
				else
				{
		...
		private string GetPackageName(string ProjectName)
		{
			if (CachedPackageName == null)
			{
				...
				if (PackageName.Contains("[PROJECT]", StringComparison.Ordinal))
				{
					...
					if (ProjectName.Contains('-', StringComparison.Ordinal))
					{
						...
						ProjectName = ProjectName.Replace("-", "_", StringComparison.Ordinal);
					}
					...
					PackageName = PackageName.Replace("[PROJECT]", ProjectName, StringComparison.Ordinal);
				}
		...
		private string GenerateManifest(AndroidToolChain ToolChain, string ProjectName, TargetType InTargetType, bool bIsForDistribution, bool bPackageDataInsideApk, string GameBuildFilesPath, bool bHasOBBFiles, bool bDisableVerifyOBBOnStartUp, UnrealArch UnrealArch, string CookFlavor, bool bUseExternalFilesDir, string Configuration, int SDKLevelInt, bool bIsEmbedded, bool bEnableBundle, bool bUsesZenStreaming)
		{
			...
			GetMinTargetSDKVersions(ToolChain, UPL!, Arch, bEnableBundle, out int MinSDKVersion, out int TargetSDKVersion, out _);
			...
			ConfigHierarchy Ini = GetConfigCacheIni(ConfigHierarchyType.Engine);
			// Discard result but call the method for the logging side-effects to preserve previous behaviour.
			_ = GetPackageName(ProjectName);
			string VersionDisplayName = GetVersionDisplayName(bIsEmbedded);
			...
			Ini.GetString("/Script/AndroidRuntimeSettings.AndroidRuntimeSettings", "RoundedEdgeSafeZone", out string RoundedEdgeSafeZone);
			Ini.GetBool("/Script/AndroidRuntimeSettings.AndroidRuntimeSettings", "bAllowResizing", out bool bAllowResizing);
			Ini.GetBool("/Script/AndroidRuntimeSettings.AndroidRuntimeSettings", "bSupportSizeChanges", out bool bSupportSizeChanges);
			...
			Ini.GetInt32("/Script/Engine.RendererSettings", "r.Mobile.PropagateAlpha", out int PropagateAlpha);
			...
			switch (InstallLocation.ToUpperInvariant())
			{
				case "PREFEREXTERNAL":
			...
			Ini.GetBool("/Script/AndroidRuntimeSettings.AndroidRuntimeSettings", "bEnableMulticastSupport", out bool bEnableMulticastSupport);

			Ini.GetBool("/Script/AndroidSingleInstanceServiceEditor.AndroidSingleInstanceServiceRuntimeSettings", "bEnableASISPlugin", out bool bASISEnabled);
			...
			if (CookFlavor.EndsWith("Server", StringComparison.Ordinal))
			{
				CookFlavor = CookFlavor.Substring(0, CookFlavor.Length - 6);
			}
			...
			if (ExtraApplicationNodeTags != null)
			{
				...
				{
					if (Line.Contains("requestLegacyExternalStorage", StringComparison.Ordinal))
					{
			...
			{
				// Opt out of Android 16+ restricted resizability so min/maxAspectRatio are honored on large-screen devices.
				// Note: this is ignored on API 37+
				...
				if (ExtraPermissions != null)
				{
					foreach (string Permission in ExtraPermissions)
					{
						...
						if (!String.IsNullOrEmpty(TrimmedPermission))
						{
							...
							if (!Text.ToString().Contains(PermissionString, StringComparison.Ordinal))
							{
		...
		private void ValidateGooglePlay(string UnrealBuildPath)
		{
			...
			if (IniAppId.Length == 0 || !Int64.TryParse(IniAppId, out _))
			{
			...
			if (File.Exists(Filename))
			{
				...
				foreach (string Line in FileContent)
				{
					...
					int StartIndex = Line.IndexOf("\"app_id\">", StringComparison.Ordinal);
					if (StartIndex < 0)
					{
						continue;
					}
					...
					StartIndex += 9;
					int EndIndex = Line.IndexOf("</string>", StringComparison.Ordinal);
					if (EndIndex < 0)
					{
						continue;
					}
					...
					if (!bInvalidIniAppId && !IniAppId.Equals(XmlAppId, StringComparison.Ordinal))
					{
					...
					else if (XmlAppId.Length == 0 || !Int64.TryParse(XmlAppId, out _))
					{
					...
					if (bInvalid)
					{
						...
						string NewLine = Line.Replace("\"app_id\">" + XmlAppId + "</string>", "\"app_id\">" + ReplacementId + "</string>", StringComparison.Ordinal);
		...
		private static bool FilesAreIdentical(string SourceFilename, string DestFilename)
		{
			// source must exist
			FileInfo SourceInfo = new FileInfo(SourceFilename);
			if (!SourceInfo.Exists)
		...
		private bool RequiresOBB(bool bDisallowPackageInAPK, string OBBLocation)
		{
			if (bNoObbs)
			{
				Logger.LogInformation("OBBs have been disabled.");
				return false;
			}
			else if (bDisallowPackageInAPK)
			{
		...
		private bool CreateRunGradle(string GradlePath)
		{
			...
			for (char DriveLetter = 'Z'; DriveLetter >= 'A'; DriveLetter--)
			{
				...
				{
					if (AllDrives[DriveIndex].Name.StartsWith(UnusedDriveLetter, StringComparison.OrdinalIgnoreCase))
					{
						bFound = true;
						break;
					}
		...
		private void GetMinTargetSDKVersions(AndroidToolChain ToolChain, UnrealPluginLanguage UPL, string NDKArch, bool bEnableBundle, out int MinSDKVersion, out int TargetSDKVersion, out int NDKLevelInt)
		{
			ConfigHierarchy Ini = GetConfigCacheIni(ConfigHierarchyType.Engine);
			...
			{
				if (Int32.TryParse(TargetOverride, out int OverrideInt))
				{
		...
		private static readonly uint[] CRCTablesSB8 = {
		...
		/// <summary>
		/// Replaces a SO in an existing apk, performing an alignment.
		/// This function can throw IO exceptions, ensure usage of proper try/catch handling
		/// </summary>
		/// <param name="SourceApk">The base apk to align</param>
		/// <param name="TargetApk">Path to output the newly aligned apk</param>
		/// <param name="Logger">Logger</param>
		/// <param name="SourceSOFile">SO file to use as a replacement</param>
		/// <param name="RelativeSOFile">Path of the SO file to replace, relative to the apk</param>
		public static void CopyAPKAndReplaceSO(FileReference SourceApk, FileReference TargetApk, FileReference SourceSOFile, string RelativeSOFile, ILogger Logger)
		{
			string DirectoryFilename = TargetApk.FullName + ".dir";

			using BinaryReader SourceReader = new BinaryReader(new FileStream(SourceApk.FullName, FileMode.Open, FileAccess.Read));
			using BinaryWriter DestWriter = new BinaryWriter(new FileStream(TargetApk.FullName, FileMode.Create));
			using BinaryWriter DirectoryWriter = new BinaryWriter(new FileStream(DirectoryFilename, FileMode.Create));
			{
				...
				while (true)
				{
					...
					SourceReader.BaseStream.ReadExactly(Filename, 0, FilenameLen);
					byte[] Extra = new byte[ExtraLen];
					SourceReader.BaseStream.ReadExactly(Extra, 0, ExtraLen);

					string FilenameStr = Encoding.UTF8.GetString(Filename, 0, FilenameLen);
					if (!FilenameStr.Equals(RelativeSOFile, StringComparison.OrdinalIgnoreCase))
					{
						TotalEntries++;
					...
					else
					{
						...
						try
						{
							FileStream SourceFile = File.OpenRead(SourceSOFile.FullName);
							uint SourceCompressedSize = CompressedSize;
							...
							using (MemoryStream CompressedStream = new MemoryStream())
							{
								...
								// advance the base stream forward by the source file size so we land on the next header
								SourceReader.BaseStream.Position += SourceCompressedSize;
							}
						...
						catch (IOException e)
						{
							...
							DestWriter.Close();
							FileReference.Delete(TargetApk);
							throw;
						}
					}
				}
		...
		private void CreateGradlePropertiesFiles(AndroidToolChain ToolChain, int MinSDKVersion, int TargetSDKVersion, string CompileSDKVersion, string BuildToolsVersion, string PackageName,
			...
			bool bForDistribution, bool bIsEmbedded, List<string> OBBFiles, bool bJavaPerfettoMarkers)
		{
			...
			GradleProperties.AppendLine("systemProp.org.gradle.internal.http.connectionTimeout=15000");
			GradleProperties.AppendLine("systemProp.org.gradle.internal.http.socketTimeout=30000");
			...
			string NDKPath = Environment.GetEnvironmentVariable("NDKROOT")!.Replace("\\", "/", StringComparison.Ordinal);
			...
			if (DestApkName != null)
			{
				GradleProperties.AppendLine(String.Format("OUTPUT_PATH={0}", Path.GetDirectoryName(DestApkName)!.Replace("\\", "/", StringComparison.Ordinal)));
				GradleProperties.AppendLine(String.Format("OUTPUT_FILENAME={0}", Path.GetFileName(DestApkName)));

				string BundleFilename = Path.GetFileName(DestApkName).Replace(".apk", ".aab", StringComparison.Ordinal);
				GradleProperties.AppendLine(String.Format("OUTPUT_BUNDLEFILENAME={0}", BundleFilename));

				if (bEnableUniversalAPK)
				{
					string UniversalAPKFilename = Path.GetFileName(DestApkName).Replace(".apk", "_universal.apk", StringComparison.Ordinal);
			...
			{
				GradleProperties.AppendLine(String.Format("OBB_FILE{0}={1}", OBBFileIndex++, OBBFile.Replace("\\", "/", StringComparison.Ordinal)));
			}

			GradleProperties.AppendLine("ANDROID_TOOLS_BUILD_GRADLE_VERSION={0}", ANDROID_TOOLS_BUILD_GRADLE_VERSION);
			GradleProperties.AppendLine("BUNDLETOOL_JAR=" + Path.GetFullPath(Path.Combine(UnrealBuildFilesPath, "..", "Prebuilt", "bundletool", BUNDLETOOL_JAR)).Replace("\\", "/", StringComparison.Ordinal));
			GradleProperties.AppendLine("GENUNIVERSALAPK_JAR=" + Path.GetFullPath(Path.Combine(UnrealBuildFilesPath, "..", "Prebuilt", "GenUniversalAPK", "bin", "GenUniversalAPK.jar")).Replace("\\", "/", StringComparison.Ordinal));
			...
			if ((Sanitizer != AndroidToolChain.ClangSanitizer.None && (Sanitizer != AndroidToolChain.ClangSanitizer.HwAddress || AndroidToolChain.HasEmbeddedHWASanSupport())) || bEnableScudoMemoryTracing)
			{
			...
			if (!String.IsNullOrEmpty(DebugKeyStore) && !String.IsNullOrEmpty(DebugKeyAlias) && !String.IsNullOrEmpty(DebugKeyStorePassword))
			{
				...
				else
				{
					...
					GradleProperties.AppendLine(String.Format("DEBUG_STORE_FILE={0}", DebugKeyStoreFilename.Replace("\\", "/", StringComparison.Ordinal)));
					...
					GradleBuildAdditionsContent.AppendLine(String.Format("\t\t\tstoreFile file('{0}')", DebugKeyStoreFilename.Replace("\\", "/", StringComparison.Ordinal)));
			...
			if (bForDistribution)
			{
				...
				GradleProperties.AppendLine(String.Format("STORE_FILE={0}", KeyStoreFilename.Replace("\\", "/", StringComparison.Ordinal)));
				...
				GradleBuildAdditionsContent.AppendLine(String.Format("\t\t\tstoreFile file('{0}')", KeyStoreFilename.Replace("\\", "/", StringComparison.Ordinal)));
			...
			if (GradlePropertiesUPL.Contains("DISABLE_MINIFY=1", StringComparison.Ordinal))
			{
			...
			if (GradlePropertiesUPL.Contains("DISABLE_PROGUARD=1", StringComparison.Ordinal))
			{
...
";
		...
		private static bool GetJavaPerfettoMarkers(ConfigHierarchy Ini, UnrealTargetConfiguration Configuration, bool bDistribution)
		{
			...
			Ini.GetBool("/Script/AndroidRuntimeSettings.AndroidRuntimeSettings", "bJavaPerfettoMarkers", out bool bJavaPerfettoMarkers);
		...
		private void MakeApk(AndroidToolChain ToolChain, string ProjectName, TargetType InTargetType, string ProjectDirectory, string OutputPath, string EngineDirectory, bool bForDistribution, string CookFlavor,
			UnrealTargetConfiguration Configuration, bool bMakeSeparateApks, bool bDisallowPackagingDataInApk, bool bDisallowExternalFilesDir, bool bSkipGradleBuild, bool bIsArchive, bool bIsFromUAT)
		{
			Logger.LogInformation("");
			...
			if (!bFastIterate &&
				!ForceAPKGeneration &&
				ProjectName == "UnrealGame" &&
				OutputPath.Replace("\\", "/", StringComparison.Ordinal).Contains("/Engine/Binaries/Android/", StringComparison.Ordinal) &&
				Path.GetFileNameWithoutExtension(OutputPath).StartsWith("UnrealGame", StringComparison.Ordinal))
			{
				if (!bSkipGradleBuild)
				{
					/*
			...
			string BuildToolsVersion = GetBuildToolsVersion(ToolChain);
			...
			// hwasan on NDK r26b+ requires wrap.sh that needs to be unpacked
			if (Sanitizer != AndroidToolChain.ClangSanitizer.None && (Sanitizer != AndroidToolChain.ClangSanitizer.HwAddress || AndroidToolChain.HasEmbeddedHWASanSupport()))
			{
			...
			if (!bCreateFromScratch)
			{
				if (File.Exists(BuildSettingsCacheFile))
				{
					...
					string? Line = File.ReadAllLines(BuildSettingsCacheFile).FirstOrDefault(Line => Line.StartsWith(Key, StringComparison.Ordinal));
					if (Line != null && !Line[Key.Length..].Equals(bEnableBundle ? "True" : "False", StringComparison.Ordinal))
					{
						bCreateFromScratch = true;
					}
				}
			...
			bool bDisableVerifyOBBOnStartUp = bNoObbs || DisableVerifyOBBOnStartUp();
			...
			List<string> RequiredOBBFiles = bNoObbs ? [] : [ObbFileLocation];
			if (!bNoObbs && File.Exists(PatchFileLocation))
			{
			...
			WriteJavaOBBDataFile(UnrealOBBDataFileName, PackageName, RequiredOBBFiles, CookFlavor, !bNoObbs && bPackageDataInsideApk, Architectures.Architectures[0]);
			...
			string CurrentBuildSettings = GetAllBuildSettings(ToolChain, UPL!, bForDistribution, bMakeSeparateApks, bPackageDataInsideApk, bDisableVerifyOBBOnStartUp, bUseExternalFilesDir, bEnableBundle, bDontBundleLibrariesInAPK, bUsesZenStreaming, TemplatesHashCode, bRequiresOBB);
			...
			if (bBuildSettingsMatch && !bSkipGradleBuild)
			{
				...
				bBuildSettingsMatch = CheckDependencies(Architectures, ProjectName, ProjectDirectory, IntermediateAndroidPath,
					UnrealBuildFilesPath, GameBuildFilesPath, JavaFiles, CookFlavor, OutputPath,
			...
			foreach ((UnrealArch _, string NDKArch) in Archs)
			{
				...
				string CompileSDKVersion = SDKAPILevel.Replace("android-", "", StringComparison.Ordinal);
				...
				{
					Logger.LogError("Failed to parse SDK version from text '{SDKVersion}'", CompileSDKVersion);
				}
			...
			if (!bBuildSettingsMatch || bOnlyGradleGeneration)
			{
				...
				BuildList = (from Arch in Architectures.Architectures
							...
							let manifest = GenerateManifest(ToolChain, ProjectName, InTargetType, bForDistribution, bPackageDataInsideApk, GameBuildFilesPath, bRequiresOBB, bDisableVerifyOBBOnStartUp, Arch, CookFlavor, bUseExternalFilesDir, Configuration.ToString(), CompileSDKLevelForArch, bSkipGradleBuild, bEnableBundle, bUsesZenStreaming)
...
#pragma warning disable CA1308 // Normalize strings to uppercase - file path should be created with the correct case
					string LLDBExtraSymbolsDirectory = Path.Combine(IntermediateAndroidPath, "LLDBSymbolsLibs", Arch.ToString().ToLowerInvariant());
...
#pragma warning restore CA1308
			...
			{
				Logger.LogInformation("Attempting BypassGradlePackaging");
				...
				foreach (Tuple<UnrealArch, string> build in BuildList)
				{
					...
					// As we are always making separate APKs we need to put the architecture into the name
					DestApkName = AndroidToolChain.InlineArchName(DestApkName, Arch);
					...
					try
					{
						FileReference DestApkFile = new FileReference(DestApkName);
						FileReference TempApkFile = new FileReference(TempAPK);
						FileReference StrippedSOFile = new FileReference(StrippedSOName);
						CopyAPKAndReplaceSO(DestApkFile, TempApkFile, StrippedSOFile, "lib/" + NDKArch + "/libUnreal.so", Logger);
					}
					catch { }
					...
					try
					{
						ToolChain.SignApk(new FileReference(TempAPK), new FileReference(DestApkName), KeyStore, KeyStorePassword, KeyAlias, KeyPassword);
					}
					finally
					{
			...
			foreach (Tuple<UnrealArch, string> build in BuildList)
			{
				...
				if (bNoObbs || !bPackageDataInsideApk)
				{
					// Try to remove main.obb.png file if we don't want OBBs
					SafeDeleteFile(Path.Combine(UnrealBuildPath, "assets", "main.obb.png"));
				}
				...
				bool bHasSplashActivity = Manifest.Contains("com.epicgames.unreal.SplashActivity", StringComparison.Ordinal);
				...
				if (OutputDirectory != null && Path.GetRelativePath(DestApkDirectory, OutputDirectory) != OutputDirectory)
				{
					...
					if (!Path.GetFileName(OutputPath).StartsWith("UnrealGame", StringComparison.Ordinal))
					{
				...
				else if (bDontBundleLibrariesInAPK)
				{
					...
					string SOPushScript = @$"
set ADB=adb
set AFS={AFSExecutable.Replace("/", "\\", StringComparison.Ordinal)}
...
";
				...
				UpdateGameActivity(NDKArch, EngineDirectory, UnrealBuildPath, bDontBundleLibrariesInAPK, bJavaPerfettoMarkers, Configuration, bHasSplashActivity);
				UpdateGameApplication(NDKArch, EngineDirectory, UnrealBuildPath, bJavaPerfettoMarkers, Configuration, SourceSOName);
				...
				if (!bSkipLibCpp)
				{
					...
					CopySTL(UnrealBuildPath, NDKArch);
				}
				CopySwappy(UnrealBuildPath, NDKArch, Configuration);
				CopyPSOService(UnrealBuildPath, UnrealPreBuiltFilesPath, NDKArch);
				CopyVulkanValidationLayers(UnrealBuildPath, NDKArch, Configuration.ToString());

				if (Sanitizer != AndroidToolChain.ClangSanitizer.None)
				{
					CopyClangSanitizerLib(ToolChain, UnrealBuildPath, NDKArch, Sanitizer);
				...
				int NDKLevelInt = 0;
				GetMinTargetSDKVersions(ToolChain, UPL, NDKArch, bEnableBundle, out MinSDKVersion, out TargetSDKVersion, out NDKLevelInt);

				// move JavaLibs into subprojects
				string JavaLibsDir = Path.Combine(UnrealBuildPath, "JavaLibs");
				PrepareJavaLibsForGradle(JavaLibsDir, UnrealBuildGradlePath, MinSDKVersion.ToString(), TargetSDKVersion.ToString(), NDKArch);
...
//				LocalProperties.AppendLine(string.Format("ndk.dir={0}", Environment.GetEnvironmentVariable("NDKROOT")!.Replace("\\", "/")));
				LocalProperties.AppendLine(String.Format("sdk.dir={0}", Environment.GetEnvironmentVariable("ANDROID_HOME")!.Replace("\\", "/", StringComparison.Ordinal)));
				File.WriteAllText(LocalPropertiesFilename, LocalProperties.ToString());

				CreateGradlePropertiesFiles(ToolChain, MinSDKVersion, TargetSDKVersion, CompileSDKVersion, BuildToolsVersion, PackageName, DestApkName, NDKArch,
				...
				CheckSOInGradleFolderAre16KbCompatible(UnrealBuildGradleMainPath);
				...
				if (!bSkipGradleBuild && !bOnlyGradleGeneration)
				{
					...
					if (!OperatingSystem.IsWindows())
					{
						...
						Logger.LogInformation("Fix gradlew permissions");
						File.SetUnixFileMode(GradleScriptPath, File.GetUnixFileMode(GradleScriptPath) | UnixFileMode.UserExecute | UnixFileMode.GroupExecute | UnixFileMode.OtherExecute);
					}
					else
					{
						...
						if (!bHasAndroidStudioProject)
						{
							...
							if (bIsBuildMachine && OperatingSystem.IsWindows())
							{
								...
								if (ExitCode == 0)
								{
									...
									{
										if (Taskline.StartsWith("java.exe", StringComparison.Ordinal))
										{
											string ProcessString = Taskline.Substring(8).TrimStart();
											ProcessString = ProcessString.Substring(0, ProcessString.IndexOf(' ', StringComparison.Ordinal));
											int ProcessId;
			...
			if (bEnableBundle && !bSkipGradleBuild && !bOnlyGradleGeneration)
			{
				...
				foreach (Tuple<UnrealArch, string> build in BuildList)
				{
					...
					foreach (string Filename in SourceFiles)
					{
						...
						string Workname = Filename.Replace("\\", "/", StringComparison.Ordinal);
						if (Workname.Contains("/.", StringComparison.Ordinal))
						{
							continue;
						}
						...
						if (Filename.EndsWith("abi.gradle", StringComparison.Ordinal))
						{
							continue;
						}

						// ignore OBBData.java, we won't use it
						if (Filename.EndsWith("OBBData.java", StringComparison.Ordinal))
						{
							continue;
						}

						// deal with AndroidManifest.xml
						if (Filename.EndsWith("AndroidManifest.xml", StringComparison.Ordinal))
						{
							...
							if (SourceManifest.Length == DestManifest.Length)
							{
								...
								for (int Index = 0; Index < SourceManifest.Length; Index++)
								{
									...
									int SourceVersionIndex = SourceManifest[Index].IndexOf("android:versionCode=", StringComparison.Ordinal);
									...
									int DestVersionIndex = DestManifest[Index].IndexOf("android:versionCode=", StringComparison.Ordinal);
									...
									int SourceVersionIndex2 = SourceManifest[Index].Substring(SourceVersionIndex + 22).IndexOf('\"', StringComparison.Ordinal);
									...
									int DestVersionIndex2 = DestManifest[Index].Substring(DestVersionIndex + 22).IndexOf('\"', StringComparison.Ordinal);
						...
						if (Filename.EndsWith("buildAdditions.gradle", StringComparison.Ordinal))
						{
							...
							if (SourceProperties.Length == DestProperties.Length)
							{
								...
								for (int Index = 0; Index < SourceProperties.Length; Index++)
								{
									...
									if (SourceProperties[Index].Contains("storeFile file(", StringComparison.Ordinal) && DestProperties[Index].Contains("storeFile file(", StringComparison.Ordinal))
									{
										continue;
									}
						...
						if (Filename.EndsWith("gradle.properties", StringComparison.Ordinal))
						{
							...
							if (SourceProperties.Length == DestProperties.Length)
							{
								...
								for (int Index = 0; Index < SourceProperties.Length; Index++)
								{
									...
									if (SourceProperties[Index].StartsWith("STORE_VERSION=", StringComparison.Ordinal) && DestProperties[Index].StartsWith("STORE_VERSION=", StringComparison.Ordinal))
									{
										continue;
									}

									if (SourceProperties[Index].StartsWith("STORE_FILE=", StringComparison.Ordinal) && DestProperties[Index].StartsWith("STORE_FILE=", StringComparison.Ordinal))
									{
										continue;
									}

									if (SourceProperties[Index].StartsWith("DEBUG_STORE_FILE=", StringComparison.Ordinal) && DestProperties[Index].StartsWith("DEBUG_STORE_FILE=", StringComparison.Ordinal))
									{
										continue;
									}

									if (SourceProperties[Index].StartsWith("OUTPUT_FILENAME=", StringComparison.Ordinal) && DestProperties[Index].StartsWith("OUTPUT_FILENAME=", StringComparison.Ordinal))
									{
										continue;
									}

									if (SourceProperties[Index].StartsWith("OUTPUT_BUNDLEFILENAME=", StringComparison.Ordinal) && DestProperties[Index].StartsWith("OUTPUT_BUNDLEFILENAME=", StringComparison.Ordinal))
									{
										continue;
									}

									if (SourceProperties[Index].StartsWith("OUTPUT_UNIVERSALFILENAME=", StringComparison.Ordinal) && DestProperties[Index].StartsWith("OUTPUT_UNIVERSALFILENAME=", StringComparison.Ordinal))
									{
										continue;
									}
				...
				if (bCombinedBundleOK)
				{
					...
					{
						int ManifestVersionIndex = ManifestContents[Index].IndexOf("android:versionCode=", StringComparison.Ordinal);
						if (ManifestVersionIndex < 0)
						{
							continue;
						}

						int ManifestVersionIndex2 = ManifestContents[Index].Substring(ManifestVersionIndex + 22).IndexOf('\"', StringComparison.Ordinal);
					...
					GradlePropertiesContent += String.Format("\nSTORE_VERSION={0}\nOUTPUT_BUNDLEFILENAME={1}\n", BaseStoreVersion,
						Path.GetFileNameWithoutExtension(OutputPath).Replace("UnrealGame", ProjectName, StringComparison.Ordinal) + ".aab");
					if (bEnableUniversalAPK)
					{
						GradlePropertiesContent += String.Format("OUTPUT_UNIVERSALFILENAME={0}\n",
							Path.GetFileNameWithoutExtension(OutputPath).Replace("UnrealGame", ProjectName, StringComparison.Ordinal) + "_universal.apk");
					}
		...
		public override bool PrepTargetForDeployment(TargetReceipt Receipt)
		{
			...
			DirectoryReference ProjectDirectory = DirectoryReference.FromFile(ProjectFile) ?? Unreal.EngineDirectory;
			string TargetName = (ProjectFile == null ? Receipt.TargetName : ProjectFile.GetFileNameWithoutAnyExtensions());

			AndroidToolChain ToolChain = (AndroidToolChain)((AndroidPlatform)UEBuildPlatform.GetBuildPlatform(Receipt.Platform)).CreateTempToolChainForProject(ProjectFile);
			...
			string BaseSoName = AndroidToolChain.RemoveArchName(OutputPaths[0].FullName);
			...
			MakeApk(ToolChain, TargetName, Receipt.TargetType, ProjectDirectory.FullName, BaseSoName, RelativeEnginePath, bForDistribution: false, CookFlavor: "", Configuration: Receipt.Configuration,
				bMakeSeparateApks: ShouldMakeSeparateApks(), bDisallowPackagingDataInApk: false, bDisallowExternalFilesDir: true, bSkipGradleBuild: bShouldCompileAsDll, bIsArchive: false, bIsFromUAT: false);

			// if we made any non-standard .apk files, the generated debugger settings may be wrong
			if (ShouldMakeSeparateApks() && (OutputPaths.Count > 1 || !OutputPaths[0].FullName.Contains("-armv7", StringComparison.Ordinal)))
			{
				Logger.LogInformation("================================================================================================================================");
		...
		public bool PrepForUATPackageOrDeploy(FileReference ProjectFile, string ProjectName, DirectoryReference ProjectDirectory, string ExecutablePath, string EngineDirectory, bool bForDistribution, string CookFlavor, UnrealTargetConfiguration Configuration, bool bIsDataDeploy, bool bSkipGradleBuild, bool bIsArchive)
		{
			...
			else if (CookFlavor.EndsWith("Server", StringComparison.Ordinal))
			{
			...
			MakeApk(ToolChain, ProjectName, Type, ProjectDirectory.FullName, ExecutablePath, EngineDirectory, bForDistribution: bForDistribution, CookFlavor: CookFlavor, Configuration: Configuration,
				bMakeSeparateApks: ShouldMakeSeparateApks(), bDisallowPackagingDataInApk: bIsDataDeploy, bDisallowExternalFilesDir: bIsDataDeploy,
		...
		private static string GenerateTemplatesHashCode(string EngineDir)
		{
...
#pragma warning restore CA5351
		...
		private void UpdateGameActivity(string NDKArch, string EngineDir, string UnrealBuildPath, bool bDontBundleLibrariesInAPK,
...
";
			...
			for (int LineIndex = 0; LineIndex < TemplateSrc.Length; ++LineIndex)
			{
				...
				if (!bJavaPerfettoMarkers)
				{
					if (SrcLine.StartsWith("Trace.", StringComparison.Ordinal))
					{
							...
							else
							{
								...
								if (TraceStartIndex == 0 || (TraceStartIndex > 0 && KVP.Value[TraceStartIndex - 1] == '\n'))
								{
									...
									do
									{
										...
										TraceStartIndex = Addition.IndexOf("Trace.", TraceStartIndex, StringComparison.Ordinal);
									} while (TraceStartIndex >= 0);
									SrcLine = SrcLine.Replace(KVP.Key, Addition, StringComparison.Ordinal);
								}
								else
								{
									SrcLine = SrcLine.Replace(KVP.Key, KVP.Value, StringComparison.Ordinal);
		...
		private void UpdateGameApplication(string NDKArch, string EngineDir, string UnrealBuildPath,
				bool bJavaPerfettoMarkers, UnrealTargetConfiguration Configuration, string SOName)
		{
			...
			List<string> UnrealDependencies = GetSODependencies(SOName);
			...
			Dictionary<string, string> Replacements = new Dictionary<string, string>{
				...
				{ "//$${gameApplicationOnCreateAdditionsAlways}$$", UPL.ProcessPluginNode(NDKArch, "gameApplicationOnCreateAdditionsAlways", "")},
		...
		private void CreateAdditonalBuildPathFiles(string NDKArch, string UnrealBuildPath, XDocument FilesToAdd)
		{
			...
			{
				string UPLNodeName = Entry.Key.Replace("/", "__", StringComparison.Ordinal).Replace(".", "__", StringComparison.Ordinal);
				string Content;
				if (Entry.Value == null)
				{
		...
		private AndroidAARHandler CreateAARHandler(string EngineDir, string UnrealBuildPath, IEnumerable<string> NDKArches, bool HandleDependencies = true)
		{
			...
			foreach (string Line in ImportList.Split('\n'))
			{
				...
				if (Trimmed.StartsWith("repository ", StringComparison.Ordinal))
				{
					string DirectoryPath = Trimmed.Substring(11).Trim(' ').TrimEnd('/', '\\');
					DirectoryPath = DirectoryPath.Replace("$(ENGINEDIR)", EngineDir, StringComparison.Ordinal);
					DirectoryPath = DirectoryPath.Replace("$(ANDROID_HOME)", AndroidHome, StringComparison.Ordinal);
				...
				else if (Trimmed.StartsWith("repositories ", StringComparison.Ordinal))
				{
		...
		private void PrepareJavaLibsForGradle(string JavaLibsDir, string UnrealBuildGradlePath, string InMinSdkVersion, string InTargetSdkVersion, string NDKArch)
		{
