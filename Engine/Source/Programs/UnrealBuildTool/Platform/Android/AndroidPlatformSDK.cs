...
namespace UnrealBuildTool
{
	partial class AndroidPlatformSDK : UEBuildPlatformSDK
	{
				...
				if (File.Exists(BashProfilePath))
				{
					...
					for (int LineIndex = BashProfileContents.Length - 1; LineIndex >= 0; --LineIndex)
					{
						foreach (KeyValuePair<string, string> kvp in EnvVarNames)
						{
							...
							if (BashProfileContents[LineIndex].StartsWith("export " + kvp.Key + "=", StringComparison.Ordinal))
							{
								string PathVar = BashProfileContents[LineIndex].Split('=')[1].Replace("\"", "", StringComparison.Ordinal);
		...
		public static string? GetNDKRoot(ILogger Logger)
		{
			...
			{
				return NDKPath.Replace("\"", "", StringComparison.Ordinal);
			}
		...
		static string? GetNdkNameFromDirectory(string Directory)
		{
			...
			if (File.Exists(SourcePropFilename))
			{
				...
				int EqualsIndex = RevisionString.IndexOf('=', StringComparison.Ordinal);
				if (EqualsIndex > 0)
				{
		...
		static string GetStudioDir()
		{
			...
			{
				string Directory = Path.Combine(Environment.GetEnvironmentVariable("HOME")!, "Applications/Android Studio.app/Contents");
				return System.IO.Directory.Exists(Directory) ? Directory : "/Applications/Android Studio.app/Contents";
			}
			else
			{
		...
		protected override bool TryGetEnvironmentForManualSDK(string SelectedSDKVersion, out Dictionary<string, string>? EnvVarValues, out List<string>? PathAdds, out List<string>? PathRemoves)
		{
			...
			if (String.IsNullOrEmpty(CurrentSdkDir) || !Directory.Exists(Path.Combine(CurrentSdkDir, "platforms", GetPlatformSpecificVersion("platforms"))) || GetInstalledSDKVersion() != SelectedSDKVersion)
			{
				try
				{
					...
					if (OperatingSystem.IsMacOS())
					{
						JavaDir = Path.Combine(JavaDir, "Contents", "Home");
					}
		...
		public override bool TryConvertVersionToInt(string? StringValue, out ulong OutValue, string? Hint)
		{
			...
			{
				if (String.Equals(Hint, "Software", StringComparison.OrdinalIgnoreCase))
				{
		...
		protected virtual bool HasAnySDK()
		{
			...
			JavaPath = JavaPath.Replace("\"", "", StringComparison.Ordinal);
			...
			{
				if (LineContents.StartsWith("JAVA_VERSION=", StringComparison.Ordinal))
				{
					// JAVA_VERSION="17.0.6"
					int VersionStartIndex = LineContents.IndexOf('\"', StringComparison.Ordinal);
					int VersionStopIndex = LineContents.IndexOf('.', StringComparison.Ordinal);
					_ = Int32.TryParse(LineContents.Substring(VersionStartIndex, VersionStopIndex - VersionStartIndex), out JavaVersion);
					break;
				}
			}
			if (JavaVersion < 17)
