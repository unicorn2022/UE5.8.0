...
namespace UnrealBuildTool
{
	...
	{
#pragma warning disable CA1823 // unused private fields - This will be removed in a future version
#pragma warning disable IDE0052 //  unused private member - This will be removed in a future version
		[Obsolete("Deprecated in UE5.8. Modern Xcode is now the only option")]
		private static readonly bool bForceModernXcode = Environment.CommandLine.Contains("-modernxcode", StringComparison.OrdinalIgnoreCase);
		[Obsolete("Deprecated in UE5.8. Legacy Xcode is no longer supported")]
		private static readonly bool bForceLegacyXcode = Environment.CommandLine.Contains("-legacy xcode", StringComparison.OrdinalIgnoreCase);
#pragma warning restore IDE0052
#pragma warning restore CA1823

		private static readonly bool bNoEntitlements = Environment.CommandLine.Contains("-noEntitlements", StringComparison.OrdinalIgnoreCase);
		...
		/// <returns></returns>
		[Obsolete("Deprecated in UE5.8. Use !CreatingAppOnWindows")]
		public static bool UseModernXcode(FileReference? ProjectFile)
		{
			return !CreatingAppOnWindows(ProjectFile);
		}

		/// <summary>
		/// Is the current project being built in Apple on Windows mode?
		/// </summary>
		/// <param name="ProjectFile"></param>
		/// <returns></returns>
		public static bool CreatingAppOnWindows(FileReference? ProjectFile)
		{
			if (OperatingSystem.IsWindows())
			{
				Log.TraceInformationOnce("Building in Apple on Windows mode because the host machine is running Windows...");
				return true;
			}
			
			Log.TraceVerboseOnce("Building in Xcode Mode...");
			return false;
		}
		...
		public static FileReference ConvertFilePath(DirectoryReference? ProductDirectory, string FilePath)
		{
			...
			{
				string? ReadPath = ConfigHierarchy.GetStructEntry(FilePath, "FilePath", false);
				if (ReadPath == null)
				{
					throw new Exception(" Failed to read file path from config");
				}
			...
			string TargetName, XcodeBuildMode BuildMode, ILogger Logger, string ExtraOptions = "", bool bForceDummySigning = false, List<string>? DestinationIds = null)
		{
			...
			else
			{
				...
				if (ExtraOptions.Contains("CODE_SIGN_IDENTITY", StringComparison.Ordinal))
				{
		...
		public static DirectoryReference GetProvisionDirectory()
		{
			...
			if (OperatingSystem.IsMacOS())
			{
				...
				{
					// Parse the major version, we don't need the minor version.
					int Major = 15;
					try
					{
