...
namespace UnrealBuildTool
{
	...
	{
		readonly DirectoryReference SourceDir;
		readonly DirectoryReference OutputDir;
		readonly IEnumerable<string> RspLines;

		readonly Dictionary<string, List<FileItem>> SingleFiles = new();
		...
		public IExternalAction? CreateAction(FileItem SourceFile, ILogger Logger)
		{
			...
			int FileNameIndex = CommandArguments.IndexOf(DummyName, System.StringComparison.Ordinal);
			...
			foreach (string L in RspLines)
			{
				...
				{
					Line = Line.Replace("SingleFile.cpp", UniqueDummyName, System.StringComparison.Ordinal);
				}
				else
				{
					Line = Line.Replace(DummyPath, SourceFile.FullName.Replace('\\', '/'), System.StringComparison.Ordinal);
				}
				NewRspLines.Add(Line);
			}

			Action Action = new Action(this);
			Action.CommandArguments = CommandArguments.Replace(DummyName, UniqueDummyName, System.StringComparison.Ordinal);
			...
			if (RspLines.Any(x => x.Contains(".cpp.i", System.StringComparison.Ordinal)))
			{
			...
			FileReference ResponseFile = RootPaths.GetLocalPath(new(Action.CommandArguments.Substring(1).Trim('"')));
