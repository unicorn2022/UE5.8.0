...
namespace UnrealBuildTool
{
	partial struct UnrealArch
	{
		...
		private static readonly IReadOnlyDictionary<UnrealArch, WindowsArchInfo> WindowsToolchainArchitectures = new Dictionary<UnrealArch, WindowsArchInfo>()
		{
	...
	public enum WindowsCompiler
	{
		...
		/// <summary>
		/// Visual Studio 2026 (Visual C++ 19.0) or older
		/// </summary>
		VisualStudio2026,

		/// <summary>
		/// Any version of Visual Studio available
		/// </summary>
		VisualStudio = VisualStudio2026, // Should be set to the latest version of VisualStudio
	...
	/// <summary>
	/// Available runtime ray tracing modes on Windows platform
	/// </summary>
	public enum WindowsRayTracingRuntimeMode
	{
		/// <summary>
		...
		/// <summary>
		/// Only inline raytracing is enabled
		/// </summary>
		Inline,

		/// <summary>
		/// Full raytracing with RTPSOs and hit shaders are available
		/// </summary>
		Full,
	}
	...
	public class WindowsTargetRules
	{
		...
		/// <summary>
		/// If enabled, resource files will be shared between Modular target dlls for non-formal builds rather than linking in different resources to each dll.
		/// Additionally the build will instead call cvtres.exe to convert a compiled resource .res file into an object up front rather than offloading to link.exe,
		/// This is an optimization for Modular builds to prevent reprocessing the same resource for every dll which can be slow and spawn two additional processes per link.exe
		/// </summary>
		public bool bSharedResourceFiles
		{
			get => bSharedResourceFilesPrivate ?? Target.LinkType != TargetLinkType.Monolithic && !Target.bFormalBuild;
			set => bSharedResourceFilesPrivate = value;
		}
		private bool? bSharedResourceFilesPrivate = null;
...
#pragma warning restore IDE0044
		...
		[Obsolete("Deprecated in UE5.8. Set MainVersion in Windows_SDK.json")]
		...
		/// </summary>
		[ConfigFile(ConfigHierarchyType.Engine, "/Script/WindowsTargetPlatform.WindowsTargetSettings", "RayTracingMode")]
		public WindowsRayTracingRuntimeMode RayTracingMode = WindowsRayTracingRuntimeMode.Disabled;
		...
		/// <summary>
		/// Enables AutoRTFM sanitizer (AutoRTFMSan)
		/// </summary>
		[XmlConfigFile(Category = "BuildConfiguration", Name = "bEnableAutoRTFMSanitizer")]
		[CommandLine("-EnableAutoRTFMSan")]
		[CommandLine("-EnableRTFMSan")]
		public bool bEnableAutoRTFMSanitizer = false;
		...
		/// <summary>
		/// Specifies the path to a manifest file for the linker to embed. Defaults to the manifest in Engine/Build/Windows/Resources.
		/// </summary>
		...
		/// <summary>
		/// Whether to define or not the resource icon to be used in Default.rc2.
		/// </summary>
		[CommandLine("-NoDefineIconResource", Value ="false")]
		public bool bDefineIconResource = true;
		...
		/// </summary>
		[RequiresUniqueBuildEnvironment]
		[Obsolete("Deprecated in UE5.8. Unreal Engine cannot be compiled without strict preprocessor conformance enabled.")]
		...
		[CommandLine("-NoStrictEnumTypes", Value = "false")]
		...
		/// <summary>
		/// Whether the embedded manifest will indicate the application is DPI aware
		/// </summary>
		[ConfigFile(ConfigHierarchyType.Engine, "/Script/WindowsTargetPlatform.WindowsTargetSettings")]
		public bool bForceHighDPIInGameMode = false;
		...
		/// <summary>
		/// Tells the compiler to check only the syntax of the source files on the command line. (/Zs)
		/// </summary>
		[XmlConfigFile(Category = "WindowsPlatform")]
		[CommandLine("-SyntaxCheckOnly")]
		public bool bSyntaxCheckOnly = false;
...
#pragma warning restore CA1822
		...
		/// </summary>
		public string? IDEDir => MicrosoftPlatformSDK.FindVisualStudioInstallations(Environment!.ToolChain, Target.Logger)
			.Select(x => x.BaseDir.FullName)
			.FirstOrDefault();
...
#pragma warning disable CS1591
		...
		public bool bSharedResourceFiles => Inner.bSharedResourceFiles;
		...
		public WindowsRayTracingRuntimeMode RayTracingMode => Inner.RayTracingMode;
		...
		public bool bEnableAutoRTFMSanitizer => Inner.bEnableAutoRTFMSanitizer;
		...
		public bool bDefineIconResource => Inner.bDefineIconResource;
		...
		public bool bSyntaxCheckOnly => Inner.bSyntaxCheckOnly;
		...
		public string? CompilerDir => Inner.Environment?.CompilerDir.FullName ?? null;
...
#pragma warning restore CS1591
	...
	/// <summary>
	/// Information about a particular Visual Studio package
	/// </summary>
	[DebuggerDisplay("{Id},{Version}")]
	record VisualStudioPackage(string id, Version version, bool isExtension)
	{
		/// <summary>
		...
		/// <summary>
		/// Whether the package refers to an external extension
		/// </summary>
	...
	record VisualStudioInstallation(WindowsCompiler Compiler, VersionNumber Version, DirectoryReference BaseDir, bool bCommunity, WindowsCompilerChannel ReleaseChannel, IDictionary<string, VisualStudioPackage> Packages)
	{
		/// <summary>
		/// Compiler type
		/// </summary>
		public WindowsCompiler Compiler { get; } = Compiler;
		...
		/// </summary>
		public DirectoryReference BaseDir { get; } = BaseDir;

		/// <summary>
		/// Whether it's a community edition of Visual Studio.
		/// </summary>
		public bool bCommunity { get; } = bCommunity;

		/// <summary>
		/// The release channel of this installation
		/// </summary>
		public WindowsCompilerChannel ReleaseChannel { get; } = ReleaseChannel;

		/// <summary>
		/// The packages added to this installation
		/// </summary>
		public IDictionary<string, VisualStudioPackage> Packages { get; } = Packages;
	}
	...
	class WindowsPlatform : UEBuildPlatform
	{
		...
		public override void ValidateTarget(TargetRules Target)
		{
			...
			if (Target.WindowsPlatform.bEnableInstrumentation && !Target.bUseAutoRTFMCompiler)
			{
			...
			{
				string Suffix = Target.Platform.ToString();
				switch (Target.Type)
				{
					case TargetType.Game:
					case TargetType.Client:
						if (Target.WindowsPlatform.bForceHighDPIInGameMode)
						{
							Suffix += "-dpiaware";
						}
						break;
				...
				Target.WindowsPlatform.ManifestFile = FileReference.Combine(Unreal.EngineDirectory, "Build", "Windows", "Resources", String.Format("Default-{0}.manifest", Suffix)).FullName;
			}
		}
		...
		void ValidateToolchainVersion(TargetRules Target)
		{
			...
			if (Target.WindowsPlatform.ToolchainVersionWarningLevel != WarningLevel.Off)
			{
				if (!MicrosoftPlatformSDK.IsPreferredVersion(Target.WindowsPlatform.Compiler, Target.WindowsPlatform.Environment.CompilerVersion))
				{
					...
					else if (Target.WindowsPlatform.Environment.CompilerVersion < preferred)
					{
					...
					{
						Logger.LogInformation("{Compiler} compiler version {Version} is newer than latest preferred version {PreferredVersion}. Please use caution as this compiler has not been heavily tested", WindowsPlatform.GetCompilerName(Target.WindowsPlatform.Compiler), Target.WindowsPlatform.Environment.CompilerVersion, preferred);
					}
				}
				...
				if (Target.WindowsPlatform.Compiler != Target.WindowsPlatform.ToolChain && !MicrosoftPlatformSDK.IsPreferredVersion(Target.WindowsPlatform.ToolChain, Target.WindowsPlatform.Environment.ToolChainVersion))
				{
					...
					else if (Target.WindowsPlatform.Environment.ToolChainVersion < preferred)
					{
					...
					{
						Logger.LogInformation("{Toolchain} toolchain version {Version} is newer than latest preferred version {PreferredVersion}. Please use caution as this toolchain has not been heavily tested", WindowsPlatform.GetCompilerName(Target.WindowsPlatform.ToolChain), Target.WindowsPlatform.Environment.ToolChainVersion, preferred);
					}
				}
			}
		}
		...
		internal static WindowsCompiler GetDefaultToolchain(TargetRules? TargetRules, UnrealArch Architecture, ILogger Logger, bool bSkipWarning = false)
		{
			...
			{
				if (PreferredAccessor == ProjectFileFormat.VisualStudio)
				{
					return WindowsCompiler.VisualStudio;
				}
			...
			// Check if any valid compilers are installed so a warning can be shown if none are available
			bool compilerAvailable = MicrosoftPlatformSDK.HasValidCompiler(WindowsCompiler.VisualStudio, Architecture, NullLogger.Instance);
			if (!compilerAvailable && !bSkipWarning && !MicrosoftPlatformSDK.IgnoreToolchainErrors)
			{
				...
				string ArchStr = Architecture == UnrealArch.X64 ? "x64/x86" : "ARM64/ARM64EC";
				string PreferredComponentStr = PreferredMsvcVersion != null ?
					$"MSVC Build Tools v{PreferredMsvcVersion.Components[0]}.{PreferredMsvcVersion.Components[1]} for {ArchStr}" :
					$"MSVC Build Tools for {ArchStr} (Latest)";

				// If we do have a Visual Studio installation, but we're missing just the C++ parts, warn about that.
				if (MicrosoftPlatformSDK.FindVisualStudioInstallations(NullLogger.Instance).Any())
				{
					Logger.LogWarning("Visual Studio is installed, but is out of date or missing a valid C++ toolchain (minimum version {MinVersion}, preferred version {PreferredVersion}). Please update Visual Studio to {MinimumVsVersionStr} or later and verify that the \"{Component}\" component is selected in the Visual Studio installation options.",
				...
				{
					Logger.LogWarning("No valid Visual C++ toolchain was found (minimum version {MinVersion}, preferred version {PreferredVersion}). Please download and install Visual Studio {MinimumVsVersionStr} or later and verify that the \"{Component}\" component is selected in the Visual Studio installation options.",
						MinimumMsvcVersionStr,
						PreferredMsvcVersionStr,
						MinimumVsVersionStr,
						PreferredComponentStr);
			...
			// Default to any available visual studio compiler
			return WindowsCompiler.VisualStudio;
		}
		...
		public override IReadOnlyDictionary<string, string>? GetSNDBSToolTemplates()
		{
			return SNDBSToolTemplates;
		}
		...
		public override void SetUpEnvironment(ReadOnlyTargetRules Target, CppCompileEnvironment CompileEnvironment, LinkEnvironment LinkEnvironment)
		{
			...
			CompileEnvironment.Definitions.Add("UE_PLATFORM_WINDOWS=1");
			CompileEnvironment.Definitions.Add("UE_PLATFORM_MICROSOFT=1");
			...
			if (Target.IsInPlatformGroup(UnrealPlatformGroup.Windows) && Target.WindowsPlatform.RayTracingMode != WindowsRayTracingRuntimeMode.Disabled && Target.Type != TargetType.Server)
			{
		...
		/// <summary>
		/// SN-DBS tool templates for Windows
		/// </summary>
		private static readonly IReadOnlyDictionary<string, string> SNDBSToolTemplates = new Dictionary<string, string>()
		{
			["cl-filter.exe"] = @"
[tool]
family=msvc
