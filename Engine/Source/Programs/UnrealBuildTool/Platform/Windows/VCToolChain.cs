...
namespace UnrealBuildTool
{
	class VCToolChain : ISPCToolChain
	{
			...
			: base(Logger)
		{
			...
			Logger.LogDebug("Resource Converter: {Path}", EnvVars.ResourceToObjectConverterPath);
		...
		protected void DynamicDebuggingInfo(List<string> Lines)
		{
			if (Target.WindowsPlatform.bDynamicDebugging)
			{
				...
				if (Target.WindowsPlatform.bEnableAutoRTFMSanitizer)
				{
					Lines.Add($" * Target.{nameof(Target.WindowsPlatform)}.{nameof(Target.WindowsPlatform.bEnableAutoRTFMSanitizer)} is enabled");
				}
		...
		public override void GetVersionInfo(List<string> Lines)
		{
			...
			if (Target.WindowsPlatform.ToolchainVersionWarningLevel == WarningLevel.Warning)
			{
				...
				{
					Lines.Add($"Warning: {WindowsPlatform.GetCompilerName(EnvVars.Compiler)} compiler {EnvVars.CompilerVersion} is not a preferred version");
				}

				if (EnvVars.Compiler != EnvVars.ToolChain && !MicrosoftPlatformSDK.IsPreferredVersion(EnvVars.ToolChain, EnvVars.ToolChainVersion))
				{
					Lines.Add($"Notice: {WindowsPlatform.GetCompilerName(EnvVars.ToolChain)} runtime {EnvVars.ToolChainVersion} is not a preferred version");
				}
			}
		}
		...
		public static void AddDefinition(List<string> Arguments, string Definition)
		{
			...
			int ValueIdx = Definition.IndexOf('=', StringComparison.Ordinal);
			if (ValueIdx == -1)
			{
		...
		public static void AddDefinition(List<string> Arguments, string Variable, string? Value)
		{
			...
			if (Value != null && !Value.StartsWith("\"", StringComparison.Ordinal) && (Value.Contains(' ', StringComparison.Ordinal) || Value.Contains('$', StringComparison.Ordinal)))
			{
				Value = "\"" + Value + "\"";
			}
		...
		public static void AddCreatePchFile(List<string> Arguments, FileItem PchThroughHeaderFile, FileItem CreatePchFile, CppRootPaths RootPaths)
		{
			...
			if (CreatePchFile.Name.EndsWith(".ifc", StringComparison.Ordinal))
			{
				...
				Arguments.Add($"/dxifcSuppressDllImportTransform");
				Arguments.Add($"-d1experimental:ifcDebugRecords");
			}
			else
			{
		...
		public static void AddUsingPchFile(List<string> Arguments, FileItem PchThroughHeaderFile, FileItem UsingPchFile, CppRootPaths RootPaths)
		{
			...
			if (UsingPchFile.Name.EndsWith(".ifc", StringComparison.Ordinal))
			{
		...
		private static void AddExceptionArguments(CppCompileEnvironment CompileEnvironment, List<string> Arguments)
		{
...
#pragma warning disable CA1308 // Normalize strings to uppercase - Clang arg may be case-sensitive
...
#pragma warning restore CA1308
					...
					HashSet<string> ValidAnalyzerCheckers = ClangWarnings.ValidAnalyzerCheckers(EnvVars.CompilerPath);
					...
					if (CompileEnvironment.StaticAnalyzerCheckers.Count > 0)
					{
						...
						foreach (string Checker in CompileEnvironment.StaticAnalyzerCheckers.Where(ValidAnalyzerCheckers.Contains))
						{
					...
					else
					{
						...
						foreach (string Checker in CompileEnvironment.StaticAnalyzerDisabledCheckers.Where(ValidAnalyzerCheckers.Contains))
						{
							Arguments.Add($"-Xclang -analyzer-disable-checker -Xclang {Checker}");
						}
						// Enable additional non-default checks.
						foreach (string Checker in CompileEnvironment.StaticAnalyzerAdditionalCheckers.Where(ValidAnalyzerCheckers.Contains))
						{
							Arguments.Add($"-Xclang -analyzer-checker -Xclang {Checker}");
						}
					}
			...
			if (!Target.WindowsPlatform.Compiler.IsClang() || !CompileEnvironment.bPreprocessOnly)
			{
				Arguments.Add("/c");
			}
			...
			// Enable syntax check only mode
			if (Target.WindowsPlatform.bSyntaxCheckOnly)
			{
				Arguments.Add("/Zs");
			}
			...
			if (Target.WindowsPlatform.bEnableInstrumentation)
			{
				...
				// When AutoRTFM is also active, verse-clang-cl handles instrumentation
				// via -fepictsan instead of using instr-clang-cl.exe.
				if (Target.WindowsPlatform.Compiler == WindowsCompiler.ClangRTFM)
				{
					Arguments.Add("-fepictsan");
				}
			}
			...
			if (Target.WindowsPlatform.bEnableAutoRTFMSanitizer)
			{
				Arguments.Add("-fsanitize=autortfm");
			}
			...
			{
				// Clang llvm-cov coverage flags
				if(Target.WindowsPlatform.Compiler.IsClang())
				{
					Arguments.Add("/Od");
					Arguments.Add("/clang:-fprofile-instr-generate");
					Arguments.Add("/clang:-fcoverage-mapping");
					Log.TraceInformationOnce("Enabling Clang code coverage flags");
				}
			}
			...
			if (CompileEnvironment.Architecture == UnrealArch.X64 && CompileEnvironment.MinCpuArchX64 != MinimumCpuArchitectureX64.None)
			{
				...
				else if (Target.WindowsPlatform.Compiler.IsClang())
				{
					...
					{
						Arguments.Add("-mavx2");
						Arguments.Add("-mfma"); // Matching MSVC which includes FMA with /arch:AVX2
					}
			...
			if (CompileEnvironment.Architecture == UnrealArch.X64 && CompileEnvironment.bMinCpuArchAPX)
			{
				if (Target.WindowsPlatform.Compiler.IsIntel())
				{
					Arguments.Add("-mapxf");
				}
				else if (Target.WindowsPlatform.Compiler.IsClang() && EnvVars.CompilerVersion >= new VersionNumber(21))
				{
					Arguments.Add("-mapxf");
				}
				else if (Target.WindowsPlatform.Compiler.IsMSVC() && EnvVars.CompilerVersion >= new VersionNumber(14, 51))
				{
					Arguments.Add("/feature:APX");
				}
			}
			...
			if (Target.WindowsPlatform.Compiler.IsMSVC())
			{
				...
				// This flag changes so msvc delay instantiation of virtual methods until they are needed.
				// This brings msvc behavior closer to clang/gcc and also reduces pch/binary size a bit while increasing obj files a tiny bit (net win)
				Arguments.Add("/dximplicitlyinstantiatevirtualfunction-");
			}
			...
			// If we are the editor we want to be precise to ensure more consistent data generation across builds.
			// Programs may get used as child processes for data generation.
			if (FPSemantics == FPSemanticsMode.Default && Target.DefaultBuildSettings >= BuildSettingsVersion.V7)
			{
			...
			switch (FPSemantics)
			{
				...
				case FPSemanticsMode.Precise:
					if (Target.WindowsPlatform.Compiler.IsClang())
					{
						// Clang defaults to -ffp-contract=on even with /fp:precise, which allows fusing multiplications and additions into FMAs.
						// Passing this alongside /fp:precise emits a warning, but /fp:precise is already the default so it's safe to omit.
						Arguments.Add("/clang:-ffp-contract=off");
					}
					else
					{
		...
		internal virtual void AppendCLArguments_CPP(CppCompileEnvironment CompileEnvironment, List<string> Arguments)
		{
			...
			if (Target.WindowsPlatform.Compiler.IsClang())
			{
				...
				if (CompileEnvironment.bAutoRTFMRedirectionInstrumentation)
				{
					Arguments.Add("-Xclang -mllvm -Xclang -autortfm-enable-redirection-instrumentation");
				}
			}
		}
		...
		protected virtual void AppendLinkArguments(LinkEnvironment LinkEnvironment, List<string> Arguments)
		{
			...
			else
			{
				...
				{
#pragma warning disable CA1308 // Normalize strings to uppercase - external argument should use the correct case
					string AssemblyName = Target.Architecture.ToString().ToLowerInvariant();
#pragma warning restore CA1308
			...
			{
				// Link clang_rt.profile-x86_64.lib
				if (Target.WindowsPlatform.Compiler.IsClang())
				{
					DirectoryReference ProfileRuntimeDir;
					string ProfileArchSuffix;
					if (EnvVars.Architecture == UnrealArch.X64)
					{
						VersionNumber ClangVersion = Target.WindowsPlatform.Compiler == WindowsCompiler.Intel ? MicrosoftPlatformSDK.GetClangVersionForIntelCompiler(EnvVars.CompilerPath) : EnvVars.CompilerVersion;
						ProfileRuntimeDir = DirectoryReference.Combine(EnvVars.CompilerDir, "lib", "clang", $"{ClangVersion.Components[0]}", "lib", "windows");
						ProfileArchSuffix = "x86_64";
					}
					else
					{
						throw new BuildException("Unsupported build architecture for Code Coverage");
					}
					string ProfileRuntimeLib = $"clang_rt.profile-{ProfileArchSuffix}.lib";
					LinkEnvironment.Libraries.Add(FileReference.Combine(ProfileRuntimeDir, ProfileRuntimeLib));
				}
			}
		}
		...
		protected virtual void AppendLibArguments(LinkEnvironment LinkEnvironment, List<string> Arguments)
		{
			...
			// Always build import libraries in deterministic mode, as no codegen is run this does not add any additional time
			if (!EnvVars.Compiler.IsClang() || !EnvVars.bAllowClangLinker)
			{
				Arguments.Add("/experimental:deterministic");
			}
		}
		...
		private VCCompileAction CreateBaseCompileAction(CppCompileEnvironment CompileEnvironment)
		{
			...
			{
				// /nologo in the shared file doesn't work to suppress logging.
				BaseCompileAction.Arguments.Add("/nologo");
			}
		...
		protected override CPPOutput CompileCPPFiles(CppCompileEnvironment CompileEnvironment, IEnumerable<FileItem> InputFiles, DirectoryReference OutputDir, string ModuleName, IActionGraphBuilder Graph)
		{
			...
			foreach (FileItem SourceFile in InputFiles)
			{
				...
				if (CompileEnvironment.PrecompiledHeaderAction == PrecompiledHeaderAction.Create)
				{
					...
					List<string> PchCppFileContents = [
						"// Generated by UnrealBuildTool (VCToolChain.cs)",
						$"// Compiler: {(EnvVars.Compiler.IsMSVC() ? $"MSVC {EnvVars.CompilerVersion}" : $"{EnvVars.Compiler} {EnvVars.CompilerVersion}, MSVC {EnvVars.ToolChainVersion}")}",
					...
					Graph.CreateIntermediateTextFile(CompileAction.SourceFile, PchCppFileContents);
					...
					if (CompileEnvironment.bUseHeaderUnitsForPch)
					{
						string ifcPath = NormalizeCommandLinePath(CompileEnvironment.PrecompiledHeaderIncludeFilename!, CompileEnvironment.RootPaths);
						string tomlString =
							$"[[header-unit]]\r\n" +
							$"name = [\"quote\", '{ifcPath.Replace('/', '\\')}']\r\n" +
							$"ifc = \"{CompileEnvironment.PrecompiledHeaderIncludeFilename!.FullName.Replace('\\', '/') + ".ifc"}\"\r\n";
						FileReference tomlFileRef = FileReference.Combine(OutputDir, SourceFile.Location.GetFileName() + ".toml");
						FileItem tomlFile = Graph.CreateIntermediateTextFile(tomlFileRef, tomlString);
						CompileAction.AdditionalPrerequisiteItems.Add(tomlFile);
						Result.ObjectFiles.Add(tomlFile);
					}
				}
				...
				if (CompileEnvironment.bPreprocessOnly)
				{
					...
					CompileAction.ResponseFile = FileItem.GetItemByFileReference(GetResponseFileName(CompileAction.PreprocessedFile));
				}
				else if (Target.WindowsPlatform.Compiler.IsClang() && Target.StaticAnalyzer == StaticAnalyzer.Default && CompileEnvironment.PrecompiledHeaderAction != PrecompiledHeaderAction.Create)
				{
					...
					CompileAction.ResponseFile = FileItem.GetItemByFileReference(GetResponseFileName(DependencyListFile));
				}
				else
				{
					...
					CompileAction.ResponseFile = FileItem.GetItemByFileReference(GetResponseFileName(ObjectFile));
				...
				if (SourceFile.HasExtension(".ixx"))
				{
					...
					DirectoryReference.CreateDirectory(IfcFile.Directory.Location);
					...
					CompileDepsAction.ResponseFile = FileItem.GetItemByFileReference(GetResponseFileName(IfcDepsFile));
		...
		public override void FinalizeOutput(ReadOnlyTargetRules Target, TargetMakefileBuilder MakefileBuilder, DirectoryReference ExeDir)
		{
			if (Target.bPGOOptimize)
			{
				...
				MakefileBuilder.AddInternalDependency(FileItem.GetItemByFileReference(FileReference.Combine(PGODirectory, "PGOProfileCompilerInfo.txt")));
			}
			...
			if (Target.bPrintToolChainTimingInfo || Target.WindowsPlatform.bCompilerTrace)
			{
				...
				if (TimingJsonFiles.Count > 1)
				{
					...
					if (Target.WindowsPlatform.Compiler.IsMSVC())
					{
						...
						FileReference ExpectedCompileTimeFile = new(Path.Combine(Makefile.ProjectIntermediateDirectory.FullName, $"{Target.Name}.json"));
			...
			OptionalUpdateSxSManifest(Target, MakefileBuilder, ExeDir);
		}

		private static readonly ConcurrentDictionary<FileReference, Lock> ManifestLocks = [];

		protected static void OptionalUpdateSxSManifest(ReadOnlyTargetRules Target, TargetMakefileBuilder MakefileBuilder, DirectoryReference ExeDir)
		{
...
#pragma warning restore CA1308
				...
				bool bDirty = false;
				FileReference IntermediateManifest = GetSxSIntermediateManifest(Target, ExeDir);
				...
				Lock Lock = ManifestLocks.GetOrAdd(IntermediateManifest, _ => new());
				lock (Lock)
				{
		...
		public override void PrepareRuntimeDependencies(List<RuntimeDependency> RuntimeDependencies, Dictionary<FileReference, FileReference> TargetFileToSourceFile, DirectoryReference ExeDir)
		{
			...
			{
				// Both ARM64 and ARM64EC use the same files. This requires the DLLs are all built as ARM64X.
				string[] PGOFiles = {
					"pgort140.dll",
					"pgosweep.exe",
				};

				// Non-x64 files are placed in sub-directories. A manifest is used to load DLLs from this directory,
				// and the EXEs just need to not stomp each other when building multiple architectures at once.
				DirectoryReference DstDir = ExeDir;
				if (!Target.Architecture.bIsX64)
				{
					DstDir = DirectoryReference.Combine(ExeDir, Target.Architecture.ToString());
				}
				...
				foreach (string PGOFile in PGOFiles)
				{
					...
					FileReference DstFile = FileReference.Combine(DstDir, PGOFile);
...
#pragma warning disable CA1308 // Normalize strings to uppercase - file path should use the correct case
				string AssemblyName = Target.Architecture.ToString().ToLowerInvariant();
#pragma warning restore CA1308
				FileReference SrcFile = GetSxSIntermediateManifest(Target, ExeDir);
		...
		public override CPPOutput CompileRCFiles(CppCompileEnvironment CompileEnvironment, IEnumerable<FileItem> InputFiles, DirectoryReference OutputDir, IActionGraphBuilder Graph)
		{
			...
			foreach (FileItem RCFile in InputFiles)
			{
				...
				CompileAction.CommandDescription = $"Compile Resource [{CompileEnvironment.Architecture.WindowsName}]";
				...
				CompileAction.PrerequisiteItems.UnionWith([
					.. CompileEnvironment.ForceIncludeFiles,
					.. CompileEnvironment.AdditionalPrerequisites,
					// track known included headers as resource compiler does not support writing input dependencies to a file
					FileItem.GetItemByFileReference(FileReference.Combine(Unreal.EngineSourceDirectory, "Runtime", "Core", "Public", "HAL", "PreprocessorHelpers.h")),
					FileItem.GetItemByFileReference(FileReference.Combine(Unreal.EngineSourceDirectory, "Runtime", "Launch", "Resources", "Windows", "resource.h")),
					FileItem.GetItemByFileReference(FileReference.Combine(Unreal.EngineSourceDirectory, "Runtime", "Launch", "Resources", "Version.h")),
				]);
				...
				{
					if (!Definition.Contains("_API", StringComparison.Ordinal))
					{
				...
				if (Target.WindowsPlatform.bSetResourceVersions)
				{
					...
					{
						AddDefinition(Arguments, $"BUILT_FROM_CHANGELIST={Target.Version.Changelist}");
					}
					if (!String.IsNullOrEmpty(Target.Version.BranchName))
					{
						AddDefinition(Arguments, $"BRANCH_NAME={Target.Version.BranchName}");
					}
					if (!String.IsNullOrEmpty(Target.BuildVersion))
					{
						AddDefinition(Arguments, $"BUILD_VERSION={Target.BuildVersion}");
					}
				}

				if (Target.WindowsPlatform.bDefineIconResource)
				{
					...
					FileItem IconFile = FileItem.GetItemByFileReference(GetApplicationIcon(!CompileEnvironment.bUseSharedBuildEnvironment ? Target.ProjectFile : null));
					CompileAction.PrerequisiteItems.Add(IconFile);

					// Setup the compile environment, setting the icon to use via a macro. This is used in Default.rc2.
					AddDefinition(Arguments, $"BUILD_ICON_FILE_NAME=\"\\\"{NormalizeCommandLinePath(IconFile, RootPaths).Replace("/", "\\\\", StringComparison.Ordinal)}\\\"\"");
				}
				...
				FileItem CompiledResourceFile = FileItem.GetItemByFileReference(FileReference.Combine(OutputDir, $"{Path.GetFileName(RCFile.AbsolutePath)}.res"));
				...
				// Create a response file for the resource compilier for informational purposes as it cannot be used
				FileItem ResponseFile = FileItem.GetItemByFileReference(GetResponseFileName(CompiledResourceFile));
		...
		public override CPPOutput ConvertRCFiles(LinkEnvironment LinkEnvironment, IEnumerable<FileItem> InputFiles, DirectoryReference OutputDir, IActionGraphBuilder Graph)
		{
			CPPOutput Result = new();

			// Do not convert resource files if the input list is empty, compiling an application, or of shared resource files are not in use
			// Converting resource files to objects when using the clang linker is not supported
			if (!InputFiles.Any()
				|| !(LinkEnvironment.bIsBuildingDLL || LinkEnvironment.bIsBuildingLibrary)
				|| !Target.WindowsPlatform.bSharedResourceFiles
				|| (EnvVars.Compiler.IsClang() && EnvVars.bAllowClangLinker))
			{
				Result.ObjectFiles.AddRange(InputFiles);
				return Result;
			}

			FileItem ConvertedResourceObjectFile = FileItem.GetItemByFileReference(FileReference.Combine(OutputDir, $"{InputFiles.First().Name}.obj"));

			List<string> ConvertArguments = [
				"/nologo",
				"/folddups",
				$"/machine:{LinkEnvironment.Architecture.WindowsName}",
				$"/out:\"{NormalizeCommandLinePath(ConvertedResourceObjectFile, LinkEnvironment.RootPaths)}\"",
				.. InputFiles.Select(x => $"\"{NormalizeCommandLinePath(x, LinkEnvironment.RootPaths)}\"")
			];

			Action ConvertAction = Graph.CreateAction(ActionType.Compile);
			ConvertAction.RootPaths = LinkEnvironment.RootPaths;
			ConvertAction.CommandDescription = $"Convert Resource [{LinkEnvironment.Architecture.WindowsName}]";
			ConvertAction.WorkingDirectory = Unreal.EngineSourceDirectory;
			ConvertAction.CommandPath = EnvVars.ResourceToObjectConverterPath;
			ConvertAction.CommandArguments = String.Join(' ', ConvertArguments);
			ConvertAction.StatusDescription = Path.GetFileName(InputFiles.First().AbsolutePath);
			ConvertAction.PrerequisiteItems.UnionWith(InputFiles);
			ConvertAction.ProducedItems.Add(ConvertedResourceObjectFile);
			ConvertAction.ArtifactMode = ArtifactMode.Enabled;
			ConvertAction.CacheBucket = GetCacheBucket(Target, LinkEnvironment);

			// Convert Resource tool can run remotely if possible
			ConvertAction.bCanExecuteRemotely = true;
			ConvertAction.bCanExecuteRemotelyWithSNDBS = false; // no tool template for SN-DBS results in warnings

			// Create a response file for the resource converter arguments for informational purposes as it cannot be used
			FileItem ResponseFile = FileItem.GetItemByFileReference(GetResponseFileName(LinkEnvironment, ConvertedResourceObjectFile));
			Graph.CreateIntermediateTextFile(ResponseFile, ConvertArguments);
			ConvertAction.PrerequisiteItems.Add(ResponseFile);

			Result.ObjectFiles.Add(ConvertedResourceObjectFile);
			return Result;
		}
		...
		public override void GenerateTypeLibraryHeader(CppCompileEnvironment CompileEnvironment, ModuleRules.TypeLibrary TypeLibrary, FileReference OutputFile, FileReference? OutputHeader, IActionGraphBuilder Graph)
		{
			...
			Action CompileAction = Graph.CreateAction(ActionType.Compile);
			CompileAction.ResponseFileContents = [.. Arguments];
			FileItem ResponseFile = Graph.CreateIntermediateTextFile(OutputFile.ChangeExtension(ResponseExt), CompileAction.ResponseFileContents, false);
		...
		public override CppCompileEnvironment CreateSharedResponseFile(CppCompileEnvironment CompileEnvironment, FileReference OutResponseFile, IActionGraphBuilder Graph)
		{
			...
			AppendCLArguments_Global(NewCompileEnvironment, Arguments);
		...
		public override FileItem[] LinkImportLibrary(LinkEnvironment LinkEnvironment, IActionGraphBuilder Graph)
		{
			...
			{
				// Windows Resource files can be excluded from import libraries
				LinkEnvironment CrossReferencedLinkEnvironment = new(LinkEnvironment);
				CrossReferencedLinkEnvironment.InputFiles.RemoveAll(x => x.Name.EndsWith(".res", StringComparison.OrdinalIgnoreCase) || x.Name.EndsWith(".res.obj", StringComparison.OrdinalIgnoreCase));
				return LinkAllFiles(CrossReferencedLinkEnvironment, true, Graph);
			}
		...
		public override FileItem LinkFiles(LinkEnvironment LinkEnvironment, bool bBuildImportLibraryOnly, IActionGraphBuilder Graph)
		{
			...
			{
				if (InputFile.HasExtension(".toml"))
				{
					// We don't want the toml file for import lib creation
					if (!LinkEnvironment.bIsBuildingDLL || !bIsBuildingLibraryOrImportLibrary)
					{
						Arguments.Add("/ifcDebugRecords");
						Arguments.Add($"/ifcMap:\"{NormalizeCommandLinePath(InputFile, RootPaths)}\"");
			...
			if (EnvVars.Compiler.IsMSVC() && LinkEnvironment.bPGOOptimize && LinkEnvironment.PGOFilenamePrefix != null && LinkEnvironment.OutputFilePath.FullName.EndsWith(".exe", StringComparison.Ordinal))
			{
			...
			if (!ProjectFileGenerator.bGenerateProjectFiles)
			{
				LinkAction.ResponseFileContents = [.. Arguments];
				FileItem ResponseFile = Graph.CreateIntermediateTextFile(ResponseFileName, LinkAction.ResponseFileContents);
		...
		{
			if (LinkEnvironment.bPGOOptimize && LinkEnvironment.OutputFilePath.FullName.EndsWith(".exe", StringComparison.Ordinal))
			{
		...
		private static IEnumerable<string> RemovePGOIgnoredSwitches(IEnumerable<string> SourceArguments)
		{
			...
			else // Clang
			{
				if (LinkEnvironment.bAllowLTCG || bPGOProfile || bPGOOptimize)
				{
					if (Target.WindowsPlatform.bAllowClangLinker)
					{
						...
						// we build everything with the same toolchain, so we don't want to upgrade debug info because it runs validation and is expensive
						Arguments.Add("-mllvm:-disable-auto-upgrade-debug-info");
		...
		protected virtual void ModifyFinalLinkArguments(LinkEnvironment LinkEnvironment, List<string> Arguments, bool bBuildImportLibraryOnly)
		{
			...
			Arguments.RemoveAll(Argument => Argument.StartsWith("/INCLUDE:IMPLEMENT_MODULE_", StringComparison.Ordinal));

			AddPGOLinkArguments(LinkEnvironment, Arguments);
		}

		private static void ExportObjectFilePaths(LinkEnvironment LinkEnvironment, string FileName, VCEnvironment EnvVars)
		{
		...
		public static string GetVCIncludePaths(UnrealTargetPlatform Platform, WindowsCompiler Compiler, WindowsCompiler ToolChain, string? CompilerVersion, string? ToolchainVersion, ILogger Logger)
		{
			// Make sure we've got the environment variables set up for this target
			VCEnvironment EnvVars = VCEnvironment.Create(Compiler, ToolChain, Platform, UnrealArch.X64, CompilerVersion, ToolchainVersion, null, null, false, false, false, Logger);
		...
		public override void SetUpGlobalEnvironment(ReadOnlyTargetRules Target, CppCompileEnvironment GlobalCompileEnvironment, LinkEnvironment GlobalLinkEnvironment)
		{
			...
			EpicGames.UBA.Utils.RegisterPathHash(EnvVars.WindowsSdkDir, EnvVars.WindowsSdkVersion.ToString());
			EpicGames.UBA.Utils.RegisterPathHash(EnvVars.CompilerDir, EnvVars.CompilerVersion.ToString());
			if (EnvVars.Compiler != EnvVars.ToolChain)
			{
				EpicGames.UBA.Utils.RegisterPathHash(EnvVars.ToolChainDir, EnvVars.ToolChainVersion.ToString());
			}

			if (!String.IsNullOrEmpty(Target.WindowsPlatform.DiaSdkDir))
			{
				EpicGames.UBA.Utils.RegisterPathHash(new(Target.WindowsPlatform.DiaSdkDir), EnvVars.ToolChainVersion.ToString());
			}
		...
		{
			if (Binary.Type == UEBuildBinaryType.DynamicLinkLibrary && (Target.WindowsPlatform.Compiler.IsMSVC() || !Target.WindowsPlatform.bAllowClangLinker))
			{
				if (Target.bShouldCompileAsDLL)
				{
					BuildProducts.Add(FileReference.Combine(Binary.OutputDir, Binary.OutputFilePath.GetFileNameWithoutExtension() + ".lib"), BuildProductType.SymbolFile);
					BuildProducts.Add(FileReference.Combine(Binary.OutputDir, Binary.OutputFilePath.GetFileNameWithoutExtension() + ".exp"), BuildProductType.SymbolFile);
				}
				else
				{
			...
			{
				FileReference IntermediateManifest = GetSxSIntermediateManifest(Target, Binary.OutputDir);
				BuildProducts.TryAdd(IntermediateManifest, BuildProductType.BuildResource);
			}
		}

		/// <summary>
		/// Get the intermediate location to store the side-by-side assembly manifest file for the given target.
		/// </summary>
		/// <see href="https://learn.microsoft.com/en-us/windows/win32/SbsCs/about-private-assemblies-" />
		/// <remarks>
		/// In theory, each target could have different dependencies, so ideally each target would have their own private
		/// assembly with its own copy of the dependencies. Unfortunately, the destination of the dependencies is scattered
		/// across various different build files (e.g. `Engine/Source/ThirdParty/Intel/TBB/IntelTBB.Build.cs`) which each
		/// specify that the dependencies should be output to a folder named for the target architecture. This means that
		/// it's impractical to output a unique assembly per target.
		/// 
		/// Instead, we can have binaries that are output to the same directory share a manifest, which ensures that all of
		/// all dependencies will be satisfied (though each application may get a few extra DLLs that they don't load).
		/// We only ever add to the manifests, because it's not safe to delete things - we can't force all targets sharing an
		/// output directory to build at the same time, and if targets A and B share an output directory, A has dep A' and
		/// B has dep B', then we would make the manifest unusable for B if we cleared it of B' when building A and not B.
		/// This does mean that we never remove dependencies that have been removed from the targets, but this is the safer
		/// end of the compromise.
		/// </remarks>
		private static FileReference GetSxSIntermediateManifest(ReadOnlyTargetRules Target, DirectoryReference ExeDirectory)
		{
			DirectoryReference BaseDirectory = Target.ProjectFile?.Directory ?? Unreal.EngineDirectory;
			DirectoryReference RootManifestDirectory = DirectoryReference.Combine(BaseDirectory, "Intermediate", "SxSManifest");

			string RelativeOutputDirectory = ExeDirectory.MakeRelativeTo(BaseDirectory);
			DirectoryReference ManifestDirectory = DirectoryReference.Combine(RootManifestDirectory, RelativeOutputDirectory);

			string ManifestName = $"{Target.Architecture}.manifest";
			return FileReference.Combine(ManifestDirectory, ManifestName);
		}
	}
}
