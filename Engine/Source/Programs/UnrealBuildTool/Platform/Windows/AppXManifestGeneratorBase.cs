...
{
	/// <summary>
	/// Information about an executable in an AppX manifest file
	/// </summary>
	public struct AppXManifestExecutable
	{
		/// Constructor
		public AppXManifestExecutable(UnrealTargetConfiguration InConfiguration, string InExecutablePath, UnrealArch? InArchitecture = null)
		{
		...
		/// The path to the executable - relative or absolute
		public readonly string ExecutablePath;

		/// The executable architecture (optional)
...
#pragma warning restore CA1707
		...
		protected string? ValidatePackageVersion(string InVersionNumber)
		{
			...
			if (WorkingVersionNumber != null)
			{
				...
				for (int VersionElement = 0; VersionElement < NumVersionElements; VersionElement++)
				{
					...
					if (QuadElement.Length == 0 || !Int32.TryParse(QuadElement, out int QuadValue))
					{
		...
		protected bool GetConfigBool(string PlatformKey, string? GenericKey, bool DefaultValue = false)
		{
			...
			ResultStr = ResultStr.Trim().ToUpperInvariant();
		...
		protected virtual bool BuildLocalizationData()
		{
			...
			if (EngineIni.GetArray(IniSection_PlatformTargetSettings, "PerCultureResources", out List<string>? PerCultureResources))
			{
				...
				foreach (string CultureResources in PerCultureResources)
				{
					...
					string CultureId;
					if (UEStageIdToAppXCultureId.ContainsKey(StageId))
					{
						CultureId = UEStageIdToAppXCultureId[StageId];
					}
					else
					{
						CultureId = StageId;
		...
		/// </summary>
		protected abstract XElement GetManifest(IEnumerable<AppXManifestExecutable> InExecutables, out string IdentityName);

		/// <summary>
		/// Perform any platform-specific processing on the manifest before it is saved
		/// </summary>
		protected virtual void ProcessManifest(IEnumerable<AppXManifestExecutable> InExecutables, string ManifestName, string ManifestTargetPath, string ManifestIntermediatePath)
		{
		}
		...
		/// </summary>
		public List<string>? CreateManifest(string InManifestName, DirectoryReference OutputDirectory, string? InTargetName, FileReference? InProjectFile, IEnumerable<AppXManifestExecutable> InExecutables)
		{
			...
			XDocument ManifestXmlDocument = new XDocument(GetManifest(InExecutables, out IdentityName));
			...
			ProcessManifest(InExecutables, InManifestName, ManifestTargetFile.FullName, ManifestIntermediateFile.FullName);
