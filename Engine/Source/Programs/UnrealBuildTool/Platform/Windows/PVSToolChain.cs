...
namespace UnrealBuildTool
{
	...
	/// </summary>
	[Flags]
#pragma warning disable CA1028 // uint data type more closely matches the underlying PVS-Studio value
...
#pragma warning disable CA1707 // Identifiers should not contain underscores - public facing API
	/// <summary>
	/// Supported PVS analyzer timeout values
	/// </summary>
	public enum AnalysisTimeout
	{
		/// <summary>
		/// Analysis timeout when not set (infinite)
		/// </summary>
		No_timeout = 0,
		/// <summary>
...
#pragma warning restore CA1707
	...
	public class PVSApplicationSettings
	{
		...
		/// </summary>
		public AnalysisTimeout AnalysisTimeout;
		...
		internal static PVSApplicationSettings? Read()
		{
			...
			if (FileReference.Exists(SettingsPath))
			{
				try
				{
					...
					return (PVSApplicationSettings?)Serializer.Deserialize(XmlReader);
				}
				catch (Exception Ex)
				{
	...
	public class PVSTargetSettings
	{
		...
		/// </summary>
		public AnalysisTimeout AnalysisTimeoutFlag
		{
			get
			{
				...
				{
					return AnalysisTimeout.After_30_minutes;
				}
			}
		...
		/// </summary>
		AnalysisTimeout? TimeoutPrivate;
	...
	public class ReadOnlyPVSTargetSettings(PVSTargetSettings Inner)
	{
		...
		/// </summary>
		public AnalysisTimeout AnalysisTimeoutFlag => Inner.AnalysisTimeoutFlag;
	...
	/// </summary>
	internal sealed class PVSGatherMode : IToolMode<PVSGatherMode>
	{
		public static string Name => "PVSGather";
...
#pragma warning restore IDE0044
		...
		/// </summary>
		const bool _ignoreThirdParty = true;
		...
		{
			// PVS-Studio output is camelCased, so we require insensitive property names to deserialize to our
			// standard C# property names.
		...
		PVSErrorInfo? GetErrorInfo(string line)
		{
			try
			{
				PVSErrorInfo errorInfo = JsonSerializer.Deserialize<PVSErrorInfo>(line, _jsonSerializerOptions) ?? throw new FormatException();
	...
	class PVSPosition
	{
		...
		{
			FileReference? fileReference = FileReference.FromString(File);
	...
	class PVSErrorInfo
	{
		...
		public required bool FalseAlarm { get; init; }

		public required int Level { get; init; }

		public required string Message { get; init; }

		public required PVSPosition[] Positions { get; init; }
	}
	...
	class PVSToolChain : ISPCToolChain
	{
		...
		public override void GetVersionInfo(List<string> Lines)
		{
			...
			Lines.Add($"Using PVS-Studio {AnalyzerVersion} at {AnalyzerFile} with analysis mode {(uint)settings.ModeFlags} ({settings.ModeFlags.ToString()})");
		}
		...
		public static string GetLangStandForCfgFile(CppStandardVersion cppStandard)
		{
		...
		{
			int cFlagLastPosition = Math.Max(Math.Max(compilerCommandLine.LastIndexOf("/TC ", StringComparison.Ordinal), compilerCommandLine.LastIndexOf("/Tc ", StringComparison.Ordinal)),
												Math.Max(compilerCommandLine.LastIndexOf("-TC ", StringComparison.Ordinal), compilerCommandLine.LastIndexOf("-Tc ", StringComparison.Ordinal)));

			int cppFlagLastPosition = Math.Max(Math.Max(compilerCommandLine.LastIndexOf("/TP ", StringComparison.Ordinal), compilerCommandLine.LastIndexOf("/Tp ", StringComparison.Ordinal)),
												Math.Max(compilerCommandLine.LastIndexOf("-TP ", StringComparison.Ordinal), compilerCommandLine.LastIndexOf("-Tp ", StringComparison.Ordinal)));

			bool compileAsCCode = cFlagLastPosition == cppFlagLastPosition
				? Path.GetExtension(sourceFileName).Equals(".c", StringComparison.OrdinalIgnoreCase)
		...
		CPPOutput PreprocessCppFiles(CppCompileEnvironment compileEnvironment, IEnumerable<FileItem> inputFiles, DirectoryReference outputDir, string moduleName, IActionGraphBuilder graph, out List<IExternalAction> preprocessActions)
		{
			...
			preprocessCompileEnvironment.Definitions.Add($"PVS_VERSION_MAJOR={AnalyzerVersion.Major}");
			preprocessCompileEnvironment.Definitions.Add($"PVS_VERSION_MINOR={AnalyzerVersion.Minor}");
		...
		void AnalyzeCppFile(VCCompileAction preprocessAction, CppCompileEnvironment compileEnvironment, DirectoryReference outputDir, CPPOutput result, IActionGraphBuilder graph)
		{
			...
			if (ApplicationSettings != null && ApplicationSettings.PathMasks != null)
			{
				...
				{
					if (pathMask.Contains(':', StringComparison.Ordinal) || pathMask.Contains('\\', StringComparison.Ordinal) || pathMask.Contains('/', StringComparison.Ordinal))
					{
						if (Path.IsPathRooted(pathMask) && !pathMask.Contains(':', StringComparison.Ordinal))
						{
							configFileContents.AppendFormat("exclude-path=*{0}*\n", pathMask);
						}
						else
			...
			if (!shouldCompileAsC)
			{
				string languageStandardForCfg = GetLangStandForCfgFile(compileEnvironment.CppStandard);

				configFileContents.AppendFormat("std={0}\n", languageStandardForCfg);

				bool disableMsExtensionsFromArgs = preprocessAction.Arguments.Any(arg => arg.Equals("/Za", StringComparison.Ordinal) || arg.Equals("-Za", StringComparison.Ordinal) || arg.Equals("/permissive-", StringComparison.Ordinal));
			...
			if (isVisualCppCompiler && preprocessAction.Arguments.Any(arg => arg.StartsWith("/await", StringComparison.Ordinal)))
			{
			...
			int timeout = (int)Settings.AnalysisTimeoutFlag;
			...
			// PVS-Studio loads global configuration files prior to the ones specified by the user.
			DirectoryReference? applicationData = DirectoryReference.GetSpecialFolder(Environment.SpecialFolder.ApplicationData);
			if (applicationData != null && DirectoryReference.Exists(applicationData))
			{
				DirectoryReference globalPVSConfigsDir = DirectoryReference.Combine(applicationData, "PVS-Studio");

				if (DirectoryReference.Exists(globalPVSConfigsDir))
				{
					foreach (FileReference configPath in DirectoryReference.EnumerateFiles(globalPVSConfigsDir, "*.pvsconfig", SearchOption.TopDirectoryOnly))
					{
						arguments.Add($"--rules-config=\"{configPath.FullName}\"");
						analyzeAction.PrerequisiteItems.Add(FileItem.GetItemByFileReference(configPath));
					}
				}
			}

			foreach (string configPath in Target.StaticAnalyzerPVSConfigs.Where(path => path.EndsWith(".pvsconfig", StringComparison.InvariantCultureIgnoreCase)))
			{
				string fullPVSConfigPath = configPath;

				if (!Path.IsPathRooted(fullPVSConfigPath) && Target.ProjectFile != null)
				{
					string projectDir = Path.GetDirectoryName(Target.ProjectFile.FullName)!;
					fullPVSConfigPath = Path.Combine(projectDir, fullPVSConfigPath);
				}

				if (Path.Exists(fullPVSConfigPath))
				{
					arguments.Add($"--rules-config=\"{fullPVSConfigPath}\"");
					analyzeAction.PrerequisiteItems.Add(FileItem.GetItemByPath(fullPVSConfigPath));
				}
			}
		...
		public static IExternalAction? SingleFileFinalizeOutput(IExternalAction finalizeAction, IEnumerable<IExternalAction> prereqActions)
		{
			...
			if (match.Success)
			{
				...
				int printLevel = Int32.Parse(match.Groups[5].Value);
				...
				singleFinalizeAction.CommandArguments = $"{singleFinalizeAction.CommandArguments[..singleFinalizeAction.CommandArguments.IndexOf("-Input=", StringComparison.Ordinal)]}{arguments}";
