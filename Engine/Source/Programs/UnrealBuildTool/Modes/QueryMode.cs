...
namespace UnrealBuildTool
{
	...
	internal class TargetIntellisenseInfo
	{
		internal class CompileSettings
		{
			...
			public List<string> ForcedIncludes { get; set; } = [];
			public string? CompilerPath { get; set; }
			public List<string> CompilerArgs { get; set; } = [];
			public Dictionary<string, List<string>> CompilerAdditionalArgs { get; set; } = [];
			public string? WindowsSdkVersion { get; set; }
		}

		public ConcurrentDictionary<UEBuildModule, CompileSettings> ModuleToCompileSettings = [];
		public ConcurrentDictionary<DirectoryReference, UEBuildModule> DirToModule = [];
	...
	internal sealed class QueryMode : IToolMode<QueryMode>
	{
		public static string Name => "Query";
		public static ToolModeOptions Options => ToolModeOptions.BuildPlatforms | ToolModeOptions.XmlConfig | ToolModeOptions.UseStartupTraceListener;
		...
		private readonly BuildConfiguration BuildConfiguration = new();
		...
		public Task<int> ExecuteAsync(CommandLineArguments Arguments, ILogger Logger)
		{
			...
			switch (Query)
			{
				case QueryType.Capabilities:
					...
					return QueryCapabilities(Logger, ResponseOptions);
		...
		private async Task<int> QueryCapabilities(ILogger Logger, JsonSerializerOptions JsonOptions)
		{
			var Reply = new
			{
		...
		private async Task<int> QueryAvailableTargetsAsync(CommandLineArguments Arguments, ILogger Logger, JsonSerializerOptions JsonOptions)
		{
			try
			{
				...
				List<UnrealTargetConfiguration> AllowedTargetConfigurations = [];
				AllowedTargetConfigurations = [.. Enum.GetValues(typeof(UnrealTargetConfiguration)).Cast<UnrealTargetConfiguration>()];
				...
				List<FileReference> Projects = ProjectFileArg != null ? [ProjectFileArg] : [.. NativeProjects.EnumerateProjectFiles(Logger)];
				...
				Dictionary<string, TargetConfigs> Targets = [];
				...
				foreach (FileReference TargetFilePath in AllTargetFiles)
				{
					try
					{
						...
						List<UnrealTargetConfiguration> SupportedConfigurations = [.. Configurations];
						List<UnrealTargetPlatform> SupportedPlatforms = [.. Platforms];
						if (bLoadTargets)
						{
							List<string> RawArgs = [TargetFilePath.GetFileNameWithoutAnyExtensions(), UnrealTargetConfiguration.Development.ToString(), UnrealTargetPlatform.Win64.ToString()];
							if (ProjectPath != null)
							{
							...
							SupportedConfigurations = [.. CurrentTarget.Rules.SupportedConfigurations];
							SupportedPlatforms = [.. CurrentTarget.Rules.SupportedPlatforms];
						}
						...
						Targets.Add(TargetName, new TargetConfigs()
						{
							...
							Configurations = [.. SupportedConfigurations.Select(x => x.ToString())],
							Platforms = [.. SupportedPlatforms.Select(x => x.ToString())]
						});
		...
		private async Task<int> QueryTargetDetails(CommandLineArguments Arguments, ILogger Logger, JsonSerializerOptions JsonOptions)
		{
			...
			Utils.TryParseProjectFileArgument(Arguments, Logger, out FileReference? ProjectFileArg);
			List<string> RawArgs = [TargetName, TargetConfiguration, TargetPlatform];
			...
			try
			{
				...
				if (!bNoIntellisense)
				{
					...
					if (CurrentTarget.Rules.bUseSharedPCHs)
					{
						...
						UEBuildTarget.CreateSharedPCHInstances(CurrentTarget.Rules, TargetToolChain, CurrentTarget.Binaries, GlobalCompileEnvironment, new NullActionGraphBuilder(Logger), Logger);
					}
					...
					await Parallel.ForEachAsync(CurrentTarget.Binaries, async (Binary, CancellationToken) =>
					{
						...
						await Parallel.ForEachAsync(Modules, (Module, CancellationToken) =>
						{
							...
							foreach (DirectoryItem Dir in Module.ModuleDirectories)
							{
								CurrentTargetIntellisenseInfo.DirToModule.TryAdd(Dir.Location, Module);
							}
							...
							Settings.ForcedIncludes = [.. ModuleCompileEnvironment.ForceIncludeFiles.Select(x => x.ToString())];
							Settings.CompilerPath = TargetToolChain.GetCppCompilerPath()?.ToString();
							Settings.CompilerArgs = [.. TargetToolChain.GetGlobalCommandLineArgs(ModuleCompileEnvironmentForArgs)];
							Settings.CompilerAdditionalArgs.Add("c", [.. TargetToolChain.GetCCommandLineArgs(ModuleCompileEnvironmentForArgs)]);
							Settings.CompilerAdditionalArgs.Add("cpp", [.. TargetToolChain.GetCPPCommandLineArgs(ModuleCompileEnvironmentForArgs)]);
				...
				var Result = new
				{
					...
					CurrentTarget.Rules.IsTestTarget
				};
			...
			try
			{
				...
				await Parallel.ForEachAsync(CurrentTarget.Binaries, async (Binary, CancellationToken) =>
				{
					...
					await Parallel.ForEachAsync(Modules, (Module, CancellationToken) =>
					{
						...
						var Info = new
						{
							ShortName = Module.Rules.ShortName,
							ModuleDirectory = Module.ModuleDirectory.FullName,
							// Strip the query suffix from the target name so we output the real path.
							IntermediateDirectory = Module.IntermediateDirectory.FullName.Replace($"{CurrentTarget.TargetName}QRY", CurrentTarget.TargetName, StringComparison.Ordinal),
