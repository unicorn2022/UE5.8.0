...
namespace UnrealBuildTool.Modes
{
	...
	/// </summary>
	internal sealed class PipInstallMode : IToolMode<PipInstallMode>
	{
		public static string Name => "PipInstall";
		public static ToolModeOptions Options => ToolModeOptions.XmlConfig | ToolModeOptions.BuildPlatformsHostOnly | ToolModeOptions.SingleInstance | ToolModeOptions.StartPrefetchingEngine | ToolModeOptions.ShowExecutionTime;
	...
	class PipEnv
	{
		...
		private readonly UnrealArch TargetArch;
		private readonly UnrealTargetPlatform TargetPlatform;
		private readonly DirectoryReference InstallDir;
		private readonly FileReference PythonVenvExe;
		private readonly string? PythonVenvVer;

		private readonly ILogger Logger;
		private readonly IBaseProgressLogFactory LoggerFactory;
		...
		public void WritePluginsListing(UEBuildTarget Target, ILogger Logger, bool bAllPlugins = false)
		{
			...
			foreach (PluginInfo Plugin in CheckPlugins)
			{
				...
				foreach (JsonObject _ in PipEnv.CompatibleRequirements(PluginJson, TargetPlatform, TargetArch))
				{
		...
		private static bool CompatiblePlatform(string? PlatformField, UnrealTargetPlatform Platform)
		{
			return (PlatformField == null)
				|| String.Equals(PlatformField, Platform.ToString(), StringComparison.OrdinalIgnoreCase)
				|| String.Equals(PlatformField, "All", StringComparison.OrdinalIgnoreCase);
		}

		private static bool CompatibleArch(string? ArchField, UnrealArch Arch)
		{
			return (ArchField == null)
				|| String.Equals(ArchField, MakeArchString(Arch), StringComparison.OrdinalIgnoreCase)
				|| String.Equals(ArchField, "All", StringComparison.OrdinalIgnoreCase);
		}
		...
		private bool MergeRequirements(FileReference PluginsListingFile, out List<string>? MergedRequirements, out List<string>? ExtraIndexUrls)
		{
			...
			foreach (string PluginPath in PythonPlugins)
			{
				...
				if (PluginFile is null || !FileReference.Exists(PluginFile))
				{
					continue;
				}
		...
		private FileReference GetEnginePythonInterpreter(FileReference? EnginePython)
		{
			...
			if (FileReference.Exists(PythonSDKFile))
			{
				...
				PythonSDKRoot = new DirectoryReference(SDKRootText.Replace("{ENGINE_DIR}", EngineDir.ToString(), StringComparison.Ordinal));
			}
			else
			{
		...
		private static FileReference? GetPythonScriptPlugin()
		{
	...
	class PythonCmdLogger : IBaseCmdProgressLogger
	{
		...
		private static void SplitOn(string InStr, char SplitChar, out string Left, out string? Right)
		{
			...
			int SplitIdx = InStr.IndexOf(SplitChar, StringComparison.Ordinal);
			if (SplitIdx < 0)
			{
				return;
			}
	...
	class PipProgressLogger : IBaseCmdProgressLogger
	{
		...
		string ReplaceMatchStr(string CheckStr)
		{
			...
			{
				if (CheckStr.Contains(ChkPair.Key, StringComparison.Ordinal))
				{
					return CheckStr.Replace(ChkPair.Key, ChkPair.Value, StringComparison.Ordinal);
				}
			}
