...
namespace UnrealBuildTool
{
	...
	/// </summary>
	internal sealed class GenerateClangDatabase : IToolMode<GenerateClangDatabase>
	{
		public static string Name => "GenerateClangDatabase";
...
#pragma warning restore IDE0044
		...
		public async Task<int> ExecuteAsync(CommandLineArguments Arguments, ILogger Logger)
		{
			...
			using (ISourceFileWorkingSet WorkingSet = new EmptySourceFileWorkingSet())
			{
				...
				foreach (TargetDescriptor TargetDescriptor in TargetDescriptors)
				{
					...
					if (CompileActions.Any())
					{
						foreach (IExternalAction Action in CompileActions)
						{
							...
							string CommandPath = Action.CommandPath.FullName.Contains(' ', StringComparison.Ordinal) ? Utils.MakePathSafeToUseWithCommandLine(Action.CommandPath) : Action.CommandPath.FullName;
				...
				Logger.LogInformation("ClangDatabase written to {DatabasePath}", DatabaseFile.FullName);
