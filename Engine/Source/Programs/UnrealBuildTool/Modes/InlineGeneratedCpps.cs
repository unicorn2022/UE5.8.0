...
namespace UnrealBuildTool
{
	...
	/// </summary>
	internal sealed class InlineGeneratedCpps : IToolMode<InlineGeneratedCpps>
	{
		public static string Name => "InlineGeneratedCpps";
						...
						foreach (UEBuildModuleCPP Module in Binary.Modules.OfType<UEBuildModuleCPP>())
						{
							...
							if (FileList.Any())
							{
								...
								foreach (FileItem InputFile in FileList)
								{
									...
									string InputFilenameWithoutCpp = InputFile.Location.GetFileName().Replace(".cpp", String.Empty, StringComparison.Ordinal);
									FileItem? FoundHeader = InputFileCollection.HeaderFiles.FirstOrDefault(hf => hf.Name.Replace(".h", String.Empty, StringComparison.Ordinal) == InputFilenameWithoutCpp);
									if (FoundHeader == null)
									{
										continue;
									}
									...
									{
										string HeaderText = await FileReference.ReadAllTextAsync(FoundHeader.Location);
									...
									List<string> TextLines = [.. await FileReference.ReadAllLinesAsync(InputFile.Location)];
									...
									try
									{
										// add the include
										{
											...
											TextLines.Insert(LastLineOfInclude + 1, String.Format("#include UE_INLINE_GENERATED_CPP_BY_NAME({0})", FoundHeader.Name.Replace(".h", String.Empty, StringComparison.Ordinal)));
											TextLines.Insert(LastLineOfInclude + 1, String.Empty);
										}
										...
										if (!bNoOutput)
										{
											...
											await System.IO.File.WriteAllLinesAsync(InputFile.FullName, TextLines);
										}
									}
									catch (Exception ex)
									{
