...
namespace UnrealBuildTool
{
	...
	/// </summary>
	internal sealed class AggregateClangTimingInfo : IToolMode<AggregateClangTimingInfo>
	{
		public static string Name => "AggregateClangTimingInfo";
		...
		public class TraceData
		{
			...
			public TraceData(FileReference inputFile, ClangTrace? trace)
			{
				...
				TotalExecuteCompiler = trace?.traceEvents?.FindLast(x => String.Equals(x.name, "Total ExecuteCompiler", StringComparison.Ordinal))?.dur ?? 0;

				// Subset of execute compiler
				TotalFrontend = trace?.traceEvents?.FindLast(x => String.Equals(x.name, "Total Frontend", StringComparison.Ordinal))?.dur ?? 0;
				TotalBackend = trace?.traceEvents?.FindLast(x => String.Equals(x.name, "Total Backend", StringComparison.Ordinal))?.dur ?? 0;

				// Subset of frontend
				TotalSource = trace?.traceEvents?.FindLast(x => String.Equals(x.name, "Total Source", StringComparison.Ordinal))?.dur ?? 0;
				TotalInstantiateFunction = trace?.traceEvents?.FindLast(x => String.Equals(x.name, "Total InstantiateFunction", StringComparison.Ordinal))?.dur ?? 0;
				TotalCodeGenFunction = trace?.traceEvents?.FindLast(x => String.Equals(x.name, "Total CodeGen Function", StringComparison.Ordinal))?.dur ?? 0;

				// Subset of backend
				TotalModuleToFunctionPassAdaptor = trace?.traceEvents?.FindLast(x => String.Equals(x.name, "Total ModuleToFunctionPassAdaptor", StringComparison.Ordinal))?.dur ?? 0;
				TotalModuleInlinerWrapperPass = trace?.traceEvents?.FindLast(x => String.Equals(x.name, "Total ModuleInlinerWrapperPass", StringComparison.Ordinal))?.dur ?? 0;
				TotalOptModule = trace?.traceEvents?.FindLast(x => String.Equals(x.name, "Total OptModule", StringComparison.Ordinal))?.dur ?? 0;

				// Frontend entry counts
				SourceEntries = trace?.traceEvents?.Where(x => String.Equals(x.name, "Source", StringComparison.Ordinal)).LongCount() ?? 0;
				InstantiateFunctionEntries = trace?.traceEvents?.Where(x => String.Equals(x.name, "InstantiateFunction", StringComparison.Ordinal)).LongCount() ?? 0;
				CodeGenFunctionEntries = trace?.traceEvents?.Where(x => String.Equals(x.name, "CodeGen Function", StringComparison.Ordinal)).LongCount() ?? 0;
		...
		private readonly ConcurrentDictionary<FileReference, ClangTrace> ClangTraceCache = new();
		...
		public async Task<int> ExecuteAsync(CommandLineArguments Arguments, ILogger Logger)
		{
			...
			IEnumerable<FileReference> SourceFiles = (await FileReference.ReadAllLinesAsync(ManifestFile)).Select(x => new FileReference(x));
			...
			if (AggregateFile != null)
			{
				...
				TraceData[] RawTraceData = await Tasks;
				List<TraceData> TraceDatas = RawTraceData.OrderBy(x => x.Module).ThenBy(x => x.Name).ToList();

				string TempFilePath = Path.Join(Path.GetTempPath(), AggregateFile.GetFileName() + ".tmp");
				using (StreamWriter Writer = new StreamWriter(TempFilePath))
				{
					await Writer.WriteLineAsync(TraceData.CsvHeader);
					foreach (TraceData Data in TraceDatas)
					{
						await Writer.WriteLineAsync(Data.CsvLine);
					}
				}
			...
			if (HeadersFile != null)
			{
				...
				ClangTrace[] RawTraceData = await Tasks;
				List<ClangTrace> ClangTraces = RawTraceData.ToList();
				...
				{
					foreach (ClangTrace.TraceEvent Event in ClangTrace.traceEvents!.Where(x => String.Equals(x.name, "Source", StringComparison.Ordinal) && x.args?.ContainsKey("detail") == true))
					{
				...
				using (StreamWriter Writer = new StreamWriter(TempFilePath))
				{
					await Writer.WriteLineAsync("Source,Count,Total,Min,Max,Average");
					...
					{
						await Writer.WriteLineAsync($"{Data.Key.FullName},{Data.Value.Count},{Data.Value.Sum()},{Data.Value.Min()},{Data.Value.Max()},{Data.Value.Average()}");
					}
				}
