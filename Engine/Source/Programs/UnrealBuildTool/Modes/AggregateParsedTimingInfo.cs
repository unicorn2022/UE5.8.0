...
namespace UnrealBuildTool
{
	...
	/// </summary>
	internal sealed class AggregateParsedTimingInfo : IToolMode<AggregateParsedTimingInfo>
	{
		public static string Name => "AggregateParsedTimingInfo";
		...
		private readonly Lock AddFileLock = new();
		...
		public async Task<int> ExecuteAsync(CommandLineArguments Arguments, ILogger Logger)
		{
			...
			string[] ParsedFileNames = await FileReference.ReadAllLinesAsync(ManifestFile);
			...
			if (Arguments.HasValue("-CompileTimingFile="))
			{
				...
				await FileReference.WriteAllTextAsync(CompileTimingFile, JsonCompileTimes);
			}

			return 0;
		}

		private static void GroupTimingDataOnName(TimingData AggregateData, string TimingName, IEnumerable<TimingData> UngroupedData)
		{
		...
		private static void FlattenIncludes(Dictionary<string, TimingData> Includes, IEnumerable<TimingData> IncludeData)
		{
		...
		private static IEnumerable<TimingData> GroupChildren(IEnumerable<TimingData> Children, TimingDataType Type)
		{
			...
			foreach (TimingData Child in Children)
			{
				...
				else
				{
					...
					string GroupName = Child.Name.Replace(Match.Groups["Template"].Value, String.Format("<{0}>", String.Join(", ", TemplateParamSig)), StringComparison.Ordinal);
