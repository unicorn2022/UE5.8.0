...
{
	internal sealed class ClReproMode : IToolMode<ClReproMode>
	{
		public static string Name => "ClRepro";
		public static ToolModeOptions Options => ToolModeOptions.UseStartupTraceListener;
		...
		private async Task<int> PrepareFullRepro(ILogger logger)
		{
			...
			{
				depends = await JsonSerializer.DeserializeAsync<ClDepends>(fstream);
			}
			...
			await FileReference.WriteAllLinesAsync(outputRspFile, (await FileReference.ReadAllLinesAsync(inputRspFile)).Select(x => FixPaths(x)));
			logger.LogInformation("Rewrote absolute paths for {Input} to {Output}", inputRspFile, outputRspFile);
			await FileReference.WriteAllLinesAsync(outputSharedRspFile, (await FileReference.ReadAllLinesAsync(inputSharedRspFile)).Select(x => FixPaths(x)));
			...
			await FileReference.WriteAllLinesAsync(batchFile, new string[]
			{
				$"@echo off",
				$"setlocal",
