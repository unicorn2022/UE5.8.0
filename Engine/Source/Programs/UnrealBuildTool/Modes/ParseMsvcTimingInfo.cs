...
namespace UnrealBuildTool
{
	...
	/// </summary>
	internal sealed class ParseMsvcTimingInfoMode : IToolMode<ParseMsvcTimingInfoMode>
	{
		public static string Name => "ParseMsvcTimingInfo";
		...
		public async Task<int> ExecuteAsync(CommandLineArguments Arguments, ILogger Logger)
		{
			...
			string[] AllLines = await FileReference.ReadAllLinesAsync(InputFile);
			...
			// Build the summary.
			TimingData Summary = new TimingData(InputFile.FullName.Replace(".timing.txt", String.Empty, StringComparison.Ordinal), TimingDataType.Summary);
		...
		private static TimingData SummarizeParsedTimingData(string SummaryName, TimingDataType TimingType, IEnumerable<string> Lines)
		{
		...
		private static List<TimingData> ParseTimingDataFromLines(TimingDataType TimingType, IEnumerable<string> Lines)
		{
		...
		private static TimingData? ParseTimingDataFromLine(TimingDataType TimingType, string Line, out int LineDepth)
		{
			Match TimingDataMatch = Regex.Match(Line, TimingDataRegex);
			if (!TimingDataMatch.Success)
			{
		...
		private static void ParseTimingDataToTracingFiles(FileReference InputFile)
		{
		...
		private static int ParseIncludeHeadersToTraces(string[] Lines, int LineIdx, FileReference OutputFile)
		{
		...
		private static int ParseDefinitions(string[] Lines, int LineIdx, FileReference OutputFile)
		{
			if (LineIdx < Lines.Length && Lines[LineIdx].StartsWith("\tCount:", StringComparison.Ordinal))
			{
				LineIdx++;
			...
			for (; LineIdx < Lines.Length; LineIdx++)
			{
				...
				int TemplateIdx = ClassName.IndexOf('<', StringComparison.Ordinal);
				if (TemplateIdx != -1)
				{
