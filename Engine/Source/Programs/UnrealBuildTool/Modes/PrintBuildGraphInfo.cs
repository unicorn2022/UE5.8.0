...
namespace UnrealBuildTool
{
	...
	/// </summary>
	internal sealed class PrintBuildGraphInfo : IToolMode<PrintBuildGraphInfo>
	{
		public static string Name => "PrintBuildGraphInfo";
					...
					foreach (UEBuildModule Module in Modules)
					{
						...
						if (ModuleDepStacks.Any())
						{
							...
							foreach (Stack<UEBuildModule> DepModuleStack in ModuleDepStacks)
							{
								...
								Logger.LogInformation("   {Dependencies}", Dependencies);
							}
						}
					}
				}
		...
		private static void FindCircularDependencyModules(UEBuildModule SearchForBuildModule, UEBuildModule CurrentBuildModule, HashSet<UEBuildModule> IgnoreReferencedModules, bool bIncludePrivateDependencyModules, Stack<UEBuildModule> CurrentStack, List<Stack<UEBuildModule>> ModuleDepStacks)
		{
