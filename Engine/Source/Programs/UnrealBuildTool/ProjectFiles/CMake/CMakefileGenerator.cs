...
namespace UnrealBuildTool
{
	...
	class CMakefileGenerator : ProjectFileGenerator
	{
		...
		/// </summary>
		public CMakefileGenerator(FileReference? InOnlyGameProject, CSharpProjectCache CSharpProjCache)
		...
		/// </summary>
		public static string CMakeExtension => ".cmake";

		/// <summary>
		/// The CMake file used to store the list of includes for the project.
		/// </summary>
		public static string CMakeIncludesFileName => "cmake-includes" + CMakeExtension;

		/// <summary>
		/// The CMake file used to store the configuration files (INI) for the engine.
		/// </summary>
		public static string CMakeEngineConfigsFileName => "cmake-config-engine" + CMakeExtension;

		/// <summary>
		/// The CMake file used to store the configuration files (INI) for the project.
		/// </summary>
		public static string CMakeProjectConfigsFileName => "cmake-config-project" + CMakeExtension;

		/// <summary>
		/// The CMake file used to store the additional build configuration files (CSharp) for the engine.
		/// </summary>
		public static string CMakeEngineCSFileName => "cmake-csharp-engine" + CMakeExtension;

		/// <summary>
		/// The CMake file used to store the additional configuration files (CSharp) for the project.
		/// </summary>
		public static string CMakeProjectCSFileName => "cmake-csharp-project" + CMakeExtension;

		/// <summary>
		/// The CMake file used to store the additional shader files (usf/ush) for the engine.
		/// </summary>
		public static string CMakeEngineShadersFileName => "cmake-shaders-engine" + CMakeExtension;

		/// <summary>
		/// The CMake file used to store the additional shader files (usf/ush) for the project.
		/// </summary>
		public static string CMakeProjectShadersFileName => "cmake-shaders-project" + CMakeExtension;

		/// <summary>
		/// The CMake file used to store the list of engine headers.
		/// </summary>
		public static string CMakeEngineHeadersFileName => "cmake-headers-ue" + CMakeExtension;
		/// <summary>
		/// The CMake file used to store the list of engine headers.
		/// </summary>
		public static string CMakeProjectHeadersFileName => "cmake-headers-project" + CMakeExtension;

		/// <summary>
		/// The CMake file used to store the list of sources for the engine.
		/// </summary>
		public static string CMakeEngineSourcesFileName => "cmake-sources-engine" + CMakeExtension;

		/// <summary>
		/// The CMake file used to store the list of sources for the project.
		/// </summary>
		public static string CMakeProjectSourcesFileName => "cmake-sources-project" + CMakeExtension;

		/// <summary>
		/// The CMake file used to store the list of definitions for the project.
		/// </summary>
		public static string CMakeDefinitionsFileName => "cmake-definitions" + CMakeExtension;
		...
		{
			if (!SourceFileRelativeToRoot.StartsWith("..", StringComparison.Ordinal) && !Path.IsPathRooted(SourceFileRelativeToRoot))
			{
		...
		private bool WriteCMakeLists(ILogger Logger)
		{
			...
			foreach (ProjectFile CurProject in GeneratedProjectFiles)
			{
				foreach (string IncludeSearchPath in CurProject.IntelliSenseIncludeSearchPaths)
				{
					...
					{
						if (IncludeDirectory.Contains(Unreal.RootDirectory.FullName, StringComparison.Ordinal))
						{
							IncludeDirectories.Add(IncludeDirectory.Replace(Unreal.RootDirectory.FullName, UnrealRootPath, StringComparison.Ordinal));
						}
						else
						{
				...
				{
					string Definition = PreProcessorDefinition.Replace("TEXT(\"", "", StringComparison.Ordinal).Replace("\")", "", StringComparison.Ordinal).Replace("()=", "=", StringComparison.Ordinal);
					string AlternateDefinition = Definition.Contains("=0", StringComparison.Ordinal) ? Definition.Replace("=0", "=1", StringComparison.Ordinal) : Definition.Replace("=1", "=0", StringComparison.Ordinal);

					if (Definition.Equals("WITH_EDITORONLY_DATA=0", StringComparison.Ordinal))
					{
					...
					if (!PreprocessorDefinitions.Contains(Definition) &&
						...
						!Definition.StartsWith("UE_ENGINE_DIRECTORY", StringComparison.Ordinal) &&
						!Definition.StartsWith("ORIGINAL_FILE_NAME", StringComparison.Ordinal))
					{
			...
			foreach (FileReference CurModuleFile in AllModuleFiles)
			{
				...
				foreach (FileReference CurSourceFile in FoundFiles)
				{
					...
					{
						if (SourceFileRelativeToRoot.EndsWith(".cpp", StringComparison.Ordinal))
						{
						...
						else if (SourceFileRelativeToRoot.EndsWith(".h", StringComparison.Ordinal))
						{
						...
						else if (SourceFileRelativeToRoot.EndsWith(".cs", StringComparison.Ordinal))
						{
						...
						else if (SourceFileRelativeToRoot.EndsWith(".usf", StringComparison.Ordinal) || SourceFileRelativeToRoot.EndsWith(".ush", StringComparison.Ordinal))
						{
						...
						else if (SourceFileRelativeToRoot.EndsWith(".ini", StringComparison.Ordinal))
						{
			...
			{
				int EqPos = PreprocessorDefinition.IndexOf('=', StringComparison.Ordinal);
				if (EqPos >= 0)
				{
					string Key = PreprocessorDefinition.Substring(0, EqPos);
					string Value = PreprocessorDefinition.Substring(EqPos).Replace("\"", "\\\"", StringComparison.Ordinal);
			...
			{
				string SourceFileRelativeToRoot = CurSourceFile.MakeRelativeTo(Unreal.EngineDirectory);
				if (SourceFileRelativeToRoot.EndsWith(".usf", StringComparison.Ordinal) || SourceFileRelativeToRoot.EndsWith(".ush", StringComparison.Ordinal))
				{
					AppendCleanedPathToList(CMakeEngineShaderFilesList, CMakeProjectShaderFilesList, SourceFileRelativeToRoot, CurSourceFile.FullName, GameProjectPath, UnrealRootPath, CMakeGameRootPath);
				}
			}
			...
			{
				string SourceFileRelativeToRoot = CurSourceFile.MakeRelativeTo(Unreal.EngineDirectory);
				if (SourceFileRelativeToRoot.EndsWith(".ini", StringComparison.Ordinal))
				{
					AppendCleanedPathToList(CMakeEngineConfigFilesList, CMakeProjectConfigFilesList, SourceFileRelativeToRoot, CurSourceFile.FullName, GameProjectPath, UnrealRootPath, CMakeGameRootPath);
				}
			}
		...
		private static bool IsPathExcludedOnLinux(string SourceFileRelativeToRoot)
		{
			...
			return SourceFileRelativeToRoot.Contains("Source/ThirdParty/", StringComparison.Ordinal);
		}

		private static bool IsPathExcludedOnMac(string SourceFileRelativeToRoot)
		{
			return SourceFileRelativeToRoot.Contains("Source/ThirdParty/", StringComparison.Ordinal) ||
				SourceFileRelativeToRoot.Contains("/Windows/", StringComparison.Ordinal) ||
				SourceFileRelativeToRoot.Contains("/Linux/", StringComparison.Ordinal) ||
				SourceFileRelativeToRoot.Contains("/VisualStudioSourceCodeAccess/", StringComparison.Ordinal) ||
				SourceFileRelativeToRoot.Contains("/WmfMedia/", StringComparison.Ordinal) ||
				SourceFileRelativeToRoot.Contains("/WindowsDeviceProfileSelector/", StringComparison.Ordinal) ||
				SourceFileRelativeToRoot.Contains("/WindowsMoviePlayer/", StringComparison.Ordinal) ||
				SourceFileRelativeToRoot.Contains("/WinRT/", StringComparison.Ordinal);
		}
		...
		private bool IsTargetExcluded(string TargetName, UnrealTargetPlatform TargetPlatform, UnrealTargetConfiguration TargetConfig)
		{
			if (TargetPlatform == UnrealTargetPlatform.IOS || TargetPlatform == UnrealTargetPlatform.TVOS)
			{
				if (
					(
						TargetName.StartsWith("UnrealGame", StringComparison.Ordinal) ||
						(IsProjectBuild && TargetName.StartsWith(GameProjectName!, StringComparison.Ordinal)) || TargetName.StartsWith("QAGame", StringComparison.Ordinal)
					) && !TargetName.StartsWith("QAGameEditor", StringComparison.Ordinal))
				{
					return false;
				}
				return true;
			...
			if (bCmakeMinimalTargets)
			{
				...
				if ((TargetName.StartsWith("UnrealEditor", StringComparison.Ordinal) && !TargetName.StartsWith("UnrealEditorServices", StringComparison.Ordinal)) ||
					TargetName.StartsWith("UnrealGame", StringComparison.Ordinal) ||
					(IsProjectBuild && TargetName.StartsWith(GameProjectName!, StringComparison.Ordinal)))
				{
					return false;
				}
				// SCW & CRC are minimally included as just development builds
				else if (TargetConfig == UnrealTargetConfiguration.Development &&
					(TargetName.StartsWith("ShaderCompileWorker", StringComparison.Ordinal) || TargetName.StartsWith("CrashReportClient", StringComparison.Ordinal)))
				{
					return false;
				}
				else if ((TargetName.StartsWith("QAGameEditor", StringComparison.Ordinal) && !TargetName.StartsWith("QAGameEditorServices", StringComparison.Ordinal)) || TargetName.StartsWith("QAGame", StringComparison.Ordinal))
				{
					return false;
				}
				return true;
		...
		private static string GetIncludeDirectory(string IncludeDir, string ProjectDir)
		{
			...
			if (Path.IsPathRooted(IncludeDir) &&
				!IncludeDir.StartsWith(FullProjectPath, StringComparison.Ordinal) &&
				!IncludeDir.StartsWith(Unreal.RootDirectory.FullName, StringComparison.Ordinal))
			{
			...
			else
			{
				...
				if (!FullPath.StartsWith(Unreal.RootDirectory.FullName, StringComparison.Ordinal))
				{
