...
namespace UnrealBuildTool
{
	internal class VSWorkspaceProjectFile : ProjectFile
	{
			...
			VCProjectFileFormat VCFormat)
		{
			...
			ProcessProjectFiles(
				...
				(UnrealTargetConfiguration Configuration, UnrealTargetPlatform Platform, ProjectTarget ProjectTarget) => Task.Run(async () =>
				{
					...
					try
					{
						foreach (UnrealArch Architecture in TargetDesc.Architectures.Architectures)
						{
					...
					catch (Exception Ex)
					{
						Logger.LogWarning(
							Ex,
							"Failed to generate include data for Target:{TargetName}, Platform: {Platform}, Configuration: {Configuration}",
							TargetDesc.Name,
							Platform.ToString(),
			...
			UEBuildTarget Target, UnrealArch? Architecture, bool bBuildByDefault, PlatformProjectGeneratorCollection PlatformProjectGenerators, ILogger Logger, VCProjectFileFormat VCFormat)
		{
			...
			foreach (UEBuildBinary Binary in Target.Binaries)
			{
				...
				foreach (UEBuildModuleCPP ModuleCpp in CandidateModules)
				{
					...
					foreach (DirectoryItem ModuleDirectory in ModuleCpp.ModuleDirectories)
					{
		...
		private static string GetVCPlatform(UEBuildTarget Target, UnrealArch? Architecture, PlatformProjectGeneratorCollection Generators, VCProjectFileFormat VCFormat)
		{
		...
		private static string GetVCConfiguration(UEBuildTarget Target, UnrealArch? Architecture, PlatformProjectGeneratorCollection Generators, VCProjectFileFormat VCFormat)
		{
			PlatformProjectGenerator? Generator = Generators.GetPlatformProjectGenerator(Target.Platform, true);
			if (Generator != null)
			{
		...
		private static string GetArchitecture(UEBuildTarget Target, UnrealArch? Architecture, PlatformProjectGeneratorCollection Generators, VCProjectFileFormat VCFormat)
		{
			PlatformProjectGenerator? Generator = Generators.GetPlatformProjectGenerator(Target.Platform, true);
			if (Generator != null)
			{
				...
				{
					string? result = Architecture.ToString();
