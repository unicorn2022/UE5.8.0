...
#pragma warning restore IDE0044
		...
		public VSWorkspaceProjectFileGenerator(
		...
		{
			Dictionary<string, AvailableConfiguration>? ProjectGenerationDescriptionData = ReadProjectGenerateDescriptionFilePath()?.Targets;
			...
			bool wroteVCProjectFiles = false;
			...
			for (int ProjectFileIndex = 0; ProjectFileIndex < ProjectsToGenerate.Count; ++ProjectFileIndex)
			{
				...
				foreach (ProjectTarget Target in CurrentProject.ProjectTargets.OfType<ProjectTarget>())
				{
		...
		private bool WriteAvailableConfigurations()
		{
			...
			for (int ProjectFileIndex = 0; ProjectFileIndex < GeneratedProjectFiles.Count; ++ProjectFileIndex)
			{
				...
				CurrentProject.CollectAvailableConfigurations(AvailableConfigurations, FilteredPlatforms, new HashSet<UnrealTargetConfiguration>(SupportedConfigurations).ToList());
			}
		...
		public override bool GenerateProjectFiles(PlatformProjectGeneratorCollection PlatformProjectGenerators, string[] Arguments, ILogger Logger)
		{
			...
			{
				return WriteAvailableConfigurations();
			}
			...
			ILogger Logger)
		{
			...
			{
				Logger.LogWarning(Ex, "Failed to write root project file");
				return false;
			}
		...
		private static readonly HashSet<string> NodesAllowList = new HashSet<string>
		{
			"CleanDependsOn",
			"Configuration",
			"CppCleanDependsOn",
		...
		private static readonly HashSet<string> NodesBlockedList = new HashSet<string>
		{
			"AdditionalOptions",
			"ClCompile",
			"ClInclude",
		...
		private static VCProjectFileFormat GetVCProjectFileFormat(ILogger Logger)
		{
		...
		private static bool IncludePlatform(AvailableConfiguration? AvailableConfiguration, UnrealTargetPlatform Platform, IList<UnrealTargetPlatform> FilteredPlatforms)
		{
		...
		private static bool IncludeConfiguration(AvailableConfiguration? AvailableConfiguration, UnrealTargetConfiguration Configuration, IList<UnrealTargetConfiguration> FilteredConfigurations)
		{
		...
		private static bool WriteVCProjectFileForTarget(
			...
			ILogger Logger)
		{
			...
			try
			{
				...
				VCProjectFileSettings Settings = new();
				Settings.MaxSharedIncludePaths = 0;

				VCProjectFile VCProject = new(Output, RootPath, Format, false, true, null, Settings, true);
				...
				if (VCProject.WriteProjectFile(FilteredPlatforms, FilteredConfigurations, PlatformProjectGenerators, Logger))
				{
					...
					while (Nodes.Count != 0)
					{
						...
						using (XmlNodeList NodeList = Node.ChildNodes)
						{
							if (NodeList != null)
							{
								foreach (XmlNode ChildNode in NodeList)
								{
									if (ChildNode != null && ChildNode.NodeType != XmlNodeType.Text)
									{
										...
										{
											foreach (Regex RegEx in BlockedNodesListRegEx)
											{
						...
						foreach (XmlNode ChildNode in RemovedChildNodes)
						{
			...
			{
				Logger.LogWarning(Ex, "Failed to create workspace project files. Disabling Visual Studio workspace MSBuild integration for target {TargetName}.", Target.Name);
		...
		private void WriteVCProjectCommonPropsFile(DirectoryReference RootPath, VCProjectFileFormat Format, ILogger Logger)
		{
			...
			try
			{
				...
				// Write the common and invalid configuration values
				{
					...
					if (OperatingSystem.IsWindows())
					{
						...
						{
							VCCommonTargetFileContent.AppendLine("    <DefaultSystemIncludePaths>{0}</DefaultSystemIncludePaths>", VCToolChain.GetVCIncludePaths(UnrealTargetPlatform.Win64, VCProjectFileGenerator.GetCompilerForIntellisense(Format), WindowsCompiler.Default, null, null, Logger));
						}
					}
					else
					{
			...
			{
				Logger.LogWarning(Ex, "Failed to create workspace project file {ProjectFilePath}.", Output.FullName);
			}
		}

		private static bool WriteVsConfigFile(ILogger Logger, DirectoryReference primaryProjectPath)
		{
