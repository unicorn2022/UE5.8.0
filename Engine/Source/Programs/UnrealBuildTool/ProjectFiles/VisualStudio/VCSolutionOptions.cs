...
#pragma warning disable CA1069 // Enums should not have duplicate values - must match https://learn.microsoft.com/en-us/windows/win32/stg/stgm-constants
		[Flags]
...
#pragma warning restore CA1069
		...
		{
			int HResult = StgOpenStorage(InputFileName, null, STGM.Direct | STGM.Read | STGM.ShareExclusive, IntPtr.Zero, 0, out IOleStorage? Storage);
			if (HResult != 0)
			{
				throw new COMException("Call to StgOpenStorage failed", HResult);
			}
		...
		{
			int HResult = StgCreateDocfile(OutputFileName, STGM.Direct | STGM.Create | STGM.Write | STGM.ShareExclusive, 0, out IOleStorage? OleStorage);
			if (HResult != 0)
			{
				throw new COMException("Call to StgCreateDocfile failed", HResult);
			}
	...
	class VCSolutionExplorerState
	{
		...
		private static void ReadEpilogue(BinaryReader Reader)
		{
		...
		private static void WriteEpilogue(BinaryWriter Writer)
		{
