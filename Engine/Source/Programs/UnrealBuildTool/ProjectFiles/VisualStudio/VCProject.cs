...
namespace UnrealBuildTool
{
	...
	abstract class MSBuildProjectFile : ProjectFile
	{
		...
		public const string VCProjectFileVersionString = "10.0.30319.1";
		...
		/// to Visual Studio
		public const string StubProjectConfigurationName = "BuiltWithUnrealBuildTool";
		...
		public const string StubProjectPlatformName = "Win64";
		...
		/// <param name="TryReadProjectGuid">Try read the project guid from already existsing file</param>
		public MSBuildProjectFile(FileReference InitFilePath, DirectoryReference BaseDir, bool TryReadProjectGuid)
			: base(InitFilePath, BaseDir)
		{
			...
			_projectGuid = new Lazy<Guid>(() =>
			{
				...
				if (TryReadProjectGuid && FileReference.Exists(ProjectFilePath))
				{
					try
					{
		...
		/// </summary>
		/// <param name="SolutionProjectParams">The solution project params to get a context for</param>
		...
		public abstract MSBuildProjectContext? GetMatchingProjectContext(
			VisualStudioPrimarySolutionProjectParams SolutionProjectParams,
		...
		readonly Lazy<Guid> _projectGuid;
	}
	...
	class VCProjectFile : MSBuildProjectFile
	{
		...
		readonly bool bMakeProjectPerTarget;
		...
		/// <param name="TryReadProjectGuid">Try read the project guid from already existsing file</param>
		public VCProjectFile(FileReference FilePath, DirectoryReference BaseDir, VCProjectFileFormat ProjectFileFormat, bool bUsePrecompiled, bool bMakeProjectPerTarget, string? BuildToolOverride, VCProjectFileSettings Settings, bool TryReadProjectGuid)
			: base(FilePath, BaseDir, TryReadProjectGuid)
		{
		...
		public override MSBuildProjectContext? GetMatchingProjectContext(
			VisualStudioPrimarySolutionProjectParams ProjectParams,
			PlatformProjectGeneratorCollection PlatformProjectGenerators,
			ILogger Logger)
		{
			...
			bool IsProgramProject = ProjectTargets.Any() && ProjectTargets[0].TargetRules != null && ProjectTargets[0].TargetRules!.Type == TargetType.Program;
			{
				...
				TargetType TargetConfigurationName = ProjectParams.ProjectTargetType;
				if (IsProgramProject)
				{
					TargetConfigurationName = TargetType.Program;
				}
				else if (ProjectParams.ProjectTargetType == TargetType.Program)
				{
				...
				{
					if (VCProjectFile.IsValidProjectPlatformAndConfiguration(ProjectTarget, ProjectParams.ProjectPlatform, ProjectParams.ProjectConfiguration))
					{
						if (ProjectTarget.TargetRules != null)
						{
				...
				if (MatchingProjectTargets.Count == 0 && ProjectParams.ProjectTargetType == TargetType.Editor && ProjectParams.ProjectPlatform == UnrealTargetPlatform.Win64)
				{
					foreach (Project ProjectTarget in ProjectTargets)
					{
				...
				if (MatchingProjectTargets.Count > 1)
				{
					throw new BuildException(
						"Not expecting more than one target for project {0} to match solution configuration {1} {2} {3}",
						ProjectFilePath,
						ProjectParams.ProjectTargetType,
						ProjectParams.ProjectConfiguration,
						ProjectParams.ProjectPlatform);
				}
				...
				if (MatchingProjectTargets.Count == 1)
				{
					...
					UnrealTargetConfiguration ProjectConfiguration = ProjectParams.ProjectConfiguration;
					if (MatchingProjectTarget.ForceDevelopmentConfiguration && ProjectParams.ProjectTargetType != TargetType.Game)
					{
						ProjectConfiguration = UnrealTargetConfiguration.Development;
					}

					// Get the matching project configuration
					UnrealTargetPlatform ProjectPlatform = ProjectParams.ProjectPlatform;
					if (IsStubProject)
					{
					...
					else
					{
						...
						ProjectConfigAndTargetCombination? Combination = ProjectConfigAndTargetCombinations.FirstOrDefault(
							ProjectPlatform == UnrealTargetPlatform.Android && ProjectParams.ProjectArchitecture == null ?
								...
								Combination =>
									...
									Combination.Configuration == ProjectConfiguration &&
									Combination.ProjectTarget == MatchingProjectTarget &&
									Combination.Architecture == ProjectParams.ProjectArchitecture);
					...
					if (ShouldBuildByDefaultForSolutionTargets)
					{
						...
						if (SupportedPlatforms == null || SupportedPlatforms.Contains(ProjectParams.ProjectPlatform))
						{
							bBuildByDefault = true;

							PlatformProjectGenerator? ProjGen = PlatformProjectGenerators.GetPlatformProjectGenerator(ProjectParams.ProjectPlatform, true);
							if (MatchingProjectTarget.ProjectDeploys ||
								((ProjGen != null) && (ProjGen.GetVisualStudioDeploymentEnabled(new VSSettings(ProjectPlatform, ProjectConfiguration, ProjectFileFormat, ProjectParams.ProjectArchitecture)) == true)))
							{
		...
		class ProjectConfigAndTargetCombination
		{
			...
			public readonly string ProjectPlatformName;
			public readonly string ProjectConfigurationName;
			public readonly ProjectTarget? ProjectTarget;
			...
			public override string ToString()
			{
				return $"{ProjectTarget} {Platform} {Configuration} {Architecture}".Trim();
			}
		}
		...
		public override void AddModuleForIntelliSense(UEBuildModuleCPP Module, CppCompileEnvironment CompileEnvironment)
		{
			...
			{
				foreach (DirectoryItem ModuleDirectory in Module.ModuleDirectories)
				{
		...
		private static string GetConformanceCompileArguments(TargetRules Target)
		{
		...
		bool PathIsFilteredOut(DirectoryReference InPath, ref string[]? FilteredList)
		{
			...
			if (FilteredList.Length > 0)
			{
				...
				sb.Append(InPath.FullName);
				sb.Append(Path.DirectorySeparatorChar);
				ReadOnlySpan<char> PathWithSeparator = sb.AsSpan();
		...
		void AppendIncludePaths(ref StackStringBuilder Builder, IncludePathsCollection Collection, HashSet<DirectoryReference> IgnorePaths)
		{
			foreach (DirectoryReference IncludePath in Collection.AbsolutePaths)
			{
				...
				{
					NormalizeProjectPath(ref Builder, IncludePath);
					Builder.Append(';');
				}
			}
		}
		...
		public override bool WriteProjectFile(List<UnrealTargetPlatform> InPlatforms, List<UnrealTargetConfiguration> InConfigurations, PlatformProjectGeneratorCollection PlatformProjectGenerators, ILogger Logger)
		{
			...
			bool bGenerateUserFileContent = PlatformProjectGenerators.PlatformRequiresVSUserFileGeneration(InPlatforms);
			...
			ConcurrentBag<AliasedFile> LocalAliasedFiles = new(AliasedFiles);
			...
			{
				Parallel2.ForEach(SourceFiles, CurFile =>
				{
					// get the filetype as represented to Visual Studio
					VCFileType FileType = GetVCFileType(CurFile.Reference.FullName);

					DirectoryReference FileSystemPathDir = CurFile.Reference.Directory;
					...
					AliasedFile LocalAliasedFile = new(CurFile.Reference, ProjectRelativeSourceFile, FilterRelativeSourceDirectory);
					LocalAliasedFiles.Add(LocalAliasedFile);
					...
					if (TryGetBuildEnvironment(Directory, out BuildEnvironment? BuildEnvironment))
					{
						...
						{
							Span<char> initial = stackalloc char[32*1024];
							StackStringBuilder Builder = new(initial);
							AppendIncludePaths(ref Builder, BuildEnvironment.UserIncludePaths, SharedIncludeSearchPathsSet);
							AppendIncludePaths(ref Builder, BuildEnvironment.SystemIncludePaths, SharedIncludeSearchPathsSet);
			...
			{
				if (!DirectoryToForceIncludePaths.IsEmpty)
				{
...

			{
				// Collapse common values
				{
					...
					void AddProperties(IDictionary<DirectoryReference, string> DirectoryToStringDict, string CommonPropertyPrefix, string PropertyNamePrefix, string PropertyValuePrefix)
					{
						Dictionary<string, string> UpdatedValues = new(StringComparer.Ordinal);
						...
						// Map each directory to a property
						{
							...
							StringBuilder PropertyNameBuilder = new(PropertyNamePrefix);
							int PropertyNameBuilderLength = PropertyNameBuilder.Length;
							string NextPropertyName = PropertyNameBuilder.ToString();
							...
							foreach (KeyValuePair<DirectoryReference, string> DirectoryKVP in KVPList)
							{
								...
								if (UpdatedValues.TryAdd(Value, NextPropertyName))
								{
									string PropertyName = NextPropertyName;
									PropertyNameBuilder.Append('_').Append(++PropertyIndex);
									NextPropertyName = PropertyNameBuilder.ToString();
									PropertyNameBuilder.Length = PropertyNameBuilderLength;
									DirectoryToStringDict[DirectoryKVP.Key] = PropertyName;
								}
								else
								{
						...
						Dictionary<string, string> ValueToCommonPropertyDict = new(StringComparer.Ordinal);
						{
							...
							CommonProperties.Sort(StringComparer.Ordinal);
						...
						foreach (KeyValuePair<string, string> kvp in UpdatedValues)
						{
							...
							Array.Sort(PathArray, String.CompareOrdinal);
					...
					// Use short names on these because there are _a ton_ of them in the big projects. the vcxproj of our biggest project went from 70mb to 31mb

					//AdditionalIncludeDirectories
					AddProperties(DirectoryToIncludeSearchPaths, "AID", "ClCompile_AdditionalIncludeDirectories", "$(NMakeIncludeSearchPath);");

					//ForcedIncludeFiles
					AddProperties(DirectoryToForceIncludePaths, "FIF", "ClCompile_ForcedIncludeFiles", String.Empty);
					...
					// AdditionalOptions
					{
						...
						StringBuilder PropertyNameBuilder = new("ClCompile_AdditionalOptions");
						int PropertyNameBuilderLength = PropertyNameBuilder.Length;
						string NextPropertyName = PropertyNameBuilder.ToString();
						...
						foreach (KeyValuePair<DirectoryReference, string?> DirectoryKVP in KVPList)
						{
							...
							{
								string PropertyName = NextPropertyName;
								PropertyNameBuilder.Append('_').Append(++ValueIndex);
								NextPropertyName = PropertyNameBuilder.ToString();
								PropertyNameBuilder.Length = PropertyNameBuilderLength;

								CommonProjectFileContent.AppendLine($"    <{PropertyName}>$(AdditionalOptions) /Yu\"{Value}\"</{PropertyName}>");
								DirectoryToPchFile[DirectoryKVP.Key] = PropertyName;
							}
							else
							{
				...
				AliasedFile[] LocalAliasedFilesArray = LocalAliasedFiles.ToArray();
				Array.Sort(LocalAliasedFilesArray);

				foreach (AliasedFile AliasedFile in LocalAliasedFilesArray)
				{
					...
					else
					{
						...
						if (TryGetBuildEnvironment(Directory, out BuildEnvironment? BuildEnvironment))
						{
							...
							{
								VCProjectFileContent.AppendLine("    <{0} Include=\"{1}\">", VCFileType, EscapeFileName(AliasedFile.FileSystemPath));
								VCProjectFileContent.Append(ClCompileInfo);
			...
			WriteTargets(ProjectPlatforms, PlatformProjectGenerators, VCProjectFileContent);
		...
		private static bool PatchVCUserFile(string FileName, string NewFileContents, VisualStudioUserFileSettings UserFileSettings, ILogger Logger)
		{
		...
		private class ProjectConfigurationForGenerator : ProjectBuildConfiguration
		{
			...
			private readonly ProjectConfigAndTargetCombination Combination;
			private readonly BuildCommandBuilder CommandBuilder;
		...
		private void WriteTargets(List<UnrealTargetPlatform> ProjectPlatforms, PlatformProjectGeneratorCollection PlatformProjectGenerators, StringBuilder VCProjectFileContent)
		{
			foreach (UnrealTargetPlatform Platform in ProjectPlatforms)
			{
				PlatformProjectGenerator? ProjGenerator = PlatformProjectGenerators.GetPlatformProjectGenerator(Platform, true);
		...
		private static bool EnsureFilterPathExists(string FilterRelativeSourceDirectory, StringBuilder VCFiltersFileContent, HashSet<string> FilterDirectories)
		{
			...
			if (!FilterDirectories.Contains(PathRemaining))
			{
				...
				for (; ; )
				{
					...
					{
						int SlashIndex = PathRemaining.IndexOf(Path.DirectorySeparatorChar, StringComparison.Ordinal);
		...
		private static bool CanVCFileTypeBeIncluded(VCFileType FileType)
		{
		...
		private static VCFileType GetVCFileType(string Path)
		{
		...
		public class BuildCommandBuilder
		{
			...
			public string GetBuildArguments(bool bAllowExtraTargets = true)
			{
				...
				if (!bUsePrecompiled && bAllowExtraTargets)
				{
...

			{
				...
				else
				{
					...
					else
					{
						...
						string BaseExeName = TargetRulesObject.Name;
					...
					if (ProjGenerator != null)
					{
						...
						VCProjectFileContent.Append(PathsStringBuilder);
					}
					...
					{
						VCProjectFileContent.AppendLine("    <NMakeOutput>{0}</NMakeOutput>", NormalizeProjectPath(NMakePath));
					}
					if (ProjectFileGenerator.bVisualStudioLinux)
					{
						VCProjectFileContent.AppendLine("    <BuildCommandLine>$(NMakeBuildCommandLine)</BuildCommandLine>");
					...
					string BuildArgumentsForFileCompiles = Builder.GetBuildArguments(bAllowExtraTargets: false);
					VCProjectFileContent.AppendLine("  <ItemDefinitionGroup {0}>", ConditionString);
					VCProjectFileContent.AppendLine("    <NMakeCompile>");
					VCProjectFileContent.AppendLine($"      <NMakeCompileFileCommandLine>$(BuildBatchScript) {BuildArgumentsForFileCompiles} -WorkingDir=$(MSBuildProjectDirectory) -Files=$(SelectedFiles)</NMakeCompileFileCommandLine>");
					VCProjectFileContent.AppendLine("    </NMakeCompile>");
