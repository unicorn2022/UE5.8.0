...
namespace UnrealBuildTool
{
	abstract class ProjectFile
	{
		...
		/// </summary>
		readonly Dictionary<FileReference, AliasedFile> AliasedFilesLookup = [];
		...
		private static void SplitDefinitionAndValue(string Definition, out string Key, out string Value)
		{
		...
		public virtual void AddModuleForIntelliSense(UEBuildModuleCPP Module, CppCompileEnvironment CompileEnvironment)
		{
			...
			foreach (DirectoryItem BaseDir in Module.ModuleDirectories)
			{
				BuildEnvironment? BuildEnvironment;
				if (!BaseDirToBuildEnvironment.TryGetValue(BaseDir.Location, out BuildEnvironment))
				{
					BuildEnvironment = new BuildEnvironment();
					BaseDirToBuildEnvironment.Add(BaseDir.Location, BuildEnvironment);
				}
		...
		public void AddIntelliSensePreprocessorDefinitions(List<string> NewPreprocessorDefinitions)
		{
			foreach (string NewPreprocessorDefinition in NewPreprocessorDefinitions)
			{
				...
				if (KnownIntelliSensePreprocessorDefinitions.Add(CurDef))
				{
					...
					SplitDefinitionAndValue(CurDef, out string Def, out _);
					...
					{
						SplitDefinitionAndValue(IntelliSensePreprocessorDefinitions[DefineIndex], out string ExistingDef, out _);
						if (ExistingDef == Def)
						{
		...
		public static void AddIntelliSenseIncludePaths(IncludePathsCollection Collection, IEnumerable<DirectoryReference> NewIncludePaths)
		{
		...
		/// Takes the given path and tries to rebase it relative to the project.
		/// </summary>
		public static void NormalizeProjectPath(ref StackStringBuilder outBuilder, FileSystemReference InputPath)
		{
			// Try to make it relative to the solution directory.
			if (InputPath.IsUnderDirectory(ProjectFileGenerator.PrimaryProjectPath))
			{
				InputPath.MakeRelativeTo(ref outBuilder, ProjectFileGenerator.IntermediateProjectFilesPath);
			}
			else
			{
				outBuilder.Append(InputPath.FullName);
			}
		}
		...
		/// </summary>
		private readonly HashSet<string> KnownIntelliSensePreprocessorDefinitions = new HashSet<string>(StringComparer.InvariantCultureIgnoreCase);
	}
}
