...
namespace UnrealBuildTool
{
	...
	{
		public const string DefaultPlatformConfigurationType = "Makefile";
		...
		public PlatformProjectGenerator(ILogger Logger)
		{
			this.Logger = Logger;
		}
		...
		readonly ConcurrentDictionary<int, bool> FoundVSISupportDict = new();
		...
		{
			int HashResult = 691;
			HashResult *= InVSSettings.Platform.ToString().GetHashCode(StringComparison.Ordinal);
			HashResult *= InVSSettings.Configuration.ToString().GetHashCode(StringComparison.Ordinal);
			HashResult *= InVSSettings.ProjectFileFormat.ToString().GetHashCode(StringComparison.Ordinal);
			HashResult *= InVSSettings.Architecture != null ? InVSSettings.Architecture.ToString()!.GetHashCode(StringComparison.Ordinal) : 1;
		...
		public class VisualStudioUserFileSettings
		{
			...
			private readonly HashSet<string> PropertiesToPatchContainer = new();
			private readonly HashSet<string> PropertiesToPatchOrderButPreserveValueContainer = new();
		}
