...
namespace UnrealBuildTool
{
	...
	class EddieProjectFile : ProjectFile
	{
		...
		public static EddieFolder? FindFolderByRelativePath(ref Dictionary<string, EddieFolder> Groups, string RelativePath)
		{
		...
		private void ParseSourceFilesIntoGroups()
		{
			foreach (SourceFile CurSourceFile in SourceFiles)
			{
				...
				if (Group != null)
				{
					if (FileName.EndsWith(".build.cs", System.StringComparison.Ordinal) ||
						FileName.EndsWith(".Build.cs", System.StringComparison.Ordinal) ||
						FileName.EndsWith(".csproj", System.StringComparison.Ordinal))
					{
		...
		private static void EmitProject(StringBuilder Content, Dictionary<string, EddieFolder> Folders, ILogger Logger)
		{
		...
		public override bool WriteProjectFile(List<UnrealTargetPlatform> InPlatforms, List<UnrealTargetConfiguration> InConfigurations, PlatformProjectGeneratorCollection PlatformProjectGenerators, ILogger Logger)
		{
			...
			return ProjectFileGenerator.WriteFileIfChanged(ProjectFilePath.FullName, ProjectFileContent.ToString(), Logger, new UTF8Encoding());
		}
	}
}
