...
namespace UnrealBuildTool
{
	...
	class CodeLiteGenerator : ProjectFileGenerator
	{
		...
		public CodeLiteGenerator(FileReference? InOnlyGameProject, CSharpProjectCache CSharpProjCache, CommandLineArguments CommandLine)
			...
			foreach (ProjectFile CurProject in AllProjectFiles)
			{
				...
				foreach (ProjectTarget CurrentTarget in CurProject.ProjectTargets.OfType<ProjectTarget>())
				{
					...
					if (ProjectFileFormat == CodeliteProjectFileFormat.CodeLite10)
					{
						...
						{
							if (ProjectName.Equals("UnrealClient", StringComparison.Ordinal) ||
								ProjectName.Equals("UnrealServer", StringComparison.Ordinal) ||
								ProjectName.Equals("UnrealGame", StringComparison.Ordinal) ||
								ProjectName.Equals("UnrealEditor", StringComparison.Ordinal))
							{
