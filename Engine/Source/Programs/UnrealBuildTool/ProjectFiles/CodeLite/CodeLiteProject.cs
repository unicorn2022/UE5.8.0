		...
		public override bool WriteProjectFile(List<UnrealTargetPlatform> InPlatforms, List<UnrealTargetConfiguration> InConfigurations, PlatformProjectGeneratorCollection PlatformProjectGenerators, ILogger Logger)
		{
			...
			foreach (Project target in ProjectTargets)
			{
				...
				foreach (SourceFile CurrentFile in FilterSourceFile)
				{
					...
					{
						if (TargetName.Equals("UnrealClient", StringComparison.Ordinal) ||
							TargetName.Equals("UnrealServer", StringComparison.Ordinal) ||
							TargetName.Equals("UnrealGame", StringComparison.Ordinal) ||
							TargetName.Equals("UnrealEditor", StringComparison.Ordinal))
						{
						...
						{
							int Idx = Path.GetDirectoryName(CurrentFile.Reference.FullName)!.IndexOf(ProjectNameRaw, StringComparison.Ordinal) + ProjectNameRaw.Length;
					...
					else if (ProjectTargetType == TargetType.Program)
					{
						...
						//
						int Idx = Path.GetDirectoryName(CurrentFile.Reference.FullName)!.IndexOf(TargetName, StringComparison.Ordinal) + TargetName.Length;
						CurrentFilePath = Path.GetDirectoryName(CurrentFile.Reference.FullName)!.Substring(Idx);
					}
				...
				foreach (UnrealTargetConfiguration CurConf in InConfigurations)
				{
					...
					if (ProjectTargetType == TargetType.Game)
					{
						if (CurConf.ToString().Contains("Debug", StringComparison.Ordinal))
						{
						...
						if (TargetName.Equals("UnrealGame", StringComparison.Ordinal))
						{
					...
					if (ProjectTargetType == TargetType.Game)
					{
						string CookGameCommandLine = "dotnet AutomationTool.dll BuildCookRun ";
