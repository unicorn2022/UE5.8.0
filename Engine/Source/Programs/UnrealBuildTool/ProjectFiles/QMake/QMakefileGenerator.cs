...
namespace UnrealBuildTool
{
	...
	{
		/// Default constructor
		public QMakefileGenerator(FileReference? InOnlyGameProject, CSharpProjectCache CSharpProjCache)
		...
		private static void AddIncludeDirectory(ref List<string> IncludeDirectories, string IncludeDir, string ProjectDir)
		{
			string FullProjectPath = ProjectFileGenerator.PrimaryProjectPath.FullName;
			string FullPath;
			if (IncludeDir.StartsWith('/') && !IncludeDir.StartsWith(FullProjectPath, StringComparison.Ordinal))
			{
		...
		private bool WriteQMakePro(ILogger Logger)
		{
			...
			// UnrealBuildTool
			// !RAKE: move to separate function
			...
			foreach (FileReference CurModuleFile in AllModuleFiles)
			{
				...
				foreach (FileReference CurSourceFile in FoundFiles)
				{
					...
					if (!SourceFileRelativeToRoot.Contains($"Source{Seperator}ThirdParty{Seperator}", StringComparison.Ordinal))
					{
						...
						if (SourceFileRelativeToRoot.EndsWith(".h", StringComparison.Ordinal))
						{
							if (!SourceFileRelativeToRoot.StartsWith("..", StringComparison.Ordinal) && !Path.IsPathRooted(SourceFileRelativeToRoot))
							{
						...
						if (SourceFileRelativeToRoot.EndsWith(".cs", StringComparison.Ordinal))
						{
							if (!SourceFileRelativeToRoot.StartsWith("..", StringComparison.Ordinal) && !Path.IsPathRooted(SourceFileRelativeToRoot))
							{
								// SourceFileRelativeToRoot = "Engine/" + SourceFileRelativeToRoot;
