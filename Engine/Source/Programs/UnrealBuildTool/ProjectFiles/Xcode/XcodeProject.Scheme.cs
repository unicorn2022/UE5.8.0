	...
	static class XcodeSchemeFile
	{
		...
		public static void WriteSchemeFile(UnrealData UnrealData, UnrealTargetPlatform? Platform, string ProjectName, List<XcodeRunTarget> RunTargets, string? BuildTargetGuid, string? IndexTargetGuid)
		{
			...
			foreach (XcodeRunTarget RunTarget in RunTargets)
			{
				...
				FileReference lldbInitFile = FileReference.Combine(SchemeFilePath.Directory, "LLDBInit");

				List<string> sourceMappings = [];
				if (UnrealData.RootPaths != null)
				{
					foreach ((uint id, DirectoryReference vfs, DirectoryReference local) in UnrealData.RootPaths)
					{
						sourceMappings.Add($"settings append target.source-map \"{vfs}\" \"{local}\"");
					}

					FileReference.WriteAllLines(lldbInitFile, sourceMappings);
				}
				...
				Lines = Lines.Select(x => x.Replace("\"", "&quot;", StringComparison.Ordinal).Replace("&&", "&amp;&amp;", StringComparison.Ordinal));
				...
				{
					Content.WriteLine("      customLLDBInitFile = \"" + lldbInitFile + "\"");
				}
				Content.WriteLine("      selectedDebuggerIdentifier = \"Xcode.DebuggerFoundation.Debugger.LLDB\"");
				Content.WriteLine("      selectedLauncherIdentifier = \"Xcode.DebuggerFoundation.Launcher.LLDB\"");
				...
				Content.WriteLine("        <AdditionalOption");
				Content.WriteLine("          key = \"APP_DISTRIBUTOR_ID_OVERRIDE\"");
				Content.WriteLine("          value = \"com.apple.AppStore\"");
