...
namespace UnrealBuildTool.ProjectFiles.Xcode
{
	...
	{
		private const string PROJECT_FILE_SEARCH_EXPRESSION = "*.pbxproj";
		private const string TEMPLATE_NAME = "FrameworkWrapper";
		...
		private const string TEMPLATE_PROJECT_NAME = "PROJECT_NAME";
		private const string COMMANDLINE_FILENAME = "uecommandline.txt";
		...
		private static void CopyAll(string SourceDirectory, string DestinationDirectory)
		{
			...
			{
				string DirDst = DirSrc.Replace(SourceDirectory, DestinationDirectory, StringComparison.Ordinal);
			...
			{
				string FileDst = FileSrc.Replace(SourceDirectory, DestinationDirectory, StringComparison.Ordinal);
		...
		private static void RenameFilesAndDirectories(string RootDirectory, string OldValue, string NewValue)
		{
			...
			{
				string NewEntryName = Path.GetFileName(Entry).Replace(OldValue, NewValue, StringComparison.Ordinal);
		...
		private static void ReplaceTextInFiles(string RootDirectory, string OldValue, string NewValue, string SearchPattern = "*")
		{
			...
			foreach (string SrcFile in Files)
			{
				...
				FileContents = FileContents.Replace(OldValue, NewValue, StringComparison.Ordinal);
	...
	class XcodeFrameworkWrapperUtils
	{
		...
		public static string GetBundleID(DirectoryReference ProjectDirectory, FileReference? ProjectFile)
		{
			...
			BundleId = BundleId.Replace("[PROJECT_NAME]", ProjectName, StringComparison.Ordinal).Replace("_", "", StringComparison.Ordinal);
			return BundleId;
		}
		...
		public static string GetBundleName(DirectoryReference ProjectDirectory, FileReference? ProjectFile)
		{
			...
			string ProjectName = (ProjectFile != null) ? ProjectFile.GetFileNameWithoutAnyExtensions() : "UnrealGame";
			BundleName = BundleName.Replace("[PROJECT_NAME]", ProjectName, StringComparison.Ordinal).Replace("_", "", StringComparison.Ordinal);
			return BundleName;
		}
