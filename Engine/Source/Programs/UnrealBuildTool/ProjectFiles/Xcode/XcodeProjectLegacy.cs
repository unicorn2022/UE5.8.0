...
namespace UnrealBuildTool.XcodeProjectLegacy
{
	...
	class XcodeProjectFile : ProjectFile
	{
		...
		/// </summary>
		readonly UnrealArch[] SupportedIOSArchitectures = { UnrealArch.Arm64 };
		...
		private static bool ShouldIncludeFileInBuildPhaseSection(XcodeSourceFile SourceFile)
		{
			...
			if (IsSourceCode(FileExtension))
			{
				...
				{
					string AltName = PlatformName == "Win64" ? "windows" : PlatformName;
					if ((SourceFile.Reference.FullName.Contains($"/{PlatformName}/", StringComparison.OrdinalIgnoreCase) || SourceFile.Reference.FullName.Contains($"/{AltName}/", StringComparison.OrdinalIgnoreCase))
		...
		/// </summary>
		private static XcodeFileGroup? FindGroupByAbsolutePath(ref Dictionary<string, XcodeFileGroup> Groups, string AbsolutePath)
		{
			...
			for (int Index = 1; Index < Parts.Length; ++Index)
			{
				...
				{
					CurrentGroup = new XcodeFileGroup(Path.GetFileName(CurrentPath), CurrentPath, CurrentPath.EndsWith(".xcassets", StringComparison.Ordinal));
		...
		{
			return InPath.Replace("\\", "/", StringComparison.Ordinal);
		}
		...
		{
			SourceFiles.Sort((x, y) => { return x.Reference.CompareTo(y.Reference); });
		...
		private static void AppendGroup(XcodeFileGroup Group, StringBuilder Content)
		{
		...
		private static void AppendBuildFileSection(StringBuilder Content, StringBuilder SectionContent)
		{
		...
		private static void AppendFileReferenceSection(StringBuilder Content, StringBuilder SectionContent)
		{
		...
		private static void AppendSourcesBuildPhaseSection(StringBuilder Content, StringBuilder SectionContent, string SourcesBuildPhaseGuid)
		{
		...
		private static void AppendCopyExtensionsBuildPhaseSection(StringBuilder Content, StringBuilder SectionContent, string CopyFilesBuildPhaseGuid)
		{
		...
		private static void AppendLegacyTargetSection(StringBuilder Content, string TargetName, string TargetGuid, string TargetBuildConfigGuid, FileReference? UProjectPath)
		{
		...
		private static void AppendRunTargetSection(StringBuilder Content, string TargetName, string TargetGuid, string TargetBuildConfigGuid, string TargetDependencyGuid,
		...
		private static void AppendGenericTargetSection(StringBuilder Content, string TargetName, string TargetGuid, string TargetType, string TargetBuildConfigGuid, string TargetAppGuid, IEnumerable<string>? TargetDependencyGuids, Dictionary<string, string> BuildPhases)
		{
		...
		private static void AppendIndexTargetSection(StringBuilder Content, string TargetName, string TargetGuid, string TargetBuildConfigGuid, string SourcesBuildPhaseGuid)
		{
			Content.Append("/* Begin PBXNativeTarget section */" + ProjectFileGenerator.NewLine);
		...
		private static void AppendShellScriptSection(StringBuilder Content, string ShellScriptGuid, FileReference? UProjectPath)
		{
		...
		private static void AppendExtensionTargetSections(StringBuilder ProjectFileContent, List<XcodeExtensionInfo> AllExtensions)
		{
			foreach (XcodeExtensionInfo EI in AllExtensions)
			{
				Dictionary<string, string> BuildPhases = new Dictionary<string, string>();
		...
		private static void AppendContainerItemProxySection(StringBuilder Content, string TargetName, string TargetGuid, string TargetProxyGuid, string ProjectGuid)
		{
		...
		private static void AppendTargetDependencySection(StringBuilder Content, string TargetName, string TargetGuid, string TargetDependencyGuid, string TargetProxyGuid)
		{
		...
		private void AppendProjectBuildConfiguration(StringBuilder Content, string ConfigName, string ConfigGuid)
		{
			...
			{
				Content.Append("\t\t\t\t\t\"" + Definition.Replace("\"", "", StringComparison.Ordinal).Replace("\\", "", StringComparison.Ordinal) + "\"," + ProjectFileGenerator.NewLine);
			}
			...
			{
				string Path = SearchPath.Contains(' ', StringComparison.Ordinal) ? "\\\"" + SearchPath + "\\\"" : SearchPath;
		...
		readonly Dictionary<string, UnrealArchitectures> CachedMacProjectArcitectures = new();
		...
		private void AppendPlatformConfiguration(StringBuilder Content, XcodeBuildConfig Config, string TargetName, FileReference? ProjectFile, bool bSupportMac, bool bSupportIOS, bool bSupportTVOS, out string? IOSRunTimeVersion, out string? TVOSRunTimeVersion, string BinariesSubDir = "/Payload")
		{
			...
			bool bIsUnrealGame = TargetName.Equals("UnrealGame", StringComparison.OrdinalIgnoreCase);
			bool bIsUnrealClient = TargetName.Equals("UnrealClient", StringComparison.OrdinalIgnoreCase);
			...
			if (bSupportMac)
			{
				...
				string MacBundleIdentifier = bBuildingEditor ? ("com.epicgames.UnrealEditor") : (IOSBundleIdentifier.Replace("[PROJECT_NAME]", TargetName, StringComparison.Ordinal).Replace("_", "", StringComparison.Ordinal));
		...
		private void AppendNativeTargetBuildConfiguration(StringBuilder Content, XcodeBuildConfig Config, string ConfigGuid, FileReference? ProjectFile, ILogger Logger)
		{
			...
			bool bIsUnrealGame = Config.BuildTarget.Equals("UnrealGame", StringComparison.OrdinalIgnoreCase);
			bool bIsUnrealClient = Config.BuildTarget.Equals("UnrealClient", StringComparison.OrdinalIgnoreCase);
			...
			if (BuildHostPlatform.Current.Platform == UnrealTargetPlatform.Mac)
			{
				...
				if (bCreateMacInfoPlist || bCreateIOSInfoPlist || bCreateTVOSInfoPlist)
				{
					...
					if (bCreateTVOSInfoPlist)
					{
						...
						AppleOnWindowsAppMaker.GenerateTVOSPList(ProjectPath.FullName, bIsUnrealGame, GameName, bIsClient, Config.BuildTarget, EngineDir.FullName, ProjectPath + "/Binaries/TVOS/Payload", BundleIdentifier);
					}
				}
			}
		...
		private static void AppendXCConfigurationList(StringBuilder Content, string TypeName, string TargetName, string ConfigListGuid, Dictionary<string, XcodeBuildConfig> BuildConfigs, string Default = "Development")
		{
		...
		private static void AppendXCConfigurationListSection(StringBuilder Content, string TargetName, string BuildTargetName, string IndexTargetName, string ProjectConfigListGuid,
									...
									if (MSBuildProjectFile.IsValidProjectPlatformAndConfiguration(ProjectTarget, Platform, Configuration))
									{
										...
										if (BuildConfigs.Where(Config => Config.DisplayName == ConfigName).ToList().Count == 0)
										{
											...
											if (BuildPlatform.Platform == UnrealTargetPlatform.Mac)
											{
												...
												string IOSExecutableName = MacExecutableName.Replace("-Mac-", "-IOS-", StringComparison.Ordinal);
												string TVOSExecutableName = MacExecutableName.Replace("-Mac-", "-TVOS-", StringComparison.Ordinal);
											...
											else if (BuildPlatform.Platform == UnrealTargetPlatform.IOS || BuildPlatform.Platform == UnrealTargetPlatform.TVOS)
											{
												...
												string TVOSExecutableName = IOSExecutableName.Replace("-IOS-", "-TVOS-", StringComparison.Ordinal);
												//string MacExecutableName = IOSExecutableName.Replace("-IOS-", "-Mac-", StringComparison.Ordinal);
		...
		private void WriteSchemeFile(string TargetName, string TargetGuid, string BuildTargetGuid, string IndexTargetGuid, bool bHasEditorConfiguration, string GameProjectPath)
		{
			...
			if (FileReference.Exists(SchemeFilePath))
			{
				...
				int OldCommandLineArgumentsStart = OldContents.IndexOf("<CommandLineArguments>", StringComparison.Ordinal) + "<CommandLineArguments>".Length;
				int OldCommandLineArgumentsEnd = OldContents.IndexOf("</CommandLineArguments>", StringComparison.Ordinal);
