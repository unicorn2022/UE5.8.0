...
namespace UnrealBuildTool.XcodeProjectXcconfig
{
	...
	class XcodeFileCollection
	{
		...
		static readonly Dictionary<string, string> FileTypeMap = new()
		{
			...
			{ ".ush", "sourcecode.metal" },
			{ ".usf", "sourcecode.metal" }
		};
		...
		private readonly List<ManualFileReference> ManualFiles = new();
		private readonly Dictionary<string, string> GuidsForGroups = new();
			...
			if (bIsForBuild)
			{
				...
				if (IsSourceCode(FileExtension))
				{
					...
					if (BuildFileOffset != -1)
					{
						...
						bool isExtension = File.Reference.FullName.Contains("Extensions", StringComparison.Ordinal);
		...
		public void Write(StringBuilder Content, UnrealTargetPlatform? Platform)
		{
			...
			foreach (XcodeSourceFile File in AllFiles)
			{
				...
				if (FileName.Contains("UserNotifications.framework", StringComparison.Ordinal))
				{
		...
		/// </summary>
		static readonly string[] ExcludedFolders = PlatformExports.GetExcludedFolderNames(UnrealTargetPlatform.Mac);
		...
		internal XcodeFileGroup? FindGroupByAbsolutePath(DirectoryReference DirectoryPath)
		{
			...
			for (int Index = 1; Index < Parts.Length; ++Index)
			{
				...
				if (!CurrentSubGroups.ContainsKey(CurrentPath))
				{
					...
					CurrentGroup = new XcodeFileGroup(Path.GetFileName(CurrentPath), CurrentPath, GetFileType(Path.GetExtension(CurrentPath)).StartsWith("folder", StringComparison.Ordinal));
					if (Groups.Count == 0)
					{
