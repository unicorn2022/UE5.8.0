...
namespace UnrealBuildTool
{
	...
	{
		public KDevelopGenerator(FileReference? InOnlyGameProject, CSharpProjectCache CSharpProjCache)
		...
		private static void WriteKDevPrimaryProjectSection(ref StringBuilder FileContent, string Name)
		{
			FileContent.Append('\n');
		...
		private void WriteIncludeSection(ref StringBuilder FileContent, ILogger Logger)
		{
			...
			foreach (ProjectFile CurProject in GeneratedProjectFiles)
			{
				...
				foreach (string CurPath in KDevelopProject.IntelliSenseIncludeSearchPaths)
				{
					...
					if (CurPath.StartsWith('/') && !CurPath.StartsWith(FullProjectPath, StringComparison.Ordinal))
					{
					...
					if (!FullPath.Contains("FortniteGame/", StringComparison.Ordinal) && !FullPath.Contains("ThirdParty/", StringComparison.Ordinal))
					{
				...
				foreach (string CurPath in KDevelopProject.IntelliSenseSystemIncludeSearchPaths)
				{
					...
					if (!FullPath.Contains("FortniteGame/", StringComparison.Ordinal) && !FullPath.Contains("ThirdParty/", StringComparison.Ordinal)) // @todo: skipping Fortnite header paths to shorten clang command line for building
					{
						SystemIncludeDirectories.Add(String.Format("{0}", FullPath));
						IncludeIndex++;
					}
			...
			List<string> Tmp = IncludeDirectories.Distinct().ToList();
			List<string> Stmp = SystemIncludeDirectories.Distinct().ToList();
		...
		/// </summary>
		/// <param name="FileContent">File content.</param>
		private static void WriteExcludeSection(ref StringBuilder FileContent)
		{
