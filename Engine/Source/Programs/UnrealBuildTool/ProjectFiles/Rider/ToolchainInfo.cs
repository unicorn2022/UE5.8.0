	...
	sealed class ToolchainInfo : IEquatable<ToolchainInfo>
	{
		...
		public override int GetHashCode()
		{
			unchecked
			{
				...
				HashCode = (HashCode * 397) ^ (Architecture != null ? Architecture.GetHashCode(StringComparison.Ordinal) : 0);
				HashCode = (HashCode * 397) ^ (Configuration != null ? Configuration.GetHashCode(StringComparison.Ordinal) : 0);
				...
				HashCode = (HashCode * 397) ^ (PrecompiledHeaderAction != null ? PrecompiledHeaderAction.GetHashCode(StringComparison.Ordinal) : 0);
				HashCode = (HashCode * 397) ^ (PrecompiledHeaderFile != null ? PrecompiledHeaderFile.GetHashCode(StringComparison.Ordinal) : 0);
				HashCode = (HashCode * 397) ^ (ForceIncludeFiles != null ? ForceIncludeFiles.GetHashCode() : 0);
				HashCode = (HashCode * 397) ^ (Compiler != null ? Compiler.GetHashCode(StringComparison.Ordinal) : 0);
