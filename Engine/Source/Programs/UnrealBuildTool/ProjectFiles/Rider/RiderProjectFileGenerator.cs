...
#pragma warning restore IDE0044
		...
		{
			SetupSupportedPlatformsAndConfigurations(true, Logger, out _);
		}
			...
			List<FileReference> AllGameProjects, ILogger Logger)
		{
			...
			AddProjectsForAllTargets(PlatformProjectGenerators, AllGameProjects, Logger,
				out ProjectFile? EngineProject, out List<ProjectFile>? GameProjects, out _);
			...
			PlatformProjectGeneratorCollection PlatformProjectGenerators, ILogger Logger)
		{
			try
			{
				...
				List<VisualStudioPrimarySolutionProjectParams> VcxSolutionProjectParamList;
				VCProjectFileGenerator.CollectSolutionConfigurations(SupportedConfigurations,
					SupportedPlatforms, AllProjectFiles, bMakeProjectPerTarget, out ValidPlatforms, out VcxSolutionProjectParamList);
				...
				using (JsonWriter Writer = new JsonWriter(OutputFile))
				{
					...
					foreach (VisualStudioPrimarySolutionProjectParams Params in VcxSolutionProjectParamList)
					{
						Writer.WriteObjectStart();
						Writer.WriteValue("Configuration", Params.ProjectConfiguration.ToString());
						Writer.WriteValue("Platform", Params.ProjectPlatform.ToString());
						Writer.WriteValue("Target", Params.ProjectTargetType.ToString());
