...
namespace UnrealBuildTool
{
	...
	class UEBuildBinary
	{
		...
		/// </summary>
		private readonly Lazy<List<FileReference>> DependentLinkLibraries;
			...
			)
		{
			...
			DependentLinkLibraries = new(() =>
			{
				List<FileReference> libs = new ();
				foreach (FileReference OutputFilePath in OutputFilePaths)
				{
					FileReference LibraryFileName;
					if (Type == UEBuildBinaryType.StaticLibrary || !PrimaryModule.Rules.Target.Platform.IsInGroup(UnrealPlatformGroup.Microsoft))
					{
		...
		public List<FileItem> Build(ReadOnlyTargetRules Target, UEToolChain ToolChain, CppCompileEnvironment CompileEnvironment, LinkEnvironment LinkEnvironment, ISourceFileWorkingSet WorkingSet, DirectoryReference ExeDir, IActionGraphBuilder Graph, ILogger Logger)
		{
			...
			// If target type is monolithic and we are building a dll it means that this link step will output final binaries which is a dynamic library
			// In this case we also want additional files (.lib etc) to end up in the same directory as the binary
			if (Target.LinkType == TargetLinkType.Monolithic && BinaryLinkEnvironment.bIsBuildingDLL)
			{
				BinaryLinkEnvironment.LocalShadowDirectory = BinaryLinkEnvironment.OutputFilePath.Directory;
			}
		...
		{
			DependentLinkEnvironment.Libraries.AddRange(DependentLinkLibraries.Value);
		}
		...
		public IEnumerable<UEBuildModule> GetAllDependencyModules(bool bIncludeDynamicallyLoaded, bool bForceCircular)
		{
			if (Modules.Count == 1 && bIncludeDynamicallyLoaded && bForceCircular)
			{
				UEBuildModule First = Modules[0];
				if (First.DependenciesForStableTopologicalSort != null)
				{
					return First.DependenciesForStableTopologicalSort;
				}
			}
		...
		public void FindModuleReferences(Dictionary<UEBuildModule, UEBuildModule?> ReferencedBy)
		{
			...
			using Pooled<HashSet<UEBuildModule>> rented = HashSetPool<UEBuildModule>.Rent();
			HashSet<UEBuildModule> VisitedModules = rented.Value;
		...
		public static FileReference GetAdditionalConsoleAppPath(FileReference BinaryPath)
		{
			...
			if (Directory.FullName.EndsWith(".app/Contents/MacOS", StringComparison.Ordinal))
			{
			...
			foreach (FileReference OutputFilePath in OutputFilePaths)
			{
				...
				foreach (RestrictedFolder BinaryFolder in BinaryFolders)
				{
					...
					if (PrimaryModule.AliasRestrictedFolders.TryGetValue(BinaryFolder.ToString(), out Alias))
					{
						...
						{
							if (Folder.ToString().Equals(Alias, StringComparison.Ordinal))
							{
		...
		private static bool IsBuildingDll(UEBuildBinaryType Type)
		{
			return Type == UEBuildBinaryType.DynamicLinkLibrary;
		}

		private static bool IsBuildingLibrary(UEBuildBinaryType Type)
		{
		...
		private LinkEnvironment SetupBinaryLinkEnvironment(ReadOnlyTargetRules Target, UEToolChain ToolChain, LinkEnvironment LinkEnvironment, CppCompileEnvironment CompileEnvironment, ISourceFileWorkingSet WorkingSet, DirectoryReference ExeDir, IActionGraphBuilder Graph, ILogger Logger)
		{
			...
			using Pooled<HashSet<UEBuildModule>> rented = HashSetPool<UEBuildModule>.Rent();
			HashSet<UEBuildModule> LinkEnvironmentVisitedModules = rented.Value;
			...
			using Pooled<HashSet<FileItem>> rented2 = HashSetPool<FileItem>.Rent();
			HashSet<FileItem> InputFilesLookup = rented2.Value;

			List<UEBuildModule> CompiledModules = [];
			...
			List<FileItem>[] LinkInputFilesArray = new List<FileItem>[CompiledModules.Count];

			// Compile call is expensive so do it in parallel
			Parallel2.For(0, CompiledModules.Count, index =>
			{
				LinkInputFilesArray[index] = CompiledModules[index].Compile(Target, ToolChain, BinaryCompileEnvironment, WorkingSet, Graph, Logger);
			});

			for (int index = 0, e = CompiledModules.Count; index != e; ++index)
			{
				UEBuildModule Module = CompiledModules[index];
				...
				List<FileItem> LinkInputFiles = LinkInputFilesArray[index];
				...
				{
					BinaryLinkEnvironment.IncludeFunctions.Add($"IMPLEMENT_MODULE_{Module.Name}");
				}
			...
			{
				VisitedModule.LinkDebuggerVisualizers(BinaryLinkEnvironment.DebuggerVisualizerFiles, ToolChain);
			}

			// Set the link output file.
			BinaryLinkEnvironment.OutputFilePaths = [.. OutputFilePaths];
			...
			// Compile any resource files
			CompileResourceFiles(ToolChain, BinaryCompileEnvironment, BinaryLinkEnvironment, [.. CompiledModules.SelectMany(x => x.ResourceFiles)], Graph);
		...
		private void CompileResourceFiles(UEToolChain ToolChain, CppCompileEnvironment BinaryCompileEnvironment, LinkEnvironment BinaryLinkEnvironment, IEnumerable<FileItem> InputFiles, IActionGraphBuilder Graph)
		{
			...
			IEnumerable<FileItem> DefaultResourceFiles = [];
			...
			FileItem PCLaunchResource = FileItem.GetItemByFileReference(FileReference.Combine(Unreal.EngineSourceDirectory, "Runtime", "Launch", "Resources", "Windows", "PCLaunch.rc"));
			if (!InputFiles.Any(x => x != PCLaunchResource))
			{
				if (BinaryLinkEnvironment.DefaultResourceFiles.Count > 0 && (BinaryLinkEnvironment.bIsBuildingDLL || BinaryLinkEnvironment.bIsBuildingLibrary))
				{
					// If no resource files are found, use the precompiled default resource file for the target
					if (!InputFiles.Any())
					{
						BinaryLinkEnvironment.InputFiles.AddRange(BinaryLinkEnvironment.DefaultConvertedResourceFiles);
						return;
					}
					// If this module has other resource files, add the shared compiled resource files to be converted as well
					DefaultResourceFiles = BinaryLinkEnvironment.DefaultResourceFiles;
				}
				else
				{
					...
					{
						FileItem ExecutableResourceFile = FileItem.GetItemByFileReference(FileReference.Combine(IntermediateDirectory, $"{OutputFilePaths[0].GetFileNameWithoutAnyExtensions()}-{DefaultResourceFile.Location.GetFileName()}"));
						Graph.CreateCopyAction(DefaultResourceFile, ExecutableResourceFile);
						DefaultResourceFile = ExecutableResourceFile;
					}
					InputFiles = [.. InputFiles, DefaultResourceFile];
				}
			}

			// Create a compile environment for resource files
			CppCompileEnvironment ResourceCompileEnvironment = new(BinaryCompileEnvironment);
			...
			List<FileItem> ObjectFiles = ToolChain.CompileRCFiles(ResourceCompileEnvironment, InputFiles, IntermediateDirectory, Graph).ObjectFiles;

			// Optionally convert the resource files depending on toolchain settings
			BinaryLinkEnvironment.InputFiles.AddRange(ToolChain.ConvertRCFiles(BinaryLinkEnvironment, [.. ObjectFiles, .. DefaultResourceFiles], IntermediateDirectory, Graph).ObjectFiles);
		}
