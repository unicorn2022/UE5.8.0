...
namespace UnrealBuildTool
{
	...
	public partial class CppCompileWarnings
	{
		...
		/// <summary>
		/// Indicates what warning/error level to treat unreachable code.
		/// MSVC - 
		///		https://learn.microsoft.com/en-us/cpp/error-messages/compiler-warnings/compiler-warning-level-4-c4702 
		///		4702 - unreachable code
		///		44702 - Note: The extra 4 is not a typo, /wLXXXX sets warning XXXX to level L
		/// Clang - 
		///		https://clang.llvm.org/docs/DiagnosticsReference.html#wunreachable-code-aggressive
		/// </summary>
		[XmlConfigFile(Category = "BuildConfiguration")]
		[CommandLine("-UnreachableCodeWarningLevel=")]
		[WarningsVCToolChainAttribute(null, ["/w44702"], ["/we4702"])]
		[WarningsClangToolChain(null, ["-Wunreachable-code-aggressive", "-Wno-error=unreachable-code-aggressive"], ["-Wunreachable-code-aggressive"])]
		[VersionWarningLevelDefault(WarningLevel.Off, BuildSettingsVersion.V1, BuildSettingsVersion.V6, InitializationContext.Any)]
		[VersionWarningLevelDefault(WarningLevel.Error, BuildSettingsVersion.V7, BuildSettingsVersion.Latest, InitializationContext.Any)]
			public WarningLevel UnreachableCodeWarningLevel
		{
			get => ResolveWarning(ParentCppCompileWarnings?.UnreachableCodeWarningLevel, _unreachableCodeWarningLevel);
			set => _unreachableCodeWarningLevel = value;
		}
		private WarningLevel _unreachableCodeWarningLevel;
		...
		/// </remarks>
		[WarningsClangToolChain(["-Wno-nan-infinity-disabled"], ["-Wnan-infinity-disabled", "-Wno-error=nan-infinity-disabled"], ["-Wnan-infinity-disabled"], [nameof(FilterID.Version18Point1Min)])]
		[BasicWarningLevelDefault(WarningLevel.Off)]
		...
		[WarningsVCToolChain(null, ["/w44191"], ["/we4191"])]
		[BasicWarningLevelDefault(WarningLevel.Off)]
		...
		[WarningsMSCV(["/wd4100"], ["/w44100"], ["/we4100"])]
		...
		private static readonly Lazy<List<(MemberInfo, List<ApplyWarningsAttribute>)>> s_membersWithApplyWarningsAttribute = new(() => GetMembersWithAttributes<ApplyWarningsAttribute>(typeof(CppCompileWarnings), typeof(WarningLevel)));
		private static readonly Lazy<List<(MemberInfo, List<WarningLevelDefaultAttribute>)>> s_membersWithWarningLevelDefaultAttribute = new(() => GetMembersWithAttributes<WarningLevelDefaultAttribute>(typeof(CppCompileWarnings), typeof(WarningLevel)).ToList());
		private static readonly ConcurrentDictionary<MemberInfo, Lazy<Dictionary<Type, List<WarningLevelDefaultAttribute>>>>[] s_groupedAttributes = { [], [], [] };
		...
		internal IEnumerable<string> GenerateWarningCommandLineArgs(CppCompileEnvironment compileEnvironment, System.Type toolChainType, VersionNumber? toolChainVersion = null, StaticAnalyzer analyzer = StaticAnalyzer.None)
		{
			...
			foreach ((MemberInfo mi, List<ApplyWarningsAttribute> miAttr) in s_membersWithApplyWarningsAttribute.Value)
			{
				ApplyWarningsAttribute? applyWarningsAttribute = null;

				bool isFirst = true;

				// We only want to iterate this enumerator once since it does quite a lot of work. Also, this is called a lot so avoid Linq
				foreach (ApplyWarningsAttribute? attr in miAttr)
				{
					if (!attr.CanApplyToContext(toolChainContext))
					{
						continue;
					}
					...
					if (isFirst)
					{
						applyWarningsAttribute = attr;
						isFirst = false;
					}
					else
					{
						applyWarningsAttribute = null;
						_logger?.LogWarning("Ambiguous ApplyWarningsAttributes on the property ({PropertyName}), for the provided tool-chain ({ToolChainName}). Skipping this property.", mi.Name, toolChainContext._toolChainType.Name);
						break;
					}
				}
				...
				{
					WarningLevel instanceWarningLevel = GetCurrentWarningLevel(mi, this);
		...
		internal static void ApplyTargetDefaults(CppCompileWarnings cppCompileWarnings, bool overwriteSourceValue = false)
		{
			...
			foreach ((MemberInfo mi, List<WarningLevelDefaultAttribute> miAttr) in s_membersWithWarningLevelDefaultAttribute.Value)
			{
				WarningLevel defaultWarningLevel = WarningLevel.Default;
				defaultWarningLevel = CoalesceDefaultWarningFromMemberInfo(mi, miAttr, InitializationContext.Target, defaultWarningLevel, cppCompileWarnings.BuildContextProvider, cppCompileWarnings._logger);
		...
		internal static WarningLevel CoalesceDefaultWarningFromMemberInfo(MemberInfo mi, List<WarningLevelDefaultAttribute> miAttr, InitializationContext defaultWarningContext, WarningLevel defaultWarningLevel, BuildSystemContext? buildSystemContext, ILogger? logger)
		{
			...
			{
				IList<WarningLevelDefaultAttribute> attributes = [.. miAttr.Where(prop => prop.Context == InitializationContext.Any || prop.Context == defaultWarningContext)];
		...
		private void ApplyDefaults()
		{
			...
			{
				WarningLevel defaultWarningLevel = WarningLevel.Default;
				defaultWarningLevel = CoalesceDefaultWarningFromMemberInfo(mi, miAttr, InitializationContext.Constructor, defaultWarningLevel, BuildContextProvider, _logger);
		...
		private static List<(MemberInfo, List<TAttr>)> GetMembersWithAttributes<TAttr>(Type type, Type targetType) where TAttr : Attribute
		{
			List<(MemberInfo, List<TAttr>)> list = [];

			void TryAddMemberInfo(MemberInfo mi, Type memberType)
			{
				if (memberType == targetType)
				{
					IEnumerable<TAttr> attrs = mi.GetCustomAttributes<TAttr>();
					if (attrs.Any())
					{
						list.Add((mi, attrs.ToList()));
					}
				}
			}
			...
			{
				TryAddMemberInfo(prop, prop.PropertyType);
			}

			foreach (FieldInfo field in type.GetFields(flags))
			{
				TryAddMemberInfo(field, field.FieldType);
			}
