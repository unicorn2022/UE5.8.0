	...
	public class RulesAssembly
	{
		...
		internal RulesAssembly(RulesScope Scope, IReadOnlyList<DirectoryReference> BaseDirs, IReadOnlyList<PluginInfo> Plugins, IReadOnlyDictionary<FileReference, ModuleRulesContext> ModuleFileToContext, IReadOnlyList<FileReference> TargetFiles, FileReference AssemblyFileName, bool bContainsEngineModules, BuildSettingsVersion? DefaultBuildSettings, bool bReadOnly, bool bSkipCompile, bool bForceCompile, RulesAssembly? Parent, ILogger Logger)
		{
			...
			if (CompiledAssembly != null)
			{
				Task.Run(() =>
				{
					//using ITimelineEvent timelineScope = Timeline.ScopeEvent("PrejitRules");
					ParallelQueue queue = new();
					foreach (Type CompiledType in CompiledAssembly.GetTypes())
					{
						if (CompiledType.BaseType == typeof(ModuleRules))
						{
							foreach (ConstructorInfo Constructor in CompiledType.GetConstructors())
							{
								ParameterInfo[] parameters = Constructor.GetParameters();
								if (parameters.Length > 0)
								{
									if (parameters[0].ParameterType == typeof(ReadOnlyTargetRules))
									{
										queue.Enqueue(() => RuntimeHelpers.PrepareMethod(Constructor.MethodHandle));
									}
								}
							}
						}
					...
					queue.Drain();
					//timelineScope.Finish();

					// Write out warnings as well
					foreach (Type CompiledType in CompiledAssembly.GetTypes())
					{
						...
						if (CompiledType.BaseType == typeof(ModuleRules))
						{
							ConstructorInfo? Constructor = CompiledType.GetConstructor([typeof(TargetInfo)]);
							if (Constructor != null)
							{
								FileReference? Location;
								if (!TryGetFileNameFromType(CompiledType, out Location))
		...
		public bool TryGetFileNameFromType(Type ExistingType, [NotNullWhen(true)] out FileReference? File)
		{
			if (ExistingType.Assembly == CompiledAssembly)
			{
				...
				if (NameWithoutTarget.EndsWith("Target", StringComparison.Ordinal))
				{
		...
		private readonly ConcurrentDictionary<string, Type?> _moduleNameToType = [];
		...
		{
			return _moduleNameToType.GetOrAdd(ModuleName, ModuleName =>
			{
		...
		protected TargetRules? CreateTargetRulesInstance(string TypeName, TargetInfo TargetInfo, ILogger Logger, bool IsTestTarget = false, TargetRulesValidationOptions ValidationOptions = TargetRulesValidationOptions.ValidateTargetAndSDK)
		{
			...
			if (!Rules.GlobalDefinitions.Any(x => x.StartsWith("WITH_VERSE_VM=", StringComparison.Ordinal)))
			{
			...
			if (Rules.OptedInModulePlatforms != null)
			{
				...
				foreach (PluginSet set in EnumeratePlugins())
				{
					...
					{
						ReadOnlySpan<char> span = info.File.FullName.AsSpan(Unreal.EngineDirectory.FullName.Length + 1);
					...
					{
						ReadOnlySpan<char> span = info.File.FullName.AsSpan(ProjectDir.FullName.Length + 1);
						if (bloomFilter.MightContainSubString(span, Path.DirectorySeparatorChar))
						{
							mightContain = true;
						}
			...
			if (Rules.LinkType != TargetLinkType.Monolithic && UEBuildPlatform.PlatformRequiresMonolithicBuilds(Rules.Platform))
			{
		...
		private static void ValidateSDKs(TargetRules Rules)
		{
		...
		public TargetRules CreateTargetRules(string TargetName, UnrealTargetPlatform Platform, UnrealTargetConfiguration Configuration, UnrealArchitectures? Architectures, FileReference? ProjectFile, CommandLineArguments? Arguments, ILogger InLogger, bool IsTestTarget = false, TargetRulesValidationOptions ValidationOptions = TargetRulesValidationOptions.ValidateTargetAndSDK, UnrealIntermediateEnvironment IntermediateEnvironment = UnrealIntermediateEnvironment.Default)
		{
			...
			// Suppress TargetRules logging during project gen
			ILogger Logger = ProjectFileGenerator.bGenerateProjectFiles ? NullLogger.Instance : InLogger;
