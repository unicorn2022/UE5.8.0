...
namespace UnrealBuildTool
{
	...
	static class XmlConfig
	{
		...
		{
			string XmlConfigMutexName = GlobalSingleInstanceMutex.GetUniqueMutexForPath("UnrealBuildTool_Mutex_XmlConfig", Unreal.EngineDirectory);
			using IDisposable XmlConfigMutex = SingleInstanceMutex.Acquire(XmlConfigMutexName);
		...
		static XmlSchemaElement CreateSchemaFieldElement(string name, Type type)
		{
			...
			else if (MaybeGetInnerCollectionType(type) is Type genericType)
			{
				XmlSchemaElement itemElement = CreateSchemaFieldElement("Item", genericType);
		...
		private static Type? MaybeGetInnerCollectionType(Type type)
		{
			if (type.IsArray)
			{
				return type.GetElementType();
			}
			if (type.GetInterfaces().FirstOrDefault(x => x.IsGenericType && x.GetGenericTypeDefinition() == typeof(ICollection<>)) is Type collectionType)
			{
			...
			foreach (XmlElement categoryElement in configFile.DocumentElement!.ChildNodes.OfType<XmlElement>())
			{
				...
				if (categoryToFields.TryGetValue(categoryElement.Name, out nameToField))
				{
					foreach (XmlElement keyElement in categoryElement.ChildNodes.OfType<XmlElement>())
					{
						...
						{
							object value;
							if (MaybeGetInnerCollectionType(field.Type) is Type innerType)
							{
								if (field.Type.IsArray)
								{
									string[] stringValues = [.. keyElement.ChildNodes.OfType<XmlElement>().Where(x => x.Name == "Item").Select(x => x.InnerText)];
									Array valueArray = Array.CreateInstance(innerType, stringValues.Length);
									for (int i = 0; i < stringValues.Length; ++i)
									{
										string stringValue = stringValues[i];
										valueArray.SetValue(ParseValue(innerType, stringValue), i);
									}
									value = valueArray;
								}
								else
								{
									throw new NotImplementedException("XmlConfig support for non-array types is not implemented.");
								}
							}
		...
		static bool WriteEnvironmentXml(FileReference location, ILogger logger)
		{
			...
			foreach (DictionaryEntry EnvironmentVariable in Environment.GetEnvironmentVariables())
			{
				...
				if (!key.StartsWith("UnrealBuildTool_", StringComparison.Ordinal) || !key.Contains("__", StringComparison.Ordinal))
				{
					continue;
				}
			...
			using (XmlWriter writer = XmlWriter.Create(output, xmlSettings))
			{
				...
				foreach (KeyValuePair<string, Dictionary<string, string>> CategoryItem in contents)
				{
					...
					foreach (KeyValuePair<string, string> FieldItem in CategoryItem.Value)
					{
						...
						if (FieldItem.Value.Contains(';', StringComparison.Ordinal))
						{
