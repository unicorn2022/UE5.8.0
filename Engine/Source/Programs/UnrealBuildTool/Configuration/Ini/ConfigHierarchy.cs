...
namespace UnrealBuildTool
{
	...
	public class ConfigHierarchySection
	{
		...
		/// <summary>
		/// List of key names in the order they were first added (preserves INI file order)
		/// </summary>
		readonly List<string> KeyOrder = new List<string>();

		/// <summary>
		/// List of (key, value) pairs in the exact order they were added from INI file
		/// Preserves interleaved order of array elements with different keys (e.g., +Steps= and +Steps[Variant]=)
		/// </summary>
		readonly List<(string Key, string Value)> ArrayElementOrder = new List<(string, string)>();
			...
			foreach (ConfigFileSection FileSection in FileSections)
			{
				foreach (ConfigLine Line in FileSection.Lines)
				{
					if (Line.Action == ConfigLineAction.RemoveKey)
					{
						...
						KeyOrder.RemoveAll(k => k.Equals(Line.Key, StringComparison.OrdinalIgnoreCase));
						ArrayElementOrder.RemoveAll(pair => pair.Key.Equals(Line.Key, StringComparison.OrdinalIgnoreCase));
						continue;
					}
					...
					if (KeyToValue.TryGetValue(Line.Key, out Values))
					{
						...
						if (Line.Action == ConfigLineAction.Set)
						{
							...
							// Track order: when Set clears the array, also remove all previous entries for this key
							ArrayElementOrder.RemoveAll(pair => pair.Key.Equals(Line.Key, StringComparison.OrdinalIgnoreCase));
							ArrayElementOrder.Add((Line.Key, Line.Value));
						}
						...
						{
							Values.RemoveAll(x => x.Equals(Line.Value, StringComparison.OrdinalIgnoreCase));
							// Also remove from order tracking
							ArrayElementOrder.RemoveAll(pair => pair.Key.Equals(Line.Key, StringComparison.OrdinalIgnoreCase) && pair.Value.Equals(Line.Value, StringComparison.OrdinalIgnoreCase));
						}
					}
					else
					{
		...
		/// </summary>
		public IEnumerable<string> KeyNames => KeyOrder;
		...
		/// <summary>
		/// Returns array elements in the exact order they were added from INI files.
		/// Preserves interleaving of elements with different keys (e.g., +Steps= and +Steps[Variant]=)
		/// </summary>
		public IReadOnlyList<(string Key, string Value)> GetArrayElementsInOrder()
		{
			return ArrayElementOrder;
		}
	}
	...
	public sealed class ConfigHierarchy : IDisposable
	{
		...
		public static bool TryParse(string Text, out bool Value)
		{
			...
			else if (Text == "0" || Text.Equals("false", StringComparison.OrdinalIgnoreCase))
			{
				Value = false;
				return true;
			}
		...
		{
			if (Text.EndsWith("f", StringComparison.Ordinal))
			{
		...
		{
			string OutString = InString.Replace("{TYPE}", BaseIniName, StringComparison.Ordinal);

			OutString = OutString.Replace("{APPSETTINGS}", Unreal.ApplicationSettingDirectory.FullName, StringComparison.Ordinal);
			OutString = OutString.Replace("{USERSETTINGS}", Unreal.UserSettingDirectory.FullName, StringComparison.Ordinal);

			DirectoryReference? UserDir = Unreal.UserDirectory;
			if (UserDir != null)
			{
				OutString = OutString.Replace("{USER}", UserDir.FullName, StringComparison.Ordinal);
			}

			OutString = OutString.Replace("{CUSTOMCONFIG}", CustomConfig, StringComparison.Ordinal);
		...
		private static string? PerformExpansionReplacements(ConfigLayerExpansion Expansion, string InString)
		{
			...
			if (!InString.Contains(Expansion.Before1, StringComparison.Ordinal) && (Expansion.Before2 == null || !InString.Contains(Expansion.Before2, StringComparison.Ordinal)))
			{
				return null;
			}

			// replace the directory bits
			string OutString = InString.Replace(Expansion.Before1, Expansion.After1, StringComparison.Ordinal);
			if (Expansion.Before2 != null)
			{
				OutString = OutString.Replace(Expansion.Before2, Expansion.After2, StringComparison.Ordinal);
			}
			return OutString;
		}
		...
		private static string PerformFinalExpansions(string InString, string PlatformName, DirectoryReference? ProjectDir)
		{
			...
			string OutString = InString.Replace("{ENGINE}", Unreal.EngineDirectory.FullName, StringComparison.Ordinal);
			OutString = OutString.Replace("{EXTENGINE}", PlatformExtensionEngineConfigDir, StringComparison.Ordinal);
			OutString = OutString.Replace("{PLATFORM}", PlatformName, StringComparison.Ordinal);
			...
			if (ProjectDir != null)
			{
				...
				OutString = OutString.Replace("{PROJECT}", ProjectDir.FullName, StringComparison.Ordinal);
				OutString = OutString.Replace("{EXTPROJECT}", PlatformExtensionProjectConfigDir, StringComparison.Ordinal);
				OutString = OutString.Replace("{RESTRICTEDPROJECT_NFL}", NFLDir.FullName, StringComparison.Ordinal);
				OutString = OutString.Replace("{RESTRICTEDPROJECT_NR}", NRDir.FullName, StringComparison.Ordinal);
				OutString = OutString.Replace("{RESTRICTEDPROJECT_LA}", LADir.FullName, StringComparison.Ordinal);
				OutString = OutString.Replace("{OPT_SUBDIR}", OptionalSubDir, StringComparison.Ordinal);
		...
		public static IEnumerable<FileReference> EnumerateConfigFileLocations(ConfigHierarchyType Type, DirectoryReference? ProjectDir, UnrealTargetPlatform Platform, string CustomConfig, TargetType? IncludePluginsForTargetType)
		{
			...
			{
				bool bHasPlatformTag = Layer.Contains("{PLATFORM}", StringComparison.Ordinal);
				bool bHasProjectTag = Layer.Contains("{PROJECT}", StringComparison.Ordinal);
				bool bHasUserTag = Layer.Contains("{USER}", StringComparison.Ordinal);
				bool bHasCustomConfigTag = Layer.Contains("{CUSTOMCONFIG}", StringComparison.Ordinal);
				...
				if (Layer.Contains("{ENGINE}", StringComparison.Ordinal) || Layer.Contains("{PROJECT}", StringComparison.Ordinal))
				{
		...
		public static Dictionary<string, string> GetStructKeyValuePairs(string Input)
		{
			...
			foreach (string AProp in Props)
			{
				...
				int Equals = Prop.IndexOf('=', StringComparison.Ordinal);
				// we must have one
				if (Equals == -1)
				{
					continue;
