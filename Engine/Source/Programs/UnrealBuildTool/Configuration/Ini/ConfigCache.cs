	...
	public static class ConfigCache
	{
		...
		class ConfigHierarchyKey
		{
			...
			public override int GetHashCode()
			{
				...
				Hash = (Hash * 31) + CustomConfig.GetHashCode(StringComparison.Ordinal);
				foreach (string OverrideString in OverrideStrings)
				{
					Hash = (Hash * 31) + OverrideString.GetHashCode(StringComparison.Ordinal);
				}
		...
		/// </summary>
		static readonly ConcurrentDictionary<FileReference, Lazy<ConfigFile?>> LocationToConfigFile = [];

		/// <summary>
		/// Cache of config hierarchies by project
		/// </summary>
		static readonly ConcurrentDictionary<ConfigHierarchyKey, Lazy<ConfigHierarchy>> HierarchyKeyToHierarchy = [];

		/// <summary>
		/// Cache of config fields by type
		/// </summary>
		static readonly ConcurrentDictionary<Type, Lazy<List<ConfigMember>>> TypeToConfigMembers = [];
			...
			string CustomConfig, CommandLineArguments CmdLineArgs, DirectoryReference? HotfixDir = null, TargetType? IncludePluginsForTargetType = null)
		{
			...
			{
				if (CmdLineArg.StartsWith(IniConfigArgPrefix, StringComparison.OrdinalIgnoreCase))
				{
					OverrideStrings.Add(CmdLineArg.Substring(IniConfigArgPrefix.Length));
				}
				if (CmdLineArg.StartsWith(CustomConfigPrefix, StringComparison.OrdinalIgnoreCase))
				{
					CustomConfig = CmdLineArg.Substring(CustomConfigPrefix.Length);
				}
				if (CmdLineArg.Equals("-dumpiniloads", StringComparison.OrdinalIgnoreCase))
				{
					bDumpIniLoads = true;
				}
			}
			...
			Logger.LogInformation("Requested Hierarchy: {Type},{ProjectDir},{Platform},{CustomConfig}", Type, ProjectDir, Platform, CustomConfig);
			...
			, DirectoryReference? HotfixDir, TargetType? IncludePluginsForTargetType, List<string> OverrideStrings)
		{
			...
			{
				ConfigFile? File;
				Logger.LogInformation("Trying to load: {IniPath}", IniFileName);
				...
				if (HotfixDir != null)
				{
					...
					{
						Logger.LogInformation("    Hotfixed with '{HotfixPath}'", HotfixFile);
						Files.Add(File);
			...
			foreach (string OverrideString in OverrideStrings)
			{
				...
				Logger.LogInformation("Trying to load override: {OverridePath}", OverrideFile);
			}
		...
		static List<ConfigMember> FindConfigMembersForType(Type TargetObjectType)
		{
			return TypeToConfigMembers.GetOrAdd(TargetObjectType, static TargetObjectType => new Lazy<List<ConfigMember>>(() =>
			{
				...
				{
					ProcessConfigTypeMember<FieldInfo>(FieldInfo, Members, (FieldInfo, Attribute) => new ConfigField(FieldInfo, Attribute));
				}
				...
				{
					ProcessConfigTypeMember<PropertyInfo>(PropertyInfo, Members, (PropertyInfo, Attribute) => new ConfigProperty(PropertyInfo, Attribute));
				}
				return Members;
			})).Value;
		}

		static void ProcessConfigTypeMember<MEMBER>(MEMBER MemberInfo, List<ConfigMember> Members, Func<MEMBER, ConfigFileAttribute, ConfigMember> CreateConfigMember)
		...
		public static bool WriteSettingToConfigFile(FileReference ConfigFile, ConfigDefaultUpdateType UpdateType, string Section, string Key, string Value, ILogger Logger)
		{
			...
			{
				if (Lines[Index].Trim().Equals(SectionString, StringComparison.OrdinalIgnoreCase))
				{
					SectionIndex = Index;
					break;
				}
			...
			for (int Index = SectionIndex + 1; Index < Lines.Count; Index++)
			{
				...
				else if (Lines[Index].StartsWith("[", StringComparison.Ordinal))
				{
