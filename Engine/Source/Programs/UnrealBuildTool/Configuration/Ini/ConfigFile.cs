...
namespace UnrealBuildTool
{
	...
	public class ConfigFileSection
	{
		...
		public bool TryGetLine(string Name, [NotNullWhen(true)] out ConfigLine? OutLine)
		{
			...
			{
				if (Line.Key.Equals(Name, StringComparison.Ordinal))
				{
					OutLine = Line;
					return true;
				}
	...
	public class ConfigFile
	{
				...
				if (Remap.TryGetValue(Key, out Remapped))
				{
					if (!WarnedKeys.Contains(Key))
					{
						...
						Log.Logger.LogWarning(
							"DEPRECATION: '{Key}', {Context}, has been deprecated. Using '{Remapped}' instead. It is recommended you update your .ini files as soon as possible, and replace {Key} with {Remapped}",
							Key,
							Context,
							Remapped,
							Key,
		...
		static ConfigFile()
		{
			...
			foreach (KeyValuePair<string, ConfigFileSection> Pair in Sections)
			{
				...
				if (Pair.Key.Equals("SectionNameRemap", StringComparison.OrdinalIgnoreCase))
				{
				...
				else
				{
					...
					Dictionary<string, string> KeyRemap = new(StringComparer.OrdinalIgnoreCase);
		...
		public ConfigFile(string IniText, ConfigLineAction DefaultAction = ConfigLineAction.Set)
		{
			...
			foreach (string Setting in SettingLines)
			{
				...
				string SectionName = Setting.Remove(Setting.IndexOf(':', StringComparison.Ordinal)).Trim(new char[] { '[', ']' });
				// lookup remaps
				...
				if (SectionName.Length > 0)
				{
					...
					if (CurrentSection != null)
					{
						string IniKeyValue = Setting.Substring(Setting.IndexOf(':', StringComparison.Ordinal) + 1);
		...
		static bool TryAddConfigLine(ConfigFileSection Section, Dictionary<string, string>? KeyRemap, string Filename, string Line, int StartIdx, int EndIdx, ConfigLineAction DefaultAction, Dictionary<string, ConfigFileSection> Sections)
		{
			...
			if (!NewKey.Equals(Key, StringComparison.Ordinal) && NewKey.Contains(':', StringComparison.Ordinal))
			{
				string SectionName = NewKey.Substring(0, NewKey.IndexOf(':', StringComparison.Ordinal));
				...
				string KeyName = NewKey.Substring(NewKey.IndexOf(':', StringComparison.Ordinal) + 1);
