...
namespace UnrealBuildTool
{
	...
	public class UnrealArchitectureConfig
	{
		...
		{
			this.Mode = Mode;
			AllSupportedArchitectures = new UnrealArchitectures(SupportedArchitectures.ToList());
		}
	}
	...
	abstract class UEBuildPlatform
	{
		...
		private static readonly string[] UATProjectParams = { "-project=", "-scriptsforproject=" };
		private static void InitializePerPlatformSDKs(string[] Args, bool bArgumentsAreForUBT, ILogger Logger)
		{
			...
			if (bArgumentsAreForUBT == false)
			{
				ProjectFiles = Args
						...
						.Select(x => NativeProjects.FindProjectFile(x.Substring(x.IndexOf('=', StringComparison.Ordinal) + 1), Logger))
		...
		public static bool IsBuildProductName(string FileName, int Index, int Count, string[] NamePrefixes, string[] NameSuffixes, string Extension)
		{
			...
			if (Count > Extension.Length && String.Compare(FileName, Index + Count - Extension.Length, Extension, 0, Extension.Length, StringComparison.OrdinalIgnoreCase) == 0)
			{
		...
		public static bool IsBuildProductName(string FileName, int Index, int Count, string[] NamePrefixes, string[] NameSuffixes)
		{
			...
			{
				if (Count >= NamePrefix.Length && String.Compare(FileName, Index, NamePrefix, 0, NamePrefix.Length, StringComparison.OrdinalIgnoreCase) == 0)
				{
					int MinIdx = Index + NamePrefix.Length;
					foreach (string NameSuffix in NameSuffixes)
					{
						int MaxIdx = Index + Count - NameSuffix.Length;
						if (MaxIdx >= MinIdx && String.Compare(FileName, MaxIdx, NameSuffix, 0, NameSuffix.Length, StringComparison.OrdinalIgnoreCase) == 0)
						{
		...
		/// <summary>
		/// Returns a detailed reason string explaining why a platform is not available to build.
		/// </summary>
		public static string GetPlatformUnavailableReason(UnrealTargetPlatform Platform)
		{
			if (!BuildPlatformDictionary.ContainsKey(Platform))
			{
				return $"Platform {Platform} is not registered. Check that the necessary platform support files exist (DataDrivenPlatformInfo.ini, SDK.json, etc).";
			}

			UEBuildPlatformSDK? Sdk = UEBuildPlatformSDK.GetSDKForPlatform(Platform.ToString());
			if (Sdk == null)
			{
				return $"Platform {Platform} is not a valid platform to build. No SDK is registered for this platform.";
			}

			StringBuilder Message = new();
			Message.AppendLine($"Platform {Platform} is not a valid platform to build. SDK validation failed:");
			...
			foreach (SDKDescriptor Desc in SdkInfo.Sdks)
			{
				...
				if (String.Equals(Desc.Name, "AutoSdk", StringComparison.OrdinalIgnoreCase))
				{
					DisplayName = "AutoSDK";
					if (UEBuildPlatformSDK.TryGetHostPlatformAutoSDKDir(out DirectoryReference? HostAutoSdkDir))
					{
						LocationHint = $" at {DirectoryReference.Combine(HostAutoSdkDir, Sdk.GetAutoSDKPlatformName())}";
					}
					else if (ReadOnlyBuildVersion.Current.IsLicenseeVersion
						|| !DirectoryReference.Exists(DirectoryReference.Combine(UnrealBuildBase.Unreal.EngineDirectory, "Restricted", "NotForLicensees")))
					{
						// Licensees don't use AutoSDK -- skip to avoid confusion about UE_SDKS_ROOT.
						continue;
					}
					else
					{
						LocationHint = " (UE_SDKS_ROOT is not set)";
					}
				}

				if (Desc.Current != null)
				{
					Message.AppendLine($"  {DisplayName}: found version {Desc.Current}{LocationHint}, required version {Desc.Min}.");
				}
				else
				{
					string Required = Desc.Min != null ? $" Required version {Desc.Min}." : "";
					Message.AppendLine($"  {DisplayName}: not found{LocationHint}.{Required}");
				}
			}
		...
		{
			// Avoid allocations by using yield (one allocation for enumerable still created)
			foreach (KeyValuePair<UnrealPlatformGroup, List<UnrealTargetPlatform>> kvp in PlatformGroupDictionary)
			{
				if (kvp.Value.Contains(Platform))
				{
		...
		/// </summary>
		[Obsolete("Deprecated in UE5.8. Replace with System.IO.Path.PathSeparator")]
		...
		/// <summary>
		/// Provides a set of SN-DBS tool templates for builds on this platform. The dictionary key is the name of the tool, such as "clang.exe", and the value is the template contents.
		/// </summary>
		/// <returns>Dictionary of tool templates or null if there are none.</returns>
		public virtual IReadOnlyDictionary<string, string>? GetSNDBSToolTemplates()
		{
			return null;
		}
...
#pragma warning disable CA2254 // Template should be a static expression - message is the static template here
...
#pragma warning restore CA2254
		...
		/// <returns></returns>
		public static bool PlatformRequiresMonolithicBuilds(UnrealTargetPlatform InPlatform)
		{
