...
#pragma warning disable CA1069 // Enums should not have duplicate values - having access to Latest is useful
		/// <summary>
...
#pragma warning restore CA1069
	...
	public class PluginDescriptor
	{
		...
		/// <summary>
		/// Whether this plugin has SceneGraph Animation enabled, which impacts the generated Verse Asset Digest.
		/// </summary>
		public bool? bEnableSceneGraphAnimation = false;
		...
		/// <summary>
		/// If true, the GameFeaturePlugin this is on should have no content other than the GameFeatureData asset.
		/// </summary>
		public bool bNoGameFeatureContent;
		...
		/// <summary>
		/// Read-only access to the raw JSON object for this plugin descriptor.
		/// Callers can use this to read non-standard fields that are not modelled by this class.
		/// </summary>
		public JsonObject RawJson => CachedJson;
		...
		public PluginDescriptor(JsonObject RawObject, FileReference PluginPath)
		{
			...
			if (Category != null && Category.Length >= 2 && Category.StartsWith("\"", StringComparison.Ordinal) && Category.EndsWith("\"", StringComparison.Ordinal))
			{
			...
			bool bPluginEnableSceneGraphAnimation;
			if (RawObject.TryGetBoolField("EnableSceneGraphAnimation", out bPluginEnableSceneGraphAnimation))
			{
				bEnableSceneGraphAnimation = bPluginEnableSceneGraphAnimation;
			}
			...
			RawObject.TryGetBoolField("NoGameFeatureContent", out bNoGameFeatureContent);
		...
		public void Write(JsonWriter Writer)
		{
			...
			{
				Writer.WriteValue("NoGameFeatureContent", bNoGameFeatureContent);
			}
			...
			if (Modules != null)
			{
				...
				if (bIsPluginExtension)
				{
					foreach (ModuleDescriptor ChildModule in Modules)
					{
						...
						{
							// The order that child plugins are merged into the parent is undefined - there is no hierarchy. Only the PlatformAllowList and PlatformDenyList are currently merged into the parent
							// Having an explicity-empty list here suggests someone is trying to create a hierarchy and may get caught out if their module declaration also includes TargetAllowList, ProgramAllowList etc. as these properties will be ignored
