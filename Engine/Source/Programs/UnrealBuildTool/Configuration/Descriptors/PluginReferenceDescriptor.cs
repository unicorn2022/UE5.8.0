...
namespace UnrealBuildTool
{
	...
	public class PluginReferenceDescriptor
	{
		...
		/// <summary>
		/// Overrides the loading phase in uplugin so that a plugin can be explicitly loaded for specified targets.
		/// </summary>
		public TargetType[]? TargetsToExplicitlyLoad;
...
#pragma warning disable CA1054 // URI parameters should not be strings - URL is not processed, so does not need to be modelled or validated
...
#pragma warning restore CA1054
		...
		public void Write(JsonWriter Writer)
		{
			...
			if (TargetsToExplicitlyLoad != null && TargetsToExplicitlyLoad.Length > 0)
			{
				Writer.WriteEnumArrayField("TargetsToExplicitlyLoad", TargetsToExplicitlyLoad);
			}
		...
		private JsonObject ToJsonObject()
		{
			...
			if (TargetsToExplicitlyLoad != null && TargetsToExplicitlyLoad.Length > 0)
			{
				PluginReferenceObject.AddOrSetFieldValue("TargetsToExplicitlyLoad", TargetsToExplicitlyLoad);
			}
			if (SupportedTargetPlatforms != null && SupportedTargetPlatforms.Length > 0)
			{
		...
		private void VerifyPlatformNames(string[]? PlatformNames)
		{
			if (PlatformNames != null)
			{
				...
				{
					if (!UnrealTargetPlatform.TryParse(PlatformName, out _))
					{
		...
		{
			string? PluginReferenceName = null;
			try
			{
				PluginReferenceName = RawObject.GetStringField("Name");

				PluginReferenceDescriptor Descriptor = new PluginReferenceDescriptor(PluginReferenceName, null, RawObject.GetBoolField("Enabled"));
				...
				if (Descriptor.bEnabled)
				{
					...
					Descriptor.TargetAllowList = RawObject.GetEnumArrayFieldWithDeprecatedFallback<TargetType>("TargetAllowList", "WhitelistTargets");
					Descriptor.TargetDenyList = RawObject.GetEnumArrayFieldWithDeprecatedFallback<TargetType>("TargetDenyList", "BlacklistTargets");
					Descriptor.TargetConfigurationAllowList = RawObject.GetEnumArrayFieldWithDeprecatedFallback<UnrealTargetConfiguration>("TargetConfigurationAllowList", "WhitelistTargetConfigurations");
					Descriptor.TargetConfigurationDenyList = RawObject.GetEnumArrayFieldWithDeprecatedFallback<UnrealTargetConfiguration>("TargetConfigurationDenyList", "BlacklistTargetConfigurations");

					RawObject.TryGetEnumArrayField<TargetType>("TargetsToExplicitlyLoad", out Descriptor.TargetsToExplicitlyLoad);
			...
			{
				ExceptionUtils.AddContext(Ex, "while parsing plugin reference descriptor '{0}'", PluginReferenceName ?? "<unknown>");
				throw;
			}
		}
